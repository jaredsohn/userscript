// ==UserScript==
// @name          Google Reader + Baidu Cang
// @description   Share Google Reader item to Baidu Cang
// @namespace     *
...
// Google Reader + Baidu Cang
// Author TonySR (TonySR@126.com)
// Adapted from Google Reader + BlueDot
// 2007-8-31 Version 0.1

// This script can't help user login into the Baidu Cang,
// so you should do it at first.
// User can click the button 'share with cang' in the reader
// to share the info piece.
// You can also clik the keyword 'c' in the content area
// and this will also paste the selected words.
...
// Use 'c' shortcut to share with baidu cang
var cangCode   = 99; // "c"
...
document.addEventListener("keypress", postBookmark, true);
...
// some methods learn from 'Google Reader + BlueDot'
	var parent = false;
	bookmarkStar=false;
	...
	for (i=0;i<parent.getElementsByTagName('SPAN').length;i++) {
		if (parent.getElementsByTagName('SPAN')[i].innerHTML == 'Share with Cang') {
...
function saveBookmark(event){
	...
	GM_xmlhttpRequest({
		...
		data: 'ct=5&iu='+encodeURIComponent(urlinput.value)+'&st=0&dc='+notes+'&it='+title+'&tn='+labels+'&_=',
...
function countWord(event) {
	...
	var msg = notes;
	notesdesc.innerHTML="Note: remain " + (140 - countMsgWord(msg)) + " characters";
}
