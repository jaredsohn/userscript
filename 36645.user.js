// ==UserScript==
// @name           SexGames cash protector
// @namespace      SexGames
// @description    Deposit  money as soon as you get some.
// @include        http://apps.facebook.com/playsexgames/accounts.php*
// @include        http://apps.new.facebook.com/playsexgames/accounts.php*
// ==/UserScript==
...
try {
	...
	newdiv = document.getElementById('app28645543422_header').appendChild(newdiv);
//	newdiv = document.body.appendChild(newdiv);
	var frm = document.getElementById("app28645543422_deposit").getElementsByTagName("form")[0];
	var cash = frm.getElementsByTagName("input")[11].value;
	var retrait = /You took out/.test(document.getElementById('app28645543422_content').innerHTML);
	if (cash > 1000 && (!retrait)) {
		...
		if (retrait) {
			...
			var mtch = document.getElementById("app28645543422_status").innerHTML.match(ptrn);
