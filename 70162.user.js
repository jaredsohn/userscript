...
// @namespace      myspace_mafia_wars_autoplayer
// @include http://profile.myspace.com/Modules/Applications/Pages/Canvas.aspx?appId=110226&appParams=%7B%22xw_controller%22%3A%22index%22%2C%22xw_action%22%3A%22view%22%7D
// @include        http://api.msappspace.com/apprendering/110226*
// @version        1.0.0
// @build          2
// ==/UserScript==
...
var SCRIPT = {
  ...
  appNo: '10979261223',
  ajaxPage: 'inner2',
  presentationurl: 'http://userscripts.org/scripts/source/70162',
  url: 'http://userscripts.org/scripts/source/70162.user.js',
  metadata: 'http://userscripts.org/scripts/source/70162.meta.js',
...
if (/myspace.com/.test(window.location.href)) {
...
if (/api.msappspace.com/.test(window.location.href)) {
  var docUrl = document.location.href;
  if (docUrl.match(/ownerid=(\w+)/)) {
    GMSTORAGE_PATH = GMSTORAGE_PATH + RegExp.$1;
  }
}
...
function checkLoadIframe() {
  ...
  var iFrameCanvas = document.getElementById('apppanel_110226_canvas');
  if (iFrameCanvas) {
    setFBParams();
...
//alert(initialized);
//alert(checkInPublishPopup());
//alert(checkLoadIframe());


//  api.msappspace.com/apprendering/110226


if (!initialized && !checkInPublishPopup() && !checkLoadIframe() &&
    (/inthemafia/.test(document.referrer.match) || /api.msappspace.com/.test(window.location.href))) {  
  // Hide zbar
  //var iFrameZBar = document.getElementById('zbar');
  //if(iFrameZBar){
  //      iFrameZBar.style.display = 'none';
  //}
...
function createDisplayTab() {
  ...
  // Set myspace account to window title
  item = makeElement('div', list);
  lhs = makeElement('div', item, {'class':'lhs'});
  rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  id = 'fbwindowtitle';
  title = 'Set window title to name on myspace account';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' Set window title to name on myspace account'));
...
function createMafiaTab() {
  ...
  // War - autowar targets
  title = 'Enter opponents myspace ID';
  id = 'autoWarTargetList';
  var warList = makeElement('textarea', rhs, {'style':'width: 12em; height: 6em;', 'id':id, 'title':'Enter each myspace ID on a separate line.'});
...
function createStaminaTab() {
  ...
  makeElement('br', rhs);
  makeElement('font', rhs, {'style':'font-size: 10px;'}).appendChild(document.createTextNode('Enter each myspace ID on a separate line.'));
...
function validateStaminaTab() {
  ...
  switch (s.staminaSpendHow) {
    ...
    case STAMINA_HOW_AUTOHITLIST: // Place hitlist bounties
      ...
      if (!list[0]) {
        alert('Enter the myspace ID of at least one opponent to hitlist.');
        return {};
      }
      break;
...
function refreshMWAPCSS() {
  try {
    ...
    var newCSS = ' #mainDiv {overflow: auto; width: 100%; height: 1000px; position: absolute;}'   +
                 (isGMChecked('leftAlign') ? ' #mw_city_wrapper {margin:0; float: left}' : '')   +
                 // Hide the Zynga bar, progress bar, email bar, sms link
                 ' #mwapHide, #zbar, #mw_zbar iframe, #setup_progress_bar, .fb_email_prof_header, .mw_sms '  +
                 // Hide action boxes
                 ...
                 '#settingsBox .tabcontent{display:blockau;height:420px;top:40px;width:600px}' +
...
function customizeMasthead() {
  ...
  // Document title
  document.title = "Mafia Wars on myspace";
...
function customizeProfile() {
  ...
  if (statsDiv) {
    ...
    if (tabElt){
      ...
      // myspace profile
      makeElement('a', statsDiv, {'href':'http://www.myspace.com/profile.php?id=' + remoteuserid}).appendChild(document.createTextNode('myspace Profile'));
      statsDiv.appendChild(document.createTextNode(' | '));

      // Add as myspace friend
      makeElement('a', statsDiv, {'href':'http://www.myspace.com/addfriend.php?id=' + remoteuserid}).appendChild(document.createTextNode('Add as Friend'));
...
function debugDumpSettings() {
  ...
  if (GM_getValue('language') != 'en') {
    ...
    addToLog('warning Icon', 'Unfortunately, only the English version of the game is fully supported. If you experience problems, set your myspace language to English and try again.');
  }
  ...
  DEBUG('[code]>  >  >  >  >  BEGIN SETTINGS DUMP  <  <  <  <  <<br>' +
        ...
        'Set window title to name on myspace account: <strong>' + showIfUnchecked(GM_getValue('fbwindowtitle')) + '</strong><br>' +
        '---------------------Mafia Tab--------------------<br>' +
...
function loadHome() {
  document.location = 'http://api.msappspace.com/apprendering/110226/canvas/01072010081501/1_0_0/render.app#TopAnc';
...
function handleResponse(responseDetails, action, context) {
  ...
  if (responseDetails.responseText.indexOf('Login to myspace') != -1) {
    ...
    addToLog('warning Icon', '<span class="warn">Or is this myspace user logged in on another computer?</span>');
    return;
  }
