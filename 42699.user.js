// ==UserScript==
// @name           Nettbys profildesignere 0.0.2
// @namespace      Nettbys profildesignere 0.0.2
...
//document.title='nettbys profildesignere by mujaff ©'
//}
...
links[i].innerHTML="Beste stedet i verden";
links[i].style.color="#9400D3";
sted++;}
if (sted==2) break;}
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Send brev") {
  links[i].innerHTML="skriv et brev til denne borgeren";
  links[i].style.color="#4682B4"; {
  }
 }
}
for(i=0;i<links.length;i++) {
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Bli venn med") {
  links[i].innerHTML=" Jeg vil bli venn med han eller hun";
  links[i].style.color="#FF6347"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Skriv en hilsen") {
  links[i].innerHTML="Gjesteboka der folk kan skrive en hilsen";
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Postkasse") {
  links[i].innerHTML="Postkassa mi";
  links[i].style.color="#ff0000"; {
  }
 }
}
...
var s="";
s=nick[i].innerHTML;
if(s.substr(0,12)=='mujaff') {
alert(nick[i].innerHTML)
alert(nick[i].outerHTML);

nick[i].outerHTML='MUJAFF';
break;}}
...
akt.innerHTML = akt.innerHTML + '<div id="En borger du kansje liker"></div>';
}
}
//CALADORION ©
for (var c = 0; c < window.document.getElementsByTagName('div').length; c++){
...
anchor.setAttribute( 'href', 'http://www.nettby.no/community/members.php?community_id=1058604' );
anchor.appendChild( document.createTextNode( 'Profildesignere på nettby' ) );
...
function poeng(e){try{e.innerHTML=e.innerHTML.replace(/Poeng i dag/,"Poeng jeg har fått i dag").replace(/(\d*)\/20\)/,"--0--)").replace(/Poeng totalt/,"Jeg har totalt").replace(/\(\d*\/2000\)/,"(2001/2000)").replace(/Bling-O-Meter:/,"PROFIL-DESIGN-BLING!:");
...
{switch(d.innerHTML){case 'Din aktivitet':case 'søk på borgere':case 'Inviter en venn til nettby':case '<span>Sist pålogget </span>':case 'Sist påloggede i byen vår':case 'Informasjon':d.className="rtl";break;case 'Aktivitet ':poeng(window.document.getElementsByTagName('div')[i+1]);
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="mujaff") {
  links[i].innerHTML="lederen får profildesignerne";
...
for(q=0;q<innom.length;q++){
	...
	if(innom[q].innerHTML=="Sist innom")
		{innom[q].innerHTML="Disse borgerne var her før deg:";
		break;
	}
}

//Setter status (Jeg er på nettby)
var imON=window.document.getElementsByTagName('div');
for(q=0;q<imON.length;q++){
	if(imON[q].innerHTML=="Jeg er på nettby")
