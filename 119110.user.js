// ==UserScript==
// @name          zeleaf VBulletin Rep-Giver
// @namespace     http://userscripts.org/scripts/show/65502
// @include       http://*.zeleaf.com/usercp.php
// @version       1.22
// @dtversion     11051922
// @timestamp     1305831309433
// @description   (zeleaf Forum) automatically get (a|some) reputation(s) giver's link 
// @author        phiyo aka mikoo (http://userscripts.org/users/prhiyo)
//
// zeleaf VBulletin Rep-Giver
// Mod :: mikoo
...
gvar.sversion = 'R' + '22';
gvar.scriptMeta = {
  timestamp: 1305831309433 // version.timestamp
...
function init(){
  ...
  gvar.codeName = 'KVRG';
  gvar.titleName = 'zeleaf vBulletin Reputation Giver';
  
  gvar.uri = {
      'avatar' : 'http://'+'static.zeleaf.com/customavatars',
      'user_link' : 'http://'+'www.zeleaf.com/member.php?u=',
      'rep_link' : 'http://'+'www.zeleaf.com/reputation.php?p=',
      'cendol_img' : 'http://'+'static.zeleaf.com/images/reputation/',
  ...
  gvar.f = ['uid','zeleaf_id','tag_id','location','join_date','total_post','avatar_url','rep_count','rep_filename','rep_rank_title'];
...
function start_Main() {
    ...
    if( gvar._SIMULATE_ ){
      ...
      // selected zeleafer just to simulate error
      ...
      // selected zeleafer just to simulate error from (killua)
...
function parseIt(page){
  ...
  var get_zeleaf_id = function(x){
      var subC = x.match(/<h1>(.+)/i), ret, cucok, b; 
      if(subC) {
          b = subC[1].indexOf('<img');
          subC = clearTag ( subC[1].substring(0,b) );
          cucok = subC.match(/(.+)(\[\+\])?/);
          if(cucok) return cucok[1]+''+(isDefined(cucok[2]) ? cucok[2] : '');
      }else{
          return '';
      }
  };
  ...
  if(ret) result[gvar.f[1]] = get_zeleaf_id(page);
...
function createSender(data, x){
  ...
  data["zeleaf_id"] = filterChar( data["zeleaf_id"] );
  var is_donat = ( /\[\$\]$/.test(data["zeleaf_id"]) ? '&nbsp;<b style="color:red;">[$]</b>':'');
  if(is_donat.length>0) data["zeleaf_id"]=data["zeleaf_id"].replace(/(\[\$\])$/,'');
  
  //overflow:wrap;  //zeleaf Donator
  var inner = ''
  ...
  +'<div><b>'+( data["zeleaf_id"] )+'</b>'+(is_donat)+'</div>'
  +'<div>'+data["tag_id"]+'</div>'
  ...
  // last act, attaching..  
  var tgt = $D("_ucnd"+x);
  tgt.innerHTML = inner + '<a class="smallfont" href="' + gvar.uri['user_link'] + data["uid"] +'"><b>' + data["zeleaf_id"] + '</b></a>'
