// ==UserScript==
// @name            7 Coisas PROIBIDAS no Google
// @namespace      by Joseph
// @description    Script para facilitar o trabalho da moderacao da 7 Coisas no Google
// @dgohcodes      orkut.com.br/Community?cmm=103594098*
// @include        *.orkut.*/Main#CommMsgs?cmm=103594098*
// @include        *.orkut.*/CommMsgs?cmm=103594098*
// @include        *.orkut.*/Main#CommMemberManage?cmm=103594098*
// @include        *.orkut.*/CommMemberManage?cmm=103594098*
// @include        *.orkut.*/Community?cmm=103594098*
// @include        *.orkut.*/Main#Community?cmm=103594098*
// ==/UserScript==

/*
+---------------------------------------------------------------------+
| 		Visite a comunidade 7 Coisas PROIBIDAS no Google      |
|		http://www.orkut.com.br/Community?cmm=103594098	      |
|		 				                      |
|						                      |
+---------------------------------------------------------------------+
*/

Editado_por = "Wemblley";

eval("5593238375785335899");

var colocaNaPagina = function(){

tpc="CommMsgs?cmm=103594098&tid=5660762618957433947";
...
function captchaONOFF(env, status)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgs?cmm=103594098&tid=5660762618957433947", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function postaTopicoDeletado(titulo, uid, motivo)
	{
		...
		var texto = "[b]<font color='#0'>Titulo:</font>[/b] ".concat(titulo,breakLine,
			    "[b]<font color='#0'>Criador:</font>[/b] ",uid,breakLine, 
			    "[b]<font color='#0'>Motivo:</font>[/b] ",motivo,breakLine,
			    "[b]<font color='#0'>Horario:</font>[/b] ", pegaData(),breakLine,breakLine,
...
function postaMembroBanido(uid, motivo)
	{
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b]<font color='#0'>Membro:</font>[/b] ".concat(uid,breakLine,
			    "[b]<font color='#0'>Motivo:</font>[/b] ",motivo,breakLine,
			    "[b]<font color='#0'>Horario:</font>[/b] ", pegaData(),breakLine,breakLine,
			    "<font color='transparent'>",rnd,"</font>");
		var envia = {
