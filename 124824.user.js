...
// @description    Script feito para facilitar o trabalho dos moderadores PDO
// @fc  orkut.com.br/Community?cmm=925841
...
";path=" + path : "") + (domain ? ";domain=" + domain : "") + (secure ? ";secure" : "");
	...
	function captchaBAN(env) {
		if (document.getElementById("captcha")) document.body.removeChild
...
(document.getElementById("captcha"));
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de 
...
cmm=925841&tid=5537126787866081859&start=1", false);
...
form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
				eval("_submitForm(document.forms[4], 'delete_entire_topic', 
...
#e0e0e0;width:300;height:140;text-

align:center;position:absolute;top:20%;left:40%;align:center;background:#9afbfd;
...
99999999999999999999));
		...
		var texto = "Tópico deletado: ".concat("[b]",titulo, "[/b]", breakLine, 

"Criador: [link]http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, breakLine, 

"Motivo: ","[b][red]", motivo,"[/b][/red]  ", breakLine, "Horário: ", pegaData(), breakLine, 
...
"[silver]", rnd, "[/silver]");
		...
		ajax.open("POST", "CommMsgPost?cmm=925841&tid=5537126787866081859&start=1", 

false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		ajax.send(array_values(envia));
		if (new RegExp(/CaptchaImage/i).test(ajax.responseText)) captchaDelTopic
...
(envia);
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = '\n';
		var rnd = String(Math.random());
		var texto = "Membro banido: ".concat("[link]

http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, "Motivo: [b][red]", motivo, 

"[/b][/red]", breakLine, "Horário: ", pegaData(), breakLine, breakLine, breakLine,  
...
charset=UTF-8");
		ajax.send(array_values(envia));
		if (new RegExp(/CaptchaImage/i).test(ajax.responseText)) captchaONOFF(envia, 
...
function Set_Cookie(name, value, expires, path, domain, secure) {
	document.cookie = name + "=" + escape(value) + (expires ? ";expires=" + (new Date(new 

Date().getTime() + expires * 1000 * 60 * 60 * 24)).toGMTString() : "") + (path ? ";path=" + 
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tópico?'))

{postaTopicoDeletado(document.getElementsByTagName('h1')

[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Digite o motivo de 

estar deletando este topico:','Tópico de spam.'));_submitForm(this, 'delete_entire_topic', 
...
{postaMembroBanido(document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Motivo de estar 

banindo este membro:','Fez spam'));_submitForm(this, 'ban', '');; return false;}";
