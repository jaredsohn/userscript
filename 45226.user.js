// ==UserScript==
// @name           AE Project Script 3
// @namespace      tom.woodall@hotmail.co.uk
// @include        http://ae-route-66.webs.com/
// @include        
...
(location.search.indexOf('economy')!=-1)
			return 'economy';
...
(location.search.indexOf('trade')!=-1)
			return 'trade';
...
(location.search.indexOf('aepconfig')!=-1)
      return 'aepConfig';
		return 'account';
	}	else if (location.href.indexOf('guild.aspx')!=-1) {
    if 

(location.search.indexOf('creategroup')!=-1)
      return 'createGroup';
...
(location.href.indexOf('board.aspx')!=-1)
		return 'board';
	else if (location.href.indexOf('messages.aspx')!=-1)
		return 'messages';
	else if 

(location.href.indexOf('ranks.aspx')!=-1)
		return 'ranks';
...
= { id: 1, className: 1, title: 1, type: 1, checked: 1 };
  var id = opt.id;
  var n = $(id);
  if (!n) {
...
attr("after");
 ...
 if (parent) {
  ...
  var text = attr("text");
  if ("undefined" != typeof text) n.textContent 
...
interpretForumLogin(html) {
	if (isForumLogin(html))
		return "<span style='color:red;'>Database access<br /><a href=\""+databaseUrl+"ucp.php\" 

target=\"_blank\">&raquo; Please login &raquo;</a><br /> or ask administrators</span>";
	else return html;
}
...
function getVars() {
  var listVars = GM_getValue('AEP_listvars', 
...
GM_getValue('AEP_name_'+serverName, '');
...
img_alarm = GM_getResourceURL("alarm");
...
profileTable = document.evaluate("//table[contains(.,'Player #')]/tbody", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
  var fullName = 

profileTable.childNodes[1].firstChild.innerHTML;
...
(GM_getValue('AEP_gameID_'+serverName, '') == '' || (GM_getValue('AEP_name_'+serverName, '') == '' || GM_getValue('AEP_guild_'+serverName, '') == '') ) {
...
var ads = document.evaluate("//table[starts-with(., 'Advertising')]", document, null, 
...
responseText = responseDetails.responseText;
  if (isForumLogin(responseText)) {
    var updateNotification = node({html: '<u>AEProject Script</u><br 

/>'+interpretForumLogin(responseText), className:'saveNotification', 
...
window.addEventListener("unload", saveVars, false);==/UserScript==