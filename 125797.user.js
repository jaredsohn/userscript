...
// @namespace http://www.dptr1988.com/

// @description Allows encryption of alliance chat in pardus.at

// @include http://chat.pardus.at/chattext.php?channel=ally*
// @include https://chat.pardus.at/chattext.php?channel=ally*
...
var universe = window.location.toString().match(/uni=(\w+)/);
if (universe == null) {
	return;
} else {
	universe = universe[1].toLowerCase();
}

var keys = {};
var n_keys = 0;

// Used to store the key selection input
var key_select;

var key_store_name = 'eac_key_storage_' + universe;
function load_keys(){

	//GM_log("Loading: " + key_store_name);
	var str = GM_getValue(key_store_name, "var raw_k = {};");

	// NOTE: This set's the 'raw_k' var.
	eval(str);

	n_keys = 0;
	for ( var gid in raw_k ) {
		var k = raw_k[gid];
		k.name = unescape(k.name);
		k.key = unescape(k.key);
		n_keys += 1;
	}
	...
	var len = key_select.options.length;
	for (var i = 0; i < len; i++ ) {
		if (key_select.options[i].value > 0) {
			// Remove option
			key_select.remove(i);
			len--;
			i--;
	...
	for (var i in keys) {
		...
		opt.innerHTML = key.gid + ":" + key.name;
		opt.value = i;
		key_select.options.add(opt);
	}
...
function store_keys(){
	...
	for ( var gid in keys ) {
		...
		first = false;
		str += gid + ": { gid: " + gid + ", name: '" + escape(keys[gid].name) + "', key: '" + escape(keys[gid].key) + "'}";
	}

	str += " } ";

	//GM_log("Storing: " + str);
	GM_setValue(key_store_name, str);
}
...
keys = {
	[1]: { gid: 1, name: "Test", key: "secret key one woinsetj" },
	[2]: { gid: 2, name: "Other", key: "second key" }
};
*/



function make_pw(gid) {
	var tmp = sjcl.misc.cachedPbkdf2(keys[gid].key, { salt: "Some salty text" });
	var pw = tmp.key.slice(0,sjcl.json.defaults.ks/32);
	return pw;
}

var sig = "EAC";
var re_string = "(" + sig + ")(\\d+),([a-zA-Z0-9\\+/]+),";
var re = new RegExp(re_string, "g");

var password = 'pass';
var iv = sjcl.codec.utf8String.toBits("Some random stuff, that may be needed for something?");


function decrypt_entry(str) {
	var res = new RegExp(re_string).exec(str);
	if (res != null) {
		if (res[1] != sig) {
			return str;
		}

		var group_id = parseInt(res[2]);
		if (! keys[group_id]) {
			return "EAC" + group_id + ":?";
		}
		var ct = res[3];
		try {
			...
			var cipher = new sjcl.cipher['aes'](make_pw(group_id));
			var new_plaintext = sjcl.mode['ccm'].decrypt(cipher, ct, iv);
			new_plaintext = sjcl.codec.utf8String.fromBits(new_plaintext,1);
			// The whole encoding process mess's up escapes, so try to fix that here.
			new_plaintext = new_plaintext.replace(/\\'/g, '\'');
			new_plaintext = new_plaintext.replace(/\\\\/g, '\\');
			return "EAC" + group_id + ": " + new_plaintext;
		} catch(e) {
			//return "Could decrypt: g:" + group_id + ", " + str;
			return "EAC" + group_id + ":!";
		}
	} else {
		return str;
...
function encrypt_entry(key_v, str) {
	orig_string = str

	// Encrypt str here.
	str = sjcl.codec.utf8String.toBits(str);

	var res = false;
	try {
		var cipher = new sjcl.cipher['aes'](make_pw(keys[key_v].gid));
		var key_id = keys[key_v].gid;
		var ct = sjcl.mode['ccm'].encrypt(cipher, str, iv);
		str = sjcl.codec.base64.fromBits(ct,1);
		res = sig + key_id + "," + str + ",";
	} catch(e) {
		alert("ERR: " + e.message + ", Couldn't encrypt '" + orig_string + "'");
	}
	return res
}

function decrypt(str){
	str = str.replace(re, decrypt_entry);
	return str;
}


function init_chatframe(){

	var orig_decode = unsafeWindow.decodeString;
	function new_decode (str) {
		// For each chat line, look for the sig and decode that line.
		str = decrypt(str);
		return orig_decode(str);
	}

	unsafeWindow.decodeString = new_decode;

	var orig_sendMsg = unsafeWindow.sendMsg;
	function new_send_message(m_text, m_color, m_type) {

		var m_encrypt = parent.document.getElementById('chatmsg_encrypt').value;
		m_encrypt = parseInt(m_encrypt);
		if (m_encrypt >= 0) {
			m_text = encrypt_entry(m_encrypt, m_text);
			if (m_text == false) {
				return false;
			}
		}
		orig_sendMsg(m_text, m_color, m_type);
	}
	unsafeWindow.sendMsg = new_send_message;

	// Decode existing stuff, since we are likely to run after the inline scripts ran.
	var chat = document.getElementById("ChatWnd");
	var text = chat.innerHTML;
	text = decrypt(text);
	text = orig_decode(text);
	chat.innerHTML = text;
...
function init(){
	// Display our key lists.
	var text_box = parent.document.getElementById('chatmsg_text');
	key_select = parent.document.createElement("select");
	key_select.id = 'chatmsg_encrypt';
	text_box.parentNode.insertBefore(key_select, text_box);

	var opt = parent.document.createElement('option');
	opt.innerHTML = 'Plaintext';
	opt.value = -1;
	key_select.options.add(opt);



	// Add some key managment options.
	function get_key(op) {
		var gid = prompt("Please enter the channel ID of the key you want to " + op + ".");
		if (gid == null) {
			return false;
		}
		gid = parseInt(gid);
	...
	var settings_links = parent.document.createElement('span');
	function add_l(c, t, func) {
		var link = parent.document.createElement('a');
		link.innerHTML = c;
		link.title = t;
		link.addEventListener('click', func, false);
		settings_links.appendChild(link);
	}

	add_l('D', "Delete", function(){
			if (n_keys > 0) {
			var key = get_key("delete");
			if (key) {
			delete keys[key.gid];
			store_keys();
			load_keys();
		...
		add_l('E', "Export", function() {
				if (n_keys > 0) {
				var key = get_key('export');
				//GM_log(key);
				if (key) {
				prompt("Copy this text:", key.gid + "," + key.name + "," + key.key);
				}
				}
				}
			 )

		add_l('C', "Create new key", function() {
				var gid = prompt("Enter a unique channel ID number. This ID number must not be used by any other keys.");
				...
				if (keys[gid]) {
				alert("Channel " + gid + " is already in use.");
				return;
				}

				var name = prompt("Ok, using channel " + gid + ". Now enter the name of this channel")
				if (name == null) {
				return;
				}
				...
				keys[gid] = { gid: gid, name: name, key: random_key };
		...
		add_l('I', "Import", function() {

				var key_str = prompt("Paste the key");

				if (key_str == null) {
				return;
				}

				var key_elements = key_str.match(/(\d+),([a-zA-Z0-9_ ]+),([a-zA-Z0-9\+\/]+)/)
				//GM_log(key_elements);
				if (key_elements == null || key_elements.length != 4) {
				alert("Invalid key: " + key_str);
				return;
				}

				var gid = key_elements[1];
				gid = parseInt(gid);

				if (keys[gid]) {
				alert("Already have a key for channel " + gid);
				return;
				}

				keys[gid] = { gid: gid, name: key_elements[2], key: key_elements[3] };

				store_keys();
				load_keys();
		})

	text_box.parentNode.appendChild(settings_links);
}

if ( parent.document.getElementById('chatmsg_encrypt') == null ) {
	init();
} else {
	key_select = parent.document.getElementById('chatmsg_encrypt');
}
