// ==UserScript==
// @name           YouTube Video Download PL
// @namespace      http://rossy2401.blogspot.com/
// @description    Pobieranie wszystkich formatów wideo, od telefonu komórkowego do wysokiej rozdzielczości 1080p. Zgodne z Greasemonkey, Opera, Chrome i innymi. Wykonuje się w całości na stronie filmu, bez kontaktu z zewnętrznymi stronami internetowymi.
// @version        3.0.1
// @author         rossy! + look997
// @license        MIT License
// @include        http://userscripts.org/scripts/source/73376.meta.js
...
if (host.substr(host.length - 11) == "youtube.com" && host != "m.youtube.com") inject(function() {
	...
	function checkForUpdates()
	{
		...
		document.body.appendChild(createElem("iframe", {
			src: "http://userscripts.org/scripts/source/73376.meta.js",
			style: {
				position: "absolute",
				width: "1px",
				height: "1px",
	...
	var createMenu = function() {
		...
		watchFlag.parentNode.insertBefore(btn = createElem("button", {
	...
	showUpdate = function() {
		...
		updateMsg.setAttribute("href", "http://userscripts.org/scripts/source/73376.user.js");
	...
	function getStreamMap()
	{
		...
		try
		{
			if (!(streamMap = yt.getConfig("PLAYER_CONFIG").args["fmt_stream_map"]))
				if (!(streamMap = yt.getConfig("PLAYER_CONFIG").args["fmt_url_map"]))
					throw "";
		}
		catch (e)
		{
			try
			{
				...
				for (var i = 0; i < flashVars.length; i ++)
				{
					...
					if (name == "fmt_stream_map" || name == "fmt_url_map")
			...
			catch (e)
			{
				try
				{
					...
					if (swfConfigTxt.substring(0, 18) == "\n    (function() {")
					{
						...
						if (swfConfig.args.fmt_stream_map)
							streamMap = swfConfig.args.fmt_stream_map;
						else
							streamMap = swfConfig.args.fmt_url_map;
					}
				}
				catch (e)
				{
	...
	catch (e)
	{
		...
		document.body.insertBefore(createElem("a", {
			href: "http://userscripts.org/scripts/show/73376",
			target: "_blank",
			children: [
...
else if (document.location.href == "http://userscripts.org/scripts/source/73376.meta.js") inject(function() {
