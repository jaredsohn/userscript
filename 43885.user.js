...
function lang(es) {
   ...
   case 'gr':
	...
	case 'es':
		...
		["Foro " + alliancefullnm +"	mensajes no leÃ­dos","A los mensajes no leÃ­dos del Foro de la Alianza "],
		...
		[" Actualizar Herramientas"+alliancefullnm,"Obtener el Ãºltimo script"]];
		IslandLegendAllies="â€¢ Aliados";
		IslandLegendNoAlliance="â€¢ No Aliados";
		IslandLegendEnemies="â€¢ Enemigos";
		TreatyAll="All players have been checked. Yellow for no treaty and Gray for existing.";
		TreatyYes="TÃº tienes actualmente un tratado cultural con este jugador";
...
#container ul.resources li{line-height:14px}\
");
}
...
function islandview(){
	var allystore=[];	
	culttreaties=GM_getValue("CultTtreaties"+location.host);
	...
	forall("//li[contains(@class,'cityLocation')]", null, function(objpar,i){
		...
		if (obj){
			...
			if (ally) {
				if (!allystore[ally.textContent]){
					allystore[ally.textContent]=1;
				} else {
					allystore[ally.textContent]+=1;					
				}
			}
...
function talkbubble(who,text,fontsize,timeout){
	...
	switch (who) {
		case 'mayor':
			parelm=$fork('advCities');
		break;
		case 'general':
			parelm=$fork('advMilitary');
		break;
		case 'scientist':
			parelm=$fork('advResearch');
		break;
		case 'diplomat':
			parelm=$fork('advDiplomacy');
		break;
	}
	bubbles++;
...
function Tranporter(){
	...
	function parsegoods(gd, map){
		if (!map['startdate']) map['startdate'] = Number(curtime);
		if (gd.limit) 
			map['limit'] = gd.limit;
		if (gd.production) 
	...
	if (tmpCnt) {
		TransporterStore[getcurcityid()]['tmpCnt']=tmpCnt.enddate - curtime;		
	}
	...
	function getperc(capacity, current,cl,bcl,h,bo){
		...
		<td style="background-color:' + col + ';height:'+h+'px;opacity:0.6;" width="' + b + '%"/>\
		<td style="background-color:'+bcl+';height:'+h+'px;opacity:0.6;" width="' +	a +'%"/>\
		</tr></table>';
		return htmls;
	}
	
	function TransporterOver(){
		DestinationCitiesTransporter.style.visibility = "visible";
	}
	function TransporterHide(){
		DestinationCitiesTransporter.style.visibility = "hidden";
	}
	...
	htmls += '<table class="lal" style="display:block;width:650px;clear:left;margin:2px 2px 2px 2px;padding:0px 0px 0px 0px;text-align:center;">';
	htmls += '<tr><th style="width:160px;border:dotted 1px yellow;text-align:center;">City</th><th style="width:50px;border:dotted 1px yellow;text-align:center;">Build</th>';
	for (var j = 0; j < 5; j++) {
		htmls += '<th style="width:70px;border:dotted 1px yellow;text-align:center;"><img src="' + restransicons[j][2] + '" width=20 height=14/></i></td>';
	}
	htmls += '<th style="width:80px;border:dotted 1px yellow;text-align:center;">Action</th></tr>';
	var woodtot = 0;
	var marble = 0;
	var sulfur = 0;
	var crystal = 0;
	...
	for (var i = 0; i < citySelect.length; i++) {
		...
		if (citytrans['tmpCnt']>0) {
			htmls += '<td style="border:dotted 1px yellow;color:#542C0F;padding:0px 5px 0px 5px;font-size:9px;">' + formattime(citytrans['tmpCnt']) + '</td>';
		} else {htmls += '<td style="border:dotted 1px yellow;"></td>';}
			...
			if (citytrans) {
				...
				switch (resourcenm) {
					...
					case 'wine':
						...
						if (expe) {
							tradenr -= parseInt(expe['spendings'][0]['amount']);
				...
				if (tradenr != 0) 
					if (tradenr > 0) {
						...
						var calc=Math.floor(citytrans['resources'][resourcenm]/Math.abs(tradenr)/24);
						if (calc<2) {
							productionstr = ' <span style="color:red;margin:0px 5px">(' + calc +' D)</span>';
						} else {
							productionstr = '(' + calc +' D)';							
						}
					}
					if (citySelect[i].selected){
						if(!$fork('prod_'+resourcenm)){
							var dummyn=node('a','prod_'+resourcenm,'dummy','position:absolute;width:50px;bottom:-9px;left:30px;color:#542C0F;font-size:8px;cursor:pointer;margin:0px 3px 0px 3px;',null,'Production');
							dummyn.innerHTML=productionstr+'<div style="position:relative;bottom:16px;opacity:0.7;">'+getperc(citytrans['maxCapacity'][resourcenm], citytrans['resources'][resourcenm],'darkorange','yellow',3)+'</div>';
							$fork('value_'+resourcenm).parentNode.appendChild(dummyn);							
						}
					}
				htmls += '<td style="border:dotted 1px yellow;color:#542C0F;padding:0px 5px 0px 5px;font-size:9px;"><span id="' + respointer + '">' + fmtNumber(citytrans['resources'][resourcenm]) + '</span>' + productionstr + getperc(citytrans['maxCapacity'][resourcenm], citytrans['resources'][resourcenm]) + '</td>';
			}
			else {
				htmls += '<td style="border:dotted 1px yellow;color:#542C0F;padding:0px 5px 0px 5px;">?</td>';
			}
		...
		if (!citySelect[i].selected) {
			htmls += '<td style="width:50px;"><a href="index.php?view=transport&amp;destinationCityId=' + citySelect[i].value + '" style="margin:0px 1px 0px 1px"><img height="15" width="20" alt="Transport" title="Transport Goods to one of your Cities" src="skin/actions/transport.gif" style="float:right"/></a>\
						<a href="index.php?view=deployment&deploymentType=fleet&destinationCityId=' + citySelect[i].value +	'" style="margin:0px 1px 0px 1px"><img height="15" width="20" alt="Transport" title="Move Fleet to one of your Cities" src="skin/actions/move_fleet.gif" style="float:right"/></a>\
						<a href="index.php?view=deployment&deploymentType=army&destinationCityId=' +  citySelect[i].value +	'" style="margin:0px 1px 0px 1px"><img height="15" width="20" alt="Transport" title="Move Army to one of your Cities" src="skin/actions/move_army.gif" style="float:right"/></a></td></tr>';
		}
		else {
	...
	htmls += '<tr><td>Total Productions</td><td></td><td>' + fmtNumber(woodtot) + '</td><td>' + fmtNumber(wine) + '</td><td>' + fmtNumber(marble) + '</td><td>' + fmtNumber(crystal) + '</td><td>' + fmtNumber(sulfur) + '</td><td></td></tr></table>';
...
function main(){
	...
	switch (bodyid){
		...
		case "city":
			...
			if(GM_getValue('CityBuildingLevels','1')=='1'&&GM_getValue('CityBuildingNames','1')=='0'){
...
var b2=new Date();
bench.innerHTML='ika-core:'+Math.ceil((b2.getTime()-currenttime.getTime()))+' ms';
// ==UserScript==
