...
// @exclude http://chan.sankakucomplex.com/post/show/*
// @grant   GM_addStyle
// ==/UserScript==
...
GM_addStyle("#viewer-controls { white-space: nowrap; margin: 0; padding: 0 0 3px 0; } #viewer-controls li { list-style: none; display: inline-block; margin: 0 1em 0 0; }");
(function($){
   ...
   $(function() {    
      var  modalSettings = {
			 ...
			 link = thumbSpan.children('a:last');
               ...
               previewLinkHandler = function(event) {
      ...
      $('.content').ajaxComplete(addPreviewLinks);
        
      // init preview link handler
      $('.image-view-link').live('click', previewLinkHandler);
      ...
      $.showImage = function(data, beforeLoad) {
         ...
         // tools separator
         var  makeToolbarMenu = function(data) {
                     var   toolbarMenu = $('<ul id="viewer-controls">'),
                               highresLink = $('#highres', data).attr('target', '_blank'),
                               addfavLink = $('#add-to-favs a', data),
                               rating = $('#rating a', data).text('★');
                               
                     return toolbarMenu.append(
                                       $('<li>').append('Original: ', highresLink),
                                       $('<li>').append(addfavLink),
                                       $('<li>').append('rating: ', rating),
                                       $('<li>').append('<a href="#" class="simplemodal-close">close</a>')
                                   );
                  },
                  imageSource = $('#image', data),
                  // fit image size to viewport size
                  size = $.resizeToViewPort(imageSource.attr('width'), imageSource.attr('height')),
		    // create image
                  img = $('<img>').attr({
		     ...
		     // create image container
                   imageContainer = $('<div>').attr('id','big-image-container').append(makeToolbarMenu(data), img);
      ...
      $.resizeToViewPort = function(widthString, heightString) {
         var   width = Number(widthString),
                   ...
                   viewportWidth = $(unsafeWindow).width() - 20,
