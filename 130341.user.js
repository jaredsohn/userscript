// ==UserScript==
// @name           spash extract javelite
// @namespace      org.eroim.essime
// @include        http://www.javelite.com/forumdisplay.php?*
// @version        0.1.1
// ==/UserScript==
(function() {
	...
	/* ======= content */
	{
		var img_matchers = new Array(new RegExp(
				/img src="[^"]+" border="0" alt="" /gi), new RegExp(
				/\/attachments[^"]+/gi));
		...
		function getThreadLinks() {
			...
			// *[@id="thread_title_356861"]
			var linkList = x(
					'//*[contains(@id,"thread_title_") and starts-with(@href,"showthread")]',
					'linkList');
		...
		/*
		 * function highlight(xpath, color) { var point = x(xpath, 'hlpoint');
		 * var backs = x(
		 * '/html/body/div/div/div/div/table[2]/tbody/tr/td[2]/form/table/tbody/tr/td[1]',
		 * 'backs'); log.debug(point.length); log.debug(backs.length); for (i =
		 * 0; i < point.length; i++) { if (point[i].getAttribute('href') ==
		 * "forumdisplay.php?f=44") { log.debug(i + "::::" +
		 * point[i].textContent); backs[i + 2].setAttribute('style',
					...
					.addFunction(function() {
						...
						if (torrent_urls && torrent_urls.length > 0) {
							...
							for ( var j = 0; j < torrent_urls.length; j++) {
								...
								// <a target="_blank" href="http://host2.jptorrent.org/link.php?ref=S5RDQQXAAU">
								var attached_torrent_link = cE(
										'a',
										{
											href : new String(torrent_url
													.match(/http\:\/\/.+link\.php[^"]*"/)).replace('"',''),
