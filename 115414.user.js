	...
	************/
	...
	Tabs.AutoTrain = {
	  ...
	  trainamt : function(cityId, unitId){
		...
		t.amt = Math.floor(t.idle/(cost[6])).toFixed(0);
 		if (Math.floor(t.food/(cost[1]*4)) < t.amt) t.amt = Math.floor(t.food/(cost[1]*4)).toFixed(0);
 		if (Math.floor(t.wood/(cost[2]*4)) < t.amt) t.amt = Math.floor(t.wood/(cost[2]*4)).toFixed(0);
 		if (Math.floor(t.stone/(cost[3]*4)) < t.amt) t.amt = Math.floor(t.stone/(cost[3]*4)).toFixed(0);
 		if (Math.floor(t.ore/(cost[4]*4)) < t.amt) t.amt = Math.floor(t.ore/(cost[4]*4)).toFixed(0);
