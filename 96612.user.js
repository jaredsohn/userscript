...
// @description     Script for The-West: TW Pro+ [SOM - Scripts-O-Maniacs] [Multilingual] (v2.0.0.1)
...
if (localStorage.getItem("TWPro_LanguagePack") && decodeURIComponent(localStorage.getItem("TWPro_LanguagePack")) != "fr" && decodeURIComponent(localStorage.getItem("TWPro_LanguagePack")) != "http://pt"){
	try{
	var myScript = document.createElement("script");
	myScript.type = "text/javascript";
	myScript.src = decodeURIComponent(localStorage.getItem("TWPro_LanguagePack"));
...
var twpro_lang = {
	info: ['Zyphir Randrott', 'mailto:zyphir.randrott@gmail.com', 4343, '.w1.'],
		...
		FBBATTLEUNITSKIRMISHER: "Escaramuçador (esquiva) | Linha da Frente",
	...
	twpro_langs.pt = {
		...
		SORTBYNAME: 'Ordenar por <b>nome</b>',     /*"Order activity list by name" button at the Inventory*/
		SORTBYXP: 'Ordenar por  <b>experiencia</b>',     /*"Order activity list by experience" button at the Inventory*/
		SORTBYWAGES: 'Ordenar por  <b>salários</b>',     /*"Order activity list by wages" button at the Inventory*/
		SORTBYLUCK: 'Ordenar por  <b>sorte</b>',     /*"Order activity list by luck" button at the Inventory*/
		SORTBYCOMB: 'Ordenar por  <b>trabalho</b>',     /*"Order activity list by job rank" button at the Inventory. The player can define the importance of experience, wages, luck and danger, which is called "Job rank"*/
		SORTBYDANGER: 'Ordenar por <b>perigo</b>',     /*"Order activity list by danger" button at the Inventory*/
		SORTBYLABORP: 'Ordenar por  <b>pontos de trabalho</b>',     /*"Order activity list by labor points" button at the Inventory*/
		FILTERJOBS: "Esconder o que não posso fazer",     /*This message is shown in a tooltip when hovering over a checkbox which is available in the "Hide activities and itemsets" window. This checkbox is used to toggle the setting for hiding activities which the player cannot do (labor points is negative)*/
		FILTERCLOTHING: 'Mostrar apenas os melhores artigos',     /*This text is shown in a tooltip when hovering over a checkbox at the Inventory. If checked, the best items for the selected activity is shown with all other items hidden*/
		CHOOSEJOB: 'Escolha a atividade...',     /*If TW Pro has calculated the activity list, this text is shown when no activity is selected*/
		CALCJOB: 'Calculando valores, por favor aguarde...',     /*If TW Pro is calculating the activity list, this text will be shown in the activity list drop-down box*/
		INVENTSTATS: 'Estatisticas do inventário',     /*The title of the tooltip, visible when hovering over the "Backpack" text at the Inventory*/
		SELLVALUE: 'Valor de vendas',     /*Visible in the Inventory statistics tooltip. The money you get for selling all items*/
		OBJECTS: 'Objectos',     /*Visible in the Inventory statistics tooltip. Objects are all items except for products*/
		...
		MULTISEL: 'Venda multipla de artigos...',     /*A button in a town trader, enabling players to sell multiple items at once*/
		SELL: 'Vender seleção',     /*The value of the button after clicking the "Sell multiple items..." button. Clicking this button sells all selected items*/
		CONFIRMSELL: 'Quer realmente vender %1 este lote de artigos?',     /*Text of a confirmation dialogue, shown after clicking "Sell selection"*/
		SELLING: 'Vendendo...',     /*The value of the button "Sell selection" during a multi-sale*/
		SALEDONE: 'Os itens selecionados foram vendidos.',     /*This text is displayed after a multi-sale*/
		NONESELECTED: 'Tem que selecionar pelo menos um artigo!',     /*This warning message is displayed when no items are selected when trying to sell multiple items*/
		JOBRANKSETTINGS: 'Níveis \ Importância',     /*This text is shown in a tooltip on hovering over the image for a job rank.  The player can define the importance of experience, wages, luck and danger, which is called "Job rank"*/
		SEARCHINVENTORY: 'Pesquisa Inventário',     /*This text is displayed in the "Search inventory" text field by default. Keep it short*/
		NOSEARCHRESULT: 'A pesquisa %1 não encontrou.%2Exibir todos os itens%3',     /*This message is displayed in the inventory (Inventory and Market) when the search query returned no items. %1 is the query, %2 marks the beginning of the "Display all items" link, %3 marks the end. Clicking "Display all items" removed the search filter (empties the "Search inventory" text field)*/
		DISABLEBESTFORJOB: 'Desativar itens escondidos, que são inferiores para uma atividade selecionada.',     /*This text is displayed when the checkbox "Just display the best clothing available for the selected activity" is checked and the inventory search returned no results*/
		SEARCHHELP: 'Busca no inventário. Você pode usar curingas (* for zero or more characters, ? for one character)',     /*When hovering over the "?" button next to the "Search inventory" field, this help message is displayed in a tooltip*/
		DUELSHOOTINGATT: ' \u25B7 Duelista (atacar)',     /*A special activity for duel clothing used by shooting duellers in the challenging role*/
		DUELSHOOTINGDEF: ' \u25B7 Duelista (defender)',     /*A special activity for duel clothing used by shooting duellers in the defending role*/
		DUELVIGOR: ' \u25B7 Duelista Armas Brancas',     /*A special activity for duel clothing used by resistance duellers*/
		FORTATTACK: ' \u25B7 Forte (ataque)',     /*A special activity for attackers in fort battles*/
		FORTDEFEND: ' \u25B7 Forte (defesa)',     /*A special activity for defenders in fort battles*/
		FORTMESSAGE: 'Mensagem aos participantes',     /*Displayed on Fortbattle page, to send a PM to all participants*/
		FORTMESSAGERCP: 'Número de destinatários',     /*Information displayed when you click on "Message to participants"*/
		HIDEJOBS: 'Gestão de Atividades e conjuntos de itens',     /*This text is shown as a tab in the TW Pro settings window*/
		CONFIRM: 'Confirmar',     /*The button for confirming/ saving changes in the "Activities & Itemsets Management" tab*/
		HIDEJOBDESC: 'Os trabalhos podem ser escondidos aqui. Marcar todas as atividades que não deve ser calculada automaticamente, e clique em Confirmar.',     /*The description for the "Activities & Itemsets Management" tab*/
		SHOWN: 'Mostrados',     /*Displayed ahead the selection field for activities that are currently shown*/
		HIDDEN: 'Escondidos',     /*Displayed ahead the selection field for activities that are currently hidden*/
		SETTINGSSAVED: 'As configurações para TW Pro foram aplicadas!',     /*After clicking "Confirm" in the "Activities & Itemsets Management" tab, this message is displayed*/
		SETTINGSSAVEDSESSION: 'As configurações para TW Pro foram aplicadas!! (só para esta sessão)',     /*After clicking "Confirm" in the "Activities & Itemsets Management" tab while having the checkbox near Confirm checked (persistent settings), this message is displayed*/
		PERSISTSETTINGS: 'Guardar estas configurações para cada sessão.',     /*Displayed in a tooltip when hovering over a checkbox next to "Confirm" in the "Activities & Itemsets Management" tab. If this checkbox is checked, the changes will be saved for future sessions. Otherwise, after a reloading the page, changes are lost*/
		CANNOTWEAR: 'Você não pode usar este conjunto, para este conjunto não tem qualquer influência sobre os cálculos de atividades.',     /*This text is displayed when hovering over an item set at the "Activities & Itemsets Management" tab for items sets you cannot wear (e.g. Dancer set cannot be used by male characters) or that have no influence on Activities (e.g.: Sleepyhead has only bonuses in "Fastest Lifepoints")*/
		SETSETTINGS: 'Desativar conjuntos para cálculos mais rápidos. Define com necessidades especiais, as exigências não atendidas são desativados por padrão.',     /*The description displayed ahead the settings for itemsets at the "Activities & Itemsets Management" tab*/
		CUSTOMNAME: 'Digite o nome desejado para uma actividade',     /*Visible in the "Activities & Itemsets Management" tab.This message is displayed in a tooltip when hovering over the text field, used for setting the name for a custom activity.*/
		CUSTOMCALCULATION: 'ntroduza um cálculo TWPro válido  aqui.',     /*This message is displayed in a tooltip when hovering over the text field, used for setting the TW Pro calculation (this calculation is used for determining the importance of skills for activities) for a custom activity. Applies to the "Activities & Itemsets Management" tab*/
		CUSTOMENABLED: 'Selecione esta caixa para ativar esta atividade',     /*This button is visible in the "Activities & Itemsets Management" tab. If this checkbox is checked, the activity is included in the activity list. Otherwise, the activity will be hidden*/
		NEW: 'Novo',     /*Visible in the "Activities & Itemsets Management" tab.This text is displayed in a tooltip when hovering over the "+" button, used for creating a new custom activity*/
		SPEED: ' \u25B7 Max Velocidade',     /*A special activity, useful for travelling great distances and saving time*/
		REGENERATION: ' \u25B7 Mais pontos de vida',     /*A special activity, useful when you are low on Health points and want to restore your health points faster*/
		SETSINFO: "Conjuntos de artigos",     /*Title of the "Itemsets information" window which displays the bonuses for each set*/
		WITHSETITEMS: 'Bonuscom %1 artigos',     /*Displayed in Setinfo window*/
		LABORPOINTS: 'Pontos de trabalho',     /*Labor points, displayed for sets. Example: +20 Labor points*/
		USEFULITEM: 'Este número indica a quantidade de artigos que são utilizados nos cálculos',     /*Displayed between parentheses after a set name in the TW Pro Config screen*/
		PERCREGENERATION: 'Recuperação',     /*__ % Regeneration with Sleepyhead items*/
		LUCK: 'sorte',     /*__ % luck with Holiday set items*/
		PRAYING: 'oraçao',     /*6 Sleepyhead items gives +1 Praying*/
		NOITEMSETS: "You don\'t have any item of this Set",     /*Displayed in the vertical Set filters*/
		AVERAGEDAMAGE: "Dano médio",     /*Displayed on every weapons*/
		PREFERENCES: "Preferencias",     /*This text is shown as a tab in the TW Pro settings window*/
		DEFAULTSORTING: " \u25B7 Ordenação padrão",     /*Visible in the "Preferences" tab.*/
		FBHEALTHPRIORITY: "Prioridade aos pontos de saúde",     /*Visible in the "Preferences" tab.*/
		FBHEALTHPRIORITYZERO: "Nenhum",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYLOW: "Baixo",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYMEDIUM: "Médio",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYHIGH: "Alto",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYAUTO: "Auto",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBBATTLEUNIT: "Unidade de batalha tipo",     /*Visible in the "Preferences" tab.*/
		FBBATTLEUNITSKIRMISHER: "Escaramuçador (esquiva) | Linha da Frente",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
		FBBATTLEUNITLINEINFANTRYMAN: "Infantaria (polivalente) | Linha Média",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
		FBBATTLEUNITMARKSMAN: "Atirador (objetivo) | Unidade de retaguarda",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
};
...
//			xhtml += '<br /><input id="twpro_jobsort_filter" type="checkbox"' + (TWPro.twpro_preference('Hide_unjobs')?' checked="checked"':'') + ' title="'+TWPro.lang.FILTERJOBS+'" />';
			...
			xhtml += '<tr><td><b>' + TWPro.lang.DEFAULTSORTING + ': </b></td><td><select size="1" onchange="TWPro.save_preferences(\'defaultSorting\',this);" style="background-color: rgb(207, 195, 166); font-size: 10px; width:130px;">';
			for(var i=0;i<TWPro.twpro_sorts.length;i++){
...
//				TWPro.twpro_jobValues[twpro_job.shortName].jobrank = Math.round((TWPro.multipliers.xp * TWPro.twpro_jobValues[twpro_job.shortName].erfahrung + TWPro.multipliers.wages * Math.min(100, TWPro.twpro_jobValues[twpro_job.shortName].lohn * pow(Math.max(1,twpro_apstmp),0.2)) + TWPro.multipliers.luck * Math.min(100, TWPro.twpro_jobValues[twpro_job.shortName].glueck * pow(Math.max(1,twpro_apstmp),0.2)) + TWPro.multipliers.danger * (100 - Math.min(100, TWPro.twpro_jobValues[twpro_job.shortName].gefahr/((Math.max(1,twpro_apstmp)+3)/8))))/Math.max(1,(TWPro.multipliers.xp+TWPro.multipliers.wages+TWPro.multipliers.luck+TWPro.multipliers.danger)));
	...
	function twpro_calculateJobs() {
		...
		for (var twpro_i=0; twpro_i<TWPro.twpro_jobs.length; twpro_i++) {
			...
			twpro_setitembonus.parsedBonus_fb[twpro_setitemjobname]["sk"+i] = (twpro_job.malus == -1 ? 0 : twpro_setitembonus.jobBonus.all) +
						(!twpro_setitembonus.jobBonus[twpro_setitemjobname] ? 0 : twpro_setitembonus.jobBonus[twpro_setitemjobname]) + twpro_setitembonus.twpro_jobbonus[twpro_job.shortName+'_sk'+i];
			  }
			}
					}
				}
