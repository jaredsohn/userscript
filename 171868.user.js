...
// @homepage    https://userscripts.org/scripts/show/171868
// @updateURL    https://userscripts.org/scripts/source/171868.meta.js
...
var CSS = [
  ...
  '#gm_fullfeed_message { position: fixed; top: 4px; right: 300px; z-index: 90900; color: black; background-color: #FFEEAA; box-shadow: 1px 1px 2px #CCCCCC; padding: 4px 8px; }',
  ...
  '.gm_fullfeed_twitter, .gm_fullfeed_delicious, .gm_fullfeed_facebook, .gm_fullfeed_hatena { position: relative; padding: 2px 4px; }',
  '.gm_fullfeed_twitter img, .gm_fullfeed_delicious img, .gm_fullfeed_facebook img, .gm_fullfeed_hatena img { width: 16px; height: 16px; vertical-align: middle; }',
  '.gm_fullfeed_twitter span, .gm_fullfeed_delicious span, .gm_fullfeed_facebook span, .gm_fullfeed_hatena span { padding: 0px 3px; vertical-align: middle; }',
...
FullFeed.checkScroll = function() {
  ...
  scrollInterval = window.setTimeout(function() {
    ...
    if (!e1 || !e2) return;
    var remain = e2.offsetTop + nextPageLink.offsetTop - e1.clientHeight - e1.scrollTop;
...
FullFeed.setCache = function(res, ctx, item, flag) {
  ...
  if (info.length > 0) {
    ...
    if (flag) {
      var hasFF = (localStorage.getItem('FeedlyFullFeed_cache')) ? true : false;
      var hasAP = (localStorage.getItem('FeedlyFullFeed_cacheAP')) ? true : false;
...
FullFeed.createSettings = function() {
  ...
  if ($id('feedlyTabs')) {
    var eFt = $id('feedlyTabs'), eC1, eC2, eMenu;
    if (eFt) {
      eC1 = eFt.children[eFt.children.length - 1];
      if (eC1) eC2 = eC1.children[eC1.children.length - 2];
      if (eC2) {
        eMenu = document.createElement('div');
        eMenu.className = 'tab';
        eMenu.innerHTML = '<div class="header target"><div id="gm_fullfeed_settings-menu" style="color:inherit; padding-left:32px;" target="new" class="label">Full Feed ' + loc[21] + '</div></div>';
        eC2.appendChild(eMenu);
      }
    }
  }
...
FullFeed.registerWidgets = function() {
  ...
  var xe = returnXpathEntry();
  el = getElementsByXPath(xe + '//div[@class="metadata"]/span');
  if (el) {
    el.some(function(i) {
  ...
  if (flag) return;
  var container = getFirstElementByXPath(xe + '//div[@class="metadata"]');
  if (!container) return;
  itemSiteInfo = [];
...
FullFeed.registerSocialIcons = function() {
  ...
  if (!c.innerContents) return;
  var xe = returnXpathEntry();
  el = getFirstElementByXPath(xe + '//div[@class="metadata"]/span[@class="gm_fullfeed_socialicon"]');
  if (el) return;
  var container = getFirstElementByXPath(xe + '//div[@class="metadata"]');
  ...
  var inter, repeat = 0, check = function() {
    var e = getElementsByXPath(xe + '//div[@class="metadata"]/span[@class="gm_fullfeed_socialicon"]/a/img'), f = true;
    repeat ++;
    if (e) {
...
var getCurrentItem = function() {
  ...
  var u = this.itemURL, xe = returnXpathEntry();
  if (/^https?:\/\/www\.feedly\.com\/home#subscription\/feed\/https?:\/\/.+/.test(u)) {
    u = u.slice(u.indexOf('/feed/')+6);
  ...
  this.articleContainer = getFirstElementByXPath(xe);
  this.innerContents = getFirstElementByXPath(xe + '//div[contains(concat(" ", @class, " "), " entryBody ")]');
  this.found = false;
};
...
var returnXpathEntry = function() {
  return ($id('timeline') && $id('timeline').classList.contains('u100EntryList')) ?
    '//div[contains(concat(" ", @class, " "), " selectedEntry ")]' :
    '//div[contains(concat(" ", @class, " "), " inlineFrame ")]';
};
...
var init = function() {
  ...
  if (GM_registerMenuCommand && typeof GM_registerMenuCommand === 'function') {
    GM_registerMenuCommand('Feedly Full Feed - reset cache', function() {
      if (window.confirm(loc[15])) FullFeed.resetCache();
    });
    GM_registerMenuCommand('Feedly Full Feed - delete cache', function() {
...
function message(mes, dur, typ) {
  ...
  var type = typ;
  box.innerHTML = mes;
  if (hasClass(box, 'gm_fullfeed_warning')) removeClass(box, 'gm_fullfeed_warning');
  if (type) addClass(box, 'gm_fullfeed_' + type);
  if (hasClass(box, 'gm_fullfeed_hidden')) removeClass(box, 'gm_fullfeed_hidden');
