...
// ==UserScript==
// @name           Friend Feed Put Alert Thingy To Bed
// @namespace      http://internetducttape.com
// @description    Alert Thingy adds a 'via AlertThingy' message which is annoying. Change it to say 'in bed'. Does the same thing for via twhirl.
...
var friendfeedalertthingyfix_version = 0.2;

(function() {
  // Script updates itself every two weeks.
  autoUpdateFromUserscriptsDotOrg({
    name: 'Friend Feed Put Alert Thingy To Bed',
	url: 'http://userscripts.org/scripts/source/25289.user.js',
	version: friendfeedalertthingyfix_version,
	});

  window.IDT_FIX_ALERT_THINGY = function() {
    unsafeWindow.jQuery('.comment:contains(...via )').each(function() {
      if(this.textContent.match('...via AlertThingy')) {
        this.textContent = this.textContent.replace('...via AlertThingy', '...in bed');
      }
      if(this.textContent.match('...via twhirl')) {
        this.textContent = this.textContent.replace('...via twhirl', '...in bed');
      }								   });
  }

  function jquery_wait() {
    if(typeof unsafeWindow.jQuery == 'undefined') { window.setTimeout(jquery_wait,100); }
    else { window.IDT_FIX_ALERT_THINGY(); }
  }
  jquery_wait();
  addFilter(window.IDT_FIX_ALERT_THINGY);
