...
//					if (wall_events_array.length <= 0){
						...
						wall_events_obj.innerHTML = wall_response_array.payload.html;
						var wall_events_array = getChildElementsByClassName(wall_events_obj,'UIImageBlock');
...
//					}
					for ( var i in wall_events_array ){
						...
						if (getChildElementsByClassName(wall_events_array[i],'UIImageBlock')[0] != undefined){
							tmp_array['user'] = {
								'link':wall_events_array[i].getElementsByTagName('a')[0].getAttribute('href').replace(/&amp;/g,'&'),
								'name':getChildElementsByClassName(wall_events_array[i],'uiStreamMessage')[0].getElementsByTagName('a')[0].innerHTML,
							...
							if (getChildElementsByClassName(wall_events_array[i],'uiAttachmentDesc')[0] != undefined){
								var store_string = getChildElementsByClassName(wall_events_array[i],'uiAttachmentDesc')[0].innerHTML;
							}else{
								var store_string = 'Store undefined';
							}
							tmp_array['action'] = {
								'query':getArgs(getChildElementsByClassName(wall_events_array[i],'UIImageBlock_Image')[1].getAttribute('href').replace(/&amp;/g,'&')),
								'store':store_string
							};
...
function actionWallEvents(id){
	...
	alert ( id ); 
	alert ('this is the action link:' + action_link); 
