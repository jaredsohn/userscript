...
// ==UserScript==
// @author      Risi, slovenian integration by PrimeEvil
// @email	rlaffers@gmail.com
// @namespace	http://userscripts.org/
// @name		Travian Trooptool Update SI
// @description	Update your troop tool with a one click from the rally point. Integrated to work with Slovenian servers.
...
if(init) {
    ...
    alert("Inicializacija neuspela, Travian Task Queue ni zagnan");
}
...
switch(sLang) {
	case "sk":
        ...
        case "ua":
                ...
                break;
	case "si":
		aLangStrings = ["lastne enote", "Vrnitev iz", "Okrepitve za", "Okrepitve iz", "Napad na", "Roparski pohod proti", "Izvidnica", "Razišči zapuščeno pokrajino", "Ujete enote v", "Poizkus vnosa se je končal z neznanim rezultatom.", "Tvoje enote so bile uspešno vnesene.", "Vnos je bil neuspešen!", "Troop Tool Updater nastavitve", "Pošlji podatke o enotah", "Troop Tool Updater:", "Vnesi avtentifikacijski ključ iz tvojega Troop tool profila"];
		break;
	case "en":
	case "com":
	case "gr":
	aLangStrings = ["Δικά μου στρατεύματα", "Δικά μου στρατεύματα", "Ενίσχυση για", "τα στρατεύματα του", "Εισβολή εναντίον", "Επίθεση εναντίον", "Ανίχνευση του", "Εισβολή εγκαταλειμμένης όασης", "Φυλακισμένα στρατεύματα από", "Update attempted with unknown result.", "Your troops were successfully updated.", "Update failed!", "Troop Tool Updater setup", "Update Troop Tool", "Troop tool Updater:", "Enter the authentication key from your Troop tool profile"];
		break;
	default:
		aLangStrings = ["Δικά μου στρατεύματα", "Δικά μου στρατεύματα", "Ενίσχυση για", "τα στρατεύματα του", "Εισβολή εναντίον", "Επίθεση εναντίον", "Ανίχνευση του", "Εισβολή εγκαταλειμμένης όασης", "Φυλακισμένα στρατεύματα από", "Update attempted with unknown result.", "Your troops were successfully updated.", "Update failed!", "Troop Tool Updater setup", "Update Troop Tool", "Troop tool Updater:", "Enter the authentication key from your Troop tool profile"];
		break;
...
function initialize() {
    ...
    if ((server) && (server[1])) {
        ...
        return true;
         ...
         _log(0, "ERROR, neznan Travian server!");
        return false;
    }
}
