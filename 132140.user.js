// ==UserScript==
// @name           PokemonUnleashed Tool
// @namespace      Unleashedrising.tk
// @include       http://*www.unleashedarena.net/*
// @exclude        http://*www.unleashedarena.net/index.php*
// ==/UserScript==
...
function findRare() {
	...
	wildText = wildText.textContent.trim();
	if (!wildText.match(/Wild (Shadow|Shiny)/)) {
		moveAround();
		return;
	}
...
unsafeWindow.AjaxRequest = function () {
	...
	this.displayLoading = function () {
		if (this.showLoading == 'sidebar') {
			...
			this.loading.innerHTML = '<p style="text-align: center; margin-top: 150px;"><img src="http://static.pokemonUnleashed.Net/images/loading.gif" width="100" height="100" alt="Loading..." /></p>';
		} else if (this.showLoading == 'message') // message
		{
			...
			this.loading.innerHTML = '<p style="text-align: center; margin-top: 75px;"><img src="http://static.pokemonUnleashed.Net/images/loading.gif" width="100" height="100" alt="Loading..." /></p>';
		} else if (this.showLoading == 'messageList') // message list
		{
			...
			this.loading.innerHTML = '<p style="text-align: center; margin-top: 50px;"><img src="http://static.pokemonUnleashed.Net/images/loading.gif" width="100" height="100" alt="Loading..." /></p>';
		} else if (this.showLoading == 'map') // map
		{
			this.loading = document.getElementById('mapLoading')
			this.loading.innerHTML = '<p style="text-align: center; margin-top: 150px;"><img src="http://static.pokemonUnleashed.Net/images/loading_white.gif" width="100" height="100" alt="Loading..." /></p>';
	...
	unsafeWindow.disableSubmitButton = function(form) {
		for (i = 0; i < form.elements.length; i++) {
			if (form.elements[i].type == 'submit') {
				form.elements[i].value = 'Please Wait... or click again - by rockingdo';
			}
		}
		return true;
	} 
