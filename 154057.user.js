	...
	function init(topObject,window,document,arrayFn,envir){
		...
		var prefs={
			...
			gallery:{//图库相关设定
				...
				wheelAction:{//'scroll' | 'switch' ；滚动和切换图片
					imgArea:'scroll',//大图区
					thumbArea:'scroll',//缩略图区
				},
		...
		*/
		...
		GalleryC.prototype={
			init:function(){
					...
					'<span class="pv-gallery-head">'+
						...
						'<span class="pv-gallery-head-command-container">'+
							...
							'<span class="pv-gallery-head-command-drop-list pv-gallery-head-command-drop-list-slide-show">'+
								'<span class="pv-gallery-head-command-drop-list-item" title="间隔时间，单位（秒）">'+
									'<input data-prefs="interval" step="1" min="1" type="number" value="5" size="6"/>'+
									'<span>间隔(s)</span>'+
								'</span>'+
								'<span class="pv-gallery-head-command-drop-list-item"  title="反方向">'+
									'<input id="pv-gallery-head-command-drop-list-item-slide-show-backward" data-prefs="backward" type="checkbox" />'+
									'<label for="pv-gallery-head-command-drop-list-item-slide-show-backward">反方向</label>'+
								'</span>'+
						...
						'<span class="pv-gallery-head-command-container">'+
							'<span title="选择图片类别" class="pv-gallery-head-command pv-gallery-head-command-category">'+
								'类别<span class="pv-gallery-vertical-align-helper"></span>'+
							'</span>'+
						...
						'<span class="pv-gallery-head-command-container">'+
							'<span title="一些命令菜单" class="pv-gallery-head-command pv-gallery-head-command-others">'+
								'命令<span class="pv-gallery-vertical-align-helper"></span>'+
							'</span>'+
						...
						'<span class="pv-gallery-head-command-container">'+
							'<span title="分享" class="pv-gallery-head-command pv-gallery-head-command-share">'+
								'分享<span class="pv-gallery-vertical-align-helper"></span>'+
							'</span>'+
				...
				addWheelEvent(eleMaps['body'],function(e){//wheel事件
					...
					if(eleMaps['sidebar-container'].contains(target)){//缩略图区滚动滚轮翻图片
						if(prefs.gallery.wheelAction.thumbArea=='scroll'){
							...
							(e.deltaY<0 || e.deltaX<0) ? self.selectPrevious() : self.selectNext();
						};

					}else{//图片区域滚动
						var distance=100;
						if(e.deltaY!=0){//y轴
							if(prefs.gallery.wheelAction.imgArea=='scroll'){
								if(e.deltaY < 0){
									distance=-distance;
								};
				...
				var slideShow={
					...
					run:function(timing){
						...
						if(slideShowOpts.skipErrorImg){
							if(self.selected.dataset['src']==self.img.src){//确保是当前图片和选中缩略图一致的时候
								if(self.imgError){
						...
						if(slideShowOpts.wait){
							if(timing!='select' && (
								timing=='loadEnd'  || 
								(self.selected.dataset['src']==self.img.src && (self.img.complete || self.imgError)))
							){
								this.go();
							};
						}else{
					...
					getEle:function(){
						return self.getThumSpan(slideShowOpts.backward)
					},
					go:function(){
						this.stop();//停止上次的。
						var interval=slideShowOpts.interval;
						var _self=this;
						slideShowOpts.timer=setTimeout(function(){
							_self.setCountdown(0);
							clearInterval(slideShowOpts.countdownTimer);
							self.select(_self.getEle());
						},interval);

						var startTime=Date.now();
						slideShowOpts.countdownTimer=setInterval(function(){
							_self.setCountdown(interval - (Date.now()-startTime));
						},100);
					},
					stop:function(){
						this.setCountdown(slideShowOpts.interval);
						clearTimeout(slideShowOpts.timer);
						clearInterval(slideShowOpts.countdownTimer);
					},
					exit:function(){
						slideShowOpts.run=true;
						this.switchStatus();
						this.stop();
					},
					...
					switchStatus:function(){
						slideShowOpts.run=!slideShowOpts.run;
						eleMaps['head-command-slide-show-button-inner'].classList[slideShowOpts.run? 'add' : 'remove']('pv-gallery-head-command-slide-show-button-inner_stop');
					},
					check:function(){
						slideShowOpts.run?  this.run('click') : this.stop();
					},
				};
				slideShow.setCountdown(slideShowOpts.interval);;
				...
				eleMaps['head'].addEventListener('click',function(e){//顶栏上面的命令
					...
					switch(target){
						...
						default:{
							if(eleMaps['head-command-slide-show'].contains(target)){
			...
			selectedIntoView:function(){
				var thumBC=this.eleMaps['sidebar-thumbnails-container'];
				var pro=this.isHorizontal ? ['offsetLeft','clientWidth'] : ['offsetTop','clientHeight'] ;
				//需要滚动的距离。
				...
				this.thumbScrollbar.scroll(needScrollDis-scrollCenter,false,true);
			},
			getImg:function(ele){
				...
				if(preImgR && this.img){
					var preILI=preImgR.loadingIndicator;
					if(preImgR.img.src!=this.img.src){
						preImgR.abort();
						preILI.style.removeProperty('display');
					}else{//读取错误的
						if(preILI.classList.contains('pv-gallery-sidebar-thumb-loading_error')){
							preILI.style.removeProperty('display');
						};
					};
				};
			...
			loadImg:function(img,relatedThumb,error){
				...
				this.eleMaps['head-left-img-info'].firstElementChild.textContent= imgNaturalSize.w + ' x ' + imgNaturalSize.h;
				...
				if(error){
					this.imgError=true;
					img.classList.add('pv-gallery-img_error');
				};
			...
			fitToScreen:function(scale){
				...
				if(this.fitContains){//适应屏幕
					...
					if(larger){
						...
						if(contentSize.h/contentSize.w >=containerSize.h/containerSize.w){
							...
							scaled=img.height/this.imgNaturalSize.h;
						}else{
							img.width=containerSize.w-20;
							scaled=img.width/this.imgNaturalSize.w;
						};
				...
				var imgScaledInfo=this.eleMaps['head-left-img-info'].lastElementChild;
			...
			clear:function(){
				...
				this.eleMaps['img-parent'].innerHTML='';
				this.eleMaps['head-left-img-info'].firstElementChild.textContent='0 x 0';
				this.eleMaps['head-left-img-info'].lastElementChild.textContent='（100%）';
				//隐藏滚动条
				this.imgScrollbarV.hide();
				this.imgScrollbarH.hide();
			...
			switchThumbVisible:function(){
				...
				if(this.isHorizontal){
					// - (count>0 ? 3 : 0) 
					this.eleMaps['sidebar-thumbnails'].style.width= 
						this.sideLengthO * count + 'px';//如果是水平放置，需要设置宽度
				};
			...
			addStyle:function(){
				...
				style.textContent='\
					...
					.pv-gallery-sidebar-thumbnails-h >  .pv-gallery-sidebar-thumb-container {\
						margin:0 3px 0 0;\
						height:100%;\
					}\
					.pv-gallery-sidebar-thumbnails-v >.pv-gallery-sidebar-thumb-container {\
						margin:0 0 3px 0;\
						width:100%;\
					}\
		...
		GalleryC.prototype.Scrollbar.prototype={//滚动条对象
			...
			reset:function(){//判断滚动条该显示还是隐藏
				...
				var scrollMax=contentSize - containerSize;
