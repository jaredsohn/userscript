// Autor: -== VelMizar ==-
// Fecha de modificacion: 02/06/12

// ==UserScript==
// @name           clik - PohTemnMaterii -ru2
// @author         VelMizar
// @date           02-06-2012
// @namespace      pohuk temnoii materii
...
Array('http://ru2.lordgalaxy.ru//game.php?page=eksp&mode=&cp=40774&re=0','на головну планету - експедиція'),
Array('http://ru2.lordgalaxy.ru//game.php?page=eksp&cmd=send&id=1','отправити перший корабль'),
Array('http://ru2.lordgalaxy.ru//game.php?page=eksp&cmd=send&id=1','отправити перший корабль'),
Array('http://ru2.lordgalaxy.ru//game.php?page=eksp&cmd=send&id=1','отправити перший корабль'),
Array('http://ru2.lordgalaxy.ru//game.php?page=fleet&mode=&cp=40774&re=0','на головну планету - флот')
);
...
vidlik1 = vidlik1 - 1;
if (vidlik1 <= 0) {
		window.open(posylannya[4][0],'_self');
		vidlik1=60;
	}else if (vidlik1 == 50){
        activProdovzhennya();
...
if (!document.getElementsByTagName) return;
		...
		for (i = 0; i < infoTagA.length; i++) {
			if (infoTagA.item(i).href == "game.php?page=eksp&cmd=send&id=1") {
			window.open(posylannya[1][0],'_self');
			};
			if (i <= (infoTagA.length-1)) {
			window.open(posylannya[4][0],'_self');
			};	
		};
}
...
'<div style="position:absolute; left: -55px; top: 13px; margin:0px 0px 5px 0px; width:222px; float:left; overflow:hidden;">'+
  ...
  '<div style="background:url(http://uni101.ogame.ru/game/img/navigation/box_1_mitte.gif) repeat-y; padding:0px 15px;">'+
    '<table width="188" cellpadding="0" cellspacing="0" style="margin: 5px 0px -15px 2px; font-size:14px;">'+
		 ...
		 '<tr>'+
          '<td colspan="2">'+ 
           '<div id="txt_6" style="color:#9F9FF8; font-size:12px; text-align:center padding-top:5px;">Відправка на пошук ТМ</div>'+
		   '</td></tr>'+ 
