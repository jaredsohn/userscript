// ==UserScript==
// @name           LoD Gestion Tool rework (based 3gokushi-avance)
// @namespace      SD3K-HELPER
// @description    LoD  Gestion Tool rework - Powered by brakhtar
// @include        http://sdsam*.nexon.com/*
// @include        http://*.lordsofdynasty.com/*
// ==/UserScript==
// Version 1.0 - première traduction
// Le script original est japonais, celui-ci est issu de la version coréenne.
...
var VERSION_NAME = "LoD Gestion Tool (powered by brakhtar)";
var IMG_DIR = "/20100510-01/img/";

///////////////////////////////////////////////
// Traitement pour Chrome
...
if( OPT_ALLY )      disp_AllianceInfo();//Liens XY appelés pour traitement
...
function initResources()
{
    ...
    for(var i=0 ; i<spns.length ; i++) {
        var str = spns[i].previousSibling.nodeValue.match(/(bois|pierre|acier|céréales)\s+(-?\d+)/);
        if( str ) {
            var name = "";
            switch( str[1] ) {
            case "bois":
...
//////////////////////
//Initialisation panneau de base
//////////////////////
function initPanel()
{
...
function disp_Options()
{
    ...
    cl.innerHTML = "3 Royaumes Gestion tool";
...
//  OPT_DECK = cloadData( "OPT_DECK", 1 );
    ...
    OPT_TTDISTANCE_ITEMS = cloadData("OPT_DISTANCE_ITEMS", "[\"Epeistes(6)\", \"Lanciers(7)\", \"Archers(5)\", \"Cavaliers(12)\", \"Béliers(3)\"]", true, true);
...
//  csaveData( "OPT_REPORT_NP", OPT_REPORT_NP );
    ...
    alert("Sauvegarde des réglages effectuée");
    deleteOptionsHtml();
}
function getMyInfo()
{
...
function resetMapLink() {
    if( !confirm("Voulez vous supprimer le lien enregistré ?") ) return;
...
function resetUserXY() {
    if( !confirm("Alliance원 Liste des Coordonnées Toutes les informations Lors de la suppression Confirmez?") ) return;
    cresetUserXY();
}

function resetUserStar() {
    if( !confirm("Joueur의 ★Information territoires Toutes les informations Lors de la suppression Confirmez?") ) return;
    cresetUserStar();
...
function addOptionsHtml() {
    ...
    //Termes des réglages
    ccreateCheckBox(td1, "OPT_VILLAGE", OPT_VILLAGE, "Affichage construction bâtiments","Construction Impossible en raison du manque de ressources Construction Impossible, LvVoir를 Pour Jaunes.",0);
    ccreateCheckBox(td1, "OPT_BASELINK", OPT_BASELINK, "Affichage raccourcis villages","Lien place centrale, map, gouverneur",0);
    ccreateCheckBox(td1, "OPT_MAPLINK", OPT_MAPLINK, "Raccourcis vers territoires","Territoires sauvegardé dans le cadre Lien",0);
    ccreateTextBox(td1, "OPT_MAPLINK_FONT_SIZE", OPT_MAPLINK_FONT_SIZE, "îcones de raccourcis","Pour l'affichage des raccourcis village", 5, 20);
    ccreateCheckBox(td1, "OPT_LARGEICON", OPT_LARGEICON, "grandes îcones","îcones pour raccourcis de gestion de villages et de liens",0);
    ccreateCheckBox(td1, "OPT_XYLINK", OPT_XYLINK, "Affichage coordonnées Chat/Forum","Affiche en lien les coordonnées(XX,YY) présentes dans le chat/forum.",0);
    ccreateCheckBox(td1, "OPT_XYLINK_NK", OPT_XYLINK_NK, "Syntaxe large","Prends en compe les coordonnées avec une syntaxe XX,YY sans les parenthèse.",20);
    ccreateCheckBox(td1, "OPT_TTBL", OPT_TTBL, "Histoire구성chevalperformance","Forums에 Voir되는 Histoire을 보여줍니다.",0);
    ccreateCheckBox(td1, "OPT_MEMO", OPT_MEMO, "Mémo","mémo simple",0);
    ccreateTextBox(td1, "OPT_MEMO_FONT_SIZE", OPT_MEMO_FONT_SIZE, "taille de la police","mémo taille de la police. Initial 10",5,20);
    ccreateTextBox(td1, "OPT_MEMO_WIDTH", OPT_MEMO_WIDTH, "largeur mémo","largeur du mémo. Initial 20",5,20);
    ccreateTextBox(td1, "OPT_MEMO_HEIGHT", OPT_MEMO_HEIGHT, "hauteur mémo","Hauteur du mémo. Initial 5",5,20);
    ccreateTextBox(td1, "OPT_MEMO_COUNT", OPT_MEMO_COUNT, "Nombre mémos(1~5)","Nombre de mémos. Initial 1",5,20);
    ccreateCheckBox(td1, "OPT_DETAILS", OPT_DETAILS, "en그Total Voirchevalperformance","Alliance en그의 Histoire+원군의 Total를 Voir합니다.",0);
    ccreateCheckBox(td1, "OPT_DETAILS_UP", OPT_DETAILS_UP, "Total를 위에 Voir","Total를 basUne 아닌 위에 Voir 합니다.",20);
//  ccreateCheckBox(td1, "OPT_DECK", OPT_DECK, "デッキ画面改善","デッキ画面の改善を行います",0);
    ccreateCheckBox(td1, "OPT_DECK_SET", OPT_DECK_SET, "Village courant montée Deck","Lors de la montée d'un héros dans le deck, le village sélectionné est le village courant.",0);
    ccreateCheckBox(td2, "OPT_CTIME_B", OPT_CTIME_B, "Temps de remplissage(Batiment)","Afficher le temps avant remplissage des bâtiments.",0);
    ccreateCheckBox(td2, "OPT_CTIME_U", OPT_CTIME_U, "Temps de remplissage(Unité)","Afficher le temps avant les ressources nécéssaires.",0);
    ccreateCheckBox(td2, "OPT_ALLY", OPT_ALLY, "Outil map","Afficher les différents masques d'alliance.",0);
    ccreateCheckBox(td2, "OPT_ALLY_IS", OPT_ALLY_IS, "Alliance Voir Trier","Alliance원 Information에 Trier을 Ajouter합니다.",20);
    ccreateCheckBox(td2, "OPT_ALLY_XY", OPT_ALLY_XY, "Alliance원 Coordonnées Voir","Alliance원들의 본 Bases(본성) Coordonnées를 Ajouter합니다.",20);
    ccreateCheckBox(td2, "OPT_ALLY_CSV", OPT_ALLY_CSV, "Alliance원 Information CSV","Alliance원 상세 Information를 CSVen Voir합니다.",20);
    ccreateCheckBox(td2, "OPT_RES_T", OPT_RES_T, "Production totale","Affiche la production totale.",0);
    ccreateCheckBox(td2, "OPT_RES_TIME", OPT_RES_TIME, "Remplissage Entrepôts","Affiche le temps avant remplissage des dépôts.",0);
    ccreateCheckBox(td2, "OPT_REMOVELIST", OPT_REMOVELIST, "Construction/Destruction des territoires","Affiche les améliorations/abandon des territoires.",0);
    ccreateCheckBox(td2, "OPT_MAPLIST", OPT_MAPLIST, "Tableau des territoires alentours","Tableau d'affichage des territoires alentours sous la map.",0);
    ccreateCheckBox(td2, "OPT_DISTANCE", OPT_TTDISTANCE, "Estimation TT","Affiche l'estimation des temps de trajet.",0);
    createDistanceBox(td2, OPT_TTDISTANCE_ITEMS, 5);
    ccreateCheckBox(td2, "OPT_TTALLYPRSN", OPT_TTALLYPRSN, "Village/Joueur/Alliance","Affiche le nom du village/Joueurs/Alliance associé aux coordonnées.",0);
    ccreateCheckBox(td3, "OPT_USER_STAR", OPT_USER_STAR, " ★ territoires joueurs","Permet d'afficher les ★ sur les profils .",0);
    ccreateCheckBox(td3, "OPT_USER_LEVEL", OPT_USER_LEVEL, "Niveaux territoires","Affiche les niveaux des territoires depuis le profil.",0);
//  ccreateCheckBox(td3, "OPT_REPORT_NP", OPT_REPORT_NP, "同盟ログ前後表示","同盟ログの詳細に、[前のログ]・[次のログ]を追加します（限定的）",0);
    ccreateCheckBox(td2, "OPT_MAPCENTER", OPT_MAPCENTER, "Centre Map","Met en surbrillance le centre de la map visualisée.",0);
    ccreateCheckBox(td3, "OPT_TBREST", OPT_TBREST, "Récupération Vitalité","Affiche le temps avant les 300/500 de vitalité au survol.",0);
    ccreateCheckBox(td3, "OPT_DELMSG", OPT_DELMSG, "Suppression Courriers/Rapports","Gère la suppression directe des courriers et rapports.",0);
    ccreateCheckBox(td3, "OPT_TSENDTIME", OPT_TSENDTIME, "출발temps calcul","출병 화면Dans le 도착 희망temps으en부터 출발temps을 calcul해줍니다.",0);
    ccreateCheckBox(td3, "OPT_SMALLBTN", OPT_SMALLBTN, "Multiserveurs","Permet d'aller d'un serveur à l'autre(inactif).",0);

    ccreateCheckBox(td3, "OPT_ATTACKMAP", OPT_ATTACKMAP, "출병 Voirchevalperformance","맵 위에 Maintenant 출병des Voir를 Ajouter합니다.",0);
    ccreateCheckBox(td3, "OPT_CARD_CMB", OPT_CARD_CMB, "동일 Carte 합성 bouton","Carte 합성 Après에 동일한 Carte 합성을 할 수 있는 bouton을 Ajouter합니다.",0);

    td3.appendChild(d.createElement("br"));
    var fs = d.createElement("fieldset");
    var lg = d.createElement("legend");
    lg.appendChild(d.createTextNode("　Present by Pikachu　") );
    fs.appendChild(lg);
    td3.appendChild(fs);
    ccreateCheckBox(fs, "OPT_PIKA_YOROZU", OPT_PIKA_YOROZU, "Une챠폰 건망 방 Gestionchevalperformance","Une차폰cette 리셋되면 알려줍니다. - Maintenant Une챠폰 Aucun",0);
    ccreateCheckBox(fs, "OPT_PIKA_HPREST", OPT_PIKA_HPREST, "Estimation Récupération","Estime le temps de récupération des Héros.",0);
    ccreateCheckBox(fs, "OPT_PIKA_MAPHELP", OPT_PIKA_MAPHELP, "맵 화면 épée색 Amélioration","XCoordonnées 부분에 999,999 같은 Tapez식으en 입력해도 됩니다.",0);
    ccreateCheckBox(fs, "OPT_PIKA_TRDHELP", OPT_PIKA_TRDHELP, "트레cette드 화면 입력 Amélioration","4자리 숫자를 입력하면 Carte Noen épée색합니다.",0);
    ccreateCheckBox(fs, "OPT_PIKA_BLINKBLD", OPT_PIKA_BLINKBLD, "도시화면 Amélioration","en construction Voir를 깜빡입니다.",0);
    ...
    fs.appendChild(lg);
    td3.appendChild(fs);
    ccreateCheckBox(fs, "OPT_SUZAN_SEISAN", OPT_SUZAN_SEISAN, "Production Bases","Montre la production de la base.",0);

    var fs = d.createElement("fieldset");
    var lg = d.createElement("legend");
    lg.appendChild(d.createTextNode("　Present by ろむ　") );
    fs.appendChild(lg);
    td4.appendChild(fs);
    ...
    ccreateButton(oc, "sauvegarde", "Modifications Sauvegardées.", function() {saveOptions();});
    ccreateButton(oc, "Annuler", "Annule les modifications et ferme ce menu.", function() {deleteOptionsHtml();});
    ccreateButton(oc, "Réinitialisation Liens", "Supprime tous les territoires enregistrés.", function() {resetMapLink();});
    ccreateButton(oc, "Alliance원 Coordonnées Initialisation", "Alliance원 Coordonnées를 모두 Initialisation합니다.", function() {resetUserXY();});
    ccreateButton(oc, "Réinitialisation ★/lvl territoires", "Supprime les données sauvegardées sur les territoires", function() {resetUserStar();});
    ccreateButton(oc, "Modif. liens", "Permet de modifier directement les liens vers les territoires.", function(event) {mapLinkList(event);});
}

function createDistanceBox(container, items, num)
{
    var sels = ["Autres", "Aucun", "Epeistes(6)",
                "Lanciers(7)", "Archers(5)", "Cavaliers(12)", "Eclaireurs(9)", "Béliers(3)",
                "Lancier Evo(10)", "Archer Evo(8)", "Cavalier Evo(15)", "Eclaireur Monté(20)", "Catapulte(6)"];
    for(var i=0 ; i<num ; i++ ) {
        var src = "";
        if( sels.indexOf(items[i]) != -1 ) src = items[i];
        var cb = ccreateComboBox(container, "OPT_DISTANCE_CB" + i , sels, src, "Voir" + (i+1), (i+1) + "행 째 Voir", 20 );

        var tb = d.createElement("input");
        tb.type = "text";
        tb.id = "OPT_DISTANCE_TX" + i;
        tb.title = "exemple) très rapide cheval(50)  과 comme dans les parenthèses taux Entrez.";
        if( src == "" ) {
            tb.value = items[i];
        }else{
...
function disp_village() {
    ...
    var costs = [];
    costs["bois"] = cost_wood;
	costs["pierre"] = cost_stone;
	costs["acier"] = cost_iron;
	costs["céréale"] = cost_rice;
	costs["Archives"] = cost_souko;
	costs["logement"] = cost_syukusya;
	costs["usine"] = cost_kojo;
	costs["aberration"] = cost_suisya;
	costs["marché"] = cost_ichiba;
	costs["labo"] = cost_kenkyu;
	costs["camp"] = cost_kunren;
	costs["forge"] = cost_kajiba;
	costs["usine d'arme"] = cost_bougu;
	costs["usine d'armure"] = cost_heiki;
	costs["동작대"] = cost_doujaku;
	costs["연병소"] = cost_renpei;
	costs["lanciers사"] = cost_heisya;
	costs["archers사"] = cost_yumi;
	costs["écuries"] = cost_uma;
	costs["star"] = cost_shiro;
	costs["Fort"] = cost_toride;
	costs["Village"] = cost_mura;
    ...
    Pika_prepareForDisplayBuildStatus();//Lv0Voir

    var xybld_a = new Array();
    var allbld = $a("//li/span[@class=\"buildStatus\" and contains(text(),\"En construction\")]/a");
    ...
    allbld = $a("//li[contains(text(), \"Suppression\")]/span[@class=\"buildStatus\"]/a");
...
function disp_mapLink()
{
    ...
    //잠정적으en split("\n")Dans le 3cette면 territoireslien, 4면 Joueurlien
    if( location.pathname == "/land.php" ) {
        ...
        if( div ) {
            ...
            lnk.innerHTML = "Ajouter aux Liens";
            div.appendChild(lnk);
...
function saveMapLink( )
{
    if( location.pathname == "/land.php" ) {
        ...
        v_name = prompt("Entrez le nom", v_name);
        if( !v_name ) return;
        ...
        disp_name = prompt("Entrez le nom", user_name);
        if( !disp_name ) return;

        comment = prompt("Entrez le nom", comment);
...
function configMapLink(n, evt)
{
    ...
    if( data.length == 3 ) {
        ccreateTextBox(lw, "LINK_TITLE", data[0], "Nom","Nom",20, 0);
        ccreateTextBox(lw, "LINK_X", data[1], "XCoordonnées","XCoordonnées",10, 0);
        ccreateTextBox(lw, "LINK_Y", data[2], "YCoordonnées","YCoordonnées",10, 0);
    }else if( data.length == 4 ) {
        var dv = d.createElement("div");
        dv.innerHTML = "id=" + data[0] + "　　" + data[1] + "<br>";
        lw.appendChild(dv);
        ccreateTextBox(lw, "LINK_TITLE", data[2], "Voir Nom","Voir Nom",20, 0);
        ccreateTextBox(lw, "LINK_COMMENT", data[3], "설Nom","설Nom",20, 0);
    }
    //réglages bouton
    ccreateButton(lw, "Sauvegarde", "contenu Sauvegarde합니다.", function() {
        var v = "";
        if( data.length == 3 ) {
    ...
    //Suppression
    ccreateButton(lw, "Suppression", "Suppression", function() {
    ...
    //Revenir en Haut
    if( n != 0 ) {
        ccreateButton(lw, "▲", "Revenir en Haut", function() {
    ...
    var maplinks = cloadData( "links", 0, true );
    if( n + 1 < maplinks ) {
        ccreateButton(lw, "▼", "Aller en bas", function() {
            var tmp = cloadData( "link" + (n+1), "", true );
            csaveData( "link" + (n+1), cloadData( "link" + n, "", true ), true );
            csaveData( "link" + n, tmp, true );
            configMapLinkClose();
    ...
    //Fermer
    ccreateButton(lw, "Fermer", "Fermer", function() {configMapLinkClose();});
    ...
    for(var i=0 ; i<maplinks ; i++) {
        ...
        if( tmp != "" ) {
            ...
            if( data.length == 3 ) {
                ...
                link1.title = "Voir(" + data[1] + "," + data[2] + ")";
                ...
                link2.title="[" + data[1] + "]편 Gestion 보내cheval";
...
//////////////////////
//Lire liens de territoires et Sauvegarde préférences
//////////////////////
...
//////////////////////
//Traitement des liens de coordonnées
//////////////////////
function disp_XYLink()
{
    ...
    if( location.pathname == "/alliance/info.php" ) {
        targetPath.push( "//tr[th[contains(text(),\"설Nom\")]]/td" );
    }
    ...
    function setXYLink(elm) {
        ...
        function XYLinkReg() {
            ...
            var txt = '<a href="'+caddSessionId('/land.php?x=' + x + '&y=' + y + '#ptop')+'" title="Voir" style="display:inline;">' + disp + '</a>';
            ...
            //Choses taggés avec une reconnexion Révision
            if( elm_child_a ) {
...
//      "//span[@class='xy']"
    ...
    //  tmpHTML = tmpHTML.replace(/[\n\r]/g,"");
        var reg = /Epeistes\s+Lanciers\s+Archers\s+Cavaliers\s+Lancier Evo\s+Archer Evo\s+Cavalier Evo\s+Eclaireurs\s+Eclaireur Monté\s+Béliers\s+Catapulte\s+武将.*(\n兵士\s+|\n)(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+).*(\n死傷\s+|\n)(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/g;
        tmpHTML = tmpHTML.replace(reg,
            function() {
                var txt = "<table class='tables'><tr><th class='solClass'>　</th>" +
                            "<th class='solClass'>Epeistes</th><th class='solClass'>Lanciers</th><th class='solClass'>Archers</th><th class='solClass'>Cavaliers</th>" +
                            "<th class='solClass'>Lancier Evo</th><th class='solClass'>Archer Evo</th><th class='solClass'>Cavalier Evo</th><th class='solClass'>Eclaireurs</th>" +
                            "<th class='solClass'>Eclaireur Monté</th><th class='solClass'>Béliers</th><th class='solClass'>Catapulte</th><th class='solClass'>武将</th>" +
                            "</tr><tr><th class='solClass'>Soldat</th>";
                for(var i = 2 ; i<= 13 ; i++) {
                    txt += "<td>" + arguments[i] + "</td>";
                }

                txt += "</tr><tr><th class='solClass'>Pertes</th>";
                for(var i = 15 ; i<= 26 ; i++) {
                    txt += "<td>" + arguments[i] + "</td>";
                }
                txt += "</tr></table>";
        ...
        reg = /Epeistes\s+Lanciers\s+Archers\s+Cavaliers\s+Lancier Evo\s+Archer Evo\s+Cavalier Evo\s+Eclaireurs\s+Eclaireur Monté\s+Béliers\s+Catapulte\s+武将.*(\n兵士\s+|\n)(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/g;
        tmpHTML = tmpHTML.replace(reg ,
            function() {
                var txt = "<table class='tables'><tr><th class='solClass'>　</th>" +
                            "<th class='solClass'>Epeistes</th><th class='solClass'>Lanciers</th><th class='solClass'>Archers</th><th class='solClass'>Cavaliers</th>" +
...
function disp_Details()
{
    ...
    var tbls = $a("//table[@class=\"tables\"][@summary=\"Défense\"]");
    ...
    th.appendChild(d.createTextNode("Défense Totale"));
    tr.appendChild(th);
    ...
    var tr = d.createElement("tr");
    var thtxt = ["Epeistes","Lanciers","Archers","Cavaliers","Lancier Evo","Archer Evo","Cavalier Evo","Eclaireurs","Eclaireur Monté","Béliers","Catapulte","Armée" ];
    var th = d.createElement("th");
    th.className = "blank";
    tr.appendChild(th);
    for(var i=0; i<12 ; i++){
...
//////////////////////
//Voir temps de remplissage(Batiment)
//////////////////////
...
function disp_CompleteTimeBuild()
{
    ...
    if( location.pathname == "/village.php" ) {
        //finale constructiontemps Sauvegarde
        var spans = $a('id("actionLog")//li//span[@class="buildStatus" and (contains(text(),"en construction") or contains(text(),"préparer la construction"))]/../span[@class="buildClock"]');
    ...
    function timerfunc() {
        ...
        for(var i = 0 ; i<spans.length ; i++){
            ...
            if( ctime ) {
                span.innerHTML = "　(" + ctime + "Remplissez)";
            }
        }
...
function disp_CompleteTimeUnit()
{
    if( location.pathname == "/facility/facility.php" ) {
        ...
        //Unité génération finale temps Sauvegarde
...
function disp_AllianceInfo()
{
    ...
    function allianceSort()
    {
        ...
        for(var i=0 ; i<8 ; i++) {
            var th = $x("//table[@summary=\"Note\"]//tr[2]//th[" + (i+1) + "]");
            if( th ) {
                ...
                img.alt = "Trier par ordre croissant";
                img.title = img.alt;
                a.appendChild(img);
                ...
                img.alt = "Trier par ordre décroissant";
                img.title = img.alt;
                a.appendChild(img);
        ...
        function row_sort(col, kind, order) {
            ...
            var strs = new Array();
            if( col == 2 || kind == "xy" ) {
                //Nom ou Coordonnées après les balises
    ...
    //Alliance원 Coordonnées Voir
    function allianceXY()
    {
        //Coordonnées collection
        ...
        if( location.pathname == "/alliance/info.php" ) {
            if( ALLY_ID || URL_PARAM.id ) {
                ...
                var head1 = $x("//table[@summary=\"Note\"]//tr[1]//th[@class=\"ttl\"]");
                if( head1 ) {
                    head1.setAttribute("colspan", parseInt(head1.getAttribute("colspan"),10) + 1 + "");
                }
                var head2 = $x("//table[@summary=\"Note\"]//tr[2]");
                if( head2 ) {
                    var th = d.createElement("th");
                    th.className = "all";
                    th.style.width = "110px";
                    th.id = "beyond_ally_xy";
                    th.innerHTML = "Coordonnées";
                    var lnk = d.createElement("a");
                    lnk.href = "javascript:void(0)";
                    lnk.innerHTML ="(GET)";
                    ...
                    $e(lnk, "click", function() {
                        ...
                        if( confirm("Cette action affiche la difficulté.\n /!\Lorsque répété plusieurs fois, le serveur croit en une attaque DOS.") == false ) return;
                        running = true;
//                      cdeleteUserXY(aid);
                        var trs = $a("//table[@summary=\"Note\"]//tr[position()>2]");
                        ...
                        function timerFunc()
                        {
                            ...
                            if( !idtd ) {
                                alert("Erreur : Le format semble avoir changé.");
                                return ;
                            }
                            ...
                            if( tmp.innerHTML != "　" ) {
                                ...
                                if( now_num < all_num ) {
                                    ...
                                    running = false;
                                    alert("Toutes les Coordonnées confirmation.");
                                }
                                return ;
                            }
    ...
    {
        var xy = html.match(/<\/a>\(Stats\)<\/td>[^<]*<td[^>]*>([\-0-9]+),([\-0-9]+)<\/td>/);
        if( xy ) {
    ...
    {
        var ret = [];
        var tmp = html.match(/<td[^>]*>rang<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["all_rank"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Général<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["all_point"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Population<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["jinko"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Attaque<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["attack"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Défense<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["defence"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Score de victimes<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["attack_score"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>Score de defense<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["defence_score"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<\/a>\(Accueil\)<\/td>[^<]*<td[^>]*>([\-0-9]+),([\-0-9]+)<\/td>/);
        if ( !tmp ) return null;
    ...
    {
        var ret = new Array();
        var tmp = html.match(/<td[^>]*>Joueur<\/td>[^<]*<td[^>]*>([^<\s]+)/);
        if( !tmp ) return null;
        var user_name = tmp[1];

        tmp = html.match(/<td[^>]*>Alliance<\/td>[^<]*<td[^>]*><a href="[^"]*">([^<]+)<\/a><\/td>/);
        if( !tmp ) return;
        var ally_name = tmp[1];
...
*/
        ...
        $e(lnk, "click", function() {
            ...
            var csv_txt = "同盟内ランク\tuser_id\t名前\t同盟内point\t寄付\t拠点\t全体ランク\t全体point\t人口\t攻撃\t防御\t撃破スコア\t防御スコア\t本拠XCoordonnées\t本拠YCoordonnées\n";
        ...
        lnk2.innerHTML ="同盟員全領地Coordonnéescsv";
        ...
        function createCSVWindow(x, y)
        {
            ...
            ccreateButton(cc, "Fermer", "", function(){
                var cc = $("beyond_csvWindow");
                if( cc ) cc.parentNode.removeChild(cc);
            });
...
function disp_RemoveList()
{
    ...
    var elms = ccreateSideBox("beyond_sidebox_removelist", icon_rl, "En construction/破棄ﾘｽﾄ");
    ...
    for(var i=0 ; i<lists.length ; i++) {
        ...
        addHtml += '<a href="'+caddSessionId('/land.php?x=' + lists[i].x + '&y=' + lists[i].y +'#ptop')+'" title="Voir"'+ sizestyle + '>' + lists[i].time.substr(-8) + '</a>';
...
function disp_ResourcesTime()
{
    ...
    for( var i=0 ; i< bldtbl.length ; i++ ){
        if( RES_MAX.fame < bldtbl[i] ) {
            var base = $x('id("status_left")/img[@title="Réputation"]');
            if( base ) {
                var dv = d.createElement("div");
    ...
    $e(a, "click", function() {
        ...
        function initTable()
        {
            ...
            tbl.innerHTML = "<tr>" +
                    ...
                    "<th>人口</th><th>領地名</th><th>Coordonnées</th><th>距離</th><th>戦力</th>" +
                    "</tr>";
...
function disp_ToolTipsDistance()
{
    ...
    function saveTrainingLevel()
    {
        ...
        var level = 0;
        var elevel = 0;
        var map = $a('//area[contains(@alt, "Camp")]');;
        if(0 < map.length) {
            for (var i = 0;i < map.length;i++) {
                var lv = map[i].alt.match(/((?:遠征)?Camp) LV.([0-9]+)/);
                if(lv[1] == "Camp") {
                    ...
                    if( RES_NOW.fame >= meisei ) {
                        ...
                        if( xy ) {
                            var lnk =  '<a href="'+caddSessionId('/territory_proc.php?x=' + xy[1] + '&y=' + xy[2] + '&mode=lvup')+'" title="レベルアップ" onclick="';
                                lnk += "return confirm('Réputation" + meisei + "を消費し、領地をレベルアップしますか？');";
    ...
    function ToubatsuRecoveryEstimates()
    {
        ...
        for( var i=0; i<decks.length ; i++ ){
            ...
            if( tb != 0 ) {
                ...
                var dayText = caddDate(now, timeText);
                var txt = "500まで" + timeText + "後 (" + dayText + "Remplissez)";
        ...
        for( var i=0; i<files.length ; i++ ){
            ...
            if( tb != 0 ) {
                var timeText = getTime( tb );
                var dayText = caddDate(now, timeText);
                var txt = "300まで" + timeText + "後 (" + dayText + "Remplissez)";
...
function cgetXYHtml(x,y)
{
    ...
    var txt = '';
    txt += '<a href="'+caddSessionId('/land.php?x=' + x + '&y=' + y + '#ptop')+'" title="Voir" style="color:#0099cc; text-decoration: none;" onmouseover="';
...
function cgetMapNofromXY(x, y, base_x, base_y, type)
{
    ...
    //map.php専用のXYCoordonnées→mapAll999
    var sc = 11;
    var hosei = 0;
    if( type == 2 ) {
        sc = 15;
...
{
    var tds = $a('//table[@class="commonTables"]//th[contains(text(),"Coordonnées")]/../preceding-sibling::tr/*[contains("tdTDthTH",name())][last()]');
    for(var i=0 ; i<tds.length ; i++) {
        tds[i].colSpan++;
    }

    var tr = $x('//table[@class="commonTables"]//th[contains(text(),"Coordonnées")]/..');
    var th = d.createElement("th");
    th.className = "ttl4";
    th.id = id + "_title";
    ...
    var trs = $a('//table[@class="commonTables"]//th[contains(text(),"Coordonnées")]/../following-sibling::tr');
    for(var i=0 ; i<trs.length ; i++) {
...
function Pika_displayBuildStatus() {
    ...
    var xpath = 'id("actionLog")//li/span[@class="buildStatus" and contains(.,"En construction")]/a';
    var nodes = $a(xpath);
    var facilities = {};
    for (var i = 0; i < nodes.length; ++i) {
    ...
    var deleting = $x('//*[@id="actionLog"]/ul/li[contains(text(), "Suppression")]/span[@class="buildStatus"]/a');
...
function Suzan_Seisan(inner)
{
    ...
    var areas = $a('//area[contains(@alt,"Céréales") or contains(@alt,"穀") or contains(@alt,"伐") or contains(@alt,"森") or contains(@alt,"pierre") or contains(@alt,"岩") or contains(@alt,"製") or contains(@alt,"鉱")]');
    ...
    var text = '<ul>';
    text += '<li><img align="middle" alt="bois" src="' + IMG_DIR + 'common/ico_wood2.gif"> bois  ' + seisan.w + '</li>';
    text += '<li><img align="middle" alt="pierre" src="' + IMG_DIR + 'common/ico_stone2.gif"> pierre  ' + seisan.s + '</li>';
    text += '<li><img align="middle" alt="acier" src="' + IMG_DIR + 'common/ico_iron2.gif"> acier  ' + seisan.i + '</li>';
    text += '<li><img align="middle" alt="céréales" src="' + IMG_DIR + 'common/ico_lice2.gif"> céréales  ' + seisan.r + '</li>';
...
function disp_nextFameTimer() {
    ...
    var img = $x('id("status_left")/img[@title="Réputation"]');
            ...
            cajaxRequest("/facility/castle.php" , "GET", "", function(req) {
                ...
                if (isPreCheckDateOver == false && nextDate.getTime() == tmpDate.getTime()) {
                    ...
                    if (cloadData("NextFameTime",null,true,true)) {
                        var fameText = $x('id("status_left")//img[@title="Réputation"]').nextSibling;
...
function initVillages() {
    ...
    function _profileInit() {
        var compList = new Object();
        var matches = null;
        var villageLinks = $a('//table[@class="commonTables"]//th[contains(text(),"Coordonnées")]/../following-sibling::tr[position() <= 10]/td[not(@*) and 1 <= normalize-space(text())]/..//a');
