...
// @description    Allows you to easily filter your Tumblr dashboard
...
// MooTools 1.11
// Modules: Core, Class, Native, Element, Window, Drag.Base, XHR, Json, Slider
// Jump to line 4241 to get to the actual script or search for "// End MooTools"
...
// Some globals
var switchboard = Json.evaluate(GM_getValue('switchboard', '{}'));
var lookup = {};
var masterVolume = GM_getValue('volume', 1);
var infiniteScroll = GM_getValue('infiniteScroll', 1);

window.addEvent('domready', function() {
    
    // Add the volume controls for each avatar
    
    setupPosts($$('.post'));
    
    // Set up the filter elements
    
    var filter = new Element('div', {
        'id': 'filter'
    });
    filter.injectInside($('content'));
    
    var scrollControl = new Element('div', {
        'id': 'infiniteScroll'
    });
    scrollControl.injectInside(filter);
    
    var label = new Element('label');
    label.setHTML('Infinite scroll');
    label.injectInside(scrollControl);
    
    var checkbox = new Element('input', {
        'type': 'checkbox',
        'checked': infiniteScroll ? 'checked' : '',
        'events': {
            'change': function() {
                infiniteScroll = checkbox.checked;
                GM_setValue('infiniteScroll', infiniteScroll);
            }
        }
    });
    checkbox.injectTop(label);
    
    var area = new Element('div', {
        'id': 'area'
    });
    area.injectInside(filter);
    ...
    var knob = new Element('div', {
        ...
        'events': {
            'mouseover': function() {
                setting.addClass('hover');
            },
            'mouseout': function() {
                setting.removeClass('hover');
            }
        }
    });
    knob.injectInside(area);
    
    setting.appendText('Attenuation: 1');
    setting.injectInside(filter);
    
    var loading = new Element('div', {
        'id': 'loadingPage'
    });
    loading.appendText('Loading next page');
    loading.injectInside(document.body);
    
    // Add some CSS goodness
    
    GM_addStyle(".avatar .volume { height: 20px; text-align: center; font-size: 11px; } " +
                ".avatar .volume .control { margin: 0 2px; display: none; text-decoration: none; } " +
                ".avatar:hover .volume .control { display: inline; } " +
                ".is_mine .avatar:hover .volume { display: none; } " +
                "#filter { position: absolute; left: 362px; top: 18px; font-size: 11px; color: #A8A8A8; } " +
                "#area { width: 100px; height: 10px; background: #EAEAEA; margin: 5px 5px 0 0; position: relative; } " +
                "#knob { width: 10px; height: 10px; background: #A8A8A8; cursor: pointer; position: absolute; } " +
                "#knob:hover { background: #3E4D5C; } " +
                "#setting { margin: 3px 0 15px 0; } " +
                ".elipses { display: none; padding: 10px; } " +
                ".quiet .post_container { display: none; } " +
                ".quiet .elipses { display: block; } " +
                "#infiniteScroll { margin-bottom: 5px; } " +
                "#infiniteScroll input { margin: 0 3px 0 0; } " +
                "#infiniteScroll label { cursor: pointer; } " +
                "#loadingPage { display: none; position: fixed; right: 10px; bottom: 10px; font: 11px verdana, sans-serif; color: #8A9EB4; } " +
                "#filter .hover, #filter label:hover { color: #3E4D5C; } ");
    
    var slider = new Slider(area, knob, {
        'steps': 9,
        'onChange': function(step) {
            GM_setValue('volume', step);
            setting.setHTML('Attenuation: ' + step);
            setAttenuation(step);
        }
    }).set(masterVolume);
    ...
    window.addEvent('scroll', function() {
        var offset = window.getScrollHeight() - window.getScrollTop();
        if (offset < window.getHeight() * 2 && !loadingPage && infiniteScroll) {
            loadingPage = true;
            $('loadingPage').style.display = 'block';
            if ($$('#pagination a').length == 2) {
                var next = $$('#pagination a')[1];
            } else {
                var next = $$('#pagination a')[0];
            }
            new XHR({
                'method': 'get',
                'onSuccess': function() {
                    
                    var nextPage = this.response.text;
                    var start = nextPage.indexOf('<ol id="posts" >') + 16;
                    var length = nextPage.indexOf('</ol>') - start;
                    var temp = new Element('ol', {
                        'id': 'tempPosts',
                        'styles': {
                            'display': 'none'
                        }
                    });
                    temp.setHTML(nextPage.substr(start, length));
                    temp.injectInside(document.body);
                    
                    var tempPosts = $$('#tempPosts li');
                    setupPosts(tempPosts);
                    tempPosts.each(function(li) {
                        li.remove();
                        li.injectInside($('posts'));
                    });
                    temp.remove();
                    
                    var navURLs = nextPage.match(/href="\/dashboard\/(\d+)"/g);
                    var nextURL = navURLs[1].substr(6, navURLs[1].length - 7);
            ...
            }).send(next.getAttribute('href'));
        }
    });
});

function refresh() {
    setAttenuation(masterVolume);
}
...
function setupPosts(posts) {
    ...
    posts.each(function(post) {
        ...
        var avatar = post.getElement('.avatar');
        if (avatar) {
            
            var url = avatar.getElement('a').getAttribute('href');
            lastUrl = url;
            
            var parent = new Element('div', {
                'class': 'volume'
            });
            parent.injectInside(avatar);
            
            var status = new Element('span', {
                'class': 'control'
            });
            
            var down = new Element('a', {
                'href': '#down',
                'class': 'control',
                'events': {
                    'click': function(e) {
                        new Event(e).preventDefault();
                        var volume = parseInt(status.innerHTML);
                        if (volume > 0) {
                            var newVolume = volume - 1;
                        } else {
                            newVolume = volume;
                        }
                        setVolume(status, url, volume, newVolume);
                    }
                }
            });
            down.appendText('-');
            down.injectInside(parent);
            
            status.injectInside(parent);
            
            var up = new Element('a', {
                'href': '#up',
                'class': 'control',
                'events': {
                    'click': function(e) {
            ...
            up.appendText('+');
            up.injectInside(parent);
        } else {
            var url = lastUrl;
        }
        
        // Add the elipses (shown when it's too quiet)
        
        var elipses = new Element('div', {
            'class': 'elipses'
        });
        elipses.injectInside(post);
        elipses.appendText('...');
        ...
        var volume = 0;
        if (switchboard[url]) {
            volume = switchboard[url];
        }
        post.addClass('volume' + volume);
        status.appendText(volume);
        
        if (!lookup[url]) {
            lookup[url] = [post];
        } else {
            lookup[url].push(post);
        }

    });
}

function setAttenuation(vol) {
    for (var i = 0; i < 10; i++) {
        if (i >= vol) {
            $$('.volume' + i).each(function(post) {
                post.removeClass('quiet');
            });
        } else {
            $$('.volume' + i).each(function(post) {
                if (post.hasClass('not_mine')) {
                    post.addClass('quiet');
                }
            });
        }
...
function setVolume(status, url, vol1, vol2) {
    lookup[url].each(function(p) {
        p.removeClass('volume' + vol1);
    });
    lookup[url].each(function(p) {
        p.addClass('volume' + vol2);
    });
    switchboard[url] = vol2;
    status.setHTML(vol2);
    refresh();
    GM_setValue('switchboard', Json.toString(switchboard));
}