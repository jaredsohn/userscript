// ==UserScript==
// @name           Contagem Membros Nomades
// @namespace      Nomades
// @description    Alterado por 100por1. Adcionado suporte a indioma português, e traduziso para português..
// @author         Luca Saba - N-I-K-O - Editado by 100por1
// @version		   0.01
 ...
 * - Adcionado suporte a indioma português, e traduziso para português..
 */
...
var lang={
  ...
  br: {'newAlert': 'Novo Nômade', 'newTotal': 'Total de Nômades', 'aband': 'Nômade deixando a casa', 'totAban': 'Total de abandonos', 'confirm': 'Você tem certeza que quer zerar os pontos?'},
}

var local = 'br'; //For i18n
...
confCell.innerHTML = "<img style='float: left' src='http://www.prositos.no/images/config.png' alt='Configuração do status dos Nômades' title='Configuraçãa' id='ainfoConfig'>";
confCell.innerHTML += "<img style='float: right' src='http://bioinf.scri.ac.uk/strudel/images/reset.png' alt='Zerar o contador' title='Reserta' id='ainfoReset'>";
...
function showToolConfig(e)
{
  ...
  else
  {
    ...
    div.innerHTML = '<h3 class="header" style="padding-top: 8px; height: 20px; background-image: url(http://s3.ikariam.it/skin/layout/bg_sidebox_header.jpg)"><b>Configurações Status</b></h3>'; 
    ...
    div.innerHTML += '<br><br><input id="eToolConfButton" type="button" class="button" value="Salvar">';
...
function getActualData()
{
  ...
  for(i=1; i < membersTab.rows.length - 1; i++)
  {
    ...
    if(typeof(recorded_points[pName]) === 'undefined')
	  {
	    membersTab.rows[i].cells[points_cell_idx].innerHTML += " (<font color='#F00'>Nova entrada</font>)";
...
function template(status, rowElement, lastOnline)
{
  if(status == 'online')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML="<div style='width: 8em'><span style='float: right'><img src='http://board.br.ikariam.com/wcf/icon/onlineS.png'></span></div>";
        break;
      default:
  ...
  else if(status == 'inactive')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML = "<div style='width: 8em'><span style='float: left'>("+lastOnline + ")</span><span style='float: right'><img src='http://www.thaisbicalho.com.br/img/navegacaoSlideshowInativo.png'></span></div>";
        break;
      default:
        rowElement.cells[0].innerHTML = "<B><font color='#708090' SIZE='1'>("+lastOnline + ")INATIVO</FONT></B>";        
        break;
    }
  }
  else if(status == 'offline')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML = "<div style='width: 8em'><span style='float: left;'>("+lastOnline+")</span><span style='float: right'><img src='http://board.br.ikariam.com/wcf/icon/offlineS.png'></span></div>";
        break;
      default:
