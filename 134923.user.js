...
// @version        1
// @namespace      Thal
// @homepage       http://userscripts.org/scripts/show/134923
...
var Options = {
  ...
  spamconfig   : {pbSpamState1:"Alliance Chat", pbSpamState2:"Global Chat", pbSpamMin:"Post Automatically Every ", pbSpamAd:"Spam", Defcon:{ Def5:"Pace (Periodo di tranquillità nessuna richiesta ai players)", Def4:"Guerra imminente, prepararsi riassettare città da guerra", Def3:"Guerra,Attacco Difesa situazione stabile", Def2:"la Guerra và male,stiamo subendo iniziamo solo a difendersi truppe in santuario e muri organizzati", Def1:"La Guerra è Persa, il Cda non può + difendere l'ally,trattative resa in atto,siete autorizzati a rifugiarvi altrove fino a cessazione ostilità (DEFCON 1 massimo periolo ally a rischio)",},}, 
...
function pbStartup (){
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    table.pbTabPad2 tr td { padding-left: 20px;}\
  ...
  window.name = 'PT';
  logit ("* KOC Streakbot v"+ Version +" Loaded");
  readUpgradeData();
  readUpgradeStats();
  readOptions();
  readChatOptions();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('Streakbot', eventHideShow, mouseMainTab);
  tabManager.init (mainPop.getMainDiv());
  actionLog ("StreakBot v"+ Version +" Loaded  (KofC version: "+ anticd.getKOCversion() +")");
 ...
 Tabs.Raid = {
  ...
  function InitResourceRequest() {
	...
	div.style.margin = '0 0 0 0';
	   div.appendChild(Builder.BlueButton(" hola ok ", atHola));
		div.appendChild(Builder.BlueButton(" Ciao ", atCiao));
		div.appendChild(Builder.BlueButton(" sto Ok ", atStook));

	   
		div.appendChild(Builder.BlueButton(" Stai ", atStai));
		div.appendChild(Builder.BlueButton(" Mando ", atMandoio));
		div.appendChild(Builder.BlueButton(" Situazione ", atSituaz));
		div.appendChild(Builder.BlueButton(" Truppa ", atTruppate));
		div.appendChild(Builder.BlueButton(" notte ", atNotte));
		div.appendChild(Builder.BlueButton(" dopo ", atDopo));
		div.appendChild(Builder.BlueButton(" Uppa ", atTrono));
		div.appendChild(Builder.BlueButton(" Buondì ", atGiorno));
		
		div.appendChild(Builder.BlueButton(" Serve ", atServe));
		div.appendChild(Builder.BlueButton(" Grz ", atGrz));
		div.appendChild(Builder.BlueButton(" prego ", atPrego));
		div.appendChild(Builder.BlueButton(" LOL ", atLol));
		div.appendChild(Builder.BlueButton(" si ", atSi));
		div.appendChild(Builder.BlueButton(" No ", atNo));
		div.appendChild(Builder.BlueButton(" forse ", atForse));
		div.appendChild(Builder.BlueButton(" Fermo ", atFermo));
		div.appendChild(Builder.BlueButton(" Richiama ", atRichiama));
	
	div.appendChild(Builder.BlueButton(" manda ", atManda));
	div.appendChild(Builder.BlueButton(" non manda ", atNonmanda));
		div.appendChild(Builder.BlueButton(" Dist ", atDist));
		div.appendChild(Builder.BlueButton(" Arriva ", atArriva));
	
	div.appendChild(Builder.BlueButton(" Trasp.ini ", atTrasportatori));
	div.appendChild(Builder.BlueButton(" SS ", atSs));
	div.appendChild(Builder.BlueButton(" Spie ", atSpie));
	div.appendChild(Builder.BlueButton(" Picche ", atPicche));
	div.appendChild(Builder.BlueButton(" Spade ", atSpade));
	div.appendChild(Builder.BlueButton(" Archi ", atArchi));
	div.appendChild(Builder.BlueButton(" Ini ", atIni));
	div.appendChild(Builder.BlueButton(" Oni ", atHC));
	div.appendChild(Builder.BlueButton(" carri ", atcarri));
	div.appendChild(Builder.BlueButton(" Ballista ", atBall));
	div.appendChild(Builder.BlueButton(" Ariete ", atAriete));
	div.appendChild(Builder.BlueButton(" Cata ", atCat));
...
function atManda(){
	...
	//alert(e.id);
	Tools.type(e, "/a Plz Manda ");
	var b = e.nextSibling;
	Tools.click(b);
}
function atTrasportatori(){
	var e = Tools.getElementByXPath("//*[@id='mod_comm_input']");
	if (e == null) { alert("XPath not found!"); return; }
	//alert(e.id);
	Tools.type(e, "/a Trasportatori");
	var b = e.nextSibling;
	Tools.click(b);
}
function atSs(){
...
function atDist(){
	...
	if (e == null) { alert("XPath not found!"); return; }
	//alert(e.id);
	Tools.type(e, "/a Qual'è la distanza??");
	var b = e.nextSibling;
	Tools.click(b);
}
function atArriva(){
...
Tabs.AutoCraft = {
	...
	init: function(div){
        ...
        m+="</table><b>Note:</b> If you complete more than one Item, the creation will be done randomly. <BR> <b>Important: Min. 5000 Aethestones and Refresh to Update the Inventar!</b> ";
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=pbupdate type=checkbox '+ (GlobalOptions.pbupdate?'CHECKED ':'') +'/></td><TD> '+ htmlSelector({0:'', 1:'Google Code'},GlobalOptions.pbupdatebeta,'id=pbupdatebeta') +' (all domains) &nbsp; &nbsp; <INPUT id=pbupdatenow type=submit value="" /></td></tr>\
        ...
        <TR><TD><INPUT id=deletes2toggle type=checkbox /></td><TD> Auto delete Attack target regardless of type</td></tr>\
...
function KOCnotFound(secs){
  ...
  div.innerHTML = '<DIV style="font-size:18px; background-color:#a00; color:#fff"><CENTER><BR>StreakBot has detected that KOC is not loaded<BR>\
    ...
    var t = myBO.Spam;
        ...
        var m = '<DIV class=boStat>'+lang.Spam.title+'</div><TABLE class=pbTab width=100% height=0% ><TR align="center">';
           ...
           if (Options.spamconfig.spamstate == 'a') {
            ...
            m += '</tr></table></div>';
           m += '<DIV class=boStat>Set</div><TABLE class=pbTab>';
     ...
     //           type="radio" name="sex"
		 	...
		 	m += '<TD align=left><INPUT type="radio" name="st" id=idAvviso'+ (Options.SpamMode.Avv?' CHECKED ':'') +'>'+uW.g_js_strings.commonstr.status+' &nbsp;</td><TD> &nbsp; &nbsp; &nbsp; '+ htmlSelector({def5:'OtoMesaj-5', def4:'OtoMesaj-4', def3:'OtoMesaj-3',def2:'OtoMesaj-2',def1:'OtoMesaj-1'},Options.spamconfig.def,'id=BOselectDefMode') +'</td></tr>';
            m += '</table><BR>';
            m += '<div style="width:500px; max-width:500px; height:auto;"; id=IdScoutAllianceProgress>'
            		+'<font color=red><b>Allarme-5</b>: </font><b>'+lang.Spam.Defcon.Def5
            		+'<br><font color=red><b>Allarme-4</b>: </font>'+lang.Spam.Defcon.Def4
            		+'<br><font color=red><b>Allarme-3</b>: </font>'+lang.Spam.Defcon.Def3
            		+'<br><font color=red><b>Allarme-2</b>: </font>'+lang.Spam.Defcon.Def2
            		+'<br><font color=red><b>Allarme-1</b>: </font>'+lang.Spam.Defcon.Def1
            		+'</div></b>';
...
spamnow: function(){
if("def1"==Options.spamconfig.def)var StatDefcon="OtoMesaj-1";else"def2"==Options.spamconfig.def?StatDefcon="OtoMesaj-2":"def3"==Options.spamconfig.def?StatDefcon="OtoMesaj-3":"def4"==Options.spamconfig.def?StatDefcon="OtoMesaj-4":"def5"==Options.spamconfig.def&&(StatDefcon="OtoMesaj-5");	
 ...
 toggleon: function(obj){
  ...
  if (Options.spamconfig.aspam == true) {
   ...
   obj.value = "OtoMesaj = KAPALI";
  }
  else {
   Options.spamconfig.aspam = true;
   obj.value = "OtoMesaj = AÇIK";
   SpamEvery.init();
  }
  saveOptions ();
...
function CdialogConfirm (msg, canNotify, contNotify, centerElement){
  ...
  if (centerElement)
    ...
    pop.centerMe(document.body);
  pop.getTopDiv().innerHTML = '<CENTER> StreakBot</center>';
...
//		alert(t.deleting);
		...
		for (k in unsafeWindow.kocThroneItems) {
			...
			if (throne_item.isBroken == false && l3 != true && throne_item.quality < Options.ThroneDeleteLevel && throne_item.isEquipped == false && throne_item.level == 0 && countItem > Options.throneSaveNum && rangeIndex1 < Options.RangeSaveModeSetting) {
...
/** window display **/   
  init : function (div){
    ...
    setTimeout(function(){ t.Rounds(1,0,0);}, 14*1000);


    t.myDiv = div;
    var selbut=0;
    var m = '<DIV id=pbTowrtDivF class=pbStat>Auto Attack</div><TABLE id=pbcrestfunctions width=100% height=0% class=pbTab><TR align="center">';
    ...
    m += '<TD><INPUT id=pbsendreport type=checkbox '+ (Options.crestreport?' CHECKED':'') +'\> ';
    m += '<INPUT id=pbsendcrestreportint value='+ Options.CrestMsgInterval +' type=text size=3 \> hours </td></table>';
  
    m += '<DIV id=pbOpt class=pbStat>Attack</div><TABLE id=pbcrestopt	 width=100% height=0% class=pbTab><TR align="center"></table>';
    m += '<DIV style="margin-bottom:10px;">Attack from city: <span id=crestcity></span></div>';
    
    m += '<TABLE class=ptTab><TR><TD>Target coords: X:<INPUT id=pbcrestx type=text size=3 maxlength=3 value=""></td>';
...
/** Show Crest Targets **/
    showCrestRoute : function () {
		...
		t.popCrestTargets.getTopDiv().innerHTML = '<TD><B>Attack Targets:</td>';
		t.paintCrestTargets();
		t._addTabHeader();
...
/** Cancel Crest Target **/
		...
		new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {	
			 ...
			 onSuccess: function (transport) {		
				...
				if (rslt.ok) {
					...
					saveCrestData();
					setTimeout (function(){callback(r,0,CrestDataNum);}, 13000);	
					return;
					
				} else {

					if (rslt.user_action) {
						new CdialogCancelContinue('<SPAN class=boldRed>CAPTCHA ALERT! Turn off crest!</span>', null, null, mainPop.getMainDiv);
						logit('send march captcha')
						setTimeout (function(){callback(r,retry,CrestDataNum);}, 1*60*1000);
						return;
					}
					setTimeout (function(){callback(r,retry,CrestDataNum);}, 9000);
					return;
				}
			},
			 onFailure: function () {
				setTimeout (function(){callback(r,retry,CrestDataNum);}, 25000);
				return;
			 }
  		});	
	},
	...
	Rounds : function (r, retry, CrestDataNum) {
		...
		switch (retry) {
			case 10:
				setTimeout(function(){ t.Rounds(r,retry,CrestDataNum);},15000);
				return;
				break;
			case 20:
				setTimeout(function(){ t.Rounds(r,retry,CrestDataNum);},15000);
		...
		if (parseInt(Seed.units[cityID]['unt2']) < CrestData[CrestDataNum].R1MM || parseInt(Seed.units[cityID]['unt3']) < CrestData[CrestDataNum].R1Scout || parseInt(Seed.units[cityID]['unt10']) < CrestData[CrestDataNum].R1Ball || parseInt(Seed.units[cityID]['unt12']) < CrestData[CrestDataNum].R1Cat || parseInt(Seed.units[cityID]['unt2']) < CrestData[CrestDataNum].R2MM || parseInt(Seed.units[cityID]['unt3']) < CrestData[CrestDataNum].R2Scout || parseInt(Seed.units[cityID]['unt4']) < CrestData[CrestDataNum].R2Pike || parseInt(Seed.units[cityID]['unt5']) < CrestData[CrestDataNum].R2Sword || parseInt(Seed.units[cityID]['unt6']) < CrestData[CrestDataNum].R2Arch || parseInt(Seed.units[cityID]['unt10']) < CrestData[CrestDataNum].R2Ball || parseInt(Seed.units[cityID]['unt11']) < CrestData[CrestDataNum].R2Ram || parseInt(Seed.units[cityID]['unt12']) < CrestData[CrestDataNum].R2Cat) {
			if (CrestData.length == 1) {
				...
				setTimeout(function(){ t.Rounds(1,retry,parseInt(CrestDataNum)+1);},8000);
			return;
		}
			...
			default:
				if (t.rallypointlevel <= slots) {
					if (CrestData.length == 1) {
						...
						setTimeout(function(){ t.Rounds(1,retry,parseInt(CrestDataNum)+1);},12000);
					}
					return;
					break;
				}
		...
		switch(r) {
			case 1:
				if ((t.rallypointlevel-slots) < 2) {
					setTimeout(function(){ t.Rounds(1,retry,CrestDataNum+1);},15000);
					return;
				}
				var params 		= 	unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
	...
	toggleCrestState: function(obj) {
		var t = Tabs.Attack;
			if (Options.crestRunning == true) {
				...
				saveOptions();
				setTimeout(function(){ t.Rounds(1,0,0);}, 12*1000);
			}
	},
  ...
  init : function (div)
  	{
		...
		var m = '<DIV class=pbStat>Mass Queue Stack</div>';
