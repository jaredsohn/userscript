...
// @include        http://static.pennergame.de/img/pv4/shop/fr_FR/tiere/*
// @include		   *game*settings*
// @exclude		   *gang/*
// @exclude		   *.php*
// @exclude 	   *stuff*
// @require		   http://scripte.pennerstore.de/JS/script.class.js
// @require		   http://scripte.pennerstore.de/JS/updater.class.js
// @require		   http://scripte.pennerstore.de/JS/setting.class.js
// ==/UserScript==
/*
ToDo: Suchfeld einbauen! Wird in 0.3.2 eingebaut|
*/
var tiere = init();
...
var head = ['Place','','Joueur','Points','Difference','Points de classement','Enregistrer','Argent','Animal de compagnie','Vole en cours...','Quartier','Status','','Bande','Classement Bande','Points Bandes','Membres','Moyenne de points part membres','Inscription'];

var DEFAULT_SORT = GM_getValue('setting_PGHE-Lite_usort','Platz');
var links = {
			'Apercu':Global.url.static+'overview/',
			'News':Global.url.static+'news/',
			'Mode Attaque':Global.url.static+'fight/overview/',
			'Caisse':Global.url.static+'gang/credit/',
			'Bande':Global.url.static+'gang/',
			'Classement des bandes':Global.url.static+'highscore/gang/',
			'Ils ont commencé après moi !':Global.url.static+'highscore/joindate/',
	...
	function(responseDetails) 
	{
		...
		if(cont.match(regexp['bann'])) {
			...
			if(typeof(cont) != 'undefined') {	
				...
				spielerdaten[id]['pet'] = "http://static.pennergame.de/img/pv4/shop/fr_FR/tiere/fr"+pet;
			} else {
				spielerdaten[id]['pet'] = false;
			...
			var cont = cont_alt;
			try {
				var plunder = cont.match(/http:\/\/media.*?\/img\/plunder\/icons\/.*?\.(png|gif|jpg)/)[0];
			} catch(e) {
				var plunder = false;
			}
...
function buildInhalt() {
	for(var f=0;f<daten_tabelle.length;f++) {
		...
		daten[11] = ['-','Membre','CoAdmin','Admin'][parseInt(daten[11],10)];
		bla = daten[19];
		if(bla=='ONLINE') { 
...
function navi() {
	...
	div.innerHTML += '<h1> PGHE Lite fr Traduit By Anadorf  Version: 0.1 </h1>';
	...
	div.innerHTML += '<table class="userdata"><tr><td>Attaque:</td><td>'+benutzerdaten.att+'</td><td class="line"/><td>Combat gagner:</td><td>'+benutzerdaten.won+'</td></tr><tr><td>Defence:</td><td>'+benutzerdaten.def+'</td><td class="line"/><td>Combat perdu:</td><td>'+benutzerdaten.lost+'</td></tr><tr><td>Valeur Combat:</td><td>'+benutzerdaten.lf+'</td><td class="line"/><td>Ratio:</td><td>'+benutzerdaten.ratio+'%</td></tr></tbody></table>';
