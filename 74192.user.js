...
(function() {
	...
	var tree = true; //Древовидные комментарии
		var dot = true; //Индикатор вложенности
		var level = 15; //Уровень вложенности
						...
						run : function() {
							...
							if (cmt) {
								...
								for (var i = 0; i < cmts.length; i++) {
									var oo = this.get_lastchild(this.get_lastchild(this.get_lastchild(this
											.get_lastchild(cmts[i]))));
									if (oo.tagName == 'A') {
										var oldLi = cmts[i];
										var sId = String(oo.innerHTML)
												.substring(1);
										var parentLi = document
												.getElementById(sId);
										if (parentLi) {
											var otstup = parseInt(parentLi
													.getElementsByTagName("span")[0].style.marginLeft);
											(isNaN(otstup))
													? otstup = this.IndentStep
													: otstup += this.IndentStep;
											if (otstup > this.IndentStep
													* (this.MaxLevel - 1))
												otstup = this.IndentStep
														* (this.MaxLevel - 1);
											var prevLi = this
													.get_nextsibling(parentLi);
											var newLi = document
													.createElement('li');
											var newDiv = document
													.createElement('span');
											newDiv.innerHTML = oldLi.innerHTML;
											newDiv
													.setAttribute("class",
															"liav");
											newDiv.style.backgroundImage = oldLi.style.backgroundImage;
											newDiv.style.marginLeft = otstup
													+ '%';
											...
											newLi
													.setAttribute(
															...
															oldLi
																	.getAttribute("id"));
											if(dot){
											newLi.setAttribute("onmouseover",
