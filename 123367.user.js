...
// @namespace 
// @version		2.0.2
// @description	Polski konwerter raportów wojennych KingsAge by ReYc, base on Ikariam script by Morpheush and stnkvcmls (bugfix Levitah)
...
//do \zmianyponixrj
//new IkariamUserScriptUpdater( 74691, "Konwerter RW" );

const lang = getLang();
const version = $("li.version span").text().substr(0, 7);

const crConvVersion = "2.0.2";
...
var unitsCost = {
		...
		//edit for kingsage
		farmersMilitia: {stone: 3, wood: 10, ore: 7},
		templar: {stone: 30, wood: 10, ore: 80}, 
		squire: {stone: 10, wood: 20, ore: 30},
		berserker: {stone: 40, wood: 50, ore: 50},
		longBow: {stone: 80, wood: 160, ore: 80},
		spy: {stone: 40, wood: 60, ore: 60},
		crusader: {stone: 100, wood: 100, ore: 300},
		blackKnight: {stone: 100, wood: 300, ore: 500},
		batteringRam: {stone: 100, wood: 500, ore: 200},
		trebuchet: {stone: 200, wood: 600, ore: 200},
		count: {stone: 100000, wood: 120000, ore: 80000},
		townWall: {stone: 0, wood: 0, ore: 0}
}
...
function convertReport(){
	...
	else textBattleFor += "[b][size=12][color=" + colors.title + "]" + reportElements.type + " " + reportElements.for_near + " " + reportElements.place + "[/color][/size][/b]";
	textBattleFor += "\n";
...
/* calculate damage */
	...
	for(i = 0; i < 22; i++){
		if(reportElements.units.codes[i] != null){
			...
			//edit for KA
			tmpInt2 = (unitsCost[tmpInt].stone + unitsCost[tmpInt].wood + unitsCost[tmpInt].ore)*reportElements.units.lostAttacker[i];
			damageAttacker += tmpInt2;
		
			tmpInt2 = (unitsCost[tmpInt].stone + unitsCost[tmpInt].wood + unitsCost[tmpInt].ore)*reportElements.units.lostDefender[i];
			damageDefender += tmpInt2;
		}
	}
...
if(aIndex > 1) tmp = language[lang].attackers;
   ...
   if( reportElements.winners[0] == null && reportElements.losers[0] == null )
	...
	bbCodeText = "[align=" + options[lang].align + "]" + font +
		...
		textDamage +
      finishedBattle +
		"[b][i][color=#999999]" + language[lang].created + " [url='myrealpoland.prv.pl/kakonwerer.js']KA Konwerter RW\n" + 
...
function codeToName(code){
	switch(code){
		...
		//ka mod
		case 401: return "farmersMilitia";
		case 402: return "templar";
		case 403: return "squire";
		case 404: return "berserer";
		case 405: return "longBow";
		case 406: return "spy";
		case 407: return "crusader";
		case 408: return "blackKnight";
		case 409: return "batteringRam";
		case 410: return "trebuchet";
		case 411: return "count";
		case 412: return "townWall";
