// ==UserScript==
// @name           Pobieracz filmów z youtube
...
var pageFunction = function(checkUpdates, updateNeeded) {
	...
	window.ytd = {formats: {
			0:  { fmt: 0 , quality: { level: 0, recdl: false, description: "Niska Jakość (rename to video.flv)" }, format: "FLV" , mres: { width: 320, height: 240 }, wmres: { width: 400, height: 226 }, aformat: "22kHz, mono"  , acodec: "MP3"   , vcodec: "SVQ"    },
			5:  { fmt: 5 , quality: { level: 1, recdl: false, description: "Niska Jakość"                       }, format: "FLV" , mres: { width: 320, height: 240 }, wmres: { width: 400, height: 226 }, aformat: "22kHz, mono"  , acodec: "MP3"   , vcodec: "SVQ"    },
			6:  { fmt: 6 , quality: { level: 2, recdl: false, description: "Wysoka Jakość"                      }, format: "FLV" , mres: { width: 480, height: 360 }, wmres: { width: 480, height: 270 }                          , acodec: "MP3"   , vcodec: "SVQ"    },
			13: { fmt: 13, quality: { level: 1, recdl: false, description: "Niska Jakość H.263"   }, mobile: true, format: "3GPP", mres: { width: 176, height: 144 }, wmres: { width: 176, height: 144 }, aformat: "8kHz, mono"   , acodec: "SAMR"  , vcodec: "H.263"  },
			15: { fmt: 15, quality: { level: -1, recdl: true, description: "Original Upload Format"            }, format: "(Unknown)"                                                                                                                                 },
			17: { fmt: 17, quality: { level: 2, recdl: false, description: "Niska Jakość MPEG-4"  }, mobile: true, format: "3GPP", mres: { width: 176, height: 144 }, wmres: { width: 176, height: 144 }, aformat: "44kHz, mono"  , acodec: "AAC"   , vcodec: "MPEG-4" },
			18: { fmt: 18, quality: { level: 5, recdl: true , description: "Kompatybilny z ipod, Wysoka Jakość"     }, format: "MP4" , mres: { width: 480, height: 360 }, wmres: { width: 480, height: 270 }, aformat: "44kHz, stereo", acodec: "AAC"   , vcodec: "H.264"  },
			...
			36: { fmt: 36, quality: { level: 3, recdl: true , description: "Wysoka Jakość MPEG-4" }, mobile: true, format: "3GPP", mres: { width: 320, height: 240 }, wmres: { width: 320, height: 240 }, aformat: "44kHz, mono"  , acodec: "AAC"   , vcodec: "MPEG-4" },
	...
	dlButton.setAttribute("title", "Zapisz plik na dysk");
	...
	if (document.documentElement.getAttribute("lang") == "fr" || document.documentElement.getAttribute("lang") == "de" || document.documentElement.getAttribute("lang") == "it")
	{
	...
	if (videoId && token)
	{
		ytd.formats[5].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=5&asv=2";
		ytd.formats[5].guessed = true;
		
		ytd.formats[18].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=18&asv=2";
		ytd.formats[18].guessed = true;
		
		ytd.formats[13].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=13&asv=2";
		ytd.formats[13].guessed = true;
		
		ytd.formats[17].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=17&asv=2";
		ytd.formats[17].guessed = true;
		
		ytd.formats[36].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=36&asv=2";
		ytd.formats[36].guessed = true;
		
		if (yt.getConfig('IS_HD_AVAILABLE'))
		{
			ytd.formats[22].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&fmt=22&asv=2";
	...
	if (fmtStreamMap)
		if (fmtStreamMap[0] == "")
		{
			if (!videoId || !token)
				return compatError();
			ytd.formats[0].url = "http://" + document.location.host + "/get_video?video_id=" + videoId + "&t=" + token + "&asv=2";
