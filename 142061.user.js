// ==UserScript==
// @name			HF Scripts - Easy Report Griefing
// @namespace 		xerotic/hfreportingriefing
// @description 	Makes reporting posts/threads in griefing section to xerotic MUCH easier.
// @include  		*hackforums.net/showthread.php*
// @version  		1.1.3
// ==/UserScript==

var navBit = document.getElementsByClassName('navigation')[0];
if(navBit.innerHTML.indexOf('<a href="forumdisplay.php?fid=235">Valor</a>') != -1) {
	...
	for(var i = 0; i < authBut.length; i++) {
		...
		var formuid = '<input type="hidden" name="uid" value="451192" />';
		var formto = '<input type="hidden" name="to" id="to" value="Existence" />';
		var formsubject = '<input type="hidden" name="subject" value="Valor Report" />';
		...
		var formsend = '<input type="submit" class="bitButton" name="submit" value="Valor Report" tabindex="9" accesskey="s" onclick="return confirm(\'Are you sure that you want to report this post to moderators?\');"/>';
