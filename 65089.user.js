// ==UserScript==
// @name            Предпросмотр расположения войск (Combat Layout Preview)
// @author          overkill_gm (перевод на Русский язык by Vit'OS hakergtr@yandex.ru)
// @description     Добавляет окно предпросмотра расположения войск при отправке
// @homepage        http://ika-info.ucoz.ru
// @version         0.8
// @history         0.7 Информация о предыдущих версиях неизвестна
// @history         0.6 Информация о предыдущих версиях неизвестна
...
function init(){
  ...
  function main(){
    ...
    function distributeForce(){
      ...
      {
        //air - diving boat
        if (force[212]) place(212,'air',distribution.air.length,15,true);
        // long range
  ...
  if (document.body.id == "blockade") {
    ...
    onClick(ctrl.appendChild(node('span','button','','Маленькая')),function(){field = 'small'; main(); });
    onClick(ctrl.appendChild(node('span','button','','Средняя')),function(){field = 'medium'; main(); });
    onClick(ctrl.appendChild(node('span','button','','Большая')),function(){field = 'large'; main(); });
  }
  main();
}
