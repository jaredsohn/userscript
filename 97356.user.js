// ==UserScript==
// @name           KOC Power Bot fr traduit par mimi28700
// @namespace      mat
...
//	logit(div);
        ...
        var m = '<DIV id=pbTowrtDivF class=ptstat>FONCTION DE LA TOUR</div><TABLE id=pbtowerfunctions width=100% height=0% class=pbTab><TR>';
        if (t.alertState.running == false) {
            m += '<TD><INPUT id=pbAlertState type=submit value="Alerte Audio = OFF"></td>';
       }
        else {
            m += '<TD><INPUT id=pbAlertState type=submit value="Alerte Audio = ON"></td>';
        }
        m += '</tr></table></div>';
        m += '<DIV id=pbAlertDivD class=ptstat>SANCTUAIRE</div><TABLE id=pbalertdetails width=100% height=0% class=ptentry><TR>';
...
// George will fix this when tower tab is refactored for Power Tools update      
    ...
    toggleAlertState: function(obj){
        ...
        if (t.alertState.running == true) {
            ...
            t.saveAlertState();
            obj.value = "Alerte Audio = OFF";
        }
        else {
            t.alertState.running = true;
            t.saveAlertState();
            obj.value = "Alerte Audio = ON";
        }
    },
	show: function(){
		var t = Tabs.tower;
...
Tabs.build = {
    ...
    init: function(div){
        ...
        if (t.buildStates.running == false) {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Construction Auto = OFF"></td>';
        }
        else {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Construction Auto = ON"></td>';
        }
		m += '<TD><INPUT id=pbBuildMode type=submit value="Mode Construction = OFF"></td>';
		m += '<TD>Mode de Construction: <SELECT id="pbBuildType">\
				<OPTION value=build>niveau superieur</option>\
				<OPTION value=max>niveau max</option>\
				<OPTION value=destruct>destruction</option>\
				</select></td>';
		m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>Demander de l aide?</td>';
		m += '</tr></table></div>';
		...
		m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><INPUT id=pbbuild_' + Cities.cities[i].id + ' type=submit value="Voir"></center></td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD>Nbre:</td><TD id=pbbuildcount_' + Cities.cities[i].id + '>' + t["bQ_" + Cities.cities[i].id].length + '</td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
    ...
    doOne : function (bQi){ 
		...
		if (typeof Seed.buildings['city' + currentcityid]["pos" + citpos][0] != 'undefined') {	
			...
			if (isNaN(curlvl)) {
				actionLog("aucune valeur correcte trouvé pour la construction en cours!!!");
				return;
			}
			if (l_bdgid != bdgid) {
				actionLog("aucune valeur correcte pour la construction en cours!!");
				return;
			}
			if (l_bid != bid) {
				actionLog("Bâtiment ID ne correspond pas!!!");
				return;
			}
			if (l_curlvl < curlvl) {
					t.cancelQueueElement(0, currentcityid, time, false);
					actionLog("Suppression de l'item: niveau de construction est égale ou supérieure!!!");
					return;
			}
			if (l_curlvl > curlvl && mode == 'construire') {
			...
			if (mode == 'build') {
				...
				if (invalid == false) {							
					...
					new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/construct.php" + unsafeWindow.g_ajaxsuffix, {
						...
						onSuccess: function(rslt){
							if (rslt.ok) {
								...
								if (rslt.error_code == 103) { // building has already the target level => just  delete
									t.cancelQueueElement(0, currentcityid, time, false);
									actionLog("élément supprimé: bâtiment à ce niveau existe déjà ou processus de construction a déjà commencé!");
								} else {
									t.requeueQueueElement(bQi);
						...
						onFailure: function(){
							document.getElementById('pbbuildError').innerHTML = "Erreur de connexion! S'il vous plaît essayer à nouveau plus tard";
						}
					});
				} else {
			...
			t.cancelQueueElement(0, currentcityid, time, false);
			actionLog("élément supprimé: construction n'existe pas!!!");
		}
	},
    ...
    bot_buildslot: function(c, a){
		...
		if (loaded_bQ.length == 0 && current_construction_pos != "" ) { //check anyway if there is currently build in progess for this specific building
			...
			for (var i = 0; i < loaded_bQ.length; i++) { // check if there are already queue items for this building or the building is currently building
				...
				if (loaded_bQ[i].buildingMode == 'destruct' && loadedSlot == buildingPos && loadedCity == cityId) { // check if destrcution is already in queue
					t.modalmessage('Destruction déjà dans la file d attente!');
					return;
				}
			}
		}
        if (t.currentBuildMode == "construire") {
		    if (buildingLevel >= 9) {
                t.modalmessage('En raison des exigences de construction (DI), les bâtiments-dessus du niveau 9 \ doit être construite manuellement.');
                return;
            }
            var buildingMode = "construction";
        ...
        if (t.currentBuildMode == "destruction") {
            var buildingMode = "destruction";
			var result = t.calculateQueueValues(cityId, buildingLevel, buildingType, buildingMode);
			var buildingMult = result[0];
			var buildingTime = result[1];
			var queueId = loaded_bQ.length;
	...
	calculateQueueValues: function (cityId, buildingLevel, buildingType, buildingMode) {
	    var t = Tabs.build;
		var now = unixTime();
		if (buildingMode == 'construction') {
			var buildingMult = Math.pow(2, buildingLevel);
        } 
		if (buildingMode == 'destruction') {
			var buildingMult = Math.pow(2, buildingLevel - 2);
		}
				
		var knights = Seed.knights["ville" + cityId];
		if (knights) {
			var polKniId = parseInt(Seed.leaders['ville' + cityId].politicsKnightId);
			if (polKniId) {
				var polValue = parseInt(Seed.knights['ville' + cityId]['knt' + polKniId].politics);
				var polBoost = parseInt(Seed.knights['ville' + cityId]['knt' + polKniId].politicsBoostExpireUnixtime);
				if ((polBoost - now) > 0) {
	...
	bot_gethelp: function (f, currentcityid) {
	  var a = qlist = Seed.queue_con["ville" + currentcityid];
	  var e = 0;
	  var d = 0;
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>liste de construction de ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="Optimizer par temps"></td>';
    ...
    toggleStateRunning: function(obj){
        ...
        if (t.buildStates.running == true) {
            ...
            t.saveBuildStates();
            obj.value = "Construction Auto = OFF";
        }
        else {
            t.buildStates.running = true;
            t.saveBuildStates();
            obj.value = "Construction Auto = ON";
        }
    },
...
Tabs.Recherche = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    div.innerHTML = '\
        <DIV class=ptentry><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>chercher pour: </td><TD>\
        <SELECT id="pasrcType">\
...
for (var i in Provinces) {
	m += '<option value="'+i+'">'+Provinces[i].name+'</option>';
      ...
      m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=pasrcStart type=submit value="Comencer recherche"/></td></tr>';
      m += '</table>';
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X peut etre entre 0 et 749<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y peut etre entre 0 et 749<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>75)
      errMsg += "la distance maximum peut etre entre 1 et 75<BR>";
    if (errMsg != ''){
    ...
    document.getElementById ('pasrcStart').value = 'Areter recherche';
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>niveau minimum a chercher:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>niveau maximum a chercher:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Type de TS:</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'Prairie/Lac', 3:'Bois', 4:'Coline', 5:'Montagne', 6:'Plaine', 0:'TOUT'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>libre seulement:</td><TD class=xtab><INPUT id=pafilUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>afficher par:</td><TD class=xtab><SELECT id=pafilSortBy>\
          <OPTION value="niveau" '+ (Options.srcSortBy=='niveau'?'SELECTIONER':'')  +'>Level</option>\
          <OPTION value="distance" '+ (Options.srcSortBy=='distance'?'SELECTIONER':'')  +'>Distance</option>\
			</select></td></tr>\
			<TR><TD class=xtab align=right>Afficher les uniquement les coordoné:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
			</table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
    } else {
		m+= '</select></td></tr><TR><TD class=xtab align=right>sous brume seulement:</td><TD class=xtab><INPUT id=pafilMisted type=CHECKBOX '+ (Options.mistedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Hostile seulement:</td><TD class=xtab><INPUT id=pafilHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Trier par:</td><TD class=xtab><SELECT id=pafilSortBy>\
          <OPTION value="puissance" '+ (Options.srcSortBy=='puissance'?'SELECTED':'')  +'>puissance</option>\
             <OPTION value="distance" '+ (Options.srcSortBy=='distance'?'SELECTED':'')  +'>Distance</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right>afficher uniquement les coordoné</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
        </table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    document.getElementById('pastatFound').innerHTML = 'Trouver: '+ dat.length;
    if (dat.length == 0){
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>coordoné</td></tr>';
      else {
      if (t.opt.searchType == 2) {
			...
			m = '<TABLE id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>coordoné</td><TD style="padding-left: 10px">Distance</td><TD style="padding-left: 10px;">Lvl</td><TD width=80%> &nbsp; Type</td><TD style=""></td></tr>';
		}
	}
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('pasrcStart').value = 'commencer recherche';
    document.getElementById ('pasrchSizeWarn').innerHTML = '';
    if (t.opt.searchType==0 && document.getElementById('KOCAttackToggle')!=null){    
      document.getElementById ('pbSrcExp').innerHTML = '<CENTER>'+ strButton20('envoyer ver koc attack', 'id=pbSrcDoExp') +'</center>'; 
...
//logit (" 3 mapCallback()");
    ...
    document.getElementById('pastatSearched').innerHTML = 'chercher: '+ t.tilesSearched;
    t.dispMapTable();
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>configuration de power bot:</b></td></tr>\
        <TR><TD><INPUT id=pballowWinMove type=checkbox /></td><TD>Déplacement de la fenêtre Activer (déplacer la fenetre en faisant glisser la barre supérieure avec la souris)</td></tr>\
        <TR><TD><INPUT id=pbTrackWinOpen type=checkbox /></td><TD>réouvrir apres l actualisation</td></tr>\
        <TR><TD><INPUT id=pbHideOnGoto type=checkbox /></td><TD>Cacher la fenêtre en cliquant sur les coordonnées de la carte</td></tr>\
        <TR><TD colspan=2><BR><B>Caractéristiques KOC:</b></td></tr>\
        <TR><TD><INPUT id=pbFairie type=checkbox /></td><TD>Désactiver toutes les fenêtres popup fete foraine</td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Écran large (tous les domaines, il faut actualiser)</td></tr>\
        <TR><TD><INPUT id=pbWatchEnable type=checkbox '+ (GlobalOptions.pbWatchdog?'CHECKED ':'') +'/></td><TD>Actualiser si KOC pas chargé au bout de 1 minute (tous domaines)</td></tr>\
        <TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>Actualiser KOC toute les <INPUT id=pbeverymins type=text size=2 maxlength=3 \> minutes</td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Placez le chat à droite (nécessite écran large)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>Utilisez la carte large (nécessite écran large)</td></tr>\
        <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>colecter automatiquement l or lorsque le bonheur ateint <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Publiez automatiquement un msg sur le chat aliance en cas d attaque.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Message a publier: &nbsp; </td><TD><INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD align=right>Alert en cas d eclairage: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Alert en cas d attaque d une TS: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Minimum de troupe pour declencer la publication: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
            </table></td></tr>\
        </table><BR><BR><HR>Note que si une case à cocher est grisée il ya probablement eu un changement a KOC, ce qui rend l option inutilisable.</div>';
      div.innerHTML = m;
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    msg += 'ma '+ target +' est '+ atkType  +' par '+ who +'. troupe arivan (arive dans '+
...
var RefreshEvery  = {
  ...
  doit : function (){
    actionLog ('rafraichir ('+ Options.pbEveryMins +' minutes expirer)');
    reloadKOC();
  }
}
...
Tabs.kdo = {
  ...
  init : function (div){
    var t = Tabs.kdo;
    t.myDiv = div;    
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD width=200></td><TD align=center><INPUT id="pasubGifts" type=submit value="Chercher les cadeau" \></td><TD width=200 align=right><INPUT id=paGiftHelp type=suumetre value=AIDE></td></tr></table><HR>\
  ...
  e_clickedGifts : function  (){     // (also cancel accepting)
    var t = Tabs.kdo;
    if (t.accepting){
      document.getElementById('pasubGifts').value = 'chercher des kdo';
      document.getElementById('giftDiv').innerHTML+= '<BR><SPAN class=boldRed>Cancelled.</span>';
      t.accepting = false;
      return; 
    }
    document.getElementById('giftDiv').innerHTML = 'recherche de kdo sur la page facebook ...';
...
//logit (inspect (t.gifts, 8, 1));     
      ...
      m += '<TABLE class=pbTab align=center><TR><TD align=right>accepter les kdo sue quel serveur: </td><TD>'
        + htmlSelector (Options.giftDomains.list, getServerId(), 'id=pbGiftServers') +'</td></tr>\
          <TR><TD align=right>suprimer les cadeau apres les avoir accepter</td><TD>'
        + htmlSelector ({y:'tout le teps', e:'seulement en cas d ereur', n:'Jamais'}, Options.giftDelete, 'id=pbGiftDel')
        + '</td></tr><TR><TD>selectioner les kdo que vous voulez accepter: </td><TD width=250><INPUT type=submit id=pbGiftDo value="Accept Gifts">\
        &nbsp; <SPAN id=pbGiftNone class=boldRed></span></td></tr></table><HR><TABLE class=pbTab><TR valign=top><TD>\
        <INPUT id=pbGiftButAll type=submit value="TOUT" style="width:100%; margin-bottom:5px"><BR><INPUT id=pbGiftButNone type=submit value="Aucun"></td>\
  ...
  getErDone : function (){ 
    ...
    if (t.doList.length==0){
      document.getElementById('pbGiftNone').innerHTML = 'Aucune sélection!';
      return;
    }
    t.doServer = document.getElementById('pbGiftServers').value;
    t.accepting = true;
    document.getElementById('pasubGifts').value = 'areter d accepter'; 
  ...
  acceptNext : function (){
    ...
    statSpan.innerHTML = 'Obtenir des données';
