...
// remove any media restrictions on the CSS
// first build a list the new stylesheets, then insert them.
// needs to be split out or we get into an endless loop
...
// add our new style sheets, inserted after the originals
for (var i=0; i < links.length; i++) {
  links[i].parentNode.insertBefore(linkList[i], links[i].nextSibling);
}
