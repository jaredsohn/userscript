// ==UserScript==
// @name            UES stranacka obavestenja
// @namespace       www.erepublik.com
// @author          Cira, Updated and Changed by Robinhood
...
{
    var VERSION = '0.08';
    var url = 'https://docs.google.com/document/d/1xuc8pTw3SzWIyasWmkWFx2mlg5VAajDoRr1JaSwUvB4/edit?hl=en_US';
    var updateUrl = 'http://userscripts.org/scripts/show/124234';
    ...
    function parseData(s)
    {
        try
        {
            ...
            var orders = doc.match(/Glasnik UeS:\s*(.+?)##/)[1];
            var link  = doc.match(/Link:\s*(.+?)##/)[1];
            var date   = doc.match(/Date:\s*(.+?)##/)[1];
            var mon    = doc.match(/Novine UES-a:\s*(.+?)##/)[1];
            
            displayFrame(images[slika-1],orders,link,mon,date);
        }
        catch(e)
        {
            displayErrFrame("Greska, nema obavestenja. Pridruzite nam se na Rizon IRC #ues.","http://rizon.net/index.php?do=chat");
            throw e;
        }
    }
    ...
    function displayFrame(img,orders,link,mon,date)
    {
        ...
        if (main) {
            ...
            logo.className = 'box';
            logo.innerHTML ='<div class="title"><h1>Obavestenje</h1></div>'+
                '<div style="float: left;" class="core">'+
                    ...
                    '</div>'+
                '</div>'+
                '<div class="item more" id="top_rated_more_link"><a class="dotted" href="'+mon+'">UES Savet</a></div>'
