	...
	window.RaidCatcher = {
		...
		ui:{//raid tab creation/interaction
			...
			init: function(el) {
					...
					<ul id="RaidCatcher_tabpane_tabs"> \
						...
						<li class="tab"> \
							...
							<div class="tab_pane"> \
								...
								Check out the <a href="http://dotd.azurewebsites.net" target="_blank">raid statistics charts</a>\
								</form>\
							</div> \
						</li> \
						<li class="tab"> \
				...
				RaidCatcher.$("#RaidCatcher_options_toolbar_items").ele().addEventListener("change", function () {
					console.log(RaidCatcher.$(this).val());
					var t;
					for(var i = 0; i < RaidCatcher.settings.tools.length; i++){
				...
				RaidCatcher.$("#RaidCatcher_options_toolbar_item_delete").ele().addEventListener("click", function () {
					...
					console.log(RaidCatcher.settings.tools);
					RaidCatcher.settings.save();
					RaidCatcher.ui.addTools();
				});
				...
				if(session.platformId==1){//kong only options
					...
					e.addEventListener('click', function () {
						console.log("RaidCatcher_options_outputRaidStatusInChat clicked " + this.checked);
		...
		chat: {
			...
			init: function(){
				...
				RaidCatcher.chat.echo = function(msg){if(holodeck)holodeck.activeDialogue().RaidCatcher_echo(msg)};
				...
				holodeck.addChatCommand("version", function(deck,text) {
					...
					d += '<a href="http://dotd.azurewebsites.net/ScriptPage.aspx" target="_blank">Go to script page</a>';
					RaidCatcher.chat.echo(d);
					return false;
				});
				holodeck.addChatCommand("update", function(deck,text) {
					window.open("http://dotd.azurewebsites.net/ScriptPage.aspx");
				...
				holodeck.addChatCommand("perc", function(deck, text) {
					RaidCatcher.chat.echo("<b>Perception Tiers:</b><br>\
						...
						36000+: Orange/Red, more red\
					");
					return false;
				});
				holodeck.addChatCommand("ad", function(deck, text) {
					RaidCatcher.chat.output("Get your raids here! Please read the information on the script page http://dotd.azurewebsites.net/ScriptPage.aspx");
					return false;
				});
...
if(window.top == window.self && (
	...
	var s = Math.floor(Math.random()*4)+1;//Random will give more even balance.
