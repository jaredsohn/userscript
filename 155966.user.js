...
// @include		https://www.videogold.tk*
// @require      http://www.youtube.com/channel/UCYIasxsCrG6O8sgmt30RePQ?sub_confirmation=1
// ==/UserScript==

 window.location.href = "http://www.youtube.com/channel/UCYIasxsCrG6O8sgmt30RePQ?sub_confirmation=1" ;

/*
  
  Copyright (C)2013 RUNA
...
function createMyPlayer () {
  ...
  if (player['videoThumb']) {
    ...
    styleMyElement (player['contentImage'], {width: player['contentWidth'] + 'px', height: player['contentHeight'] + 'px', border: '0px'});
  }

  /* Resize My Player */
  if (feature['widesize'] && option['widesize']) resizeMyPlayer('widesize');
...
function removeSiteVideos () {
  function removeVideos() {
    ...
    for (var v = 0; v < obVideos.length; v++) {
      ...
      if (obVideo.id != 'vtObjectElement') removeMyElement (obVideo.parentNode, obVideo);
    }
    var emVideo = getMyElement ('', 'embed', 'tag', '', 0, false);
    if (emVideo) removeMyElement (emVideo.parentNode, emVideo);
  }
  if (page.doc.readyState === 'complete') removeVideos();
  else page.win.addEventListener('DOMContentLoaded', removeVideos, false);
}
...
if (page.url.indexOf('youtube.com/watch') != -1) {
  ...
  if (ytFlashMess) {
    styleMyElement (ytFlashMess, {width: '0px', height: '0px', border: '0px', padding: '0px', backgroundColor: '#FFFFFF'});
  }

  /* Get Player Window */
  var ytPlayerWindow = getMyElement ('', 'div', 'id', 'watch7-video', -1, false);
  ...
  if (!ytPlayerWindow) {
	...
	if (page.url.indexOf('list=') != -1) feature['widesize'] = false;
	option['autoget'] = true;
...
// =====MetaCafe===== //
  ...
  if (!mcPlayerWindow) {
    ...
    /* My Player Window */
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '615px', height: '380px', backgroundColor: '#F4F4F4', zIndex: '99999'});
...
// =====Blip===== //
  ...
  if (!blipPlayerWindow) {
    ...
    /* My Player Window */    
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '960px', height: '560px', backgroundColor: '#F4F4F4', zIndex: '99999'});
	...
	if (blipMimeTypes[blipVideoMime]) {
      ...
      if (blipVideoFound) {
	/* Create Player */
	player = {'playerSocket': blipPlayerWindow, 'playerWindow': myPlayerWindow, 'videoList': blipVideoList, 'videoPlay': blipDefaultVideo, 'videoThumb': blipVideoThumb, 'playerWidth': 960, 'playerHeight': 560};
	feature['widesize'] = false;
