// ==UserScript==
// @name           LeproTotalCommentsMod
// @namespace      yes_its_me
// @description    Доработка скрипта http://userscripts.org/scripts/show/36876 для работы с комментариями с YouTube роликами
...
function sortComments() {
    ...
    for (i=0; i<comments.length; i++) {
        ...
        if (isNiceAndYouTube(comment.dom)) {
            comment.niceyoutube = 1;
            counter.niceyoutube++;
        }
...
// Остроумный комментарий с множеством плюсов
function isNiceAndYouTube(comment) {
    var text = getCommentText(comment);
    return ((isNice(comment) && text.search(/youtube\.com\/watch/ig) != -1) ? 1 : 0);
}
