...
// ==UserScript==
// @name           Word Count and Goal Tracker 2012
// @description    Shows word count; also allows working toward a goal (eg, NaNoWriMo). The word count is calculated when you view the word count in Google Docs. This can be done with the shortcut CTRL + SHFT + C
// @namespace      http://www.userscripts.org/
//
// @contributor    http://userscripts.org/users/glennji
// @contributor    http://userscripts.org/users/4440
// @contributor    http://userscripts.org/users/492418
//
...
// Based on the original "Word Count and Goal Tracker" script written by Arthaey Angosii,
// available at http://userscripts.org/scripts/show/60914
//
...
(function (){
    ...
    function getWordCount() {
		...
		var metrics = window.parent.document.getElementsByClassName('kix-documentmetricsdialog-row');

		for (var i = 0; i< metrics.length - 1; i++) {
			if (metrics[i].innerHTML == "Words") {
				count = metrics[i+2].innerHTML;
				break;
			}
		}
    ...
    function updateWordCount(e) {
        ...
        var today = new Date();
		var hoursOffset = 0; //I had to factor in an hour to correct my time, but you may not need to
		today = new Date(today.getTime() - (hoursOffset * 3600000)); 
    ...
    */
	...
	runLoad = function() {
		if (window.parent.document.getElementsByClassName('kix-documentmetricsdialog-row').length > 0) {
			...
			if (!document.getElementById('nanocounty')) {
				...
				document.getElementById('docs-toolbar-wrapper').appendChild(div);
...
// CHANGELOG:
//
//  - version 3.0 11-8-2012:
//      - updated for latest version of google docs, and probably broke a lot of the functionality because I don't understand it yet
//  - version 2.0.1:
