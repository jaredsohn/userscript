// ==UserScript==
// @name          SG - AutoJoin ByPass Password
// @namespace     bf3autojoin
// @author        sharkiller
// @description   Alt+3 activar espera automatica
...
function bf3aj_acceptpass(){
    if($('#joinflow-passwordprompt-submit').is(':visible'))
        $('#joinflow-passwordprompt-submit').click();
}

function bf3aj_join() {
    $('#serverguide-show-joinserver-form').submit();
    setTimeout(bf3aj_acceptpass,100);
}
...
function bf3aj_checkState() {
    if(bf3aj_state==0) {
        ...
        if($('.gamemanager-launchstate').hasClass('gamemanager-launchstate-launch_error')) {
            ...
            bf3aj_counter++;
            if(bf3aj_counter==200) bf3aj_state = 2;
        }
    } else if(bf3aj_state==2) {
        //alert("Happy BF3");
        bf3aj_doNotify('Ya estás jugando<br/>Battlefield 3<br />',30000);
        bf3aj_dotimer = false;
    }
}
...
function bf3aj_timer() {
    ...
    bf3aj_checkState();
    setTimeout(bf3aj_timer,200);
}

function bf3aj_start() {
    bf3aj_doNotify('AutoJoin Activado',3000);
    bf3aj_dotimer = true;
    bf3aj_state = 0;
    bf3aj_timer();
    bf3aj_join();
...
function bf3aj_stop() {
    bf3aj_doNotify('AutoJoin Desactivado',3000);
    bf3aj_dotimer = false;
}
