...
// (c) 2010-2013, demin  ( http://www.heroeswm.ru/pl_info.php?id=15091 )
// (c) 2008-2009, xo4yxa
...
var google_del = document.querySelectorAll("ins[style*='width:728']");
...
// link to img    http://dcdn.heroeswm.ru/i/top/line/lpart.jpg    i/top_ny_rus/line/    i/top_ny_eng/line/
...
var hwm_trudogolik_show = GM_getValue( nick+"hwm_time_work_trudogolik_show" , '0' );
...
var dragonLeftImage = document.querySelector("img[src$='dragon__left.jpg']");
var dragonRightImage = document.querySelector("img[src$='dragon__right.jpg']");
if(!dragonLeftImage) {
	dragonLeftImage = document.querySelector("img[src$='dragon__left_.jpg']");
	dragonRightImage = document.querySelector("img[src$='dragon__right_.jpg']");
}
var x1 = dragonLeftImage.getBoundingClientRect().left;
var x2 = dragonRightImage.getBoundingClientRect().left;
...
'<table cellpadding=0 cellspacing=0 align="center" class="hwm_tb" width=' + ( x2 - x1 + 132 ) +'px'+'>' +
'<tr height=26>' +
'<td>' +
...
dragonLeftImage.parentNode.appendChild(d);

addEvent( $("pers_m"), "click", setting_tr );
...
// replace uze ustroen
		...
		if ( ( time_cur > Number( GM_getValue( nick+"hwm_time_work_end" ) ) ) && ( parent_trud.innerHTML.match(uze_ustroen) || ( uze_ustroen = parent_trud.innerHTML.match(uze_ustroen2) ) ) ) {
			parent_trud.innerHTML = parent_trud.innerHTML.replace(uze_ustroen, '<blink><font color=red><b>'+uze_ustroen+'</b></font></blink>');
		}
...
if ( document.querySelector("img[src*='map/nl']") && !document.querySelector("img[src*='css/loading.gif']") ) {
	GM_setValue( nick+"hwm_map_hunter" , 'false' );
	GM_setValue( nick+"hwm_time_go_end" , '1300000000000' );
...
for ( var i=go_link_action_skip.length; i--; ) {
	...
	{
	GM_setValue( nick+"hwm_time_go_end" , '' + ( time_cur_now + 10*60000 * GM_getValue( nick+"hwm_time_percent_faster" ) * GM_getValue( nick+"hwm_time_percent_prem" ) * GM_getValue( nick+"hwm_time_percent_lic_mo" ) + 3000 ) );
	} else {
	GM_setValue( nick+"hwm_time_go_end" , '' + ( time_cur_now + 20*60000 * GM_getValue( nick+"hwm_time_percent_faster" ) * GM_getValue( nick+"hwm_time_percent_prem" ) * GM_getValue( nick+"hwm_time_percent_lic_mo" ) + 3000 ) );
	}
...
if ( b.innerHTML.match( regexp_map_go ) && GM_getValue( nick+"hwm_time_go_end" ) == '1300000000000' ) {
	GM_setValue( nick+"hwm_time_go_end_yes" , 'no' );
	var time_cur_now = new Date().getTime();
...
function setting_tr()
{
	...
	addEvent(bg, "click", form_close_tr);
...
'<tr><td>'+st_gre_check+': <input type=checkbox '+(hwm_gre_check=="1"?"checked":"")+' id=hwm_gre_check_id title=""><br><br></td></tr>'+
...
'<tr><td style="text-align:right">'+st_author+': <a href="pl_info.php?id=15091">Demin</a> <a href="#" id="open_transfer_id" onclick="return false;">?</a></td></tr>'+
'</table></div>';

	addEvent($("bt_close_tr"), "click", form_close_tr);
