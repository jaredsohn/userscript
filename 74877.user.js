// ==UserScript==
// @name           Treasure Isle Wall Manager
// @description    Accepts bonuses, collects fruit, adopts animals, gathers gems and more.
// @include        http://*.facebook.com/*
// @include        https://*.facebook.com/*
// @include        http://facebook.com/*
// @include        https://facebook.com/*
// @version        1.4.1
// @require        http://sizzlemctwizzle.com/updater.php?id=74333
// ==/UserScript==
...
var wC;
var pack=false;
var version = "1.4.1"; // cant use cdata+regexp to grab version dynamically because of webkit
	...
	GM_config.frame.addEventListener('load', function(){
		...
		frameBody.appendChild(obj.create('div', {id:'buttons_holder', kids:new Array(
			...
			obj.create('div', {className:'reset_holder block', kids:new Array(
				obj.create('a',{id:'resetLink',textContent:'Restore Default Settings',href:'#',title:'Restore settings to default configuration',className:'reset',onclick:obj.reset})
		)}))}));
...
gameID : "234860566661", // game id
gameURLpart : "treasureisle", // game url folder for apps.facebook.com/HERE/ (only some games have this)
whitelistWhenPaused : "", // categories separated by commas to be accepted even if gift box is full
gameName : "Treasure Isle",
gameAcronym : "T.I.G.M.",
gameKeyUrlKeyword : "key", // used in the regex and xpath to look for "key=" or "sk=" or whatever it may be
xpQueryTextExcludes : ["Help friend out", "Share", "Play Treasure Isle now"],

// empty options object for later modification
opts : {},

// all regexps are stored here
whichRegex : /(gem|bonus|save|fruit|unknown|gift|treasure|pack)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
colorRegex : /(red|Blue|orange|green|purple|wish)/g,
creatureRegex : /(sloth|margay|toucan|dolphin|baby seal|panther)/,
...
accText : {
			...
			unknown : "Adopted this Animal!",
			fruit : "Found Fruit!",
			gift : "Gem Sent!",
			pack : "Energy Pack Sent!",
			treasure : "Got this Treasure!"
		},
...
switch(w) {
case "save": w="adopt"; break;
}
return w;
},

// get which color an creature is
whichColor : function(e) {
var w=e.match(main.colorRegex);
w = (w!=null) ? w : "";
switch(w) {
...
// get which creature it is
whichCreature : function(e) {
var w=e.textContent.toLowerCase().match(main.creatureRegex), color=main.whichColor(e.parentNode.parentNode.parentNode.textContent.toLowerCase()), text=e.parentNode.parentNode.parentNode.textContent;
w = (w!=null) ? w[1].replace(main.spaceRegex,"") : "unknown";
switch(w) {
}
//switch(w != "unknown") {case true: w += color; break;} // add color to creature if the creature is a known one
return w;
},
...
if(confirm("Sei sicuro di voler resettare gli elementi presi?")) window.setTimeout(function(){
...
var statusText = !main.boxFull ? (!main.paused?"["+main.gameAcronym+"] "+main.currReqs+" Current Requests":(!main.pauseClick?("["+main.pauseCount+"] "):"")+"[PAUSED] Click this box to unpause") : "[STOP] Giftbox FULL - Refresh page to continue";
...
if(main.paused  || (main.opts["maxrequests"]-main.currReqs) == 0) {
return; 
}
...
if($("GM_config") || main.currReqs >= main.opts["maxrequests"]) {return;}
		...
		var open=main.open, accText=main.accText, getKey=main.getKey,
			which=main.which, whichCreature=main.whichCreature, whichEgg=main.whichEgg, acc=main.getAccepted(), accTime=main.getAcceptedTime();
...
case false: if(!$(key)) {
	switch(w) {
		...
		case "gem": if(opts["gem"] && opts[wC]) open(item.href, key, w); break; // open request in iframe
		case "gift": if(opts["gift"] && wC==main.opts["mygem"]) open(item.href, key, w); break; // open request in iframe
...
// pre-load the config
GM_config.init("<center><table><tr><td><img src='"+imgs.logo+"' width=50% height=50%></td><td>Treasure Isle Wall Manager v. "+version+" by Sisko (translated by Phenom)</td></tr></table></center>", {
	bonus : {
		section : ["Options"],
		label : "Accept Bonus?",
		type: "checkbox",
	...
	pack : {
		label : "Send Energy Pack?",
		type : "checkbox",
		"default" : true,
	},*/
	gift : {
		label : "Send Gem? (Only the color of your tree)",
		type: "checkbox",
		"default" : true,
		kids : {
			mygem : {
				label : "Select the color of the gem produced by your tree",
				type : "select",
				options : {
				off : "Choose",
				red : "Red",
	...
	filteronly : {
		label : "Only run on filtered page?",
		type : "checkbox",
		"default" : false
	},
	/*autolike : {
	...
	inputtimeoutenable : {
		label : "Enable script pausing",
		type : "checkbox",
		"default" : false,
		kids : {
			inputtimeout : {
				label : "Timeout (pause duration)",
				type : "float",
				"default" : 30
			}
		}
	...
	minposts : {
		label : "Minimum number of posts to display",
		type : "select",
		options : {
		off : "Off",
		5 : "5",
	...
	maxrequests : {
		label : "Maximum simultaneous requests",
		type : "float",
		"default" : 1,
	},
	reset : {
		label : "Reset changed items",
		type : "button",
...
try {
GM_registerMenuCommand("Treasure Island Wall Manager Options v. "+version, main.config);
} catch(e) {}
...
main.create("a", {className:"item", href:"javascript:void(0);", onclick:main.config}, new Array(
	...
	main.create("span", {textContent:"Treasure Island Wall Manager Options v. "+version})
))
)));
}
