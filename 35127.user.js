...
if (url == 'http://goallineblitz.com/game/new_message.pl?to=172134'){
	if(GM_getValue("dantae_help") == 1){
		element = document.getElementById('subject')
		element.value = "League Ranking v2.0 - Question or Comment"
		GM_setValue("dantae_help", 0)
	}
}
...
function get_data(leagueId, teamId, stat){
	GM_xmlhttpRequest({
   method: 'GET',
   url: 'http://goallineblitz.com/game/team_stats.pl?league_id=' + leagueId + stat,
   headers: {
...
function update(teamId, cache){
	if(cache == 0){
		alert("This team has been blocked from using the League Ranking script")
	}
	else{
	...
	var team = record.nextSibling.innerHTML
...
//alert("<" + team1[0] + "-" + team1[1] + "-" + tie + ">")
	...
	if(total == 0){
		...
		update.innerHTML = '<b>No Records: [Preseason or no games played]</b>'
		}
...
function build_table(){
	//var test == GM_getValue("dantae_help")
	...
	ranking.setAttribute("id", "ranking_box")
		...
		medium_h.innerHTML = 'League Ranking <A href="http://goallineblitz.com/game/new_message.pl?to=172134"><img src="http://64bitstuff.com/question.gif"></a>'
		
		medium_h.childNodes[1].addEventListener("click", help_d, false)
		
		medium_h.childNodes[1].addEventListener("mouseover", function(event) {
			 unsafeWindow.set_tip(help_menu(), 0, 1, 1, 1);
		}, false)
		
		medium_h.childNodes[1].addEventListener("mouseout", function(event) { 
...
function help_d(){
	GM_setValue("dantae_help", 1)
	//alert("test help")
};

function help_menu(){
	return "<b>Offense</b><br>Overall: total points scored<br>Passing: total passing yards<br>Rushing: total rushing yards<br><br><b>Defense</b><br>Overall: total points allowed<br>Passing: passing yards allowed<br>Rushing: tushing yards allowed<br><br><b>Special Teams</b><br>Kick Ret: kick return average<br>Punt Ret: punt return average<br>Punting: average yard per punt<br>Field Goal: field goal percentage<br><br><b>Turnover Ratio:</b> +/- turnovers vs. takeaways<br><br>(click icon for support)"	
};
