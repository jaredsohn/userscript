...
// @version     2.8.4.
// @require  http://usocheckup.dune.net/153401.js
// ...
...
//find out whether 503-Error or Log-out...
if(document.getElementsByTagName("h1")[0].innerHTML == "503 Service Unavailable"){
...
if(document.getElementsByClassName("outerLoginBox")){//log-outerHeight
warten =  Math.random()*1200000+3000000; //1h +- 10min
...
/* phpmv2 stats are no longer used... too much trouble/traffic...
//stats via phpmv2... Anonymous stats about how you use my Script... so far just basics are transmitted i.e. Browser, Screen resolution, region, plugins...
function Includephpmv2Stats(pagename){
var a_vars = Array();
var phpmyvisitesSite = 1;
var phpmyvisitesURL = "http://84.23.67.117/phpmv2/phpmyvisites.php";
...
function pmv_log(pmv_urlPmv, pmv_site, pmv_pname, pmv_vars)
{
	...
	document.getElementById("footer").innerHTML+='<img src="'+pmv_src+'" alt="phpMyVisites" style="border:0" />'
}
pmv_log(phpmyvisitesURL, phpmyvisitesSite, pagename, a_vars);
}
Includephpmv2Stats('T4_Auto_Auction_Bid_Script');
*/
//Arry-Funktionen:
...
if(Math.floor(price*number)<=silv2){//price too expansive, except...
...
catch(err){//too less silver ==> No bid button ==> No aid
base.style.color='#E39210'; // some orange
}
}
}
}
}
GM_log("Finished analyzing. Now Auto-Action");
if(!isMarketPaused()){
switch(getPageLocation()){
case "acco": //Accounting
{/************************************************************** START Collecting Infos 
...
//GM_log("log log zu wenig Silber und aid:" + aid);
	...
	{
	{
GM_log("Bieten vorbereiten von aid"+aid);
