...
function incrementDay() {
    ...
    if (getAge() >= extended_plateau_age) {
	    if ((getLevel()+getBoosts()) < 72) {
	    	// plateau players can boost to 72
...
function singleTrain() {
	...
	if (tpCost > getTP()) {
        ...
        var trainingType = document.getElementById('trainingSelect').value;
        var btGain = 0;
        if (trainingType==TRAININGTYPE.LIGHT) btGain = 3;
        else if (trainingType==TRAININGTYPE.NORMAL) btGain = 2;
        else if (trainingType==TRAININGTYPE.INTENSE) btGain = 1;
