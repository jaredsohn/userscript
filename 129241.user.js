// ==UserScript==
// @name           Another scrobble songs from YouTube.
// @description    Scrobble music videos from YouTube to Last FM. Contains both automatic scrobbling and manual scrobbling.
// @author         Ritador
// ==/UserScript==
...
var altScrobbleSongUrl = "http://daan.hostei.com/lastfm/scrobbleSong/";
 ...
 */
function initialize() {
	//Since this whole GM_runScript function is executed on page load I reset it.
	//This to prevent doing things twice or perhaps even infinite. 
	//GM_runScript = new function() { console.log("GreaseMonkey script already ran.") };
...
function addMenuPop() {
	...
	"	<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-moreoptions-ok\" title=\"Save changes\"><span class=\"yt-uix-button-content\">Save</span></button>\n" +
	"	<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-moreoptions-cancel\" title=\"Discard changes\" style=\"margin-left: 14px;\"><span class=\"yt-uix-button-content\">Cancel</span></button>\n" +
	"</div>\n";	
...
function tryGetAuthToken() {
	...
	alert(authenticationSessionUrl + "?token=" + token);
...
function getSessionKey(response) {
	...
	//var text = response.responseText.split(/\r?\n/gim)[0];
		...
		alert("FF " + response.responseText);
	var jsonObject = eval("(" + response.responseText + ")");
	...
	if(jsonObject.error != null) {
		...
		alert("FF " + jsonObject.session.key);
		if(useHtml5LocalStorage) {
...
function addStyle() {
	...
	"ul#lastfm-options li { height: 94px; float: left; margin: 8px 0px; padding: 0px 24px; border-right: 1px solid #CCCCCC; }\n" + 
	"ul#lastfm-options p { padding-left: 1px; }\n" +
	"#lastfm-title { margin-top: 4px;}\n" +
	"#lastfm-moreoptions { margin-bottom: 6px; }\n" +
	"#lastfm-moreoptions-pop { height:296px; width:432px; position: absolute; top: 0px; right: 0px; background:#FFF; border:2px solid #CCC; z-index:4; padding:12px; font-size:13px; }\n" +
	...
	"#masthead-user-image img { height:24px; }\n";	
}
/**
...
function addLastFmButton() {
	...
	var lastFmImgCode = "data:image/png;base64,";
	...
	"				    <img alt=\"Last.FM\" src=\"" + lastFmImgCode + "\">" +
	...
	if(scrobbleAutoCollapse === "expand") {
		autoToggleLastFmForm();
	}
}
/**
...
function addLastFmOptions() {
	...
	"			<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-reset\" title=\"Log out from Last FM\"><span class=\"yt-uix-button-content\">Reset</span></button>\n" +
	"		</li>\n" +
	"		<li id=\"lastfm-options-login\">" +
	"			<h3>Authenticate</h3>\n" +
	"			<p>To log in this page <br/>will be closed.</p>" +
	"			<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-login\" title=\"Log in to Last FM\"><span class=\"yt-uix-button-content\">Authenticate</span></button>" +
	"		</li>\n" +
	"		<li>\n" +
	"			<h3>Miscellaneous</h3>\n" +
	"			<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-moreoptions\" title=\"More options menu\"><span class=\"yt-uix-button-content\">More options</span></button><br/>\n" +
	"			<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-man-scrobble\" title=\"Scrobble now\"><span class=\"yt-uix-button-content\">Scrobble manual</span></button>\n" +
	"		</li>\n" +
	"		<li>\n" +
	"			<h3>Scrobbling</h3>\n" +
	"			<table>\n" +
	...
	"				<button class=\"yt-uix-button yt-uix-button-default yt-uix-tooltip\" id=\"lastfm-noscrobble\" title=\"Stops the automatic scrobble timer\"><span class=\"yt-uix-button-content\">Don't scrobble</span></button>\n" +
	"			</div>\n" +
...
/**
 * Toggles the Last FM form added by this plug in and updates the arrow indication. 
 * This indication is normally triggered by clicking and done by YouTube. 
 */
function autoToggleLastFmForm()
{
	toggleLastFmForm();
		
	//Set the button arrow correct 
	var buttonCont = document.getElementById("lastfm-button").parentNode;
	var newClass = buttonCont.getAttribute("class");
	var regex = /\s?yt-uix-expander-collapsed\s?/gi;
...
function toggleLastFmForm() {
	...
	var buttonCont = document.getElementById("lastfm-button").parentNode;
	var buttonClass = buttonCont.getAttribute("class");
	if(countOccurencesInString(buttonClass, "yt-uix-expander-collapsed") >= 1) {
