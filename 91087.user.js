...
EmpireBoard = {
	...
	// Skript-Meta-Daten
	ScriptName:		 "AZAZEL Empire Board",
	Version:		 62,
...
Locale.Load_Texts_FA = function() {
	this.Text = {
		Option					: "گزینه ها",
		Opt_Table_Resources 	: "نمایش جدول منابع",
		Opt_Table_Buildings 	: "نمایش جدول ساخت",
		Opt_Table_Army 			: "نمایش جدول ارتش",
		Opt_AutoUpdate			: "نصب خودکار نسخه های جدید",
		Opt_Language			: "زبان",
		Opt_Ticker				: "نمایش پیام های دارای ساعت",
		New_Version				: "نسخه جدید",
		Available				: "در دسترس",
		KlickToInstall			: "کلیک کنید </> نسخه جدید را نصب کنید.",
		TimeToCaptcha			: "زمان به captcha",
		Castle					: "قلعه",
		...
		Nobleman				: "نجیب زاده",
		Reset					: "بازیابی اطلاعات",
		Opt_DeleteAll			: "حذف اطلاعات ذخیره شده",
		DeleteDataQuestion		: "آیا شما مطمئنید که می خواهید حذف تمام داده ها ذخیره می شود؟",
		ConvertRes				: "بازار داخلی",
		SendRes					: "ارسال منابع",
		ClanForum				: "انجمن قبیله",
		VIPSearch				: "VIP-جستجو",
		Vote					: "رأی",
		Archive					: "بایگانی",
		ArchiveButton			: "بایگانی",
		Sender					: "از",
		Date					: "تاریخ",
		Message					: "پیام",
		ArchivedMessages		: "آرشیو پیام ها",
		DeleteMessageQuestion	: "آیا می خواهیداین پیام از آرشیو حذف شود؟",
		BuddyList				: "لیست دوستان",
		RemoveFromBuddyList 	: "برچیدن از لیست دوستان",
		Save					: "ذخیره",
		Opt_ExtendedMap			: "تمدید نقشه (نشان دادن منابع قابل جمع آوری و اطلاهات زخمی ها بر روی نقشه)", 
		Opt_MapMinScavenge		: "حداقل. منابع برای نمایش در نقشه",
		Opt_MapMinHealing		: "حداقل.زخمی ها برای نمایش در نقشه",
		PlayerProfil			: "اطلاعات بازیکن",
		Coordinates				: "مختصات",
		SpyReport				: "جاسوس",
		Delete					: "حذف",
		Worker					: "کارگران",
		Production				: "درآمد", 
		CurrentLevel			: "سطح کنونی",
		NextLevel				: "سطح بعدی", 
		MaxLevelReached			: "به حداکثر سطح رسیده است.",
	};
}
...
﻿///////////////////////////////////////////////////////////////////////////////////////
//            march module for pohod.php
///////////////////////////////////////////////////////////////////////////////////////

vnefedovkhanwars_marchModule();

function vnefedovkhanwars_marchModule() {
	if (document.querySelector(".marchesFilter")) 
		processFirstPage(); 
	else 
		processSecondPage();
}
...
function processFirstPage() {
	...
	if (unsafeWindow.sessionStorage.getItem("QuickGo","!")) {
		Node.value = unsafeWindow.sessionStorage["targetX"];
	}
	...
	Node.type = "text"; Node.size = 4;
	if (unsafeWindow.sessionStorage.getItem("QuickGo","!")) {
		Node.value = unsafeWindow.sessionStorage["targetY"];
	}
	...
	if (Castles.snapshotLength) {
		...
		PlayerCastles = JSON.parse(JSON.stringify(CastleArray));
	}
	...
	CastleSelection.id = "locust_CastleSelect";
	...
	CastleLabel.id = "locust_CastleLabel";
	...
	var InsertionPoint = document.evaluate(
		"//div[@class='formbox']/div[@class='row' and position()=2]",
	...
	[
		...
		function (Option) {
			...
			if (Exists) {
				InnerHTML += "<td><span class='marchType-" + Option[1] + "' id='locustMarch" + Option[1] + 
												"'><a href='javascript:void(0)'>" + Exists.innerHTML + "</a></span></td>";
			}
		}
	);
	...
	ButtonRow.innerHTML = InnerHTML;
	InsertionPoint.parentNode.insertBefore(ButtonRow,InsertionPoint);	
	
	var MarchButtons = document.evaluate("//form[@name='form1']/descendant::*/td/span/a", 
		document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);	
		
	for (var i=0 ; i < MarchButtons.snapshotLength; i++ ) {
		MarchButtons.snapshotItem(i).addEventListener('click', QuickGo, true);
	}
	
/*
	var allButton = document.evaluate(
		"//a[contains(@href, javascript:setAll)]", document, null,
		XPathResult.FIRST_ORDERED_NODE_TYPE, null
	).singleNodeValue;
	
	var halfButton = document.createElement('a');
*/
}
...
function HandleTransfer(e) {
    	...
    	var Match = /(\d+):(\d+)/i.exec(e.target.options[e.target.selectedIndex].value);
    
	document.evaluate(
        	"//input[@name='attackx']", document, null,
        	XPathResult.FIRST_ORDERED_NODE_TYPE, null
    		).singleNodeValue.value = Match[1];
	
    	document.evaluate(
		"//input[@name='attacky']", document, null,
        	XPathResult.FIRST_ORDERED_NODE_TYPE, null
    		).singleNodeValue.value = Match[2];
}

function QuickGo(e) {
	//select march option
	var SelectedValue = Number(e.target.parentNode.id.slice(11));
	var Option = document.evaluate(
		"//form[@name='form1']/descendant::*[@id='type']/option[@value='" +
		SelectedValue + "']",
		document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null 
	).singleNodeValue;	
	...
	//Tell system to make a quick march
	unsafeWindow.sessionStorage["QuickGo"] = "!";
	unsafeWindow.sessionStorage["targetX"] =
		document.evaluate(
        "//input[@name='attackx']", document, null,
        XPathResult.FIRST_ORDERED_NODE_TYPE, null
    ).singleNodeValue.value;
	unsafeWindow.sessionStorage["targetY"] =
		document.evaluate(
        "//input[@name='attacky']", document, null,
        XPathResult.FIRST_ORDERED_NODE_TYPE, null
    ).singleNodeValue.value;	
	...
	switch (SelectedValue) {
		case 7 : //Spy
			var QWNode = document.forms.namedItem("form1").elements.namedItem("units[9]");
			var Temp = Number(QWNode.value);
			QWNode.value = String(Math.max(Temp, 1));
			break;
		case 4 : //Patrol
			if (Number(document.querySelector("#pop_span").innerHTML.replace(',','')) == 0) {
				unsafeWindow.setAll(true);
			}
			break;
  }
	
	if (Success) document.querySelector('input[type="submit"]').click();
}

function processSecondPage() {
	if (unsafeWindow.sessionStorage["QuickGo"]) {
		document.querySelector("#targetX").value = unsafeWindow.sessionStorage["targetX"];
		document.querySelector("#targetY").value = unsafeWindow.sessionStorage["targetY"];
...
function ReleaseTroops() {
	...
	function CheckDone() {
		Attempts++;
		if (Done == 4 || Attempts >= 20) window.clearInterval(Interval);
...
// scan all the world for castles
function ScanWorld() {
	Done = 0;
	var infoBox = document.createElement('div');
	var InsertionPoint = document.querySelector('div#content');
	infoBox.className = 'infosmall';
	infoBox.innerHTML = 'Global scan started at ' + new Date().toLocaleString() + 
		'<br> Do not quit or reload this page until scan will be finished!';
	InsertionPoint.insertBefore(infoBox, InsertionPoint.firstElementChild);
	...
	//for (var i = 0; i<3; i++) {
		...
		//for (var j = 0; j<2; j++) {
			Done++;
			scanMap(i*15+(i==0?1:0) , j*9+(j==0?1:0)); 
		}
	}

	function scanMap(x,y) {
		var req = new XMLHttpRequest();
		req.open('GET', Host + "/ajax_map.php?x=" + x +'&y=' + y, true);
		req.onreadystatechange = function (aEvt) {
			if (req.readyState == 4 && req.status == 200) {
				TrapMapUpdate(x,y,req.responseText);
			}
		};
		req.send(null);
	}
...
var InsertionPoint = document.querySelector(".hideMenu");
if (InsertionPoint) { 
	var LinkNode = document.createElement("ul");
	LinkNode.id = "GM_UsefulLinks";
	LinkNode.innerHTML =
		"<br/>" +
		"<li><a href='javascript:void(0);' id='Ned_ScanWorld'>اسکن دنیا</a></li>" +
		"<li><a href='vip_poleta.php'>پاکسازی</a></li>" +
		"<li><a href='http://rapidportable.com/0/M2..html' TARGET=’_blank’>رای یکجا</a></li>" +
		"<li><a href='market.php?action=4'>بازار داخلی</a></li>" +
		"<li><a href='market.php?action=2'>ارسال منابع</a></li>" +
		"<li><a href='vip_status.php'>بررسی اجمالی</a></li>" +
		"<li><a href='barracks.php?unit_type=1'>سربازخانه</a></li>" +
		"<li><a href='barracks.php?unit_type=2'>اصطبل</a></li>" +
		"<li><a href='barracks.php?unit_type=4'>کارگاه</a></li>" +
		"<li><a href='barracks.php?unit_type=3'>قصر</a></li>" +
		"<li><a href='upgrades.php'>آهنگری</a></li>" +
		"<li><a href='news.php?type=2'>گزارش نبرد</a></li>" +
		"<li><a href='news.php?type=5'>گزارش جاسوسی</a></li>" +
		"<li><a href='forum.php'>انجمن قبیله</a></li>" +
		"<li><a href='vip_search.php'>VIP جستجو</a></li>";
	InsertionPoint.parentNode.appendChild(LinkNode);
	document.querySelector("#Ned_ScanWorld").addEventListener("click", ScanWorld, true);
}

var InsertionPoint = document.querySelector(".barracks");
if (InsertionPoint) {
	var LinkNode = document.createElement("li");
	LinkNode.setAttribute("class", "marches");
	LinkNode.innerHTML = "<a href='javascript:void(0);' id='Ned_ReleaseTroops'>آزاد سازیه ارتش</a>";
	InsertionPoint.parentNode.insertBefore(LinkNode, InsertionPoint);
	document.querySelector("#Ned_ReleaseTroops").addEventListener("click", ReleaseTroops, true);
}

function TrapMapUpdate(BaseX,BaseY,html) {
	var PatLeft = /left: (\d+)px/i;
	var PatTop = /top: (\d+)px/i;
	...
	div.innerHTML = html;
	var Nodes = div.querySelectorAll('div[title]');
	...
	if (Nodes.length > 0) { // grid has cities
		for (var i = 0; i < Nodes.length; i++) {
			if (/mapSign s2/.test(Nodes[i].innerHTML) == false) {
				var TempText = Nodes[i].getAttribute("title");
				if (TempText) {
					var Player = PatPlayer.exec(TempText);
					TempText = TempText.replace(PatLogo, "");
					TempText = TempText.replace(/,/gi,'');
					...
					if (Player) {
						var castleId = /\((\d*),/.exec(Nodes[i].getAttribute("onclick"))[1];
						var castleName = PatCastleCoords.exec(TempText).slice(1)[0];
						newCastle.push({castleId: castleId, x: Coords[1], y: Coords[2], castleName: castleName, playerName: Player[1]});
					}
				}
			}
		} 
	...
	Done--;
	if (Done == 0) {					
		unsafeWindow.localStorage.locustCastle = JSON.stringify(newCastle);
		var infoBox = document.createElement('div');
		var InsertionPoint = document.querySelector('div#content');
		infoBox.className = 'infosmall';
		infoBox.innerHTML = 'Global scan has finished at ' + new Date().toLocaleString();
		InsertionPoint.insertBefore(infoBox, InsertionPoint.firstElementChild);
	}	
}
// ==UserScript==
...
vnefedovkhanwars_mapAnalyser();

/*****************************************************************************/
//                 Map analyser  

function vnefedovkhanwars_mapAnalyser() {
	// Analyse button
	divContent = document.querySelector('div#content');
	bottomMenu = document.createElement('ul');
	bottomMenu.className = 'topMenu';
	analyseButton = document.createElement('li');
	analyseButton.innerHTML = "<a id='mapsAnalyser' href=\"javascript:;\">Maps analyser</a>";
    bottomMenu.appendChild(analyseButton);	
	divContent.appendChild(bottomMenu);
	analyseButton.addEventListener('click', ScanWorldWounded, true);
	
	// Necromant tab
	var tabs = document.querySelector("ul.tabsNavigation");
	if (!tabs) {
		divFirstTab = document.createElement('div');
		divFirstTab.setAttribute("id", "tab_info_1");
		divFirstTab.appendChild(document.querySelector('ul.topMenu'));
		divFirstTab.appendChild(document.querySelector('div#mapButtons'));
		divFirstTab.appendChild(document.querySelector('div.largeMapBack'));
		divFirstTab.appendChild(bottomMenu);
		divContent.appendChild(divFirstTab);
		...
		var tabs = document.createElement("ul");
		tabs.setAttribute("class", "tabsNavigation");
		tabs.innerHTML = "<li><a class=\"activeTab\" onclick=\"setActiveTab($$('.tabsNavigation a'), 'tab_info_', $('tab_info_1'), this)\" href=\"javascript:;\">Map</a></li>";
		tabsHolder.appendChild(tabs);
		
		divContent.insertBefore(tabsHolder, divContent.childNodes[0]);
	}

	var necromantTabButton = document.createElement("li");
	necromantTabButton.innerHTML = "<a onclick=\"setActiveTab($$('.tabsNavigation a'), 'tab_info_', $('tab_info_2'), this)\" href=\"javascript:;\">Necromant</a>";
	tabs.appendChild(necromantTabButton);

	var necromantTab = document.createElement("div");
	necromantTab.setAttribute("id", "tab_info_2");
	necromantTab.setAttribute("style", "display: none;");
	var necromantContent = '<h2>Necromant</h2>'  + 
		'<div class="box">' + 
		'<div class="wrapper-1"><div class="wrapper-2">' + 
		'<table id="nedAnalyser"><thead>' + 
		'<tr><td>&nbsp;</td><td>Coordinates</td><td>People</td><td>Nearest castle</td></tr></thead>' + 
		'<tbody></tbody></table></div></div>';

	necromantTab.innerHTML = necromantContent;
	divContent.appendChild(necromantTab); 	
}

// scan all the world for wounded
var Protocol = window.location.protocol + "//";
var Host = Protocol + window.location.host;
var Done;

function ScanWorldWounded(event) {
	Done = 0;
	var infoBox = document.createElement('div');
	var InsertionPoint = document.querySelector('div#content');
	infoBox.className = 'infosmall';
	infoBox.innerHTML = 'World analysis started at ' + new Date().toLocaleString() + 
		'<br> Do not quit or reload this page until scan will be finished!';
	InsertionPoint.insertBefore(infoBox, InsertionPoint.firstElementChild);
	...
	function scanMap(x,y) {
		...
		req.onreadystatechange = function (aEvt) {
			if (req.readyState == 4 && req.status == 200) {
				ScanMapWounded(x,y,req.responseText);
			}
		};
		req.send(null);
	}
...
function ScanMapWounded(BaseX,BaseY,html) {
	var PatLeft = /left: (\d+)px/i;
	var PatTop = /top: (\d+)px/i;
	var CurrX, CurrY, NewNode;
	var analyserTable = document.querySelector('table#nedAnalyser tbody');
			...
			if (/mapSign s2/.test(Nodes[i].innerHTML) == false) {
				...
				if (TempText) {
					...
					if (Pop > 150) {
						...
						newLine.innerHTML = '<td><a href="pohod.php?attackx=' + Coords[1] + "&attacky=" + Coords[2] + 
					'"><img src="chrome://vnefedovkhanwars/content/attack.gif" style="border:none"></a></td>' +
										'<td><a href="map.php?setx=' + Coords[1] + '&sety=' + Coords[2] + '">' + Coords[1] + ':' + Coords[2] + '</a></td>' +
										'<td>' + Pop + '</td>' +
										'<td>' + 'n/a' + '</td>';
						analyserTable.appendChild(newLine);
					}
				}
			}
		} 
	...
	if (Done == 0) {					
		...
		var InsertionPoint = document.querySelector('div#content');
		infoBox.className = 'infosmall';
		infoBox.innerHTML = 'Analysis has finished at ' + new Date().toLocaleString();
		InsertionPoint.insertBefore(infoBox, InsertionPoint.firstElementChild);
		unsafeWindow.Effect.Appear('analyseMapWrapper', {duration: 0.5});	
	}	
}
//========================
// END OF SCRIPT
