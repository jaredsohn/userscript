// ==UserScript==
// @name           Yahoo Basketball Farm Players
// @namespace      
// @include        http://basketball.fantasysports.yahoo.com/nba/22991*
...
var ELIGIBILITY_FOOTNOTE = '*Exceeds rookie eligibility requirements (>82 GP or >40 GS)';
...
function RookieEligibility( gamesStarted, gamesPlayed )
{
	...
	this.isEligible = function()
	{
		if ( this.gamesStarted ) 
			return ( this.gamesStarted <= 350 );
		else
			return ( this.gamesPlayed <= 350 );
	}
}
...
function handleRosterData( responseDetails )
{
	...
	var ownerID = null;
	// http://basketball.fantasysports.yahoo.com/nba/22991/team?date=2010-12-11&week=7&mid=2
	...
	if ( ownerID && owners.hasOwnProperty( ownerID ) )
	{
		...
		if ( lastRoster && tableLegend )
		{
			...
			for ( var iPlayer = 0; iPlayer < owner.players.length; iPlayer++ )
			{
				...
				html.push( '<tr class="' + class + '"><td class="player""><div>' + playerHtml + '</div></td>' + 
		...
		if ( matches )
		{
			...
			for ( var iRow = 2; iRow < table.rows.length; iRow++ )
			{
				...
				else if ( /career/i.test( row.cells[ 0 ].innerHTML ) )
				{
					...
					var gamesStarted = getDataValue( row, 'gs' );
					if ( gamesStarted )
					{
						gamesStarted = parseInt( gamesStarted, 10 );
						player.dataCell.innerHTML = gamesStarted + ' games started';
					}
					else
					{
