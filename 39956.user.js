...
var keyPrefix = "AutoFarming."; // I also use a '.' for seperation
...
var delButtonImage = "data:image/gif;base64,";
...
function geefOverzicht() {
	...
	//alert(farms.length);
	if (farms.length > 0) {
		for (b=0;b<farms.length-1;b++) {
			//alert(farms[b]);
			var ntr = document.createElement("tr");
			ntr.className = "unit";			
			
			var delButton = document.createElement('div');
			delButton.className = "ddb";			
			var img = document.createElement('img');
			img.src = delButtonImage; //adding delete image
			img.setAttribute('height','12'); img.setAttribute('width','12');	img.style.height = '12px';img.style.width = '12px';
			delButton.appendChild(img);			
			var ntd = document.createElement("td");
			ntd.className = 'r7';
			//Vilage
			ntd.innerHTML += "<div class='ddb'>"+b+") </div>"+farms[b].split("|")[2]+' ('+(farms[b].split("|")[0]).replace(',','|')+')';
			delButton.addEventListener('click', createRemoveFarmCallback(b), 0); //adding clicking event
			ntd.appendChild(delButton);			
			ntr.appendChild(ntd);			
...
function removeFarm(itemToRemove) {
	...
	if (itemToRemove < farms.length) {
		if (confirm('Removing: ' + farms[itemToRemove])) {
			for (iter = 0; iter < farms.length-1; iter++) {
				if (iter != itemToRemove) {
					newFarmList = newFarmList + farms[iter] + "&nbsp;";
				}
			}
		}
		else {
...
function valAan()
{
	...
	if(GM_getValue("valaan"+tekst,0) == 1)
	{
		...
		if(intevullen!=""){
			...
			intevullen[aantal] = intevullen[aantal].split("|");
			messageStr += aantal+") ";
			intevullen[aantal][2] =  regxRemoveAll(intevullen[aantal][2], /<[\/]?div[^>]*/, ""); 
			intevullen[aantal][2] =  regxRemoveAll(intevullen[aantal][2], />/, ""); 
			messageStr += intevullen[aantal][2]+" - at ("+intevullen[aantal][0]+")\n"+intevullen[aantal][1];
		}
		addButton = document.createElement('input');
		addButton.type = "button"; // type bepalen
		addButton.value = "Script running, press here to stop!\nNext Target :"+messageStr;
	
		addButton.addEventListener('click',startenstop,true);
		addButton.style.position = "absolute";
		addButton.style.top = "550px";
		addButton.style.left = "750px";
		addButton.style.zIndex = "999";
		addButton.style.width = "450px";
