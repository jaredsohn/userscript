// ==UserScript==
// @name           ShareTheRemote Advanced Buffer Calculator
// @namespace      http://jonls.dk
// @description    Show buffer in page header and on user pages.
// @author         jonls (I just adapted it to StR - dark_skeleton)
// @include        http://sharetheremote.org/*
// @include        https://ssl.sharetheremote.org/*
// @require        http://ajax.googleapis.com/ajax/libs/jquery/1.3/jquery.min.js
// @version        0.1.6
// @date           2009-05-29
// ==/UserScript==
//Edited to fit ShareTheRemote tracker by dark_skeleton
...
(function() {
	...
	GM_registerMenuCommand('ShareTheRemote Advanced Buffer Calculator: '+(display_in_header ? 'Hide buffer in header' : 'Show buffer in header'), function() {
	...
	GM_registerMenuCommand('ShareTheRemote Advanced Buffer Calculator: '+(use_ratio_coloring ? 'Use simple ratio coloring' : 'Use advanced ratio coloring'), function() {
	...
	if (/sharetheremote\.org\/user\.php\?id=/.test(document.URL)) {
		/* Find stats box */
		...
		if (user_up < 15*GiB) append_transition_date(15*GiB, 'Member');
		else if (user_up < 30*GiB) append_transition_date(30*GiB, 'Power User');
		else if (user_up < 100*GiB) append_transition_date(100*GiB, 'Elite');
		else if (user_up < 1*TiB) append_transition_date(1*TiB, 'Couch Potato (TorrentMaster)');
	}
