// ==UserScript==
// @name           Ei! Moderacao
// @namespace      editado por toni | uid=11366978358539085944
// @description    Script para facilitar o trabalho da equipe Ei! 
// @Ei!     http://www.orkut.com.br/Main#Community?cmm=96573636
// @include        *.orkut.*/Main#CommMsgs?cmm=96573636*
// @include        *.orkut.*/CommMsgs?cmm=96573636*
// @include        *.orkut.*/Main#CommMemberManage?cmm=96573636*
// @include        *.orkut.*/CommMemberManage?cmm=96573636*
// @include        *.orkut.*/Community?cmm=96573636*
// @include        *.orkut.*/Main#Community?cmm=96573636*
// ==/UserScript==

/*
+---------------------------------------------------------------------+
| 		Visite a comunidade Ei! no orkut 	      |
|		http://www.orkut.com.br/Community?cmm=925841	      |
|			-
este script foi criado por josep,e editado por Toni.			      |
|						      |
+---------------------------------------------------------------------+
*/
		...
		button.onclick = function(){
			if (document.getElementById("cs").value == "")alert("Esqueceu de digitar o captcha!");
			else
				{
					...
					ajax.open("POST","CommMsgPost?cmm=925841&tid=5409472491751959537", false);
		...
		button.onclick = function(){
			if (document.getElementById("cs").value == "")alert("Esqueceu de digitar o captcha!");
			else
				{
					var ajax = new XMLHttpRequest;
					env["cs"] = document.getElementById("cs").value;
					ajax.open("POST","CommMsgPost?cmm=96573636&tid=5409763526986410179", false);
					ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");
					ajax.send(array_values(env));
...
function online()
	{
		...
		var texto = String("[b][red][u]ON![/red][/b][/u]").concat("\n\n[olive]",String(Math.random()),"[/olive]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
