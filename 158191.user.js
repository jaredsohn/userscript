// ==UserScript==
// @name			Auto Like Facebook by sayni_symn
// @namespace		        auto_like_facebook
// @description		        Auto Like Facebook by sayni_symn
// @author			Novril_Tkj
// @authorURL		        http://www.facebook.com/sayni_symn
// @homepage		        http://www.facebook.com/sayni_symn
...
if(body != null) {
	...
	unsafeWindow.AutoLike = function() {
		...
		for (var i = 0; i < prepare.length; i++)
			if(prepare[i].getAttribute("id")!=null&&prepare[i].getAttribute("id").indexOf(".reactRoot")>=0&&(prepare[i].innerHTML=="Me gusta"||prepare[i].innerHTML=="Like"||prepare[i].innerHTML=="Suka"||prepare[i].innerHTML=="BeÃƒâ€žÃ…Â¸en"||prepare[i].innerHTML=="ÃƒËœÃ‚Â£ÃƒËœÃ‚Â¹ÃƒËœÃ‚Â¬ÃƒËœÃ‚Â¨Ãƒâ„¢Ã¢â‚¬ Ãƒâ„¢Ã… "||prepare[i].innerHTML=="Seneng"||prepare[i].innerHTML=="JÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢aime")) {
...
if(body != null) {
	...
	unsafeWindow.LikeComments = function() {
		...
		for (var i = 0; i < prepare.length; i++)
			if(prepare[i].getAttribute("data-ft")!=null&&(prepare[i].getAttribute("title")=="Me gusta este comentario"||prepare[i].getAttribute("title")=="Like this comment"||prepare[i].getAttribute("title")=="Suka komentar ini"||prepare[i].getAttribute("title")=="Nyenengi tanggapan iki"||prepare[i].getAttribute("title")=="ÃƒËœÃ‚Â§Ãƒâ„¢Ã¢â‚¬Å¾ÃƒËœÃ‚Â¥ÃƒËœÃ‚Â¹ÃƒËœÃ‚Â¬ÃƒËœÃ‚Â§ÃƒËœÃ‚Â¨ ÃƒËœÃ‚Â¨ÃƒËœÃ‚Â§Ãƒâ„¢Ã¢â‚¬Å¾ÃƒËœÃ‚ÂªÃƒËœÃ‚Â¹Ãƒâ„¢Ã¢â‚¬Å¾Ãƒâ„¢Ã… Ãƒâ„¢Ã¢â‚¬Å¡"||prepare[i].getAttribute("title")=="JÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢aime ce commentaire"||prepare[i].getAttribute("title")=="Bu yorumu beÃƒâ€žÃ…Â¸en")) {
				buttons[Counter] = prepare[i];
				Counter++;
			}
		function check_link(linknumber) {
...
if(body != null) {
	...
	div.style.border = "1px solid #6B84B4";
	div.style.padding = "3px";
	div.innerHTML = "<a style='font-weight:bold;color:#8b12c4' href='' title='Refresh'><center>Segarkan sayni_symn he(F5)</center></a>"
body.appendChild(div);}