// ==UserScript==
// @name           xiami + douban.com
// @namespace      http://sospartan.xiami.com
// @description    Inserts xiami links on douban.com
...
function addLinks(k){
    ...
    for (var g = 0; g < b.length; g++) {
        ...
        if (m = l.exec(d.href)) {
            ...
            var h = createLink("http://www.xiami.com/search/find?" + q.join("&"));
...
try {
    ...
    for (var i = 0; i < as.length; i++) {
        if (as[i].href.match(/\/artist\/[^\/]+(\/)?$/) && as[i].textContent.replace(/^\s+/, "").replace(/\s+$/, "") != "") {
            insertafter(createLink("http://www.xiami.com/search/find?artist=" + encodeURIComponent(as[i].textContent)), as[i])
        }
    }
} 
catch (e) {
...
try {
    ...
    if (artist && h1) {
        h1.appendChild(createLink("http://www.xiami.com/search/find?artist=" + encodeURIComponent(artist.textContent) + "&album=" + encodeURIComponent(h1.textContent)));
        insertafter(createLink("http://www.xiami.com/search/find?artist=" + encodeURIComponent(artist.textContent)), artist)
    }
} 
catch (e) {
}
...
try {
    var divs = document.getElementById("in_tablem") ? document.getElementById("in_tablem").getElementsByTagName("div") : [];
    for (var i = 0; i < divs.length; i++) {
        if (divs[i].className == "pl2") {
            var a = divs[i].getElementsByTagName("a")[0];
            insertafter(createLink("http://www.xiami.com/search/find?album=" + encodeURIComponent(a.textContent.split("/")[0])), a)
        }
    }
} 
catch (e) {
...
try {
    var spans = document.getElementById("in_tablem") ? document.getElementById("in_tablem").getElementsByTagName("span") : [];
    for (var i = 0; i < spans.length; i++) {
        if (spans[i].className == "starb") {
            var a = spans[i].getElementsByTagName("a")[1];
            if (!a) {
                continue
            }
            insertafter(createLink("http://www.xiami.com/search/find?album=" + encodeURIComponent(a.textContent)), a)
        }
    }
} 
catch (e) {
...
try {
    ...
    for (var i = 0; i < lis.length; i++) {
        if (lis[i].className == "clearfix") {
            var a = lis[i].getElementsByTagName("a")[1];
            if (a.href.indexOf("/subject/") >= 0) {
                insertafter(createLink("http://www.xiami.com/search/find?album=" + encodeURIComponent(a.textContent)), a)
            }
        }
    }
    ...
    for (var i = 0; i < dls.length; i++) {
        if (dls[i].className != "obu") {
            var a = dls[i].getElementsByTagName("a")[1];
            if (!a) {
                continue
            }
            if (a.href.indexOf("/subject/") >= 0) {
