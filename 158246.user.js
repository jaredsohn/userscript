...
acss =	"table#"+allIDs[0]+" {width:100%; border-collapse:collapse; font-size:8pt; text-align:center; background-color:white; padding:2px; margin:1px;}" +
	...
	"."+allIDs[33]+" { height:12px;width:18px;background: url("+img_car+") no-repeat 0px 0px !important; }" +
	"."+allIDs[34]+" { height:12px;width:18px;background: url("+img_def+") no-repeat 0px 0px !important; }" +
...
var vLinksPat = ver4FL?'//div[@id="'+(ver4nFL?'sidebarBoxVillagelist':'villageList')+'"]//li/a'+(ver4nFL?'/div':''):'//table[@id="vlist"]//div/a[contains(@href, "newdid")]';
...
function karteDistance () {
	if( ver4FL ) {
		...
		if( ! ver4nFL && $gc('sideInfoAlly',$g(pageElem[2])).length == 0 ) topRS[RB.Setup[2]] += 30;
		karteDistance4();
	} else karteDistance3();
}
...
function buildDispatcher () {
	...
	if( gid == 'gid17' ) {
		...
		crannyCalc();
	} else if( gid == 'gid19' || gid == 'gid20' || gid == 'gid21' || gid == 'gid25' || gid == 'gid26' || gid == 'gid36' ) {
		calcTroopCost();
...
function returnQuickHelp () {
	...
	if( qh ) $at($gt('A',qh)[0],[['href','#'],['onclick',i]]);
		...
		if( ver4nFL )
			$g('outOfGame').appendChild($ee('li', $ee('A',trImg(''),[['href','#'],['target','_blank'],['onclick',i]]), [['class',"help"]]));
		else
...
function reportsDelOrSearch () {
	...
	if( ver4FL ) {
		...
		var bc = ver4nFL?'button-content':'button-contents';
