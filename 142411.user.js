// ==UserScript==
// @name          Travian MarketPlace/RallyPoint Auto-Helper by Infected
...
// @author        Infected89 based on Guy Fraser and original by Ankur Saxena
// ==/UserScript==
...
(function() {
 ...
 var i = villages.length;
  if (brally) {
  	 sel += "<option value = '"+(-1*i)+"' onClick='document.snd.x.value=0;document.snd.y.value=0;'>= Eigene Dörfer =</option>";
	 ...
	 // sel += "<option value = '"+(-1*i)+"' onClick='document.snd.x.value=0;document.snd.y.value=0;'>----------------</option>";
} else {
  	sel += "<option value = '"+(-1*i)+"' onClick='document.snd.x.value=0;document.snd.y.value=0;'>= Eigene Dörfer =</option>";
	 for (cnt = 1; cnt<=20; cnt++) {
