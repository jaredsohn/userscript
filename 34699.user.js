// ==UserScript==
// @name           IK專用"警示及概覽表"Script包(for IK v.0.2.8)_0.1.1beta
// @author         小麥子;tiger1971       
// @namespace      
// @description    Original script:http://userscripts.org/scripts/show/29431
//                 功能概述
//                 1.自動更新IK網頁（每5-10分鐘）
...
/**************************************************************************************************
Version history:
- 2008.10.23(0.1.1 beta): fixed bug storage apacity of warehouse
- 2008.10.23(0.1.0 beta): fixed bug tavern LV18+
- 2008.10.22(0.0.9 beta): add for IK TW server
- 2008.09.30(0.0.7 beta): Traditional Chinese translation
- 2008.09.29(0.0.1 beta): modify release
**************************************************************************************************/
...
var warehouseWoodCapacities = [1000, 1160, 2200, 3576, 5336, 7424, 9975, 12799, 16152, 19944, 24200, 28791, 34040, 43520, 54439, 66528, 80024, 320096, 640192, 1280384, 2560768, 5121536, 10243072, 20486144, 40972288, 81944576, 163889152, 327778304, 655556608, 1311113216, 2622226432, 5244452864, 10488905728];
var warehouseOtherCapacities = [300, 420, 500, 1152, 2052, 2248, 3507, 3780, 5332, 7179, 9347, 11784, 14499, 20028, 23548, 27484, 34932, 139728, 279456, 558912, 1117824, 2235648, 4471296, 8942592, 17885184, 35770368, 71540736, 143081472, 286162944, 572325888, 1144651776, 2289303552, 4578607104];
...
function getLocalizedTexts() {
  if (language == "hu") {
  ...
  } else if (language == "tw") { //by tiger1971
    buildings = {
      "townHall"      : ["市政府", "市政府"],
      "academy"       : ["學院", "學院"],
      "port"          : ["港口", "港口"],
    ...
    texts = {
      "cityName": "城市", "currentlyBuilding": "目前正在建設",
      "summary" : "整體:", "hide_settings": "隱藏設置", "show_settings": "顯示設置",
	  "population": "人口", "wood": "木頭", "wine": "葡萄酒", "marble": "石頭", "glass": "水晶", "sulfur": "硫磺",
    };
  } else {
    buildings = {
...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>"+" 尚餘 "+floatFormat(remhour, 1) + " 小時資源滿載";
    } else if (factPerHour < 0) {
      remaining = "<br>"+" 尚餘 "+floatFormat(curres / -factPerHour, 1) + " 小時資源告竭";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + " 容量單位<br>" + perc+"% 滿載" + remaining)+">"+
...
function createProd(prodPerHour, extraTooltip) {
  ...
  var tooltip = mynumberformat(Math.round(24 * prodPerHour), true)+" / 天";
...
function renderTables() {
  ...
  if (TABLE_RESOURCES) {
	...
	if (language == "tw")
	{
		woodName = texts["wood"];
		wineName = texts["wine"];
		marbleName = texts["marble"];
		glassName = texts["glass"];
		sulfurName = texts["sulfur"];
		populationName = texts["population"];
	}
...
//        growthRemainingHours = floatFormat((parseInt(spacetotal) + parseInt(bonusspace) - parseInt(population)) / (0.02 * happiness), 1) + " h";
  ...
  if (true) {
    ...
    t.appendChild(createRowChk("---Alert sounds  警告音效開啟:", "ALERT_SOUNDS", ALERT_SOUNDS));
    t.appendChild(createRowInput("---Alert volume  警告音效大小 (0 - 100):", "ALERT_VOLUME", ALERT_VOLUME));
    t.appendChild(createRowInput("---Warning volume  警示音效大小 (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("---Auto refresh  自動更新頁面開啟:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("---Auto refresh min seconds  自動更新頁面最小秒數:", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("---Auto refresh max seconds  自動更新頁面最大秒數:", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("---Show resources table  顯示資源表:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("---Show buildings table  顯示建物表:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("---Show army and fleet table  顯示部隊表:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("---Show players and cities table  顯示玩家及城市表:", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("---Resource progress bar mode  資源進度顯示模式:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "關閉", time: "時間", percent: "百分比"}));
    t.appendChild(createRowSlct("---Language  語言:", "LANGUAGE", language, {"": "Automatic from server name", en: "English", hu: "Magyar", de: "German", cz: "Czech", tr: "Turkish", es: "Spanish", ba: "Bosnian", it: "Italian", pt: "Portuguese", fr: "French", tw: "繁體中文"}));
    t.appendChild(createRowInput("---Own alliance (short name)  所屬聯盟(請用縮寫):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("---Friendly alliances (short names, separated by comma)  友盟(請用縮寫, 多項可用逗號分隔):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("---Hostile alliances (short names, separated by comma)  敵盟(請用縮寫, 多項可用逗號分隔):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("---CSS  串接樣式表:", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("---Log debug messages  日誌除錯訊息開啟:", "DEBUG_LOG", DEBUG_LOG));
