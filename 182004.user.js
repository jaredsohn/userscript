...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            m += "<tr align=center valign=top><th width=10%>Chairs</th><th width=10%>Tables</th><th width=10%>Windows</th><th width=10%>Banners</th><th width=10%>Advisors</th><th width=10%>Throphies</th><th width=10%>Candelabra</th><th width=10%>Heroes</th></tr>";
            m += '</table></div>';
            m += '</div>';
            m += '</div>';
...
Tabs.Throne = {
  ...
  init : function (div){
	...
	main +='<TD><INPUT class=pbSubtab ID=ptmrchSubST type=submit value="'+lang.AUTOTRONE.Stats+'"></td><TD><INPUT class=pbSubtab ID=ptmrchSubOR type=submit value="'+lang.AUTOTRONE.equip.equip+'"></td><TD><INPUT class=pbSubtab ID=ptmrchSubPR type=submit value="Preset"></td><TD><INPUT class=pbSubtab ID=ptmrchSubTC type=submit value="Caps"></td>\
	<td><INPUT class=ksSubtab ID=ptUniqueItems type=submit value="'+uW.g_js_strings.throneRoom.unique+'"></td></tr></table><HR class=ptThin>';
	...
	ById('ptUniqueItems').addEventListener('click', t.UniqueItems, false);
 	...
 	var t = Tabs.Throne;
 	 try {
 		if(t.viewUniqueItems == null)
 			t.viewUniqueItems = new CPopup ('BOviewUniqueItems', 0, 0, 1050, 750, true);
 		...
 		var items_type = ["","chair", "advisor", "window", "banner",  "table", "trophy", "candelabrum",  "hero" ];
 		var uniqueInThrone = [];
 		...
 		var m= '';
 		m += '<div style="max-height:700px; height:700px; overflow-y:auto">';
 		m += '<TABLE width=100%>';
 		for (var i in unique) {
 			var item = unique[i];
 			if (parseInt(i) > 30000 && parseInt(i)!=30240 && parseInt(i)!=30241 && parseInt(i)!=30250 && parseInt(i)!=30251 && parseInt(i)!=30263 && parseInt(i)!=30265 && parseInt(i)!=30266 && parseInt(i)!=30264) {
 				if (uniqueInThrone.indexOf(item.Id)) var color = "#ffc"; else var color = "";
 				m += "<tr>";
 				m += '<td colspan=2 style="background-color:'+color+'">';
 				m += '#'+item.Id + ' ';
 				m += '<span class="boldRed">'+uW.g_js_strings.throneRoom['unique_'+items_type[item.Type]+item.Id]+'</span></td>';
 				for (var j = 1; j < 15; j++) {
 				...
 				for (var j = 1; j < 15; j++) {
 					...
 					for (var e = 0; e<itemeffects.length; e++) {
 						...
 						if (!p) p = uW.cm.thronestats.tiers[id][tier-1];
 						if (!p) p = uW.cm.thronestats.tiers[id][tier-2];
 						if (!p) p = uW.cm.thronestats.tiers[id][tier-3];
 						if (p) {
 						var base = p.base || 0;
 		...
 		t.viewUniqueItems.getTopDiv().innerHTML = '<DIV class="popupBanner">'+uW.g_js_strings.throneRoom.unique+'</div>';
 ...
 Caps : function (){
 	 ...
 	 function calc(ue, item, level, H) {
 		...
 		m += "<tr>";
 		m += '<td>&nbsp;</td>';
 		for (var i = 1; i<15; i++) m+= '<td style="text-align:center;"><b>+'+i+'</b></td>';
 ...
 S2createRow : function()
     ...
     pickCity : function () {
            ...
            var ind = -1,lowest = Math.round(uW.cm.WorldSettings.getSetting("DARK_FOREST_AETHERSTONE_CAP")*(1+ (Math.min(uW.cm.ThroneController.effectBonus(88),uW.cm.thronestats.boosts.ResourceCap.Max) *0.01)));
            if (salvageData.anyCity)   {
			 for (i= 0; i < Seed.cities.length; i++)  {
               if (salvageData.overflow == "lowest")  {
        ...
        S2EditRule:function(i) {
			var t = Tabs.Throne;
		ById('BOCardType').value=salvageData.ruleSet[i].type;
		ById('BOFactionType').value=salvageData.ruleSet[i].faction;
		ById('BOCardType').style.backgroundColor="#FF9999";
		ById('BOFactionType').style.backgroundColor="#FF9999";
		var table = ById('BOConditionTable');
	    var rowCount = table.rows.length;
		...
		//var a=0;
		t.rowNum=0;
		for (var a=0;a < salvageData.ruleSet[i].conditions.length ; a++) {

		  t.S2createRow();
		  var condition = salvageData.ruleSet[i].conditions[a];
		  if (condition.mustHave != "false")
		   ById("BOr"+a+"sel1").value="true";
		  else
		   ById("BOr"+a+"sel1").value="false";
		  ById("BOr"+a+"sel1").style.backgroundColor="#FF9999";
		  ById("BOr"+a+"sel2").style.backgroundColor="#FF9999";
		  ById("BOr"+a+"sel3").style.backgroundColor="#FF9999";
		  ById("BOr"+a+"sel2").value=condition.number;
		  ById("BOr"+a+"sel3").value=condition.effect;
		  ById("BOr"+a+"slot1").checked=condition.slots[0];
		  ById("BOr"+a+"slot2").checked=condition.slots[1];
		  ById("BOr"+a+"slot3").checked=condition.slots[2];
		  ById("BOr"+a+"slot4").checked=condition.slots[3];
		  ById("BOr"+a+"slot5").checked=condition.slots[4];
		  ById("BOr"+a+"slot1").style.backgroundColor="#FF9999";ById("BOr"+a+"slot2").style.backgroundColor="#FF9999";
		  ById("BOr"+a+"slot3").style.backgroundColor="#FF9999";ById("BOr"+a+"slot4").style.backgroundColor="#FF9999";
		  ById("BOr"+a+"slot5").style.backgroundColor="#FF9999";
		 }
		},
        ...
        S2readRows : function()
	        ...
	        countp:false, countpp:0,
			 ...
			 if (num_items > 120)
                ...
                $("a.buttonv2.throne").css('color', 'black');
            if(!salvageData.salvageActive)return;
            if (t.deleting == true) return;
              if (t.countp==true && t.countpp<5) {
				t.countpp++;
				return;
			}
			t.countpp=0;t.countp=false;
            t.deleting = true;
            t.setStatus(lang.AUTOTRONE.autosalvage2.status1);
            t.delItems = t.S2buildList(false);
            if (t.delItems.length > 0) {
                if (salvageData.UpgradeP1)t.S2doUpgrade();
				else t.S2doDelete();
            } else {
                t.deleting = false;
			    ...
			    onSuccess: function (transport) {
                    ...
                    var t = Tabs.Throne;
                     if (m.ok === true){
	                     ...
	                     setTimeout(function() {t.S2doDelete();},1500);
...
Upgrade_Enhance :function (){
    ...
    //try {
      ...
      m += '<td><input type=checkbox id=BOuse20006 '+(ThroneOptions.Use20006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20001.name+'" src="'+gameurl+'img/items/70/20001.jpg">\
      <input type=checkbox id=BOuse20002 '+(ThroneOptions.Use20006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20002.name+'" src="'+gameurl+'img/items/70/20001.jpg">\
      <input type=checkbox id=BOuse20005 '+(ThroneOptions.Use20006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20005.name+'" src="'+gameurl+'img/items/70/20001.jpg">\
      <input type=checkbox id=BOuse20006 '+(ThroneOptions.Use20006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20006.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <input type=checkbox id=BOuse20004 '+(ThroneOptions.Use20006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20004.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <br><input type=checkbox id=BOuse20012 '+(ThroneOptions.Use2006?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20012.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <input type=checkbox id=BOuse20013 '+(ThroneOptions.Use20013?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20013.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <input type=checkbox id=BOuse20014 '+(ThroneOptions.Use20001?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20014.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <input type=checkbox id=BOuse20015 '+(ThroneOptions.Use20001?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20015.name+'" src="'+gameurl+'img/items/70/20021.jpg">\
      <input type=checkbox id=BOuse20016 '+(ThroneOptions.Use20001?'CHECKED':'')+'><img width=25 title="'+uW.itemlist.i20016.name+'" src="'+gameurl+'img/items/70/20021.jpg"></td><TD><INPUT id=ShowHistory type=submit value="'+lang.AUTOTRONE.history+'"></td>';
	...
	ById("BOuse20015").addEventListener ('click', function (){ ThroneOptions.Use20015=ById("BOuse20015").checked;saveThroneOptions();	},false);
	ById("BOuse20016").addEventListener ('click', function (){ ThroneOptions.Use20016=ById("BOuse20016").checked;saveThroneOptions();	},false);
   ...
   if (ThroneOptions.Tries > 0) ById('ShowTries').innerHTML = lang.AUTOTRONE.upgrade.retry+"" + ThroneOptions.Tries + "<br />"+lang.AUTOTRONE.upgrade.good+" " + ThroneOptions.Good + "  "+lang.AUTOTRONE.upgrade.bad+" " + ThroneOptions.Bad;
	...
	t.paintHoover();
    t.timer = setTimeout(t.paintStones,5000);
},
...
showNext : function () {
            ...
            $.each(X.effects, function (Z, aa) {
                ...
                else if (uW.cm.thronestats.tiers[aa.id][aa.tier-1]) S = uW.cm.thronestats.tiers[aa.id][aa.tier-1];
                else S = uW.cm.thronestats.tiers[aa.id][aa.tier-2];
...
buildCard : function(I){
     ...
     for (M in I.effects)
     {
       ...
       else if (uW.cm.thronestats.tiers[N.id][N.tier-1])  tier=uW.cm.thronestats.tiers[N.id][N.tier-1];
        else tier=uW.cm.thronestats.tiers[N.id][N.tier-2];
...
calcTimeBroked:function(){
    ...
    var ag=0,ac=0,ad=0,af=0;
	$.each(G,function(J,I){
		++A;
		if(A>D){return false}
		if (I.isBroken) {
		 Y=uW.cm.ThroneController.hasFactionBonus([I]);
		 if(I.brokenType=="quality"){
		 	ag=uW.cm.thronestats.repairCostEnhance[I.quality].Time;
		 }else{
		  if(I.brokenType=="level"){
		 	 ag=uW.cm.thronestats.repairCostUpgrade[I.level].Time;
		  }
		 }
		 if(Y.hazBonus&&Y.faction==="briton"){
		 		ad=uW.cm.ThroneController.effectBonus(94);
...
Stats :function (numslot){
   ...
   // try {
    ...
    m = '<DIV class=boStat><b>'+lang.AUTOTRONE.stats.title+'</b></div><center><table border=0 width=75%><tr><td>'+lang.AUTOTRONE.stats.power+' <b>'+addCommas(uW.cm.ThroneController.calcMight())+'</b><br>'+lang.AUTOTRONE.stats.powerbrok+' '+addCommas(t.calcMightNoBroked())+'<br>'+lang.AUTOTRONE.stats.timebrok+' '+ uW.timestr(t.calcTimeBroked())+'</td>\
    <td><button id=boXLS>Items List</button></td></tr></table><table><tr><td><b><center>'+lang.AUTOTRONE.equip.preset2+'<br>('+ThroneOptions.presetname[Seed.throne.activeSlot]+')</center></td><td><enter><b>Comparer : <select id="BOListPStat">'+tt+'</select></td></tr><tr><td align=top><table align=top>';
    var k=Seed.throne.slotEquip[Seed.throne.activeSlot],i=Seed.throne.inventory,e,faction;
      var B=0, w={}, x, C;
           ...
           for (var K in k) {
            ...
            if (F.quality >= x) {
             ...
             else if (uW.cm.thronestats.tiers[F.effects[M].id][F.effects[M].tier-1]) z = uW.cm.thronestats.tiers[F.effects[M].id][F.effects[M].tier-1];
             else z = uW.cm.thronestats.tiers[F.effects[M].id][F.effects[M].tier-2];
 	...
 	var title=uW.g_js_strings.commonstr.throneroom + ' (' + ThroneOptions.presetname[numslot]+') ';
	  ...
	  ById("boXLS").addEventListener('click', function(){
	   		 t.ExportXLS();
	  },false);
},
viewXLSItems:null,
ExportXLS: function() {
	var t = Tabs.Throne;
	var m ="";
	if(t.viewXLSItems == null)
 			t.viewXLSItems = new CPopup ('BOViewItemXLS', 0, 0, 1050, 750, true);
 	t.viewXLSItems.centerMe (mainPop.getMainDiv());
 	m = "<DIV style='height:575px; max-height:575px; overflow-y:auto'><table><tr><td>Domain<td>Name</td><td>Type</td><td>Faction</td><td>Level</td><td>Effet 1</td><td>Effet 2</td><td>Effet 3</td><td>Effet 4</td><td>Effet 5</td></tr>";
 	for(i in uW.kocThroneItems) {
 		var item = uW.kocThroneItems[i];
		if (item) {
		 if (item.quality>=5) {
			m+="<tr><td>"+getServerId()+"</td><td>"+ThroneItemName(item)+"</td><td>"+item.type+"</td><td>"+item.faction+"<td>"+item.level+"</td>";
			for (var u in item.effects) {
			  L = item.effects[u];
			  var id = item.effects[u].id;
			  var tier = item.effects[u].tier;
			  effect = uW.cm.thronestats.effects[id];
              p = false;
			  while (!p && (tier > -1)) {
			  	p = uW.cm.thronestats.tiers[id][tier];
			  	tier--;
			  ...
			  var base = p.base || 0;
			  var growth = p.growth ||0;
			  var percent = + base + (item.level * item.level + item.level) * + growth * 0.5;
              percent = Math.ceil(percent);
			  m+="<td>" + percent + "% " + effect[1] + "</td>";
			}
			m+="</tr>";
		 }
	...
	m+="</table></div>";
	t.viewXLSItems.getMainDiv().innerHTML = m;
	t.viewXLSItems.getTopDiv().innerHTML = '<DIV class="popupBanner">List</div>';
 	t.viewXLSItems.show(true);
        ...
        if (ById('ShowQueue')) {
	     ...
	     if (y) {
			 ...
			 for (i=1;i<=5;i++) {
			 			   ...
			 			   p = false;
						   while (!p && (tier > -1)) {
						   	p = uW.cm.thronestats.tiers[idzz][tier];
						    tier--;
						   }
						   var base = p.base || 0;
						   var growth = p.growth ||0;
			 			   var pourc = + base + (level * level + level) * + growth * 0.5;
				 		   ...
				 		   if (i<=quality) namef+='<TR><TD align=left><FONT size=1 color=black>' + pourc + "% " + uW.cm.thronestats["effects"][idzz]["1"] + '</font></td></tr>';
			 					   else namef+='<TR><TD align=left><FONT size=1 color=black>' + pourc + "% " + uW.cm.thronestats["effects"][idzz]["1"] + '</font></td></tr>';
...
addToQueue : function (id,action){
    ...
    for (k=0;k<ThroneOptions.Items.length;k++){
		...
		if (uW.kocThroneItems[ThroneOptions.Items[k]["id"]] != undefined) {
				...
				if (ThroneOptions.Items[k]["action"] == "Upgrade") {
					...
					if (ThroneOptions.Items[k]["levelto"]>14 && !firstRun) {ThroneOptions.Items.splice (k,1);saveThroneOptions();return;}
				}
  ...
  doEnhance : function() {
				   ...
				   if (Options.mailtrone) {
				    var params = uW.Object.clone(uW.g_ajaxparams);
			   	    ...
			   	    for (var u in s.effects) {
			   				  ...
			   				  else if (uW.cm.thronestats.tiers[L.id][L.tier-1]) tier = uW.cm.thronestats.tiers[L.id][L.tier-1];
			   				  else tier = uW.cm.thronestats.tiers[L.id][L.tier-2];
	...
	doUpgrade : function() {
		...
		if (y.level+1 == 9 && ThroneOptions.Use20015 && Seed.items.i20015 && parseInt(uW.ksoItems[20015].count) > 0) {
			params.buffItemId = 20015; obk = 20015;
		}
		if (y.level+1 == 10 && ThroneOptions.Use20016 && Seed.items.i20016 && parseInt(uW.ksoItems[20016].count) > 0) {
					params.buffItemId = 20016; obk = 20016;
		}
      	new AjaxRequest(uW.g_ajaxpath + "ajax/_dispatch53.php" + uW.g_ajaxsuffix, { method: "post", parameters: params, loading: true, onSuccess: function (transport) {
      	  var rslt = eval("(" + transport.responseText + ")");
      	  if(rslt.ok){
			if (obk != 0) { actionLog("AutoTrone", uW.ksoItems[obk].name +" sur " + y.name+ " (reste "+uW.ksoItems[obk].count+")");uW.ksoItems[obk].subtract(); }
			var gems=0;
			if (rslt.gems > 0) {
			 if (ById('ShowStatus')) ById('ShowStatus').innerHTML = 'Upgrader accidentally spent gems!  Turning upgrader off!!';
			 ThroneOptions.Active = false;
				...
				var rslt = eval("(" + transport.responseText + ")");
					...
					t.repairTimerUpdate();
				   		...
				   		if (m.error_code==256) {
										...
										uW.kocThroneItems[id].brokenType="";
										uW.cm.ThroneView.renderInventory(uW.kocThroneItems);
						}
						saveThroneOptions();
				   }
  				...
  				var rslt = eval("(" + transport.responseText + ")");
  					if(rslt.ok){
  							...
  							if (ById("ShowTrPresets"))
								setTimeout(Dashboard.ShowTrPresets,1000)
		...
		if (!ThroneOptions.Items["0"]) { ThroneOptions.Items=[];return;}
		 if (typeof(uW.kocThroneItems[ThroneOptions.Items["0"]["id"]]) == 'number') {
		  ...
		  for (i=1;i<=5;i++) {
			   ...
			   else if (uW.cm.thronestats.tiers[id][tier-1]) p = uW.cm.thronestats.tiers[id][tier-1];
			   else p = uW.cm.thronestats.tiers[id][tier-2];
