...
function main() {
	...
	(function () {
		...
		if (nightmode) {
		  jQueryUI_theme.attr('href', 'http://37.187.19.134/abw/ABW-night/jquery-ui-1.10.3.custom.css');
		}
		else {
		...
		//
			$('<style />').html(
            ".abw_index {float: right;background-color: rgba(255, 255, 255, 0.2);padding: 5px;"+
            "border-radius: 5px;-webkit-transition: 0.15s linear;cursor: pointer;}"+
            ".abw_index:hover {background-color: rgba(230, 75, 61, 1);color: #fff;}"+
			...
			" div.raport_abw{ color: #fff; background-color: #e74c3c; border: none; font-size: 15px;font-weight: normal;line-height: 1.4; border-radius: 4px; padding: 10px 15px; -webkit-font-smoothing: subpixel-antialiased;-webkit-transition: 0.25s linear;transition: 0.25s linear;"+
			"font-size:17px!important;text-align:center; -webkit-transition: 0.25s linear;"+
			"} div.raport_abw:hover{ background-color: #ec7063;}"
	    ));
	    ...
	    if (Configuration[1] === '1') {
		  ...
		  if (WykopAction === 'index' || WykopAction === 'upcoming') {
			...
			jaszczur_button = $('<div />')
				  .attr('class', 'raport_abw margin5_0 margintop10')
				  .data('id', document.URL)
				  .data('title', $('.userstory h2').text().killWhiteSpace())
				  .text('Zgłos do ABW')
				  .click(ABW_report);
			$('.userstory .contact').after(jaszczur_button);
		  }
