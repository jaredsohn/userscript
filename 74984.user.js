...
switch (navigator.language){
	default : // en-US：tanslated by foxhound
		Lng = {
			...
			resAbout1:'Nwes,focus,videos,pics,blogs related to ',
			resAbout2:'',
			gui_title:'Prefs',
			...
			gui_options_floatOptionsBox:'Options... floats on the left',
			gui_options_shortenUserName:'Shorten the user’s name（user@gmail.com -> user）',
			gui_options_removeAd:'Remove Ads on the right',
			gui_options_EnableSearchInOtherEngines:'Add other search engines',
	...
	case 'zh-CN':
		Lng = {
			OtherEngines:'在百度中搜索【{word}】|http://www.baidu.com/s?ie=utf-8&wd={word}',
			preview_title:'缩略图',
			preference:'GKK设置',
			site_filter:'在此站点中搜索',
			resAbout1:'关于',
	...
	case 'zh-TW':
		Lng = {
			...
			gui_options_floatInputPos_2:'底部',
			gui_options_floatOptionsBox:'百寶箱浮動在頁面左側',
...
function doit(){
	switch (matchNode('//div[@id="res"]/descendant::div/ol/li').snapshotLength > 0) {
		// 若存在搜索结果
	    case true:
	           
	        var ssbLeft = getComputedStyle(document.getElementById('ssb'), '').getPropertyValue('margin-left').replace('px','');
	        var resLeft = getComputedStyle(document.getElementById('res'), '').getPropertyValue('margin-left').replace('px','');
    		// 添加 CSS 样式
    		var css = '\
    				   /* 让li以table方式显示，避免分列时被拆断，以及添加圆角效果等 */\
    				      	.g {display: inline-table; -moz-border-radius: 0.75em; -webkit-border-radius: 0.75em;}\
							.gkk_e, .gkk_tope {-moz-border-radius: 0.75em; -webkit-border-radius: 0.75em; margin-left: ' + (resLeft-ssbLeft) + 'px; padding: 0.2em 0.5em;}\
							#gkk_rel {-moz-border-radius: 0.75em; -webkit-border-radius: 0.75em; padding: 0.2em 0.5em;}\
							.gkk_nav {margin-left: ' + (resLeft-ssbLeft) + 'px;}\
    				   /* 给后面的“共有xx条搜索结果”条目添加向右浮动的样式 */\
    				      	#ssb {margin-bottom: 5px !important;} #ssb p {float: right;}\
    				   /* 添加新闻、博客等相关内容框的样式 */\
    						#resAbout {border: 2px solid #C0CDF7; margin-bottom: 1em; margin-left: ' + (resLeft-ssbLeft) + 'px; padding: 0.5em 1em 0em 0.5em; -moz-border-radius: 0.75em; -webkit-border-radius: 0.75em;}\
    				  ...
    				  '
    				   ...
    				   +
    				   // 添加分栏样式
    				   (' .gkk_MultiCol {-moz-column-count: ' + Pref.columns + '; -moz-column-gap: 0.6em; -webkit-column-count: ' + Pref.columns + '; -webkit-column-gap: 0.5em;}')
    				   +
    				   // 隐藏自动翻页分隔符
		    ...
		    GM_addStyle(css);
	   	    ...
	   	    if(Pref.addFavicon){
	   	     ...
	   	     // 清除“共有xx条搜索结果”条目向右浮动对后面元素的影响
	   	     var divSsb = document.getElementById('ssb');
	   	     var divClear = document.createElement('div');
	   	     divClear.setAttribute('style','clear: both;');
	   	     divSsb.parentNode.insertBefore(divClear, divSsb.nextSibling);
	   	     
	   	     // “共有xx条搜索结果”条目设置向右浮动的样式后，背景色会显示不正常。以下代码修复此问题。
	   	     var br = document.createElement('br');
	   	     br.setAttribute('style','clear: both;');
	   	     divSsb.appendChild(br);
			...
			addId2SearchTip(document.body);
			// 将相关搜索提示移到页面上部
            if (Pref.SearchTipTop) {
                moveSearchTip2Top(document.body);
            }
	        ...
	        setTimeout(function(){
	            ...
	            // 浮动百宝箱
	            if (Pref.floatOptionsBox) {
	                var tbd = document.getElementById('tbd');
	                if (tbd) {
	                    var resTop = document.getElementById('res') ? document.getElementById('res').offsetTop : 10000;
	                    var resAboutTop = document.getElementById('resAbout') ? document.getElementById('resAbout').offsetTop : 10000;
	                    var resRelTop = document.getElementById('gkk_rel') ? document.getElementById('gkk_rel').offsetTop : 10000;
	                    var tbdoffsetTop = resTop < resAboutTop ? resTop : resAboutTop;
	                    tbdoffsetTop = tbdoffsetTop < resRelTop ? tbdoffsetTop : resRelTop;
	                    GM_addStyle('#tbd {position: fixed !important; top: ' + tbdoffsetTop + 'px !important;}');
	                }
	            }
	            ...
	            if (tbpi) {
	            	var mLeft;
	            	document.getElementById('res').addEventListener('DOMAttrModified', function(e){
	            		if (e.attrName != 'style' || e.target.getAttribute('id') != 'res') return;
						mLeft = getComputedStyle(e.target, '').getPropertyValue('margin-left').replace('px','') - 8;
	        			mLeft = mLeft<8?8:mLeft;
	            		GM_addStyle('#resAbout {margin-left: ' + mLeft + 'px;} .gkk_e, .gkk_tope {margin-left: ' + mLeft + 'px !important; width: auto !important;} .gkk_nav {margin-left: ' + mLeft + 'px;}');
	            	}, false);
	            }
	            ...
	            if (Pref.AutoNextPage) {
	                ...
	                window.addEventListener('keydown', function(event){
	                    if ((event.altKey == alt_Next) && (event.ctrlKey == ctrl_Next) && (event.shiftKey == shift_Next) && (event.keyCode == keycode_Next) && (event.target.localName != 'INPUT') && !document.getElementById('preferences')) {
							...
							var i, len = nextSplit.snapshotLength, posOffset = (Pref.floatInput && Pref.floatInputPos==1)?document.getElementById('gkk_header').offsetHeight:0;
							for(i=0;i<len;i++){
								if(window.scrollY<nextSplit.snapshotItem(i).offsetTop - posOffset){
									window.scrollTo(0, nextSplit.snapshotItem(i).offsetTop - posOffset);
									break;
								}
							}
	                ...
	                window.addEventListener('keydown', function(event){
	                    if ((event.altKey == alt_Prev) && (event.ctrlKey == ctrl_Prev) && (event.shiftKey == shift_Prev) && (event.keyCode == keycode_Prev) && (event.target.localName != 'INPUT') && !document.getElementById('preferences')) {
							...
							var i, len = prevSplit.snapshotLength, posOffset = (Pref.floatInput && Pref.floatInputPos==1)?document.getElementById('gkk_header').offsetHeight:0;
							for(i=len-1;i>=0;i--){
								if(window.scrollY>prevSplit.snapshotItem(i).offsetTop - posOffset){
									window.scrollTo(0, prevSplit.snapshotItem(i).offsetTop - posOffset);
									break;
								}
							}
...
function addResultNums(doc){
	var elements = matchNode('//li[child::div[starts-with(@class,"s")] and descendant::a[@class="l"]][not(descendant::*[@class="gkk_num"])]/*[1]', doc);
...
function floatInput(){
	...
	document.body.insertBefore(header, document.getElementById('gbar'));

	with (header) {
		appendChild(document.getElementById('gbar'));
		appendChild(document.getElementById('guser'));
		appendChild(document.getElementById('tsf'));
	}
	document.body.insertBefore(headerContainer, header);
		...
		switch (Pref.floatInputPos) {
			case 1: // 顶部
				...
				if (Pref.OtherEnginsFloat) {
					appendChild(document.getElementById('ssb'));
					rplheader.style.height = headerContainer.offsetHeight + 'px';
					css = '#gkk_header {position: fixed; top: 0px; width: 99%; background-color: white; z-index: 10;}';
				}
				else {
					rplheader.style.height = headerContainer.offsetHeight-(isChrome?0:3) + 'px';
			...
			case 2: // 底部
				...
				if (Pref.OtherEnginsFloat) {
					...
					appendChild(header);
					rplheader.style.height = headerContainer.offsetHeight + 'px';
					css = '#gkk_header {position: fixed; bottom: 0px; width: 99%; background-color: white; z-index: 10;}';
				}
				else {
					appendChild(header);
					rplheader.style.height = headerContainer.offsetHeight + 'px';
				...
				// 隐藏底部的搜索框、减小搜索框上的间隙
				css += '.clr {display: none;} #sft table ,#logo {margin-top: 0 !important;}';

				// 将搜索框中的“网页 图片 地图 资讯 视频 博客 更多”栏目放到底部
				header.insertBefore(document.getElementById('tsf'), header.firstChild);
	...
	// 隐藏分割线
	var gbhs = matchNode('//div[@class="gbh"]');
	var i, gbh, len = gbhs.snapshotLength;
	for(i=0;i<len;i++){
		gbhs.snapshotItem(i).style.visibility = 'hidden';
	}
	GM_addStyle(css);
}
...
function removeTracking(doc){
	var links = matchNode('//a[string-length(@onmousedown)>0]',doc);
...
function OpenInNewTabs(doc){
	var links = matchNode('//a[(@class="l" and ancestor::div[@id="res"]) or (ancestor::div[@id="resAbout"])]',doc);
	var i, len = links.snapshotLength;
    for (i = 0; i < len; i++) {
...
function addseatchSite(doc){
    ...
    for (i = 0; i < len; i++) {
		...
		while(span.nodeType != 1 && span != null) {
...
// 给相关搜索提示添加一个id = gkk_rel
function addId2SearchTip(doc){
    var relDivs = matchNode('//div[@id="res"]/descendant::div[contains(@class,"e") and child::table[@id="brs" or descendant::*[contains(@class,"std")]]]', doc);
	var len = relDivs.snapshotLength, relDiv;
    for(var i=0;i<len;i++){
    	relDivs.snapshotItem(i).setAttribute('id','gkk_rel');
    }
}

// 将相关搜索提示移到页面上部
function moveSearchTip2Top(doc){
    var relDivs = matchNode('//div[@id="gkk_rel"]', doc);
	var len = relDivs.snapshotLength, relDiv;
    if (len > 0) {
...
try{
	var relDivs = matchNode('//div[@id="gkk_rel"]');
    if (relDivs.snapshotLength > 0) {
        return;
    }
    var i, len;
...
try{
    var i, len;
	var ols = matchNode('//div[@id="res"]/descendant::div/ol', doc)
	var resContainer;
	...
	var resAboutLis = matchNode('//div[@id="res"]/descendant::div/ol/li[not((@class="g" or starts-with(@class,"g ")) and child::div[starts-with(@class,"s")])]\
								...
								|//div[@id="res"]/descendant::div/ol/li[(@class="g" or starts-with(@class,"g ")) and child::div[starts-with(@class,"s")] and child::h3[@class="r"]/a[not(@class="l")]]\
								', doc);
    ...
    if (Pref.direction == 1 && Pref.columns > 1) {
        var resLis = matchNode('//div[@id="res"]/descendant::div/ol[starts-with(@id,"gkk")]/li', doc);
    ...
    // 删除自动加载下一页中的 相关搜索和相关服务条目，如果有的话。。。
    if (doc != document.body && pagecount != 'pref') {
        var relDiv = matchNode('//div[@id="res"]/descendant::div[@class="e" and child::table]', doc);
		len = relDiv.snapshotLength;
        for (i = 0; i < len; i++) {
			relDiv.snapshotItem(i).parentNode.removeChild(relDiv.snapshotItem(i));
        }
    }
}
catch(e){
...
function format(id, Pref){
    ...
    var resTables = matchNode('//ol[@id="' + id + '"]/li[@class="g" or starts-with(@class,"g ") or @class="plus"]');
    ...
    if (len > 0) {
        ...
        var tables = matchNode('//ol[@id="' + id + '"]/li[@class="g" or starts-with(@class,"g ") or @class="plus"]');
        len = tables.snapshotLength;
        var rows = len / Pref.columns;
//		var modHeight = tables.snapshotItem(0).offsetHeight - parseInt(getComputedStyle(tables.snapshotItem(0),'').height);
		...
		for(i=0;i<len;i++){
//			modTables[i][0].setAttribute('style', modTables[i][0].getAttribute('style') + '; height: ' + (modTables[i][1] - modHeight) + 'px !important;');
			modTables[i][0].setAttribute('style', modTables[i][0].getAttribute('style') + '; height: ' + (modTables[i][1]) + 'px !important;');
		}
...
function prefOnChange(){
    ...
    var gkk_pref_res = document.getElementById('gkk_pref_res').cloneNode(true);
	...
	// 添加百宝箱占位符
	if(optionsBox){
		matchNode('//div[@id="gkk_pref_tbd"]', gkk_pref_res).snapshotItem(0).style.display='block';
		matchNode('//div[@id="gkk_pref_tbd"]', gkk_pref_res).snapshotItem(0).style.width = (parseInt(optionsBoxMarginLeft) - 27) + 'px' ;
	}
	else{
		matchNode('//div[@id="gkk_pref_tbd"]', gkk_pref_res).snapshotItem(0).style.display='none';
	}

	// 将相关搜索提示移到页面上部
    if (gkk_pref_Pref.SearchTipTop) {
		moveSearchTip2Top(gkk_pref_res);
    }
        
	// 先清除现有的预览效果
	var div = matchNode('//div[@id="gkk_pref_res" and not(contains(@style,"display:"))]').snapshotItem(0);
                            ...
                            <tr>\
                                <td colspan="2">\
                                    <fieldset id="gkk_pref_preview">\
										...
										<div id="gkk_pref_res" class="gkk_pref_preview_id-res" style="display: none;">\
											<div id="gkk_pref_tbd" style="border: 1px solid rgb(187, 187, 187); margin: 6px 4px -3px 0px; height: 14em; position: absolute; word-wrap: break-word;">\
												<div style="margin: 6em 1em 5em 1em;">' + Lng.gui_preview_optionsBox + ' ...</div>\
											</div>\
											<div id="res">\
                            ...
                            <tr id="gkk_pref_options">\
                                <td width="50%" valign="top">\
                                    <fieldset>\
										...
										<div>\
						                    <label>\
						                        <input name="SearchInOtherEngines" id="SearchInOtherEngines" type="checkbox" ' + (Pref.EnableSearchInOtherEngines ? 'checked="true"':'') + '>&nbsp;' + Lng.gui_options_EnableSearchInOtherEngines + '\
						                    </label>\
											...
											<table style="margin-top: 0.4em; padding: 0.5em 0.5em;">\
												<tr>\
													<td>\
						                    			...
						                    			<label>\
						                    			    <textarea rows="6" cols="61" id="customizeCSS" wrap="off" style="font-size: small;">' + Pref.customizeCSS + '</textarea>\
						                    			</label>\
													</td>\
												</tr>\
											</table>\
    ...
    if (!IsLoadingNext && NextPageLink && document.body.scrollHeight - window.scrollY < window.innerHeight * 3 && !document.getElementById('preferences')) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(d){
				...
				// 删除页面导航条中的背景图片，只保留文字链接
				var navSpan = matchNode('//span', navTable);
				len = navSpan.snapshotLength;
				for(var i=0;i<len;i++){
					navSpan.snapshotItem(i).parentNode.removeChild(navSpan.snapshotItem(i));
				}
				// 向页面导航条中插入翻页提示
				...
				td2.innerHTML = '】';
				navTable.firstChild.firstChild.insertBefore(td1,navTable.firstChild.firstChild.firstChild);
				navTable.firstChild.firstChild.appendChild(td2);
                // 在当前页插入自动翻页分隔符
                var splitDiv = document.createElement('div');
				with(splitDiv){
					setAttribute('class','gkk_nav');
...
function showUpdateMessage(newVersion){
	var seperate = document.createElement('apsn');
	seperate.innerHTML = '&nbsp;&nbsp;-&nbsp;&nbsp;';
