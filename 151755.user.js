// ==UserScript==
// @name           OGameRediseño Compactador Batallas by Pepineitor
// @namespace      http://userscripts.org/users/491961
// @description    OGameRediseño Compactador Batallas
// @version        1.0
// @author         Pepineitor
// @updateURL      http://userscripts.org/scripts/source/151755.meta.js
...
var SCRIPT = {
  name: "Compactador de batallas mejorado de Pepineitor"
  ,url: "http://userscripts.org/scripts/show/151755"
  ,version: "1.0"
  ,funciona_ok: "5.1.99.99"
};
...
function codificar(patron, tipo, version) {
    ...
    var colores = [
        ...
        [/{COLOR_T1}/gi, '#0066FF'], // titulo 1 ("defensores", "atacantes", "perdidas", y "rentabilidad"
        [/{COLOR_A1}/gi, '#00FF40'],
        [/{COLOR_A2}/gi, '#00DDDD'],
        [/{COLOR_D1}/gi, '#ED7010'], 
...
function compactar()  {
        ...
        if(numRondas == 1) {
			patron += '{SIZE_MED}' + getFecha() + '{/SIZE}{NL}';
			patron += '{SIZE_PEQ}La batalla duró {SIZE_MED}1{/SIZE} ronda{/SIZE}{NL}';
        }
        else {
			patron += '{SIZE_MED}' + getFecha() + '{/SIZE}{NL}';
			patron += '{SIZE_PEQ}La batalla duró ' + numRondas + ' rondas {/SIZE}{NL}';
        }
        
        
        // ATACANTES
        patron += '{COLOR_T1}{B}{SIZE_MED}Atacantes (' + lstAtaq.length() + '):{/SIZE}{/B}{/COLOR}{NL}';
        ...
        for(var i = 0; i < lstAtaq.length(); i++){
         ...
         // DEFENSOR
        patron += '{COLOR_T1}{B}{SIZE_MED}Defensores (' + lstDef.length() + '):{/SIZE}{/B}{/COLOR} {NL}';
        ...
        perdidasTotales[3] = (perdidasAtaq[0] + perdidasDef[0]) + (perdidasAtaq[1]*2 + perdidasDef[1]*2) + (perdidasAtaq[2]*3 + perdidasDef[2]*3);
