// ==UserScript==
// @name           FlickrGroupPoolTagger
// @namespace      http://www.flickr.com/alesadam
// @description    This script allows you to easily add extra tags to your pool's photos
// @creator        Alesa Dam (http://flickr.com/alesadam/)
// @date           20/01/2011
// @version        1.0.2
// @modified       Feb 04, 2011
//
// @include        http://*flickr.com/groups/*/pool/tags/*
// @include        http://*userscripts.org/*
// @exclude        http://*flickr.com/groups/*/?search*
//
// ==/UserScript==
//
// Discussion: http://www.flickr.com/groups/adminhelp/discuss/72157625730021161/
//
// --------------------------------------------------------------------
//
...
(function () {
var FGPTtoolversion="V1.0.2";
var scriptNumber = "95052";
// Greased MooTools:
/*
---
...
if (Browser.Engine.webkit || // Chrome, Safari
    ...
    var keyPrefix = 'FlickrGroupPoolTagger.';
    ...
    GM_getMagisterLudi = function () {
        return unsafeWindow.global_magisterLudi;
    }
...
function showUpdateNotification() {
    ...
    new Element('a', {
        html: 'FlickrGroupPoolTagger: update available',
    ...
    new Element('a', {
        ...
        href: 'http://www.flickr.com/groups/adminhelp/discuss/72157625730021161/',
        styles: {
            'text-decoration': 'none'
        },
        target: '_blank'
...
function checkVersion() {
  try {
    ...
    if ($chk(onlineVersion)) {
        ...
        var currentVersionParts = reVersionMatch.exec(FGPTtoolversion);
...
var FGPTPreferences = new Class({
    options: {
        onlyAdmin: true,
        rememberEnforceTag: true
    },
    initialize: function () {
        var storedOnlyAdmin = GM_getValue("FGPT.preference.onlyAdmin");
        if (storedOnlyAdmin !== undefined) {
            this.options.onlyAdmin = storedOnlyAdmin === true; // Google chrome trickery
        }
        var storedRemember = GM_getValue("FGPT.preference.rememberEnforceTag");
        if (storedRemember !== undefined) {
            this.options.rememberEnforceTag = storedRemember === true;
        }
    },
    onlyAdmin: function () {
        return this.options.onlyAdmin;
    },
    setOnlyAdmin: function (b) {
        if (b !== this.options.onlyAdmin) {
            GM_setValue("FGPT.preference.onlyAdmin", b);
            this.options.onlyAdmin = b;
            return true;
        }
        return false;
    },
    ...
    setRememberEnforceTag: function (b) {
        if (b !== this.options.rememberEnforceTag) {
            GM_setValue("FGPT.preference.rememberEnforceTag", b);
...
function isGroupAdministratorOrModerator(groupname, groupId, userNsid) {
    var retval = GM_getValue("FGPT.groupAdminOrMod." + groupname + "." + userNsid);
    retval = retval == true || retval == 'true';
    var lastReadTime = GM_getValue("FGPT.groupAdminOrMod." + groupname + "." + userNsid + ".lastReadTime");
    //GM_log("admin: " + retval + " - time: " + lastReadTime);
    var asyncUpdate = true;
    if (retval === undefined || lastReadTime === undefined) {
        ...
        //GM_log("no need for checking, yet");
            return retval;
        }
    }
    var apiData = {
    ...
    new Request({
            ...
            onSuccess: function (responseText, responseXML) {
                ...
                GM_setValue("FGPT.groupAdminOrMod." + groupname + "." + userNsid, retval);
                GM_setValue("FGPT.groupAdminOrMod." + groupname + "." + userNsid + ".lastReadTime", 
...
function getGroupId() {
  try {
    var groupId;
    var groupBuddyIcon = $('Main').getElement('table td h1 a img.xBuddyIconH');
    if (!$chk(groupBuddyIcon)) { // sponsored group?
        groupBuddyIcon = $('Main').getElement('div#sg_buddy_icon a img.xBuddyIconH'); // never test!!
    }
    if ($chk(groupBuddyIcon)) {
        try {
            groupId = /.*flickr.com.*buddyicons\/(\d+@\w\d{2})\.jpg/.exec(groupBuddyIcon.get('src'))[1];
        } catch (e) {
            GM_log("unable to retrieve groupId");
        }
    }
    //GM_log("groupId: " + groupId);
    return groupId;
  } catch (e) {
    GM_log("error reading group id: " + e);
  }
}

var username = GM_getLoggedInUser();
var user;
var userNsid = unsafeWindow.global_nsid;
...
function updateEnforcingTagsNumbers(selectedBoxes, idx, outcome) {
    ...
    $('FGPT.success').set('html', selectedBoxes.filter(function(item) {
            return item == true;
        }).length
    );
    $('FGPT.fail').set('html', selectedBoxes.filter(function(item) {
            return item == false;
        }).length
    );
    $('FGPT.done').set('html', selectedBoxes.filter(function(item) {
...
function addTags(photoId, tags, selectedBoxes, idx) {
    ...
    GM_setValue("FGPT.version", FGPTtoolversion);
    ...
    checkVersion();
    var preferences = new FGPTPreferences();

    if (document.location.href.match(/.*flickr.com\/groups\/[^\/]*\/pool\/tags\//)) {
        var groupname = document.location.href.match(/.*flickr.com\/groups\/([^\/]*)\/pool\/tags\//)[1];
        var groupId = getGroupId();
        if (preferences.onlyAdmin()) {
            var isAdminOrMod = isGroupAdministratorOrModerator(groupname, groupId, userNsid);
            if (!isAdminOrMod) {
                GM_log("not an admin");
                return;
            }
        }
        var photoList = $$('div.GroupTagPics')[0];
        photoList.getElements('p.PoolList').each(function (photo) {
            photoId = photo.get('id').replace('pool_', '');
            photo.adopt(
                new Element('br'),
                new Element('small').adopt(
                            new Element('input', {
                                type: 'checkbox',
                                id: 'FGPT.checkbox.' + photoId,
                                class: 'FGPT-tag-enforcer-checkbox'
                            }),
                            new Element('label', {
                                html: 'enforce tag',
                                'for': 'FGPT.checkbox.' + photoId
                            })
                )
            );
        });

        GM_log("adding enforcer form to photoList");
        
        enforceDiv = new Element('div', {
            id: 'FGPT.enforce_tag_div'
        }).inject(photoList, 'before');
        ...
        enforceSmall.adopt(
                ...
                events: {
                    click: function (evt) {
                        if (this.checked == true || this.checked == "true") {
                            $$('input.FGPT-tag-enforcer-checkbox').each(function(checkbox) {
            ...
            new Element('button', {
                ...
                events: {
                    click: function (evt) {
                        var tags = $('FGPT.enforced_tag').value;
                        if (tags && tags.length > 0 && $('FGPT.remember').checked) {
                            GM_setValue(groupId + '.lastEnforcedTag', tags);
                        }
                        preferences.setRememberEnforceTag($('FGPT.remember').checked ? true : false);
                        var enforceDiv = $('FGPT.enforce_tag_div');
                        enforceDiv.getElement('small').empty();
                        var selectedBoxes =  new Array($$('input.FGPT-tag-enforcer-checkbox:checked').length);
                        ...
                        $$('input.FGPT-tag-enforcer-checkbox:checked').each(function(checkbox, idx) {
                            var photoId = checkbox.id.replace('FGPT.checkbox.', '');
                            new Element('img', {
                                src: 'http://l.yimg.com/www.flickr.com/images/pulser2.gif',
                                id: 'FGPT.spinner.' + photoId
                            }).replaces(checkbox);
            ...
            new Element('br'),
                    ...
                    new Element('input', {
                        ...
                        events: {
                            click: function (e) {
                                if ($('FGPT.remember').checked) {
                                    if (!$chk($('FGPT.enforced_tag').value)) {
                                        $('FGPT.enforced_tag').set('value', GM_getValue(groupId + '.lastEnforcedTag'));
                                    }
                                }
                            }
                        }
