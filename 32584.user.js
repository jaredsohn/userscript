...
var NEW_DIV_CSS_TEXT = "position: fixed; top: 10px; right: 10px; display: none; z-index:1000; "
        + "width:620px; font-size: 85%; background-color:lightgrey; padding:20px; overflow: auto; "
...
function processLink(_a) {
  ...
  return function(details) {
    if (details.responseText) {
      ...
      var postText = getTagMiddle(details.responseText, "<section class=\"userbody\">", "</section>");
      
      // check for tabbed images
      if (postText.indexOf("imgList") != -1) {
          var bodyText = getTagMiddle(details.responseText, "<section id=\"postingbody\">", "</section>");
          var imgText = getTagMiddle(details.responseText, "imgList = ", ";");
          postText = bodyText + "<br/>";
          var images = eval(imgText);
          for (var i = 0; i < images.length; i++) {
            postText += "<img height=\"175\" src=\"" + images[i] + "\"/>";
          }          
      } else {
...
function getTagMiddle(fullText, startText, endText) {
    var startIndex = fullText.indexOf(startText);
    var endIndex = fullText.indexOf(endText, startIndex);
    var middleText = fullText.substring(startIndex + startText.length, endIndex);
    return middleText;
}
