// TrExMa on Shortlist by DB5L (strongly based on Mike Cornell's work)
// for Mike's work please visit: http://twofourone.blogspot.com
...
var positionNames = ["D C", "D L", "D R", "DM C", "DM L", "DM R", "M C", "M L", "M R", "OM C", "OM L", "OM R", 
...
document.calculateSkillWeight  = function(positionWeightLevels, weights, index) {
  var weight = 0;
  weight = weights[positionWeightLevels[index]-1] / document.numberAtWeight(positionWeightLevels, 

positionWeightLevels[index]) * 10;
  return weight;
};
...
document.showPlayerSkills = function(playerRow) {
  ...
  if (tableData.length > 1) {
	...
	var dbsl = pomocny.length;
	var lom = pomocny.slice(pomocny.indexOf(" ")+1,dbsl);
	if (pomocny.indexOf("/") > 0) {
	  playersPositions[0]=pomocny.slice(0,pomocny.indexOf("/"))+pomocny.slice(dbsl-2,dbsl);
	  playersPositions[1]=pomocny.slice(pomocny.indexOf("/")+1,pomocny.indexOf(" 

"))+pomocny.slice(dbsl-2,dbsl);
	}
	else if (pomocny.indexOf(",")>0) {
	  playersPositions[0]=pomocny.slice(0,pomocny.indexOf(","));
	  playersPositions[1]=pomocny.slice(pomocny.indexOf(",")+2,dbsl);
	}
	else if (lom.length > 1) { 
	  playersPositions[0]=pomocny.slice(0,dbsl-1);
	  playersPositions[1]=pomocny.slice(0,pomocny.indexOf(" ")+1)+pomocny.slice(dbsl-1,dbsl);
	}
	else {
    ...
    for (var j = 0; j < playersPositions.length; j++) {
	  ...
	  if (positionIndex > -1) {
	    if (positionIndex == 99) {	  	  // GK
		  ...
		  positionSkill = document.calculateSkill(skills, positions[positionIndex], 
