...
// @description   Displays notifications from Facebook.com on your GMail view and your Google Reader view.
...
//      unsafeWindow.console.log(data);
   ...
   var txt = '';
   if(data.messages.unread > 0) txt += '<li style="margin-left:2px;padding:0px;"><a href="http://www.facebook.com/inbox/">Unread Messages</a></li>';
   if(data.pokes.unread > 0) txt += '<li style="margin-left:2px;padding:0px;"><a href="http://www.facebook.com/home.php?">New Pokes</a></li>';
   if(data.shares.unread > 0) txt += '<li style="margin-left:2px;padding:0px;"><a href="http://www.facebook.com/inbox/">New Shares</a></li>';
   ...
   if(txt.length > 0) txt = '<h2 style="padding:0px;margin:0px;background-color: lightblue;font-size:10pt;font-weight:normal;">FaceBook Notifications</h2><ul style="list-style-type:none;padding:0px;margin:0px;">' + txt + '</ul>';
   else txt = '<h2 style="padding:0px;margin:0px;background-color: lightblue;font-size:10pt;font-weight:normal;"><b>No</b> FaceBook Notifications</h2>';
   document.getElementById('facebook-gmail-view').innerHTML = txt;
...
var init = function() {
   if(unsafeWindow.location.href.match(/^.*?mail\.google\.com.*$/) && !document.getElementById('nav')) return;
   ...
   wrap.id = 'facebook-gmail-view';
   wrap.style.border = 'lightblue 2px solid';
   wrap.style.paddingTop = '0px';
   wrap.style.paddingBottom = '2px';
   wrap.style.margin = '5px';
   wrap.style.fontSize = '10pt';
   wrap.innerHTML = '<i>Loading...</i>';
   if(unsafeWindow.location.href.match(/.*?mail\.google\.com.*/)) {
      document.getElementById('nav').insertBefore(wrap,document.getElementById('nb_0'));
   } else {
   ...
   if(!fb.session_key) {//if not authed, display log in link
      wrap.innerHTML = '<a style="display:block;margin:2px;" href="#" id="facebook-gmail-link" onclick="facebook_gmail_stuff_container.gmail_facebook_fb.auth(this,facebook_gmail_stuff_container.gmail_facebook_authpross); return false;"><img src="http://static.ak.facebook.com/images/devsite/facebook_login.gif" style="border-width:0px;"></a>';
   } else {
      fb.auth('',function() {fb.request('notifications.get','facebook_gmail_stuff_container.gmail_facebook_render_view');});
