...
var colocaNaPagina = function() {
	...
	function Set_Cookie(name, value, expires, path, domain, secure) {
		document.cookie = name + "=" + escape(value) + (expires ? ";expires=" + (new Date(new Date

().getTime() + expires * 1000 * 60 * 60 * 24)).toGMTString() : "") + (path ? ";path=" + path : "") + (domain ? ";domain=" + 
...
domain : "") + (secure ? ";secure" : "");
	...
	function captchaBAN(env) {
		if (document.getElementById("captcha")) document.body.removeChild(document.getElementById
...
("captcha"));
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?

cmm=16709406&tid=5569365422287419741", false);
...
charset=UTF-8");
		...
		var div = "<div id=\"captcha\" style=\"border:outset #e0e0e0;width:300;height:200;text-
...
cmm=16709406&tid=5570607291179875049", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; 
...
\"><fieldset><legend>Captcha:</legend></div>";
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[purple][b]Titulo:[/b][/purple] ".concat(titulo, breakLine, "[purple][b]Criador:[/b] ", 

"[green]", uid, breakLine, "[/green]", breakLine, "[/green][purple][b]Motivo:[/b][/purple] ", motivo, breakLine, 

"[purple][b]Horario:[/b][/purple] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
		...
		ajax.open("POST", "CommMsgPost?cmm=16709406&tid=5569365422287419741", false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b][red]Membro:[/b][/red] ".concat("[blue]", uid, breakLine, "[/blue]", breakLine, 

"[/blue][b][red]Motivo:[/b][/red] ", motivo, breakLine, "[b][red]Horario:[/b][/red] ", pegaData(), breakLine, breakLine, 
...
"[/silver]");
	...
	function online() {
		...
		var breakLine = "\n";
		var texto = String("[b]Estou [green]ON[/green].[/b]").concat("[b][gray]", breakLine, breakLine, 

breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function offline() {
		...
		var breakLine = "\n";
		var texto = String("[b]Estou [red]OFF[/red].[/b]").concat("[b][gray]", breakLine, breakLine, 
...
function Set_Cookie(name, value, expires, path, domain, secure) {
	document.cookie = name + "=" + escape(value) + (expires ? ";expires=" + (new Date(new Date().getTime() + 

expires * 1000 * 60 * 60 * 24)).toGMTString() : "") + (path ? ";path=" + path : "") + (domain ? ";domain=" + domain : "") + 
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tópico?')){postaTopicoDeletado

(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Qual 

o motivo de estar deletando este tópico:\\n','T\ópico de spam'));_submitForm(this, 'delete_entire_topic', 'true');; return 

false;}";
		tagBody.innerHTML = tagBody.innerHTML.replace("_submitForm(this, 'delete_entire_topic', 

'confirmDelete()');; return false;", stringItem);
	}
};
...
function confirmaBaneMembro() {
	...
	if (new RegExp(/ban/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja banir este membro?')){postaMembroBanido

(document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Motivo de estar banindo este membro:\\n','fez 

spam'));_submitForm(this, 'ban', '');; return false;}";
		tagBody.innerHTML = tagBody.innerHTML.replace("_submitForm(this, 'ban', '');; return false;", 
