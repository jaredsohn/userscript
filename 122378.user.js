// ==UserScript==
// @name           eRepublik HUNGARIAN Translation Dev.
// @namespace      Hungarian Translation
// @author         HUNited80
// @description    eRepublik Magyar nyelven
// @version        1.2012.01.06
...
// @require        http://sizzlemctwizzle.com/updater.php?id=122378&days=3&uso
// ==/UserScript==
...
/* Day1508 eMagyarország @HUNited80 
 */
...
//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	// header
//	$("input#search_field").attr("value","Polgárok keresése");
	// common catalog
	$('#menu ul li#menu2 a').css('background-image','url(\"http://79.172.211.211/erep/map-erepublik-logged.new.png\")');
	$('#menu ul li#menu3 a').css('background-image','url(\"http://79.172.211.211/erep/map-erepublik-logged.new.png\")');
	$('#menu ul li#menu4 a').css('background-image','url(\"http://79.172.211.211/erep/map-erepublik-logged.new.png\")');
	$('#menu ul li#menu5 a').css('background-image','url(\"http://79.172.211.211/erep/map-erepublik-logged.new.png\")');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Marketplace')").text('Piac');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Monetary')").text('Valuta piac');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Job')").text('Munkaerő piac');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Companies for sale')").text('Eladó cégek');
	$("#menu > ul > li#menu5 > ul > li > a:contains('World Map')").text('Világtérkép');
	$("#menu > ul > li#menu5 > ul > li > a:contains('My Party')").text('Pártom');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Elections')").text('Választások');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Military Unit')").text('Katonai egységem');
//	$('#menu > ul > li#menu5 > ul > li:eq(5) > a').text('Országirányító központ');
//	$('#menu > ul > li#menu5 > ul > li:eq(6) > a').text('Top játékosok');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Invite friends')").text('Barátok meghívása');
	$("#menu > ul > li#menu5 > ul > li > a:contains('My Organizations')").text('Szervezeteim');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Badges')").text('Jelvények');
	$('#menu > ul > li#menu5 > ul').prepend('<li><a href=\"http://www.erepublik.com/en/news/latest/all\" target="_self">Újságok</a></li>');
	$('#menu > ul > li#menu5 > ul').append(
		'<li><a href=\"http://qchat.rizon.net/?channels=PTT_formosa\" target="_blank">eMagyarország Chat</a></li>');
	// sidebar
	...
	$('#experienceTooltip').html('<img src=\"http://www.erepublik.com/images/modules/sidebar/yellow_arrow_tip.png\" class=\"tip\" alt=\"\" />'+
		'Tapasztalati szint: <strong>LV. '+Explevel+'</strong><br />Tapasztalat pontok: <strong>'+Exppoint+'</strong><br />Következő szint: <strong>'+Nextlevel+'</strong> pontnál');
	$('#eatFoodTooltip > p:eq(0)').text('A gomb megnyomásával az egészséged növelheted a lehetséges mértékig a raktárodból.');
	$('#eatFoodTooltip > p:eq(1)').html('Újratölthetsz <big>'+healLimit+'</big> egészséget.');
	$('#eatFoodTooltip > p:eq(1) > big').attr('class','tooltip_health_limit');
	$('#eatFoodTooltip > small').replaceText('100 more recoverable Health in','Várakozás újabb 100 egészség feltöltéséhez:')
	$('#sidebar_missions > .content > b:first').text('Feladatok');
	$('strong#foodText').text('Evés');
	// others
	$('a.logout').text('Kilépés');
	$('a.prev').text('előző');
	$('a.next').text('következő');
	// footer
	$('#footer > p > a:eq(0)').text('Fórum');
	$('#footer > p > a:eq(1)').text('Tudásbázis (Wiki)');
	$('#footer > p > a:eq(2)').text('Blog');
	$('#footer > p > a:eq(3)').text('Újságíróknak');
	$('#footer > p > a:eq(4)').text('Támogatás és kapcsolat');
	$('#footer > p > a:eq(5)').text('Álláslehetőségek');
	$('#footer > p > a:eq(6)').text('ÁSZF');
	$('#footer > p > a:eq(7)').text('Adatvédelem');
	$('#footer > p > a:eq(8)').text('Partnerprogram');
	$('#footer > p > a:eq(9)').text('eRepublik törvényei');
	// remove kosovo ad
...
if (host==='www') {
	...
	if ((document.location.toString()=="http://www.erepublik.com/"+lang) || (document.location.toString().indexOf("?viewPost=")!==-1)) {
		// daily order
		$('#orderContainer > h1').text('Napi utasítás');
		$('#orderContainer > div > strong').text($('#orderContainer > div > strong').text().replace('Defeat enemies from','擊倒')+' 戰場上的敵人');
		$('#orderContainer > div > a.blue_beauty').text('Harcmező');
		$('#orderContainer > div > a.green_beauty').text('Jutalmazás');
		// daily task
		$('.column > .boxes.daily_tasks > h1').text('Napi feladat');
		$('.column > .boxes.daily_tasks > div > strong').text('Munka és edzés');
		$('.column > .boxes.daily_tasks > div > a.blue_beauty').text('Saját területen');
		$('.column > .boxes.daily_tasks > div > a.green_beauty').text('Jutalmazás');
		// daily task pop-box
		$('#daily_pop > h3').text('Napi feladat');
		$('#daily_pop > h2').text('Feladat elvégzése!');
		$('#daily_pop small:eq(0)').text('Erőnlét');
		$('#daily_pop small:eq(1)').text('Tapasztalat');
		$('#daily_pop > a').text('Meghatározott');
		// battle list
		$('#homepage_feed .column:eq(0) > h1.noborder').text('Harcok listája');
		$("#battle_listing > h4:exact('Campaign of the day')").text('A nap harca');
		$("#battle_listing > h4:contains('Allies')").text('Szövetségesek');
		$("#battle_listing > h4:contains('Campaigns')").text('Jelenleg '+$("#battle_listing > h4:contains('Campaigns')").replaceText('Campaigns','kampányok').text());
		$("#battle_listing span:exact('Fight')").text('Harc');
		$('#battle_listing .rest > a:eq(0)').text('További harcok');
		$('#battle_listing .rest > a:eq(1)').text('Hadügyi hírek');
		$('#battle_listing > ul.resistance_war').find('a#fundRW_btn > span').text('Támogatás');
		$('#battle_listing > ul.resistance_war').find('.info > small').text('Szabadságharc');
		// citizen feed
		$('#homepage_feed .column:eq(1) > h1:first').text('Üzenőfalam');
		$('#citizen_feed form#wall_post > textarea#shout').text('Írj az üzenőfalra...');
		$('#citizen_feed a.report').text('jelentés');
		$('#citizen_feed a.post_button > b').text('Elküldöm');
		$('#citizen_feed a.previous_comments').text('Régebbi üzenetek elolvasása');
		$("#citizen_feed a[trigger='reply']").text('Hozzászólások');
		$("#citizen_feed a[trigger='post_like']").text('Tetszik');
		$("#citizen_feed a[trigger='post_unlike']").text('Nem tetszik');
//		$("#citizen_feed a[trigger='comment_like']").text('Tetszik');
//		$("#citizen_feed a[trigger='comment_unlike']").text('Nem tetszik');
		$("#citizen_feed span[trigger='add_comment']").text('Hozzászólok');
		$('#citizen_feed textarea.comment_reply_post').text('Üzenősáv');
		$('#citizen_feed div.fake_input').text('Üzenősáv');
		$('#citizen_feed').find("div.vote_bar em:contains('voted this.')").each( function() {
			$(this).find('b.personal').text('Te, ');
			$(this).replaceText('and','és').replaceText('voted this.','szavaztál rá.');});
		$('#citizen_feed > .previous_posts > a > span').text('Régebbi üzenetek');
		// news list
		$('#news.box > .title > h1:first').text('Hír kategóriák');
		$('#articles > div > a.mbutton:eq(0) > span').text('Első lépések az eRepublikon');
		$('#articles > div > a.mbutton:eq(1) > span').text('Hadügyi utasítások');
		$('#articles > div > a.mbutton:eq(2) > span').text('Hadügyi elemzések');
		$('#articles > div > a.mbutton:eq(3) > span').text('Politikai elemzések, cikkek');
		$('#articles > div > a.mbutton:eq(4) > span').text('Üzleti élet és pénzügyek');
		$('#articles > div > a.mbutton:eq(5) > span').text('Társadalom és szórakozás');
		$('#articles > div > a.mbutton:eq(6) > span').text('Előfizetett újságok');
	};
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	// www.erepublik.com/en/main/citizen
	// www.erepublik.com/en/economy/donate-items/
	// www.erepublik.com/en/economy/donate-money/
	// www.erepublik.com/en/economy/citizen-accounts/
	// www.erepublik.com/en/main/citizen-friends/
	// www.erepublik.com/en/citizen/change-residence
	// www.erepublik.com/en/citizen/change-password
	// www.erepublik.com/en/citizen/edit/profile
	// www.erepublik.com/en/citizenship
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	if ((document.location.toString().indexOf("/citizen")!==-1) || (document.location.toString().indexOf("/economy/donate-")!==-1)) {
		// layout menu
		$('.citizen_profile_header.auth > .citizen_edit > a > span').html('<img src="http://www.erepublik.com/images/modules/citizenprofile/edit.png">Profil szerkesztése');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(0) > a').text('Áttekintés');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(1) > a').text('Pénzügyi adatok');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(2) > a').text('Raktárkészlet');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Dead citizen')").replaceText('Dead citizen','Halott polgár');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Temporary banned')").replaceText('Temporary banned','Átmenetileg kitiltva');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Permanent banned')").replaceText('Permanent banned','Véglegesen kitiltva');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_friend.tip').attr('title','Barátnak jelölöm');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_friend_remove.tip').attr('title','Már nem a barátom');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_message.tip').attr('title','Üzenek neki');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_donate.tip').attr('title','Adományozok neki');
		$('.citizen_profile_header.auth > .citizen_status > div > small').text('Tapasztalati pontok');
		// profile sidebar
		$('.citizen_sidebar > div > small:eq(0)').html('Jelenlegi lakóhely <a href="http://www.erepublik.com/en/citizen/change-residence" title="">(Költözés)</a>');
		$('.citizen_sidebar > div > small:eq(1)').html('Állampolgárság <a href="http://www.erepublik.com/en/citizenship" title="">(Módosítás)</a>');
		$('.citizen_sidebar > div > .citizen_second > small:eq(0)').text('eRepublik születésnap');
		$('.citizen_sidebar > div > .citizen_second > small:eq(1) > a').replaceText('National rank','Nemzeti rang');
		$('.citizen_sidebar > .citizen_activity > div.place:eq(0) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_political_activity.png">Nem politizál');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Party Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/party_member.png">Párttagság');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Party President')").html('<img src="http://www.erepublik.com/images/modules/_icons/party_president.png">Pártelnök');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Congress Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/congress_member.png">Kongresszusi tag');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Country President')").html('<img src="http://www.erepublik.com/images/modules/_icons/country_president.png">Az ország elnöke');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Far-left Wing')").text('Szélsőbal párt');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center-left Wing')").text('Balközép párt');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center Wing')").text('Közép párt');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center-right Wing')").text('Jobbközép párt');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Far-right Wing')").text('Szélsőjobb párt');
		$('.citizen_sidebar > .citizen_activity > div.place:eq(1) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_mu.png">Nem csatlakozott hadsereghez');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Recruit')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">Légiós újonc');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">Hadügyi tagság');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Captain')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">Kapitány');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('2nd Commander')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">Parancsnok helyettes');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Commander')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">Parancsnok');
		if ($('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity > a').length==1) {
			$('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_media_activity.png"><a href="http://www.erepublik.com/en/create-newspaper">Újság létrehozása</a>');
		} else {
			$('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_media_activity.png">Nincs újságja');};
		$(".citizen_sidebar > .citizen_activity > div.place:eq(2) > h3:contains('Press director')").html('<img src="http://www.erepublik.com/images/modules/_icons/press_director.png">Újság kiadó');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(2) > h3:contains('Create newspaper')").html('<img src="http://www.erepublik.com/images/modules/_icons/press_director.png"><a href="/en/create-newspaper">Újság indítása</a>');
		$('.citizen_sidebar > .citizen_activity > h4.friends_title').replaceText('Friends','Barátok ');
		$('.citizen_sidebar > .citizen_activity > a.view_friends').text('Összes barát megtekintése');
		// profile content
		var cizName = $(".citizen_profile_header > h2").text();
		$('.citizen_content > .my_land_profile > p > strong').text(cizName+' földje:');
		$('.citizen_content > .my_land_profile > p').replaceText('buildings','épülete van');
		$('.citizen_content > .my_land_profile > a > span').text("Megnézem");
		$('.citizen_content > h3.mbot2:eq(0)').html('<br><br><br><br><br>Bemutatkozás:');
		$('.citizen_content > h3.mbot2:eq(1)').html('<br><br><br><br><br>Elért medálok:');
		// achievment
		$("ul#achievment > li:eq(0) > .hinter > span > p:eq(0) > strong").text("A munka hőse");
		$("ul#achievment > li:eq(0) > .hinter > span > p:eq(1)").text("Folyamatos munka 30 napon át");
		$("ul#achievment > li:eq(1) > .hinter > span > p:eq(0) > strong").text("Parlamenti képviselő");
		$("ul#achievment > li:eq(1) > .hinter > span > p:eq(1)").text("Nyert a parlamenti választásokon");
		$("ul#achievment > li:eq(2) > .hinter > span > p:eq(0) > strong").text("Államelnök");
		$("ul#achievment > li:eq(2) > .hinter > span > p:eq(1)").text("Megnyerte a választásokat");
		$("ul#achievment > li:eq(3) > .hinter > span > p:eq(0) > strong").text("Média fejedelem");
		$("ul#achievment > li:eq(3) > .hinter > span > p:eq(1)").text("Több mint 1000 előfizetője van az újságjának");
		$("ul#achievment > li:eq(4) > .hinter > span > p:eq(0) > strong").text("A csata hőse");
		$("ul#achievment > li:eq(4) > .hinter > span > p:eq(1)").text("A legtöbb sebzést szerezte a csatában");
		$("ul#achievment > li:eq(5) > .hinter > span > p:eq(0) > strong").text("A háború hőse");
		$("ul#achievment > li:eq(5) > .hinter > span > p:eq(1)").text("A legtöbb sebzést szerezte a háborúban");
		$("ul#achievment > li:eq(6) > .hinter > span > p:eq(0) > strong").text("Szabadságharcos hős");
		$("ul#achievment > li:eq(6) > .hinter > span > p:eq(1)").text("Szabadságharcot indított, és felszabadította a régiót");
		$("ul#achievment > li:eq(7) > .hinter > span > p:eq(0) > strong").text("Szuper katona");
		$("ul#achievment > li:eq(7) > .hinter > span > p:eq(1)").text("250 erőt gyűjtött össze");
		$("ul#achievment > li:eq(8) > .hinter > span > p:eq(0) > strong").text("A társadalom építője");
		$("ul#achievment > li:eq(8) > .hinter > span > p:eq(1)").text("Beszervezett és eljuttatott a 10. szintig 10 új eRepublik játékost");
		$("ul#achievment > li:eq(9) > .hinter > span > p:eq(0) > strong").text("Zsoldos");
		$("ul#achievment > li:eq(9) > .hinter > span > p:eq(1)").text("50 különböző országban legyőzött 25 ellenséget");
		// military skill
		$(".citizen_content > h3:contains('Military Skills')").html('<br><br><br><br><br>Katonai képességek');
		$('.citizen_content > .citizen_military:eq(0) > strong:first').text('Erő');
		$('.citizen_content > .citizen_military:eq(0) > div > small:first').replaceText('Super soldier:','Szuper katona:');
		$('.citizen_content > .citizen_military:eq(1) > strong:first').text('Rangjelzés');
		$('.citizen_content > .citizen_military:eq(1) > div > small:first').text('Ranglétra előrehaladás:');
		if (document.location.toString().indexOf("/economy/")!==-1) {
			if (document.location.toString().indexOf("/donate-")!==-1) {
				// donate tab
				$('.citizen_content > div > h2.special.padded').replaceText('Donate','Adományozás');
...
//			} else if (document.location.toString().indexOf("/citizen-accounts/")!==-1) {
	...
	if (document.location.toString().indexOf("/wars/")!==-1) {
		...
		if (document.location.toString().indexOf("/wars/show/")!==-1) {
			...
			$('.war_list_header > .country.left_side > div > h3 > #CHT');
...
//			$('#pvp_actions > .action_holder > a#heal_btn').attr('title','按此吃食物');
			...
			$("#battle_listing > p.campaigns_header.allies.toggled:contains('Allies')").html('<span class="arrow"></span>目前 同盟國 的戰役');
			$("#battle_listing > p.campaigns_header.allbattles.toggled:contains('All')").html('<span class="arrow"></span>目前 全部國家 的戰役');
	...
	if (document.location.toString().indexOf("/news/")!==-1) {
		...
		// create newspaper
		$('.holder > h1:first').text('創立報紙');
		$('.holder > a.wikiicon.largepadded').text('更多說明');
		$('.accountinfo.largepadded > .accountdisplay > span.rightpadded:first').text('你的現金帳戶');
		$(".holder > h2:first:contains('Requirements')").text('報社需求');
		$(".holder > .smallhoderleft.goleft > p:contains('Level')").text('等級');
		$(".holder > .smallhoderleft.goleft > p:contains('Cost')").text('花費');
		$('.holder > .smallholderright.goleft > a.btnGetGold').text('購買黃金');
		$(".holder > h2.section:contains('Newspaper details')").text('報社詳情');
		$('form.companydetails > p.largepadded:eq(0) > span.fieldname').text('報社名稱');
		$('form.companydetails > p.largepadded:eq(0) > span#error_for_newspaper_name_twin').text('只允許6-25個字元.');
		$('form.companydetails > p.largepadded:eq(1) > span.fieldname').text('報社頭像');
		$('form.companydetails > p.largepadded:eq(1) > span.goleft > span#error_for_newspaper_image_twin').text('只允許上傳 JPG 圖檔.');
...
//		$(".bordersep > h2:exact('Country')").text("請選擇一個國家");
	...
	if (document.location.toString().indexOf("/group-")!==-1) {
		if (document.location.toString().indexOf("/group-home/")!==-1) {
			...
			$("#group_right_column > .leader > h3:last:exact('Captain')").text('軍團小隊長');
			$("#group_right_column > .leader > h3:last:exact('No Captain')").text('沒有軍團小隊長');
			$('#group_right_column > .members > h3:first').replaceText('Members','軍團成員');
			$('#group_right_column > .members > a.show_all').text('查看全部');
			$('#orderContainer > h3').replaceText('Order of the day','每日軍團任務');
			$('#orderContainer > .requirements.fight > ul.req_content > li.condition0 > .details > strong').text($('#orderContainer > .requirements.fight > ul.req_content > li.condition0 > .details > strong').text().replace('Defeat enemies from','擊倒')+' 戰場上的敵人');
...
//			$("#group_feeds > #citizen_feed a[target='reportpopup']").text('檢舉');
	...
	if ((document.location.toString().indexOf("/country")!==-1) || (document.location.toString().indexOf("/law/")!==-1)) {
		...
		if (document.location.toString().indexOf("/country/economy/")!==-1) {
			...
			$("span.fakeheight:contains('Moving Tickets')").text("機票");
			$("span.fakeheight:contains('Food Raw Material')").text("食物原料");
			$("span.fakeheight:contains('Weapon Raw Material')").text("武器原料");
			$("span.fakeheight:contains('Hospital')").text("醫院");
			$("span.fakeheight:contains('Defense System')").text("防禦設施");
...
//			$(".holder:eq(1) > .indent > p.largepadded:contains('Do you agree to transfer')").replaceText('Do you agree to transfer','請問你是否同意從國庫中捐贈').replaceText('from the country accounts to','到');
	...
	if (document.location.toString().indexOf("/tickets/report/")!==-1) {
		...
		$("form.reportAbuse > select#report_reason > option[value='16']").text('製造糾紛');
...
//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
if ((host==='economy') && (document.location.toString().indexOf("/citizen/profile/")!==-1)) {
