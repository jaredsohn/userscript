...
// @required       http://ajax.googleapis.com/ajax/libs/jquery/1.6.2/jquery.min.js
// ==/UserScript==
...
//21/06/2555 Version 3.22
//14/08/2555 Version 4.11
...
function CreateCSS() {
    ...
    StyleStr = '.Container { z-index:10000; display:block; width:100%; height:25px; background-color:Black; font-family:Tahoma; font-size: 11px; position:fixed; bottom:0px; left:0; color:White; background: -moz-linear-gradient(top, #444444, #000000); background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#444444), to(#000000));} .Container a { text-decoration:none; } .Container hr { display:block; } .Credit { float: right; margin: 5px 5px 0px 0px; } .MainNav { float: left; margin:0 0 0 5px; } .MainNav ul { float: left; margin: 0; list-style: none; } .MainNav .NavList { float:left; border-left: 1px solid black; border-right: 1px solid black; } .MainNav li a { display: block; line-height: 24px; overflow: hidden; float: left; color:White; padding: 0px 5px 0px 5px; } .NavList span:hover .aNavList { background: -moz-linear-gradient(top, #000000, #444444); background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#000000), to(#444444)); color:Gold; } .MainNav a .menu-mid { height: 23px; line-height: 21px; display: block; float: left; } .MainNav li .subList { display: none; position: fixed; bottom:25px; padding-top: 3px; border: 1px solid Black; -moz-border-radius: 7px 7px 0px 0px; -webkit-border-radius: 7px 7px 0px 0px; border-radius: 7px 7px 0px 0px;} .subList { box-shadow: 0px 0px 3pt 2pt black; z-index:10000; background: -moz-linear-gradient(top, #444444, #000000); background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#444444), to(#000000)); border: 1px solid white; -moz-border-radius: 7px 7px 0px 0px; -webkit-border-radius: 7px 7px 0px 0px; border-radius: 7px 7px 0px 0px; } .MainNav li:hover .subList { display:block; } .MainNav li ul { width: auto; height: auto; margin: 0; padding: 0px 0px 0px 5px; list-style: none; } .MainNav li ul li { width: auto; display:block; padding: 0px 5px 0px 0px; } .MainNav li .subList li a { display:block; width:100%; padding: 0px 0px 0px 0px; } .MainNav li .subList li a:hover { background: -moz-linear-gradient(top, #999999, #000000); background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#999999), to(#000000)); -moz-border-radius: 7px 7px 0px 0px; -webkit-border-radius: 7px 7px 0px 0px; border-radius: 7px 7px 7px 7px; } .MainNav li .subList li a:active { background: -moz-linear-gradient(top, #000000, #444444); background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#000000), to(#444444)); -moz-border-radius: 7px 7px 0px 0px; -webkit-border-radius: 7px 7px 0px 0px; border-radius: 7px 7px 7px 7px; } .UnitImg { width: 25px; height: 25px; float: left; } .divText { float: left; margin: 5px 5px 0px 5px; } .divLabel { float: left; text-align: left; width: 100px; padding: 0px 0px 0px 10px; } .divNumber { float: left; margin: 5px 5px 0px 0px; text-align: right; width: 75px; } .floatLeft { float: left; } .hrLine { display:block; margin: 1px 0px 1px 0px; } .pHead { text-align:center; font-weight:bold; }';
...
function CreateMainDiv() {
    ...
    Credit.innerHTML = ' kknTool v4.11 | Develop By kkn ( XIV ) ';
...
function SetupCalPowerMenu() {
    ...
    CalPowerSubDiv.id = 'CalPowerSubDiv';
    CalPowerSubDiv.className = 'subList';
    CalPowerSpan.className = 'menu-mid';
    CalPowerSpan.innerHTML = ' คำนวนพลังโจมตี '
    CalPowerA.href = '#';
    CalPowerA.className = "aNavList";
    CalPowerSubDiv.appendChild(CalPowerUL);
    CalPowerA.appendChild(CalPowerSpan);
    span.appendChild(CalPowerA);
    span.appendChild(CalPowerSubDiv);
    CalPowerLi.appendChild(span);

    //****************************************

    var liHead = document.createElement('li');
    liHead.innerHTML = '<p class="pHead">คำนวนพลังโจมตี</p><hr class="hrLine">';

    CalPowerUL.appendChild(liHead);
    ...
    liUnitQty.innerHTML = '<hr class="hrLine">จำนวนทหาร';
    ...
    liTotalPower.innerHTML = '<hr class="hrLine">พลังโจมตีรวม';
    ...
    var liTotalPower = document.createElement('li');
    liTotalPower.innerHTML = '<hr class="hrLine"><a href="http://sososoru.nusutto.jp/tekihei.html">Credit : sososoru.nusutto.jp</a>';
...
function SetupAreaPowerMenu() {
    if (location.pathname == '/map.php') {
        ...
        WarPowerA.href = '#';
        WarPowerA.className = "aNavList";
        WarPowerSubDiv.appendChild(WarPowerUL);
        WarPowerA.appendChild(WarPowerSpan);
        span.appendChild(WarPowerA);
        span.appendChild(WarPowerSubDiv);
        WarPowerLi.appendChild(span);

        //****************************************

        var liHead = document.createElement('li');
        liHead.innerHTML = '<p class="pHead">ระดับอาณาเขต</p><hr class="hrLine">';

        WarPowerUL.appendChild(liHead);
...
function getQueryString(InputStr, key) {
...
/*$.get('http://w1.sangokushi.in.th/alliance/alliance_log.php', function (data) {
    $('#result').hide();
    //var input_form = $(data).find('#input_form');
    //$('#result').html(input_form);
    $('#result').html(data);
    //alert('Load was performed.');
    //$('#result').fadeIn();
    $('#showResult').show();
    $('#showResult').html($('#result').find('#gray02Wrapper'));
    $('#showResult').fadeIn();
    //$('#result').find('#gray02Wrapper').fadeIn();
    $('#result').html('');
});*/

/*$.ajax({
    url: 'http://w1.sangokushi.in.th/alliance/alliance_log.php',
...
function BuildingStatus() {
    if (location.pathname == '/village.php') {
        var IconClass = new Array();

        var mapiconno = 1;
        for (x = 0; x <= 6; x++) {
            for (y = 0; y <= 6; y++) {
                IconClass[mapiconno - 1] = [x + "" + y, "mapicon" + padZero(mapiconno)];
                mapiconno += 1;
            }
        }

        var Building = $('#actionLog ul *:contains("กำลังก่อสร้าง") a').attr('href')
        var BuildingXY = '';
        if (Building != undefined) {
            BuildingXY = getQueryString(Building, 'x') + getQueryString(Building, 'y')
        }

        var Queuing = $('#actionLog ul *:contains("กำลังเตรียมการก่อสร้าง") a').attr('href')
        var QueuingXY = ''
        if (Queuing != undefined) {
            QueuingXY = getQueryString(Queuing, 'x') + getQueryString(Queuing, 'y')
        }

        var Deleting = $('#actionLog ul *:contains("กำลังรื้อถอน") a').attr('href')
        var DeletingXY = ''
        if (Deleting != undefined) {
            DeletingXY = getQueryString(Deleting, 'x') + getQueryString(Deleting, 'y')
        }

        for (i = 0; i < 49; i++) {
            switch (true) {
                case (BuildingXY == IconClass[i][0]):
                    var css = {
                        'border': 'solid 3px lime',
                        'box-shadow': '0px 0px 3pt 2pt black',
                        'background-color': 'lime',
                        '-moz-border-radius': '5px 5px 5px 5px',
                        '-webkit-border-radius': '5px 5px 5px 5px',
                        'opacity': '1.7'
                    }
                    $('.' + IconClass[i][1]).css(css);
                    $('<img src="http://findicons.com/files//icons/1700/2d/512/arrow_up.png" class=' + IconClass[i][1] + ' style="padding:10px 0px 0px 14px; opacity:1.7;" />').insertAfter('.' + IconClass[i][1]);
                    break;
                case (QueuingXY == IconClass[i][0]):
                    var css = {
                    ...
                    $('.' + IconClass[i][1]).css(css);
                    $('<img src="http://upload.wikimedia.org/wikipedia/commons/b/b2/Red_clock.png" class=' + IconClass[i][1] + ' style="padding:10px 0px 0px 14px; opacity:1.7;" />').insertAfter('.' + IconClass[i][1]);
                    break;
                case (DeletingXY == IconClass[i][0]):
                    var css = {
                    ...
                    $('.' + IconClass[i][1]).css(css);
                    $('<img src="http://images1.wikia.nocookie.net/__cb57088/althistory/images/e/eb/Under_contruction_icon-red.svg" class=' + IconClass[i][1] + ' style="padding:10px 0px 0px 14px; opacity:1.7;" />').insertAfter('.' + IconClass[i][1]);
                    break;
            }
        }
    }
