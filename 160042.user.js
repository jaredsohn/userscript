// ==UserScript==
// @name        clickersptc
// @namespace   clickersptc
// @include     *://*clickersptc.com/index.php?view=account&ac=click*
// @include     *://*clickersptc.com/gpt.php?v=entry*
...
if(wparent.location.href.indexOf('index.php?view=account&ac=click') != -1 && top == self){
	...
	$.each($('.ptcList tr:not(:first) a'),function(k,v){
		objj[k];
		if($(v).attr('href')){
			if($(v).attr('href').indexOf('gpt.php?v=entry') >= 0 && $(v).text().indexOf('Cheat Check') == -1){
				var obj = {
					href 	: $(v).attr('href'),
					jObj	: $(v)
				}
