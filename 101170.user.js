// ==UserScript==
// @name           Powertools Kings Of Damned
// @namespace      noobs
...
var Version = '1.2';20110321
var Title = 'Kings Of Damned Power Tools';
...
{ document.getElementById("mod_comm_list2").style.backgroundColor = '#F0E68C';}
if(document.getElementById("mod_comm_list1"))
{ document.getElementById("mod_comm_list1").style.backgroundColor = '#ADD8E6';}
...
var Options = {
  ...
  alertConfig  : {aChat:false, aPrefix:'** Je suis attaqu&eacute; **', scouting:false, wilds:false, minTroops:1, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    .ptChatWhisper {}\
    .ptChatAlliance {background-color: transparent}\
    .ptChatGlobal {background-color: transparent}\
    .ptChatIcon {border: 3px inset green}\
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1));
  ...
  AddMainTabLink('Kings', eventHideShow, mouseMainTab);
}
...
Tabs.kings = {
  ...
  myDiv : null,
  timer : null,
  forumlink :  'http://kingsofdamned.actifforum.com',

  init : function (div){    // called once, upon script startup
    var t = Tabs.kings;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="http://kingsofdamned.actifforum.com" width="100%" height=550 name="board_in_a_box" <p>Votre navigateur ne supporte pas les cadres, vous pouvez appeler la page incorpor&eacute;e par ce lien: <a href="http://kingsofdamned.actifforum.com">Forum kings of damned</a></p> </iframe> <BR></center>';
  },
...
/****************************  Tab Gems   *****************************
Tabs.Gems = {
  tabOrder : 36,                    // order to place tab in top bar
  myDiv : null,
  timer : null,
  gemslink :  'http://apps.facebook.com/kingdomsofcamelot/?page=claimgem',

  init : function (div1){    // called once, upon script startup
    var t = Tabs.gems;
    t.myDiv = div1;
    div1.innerHTML = '<iframe src="'+ t.gemslink +'" width="100%" height="550" name="board_in_a_box" <p>Votre navigateur ne supporte pas les cadres, vous pouvez appeler la page incorpor&eacute;e par ce lien: <a href="'+ t.gemslink +'">Je veux des Gems !!!</a></p> </iframe>';
  },
...
//logit ('displayReports: '+ Options.allowAlterAR);
        if (Options.allowAlterAR)
          msg.push("<div id='modal_alliance_reports_tablediv' class='modal_msg_list'><table width=800 cellpadding='0' cellspacing='0' class='msgviewtable reportviewtable alliancetable'>");
        else
          msg.push("<div id='modal_alliance_reports_tabledivNKA' class='modal_msg_list'><table width=800 cellpadding='0' cellspacing='0' class='msgviewtable reportviewtable alliancetable'>");
...
Tabs.AllianceList = {
  ...
  unsafeWindow.PCalClickPrev = t.eventListPrev;
    ...
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Entrer une partie ou le nom entier du joueur: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Rechercher" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab>OR: </td><TD class=xtab>Entrer une partie ou le nom entier de l\'alliance: &nbsp;</td>\
          <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Rechercher" /></td>\
  ...
  displayMembers : function (allName, numPlayers){
    ...
    <TD class=clickable><A><DIV>Last Login</a></div></td></tr></thead>\
      ...
      <DIV style="top:670px; left:0px; position:static; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Les donn&eacute;es viennent du Tableau de Bord et peuvent avoir plus de 24 heures!</td>\
          <TD class=xtab align=right>Cliquer sur les titres des colonnes pour classer</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
  ...
  var citySelect = '   <SELECT id=fakeCity>';
    ...
    <TR><TD align=right># of Scouts: </td><TD><INPUT type=text size=6 value=0 id=faketroop2></td></tr>\
      ...
      <TR><TD align=right>Target city: </td><TD>'+citySelect+'</td></tr>\
        ...
        <BR><DIV id=testDiv style="background-color:#fffff0; maxwidth:750; max-height:430px; height:430px; overflow-y:auto;"></div>';
    t.cont.innerHTML = m;
    ...
    try {
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Show \'food left\' in RED if food will run out in less than \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> hours; THIS OPTION IS ALSO USED FOR ALLIANCE FOOD ALERT!</td></tr>\
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    if (t.searchRunning){
      t.stopSearch ('Recherche annul&eacute;e!');
      return;
    }
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      ...
      <SELECT id=ptttType>\
        <option value='1'>Ravitailleur</option>\
        ...
        <option value='10'>Balliste</option>\
      ...
      <SELECT id=pttdType>\
        <option value='53'>Arbaletes</option>\
        <option value='55'>Trebuchet</option>\
        <option value='60'>Pi&egrave;ge</option>\
        <option value='61'>Chausse Trappe</option>\
        <option value='62'>Palissade à pointe</option>\
...
if (t.limitingFactor){
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (t.running){
      t.stopTraining('<SPAN class=boldRed>Formation annul&eacute;e par l\'utilisateur</span>');
      return;
    }
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Nombre de troupes par slot doit etre sup&eacute;crieur &agrave; 0.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Impossible d\'entrainer cette quantit&eacute; de troupes (max is '+ t.stats.MaxTrain +' total)</font>';
      return;
    }
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Nombre de slots incorrects.</font>';
      return;
    }

    t.TDbutDo.disabled = true;
    t.TTbutDo.className = 'ptButCancel';
    t.TTbutDo.value = 'ANNULER';
...
//end of added21mar
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><SPAN id=ptttr_food><B>Pommes:</b><BR>'+ addCommasInt(t.stats.food) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_wood><B>Bois:</b><BR>'+ addCommasInt(t.stats.wood) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_ore><B>Minerais:</b><BR>'+ addCommasInt(t.stats.ore) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_gold><B>Or:</b><BR>'+ addCommasInt(t.stats.gold) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_pop><B>Pop dispo.:</b><BR>'+ addCommasInt(t.stats.idlePop) +'</span></td></tr></table><BR>';
...
// defense queue ....
  ...
  stopTraining : function (msg){
    ...
    t.TTbutDo.value = 'Former !';
    t.TDbutDo.value = 'Construire !';
    t.TTbutDo.className = '';
    t.TDbutDo.className = '';
    t.running = false;
  },
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.stopTraining ('<B>Formations lanc&eacute;es.</b>');
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Formation  '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +' at '+ Cities.byID[cityId].name +' (reste '+ que.length +' slots )<BR>');
    ...
    function _row (name, row, noTotal){
      ...
      if (noTotal){
        ...
        for (i=0; i<row.length; i++)
          tot += parseInt(row[i]);
        m.push ('<TD style="background: #CCFFCC">');
...
//DebugTimer.start();
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;" width=1000><TABLE cellspacing=0 cellpadding=0 class=ptTab width=100% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Joue depuis:</span> '+ dt.toLocaleDateString() +'</td>\
    ...
    if(Gate == 0)
      ...
      str += _row ('Or', rows[0]);
      str += _row ('Pommes', rows[1]);
      str += _row ('Bois', rows[2]);
      str += _row ('Pierre', rows[3]);
      str += _row ('Minerais', rows[4]);
      str += '<TR><TD><BR></td></tr>';
      for (r=1; r<13; r++){
        rows[r] = [];
        for(i=0; i<Cities.numCities; i++) {
...
//**********************************
    ...
    for (r=2; r<26; r+=2){
      ...
      str += _row ('Ravitailleur', rows[1]);
      str += _row ('Milicien', rows[2]);
      str += _row ('Eclaireur', rows[3]);
      str += _row ('Piquier', rows[4]);
      str += _row ('Paladin', rows[5]);
      str += _row ('Archer', rows[6]);
      str += _row ('Cavalier', rows[7]);
      str += _row ('Cavalier Lourd', rows[8]);
      str += _row ('Wagon', rows[9]);
      str += _row ('Balliste', rows[10]);
      str += _row ('Belier', rows[11]);
      str += _row ('Catapulte', rows[12]);
      str += _row ('<i><span style="color:Maroon">Ravitailleur F.</span></i>', t_rows[2]);
      str += _row ('<i><span style="color:Maroon">Milicien F.</span></i>', t_rows[4]);
      str += _row ('<i><span style="color:Maroon">Scout F.</span></i>', t_rows[6]);
      str += _row ('<i><span style="color:Maroon">Piquier F.</span></i>', t_rows[8]);
      str += _row ('<i><span style="color:Maroon">Paladin F.</span></i>', t_rows[10]);
      str += _row ('<i><span style="color:Maroon">Archer F.</span></i>', t_rows[12]);
      str += _row ('<i><span style="color:Maroon">Cavalier F.</span></i>', t_rows[14]);
      str += _row ('<i><span style="color:Maroon">Cavalier L. F.</span></i>', t_rows[16]);
      str += _row ('<i><span style="color:Maroon">Wagon F.</span></i>', t_rows[18]);
      str += _row ('<i><span style="color:Maroon">Baliste F.</span></i>', t_rows[20]);
      str += _row ('<i><span style="color:Maroon">Belier F.</span></i>', t_rows[22]);
      str += _row ('<i><span style="color:Maroon">Catapulte F.</span></i>', t_rows[24]);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Nouriture Restante', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('#Terres Sauvages', row, true);
      ...
      str += _row ('NB Chevalier', row, true);
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>inclure les troops en d&eacute;lacement </td></tr>';
...
Tabs.Marches = {
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['Ravit', 'Mil', 'Eclair', 'Piquier', 'Paladin', 'Archer', 'Cav', 'Cav L.', 'Wagon', 'Balli', 'Belier', 'Cat'];
    var t = Tabs.Marches;
    clearTimeout (t.displayTimer);
...
//logit ("enc: "+ inspect (enc, 6, 1));
    ...
    s = '<div class=ptstat>Liste troupes pr�sentes dans chacune de vos ambassades.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Pas de troupe</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Joueur (Chevalier)</b></td>';
      ...
      for (c in Cities.cities){
        ...
        if (enc[dest]){
          ...
          s+= '<TR><TD class="Ville" colspan=13 align=center><B>'+ Cities.cities[c].name +'</b></td></tr>';
          for (p in enc[dest]){
            try {
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
  ...
  pop.getMainDiv().innerHTML = '<TABLE class=ptTab align=center style="height: 100%"><TR align=center height=90%><TD>'+ msg +'</td></tr>\
      <TR align=center><TD><INPUT id=ptcccancel type=submit value="ANNULER" \> &nbsp; &nbsp; <INPUT id=ptcccontin type=submit value="CONTINUE" \></td></tr></table>';
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>KOC Power Tools</center>';
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Une erreur est apparue :</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Nouvel essai automatique dans <SPAN id=paretrySeconds></b></span> secondes ...<BR><BR><INPUT id=paretryCancel type=submit value="ANNULER" \>';
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  this.div.style.width = '1000px'; //essai chris avant =>width + 'px'
