// ==UserScript==
// @name           Favorites Spy Reports PT 1.0 by trijin (tradução por HeD_pE)
// @namespace      ogc20-modules/
// @description    Guarda o link e cabeçalho dos relatórios de espionagem (Somente para o GreaseMonkey)
// @version        1.0beta
...
var Favors = {
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Este relatório já está nos favoritos');
				return false;
			}
	...
	Fsave: function(obj) {
		try {
			...
			unsafeWindow.fadeBox('Alterações efectuadas');
		} catch (e) {}
	},
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">Remetente</th><th class="subject">Assunto</th><th class="date">Data</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Comando da frota</td><td class="subject">' +
				...
				'<a title="|Remover dos favoritos" class="ogc20Fdel tips" rel="'+el.id+
			...
			unsafeWindow.fadeBox('Nenhum relatório guardado');
		}
		return false;
	},
