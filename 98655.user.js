...
// @name           VillageRenamer
// @namespace      @DaHaiz
// @description    (v 1.2.5) Setzt alle umbenennen-Inputs auf sichtbar und gibt die Benennung nach dem unter Einstellungen->Einstellungen bestimmten Muster vor.
...
var PHDescriptions = {
	...
	"&COUNTER":"Enthält die Nummer des Dorfes in der AKTUELLEN Auflistung (kann nach dem umbenennen abweichen)",
...
else if(document.location.href.match(/village=[0-9]+&screen=overview_villages(&t=[0-9]+)?$/))
	{
	overviewInit();	
	}
else if(document.location.href.match(/village=[0-9]+&screen=overview_villages(&t=[0-9]+)?&mode=combined$/))
	{
	overviewInit();
	}
else
...
function settingsInit()
	...
	var tableXP = document.evaluate('/html/body/table[3]/tbody/tr/td/table/tbody/tr/td/table/tbody/tr/td/table/tbody/tr/td[2]/form/table', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
...
function saveSettings()
	...
	for(var option in optionsDefault)
		...
		if(checkSettingValue(option))
			...
			{
			alert(options[option]["label"]+" enthält einen ungültigen Wert!");
			}
		}
...
function getSingleSetting(setting)
	...
	if(options != null)
		...
		if(options[setting] != null)
			...
			{
			var redirect = confirm("Unbekannte Einstellung '"+setting+"'. Bitte teilen Sie dies dem Entwickler mit. Drücken Sie OK, um zum DS-Forum-Thread weitergeleitet zu werden.");
			if(redirect == true)
				{
...
//F�gt das Skript-spezifische HTML ins DOM ein und initialisiert die n�tigen Ereignisse auf der Kombinier-Übersicht
function overviewInit()
	{
	//Das "Dorf"-TH ermitteln
                                  
	var tdXP = document.evaluate('//table[@id="combined_table"]/tbody/tr/th', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
	...
	//Bild erstellen, das sp�ter das umbenennen-Icon enthalten wird
	...
	//Leerzeichen an das TH anh�ngen
	td.appendChild(document.createTextNode(" "));
	//Das Bild ans TH anh�ngen
	td.appendChild(img);
	}
	
//Setzt die umbenennen-Felder auf sichtbar und f�llt sie mit den neuen Namen
function rename()
	{
    ...
    var aXP = document.evaluate('//table[@id="combined_table"]/tbody/tr[position()>1]/td/span/a[2]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	...
	for(var i = 0;i<COUNT;i++)
		...
		//Pr�fen, ob ein Input gefunden wurde		
		if(input)
			{
			//getNewName f�r neuen Namen aufrufen
			var newName = getNewName(ID,i+1,input.value,X,Y,K,COUNT);
			//Neuen Namen ins Eingabefeld schreiben
			input.value = newName;
			
			/* ACHTUNG: ABSCHICKEN DES FORMULARS ZUR NAMENS�NDERUNG IST VERBOTEN, DA DAS SKRIPT SONST MEHRERE SPIELAKTIONEN AUTOMATISCH AUSL�SEN W�RDE */
			}
		}
...
function getNewName(id,counter,currentName,x,y,k,count)
	{
	//Alle Werte f�r alle m�glichen Platzhalter ermitteln
	//Falls kein Wert verf�gbar ist, wird ein leerer String verwendet
	//F�r Erkl�rungen der einzelnen Werte siehe Anfang des Skripts
	...
	//COUNTER und COUNT als Stringvariable speichern, um die länge zu bestimmten
