...
function addVersion(url) {
    return url+'&version=20130526'; // nam-thang-ngay
}
...
function taobao(cart_url,url_save) {
        ...
        if(p_e != null)
        {
            ...
            if(document.getElementById('J_PromoPrice')!= null && document.getElementById('J_PromoPrice')!='undefined')
            {
                try {
                    ...
                    if( p_p.getElementsByTagName("strong").length > 0 && 
                        p_p.getElementsByTagName("strong")[0].textContent!='' && tb.indexOf('VIP')==-1) 
                        {
    ...
    function getSellerName()
    {
        ...
        if(window.location.href.indexOf('tmall') > 0) {
            ...
            if(document.getElementsByClassName('slogo').length > 0) { // Page detail of shop
                if(document.getElementsByClassName('flagship-icon').length > 0) {
    ...
    function getComment()
    {
        var comment = document.getElementById("txtBz_nhaphang");
        if(comment != null ) {
    ...
    function getLink()
    {
        try {
            ...
            if( host.indexOf('tmall') == -1) {
                ...
                } else if(document.getElementById('J_listBuyerOnView') != null) {
            ...
            */
            ...
            alert("Có sự cố xảy ra do website đã thay đổi! [" + e + "]");
        }
        // return href+params;
    ...
    function xmlhttpPost(strURL) {
        ...
        var beforeHtml= document.getElementById("block_button_nhaphang").innerHTML;
        document.getElementById("block_button_nhaphang").innerHTML = 
        '<img style="margin-top:12px;margin-right:50px" src="http://nhaphang.vn/frontend/images/ajax-loader.gif" alt="" />';
        ...
        self.xmlHttpReq.onreadystatechange = function() {
            if (self.xmlHttpReq.readyState == 4) {
                ...
                document.getElementById("block_button_nhaphang").innerHTML=beforeHtml;
    ...
    this.htmlOnLoad=function ()
    {
        ...
        if(window.location.href.indexOf('tmall') > 0) {
            var shop_info = null;
            try{
                shop_info = document.getElementById('J_StrPriceModBox');
                shop_info.className = shop_info.className + ' notranslate';
                shop_info = document.getElementById('J_PromoBox');
                shop_info.className = shop_info.className + ' notranslate';
            } catch(e) {
                alert(e);
            }
        ...
        var self = this;
        self.xmlHttpReq = new XMLHttpRequest();
        self.xmlHttpReq.open('POST', 'http://nhaphang.vn', true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
        self.xmlHttpReq.onreadystatechange = function() {
            var ts = new Date();
            while (!loaded) {
                var te = new Date();
                if(te.getTime() - ts.getTime() >= 5*1000) {
                    loaded = true;
                    update_html();
                }
            }
    ...
    function update_html() {
        var src     = 'http://nhaphang.vn/nhaphang_order.gif';
        var rate    = rateMoney();
        ...
        //html vung luu lai
        var save_text='<div style="float:right;margin-right:50px;margin-top:6px">[<a id="id_nhaphang_save_item" href="javascript:;">Lưu lại đặt hàng sau</a>]</div>';
        ...
        ' <div style="float:left;padding-left:55px" id="block_button_nhaphang"><a id="id_nhaphang_add_cart" href="javascript:;"><img src="' + src + '" alt="" /></a></div>'+save_text+'</div></div></li>'+
        '';
...
function alibaba(cart_url,url_save) {
    ...
    function getSellerId() {
        ...
        try{
            ...
            alert('Không l\u1ea5y được thông tin người bán!');
        }
    ...
    function get_item_data() {
        ...
        try {
            // Multi buy
            var tbl_wrap = document.getElementsByClassName('table-wrap');
            if(tbl_wrap.length > 0) {
                input_data = tbl_wrap[0].getElementsByClassName('amount-input');
                if(input_data.length > 0) {
                    ...
                    for(inc in input_data) {
                        ...
                        result[i] = new Array();
                        // Add data to array
                        result[i][0] = isNaN(input_data[inc].value) ? 0 : input_data[inc].value;
                        result[i][1] = input_data[inc].getAttribute('data-count');
                        result[i][2] = input_data[inc].getAttribute('data-prop2');
                        result[i][3] = input_data[inc].getAttribute('data-prop1');
                        result[i][4] = input_data[inc].getAttribute('data-price');
                        i++;
                    }
                }
            } else {
                tbl_wrap = document.getElementsByClassName('content-wrapper');
                var content = null;
                if(tbl_wrap.length > 0) {
        ...
        for(var o in data) {
            try {
                ...
                if(parseInt(data[0]['amount']) < parseInt(min_amount)) {
                    alert('Số lượng tối thiểu không đủ!');
                    continue;
                }
    ...
    function linkSave() {
        ...
        {
            alert("Số lượng sản phẩm không được bé hơn "+min_quantity+"!");
            document.getElementById("id_nhaphang_save_item").setAttribute('href','javascript:void(0);');
            return ;
        }
        ...
        {
            alert("Số lượng sản phẩm phải là bội số của "+min_quantity+"!");
            document.getElementById("id_nhaphang_save_item").setAttribute('href','javascript:void(0);');
            return ;
        }
    ...
    this.htmlOnLoad=function ()
    {
        var self = this;
        self.xmlHttpReq = new XMLHttpRequest();
        self.xmlHttpReq.open('POST', 'http://nhaphang.com', true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
        self.xmlHttpReq.onreadystatechange = function() {
            var ts = new Date();
    ...
    function update_html() {
        ...
        + '	<div style="font-weight:bold;color: blue; font-size: 24px;margin-bottom:10px;margin-left:41px" id="hangnhap-price">'+(price_result)+' VNĐ</div>'
        + '	<div>'
        + '		<span style="width:50px;">Mô tả：</span><textarea cols="64" id="txtBz" name="txtBz"></textarea>'
        + '	</div>'
        + '	<div class="clr" style="width:100%;float:right">'
        + '		<div>'
        + '			<span style="float:left;margin-left:41px" id="block_button_nhaphang">'
        + '				<a id="id_nhaphang_add_cart" href="javascript:;">'
        + '					<img src="' + src + '" alt="" />'
        + '				</a>'
        +	'			</span>'
        + '			<span style="float:right;margin-right:40px;margin-top:10px">[<a id="id_nhaphang_save_item" href="javascript:;">Lưu lại đặt hàng sau</a>]</span>'
        + '		</div>'
        + '	</div>'
        + '</div>';
        ...
        document.getElementsByClassName('d-wrap')[0].insertBefore(div, document.getElementsByClassName('d-wrap')[0].lastChild);
        
        document.getElementById("id_nhaphang_add_cart").addEventListener( 'click', linkClick);
        document.getElementById("id_nhaphang_save_item").addEventListener( 'click', linkSave);
        
        beforeHtml = document.getElementById("block_button_nhaphang").innerHTML;
    }
}
...
function paipai(cart_url,url_save) {
    ...
    function xmlhttpPost(strURL) {
        ...
        var beforeHtml= document.getElementById("block_button_nhaphang").innerHTML;
        document.getElementById("block_button_nhaphang").innerHTML='<img style="margin-top:12px;margin-right:50px" src="http://nhaphang.com/frontend/images/ajax-loader.gif" alt="" />';
        self.xmlHttpReq.open('POST', strURL, true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
    ...
    this.htmlOnLoad=function ()
    {
        ...
        var save_text='<span style="float:right;margin-right:40px;margin-top:10px">[<a id="id_nhaphang_save_item" href="'+href+'">Lưu lại đặt hàng sau</a>]</span>';
        ...
        s +='<div><span style="float:left;margin-left:44px" id="block_button_nhaphang"><a id="id_nhaphang_add_cart" href="' + href + '"><img src="' + src + '" alt="" /></a></span>&nbsp; '+save_text+'</div>';
...
function mofayichu(cart_url,url_save) {
    ...
    this.htmlOnLoad=function ()
    {
        ...
        var save_text='<div style="float:right;margin-top:9px">[<a id="id_nhaphang_save_item" href="'+href+'">Lưu sản phẩm</a>]</div>';
        ...
        s +='<div><div style="float:left" id="block_button_nhaphang"><a id="id_nhaphang_add_cart" href="' + href + '"><img width="200" height="30" src="' + src + '" alt="" /></a></div>&nbsp;'+save_text+'</div>';
...
function _5taobao(cart_url,url_save) {
    ...
    this.htmlOnLoad=function ()
    {
        ...
        /*var s = '<div class="g-group-member" style="height: 40px; border: 2px solid blue;"><div class="clr" style="width:100%;float:right">';
			s +='<div><span style="float:right;margin-top:2px" id="block_button_nhaphang"><a id="id_nhaphang_add_cart" href="' + href + '"><img width="200" height="30" src="' + src + '" alt="" /></a></span>&nbsp;<span style="font-weight:bold;color: blue; font-size: 17px;float:left;margin-top:10px" id="hangnhap-price">'+(price_result)+' VNĐ</span> </div>';
...
function yiranmeifushi(cart_url,url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div class="g-group-member" style="height: 94px; border: 2px solid blue; padding: 5px;">'
        + '<div><span style="width:10%;">Mô tả：</span><textarea style="width:85%; float:right;" id="txtBz" name="txtBz"></textarea></div>'
        + '<div class="clr" style="width:100%;float:right">'
        + '		<div>'
        + '			<span style="float:right;margin-top:2px" id="block_button_nhaphang">'
        + '				<a id="id_nhaphang_add_cart" href="javascript:;"><img src="' + src + '" alt=""/></a>'
        + '			</span>'
        + '			<span style="font-weight:bold;color:blue;font-size:22px; float:left;margin-top:10px" id="hangnhap-price">'
        + 				(price_result) +' VNĐ'
        + '			</span>'
        + '		</div>'
        + '</div></div>';
...
function jj_fashion() {
    ...
    this.htmlOnLoad=function ()
    {
        ...
        + '          <br/><a href="http://nhaphang.vn" title="trang chủ" target="_blank" style="color: blue !important; float: right; font-size: 13px; position: relative; top: 10px;">'
        + '              nhaphang.vn'
        + '          </a>'
        + '      </h2>'
        + '  </div>';
...
function baobaopifa(cart_url,url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        + '		<div>'
        + '			<span style="float:right;margin-top:2px" id="block_button_nhaphang">'
        + '				<a id="id_nhaphang_add_cart" href="javascript:;"><img width="160" height="30" src="' + src + '" alt=""/></a>'
        + '			</span>'
...
function hongwufushi(cart_url) {
    ...
    // hàm lấy số lượng mua
    function getQuantity() {
        var _quanlity = document.getElementById('nhaphang_tool_amount').value;
    ...
    this.htmlOnLoad = function () {				
        try {
            ...
            try {
                ...
                alert("Can't find element on page." + e);
            }
        } catch(e) { 
            alert(e);
        }
...
var url = "http://nhaphang.vn/cart_taobao?";
...
var url_save = "http://nhaphang.vn/item/item_collect_addon?";
