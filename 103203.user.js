// ==UserScript==
// @name           KOC Power Tools(traducido por Josshsx3)
// @namespace      mat
...
var Title = 'KOC Power Tools(traducido por Josshsx3)';
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  logit ("* KOCpowerTools v"+ Version +" Cargada");
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('Menu Power', eventHideShow, mouseMainTab);
 TestSomething.init ();
//setInterval (function(){logit (inspect (getClientCoords (mainPop.getMainDiv()), 3, 1))}, 2000);  
}
function cerrar() {
var confirm = window.self;
confirm.opener = window.self;
confirm.close();
}
...
/*
var knightRoles = [
  ['Capataz', 'politica', 'Pol'],
  ['Mariscal', 'combate', 'Com'],
  ['Alquimista', 'inteligencia', 'Int'],
  ['Administrador', 'ingenio', 'Res'],
];
*/
...
Tabs.Knights = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (row++ % 2)
        sty = 'class=ptOddrow ';        
		m = '<TR '+ sty +'valign=top align=left><TD><B>'+ (roleId==null ? 'Sin trabajo':knightRoles[rid][0]) +'</b></td><TD align=left>';//tarea asignada
...
var AttackDialog = {
  ...
  initCityPicker : function (){
	...
	var div = document.getElementById('modal_attack_target_numflag'); // KofC version 96;
    var mySpan;
    if (div) {
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    msg += 'Mi '+ target +' sera '+ atkType  +' por '+ who +'. Las tropas que llegan son (llegan en '+
...
logit ("ajax/allianceGetMembersInfo.php:\n"+ inspect (rslt, 5, 1));      
  ...
  init : function (div){
    ...
    if (getMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER>Usted necesita estar en una alianza para usar esta función.</center>';
      t.state = 1;
      return;
    }
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Ponga todo o parte del nombre de un jugador: &nbsp;</td>\
        ...
        <TR><TD class=xtab></td><TD class=xtab> Ponga todo o parte del nombre de Alianza: &nbsp;</td>\
        <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Buscar Alianza" /> &nbsp; <INPUT id=myAllSubmit type=submit value="'+ getMyAlliance()[1] +'" /></td>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'Ponga al menos 3 caracteres';
      return;
    }
  ...
  eventGotPlayerOnlineList : function (rslt){
	...
	var m = '<DIV class=ptstat>Mostrando jugadores coincidentes con <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE width=100% align=center class=ptTab cellspacing=0><TR style="font-weight:bold"><TD>Nombre</td>\
      <TD align=right>Poder</td><TD> &nbsp; Online</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Operaciones de búsqueda</td></tr>';
    var row=0;
    var cl='';
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "buscando información de clasificación ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Clasificación:</b> no se encuentra! (posible niebla?)';
      return;
    }
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Clasificación: </b></td><TD colspan=2> Poderio: '+ p.might  +' &nbsp; Alianza: '+ an +'</td></tr>'; 
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'Ponga al menos 3 caracteres';
      return;
    }
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Mostrando resultados de busqueda <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Nombre de Alianza</td><TD class=xtab>Ranking</td><TD class=xtab>Miembros</td>\
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Jugador/a</div></a></td>\
        <TD id=clickCol1 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Poderio</a></div></td>\
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Ciudad/des</a></div></td>\
      ...
      <DIV style="top:670px; left:0px; position:absolute; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Estos datos de clasificación y poderio pueden ser de hasta hace 24 horas!</td>\
          <TD class=xtab align=right>Haga clic en los encabezados de las columnas para ordenar</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("No se encontrarón coincidencias")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
...
Tabs.Test = {
  ...
  init : function (div){
    ...
    var m = '<TABLE><TR><TD align=right>Explorador: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        ...
        <TR><TD align=right>Falso Reporte: </td><TD><INPUT type=checkbox id=fakeFalse></td></tr>\
        <TR><TD align=right>Segs: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
        <TR><TD align=right>de milicianos: </td><TD><INPUT type=text size=6 value=5000 id=fakeMilitia></td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=testSendMarch type=submit value="Falso Ataque" \></td></tr></table>\
        <INPUT id=ptReloadKOC type=submit value="Recargar KOC" \><BR>\
...
//uwMYmodal_alliance_report_view("6149415",1,51,8,13459797,"Anngrethe","F","Dudious","M",2,451,151,1299907771,0,267,433);
  ...
  reloadKOC : function (){
	...
	//var goto = 'http://apps.facebook.com/kingdomsofcamelot/?';
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>INFORMACIÓN GENERAL</div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>COSTES DE CONSTRUCCIÓN</b></td><TD class=xtabHL colspan=7><B>ESTADÍSTICAS</b></td><TD class=xtabHL><B>COSTE</b></td></tr>\
        ...
        <TD class=xtabH>Mineral</td><TD class=xtabH>Población</td><TD class=xtabHL>Poderio</td><TD class=xtabH>Vida</td><TD class=xtabH>Ataque</td><TD class=xtabH>Defensa</td><TD class=xtabH>Velocidad</td><TD class=xtabH>Rango</td><TD class=xtabH>Carga</td>\
    ...
    m += '<BR><DIV class=ptstat>CALCULADOR DE DISTANCIAS</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      ...
      <BR><BR></center>';
    m += '<DIV class=ptstat>MAPA DE PROVINCIAS</div><DIV id=ptProvMap style="height:'+ provMapCoords.imgHeight +'px; width:'+ provMapCoords.imgWidth +'px; background-repeat:no-repeat; background-image:url(\''+ URL_PROVINCE_MAP +'\')"></div>';
...
Tabs.Options = {
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        <TR><TD colspan=2><B>Configuración:</b></td></tr>\
        ...
        <TR><TD><INPUT id=ptHideOnGoto type=checkbox /></td><TD>Ocultar ventana al hacer clic en el mapa de coordenadas. <SPAN class=boldRed>&nbsp;(NUEVO)</span></td></tr>\
		<TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Mostrar \'Alarma de alimento\' en rojo si solo hay alimento para menos de \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> horas</td></tr>\
        <TR><TD colspan=2><P><B>Características de KofC:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Mostrar numero de mensajes personales y de reportes de tropa por separado en el icono de mensajes.</td></tr>';
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Habilitar mejoras en informes de alianza.</td></tr>\
        <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Permitir a scrpits cambiar el formato de los informes de la Alianza.</td></tr>\
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Habilitar mejoras de mensajes (botones "adelante/atras " y "todos los oficiales").</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Mayor exploración de páginas de mensajes e informes.</td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Avisar si intentan marchar a la ubicación 0,0.</td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Habilitar reloj GMT junto al "Tiempo de Camelot" <SPAN class=boldRed>&nbsp;(NUEVO)</span></td></tr>\
        <TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Habilitar mejoras de Chat (hacer clic en coords, mensajes globales en rosa, haga clic en el icono de susurro) <SPAN class=boldRed>&nbsp;(NUEVO)</span></td></tr>\
        <TR><TD><INPUT id=togAttackPicker type=checkbox /></td><TD>Habilitar selector de la ciudad de destino en el diálogo de ataque (reforzar, reasignar y transporte) <SPAN class=boldRed>&nbsp;(NUEVA Opción)</span></td></tr>\
        <TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Mostrar rondas en los informes de batalla <SPAN class=boldRed>&nbsp;(NUEVO)</span></td></tr>\
        <TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>Habilitar el botón de eliminar al mostrar informe de batalla <SPAN class=boldRed>&nbsp;(NUEVO)</span></td></tr>\
		<TR><TD colspan=2><BR><BR><B>Errores corregidos de KofC:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Mejorar informe de torre para mostrar el destino exacto (de la ciudad, tierra o no válido).</td></tr>\
        ...
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Fijar alertas de falsos ataque creados a partir de misiones de exploración.</td></tr>\
        <TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD>No seleccionar automáticamente un caballero al cambiar el tipo de marcha, explorador, transporte o cambio de destino <SPAN class=boldRed>(NUEVO)</span></td></tr>\
        <TR><TD><INPUT id=togCoordBox type=checkbox /></td><TD>Mantenerla caja/marcador del mapa de coordenadas en la parte superior de la actividad de tropas <SPAN class=boldRed>(NUEVO)</span></td></tr>\
		</table><BR><BR><HR>Tenga en cuenta que si una casilla es de color gris, es porque probablemente ha habido un cambio de código en KofC, lo que hace la opción inoperables.';
      t.cont.innerHTML = m;
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Entrenamiento de tropas y construcción de defensas</div><DIV style='height:5px'></div><DIV class=ptentry>\
+      <DIV style='text-align:center; margin-bottom:5px;'>Seleccionar Ciudad: &nbsp; <span id=ptspeedcity></span></div>\
      <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
      <TABLE align=center><TR><TD align=right>Tipo de tropa: </td><TD colspan=2>\
      <SELECT id=ptttType>\
        <option value='1'>T.Suministro</option>\
        ...
        <option value='7'>C.Ligera</option>\
        <option value='8'>C.Pesada</option>\
        <option value='9'>Carretas</option>\
        <option value='10'>Ballistas</option>\
        <option value='11'>Arrietes</option>\
        <option value='12'>Catapultas</option>\
      </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
      <TR><TD align=right>Unidades por grupo de cola: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right>Cantidad de grupos de cola: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
        <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
      <TR><TD align=right valign=top>Establecer los trabajadores como inactivos: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Le permite entrenar más tropa. Puede establecer temporalmente la población como negativa.</span></td></tr>\
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='Entrenar Tropa'\></td></tr>\
      </table></td><TD width=20></td><TD style='border-left:solid 2px;' width=50% align=center>\
      <TABLE align=center><TR><TD align=right>Tipo Defensas: </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Ballistas M.</option>\
        <option value='55'>Fundibulos</option>\
        <option value='60'>Trampas</option>\
        <option value='61'>Abrojos</option>\
        <option value='62'>Ba.Picos</option>\
      </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
      <TR><TD align=right>Cantidad por Grupo en cola: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right>Numero de Grupos en cola: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>El numero de tropas por grupo debe ser mayor de cero.</font>';
      return;
    }
    ...
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero de grupos no valido.</font>';
      return;
    }        
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; En propiedad:'+ t.stats.defOwned;   
    ...
    t.TDspSpace.innerHTML = 'Nivel de Muro: <B>'+ t.stats.wallLevel +'</b><BR>Espacios en Muro: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\  Espacios en Campo: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
	if (t.running){
      t.stopTraining('<SPAN class=boldRed>Entrenamiento cancelado por usuario</span>');
      return; 
    }    
    if (perSlot<1){
    ...
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>No se pueden entrenar mas unidades (maximo '+ t.stats.MaxDefTrain +' en total)</font>';
      return;
    }
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><SPAN id=ptttr_food><B>Alimento:</b><BR>'+ addCommasInt(t.stats.food) +'</span></td>\
        ...
        <TD width=16%><SPAN id=ptttr_pop><B>Inactividad:</b><BR>'+ addCommasInt(t.stats.idlePop) +'</span></td></tr></table><BR>';
...
// defense queue ....
  ...
  stopTraining : function (msg){
    ...
    t.TDbutDo.value = 'Hacer defensas';
    t.TTbutDo.className = '';
    t.TDbutDo.className = '';
    t.running = false;
  },
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Entrenando '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +' en '+ Cities.byID[cityId].name +' ('+ que.length +' slots restantes)<BR>');
      ...
      logit (inspect (err, 8, 1));
      t.stopTraining  ('<SPAN class=boldRed>ERROR DE PROGRAMA: '+ err.message +'</span>');
    }
  },
}
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Conectado desde el </span> '+ dt.toLocaleDateString() +'</td>\
      ...
      str += _row ('Duración Alim.', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Cola Defensas', row, true);
       str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Incluir marcha de Tropas /o Recursos</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptoverIncTraining'+ (Options.includeTraining?' CHECKED':'') +'>Incluir formación de las tropas</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Permitir desbordamiento de ancho \
         &nbsp; &nbsp; Tamaño de fuente: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr>';
      str += "</table></div>";
...
Tabs.Marches = {
  ...
  showAttacks : function (){
    var t = Tabs.Marches;
    t.marchDiv.innerHTML = '<CENTER>Llegada de proximos Ataques enemigos!</CENTER>';;
  },
  ...
  showMarches : function (){
    var t = Tabs.Marches;
    t.marchDiv.innerHTML = '<CENTER>Marchas SALIENTES!</CENTER>';
  },
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['T.Sum.', 'Mili.', 'Explo.', 'Pique.', 'Espa.', 'Arque.', 'C.Lig.', 'C.Pes.', 'Carre.', 'Ballis.', 'Arri.', 'Cata.'];
    var t = Tabs.Marches;
      clearTimeout (t.displayTimer);    
...
//logit ("enc: "+ inspect (enc, 6, 1));
    s = '<div class=ptstat>Mostrando tropas acampadas en cada uno de las embajadas.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>No hay tropas acampadas.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=center><TD align=left width=16%><B>Jugador (Poderio)</b></td>';
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
      ...
      s += '<TR><TD colspan=13><BR><BR></td></tr><TR align=center><TD class="tot" align=left><B>TOTALES:</b></td>';
      for (i=1; i<13; i++)
        s+= '<TD class="tot">'+ tot[i] +'</td>';
      s += '</tr></table>';
    }

    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Mostrar origen de tropas';
    s += '<BR><BR><DIV style="font-size: 10px">NOTA: Tendrá que recargar KofC para mostrar tropas acampadas o las tropas que quedan después de una batalla.</div>';
...
var WarnZeroAttack = {
  ...
  hook : function (){
    ...
    && parseIntZero(document.getElementById('modal_attack_target_coords_y').value) == 0){
      new CdialogCancelContinue ('<SPAN class=boldRed>Estás a punto de marchar a la ubicación 0,0!</span>', null, unsafeWindow.modal_attack_check, document.getElementById('modalInner1'));      
    } else {
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    m += '<TD class=spacer width=90% align=right>'+ Version +'(Traducido por Aioras)&nbsp;</td></tr></table>';
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
  ...
  pop.getMainDiv().innerHTML = '<TABLE class=ptTab align=center style="height: 100%"><TR align=center height=90%><TD>'+ msg +'</td></tr>\
      <TR align=center><TD><INPUT id=ptcccancel type=submit value="CANCELAR" \> &nbsp; &nbsp; <INPUT id=ptcccontin type=submit value="CONTINAR" \></td></tr></table>';
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>KOC Menu Tools</center>';
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Ha ocurrido un error:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Automáticamente volver a intentar en <SPAN id=paretrySeconds></b></span> sgs ...<BR><BR><INPUT id=paretryCancel type=submit value="CANCELAR Reintento" \>';
...
if (DEBUG_TRACE) logit ("myAjaxRequest.myFailure(): "+ inspect(rslt, 1, 1));
    o.ok = false;
    o.errorMsg = "Fallo de comunicación de AJAX";
    wasFailure (o);
  }
...
function inspect(obj, maxLevels, level, doFunctions){
  ...
  if(maxLevels < 1)
      return 'Error inesperado: El numero de nivel debe ser mayor que 0';
  if(obj == null)
...
// Remove Facebook 
//////////////////////////////////////////////////////////////////////////////////// 
//And then change the line where it says "mainbody=...." from "left:765px" to "left:0px"
/* 
