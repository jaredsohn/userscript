...
//---------------
	...
	var win_mouse_right = 0;
	var win_mouse_top = 0;
	var move = function(id){
		$('#'+id+'_title').mousedown(function(e){
			var position = $('#'+id).position();
			win_mouse_right = e.pageX-position.right;
			win_mouse_top = e.pageY-position.top;
			$(document).mouseup(function(){
				$(document).unbind('mousemove');
				var position = $('#'+id).position();
				localStorage.setItem(id+'_win', JSON.stringify({'x':position.right,'y':position.top}));
			});
			$(document).mousemove(function(e){
				$('#'+id).css({'right':Math.max(e.pageX-win_mouse_right, 0)+'px','top':Math.max(e.pageY-win_mouse_top, 0)+'px'});
			});
		});
	}
	var new_win = function(id, title, h, w){
		...
		$('#'+id).css({'opacity':'0.8','color':'black','position':'absolute','right':Math.max(win_pos.x, 0)+'px','top':Math.max(win_pos.y, 0)+'px','z-index':'1000','-moz-border-radius': '5px 5px 5px 5px','background-color':'white','height':h+'px','width':w+'px','border':'1px solid', 'border-color':'rgb(236,236,236)'});
...
//---------------
	...
	$('#BGConfig').css({'-moz-user-select':'none','cursor':'default','opacity':'0.8','color':'black','position':'fixed','right':'0px','top':'0px','z-index':'1000','-moz-border-radius': '0px 0px 5px 0px','background-color':'white','border':'1px solid', 'border-color':'rgb(236,236,236)'});
