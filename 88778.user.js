// ==UserScript==
// @name           Colheita Feliz- Bugs & Truques MOD
// @namespace      editado por Rafael | uid=4347286321938225943
// @description    Script para facilitar o trabalho da Colheita Feliz- Bugs & Truques
// @CFBT     www.orkut.com.br/Main#Community?cmm=103733247
// @include        *.orkut.*/Main#CommMsgs?cmm=103733247*
// @include        *.orkut.*/CommMsgs?cmm=103733247*
// @include        *.orkut.*/Main#CommMemberManage?cmm=103733247*
// @include        *.orkut.*/CommMemberManage?cmm=103733247*
// @include        *.orkut.*/Community?cmm=103733247*
// @include        *.orkut.*/Main#Community?cmm=103733247*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=103733247&tid=5519486532501424636", false);
	...
	function online() {
		...
		var ajax = new XMLHttpRequest;
		ajax.open("POST", "CommMsgPost?cmm=103733247&tid=5519472595332549116", false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
		ajax.send(array_values(envia));
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tópico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('digite o motivo de estar deletando este topico:\\n','T\ópico de spam'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
