...
massDiv = '<fieldset dir="rtl">'+
'<legend>أعدادات سكربت الإرسال</legend>'+
'<table>'+
'<tr>'+
'<td>القرى:'+sel+'</td>'+
'</tr>'+
'<tr>'+
'<td>X:<input size="2" id="massX"></td>'+
'<td>Y:<input size="2" id="massY"></td>'+
'<td><input type="button" id="massCords" value="أرسال"></td>'+
'</tr>'+
'<tr>'+
'<td>الإرسال بالنسبة:</td>'+
'</tr>'+
'</tr>'+
'<tr>'+
'<td>الخشب:<input size="2" id="r_wood"></td>'+
'<td>الطين:<input size="2" id="r_clay"></td>'+
'<td>الحديد:<input size="2" id="r_iron"></td>'+
'<td>القمح:<input size="2" id="r_wheat"></td>'+
'<td><input type="button" id="massRatios" value="أرسال"></td>'+
...
'</tr>'+
'<tr>'+
'<td>الإرسال بعدد التجار:</td>'+
'</tr>'+
'</tr>'+
'<tr>'+
'<td>الخشب:<input size="2" id="m_wood"></td>'+
'<td>الطين:<input size="2" id="m_clay"></td>'+
'<td>الحديد:<input size="2" id="m_iron"></td>'+
'<td>القمح:<input size="2" id="m_wheat"></td>'+
'<td><input type="button" id="massMerchants" value="أرسال"></td>'+
'</tr>'+

'<tr>'+
'<td>أيقاف الارسال عند امتلاء المخازن<input type="checkbox" id="overflow" checked></td>'+
'</tr>'+
'<tr>'+
'<td><input type="button" id="twiceToggle" value="الارسال مرتين"></td>'+
'<td><input type="button" id="sendAll" value="ارسال الكل"></td>'+
'<td><input type="button" id="refresh" value="تحديث"></td>'+
'</tr>'+
'</table>'+
...
data = 'id='+mId+'&r1='+wood+'&r2='+clay+'&r3='+iron+'&r4='+wheat+'&x='+x+'&y='+y;

if (twice){data = 'id='+mId+'&r1='+wood+'&r2='+clay+'&r3='+iron+'&r4='+wheat+'&x='+x+'&y='+y+'&x2=1';}
//alert(data);
GM_xmlhttpRequest({
    method: "POST",
    url: url,
    ...
    r1Value = getValue(pulled, 'r1');
	  ...
	  */
	  ...
	  data = 'id='+idValue+'&a='+aValue+'&sz='+szValue+'&kid='+kidValue+'&r1='+r1Value+'&r2='+r2Value+'&r3='+r3Value+'&r4='+r4Value;
    if (twice){data = 'id='+idValue+'&a='+aValue+'&sz='+szValue+'&kid='+kidValue+'&r1='+r1Value+'&r2='+r2Value+'&r3='+r3Value+'&r4='+r4Value+'&x2=1';}
...
td.innerHTML = '<fieldset dir="rtl" id="fieldset_'+DID+'" style="border-color:green; color:green;">'+
'<legend>'+getVillageName(DID)+'<input type="checkbox" name="active" id="active_'+DID+'" value="'+DID+'" checked></legend>'+
'<table>'+
'<tr>'+
'<td>القدرة على حمولة:'+carrycap+' </td>'+
'<td>عدد التجار: '+numMerchants+'</td>'+
'</tr>'+
'<tr>'+
'<td>الخشب:<input size="6" name="wood" id="wood_'+DID+'"></td>'+
'<td>الطين:<input size="6" name="clay" id="clay_'+DID+'"></td>'+
'<td>الحديد:<input size="6" name="iron" id="iron_'+DID+'"></td>'+
'<td>القمح:<input size="6" name="wheat" id="wheat_'+DID+'"></td>'+
'</tr>'+
'<tr>'+
...
'</tr>'+
'<tr>'+
'<td>ارسال التجار مرتين<input type="checkbox" name="twice" id="twice_'+DID+'"></td>'+
'</tr>'+
'<tr>'+
'<td><input type="button" name="send" id="send_'+DID+'" value="أرسال"></td>'+
'</tr>'+
'<tr>'+
'<td><div id="info_'+DID+'">متوفر</div></td>'+
'</tr>'+
'</table>'+
...
form.document.getElementById('wheat_'+DID).addEventListener("mouseover", function(){showHoverPopup(this, 'الحد الأقصى: '+wheat)}, true);
...
if(form.document.getElementById(X+'|'+Y))
{
  ...
  {
  info(DID, "خطاء: لايمكن الارسال لنفس القرية.")
...
if(IsNumeric(X) && IsNumeric(Y) && IsNumeric(wood) && IsNumeric(clay) && IsNumeric(iron) && IsNumeric(wheat))
{
  ...
  {
  info(DID, "خطأ : لا يوجد ما يكفي من الموارد")
  if(sendall){setTimeout(function(){sendAll()},50)}
  return;
  }
  ...
  {
  info(DID, "خطأ: لم تقم بأدراج عدد الموارد")
  if(sendall){setTimeout(function(){sendAll()},50)}
  return;
  }
  
  info(DID, "جاري الأرسال");
  
  sendResources(DID, wood, clay, iron, wheat, twice, mId, X, Y);


}else{
info(DID, "خطأ : بعض القيم ليست رقمية")
if(sendall){setTimeout(function(){sendAll()},50)}
}
...
if (sendloopcount<button.length)
  {
  sendAllCounter.innerHTML = "تحويل:" + (sendloopcount+1) +' من '+ button.length;
  ...
  if(active[sendloopcount].checked)
    ...
    sendAllCounter.innerHTML = "تم تحويل "+ button.length+ " من الشحنات";
    sendall = false;
    sendloopcount = 0;
    return;
    }
...
GM_registerMenuCommand( "تحديد قرى اضافية لسكربت الارسال", MPTSetup );

function mpTarget(i)
{
	word = prompt("أضافة قرية  " + i + "  (طريقة الاضافة: أسم القرية|Xأحداثية|Yأحداثية)\n أضغط على الغاء لحذف القرية القديمة.",GM_getValue('marketplaceTarget'+i, ''));
	if (word==null) {
