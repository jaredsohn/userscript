// ==UserScript==
// @name           OkC Forum Ignore List
// @namespace      http://estragib.googlepages.com/custom
// @url            http://userscripts.org/scripts/source/33443.user.js
// @description    Hides posts by users on your ignore list.
...
function check() {
	...
	for each(var link in userLinks) {
		for each(var entry in ignoreList_) {
			var opacity = 1;
...
function restoreAll() {
	...
	for each(var userLink in userLinks) {
		var row = userLink.parentNode.parentNode.parentNode; // a -> p/div -> td -> tr
...
function editIgnoreList(wrapper) {
	...
	table.innerHTML = '<col class="' + CLASS_COL_ENTRY + '" />' +
	                  ...
	                  '<th class="' + CLASS_COL_RE + '"><span title="Regular expression syntax">Pattern</span></th>' +
	...
	for each(var entry in ignoreList_) addRow(body, entry);
	addRowNewEntry(body);
...
function saveEntry(row) {
	...
	if(entry.length && entry != INPUT_ADD_ENTRY_VALUE) {
		...
		if(old.toLowerCase() != entry.toLowerCase()) {
			...
			for each(button in buttons) button.name = entry;
