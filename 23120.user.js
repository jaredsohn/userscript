...
// @namespace      Travian
// @description    Battle Analyse 4.1.0
...
function attacktable(){
	...
	if(tm!=null){
		...
		if (warsim != 1) {
			ts1 = table[3].textContent;
			ts2 = table[tda-1].textContent;
			ts3 = table[tdl-1].textContent;
		}
		for(var i=0;i<=(9+statushero);i++){
			if (warsim != 1) {
				ts1 = ts1 + "\t[" + table[3+i].firstChild.title + "]";
				ts2 = ts2 + "\t" + table[tda+i].textContent;
				ts3 = ts3 + "\t" + table[tdl+i].textContent;
			}
		...
		table[2-warsim*2].innerHTML='<img title="'+langfile[0]+'" src="'+imgp('img/un/a/att_all.gif')+'" align="absmiddle" height="15" width="15">&nbsp;<font class="f8"><i>'+atstemp[0]+'</i></font>';
