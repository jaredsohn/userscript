// ==UserScript==
// @name           Megavideo Clean HTML And Play With Mplayer
// @description    Removes all HTML megavideo page and leaves only the player with 1280x720 resolution. Also change the player to be mplayer instead of flashplayer. To operate must install mplayer and mplayer plugin. Linux debian user or mint: -sudo apt-get install mplayer -sudo apt-get install ffmpeg -sudo apt-get install totem-mozilla
...
function download_tag() {
	var tag = tagload();
	flv = 'http://www'+vars.s+'.megavideo.com/files/'+decrypt(vars.un, vars.k1, vars.k2)+'/'+vars.title+'.flv';
...
function load_player() {
  ...
  $("playerdiv").innerHTML = '<embed height="720" width="1280" autoplay="true" scale="tofit" src="'+flv+'" type="application/x-mplayer2" id="no-flash-player">';
}
...
(function() {
    ...
    (document.getElementsByTagName('tr')[0]).innerHTML = '<td colspan="5"><br></td>';
    (document.getElementsByTagName('tr')[2]).innerHTML = '<td colspan="5"></td>';
    (document.getElementsByTagName('tr')[3]).innerHTML = '<td colspan="5"></td>';
