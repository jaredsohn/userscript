// ==UserScript==
// @name           BomBon calcula el tiempo de los misiles en español
// @namespace      BomBon
// @description    Calculo de la hora de impacto de los misiles y tiempo de vuelo 
...
function serveurTimer()
{
	...
	if (diffSec > 3 || diffSec < -3) // Si la différence est trop grande ...
	{
		diffSec=0;
	}*/
	...
	if(vitesse == -1)
		{
			newElement.innerHTML = 'Por favor BomBon le pide que visite la pagina de recursos(opciones de recursos) para sacar la velocidad de vuelo ';
		}
		else
                {
			newElement.innerHTML = '<div align=center> BomBon ha calculado que el Impacto de los misiles sera el ' + day + ' a las ' + hours+ ':'+minutes+':'+seconds+ '. Tiempo de vuelo de los misiles : '+ h + ':' + m +':'+ s  +' .</div>'+'<br>'+'<div align=center>Velocidad del universo  * ' + vitesse + '</div>'; // Texte à afficher
		}
	}
	else
	{
		newElement.innerHTML = 'BomBon dice que no se encuentra en la galaxia adecuada';
	}
