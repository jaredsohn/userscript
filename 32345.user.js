// ==UserScript==
// @name           Vikatan Easy Navigate
// @namespace      http://saravan.blogspot.com
// @description    Easier Navigation for vikatan sites
// @include        http://www.vikatan.com/*
// ==/UserScript==

/*********************************************************************
* Vikatan Easy Navigate - Easier Navigation for vikatan sites
...
function btnOpenInTab_Click() {
	...
	for(var i=0; i<selectedLinks.snapshotLength; i++) {
		...
		var u = chk.value;
	        ...
	        window.setTimeout(GM_openInTab, (lnkCnt++)*3000, u);
	    }
	}
}
...
function findAndAddMenuItems() {
	var mainMenuItems = document.evaluate("//a[contains(@id, 'link')]", document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	...
	try {
		var rightColResult = document.evaluate("//html/body/table/tbody/tr/td[3]", document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
		if(rightColResult.snapshotLength>0) {
			var rightCol = rightColResult.snapshotItem(0);
			rightCol.insertBefore(menuDiv, rightCol.firstChild);
			//rightCol.appendChild(menuDiv);
			var contentTDResults = document.evaluate("//td[@width='779']", document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
			if(contentTDResults.snapshotLength>0) {
				contentTDResults.snapshotItem(0).setAttribute("valign", "top");
			}
		}
		else {
	...
	GM_addStyle('.__gmMenu { background-color: #fff; color: #000000; font-family: Vikatan_TAM, Latha,Arial Unicode MS,"trebuchet ms",sans-serif; padding-bottom: 10px;} .__gmMenu h2 { font-size: small; text-align: left; font-weight: bold; margin-top: 10px; margin-bottom: 5px; } .__gmMenu ul { padding-left: 0px; margin: 0px; margin-bottom: 0px; } .__gmMenu li { 	margin-top:5px; margin-left: 20px; font-size: small; text-align: left; list-style-type: none; } .__gmMenu a:link, a:visited,  a:active  { color: #006666; text-decoration: none; } .__gmMenu a:hover { color: #999999; text-decoration: underline;}');
}

function removeMarquee() {
	var allMarq = document.getElementsByTagName("marquee");
	for(var i= allMarq.length-1;i>=0;i--) {
		allMarq[i].parentNode.removeChild(allMarq[i]);
	}
}
findAndAddMenuItems();
//removeMarquee();
