...
my.Wilds = {
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      t.wildList[c] = [];       
      m += '<TR><TD colspan=20><DIV>'+ city.name +' &nbsp; ('+ city.x +','+ city.y +')</div></td></tr>';
      var row = 0;  
      var sortem = [];
...
my.AllianceList = {
  ...
  reDisp : function (){
	...
	exporta += t.dat[i][0]+'?'+addCommasInt(t.dat[i][1])+'?'+t.dat[i][7]+'?'+t.dat[i][5] +'?'+ t.dat[i][6]+';';
			
    }
	
	m +='<TR style="max-height:30px"><TD class=xxtab colspan=8><textarea cols=100 rows=10 onclick=javascript:this.select()>'+exporta+'</textarea></TD></TR>';
