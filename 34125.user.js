// ==UserScript==
// @name           2ch: profit!
// @description    Some tweaks for 2ch.ru imageboard
// @namespace      http://2ch.ru/
// @include        http://2ch.ru/*
// ==/UserScript==
...
var url = location.pathname.substr(1), url_ = url.split('/'), elt, delform = $('delform'),
	host_strings = {name:	{'2ch.ru': 'akane',		'iichan.ru': 'nya1'},
					email:	{'2ch.ru': 'nabiki',	'iichan.ru': 'nya2'},
					topic:	{'2ch.ru': 'kasumi',	'iichan.ru': 'nya3'},
					message:{'2ch.ru': 'shampoo',	'iichan.ru': 'nya4'},
					capurl:	{'2ch.ru': '',			'iichan.ru': '/'},
					gb2:	{'2ch.ru': 'gb2" value="thread', 'iichan.ru': 'postredir" value="1'}};

if (url == '/') { document.body.cols = '230,*'; return; }

// SETUP MENU
if (url.indexOf('/') == -1 && url != 'front.html') return;
...
with (document.body.appendChild(document.createElement('div')))
	style.cssText = 'position:fixed;z-index:99;top:1px;left:1px;background-color:#000;color:#fff;-moz-border-radius:15px;padding:0 20px 25px 20px;display:none',
...
if (config[17]) {
	var storebm = function() {
		...
		if (vis === true) {
			...
			newdiv.innerHTML = '<a href="http://2ch.ru/' + bo + '/res/' + id + '.html">' + id + '</a> (<a href="#" id="' + bo + '_' + id + '">x</a>) - ' + preview;
	...
	for (var i = 0, l = subs1.length; i < l; i++) {
		...
		for (var j = 1; j < subsect.length; j++)  {
			...
			sect += '<div><a href="http://2ch.ru/' + subsect[0] + '/res/' + nfo[0] + '.html">' + nfo[0] + '</a> (<a href="#" id="' + subsect[0] + '_' + nfo[0] + '">x</a>) - ' + unescape(nfo[1]) + '</div>';
		}
