...
// @date           05.05.2012
// @version        0.0.4
// @include		   http://userscripts.org/scripts/show/131744
...
function explode(str,debugName,defaultReturn){
	...
	if(str==undefined){ 
		throw ("Explode error "+debugName+". Nie zdefiniowane."); 
	}
	if(typeof str != "number" && typeof str != "string"){
		throw ("Explode error "+debugName+". Ani text ani cyfra."); 
	}
...
const GAMEPAGES = {"pl":"zieloneimperium.pl"};
var LNG = null;
var SERVER = null;
var PAGE = null;
...
// ********** Multilingual *****************************************************
if (location.hostname.match(GAMEPAGES["pl"])) {
	LNG = "pl";
	delimThou = ",";
	regDelimThou = /,/g;
	delimDeci = ".";
...
case "pl": {
texte["Opcje"] = "Opcje";
texte["StronaScryptu"] = "StronaScryptu";
texte["Dzialaj"] = "Dzialaj";
texte["O %1% gotowe"] = "O %1% gotowe";
texte["Idz do ogrodu %1%"] = "Idz do ogrodu %1%";
// ustawienia
texte["Liczba ogrodow"] = "Liczba ogrodow";
texte["Kretowiska"] = "Kretowiska Nie C..";
texte["Moj bonus Podlewania to"] = "Moj bonus Podlewania to";
texte["Podswietl gdy mam mniej niz ..."] = "Podswietl gdy mam mniej niz ...";
texte["Konto Aktyw.."] = "Konto Aktyw..";
texte["server"] = "Server";
texte["name"] = "Login";
texte["password"] = "Haslo";
texte["Usun"] = "Usun";
texte["Pokaz haslo"] = "Pokaz haslo";
break;}

}
if(LNG==null){ return false; }

GM_registerMenuCommand("Imperium"+" "+"Update", function(){
	location.href=USO_Source;
});
GM_registerMenuCommand("Imperium"+" "+texte["StronaScryptu"], function(){
...
function do_main () {
	...
	function buildInfoPanel(mode){
		if(mode==$("infoPanel").getAttribute("name")){ closeInfoPanel(); }
			...
			switch(mode){
			case "options":{	
				createElement("div",{"align":"center","style":"line-height:30px;font-weight:bold;"},divInfo,texte["Opcje"]);
				...
				createElement("td",{},newtr,texte["Liczba ogrodow"]);
				...
				createElement("td",{},newtr,texte["Kretowiska"]);
				...
				createElement("td",{},newtr,texte["Moj bonus Podlewania to"]);
				...
				createElement("td",{},newtr,texte["Podswietl gdy mam mniej niz ..."]);
				...
				createElement("td",{},newtr,"Numer Pojzdu");
				...
				createElement("td",{},newtr,"Mini Klik");
				...
				createElement("span",{},newtd,"ms");
				createElement("td",{},newtr,"Maxi Klik");
				...
				function buildLoginTable(showPW) {
					...
					createElement("th",{},newtr,texte["password"]);
					var newtd,newinput,newselect,newdiv
					for (var v=0;v<logindata.length;v++){
						newtr = createElement("tr",{},newtable);
						newtd = createElement("td",{},newtr);
						newinput = createElement("input",{"id":"loginActive"+v,"type":"checkbox","title":texte["Konto Aktyw.."],"checked":logindata[v][4]},newtd);
						...
						newtd=createElement("td",{"title":texte["Usun"],"id":"loginDelete"+v},newtr);
				...
				createElement("span",{},newdiv,texte["Pokaz haslo"]);
	...
	var newbutton = createElement("button",{"type":"button","class":"link"},settingsDiv,texte["Opcje"]);
	...
	function showCropReadyTime(){
		...
		$("bedientext").innerHTML = '<font color="blue">'+texte["za %1% gotowe"].replace(/%1%/,zeitstr)+'</font>';
	}
	showCropReadyTime();
...
function do_garten_map(){
	...
	allbtn = createElement("button",{"id":"buttonDzialaj","type":"button","class":"link","style":"position:fixed;top:0px;left:0px;height:20px;"},ALL,texte["Dzialaj"]);
...
try{
	function giess() {
		...
		click($top("giessen"));
		window.setTimeout(function(){
			click(getFrameByName('garten').contentDocument.querySelector("#buttonDzialaj"));
		},1000);
		...
		if(currDoLogin){
			...
			if (GM_getValue("valAutoLogin",false) && (c>0) && (Now-lastbusy>15)){
				...
				if (c==1) {
					...
					$("divInfo").innerHTML = "<h1>"+"Czekaj<br>..."+"</h1>";
					...
					window.setTimeout(function(){
						...
						if (c==-1) {
							...
							$("divInfo").innerHTML = "<h1>"+"Dziala."+"</h1>";
