...
function _gdb_watch_key(e) {
	var el=e.target;
	var button=e.target.ownerDocument.getElementById('_gdb_');
	...
	e.preventBubble();
	_gdb_do_delete(button);
}

function _gdb_do_delete(button) {
	//find container
	var action='tr';
	var cont=button;
...
if ('undefined'==typeof GM_registerMenuCommand) {
	...
	function _gdb_option_text() {
		GM_setValue('gdb_text',
			prompt(
				'Please enter the text to show on the button.\n'+
				'NOTE: Leave blank to use default text\n'+
				'NOTE: You will need to re-load GMail for this to take effect',
				GM_getValue('gdb_text', '')
			)
		);
	}
	GM_registerMenuCommand(
		'Gmail Delete Button Text...',
		_gdb_option_text
	);

	function _gdb_option_key() {
		GM_setValue('gdb_key',
			prompt(
				'Please enter the delete hotkey.\n'+
				'Any one character, default is "d".',
				GM_getValue('gdb_key', 'd')
			)
		);
	}
	GM_registerMenuCommand(
		'Gmail Delete Button Hotkey...',
		_gdb_option_key
	);
}
...
if (window.document.location.href.match(/name=js/)) {
	...
	var customText=GM_getValue('gdb_text', '');
