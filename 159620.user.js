// ==UserScript==
// @name           warian: جابه‌جا کنندهء ساختمان‌ها
// @namespace      warian: جابه‌جا کنندهء ساختمان‌ها
// @description    با داشتن اين اسکريپت مي‌توانيد ساختمان‌هاي مرکز دهکده را آنگونه که مايليد جا‌به‌جا کنيد. 
// @version        1.5.1
// @include        http://*.warian.*/dorf2.php*
...
function reset_vil(){
    if (!window.confirm("آيا مايليد ساختمان‌ها به موقعيت پيشفرض بازگردند؟")) return;
    delete mapping[did];
...
GM_registerMenuCommand("BM: Reset all buildings", function(){
        if (!window.confirm("آيا مايليد ساختمان‌هاي همهء دهکده‌ها به موقعيت پيشفرض بازگردند؟")) return;
        GM_setValue('mapping', '({})');
        location.reload();
    });
...
div.addEventListener('click', function(){
        ...
        truck_stage++;
        div.childNodes[0].title = 'بازگشت ساختمان‌ها به موقعيت اوليه!';
        ...
        for (var i in poly) poly[i].addEventListener('click', function(e){
                ...
                // Error conditions
                if (dest == '21'){
                    notify(' <b>شما نمي‌توانيد <font color=red>اردوگاه</font> را جابه جا کنيد.</b> ');
                    return;
                }
                if (dest == '22'){
                    notify(' <b>شما نمي‌توانيد <font color=red>ديوار</font> را جابه جا کنيد.</b> ');
                    return;
                }
        ...
        notify('<b>بر روي ساختمان اول کليک کنيد...</b>');
    }, false);