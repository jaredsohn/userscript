...
// @include        http://www.forumwarz.com/job_board
...
function prepareJobsTable(){
    ...
    headerRowHTML = headerRowHTML.replace("<th>&nbsp;</th>", "<th><a id='skcc_resetLink' href='' title='Reset the jobs list'>Reset</a></th>");
    headerRowHTML = headerRowHTML.replace("<th>Forum</th>", "<th><a id='skcc_forumLink' href='#' title='Order by forum name'>Forum</a></th>");
    headerRowHTML = headerRowHTML.replace("<th>Description</th>", "<th><a id='skcc_descriptionLink' href='#' title='Order by job description'>Description</a></th>");
    headerRowHTML = headerRowHTML.replace("<th>Difficulty</th>", "<th><a id='skcc_difficultyLink' href='#' title='Order by difficulty'>Difficulty</a></th>");
    headerRowHTML = headerRowHTML.replace("<th>Reward</th>", "<th><a id='skcc_rewardLink' href='#' title='Order by Flezz reward'>Reward</a></th>");
    headerRowHTML = headerRowHTML.replace("<th>JP</th>", "<th><a id='skcc_jobPointsLink' href='#' title='Order by Job Points'>JP</a></th>");
...
function deleteStreakedForums(rows){
    ...
    for(var i in rows){
        var forumName = rows[i].childNodes[4].childNodes[0].innerHTML;
        //Remove spaces
        forumName = forumName.replace(/\s/gi, "");
...
function getOrderedJobs(){
    ...
    while(origRows[i]){
        ...
        difficulty = difficulty.replace("<a href=\"/spoilers/wiki/Forum_Difficulty\"><img alt=\"Difficulty Level ", "");
        ...
        reward = reward.replace("<span class=\"flezz\">", "");
        reward = reward.replace("</span>", "");
