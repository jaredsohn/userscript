...
// @description    	Grooveshark song downloader and ad remover.
// @include        	http://*.grooveshark.com/*
// @include        	http://grooveshark.com/*
// @match        	http://*.grooveshark.com/*
// @match        	http://grooveshark.com/*
// @author         	NoFaTe (http://n0.pointlimit.com)
// ==/UserScript==
...
function GrooveDown() {
	...
	function ready(isReady) { 
		...
		else 
		{
			...
			$.subscribe('gs_player.change', addDLButton);
			
			// Changes the page's title based on the current song
			setInterval(function() {
				if (gs_player.isPlaying)
				{
					var song = gs_player.getCurrentSong();
	...
	function addButton(uid, options) {
		var buttonTag = $('#queue_songs_button', '#playerDetails_queue').clone().attr('id', uid.slice(1)).html('<span>' + options.label + '</span>');

		$(buttonTag).click(options.onclick);
		$('#queue_radio_button', '#playerDetails_queue').after(buttonTag);
    }
