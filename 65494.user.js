// ==UserScript==
// @name Shoutbox warner  Pennergame 4.0 hamburg berlin 4.0
// @namespace http://pennerhack.foren-city.de by basti1012-pennerhack
// @description Sobald ein neuer veitra in der shoutbox geschrieben wird wirst du mit den text gewarnt egal wo du dich gerade in pennergame aufhalten tust 
	...
	GM_xmlhttpRequest({
		...
		onload: function(gangresponseDetails) {
			var gangcontent = gangresponseDetails.responseText;
				shotboxfeld = gangcontent.split('<strong>Shoutbox:</strong>')[1].split('</table>')[0];

scheckfeld = shotboxfeld.split('schrieb am ')[1].split(' </span>')[0];
id = shotboxfeld.split('<a')[1].split('/a>')[0];

idname = id.split('id:')[1].split('/')[0];
idid = id.split('>')[1].split('<')[0];

betreff = shotboxfeld.split('<p>')[1].split('</p>')[0];
