...
function AdaBangsat() {
  for (var i = 0; i < arrayTokohPengganggu.length; ++i) {
    // if ($(document).search(arrayTokohPengganggu[i]) = true) {return true; break;}
    //  else {return false;}
    //return $(document).search(arrayTokohPengganggu[i];
   var testing = toString(arrayTokohPengganggu[i]);
  // alert (arrayTokohPengganggu[i]);
  alert ($('//a[@href]').search('test'));
  }
} 
