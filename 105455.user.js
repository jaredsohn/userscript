...
Tabs.AllianceList = {
		...
		new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/viewCourt.php" + unsafeWindow.g_ajaxsuffix, {
			...
			onSuccess: function (rslt) {
				...
				if (lastLogin)
					...
					notify (rslt);
				//GM_log(inspect(rslt, 10, 1, false));
			},
			onFailure: function (rslt) {
				notify (rslt);
			},
	...
	init : function (div){
		...
		m += '<TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text />&nbsp;<INPUT id=playSubmit type=submit value="Find Player" /></td>';
	...
	gotPlayerDetail : function (rslt, span){
		...
		//GM_log(inspect(rslt.userInfo[0], 10, 1, false));
		var a = 'None';
		if (u.allianceName)
	...
	eventGotAllianceList : function (rslt){
		...
		<TABLE><TR style="font-weight:bold"><TD class=xtab>Alliance Name</td><TD class=xtab>Rank</td><TD class=xtab>Members</td>\
				<TD align=right class=xtab>Might</td><TD class=xtab>Diplomacy</td></tr>';
		for (k in rslt.alliancesMatched){
			...
			m += '</td><TD align=right class=xtab>'+ all.membersCount +'</td><TD align=right class=xtab>'+ addCommasInt(all.might) +'</td><TD class=xtab>'+ dip +'</td></tr>';
		}
		document.getElementById('allListOut').innerHTML = m;
	},
	...
	eventListSubmit : function (){
		...
		new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/allianceGetInfo.php" + unsafeWindow.g_ajaxsuffix, {
			...
			parameters: params,
			onSuccess: function (rslt) {
				t.curPage = 1 + parseInt(rslt.allianceInfo.ranking/10);
	...
	eventGotOtherAlliancePage : function (rslt){
		...
		m += '<th align=right class=xtab>Might</th><th class=xtab>Diplomacy</th></tr></thead><tbody>';
		document.getElementById('allListOut').innerHTML = m;
...
Tabs.Info = {
	...
	init : function (div){
		...
		m += '<BR><DIV class=ptstat>TRAINING ESTIMATES</div><BR /><TABLE align=center cellpadding=1 cellspacing=0>\
		...
		_displayrow ("Marshal Combat Score", infoRows[2]);
		_displayrow ("Geometry Level",       infoRows[3]);
		_displayrow ("Stable Level",         infoRows[4]);
		_displayrow ("Workshop Level",       infoRows[5]);
		m += "<TR><TD nowrap align=center colspan="+(Cities.numCities+1)+"><B>Number trained per hour</B></TD>";
		_displayrow ("Supply Troop",  infoRows[6]);
		...
		var crestname = {
			1101:'Sir Bor&#39;s Crest',
			1102:'Sir Ector&#39;s Crest',
			1103:'Sir Kay&#39;s Crest',
			1104:'Sir Bedivere&#39;s Crest',
			1105:'Sir Gawain&#39;s Crest',
			1106:'Sir Percival&#39;s Crest',
			1107:'Sir Galahad&#39;s Crest',
			1108:'Sir Lancelot&#39;s Crest',
			1109:'King Arthur&#39;s Crest',
			1110:'Morgana&#39;s Seal',
			1111:'Mordred&#39;s Seal',
			1112:'Stag King&#39;s Seal',
			1113:'Pendragon Seal',
			1114:'Lady Of The Lake&#39;s Seal'};
		var crest = {};
		for (k in crestname){
			if(Seed.items['i'+k])
		...
		m += '<DIV class=ptstat>CITY REQUIREMENTS</div><BR /><TABLE align=center cellpadding=1 cellspacing=0>\
			<TR CLASS="xtabH"><TD class=xtab></TH>\
			<TD class=xtabHL><b>Owned/Required 1</b></TD>\
			<TD class=xtabHL><b>Owned/Required 2</b></TD>\
			<TD class=xtabHL><b>Owned/Required 3</b></TD></TR>\
		...
		if (t.state == null){
			numFriends = 0; // Seed.appFriends.length doesn't work, so iterate through :-(
		...
		if (cityCount == 1)
			...
			csty = ' style="color:#060"';
		m += '<TR><TD class=xtab'+csty+'><B>City 2</B></TD>';
		if (parseInt(Seed.player.title) < 7)
			m += '<TD class=xtabL><SPAN class=boldRed>'+Seed.player.title+'</SPAN>/7 Player Level</TD>';
		else
			m += '<TD class=xtabL'+csty+'>'+Seed.player.title+'/7 Player Level</TD>';
		if (numFriends < 10 && cityCount == 1)
			m += '<TD class=xtabL><SPAN class=boldRed>'+numFriends+'</SPAN>/10 Friends</TD>';
		else
			...
			for(u in crestreq[k]) {
				if (k <= cityCount)
					...
					if (crest[u] < crestreq[k][u])
						m+='<TD class=xtabL><SPAN class=boldRed>'+crest[u]+'</SPAN>/'+crestreq[k][u]+' '+crestname[u]+ '</TD>';
					else
						m+='<TD class=xtabL>'+crest[u]+'/'+crestreq[k][u]+' '+crestname[u]+ '</TD>';
				}
			}
			m += '</TR>';
		}
		...
		m += '<BR><DIV class=ptstat>DISTANCE CALCULATOR</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
			...
			<div><b>Calculate ETA with: </b><select id="idETASelect">\
			<option value="0">-- Select --</option>';
			for (ui=1; ui<13; ui++) {
				m += '<option value="' + ui + '">' + unsafeWindow.unitcost['unt'+ui][0] + '</option>';
			}
		m += '</select></div>\
			<DIV style="width:50%; font-size:14px; border: 1px solid; background-color:white; margin:20px 3px 3px 0px; padding:4px" id=ptETAout></div>\
			</CENTER></div><BR>';
		...
		m += '<a href="http://koc.dunno.com/index.sjs?f=KocMapViewer&server_id='+GetServerId()+'">KoC Map Viewer</a>, e.g. right-click and open in a new tab<BR />';
	...
	eventLocChanged : function (city, x, y){
		...
		if (city == null || city.id == undefined) {
			cityId = getCityIdFromXY(x1, y1);
			if (cityId == -1) {
				cityId = getCityIdFromXY(x2, y2);
			}
		} else {
			cityId = city.id;
		}
		...
		m = estimateTroopETA(dist, cityId, document.getElementById('idETASelect').value);
		if (m.ETAstr != null)
...
Tabs.Options = {
	...
	init : function (div){
		...
		try {
			m = '<TABLE class=ptTab>\
				<TR><TD colspan=2><P><B>Config:</b></td></tr>\
				...
				<TR><TD><INPUT id=ptEnableBarbSummary type=checkbox /></TD><TD>Keep <INPUT id=optBarbHitsKeep type=text size=3 value="'+ Options.barbHitsKeep +'"> barb camp hits for the summary in Marches - Barbs.&nbsp;&nbsp;<INPUT id="optResetBarbSummary" type=submit value="Reset Barb Summary"\></td></tr>';
			m += '<TR><TD colspan=2><P><B>Chat Layout:</b> (most of these require a refresh)</td></tr>\
				<TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Enhance Chat.&nbsp;&nbsp;\
				<INPUT id=togChatBold type=checkbox />&nbsp;Bold.&nbsp;&nbsp;<INPUT id=togChatWhisper type=checkbox />&nbsp;Whisper in Red.&nbsp;&nbsp;\
				<INPUT id=togChatAttack type=checkbox />&nbsp;Red background on incoming attack in chat.</td></tr>\
				<TR><TD><INPUT id=togChatGlobal type=checkbox /></td><TD>Global background color.&nbsp;&nbsp;\
				<INPUT id=ptEnableWisperAlert type=checkbox />&nbsp;Sound alert on whisper.&nbsp;&nbsp;<INPUT id=ptEnableTowerAlert type=checkbox />&nbsp;Sound alert on incoming attack in chat.</td></tr>';
			m += '<TR><TD colspan=2><P><B>KoC Features:</b></td></tr>\
				<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Enhance Alliance Reports. Add:&nbsp;&nbsp;\
				<INPUT id=ptEnableReportNumber type=checkbox />&nbsp;Report Number&nbsp;&nbsp;\
				<INPUT id=ptAtkAlliance type=checkbox />&nbsp;Attacker Alliance&nbsp;&nbsp;\
				<INPUT id=ptTgtAlliance type=checkbox />&nbsp;Target Alliance</td></tr>\
				<TR><TD></TD><TD>Attacker City Name: <SELECT id=ptAtkCityName>\
				<OPTION value="no" '+ (Options.atkCityName=='no'?'SELECTED':'')  +'>No</option>\
				<OPTION value="same" '+ (Options.atkCityName=='same'?'SELECTED':'')  +'>On same line</option>\
				<OPTION value="new" '+ (Options.atkCityName=='new'?'SELECTED':'')  +'>On new line</option>\
				</select>&nbsp;&nbsp;&nbsp;Target City Name: <SELECT id=ptTgtCityName>\
				<OPTION value="no" '+ (Options.tgtCityName=='no'?'SELECTED':'')  +'>No</option>\
				<OPTION value="same" '+ (Options.tgtCityName=='same'?'SELECTED':'')  +'>On same line</option>\
				<OPTION value="new" '+ (Options.tgtCityName=='new'?'SELECTED':'')  +'>On new line</option>\
				</select></td></tr>\
				...
				<TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Display number of rounds in battle reports.</td></tr>\
			...
			m += '<TR><TD colspan=2><P><B>KoC Bug Fixes:</b></td></tr>\
				...
				</table><BR /><HR>Note that if a checkbox is greyed out there has probably been a change of KoC\'s code, rendering the option inoperable.';
			t.cont.innerHTML = m;
...
Tabs.Train = {
	...
	init : function (div){
		...
		s = "<DIV id=trainTopSelect>\
			...
			<TR><TD align=right>Limited by:</td><td colspan=2><span id=ptttLimiter class=boldRed></span></TD></TR>\
			...
			<TR><TD colspan=3 align=center><span style='color:grey'>Auto-train up to </SPAN><INPUT id='pttdMaxDefs' size=2 type='text' value='0' DISABLED\><span style='color:grey'> traps, spikes and caltrops<span style='color:grey'></TD></TR>\
			<TR align=center><td align=right>Limited by: </td><TD colspan=2 align=left><span id=pttdLimiter class=boldRed></span></td></tr>\
			...
			<div style='height: 315px; background: #e8ffe8'>\
			<TABLE width=100% class=ptTab><TR><TD colspan=3><DIV id=divSTtop></div></td></tr>\
			<TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Troop Queue (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:190px; max-height:190px'></div></td>\
				<TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Defense Queue (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:190px; max-height:190px'></div></td></tr>\
			</div>";
		t.cont.innerHTML = s;
...
/*******  DEF  ******/
	...
	displayCityStats : function (){
		...
		m += '<TABLE class=ptTab width=100%><TR align=center>\
				<TD width=18%><B>Food:</b><BR>'+ addCommasInt(t.stats.food) +'</td>\
				<TD width=16%><B>Wood:</b><BR>'+ addCommasInt(t.stats.wood) +'</td>\
				<TD width=16%><B>Stone:</b><BR>'+ addCommasInt(t.stats.stone) +'</td>\
				<TD width=16%><B>Ore:</b><BR>'+ addCommasInt(t.stats.ore) +'</td>\
				<TD width=16%><B>Idle Pop/Max:</b><BR>'+ addCommasInt(t.stats.idlePop) +'/'+ addCommasInt(t.stats.maxIdlePop) +'</td></tr></table><BR>';
	...
	show : function (){
		...
		function _row (name, row, noTotal){
			...
			if (noTotal){
				...
				if ((name == 'Food') || (name == 'Wood') || (name == 'Stone') || (name == 'Ore') || (name == 'Gold') || (name == 'Food +/-') || (!Options.includeMarching) || (Options.includeMarching && !Options.includeCityMarch))
					rl = row.length;
				else
					rl = row.length - 1;
				for (i=0; i<rl; i++)
		...
		try {
			if (Options.includeMarching || Options.includeCityMarch)
				march = getMarchInfo();
			...
			if (Options.showTroops) {
				...
				if (Options.includeCityMarch || Options.includeCityRaid) {
					for (r=1; r<13; r++) {
						for(i=0; i<Cities.numCities; i++) {
							if ((march.botMarchStatus == undefined && Options.includeCityMarch) || (march.botMarchStatus != undefined && Options.includeCityRaid))
			...
			if (matTypeof(q) != 'array'){
				for (var m in q){
					if (m != undefined) {
						...
						else if (s.destinationUnixTime > 60)
...
function estimateTroopETA (dist, cityId, troopId) { // Need Relief Station Levels to estimate transport, reinf, or reassign times.
	var ret = {ETAstr: '', hsec: 0, fsec: 0};
	if (dist == 0 || parseInt(troopId) == 0) {
		ret.ETAstr = 'No ETA';
	} else {
	...
	for(var c=0; c<Cities.numCities; c++) { // each city
		...
		for (r in Seed.queue_atkp[cityID]){ // each march
			...
			if (typeof (march) == 'object'){
				if (march.botMarchStatus == undefined) { // Exclude troops that are raiding but include any resources they're bringing back
					for (ii=0; ii<13; ii++){
...
var debugWin = {
	...
	doit : function (){
		...
		if (t.popDebug == null){
//    t.popDebug = new CPopup ('db', 50, 50, 400, 830, true);
