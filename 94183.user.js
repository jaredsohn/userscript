...
//Another fix for the fix late marches.
//Added scout search to HUD.
//Bug fixes and improvments to display of marches.
//Added code to display troop amounts in training tab.
...
// "Report No: 5867445"  --->  see unsafeWindow.modal_alliance_report_view()
  ...
  chatDivContentHook : function (msg){
    var t = ChatStuff; 
    var aclass = '';
    var m = /div aclass='info'>.*<\/div>/im.exec(msg);
    if (m == null)
      return msg;
    if (m[0].indexOf('whispers') >= 0)
      aclass = 'ptChatWhisper';
    else if (m[0].indexOf('to the alliance') >= 0)
      aclass = 'ptChatAlliance'; 
    else  
      aclass = 'ptChatGlobal'; 
    msg = msg.replace ("class='content'", "class='content "+ aclass +"'");
...
return 0;        
	...
	var might='';
        if (t.dat[i][1] > 1000000000)
            might = (t.dat[i][1]/1000000000).toFixed(1) + 'B';
        else if (t.dat[i][1] > 1000000)
            might = (t.dat[i][1]/1000000).toFixed(1) + 'M';
        else if (t.dat[i][1] > 1000)
            might = (t.dat[i][1]/1000).toFixed(1) + 'K';
        else might = addCommasInt(t.dat[i][1]);
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ might 
...
Copied from official scripters. Area52
****/ 
...
if (TEST_WIDE)
  m.push ('X,'); 
        ...
        if (Math.abs(tot) > 1000000000000) {
        	m.push ((tot/1000000000000).toFixed(1) + 'T');
	}else if (Math.abs(tot) > 1000000000) {
        	m.push ((tot/1000000000).toFixed(1) + 'B');
	}else if (Math.abs(tot) > 1000000) {
        	m.push ((tot/1000000).toFixed(1) + 'M');
	}else if (Math.abs(tot) > 1000) {
        	m.push ((tot/1000).toFixed(1) + 'K');
	}else m.push (addCommas(tot));
...
if (TEST_WIDE)
  m.push ('X,');        
        ...
        if (Math.abs(row[i]) > 1000000000000) {
        	m.push ((row[i]/1000000000000).toFixed(1) + 'T');
        }else if (Math.abs(row[i]) > 1000000000) {
        	m.push ((row[i]/1000000000).toFixed(1) + 'B');
	}else if (Math.abs(row[i]) > 1000000) {
        	m.push ((row[i]/1000000).toFixed(1) + 'M');
	}else if (Math.abs(row[i]) > 1000) {
        	m.push ((row[i]/1000).toFixed(1) + 'K');
	}else m.push (addCommas(row[i]));
        m.push ('</td>');
      }
      m.push ('</tr>');
      return m.join('');
...
Tabs.Marches = {
  ...
  init : function (div){
    ...
    unsafeWindow.ptShowWall = t.showWall;
...
***/
  ...
  getMarchResPretty:function(march) {
    var ret = addCommasInt(march['gold']);
    for (ii=1; ii<5; ii++){
        ret += ' | ' + addCommasInt(march['resource'+ ii ]);
    } 
    return ret;
  },
  getScoutedRes:function(report) {//addcommas
    try {
      if (report.winner == 0) 
        return 'NA';

      var ret = '0,'//gold
      ret += parseInt(report.rsc.r1);
      for (var i=2; i<5; i++){
  ...
  getScoutedResPretty:function(report) {//addcommas
    try {
      if (report.winner == 0) 
        return 'NA';

      var ret = '0 | '//gold
      ret += addCommasInt(parseInt(report.rsc.r1));
      for (var i=2; i<5; i++){
        ret += ' | ' + addCommasInt(parseInt(report.rsc['r'+ i ]));
      } 
    }
    catch (e) {
      var ret = 'NA';
    ...
    for(var i=0; i<Cities.numCities; i++) {   
     ...
     //2. return troops ( if reassign must handle differently)
       ...
       if (returnTroopsCount) {
          ...
          for (i=1; i<13; i++) {
              if (parseInt(m[i]) > 0) {
                //logit ("bef returnTroops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+(i+1)]);
                unsafeWindow.seed.units[cityId]['unt'+(i+1)] = parseInt(unsafeWindow.seed.units[cityId]['unt'+(i+1)])+parseInt(m[i-1]);
                //logit ("aft returnTroops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+(i+1)]);
             }
          }
       }
       else {
         if (unsafeWindow.seed.units[cityId]) {
          ...
          for (i=1; i<13; i++) {
              if (parseInt(m[i]) > 0) {
                //logit ("bef troops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+(i+1)]);
                unsafeWindow.seed.units[cityId]['unt'+(i+1)] = parseInt(unsafeWindow.seed.units[cityId]['unt'+(i+1)])+parseInt(m[i-1]);
                //logit ("aft troops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+(i+1)]);
              }
           }
         }
       }
...
logit ("rptHead "+JSON2.stringify(rptHead)+' i: '+i);
            var report = t.scoutingReports[i].report;
            if (rptHead.side0XCoord==march.toXCoord && 
                rptHead.side0YCoord==march.toYCoord &&
                //(Math.abs(parseInt(rptHead.reportUnixTime)-parseInt(march.reportUnixTime)) < 30) &&
                march.marchType == rptHead.marchType)  {// match report, march
               return report;
            }
        }
     }
     logit ("March not matched "+JSON2.stringify(march)+' len: '+t.scoutingReports.length);
     logit ("getScoutingReport returned not found.");
     return result;
  },
  counts:[],
  ...
  showWall : function (data) {
     new CPopupDisplayData ('ptDisplayWall',250,'Wall','wall',data);
  ...
  showMarches : function (){
      ...
      for (var k in Seed.queue_atkp[cityId]){   
        ...
        if (typeof (march) == 'object'){
          ...
          if(march.returnUnixTime) {
            ...
            if (march.marchType==2 && now<march.destinationUnixTime) { // reinf
               eta=march.destinationUnixTime-now;
               etaStr = ((eta<0)?'<td>-'+ timestr(-1*(eta)) +'</td>':'<td>'+ timestr(eta) +'</td>')
            }
          }
          if ( march.marchType == 3 && now>=march.destinationUnixTime){
             //logit ("Scout Found");
          ...
          var lootDisp = t.getMarchResPretty (march);
          ...
          if (loot != '')
            m += '<td nowrap><A onclick="ptShowLoot(['+loot+'])" >'+lootDisp+'</a></td>';
          else
            m += '<td>'+loot+'</td>';
    ...
    //Scouting marches returning table
       m += '<center><table id=idScoutTable ><thead><th>#</th><th>City</th><th>Type</th><th>Target</th><th>Lvl</th><th>Loc</th>\
             <th>LL</th><th>Wall (cb,tr,t,c,sb)<br>Troops<br>Resources</th><th>Ago</th><th>ETA</th><th>Status</th><th>Late</th></thead>\
             <tbody>';
    for(i=0; i<Cities.numCities; i++) {   
      cityId = 'city'+ Cities.cities[i].id;
      if (!Options['MonitorMarchCity_'+i])
        ...
        if (typeof (march) == 'object'){
          ...
          if( t.marchTypeToStr(march.marchType) == 'scout') {
            ...
            //if ( targType=='city') {
              ...
              var resDisp = t.getScoutedResPretty (report);
            ...
            m += '<td><A onclick="ptGotoMap('+march.toXCoord +','+ march.toYCoord+')">'+ march.toXCoord +','+ march.toYCoord +'</a></td>';
           if (status == 'Returning' && report.lstlgn+'' != 'undefined') {
                if ((now-report.lstlgn+'') != 'NaN')  
                ...
                m += '<td>'+'NA'+'</td>';
            //m += '<td>'+wall+'<br>';
            m += '<td><A onclick="ptShowWall(['+wall+'])" >'+wall+'</a>'+'<br>';
                //'+troops+'<br>'+res+'</td>';
            m += '<A onclick="ptShowTroops(['+troops+'])" >'+troops+'</a><br>';
            m += '<A onclick="ptShowLoot(['+res+'])" >'+resDisp+'</a></td>';
            m += '<td>'+timestr(now-march.marchUnixTime,1)+'</td>';
            m += etaStr;
            m += '<td>'+status+'</td><td>'+((late)?'Y':'N')+'</td></tr>'; 
          }
    ...
    if ( count == 0) {
       ...
       table = document.getElementById('idScoutTable');
       if ( table )
          table.innerHTML = '';
    }
...
//march types: 1-transport, 2-reinforce, 3-scout, 4-attack, 5-reassign
       if (march.marchType==3) {
          ...
          logit ("A SCOUTING report: "+JSON2.stringify({'rptHead':rptHead,'report':rslt}));
       }

       //logit ("The report: "+JSON2.stringify({'rptHead':rptHead,'report':rslt}));
...
function CPopupDisplayData (Id,Height,Heading,DataType, Data) {
  ...
  switch (DataType) {
    ...
    case 'wall':
      ...
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_53_30.jpg"></td>';
      m += '<TD>Crossbows</td>';
      m += '<TD>'+addCommasInt(Data[0])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_55_30.jpg"></td>';
      m += '<TD>Trebuchets</td>';
      m += '<TD>'+addCommasInt(Data[1])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_60_30.jpg"></td>';
      m += '<TD>Traps</td>';
      m += '<TD>'+addCommasInt(Data[2])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_61_30.jpg"></td>';
      m += '<TD>Caltrops</td>';
      m += '<TD>'+addCommasInt(Data[3])+'</td></tr>';
      
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_62_30.jpg"></td>';
      m += '<TD>Spikes</td>';
      m += '<TD>'+addCommasInt(Data[4])+'</td></tr>';
    ...
    case 'LeaderBoard':
     ...
     for (var i=0; i<3; i++){
      ...
      m += '<TR align=left ><TH></th><TH><Center>City</center></th><TH><Center>Level</center></th></tr>';
...
//if (DEBUG_TRACE) logitno (" 0 myAjaxRequest: "+ url +"\n" + inspect (o, 2, 1));
...
//if (DEBUG_TRACE == true) logitno (" 1a myAjaxRequest: "+ url +"\n" + inspect (o, 2, 1));
  ...
  function myFailure(){
    var o = {};
//if (DEBUG_TRACE) logitno ("myAjaxRequest.myFailure(): "+ inspect(rslt, 1, 1));
    o.ok = false;
  ...
  function mySuccess (msg){
    ...
    var x;
    if (rslt.ok){
//if (DEBUG_TRACE) logitno (" 1b myAjaxRequest.mySuccess(): "+ inspect(rslt, 1, 1));
      rslt.errorMsg = null;   ///// !!!!!!!!!!!!!  ************
      if (rslt.updateSeed)
        unsafeWindow.update_seed(rslt.updateSeed);
      wasSuccess (rslt);
...
//if (DEBUG_TRACE) logitno (" 1b myAjaxRequest.mySuccess() !ok : "+ inspect(rslt, 3, 1));
...
//if (DEBUG_TRACE) logitno (" 1d myAjaxRequest.mySuccess() else");
      wasSuccess (rslt);
    }
  }
}
...
logit ( "Leaderboard: "+JSON2.stringify(rslt));
    //logit("new CPopupDisplayData ('ptLeaderBoard',400,'LeaderBoard','LeaderBoard', '');");

    var an = p.allianceName;
    if (!an || an=='' || an == null || an == 'null')
      an = 'none';

    if ( p.allianceId == 0)
       an = 'none';
   
    if (p.officerType != null && p.officerType < 4)
      an += ' ('+ officerId2String(p.officerType) +')';
    data[j++] = '<TR><TD align=right>&nbsp; </td><TD align=right>Name: &nbsp; </td><TD>'+p.displayName+'</td></tr>';
    data[j++] = '<TR><TD align=right>&nbsp; </td><TD align=right>Might: &nbsp; </td><TD>'+p.might+'</td><TD></td></tr>';
    data[j++] = '<TR><TD align=right>&nbsp; </td><TD align=right>Alliance: &nbsp; </td><TD>'+an+'</td><TD></td></tr>';

    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Might: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
	data[j++] = '<TR><TD align=right></td><TD nowrap> &nbsp; '+ c.cityName 
      +' (<a onclick="ptGotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp;  <Center>'
        + c.tileLevel +'</center></td></tr>';
    }  
    
	logit ( "Data: "+JSON2.stringify(data));
    var pop = new CPopupDisplayData ('ptLeaderBoard',400,'LeaderBoard','LeaderBoard', data);
 },
  ...
  srchTypeHandler : function (){
    ...
    if (t.opt.searchType == 1){
      ...
      m += '<TR><TD class=xtab align=right>DarkForest<INPUT id=dfWild type=CHECKBOX '+ (Options.dfWild?' CHECKED':'') +'></td>\
           <TD</td></tr>';
    ...
    document.getElementById('dfWild').addEventListener ('change', function(){
        Options.dfWild = document.getElementById('dfWild').checked;
        saveOptions();
        t.dispMapTable();
        }, false);
  ...
  clickedSearch : function (){
    ...
    m = '<DIV class=ptstat><TABLE width=100% cellspacing=0><TR><TD class=xtab width=125><DIV id=statSearched></div></td>\
	  ...
	  <TR><TD colspan="2"><DIV id=divOutTab style="width:100%; height:430px; max-height:470px; overflow-y:auto;"></div></td></tr></table>';
...
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Barb Camp', 'Grassland', 'Lake', 'Woodlands', 'Hills', 'Mountain', 'Plain', 'City', 'Dark Forest' ];
    var t = Tabs.Search;
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A style="font-size:9px;" >Details</a></div> <DIV style="" onclick="PTpl(this, '+ dat[i][7] +')"><A style="font-size:9px;">Leaderboard</a></div>';
...
Tabs.Hud = {
  ...
  init : function (div){
    ...
    unsafeWindow.ptShowTroops2 = t.showTroops;
    unsafeWindow.ptShowLoot2 = t.showLoot;
    unsafeWindow.ptShowWall2 = t.showWall;
    ...
    t.cont.innerHTML = ' \
        ...
        <DIV class=ptstat>Search battle reports for scouting reports.</div>\
        <DIV class=ptentry style="height:30px"><table>\
        <tr><td class=xtab> Pages:\
        <span id=idSpanNumPages2>1</span>\
        <select id="idHudSelect2">\
        <option value=1> -- Select -- </option>\
        <option value=1> 1 </option>\
        <option value=5> 5 </option>\
        <option value=10> 10 </option>\
        ...
        <TD class=xtab><INPUT id=idHudSearch2 type=submit value="Start Search" />\
        <TD class=xtab><INPUT id=idHudFetchBR type=submit value="Get Reports" disabled="disabled" />\
        <span id=idSpanHudErrorMsg2></span></td></tr>\
        </table></div>\
        ...
        document.getElementById('idHudSearch2').addEventListener ('click', t.handleHudBattleSearch, false);
        document.getElementById('idHudSelect2').addEventListener ('click', t.handleHudBattleSelect, false);
        document.getElementById('idHudFetchBR').addEventListener ('click', t.fetchReports, false);
...
//march types: 1-transport, 2-reinforce, 3-scout, 4-attack, 5-reassign
  ...
  getScoutedResPretty:function(report) {
    try {
      if (report.winner == 0) 
        return 'NA';
      var ret = '0 | '//gold
  ...
  showLoot : function (data) {
     new CPopupDisplayData ('ptDisplayLoot',250,'Resources','loot',data);
  },
  showWall : function (data) {
     new CPopupDisplayData ('ptDisplayWall',250,'Wall','wall',data);
  },

  DisplayReports : function (searchType){
    var t = Tabs.Hud;
    var data = t.data;
    ...
    if (searchType == 2) {//Searching battle reports for my scouting reports.
      var m = '<center><table><thead><th>Page#</th><th>When</th><th>Act</th><th>Target</th><th>Loc</th></thead>';
      m += '<tbody>';
      for ( var i=0; i<t.data.length;i++) {
         var rpt = data[i];
         if (rpt.side0Name=='undefined') 
            continue;
         //m += '<tr><td><SPAN onclick="ptBattleReports('+rpt.page+')"> <a>'+rpt.page+'</a></span></td>';
         m += '<tr><td>'+rpt.page+'</td>';//Navigate TBD
      ...
      m += '</tbody></table></center>';
      document.getElementById("idHudFetchBR").disabled = '';
    }
    if (searchType == 3) {//Display scouting repors.
        //logit ( "DisplayReports (3): 1: "+t.reportsList.length);
        var m = '';//'Display reports TBD: '+t.reportsList.length;
        m += '<center><table><thead><th>#</th><th>When</th><th>Target</th><th>Loc</th>\
                  <th>Last<br>Login</th><th>Wall (xbow,treb,trap,caltrop,spike)<br>Troops<br>Resources (F,W,S,O)</th><th>Fields</th></thead>\
                  <tbody>';
        var now = unixTime();
        for (var i=0; i<t.reportsList.length; i++) {
            //logit ( "DisplayReports (3): 2a: "+(i+1));
            var rpt = t.reportsList[i].report;
            //logit ( "DisplayReports (3): 2a2: "+JSON2.stringify(rpt));
            var rptHead = t.reportsList[i].rptHead;
            //logit ( "DisplayReports (3): 2a2: "+(i+1));
            if ( rpt == 'undefined' || rptHead == 'undefined' )
                continue;
            try {
              var troops = t.getScoutedTroops (rpt);
              //logit ( "DisplayReports (3): 2a3: "+(i+1));
              var res = t.getScoutedRes (rpt);
              var resDisp = t.getScoutedResPretty (rpt);
              //logit ( "DisplayReports (3): 2a4: "+(i+1));
              var wall = t.getScoutedWall (rpt);

              //logit ( "DisplayReports (3): 2b: "+(i+1));

              m += '<tr><td>' + (i+1) + '</td>';
              m += '<td>'+unsafeWindow.formatDateByUnixTime(rptHead.reportUnixTime)+'</td>';
              //m += '<td>'+ t.marchTypeToStr(rptHead.marchType) +'</td>';
              var side0Name = rptHead.side0Name+'';
              if (side0Name!='undefined')
                m += '<td>'+ rptHead.side0Name +'</td>';
              else {
              //logit ( "DisplayReports (3): 2a2: "+JSON2.stringify(rptHead.side0TileType));
                if (rptHead.side0TileType == 51)
                    m += '<td>Camp</td>';
                else
                    m += '<td>Wild</td>';
              }
              var link = '<a onclick="ptGotoMap ('+ rptHead.side0XCoord +',' +rptHead.side0YCoord+ ')">'+ rptHead.side0XCoord +','+ rptHead.side0YCoord +'</a>';
              m += '<td>'+ link +'</td>';
              //m += '<td>'+ rptHead.side0XCoord +','+ rptHead.side0YCoord +'</td>';
 
              //logit ( "DisplayReports (3): 2b2: "+(i+1)+' Last Login, '+rpt.lstlgn);
              var lstlgn =  rpt.lstlgn+'';
              if ( lstlgn!='undefined') { 
                //logit ( "DisplayReports (3): 2b3a: "+(i+1)+' Last Login, '+lstlgn + ' time: '+(now-parseInt(rpt.lstlgn)));
                if ( (now-parseInt(rpt.lstlgn)+'') != 'NaN') {
                   //logit ( "DisplayReports (3): 2b3b: "+(i+1)+' Last Login, '+lstlgn + ' time: '+(now-parseInt(rpt.lstlgn)));
                   m += '<td>'+timestr(now-rpt.lstlgn,1)+'</td>';
                }
                else
                    m += '<td></td>';
              }
                  ...
                  m += '<td></td>';
              //logit ( "DisplayReports (3): 2c: "+(i+1));
              m += '<td><A onclick="ptShowWall2(['+wall+'])" >'+wall+'</a>'+'<br>';
              m += '<A onclick="ptShowTroops2(['+troops+'])" >'+troops+'</a>';
              m += '<br><A onclick="ptShowLoot2(['+res+'])" >'+resDisp+'</a></td>';
              //logit ( "DisplayReports (3): 2b4: "+JSON2.stringify(rpt));
              m += '<td>'+'TBD'+'</td>';
             m += '</tr>';
            }
            catch (e) {
              //logit ( "DisplayReports (3): 3c: "+(i+1));
              //logit ( "DisplayReports (3): 3c2: "+JSON2.stringify(rpt));
              //logit ( "DisplayReports (3): 3c3: "+JSON2.stringify(rptHead));
              //logit ( "DisplayReports (3): 3c4: "+rpt.lstlgn);
              //logit ( "DisplayReports (3): 3c5: "+troops);
              //logit ( "DisplayReports (3): 3c6: "+res);
              //logit ( "DisplayReports (3): 3c7: "+wall);
              //logit ( "DisplayReports (3): 3c8: "+rptHead.side0Name);
            }
        }
        m += '</tbody></table></center>';
        document.getElementById("idHudFetchBR").disabled = 'disabled';
       //logit ( "DisplayReports (3): 3");
   }
    results.innerHTML = m;
  },
  ...
  handleHudBattleSelect : function() {
    var t = Tabs.Hud;
    t.maxPages=document.getElementById("idHudSelect2").value;
    if ( t.maxPages==99999)
       t.maxPages=t.totalPages2;
    document.getElementById("idSpanNumPages2").innerHTML = t.maxPages+'';
  },
  handleHudBattleSearchCB : function(rslt, page) {
    var t = Tabs.Hud;
    if (rslt) {
       if (!rslt.ok) {
          document.getElementById("idSpanHudErrorMsg2").innerHTML = 'Error searching page: ' + page + ', ' + rslt.errorMsg;
          t.DisplayReports(2);
          return;
       }
       t.totalPages2=rslt.totalPages;
       if (rslt.arReports && page) {
         var ar = rslt.arReports;
         var rptkeys = unsafeWindow.Object.keys(ar);
         var myAllianceId = getMyAlliance()[0];
         for (var i = 0; i < rptkeys.length; i++) {
              ...
              if (rpt.marchType == 1)
                  ...
                  rpt.marchName = 'Reinf';
              else if (rpt.marchType == 5)
                  rpt.marchName = 'Reassign';
              else if (rpt.marchType == 4)
                  rpt.marchName = 'Attacked';
              else rpt.marchName = 'unknown';
              if (myAllianceId == rpt.side1AllianceId && rpt.marchType == 3 || rpt.marchType == 11) {
                 logit ("handleHudBattleSearchCB - push rpt: "+JSON2.stringify(rpt));
                 t.data.push(rpt);
              }
         }
       }
       if (parseInt(page)+1 <= t.maxPages) {
          var results=document.getElementById("hudResultsDiv");
          results.innerHTML = '<center><b>...Searching '+(parseInt(page)+1)+'...</b></center>';
          setTimeout (function(){Tabs.Hud.getBattleReports(parseInt(page)+1);},7000);
       }
       else if (page) 
           t.DisplayReports(2);
       else {
            ...
            <option value=1> -- Select -- </option>\
            <option value=1> 1 </option>';
            m += ((t.totalPages2>=5)?'<option value=5> 5 </option>':'');
            m += ((t.totalPages2>=10)?'<option value=10> 10 </option>':'');
            m += ((t.totalPages2>=20)?'<option value=20> 20 </option>':'');
            m += ((t.totalPages2>=30)?'<option value=30> 30 </option>':'');
            m += ((t.totalPages2>=40)?'<option value=50> 40 </option>':'');
            m += ((t.totalPages2>=50)?'<option value=50> 50 </option>':'');
            m += ((t.totalPages2>=60)?'<option value=60> 60 </option>':'');
            m += ((t.totalPages2>=70)?'<option value=70> 70 </option>':'');
            m += ((t.totalPages2>=80)?'<option value=80> 80 </option>':'');
            m += ((t.totalPages2>=90)?'<option value=90> 90 </option>':'');
            m += ((t.totalPages2>=100)?'<option value=100> 100 </option>':'');
            m += '<option value=99999> All </option>';
            document.getElementById("idHudSelect2").innerHTML = m;
       }
    }
  },
  ...
  handleHudBattleSearch : function() {
    var t = Tabs.Hud;
    var results=document.getElementById("hudResultsDiv");
    //logit("handleHudSearch");
    t.maxPages=document.getElementById("idHudSelect2").value;
  ...
  getBattleReports : function (pageNum){
    ...
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/listReports.php" + unsafeWindow.g_ajaxsuffix, {
      ...
      parameters: params,
      onSuccess: function (rslt) {
          Tabs.Hud.handleHudBattleSearchCB (rslt, pageNum);     
      },
      onFailure: function (rslt) {
          Tabs.Hud.handleHudBattleSearchCB (rslt, pageNum);     
      },
  ...
  fetchReports : function () {
    ...
    t.rptList = [];
    t.reportsList = [];
    //logit ( "fetchReports - 1");
    for ( var i = 0; i<t.data.length; i++) {
        t.rptList.push(t.data[i]);
    }
    //logit ( "fetchReports - 2");
    if (t.rptList.length) {
       t.rptHead = t.rptList.shift();
       //logit ( "fetchReports - 3 - rptHead: "+JSON2.stringify(t.rptHead));
       document.getElementById("idHudFetchBR").disabled = 'disabled';
       var results=document.getElementById("hudResultsDiv");
       results.innerHTML = '<center><b>...Fetching '+t.rptList.length+' reports...</b></center>';
       setTimeout (function(){Tabs.Hud.fetchReport(t.rptHead.reportId);},1000);
    }
  },

  fetchReportCB : function (rptNum, rslt){
    var t = Tabs.Hud;
    //logit ( "fetchReportCB - 0");
    //if (!rslt)
       //return;
    if (rptNum < 0) {
       return;
    ...
    //   return;
    //}
    //logit ( "fetchReportCB - 1");

    t.reportsList.push( {'rptHead':t.rptHead,'report':rslt});
    //logit ( "fetchReportCB - 2");

    if (t.rptList.length) {
       //logit ( "fetchReportCB - 3");
       t.rptHead = t.rptList.shift();
       //logit ( "fetchReportCB - 3b - rptHead: "+JSON2.stringify(t.rptHead));
      setTimeout (function(){Tabs.Hud.fetchReport(t.rptHead.reportId);},1500);
    }
    else {
       //logit ( "fetchReportCB - 4");
       t.DisplayReports(3);
    }
  },
  ...
  fetchReport : function (reportID){
    var t = Tabs.Hud;
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    //logit ( "fetchReport - 1: "+reportID);
    var results=document.getElementById("hudResultsDiv");
    results.innerHTML = '<center><b>...Fetching report '+reportID+'...</b></center>';
    ...
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/fetchReport.php" + unsafeWindow.g_ajaxsuffix, {
      ...
      parameters: params,
      onSuccess: function (rslt) {
        //logit ( "fetchReport - 2: "+reportID);
        t.fetchReportCB(reportID, rslt);
      },
      onFailure: function (rslt) {
       //logit ( "fetchReport - 3: "+reportID);
       t.fetchReportCB(-1, rslt);
      },
    });
  }, 
