  ...
  _highlightTable: function(target,equip) {
    ...
    function checkPreffix(td,preffix) {
      return td.indexOf(preffix) == 0 ||
        (/Surtr Fiery|Mjolnir Shocking|Freyr Tempestuous|Niflheim Arctic/.test(preffix) && /Match/i.test(td))||
		(/Fiery|Shocking|Tempestuous|Arctic|Demonic|Hallowed/.test(preffix) && /^Prefix$/i.test(td))||
        (/Demonic/.test(preffix) && /Dark/i.test(td))||
        (/Hallowed/.test(preffix) && /Holy/i.test(td))||
	...
	//alert(suffix)
    for (var i=0;i<n;i++) {
      var rowspan = rows[i].firstElementChild.getAttribute('rowspan')||1, temp = false, temp2 = null; premarch = false;
      for (var j=0;j<rowspan;j++, i++) {
       ...
       // alert(suffixTDtext)
		if (checkSuffix(suffixTDtext,suffix.match(/\S+/))) {
          temp = true;
          highlightRow(rows[i]);
