// ==UserScript==
// @name			VIPHF Pack
// @namespace		circl/vhfup
// @description		Utility pack that adds many features to VIPHackForums.
// @require			http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js
// @include			*viphackforums.net*
// @grant 			GM_info
// @grant 			GM_addStyle
// @grant 			GM_setValue
// @grant 			GM_getValue
...
function update_hfup() {
	...
	$.each(update_settings, function(name, value) {
		console.log(name + ':' + value);
		localStorage.setItem(name, value); //loop through defaul
	});
...
function userProfileTagging() {
	// localStorage.setItem('profileTags', '{"175033":"HFUP Developer"}')
	var profileTags = JSON.parse(hfup_settings['profileTags']);
...
function quickCharacterCounter() {
	...
	$('#message').bind('input propertychange', function() {
		if($('#message').val().length >= 36 && $('#message').val().length <= 18000) {
...
var installed_version = parseFloat(localStorage.getItem("vhfup_version"));
var installed = GM_getValue("vhfup_installed");
vhfup_settings = {};

if(!installed) {
	console.log('[VHFUP] Installing.');
	install_vhfup();
} else if(installed_version < script_version) {
	console.log('[VHFUP] Updating.');
	update_vhfup();
} else {
	console.log('[VHFUP] Loading settings.');
	load_settings();
}

/* This is ran before DOM is ready */
console.log('[VHFUP] Running before DOM functions.');
if(vhfup_settings['removeLogo']) { removeLogo(); }
if(vhfup_settings['betterPMs']) {
...
function mainContentLoaded() {
	console.log('[VHFUP] Running after DOM functions.');
	/* Global alterations */
	if(JSON.parse(vhfup_settings['topLinks']).length) { topLinks(); }

	pageTitle = location.href;
	if(pageTitle.contains("showthread.php")) {
		if(vhfup_settings['profileTagging']) { threadProfileTagging(); }
		if(vhfup_settings['multiQuote']) { multiQuote(); }
		if(vhfup_settings['postLinkToActivity']) { postLinkToActivity(); }
		if(vhfup_settings['threadToBottom']) { threadToBottom(); }
		if(vhfup_settings['threadRatings']) { showthreadRatings(); }
		if(vhfup_settings['characterCounter']) { quickCharacterCounter(); }
		if(vhfup_settings['presetPost'].length) { quickPresetPost(); }
	} else if(pageTitle.contains("newreply.php")) {
		//if(vhfup_settings['characterCounter']) { fullCharacterCounter(); }
	} else if(pageTitle.contains("private.php") && pageTitle.contains("action=read")) {
		if(vhfup_settings['profileTagging']) { threadProfileTagging(); }
		if(vhfup_settings['quickReplyPM']) { quickReplyPM(); }
		if(vhfup_settings['betterPMs']) { betterPMs(); }
	} else if(pageTitle.contains("private.php")) {
		if(vhfup_settings['pmCountdownTimer']) { pmCountdownTimer(); }
		...
		if($("td span strong a:contains('Post')").html().length == 4) {
			if(vhfup_settings['searchAddNewPosts']) { searchAddNewPosts(); }
		}
		if(vhfup_settings['previewThread']) { previewThread(); }
	} else if(pageTitle.contains("forumdisplay.php")) {
		if(vhfup_settings['enhancedSYT']) { enhancedSYT(); }
		if(vhfup_settings['threadRatings']) { forumDisplayRatings(); }
		if(vhfup_settings['previewThread']) { previewThread(); }
	} else if(pageTitle.contains("bans.php")) {
		if(vhfup_settings['countStatistics']) { countBannedMembers(); }
	} else if(pageTitle.contains("negreplog.php")) {
		if(vhfup_settings['countStatistics']) { negRepLog(); }
	} else if(pageTitle.contains("reputation.php")) {
		if(vhfup_settings['clickableRepLinks']) { clickableRepLinks(); }
	} else if(pageTitle.contains("paidstickies.php")) {
		if(vhfup_settings['countStatistics']) { countPaidStickies(); }
	} else if(pageTitle.contains("showgroups.php")) {
		if(vhfup_settings['countStatistics']) { countGroups(); }
	} else if(pageTitle.contains("showstaff.php")) {
		if(vhfup_settings['countStatistics']) { countStaffAndAdmins(); }
	} else if(pageTitle.contains("showmods.php")) {
		if(vhfup_settings['countStatistics']) { countForumMods(); }
	} else if(pageTitle.contains("misc.php")) {
		//if(vhfup_settings['']) { run_misc(); }
	} else if(pageTitle.contains("member.php") && pageTitle.contains("action=profile")) {
		if(vhfup_settings['getProfileBan']) { getProfileBan(); }
		if(vhfup_settings['profileTagging']) { userProfileTagging(); }
	} else if(pageTitle.contains("myawards.php")) {
		//if(vhfup_settings['']) { run_myawards(); }
	} else if(pageTitle.contains("index.php") || document.title == "Vip Hack Forums") {
		if(vhfup_settings['indexAddNewPosts']) { indexAddNewPosts(); }
	} else if(pageTitle.contains("usercp.php") && pageTitle.contains("action=options")) {
		settings_page();
	} else if(pageTitle.contains("usercp.php")) {
		//if(vhfup_settings['']) { run_usercp(); }
	} else {
		//if(vhfup_settings['']) { run_generic(); }
	}
	
	if(vhfup_settings['forceUnknownLocation']) { forceUnknownLocation(); } //Do this last just in case other page loads add to status
	
	console.log('[VHFUP] All done!');
}
