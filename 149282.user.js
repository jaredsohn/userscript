// ==UserScript==
// @name           GLBSurvivorHelper
// @namespace      	glb.warriorgeneral.com
// @include        http://glb.warriorgeneral.com/game/league_survivor.pl?league_id=*
// ==/UserScript==
...
var season		= 23; // what season number are we in?
var url = window.location.host;
var teamId = new Array;
var survivorGames = new Array;
...
function runPowerRankings(){
	...
	var location = document.getElementById('footer');
	if(!location){
		return;
	}
	...
	form = 	'<hr />' +
		...
		'<p>' +
			...
			'</span>' +
			'<input type="radio" class="talent" name="talent"value="1" checked>Yes </input>' +
			'<input type="radio" class="talent" name="talent" value="0">No </input>' +
		'</p>' +
		...
		'<p>' +
			...
			'</span>' +
			'<input type="radio" class="forumformat" name="forumformat" value="1">Yes </input>' +
			'<input type="radio" class="forumformat" name="forumformat" value="0" checked>No </input>' +
		'</p>' +		
		'<p>' +
...
function gatherData(userInput) {
	...
	// get team ids
	var leagueURL = document.location;
	var regexp = /league_survivor.pl\?league_id=(\d+)/img;
	var league_id = regexp.exec(leagueURL)[1];
	var params = [
			"league_id=", league_id
	];
	GM_xmlhttpRequest( {
		method :"POST",
		url :"http://glb.warriorgeneral.com/game/league.pl?league_id="
				+ league_id,
		data :params.join(""),
		headers : {
		...
		onload : function(response) {
			...
			var location = doc.getElementById('conferences');
			j=0;
			for (i=1; i<34; i++) {
				if (i == 17)
					i++;
				...
				if (league_id == 280) { // WORLD LEAGUE
					teamPage = location.getElementsByTagName('tr')[i].childNodes[5].firstChild.href.split('=', 2);
					teamName = location.getElementsByTagName('tr')[i].childNodes[5].firstChild.innerHTML;
				} else {
					teamName = location.getElementsByTagName('tr')[i].childNodes[3].firstChild.innerHTML;
				}
				//teamId[j] = teamPage[1];
				storeTeamId(teamPage[1],j);
				document.getElementById('rankingDataTable').getElementsByTagName('tr')[j].id = teamPage[1];
				document.getElementById(teamPage[1]).getElementsByTagName('td')[0].innerHTML = teamName;
				document.getElementById(teamPage[1]).getElementsByTagName('td')[0].setAttribute('class', 'completed');
				j++;
			}
			
			// gets team/matchup data from dropdown list	
			var j=0;	
			i=1;
			do {
				teamPage = document.getElementsByName('my_pick')[0].getElementsByTagName('option')[i].value.split('_');
				teamName = document.getElementsByName('my_pick')[0].getElementsByTagName('option')[i].childNodes[0].textContent.split(' over ');
				
				//var teamPage = document.getElementByName('conferences').getElementsByTagName('tr')[i].childNodes[3].firstChild.href.split('=', 2);
				...
				exists = 0;
				for(k=0;k<survivorGames.length;k++) {
					if(survivorGames[k][1]==teamPage[1]) {
						exists=1;
					}
				}
				survivorGames[i-1]=new Array;
				
				if(exists==0) {
					//survivorGames[i-1][0]=teamPage[1];
					for(k in teamId) {
						if(teamId[k]==teamPage[1]) {
							survivorGames[i-1][0]=k;
							k = teamId.length;
						}
					}
					j++;
					...
					//survivorGames[i-1][1]=teamPage[2];
					for(k in teamId) {
						if(teamId[k]==teamPage[2]) {
							survivorGames[i-1][1]=k;
							k = teamId.length;
						}
					}
					j++;
					...
					// exists, so must be the same teams, switched
					survivorGames[i-1][0]=survivorGames[i-2][1];
					survivorGames[i-1][1]=survivorGames[i-2][0];
				}
				
				i++;
			} while (document.getElementsByName('my_pick')[0].getElementsByTagName('option')[i] != undefined);
...
function calculateResults(userInput){
	...
	fullLocation = document.getElementById('rankingDataTable').getElementsByTagName('tr');
	
	//for(i in teamId) {
	for(i in teamId) { 
		results[i] = new Array();
		
		results[i][0] = teamId[i]; //Team Id	
		location = fullLocation[i].getElementsByTagName('td');
...
function output_a_prediction(resultLine1, resultLine2, userInput,i) {
	...
	expected1 = expected1.toFixed(0);
	expected2 = expected2.toFixed(0);
	// output the predicted score
	if (userInput[14] && expected1 < 100)
		html += '&amp;nbsp;&amp;nbsp;'
	if (userInput[14] && expected1 < 10)
		html += ' &amp;nbsp;'
	html += expected1 + ' - ';
	if (userInput[14] && expected2 < 100)
		html += '&amp;nbsp;&amp;nbsp;'
	if (userInput[14] && expected2 < 10)
		html += ' &amp;nbsp;'
	html += expected2;
	if (userInput[14])
		html += '&amp;nbsp; &amp;nbsp;';
	html += '</td>';
	html += '<td>' + resultLine1[1] + " (" + resultLine1[26] + ")";
	if (userInput[18])
		html += ' (' + getRecord(userInput[0],resultLine1) + ')';
	html += ' vs ';
	html += resultLine2[1] + " (" + resultLine2[26] + ")";
	if (userInput[18])
		html += ' (' + getRecord(userInput[0],resultLine2) + ')';
	html += '</td><td> = ' + (expected1 - expected2) + '</tr></table>';
	survivorGames[i][2] = expected1 - expected2;
	survivorGames[i][3] = html;
...
function outputMatchups(userInput, results, games) {
	...
	location = document.getElementById('rankings');
	location.innerHTML += html;
	for(i in survivorGames) {
		var team1 = survivorGames[i][0];
		var team2 = survivorGames[i][1];
		output_a_prediction(results[team1],results[team2],userInput,i);
	}
	
	
	scorehtml = new Array;
	keyArray = new Array;
	//while(survivorGames[i] != undefined) {
	for(i in survivorGames) {
		sorter = "";
		if(survivorGames[i][2]<0) {
			survivorGames[i][2] = -(999+survivorGames[i][2]);
			sorter = "-";
		}
		if(survivorGames[i][2]<10)
			sorter =  sorter + "0";	
		if(survivorGames[i][2]<100)
			sorter =  sorter + "0";			
		sorter =  sorter + survivorGames[i][2] + "_" + [i];
		keyArray.push(sorter);
		scorehtml[sorter] = survivorGames[i][3];
		i++;
	}
	
	keyArray.sort();
	for(i in keyArray) {
	  location.innerHTML += scorehtml[keyArray[i]];
	}
	
	
	
	
	//while(teamId[i] != undefined) {
	//	// next game
	//	var j = results[i][6]+1;
	//	if(games[i][j][3] != '') { // 3 is opponent if the game hasn't been played yet
	//		k=0;
	//		while(results[k] != undefined) {
...
function updateProgress(userInput){
	...
	var denom = teamId.length*(78 - 3*(4-userInput[12]));
	document.getElementById('progress').innerHTML = 'Getting Rankings: ' + parseInt((progress.length/denom)*100) + '% ' + progress.length;
	if(teamId.length >= 1 && progress.length == denom){ 
...
function getHistoricalRecord(teamId, season, userInput){
	GM_xmlhttpRequest({ 
			method: 'GET',
			url: 'http://glb.warriorgeneral.com/game/team.pl?season=' + season + '&team_id=' + teamId,
			headers: {
			    'User-agent': navigator.userAgent,
			    'Accept': 'text/xml'
			},
				...
				if(userInput[0] == 1){					
					...
					for (var i=0; i<userInput[12]; i++){
						...
						if (gameId != ''){ 		
							...
							temptag= temp.firstChild.tagName.toUpperCase();
							if(temptag == 'I' || temptag == 'B'){
...
function storeTeamId(teamPage,j) {
	teamId[j]=teamPage;
}