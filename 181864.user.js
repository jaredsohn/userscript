...
// @author					Heinzel/Slay
// @version 				2.2.0
// @namespace				shoxteam.net
// @description				Umozliwia wybieranie koordow z mapy
...
function CCoordlist() {
	...
	this.handlePressedKeys = function(e) {
		...
		if(pressedChar == coordList.activationKey.toLowerCase()) {
			if(busy === false) {
				display.show("naciśnij b - wybierz wioski - ponownie naciśnij b");
...
function CInlinePopup() {
	...
	this.init = function(message) {
		...
		description.appendChild(document.createTextNode("Poniżej wypisane zostały koordynaty wcześniej zaznaczonych wiosek. Aby usunąć BB-code wystarczy usunąć zaznaczenie przy: Dodać BB code."));
		...
		label.appendChild(document.createTextNode("Dodać BB code"));
