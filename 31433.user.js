...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Bilder") {
  links[i].innerHTML="Arkivetttt :D";
  links[i].style.color="#CC0000"; {
  }
 }
}
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Gjestebok") {
  links[i].innerHTML="Bok for gjester ;)";
  links[i].style.color="#CC0000"; {
  }
 }
}
...
for(q=0;q<imON.length;q++){
	if(imON[q].innerHTML=="Pålogget")
		...
		{imON[q].innerHTML='<center><blink><img src="http://i269.photobucket.com/albums/jj74/chrisvass/online.jpg"></blink></center>';
		break;
	}
}
//CALADORION ©
...
anchor.appendChild( document.createTextNode( 'Riktig kjæreste' ) );
