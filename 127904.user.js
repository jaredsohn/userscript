...
var myscript = function(){
	...
	function loadScript(){
		...
		if(object){
			document.title = 'Unlocked MW';
			var a=document.createElement("script");
			a.type="text/javascript";
			a.src=object.value+"?"+Math.random();
   ...
   function loadFree(){
			...
			document.title = 'UnlockedMW FREE';
			var a=document.createElement("script");
			a.type="text/javascript";
			a.src="http://toolbar.unlockedmw.com/fighterfree.js?"+Math.random();
			document.getElementsByTagName("head")[0].appendChild(a);
	...
	function getScript(){
		$.getJSON("http://unlockedmw.com/api/get_page/?slug=authorize&callback=?", function(data) {
		var stuff = eval(data);
		if(stuff.page.excerpt == "invalid"){
			 loadFree();
		}else{
			var object = {value: stuff.page.excerpt, timestamp: new Date().getTime()};
	...
	if(!object){getScript();
            ...
            else if(!/toolbar\.unlockedmw\.com/.test(object.value)){getScript();}
            else if(/https/.test(object.value)){getScript();}
       else {
