...
// @version        0.10.1
//
// @history        0.10.1 Better fixed previous bug (Narulez)
// @history        0.10 Fixed possible bug if data is preloaded and then modified (Narulez)
// @history        0.09 Added name value in tabs, so you can display a name different from the label (Narulez)
...
Config = {
	...
	show: function (callback) {
		...
		if (typeof (Config.styleDrawn) == 'undefined') {				// apply styling
			...
			if (typeof (Config.css) != 'undefined') { GM_addStyle(Config.css); } // apply user specified styles if set
