// ==UserScript==
// @name           [-GF-] Leecher Checker
// @version        1.0
// @namespace      WHSK_LCHCHKR
// @description    Hỗ trợ việc đôn đốc đóng góp gỗ nâng mỏ.
// @include        http://s*.ikariam.*/*
// @exclude        http://board.ikariam.*/*
// ==/UserScript==
// Original script built by Leecher Checker v 1.2.3
// New script built by Joshua Pack
// Branching version with Vietnamese Translation and code modifications by SnOoOpy
...
*	0-90	Đóng góp chưa đủ số lượng, ,màu hiển thị = đỏ
*	90-100	Đóng góp gần đủ, màu hiển thị = xanh nước biển
*	100-110	Đã đóng góp đủ số lượng, màu hiển thị = da cam
*	110 - ??	Đã đóng góp vượt chỉ tiêu, màu hiển thị = xanh lá cây
*/
...
GM_xmlhttpRequest({
  ...
  onload:function(details) {
	...
	is_it_updated = "<a href='"+updatesite+"'><b>Phiên bản đã cũ, bạn cần phải nâng cấp bản mới!</b></a>"; 
	updatecolor = "RED";
	} else {
	is_it_updated = "Phiên bản hiện hành"; 
	updatecolor = "GREEN";
	}
	} else {
	is_it_updated = "<a href='"+updatesite+"'>Không nhận dạng được phiên bản hiện hành</a>"; 
	updatecolor = "GREY";
	}
	var footer_update = document.getElementById('resourceUsers').childNodes[1];
	footer_update.innerHTML += " (<font color='"+updatecolor+"'>Đang cập nhật "+is_it_updated+"</font>)";
  }
});
...
try
{
	...
	if ( (page == 'tradegood') || (page == 'resource') )
	{
		...
		for (var j = 1; j < donationList.rows.length; j+=cities)
		{
			...
			if (sort_all_mills==2) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"tooltip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>888已經捐獻 "+Comma(Math.round(ordered_towns[j][6]))+" gỗ  .</span></a>";
			}
			if (sort_all_mills==1) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"tooltip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>Mỗi thành phố đã đóng góp "+Comma(Math.round(ordered_towns[j][7]))+" gỗ  .</span></a>";
			}
			var woodneed100 = "<font color=\"orange\">Để đạt 100% cần góp thêm "+Comma(Math.round(needed_wood))+" gỗ  .</font><br/>";
			var woodneed110 = "<font color=\"green\">Để đạt 110% cần góp thêm "+Comma(Math.round(needed_wood110))+" gỗ  .</font>";
			if (needed_wood < 1) {woodneed100 = "";}
			if (needed_wood110 < 1) {
			var over100percent = Math.round(needed_wood110)*-1;
			woodneed110 = "Đóng vượt chỉ tiêu "+Comma(over100percent)+" gỗ  .";
			}
			...
			if (thisElement.elements[0].value == 'Options')
			{
				...
				div.innerHTML = 
					"<div id='leecher_checker'>"+
                                            "<h3>[-GF-] Leecher Checker v"+lversion+" Options</h3>"+
						"<table cellpadding='0' cellspacing='0'>"+
							"<tr>"+ 
								"<td align='center' colspan='2'><i>Vật Liệu Xây Dựng</i></td>"+
							"</tr>"+	
							"<tr>"+
					            "<th>Luật đóng góp:</th>"+
								"<td>"+
									"<input id='radio_1'  type='radio' class='radio' name='method_building' value='1' "+breafious_building+"/> Bình thường<br />"+
									"<input id='radio_2'  type='radio' class='radio' name='method_building' value='2' "+sbreafious_building+" /> Nghiêm ngặt<br />"+
							...
							"</tr>"+
							"<tr>"+ 
								"<td align='center' colspan='2'><i>Tài Nguyên Xa Xỉ<i></td>"+
							"</tr>"+
							"<tr>"+
								"<th>Luật Đóng Góp:</th>"+
								"<td>"+
									"<input id='radio_4' type='radio' class='radio' name='method_luxury' value='1' "+breafious_luxury+"/> Bình thường<br />"+
									"<input id='radio_5' type='radio' class='radio' name='method_luxury' value='2' "+sbreafious_luxury+" /> Nghiêm ngặt<br />"+
							...
							"</tr>"+
							"<tr>"+ 
								"<td align='center' colspan='2'><i>Tùy chọn khác<i></td>"+
							"</tr>"+
							"<tr>"+
								"<th>Chế độ sửa lỗi:</th>"+
								"<td>Bật <input id='radio_7' type='radio' class='radio' name='debug_mode' value='1' "+debug_mode_on+"/> Tắt  <input id='radio_8' type='radio' class='radio' name='debug_mode' value='0' "+debug_mode_off+"/></td>"+
							"</tr>"+
							"<tr>"+ 
								"<td align='center' colspan='2'><i>Sắp Xếp<i></td>"+
							"</tr>"+
							"<tr>"+
								"<th>Sắp xếp theo lượng đóng góp từng town:</th>"+
								"<td><input id='radio_9_0' type='radio' class='radio' name='sort_all_mills' value='2' "+sort_all_mills_on2+"/> Bật </td>"+
							"</tr>"+
							"<tr>"+
								"<th>Sắp xếp theo tổng lượng đóng góp</th>"+
								"<td><input id='radio_9' type='radio' class='radio' name='sort_all_mills' value='1' "+sort_all_mills_on+"/> Bật </td>"+
							"</tr>"+
							"<tr>"+
								"<th>Tắt chế độ sắp xếp:</th>"+
								"<td><input id='radio_10' type='radio' class='radio' name='sort_all_mills' value='0' "+sort_all_mills_off+"/> Tắt </td>"+
							"</tr>"+
				        "</table>"+
				    "</div>";
				...
				var debug_mode 	= GM_getValue('debug_mode','0');
				if (debug_mode == 1) {
				alert("[-GF-] Leecher Checker v"+lversion+"\n nếu trong quá trình sử dụng có lỗi, bạn hãy báo ngay cho SnOoOpy, Green Fields Clan ....\n\n\n" + er)
				}
				}
},
    true);
