...
var Vill = {
	...
	List : [],
	Init : function(){
		vArea = Doc.Element("vlist");
		if (vArea){
			vLinks = vArea.getElementsByTagName("a");
			vXs = vArea.getElementsByClassName("cox");
			vYs = vArea.getElementsByClassName("coy");
			for (var i = 1 ; i < vLinks.length ; i++){
				this.List[i-1] = [vLinks[i].innerHTML,vLinks[i].href.split("?")[1],
													vXs[i-1].innerHTML.split("(")[1],vYs[i-1].innerHTML.split(")")[0]];
			}
		}else{
			dorfData = decodeURIComponent(GM_getValue(dataIndex+"_dorf",false));
			if (dorfData != 'false' && dorfData.length > 1){
				dd = dorfData.split("$");
				this.List = [[dd[0],dd[1],dd[2],dd[3]],];			
			}else {
				...
				GM_xmlhttpRequest({
					...
					onload: function(responseDetails) {
						...
						vArea = document.getElementById("villages");
						vLinks1 = document.getElementsByClassName("vil fc");
						vLinks = vLinks1[0].getElementsByTagName("a")[0];
						vXs = vArea.getElementsByClassName("cox");
						vYs = vArea.getElementsByClassName("coy");
						this.List = [[vLinks.innerHTML,vLinks.href.split("?")[1],
													vXs[0].innerHTML.split("(")[1],vYs[0].innerHTML.split(")")[0]]];
						vstr = this.List[0][0] + "$" + this.List[0][1] + "$" + this.List[0][2] + "$" + this.List[0][3];
	...
	getUserId : function(){
		navi = Doc.Element("side_navi");
		navi_p = navi.getElementsByTagName("p")[0];
		profile_link = navi_p.getElementsByTagName("a")[2];
...
var Time = {
	Sync : function(x,y,x1,y1,last_sent,farm_id,trps,interval){
		...
		var hhh = Math.round(300*Math.random())+300;
		hhh = parseInt(hhh);
		msec = parseInt((msec * 2)/interval+hhh);
		mode = 'def1';
		dsec = nsec - lsec;
...
var Farms = {
	...
	List : function(){
		...
		FarmDiv = Doc.New("DIV",[['id','bfarms'],['style','float:right;margin: 20px;']]);
		var strFarms = decodeURIComponent(GM_getValue(dataIndex,false));
		if (strFarms != 'false' && strFarms.length > 1) {
			aFarms = new Array();
			sFarms = strFarms.split("$$");
			...
			TotalIndexT = 1;
			if (crtPage.match(/karte.php/)){
				did = crtPage.split('&')[0].split('=')[1];
				sXY = Doc.id2xy(parseInt(did))
				tmpFarms = [];
				tmpFarmsIndex = 0;
				for (var i = 0 ; i < sFarms.length ; i++){
...
var Action = {
	New : function(){
		...
		if (crtPage.match(/karte.php/)){
			did = crtPage.split('&')[0].split('=')[1];
			sXY = Doc.id2xy(parseInt(did))
			Doc.Element("NewFarmX").value = sXY[0];
			Doc.Element("NewFarmY").value = sXY[1];
			h1 = document.getElementsByTagName("h1")[0];
			vName = h1.getElementsByTagName("div")[0].innerHTML;
...
}else if (crtPage.match(/karte.php/)){
	dataURL = crtPage.split("?")[1];
	
	if (dataURL && dataURL.split("")[0] == 'd'){
		Vill.Race();
		Vill.Init();
		Farms.List();
	}else {
