...
// @include		http://*theancientwars.com/stats.php*
// ==/UserScript==
...
(function(){
	...
	function ShowPreferences(){
		...
		if(!prefs){
			...
			var s = "<div><b>The ancient wars :: User Stats Logs</b><br />";
...
GM_registerMenuCommand("TAW User Stats Prefs...", ShowPreferences);
...
if(GM_getValue('_userStatsToggle') == 1){
	...
	while(offset <= end){
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://www.theancientwars.com/intel.php?b_start='+offset+'&o_start='+offset,
			headers: {
			...
			onload: function(responseDetails) {
				...
				for(var j = 0; j < intelTemp.length; j++){
					if(intelTemp[j].indexOf(userName) != -1){
						...
						GM_xmlhttpRequest({
							method: 'GET',
							url: 'http://www.theancientwars.com/' + temp,
							headers: {
								'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
								'Accept': 'application/atom+xml,application/xml,text/xml',
							},
	...
	while(offset <= end){
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://www.theancientwars.com/attacklog.php?b_start='+offset+'&o_start='+offset,
			headers: {
				'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
				'Accept': 'application/atom+xml,application/xml,text/xml',
			},
	...
	while(offset <= end){
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://www.theancientwars.com/inbox.php?b_start='+offset+'&o_start='+offset,
			headers: {
				'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
				'Accept': 'application/atom+xml,application/xml,text/xml',
			},
