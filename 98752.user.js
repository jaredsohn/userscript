...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('Flooger BOT', eventHideShow, mouseMainTab);
  kocWatchdog ();
  WideScreen.init ();
...
//	logit(div);
        ...
        var m = '<DIV id=pbTowrtDivF class=ptstat>KULE HAREKETLERI</div><TABLE id=pbtowerfunctions width=100% height=0% class=pbTab><TR>';
        if (t.alertState.running == false) {
            m += '<TD><INPUT id=pbAlertState type=submit value="Alarm = Kapali"></td>';
       }
        else {
            m += '<TD><INPUT id=pbAlertState type=submit value="Alarm = ACIK"></td>';
        }
        m += '</tr></table></div>';
        m += '<DIV id=pbAlertDivD class=ptstat>SEHIR SAVUNMALARI</div><TABLE id=pbalertdetails width=100% height=0% class=ptentry><TR>';
		...
		for (var i = 0; i < Cities.cities.length; i++) {
		    if (parseInt(Seed.citystats["city" + Cities.cities[i].id].gate) == 0) {
				 m += '<TD><CENTER><INPUT id=pbsanctuary_' + Cities.cities[i].id + ' type=submit value="def = Kapali" style="border:1px solid black; background-color:#0a0;"></center></td>';
			}
			if (parseInt(Seed.citystats["city" + Cities.cities[i].id].gate) == 1) {
				 m += '<TD><CENTER><INPUT id=pbsanctuary_' + Cities.cities[i].id + ' type=submit value="def = Acik" style="border:1px solid black; background-color:red;"></center></td>';
			}
        }
        m += '</tr><TR>';
        m += '</tr></table></div>';
...
// George will fix this when tower tab is refactored for Power Tools update      
    ...
    toggleAlertState: function(obj){
        ...
        if (t.alertState.running == true) {
            ...
            t.saveAlertState();
            obj.value = "Alarm = Kapali";
        }
        else {
            t.alertState.running = true;
            t.saveAlertState();
            obj.value = "Alarm = Acik";
        }
    },
	show: function(){
		var t = Tabs.tower;
