// ==UserScript==
// @name           Plunderverkauf V2.1 B
// @namespace      http://forum.ego-shooters.net
// @description    Plunderverkauf erweitert den standart Plunderverkaufsbereich um die Funktion mehrere Plunder zu verkaufen.
// @include        http://*pennergame.de/stock/plunder/*
// @exclude        http://berlin.pennergame.de/stock/plunder/craft/*
// ==/UserScript==
...
function plunder_function(x, tr_plunder){
	...
	if(plunder_daten[0] != null && plunder_daten[1] != null && plunder_beschr[0] != null){
		tr_plunder[x].getElementsByTagName('td')[1].innerHTML += '<ul>'+plunder_daten[0].innerHTML+plunder_daten[1].innerHTML+'</ul><br /><div style="overflow: auto; width: 100px;">'+plunder_beschr[0].innerHTML+'</div>';
		...
		tr_plunder[x].getElementsByTagName('td')[1].innerHTML += '<ul>'+plunder_daten[0].innerHTML+'</ul><br /><div style="overflow: auto; width: 100px;">'+plunder_beschr[0].innerHTML+'</div>';
	}else if(plunder_daten[0] == null && plunder_daten[1] == null && plunder_beschr[0] != null){
		tr_plunder[x].getElementsByTagName('td')[1].innerHTML += '<br /><div style="overflow: auto; width: 100px;">'+plunder_beschr[0].innerHTML+'</div>';
	}else if(plunder_daten[0] != null && plunder_daten[1] == null && plunder_beschr[0] == null){
...
function sell_plunder(plunder_id, anzahl, anzahl_plunder, stueck){
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://berlin.pennergame.de/stock/plunder/sell/'+ plunder_id +'/',
...
plunder_table_start();
//Script von NewMan im Penergame unter NewMan01 zu finden. Ab und an auch unter NewMan01_2 wenn PG seinen Acc gesperrt hat ;).