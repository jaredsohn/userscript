// ==UserScript==
// @name          Kaskus Quick Reply.co.id
// @namespace     http://userscripts.org/scripts/show/134276
// @include       http://www.kaskus.co.id/showthread.php?*
// @version       10.1.3
// @dtversion     110521318
// @timestamp     1305932549494
// @description   QR FOR KASKUS.CO.ID
// @author        idx
...
const OPTIONS_BOX = {
 ...
 ,KEY_SAVE_LAYOUT_SIGI:      [''] // signature layout, eg. [RIGHT][IMG]http://i1081.photobucket.com/albums/j344/Adhie_Sebul/addemokecil.png[/IMG]&#8482;[/RIGHT]
...
function getSettings(){
  ...
  */
  ...
  if(!hVal) hVal = ['', '[RIGHT][IMG]http://i1081.photobucket.com/albums/j344/Adhie_Sebul/addemokecil.png[/IMG]&#8482;[/RIGHT]'];
  gvar.settings.userLayout.signature = decodeURIComponent(hVal[1]).replace(/\\([\!\:])/g, "$1");
  
  hVal=getValueForId(gvar.user.id, 'LAYOUT_TPL', ['<!>','::']);
  if(!hVal) hVal = ['', '[FONT="Berlin Sans FB"][COLOR="RoyalBlue"]{message}[/COLOR][/FONT]'];  
  ...
  if(isUndefined(reply_html)){ // is there ret from XHR :: reply_html
    ...
    var parse_ajax_post = function(html){
        ...
        if(html.indexOf('POSTERROR')!=-1){ // there's some error          
            ...
            r.msg = 'This Thread may be removed by moderator';
        }
...
,'907': [H+'fd_2.gif', ':cd:', 'Cape deeehh']
 ...
 ,getTPL_prompt_reCAPTCHA: function(){
  return (''
     ...
     +  '<a href="http://'+'userscripts.org/scripts/show/134276" target="_blank" title="Info, Tips, Suggestion, Digitalize">HOME</a>&nbsp;&#8212;'
