...
// @require        http://userscripts.org/scripts/source/172400.user.js
// @require        http://userscripts.org/scripts/source/172396.user.js
// ==/UserScript==

function Refresh2(){
	if (GM_getValue("SWBotMonya",false)==true){
...
function AddDivElement(html){
	sLastAtack = '<div id="SWBotLastDay" style="height:400px;width:200px;position:fixed;top:120px;right:0px;z-index:2;margin:0 auto;background-color:#e0ffff;border: 1px red solid;">'+html+'</div>';
...
function AddToNextAction(strText){
	AddTextToDiv('SWBotNextAction',strText);
}
...
function MainScript(jQuery){
	...
	GM_setValue("SWBotLastUpdate",curUpdateTime);
	...
	function TaxiModeSubmit(){
		...
		GM_setValue("SWBotRideCount",parseInt(GM_getValue("SWBotRideCount",""))+1);
	...
	function ProcedPetricks(){
		...
		if (hrefPage.substr(0,9)=="/factory/"){
			...
			GM_setValue("SWBotPetricks",curUpdateTime+timerFactory);
			if (timerFactory==0){
		...
		if ((curUpdateTime>=(GM_getValue("SWBotPetricks",0)+110)) && (intMoney>700) && (intOre>=5)) {
	...
	function ProcedThimble(hrefPage,intMoneyStart){
		GM_setValue("SWBotMonya",false);
		if (hrefPage.substr(0,9)=="/thimble/"){
			GM_setValue("SWBotMonya",true);
	...
	//GM_setValue("SWBotDayStorage","");
	function ReCalcDayStats(){
		var storageData = GM_getValue("SWBotDayStorage","");
	//	alert(GM_getValue("SWBotDayStorage",""));
		var storageArray = [];
		if (storageData!="")
			storageArray = storageData.split(';');
		var i=0,j=0;
		
		var outDate = OutTime(new Date((GM_getValue("SWBotLastUpdate",0)-86400)*1000));
		var outDate2 = OutTime(new Date((GM_getValue("SWBotLastUpdate",0)-82800)*1000));
		...
		GM_setValue("SWBotDayStorage",resultData);
		GM_setValue("SWBotCountAtack",storageArray.length);
		...
		//SetupTextByID("SWBotLastDay",dayStats);
		//alert($("#SWBotLastDay").html(	));
		AddDivElement(dayStats);
		GM_setValue("SWBotCountMoney",moneyCount);
		GM_setValue("SWBotReachCount",reachCount);
		GM_setValue("SWBotCountMoneyDay",moneyCountDay);
		GM_setValue("SWBotCountAtackDay",countCountDay);
		GM_setValue("SWBotCountMoneyDay2",moneyCountDay2);
		GM_setValue("SWBotCountAtackDay2",countCountDay2);
		GM_setValue("SWBotPoorCount",poorCount);
	}
	ReCalcDayStats();
	...
	//alert(GM_getValue("SWBotRideCount",""));
	if (GM_getValue("SWBotRideCount","")>=5){
		if (hrefPage == '/automobile/car/7937/'){
			...
			if (fuelCount==0){
				...
				GM_setValue("SWBotRideCount",5-fuelCount);
	...
	if (hrefPage == '/huntclub/'){
		...
		GM_setValue("SWBotHuntCount",countText);
		GM_setValue("SWBotHuntCountUpdate",curUpdateTime);
	}
...
/*	 if (hrefPage == '/arbat/'){
		...
		GM_setValue("SWBotTaxiEndTime",taxiEndTime);
		...
		var taxiEndTimeSaved = GM_getValue("SWBotTaxiEndTime",0);
	...
	if ((curUpdateTime - GM_getValue("SWBotHuntCountUpdate",curUpdateTime))>3600){
		setTimeout(GoToHuntclub,10000);
	}
	var sportLotoLastView = new Date((GM_getValue("SWBotSportLotoLastView",curUpdateTime))*1000);
	...
	if (hrefPage == '/casino/sportloto/'){
		...
		GM_setValue("SWBotSportLotoLastView",curUpdateTime);
		setRandTimeout(GoToAlley,3000);
		return;
	}
	if (globalUpdateDateDay!=sportLotoLastViewDay){
		GM_setValue("SWBotHuntCount",27);
...
/*		var new_win = window.open('', 'разное','scrollbars=yes,menubar=no,top=0,left=0,toolbar=no,width=820,height=600,resizable=yes');
		var strWindow = (GM_getValue("SWBotDayStorage","").replace(new RegExp(";",'g'),'</td></tr><tr><td>'));
	...
	//alert(GM_getValue("SWBotCountAtackDay2",0));
	...
	if (hrefPage == '/alley/'){
		GM_setValue("SWBotPatrolAvaible",0);
		if (document.documentElement.innerHTML.indexOf('<option value="10">10 минут</option>') >= 0 ){
			GM_setValue("SWBotPatrolAvaible",1);
		}
		if (document.documentElement.innerHTML.indexOf('Улизнуть с патрулирования') >= 0){
			GM_setValue("SWBotPatrolAvaible",1);
		}
	}
	if ((hrefPage == '/alley/')&&(hrefTimeOut == '/alley/')&&(intMoney>10)&&(intMoney<9000)&&(GM_getValue("SWBotCountAtackDay2",0)<=500)){
		if (document.documentElement.innerHTML.indexOf('<option value="10">10 минут</option>') >= 0){
			GM_setValue("SWBotPatrolAvaible",1);
...
/*		if (GM_getValue("SWBotCountAtackDay2",0)>=140){
			if (ProcedThimble(hrefPage,9000)) return;
		}else{*/
			var LastUpdateHour = (new Date(parseInt(GM_getValue("SWBotLastUpdate",0))*1000)).getHours();
			if (LastUpdateHour<8){
	...
	if (hrefPage.substr(0,13)=='/alley/fight/'){
		...
		if (GM_getValue("SWBotHntrs",false)==true){
			GM_setValue("SWBotHuntCount",parseInt(GM_getValue("SWBotHuntCount",0))-1);
			GM_setValue("SWBotHunterClubLastTryCount",10);
			GM_setValue("SWBotHunterClubLast",'');
		}
	...
	//	GM_setValue("SWBotHunterClub","");
	if (hrefPage.substr(0,8) == '/player/') {
		//Lives = GetLives(document.documentElement.innerHTML,document.location.href);
		setRandTimeout(GoToAlley,5000);
	}
	if ((GM_getValue("SWBotHntrs",false)==true)&&(parseInt(GM_getValue("SWBotHuntCount",0))>0)){
		AddToNextAction('Last target: '+GM_getValue("SWBotHunterClubLast",''));
		AddToNextAction('Last count: '+GM_getValue("SWBotHunterClubLastTryCount",''));
		if (hrefPage.substr(0,8) == '/player/') {
			//alert(parseInt(GM_getValue("SWBotHunterClubLastTryCount",0))<=10);
			if (hrefPage == '/player/'+GM_getValue("SWBotHunterClubLast",'')+'/'){
				//curLives = GetLives(response.responseText,response.finalUrl);
				if (parseInt(GM_getValue("SWBotHunterClubLastTryCount",0))<=5){
					GM_setValue("SWBotHunterClubLastTryCount",parseInt(GM_getValue("SWBotHunterClubLastTryCount",0))+1);
				//alert("retry");
					unsafeWindow.alleyAttack(GM_getValue("SWBotHunterClubLast",''));
					return;				
				}
			}
		}	
		GM_setValue("SWBotHunterClubLastTryCount",0);
		...
		if (hrefPage.substr(0,17) == '/huntclub/wanted/'){
			...
			var hunterData = GM_getValue("SWBotHunterClub","");
			...
			while (str_table.indexOf('<td class="name">')>=0){
				...
				if (hunterPlayers[indexDef]!= undefined){
					if (parseInt(hunterPlayers[indexDef])<3000){
						GM_setValue("SWBotHunterClubLastCurLives",hunterPlayers2[indexDef]);
						GM_setValue("SWBotHunterClubLast",indexDef);
				...
				GM_xmlhttpRequest({
					...
					onload: function(response) {	
						...
						var st = lives[2];
						if (GM_getValue("SWBotHunterClub","")==""){
							GM_setValue("SWBotHunterClub", lives[2]+','+lives[1]);
						}else{
							GM_setValue("SWBotHunterClub", GM_getValue("SWBotHunterClub","")+';'+lives[2]+','+lives[1]+','+lives[0]);
						}
						AddToNextAction('Add info: '+lives[2]+', lives: '+parseInt(lives[1]));
						if (parseInt(lives[1])>3000) return;
						GM_setValue("SWBotHunterClubLast",lives[2]);
						GM_setValue("SWBotHunterClubLastCurLives",lives[0]);
	...
	if (GM_getValue("SWBotOn",false)==true){
		if((intMoney>700)&&(intMoney<6000)&&(GM_getValue("SWBotCountAtackDay2",0)>120)){
		//if((GM_getValue("SWBotCountAtackDay2",0)>30)){
	...
	if (hrefTimeOut=='/alley/'){
		...
		GM_setValue("SWBotHunterClubLastTryCount",0);
	//	StartPatrol();
		//alert(GM_getValue("SWBotHunterClubLastTryCount",0));
	}
	if (curUpdateTime<(GM_getValue("SWBotPetricks")-110)) {
		var outDate = OutOnlyTime(new Date(GM_getValue("SWBotPetricks",0)*1000));
		AddToNextAction('Ожидание таймера (петрики) - '+outDate);
		setRandTimeout(GoToAlley,(GM_getValue("SWBotPetricks",0)-curUpdateTime+121)*1000);
	}	
	
	if ((hrefPage=='/shop/section/other/')&&(GM_getValue("SWBotKirkaCount",0)<=0)){
		if (intMoney>1500){
...
/*	if (hrefTimeOut=='/metro/'){
		...
		if ((hrefPage=="/alley/search/level/")||(hrefPage=="/alley/search/again/")){
			...
			if (Fights.length>=2){
				...
				if (Avarage<200){
					...
					if (GM_getValue("SWBotSkiped","")==""){
						GM_setValue("SWBotSkiped",targetID);
					}else{
						GM_setValue("SWBotSkiped",GM_getValue("SWBotSkiped","")+';'+targetID);
					}
					return;
				}
			}
	...
	if (hrefTimeOut=='/metro/'){
		if ((GM_getValue("SWBotKirkaCount",0)<=0)&&(hrefPage=='/metro/')){
	...
	if ((GM_getValue("SWBotMetroDig",false)==true)&&(parseInt(GM_getValue("SWBotKirkaCount",0))>0)&&(intMoney<9000)&&(parseInt(curUpdateTime)-parseInt(GM_getValue("SWBotRatLastMeet",0))>46800)&&(GM_getValue("SWBotPatrolAvaible",1)==0)){
		if (hrefPage=="/metro/"){
			...
			if(TakeSubString(document.body.innerHTML,'<span id="ratlevel">',"</span>")!=""){
				...
				GM_setValue("SWBotRatCount",(parseInt(GM_getValue("SWBotRatCount",0))+1));
				GM_setValue("SWBotRatLastMeet",curUpdateTime);
				return;
			}
		}
		...
		if ((hrefTimeOut=='/alley/')||((hrefTimeOut=='//')&&(GM_getValue("wigFarm",false)!=true))){
			if ((hrefPage=="/metro/")||(GM_getValue("wigFarm",false)!=true)){
				GM_setValue("SWBotKirkaCount",(parseInt(GM_getValue("SWBotKirkaCount",0))-1));
	...
	if ((GM_getValue("SWBotMetroDig",false)==true)&&(GM_getValue("wigFarm",false)==true)){
	//	SWBotMetroDig,wigFarm
	}
	if (hrefPage=="/metro/"){
		setRandTimeout(GoToAlley,1000);
		return;
	...
	if (hrefPage=="/player/"){
		...
		GM_setValue("SWBotKirkaCount",count);
		if (count==0){
...
var sLastAtack = '<div id="SWBotLastDayInner" style="height:300px;width:100%;margin:0 auto;background-color:#409999;border: 0px black solid;text-align: center;color:#FFFFFF;">Последние 24 часа</div>';
sLastAtack = '<div id="SWBotLastDay" style="height:300px;width:200px;position:fixed;top:0;left:852px;z-index:2;margin:0 auto;background-color:#e0ffff;border: 1px black solid;">'+sLastAtack+'</div>';
