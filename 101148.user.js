// ==UserScript==
// @name           Ikariam Супер маркет
// @namespace      http://userscripts.org/scripts/show/67519
// @description    Показывает команды покупки и продажи ресурсов на одной странице и позволяет переключаться между ресурсами без перезагрузки страницы.
...
//
// @history      0.05 Switched script namespace.  Fixed a small bug in quickly updating viewed offers.
...
ScriptUpdater.check(67519, '0.05')
...
IkariamSuperMarketOffers = function(cityId, position, resource, range, orderBy, parentNode) {
  this.buyHeader = $('<h3 class="header"><span class="textLabel">Купить</span></h3>');
  this.buyOffersDiv = $('<div class="ISM_offers_div ISM_loading content"></div>');
  this.buyFooter = $('<div class="footer"/>');
  this.sellHeader = $('<h3 class="header"><span class="textLabel">Продать</span></h3>');
...
IkariamSuperMarketOffers.prototype._startLoadingOffer = function(loadInfo, delay) {
  ...
  if (loadInfo.timerId) {
  	debug("Cancelling time: " + loadInfo.timerId);
    window.clearTimeout(loadInfo.timerId);
  }
  
  loadInfo.node.html('Загрузка...');
...
IkariamSuperMarket.prototype.prepareScreen = function() {
  ...
  var wineSelector = $('<li id="ISM_wine_selector" class="ISM_selector selected"><a title="Вино" href="javascript:void(0);"><em><img src="/skin/resources/icon_wine.gif" width="12px" height="10px"/> Вино</em></a></div>');
  var marbleSelector = $('<li id="ISM_marble_selector" class="ISM_selector"><a title="Мрамор" href="javascript:void(0);"><em><img src="/skin/resources/icon_marble.gif" width="12px" height="10px"/> Мрамор</em></a></div>');
  var crystalSelector = $('<li id="ISM_crystal_selector" class="ISM_selector"><a title="Хрусталь" href="javascript:void(0);"><em><img src="/skin/resources/icon_crystal.gif" width="12px" height="10px"/> Хрусталь</em></a></div>');
  var sulphurSelector = $('<li id="ISM_sulphur_selector" class="ISM_selector"><a title="Сера" href="javascript:void(0);"><em><img src="/skin/resources/icon_sulfur.gif" width="12px" height="10px"/> Сера</em></a></div>');
  var woodSelector = $('<li id="ISM_wood_selector" class="ISM_selector"><a title="Дерево" href="javascript:void(0);"><em><img src="/skin/resources/icon_wood.gif" width="12px" height="10px"/> Дерево</em></a></div>');
