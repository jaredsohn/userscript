...
// disabled @require        http://usocheckup.dune.net/index.php?scriptid=76950

// disable fastLoad if the computer is slow or many games are running at the same time.
var fastLoad=true;
var debug=true;

// update script from: http://userscripts.org/scripts/review/76950

var SUC_script_num = 76950; 
...
Fight:function() {
	...
	if(lastFight.length>1) {
		if(this.stats.stamina.num == lastFight[0]) {
			...
			var nextFight=parseInt(lastFight[1])+(1*10*1000);
			var now=(new Date().getTime());
			if(now<nextFight) {
				this.SetFightMessage("<font style='font-size: 12pt'>No stamina lost in last fight, maybe the user is dead, let's wait "+Math.floor((nextFight-now)/1000)+" secs</font>");
