...
// @name           QLRanks.com Display [with average server elo announcer]
// @version        1.90 [Modified v1.9]
// @description    Modified version of wn's script to announce average server elo, and changed the style of announce slightly. Intended mainly for personal use.
// @namespace      phob.net
// @homepage       http://www.qlranks.com
// @author         wn [Modified by earth_quake]
// @contributor    szr
// @contributor    syncore
// @contributor    simonov
...
// @run-at         document-end
// @updateURL      https://userscripts.org/scripts/source/417171.meta.js
// ==/UserScript==
      ...
      quakelive.serverManager.RefreshServerDetails(quakelive.currentServerId, {
        ...
        onSuccess: function() {
          ...
          QLRD.waitFor(names, gt, function(error, players, gt) {
		   ...
		   //Work out the average server elo (savg)
			var savg = 0;
				for (var i = 0, out = [], len = players.length; i < len; ++i) {
					savg = savg + players[i].elo;
				}
			savg = Math.round( savg / players.length );
			
			//Announce the average server elo
            qz_instance.SendGameCommand(currentOut + " ^2Average ELO on this server:^7 " + savg + ";");
			
			// gt.toLowerCase() is the gametype in lowercase
			
			//Announce the average team elos
			//qz_instance.SendGameCommand(currentOut + " ^1Red team ^3[^7" + ravg + "^3] ^3Versus ^4Blue team ^3[^7" + bavg + "^3]" + ";");
			
			//Announce the individual player elos
            for (var i = 0, out = [], len = players.length; i < len; ++i) {
			  //To announce player rank use players[i].rank
              ...
              if ((i+1) % 4 == 0 || (i+1) == len) {
                setTimeout(function(txt, isLast) {
                  ...
                  if (isLast) {
                    setTimeout(function() {
					//Add text between " " for an announcement upon completion
                      QLRD.igAnnounce("", false);
                    }, step);
                  }
