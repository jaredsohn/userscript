...
var mainURL = "http://lacn.dividinglimits.com/clicker";
...
function updateStats(response) {
  try {
    ...
    var moraleText = extractText(response, 'Your morale: ', ' &nbsp;');
    if (moraleText) {
...
function maybeSendRecaptchaResponse(e) {
  ...
  if (0xd == e.keyCode) {
    ...
    if (RecaptchaType_MISCLICK == recaptchaType) {
      details.url =
          'http://www.kingsofchaos.com/click.php?clickerid=' + myStatsid;
...
function getNextCaptchaImage() {
  ...
  var details = {};
  details.method = 'GET';
  details.url = 'http://www.kingsofchaos.com/click.php?clickerid=' + myStatsid
              + '&uniqid=' + uniqid;
  details.onerror = function (responseDetails) {
    --captchaFetchesInProgress;
...
function initializeRecruiter() {
  ...
  document.body.innerHTML =
      '<div id="linkBar">' +
      '  <span style="color:#fff">Eric YaR v' + VERSION +
      '  </span> &#xbb; ' +
      '  <a href="'+mainURL+'/top.php" target="_blank">' +
      '    Recruiter Stats</a> ' +
      '  <a href="'+mainURL+'/faq.php" target="_blank">' +
      '    Faq</a> ' +
      '</div>' +
      ...
      '<div id="pass">&nbsp;&nbsp;&nbsp;Enter Code&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;<INPUT TYPE="text" NAME="list" id="paslist" SIZE="20" MAXLENGTH="35"><input type="button" id="Open" value="Save"/></div>'+
      '<div id="cpass">&nbsp;&nbsp;&nbsp;Change Code :&nbsp;<INPUT TYPE="text" NAME="list" id="cpaslist" SIZE="20" MAXLENGTH="35"><input type="button" id="Open1" value="Save"/></div>';
...
try {
  ...
  if ('BUTTON' == clickPrefForm.lastChild.tagName &&
    ...
    document.body.innerHTML +=
        ...
        '  <a href="'+mainURL+'/eric_yar.user.js" ' +
