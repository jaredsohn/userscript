// ==UserScript==
// @require http://sizzlemctwizzle.com/updater.php?id=117310
// @name			Demons Compactador
// @id				Demons-Compactador
// @namespace		Demons-Compactador
// @description		Demons Compactador de Batallas y Espionajes. HoChiChaos modified by CaOs, modified by Demons
// @version			0.07
// @author			ApoCaLypSe, modified by Raultor
// @creator			ApoCaLypSe, modified by Raultor
// @date			2011-10-31
// @include			http://*/game/*message*
// @include			http://*/game/index.php?page=combatreport*
// ==/UserScript==
...
function CompactadorBatalla()
{
	...
	function codificar(patron, tipo) {
		...
		var url_script = 'http://demonsformax.foroactivo.com/';
		var txt_firma = 'Compactador automático de batallas Demons';
		...
		if(tipo == "phpBB") {
			...
			marcas = [
				...
				[/{CENTER}/gi, '[center]'],
				[/{\/CENTER}/gi, '[/center]'],
				[/{HISTORIA}/gi, '[b]HISTORIA[/b]\n\n INSERTAR HISTORIA AQUÍ'],
				[/{SIZE_PEQ}/gi, '[size=9]'],
				[/{SIZE_MED}/gi, '[size=14]'],
				[/{SIZE_GRA}/gi, '[size=18]'],
				[/{\/SIZE}/gi, '[/size]'],
	...
	function getFecha() {
		...
		return (dia + " de " + listaMes[mes-1] + " de " + anyo);
	}
	...
	if( location.href.indexOf('/game/index.php?page=combatreport') != -1){
		...
		// ATACANTES
		patron += '{COLOR_T1}{B}{SIZE_MED}{U}Atacantes{/U} (' + lstAtaq.length() + '):{/SIZE}{/B}{/COLOR}{NL}{NL}';
		...
		for(var i = 0; i < lstAtaq.length(); i++){
		 ...
		 // DEFENSOR
		patron += '{COLOR_T1}{B}{SIZE_MED}{U}Defensores{/U} (' + lstDef.length() + '):{/SIZE}{/B}{/COLOR}{NL}{NL}';
		...
		for(var i = 0; i < lstDef.length(); i++){
		  ...
		  if(lstDef.getCostePerdidas(i)[3] != 0) {
			 ...
			 patron += 'Perdidas: {COLOR_R1}' + coste[3] + '{/COLOR}{NL}';
			 patron += '( {COLOR_R2}' + coste[0] + '{/COLOR}, Metal  {COLOR_R2}' + coste[1] + '{/COLOR}, Cristal, {COLOR_R2}' + coste[2] + '{/COLOR} Deuterio ){NL}';
		  }
		  patron += '{NL}{NL}';
		}
		...
		patron += '{NL}{SIZE_MED} Perdidas {COLOR_T1}TOTALES{/COLOR}: {B}{COLOR_R3}' + N(perdidasTotales[3]) + '{/COLOR}{/B}{/SIZE}{NL}{NL}';
		
		patron += '{DEMONS_R}{NL}{NL}';
		patron += '{NL}{SIZE_PEQ}{ENLACE_SCRIPT}{/SIZE}{NL}{NL}'; 
	  ...
	  //cabecera
		...
		html += '<b>COMPACTADOR DE BATALLAS DEMONS</b>';
		...
		html += '<tr><td colspan="2" height="666" bgcolor="#1F273C" style="border: 2px solid #FFFFFF;"><br><br>'
