	...
	// Google Chrome
		...
		function affichageDuScript()
		{
		 ...
		 if(! lang_fr) MAJ = 'there is an update available';
				...
				document.getElementById("boutonBBcode").addEventListener("click", function(event) 		
				{
					...
					else if (lang_ru) var bbcode =  center +'[size='+sizeset[6]+']'+ bold + underline +'[color='+couleur[10]+'] ???????? ? ' +GM_getValue(nomScript+'date'+serveur ,'')+ '[/color]' + finunderline + finbold+'[/size] '+fincenter+' \n\n ' +affiche_leg; 
		...
		*/
					...
					for (var i =0 ; i< infoJoueur.length ; i++)
					{
						...
						{
							var classement2 = bold+'[color='+couleur[3]+']???????:[/color] '+finbold; // couleur du mot Classement
							var pointsoit = '[color='+couleur[3]+']'+bold+'????? ?'+finbold+'[/color] '; // couleur des mots "points soit"
							var classprog = bold+'[color='+couleur[3]+']????????: [/color]'+ newRang[i]+''+finbold+' '; // couleur de la progression
							progression2 = bold+'[color='+couleur[3]+']????????: [/color] '+finbold; // couleur du mot Progression
