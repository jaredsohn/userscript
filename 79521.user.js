...
// @namespace   arkangl
// @description Based on 'MUCH Improved Flickr Show All Sizes' (http://userscripts.org/scripts/show/168642). Adds an 'Embed' field conteining HTML code on each photo page. 
// @version     2014-04-21
...
(function(uwin, win){
	...
	var setSizesHTML = function(szHTML, total, Y){
		var opts = [
			...
			html = [
				...
				szHTML,opts,
				'<div style="color: white; display:inline;">Embed <input type="text" name="textfield" onFocus="this.select();" style="width:200px;" id="EMBED" value="Loading"/></div>',
				'</div></div>'
			].join(''),
	...
	var generateEmbedCode = function(embedId, largeSrc, mediumSrc, width, height, Y){
		...
		Y.one('#'+embedId).set('value', embedCode);
	};
	...
	var processSizes = function(pid, sizes, proto, Y){
		...
		generateEmbedCode('EMBED', 'https:'+largeUrl, 'https:'+mediumUrl, mediumWidth, mediumHeight, Y);		
	};
	...
	else
	{
		...
		document.getElementById('global-nav').style.top = '0px';
		document.getElementById('global-nav').style.backgroundColor = 'black';
