...
if (location.href.search(/screen=info_village/) != -1 && GM_getValue('copyPasteBB',1)) {
	...
	if (GM_getValue('copyPasteBB',1) || GM_getValue('extLinks')) {
		...
		if (GM_getValue('copyPasteBB',1)) {
			...
			extrasText+='</td></tr>' +
						'<tr><td>'+s[lang]['farming']+': </td><td>[url=http://en41.tribalwars.net/game.php?village='+getVillageId(location.href)+'&screen=place&mode=command&target='+vilId+']Attack[/url] [village]'+coords+'[/village]';
			if (player)
				extrasText+=' ([player]'+player+'[/player])';
			extrasText+='</td></tr>';
		}
