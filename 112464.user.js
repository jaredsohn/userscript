...
var nbUnis			= infos_version.nbUnis;
		...
		for (var i=0; i<nbScripts; i++)
		{
			...
			switch(script)
			{
				...
				case "Carto" :		inputs = options[2].getElementsByTagName("input");
									...
									inputs[1].addEventListener("click", function(){alert(config.Carto);}, false);
									inputs[2].addEventListener("click", function(){if(confirm("Reset ?")){config.Carto="";GM_setValue("config_scripts_uni_"+uni, JSON.stringify(config));}}, false);
									break;
				case "Markit" :		inputs = options[3].getElementsByTagName("input");
									...
									inputs[4].addEventListener("click", function(){if(confirm("Reset ?")){config.Markit.coord={};GM_setValue("config_scripts_uni_"+uni, JSON.stringify(config));}}, false);
	...
	if (can_load_in_page("TChatty") && infos_scripts.TChatty )
	{
		...
		jscolorid.addEventListener("click", function(){	document.getElementById("jscolor_box").addEventListener("mouseout", function(){	config.TChatty.color = 	document.getElementById("jscolorid").value;GM_setValue("config_scripts_uni_"+uni, JSON.stringify(config));}, false);	}, false);
	...
	if (can_load_in_page("Markit") && infos_scripts.Markit  || can_load_in_page("Carto") && infos_scripts.Carto )
	{
		...
		for (var i=0; i<2; i++)
			...
			{
				players_list[i][j].parentNode.parentNode.getElementsByClassName("galaxy_float80")[1].style.width = "140px";
