// ==UserScript==
// @name           Muzyczne Imperium wer.1.0
// @namespace      robert197648
// @description    przypomina dzwiekiem o zbiorach
...
var loc=document.location.toString().match(/http:\/\/s(.*?)\.zieloneimperium\.pl\/(.*?)\.php/);
switch (loc[2]) {
...
function soundplay()
{
  if (!$("player"))
  {
    var player = createElement('div', {'id': 'player', 'style': 'text-align:right;border: 1px solid black; position:absolute; top:2px;right:2px; padding:2px; z-index:1; background-color:white; color:black;'}, document.body);
    player.innerHTML='\
...
function do_garten_map()
{
  ...
  window.setInterval(function () {
    ...
    if (lasttime == 9999999999)
      lasttime=Now;
    if (lasttime-Now<=0)
    { soundplay(); parent.document.title="GOTOWE - Zieloneimperium"; }
    else
    { soundstop(); parent.document.title=time2str(lasttime-Now)+" - Zieloneimperium"; }
  },500);
}
