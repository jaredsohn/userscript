...
// @description Search NZBmatrix/NZBs.Org from MyEpisodes, and change the width to 100%
...
// ==/UserScript==
    ...
    //======[Settings]======\\
    var SearchEpisodeNumber = 'yes';    //==['yes' or 'no']
    var ConvertEpisodeText = 'yes';     //==['yes' or 'no']
    var SearchURL = 'http://nzbmatrix.com/nzb-search.php?search=--Search'; //==[The Key is ( --Search )]
    var SearchText = '[NZBm]';    //==[keep it empty ( '' ) to disable]
    var SearchURL2 = 'http://nzbs.org/index.php?action=search&q=--Search'; //==[The Key is ( --Search )]
    var SearchText2 = '[NZBs]';   //==[keep it empty ( '' ) to disable]
    var ChangeWidth = 'yes';   //==['yes' or 'no', Change the table width to 100%]
...
function getElementsByClass(searchClass,node,tag) {
    ...
    for (ii = 0; ii < ShowsText.length; ii++) {
        ...
        if (SearchEpisodeNumber == 'yes'){
            
            EpisodeNo = EPText[ii].innerHTML;
            
            if (ConvertEpisodeText == 'yes'){
                EpisodeNo = EpisodeNo.replace(/x/i,"E").trim();
                
                if (EpisodeNo.length == 4 ) {
                    EpisodeNo = "S0" + EpisodeNo;
                } else {
