...
// select "TrailerAddict - EZ Download", and click Uninstall.
//
// --------------------------------------------------------------------
//
// ==UserScript==
// @name          TrailerAddict - EZ Download
// @namespace     http://www.Daem0nX.com (Daem0nX@gmail.com)
// @description   Adds menu (bottom right) with support for SD/HD file downloads - trailers and clips.
// @version		  0.7.2
// @notes		  Fixed SD and HD download links.<br/>Fixed menu and overlay border radius not working in Firefox 13.<br/>Fixed menu error message using wrong id.<br/>Upgraded jQuery from 1.7.1 to 1.7.2.<br/>^Changed .bind() and .unbind() to .on() and .off().
// @include       http*://www.traileraddict.com/trailer/*/*
// @include		  http*://www.traileraddict.com/clip/*/*
// ==/UserScript==
...
scriptName='TrailerAddict - EZ Download';
scriptId='81270';
scriptVersion='0.7.2';
scriptUpdateText='Fixed SD and HD download links.<br/>Fixed menu and overlay border radius not working in Firefox 13.<br/>Fixed menu error message using wrong id.<br/>Upgraded jQuery from 1.7.1 to 1.7.2.<br/>^Changed .bind() and .unbind() to .on() and .off().';
// === Stop editing here. ===
...
var debugGM = false;
var isWebkit = false;
var donationLink_paypal = 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=X6SY3QRBQM9ZC';
var donationLink_flattr = 'https://flattr.com/thing/412444/TrailerAddict-EZ-Download';
...
$ljq("#gmmnu").html(""
		+"\n<div id='gmmnu_title' data-title='TrailerAddict Download'>TrailerAddict Download</div>\n"
		+"<div id='gmmnu_menu'>\n"
	...
	//}
	//alert($ljq("#gmmnu_links_hd").css("display"));
...
var trailers = {
  sd : {
    file : {name : "MP4 for iPod", ext : "mp4", size : ""}
  },
  hd : {
    file : {name : "Flash", ext : "flv", size : ""}
  }
}

var curwindow = window.location.toString();
//alert(curwindow);
pattern = /(http:\/\/.*\.traileraddict.com\/(trailer|clip)\/(.*)\/.*)/ig;
matches = curwindow.match(pattern);
Vidtype = matches[0].replace(pattern, "$2").toLowerCase();
//alert(matches);
//alert(matches[0].replace(pattern, "$3"));
Pagebase = matches[0].replace(pattern, "$3");
//Matches if JS is disabled
Pageid = $ljq("noscript object param[name='movie']").attr("value");
if (Pageid == null || Pageid == "") {
	//alert("Blank");
	//Matches if JS is enabled
	Pageid = $ljq("noscript:contains('traileraddict.com/emb/')").html();
}
if (Pageid == null || Pageid == "") {
	showError();
}
//Pageid = $ljq("noscript embed[src*='emb']").html();
//alert(Pageid);
pattern = /(http:\/\/.*\.traileraddict.com\/(emb)\/([0-9]{1,99}))/ig;
matches = Pageid.match(pattern);
//alert(matches[0].replace(pattern, "$3"));
//http://www.traileraddict.com/emb/23554
Pageid = parseInt(matches[0].replace(pattern, "$3"));
//pattern = "";
//matches = "";
//alert(Pagebase);
//alert(Pageid);
...
//$ljq(document).ready(function(){
	...
	GMlog("#videotabs #vidopts area[title*='Unavailable'] : "+ $ljq("#videotabs #vidopts area[title*='Unavailable']").length);
	if ($ljq("#videotabs #vidopts area[title*='Unavailable']").length > 0) {
		//getSDfile();
		getFile('sd');
		$ljq("#gmmnu_links_hd").hide();
	} else {
...
function getFile(type) {
	var xmlfile = '';
	if (type == 'sd') {
		xmlfile = 'http://www.traileraddict.com/fvar.php?tid='+Pageid;
	}
	if (type == 'hd') {
		xmlfile = 'http://www.traileraddict.com/fvarhd.php?tid='+Pageid;
	}
	
	GMlog('xmlfile: ' + xmlfile);
	...
	GM_xmlhttpRequest({
    	...
    	onload: function(res) {
    		...
    		if (status == '200') {
	    		...
	    		var text = unescape(res.responseText);
	    		GMlog(xmlfile);
	    		GMlog(text);
    			//alert("SD: \n"+text);
    			try {
					//var fileurl = text.replace(/.*?fileurl=(.*?\.(mp4|flv)).*/ig, "$1")
					//GMlog("fileurl: "+fileurl);
					//var filesecuritylong = text.replace(/.*?fileurl=.*?\.(mp4|flv)\?([a-z0-9]+).*/ig, "$2")
					//GMlog("filesecuritylong: "+filesecuritylong);
					//var fileexpire = text.replace(/.*?(.expires=[0-9]+).*/ig, "$1")
					//GMlog("fileexpire: "+fileexpire);
					//var filetoken = text.replace(/.*?(.token=[a-z0-9]+).*/ig, "$1")
					//GMlog("filetoken: "+filetoken);
					pattern = /fileurl=((.*?\.(mp4|flv))(\?[a-z0-9]+|\|[a-z0-9|]+)?(.expires=[0-9]+)?(.token=[a-z0-9]+)?)/ig;
					matches = text.match(pattern);
					...
					if (type == 'sd') {
						...
						$ljq("#gmmnu_linksfiles_sd").append(''
							+'<div class="link"><a title="'+ Pagebase +'"'
							+'href="'+ SDLink +'" '
							+'class="icon_ico">' + Pagebase + '</a></div>'
							+'');
					}
					if (type == 'hd') {
						HDLink = matches[0].replace(pattern, "$1");
						GMlog(HDLink);
						$ljq("#gmmnu_linksfiles_hd").append(''
							+'<div class="link"><a title="'+ Pagebase +'"'
							+'href="'+ HDLink +'" '
							+'class="icon_ico">' + Pagebase + '</a></div>'
							+'');
...
//    Use in compliance with Free for commercial use

//
// Thanks
//  lastmoexit - helped test, reported issues
//

//
// ChangeLog
// 2012-06-18 - 0.7.2
//  Fixed SD and HD download links.
//  Fixed menu and overlay border radius not working in Firefox 13.
//  Fixed menu error message using wrong id.
//  Upgraded jQuery from 1.7.1 to 1.7.2.
//    Changed .bind() and .unbind() to .on() and .off().
//
// 2011-11-26 - 0.7.1
//  Fixed HD download link not showing on HD pages.
//  Upgraded jQuery from 1.7.0 to 1.7.1.
//  Modified CSS and minor code clean up.
//
// 2011-11-19 - 0.7.0
//  Fixed HD download link.
...
//  Upgraded jQuery from 1.6.1 to 1.7.0.
//    Changed .live() calls to .on()
//
// 2011-06-15 - 0.6.0
//  Changed name from TrailerAddict.com - EZ Download to TrailerAddict - EZ Download.
//    If upgrading make sure to remove the old version from your greasemonkey scripts.
//  Fixed menu and links not loading if Javascript was enabled on TrailerAddict.
//  Upgraded jQuery from 1.3.2 to 1.6.1.
...
//
// 2011-02-03 - 0.5
//  Fixed trailer and clip downloads.
//  Checks for HD button before looking for HD file.
...
//
// 2010-09-23 - 0.4
//  Now checks for MP4 or FLV as the download source.
//
// 2010-07-12 - 0.3
//  Added ability to download clips.
//
// 2010-07-11 - 0.2
//  If no SD/HD link is found, its section is hidden.
//
// 2010-07-11 - 0.1
//  Initial release.
//  Adds menu with SD/HD file links.
//