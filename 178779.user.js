// ==UserScript==
// @name        The West - Player Checker (sp)
// @author      neversleep1911 (translation - pepe100)
...
(function (func) {
	...
	TW_PlayerChecker.check = function () {
		...
		if (text == "") {
			alert('Introduzca el Alias del jugador, uno por línea.');
			return;
		}
		
		TW_PlayerChecker.checking = true;
		TW_PlayerChecker.gui.txtLog.setText("Espere...");
		...
		doCheck(text.split('\n'), function (players) {
			...
			for (var i = 0; i < players.length; i++) {
				...
				if (player == TW_PlayerChecker.PlayerState.Player_NotFound)
                    ...
                    text += " | ND:" + player.duel_level + " | " + Character.calcWayTo(player.x, player.y).formatDuration() + "\n";
				}
			}
	...
	TW_PlayerChecker.showMainWindow = function () {
		...
		var playersGroup = new TW_Widgets.Group("Jugadores", TW_PlayerChecker.gui.mainWindow.getContentPane());
		playersGroup.setWidth(250);

		var statusGroup = new TW_Widgets.Group("Estado", TW_PlayerChecker.gui.mainWindow.getContentPane());
		statusGroup.setLeft(255).setWidth(440);

		var checkButton = new TW_Widgets.Button("Verificar", TW_PlayerChecker.gui.mainWindow.getContentPane(), TW_PlayerChecker.check);
		checkButton.setTop(340);

		TW_PlayerChecker.gui.txtPlayers = new TW_Widgets.TextArea();
		TW_PlayerChecker.gui.txtPlayers.setWidth(200).setHeight(260).setText("pepe100");
