// ==UserScript==
// @name                azorder24.com
// @namespace	        http://azorder24.com
// @description	        azorder24.com - Công cụ tối ưu đặt hàng trên các site Trung Quốc
// -------------------------------------------------------------------------------------
...
var Settings = {
    cart_url:"http://azorder24.com/cart/add?",
    cart_view_url:"http://azorder24.com/gio-hang.html",
    ...
    priceArea:$('<div style="font-weight:bold;color: #f60; font-size: 24px;margin-left:14px" id="hangnhap-price">0 VNĐ <span style="float:right;font-size:12px;padding-right:13px"><a href="http://azorder24.com" style="color:#f60">azorder24.com</a></span></div>'),
    //<div style="font-weight:bold;color: #f60; font-size: 24px;margin-left:14px" id="hangnhap-price">'+price_result+' VNĐ <span style="float:right;font-size:12px;padding-right:3px"><a href="http://azorder24.com" style="color:#f60">azorder24.com</a></span></div>
    Result:{
        notFillProps:new Result(false,"Bạn chưa chọn đủ thuộc tính của sản phẩm"),
        quantity:{
            below1:new Result(false,"Hết hàng hoặc bạn chưa chọn số lượng")
        }
    },
    account:{
        user:"Vincent",
...
var Sys={
    ...
    updatePrice:function(){
        ...
        Settings.priceArea.html(getVNDPrice(Sys.currentPrice)+" VNĐ");
    },
    ...
    disable:function(d,key){
        ...
        if(d){
            var parent = $("#dathang247area");
            var d_layer = $("<div id='dathang247-block-layer_"+key+"'></div>");
            d_layer.css({
                position:"absolute",
                width:parent.width(),
            ...
            $("#dathang247-block-layer_"+key+"").remove();
        }
    },
	...
	checkNewVersion:function(){
		return;
		$.ajax({
			url:"http://www.azorder24.com/update.json?_t="+new Date().getTime(),
...
//            }
        ...
        factoryMerchant1:function(obj){
            ...
            this.getPrice = function(){
                ...
                if(p!=null&&p.length==1){
                    ...
                    if($(obj.getPrice)[0]){
                        $(obj.getPrice).html("<img src='http://azorder24.com.vn/images/ajax-loader.gif'>");
    ...
    this.myHTML = function () {
        ...
        var src = 'http://azorder24.com/images/dathang.png';
        var src_cart = 'http://azorder24.com/images/cart.png';

        var price_raw = this.getTextPrice();
        var price_result = getVNDPrice(price_raw);
        //var com_text = '<div style="font-weight:bold;color: #f60; font-size: 24px;margin-left:14px" id="hangnhap-price">'+price_result+' VNĐ <span style="float:right;font-size:12px;padding-right:3px"><a href="http://azorder24.com" style="color:#f60">azorder24.com</a></span></div>';
        var com_text = '<div style="margin-left:14px"><div>Yêu cầu thêm của bạn về sản phẩm này:</div><textarea cols="70" rows="3" id="txtCmt" name="txtCmt"></textarea></div>';

        var s = '<div id="dathang247area" class="g-group-member" style="position:relative;overflow: hidden;height: 170px; border: 2px solid #f60;padding-top:10px">' + com_text + '<div class="" style="width:100%;float:right">';
        s += '<div><div style="float:left;margin-left:14px" id="block_button"><a id="id_noimua_add_cart" href="javascript:void(0)"><img src="' + src + '" alt="" /></a>' +
    ...
    this.addToCart = function () {
        ...
        var params = ("type={0}&item_id={1}&item_title={2}&item_image={3}&comment={4}&item_link={5}&item_price={6}" +
            ...
            "&max={14}&min={15}&multiply={16}&ct={17}")
                ...
                this.getAllowedMinQuantity(),this.getAllowedMultiplyQuantity(),this.getCookie('t'));
...
Merchant.templateFunction = {
    addToCartHandler:function(data){
        ...
        if(ok){
			...
			var msg = "Đặt hàng thất bại, xin mời đặt hàng lại"
			if(tmp_data.reason){
				msg+="\nLý do:"+tmp_data.reason
			}
            alert(msg);
			if(tmp_data.script){
				try{
					eval(tmp_data.script)
				}catch(e){
					console.error("Cannot evaluate script:"+e.message);
				}
			}
        }
    }
...
//console.log("detected "+merchant.MerchantName);
	if(merchant.MerchantName.indexOf("tmall")>-1){
		$(function(){
			...
			$("#id_noimua_add_cart").click(function () {
...
var mTaobao = new Merchant("taobao",'taobao');
	...
	mTaobao.getCookie=function(c_name) {
    ...
    mTaobao.getItemTitle=function(){
        var hd = document.getElementsByClassName('tb-item-title')[0];
        if(typeof hd === 'undefined') {
            hd = document.getElementsByClassName('tb-detail-hd')[0];
        }
    ...
    mTaobao.getSellerId=function(){
        ...
        if(hd!=null) {
            ...
            if(typeof hd === 'undefined') {
                hd=document.getElementsByClassName('shop-name')[1].getElementsByTagName('span')[0];
                if (typeof hd === 'undefined') {
                    hd=document.getElementsByClassName('shop-name')[1].getElementsByTagName('a')[0];
                }
            }
...
mTmall.getPrice=function(){
    var p_o=document.getElementById('J_StrPriceModBox');
    if(p_o!=null)
    {
        p_e = p_o.getElementsByTagName('span')[0];
        ...
        if(document.getElementById('J_PromoPrice')!=null && document.getElementById('J_PromoPrice')!='undefined')
        {
			...
			if(document.getElementsByClassName("tm-promo-price").length>0){
				var tb=document.getElementsByClassName("tm-promo-price")[0].textContent;
				if(p_p.getElementsByTagName("span").length > 0 && p_p.getElementsByTagName("span")[0].textContent != '' && tb.indexOf('VIP') == -1) {
                    p_e=p_p.getElementsByTagName("span")[0];
                }
			}
        }
...
mTmall.getQuantity=function(){
    var amount=$("#J_IptAmount");
    var amount2=$(".mui-amount-input");
    
    if(amount.length==1){
        return amount.val();
    }else if(amount2.length==1){
        return amount2.val();
    }else{
        return 0;
    }
...
var mAlibaba = new Merchant("alibaba",['alibaba','1688']);
    mAlibaba.onLoad = function() {
        var self = this;
        var div = $(self.myHTML())[0];

        var tar = null;
        if ($('.region-detail-price').length > 0) {
            tar = $('.region-detail-price');
        } else if ($('.mod-detail-limited-price').length > 0) {
            tar = $(".mod-detail-limited-price");
        } else if ($('.mro-price').length > 0) {
            tar = $('.mro-price');
        } else if ($('.mod-detail-price').length > 0) {
            tar = $('.mod-detail-price');
        } else if ($('.mod-detail-price-sku').length > 0) {
            tar = $('.mod-detail-price-sku');
        } else {
            alert('Addon không chạy trên trang hiện tại, xin hãy gửi thông báo để Azorder24 có thể khắc phục tình trạng lỗi này một cách nhanh chóng, xin cảm ơn bạn!');
        }
        $(div).insertAfter(tar);
    ...
    mAlibaba.getPrice=function(quantity){
        ...
        var price = 0;
        var span_price=document.getElementsByClassName('mod-detail-price-sku')[0];
        // Một mức giá.
        if(typeof span_price != 'undefined')
        {
            //price=span_price.textContent;
            var e_num=span_price.getElementsByTagName('span')[1].textContent;
            var p_num=span_price.getElementsByTagName('span')[2].textContent;
            price = e_num + p_num;
        ...
        var span_prices = {};
        if(div_prices !== null) {
            span_prices = div_prices.getElementsByTagName("span");
        }

        // Trường hợp có giá giới hạn.
        div_limit_price = document.getElementsByClassName("mod-detail-limited-price")[0];
        if(div_limit_price !== undefined) {
            div_prices = div_limit_price.getElementsByClassName("limited-price")[0];
            if(div_prices !== null) {
                span_prices = div_prices.getElementsByClassName("price")[0];
                if(span_prices !== null) {
                    price = span_prices.textContent;
                    return processPrice(price);
                }
            }
        }
    ...
    mAlibaba.getTextPrice = function(){
		var num = 1;
		try{
			num= parseInt(document.getElementsByClassName('total')[0].getElementsByClassName('amount')[0].textContent);
			if(num < 1 ) num = 1;
		}catch(e){
			num = 1;
		}
    ...
    mAlibaba.get_item_data = function() {
        ...
        // Mua hàng số lượng lớn.
        } else if($(".table-wrap").length > 0) {
            var amount = parseInt(document.getElementsByClassName('amount-input')[0].value);
            $(".table-wrap table tbody tr").each(function(i, obj) {
                var data = JSON.parse($(obj).attr("data-range"));
                if(isNaN(amount) || amount == 0) {
                    return;
                }
                if ((amount >= parseInt(data.begin) && amount <= parseInt(data.end)) || (amount >= parseInt(data.begin) && data.end === '')) {
                    result[i] = new Array();
                    // Add data to array
                    result[i]['amount'] = amount;
                    result[i]['min_amount'] = data.begin;
                    result[i]['max_amount'] = data.end;
        ...
        // Update for new structure - 18/02/2014
        if ($('.sub-total li').length > 0) {
            result = [];
            if ($('.sub-total .info-item').length > 0) {
                $('.sub-total .info-item').each(function (i, item) {
                    var data_sub_config = $.parseJSON($(item).closest('.sub-total').attr('data-sku-config'));
                    var data_item_config = $.parseJSON($(item).attr('data-sku-config'));

                    result[i] = [];
                    result[i]['amount'] = $(item).find('.number .display em').text();
                    result[i]['max_amount'] = data_item_config.max;
                    result[i]['min_amount'] = data_sub_config.min;
                    result[i]['color'] = $(item).closest('.sub-item').attr('data-name');
                    result[i]['size'] = data_item_config.skuName;
                });
            } else {
                $('.sub-total li').each(function (i, item) {
                    var data_sub_config = $.parseJSON($(item).closest('.sub-total').attr('data-sku-config'));

                    result[i] = [];
                    result[i]['amount'] = $(item).find('.sub-amount em').text();
                    result[i]['max_amount'] = data_sub_config.max;
                    result[i]['min_amount'] = data_sub_config.min;
                    result[i]['color'] = $(item).find('.sub-name').text();
                });
            }
        }
    ...
    mAlibaba.getLink=function(item_data) {
        var params = ("\
            ...
            this.getTextPrice(), 
            this.getPriceTable(), 
            [item_data['size'], item_data['color']].join(";"),
            this.getSellerId(),
            this.getSellerId(), 
            item_data['amount'], 
            this.getColor(), 
            this.getSize(),
            item_data['max_amount'],
            item_data['min_amount'],
            1,
