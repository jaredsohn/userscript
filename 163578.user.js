// ==UserScript==
// @name          Download Facebook Album by one click
...
function detectFbLayout(){
	if(location.href.match(/.*\/photos/) && !!qS('.fbTimelineTopSection a[href$="/timeline"]')){
		// should be new layout
		g.newL = true;
	}else{
		g.newL = false;
	}
...
function fbAutoLoad(elms){
	...
	if(isGp){
		...
		var collectionId = qS('#pagelet_timeline_medley_photos a[aria-selected="true"]').getAttribute('aria-controls').match(/.*_(.*)/)[1];
		var userId = collectionId.match(/(\d*):.*/)[1];
		var s=qSA("script");
		for(var i=0;i<s.length;i++){
			if(!s[i].src&&s[i].textContent.indexOf('TimelineAppCollection')>0){s=s[i].textContent;break;}
		}
		var cursor = s.match(/.*\"},\"(.*)\"].*StarGrid/)[1];
		aInfo = {"collection_token": collectionId, cursor: cursor, "tab_key":"photos_stream","profile_id": +userId,"overview":false,"ftid":null,"sk":"photos"};
	}
	...
	if(!g.newL || isGp || isAl){
		...
		var targetURL=tab[0].getAttribute('aria-selected')?'TaggedPhotosAppCollection':'AllPhotosAppCollectionPagelet';
		ajaxAlbum=location.protocol+'//www.facebook.com/ajax/pagelet/generic.php/'+targetURL+'Pagelet?data='+JSON.stringify(aInfo)+'&__user='+g.Env.user+'&__a=1';
	}
	var xhr = new XMLHttpRequest();
	xhr.onload = function(){
		clearTimeout( g.timeout );
