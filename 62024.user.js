...
{ 
	var rs = db2.execute('select ValueContent from GM where KeyName = ?', [key]); 
	if (rs.isValidRow())
	{ 
		db2.execute('update GM set ValueContent =? where KeyName = ?', [value, key]); 
	} 
	else 
	{ 
		db2.execute('insert into GM values (?, ?)', [key, value]); 
	} 
} 
...
function dataBaseGMInit()
{ 
	try 
	{ 
		db2 = google.gears.factory.create('beta.database'); 
		db2.open('database-demo2'); 
		db2.execute('create table if not exists GM' + ' (KeyName text, ValueContent text)'); 
	} 
	catch (ex) 
	{ 
		dbg('Could not create database: ' + ex.message); 
	} 
}
...
// update script from: http://userscripts.org/scripts/review/62024

var SUC_script_num = 62024; // Change this to the number given to the script by userscripts.org (check the address bar)
...
SetControls:function() {
	...
	controlHtml+="<img title='' src='http://photos-b.ak.fbcdn.net/hphotos-ak-snc3/hs022.snc3/10938_102716103081176_100000283613396_72667_1476232_s.jpg' />: <input type='text' id='PreferMonsterBy' title='"+preferInstructions+"' /><br />";
		"<br />";
	controlHtml+="<input type='checkbox' id='AutoZyngaDisabled' /><img title='' src='http://photos-d.ak.fbcdn.net/hphotos-ak-snc3/hs022.snc3/10938_102714446414675_100000283613396_72490_8052149_s.jpg' /><br />";
...
CheckPage:function() {
	...
	if(!gameDisabled) {
		...
		if (autoElite!='') {	
			if ((this.GMGetValue('MyEliteTodo','').trim()=='' && this.WhileSinceDidIt('AutoEliteGetList',60*60*12)) ) {
