// ==UserScript==
// @name           HeroesVsVillains
// @description    autoplayer for the Heroes vs Villains game
// @namespace      facebook
// @version        2.0.3
// @include        http://apps.facebook.com/heroesvillains/*
// @author         blannie
// @contributor	   IEF, colin1497
// ==/UserScript==
...
var SCRIPT = {
	url: 'http://userscripts.org/scripts/source/40919.user.js',
	version: '2.0.3',
	name: 'heroesvillains',
	appID: 'app97498830044',
	money: 'vial',
	clan: 'league',
	presentationurl: 'http://userscripts.org/scripts/show/40919'
};
...
// Name Hero -- Name Villain, Energy , Tab , Item on tab, form ID, experience(power)
	["Save a cat from a tree -- Mug an innocent bystander",1,1,0,1,1],//1
	["Stop a NYC mugging -- Assassinate a local hero",3,1,1,2,3],	 //2
	["Capture the Mafia Family terrorizing the City -- Unleash a killer virus",5,1,2,3,5], //3
	["Prevent two trains from colliding -- Free the criminals in prison",7,1,3,4,8], //4
	["Foil a casino robbery in Vegas -- Loot treasure from the Pyramids",1,1,4,11,1], //11
	["Protect a UN ambassador -- Plot with your werewolf allies",2,1,5,12,2], //12
	
	["Rescue a damsel in distress -- Plant bombs around the city",10,2,0,5,15], //5
	["Save passengers from a sinking ship -- Blow up a research facility",5,2,1,13,5], //13
	["Imprison a villian in an alternate dimension -- Erase your victims' memories",6,2,2,14,6], //14
	["Prevent the Mad Scientist creating an Army of Robots -- Hold the Mayor hostage",15,2,3,6,20], // 6
	
	
	["Find the Nuke before it detonates -- Assist the Mad Scientist in creating an Army of Robots",18,3,0,7,22], // 7
	["Stop a rogue comet from destroying the Planet Earth -- Sneak into a government facility",25,3,1,8,0], // 8
	["Destroy the villaneous Flying Ninjas -- Cause a rogue comet to destroy a city",30,3,2,9,0], // 9  
	["Hone in on a target from far away -- Create a city-wide blackout",13,3,3,16,0], //16
	["Destroy your archenemy's jungle lair -- Destroy your archenemy's aquatic base",15,3,4,17,0], //17
	
	
	["Save yourself in an Alternate Dimension","Detonate a Nuke in the city",30,4,0,10,0], // 10
	["Fight global warming","Create a devastating tornado",35,4,1,19,0], //19
	["Prevent a clone army from dominating the world","Poison the city's water supply",40,4,2,20,0], //20	
	
	["xxxx","xxxx",255,5,0,15,0], //15
	["xxxx","xxxx",255,5,1,18,0], //18
	["Save the city from cosmic rays","Inundate the city with death rays",50,5,2,21,0], //21
	["Convert a Villain to your side","Convert a Hero to your side",50,5,3,22,0] //22
	);
...
var gold = document.getElementById(SCRIPT.appID+'_stats_bar_cash').childNodes[1].childNodes[2].innerHTML.substring(document.getElementById(SCRIPT.appID+'_stats_bar_cash').childNodes[1].childNodes[2].innerHTML.indexOf(">")+1);
			...
			//basic Mastery logic. 
				...
				for (var i=0;i<missions.length;i++)
					...
					if (missions[i][2]==currentTab)
						...
						//addToLog(missions[i][0] + ' ' +missions[i][3] + ' ' + masteryPerc); //debug code
...
function scanfightlist()
	...
	if (location.href.indexOf(SCRIPT.name+'/fight.php') != -1)
		...
		for (var i=0;i<fightForms.snapshotLength;i++)
			{
			var inList = 0;
			fightTable = fightForms.snapshotItem(i).parentNode.parentNode;
...
//			fightInfo=document.evaluate(".//div",fightTable,null,XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,null);
			var opponentLevel =  parseInt(fightTable.innerHTML.split('Level ')[1]);
			var opponentClan = parseInt(fightTable.innerHTML.split('groupsize">')[1]);
