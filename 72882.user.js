...
// @include        http://*.grepolis.*/game/alliance?action=forum*forum[thread_id]*
// @description    Display costs for losses in Townwall, fight reports and simulator.
// @version        1.5.2
// ==/UserScript==
...
/**
 * Adds costs.
 * @param iCount - Number of units (as number)
 * @param sName - Unit name ("slinger" e.t.c)
 */
ResCounter.prototype.add = function( iCount, sName )
{
	...
	{	
		...
		if ( uc.population )
			this.iBH      += iCount * uc.population;
		if ( uc.favor )
			this.iFavor   += iCount * uc.favor;
	}		
};
...
{
        return { Wood  : ResCalc.fInt(this.iWood  ),
...
ResCalc = 
{
    ...
    de:{
	  ...
	  popups : {
		...
		rcwood_lost  : "<h4>Holz</h4>Basiskosten Holz, die f&uuml;r verlorenen Einheiten aufgewendet werden m&uuml;&szlig;ten.<br/>Nicht ber&uuml;cksichtigt werden Forschungen wie <i>'Wehrpflicht'</i>.",
		rcstone_lost : "<h4>Stein</h4>Basiskosten Stein, die f&uuml;r verlorenen Einheiten aufgewendet werden m&uuml;&szlig;ten.<br/>Nicht ber&uuml;cksichtigt werden Forschungen wie <i>'Wehrpflicht'</i>.",
		rciron_lost  : "<h4>Silber</h4>Basiskosten Silber, die f&uuml;r verlorenen Einheiten aufgewendet werden m&uuml;&szlig;ten.<br/>Nicht ber&uuml;cksichtigt werden Forschungen wie <i>'Wehrpflicht'</i>.",
		rcfavor_lost : "<h4>Gunst</h4>Gunst, die f&uuml;r verlorenen Einheiten aufgewendet werden m&uuml;&szlig;ten.",
		...
		wallFoeCosts : "Kosten Deiner Gegner"
	  }	  
    },
	
    en:{
      lostTable: "Tabelul resurselor",
      lostTitle: "Resurse pierdute",
	  ...
	  popups : {
	    // Texts uses in pop-ups.
		 rcwood_lost  : "<h4>Lemn</h4>Costul de lemn care ar trebui să fie cheltuit pentru unităţi </br>nu vor fi luate în calcul în cazul cercetării <i>'Serviciu militar obligatoriu'</i>.",
					rcstone_lost : "<h4>Piatra</h4>Costul de piatră care ar trebui să fie cheltuit pentru unităţi </br>nu vor fi luate în calcul în cazul cercetării <i>'Serviciu militar obligatoriu'</i>.",
					rciron_lost  : "<h4>Argint</h4>Costul de arginţi care ar trebui să fie cheltuit pentru unităţi </br>nu vor fi luate în calcul în cazul cercetării <i>'Serviciu militar obligatoriu'</i>.",
					rcfavor_lost : "<h4>Favoruri</h4>Costul de favoruri care ar trebui să fie cheltuit pentru unităţi </br>nu vor fi luate în calcul în cazul cercetării <i>'Serviciu militar obligatoriu'</i>.",
					...
					wallYourCosts: "Costurile dumneavoastră",
					wallFoeCosts : "Costurile adversarului"
    ...
    fr:{
	  ...
	  popups : {
	    // Texts uses in pop-ups.
		  rcwood_lost  : "<h4>Bois</h4><br/><i>'Recherches'</i> ignoré.",
		  rcstone_lost : "<h4>Pierre</h4><br/><i>'Recherches'</i> ignoré.",
		  rciron_lost  : "<h4>Argent</h4><br/><i>'Recherches'</i> ignoré.",
    ...
    nl:{
	  ...
	  popups : {
	    // Texts uses in pop-ups.
		  ...
		  rcwood_lost  : "<h4>Hout</h4><br/><i>'Onderzoek'</i> wordt niet meegerekend.",
		  rcstone_lost : "<h4>Steen</h4><br/><i>'Onderzoek'</i> wordt niet meegerekend.",
		  rciron_lost  : "<h4>Zilver</h4><br/><i>'Onderzoek'</i> wordt niet meegerekend.",
	...
	init : function()
	{
		...
		if (sr) {
			...
			if ( !ResCalc.text )
				ResCalc.text = ResCalc.text_translations["en"];
		}		
		...
		else
		{
			...
			{
				// Debugging output, need no translation.
		...
		if ( ResCalc.UnitData )
		{
			...
			else if ( uW.document.URL.indexOf( "action=simulator") > 0 ) 
			{
				...
				ResCalc.createResPopup('fcSim', ResCalc.text.lostTable);
			...
			else if ( uW.document.URL.indexOf( "game/building_wall?") > 0 ) 
			{
				...
				ResCalc.createResPopup( 'fcWall', ResCalc.text.lostTitle  );
			...
			else
			{
				...
				if (rv)
				{
					...
					// Put the two sides (attack & Defend) together
					var joinsReport = ResCalc.joinsReportSides( attCostsTable, defCostsTable );
					if ( joinsReport )
					{
						var sumNode = $(joinsReport);
	...
	/**
	 * Join two report resource results in one html element.
	 * (used for reports and embedded reportes)
	 */
