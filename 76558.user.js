...
// select "InsaneJournal ljwho", and click Uninstall.
// 
// ==UserScript==
// @name    InsaneJournal ljwho
// @namespace		http://tearex.nfshost.com/gm/
// @description		Annotate usernames on InsaneJournal
// @longdescription	Similar to the "Notes" feature for paid users on LJ.
// @version		0.21
...
function getLJNameFromLink(link)
{
    ...
    var who;
    if (href) {
      matched = href.match(/([\w\-\_]+)\.insanejournal.com\/?$/);
      if (matched) {
        who = matched[1];
      } else {
        matched = href.match(/^http\:\/\/(insanejournal.com\/users||www.insanejournal.com\/users|www.insanejournal.com\/asylums|asylums.insanejournal.com|syndicated.insanejournal.com)\/([\w\_\-]+)\/?$/);
        if (matched) {
          who = matched[2];
        }
      }	 
