...
// @include        http://ae*.tribalwars.ae/groups.php*
// @exclude        http://ae*.tribalwars.ae/groups.php*mode=village*
// ==/UserScript==
...
(function(){
		...
		if(!/[&?]t=\d+/.test(location.href)) {
			...
			if(win.localStorage) {
				...
				if(value.length > 4096) {
					alert('Die Einstellungen für die Gruppen-Markierungen auf der Karte sind zu groß um in einem Cookie gespeichert zu werden. Es kann daher zu Problemen kommen. Das Löschen einiger Gruppen kann das Problem lösen.');
				}
	...
	var get_cookie = function() {
		...
		if(markedgroups) {
			...
			return _JSON.parse(unescape(win.localStorage.markedgroups));
		}
		return [];
	};
