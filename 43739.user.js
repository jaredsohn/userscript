...
function post(url, data, callback, options) {
	...
	printMsg('<span class="ttq_village_name" style="display:block;">' + data + '</span>');
	data = encodeURI(data);
	...
	if(at == '') {
		...
		if(bUseServerTime) {  //server time
			...
			if(iServerTimeOffset == false) {  //problem. do nothing.
				...
				("We could not schedule this task, because we were unable to determine server's timezone.", true);
				return false;
			}			
