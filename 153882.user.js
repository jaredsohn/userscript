// ==UserScript==
// @name        MaelstromTools Dev MOD
// @namespace   MaelstromToolsMOD
// @description Just a set of statistics & summaries about repair time and base resources. Mainly for internal use, but you are free to test and comment it.
// @version     0.1.2.3 pre
// @author      Maelstrom, HuffyLuf, KRS_L and Krisan, MOD by unicode
      ...
      function createMaelstromTools() {
        ...
        qx.Class.define("MaelstromTools.Language", {
          ...
          members: {
            ...
            loadData: function (language) {
              ...
              this.Data["Next MCV"] = ["N?chstes MBF", "Tempo restante até ao próximo MCV", "Prochain VCM :", "S?radaki MCV"][l];
        ...
        qx.Class.define("MaelstromTools.Base", {
          ...
          members: {
			...
			mcvInterval: 1,
			repairUnitCounter: 0,
			repairUnitInterval: 2,
			repairBuildingCounter: 1,
			repairBuildingInterval: 2,
			missionTrackerCounter: 0,
			missionTrackerInterval: 5,
			autoCollectCounter: 0,
			runMainTimer: function () {
              try {
                MT_Cache.updateCityCache();

                var self = this;
				if(++this.checkPackageCounter==this.checkPackageInterval) {
					this.checkPackageCounter = 0;
                ...
                if (CCTAWrapperIsInstalled()) {
					if(++this.repairUnitCounter==this.repairUnitInterval) {
						this.repairUnitCounter = 0;
						this.checkRepairAllUnits();
					}
					if(++this.repairBuildingCounter==this.repairBuildingInterval) {
						this.repairBuildingCounter = 0;
						this.checkRepairAllBuildings();
					}
				}
                  
                if(++this.missionTrackerCounter==this.missionTrackerInterval) {
					this.missionTrackerCounter = 0;
                    if(this.missionTracker == null)
                        this.missionTracker = typeof (qx.core.Init.getApplication().getMissionsBar) === 'function' ? qx.core.Init.getApplication().getMissionsBar() : qx.core.Init.getApplication().getMissionTracker(); //fix for PerforceChangelist>=376877
					if (MT_Preferences.Settings.autoHideMissionTracker) {

					  if (this.missionTracker.isVisible()) {
						this.missionTracker.hide();
				...
				if(++this.autoCollectCounter==MT_Preferences.Settings.AutoCollectTimer * 60) {
					this.autoCollectCounter = 0;
            ...
            calculateCostsForNextMCV: function () {
              try {
                ...
                if (!this.mcvPopup) {
                  ...
                  this.mcvPopup.setPosition("bottom-right");
                  ...
                  var target = qx.core.Init.getApplication().getBaseNavigationBar();
                ...
                this.mcvInfoLabel.setValue(Lang.gt("Next MCV") + " ($ " + MaelstromTools.Wrapper.FormatNumbersCompact(creditsNeeded) + ")");
