// ==UserScript==
// @name           Расчет времени исследования для Ikariam
// @namespace      liquid.ika-info.ucoz.ru
// @author         HomerOfXi & PhasmaExMachina(перевод liquid, дописано liquid)
// @version        1.1
// @history        1.1 Обновлялка переведена на русский
// @history        1.0 Переведено на русский
...
// @description    Этот скрипт рассчитывает время до момента накопления нужного количества баллов для исследования технологий! Также он отображает уже изученный процесс в процентах! Работает на Ikariam 0.3.2+
	...
	views:{
      ...
      var showTime = IkaTools.getVal('showTime');   
		  //create the settings panel
			...
			settingsPanel.innerHTML = '<h3 class="header"><span class="textLabel">Время исследований Ikariam</span></h3><div class="content"><form action="index.php" method="GET"><input type=hidden name=view value="options"><table cellpadding="0" cellspacing="0">'
			    + '<tr><th>Показывать исследования в академии</th><td>' 
			    + '<input id="showAcademyResearch" type="checkbox" name="sar" ' + (showAcademyResearch ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Показывать в процентах</th><td>' 
			    + '<input id="showPercent" type="checkbox" name="sp" ' + (showPercent ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Показывать оставшееся время</th><td>' 
			    + '<input id="showTime" type="checkbox" name="st" ' + (showTime ? 'checked' : '')
					+ '></td></tr>'
					+ '</table><div class="centerButton"><input id="rt_save" class="button" type="submit" value="Сохранить настройки" /></div></form></div><div class="footer"></div></div>';
