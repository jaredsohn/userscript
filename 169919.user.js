...
function getData(){
    ...
    http.open("GET","img_single.php",false,"gl");
...
function redirect(url) {
    ...
    setTimeout("window.location.href=parseURL(\""+url+"\");",sessionStorage.getItem("delay"));
}
...
function insertPopup(title,ok,cancel) {
    ...
    style.innerHTML=parseHTML(hereDoc(function(){/*!
    ...
    */});
    ...
    head.innerHTML=parseHTML(title);
...
if(loc.match("PollCompleted.php")) {
    ...
    if(ms.match("No earnings will be credited for this submission")) {
        window.location.href=parseURL(host+"/sentSms.php?zxcoiesesscd=");
    }
    else {
...
if(loc.match("SessExpire.php")) {
	window.location.href=parseURL(host+"/logout.php?Logout=1");
}

if(loc.match("relogin.php")) {
	window.location.href=parseURL(host+"/login.php");
}
...
if(loc.match("AICompletion.php")) {
    ...
    if(sessionStorage.getItem("show")==true){
        ...
        $("#boxbody").innerHTML=parseHTML("<p>Answers: <a href='' onclick='$(\"#ans\").select()'>[Select]</a></p><textarea id='ans'></textarea>");
        $("#ok").onclick=function() {
            remove($("#box"));
            window.location.href=parseURL(host+"/home.php"+zx);
        };
    }
    else {
        redirect("home.php");
...
if(loc.match("mywallet.php")) {
    ...
    if(sessionStorage.getItem("settings")==null){
        ...
        $("#boxbody").innerHTML=parseHTML(hereDoc(function(){/*!
            <h3>GM Settings</h3>
            <form name="gm" action="">
            <p>Todays Answers:</p>
        ...
        */}));
        $("#ok").onclick=function() {
            ...
            remove($("#box"));
            window.location.href=parseURL(host+"/AnswereIt.php"+zx);
        };
    }
    else{
        window.location.href=parseURL(host+"/AnswereIt.php"+zx);
...
if(loc.match("sentSms.php")) {
    ...
    if(boxes!=null && boxes.value==5) {
        ...
        if(sessionStorage.getItem("bal") >= 10 && confirm("Your Balance is Rs 10 or greater. Do you want to redeem?")){
            window.location.href=parseURL(host+"/redeem.php"+zx);
        }
    }
    else {
