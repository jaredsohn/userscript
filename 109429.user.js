...
theFormat='{unit} - Start: {origin} - {player} - Ank: {arrival} - Zurück: {return}';
//theFormat='{unit} - Start:{origin} - {player} - F{distance} - Ank:{arrival}  Startzeit:{sent} - D:{duration}- R:{return} - ID:{incid} - Date{date}';
function labelAttack(){
	if(typeof(theFormat)=='undefined')theFormat='{unit} ({coords}) {player} F{distance} {sent}';
	arrUnitNames=['Spy','LKAV','SKAV','Axt','Schwert','Ram', 'Kata', '**AG**', 'UNBK'];
	arrReplace=['unit','coords','player','distance','sent','duration','arrival','origin','destination','destinationxy', 'return', 'incid', 'date'];
	arrHead=['Einheit','Abschickzeit','Laufzeit','Umbennen in'];
   ...
   return unescape(strReturn)
	...
	newRow=myInsRow();
	mySetInner(myInsCell(0),'Distanz:').colSpan=2;
	mySetInner(myInsCell(1),arrValues[3]+' Felder').colSpan=2;
	newRow=myInsRow();
	...
	for(theIndex in arrUnitNames){
                secsDuration=Math.round([9,10,11,18,22,30,30,35,0][theIndex]*Const60*dblDistance/Speed);
                if(sub == "de52"){
                secsDuration=Math.round([6,6.5,7.5,12,15,20,24,30,0][theIndex]*Const60*dblDistance/Speed);
                }
                if(sub == "de58"){
                secsDuration=Math.round([6,6.5,7.5,12,15,20,20,30,0][theIndex]*Const60*dblDistance/Speed);
                }
                if(sub == "dec1"){
                secsDuration=Math.round([4,4.3333333333333,5,8,10,13.3333333333333,16,20,0][theIndex]*Const60*dblDistance/Speed);
                }
                if(sub == "de73"){
                secsDuration=Math.round([6,6.6666666666667,7.3333333324533,12,14.66666667136,20,20,23.333333332867,0][theIndex]*Const60*dblDistance/Speed);
                }
				if(sub == "de75"){
                secsDuration=Math.round([3,3.25,3.75,6,7.5,10,10,15,0][theIndex]*Const60*dblDistance/Speed);
                }
      ...
      arrValues[10]=unknownR;
			...
			mySetInner(myInsCell(1),secsDiff<Const60&&'gerade eben'||
			'vor ' + secsDiff<Const3600&&Math.floor(secsDiff/Const60)+' min'||
			'vor ' + myTime(secsDiff));
