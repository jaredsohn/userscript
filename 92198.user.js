// ==UserScript==
// @name             KOCAttack - Extra Features Spanish!
// @version          0.4.0
...
function inspect(obj, maxLevels, level){
  var str = '', type, msg;
    ...
    if(maxLevels < 1)     
        return '<font color="red">Error: El numero de nivele debe ser > 0</font>';
...
ShowOptionsDialog:function() {
	...
	div.innerHTML="<form><table>"+
		...
		"Orden de Ataque: <select id='KOCAttackOrder'><option value='mostTroops'>Usar el maximo de tropas primero</option><option value='closest'>Terminar los objetivos primero</option></select><br />"+
		"Tipo de Ataque: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbaros "+
		"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Tierras "+
		"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Ciudad "+
		"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transporte<br> "+
		"<br />"+
		"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> segundos entre cada envio de ataque <br />"+
		"Tiempo entre envio al <u>mismo objetivo</u>...<br />"+
		"<div style='margin-left: 40px'>"+
		"Atacando Barbaros:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />hrs<br />"+
		"Atacando Tierras:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />hrs<br />"+
		" Atacando Ciudad:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />hrs<br />"+
		" Transportando:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />mins"+
		"</div>"+
		"<br />"+
		"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distancia maxima de la ciudad a atacar.<br />"+
		"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Solo enviar los ataques desde la ciudad que se programó al principio.<br />"+
		"<br />"+
		"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Reintentar la marcha si da error desconocido o error de exceso de trafico(refresca despues de  cambiar esta opcion)..<br />"+
		"Abrir esta URL cuando estemos bajo ataque...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
		...
		"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Eliminar mensajes de ataques a Barbaros/Tierras .<br />"+
		"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> informes de ataque maximos en ventana de informes.<br />"+
		"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Desactivar la visualización de ataques  de la página de informes..<br />"+
		
		"</td></tr>"+
		"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
		"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de la población disponible antes de hacer el entrenamiento automatico..<br />"+
		"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de felicidad antes de cobrar oro automaticamente.<br />"+
		"</td></tr>"+
		
		"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
		"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Ayudar automáticamente  a miembros de tu alianza en la construcción / investigación.<br />"+
		"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Ocultar  informes en el chat después de auto-ayuda.<br />"+
		"<input id='KOCAttackAutoPublishHelpRequests' type='checkbox' "+(this.options.autoPublishHelpRequests?'checked':'')+" /> Automáticamente acelerar la construcción o la investigación mediante la publicación de solicitudes de ayuda en el muro.<br />"+
		"Si lo anterior se comprueba, qué configuración de privacidad debemos usar? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Siempre</option><option value='50'>Amigos de amigos</option><option value='40'>Solo amigos</option><option value='10'>Solo yo</option></select><br />"+
		
		"</td></tr>"+
		
		"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
		"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> segundos entre cambio de ciudades.<br />"+
		"Recorer todas las ciudades <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> veces y esperar "+
		"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /> segundos antes de actualizar.<br />"+
		"Atacar Ciudades desde: "+okCitiesHtml+'<br />'+

		"<tr><td valign='top' align='center'></td><td valign='top'>"+
		"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />%en demora aleatoria  (para parecer más humano) .<br />"+
		"Importar/Exportar: Pega o copia los ajustes aqui...                  <br /><textarea id='KOCAttackImport'></textarea>"+
		"<a class='button20' id='KOCAttackImportButton'><span>Importar</span></a> <a class='button20' id='KOCAttackExportButton'><span>Exportar</span></a><br />"+
		"</td></tr>"+

		"</table>"+
		
		"<a id='KOCAttackOptionsSave' class='button20'><span>Grabar</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Cancelar</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Reset opciones</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Resetear todo!</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Borar todos los ataques guardados</span></a></form>";
	var t=this;
...
//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
	if(autoAttack) {
		toggle.innerHTML='<span>Auto Ataque - On</span>';
	} else {
		toggle.innerHTML='<span>En taberna - Off</span>';
	}
},
...
BulkAddAttackLink:function(box) {
	...
	AddHtml(bulkAddDiv,"Copia y pega aquí (ej. 343,434) una coordenada en cada línea...<br />Nota: Añadir solo objetivos para el número actual de tropas elegidas en esta pantalla.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Sobreescribir si el ataque ya existe <br />");

	
	// radio boxes for defining bulk coordinate type
	AddHtml(bulkAddDiv,"<hr />");
	AddHtml(bulkAddDiv,"Tipo de ubicaciones (Todas las coordenadas debe ser para este tipo):<br />");
	var arrData = [["Camp", "Barbaros"],["Wilderness", "Tierras virgenes"]];
	...
	AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Esta es una ola inicial de suicidio para acabar con las trampas en una tierra.<br />");
	...
	bulkAdd.innerHTML='<span>Añadir</span>';
	bulkAddDiv.appendChild(bulkAdd);
	bulkAdd.addEventListener('click',function() {
		// Determine location type
		var locationType = "Barbaros"; // Default value
		...
		window.alert('Añadidas '+added+' coordenadas');
	},false);
	AddHtml(bulkAddDiv,"<br />");

	var bulkAddAttackLink=document.createElement('a');
	bulkAddAttackLink.className='buttonDown20';
	bulkAddAttackLink.innerHTML='<span>Añadir coordenadas</span>';
...
HideAttackEfforts:function() {
	...
	var a=document.createElement('a');
	a.innerHTML='Ocultar/Mostar - Esfuerzos de ataque';
	//a.href='javascript:;';
...
OnAttackBoxAppear:function(box) {
	...
	AddText(div2,'Ignorar en la lista de ataques');
...
//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
	...
	if(autoAttack && autoAttack.x==xy[0] && autoAttack.y==xy[1] && !ignore.checked) {
		...
		&& btnMarch.style.opacity!=0.5
			...
			this.Log('auto ataque, na hay suficientes tropas/caballeros: '+xy[0]+','+xy[1]+
				", disponibilidad de caballeros:"+(knightSelect.options.length-1)+", Insuficientes tropas/recursoss:"+notFullTroops+'/'+notFullResources+', necesitas:'+totalTroops+'/'+totalResources);
			this.DoUnsafeWindow("Modal.hideModal();");
...
FindLevelFromMessages:function(attack) {
	...
	for(var a=0; a<attack.messages.length; a++) {
		...
		if(!m) {
			this.Log("No se puede encontrar información de nivel y ubicación  en los informes en caché!");
			continue;
		}
...
GetAttackDelay:function(attack) {
	...
	if(!levelInfo) {
		this.Log("Imposible calcular retardo de ataque : Informacion de nivel no encontrado en las coordenadas ("+attack.x+","+attack.y+"). Supuesto retraso 0.");
	}else{
		if(levelInfo.type=='Camp') {
			...
			if(tempAttack.type==0) {
				...
				this.Log("Tipo de atque desconocido para las coordenadas ("+attack.x+","+attack.y+"). Supuesto retraso de 0.");
			}
		}else{
			this.Log("Tipo de atque desconocido para las coordenadas ("+attack.x+","+attack.y+"). Supuesto retraso de 0.");
		}
...
FindBiggestCampAttack:function(attacks,currentTroops,currentResources) {
	...
	this.Log("Actual nivel del punto de reunion: "+this.currentRallyPointLevel+" Marchas actuales: "+this.currentMarchesNum);
	...
	for(var a=0; a<attacks.length; a++) {
		...
		if(!levelInfo) {
			this.Log("Ataque nulo: Informacion de nivel no encontrado para las coordenadas ("+attack.x+","+attack.y+")");
			continue;
		}
		
		if(levelInfo.type=='Camp') {
			if(this.options.attackTypeCamp!=true) {
				this.Log("Ataque nulo: Barbaros nulos! para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		} else if(wilderness[levelInfo.type]) {
			if(this.options.attackTypeWild!=true) {
				this.Log("Ataque nulo: Tierra nula! para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		} else if (levelInfo.type!='Camp' && !wilderness[levelInfo.type]) {
			var tempAttack=this.GetAttack(attack.x,attack.y);
			if(tempAttack.type==0) {
				if(this.options.attackTypeCity!=true) {
					this.Log("Ataque nulo: Ciudad nula para las coordenadas ("+attack.x+","+attack.y+")");
					continue;
				}
			} else if(tempAttack.type==1) {
				if(this.options.attackTypeTransport!=true) {
					this.Log("Ataque nulo: Sin envio de trasporte para las coordenadas ("+attack.x+","+attack.y+")");
					continue;
				}
			}
		} else {
			if(levelInfo.type!='' && levelInfo.type!='Camp') {
				this.Log("Ataque nulo: Tipo de ataque desconocido para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		}
		...
		if(!attack.a.troops) {
			...
			this.Log("Ataque nulo: Tropas no definidas para las coordenadas ("+attack.x+","+attack.y+")");
			continue;
		}
		if(attack.a.ignore) {
			this.Log("Ataque nulo: Ubicacion ignorada pora las coordenadas ("+attack.x+","+attack.y+")");
			continue;
		}
		if(attack.dist>=this.options.attackMaxDistance) {
			this.Log("Ataque nulo: Distancia demasiado lejos para las coordenadas ("+attack.x+","+attack.y+")");
			continue;
		}
		if(this.options.lockAttackFromCity) {
		...
		if(attack.a.suicidewave && attack.a.currenttattackwavetype == "suicide"){
			if(available_marches_num >= 2 && this.currentRallyPointLevel >= 2){
				...
				this.Log("Ataque nulo: Todos los espacios en punto de reunión se están usando siendo imposible lanzar tanto la ola de suicidio como la segunda ola a las coordenadas ("+attack.x+","+attack.y+")");
				break;
			}
		}
		...
		if(available_marches_num<1){
			this.Log("Ataque nulo: Todos los espacios en punto de reunión se están usando siendo imposible lanzar un ataque para las coordenadas ("+attack.x+","+attack.y+")");
			break;
		}
		
		if(!this.IsEnoughTroops(currentTroops,troops_to_send)) {
			this.Log("Ataque nulo: No hay suficientes tropas para las coordenadas ("+attack.x+","+attack.y+")");
			continue;
		}
		if (attack.a.type==1) {
			if(!this.IsEnoughResources(currentResources,attack.a.resources)) {
				this.Log("Ataque nulo: No hay suficientes recursos para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		}
		var lastAttack;
		...
		if(levelInfo.type=='') {
			if(attack.a.type==0) {
				if(lastAttack<this.options.attackSecsSinceLastCity) {
					this.Log("Ataque nulo: último ataque ("+lastAttack+") < this.options.attackSecsSinceLastCity ("+this.options.attackSecsSinceLastCity+") para las coordenadas ("+attack.x+","+attack.y+")");
					continue;
				}
			} else {
				if(lastAttack<this.options.attackSecsSinceLastTransport){
					this.Log("Ataque nulo: último ataque ("+lastAttack+") < this.options.attackSecsSinceLastTransport ("+this.options.attackSecsSinceLastTransport+") para las coordenadas ("+attack.x+","+attack.y+")");
					continue;
				}
			}
		} else if(levelInfo.type.toUpperCase()=='C') {
			if(lastAttack<this.options.attackSecsSinceLastCamp) {
				this.Log("Ataque nulo: último ataque ("+lastAttack+") < this.options.attackSecsSinceLastCamp ("+this.options.attackSecsSinceLastCamp+") para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		} else  {
			if(lastAttack<this.options.attackSecsSinceLastWild) {
				this.Log("Ataque nulo: último ataque ("+lastAttack+") < this.options.attackSecsSinceLastWild ("+this.options.attackSecsSinceLastWild+") para las coordenadas ("+attack.x+","+attack.y+")");
				continue;
			}
		}
		var armySize=0;
		...
		if(attack.a.suicidewave) {
			...
			if(suicideArmySize<=0) {
				this.Log('Ola de suicidio no válida por tamaño del ejército para estas coordenadas(no es suficiente ) ('+attack.x+','+attack.y+')');
				continue;
			}
		}
		if(armySize<=0) {
...
//~~~ problem here when under attack, the city isn't marked as selected?
	...
	if(!cityA || this.autoAttackCitiesDone>=this.options.autoAttackCitiesDoneMax) {
		...
		if(this.nextAutoAttackTimeout==null) {
			...
			this.Log('No se puede cambiar la ciudad, a punto de atacar o cambiar de ciudad');
		}
//		this.autoAttackCityUpto++;
	}
},
...
NextAutoAttack:function() {
	...
	if(!this.options.okCities[this.autoAttackCityUpto]) {
		this.Log('Ignorar la ciudad situada en las opciones:'+this.autoAttackCityUpto);
		this.NextAutoAttackCity();
		return;
	}
...
SetupMenu:function() {
	var t=this;
	GM_registerMenuCommand('KOCAttack - Opciones',function() {
		t.ShowOptionsDialog();
	});
	GM_registerMenuCommand('KOCAttack - Auto Ataque',function() {
		t.ToggleAutoAttack();
	});
