...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('MONSTER!!', eventHideShow, mouseMainTab);
  tabManager.init (mainPop.getMainDiv());
  actionLog ("MONSTER actuellement te pete les dents... ");
...
Tabs.ActionLog = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<DIV class=pbStat>ACTION LOG - VERSION: '+ Version+'</div><DIV style="height:355px; max-height:355px; overflow-y:auto">\
...
Tabs.RESET = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    try {      
      m = '<DIV style="height:550px; max-height:550px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
		<center><BR><BR><HR><BR><BR>Upgrade reglage a 25k de pierres minmum<BR><BR>Enhance reglage a 45k de pierres minimum<BR><BR><img src="http://fc04.deviantart.net/fs71/i/2010/112/0/f/The_Kraken_by_JSMarantz.jpg" /></center></div>';
...
var DeleteThrone = {
	...
	init : function(){
		...
		if(Options.ThroneDeleteItems) {
			setInterval(t.startdeletethrone, 1*60*120);
		}
	},
...
Tabs.upgrader = {   
	...
	init : function (div){
       ...
       // header stuff
       var m = '<Div><DIV id=pbUpgrader class=pbStat>MONSTER !!!!!!!!!!!!!!!!!!!!!</div><TABLE id=pbupgrader width=100% height=0% class=pbTab><TR align="center">';
       ...
       m += '<option value="1">nul</option>';
       m += '<option value="2">bof</option>';
       m += '<option value="3">moyen</option>';
       m += '<option value="4">monster!</option>';
       m += '<option value="5">chuck!</option>';
       m += '</select></div></td>';
       m += '<td><div id=pbEnhanceCost class=indent25> <br> </div></td></tr>';
       
       m += '<tr><TD><INPUT id=pbSwithUpgrade type=checkbox '+ (upgradeData.switchToUpgrade?' CHECKED':'') +'\>  passer de enchance a upgrade quand manster decide de vous up un item!!</td></tr>';
       m += '</tr></table>';
	   ...
	   document.getElementById ('MaxQuality').addEventListener ('click', function() {
    	  if (this.value >= 0) {
    		  logit(" espoir de Enhance selectionne: " + this.value);
       ...
       t.setResult(Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] + " aetherstones restant.");
	   t.setStatus("Loading ....");
	   var d = 5;
	   if (Seed.queue_throne != null && Seed.queue_throne.end != null)
	   {
	      ...
	      t.setStatus("attendez " + n.toLocaleTimeString() + " que Monster repare votre connerie");
   ...
   setUpgradeItem : function(item)
   {
	  ...
	  {
         logit(" item ridicule en selection: " + item + "/" + unsafeWindow.kocThroneItems[item].name);
   ...
   setEnhanceItem : function(item)
   {
      ...
      if (item > 0 && unsafeWindow.kocThroneItems[item] != null)
      {
         logit(" Item indigne a enhance: " + item + "/" + unsafeWindow.kocThroneItems[item].name);
	...
	togglePower: function(obj){
	 ...
	 if (upgradeData.active == true) {
		...
		t.setStatus("ATTEND MOSTER!!");
	} else {
		upgradeData.active = true;
		obj.value = "Upgrade = ON";
		t.setStatus("VAS Y MONSTER!!");
	}
	  ...
	  try {
	    ...
	    if ( ti <= 0)
	    {
          ...
          else
          {
	        ...
	        retryTime = ti + 5;	        
	        var n = new Date(t.repairEnd *1000);
	        t.setStatus("attendez " + n.toLocaleTimeString() + " pour que Monster repare votre connerie");
        }
  ...
  doEnhance : function() {
		...
		{
			t.setStatus("monster a un cadeau pour toi");
			actionLog('ton item '+unsafeWindow.kocThroneItems[eItem].name + ' vient de se faire eclater au niveau ' + y.quality);
		...
		if ( Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] < 45000)
		{
		   t.setStatus("pas de pierre tu ne serais pas un peu dans la merde?");
		   return;	
		}
	    ...
	    t.setStatus("tentative de enhance");
      	new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
			method: "post",
			parameters: params,
			loading: true,
			onSuccess: function (transport) {
				...
				if(rslt.ok){
					...
					if (rslt.success)
					{					
					   actionLog('item ameliore par Monster '+unsafeWindow.kocThroneItems[eItem].name + ' a la qualite ' + rslt.item.quality);
					   ...
					   saveUpgradeStats();
					   y.name = y.createName();
					   t.setResult("WAAAZZZAAAAA Enhance offert par monster!.  "  + rslt.aetherstones + " aetherstones used, " +  Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] + " remaing."  );
				    ...
				    {
					   actionLog('nananere echec! '+unsafeWindow.kocThroneItems[eItem].name);
					   ...
					   saveUpgradeStats();
					   y.isBroken = true;
					   y.brokenType = "qualite";
					   y.name = y.createName();
					   t.setResult("monster a bouffe ton item!.  "  + rslt.aetherstones + " aetherstones used, " +  Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] + " remaing.");
					   t.setStatus("allez monster repare ca...");
					   t.doRepair(y.id);
				    }
				    if (rslt.gems > 0)
				    {
			    ...
			    {
				  logit("resultat de Enhance:" + inspect (rslt, 3, 1));
				  t.setStatus("action impossible en attente du prochain cycle");
			    }
			    return;
			},
			onFailure: function () {
			   logit("erreur de Enhance.  attendre le prochain cycle.");
			   t.setStatus("requete impossible. attendre prochain cycle");
			   return;
			},
		});
	} catch (e) {
	...
	doUpgrade : function() {
		...
		if ( Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] < 25000)
		{
		   t.setStatus("espece de pauvre t'as plus de pierre");
		   return;	
		}
		...
		params.payment = "aetherstone";
		params.cityId = Seed.cities[0][0];
		t.setStatus("tu demande une chose etrange a monster");
      	new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
			method: "post",
			parameters: params,
			loading: true,
			onSuccess: function (transport) {
				...
				if(rslt.ok){
					...
					if (rslt.success)
					{
					   actionLog('monster reflechie a ta demande '+unsafeWindow.kocThroneItems[uItem].name + ' au level ' + rslt.item.level);
					   upgradeStats.upgradeSuccess[y.quality][y.level]++;
					   saveUpgradeStats();
					   y.level = rslt.item.level;
					   y.quality = rslt.item.quality;
					   y.name = y.createName();
					   t.setResult("Upgrade accepte par monster.  "  + rslt.aetherstones + " aetherstones used, " +  Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] + " remaing.");
					   t.setStatus("Waiting for next cycle");
					   // update the cost line
					   t.setUpgradeItem(upgradeData.item);
				    }
				    ...
				    {
					   actionLog('monster a bouffe ton item '+unsafeWindow.kocThroneItems[uItem].name);
					   ...
					   y.status = rslt.item.status;
                       y.name = y.createName();
                       t.setResult("monster refuse de monter ton item  "  + rslt.aetherstones + " aetherstones used, " +  Seed.resources["city" + Seed.cities[0][0]]["rec5"][0] + " remaing.");
                       t.setStatus("Monster repare tes conneries! ");
				...
				else
				{
				    ...
				    logit("resultat:" + inspect (rslt, 3, 1));
			    }
			    //unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
			    return;
			},
      	...
      	new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
			...
			onSuccess: function (transport) {
				var rslt = eval("(" + transport.responseText + ")");
				   //logit("rslt: " + inspect(rslt,3,1));
				   if(rslt.ok){
				 	 actionLog('monster repare... '+unsafeWindow.kocThroneItems[rItem].name);
					 ...
					 var n = new Date(t.repairEnd *1000);
	                 t.setStatus("monster bosse ... il finira Ã  " + n.toLocaleTimeString());
				   ...
				   {
					   if (rslt.msg == "Item entier")
					   {
						   t.setStatus("item est de nouveau dispo.");
						   t.repairId = rItem;
						   t.repairEnd = 0;
						   t.clearRepair();
					   }
	...
	clearRepair : function () {
	    ...
	    if (timeUntilDone <= 0)
		{
	      ...
	      if (t.repairId != 0 && unsafeWindow.kocThroneItems[t.repairId]  != null)
	      {
	         ...
	         {
		        t.setStatus("monster a fini de reparer!");
	         }
