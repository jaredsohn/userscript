...
// @version        1.1.0
// @author         MeXaon fixed by bluelovers
// @namespace      Travian
// @description    Extended Map fixed by bluelovers
...
var ScriptAutor='MeXaon fixed by bluelovers';
...
//	var grounddata=find('//div[@id="map_content"]/map/area[@onmouseover]',XPList);
	var grounddata=find('//div[@id="map_content"]/map/area[@xy]',XPList);
...
//	for(var i=0;i<grounddata.snapshotLength;i++){
	...
	for(var i = 0; i<mapr; i++){
		...
		var idx = parseInt(i*mapr+j)+maps;
		var cgd=grounddata.snapshotItem(idx);
//		var onmo=cgd.getAttribute('onmouseover');
		var onmo=cgd.getAttribute('id');
		var groundhref=cgd.getAttribute('goto');
		var typeground=groundimg.snapshotItem(i).src.match(/img\/un\/m\/(\w)(\d+)\.gif/i).pop();
		var ni='-',na='-',nv='',np='-',mt='';
				...
				case 'o':
					switch(v){
						...
						case '2':
							...
							break;
						case '3':
							cont='+25%<img src='+imgp('img/un/r/1.gif')+'> +25%<img src='+imgp('img/un/r/4.gif')+'>';
							break;
						case '4':
						case '5':
							...
							break;
						case '6':
							cont='+25%<img src='+imgp('img/un/r/2.gif')+'> +25%<img src='+imgp('img/un/r/4.gif')+'>';
							break;
						case '7':
						case '8':
							...
							break;
						case '9':
							cont='+25%<img src='+imgp('img/un/r/3.gif')+'> +25%<img src='+imgp('img/un/r/4.gif')+'>';
							break;
						case '10':
						case '11':
...
//		}
		...
		var who = cgd.getAttribute('details');
		if (who) {
			ni = who.name;
			na = who.ally;
			nv = who.dname;
		...
		if(nv!=''){
			mt='mt'+(idx+1);
			var vill=elem('TD','mt',mt,'','<a mt='+mt+' href='+groundhref+'>'+nv+'</a><a  mt='+mt+' target="_blank" href='+groundhref+' class="done"><img mt='+mt+' src='+imgnw+'></a> ' + cont);
...
//			rowt.appendChild(elem('TD','','','',''));
			...
			cgd.setAttribute('mt',mt);
			cgd.addEventListener('mouseover',eventtableover2,false);
			cgd.addEventListener('mouseout',function(){marker.style.display='none';},false);
	...
	if (doneok) document.getElementById('lmid1').appendChild(table);
};
...
//			var cgd=grounddata.snapshotItem(parseInt(i*mapr+j)+8);
			var cgd=grounddata.snapshotItem(parseInt(i*mapr+j)+maps);
			groundimg.snapshotItem(parseInt(i*mapr+j)).src.match(/img\/un\/m\/(\w)(\d+)\.gif/i).pop();
			var t=RegExp.$1;
			var v=RegExp.$2;
			switch(t){
				case 'o':
...
//	for(var i=0;i<8;i++){
	for(var i=grounddata.snapshotLength-7;i<grounddata.snapshotLength-1;i++){
		var cgd=grounddata.snapshotItem(i);
//		var kid=cgd.getAttribute('onclick').match(/z=(\d+)/).pop();
		var kid=cgd.getAttribute('href').match(/z=(\d+)/).pop();

		cgd.setAttribute('goto','d=' + (parseInt(kid) + (despl[i % 4] * (d - 1))));
...
//	myajax('ajax.php?action=map_content&z='+mid, function(z){
	myajax('ajax.php?f=k7&x=' + x + '&y=' + y + '&xx=' + xx + '&yy=' + yy, function(z){
		if (z.responseText) document.getElementById('map_content').innerHTML=z.responseText;
		drawNewMap();
		infomap();
		showtype();
		});
...
function eventmappopup(ev){
	...
	divpopup.innerHTML='';
	divpopup.innerHTML ='<a href='+link+' class="done"><img src="'+imgenter+'" height="16"></a>'
						+ '<a target="_blank" href='+link+' class="done"><img src="'+imgnw+'" height="16"></a>'
						+ '<a href="a2b.php?z='+kid+'" target="_blank" class="done"><img src="'+imgatt+'" ></a>';
...
function eventtableover2(ev){
	marker.className=this.getAttribute('mt');
	marker.style.display='';
};
