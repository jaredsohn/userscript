// ==UserScript==
// @name           Google Rusian/English Language Changer
// @namespace      http://www.karpach.com
// @description    Add a select-box for the lr parameter on google result pages.
// @version        1.0
...
function insertSelector(list) {	
	...
	var hl = document.getElementsByName('lr');
	...
	sel.addEventListener("change", function (event) {
		var hl = document.getElementsByName('lr')
		if (hl.length > 0) {
			hl[0].value = this.options[this.selectedIndex].value;
		}
		else
		{
			var hl = document.createElement('input');
			hl.setAttribute('name','lr');
			hl.setAttribute('type','hidden');
			hl.setAttribute('value',this.options[this.selectedIndex].value);
			var googleSpan = document.getElementById('tophf');
			googleSpan.appendChild(hl);
		}
