// ==UserScript==
// @name			Ikariam Spy Tracker (beta versions)
...
/*
1.0.2b1/19.04.2011
	Feature: Dockable/resizable pad with position and size memory
	Feature: Odd/even rows displayed with different colors
	Fix: Pad not displayed if trying to execute spy mission with no spies in the target city
	Fix: Wrong individual resource amounts displayed in Pillage view. Total amount is correct.
	
1.0.1/29.03.2011 
	Fix: Wrong report ages and negative production rates sometimes displayed in city tooltip
1.0.0/28.03.2011	
	Feature: Support for Ikariam v0.4.2 with the whole new spy system
...
0.9.1/29.11.2010 
	Feature/fix: Multiwarehouse support. Warehouse level is the sum of all the warehouse levels
0.9.0/29.11.2010
	Feature: Mouseover tooltip displaying city details and associated spy reports
...
0.1.9/5.10.2010	
	Fix: Calculation of information age should use server time not local
0.1.8/4.10.2010
	Feature: Multidomain/server support, separate Spy Tracker databases for each.
0.1.7/4.10.2010	
	Fix: Safelevels of ACTIVE cities are slightly incorrect (~1 ship)
0.1.6/4.10.2010	
	Feature: Set city active/inactive by clicking the total lootable value.
	Inactive city name displayed in italic. Warehouse safelevels calculated according to this setting.
0.1.5/4.10.2010	
	Feature: Pad displayed in safehouse view
...
0.1.4/4.10.2010	
	Bug fix: Conflict with "City Select Re-orderer" script. Spy Tracker was unable to initialise properly if "City Select Re-orderer" was not running.
0.1.3/4.10.2010	
	Some dead code removed and more debugging messages added
0.1.2/3.10.2010
	Syslogging enabled for beta testing purposes
0.1.1/3.10.2010
	Fix: Invalid link and journey time displayed if no city information available
0.1.0/3.10.2010
	Initial Release 
*/ 
//
...
/* Internal todo
- Resource growth rates as the seconds row of the tooltip
*/
...
const displayDefaultTop = 20;
const displayDefaultLeft = 0;
const displayDefaultWidth = 215;
const displayDefaultHeight = 350;
...
const IMAGES =
{
	...
	dock:				'data:image/gif;base64,',
	...
	resize:				'data:image/gif;base64,',
	...
	undock:				'data:image/gif;base64, '
};

const SETTINGS = 
{
	sortByColumn: 'sortByColumn',
	displayDocked: 'displayDocked',
...
function isNumBetween(value, min, max) {
...
function appendClass(elem, newClass) {
...
function position(element) {
	this.borderWidth = parseInt(element.style.borderWidth, 10);
	this.left = 0;
	this.top = 0;
	this.right = 0;
	this.bottom = 0;
	...
	this.right = this.left + parseInt(element.scrollWidth) - 1;
	this.bottom = this.top + parseInt(element.scrollHeight) - 1;
}
...
position.prototype.isCursorOnBorder = function(cursorX, cursorY) {
	var result = BORDER_POSITION.none;
		
	// on left border
	if(isNumBetween(cursorY, this.top, this.bottom) && isNumBetween(cursorX, this.left, this.left+this.borderWidth)) {
		result = BORDER_POSITION.left;
	}
	// on right border
	if(isNumBetween(cursorY, this.top, this.bottom) && isNumBetween(cursorX, this.right-this.borderWidth, this.right)) {
		result = BORDER_POSITION.right;
	}
	// on top border
	if(isNumBetween(cursorX, this.left, this.right) && isNumBetween(cursorY, this.top, this.top+this.borderWidth)) {
		result = BORDER_POSITION.top;
	}
	// on bottom border
	if(isNumBetween(cursorX, this.left, this.right) && isNumBetween(cursorY, this.bottom-this.borderWidth, this.bottom)) {
		result = BORDER_POSITION.bottom;
	}
	// on bottom right border
	if(isNumBetween(cursorX, this.right-10, this.right) && isNumBetween(cursorY, this.bottom-10, this.bottom)) {
		result = BORDER_POSITION.bottomRight;
	}
	return result;
}
...
function ikariamCityViewGetBuildingLevel(building) {
	var result = 0;
	var elements = document.getElementsByClassName(building);
	for (var i=0; i < elements.length; i++) {
		...
		// if there are multiple buildings the levels are summed
...
function ikariamCityViewGetIsland(){
	var elements = document.getElementsByClassName("island");
	if (elements[0]){
...
spyTrackerReport.prototype.readFromIkariam = function(resourcesTable) {
	var rows = resourcesTable.getElementsByTagName('tr');
	var resIndex = -1;
	for (var i = 1; i < rows.length; i++) {
		resIndex = -1;
		if(rows[i].innerHTML.indexOf('icon_wood.gif') > 0) resIndex = 0;
		else if(rows[i].innerHTML.indexOf('icon_wine.gif') > 0) resIndex = 1;
		else if(rows[i].innerHTML.indexOf('icon_marble.gif') > 0) resIndex = 2;
		else if(rows[i].innerHTML.indexOf('icon_glass.gif') > 0) resIndex = 3;
		else if(rows[i].innerHTML.indexOf('icon_sulfur.gif') > 0) resIndex = 4;
		if (resIndex >= 0) {
			var resStr = getElementsByClass(rows[i], "count", false);
			this.resources[resIndex] = parseInt(resStr[0].innerHTML.replace(/,/g,''));		
		}
	}
}
...
function spyTrackerSettings() {
	...
	this.displayDocked = true;
	this.displayTop = displayDefaultTop;
	this.displayLeft = displayDefaultLeft;
	this.displayWidth = displayDefaultWidth;
	this.displayHeight = displayDefaultHeight;
}

spyTrackerSettings.prototype.toString = function() {
	var result = SETTINGS.sortByColumn + '=' + this.sortByColumn;
	result += ';' + SETTINGS.displayDocked + '=' + this.displayDocked;
	result += ';' + SETTINGS.displayTop + '=' + this.displayTop;
	result += ';' + SETTINGS.displayLeft + '=' + this.displayLeft;
	result += ';' + SETTINGS.displayWidth + '=' + this.displayWidth;
	result += ';' + SETTINGS.displayHeight + '=' + this.displayHeight;
	return result;
}
...
spyTrackerSettings.prototype.fromString = function(str) {
	...
	for (var i = 0; i < settings.length; i++) {
		...
		if(setting[0] == SETTINGS.sortByColumn) this.sortByColumn = setting[1];
		if(setting[0] == SETTINGS.displayDocked) this.displayDocked = stringToBoolean(setting[1]);
		if(setting[0] == SETTINGS.displayTop) this.displayTop = parseInt(setting[1], 10);
		if(setting[0] == SETTINGS.displayLeft) this.displayLeft = parseInt(setting[1], 10);
		if(setting[0] == SETTINGS.displayWidth) this.displayWidth = parseInt(setting[1], 10);
		if(setting[0] == SETTINGS.displayHeight) this.displayHeight = parseInt(setting[1], 10);
	}
}
...
spyTrackerDB.prototype.viewSpyMissions = function() {
	...
	if(compactSpyMissionView) {
		...
		if(!missionForm) return; // city missing use case at least
	...
	if(sc) {
		...
		if(sc.spyTrackerReports[0]) {
			...
			for (var i in loot) {
				...
				var timeDiffAsDays = Math.abs(sc.spyTrackerReports[0].calculateTimeDiffAsDays(new spyTrackerReport()));
				lootAmount.textContent = Math.ceil(loot[i]+growthRates[i]*timeDiffAsDays);
...
spyTrackerDB.prototype.updateSpyTrackerPad = function() {
	...
	for (var i in this.spyCities) {
		...
		var tr = htmlTableRow();
		if (i % 2 == 0)
			appendClass(tr, 'spyTrackerTableRowEven');
		else
			appendClass(tr, 'spyTrackerTableRowOdd');
		// selection check box
		/*
		...
		*/
		...
		if(sc.spyWarehouseEnabled()) {
			var urlSpy = '?view=spyMissions&id=' + sc.ID;
			var tdSpy = spyTrackerPadActionTD(IMAGES.spyMission, 'Spy Missions', urlSpy);
		...
		if(sc.pillageEnabled()) {
			var urlPillage = '?view=plunder&destinationCityId=' + sc.ID;
			var tdPillage = spyTrackerPadActionTD(IMAGES.pillage, 'Pillage', urlPillage);
			tr.appendChild(tdPillage);
		}
		else {
			tr.appendChild(htmlTableCell(''));
		}
		...
		var tdClear = spyTrackerPadActionTD(IMAGES.clear, 'Clear total lootable');
		tdClear.setAttribute('name', sc.name);
		tdClear.addEventListener('click', clearButtonOnClickHandler, false);
		...
		if(sc.sendSpyEnabled()) {
			var urlSendSpy = '?view=sendSpy&destinationCityId=' + sc.ID + '&islandId=' + sc.island.ID;
			var tdSendSpy = spyTrackerPadActionTD(IMAGES.sendSpy, 'Send Spy', urlSendSpy);
			tr.appendChild(tdSendSpy);
		}
		else {
			tr.appendChild(htmlTableCell(''));
		}
		...
		var tdRemove = spyTrackerPadActionTD(IMAGES.remove, 'Remove city from Spy Tracker');
		tdRemove.setAttribute('name', sc.name); // TODO: remove should be based on id
		tdRemove.addEventListener('click', removeButtonOnClickHandler, false);
...
function spyTrackerPadActionTD(image, title, url) {
	var result = null;
	var img = htmlImg(image, title);
	img.setAttribute('class', 'spyTrackerActionImage');
	if(url)
		result = htmlTableCell(htmlAHref(url, img));
	else
		result = htmlTableCell(img);
	result.setAttribute('class','spyTrackerActionTD');
...
function spyTrackerCSS() {
	var result = <r><![CDATA[
        <style type="text/css">
				...
				.spyTrackerTableRowEven {
					background-color: transparent;
				}
				.spyTrackerTableRowOdd {
...
function initSpyTrackerPad() 
{
	...
	updateButton.id = 'spyTrackerPadUpdateButton';
	...
	// undock button
	var undockButton = htmlDiv(htmlImg(IMAGES.undock, 'Undock', 17));
	undockButton.id = 'spyTrackerPadUndockButton';
	undockButton.style.display = 'none';
	undockButton.style.cursor = 'pointer';
	undockButton.style.cssFloat = 'right';
	undockButton.style.margin = '5px 5px 0px 0px';
	undockButton.addEventListener('click', spyTrackerPadUndock, false);
	header.appendChild(undockButton);
	
	// dock button
	var dockButton = htmlDiv(htmlImg(IMAGES.dock, 'Dock', 17));
	dockButton.id = 'spyTrackerPadDockButton';
	dockButton.style.display = 'none';
	dockButton.style.cursor = 'pointer';
	dockButton.style.cssFloat = 'right';
	dockButton.style.margin = '5px 5px 0px 0px';
	dockButton.addEventListener('click', spyTrackerPadDock, false);
	...
	tableDiv.setAttribute('id', 'spyTrackerPadTableDiv');
	tableDiv.setAttribute('class', 'content');
	tableDiv.style.overflow = 'auto';
	tableDiv.style.height = displayDefaultHeight + 'px';
	tableDiv.style.width = displayDefaultWidth + 'px';
	...
	if (container == null) {
		syslog('Spy Tracker pad initialisation failed due to missing parent container',1);
		return false;
	}
	...
	// place the pad in the correct place
	...
	if(db.settings.displayDocked) spyTrackerPadDock()
	else spyTrackerPadUndock();
...
function spyTrackerPadUndock(e) {
	syslog('Spy Tracker - unDock activated', 1);
	
	// store undock permanently to settings
	db.settings.displayDocked = false;
	db.storeSettings();
	
	// initialise unDocked state using stored settings
	var spyTrackerPad = $('spyTrackerPadDiv');
	spyTrackerPad.style.position = 'absolute';
	spyTrackerPad.style.left = db.settings.displayLeft + 'px';
	spyTrackerPad.style.top = db.settings.displayTop + 'px';
	spyTrackerPad.style.width = db.settings.displayWidth + 'px';
	spyTrackerPad.style.zIndex = 9999;
	spyTrackerPad.style.backgroundImage = 'url()';
	spyTrackerPad.style.backgroundColor = 'rgb(246, 235, 188)';
	spyTrackerPad.style.border = 'double';
	spyTrackerPad.style.borderWidth = '2px';
	spyTrackerPad.style.borderColor = 'rgb(210,179,124)';
	spyTrackerPad.addEventListener('mousemove', onSpyTrackerPadMouseMove, false);
	spyTrackerPad.addEventListener('mousedown', onSpyTrackerPadMouseDown, false);
	
	// update header
	var spyTrackerPadHeader = $('spyTrackerPadHeader');
	spyTrackerPadHeader.style.backgroundImage = 'url()';
	spyTrackerPadHeader.style.backgroundColor = 'rgb(222, 181, 119)';
	spyTrackerPadHeader.style.border = 'solid';
	spyTrackerPadHeader.style.borderColor = 'rgb(198,155,100)';
	spyTrackerPadHeader.style.borderWidth = '1px';
	spyTrackerPadHeader.style.margin = '1px';
	spyTrackerPadHeader.style.height = '24px';
	spyTrackerPadHeader.style.cursor = 'move';
	
	// update tablediv
	var spyTrackerPadTableDiv = $('spyTrackerPadTableDiv');
	spyTrackerPadTableDiv.style.width = parseInt(spyTrackerPad.style.width, 10) - 5 + 'px';
	spyTrackerPadTableDiv.style.height = db.settings.displayHeight + 'px';
	
	// hide undock and make dock button visible
	var dockButton = $('spyTrackerPadDockButton');
	var undockButton = $('spyTrackerPadUndockButton');
	dockButton.style.display = 'block';
	undockButton.style.display = 'none';
	
	// skale pad content
	skaleSpyTrackerPadContent();
}

function spyTrackerPadDock(e) {
	syslog('Spy Tracker - Dock activated', 1);
	
	// store dock permanently to settings
	db.settings.displayDocked = true;
	db.storeSettings();

	// update pad
	var spyTrackerPad = $('spyTrackerPadDiv');
	spyTrackerPad.style.position = '';
	spyTrackerPad.style.left = '0px';
	spyTrackerPad.style.top = '0px';
	spyTrackerPad.style.width = displayDefaultWidth + 5 + 'px';
	spyTrackerPad.removeEventListener('mousemove', onSpyTrackerPadMouseMove, false);
	spyTrackerPad.removeEventListener('mousedown', onSpyTrackerPadMouseDown, false);
	
	// update header
	var spyTrackerPadHeader = $('spyTrackerPadHeader');
	spyTrackerPadHeader.style.cursor = 'auto';
		
	// update table div
	var spyTrackerPadTableDiv = $('spyTrackerPadTableDiv');
	spyTrackerPadTableDiv.style.height = displayDefaultHeight + 'px';
	spyTrackerPadTableDiv.style.width = displayDefaultWidth + 'px';
	
	// hide dock and make undock button visible
	var dockButton = $('spyTrackerPadDockButton');
	var undockButton = $('spyTrackerPadUndockButton');
	dockButton.style.display = 'none';
	undockButton.style.display = 'block';
...
function onSpyTrackerPadMouseMove(e) {
	if(!e) return;
	// evaluate only in undocked state
	if(db.settings.displayDocked) return;
	
	// current absolute mouse cursor position
	var cursorX = e.clientX + window.scrollX;
	var cursorY = e.clientY + window.scrollY;

	// current absolute pad position
	var spyTrackerPadDiv = $('spyTrackerPadDiv');
	var padPosition = new position(spyTrackerPadDiv);

	// check if cursor is on the resize position of the pad
	var borderPosition = padPosition.isCursorOnBorder(cursorX, cursorY);
	if(borderPosition == BORDER_POSITION.bottomRight) 
		spyTrackerPadDiv.style.cursor = 'nw-resize';
	else
		spyTrackerPadDiv.style.cursor = 'auto';
}

function onSpyTrackerPadMouseDown(e) {
	if(!e) return;
	// current absolute mouse cursor position
	var cursorX = e.clientX + window.scrollX;
	var cursorY = e.clientY + window.scrollY;
	// get current absolute pad position
	var padPosition = new position(e.target);

	// check if targets cursor is in resize state
	//if(padPosition.isCursorOnBorder(cursorX, cursorY) == 'bottomRight') {
	if(e.target.style.cursor == 'nw-resize') {
		// store current cursor position as the initial position
		dragPrevX = cursorX;
		dragPrevY = cursorY;
		// Attach mousemove and mouseup event handlers
		document.addEventListener("mousemove", onSpyTrackerPadResize, false);
		document.addEventListener("mouseup", onSpyTrackerPadResizeFinish, false);
		// cancel the default behaviour
		e.preventDefault();
	}
	
	// check if cursor is on the move position of the pad
	if((e.target.id == 'spyTrackerPadHeader') || (e.target.parentNode && (e.target.parentNode.id == 'spyTrackerPadHeader'))) {
		// store current cursor position as the initial position
		dragPrevX = cursorX;
		dragPrevY = cursorY;
		// Attach mousemove and mouseup event handlers
		document.addEventListener("mousemove", onSpyTrackerPadMove, false);
		document.addEventListener("mouseup", onSpyTrackerPadMoveFinished, false);
		// cancel the default behaviour
		e.preventDefault();
	}
}
...
function onSpyTrackerPadMove(e) {
	if(!e) return;
	// get current cursor positions
	var currentX = e.clientX + window.scrollX;
	var currentY = e.clientY + window.scrollY;

	// Move pad with the same amount as the cursor has moved
	var spyTrackerPadDiv = $('spyTrackerPadDiv');
	var currentLeft = parseInt(spyTrackerPadDiv.style.left, 10);
	if (isNaN(currentLeft)) currentLeft = displayDefaultLeft;
	var currentTop = parseInt(spyTrackerPadDiv.style.top, 10);
	if (isNaN(currentTop)) currentTop = displayDefaultTop;
	spyTrackerPadDiv.style.left = currentLeft + currentX - dragPrevX + 'px';
	spyTrackerPadDiv.style.top = currentTop + currentY - dragPrevY + 'px';
	
	
	// Get current absolute pad position
	var padPosition = new position(spyTrackerPadDiv);
	// don't allow the pad to be positioned at the top of the screen since it cannot
	// be moved from there (due to z-index). 
	if (padPosition.top < displayDefaultTop)
		spyTrackerPadDiv.style.top = displayDefaultTop + 'px';
...
function onSpyTrackerPadMoveFinished(e) {
	if(!e) return;
	// unregister mousemove and mouseup event handlers
    document.removeEventListener("mousemove", onSpyTrackerPadMove, false);
    document.removeEventListener("mouseup", onSpyTrackerPadMoveFinished, false);
	// store current location permanently to settings
	var spyTrackerPadDiv = $('spyTrackerPadDiv');
	db.settings.displayTop = parseInt(spyTrackerPadDiv.style.top, 10);
	db.settings.displayLeft = parseInt(spyTrackerPadDiv.style.left, 10);
	db.storeSettings();
}

function onSpyTrackerPadResize(e) {
	if(!e) return;
	syslog(e.target.id, 1);
	...
	// cursor movement
	var diffX = currentX - dragPrevX;
	var diffY = currentY - dragPrevY;
	
	// Resize the pad (only width) and the table (width and height) within pad with the same amount as the cursor has moved
	var spyTrackerPadDiv = $('spyTrackerPadDiv');
	var currentWidth = parseInt(spyTrackerPadDiv.style.width, 10);
	//if(isNaN(currentWidth)) currentWidth = parseInt(spyTrackerPadDiv.offsetWidth, 10);
	spyTrackerPadDiv.style.width = currentWidth + diffX + 'px';
	var spyTrackerPadTableDiv = $('spyTrackerPadTableDiv');
	spyTrackerPadTableDiv.style.width = parseInt(spyTrackerPadTableDiv.style.width, 10) + diffX + 'px';
	spyTrackerPadTableDiv.style.height = parseInt(spyTrackerPadTableDiv.style.height, 10) + diffY + 'px';
...
function onSpyTrackerPadResizeFinish(e) {
	if(!e) return;
	syslog(e.target.id, 1);

	// unregister mousemove and mouseup event handlers
    document.removeEventListener("mousemove", onSpyTrackerPadResize, false);
    document.removeEventListener("mouseup", onSpyTrackerPadResizeFinish, false);

	// store size permanently to settings
	var spyTrackerPadDiv = $('spyTrackerPadDiv');
	var spyTrackerPadTableDiv = $('spyTrackerPadTableDiv');
	db.settings.displayWidth = parseInt(spyTrackerPadDiv.style.width, 10);
	db.settings.displayHeight = parseInt(spyTrackerPadTableDiv.style.height, 10);
	db.storeSettings();

	return false;
}

function skaleSpyTrackerPadContent() {
	syslog('init', 1);
	// evaluate only in undocked state
	if(db.settings.displayDocked) {
		syslog('docked', 1);
		// docked style
		GM_addStyle('#SpyTrackerPad tr td { font-size:11px }');
		GM_addStyle('.spyTrackerActionImage { width: 14px; height: 12px }');
		GM_addStyle('.spyTrackerActionTD { width: 100%; height: 100% }');
		
	}	
	else {
		syslog('undocked', 1);
		// undocked style
		GM_addStyle('#SpyTrackerPad tr td { font-size:12px }');
		GM_addStyle('.spyTrackerActionImage { width: 100%; height: 100% }');
		GM_addStyle('.spyTrackerActionTD { width: 20px; height: 15px }');
	}
}
...
/*  -----------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------*/
function spyTrackerInit() {
	  
	syslog('SpyTracker - Starting Spy Tracker',1);
	...
	if(initSpyTrackerPad()) {
		syslog('Spy Tracker initialisation successfull', 1);
...
// initialise spy tracker without delay
spyTrackerInit();
