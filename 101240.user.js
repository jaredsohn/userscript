// ==UserScript==
// @name           PowerTools - Unanimi- ITALIANO
// @namespace      Kok
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @description    traduzione italiana power tools
...
var Version = '1.5';20110415
var Title = 'Unanimi by Kok';
...
{ document.getElementById("mod_comm_list2").style.backgroundColor = '#FFFFE6';}
if(document.getElementById("mod_comm_list1"))
{ document.getElementById("mod_comm_list1").style.backgroundColor = '#FFF28E';}
...
var Options = {
  ...
  alertConfig  : {aChat:true, aPrefix:'** Mi stanno attaccando **', scouting:true, wilds:true, minTroops:1, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    .pthostile td { background:#ffa8a8; }.ptfriendly td{background:lightblue; }.ptally td{background:royalblue; }\
    ...
    .ptChatWhisper {}\
    .ptChatAlliance {background-color: #FFFFE6}\
    .ptChatGlobal {background-color: #FFF28E}\
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('Unanimi by Kok', eventHideShow, mouseMainTab);
}
...
Tabs.Forum = {
  ...
  myDiv : null,
  timer : null,
  forumlink :  'http://www.prosoftservizi.com',  
  
  init : function (div){    // called once, upon script startup
    var t = Tabs.Forum;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="750px" name="board_in_a_box" <p>Ihr Browser kann leider keine eingebetteten Frames anzeigen: Sie k&ouml;nnen die eingebettete Seite &uuml;ber den folgenden Verweis aufrufen: <a href="'+ t.forumlink +'">Web agency Prosoft Servizi </a></p> </iframe> <BR></center>';
  },
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Terre Selvagge: '+ totw +' di '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];

      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Tipo Terra</td><TD></td><TD align=left>Coords</td><TD>Trappole</td><TD align=left>Mercenari</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' CAMBIA DIFESE ') +'</td></tr>';
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Costruisci Trappole:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
          ...
          m += '<TD class=ptentry> &nbsp; &nbsp; <B>Mercenari:</b> ' + htmlSelector(mercNames, wildDef.mercLevel, 'id=ptwm_'+ c +'_'+ i) +' &nbsp; &nbsp; </td></tr>';
        }
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Costo: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("SETTA DIFESE", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Oro: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
if (wildDef==undefined || !wildDef)
  wildDef = {fort60Count:0, mercLevel:0};
    ...
    popDiv.innerHTML = '<TABLE class=ptTab width=100% height=100%><TR><TD>\
          <DIV class=ptstat>Configura Difese delle Terre</div>\
          <DIV id=ptWildBuildDiv class=ptDiv style="padding:10px; height:225px; max-height:225px; overflow-y:auto"></div>\
          </td></tr><TR><TD align=center>'+ strButton20('CANCELLA', 'id=ptWildCancel') +'</td></tr></table>';
  ...
  processQue : function (errMsg){
    ...
    if (what==null || errMsg){
      if (errMsg)
        div.innerHTML += '<BR><SPAN style="white-space:normal;" class=boldRed>ERRORE: '+ errMsg +'</span>';
      else
        div.innerHTML += 'Finito.<BR>';
    ...
    if (what[0] == 'T'){
      div.innerHTML += 'Sto costruendo '+ what[2] +' trappole per '+ Cities.byID[t.buildList.cityId].name +' sulla terra alle coordinate '+ wild.xCoord +','+ wild.yCoord +' ... ';
      t.postBuyTraps (t.buildList.cityId, what[1], what[2], t.processQue);
    } else {
      div.innerHTML += 'Sto settando i Mercenari '+ mercNames[what[2]] +' per '+ Cities.byID[t.buildList.cityId].name +' sulla terra alle coordinate '+ wild.xCoord +','+ wild.yCoord +' ... ';
...
Tabs.Cavalieri = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        ...
        if (knight.knightStatus == 10){
          ass = '<TD class=ptentry align=left colspan=4>In Marcia</td>';
        } else {  
          if (unpoints > 0){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          <TR class=ptwpad style="font-weight:bold" align=right><TD width=70>Ruolo</td><TD width=160 align=center>Nome</td><TD width=26>Pol</td><TD width=26>Com</td>\
          <TD width=26>Int</td><TD width=26>Res</td><TD width=90 align=center colspan=5>--- Non Assegnato ---</td><TD width=40 align=right> Salario </td><TD width=35>Livello</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
  ...
  clickedAssignPoints : function (e, cid, kid, rid){
    ...
    div.innerHTML = 'Assegnati '+ unassigned +' punti a '+ knightRoles[rid][1] +' ... ';
  ...
  postDone : function (rslt, div){
    ...
    clearTimeout (t.displayTimer);
    if (rslt.ok){
      div.innerHTML += '<B>Finito.</b>';
      t.displayTimer = setTimeout (t.show, 5000);
    } else {
      div.innerHTML += '<BR><SPAN class=boldRed>ERRORE: '+ rslt.errorMsg +'</span>';
      t.displayTimer = setTimeout (t.show, 10000);
    }
  },
  ...
  e_eachMinute : function (){   
    ...
    for(i=0; i < Cities.numCities; i++) {
      ...
      if (timeLeft<0){
        ...
        var msg = '';
        msg += 'Nella mia città ' + Cities.cities[i].name.substring(0,10);
        msg += ' (' + Cities.cities[i].x +','+ Cities.cities[i].y + ')';
        msg += ' è quasi finito il cibo. Ne resta : '+addCommasWhole(foodleft);
        msg += ' (' + timestrShort(timeLeft) + ') Consumo : ' + addCommas(usage);
        sendChat ("/a " + msg);
      }
    }  
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    if (m.fromXCoord)
      who += ' alle coordinate '+ m.fromXCoord +','+ m.fromYCoord ;
    ...
    msg += 'La mia '+ target +' sta per essere '+ atkType  +' da '+ who +' ' +alliance+'. Truppe in arrivo (in '+
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' La mia ambasciata ha '+ availSlots +' di '+ emb.maxLevel +' slots disponibili.';
      }
    }
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    if (getMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER>Per usare questo strumento hai bisogno di entrare in una alleanza.</center>';
      t.state = 1;
      return;
    }
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Inserisci il nome o una parte del nome del giocatore: &nbsp;</td>\
        ...
        <TR><TD class=xtab>OR: </td><TD class=xtab> Inserisci il nome o una parte del nome dell\'alleanza: &nbsp;</td>\
		...
		<TR><TD class=xtab>Oppure: </td><TD class=xtab align=right> Lista delle alleanza: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="La mia alleanza"/>\
            <INPUT align=left id=allListSubmit type=submit value="Lista di tutte" /></td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'Inserisci almeno 3 caratteri';
      return;
    }
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Sto cercando ...</center>';
...
Tabs.Test = {
  ...
  init : function (div){
	...
	var citySelect = '   <SELECT id=fakeCity>';
	    ...
	    citySelect += '</select>';
    var m = '<TABLE><TR><TD align=right>Eaplorazione: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        <TR><TD align=right>Terre: </td><TD><INPUT type=checkbox id=fakeIsWild></td></tr>\
        <TR><TD align=right>Report Falso: </td><TD><INPUT type=checkbox disabled id=fakeFalse></td></tr>\
        <TR><TD align=right>Secondi: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
        <TR><TD align=right># di Trasportatori: </td><TD><INPUT type=text size=6 value=0 id=faketroop0></td></tr>\
		<TR><TD align=right># di S. Semplici: </td><TD><INPUT type=text size=6 value=0 id=faketroop1></td></tr>\
		<TR><TD align=right># di Esploratori: </td><TD><INPUT type=text size=6 value=0 id=faketroop2></td></tr>\
		  <TR><TD align=right># di Lancieri: </td><TD><INPUT type=text size=6 value=0 id=faketroop3></td></tr>\
		  <TR><TD align=right># di Spadaccini: </td><TD><INPUT type=text size=6 value=0 id=faketroop4></td></tr>\
		  <TR><TD align=right># di Arcieri: </td><TD><INPUT type=text size=6 value=0 id=faketroop5></td></tr>\
		  <TR><TD align=right># di Cavallini: </td><TD><INPUT type=text size=6 value=0 id=faketroop6></td></tr>\
		  <TR><TD align=right># di Cavalloni: </td><TD><INPUT type=text size=6 value=0 id=faketroop7></td></tr>\
		  <TR><TD align=right># di Carretti: </td><TD><INPUT type=text size=6 value=0 id=faketroop8></td></tr>\
		  <TR><TD align=right># di Baliste: </td><TD><INPUT type=text size=6 value=0 id=faketroop9></td></tr>\
		  <TR><TD align=right># di Arieti: </td><TD><INPUT type=text size=6 value=0 id=faketroop10></td></tr>\
		  <TR><TD align=right># di Catapulte: </td><TD><INPUT type=text size=6 value=0 id=faketroop11></td></tr>\
		  <TR><TD align=right>Nome di chi ti attacca: </td><TD><INPUT type=text size=13 value=UtenteTEST id=fakeName></td></tr>\
		  <TR><TD align=right>Coordinate Città: </td><TD>'+citySelect+'</td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=testSendMarch type=submit value="Invia Attacco Falso" \></td></tr></table>\
        <INPUT id=ptReloadKOC type=submit value="Riavvia KOC" \>\
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>INFORMAZIONI SULLE UNITA\'</div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>COSTO COSTRUZIONE</b></td><TD class=xtabHL colspan=7><B>STATISTICHE</b></td><TD class=xtabHL><B>Consumo</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Cibo</td><TD class=xtabH>Legna</td><TD class=xtabH>Pietre</td>\
        <TD class=xtabH>Miner.</td><TD class=xtabH>Popol.</td><TD class=xtabHL>Pot.</td><TD class=xtabH>Vita</td><TD class=xtabH>Att.o</td><TD class=xtabH>Dif.</td><TD class=xtabH>Vel.</td><TD class=xtabH>Range</td><TD class=xtabH>Carico</td>\
        <TD class=xtabHL>Cibo</td></tr>\
...
var crestreq = { 3:{1101:4, 1102:2, 1103:1},
 ...
 m += '<DIV class=ptstat>RICERCA ELMI</div><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR CLASS="xtabH">\
        <TD class=xtab></TH>\
        <TD class=xtabHL><b>Elmo (trovati)</b></TD>\
        <TD class=xtabHL><b>Elmo (trovati)</b></TD>\
        <TD class=xtabHL><b>Elmo (trovati)</b></TD>\
      </TR>\
	  <TR style="height:1px;"><TD style="padding:0px; spacing:0px; height:1px; border-color:black; border-width: 1px; border-style: none none solid none" colspan=14></td></tr>';
 ...
 m += '</TABLE>';
	//End crest info
    m += '<BR><DIV class=ptstat>CALCOLA DISTANZA</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Coord. Partenza: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> oppure seleziona la città: <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>Coord. Arrivo: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> oppure seleziona la città: <SPAN id=ptloc2></span></td></tr></table>\
      ...
      <BR><BR><DIV class=ptstat>MAPPA PROVINCE</div><DIV id=ptProvMap><IMG width=680 height=654 SRC="'+ URL_PROVINCE_MAP +'"></div></center>';
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = 'La distanza da '+ x1 +','+ y1 +' a '+ x2 +','+ y2 +' è: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Attiva il movimento della finestra (ti permette di muovere la finestra de \'Unanimikok\' col mouse)</td></tr>\
        <TR><TD><INPUT id=togWhisperOn type=checkbox /></td><TD>Attiva il Sussuro Audio <SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=ptHideOnGoto type=checkbox /></td><TD>Fai sparire questa finestra quando si clicca sulle coordinate sulla mappa. <SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Visualizza \"l\'autonomia del cibo\" in ROSSO se il cibo è inferiore a \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> ore. QUESTA OPZIONE POSTA IN CHAT L\'AVVISO !</td></tr>\
        <TR><TD colspan=2><P><B>Caratterizza KofC:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Visualizza il numero di messaggi separatamente dal numero di reports sull\'icona messaggi.</td></tr>';
		if (ENABLE_ALERT_TO_CHAT){
	 m += '<TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Posta automaticamente l\'attacco in chat alleanza.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
          <TR><TD align=right>Prefisso messaggio: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
          <TR><TD align=right>Allerta se mi esplorano: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Alleerta se attaccano le miet terre: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Posta la diplomazia tra le nostre alleanze: &nbsp; </td><TD><INPUT id=alertAlliance type=checkbox '+ (Options.alertConfig.includealliance?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Posta il potere di chi attacca: &nbsp; </td><TD><INPUT id=alertMight type=checkbox '+ (Options.alertConfig.includemight?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Avvisami con un suono: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.sound?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Minimo numero di truppe: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
          </table></td></tr>';
		  }
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Abilita il report dei rinforzi in chat alleanza.</td></tr>\
        <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Consenti agli altri scripts di cambiare il formato dei reports in chat alleanza.</td></tr>\
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Abilita altri pulsanti nei messaggi (pulsante "forward" e "all officers").</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Rafforza la navigazione dei messaggi e dei reports.</td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Avvisa se stai mandando un attacco alle coordinate 0,0.</td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Abilita l\'orologio GMT dopo il "Camelot Time" <SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Abilita opzioni supplementari (coordinate cliccabili, messaggi globali in viola, click sull\'icona per bisbigliare) <SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=togAttackPicker type=checkbox /></td><TD>Abilita le coordinate della città dell\'attacco (rinfozi, riassigna e transporto) <SPAN class=boldRed>&nbsp;(NUOVA opzione)</span></td></tr>\
        <TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Visualizza il numero di round nei repots delle battaglie <SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>Abilita il pulsante DELETE nei repots di battaglia<SPAN class=boldRed>&nbsp;(NUOVO)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>KofC Bug Fixes:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Correggi i repots della torre per visualizzare esattamente gli obbiettivi (città, terre o non valide)</td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Correggi la mappa per visualizzare correttamente le distanze dalla città selezionata, invece che sempre dalla prima città.</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Correggi i falsi attacchi creati da semplici esplorazioni.</td></tr>\
        <TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD>Non selezionare automaticamente un cavaliere quando si cambia da attacco a esplorazione, transporto o riassegna <SPAN class=boldRed>(NUOVO)</span></td></tr>\
        <TR><TD><INPUT id=togCoordBox type=checkbox /></td><TD>Mantieni il riquadro delle coordinate della mappa sopra le attività dela truppa <SPAN class=boldRed>(NUOVO)</span></td></tr>\
        </table><BR><BR><HR>Nota : se l\'opzione è in grigio probabilmente hanno modificato il codice di KofC, questo rende l\'opzione non più disponibile.';
      t.cont.innerHTML = m;
...
Tabs.Cerca = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Ricerca per: </td><TD>\
      <SELECT id="srcType">\
        <OPTION value=0>Campi Barbari</option>\
        <OPTION value=1>Terre Selvagge</option>\
    ...
    m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Centra in &nbsp; X: </td><TD class=xtab>\
	...
	m += '</td></tr><TR><TD class=xtab align=right>Distanza Max: </td><TD class=xtab><INPUT id=srcDist size=4 value=10 /> &nbsp; <SPAN id=spInXY></span></td></tr>';
    m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Avvia Ricerca"/></td></tr>';
    m += '</table>';
...
clickedPlayerDetail : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "ricerca dettagli ...";
    t.fetchPlayerInfo (uid, function (r) {t.gotPlayerDetail(r, span)});
  },
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "ricerca informazioni ...";
    t.fetchLeaderboard (uid, function (r) {t.gotPlayerLeaderboard(r, span)});
  },
  ...
  clickedPlayerCheckOnline : function (span, uid){
	...
	var s = Tabs.Cerca;
    span.onclick = '';
    span.innerHTML = "ricerca stato online ...";
  ...
  clickedPlayerGetLastLogin : function (span, uid){
	...
	var s = Tabs.Cerca;
    span.onclick = '';
    span.innerHTML = "ricerca ultimo accesso ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Informazioni:</b> Non trovato! (sarà nascosto sotto nebbia ?)';
      return;
    }
    var p = rslt.results[0];
    var an = p.allianceName;
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ officerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Informazioni: </b></td><TD colspan=2> Potere: '+ p.might  +' &nbsp; Alleanza: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      m += '<TR><TD align=right><B>Città #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName +' '+coordLink (c.xCoord, c.yCoord)+'</td><TD width=75%> &nbsp; livello: '
        + c.tileLevel +' &nbsp; &nbsp; stato: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; creata: ' + c.dateCreated.substr(0,10) +'</td></tr>';
    }  
    span.innerHTML = m + '</table>';
  },
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Dettagli:</b> &nbsp; </td><TD>Alleanza: '+ a +' &nbsp; Città: '
          + u.cities +' &nbsp; Popolazione: '+ u.population +'</td></tr><TR><TD></td><TD>Province: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
      p.push(unsafeWindow.provincenames['p'+pids[i]]);
  ...
  gotPlayerStatus : function (rslt, span,uid){
    ...
    var p = rslt.data;
    if (p[uid] == true) {
      m = '<span style="color:green"><b>in linea!</b></span>';
    } else {
       m = '<span style="color:red"><b>potrebbe non essere in linea</b></span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:black">Ultimo Accesso: '+lastLogin+'</span>';
    } else {
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    if (t.searchRunning){
      t.stopSearch ('Ricerca Interrotta!');
      return;
    }
    ...
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y deve essere tradeve essere tra 0 e 749<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>750)
      errMsg += "La Distanza Max deve essere tra 1 e 750<BR>";
    if (errMsg != ''){
      document.getElementById('srcResults').innerHTML = '<FONT COLOR=#660000>ERRORE:</font><BR><BR>'+ errMsg;
      return;
    }

    t.searchRunning = true;
    document.getElementById ('srcStart').value = 'Ferma Ricerca';
    ...
    if (t.opt.searchType == 0)
      typeName = 'Campi Barbari';
    else if (t.opt.searchType == 1)
      typeName = 'Terre Selvagge';
	else 
	  typeName = 'Città';

    m = '<CENTER><B>Ricerca per '+ typeName +'<BR>\
        Centra su: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distanza: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>LISTA OPZIONI:</b><BR></td></tr>';
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Livello Min. da cercare:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Livello Max. da cercare:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Tipo di Terre:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Radure/Laghi', 3:'Foreste', 4:'Colline', 5:'Montagne', 6:'Pianure', 0:'TUTTE'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Solo Non Conquistate:</td>\
			<TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD class=xtab align=right>Ordina per:</td><TD class=xtab><SELECT id=filSortBy>\
				<OPTION value="level" '+ (Options.srcSortBy=='level'?'SELECTED':'')  +'>Livello</option>\
				<OPTION value="level" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distanza</option>\
            </select></td></tr>\
            <TR><TD class=xtab align=right>Visualizza solo Coordinate:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
	} else {
	 m += '<TR><TD class=xtab align=right>Tipo Città:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'Tutte', 2:'Alleate', 3:'Amichevoli', 4:'Ostili', 5:'Neutrali', 6:'Senza Alleanza', 7:'Sotto Nebbie'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Ordina per:</td><TD class=xtab><SELECT id=filSortBy>\
		   ...
		   <TR><TD class=xtab align=right>Potere Min da visualizzare:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>Visualizza solo Coordinate:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
    ...
    document.getElementById ('statStatus').innerHTML = 'Sto Cercando in '+ xxx +','+ yyy;
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Campo Barbari', 'Radura', 'Lago', 'Foresta', 'Collina', 'Montagna', 'Pianura', 'Città' ];
    var t = Tabs.Cerca;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    document.getElementById('statFound').innerHTML = 'Trovati: '+ dat.length;
    if (dat.length == 0){
      m = '<BR><CENTER>Non ho trovato niente</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Coordinate</td></tr>';
      else {
        if (t.opt.searchType == 2) {
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Coordinate</td><TD >Distanza</td><TD>Città</td><TD>Utente</td><TD>Potere</td><td>Alleanza               </td><TD width=80% style="font-size:10px;">Altre Info</td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Coordinate</td><TD style="padding-left: 10px">Distanza</td><TD style="padding-left: 10px;">Livello</td><TD width=80%> &nbsp; Tipo</td><TD style=""></td></tr>';
		}
	  }
      var numRows = dat.length;
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTA: La Tabella visualizza solo 500 risultati di '+ dat.length +'.</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
        ...
        if (coordsOnly)
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					m += '<DIV onclick="PTscout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A style="font-size:9px;" >Esploralo</a></div>';
				} else 
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A style="font-size:9px;" >Dettagli</a></div>\
					<DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Informazioni</a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Stato</a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Ultimo Accesso</a></div>';
					m+= '</td><TD  valign="top">'+ (dat[i][5]?' Sotto nebbie':'') +'</td></tr>';
			} else { 
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'Avvia Ricerca';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Cercati: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('statStatus').innerHTML = 'Sto Cercando in '+ x +','+ y;
...
Tabs.Truppa = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Truppare e Costruire le Difese delle Mura</div><DIV style='height:10px'></div><DIV class=ptentry>\
      <DIV style='text-align:center; margin-bottom:10px;'>Seleziona la Città: &nbsp; <span id=ptspeedcity></span></div>\
      <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
      <TABLE align=center><TR><TD align=right>Tipo Truppe: </td><TD colspan=2>\
      <SELECT id=ptttType>\
        <option value='1'>Truppe Trasporto</option>\
        <option value='2'>Soldati Semplici</option>\
        <option value='3'>Esploratori</option>\
        <option value='4'>Lanceri</option>\
        <option value='5'>Spadaccini</option>\
        <option value='6'>Arcieri</option>\
        <option value='7'>Cavallini</option>\
        <option value='8'>Cavalloni</option>\
        <option value='9'>Carretti</option>\
      ...
      <TR><TD align=right>Truppe per slot: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right>Numero di slots: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
        <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
      <TR><TD align=right valign=top>Seleziona come truppare: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Ti permette di truppare di più, ma porta la popolazione in negativo.</span></td></tr>\
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='Inizia a Truppare'\></td></tr>\
      </table></td><TD width=20></td><TD style='border-left:solid 2px;' width=50% align=center>\
      <TABLE align=center><TR><TD align=right>Tipo di Difesa: </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Balestre</option>\
        <option value='55'>Trabucchi</option>\
        <option value='60'>Trappole</option>\
        <option value='61'>Scarabocchi</option>\
        <option value='62'>Barriere</option>\
      </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
      <TR><TD align=right>Difese per slot: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right>Numero di slots: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
      ...
      <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Coda Truppe &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
        <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Coda Difese &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
      </div>";
    t.cont.innerHTML = s;
...
if (t.limitingFactor){
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (t.running){
      t.stopTraining('<SPAN class=boldRed>Accodamento cancellato dall\'utente</span>');
      return; 
    }    
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Il numero di truppe per slot deve essere maggiore di 0.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Non posso accodare queste truppe (il max è '+ t.stats.MaxTrain +')</font>';
      return;
    }
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero di slots non valido.</font>';
      return;
    }
    
    t.TDbutDo.disabled = true;
    t.TTbutDo.className = 'ptButCancel';
    t.TTbutDo.value = 'CANCELLA';
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    t.TDspSpace.innerHTML = 'Livello Mura: <B>'+ t.stats.wallLevel +'</b><BR>Spazio sulle Mura: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        Spazio avanti Mura: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      t.displayCityStats();
      if (errMsg){
        t.stopTraining ('<SPAN class=boldRed>ERRORE: '+ errMsg +'</span>');
        return;
      }
      var cmd = que.shift();
      if (!cmd){
        t.stopTraining ('<B>Ho finito di accodare le difese.</b>');
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Accodamento '+ cmd[2] +' '+  fortNamesShort[cmd[1]] +' in '+ Cities.byID[cityId].name +' (restano '+ que.length +' slots)<BR>');
        doDefTrain ( cityId, cmd[1], cmd[2], 
          function(errMsg){
            t.trainTimer = setTimeout(function (){Tabs.Truppa.doDefQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500);
          } );
      }
    } catch (err) {
      logit (inspect (err, 8, 1));
      t.stopTraining ('<SPAN class=boldRed>ERRORE DI PROGRAMMA: '+ err.message +'</span>');
    }
  },
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B>Trasp.:</b></td><TD width=8%><B>Semplici:</b></td><TD width=8%><B>Esploratori:</b></td>\
        <TD width=8%><B>Lance:</b></td><TD width=8%><B>Spade:</b></td><TD width=8%><B>Archi:</b></td>\
        <TD width=8%><B>Cavallini:</b></td><TD width=8%><B>Cavalloni:</b></td><TD width=8%><B>Carretti:</b></td>\
        <TD width=8%><B>Baliste:</b></td><TD width=8%><B>Arieti:</b></td><TD width=8%><B>Catapulte:</b></td><tr>';
...
//end of added21mar
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><SPAN id=ptttr_food><B>Cibo:</b><BR>'+ addCommasInt(t.stats.food) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_wood><B>Legna:</b><BR>'+ addCommasInt(t.stats.wood) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_ore><B>Minerali:</b><BR>'+ addCommasInt(t.stats.ore) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_gold><B>Oro:</b><BR>'+ addCommasInt(t.stats.gold) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_pop><B>Popolazione :</b><BR>'+ addCommasInt(t.stats.idlePop) +'</span></td></tr></table><BR>';
...
// defense queue ....
  ...
  stopTraining : function (msg){
    ...
    t.TTbutDo.value = 'Inizia a Truppare';
    t.TDbutDo.value = 'Costruisci Difese';
    t.TTbutDo.className = '';
    t.TDbutDo.className = '';
    t.running = false;
  },
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.stopTraining ('<B>Ho finito di truppare.</b>');
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Accodamento '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +' in '+ Cities.byID[cityId].name +' (restano '+ que.length +' slots)<BR>');
        doTrain (cityId, cmd[1], cmd[2], 
          function(errMsg){
            if (t.running)
              t.trainTimer = setTimeout(function (){Tabs.Truppa.doQueue(cityId, que, errMsg);}, (Math.random()*2500)+1000 );
          }
        );
      }
    } catch (err) {
...
Tabs.Generale = {
  ...
  hide : function (){
    clearTimeout (Tabs.Generale.displayTimer);
  },
  ...
  show : function (){
    ...
    function _row (name, row, noTotal){
      if (rownum++ % 2)
        ...
        style = ' style = "background: #FFFFE6 "';
      var tot = 0;
      var m = [];
      m.push ('<TR style="background: #FFFFE6 " align=right');
      m.push (style);
      m.push ('><TD');
      m.push (style);
      m.push ('><B>');
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Ultimo Accesso:</span> '+ dt.toLocaleDateString() +'</td>\
        <TD><SPAN class=ptStatLight>Potere:</span> ' + addCommas(Seed.player.might) +'</td>\
        <TD><SPAN class=ptStatLight>Alleanza:</span> ' + getMyAlliance()[1] +'</td>\
        <TD align=right><SPAN class=ptStatLight>Dominio:</span> ' + unsafeWindow.domainName +'</td></tr></table></div>';

      tabClass = 'ptTabOverview';  
      if (Cities.numCities > 6)
        tabClass = 'ptTabOverview7';  
              
      str += "<TABLE class="+ tabClass +" cellpadding=0 cellspacing=0><TR valign=top align=right><TD width=65></td><TD width=88 style='background: #ffc'><B>TOTALE</b></td>";
      for(i=0; i<Cities.numCities; i++) {
      ...
      if (Options.includeMarching)
        str += '<TD width=81><B>In Marcia</b></td>';
      str += "</tr>";
	  ...
	  for(i=0; i<Cities.numCities; i++){
		...
		if(Gate == 0)
			str += '<TD style=\'background: #CCFFCC\'>Non Difesa</td>';
		else
			str += '<TD style=\'background: #CCFFCC\' ><SPAN class=boldRed><blink>In Difesa</blink></span></td>';
	  }
      ...
      str += _row ('Oro', rows[0]);
      str += _row ('Cibo', rows[1]);
      str += _row ('Legna', rows[2]);
      str += _row ('Pietra', rows[3]);
      str += _row ('Minerali', rows[4]);
      str += '<TR><TD><BR></td></tr>';
      for (r=1; r<13; r++){
        rows[r] = [];
        for(i=0; i<Cities.numCities; i++) {
...
//**********************************
      ...
      str += _row ('Truppe Trasporto', rows[1]);      
      str += _row ('Soldati Semplici', rows[2]);
      str += _row ('Esploratori', rows[3]);
      str += _row ('Lancieri', rows[4]);
      str += _row ('Spadaccini', rows[5]);
      str += _row ('Arcieri', rows[6]);
      str += _row ('Cavallini', rows[7]);
      str += _row ('Cavalloni', rows[8]);
      str += _row ('Carretti', rows[9]);
      str += _row ('Baliste', rows[10]);
      str += _row ('Arieti', rows[11]);
      str += _row ('Catapulte', rows[12]);
      str += _row ('<i><span style="color:green">Coda Truppe Tr.</span></i>', t_rows[2]);
      str += _row ('<i><span style="color:green">Coda S. Semplici</span></i>', t_rows[4]);
      str += _row ('<i><span style="color:green">Coda Esploratori</span></i>', t_rows[6]);
      str += _row ('<i><span style="color:green">Coda Lancieri</span></i>', t_rows[8]);
      str += _row ('<i><span style="color:green">Coda Spadaccini</span></i>', t_rows[10]);
      str += _row ('<i><span style="color:green">Coda Arcieri</span></i>', t_rows[12]);
      str += _row ('<i><span style="color:green">Coda Cavallini</span></i>', t_rows[14]);
      str += _row ('<i><span style="color:green">Coda Cavalloni</span></i>', t_rows[16]);
      str += _row ('<i><span style="color:green">Coda Carretti</span></i>', t_rows[18]);
      str += _row ('<i><span style="color:green">Coda Baliste</span></i>', t_rows[20]);
      str += _row ('<i><span style="color:green">Coda Arieti</span></i>', t_rows[22]);
      str += _row ('<i><span style="color:green">Coda Catapulte</span></i>', t_rows[24]);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Autonomia Cibo', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Coda Mura', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Includi Truppe e Risorse in marcia</td></tr>';
      str += '<TR><TD></td><TD colspan=4>PowerTools - Unanimi by Kok</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
        str += '<BR><INPUT id=subSeed type=submit name="SEED" value="DEBUG">';
      Tabs.Generale.cont.innerHTML = str;
...
//DebugTimer.display ('Draw Overview');    
      ...
      Tabs.Generale.cont.innerHTML = '<PRE>'+ e.name +' : '+ e.message +'</pre>';
    }   
    t.displayTimer = setTimeout (t.show, 5000);
  },
};
...
Tabs.Marce = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.cont.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubA type=submit value=Attacchi></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubM type=submit value=Marce></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubR type=submit value=Rinforzi></td></tr></table><HR class=ptThin>\
  ...
  showAttacks : function (){
    var t = Tabs.Marce;
    t.marchDiv.innerHTML = 'Qui puoi visuallizzare i piallaggi imminenti !';;
  },
  ...
  showMarches : function (){
    var t = Tabs.Marce;
    t.marchDiv.innerHTML = 'Qui puoi visuallizzare i rinforzi in arrivo !';
  },
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['Truppe Trasporto', 'Soldati Semplici', 'Esploratori', 'Lancieri', 'Spadaccini', 'Arcieri', 'Cavallini', 'Cavalloni', 'Carretti', 'Baliste', 'Arieti', 'Catapulte'];
    var t = Tabs.Marce;
    clearTimeout (t.displayTimer);
...
//logit ("enc: "+ inspect (enc, 6, 1));    
    ...
    s = '<div class=ptstat>Visualizza le truppe accampate ed in arrivo nella tua ambasciata</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Nessuna truppa accampata.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Giocatore (cavaliere)</b></td>';
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
      ...
      s += '<TR><TD colspan=13><BR><BR></td></tr><TR align=right><TD class="tot" align=left><B>TOTALI:</b></td>';
      for (i=1; i<13; i++)
        s+= '<TD class="tot">'+ tot[i] +'</td>';
      s += '</tr></table>';
    }

    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Visualizza le truppe';
    s += '<BR><BR><DIV style="font-size: 10px">NOTA: Hai bisogno di refreshare  il KofC per visualizzare le nuove truppe accampate o quelle che restano dopo una battaglia.</div>';
...
var WarnZeroAttack = {
  ...
  hook : function (){
    ...
    && parseIntZero(document.getElementById('modal_attack_target_coords_y').value) == 0){
      new CdialogCancelContinue ('<SPAN class=boldRed>Stai mandando un\'attacco alle coordinate 0,0!</span>', null, unsafeWindow.modal_attack_check, document.getElementById('modalInner1'));      
    } else {
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>KOC Power Tools - Unanimi</center>';
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>KOC Power Tools - Unanimi</center>';
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Si è verificato un errore:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Ci riprovo fra <SPAN id=paretrySeconds></b></span> secondi ...<BR><BR><INPUT id=paretryCancel type=submit value="CANCEL Retry" \>';
