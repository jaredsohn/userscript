// ==UserScript==
// @name           TLTTF KOC Power Bot
// @version        1.1.3
// @namespace      mat
// @homepage       http://userscripts.org/scripts/show/103838
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @description    Automated features for Kingdoms of Camelot
// @require        http://tomchapin.me/auto-updater.php?id=103838
// ==/UserScript==
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){  
    ...
    if (Options.pbFoodAlert && timeLeft<(6*3600)) {
                ...
                msg += ' is low on weed. Remaining: '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+') Upkeep: '+addCommas(usage);
                sendChat ("/a " + msg);
          }
    }
      } 
  ...
  postToCell : function (m){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        {
            data.stat = 'are to stoned to fight and are HIDING';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            data.stat = 'dont want to lose there weed so are DEFENDING';
        }
      }
    }
  ...
  postToChat : function (m){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' My growroom has '+ availSlots +' of '+ emb.maxLevel +' plants left to smoke, send some troops NOW!!.';
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
            msg+= ' My troops to stoned and are HIDING!';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            msg+= ' My troops dont want to lose there weed and are DEFENDING!';
        }
      }
    }
...
// TODO: save memory, remove city name ?   			
  ...
  gotPlayerInfo : function (rslt, span){
    ...
    m += '<TR><TD class=pbDetLeft>Player Name:</td><TD>'+ p.genderAndName +'</td></tr>\
      ...
      <TR><TD class=pbDetLeft>Last Login:</td><TD>not working yet</td></tr>\
...
Tabs.Test = {
  ...
  createFakeAttack : function (cityNum, isScout, isWild, isFalse, secs, numMilitia){
    ...
    march.players.u1234567.n = 'A PINK ELEPHANT';
