...
*/
	if( pl[14] > 0 )
	{
		...
		var stop = pl[0] != mz ? ' [<a href="javascript: void(0)" id="a_stop_move">stop</a>]' : '' ;
	...
	} else
	{
		...
		for( l = 1 ; l < locArr.length; l++ )
		{
			...
			} else
			{
				...
				a.title = locArr[l][5] + ' (' + getTimeL( pl[0] , l , 0 ) + ')' ;
				d.appendChild( a );
			}
			if( pl[13] == -1 && ( l == 3 ) )
			{
...
// есть охота
	...
	var eC = eval( 'locE.e' + kC[3] + '_' + kM[3] ) ;//exit
	var eM = eval( 'locE.e' + kM[3] + '_' + kC[3] ) ;//enter

	if( kC[3] != kM[3] )
	{
		if( cz == eC )
		{
			window.location.href = url + 'move_sector.php?id=' + eM ;
			return ;
		}
		move( cz , eC ) ;
		return ;
...
function setStop()
{
	GM_setValue( "lwm_map_move" , 0 ) ;
	GM_setValue( "lwm_map_move_mz" , 0 ) ;
	$('hint_move').innerHTML = 'Final destination: <b>' + locArr[pl[0]][5] + '</b>' ;
}
...
// если прибыли в конечный пункт
	...
	var eC = eval( 'locE.e' + kC[3] + '_' + kM[3] ) ;//координаты выхода
	var eM = eval( 'locE.e' + kM[3] + '_' + kC[3] ) ;//координаты входа
...
// если есть исключения, следующий сектор будет равен ид исключения
	...
	{
// если разные "комнаты", и текущая равна выходу, то вход, нет, то выход
		nz = kC[3] != kM[3] ? ( cz == eC ? eM : eC ) : 0 ;
		id2 = eval( 'locP.l' + cz + '_' + nz ) //Ид исключение
		if( id2 && id2 > 0 )
		{
			nz = id2 ;
		}
