// ==UserScript==
// @name           Travian upgrad v 1 3.9
// @author         ThunderStorm - AkkArApOn SeRmSRup
// @email   akkarapon.sermsup@gmail.com
// @namespace      Akkarapon.TravianAutoUpgrade
// @include        http://*.travianspeed.*/*.php*
// @exclude        http://*.travianspeed.*/activate.php*
// @exclude        http://*.travianspeed.*/ajax.php*
// @exclude        http://*.travianspeed.*/anleitung.php*
// @exclude        http://*.travianspeed.*/anmelden.php*
// @exclude        http://*.travianspeed.*/hilfe.php*
// @exclude        http://*.travianspeed.*/impressum.php*
// @exclude        http://*.travianspeed.*/index.php*
// @exclude        http://*.travianspeed.*/links.php*
// @exclude        http://*.travianspeed.*/manual.php*
// @exclude        http://*.travianspeed.*/support.php*
// @exclude        http://*.travianspeed.*/tutorial.php*
// @exclude        http://*.travianspeed.*/ad/*
// @exclude        http://*.travianspeed.*/chat/*
// @exclude        http://forum.travian.*
// @exclude        http://board.travian*.*
// @exclude        http://help.travian*.*
// @exclude        http://shop.travian*.*
// @exclude        http://www.travianspeed.*
// @version        0.13.2.2
// ==/UserScript==
...
const UPDATEURL = 'http://userscripts.org/scripts/source/79893.user.js';
const SCRIPTURL = 'http://userscripts.org/scripts/show/79893';

/******************** prepare all variable ********************/
var appConf = {
    fullServerName:"www.travianspeed.com",
    gServer:"www_com",
    lang:'fa',
    autoLogin:true,
    scriptPause:false,
...
function getCrtServer() {crtPage.search(/http:\/\/(.*)\//); appConf.fullServerName = RegExp.$1; appConf.gServer = appConf.fullServerName.replace(/\.travianspeed\./,'_'); return;};
...
function doLogin() {
    ...
    if(loginBtn != null) {	
	    ...
	    if ((passBox != null) && document.getElementsByName("name")[0].value.length > 0 ) {
		    ...
		    GM_setValue(appConf.gServer+'_RefreshPage','village2.php');
		    window.setTimeout(function() {loginBtn.click();}, 1000);
	    } else log(3,"Travian Auto Build: Auto-Login failed. You must have FireFox store the username and password.");
        retVal = true;
    } else {
        ...
        else if (refreshPage=='village2.php') {GM_setValue(appConf.gServer+'_RefreshPage','village1.php'); refreshTime = 1000; }
...
function getLanguage() {
	...
	if (iP) {
		...
		if ($xf("//img[contains(@src, 'travian1.gif')]")) appConf.plusAct = true;
	};
	iP = null; ahref = null;
}
...
function getSingleTownNewdid() {
	ajaxRequest("/village3.php", 'GET', null, function(ajaxResp) {
		if (ajaxResp) {
		    var ad = ajax2DIV(ajaxResp);
...
function startProcess() {
	...
	if (crtPage.match(/village2\.php/)) processUrlvillage2();
