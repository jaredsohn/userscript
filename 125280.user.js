 ...
 * @return
 */
function normalTrain(playerId, stepsDone, breakThroughCount) {
	if (scriptData.canceled) {
		log("script canceled");
		retirePlayer(playerId);
		return;
		...
		var params = [
				"training_type=normal&training1=", scriptData.firstAtt,
...
function handleTrainingResponse(responseText, playerId, stepsDone,
	...
	if (index > -1) {
		...
		if (responseText.indexOf(nonBTText) > -1) {
			debug("Normal Trained: ---");
		} else {
			debug("Normal Trained: Break Through!");
			breakThroughCount++;
		}
		normalTrain(playerId, stepsDone + 1, breakThroughCount);
	}
}
