...
// ==UserScript==
// @name           CSS Vault Redirect
// @namespace      http://binq.org/greasemonkey/
// @description    CSS Vault Redirect v2.0: CSS Beauty pages that are opened in a new tab/window take you directly to the dugg article.  It's very useful when looking through RSS feeds.
// @include        http://*.cssvault.com/*/*
// @include        http://cssvault.com/*/*
// ==/UserScript==
...
BINQ.cssvaultLinkJumper = {
	init: function() {
		this.jumpToFirstValidLink = BINQ.linkJumper.jumpToFirstValidLink;
		return this;
	},
	searchPath: "//div[@class='screenshot']//a[1]",
}

if (history.length == 1) { BINQ.cssvaultLinkJumper.init().jumpToFirstValidLink(); }