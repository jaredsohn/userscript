// ==UserScript==
// @name DS Letztes Dorf
// @description Fügt im Versamlungsplatz einen Link für das letzte angegriffene Dorf hinzu
// @author Michael Richter
// @namespace http://osor.de/
// @include http://*.staemme.ch.de/game.php*screen=place*
// ==/UserScript==
		...
		if(coords) {
			...
			if(td) {
				...
				if(coords.length == 3) {
					...
					link.textContent = unescape('%BB') + ' Letztes (' + coords[1] + '|' + coords[2] + ')';
				} else {
					...
					link.textContent = unescape('%BB') + ' Letztes (' + coords[1] + ':' + coords[2] + ':' + coords[3] + ')';
				}
				...
				if(coords.length == 3) {
					...
					link.title = img.alt = 'Auf Karte zentrieren';
