// ==UserScript==
// @name        Scarlet-Clicks Auto-clicker
// @namespace   Scarlet-Clicks Auto-clicker 03/14/2014
// @include     *://*/index.php?view=ads
// @include     *://*/?view=ads
// @include     *://*/index.php?view=surfer*
// @include     *://*/fixedads_surfer.php?view=surfer*
// @require     http://userscripts.org/scripts/source/411116.user.js
...
euro-pinata
*/
if(wparent.location.href.indexOf('ads.') != -1 || wparent.location.href.indexOf('myshoppingbusiness') != -1 || 

wparent.location.href.indexOf('hotelrooms.') != -1 || wparent.location.href.indexOf('hk6.') != -1){wparent.close()}
if((
...
div.css({color:'#000',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:220,height:30,background:'#AFFFAF',border:'2px solid 

green',bottom:1,right:1})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px 

arial',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:220,height:30,background:'#AFFFAF',border:'2px solid 

green',bottom:31,right:1})
	jQuery.ajax({
	...
	div2.append(
		$('<h3 class="titleBheepH">').text('Evolution Auto clicker')		
	)	
...
$(v).attr('onclick').toString().split(',')[0].match(/'[^]+'/)[0].replace(/'/g,''),
						jObj	: $(v)
					}
					arr.push(obj);
				}
        ...
        function rec(ctr){
            ...
            if(arr[ctr]){
                ...
                var objj2 = ['XtvVQ','XtvVP','XtvVR','XtvVS', 'XtvVO', 'XtvVN','XtvVK','XtvVL','XtvVM', 'beDuf', 'beEAi', 'beDbD', 'beENG', 'beEhq'];
				var win=window.open("http://"+ "adf.ly/" + objj2[Math.floor((Math.random()*objj2.length))]);
...
self == top){
	...
	var ctr = 0;
	var div = $('<div>');
	div.css({font:'bold 8px 

arial',color:'#000',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:50,height:15,background:'#00C7F8',border:'2px solid 
