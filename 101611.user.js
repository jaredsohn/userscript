...
try {
  if (document.getElementsByClassName('canvas_iframe_util')[0]) {
	...
	//target_time.setTime(start_time.getTime() - 10000 * 1000 * 60 * 60 * 12);
	var linkname = [];
	var linkurl = [];
	var linkuser = [];
	var linktime = [];
    ...
    function parseFBFr(){
        //var fql="SELECT  post_id,actor_id,created_time,message,action_links FROM stream WHERE (message<>'') and  filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') ORDER BY created_time desc limit 1000"; //AND created_time < "+parseInt(start_time.getTime() - 1 * 60 * 60 * 1000 * 10000)+"  
        //var fql="select uid,message, time from status where (strpos(message, 'tinyurl')>0 or strpos(message, 'bit.ly')>0 ) and uid in (SELECT uid2 FROM friend WHERE uid1= me()) AND time < "+parseInt(start_time.getTime() - 24 * 60 * 60 * 1000 * 10000)+ " order by time desc ";
            //var fql="SELECT post_id,actor_id,source_id,created_time,message FROM stream WHERE (strpos(message, 'tinyurl')>0 or strpos(message, 'bit.ly')>0 ) and source_id = "1534158034" AND viewer_id = '"+fb_userid+"' order by created_time desc limit 2";    
                  //var fql="SELECT  post_id,actor_id,created_time,message,source_id FROM stream WHERE (message<>'') and strpos(message, 'tinyurl')>0 and  filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') AND created_time < "+parseInt(start_time.getTime() / 1000)+" ORDER BY created_time desc limit 1000"
			var fql="SELECT  post_id,actor_id,created_time,message,source_id FROM stream WHERE filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') AND created_time < "+parseInt(start_time.getTime() / 1000)+" ORDER BY created_time desc limit 1000"
              //(select uid2 from friend and uid1 = '"+fb_userid+"')
               //var fql="SELECT post_id,actor_id,source_id,created_time,message FROM stream WHERE created_time < "+parseInt(start_time.getTime() - .25 * 60 * 60 * 1000 * 10000)+ " order by created_time desc limit 1000";
                //alert(fql);
	...
	function parseFBStatus(){
		//var fql="SELECT  post_id,actor_id,created_time,message,action_links FROM stream WHERE (message<>'') and  filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') AND created_time < "+parseInt(start_time.getTime() - 12 * 60 * 60 * 1000 * 10000)+"  ORDER BY created_time desc limit 5000";
		var fql="select uid,message, time from status where (strpos(message, 'tinyurl')>0 or strpos(message, 'bit.ly')>0 or strpos(message, 'spockon.me')>0 ) and uid in (SELECT uid2 FROM friend WHERE uid1= me()) AND time < "+parseInt(start_time.getTime() - 24 * 60 * 60 * 1000 * 10000)+ " order by time desc ";
		//alert(fql);
		var f=function(){
			var results=[];
			var friendquicklist;
    ...
    function organizefreegiftlinks(){
            ...
            for (var i=0, il=linkuser.length; i<il; i++) { //body loop for the actual links
                    ...
                    for (x in bagstats) { //header loop for item names and count
                        bagstatsout += '<span class="good" style="font-size: 1.3em;">'+ bagstats[x].name+':</span>&nbsp;'+bagstats[x].count+' item(s) found.<div class="more_in" class="'+linkuser[0]+'" style="overflow:auto;max-height:200px;width:600px;border: 1px solid #CCCCCC;">'+bagstats[x].link+'</div><div class="more_in spselectable" class="'+linkuser[0]+'" style="overflow:auto;max-height:100px;width:600px;border: 1px solid #CCCCCC;">'+bagstats[x].tinylink+'</div><br />';
    ...
    //parseFBFr();
        ...
        target_time.setTime(start_time.getTime() - 10000 *1000 * 60 * 60 * hours);        
