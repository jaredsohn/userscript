// ==UserScript==
    ...
    function fightBoss()
    {
            ...
            if (!executeCommandsInQueue())
            {
                    //fightWithSkillOrRefillSkillPoints(1)
                    //this means to use the first skill you have
                    //fightWithSkillOrRefillSkillPoints(2)
                    //this means the seccond skill
                    //fightWithSkillOrRefillSkillPoints(0)
                    // this means use your strongest skill
                    if ($inc1("mobnamespan") == "Elite Alien Witch")
                    {
                            addToQueue(["fightWithSkillOrRefillSkillPoints(10)","clickButton('Continue')"]);
                    }
                    else if ($inc1("mobnamespan") == "Elite Alien Scout")
                    {
                            addToQueue(["fightWithSkillOrRefillSkillPoints(10)","clickButton('Continue')"]);
                    }
                    else if ($inc1("mobnamespan") == "Elite Alien Scientist")
                    {
                            addToQueue(["fightWithSkillOrRefillSkillPoints(10)","clickButton('Continue')"]);
                    }
                    else if ($inc1("mobnamespan") == "Timelord")
                    {
                            addToQueue(["fightWithSkillOrRefillSkillPoints(10)","clickButton('Continue')"]);
                    }
                    else // this must be ms so use best continue
                    {
                            addToQueue(["fightWithSkillOrRefillSkillPoints(0)","clickButton('Continue')"]);
                    }
            }
    }
    ...
    {
            return getCursp()<60?refillSkillPoints():fightWithSkill(skill)
    }
    ...
    {
            if (isLoading()) return;
            exploreCommand="clickExploreOrWait(4)";
            fightCommand="fightWithNormalcontinueAndDisposeItem()";
            setAutofightEnabled(false);
                    ...
                    else if ($inc1("enctext"))
                    {
                            ...
                            switch(mob.textContent)
                            {
                                    ...
                                    case "Guardian of Dreams":
                                            ...
                                            break;
                                    case "Little Imp":
                                            queue=["clickButton('Accept')",fightCommand,exploreCommand];
                                            break;
                                    case "Magic Elf":
                                            queue=["clickButton('continue')","fightWithSkill(0)","getFightResultAndDisposeItem()",exploreCommand];
                                            break;
                                    // case "New Ally":
                                            // break;
    ...
    function fightWithNormalcontinueAndDisposeItem()
    {
            result=fightWithNormalcontinue();
            if (result)
            {
    ...
    function openInventoryAndUseItem(item,addCommands)
    {
            ...
            queue.push("clickButton('Close',1)");
    ...
    function searchForMoney()
    {
            ...
            fightCommand="fightWithNormalcontinue()";
            setPuzzleSolverEnabled(puzzleSolverEnabled);
            if (!executeCommandsInQueue())
            {
                    var queue=null;
    ...
    function fightWithNormalcontinue()
    {
            var result=getFightResult();
            if (!result && clickButton("continue")) resetQueueFailedRepeatedCommand();
            return result;
    }
    ...
    {
            return getCursp()<60?openInventoryAndUseItem("fairy dust"):false;
    }
    ...
    function followTutorial()
    {
            ...
            if (result) addToQueue(["clickButton('Village Chief')","clickButton('Back to Town')","clickButton('The Marsh')","emptyTurn()","clickExplore()","emptyTurn()","clickButton('Taste')","reload()","changeStateAction('maxMoney')"]); //,"clickExplore()","continue()","clickExplore()","clickButton('Town Map')"
    }
