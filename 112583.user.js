// ==UserScript==
// @name           Massive-RT
// @namespace      http://najashark.blogspot.com
// @description    test mass RT
...
if(body != null) {
	...
	divLike.innerHTML = "<a style=\"font-weight:bold;color:#ffffff\" href=\"JavaScript:macroLike()\">M-RT</a>"
	...
	divUnlike.innerHTML = "<a style=\"font-weight:bold;color:white\" href=\"JavaScript:macroUnLike()\">M-FAV</a>"
	...
	divCom.innerHTML = "<a style=\"font-weight:bold;color:#ffffff\" href=\"JavaScript:macroCom()\">M-REPLY</a>"
	...
	unsafeWindow.macroLike = function() {
		...
		for(i = 0; i < buttonsLike.length; i++) {
			...
			if(myClass != null && myClass.indexOf("like_link") >= 0){
				if(buttonsLike[i].getAttribute("name") == "RT"){
					buttonsLike[i].click();
					globalCounterLike++;
	...
	unsafeWindow.macroUnLike = function() {
		...
		for(i = 0; i < buttonsUnlike.length; i++) {
			...
			if(myClass != null && myClass.indexOf("like_link") >= 0){
				if(buttonsUnlike[i].getAttribute("name") == "FAV"){
	...
	unsafeWindow.macroCom = function() {
		...
		for(i = 0; i < buttonsComment.length; i++) {
			if(buttonsComment[i].getAttribute("name")=="reply"){
