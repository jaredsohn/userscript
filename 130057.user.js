...
var SCRIPT = {
	name: 'Core Family (dynamic) 1.0',
	namespace: 'vladykins',
	source: 'http://userscripts.org/scripts/show/130057',
	identifier: 'http://userscripts.org/scripts/source/130057.user.js',
	meta: 'http://userscripts.org/scripts/source/130057.meta.js',
	version: '1.0.9',
	date: '2012-04-03',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4868998';
var postId = '45030512';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4868998';
// var postId = '4868998';
var forumAccessURL = 'http://goallineblitz.com/game/forum_private_access.pl?forum_id=11594';
...
var listVersion = 0;
var listNames = ['family', 'apprentice', 'extended', 'waitlist', 'hitlist'];
var list = {
	family: {
		member: {},
		title: 'Core.',
	...
	apprentice: {
		...
		title: 'n00b',
		style: 'color: yellow;',
		agentTitle: function(id, name) { return 'n00b'; },
		forumAccess: 1,
	},
	extended: {
		member: {},
		...
		style: 'color: blue;',
		agentTitle: function(id, name) { return list.extended.member[id]; },
		forumAccess: 1,
	},
	waitlist: {
		member: {},
	...
	hitlist: {
		...
		agentTitle: function(id, name) { return '!! CAUTION !!'; },
		forumAccess: 0,
	},
};
var listName = {};
		...
		if (pref.indexHilite.val) {
			$('#content table.forums span.last_post_user').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html(parts[0] + '<br><b style="color: black;">' + parts[1] + '</b>');
				}
			});
			$('#threads span.thread_starter').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html('Started by: <b style="color: black;">' + name + '</b>');
				}
			});
			$('#threads span.last_post_user').each(function() {
				...
				var id = nameToId[name];
				if (list.family.member[id]) {
					$(this).html('<b style="color: black;">' + name + '</b>');
				}
			});
		}
		break;
...
function loadFromCurPage() {
	...
	(function() {
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			var membersextended = 0;
			var membersWaitList = 0;
			var membershitlist = 0;
			var membersMisnamed = 0;
			$('#content div.content_container tbody tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				if (list.hitlist.member[id]) {
					...
					delete names[id];
					names.num--;
				} else if (list.extended.member[id] || list.apprentice.member[id] || list.family.member[id]) {
					if (list.family.member[id]) {
						++membersWithTitles;
					} else if (list.apprentice.member[id]) {
						++membersApprentice;
					} else if (list.extended.member[id]) {
						++membersextended;
					}
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			if (membersWithTitles) {
				statusMsg += prefix + '<b style="color: black;">' + membersWithTitles + ' Core.</b>';
				prefix = ', ';
			}
			...
			if (membersextended) {
				statusMsg += prefix + '<b style="color: blue;">' + membersextended + ' friendly</b>';
				prefix = ', ';
			}
			if (membersWaitList) {
			...
			statusMsg += ')';
			if (nonMemberAddicts) {
				statusMsg += ', ' + nonMemberAddicts + ' without forum access';
			}
