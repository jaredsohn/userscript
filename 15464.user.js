// ==UserScript==
// @name          Amazon Goshen Public Library Linky
// @namespace     http://fatknowledge.blogspot.com
// @description	  v1.3 Search the Goshen Public Library Catalog from Amazon book listings.  
// @include       http://*.amazon.*
// ==/UserScript==
...
(function(){
//	GM_log('Amazon Goshen Public Library Linky');	

	var libraryIsbnUrlPattern = 'http://ipac.goshenpl.lib.in.us/ipac20/ipac.jsp?index=ISBN&term='
	var libraryTitleUrlPattern = 'http://ipac.goshenpl.lib.in.us/ipac20/ipac.jsp?index=TW&term='
	var libraryName = 'the Goshen Public Library';
...
// updated url from 'http://labs.oclc.org/xisbn/' to 'http://old-xisbn.oclc.org/webservices/xisbn/'
	var wbUrl = 'http://old-xisbn.oclc.org/webservices/xisbn/' + isbn;
	GM_xmlhttpRequest({
	    method: 'GET',
	    url: wbUrl,
	    headers: {
...
//	GM_log('Searching: '+libraryUrlPattern);	
	...
	var libraryNewShelves = /New Book Area/;
	var libraryOnOrder = /On Order/;
	var libraryInProcess = /In Process/;
	var libraryDueBack = /(\d{2}\ \D{3}\ \d{2})/;
...
//					"#2bff81" //light green
			...
			else if ( libraryNewShelves.test(page) )
				{
				setLibraryHTML(
					libraryUrlPattern, isbn,
					"On the shelf now!",
