...
// Create the tooltip and get the translation
function lookup(evt)
{
    var divResult = null;
    var divDic = getId('divDic');
    ...
    //div container
    divDic = createElement('div', {id:'divDic', style:'background: rgba(237, 244, 252, 0.9); color:#000000; position:absolute; top:'+top+'; left:'+left+'; min-width:250px; min-height:50px; max-width:50%; padding:5px; font-size:small; text-align:left; z-index:10000; border-radius:4px; box-shadow: -2px 0px 9px 5px #898D91'});
    ...
    //options link
    divDic.appendChild(createElement('a', {id:'optionsLink', href:HREF_NO, style:'position:absolute; bottom:3px; right:13px; font-size:18px; text-decoration:none!important;background:#528DDF;padding:1px;color:#fff;border-radius:6px 6px 6px 6px;border:2px solid #EEEEEE;font-weight:bold;width:20px;text-align:center;display:block;'}, 'click options false', '+'));
    ...
    else
    {
		...
		currentURL = "http://translate.google.fr/translate_a/t?client=t&text=" + encodeURIComponent(txtSel) + "&langpair=" + lang; // This adress return an array as answer
        GM_xmlhttpRequest({
            method: 'GET',
            url: currentURL,
...
// Lanched when we select an other language
function quickLookup()
{
    getId('divResult').innerHTML = 'Loading...'
		...
		currentURL = "http://translate.google.fr/translate_a/t?client=t&text=" + txtSel + "&langpair=" + getId('optSelLangFrom').value + "|" + getId('optSelLangTo').value; // This adress return an array as answer
    GM_xmlhttpRequest({
        method: 'GET',
        url: currentURL,
        onload: function(resp) {
...
{
	arr = eval(gTradStringArray); // eval is used to transform the string to an array. I alse made a custom parsing function, but it doesn't handle antislashed characters, so I prefer using eval()
