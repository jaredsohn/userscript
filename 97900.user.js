...
var Script = {
    ...
    init : function() {
        ...
        GM_registerMenuCommand("Thai CA Monster Info Setting", function() {Setting.init();});
        Setting._read();
...
UI = {
    ca_stats_panel : function() {
        ...
        if(!ca_info.size()) {
            ca_info = $j("<textarea></textarea>").attr({ id:'ca_info', wrap:'on', rows : 13}).css({margin:0, padding:'5px',border:"solid 1px black", width:'188px'});
...
Parser = {
    ...
    stats : function() {
        ...
        if(/You need (\d+) more experience points to get to the next level/.test($j("#app46755028429_st_5").attr("title"))) {
            ...
            if(this.battle_monster()) {
                var tmpUser =  document.getElementById('navAccountName');
				var user = tmpUser.toString().split("="); 
                if(user.length) {
                    dmg = $j.trim($j("div.statsT2 table table table tr td:has(a[href=http://apps.facebook.com/castle_age/keep.php?casuser="+user[1]+"]) + td").text());
    ...
    battle_monster : function() {
        var arrUrl = Parser.clickUrl.split('?',2), query, summoner, mpool, mid, time, ca_info, user_locale, gettext;
        ...
        if(!(/\/battle_monster.php$/.test(arrUrl[0]) && arrUrl.length == 2 && (query = Util.parseQuery(arrUrl[1])) && (summoner = (query.casuser || query.user)) && (mpool = query.mpool))) {
			if(!(/\/festival_battle_monster.php$/.test(arrUrl[0]) && arrUrl.length == 2 && (query = Util.parseQuery(arrUrl[1])) && (summoner = (query.casuser || query.user)) && (mpool = query.mpool) && (mid = query.mid))) {
				if(!(/\/player_monster_list.php$/.test(arrUrl[0]))) {
					 if(!(/\/public_monster_list.php$/.test(arrUrl[0]))) {
						UI.ca_info_panel().remove();
						return false;
					 }else{						
						UI.ca_info_panel().remove();
        ...
        if(time.length == 3) {
			...
			if(mid != undefined){
				ca_info.text(info.replace('[links]', "http://apps.facebook.com/castle_age/festival_battle_monster.php?casuser=" + summoner + "&mpool=" + mpool +"&mid=" + mid + (boss.siege ? "&action=doObjective" : "")+"\n" ));		
			}else{	
...
Setting = {
    ...
    locale : function(value) {
        return this._set("LOCALE", "th_th", value);
    },
