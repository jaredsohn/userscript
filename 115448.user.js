...
// @version			The Invincible Black Edition v3
// ==/UserScript==
(function() {
 ...
 ********************************************************************************/
// Script information
var Version = 'The Invincible Black Edition v3'; // Year, Month, Day, Revision, Maturity (e.g. YYYMMDDa_BETA)
...
var kScanningMap = 'Scanning map within 50 miles<BR>This should take about a minute';
var kList = ' List';
var kDistance = 'Dist';
var kCoords = 'Coords';
var kLvl = 'Lvl';
...
*) added Maps tab - searches and displays ant camps, cities, outposts, grasslands, swamps, lakes, hills, plains, mountains, and forests within 50 miles
...
var Styles = '\
    div.' + rndClass['tab_title'] + ' {border:1px solid; border-color:#ffffff; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#222}\
    ...
    div.' + rndStyle['classPrefix'] + 'Input {border:2px ridge yellow; background-color:#5F5F5F; padding:3px}\
    div.' + rndStyle['status_ticker'] + ' {border:2px ridge black; background-color:#5F5F5F; padding:2px}\
	div.' + rndStyle['status_feedback'] + ' {height: 34px; border:1px solid white; color: white; background-color:#111; padding: 2px 0px 2px 2px; text-align:left; font-weight:bold;}\
    ...
    table tr.' + rndStyle['classPrefix'] + 'TabHdr1 td {background-color:#6F6F6F; font-weight:bold}\
    ...
    .' + rndStyle['classPrefix'] + 'CP .' + rndStyle['classPrefix'] + 'CPMain { background-color:dimgrey; padding:6px;}\
    ...
    input.short {width:30px}\
        input.' + rndClass['red_button'] + ', input.' + rndClass['refresh_button'] + ', input.' + rndClass['blue_button'] + ', input.' + rndClass['yellow_button'] + ', input.' + rndClass['cyan_button'] + ', input.' + rndClass['purple_button'] + ' {width:130px; color:white; font-weight:bold; cursor:hand; cursor:pointer;}\
    input.' + rndClass['red_button'] + ':active, input.' + rndClass['refresh_button'] + ':active, input.' + rndClass['blue_button'] + ':active, input.' + rndClass['yellow_button'] + ':active, input.' + rndClass['cyan_button'] + ':active, input.' + rndClass['purple_button'] + ':active {width:130px; background-color:black; color:white; font-weight:bold; cursor:hand; cursor:pointer; }\
    input.' + rndClass['red_button'] + ':hover, input.' + rndClass['refresh_button'] + ':hover, input.' + rndClass['blue_button'] + ':hover, input.' + rndClass['yellow_button'] + ':hover, input.' + rndClass['cyan_button'] + ':hover, input.' + rndClass['purple_button'] + ':hover {width:130px; color:white; font-weight:bold; cursor:hand; cursor:pointer; }\
	input.' + rndClass['red_button'] + ' {background-color:#BF0000;}\
	input.' + rndClass['red_button'] + ':hover {background-color:#DF0000;}\
	input.' + rndClass['refresh_button'] + ' {background-color:#2F2F2F;}\
	input.' + rndClass['refresh_button'] + ':hover {background-color:#4D4D4D;}\
	input.' + rndClass['blue_button'] + ' {background-color:#2F2F2F;}\
	input.' + rndClass['blue_button'] + ':hover {background-color:#4D4D4D;}\
    ...
    .button {cursor:hand; cursor:pointer; border: 1px solid #006; background: #BC393F; color: white; padding: 2px; font-weight:bold; font-size:13px; border-style: solid solid none solid;}\
//  .button:hover {background: dimgrey; font-weight:bold; font-size:13px; color: black; border-style: solid solid none solid; }\
//  .button:active {background: black; font-weight:bold; font-size:13px; color: white; border-style: none none none none;}\
    ...
    #hdRnd ul.tabs li.tab a.' + rndStyle['classPrefix'] + 'toolbutOff {background-color:#000 ! important; color:white ! important}\
    ';
...
//*** Attacks Tab - Stats Sub-tab ***
  ...
  clearStats : function (){
	...
	checkMapData : function () {
		...
		if (t.checkMapBusy)
			return false;
		if (Data.targets.radius!=50 || Data.targets.center.x!=Seed.cities[0].x || Data.targets.center.y!=Seed.cities[0].y) {
			if (Data.options.verboseLog.enabled) {
			...
			Seed.fetchSeed(function (rslt) {
				if (rslt.ok) {
					...
					if (Data.targets.radius!=50 || Data.targets.center.x!=Seed.cities[0].x || Data.targets.center.y!=Seed.cities[0].y) {
						...
						t.scanMap(50, function(){logit('****** Setting checkMapBusy to FALSE'); Tabs.AutoAttack.checkMapBusy = false});
						return false;
					}
				} else {
					if (Data.options.verboseLog.enabled)
 ...
 // Data.options.campAttack {enabled:false, maxDist:7, repeatTime:3660, delayMin:15, delayMax:25, levelEnable:[], levelDist:[]}
  ...
  tabConfigLevels : function (){
    ...
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+ kAutoAttack + Data.options.mapChoice + kLevels +'</div>\
        ...
        <TR align=center class=' + rndStyle['classPrefix'] + 'TabHdr1><TD style="background:none !important;"></td><TD>1</td><TD>2</td><TD>3</td><TD>4</td><TD>5</td><TD>6</td><TD>7</td><TD>8</td><TD>9</td><TD>10</td><TD>11</td></tr>\
...
//    researchTimer   : null,
	...
	tabJobInfo : function (){
        ...
        function cityTitle (cityIdx){
            var city = Seed.cities[cityIdx];
		var Ops = new Array();
				Ops[city.name] = "<span style=color:forestgreen>" + city.name + "</span>";
				Ops['Outpost 1'] = "<span style=color:blue>Waterdragon Outpost</span>";
				Ops['Outpost 2'] = "<span style=color:grey>Stonedragon Outpost</span>";
				Ops['Outpost 3'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops['Outpost 4'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
            // Outposts are always defending (until further notice)
            var wallStatus = '';
            var alliance_name = (Seed.s.alliance) ? Seed.s.alliance.name : '';
...
var Ops = new Array();
				Ops[kCityNumber + '1'] = "<span style=color:forestgreen>City</span>";
				Ops[kCityNumber + '2'] = "<span style=color:blue>Waterdragon Outpost</span>";
				Ops[kCityNumber + '3'] = "<span style=color:grey>Stonedragon Outpost</span>";
				Ops[kCityNumber + '4'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops[kCityNumber + '5'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
	...
	tabJobResearch : function (){
        ...
        var city = Seed.cities[0];
    var Ops = new Array();
				Ops[kCityNumber + '1'] = "<span style=color:forestgreen;font-weight:bold;>City</span>";
...
var Ops = new Array();
				Ops[city.name] = "<span style=color:forestgreen>City</span>";
				Ops['Outpost 1'] = "<span style=color:blue>Waterdragon Outpost</span>";
				Ops['Outpost 2'] = "<span style=color:grey>Stonedragon Outpost</span>";
				Ops['Outpost 3'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops['Outpost 4'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
