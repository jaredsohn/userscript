...
var SCRIPT = {
	name: 'Brethren Krewe',
	namespace: 'DavyJonesLocker',
	source: 'http://userscripts.org/scripts/show/159427',
	identifier: 'http://userscripts.org/scripts/source/159427.user.js',
	meta: 'http://userscripts.org/scripts/source/159427.meta.js',
	version: '1.0',
	date: '2012-02-01',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=5046466';
var postId = '46941133';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=5046466&page=2';
// var postId = '46941133';
var forumAccessURL = 'http://goallineblitz.com/game/forum_private_access.pl?forum_id=1107';
...
var listVersion = 0;
var listNames = ['family', 'Admiral', 'Captain', 'Quartermaster', 'Cockswain', 'Pirate', 'Landlubber', 'Walked The Plank!'];
var list = {
	family: {
		member: {},
		title: 'Brethren Krewe',
		style: 'color: Red; background-color: black; padding: 0px 3px 0px 3px;',
	...
	admiral: {
		...
		agentTitle: function(id, name) { return 'Admiral'; },
		forumAccess: 1,
	},
	captain: {
		member: {},
		...
		agentTitle: function(id, name) { return 'Captain'; },
		forumAccess: 1,
	},
	quartermaster: {
		member: {},
		title: 'QUartermaster',
		style: 'color: silver;',
		agentTitle: function(id, name) { return 'Quartermaster'; },
		forumAccess: 1,
	},
	cockswain: {
		member: {},
		title: 'COckswain',
		style: 'color: red;',
		agentTitle: function(id, name) { return 'Cockswain'; },
		forumAccess: 1,
	},
	pirate: {
		member: {},
		title: 'PIrate',
		style: 'color: red;',
		agentTitle: function(id, name) { return 'Pirate'; },
		forumAccess: 1,
	},
	landlubber: {
		member: {},
		title: 'LAndlubber',
		style: 'color: green;',
		agentTitle: function(id, name) { return 'Landlubber'; },
		forumAccess: 1,
	},
	walkedtheplank!: {
		member: {},
		title: 'WalkedThePlank!',
		style: 'color: pink; background-color: white; padding: 0px 3px 0px 3px;',
		agentTitle: function(id, name) { return 'Walked The Plank!'; },
		forumAccess: 0,
	},
};
var listName = {};
		...
		if (pref.indexHilite.val) {
			$('#content table.forums span.last_post_user').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html(parts[0] + '<br><b style="color: LightGoldenrodYellow; background-color: black; padding: 0px 3px 0px 3px;">' + parts[1] + '</b>');
				}
			});
			$('#threads span.thread_starter').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html('Started by: <b style="color: LightGoldenrodYellow; background-color: black; padding: 0px 3px 0px 3px;">' + name + '</b>');
				}
			});
			$('#threads span.last_post_user').each(function() {
				...
				var id = nameToId[name];
				if (list.family.member[id]) {
					$(this).html('<b style="color: LightGoldenrodYellow; ">' + name + '</b>');
				}
			});
		}
		break;
...
function loadFromCurPage() {
	...
	(function() {
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			var members = 0;
			var membersBrethren Krewe = 0;
			var membersAdmiral = 0;
			var membersCaptain = 0;
			var membersQuartermaster = 0;
			var membersCockswain = 0;
			var membersPirate = 0;
			var membersLandlubber = 0;
			var membersWalked The Plank! = 0;
			...
			$('#content div.content_container tbody:eq(0) tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				++members;
				if (list.walkedtheplank.member[id]) {
					++memberWalkedtheplank;
					console.warn('Walked The Plank! agent found on forum access list: ' + id + ' ' + realName + ' (listed as ' + names[id] + ')');
					delete names[id];
					names.num--;
				} else if (list.landlubber.member[id] || list.pirate.member[id] || list.cockswain.member[id] || list.quartermaster.member[id] || list.captain.member[id] || list.admiral.member[id] || list.family.member[id]) {
					if (list.family.member[id]) {
						++membersBrethren Krewe;
					} else if (list.admiral.member[id]) {
						++membersAdmiral;
					} else if (list.captain.member[id]) {
						++membersCaptain;
					} else if (list.quartermaster.member[id]) {
						++membersQuartermaster;
					} else if (list.cockswain.member[id]) {
						++membersCockswain;
					} else if (list.pirate.member[id]) {
						++membersPirate;
					} else if (list.landlubber.member[id]) {
						++membersLandlubber;
					}
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			$.each(names, function(id, name) {
				if (!(id = parseInt(id))) return;
					...
					++nonMemberAddicts;
					$('#fam_list_family a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.admiral.member[id]) {
					$('#fam_list_admiral a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.captain.member[id]) {
					$('#fam_list_captain a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.quartermaster.member[id]) {
					$('#fam_list_quartermaster a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.cockswain.member[id]) {
					$('#fam_list_cockswain a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.pirate.member[id]) {
					$('#fam_list_pirate a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.landlubber.member[id]) {
					$('#fam_list_landlubber a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list[idToList[id]].forumAccess == 1) {
					$('#post_content_' + postId + ' a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				}
			});
			var prefix = ' (';
			...
			if (membersBrethrenKrewe) {
				statusMsg += prefix + '<b style="color: Red; background-color: black; padding: 0px 3px 0px 3px;;">' + membersBrethrenKrewe + ' Brethren Krewe</b>';
				prefix = ', ';
			}
			if (membersADmiral) {
				statusMsg += prefix + '<b style="color: darkblue;">' + membersADmiral + ' Admiral</b>';
				prefix = ', ';
			}
			if (membersCAptain) {
				statusMsg += prefix + '<b style="color: LightGoldenrodYellow;">' + membersCAptain + ' Captain</b>';
				prefix = ', ';
			}
			if (membersQUartermaster) {
				statusMsg += prefix + '<b style="color: silver;">' + membersQUartermaster + ' Quartermaster</b>';
				prefix = ', ';
			}
			if (membersCOckswain) {
				statusMsg += prefix + '<b style="color: red;">' + membersCOckswain + ' Cockswain</b>';
				prefix = ', ';
			}
			if (membersPIrate) {
				statusMsg += prefix + '<b style="color: red;">' + membersPIrate + ' Pirate</b>';
				prefix = ', ';
			}
			if (membersLAndlubber) {
				statusMsg += prefix + '<b style="color: green;">' + membersLAndlubber + ' Landlubber</b>';
				prefix = ', ';
			}
			if (memberWalkedThePlank!) {
				statusMsg += prefix + '<b style="color: pink; background-color: white; padding: 0px 3px 0px 3px;">' + memberWalkedThePlank + ' Walked The Plank!</b>';
				prefix = ', ';
			}
			if (unknown.length) {
			...
			statusMsg += ')';
			if (nonMemberAddicts) {
				statusMsg += ', ' + nonMemberAddicts + ' Gone to Davy Jones' Locker';
			}
