...
// ==UserScript==
// @name           KOC Power Bot By Joshua Offord
// @version        FORK_20130221a
// @namespace      Joshua Offord
...
Tabs.tower = {
  ...
  init: function(div){
    ...
    //m += '<BR><IFRAME style=";border:0;width:500px;max-width:500px;height:50px;max-height:50px;overflow:auto" src="http://nicodebelder.eu/koc/status.php"></iframe>';
    m += '<BR><DIV id=FBstatus></DIV>';
    m += '<BR><DIV id=FBconnect></DIV>';

    //m += '<BR><IFRAME style=";border:0;width:500px;max-width:500px;height:1px;max-height:1px;overflow:auto" src="http://nicodebelder.eu/koc/test.html"></iframe>';
    m += '<BR><INPUT id=TowerMail type=submit value=Authenticate><FONT color=red><B>&nbspAuthenticate this app to receive a E-Mail on tower alert.</b></font>';
    ...
    document.getElementById('TowerMail').addEventListener ('click', function (){unsafeWindow.top.location="http://nicodebelder.eu/koc/alert.php";}, false);
...
Tabs.Dashboard = {
    ...
    init : function (div){
        ...
        if (GlobalOptions.pbWideScreenStyle=="wide" || GlobalOptions.pbWideScreenStyle=="ultra") {
	        ...
	        m += '<DIV id=EmailStatus></div>';
	        m += '<DIV id=EmailFrame></div>';       
        } else {
        	var m = '<div class="pbStat" id=dbHeader></div>';
	        m += '<TABLE width=100% class=pbTab cellspacing=0 cellpadding=0>';
...
if (typeof(GM_xmlhttpRequest) !== 'undefined' && typeof(GM_updatingEnabled) === 'undefined')  setTimeout(function(){AutoUpdater_155469.check();},15000); 
...
function Authenticate (){
	...
	var msg = 'A new function has been added to Power BOT: Send to mail';
	msg += '\n\t- Send your Tower alerts to your E-Mail\n\t- Forward messages to your E-mail';
	msg += '\n\nIf you want to enable this function, press ok and allow the app.';
	var r=confirm(msg);
	if (r==true) {
		Options.alertConfig.email = true;
		Options.Authenticate = false;
    	saveOptions();
    	unsafeWindow.top.location="http://nicodebelder.eu/koc/alert.php";
	} else {
		Options.Authenticate = false;
		saveOptions();
		alert("You can enable this app by pressing \"Authenticate\" in the tower tab.");
	} 
}
...
function GetFbToken (){
	document.getElementById('EmailFrame').innerHTML = '<IFRAME style=";border:0;width:1px;max-width:1px;height:1px;max-height:1px;overflow:auto" src="http://nicodebelder.eu/koc/fb.html"></iframe>';
	document.getElementById('EmailStatus').innerHTML = '<IFRAME style=";border:0;max-width:300px;max-height:75px;overflow:auto" src="http://nicodebelder.eu/koc/status.php"></iframe>';
}
...
function eachSecond (){
	...
	if (Seconds==5) GetFbToken();
	if (Seconds%600==0) GetFbToken();

	//if (Seconds=10) alert(unsafeWindow.cm.mww.toSource());
}
