...
'</fieldset><!-- end GAMEPLAY --><div style="position:fixed;top:55px;text-align:right;width:570px;"><strong><a href="javascript:void(0)" id="close-user-guide" style="background:#eeffee;">[x]</a></strong></div></div>';
	...
	function updateAction() {
		try{
			actionString = document.getElementById('action').textContent;
		} catch(err) {
			return;
		}
		if (actionString) {
		...
		for (var i in mapInfo.countries) {
			...
			if (country.name == fromTerritory) {
				setCursor('pointer');
				fromTerritory_armies = getArmiesForIndex(i).quantity;
	...
	function onMapMouseMove(e) {
		...
		mouseOverX = e.pageX - mapOffsetLeft - 19;
		mouseOverY = e.pageY - mapOffsetTop + 4;
		try{
			document.getElementById('magicmap').title = '';
		}catch(err){}
		innerMap.title = '';
	...
	function onMapClick(e) {
		...
		if (selectQuantity) {
			numArmies = selectQuantity.value;
			selectQuantity.focus();
		}
		switch(doAction) {
			case 'Deploy':
			...
			case 'Assault':
				...
				if (playerNumber == currentPlayerNumber) {
					...
					try{
						...
						selectFromCountry.dispatchEvent(evt);
					}catch(err){
						wait('off');
						return;
					}
	...
	function getCurrentPlayer() {
		...
		for (i = 0; i < allPlayers.length; i++) {
			debug('checking player: '+allPlayers[i]);
	...
	function debug(text) {
		if (debugging && unsafeWindow.console) {
		...
		button.addEventListener('click', function (evt) {
				...
				if (userPrefs['confirm'+command] == 'Y') {
					if (command == "Deploy") {
						...
						confirmMessage = 'Advance '+numArmies+' troop(s)?';
					} else if (command == "Reinforce") {
						confirmMessage = 'Reinforce '+numArmies+' troop(s) from '+fromTerritory+' to '+toTerritory+'?';
					}
...
// DECLARATIONS
var outerMap,innerMap,mapInfo,outerMapPos,mapCell,currentPlayerName,currentPlayerNumber, actionForm,images,fortificationType,mapSize,
mapx,mapy,mapOffsetLeft,mapOffsetTop,fromTerritory,toTerritory,attackedFromTerritory,
attackedToTerritory,selectQuantity,selectedQuantity,actionString,doAction,chatHasFocus, userGuide,backScreen,menuDiv,clickableMapMenuDiv,
clickableMapMenuHeading,clickableMapMenu,minArmies,remainingArmies,remainingArmiesD, fromTerritory_armies, overTerritory, armiesArray,
userPrefs, defaultPrefs = [], autoAdvance = false, autoAttackX = false, deployDeferred = false, sendingRequest = false, actionUpdated = false, 
bob = false,debugging = true, beginTurn = false, sprites = {};
