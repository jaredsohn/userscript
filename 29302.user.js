...
// ==UserScript==
// @author      Payal / Mohd Fazial Muhamad
// @email	fazial@gmail.com/
// @namespace	http://userscripts.org/
// @name	Travian: Penjadual Tugasan 
// @description	Menjadualkn pembinaan, upgrade dan serangan. Juga digunakan untuk penghataran dagangan di pasar.
...
var sLang = "my";  // Set this to override the automatic language detection. Available translations: see below.
...
var sCurrentVersion = "1.1.2";  //Version number with which we need to run the update fu
...
case "tw":  //Shao-Pin
	...
	case "bg": //by penko
		...
		aLangStrings = ["????????? ??-?????", "?????????? ??-?????", "?????????? ??-?????", "???????? ??-?????", "???????? ???? ?????? ?? ??-?????.", "??????? ?????? ", " ??????? ? ?????? ????????.", "????", " ?? ???? ?? ???? ?????????.", " ?? ???? ?? ???? ??????????.", "???????? ? ?????????.", "?????? ?????????:", "?? ? ???????? ???? ?????? ?? ???? ????????? ????.", "??????????? ?????? ?? ? ????????!", "????????? ??????", "?????????", "??????? ??-?????", "??????? ?? ???? ?? ???? ?????????, ?????? ?? ?? ??????? ???????.", "?????? ??????? ?? ????????? ???", "?????? ??????? ?? ????? ?? ????? ????????? ???", "???????????? ???", "????? ???", "????? ???", "??????????? ?? ????? ?", "????????", "?", "??? ????", "???????", "??????", "????", "????", "?????????? ?? ??????? ? ??????", "?????????? ?? ?????? ? ??????", "??????", "??????? ?? ???? ?? ???? ????????? ??? ???? ?? ? ??????? ???.", "at site no.", "????????? ??:", "??? ", "????? ", "??? ", "????? ", "???? ", "??????? ?? ????????", "?????????? ?? ?????????","Queue this task"];
	...
	case "hk":  //by Angus
		aLangBuildings = ["", "???", "??", "???", "??", "???", "??", "?????", "???", "???", "??", "??", "??", "???", "???", "????", "???", "??", "???", "??", "??", "??", "???", "??", "???", "??", "??", "???", "???", "???", "???", "??", "??", "??", "???", "???", "???", "???", "???", "???", "????"];
		aLangTasks = ["??", "??", "??", "??", "??","Send Resource"];
		aLangStrings = ["????", "????", "????", "????", "??????????.", "????? ", " ????????.", "??", " ????.", " ????.", "??????????.", "????:", "????????????.", "????????!", "?????????", "??", "????", "????????????????.","???????", "????????", "??", "??", "??", "??????",	"??", "?", "???",	"?", "?", "?", "?", "???????", "???????","??", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ","Queue this task"];
		aLangTroops[0] = ["Legionnaire", "Praetorian", "Imperian", "Equites Legati", "Equites Imperatoris", "Equites Caesaris", "Battering Ram", "Fire Catapult", "Senator", "Settler", "Hero"];  //Romans
		aLangTroops[1] = ["Clubswinger", "Spearman", "Axeman", "Scout", "Paladin", "Teutonic Knight", "Ram", "Catapult", "Chief", "Settler", "Hero"];  //Teutons
		aLangTroops[2] = ["Phalanx", "Swordsman", "Pathfinder", "Theutates Thunder", "Druidrider", "Haeduan", "Ram", "Trebuchet", "Chieftain", "Settler", "Hero"];  //Gauls
		break;
...
break;
	...
	case "us":  //by m4rtini
		aLangBuildings = ["", "Woodcutter", "Clay Pit", "Iron Mine", "Wheat Field", "Sawmill", "Brickyard", "Iron Foundry", "Flour Mill", "Bakery", "Warehouse", "Granary", "Blacksmith", "Armoury", "Tournament Square", "Main Building", "Rally Point", "Marketplace", "Embassy", "Barracks", "Stable", "Siege Workshop", "Academy", "Cranny", "City Hall", "Residence", "Palace", "Treasury", "Trade Office", "Great Barracks", "Great Stable", "City Wall", "Earth Wall", "Palisade", "Stonemason", "Brewery", "Trapper", "Hero's Mansion", "Great Warehouse", "Great Granary", "Wonder"]; 
		aLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train","Send Resource"];
		aLangStrings = ["Build later", "UpgradeD later", "Attack later", "Research later", "Schedule this task for later.", "We started builing ", " was attempted with unknown result.", "level", " cannot be built.", " cannot be upgraded.", "The task was scheduled.", "Current production:", "We can't schedule this task right now.", "Task scheduling is not available!", "Scheduled Tasks", "Delete", "Send later", "No troops were selected.", "Your troops were sent to", "Your troops could not be sent to", "Support", "Attack", "Raid", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "away", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ","Queue this task"];
		aLangTroops[0] = ["Legionnaire", "Praetorian", "Imperian", "Equites Legati", "Equites Imperatoris", "Equites Caesaris", "Battering Ram", "Fire Catapult", "Senator", "Settler", "Hero"];  //Romans
		aLangTroops[1] = ["Clubswinger", "Spearman", "Axeman", "Scout", "Paladin", "Teutonic Knight", "Ram", "Catapult", "Chief", "Settler", "Hero"];  //Teutons
		aLangTroops[2] = ["Phalanx", "Swordsman", "Pathfinder", "Theutates Thunder", "Druidrider", "Haeduan", "Ram", "Trebuchet", "Chieftain", "Settler", "Hero"];  //Gauls
		break;
...
function createQueueLinks() {
	...
	if(xpathRes.snapshotLength > 0) {  //standing building
		...
		var re = new RegExp("(.*)\\s" + t("tahap") + "\\s[0-9]{1,3}$", "i");  // Will be used later for matching buildings and resource sites
...
function getEndOfTheLastScheduledTask(iTask,villagedid){	
	...
	for (var i =0;i<Tasks.length;i++){
		...
		if(iTaskType==iTask && (iCityId==villagedid)){
	...
	// Get the current village of the User 		
		...
		if(iX != '' && iY != '') {
			...
			if( villageTargetName==''){
				...
				printMsg(t("Nak hantar res ko ke mana da... Bagi tau destinasi k."), true);
				return false;
			}
		}					
...
function scheduleAttack(e) {
	...
	if(iVillageId == null) {
		...
		printMsg(t("Kau nak serang mana nih ha? Bagi la destinasi kalo yer pn."), true);
		return false;
	}
...
function getBuildingId() { 
	...
	if(iSiteId != null) {
		...
		re = /.*build\.php\?([a-z=0-9&]*&)?gid=([0-9]{1,2})/i;
		iSiteId = window.location.href.match(re);
		if(iSiteId != null) {
			return parseInt(iSiteId[2]);
		} else {
...
function performUpgrade() {
	...
	alert("Skrip Penjadualan Tugasan untuk Travian anda telah dikemaskinikan. \nHarap Maklum. Terima Kasih.");
		_log(3, "<- performUpgrade()");
}
...
function makeMenuToggle(key, defaultValue, toggleOn, toggleOff, prefix) {
  ...
  GM_registerMenuCommand((prefix ? prefix+": " : "") + (window[key] ? toggleOff : toggleOn), function() {
	...
	if(key == 'USE_SERVER_TIME' && !window[key]) {
		...
		var promptMsg = (iServerTimeOffset == false) ? "Penjadual Tugasan:\nUntuk menggunakan masa server, sila masukkan timezone. \nBiasa Malaysia kn +8. Masukkan la 8. time kacih...." : "Penjadualan Tugasan:\nTimezone server anda sekarang adalah " +iServerTimeOffset+ "jam dari GMT.\Kalau tak betul tu betulkan la k. Kalo x biarkan ia kosong.";	
...
function useLocalTime() {
	...
	alert("Sekarang anda menggunaan masa local untuk menetapkan jadual tugasan.");
	location.reload();	
}
...
function useServerTime() { 
	...
	if(iServerTimeOffset == false) { 
		...
		iServerTimeOffset = prompt("Untuk menggunakan masa server, sila masukkan timezone. \nBiasa Malaysia kn +8. Masukkan la 8. time kacih...");
...
function promptRace() {
	...
	while(!isInt(newRace)) {
		...
		var newRace = prompt("Penjadual Tugasan: \nKau puak apa kat server ni?\n(0 utk Roman, 1 utk Teuton, 2 utk Gaul.) \nKaum kau sekarang: " +iMyRace);
		if(isInt(newRace)) {
...
function promptHistory() {
	...
	while(!isInt(newHistoryLength)) {
		...
		var newHistoryLength = prompt("Penjadual Tugasan: \nBerapa byk history kau nk simpan?\n(Masukkn 0 kalo taknak simpan.) \nHistory disimpan sekarang: " +iHistoryLength);
...
function promptDelay() {
	...
	while(!isInt(newDelay)) {
		...
		var newDelay = prompt("Penjadual Tugasan: \nTambahan minit jika gagal?\n(Masukkan 0 untuk batal) \nSekarang ni... " +iDelay);
		if(isInt(newDelay)) {
...
function onLoad() {
	...
	makeMenuToggle("USE_SERVER_TIME", false, "Guna masa server", "Guna masa local", "Queue Penjadual Tugasan");
	GM_registerMenuCommand("Queue Penjadual Tugasan: Set Puak", promptRace);
	GM_registerMenuCommand("Queue Penjadual Tugasan: History Tugasan", promptHistory);
	//by ALFik
	GM_registerMenuCommand("Queue Penjadual Tugasan: Delay Kalau Error", promptDelay);
