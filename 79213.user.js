...
// pagetitle.innerHTML += '<span>&nbsp;<input type="button" class="okButton" id="monstroButton" value="Mise ÃƒÆ’  jour Syndicat"/></span>';
pagetitle.innerHTML += '<a type="button" class="okButton" id="monstroButton">Contrat V8</a>';
	
var monstroButton = document.getElementById('monstroButton');
if (!monstroButton) return;
	//Ajout de l'ÃƒÆ’Ã‚Â©vÃƒÆ’Ã‚Â¨nement
...
function traitement() 
{
	...
	//RecupÃƒÆ’Ã‚Â©ration des infos sur les statistiques
	//--------------------------------------------
		...
		headers: {
			...
			onload: function(responseDetails) 
			{
				...
				if (temp2!= null)
				{
					...
					pos4 = temp2.lastIndexOf("<td>1</td>");
					if (pos4>0)
					{
						pos6 = temp2.indexOf("\">",pos4);
						pos7 = temp2.indexOf("</a>",pos6);
						
						pos5 = temp2.indexOf("\">",pos7);
						pos4 = temp2.indexOf("pts",pos5);
						if (pos4>0)
						{
							prems[num] = temp2.substring(pos6+2,pos7);
						}
						else
						{
							prems[num] = '?' ;
						...
						point_prems[num] = temp2.substring(pos5+2,pos4);
					}
					else
					{
						prems[num] = '?' ;
					...
					{																																																					
						...
						for(var k=0;k<=ee.length;k++)
						{
							if (ee[k].className == "contractMonster")
							{
								...
								dif_prems = point_bob[jj]-point_prems[villeid[jj]];
								
								ee[k].setAttribute('onMouseOver',"javascript:Tip.show(null,'Point bob : "+point_bob[jj]+"  Point tene : "+ point_tene[villeid[jj]] + "      différence avec tenebrae : " + dif +"        premier : " + prems[villeid[jj]] +" (" + point_prems[villeid[jj]] + "pts)             différence avec le 1er : " + dif_prems + "' ,null,event);");
