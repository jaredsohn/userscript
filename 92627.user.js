// ==UserScript==
// @name             KOCAttack - Avec Options Supplémentaires!
// @version          0.5.3 FR
// @namespace        KOCAttack - Avec Options Supplémentaires!
// @homepage         http://userscripts.org/scripts/show/92627
// @description      Le même script que "KOCAttack - Extra Features!" de Thomas Chapin mais traduit en français

// @include          http://*.kingdomsofcamelot.com/*main_src.php*
// @include          http://www.facebook.com/connect/uiserver.php*

// @require          http://sizzlemctwizzle.com/updater.php?id=92627
// ==/UserScript==
...
function inspect(obj, maxLevels, level){
  var str = '', type, msg;
    ...
    if(maxLevels < 1)     
        return '<font color="red">Erreur: le niveaux doit être > à 0</font>';

    // We start with a non null object
    if(obj == null)
    return '<font color="red">Erreur: Objet <b>INVALIDE</b></font>';
    ...
    for(property in obj)
    {
      try
      {
          ...
          str += '<li>(' + type + ') ' + property + 
                 ( (obj[property]==null)?(': <b>invalide</b>'):(': '+obj[property])) + '</li>';
      ...
      catch(err)
      {
        ...
        str += '<li><font color="red">(Erreur) ' + property + ': ' + msg +'</font></li>';
      }
    }
...
var KOCAttack={
	...
	DoUnsafeWindow:function(func, execute_by_embed) {
		if(this.isChrome || execute_by_embed) {
			...
			try {  
				...
				this.Log("Une erreur s\'est produite lors de l\'exécution de la fonction javascript DoUnsafeWindow. Description de l\'erreur: "+error.description);
			}
		}
	},
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Order d\'attaques: <select id='KOCAttackOrder'><option value='mostTroops'>Utilisez le plus de troupes e premier</option><option value='closest'>Cibles les plus proches en premier</option></select><br />"+
			"Type d\'attaques: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Camp "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Terre Sauvage "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Ville "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transport<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> nombres de secondes entre chaque attaque<br />"+
			"Temps entre l\'envoi de troupes sur la <u>même cible</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Attaquer les camps toutes les: <input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />heures<br />"+
			"Attaquer les terres sauvage toutes les: <input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />heures<br />"+
			"Attaquer les villes toutes les: <input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />heures<br />"+
			" Transporting: <input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />minutes"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> Distance maximum de la ville la plus loin à attaquer.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Lancer les attaques seulement de la ville d\'ou elles ont été lancé la première fois.<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Réessayer si une erreur inconnu ou un trafic exesif survient (Rafréchir après avoir changé cette option).<br />"+
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Auto supprimer les rapports d\'attaques sur camps barbares / terre sauvage.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Nombres maximum de rapports d\'attaque à conserver dans la boite de messages.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Désactiver l'affichage des rapports, serons aussi désactiver la collecte des rapports d'attaque.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% De la population inactive disponibles avant d\'inissier la formation automatique.<br />"+
			...
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Automatiquement aider les membres de l\'alliance avec la construction/la recherche.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Masquer les demandes d\'aide alliance/rapports dans le chat (si-dessus est cochée, le faire après les avoir aidé).<br />"+
			"<input id='KOCAttackAutoPublishHelpRequests' type='checkbox' "+(this.options.autoPublishHelpRequests?'checked':'')+" /> Automatiquement accélérer la construction/la recherche en publiant les demandes d\'aide sur mon mur.<br />"+
			"Si ci-dessus est coché, ce paramètre de confidentialité doit-on utiliser? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tout le monde</option><option value='50'>Amis d\'amis</option><option value='40'>Amis seulement</option><option value='10'>Moi seulement</option></select><br />"+
			"</td></tr>"+
			...
			"Passer à traver les villes <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> fois puis attendre "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /> nombres de secondes avant de rafréchir.<br />"+
			"Cities to attack from: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' /> % d\'ajustement aléatoire des retards (pour avoir l\'aire plus naturel).<br />"+
			...
			"<a id='KOCAttackOptionsSave' class='button20'><span>Enregistrer</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Supprimer</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Réinitialiser les options</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Réinitialiser TOUTES les options!</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Supprimer les attaques sauvegardées</span></a></form>";
		var t=this;
	...
	*/			
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>Auto Attaque - Oui</span>';
		} else {
			toggle.innerHTML='<span>Auto Attaque - Non</span>';
		}
	},
	...
	/*	
		var t=this;attaque précédente matches attaque en cours. Ce n'est pas la première attaque
	...
	*/	
	...
	IsFirstAttackAtLocation:function(x,y) {
		if(this.prevAttack.x && this.prevAttack.y){
			if(this.prevAttack.x==x && this.prevAttack.y==y) {
				this.Log("l\'attaque précédente est la même de l\'attaque en cours. Ceci n'est pas la première attaque!");
				return false;
			}
		}
		var attack=this.GetAttack(x,y);
		if(!attack){
			this.Log("l\'attaque précédente n\'est pas définis. Ceci est la première attaque!");
			return true;
		}
		if(attack.a) { attack = attack.a; }
		if(!attack.suicidewavetime || !attack.time){
			this.Log("Temps de la vague suicide: "+attack.suicidewavetime+". temps d\'attaque: "+attack.time+". Ceci pas la première attaque!");
			return true;
		}
		...
		if( attack.suicidewavetime <= (nowSecs-(this.options.attackDelay+attackDelay)) && attack.time > attack.suicidewavetime && attack.time < nowSecs-attackDelay){
			this.Log("Le temps d\'attaque de la vague suicide a expiré. Ceci la première attaque!");
			return true;
		}
		return false;
	...
	GetGuiCoords:function() {
		...
		if(!x || !y) {	
			this.Log("Coordonées du GUI introuvable");
			return null;
		}
	...
	IsCurrentlySendingMultipleWaves:function() {
		...
		if(timeDifference > waveTimerDelay && this.SendingMultipleWaves==true){
			...
			this.Log("Le temps d\'attaque multiples \("+waveTimerDelay+"\ secondes) à expiré. La dernière attaque connue a été envoyée il y a "+timeDifference+" secondes. Minuterie réinitialisée et redémarrée...");
		}
	...
	SetAttackFromGuiXY:function(x,y,box,isSuicideWave) {
		...
		if(!type) {
			throw("Type d\'attaque introuvable");
		}

		if(totalTroops<=0) {
			this.Log("Pas de troupes, attaque non sauvegarder");
			return null;
		}
		...
		if(type.selectedIndex==0) {
			...
			this.Log("Première attaque?: "+firstAttack);
			
			this.Log("Attaque scuisside definite? : "+SuicideAttackDefined);
			...
			if(totalTroops>0 && (((troops[1]+troops[2]+troops[10])==totalTroops && firstAttack) || isSuicideWave)) {
				if(totalTroops>0 && ((troops[1]+troops[2]+troops[10])==totalTroops && firstAttack)){
					this.Log("Attaque suicide déterminé par le type de troupes et par la première attaque");
				}else if(isSuicideWave){
					this.Log("Attaque suicide déterminé par le checkbox");
				}
				// nothing but supply troops and/or militiamen and/or ballista. This must be an anti-defense suicide wave attack
				this.Log("Vague Suicide :"+troops);
		...
		for(var r=0; r<coordRows.length; r++) {
			...
			if(attack && !force) {
				if(SuicideAttackDefined && isSuicideWave==true){
					this.Log('Vague suicide sur '+x+','+y+' a déja été ajoutée. Essayez de forcer la ré-écriture.');
					continue;
				}else if(attack.troops){
					this.Log('Attaque sur '+x+','+y+' a déja été ajoutée. Essayez de forcer la ré-écriture.');
					continue;
				}
			}
			...
			if(attack) {
				...
				if(SuicideAttackDefined || isSuicideWave || previous_suicidewave){
					...
					if(previous_suicidewave && (!SuicideAttackDefined || !isSuicideWave)){
						this.Log("Une vague de suicide antérieures a été écrasé par cette nouvelle attaque. Fusioner et restaurer...");
				...
				this.SetAttack(x,y,attack);
				this.Log(x+','+y+' attaque ajouté: '+inspect(attack));
				this.Log("Type de la vague d\'attaqe actuelle: "+attack.currenttattackwavetype);
				added++;
			}
		}
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"Copiez, collez les coordonées ici (example 343,434) une sur chaque ligne...<br />Note: il ne fera qu\'ajouter des cible en utilisant le nombre actuel de troupes inscrite dans les cadrant.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Ecraser les attaques déjà existantes<br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Type d'emplacements (Toutes les coordonnées doivent correspondre à ce type):<br />");
		...
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Il s\'agit d'une vague de suicide initiale pour éliminer les pièges d\'une terre sauvage.<br />");
		AddHtml(bulkAddDiv,"<hr />");
		...
		bulkAdd.innerHTML='<span>Ajout en masse</span>';
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert(added +' Terre sauvage on été ajoutés en masse');
		},false);
		AddHtml(bulkAddDiv,"<br />");

		var bulkAddAttackLink=document.createElement('a');
		bulkAddAttackLink.className='buttonDown20';
		bulkAddAttackLink.innerHTML='<span>Ajout en masse de coordonées</span>';
	...
	HideAttackEfforts:function() {
		...
		var a=document.createElement('a');
		a.innerHTML='Masquer / Afficher les efforts attaque';
		//a.href='javascript:;';
	...
	SetResourceInput:function(num,resourceCount) {
		...
		if(parseInt(resourceCount)>0) {
			if(resource.value!=resourceCount) {
				this.Log('Pas en mesure d\'assigner le compte de ressources: '+num+', nésséssaire: '+resourceCount+', comte: '+resource.value);
	...
	SetTroopInput:function(num,troopCount) {
		...
		if(parseInt(troopCount)>0) {
			if(troop.value!=troopCount) {
				this.Log('Pas en mesure d\'assigner le nombre de troupes: '+num+', nésséssaire: '+troopCount+', comte: '+troop.value);
		...
		if(AllianceChatBox){
			...
			if(chatPosts){
				...
				for (var i = 0; i < chatPosts.snapshotLength; i++) {
					...
					if(this.options.hideAllianceHelpRequests){
						// Look for any alliance assist links in this current post item
						var myregexp1 = /Vous êtes le [1-5] sur 5 à aider/i;
						var myregexp2 = /\ n a pas nesoin d aide\./i;
						var myregexp3 = /\ a compléter sont projet\./i;
						var myregexp4 = /\ a déjà reçu l aide maximum\./i;
