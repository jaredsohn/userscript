...
// @description    Minor refresh to the look and feel of MotoIQ

// @include        http://motoiq.com/*
// @include	   http://www.motoiq.com/*
// @include        http://motoriq.com/*
// @include	   http://www.motoriq.com/*
...
function cleanPage() {
	var tags = getElementsByClassName(document, 'div', 'containermain')
	for (var key in tags) with (tags[key]) {
		var elms = tags[key].getElementsByTagName("TD");
		var myPath = location.pathname;
		if (location.pathname.indexOf('MagazineArticles') > 0) {
			for(var i = 0, maxI = elms.length; i < maxI; ++i) {
				var elm = elms[i];
				elm.style.border = 0;
			}
		}
		else if ((location.pathname.indexOf('Columns/tabid') > 0) | (location.pathname.indexOf('Projects/tabid') > 0)) {
			for(var i = 0, maxI = elms.length; i < maxI; i++) {
				if (i % 2){
					var elm = elms[i];
					elm.style.border = 0;
	...
	tags = getElementsByClassName(document, 'div', 'articlePaging')
	for (var key in tags) with (tags[key]) {
		style.display = 'none';
	}
}
...
var SITEINFO = [
    ...
    {
        url:          'http://(.*).motoiq.+/(MagazineArticles).+',
        nextLink:     '//div[@class="articlePaging Normal"]/a[last()]',
        pageElement:  '//div[@class="articleEntry Normal"]',
        exampleUrl:   'http://www.motoiq.com/MagazineArticles/tabid/59/articleType/ArticleView/articleId/12/PageID/42/The-Great-Wideband-Shootout-Part-1-How-do-wideband-air-fuel-sensors-work.aspx',
    },
