...
(function () {
  ...
  function queryBilibiliSP(titles, callback) {
    ...
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'http://api.bilibili.tv/sp?type=json&appkey=' + APPKEY +
  ...
  function parseBilibiliBgmPage(content) {
    ...
    if (!resultUrl || !resultTitle)
      return null;
    return {
      url: 'http://www.bilibili.tv' + resultUrl,
      title: resultTitle
    };
  }
  function getBilibiliLink(spid, seasonId, lastupdate, callback) {
    ...
    var url = 'http://www.bilibili.tv/sppage/' +
  ...
  if (location.pathname == '/') {
    ...
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'http://api.bilibili.tv/bangumi?type=json&appkey=' + APPKEY +
          '&_=' + Date.now(),
      onload: function (resp) {
