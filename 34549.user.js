...
/* Update */
var version_timestamp = 1219836393000;
/***
...
Card.prototype.steal = function(dropcard, droppable) {
...
function collection_updateData(manual) {
  try {
    ...
    alert("Collection database updated.");
  } catch(ex) {
...
GM_registerMenuCommand("Update collections database",collection_updateData);
...
function collection_shoppingList() {
  ...
  for (var i in col_missing) {
    if (col_missing[i] > 1) {
      msg.push(col_missing[i] + '&nbsp;' + collection_cardName(i).replace(/ /g,'&nbsp;') + 's');
    } else {
...
function collection_preferencesInterface(prefs, profile) {
  ...
  option.push('Personalized shopping list</input><br/>');
...
function collection_preferencesHandler(form, profile) {
  ...
  var types = ["all", "automatic", "personal"];
  var value = "automatic";
...
function stalk_exec() {
  ...
  if (profile.preferences.stalk_delay == undefined) profile.preferences.stalk_delay = 120;
  ...
  if (Page.c_page != 'market' && Page.c_page != 'purchase' && Page.c_page != 'markets') {
  ...
  else action = new Timer(Math.floor(1000 * profile.preferences.stalk_delay / num), Math.floor(1000 * profile.preferences.stalk_delay / num));
  action.type = 'gotourl';
...
function stalk_getMarkets() {
  ...
  var result = new Array();
  for (var item in obj) result.push(item);
  ...
  switch (Page.c_page) {
    case 'user':
      ...
      if (profile.preferences.steal_steal) {
        ...
        for (var i = 0; i < pagecards.cards.length; i++) {
          var card = pagecards.cards[i];
          if (collection_isInShoppingList(card.collection, card.name) && card.steal_id) {
            best = card;
            svalue = card.points;
            break;
          }
