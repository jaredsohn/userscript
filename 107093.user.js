// ==UserScript==
// @name          Lplus Pimper by Tim Kretschmann
// @namespace     http://www.pimpmylplus.nothing
// @description   The Lplus Pimper
// @include       http://lplus.onlinetrainingport.com/TrainingModeFrame.aspx
// @include       http://lplus.onlinetrainingport.com//TrainingModeFrame.aspx
// @version       10.4
// @screenshot    http://www.3wishes.com/images/sexymilehighcaptain-d.jpg
// @id            pimpmylplus@ptntools
// @author        Tim Kretschmann <tim.kretschmann@gmail.com>
// @require       http://code.jquery.com/jquery-1.6.min.js
// @require       http://pastebin.com/raw.php?i=ihV5JM52
// @require       http://yelotofu.com/labs/jquery/snippets/shuffle/jquery.shuffle.js
// ==/UserScript==
...
// 10.2 -> 10.3
//  1 found the make-it-slow bug. yaay. 
...
function updateStatBox() {
    ...
    if (perc == 100) {
        color = "#FFD700";
        if (ans_quest > 10) {
            comment = '<a target="new" style="color:#FFD700; font-weight: bold;" href="http://www.youtube.com/watch?v=9-w9Z3NsL1A">YEAH! Weiter so!!</a>';
        } else if (ans_quest > 7) {
            color = "#ccac00";
            comment = 'Yooou <span style="color:#ccac00;">get it </span>baby!!';
        }     else if (ans_quest > 4) {
            color = "#b29600";
            comment = 'You\'re <a target="new" style="color: #b29600;" href="http://www.youtube.com/watch?v=a1Y73sPHKxw">on the roll</a> ...';
        }    else if (ans_quest > 1) {
            color = "#806c00";
        ...
        comment = '<a target="new" href="http://www.youtube.com/watch?v=UTDZzCyxstQ">First class.</a>';
        color = "#32CD32";
    } else if (perc >= 80) {
        comment = '<b>you\'re <a target="new" style="color:#32CD32;" href="http://www.youtube.com/watch?v=919IA_Lj0Ko">the one</a></span>!</b>';
        color = "#32CD32";
    } else if (perc >= 75) {
        color = "#FFB1A3"; 
        comment = 'Bisschen knapp...';
    } else if (perc > 50) {
        color = "#FF6347";
        comment = '<b><a href="http://www.youtube.com/watch?v=N-NhypCqnME&feature=related" target="new" style="color:#FF6347;">you suck</a>!!</b>';
    } else if ( ans_quest > 3){
        color = "red";
        comment = '<span style="color:red">Kacknoob!.</span>';
    } else if ( ans_quest == 3){
        color = "red";
...
function display_message(message) {
    ...
    if ($('#message_table', wf2d).size()) {
        $('#message_table', wf2d).remove();
    }
    $('body', wf2d).children().each( function() {
        $(this).addClass('hide-row'); 
    });
    var newhtml = '\<table id="message_table"><tr><td>'
        + message
        + '</td></tr></table>';
    $('body > :first-child', wf2d).before(newhtml);
}

function undisplay_message() {
    var wf2d = window.frames[2].document;
    if ($('#message_table', wf2d).size()) {
        $('#message_table', wf2d).remove();
    }
...
function randomize_qs() {
    var wf1d = window.frames[1].document;
    // jquery 1.6 breaks something within the plugin. 
    // using try..catch makes it work again.
    // sucky patch, but I have no idea what is going ballistic here.
    try {
        $('table .textnormal > tbody', wf1d).shuffle();
    } catch (e) {
        // the error message is "a.eval is undefined"
        // shuffling a 2nd time works just fine.
    }
    // go to active question again :) 
    var n = initNeighbors();
    n = filterNeighbors(n);
    if (! n[1].length) { 
        if (n[2].length) { 
            // remember: clickOn needs a DOM object!!
            clickOn(n[2][0][0], wf1d);
            goTo(n[2][0], window.frames[1]);
        } else {
            clickOn(n[0][0][0], wf1d);
            goTo(n[0][0], window.frames[1]);
        }
    } else {
...
function dfil_qjump_later(q, click) {
    ...
    filter_wait = setInterval(doit, 350);
}
...
function info_screen() {
    ...
    ifr.find('object').attr('data', 
        'http://lplus.onlinetrainingport.com/ShowQuestionInfo.aspx');
...
function help_screen() {
    ...
    <table width="75%" id="help_table" class="pmc_screen" \
        ...
        <tr>\
            <td>x</td>\
            <td>randomize questions. could take some time (0.5sec), sorry ... </td>\
        </tr>\
        <tr>\
            <td>w / W</td>\
...
function pimpAjax(qbox) {
    ...
    var my_url = my_url.substr(0, my_url.indexOf('://'))
        + '://lplus.onlinetrainingport.com/ShowQuestionInfo.aspx';
...
function pimpKeyboardShortcuts() {
    ...
    $(wf2d).bind('keydown', 'x', function() { return k() && randomize_qs(); });
    // Go to wiki :)
    var shw = "shift+w";
