...
// @description    Orders for Switzerland (eRepublik)
// @version        0.2
...
GM_xmlhttpRequest({
	method: 'GET',
	url: 'http://erepublik.satonio.net/defensa/orden.php?version=0.2&lang=en',
	onload:function(response){
		...
		$('#latestnews').css('margin-top', '30px').before('<div id="earmy"></div>');
		var earmy = $('#earmy');
		earmy.append('<h1 style="padding-bottom: 5px;"><embed class=\"sIFR-flash\" height=\"28\" width=\"250\" src=\"/flash/delicious.swf\" quality=\"best\" flashvars=\"txt=eSwitzerland Army&&textcolor=#737373&hovercolor=null&linkcolor=null&w=250&h=28\" wmode=\"opaque\" bgcolor=\"null\" sifr=\"true\" type=\"application/x-shockwave-flash\" style=\"width: 250px; height: 28px;\"/><span class=\"sIFR-alternate\">eSwitzerland Army</span></h1>');

		earmy.append('<ul  style="float: left;" class="tabs">'
				+'<li id="ordenes"><a class="on" href="#"><span style="width: 100px;">ORDERS</span></a></li>'
				+'<li id="foro"><a target="_BLANK" href="http://forum.ehelvetia.ch/"><span style="width: 100px;">Forum</span></a></li>'
				+'<li id="irc" class="last"><a target="_BLANK" href="http://tinyurl.com/ycs39zd"><span style="width: 100px;">IRC</span></a></li>'
				+'</ul>');
earmy.append('<div style="width: 100%; float: left; " id="ordenesnuevas"></div>'
				+'<a id="toggleLeidas" style="float: left; width: 100%; display: block; text-align: right; margin: 10px 0 0 0;" >See read orders</a>'
		...
		$('#toggleLeidas').bind('click', function() {
			if (ordenesleidas.css('display') == 'none') 
				$(this).html('Hidden read orders');
			else 
				$(this).html('See read orders');
		...
		if (ordenesnuevas.children().size() == 0) {
			ordenesnuevas.html('There is no new orders');
		} 
		...
		$('a.confirm').click(function() {
			...
			if ($(this).hasClass('NoLeido')) { //la sube
				...
				$(this).html('Put as read').removeClass('NoLeido').addClass('Leido');
				$(this).parent().prev().css('background','transparent url('+imagenes['block']+') no-repeat top left');
				if (ordenesnuevas.html() == 'There is no new orders') 
					ordenesnuevas.empty();
				$(this).parent().parent().appendTo('#ordenesnuevas');
				//$(this).parent().parent().remove();
				if (ordenesleidas.children().size() == 0) {
					ordenesleidas.html('There is no read orders').hide();
					$('#toggleLeidas').html('See previous orders');
				} 
				...
				$(this).html('Put as no read').removeClass('Read').addClass('NotRead');
				$(this).parent().prev().css('background','transparent url('+imagenes['none']+') no-repeat top left');
				if (ordenesleidas.html() == 'There is no read orders.') 
					ordenesleidas.empty();
