...
2011-06-22 - Tweaked it so the userscript doesn't constantly check for new posts even when it'd be impossible to have new posts. Added a check to see if an 'x' already exists.
2011-06-18 - First release. Someone break this sucker.
2011-06-15 - Created. Or in the words of Barney Stinson, "Challenge -- accepted."
...
function delMute(id, post)
{
	...
	$('tunmute-' + post).fade();
	$('tmuted-' + post).toggle();*/
	$('tunmute-' + post).innerHTML = 'Post unmuted. <a style="cursor: pointer;" onclick="window.location.reload(false);">Refresh</a> to see post.';
}
...
function tmute(post, id, newID)
{
	...
	for (var i = 0; i < hidePost.length; i++)
	{
		...
		mutePost.innerHTML = '<span id="tmuted-' + post + '" style="display:none">' + mutePost.innerHTML + '</span><span id="tunmute-' + post + '"> Post Muted. <a onclick="delMute('+ id + ', ' + post + ')" style="cursor: pointer;">Unmute</a></span>';
		//mutePost.fade();
		if (newID)
			addMute(id);
	}
...
function getPostsMute(startNum, mutePosts)
{
	...
	for (var i = 0; i < postControls.length; i++)
	{
		for (var j = 0; j < postControls[i].length; j++)
		{
			if (postControls[i][j].getAttribute('id') != null)
			{
				...
				{
					checkPostMute(postControls[i][j].getAttribute('id'), mutePosts);
...
function checkPostMute(postID, mutePosts)
{
...
function mainMute()
{
	...
	startNum = getPostsMute(startNum, mutePosts);
	...
	Ajax.Responders.register({
		...
		checkPage = setInterval(function() {
			...
			{
				getPostsMute(pageLength, mutePosts);
...
embedElement("script", checkPostMute, false);
...
embedElement("script", getPostsMute, false);
embedElement("script", addX, false);
embedElement("script", mainMute, true);