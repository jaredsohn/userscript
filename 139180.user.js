...
var names = new Array();
names["Tashunka Witko"] = 1;
names["Андрей Улитин"] = 2;
names["NickKA"] = 3;
names["Dragan Aquila"] = 4;

var playerName = document.getElementById('character_name').textContent;
var Notification = 
{
	...
	show : function showNotifications (data)
	{
		if (data.type == "TypeUser" && data.from.name != playerName)
		{
			...
			message.replace(message.match(/\/\d{3}/),"");
...
{
	if (navigator.vendor == "Google Inc." && localStorage["ns_notificationEnable"] && localStorage["ns_notificationEnable"]=="true")
	{
		chatcontrol.addListener(function(e){if (navigator.vendor == "Google Inc." && localStorage["ns_notificationEnable"] && localStorage["ns_notificationEnable"]=="true") Notification.show(e);});
	}
	...
	{
		document.getElementById('chatwindow_say').value += top+document.getElementsByClassName('fort_battle_poster')[0].childNodes[3].childNodes[0].childNodes[1].innerHTML+"* ";
		document.getElementById('chatwindow_say').focus();
	};
