...
(function(){
  ...
  var yt;
  var ytcenter = {};
  ytcenter.version = "1.30.12";
  ytcenter.revision = 83;
  ytcenter.icon = {};
    ...
    if (ytcenter.html5) {
      ...
      if (a && a.canPlayType) {
        ...
        for (var i = 0; i < ytcenter.video.stream.length; i++) {
          ...
          if ($ArrayIndexOf(priority, ytcenter.video.stream[i].quality) <= $ArrayIndexOf(priority, vq) && $ArrayIndexOf(priority, ytcenter.video.stream[i].quality) > $ArrayIndexOf(priority, yt.playerConfig.args.vq) && a.canPlayType(ytcenter.video.stream[i].type.split(";")[0])) {
            yt.playerConfig.args.vq = ytcenter.video.stream[i].quality;
          }
        }
      }
    } else {
      for (var i = 0; i < ytcenter.video.stream.length; i++) {
        if ($ArrayIndexOf(priority, ytcenter.video.stream[i].quality) <= $ArrayIndexOf(priority, vq) && $ArrayIndexOf(priority, ytcenter.video.stream[i].quality) > $ArrayIndexOf(priority, yt.playerConfig.args.vq)) {
          yt.playerConfig.args.vq = ytcenter.video.stream[i].quality;
        }
      }
    }
    console.log("YTCenter: Setting Video Quality to " + yt.playerConfig.args.vq);
    if (typeof ytcenter.player.reference != "undefined" && ytcenter.player.reference.onReadyCalled) {
      console.log("YTCenter: Setting PlaybackQuality to " + yt.playerConfig.args.vq);
      ytcenter.player.reference.api.setPlaybackQuality(yt.playerConfig.args.vq);
    }
  };
  ytcenter.player.update = function(onplayerloaded){
    console.log("YTCenter: Update Called");
    if (!ytcenter.player.jsapicallback) {
      ytcenter.player.jsapicallback = yt.playerConfig.args.jsapicallback; // Save the default jsapicallback for later use
    }
    ...
    yt.playerConfig.args.jsapicallback = "ytcenter.onplayerloaded";
  ...
  ytcenter.parseStream = function(){
    ...
    var fmt = parser1(yt.playerConfig.args.fmt_list);
    var stream = parser2(yt.playerConfig.args.url_encoded_fmt_stream_map);
    var a = [];
  ...
  var ytwatchinit = function(){
    ...
    ytcenter.video.stream = ytcenter.parseStream();
    
    ytcenter.video.id = yt.playerConfig.args.video_id;
    ytcenter.video.title = (function(){
    ...
    ytcenter.player.update(function(playerid, player, listener){
      ...
      if (ytcenter.watch7 && !ytcenter.html5) {
        player.target.children[0].stylezIndex = "102";
      } else {
