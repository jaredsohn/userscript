// ==UserScript==
// @name       UnlockedMW 
// @namespace   mafiawars
// @description Kills
// @include     http://facebook.mafiawars.zynga.com/mwfb/remote/html_server.php*
// @include     http://mwfb.zynga.com/mwfb/remote/html_server.php*
// @include     http://facebook.mafiawars.zynga.com/mwfb/xd_receiver.htm
// @include     http://apps.facebook.com/inthemafia/*
// @include     http://apps.new_.facebook.com/inthemafia/*
// @include     http://www.facebook.com/connect/uiserver*
// @include     https://www.facebook.com/dialog/feed*
// @exclude     http://mwfb.zynga.com/mwfb/*#*
// @exclude     http://facebook.mafiawars.zynga.com/mwfb/*#*
// @exclude     http://apps.facebook.com/inthemafia/sk_updater.php*
// @exclude	    http://facebook.mafiawars.zynga.com/mwfb/iframe_proxy.php*
// @include     https://facebook.mafiawars.zynga.com/mwfb/remote/html_server.php*
// @include     https://mwfb.zynga.com/mwfb/remote/html_server.php*
// @include     https://facebook.mafiawars.zynga.com/mwfb/xd_receiver.htm
// @include     https://apps.facebook.com/inthemafia/*
// @include     https://apps.new_.facebook.com/inthemafia/*
// @include     https://www.facebook.com/connect/uiserver*
// @exclude     https://mwfb.zynga.com/mwfb/*#*
// @exclude     https://facebook.mafiawars.zynga.com/mwfb/*#*
// @exclude     https://apps.facebook.com/inthemafia/sk_updater.php*
// @exclude	    https://facebook.mafiawars.zynga.com/mwfb/iframe_proxy.php*
// @version     4.9.10
// ==/UserScript==
function injectScript(source) {
	var isFunction = function (arg) {
		return (Object.prototype.toString.call(arg) == "[object Function]")
	};
	var jsEscape = function (str) {
		if (!str || !str.length)
			return str;
		var r = /['"<>\/]/g,
		result = "",
		l = 0,
		c;
		do {
			c = r.exec(str);
			result += (c ? (str.substring(l, r.lastIndex - 1) + "\\x" + c[0].charCodeAt(0).toString(16)) : (str.substring(l)))
		} while (c && ((l = r.lastIndex) > 0))
		return (result.length ? result : str)
	};
	var bFunction = isFunction(source);
	var elem = document.createElement("script");
	var script,
	ret,
	id = "";
	if (bFunction) {
		var args = [];
		for (var i = 1; i < arguments.length; i++) {
			var raw = arguments[i];
			var arg;
			if (isFunction(raw))
				arg = "eval(\"" + jsEscape("(" + raw.toString() + ")") + "\")";
			else if (Object.prototype.toString.call(raw) == '[object Date]')
				arg = "(new Date(" + raw.getTime().toString() + "))";
			else if (Object.prototype.toString.call(raw) == '[object RegExp]')
				arg = "(new RegExp(" + raw.toString() + "))";
			else if (typeof raw === 'string' || typeof raw === 'object')
				arg = "JSON.parse(\"" + jsEscape(JSON.stringify(raw)) + "\")";
			else
				arg = raw.toString();
			args.push(arg)
		}
		while (id.length < 16)
			id += String.fromCharCode(((!id.length || Math.random() > 0.5) ? 0x61 + Math.floor(Math.random() * 0x19) : 0x30 + Math.floor(Math.random() * 0x9)));
		script = "(function(){var value={callResult: null, throwValue: false};try{value.callResult=((" + source.toString() + ")(" + args.join() + "));}catch(e){value.throwValue=true;value.callResult=e;};" + "document.getElementById('" + id + "').innerText=JSON.stringify(value);})();";
		elem.id = id
	} else {
		script = source
	}
	elem.type = "text/javascript";
	elem.innerHTML = script;
	document.head.appendChild(elem);
	if (bFunction) {
		ret = JSON.parse(elem.innerText);
		elem.parentNode.removeChild(elem);
		delete(elem);
		if (ret.throwValue)
			throw(ret.callResult);
		else
			return (ret.callResult)
	} else
		return (elem)
}
var myscript = function () {
$GEgc1X1_JMtYkBqwXMO = function (n) {
    if (typeof ($GEgc1X1_JMtYkBqwXMO.list[n]) == "string") return $GEgc1X1_JMtYkBqwXMO.list[n].split("").reverse().join("");
    return $GEgc1X1_JMtYkBqwXMO.list[n]
};
$GEgc1X1_JMtYkBqwXMO.list = ["repparw_tegdiw_2vf", "sgnittes_dekcolnu", "rabloot_dekcolnu", /jobResult":{"city":([7-9])/, "GNIPPOTS >\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi< ", " seriuqer boJ :reyalS", "boj a ton yltnerappA", /masteryTotal/, /Job Mastery: ([\d]+)%/, / Current Page: index_controller /, "wen_llac_yxes. raw_eralced#", /does not exist/i, /are already at war/, /their war status/, "tidnab-boj ofni-tidnab", "rellortnoc_boj", ">vid/<>retnec/<>rb<>rb<", ">rb<>retnec<>\"eltit-fnoc\"=ssalc vid<>a/<>\"41\"=htdiw \"41\"=thgieh \"fig.10_41x41_nottub_esolc_kcalb/scihparg/bfwm/6e/ten.dwnll.sh.vpagnyz//:", ";xp006 :pot ;%94 :tfel ;%04 :htdiw ;kcolb :yalpsid", "/: nwodlooc no si", "=ytic_werc&1=xajasi&etavitca=noitca_wx&werCytiC=rellortnoc_wx?php.revres_lmth/etomer/bfwm/moc.agnyz.srawaifam.koobecaf//:", "9-llatcelloc-2vporp", "7-llatcelloc-2vporp", ">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"8\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<", "3_elggoTslavir", "\"=crs gmi<>naps<>naps<>\"3_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<", "\"=crs gmi<>naps<>naps<>\"2_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<", "1_elggoTslavir", "\"=crs gmi<>naps<>naps<>\"1_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<", "_elggoTtegrat\"=di \";evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<", "_elggoTtegrat\"=di \";evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<", "\"=crs gmi<>naps<>naps<>\"0_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<", "\"=crs gmi<>naps<>naps<>\"0_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<", /targetToggle_(\d+)/, "\"=crs gmi<>naps<>naps<", /rivalsToggle_(\d+)/, "cni_tl_kta_wmu", "ltt_tl_kta_wmu", "erotS ppA telkramkooB", ">vid/<>elbat/<", "\"=crs gmi<>\"knalb_\"=tegrat \"/piv-ot-edargpu/moc.wmdekcolnu//:ptth\"=ferh \"ppa_elggot\"=ssalc \";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<", "\"=crs gmi<>\"", ">dt/< >llams/<>a/<evomer>\"", ">dt/< >llams/<", ">llams<>dt<>dt/<>\"", ">\"030303#\"=rolocgb rt<", "!ylranG gnihtemoS", "])\'0 :hserfeR\',)(gnirts(sniatnoc[naps//naps//]\"tsoc_hserfer_bor\"=di@[a//", "=noitanitsed&7=ytic_wx&levart=noitca_wx&levart=rellortnoc_wx?php.revres_lmth/etomer", ";)bor.rellortnoCgnibboR ,)2(boRerp.rellortnoCgnibboR ,0 ,1 ,\'", "llAboRkcilCeno", "gnibboR ot nruteR :reyalS", ">vid/<>vid/<>retnec/<>naps/<", ">\"xp5-:pot;evitaler:noitisop\"=elyts \"eltit_pupoPym\"=di naps<>\"eltit_eludom_eripme\"=ssalc \"xp02:thgieh;xp", "\"=htdiw \"fig.002x067_tneidarg_gb/eripme/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>\"pop_ofni_PE_inim\"=ssalc \"99999:xedni-z\"=elyts vid<>a/<>\"))di.edoNtnerap.siht(dIyBtnemelEteg.tnemucod(dlihCevomer.)\'reddof_pupop\'(dIyBtnemelEteg.tnemucod\"=kcilcno \"esolc_pop\"=ssalc \"#\"=ferh \"esolC_poPym\"=di a<>\";999999:xedni-z;xp", ":tfel;xp001:pot;etulosba:noitisop\"=elyts \" xob_pop\"=ssalc \"", "!WM ot kcab emoc neht ,txen WMdekcolnU ot nigol ot deen lliw uoY .WMdekcolnU ruoy etadpu tsum uoY", /\n/g, "sgat_tsilkcalb_dekcolnu", "delbasid_tsilkcalb_dekcolnu", "delbasid_tsiletihw_dekcolnu", /true/, "sgnitteSevaS", ">vid/<>a/<>naps/<>naps/<etadpU & egarotS raelC>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"etaDpUecroF\"=di a<>rb<>rb<>a/<>naps/<>naps/<erotS ppA>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"MBmotsuC\"=di a<;psbn&;psbn&>a/<>naps/<>naps/<evaS>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"sgnitteSevaS\"=di a<", "\"=eulav tupni<;psbn&]0001-*333[ etaR kcattA thgiF", "\"=eulav tupni<;psbn&etaR yaleD gnibboR/sdauqS boR>\"dekcehC\"=eulav \"dauqsbor\"=eman \"dauqsbor\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<YN ni;psbn&>\"dekcehC\"=eulav \"ynlaeh\"=eman \"ynlaeh\"=di \"xobkcehc\"=epyt tupni<>\"2dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", "\"=eulav tupni<;psbn&tegrat rep skcatta mumixaM>\"dekcehC\"=eulav \"kcehcpots\"=eman \"kcehcpots\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<stegraT yhtlaeH diovA;psbn&>\"dekcehC\"=eulav \"yhtlaeh\"=eman \"yhtlaeh\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"3dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", "\"=eulav tupni< fo stsruB esU>\"dekcehC\"=eulav \"tsrub\"=eman \"tsrub\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"dimroftsop\"=di \";xp09:htdiw;enon:eziser\"=elyts \"txet\"=epyt \"", ">\"niaga laeh nac ew litnu sdnoces ", "tnoc_htlaeh_tats", "1=yticx&laeh=noitca_wx&latipsoh=rellortnoc_wx?php.revres_lmth", "yevrus_spn_wohs=noitca_wx&yevrus=rellortnoc_wx?php.revres_lmth", "0000000001=tnuoma&", "8=di_tneilc_wx&5=ytic&6=epyt_gnidliub&", /\,/g, "tsoob eerf a teG", /Need help icing yours.+/, ".slavir ruoy eci ot tsoob thgif eerf ruoy teG", /var feed = (.+?); MW\.Feed/, /msg.fight_result =/, "tnuoc_latot_wmu", "tnuoc_eci_wmu", "deci_yalrevo_rednefed_2vf", /user_fields\['user_stamina'\] = parseInt\("([\d]+)/, /'user_cash_london'\] = "\\u00a3([\d,]+)"/, /xw_controller=hitlist&xw_action=attack([&\w\d=]+)/, /message_body">You <strong>LOST<\/strong> the fight/, "yub_eslupmi wen_kcatta_yxes der muidem wen_nottub_yxes", /user_stamina":([\d]+)/, "ogacihc_hsac_resu", /exp_to_next_level":([\d]+),/, "ffo_tsoob_ntbktarewop_2vthgif", "ntBkcattA2Vthgif yub_hctab_eslupmi der trohs wen_nottub_yxes", /Lost/, ".skcus kcehceci agnyZ :reyalS", /is already dead or too weak!/, "=nekot_yzwm&", "=ytic_wx&tes=noitca_wx&tsiltih=rellortnoc_wx?php.revres_lmth/etomer/bfwm/moc.agnyz.srawaifam.koobecaf//:", /xw_controller=hitlist&xw_action=set&xw_city=(\d+).*?tmp=([a-z0-9]+).*?cb=([a-z0-9]+).*?target_pid=(\d+).*?mwzy_token=([a-f0-9]+)/, /Mafia you Attacked/, /Your Attackers/, /targetToggle/, "1=bat&weiv=noitca_wx&thgif=rellortnoc_wx?php.revres_lmth/etomer", /10 Attack/, "wen_hserfer_egap_yxes wen_nottub_yxes", "!tsilkcalb eht no si ylimaf stegrat reyalS", "p_kcatta_ntb", "levart=unem&=smaraPtxen&xedni=morf&", "=resu&weiv=noitca_wx&stats=rellortnoc_wx?php.revres_lmth/etomer", "ntb-etanimile-tidnab", /'user_cash_vegas'\] = "([V$\d,]+)"/, /user_cash_nyc'\] = "([$\d,]+)"/, /user_cash'\] = parseInt\("([\d]+)/, /user_energy'\] = parseInt\("([\d]+)/, /"user_stamina":([\d]+),"user_max_stamina/, " :htdiw ;tfel :taolf ;tfel :ngila-txet ;)94 ,202 ,14(bgr :roloc-dnuorgkcab ;neddih :y-wolfrevo ;neddih :x-wolfrevo", "lizarb_hsac_resu", "sagev_hsac_resu", "nodnol_hsac_resu", /user_cash_nyc":"([$\d,]+)"/, "=di_resu_wx_fs&1=daoletil&1=xaja", "epyT-tnetnoC", "htiW-detseuqeR-X", /(p\|[\d]+)/, "]])\"552 ,552 ,552\",)(gnirts(sniatnoc[elyts@[]\"ofni_boj\"=ssalc@[vid//", /job=([\d]+)/, /cb=([\da-f]+)/, "=ytic_wx&bojod=noitca_wx&boj=rellortnoc_wx?php.revres_lmth", "]])\";etihw dilos xp1 :redrob\",)(gnirts(sniatnoc[elyts@[]\"xifraelc ydob_egassem\"=ssalc@[vid//", "egap_raw_eralced=morf&", /([\d]+)/, ">a/<2$>\"knalb_\"=tegrat \"2$//:ptth\"=ferh a<1$", ">a/<1$>\"knalb_\"=tegrat \"1$\"=ferh a<", ">\";2.0 :yticapo;)02=yticapo(ahpla :retlif;%09:htdiw\"=elyts rh<", "\"=ferh a< >rb<", ">\";yerGmid :roloc\"=elyts naps<psbn&-;psbn&>a/<", ">rb<>a/<>\")\';enon :yalpsid\',\'elyts\'(etubirttAtes.)\'deef_golb_wmu\'(dIyBtnemelEteg.tnemucod\"=kcilcno \"esolc_pop\"=ssalc \"#\"=ferh \"esolC_poPym\"=di a<>a/<>\"gnp.dekcolnu/segami/moc.wmdekcolnu.ehcac//:ptth\"=crs gmi<>\"knalb_\"=tegrat \"moc.wmdekcolnu//:ptth\"=ferh a<>\";xp082 :thgieh-nim ;xp717 :htdiw-nim;xp082 :thgieh-xam ;xp717 :htdiw-xam;xp082 :thgieh ;xp717 :htdiw;ezilatipac:mrofsnart-txet;xp21 :ezis-tnof;neddih :x-wolfrevo ;otua :y-wolfrevo ;xp01 xp01 xp01 xp01 :nigram\"=elyts vid<", "gnidaol_deef_golb_wmu", "xp0 :thgieh ;xp0 :thgieh-nim ;xp0 :thgieh-xam ;xp0 :htdiw ;xp0 :htdiw-nim ;xp0 :htdiw-xam ;kcolb :yalpsid ;xp", "rabloot_mlohkcops", /px/, ">vid/<>naps/<>naps/<0>\";yerg:roloc;xp04:htdiw;xp5:tfel;evitaler:noitisop\"=elyts \")slliK + secI( latoT\"=eltit \"tnuoc_latot_wmu\"=di naps<>naps/<:T >\";xp5:tfel;evitaler:noitisop\"=elyts naps<>naps/<0>\";yerg:roloc;xp04:htdiw;xp5:tfel;evitaler:noitisop\"=elyts \"slliK\"=eltit \"tnuoc_llik_wmu\"=di naps<>naps/<:K >\";xp5:tfel;evitaler:noitisop\"=elyts naps<>naps/<0>\";yerg:roloc;xp04:htdiw;evitaler:noitisop\"=elyts \"secI\"=eltit \"tnuoc_eci_wmu\"=di naps<>naps/<:I>naps<>\"sutatSdekcolnu\"=di \";xp81:ezis-tnof;xp002:htdiw;xp651:tfel;xp2-:pot;evitaler:noitisop\"=elyts naps<>a/<>naps/<>naps/<>\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp541:tfel\"=elyts \"ffOnOsboJ\"=di a<>a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp041:tfel\"=elyts \"ffOnOmatS\"=di a<>a/<>naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp531:tfel\"=elyts \"ffOnOlaeH\"=di a<>a/<>naps/<>naps/<>\"", "\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp521:tfel\"=elyts \"ffOnOlivE\"=di a<>a/<>naps/<>naps/<>\"gnp.hcnerw_remmah_noci/3v/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp021:tfel\"=elyts \"sgnitteSkcolnu\"=di a<>a/<>naps/<>naps/<>\"fig.10_61x61_hsac_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<>\"xp01:tfel\"=elyts \"BnottuBknaB\"=di \"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc a<>vid/<>\";enon :yalpsid\"=elyts \"xob_pop\"=ssalc \"deef_golb_wmu\"=di vid<>a/<>naps/<>naps/<>\"", ">a/<!WON LAEH>\"#\"=ferh a<", "sgnitteSkcolnu", "=ytic_wx&metIsunoBdda=noitca_wx&sunoBpUlevel=rellortnoc_wx?php.revres_lmth/etomer", /unlocked_toolbar=(ey[^\&]+)&/, "reniatnoc_sloot", ">vid/<>vid/<", "\"=di \"mbdaol_dekcolnu\"=ssalc a<", "reniatnoc_gol", "\"=crs \";6.0 :yticapo;)06=yticapo(ahpla :retlif ;xp9 :pot ;evitaler :noitisop\"=elyts \"retoof_gol_WMU\"=di gmi<>\"077063887536612/emaG-sraW-aifaM-ruoy-kcolnU-WMdekcolnU/segap/moc.koobecaf.www//:ptth\"=ferh \"knalb_=tegrat \"egapnaF WMdekcolnU\"=eltit a<>vid/<(: deggol sgnihtoN>\";yerg:roloc;%43:tfel;xp2:pot;evitaler:noitisop\"=elyts vid<>\";kcolb:x-wolfrevo;kcolb:y-wolfrevo;xp56:thgieh-nim;xp073:thgieh-xam;xp6:pot;xp6 :pot ;xp6 xp6 :suidar-tfel-mottob-redrob ;xp6 xp6 :suidar-thgir-mottob-redrob ;xp6   :thgirmottob-suidar-redrob-zom- ;xp6   :tfelmottob-suidar-redrob-zom-;kcalb:roloc-dnuorgkcab;xp691-:tfel;evitaler:noitisop;xp092:htdiw;xp1-:pot-nigram;enon :yalpsid ;02 :xedni-z\"=elyts \"gol-WMU\"=di vid<>a/< >naps/<naps/<goL>\";suounitnoc :ycilop-enilni-dnuorgkcab-zom- ;gniddap :nigiro-dnuorgkcab-zom- ;redrob :pilc-dnuorgkcab-zom- ;tfel :ngila-txet ;%05 xp57 llorcs taeper-on )fig.worra_levart_nwodpord/scihparg/bfwm/moc.agnyz.citats.bfwm//:ptth(lru tnerapsnart :dnuorgkcab\"=elyts naps<>naps<>\";xp59 :htdiw\"=elyts \"redrob_etihw_kcalb trohs wen_nottub_yxes\"=ssalc a<>\";xp371 :pot ;xp056 :tfel ;xp001 :htdiw;etulosba:noitisop\"=elyts \"reniatnoc_gol\"=di vid<>elyts/<};xp8- :pot ;evitaler :noitisop ;xp5 :mottob-nigram ;otua :thgir-nigram ;otua :tfel-nigram ;kcolb :yalpsid{ retoof_gol_WMU# };xp2 0 :nigram ;dlob :thgiew-tnof ;xp21 :ezis-tnof ;xp1 xp1 xp1 :htdiw-redrob ;dcdcdc# dilos xp1 :redrob{gol-WMU#>\"ssc/txet\"=epyt elyts<", "retoof_gol_WMU", "fws.01draobpilCoreZ/segami/moc.wmdekcolnu.ehcac//:ptth", "\"=crs \";6.0 :yticapo;)06=yticapo(ahpla :retlif ;xp4 :pot ;evitaler :noitisop\"=elyts \"retoof_gol_WMU\"=di gmi<>\"077063887536612/emaG-sraW-aifaM-ruoy-kcolnU-WMdekcolnU/segap/moc.koobecaf.www//:ptth\"=ferh \"knalb_=tegrat \"egapnaF WMdekcolnU\"=eltit a<>vid/<", ">ht/<>\"%52\"=htdiw ht<>ht/<>naps/<", ">rt<>\"0\"=redrob elbat<", /^Slayer/, ")1=yticapo(ahpla", "=di_tnenoppo", "stegraT reppoH gnihcteF reyalS", "tegrat reppoH txeN gnidaoL reyalS", "tsilthgif gnidaoL-eR reyalS", "!god sdneirf si lla\'aY reyalS", "evitcA edoM reppoH ykcitS reyalS", "reddof_pupop", ">rb<>rb<>a/<>naps/<>naps/<tratS>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"hcnuaL\"=di a<>rb<>rb<ykcitS ekaM>\"ykcits\"=di \"xobkcehc\"=epyt tupni<>rb<>\"01dimroftsop\"=di \";xp051:htdiw;enon:eziser\"=elyts \"txet\"=epyt\"", "stegraT reppoH moc.stsilwM thgiF", "etihw muidem wen_nottub_yxes", /Declare war now for/, "evitcA ylimaF thgiF reyalS", /user=(.+?)\"/, "])\"seilimaF\" ,.(sniatnoc[a//])\"tsrif_bat no_bat\" ,ssalc@(sniatnoc[il//", "evitcA edoM slaviR reyalS", "evitcA edoM eliforP reyalS", /user=p\|(\d+)'.+>Profile/, "evitcA edoM gnibboR ykcitS reyalS", "evitcA slaviR ykcitS reyalS", "evitcA tsilthgiF ykcitS reyalS", ">a/<>naps/<>naps/<>\"\"", "ngilAvWMU egnaro trohs wen_nottub_yxes", ">naps/<>naps/<>\"", ">naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<", "ngilAvWMU neerg trohs wen_nottub_yxes", ">naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<", ">retnec/<>b/<serocS metI tsewoL>b<>retnec<", ">rt/<>ht/<0>\"cni_tl_fed_wmu\"=di \"doog\"=ssalc \"%04\"=htdiw ht<>ht/<000,000>\"ltt_tl_fed_wmu\"=di \"esnefed_puorg\"=ssalc \"lamron:thgiew-tnof\"=elyts \"%06\"=htdiw ht<>rt<>rt/<>ht/<0>\"cni_tl_kta_wmu\"=di \"doog\"=ssalc \"%04\"=htdiw ht<>ht/<000,000>\"ltt_tl_kta_wmu\"=di \"kcatta_puorg\"=ssalc \"lamron:thgiew-tnof\"=elyts \"%06\"=htdiw ht<>rt<", "gnikcart_tats_wmu", ">vid/<>\"elddimsba\"=ngila \"61\"=thgieh \"91\"=htdiw \"fig.10_61x22_kcatta_aifam_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>\"serocS metI tsewoL\"=eltit \";xp85:pot;xp01-:tfel;evitaler:noitisop;xp02:htdiw\"=elyts \"", ".esolc ot em kcilC", "nottub_ekil_wm#", "1yalrevOrorrE#", "yalrevOgnidaoL#", "=/+9876543210zyxwvutsrqponmlkjihgfedcbaZYXWVUTSRQPONMLKJIHGFEDCBA", "efil ot kcab thguorb enord s\'ytfeL", "sknil fo not a nepo ot siht esU", ".YLNO gnithgif ot sdrager ni ,esnefed + kcatta rof smeti evitca lla stsiL", "sj.rotarekop/telkramkoob/moc.telkcops//:ptth", "rotan-a-znieH", ".ereht tuo sOH etarc uoy lla roF", ".ay rof sboj staepeR", "sj.tluserelttab/mb/moc.reficul-ufmm//:ptth", "sj.enordu/sppa/moc.wmdekcolnu.rabloot//:ptth", "enorD dekcolnU", "ecivres em.nokcops gnisu SLRU yniT etareneG", "sdneirf ruoy ot sgaB yretsyM deR tfiG", "sj.pxtcellocnalc/mb/moc.reficul-ufmm//:ptth", "PX tcelloC nalC", ".egap elttab morf esu ,elttab ylimaf morf tooL tcelloC", "sj.retsop_lliks/telkramkoob/moc.telkcops//:ptth", "nissassA tnioP llikS", "loot rehto yna naht retteb sessob ylimaf eht sthgiF", "sj.rethgifssobyn/telkramkoob/moc.telkcops//:ptth", "rethgiF ssoB YN", ".serohc ruoy od ot uoy dnimer ,tsilkcehC yliaD", ".egap ylimaf eht no noitamrofni erom teG", "aifam ruoy ot stfig dnes ot siht esU", ".setar PX ot ygrenE seviG", ".pu gnilevel uoy peek ot cte ,sbor ,sboj snuR", ".ffuts elbatfig htiw skrow ,aifam ruoy ot stfig dneS", ".pupop a gnisu ,lru gnol a morf LRUyniT etareneG", ".fo dir teg nac uoy tahw ees ,yrotnevni ruoy sezylanA", "sj.5ssobsmafwm/mb/moc.reficul-ufmm//:ptth", "sj.rennacs-maerts/telkramkoob/moc.telkcops//:ptth", "rennacS maertS", ".siht htiw deen uoy tahw tcelloc ,stfig htiw aifam ruoy gnitsalb retfA", "sj.rethgif-ssob-ylimaf/telkramkoob/moc.telkcops//:ptth", "rethgiF ssoB kcopS", ".smeti depuorg fo evah uoy tool ynam woh no weiv a teg ylkciuQ", "sj.yrotsih_ylimaf/telkramkoob/moc.telkcops//:ptth", "sj.yrotsih_aifam/telkramkoob/moc.telkcops//:ptth", "reganaM aifaM", ".tnemeveihca eht teg meht pleh ,stols sdneirf ruoy yalP", ".aifam eritne ruoy ot stfig fo stsol tsalB", "sj.reganam.ytreporp/telkramkoob/moc.telkcops.www//:ptth", "reganaM ytreporP", ".kcab tfig otua htiw ,retneC egasseM agnyZ eht morf stfiG tcelloC", "sj.kcapinim/aifam/moc.mlohkcops.www//:ptth", "sj.xuder.rebbor/telkramkoob/moc.telkcops//:ptth", "xudeR rebboR", ".egap boR eht no nur eb ot deen ton seod ,telkramkoob gnibboR weN", ".esu erofeb relaeh WMdekcolnU ffo nruT .loot gnithgif hcir erutaeF", "sj.hctiws/moc.wmdekcolnu.rabloot//:ptth", "sj.emarfnu/sppa/moc.wmdekcolnu.rabloot//:ptth", "sj.smegtluav/sppa/moc.wmdekcolnu.rabloot//:ptth", "sj.dliubdeeps/sppa/moc.wmdekcolnu.rabloot//:ptth", "EC39TZqMWdprv7RA6PHmt6072LVm4gzDt", /mw_city(\d)/, "==,46esab;gnp/egami:atad", "=,46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", ",46esab;gnp/egami:atad", "==,46esab;gnp/egami:atad", "==,46esab;gnp/egami:atad", "repparw_ytic_wm", ".ylno srebmem PIV rof si loot sihT .PR gnisu gnitfarC ylraE", ".ylno srebmem PIV ruo rof si loot sihT .lizarB ni smeG eht tluaV", ".emag eht emarfnU", ".neercs thgif ro egap eliforp no esU .koobecaf ot sraW aifaM morf sehctiwS", "rotaN-a-nissassA", "sj.rotan-a-nissassa/telkramkoob/moc.telkcops//:ptth", "sj.gb.rebbor/telkramkoob/moc.telkcops.www//:ptth", ".seitreporp cificeps bor ottelkramkoob gnibboR", ".resworb ruoy ni rablooT agnyZ eht gnillatsni tuohtiw tsoob modnar eht teG", "rotcelloC CMZ", "sj.ateb-cmz/telkramkoob/moc.telkcops//:ptth", ".gnitfarc yliad ruoy od dna ,seitreporp ruoy morf tcelloC", "retsalB tfiG", "sj.retsalbtfig/telkramkoob/moc.telkcops//:ptth", "sj.stols_esool/telkramkoob/moc.telkcops//:ptth", ".yalp t\'nod taht seno eht tuo kcik ,aifam ruoy eganaM", "reganaM ylimaF", "spuorG yrotnevnI", "sj.puorgyrotnevni/telkramkoob/moc.telkcops//:ptth", ".thgif ssoB eht ni ecno esu ,sessob ylimaf eht thgiF", "rotcelloC tfiG", "sj.rotcelloctfig/telkramkoob/moc.telkcops//:ptth", ".weivrevo reisae rof meht spuorg dna stsop detaler sraW aifaM rof deef swen ruoy nacS", "rethgiF ssoB reficuL", "rezylanA metI", "sj.rezylana.yrotnevni/telkramkoob/moc.telkcops//:ptth", "sj.ynitteg/mb/moc.reficul-ufmm//:ptth", "sj.rekcuhC/sg/kd.ynomis//:ptth", "sj.SRAlluFenorD/stpircS/aifaM/ten.0ymisrm.ndc//:ptth", "sj.pX2nE/stpircS/aifaM/ten.0ymisrm.ndc//:ptth", "sj.skniltfigwm/mb/moc.reficul-ufmm//:ptth", "sj.ofni_ylimaf/telkramkoob/moc.telkcops//:ptth", "sj.ateb-tsilkcehc/telkramkoob/moc.telkcops//:ptth", ".YN ni sessob eht sthgiF", "rethgiF ssoB dekcolnU", "sj.ssob_ylimaf/sppa/moc.wmdekcolnu.rabloot//:ptth", ".stniop lliks ruoy sngissA", "rotcelloC elttaB", "sj.sdrawerelttab/mb/moc.reficul-ufmm//:ptth", "ffuts yretsam ylimaf morf PX tcelloc ot siht esU", "retfiG gaB yretsyM deR", "sj.bm_der/sppa/moc.wmdekcolnu.rabloot//:ptth", "sj.renetrohs_lru/sppa/moc.wmdekcolnu.rabloot//:ptth", "!enorD wen ruo tseT ateB", "selttab ylimaf eht tuoba ofni tnatropmi teG", "xudeR boJtaepeR", "sj.xuder.bojtaeper/telkramkoob/moc.telkcops//:ptth", "sj.reyuBetarC/stpircS/aifaM/ten.0ymisrm.ndc//:ptth", ".ay rof mekop eht syalP", "smetI evitcA yM", "sj.smeti_evitca_ym/sppa/moc.wmdekcolnu.rabloot//:ptth", "sj.renepoknil/telkramkoob/moc.telkcops//:ptth", "sj.redaolpgo/sg/kd.ynomis//:ptth", /20273871374b6c215c21337054646e44347627713c7e22455378464c27/, "dnuorgkcaBgnidaoL#", "2yalrevOrorrE#", ";)(elggot.eludoMsmetItsroW.WM", "\"=kcilcno vid<", ";kcolb :yalpsid", ";xp521 :htdiw;xp5:pot;xp7-:tfel;evitaler:noitisop", "eldnahtnihmetiwol", "ngilAvWMU der trohs wen_nottub_yxes", "ngilAvWMU kcalb trohs wen_nottub_yxes", ">naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs  gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<", "\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<", ">naps/<>naps/<>\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<", "deppotS reyalS", "])\"srethgiF\" ,.(sniatnoc[a//])\"elddim_bat no_bat\" ,ssalc@(sniatnoc[il//", "])\"slaviR\" ,.(sniatnoc[a//])\"elddim_bat no_bat\" ,ssalc@(sniatnoc[il//", "])\"gnibboR\" ,.(sniatnoc[a//])\"no_bat\" ,ssalc@(sniatnoc[il//", "])\"eliforP\" ,.(sniatnoc[a//])\"reniatnoc_bat\" ,di@(sniatnoc[vid//", /levels">\((.*?)\)/, "evitcA edoM tsilthgiF reyalS", "evitcA edoM gnibboR reyalS", "detceles bat_van_nalc", "evitcA edoM elttaB reyalS", "daeh_raw_eralced", ".yrter neht ,eralced ot tnaw yllaer uoy fi tser remit a esahcrup ,raw ta ydaerlA", "eludoMetadpUreyalP", "\"=eulav tupni<;psbn& DI tekcuB kcattA>rb<>rb<>rb<!reppoh morf stegrat thgif yllacitamotua ot WMdekcolnU esu won nac uoy ,moc.stsilwm ta sdneirf ruo morf ecivres a si reppoH>rb<", "hcnuaLreppoH", "01dimroftsop", /This player is currently part of your mafia/, "lautxetnoc tpmorp_yub_eslupmi", "0=bat&weiv=noitca_wx&thgif=rellortnoc_wx?php.revres_lmth/etomer", "=tekcub?php.wmdekcolnu_evilteg/stelkraMkooB/moc.stsilwm//:ptth", /opponent_id=p%7C\d+/, "gnorts oot llA ro stegraT eviL oN :reyalS", ")06=yticapo(ahpla", ">\";xp682:htdiw;xp051 :thgieh-nim ;xp003 :thgieh-xam;kcolb:x-wolfrevo;llorcs:y-wolfrevo\"=elyts vid<>a/<;psbn&;psbn&goL raelC;psbn&;psbn&>\"gol_raelc-WMU\"=di a<>\";2.0 :yticapo;)02=yticapo(ahpla :retlif;xp2-:pot;evitaler:noitisop;%09:htdiw\"=elyts rh<", ">\";xp2-:pot;xp5:tfel;evitaler:noitisop\"=elyts naps<>\"%57\"=htdiw ht<", ">\";2.0 :yticapo;)02=yticapo(ahpla :retlif;xp1-:pot;evitaler:noitisop;%09:htdiw\"=elyts rh<>elbat/<>rt/<", ">vid/<>vid/<>a/<>naps/<>naps/<)!ateb( ypoC>naps<>naps<>\";xp03-:pot ;xp081:tfel ;evitaler:noitisop\"=elyts \"wen_nottub_yxes\"=ssalc \"#\"=ferh \"golelbaypoc\"=di a<>a/<>\"", ">naps/<>naps/<!deipoC secI>naps<>naps<", "gol_raelc-WMU", ">vid/<>vid/<>a/<>\"", "> \";x051:htdiw;xp2-:pot-nigram;enon :yalpsid ;02 :xedni-z\"=elyts \"uneMsloot\"=di vid<>a/< >naps/<naps/<slooT>\";suounitnoc :ycilop-enilni-dnuorgkcab-zom- ;gniddap :nigiro-dnuorgkcab-zom- ;redrob :pilc-dnuorgkcab-zom- ;tfel :ngila-txet ;%05 xp57 llorcs taeper-on )fig.worra_levart_nwodpord/scihparg/bfwm/moc.agnyz.citats.bfwm//:ptth(lru tnerapsnart :dnuorgkcab\"=elyts naps<>naps<>\";xp59 :htdiw\"=elyts \"redrob_etihw_kcalb trohs wen_nottub_yxes\"=ssalc a<>\";xp371 :pot ;xp411 :tfel ;etulosba :noitisop ;xp001 :htdiw\"=elyts \"reniatnoc_sloot\"=di vid<>elyts/<};xp2 0 :nigram ;dlob :thgiew-tnof ;xp21 :ezis-tnof ;xp1 xp1 0 :htdiw-redrob ;dcdcdc# dilos xp1 :redrob{ uneMsloot#>\"ssc/txet\"=epyt elyts<", ">\"elddim noitanitsed_yxes\"=ssalc vid<>\"", "sknil_kcolnu", "mbdaol_dekcolnu", "tpircsavaj/txet", "rabloot_kcolnu", "htlaeh_kcolc", "\"=crs \"42\"=htdiw \"42\"=thgieh \";xp1 :tfel ;xp4-:pot ;evitaler:noitisop\"=elyts gmi<>naps<>naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \"ogol_wmu\"=di a<>elyts/<} ;xp5-:pot ;evitaler:noitisop { ngilAvWMU.>\"ssc/txet\"=epyt elyts<>\";xp057:htdiw;xp03 :thgieh\"=elyts \"eltit_eludom_eripme xifraelc\"=ssalc \"tnetnoc_rabloot\"=di vid<", "\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp031:tfel\"=elyts \"ffOnOtsoP\"=di a<>a/<>naps/<>naps/<>\"\"", "deef_golb_wmu", ";enon :yalpsid", " :pot ;xp9 :tfel ;00002 :xedni-z ;etulosba :noitisop", ">retnec/< gnidaoL>\"gnidaol_deef_golb_wmu\"=di retnec<>rb<>rb<>rb<>rb<>rb<>rb<", "01=tnuoc&stnemhcatta,stnemmoc,nialp_eltit,noitpac,sutats,sutats_tnemmoc,tnuoc_tnemmoc,rohtua,sgat,seirogetac,deifidom,epyt,guls,tnetnoc=edulcxe&stsop_tnecer_teg=nosj?/moc.wmdekcolnu.ehcac//:ptth", " >\";xp51 :ezis-tnof\"=elyts \"knalb_\"=tegrat \"", ">\" ;xp01 xp0 xp01 xp0 :nigram\"=elyts vid<>naps/<", ">vid/<>a/<moc.WMdekcolnU @ erom daeR>\"knalb_\"=tegrat \"", /(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim, /(^|[^\/])(www\.[\S]+(\b|$))/gim, "=ytic_wx&raw_eralced=noitca_wx&raw=rellortnoc_wx?php.revres_lmth/etomer", "C7%p=di_tegrat&", "level_txen_ot_pxe", /([\d]+) Energy/, /tmp=([\da-f]+)/, /person=([\d]+)/, /tab=([\d]+)/, /job([\d]+)/, /'p\|([\d]+)',0,'(.+)'\);/, "tseuqeRpttHLMX", "dedocnelru-mrof-www-x/noitacilppa", "1=emarf_qer_piks&", "cyn_hsac_resu", /user_cash_london":"([Â£\d,]+)"/, /user_cash_vegas":"([V$\d,]+)"/, /user_cash_brazil":"([BRL$\d,]+)"/, /percent_complete":([\d]+),"/, /user_cash_nyc'\] = "([$\d,]+)/, /percent_complete'\] = "([\d]+)/, /exp_to_next_level'\] = parseInt\("([\d]+)/, /'user_cash_london'\] = "([Â£\d,]+)"/, /'user_cash_brazil'\] = "([BRL$\d,]+)"/, "yub_eslupmi der trohs wen_nottub_yxes wen_nottub_yxes", "=noitanitsed&levart=noitca_wx&levart=rellortnoc_wx?php.revres_lmth/etomer", "daed_reyalp_tsil_thgif", "!daed ro gnorts oot era stegrat reyalS", " yub_eslupmi wen_kcatta_yxes trohs wen_nottub_yxes", " yub_eslupmi wen_kcatta_yxes muidem wen_nottub_yxes", ".tsilthgif gnidaoL-eR reyalS", /Your Rivals/, /Family Rivals/, /btn_attack_p/, /sf_xw_user_id=([a-z]\|[0-9]+)/, /https/, "=dip_tegrat&", "8=di_tneilc_wx&", /The action was not able to be completed/, "sutats_thgif_rekcatta", "ntBkcattA2Vthgif yub_eslupmi der trohs wen_nottub_yxes", "no_tsoob_ntbktarewop_2vthgif", "htlaeh_rednefed", /user_cash_london":"\\u00a3([\d,]+)"/, /user_cash_chicago":"\\u00a2([\d,]+)"/, "nelots_yalrevo_rednefed_2vf", "])\'niagA kcattA\',)(gnirts(sniatnoc[]\'wen_kcatta_yxes der trohs wen_nottub_yxes\'=ssalc@[a//", "?php.revres_lmth", /user_fields\['exp_to_next_level'\] = parseInt\("([\d]+)/, /'user_cash_chicago'\] = "\\u00a2([\d,]+)"/, "tsil_diova_dekcolnu", /boost', 'link': '(.+?)'/, "tnuoc_llik_wmu", "dellik_yalrevo_rednefed_2vf", /'feed_js': '(.+?)'/, ".slavir ruoy eci ot tsoob thgif eerf a teG", /Think you can do better.+/, /Need to whack someone.+/, /V\$([\d,]+)/, "=tnuoma&tisopeDknaBnoitcA=noitcaod&5=ytic_wx&noitcaod=noitca_wx&2Vytreporp=rellortnoc_wx?php.revres_lmth", "=ytic_wx&lla_tisoped=noitca_wx&knab=rellortnoc_wx?php.revres_lmth", "htlaeh_xam_resu", /,/, "laeh=noitca_wx&latipsoh=rellortnoc_wx?php.revres_lmth", "\"=eltit \"yerg:roloc\"=elyts naps<( htlaeH", "\"=eulav tupni<;psbn& ot seci tsopotuA >\"dekcehC\"=eulav \"tsopotua\"=eman \"tsopotua\"=di \"xobkcehc\"=epyt tupni<>rb<", "\"=eulav tupni<;psbn&gniniamer PX ta esuaP>\"dekcehC\"=eulav \"kcehcesuap\"=eman \"kcehcesuap\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<tahC ediH >\"dekcehC\"=eulav \"tahCevomer.sgnittes.dekcolnu\"=eman \"tahcmer\"=di \"xobkcehc\"=epyt tupni<;psbn&knabotuA >\"dekcehC\"=eulav \"tiknab\"=eman \"knabotua\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<srelaetS ecI >\"dekcehC\"=eulav \"nelotswollof\"=eman \"nelotswollof\"=di \"xobkcehc\"=epyt tupni<;psbn&;psbn&>\"77dimroftsop\"=di \";xp01:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", "\"=eulav tupni<;psbn&dlohserhT laeH>rb<>rb<>\"7dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", ">rb<>rb<sunoB pu-leveL mialC >\"dekcehC\"=eulav \"levelmialc\"=eman \"levelmialc\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"5dimroftsop\"=di \";xp53:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", ">vid/<>vid/<>aeratxet/<>\";%56:htdiw;enon:eziser\"=elyts \"sgat_tsilkcalb_dekcolnu\"=di aeratxet<>rb<ffO >\"elggot_tsilkcalb_dekcolnu\"=eman \"delbasid_tsilkcalb_dekcolnu\"=di \"oidar\"=epyt tupni<nO >\"elggot_tsilkcalb_dekcolnu\"=eman \"delbane_tsilkcalb_dekcolnu\"=di \"oidar\"=epyt tupni<:tsilkcalB>\".yltcaxe gat emag-ni eht hctam TSUM deretne gat ehT\n\n)tsilthgiF & egap slaviR( .gat siht htiw sreyalp piks lliw reyalS dna ,enil rep eno ,gat ylimaf a retnE\"=eltit vid<>vid/<>aeratxet/<>\";%56:htdiw;enon:eziser\"=elyts \"sgat_tsiletihw_dekcolnu\"=di aeratxet<>rb<ffO >\"elggot_tsiletihw_dekcolnu\"=eman \"delbasid_tsiletihw_dekcolnu\"=di \"oidar\"=epyt tupni<nO >\"elggot_tsiletihw_dekcolnu\"=eman \"delbane_tsiletihw_dekcolnu\"=di \"oidar\"=epyt tupni<:tsiletihW>\".yltcaxe gat emag-ni eht hctam TSUM deretne gat ehT\n\n)ylno egap slaviR( .gat siht kcatta ylno lliw reyalS dna ,enil rep eno ,gat ylimaf a retnE\"=eltit vid<>vid<>rb<>rb<agnyZ yb dewolla deeps tsetsaf eht si 333*>rb<>\"6dimroftsop\"=di \";xp53:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"", "sgnitteSdekcolnU", "nelotswollof", "delbane_tsiletihw_dekcolnu", "delbane_tsilkcalb_dekcolnu", "sgat_tsiletihw_dekcolnu", /(.+)/, "77dimroftsop", "php.nigol-pw/moc.wmdekcolnu//:ptth", " :htdiw;kcolb :yalpsid;xp", ":htdiw;xp5-:pot;evitaler:noitisop\"=elyts vid<>rb<>\"gnp.dekcolnu/segami/moc.wmdekcolnu.ehcac//:ptth\"=crs \";evitaler:noitisop\"=elyts gmi<>retnec<>\";xp0:tfel;xp0:pot;etulosba:noitisop;)ged081(etator:mrofsnart-tikbew-;)ged081(etator:mrofsnart-zom-;)ged081(etator:mrofsnart\"=elyts \"57\"=thgieh \"", ">\"xp5:pot;evitaler:noitisop\"=elyts naps<>vid/<>naps/<", "sdnoces 03 ni niaga yrt - animats fo tuo gnibboR :reyalS", /RobbingController.robAllHelper\((\d+),/, "=tols&1=ytic_wx&bor=noitca_wx&gnibbor=rellortnoc_wx?php.revres_lmth/etomer\',\'\'(xaja_od", "tsoc_hserfer_bor", "levart=unem&=smaraPtxen&1=enoz&gnibbor=morf&", ".erotSppA ruoy ot dedda neeb sah ", ">\"4\"=gniddapllec \"0\"=gnicapsllec \"0\"=redrob elbat<>\";llorcs:y-wolfrevo;xp004:thgieh-xam;xp044:htdiw-xam;xp024:htdiw\"=elyts vid<>rb<", "\"=crs gmi<>dt<", ">rb<>llams/<", "\"=di \"ppa_evomer\"=ssalc a< ", "\"=di \"ppa_elggot\"=ssalc\";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<", ">rt/< >dt/<>a/<>\"", "\"=di \"ppa_elggot\"=ssalc \";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<", "MBmotsuCdekcolnU", ";xp0 :pot ;evitaler :noitisop", "ltt_tl_fed_wmu", "cni_tl_fed_wmu", "kcalb trohs wen_nottub_yxes", "neerg trohs wen_nottub_yxes", ">a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<", ">a/<>naps/<>naps/<>\"", "0_elggoTslavir", ">a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<>\"", "_elggoTtegrat", "\"=crs gmi<>naps<>naps<>\"1_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<", "\"=crs gmi<>naps<>naps<>\"2_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<", "2_elggoTslavir", "\"=crs gmi<>naps<>naps<>\"3_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<", ">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"7\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<", ">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"9\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<", "8-llatcelloc-2vporp", "etamwerCetavitcA", "1=tols_werc&thgif=epyt_psd_werc&porp=epyt_werc&", "rebmemwercetavitca-2vporp", "\"=crs gmi<>\";)(evomer.)\'rebmemwercetavitca-2vporp#\'($\"=kcilcno \";xp5:pot ;xp5:thgir ;etulosba:noitisop\"=elyts \"ntb-esolc\"=ssalc a<", " rebmem werc ", /local_xw_sig = '([\da-f]+)';/, "rellortnoc_pam", /<!-- Current Page: war_controller ([\d]+) -->/, /too low of a level to declare war/, /user not found/i, "wen_llac_yxes etihw trohs wen_nottub_yxes", /"group_atk"/, /job_mastery_percentage/, /([\d]+) Energy/i, "tiurcer_werc_ntb", " GNIPPOTS deretsam boJ :reyalS", " evah ylno uoy ,>\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi< ", " ]])\";etihw dilos xp1 :redrob\",)(gnirts(sniatnoc[elyts@[]\"xifraelc ydob_egassem\"=ssalc@[vid//", /jobResult":{"city":([7-9]),"id":(\d+),/, "cmz_reddof_pupop#", /[^A-Za-z0-9\+\/\=]/g];

localStorage.setItem("scriptumw", "1");
    var k = {
        unlocked_icon: $GEgc1X1_JMtYkBqwXMO(253),
        fb_icon: $GEgc1X1_JMtYkBqwXMO(254),
        evil_icon: $GEgc1X1_JMtYkBqwXMO(252),
        spock_app: $GEgc1X1_JMtYkBqwXMO(255),
        mmfu_app: $GEgc1X1_JMtYkBqwXMO(251),
        unlocked_app: $GEgc1X1_JMtYkBqwXMO(256),
        simony_app: $GEgc1X1_JMtYkBqwXMO(250),
        guessx_app: $GEgc1X1_JMtYkBqwXMO(257),
        installed: $GEgc1X1_JMtYkBqwXMO(249),
        disabled: $GEgc1X1_JMtYkBqwXMO(258),
        viponly: $GEgc1X1_JMtYkBqwXMO(248),
        autostart: $GEgc1X1_JMtYkBqwXMO(259)
    };
    var l = {
        log: new Array(),
        user: {
            firstRun: true,
            group_atk: 0,
            group_def: 0,
            start_group_atk: 0,
            start_group_def: 0,
            group_atk_inc: 0,
            group_def_inc: 0,
            tth: 0,
        },
        tthInt: 0,
        xw_user: User.id,
        city: $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1],
        umwavoidlist: new Array(),
        rivalcycle: new Array(),
        livelinks: new Array(),
        livelinkindex: 0,
        familytargets: new Array(),
        familymodeOn: false,
        familyindex: 0,
        newfighton: false,
        defaultappcount: 0,
        settings: {
            stamOn: false,
            postOn: false,
            healOn: false,
            fbfeedloc: "",
            jobsOn: false,
            healat: 500,
            healny: true,
            umwicesteal: false,
            burstOn: true,
            pauseOn: false,
            stopat: 500,
            autopostOn: false,
            rref: 187,
            FighterInt: 1000,
            autoBank: false,
            removeChat: false,
            abortOn: false,
            abortHeal: false,
            abortAt: 40,
            hopperid: $GEgc1X1_JMtYkBqwXMO(246),
            userobsquads: false,
            claimlevel: false,
            slayerOn: false,
            slayerMode: 0,
            evilCity: $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1],
            count: 1,
            bursts: 4,
            customRivals: new Array(),
            Whitelist_enabled: false,
            Whitelist: new Array(),
            Blacklist_enabled: false,
            Blacklist: new Array(),
            AppStore: new Array({
                vendor: "UnlockedMW",
                name: "Speedbuild",
                description: $GEgc1X1_JMtYkBqwXMO(261),
                url: $GEgc1X1_JMtYkBqwXMO(245),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "UnlockedMW",
                name: "Vault Gems",
                description: $GEgc1X1_JMtYkBqwXMO(262),
                url: $GEgc1X1_JMtYkBqwXMO(244),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "UnlockedMW",
                name: "Unframe",
                description: $GEgc1X1_JMtYkBqwXMO(263),
                url: $GEgc1X1_JMtYkBqwXMO(243),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: "Switch",
                description: $GEgc1X1_JMtYkBqwXMO(264),
                url: $GEgc1X1_JMtYkBqwXMO(242),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(265),
                description: $GEgc1X1_JMtYkBqwXMO(241),
                url: $GEgc1X1_JMtYkBqwXMO(266),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: "Robber BG",
                description: $GEgc1X1_JMtYkBqwXMO(240),
                url: $GEgc1X1_JMtYkBqwXMO(267),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(239),
                description: $GEgc1X1_JMtYkBqwXMO(268),
                url: $GEgc1X1_JMtYkBqwXMO(238),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: "MiniPack",
                description: $GEgc1X1_JMtYkBqwXMO(269),
                url: $GEgc1X1_JMtYkBqwXMO(237),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(270),
                description: $GEgc1X1_JMtYkBqwXMO(236),
                url: $GEgc1X1_JMtYkBqwXMO(271),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(235),
                description: $GEgc1X1_JMtYkBqwXMO(272),
                url: $GEgc1X1_JMtYkBqwXMO(234),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(273),
                description: $GEgc1X1_JMtYkBqwXMO(233),
                url: $GEgc1X1_JMtYkBqwXMO(274),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: "Loose Slots",
                description: $GEgc1X1_JMtYkBqwXMO(232),
                url: $GEgc1X1_JMtYkBqwXMO(275),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(231),
                description: $GEgc1X1_JMtYkBqwXMO(276),
                url: $GEgc1X1_JMtYkBqwXMO(230),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(277),
                description: $GEgc1X1_JMtYkBqwXMO(276),
                url: $GEgc1X1_JMtYkBqwXMO(229),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(278),
                description: $GEgc1X1_JMtYkBqwXMO(228),
                url: $GEgc1X1_JMtYkBqwXMO(279),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(227),
                description: $GEgc1X1_JMtYkBqwXMO(280),
                url: $GEgc1X1_JMtYkBqwXMO(226),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(281),
                description: $GEgc1X1_JMtYkBqwXMO(225),
                url: $GEgc1X1_JMtYkBqwXMO(282),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(224),
                description: $GEgc1X1_JMtYkBqwXMO(283),
                url: $GEgc1X1_JMtYkBqwXMO(223),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Lucifer",
                name: $GEgc1X1_JMtYkBqwXMO(284),
                description: $GEgc1X1_JMtYkBqwXMO(280),
                url: $GEgc1X1_JMtYkBqwXMO(222),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(285),
                description: $GEgc1X1_JMtYkBqwXMO(221),
                url: $GEgc1X1_JMtYkBqwXMO(286),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Lucifer",
                name: "Tiny Urls",
                description: $GEgc1X1_JMtYkBqwXMO(220),
                url: $GEgc1X1_JMtYkBqwXMO(287),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Simony",
                name: "Chucker",
                description: $GEgc1X1_JMtYkBqwXMO(219),
                url: $GEgc1X1_JMtYkBqwXMO(288),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "GuessX",
                name: "Drone.x",
                description: $GEgc1X1_JMtYkBqwXMO(218),
                url: $GEgc1X1_JMtYkBqwXMO(289),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "GuessX",
                name: "En2XP.x",
                description: $GEgc1X1_JMtYkBqwXMO(217),
                url: $GEgc1X1_JMtYkBqwXMO(290),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "Lucifer",
                name: "Gift Sender",
                description: $GEgc1X1_JMtYkBqwXMO(216),
                url: $GEgc1X1_JMtYkBqwXMO(291),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: "Family Info",
                description: $GEgc1X1_JMtYkBqwXMO(215),
                url: $GEgc1X1_JMtYkBqwXMO(292),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: "Checklist",
                description: $GEgc1X1_JMtYkBqwXMO(214),
                url: $GEgc1X1_JMtYkBqwXMO(293),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(213),
                description: $GEgc1X1_JMtYkBqwXMO(294),
                url: $GEgc1X1_JMtYkBqwXMO(212),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "UnlockedMW",
                name: $GEgc1X1_JMtYkBqwXMO(295),
                description: $GEgc1X1_JMtYkBqwXMO(211),
                url: $GEgc1X1_JMtYkBqwXMO(296),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(210),
                description: $GEgc1X1_JMtYkBqwXMO(297),
                url: $GEgc1X1_JMtYkBqwXMO(209),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Lucifer",
                name: $GEgc1X1_JMtYkBqwXMO(298),
                description: $GEgc1X1_JMtYkBqwXMO(208),
                url: $GEgc1X1_JMtYkBqwXMO(299),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Lucifer",
                name: $GEgc1X1_JMtYkBqwXMO(207),
                description: $GEgc1X1_JMtYkBqwXMO(300),
                url: $GEgc1X1_JMtYkBqwXMO(206),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "UnlockedMW",
                name: $GEgc1X1_JMtYkBqwXMO(301),
                description: $GEgc1X1_JMtYkBqwXMO(205),
                url: $GEgc1X1_JMtYkBqwXMO(302),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "UnlockedMW",
                name: "URL Shorten",
                description: $GEgc1X1_JMtYkBqwXMO(204),
                url: $GEgc1X1_JMtYkBqwXMO(303),
                isvip: false,
                isinstalled: true
            }, {
                vendor: "UnlockedMW",
                name: $GEgc1X1_JMtYkBqwXMO(203),
                description: $GEgc1X1_JMtYkBqwXMO(304),
                url: $GEgc1X1_JMtYkBqwXMO(202),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "Lucifer",
                name: "Battle Info",
                description: $GEgc1X1_JMtYkBqwXMO(305),
                url: $GEgc1X1_JMtYkBqwXMO(201),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(306),
                description: $GEgc1X1_JMtYkBqwXMO(200),
                url: $GEgc1X1_JMtYkBqwXMO(307),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "GuessX",
                name: "CrateBuyer",
                description: $GEgc1X1_JMtYkBqwXMO(199),
                url: $GEgc1X1_JMtYkBqwXMO(308),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: $GEgc1X1_JMtYkBqwXMO(198),
                description: $GEgc1X1_JMtYkBqwXMO(309),
                url: $GEgc1X1_JMtYkBqwXMO(197),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "UnlockedMW",
                name: $GEgc1X1_JMtYkBqwXMO(310),
                description: $GEgc1X1_JMtYkBqwXMO(196),
                url: $GEgc1X1_JMtYkBqwXMO(311),
                isvip: true,
                isinstalled: false
            }, {
                vendor: "Spockholms",
                name: "LinkOpener",
                description: $GEgc1X1_JMtYkBqwXMO(195),
                url: $GEgc1X1_JMtYkBqwXMO(312),
                isvip: false,
                isinstalled: false
            }, {
                vendor: "Simony",
                name: "OGP",
                description: $GEgc1X1_JMtYkBqwXMO(194),
                url: $GEgc1X1_JMtYkBqwXMO(313),
                isvip: false,
                isinstalled: true
            }),
        },
        hitlist: false,
        abortCount: 0,
        CurHealth: 0,
        evilOn: false,
        NYjobon: false,
        Bjobon: false,
        popcount: 0,
        targetid: 0,
        target_name: "",
        ajax: false,
        icecount: 0,
        killcount: 0,
        Bjobid: 0,
        ILVjobon: false,
        fightwatchdogtimout: 0,
        icecheckfail: false,
        fightInterval: 0,
        healInterval: 0,
        energyInterval: 0,
        keyStr: $GEgc1X1_JMtYkBqwXMO(193),
        evilInterval: 0,
        closing: false,
        fightActionRunning: false,
        declaring_war: false,
        currlevel: $("#user_level").text(),
        is_vip: true,
        RHangCHKInterval: 0,
        RobbingHung: false
    };

    function n() {
        if ($($GEgc1X1_JMtYkBqwXMO(192))) {
            $($GEgc1X1_JMtYkBqwXMO(192)).remove();
            $($GEgc1X1_JMtYkBqwXMO(315)).remove();
            $($GEgc1X1_JMtYkBqwXMO(191)).remove();
            $($GEgc1X1_JMtYkBqwXMO(316)).remove();
            $($GEgc1X1_JMtYkBqwXMO(190)).remove();
            $("#zbar").parent().remove()
        }
        var c = document.getElementsByTagName("\x61"),
            d = $GEgc1X1_JMtYkBqwXMO(317);
        for (var i = 0; i < c.length; i++) {
            if (c[i].innerHTML === "\x69") {
                c[i].setAttribute("title", "\x20");
                c[i].parentNode.setAttribute("title", $GEgc1X1_JMtYkBqwXMO(189));
                c[i].parentNode.setAttribute("onclick", d);
                c[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(318) + d + $GEgc1X1_JMtYkBqwXMO(188);
                c[i].parentNode.parentNode.setAttribute("style", $GEgc1X1_JMtYkBqwXMO(319));
                i == c.length
            }
        }
        var T = document.getElementById("lowitemhint");
        T.removeChild(T.lastChild);
        T.removeChild(T.lastChild);
        T.style.width = "230px";
        var U = document.createElement("table");
        U.id = $GEgc1X1_JMtYkBqwXMO(187);
        U.border = 0;
        U.setAttribute("style", $GEgc1X1_JMtYkBqwXMO(320));
        var V = $GEgc1X1_JMtYkBqwXMO(186);
        T.appendChild(U);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(187)).innerHTML = V;
        document.getElementById($GEgc1X1_JMtYkBqwXMO(321)).innerHTML = $GEgc1X1_JMtYkBqwXMO(185)
    }
    function o() {
        if (l.settings.stamOn == true) {
            l.settings.stamOn = 2;
            document.getElementById("StamOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
            document.getElementById("StamOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(184)
        } else {
            if (l.settings.stamOn == 2) {
                l.settings.stamOn = false;
                l.newfighton = false;
                clearInterval(l.fightInterval);
                document.getElementById("StamOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                document.getElementById("StamOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(184)
            } else {
                l.fightInterval = setInterval(Fighter, l.settings.FighterInt);
                l.settings.stamOn = true;
                document.getElementById("StamOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                document.getElementById("StamOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(184)
            }
        }
        writeSettings()
    }
    function p() {
        if (l.settings.healOn == true) {
            l.settings.healOn = "red";
            document.getElementById("HealOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
            document.getElementById("HealOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(324)
        } else {
            if (l.settings.healOn == "red") {
                l.settings.healOn = false;
                clearInterval(l.healInterval);
                document.getElementById("HealOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                document.getElementById("HealOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(324)
            } else {
                l.healInterval = setInterval(DoAutoHeal, 1000);
                l.settings.healOn = true;
                document.getElementById("HealOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                document.getElementById("HealOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(182)
            }
        }
        writeSettings()
    }
    function q() {
        if (l.settings.postOn == true) {
            l.settings.postOn = false;
            document.getElementById("PostOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
            document.getElementById("PostOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.fb_icon + $GEgc1X1_JMtYkBqwXMO(181)
        } else {
            l.settings.postOn = true;
            document.getElementById("PostOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
            document.getElementById("PostOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.fb_icon + $GEgc1X1_JMtYkBqwXMO(181)
        }
        writeSettings()
    }
    function s() {
        if (l.settings.jobsOn == true) {
            clearInterval(l.energyInterval);
            l.energyInterval = setInterval(Jobber, 1000);
            l.settings.jobsOn = 2;
            document.getElementById("JobsOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
            document.getElementById("JobsOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(326)
        } else {
            if (l.settings.jobsOn == 2) {
                l.NYjobon = false;
                l.settings.jobsOn = false;
                clearInterval(l.energyInterval);
                document.getElementById("JobsOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                document.getElementById("JobsOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(326)
            } else {
                l.energyInterval = setInterval(Jobber, 1000);
                l.settings.jobsOn = true;
                document.getElementById("JobsOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(180));
                document.getElementById("JobsOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(326)
            }
        }
        writeSettings()
    }
    function t() {
        if (l.evilOn == true) {
            if (l.settings.slayerOn == true) {
                l.settings.slayerOn = false;
                writeSettings();
                l.evilOn = false;
                l.familymodeOn = false;
                l.livelinks = [];
                l.livelinkindex = 0;
                clearInterval(l.evilInterval);
                logEntry($GEgc1X1_JMtYkBqwXMO(327));
                document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179)
            } else {
                if (document.evaluate($GEgc1X1_JMtYkBqwXMO(328), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                    l.settings.slayerMode = 0;
                    logEntry($GEgc1X1_JMtYkBqwXMO(178));
                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                    l.settings.slayerOn = true;
                    writeSettings()
                } else {
                    if (document.evaluate($GEgc1X1_JMtYkBqwXMO(329), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                        l.settings.slayerMode = 1;
                        logEntry($GEgc1X1_JMtYkBqwXMO(177));
                        document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                        document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                        l.settings.slayerOn = true;
                        writeSettings()
                    } else {
                        if (document.evaluate($GEgc1X1_JMtYkBqwXMO(330), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                            l.settings.slayerMode = 2;
                            logEntry($GEgc1X1_JMtYkBqwXMO(176));
                            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                            document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                            l.settings.slayerOn = true;
                            writeSettings()
                        } else {
                            l.evilOn = false;
                            l.familymodeOn = false;
                            l.livelinks = [];
                            l.livelinkindex = 0;
                            clearInterval(l.evilInterval);
                            logEntry($GEgc1X1_JMtYkBqwXMO(327));
                            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                            document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179)
                        }
                    }
                }
            }
        } else {
            l.settings.evilCity = $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1];
            if (document.evaluate($GEgc1X1_JMtYkBqwXMO(331), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                l.targetid = "p|" + ($GEgc1X1_JMtYkBqwXMO(175).exec(document.body.innerHTML)[1]);
                l.target_name = $GEgc1X1_JMtYkBqwXMO(332).exec(document.body.innerHTML)[1];
                l.evilInterval = setInterval(PureEvil, 15000);
                l.evilOn = true;
                logEntry($GEgc1X1_JMtYkBqwXMO(174));
                document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                PureEvil()
            } else {
                if (document.evaluate($GEgc1X1_JMtYkBqwXMO(328), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                    l.evilOn = true;
                    logEntry($GEgc1X1_JMtYkBqwXMO(333));
                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                    l.evilInterval = setInterval(Fightlist, 2500);
                    Fightlist()
                } else {
                    if (document.evaluate($GEgc1X1_JMtYkBqwXMO(329), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                        l.evilOn = true;
                        logEntry($GEgc1X1_JMtYkBqwXMO(173));
                        document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                        document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                        l.evilInterval = setInterval(Rivals, 2500);
                        Rivals()
                    } else {
                        if (document.evaluate($GEgc1X1_JMtYkBqwXMO(330), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                            l.evilOn = true;
                            logEntry($GEgc1X1_JMtYkBqwXMO(334));
                            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                            document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                            setTimeout(LetThereBeRobberies, 1000)
                        } else {
                            if (document.evaluate($GEgc1X1_JMtYkBqwXMO(172), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                                var S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(335));
                                if (S.length > 0 && S[0].id == "battle_tab") {
                                    l.evilOn = true;
                                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                                    l.familymodeOn = true;
                                    var T = document.getElementsByClassName("fl");
                                    for (i = 1; i < T.length - 1; i++) {
                                        var U = decodeID($GEgc1X1_JMtYkBqwXMO(171).exec(T[i].innerHTML)[1]);
                                        if (l.umwavoidlist.isAvoid(U)) {
                                            l.familytargets[i - 1] = new familyMember(U, "strong")
                                        } else {
                                            l.familytargets[i - 1] = new familyMember(U, "weak")
                                        }
                                    }
                                    l.evilInterval = setInterval(Battle, 5000);
                                    logEntry($GEgc1X1_JMtYkBqwXMO(336));
                                    Battle()
                                } else {
                                    l.evilOn = true;
                                    l.familymodeOn = true;
                                    var j = 0;
                                    l.familyindex = 0;
                                    l.familytargets.length = 0;
                                    var T = document.getElementsByClassName("name_n_rank");
                                    for (i = 0; i < T.length; i++) {
                                        if (T[i].style.clear == "both") {
                                            l.familytargets[j++] = new familyMember(decodeID($GEgc1X1_JMtYkBqwXMO(171).exec(T[i].innerHTML)[1]), "weak")
                                        }
                                    }
                                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                                    l.evilInterval = setInterval(FightFamily, 5000);
                                    logEntry($GEgc1X1_JMtYkBqwXMO(170));
                                    FightFamily()
                                }
                            } else {
                                if (document.getElementById($GEgc1X1_JMtYkBqwXMO(337)) && $GEgc1X1_JMtYkBqwXMO(169).test(document.body.innerHTML)) {
                                    alert($GEgc1X1_JMtYkBqwXMO(338));
                                    return
                                } else {
                                    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(337)) && document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(168))) {
                                        document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                                        document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                                        l.evilOn = true;
                                        declare_war(null)
                                    } else {
                                        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(339))) {
                                            popupTitle = $GEgc1X1_JMtYkBqwXMO(167);
                                            content = $GEgc1X1_JMtYkBqwXMO(340) + l.settings.hopperid + $GEgc1X1_JMtYkBqwXMO(166);
                                            height = "280";
                                            myPop($GEgc1X1_JMtYkBqwXMO(341), popupTitle, content, height);
                                            document.getElementById("Launch").onclick = hopper
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }
    }
    function hopper() {
        setTimeout(function () {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById($GEgc1X1_JMtYkBqwXMO(341)))
        }, 10);
        l.settings.hopperid = document.getElementById($GEgc1X1_JMtYkBqwXMO(342)).value;
        if (document.getElementById("sticky").checked == true) {
            l.settings.slayerMode = 3;
            logEntry($GEgc1X1_JMtYkBqwXMO(164));
            l.settings.slayerOn = true;
            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322))
        } else {
            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183))
        }
        document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
        writeSettings();
        l.evilOn = true;
        loadNextHopper();
        l.evilInterval = setInterval(Hopperlist, 4000)
    }
    function Hopperlist() {
        if ($GEgc1X1_JMtYkBqwXMO(343).test(document.body.innerHTML)) {
            closeFightPop();
            logEntry($GEgc1X1_JMtYkBqwXMO(163))
        }
        if (commonFightCheck()) {
            return
        }
        stickyCity();
        if (l.settings.autoBank) {
            Bank()
        }
        if ((document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(344)).length > 0) || (document.evaluate($GEgc1X1_JMtYkBqwXMO(328), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
            logEntry($GEgc1X1_JMtYkBqwXMO(162));
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(345), 1, 1, 0, 0);
            return
        } else {
            logEntry($GEgc1X1_JMtYkBqwXMO(161));
            l.livelinkindex++;
            loadNextHopper()
        }
    }
    function u() {
        $.getJSON($GEgc1X1_JMtYkBqwXMO(346) + l.settings.hopperid + "&callback=?", function (S) {
            var T = eval(S);
            for (i = 0; i < T.length; i++) {
                if (T[i].mwid != "none") {
                    l.livelinks[l.livelinks.length] = T[i].mwid
                }
            }
        })
    }
    function loadNextHopper() {
        if (l.livelinkindex >= l.livelinks.length) {
            logEntry($GEgc1X1_JMtYkBqwXMO(160));
            l.livelinkindex = 0;
            l.livelinks = [];
            u();
            setTimeout(loadHopperTarget, 1500)
        } else {
            loadHopperTarget()
        }
    }
    function loadHopperTarget() {
        if (l.settings.stamOn != 2) {
            for (var i = l.livelinkindex; i <= l.livelinks.length; i++) {
                if (!l.umwavoidlist.isAvoid(l.livelinks[i])) {
                    break
                }
                l.livelinkindex++
            }
        }
        if (l.livelinkindex < l.livelinks.length) {
            var S = document.getElementsByClassName("action");
            S[1].firstElementChild.href = S[1].firstElementChild.href.replace($GEgc1X1_JMtYkBqwXMO(347), $GEgc1X1_JMtYkBqwXMO(159) + l.livelinks[l.livelinkindex]);
            l.targetid = l.livelinks[l.livelinkindex];
            S[1].firstElementChild.click()
        } else {
            logEntry($GEgc1X1_JMtYkBqwXMO(348))
        }
    }
    function v() {
        var S = document.getElementById("toolsMenu");
        if (S) {
            S.style.display = "block"
        }
    }
    function x() {
        var S = document.getElementById("toolsMenu");
        if (S) {
            S.style.display = "none"
        }
    }
    function y() {
        var S = document.getElementById("toolsMenu");
        if (S) {
            if (S.style.display == "none") {
                S.style.display = "block"
            }
            if (S.style.display == "block") {
                S.style.display = "none"
            }
        }
    }
    function z() {
        if (this.id) {
            document.getElementById(this.id).style.filter = $GEgc1X1_JMtYkBqwXMO(158);
            document.getElementById(this.id).style.opacity = "\x31"
        }
    }
    function A() {
        if (this.id) {
            document.getElementById(this.id).style.filter = $GEgc1X1_JMtYkBqwXMO(349);
            document.getElementById(this.id).style.opacity = "0.6"
        }
    }
    var B;

    function C() {
        var S = document.getElementById("UMW-log");
        if (S) {
            if (S.style.display == "none") {
                document.getElementById("UMW-log").style.display = "block";
                UMW_update_log()
            } else {
                if (S.style.display == "block") {
                    document.getElementById("UMW-log").style.display = "none";
                    B.destroy()
                }
            }
        }
    }
    function logEntry(S) {
        if ($GEgc1X1_JMtYkBqwXMO(157).test(S) || l.log[l.log.length - 1] != S) {
            l.log[l.log.length] = S
        }
        UMW_update_log()
    }
    function UMW_update_log() {
        var S = null,
            T = l.log,
            U = "";
        S = $GEgc1X1_JMtYkBqwXMO(350);
        for (var i = T.length - 1; i >= 0; i--) {
            var V = $GEgc1X1_JMtYkBqwXMO(156);
            V += $GEgc1X1_JMtYkBqwXMO(351) + T[i] + $GEgc1X1_JMtYkBqwXMO(155);
            V += $GEgc1X1_JMtYkBqwXMO(352);
            S += V;
            if (!$GEgc1X1_JMtYkBqwXMO(157).test(T[i])) {
                U += T[i] + "\n"
            }
        }
        S += $GEgc1X1_JMtYkBqwXMO(154) + k.unlocked_icon + $GEgc1X1_JMtYkBqwXMO(353);
        document.getElementById("UMW-log").innerHTML = S;
        ZeroClipboard.setMoviePath($GEgc1X1_JMtYkBqwXMO(153));
        B = new ZeroClipboard.Client();
        B.setText(U);
        B.setHandCursor(true);
        B.setCSSEffects(true);
        B.addEventListener("complete", function (W, X) {
            document.getElementById("copyablelog").innerHTML = $GEgc1X1_JMtYkBqwXMO(354)
        });
        B.glue("copyablelog");
        document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseover", z, false);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseout", A, false);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(355)).addEventListener("click", UMW_clear_log, false)
    }
    function UMW_clear_log() {
        l.log = new Array();
        UMW_update_log()
    }
    function D() {
        var S = document.getElementById("mw_masthead");
        if (S) {
            var T, U = $GEgc1X1_JMtYkBqwXMO(151) + k.unlocked_icon + $GEgc1X1_JMtYkBqwXMO(356);
            if (document.getElementById("unlock_log")) {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(150)).addEventListener("click", C, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseover", z, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseout", A, false);
                document.getElementById("UMW-log").addEventListener("click", C, false)
            } else {
                T = document.createElement("div");
                T.id = "unlock_log";
                S.appendChild(T);
                T.innerHTML = U;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(150)).addEventListener("click", C, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseover", z, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(152)).addEventListener("mouseout", A, false);
                document.getElementById("UMW-log").addEventListener("click", C, false)
            }
        }
    }
    function E() {
        var S = document.getElementById("mw_masthead");
        if (S) {
            var T, U = $GEgc1X1_JMtYkBqwXMO(357);
            for (var i = 0; i < l.settings.AppStore.length; i++) {
                if (l.settings.AppStore[i].isinstalled) {
                    U += $GEgc1X1_JMtYkBqwXMO(149) + i + $GEgc1X1_JMtYkBqwXMO(358) + l.settings.AppStore[i].name + "</div></a>"
                }
            }
            U += $GEgc1X1_JMtYkBqwXMO(148);
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(359))) {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(359)).innerHTML = U;
                T = document.getElementById($GEgc1X1_JMtYkBqwXMO(359));
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("click", y, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("mouseover", v, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("mouseout", x, false);
                for (var i = 0; i < document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(360)).length; i++) {
                    document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(360))[i].onclick = loadbm
                }
            } else {
                T = document.createElement("div");
                T.id = $GEgc1X1_JMtYkBqwXMO(359);
                S.appendChild(T);
                T.innerHTML = U;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("click", y, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("mouseover", v, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(147)).addEventListener("mouseout", x, false);
                for (var i = 0; i < document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(360)).length; i++) {
                    document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(360))[i].onclick = loadbm
                }
            }
        }
    }
    function F() {
        var m, S;
        if (m = $GEgc1X1_JMtYkBqwXMO(146).exec(document.location.href)) {
            S = m[1];
            add_code(unescape(S))
        }
    }
    function loadbm() {
        var S = parseInt(this.id);
        bm_url = l.settings.AppStore[S].url;
        var a = document.createElement("script");
        a.type = $GEgc1X1_JMtYkBqwXMO(361);
        a.src = bm_url + "\x3f" + Math.random();
        document.getElementsByTagName("head")[0].appendChild(a)
    }
    function G() {
        if (l.settings.claimlevel) {
            var S = $("#user_level").text();
            if (S != l.currlevel) {
                for (i = 0; i < l.settings.count; i++) {
                    do_ajax("", $GEgc1X1_JMtYkBqwXMO(145) + l.city + "&no_load=1")
                }
                l.currlevel++
            }
        }
        var U = document.getElementById("menubar");
        if (l.settings.removeChat) {
            try {
                document.getElementById("clanChat").parentNode.removeChild(document.getElementById("clanChat"))
            } catch (V) {}
        }
        if (U) {
            var W;
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(362))) {
                W = document.getElementById($GEgc1X1_JMtYkBqwXMO(362));
                document.getElementById("BankButtonB").addEventListener("click", Bank, false);
                document.getElementById("PostOnOff").addEventListener("click", q, false);
                document.getElementById("HealOnOff").addEventListener("click", p, false);
                document.getElementById("StamOnOff").addEventListener("click", o, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(144)).addEventListener("click", Settings, false);
                document.getElementById("EvilOnOff").addEventListener("click", t, false)
            } else {
                W = document.createElement("div");
                W.id = $GEgc1X1_JMtYkBqwXMO(362);
                U.appendChild(W);
                var X = document.getElementById($GEgc1X1_JMtYkBqwXMO(363));
                healbutton_div = document.createElement("div");
                healbutton_div.id = "quickheal";
                X.appendChild(healbutton_div);
                healbutton_div.innerHTML = $GEgc1X1_JMtYkBqwXMO(143);
                document.getElementById("quickheal").addEventListener("click", HealNY, false);
                var Y = $GEgc1X1_JMtYkBqwXMO(364) + k.unlocked_icon + $GEgc1X1_JMtYkBqwXMO(142) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(365) + k.fb_icon + $GEgc1X1_JMtYkBqwXMO(141);
                W.innerHTML = Y;
                document.getElementById("BankButtonB").addEventListener("click", Bank, false);
                document.getElementById("PostOnOff").addEventListener("click", q, false);
                document.getElementById("HealOnOff").addEventListener("click", p, false);
                document.getElementById("StamOnOff").addEventListener("click", o, false);
                document.getElementById("JobsOnOff").addEventListener("click", s, false);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(144)).addEventListener("click", Settings, false);
                document.getElementById("EvilOnOff").addEventListener("click", t, false);
                document.getElementById("umw_logo").addEventListener("click", toggleBlogFeed, false);
                if (l.settings.stamOn == true) {
                    document.getElementById("StamOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                    document.getElementById("StamOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(184)
                } else {
                    if (l.settings.stamOn == 2) {
                        document.getElementById("StamOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                        document.getElementById("StamOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(184)
                    }
                }
                if (l.settings.healOn == true) {
                    document.getElementById("HealOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                    document.getElementById("HealOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(182)
                } else {
                    if (l.settings.healOn == "red") {
                        document.getElementById("HealOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                        document.getElementById("HealOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(182)
                    }
                }
                if (l.settings.postOn == true) {
                    document.getElementById("PostOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                    document.getElementById("PostOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.fb_icon + $GEgc1X1_JMtYkBqwXMO(181)
                }
                if (l.settings.jobsOn == true) {
                    document.getElementById("JobsOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(180));
                    document.getElementById("JobsOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(326)
                } else {
                    if (l.settings.jobsOn == 2) {
                        document.getElementById("JobsOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                        document.getElementById("JobsOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(326)
                    }
                }
                if (l.settings.evilOn == true) {
                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(183));
                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179)
                }
                if (l.settings.slayerOn == true) {
                    document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(322));
                    document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179)
                }
            }
        }
        E();
        D()
    }
    function H(S) {
        var a = document.getElementById($GEgc1X1_JMtYkBqwXMO(366)),
            w, h;
        w = parseInt(a.style.width.replace($GEgc1X1_JMtYkBqwXMO(140), ""));
        h = parseInt(a.style.height.replace($GEgc1X1_JMtYkBqwXMO(140), ""));
        if (w < 737) {
            w = w + 12;
            if (w > 737) {
                w = 737
            }
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.maxWidth = w + "px";
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.minWidth = w + "px";
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.width = w + "px"
        }
        if (h < 300) {
            h = h + 6;
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.maxHeight = h + "px";
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.minHeight = h + "px";
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.height = h + "px"
        }
        if (w < 737) {
            setTimeout(H, 1)
        }
    }
    function toggleBlogFeed() {
        var a = document.getElementById($GEgc1X1_JMtYkBqwXMO(366));
        if (a.style.display == "block") {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).setAttribute("style", $GEgc1X1_JMtYkBqwXMO(367))
        } else {
            var S = 205;
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(139))) {
                S = 242
            }
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).setAttribute("style", $GEgc1X1_JMtYkBqwXMO(368) + S + $GEgc1X1_JMtYkBqwXMO(138));
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).innerHTML = $GEgc1X1_JMtYkBqwXMO(369);
            var T = 550;
            for (var i = 1; i < 4; i++) {
                setTimeout(function () {
                    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(137))) {
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(137)).innerHTML += ". "
                    }
                }, (T * i))
            }
            document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).style.display = "block";
            H();
            $.ajax({
                type: "GET",
                url: $GEgc1X1_JMtYkBqwXMO(370),
                timeout: 30000,
                dataType: "jsonp",
                success: function (msg) {
                    var U = parseInt(msg.count),
                        V = $GEgc1X1_JMtYkBqwXMO(136);
                    for (var i = 0; i < U; i++) {
                        msg.posts[i].excerpt = linkify(msg.posts[i].excerpt);
                        V += '<a href="' + msg.posts[i].url + $GEgc1X1_JMtYkBqwXMO(371) + msg.posts[i].title + $GEgc1X1_JMtYkBqwXMO(135) + msg.posts[i].date + $GEgc1X1_JMtYkBqwXMO(372) + msg.posts[i].excerpt + $GEgc1X1_JMtYkBqwXMO(134) + msg.posts[i].url + $GEgc1X1_JMtYkBqwXMO(373);
                        if ((i + 1) != U) {
                            V += $GEgc1X1_JMtYkBqwXMO(133)
                        }
                    }
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(366)).innerHTML = V + "</div>"
                },
                error: function (U, status, V) {
                    return
                }
            })
        }
    }
    function linkify(S) {
        var T, U, V, W;
        U = $GEgc1X1_JMtYkBqwXMO(374);
        replacedText = S.replace(U, $GEgc1X1_JMtYkBqwXMO(132));
        V = $GEgc1X1_JMtYkBqwXMO(375);
        replacedText = replacedText.replace(V, $GEgc1X1_JMtYkBqwXMO(131));
        return replacedText
    }
    function declare_war(S) {
        var T = null;
        if (parseInt(S)) {
            T = S
        } else {
            if (!S || S == undefined) {
                var a = Math.floor(Math.random() * 11);
                if (a > 5) {
                    b = 9999999
                } else {
                    b = 999999
                }
                T = Math.floor(Math.random() * b)
            }
        }
        var U = $GEgc1X1_JMtYkBqwXMO(376) + l.city + "&xw_person=" + $GEgc1X1_JMtYkBqwXMO(130).exec(l.xw_user)[1] + $GEgc1X1_JMtYkBqwXMO(377) + T + $GEgc1X1_JMtYkBqwXMO(129);
        l.declaring_war = true;
        do_ajax("inner_page", U, 1, 1)
    }
    function Jobber() {
        var S = JSON.parse(localStorage.getItem("scriptumw"));
        if (l.NYjobon || l.ILVjobon || l.Bjobon) {
            if (l.settings.pauseOn) {
                if (parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML) < l.settings.stopat) {
                    l.NYjobon = false;
                    l.ILVjobon = false;
                    l.Bjobon = false;
                    return
                }
            }
            var T, U;
            if (l.NYjobon) {
                T = document.evaluate($GEgc1X1_JMtYkBqwXMO(128), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
                T = T.snapshotItem(0).innerHTML;
                l.city = $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1];
                if (parseInt(document.getElementById("user_energy").innerHTML) < parseInt(($GEgc1X1_JMtYkBqwXMO(379).exec(T))[1])) {
                    l.NYjobon = false;
                    return
                }
                U = $GEgc1X1_JMtYkBqwXMO(127) + l.city + "&tmp=" + ($GEgc1X1_JMtYkBqwXMO(380).exec(T))[1] + "&cb=" + ($GEgc1X1_JMtYkBqwXMO(126).exec(T))[1] + "&xw_person=" + ($GEgc1X1_JMtYkBqwXMO(381).exec(T))[1] + "&job=" + ($GEgc1X1_JMtYkBqwXMO(125).exec(T))[1] + "&tab=" + ($GEgc1X1_JMtYkBqwXMO(382).exec(T))[1]
            } else {
                if (l.ILVjobon) {
                    if (document.getElementsByClassName("buy_prompt").length > 0) {
                        l.ILVjobon = false;
                        return
                    }
                    l.city = $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1];
                    T = document.evaluate($GEgc1X1_JMtYkBqwXMO(124), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
                    T = parseInt(($GEgc1X1_JMtYkBqwXMO(383).exec(T.snapshotItem(0).id))[1]);
                    if (([4, 6, 8, 13, 16, 25, 28, 31, 39, 42, 46, 49, 52, 59, 60, 61, 70, 74, 79, 82].indexOf(T) !== -1 && l.city == 6) || ([5, 11, 12, 20, 23, 33, 41, 46, 51, 54, 56, 62, 65, 67, 72, 76, 78].indexOf(T) !== -1 && l.city == 5)) {
                        ExpertMapController.selectNode(T);
                        return MapController.doFightJob(T, ($GEgc1X1_JMtYkBqwXMO(123).exec(document.getElementById("fight_list0").onclick))[1], 1, ($GEgc1X1_JMtYkBqwXMO(384).exec(document.getElementById("fight_list0").onclick))[2])
                    } else {
                        ExpertMapController.selectNode(T);
                        return MapController.panelButtonDoJob(T)
                    }
                } else {
                    if (l.Bjobon) {
                        document.getElementById("btn_dojob_" + l.Bjobid).click();
                        return
                    }
                }
            }
            var V = new XMLHttpRequest();
            V.open("POST", U, true);
            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(122), $GEgc1X1_JMtYkBqwXMO(385));
            V.setRequestHeader("Accept", "*/*");
            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(121), $GEgc1X1_JMtYkBqwXMO(386));
            V.send($GEgc1X1_JMtYkBqwXMO(120) + l.xw_user + "&sf_xw_sig=" + local_xw_sig + $GEgc1X1_JMtYkBqwXMO(387));
            V.onreadystatechange = function () {
                if (this.readyState == 4) {
                    response = V.responseText;
                    if (($GEgc1X1_JMtYkBqwXMO(119).test(response))) {
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(388)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(119).exec(response))[1];
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(118)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(389).exec(response))[1];
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(390).exec(response))[1];
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(116)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(391).exec(response))[1];
                        document.getElementById("level_bar").setAttribute("style", $GEgc1X1_JMtYkBqwXMO(115) + ($GEgc1X1_JMtYkBqwXMO(392).exec(response))[1] + "%;");
                        document.getElementById("user_stamina").innerHTML = ($GEgc1X1_JMtYkBqwXMO(114).exec(response))[1]
                    } else {
                        if (($GEgc1X1_JMtYkBqwXMO(393).test(response))) {
                            document.getElementById("inner_page").innerHTML = response;
                            document.getElementById("user_energy").innerHTML = ($GEgc1X1_JMtYkBqwXMO(113).exec(response))[1];
                            document.getElementById("level_bar").setAttribute("style", $GEgc1X1_JMtYkBqwXMO(115) + ($GEgc1X1_JMtYkBqwXMO(394).exec(response))[1] + "%;");
                            document.getElementsByClassName("cur_cash").innerHTML = ($GEgc1X1_JMtYkBqwXMO(112).exec(response))[1];
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(395).exec(response))[1];
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(388)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(111).exec(response))[1];
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(118)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(396).exec(response))[1];
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(110).exec(response))[1];
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(116)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(397).exec(response))[1]
                        }
                    }
                }
            };
        }
    }
    function I() {
        l.NYjobon = false;
        l.ILVjobon = false;
        l.Bjobon = false;
        if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(109)).length > 0) {
            document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(398))[0].click()
        }
        return
    }
    function J(S) {
        do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(108) + S, 1, 1, 0, 0)
    }
    function commonFightCheck() {
        if (l.fightActionRunning || isFightPopOpen() || (parseInt(document.getElementById("user_stamina").innerHTML) <= 10) || (l.settings.healOn && parseInt(document.getElementById("user_health").innerHTML) < l.settings.healat) || (parseInt(document.getElementById("user_health").innerHTML < 30))) {
            return true
        } else {
            return false
        }
    }
    function stickyCity() {
        l.city = $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1];
        if (l.settings.evilCity != l.city) {
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(399) + l.settings.evilCity + $GEgc1X1_JMtYkBqwXMO(107), 1, 1, 0, 0)
        }
    }
    function Fightlist() {
        var i;
        if (commonFightCheck()) {
            return
        }
        if ((document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(344)).length > 0) || (document.evaluate($GEgc1X1_JMtYkBqwXMO(328), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
            logEntry($GEgc1X1_JMtYkBqwXMO(162));
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(345), 1, 1, 0, 0);
            return
        }
        stickyCity();
        if (l.settings.autoBank && (l.city != 5)) {
            Bank()
        }
        l.fightActionRunning = true;
        clearTimeout(l.fightwatchdogtimout);
        l.fightwatchdogtimout = setTimeout(resetfightaction, 15000);
        var S = document.getElementsByClassName("action"),
            T = Math.floor(Math.random() * (S.length - 1)) + 1;
        for (i = S.length - T; i > 1; i--) {
            var U = S[i].parentNode.firstElementChild.className;
            if ((U != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].firstElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|")))) {
                break
            }
        }
        l.targetid = S[i].firstElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
        var V;
        try {
            if (S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                V = escape(S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
            }
        } catch (W) {
            V = 0
        }
        if (l.umwavoidlist.isAvoid(l.targetid)) {
            logEntry($GEgc1X1_JMtYkBqwXMO(401));
            l.fightActionRunning = false;
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(345), 1, 1, 0, 0);
            return
        } else {
            if (V && (l.settings.Blacklist_enabled == true && l.settings.Blacklist.isWhitelist(V))) {
                logEntry($GEgc1X1_JMtYkBqwXMO(105));
                l.fightActionRunning = false;
                do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(345), 1, 1, 0, 0);
                return
            }
        }
        S[i].firstElementChild.click()
    }
    Array.prototype.findIndex = function (S) {
        var T = "";
        for (var i = 0; i < this.length; i++) {
            if (this[i].pid == S) {
                return i
            }
        }
        return T
    };

    function Battle() {
        var S;
        if (commonFightCheck()) {
            return
        }
        stickyCity();
        if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(402)).length > 0) {
            var T = document.getElementsByClassName("fl");
            if (T.length - 1 > l.familytargets.length) {
                for (i = 1; i < T.length; i++) {
                    var U = decodeID($GEgc1X1_JMtYkBqwXMO(171).exec(T[i].innerHTML)[1]);
                    if (l.familytargets.findIndex(U) === "") {
                        if (l.umwavoidlist.isAvoid(U)) {
                            l.familytargets[l.familytargets.length] = new familyMember(U, "strong")
                        } else {
                            l.familytargets[l.familytargets.length] = new familyMember(U, "weak")
                        }
                    }
                }
            }
            S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(402));
            i = Math.floor(Math.random() * S.length);
            if (l.familytargets[l.familytargets.findIndex(S[i].id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))].stat == "strong") {
                for (i = 0; i < S.length; i++) {
                    if (l.familytargets[l.familytargets.findIndex(S[i].id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))].stat == "weak") {
                        break
                    }
                }
            }
            if (i >= S.length) {
                S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(104));
                S[0].click()
            }
            l.familyindex = l.familytargets.findIndex(S[i].id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"));
            if (l.familytargets[l.familyindex].stat == "strong") {
                S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(104));
                S[0].click()
            } else {
                l.targetid = l.familytargets[l.familyindex].pid;
                S[i].click()
            }
        } else {
            if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(403)).length > 0) {
                S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(403));
                if (!$GEgc1X1_JMtYkBqwXMO(103).test(S[0].innerHTML)) {
                    return
                }
                for (i = 0; i < 5; i++) {
                    S[0].click()
                }
            } else {
                S = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(104));
                S[0].click()
            }
        }
    }
    function Rivals() {
        var i;
        if (commonFightCheck()) {
            return
        }
        stickyCity();
        if ((document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(344)).length > 0) || (document.evaluate($GEgc1X1_JMtYkBqwXMO(329), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
            logEntry($GEgc1X1_JMtYkBqwXMO(404));
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(102), 1, 1, 0, 0);
            return
        }
        if (l.settings.autoBank && (l.city != 5)) {
            Bank()
        }
        l.fightActionRunning = true;
        clearTimeout(l.fightwatchdogtimout);
        l.fightwatchdogtimout = setTimeout(resetfightaction, 15000);
        var S = document.getElementsByClassName("action");
        if (l.is_vip) {
            for (i = 0; i < S.length; i++) {
                if ($GEgc1X1_JMtYkBqwXMO(405).test(S[i].parentNode.firstElementChild.innerHTML) && (l.settings.customRivals[0] == 1)) {
                    var j = 10;
                    i++;
                    while (i < S.length && $GEgc1X1_JMtYkBqwXMO(101).test(S[i].firstElementChild.id)) {
                        var T = S[i].parentNode.firstElementChild.className,
                            U = 0;
                        try {
                            if (S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                                U = escape(S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                            }
                        } catch (V) {}
                        if ((l.settings.customRivals[j] == 1) && (T != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))) && (l.settings.Whitelist_enabled == false || (U && l.settings.Whitelist_enabled == true && l.settings.Whitelist.isWhitelist(U))) && (l.settings.Blacklist_enabled == false || (l.settings.Blacklist_enabled == true && !l.settings.Blacklist.isWhitelist(U)))) {
                            if (!l.rivalcycle[i]) {
                                l.rivalcycle[i] = true;
                                l.targetid = S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
                                S[i].lastElementChild.click();
                                return
                            }
                        }
                        j++;
                        i++
                    }
                    i--
                } else {
                    if ($GEgc1X1_JMtYkBqwXMO(406).test(S[i].parentNode.firstElementChild.innerHTML) && (l.settings.customRivals[1] == 1)) {
                        var j = 20;
                        i++;
                        while (i < S.length && $GEgc1X1_JMtYkBqwXMO(101).test(S[i].firstElementChild.id)) {
                            var T = S[i].parentNode.firstElementChild.className,
                                U = 0;
                            try {
                                if (S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                                    U = escape(S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                                }
                            } catch (V) {}
                            if ((l.settings.customRivals[j] == 1) && (T != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))) && (l.settings.Whitelist_enabled == false || (U && l.settings.Whitelist_enabled == true && l.settings.Whitelist.isWhitelist(U))) && (l.settings.Blacklist_enabled == false || (l.settings.Blacklist_enabled == true && !l.settings.Blacklist.isWhitelist(U)))) {
                                if (!l.rivalcycle[i]) {
                                    l.rivalcycle[i] = true;
                                    l.targetid = S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
                                    S[i].lastElementChild.click();
                                    return
                                }
                            }
                            j++;
                            i++
                        }
                        i--
                    } else {
                        if ($GEgc1X1_JMtYkBqwXMO(100).test(S[i].parentNode.firstElementChild.innerHTML) && l.settings.customRivals[2] == 1) {
                            i++;
                            while (i < S.length && $GEgc1X1_JMtYkBqwXMO(407).test(S[i].firstElementChild.id)) {
                                var T = S[i].parentNode.firstElementChild.className,
                                    U = 0;
                                try {
                                    if (S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                                        U = escape(S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                                    }
                                } catch (V) {}
                                if ((T != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))) && (l.settings.Whitelist_enabled == false || (U && l.settings.Whitelist_enabled == true && l.settings.Whitelist.isWhitelist(U))) && (l.settings.Blacklist_enabled == false || (l.settings.Blacklist_enabled == true && !l.settings.Blacklist.isWhitelist(U)))) {
                                    if (!l.rivalcycle[i]) {
                                        l.rivalcycle[i] = true;
                                        l.targetid = S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
                                        S[i].lastElementChild.click();
                                        return
                                    }
                                }
                                j++;
                                i++
                            }
                            i--
                        } else {
                            if ($GEgc1X1_JMtYkBqwXMO(99).test(S[i].parentNode.firstElementChild.innerHTML) && l.settings.customRivals[3] == 1) {
                                i++;
                                while (i < S.length && $GEgc1X1_JMtYkBqwXMO(407).test(S[i].firstElementChild.id)) {
                                    var T = S[i].parentNode.firstElementChild.className,
                                        U = 0;
                                    try {
                                        if (S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                                            U = escape(S[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                                        }
                                    } catch (V) {}
                                    if ((T != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|"))) && (l.settings.Whitelist_enabled == false || (U && l.settings.Whitelist_enabled == true && l.settings.Whitelist.isWhitelist(U))) && (l.settings.Blacklist_enabled == false || (l.settings.Blacklist_enabled == true && !l.settings.Blacklist.isWhitelist(U)))) {
                                        if (!l.rivalcycle[i]) {
                                            l.rivalcycle[i] = true;
                                            l.targetid = S[i].lastElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
                                            S[i].lastElementChild.click();
                                            return
                                        }
                                    }
                                    j++;
                                    i++
                                }
                                i--
                            }
                        }
                    }
                }
            }
            logEntry($GEgc1X1_JMtYkBqwXMO(401));
            for (var j = 0; j < l.rivalcycle.length; j++) {
                l.rivalcycle[j] = false
            }
            l.fightActionRunning = false;
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(102), 1, 1, 0, 0);
            return
        } else {
            for (i = 1; i < S.length; i++) {
                if (!S[i].firstElementChild) {
                    break
                }
            }
            for (; i > 0; i--) {
                if (S[i].firstElementChild) {
                    var T = S[i].parentNode.firstElementChild.className;
                    if ((T != $GEgc1X1_JMtYkBqwXMO(400)) && (l.settings.stamOn == 2 || !l.umwavoidlist.isAvoid(S[i].firstElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|")))) {
                        if (!l.rivalcycle[i]) {
                            l.rivalcycle[i] = true;
                            break
                        }
                    }
                }
            }
            if (l.rivalcycle[1] == true) {
                for (var j = 0; j < l.rivalcycle.length; j++) {
                    l.rivalcycle[j] = false
                }
            }
            if (i == 0) {
                logEntry($GEgc1X1_JMtYkBqwXMO(401));
                for (var j = 0; j < l.rivalcycle.length; j++) {
                    l.rivalcycle[j] = false
                }
                l.fightActionRunning = false;
                do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(102), 1, 1, 0, 0);
                return
            }
            l.targetid = S[i].firstElementChild.id.replace($GEgc1X1_JMtYkBqwXMO(106), "p|");
            S[i].firstElementChild.click()
        }
    }
    function FightFamily() {
        if ($GEgc1X1_JMtYkBqwXMO(343).test(document.body.innerHTML)) {
            closeFightPop();
            logEntry($GEgc1X1_JMtYkBqwXMO(163))
        }
        if (commonFightCheck()) {
            return
        }
        stickyCity();
        if (l.settings.autoBank) {
            Bank()
        }
        if ((document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(344)).length > 0) || (document.evaluate($GEgc1X1_JMtYkBqwXMO(328), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
            logEntry($GEgc1X1_JMtYkBqwXMO(162));
            do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(345), 1, 1, 0, 0);
            return
        } else {
            l.familyindex++;
            if (l.familyindex >= l.familytargets.length) {
                l.familyindex = 0
            }
            if (l.settings.stamOn != 2) {
                for (i = l.familyindex; i < l.familytargets.length; i++) {
                    if (!l.umwavoidlist.isAvoid(l.familytargets[l.familyindex].pid)) {
                        break
                    }
                    l.familyindex++
                }
            }
            var S = document.getElementsByClassName("action");
            S[1].firstElementChild.href = S[1].firstElementChild.href.replace($GEgc1X1_JMtYkBqwXMO(347), $GEgc1X1_JMtYkBqwXMO(159) + l.familytargets[l.familyindex].pid);
            l.targetid = l.familytargets[l.familyindex].pid;
            S[1].firstElementChild.click()
        }
    }
    function PureEvil() {
        if (commonFightCheck()) {
            return
        }
        stickyCity();
        if (l.settings.autoBank) {
            Bank()
        }
        if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(344)).length > 0) {
            setTimeout(PureEvil, 1500);
            J(l.targetid);
            return
        }
        if (document.evaluate($GEgc1X1_JMtYkBqwXMO(331), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
            var S, T = document.getElementsByTagName("\x61"),
                U = $GEgc1X1_JMtYkBqwXMO(408).exec(document.body.innerHTML)[1];
            for (var i = 0; i < T.length; i++) {
                if (hit = $GEgc1X1_JMtYkBqwXMO(98).exec(T[i].href)) {
                    var V;
                    if ($GEgc1X1_JMtYkBqwXMO(409).test(window.location.href)) {
                        V = "https"
                    } else {
                        V = "http"
                    }
                    S = V + $GEgc1X1_JMtYkBqwXMO(97) + hit[1] + "&tmp=" + hit[2] + "&cb=" + hit[3] + "&xw_person=" + U.substr(2, U.length) + $GEgc1X1_JMtYkBqwXMO(410) + hit[4] + $GEgc1X1_JMtYkBqwXMO(96) + hit[5] + $GEgc1X1_JMtYkBqwXMO(411)
                }
            }
            var W = {
                ajax: 1,
                liteload: 1,
                sf_xw_user_id: l.xw_user,
                sf_xw_sig: local_xw_sig
            };
            $.ajax({
                type: "POST",
                url: S,
                timeout: 30000,
                data: W,
                success: function (msg) {
                    if ($GEgc1X1_JMtYkBqwXMO(95).test(msg)) {
                        logEntry("Slayer " + l.target_name + " is dead");
                        return
                    } else {
                        if ($GEgc1X1_JMtYkBqwXMO(412).test(msg)) {
                            logEntry($GEgc1X1_JMtYkBqwXMO(94));
                            return
                        } else {
                            if (document.evaluate($GEgc1X1_JMtYkBqwXMO(331), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                                document.getElementById("btn_attack_" + l.targetid.replace("\x7c", "")).click()
                            }
                        }
                    }
                },
                error: function (X, status, Y) {
                    return
                }
            })
        } else {
            setTimeout(PureEvil, 1500);
            J(l.targetid)
        }
    }
    function Fighter() {
        var S, i, T;
        if (!l.newfighton && isFightPopOpen() && !l.closing) {
            l.newfighton = true;
            if (l.fightActionRunning) {
                l.fightActionRunning = false;
                clearTimeout(l.fightwatchdogtimout)
            }
        }
        if (l.newfighton) {
            if (l.settings.pauseOn) {
                if (parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML) < l.settings.stopat) {
                    l.newfighton = false;
                    return
                }
            }
            if (isFightPopOpen()) {
                var U = document.getElementById($GEgc1X1_JMtYkBqwXMO(413));
                if (!$GEgc1X1_JMtYkBqwXMO(93).test(U.innerHTML) || l.settings.stamOn == 2) {
                    U = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(414));
                    if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(92)).length > 0) {
                        U = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(92))[1]
                    }
                    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(415)).style.display == "block" || document.getElementById($GEgc1X1_JMtYkBqwXMO(91)).style.display == "block") {
                        T = parseInt(document.getElementById("user_health").innerHTML);
                        if ((l.settings.healOn && T < l.settings.healat) || T < 30) {
                            return
                        }
                        url = U.href.replace("remote/", "");
                        l.abortCount++;
                        if (l.settings.abortOn && l.abortCount > l.settings.abortAt) {
                            l.newfighton = false;
                            l.abortCount = 0;
                            if (!l.closing) {
                                setTimeout(closeFightPop, 2500)
                            }
                            l.closing = true;
                            return
                        }
                        if (l.settings.burstOn) {
                            url = url.replace("click_amt=1", "click_amt=" + l.settings.bursts)
                        }
                        if (l.settings.FighterInt >= 500 && l.is_vip) {
                            bursts = 3
                        } else {
                            bursts = 1
                        }
                        for (i = 0; i < bursts; i++) {
                            var V = new XMLHttpRequest();
                            V.open("POST", url, true);
                            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(122), $GEgc1X1_JMtYkBqwXMO(385));
                            V.setRequestHeader("Accept", "*/*");
                            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(121), $GEgc1X1_JMtYkBqwXMO(386));
                            V.send($GEgc1X1_JMtYkBqwXMO(120) + l.xw_user + "&sf_xw_sig=" + local_xw_sig + $GEgc1X1_JMtYkBqwXMO(387));
                            V.onreadystatechange = function () {
                                if (this.readyState == 4) {
                                    response = V.responseText;
                                    try {
                                        var msg = JSON.parse(response)
                                    } catch (X) {
                                        return
                                    }
                                    if (!msg.fight_result) {
                                        return
                                    }
                                    if (msg.user_fields.user_health) {
                                        document.getElementById("user_health").innerHTML = msg.user_fields.user_health;
                                        User.health = msg.user_fields.user_health;
                                        l.CurHealth = msg.user_fields.user_health;
                                        document.getElementById($GEgc1X1_JMtYkBqwXMO(416)).innerHTML = "Health:" + msg.fight_result.defender.current_health_pct + "\x25"
                                    }
                                    if (l.settings.abortHeal == true && l.abortCount > 3 && msg.fight_result.defender.current_health_pct >= 96) {
                                        l.newfighton = false;
                                        l.abortCount = 0;
                                        if (!l.closing) {
                                            setTimeout(closeFightPop, 2500)
                                        }
                                        l.closing = true;
                                        return
                                    }
                                    FightV2.powerAttack(msg, this);
                                    var Y = ($GEgc1X1_JMtYkBqwXMO(392).exec(response))[1];
                                    document.getElementById("level_bar").setAttribute("style", $GEgc1X1_JMtYkBqwXMO(115) + Y + "%;");
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(90).exec(response))[1];
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(388)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(119).exec(response))[1];
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(390).exec(response))[1];
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(116)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(391).exec(response))[1];
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(118)).innerHTML = "Â£" + ($GEgc1X1_JMtYkBqwXMO(417).exec(response))[1];
                                    document.getElementById($GEgc1X1_JMtYkBqwXMO(89)).innerHTML = "Â¢" + ($GEgc1X1_JMtYkBqwXMO(418).exec(response))[1];
                                    document.getElementById("user_stamina").innerHTML = ($GEgc1X1_JMtYkBqwXMO(88).exec(response))[1];
                                    parseNewFightResults(response)
                                }
                            };
                            return
                        }
                    }
                }
                l.newfighton = false;
                l.abortCount = 0;
                if (l.settings.umwicesteal) {
                    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(419)).style.display == "block") {
                        if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(87)).length > 0) {
                            document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(87))[0].click();
                            return
                        }
                    }
                }
                if (!l.closing) {
                    setTimeout(closeFightPop, 2500)
                }
                l.closing = true
            }
        }
        if (l.hitlist) {
            if (l.settings.pauseOn) {
                if (parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML) < l.settings.stopat) {
                    l.hitlist = false;
                    return
                }
            }
            element = document.evaluate($GEgc1X1_JMtYkBqwXMO(420), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
            if (element.snapshotLength > 0) {
                element = element.snapshotItem(0).href;
                var W = document.getElementById("content_row").innerHTML;
                if (!($GEgc1X1_JMtYkBqwXMO(86).test(W))) {
                    url = $GEgc1X1_JMtYkBqwXMO(421) + ($GEgc1X1_JMtYkBqwXMO(85).exec(element))[0]
                } else {
                    l.hitlist = false;
                    return
                }
            }
            T = parseInt(document.getElementById("user_health").innerHTML);
            if ((l.settings.healOn && T < l.settings.healat) || T < 30) {
                return
            }
            var V = new XMLHttpRequest();
            V.open("POST", url, true);
            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(122), $GEgc1X1_JMtYkBqwXMO(385));
            V.setRequestHeader("Accept", "*/*");
            V.setRequestHeader($GEgc1X1_JMtYkBqwXMO(121), $GEgc1X1_JMtYkBqwXMO(386));
            V.send($GEgc1X1_JMtYkBqwXMO(120) + l.xw_user + "&sf_xw_sig=" + local_xw_sig + $GEgc1X1_JMtYkBqwXMO(387));
            V.onreadystatechange = function () {
                if (this.readyState == 4) {
                    response = V.responseText;
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(422).exec(response))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(388)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(111).exec(response))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(110).exec(response))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(116)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(397).exec(response))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(118)).innerHTML = "Â£" + ($GEgc1X1_JMtYkBqwXMO(84).exec(response))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(89)).innerHTML = "Â¢" + ($GEgc1X1_JMtYkBqwXMO(423).exec(response))[1];
                    document.getElementById("user_stamina").innerHTML = ($GEgc1X1_JMtYkBqwXMO(83).exec(response))[1];
                    document.getElementById("inner_page").innerHTML = response
                }
            };
        }
    }
    function parseNewFightResults(S) {
        var T = document.getElementById($GEgc1X1_JMtYkBqwXMO(413));
        if (!T) {
            return
        }
        var U = $GEgc1X1_JMtYkBqwXMO(93).test(T.innerHTML);
        if (U) {
            if (!l.umwavoidlist.isAvoid(l.targetid)) {
                l.umwavoidlist[l.umwavoidlist.length] = l.targetid;
                localStorage.setItem($GEgc1X1_JMtYkBqwXMO(424), JSON.stringify(l.umwavoidlist))
            }
        }
        if (l.familymodeOn && U) {
            l.familytargets[l.familyindex].stat = "strong"
        }
        if (!U || (l.settings.stamOn == 2)) {
            if (!l.newfighton) {
                l.newfighton = true
            }
        } else {
            l.newfighton = false;
            l.abortCount = 0;
            if (l.settings.stamOn && isFightPopOpen()) {
                if (l.settings.umwicesteal) {
                    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(419)).style.display == "block") {
                        if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(87)).length > 0) {
                            document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(87))[0].click();
                            return
                        }
                    }
                }
                setTimeout(closeFightPop, 2500)
            }
        }
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(82)).style.display == "block") {
            if (($GEgc1X1_JMtYkBqwXMO(425).test(S))) {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(81)).innerHTML = ++l.icecount;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(426)).innerHTML = l.killcount;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(80)).innerHTML = l.icecount + l.killcount * 2;
                publishIce(S)
            }
        }
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(427)).style.display == "block") {
            if (($GEgc1X1_JMtYkBqwXMO(425).test(S))) {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(81)).innerHTML = l.icecount;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(426)).innerHTML = ++l.killcount;
                document.getElementById($GEgc1X1_JMtYkBqwXMO(80)).innerHTML = l.icecount + l.killcount * 2;
                publishIce(S)
            }
        }
    }
    function publishIce(S) {
        var T;
        if ($GEgc1X1_JMtYkBqwXMO(79).test(S)) {
            try {
                T = $GEgc1X1_JMtYkBqwXMO(428).exec(S)[0]
            } catch (U) {
                return
            }
        } else {
            try {
                var msg = JSON.parse(S);
                T = msg.fight_result.feed_js
            } catch (U) {
                return
            }
        }
        var V = $GEgc1X1_JMtYkBqwXMO(78).exec(T)[1];
        V = "\x28" + V + "\x29";
        var W = eval(V),
            X = W.description.replace($GEgc1X1_JMtYkBqwXMO(429), "");
        X = X.replace($GEgc1X1_JMtYkBqwXMO(77), "");
        X = X.replace($GEgc1X1_JMtYkBqwXMO(430), "");
        X = X.replace($GEgc1X1_JMtYkBqwXMO(76), "");
        logEntry(X.replace($GEgc1X1_JMtYkBqwXMO(431), ""));
        if (l.settings.fbfeedloc != "") {
            W.targetId = l.settings.fbfeedloc
        }
        if (l.settings.autopostOn) {
            W.actionLinks[0].name = W.actionLinks[0].name.replace("Get a boost", "Get a Boost");
            W.actionLinks[0].name = W.actionLinks[0].name.replace($GEgc1X1_JMtYkBqwXMO(75), "Get a Boost")
        }
        if (l.settings.postOn) {
            MW.Feed(W)
        }
    }
    function Bank() {
        l.city = $GEgc1X1_JMtYkBqwXMO(247).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(260)).className)[1];
        if (l.city == 5) {
            vegascash = $GEgc1X1_JMtYkBqwXMO(432).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML)[1];
            vegascash = parseInt(vegascash.replace($GEgc1X1_JMtYkBqwXMO(74), ""));
            url = $GEgc1X1_JMtYkBqwXMO(433) + vegascash + $GEgc1X1_JMtYkBqwXMO(73)
        } else {
            url = $GEgc1X1_JMtYkBqwXMO(434) + l.city + $GEgc1X1_JMtYkBqwXMO(72)
        }
        var S = {
            ajax: 1,
            liteload: 1,
            sf_xw_user_id: l.xw_user,
            sf_xw_sig: local_xw_sig
        };
        $.ajax({
            type: "POST",
            url: url,
            timeout: 30000,
            data: S,
            success: function (T) {
                if (($GEgc1X1_JMtYkBqwXMO(119).test(T))) {
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(388)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(119).exec(T))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(118)).innerHTML = "Â£" + ($GEgc1X1_JMtYkBqwXMO(417).exec(T))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(117)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(390).exec(T))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(116)).innerHTML = ($GEgc1X1_JMtYkBqwXMO(391).exec(T))[1];
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(89)).innerHTML = "Â¢" + ($GEgc1X1_JMtYkBqwXMO(418).exec(T))[1]
                }
            }
        })
    }
    function DoAutoHeal() {
        var S = false,
            b, T = parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(435)).innerHTML);
        l.CurHealth = parseInt(document.getElementById("user_health").innerHTML);
        if (l.user.tth <= 0 && parseInt(l.CurHealth) < l.settings.healat && l.settings.healOn == "red" && parseInt(User.stamina) > 5 && (l.evilOn || l.newfighton)) {
            HealNY();
            S = true
        } else {
            if (l.user.tth <= 0 && parseInt(l.CurHealth) < l.settings.healat && l.settings.healOn == true) {
                HealNY();
                S = true
            }
        }
        var U = new XMLHttpRequest();
        U.open("POST", $GEgc1X1_JMtYkBqwXMO(71), true);
        U.setRequestHeader($GEgc1X1_JMtYkBqwXMO(122), $GEgc1X1_JMtYkBqwXMO(385));
        U.setRequestHeader("Accept", "*/*");
        U.setRequestHeader($GEgc1X1_JMtYkBqwXMO(121), $GEgc1X1_JMtYkBqwXMO(386));
        U.send($GEgc1X1_JMtYkBqwXMO(120) + l.xw_user + "&sf_xw_sig=" + local_xw_sig + $GEgc1X1_JMtYkBqwXMO(387));
        U.onreadystatechange = function () {
            if (this.readyState == 4) {
                response = U.responseText;
                if (U.responseText.length > 15 && JSON.parse(U.responseText)) {
                    var b = JSON.parse(U.responseText);
                    document.getElementById("user_health").innerHTML = b.user_fields.user_health;
                    document.getElementById("user_stamina").innerHTML = b.user_fields.user_stamina;
                    document.getElementById("user_energy").innerHTML = b.user_fields.user_energy;
                    l.CurHealth = b.user_fields.user_health;
                    User.health = b.user_fields.user_health;
                    l.user.group_atk = b.fightbar.group_atk;
                    l.user.group_def = b.fightbar.group_def;
                    if (l.user.firstRun == true) {
                        l.user.start_group_atk = b.fightbar.group_atk.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                        l.user.start_group_def = b.fightbar.group_def.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                        l.user.firstRun = false
                    }
                    update_user_scores()
                }
            }
        };
        if (S === false && l.user.tth <= 0 && parseInt(l.CurHealth) < l.settings.healat && l.settings.healOn == "red" && parseInt(User.stamina) > 5 && (l.evilOn || l.newfighton)) {
            HealNY()
        } else {
            if (S === false && l.user.tth <= 0 && parseInt(l.CurHealth) < l.settings.healat && l.settings.healOn == true) {
                HealNY()
            }
        }
    }
    function HealNY() {
        var b, S = (l.settings.healny === true) ? $GEgc1X1_JMtYkBqwXMO(70) : $GEgc1X1_JMtYkBqwXMO(437),
            T = new XMLHttpRequest();
        T.open("POST", S, true);
        T.setRequestHeader($GEgc1X1_JMtYkBqwXMO(122), $GEgc1X1_JMtYkBqwXMO(385));
        T.setRequestHeader($GEgc1X1_JMtYkBqwXMO(121), $GEgc1X1_JMtYkBqwXMO(386));
        T.send($GEgc1X1_JMtYkBqwXMO(120) + l.xw_user + "&sf_xw_sig=" + local_xw_sig);
        T.onreadystatechange = function () {
            if (this.readyState == 4) {
                if (T.responseText.length > 15 && JSON.parse(T.responseText)) {
                    var b = JSON.parse(T.responseText);
                    document.getElementById("user_health").innerHTML = b.user_fields.user_health;
                    l.CurHealth = b.user_fields.user_health;
                    User.health = b.user_fields.user_health;
                    l.user.group_atk = b.fightbar.group_atk;
                    l.user.group_def = b.fightbar.group_def;
                    if (l.user.firstRun == true) {
                        l.user.start_group_atk = b.fightbar.group_atk.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                        l.user.start_group_def = b.fightbar.group_def.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                        l.user.firstRun = false
                    }
                    update_user_scores();
                    l.user.tth = b.waitHealTimer;
                    if (l.tthInt === 0) {
                        l.tthInt = setInterval(function () {
                            l.user.tth--;
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(69)).getElementsByClassName("health")[0].innerHTML = $GEgc1X1_JMtYkBqwXMO(438) + l.user.tth + $GEgc1X1_JMtYkBqwXMO(68) + l.user.tth + "</span>)";
                            if (l.user.tth <= 0) {
                                clearInterval(l.tthInt);
                                l.tthInt = 0
                            }
                        }, 1000)
                    }
                    f;
                    if (b.heal_success === 1) {
                        logEntry("Slayer: " + b.hospital_message)
                    }
                }
            }
        };
    }
    function Settings() {
        var S = ".9.10",
            T = l.settings.fbfeedloc;
        popupTitle = "UnlockedMW";
        if (l.is_vip) {
            popupTitle += " VIP 4"
        } else {
            popupTitle += " PRO 3"
        }
        popupTitle += S;
        content = $GEgc1X1_JMtYkBqwXMO(439) + T + $GEgc1X1_JMtYkBqwXMO(67) + l.settings.bursts + $GEgc1X1_JMtYkBqwXMO(440) + l.settings.stopat + $GEgc1X1_JMtYkBqwXMO(66) + l.settings.abortAt + $GEgc1X1_JMtYkBqwXMO(441) + l.settings.healat + $GEgc1X1_JMtYkBqwXMO(65) + l.settings.rref + $GEgc1X1_JMtYkBqwXMO(442);
        if (l.is_vip) {
            content += $GEgc1X1_JMtYkBqwXMO(64) + l.settings.FighterInt + $GEgc1X1_JMtYkBqwXMO(443)
        }
        content += $GEgc1X1_JMtYkBqwXMO(63);
        height = "760";
        myPop($GEgc1X1_JMtYkBqwXMO(444), popupTitle, content, height);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(62)).onclick = saveSettings;
        document.getElementById("CustomBM").onclick = display_appstore;
        document.getElementById("ForceUpDate").onclick = forceupDate;
        if (l.settings.umwicesteal) {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(445)).checked = true
        }
        if (l.settings.burstOn) {
            document.getElementById("burst").checked = true
        }
        if (l.settings.pauseOn) {
            document.getElementById("pausecheck").checked = true
        }
        if (l.settings.abortOn) {
            document.getElementById("stopcheck").checked = true
        }
        if (l.settings.autopostOn) {
            document.getElementById("autopost").checked = true
        }
        if (l.settings.autoBank) {
            document.getElementById("autobank").checked = true
        }
        if (l.settings.removeChat) {
            document.getElementById("remchat").checked = true
        }
        if (l.settings.userobsquads) {
            document.getElementById("robsquad").checked = true
        }
        if (l.settings.healny) {
            document.getElementById("healny").checked = true
        }
        if (l.settings.abortHeal) {
            document.getElementById("healthy").checked = true
        }
        if (l.settings.claimlevel) {
            document.getElementById("claimlevel").checked = true
        }
        if ($GEgc1X1_JMtYkBqwXMO(61).test(l.settings.Whitelist_enabled)) {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(446)).checked = true
        } else {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(60)).checked = true
        }
        if ($GEgc1X1_JMtYkBqwXMO(61).test(l.settings.Blacklist_enabled)) {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(447)).checked = true
        } else {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(59)).checked = true
        }
        if (l.settings.Whitelist) {
            var a = unescape(l.settings.Whitelist[0]);
            for (var i = 1; i < l.settings.Whitelist.length; i++) {
                a += "\n" + unescape(l.settings.Whitelist[i])
            }
            document.getElementById($GEgc1X1_JMtYkBqwXMO(448)).value = a
        }
        if (l.settings.Blacklist) {
            var a = unescape(l.settings.Blacklist[0]);
            for (var i = 1; i < l.settings.Blacklist.length; i++) {
                a += "\n" + unescape(l.settings.Blacklist[i])
            }
            document.getElementById($GEgc1X1_JMtYkBqwXMO(58)).value = a
        }
    }
    function saveSettings() {
        setTimeout(function () {
            document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById($GEgc1X1_JMtYkBqwXMO(444)))
        }, 10);
        if (l.is_vip) {
            l.settings.FighterInt = document.getElementById("postformid6").value;
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(447)).checked) {
                l.settings.Blacklist_enabled = true
            } else {
                l.settings.Blacklist_enabled = false
            }
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(446)).checked) {
                l.settings.Whitelist_enabled = true
            } else {
                l.settings.Whitelist_enabled = false
            }
            if ($GEgc1X1_JMtYkBqwXMO(449).test(document.getElementById($GEgc1X1_JMtYkBqwXMO(448)).value)) {
                var a = document.getElementById($GEgc1X1_JMtYkBqwXMO(448)).value.split($GEgc1X1_JMtYkBqwXMO(57));
                l.settings.Whitelist = new Array();
                for (var i = 0; i < a.length; i++) {
                    l.settings.Whitelist[i] = escape(a[i])
                }
            }
            if ($GEgc1X1_JMtYkBqwXMO(449).test(document.getElementById($GEgc1X1_JMtYkBqwXMO(58)).value)) {
                var a = document.getElementById($GEgc1X1_JMtYkBqwXMO(58)).value.split($GEgc1X1_JMtYkBqwXMO(57));
                l.settings.Blacklist = new Array();
                for (var i = 0; i < a.length; i++) {
                    l.settings.Blacklist[i] = escape(a[i])
                }
            }
        }
        l.settings.rref = parseInt(document.getElementById("postformid5").value);
        l.settings.fbfeedloc = document.getElementById("postformid").value;
        l.settings.healat = parseInt(document.getElementById("postformid2").value);
        l.settings.stopat = parseInt(document.getElementById("postformid3").value);
        l.settings.abortAt = parseInt(document.getElementById("postformid7").value);
        l.settings.bursts = parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(450)).value);
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(445)).checked == true) {
            l.settings.umwicesteal = true
        } else {
            l.settings.umwicesteal = false
        }
        if (document.getElementById("autopost").checked == true) {
            l.settings.autopostOn = true
        } else {
            l.settings.autopostOn = false
        }
        if (document.getElementById("burst").checked == true) {
            l.settings.burstOn = true
        } else {
            l.settings.burstOn = false
        }
        if (document.getElementById("pausecheck").checked == true) {
            l.settings.pauseOn = true
        } else {
            l.settings.pauseOn = false
        }
        if (document.getElementById("stopcheck").checked == true) {
            l.settings.abortOn = true
        } else {
            l.settings.abortOn = false
        }
        if (document.getElementById("autobank").checked == true) {
            l.settings.autoBank = true
        } else {
            l.settings.autoBank = false
        }
        if (document.getElementById("robsquad").checked == true) {
            l.settings.userobsquads = true
        } else {
            l.settings.userobsquads = false
        }
        if (document.getElementById("remchat").checked == true) {
            l.settings.removeChat = true
        } else {
            l.settings.removeChat = false
        }
        if (document.getElementById("healthy").checked == true) {
            l.settings.abortHeal = true
        } else {
            l.settings.abortHeal = false
        }
        if (document.getElementById("healny").checked == true) {
            l.settings.healny = true
        } else {
            l.settings.healny = false
        }
        if (document.getElementById("claimlevel").checked == true) {
            l.settings.claimlevel = true
        } else {
            l.settings.claimlevel = false
        }
        writeSettings()
    }
    function forceupDate() {
        if (confirm($GEgc1X1_JMtYkBqwXMO(56))) {
            localStorage.clear();
            top.location.href = $GEgc1X1_JMtYkBqwXMO(451)
        }
    }
    function K(S) {
        var T = null,
            i;
        for (i = 0; i < S.length; i++) {
            if (S[i].checked) {
                T = S[i].value;
                break
            }
        }
        return T
    }
    function myPop(S, T, U, V, W, X) {
        if (!V || V == undefined) {
            V = "600"
        }
        if (!W || W == undefined) {
            W = "330"
        }
        var Y = '<div id="' + S + $GEgc1X1_JMtYkBqwXMO(55) + (375 - (W / 2)) + $GEgc1X1_JMtYkBqwXMO(452) + W + "px;height:" + V + $GEgc1X1_JMtYkBqwXMO(54) + W + $GEgc1X1_JMtYkBqwXMO(453) + (W - 10) + $GEgc1X1_JMtYkBqwXMO(53) + T + $GEgc1X1_JMtYkBqwXMO(454) + U + $GEgc1X1_JMtYkBqwXMO(52);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).innerHTML += Y;
        if (X) {
            setTimeout(function () {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById(S))
            }, X)
        }
    }
    function L() {
        document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById("myPopup"))
    }
    function LetThereBeRobberies() {
        var S = parseInt(document.getElementById("user_stamina").innerHTML);
        if (!l.evilOn || l.RobbingHung) {
            return
        }
        if (l.settings.pauseOn) {
            if (parseInt(document.getElementById($GEgc1X1_JMtYkBqwXMO(378)).innerHTML) < l.settings.stopat) {
                l.evilOn = false;
                logEntry($GEgc1X1_JMtYkBqwXMO(327));
                document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                return
            }
        }
        if (S < 50) {
            logEntry($GEgc1X1_JMtYkBqwXMO(455));
            setTimeout(LetThereBeRobberies, 30000)
        } else {
            if (document.evaluate($GEgc1X1_JMtYkBqwXMO(330), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0) {
                TravelRobbing();
                logEntry($GEgc1X1_JMtYkBqwXMO(51));
                setTimeout(LetThereBeRobberies, 3000)
            } else {
                if (parseInt($GEgc1X1_JMtYkBqwXMO(456).exec(document.getElementById($GEgc1X1_JMtYkBqwXMO(50)).onclick)[1]) > 0 && l.settings.userobsquads) {
                    setTimeout(RefreshMeRS, (6900));
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(50)).click()
                } else {
                    l.RobbingHung = true;
                    l.RHangCHKInterval = setTimeout(RobbingWatchdog, 30000);
                    robTheBoard();
                    setTimeout(RefreshMe, 6900)
                }
            }
        }
    }
    function robTheBoard() {
        if (l.RobbingHung) {
            var S = 8;
            for (var i = 8; i >= 0; i--) {
                setTimeout($GEgc1X1_JMtYkBqwXMO(457) + S + $GEgc1X1_JMtYkBqwXMO(49), i * l.settings.rref);
                S--
            }
        }
    }
    function RefreshMeRS() {
        CloseDoopidPopup();
        setTimeout(LetThereBeRobberies, 4000);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(458)).click()
    }
    function RobbingWatchdog() {
        clearTimeout(l.RHangCHKInterval);
        l.RobbingHung = false;
        TravelRobbing();
        setTimeout(LetThereBeRobberies, 5000);
        return
    }
    function TravelRobbing() {
        do_ajax("inner_page", $GEgc1X1_JMtYkBqwXMO(48) + l.settings.evilCity + $GEgc1X1_JMtYkBqwXMO(459), 1, 1, 0, 0)
    }
    function RefreshMe() {
        try {
            var S = document.evaluate($GEgc1X1_JMtYkBqwXMO(47), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
            if (S.snapshotLength >= 1) {
                clearTimeout(l.RHangCHKInterval);
                l.RobbingHung = false;
                CloseDoopidPopup();
                setTimeout(LetThereBeRobberies, 4000);
                document.getElementById($GEgc1X1_JMtYkBqwXMO(458)).click()
            } else {
                if (l.RobbingHung) {
                    robTheBoard();
                    setTimeout(RefreshMe, 5000)
                }
            }
        } catch (T) {}
    }
    function resetfightaction() {
        l.fightActionRunning = false;
        closeFightPop()
    }
    function add_code(S) {
        try {
            code = JSON.parse(atob(S))
        } catch (e) {
            code = null
        }
        if (code && code.name && code.url) {
            code.isvip = false;
            code.isinstalled = true;
            l.settings.AppStore.push(code);
            writeSettings();
            alert(code.name + $GEgc1X1_JMtYkBqwXMO(460))
        } else {
            alert($GEgc1X1_JMtYkBqwXMO(46))
        }
    }
    function display_appstore() {
        try {
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(444))) {
                document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById($GEgc1X1_JMtYkBqwXMO(444)))
            }
        } catch (e) {}
        var b = $GEgc1X1_JMtYkBqwXMO(461),
            d = 500,
            i;
        for (i = 0; i < l.settings.AppStore.length; i++) {
            if (i % 2 == 0) {
                b += "<tr>"
            } else {
                b += $GEgc1X1_JMtYkBqwXMO(45)
            }
            var S;
            if (l.settings.AppStore[i].vendor == "Spockholms") {
                S = k.spock_app
            } else {
                if (l.settings.AppStore[i].vendor == "Simony") {
                    S = k.simony_app
                } else {
                    if (l.settings.AppStore[i].vendor == "GuessX") {
                        S = k.guessx_app
                    } else {
                        if (l.settings.AppStore[i].vendor == "Lucifer") {
                            S = k.mmfu_app
                        } else {
                            S = k.unlocked_app
                        }
                    }
                }
            }
            b += $GEgc1X1_JMtYkBqwXMO(462) + S + $GEgc1X1_JMtYkBqwXMO(44) + l.settings.AppStore[i].vendor + $GEgc1X1_JMtYkBqwXMO(463) + (i + 1) + ".&nbsp;" + l.settings.AppStore[i].name + "</td>";
            if (i < l.defaultappcount) {
                b += "<td><small>" + l.settings.AppStore[i].description + $GEgc1X1_JMtYkBqwXMO(43)
            } else {
                b += "<td><small>" + l.settings.AppStore[i].description + $GEgc1X1_JMtYkBqwXMO(464) + i + $GEgc1X1_JMtYkBqwXMO(42)
            }
            if (l.settings.AppStore[i].isinstalled == 2) {
                b += $GEgc1X1_JMtYkBqwXMO(465) + i + $GEgc1X1_JMtYkBqwXMO(41) + k.autostart + $GEgc1X1_JMtYkBqwXMO(466)
            } else {
                if (l.settings.AppStore[i].isinstalled == true) {
                    b += $GEgc1X1_JMtYkBqwXMO(465) + i + $GEgc1X1_JMtYkBqwXMO(41) + k.installed + $GEgc1X1_JMtYkBqwXMO(466)
                } else {
                    if (l.settings.AppStore[i].isvip && !l.is_vip) {
                        b += $GEgc1X1_JMtYkBqwXMO(40) + k.viponly + $GEgc1X1_JMtYkBqwXMO(466)
                    } else {
                        b += $GEgc1X1_JMtYkBqwXMO(467) + i + $GEgc1X1_JMtYkBqwXMO(41) + k.disabled + $GEgc1X1_JMtYkBqwXMO(466)
                    }
                }
            }
        }
        b += $GEgc1X1_JMtYkBqwXMO(39);
        myPop($GEgc1X1_JMtYkBqwXMO(468), $GEgc1X1_JMtYkBqwXMO(38), b, d, "440");
        var T = document.getElementsByClassName("toggle_app");
        for (var r = 0; r < T.length; r++) {
            document.getElementsByClassName("toggle_app")[r].onclick = toggle_app
        }
        var U = document.getElementsByClassName("remove_app");
        for (var r = 0; r < U.length; r++) {
            document.getElementsByClassName("remove_app")[r].onclick = remove_app
        }
    }
    function toggle_app() {
        var S = parseInt(this.id);
        if (l.settings.AppStore[S].isvip && !l.is_vip) {
            return
        }
        if (l.settings.AppStore[S].isinstalled == 2) {
            l.settings.AppStore[S].isinstalled = false;
            document.getElementsByClassName("toggle_app")[S].innerHTML = '<img src="' + k.disabled + '">';
            document.getElementsByClassName("toggle_app")[S].setAttribute("style", $GEgc1X1_JMtYkBqwXMO(469))
        } else {
            if (l.settings.AppStore[S].isinstalled == true) {
                l.settings.AppStore[S].isinstalled = 2;
                document.getElementsByClassName("toggle_app")[S].innerHTML = '<img src="' + k.autostart + '">';
                document.getElementsByClassName("toggle_app")[S].setAttribute("style", $GEgc1X1_JMtYkBqwXMO(469))
            } else {
                l.settings.AppStore[S].isinstalled = true;
                document.getElementsByClassName("toggle_app")[S].innerHTML = '<img src="' + k.installed + '">';
                document.getElementsByClassName("toggle_app")[S].setAttribute("style", $GEgc1X1_JMtYkBqwXMO(469))
            }
        }
        writeSettings();
        E()
    }
    function remove_app() {
        var S = parseInt(this.id);
        l.settings.AppStore.splice(S, 1);
        writeSettings();
        document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).removeChild(document.getElementById($GEgc1X1_JMtYkBqwXMO(468)));
        display_appstore()
    }
    function update_user_scores() {
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(187))) {
            var S = l.user;
            document.getElementById($GEgc1X1_JMtYkBqwXMO(37)).innerHTML = S.group_atk;
            document.getElementById($GEgc1X1_JMtYkBqwXMO(470)).innerHTML = S.group_def;
            var T = parseInt(parseInt(S.group_atk.replace($GEgc1X1_JMtYkBqwXMO(436), "")) - parseInt(S.start_group_atk.replace($GEgc1X1_JMtYkBqwXMO(436), ""))),
                U = document.getElementById($GEgc1X1_JMtYkBqwXMO(36));
            if (T >= 0) {
                U.innerHTML = "\x2b" + T;
                U.className = "good"
            } else {
                if (T < 0) {
                    U.innerHTML = T;
                    U.className = "bad"
                }
            }
            var V = parseInt(parseInt(S.group_def.replace($GEgc1X1_JMtYkBqwXMO(436), "")) - parseInt(S.start_group_def.replace($GEgc1X1_JMtYkBqwXMO(436), ""))),
                W = document.getElementById($GEgc1X1_JMtYkBqwXMO(471));
            if (V >= 0) {
                W.innerHTML = "\x2b" + V;
                W.className = "good"
            } else {
                if (V < 0) {
                    W.innerHTML = V;
                    W.className = "bad"
                }
            }
        }
    }
    function M() {
        var S = this.id;
        rivalindex = $GEgc1X1_JMtYkBqwXMO(35).exec(S)[1];
        if (l.settings.customRivals[rivalindex] == 1) {
            document.getElementById(S).setAttribute("class", $GEgc1X1_JMtYkBqwXMO(472));
            document.getElementById(S).innerHTML = $GEgc1X1_JMtYkBqwXMO(34) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
            l.settings.customRivals[rivalindex] = 0;
            writeSettings()
        } else {
            document.getElementById(S).setAttribute("class", $GEgc1X1_JMtYkBqwXMO(473));
            document.getElementById(S).innerHTML = $GEgc1X1_JMtYkBqwXMO(34) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
            l.settings.customRivals[rivalindex] = 1;
            writeSettings()
        }
    }
    function N() {
        var S = this.id;
        rivalindex = $GEgc1X1_JMtYkBqwXMO(33).exec(S)[1];
        if (l.settings.customRivals[rivalindex] == 1) {
            document.getElementById(S).setAttribute("class", $GEgc1X1_JMtYkBqwXMO(472));
            document.getElementById(S).innerHTML = $GEgc1X1_JMtYkBqwXMO(474);
            l.settings.customRivals[rivalindex] = 0;
            writeSettings()
        } else {
            document.getElementById(S).setAttribute("class", $GEgc1X1_JMtYkBqwXMO(473));
            document.getElementById(S).innerHTML = $GEgc1X1_JMtYkBqwXMO(474);
            l.settings.customRivals[rivalindex] = 1;
            writeSettings()
        }
    }
    function O() {
        var S = document.getElementsByClassName("action");
        for (i = 0; i < S.length; i++) {
            if (!S[i].firstElementChild) {
                if ($GEgc1X1_JMtYkBqwXMO(405).test(S[i].parentNode.firstElementChild.innerHTML)) {
                    var j = 10;
                    if (l.settings.customRivals[0] == 1) {
                        S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(32) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                    } else {
                        S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(31) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                    }
                    document.getElementById($GEgc1X1_JMtYkBqwXMO(476)).onclick = M;
                    i++;
                    while (i < S.length && S[i].firstElementChild) {
                        S[i].setAttribute("style", "width:200px");
                        if (l.settings.customRivals[j] == 1) {
                            S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(30) + j + $GEgc1X1_JMtYkBqwXMO(477) + S[i].innerHTML
                        } else {
                            S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(29) + j + $GEgc1X1_JMtYkBqwXMO(477) + S[i].innerHTML
                        }
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(478) + j).onclick = N;
                        j++;
                        i++
                    }
                    i--
                } else {
                    if ($GEgc1X1_JMtYkBqwXMO(406).test(S[i].parentNode.firstElementChild.innerHTML)) {
                        var j = 20;
                        if (l.settings.customRivals[1] == 1) {
                            S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(28) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                        } else {
                            S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(479) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                        }
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(27)).onclick = M;
                        i++;
                        while (i < S.length && S[i].firstElementChild) {
                            S[i].setAttribute("style", "width:200px");
                            if (l.settings.customRivals[j] == 1) {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(30) + j + $GEgc1X1_JMtYkBqwXMO(477) + S[i].innerHTML
                            } else {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(29) + j + $GEgc1X1_JMtYkBqwXMO(477) + S[i].innerHTML
                            }
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(478) + j).onclick = N;
                            j++;
                            i++
                        }
                        i--
                    } else {
                        if ($GEgc1X1_JMtYkBqwXMO(100).test(S[i].parentNode.firstElementChild.innerHTML)) {
                            if (l.settings.customRivals[2] == 1) {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(480) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                            } else {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(26) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                            }
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(481)).onclick = M
                        } else {
                            if (l.settings.customRivals[3] == 1) {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(25) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                            } else {
                                S[i].innerHTML = $GEgc1X1_JMtYkBqwXMO(482) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(475)
                            }
                            document.getElementById($GEgc1X1_JMtYkBqwXMO(24)).onclick = M
                        }
                    }
                }
            }
        }
    }
    function P() {
        var S = $GEgc1X1_JMtYkBqwXMO(483),
            T = $GEgc1X1_JMtYkBqwXMO(23),
            U = $GEgc1X1_JMtYkBqwXMO(484);
        document.getElementById($GEgc1X1_JMtYkBqwXMO(22)).innerHTML += S;
        document.getElementById($GEgc1X1_JMtYkBqwXMO(485)).innerHTML += T;
        document.getElementById($GEgc1X1_JMtYkBqwXMO(21)).innerHTML += U;
        var V = document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(486));
        for (var i = 0; i < V.length; i++) {
            document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(486))[i].onclick = ActivateCrewmate
        }
    }
    function ActivateCrewmate() {
        var S = parseInt(this.getAttribute("city"));
        if (S) {
            var T;
            if ($GEgc1X1_JMtYkBqwXMO(409).test(window.location.href)) {
                T = "https"
            } else {
                T = "http"
            }
            var U = ["Brazil", "Chicago", "London"],
                V = "",
                W = T + $GEgc1X1_JMtYkBqwXMO(20) + S + $GEgc1X1_JMtYkBqwXMO(487);
            $.ajax({
                type: "POST",
                url: W,
                timeout: 30000,
                data: V,
                success: function (msg) {
                    var X = JSON.parse(msg),
                        Y;
                    if (X.data.crewstatus == "ok") {
                        Y = "activated !"
                    } else {
                        if (X.data.crewmsg) {
                            Y = $GEgc1X1_JMtYkBqwXMO(19)
                        }
                    }
                    if (Y.length > 0) {
                        var Z = document.createElement("div");
                        Z.id = $GEgc1X1_JMtYkBqwXMO(488);
                        Z.setAttribute("class", "pop_box");
                        Z.setAttribute("style", $GEgc1X1_JMtYkBqwXMO(18));
                        Z.innerHTML = $GEgc1X1_JMtYkBqwXMO(489) + T + $GEgc1X1_JMtYkBqwXMO(17) + U[(S - 7)] + $GEgc1X1_JMtYkBqwXMO(490) + Y + $GEgc1X1_JMtYkBqwXMO(16);
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(165)).appendChild(Z)
                    }
                },
                error: function (X, status, Y) {
                    return
                }
            })
        }
    }
    $("body").ajaxComplete(function (e, S, T) {
        if (l.is_vip) {
            if (document.evaluate($GEgc1X1_JMtYkBqwXMO(329), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                O()
            }
        }
        var U = S.responseText;
        if ($GEgc1X1_JMtYkBqwXMO(491).test(U)) {
            local_xw_sig = $GEgc1X1_JMtYkBqwXMO(491).exec(U)[1]
        }
        if (l.ajax) {
            l.ajax = false;
            return
        }
        if (l.fightActionRunning) {
            l.fightActionRunning = false;
            clearTimeout(l.fightwatchdogtimout)
        }
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(413))) {
            parseNewFightResults(U)
        }
        if (document.evaluate($GEgc1X1_JMtYkBqwXMO(420), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
            l.hitlist = true
        } else {
            if (l.hitlist) {
                l.hitlist = false
            }
        }
        if (l.settings.jobsOn != false && (document.getElementById("inner_page").getAttribute("class") === $GEgc1X1_JMtYkBqwXMO(15) || (document.getElementById("inner_page").getAttribute("class") === $GEgc1X1_JMtYkBqwXMO(492)))) {
            if (document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(109)).length > 0 && document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(14)).length > 0) {
                document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(398))[0].click()
            }
            setTimeout(function () {
                handleJobresponse(U)
            }, 50);
            return
        }
        if (l.declaring_war == true && $GEgc1X1_JMtYkBqwXMO(493).test(U)) {
            if ($GEgc1X1_JMtYkBqwXMO(13).test(U)) {
                setTimeout(function () {
                    declare_war(null)
                }, 0)
            } else {
                if ($GEgc1X1_JMtYkBqwXMO(494).test(U)) {
                    setTimeout(function () {
                        fP(null)
                    }, 0)
                } else {
                    if ($GEgc1X1_JMtYkBqwXMO(12).test(U)) {
                        l.declaring_war = false
                    } else {
                        if ($GEgc1X1_JMtYkBqwXMO(495).test(U) || $GEgc1X1_JMtYkBqwXMO(11).test(U)) {
                            setTimeout(function () {
                                declare_war(null)
                            }, 0)
                        } else {
                            setTimeout(function () {
                                var V = $GEgc1X1_JMtYkBqwXMO(130).exec(document.getElementsByClassName($GEgc1X1_JMtYkBqwXMO(496))[0].getAttribute("onclick"))[1];
                                MW.Feed(postFeedAndSendCallForHelpNew);
                                $($GEgc1X1_JMtYkBqwXMO(10)).css("display", "none");
                                MW.Popup.hideAndRemove(V);
                                l.declaring_war = false
                            }, 1500);
                            document.getElementById("EvilOnOff").setAttribute("class", $GEgc1X1_JMtYkBqwXMO(323));
                            document.getElementById("EvilOnOff").innerHTML = $GEgc1X1_JMtYkBqwXMO(325) + k.evil_icon + $GEgc1X1_JMtYkBqwXMO(179);
                            l.evilOn = false
                        }
                    }
                }
            }
            return
        }
        if (($GEgc1X1_JMtYkBqwXMO(497).test(U))) {
            var b = eval("\x28" + U + "\x29");
            l.user.group_atk = b.fightbar.group_atk;
            l.user.group_def = b.fightbar.group_def;
            if (l.user.firstRun == true) {
                l.user.start_group_atk = b.fightbar.group_atk.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                l.user.start_group_def = b.fightbar.group_def.replace($GEgc1X1_JMtYkBqwXMO(436), "");
                l.user.firstRun = false
            }
            update_user_scores()
        }
        if ($GEgc1X1_JMtYkBqwXMO(9).test(U)) {
            P()
        }
    });

    function handleJobresponse(S) {
        var T, U;
        if ($GEgc1X1_JMtYkBqwXMO(498).test(S)) {
            var jobObj = JSON.parse(S);
            jobObj = JSON.parse(jobObj.data);
            T = parseInt(jobObj.job_results.job_mastery_percentage);
            U = parseInt(jobObj.job_results.energy_consumed)
        } else {
            if ($GEgc1X1_JMtYkBqwXMO(8).test(S)) {
                T = parseInt($GEgc1X1_JMtYkBqwXMO(8).exec(S)[1]);
                U = parseInt($GEgc1X1_JMtYkBqwXMO(499).exec(S)[1])
            } else {
                if ($GEgc1X1_JMtYkBqwXMO(7).test(S)) {
                    var jobObj = JSON.parse(S);
                    T = parseInt(jobObj.jobResult.masteryTotal);
                    U = parseInt(jobObj.jobResult.energy);
                    if (document.getElementById("btn_queue").childNodes[0].childNodes[0].childNodes[2].innerHTML && (parseInt(document.getElementById("btn_queue").childNodes[0].childNodes[0].childNodes[2].innerHTML) <= 22)) {
                        document.getElementById($GEgc1X1_JMtYkBqwXMO(500)).click()
                    }
                } else {
                    console.log($GEgc1X1_JMtYkBqwXMO(6))
                }
            }
        }
        if (l.settings.jobsOn != 2 && T == 100) {
            logEntry($GEgc1X1_JMtYkBqwXMO(501));
            I();
            return
        } else {
            if (U > User.energy) {
                logEntry($GEgc1X1_JMtYkBqwXMO(5) + U + $GEgc1X1_JMtYkBqwXMO(502) + jobObj.user_fields.user_energy + $GEgc1X1_JMtYkBqwXMO(4));
                I();
                return
            } else {
                if (l.settings.jobsOn == 2 && document.evaluate($GEgc1X1_JMtYkBqwXMO(503), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                    l.NYjobon = true;
                    return
                } else {
                    if (l.NYjobon) {
                        l.NYjobon = false;
                        return
                    }
                }
                if (document.evaluate($GEgc1X1_JMtYkBqwXMO(124), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0 && document.getElementsByClassName("buy_prompt").length == 0) {
                    l.ILVjobon = true;
                    return
                } else {
                    if (l.ILVjobon) {
                        l.ILVjobon = false;
                        return
                    }
                }
                if ($GEgc1X1_JMtYkBqwXMO(3).test(S)) {
                    if ($GEgc1X1_JMtYkBqwXMO(504).test(S)) {
                        l.Bjobid = $GEgc1X1_JMtYkBqwXMO(504).exec(S)[2]
                    }
                    l.Bjobon = true;
                    return
                } else {
                    if (l.Bjobon) {
                        l.Bjobon = false;
                        return
                    }
                }
            }
        }
    }
    function Q() {
        n();
        if (document.location.href.indexOf($GEgc1X1_JMtYkBqwXMO(2)) != -1) {
            setTimeout(F, 9000)
        }
        if (!localStorage.getItem($GEgc1X1_JMtYkBqwXMO(424))) {
            l.umwavoidlist[0] = "p|45293801";
            localStorage.setItem($GEgc1X1_JMtYkBqwXMO(424), JSON.stringify(l.umwavoidlist))
        } else {
            l.umwavoidlist = JSON.parse(localStorage.getItem($GEgc1X1_JMtYkBqwXMO(424)))
        }
        readSettings();
        G();
        if (l.settings.stamOn == true || l.settings.stamOn == 2) {
            l.fightInterval = setInterval(Fighter, l.settings.FighterInt)
        }
        if ((l.settings.healOn == true) || (l.settings.healOn == "red")) {
            l.healInterval = setInterval(DoAutoHeal, 1000)
        }
        if (l.settings.jobsOn == true || l.settings.jobsOn == 2) {
            l.energyInterval = setInterval(Jobber, 1000)
        }
        if (l.settings.slayerOn == true) {
            if (l.settings.slayerMode == 0) {
                l.evilInterval = setInterval(Fightlist, 2500);
                l.evilOn = true
            } else {
                if (l.settings.slayerMode == 1) {
                    l.evilInterval = setInterval(Rivals, 2500);
                    l.evilOn = true
                } else {
                    if (l.settings.slayerMode == 2) {
                        setTimeout(LetThereBeRobberies, 3000);
                        l.evilOn = true
                    } else {
                        if (l.settings.slayerMode == 3) {
                            l.evilOn = true;
                            loadNextHopper();
                            l.evilInterval = setInterval(Hopperlist, 3000)
                        }
                    }
                }
            }
        }
        setInterval(G, 10000);
        setTimeout(KillStupidPop, 10000);
        for (var i = 0; i < l.settings.AppStore.length; i++) {
            if (l.settings.AppStore[i].isinstalled == 2) {
                var S = l.settings.AppStore[i].url,
                    a = document.createElement("script");
                a.type = $GEgc1X1_JMtYkBqwXMO(361);
                a.src = S + "\x3f" + Math.random();
                document.getElementsByTagName("head")[0].appendChild(a)
            }
        }
        setTimeout(P, 500)
    }
    if (document.getElementById($GEgc1X1_JMtYkBqwXMO(362))) {
        return
    } else {
        Q()
    }
    function KillStupidPop() {
        if ($($GEgc1X1_JMtYkBqwXMO(505)).css("display") == "block") {
            $($GEgc1X1_JMtYkBqwXMO(505)).css("display", "none")
        }
    }
    function writeSettings() {
            localStorage.setItem($GEgc1X1_JMtYkBqwXMO(1), JSON.stringify(l.settings))
    }
    function readSettings() {
        l.defaultappcount = l.settings.AppStore.length;
        if (!localStorage.getItem($GEgc1X1_JMtYkBqwXMO(1))) {
            writeSettings()
        } else {
            tempsettings = JSON.parse(localStorage.getItem($GEgc1X1_JMtYkBqwXMO(1)));
            if (Object.keys(tempsettings).length != Object.keys(l.settings).length) {
                writeSettings()
            } else {
                if (tempsettings.AppStore.length < l.settings.AppStore.length) {
                    writeSettings()
                } else {
                    l.settings = tempsettings
                }
            }
        }
    }
    function R(S, T) {
        T = T || document;
        return document.evaluate(S, T, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
    }
    HTMLElement.prototype.click = function () {
        $(this).click()
    };

    function familyMember(S, T) {
        this.pid = S;
        this.stat = T
    }
    function decodeID(S) {
        S = unescape(S);
        if (isNaN(S)) {
            S = decode64(S)
        }
        return S
    }
    function decode64(S) {
        var T = "",
            U, V, W = "",
            X, Y, Z, aa = "",
            i = 0,
            ab = $GEgc1X1_JMtYkBqwXMO(506);
        if (ab.exec(S)) {}
        S = S.replace($GEgc1X1_JMtYkBqwXMO(506), "");
        do {
            X = l.keyStr.indexOf(S.charAt(i++));
            Y = l.keyStr.indexOf(S.charAt(i++));
            Z = l.keyStr.indexOf(S.charAt(i++));
            aa = l.keyStr.indexOf(S.charAt(i++));
            U = (X << 2) | (Y >> 4);
            V = ((Y & 15) << 4) | (Z >> 2);
            W = ((Z & 3) << 6) | aa;
            T = T + String.fromCharCode(U);
            if (Z != 64) {
                T = T + String.fromCharCode(V)
            }
            if (aa != 64) {
                T = T + String.fromCharCode(W)
            }
            U = V = W = "";
            X = Y = Z = aa = ""
        } while (i < S.length);
        return T
    }
    function isFightPopOpen() {
        if (document.getElementById($GEgc1X1_JMtYkBqwXMO(0))) {
            if (document.getElementById($GEgc1X1_JMtYkBqwXMO(0)).style.display != "none") {
                return true
            } else {
                return false
            }
        } else {
            return false
        }
    }
    function closeFightPop() {
        l.ajax = true;
        l.closing = false;
        CloseJS()
    }
    function CloseDoopidPopup() {
        if ($(".pop_bg").length > 0) {
            $(".pop_bg").each(function () {
                var S = this.id;
                MW.Popup.hide(S.substr(S.lastIndexOf("\x5f") + 1))
            })
        }
    }
    Array.prototype.isAvoid = function (S) {
        for (var i = 0; i < this.length; i++) {
            if (this[i] == S) {
                return true
            }
        }
        return false
    };
    Array.prototype.isWhitelist = function (S) {
        for (var i = 0; i < this.length; i++) {
            if (this[i] == S) {
                return true
            }
        }
        return false
    };
    Array.prototype.isBlacklist = function (S) {
        for (var i = 0; i < this.length; i++) {
            if (this[i] == S) {
                return true
            }
        }
        return false
    };
};
injectScript(myscript);
