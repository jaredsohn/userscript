...
Copyright (C) 2008 - 2010 Booboo <scripts.booboo@gmail.com>
...
// @news !News in this release: v. 1.12.2
// @news Background functionalities like saving and langpacks are moved to another host, so please update. ;)  
// @news v. 1.12
// @news New host for the images (flags).
// @news Newer improved default saving format (2.1).
// @news New extended searching (population & race).
// @news You can filter your hits by the population and race* (* Only with Quick Search mode).
// @news SQL saving is integrated now. ;) Thanx to blablubbb.
// @news Default searching method is set to Quick Search.
// @news Minor code optimization. :)
// @news !Localization :)
...
var hostUrl = "http://biztonsagostarhely.eu/booboo/script/travian/tma/";
var fileHost = hostUrl;
var imgHost = "http://booboo.tvn.hu/img/";
...
var defObj = {
	...
	recovery = inherit(defObj), i;

objCopy (recovery, getObj("recovery_" + server));
...
objCopy(SQL, getObj("sql_" + server, defSQL));
...
style += "div#sql span.btn {display:block; background-color:#20B0FF; color:black; text-align:center; height:18px; width:-moz-fit-content; -moz-border-radius:3px; padding:1px 3px 0; margin:2px; float:left; cursor:pointer; border:1px solid transparent; -moz-user-select:none;}";
...
// button
style += "div.tma_button {-moz-user-select:none; cursor:default; display:block; text-align:center; min-height:22px; min-width:35px; width:-moz-fit-content; background-color:#ed6; color:#a24; border:2px outset orange; -moz-border-radius:20px; margin:2px 3px;}";
...
style += "textarea.tma {font-family: monospace; font-size:13px; border:0px; padding:0px; text-align:center; min-width:354px; background-color:ivory;}";
// input fields
...
{	var node = document.createElement(tag);
	...
	{	if (args.html)
		...
		if (args.p)
		{	var parent = typeof args.p === "string" ? $(args.p) : args.p;
...
{	var kk = {x: coordFixer(center.x - 7 * radius), y: coordFixer(center.y + 7 * radius)};
	...
	for (var iy = 0; iy <= 2 * radius; iy++)
		for (var ix = 0; ix <= 2 * radius; ix++)
			zonak.push(zonaUrl + xy2z({x:kk.x + 7 * ix, y:kk.y - 7 * iy}));
	return zonak;
}
...
{	var r = recovery.datas.hits;
	...
	{	case "normal":
		...
		case "csv":
			...
			var idx = rendez(dist);
			var text = LD.DIST+";"+LD.TYPE+";"+LD.HITS+";X;Y;"+LD.PLAYER+";"+LD.ALLIANCE+";"+LD.BONUS+(r.extended ? ";"+LD.POP+";"+LD.RACE : "")+"\n";
			for (var i in idx)
		...
		case "sql":
			...
			for each (var f in r.data) {
				var zk = f.url.split("|");
				text += ins + zk[0] + (SQL.colNames.c[1] ? ",'" + zk[1] + "'" : "");
...
{	var r = recovery.datas.hits, ver, i, db, ft, b,
	...
	{	// Newer versions
		...
		if (arr[0].indexOf("b") > -1) {
			...
			r.extended = false;
		r.sdcr[0] = arr[1];
		r.sdcr[1] = ver >= 2.1 ? b64_dec(arr[2]) : arr[2];
		r.sdcr[2] = b64_dec(arr[3]);
		r.sdcr[3] = parseInt(arr[4], 10);
		db = r.extended ? 9 : 7;
		for (i = 0; i < parseInt(arr[5], 10); i++)
		{	var field =
			{	type: null,
				url: null,
				name: null,
			...
			{	field.player = arr[6 + i * db + 3];
				field.uid = b64_dec(arr[6 + i * db + 4]);
				if (arr[6 + i * db + 5]) // Allies
				{	field.ally = arr[6 + i * db + 5];
					field.aid = b64_dec(arr[6 + i * db + 6]);
				}
				if (r.extended) {
	...
	{	// Old one
		...
		r.sdcr[0] = arr[0];
		r.sdcr[1] = arr[1];
		r.sdcr[2] = xy2z({x:arr[2], y:arr[3]});
		r.sdcr[3] = parseInt(arr[4], 10);
		db = parseInt(arr[5], 10);
		for (i = 0; i < db; i++)
		{	var field =
			{	type: null,
				url: null,
...
{	wins: [],
	...
	{	var win =
		...
		var titleContainer = $elem("div", {a:{"class":"title-container"}, p:title});
		var span = $elem("span", {html:win.title, a:{"class":"titlebar-text"}, s:{"direction":textDirection}, p:titleContainer});
		win.telem = span;
		if (win.buttons)
		{	var buttons = $elem("div", {a:{"class":"buttons-container"}, p:title});
			...
			{	switch (tmp[i])
				{	case "mini":
						var btn = $elem("div", {html:"<img src='" + IMG.minimize + "' height='5' width='12' style='margin-top:7px;' />", a:{"btn":"mini", "class":"gomb normal", "title":LD['WIN_MINIMIZE']}, p:buttons});
						break;
					case "full":
						var btn = $elem("div", {html:"<img src='" + IMG.full + "' height='10' width='12' style='margin-top:2px;' />", a:{"btn":"full", "class":"gomb normal", "title":LD['WIN_FULLSIZE']}, p:buttons});
						break;
					case "close":
						var btn = $elem("div", {html:"<img src='" + IMG.close + "' height='10' width='10' style='margin-top:2px;' />", a:{"btn":"close", "class":"gomb red close", "title":LD['WIN_CLOSE']}, p:buttons});
						break;
				}
				if (i == 0)
...
{	if (recovery.datas.igm != "")
	{	$('subject').value = "TMA_IGM v2.1";
		$('copy_subject').value = "TMA_IGM v2.1";
...
{	var td = xpath(t35 ? "//div[@class='message']" : "//table[@class='msg_rw']|//td[@colspan='3']", XPFirst);
	text = td.textContent;
	if (/--\/\/TMA begin\/\/--\n((.*\n)+)--\/\/TMA end\/\/--/m.test(text))
...
{	var ablak = winObj.open(
	...
	var center = $elem("center", {p:ablak.body});
	var table = $elem("table", {a:{"width":"100%", "border":0}, p:$elem("div", {a:{"dir":"ltr"}, s:{"width":"238px", "height":"57px"}, p:center})});
	var tr = $elem("tr", {p:table});

	var search = $elem("img", {a:{"title":LD["SEARCH"], "src":IMG.mag48, "width":48, "height":48}, s:{"cursor":"pointer", "margin":(t35 ? "2px 5px":"0px 3px")}, p:$elem("td", {a:{"align":"center"}, p:tr})});
	var dist = $elem("img", {a:{"title":LD["DIST_CALC"], "src":IMG.radar48, "width":48, "height":48}, s:{"cursor":"pointer", "margin":(t35 ? "2px 4px":"0px 3px")}, p:$elem("td", {a:{"align":"center"}, p:tr})});
	var io = $elem("img", {a:{"title":LD["LOAD_CONTINUE"], "src":IMG.floppy48, "width":48, "height":48}, s:{"cursor":"pointer", "margin":(t35 ? "2px 4px":"0px 3px")}, p:$elem("td", {a:{"align":"center"}, p:tr})});
	var setup = $elem("img", {a:{"title":LD["SETUP"], "src":IMG.config48, "width":48, "height":48}, s:{"cursor":"pointer", "margin":(t35 ? "2px 5px":"0px 3px")}, p:$elem("td", {a:{"align":"center"}, p:tr})});
	...
	$elem("a", {html:LD["CONTACT"], a:{"href":"mailto:" + headers["e-mail"], "title":headers["e-mail"], "target":"_blank"}, s:{"cssFloat":"left", "color":"#d71", "fontSize":"10px", "margin":"5px"}, p:ablak.body});
	$elem("a", {html:"v.: " + headers["version"], a:{"href":verUrl, "title":LD["HOMEPAGE"], "target":"_blank"}, s:{"cssFloat":"right", "color":"#d71", "fontSize":"10px", "margin":"5px"}, p:ablak.body});
}
...
{	/** beállítási felület **/
	...
	{	var body = $elem("div"),
		...
		$elem("td", {html:"x:", p:tr});
		$elem("td", {html:'<input id="tma_x" size="2" maxlength="4" />', p:tr});
		$elem("td", {html:"y:", p:tr});
		$elem("td", {html:'<input id="tma_y" size="2" maxlength="4" />', p:tr});
		$elem("td", {html:'<span title="' + LD["RAD_TITLE"] + '">' + LD["RADIUS"] + '</span>', p:tr});
		$elem("td", {html:'<input id="tma_r" size="1" maxlength="2" />', a:{"title":LD["RAD_TITLE"]}, p:tr});
		...
		div.style.display = recovery.datas.search.switches.extended ? "" : "none";
	...
	{	var body = $elem("div", {s:{"minHeight":"322px", "width":"400px"}});
		...
		$elem("div", {html:"&nbsp;", a:{"id":"tma_fj"}, s:{"backgroundImage":"url(" + IMG.fjFG + ")", "height":"15px", "width":"0%", "backgroundRepeat":"repeat-x", "whiteSpace":"nowrap", "color":"#cbf5f2", "overflow":"hidden", "textAlign":"center", "fontWeight":"bold"}, p:$elem("div", {a:{"dir":"ltr"}, s:{"backgroundImage":"url(" + IMG.fjBG + ")", "backgroundRepeat":"repeat-x", "width":"398px", "border":"1px inset black"}, p:body})});
	...
	{	function areasGet(html)
		{	if (GM_getValue("quickmode", true) && t35)
			...
			{	var areas = xpath("//area[starts-with(@id, 'a_') and @href]", XPList, $elem("div", {html:html}));
				recovery.datas.search.areas = [];
		...
		{	if (stopAndSaveFlag)
			...
			var min = r.switches.pop[0] == 0 ? Number.NEGATIVE_INFINITY : r.switches.pop[0],
				max = r.switches.pop[1] == 0 ? Number.POSITIVE_INFINITY : r.switches.pop[1];
			...
			var percent = 100 * recovery.datas.stat.fields_done/recovery.datas.stat.max_fields;
		...
		{	if (stopAndSaveFlag)
			...
			var page = $elem("div", {html:html});
			var r = recovery.datas.search;
			var scrpt = xpath("id('content')//script", XPList, page);
			eval("var text_k={}\n" + scrpt.snapshotItem(0).textContent + "var m_c={}\n" + scrpt.snapshotItem(1).textContent);
			...
			{	var col = parseInt(i/7, 10);
				...
				{	case "f1":
					case "f6":
						if (r.switches.extended && !((min <= field.pop && max >= field.pop) && field.race ? r.switches[rFil[field.race-1]] : false))
							break;
						if (r.switches.crop)
							r.hits.push(field);
						break;
		...
		animation();
		// A fő kereső rész
		var zp = zonapontok(z2xy(recovery.datas.search.centerOfSearch), recovery.datas.search.r);
	...
	{	case 1:
			...
			$("tma_extended").checked = recovery.datas.search.switches.extended;
			$("tma_popmin").value = recovery.datas.search.switches.pop[0];
			$("tma_popmax").value = recovery.datas.search.switches.pop[1];
			$("tma_rom").checked = recovery.datas.search.switches.rom;
			$("tma_ger").checked = recovery.datas.search.switches.ger;
			$("tma_gal").checked = recovery.datas.search.switches.gal;
			...
			{	if ($names("xp").length)
				...
				{	coords = z2xy(window.location.href.match(/d=(\d+)&c=/)[1]);
				}
			}
...
{	var ablak = winObj.open(
	...
	{	var div = $elem("div", {html:text, a:{"dir":textDirection}, s:{"border":"2px outset gray", "backgroundColor":"#977", "color":"ivory", "minHeight":"25px", "minWidth":"350px", "MozUserSelect":"none", "cursor":(onClck) ? "pointer":"default"}});
		if (onClck)
...
{	var ablak = winObj.open(
	...
	var def = $elem("img", {a:{"src":IMG.chip48, "title":LD["DEFAULT_TAB"]}, s:{"margin":"2px 6px", "cursor":"pointer"}, p:center});
	var lang = $elem("img", {a:{"src":IMG.flags48, "title":LD["LANGUAGE_TAB"]}, s:{"margin":"2px 6px", "cursor":"pointer"}, p:center});
	var colors = $elem("img", {a:{"src":IMG.palette48, "title":LD["COLORS_TAB"]}, s:{"margin":"2px 6px", "cursor":"pointer"}, p:center});
	...
	var table = $elem("table", {p:def_div});

	var tr = $elem("tr", {p:table});
	$elem("td", {html:"<b>" + LD["RADIUS"] + "</b>", p:tr});
	var td = $elem("td", {p:tr});
	var inp = $elem("input", {a:{"type":"text", "class":"tma", "size":1, "maxlength":2, "value":GM_getValue("default_radius", 0)}, s:{"width":"18px"}, p:td});
	...
	inp = $elem("input", {a:{"type":"text", "class":"tma", "size":1, "maxlength":3, "value":GM_getValue("updateInterval", 8)}, s:{"width":"24px"}, p:$elem("td", {p:tr})});
	td = $elem("td", {p:tr});
	...
	function sqlSetup(){
		...
		txt += ");";
		// syntax markup
		txt = txt.replace(/\b(\d+)(?!['"])\b/g, '<span class="num">$1</span>');
		txt = txt.replace(/(-{2}\s.*)/g, '<span class="comment">$1</span>');
		txt = txt.replace(/([A-Z_]{2,}\b)(?![a-z`])/g, '<span class="inst">$1</span>');
		txt = txt.replace(/`(\w+)`/g, '<span class="id">`$1`</span>');
		...
		// click handlers ;)
		var btns = xpath("//span[@class='btn' and @btn]", XPList, sqlDiv);
		if (btns) {
			...
			for (i = 0; i < opts.snapshotLength; i++) {
				...
				function(){
					...
					switch (sql){
						...
						case "tableName":
							name = prompt(LD.NEW_NAME, SQL.tableName);
							if (name) SQL.tableName = name; break;
						case "insRep": SQL.insRep = !SQL.insRep; break;
						case "id":
							name = prompt(LD.NEW_NAME, SQL.colNames.id[0]);
	...
	var center = $elem("center", {p:col_div});
	...
	["25%", "vas"]];
	for (var i = 0; i < 15; i++)
	{	tr = $elem("tr", {s:{"backgroundColor":(i & 1 ? "#f6f2cc" : "ivory")}, p:table});
		td = $elem("td", {p:tr});
		if (i == 0)
	...
	{	var node = e.target;
		...
		var center = $elem("center", {p:div});
		var inp = $elem("input", {a:{"type":"text", "maxlength":6, "size":6, "value":colorcode.substr(1)}, s:{"color":"grey", "fontWeight":"bolder", "marginBottom":(t35 ? "5px" : "0px")}, p:center});
		var img = $elem("img", {a:{"src":IMG.tick, "height":20, "align":"top"}, p:center});
		$elem("br", {s:{"marginBottom":"5px"}, p:center});
		var span1 = $elem("span", {html:"<b>" + LD["SAMPLE_TEXT"] + "</b>", s:{"fontSize":"10px", "padding":"1px 3px", "border":"1px solid #ddec94", "backgroundColor":"ivory", "color":colorcode}, p:center});
		var span2 = $elem("span", {html:"<b>" + LD["SAMPLE_TEXT"] + "</b>", s:{"fontSize":"10px", "padding":"1px 3px", "border":"1px solid #ddec94", "backgroundColor":"#f6f2cc", "color":colorcode}, p:center});
...
{	if (!recovery.datas.hits.data.length)
	...
	{	var tmp = [];
		...
		{	case "hits":
			...
			case "race":
				for (var i in field)
					tmp[i] = [field[i].race ? field[i].race : recovery.datas.hits.sm == "n" ? "\uFFFF" : 100, dist[i]];
				break;
		}
		return tmp;
	}
	...
	{	var node = event.target;
		...
		{	case "hits":
				...
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"hits"});
				break;
			case "players":
				...
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"players"});
				break;
			case "allies":
				...
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"allies"});
				break;
			case "times":
				...
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"times"});
				break;
			case "pop":
				if (recovery.datas.hits.sortby[0] == "pop")
					recovery.datas.hits.sortby[1] = !recovery.datas.hits.sortby[1];
				else {
					recovery.datas.hits.sortby[0] = "pop";
					recovery.datas.hits.sortby[1] = false;
				}
				idx = rendez(sortby("pop"), recovery.datas.hits.sortby[1]);
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"pop"});
				break;
			case "race":
				if (recovery.datas.hits.sortby[0] == "race")
					recovery.datas.hits.sortby[1] = !recovery.datas.hits.sortby[1];
				else {
					recovery.datas.hits.sortby[0] = "race";
					recovery.datas.hits.sortby[1] = false;
				}
				idx = rendez(sortby("race"), recovery.datas.hits.sortby[1]);
				tables();
				$elem("img", {a:{"src":(recovery.datas.hits.sortby[1] ? IMG.arrowDwn : IMG.arrowUp), "align":"top"}, p:"race"});
				break;
		}
	}
	...
	{	/** A keresés adatai **/
		...
		$elem("td", {html:recovery.datas.hits.sdcr[0], s:{"color": "darkgoldenRod"}, p:tr});
		$elem("td", {html:"&nbsp;&nbsp;", p:tr});
		$elem("td", {html:"<b>" + LD["DATE"] + "</b>", p:tr});
		var date = typeof recovery.datas.hits.sdcr[1] == "string" ? recovery.datas.hits.sdcr[1] : new Date(recovery.datas.hits.sdcr[1]).toLocaleString();
		$elem("td", {html:date, s:{"color": "darkgoldenRod"}, p:tr});

		table = $elem("table", {a:{"border":0}, p:cent});
		tr = $elem("tr", {p:table});
		$elem("td", {html:"<b>" + LD["CENTER"] + "</b>&nbsp;", p:tr});
		$elem("td", {html:"<b>x:</b>", p:tr});
		$elem("td", {html:String(center.x), s:{"color": "darkgoldenRod"}, p:tr});
		$elem("td", {html:"&nbsp;<b>y:</b>", p:tr});
		$elem("td", {html:String(center.y), s:{"color": "darkgoldenRod"}, p:tr});
		$elem("td", {html:"&nbsp;<b>" + LD["RADIUS"] + "</b>", p:tr});
		$elem("td", {html:String(recovery.datas.hits.sdcr[3]), s:{"color": "darkgoldenRod"}, p:tr});
		tr = $elem("tr", {s:{"textAlign":(textDirection == "ltr" ? "left" : "right")}, p:table});
		var inp = $elem("input", {a:{"type":"text", "size":1, "maxlength":2, "class":"tma", "value":String(recovery.datas.hits.unitSpeed || unitSpeed)}, s:{"fontSize":"11px", "width":"16px"}, p:$elem("td", {html:"<b>" + LD["UNITSPEED"] + "</b>&nbsp;", a:{"colspan":6}, s:{"fontSize":"11px"}, p:tr})});
		inp.addEventListener("change", function()
		{	var val = parseInt(this.value, 10);
			val = val < 4 ? 4 : val > 38 ? 38 : val;
		...
		/* Fejléc */
		tr = $elem("tr", {s:{"backgroundColor":"silver", "cursor":"pointer"}, p:table});
		$elem("td", {html:"&nbsp;", a:{"id":"dummy"}, s:{"border":"0px", "cursor":"default"}, p:tr});
		var hits = $elem("td", {a:{id:"hits", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
		$elem("td", {html:LD.PLAYER, a:{id:"players", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
		$elem("td", {html:LD.ALLIANCE, a:{id:"allies", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
		if (recovery.datas.hits.extended){
			$elem("td", {html:LD.POP, a:{id:"pop", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
			$elem("td", {html:LD.RACE, a:{id:"race", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
		}
		$elem("td", {html:LD.TRAV_TIME, a:{id:"times", title:LD.CLICK_TO_SORT}, s:{"border":"1px solid", "borderColor":"#fff #555 #555 #fff"}, p:tr});
		...
		for (var i = sp; i < ep; i++)
		{	tr = $elem("tr", {s:{"backgroundColor":(i & 1 ? "#f6f2cc" : "ivory")}, p:table});
			$elem("td", {html:i + 1, s:{"textAlign":"center", "backgroundColor":"silver", "borderColor":"#fff #555 #555 #fff"}, p:tr});
			var td = $elem("td", {p:tr});
			...
			{	/** 1 **/
				switch (type)
				{	case "f1": $elem("div", {html:"<b class='tma_" + type + "n'>9</b>", p:td}); break;
					case "f6": $elem("div", {html:"<b class='tma_" + type + "n'>15</b>", p:td}); break;
					case "f3": $elem("div", {html:"<b class='tma_" + type + "n'>6</b>", p:td}); break;
					case "f7": $elem("div", {html:"<b class='tma_" + type + "n'>7</b>", p:td}); break;
					case "f8": $elem("div", {html:"<b class='tma_" + type + "n'>7</b>", p:td}); break;
					case "f9": $elem("div", {html:"<b class='tma_" + type + "n'>7</b>", p:td}); break;
					default: $elem("div", {html:"<b class='tma_" + type + "n'>5</b>", p:td});
				}
				/** 2 **/
				switch (type)
				{	case "f2":
					case "f11":	$elem("div", {html:"<img height='9' src='" + IMG.vas + "'/>", p:td}); break;
					case "f4":
					case "f10": $elem("div", {html:"<img height='9' src='" + IMG.agyag + "'/>", p:td}); break;
					case "f5":
					case "f12": $elem("div", {html:"<img height='9' src='" + IMG.fa + "'/>", p:td}); break;
					default: $elem("div", {html:"<img height='9' src='" + IMG.gabona + "'/>", p:td});
				}
				/** 3 **/
				var uri = field[idx[i]].url.split("|");
				$elem("div", {html:"<a class='tma_" + type + "' " + href + "karte.php?d=" + uri[0] + "&c=" + uri[1] + "'>" + field[idx[i]].name + "</a>", p:td});
				/** 4 **/
				var crds = z2xy(uri[0]);
				$elem("div", {html:"<a class='tma_" + type + "' " + href + "karte.php?z=" + uri[0] + "'>(" + crds.x + "|" + crds.y + ")</a>", p:td});
				/** 5 bonus **/
				if (bonus)
			...
			{	if (type == "o12")
					$elem("div", {html:"<b class='tma_" + type + "n'>50%</b>", p:td});
				else
					$elem("div", {html:"<b class='tma_" + type + "n'>25%</b>", p:td});
				/** 2 **/
				switch (type)
				{	case "o1":
					case "o2": $elem("div", {html:"<img height='9' src='" + IMG.fa + "'/>", p:td}); break;
					case "o3": $elem("div", {html:"<img height='9' src='" + IMG.gabona + "'/><img height='9' src='" + IMG.fa + "'/>", p:td}); break;
					case "o4":
					case "o5": $elem("div", {html:"<img height='9' src='" + IMG.agyag + "'/>", p:td}); break;
					case "o6": $elem("div", {html:"<img height='9' src='" + IMG.gabona + "'/><img height='9' src='" + IMG.agyag + "'/>", p:td}); break;
					case "o7":
					case "o8": $elem("div", {html:"<img height='9' src='" + IMG.vas + "'/>", p:td}); break;
					case "o9": $elem("div", {html:"<img height='9' src='" + IMG.gabona + "'/><img height='9' src='" + IMG.vas + "'/>", p:td}); break;
					default: $elem("div", {html:"<img height='9' src='" + IMG.gabona + "'/>", p:td});
				}
				/** 3 **/
				var uri = field[idx[i]].url.split("|");
			...
			if (recovery.datas.hits.extended) {
				...
				$elem("td", {s:{textAlign:"center"}, html:field[idx[i]].race ? recovery.datas.hits.sm == "q" ? '<img src="img/x.gif" class="unit u' + (field[idx[i]].race * 10 -1) + '"/>' : field[idx[i]].race : "-", p:tr});
			}
			$elem("td", {html:utido(dist[idx[i]]), p:tr});
		}
		/** Lapozók **/
		tr = $elem("tr", {p:table});
		var td = $elem("td", {a:{"colspan": recovery.datas.hits.extended ? 7 : 5}, s:{"textAlign":"center", "backgroundColor":"silver", "borderColor":"#fff #555 #555 #fff"}, p:tr});
	...
	var dist = []; // Kiszámoljuk a találatok távolságait a keresés középpontjához képest. dist tömbben vannak az eredmények
	var center = z2xy(recovery.datas.hits.sdcr[2]);
	...
	var cent = $elem("center", {s:{"whiteSpace":"nowrap"}, p:div});
	...
	var letter = $elem("img", {a:{"src": IMG.letter48, "title": LD["IGM"]}, s:{"margin": "0px 5px", "cursor": "pointer"}, p:div});
	var bonus = $elem("img", {a:{"src": IMG.buza48, "title": LD["CROP_BONUS"]}, s:{"margin": "0px 5px", "cursor": "pointer"}, p:div});
...
{	function bonusScan(html)
	...
	ablak = winObj.wins[ablak];
	var fj = $elem("div", {html:"&nbsp;", s:{"backgroundImage":"url(" + IMG.fjFG + ")", "height":"15px", "width":"0%", "backgroundRepeat":"repeat-x", "whiteSpace":"nowrap", "color":"#cbf5f2", "overflow":"hidden", "textAlign":"center", "fontWeight":"bold"}, p:$elem("div", {a:{"dir":"ltr"}, s:{"backgroundImage":"url(" + IMG.fjBG + ")", "backgroundRepeat":"repeat-x", "minWidth":"160px", "border":"1px inset black"}, p:ablak.body})});
	var percent = 0;
...
{	var ablak = winObj.open(
	...
	{	ablak.body.innerHTML = "";
		...
		$elem("td", {html:"x:", p:tr});
		var x1 = $elem("input", {a:{"k":0, "size":2, "maxlength":4, "value":r.koords[0]}, p:tr});
		$elem("td", {html:"y:", p:tr});
		var y1 = $elem("input", {a:{"k":1, "size":2, "maxlength":4, "value":r.koords[1]}, p:tr});
		tr = $elem("tr", {p:table});
		$elem("td", {html:LD.TO, p:tr});
		$elem("td", {html:"x:", p:tr});
		var x2 = $elem("input", {a:{"k":2, "size":2, "maxlength":4, "value":r.koords[2]}, p:tr});
		$elem("td", {html:"y:", p:tr});
		var y2 = $elem("input", {a:{"k":3, "size":2, "maxlength":4, "value":r.koords[3]}, p:tr});
		...
		var d = distance({x:r.koords[0], y:r.koords[1]}, {x:r.koords[2], y:r.koords[3]});
		var td_Dist = $elem("td", {html:String((d-parseInt(d, 10) ? d.toFixed(3) : d)), s:{"color":"green"}, p:tr});
		tr = $elem("tr", {p:table});
		$elem("td", {html:LD.TRAV_TIME, p:tr});
		var td_Time = $elem("td", {html:timeToString((d/r.unitSpeed) * 1000 * 3600), s:{"color":"royalBlue"}, p:tr});
	}
	var coords1 = {x:0, y:0};
	var coords2 = {x:0, y:0};
	try
	...
	{	if ($names("xp").length)
		...
		{	coords2 = z2xy(window.location.href.match(/d=(\d+)&c=/)[1]);
		}
	}
...
{	switch (recovery.datas.update.phase)
	...
	$elem("img", {a:{"src":IMG.flower48, "align":"top"}, p:ablak.body});
	$elem("center", {html:"<b>" + LD["NEWVER"] + "&nbsp;&nbsp;(" + newver + ")</b>", s:{"position":"relative", "top":"-34px", "color":"#e73", "fontSize":"1.1em"}, p:ablak.body});
	var div = $elem("div", {a:{"dir":"ltr"}, p:ablak.body});
	var flag = true;
	for (var i in news)
	{	if (news[i][0] == "!")
		{	$elem("b", {html:"<u>" + news[i].substr(1) + "</u>", s:{"margin":"0px 5px", "color":"darkGoldenRod"}, p:div});
			flag = true;
			continue;
		}
		if (flag)
		{	var ul = $elem("ul", {s:{"color":"darkGoldenRod"}, p:div});
			flag = false;
		}
	...
	for (var i in bugfix)
	{	if (bugfix[i][0] == "!")
		{	$elem("b", {html:"<u>" + bugfix[i].substr(1) + "</u>", s:{"margin":"0px 5px", "color":"darkRed"}, p:div});
			flag = true;
			continue;
		}
		if (flag)
		...
		$elem("li", {html:bugfix[i], s:{"color":"darkRed"}, p:ul});
	}

	div = $elem("div", {p:ablak.body});
	var yes = btnMaker(LD["YES"]);
	var no = btnMaker(LD["NO"]);
	var table = $elem("table", {a:{"align":"center", "border":0}, p:div});
	var tr = $elem("tr", {p:table});
	$elem("td", {html:LD["UPDATENOW"], s:{"fontWeight":"bold", "color":"#353"}, p:tr});
	var td = $elem("td", {p:tr});
	td.appendChild(yes);
	td = $elem("td", {p:tr});
	td.appendChild(no);
