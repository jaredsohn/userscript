// ==UserScript==
// @name        Multiple Windows Live IDs 2
// @namespace   http://userscripts.org/scripts/show/118175
...
window.addEventListener("load", function(){
	window.setTimeout(function(){
		...
		var profileString = GM_getValue("MWLID2.profiles"),
			profiles = [
		...
		function paint(){
			...
			for(var i in profiles){
				...
				profileDiv.classList.add(contrastDark ? "dark" : "light");
				...
				profileImg.setAttribute("src", profile.photo || profile.img || (contrastDark ? imagePhotoDark : imagePhotoLight));
				...
				if(i != 0){
					...
					profileManageLeft.setAttribute("src", contrastDark ? imageLeftDark : imageLeftLight);
					...
					profileManageLeft.addEventListener("click", proxy(function(_event, _profile, _i){
						...
						if(editAccountId.value == index) editAccountId.value = index - 1;
				...
				if(i != profiles.length - 1){
					...
					profileManageRight.setAttribute("src", contrastDark ? imageRightDark : imageRightLight);
				...
				profileManageEdit.setAttribute("src", contrastDark ? imageEditDark : imageEditLight);
				profileManage.appendChild(profileManageEdit);
				profileManageEdit.addEventListener("click", proxy(function(_event, _profile, _i){
					_event.stopPropagation();
					
					var signInTD = document.getElementById("signInTD");
					signInTD.style.display = "none";
					
					var editAccountTD = document.getElementById("editAccountTD");
					editAccountTD.style.display = "block";
				...
				profileManageDelete.setAttribute("src", contrastDark ? imageDeleteDark : imageDeleteLight);
			...
			addAccountBtnDiv.classList.add("profile");
			addAccountBtnDiv.classList.add("addAccountBtn");
			addAccountBtnDiv.setAttribute("title", "Add account");
			addAccountBtnDiv.style.backgroundColor = "#0072C6";
			addAccountBtnDiv.addEventListener("click", function(_event){
				var signInTD = document.getElementById("signInTD");
				signInTD.style.display = "none";
			...
			addAccountBtnImg.setAttribute("src", imageAddLight);
		...
		editAccountDiv.innerHTML = 
			...
			'<div id="i0272" class="signInHeader" style="height: 80px;">' +
				'<h1><img src="' + imageHeader + '" alt="Multiple Windows Live IDs 2" /></h1>' +
			'</div>' +
			...
			'<div style="width: 320px;" class="floatLeft">' +
				...
				'<div class="signUpFloat">Multiple Windows Live IDs 2. <a class="TextSemiBold" href="http://userscripts.org/scripts/show/118175" target="_blank">More info...</a></div>' +
			'</div>';
		mainTD.appendChild(editAccountDiv);
		
		var editAccountHeader1 = document.getElementById("editAccountHeader1");
		var editAccountHeader2 = document.getElementById("editAccountHeader2");
		
		var editAccountId = document.getElementById("editAccountId");
		var editAccountName = document.getElementById("editAccountName");
		var editAccountMail = document.getElementById("editAccountMail");
		var editAccountPass = document.getElementById("editAccountPass");
		var editAccountPhoto = document.getElementById("editAccountPhoto");
		var editAccountColor = document.getElementById("editAccountColor");
		
		var editAccountMailError = document.getElementById("editAccountMailError");
		var editAccountPassError = document.getElementById("editAccountPassError");
		...
		if(addPassMask) addPassMask(editAccountPass);
		...
		document.getElementById("editAccountSubmit").addEventListener("click", function(){
			...
			var signInTD = document.getElementById("signInTD");
			signInTD.style.display = "block";
			
			var editAccountTD = document.getElementById("editAccountTD");
			editAccountTD.style.display = "none";
		...
		function addPassMask(elm){
			var img = document.createElement("img");
			img.classList.add("passMask");
			img.setAttribute("src", imagePassMask);
		...
		if(addPassMask) addPassMask(document.getElementById("i0118"));  // Microsoft password;
	}, 100);
});
...
// ==UserStats==
// Chars (excl. spaces): 29.226
// Chars (incl. spaces): 31.744
// Words: 1.425
// Lines: 449
// ==/UserStats==