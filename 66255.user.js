// ==UserScript==
// @name		Script Updater RU (userscripts.org)
// @namespace		PhasmaExMachina
// @description		Dead simple user script update checker for use with your user scripts. Includes simple means of showing your version history through meta tags.RU
// @version		1.1
//
...
ScriptUpdater = {
	...
	getLatestVersion:function(scriptId, callback) {	
		if(typeof(callback) != 'function')
			alert("Ошибка обновления скрипта:\n\n scriptUpdater.getLatestVersion() требуется callback функция в качестве третьего параметра."); 
	...
	showNotice:function() {
		if(ScriptUpdater.meta.name && ScriptUpdater.meta.version) {	
			...
			noticeWrapper.id = "ScriptUpdater" + ScriptUpdater.scriptId + "BodyWrapper";
				...
				html.push('" align="absmiddle" style="margin-top:-2px;"/><a href="http://userscripts.org/scripts/show/66255" target="_blank" title="Подсказка о Userscripts.org Script Updater v');
				html.push(ScriptUpdater.meta.version);
				html.push('">Обновление скриптов от Ika-info</a></h1>');				if(!ScriptUpdater.forceNoticeEnabled) {
					html.push('<p>Появилась новая версия скрипта <strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="Перейти на страницу скрипта">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong>, доступная для установки.</p>');
				} else {
					html.push('<p><strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="Перейти на страницу скрипта" style="margin:0; padding:0;">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong></p>');
				}				
				if(ScriptUpdater.scriptCurrentVersion) {
					html.push('<p>У вас уcтановленна версия <strong>');
					html.push(ScriptUpdater.scriptCurrentVersion)
					html.push('</strong>. Последняя версия <strong>');
					html.push(ScriptUpdater.meta.version);
					html.push('</strong></p>');
				}
				
				if(ScriptUpdater.meta.history) {
					html.push('<h2>История версий:</h2><div id="ScriptUpdater' + ScriptUpdater.scriptId + 'History">');
					...
					if(typeof(ScriptUpdater.meta.history) != 'string') {
						...
						[, version, change] = ScriptUpdater.meta.history.match(/(\S+)\s+(.*)$/);
						history[version] = typeof(history[version]) == 'undefined' ? new Array() : history[version];
						history[version].push(change);
					}
				...
				*/
				...
				html.push('<input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'CloseButton" value="Назад" style="background-image:url(');
				html.push(ScriptUpdater.icons.close);
				html.push(')"/><input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'BodyInstall');
				html.push(ScriptUpdater.scriptId);
				html.push('" value="Обнови" style="background-image:url(');
				html.push(ScriptUpdater.icons.install);
				html.push(');"/>');
				html.push('Проверять скрипт на обновления ');

				html.push('<select id="ScriptUpdater' + ScriptUpdater.scriptId + 'Interval"> \
								<option value="3600000">каждый час </option>\
								<option value="21600000">каждые 6 часов </option>\
								<option value="86400000">каждый день </option>\
								<option value="604800000">каждую неделю </option>\
