...
// @include     *://*/view.php?id=*
// @include     *://*/index.php?view=play_grid*
// @include     *://*/grid.php?
// @require     http://userscripts.org/scripts/source/163372.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link*.com/*
// @include     *://*ysear.ch/*
...
top == self){
	...
	div.css({color:'#000',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:200,height:20,background:'#00C7F8',border:'2px solid blue',bottom:10,right:10})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px arial',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:200,height:40,background:'#00C7F8',border:'2px solid blue',bottom:45,right:10})
	jQuery.ajax({
	...
	div2.append(
		$('<h4 class="titleBheepH">').text('AURORA AUTO CLICKER  Edited by : mr.BheepH')		
	)	
	...
	setTimeout(function(){
		...
		$.each($('a'),function(k,v){
			if($(v).text().toLowerCase().indexOf('anti bot') >= 0 ){
		...
		function rec(ctr){
			...
			if(arr[ctr]){
				...
				if(adsWind == null){
					 div.css({background:'#FCD7C7',border:'2px solid yellow'}).text('Allow Popup Blocker !!!')
				}else{
...
if(wparent.location.href.indexOf('gpt.php?v=read') != -1 && self == top){
	var div = $('<div>');
	div.css({font:'bold 10px arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:60,height:20,background:'#00C7F8',border:'2px solid blue',top:10,left:10})
		.text('');
...
self == top){
	...
	var div = $('<div>');
	div.css({font:'bold 10px arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:60,height:20,background:'#00C7F8',border:'2px solid blue',top:1,left:1})
		.text('');
	$('html').css({position:'relative'}).append(div);
	...
	if($('frame:last').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('scarlet-clicks')!=-1)cheatCheck = [2,7];
		...
		if(wparent.location.host.indexOf('earningnation')!=-1)cheatCheck = [47,55,58,36,62,31,51,17];
		if(wparent.location.host.indexOf('adsclickers')!=-1)cheatCheck = [15];
		...
		if(wparent.location.host.indexOf('fjptc')!=-1)cheatCheck = [10,13,25,19,33,2];
		if(wparent.location.host.indexOf('clixten')!=-1)cheatCheck = [45];
		if(wparent.location.host.indexOf('juicybux')!=-1)cheatCheck=[2,7,12];
		if(wparent.location.host.indexOf('lovemyclicks')!=-1)cheatCheck=[1,6];
		if(wparent.location.host.indexOf('camelfamilyptc')!=-1)cheatCheck=[6,1];
