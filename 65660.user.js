// ==UserScript==
// @name        Utopia Kingdoms Interface Tools
// @description Major revamp of Utopia Kingdoms interface
...
var InfantryList = {
  ...
  "u5": null, "u6": null, "u7": null, "u8": null, "20": null
};
...
var World = /Utopia Kingdoms \/\/ World (\d+)/i.
  exec(document.title);
World ? World = World[1] : World = "0";
var UserName = document.getElementById("user").value;
var Server = /game(\d+)\.utopiakingdoms.com/i.exec(window.location);
Server ? Server = Server[1] : Server = "";

var PlayerCastles =
  eval(GM_getValue(World + "_" + UserName + "Castles")) ||
  [[].concat([null, HomeName], Home)];
var PlayerMerchants =
  eval(GM_getValue(World + "_" + UserName + "Merchants")) || [];
var UnitNameToID = [];
var UnitMaxID = [];
for (var N in Units) {
...
for (var N = 1; N < Buildings.length; N++) {
  BuildingNameToID[Buildings[N][0]] = N; BuildingMaxID.push(N);
}
BuildingMaxID.sort(CompNum).reverse();
BuildingMaxID = BuildingMaxID[0];

var DefaultAttack = GM_getValue(World + "_" + UserName + "DefaultAttack", "");
var ArcherLevel = Number(GM_getValue(World + "_" + UserName + "Archer", 0));
var MysticLevel = Number(GM_getValue(World + "_" + UserName + "Mystic", 0));
var ScoutLevel = Number(GM_getValue(World + "_" + UserName + "Scout", 0));
var MerchantLevel = Number(GM_getValue(World + "_" + UserName + "Merchant", 0));
var CavalryLevel = Number(GM_getValue(World + "_" + UserName + "Cavalry", 0));
var WarriorLevel = Number(GM_getValue(World + "_" + UserName + "Warrior", 0));
var DefenderLevel = Number(GM_getValue(World + "_" + UserName + "Defender", 0));
var StealFrac = Number(GM_getValue(World + "_" + UserName + "StealFrac", 0));
var HealThreshold = Number(GM_getValue(World + "_" + UserName + "MinHeal", 1));
var ScavengeThreshold =
  Number(GM_getValue(World + "_" + UserName + "MinScavenge", 1));
var TimeOffset =
  Number(GM_getValue(World + "_" + UserName + "ServerOffset", 0));
var MaxX = Number(GM_getValue(World + "_" + UserName + "MaxX", 200));
var MaxY = Number(GM_getValue(World + "_" + UserName + "MaxY", 200));
...
for (var Unit in Units) {
  ...
  if (Bonus)
    UnitUpgrades[Unit] = [
      Number(GM_getValue(World + "_" + UserName + Unit + "a", 0)), Bonus,
      Number(GM_getValue(World + "_" + UserName + Unit + "d", 0)), 10
    ];
}
...
function GetCastleName(arguments) {
  ...
  if (! FoundCastle[0].length) {
    FoundCastle[1] = eval(GM_getValue(
      World + "_MapData_" + Coords[0] + "_" + Coords[1],
      "[]"
    ));
    if (FoundCastle[1][2]) {
...
function ParseClipboardUnits(InText) {
  ...
  GM_setValue(World + "_CopyArmy", uneval(ArmyArray));
}
...
if (Node) {
  ...
  var PageMatch = new RegExp(
    "^http://game.*\\.utopiakingdoms\\.com/pohod\\.php(?:\\?" +
      ".*attackx=(\\d+)&attacky=(\\d+))?",
    "i"
...
/* // maro edit -- remove local time conversion
  var LocalSeconds =
...
var PatDate = new RegExp(
  "(?:Mo|Tu|We|Th|Fr|Sa|Su)?(\\d{4,4})-(\\d\\d)-(\\d\\d)(?:\\s*|@|<br />)" +
    "(\\d\\d):(\\d\\d):(\\d\\d)",
  "i"
);
...
function ReleaseTroops() {
  ...
  for (var N = 1; N <= 4; N++) {
    DoRequest(
      "http://game" + Server +
        ".utopiakingdoms.com/barracks.php?unit_type=" + String(N),
      N
    );
  }
  var Attempts = 0;
...
if (InsertionPoint) {
  ...
  // maro edit -- addded a few more items in the list below.
  LinkNode.innerHTML =
    "<a href='market.php?action=4'>Convert Res</a> | " +
    "<a href='market.php?action=2'>Send Res</a> | " +
    "<a href='vip_status.php'>Overview</a> | " +
    "<a href='forum.php'>Clan Forum</a> | " +
    "<a href='editclan.php?id=45'>Clan Admin</a> | " +
    "<a href='vip_search.php'>VIP Search</a>";
  InsertionPoint.insertBefore(LinkNode, InsertionPoint.firstChild);
}

var InsertionPoint = document.getElementsByClassName("blacksmith")[0]; // maro change barracks to blacksmith
if (InsertionPoint) {
  var LinkNode = document.createElement("li");
  LinkNode.setAttribute("class", "marches");
  LinkNode.innerHTML =
    "<a href='javascript:void(0);' id='GM_ReleaseTroops'>Release Troops</a>";
...
var MarchInfo = new RegExp(
  "^http://game.*\\.utopiakingdoms\\.com/info\\.php\\?what=pohod&posted_id=" +
    "\\d+$",
  "i"
).test(window.location);
...
function HMS(Seconds) {
  ...
  return String(Hours) + "h" +
    Zero(String(Minutes), 2) + "m" + Zero(String(Seconds), 2) + "s";
}

// convert DATE object into DDyyyy-mm-dd@hh:mm:ss format
function DateFormat(NewDate, Break) {
  return ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][NewDate.getDay()] +
...
function MarchTime(Dist, Speed, Separator, SpeedLimiter, ShowServer) {
  ...
  for (var N = 0; N < SpeedLimiter.length; N++) {
    ...
    TimeString += "<span class='hovertext' title='One-Way (" + String(N) +
      ...
      (ShowServer
        ? "<br /><span style='font-weight: lighter;'>" +
          "<i>Estimated</i> Server Arrival: " + DateFormat(OneWayServer) +
          "</span>"
        : ""
      ) + "<br />";
  }
...
function MerchantCapacity() {
  ...
  NewMessage.innerHTML = "Total Res: " + String(GlobalTotal);
  for (var N = 0; N < PlayerMerchants.length; N++) {
    if (Home[0] == PlayerMerchants[N][0] && Home[1] == PlayerMerchants[N][1]) {
      ...
      NewMessage.innerHTML += "; Max Merchant: " + String(MerchantLimit) +
        "; Extra: " + String(Extra);
      break;
    }
  }
...
function TrapCastleUpdate(Event) {
  ...
  window.setTimeout(
    GM_setValue, 0, World + "_" + UserName + "Castles",
    uneval(TrapCastleUpdateArray)
  );
}
...
function PasteCoords(Event) {
  if (
    ...
    if (Coords) {
      if (
        /http:\/\/game.*\.utopiakingdoms\.com\/map\.php/.test(window.location)
      ) {
        document.getElementById("searchX").value = Coords[1];
        document.getElementById("searchY").value = Coords[2];
      } else if (
        /^http:\/\/game.*\.utopiakingdoms\.com\/pohod\.php/i.
          test(window.location) &&
...
function ResetPrefs() {
  if (document.getElementById("GM_ResetCheck").checked) {
    ...
    var Pat = new RegExp(
      "^" + document.getElementById("GM_ResetWorld").value + "_", "i"
    );
    List.forEach(
      function (Option) {
...
var Configs = [
  "Warrior", "Archer", "Cavalry", "Defender", "Scout", "Merchant", "Mystic",
  ["MinHeal", "Minimum Heal"], ["MinScavenge", "Minimum Scavenge"],
  ["StealFrac", "Steal Fraction"], ["WorldSpeed", "World Speed", "1"],
  [
    "ServerOffset",
    "Server Offset, Minutes (recommend " +
      StripDecimals(Round(InitOffset, 0.01), 2) + " or " +
      StripDecimals(Round(1440 + InitOffset, 0.01), 2) + ")",
    "0"
  ],
  ["MaxX", "Maximum X", "200"], ["MaxY", "Maximum Y", "200"],
  ["DefaultAttack", "Default Attack Formation", ""]
];
for (var N in Units) {
  if (
    (
      N in InfantryList || N in ArcherList ||
      N in CavalryList || N in MysticList
    ) &&
    ! (N in Unupgraded)
  ) {
    Configs.push([N + "a", Units[N]["name"] + " Attack"]);
    Configs.push([N + "d", Units[N]["name"] + " Defence"]);
  }
}
...
Configs.forEach(
  function(Config) {
    ...
    var Default = GM_getValue(World + "_" + UserName + Name, "0");
...
ClearStuff.innerHTML =
  "<input id='GM_Reset' type='button' value='Delete ALL Data (Map Too)' />" +
  "<br />For world: " +
  "<input id='GM_ResetWorld' type='text' size='10' value='0' />" +
  "<br />Really! (This could take a long time.) " +
...
// maro's edits -- remove the scroll thing
var scrollThing = document.getElementById("scroll");
scrollThing.parentNode.removeChild(scrollThing);

// maro's edit -- get rid of the message to motivate
// http://game2.utopiakingdoms.com/pohod.php?sent=1&motivated=0
// this has to be below the next section because window.name is cleared there... for some reason
var PageMatch = /^http:\/\/game.*\.utopiakingdoms\.com\/pohod\.php.*sent=1&motivated=0/i.test(window.location);
if (PageMatch && window.name) {
    var msg = document.getElementsByClassName("infosmall")[0];
    msg.innerHTML = "                March Sent Successfully!                                <br/>"
}

/******************************************************************************/
// redir coords on success

var PageMatch = /^http:\/\/game.*\.utopiakingdoms\.com\/pohod\.php\?.*sent=1/i.
  test(window.location);
if (PageMatch && window.name) {
...
function QuickGo(Event) {
  ...
  var SubmitButton = document.evaluate(
    "//input[@type='submit'][@value='Continue']", document, null,
    XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  ...
  switch (ID[1]) {
    case "QAtt":
    case "Attack": MarchType.selectedIndex = 0; break;
    case "QScav":
    case "Scavenge": MarchType.selectedIndex = 2; break;
    case "QTransfer":
    case "Transfer": MarchType.selectedIndex = 5; break;
    case "Spy":
      var QWNode = document.forms.namedItem("form1").elements.namedItem("u9");
      var Temp = Number(QWNode.value);
      SetAll(false); QWNode.value = String(Math.max(Temp, 1));
      MarchType.selectedIndex = 3;
      break;
    case "QSupport":
    case "Support": MarchType.selectedIndex = 1; break;
  }
  SubmitButton.click();
}
...
function UpdateMarchNums(Event) {
  ...
  var MapData = GM_getValue(
    World + "_MapData_" + String(Target[0]) + "_" + String(Target[1]),
    ""
  );
  var CastleName = "";
  MapData ? CastleName = eval(MapData)[4] || "" : CastleName = "";
  document.getElementById("GM_CastleLabel").innerHTML =
    " <a href='http://game" + Server +
        ".utopiakingdoms.com/map.php?setx=" +
  ...
  for (var N = 0; N < Form.elements.length; N++) {
    DamageBonus = 0;
    if (Form[N].name in Units) {
      var Marched = Number(Form[N].value);
      if (Form[N].name in ArcherList) {
        DamageBonus += ArcherLevels[ArcherLevel];
      }
      if (Form[N].name in MysticList) {
        DamageBonus += MysticLevels[MysticLevel];
      }
      if (Form[N].name in UnitUpgrades) {
        DamageBonus +=
          UnitUpgrades[Form[N].name][0] * UnitUpgrades[Form[N].name][1];
      }
      Temp = Math.floor(
        Marched * (1 + DamageBonus / 100) *
        Units[Form[N].name]["building"] * Units[Form[N].name]["wallcrasher"]
      );
      Damage += Temp;
      if (Form[N].name in MysticList) Mystic += Temp;
      Encumbrance += Marched * Units[Form[N].name]["carry"];
      Pop += Marched * Units[Form[N].name]["pop"];
      NonMysticPop += Marched * Units[Form[N].name]["pop"] *
        ! (Form[N].name in MysticList);
      if (Marched > 0) {
        Speed = Math.max(
          Speed,
          Math.round(
            Units[Form[N].name]["speed"] / (1 + ScoutLevels[ScoutLevel] / 100)
          )
        );
      }
    }
  ...
  Node = document.getElementById("GM_Mystic");
  ...
  switch (Type) {
    case ("CastleSelect"):
      document.evaluate(
        "//form[@name='form1']/descendant::*[@id='type']",
        document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
      ).singleNodeValue.selectedIndex = 5;
...
function CreateCancelTimer(Event) {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function (Response) {
      ...
      Arrives =
        LocalTime(
          /Arrives.*?>([\-\d:\s]+?)</i.exec(Response.responseText)[1]
        );
      Returns =
        LocalTime(
          /Comes back.*?>([\-\d: ]+?)</i.exec(Response.responseText)[1]
        );
...
var PageMatch =
  /^http:\/\/game.*\.utopiakingdoms\.com\/pohod\.php/i.test(window.location);
var Formation = document.getElementById("army_orders");
if (PageMatch && ! Formation) {
  ...
  var PatCargo = new RegExp(
    "(\\d+?)Gold[\\s\\S]*?(\\d+?)Iron[\\s\\S]*?(\\d+?)Wood[\\s\\S]*?" +
      "(\\d+?)Food[\\s\\S]*?(\\d+?)People",
    "i"
  );
  var PatCoords = /(\d+):(\d+)/i;
  ...
  for (var N = 0; N < SelNodes.snapshotLength; N++) {
    ...
    if (Match) {
      ...
      LinkNode.innerHTML =
        "<a href='http://game" + Server +
        ".utopiakingdoms.com/pohod.php?attackx=" +
  ...
  if (Castles.snapshotLength) {
    ...
    GM_setValue(World + "_" + UserName + "Castles", uneval(CastleArray));
  }
  ...
  var Nodes = document.evaluate(
    "//a[starts-with(@href, 'info.php?what=pohod')]/ancestor::tr[1]",
    document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  var PatCoords = /(\d+):(\d+)/i;
  for (var N = 0; N < Nodes.snapshotLength; N++) {
    ...
    var TempName =
      eval(GM_getValue(World + "_MapData_" + Coords[0] + "_" + Coords[1], ""))
        ||
      "";
    ...
    var Node = Nodes.snapshotItem(N).childNodes[7];
  ...
  function SetMaximum(ID, Value) {
    var Form = document.forms.namedItem("form1");
    var Item = Form.elements.namedItem(ID);
    Item.focus();
    Item.value = Value;
    Item.blur();
  }
  ...
  function SetAll(On) {
    var Form = document.forms.namedItem("form1");
    for (var N = 0; N < Form.elements.length; N++) {
      if (/^u\d+$/i.test(Form[N].name)) {
        if (On) {
          SetMaximum(Form[N].name, FindNum(Form[N]));
        } else {
          SetMaximum(Form[N].name, "0");
        }
      }
    }
  }
  ...
  function SingleInfantry() {
    var Form = document.forms.namedItem("form1");
    for (var N = 0; N < Form.elements.length; N++) {
      if (/^u(1|2|3|4)$/i.test(Form[N].name)) {
        var Available = Number(FindNum(Form[N]));
        SetMaximum(Form[N].name, Available ? "1" : "0");
      }
    }
  }
  ...
  function PasteArmyMarch(Event) {
    var ArmyArray = eval(GM_getValue(World + "_CopyArmy", ""));
    if (ArmyArray) {
      var Form = document.forms.namedItem("form1").elements;
      var NotEnough = false;
      for (var N = 1; N < ArmyArray.length; N++) {
        if (ArmyArray[N]) {
          var TextBox = Form.namedItem("u" + String(N));
          var Available = Number(FindNum(TextBox)) || 0;
          if (Available < ArmyArray[N]) NotEnough = true;
          TextBox.value = String(Math.min(ArmyArray[N], Available));
        }
      }
      if (NotEnough) alert("Warning: not enough troops to paste all.");
    }
  }
  ...
  AllNone.innerHTML = "<label>Enter:</label><div class='right'>" +
    "<a href='javascript:void(0)' id='GM_All'>All</a> / " +
    "<a href='javascript:void(0)' id='GM_None'>None</a> / " +
    "<a href='javascript:void(0)' id='GM_Infantry'>Single Infantry</a> / " +
    "<a href='javascript:void(0)' id='GM_Paste'>Paste</a> / " +
    "<a href='javascript:void(0)' id='GM_Invert'>Invert</a>";
  ...
  ["Attack", "Spy", "Transfer", "Support", "Scavenge"].forEach(
    function (Option) {
      var NewNode = document.createElement("input");
      NewNode.value = Option + " Now";
      NewNode.type = "button";
      NewNode.id = "GM_" + Option + "0";
  ...
  var Formations = GM_getValue(World + "_" + UserName + "Formations", "");
  Formations ? Formations = eval(Formations) : Formations = [["Default", ""]];
  ...
  FormationNode.innerHTML = "Formation:<br /><select id='GM_Formation'>" +
    TempText + "</select>";
  ...
  for (var N = 0; N < SelNodes.length; N++) {
    if (SelNodes[N].innerHTML == DefaultAttack) {
      SelNodes[N].selected = true; Found = true;
      break;
    }
  }
  if (! Found && DefaultAttack) alert("Default attack formation not found.");
  ...
  [
    "Speed", "Damage", "Mystic", "Encumbrance", "Non-Mystics", "Castles"
  ].forEach(
    function (Option) {
  ...
  QuickNode.innerHTML = "<label>Quick Roads: </label><input id='" +
  ...
  for (var N = 0; N < Form.elements.length; N++) {
    if (Form[N].name in Units) {
      ...
      NewLinks.innerHTML =
        ...
        " <a href='javascript:void(0);' class='GM_Unit_ChangeOne'>+1</a>" +
        " <a class='GM_Unit_" + Form[N].name + "_0' href='javascript:void(0);'" +
        ">0%</a>" +
        " <a class='GM_Unit_" + Form[N].name + "_" + String(Math.floor(Num / 3)) +
        "' href='javascript:void(0);'>33%</a>" +
        " <a class='GM_Unit_" + Form[N].name + "_" + String(Math.floor(Num / 2)) +
        "' href='javascript:void(0);'>50%</a>" +
        " <a class='GM_Unit_" + Form[N].name + "_" + String(Num) +
...
function CalcMarch2Time(Event) {
  ...
  var OneWay = document.getElementById("z").innerHTML;
  ...
  var TargetDistance = Dist([
    document.getElementById("att_x").value,
    document.getElementById("att_y").value
  ]);
  var NumEffSpeed = Round(OneWay / 60 / TargetDistance, 0.01);
  var EffSpeed = StripDecimals(NumEffSpeed, 2);
  var SpeedNode = document.evaluate(
    "//div[@class='left'][contains(text(), 'Speed:')]/following-sibling::div",
    document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  SpeedNode.innerHTML = SpeedNode.innerHTML.split(" ")[0] + " (Effective " +
    EffSpeed + ")";
  if (! NewNode) {
    NewNode = document.createElement("div");
  ...
  ServerArriveNode.innerHTML = "Server Arrival: " +
    HMS((ToSeconds(ServerTime) + OneWay) % 86400).replace(/h/, ":").
      replace(/m/, ":").replace(/s/, "") + "<br />Server Return: " +
    HMS((ToSeconds(ServerTime) + 2 * OneWay) % 86400).replace(/h/, ":").
      replace(/m/, ":").replace(/s/, "") + "<br />Local Arrival: " +
    DateFormat(LocalArrive) + "<br />Local Return: " +
    DateFormat(LocalReturn);
  var Speed = Number(document.evaluate(
    "//div[@class='left'][contains(text(), 'Speed:')]/following-sibling::div",
    document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue.innerHTML.split(" ")[0]);
  var Now = new Date();
  NewNode.innerHTML = "<div class='left'>Timing (no coins):</div>" +
    "<div class='right'>" +
    MarchTime(TargetDistance, Speed, null, null, true) + "</div>";
  var Name = GetCastleName([
    null, document.getElementById("att_x").value,
    document.getElementById("att_y").value
  ]);
  NameNode.innerHTML = " " +
    (Name[0].length
...
if (PageMatch && Formation) {
  ...
  GM_setValue(World + "_" + UserName + "Formations", uneval(Formations));
  ...
  var Options = document.getElementById("ppp").options;
  ...
  QuickNode.setAttribute("class", "row");
  QuickNode.innerHTML =
    "<div class='row'><div class='left'>Quick Roads:</div>" +
    "<div class='right'><input id='" +
    "GM_QuickRoads' type='checkbox' /></div></div>"
  InsertionPoint = document.getElementById("z").parentNode.parentNode;
  ...
  var TempoNode = document.getElementById("ppp").parentNode.parentNode;
  var FormationNode =
    SelNodes.snapshotItem(1).parentNode.parentNode.parentNode;
  for (var N = 0; N < Options.length; N++) {
    Options[N].innerHTML = "Speed Cap " + String(N);
  }
  var InsertionPoint = document.getElementById("ppp").
  ...
  if (DefaultSpeed != -1) {
    ...
    if (! Found && window.name) {
      alert("Selected attack formation not found.");
    } else {
      ButtonNode.firstChild.click();
    }
  }
}
...
function CalcCancelTime(Arrives, OneWay, ArrivesNode) {
  ...
  if (CancelTime) {
    Node.innerHTML = "(Cancel: " + HMS(CancelTime) + ")";
  } else {
...
var PageMatch =
  /^http:\/\/game.*\.utopiakingdoms\.com\/info\.php\?what=pohod&posted_id=\d+$/i.
    test(window.location);
if (PageMatch) {
  var PatCoords = /(\d+):(\d+)/i;
  var Nodes = document.evaluate(
  ...
  for (var N = 0; N < Nodes.snapshotLength; N++) {
    ...
    switch (Node.innerHTML) {
      ...
      case ("From:"):
        ...
        var Temp =
          eval(GM_getValue(World + "_MapData_" + From[0] + "_" + From[1], ""));
      ...
      case ("to:"):
        ...
        var Temp =
          eval(GM_getValue(World + "_MapData_" + To[0] + "_" + To[1], ""));
        Temp ? Temp = Temp[4] : Temp = "";
        Node.nextSibling.innerHTML += " " + Temp;
        break;
    }
  ...
  NewNode.innerHTML =
    ...
    StripDecimals(Distance, 2) + " (Effective Speed " +
    StripDecimals(EffSpeed, 2) + ")</div></div>" +
    "<div class='row'><div class='left'>One Way:</div><div class='right'>" +
    OneWay + "</div></div>";
  ...
  CopyButton.type = "button"; CopyButton.value = "Copy";
...
function CopySpyUnits(Event) {
  var NextArmyType;
  Event.target.id == "Units"
    ? NextArmyType = "Supports"
    : NextArmyType = "Camping";
  var PatUnits = new RegExp(
    "<b>" + Event.target.id + ":</b>([\\s\\S]*?)(?:" + NextArmyType + "|$)",
    "i"
  );
  var Report = Event.target.parentNode;
  var Units = BuildingLevel(Report, PatUnits);
...
function SaveMessage(Event) {
  ...
  var SavedMessages =
    eval(GM_getValue(World + "_" + UserName + "Messages", "[]"));
  var Found = false;
  ...
  GM_setValue(World + "_" + UserName + "Messages", uneval(SavedMessages));
}

function DisplayMessages() {
  var Nodes = document.evaluate(
    "//*[starts-with(text(), 'No messages in this category.')]",
    document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  for (var N = 0; N < Nodes.snapshotLength; N++) {
  ...
  var DelNode = document.getElementById("GM_DeletePage");
  ...
  DelNode = document.evaluate(
    "//a[text()='Delete all messages in this category']",
    document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (DelNode) {
    DelNode.id = "GM_Del-2";
  ...
  var InsertionPoint = document.forms.namedItem("delMsgs");
  var SavedMessages =
    eval(GM_getValue(World + "_" + UserName + "Messages", "[]"));
  for (var N = 0; N < SavedMessages.length; N++) {
    var NewNode = document.createElement("div");
    ...
    InsertionPoint.insertBefore(NewNode, InsertionPoint.firstChild);
    FormatSpyReport(NewNode, true);
    DelNode = document.evaluate(
      "descendant::a[@id='delete'][text()='Delete']",
...
function FormatSpyReport(Report, SavedLocal) {
  ...
  var PatName = /<br>\s*Information on (.+?)\s*?(?:&nbsp;)/i;
  var Name = BuildingLevel(Report, PatName);
  var PatCoords = /\[(\d+):(\d+)\]/i;
  var PatCoordsG = /(?:<a[^>]*>)?\[(\d+):(\d+)\](?:<\/a>)?/ig;
  var PatGold = /Gold: (\d+)/i;
  var PatIron = /Iron: (\d+)/i;
  var PatWood = /Wood: (\d+)/i;
  var PatFood = /Food: (\d+)/i;
  var PatGoldProd = /Gold Mine - (\d+) Level/i;
  var PatIronProd = /Iron Mine - (\d+) Level/i;
  var PatWoodProd = /Lumbermill - (\d+) Level/i;
  var PatFoodProd = /Farms - (\d+) Level/i;
  var PatWall = /Wall - (\d+) Level/i;
  var PatHomes = /Homes - (\d+) Level/i;
  var PatOrder = /Order - (\d+) Level/i;
  var PatBarracks = /Barracks - (\d+) Level/i;
  var PatStables = /Stables - (\d+) Level/i;
  var PatGuild = /Guild - (\d+) Level/i;
  var PatGuard = /Guard Station - (\d+) Level/i;
  var PatStore = /Store House - (\d+) Level/i;
  var PatBuildings = /(<b>Buildings:<\/b>[\s\S]*?)<\/p>/i;
  var PatUnits = /<b>Units:<\/b>([\s\S]*?)(?:Supports|<\/p>)/i;
  var PatSupports = /<b>Supports:<\/b>([\s\S]*?)(?:Camping|<\/p>)/i;
  var Now = new Date();
    ...
    function () {
      ...
      if (Names[0].length) {
        ...
        if (! PatName.test(Report.innerHTML))
          Start = Names[1][1];
        Start += " (" + Names[1][0] + ") ";
        if (Names[1][0] == "UK Admin") Abandon = true;
      }
      return Start + arguments[0] + "&nbsp;(<a href='" +
        "http://game" + Server + ".utopiakingdoms.com/pohod.php?attackx=" +
        arguments[1] + "&attacky=" + arguments[2] +"'>march</a>)";
    }
  );
  ...
  //  "Message is " + HMS(Seconds) + " old.</span><br />" + Report.innerHTML;

  if (PatBuildings.test(Report.innerHTML)) {
    var Coords = [
    ...
    var Units = BuildingLevel(Report, PatUnits);
    if (Units.indexOf("None Presented") > -1) Units = "0";
    var Supports = BuildingLevel(Report, PatSupports);
    ...
    Report.innerHTML = Report.innerHTML.replace(
      PatBuildings,
      "<table><tr id='GM_Buildings" + String(N) + "'><td>" + CastleDists[0] +
      "<br /><a href=\"http://game" + Server +
      ".utopiakingdoms.com/pohod.php?attackx=" +
      String(Coords[0]) + "&attacky=" + String(Coords[1]) + "\">march</a> " +
      "<a href=\"http://game" + Server + ".utopiakingdoms.com/map.php?setx=" +
      Coords[0] + "&sety=" + Coords[1] + "\">" + "map</a><br />" +
      "<input type='text' value='" +
      [
        "&apos;" + (Abandon ? "Abandon " : "") + DateStamp,
      ...
      "Total Res: " + String(Total) + "<br />Max " + String(MaxTotal) +
      "<br /><br />" + "$1</td></tr></table>"
    );
    ...
    ["Units", "Supports"].forEach(
      function (Option) {
        if (eval(Option + " != '0'")) {
          var InsertionPoint = document.evaluate(
            "descendant::b[text()='" + Option + ":']",
...
var PageMatch = new RegExp(
  "^http://game.*\.utopiakingdoms\.com/(?:news|writenote)\.php" +
    "(?:.*?(?:type=(\\d+)))?",
  "i"
...
if (PageMatch) {
  ...
  var Node = document.getElementsByClassName("market")[0];
  Node.innerHTML = Node.innerHTML.slice(0, -12) +
    " (<span " +
    "onclick='javascript:window.location=\"news.php?type=3&delall=yes\"'>" +
    "Del</span>)";
  Node = document.getElementsByClassName("marches")[0];
  Node.innerHTML = Node.innerHTML.slice(0, -12) +
    " (<span " +
    "onclick='javascript:window.location=\"news.php?type=4&delall=yes\"'>" +
    "Del</span>)";
  if (Paging) {
  ...
  var Headers = document.evaluate(
    "//h2[contains(text(), 'Utopia Kingdoms')]", document, null,
    XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  for (var N = 0; N < Headers.snapshotLength; N++) {
    Headers.snapshotItem(N).parentNode.removeChild(Headers.snapshotItem(N));
  }
  ...
  NewNode.innerHTML =
    "<a href='javascript:void(0);'>Saved</a>";
  ...
  switch(PageMatch[1]) {
        ...
        if (Match && ! LinkAncestor) {
          Curr.innerHTML = Curr.innerHTML.replace(
            ...
            "$1<span style='background-color: black'>" +
              ...
              "(<a href='pohod.php?attackx=$2&attacky=$3'>march</a>)$4"
          );
        }
      }
      break;
    // battle reports
    case ("2"):
      function CallBR(Node, URL) {
        GM_xmlhttpRequest({
          method: "GET",
          url: URL,
          onload: function (Response) {
            var PatScavenge = new RegExp(
              "[\\s\\S]*?<li>Scavenge:[\\s\\S]*?(\\d+)[\\s\\S]*?(\\d+)" +
                "[\\s\\S]*?(\\d+)[\\s\\S]*?(\\d+)",
              "ig"
            );
            var Match = PatScavenge.exec(Response.responseText);
            var TotalScavenge = ArraySum(Match.slice(1));
            Match = PatScavenge.exec(Response.responseText);
            if (Match) TotalScavenge += ArraySum(Match.slice(1));
            var StolenText = new RegExp("<li>Stolen Resources:", "i").
              test(Response.responseText);
            var TotalStolen = 0;
            if (StolenText)
              TotalStolen = ArraySum(new RegExp(
                "[\\s\\S]*?<li>Stolen Resources:[\\s\\S]*?(\\d+)[\\s\\S]*?" +
                  "(\\d+)[\\s\\S]*?(\\d+)[\\s\\S]*?(\\d+)",
                "i"
              ).exec(Response.responseText).slice(1));
            var TotalCapacity = 0;
            var LastAttack =
              /[\s\S]*<div class="armyOrder attacker">([\s\S]*)<h3>Defender/i.
                exec(Response.responseText)[1];
            var PatUnits = /id="unitPicSmall-(\d+)">(\d+) \//ig;
            do {
              Match = PatUnits.exec(LastAttack);
              if (Match)
                TotalCapacity +=
                  Units["u" + Match[1]]["carry"] * Number(Match[2]);
            } while (Match);
            var AttackingCastle = new RegExp(
              "<div class=\"playerItem\" id=\"AttackerInfo\">[\\s\\S]*?" +
                "\"View Castle\">(\\d+):(\\d+)</a>",
              "i"
            ).exec(Response.responseText);
            var Name = GetCastleName(AttackingCastle);
            var NewNode = document.createElement("span");
            NewNode.innerHTML = "<br />Attacker: " +
              "<span style='background-color: black'><a href=\"http://game" +
              Server + ".utopiakingdoms.com/map.php?setx=" +
              ...
              "<br />Stolen/Capacity: " + String(TotalStolen) + "/" +
      ...
      for (var N = 0; N < BattleReports.snapshotLength; N++) {
        var URL = document.evaluate(
          "descendant::a[text()='Take a look at the report.']",
      ...
      break;
    // get total scavenge source, all pages or particular coordinates
    case ("3"):
      Paging = document.getElementById("GM_Paging");
      var TotalCollected = 0;
      var Button = document.createElement("center");
      Button.innerHTML = "<span style='color:white'>" +
        "<input type='button' value='Count Scavenge, All Messages' " +
        "id='GM_GetMarketSource' /> x:<input type='text' size='3' " +
        "id='GM_X' /> y:<input type='text' size='3' id='GM_Y' /><br />" +
        "<span id='GM_Total'></span></span>";
      document.forms.namedItem("delMsgs").
        insertBefore(Button, document.forms.namedItem("delMsgs").firstChild);
      document.getElementById("GM_GetMarketSource").addEventListener(
        "click",
        function () {
          function GetMarketPage(URL, Index) {
            GM_xmlhttpRequest({
              method: "GET",
              url: URL,
              onload: function (Response) {
                var X = document.getElementById("GM_X").value;
                var Y = document.getElementById("GM_Y").value;
                var Specific = true;
                if (! X || ! Y) {
                  X = "(?:\\d+)"; Y = "(?:\\d+)"; Specific = false;
                }
                var PatRes = new RegExp(
                  "<div class=\"box message\" id=\"msg_item_(\\d+)\">" +
                    "[\\s\\n]*?" +
                    "<p>Our armies from.*?sent to scavenge around.*?" +
                    "<a href=\"map\\.php\\?setx=" + X + "&sety=" + Y + "\">" +
                    "\\[" + X + ":" + Y + "\\]<\/a>.*have reached the town." +
                    "<br\\s*/?><br\\s*/?>They managed to gather:<br\\s*/?>" +
                    "\\s*(\\d+) Gold (\\d+) Iron (\\d+) Wood (\\d+) Food" +
                    ".*?</p>",
                  "ig"
                );
                var Match = PatRes.exec(Response.responseText);
                while (Match) {
                  var ID = Match[1];
                  if (ArrMember(ID, IDs) == -1) {
                    IDs.push(ID);
                    document.getElementById("GM_Messages").value +=
                      Match.slice(2).join("/") + " " + URL + "\n";
                    TotalCollected += ArraySum(Match.slice(2));
                    document.getElementById("GM_Total").innerHTML =
                      "Total" +
                      (Specific ? " from [" + X + ":" + Y + "]" : "") +
                      ": " + String(TotalCollected);
                  }
                  Match = PatRes.exec(Response.responseText);
                }
                var Temp = Links[Index].innerHTML;
                Links[Index].innerHTML = "<s>" + Temp + "</s>";
              }
            });
          }
          TotalCollected = 0;
          ...
          if (Paging) {
            ...
            NewNode.href = "news.php?ot=0&type=3";
            Links = [NewNode];
          }
          var IDs = [];
          NewNode = document.getElementById("GM_Messages");
          if (! NewNode) {
            var NewNode = document.createElement("center");
            NewNode.innerHTML = "<textarea type='textarea' id='GM_Messages'" +
              " rows='10' cols='80'></textarea>";
            document.getElementById("GM_GetMarketSource").parentNode.
              appendChild(NewNode);
          } else {
            NewNode.value = "";
          }
          for (var N = 0; N < Links.length; N++) {
            GetMarketPage(Links[N].href, N);
          }
        },
        true
      );
...
var PageMatch = /^http:\/\/game.*\.utopiakingdoms\.com\/vip_poleta\.php/i.
  test(window.location);
if (PageMatch) {
  var PatCoords = /\[(\d+):(\d+)\]/i;
  var PatResources = new RegExp(
    "<strong>(\\d+)</strong> Gold[\\s\\S]*" +
      "<strong>(\\d+)</strong> Iron[\\s\\S]*" +
      "<strong>(\\d+)</strong> Wood[\\s\\S]*" +
      "<strong>(\\d+)</strong> Food[\\s\\S]*" +
      "<strong>(\\d+)</strong> People[\\s\\S]*",
    "i"
  );
  var SelNodes = document.evaluate(
  ...
  for (var N = 0; N < SelNodes.snapshotLength; N++) {
    ...
    var CarryString = (Colour ? "<font color='" + Colour + "'>" : "") +
      ...
      String(Math.ceil(Resources / Units["u10"]["carry"])) + " LC or " +
      String(Math.ceil(Resources / Units["u19"]["carry"])) + " CA" +
      (Colour ? "</font>" : "");
    var NewNode = document.createElement("td");
...
function CalcTime(Event) {
  ...
  function ManageMerchantTargets(Event) {
    var MerchantTargets =
      eval(GM_getValue(World + "_" + UserName + "MerchantTargets", "[]"));
    if (! Event) {
      ...
      if (Event.target.type == "select-one") {
        ...
        if (! Found && Event.target.value == "Save") {
          MerchantTargets.push(Curr);
          GM_setValue(
            World + "_" + UserName + "MerchantTargets",
            uneval(MerchantTargets)
          );
        }
        if (Found && Event.target.value == "Delete") {
  ...
  var Limit = Number(document.evaluate(
    "//div[@class='left'][contains(text(), 'Transportable Resources:')]/" +
  ...
  var GoldNode, IronNode, WoodNode, FoodNode,
  ...
  if (! TimeNode) {
    ...
    ResList.forEach(
      function (Option) {
        ...
        TopUpNode.innerHTML = " <a href='javascript:void(0)'>Top Up</a>";
    ...
    Node = document.evaluate(
      "//div[contains(text(), 'Predefined Quantities:')]/" +
        "following-sibling::div[position()=1]",
      document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
    ).singleNodeValue;
    NewNode = document.createElement("span");
    NewNode.innerHTML =
      "<br /><a href='javascript:void(0);'>Distribute Evenly</a><br />";
    NewNode.addEventListener("click", DivideEvenly, true);
    Node.appendChild(NewNode);
    NewNode = document.createElement("div");
    NewNode.class = "left";
    NewNode.innerHTML = "<a href='javascript:void(0);'>Reset</a>";
    NewNode.addEventListener(
    ...
    NewNode = document.createElement("div");
    NewNode.class = "left";
    NewNode.innerHTML = "<label>Room Left:</label> <span id='GM_Left'></span>";
  ...
  TimeNode.innerHTML =
    ...
    String(Resources) + "/" + String(Limit) + " (" + String(Extra) + " extra)";
}

var PageMatch = /http:\/\/game.*\.utopiakingdoms\.com\/market\.php\?action=2/i.
  test(window.location);
if (PageMatch) {
  ...
  var QuickNode = document.createElement("div");
  QuickNode.innerHTML = "<div class='left'><label>Quick Roads: </label></div>" +
...
var PageMatch = new RegExp(
  "^http://game.*\\.utopiakingdoms\\.com/market\\.php\\?action=1", "i"
).exec(window.location);
if (PageMatch) {
  var Offers = document.evaluate(
    "//h2[text()='Current Offers']/following-sibling::div[@class='box'][1]" +
      "/descendant::tr",
    document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  for (var N = 0; N < Offers.snapshotLength; N++) {
    if (N == 0) {
      ...
      var NewNode = document.createElement("td");
      NewNode.innerHTML = "Profit Ratio";
      InsertionPoint.parentNode.
...
var PageMatch = new RegExp(
  "^http://game.*\\.utopiakingdoms\\.com/market\\.php\\?action=4", "i"
).exec(window.location);
if (PageMatch) {
  ResList.forEach(
    function (Option) {
...
function CopySkills(Event) {
  GM_setValue(World + "_CopySkills", Event.target.getAttribute("GM_Skills"));
}

// save army from report
function CopyArmy(Event) {
  GM_setValue(World + "_CopyArmy", Event.target.getAttribute("GM_Army"));
}
...
function CopyFormation(Event) {
  ...
  GM_setValue(World + "_CopyFormation", uneval(CopyFormation));
}
...
function ParseSkills(Node) {
  ...
  var Level = /<\/strong>.*?Level\s*(\d+)/i.exec(Node.innerHTML)[1];
  Skills["Level"] = Level;
  Skills2 = [0].concat(Skills2);
  var SaveButton = document.createElement("input");
  SaveButton.type = "button"; SaveButton.value = "Copy Skills";
  ...
  InsertionPoint.parentNode.insertBefore(SaveButton, InsertionPoint);
  return Skills;
}
...
function ParseUnits(Node, Skills, Phase1, End) {
    ...
    if (SpecialNode) {
      ...
      if (SpecialNode) {
        ...
        switch (
          ...
          case ("Warrior")   : SpecialInfantry = SpecialTemp; break;
          case ("Cavalryman"): SpecialCavalry = SpecialTemp; break;
          case ("Defender")  : SpecialDefender = SpecialTemp; break;
        }
      }
    }
  }
  var AttackerWin = /Attacker Wins/i.test(document.evaluate(
    "//div[@class='infosmall']",
    Node, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue.innerHTML);
  var TotalPop = 0;
  ...
  var PatName = /^header=\[\s*(.+?)\s*\]/i;
  var PatFled = /<dt>Fled:<\/dt><dd>(\d+)<\/dd>/i;
  var PatInfantry = /<dt>Attack Infantry:<\/dt><dd>(\d+)<\/dd>/i;
  var PatBuilding = /<dt>Attack Building:<\/dt><dd>(\d+)<\/dd>/i;
  var PatCavalry = /<dt>Attack Cavalry:<\/dt><dd>(\d+)<\/dd>/i;
  var PatArcher = /<dt>Attack Archer:<\/dt><dd>(\d+)<\/dd>/i;
  var PatWizard = /<dt>Attack Wizard:<\/dt><dd>(\d+)<\/dd>/i;
  var PatLife = /<dt>Life:<\/dt><dd>(\d+)<\/dd>/i;
  var PatRange = /<dt>Range:<\/dt><dd>(\d+)<\/dd>/i;
  ...
  for (N = 0; N < Army.length; N++) {
    ...
    var Fled = PatFled.exec(Stats)[1];
    var Infantry = PatInfantry.exec(Stats)[1];
    var Building = PatBuilding.exec(Stats)[1];
    var Cavalry = PatCavalry.exec(Stats)[1];
    var Archer = PatArcher.exec(Stats)[1];
    var Wizard = PatWizard.exec(Stats)[1];
    var Life = PatLife.exec(Stats)[1];
    var Range = PatRange.exec(Stats)[1];
    ...
    LostPop += Units[ID]["pop"] * Number(Match[2]);
    LostPop2 += Units[ID]["pop"] * Number(Match[2]) * IsNotJunk;
    TotalLife += Number(Life);
    ResList.forEach(
      function (Option) {
        var ResName = Option[0];
        Num = Units[ID][ResName.toLowerCase()] * Number(Match[2]) || 0;
        eval("Lost" + ResName + " += Num;");
        eval("Lost" + ResName + "2 += IsNotJunk * Num;");
/*!!!        if (! AttackerWin) {
          Num = Units[ID][ResName.toLowerCase()] * Number(Fled) || 0;
          eval("Lost" + ResName + " += Num");
          eval("Lost" + ResName + "2 += IsNotJunk * Num");
        }*/
      }
    ...
    if (! End) {
      ...
      var OutString = AliveDead + " / " + Fled +
        "<br />I: " + Infantry + "<br />Cavalry: " + Cavalry +
        "<br />A: " + Archer + "<br />M: " + Wizard +
        "<br />L: " + Life + "<br />Atk+: " + String(BonusAttack) +
        "<br />Def+: " + String(BonusDef) +
  ...
  if (! End) {
    ...
    ["Formation", "Army"].forEach(
      function (Option) {
        var CopyButton = document.createElement("input");
        CopyButton.type = "button"; CopyButton.value = "Copy " + Option;
  ...
  if (End) {
    var NewNode = document.createElement("div");
    NewNode.innerHTML = (Attacker ? "Attacker" : "Defender");
    NewNode.innerHTML +=
      "<br />Lost Resources: " + String(LostGold) + "/" +
      String(LostIron) + "/" + String(LostWood) + "/" + String(LostFood) +
      "; Total " + String(LostTotal) +
      "<br />Non-Junk: " + String(LostGold2) + "/" +
      String(LostIron2) + "/" + String(LostWood2) + "/" + String(LostFood2) +
      "; Total " + String(LostTotal2) + "<br />Total Pop: " + TotalPop +
      "<br />Lost Pop: " + String(LostPop) + " (" +
      (TotalPop
    ...
    ScavengeNode.innerHTML =
      "Estimated Scavenge: " + EstScavenge + "<br /><br />";
...
var PageMatch = new RegExp(
  "http://game.*\\.utopiakingdoms\\.com/battle-report\\.php\\?id=.+" +
    "(?:&world_id=.+)?",
  "i"
).test(window.location);
if (PageMatch) {
  ...
  for (var N = 0; N < Coords.snapshotLength; N++) {
    ...
    if (Match) {
      ...
      Name[0].length ? Name = Name[0] : Name = Name[1][1];
      Coords.snapshotItem(N).innerHTML += "--" + Name;
    }
  }
  var AttackerSkills = ParseSkills(document.getElementById("AttackerInfo"));
  document.getElementById("pageBody").
    setAttribute("GM_AttackerLevel", AttackerSkills["Level"]);
  var Multis = document.evaluate(
    "//*[@id='DefenderInfo']", document, null,
    XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  ...
  if (SupporterSkills) {
    var Attacker2 = document.evaluate(
      "//div[@class='infosmall'][1]/" +
        "following-sibling::div/div[@class='armyOrder attacker']",
      document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
    );
    var Supporter = document.evaluate(
      "//div[@class='infosmall'][1]/" +
        "following-sibling::div/div[@class='armyOrder defender']",
      document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
    );
  ...
  NewNode.innerHTML =
    ...
    "Estimated Total Scavenge: " + String(Attacker) + " + " +
    String(Defender) + " = " + String(Scavenge) + " (" +
    String(Math.ceil(Scavenge / Units["u10"]["carry"])) + " LC/" +
    String(Math.ceil(Scavenge / Units["u19"]["carry"])) + " CA" + ")";
  ResultNode.appendChild(NewNode);
  var SurvivorCarry = Number(
    document.getElementById("GM_DataNode").getAttribute("SurvivorCarry")
  );
  var InsertionPoint = document.evaluate(
    "//ul[@class='battleResults']/li[contains(text(), 'Stolen Resources')" +
      " or contains(text(), 'Scavenge')]",
    document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );

  var TotalScavenge = Stolen = 0;
  for (var N = 0; N < InsertionPoint.snapshotLength; N++) {
    var Scavenge = (InsertionPoint.snapshotItem(N).innerHTML.indexOf(
      "Scavenge:"
    ) > -1);
    var Actual =
    ...
    NewNode.innerHTML = "Actual: " + String(Actual) +
      " (" + String(Math.ceil(Actual / Units["u10"]["carry"])) + " LC/" +
      String(Math.ceil(Actual / Units["u19"]["carry"])) + " CA" + ")";
  ...
  if (Stolen) {
    ...
    NewNode.innerHTML = "Survivor Capacity: " + String(Stolen) + "/" +
      String(Capacity) + " (" +
...
/*!!!    case "Paste Clipboard":
  ...
  var ArmyArray = eval(GM_getValue(World + "_CopyArmy", "")) || "";
  var SavedSkills = eval(GM_getValue(World + "_CopySkills", "")) || "";
  var Matches = 0;
  for (var N = 0; N < IDs.snapshotLength; N++) {
    ...
    if (ID) {
      ...
      if (MaxType == "0") {
        ...
        if (Event.target.innerHTML.indexOf("Add") == 0) {
...
var PageMatch =
  /http:\/\/game.*\.utopiakingdoms\.com\/calculator\.php/.test(window.location);
if (PageMatch) {
  var Level = document.evaluate(
    "//div[@id='userBlock']/descendant::span[@class='level']",
    document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (Level) {
    Level = /Level (\d+)/i.exec(Level.innerHTML)[1];
    document.evaluate(
      "//input[@name='alevel']",
      document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
    ).singleNodeValue.value = Level;
  }
  ...
  for (var N = 1; N < Header.length; N++) {
    [
      ["Unit", "0", "Min Unit"], ["Paste", "2", "Paste Unit"],
      ["Add", "2", "Add Unit"],
/*!!!      ["PasteClip", "2", "Paste Clipboard"], ["AddClip", "2", "Add Clipboard"],*/
      ["Upgrade", "1", "Max Upgrade"], ["PasteSkill", "1", "Paste Skill"],
      ["Skill", "1", "Max Skill"]
    ].forEach(
      function (Option) {
        var NewNode = document.createElement("div");
...
function SaveMapData(MapData) {
  ...
  for (var X = 0; X < 8; X++) {
    for (var Y = 0; Y < 8; Y++) {
      window.setTimeout(
        GM_setValue, 0,
        World + "_MapData_" + MapData[X][Y][0] + "_" +
          MapData[X][Y][1],
        uneval(MapData[X][Y])
      );
    }
...
function GetEmptyGridStart(XY) {
  var Other;
  XY == "x" ? Other = "y" : Other = "x";
  document.getElementById(XY + "11").addEventListener(
    "DOMNodeInserted",
    function (Event) {
      eval(
      ...
      if (eval("Global" + Other.toUpperCase() + "1")) {
        ...
        if (! HasNodes1 && ! HasNodes2 && ! HasNodes3) {
          var MapData = [];
          for (var X = 0; X < 8; X++) {
          ...
          document.getElementById("x11").removeEventListener(
            "DOMNodeInserted", GetEmptyGridStart, true
          );
          document.getElementById("y11").removeEventListener(
            "DOMNodeInserted", GetEmptyGridStart, true
          );
        }
      }
...
function TrapMapUpdate(Event) {
  ...
  if (ID == "now_map" || ID == "new_map" || ID == "old_map") {
    ...
    var PatCapital = /img_ut\/revolution\/icon_crown\.gif/i;
    var PatCastleCoords = /> ([^>]+?) \((\d+):(\d+)\)/i;
    var PatPlayer =
      /img_ut\/revolution\/overview\/player\.gif' \/> (.+?)\s*</i;
    var PatLevel = /exp\.gif.+?>\s*\d+\s*\((\d+)\)\s*<br\/>/i;
    var PatClan = /img_ut\/revolution\/overview\/clan\.gif' \/> (.+?)\s*</i;
...
//      GetEmptyGridStart("y");
      ...
      for (var N = 0; N < Nodes.snapshotLength; N++) {
        ...
        TempText = TempText.replace(
          PatFood,
          function () {
            return arguments[0] + "(Total: " + String(Scavenge) + ")";
          }
        );
        var Distance = Dist(Coords);
        TempText = TempText.replace(
          /^header=\[(.+)/i,
          "header=[<span style='font-size: 80%'>Merchant " +
            MarchTime(Distance, MerchantLevels[MerchantLevel], " ", [0]) +
            "Fastest " +
            MarchTime(Distance, Speeds[Speeds.length - 1][0], " ", [0]) +
            "Slowest " + MarchTime(Distance, Speeds[0][0], " ", [0]) +
            "</span>$1"
        );
        Nodes.snapshotItem(N).setAttribute("title", TempText);

        if (Number(Scavenge) > 0) {
            Nodes.snapshotItem(N).innerHTML +=
            "<span style='color: " + Colour + "; font-family: sans-serif;'><b>" +
            String(Math.ceil(Scavenge / Units["u10"]["carry"])) + "LC<br />" +
            String(Math.ceil(Scavenge / Units["u19"]["carry"])) + "CA";
            if (Number(Pop) >= MinHeal || Number(Pop) >= MinScavenge) {
                Nodes.snapshotItem(N).innerHTML +=  "<br />" + Pop;
            }
            Nodes.snapshotItem(N).innerHTML += "</b></font>";
        }
...
function MapZoom() {
  window.location.href = "http://game" + Server +
    ".utopiakingdoms.com/map.php?setx=" +
...
function ReportMapData(Event) {
  ...
  var TempArr = [
    "X", "Y", "Player", "Level", "Castle", "Capital", "Clan",
  ...
  for (var GlobalX1 = 1; GlobalX1 <= MaxX; GlobalX1++) {
    for (var GlobalY1 = 1; GlobalY1 <= MaxY; GlobalY1++) {
      var CurrSquare = GM_getValue(
        World + "_MapData_" + String(GlobalX1) + "_" + String(GlobalY1), ""
      );
      if (CurrSquare != "") {
        ...
        if (CurrSquare[2] != null) {
          ...
          for (var N = 0; N < CurrSquare.length; N++) {
            NewText += "<td>" +
              (N == 7 || N == 8
                ? (N == 7 && CurrSquare[N] == null
                    ? "?"
                    : StripDecimals(
                        Round((Now - CurrSquare[N]) / 86400000, 0.01), 2
                      )
                  ) + "<td>" + String(CurrSquare[N]) + "</td>"
                : CurrSquare[N]
              ) +
              "</td>";
          }
...
function UpdateMapDists(Event) {
  ...
  if (! NewNode) {
    ...
    var Temp = "";
    Temp = " <label>Speed: </label><select id='GM_SelectSpeed'>";
  ...
  if (Target) {
    ...
    NewNode.innerHTML = "<center><table><tr><td>Times to " +
      Target[0] + ":" + Target[1] +
      (Name[0].length
         ? " (" + Name[0] + ")"
         : (Name[1].length ? " (" + Name[1].join(", ") + ")" : "")
...
if (PageMatch) {
  ...
  var InsertionPoint = document.getElementsByClassName("largeMapFrame")[0];
  var Button = document.createElement("center");
  Button.innerHTML = "<input type='button' value='Generate Map Report' />";
...
function UpdateResUse(Event) {
  if (! document.getElementById("GM_TrainRes")) {
    ...
    NewNode.innerHTML = "Total Resources to Train: <span id='GM_TrainRes'>" +
      "</span><br />Total Time to Train: <span id='GM_TrainTime'></span>";
...
var PageMatch =
  /http:\/\/game.*\.utopiakingdoms\.com\/barracks\.php/.test(window.location);
if (PageMatch) {
  var Head = document.getElementsByTagName("head")[0];
  var NewScript = document.createElement("script");
  ...
  var PatID = /<img src=\"img_ut\/units\/thumbs\/(\d+)\.jpg/i;
  ...
  for (var N = 0; N < InProgress.snapshotLength; N++) {
    ...
    var NewNode = document.createElement("div");
    NewNode.innerHTML = "Cancel: " + String(CurrGold) + "/" + String(CurrIron) +
  ...
  var NewNode = document.createElement("div");
  NewNode.style.color = "white";
  NewNode.innerHTML = "Total Cancel: " + String(CancelGold) + "/" +
...
var PageMatch = /http:\/\/game.*\.utopiakingdoms\.com\/overview\.php/i.
  test(window.location);
if (PageMatch) {
  ...
  var Limit = document.evaluate(
    "string(//div[@class='left']" +
      "[contains(text(), 'Trasportable amount') or " +
      "contains(text(), 'Transport Limit')]" +
      "/following-sibling::div/text())", document, null,
  ...
  GM_setValue(World + "_" + UserName + "Merchants", uneval(PlayerMerchants));
  MerchantCapacity();
/*
  // building notes -- removed by maro
  var InsertionPoint = document.getElementById("overview");
  var ULNode = document.createElement("ul");
  ULNode.style.color = "white";
  InsertionPoint.insertBefore(ULNode, InsertionPoint.firstChild);
  var PatLevel = /Level (\d+)\]/i;
  for (var N = 1; N <= BuildingMaxID; N++) {
    ...
    if (Level == Buildings[N].length - 1) {
      LINode.innerHTML += "max level reached";
    } else {
      var ResNeeded = Buildings[N][Level + 1].slice(0, 4);
      LINode.innerHTML += "level " + String(Level + 1) + " uses " +
        String(ArraySum(ResNeeded)) + " resources";
      ResNeeded = [
        ResNeeded[0] - GlobalGold, ResNeeded[1] - GlobalIron,
        ResNeeded[2] - GlobalWood, ResNeeded[3] - GlobalFood
      ];
      var WorldSpeed =
        Number(GM_getValue(World + "_" + UserName + "WorldSpeed", 1));
      ...
      LINode.innerHTML += "; needed: " + ResNeeded +
...
function PasteFormation(Event) {
  var Formation = GM_getValue(World + "_CopyFormation", "");
  if (Formation) {
...
var PageMatch = /^http:\/\/game.*\.utopiakingdoms\.com\/army_order\.php/i.
  test(window.location);
if (PageMatch) {
  ...
  document.getElementById("enemybox").innerHTML += "<br /><br /><center>" +
    "<input type='button' value='Paste Formation' id='GM_PasteFormation' />" +
    "</center>";
