...
window.mostrarMensaje= function(){
        ...
        if(!document.getElementById('ogameextensions')){
            ...
            bigdiv.style.left = "5px";
            bigdiv.style.top = "650px";
...
LinkDiv.id = 'LinkDiv';
LinkDiv.innerHTML += '</script><a class="menubutton " href="" accesskey="" target="_self"> <span class="textlabel">reloj</span></a></li>
}
...
window.autoreload = function(){
   ...
   var tMission
   listaElementos = holder.getElementsByClassName("eventFleet");
   for ( var i = 0 ; i < listaElementos.length ; ++i ) {
		elementoActual = listaElementos[i].getElementsByClassName("hostile");
		//tPlayerType = tEvents[i].getElementsByClassName("friendly");
                if ( elementoActual.length < 1 ) elementoActual = listaElementos[i].getElementsByClassName("hostile");
		if ( elementoActual.length < 1 ) continue;
		tMission = listaElementos[i].getElementsByClassName("missionFleet");
		...
		if ( tMission[0].firstChild.src.indexOf("spionage") != -1);
		{
		   numeroFlotasEnemigas++;
         	   misionFlotasEnemigas.push("Esp");
          	   misionFlotasEnemigasLong.push("Espiar");
		} 
                ...
                if (tMission[0].firstChild.src.indexOf("angriff") != -1);
		{
		   numeroFlotasEnemigas++;
	           misionFlotasEnemigas.push("Atk");
           	   misionFlotasEnemigasLong.push("Atacar");
		}
...
//----------------¿Estamos en preferences?-------------------------

if(document.baseURI.indexOf("preferences") != -1) {
