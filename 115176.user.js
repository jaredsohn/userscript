// ==UserScript==
// @name Youtube Favorites hook up!
// @description Favorites the way they should be!.
...
if (share) {
	...
	fav.innerHTML = '<button style="margin-right:6px;" onclick=";return false;" title="Add to favorites" type="button" class="yt-uix-tooltip-reverse yt-uix-button yt-uix-tooltip" id="downloadbtn" data-button-action="yt.www.watch.add" role="button" aria-pressed="false" data-tooltip="Add to favorites" data-button-listener="29" data-tooltip-title="Add to favorites" data-tooltip-timer="348746">Favorite</span></button>';
...
var SEARCH_BOX 	= "<input id='" + SEARCH_BOX_ID + "' class='search-term' type='text' style='border:1px solid #999999; font-size:100% !important; height:1.38462em; padding:4px 1px 1px; vertical-align:top; width:25em; margin-right: 10px;'/>";
var SEARCH_BUTTON = "<button id='" + SEARCH_BUTTON_ID + "' class='yt-uix-button' style='margin-left: -6px'><span class='yt-uix-button-content'>Search Favourites</span></button>";
var SEARCH_CHECKBOX = "<input id='" + SEARCH_CHECKBOX_ID + "' type='checkbox' style='margin-left: 10px; margin-right: 5px; vertical-align: middle'/><span style='vertical-align: middle'>Match all terms</span>";
...
window.addEventListener('load', function () 
{
	...
	//Inserts the search box and button into the web page
	searchBar.innerHTML = SEARCH_BOX + SEARCH_BUTTON + SEARCH_CHECKBOX;
	...
	//Adds a key listener to the search box and a click listener to the search button
	...
	if (query != EMPTY_VALUE)
	{
		...
		{
			videoDIV = document.getElementById("vm-playlist-video-list-ol");
			
			//Removes the first video in the list due to the fact that the search needs to come up with at least
			//one video before running the script
			videos[0].style.display = "none";
			
			//Run through all the videos in the catalog and check if their title matches any of the search keywords
			for (var i = 0; i < storedVideosArray.length; i++)
			{
				var matchesFound = 0;
				//Extract video title
				var title = storedVideosArray[i].split("title=\"")[1];
				title = title.split("\" onclick")[0];
				...
				for (var j = 0; j < splitQuery.length; j++)
				{
					...
					if (match != -1)
					{
						...
						//If a match is found then insert into the video into the page
						if (matchTerms == EMPTY_VALUE || matchesFound == splitQuery.length)
						{
							videoDIV.innerHTML = videoDIV.innerHTML 
									+ '<li class="" id="vm-video-' + i +'">' 
									+ storedVideosArray[i] 
									+ '</li>';
		...
		else
		{
			...
			for (var i = 0; i < videos.length; i++)
			{
				...
				var title = videos[i].getElementsByClassName("vm-video-title")[0].getElementsByTagName("a")[0].innerHTML;
				...
				//Store video div into array
				storedVideosArray[storedVideosArray.length] = videos[i].innerHTML;
			...
			//After the for loop has completed store the entire array into the database
			GM_setValue("videos_array", storedVideosArray.toSource());
			window.status = "Video catalog updated with a total of " + storedVideosArray.length + " videos";
		}
	}
