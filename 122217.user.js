// ==UserScript==
// @name           Transenauslastung
// @namespace      
// @author         Athene 
// @version        1.2
// @description    Gives hints for recruiting transporters and ground units to achieve transport capacity balancing (supports german language only actually)
...
if (true)
{
	...
	{\
		...
		{\
			...
			$('<a id=\\\'CapacityTool\\\' style=\\\'color:#FC6;padding:2px;text-align:left;\\\' href=\\\' \\\' >Transportrechner</a><br/>').appendTo('#menuCapacity');\
	...
	{\
		var textCapacity = 'verfügbare Transportkapazität: ' + existingCapacity;\
		var textUnits = 'zu transportierende Einheiten:   ' + existingGroundUnits;\
		...
		if(textHintSmall != textHintBig)\
			return '<br>' + textCapacity + '<br>' + textUnits  + '<br><br>' + textHintSmall + '<br><br>oder:<br><br>' + textHintBig +'<br>';\
		else\
	...
	{\
		var buttonClose = '<br><a id=\\\'closeButton\\\' href=\\\'#\\\' class=\\\'button\\\'><span class=\\\'left\\\'><span class=\\\'right\\\'><span class=\\\'middle\\\'>Schließen</span></span></span></span></a>';\
		\
	...
	{\
		...
		\
		var transporterAction = transporterToBuild > 0 ? 'Baue ' : 'Schrotte ';\
		var groundUnitsAction = groundUnitsToBuild > 0 ? 'baue ' : 'schrotte ';\
		\
		var transporterHint = transporterToBuild != 0 ? transporterAction + Math.abs(transporterToBuild) + ' schnelle Transen und' : 'Genügend Transportkapazität vorhanden';\
		var groundUnitsHint = groundUnitsToBuild != 0 ? groundUnitsAction + Math.abs(groundUnitsToBuild) + ' Einheiten!' : 'alle Einheiten können transportiert werden';\
		\
	...
	{\
		...
		var groundUnitsAction = groundUnitsToBuild > 0 ? 'baue ' : 'schrotte ';\
		\
		var transporterHint = transporterToBuild != 0 ? transporterAction + Math.abs(transporterToBuild) + ' langsame Transen und' : 'Genügend Transportkapazität vorhanden';\
		var groundUnitsHint = groundUnitsToBuild != 0 ? groundUnitsAction + Math.abs(groundUnitsToBuild) + ' Einheiten!' : 'alle Einheiten können transportiert werden';\
		\
		return  transporterHint + '<br>' + groundUnitsHint;\
	}\
