...
// @description		XiTools II to improve the user experience of OGame 5.x.x
// @version			1.9.3
// @author			Minguez
// @date			31/10/2012
// @match			http://*.ogame.com.es/game/index.php?page=*
// @match			http://*.ogame.com.ar/game/index.php?page=*
// @match			http://*.ogame.com.br/game/index.php?page=*
// @match			http://*.ogame.de/game/index.php?page=*
// @match			http://*.ogame.fr/game/index.php?page=*
// @match			http://*.ogame.it/game/index.php?page=*
// @match			http://*.ogame.org/game/index.php?page=*
// @match			http://*.ogame.ru/game/index.php?page=*
// @match			http://*.ogame.pl/game/index.php?page=*
// @include			http://*.ogame.*/game/index.php?page=*
// ==/UserScript==
function gU(){XiTClass.prototype.kX=false;XiTClass.prototype.jM=0;XiTClass.prototype.kM=0;XiTClass.prototype.Message=(function(kY){if(!this.V.eblspioplunder.aT()&& !this.V.eblspiodf.aT()&& !
this.V.eblspiosim.aT())return false;function getData(kL){var i;var l;var h;var gs;var kx;var time;var e=kL.find(".fragment.spy2 td");if(!e){return null;}var gv={fE:kL.find('table:last'),data:{},bJ:
e.eq(1).text().replace(/\D/g,'').bn(),kr:e.eq(3).text().replace(/\D/g,'').bn(),ca:e.eq(5).text().replace(/\D/g,'').bn(),bK:e.eq(7).text().replace(/\D/g,'').bn(),position:[]};var key=kL.find(
'table .key');var val=kL.find('table .value');l=key&&val&&key.length===val.length?key.length:0;for(i=0;i<l;i++){gv.data[XiT.jW($.trim(key.eq(i).text()))]=val.eq(i).text().replace(/\D/g,'');}var pos=
kL.find('a.btn_blue').attr('href').match(lu);if(pos&&pos.length===5){for(i=1;i<5;i++){gv.position.push(pos[i].bn());}}else{return null;}h=kL.find('.area');if(!h|| !h.length){return null;}gs=h.eq(0)
.text().match(/([^\s]*) ([^\s]*) ([^\[]*) \[/);if(gs&&gs.length){gv.lk=gs[3];}else{return null;}gv.kx=h.eq(0).find("span[class*='status_abbr']").text();if(h.eq(0).find("span[class*='inactive']")[0]){
gv.la=[0.5];}else{gv.la=(h.eq(0).find("span[class*='rank_bandit']")[0])?[1]:(h.eq(0).find("span[class*='status_abbr_honorableTarget']")[0])?[0.75]:[0.5];}gv.lp=h.length-1;return gv;};var jI=(kY)?$(
".showmessage"):$("#mailz");var t=jI.find(".material.spy");var kZ=[];var i;var l=t?t.length:0;var gv;var lu=/&galaxy=(\d+)&system=(\d+)&position=(\d+)&type=(\d+)&/;var lA=new RegExp(
"\\(.+\\s\'(.+)\\'\\)");var kA=new RegExp("(\\d{2}).(\\d{2}).(\\d{4})\\s(.+)");for(i=0;i<l;i++){gv=getData(t.eq(i).parent());if(gv){kZ.push(gv);}}var lw=this.V.eblspioplunder.aT();var ly=
this.V.eblspiodf.aT();var jV=this.V.eblspiosim.aT();var kw=(this.V.deftodebris=="1")?(1-this.V.defrepfactor):0;var kI=this.V.debrisfactor;var mq=document.getElementsByName('ogame-language')
[0].content;var mD=this.V.eblosim.aT();var ma=this.V.eblwebsim.aT();var mP=this.V.ebldragosim.aT();var p=this.cj.split(":");for(var key in kZ){var gv=kZ[key];var fQ="";var percent=0;var m=0;var c=0;
var jH=0;var kG=[];var kR=[];if(lw){for(i=0;i<gv.la.length;i++){kG[0]=Math.ceil(gv.bJ*gv.la[i]);kG[1]=Math.ceil(gv.kr*gv.la[i]);kG[2]=Math.ceil(gv.ca*gv.la[i]);jH=kG[0]+kG[1]+kG[2];kR=[];kR.push(
XiT.R(XiT.ah(kG[0]),null,'XiCMet'));kR.push(XiT.R(XiT.ah(kG[1]),null,'XiCCry'));kR.push(XiT.R(XiT.ah(kG[2]),null,'XiCDeu'));kR.push(XiT.R(XiT.ah(jH),'lime'));var h=Math.max(gv.bJ+gv.kr+gv.ca,Math.min(
(2*gv.bJ+gv.kr+gv.ca)*3/4,(2*gv.bJ+gv.ca)))*gv.la[i];kR.push(XiT.R(XiT.ah(Math.ceil(h/J.data[J.dJ.gS].bU)),'#fff'));kR.push(XiT.R(XiT.ah(Math.ceil(h/J.data[J.dJ.he].bU)),'#fff'));kR.push(gv.la[i]*100)
;if(fQ!="")fQ+="<br>";fQ+=this.kc(fp.kQ,kR);}}if(ly){percent=kw;if(percent>0){var list=J.list.hk;for(var eB=0,jY=list.length;eB<jY;eB++){if(gv.data[list[eB]]){m+=gv.data[list[eB]]*J.data[list[eB]].m*
percent;c+=gv.data[list[eB]]*J.data[list[eB]].c*percent;}}}percent=kI;if(percent>0){var list=J.list.dh;for(var eB=0,jY=list.length;eB<jY;eB++){if(gv.data[list[eB]]){m+=gv.data[list[eB]]*
J.data[list[eB]].m*percent;c+=gv.data[list[eB]]*J.data[list[eB]].c*percent;}}}if((m+c)>0){var lf=[XiT.R(XiT.ah(m),null,'XiCMet'),XiT.R(XiT.ah(c),null,'XiCCry'),XiT.R(XiT.ah(Math.ceil((m+c)/
J.data[J.dJ.gB].bU)),'#fff')];if(fQ!="")fQ+="<br>";fQ+=this.kc(fp.lv,lf);}}if(jV){var lq="http://www.osimulate.com/?lang="+mq;var ky="http://websim.speedsim.net/index.php?lang="+fp.kW;var lm=
"http://drago-sim.com/index.php?lang="+fp.jP;var le=[gv.kx];lq+='&tech_a0_0='+this.data.k.tech[J.bj.gd]+'&tech_a0_1='+this.data.k.tech[J.bj.ew]+'&tech_a0_2='+this.data.k.tech[J.bj.K];ky+=
'&tech_a0_0='+this.data.k.tech[J.bj.gd]+'&tech_a0_1='+this.data.k.tech[J.bj.ew]+'&tech_a0_2='+this.data.k.tech[J.bj.K];lm+='&techs[0][0][w_t]='+this.data.k.tech[J.bj.gd]+'&techs[0][0][s_t]='+
this.data.k.tech[J.bj.ew]+'&techs[0][0][r_p]='+this.data.k.tech[J.bj.K];lq+='&engine0_0='+this.data.k.tech[J.bj.fO]+'&engine0_1='+this.data.k.tech[J.bj.gq]+'&engine0_2='+this.data.k.tech[J.bj.gh];lq+=
'&start_pos='+p[0]+':'+p[1]+':'+p[2];ky+='&engine0_0='+this.data.k.tech[J.bj.fO]+'&engine0_1='+this.data.k.tech[J.bj.gq]+'&engine0_2='+this.data.k.tech[J.bj.gh];ky+='&start_pos='+p[0]+':'+p[1]+':'+
p[2];lq+='&fleet_debris='+kI*100+'&defense_debris='+kw*100+'&defense_repair='+this.V.defrepfactor*100+'&uni_speed='+this.bA+'&rapidfire='+this.V.rapidfire+'&plunder_perc='+gv.la[0]*100;ky+=
'&perc-df='+kI*100;lm+='&debris_ratio='+kI;lq+='&enemy_name='+gv.lk+'&enemy_pos='+gv.position[0]+':'+gv.position[1]+':'+gv.position[2];ky+='&enemy_name='+gv.lk+'&enemy_pos='+gv.position[0]+':'+
gv.position[1]+':'+gv.position[2];lm+='&v_planet='+gv.lk+'&v_coords='+gv.position[0]+':'+gv.position[1]+':'+gv.position[2];lq+='&enemy_metal='+gv.bJ+'&enemy_crystal='+gv.kr+'&enemy_deut='+gv.ca;ky+=
'&enemy_metal='+gv.bJ+'&enemy_crystal='+gv.kr+'&enemy_deut='+gv.ca;lm+='&v_met='+gv.bJ+'&v_kris='+gv.kr+'&v_deut='+gv.ca;var kV=[J.bj.gd,J.bj.ew,J.bj.K];var kl=['w_t','s_t','r_p'];for(var eB=0,jY=
kV.length;eB<jY;eB++){if(gv.data[kV[eB]]){lq+='&tech_d0_'+eB+'='+gv.data[kV[eB]];ky+='&tech_d0_'+eB+'='+gv.data[kV[eB]];lm+='&techs[1][0]['+kl[eB]+']='+gv.data[kV[eB]];}}var lb=J.list.dh.concat(
J.list.hk);var lg=['k_t','g_t','l_j','s_j','kr','sc','ko','re','sp','bo','so','z','t','sk','ra','l_l','s_l','g','i','p','k_s','g_s'];for(var eB=0,jY=lb.length;eB<jY;eB++){if(gv.data[lb[eB]]){lq+=
'&ship_d0_'+eB+'_b='+gv.data[lb[eB]];ky+='&ship_d0_'+eB+'_b='+gv.data[lb[eB]];lm+='&numunits[1][0]['+lg[eB]+']='+gv.data[lb[eB]];le.push(lb[eB]);le.push(gv.data[lb[eB]]);}}if(gv.data[J.ew.hH]){lq+=
'&abm_b='+gv.data[J.ew.hH];ky+='&abm_b='+gv.data[J.ew.hH];lm+='&missiles_available_v='+gv.data[J.ew.hH];}var ld=[];if(mD){ld.push('<a target="_blank" href="'+lq+'" class="btn_blue">OSimulate</a>');}
if(ma){ld.push('<a target="_blank" href="'+ky+'" class="btn_blue">SpeedSim</a>');}if(mP){ld.push('<a target="_blank" href="'+lm+'" class="btn_blue">DragoSim</a>');}ld.push(
'<a href="javascript:void(0);" onclick="XiT.kK(\''+le.join(',')+'\');" class="btn_blue">'+fp.kU+'</a>');if(fQ!="")fQ+="<br>";fQ+='</td></tr><tr><td class="attack" align="center" colspan="2">'+ld.join(
' &nbsp; ');}if(fQ!="")gv.fE.find("tr:first").after('<tr><td class="textCenter textBeefy" colspan="2">'+fQ+'</td></tr>');}});XiTClass.prototype.lj=(function(){var lb=J.list.dh;var kC=0;var ka="lime";
this.kM=0;for(var key in lb){var ge=this.kX.find("#XiTtr1_"+lb[key]).val();this.kM+=(J.data[lb[key]].m+J.data[lb[key]].c+J.data[lb[key]].d)*J.data[lb[key]].tr*ge;}kC=(this.jM>0)?this.kM/this.jM:0;ka=(
kC>=5)?"red":(kC>=3)?"yellow":ka;this.kX.find("div").eq(0).html(this.R(number_format(kC,2),ka));});XiTClass.prototype.kK=(function(kn){if(!this.kX){var kD=$(
'<div id="XiTtactical" style="display:none"><div></div><table class="list" style="width:385px;"><tr><th style="width:180px;">'+fp.ke+'</th><th>'+this.fG+'</th><th>Defender</th></tr></table></div>');
var table="";var aL=(typeof(this.data.aL)=="object")?this.data.aL:$.parseJSON(this.aS('strings'));var lb=J.list.dh;for(var key in lb){table+='<tr>';table+='<td>'+aL[lb[key]]+'</td>';table+=
'<td><input id="XiTtr1_'+lb[key]+'" type="text" maxlength="7"/></td>';table+='<td><span id="XiTtr0_'+lb[key]+'"></span></td>';table+='</tr>';}kD.find("table").append(table);kD.find("table tr:odd")
.addClass("alt");kD.find("table tr").each(function(){$(this).find("td:first").css("text-align","left")});kD.find("input").bind('keyup change',function(){var value=$(this).val();if(value=="")
return false;value=value.replace(/\D/g,'');$(this).val(value);XiT.lj();});this.kX=kD;}this.jM=0;var ki=kn.split(',');this.kX.find("div").eq(0).html(this.R("0,00","lime"));this.kX.find("input").val("")
;this.kX.find("span[id*='XiTtr0_']").html("-");this.kX.find("table th").eq(2).text(ki[0]);for(var i=1,l=ki.length-1;i<l;i+=2){if(typeof(J.data[ki[i]].tr)!='undefined'){this.kX.find("#XiTtr0_"+ki[i])
.text(this.format(ki[i+1]));this.jM+=(J.data[ki[i]].m+J.data[ki[i]].c+J.data[ki[i]].d)*J.data[ki[i]].tr*ki[i+1].bn();}}this.kX.dialog({title:fp.jG,height:"auto",width:"400px",position:"center",
resizable:false});this.kX.dialog("moveToTop");});$(".mailWrapper").ajaxSuccess(function(e,xhr,settings){if(settings.url.indexOf('?page=showmessage')>0){XiT.Message(true);}if(settings.url.indexOf(
'?page=messages')<0){return false;}XiT.Message(false);$("select").unbind('change');$("select.choose").change(function(){$(".buttonOK").show();mod=$("select.choose option:selected").attr("id");});});};
function aK(lang){var aL={'es':{save:"Guardar",cancel:"Cancelar",ai:"Espionaje",dR:"Reducir",gr:"No hay recicladores disponibles",cD:"Mensajes Circular Rapido",hl:
"El mensaje circular ha sido enviado",send:"Enviar",dM:"Galaxia",cW:"Flota",cA:"General",dO:"Edificios",kP:"Mensajes",fm:"Información",cZ:"Buscar actualizacion cada",cr:"Nunca",aF:"días",cn:
"Ampliar detalles en construcciones",cJ:"Abreviar valores",du:"Resaltar escombros",cQ:"Resaltar escombros (medio)",dz:"Resaltar escombros (alto)",aU:"Mostrar rank al lado del nombre",cs:
"Resaltar Amigos y aliados",db:"Resaltar coordenadas de link",cC:"Espiar lunas con un click",cY:"Recolectar con un click",cP:"Activar recursos en transito",ht:"Naves en FleetII/FleetIII",hG:
"Accesos directos en FleetII",hn:"Acceso directo",cb:"Consumo de deuterio",au:"Desconocido",cV:"Disponible en",dW:"Producir n&uacute;mero m&aacute;ximo",dA:
"Existe una nueva version del plugin XiTools.",da:"Actualizar",cg:"versión",cG:"Error, Escombros medios:\nRequerido.\nNúmerico.",bx:
"Error, Escombros altos:\nRequerido.\nNúmerico.\nMayor que escombros medios.",cF:"Planetas",dr:"L",cB:"PP",cE:"Mostrar información detallada",ce:"Estadísticas",bG:{0:"General: %t (%p pts.)",1:
"Economía: %t (%p pts.)",2:"Investigacíon: %t (%p pts.)",3:"Militar: %t (%p pts.)",4:"Militar Perdidos: %t (%p pts.)",5:"Militar Construidos: %t (%p pts.)",6:"Militar Destruidos: %t (%p pts.)",7:
"Honor: %t (%p pts.)",8:"unidades"},eD:"Planetas Marcados",cU:"No se encontraron planetas marcados.",cq:{0:"Metal",1:"Cristal",2:"Deuterio"},bZ:"Total",fk:"Recursos esperados para este planeta",jd:
"Ayuda para transporte",ik:"Transporte",jo:"Todo",ja:"Faltante",iq:"Restablecer opciones",ap:"Información de producción",ev:"Requiere <b>'Ampliar detalles en construcciones'</b> activado.",fF:
"última actualización",ab:"necesita actualizar",dI:"Este planeta",cS:"Todos los planetas",cI:"por día",dg:"por semana",fN:"detalle",mo:"Producción de naves",lQ:"Producción de defensas",bD:
"Compactador de batallas",aq:"Repartidor de escombros",jX:"Velocidad del universo",kk:"Escombros generados por flota",kN:"Escombros generados por defensa",jQ:"Mostrar saqueo posible",jK:
"Mostrar campo de escombros posible",jZ:"Mostrar botones a Simuladores",kQ:
...
"Military destroyed: %t (%p pts.)",7:"Honour: %t (%p pts.)",8:"units"},eD:"Highlights Planets",cU:"There were no highlights planets.",cq:{0:"Metal",1:"Crystal",2:"Deuterium"},bZ:"Total",fk:
"Resources expected at this planet",jd:"Easy transport",ik:"Transport",jo:"All",ja:"Missing",iq:"Reset all data",ap:"Info Production",ev:"<b>'Extended details on construction'</b> required",fF:
"last update",ab:"need update",dI:"This planet",cS:"All planets",cI:"per day",dg:"per week",fN:"detail",mo:"Fleet production",lQ:"Defense production",bD:"Combat Report Converter",aq:
"ACS Debris Distribute",jX:"Universe speed",kk:"Fleet to debris",kN:"Defense to debris",jQ:"Show possible plunder",jK:"Show possible derbis field",jZ:"Show battle sim links",kQ:
...
"Боевых очков уничтожено: %t (%p очков)",7:"Очки чести: %t (%p очков)",8:"единиц"},eD:"Подсветка планет",cU:"Нет подсвечиваемых планет.",cq:{0:"Металл",1:"Кристалл",2:"Дейтерий"},bZ:"Всего",fk:
"Ожидается ресурсов на планете",jd:"Простая транспортировка",ik:"Транспорт",jo:"все",ja:"потерявшийся",iq:"Reset all data",ap:"Info Production",ev:"<b>'Extended details on construction'</b> required",
fF:"last update",ab:"need update",dI:"This planet",cS:"All planets",cI:"per day",dg:"per week",fN:"detail",mo:"Fleet production",lQ:"Defense production",bD:"Конвертер боевых докладов",aq:
"ACS Debris Distribute",jX:"Universe speed",kk:"Fleet to debris",kN:"Defense to debris",jQ:"Show possible plunder",jK:"Show possible derbis field",jZ:"Show battle sim links",kQ:
"Catchable resources ($7%): $1 Metal, $2 Crystal, $3 Deuterium <br> Needed cargo space: $4 units - Needed cargo ships on attack: $5 SC or $6 LC",lv:
...
"Punti pianeti",cU:"Non ci sono punteggi per i pianeti.",cq:{0:"Metallo",1:"Cristallo",2:"Deuterio"},bZ:"Totale",fk:"Risorse attese su questo pianeta",jd:"Facilità di trasporto",ik:"Trasporto",jo:
"Tutti",ja:"Mancante",iq:"Reset all data",ap:"Info Production",ev:"<b>'Extended details on construction'</b> required",fF:"last update",ab:"need update",dI:"This planet",cS:"All planets",cI:"per day",
dg:"per week",fN:"detail",mo:"Fleet production",lQ:"Defense production",bD:"Convertitore di CR",aq:"Distribuzione detriti per ACS",jX:"Universe speed",kk:"Fleet to debris",kN:"Defense to debris",jQ:
...
cq:{0:"Metal",1:"Kryształ",2:"Deuter"},bZ:"Razem",fk:"Zasoby przewidywane na tej planecie",jd:"łatwy transport",ik:"Transportu",jo:"wszystko",ja:"brakujący",iq:"Reset all data",ap:"Info Production",
ev:"<b>'Extended details on construction'</b> required",fF:"last update",ab:"need update",dI:"This planet",cS:"All planets",cI:"per day",dg:"per week",fN:"detail",mo:"Fleet production",lQ:
"Defense production",bD:"Konwerter raportów wojennych",aq:"Podział złomu po ACSie",jX:"Universe speed",kk:"Fleet to debris",kN:"Defense to debris",jQ:"Show possible plunder",jK:
"Show possible derbis field",jZ:"Show battle sim links",kQ:
"Catchable resources ($7%): $1 Metal, $2 Crystal, $3 Deuterium <br> Needed cargo space: $4 units - Needed cargo ships on attack: $5 SC or $6 LC",lv:
...
"Honneur: %t (%p pts.)",8:"Unités"},eD:"Planètes en surbrillance",cU:"Il n'y a pas de Planètes en surbrillance.",cq:{0:"Mètal",1:"Cristal",2:"Deutérium"},bZ:"Total",fk:
"Ressources attendues sur cette planète",jd:"Transport Facile",ik:"Transport",jo:"Toutes",ja:"Manquantes",iq:"Remise à zéro des données",ap:"Info Production",ev:
"'Détails étendus de construction' requis",fF:"Dernière mise à jour",ab:"Mise à jour requise",dI:"Cette planète",cS:"Toutes les planètes",cI:"Par jour",dg:"Par semaine",fN:"détail",mo:
"Production de Flotte",lQ:"Production de Défense",bD:"Combat Report Converter",aq:"ACS Debris Distribute",jX:"Vitesse univers",kk:"Flotte dans le CDR",kN:"Défense dans le CDR",jQ:
"Montrer pillage possible",jK:"Montrer champs de ruines possible",jZ:"Montrer liens de simulations combats",kQ:
...
"champs de ruines possible : $1 Metal, $2 Crystal, $3 Recycler",kU:"Retrait tactique",jG:"Retraite tactique",ke:"Vaisseaux",kW:"fr",jP:"french"}};return aL[lang];};function gE(){var ct=
window.location+"";var arguments=ct.substring(ct.indexOf('?')+1).split("&");try{return arguments[0].substring(arguments[0].indexOf('=')+1);}catch(e){return "unknown";}};function XiTClass(call){
this.version="1.9.3";this.fz="http://userscripts.org/scripts/source/137752.user.js";this.eH="http://userscripts.org/scripts/show/137752";this.dc=new Array();this.bw=(function(selector,update){if(!
XiT.dc[selector]||update){XiT.dc[selector]=$(selector);}return XiT.dc[selector];});this.ix=(function(selector,update){if(!this.dc[selector]||update){this.dc[selector]=$(selector);}
return this.dc[selector];});String.prototype.bn=(function(cL){var value=this;if(value!=""){bS=parseInt(value);bS=(isNaN(bS))?0:bS;bS=(cL)?bS:Math.abs(bS);return bS;}else{return 0;}});
String.prototype.aT=(function(){var bu=this;return(bu=="0")?false:true;});this.fg=(function(data){var ip=data.version.split(".");var iI=XiT.version.split(".");var nh=(ip[3])?true:false;var mv=(iI[3])?
true:false;if(ip[0]=="0")return false;if((ip[0]>iI[0]||ip[1]>iI[1]||ip[2]>iI[2])||(nh==true&&mv==false)){var gg=
...
this.gp=(function(){$("body").ajaxSuccess(function(e,xhr,settings){if(settings.url.indexOf('?page=planetGiveup')<0){return false;}var data=$.parseJSON(xhr.responseText);if(data["status"]){
...
!='undefined'&&O!=null){$.extend(this.V,$.parseJSON(this.aS('op','{'+this.bH(this.V)+'}')));this.data.aG=O;}else{var now=new Date().getTime().toString();this.data.aG=now;this.aY("op",'{'+this.bH(
this.V)+'}');this.aY("gmark",'{}');this.aY("player",this.data.k);this.aY("planets",this.data.eS);this.aY("strings",this.data.aL);this.aY("transport",this.data.eE);this.aY("pluginupdate",now);}if(
this.V.rapidfire=="null")this.jJ('serverData');if(this.V.strloaded=="0")this.jJ('techStrings');}catch(e){console.log("Unknown error "+e+".");}});this.ig=(function(){this.bk("pluginupdate");
location.href="/game/index.php?page=overview";});this.az=(function(){var cd={};var gz=window.location.href.replace(/[?&]+([^=&]+)=([^&]*)/gi,function(m,key,value){cd[key]=value;});return cd;});
this.hI=(function(){var text=$("#XiTMailText").val();$('#XiTMailtb').dialog('close');$.post('http://'+XiT.bL+'/game/index.php?page=allianceBroadcast',{empfaenger:200,text:text},function(data){$(
"#XiTMailText").val("");this.be="";fadeBox(fp.hl,false);})});this.hb=(function(aX){this.be=$("#XiTMailText").val();if(this.be.length>aX){$("#XiTMailText").val(this.be);}else{this.be=$("#XiTMailText")
.val();}});this.eo=(function(value,cL){if(value!=""){bS=parseInt(value);bS=(isNaN(bS))?0:bS;bS=(cL)?bS:Math.abs(bS);return bS;}else{return 0;}});this.aD=(function(name,value,fK){this.aY(name,value);
if(fK===true){$('#XiTOptionstb').dialog('close');}});this.fh=(function(){var aj='<span class="menu_icon"></span>';if(this.cT!="0"){aj='<span class="menu_icon">';aj+=
'<a href="javascript:;" class="overlay" data-overlay-title="'+fp.cD+'" data-overlay-inline="#XiTMailtb">';aj+='<img class="mouseSwitch" src="'+this.eu+'" height="29" width="38" rel="'+this.dZ+'">';
aj+='</a></span>';}$('#menuTable').append('<li>'+aj+
...
;});this.ll=(function(){if(this.lz)return false;this.lz=true;for(var as in this.af){if($("#XiT"+as).attr("type")=="checkbox"){if(this.V[as]=="1"){$("#XiT"+as).attr('checked',true);}}else{$("#XiT"+as)
.val(this.V[as]);}}var kw=(this.V.deftodebris=="1")?(1-this.V.defrepfactor):0;$("#XiTunispeed").html("x"+this.bA);$("#XiTdebrisf").html((this.V.debrisfactor*100)+"%");$("#XiTdebrisd").html((kw*100)+
"%");$("#XiTversion").html("v"+this.version);});this.call=(function(){if(this.ac!=""){eval("this."+this.ac+"();");}});this.gu=(function(){for(var as in this.af){var dY=this.af[as].check;if(dY===null)
return false;var bu=dY;var pattern=/\[|,|\]/;var result=bu.split(pattern);if(!this.eC("#XiT"+as,result)){alert(this.af[as].text);return false;}if($("#XiT"+as).attr("type")=="checkbox"){if($("#XiT"+as)
.is(":checked")){this.V[as]="1";}else{this.V[as]="0";}}else{this.V[as]=$("#XiT"+as).val();}};this.aY("op",'{'+this.bH(this.V)+'}');$('#XiTOptionstb').dialog('close');});this.V={updatetime:"1",
eblbuildingextend:"1",eblshortenvalue:"0",ebleasytransp:"1",eblbuildingqueue:"1",eblhld:"1",debrislow:100000,debrishigh:1000000,eblrank:"1",eblhlba:"1",eblcoordlink:"1",eblspy:"0",eblrecy:"0",
eblresontransit:"1",eblfleetlist:"1",eblshorcut:"1",strloaded:"0",eblspioplunder:"1",eblspiodf:"1",eblspiosim:"1",eblosim:"1",eblwebsim:"1",ebldragosim:"0",debrisfactor:"0",deftodebris:"0",
defrepfactor:"0",rapidfire:"null"};this.data={bE:"{}",k:'{"tech":{"queue":{"id":"0","time":""}},"commander":"0","admiral":"0","engineer":"0","geologist":"0","technocrat":"0"}',eS:'{}',aL:'{}',eE:'{}',
aG:""};this.af={updatetime:{check:"notrequired"},eblbuildingextend:{check:"notrequired"},eblshortenvalue:{check:"notrequired"},ebleasytransp:{check:"notrequired"},eblbuildingqueue:{check:
"notrequired"},eblhld:{check:"notrequired"},debrislow:{check:"required,onlyNumber",text:fp.cG,type:"numeric"},debrishigh:{check:"required,onlyNumber[notZero],condition[>,debrislow]",text:fp.bx,type:
"numeric"},eblrank:{check:"notrequired"},eblhlba:{check:"notrequired"},eblcoordlink:{check:"notrequired"},eblspy:{check:"notrequired"},eblrecy:{check:"notrequired"},eblresontransit:{check:
"notrequired"},eblfleetlist:{check:"notrequired"},eblshorcut:{check:"notrequired"},eblspioplunder:{check:"notrequired"},eblspiodf:{check:"notrequired"},eblspiosim:{check:"notrequired"},eblosim:{check:
"notrequired"},eblwebsim:{check:"notrequired"},ebldragosim:{check:"notrequired"},};this.eC=(function(caller,rules){var fX=false;if(rules[0]=="notrequired"&& !$(caller).val())return true;for(var i=0;i<
rules.length;i++){switch(rules[i]){case "required":if(this.fw(caller))return false;break;case "onlyNumber":if(this.fd(caller,rules,i))return false;break;case "length":if(this.ff(caller,rules,i))
return false;break;case "regex":if(this.eJ(caller))return false;break;case "condition":if(this.fA(caller,rules,i))return false;break;default:}}return true;});this.fw=(function(bg){var bi=$(bg).attr(
"type");if(bi=="text"||bi=="password"||bi=="textarea"){if(!$(bg).val()){return true;}}if(bi=="radio"||bi=="checkbox"){et=$(bg).attr("name");if($("input[name='"+et+"']:checked").size()===0){
return true;}}if(bi=="select-one"){if(($(bg).val()=="")||($(bg).val()=="0")){return true;}}return false;});this.fd=(function(bg,rules,position){var value=$(bg).attr('value');if(!value.match(
...
!value.match(ey))return true;return false;});this.fA=(function(bg,rules,position){var value=this.eo($(bg).attr('value'));var ez=rules[position+1];var eG=$("#XiT"+rules[position+2]).attr('value');
var result=(eval(value+ez+eG))?false:true;return result;});this.fa=(function(tab){var left=0-(688*tab);$("#XiTsilidertabs span").removeClass("on");$("#XiTsilidertabs span:nth-child("+(tab+1)+")")
.addClass("on");$("#XiTslidersContent").animate({marginLeft:left+"px"},500);});this.format=(function(number){number=this.eo(number,true);var cO=(number<0)?"-":"";number=Math.abs(number);if(
...
);this.dX=(function(){try{if(!this.data.eS[this.T]){this.data.eS[this.T]={};this.data.eS[this.T].queue={id:"0",time:""};};O=jQuery(this.am("#metal_box"));this.data.eS[this.T].m=O.find("td").eq(0)
.text().replace(/\D/g,'');this.data.eS[this.T].mp=O.find("td").eq(2).text().replace(/\D/g,'');O=jQuery(this.am("#crystal_box"));this.data.eS[this.T].c=O.find("td").eq(0).text().replace(/\D/g,'');
this.data.eS[this.T].cp=O.find("td").eq(2).text().replace(/\D/g,'');O=jQuery(this.am("#deuterium_box"));this.data.eS[this.T].d=O.find("td").eq(0).text().replace(/\D/g,'');this.data.eS[this.T].dp=
O.find("td").eq(2).text().replace(/\D/g,'');O=jQuery(this.am("#energy_box"));this.data.eS[this.T].ene=O.find("td").eq(0).text().replace(/[^0-9\-]/g,'');this.data.eS[this.T]['coords']=this.cj;
this.data.eS[this.T]['type']=this.jE;this.data.eS[this.T]['upd']=serverTime.getTime().toString();if(this.jE=="moon"){this.data.eS[this.T].temp=["0","0"];}else{if(!this.data.eS[this.T].temp){if(O=
this.am('#rechts a[href*="&cp='+this.T+'"]').split(/<br>|<br\/>/i)[2]){this.data.eS[this.T].temp=O.match(/(\-?[0-9]{1,3})/gi)}}}var selector="";switch(currentPage){case "resources":selector=
'li div[class*="supply"]';break;case "station":selector='li div[class*="station"]';break;case "research":selector='li div[class*="research"]';break;case "overview":this.data.eS[this.T].temp=
textContent[3].match(/(\-?[0-9]{1,3})/gi);this.data.k.commander=($("a.pic1").hasClass("on"))?"1":"0";this.data.k.admiral=($("a.pic2").hasClass("on"))?"1":"0";this.data.k.engineer=($("a.pic3")
.hasClass("on"))?"1":"0";this.data.k.geologist=($("a.pic4").hasClass("on"))?"1":"0";this.data.k.technocrat=($("a.pic5").hasClass("on"))?"1":"0";default:this.aD('planets','{'+this.bH(this.data.eS)+'}')
;this.aD('player','{'+this.bH(this.data.k)+'}');return false;}if(currentPage=="research"){var bv=this.data.k.tech;}else{var bv=this.data.eS[this.T];}bv.queue.id="0";bv.queue.time="";$(selector).each(
function(){var bh=$(this).attr('class').split(" ")[0].match(/([0-9]{1,3})/i)[1];var eg=$(this).find("span.level").text().match(/([0-9]{1,6})/i)[1];bv[bh]=eg;if(bh!="212"&&$(this).find("span.eckeoben")
.text()){bv.queue.id=bh;bv.queue.time=serverTime.getTime()+parseInt($("#box script:last").html().match(/baulisteCountdown\([^;]*;/gi)[0].split(",")[1]+"000");}});if(currentPage=="research"){this.aD(
'player','{'+this.bH(this.data.k)+'}');}this.aD('planets','{'+this.bH(this.data.eS)+'}');}catch(e){console.log("dX() error: "+e+".");$('#menuTable').after(
...
return false;}$("#detail").ajaxSuccess(function(e,xhr,settings){if(settings.url.match(/page=resources|page=station|page=research|page=shipyard|page=defense/g)){if(XiT.V.eblbuildingextend.aT()){var bh=
XiT.eo(bw("#detail",true).find("input[name='type']").val());XiT.cN(bh,1,true);}}});});this.ah=(function(value,to){if(this.V.eblshortenvalue.aT()){var ep=0;var dQ='';var fc=(value<0)?true:false;value=
Math.abs(value).toString();if(value.length>to){if(value.length-3<to){}else if(value.length-6<to){value=parseFloat(value)/Math.pow(10,6);dQ=LocalizationStrings['unitMega'];}else if(value.length-9<to){
value=parseFloat(value)/Math.pow(10,9);dQ=LocalizationStrings['unitMilliard'];}ep=(value-Math.floor(value)==0)?0:2;}if(fc)value=0-value;return number_format(value,ep)+dQ;}return this.format(value);});
this.bm=null;this.cN=(function(id,aX,eI){var by=bw("#detail");var dK=by.find("input[id='number']").attr('name');var ao=this.data.eS[this.T];if(aX<1)aX=1;if(!dK){var aQ=(currentPage=="research")?
this.data.k.tech[id].bn():ao[id].bn();var L=J.aM.fe(id,aQ);}else{var L=J.aM.fe(id,aX);}dE=(J.j.cX==id||J.j.bY==id||J.j.eZ==id||J.j.dw==id||J.j.dH==id||J.dJ.cc==id)?true:false;if(eI){var ee=by.find(
".enter").html();var bz=by.find("#resources");bz.html("");by.find("#action li").not(":first,.techtree,.demolish").remove();if(L.m>0){bz.append(
...
var aL=(typeof(XiT.data.aL)=="object")?XiT.data.aL:$.parseJSON(XiT.aS('strings'));html=$(html);html.find("a[href*='page=techinfo&techID=']").each(function(){var bh=$(this);var kf=bh.attr("href")
.match(/techinfo&techID=(\d{1,3})/)[1];var jS=$.trim(bh.text());aL[kf]=jS;});XiT.V.strloaded="1";XiT.aY("op",'{'+XiT.bH(XiT.V)+'}');XiT.aD('strings','{'+XiT.bH(aL)+'}');};if(jU=="serverData"){var url=
'/api/serverData.xml';var lo=eV;}else if(jU=="techStrings"){var url='/game/index.php?page=globalTechtree&techID=1';var lo=fD;}$.ajax({type:'GET',url:url,cache:false,success:lo});});this.kj=(function()
...
.find("table tr:even").addClass("alt");$("#XiInfoProducDef table td:nth-child(3n+1)").css("text-align","left");$("#XiInfoProducShip table td:nth-child(3n+1)").css("text-align","left");});this.eu=
"data:image/gif;base64,"+"R0lGODlhJgAdAPeeABMaIBUeIhATFAMDBBIaHxYgJhIZHzQ4PBIaHgUKDRAXGy4zNjA1OAQFBwULDRAZHREYHQIDBAQGCAQFCAQF"+
...
"IIKXG4Qp5hVXiLlBmWGWKYZSvDwBRRi33HLBnBTUaWctddYy5wWb9LkJmwIFsgUqpmBgaAIHIJqoBIs2SgIJgEYq6aSUAhoQADs=";this.ac=call;this.be="";this.jq="";this.jr=0;this.iA={};this.lz=false;this.kF=false;this.jO=null;
this.bL=document.getElementsByName('ogame-universe')[0].content;this.bA=document.getElementsByName('ogame-universe-speed')[0].content;this.hU=document.getElementsByName('ogame-player-id')[0].content;
this.fG=document.getElementsByName('ogame-player-name')[0].content;this.cT=document.getElementsByName('ogame-alliance-id')[0].content;this.gA=document.getElementsByName('ogame-alliance-tag')
[0].content;this.gK=document.getElementsByName('ogame-alliance-name')[0].content;this.T=document.getElementsByName('ogame-planet-id')[0].content;this.gw=document.getElementsByName('ogame-planet-name')
[0].content;this.cj=document.getElementsByName('ogame-planet-coordinates')[0].content;this.jE=document.getElementsByName('ogame-planet-type')[0].content;this.eY=(currentPage.match(
/techinfo|notices|showmessage|phalanx|buddies|^search/i))?false:true;this.fM();if(this.V.eblbuildingextend.aT()){this.data.eS=$.parseJSON(this.aS('planets'));this.data.k=$.parseJSON(this.aS('player'))
;if(this.V.ebleasytransp.aT()){this.data.aL=$.parseJSON(this.aS('strings','{}'));this.data.eE=$.parseJSON(this.aS('transport','{}'));}}if(this.eY){this.fh();this.call();if(this.V.eblbuildingextend.aT(
)){this.dX();this.fH();if(this.V.ebleasytransp.aT()&&currentPage.match(/resources|station|research|shipyard|defense|fleet1|fleet2|fleet3/i)){this.jB();}this.eA();}}};function hx(){
XiTClass.prototype.eW=false;XiTClass.prototype.ku=(function(){this.ib=this.az()['galaxy'];this.kb=this.az()['system'];this.kJ=(this.az()['position'])?this.az()['position']:this.az()['planet'];
this.data.bE=$.parseJSON(this.aS('gmark'));$("#galaxyheadbg td").append('<div style="position:absolute; right:42px;top:42px;"> '+fp.eD+
...
"markedOpen")){$("#trmarked").slideUp('fast');$(this).removeClass('markedOpen');$(this).children().attr("src","http://gf2.geo.gfsrv.net/cdnad/5133388a9b6282d6c7e7f00e2beb6e.gif");}else{var O=XiT.ks();
$("#trmarked").slideDown('fast');$(this).addClass('markedOpen');$(this).children().attr("src","/cdn/img/layout/fleetCloseAll.gif");}return false;})});XiTClass.prototype.eN=(function(){return galaxy});
...
url:'/api/playerData.xml?id='+k,cache:false,dataType:($.browser.msie)?'text':'xml',success:function(data){var xml;if(typeof data=='string'){xml=new ActiveXObject('Microsoft.XMLDOM');xml.async=false;
xml.loadXML(data);}else{xml=data;}var fR=new Array();var ej=new Array();var fB="";var O=0;$(xml).find('planet').each(function(){var id=$(this).attr('id');var name=$(this).attr('name');var coords=$(
this).attr('coords').split(":");var eF="["+$(this).attr('coords')+"]";var galaxy=coords[0];var system=coords[1];var pos=coords[2];var kd=$(this).find("moon").length?true:false;var kB=(O==0)?true:
false;fR.push(eF);ej[eF]=new Array();ej[eF][0]=id;ej[eF][1]=name;ej[eF][2]=$(this).attr('coords').split(":");ej[eF][3]=kd;ej[eF][4]=kB;O++;});fR.sort(XiT.dk);for(var i=0;i<fR.length;i++){jT=(i<9)?(
"0"+(i+1)):(i+1);eO=fR[i];fB+="<br>"+jT+" - <a href='javascript:void(0);' onClick='XiT.iW("+ej[eO][2][0]+","+ej[eO][2][1]+","+ej[eO][2][2]+");'>";fB+="["+ej[eO][2][0]+":"+ej[eO][2][1]+":"+
ej[eO][2][2]+"]";fB+=(ej[eO][4])?(" ["+fp.cB+"]"):"";fB+=(ej[eO][3])?(" ["+fp.dr+"]"):"";fB+="</a>";}var hw="<u>"+fp.cF+":</u>";hw+=fB+"<br><br><u>"+fp.ce+":</u>";$(xml).find('position').each(
function(){var line="";var ha="<span class='%c'>"+XiT.format($(this).attr('score'))+"</span>";var ko=($(this).attr('score')>=0)?"undermark":"overmark";var dh=$(this).attr('ships');ha=ha.replace("%c",
ko);lx="<span class='"+(($(this).text()<100)?"overmark":"undermark")+"'><strong>"+XiT.format($(this).text())+"</strong></span>";line=fp.bG[$(this).attr('type')].replace("%t",lx).replace("%p",ha);
line+=(dh)?'<br><span style="margin-left:40px;"><i>(<span style="color:#FF6">'+XiT.format(dh)+'</span> '+fp.bG[8]+')</i></span>':'';hw+="<br>"+line;});XiT.gJ[k]=hw;$("#player"+k+" div.player-more")
.html(hw);$("div#player"+k).each(function(){Tipped.refresh($(this)[0]);});}});});XiTClass.prototype.gJ={};$("#galaxyContent").ajaxSuccess(function(e,xhr,settings){if(settings.url.indexOf(
...
"#probeValue").html()));if(XiT.fG!=kT&&kv>0){eP.find("#moon"+eL+" .ListLinks").append('<li><a onclick="sendShips(6, '+XiT.eN()+', '+XiT.ft()+', '+eL+', 3, '+spionageAmount+
');return false" href="javascript:void(0);">'+fp.ai+'</a></li>');}});}if(XiT.V.eblhld.aT()||XiT.V.eblrecy.aT()){eP.find('div[id*="debris"]:not(.XiT-done)').each(function(){var ed=$(this);ed.addClass(
...
".ListLinks li:last").html('<a onclick="sendShips(8, '+XiT.eN()+', '+XiT.ft()+', '+eL+', 2, '+km+');return false" href="javascript:void(0);">'+fp.dR+'</a>');}else{ed.find(".ListLinks li:last").html(
'<a href="#" onclick="fadeBox(\''+fp.gr+'\',true,\'\');return false">'+fp.dR+'</a>');}}if(XiT.V.eblhld.aT()){if(jl>=XiT.V['debrishigh']){eP.find('a[rel="debris'+eL+'"] img').attr("src",kH);}else if(
jl>=XiT.V['debrislow']){eP.find('a[rel="debris'+eL+'"] img').attr("src",kS);}}});}eP.find('td[rel*="planet"]:not(.XiT-done)').each(function(){$(this).addClass("XiT-done");var eL=$(this).attr('rel')
.replace("planet","");var tr=eP.find("tbody tr:nth-child("+(eL.bn())+")");var hv=tr.find(".allytag span:first").attr("rel");var kh=tr.find(".planetname").hasClass('status_abbr_buddy');var ie=(
...
coords+=aJ.find(".originPlanet").eq(0).text().match(/[\(\)]/i)?"["+fp.dr+"]":"";}else{coords=aJ.find(".destinationCoords").eq(0).text();coords+=aJ.find(".destinationPlanet").eq(0).text().match(
/[\(\)]/i)?"["+fp.dr+"]":"";}if(F[coords]){bJ+=F[coords][0];bI+=F[coords][1];ca+=F[coords][2];}F[coords]=[bJ,bI,ca];}}});this.fL(F);}});XiTClass.prototype.fL=(function(F){var aR="["+this.cj+"]"+(
...
"data:image/gif;base64,"+"R0lGODlhEAAQAJEAAG6JnoKcr////////yH5BAEAAAMALAAAAAAQABAAAAItHI6Zpu0PnRAxzYqupJk3fYDBOE6BaZIqKpytSrIu"+"HLcyPOX6S/f+TxoAVYMCADs=";$("#eins").ajaxSuccess(function(e,xhr,
...
"position","relative");wf.find("#mission tr:first th:last").text(fp.hn+":");wf.find("#shortcuts").hide();wf.find(".briefing").css("width","455px");wf.find("#fleetBriefingPart1").after(
'<br class="clearfloat">');wf.find(".briefing li").css("width","380px");wf.prepend('<div id="XShortcut" class="border5px"><table width="100%">'+gn+'</table></div>');});XiTClass.prototype.gG=(function(
hi){var fY=hi.split("#");$("#galaxy").val(fY[0]);$("#system").val(fY[1]);$("#position").val(fY[2]);setTType(fY[3]);(fY[4]=="-")?'':$("#targetPlanetName").html(fY[4]);targetGalaxy=fY[0];targetSystem=
fY[1];targetPosition=fY[2];updateVariables();});var jg="data:image/gif;base64,"+"R0lGODlhFwASAPe9AAAAAP///xYXGxkbIAIECAYIDAgKDgkLDwoMEAsNEQ0PExgaHhsdISkrLwsOEw0QFRATGBodIh0gJSQnLAAC"+
...
document.getElementsByName('ogame-language')[0].content;switch(al){case "mx":case "ar":al="es";}if(al!='es'&&al!='en'&&al!='ru'&&al!='it'&&al!='pl'&&al!='fr'){al='en';}var fp=aK(al);var currentPage=
gE();co+="var currentPage = '"+currentPage+"'; ";co+="var fp=aK('"+al+"'); ";ir+="var bw = XiT.bw;";switch(currentPage){case "galaxy":ak+=hx.toString()+" hx();";ac="ku";break;case "movement":ak+=
fs.toString()+" fs();";ac="initMovement";break;case "resources":break;case "fleet2":case "fleet3":ak+=hX.toString()+" hX();";ac="hQ";break;case "research":break;case "messages":case "showmessage":ak+=
gU.toString()+" gU();";break;case "alliance":ak+=hs.toString()+" hs();";break;case "overview":ac="gp";break;default:}function ea(){var aE=this;this.data={1:{m:60,c:15,d:0,e:0,f:1.5,p: -10,i: -1000},2:
{m:48,c:24,d:0,e:0,f:1.6,p: -10,i: -1300},3:{m:225,c:75,d:0,e:0,f:1.5,p: -20,i: -1200},4:{m:75,c:30,d:0,e:0,f:1.5,p:20,i: -1100},12:{m:900,c:360,d:180,e:0,f:1.8,p:30,i: -900},14:{m:400,c:120,d:200,e:
0,f:2.0,i: -1000},15:{m:1000000,c:500000,d:100000,e:0,f:2.0,i: -1000},21:{m:400,c:200,d:100,e:0,f:2.0,i: -1000},22:{m:1000,c:0,d:0,e:0,f:2.0,i: -1000},23:{m:1000,c:500,d:0,e:0,f:2.0,i: -1000},24:{m:
1000,c:1000,d:0,e:0,f:2.0,i: -1000},25:{m:2645,c:0,d:0,e:0,f:2.3,i: -1000},26:{m:2645,c:1322.5,d:0,e:0,f:2.3,i: -1000},27:{m:2645,c:2645,d:0,e:0,f:2.3,i: -1000},31:{m:200,c:400,d:200,e:0,f:2.0,i: -
1000},33:{m:0,c:50000,d:100000,e:1000,f:2.0,i: -1000},34:{m:20000,c:40000,d:0,e:0,f:2.0,i: -1000},41:{m:20000,c:40000,d:20000,e:0,f:2.0,i: -1000},42:{m:20000,c:40000,d:20000,e:0,f:2.0,i: -1000},43:{m:
2000000,c:4000000,d:2000000,e:0,f:2.0,i: -1000},44:{m:20000,c:20000,d:1000,e:0,f:2.0,i: -1000},106:{m:200,c:1000,d:200,e:0,f:2.0,n: -8,l:3},108:{m:0,c:400,d:600,e:0,f:2.0,n: -10,l:1},109:{m:800,c:200,
d:0,e:0,f:2.0,n: -3,l:4},110:{m:200,c:600,d:0,e:0,f:2.0,n: -6,l:6},111:{m:1000,c:0,d:0,e:0,f:2.0,n: -2,l:2},113:{m:0,c:800,d:400,e:0,f:2.0,n: -12,l:1},114:{m:0,c:4000,d:2000,e:0,f:2.0,n:8,l:7},115:{m:
400,c:0,d:600,e:0,f:2.0,n: -6,l:1},117:{m:2000,c:4000,d:600,e:0,f:2.0,n: -6,l:2},118:{m:10000,c:20000,d:6000,e:0,f:2.0,n: -8,l:7},120:{m:200,c:100,d:0,e:0,f:2.0,n:12,l:1},121:{m:1000,c:300,d:100,e:0,
f:2.0,n:5,l:4},122:{m:2000,c:4000,d:1000,e:0,f:2.0,n:7,l:4},123:{m:240000,c:400000,d:160000,e:0,f:2.0,n: -1,l:10},124:{m:4000,c:8000,d:4000,e:0,f:1.75,n: -1,l:3},199:{m:0,c:0,d:0,e:300000,f:3.0,n:1,l:
12},202:{m:2000,c:2000,d:0,C:400,K:10,G:5,bU:5000,bP:115,ad:5000,bV:10,aB:117,bF:10000,aW:20,ar:5,tr:0.25},203:{m:6000,c:6000,d:0,C:1200,K:25,G:5,bU:25000,bP:115,ad:7500,bV:50,aB:0,bF:0,aW:0,ar:0,tr:
0.25},204:{m:3000,c:1000,d:0,C:400,K:10,G:50,bU:50,bP:115,ad:12500,bV:20,aB:0,bF:0,aW:0,ar:0,tr:1},205:{m:6000,c:4000,d:0,C:1000,K:25,G:150,bU:100,bP:117,ad:10000,bV:75,aB:0,bF:0,aW:0,ar:0,tr:1},206:{
m:20000,c:7000,d:2000,C:2700,K:50,G:400,bU:800,bP:117,ad:15000,bV:300,aB:0,bF:0,aW:0,ar:0,tr:1},207:{m:45000,c:15000,d:0,C:6000,K:200,G:1000,bU:1500,bP:118,ad:10000,bV:500,aB:0,bF:0,aW:0,ar:0,tr:1},
208:{m:10000,c:20000,d:10000,C:3000,K:100,G:50,bU:7500,bP:117,ad:2500,bV:1000,aB:0,bF:0,aW:0,ar:0,tr:0.25},209:{m:10000,c:6000,d:2000,C:1600,K:10,G:1,bU:20000,bP:115,ad:2000,bV:300,aB:0,bF:0,aW:0,ar:
0,tr:0.25},210:{m:0,c:1000,d:0,C:100,K:0,G:0,bU:5,bP:115,ad:100000000,bV:1,aB:0,bF:0,aW:0,ar:0,tr:0},211:{m:50000,c:25000,d:15000,C:7500,K:500,G:1000,bU:500,bP:117,ad:4000,bV:1000,aB:118,bF:5000,aW:
1000,ar:8,tr:1},212:{m:0,c:2000,d:500,C:200,K:1,G:1,bU:0,bP:0,ad:0,bV:0,aB:0,bF:0,aW:0,ar:0,tr:0},213:{m:60000,c:50000,d:15000,C:11000,K:500,G:2000,bU:2000,bP:118,ad:5000,bV:1000,aB:0,bF:0,aW:0,ar:0,
tr:1},214:{m:5000000,c:4000000,d:1000000,C:900000,K:50000,G:200000,bU:1000000,bP:118,ad:100,bV:1,aB:0,bF:0,aW:0,ar:0,tr:1},215:{m:30000,c:40000,d:15000,C:7000,K:400,G:700,bU:750,bP:118,ad:10000,bV:
250,aB:0,bF:0,aW:0,ar:0,tr:1},401:{m:2000,c:0,d:0,C:200,K:20,G:80},402:{m:1500,c:500,d:0,C:200,K:25,G:100},403:{m:6000,c:2000,d:0,C:800,K:100,G:250},404:{m:20000,c:15000,d:2000,C:3500,K:200,G:1100},
405:{m:2000,c:6000,d:0,C:800,K:500,G:150},406:{m:50000,c:50000,d:30000,C:10000,K:300,G:3000},407:{m:10000,c:10000,d:0,C:2000,K:2000,G:1},408:{m:50000,c:50000,d:0,C:10000,K:10000,G:1},502:{m:8000,c:0,
d:2000,C:800,K:1,G:1},503:{m:12500,c:2500,d:10000,C:1500,K:1,G:12000}};this.list={j:[1,2,3,4,12,14,15,21,22,23,24,25,26,27,31,33,34,41,42,43,44],bj:[106,108,109,110,111,113,114,115,117,118,120,121,
122,123,124,199],gL:[113,120,121,114,122,115,117,118,106,108,124,123,199,109,110,111],dh:[202,203,204,205,206,207,208,209,210,211,212,213,214,215],gV:[202,203,204,205,206,207,208,209,210,211,213,214,
215],ew:[401,402,403,404,405,406,407,408,502,503],hk:[401,402,403,404,405,406,407,408]};this.ew={gH:401,gy:402,bl:403,gk:404,fi:405,eU:406,ho:407,gi:408,hH:502,hy:503};this.dJ={gS:202,he:203,gM:204,
gl:205,gT:206,fP:207,hE:208,gB:209,ai:210,gW:211,cc:212,dd:213,hm:214,hf:215};this.bj={bK:113,gO:120,fi:121,hA:114,eU:122,fO:115,gq:117,gh:118,gv:106,gP:108,gc:124,fT:123,gQ:199,gd:109,ew:110,K:111};
this.j={cX:1,bY:2,eZ:3,dw:4,dH:12,gm:22,fW:23,hu:24,fU:25,hg:26,fS:27,ef:14,aO:15,fI:31,gF:33,ag:21,hC:34,gf:41,es:42,hF:43,fZ:44};this.iV={1:0,2:1,3:2,4:3,12:4,14:5,15:6,21:7,22:8,23:9,24:10,25:11,
26:12,27:13,31:14,33:15,34:16,41:17,42:18,43:19,44:20,106:21,108:22,109:23,110:24,111:25,113:26,114:27,115:28,117:29,118:30,120:31,121:32,122:33,123:34,124:35,199:36,202:37,203:38,204:39,205:40,206:
41,207:42,208:43,209:44,210:45,211:46,212:47,213:48,214:49,215:50,401:51,402:52,403:53,404:54,405:55,406:56,407:57,408:58,502:59,503:60};this.iH=null;this.jn=(function(){if(aE.iH!=null)return aE.iH;
if(XiT.data.k.commander.aT()&&XiT.data.k.admiral.aT()&&XiT.data.k.engineer.aT()&&XiT.data.k.geologist.aT()&&XiT.data.k.technocrat.aT()){aE.iH=true;}else{aE.iH=false;}return aE.iH;});this.aM={fe:
function(id,bf){var H={"m":0,"c":0,"d":0,"e":0};var item=aE.data[id];if(typeof item==="undefined"){return H;}if(typeof item.f!=="undefined"){if(id===124){H.m=100*Math.ceil(item.m/100*Math.pow(item.f,
bf));H.c=100*Math.ceil(item.c/100*Math.pow(item.f,bf));H.d=100*Math.ceil(item.d/100*Math.pow(item.f,bf));}else{H.m=Math.floor(item.m*Math.pow(item.f,bf));H.c=Math.floor(item.c*Math.pow(item.f,bf));
H.d=Math.floor(item.d*Math.pow(item.f,bf));H.e=Math.floor(item.e*Math.pow(item.f,bf));}}else{H.m=item.m*bf;H.c=item.c*bf;H.d=item.d*bf;}return H;},fC:function(temp,count){return Math.floor(Math.floor(
(temp+140)/6)*count*1*(XiT.data.k.engineer.aT()?(aE.jn()?1.12:1.1):1.0));},dC:function(id,level){var c=1.1;var item=aE.data[id];var o;if(typeof item==='undefined'||typeof item.p==='undefined'){
return 0;}o=XiT.data.k.engineer.aT()===true&&item.p>0?(aE.jn()?1.12:1.1):1.0;if(id===12){c=1.05+0.01*aE.data[113].v;}return Math.floor(o*item.p*level*Math.pow(c,level));},fl:function(id,level){
return level>=0&&id===aE.j.dH?Math.ceil(10*level*Math.pow(1.1,level)):0;},fj:function(id,count){return((J.data[id].m+J.data[id].c)/(2500*(aE.data[21].v+1)*Math.pow(2,aE.data[15].v)*XiT.bA.bn()))*3600*
count;},hz:function(id,level,aO){var bC=0;if(level>=0){switch(1*id){case aE.j.ef:case aE.j.ag:bC=(2500+2500*level)*Math.pow(2,aO)*XiT.bA.bn();break;case aE.j.fI:bC=(1000+1000*level)*XiT.bA.bn();break;
default:}}return bC;},hq:function(id,level,temp,cl){var bC=0;var ax;var bN;if(level>=0){ax=XiT.data.k.geologist.aT()?(aE.jn()?1.12:1.1):1.0;bN=XiT.bA.bn();switch(id){case aE.j.cX:bC=bN*((cl?0:30)+
Math.floor(ax*30*level*Math.pow(1.1,level)));break;case aE.j.bY:bC=bN*((cl?0:15)+Math.floor(ax*20*level*Math.pow(1.1,level)));break;case aE.j.eZ:bC=bN*(0+Math.floor(ax*10*level*Math.pow(1.1,level)*(
1.44-0.004*temp)));break;default:}}return bC;},fJ:function(L,H){var time=0;var dB=0;var cz;var ao=XiT.data.eS[XiT.T];if(L.e==0){if(!(L.d>0&&(ao.dp.bn()==0))){for(var aA in H){if(H[aA]<0){cz=ao[aA+
"p"].bn();dB=Math.floor(Math.abs(H[aA])/(cz/3600));if(dB>time)time=dB;}}}else{time=false;}}else{time=false;}return time;},eK:function(L){var aX=99999;var iv;var ao=XiT.data.eS[XiT.T];for(var aA in L){
if(L[aA]>0&&aA!="e"){iv=Math.floor(ao[aA].bn()/L[aA]);if(iv<aX)aX=iv;}}return aX;},gx:function(id,level){return level>=0&&id===aE.j.es?level*level:0;},jC:function(start,end){var dG=Math.abs(start[0]-
end[0]);var dS=Math.abs(start[1]-end[1]);var dP=Math.abs(start[2]-end[2]);var bC=5;if(dG>0){bC=dG*20000;}else if(dS>0){bC=dS*95+2700;}else if(dP>0){bC=dP*5+1000;}return bC;},jf:function(id){var dJ=
aE.data[id];var bC=dJ.bV;if(typeof dJ.bP==='undefined'){bC=0;}else if(dJ.ar>0&&XiT.data.k.tech[dJ.aB]>=dJ.ar){bC=dJ.aW;}return bC;},jp:function(id){var dJ=aE.data[id];var hY;var iC;var f=0;if(
typeof dJ.bP==='undefined'){return 0;}hY=dJ.bP;iC=dJ.ad;if(dJ.ar>0&&XiT.data.k.tech[dJ.aB]>=dJ.ar){hY=dJ.aB;iC=dJ.bF;}switch(hY){case 115:f=0.1;break;case 117:f=0.2;break;case 118:f=0.3;break;default:
}return iC*(1+f*XiT.data.k.tech[hY]);},iM:function(hV,percent){var max=0;for(key in hV){if(hV[key][1]<1){return;}var s=aE.aM.jp(hV[key][0]);max=max===0?s:Math.min(max,s);}return max*percent;},jy:
function(d,iw,iZ){return Math.round(((3500/iw*Math.sqrt(d*10/iZ)+10)/XiT.bA.bn()));},iX:function(dh,duration,distance,iT){var jh=0;var hK=0;var bC=[];var z;var z1;var z2;for(key in dh){z=dh[key][1]*
aE.aM.jf(dh[key][0]);if(z>0){z1=z*distance/35000*Math.pow(3500/(duration*XiT.bA.bn()-10)*Math.sqrt(distance*10/aE.aM.jp(dh[key][0]))+1,2);z2=z*iT;jh+=z1;hK+=z2;z1=Math.round(z1)+1;z2=iT>0?Math.max(
Math.floor(z2/10),1):0;bC[key]=[z1+z2,z1,z2];}else{bC[key]=[0,0,0];}}if(jh>0){z1=Math.round(jh)+1;z2=iT>0?Math.max(Math.floor(hK/10),1):0;bC["total"]=[z1+z2,z1,z2]}else{bC["total"]=[0,0,0];}return bC;
}};};var iE="data:image/png;base64,"+"/9j/4QAYRXhpZgAASUkqAAgAAAAAAAAAAAAAAP/sABFEdWNreQABAAQAAAA8AAD/4QNtaHR0cDovL25zLmFkb2JlLmNvbS94YXAv"+
...
fp.jQ+'</div><div class="fieldvalue"> <input type="checkbox" id="XiTeblspioplunder" value="1"></div> </div> <div class="fieldrow"> <div class="fieldlabel">'+fp.jK+
