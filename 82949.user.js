// ==UserScript==
// @name           BBCodes para Taringa - Fix Imagenes Tinypic
// @namespace      SharkaleBBCODER
// @description    BBCoder para T! y P! (Crear post, comentarios, menú) ATENCION: Si te molesta ser miembro de mi comunidad no instales el script!
// @include        http://*taringa.net/*
// @include        http://*poringa.net/*
// @include        http://plugin.tinypic.com/*
// @exclude        http://br.taringa.com/*
// @require        http://www.sharkale.com.ar/bbcoder/script/jquery.js?v=29.3
// @require        http://www.sharkale.com.ar/bbcoder/script/plugins.js?v=29.3
// @version        29.31
// @copyright      Copyright (c) 2010, Sharkale
// @creator        Sharkale (Alejandro Barreiro) ® 2010
// ==/UserScript==
...
switch(pathA[1]){
	...
	case '':case 'index.php':case 'categorias':
...
var HideComment = GM_getValue("HideComment",false);
	...
	if(CambiarTheme) GM_registerMenuCommand("BBCoder para T! y P! - Cambiar el theme de la página", Cambiar_Theme);
	...
	GM_registerMenuCommand("BBCoder para T! y P! - Versión Instalada: " + ultversion, function() {
		unsafeWindow.mydialog.alert("BBCoder para T! y P! - Sharkale®","Tu versión del script instalada actualmente es: " + ultversion);
	},null,null,"V");
...
function CrearBarraBBC(){
		...
		if(path == 'post' || path == 'comunidades'){
			nuevaHTML +='&nbsp;<a href="" onclick="act_comment();return false;"><img src="http://imgur.com/pVzhG.png" class="carg_act_comm" title="Actualizar Comentarios" height="16" style="margin-left:5px;"/></a>';
		}
		nuevaHTML += '</div>';
		return nuevaHTML;
}
...
function Cargar_Emoticones(lista){
	...
	for (var i = 0; i < emot.length; i=i+2){
		nroemot++;
		html += '<span id="shark_emot_'+nroemot+'" class="ui-state-default"><img src="'+emot[i+1]+'" alt="" title="'+emot[i]+'" style="height:40px;width:40px;"><a href="" onclick="$(\'#shark_emot_'+nroemot+'\').remove();return false;"><img src="http://imgur.com/jvjOv.gif" alt="Eliminar" title="Eliminar" height="12" width="12"></a><input type="hidden" name="sharkale_emot_'+nroemot+'" value="'+emot[i]+'"><input type="hidden" name="sharkale_emot_'+nroemot+'" value="'+emot[i+1]+'"></span>';
	}
	return html;
}
		...
		if(urlbarra.search(/^https?:\/\//gi) != -1 && urlbarra.search(/,/) == -1){
			...
			if(urlemot != '' && urlemot != null){
				if(urlemot.search(/^https?:\/\//gi) != -1 && urlemot.search(/,/) == -1){
					nuevosemot++;
					html = '<span id="shark_emot_nueva_'+nuevosemot+'" class="ui-state-default"><img src="'+urlbarra+'" alt="" title="[img='+urlemot+']" style="height:40px;width:40px;"><a href="" onclick="$(\'#shark_emot_nueva_'+nuevosemot+'\').remove();return false;"><img src="http://imgur.com/jvjOv.gif" alt="Eliminar" title="Eliminar" height="12" width="12"></a><input type="hidden" name="sharkale_emot_nueva_'+nuevosemot+'" value="[img]'+urlemot+'[/img]"><input type="hidden" name="sharkale_emot_nueva_'+nuevosemot+'" value="'+urlbarra+'"></span>';
...
case 'letrota': //Link
	...
	unsafeWindow.mydialog.title('Letrota - Sharkale®');
...
function procesar_resultados(nick,datos,pos,sexo){
	...
	for(var i=0; i<datos.size();i++){
		...
		if(nickonline.toLowerCase() == nick.toLowerCase()) {
			encontrado = true;
			$('.sendMsg:last a').html('<img src="http://imgur.com/Wz4Mg.png" height="16" align="absmiddle"> Usuario Online');
			return;
		}
	}
	if(!encontrado) {
...
Taringa_Set_Function("verificar_estado", function(nick,sexo,sig,num){
	if(sig){
		if(num > 2000) {
			$('.sendMsg:last a').html('<img src="http://imgur.com/gynzr.png" height="16" align="absmiddle" title="Estado desconocido. Puede que este Online u Oculto"> Desconocido');
			return;
		}
		$.ajax({
 			type: 'GET',
			...
			dataType: 'text',
 			success: function(h){
				...
				}else $('.sendMsg:last a').html('<img src="http://imgur.com/16MhA.png" height="16" align="absmiddle" title="Estado desconocido. Puede que este Online u Oculto"> Desconocido');
 			},
 			error: function(h){
				error_online++;
			...
			dataType: 'text',
 			...
 			error: function(h){
				...
				if(error_online < 20) verificar_estado(nick,sexo);
					...
					$('.sendMsg:last a').html('<img src="http://imgur.com/QUMMp.png" height="16" align="absmiddle" title="Estado desconocido. Puede que este Online u Oculto"> Desconocido');
				}
 			}
 		});
	}
			...
			$.ajax({
					...
					data: 'titulo=Preview&cuerpo='+encodeURIComponent(txt.value),
 					success: function(h){
						...
						if(path == 'mensajes'){
							var HTML = '<br><div class="cita" style="color:#464646;text-shadow:0 1px 0 #CCCCCC;"><img src="'+URL+'cerrar.gif" alt="[X]" title="Cerrar Previsualización" border="0" onclick="($(\'#return_agregar_comentario\').slideUp(1000))" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />Previsualización - Sharkale®</div>';
							HTML += '<div class="citacuerpo">'+respuesta+'</div><br>';
						}else{
							var HTML = '<div class="respuesta clearfix"><div class="answerInfo"></div>';
							HTML += '<div class="answerTxt"><div class="answerContainer"><div class="Container" style="padding:0"><div class="answerOptions"><div class="floatL metaDataA"><img src="'+URL+'cerrar.gif" alt="[X]" title="Cerrar Previsualización" border="0" onclick="($(\'#return_agregar_comentario\').slideUp(1000))" align="absmiddle" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />Previsualización - Sharkale®</div></div><div style="margin: 10px !important;" class="cuerpo_comm">'+respuesta+'</div></div></div></div></div>';
						}
 						$('#return_agregar_comentario').slideUp(1000, function(){
			...
			$('#post').html(HTML).slideDown(1500, function(){
				$.ajax({
					...
					success: function(h){
						...
						HTML += '<div class="comment-box"><div class="dialog-c"></div><div class="comment-info clearbeta"><div class="floatL"><img src="'+URL+'cerrar.gif" alt="[X]" align="absmiddle" title="Cerrar Previsualización" border="0" onclick="$(\'#post\').slideUp(1000)" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" /> <b>Previsualización - Sharkale®</b></div></div><div class="comment-content">'+respuesta+'</div></div></div>';
						$('#post').slideUp(1500, function(){
...
Taringa_Set_Function("validate_form", function(form, elementos){
	...
	if(errores != ''){
		unsafeWindow.mydialog.alert('Error al Crear Tema - Sharkale®','<div align="left" style="padding:0px 20px"><b>Se han encontrado los siguientes errores:<br><br><ul style="margin-left:20px">'+errores+'</ul></b></div>');
		return false;
	}
...
Taringa_Set_Function("show_preview", function(titulo,cuerpo,tags,f,forzar){
	...
	if(errores != ''){
		unsafeWindow.mydialog.alert('Error al Crear Post - Sharkale®','<div align="left" style="padding:0px 20px"><b>Se han encontrado los siguientes errores:<br><br><ul style="margin-left:20px">'+errores+'</ul></b></div>');
		return false;
	}
...
Taringa_Set_Function("act_comment", function(pos){
	if(en_comm) return;
	en_comm = true;
	$('.carg_act_comm').attr('src','http://imgur.com/lZehg.gif');
	...
	$.ajax({
		...
		url: url_comm,
		success: function(h){
			$('.carg_act_comm').attr('src','http://imgur.com/AuDTV.png');
			if(path == 'comunidades'){
				...
				$('.titulorespuestas').append(' <a href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"> <img src="http://imgur.com/UBm8M.png" class="carg_act_comm" title="Actualizar Comentarios" height="15" align="absmiddle" style="margin-left:5px;"/></a>');
			  ...
			  if($(h).find('.post-deleted').size() == 0){
				...
				if($('.paginadorCom ul').size() > 0)
					$('.paginadorCom ul').append('<li class="numbers"><a class="here" href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"><img src="http://imgur.com/DlAbE.png" class="carg_act_comm" title="Actualizar Comentarios" height="14" /></a></li>');
				else
					$('.titulorespuestas').append(' <a href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"> <img src="http://imgur.com/DlAbE.png" class="carg_act_comm" title="Actualizar Comentarios" height="15" align="absmiddle" style="margin-left:5px;"/></a>');
			...
			$('.comment-info div.floatR ul').append('<li><a href="#maincontainer" title="Ir a la Cielo"><img src="http://imgur.com/vNIEB.png"/></a></li><li><a href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/KWYmq.png"/></a></li>');
		...
		error: function(h){
			$('.carg_act_comm').attr('src','http://imgur.com/7whOT.png');
			en_comm = false;
		}
	});
});
	...
	$.ajax({
		...
		success: function(h){
			...
			$('#derecha').fadeOut('slow',function(){
				...
				$("div[class='box_txt ultimos_posts']:last").html('Post de Novatos <a href="" onclick="act_post_nov();return false;" title="Actualizar Post" > <img src="http://imgur.com/7whOT.png" align="top" height="16" width="16" /></a>');
	...
	$.ajax({
		...
		success: function(h){
			$('#izquierda').fadeOut('slow',function(){
				...
				$("div[class='box_txt ultimos_posts']:first").html('Últimos posts <a href="" onclick="act_post_nfu();return false;" title="Actualizar Post" > <img src="http://imgur.com/7whOT.png" align="top" height="16" width="16" /></a>');
...
Taringa_Set_Function("administrar_post", function(key, numpost){
	...
	$.ajax({
		...
		success: function(h){
			...
			HTML = '<div style="font-size: 11px; text-align:center;"><a href="#abajo" id="arriba"> <img src="http://imgur.com/71D9O.png" align="top" height="16" width="16" /> <strong>Ir a la Tierra</strong></a> | <a href="" onclick="administrar_post(\''+key+'\',\''+numpost+'\');return false;"> <img src="http://i.imgur.com/7whOT.png" align="top" height="16" width="16" /> <strong>Actualizar Comentarios</strong></a></div><span class="Sharkale_cargando"><img src="'+URL+'cargando.gif" width="16" height="16" align="absmiddle"/> Por favor espere...</span>'+HTML;
			HTML += '<div style="font-size: 11px; text-align:center;"><a href="#arriba" id="abajo"> <img src="http://imgur.com/zEwn0.png" align="top" height="16" width="16" /> <strong>Ir al Cielo</strong></a> | <a href="" onclick="administrar_post(\''+key+'\',\''+numpost+'\');return false;"> <img src="http://i.imgur.com/7whOT.png" align="top" height="16" width="16" /> <strong>Actualizar Comentarios</strong></a></div>';
...
function add_favs_tema(temaid){
	...
	if(favs != ''){
		...
		for (var i = 0; i < favs.length-1; i=i+3){
			if(favs[i] == temaid){
				unsafeWindow.mydialog.alert("Favoritos de Comunidades - Sharkale®",'Esta tema ya esta guardado en favoritos.');
				return;
			}
		}
		GM_setValue('temasFavs',favs.join('//#//')+'//#//'+aguardar);
		unsafeWindow.mydialog.alert("Favoritos de Comunidades - Sharkale®",'El tema fue guardado correctamente!');
	}else{
		unsafeWindow.mydialog.alert("Favoritos de Comunidades - Sharkale®",'El tema fue guardado correctamente!');
...
// ==URL de Imagenes==
var MenuTaringa = GM_getValue("MT", "http://imgur.com/tO98D.png");
var MenuMensajesVacio = GM_getValue("MMV", "http://imgur.com/4UbZF.png");
var MenuMensajesLleno = GM_getValue("MML", "http://imgur.com/A7a7a.png");
var MenuMonitor = GM_getValue("MM", "http://imgur.com/rkIJO.png");
var MenuFavoritos = GM_getValue("MF", "http://imgur.com/c0XhY.png");

var MenuBorradores = GM_getValue("MB", "http://imgur.com/9bQWU.png");

var MenuCuenta = GM_getValue("MC", "http://imgur.com/YurUO.png");
var MenuCerrarSesion = GM_getValue("MCS", "http://imgur.com/m4Vx1.png");
var MenuModeracion = GM_getValue("MMod", "http://imgur.com/TVWX3.png");
var MenuOpciones = GM_getValue("MO", "http://imgur.com/P3PQI.png");
var MenuActualizacion = GM_getValue("MA", "http://imgur.com/Q6fXa.png");
...
function Restablecer_Imagenes(){
	$('#mt-url').val("http://imgur.com/yzUqL.png");
	$('#mmv-url').val("http://imgur.com/F0Tkh.png");
	$('#mml-url').val("http://imgur.com/8wVxo.png");
	$('#mm-url').val("http://imgur.com/7rIpP.png");
	$('#mf-url').val("http://imgur.com/qgLuv.png");
	$('#mb-url').val("http://imgur.com/9bQWU.png");
	$('#mc-url').val("http://imgur.com/7yw09.png");
	$('#mcs-url').val("http://imgur.com/FZzQE.png");
	$('#mmod-url').val("http://imgur.com/7zZva.png");
	$('#mo-url').val("http://imgur.com/6LD9l.png");
	$('#ma-url').val("http://imgur.com/Q6fXa.png");
	$('#ms').attr("checked", "checked");
	Guardar_Imagenes();
}
function Restablecer_Imagenes_T(){
	$('#mt-url').val("http://imgur.com/yzUqL.png");
	$('#mmv-url').val("http://imgur.com/garCF.png");
	$('#mml-url').val("http://imgur.com/iFhzR.png");
	$('#mm-url').val("http://imgur.com/hFgAg.png");
	$('#mf-url').val("http://imgur.com/A5BGe.png");
	$('#mb-url').val("http://imgur.com/9bQWU.png");
	$('#mc-url').val("http://imgur.com/hRdmk.png");
	$('#mcs-url').val("http://imgur.com/dMIEG.png");
	$('#mmod-url').val("http://imgur.com/Tw5FF.png");
	$('#mo-url').val("http://imgur.com/6LD9l.png");
	$('#ma-url').val("http://imgur.com/Q6fXa.png");
...
function Sharkale_Opciones(){
	...
	nuevaHTML += '	Mis Borradores <img id="previe-mb" src="'+MenuBorradores+'" width="25" height="25" align="absmiddle"/>';
	nuevaHTML += '	<input type="text" id="mb-url" src="'+MenuBorradores+'" size="50" value="'+MenuBorradores+'"/>';
	nuevaHTML += '	<input type="button" id="mb-url" onclick="$(\'#previe-mb\').attr(\'src\',$(\'#mb-url\').attr(\'value\'))" value="Previsualizar" width="50" class="ui-state-default ui-corner-all"/><br/>';
	...
	nuevaHTML += '    <a href="http://www.guerrastribales.es/585151-es4.html" target="_blank"><img src="http://i46.tinypic.com/2gt42h2.gif" alt="Unite al Mundo 4 de este espectacular juego online"/></a>';
	nuevaHTML += '    <hr><a href="http://sp4mm3r.blogspot.com" target="_blank"><img src="http://i47.tinypic.com/28u5ks3.jpg" /></a>';
...
var newCSS  = '.menuTabs{width:400px;background:none;}#menu{background:url("http://i.t.net.ar/images/bgLogged.gif") repeat-x scroll 0 0 #CCCCCC}';
	...
	menu += '<ul class="tMenu">';
//----Actualizar----//
	menu += '	<li id="MenuAct" style="display:none;"><a href="" id="Sharkale_Actualizar" title="Hay una nueva versión disponible del Script" style="text-decoration:blink;"><img src="'+MenuActualizacion+'" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle" /> Nueva Versión</a></li>';
//----Inicio----//
...
//----Mi Perfil----//
    ...
    menu += '		<li><a href="/perfil/'+user+'/comentarios">Mis últimos comentarios</a></li>';
    menu += '	</ul>';
	menu += '	</li>';
//----Logout----//
...
//----Moderación----//
	menu += '	<li><a href="/mod-history/" title="Historial de Moderación"><img src="'+MenuModeracion+'" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle" alt="X" border="0"/></a></li>';
//----Opciones----//
	menu += '	<li style="-moz-border-radius-topright: 7px;"><a href="" id="Sharkale_Opciones" title="Opciones Avanzadas del Script"><img src="'+MenuOpciones+'" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle" alt="OPC" /></a></li>';
	...
	submenu += '	<li><a title="Comunidad Oficial del Script®" href="http://www.taringa.net/comunidades/bbcoder/">Comunidad Oficial</a></li>';
...
case 'post':
	...
	if($('.miComentario').size() != 0){
		...
		$('.comment-info div.floatR ul').append('<li><a href="#maincontainer" title="Ir a la Cielo"><img src="http://imgur.com/Htc64.png"/></a></li><li><a href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/YKepx.png"/></a></li>');
	...
	$('.titulorespuestas').append(' <a href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"> <img src="http://imgur.com/7TKYk.png" class="carg_act_comm" title="Actualizar Comentarios" height="15" align="absmiddle" style="margin-left:5px;"/></a> <input type="button" class="button" value="Mostrar/Ocultar Comentarios" title="Mostrar/Ocultar Comentarios" onclick="$(\'#comentarios\').toggle(\'show\')">');
	if(HideComment) $('#comentarios').hide();
	///////////////------------------Agregar Boton Ir Abajo--------------------////////////////////
	if($('.post-title').size() > 0){
		$('a[href="#cielo"]').attr('id','pie-a').removeClass('irCielo').attr('href','#maincontainer').html('<img src="http://imgur.com/qvzBl.png" title="Ir al Cielo"/>').before('<a href="#medio" title="Ir al Medio"><img src="http://imgur.com/hOEqm.png"/></a> ');
		var Tierra = '<span style="vertical-align: middle;float: right;padding-top: 1px;padding-right: 30px">';
		Tierra += (!LinkCheck? '<a href="" id="linkchecker" title="Verificar disponibilidad de los links"><img src="http://imgur.com/HjLF0.png" /></a> ':'')+'<a href="#medio" title="Ir al Medio"><img src="http://imgur.com/8QsiM.png"/></a> <a href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/QSl0s.png"/></a></span>';
		$('.post-title:eq(0)').html(Tierra + '<span style="vertical-align: middle;padding-top: 1px;">' + $('.post-title:eq(0)').html()+ '</span>');
		$('.dar-puntos:eq(0)').before('<span style="float:right"><a id="medio" href="#maincontainer" title="Ir a la Cielo"><img src="http://imgur.com/wSTlB.png"/></a> <a href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/iAumU.png"/></a></span>');
	}
	//--Numeración de comentarios--//
	var n_com_ini = 100 * parseInt($('a[class=here]:first').html()) - 100 || 0;
	var n_com = $('.comment-info .nick');for(var i=0;i < n_com.size();i++){n_com.eq(i).before('#'+(i+n_com_ini+1)+' - ');}
...
case 'edicion':
	...
	var html = '<input id="Btn_Agregar_BBCguardados" class="button" type="button" title="Guardar el BBCode del texto seleccionado para utilizarlo en futuras ocaciones" value="Agregar BBC del texto seleccionado"/>';
		...
		html +='<iframe id="tinypic" src="http://plugin.tinypic.com/plugin/index.php?popts=l,narrow|t,images|c,forum|i,es|s,false" style="" scrolling="no" width="235" frameborder="0" height="360"></iframe>';
...
case 'principal':
	...
	$('#derecha').html('');
	$('div[class="box_txt ultimos_posts"]:first').append('<a href="" onclick="act_post_nfu();return false;" title="Actualizar Post" > <img src="http://imgur.com/Bwr5A.png" align="top" height="16" width="16" /></a>');
	var ant = $('#izquierda div[class=size13] a:contains(« Anterior)');
	var sig = $('#izquierda div[class=size13] a:contains(Siguiente »)');
	if(ant.size() != 0){
		ant.attr('url', ant.attr('href'));
	...
	if(NovatosPrinc){
		...
		$.ajax({
			...
			success: function(h){
				...
				$("div[class='box_txt ultimos_posts']:last:contains(posts)").html('Post de Novatos <a href="" onclick="act_post_nov();return false;" title="Actualizar Post" > <img src="http://imgur.com/Bwr5A.png" align="top" height="16" width="16" /></a>');
				var ant = $('#derecha div[class=size13] a:contains(« Anterior)');
				var sig = $('#derecha div[class=size13] a:contains(Siguiente »)');
				if(ant.size() != 0){
					ant.css('font-weight','bold');
	...
	break;
case 'cuenta':
	var html = '<form action="/miavatar_cambiar.php" method="post" name="per">';
		html += '<h3>Avatar por URL</h3>Ej: <b>http://i.t.net.ar/images/avatar.gif</b><br><br>';
		html += '<input type="text" value="'+unsafeWindow.avatar.current+'" name="avatar" maxlength="255" size="44" gtbfieldid="15"><br><br>';
		html += '<input type="hidden" value="'+unsafeWindow.global_data.user_key+'" name="key">';
		html += '<center><input type="submit" title="Guardar Avatar URL" value="Guardar Avatar URL" class="mBtn btnOk"></center><br>';
		html += '</form>';
	$('.edit').after(html);
	break;
case 'perfil':
	...
	if(perfilde == user){
		...
		for (var i = 0; i < mislinks.size(); i++) {
			...
			var HTML = '<a href="" onclick="global_data.postid=\''+numpost+'\';borrar_post();return false;" title="Eliminar Post"><img src="http://imgur.com/FwXWI.png" align="absmiddle"/></a> ';
			HTML += '<a href="http://'+host+'/edicion.form.php?id='+numpost+'" title="Editar Post"><img src="http://imgur.com/XpOvt.png" align="absmiddle"/></a> ';
			HTML += '<a href="" onclick="administrar_post(\''+key+'\',\''+numpost+'\');return false;" title="Administrar Post"><img src="http://imgur.com/Q3CW4.png" align="absmiddle"/></a> ';
...
case 'comunidades':
	...
	$('.denunciar').css('background','url("http://imgur.com/x2hPs.png") no-repeat scroll left top transparent');
	...
	if(rango == "Administrador" || rango == 'Moderador'){
		...
		if(id != null) $('.postBy ul').append('<li><a href="javascript:com.admin_users(\''+id+'\')"><img src="http://imgur.com/CqYcc.png" /></a></li>');
		for(var i=0;i<users_com.size();i++){
			nick_user = users_com.eq(i).find('li:eq(0) a').attr('href').split('/')[3];
			id_user   = /[0-9]+$/.exec(users_com.eq(i).find('li:eq(1)').attr('class'));
	...
	if($('.temaCont .floatR:first').size() > 0){
		...
		$('#cuerpocontainer').append('<div align="center" style="margin:10px;font-weight:bold;"><a id="pie-a" href="#maincontainer"><img src="http://imgur.com/wSTlB.png" title="Ir al Cielo"/> Ir al Cielo</a></div>');
		var Tierra = (!LinkCheck? '<a class="btnActions" href="" id="linkchecker" title="Verificar disponibilidad de los links"><img src="http://imgur.com/HjLF0.png" align="absmiddle"/></a> ':'')+'<a class="btnActions" href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/iAumU.png" align="absmiddle"/></a>';
	...
	var n_com_ini = 20 * parseInt($('a[class=here]:first').html()) - 20 || 0;
	var n_com = $('.comment-info .nick');for(var i=0;i < n_com.size();i++){n_com.eq(i).before('#'+(i+n_com_ini+1)+' - ');}
	$('.comment-info div.floatR ul').append('<li><a href="#maincontainer" title="Ir a la Cielo"><img src="http://imgur.com/wSTlB.png"/></a></li><li><a href="#pie-a" title="Ir a la Tierra"><img src="http://imgur.com/iAumU.png"/></a></li>');
	$('.paginadorCom').css('width','755px').find('ul:first').css('width','500px').parent().css('width','520px');
	$('.paginadorCom ul:last').css('width','500px').parent().css('width','520px');
	$('.titulorespuestas').append(' <a href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"> <img src="http://imgur.com/Bwr5A.png" class="carg_act_comm" title="Actualizar Comentarios" height="15" align="absmiddle" style="margin-left:5px;"/></a>');
...
case 'buscador':
	...
	if($(':input[name="autor"]').val().toLowerCase() == user.toLowerCase()){
		...
		for (var i = 0; i < mislinks.size(); i++) {
			...
			var numpost = esteLink.attr('href').split('/')[3];
			var categ = esteLink.attr('href').split('/')[2];
			var HTML = '<a href="" onclick="global_data.postid=\''+numpost+'\';borrar_post();return false;" title="Eliminar Post"><img src="http://i.imgur.com/FwXWI.png" align="absmiddle"/></a> ';
			HTML += '<a href="http://'+host+'/edicion.form.php?id='+numpost+'" title="Editar Post"><img src="http://i.imgur.com/XpOvt.png" align="absmiddle"/></a> ';
			HTML += '<a href="" onclick="administrar_post(\''+key+'\',\''+numpost+'\');return false;" title="Administrar Post"><img src="http://i.imgur.com/Q3CW4.png" align="absmiddle"/></a> ';
			esteLink.before(HTML);
		}
	}
	break;
case 'buscador-ext':
	...
	for (var i = 0; i < busc_links.size(); i++) {
		...
		if(RegExp('autor='+user).exec(location.href.toLowerCase())){
			if(tip == 'posts'){
				var HTML = '<a href="http://www.taringa.net/edicion.form.php?id='+num+'" title="Editar Post"><img src="http://i.imgur.com/XpOvt.png" align="absmiddle" border="0"/></a> ';
				esteLink.before(HTML);
			}else{
				var HTML = '<a href="http://www.taringa.net/comunidades/'+com+'/editar-tema/'+num+'/" title="Editar Tema"><img src="http://i.imgur.com/XpOvt.png" align="absmiddle" border="0"/></a> ';
				esteLink.before(HTML);
			}
		}
	}
...
function Cambiar_Theme() {
  ...
  var imagenColores = $('<img src="http://imgur.com/C2Dda.png" alt="" width="200" height="200" style="display:block"/>');
...
function addStyle(theme) {
		var css = "a{color:"+theme.fondofuera+";}\
				...
				#maincontainer{-moz-border-radius:10px;padding:12px;background:url(http://imgur.com/UUaVo.png) no-repeat scroll 0 0 transparent !important;}\
		...
		GM_xmlhttpRequest({
			...
			onload: function(r) {
				...
				if (parseFloat(ultvers.ver) <= parseFloat(SCRIPT.version)){
					if(SCRIPT.forzado){
						unsafeWindow.mydialog.procesando_fin();
						unsafeWindow.mydialog.alert("Actualizador del BBCoder®","No hay actualizaciones disponibles en este momento.");
					}
					return;
...
GM_addStyle("#link_vivo {background:transparent url(http://imgur.com/xno2N.png) no-repeat scroll 100% 50%;padding-right:17px;}"+
			"#link_muerto {background:transparent url(http://imgur.com/tFxwX.png) no-repeat scroll 100% 50%;padding-right:17px;}"+
			"#link_inaxesible {background:transparent url(http://imgur.com/TewmV.png) no-repeat scroll 100% 50%;padding-right:17px;}"+
			"#link_redirector {background:transparent url(http://imgur.com/JKUlB.png) no-repeat scroll 100% 50%;padding-right:65px;}");
