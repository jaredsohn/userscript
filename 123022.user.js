...
// @version 0.5.4.9 b5
// @description http://userscripts.org/topics/95307
...
function initjQuery() {
    ...
    function onLoad() {
        ...
        httpAjaxRequest({url:'remote/'+MW.getIntURL('inventory'), success:MW.parseInventoryData});
...
function Battlefieldv2() {
    ...
    var CSSessionStats = function() {
         ...
         * @param {Object} value
         */
        this.add = function(stat_name, value) {
            var opp = PlayerList.current;
            var statVal;
            
            if (!Util.isSet(value)) { value = 1; }
            
            if (Util.isObject(Session[gVar.CurrentCity])) {
                statVal = Session[gVar.CurrentCity][stat_name];
                if (Util.isObject(statVal)) {
                    ...
                    Total[stat_name] += value;
                    Session[gVar.CurrentCity][stat_name] += value;
                }
            }
        };
        /**
    ...
    var CSItemLoot = function(item_html, count) {
        var me = this, loot = gVar.InventoryData, $itm = c$('div').html(item_html);
        this.name     = 'Unknow';
        this.type     = 'Unknow'; 
    ...
    var CSLootList = function() {
        ...
        function estimateStatsIncrease(loot) {
            if (!gVar.InventoryData) {
                return loot;
            }
            var best = gVar.InventoryData[loot.item_id];
            var worst = (best && gVar.WorstItems) ? gVar.WorstItems[best.type] : false;
            if (best && worst) {
                if (best.attack && (best.attack = parseInt(best.attack)) > (worst.att = parseInt(worst.att))) {
                    AttackerStats.atkGained += (loot.increaseAttack = (best.attack - worst.att))*loot.count;
                }
                if (best.defense && (best.defense = parseInt(best.defense)) > (worst.def = parseInt(worst.def))) {
        ...
        this.add = function(loot) {
            ...
            return (cache[loot.item_id] = estimateStatsIncrease(loot));
        };
        /**
    ...
    var Events = {
        ...
        stop_click: function() {
            $(this).remove();
            gVar.Aborted = true;
            PlayerList.current.result_text = 'User stopped.';
    ...
    function isValidOpponent(opp) {
        ...
        if (options.levelRangeActive === true) {
            bInRanges = isInLevelRange() || (options.mafiaRangeActive && isInMafiaRange());
        }
        else if (options.mafiaRangeActive === true) {
            bInRanges = isInMafiaRange();
        }
        if (!bInRanges) {
    ...
    function addNewOpponents(htmlText) {
        ...
        function loadFightTable() {
            ...
            // fighters table
            $html.find('.fight_table tr:has(td[class^=fight_list_player])').each(function(index, element) {
                var $el, opp;
                if (e$('a:regex(href,action=rivals_add)', element)) {
                    return;
                }
                try {
                    ...
                    opp.isRival   = (e$('a:regex(href,action=rivals_remove)', element) !== null);
    ...
    function addLog(message, icon, pic, id) {
        ...
        updateItem.prependTo('#items_logs').empty();
        c$('div', 'class:update_timestamp').appendTo(updateItem).html((new Date()).toUTCString());
    ...
    function addEventLog(opp, reason) {
        ...
        $li.html(Util.render('&#187;${action} ${oppname}:${result}', {
            ...
            oppname : opp.anchor(),
            result  : ((opp.name+text).length > 50 ? '<br>' : ' ' ) + text
        }));
    ...
    function clearAllTimers() {
        ...
        $('#timer_timetotravel, #timer_timetopause, #timer_loadrivals').hide();
    }
    ...
    function addFightResumeCountdown() {
        ResumeCountdown.clear();
        ResumeCountdown.start(options.timerFightResume*60);
    }
    ...
    function preAttack() {
        ...
        if (opp.powerAttack && (!options.attackUseMax||opp.fights > 1)) {
        ...
        if ( sStopMessage ) {
            gVar.Aborted = true;
            clearAllTimers();
            checkBankMoney(updateCurrentOppStats);
                ...
                switch( fr.success ) {
                    ...
                    case ERROR_NO_FIGHT_RESULT:
                        if (bAuto) {
                            ...
                            PlayerList.current.attack_url = null;
                            addManualControls(true);
                        }
                        break;
    ...
    function updateCurrentOppStats() {
        ...
        if (opp.cashWon > 0) {
            SessionStats.add('cashWon', opp.cashWon);
        } 
        if (opp.cashLost > 0) {
            SessionStats.add('cashLost', opp.cashLost);
        }
        if (opp.won > 0) {
    ...
    function Initialize(data) {
        ...
        // Initialize objects
        gVar.InventoryData = (data && data.Items) ? data.Items.data : false;
        ...
        showDiv('items', '_logs');
        toStartScreen();
        $('#ctrlcontainer, #timer_timetotravel, #timer_timetopause, #timer_loadrivals').hide();
    ...
    options.load(function() {
        if (!global.inventoryData) {
            MW.getInventoryData(Initialize);
        }
        else {
            Initialize(global.inventoryData);
        }
    });
}
// ==Script==
...
function HomeFeedCenter()
{
    ...
    function collectDailyTake(htmlText, callback) {
        ...
        var priority = String(options.dtLootPriority).split(/\n/);
        ...
        if (priority.length > 0) {
            for (var i = 0; i < priority.length; i++) {
                if ( (temp = rewards.getByName(priority[i])) ) {
...
function InventoryAnalizer() {
    ...
    function Initialize(Items) {
        ...
        Util.each(Groups, function(index, groupTag) {
            ...
            Util.each(Inventory[groupTag]['top_combined'], function(index, item) {
                if (nMaxCombined > 0) {
            ...
            Util.each(['offense', 'defense', 'combined'], function(index, tagName) {
                ...
                Util.each(Inventory[groupTag][sTag], function(index, Item) {
            ...
            Util.each($.merge(aMergedWeak, Inventory[groupTag]['weak_combined']), function(index, Item) {
            ...
            Util.each(['offense', 'defense', 'combined'], function(index, tagName) {
                ...
                Util.each(Inventory[groupTag][tags.top], function(index, Item) {
