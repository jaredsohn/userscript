...
var SaveTextAreas = new Array();
unsafeWindow["SaveTextAreas"]=SaveTextAreas;
unsafeWindow["RestoreTextArea"] = function(theIndex) {
	theTextArea=SaveTextAreas[theIndex];
	theTextArea.restore();
}
...
function SaveTextArea(txta) {
	...
	this.committed   = '';
	this.index = SaveTextAreas.length;
	SaveTextAreas[SaveTextAreas.length]=this;
...
SaveTextArea.prototype = {
	...
	restoreButton: function () {
		...
		if(this.is_significant( GM_getValue(this.key()) )) {
			var restoreButtonObj = document.createElement('input');
			var text_to_set='RestoreTextArea('+this.index+')';
			restoreButtonObj.setAttribute('onclick',text_to_set);
			restoreButtonObj.setAttribute('type','button');
			restoreButtonObj.setAttribute('value','Restore Saved Text');
			restoreButtonObj.setAttribute('value','Restore Saved Text');
			restoreButtonObj.style.fontSize='x-small';
			this.ta.parentNode.insertBefore(restoreButtonObj,this.ta.nextSibling);
			this.ta.parentNode.insertBefore(document.createElement('br'),this.ta.nextSibling);
		}
	},
