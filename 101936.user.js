...
function group_incs() {
	...
	for(var x = 0; x < rows.length; x++) {
		...
		var target_village = cells[1].getElementsByTagName("a")[0].innerHTML.match(/\d{1,3}\|\d{1,3}/); // Zieldorf
	...
	for(var x = 0; x < rows.length; x++) {
		...
		switch(selected) {
			case "attacker":
				...
				break;
			case "origin_village":
				var this_value = (caption.match(/(\d{1,3}\|\d{1,3})/)) ? RegExp.$1 : false;	// Herkunfsdorf
				break;
			case "slowest_unit":
				var slowest_unit = caption.match(/\b(SP.H|SP.HER|SPY|LKAV|LEICHTE|SKAV|SCHWERE|AXT|AXTIES|SCHW|SCHWERT|RAM|RAMME|AG|AGS)\b/i);
				...
				break;
			case "target_village":
				var this_value = cells[1].getElementsByTagName("a")[0].innerHTML.match(/\d{1,3}\|\d{1,3}/).toString(); // Zieldorf
		}
...
(function main() {
	try {
		var mode = document.getElementById("overview");
		if(mode.value != "incomings") {
			return;		// falsche Uebersicht
		}
	} catch(e) {
		return; 		// kein PA vorhanden
	}
	
	// Stell im Text ermitteln, an der die Auswahlliste und der Button gesetzt werden sollen
	var head_cell = _evaluate('//th[. = "Befehl"]')[0];
