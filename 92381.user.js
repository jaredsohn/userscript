// ==UserScript==
// @name          Forum Pornifier - Less Annoying Edition
// @version 2.0.3.17
...
// @changes       12/6/10 - disabled sites that have long been dead, added http://www.rhyzzy.com/
// ==/UserScript==
var SCRIPT = {
	url: 'http://userscripts.org/scripts/source/92381.user.js',
	version: '2.0.3.17',
};

DEBUG = false; //makes it log shit you dont care about.
/* ------------ settings -----------------*/
//these are for forums:
...
var getSupportedLinks = function(){
	...
	for(var s in sites){
		...
		if(t.href){
      count++;
			str += "<a  style='background-color:black;color:white; text-decoration:none;' href='"+t.href+"'>["+t.text+"]</a>&nbsp;";
		}
	}
	return str;
}
...
var getPreviewPane = function(){
	...
	var conf = function(){
	  ...
	  $("<a>", {
						...
						css: {display:"block", "color":"black"},
						click: function(){
							unsafeWindow.location.href= "http://userscripts.org/scripts/show/92381";
						}
			}),
	  $("<div>"),];//
		$.each(opts, function(){
...
var pane = $("<div id='previewPane' style='font-size:13px; position:absolute;top:"+top+"px;left:0;width:"+ww+"px;background-color:black; color:white;'> <a onclick='$(\"#previewPane\").remove()'>[close]</a> "+getSupportedLinks()+" "+sort+" " +pNav+" </div> ");	$body.append(pane);
...
function slideshow(imgList, container, threadId, onChange, dls) {
    ...
    var initialize = function() {
        ...
        $info = $("<div>").css({"background":"#DDD", "color":"#D00", "position":"absolute", "top":"5px", "left":"5px", "zIndex":2});
...
function updateScript(notify) {
  var notifyFn = function(){
  	$("#updateScript").css({"color":"red", "backgroundColor":"black", "cursor":"pointer"}).text(" >> Update Available << ");
  };
  try {
    setTimeout(function(){
    GM_xmlhttpRequest({
...
//        console.log("opened viewer:"+id+" data:"+data);
    ...
    var data = [];
    var dbViews = debounce(openViewer, 3000);
		var clck = function(){
...
function startAnonibApp(){
  ...
  var clck = function(it){
  	...
  	var th = thread.find("a[target=_blank]");
		...
		th.each(function(){
			var href = $(this).attr("href");
			if(href.indexOf("http://www.rhyzzy.com/") == 0){
		//		alert(href);
