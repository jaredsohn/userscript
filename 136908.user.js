// ==UserScript==
// @name           Facebook için Tümünü Seç Butonu (InviteThemAll)
// @namespace      Facebook Tümünü Seç Butonu (InviteThemAll)
// @description    Facebook için; Tüm arkadaşlarını tek tıklama ile seçme butonu. (DoctoR.Ossi)
// @include        /https?://(|.*\.)facebook.com/?.*/
// @include        http://*.facebook.com/*
// @include        https://*.facebook.com/*
// @version        1.0
// @require        http://sizzlemctwizzle.com/updater.php?id=136908
// ==/UserScript==
...
(function () {
	...
	function errorAlert(message, positionArray, errorAlertCallback) {
		...
		try {
			...
			header.appendChild(document.createTextNode('Hata!'));
	...
	function addButton(inviteBoxParameter) {
		...
		if (inviteBox !== undefined && !executing && !document.getElementById('selectAllButton')) {
			...
			// Button texts.
			var selectButtonText1 = 'Seç', selectButtonText2 = 'İptal';
			...
			// Create it's label.
			var autoScrollCheckBoxLabel = createLabel('Otomatik Kaydırma?', autoScrollCheckBox);
			...
			var fromFieldDefaultValues = ['**', '**'];
			fromField.defaultValue = fromFieldDefaultValues[0];
			// Create it's label
			var fromFieldLabelValues = ['Sadece şu aralıktakiler:', 'Sadece şu aralıktakiler #:'],
			...
			var toFieldDefaultValues = ['**', '**'];
			toField.defaultValue = toFieldDefaultValues[0];
			// Create it's label
			var toFieldLabelValues = ['ile :', 'ile #:'],
			...
			function fieldBlurEventListener() {
				...
				default:
					...
					if (isNaN(thisField.value)) {
						fieldErrorAlert.call(thisField, [document.createTextNode('Sadece sayısal değer giriniz!'),
							document.createTextNode('Geçerli bir değer giriniz.   /DoctoR.Ossi')]);
						break;
					...
					// Else, there is no friend #0, so report an error.
						...
						fieldErrorAlert.call(thisField, [document.createTextNode('Böyle bir arkadaşın yok #0.'),
							document.createTextNode('Geçerli bir değer giriniz.   /DoctoR.Ossi')]);
						break;
					} else if (parseInt(thisField.value, 10) < 0) {
						fieldErrorAlert.call(thisField, [document.createTextNode('Negatif değer eklenemez!'),
							document.createTextNode('Geçerli bir değer giriniz.   /DoctoR.Ossi')]);
						break;
					}
