...
// @resource       alignCenter https://raw.github.com/legnaleurc/nopicads/v4.27.0/css/align_center.css
// @resource       scaleImage https://raw.github.com/legnaleurc/nopicads/v4.27.0/css/scale_image.css
// @resource       bgImage https://raw.github.com/legnaleurc/nopicads/v4.27.0/img/imagedoc-darknoise.png
// @include        http://*
// @include        https://*
// ==/UserScript==
console.debug('NPA -- initialize (on ' + window.location.toString() + ')');
var _a = Date.now();
function _b(msg) {
    var tmp = Date.now();
    console.debug('NPA: took ' + (tmp - _a) + ' ms -- ' + msg + ' (on ' + window.location.toString() + ')');
    _a = tmp;
}
  ...
  function bootstrap (context) {
    ...
    $._main = function (isNodeJS) {
      ...
      if (!handler) {
        ...
        _b('before load config');
      config = load();
        _b('after load config');
      _.info('working on\n%s \nwith\n%o', window.location.toString(), config);
      disableWindowOpen();
        _b('before pre-handler');
      handler.start();
        _b('after pre-handler, waiting for DOM loaded');
      document.addEventListener('DOMContentLoaded', function () {
          _b('DOM loaded, before post-handler');
        disableLeavePrompt();
        handler.ready();
          _b('after post-handler');
      });
    };
...
_b('all module declared, start main function');
$._main();
_b('done main function');
