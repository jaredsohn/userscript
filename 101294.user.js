...
// @name           Farming Machine Customized - <Craja>

// @author         Craja (Update from Black_Blood)

// @namespace	   FM_Craja

// @description    Updated version of Farming Machine for single village, to arrest captcha
...
var SCRIPT = {
	url : 'http://userscripts.org/scripts/source/101294.user.js',
	version : '1.6.1.7', //same value as @version
	name : 'Farming Machine Customized - <Craja>'
};
...
var minWait = 3700; 			//Don't make it smaller then 5000ms!

var maxWait = 2 * minWait;		//1 * minWait
...
function main() {
	...
	if (html.indexOf(" <!-- ERROR ITEM CONTAINER") != -1) {
		...
		FM_log(1,"Unable to load site"); //craja
		return; //craja
	...
	if (GM_getValue("Active_" + suffixGlobal, -1) > -1) {
		...
		if (isFarmerVillage(currentVillageId)) {
			...
			if (fque == "" || fque.length == 0 || doneHere == 1) {
				...
				messageStr = T('Halt_Farming');

				setTextMessage(titleStr, messageStr);

				//acVillageFlag = false; craja
				return;
				//stopEngine(); //this is stoping without trace, less informative on farming complete
...
function loginCheck()
{
	if (GM_getValue('wearein') == 1) {
		//location.href = rootPath+"build.php?id=39"; craja
...
function rp_createFarmRow(index, sIndex, eIndex) {
	...
	if (!(troops == null || troops == "")) {
		...
		tableCol.innerHTML = "<div style='text-align:left;'>"
			/*	+ "[" + index + "] "
		...
		} else {tableCol.innerHTML = "<div style='text-align:left;'>"
			/*	+ "[" + index + "] " */
...
if (document.body) {
	...
	if (url1.indexOf("a2b.php") > -1){
 	...
 	main();
	GM_registerMenuCommand("Check update - FM Customized <Craja>", updateFMCC);
}