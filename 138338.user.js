...
// @name			ffaab Chat Loader
// @namespace		http://jvflux.com/jvchat/
// @description		Discussions en temps réel sur les forums de Kyrianis
// @version			1.2
// @include			http://www.kyrianis.com/forum-1-1.html
// @include			http://www.kyrianis.com/forum-6-1.html
// @include			http://www.kyrianis.com/forum-4-1.html
// @include			http://www.kyrianis.com/forum-3-1.html
// @include			http://www.kyrianis.com/forum-7-1.html
// @include			http://www.kyrianis.com/jvchat
// ==/UserScript==
...
pseudo.add("Axel", "#000000");
pseudo.add("AppIication", "red");
pseudo.add("Guillaume", "#7F12DF");
pseudo.add("Axel", "#000000");
...
if (new RegExp("\\.kyrianis\\.com/forums/([0-3]+)\\-([0-3]+)\\-([0-3]*)\\-([0-3]*)\\-([^-]*)\\-([0-3]*)\\-([0-3]*)\\-(.*)\\.htm#?(.*)").test(location.href)) {
	switch (RegExp.$1) {
	case "0" :
		try {
...
if (location.pathname == '/jvchat') {
	...
	elem.src = '//'+(document.getElementById('_jvchat_localhost') ? '127.0.0.1' : '192.168.1.23')+'/jvchat.js';
	...
	if (url_mode === 0 && url_index === 1) { // première page uniquement
		...
		for (i in elems) {
			elems[i].innerHTML = '<a  class="jvchat" href="/jvchat#'+url+'">Chat by ffaab and Guillaume</a>';
		}
	}
	...
	if (url_mode === 1 && document.getElementsByClassName('bt_repondre').length > 0) { // pas sur topics bloqués
		...
		for (i in elems) {
			...
			if (typeof elems[i] == 'object') {
				//elems[i].innerHTML += '<a  class="jvchat" href="/jvchat#'+url+'">ffaab Chat</a>';
				elem = document.createElement('a');
				
				elem.setAttribute('class', 'jvchat');
				elem.setAttribute('href', '/jvchat#'+url);
				elem.appendChild(document.createTextNode('ffaab chat'));
	...
	if (url_mode === 3 
		&& elems.length
			&& elems[0].getElementsByTagName('a').length
				&& elems[0].getElementsByTagName('a')[0].href === 'http://www.kyrianis.com/listeSmileys.php') {
		elems[0].innerHTML += ' | <a  class="jvchat" href="/jvchat#'+url+'">ffaab Chat</a>';
	}
	...
	if (0) {
		...
		if (elem !== null) {
			elem.innerHTML += '<li><a  class="jvchat" href="/jvchat#">ffaab Chat</a><li>';
		}
	}
