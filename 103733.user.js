// ==UserScript==
// @name           TW Massrecruiter
// @description    Advanced Massrecruiter
// @author         Michael Richter, Vertaler nl: burnacid, knabbelaar
...
// NL:   Aanpassingen en verspreiding van dit script verboden zonder toestemming
//       van de auteur.
// -----------------------------------------------------------------------------
...
(function(){
	...
	var lang = {
		...
		'ae': {
			'settings': 'تدريب مجموعة :',
			'troupsize': 'عدد الجنود المطلوب :',
			'troupsizedesc': 'نوع الوحدة المفضل (لكل المجموعات)',
			'openend': 'تدريب المزيد :',
			'openenddesc': 'عند اكتمال العدد يتم تدريب النوع إلى حد اكتمال المزارع',
			'farmfree': 'ترك مساحة من المزارع',
			'farmfreedesc': 'ترك 100 من مساحة المزارع (مثال 100 لبناء النبيل)',
			'resfree': 'ترك عدد من الموارد :',
			'resfreedesc': 'ترك عدد من الموارد في كل قرية و عدم استعمال كل الموارد في التدريب',
			'save': 'حفظ',
			'fill': 'عدد القوات ',
			'recruit': 'تدريب',
			'running': 'جاري ... ',
			'troupneeds': '(عدد الجنود المطلوب للعمال 1 %)',
			'filled': 'وضع: %1 في  %2 القرى',
			'settoobig': 'سكريبت التدريب على مستوى المملكة كبير جدا على الكوكيز ، لحل هذه المشكلة عليك بحذف البعض من مجموعات القرى',
			'wood': 'خشب',
			'clay': 'طمي',
			'iron': 'حديد'
		}
	...
	if(/en\d+\.tribalwars\.net/.test(location.href)) {
		...
		curlang = 'de';
	} else if(/ae\d+\.tribalwars\.ae/.test(location.href)) {
		curlang = 'ae';
	}
	...
	th.title = lang[curlang]['farmfreedesc'];
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	...
	for(var i = 0; i < 3; i++) {
		...
		if(i == 0) {
			...
			img.title = img.alt = lang[curlang]['wood'];
		} else if(i == 1) {
			img.src = 'graphic/lehm.png';
			img.title = img.alt = lang[curlang]['clay'];
		} else if(i == 2) {
	...
	btn.addEventListener('click', function(evt){
		...
		document.getElementById('massrecruitbuilt').textContent = lang[curlang]['filled'].replace(/%1/, builtsum.join(', ')).replace(/%2/, builtvilnum);
