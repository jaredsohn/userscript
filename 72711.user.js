...
// @description    A couple of Neogame functions for Ogame Redesign - v1.21.6
// @version	1.21.6
...
/*
15.04.2010 v1.21.6
	- Bug with local time on Messages tab
	
15.04.2010 v1.21.5
	- A few more fixes for ogame 1.2.1

12.04.2010 v1.21.4
	+ Option: Move moons to the end of the shortlinks list
	- Bug with cluetips disabling
	
05.04.2010 v1.21.3
	+ Moons highlighting in the shortlinks box (Fleet dispatch 2)
	- Alliance list collapsed after sorting
	
03.04.2010 v1.21.2
	-  Continue fixing issues with time in 1.2.1
	
02.04.2010 v1.21.1
	- Extended resources style fixes for 1.2.1

02.04.2010 v1.21.0
	+ Option: minimize planet image
	+ Option: clock/countdown by default in the reduced Event List
	* Fixed Time processing in 1.2.1
	* Fixed Galaxy view in 1.2.1
	- Fixed 'undefined' in antigame options
	- Bug with 'Show names' on moons (factories)
	
28.03.2010 v1.20.2
	* Reduced event list only when embedded into Overview page
	- EventList 3-state indicator fixed
	- Temporary fix for Timezone offset detection

22.02.2010 v1.20.1
	- 'Station' page on Moons was not detected (different id)

22.02.2010 v1.20.0
	+ Event List: show state indicator for 3-state missions (Hold, Expedition)
	+ Options: Reduced Event List
	+ Shows Expeditin Points together with Total capacity
	+ Buttons for setting production to 0% or 100%
	* Improved current page detection
	- 'Show Server time for Phalanx' didn't work
	- Time was not changed to local in EventList in some cases

01.02.2010 v1.19.1
	- Bug with date parsing
	- Auto-updater now checks for the valid server response
	
07.01.2010 v1.19.0
	+ Show target player name on Event List
	
07.01.2010 v1.18.2
	+ Message navigation buttons are cloned at the top of messages
	* Players highlighting is not case-sensitive now
	- Bug with ships links in Calculator

30.12.2009	v1.18.1
	* Shortlinks adedd to Calculator
	- A few fixes for Calculator
	- Websim button didn't work with a spy report opened in a frame
...
23.12.2009	v1.16.3
	- Bug with AM/PM calculaton

21.12.2009	v1.16.2
	+ Option added: Use 12-hours time format
	* Russian translation updated
	* Fleet capacity (fleet dispatch I) layout is fixed for accounts with Commander

16.12.2009	v1.16.1
	+ Show target player name on Fleet Movement
	- Bug with number formatting

16.12.2009	v1.16.0
	+ Option: thousand separator
	+ ES language for AR and MX servers
	* Local time in Vacation Mode tooltip
	* Logic of 'Local time' option changed

10.12.2009	v1.15.5
	- Bugfix: Websim button didn't work with 'Show full spy reports' options

09.12.2009	v1.15.4
	* Player's techs/drives and coords are being passed to Websim
	* Theoretical debris is not displayed if there's no fleet in the report

08.12.2009	v1.15.3
	* Support for Websim Extension script
	* Change style for ext. resources

06.12.2009	v1.15.2
	- Bug with Reset coords button

05.12.2009	v1.15.1
	- 'Show construction names' was ignored when 'Show names' was switched off

04.12.2009	v1.15.0
	+ Option added: Add buttons for submitting spy reports to WebSim
	* Option added: 'Shows construction titles'
	- Construction titles overlapped with the planet name
	= Strings to translate:
		...
		opt_fleet2_checkProbeCapacity: 'Check probes capacity before departure(page 2)' (since 1.14.0),

29.11.2009	v1.14.0
	+ Option added: Check probe capacity on departure
	+ Shows construction title in the planet list
	- Fleet capacity layout fixed
...
28.11.2009	v1.13.8
	...
	+ User specified colors for names depending on their state (available, not available, not enough resources)
...
\
		...
		{
			var code = ' old_UhrzeitAnzeigen = UhrzeitAnzeigen; UhrzeitAnzeigen = function() { \
					var savedTime = startServerTime; \
					startServerTime -= ' + this.TimeZoneDelta + '; \
					old_UhrzeitAnzeigen(); \
					startServerTime = savedTime; \
				}';
		...
		showPageStartTime: function()
		{
			...
			if (Utils.ogameVersion=='1.1' && Options.showServerOgameClock || Utils.ogameVersion=='1.2.1' && ! (Options.timeSetting == 1 && 

!Options.showServerOgameClock) )
		...
		Init: function()
		{
			...
			if (Options.timeSetting == 2) {
				var code = 'window.old_getFormatedDate = window.getFormatedDate; window.getFormatedDate = function(date,format) { return 

window.old_getFormatedDate(date-'+this.TimeZoneDelta+',format) }';
				Utils.runScript(code);
			}
			
			if ( Utils.ogameVersion=='1.2.1' && (Options.timeSetting == 1 && !Options.showServerOgameClock || Options.timeSetting == 2) ) // 

ogame 1.2.1
				this.changeOgameClocks2Local();
			if (Utils.ogameVersion=='1.1' && Options.timeSetting != 2 && Options.showServerOgameClock) // ogame 1.1
...
"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAALHRFWHRDcmVhdGlvbiBUaW1lAHPhYiAzMSBPdXQgMjAwOSAxODoyNjowOSAtMDAwMBvBwloAAAAHdElNRQfZC

h8SGy7RbQlkAAAACXBIWXMAAB7BAAAewQHDaVRTAAAABGdBTUEAALGPC/xhBQAAAA1JREFUeNpjYGBgmAEAAJ0AmeuxAnUAAAAASUVORK5CYII=",
...
window.ScriptEngine && ScriptEngine() == 'JScript' && i.match(/^\d+$/) ) {
				...
				Array.prototype.toRecoverableString = function () {
					...
					if( !this.length ) {
						var oCountProp = 0;
						for( var i in this ) { if( i != 'toRecoverableString' && i != 'toRecoverableObString' && i != 

'tempLockIgnoreMe' && i != 'prototype' && i != 'constructor' && i != 'index' && i != 'input' && i != 'length' ) { oCountProp++; } }
...
this.toString().replace(/^\s+|\s+$/g,'').replace(/^function\s*\w*\([^\)]*\)\s*\{\s*\[native\s+code\]\s*\}$/i,'function () {[\'native code\'];}');
				};
		...
		killCluetips: function(selector)
		{
			...
			setTimeout( function(){ Utils.$(selector).cluetip('destroy').unbind('mouseover').each(function(){this.title = 

this.title.replace('|','')}) }, 0);
		},
...
'data:image/gif;base64,',
		img_off: 

'data:image/gif;base64,',
		img_hl: 

'data:image/gif;base64,',
	...
	var EventList =
	{
		...
		insertCSSRules: function()
		{
			Utils.insertCSSRule('.eventFleet .playername, .eventFleet .playername2, .partnerInfo .playername, .partnerInfo .playername2, 

.partnerInfo .playername3  { \
				top: 38px; \
				font-size: 10px; \
				color: green; \
				white-space:nowrap; \
			...
			if (this.reduced) {
				...
				Utils.insertCSSRule('.countDown, .arrivalTime, .coordsOrigin, .detailsFleet, .destCoords, .sendMail, .sendProbe { top: 4px 
...
!important}');
				...
				Utils.insertCSSRule('.acsFleet .arrivalTime, .acsFleet .missionFleet, .acsFleet .coordsOrigin, .acsFleet .detailsFleet, 

.acsFleet .destCoords { top: 13px !important}');
				...
				// time mode
				Utils.insertCSSRule('#evt_timeMode { position: absolute; width: 250px; left:40px; top:9px; text-align:left; 

color:#6F9FC8}');
				Utils.insertCSSRule('.tmTime #tmTime, .tmCountdown #tmCountdown { /*text-decoration: none; cursor:default;*/ 
...
Utils.XPath('//*[@class="missionFleet"]/IMG[contains(@src,"icon-'+icon+'")]/ancestor::*[contains(@id,"eventRow")]');
				var holdlist = [];
		...
		process: function()
		{
			try {
				...
				if (this.reduced) {
					...
					var f = function(e){ 
						if ((e.target.id=='tmTime' || e.target.id=='tmCountdown') && container.className!=e.target.id) 

container.className=e.target.id;
					}
					var div = document.createElement('div');
					div.id = 'evt_timeMode';
					div.innerHTML = '<span id="tmTime">'+Options.Interface.lbl_tmTime+'</span> / <span 

id="tmCountdown">'+Options.Interface.lbl_tmCountdown+'</span>';
		...
		showEventList: function ()
		{
			try {
				...
				var div = document.createElement('div');
				div.id = 'newEventBox';
				div.innerHTML = '<iframe id="iframeEventBox" src="index.php?page=eventList&pro=1&session='+ Utils.unsafeWindow.session 
	...
	var FleetMovement = 
	{
		...
		insertCSSRules: function ()
		{
			if (Options.mvmt_expandFleets) {
				...
				Utils.insertCSSRule(".anti_fleetDetails {left:60px; width:290px; white-space:normal; padding:0px 7px; font-size:0.9em; 
...
text-align:left; line-height:1.2em}");
		...
		expandFleet: function (fleet)
		{
			try {
				...
				btn.addEventListener("click", function (){ setTimeout(function (){ FleetMovement.myOpenCloseFleet(fleet.id, 1); }, 0); }, 
		...
		show: function(parent)
		{
			...
			function addSL() {
				...
				for (var i=0; i<slinks.length; i++) {
					var s = slinks[i];
					sl_select += '<option '+((s.galaxy==t.galaxy&&s.system==t.system&&s.planet==t.planet)?'selected':'')+' 

value="'+s.galaxy+'#'+s.system+'#'+s.planet+'">'+s.name+' ('+s.galaxy+':'+s.system+':'+s.planet+')</option>';
				}
			...
			try {
				...
				var coords = '<div id="calc_coords"><input id="calc_g" value="'+target.galaxy+'"> : <input id="calc_s" 

value="'+target.system+'"> : <input id="calc_p" value="'+target.planet+'"></div>';
...
id="calc_res_hdr"><th></th>'+addResLink('metal')+addResLink('crystal')+addResLink('deuterium')+'<th>'+Options.Interface.lbl_total+'</th></tr>' +
					'<tr id="calc_res"><td></td>'+addResInput('metal')+addResInput('crystal')+addResInput('deuterium')+'<td 

id="calc_total">0</td></tr>' +
					

'<tr><th></th><th>'+Options.Interface.lbl_shipSCargoAlt+'</th><th>'+Options.Interface.lbl_shipLCargoAlt+'</th><th>'+Options.Interface.lbl_shipRecyclerAlt+'<

/th><th>'+coords+'</th></tr>' +
					'<tr 

id="calc_ships"><th>'+Options.Interface.lbl_Quantity+'</th>'+addShipLink(202)+addShipLink(203)+addShipLink(209)+addSL()+'</tr>' +
					'<tr 

id="calc_cons"><th>'+Options.Interface.lbl_Consumption+'</th>'+addShipCons(202)+addShipCons(203)+addShipCons(209)+btnClear+'</tr>' +
					'<tr 

id="calc_dur"><th>'+Options.Interface.lbl_Duration+'</th>'+addShipDur(202)+addShipDur(203)+addShipDur(209)+btnSave+'</tr>' +
					'</table>';
				...
				;
				Utils.$('#calculator #calc_ships a').bind('click', function(){ FleetSend.setShips('ship_'+this.getAttribute('ref'), 

Utils.parseInt(this.innerHTML))});
	...
	var FleetSend = 
	{
		...
		Fleet1_Run: function()
		{
			try {
				...
				var res = Utils.getIntById('resources_metal') + Utils.getIntById('resources_crystal') + 

Utils.getIntById('resources_deuterium');
				...
				if (no_fleet) {
					...
					if (!Options.fleet1_showResCalc) {
						document.getElementById('SCargo').addEventListener('click', function (){ FleetSend.setShips('ship_202', 

scargo); return true; }, false );
						document.getElementById('LCargo').addEventListener('click', function (){ FleetSend.setShips('ship_203', 

lcargo); return true; }, false );
					}
		...
		setMission: function ()
		{
			...
			if (Utils.XPath('//*[@id="missions"]/descendant::*[contains(@id,"missionButton") and contains(@class,"selected")]').snapshotLength > 
...
Utils.unsafeWindow.$(selector).cluetip("destroy").cluetip({local:true,cluetipClass:"galaxy",width:250,showTitle:false,closeDelay:250,mouseOutClose:true,hove
...
rIntent:false})
	...
	var SpyReport = 
	{
		...
		calculateDebris: function(report)
		{
			...
			this.debris_metal = metal * 0.5;
			this.debris_crystal = crystal * 0.5;
			...
			for (var i=0; i<rows.snapshotLength; i++) {
				...
				if (Options.msg_PlunderThreshold && Utils.isCurrentPage('messages') ) {
					...
					if (total_pl < Options.msg_PlunderThreshold*1000 && total_df < Options.msg_DebrisThreshold*1000)
						document.getElementById( report.parentNode.parentNode.id.replace('spioDetails_','')+'TR' ).className += ' 
		...
		Resources_Run: function()
		{
			...
			for (var i=0; i<this.res_array.length; i++)
			{
				...
				if (res != 'energy') {
					...
					if (time_to_fill>0) {
						node.innerHTML += '<br/><span 

class="finishtime">'+DateTime.formatDate2(DateTime.getFinishTime(time_to_fill))+'</span>';
					}
