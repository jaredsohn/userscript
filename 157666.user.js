...
//<form method="post" autocomplete="off" id="fastpostform" action="forum.php?mod=post&amp;action=reply&amp;fid='+fid+'&amp;tid='+tid+'&amp;extra=&amp;replysubmit=yes&amp;infloat=yes&amp;handlekey=fastpost" onSubmit="return fastpostvalidate(this)">
...
function Yevent(Fsay,Ffre) {//alert(Fsay.outerHTML);
Fsay.value=rand;
...
//var msay=mage[i];//alert(msay);
var Fsay=$('fastsay'+[i]+'');//,YUA=$('YUA'+[i]+''); //alert(Fsay.outerHTML);
var Ffre=fre[i];
Yevent(Fsay,Ffre);
}}
//截获循环结束了
...
//来个漂亮的提示怎么样
function Stips() {
showError('哈哈，开什么玩笑，你难道不想说点什么吗？:)');//alert(is.value);
}

function Sdata(th) {if(!$('fastsay0')){
if (YUA0.checked){th.value=th.value+TAil;}
$('fastpostform').message.value=th.value;$('fastpostsubmit').click();th.value='';return;//alert('he');
};
var parN=th.parentNode;
if(parN.getElementsByTagName('input')[1].checked){//alert(YUA);
//if(event.keyCode==13){
//say.value=say.value+'[hr][size=1][color=Silver]'+navigator.userAgent+'[/color][/size]'; }
th.value=th.value+TAil;}//alert(i);
showWindow('reply', th.name);//hideWindow('reply')
var tva=th;
postf = window.setInterval(function () {$('fwin_reply').style.display='none';var pomev=$('postform').message;
if (pomev){pomev.value=tva.value;$('postsubmit').click();clearInterval(postf);tva.value='';}}, 100); //if($('fctrl_reply')){clearInterval(timer1)} 
  }
