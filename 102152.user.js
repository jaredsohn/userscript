// ==UserScript==
// @name           YAGrepoFarmhelper
// @namespace      rdg_YAGrepoFarmhelper
// @description    Yet another Grepo Farmhelper helps with the farming by showing several aditional data. Works on Grepolis 1, not tested on 2
// @include        http://*.grepolis.*/game/map*
// @author         robsDobs
// @version        0.15
// @license        Creative Commons: Namensnennung - Share Alike 3.0 Deutschland (http://creativecommons.org/licenses/by-sa/3.0/de/)
// ==/UserScript==

// This is a modified version, of the modified version.
// The original Script can be found under http://userscripts.org/scripts/show/78709
// another previous Versions can be found at these Places
// http://userscripts.org/scripts/show/78709 some tweaks to the original
// http://userscripts.org/scripts/show/71277 a modification
// http://userscripts.org/scripts/show/72488 german translation 
// http://userscripts.org/scripts/show/65877 the original german script 
// http://userscripts.org/scripts/show/70651 the english translation
// some mofidications came from discussions on the original script that can be found at http://userscripts.org/topics/45550

/* CHANGELOG
 * 0.15
 * - added thinks from http://userscripts.org/scripts/show/79520
 * - changed the calculation a little bit
 * - added tweaks from http://userscripts.org/scripts/show/78709
 *
 * 0.14
 * - translated to german
 * - added view of the mood
 * - added some coloring for the data 
 *
 * 0.13
 * - fixed the following issue http://userscripts.org/topics/53424
 * 
 * 0.12
 * - Fixed bug in layout not showing correctly when attaking players
 *
 * 0.11
 * - Changed auto update support to make script shorter
 * - Changed default command action from Looting to Strenghten as it is used more often.
 * - Fixed some layout issues with grepolis 1.09 version.
 * - Fixed bug with "Demand Troops" not doing anything when clicked
 * - Fixed bug when calculating amount of troops to be sent on a "Ask for resources" command, not taking into account other units already selected
 *   
 * 0.10
 * - Added extra information to loot capacity and max loot given the current strenght, thanks to Kedor (http://userscripts.org/users/131203)
 * - When clicked on the troops image it will no longer fill the input with the maximun troop number, but instead if it its a strengthening action it will 
 *  calculate the correct unit quantity to strengthen, if it is a looting action, it will fill the correct unit quantity to loot the current max loot.
 * 
 */

( function() {

	// access to window object cross-browser
	var uW;
	if (typeof unsafeWindow === 'object') {
		uW = unsafeWindow;
		GM_log('unsafeWindow');
	} else {
		uW = window;
		GM_log('window');
	}

	// get jQuery
	var $ = uW.jQuery;

	// cookie-based alternative for GM_*Value functions
	var value, newValueLib = function(prefix) {
		var prefix = 'tilx_' + prefix + '_';

		// cookie-functions by Peter-Paul Koch
		// (http://www.quirksmode.org/js/cookies.html#script)
		var createCookie = function(name, value, days) {
			var expires = "";
			if (days) {
				var date = new Date();
				date.setTime(date.getTime() + (days * 24 * 60 * 60 * 1000));
				expires = "; expires=" + date.toGMTString();
			}
			document.cookie = name + "=" + value + expires + "; path=/";
		};
		var readCookie = function(name) {
			var nameEQ = name + "=";
			var ca = document.cookie.split(';');
			for ( var i = 0; i < ca.length; i++) {
				var c = ca[i];
				while (c.charAt(0) == ' ') {
					c = c.substring(1, c.length);
				}
				if (c.indexOf(nameEQ) == 0) {
					return c.substring(nameEQ.length, c.length);
				}
			}
			return undefined;
		};

		return {
			set : function(name, value) {
				createCookie(prefix + name, value, 365);
			},
			get : function(name, def) {
				var ret = readCookie(prefix + name);
				if (ret !== undefined) {
					return ret;
				} else {
					return def;
				}
			}
		};
	};

	// Object.create() by Douglas Crockford
	if (typeof Object.create !== 'function') {
		Object.create = function(o) {
			var F = function() {
			};
			F.prototype = o;
			return new F();
		};
	}
	
	// ---------------------------------------------------------------------------------------------------------------------------
	// the actual script
	var YAGrepoFarmhelper = ( function() {
		GM_log('starting Script , initialize variables' + SCRIPTNAME);

		var SCRIPTNAME = 'YAGrepoFarmhelper';

		var images = {
			'booty' :'data:image/png;base64,',
			'mood_drop' :'data:image/png;base64,',
			'strength_drop' :'data:image/png;base64,',
			'strength' :'data:image/png;base64,',
			'strength_prev' :'data:image/png;base64,',
			'strength_now' :'data:image/png;base64,',
			'strength_total' :'data:image/png;base64,',
			'mood_now' :'data:image/png;base64,'
		};

		var GameData;
		var TownInfo;

		var bindDurationCounter_old;
		var sendUnits_old;
		var init_old;

		var townId = parseInt(uW.Game.townId, 10);

		var strength_prev = 0;
		var strength_now = 0;
		var mood_now = 0;
		var booty = 'false';
		var autoSendUnits = 'true';
		var diplomacy = 'false';

		var strengthMax = 100;

		var r = function(n, x) {
			var f = Math.pow(10, x);
			return Math.round(n * f) / f;
		};

		var r2 = function(n) {
			return Math.round(n * 100) / 100;
		};

		var localiser = {
			en:{
                tilx_res: 'Maximum loot of the current attack',
                tilx_mood_drop: 'Mood lowering of the current attack (when fully looted)',
                tilx_strength_drop: 'Strength reduction of the current attack (when fully looted)',
                tilx_strength: 'Increasing strength of the current attack',
                tilx_strength_prev: 'Increased strength of previous attacks',
                tilx_strength_now: 'Strength of the farming village at the moment',
                tilx_mood_now: 'Mood of the farming village at the moment',
                tilx_strength_total: 'New strength in total',
				tilx_autoSendUnits_chk: 'auto accept attack',
				tilx_diplomacy_chk: 'diplomacy researched',				
                chkbooty : ' With <em>Booty</em>'			
	    		},
	    		de:{
				tilx_res: 'Maximale Beute des aktuellen Angriffes',
				tilx_mood_drop: 'Stimmungssenkung des aktuellen Angriffs (bei voller Beute)',
				tilx_strength_drop: 'St&auml;rkesenkung des aktuellen Angriffs (bei voller Beute)',
				tilx_strength: 'St&auml;rkesteigerung des aktuellen Angriffs',
				tilx_strength_prev: 'St&auml;rkesteigerung der vorherigen Angriffe',
				tilx_strength_now: 'St&auml;rke des Bauerndorfes im Moment',
				tilx_mood_now: 'Stimmung des Bauerndorfes im Moment',
				tilx_strength_total: 'Neue St&auml;rke insgesamt',
				tilx_autoSendUnits_chk: 'Automatisch Angriff absenden',
				tilx_diplomacy_chk: 'Diplomatie erforscht',				
                chkbooty : '<em>Beute</em> erforscht'				
	    		}
                }		



		var initStyle = function() {
			var style = [];
			style.push('.tilx_infos {display:block; padding:7px 0 0 35px; height:23px; border:solid #f7cd81 0px; overflow:hidden; white-space:nowrap; background-position:3px 50%; background-repeat:no-repeat; }');
			style.push('.tilx_infos.error, .tilx_infos .error {color:#c00}');
			style.push('.tilx_infos.moodlow, .tilx_infos .moodlow {color:#E80000}');
			style.push(".mFInfo { color:#FFCC66; font-size:0.8em; border:0px; margin-top:10px;margin-left:20px;text-shadow:0.1em 0.1em 0.2em black;}");
			style.push(".mFOK { color:#40E040;  }");
			style.push(".mFDanger { color:#E80000;}");
			style.push(".mFSOK { color:#40CC66;  }");
			style.push(".mFSGood { color:#FFDD66;  }");
			
			$('<style type="text/css">' + style.join("\n") + '</style>').appendTo('head');
		}

		var calcRes = function() {
			var res = 0;

			$('.unit_input_ground').each(
					function() {
						var uCount = parseInt($(this).val(), 10);
						if (isNaN(uCount)) {
							uCount = 0;
						}
						res += parseInt(
								GameData.units[$(this).attr('name')].booty, 10)
								* uCount;
					});
			if ($('#tilx_booty').is(':checked')) {
				res *= 1.3;
			}
			res = Math.floor(res);

			return res;
		};

		var handleResFarm = function() {
			var res = calcRes();
			
			getCheckboxes();

			// var maxLoot = Math.round(9*Math.pow( strength_now, Math.sqrt(2)));
			var percent = (strength_prev + strength_now) / 100;
			if (percent > 1) {
				percent = 1;
			}
			var maxLoot = Math.round(6005 * Math.pow(percent, Math.sqrt(2)));
			var text = res;

			if (res > maxLoot) {
				$('#tilx_res').addClass('error');
				text = res + "/" + maxLoot;
				res = maxLoot;
			} else {
				$('#tilx_res').removeClass('error');
			}

			var strength_total = (strength_prev + strength_now) / 100;
			var mood_drop = r2(-res / (125 * strength_total));
			
			if ((mood_now - mood_drop) < (diplomacy ? 60 : 75 )) {
				$('#tilx_mood').addClass('moodlow');
			} else {
				$('#tilx_mood').removeClass('moodlow');
			}
			
			$('#tilx_res').text(text);
			$('#tilx_mood_drop').text(mood_drop);
			$('#tilx_strength_drop').text(r2(-res / (375 * strength_total)));
		};

		var handleResPlayer = function() {
			var res = calcRes();

			getCheckboxes();

			$('#tilx_res').text(res);
		};


		var getCheckboxes = function() {
			value.set('booty' + townId, $('#tilx_booty').is(':checked'));
			booty = $('#tilx_booty').is(':checked');

			value.set('autoSendUnits', $('#tilx_autoSendUnits').is(':checked'));
			autoSendUnits = $('#tilx_autoSendUnits').is(':checked');

			value.set('diplomacy' + townId, $('#tilx_diplomacy').is(':checked'));
			diplomacy = $('#tilx_diplomacy').is(':checked');
		};

		var calcStrength = function() {
			var strength = 0, strength_total = strength_prev + strength_now;

			$('.unit_input_ground').each(
					function() {
						var uCount = parseInt($(this).val(), 10);
						if (isNaN(uCount)) {
							uCount = 0;
						}
						strength += parseInt(GameData.units[$(this)
								.attr('name')].population, 10)
								* uCount / 5;
					});

			if (strength > 15) {
				$('#tilx_strength').addClass('error');
				strength = 15;
			} else {
				$('#tilx_strength').removeClass('error');
			}

			strength_total += strength;
			if (strength_total > 100) {
				$('#tilx_strength_total').addClass('error');
				strength_total = 100;
			} else {
				$('#tilx_strength_total').removeClass('error');
			}

			if (mood_now < (diplomacy ? 60 : 75 )) {
				$('#tilx_mood').addClass('moodlow');
			} else {
				$('#tilx_mood').removeClass('moodlow');
			}

			$('#tilx_strength').text(r2(strength));
			$('#tilx_strength_total').text(r2(strength_total));

			return strength;
		};

		var unitQuantity = function() {
			var $this = $(this);
			if ($("#attack_type_input").val() == 'farm_attack') {
				var population = (GameData.units[$this.attr('id')].population) / 5;
				var missing_strength = 100 - $('#tilx_strength_total').text();
				var total;
				if (missing_strength > 15) {
					missing_strength = 15;
					total = Math.floor(missing_strength / population);
				} else {
					total = Math.ceil(missing_strength / population);
				}
				
				var maxU = $this.find("span.black").text();
				var id = $this.attr('id');
				//Original: total = total > maxU ? maxU : total;
				total = total >= maxU ? maxU - (id=='hoplite' || id=='sword' || id=='archer' || id=='slinger' ? 1 : 0) : total;
				$('#unit_type_' + id).attr('value', total);
				TownInfo.setAttackType('attack_type_input', 'farm_attack');
				if ($('#tilx_autoSendUnits').is(':checked')) {
					TownInfo.sendUnits('','farm_town_info');
					$('#unit_type_' + id).attr('value', 0);
				}
				
				//if (strength_now + strength_prev >= strengthMax) {
					//TownInfo.setAttackType('attack_type_input', 'ask_farm_for_resources');
				//} else {
					//TownInfo.setAttackType('attack_type_input', 'farm_attack');
				//}
			} else if ($("#attack_type_input").val() == 'ask_farm_for_resources') {
				var lbooty = parseInt(GameData.units[$this.attr('id')].booty, 10);
				lbooty = $('#tilx_booty').is(':checked') ? lbooty *= 1.3 : lbooty;
				var percent = (strength_prev + strength_now) / 100;
				percent = percent > 1 ? 1 : percent;
				var res = calcRes();
				var maxLoot = Math
						.floor(6005 * Math.pow(percent, Math.sqrt(2)));
				res = maxLoot - res;
				res = res < 0 ? 0 : res;

				var total = Math.ceil(res / lbooty);
				var maxU = $this.find("span.black").text();
				total = total > maxU ? maxU : total;

				$('#unit_type_' + $this.attr('id')).attr('value', total);
			}else{//'ask_farm_for_units'
				$('#unit_type_' + $this.attr('id')).attr('value', $this.find("span.black").text());
			}
		};

		var calc = function() {
			handleResFarm();
			calcStrength();

		};

		var bindDurationCounter_new = function() {
			if (TownInfo.type === 'farm_town_info') {
				if ($('#duration_container').length !== 1
						|| $('#attack_type').length !== 1) {
					alert('Needed HTML content is not available! The Script YAGrepoFarmhelper is not compatible with this Version of Grepolis! '
					      + 'German: Benoetigte HTML-Elemente nicht vorhanden! Das Script YAGrepoFarmhelper ist mit diese Version von Grepolis nicht kompatibel.');
					return false;
				}

				if (strength_now + strength_prev < strengthMax) {
					//unsafeWindow.setTimeout(function() {
					//TownInfo.setAttackType('attack_type_input', 'ask_farm_for_resources');
					//}, 1000);
					//} else {
					//unsafeWindow.setTimeout(function() {
    					TownInfo.setAttackType('attack_type_input', 'farm_attack');
					//}, 1000);
				}

				var html = '<div style="position: absolute; width: auto; right: 107px; top: 235px;">' //right: 107px;
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.booty
						+ ');border-width:1px 0 1px 1px;padding-right:5px;width:auto;" id="tilx_res_cont">'
						+ '<span id="tilx_res">0</span> '
						+ '<label style="padding-left:15px;padding-right:15px;"><input type="checkbox" id="tilx_booty"><em>Beute</em> erforscht</label>'
						+ '</div>'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.mood_now
						+ ');float:left;width:auto;border-width:0 1px 1px;padding-right:10px;" id="tilx_mood_now">'
						+ '<span id="tilx_mood">' + r2(mood_now) + '</span> '
						+ '</div>'		
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.mood_drop
						+ ');float:left;width:auto;border-width:0 1px 1px;padding-right:10px;" id="tilx_mood_drop">0</div>'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.strength_drop
						+ ');border-width:0 0 1px;width:auto;" id="tilx_strength_drop">0</div>'
						+ '</div>'
						+ '<div style="position: absolute;  width: auto; right: 107px; top: 300px;">'
						+ '<div class="tilx_infos" style="float:left;width:auto;border-width:1px 0 1px 1px;padding-right:5px;width:auto;" id="tilx_autoSendUnits_chk">'
						+ '<label>Ang.</label>'
						+ '<input type="checkbox" id="tilx_autoSendUnits">'
						+ '</div>'
						+ '<div class="tilx_infos" style="float:left;width:auto;border-width:1px 0 1px 1px;padding-right:5px;width:auto;" id="tilx_diplomacy_chk">'
						+ '<label>Dipl.</label>'
						+ '<input type="checkbox" id="tilx_diplomacy">'
						+ '</div>'
						+ '</div>'
						+ '<div style="position: absolute; top: 172px; left: 373px;">'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.strength
						+ ');margin-top:3px;border-width:1px 1px 0;width:35px;" id="tilx_strength">0</div>'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.strength_prev
						+ ');border-width:0 1px;" id="tilx_strength_prev">'
						+ r2(strength_prev)
						+ '</div>'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.strength_now
						+ ');border-width:0 1px;" id="tilx_strength_now">'
						+ r2(strength_now)
						+ '</div>'
						+ '<div class="tilx_infos" style="background-image:url('
						+ images.strength_total
						+ ');border-width:1px;border-top-style:dashed;" id="tilx_strength_total">0</div>'
						+ '</div>';

				$('#units').after(html);

				if (booty) {
					$('#tilx_booty').attr('checked', 'checked');
				}

				if (autoSendUnits) {
					$('#tilx_autoSendUnits').attr('checked', 'checked');
				}

				if (diplomacy) {
					$('#tilx_diplomacy').attr('checked', 'checked');
				}

				$('#tilx_res_cont').setPopup('tilx_res');
				$('#tilx_mood_drop').setPopup('tilx_mood_drop');
				$('#tilx_mood_now').setPopup('tilx_mood_now');
				$('#tilx_strength_drop').setPopup('tilx_strength_drop');
				$('#tilx_strength').setPopup('tilx_strength');
				$('#tilx_strength_prev').setPopup('tilx_strength_prev');
				$('#tilx_strength_now').setPopup('tilx_strength_now');
				$('#tilx_strength_total').setPopup('tilx_strength_total');
				$('#tilx_autoSendUnits_chk').setPopup('tilx_autoSendUnits_chk');
				$('#tilx_diplomacy_chk').setPopup('tilx_diplomacy_chk');

				$('.index_unit').attr('onclick', '');
				$('.index_unit').bind('click', unitQuantity);
				$('.index_unit').bind('click', calc);

				$('.unit_input').bind('keyup', calc);
				$('#tilx_booty').bind('click', calc);
				$('#tilx_autoSendUnits').bind('click', getCheckboxes);
				$('#tilx_diplomacy').bind('click', getCheckboxes);

				calc();
			} else if (TownInfo.type === 'town_info') {
				var html = '<div class="tilx_infos" style="background-image:url('
						+ images.booty
						+ ');float:right;" id="tilx_res_cont">'
						+ '<span id="tilx_res"></span> '
						+ '<label style="padding-left:15px;padding-right:15px;">'
						+ '<input type="checkbox" id="tilx_booty"><em>Beute</em> erforscht</label>'
						+ '</div>';

				$('#duration_container').prepend(html).width(300);

				$('#way_duration, #arrival_time').width(70);

				if (booty) {
					$('#tilx_booty').attr('checked', 'checked');
				}

				$("#tilx_res_cont").setPopup('tilx_res');

				$('.index_unit').bind('click', handleResPlayer);
				$('.unit_input').bind('keyup', handleResPlayer);
				$('#tilx_booty').bind('click', handleResPlayer);
			}

			return bindDurationCounter_old.apply(TownInfo, arguments);
		};

		var sendUnits_new = function() {
			if (TownInfo.type === 'farm_town_info'
					&& $('#attack_type_input').val() === 'farm_attack') {
				strength_prev += calcStrength();
				//unsafeWindow.console.log("strength_prev: " + strength_prev);
				$('#tilx_strength_prev').text(r2(strength_prev));
			}

			var ret = sendUnits_old.apply(TownInfo, arguments);

			if (strength_prev + strength_now >= strengthMax) {
				TownInfo.setAttackType('attack_type_input', 'ask_farm_for_resources');
			}

			return ret;
		};

		var init_new = ( function() {
			var handleAjaxComplete = function() {
				if ($(this).find('#farmtown_strength').length === 1) {
					strength_now = parseInt($(
							'#farmtown_strength .farm_bar_container').text(),
							10);
				}
				if ($(this).find('#farmtown_mood').length === 1) {
					mood_now = parseInt($(
							'#farmtown_mood .farm_bar_container').text(),
							10);
				}			
				$('#townWindow').unbind('ajaxComplete', handleAjaxComplete);
			};

			return function(tid) {
				var r = init_old.apply(TownInfo, arguments);

				strength_prev = 0;

				$('#townWindow').ajaxComplete(handleAjaxComplete);

				return r;
			}
		}());
		
		var init_Overlay = function() {
			_wmap = uW.WMap;
			if ( _wmap && _wmap.mapData ){			
				GM_log('WMap clear?');
				var towns = _wmap.mapData.getTowns(
					_wmap.mapTiles.tile.x, 
					_wmap.mapTiles.tile.y, 
					_wmap.mapTiles.tileCount.x, 
					_wmap.mapTiles.tileCount.y
				);

				for (var townNr in towns ) {
					var town = towns[townNr];
				
					if ( town.mood ) {
						var tel = $("#farm_town_"+town.id );
						if (tel && tel.length > 0) {
							tel.html( '<table class=mFInfo><tr><td align="center"><span class="' 
							+ (( town.mood < (diplomacy ? 60 : 75 ) ) ? "mFDanger" : "mFOK" )+'">'
							+ town.mood + '%</span>&nbsp;<span class="' 
							+ (( town.strength < strengthMax ) ? "mFSGood" : "mFSOK" )+'">'
											+ town.strength +'%</span></td></tr><tr><td><span class="resource_'
											+ town.demand + '_icon"></span><span style="color:#E0E0E0;" class="popup_ratio">'
							+ town.ratio + '</span><span class="resource_'
							+ town.offer + '_icon"></span></td></tr></table>');
						}
					}
				}
			}
		};
		// ---------------------------------------------------------------------------------------------------------------------------
		// The Script Functions
		return function() {
			GM_log('starting Script' + SCRIPTNAME);

			value = newValueLib(SCRIPTNAME);
			if (document.URL.indexOf('game/map') !== -1) {
				GM_log('initialize style');
				GM_log('townId =' + townId);
				initStyle();

				booty = value.get('booty' + townId, 'false') == 'true';
				autoSendUnits = value.get('autoSendUnits', 'false') == 'true';
				diplomacy = value.get('diplomacy' + townId, 'false') == 'true';

				GM_log('booty =' + booty);
				GM_log('autoSendUnits =' + autoSendUnits);
				GM_log('diplomacy =' + diplomacy);
				
				GameData = uW.GameData;
				TownInfo = uW.TownInfo;

				if (typeof TownInfo !== 'object'
						|| typeof TownInfo.bindDurationCounter !== 'function'
						|| typeof TownInfo.sendUnits !== 'function'
						|| typeof GameData !== 'object'
						|| typeof GameData.units !== 'object') {
					GM_log('Error occured: This version is not compatible with the current grepolis version!');
					alert('This version is not compatible with the current grepolis version!');
					return false;
				}

				bindDurationCounter_old = TownInfo.bindDurationCounter;
				sendUnits_old = TownInfo.sendUnits;
				init_old = TownInfo.init;

				TownInfo.bindDurationCounter = bindDurationCounter_new;
				TownInfo.sendUnits = sendUnits_new;
				TownInfo.init = init_new;

				GM_log('adding popupTexts');
				// Add Pop-up texts we use here (will be bound to html elements later by "setPopup" calls)
				uW.PopupFactory.addTexts({
							tilx_res: 'Maximale Beute des aktuellen Angriffes',
							tilx_mood_drop: 'Stimmungssenkung des aktuellen Angriffs (bei voller Beute)',
							tilx_strength_drop: 'St&auml;rkesenkung des aktuellen Angriffs (bei voller Beute)',
							tilx_strength: 'St&auml;rkesteigerung des aktuellen Angriffs',
							tilx_strength_prev: 'St&auml;rkesteigerung der vorherigen Angriffe',
							tilx_strength_now: 'St&auml;rke des Bauerndorfes im Moment',
							tilx_mood_now: 'Stimmung des Bauerndorfes im Moment',
							tilx_strength_total: 'Neue St&auml;rke insgesamt',
							tilx_autoSendUnits_chk: 'Automatisch Angriff absenden',
							tilx_diplomacy_chk: 'Diplomatie erforscht',
				});

				GM_log('preparing town Infos later');
				setTimeout( init_Overlay, 250 );

			}

		}
	}());

	YAGrepoFarmhelper();
}());
