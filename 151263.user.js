               ...
               members: {
                   ...
                   initialize: function() {
                       ...
                       this.createClaimButtons();
						this.createUploadTSButton();
                   },
					createUploadTSButton: function() {
						var buttonLayoutTS = new qx.ui.layout.HBox(4).set( {alignX:"center"} );
						this.uploadButtonRow = new qx.ui.container.Composite( buttonLayoutTS ).set({maxWidth:306});

						this.app.getCityInfoView().container.add(this.uploadButtonRow);

						btnUpload = new qx.ui.form.Button(" Upload TS ").set( {Width:80, height: 32, alignX: "center", paddingLeft: 0, paddingRight: 0} );
						btnUpload.addListener("click", function() {this.uploadTs()}, this);
						this.uploadButtonRow.add(btnUpload);
                   ...
                   createClaimButtons: function() {
					   ...
					   btnClaim = new qx.ui.form.Button(" Baron ").set({Width:40, height: 32, alignX: "center", paddingLeft: 0, paddingRight: 0});
					   btnClaim.addListener("click", function() {this.claimThis("claim")}, this);
					   this.buttonRow.add(btnClaim);

					   btnLoc = new qx.ui.form.Button(" Loc ").set({Width:40, height: 32, alignX: "center", paddingLeft: 0, paddingRight: 0});
					   btnLoc.addListener("click", function() {this.claimThis("loc")}, this);
					   this.buttonRow.add(btnLoc);
					   
					   btnPlayer = new qx.ui.form.Button(" Player ").set({Width:40, height: 32, alignX: "center", paddingLeft: 0, paddingRight: 0});
					   ...
					   */
					   ...
					   btnClaimN = new qx.ui.form.Button(" Baron ").set({Width:40, height: 32, alignX: "center", paddingLeft: 0, paddingRight: 0});
					...
					claimThis: function(v) {
						...
						if (v == "check" || v == "claim" || v == "fight" || v == "player" || v == "shoot" || v == "loc") {
							...
							switch (v) {
								case "check": botAction = "/w " + this.getBotName() + " !check "; break;
								case "claim": botAction = "/w " + this.getBotName() + " !baron "; break;
								/*case "fight": botAction = "/w " + this.getBotName() + " !fight "; break;*/
								case "player": botAction = "/w " + this.getBotName() + " !player "; break;
								/*case "shoot": botAction = "/w " + this.getBotName() + " !shoot "; break;*/
								case "loc": botAction = "/w " + this.getBotName() + " !loc "; break;
							}
						}
						else if (v == "checkn" || v == "claimn") {
							posX = this.newcityView.cityPosX; posY = this.newcityView.cityPosY;
							switch (v) {
								case "checkn": botAction = "/w " + this.getBotName() + " !check "; break;
								case "claimn": botAction = "/w " + this.getBotName() + " !baron "; break;
							}
						}
					...
					, getBotName: function() {
						...
						switch (webfrontend.data.Alliance.getInstance().getName()) {
							case "Pixel Warriors":
								botName = "jabnael";
								break;
							case "Pixel Armada":
					...
					, uploadTs: function() {
						...
						if (units) {
							...
							sb.add("/w " + this.getBotName() + " !ts ", city.getId(), " max:", city.getUnitLimit())

							for (var key in units) {
								sb.add(" ", key, ":", units[key]["total"])
							}

							this.chat.chatLine.setValue(sb.get());
							this.chat.sendCurrent();	
						}
						else {
							this.addChatMessage('Sorry! No units to report in this city!', true);
						}
					}
