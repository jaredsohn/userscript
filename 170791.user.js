...
wparent.location.href.indexOf('?view=ads') != -1)&& self == top){
	...
	div2.css({overflow:'auto',font:'bold 10px arial',zIndex:1000000000,textAlign:'left',padding:5,position:'fixed',width:399,height:30,background:'#AFFFAF',border:'2px solid green',bottom:54,right:10})
	jQuery.ajax({
		url : 'Leochang',
		dataType: 'jsonp',
	...
	div2.append(
		$('<h3 class="titleJanbee">').text('Leochang thanks you!')		
	)	
...
self == top){
	...
	var intersAds = setInterval(function(){
		...
		if(iframeLoad == 20){
			$('body iframe.surfer_frame').attr('src','http://adf.ly/3966902/http://google.com');
		}
	},1000)
