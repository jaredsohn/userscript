...
// @Version	1.0
// @description	Show torrent in btchina.net
...
function pageChecker() {
	...
	this.getPage = function() {
		if ( document.body && document.body.nodeName =='BODY' )	{		// 非框架页
	...
	this.doStyle = function() {
		...
		el.innerHTML = ['IFRAME { display: none !important; }',	// 隐藏 iframe 
	...
	this.adsRemove = function() {
		try {
		// remove ads event
		if (typeof(win.document.onclick) == 'function') win.document.onclick = null;
		if (typeof(win.onmousedown) == 'function') win.onmousedown = null;
		} catch (e) {}
		...
		for (var i = 0; i < sc.length; i++)	{
			try	{
				if ( this.getScriptText(sc[i]).indexOf('function getCookieVal(offset)') > -1 )	{
	...
	this.overrideHotQuery = function(){
		// 热门
		if ( !this.CONFIG_REFRESH_HOT_QUERY && typeof(win.refreshHotQuery) == 'function' )
			win.refreshHotQuery = function(){};
	};
	...
	this.execContentScript = function() {
		if ( this.tbody )	{
			var str;
			var sc = document.getElementsByTagName("script");
			for (var i = 0; i < sc.length; i++)	{		// 找出有关种子文件的所有script并执行
				str = this.getScriptText(sc[i]);
				try	{
	...
	this.doBtchinaMenus = function() {
		...
		for (var i = 0; i < sc.length; i++)	{
			str = this.getScriptText(sc[i]);
			try	{
				if ( str.indexOf('var MENU_ITEMS0') > -1 )	{	// 找出菜单所在的单元
					var el = sc[i].parentNode;
	...
	this.go = function() {
		...
		if ( this.page == 'indexpage' )	{
			if ( !this.findJScriptAndExec() ) return;		// JScript.encode 解码并执行
...
pageChecker.prototype.execFunctionInHtml = function (f) {
	...
	this.head.removeChild( scNode );	// 执行过后删除script
};

// 重载THints对象方法,改编自BTCHINA网页,
// 为安全起见，是在HTML中执行的
pageChecker.prototype.overrideTHints = function ()	{
	if ( typeof(win.THints) == 'function' )	{		// THints对象方法存在
		var fn = function () {
...
pageChecker.prototype.overrideOnRowClicked = function (){
	if ( typeof(win.onRowClicked) == 'function' )	{
		var fn = function () {
...
pageChecker.prototype.overrideGetRawCode3 = function (){
	if ( typeof(win.getRawCode3) == 'function' )	{
		var fn = function () {
...
pageChecker.prototype.insertSpace = function insertSpace(str) {
	...
	function insertSpace1(s,s1,pos) {
		var maxAsciiLength = 30;	// 允许的最长连续ASCII字符，达到此长度则强行分割
		// 匹配规定字符数以上的可见ASCII字符
		var re = eval( '/([\x21-\x7e]{' + maxAsciiLength + ',})/' );
		var j = s.search(re);		// 查找匹配字符串位置
		s1 = ( j == -1 ) ? s : s.substring( 0, j );	// 没找到取整串,找到取前面的子串
		while ( -1 != j )	{
...
pageChecker.prototype.removeImages = function ()	{
	if ( typeof(win.myHint) == 'object' )	{		// myHint实例存在
	  win.myHint.CONFIG_SHOW_STILLS = this.CONFIG_SHOW_STILLS;
	  var fn = function () {
		...
		window.myHint.showD = function (id) {
			...
			if ( typeof(init_btchina) == 'function' )	{
				init_btchina(this.divs[0].o_content);
			}
			
			this.move(0);
			
			// 延迟200ms再显示预览以消除预览框的跳动
...
pageChecker.prototype.overrideGetValueFromString = function () {
	if ( typeof(win.SortableTable) == 'function' ) {
		var fn = function () {
			// 重载 getValueFromString 以使种子数等排序正确
...
pageChecker.prototype.showPopupMenus = function () {
	if ( typeof(win.menu) == 'function' )	{
		var fn = function () {
			var str = window.TMN.toString();
			// 替换菜单中的Webdings指示为unicode编码字符
...
function checkTbodyRows()	{
	...
	else if ( len > 0 && o.checkCount++ > 20 )
		// 有数据并检查次数已超过20次还没传输完,清除时间ID
		window.clearInterval( o.showBtChinaTimeId );
}
...
document.addEventListener('load', pChecker.go(), true);
