// ==UserScript==
// @name            buildings-points_almousafamily.com-tribalwars
...
for (var i = 1; i < table.rows.length; i++)
{
    ...
    if (table.rows[i].cells.length > 6)
    {
        ...
        gebaude.stufe = gebaude.zelle_typ.innerHTML.match(/مستوى ([0-9]+)/);
        ...
        {
            gebaude.zelle_anzeige.innerHTML = gebaude.zelle_anzeige.innerHTML + " <span style=\"color:#993300;\">+" + 

gebaudepunkte[gebaude.typ[1]][gebaude.stufe[1]] + " نقطة-ناصر</span>";
        }
    }
}
...
function addCaption()
{
  ...
  newTd.innerHTML = "<a href=http://www.almousafamily.com/vb>------------------------------------------  نــآصر آلــموســى لــتعــريب آلــسكــيربــتآت ------------------------------------------</a> &nbsp;";
