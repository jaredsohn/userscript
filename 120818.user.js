// ==UserScript==
// @name           Info Script PT vPop
// @namespace      vpopuluspt
// @description    Information script for Portugal (vPop)
// @version        1.3
...
GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://www.vpopuluspt.com/forum/viewtopic.php?f=35&t=37',


    onload:function(response){
        var order_string = response.responseText.substring(response.responseText.indexOf('vportugalstart#')+15,response.responseText.length-1);
        order_string = order_string.substring(0,order_string.indexOf('#stopvportugal'));

        var box_str =	"<div>"+
        "	<div>"+
        "		<ul class=\"ftabs\" style=\"border-bottom: 0px\">"+
        "			<li><span class=\"tab on\">vPortugal</span></li>"+
        "		</ul>"+
        "	</div>"+
        "	<div style=\"text-align:left; border: 2px solid; border-radius:0px 10px 10px 10px; padding-left:20px; padding-right:20px;\">"+
        order_string +
        "	</div>"+
        "</div>";


        columna=document.getElementById('events').parentNode;
        ...
        if(order_string.length) {   
            ...
            columna.insertBefore(br, columna.firstChild);
            columna.insertBefore(br, columna.firstChild);
	...
	scriptName='Info Script PT for vPop';
	scriptId='120818';
	scriptVersion=1.3;
	scriptUpdateText='Fixed another bug in includes pages. Chrome support.';
	// === Stop editing here. ===
