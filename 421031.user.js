// ==UserScript==
// @name			OGame Alarm by el 121
// @version			2.5
// @namespace		el 121 educito
// @description		Ogame alarm by 121 for the redesign, audio and email alarm for incoming attacks
// @include			http://*.ogame.*/*
// @include			http://ogame.*/
...
//var probeSound = "http://static.sonidos-gratis.com/snd/98/96/98964f6e062e18e6df20d6e5cc05683a.mp3";
//var attackSound    = "http://static.sonidos-gratis.com/snd/b8/cb/b8cbc72062437231306758abdfb14487.mp3";
//var attackSoundRIP = "http://sonidosmp3gratis.com/sounds/emergency029.mp3";
//var messageSound   = "http://amazingsounds.iespana.es/carhornold2.wav";
//var messageSound -> http://amazingsounds.iespana.es/carhornold2.wav
...
var attackSoundRIP    ="http://sonidosmp3gratis.com/sounds/emergency029.mp3";
var messageSound   = "http://amazingsounds.iespana.es/carhornold2.wav";
var confedeSound   = "http://sonidosmp3gratis.com/sounds/alarma_5.mp3";
var destroySound   = "http://sonidosmp3gratis.com/sounds/alarma-de-incendios.mp3";
...
var MIN = 60;  // Minmum number of seconds to wait until reload overview. Default = 5 minutes = 180 seconds
var MAX = 300; // Maximum number of seconds to wait until reload overview. Default = 30 minutes = 1800 seconds
// ***** End User Variables ****
...
function playSound() {
	...
	switch ( theSound ) {
		...
		case probeSound:	emb.setAttribute("volume", volProbes);
							break;
                case destroySound:	emb.setAttribute("volume", volAttacks);
							break;
...
if ( tMission.length < 1 ) continue;
						if ( tMission[0].firstChild.src.indexOf("spionage") != -1 ) {
							theSound = probeSound;
						} else if ( tMission[0].firstChild.src.indexOf("angriff") != -1 ) {
							theSound = attackSound;
						} else if ( tMission[0].firstChild.src.indexOf("verband") != -1 ) {
							theSound = attackSound;
                                                } else if ( tMission[0].firstChild.src.indexOf("zerstoeren") != -1 ) {
							theSound = destroySound;
						break;
						}
					}
					if ( theSound != 0 ) {
