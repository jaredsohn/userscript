// ==UserScript==
// @name Travian JScript (TBJS) by dbgg  v0.6
// @author dbgg
...
//================================================================
// Web site: http://hack-travian.sitecity.ru/
// Автор/Author: dbgg
//---------------------------------------------------------------
// НАЗНАЧЕНИЕ:
...
---------------------------------------------------------------
 ...
 - для Римлян учтен механизм постройки одновременно двух зданий. Его можно включить
			нажав галочку
---------------------------------------------------------------*/
// ПРЕДУПРЕЖДЕНИЯ И ЛИЦЕНЗИИ:
// - TJS скрипт предоставляется КАК ЕСТЬ (AS IS), вы его используете на СВОЙ СТРАХ
...
//			никакой ответственности за какие-то нибыло последствия, которые может
...
0.6 : 
 ...
 - исплавлена ошибка с торможением на браузерах Opera v9 и ниже.
 - исправлена ошибка с вылетанием браузера
 - исправлена некорректная работа функции переодического выполнения задач. В данной
 	версии не выявлено некорректного выполнениях "списка задач".
 ...
 BETA 1 :
 - добавлена внизу страницы ссылка на сайт Hack-Travian
---------------------------------------------------------------*/
...
var web		= 'http://hack-travian.sitecity.ru';
...
function onLoad()
{
	...
	div1.id = 'idd';
//	$(document.body).appentChild(div1);
	// Определяем язык =====================================
...
//--------------------------------------------
	...
	//resource
	htm	= htm+"</table>";
	htm	= htm+"<br />Сайт Хак-Травиан: <a href='"+web+"'>ХАКНЕМ?</a>";
	$('id4').setHTML(htm);
...
function addElement()
{
	...
	var text_  = text_+'<span id="_r0"></span><span id="_r1"></span><span id="_r2"></span><span id="_r3"></span><span id="_r4"></span></div>';
...
{//zxc
	...
	// Перезагружаем текущую страницу каждые 10 минут, для того, чтобы
	// неотлогиниться от сервера, пока  ожадием выполнения задания
	tm_1 = $('tp1').innerText;
	tm_1 = tm_1.split(':');
	if(((parseInt(tm_1[1]) == 30) || (parseInt(tm_1[1]) == 0) || (parseInt(tm_1[1]) == 10) || (parseInt(tm_1[1]) == 45)) & (parseInt(tm_1[2]) == 0))
