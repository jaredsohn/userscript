...
function RemoveCookies(s, n){
    n = (n == 'speed') ? 'speed' : 's'+n;
	window.location = 'http://' + n + '.travian.' + s + '/login.php?del_cookie'
}
...
function setUser(name, pass, userver, servernu){
	...
	for (i=1;i<=10;i++){
		if (getUsers(i)==name){
			if (!userver) userver=prompt("What is your travian server prefix? (Example: com, ir, ...)",GM_getValue('tuc_user_server_' + i,'org'));
			...
			if (!userver) userver=prompt("What is your travian server prefix? (Example: com, ir, ...)",'org');
...
function main(){
    ...
    pra.appendChild($e('b', 'Меню'));
	pra.appendChild($e('hr'));
	pra.appendChild(CreateCI("Добавить акк", function(){setUser()},""));
	pra.appendChild(CreateCI("Удалить акк", function(){delUser()},""));
	pra.appendChild(CreateCI("And...", function(){alert("Keep rockin >:)")},""));
    pra.appendChild($e('hr'));
    pra.appendChild($e('b', 'Аккаунты'));
	pra.appendChild($e('hr'));
	var i=1;
	for (i=1;i<=10;i++){
		var name=getUsers(i);
