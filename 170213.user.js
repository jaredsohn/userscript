// ==UserScript==
// @name           UnfollowHater Pro + Last Update
...
function crearCaja(){
	...
	cerrar.innerHTML="X";
	cerrar.setAttribute("style","border:1px solid black;background-color:#BAC4CF; display:none; text.decoration:none; text-align:center;font-weight:bold;margin-left:315px;margin-top:-19px;width:15px; height:16px;");
	...
	reset.innerHTML="RST! ";
	reset.setAttribute("style","border:1px solid black;background-color:#BAC4CF;font-weight:bold; text.decoration:none; text-align:center; margin-left:283px; margin-top:-18px;width:15px; height:20px;");
	reset.addEventListener("click", function(event){
		var tumblelog = getTumblelog();
		localStorage.removeItem(tumblelog+"_followersList");
		localStorage.removeItem(tumblelog+"_unfollowersList");
		localStorage.removeItem(tumblelog+"_newFollowers");
		localStorage.removeItem(tumblelog+"_cantFollowers");
		localStorage.removeItem(tumblelog+"_cantCompareList");
		localStorage.removeItem(tumblelog+"_allUnfollowersList");
		window.location = blogUrl + getTumblelog();
	}, false);
	newDivA.appendChild(reset);
...
function alimentarLista(divHTML,listadoGuardar){
	...
	console.info(paginaFollowers.length);
	for(var i=0; i<paginaFollowers.length; i++){
		var url = paginaFollowers[i].getElementsByTagName("div")[0].getElementsByTagName("div")[0].getElementsByTagName("a")[0].childNodes[0].nodeValue;
