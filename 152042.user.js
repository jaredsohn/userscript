// ==UserScript==
// @name           LOU Programa ATK's
// @description    Einfacher Programa ATK's
// @author         Damien69
...
LouTKSMassAttack.prototype.init = function() {
   ...
   // ***** City Layout Button ***** //
   var layoutButton = new qx.ui.form.Button("Programa ATK's");
   layoutButton.set({appearance: "button-text-small", toolTipText: "Envia ATK masivo."});
   layoutButton.addListener("click", ShowMassScheduler, false);
   qx.core.Init.getApplication().cityInfoView.buildingQueue.header.add(layoutButton, {top: 5, left: 71});

   // Execute Plan Button
   qx.core.Init.getApplication().getMailPage();
   var ExecuteButton = new qx.ui.form.Button("Importa Plan ATK's");
   ...
   var CityIntelButton = new qx.ui.form.Button("Obtener Int.");
   CityIntelButton.set({appearance: "button-text-small", toolTipText: "Crear reporte de intel"});
   ...
   if (cityIntelIndex >= 0) {
      ...
      if (cityIntelArray.length > 0) {
         ...
         qx.core.Init.getApplication().getDesktop().add(exportwin, {left: 0, right: 0, top: 0, bottom: 0});
            ...
            dialog.showGenericNotice("Obtener Int.", "No hubo info de relevancia para recolectar", "", "webfrontend/ui/bgr_popup_survey.gif");
...
function AddDecoy()
{
   if (qx.core.Init.getApplication().cityDetailView && qx.core.Init.getApplication().cityDetailView.city) {
         ...
         qx.core.Init.getApplication().cityDetailView.cityImage.getSource().indexOf("_military_") >= 0)
      {
         ...
         var x=cx&0xFFFF;
         var y=cx>>16;
		var z1="0"; if (x>=100) z1=""; //agregada para arreglar las coordenadas
		var z2="0"; if (y>=100) z2=""; //agregada para arreglar las coordenadas
		text += z1+x+":"+z2+y;
...
function PrepareEmail()
{
   ...
   emailString += "[b]Ataque Fuerte:[/b] "+d+" . "+m+" . "+y+" @ "+h+" : "+n+" : "+s+" ("+attackTime+")\n";
   emailString += "[b]Coordenadas:[/b] "+RealTarget.getValue()+"\n\n";
   emailString += "[b]ATK Falso:[/b]:\n";
   ...
   if(qx.core.Init.getApplication().sendMail != null && qx.core.Init.getApplication().currentOverlay==qx.core.Init.getApplication().sendMail && qx.core.Init.getApplication().sendMail.hasUnsaved()) {
      ...
      qx.core.Init.getApplication().sendMail.subject.setValue("Programar ATK's");
   }
}
...
function ExecuteAttackPlan() {
   ...
   if (qx.core.Init.getApplication().getMailPage().__Ou.$$user_value.indexOf("Programar ATK's") != 0) {
      err = 1;
      console.log("PS Error: start of ExecuteAttackPlan: Line 261");
   } else {
   
      var msg = qx.core.Init.getApplication().getMailPage().__OD.$$user_value;
      var sTroopAttackTick = ExtractString(msg, "<b>Ataque Fuerte:</b>", "<br />", "(", ")");
      var sWGAttackTick = ExtractString(msg, "<b>Coordenadas:</b>", "<br />", "(", ")");
      var sAttackTarget = ExtractString(msg, "<b>ATK Falso:</b> ", "/a><br />", ">", "<");
      var decoyString = "";
      if (sTroopAttackTick == "" || sWGAttackTick == "") {
         err = true;
      } else {
         var searchString = "<b>ATK Falso:</b><br />";
...
console.log("PS Error: start of ExecuteAttackPlan Error at line 283: " + err);
   if (!err) {
     ...
     decoyUnitTypeChanged();
      ...
      var dialog = new webfrontend.gui.ConfirmationWidget();
      dialog.showGenericNotice("Ejecutar Plan ATK's", "Este no es un plan de ataque válido.", "", "webfrontend/ui/bgr_popup_survey.gif");
      qx.core.Init.getApplication().getDesktop().add(dialog, {left: 0, right: 0, top: 0, bottom: 0});
   }
}
...
function showExportWin(text) {
   ...
   wdg.showExportWin = function() {
      ...
      var la = new qx.ui.basic.Label("Intel Obtenida");
      ...
      var okButton = new qx.ui.form.Button("Aceptar");
...
function ShowMassScheduler() {
   if (!opts) {   // Don't reload the page, otherwise you lose the settings
      ...
      opts.setTitle("LOU Programa ATK's - Versión 5.1.1.2");
   ...
   // Unit Type
      var UnitLabel = new qx.ui.basic.Label("Tipo Tropas").set({width:85});
      ...
      resetUnits();
      UnitDrop.setToolTipText("Seleccionar unidades para enviar en ataque masivo.");
   ...
   // Attack Type
      var AttackLabel = new qx.ui.basic.Label("Tipo Unidad").set({width:85});
      ...
      AttackDrop.add(new qx.ui.form.ListItem("Exploraciones",null,1));
      AttackDrop.add(new qx.ui.form.ListItem("Saqueos",null,2));
      AttackDrop.add(new qx.ui.form.ListItem("Asaltos",null,3));
      AttackDrop.add(new qx.ui.form.ListItem("Apoyos",null,4));
      AttackDrop.add(new qx.ui.form.ListItem("Asedios",null,5));
   ...
   // Unit Count
      var NumOfUnitsToSendLabel = new qx.ui.basic.Label("Num Tropas").set({ width:85 });
   ...
   // Date
      ...
      var DateLabel = new qx.ui.basic.Label(" Llegada (fecha):").set({width:85});
   ...
   // Time
      ...
      var TimeLabel = new qx.ui.basic.Label(" Llegada (hora):").set({width:85});
   ...
   // City box
      ...
      var CityLabel = new qx.ui.basic.Label("Coordenada Falsa (Max 16)");
   ...
   // Add to page
      ...
      var decoyGroup = new qx.ui.groupbox.GroupBox("ATK Falso");
      ...
      var RealGroup = new qx.ui.groupbox.GroupBox("Coordenada");
...
function AddTroopInfo() {
   ...
   RealAttackDrop.add(new qx.ui.form.ListItem("Exploraciones",null,1));
   RealAttackDrop.add(new qx.ui.form.ListItem("Saqueos",null,2));
   RealAttackDrop.add(new qx.ui.form.ListItem("Asaltos",null,3));
   RealAttackDrop.add(new qx.ui.form.ListItem("Apoyos",null,4));
   RealAttackDrop.add(new qx.ui.form.ListItem("Asedios",null,5));
   RealAttackDrop.setSelection([RealAttackDrop.getChildren()[4]]);

   var row = new qx.ui.container.Composite(new qx.ui.layout.HBox());
   row.setAlignY("bottom");
   RealTargetLabel = new qx.ui.basic.Label("Coordenada").set({width:85});
...
/*
Exception: document.getElementsByTagName("head")[0] is undefined
@Scratchpad:997
@Scratchpad:9
*/