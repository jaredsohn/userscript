// EBPL MODIFIED VERSION OF
// IMDB Lycoming County Library System Search 1.0
// Jeffrey Bower
// Original Script - Sean LeBlanc - IMDB Jefferson Lookup 1.0

// This puts a link into a search on EBPL library system from the movie page.
...
// select "IMDB - East Brunswick Public Library Search 1.0", and click Uninstall.

//
// ==UserScript==
// @name          IMDB - East Brunswick Public Library Search 1.0
// @namespace     http://userscripts.org/users/51482
// @description	  While viewing a title on IMDB, it will place a link to a search for said title in the East Brunswick Public Library (New Jersey, USA).
...
var titleXPath1 = "//div[@id='tn15content']";
...
(function() {
	...
	//alert("ok, we've looked for the spot at amazon.com to drop content");
		...
		if (isbn)  {
			...
			GM_xmlhttpRequest( { 
				method:"POST", 
				url:'http://librarycatalog.ebpl.org/ipac20/ipac.jsp?&menu=search&index=PISBN&term="' + isbn + '"',
				data:'',
					...
					for (var i=0; i < arrContent.length; i++) {
						if ( arrContent[i].match(/Place Hold/)) {
							...
							temp = temp.split('"')
							temp = temp[0]
							placeHold = "http://librarycatalog.ebpl.org" + temp;
					...
					if(!flagME){
						...
						itemType = "DVD";
						var mediaUrl = 'http://librarycatalog.ebpl.org/ipac20/ipac.jsp?&menu=search&index=PISBN&term=';
						url = mediaUrl
						insertItemSearchText(itemAvailabilityDiv, "East Brunswick Public Library Search Results", url, itemType);
					}
					else{
						itemType = "DVD";
						var mediaUrl = 'http://librarycatalog.ebpl.org/ipac20/ipac.jsp?&menu=search&index=PISBN&term="';
...
function getTitleLocation() {
	...
	if(xpathFirst(titleXPath1) != null){
		...
		var mainTitleSpan = xpathFirst("//div[@id='main']")
	}
		...
		if(available[i] != "Title Unavailable"){
			if(available[i] != "Checked Out"){
				if(available[i] != "Available online"){
					div2.innerHTML += "<table width='100%;' height='30px' cellpadding='0' cellspacing='0' style='padding-left:5px; padding-bottom:5px;'><tr><td><b>" + available[i] + " - <a href='" + url + "' target='_blank'>view</a> | <a href='" + placeHold + "' target='_blank'>place hold</a></b></td></tr></table>";
				}
			}
			else{
				div2.innerHTML += "<table width='100%;' height='30px' cellpadding='0' cellspacing='0' style='padding-left:5px; padding-bottom:5px;'><tr><td><b>" + available[i] + " - <a href='" + url + "' target='_blank'>view</a></b></td></tr></table>";
			}
		}
		else{
			div2.innerHTML += "<table width='100%;' height='30px' cellpadding='0' cellspacing='0' style='padding-left:5px; padding-bottom:5px;'><tr><td><b>" + available[i] + "</b></td></tr></table>";
		}
	}
...
if (!GM_xmlhttpRequest || !GM_log) {
    alert('The EBPL imdb Lookup script requires Greasemonkey 0.5.3 or higher.  Please upgrade to the latest version of Greasemonkey.');
    return;
}