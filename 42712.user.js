...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Skriv en hilsen") {
  links[i].innerHTML="Skriv en litt designet hilsen:p";
  links[i].style.color="#FF8C00
"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Postkasse") {
  links[i].innerHTML="Mine private brev";
  links[i].style.color="#FFFF00"; {
  }
 }
}
...
akt.innerHTML = akt.innerHTML + '<div id="Bestfriends"></div>';
}
}
//CALADORION ©
for (var c = 0; c < window.document.getElementsByTagName('div').length; c++){
...
anchor.setAttribute( 'href', 'http://www.nettby.no/community/news.php?community_id=942424' );
anchor.appendChild( document.createTextNode( 'Pimp min profil 6' ) );
...
anchor.appendChild( document.createTextNode( 'Profildesignerne' ) );
...
anchor.appendChild( document.createTextNode( 'Se på andres profiler' ) );
document.getElementById( 'Random' ).appendChild( anchor );

function poeng(e){try{e.innerHTML=e.innerHTML.replace(/Poeng i dag/,"Poeng du har grabba i dag").replace(/(\d*)\/20\)/,"--0--)").replace(/Poeng totalt/,"Jeg har totalt").replace(/\(\d*\/2000\)/,"(2001/2000)").replace(/Bling-O-Meter:/,"BLING!BLING!BLING!:");
...
{switch(d.innerHTML){case 'Din aktivitet':case 'Finn borger':case 'Inviter en venn':case '<span>Sist pålogget </span>':case 'Sist påloggede i byen vår':case 'Informasjon':d.className="rtl";break;case 'Aktivitet':poeng(window.document.getElementsByTagName('div')[i+1]);
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="mujaff") {
  links[i].innerHTML="Leder over Profildesignerne";
...
for(q=0;q<innom.length;q++){
	...
	if(innom[q].innerHTML=="Sist innom")
		{innom[q].innerHTML="Disse har vært her før deg profildesigner:";
		break;
	}
}
...
//CALADORION ©
for (var c = 0; c < window.document.getElementsByTagName('div').length; c++){
if (/^Inviter en venn til oss$/.test(window.document.getElementsByTagName('div')[c].innerHTML)) {
