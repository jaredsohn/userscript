...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=16709406&tid=5570611912564685545", false);
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[blue][b]Titulo:[/b][/blue] ".concat(titulo, breakLine, "[red][b]Criador:[/red][/b] ", "[green]", uid, breakLine, "[/green]", breakLine, "[/green][green][b]Motivo:[/b][/green] ", motivo, breakLine, "[silver][b]Horario:[/b][/silver] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
	...
	function online() {
		...
		var breakLine = "\n";
		var texto = String("[b]Guilherme [green]Entrou[/b][/green]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function offline() {
		...
		var breakLine = "\n";
		var texto = String("[b]Guilherme [red]Saiu[/b][/red]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent(""),
