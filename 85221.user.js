// ==UserScript==
// @name           AutoClicker for DeafBux
// @namespace      http://bestbuxlist.com
// @description    An Auto-click for DeafBux, makes you validated without viewing the ads. Ads loaded faster, because this script ignores all tags. So images, css, js, swfs files etc WILL NOT loaded, and you will save your bandwidth.
// @include        *://*deafbux.com/*
// @copyright      Blank
// ==/UserScript==
var force_to = "blanks";
...
if(correctURL(/\/ads\.php/)) {
	if(login) {
		...
		load = function() {
            ...
            msg.innerHTML = "Loading ads <b id='AutoClicker-current'>\"" + (urls[current].parentNode.parentNode.parentNode.getElementsByTagName('a')[0].innerHTML) + "\"</b>...<br /><div id='AutoClicker-loading'></div>";
            b.open("GET", urls[current].href, true);
            b.send(null)
        };
        for (i = A.length - 1; i >= 0; i--) if (isAds(A[i])) urls[urls.length] = A[i];
        robot.id = "AutoClicker-container";
        robot.align = "center";
        html = "<style>";
        html += "#AutoClicker-container *{font-family:arial;color:black;font-weight:bold;text-decoration:none}";
