...
function tToggle() {
   $(document).ready(function(){
	  ...
	  $("#content div.bg-img-green h4 a").find("a:first").attr("title",hint).click(togf);
      $("#content div.bg-img-green h4").find("a:first").attr("title",hint).click(togf).each(function(){
           var name = $(this).attr("href");
           $(this).parent("h4").append(getIgnore(name));
       })
      $("#content div.bg-img-green h4 a.ignore").click(igf);
   }); 
}
...
function igf() {
   //alert('ignore');
   var name = $(this).attr("href");
   $("ul.topic-reply li").filter(function(){
	   var s = $(this).find("div.user-face a").attr("href"); 
	   return (s==name)
	}).remove();
   return false
}
...
function set_levels() {
   var all_comments = $("ul li div div h4");
...
function gotolevel(l){
   if (document.getElementsByClassName('user-face')[l%100]==undefined) { // outbound.
     window.scrollTo(0, document.body.scrollHeight);
   }
   else {
...
function set_paginator(){
  ...
  if ($("div.paginator").length > 0) {
	$("div.paginator").append(gotobus);
  }else{ 
	$('div.topic-doc h3 span.pl20').after(gotobus);
  }
  
  var paginator = $("<div class='paginator'/>").html($("div.paginator").html());
  $("div.article").prepend(paginator);  
...
function checkPageType(a) {
    ...
    else if ((parent == "DIV") && (hasTagP > 0)) {//!! this is in group page & photo_note page.
...
function setLevels(query) {
	//FIXME 
	query = getLevelsQuery(query);//generate new query
...
function getLevelsQuery(query){
	var _href = location.href;
	if(_href.indexOf('http://www.douban.com/group/') != (-1)){
		query = "//div[@class='group_banned']";
	}
	return query;
}
