...
// @version	       2012-09-12_01
// @description    Generates MusicBrainz DiscIds from online EAC logs, and check existence in MusicBrainz database.
...
$(document).ready(function(){
    ...
    if (window.location.host.match(/(what\.cd|lztr\.us)/)) {
        gazellePageHandler();
...
function gazellePageHandler() {
	...
	var texts = $("#content div.thin h2 span");
    if (texts.length == 0) { texts = $("#content div.thin h2") };
	...
	$('tr.group_torrent').filter(function() { return $(this).attr("id"); }).each(function() {
		...
		$(torrentInfo).find('a')
			...
			.filter( function(index) { return $(this).attr("href").match(/action=viewlog/) || $(this).text() == "(View Log)"; })
			.each(function() {
