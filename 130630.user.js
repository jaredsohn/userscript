// ==UserScript==
// @name			4chan Expand Images on Click
// @namespace	4chanExpandImagesOnClick
...
// @version			2.2.4
// @updateURL		      http://userscripts.org/scripts/source/130630.user.js
	...
	function setSize(imgObj, state) {
		...
		if (state === 0) {
			...
			if (imgObj.resizedEl) {
				...
				if (result) {
					imgObj.resizedEl.setAttribute("src", "http://images.4chan.org/"+result[2]+"/src/"+result[3]);
				} else {
	...
	function initExpandImages(container) {
		...
		for (i = 0; i < img.length; i += 1) {
			...
			if (parentNode.nodeName.toLowerCase() === "a" &&
				(parentNode.href.indexOf("images.4chan.org") !== -1 || parentNode.href.indexOf(".foolz.us/") !== -1 || parentNode.href.indexOf("/image_redirect/") !== -1 ) &&
