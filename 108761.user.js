...
// @description   forumwarz automatic blackjack script FIXED FOR NEWER FIREFOXES
...
function Start(chat) {
	...
	if(chat.innerHTML.indexOf("You can play") != -1) {
		...
		flezz = parseInt(chat.getElementsByTagName("span")[0].innerHTML.replace(/K/, "000").replace(/[^0-9]/g, ""));
...
function takeChoice(text) {
	...
	for(Choice = 0; Choice<Choices.length; Choice++) {
		...
		if(Choices[Choice].innerHTML.indexOf(text) != -1) {

			try {

				var fireOnThis = Choices[Choice].getElementsByTagName("a")[0];
	...
	while(pos < items.length) {
		...
		if(items[pos].innerHTML.indexOf("Your Hand") != -1) {
			...
			for(card = 0; card<cards.length; card++) {
				...
				var url = cards[card].getAttribute("src");
...
function Finish(chat) {
	...
	for(div = 0; div<divs.length; div++) {
		...
		if(divs[div].innerHTML.indexOf("You have") != -1) {

			flezz = parseInt(divs[div].getElementsByTagName("span")[0].innerHTML.replace(/K/, "000").replace(/[^0-9]/g, ""));
