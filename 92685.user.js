// ==UserScript==
// @name             KOC Attack - Extra Features Español!
// @version          2.1
// @namespace        KOCAttack-Extra Español
// @description      Script de niknah. Publicado y mejorado por Thomas Chapin. Traducido por Pedro Vico.
...
var KOCAttack={
	...
	GetSeed:function() {
		...
		citysel += '<option value=All>Todas</option>';
		...
		levelsel += '<option value="All">Todos</option>';
		for(var c=1; c<=11; c++) {
			levelsel += '<option value="'+c+'">Nivel '+c+'</option>';
		}
		levelsel += '</select>';
		...
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Búsqueda para: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Todo</option>\
		<OPTION value=Camp>Campamentos Bárbaros</option>\
		<OPTION value=Wilderness>Tierras Vírgenes</option>\
		<OPTION value=Grassland>Praderas</option>\
		...
		<td class=xtab width=100 align=center>Selecciona Ciudad: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Selecciona Nivel: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>Cancelar</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Listar Ataques</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Limpiar Lista</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Borrar Seleccionados</span></a>\
		<a id="KOCAttackViewAttacksIgnore" class="button20"><span>Ignorar/DesIgnorar</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<thead><tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Ciudad</td><td>Coordes</td><td nowrap>Tipo</td>';
		  h += '<td>Tropas en ataque</td><td>Ola de Suicidios/Recursos</td><td>Tiempo</td><td>Ignorar</td></tr></thead>';
		  var tableRows= '';
		  var count = 1
		  for(var a=0; a<t.attacks.length; a++) {
			...
			tableRows += '<td nowrap>'+type+' Nivel '+levelInfo.level+'</td>';
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Orden de Ataque: <select id='KOCAttackOrder'><option value='mostTroops'>Prioridad tropas mas numerosas</option><option value='closest'>Prioridad objetivos cercanos </option></select><br />"+
			"Tipo de Ataque: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Bárbaros "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Tierras Vírgenes "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Ciudades "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transporte<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> segundos entre ataques<br />"+
			"Tiempo entre envío al <u>mismo objetivo</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Atacando bárbaros:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />hrs<br />"+
			"Atacando tierras:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />hrs<br />"+
			" Atacando ciudades:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />hrs<br />"+
			" Transportando:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' /> minutos"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distancia máxima de la ciudad a atacar.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Sólo enviar los ataques desde la ciudad que se mandó por primera vez.<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Reintentar la marcha si da error desconocido/exceso de trafico (refresca con F5 después de cambiar esta opción).<br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Paro automático de ataques al recibir alerta de ataque.<span style='color:red'><br /> (No reinicia los ataques después. Utilizalo bajo tu propio riesgo))</span><br />"+
			"Abrir esta url en una pestaña cuando estemos bajo ataque...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Eliminar automáticamente informes de ataque a bárbaros /tierras.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> informes de ataque como máximo en el cuadro de ataques.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Desactivar la visualización de informes.Esto también desactivará la recogida de informes en el cuadro de ataques.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de la población inactiva antes de hacer entrenamiento automático.<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de felicidad antes que recaudemos Oro automáticamente.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> Deshabilitar la mosca cojonera emergente \"Invitar Amigos\" del juego.<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Ayudar automáticamente  a miembros de la alianza en la construcción/investigación.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Ocultar solicitudes de ayuda/informes en el chat.<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Publicar solicitudes de ayuda en el muro automáticamente para acelerar construcciones o investigaciones.<br />"+
			"Si lo anterior se confirma, qué configuración de privacidad debemos usar? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Todo el mundo</option><option value='50'>Amigos de Amigos</option><option value='40'>Solo Amigos</option><option value='10'>Solo Yo</option></select><br />"+
			"</td></tr>"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Automáticamente volver a iniciar sesión en el dominio si se desconecta debido a  mantenimiento o por servidor inactivo.<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Habilitar el diagnóstico de registro para error de Firefox en la ventana de la consola de mensajes (útil si intenta depurar un problema o si está presentando los detalles junto con un informe de error).<br />"+
			"</td></tr>"+
			...
			"Cycle thru all the cities <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> vesces y esperar "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />segundos antes de  refrescar.<br />"+
			"Cities to attack from: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% de oscilacion al azar de todos los retardos (para parecer más humano).<br />"+
			"Importar/Exportar: PPegar o copiar la configuración aquí...<br /><textarea id='KOCAttackImport'></textarea>"+
			...
			"<TABLE width=100%><TR><TD><a id='KOCAttackOptionsSave' class='button20'><span>Guardar</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Cancelar</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Reiniciar opciones</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Reiniciar todo!</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Borrar ataques almacenados</span></a></td><TD align=right>"+ KOCAversion +"</td></tr></form>";
		var t=this;
		...
		ById('KOCAttackOptionsSave').addEventListener('click',function() {
			...
			if(prev_disableInviteFriends != t.options.disableInviteFriends){
				alert("Has cambiado la opción de deshabilitar / habilitar \ Invitar amigos \ nPor favor, ten en cuenta que tendrás que volver a cargar la ventana del juego  para que la nueva configuración tenga efecto!!");
			}
	...
	*/			
	...
	SetAttackStatusMessage:function() {
		...
		if(!toggle) {
			var t=this;
			toggle=this.AddTabLink('Auto Ataque');
			if(!toggle) {
				window.setTimeout(function() {
					t.SetAttackStatusMessage();
				},t.GetRandTime(250));
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			...
			toggle.innerHTML='<span>Auto Ataque - Off</span>';
		}
	},
	...
	*/	
		...
		for(var r=0; r<coordRows.length; r++) {
			...
			if(attack && !force) {
				if(SuicideAttackDefined && isSuicideWave==true){
					this.Log('Ola de suicidio para '+x+','+y+' ya fué añadida. Pruebe sobreescribirla.');
					continue;
				}else if(attack.troops){
					this.Log('Ola de ataque para '+x+','+y+' hya fué añadida. Pruebe sobreescribirla.');
					continue;
				}
			}
			...
			if(attack) {
				...
				this.SetAttack(x,y,attack);
				tthis.Log(x+','+y+' Ataque añadido: '+inspect(attack));
				this.Log("Actual ola de ataque tipo: "+attack.currenttattackwavetype);
				added++;
			}
		}
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"Copia y pega aquí las coordenadas (ej. 343,434) una por linea...<br />Nota: sólo añadir destinos para el número actual de tropas selccionadas en pantalla.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Overwrite existing attack if one already exists<br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Tipo de Ubicaciones (Todas las coordenadas deben ser para este mismo tipo):<br />");
		var arrData = [["Camp", "Bárbaros"],["Wilderness", "Tierras Vírgenes"]];
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert('Añadidas '+added+' cordenadas');
		},false);
		AddHtml(bulkAddDiv,"<br />");
    ...
    return;
	  ...
	  a.innerHTML = 'Mostrar/Ocultar Aceleradores';   
	  //inp.type='checkbox';
	  span.appendChild (a);
...
function StartAll() {
	...
	if(mixpanelRemoved || startAllTimeout<now) {
		if(startAllTimeout<now) {
			GM_log("No quite mixpanel, arrancara de todos modos");
		}
