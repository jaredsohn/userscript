...
// @description    Funciones automaticas para Kingdoms of Camelot.
// ==/UserScript==
...
Tabs.tower = {
  ...
  towerMarches: [],
  Providers : {
		0: { 'country': "País:", 'provider': "--Provider--" },
...
unsafeWindow.CraftingItem = t.CraftingItem;
    ...
    var m = '<DIV class=pbStat>ALERTA DE LA TORRE</div><TABLE class=pbTab><TR align=center>';
    ...
    m += '</tr></table><BR><DIV><CENTER><INPUT id=pbSoundStop type=submit value="Parar Sonido de la Alerta "></center></div><DIV id=pbSwfPlayer></div>';
    ...
    <td align=left>Mensaje de ataque entrante: <INPUT id=pbnum1 type=text size=4 maxlength=4 value="'+ Options.celltext.num1 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum2 type=text size=3 maxlength=3 value="'+ Options.celltext.num2 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum3 type=text size=4 maxlength=4 value="'+ Options.celltext.num3 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\> <span style="color:#800; font-weight:bold">(Los mensajes son estándar a las tarifas)</span></td></tr><tr><td></td>\
    <TD align=left>País: <select id="pbfrmcountry">';
 ...
 if(t.Providers[i].country == t.Providers[Options.celltext.provider].country)
    ...
    m += '</select></td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Postear alertas ataques automaticos al chat de la alianza.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Mensaje de Alerta: &nbsp; </td><TD><INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD align=right>Alerta en exploración: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Alerta en ataque a tierra vírgen: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Mostrar estado de defensa: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
        ...
        <TR><TD align=right><INPUT id=pbalertraid type=checkbox '+ (Options.alertConfig.raid?'CHECKED':'') +'/></td><TD>Detener los RAIDS inmediatamente</td></tr>\
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Reproducir sonido en caso de ataque inmediatamente:</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>Loading SWF player</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            ...
            <TR><TD></td><TD>Sonar por <INPUT id=pbSoundLength type=text size=3 maxlength=5 value="'+ Options.alertSound.playLength +'"> segundos</td></tr>\
            <TR><TD></td><TD><INPUT type=submit value="Hacer sonar ahora" id=pbPlayNow></td></tr></table></div></td></tr>\
        </table><BR>';
  	t.myDiv.innerHTML = m;
...
Tabs.build = {
    ...
    init: function(div){
		...
		m += '<TD><INPUT id=pbBuildMode type=submit value="Construir Modo = OFF"></td>';
		m += '<TD>Tipo de construcción: <SELECT id="pbBuildType">\
				<OPTION value=build>1 nivel</option>\
				<OPTION value=max>nivel máx.</option>\
				<OPTION value=destruct>destruir</option>\
				</select></td>';
		m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>Preguntar por ayuda?</td>';
		m += '</tr></table></div>';
		...
		m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><INPUT id=pbbuild_' + Cities.cities[i].id + ' type=submit value="Mostrar"></center></td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    m = '<DIV class=ptentry><TABLE width=100% class=pbTab><TR><TD class=pbDetLeft>Buscar por: </td><TD width=99%>';
    m += htmlSelector ({0:"Campamento Barbaro", 1:"Tierra Vírgen", 2:"Ciudades"}, null, 'id=pasrcType');
	m += '&nbsp; &nbsp; &nbsp; <span class=pbDetLeft>Estilo de buscar: &nbsp;';
	m += htmlSelector({square:"Cuadrado", circle:"Circulo"}, Options.srcdisttype, 'id=pbsrcdist');
    m += '</span></td></tr><TR><TD class=pbDetLeft>En: </td><TD class=xtab>X=<INPUT id=pasrchX type=text\> &nbsp;Y=<INPUT id=pasrchY type=text\>\
      ...
      <TR><TD class=pbDetLeft>Or:</td><TD>Buscar entre provincias: <select id="provinceXY"><option>--provincias--</option>';
  ...
  clickedSearch : function (){
    ...
    m = '<CENTER><B>Buscar por: '+ typeName +'<BR>\
        ...
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>OPCIONES DE LISTA:</b><BR></td></tr>';
        
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. nivel para buscar:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Máx. nivel para mostrar:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Tipo de Tierra Vírgen</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'Pradera/Lago', 3:'Bosques', 4:'Colinas', 5:'Montañas', 6:'Planicies', 8:'Bosques Oscuros', 0:'TODO'}, Options.wildType );
	  m+= '</select></td></tr>';
      ...
      m += '</select></td></tr><TR><TD class=xtab align=right>Sin dueño solo:</td><TD class=xtab><INPUT id=pafilUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Ordenar por:</td><TD class=xtab><SELECT id=pafilSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
			...
			<TR><TD class=xtab align=right>Coordenadas solo:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
		...
		m+= '<TR><TD class=xtab align=right>Hostil:</td><TD class=xtab><INPUT name=pbfil id=pafilHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Amigable</td><TD class=xtab><INPUT name=pbfil id=pafilFriendly type=CHECKBOX '+ (Options.friendlyOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Aliado:</td><TD class=xtab><INPUT name=pbfil id=pafilAllied type=CHECKBOX '+ (Options.alliedOnly?' CHECKED':'') +'\><td></tr>';
		...
		m+= '<TR><TD class=xtab align=right>Sin Alianza:</td><TD class=xtab><INPUT name=pbfil id=pafilunAllied type=CHECKBOX '+ (Options.unalliedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Todo:</td><TD class=xtab><INPUT name=pbfil id=pafilAll type=CHECKBOX '+ (Options.srcAll?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Ordenar por:</td><TD class=xtab><SELECT id=pafilSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Poderío</option>\
             <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
        </select></td></tr>\
		<TR><TD class=xtab align=right>Min. poderío:</td><TD class=xtab><INPUT type=text id=paminmight size=6 value='+ Options.minmight +'>\
        <TR><TD class=xtab align=right>Coordenadas solo:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
        </table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
  ...
  dispMapTable : function (){
    ...
    if (dat.length == 0){
      m = '<BR><CENTER>No encontrado</center>';
    } else {
      dat.sort(mySort);
      if (DEBUG_SEARCH) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Localización</td></tr>';
      else {
      if (t.opt.searchType == 2) {
			 m = '<TABLE id=pasrcOutTab class=pbSrchResults cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Loc</td><TD align=right>Dist</td><TD>Jugador</td><TD align=right>Poderío</td><TD>Alianza</td><TD>Conectado</td><TD></td></tr>';
		} else {
			m = '<TABLE id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nvl</td><TD width=100px> &nbsp; Tipo</td><TD></td><TD>Exportar al RAID</td></tr>';
		}
	}
...
// TODO: save memory, remove city name ?   			
    ...
    document.getElementById('pastatSearched').innerHTML = 'Buscados: '+ t.tilesSearched;
    t.dispMapTable();
  ...
  e_clickedShow : function (){    // show all current attack data
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      for (i=0; i<atks.length; i++)
        m += '<TR><TD>Campamento Barbaro '+ atks[i].lvl +'</td><TD>'+ atks[i].x +','+ atks[i].y +'</td><TD> &nbsp; Dist='+ atks[i].dist.toFixed(2) +'</td></tr>';
    }    
...
Tabs.AutoCraft = {
	...
	init: function(div){
        ...
        var m = '<DIV id=pbCraftingDiv class=pbStat>AUTO CREACIÓN - OPCIONES</div><TABLE id=pbcraftingfunc width=100% height=0% class=pbTab><TR><TD width="10%">Intervalo: <input type=text value="'+TrainOptions.CraftIntervallMin+'" size=2  maxlength=2 id=pbCraftIntervall> Minuto(s) <b>(</b><span class=boldRed>Necesita Refrescar</span><b>)</b></td>';
        ...
        m += '<DIV id=pbCraftingList class=pbStat>AUTO CREACIÓN LISTA</div><TABLE id=pbcraftingqueues width=100% height=0% class=pbTabLined><TR>';

        m += "<td colspan=2><center><b>Objetos</b></center></td><td><center><b>Inventar</b></center></td><td><b>Cantidad</b></td></tr>"; 
        ...
        m+="</table><b>Nota:</b> Si has puesto más de un objeto a crear,la creación se hará aleatoriamente. <BR> <b>Importante: Min. 50.000 Roca etérea y refrescar para actualizar el inventario!</b> ";
          m += '<DIV id=pbCraftingStats class=pbStat>ROCA ETÉREA Y TIEMPO DE CREACIONES</div><span id="CraftStat"></span>';
...
Tabs.Reassign = {
  ...
  check:false,
    ...
    init: function(div){
      ...
      var m = '<DIV id=pbReMainDivF class=pbStat>FUNCIÓN AUTOMATICA DE REASIGNAR </div><TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center">';
      ...
      m += '</tr></table></div>';
      m += '<DIV id=pbReassignDivD class=pbStat>AÑADIR RUTAS DE REASIGNACIÓN</div>';
      ...
      m += '<TD width=20px>To City:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptassigncityTo></span></div></td>';
	    ...
	    m += '<TR align="left">';
	    m += '<TD colspan=4>Tiempo para chequear reasignación: <INPUT id=pbreassigninterval type=text size=2 value="'+Options.reassigninterval+'"\> minutos</td></tr>';
	    m += '<TR><TD><INPUT id=autofilloff type=checkbox unchecked=true\> Bloquear el valores de las tropas</TR></TD></table>';
      m += '<DIV style="margin-top:10px;margin-bottom:5px;">Rellene el número de tropas que desea conservar en una ciudad:</div>';
      m += '<TABLE id=pbaddreasignroute width=100% height=0% class=pbTab><TR align="center">';
      
      m += '<TR><TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_50.jpg?6545"></td>';
      m += '<TD>Tropas de Suministros</td>'
      m += '<TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_50.jpg?6545"></td>'
      m += '<TD>Milicianos</td>'
      m += '<TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_3_50.jpg?6545"></td>'
      ...
      m += '<TD>Caballerías</td>'
      m += '<TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_8_50.jpg?6545"></td>'
      m += '<TD>Caballerías Pesadas</td></tr>'
      ...
      m += '<TD>Ballistas</td>'
      m += '<TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_11_50.jpg?6545"></td>'
      m += '<TD>Carretas de Suministros</td>'
      m += '<TD rowspan="2"><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_12_50.jpg?6545"></td>'
      ...
      m += '<DIV style="text-align:center; margin-top:15px"><INPUT id=pbSaveRouteReassign type=submit value="Añadir Ruta"></div>';
...
Tabs.Reinforce = {
  ...
  checkETA:null,
    ...
    init: function(div){
      ...
      var m = '<DIV id=pbReinfMain class=pbStat>REFUERZOS</div><TABLE id=pireinforce width=100% height=0% class=pbTab><TR align="center">';
