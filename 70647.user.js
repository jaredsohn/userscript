...
// @description    Player GameLog Charts ESPN nhl stats
	...
	for(var j = 0; j < allTables.length; j++)
	{
		...
		var chartURL = "http://chart.apis.google.com/chart?";
			...
			for(var i = (allTables[j].rows.length) -2; i >=0; i--)
			{
				...
				if(goals == "")
					{goals = goals+allTables[j].rows[i].cells[3].innerHTML;
					g_accum += parseInt(allTables[j].rows[i].cells[3].innerHTML);
					g_accum_str += g_accum;
					}
				else
					{goals = goals+","+allTables[j].rows[i].cells[3].innerHTML;
				...
				if(assists == "")
					{assists = assists+allTables[j].rows[i].cells[4].innerHTML;
					a_accum += parseInt(allTables[j].rows[i].cells[4].innerHTML);
					a_accum_str += a_accum;
					}
				else
					{assists = assists+","+allTables[j].rows[i].cells[4].innerHTML;
				...
				if(points == "")
					{	
						points = points+allTables[j].rows[i].cells[5].innerHTML;
						p_accum += parseInt(allTables[j].rows[i].cells[5].innerHTML);
						p_accum_str += p_accum;
						if(parseInt(allTables[j].rows[i].cells[5].innerHTML) > maxpoints)
							maxpoints = parseInt(allTables[j].rows[i].cells[5].innerHTML);
					}
				else
					{
						points = points+","+allTables[j].rows[i].cells[5].innerHTML;
