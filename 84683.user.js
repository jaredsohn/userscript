// ==UserScript==
// @name         (ma6qoq) Travian: Attack Builder (3)
...
switch(sLang)
{
	...
	case "ae":
        ...
        case "sy":
        case "eg": ///ma6qoq
text = ["تم الهجوم =)","نوع الهجوم خاطئ!","لاتوجد إحداثيات!","عدد القوات الموجوده غير كافي! القوات رقم  #","لم يتم إختيار القوات","البداية","ربما الإحداثيات خاطئة","لم تختار وحدات كشافه","تحياتي:علي الزهراني","أعدادات القوات:","أضافة قوات هجوم جديدة","مسح القوات","أعدادات الهجوم:","نوع الهجوم:","هجوم: كامل","مساندة","هجوم: للنهب","التجسس على الموارد والقوات","التجسس على التحصينات والقوات","الإحداثيات","هجوم الآن! =)","وقت الوصول","الوصول في:","إختار وقت الوصول","عدد الهجمات","للهجوم على أكثر من قريه ضع هذه بين الإحداثيات',' مثال: 00|1,1|0*الوقت بين الهجمات 400 اي ان كل ثلاث هجمات ستكون بنفس الثانيه حتى لا تحظر من الصياد","خطأ:","تعذر الوصول الى القريه النشطه. من المفترض ان يكون من حساب واحد وقريه واحده","وقت الوصول حدد على:","الوقت بين الهجمات: (الإفتراضي 400) ","VIP ĹεğεņÐ (ma6qoq)"];
...
//	{dank68
	...
	case "hk":
	case "tw":
		text = text = ["兵力調動中","無效的攻擊類型!","未指定攻擊目標!","沒有足夠的軍隊! (軍隊#","沒有設定攻擊軍隊","正在派遣軍隊","錯誤的攻擊目標","沒有偵察軍種","完成派兵","攻擊設定","新增攻擊","還原設定","攻擊設定:","攻擊類型:","正常攻擊","增援","搶奪攻擊","偵察資源/軍隊","偵察防禦/軍隊","攻擊目標","立即派遣軍隊","軍隊到達時間","到達目標於:","設定攻擊到達時間","攻擊的波數","用 ','分隔每一條村 ex: 0|0,1|1","錯誤:","無法取得目標村莊的資訊","軍隊到達時間:","波次延遲(毫秒)",""]

		cataText = ["設定車攻目標","隨機","伐木場","泥坑","鐵礦場","農場","鋸木廠","磚廠","鋼鐵鑄造廠","麵粉廠","麵包店","倉庫","穀倉","鐵匠","盔甲廠","競技場","村莊大樓","集結點","市場","大使館","兵營","馬廄","工場","研究院","村會堂","行宮","皇宮","交易所","大兵營","大馬廄","英雄宅","大倉庫","大穀倉","世界奇觀","寶物庫"];
		break;
...
attackInterface.innerHTML = '<fieldset><legend>'+text[12]+'</legend><div id=start>' +
	...
	'<tr><td colspan="3">' +
		'<select name=\"typeAttack\" id=\"typeAttack\" class=\"dropdown\">' +
	...
	'</tr><tr><td colspan="3">'+
		'<input type=\"text\" value="' +getCords() +  '" name=\"cords\" id=\"cords\" class=\"text\" style=\"width: 98%\" /><br>'+ text[29] +'<input class=\"text\" style=\"color:red;\" id=\"timebetween\" type=\"text\" value=\"400\" size=\"4\" />' +
	'</td>'+
	...
	+text[22]+
	'</td></tr><tr><td colspan="3">'+
		'<input id=\"timedArrivalInput\" value=\"00:00:00" class=\"text\"> <button id=\"timedArrivalButton">'+text[23]+'</button>'+
	'</td></tr></table>';
...
var table = "<legend>"+text[9]+"</legend><table id=\"myTable\"  ><tr></td>التكرار</td>";
...
table += '</tr></table><button id="newWaveButton">'+text[10]+'"<td><img src=\"gpack/travian_default/img/a/att2.gif\" class=\"unit uhero\"></td>"</button> <button id="resetButton">'+text[11]+'</button> <span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:grey;">( 
 ...
 '+text[24]+' )</span><span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:#484006;">'+text[30]+'</span>';
...
function addNewWave() {
	...
	input ="<input  size=\"1%\" maxlengt=\"6\" type=\"text\" name=\"number\" value=\"1\" class=\"text\">";
	col.innerHTML = (input);
	...
	for (var i=1;i<=9;i++){
		...
		{
			col.style.width = '35px';
			input ="<input style=\"width: 90%\" size=\"2\" maxlengt=\"6\" type=\"text\" name=\"troop_" + i + "\" value=\"0\" class=\"text\">";
		}else{
			input ="<input size=\"1\" maxlengt=\"6\" type=\"text\" name=\"troop_" + i + "\" value=\"0\" class=\"text\">";
		}
		col.innerHTML = (input);
		newRow.appendChild(col);
	}
	...
	col = document.createElement('td');
	col.style.width = '35px';
	input ="<input style=\"width: 90%\" size=\"1\" maxlengt=\"6\" type=\"text\" name=\"troop_10\" value=\"0\" class=\"text\">";
	col.innerHTML = (input);
	newRow.appendChild(col);
	...
	//end Hill's edition
	select = '<select id="gm_kata_' + nthWave + '" name="gm_kata_' + nthWave + '" class="dropdown">'+
	...
	col.setAttribute('rowspan',"1");
	//end Hill's edition
	select = '<select id="gm_kata2_' + nthWave + '" name="gm_kata2_' + nthWave + '" class="dropdown">' +
...
function getTroopIndexTitleFromImage(tImg) {
		var tInfo = [0, ""];
		if (tImg.src.match(/c\/un\/u\/(\d+)\.gif/)) {
			tInfo[0] = RegExp.$1;
			tInfo[1] = tImg.title;
		} else {
