...
// Konfigurationsseite
if (adr == "http://www.feuerwache.net/Fillhorn-Config") {
	KonfigSeite();
}
...
function setConfigLink() {
	...
	var text = document.createTextNode("Fillhorn Konfiguration");
	a.setAttribute('href', '/Bettkante-Config');
	a.appendChild(text);
	li.appendChild(a);
...
function UebersichtSeite() {
	...
	if (autoclick_uebersicht) {
		...
		for (var i=0;TR.length > i; i++) {
			...
			if (imgElement && imgElement.src == "http://www.feuerwache.net/images/lightning.png") {
...
function KonfigSeite() {
	...
	ueberschrift.appendChild(document.createTextNode('Konfiguration des Fillhorn-Bot'));
...
{  
  ...
  htmlCode += "> Fillhorn aktivieren";
  
  
  // ~~~~~~~~~~~~~~~~~~~~~~
  // Einsatz Übersichtsseite
  htmlCode += "<br><br><h2>Verhalten auf der Einsatz-Übersichtsseite</h2>\n";
...
function KonfigSeite_checkBoxClicked() {
	var checkBoxes = document.getElementsByName("fillhorn_config");
