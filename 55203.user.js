// ==UserScript==
// @name           HighscoreScript v1.4.5
// @namespace      
// @description    HighscoreScript v1.4.5 by KennyKiller @ PennerProfi.de.vu
// @include        http://highscore.menelgame.pl/highscore/*
...
newth_2.innerHTML = 'Pieni&#261;dze/ <br /> Data rej./ <br /> Promile';
	...
	GM_xmlhttpRequest({
   	...
   	url: 'http://www.menelgame.pl/dev/api/user.' + id + '.xml',
		...
		try
		{
			...
			if (cash >= 150000 && cash <= 500000) {
   newtd.style.color = "#FFFF00";
}
else if (cash >= 500010 && cash <= 1000000) {
   newtd.style.color = "#00FF00";
}
else if (cash >= 1000010 && cash <= 15000000){
   newtd.style.color = "#FF0000";
   newtd.style.fontWeight = "bold";
}
else if (cash >15000010) {
   newtd.style.color = "#FF0000";
   newtd.style.fontWeight = "bolder";
   newtd.style.fontSize = "16px";
   alert ('Miejsce:' + x + name + ' ma ' + ' wi&#281;cej ni&#380; 150 000 z&#322;');
}
			...
			if(cash.length >= 9)
			{
			 newtd.innerHTML = "z&#322; " + cash.substring(0,cash.length-8) + "." + cash.substring(cash.length-8,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + '<br />' + reg + pskript;
			}
			else if (cash.length>=6)
			{
			newtd.innerHTML = "z&#322; " + cash.substring(0,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + '<br />' + reg + pskript;
			}
			else if(cash.length>2)
			{
			newtd.innerHTML = "z&#322; " + cash.substring(0,cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + '<br />' + reg + pskript;
			}
			else if(cash.length==2)
			{
			...
			{
			newtd.innerHTML = "z&#322; 0,0" + cash + '<br />' + reg + pskript;
			}
