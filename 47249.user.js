...
function updateStatus() {
    ...
    // my modificated line:
                         Math.floor(mostAccurateFromTime / 60) + ':' + ((mostAccurateFromTime % 60) > 9 ? (mostAccurateFromTime % 60) : '0' + (mostAccurateFromTime % 60)) + ' '
        		     	)
    // original line:
    // status += 'to ' + (looper.to > 0 ? looper.to + 's ' : 'finish ')
    // my modificated line:
    status += 'to ' + (looper.to > 0 ? Math.floor(looper.to / 60) + ':' + ((looper.to % 60) > 9 ? (looper.to % 60) : '0' + (looper.to % 60)) + ' ' : 'finish ')
