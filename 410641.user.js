// ==UserScript==
// @name           Wall Manager Sidekick (Riddle Stones)
// @description    Assists Wall Manager with Riddle Stones posts
// @include        https?://apps.facebook.com/riddlestone/*
// @include        https?://riddlestone.ooblada.com/*
...
(function() {
	...
	var version = "3.0"; //use something like 1.2.3.4 where 1 is the version, 2 is the major revision, 3 is the minor revision, and 4 is a bug fix
	var appID="486257998121419"; //find your appID on facebook or use google
     var scriptID=""; //the id of your script on userscripts.org
     var appName="Riddle Stones"; //the text name of the app/game you are collecting for
	var appIcon="https://fbcdn-photos-g-a.akamaihd.net/hphotos-ak-prn1/t39.2081/851579_487136498033569_580246304_n.png"; //example:"http://photos-d.ak.fbcdn.net/photos-ak-snc1/v85006/71/100333333405439/app_2_100333333405439_3507.gif"
	...
	function dock(){
		...
		var materials = [
			{name:"scatter solver",event:"Boosts"}, 
			{name:"time freezer",event:"Boosts"}, 
		];
	...
	function read(){
		...
		*************************************************** */
		...
		//over-limits
		else if (text.match(/(cannot have anymore|reached the collection limit|maximum energy has been awarded|reached your limit)/)) statusCode=-3; 
		...
		//find a button to click so we can process this post
		else {
			//look for the first type of button
			var btn = selectSingleNode(".//a[contains(@class,'msg_button msg_back')]");
			if (btn||null){
				//click it
				click(btn);
				//and wait
				return;
				//clicking it should take us to a new address
				//at which time the sidekick will start up again and
				//perform this read function again
			}
			
			//look for the second type of button
			btn = selectSingleNode(".//button[contains(@name,'__CONFIRM__')]");
			if (btn||null) {
				click(btn);
				return;
			}
			
			//check for having been taken to the game, either after bonus collection
			//or after clicking a play link
			
			if (window.location.href.startsWith("https://apps.facebook.com/riddlestone/?referrer=")) {
				//yes we're at the game for one reason or another
