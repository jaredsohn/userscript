// ==UserScript==
// @name        AutoNovaMatriceAgora
// @namespace   InGame
// @include     http://www.dreadcast.net/Forum/1-111-cercle-agora-rebelle*
// @version     1
// @grant       none
// ==/UserScript==
...
function isOnOrOff(node) {      
    if(typeof localStorage!='undefined') {
        var cat = localStorage.getItem("catAgo"+node.id);
        if(cat==null || cat != "on") 
        {
...
function addClickEvent(node) {
   	node.children[0].onclick = function(){
        ...
        if(typeof localStorage!='undefined') {
           ...
           if(cat!=null && cat == "on") 
           {
               localStorage.setItem('catAgo'+node.id,'off');     
               return;
           }
       }
        
       localStorage.setItem('catAgo'+node.id,'on');     
   	};
}
...
function addClickEventAddTag(node) {
       ...
       if(saisie != null)
       {
          ...
          if(typeof localStorage!='undefined') {
              localStorage.setItem('ago'+nomOriginal.toLowerCase(),'['+saisie+']');     
          }
       }
...
function addClickEventSupprTag(node) {
   	node.onclick = function(){
       var nomOriginal = $('#divOriginalNom_'+node.id.substring(node.id.indexOf('_')+1)).text();
       localStorage.removeItem('ago'+nomOriginal.toLowerCase());     
...
//récupération des tags	
	...
	for (var i = 0; i < list.length; ++i) {
        ...
        if(typeof localStorage!='undefined') {
           var tag = localStorage.getItem("ago"+nom.toLowerCase());
           if(tag!=null) {
               nom = tag +" "+nom;
               var t = document.createElement('div');
               t.id = 'tagSuppr_a'+i;
...
(function() {    
    ...
    var auth = localStorage.getItem('majForge_Nova-Ma-trice pour Agora');
