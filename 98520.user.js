...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
	...
	if(t.postToChatOptions.includemight)
	  who += ' -- MIGHT: '+ addCommas(attackermight);
    if (m.fromXCoord)
    ...
    msg += 'My '+ target +' is being '+ atkType  +' by '+ who +' ' +alliance+' -- ETA: '+
        unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +' -- INCOMING: ';
    var totTroops = 0;
    for (k in m.unts){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += '\nMy embassy has '+ availSlots +' of '+ emb.maxLevel +' slots available.';
      }
    }
