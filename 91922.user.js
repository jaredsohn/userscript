...
/*** 20100412a
*) Player Search now has both leaderboard info and normal details
*) Change You can Train with 1x speed, to use the Time bonus from the Marshal Knight
*) Change Speed to just normal Trainer
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  readOptions();
  AddMainTabLink('POWER', eventHideShow);
  setCities();
...
my.AllianceList = {
  ...
  init : function (){
    ...
    unsafeWindow.PTgotoMap = t.gotoMap;
    unsafeWindow.PTpd = t.clickedPlayerDetail;
	unsafeWindow.PTpd2 = t.clickedPlayerDetail2;
    return t.cont;
  },
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Showing players matching <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:520px; max-height:520px; overflow-y:auto">\
      <TABLE cellpadding=0><TR style="font-weight:bold"><TD class=xtab>Name</td><TD class=xtab width=20></td>\
      <TD align=right class=xtab>Might</td><TD class=xtab>Facebook &nbsp; </td><TD class=xtab>Details &nbsp; &nbsp; </td></tr>';
    for (k in rslt.matchedUsers){
      var u = rslt.matchedUsers[k];
      m += '<TR valign=top><TD>'+ u.genderAndName +'</td><TD></td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          <TD align=center><A target="_tab" href="http://www.facebook.com/profile.php?id='+ u.fbuid +'">profile</a></td>\
          <TD><DIV onclick="PTpd(this, '+ u.userId +')"><A>More Detials</a></div><DIV onclick="PTpd2(this, '+ u.userId +')"><A>Leaderboard</a></div></td></tr>';
    }
    m += '</table></div>';
...
//    unsafeWindow.track_chrome_btn('Map_Flag');
  },
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class=xxtab>'+ t.dat[i][3] +'</td>\
            ...
            <TD align=center class=xxtab><DIV onclick="PTgotoMap(this)"><A>'+ t.dat[i][5] +','+ t.dat[i][6] +'</a></div></td>\
...
//new CtableToText('tabAllMembers').toText();     
  ...
  fetchPlayerList : function (name, notify){  // at least 3 chars!!     UNTESTED
	...
	pdDiv : null,  
  clickedPlayerDetail : function (div, uid){
    var t = my.AllianceList;
    div.innerHTML = "fetching details ..."; 
    t.pdDiv = div;
    t.fetchPlayerInfo (uid, t.gotPlayerDetail);
	
  },
  clickedPlayerDetail2 : function (div, uid){
    var t = my.AllianceList;
    div.innerHTML = "fetching details ..."; 
    t.pdDiv = div;   
	t.fetchPlayerLeaderboard (uid, t.gotPlayerLBDetail);
  },
   
  gotPlayerDetail : function (rslt){
    var t = my.AllianceList;
    if (!rslt.ok){
      t.pdDiv.innerHTML = rslt.errorMsg;
      return;
    }
    ...
    var m = '<B>Alliance:</b> '+ a +' &nbsp; <B>Cities:</b> '+ u.cities +' &nbsp; <B>Population:</b> '+ u.population +'<BR><B>Provinces:</b> ';
    t.pdDiv.onclick = "";
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
      p.push(unsafeWindow.provincenames['p'+pids[i]]);
    t.pdDiv.innerHTML = m + p.join (', ');;   
  },
  ...
  gotPlayerLBDetail : function (rslt){
    var t = my.AllianceList;
    if (!rslt.ok){
      t.pdDiv.innerHTML = rslt.errorMsg;
      return;
    ...
    var u = rslt.results[0];
    var a = 'None';
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<B>Alliance:</b> '+ a +' <B>Cities:</b><br> ';
    t.pdDiv.onclick = "";
	var cis = [];
	
	for (var i=0; i<u.cities.length; i++)
	{
	  var c = u.cities[i];
	  cis.push('<DIV onclick="PTgotoMap(this)">' + c.cityName +' (<A>'+ c.xCoord +','+ c.yCoord +'</a>) @ '+ unsafeWindow.provincenames['p'+ c.tileProvinceId] + '</div>');
	}
	
    t.pdDiv.innerHTML = m + cis.join ('<br>');
  },
};  
...
my.Test = {
  ...
  show : function (){
    ...
    if (t.state == null){
      var m = '<TABLE><TR><TD align=right>Scout: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
          ...
          <BR><INPUT id="subGifts" type=submit value="Gifts" \>\
          <BR>X: <INPUT id=testXbox  type=text /> Y: <INPUT id=testYbox type=text /> Or, choose city: <SPAN id=testcsel></span>\
           &nbsp; &nbsp; &nbsp; <SPAN id=testNotify></span>\
          <BR><DIV id=testDiv style="background-color:#ffffff; maxwidth:675; maxheight:400px; height:400px; overflow-y:auto;"></div>';
      t.cont.innerHTML = m;
      document.getElementById('testSendMarch').addEventListener ('click', t.clickFakeAttack, false);
      document.getElementById('subGifts').addEventListener ('click', t.clickGifts, false);
      new CdispCityPicker ('testdcp', document.getElementById('testcsel'), true, xyNotify, 0).bindToXYboxes (document.getElementById('testXbox'), document.getElementById('testYbox'));
      t.state = 1; 
    }
  },
  ...
  clickGifts : function (){
    var t = my.Test;
    document.getElementById('testDiv').innerHTML = 'fetching facebook gifts page ...';
    t.fetchFBreqs(t.gotGiftsPage);
  },
  
  gotGiftsPage : function (rslt){
    var t = my.Test;
//logit (inspect (rslt, 8, 1));    
    var gifts = t.parseGiftsPage(rslt.responseText);  
//logit (inspect (gifts, 8, 1));

    var m = '<TABLE>';
    for (var i=0; i<gifts.length; i++){
      m += '<TR><TD>'+ gifts[i].gift +'</td><TD>'+ gifts[i].name +'</td></tr>'
    }
    m += '</table>';
    document.getElementById('testDiv').innerHTML = m;
  },  
    
  parseGiftsPage : function (p){
    var t = my.Test;
    var m = findAllBetween (p, '<div id="app_130402594779_0_', 'actions[reject');
    var ret = [];
logit (p);    
    for (var i=0; i<m.length; i++){
      mm = m[i].match( /profile.php\?id=[0-9]*">(.*)<\/a><\/strong>.*(?:a gift of (.*)in <b>|here is a(.*)you can use).*Accept and play.*actions\[(.*)\]/im );    
logit (inspect (mm, 8, 1));      
      if (mm != null){
        ret.push ({name:mm[1], url:mm[4], gift:mm[2]?mm[2]:mm[3] });
      }
    }
    return ret;
  },
  ...
  fetchFBreqs : function (notify){
    GM_xmlhttpRequest({
      url : 'http://www.facebook.com/reqs.php#confirm_130402594779_0',
      method: 'get',
      onload: function (r){
        notify (r);
      }, 
  ...
  createFakeAttack : function (cityNum, isScout, isWild, secs, numMilitia){
    ...
    march.departureTime = unixTime() - 20; 
    march.unts = {}
    march.unts.u3 = 1
...
// add a false report too:
//Seed.queue_atkinc.m77777 = {};      
  },
  ...
  clickFakeAttack : function (){
    ...
    t.createFakeAttack (0, isScout, isWild, secs, mil);
  },  
}
  ...
  show : function (){
    ...
    if (t.state == null){
      m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
          ...
          <DIV class=ptstat>UNIT INFORMATION</div><TABLE align=center cellpadding=1 cellspacing=0>\
      ...
      m += '<BR><DIV class=ptstat>DISTANCE CALCULATOR</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
        ...
        <TR><TD class=xtab><B>Second Location: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> Or, choose city: <SPAN id=ptloc2></span></td></tr></table></div>\
  ...
  show : function (){
    ...
    if (t.state != 1){
      m = '<BR><TABLE>\
        ...
        <TR><TD class=xtab><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD class=xtab>Automatically post incoming attacks to alliance chat.</td></tr>\
        <TR><TD class=xtab></td><TD class=xtab><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD class=xtab align=right>Message Prefix: &nbsp; </td><TD class=xtab><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD class=xtab align=right>Alert on scouting: &nbsp; </td><TD class=xtab><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD class=xtab align=right>Alert on wild attack: &nbsp; </td><TD class=xtab><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD class=xtab align=right>Minimum # of troops: &nbsp; </td><TD class=xtab><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
        </table></td></tr>\
        <TR><TD class=xtab><INPUT id=togAllRpts type=checkbox /></td><TD class=xtab>Enable enhanced Alliance Reports.</td></tr>\
        <TR><TD class=xtab><INPUT id=togAllowAlter type=checkbox /></td><TD class=xtab>Allow other scripts to change format of Alliance Reports.</td></tr>\
        <TR><TD class=xtab><INPUT id=togEnhanceMsging type=checkbox /></td><TD class=xtab>Enable enhanced messaging ("forward" and "all officers" buttons).</td></tr>\
        <TR><TD class=xtab colspan=2><BR><BR><B>Kingdoms of Camelot BUG FIXES:</b></td></tr>\
...
\":100,\"autoGoldHappiness\":99,\"okCities\":[1,true,true,true,false,false,false,false,false,1],\"impendingAttackUrl\":\"\",\"attackTypeCity\":false,\"attackTypeWild\":false,\"attackTypeTransport\":false,\"lockAttackFromCity\":false}","attack_150_38,494":"{\"type\":0,\"comment\":\"\",\"time\":1288419693.014,\"troops\":[null,0,0,0,0,0,16666,0,0,0,0,0,0]}"}';
...
my.Search = {
  ...
  gotPlayerLBDetail : function (rslt){
	...
	{
	  var c = u.cities[i];
	  cis.push('<span onclick="PTgotoMap(this)">' + c.cityName +' (<A>'+ c.xCoord +','+ c.yCoord +'</a>) @ '+ unsafeWindow.provincenames['p'+ c.tileProvinceId] + '</span> ');
	}
	
    t.pdDiv.innerHTML = m + cis.join (' | ');
  },
  getContent : function (){
    var t = my.Search;
    return t.cont;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
		...
		if (t.opt.searchType == 2) {
			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Loc</td><TD >Dist</td><TD style="">City</td><TD width=80% tyle="font-size:9px;">More info</td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
      ...
      for (i=0; i<numRows; i++){
        ...
        if (coordsOnly)
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { //onclick="modal_attack(3,456,498);return false;"
				 m += '<TD align=right>'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD align=right>'+ dat[i][8] +'</td><TD>';
				if (dat[i][5]) {
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    for (k in map){
      ...
      if (dist <= t.opt.maxDistance){
		if (t.opt.searchType==2) {
		isOwned = map[k].tileUserId == 0 || false;		
        t.mapDat.push ([map[k].xCoord, map[k].yCoord, dist, type, map[k].tileLevel, isOwned, map[k].tileCityId, map[k].tileUserId, map[k].cityName, map[k].misted ]);
  ...
  show : function (){
    ...
    if (t.state == null){
      s = "<DIV id=trainTopSelect style='height: 148px;'>\
        <DIV class=ptstat>This screen takes advantage of a bug in KofC that lets you train FASTer!</div>\
      ...
      s += "</select></td>\
        </tr><TR><TD class=xtab align=right>Troop Type:</td><TD class=xtab></td><TD class=xtab>\
        ...
        <TR><TD class=xtab align=right># to train:</td><TD class=xtab></td>\
...
/**/
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE width=100%><TR align=center>\
      ...
      <TD class=xtab width=16%><B>Ore:</b></td>\
      <TD class=xtab width=16%><B>Gold:</b></td>\
      <TD class=xtab width=16%><B>Idle Pop:</b></td></tr>\
      <TR align=center><TD class=xtab>'+ addCommasInt(t.stats.food) +'</td>\
      <TD class=xtab>'+ addCommasInt(t.stats.wood) +'</td>\
      <TD class=xtab>'+ addCommasInt(t.stats.stone) +'</td>\
      <TD class=xtab>'+ addCommasInt(t.stats.ore) +'</td>\
      <TD class=xtab>'+ addCommasInt(t.stats.gold) +'</td>\
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<HR><B>Done queueing troops.</b> &nbsp; &nbsp; <HR>');
        document.getElementById('trainDo').disabled = false;
        return;
      }
      if (cmd[0] == 'T'){
...
// TODO: food +/- ??
    ...
    str += 'Alliance: ' + getMyAlliance()[1] +' &nbsp; Domain: '+ unsafeWindow.domainName +'<BR>';
