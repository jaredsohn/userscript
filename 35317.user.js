// coding: utf-8
// ==UserScript==
// @version 	   Turkish Version 1.0
// @name           ikariam alarm and overview table (Türkçe verisyon) (IK v.0.2.8 için)
// @namespace      K0R54N
// @description    
...
function defineLanguage(langTDL) {
    switch (langTDL) {
        case "tr":
            language = { inline:"Skor Tablosu",
            fetch:"Yükleniyor...",
            score:"Toplam Puan",
            military:"Askerı Puan",
            gold:"Altın Puanı" }
            break;
        default:
            language = { inline:"Skor Tablosu",
...
var unitsAndShipsIndexes = {
  ...
  "unit Şahmerdan_Gemisi" : 13,
  "unit Okçu_Gemisi" : 14,
  "unit Alev_Atıcı" : 15,
  "unit Mancılık_Gemisi" : 16,
  "unit Havantopu_Gemisi" : 17,
  "unit Buharlı_Şahmerdan" : 18,
  "unit Denizaltı" : 19,
};
...
var unitScoreBasePoints = {"odun": 2, "şarap": 4, "kristal": 4, "sülfür": 4};
...
function getLocalizedTexts() {
  if (language == "tr") {
    buildings = {
      "townHall"      : ["Belediye Binası", "Belediye Binası"],
    ...
    texts = {
      "cityName": "Şehir Adı", "currentlyBuilding": "Şu anda Kurulan Bina", "summary": "Özet:",
      "hide_settings": "Ayarları Gizle", "show_settings": "Ayarları Göster", "ana_kaynak": "Ana Kaynak",
    };
  } else{
    buildings = {
      "townHall"      : ["Belediye Binası", "Belediye Binası"],
...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>"+"1 satte +" + floatFormat(remhour, 1) + " artış.";
    } else if (factPerHour < 0) {
      remaining = "<br>"+floatFormat(curres / -factPerHour, 1) + " saat sonra depo boşalacak.";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + " toplam kapasite<br>" + perc+"% dolu" + remaining)+">"+
...
function createProd(prodPerHour, extraTooltip) {
  ...
  var tooltip = mynumberformat(Math.round(24 * prodPerHour), true)+" / 1 günde";
...
function getGrowthRemainingHours(population, maxPopulation, startTime, happiness) {
  ...
  return floatFormat("Satte " + (time - Number(startTime)) / 1000 / 3600, 1) + " artış.";
}
...
function createLastUpdateAsTooltip(content, time) {
  return createTooltip(content, "son güncellenme: "+smartDateFormat(time, true));
}
...
function renderTables() {
  ...
  if (TABLE_RESOURCES) {
    var woodName =   getNodeTitle("//div[@id='cityResources']/ul[@class='resources']/li[@class='wood']", "Odun");
    var wineName =   getNodeTitle("//div[@id='cityResources']/ul[@class='resources']/li[@class='wine']", "Şarap");
    var marbleName = getNodeTitle("//div[@id='cityResources']/ul[@class='resources']/li[@class='marble']", "Mermer");
    var glassName =  getNodeTitle("//div[@id='cityResources']/ul[@class='resources']/li[@class='glass']", "Kristal");
    var sulfurName = getNodeTitle("//div[@id='cityResources']/ul[@class='resources']/li[@class='sulfur']", "Sülfür");
    var populationName = getNodeTitle("//div[@id='cityResources']//li[@class='population']", "Halk");
...
//        growthRemainingHours = floatFormat((parseInt(spacetotal) + parseInt(bonusspace) - parseInt(population)) / (0.02 * happiness), 1) + " h";
  ...
  if (true) {
    function reset_all_data() {
      var answer = confirm("Tüm değişiklikler sıfırlanacak. Emin misiniz?");
      if (answer) {
        setVar("config", "");
        setVar("players", "");
    ...
    t.appendChild(createRowChk("Uyarı:", "ALERT_SOUNDS", ALERT_SOUNDS));
    t.appendChild(createRowInput("Uyarı Sesi (0 - 100):", "ALERT_VOLUME", ALERT_VOLUME));
    t.appendChild(createRowInput("Tehlike Sesi (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("Otomatik Yenileneme:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("Otomatik Yenileme Süresi (En Az):", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("Otomatik Yenileme Süresi (En fazla):", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("Kaynak Tablosunu Göster:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("İnşa Tablosunu Göster:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("Ordu Tablosunu Göster:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("Oyuncu ve Şehir Tablosunu Göster:", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("Kaynak Geşimi Bar Modu:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "Kapalı", time: "Kalan zamana göre", percent: "Her Zaman"}));
    t.appendChild(createRowSlct("Dil:", "LANGUAGE", language, {"": "Sorry. Only Turkish. Closed Other Language.", tr: "Türkçe"}));
    t.appendChild(createRowInput("İttifak (kısa ismi):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("Dost ittifaklar (kısa ismi):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("Düşman ittifaklar (kısa ismi):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("CSS (Renk Stili. Kodlardan anlamıyorsanız ellemeyin.):", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("Mesajları kaydet:", "DEBUG_LOG", DEBUG_LOG));
...
if (getCfgValue("TABLE_PLAYERS", false) == true) {
  var time = new Date().getTime();
  setVar("oyuncular", uneval(players));
  log("time uneval: "+(new Date().getTime() - time)+" msec");
}

var _endTime = new Date().getTime();
log("toplam zaman: "+(_endTime - _startTime)+" msec");
