...
// @description  Modded by Borch - Dragons of Atl.
// ==/UserScript== 440,820 medidas para ventana default 420,800

var Version = '20110622a';
var Title = 'DOA Modded by Borch';
var WebSite = 'userscripts.org/105182';
var VERSION_CHECK_HOURS = 1;
var DEBUG_TRACE_AJAX = 2;
var DEBUG_MARCHES = false;
var MAP_DELAY = 1250;
var MIN_CAMP_DELAY = 15;
var EMULATE_NET_ERROR = 0;  // percentage
var ENABLE_DEBUG_TAB = true;
...
var translateSPArray = {
	...
	'Quarry':'Cantera',
	'TrainingCamp':'Camp. Entrenamiento',
	'Silo':'Silo',
	'Metalsmith':'Fragua',
	'ScienceCenter':'C. Investgacion',
	'Rookery':'Criadero',
	'Theater':'Teatro',
	'Factory':'Factoria',
	'level':'nivel',
	...
	'ArmoredTransport':'Trans.Aereo',
	'Giant':'Gigante',
	'FireMirror':'Esp.Fuego',
	'StoneTroop':'Ogr.Granito',
	'GreatDragon':'Drag.Gran',
	'WaterDragon':'Drag.Agua',
	'StoneDragon':'Drag.Piedra',
	'AquaTroop':'Tritones',
	//new adds
	...
	'Auto-attack Camp Levels':'Auto-Ataque a Campamentos por Niveles',
	'Auto-attack Configuration':'Configuracion Auto-Ataque',
	'Enable':'Habilitar',
	'Random delay between attacks':'Retraso entre ataques',
	'Same target delay':'Retraso entre mismo objetivo',
	...
	'Auto-collect resources at outpost every 8 hours':'Auto-Recolecta de recursos en puesto avanzado <BR>cada 5 minutos',
	'Attack':'Atacar',
	'Config':'Configuracion',
	...
	'DOA Power Tools Options':'Opciones DoA',
	...
	'Hop':'Salto',
	'StoneTroopItem':'Raiz Mandragoras',
	'Loaded':'Cargado',
	'per hour':'por hora',
	'Levels':'Niveles',
	...
	'Building level':'Nivel de Edificio',
	'Researching level':'Nivel de Investigacion',
	'of':'de',
	'at':'en',
	'Wave sent to':'Oleada enviada a',
	'Wave attack sent to':'Ataque oleada enviado a',
	'WD Armor-1':'Armadura DA'
};
...
Data.options.autoCollect.delay = 1*300;
  ...
  //C.attrs.sessionId = 'xd234';  //test session id failure
    Seed.init(function(rslt){
      if (rslt.ok){
        ...
        dialogFatal ('Error iniciando DOA Borch:<BR>'+ rslt.errmsg);
        return;
      }
    });
    
    logit ("* DOA Modded by Borch v"+ Version +" "+translate( 'Loaded' ) );
  ...
  // TODO: Make sure WinPos is visible on-screen ?
    ...
    mainPop = new CPopup ('dtmain', Data.options.ptWinPos.x, Data.options.ptWinPos.y, 625,790, Data.options.ptWinDrag, 
        function (){
          tabManager.hideTab();
    ...
    function gotSeed (rslt){    // TODO: check result, retry or disable tools?
      ...
      AddMainTabLink('DOA Borch', eventHideShow, mouseMainTab);
    ...
    dialogFatal ('<B>Error initializing DOA by Borch:</b><BR><BR>'+  e);
    logit (inspect (e, 8, 1));
  }  
...
function checkVersion (){
  ...
  GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://userscripts.org/scripts/review/105182',
    onload : function(r){
      ...
      if (m && m[1]!=Data.options.version.checkVersion){
        ...
        pop.getMainDiv ().innerHTML = '<STYLE>'+ Styles +'</style><BR><CENTER>Hay una nueva version del Mod de Borch<BR><BR>Version: '+ m[1] 
...
Tabs.Waves = {
  ...
  tabDisabled : false,
  cont : null,
  troopList : ['Espia', 'Porta', 'Mino', 'Arquer', 'DBatal', 'DRapid', 'EFuego', 'Triton', 'TBlind', 'OgroGr', 'DraGran', 'DraAgua'],
  enabled : false,
  attackTimer : null,
  marchTimer: null,
  attackErrors : 0,
...
logit ('************** autoCheckTargets Shouldn\'t happen *************');    
    ...
    if (marchCount >= Data.options.campAttack.maxMarches){
      t.dispFeedback ('Alcanzado el numero max. de marchas establecido.');
      t.attackTimer = setTimeout (t.autoCheckTargets, 5000);
      return;
    }
    
    if (getMusterPointSlots (cityIdx) <= 0){
      t.dispFeedback ('Punto de encuentro al maximo');
      t.attackTimer = setTimeout (t.autoCheckTargets, 5000);
      return;
    }
    if ((gen = getAvailableGeneral ()) == null){
    ...
    t.dispFeedback ( translate( 'No hay objetivos/tropas disponibles') );
  ...
  checkTroops : function (cityIdx, campLevel){
    ...
    for (var p in troops){
      if (troops[p] > 0){
        totTroops += troops[p];
        if (Seed.s.cities[cityIdx].units[p] < troops[p]){
          return ( translate( 'No hay suficientes ' ) +' '+ p);
        }
      }
    }
    if (totTroops <= 0){
      return translate( 'No se definieron Tropas' );
    }
    return null;
  },
  ...
  checkAttack : function (camp, notify){
    ...
    for (var p in troops){
      if (troops[p] > 0){
        totTroops += troops[p];
        if (Seed.s.cities[cityIdx].units[p] < troops[p]){
          notify ( translate( 'No hay suficiente' ) +' '+ p);
          return;
        }
      }
    }
    if (totTroops <= 0){
        notify ('No hay Tropas Definidas');
        return;
    }
    // TODO: 'too many troops for muster point level'
    if (getMusterPointSlots (cityIdx) <= 0){
      notify ('Punto de Encuentro lleno');
      return;
    }
    if ((gen = getAvailableGeneral ()) == null){
      notify ('No hay Generales disponibles');
      return;
    }
...
// fix this :P
  ...
  scanMap : function (radius, notify){
    ...
    dial.getContentDiv().innerHTML = 'Escaneando el mapa<BR>esto puede llevar 1 ratillo';
    var ix=0; iy=0;
...
function marchTable (myId){
  ...
  for (var p in Seed.marches){
    ...
    m += '<TR class='+ class +'><TD>'+translate( 'Attack' )+' '+ march.x +','+ march.y +'</td><TD>('+ translate( march.target ) +'-'+ march.terrain_level +')</td><TD>'+ translate( march.status ) +'<TD>'+ time +'</td></tr>';
  }
  return m +'</table>';
}
...
Tabs.Info = {
  ...
  init : function (div){
    var t = Tabs.Info;
    t.cont = div;
    div.innerHTML = '<DIV class=pbTitle>DOA Modded by Borch - v'+ Version +'<BR>'+ WebSite +'</div>\
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    for (var c=0; c<Seed.s.cities.length; c++){
		...
		m += '<DIV class=pbSubtitle>'+translate('City')+' #'+ (c+1) +' ('+ city.type +')</div><TABLE class=pbTab><TR valign=top><TD width=150><TABLE class=pbTab>';
...
Tabs.Research = {
  tabOrder : 35,
  tabLabel : translate( 'Research' ),
  cont : null,
  timer : null,
  researchList : ['Agriculture', 'Woodcraft', 'Masonry', 'Mining', 'Clairvoyance', 'RapidDeployment', 'Ballistics', 'Metallurgy', 'Medicine', 'Dragonry', 'Levitation', 'Mercantilism','AerialCombat'],
  ...
  init : function (div){
    ...
    var city = Seed.s.cities[0];
	t.cont = div;
   div.innerHTML = '<DIV class=pbTitle style="margin-bottom:10px">--- '+translate( 'Research' )+' ---</div>';
    t.showStuff();
  },
  ...
  showStuff : function (){
	...
	m += '<TABLE class=pbTab id=pbResearch ><TR valign=top><TD colspan=2><B>En desarrollo<BR><BR>'+ dispResearchJob(0) + '</b></td></tr></table>';

//	document.getElementById('pbResearch').innerHTML = m; 
    var now = new Date();  
    now.setTime(now.getTime() + (now.getTimezoneOffset()*60000));
    t.timer = setTimeout (t.showStuff, 1000);
...
/*
    ...
    t.cont = div;
    try {      
      m = '<DIV class=pbTitle style="margin-bottom:10px">'+translate( 'Research' )+'</div><TABLE class=pbTab>\
        <TR valign=top><TD colspan=2><B>En desarrollo<BR><BR>'+ dispResearchJob(0) + '</b></td></tr>\
        </table><BR><HR>';
      t.cont.innerHTML = m;
      t.timer = setTimeout (t.showStuff, 1000);
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>DOA Borch</center>';
...
function DialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  var cdTimer;
    ...
    pop.getTopDiv().innerHTML = '<CENTER>DOA Modde by Borch</center>';
    pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Ocurrio un error:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
        <BR><BR><B>DOA BorchT lo reintentara en <SPAN id=paretrySeconds></b></span> segundos ...<BR><BR><INPUT id=paretryCancel type=submit value="CANCELAR" \>';
...
// Firefox bug??? It appears as if a new thread is started on open, withOUT reusing same window
      t.win = window.open('', 'uwtrace', 'top=30,left=0,width=900,height=700,scrollbars=auto,location=no,menubar=no,directories=no,status=no');
t.isOpening = false; 
t.state = null; 
    }
...
function translate( wordToTranslate ) {
	...
	if ( navigator.language != 'es' || navigator.language == 'nl' || navigator.language == 'du' || navigator.language == 'fr' || navigator.language == 'in') {
