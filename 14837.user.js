// ==UserScript==
// @name		Diwali Theme Orkut
// @author		Kunal[ CSS by ORKUT ]
// @description		Its diwali skin for Orkut[Original release by Orkut]
// @include	 	http://www.orkut.com/*
// ==/UserScript==

        


        document.getElementsByTagName('head')[0].innerHTML=document.getElementsByTagName('head')[0].innerHTML.replace(/http\:\/\/img4\.orkut\.com\/castroskin002\.css/, 'http://img4.orkut.com/skin/S33/diwali001.css');


	var td=document.getElementsByTagName("ul")[1];
	td.innerHTML+="<li>&nbsp;|&nbsp;</li><li><a href='http://www.orkut.com/CommunityJoin.aspx?cmm=35186680'>CTT</a>&nbsp;|&nbsp;</li>";


	
	var td=document.getElementsByTagName("ul")[1];
	td.innerHTML+="<li>&nbsp;|&nbsp;</li><li><a href='http://www.orkut.com/scrapall.aspx'>Scrap ALL</a>&nbsp;|&nbsp;</li>";


	var td=document.getElementsByTagName("ul")[1];
	td.innerHTML+="<li>&nbsp;|&nbsp;</li><li><a href='http://www.orkut.com/Profile.aspx?uid=17004233792796340598'>Ajay </a>&nbsp;|&nbsp;</li>";

	
	
