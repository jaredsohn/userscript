...
// @exclude         http://www.moswar.ru/fight/*
// @run-at          document-end
// ==/UserScript==
...
var digging_preference = 17;	      // Proximity that bot would go into metro
...
//var states = ["%enemy","%victim", "%weak"];        // %enemy
...
var raider_stats_difference = 1.1;		// Allow victim to be 20% stronger then us
...
var mcD_work_hours = [23,7];
var reduceCost = 300;			// Bribe size
var do_bribes  = 1;			// Давать взятки
var do_headhunting = 0;			// Self explaining
...
const main_host = "http://www.moswar.ru";
const urls = ["/alley/",
	    "/square/",
	    "/home/",
	    "/clan",
...
{ 
  var code="CODE: \n TAB T=1" + jsNewLine;
//  code = code + "URL GOTO=http://www.moswar.ru/player/" + jsNewLine;
...
function buyBottles(amount) {
 var code="CODE: \n TAB T=1" + jsNewLine;
 code = code + "URL GOTO=http://www.moswar.ru/shop/section/pharmacy/buy/83/"+amount+"/"+jsNewLine;
...
function playLottery(runcount, do_logging, root_folder){
  ...
  if (do_logging == 1) {iimDisplay("Logging enabled !");}
    visitURL("http://www.moswar.ru/thimble/start/",0,2);
  
    var gameCode = "CODE:"  + jsNewLine;
    if (DEBUG == 1) { gameCode = gameCode + "SET !REPLAYSPEED SLOW";}
    gameCode += "URL GOTO=http://www.moswar.ru/thimble/play/2/" + jsNewLine;
    gameCode += "WAIT SECONDS="+ getRandomInt(3,15) + jsNewLine;    
    gameCode += "URL GOTO=http://www.moswar.ru/thimble/guess/#guess#/" + jsNewLine;
...
/*     if (do_logging == 1) {getMoney();
  ...
  gameCode += "URL GOTO=http://www.moswar.ru/thimble/" + jsNewLine;
  gameCode += "TAG POS=2 TYPE=DIV ATTR=TXT:Я<SP>наигрался,<SP>хватит" + jsNewLine;
  iimPlay(gameCode);

//  visitUrl("http://www.moswar.ru/thimble/leave/",0,2);
}
...
function doWar() {
 var tmpCode = "CODE:" + jsNewLine;
 tmpCode += "URL GOTO=http://www.moswar.ru/alley/" + jsNewLine;
...
function doHealing() {
 ...
 tmpCode += "URL GOTO=http://www.moswar.ru"+ healing_link + jsNewLine;
 tmpCode += "WAIT SECONDS="+getRandomInt(2,10) + jsNewLine;
 var tmp_retcode = iimPlay(tmpCode);
 if (tmp_retcode < 0) { iimDisplay(iimGetLastError()); }
 return tmp_retcode;
...
function doFullHealing() {
 iimDisplay("healing");
 var tmpCode = "CODE:" + jsNewLine;
 tmpCode += "URL GOTO= http://www.moswar.ru/home/heal/" + jsNewLine;;
 tmpCode += "WAIT SECONDS="+getRandomInt(2,10) + jsNewLine;;
...
function getHealth() {
 ...
 getHealthCode = getHealthCode + "URL GOTO=http://www.moswar.ru/alley/" + jsNewLine;
...
{
  var code = "CODE:"  + jsNewLine;
   code += "URL GOTO=http://www.moswar.ru/alley/" + jsNewLine;
  ...
  if (criteria == "%raider") { 
    ...
    searchRaider(); } else { 
     var Fight = "CODE:" + jsNewLine;
	  Fight = Fight + "URL GOTO=http://www.moswar.ru/alley/" + jsNewLine;
...
function doPetrics(v_money, v_ore) {
 ...
 if ((v_money>=money_for_petrics)&&(v_ore>=ore_for_petrics)) { 
   ...
   petricCode = petricCode + "URL GOTO=http://www.moswar.ru/factory/" + jsNewLine;
...
function searchRat(tx2)
{
 ...
 var tmpCode = "CODE:"+jsNewLine;
 tmpCode +="URL GOTO=http://www.moswar.ru/metro/" + jsNewLine;
...
function reduceWantedLevel() {
 var code="CODE: \n TAB T=1" + jsNewLine;
 code = code + "URL GOTO=http://www.moswar.ru/police/" + jsNewLine;
...
{
 var code="CODE: \n TAB T=1" + jsNewLine;
 code = code + "URL GOTO=http://www.moswar.ru/quest/" + jsNewLine;
...
function mcDonalds(hoursToSpend)
{
 ...
 swapItem(8523673);
 var code="CODE: \n TAB T=1" + jsNewLine;
 code = code + "URL GOTO=http://www.moswar.ru/shaurburgers/" + jsNewLine;
...
/*function addVictim(name, typex)
{
 var variants = ["%enemy", "%victim"];
 var code="CODE: \n TAB T=1" + jsNewLine;
 code += "URL GOTO=http://www.moswar.ru/phone/contacts/add/" + jsNewLine;
 code += "TAG POS=1 TYPE=INPUT:TEXT FORM=NAME:contactForm ATTR=NAME:name CONTENT=" +name+ jsNewLine;
 code += "TAG POS=1 TYPE=SELECT FORM=NAME:contactForm ATTR=NAME:type CONTENT="+variants[typex] + jsNewLine;;
 code += "TAG POS=1 TYPE=BUTTON ATTR=TXT:Добавить<SP>в<SP>список" + jsNewLine;
 iimPlay(code);
} */
...
function getVictimList(target_lvl)
{
 ...
 var code="CODE: \n TAB T=1" + jsNewLine;
/* code += "URL GOTO=http://www.moswar.ru/huntclub/wanted/page/1/" + jsNewLine;
...
// alert(iimGetLastExtract(1));
 ...
 { 
    var code="CODE: \n TAB T=1" + jsNewLine;
	code += "URL GOTO=http://www.moswar.ru/huntclub/wanted/page/"+z+"/" + jsNewLine;
	result = iimPlay(code);
	if (result < 0) {return;}
  for (i=1;i<=20;i++)
  {    	
...
{
   visitURL("http://www.moswar.ru/phone/contacts/enemies/",0,2);
   postUrl("http://www.moswar.ru/phone/contacts/add/", "name="+name_x+"&info=&type=enemy");
}
...
function clearEnemyList()
{
   ...
   do {
	   var code="CODE: \n TAB T=1" + jsNewLine;
	   code += "URL GOTO=http://www.moswar.ru/phone/contacts/enemies/" + jsNewLine;
	   ...
	   s = s.substring(p,s.indexOf(");\">")).replace(/\'/g, '');
           ...
           postUrl("http://www.moswar.ru/phone/contacts/","action=delete&id="+fields[1]+"&nickname="+fields[0]+"&type=contact");
...
{
  var code="CODE: \n TAB T=1" + jsNewLine;
  code += "URL GOTO=http://www.moswar.ru/player" + jsNewLine;
  ...
  if (t != NOT_FOUND) {
  	code =  "CODE: \n TAB T=1" + jsNewLine;
  	code += "URL GOTO=http://www.moswar.ru/huntclub/me/" + jsNewLine;
...
{
  var code="CODE: \n TAB T=1" + jsNewLine;
  code += "URL GOTO=http://www.moswar.ru/shop/section/zoo/buy/91/" + jsNewLine;
  iimPlay(code);
}
...
function MetroCode() {
 ...
 MetroCode += "TAG POS=1 TYPE=A ATTR=HREF:http://www.moswar.ru/square/" + jsNewLine;
 ...
 {   
   ...
   MetroCode += "URL GOTO=http://www.moswar.ru/metro/" + jsNewLine;
...
function buyKirka(money_x) {
 ...
 Kirka += "URL GOTO=http://www.moswar.ru/square/" + jsNewLine;
 Kirka += "TAG POS=2 TYPE=DIV ATTR=TXT:Торговый<SP>центр" + jsNewLine;
 Kirka += "TAG POS=1 TYPE=A ATTR=HREF:http://www.moswar.ru/shop/section/other/" + jsNewLine; 
 Kirka += "WAIT SECONDS="+getRandomInt(0,additional_wait[0])+ jsNewLine; 
 Kirka += "URL GOTO=http://www.moswar.ru/shop/section/other/buy/"+ lnk + jsNewLine;
 iimPlay(Kirka);
 visitURL("http://www.moswar.ru/shop/section/other/buy/69/", 0, 1);
}
...
function checkPolice()
{
  var result = -1;
  var Code = "CODE:" + jsNewLine;
  Code += "TAG POS=1 TYPE=A ATTR=HREF:http://www.moswar.ru/square/" + jsNewLine;
...
function getOwnStats()
{
  ...
  var code = "CODE:" + jsNewLine;
//  code += "URL GOTO=http://www.moswar.ru/player/" + jsNewLine;
  iimPlay("CODE: URL GOTO=http://www.moswar.ru/player/");
...
function getPlayCount()
{
  var result = -1;
  var code = "CODE:"  + jsNewLine;
  code += "URL GOTO=http://www.moswar.ru/metro/" + jsNewLine;
...
function getAssaultCount()
{
  var result = -1;
  var code = "CODE:"  + jsNewLine;
  code += "URL GOTO=http://www.moswar.ru/huntclub/" + jsNewLine;
...
// MetroCode = "CODE:";
// MetroCode = MetroCode + "URL GOTO=http://www.moswar.ru/metro/" + jsNewLine;
...
for (i=0; i< z; i = i+1)
{
    ...
    if (mcD_doWork == 1) {
     var t = dd.getHours(); 
     var m1 = Math.abs(t - mcD_work_hours[1]);
     if (m1 > 8) {m1 = 8;} 
     if ( t >= mcD_work_hours[0] || t < mcD_work_hours[1]) { mcDonalds(m1); }
    }    
