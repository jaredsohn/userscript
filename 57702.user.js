      ...
      li.addEventListener(
        ...
        function addProfileImages() {
                ...
                if (res != "") {
                  ...
                  if (keepGoing) {
                    ...
                    if (!/<div/.test(res))
                      ...
                      getPage("http://www.facebook.com/ajax/photos.php?id=" + personId + "&vm=photosofmeviewer=" + userId + "&so=" + ((prev ? prevNo-- : nextNo++) * 15) + "&action=page&section=photos_of", prev)
          ...
          if (navLinks.innerHTML.match(/prev/i))
            getPage("http://www.facebook.com/ajax/photos.php?id=" + personId + "&vm=photosofmeviewer=" + userId + "&so=" + ((prevNo-- * 15) - 15) + "&action=page&section=photos_of", true)
          else
            prevDone = true
          //if there's a next link, start going forwards
          if (navLinks.innerHTML.match(/next/i))
            getPage("http://www.facebook.com/ajax/photos.php?id=" + personId + "&vm=photosofmeviewer=" + userId + "&so=" + ((nextNo++ * 15) - 15) + "&action=page&section=photos_of", false)
          else
            nextDone = true
      ...
      for (i = navLinks.snapshotLength - 1; i >= 0; i--) {
        ...
        li.addEventListener(
          ...
          function addImages() {
                  ...
                  if (keepGoing) {
                    ...
                    if (h4) {
                      ...
                      if ((numbers[1] == 1) && (numbers[2] == numbers[3])) {
                        ...
                        pageNumber = parseInt(link.match(/page=(\d+)/)[1]) + (prev ? -1 : 1);
            ...
            if (navLinks.innerHTML.match(/prev/))
              getPage((navLinks.getElementsByTagName("a"))[0].href, true)
            if (navLinks.innerHTML.match(/next/)) 
              getPage((links = navLinks.getElementsByTagName("a"))[links.length - 2].href, false)
