// ==UserScript==
// @name           eRepublik Optimizer - Hungarian
...
// GM menu berakasa, a profil es munkahely URL beallitasahoz
GM_registerMenuCommand("Profil URL megadása", function(){ GM_setValue('PROFILE',prompt("Másold ide a Profilod URL-jét:") ) } );
GM_registerMenuCommand("Munkahely URL megadása", function(){ GM_setValue('COMPANY',prompt("Másold ide a Munkahelyed URL-jét:") ) } );
GM_registerMenuCommand("URL-ek mutatása", function(){ var mutatni; pro_url = GM_getValue('PROFILE'); co_url = GM_getValue('COMPANY'); alert('Profilod: '+pro_url+'\nMunkahelyed: '+co_url) } );
...
var profile_URL;
var company_URL;
profile_URL = GM_getValue('PROFILE');
company_URL = GM_getValue('COMPANY');
var header = document.getElementById('header');
if  (header) {
	var quicklinks = document.createElement("div");
	quicklinks.innerHTML = '<div class="menubar" id="menubar"><p style="margin: 2px 0 1px 0; padding-top:1px; padding-bottom:7px;">' + '<div style="float:left;">' + '<a href="'+profile_URL+'">Saját profil</a> | ' +  '<a href="'+company_URL+'">Munkahely</a> | ' + '<a href="http://www.erepublik.com/en/my-places/army">Hadsereg</a> | <a href="http://www.erepublik.com/en/wars/1">Aktív háborúk</a></div>' + '<div style="float:right;">' + '<strong><u>Piac:</u></strong> <a href="http://www.erepublik.com/en/market/country-13-industry-1-quality-0">kaja</a>, <a href="http://www.erepublik.com/en/market/country-13-industry-2-quality-0">gift</a>, <a href="http://www.erepublik.com/en/market/country-13-industry-4-quality-0">repjegy</a>, <a href="http://www.erepublik.com/en/market/country-13-industry-3-quality-0">fegyver</a> | <a href="http://widget.mibbit.com/?channel=%23erep">Magyar Mibbit Chat</a></div></p><br>' + '<div style="float:right;" dir="ltr">' + '<div class="information"><br>eRep idő: ' + livetime.innerHTML + ' @ ' + date + '<br>Ma, az Új Világ <b>' + dateCount + '</b>. napja van.</div></div>' + '<br><div align="left">' + searchholder.innerHTML + '</div></div>';
