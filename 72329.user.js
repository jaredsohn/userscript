// ==UserScript==
// @name tribalwarsfast-www.no9ba.eb2a.com
// @description روابط سريه لي ارسل هجمات و دخول القرى
...
(function() {
	...
	var dsrbAddButtons = function(node) {
		...
		var overview = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.target + '&screen=overview', icons.overview, 'قرية لمحة عامة n\ www.no9ba.eb2a.com');
		if(coords) {
			var center = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=map&x=' + coords[0] + '&y=' + coords[1], icons.center, 'مركزا على الخريطة \n www.no9ba.eb2a.com');
		}
		var troops = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=place&mode=command&target=' + vilid.target, icons.troops, 'ارسل قوات\n www.no9ba.eb2a.com');
	...
	for(var i = 0; i < nodes.length; i++) {
		if(/village=\d+&screen=info_village&id=\d+$/.test(nodes[i].href)) {
