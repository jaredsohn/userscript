...
// @require     http://userscripts.org/scripts/source/398811.user.js
// @run-at      document-end
...
var initialize = function() {
    ...
    var itemNamePanels = document.querySelectorAll(".item");
...
var attachExtraPanelsAndListeners = function(itemNamePanels) {
    for (var i = 0, length = itemNamePanels.length; i < length; i++) {
        ...
        itemNamePanel.addEventListener("mouseover", getLowestPriceHandler, false);
...
var getLowestPriceHandler = function() {
	...
	//var itemNameElement2 = this.querySelector(".extraPanel");
	var displayer = this.querySelector(".scriptStatus");
    // don`t try to get the price if we've already retrieved it
    if (displayer.innerHTML != "Ready") {
        return;
    }
    var theItem = itemNameElement.querySelector("b").innerHTML.trim();
    var theItemString = encodeURIComponent(theItem);
	// don`t try to get the price if we've already retrieved it
    if (undefined !== itemPrices[theItemString]) {
        displayer.innerHTML = itemPrices[theItemString];
		return;
    }
    // from Steam's community market website
    var appID = 730;
    displayer.innerHTML = "Loading...";
    GM_xmlhttpRequest({
        method: "GET",
        ...
        onload: function (response) {
            ...
            var priceWithFee = "<span class='" + (match ?
                ...
                + "</span>";
            itemPrices[theItemString] = priceWithFee;
            match = lowestPriceWithoutFeeRegExp.exec(httpResponse);
            var priceWithoutFee = match ? match[1] + " - without fee (seller receives)" : ""; 
            displayer.innerHTML = "<a href=\"http://steamcommunity.com/market/listings/" + appID + "/" + theItemString + "/\"><span title='" + priceWithoutFee + "'>" + priceWithFee + "</span></a>";
			//alert(priceWithFee);
        }
    });
}
...
var addButton = function() {
	...
	var extraButton = document.createElement('li');
        extraButton.innerHTML = "<a><img src=\"http://csgolounge.com/img/bookmarks.png\" alt=\"Bookmarks\" /><span>add prices</span></a>";
		extraButton.addEventListener("click", initialize, false);
        menu.appendChild(extraButton);
}
