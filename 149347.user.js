...
// @description    Позволяет просмотреть результат и статистику бойя перед атакой.
// @namespace      http://userscripts.org/scripts/show/145717
// @include        https://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @version        2.2 RUS
// @author         Quor | WildKatana | Updated by CodeEcho, PythEch, Matthias Fuchs, Enceladus, KRS_L, TheLuminary, Panavia2, Da Xue | Translation by 777lexa777 
...
function initializeTools() {
	...
	toolsbtn = new qx.ui.form.Button("Открыть");
    toolsbtn.set({
        width: 70,
        height: 26,
        appearance: "button-text-small",
        toolTipText: "Обновить статистику боя"
	});
	...
	battleResultsBox = (new qx.ui.window.Window("Симулятор", "FactionUI/icons/icon_loading_logo.gif")).set({
		contentPaddingTop: 0,
	...
	t.add(new qx.ui.basic.Label("Вражеская база:"), {
row: 0,
		column: 0
		...
		row: 4,
		column: 1
		}), t.add(new qx.ui.basic.Label("Постройки:"), {
		row: 3,
		column: 0
		...
		row: 5,
		column: 1
		}), t.add(new qx.ui.basic.Label("Оборонный цех:"), {
		row: 6,
		column: 0
	...
	t.add(new qx.ui.basic.Label("Общий:"), {
		row: 0,
		column: 0
	...
	t.add(new qx.ui.basic.Label("Результат:"), {
		row: 0,
		column: 0
		}), labels.damage.outcome = new qx.ui.basic.Label("Неизвестно"), t.add(labels.damage.outcome, {
		row: 0,
		column: 1
		}), t.add(new qx.ui.basic.Label("Время боя:"), {
		row: 1,
		column: 0
	...
	////////////////// Info ////////////////////
	var infoPage = new qx.ui.tabview.Page("Добыча");
	...
	infoPage.add(psVBox);
	psVBox.add(new qx.ui.basic.Label("Трофеи"));
	// Tiberium
...
function initSimulateBattle() {
    ...
    var simulatebtn = new qx.ui.form.Button("Просмотр");
    simulatebtn.set({
		width: 70,
		height: 26,
		appearance: "button-text-small",
		toolTipText: "Посмотреть бой в симуляторе"
	});
	...
	var unlockbtn = new qx.ui.form.Button("Открыть");
	unlockbtn.set({
		width: 55,
		height: 46,
		appearance: "button-text-small",
		toolTipText: "Открыть кнопку для атаки"
	});
	...
	var backbtn = new qx.ui.form.Button("Настройка");
    backbtn.set({
		width: 73,
		height: 24,
		appearance: "button-text-small",
	...
	arrows.ShiftFormationUp.set(
	{
		width: 30,
		appearance: "button-text-small",
		toolTipText: "Сдвинуть отряды вверх"
	});
	arrows.ShiftFormationUp.addListener("click", function(){shiftFormation('u');}, this);
	
	arrows.ShiftFormationDown = new qx.ui.form.Button("v");
	arrows.ShiftFormationDown.set(
	{
		width: 30,
		appearance: "button-text-small",
		toolTipText: "Сдвинуть отряды вниз"
	});
