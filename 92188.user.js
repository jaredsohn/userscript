// ==UserScript==
// @name          Hattrick PsicoTSI in Greek
// @description   Classic PsicoTSI in Greek
// @version       0.67
// @copyright     Dec-2010, dinolihas in Greek (http://www.aldeaglobal.net/psicotsi/)
...
org.hattrick.PsicoTSI = function () {
   ...
   const CURRENCY = "EUR";
   const LANGUAGE = "gr";
   const SHOW_CURRENCIES = false; //shows an alert with all possible currencies
   const SHOW_LANGUAGES = false; //shows an alert with all possible languages
   ...
   const LOCALIZATION = {
      ...
      "eu": {
         "name": "Basque",
         "twoMainSkills":"OHARRA: Jokalari honek bi dohain nagusi ditu, iragarpenaren akatsa altuegia izan daiteke",
      ...
      "pl": {
         "name": "Polski",
         "twoMainSkills": "UWAGA: Ten gracz ma dwie równorzędne umiejętności. Błąd w pomiarze może być bardzo duży",
         "injuredPlayer": "UWAGA: Gracz kontuzjowany, główna umiejętność jest niedowartościowana",
         "oldPlayer": "UWAGA: Gracz ma więcej niż 27 lat, główna umiejętność jest niedowartościowana",
         "isGoalkeeper": "PsicoTSI nie mierzy podpoziomu umiejętności bramkarskich",
         "skillsNotAvailable": "Podgląd umiejętności tego gracza jest niedostępny.",
         "form": "Forma",
         "wage": "Pensja",
         "high": "Wysoka",
         "average": "Średnia",
         ...
         "skillWinger": "Skrzydłowy",
         "skillScoring": "Skuteczność",
         "skillPassing": "Podania",
         "skillDefending": "Defensywa",
         "skillKeeper": "Bramkarz",
         "limitLow": "Ten gracz poprawił ostatnio swoją umiejętność albo jego drugorzędne umiejętności i/lub forma mają bardzo niski podpoziom",
         "limitHigh": "Ten gracz jest bardzo blisko poprawienia swojej umiejętności albo jego drugorzędne umiejętności i/lub forma mają bardzo wysoki podpoziom"
      },
      "pt-BR": {
   ...
   tableExists = function (HTMLDocument) {
      try {
         var links = HTMLDocument.evaluate("//a[@class='skill']", HTMLDocument, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
   ...
   getAge = function (HTMLDocument) {
      var age = -1;
      try {
         var node = HTMLDocument.getElementById("ctl00_ctl00_CPContent_CPMain_ucPlayerFace_pnlAvatar");
         if (!node) {
            //not a supporter or face not showed
            node = HTMLDocument.getElementById("ctl00_ctl00_CPContent_CPMain_pnlplayerInfo");
   ...
   playerPagePsicoTSI = function (HTMLDocument) { /* GETTING SKILLS */
      ...
      /*
       //DEBUG
       var debugItems = "Index : link\n"
       for (kk = 0; kk < links.snapshotLength; kk++) {
       debugItems += kk + ": " + links.snapshotItem(kk).innerHTML + "\n";
       }
       alert (debugItems + "\nTotal: " + links.snapshotLength);
       */
   ...
   searchPagePsicoTSI = function (HTMLDocument) {
      ...
      var firstTLPlayer = -1;
      var PLAYERS_ON_PAGE = 25;
      
      for (i = 0; i < PLAYERS_ON_PAGE; i++) {
         var ageR = HTMLDocument.getElementById('ctl00_ctl00_CPContent_CPMain_dl_ctrl' + i + '_TransferPlayer_r3');

         if (TLTableExists(i, HTMLDocument)) {
      ...
      for (i = firstTLPlayer; i < PLAYERS_ON_PAGE; i++) {
         ...
         var tableExist = TLTableExists(i, HTMLDocument);
         try {

            var sta = parseInt(getValueFromLink(links.snapshotItem(3 + offset).href));

            var frm = parseInt(getValueFromLink(links.snapshotItem(2 + offset).href));
            var goa = parseInt(getValueFromLink(links.snapshotItem(4 + offset).href));
            var def = parseInt(getValueFromLink(links.snapshotItem(8 + offset).href));
            var pla = parseInt(getValueFromLink(links.snapshotItem(5 + offset).href));
            var win = parseInt(getValueFromLink(links.snapshotItem(7 + offset).href));
            var pas = parseInt(getValueFromLink(links.snapshotItem(6 + offset).href));
            var sco = parseInt(getValueFromLink(links.snapshotItem(9 + offset).href));
            var sp = parseInt(getValueFromLink(links.snapshotItem(10 + offset).href));

            var ageR = null;
            var tsiR = null;
            try {
               var ageR = HTMLDocument.getElementById('ctl00_ctl00_CPContent_CPMain_dl_ctrl' + i + '_TransferPlayer_r3');
               var tsiR = HTMLDocument.getElementById('ctl00_ctl00_CPContent_CPMain_dl_ctrl' + i + '_TransferPlayer_r4');

               if (ageR != null) {
                  var ageRow = ageR.cells[1];
                  var tsiRow = tsiR.cells[1];

                  var age = parseInt(ageRow.textContent.replace(/[\s]*/gi, ""));
               ...
               //GM_log("In function searchPagePsicoTSI\n" + e);
            }
            ...
            try {
               ...
               var node = HTMLDocument.getElementById('ctl00_ctl00_CPContent_CPMain_dl_ctrl' + i + '_TransferPlayer_r1');
               var testSearch = HTMLDocument.evaluate("//img[contains(@class,'injuryInjured')]", HTMLDocument, null, Components.interfaces.nsIDOMXPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
         ...
         if (tableExist) drawMessageInSearchPage(HTMLDocument, entryPoint, isGK, undef, injured, age > 27, maxSkill, valMaxSkillHigh, valMaxSkillAvg, valMaxSkillLow, valMaxSkillWage, limit);
      }
   };
   ...
   drawMessage = function (HTMLDocument, entryPoint, isGK, isUndefinedMainskill, isInjured, isOld, maxSkill, valMaxSkillHigh, valMaxSkillAvg, valMaxSkillLow, valMaxSkillWage, limit) {
      ...
      if (!SETT_HIDE_UNDER_SKILLS) {
         ...
         if (isAlertBoxStyle) {
            ...
            title.appendChild(document.createTextNode("PSICOTSI UBIQUITOUS EDITION " + VERSION));
      ...
      if (SETT_PREF_LEFT) {
         var messagePTSI = document.createElement("div");
         ...
         if (!isGK) {
            var wageNGK = "[" + STR_WAGE + "=\u20AC]=" + valMaxSkillWage + "<br>";
         } else {
            var wageNGK = " ";
         }
   ...
   drawMessageInSearchPage = function (doc, entryPoint, isGK, isUndefinedMainskill, isInjured, isOld, maxSkill, valMaxSkillHigh, valMaxSkillAvg, valMaxSkillLow, valMaxSkillWage, limit) {
      ...
      strong.appendChild(doc.createTextNode(headerText + " " + VERSION));
   ...
   calcMaxSkillGK = function (TSI, FORM, subForm) {
      ...
      return Math.round(((skill2 / 100) + skill) * 100) / 100;
   };
   ...
   pub.main = function () {
      ...
      try {
         ...
         else if (isHattrickSearchPage(href)) searchPagePsicoTSI(HTMLDocument);
      } catch(e) { }
   };
