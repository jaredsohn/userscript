...
for (i=0; el=res.snapshotItem(i); i++) {
    ...
    if(x!=null) {
	  ...
	  var tt=l;
	  //New part for download link
	  var tlink = new RegExp("download.php/.*?\.torrent");

			tt = tlink.exec(document.body.innerHTML);
			tt= "http://www.torrentleech.org/"+tt;

	 	  //End of download link part
      title = document.createElement("a");
		  title.appendChild(document.createTextNode(""));
		  title.className='title';
      title.setAttribute('href',tt);
    //  getTitle(title);
	title.appendChild(document.createTextNode("INDIR"));
	...
	GM_xmlhttpRequest({
		...
		onload: function (responseDetails) {
			...
			if (responseDetails.status == 200) {
				...
				var re = new RegExp(">(...)<span>/10");
				m = re.exec(responseDetails.responseText);

				if (m != null) {

				//	link.appendChild(document.createTextNode("<h1>"+m[1]+"/10</h1>"));
				    var label = document.createElement("span");
				  label.innerHTML = "<span><a  style='font-size:26px;color:red' href='"+link.href+"'>"+m[1]+"/10</a></span>";
				 label.setAttribute('href',link.href);
				  main.appendChild(label);
					link.className= "imdb"+m[1];
				}
