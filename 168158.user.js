...
<input type=\"checkbox\" id=\"sanction\" "+schecked+" /> Autokill player with name of <input type=\"text\" style=\"border: 1px solid green; width: 4em; color: #00ff00; background: black; text-align: center;\" id=\"sanctionlevel\" value=\""+sanctionLevel+"\" /> when rage is at least <input type=\"text\" style=\"border: 1px solid green; width: 4em; color: #00ff00; background: black; text-align: center;\" id=\"sanctionammo\" value=\""+sanctionAmmo+"\" /><br>\
...
function autobuild()
{
	if (page == "investment")
	{
		...
		if (def || inc)
		{
			...
			document.getElementById('wwfixernextbuilding').innerHTML = '<font color="#00ff00"><small><b>Next building: '+names[minIndex]+', need <img src="http://static.storm8.com/zl/images/money.png?v=140">'+fixNum(need)+'</b> - '+Math.ceil(need / rate)+'h</small></font>';
...
function attackSanc(noReattack)
{
	...
	if (document.getElementById('staminaCurrent').innerHTML == 0)
	{
		...
		setTimeout("document.location = 'http://zl.storm8.com/hitlist.php'", Math.floor(1000*seconds));
		return;
	}
	...
	if (!found)
		document.location = 'http://zl.storm8.com/hitlist.php';
		//setTimeout("document.location = 'http://zl.storm8.com/hitlist.php'", Math.floor(Math.random() * 40)*90);
		//setTimeout("document.location = 'http://zl.storm8.com/hitlist.php'", 2000);
