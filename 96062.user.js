// ==UserScript== 
// @name           Team Record Book Season 20
...
function getGameData(gameId, teamId, season){
    ...
    if(season == 20){ //new sortable tables
                ...
                for(var k=1;k<td.length;k++){
                    if(i==4 || i==5){//receiving category
                        if(k==6){
                            ...
                            var yac = parseFloat(td[6].innerHTML.replace(',','').replace(',',''));
                            var catches = parseFloat(td[3].innerHTML.replace(',','').replace(',',''));
    ...
    if(array.length != 0){ //no need to flatten if it is empty        
        ...
        ' getting schedule for season ' + i + '...';                        stat[3+offset] = (stat[2+offset]/stat[1+offset]); //ypc = yards/rushes
                            ...
                            case 2: //receiving category
                                stat[4+offset] = (stat[3+offset]/stat[2+offset]); //yds per rec = yards/rec
                                break;
...
function updateRecordBook(recordBook,data,minAttempts){
    ...
    if(recordBook == ''){ 
        for(var i=0;i<data.length;i++){
            ...
            var catLength = [13,10,12,9,3,8,5,18,10];
...
function format(data,template){
    ...
    const categories=[['Passing','','Comp','ATT','Yds','Pct','Y/A','Hry','Sck','','BadTH','Drop','Int','TD'],
                     ['Rushing','','','Yds','Avg','TD','RushRZ','BrTk','TFL','Fum','FMBLLost'],             

['Receiving','','Targ','Rec','Yds','Avg','YAC','TD','Drop','TargRZ','Targ3D','',''],
                     ['Kicking','FGM','','0-19','20-29','30-39','40-49','50+','XPM',''],
                     ['Punting','Punts','Yds','Avg'],
                     ['Return','KRs','KR Yds','KRAvg','KRTD','PRs','PRYds','PRAvg','PRTD'],
                     ['Offesnsive Line','','Pnk','RevPnkd','HryAlw','SacAlw'],   

['Defensive','','Tk','MsTk','Sack','','Hry','TFL','FFum','FumRec','Targ','RecAlw','KLoose','PD','Int','','TD','PNKD','RevPnk'],
['Special Teams','','TKL','MSTK','FF','FBREC','FUMRTD','PNKD','BRTK','FUM','FUML']];
