...
/* -------------------------------------------- 设置开始 ------------------------------------------*/
	...
	var WHITE_LIST_KEYWORD = ['player', 'video', 'tudou.com', 'viddler'];
...
/* -------------------------------------------- 设置结束 ------------------------------------------*/
	...
	function move_popup_player_to_center(e){
		if(playerMoved == true){
			...
			$('#fv_PopupFlash').removeAttribute('class', 'move');
			set_option_and_menu_pos_no_reload();
			set_flash_pos_no_reload();
		}
	}
	...
	function resize_popup_player(e){
		...
		var edgeLeft = 'EDGE_NONE', edgeTop = 'EDGE_NONE';
		...
		{
		case 'EDGE_LEFT_RIGHT':
			resize_popup_flash_by_drag(flash, flashPreSize.w + (Math.abs(x - dot.x) - diff.offsetW) * multi, 0, edgeLeft, 'EDGE_NONE');
			break;
		case 'EDGE_TOP_BOTTOM':
			resize_popup_flash_by_drag(flash, 0, flashPreSize.h + (Math.abs(y - dot.y) - diff.offsetH) * multi, 'EDGE_NONE', edgeTop);
			break;
		case 'EDGE_CORNER':
			if(edgeLeft == 'EDGE_LEFT' && edgeTop == 'EDGE_TOP'){
				break;
			}
			resize_popup_flash_by_drag(flash, flashPreSize.w + (Math.abs(x - dot.x) - diff.offsetW) * multi, flashPreSize.h + (Math.abs(y - dot.y) - diff.offsetH) * multi, edgeLeft, edgeTop);
			break;
		default:
			break;
		}
	...
	function hide_original_player(obj){
		...
		_saveAndSetObjAttri(obj, 'id', 'fv_hidden_player');
	...
	function show_popup_icon(e, obj){
		if($('#fv_popup_icon')){
			del_popup_icon();
		}
