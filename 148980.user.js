// ==UserScript==
// @name        VCDQ - Binsearch + ThePirtateBay Searcher
// @namespace   PutterPlace
// @description Adds a "U" and "T" button to a release's line on the main page, and the release's page itself, on VCDQ.com - Search for releases on usenet (via Binsearch.info) or torrents (via ThePirateBay.sx) - Based on the script by Dink *http://userscripts.org/scripts/show/7083*
...
if(document.location.href.indexOf('vcdq.com') > -1)
{
    ...
    function addLinks(link, elementID)
    {
        thelink = link.getElementsByTagName("a").item(elementID);
        ...
        torrenturl = 'http://thepiratebay.sx/search/$1'.replace("$1", encodeURIComponent(fulltitle));
        ...
        torlink.innerHTML = "<img title='Search thepiratebay.sx for " + fulltitle + "' src=" + ticon + " border='0'>";
    ...
    {
		var elementID = 0;
		if(document.documentElement.innerHTML.indexOf('fancyContent') > -1)
		{
			elementID = 1;
		}
        for (var i=0; i<releases.snapshotLength; i++)
            addLinks(releases.snapshotItem(i), elementID);
    }
