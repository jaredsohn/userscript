// ==UserScript==
// @name           Pokemon Votex Tool
// @namespace      http://userscripts.org/scripts/show/132382
// @description    Pokemon Votex bot.
// @version        0.3
// @include        http://*delugerpg.com/*
// @exclude        http://*delugerpg.com/adv.php*
// @exclude        http://*.delugerpg.com/ads2.htm
// @exclude        http://*.delugerpg.com/members.php*
// @exclude        http://forums.delugerpg.com/*
// @exclude        http://*.delugerpg.com/mobile*
// ==/UserScript==
...
var settings = {
	'keys': ['autoBattle', 'findPoke', 'findlegendary','catchPok'],
	'data': {},
	'save': function() {
...
function moveAround() {
	...
	unsafeWindow.PlayRequest(xpos + 20, ypos + 14, parseInt(Math.random()*8)+1);
}
...
function catchPok() {
	try {
		...
		if (linkReturnToMap && linkReturnToMap.textContent.trim() == 'Return to the Map') {
			settings.data.nextBattle = new Date().getTime() + 1000;
...
function findLegendary() {
	...
	if(rareType == 'all'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
			}
		}
	}
	
	if(!wildText.match(/Wild (Dark|Shiny|Ancient|Mystic)/) && rareType == 'normal'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	if(wildText.match(/Wild (Dark|Shiny|Ancient|Mystic)/) && rareType == 'rare'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	if(wildText.match(/Wild (Shiny)/) && rareType == 'shiny'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	if(wildText.match(/Wild (Ancient)/) && rareType == 'ancient'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	if(wildText.match(/Wild (Dark)/) && rareType == 'dark'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	if(wildText.match(/Wild (Mystic)/) && rareType == 'mystic'){
		for (var i = 0; i < legends.length; i++) {
			if (wildText.match(legends[i])) {
				unsafeWindow.console.info('Legendary Rare found ', wildText);
				return;
	...
	//unsafeWindow.console.info('Found: ', wildText);
	moveAround();
...
function findPoke() {
	...
	if (!wildText.match(/Wild (Dark|Shiny|Ancient|Mystic)/)) {
		for (var i = 0; i < legends.length; i++) {
			if (!wildText.match(legends[i])) {
				if(rareType == "normal"){
					unsafeWindow.console.info('Normal found ', wildText);
					return
				}
			}
		}
	...
	if (wildText.match(/Wild (Shiny)/)) {
		for (var i = 0; i < legends.length; i++) {
			if (!wildText.match(legends[i])) {
				if(rareType == "shiny"){
					unsafeWindow.console.info('Shiny found ', wildText);
					return
				}
			}
		}
	...
	if (wildText.match(/Wild (Dark)/)) {
		for (var i = 0; i < legends.length; i++) {
			if (!wildText.match(legends[i])) {
				if(rareType == "dark"){
					unsafeWindow.console.info('Dark found ', wildText);
					return
				}
			}
		}
	...
	if (wildText.match(/Wild (Ancient)/)) {
		for (var i = 0; i < legends.length; i++) {
			if (!wildText.match(legends[i])) {
				if(rareType == "ancient"){
					unsafeWindow.console.info('Ancient found ', wildText);
					return
				}
			}
		}
	...
	if (wildText.match(/Wild (Mystic)/)) {
		for (var i = 0; i < legends.length; i++) {
			if (!wildText.match(legends[i])) {
				if(rareType == "mystic"){
					unsafeWindow.console.info('Mystic found ', wildText);
					return
				}
			}
		}
...
function autoBattle() {
	try {
		...
		if (btnBattle) {
			// wait until 1 seconds since last battle
...
function rareOption() {
	...
	toggleEnable.innerHTML = ('<b><u>*ENABLED ONLY 1 CHEAT AT A TIME!*</u></b></br></br>'+ "Select Rare: " +
	<select id="select">
	<option value="all">All Type</option>
	<option value="normal">Non-Rare</option>
	<option value="rare">All Rare</option>
	...
	selectmenu.onchange=function(){
 		var chosenoption=this.options[this.selectedIndex];
 			rareType = chosenoption.value; 
 			unsafeWindow.console.info('Type Change ', rareType);
	}
...
function init() {
	...
	createToggler(container, 'Find Non-Legendary:', 'findPoke');
	createToggler(container, 'Find Legendary:', 'findLegendary');
	createToggler(container, 'Catch Pokemon(MUST HAVE MASTERBALL):', 'catchPok');
	autoContinue();
}
