...
var show = { złoto: 4, armia: 2, punkty: 1 };
var post = {
    punkty: "score",
     armia: "army_score_main",
     złoto: "trader_score_secondary"
};
...
function fetchScoresFor(name, ul, n, id) {
  function searchbutton(type) {
    var url = "url(/skin/" + ({
       punkty: "layout/medallie32x32_gold.gif) no-repeat -7px -9px",
        armia: "layout/sword-icon2.gif) no-repeat 0 2px;",
        złoto: "resources/icon_gold.gif) no-repeat 0 0; width:18px"
      })[type];
  ...
  for (type in show) {
    if (!(whatToShow & show[type]))
      continue;
    if ("złoto" == type && isMyCity(ul) && viewingRightCity(ul)) {
...
function mkItem(type, value) {
  ...
  var title = (type in show) ?
    type.charAt().toUpperCase() + type.slice(1) + "" : "Informacje:";
...
<table border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td style="width: 43%; text-align: right">Pokazuj wynik w jednej lini:</td>
