...
// @description    Zezanje
// @author         Lother
// @match          http://www.erepublik.com/*/main/citizen-invited-friends/1
// @include        http://www.erepublik.com/*/main/citizen-invited-friends/1
...
function StartFunctions() {
	...
	(function(){
		...
		var items_html="<b>Spisak proizvoda</b><br><table>";
		{
		...
		$( "html" ).html ("<title>TTE Auto kupovina</title>"+
						  "<table><tr valign=top><td><h3><b>TTE Auto kupovina</b></h3>"+
						  "<table>"+
						  "<tr><td>Drzavljanstvo </td><td><canvas class='loading' width='16' height='16'></canvas><b id='mymc'>Nepoznato</b></td></tr>"+
						  "<tr><td>Lokacija </td><td><canvas class='loading' width='16' height='16'></canvas><b id='mylc'>Nepoznato</b></td></tr>"+
						  "<tr><td>Skladiste</td><td><canvas class='loading' width='16' height='16'></canvas><b id='inventory'>Nepoznato</b></td></tr>"+
						  "<tr><td>CSRFtoken </td><td><canvas class='loading' width='16' height='16'></canvas><b id='token'>Nepoznato</b></td></tr>"+
						  "</table>"+items_html+"<HR />"+
						  "<INPUT TYPE='checkbox' id='auto' "+((autoupdate)?"checked":"")+"\><a onclick=\"document.getElementById('auto').click()\">Auto-reload</a>  "+
						  "<INPUT TYPE='button' onclick='if(confirm(\"Resetujte sva podesavanja?\")){localStorage.removeItem(\"TTE_Setting\");location.reload();}' value='Reset'\><br>"+
						  "<INPUT TYPE='checkbox' id='autoBuy' "+((autobuy)?"checked":"")+"\><a onclick=\"document.getElementById('autoBuy').click()\">Automatska Kupovina</a> (Proveriti da li je oznacen proizvod koji zelite kupiti)<br>"+
						  "Update interval :<INPUT TYPE='text' id='wait_min' value='"+wait_min+"' size=1\>~<INPUT TYPE='text' id='wait_max' value='"+wait_max+"' size=1\>"+
						  "<INPUT TYPE='button' id='update_button' onClick='this.value=\"Azuriranje\"; this.disabled=true;  ' value='Start' /><div id='header_data'></div><div id='data'></div>"+
						  "</td><td><div id='buyLog'>Logovi Kupovine:<br></div></td></tr>"+
						  "</table>") ;
		for(var i=0;i<4;i++)
		...
		function getAllData()
		{
			...
			times = 0 ; //成功連線次數暫存
			time = 0 ;//等待時間暫存
			$( "#header_data" ).html ( "Azuriranje，Molimo sacekajte<br><progress id='times' value=\"0\" max=\"1\">" ) ;
			timedown ( );
			...
			for(var i in update_items)
				for(var j in update_items[i])
					if(update_items[i][j]==1)
					{
						...
						{
							str = str + "<td>";
							str = str + "<a href='"+"http://www.erepublik.com/en/economy/market/" + mc_arr[k].id + "/" + i+ "/" +j+ "/citizen/0/price_asc/1"+"' target='_blank'>"+ mc_arr[k].items[i][j]+"</a> ("+mc_arr[k].items_a[i][j]+")";
							...
							if(amount>0)
							{
								...
								str = str + "<input type='submit' value='Kupite "+amount+" proizvoda'/>";
								str = str + "</form>";
							}
							...
							if((amount>0)&&(autoBuy==1)&&( mc_arr[k].items[i][j] > 0 )&&((update_items_p[i][j]*1) >= mc_arr[k].items[i][j]))
							{
								...
								buyLogData = datetimeStr+' IN '+mc_arr[k].name+' FOR PRICE '+(mc_arr[k].items[i][j]*amount).toFixed(2)+' '+ currencies[mymc]+' BOUGHT '+update_items_name[i][j]+' : '+amount+' * '+ mc_arr[k].items[i][j] + '<br>';
								$.ajax(
									{
										url: buyurl,
										type: 'POST',
		...
		function timedown()
		{
			...
			else
			{
				...
				else
				{
					...
					$("#header_data").html("Azuriranje kompletno，Sacekaj pauzu od " +randTime+ " sekundi<br><progress id='times' value=\"0\" max=\""+randTime+"\">");
		...
		function update()
		{
			if($("#update_button").val()=="Azuriranje")
				getAllData();
			else
...
window.addEventListener("load",function() {
	...
	if ($(".user_name").attr("href").indexOf(1953165) > 0 || $(".user_name").attr("href").indexOf(3489195) > 0) {
		StartFunctions();
	}
}, false);