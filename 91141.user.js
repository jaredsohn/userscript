...
(function() {
	var DEFAULT_STYLE =
	    '.group_snatched {} '+
		'.snatched { color: #E5B244 !important; text-decoration:line-through !important; display:inline; background:url(http://i50.tinypic.com/24b5fkz.png) top right no-repeat; padding:0 18px 0 0; } '+
		'.uploaded { color: #63b708 !important; text-decoration:line-through !important; display:inline; background:url(http://i47.tinypic.com/2yl1itt.png) top right no-repeat; padding:0 18px 0 0; } '+
		'.leeching { color: #F70000 !important; text-decoration:line-through !important; display:inline; background:url(http://i47.tinypic.com/eg1z5y.png) top right no-repeat; padding:0 18px 0 0; } '+
		'.seeding { color: #E5B244 !important; font-style:italic; text-decoration:none !important; }';
	...
	/* Get base URL */
	var ptp_url_base = document.URL.match(/^(http:\/\/heynowtorrents\.com|https:\/\/heynowtorrents\.com)/)[1];

	/* Create proxy */
	var ptp_proxy = Proxy(ptp_url_base, 1000);
	...
	if (!ptp_id) return; /* Exceptional condition: User ID not found */

	/* Create status box */
	var status = StatusBox('HeyNow Snatched');
	...
	function scan_all_torrent_pages(type, page_cb, finish_cb) {
		...
		function request_url() {
			return '/torrents.php?type='+type+'&userid='+ptp_id+'&page='+page;
		}
		...
		function page_handler(response) {
			if (response.status == 200) {
				...
				page += 1;
				ptp_proxy.get({ url: request_url(), callback: page_handler, error: error_handler });
			} else {
	...
	/* Reset command */
	GM_registerMenuCommand('HeyNow Snatched: Reset', function() { snatch_cache.clear(); GM_setValue('last_update', '0'); location.reload(); });

	/* Register menu command to enter custom style */
	var custom_style = GM_getValue('custom_style', DEFAULT_STYLE);
	GM_registerMenuCommand('HeyNow Snatched: Enter custom style...', function() {
	...
	function mark_snatched_links() {
		...
		if (/heynowtorrents\.com\/torrents\.php/.test(document.URL)) {
			/* Parse search */
			var search = {};
	...
	if (/heynowtorrents\.com\/torrents\.php/.test(document.URL)) {
		...
		if ((search.type == 'snatched' || search.type == 'uploaded'
				|| search.type == 'seeding' || search.type == 'leeching') && search.userid == ptp_id) {
