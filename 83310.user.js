// ==UserScript==
// @name           Links2tabs
// @namespace      http://userscripts.org/scripts/show/83310
// @description    Force links with no target to open in a new tab.
// @include        *
...
GM_config.init(
	'Option', {
		'excludes': {
			'section': ['Exclude', 'Exclude URLS: (One pr. line, regular expression syntax)'],
			'label': 'Excludes',
			'type': 'textarea',
			'cols': 80,
			'rows': 10
		...
		'noprotocol': {
			'section': ['Further', 'Specific options.'],
			'label': 'Open links without protocol in current tab',
			'type': 'checkbox',
			'default': 'checked'
		},
		'samedomain': {
			'label': 'Open links from same domain in current tab',
			'type': 'checkbox',
			'default': 'checked'
		},
	}
...
GM_registerMenuCommand("Links2Tabs", function(){GM_config.open();});

links = document.links;

var ex = GM_config.get('excludes').split("\n");

// Iterate through links and add those not matched by excludelist
...
// Iterate through (remaining) links and add target according to the settings
for(i=0;i<toCheck.length;i++) {
	links[toCheck[i]].setAttribute('target', '_blank');
	if (links[toCheck[i]].href.indexOf('://') == -1) {
		if (GM_config.get('noprotocol')) {
			links[toCheck[i]].setAttribute('target', '_self');
		}
	}
	else {
		if (GM_config.get('samedomain')) {
			var cur_p = document.location.href.split('://');
			var cur_d = cur_p[1].split('/', 1);
			var p = links[toCheck[i]].href.split('://');
			var ld = p[1].split("/", 1);
			if (cur_d[0] == ld[0]) {
				links[toCheck[i]].setAttribute('target', '_self');
			}
