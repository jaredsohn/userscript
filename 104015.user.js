...
(function(){
	...
	if(/screen=place/.test(location.href) && /try=confirm/.test(location.href)) {
		...
		if(coords) {
			var td = $x('//form[contains(@action,"screen=place") and contains(@action,"try=confirm")]/descendant::a[contains(@onclick,"mode=recent")]');
			if(td) {
				...
				if(coords.length == 3) {
					var link = document.createElement('a');
					var id = /village=(\d+)/.exec(td[0].getAttribute('onclick'));
