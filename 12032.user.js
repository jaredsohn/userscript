// ==UserScript==
// @name        Fast Look up YahooJp.BlogSearch
// @namespace   http://d.hatena.ne.jp/hatecha/
// @description search on YahooJp.BlogSearch by selected word
// @include     http://*
// ==/UserScript==
...
(function () {
	var popups = [];
	limit =100;
	var urllink = 'http://blog-search.yahoo.co.jp/';
	var urlbase = 'http://blog-search.yahoo.co.jp/search?p=';
	...
	window.addEventListener("mouseup", function (e) {
		...
		GM_xmlhttpRequest({
			method: 'get',
			url: urlbase +EscapeEUCJP(selection),
			onload : function (req) {
				var t = $N('div');
				var count =0;
...
/*
				$X('.//center/table/tbody/tr/td/table', t).forEach(function (e) {
					replUrl(e);
					area.appendChild(e);
					area.appendChild($N('hr'));
				});
*/
				$X('.//div/ol/li', t).forEach(function (e) {
					count ++;
					//replUrl(e);
					area.appendChild(e);
				});
				...
				if( count>0 ){
					area.appendChild(linkmk(urlbase+EscapeEUCJP(selection),">> YahooBlgSearch: "+selection));
					popups.push(area);
