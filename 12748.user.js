...
document.getOrdinals = function(column) {
  ...
  for (var i = 0; i < images.length; i++) {
    if (images[i].hasAttribute("src")) {
      ...
      if (imageSrc != null) {
        // Only one image will have a query string, the image with the graph
        var queryString = imageSrc.substring(imageSrc.indexOf("?")); 
        var queryAttributes = queryString.split("&");
        for (var j = 0; j < queryAttributes.length; j++) {
          // Look for the training string.  And parse it to find the ordinal values.
          if (queryAttributes[j].indexOf("playertraining") > -1) {
            var graphValues = (queryAttributes[j].substring(queryAttributes[j].indexOf("=")+1)).split(";");
...
document.constructOrdinalTable = function(ordinals, column) {
  ...
  intensity.innerHTML = "Intensitet";
...
(function() {
  var ordinalValues = document.getOrdinals(document.getElementById("column2"));
