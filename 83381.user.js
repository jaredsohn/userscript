// ==UserScript==
// @name           MyEpisodes.com Links to SceneSource
// @namespace      ghoofy007
...
for(var i=0 ; i < mylistRows.snapshotLength ; i++){
   ...
   if(row.className == "header"){
      // Header row, need only extend it to add src colum
...
if(seasonHeaders.length>0){
   // Add Episodes by Show link to find all episodes on Scnscr.me
   var offset = 11;
...
// Create the table cell (td) that contains the scenesource link.
function getSRCcell(linkURL, fadeLink){
...
// Create the SceneSource search URL
