// ==UserScript==
// @name           Xbox.com Achievement Timestamps
...
window.addEventListener("load", function(e) {
	...
	function getTimestamp(gamer){
		...
		if(tempAch.innerHTML != "unlocked"){  /*offline achievements will simply have the text "unlocked"*/
			...
			searchAch = searchAch.replace(/(&lt;)/g,"(\\\\u003c)");     /*deal with achievements that have less than sign*/
			searchAch = searchAch.replace(/(&gt;)/g,"(\\\\u003e)");     /*deal with achievements that have greater than sign*/
			searchAch = searchAch.replace(/(&amp;)/g,"(\u0026)");     /*deal with achievements that have ampersand (&)*/
			searchAch = searchAch.replace(/[(]/g,"");     /*deal with achievements that have left paren*/
			searchAch = searchAch.replace(/[)]/g,"");     /*deal with achievements that have right paren*/
			searchAch = searchAch.replace(/[\*]/g,"(\\*)");     /*deal with achievements that have */
            searchAch = searchAch.replace(/[\?]/g,"(\\?)");     /*deal with achievements that have ?*/
            searchAch = searchAch.replace(/[\r]/g,"");     /*deal with achievements that have \r*/
            searchAch = searchAch.replace(/[\n]/g,"");     /*deal with achievements that have \n*/
			...
			for(j=0;j<rawData.length;j++){	/*loop through the raw data to find timestamp for this achievement*/
                ...
                d = d.replace(/[(]/g,"");     /*deal with achievements that have left paren*/
				d = d.replace(/[)]/g,"");     /*deal with achievements that have right paren*/
			...
			if(achFound == -1 && searchAch != "secret achievement"){	/*if the achievement was not found anywhere (this should never be true)*/
			...
			if(achData.search(gamer) != -1){         /*search the acheivement data to see if the gamertag of current "gamer" is found*/
				...
				if(dateRaw != null){
					...
					if(milli < 10)
					   ...
					   milli = "0" + milli;
					formattedDate += "." + milli;  /*add the milliseconds to timestamp*/
