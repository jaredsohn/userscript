...
const RULES = new Array(
  // Block in sidebar
  ".feedsBlock {-moz-border-radius: 5px; background: #66CC00; margin: 10px 7px 0 0; padding: 3px;}",
  ".refreshButton {display: block; cursor: pointer; float: right; margin-top: -2px;}",
  ".feedsBlockList {padding-left: 5px; background: white; overflow: hidden; display: none;}",
  ".feedListItem {color: #66CC33;}",
  ".feedListItemUnread {color: #66CC33; font-weight: bold;}",
  ".showAllLink {background:white; text-align: right; color: #66CC33; padding: 2px 0px 5px 0;}"
);
...
function initializeReader() {
  ...
  readerText.appendChild(newText(" Reader"));
  ...
  if (getCookie(READER_SHOW_ALL_FEEDS_COOKIE) == "0") {
    showAllLinkText = newText("Show All Feeds");
    showAllFeeds = false;
  } else {
    showAllLinkText = newText("Only Updated");
    showAllFeeds = true;
  }
...
function toggleShowAllFeeds() {
  ...
  if (showAllFeeds) {
    showAllLinkText.nodeValue = "Show All Feeds";
    showAllFeeds = false;
    setCookie(READER_SHOW_ALL_FEEDS_COOKIE, "0");
  } else {
    showAllLinkText.nodeValue = "Only Updated";
    showAllFeeds = true;
