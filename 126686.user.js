// ==UserScript==
// @name         Röfi készlet
// @namespace    http://userscripts.org/scripts/show/126454
// @version      1.0
// @description  Röfi szmájlik és a The Westtel kapcsolatos honlapok
// @author      w.vikice
// @include	http://*.the-west.*/game.php*
// @require	http://code.jquery.com/jquery-1.7.1.min.js
// @history	2012.02.25. Röfi készlet elkészült - Röfiszmájlik, Honlap gyorsgombok és "Egy kis malac" vers első versszaka
// ==/UserScript==

var off_img = 

"data:image/png;base64,";

var on_img = 

"data:image/png;base64,";

var helptk =
"data:image/png;base64,";

var wino =
"data:image/png;base64,";

var rofi_smly =
"data:image/png;base64,";

var twdb =
"data:image/png;base64,";

var forts =
"data:image/png;base64,";

var nyan =
"data:image/png;base64,";

var whp_img =
"data:image/png;base64,";

var on_checker = 0;
var name1 = '';
var name2 = '';
var nameswitch = 0;


var popup = document.createElement('div');
    popup.setAttribute('id', 'popupwindow');
    popup.setAttribute('style', 'position: absolute;visibility: hidden;overflow: hidden;border:2px solid #CCC;z-index: 1000000;background-color: rgba(255,140,255,0.6);border:2px solid  #333;padding:5px;width: 230px;height: 125px;top: 250px;right: 8px;');
var popup_header = '<p align="center"><font face="Arial" color="white" size="1"><b><img src=' +off_img+ ' /> Röfi készlet <img src=' +on_img+ ' /></b></font>';
    popup.innerHTML = popup_header;


var on_off = document.createElement('div');
    on_off.setAttribute('id', 'on_off');
    on_off.setAttribute('title', 'Röfi bekapcsolása');
    on_off.setAttribute('style', 'position:absolute;z-index:1000000;width:15px;cursor:pointer;text-align:center;background-color: rgba(0,0,0,0);font-size:12px;padding:5px 5px 0px 2px;left:50px;');
    on_off.innerHTML = '<img id="on_off" src=' +off_img+ ' width="16px" heoght="16px" />';
    on_off.onclick = function(){

popup.innerHTML = popup_header + '<p><font size="2" color="#5A005A"><b>Röfi dal</b></font></p><p align="center"><font size="2" color="#5A005A" face="Arial"><i>Egy kis malac, röf- röf- röf,</i></font></p><p align="center"><font size="2" color="#5A005A" face="Arial"><i>Trombitálgat, töf- töf- töf,</i></font></p><p align="center"><font size="2" color="#5A005A" face="Arial"><i>Trombitája víg ormánya,</i></font></p><p align="center"><font size="2" color="#5A005A" face="Arial"><i>földet túrja, döf- döf- döf.</i></font></p><p align="center"><font size="2" color="white"> (röff-röff> <img src=' +rofi_smly+ ' /><font color="white" size="2">☞ </font><a href="http://www.whp.atw.hu" target="new blank"><img src=' +whp_img+ ' /></a> <a href="http://tw-db.info/" target="new blank"><img src=' +twdb+ ' /></a> <a href="http://www.west-help.tk" target="new blank"><img src=' +helptk+ ' /></a> <a href="http://www.wini.lapunk.hu" target="new blank"><img src=' +wino+ ' /></a> <a href="http://www.westforts.com/" target="new blank"><img src=' +forts+ ' /></a> <a href="http://nyanit.com/www.the-west.hu/" target="new blank"><img src=' +nyan+ ' /></a></p>';

if (on_checker<1) {
popup.style.visibility = 'visible';
on_off.innerHTML = '<img id="on_off" src=' +on_img+ ' width="16px" heoght="16px" />';
on_off.title = "Röfi kikapcsolása";
on_checker = 2;
document.addEventListener('click',getnames,false);
 }
else {
popup.style.visibility = 'hidden';
on_off.innerHTML = '<img id="on_off" src=' +off_img+ ' width="16px" heoght="16px" />';
on_off.title = "Röfi bekapcsolása";
on_checker = 0;
document.removeEventListener('click',getnames,false);
 }

 


   };

document.body.appendChild(on_off);
document.body.appendChild(popup);
