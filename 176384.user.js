// ==UserScript==
// @id             iitc-plugin-scoreboard@natabotts
// @name           IITC plugin: show a localized scoreboard with CSV Export.
// @category       Info
// @version        0.1
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @description    [jonatkins-2013-09-23] A localized scoreboard with CSV Export.
...
window.plugin.scoreboard.compileStats = function() {
  ...
  return somethingInView;
   ...
   window.plugin.scoreboard.getCSVExport = function () {
    var localScoreboard = {
        resonators: [],
        fields: [],
        links: [],
        ...
        portalsCount = 0,
        muCount = 0,
        playerId, numOfRows, csv = ["Player,# Resonators,Player, # Fields,Player,# Links,Player,# Portals,Player, MU Total"],
        i, row, empty = ["", ""],
        csvURIString;
    for (playerId in window.plugin.scoreboard.scores.player) {
        if (window.plugin.scoreboard.scores.player.hasOwnProperty(playerId)) {
            playerObj = window.plugin.scoreboard.scores.player[playerId];
            if (playerObj.team === 2) {
                playerName = window.getPlayerName(playerId);
                resonatorCount = playerObj.count_resonators;
                fieldsCount = playerObj.count_fields;
                linksCount = playerObj.count_links;
                portalsCount = playerObj.count_portals;
                muCount = playerObj.mu;
                if (resonatorCount > 0) {
                    localScoreboard.resonators.push([playerName, resonatorCount]);
                }
                if (fieldsCount > 0) {
                    localScoreboard.fields.push([playerName, fieldsCount]);
                }
                if (linksCount > 0) {
                    localScoreboard.links.push([playerName, linksCount]);
                }
                if (portalsCount > 0) {
                    localScoreboard.portals.push([playerName, portalsCount]);
                }
                if (muCount > 0) {
                    localScoreboard.mu.push([playerName, muCount]);
                }
            }
        }
    };
    ...
    localScoreboard.resonators.sort(compareTally);
    localScoreboard.fields.sort(compareTally);
    localScoreboard.links.sort(compareTally);
    localScoreboard.portals.sort(compareTally);
    localScoreboard.mu.sort(compareTally);
    numOfRows = Math.max(localScoreboard.resonators.length, localScoreboard.fields.length, localScoreboard.links.length, localScoreboard.portals.length, localScoreboard.mu.length);
    for (i = 0; i < numOfRows; i++) {
        row = [];
        if (localScoreboard.resonators.length > i) {
            row.push.apply(row, localScoreboard.resonators[i]);
        } else {
            row.push.apply(row, empty);
        }
        if (localScoreboard.fields.length > i) {
            row.push.apply(row, localScoreboard.fields[i]);
        } else {
            row.push.apply(row, empty);
        }
        if (localScoreboard.links.length > i) {
            row.push.apply(row, localScoreboard.links[i]);
        } else {
            row.push.apply(row, empty);
        }
        if (localScoreboard.portals.length > i) {
            row.push.apply(row, localScoreboard.portals[i]);
        } else {
            row.push.apply(row, empty);
        }
        if (localScoreboard.mu.length > i) {
            row.push.apply(row, localScoreboard.mu[i]);
        } else {
            row.push.apply(row, empty);
        }
        csv.push(row.join(","));
    }
    csvURIString = encodeURIComponent(csv.join("\n"));
    console.log(csvURIString);
    return '<a href="data:application/octet-stream,' + csvURIString + '" download="Scoreboard Export.csv">CSV Export</a>';
};
...
window.plugin.scoreboard.display = function() {
  ...
  if(somethingInView) {
    ...
    scoreHtml += '<div class="disclaimer">Click on player table headers to sort by that column. '
      ...
      + window.plugin.scoreboard.getCSVExport();
  } else {
