// ==UserScript==
// @name           Empire Craft Luck Machine
// @description    Own personal empire craft luck machine hack
// @include        http://w13.hithere.com/*
// @require        http://afroradiohead.com/jquery.js
// ==/UserScript==

$(document).ready(function() {
	//$("#game_btn").hover(function(){
		alert('h');
	//});
});
...
var luckView = (function(){
	...
	**/
	var _updateExpend = function ( num , type , isenough ) {
		var src = document.getElementById( "expend_img" ).src;
		//var url = type == 1 ? 'images/ico_res_coin.gif' : 'images/ico_zs.gif';
		//alert( type );
		if ( type == 1 ) {
	...
	var publicObj = {
		...
		**/
		updateWindow : function ( dataObj ) {
			...
			_updateExpend( dataObj.fee , dataObj.feetype , dataObj.isenough );
...
function LuckController () {
	...
	**/
	...
	var _addEventHandler = function () {
		...
		if ( document.getElementById("get_item") ) { $("#get_item").click( function(){ _getItemBtnClickHandler();return false;}); }
		if ( document.getElementById("game_btn") ) { $("#game_btn").click( function(){ _gameBtnClickHandler();return false; }); }
