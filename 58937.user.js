// ==UserScript==
// @name           Duel logovnitsa
// @namespace      www.the-west.rus
// @description    Send duel report to site.
// @description    Based on The West Reproter_skcz
...
(function(){
	...
	function whiteSpaceRemove2(str) {
		s = str.replace(/\s+/g,'');
		return s;
	}
	...
	function convertTitle(title, player1, player2){
		...
		title = title.replace(p1_name,'&n1='+escape(p1_name)+'&l1='+p1_level+'&r1='+p1_dlevel);
		title = title.replace(p2_name,'&n2='+escape(p2_name)+'&l2='+p2_level+'&r2='+p2_dlevel);
		title = title.replace('Дуэль: ','');
	...
	function convertDuelReport(div) {
		...
		loc = loc.replace('Место происшествия: ','');
		loc = loc.replace(' в ','_in_');
		loc = escape(loc);
		loc = loc.replace('_tm_','&date=');
		loc = loc.replace('_in_','&time=');
		...
		do{ 
			old = hits;
			hits = hits.replace('Мимо','/a');
			hits = hits.replace(' - ',';');
			hits = hits.replace('Левая рука','/b');
			hits = hits.replace('Левое плечо','/c');
			hits = hits.replace('Голова','/d');
			hits = hits.replace('Правое плечо','/e');
			hits = hits.replace('Правая рука','/f');
			hits = hits.replace('Суммарный урон','/s');
		}
		while (old != hits);
		hits = whiteSpaceRemove2(hits);
		hits = hits.substring(hits.indexOf('/'));
		ii = 1;
		do{
			old = hits;
			hits = hits.replace('/','&lhit'+ii+'=');
			hits = hits.replace('/','&rhit'+ii+'=');
			++ii;
		}
		while (old != hits);
		hits=hits.replace('\n','');
		outcome = escape(outcome.replace('\n',''));
		...
		url = 'http://west-logs.ru/log.hph?null=null';
		code = url+code;
//		code = '[code]'+title+loc+hits+outcome+'[/code]';
		...
		http_reques.open('GET',code,true);
		http_reques.send(null);
	...
	function hookReport(div) {
		...
		if (!titleRow.textContent.match(/Дуэль:/)&&!titleRow.textContent.match(/Duel:/))
			return;
		var t = dc({
			"th": { 
				tag: "th", 
