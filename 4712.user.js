...
if( !done ) {
  if( location.pathname.match( 'publish-body.g' ) ) {
    ...
    moveTagsToHeader();
  }//end if-else location.pathname.match publish-body.g
}//end if ! done
...
function doCleanup() {//clean up GM_vars
   var myid = getBlogId();
   GM_setValue( 'postdata-'+myid, '' );//clear post data
   GM_setValue( 'postbody-'+myid, '' );//clear body, trackback, and link data so we don't send trackbacks twice
   GM_setValue( 'posttrackback-'+myid, '' );
   GM_setValue( 'postlink-'+myid, '' );
   GM_setValue( 'posttitle-'+myid, '' );
   GM_setValue( 'postblogname-'+myid, '' );
...
function blogger_trackback_submit(trackbackurl) {
   var out = document.getElementById('blogger_trackback_status');
   ...
   GM_xmlhttpRequest({'method':'POST',
                      ...
                      'data':'url='+encodeURIComponent(url)
                            ...
                            +'&blog_name='+encodeURIComponent(name)
...
function doTrackback() {
   var myid = getBlogId();
   excerpt = GM_getValue( 'postbody-'+myid, '' );//get body, trackback, title, and link data for post
   url = GM_getValue( 'postlink-'+myid, '' );
   title = GM_getValue( 'posttitle-'+myid, '' );
   name = GM_getValue( 'postblogname-'+myid, '' );
   var trackbacks = GM_getValue( 'posttrackback-'+myid, '' ).split(/\n/);
   if(!trackbacks || !trackbacks[0] || !url)
      return;
   for(i in trackbacks)
      blogger_trackback_submit(trackbacks[i]);
   var out = document.getElementById('blogger_trackback_status');
...
function linkDelicious()
{
  ...
  if( url )
  {
    ...
    if(GM_getValue( 'postdata-'+myid, '' ))//only write the del.icio.us link if there is post data
  ...
  GM_setValue( 'postlink-'+myid, url );//set link for use by trackback
  try{doTrackback();}catch(e){alert(e);}//fire trackback (here to make use of setTimeout above)
...
function storePostData()
{
  ...
  var dtstamp = time + (divs.link ? ' ' + divs.link.value : '');
  var postbody = $('richeditorframe').contentDocument.body.innerHTML.replace(/&nbsp;/,' ');
  postbody = stripHTML(postbody).substr(0,200) + '...';
  var extstyle = GM_getValue( 'extstyle-'+myid );
  var data = { title : divs.title ? divs.title.value : '',
	       extended : (extstyle == 'dtstamp' ? dtstamp : postbody),
	       tags : tags
             };
  var save = '';
  for( var i in data )
  ...
  GM_setValue( 'postbody-'+myid, postbody );
  GM_setValue( 'posttitle-'+myid, data.title );
  GM_setValue( 'posttrackback-'+myid, divs.trackback.value );
  GM_setValue( 'postblogname-'+myid, getBlogName() );
}
...
function injectTagsField( divs, tags )
{
  ...
  var trackbackRow = tagRow.cloneNode(false);
  ...
  if( before ) {
    ...
    before.parentNode.insertBefore( trackbackRow, before );
  } else {
    divs.head.appendChild( tagRow );
    divs.head.appendChild( trackbackRow );
  }//end if before
  if( blogthis ) {
    tagRow.innerHTML = '<label id="TagsConfig" for="tags" style="color:blue;">Tags:</label>' +
      '<span class="errorbox-good">' +
      '<input id="tags" class="titlefield" ' +
      ' style="font-size:11px;width:180px;" tabindex="1"' +
      ' value="'+ tags +'">' +
      '</span>';
    trackbackRow.innerHTML = '<label for="trackback">Trackback:</label>' +
      '<span class="errorbox-good">' +
    ...
    tagRow.innerHTML = '<th><label id="TagsConfig" for="tags" style="color:blue;">Tags:' +
      '</label></th><td><div class="errorbox-good">' +
      '<input id="tags" class="text" style="width:657px" tabindex="1" ' +
      ' value="'+ tags +'">' +
      '</div></td>';
    trackbackRow.innerHTML = '<th><label for="trackback">Trackback:' +
      '</label></th><td><div class="errorbox-good">' +
