...
// 07/08/2010 FHEL2 logging added
//            Specific user attack count getting reset fixed
//            Added restart and heal on stop features
//            Added ice count criteria for stop
// 18/08/2010 Lost track of previous changes *thumbs up*
//            Victory points logging, change etc logged
// 21/08/2010 Minor change for new game layout
// 25/08/2010 Banking call doesn't require temp key/cb variables now
// 02/09/2010 Power attack and attack button text changed
// 03/10/2010 Changed xpath expressions for fightlist loading
...
//    var vault_tmp_variable='', vault_cb_variable='';
    ...
    var table_html='<form id="something">'+
            ...
            '<tr>'+
            '<th width="50%" style="font-size:20px; padding-left:15px;text-align: left">Brawler RPS</th>'+
			'<th width="48%" style="font-size:12px; text-align:right"> <a id="Website" align="right" href="http://rpstools.ourtoolbar.com/" target="_blank">RPS Toolbars</a> - <a id="Forum" align="right" href="http://www.facebook.com/group.php?gid=130491836991522" target="_blank">RPS GROUP</a> - <a id="Donate" href="http://arun.keen-computing.co.uk/?page_id=31" target="_blank">Donate</a></th>'+
            ...
            '<td width="9%"><a id="pause" href="#" style="display:none"><img align="right" alt="Pause" src="http://codeoutpost.com/Scripts/pause.png"></img></a>'+
            '<a id="begin" href="#" style="display:inline"><img align="right" alt="Start" src="http://codeoutpost.com/Scripts/play.png"></img></a></td>'+
            '</tr></table>'+
			'</tr>'+
            ...
            '<td width="1%">:</td>'+
            '<td width="89%"><input type="checkbox" id="restart_enable"> Restart in <input type=text id="restart_min" value="1" class="sexy_input" style="width:25px" onkeydown="return field_validate(event.keyCode);"> minutes, '+
            '<input type="checkbox" id="finish_heal_enable"> Heal '+
//            'Travel to <select class="sexy_select" id="travel_city">'+
//            '<option value="1">New York</option>'+
//            '<option value="2">Cuba</option>'+
//            '<option value="3">Moscow</option>'+
//            '<option value="4">Bangkok</option>'+
//            '<option value="5">Las Vegas</option>'+
//            '</select>'+
            '</td>'+
            '</tr></table>'+
			'</tr>'+
            ...
            '<td width="1%">:</td>'+	
			'<td width="5%"><span class="good" id="percentage">0%</span></td>'+
			'</tr>'+
			'<tr>'+
...
//        alert(fighttable.snapshotLength);
        for(i=1; i<fighttable.snapshotLength; i++){
            ...
            fightlist_iced[fightlist_iced.length] = (fighttable.snapshotItem(i).getElementsByTagName('td')[0].innerHTML);
            if(fight_city==4){
...
//        alert(user_urls);
    ...
    function attack_user(){
        logmsg('attacking user ' + user_names[j],'attack_log');
...
//        document.getElementById('user_experience').innerHTML = /user_fields\['user_experience'\] = parseInt\("([0-9]+)/.exec(attack_results)[1];
        exp_to_levelup = parseInt(/user_fields\['exp_for_next_level'\] = parseInt\("([0-9]+)/.exec(attack_results)[1]) - parseInt(/user_fields\['user_experience'\] = parseInt\("([0-9]+)/.exec(attack_results)[1]);
        try{document.getElementById('exp_to_next_level').innerHTML = exp_to_levelup;}
        catch(err){document.getElementById('user_xp_to_next_level').innerHTML = exp_to_levelup;}
        try{document.getElementById('user_experience').innerHTML = /user_fields\['user_experience'\] = parseInt\("([0-9]+)/.exec(attack_results)[1];}
        catch(err){}
        ...
        mafia_attack = parseInt((/title="Mafia Attack Strength".+?> (.+?)<\/div>/.exec(attack_results)[1]).replace(/,/g,''));
        if(initial_mafia_attack == 0){
            initial_mafia_attack = mafia_attack;
        }
        mafia_defense = parseInt((/title="Mafia Defense Strength".+?> (.+?)<\/div>/.exec(attack_results)[1]).replace(/,/g,''));
        ...
        document.getElementById('attack_diff').innerHTML = (mafia_attack >= initial_mafia_attack)?'<span class="good">+'+(mafia_attack - initial_mafia_attack)+'</span>':'<span class="bad">-'+(initial_mafia_attack - mafia_attack)+'</span>';
        document.getElementById('defense_diff').innerHTML = (mafia_defense >= initial_mafia_defense)?'<span class="good">+'+(mafia_defense - initial_mafia_defense)+'</span>':'<span class="bad">-'+(initial_mafia_defense - mafia_defense)+'</span>';
        if(vic_pts == 0){
            initial_vic_pts = /<div class="fightmastery_tokens">[^\d]+(\d+)/.exec(attack_results)[1];
        }
        vic_pts = /<div class="fightmastery_tokens">[^\d]+(\d+)/.exec(attack_results)[1];
        document.getElementById('Victorycoins').innerHTML = vic_pts + '[<span class="good">'+(vic_pts-initial_vic_pts)+'</span>]';
        
        
        try{
            temp='';
            loot_log = [];
            if(/(found|gained|earned) (some|an|a|A|1) (.+?)(See|while|\.|)/.test(attack_results)){
                temp_loot = attack_results.match(/(<div class="fightres_bonus_message".+?<img src="(.+?)".+?|)(found|gained|earned) (some|an|a|A|1) (.+?)(See|while|\.)/g);
                for(z=0;z<temp_loot.length;z++){
                    temp = /(found|gained|earned) (some|an|a|A|1) (.+?)(See|while|\.)/.exec(temp_loot[z]);
                    loot_img = /<div class="fightres_bonus_message".+?<img src="(.+?)"/.exec(temp_loot[z]);
                    loot_img = loot_img ? loot_img[1]:"";
                    temp = temp[3].replace(/!/g,'');
                    ...
                    if(loot_img!=""){
                    	...
                    	loot_log[loot_log.length] =  ' '+temp;
                    }
                }
            }
        }        
        catch(err){alert('Error ! '+err);alert('temp_loot length = '+temp_loot.length);alert('Error occured, temp_loot = '+temp_loot);return;}
        
        try{
            if(/secret stash/.test(attack_results)){
                var stash_script = /function continuation_popFightLootFeed(.+?)<\/script>/.exec(attack_results)[0].replace(/<\/script>/,'');
                ...
                document.getElementById('stash_posts_log').innerHTML += '<a'+stash_owner+'<\/a> - <a href="#" id="stash_post'+(stash_posts.length-1)+'" onclick="return false;">Stash #'+stash_posts.length+'</a><br>';
//                document.getElementById('stash_post'+(stash_posts.length-1)).onclick = new Function(this.id,publish_stash);
        ...
        try{
            if(/Share Coins/.test(attack_results)){
                var coins_script = /function continuation_postW2Wback(.+?)<\/script>/.exec(attack_results)[0].replace(/<\/script>/,'');
                coins_script = coins_script.replace(/"/g,'\"');
                coin_posts[coin_posts.length] = coins_script;
                var coin_post_owner = /<a(.+?)<\/a><\/span> <span>earned you \d extra Victory Coins/.exec(attack_results)[1];
                coin_owner_id[coin_owner_id.length] = /postW2Wback\('(.+?)'\)/.exec(attack_results)[1];
                document.getElementById('coin_posts_log').innerHTML += '<a'+coin_post_owner+'<\/a> - <a href="#" id="coin_post'+(coin_posts.length-1)+'" onclick="return false;">VCoin Post #'+coin_posts.length+'</a><br>';
//                document.getElementById('stash_post'+(stash_posts.length-1)).onclick = new Function(this.id,publish_stash);
                for(l=0;l<coin_posts.length;l++){
        ...
        catch(err){$('#inner_page').html(attack_results);alert('Error ! '+err);return;}
        ...
        if(/killed your opponent/.test(attack_results)){
            killed=true;
            kills++;
            document.getElementById('ice_posts_log').innerHTML += 'Killed: ' + fightlist_names[i] + '<br>';
            killed_count = /You killed your opponent, bringing your total body count to (\d+?)!/.exec(attack_results)[1];
        ...
        if(/iced_pop_text/.test(attack_results)){
            iced=true;
            ices++;
            iced_count = /<div class=\\"iced_pop_body_count_number\\">(.+?)<\/div>/.exec(attack_results)[1].replace(/,/g,'');
            document.getElementById('iced_log').innerHTML = iced_count +'[<span class="good">'+ices+'</span>]';
            var script = /function continuation_postFeedAndSendFightBrag(.+?)<\/script>/.exec(attack_results)[0].replace(/<\/script>/,'');
            ...
            document.getElementById('ice_posts_log').innerHTML += iced_posts[iced_posts.length-1][0]+' - '+fightlist_names[i]+' - <a href="#" id="ice_post'+(iced_posts.length-1)+'"return false;">Post</a><br>';
//            document.getElementById('ice_post'+(iced_posts.length-1)).onclick = new Function(this.id,publish_ice);
        ...
        if(check_city != fight_city){
            ...
            url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+check_city+"&cb="+cb+"&destination="+fight_city+"&from=fight&nextParams=&xw_client_id=8",
                success: function (msg){
                    attack_or_not=true;
                    loadfightpage();
                    return;
        ...
        try{
            if(fight_city == 4){
                ...
                document.getElementById('Yakuza_points').innerHTML = Yakuza = parseInt(/zy_progress_bar_faction_text">(.+) \/ 1500/.exec(faction_level[0])[1]);
                document.getElementById('Triad_points').innerHTML = Triad = parseInt(/zy_progress_bar_faction_text">(.+) \/ 1500/.exec(faction_level[1])[1]);
            }
        }
        catch(err){}
        ...
        switch(parseInt(fight_city)){
            case 1:
                document.getElementById('user_cash_nyc').innerHTML = /user_fields\['user_cash_nyc'\] = "([^"]+)/.exec(attack_results)[1];
            ...
            break;
            case 2:
                document.getElementById('user_cash_cuba').innerHTML = /user_fields\['user_cash_cuba'\] = "([^"]+)/.exec(attack_results)[1];
            ...
            break;
            case 3:
                document.getElementById('user_cash_moscow').innerHTML = /user_fields\['user_cash_moscow'\] = "([^"]+)/.exec(attack_results)[1];
            ...
            break;
            case 4:
                document.getElementById('user_cash_bangkok').innerHTML = /user_fields\['user_cash_bangkok'\] = "([^"]+)/.exec(attack_results)[1];
            ...
            break;
            case 5:
                document.getElementById('user_cash_vegas').innerHTML = /user_fields\['user_cash_vegas'\] = "([^"]+)/.exec(attack_results)[1];
            ...
            break;
            case 6:
                document.getElementById('user_cash_italy').innerHTML = /user_fields\['user_cash_italy'\] = "([^"]+)/.exec(attack_results)[1];
        ...
        if(/You won/i.test(attack_results)){
            ...
            if(/Attack Again/.test(attack_results)){
                ...
                logtext = '<img src="http://codeoutpost.com/Scripts/attack.png" alt="alive"></img> ';
            }
...
//            else{
                ...
                logtext += '<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_experience_16x16_01.gif" alt="Exp"> <span class="good">' + parseInt(/\+(\d*) Experience/.exec(attack_results)[1])+'</span>';
//            }
            ...
            if(/Fan Bonus/.test(attack_results)){
                if(/rallied/.test(attack_results)){
                    ...
                    logtext += ' Fan Bonus - <span class="good">' + (2 * parseInt(/(\d*) Fan Bonus/.exec(attack_results)[1]))+'</span>';
                }
                else{
                    exp_gained += parseInt(/(\d*) Fan Bonus/.exec(attack_results)[1]);
                    logtext += ' Fan Bonus - <span class="good">' + parseInt(/(\d*) Fan Bonus/.exec(attack_results)[1])+'</span>';
                }
            }
//            alert(/(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_new_york_cash|sexy_vegas_cash) good"\>(\n|\f|\r)([^<]+)/.exec(attack_results)[0]);
            
            cash = /(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_vegas_cash|sexy_italy_cash|sexy_new_york_cash) good"\>(\n|\f|\r)([^<]+)/.exec(attack_results)[3];
//            cash = cash.replace(/\n/g,'');
            logtext += ' <img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_16x16_01.gif" alt="cash"></img> <span class="good">'+cash+'</span>';            
            
            if(iced == true){
                logtext += ' <img src="http://codeoutpost.com/Scripts/Ice.png" alt="Iced"></img>';
            }
            
            if(killed == true){
                logtext += ' <img src="http://codeoutpost.com/Scripts/kill.png" alt="Kill"></img>';
            }
            try{
                logtext += ' <img src="http://mwfb.static.zynga.com/mwfb/graphics/victory_icon.gif"></img>+<span class="good">'+ (/\+(\d+).*Victory Coins/.exec(attack_results)[1])+'</span> ';
            }
            catch(err){}
            ...
            switch(true){
                case (/sexy_cuba_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(2,(/sexy_cuba_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
                    cash_city = 2;
                break;
                case (/sexy_moscow_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(3,(/sexy_moscow_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,''))); 
                    cash_city = 3;
                break;
                case (/sexy_bangkok_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(4,(/sexy_bangkok_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
                    cash_city = 4;
                break;
                case (/sexy_vegas_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(5,(/sexy_vegas_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
                    cash_city = 5;
                break;
                case (/sexy_italy_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(6,(/sexy_italy_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
                    cash_city = 6;
                break;
                case (/sexy_new_york_cash good"\>[^\$]+\$([^<]+)/.test(attack_results)):
                    process_cash(1,(/sexy_new_york_cash good"\>[^\$]+\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
                    cash_city = 1;
                break;
            }
     
            if(parseInt(/(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_vegas_cash|sexy_italy_cash|sexy_new_york_cash) good"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')) < parseInt(cash_limit)){
            ...
            attack_count++;
            if((attack_count >= parseInt(document.forms.something.attack_limit.value)) && document.forms.something.attack_limit_check.checked){
                logmsg('Done with '+fightlist_names[i]+' Fetching next target in '+random_delay+' seconds..','attack_log');
                next();
                return;
            }
            ...
            if(/Attack Again/.test(attack_results)){
                ...
                logmsg('Done with '+fightlist_names[i]+' after ' + attack_count + ' attacks','attack_log');
                if (iced_in_row >= 2)
                {
                  logmsg('Two iced in a row, reloading', 'attack_log');
                  loadfightpage();
                }
                else
                {
            ...
            if(/You lost/i.test(attack_results)){
                logtext = '';
                loss++;
...
//                document.getElementById('logger').innerHTML = 'You Lost ! Cash Lost - '+cash; 
            cash = /(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_vegas_cash|sexy_italy_cash|sexy_new_york_cash) (bad|good)"\>(\n|\f|\r)([^<]+)/.exec(attack_results)[4];
//                document.getElementById('popup_fodder').innerHTML = attack_results;
//                return;
                try{
                    switch(true){
                        case (/sexy_cuba_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(2,-(/sexy_cuba_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));
                        break;
                        case (/sexy_moscow_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(3,-(/sexy_moscow_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));                
                        break;
                        case (/sexy_bangkok_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(4,-(/sexy_bangkok_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));
                        break;
                        case (/sexy_vegas_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(5,-(/sexy_vegas_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));
                        break;
                        case (/sexy_italy_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(6,-(/sexy_italy_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));
                        break;
                        case (/sexy_new_york_cash (bad|good)"\>[^\$]+\$([^<]+)/.test(attack_results)):
                            process_cash(1,-(/sexy_new_york_cash (bad|good)"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')));
    //                        alert(-(/fightres_cash sexy_cash bad">\n(.*)-(.*)\$([^<]+)/.exec(attack_results)[1].replace(/,/g,'')));
            ...
            document.getElementById('ice_posts_log').innerHTML += 'Too Strong: ' + fightlist_names[i] + '<br>';
    ...
    function process_user_attack(attack_results){
        ...
        if(/session has timed out/.test(attack_results)){
...
//        document.getElementById('user_experience').innerHTML = /user_fields\['user_experience'\] = parseInt\("([0-9]+)/.exec(attack_results)[1];
        ...
        try{
            temp='';
            loot_log = [];
            if(/(found|gained|earned) ([0-9]+) (.+?)(See|while|\.|)/.test(attack_results)){
                temp_loot = attack_results.match(/(<div class="fightres_bonus_message".+?<img src="(.+?)".+?|)(found|gained|earned)([0-9]+) (.+?)(See|while|\.)/g);
                for(z=0;z<temp_loot.length;z++){
                    temp = /(found|gained|earned)([0-9]+) (.+?)(See|while|\.)/.exec(temp_loot[z]);
                    loot_img = /<div class="fightres_bonus_message".+?<img src="(.+?)"/.exec(temp_loot[z]);
                    loot_img = loot_img ? loot_img[1]:"";
                    temp = temp[3].replace(/!/g,'');
                    temp = temp.replace(/<br\/>/g,'');
...
//                document.getElementById('stash_post'+(stash_posts.length-1)).onclick = new Function(this.id,publish_stash);
        ...
        if(fight_city==4){
            var faction_level = attack_results.match(/zy_progress_bar_faction_text">(.+) \/ 1500/g);
            document.getElementById('Yakuza_points').innerHTML = /zy_progress_bar_faction_text">(.+) \/ 1500/.exec(faction_level[0])[1];
            document.getElementById('Triad_points').innerHTML = /zy_progress_bar_faction_text">(.+) \/ 1500/.exec(faction_level[1])[1];
        }
        ...
        switch(parseInt(fight_city)){
            case 1:
                document.getElementById('user_cash_nyc').innerHTML = /user_fields\['user_cash_nyc'\] = "([^"]+)/.exec(attack_results)[1];
                cash_in_hand = document.getElementById('user_cash_nyc').innerHTML.replace(/,/g,'').replace(/\$/g,'');
                if((parseInt(cash_in_hand) > parseInt(document.getElementById('user_bank_limit').value)) && (document.forms.something.user_bank_enable.checked)){
                    logmsg('Banking <span class="good">'+document.getElementById('user_cash_nyc').innerHTML+'</span>','attack_log');
                    bank(cash_in_hand);
                }
            break;
...
//            alert(/fightres_cash sexy_cash good">\n(.*)\+( *)([^<]+)/.exec(attack_results));
            ...
            exp_ratio = exp_gained/(win+loss);
            document.getElementById('exp_ratio').innerHTML = exp_ratio.toFixed(2);
//            document.getElementById('logger').innerHTML = 'Fought '+ fightlist_names[i] +' and You won ! Total Exp gained - '+exp_gained;
//            alert();
...
//            setTimeout(attack_user,random_delay*1000);
            ...
            if(/Attack Again/.test(attack_results)){
                ...
                if(user_list.length > 1){
                    logmsg('Done with '+user_names[j]+' Fetching next target in '+random_delay+' seconds..','attack_log');
                }
...
//                    logmsg('Reached last member, reloading..','attack_log');
    ...
    function stop(){
        if((document.getElementById('finish_heal_enable').checked) && (heal_city != 0)){
        ...
        if(document.getElementById('restart_enable').checked){
            var restart_time = parseInt(document.getElementById('restart_min').value);
            attack_or_not=true;
            logmsg('Attack run complete, Restarting in '+restart_time+' minute(s)..','status');
            setTimeout(loadfightpage,restart_time*60*1000);
            return;
        }
        document.getElementById('pause').style.display = 'none';
        document.getElementById('begin').style.display = 'inline';        
    ...
    function pre_check(){
        ...
        if(document.getElementById('SpecialChars').value != ''){
            ...
            for(k=0; k < avoid_chars.length; k++){
                var clean_fl_name = clearspaces (fightlist_char_names[i]);
                var clean_ignore_name = clearspaces (avoid_chars[k]);
                if(clean_fl_name.indexOf(clean_ignore_name) != -1) {
                    logmsg('Skipping '+fightlist_names[i]+' fetching next target','attack_log');
                    document.getElementById('ice_posts_log').innerHTML += 'Skipping '+fightlist_names[i] + ' due to ' + avoid_chars[k] + '<br>';
        ...
        if(document.getElementById('ice_check').checked){
            logmsg('Running Ice Check on '+ fightlist_iced[i],'status');
                    if (fightlist_iced[i].indexOf ('(iced)') != -1) {
                        logmsg(fightlist_names[i]+' is already Iced/Dead, skipping..','status');
                        //logmsg(fightlist_iced[i]+' is already Iced/Dead, skipping..','attack_log');
                        skip_reason = 'Iced';
                        next();
                        return;
                    }
    ...
    function Add_to_loot(loot,count,img){
        var ChopShop = /Cement Block|Power Tool|Car Lift|Acetylene Torch|Shipping Container|Car Part|High Tech Car Part|Cuban Car Part|Thai Car Part|Russian Car Part|Solar Panel|Bulletproof Glass|Suspension Coil/;
		...
		var Armory = /Armor Part|Micro-Fission Cell|Bio-Monitor/; 
		var Consumable = /Set of Blackmail Photos|Computer Set-Up|Set of Illegal Transaction Records|Concealable Camera|Untraceable Cell Phone/;
		var CHEL = /TNT|Canonazo|Camouflage Body Armor|Guerrilla Squad|Track Loader|Armored State Car/;
		...
		var VHEL = /Goldsmobile|Foo Fighter|Bighorn|Ballista Missile Launcher|Cooling Vest|Reinhardt and Otto|Arcturion Assault Rifle|Diamond Back/;
		var IHEL = /Meadow Viper|Corpo Armatura|Pulcinella|Avanti Tutta /;        
        ...
        var FHEL3 = /Ottoman Krug|M45 Overcast|Phantasm|Nyala/;
        ...
        if(loot.search(Armory) > -1){
            loot = '<span class="more_in">[Armory]</span> ' + loot;
        }
        if(loot.search(Consumable) > -1){
            loot = '<span class="more_in">[Consumables]</span> ' + loot;
        }
        if(loot.search(CHEL) > -1){
            loot = '<span class="more_in">[CHEL]</span> ' + loot;
        }
        ...
        if(loot.search(VHEL) > -1){
            loot = '<span class="more_in">[VHEL]</span> ' + loot;
        }
        if(loot.search(IHEL) > -1){
            loot = '<span class="more_in">[IHEL]</span> ' + loot;
        }
        if(loot.search(FHEL) > -1){
            loot = '<span class="more_in">[FHEL]</span> ' + loot;
        }
        if(loot.search(FHEL2) > -1){
            loot = '<span class="more_in">[FHEL2]</span> ' + loot;
        }
        if(loot.search(FHEL3) > -1){
            loot = '<span class="more_in">[FHEL3]</span> ' + loot;
        }
...
//        document.getElementById('popup_permanence').innerHTML += 'Heal travel value - '+heal_travel+'<br>';
        ...
        if(fight_city==heal_city){
            ...
            $.ajax({type: "POST",data: params, dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8",
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|V|L|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                        logmsg('<img src="http://codeoutpost.com/Scripts/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|L|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Resuming attacks..','attack_log');
                    }
            ...
            $.ajax({type: "POST",data: params, dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8",
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|V|L|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        logmsg('<img src="http://codeoutpost.com/Scripts/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|L|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','status');
                        logmsg('<img src="http://codeoutpost.com/Scripts/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|L|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','attack_log');
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    }
                    else if(msg.user_fields.user_health == msg.user_fields.user_max_health){
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    ...
                    $.ajax({type: "POST", data: params, url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+heal_city+"&cb="+cb+"&destination="+fight_city+"&from=fight&nextParams=&xw_client_id=8",
                    success: function (msg){
            ...
            $.ajax({type: "POST", data: params, url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+fight_city+"&cb="+cb+"&destination="+heal_city+"&from=fight&nextParams=&xw_client_id=8",
            success: function (msg){
                heal_travel=true;
...
//    alert('city - '+city+' Cash - '+parseInt(cash));
    try{
        switch(parseInt(city)){
            ...
            break;
            case 6:
                italy_cash += parseInt(cash);
            break;
        }
        if(ny_cash < 0){
            document.getElementById('cash').innerHTML = '&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_16x16_01.gif"></img>&nbsp;<span class="bad">-$'+format_cash(ny_cash)+'</span>';
        }
        else{
            document.getElementById('cash').innerHTML = '&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_16x16_01.gif"></img>&nbsp;<span class="good">$'+format_cash(ny_cash)+'</span>';
        }
        if(cuba_cash < 0){
...
// Adf Stylez with the seconds included to remove just comment out var second and second on both timestamp spots
    ...
    function bank(amount,city){
        if(fight_city == 5){
            ...
            $.ajax({type: "POST", data: params, 
                    url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=propertyV2&xw_action=doaction&xw_city=5&doaction=ActionBankDeposit&xw_person="+person+"&amount="+amount+"&city=5&building_type=6&xw_client_id=8",
                success: function (msg){},error: function (s1,s2,s3){}
            });
            return;
        }
    	do_ajax('','remote/html_server.php?xw_controller=bank&xw_action=deposit&xw_city='+cash_city+'&cb='+userid+UnixTS()+'&amount='+amount+'&city='+city,1,0,0,0);
    }
    ...
    function writeCookieStuff(){
        ...
        var cookieStuff = wait1+'|'+wait2+'|'+heal_city+'|'+heal_city_text+'|'+levelup_halt+'|'+levelup_text+'| |'+
                        bank_enable+'|'+user_bank_enable+'|'+bank_limit+'|'+attack_limit_enable+'|'+attack_limit_count+'|'+lower_mafia+'|'+upper_mafia+'|'+
                        lower_level+'|'+upper_level+'|'+min_cash+'|'+same_city_cash+'|'+faction_bal_points+'|'+faction_attack+'|'+
                        triad_fac+'|'+yakuza_fac+'|'+faction_bal+'|'+ice_check_enable+'|'+power_attack_enable+'|'+
                        user_power_attack_enable+'|'+user_iced_check;
    ...
    function readCookieStuff(){
        try{
            ...
            user_iced_check = cookieStuff[26];
        }
        catch(err){}
    }
    ...
    function load(){
        try {
        	var pageTracker = _gat._getTracker("UA-12870604-2");
        	pageTracker._trackPageview("/Brawler");
        }
        catch(err) {}
    }
...
//    alert(key_code);
	if ((key_code>=48 && key_code<=57) || (key_code>=96 && key_code<=105) || key_code==8 || key_code==127 || key_code==37 || key_code==39 || key_code==9 || key_code==46 || key_code==13 || key_code==17 || key_code==86){
		return true;
	}
	else{
		return false;
