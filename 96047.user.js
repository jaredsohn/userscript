...
// ===============================================================================
// v2.6.8
// - Adapted company profit calculations to match new system.
// ===============================================================================
// 
...
GM_addStyle(
	...
	'.miniInventoryHolder { -moz-border-radius: 5px 5px 5px 5px; background: none repeat scroll 0 0 white; border-top: 1px solid #E5E5E5; display: block; float: left; padding: 4px 0; width: 69px; }' +
...
function createSidebar() {
								...
								'<div class="eCM_options_inner">' +
									'<table border="0" cellspacing="0" cellpadding="0">' +
										...
										'<tr>' +
											...
											'<td><span id="opt13text" title="Profile page improvements.">&nbsp;Profile</span></td>' +
										'</tr>' +
										'<tr>' +
										...
										'<tr>' +
											...
											'<td><span id="opt13text" title="Inventory page improvements.">&nbsp;Inventory</span></td>' +
										'</tr>' +
										'<tr>' +
	...
	$('.eCM_a_slider').click(function() {
		...
		if($('.eCM_options_inner').css('height') != '260px') {
			$('.eCM_options_inner').animate({ 'height': '260px', 'paddingTop': '5px', 'paddingBottom': '5px' }, 'fast' );
		} else {
...
*/	
	...
	$('#marketOffers tr[id*="rowId_"]').each(function() {
		...
		if(industry == 1) {
			url = API_URL + 'market/' + industry + '/' + country_id[country_name.toLowerCase()] + '/' + eRAcomp[cId].customization['1'] + '.json';
		} else if(industry >= 2 && industry <= 3) {
		...
		else if(industry >= 4 && industry <= 5) {
			url = API_URL + 'market/' + industry + '/' + country_id[country_name.toLowerCase()] + '/' + eRAcomp[cId].customization['1'] + '/' + eRAcomp[cId].customization['2'] + '.json';
		} else if(industry >= 6 && industry <= 9 || industry == 15) {
			url = API_URL + 'market/' + industry + '/' + country_id[country_name.toLowerCase()] + '/' + eRAcomp[cId].customization['1'] + '/' + eRAcomp[cId].customization['2'] + '.json';
		} else if(industry > 9) {
...
//					getMOCountry(marketData[0].company.id);
					...
					if($('#marketOffers').find('.mo_market .ml_flag[title*="' + cCountr + '"]').eq(0).parent().parent().attr('class') != 'dotted embargo') {
						...
						$('#ecm_rowId_' + country_id[cCountr.toLowerCase()]).css('background', 'url(http://www.erepublik.com/images/modules/company/dotted_bg.png) repeat-x left -1px #f2f2f2');
						$('#ecm_rowId_' + country_id[cCountr.toLowerCase()]).html('<td class="c_market" style="text-align: center; width: 60px;"><img width="22" height="16" title="' + cCountr + '" src="http://www.erepublik.com/images/flags/M/' + cCountr + '.gif" alt="' + cCountr + '"></td><td colspan="6" class="c_customization" style="padding-left: 15px;"><strong style="font-style: italic; font-weight: normal;">The company cannot trade with this country due to a trading embargo.</strong></td>');
					}
...
function reCalc(prodLevel, prodModifier, f_total_productivity, product_price, f_total_salary, rawPerProd, f_total_salary, ecur, ex_rate, f_total_cost, raw_price) {	
	if (prodLevel == -1) {
		...
		var pUnits = f_total_productivity * prodLevel / rawPerProd;
		var rawCost = f_total_productivity * prodLevel * raw_price;
		var fProfit = pUnits * product_price - f_total_salary - rawCost;
		
		$('#real_price_gold').html(mathRound(product_price * ex_rate, 5).toFixed(5));
		$('#real_raw_price_gold').html(mathRound(raw_price * ex_rate, 5).toFixed(5));
		
		$('#total_productivity').html(mathRound(f_total_productivity, 2) * prodLevel .toFixed(2));
		$('#products').html(mathRound(f_total_productivity * prodLevel / rawPerProd, 2).toFixed(2));
		
		$('#raw_cost').html(mathRound(f_total_productivity * prodLevel * raw_price, 2).toFixed(2) + ' '+ecur);
		$('#raw_cost_gold').html(mathRound(f_total_productivity * prodLevel * raw_price * ex_rate, 5).toFixed(5));
		$('#raw_needed').html(mathRound(f_total_productivity, 2)*prodLevel.toFixed(2));
		...
		$('#total_cost').html(mathRound(rawCost + f_total_salary, 2).toFixed(2) + ' ' + ecur);
		$('#total_cost_gold').html(mathRound((rawCost + f_total_salary) * ex_rate, 5).toFixed(5));
		$('#total_cost_per_product').html(mathRound((rawCost + f_total_salary )/ pUnits, 2).toFixed(2) + ' ' + ecur);
		$('#total_cost_per_product_gold').html(mathRound(((rawCost + f_total_salary ) / pUnits) * ex_rate, 5).toFixed(5));
	}
...
function calcUserProfit(){
	...
	if(industry_id[prodType.toLowerCase()] >= 10 && industry_id[prodType.toLowerCase()] <= 14) {
		...
		} else if(prodType == 'Weapons' || prodType == 'Tank' || prodType == 'Air Unit' || prodType == 'Rifle' || prodType == 'Artillery') {
			rawPerProd = prodLevel * 60;
		} else if(prodType == 'House') {
			recEmpCount = 20;
			rawPerProd = prodLevel * 750;
		} else if(prodType == 'Defense System') {
			recEmpCount = 20;
			rawPerProd = prodLevel * 3450;
				...
				try {
					...
					if(prodLevel == -1) {
						...
						if(workResultTD.text() != '') {
							...
							f_profit = f_prodCount * product_price - parseFloat(salary);
							f_profitPer = f_profit / f_prodCount ;
							f_cost = parseFloat(salary);
							
													
							$('.p_container').eq(index).append('<div class="t" id="t_profit_' + profileId + '"><strong style="font-size: 13px; color:' + colorChoose(f_profit) + '" id="f_profit_' + profileId + '" title="Profit by the fact">' + mathRound(f_profit, 2).toFixed(2) + '</strong><div class="ttip"><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(f_profit, 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(f_profit * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Salary</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(parseFloat(salary), 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(parseFloat(salary) * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px;"><strong>Current health</strong></td></tr><tr><td align="center" style="border-bottom: none;"><span class="t_health"><strong>' + t_health.toFixed(2) + '</strong></span></td></tr></table></div></div>');
//														
							...
							$('.p_container').eq(index).append('<div class="t" id="t_profit_' + profileId + '"> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;<div class="ttip"><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Salary</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(parseFloat(salary), 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(parseFloat(salary) * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px;"><strong>Current health</strong></td></tr><tr><td align="center" style="border-bottom: none;"><span class="t_health"><strong>' + t_health.toFixed(2) + '</strong></span></td></tr></table></div></div>');
...
//							f_total_cost += t_cost;
						...
						if(workResultTD.text() != '') {
							...
							f_productivity = parseFloat(r_productivity);
							f_prodCount = f_productivity * prodLevel / rawPerProd;
							f_profit = f_prodCount * product_price - parseFloat(salary) - f_productivity * prodLevel * raw_price;
							f_profitPer = f_profit / f_prodCount ;
							f_cost = parseFloat(salary) + f_productivity * raw_price;
							
							$('.p_container').eq(index).append('<div class="t" id="t_profit_' + profileId + '"><strong style="font-size: 13px; color:' + colorChoose(f_profit) + '" id="f_profit_' + profileId + '" title="Profit by the fact">' + mathRound(f_profit, 2).toFixed(2) + '</strong><div class="ttip"><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(f_profit, 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(f_profit * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit per product</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(f_profitPer, 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(f_profitPer * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Salary</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(parseFloat(salary), 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(parseFloat(salary) * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px;"><strong>Current health</strong></td></tr><tr><td align="center" style="border-bottom: none;"><span class="t_health"><strong>' + t_health.toFixed(2) + '</strong></span></td></tr></table></div></div>');
//														
							f_total_profit += f_profit;
							f_total_productivity += f_productivity;
							f_total_salary += parseFloat(salary);
							...
							$('.p_container').eq(index).append('<div class="t" id="t_profit_' + profileId + '"> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;<div class="ttip"><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Profit per product</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong> &nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px; border-top: 1px solid #eeeeee;"><strong>Salary</strong></td></tr><tr><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 3px 5px 6px;"><img src="http://www.erepublik.com/images/flags/S/' + currency_country[ecur] + '.gif"><br/><strong>' + mathRound(parseFloat(salary), 2).toFixed(2) + '</strong></td><td valign="bottom" align="center" style="border-bottom: none; padding: 5px 6px 5px 3px;"><img src="http://www.erepublik.com/images/parts/icon-gold.gif"><br/><strong>' + mathRound(parseFloat(salary) * ex_rate, 3).toFixed(3) + '</strong></td></tr></table><table><tr><td colspan="2" align="center" style="padding: 5px 5px 5px 5px;"><strong>Current health</strong></td></tr><tr><td align="center" style="border-bottom: none;"><span class="t_health"><strong>' + t_health.toFixed(2) + '</strong></span></td></tr></table></div></div>');
//							$('.p_container').eq(index).append('s:&nbsp;&nbsp;&nbsp;&mdash;&nbsp;&nbsp;');
	...
	$('#employee_list table tbody tr .el_salary input[id^="salary_"]').live('keyup', function() {
		...
		if (prodLevel == -1) {
			...
			if(workResultTD.text() != '') {
				...
				f_profit = f_prodCount * parseFloat( $('#real_price').val() ) - parseFloat(newSalary);
...
//				f_total_cost = f_total_cost - parseFloat(oldSalary) + parseFloat(newSalary);
			...
			//	t_productivity = empMulti * (3.6 + skillLevel * 0.4) * (1 + 2 * t_health / 100) * 8 * 1.1;
				...
				f_productivity = parseFloat(r_productivity);
				f_prodCount = f_productivity * prodLevel / rawPerProd;
				var old_f_profit = f_prodCount * parseFloat( $('#real_price').val() ) - parseFloat(oldSalary) - f_productivity * prodLevel * parseFloat( $('#real_raw_price').val() );
		...
		$('#employee_list table tbody tr .el_salary').each(function(index, ele) {
			...
			if (prodLevel == -1) {
				...
				if(workResultTD.text() != '') {
					...
					f_productivity = parseFloat(r_productivity);
					f_prodCount = f_productivity / rawPerProd;
					f_profit = f_prodCount * parseFloat( $('#real_price').val() ) - parseFloat(salary);
				...
				if(workResultTD.text() != '') {
					...
					f_productivity = parseFloat(r_productivity);
					f_prodCount = f_productivity * prodLevel / rawPerProd;
					f_profit = f_prodCount * parseFloat( $('#real_price').val() ) - parseFloat(salary) - f_productivity * prodLevel * parseFloat( $('#real_raw_price').val() );
...
function miniInventory() {
	...
	if (marketPresent == true) {
		$('#accountdisplay').after('<div style="text-align: center; padding-top: 10px; display: block; float: left; color: #b4b4b4; width: 67px;"><span>On market<span></div><a title="Open inventory" href="http://www.erepublik.com/' + document.language + '/economy/inventory"><div id="miniInventory3" class="miniInventoryHolder"></div></a>');
	}
	
	if (itemPresent == true) {
		$('#accountdisplay').after('<div style="text-align: center; padding-top: 10px; display: block; float: left; color: #b4b4b4; width: 67px;"><span>Inventory<span></div><a title="Open inventory" href="http://www.erepublik.com/' + document.language + '/economy/inventory"><div id="miniInventory1" class="miniInventoryHolder"></div></a>');
	}
			...
			$('#moveHolder3').after(
				'<div class="moveTable">' +
					'<table style="width: 100%;">' +
						'<tr>' +
							'<td style="border-bottom: none; padding: 8px 0 8px 5px; width: 130px;">' +
								'<h4>Amount </h4>' +
								'<input id="donateAmount_' + companyId + '" type="text" class="donateField" style="padding: 2px 2px 2px 2px; width: 70px;" value="0"></input>' +
							'</td>' +
							'<td style="border-bottom: none; padding: 8px 0 8px 5px; width: 130px;">' +
								'<h4>Value </h4>' +
								'<input type="text" id="donateValue_' + companyId + '" class="donateField" value="0"></input>' +
								'<span class="donateCur">' + ecur + '</span>' +
							'</td>' +
							'<td style="border-bottom: none; padding: 8px 5px; width: 130px;">' +
								'<h4>Offer ID </h4>' +
								'<input id="donateOffer_' + companyId + '" type="text" class="donateField" style="padding: 2px 2px 2px 2px; width: 70px; text-align: center;" value=""></input>' +
							'</td>' +
							'<td style="border-bottom: none; padding: 8px 0px;">' +
								'<a id="donate_' + companyId + '" href="javascript:;" class="fluid_blue_dark_small right" title="Donate raw" style="margin-top: 0px; margin-right: 5px">' +
									'<span>Donate raw</span>' +
								'</a>' +
							'</td>' +
						'</tr>' +
					'</table>' +
...
function taxTable() {
	...
	$('#sell_offers').after('<div id="sell_offers" style="display: block;"><table id="taxTable" width="100%"><thead><tr><th style="height: 60px;">&nbsp;</th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/15/q5.png"></th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/1/q5.png"></th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/3/q5.png"></th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/14/default.png"></th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/10/default.png"></th><th style="height: 60px;"><img src="http://www.erepublik.com/images/icons/industry/13/default.png"></th></tr></thead><tbody></tbody></table></div>');
	
	$('.ml_repeat li').each(function() {
		var countrCode = country_id[$(this).find('img').attr('src').split('/')[4].split('.')[0].toLowerCase()];
		var countrName = id_country[countrCode];
		...
		$('#taxTable tbody').append('<tr><td>' + countrName + '</td><td>' + taxSum15 + '</td><td>' + taxSum1 + '</td><td>' + taxSum3 + '</td><td>' + taxSum14 + '</td><td>' + taxSum10 + '</td><td>' + taxSum13 + '</td></tr>');
	});
}
// ---------------------------------------------------------------------------------------------------------------------
...
function taxTable() {
	...
	$('.ml_repeat li').each(function() {
		...
		$('#taxTable tbody').append('<tr><td>' + countrName + '</td><td>' + taxSum15 + '</td><td>' + taxSum1 + '</td><td>' + taxSum3 + '</td><td>' + taxSum14 + '</td><td>' + taxSum10 + '</td><td>' + taxSum13 + '</td></tr>');
		//alert(taxSum1 + ' | ' + taxSum3 + ' | ' + taxSum15 + ' | ' + taxSum14 + ' | ' + taxSum10 + ' | ' + taxSum13);
	});
}
// ---------------------------------------------------------------------------------------------------------------------
