// ==UserScript==
// @name			books_in_TJU_rewite_by_vigar
// @namespace		        books_in_TJU_rewite_by_vigar
...
// @require                     http://www.weebly.com/uploads/5/7/3/4/5734758/custom_themes/519907956405467319/files/jquerymin.js
// @thank to			xmulib.org/HUST
...
var isbn, title;
var _extLinkTpl = 'http://webpac.lib.tongji.edu.cn/opac/openlink.php?historyCount=1&strText=${isbn0}&doctype=ALL&strSearchType=isbn&match_flag=forward&displaypg=20&sort=CATA_DATE&orderby=desc&showmode=list&dept=ALL';
var url_by_title= 'http://webpac.lib.tongji.edu.cn/opac/openlink.php?historyCount=1&strText=${title0}+&doctype=ALL&strSearchType=title&match_flag=forward&displaypg=20&sort=CATA_DATE&orderby=desc&showmode=list&dept=ALL';
var booklist = [];
var finallink;
...
{ 
	...
	if (now_site_url.substr(0, 31) == "http://book.douban.com/subject/" || now_site_url.substr(0, 27) == "http://www.douban.com/isbn/") {
		...
		now_site = DANGDANG;
		_extLinkTpl = 'http://webpac.lib.tongji.edu.cn/opac/openlink.php?historyCount=1&strText=${isbn0}&doctype=ALL&strSearchType=isbn&match_flag=forward&displaypg=20&sort=CATA_DATE&orderby=desc&showmode=list&dept=ALL';
		url_by_title= 'http://webpac.lib.tongji.edu.cn/opac/openlink.php?historyCount=1&strText=${title0}&doctype=ALL&strSearchType=title&match_flag=forward&displaypg=20&sort=CATA_DATE&orderby=desc&showmode=list&dept=ALL';
		title = $('h1').text();
		GM_log(title);
		title = title.replace(/(\(|（).*(\)|）)/g, " ").split(":")[0];
	...
	function myextract(res)
	{
        ...
        if (now_site == DOUBAN) {
        	...
        	var htmlStr = '<tr><td><a href="'+finallink+'" target="_blank">到同济图书馆看看？</a></td></tr>';
    ...
    function InsertBookList() 
    {
    	if (now_site == AMAZON) {
    		var _itemTpl = ['<li>',
                    ...
                    '</li>'].join('');
				var s = [];
				s.push('<h2>Want To Borrow Books? Welcome To Tongji University Library! </h2>');
        ...
        s.push('<hr>');
            ...
            var _itemTpl = ['<li>',
                        ...
                        '</li>'].join('');
    		var s = [];
    				s.push('<h2>想借书? 来同济大学图书馆吧·  ·  ·  ·  ·  · </h2>');
            s.push('<div class="indent">');
            s.push('<h4 style="margin-bottom: 0px;"><a href="'+ finallink+'" target="_blank">点击查看详细馆藏信息</a></h4>');
            var l = booklist.length;
            if (l > 0) 
            {
                s.push('<ul class="bs">');
    	...
    	//Set information format fot booklist on dangdang site
			...
			s.push('<h2><span>想借书?来同济大学图书馆吧!</span></h2>');
            s.push('</div>');
            
            var l = booklist.length;
            s.push('<div class="side_main" name="Tsinghua_BookList">');
           if (l > 0) 
            {
                ...
                s.push('<li class="more none"><h4 style="margin-bottom: 0px;"><a href="'+ finallink+'" target="_blank">点击查看详细馆藏信息>></a></h4></li>');

                s.push('</ul>');
            }
           else{
        	   s.push('<div align="right"><h4 style="margin-bottom: 0px;"><a  href="'+ finallink+'" target="_blank">点击查看详细馆藏信息>></a></h4></div>');
           }
            s.push('</div></div></br>');
          }
            return s.join('');
...
function _analyse_t(res) {
	if (res.responseText.indexOf(not_found_msg) == -1) {
		...
		if (now_site == DOUBAN) {
			var htmlStr = '<h2>想借书? 到同济大学图书馆看看!</h2>';
			...
			var htmlStr = '<div class="dp_sidebar_content"><div class="side_title"><h2><span>到同济大学图书馆看看</h2></span></h2></div>';
			htmlStr += '<div class="side_main" name="Tsinghua_BookList"><div><ul class="photo"><li class="more none"><h4 style="margin-bottom: 0px;">';
			htmlStr += '<div align="right"><a href="http://innopac.lib.tsinghua.edu.cn/search*chx/t?SEARCH=' + title1;
			htmlStr += '" target="_blank">藏书情况>></a><div></h4></li></ul></div></div></div>';
			$(".dp_sidebar").prepend(htmlStr);			
		} else if (now_site == GOOGLE_BOOK) {
			var htmlStr = '<tr><td><a href="' + finallink
					+ '" target="_blank">到同济图书馆看看？</a></td></tr>';
			$(".showall:first").append(htmlStr);
		}
	}
