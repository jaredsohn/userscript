// ==UserScript==
// @name           VKPower
// @namespace      http://success.ayodesign.com.ua/
// @description    Расширение функционала ВКонтакте.ру
// @include        http://vkontakte.ru/*
// ==/UserScript==
// Author: Success
// Version: 0.00.01
// Site: success.ayodesign.com.ua
...
GM_xmlhttpRequest({
    ...
    var c=xmlobject.getElementsByTagName('location')[0];
     ...
     addon.innerHTML="Cкачать FLV";
     document.getElementById("videoactions").appendChild(addon);
     var addon=document.createElement("a");
     addon.setAttribute("href","http://vixy.net?u="+c.firstChild.nodeValue);
     addon.innerHTML="Скачать AVI/MOV/MP4/MP3/3GP";
     document.getElementById("videoactions").appendChild(addon); 
}});
}
...
if (location.pathname == '/search.php' && useLinks) {
	...
	var reg_notfriend = new RegExp(/<a href=\"\#add\" onclick=\"addToFriend\(([0-9]*)\); return false;\">([А-Яа-я ]*)<\/a>/);
	var reg_friend = new RegExp(/<a href=\"friend.php\?act=remove\&amp;id=([0-9]+)\">([А-Яа-я ]*)<\/a>/);
	...
	if (document.getElementById("searchResults") != null) {
		...
		for (i=0;i<=resultsElement.length-1;i++) {
			if (resultsElement[i].className == 'result clearFix') {
				...
				if (!horLinks) {
					notFriendLinks = "<li>" + getLinksForDeleted(userid).join("</li>\n<li>") + "</li></ul>";
					friendLinks = "<li>" + getLinksForDeleted(userid).join("</li>\n<li>") + "</li></ul>";
					deletedLinks = "<ul id=\"nav\" style=\"margin: 0px; width: 150px;\"><li>" + getLinksForDeleted(userid).join("</li>\n<li>") + "</li></ul>";
				} else {
					notFriendLinks = "</ul><tr><td colspan='3'><br><center>| " + getLinksForDeleted(userid).join(" | ") + " |</center></td></tr>";
				...
				resultsElement[i].innerHTML = resultsElement[i].innerHTML.replace(reg_notfriend,'<ul id="nav" style="margin: 0px; width: 150px;"><a href="#add" onclick="addToFriend($1); return false;">$2</a></li>'+notFriendLinks);
...
if (location.pathname == '/settings.php') {
	...
	if (GET['act'] != 'VKPower') {
		...
		menuElements.innerHTML += '<li>'+settInActiveLink('VKPower','?act=VKPower','4.8em')+'</li>';
		...
		// Добавляем нашу вкладку
		menuElements.innerHTML += '<li class="activeLink">'+settActiveLink('VKPower','?act=VKPower','4.8em')+'</li>';
		
		editorPanelElements.innerHTML = "     <div id=\"cname\" class=\"settingsPanel\">"+
		"<h4>VKPower Настройки</h4>"+
		"<form method=\"GET\" id=\"vkpowerSettings\" name=\"vkpowerSettings\" action=\"\"><input type=\"hidden\" name=\"act\" value='VKPower'><input type=\"hidden\" name=\"vkpower\">"+
		...
		"<span class=\"ncc\"><a href=\"javascript:document.vkpowerSettings.submit();\">Сохранить</a></span>"+
...
function showMessageSettSaved() {
	...
	message = 'Настройки VKPower сохранены.';
...
if (document.getElementById('vkontakte')) {
	...
	link.innerHTML = '<a href=\"http://klinifini.livejournal.com/tag/VKPower\" target=\"_blank\">VKPower</a>';
