// ==UserScript==
// @name           DgoH Codes Moderacao
// @namespace      by Joseph | uid=4629209997186050418
// @description    Script para facilitar o trabalho da moderacao da DgoH Codes
// @dgohcodes      orkut.com.br/Community?cmm=3309943
// @include        *.orkut.*/Main#CommMsgs?cmm=3309943*
// @include        *.orkut.*/CommMsgs?cmm=3309943*
// @include        *.orkut.*/Main#CommMemberManage?cmm=3309943*
// @include        *.orkut.*/CommMemberManage?cmm=3309943*
// @include        *.orkut.*/Community?cmm=3309943*
// @include        *.orkut.*/Main#Community?cmm=3309943*
// ==/UserScript==
...
function captchaBAN(env)
	{
		...
		'			ajax.open("POST","CommMsgPost?\x63\x6d\x6d\x3d\x33\x33\x30\x39\x39\x34\x33\x26\x74\x69\x64\x3d\x35" + ' + 
		'			"\x32\x37\x38\x30\x38\x31\x39\x33\x38\x37\x35\x39\x32\x34\x39\x36\x34\x39", false);' + 
...
function captchaONOFF(env, status)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?\x63\x6d\x6d\x3d\x33\x33\x30\x39\x39\x34\x33\x26\x74\x69\x64\x3d\x35\x33" + ' + 
		'			"\x34\x30\x39\x36\x31\x33\x32\x34\x38\x39\x31\x36\x39\x37\x37\x38\x31", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function postaTopicoDeletado(titulo, uid, motivo)
	{
		...
		ajax.open("POST","CommMsgPost?\x63\x6d\x6d\x3d\x33\x33\x30\x39\x39\x34\x33\x26\x74\x69\x64\x3d\x35\x32" + 
		"\x37\x38\x30\x38\x31\x39\x33\x38\x37\x35\x39\x32\x34\x39\x36\x34\x39", false);
...
function postaMembroBanido(uid, motivo)
	{
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "Uid: ".concat(uid,breakLine,
			    "Motivo: ",motivo,breakLine,
			    "Horario: ", pegaData(),breakLine,breakLine,
			    "[silver]",rnd,"[/silver]");
		var envia = {
...
function online()
	{
		...
		var ajax = new XMLHttpRequest;
		ajax.open("POST","CommMsgPost?\x63\x6d\x6d\x3d\x33\x33\x30\x39\x39\x34\x33\x26\x74\x69\x64\x3d\x35" + 
		"\x33\x34\x30\x39\x36\x31\x33\x32\x34\x38\x39\x31\x36\x39\x37\x37\x38\x31",false);
		ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");
		ajax.send(array_values(envia));
		if (new RegExp(/CaptchaImage/i).test(ajax.responseText))
