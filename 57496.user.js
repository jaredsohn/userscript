...
var oldList = "All\t3-4\t3-3 Nickel\t3-2 Dime\t4-3\t4-2 Nickel\t4-1 Dime\tQuarter\t";
var defaultTabs = "All\t3-4\t3-3 Nickel\t3-2 Dime\t4-3\t4-2 Nickel\t4-1 Dime\tQuarter\t4-4\t";
var tabs = null;
var plays = new Array();
var formstr = null;

var farr = new Array();
var teamId = window.location.toString().split("=")[1].split("&")[0];
...
window.setTimeout(
    function() {
        ...
        tabs = GM_getValue("dpcsort_tabs_"+teamId,defaultTabs);
        GM_setValue("dpcsort_tabs_"+teamId,tabs);
	if (tabs == oldList) {
		tabs = defaultTabs;
	        GM_setValue("dpcsort_tabs_"+teamId,tabs);
	}
        ...
        for (var i=0; i<formstr.length; i++) {
            var string = GM_getValue("dpcsort_tabs_"+teamId+"_"+i,null);
...
function ai_tab_pressed() {
    ...
    var valid = GM_getValue("dpcsort_tabs_"+teamId+"_"+idx,"").split("\t");
    var p = document.getElementsByClassName("custom_play");
...
function formationCheck() {
    ...
    if (el.length > 0) {
        ...
        for (var i=0; i<farr.length; i++) {
            ...
            GM_setValue("dpcsort_tabs_"+teamId+"_"+i,str);
...
function resetTabs() {
    ...
    if (ret == true) {
        GM_deleteValue("dpcsort_tabs_"+teamId);
        
        var i=0;
        while (true) {
            var v = GM_getValue("dpcsort_tabs_"+teamId+"_"+i, null);
            if (v != null) {
                GM_deleteValue("dpcsort_tabs_"+teamId+"_"+i);
                i++;
            }
            else {
                break;
...
function createTab() {
    ...
    if (name.length > 0) {
        ...
        var str = GM_getValue("dpcsort_tabs_"+teamId);
        str += name + "\t";
        GM_setValue("dpcsort_tabs_"+teamId,str);
        window.location.reload();
    }
}
    ...
    if (name.length > 0) {
        ...
        if (idx != -1) {
            ...
            for (var i=idx; i<farr.length-1; i++) {
                GM_setValue("dpcsort_tabs_"+teamId+"_"+i, GM_getValue("dpcsort_tabs_"+teamId+"_"+(i+1), ""));
            }
            GM_deleteValue("dpcsort_tabs_"+teamId+"_"+(farr.length-1));
            window.location.reload();
        }
    }
}
...
function addPlayToTab() {
    ...
    if (name.length > 0) {
        ...
        var idx = formstr.indexOf(name);
        if (idx != -1) {
            var str = GM_getValue("dpcsort_tabs_"+teamId+"_"+idx);
            if (str.indexOf("\t"+id+"\t") == -1) {
                str += id+"\t";
                GM_setValue("dpcsort_tabs_"+teamId+"_"+idx, str);
                window.location.reload();
            }
        }
    }
...
function initial_page_load() {
	...
	if (row.getAttribute("class") != null) {
	        ...
	        row.setAttribute("class", "missing");
	}
        output.push(p);
    }
...
function loadPlay(addr, page) {
    var fixstr = ["All","3-4","Nickel 3","Dime 3","4-3","Nickel","Dime","Quarter","4-4"];
