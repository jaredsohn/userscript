...
//
// ==UserScript==
// @name          Dirty Tooltip Reloaded 
// @namespace     http://dirty.ru/
// @description   Dirty Tooltip Reloaded
		...
		if(xmlhttp.readyState == 4){
			if(xmlhttp.status == 200) {
				...
				if(dup_temp.indexOf('{"uid":"'+dup_id+'"')>0){
					...
					dup_temp = '<b>Ваша оценка '+((dup_sex=='f')?'eё':'eго')+': <span style="font-size:9pt;color:'+((dup_temp>0)?'green">+':'red">')+dup_temp+'</span></b>';
...
for(var i=0;i<document.getElementsByTagName('a').length;i++){
	var dup_an = document.getElementsByTagName('a')[i].toString();


	if(((dup_an.indexOf('dirty.ru/user/')>0||dup_an.indexOf('dirty.ru/users/')>0)&&dup_an[dup_an.length-1]!='/')&&dup_an.indexOf('#')<0||i==0){

			document.getElementsByTagName('a')[i].addEventListener('mouseover',function(e){clearTimeout(dup_showing);dup_showBaloon(e.target)},true);
			document.getElementsByTagName('a')[i].addEventListener('mouseout',function(){clearTimeout(dup_showing);dup_processing=0},true);
	}
}
...
function documentChanged(event) {
	if(typeof(event.target) !== 'undefined' && typeof(event.target.className) !== 'undefined'  && event.target.className.indexOf("comment")>-1){
		...
		for(var i=0;i<event.target.getElementsByTagName('a').length;i++){
			var dup_an = event.target.getElementsByTagName('a')[i].toString();

			if((dup_an.indexOf('dirty.ru/user/')>0&&dup_an[dup_an.length-1]!='/')&&dup_an.indexOf('#')<0||i==0){
					event.target.getElementsByTagName('a')[i].addEventListener('mouseover',function(e){clearTimeout(dup_showing);dup_showBaloon(e.target)},true);
					event.target.getElementsByTagName('a')[i].addEventListener('mouseout',function(){clearTimeout(dup_showing);dup_processing=0},true);
			}
		}
	}
	
	if(typeof(event.target) !== 'undefined' && typeof(event.target.tagName) !== 'undefined' && 
		...
		(event.target.tagName == "LI" || event.target.tagName == "li")){
...
var dup_left_panel = document.getElementsByClassName("left_col_nav");
if(dup_left_panel.length>0){
	dup_left_panel = dup_left_panel[0];

	dup_left_panel.innerHTML += '<li><a id="dup_setting_link" style="cursor:pointer;text-decoration:underline;">Настройки плюшек</a></li>';
	dup_left_panel.innerHTML += '<div id="dup_settings_detail" style="margin:10px 0 0 10px;display:none;font-size:8pt;font-family:Arial;color:#515151">	<div style="margin-right:10px;float:left"><input id="dup_checkbox_picture" type="checkbox" '+((dup_pictures==0)?'checked="checked"':'')+'></div><div style="float:left;width:100px">Tooltip без картинок</div><div style="clear:both;height:10px"></div>  </div>';
