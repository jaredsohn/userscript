...
// @namespace       heroeswm
// @name            HWM Art Info
// @description     Добавляет полезную информацию на страницу описания артефакта.
// @version         2.7.2
// @grant           GM_setValue
// @grant           GM_getValue
...
if (window === top) {
    ...
    var desc = {
        'details':
              '<ul>'
            + '<li>Написан на основе скрипта <a href="http://hwm.xo4yxa.ru/js/cost1batl/">Стоимость боя в информации об артефакте</a> от <a href="http://hwm.xo4yxa.ru/">xo4yxa</a>.</li>'
            + '<li>Для расчета стоимости гильдии кузнецов использовалась статья <a href="http://hwmguide.ru/articles/notes-about-hwm/kachaem-kuznicu-deshevo/">Качаем кузницу дёшево</a> от <a href="http://hwmguide.ru/user/7473/">перф</a>.</li>'
            + '<li>Добавляет ссылку на рынок для быстрого перехода. Отображает текущую рыночную стоимость.</li>'
            + '<li>Добавляет ссылку для выставления лота на рынке, при переходе по которой автоматически выбирается продаваемый артефакт и указывается его цена.</li>'
            +'</ul>',
        'homepage': {'title': 'userscripts.org', 'link': 'http://userscripts.org/users/hotamateurxxx/scripts'},
    ...
    var conf = {
        ...
        'precision': {'title': 'Точность', 'def': 0, 'desc': 'Количество отображаемых знаков после запятой.'},
        'maxPrices': {'title': 'Показывать цен', 'def': 3, 'desc': 'Количество рыночных цен для отображения.'},
        'showBattleCost': {'type': 'bool', 'title': 'Стоимость боя', 'def': true, 'desc': 'Отображать стоимость прокачки одного боя.'},
        'showGuildCost': {'type': 'bool', 'title': 'Стоимость ГК', 'def': true, 'desc': 'Отображать стоимость прокачки одного очка гильдии кузнецов.'},
        'showAuctionLink': {'type': 'bool', 'title': 'Ссылка на рынок', 'def': true, 'desc': 'Отображать ссылку на рынок.'},
        'showAuctionCost': {'type': 'bool', 'title': 'Рыночная стоимость', 'def': true, 'desc': 'Отображать текущую рыночную стоимость.'},
        'showMinMax': {'type': 'bool', 'title': 'Мин. и макс. цены', 'def': true, 'desc': 'Отображать минимальную и максимальную зафиксированные рыночные цены.'},
        'auctionWork': {'type': 'bool', 'title': 'Работать на аукционе', 'def': true, 'desc': 'Парсить и запоминать цены на страницах аукциона.'},
    }
...
if (!disabled) {
    ...
    // Если мы зашли на домашнюю страницу
    if (url.attr('path') == '/home.php') {
        
        var home = Home.parse(document);
        GM_setValue('guildLevel', home.playerGuilds.blacksmithsLevel);
    ...
    if (url.attr('path') == '/art_info.php') {
        ...
        var showBattleCost = GM_getValue('showBattleCost', true);
        var showGuildCost = GM_getValue('showGuildCost', true);
        var showAuctionLink = GM_getValue('showAuctionLink', true);
        var showAuctionCost = GM_getValue('showAuctionCost', true);
        var precision = parseInt(GM_getValue('precision', 0));
        var guildLevel = parseInt(GM_getValue('guildLevel', 0));
        
        var artId = url.param('id');
        var art = Art.parse(artId, undefined, document);
        
        // Стоимость
        var xpath = "//b[text()=' Стоимость:']";
        var nodeCost = XPath.findFirst(xpath);
        if (nodeCost !== null) {
            
            // Стоимость золотом
            var artCost = Cost.fromNode(nodeCost.nextSibling);
            if (artCost.res.length > 1) {
                var nodeGold = document.createElement('div');
                nodeGold.innerHTML = '<b>Стоимость золотом:</b>' + Auction.htmlPrice(Cost.toGold(artCost)) + '<br/>';
                nodeCost.parentNode.insertBefore(nodeGold, nodeCost.nextSibling.nextSibling.nextSibling);
            }
        ...
        // Стоимость ремонта
        var xpath = "//b[text()=' Стоимость ремонта:']";
        var nodeRep = XPath.findFirst(xpath);        
        
        // Обеспечиваем существование узла с комментариями
        var nodeComments = nodeRep.parentNode.lastChild;
        if (nodeComments.tagName !== 'I') {
            nodeRep.parentNode.appendChild(document.createElement('br'));
            nodeRep.parentNode.appendChild(document.createElement('i'));
        }
        var nodeComments = nodeRep.parentNode.lastChild;
        nodeComments.parentNode.insertBefore(document.createElement('br'), nodeComments);
        
        
        if (showBattleCost) {
            
            // Стоимость боя
            var nodeBat = document.createElement('div');
            var legend = 'Так как стоимость рассчитывается относительно стоимости ремонта, то для редких вещей и наград, носит сильно условный характер.';
            nodeBat.setAttribute('title', legend);
            nodeBat.innerHTML = '<b>Стоимость боя:</b>' + Auction.htmlPrice(art.costBat, precision) + '<br/>';
            nodeRep.parentNode.insertBefore(nodeBat, nodeRep.nextSibling.nextSibling.nextSibling);
            
        }
        
        if (showGuildCost) {
            
            // Стоимость гильдии кузнецов
            var costGuild = 4000 - 3500 * (guildLevel + 1) / 10 + 6500 / art.toughMax;
            var nodeGuld = document.createElement('div');
            var legend = 'Цена считается для первого ремонта когда максимальная прочность равна базовой.';
            nodeGuld.setAttribute('title', legend);
            nodeGuld.innerHTML = '<b>Стоимость очка гильдии кузнецов:</b>' + Auction.htmlPrice(costGuild) + '<br/>';
            nodeRep.parentNode.insertBefore(nodeGuld, nodeRep.nextSibling.nextSibling.nextSibling);
            
        }
        
        // Ссылка на рынок
        
        /** Обработчик загрузки документа аукциона (артефакт)
        * @param Array prices
        */
        function onLoadPrices(prices) {
            
            Auction.saveMaxMinPrices({id: artId}, prices);
            if (prices.length === 0)
                return false;
            
            var xpath = "//b[text()='Стоимость боя:']";
            var nodeBat = XPath.findFirst(xpath).parentNode;
            var nextNode = nodeBat.nextSibling;
            if (nodeBat == null) {
                var xpath = "//b[text()='Стоимость очка гильдии кузнецов:']";
                var nodeBat = XPath.findFirst(xpath).parentNode;
                var nextNode = nodeBat.nextSibling;
                if (nodeBat == null) {
                    var xpath = "//b[text()=' Стоимость ремонта:']";
                    var nodeBat = XPath.findFirst(xpath);
                    var nextNode = nodeBat.nextSibling.nextSibling.nextSibling;
                }
            }
            
            // HTML рыночной стоимости
            var table = '';
            var maxPrices = GM_getValue('maxPrices', 3);
            var count = Math.min(maxPrices, prices.length);
            for (var i = 0; i < count; i++) {
                table += 
                    ((i > 0) ? '<td>&nbsp;&nbsp;&nbsp;</td>' : '') + 
                    '<td title="С учетом налогов: ' + Auction.formPrice(Math.round(prices[i].cost * 0.99)) + '">' + prices[i].costHtml + '</td>';
            }
            table = '<table cellspacing="0" cellpadding="0" border="0"><tr>' + table + '</tr></table>';
            
            // Изменяем страницу (рыночная стоимость)
            var nodeAucPrice = document.createElement('div');
            var legend = 'В качестве стоимости на рынке берется минимальная запрашивая цена в режиме "Купить сразу" за данный артефакт с полной прочностью.';
            nodeAucPrice.setAttribute('title', legend);
            nodeAucPrice.innerHTML = '<b>Рыночная стоимость:</b>' + table + '<br/>';
            nodeBat.parentNode.insertBefore(nodeAucPrice, nextNode);
            
            
            if (showBattleCost) {
                
                // Рыночная стоимость боя
                var costAucBat = prices[0].cost / art.toughMax;
                var nodeAucBat = document.createElement('div');
                var legend = 'Минимальная раночная цена разделенная на максимальную прочность.';
                nodeAucBat.setAttribute('title', legend);
                nodeAucBat.innerHTML = '<b>Рыночная стоимость боя:</b>' + Auction.htmlPrice(costAucBat, precision) + '<br/>';
                nodeAucPrice.parentNode.insertBefore(nodeAucBat, nodeAucPrice.nextSibling);
            ...
            if (showMinMax) {
                ...
                var auctionArt = Auction.loadArt({id: artId});
                
                // HTML минимальной и максимальной цен
                table = 
                    '<table cellspacing="0" cellpadding="0" border="0"><tr>' + 
                    '<td id="auctionPriceMin" title="' + 
                        'Зарегистрирована: ' + Time.toDateTimeString(Time.fromNumber(auctionArt.priceMinTime)) + 
                        '\nС учетом налогов: ' + Auction.formPrice(Math.round(auctionArt.priceMin * 0.99)) + 
                        '\nДвойной клик для сброса.">' + Auction.htmlPrice(auctionArt.priceMin) + 
                    '</td>' +
                    '<td>&nbsp;&nbsp;&nbsp;</td>' +
                    '<td id="auctionPriceMax" title="' +
                        'Зарегистрирована: ' + Time.toDateTimeString(Time.fromNumber(auctionArt.priceMaxTime)) + 
                        '\nС учетом налогов: ' + Auction.formPrice(Math.round(auctionArt.priceMax * 0.99)) + 
                        '\nДвойной клик для сброса.">' + Auction.htmlPrice(auctionArt.priceMax) + 
                    '</td>' +
                    '</tr></table>';
                
                // Изменяем страницу (минимальная цена)
                var nodeAucPriceMinMax = document.createElement('div');
                var legend = 'Минимальная и маскимальная цены среди минимальных цен на рынке. При наведении указателя на цену отображается время регистрации. Сброс по двойному клику.';
                nodeAucPriceMinMax.setAttribute('title', legend);
                nodeAucPriceMinMax.innerHTML = '<b>Колебание на рынке:</b>' + table + '<br/>';
                nodeBat.parentNode.insertBefore(nodeAucPriceMinMax, nextNode);
                
                // Сброс минимальной цены
                var cell = document.getElementById('auctionPriceMin');
                cell.style.cursor = 'pointer';
                cell.ondblclick = function() {                     
                    auctionArt.priceMin = prices[0].cost;
                    auctionArt.priceMinTime = Time.toNumber();
                    Auction.saveArt(auctionArt);
                    document.location.reload();
                }
                
                // Сброс максимальной цены
                var cell = document.getElementById('auctionPriceMax');
                cell.style.cursor = 'pointer';
                cell.ondblclick = function() {                     
                    auctionArt.priceMax = prices[0].cost;
                    auctionArt.priceMaxTime = Time.toNumber();
                    Auction.saveArt(auctionArt);
                    document.location.reload();
                }
            ...
            // Добавляем ссылку 'выставить лот'
            var src = 'http://' + document.location.hostname + '/auction_new_lot.php';
            var data = '[<a href="' + src + '">выставить лот</a>]';
            $(nodeComments).append('&nbsp;&nbsp;&nbsp;' + data);
            $('a[href="' + src + '"]', nodeComments).click(
                function() {
                    GM_setValue('art', artId);
                    GM_setValue('price', prices[0].cost);
                }
            );
        ...
        /** Обработчик загрузки списка артефактов
        * @param Array arts
        */
        function onLoadList(arts) {
            
            // Ищем требуемый артефакт
            var auctionArt = null;
            ...
            if ((auctionArt !== null) && (art.url === undefined))
                auctionArt.url = 'http://' + document.location.hostname + '/auction.php?cat=' + auctionArt.cat + '&sort=0&art_type=' + auctionArt.id;
            
            
            // Изменяем страницу
            var data = (auctionArt !== null) ? '[<a href="' + auctionArt.url + '">смотреть на рынке</a>]' : '[нет на рынке]';
            $(nodeComments).append('&nbsp;&nbsp;&nbsp;' + data);
            
            if (auctionArt !== null) {
                
                // <img width="20" height="20" border="0" src="http://dcdn.heroeswm.ru/i/no_transfer.gif" title="Одеяние мага">
                var xpath = "//img[@src='http://dcdn.heroeswm.ru/i/no_transfer.gif']";
                var nodeNoTransfer = XPath.findFirst(xpath);
                
                // Если на артефакте не лежит запрет продажи
                if (nodeNoTransfer === null) {
                    
                    if (showAuctionCost) {
                        
                        // Загружаем цены артефакта
                        Auction.loadPricesList(auctionArt, document, onLoadPrices);
                        
                    }
                    
                } else {
                    
                    var data = '[не передается]';
                    $(nodeComments).append('&nbsp;&nbsp;&nbsp;' + data);
        ...
        // Если нужна ссылка на рынок
        if (showAuctionLink) {
            
            var auctionArt = Auction.loadArt({id: artId});
            if (auctionArt.cat === undefined) {
                
                // Загружаем общий список артефактов
                Auction.loadArtsList(document, onLoadList);
                
            } else {
                
                // Сразу добавляем ссылку на рынок
                onLoadList([auctionArt]);
    ...
    // На странице аукциона
    if (url.attr('path') == '/auction.php') {
        
        var auctionWork = GM_getValue('auctionWork', true);
        if (auctionWork) {
            
            // <input type="checkbox" onclick="javascript:window.location = '/auction.php?sort=4&amp;cat=cuirass&amp;art_type=hauberk&amp;type=0&amp;snew=0'" checked="" title="Показывать только целые артефакты" name="chb3">
            var checkbox = XPath.findFirst('//input[@name="chb3"]');
            if (checkbox.checked) {
            
                if (
                    (url.param('cat') !== undefined) &&
                    (url.param('art_type') !== undefined)
                ) {
                    
                    var maxPrices = GM_getValue('maxPrices', 3);
                    var prices = Auction.parsePricesList(document);
                    Auction.saveMaxMinPrices({id: artId}, prices);
