...
// @name           Dice Status
// @namespace      d2jsp
// @description    A list for dice games
...
holder.innerHTML += '<a href="http://forums.d2jsp.org/user.php?i=715221"' +
 ' title="List Runner" target="_blank">Dice Status</a>: ';
switch(resolveDiceType(user)){
 case 'med':
...
function getNames(){
	...
	{
	    method: 'GET',
	    url: 'http://multis.angelfire.com/dice',
	    headers: 
		{
	    ...
	    onload: function(response)
		{
			...
			var names = str.substring(str.indexOf('DiceStatusHere') + 14,str.indexOf('FinishDice'));
			eval(names);
			doThePage();
		}
	});
