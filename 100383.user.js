...
// @include http://*.klanlar.org/game.php*screen=place*
// ==/UserScript==
...
(function(){
	...
	if(/screen=place/.test(location.href) && /try=confirm/.test(location.href)) {
		...
		if(coords) {
			var td = $x('//form[contains(@action,"screen=place") and contains(@action,"try=confirm")]/descendant::a[contains(@href,"mode=recent")]');
			if(td) {
