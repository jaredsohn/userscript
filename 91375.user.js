// ==UserScript==
// @name           Facebook - Kendi İçinde Link Açma
// @namespace      Facebook - Kendi İçinde Link Açma
...
function start()
{
    ...
    if (c = document.getElementById('content')) {
    	check();
		if(document.location.href.indexOf(".facebook.com/external")>-1 && document.location.href.indexOf("#!/")==-1){
		}else{
...
function check(){
	if(document.location.href.indexOf(".facebook.com/external")>-1 && document.location.href.indexOf("#!/")==-1){
		...
		iframe.src = decodeURIComponent(document.location.href.split("/external?link=")[1]).replace(/\[dot\]/gi,".");
		...
		document.title = "MsNKoPaTFoRuM | " + iframe.src;
		...
		linkDesc.innerHTML = iframe.src + "&nbsp;&nbsp;<a style='color:#fff; font-weight:bold;' href='"+iframe.src+"'>Linki Aç</a>";
		...
		setTimeout(function(e){
			...
			for(e=0; e<a.length; e++){
				if(a[e].href.indexOf(".facebook.com")==-1 && a[e].href.indexOf("/external?link=")==-1 && a[e].href != "" && a[e].href != "#" && isntIgnored(a[e].href)){
					...
					a[e].href = "http://www.facebook.com/external?link=" + encodeURIComponent(a[e].href).replace(/\./gi,"[dot]");
				}
			}
		},100);
	}
