...
DataStorage = {
	...
	init: function() {
		if (DataStorage.hasLocalSQLiteStorage()) {
			try {
				DataStorage.database = openDatabase("gangwars_storage_redesign", "1.0", "Gangwars Redesign Storages");
				if (!DataStorage.database) {
					alert("Failed to open the database on disk. This is probably because the version was bad or there is not enough space left in this domain's quota");
				} else {
					DataStorage.database.transaction(function(tx) {
						tx.executeSql("SELECT COUNT(*) FROM gangwars_storage_redesign", [], function(result) {}, function(tx, error) {
							tx.executeSql("CREATE TABLE gangwars_storage_redesign (id TEXT UNIQUE, value TEXT, unixtime REAL)", [], function(result) {});
						});
					});
				}
			} catch (err) {}
	...
	set: function(key, value) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			...
			DataStorage.database.transaction(function(tx){
				tx.executeSql('INSERT INTO gangwars_storage_redesign (id, value, unixtime) VALUES (?, ?, ?)', [key, value.toSource(), 0]);
			});
		} else {
	...
	get: function(key) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			var value = null;
			DataStorage.database.transaction(function(tx){
				tx.executeSql('SELECT * FROM gangwars_storage_redesign WHERE id = ?', [key], function(result){
	...
	setCache: function(key, value, unixtime) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			DataStorage.remove(key);
			DataStorage.database.transaction(function(tx){
				tx.executeSql('INSERT INTO gangwars_storage_redesign (id, value, unixtime) VALUES (?, ?, ?)', [key, value.toSource(), parseInt(unixtime)]);
			});
		} else {
			unsafeWindow.sessionStorage.setItem(DataStorage.getHostname() + '::' + key, value.toSource());
	...
	remove: function(key) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			DataStorage.database.transaction(function(tx){
				tx.executeSql('DELETE FROM gangwars_storage_redesign WHERE id = ?', [key]);
			});
		} else {
...
var gw_urls = {
	police: 'http://redesign.gangwars.de/gangwars/police.php'
};
...
$(document).ready(function(){	
	...
	function highlightBuildings() {
		...
		$('#divMainContainer .tableSubContainer tbody #impress').each(function(){
