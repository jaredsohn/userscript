// ==UserScript==
// @name           spash extract javforum
// @namespace      org.eroim.essime
// @include        http://www.javforum.net/forumdisplay.php?*
// @version        0.1.1
// ==/UserScript==
(function() {
	...
	/* ======= content */
	{
		...
		function getThreadLinks() {
			...
			var linkList = x(
					'//*[@class="title" and starts-with(@href,"showthread")]',
					'linkList');
					...
					.addFunction(function() {
						...
						if (this.pb) {
							torrent_urls = this.pb
									.match(/attachment\.php.+type=torrent.+<\/a>/g);
						}
						...
						if (torrent_urls && torrent_urls.length > 0) {
							...
							for ( var j = 0; j < torrent_urls.length; j++) {
								...
								var attached_torrent_link = cE(
										...
										{
											href : torrent_url
													.match(/attachment\.php.+type=torrent[^"]*/),
