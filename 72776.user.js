...
var version = "1.2.177"; // cant use cdata+regexp to grab version dynamically because of webkit
...
whichRegex : /(decoration|bonus|hatch|adopt|bouquet|perfect bunch|present|fuel|help|collectible|get materials|horse|valentine|claim|some|gold|get an egg|get some eggs)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
colorRegex : /(baby|clydesdale|percheron|pinto|brown|black|white|gr[ae]y|pink patch|kelly green|green|pink)/,
...
easterRegex : /(get an egg|get some eggs)/,
...
accText : {
			...
			arborists : "Got this arborist!",
                        easter : "Got these painted eggs!",
                        claimeggs : "Got this Easter bonus item!"
		},
...
case "getsomeeggs" : if(text.find("Get some Eggs")) w="easter"; break;
case "getanegg" : if(text.find("Get an egg")) w="easter"; break;
...
case "claim": if(text.find("Gold pieces")) w="claimgold";
                                else if(text.find("Spring Eggs")) w="claimeggs";
...
gotItem : function(d) {
var doc=d, url=doc.URL.toLowerCase(), text=doc.body.textContent.toLowerCase(),
...
var doc=e.target.contentDocument, w=e.target.getAttribute("which"), key=e.target.getAttribute("id"), acc=main.getAccepted(), accTime=main.getAcceptedTime(), failed=main.getFailed(), failedTime=main.getFailedTime(), item=$("item_"+key), gotItem=main.gotItem(doc);
...
if(doc.URL.find("max_gift") || main.boxFullRegex.test(doc.body.textContent)) { // auto-pause when signal received of a full gift box
main.boxFull =  true;
...
if($("GM_config") || main.currReqs >= main.opts["maxrequests"]) {return;}
		var opts=main.opts, wallposts=$g(".//a[contains(@href,'"+main.gameURLpart+"') and not(starts-with(@id,'item_')) and contains(@href,'"+main.gameKeyUrlKeyword+"=') and not(contains(.,'"+main.xpQueryTextExcludes.join("')) and not(contains(.,'")+"'))]", {type:7, node:$(main.streamID)});
...
switch(w) {case "horse": w="adopt"; wA="grayhorse"; break;}
if(w != "none") {
try {
if(!acc[w]) {
acc[w] = new Array();
...
GM_config.init("<img src='"+imgs.logo+"'> v"+version, {
	...
	gold :  {
  	...
  	easter :  {
		...
		kids : {
			claimeggs : {
			label : "Claim Easter bonus items?",
			type : "checkbox",
			"default" : false
			}
		}
