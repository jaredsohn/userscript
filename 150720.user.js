// ==UserScript==
// @name        Analytics Realtime Infoscreen
// @namespace   codenerdari
// @description A script that transforms Google Analytics Realtime to be used on infoscreen/tv
// @author      @codenerd - http://codenerd.dk
// @include    *google.com/analytics*realtime*
...
// @resource  meta http://userscripts.org/scripts/source/150720.meta.js
// @version     1.0
...
var codenerdInfoSetup = function() {
	GM_config.init('Google Analytics Realtime Infoscreen Settings',
	/* Fields object */ 
	{
		'title':
		{
			'label': 'Custom title for counter (ie Active users on site)', // Appears next to field
			'type': 'text', // Makes this setting a text field
			'default': GM_getValue('title') // Default value if user doesn't change it
		},		
		'hideScroll':
		{
			'label': 'Hide scrollbars (cause scrollbars suck on info screens', // Appears next to field
			'type': 'checkbox', // Makes this setting a text field
			'default': GM_getValue('hideScroll') // Default value if user doesn't change it
		},
		'largeScreenFont':
		{
			'label': 'Optimize font size for big screen', // Appears next to field
			'type': 'checkbox', // Makes this setting a text field
			'default': GM_getValue('largeScreenFont') // Default value if user doesn't change it
		},
		'fontColor':
		{
			'label': 'Base font color (hex ie #000000)', // Appears next to field
			'type': 'text', // Makes this setting a text field
			'default': GM_getValue('fontColor') // Default value if user doesn't change it
		},
		'bgColor':
		{
			'label': 'Background color (hex ie #FFFFFF', // Appears next to field
			'type': 'text', // Makes this setting a text field
			'default': GM_getValue('bgColor') // Default value if user doesn't change it
		},	
		'bgImg':
		{
			'label': 'Background image (must be an url)', // Appears next to field
			'type': 'text', // Makes this setting a text field
			'default': GM_getValue('bgImg') // Default value if user doesn't change it
		},
		'dataTables':
		{
			'label': 'Show datatables', // Appears next to field
			'type': 'checkbox', // Makes this setting a text field
			'default': GM_getValue('dataTables') // Default value if user doesn't change it
		},					
...
var activateScript = function() {
	//wait a sec, before running
	window.setTimeout(function() {
		window.setInterval(function() {
		//remove backgrounds on all divs except graphs
		$("div").each(function (i) { 
			if (!$(this).hasClass('ACTION-filter')) {
				$(this).css({'background':'none'});
			}
		});
		//background  color and img
		if (GM_getValue('bgImg')!='') {
			$('body').css('background','url('+GM_getValue('bgImg')+')');			
		} else {
			$('body').css({
				backgroundColor:GM_getValue('bgColor')
			});
		}
		//remove the basic stuff everyone wants to get rid of
	
		//header 
		$('#ID-headerPanel,#ID-titlePanel').hide();
		//footer
		$('#ID-footerPanel').hide();
		//overtime is sucky on tv screen
		$('#ID-overtimePanel').hide();
		$('#ID-contentPanel-TableControlBar, #ID-contentPanel-Toggler').hide();

		$('#ID-realtimeReportFootnote').hide();
		$('#ID-contextOverviewPanel').css({
			position:'relative'
		});
		...
		try {
			if ($('.kfb').length>0) {
				var $elm = $('.kfb');	
			} else if ($('.Sdb').length>0) {
				var $elm = $('.Sdb');	
			}
		var width = ($('body').css('width').replace(/px/,'')/2)-($elm.css('width').replace(/px/,'')/2);	
		$elm.css({
			marginLeft:width+'px'
		});
		} catch(e) { }
		//font color
		$('.jD,.OC').css({
			color:GM_getValue('fontColor')
		});
		//title custom
		//quirky height calculator with custom title
		plusHeight=390;
		if (GM_getValue('title')!='') {
			$('.jD,.OC').text(GM_getValue('title'));
			plusHeight+=(GM_getValue('title').length/10)*80;
		} 
		//increase the counter font size
		$('#ID-overviewCounterPanel').css({
			color:GM_getValue('fontColor')
		});
		$('#ID-overviewCounterValue').css({
			color:GM_getValue('fontColor')
		});	
					
		var largeScreen = GM_getValue('largeScreenFont');
		if (largeScreen) {
			//increase font size
			$('.jD,.OC').css({
				fontSize:'100px',
			...
			//move the data tables down
			$('#ID-contextDetailsPanel').css({
				marginTop:plusHeight+'px'
			});
		}
		//datatables - overrule if we are on location page, map will never look good in high res, bye bye
		if (GM_getValue('dataTables')==0 || window.location.href.indexOf('location')!=-1) {
			$('#ID-contextDetailsPanel').hide();
			$('#ID-contextOverviewPanel').css({
				minHeight: $(window).height()

			});			
		} else {
			$('#ID-contextDetailsPanel').show();
			$('#ID-contextOverviewPanel').css({
				minHeight: 'auto'
				...
				//better readability in graphs
				$('.fb,.tB,.qob,.eP,.ZA,.ACTION-filter').css({
					color:'#444444',
					fontSize:'17px'
				});
			
					
		//hide the headers of tables, irrellevant 
		$('#ID-trafficPanel-TableControlBar').hide();
		//hide overflow - scrollbars on infoscreens SUCK
		$('body, html').css('overflow',(GM_getValue('hideScroll') ? 'hidden':'auto'));
},10);
	
	},1000);
}
$(document).ready(function($) {
	GM_setValue('standardTitle',$('.jD,.OC').text());	
	//check for first run after install
	if (GM_getValue('isInstall', 0) == 0) {
		
		//set standard values 
		GM_setValue('hideScroll',0);
		
		//font color
		GM_setValue('fontColor','#00000');

		//bg color
		GM_setValue('bgColor','#FFFFF');
		
		//bg image
		GM_setValue('bgImg','');
		
		//hide datatables
		GM_setValue('dataTables',0);
		
		//title
		GM_setValue('title','');
		//firstrun		
		GM_setValue('isInstall', 1);
  ...
  save: function () {
    ...
    for (id in fields)
      //if (fields[id].toValue() === null) // invalid value encountered
       // return;
		//codenerd
	GM_setValue(id,fields[id].toValue());
    this.write();
...
(function() {
  ...
  try {GM_configStruct.prototype.log = isGM ? GM_log : (window.opera ? opera.postError : console.log);} catch(e) { }
})();
...
//setup link in menu
GM_registerMenuCommand("Settings", codenerdInfoSetup);