...
function getVariable(name, defaultValue) {
    ...
    name = sCurrentServer +userID+"_"+name;
...
function createLink() {
		...
		var xpathRes = null;
		if(document.getElementById("textmenu")!=null)
			xpathRes = xpath("id('textmenu')//a[@href='warsim.php']");
	else
...
var tables=null;
	...
	tables = xpath("id('build')/table[@class='troop_details']");
...
function getUserID() {
		uLink = xpath("//div[@id='side_navi']//a[contains(@href, 'spieler.php')]");
		if(uLink.snapshotLength>0)
			uLink=uLink.snapshotItem(0);
		if (uLink) userID = uLink.href.split("uid=")[1];
		uLink = null;
		_log(1,'user id is '+userID)
		return;
	};