// ==UserScript==
// @name           TW - medeleden sorteren
// @namespace      Tribal Wars
// @description    Versie 1.1.0 | Maakt het sorteren van medeledenlijsten in browsergame "Tribal Wars" mogelijk.
...
var vers_ist = "TW - Medeleden sorteren 1.1.0";
...
if(url.match(/screen=settings&mode=settings/)) {
	...
	if(url.match(/einstellung=mitglieder/)) {
		...
		if(confirm("Titel:\n\nMoeten de titels van de medeleden verborgen worden en door een sterretje vervangen worden?\nAls je op afbreken klikt, worden de titels normaal weergegeven (naast de spelersnaam).")) {
	...
	if((GM_getValue("Titel-" + welt) == undefined)) {
		td[0].innerHTML = "<a href='http://" + land + welt + adresse[land] + land + "/game.php?village=" + dorf_id + "&screen=settings&mode=settings&einstellung=mitglieder''>Instellingen opslaan</a>";
		td[1].innerHTML = "<span class='grey'>Er werden momenteel nog geen instellingen opgeslagen.</span>";
	}
	else {
		td[0].innerHTML = "<a href='http://" + land + welt + adresse[land] + land + "/game.php?village=" + dorf_id + "&screen=settings&mode=settings&einstellung=mitglieder''>Instellingen veranderen</a>";
		var stammestitel = GM_getValue("Titel-" + welt);
		
		td[1].innerHTML += "<b style='padding-right:69px;'>Titel:</b>" + stammestitel.replace("an", "<span style='color:#090'>aan</span> <span class='grey'>(De titels van medeleden worden normaal getoond)</span>").replace("aus", "<span style='color:#C00'>uit</span> <span class='grey'>(De titels van medeleden worden door &#x2605; vervangen en bij het overgaan met de muis getoond.)</span>") + "<br>";
	}
	...
	if((url.match(/screen=/)) && 
		...
		if(confirm(vers_ist + " is geïnstalleerd.\n\nKlik op OK om de instellingen voor het tonen van de medeledenlijst door te nemen.")) {
...
if(url.match(/bashrank/)) {
	...
	for(i=1; i<zeilen; i++) {
		...
		if(allg.indexOf("Mio") > -1) {
			if(allg.replace(" Mio.", "").length > 1) {
				plus[i] += allg.replace(" Mio.", " <span class='grey'>miljoen</span>");
			}
			else {
				plus[i] += allg.replace(" Mio.", ".0 <span class='grey'>miljoen</span>");
			}
		}
		else {
			plus[i] += allg;
		...
		if(off.indexOf("Mio") > -1) {
			if(off.replace(" Mio.", "").length > 1) {
				plus[i] += ", " + off.replace(" Mio.", " <span class='grey'>miljoen</span>");;
			}
			else {
				plus[i] += ", " + off.replace(" Mio.", ".0 <span class='grey'>miljoen</span>");
			}
		}
		else {
			plus[i] += ", " + off;
		...
		if(deff.indexOf("Mio") > -1) {
			if(deff.replace(" Mio.", "").length > 1) {
				plus[i] += ", " + deff.replace(" Mio.", " <span class='grey'>miljoen</span>");;
			}
			else {
				plus[i] += ", " + deff.replace(" Mio.", ".0 <span class='grey'>miljoen</span>");
			}
		}
		else {
			plus[i] += ", " + deff;
	...
	document.getElementsByTagName("body")[0].innerHTML += "<div style='display:block; padding-bottom:10px; text-align:center;'><a href='http://userscripts.org/scripts/show/41120' target='_blank'>" + vers_ist.replace("TW - Medeleden sorteren", "TW - Medeleden sorteren <sup>") + "</sup></a> | <small style='color:#C00;'>heeft de huidige waarde opgeslagen.</small></div>";
...
if(url.match(/screen=ally/)) {
	...
	if((url.match(/screen=ally&mode=members/))) {
		...
		if(GM_getValue("Stamm-ID-" + welt) == undefined) {
			table.getElementsByClassName("vis")[0].getElementsByTagName("a")[1].innerHTML = "<blink style='color:red;' title='Om het script \"" + vers_ist + "\" te kunnen laten functioneren moet het profiel van de eigen stam minstens 1x bekeken zijn.'>" + table.getElementsByClassName("vis")[0].getElementsByTagName("a")[1].innerHTML + "</blink>";
		}
		...
		document.getElementsByTagName("h2")[0].innerHTML = "Medeleden <a href='" + stamm_link + "'>" + stamm_lang + "</a>";
	}
}
...
if((url.match(/screen=info_member/))) {
	...
	// Stamm:
	var stamm_kurz = document.getElementsByTagName("h2")[0].innerHTML.replace("Medeleden ", "");
	...
	document.getElementsByTagName("h2")[0].innerHTML = "Medeleden <a href='" + stamm_link + "'>" + stamm_kurz + " <small>" + stamm_lang + "</small></a>";
}
...
if((url.match(/screen=info_member/)) || (url.match(/screen=ally&mode=members/))) {
	...
	if(dots >= 1) {
		if(spalten == 12) {
			ausgabe += "<th colspan='16' style='text-align:center; -moz-border-radius-topleft:10px; -moz-border-radius-topright:10px; cursor:default;'>Algemene informatie</th>";
		}
		else {
			ausgabe += "<th colspan='9' style='text-align:center; -moz-border-radius-topleft:10px; -moz-border-radius-topright:10px; cursor:default;'>Algemene informatie</th>";
		}
	}
	else {
		ausgabe += "<th colspan='5' style='text-align:center; -moz-border-radius-topleft:10px; -moz-border-radius-topright:10px; cursor:default;'>Algemene informatie</th>";
	}
	...
	// Bashing:
	ausgabe += "<th colspan='5' style='text-align:center; -moz-border-radius-topleft:10px; -moz-border-radius-topright:10px;'>Basher-informatie <span class='grey'>von</span> <a href='http://bashrank.terenceds.de/show.php?server=de" + welt + "&ally=" + stamm_id + "&order=all' target='_blank' title='Om de basherpunten te tonen moet deze site minstens 1x bezocht worden.'>terenceds.de</a> bezoeken</th>";
	ausgabe += "</tr>";
	ausgabe += "<tr>";
	...
	for(i=0; i<spalten; i++) {
		...
		if(i == spalten-1) {
			if(dots >= 1) {
				ausgabe += "<th colspan='4' style='text-align:center;'><a href='" + sort_link + "S' title='Status van de speler: actief | inactief| VV'><img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/blue.png'> <img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/green.png'> VV <img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/yellow.png'> <img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/red.png'></a></th>";
			}
			ausgabe += "<th style='text-align:center;' title='basherpunten: algemeen'><a href='" + sort_link + "A'><img src='http://" + land + welt + adresse[land] + land + "/graphic/buildings/statue.png' style='height:12px;'><br />Alg.</a></th>";
			ausgabe += "<th style='text-align:center;' title='basherpunten: aanval'><a href='" + sort_link + "O'><img src='http://" + land + welt + adresse[land] + land + "/graphic/unit/att.png' style='height:12px;'><br />Aanv.</a></th>";
			ausgabe += "<th style='text-align:center;' title='basherpunten: verdediging'><a href='" + sort_link + "V'><img src='http://" + land + welt + adresse[land] + land + "/graphic/unit/def.png' style='height:12px;'><br />Verd.</a></th>";
			ausgabe += "<th style='text-align:center;' title='Kills in verhouding met eigen stam'><a href='" + sort_link + "KS'><img src='http://" + land + welt + adresse[land] + land + "/graphic/unit/unit_priest.png' style='height:12px;'><br />Kill: stam</a></th>";
			ausgabe += "<th style='text-align:center;' title='Kills in verhouden met eigen punten'><a href='" + sort_link + "KP'><img src='http://" + land + welt + adresse[land] + land + "/graphic/unit/unit_priest.png' style='height:12px;'><br />Kill: ptn.</a></th>";
		}
	}
	ausgabe += "</tr>";
	ausgabe += "</thead>";
	...
	var dorf_ges = 0;
	var dorf_ges = 0;
	var recht_ges = new Array(0, 0, 0, 0, 0, 0, 0);
	var gesamt_allg = 0;
	var gesamt_off = 0;
	var gesamt_deff = 0;
	var gesamt_kpro = 0;
	...
	for(i=1; i<zeilen; i++) {
		...
		if(zellen >= 5) {
			var uv = zellen-1;
			var recht = new Array("0", "0", "0", "0", "0", "0", "0");
			...
			if(zellen == 12) {
				...
				if(tabelle.getElementsByTagName("tr")[i].getElementsByTagName("td")[0].getElementsByTagName("img").length > 1) {
					...
					switch(dot[5]) {
						case "red.png?1":
						var stat = "<span class='grey'>al 1 week inactief</span>";
						var stat = "<span class='grey'>1 week</span>";
						var vert = "3-" + name;
						var farbe = "#FFDED3";
						status_w++;
						break;
						...
						case "yellow.png?1":
						var stat = "<span class='grey'>al 2 dagen inactief</span>";
						var stat = "<span class='grey'>2 dagen</span>";
						var vert = "2-" + name;
						var farbe = "#FFF6D3";
						status_t++;
						break;
						...
						default:
						var stat = "<span class='grey'>actief</span>";
						var vert = "1-" + name;
						var farbe = "#F7F6D3";
						status_a++;
					}
				...
				// UV anzeigen:
				var stat = "<span class='grey'>VV door</span> " + tabelle.getElementsByTagName("tr")[i].getElementsByTagName("td")[uv].innerHTML;
				var vert = "0-" + tabelle.getElementsByTagName("tr")[i].getElementsByTagName("td")[uv].getElementsByTagName("a")[0].innerHTML.toLowerCase() + name;
				var farbe = "#efeee3";
				status_u++;
			}
		...
		if(bash[0].indexOf("Mio") > 0) {
			gesamt_allg = (gesamt_allg*1) + (bash[0].replace(" <span class='grey'>miljoen</span>", "")*1000000);
		}
		else {
		...
		if(bash[1].indexOf("Mio") > 0) {
			gesamt_off = (gesamt_off*1) + (bash[1].replace(" <span class='grey'>miljoen</span>", "")*1000000);
		}
		else {
		...
		if(bash[2].indexOf("Mio") > 0) {
			gesamt_deff = (gesamt_deff*1) + (bash[2].replace(" <span class='grey'>miljoen</span>", "")*1000000);
		}
		else {
		...
		if(sort_typ == "N") {
			...
			if(bash.length > 1) {
				if(bash[0].indexOf("Mio") > 0) {
					mitglied[i][0] = bash[0].replace(" <span class='grey'>miljoen</span>", "")*1000000;
				}
				else {
			...
			if(bash.length > 1) {
				if(bash[1].indexOf("Mio") > 0) {
					mitglied[i][0] = bash[1].replace(" <span class='grey'>miljoen</span>", "")*1000000;
				}
				else {
			...
			if(bash.length > 1) {
				if(bash[2].indexOf("Mio") > 0) {
					mitglied[i][0] = bash[2].replace(" <span class='grey'>miljoen</span>", "")*1000000;
				}
				else {
		...
		if(titel.length > 8) {
			...
			mitglied[i][1] += "<td style='text-align:left; padding-right:5px; background-color:" + farbe + ";' title='" + memb + " heeft geen titel'>" + info.split("(")[0] + "</td>";
		}
		
		mitglied[i][1] += "<td style='text-align:right; padding-left:5px; background-color:" + farbe + ";' title='Rang in stam'>" + rang + "</td>";
		mitglied[i][1] += "<td style='text-align:right; padding-left:5px; background-color:" + farbe + ";' title='Totaal aantal punten van " + memb + "'>" + pkte + "</td>";
		mitglied[i][1] += "<td style='text-align:right; padding-left:5px; background-color:" + farbe + ";' title='Aantal dorpen van " + memb + "'>" + dorf + "</td>";
		if(zellen > 5) {
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Oprichter'>" + recht[0] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Leider'>" + recht[1] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Uitnodigen'>" + recht[2] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Diplomatie'>" + recht[3] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Rondschrijven'>" + recht[4] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Moderator in het interne forum'>" + recht[5] + "</td>";
			mitglied[i][1] += "<td style='text-align:center; background-color:" + farbe + ";' title='Verstopte fora'>" + recht[6] + "</td>";
		}
		if(dots >= 1) {
			mitglied[i][1] += "<td colspan='4' style='text-align:left; padding-left:5px; background-color:" + farbe + ";' title='Huidige status van " + memb + "'>" + stat + "</td>";
		}
		...
		if(bashing != undefined) {
			...
			mitglied[i][1] += "<td colspan='5' style='text-align:center; background-color:" + farbe + ";'><span class='grey'>Momenteel geen informatie beschikbaar.</span></td>";
		}
	...
	// Zusammenfassung:
	if(pkte_ges >= 1000000) {
		var pkte_gesamt = (pkte_ges/1000000).toFixed(1) + " <span class='grey'>miljoen</span>";
		if((pkte_ges/(zeilen-1)) >= 1000000) {
			var pkte_mittel = ((pkte_ges/1000000)/(zeilen-1)).toFixed(1) + " <span class='grey'>miljoen</span>";
		}
		else {
	...
	if(dorf_ges >= 1000000) {
		var dorf_gesamt = (dorf_ges/1000).toFixed(1) + " <span class='grey'>miljoen</span>";
		var dorf_mittel = ((dorf_ges/1000)/(zeilen-1)).toFixed(1) + " <span class='grey'>miljoen</span>";
	}
	else {
	...
	ausgabe += "<tr>";
	ausgabe += "<td style='text-align:right; cursor:default;' colspan='2'><span class='grey'>Samen:</span></td>";
	ausgabe += "<td style='text-align:right; cursor:default;' title='Aantal medeleden van de stam'>" + (zeilen-1) + "</td>";
	ausgabe += "<td style='text-align:right; cursor:default;' title='Totaal aantal punten'>" + pkte_gesamt + "</td>";
	ausgabe += "<td style='text-align:right; cursor:default;' title='Totaal aantal dorpen'>" + dorf_gesamt + "</td>";
	...
	if(gesamt_allg.toFixed(0) >= 1000000) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + (gesamt_allg/1000000).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	if(gesamt_off.toFixed(0) >= 1000000) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + (gesamt_off/1000000).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	if(gesamt_deff.toFixed(0) >= 1000000) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + (gesamt_deff/1000000).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	// Durchschnitt:
	ausgabe += "<tr>";
	ausgabe += "<td style='text-align:right; cursor:default;' colspan='2'><span class='grey'>Ø gemiddelde:</span></td>";
	ausgabe += "<td style='text-align:right; cursor:default;'></td>";
	ausgabe += "<td style='text-align:right; cursor:default;' title='Puntengemiddelde van de medeleden'>" + pkte_mittel + "</td>";
	ausgabe += "<td style='text-align:right; cursor:default;' title='Gemiddeld aantal dorpen per lid'>" + dorf_mittel + "</td>";
	// Durchschnitt (Status):
	if(zellen > 5) {
		ausgabe += "<td title='Oprichter'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/found.png'></td>";
		ausgabe += "<td title='Leider'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/lead.png'></td>";
		ausgabe += "<td title='Uitnodigen'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/invite.png'></td>";
		ausgabe += "<td title='Diplomatie'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/diplomacy.png'></td>";
		ausgabe += "<td title='Rondschrijven'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/mass_mail.png'></td>";
		ausgabe += "<td title='Moderator in het interne forum'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/forum_mod.png'></td>";
		ausgabe += "<td title='Verstopte fora'><img src='http://" + land + welt + adresse[land] + land + "/graphic/ally_rights/internal_forum.png'></td>";
	}
	...
	if(dots >= 1) {
		ausgabe += "<td style='text-align:right; width:25px;' title='VV'><img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/blue.png'></td>";
		ausgabe += "<td style='text-align:right; width:25px;' title='actief'><img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/green.png'></td>";
		ausgabe += "<td style='text-align:right; width:25px;' title='al 2 dagen inactief'><img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/yellow.png'></td>";
		ausgabe += "<td style='text-align:right; width:25px;' title='al 1 week inactief'><img src='http://" + land + welt + adresse[land] + land + "/graphic/dots/red.png'></td>";
	}
	
	// Durchschnitt (Bashing):
	if(gesamt_allg.toFixed(0) >= (1000000*(zeilen-1))) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + ((gesamt_allg/1000000)/(zeilen-1)).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	if(gesamt_off.toFixed(0) >= (1000000*(zeilen-1))) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + ((gesamt_off/1000000)/(zeilen-1)).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	if(gesamt_deff.toFixed(0) >= (1000000*(zeilen-1))) {
		ausgabe += "<td style='text-align:right; cursor:default;'>" + ((gesamt_deff/1000000)/(zeilen-1)).toFixed(1) + "&nbsp;<span class='grey'>miljoen</span></td>";
	}
	else {
	...
	if(dots >= 1) {
		if(spalten == 12) {
			ausgabe += "<th colspan='16' style='text-align:center; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; cursor:default;'>Algemene informatie</th>";
		}
		else {
			ausgabe += "<th colspan='9' style='text-align:center; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; cursor:default;'>Algemene informatie</th>";
		}
	}
	else {
		ausgabe += "<th colspan='5' style='text-align:center; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; cursor:default;'>Algemene informatie</th>";
	}
	if(GM_getValue("Datum-" + welt + "-" + stamm_id) != undefined) {
		ausgabe += "<th colspan='5' style='text-align:center; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; cursor:default;'>laatste update: <span class='grey'>" + GM_getValue("Datum-" + welt + "-" + stamm_id) + "</span></th>";
	}
	else {
		ausgabe += "<th colspan='5' style='text-align:center; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; cursor:default;'>Basher-infos <span class='grey'>van</span> <a href='http://bashrank.terenceds.de/show.php?server=de" + welt + "&ally=" + stamm_id + "&order=all' target='_blank' title='Om de basherpunten te tonen moet deze site minstens 1x bezocht worden.'>terenceds.de</a> bezoeken</th>";
	}
