// coding: utf-8
// ==UserScript==
// @name           Ikariam Marine Inutile
// @version	100
// @author		djbrown
// @namespace      marine-inutile.ikariam
// @description    Cacher les ressources transporte dans l'Armée de conseiller vue. Et cacher les mouvements des troupes en vue de la marine marchande.
// @include        http://s*.ikariam.*/*
// @include        http://s*.*.ikariam.*/*
// @exclude        http://support.ikariam.*/*
// ==/UserScript==

if (!MarineInutile) var MarineInutile = {};
...
MarineInutile =
	...
	/* Script metas */
	ScriptName:		 'Ikariam Marine Inutile',
	Version:		 100,
	HomePage:		 '',
	ScriptURL:		 '',
	UserScriptsID:	 108278
	};
	
MarineInutile.Init = function()
	{
	...
	this.PoweredBy		 += 'Renforcée par <a target="_blank" href="'+this.HomePage+'"><b>'+this.ScriptName+'</b></a> <span dir="ltr">(v. <i>'+this.Version+'</i>)</span>.';
	if (this.DB.Options['AvailableVersion'] > this.Version)
		this.PoweredBy += ' <a href="'+this.ScriptURL+'?version='+this.DB.Options['AvailableVersion']+'.user.js'+'" style="color: red;"><b>NOUVELLE VERSION V. <i>'+this.DB.Options['AvailableVersion']+'</i> DISPONIBLE !</b></a>';
	this.PoweredBy		 += '</p>';
...
MarineInutile.ViewIsMilitaryMovements = function()
	{
...
MarineInutile.Prepend_MilitaryMovements = function()
	{
	var nbMO = 0;
	var nbGT = 0;
	var nbFT = 0;
...
MarineInutile.Select_MilitaryMovements_Tab = function(filter)
	{
	// filter = MO, GT, or FT
...
MarineInutile.ViewIsMerchantNavy = function()
	{
	this.Set_Common_Styles();
...
MarineInutile.Collapse_MerchantNavy_Payloads = function()
	{
...
MarineInutile.Expand_MerchantNavy_Payloads = function()
	{
	var mainviewDIV = document.getElementById("mainview");
	if (mainviewDIV != null)
		{
...
MarineInutile.Insert_MerchantNavy_Header = function()
	{
	...
	var MarineInutileHeader = document.createElement('div');
	MarineInutileHeader.setAttribute("class", 'yui-navset');
	MarineInutileHeader.id = 'UselessNavyHeader';
	MarineInutileHeader.innerHTML = HeaderHTML;
	
	mainview.insertBefore(MarineInutileHeader, divs[1]);
...
MarineInutile.Select_MerchantNavy_Tab = function(boxIdSelected)
	{
	var tabs = this.DOM.Get_Nodes("//div[@id='UselessNavyHeader']//li");
	if (tabs.snapshotLength > 0)
		{
...
MarineInutile.Insert_MilitaryMovements_Header = function()
	{
	var mainview = document.getElementById("mainview");
	var combatsInProgress = document.getElementById("combatsInProgress");
...
MarineInutile.Insert_Footer = function()
	{
	var div = document.getElementById("UselessNavyFooter");
	if (div == null)
		{
...
MarineInutile.CheckScriptUpdate = function()
	{
...
MarineInutile._CompareScriptUpdate = function(availableVersion)
	{
...
MarineInutile.Set_Common_Styles = function()
	{
	// define CSS 
...
MarineInutile.Set_MilitaryMovements_Styles = function()
	{
	// define CSS 
...
MarineInutile.Display_Military_Movements = function()
	{
	// Soon deprecated
	var HiddenMovements = 0;
	var nodes = MarineInutile.DOM.Get_Nodes("//div[@id='mainview']//td[contains(@class, 'mission')]");
	if (nodes != null)
		{
	...
	if (div != null)
		{
		var sHTML = '<a id="HideUselessMovements" class="button" href="javascript://void(0);">Cacher '+HiddenMovements+' mouvement(s) inutile(s)</a>  Affiché pour votre plaisir.'+UselessNavy.PoweredBy;
		div.innerHTML = sHTML;
		
		var HideUselessMovements = document.getElementById("HideUselessMovements");
		HideUselessMovements.addEventListener('click', MarineInutile.Hide_Military_Movements, false);
		}
		
	};
	
MarineInutile.Hide_Military_Movements = function()
	{
	// Soon deprecated
	var HiddenMovements = 0;
	var nodes = MarineInutile.DOM.Get_Nodes("//div[@id='mainview']//td[contains(@class, 'mission')]");
	if (nodes != null)
		...
		for (var i=0; i < nodes.snapshotLength; i++)
			...
			if (mayHide == true)
				...
				MarineInutile.DOM.Add_ClassName(tr, 'useless');
	...
	if (div != null)
		{
		var sHTML = '<a id="DisplayUselessMovements" class="button" href="javascript://void(0);">Afficher '+HiddenMovements+' mouvement(s) inutile(s)</a> cachés pour votre commodité.'+UselessNavy.PoweredBy;
		div.innerHTML = sHTML;
		
		var DisplayUselessMovements = document.getElementById("DisplayUselessMovements");
		DisplayUselessMovements.addEventListener('click', MarineInutile.Display_Military_Movements, false);
		}
		
	};
		
MarineInutile.Set_MerchantNavy_Styles = function()
	{
	// define CSS 
...
MarineInutile.Set_MilitaryMovements_Alt_ClassNames = function()
	{
...
MarineInutile.Show_Useless_MilitaryMovements = function()
	{
	...
	if (div != null)
		{
		var sHTML = '<a id="HideUselessNavy" class="button" href="javascript://void(0);">Cacher '+HiddenTransports+' mouvement(s) inutile(s)</a> Affiché pour votre plaisir.'+this.PoweredBy;
		div.innerHTML = sHTML;
...
MarineInutile.Hide_Useless_MilitaryMovements = function()
	{
	var HiddenTransports = 0;
	var resMi = this.DOM.Get_Nodes("//div[@id='fleetMovements']//table[contains(@class, 'locationEvents')]/tbody/tr/td/img[contains(@src, 'mission_')]");
	if (resMi.snapshotLength > 0)
	...
	if (div != null)
		{
		var sHTML = '<a id="DisplayUselessNavy" class="button" href="javascript://void(0);">Afficher '+HiddenTransports+' mouvement(s) inutile(s)</a> cachés pour votre commodité.'+this.PoweredBy;
		div.innerHTML = sHTML;
...
MarineInutile.DB.Init = function(parent, host)
	{
	// Requires: Ikariam
	this._Parent = parent;
...
MarineInutile.DB.Serialize = function(data)
	{
	return uneval(data);
	};

MarineInutile.DB.UnSerialize = function(data)
	{
	return eval(data);
	};
	
MarineInutile.DB.Load_Options = function()
	{
...
MarineInutile.DB.Save_Options = function()
	{
	GM_setValue(this.Prefix+'.Opt', this.Serialize(this.Options));
	};
	
MarineInutile.Ikariam =
	{
	_Parent:				 null,
	_View:					 null,
	_Host:					 null,
...
MarineInutile.Ikariam.Init = function(parent)
	{
	this._Parent = parent;
	};
	
MarineInutile.Ikariam.View = function()
	{
	if (this._View == null)
		{
		this._View = '';
...
MarineInutile.Ikariam.Host = function()
	{
	if (this._Host == null)
		{
		this._Host = '';
...
MarineInutile.Ikariam.Grab_JSCompleteScript_Version = function(defaultVersion)
	{
	// Requires: DOM
	var Version = '';
...
MarineInutile.Ikariam.Version = function()
	{
	// Requires: DOM
...
MarineInutile.Ikariam.Revision = function()
	{
	// Requires: DOM, Str
...
MarineInutile.Ikariam.Is_Version_032x = function()
	{
	// Requires: Str
...
MarineInutile.Ikariam.Is_Version_041x = function()
	{
	// Requires: Str
...
MarineInutile.Ikariam.Is_Version_043x = function()
	{
	// Requires: Str
...
MarineInutile.Ikariam.FleetMovement_Object = function()
	{
...
MarineInutile.Ikariam.Fetch_FleetMovements = function(database)
	{
	// Require: DOM, Str
...
MarineInutile.Ikariam.MerchantNavyBox_Object = function()
	{
...
MarineInutile.Ikariam.Fetch_MerchantNavy_Boxes = function(boxesDB, includeMissions)
	{
...
MarineInutile.Handlers =
	{
	_Parent: null
	};
	
MarineInutile.Handlers.Init = function(parent)
	{
	this._Parent = parent;
	};
	
MarineInutile.Handlers.Attach_MerchantNavy_Tabs_Events = function()
	{
	var self = this;
...
MarineInutile.Handlers.TransportPayload_Button_Click_Event = function(e)
	{
	if (!e) { e = window.event; }
...
MarineInutile.Handlers.CombatInProgress_Bar_Click_Event = function(e)
	{
	if (!e) { e = window.event; }
	var obj = e.srcElement ? e.srcElement : e.target;
...
MarineInutile.Handlers.MerchantNavy_Tab_Click_Event = function(e)
	{
	if (!e) { e = window.event; }
	var obj = e.srcElement ? e.srcElement : e.target;
	while (obj.tagName != 'A')
...
MarineInutile.DOM.Init = function(parent)
	{
	this._Parent = parent;
	};

MarineInutile.DOM.Create_Document = function(responseText)
	{
...
MarineInutile.DOM.Get_Nodes = function(path, root)
	{
...
MarineInutile.DOM.Get_First_Node = function(path, root)
	{
...
MarineInutile.DOM.Get_First_Node_TextContent = function(path, defaultValue, root)
	{
...
MarineInutile.DOM.Get_First_Node_Title = function(path, defaultValue, root)
	{
	var value = this.Get_First_Node(path, root);
...
MarineInutile.DOM.Has_ClassName = function(oElm, strClassName)
	{
...
MarineInutile.DOM.Add_ClassName = function(oElm, strClassName)
	{
	/*
...
MarineInutile.DOM.Remove_ClassName = function(oElm, strClassName)
	{
	/*
	Copyright Robert Nyman, http://www.robertnyman.com
	Free to use if this text is included
...
MarineInutile.Log.Init = function(parent)
	{
	this._Parent = parent;
	};
	
MarineInutile.Log.Add = function(msg)
	{
...
MarineInutile.Updater.Init = function(parent)
	{
	this._Parent = parent;
	};
	
// CallBackFct function receive available version number (or null value if failed) as argument
MarineInutile.Updater.Check = function(ScriptURL, CallBackFct)
	{
...
MarineInutile.Updater._ParseScript = function(response, CallBackFct)
	{
...
MarineInutile.Str.Init = function(parent)
	{
	this._Parent = parent;
	};
	
MarineInutile.Str.Trim = function(str)
	{ 
	if (str != undefined)
		{
		str = str.replace(/&nbsp;/gi, " ");
...
MarineInutile.Str.Trim_Accodances = function(str)
	{
	str = str.replace(/\[.+\]/gi, "");
...
return 0 if v2 = v1
	...
	-1 if v2 < v1
*/
MarineInutile.Str.Compare_Versions = function(v1, v2)
	{
	var result = 0;
...
MarineInutile.Str.To_Integer = function(str, defaultValue)
	{
