...
// @description    +KONTROLL.PANEL+ for Kingdoms of Camelot
// @require        http://sizzlemctwizzle.com/updater.php?id=95542
// ==/UserScript==


var Version = 'HAKAN.A.-MM-0.0.2.1.1';
...
var Options = {
  ...
  pbChatOnRight: false,
  pbWideMap    : false,
  alertConfig  : {aChat:false, aPrefix:'***Liebe Alli werde angegriffen,bitte unterstütz mich!! ***', scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
...
function pbStartup (){
  ...
  window.name = 'PT';
  logit ("* +kontroll.panel+:" + Version + " geladen");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  actionLog ("+KONTROLL.PANEL+:" + Version + " geladen.. (KofC version: "+ anticd.getKOCversion() +")");
...
//	logit(div);
        ...
        var m = '<DIV id=pbTowrtDivF class=ptstat>BURG OPTION</div><TABLE id=pbtowerfunctions width=100% height=0% class=pbTab><TR>';
        if (t.alertState.running == false) {
            m += '<TD><INPUT id=pbAlertState type=submit value="Audio ALARM = ZU"></td>';
       }
        else {
            m += '<TD><INPUT id=pbAlertState type=submit value="Audio ALARM = OFFEN"></td>';
        }
        m += '</tr></table></div>';
        m += '<DIV id=pbAlertDivD class=ptstat>STÄDTE ZUSTAND</div><TABLE id=pbalertdetails width=100% height=0% class=ptentry><TR>';
		...
		for (var i = 0; i < Cities.cities.length; i++) {
		    if (parseInt(Seed.citystats["city" + Cities.cities[i].id].gate) == 0) {
				 m += '<TD><CENTER><INPUT id=pbsanctuary_' + Cities.cities[i].id + ' type=submit value="VERTEIDIGUNG = NEIN" style="border:1px solid black; background-color:#0a0;"></center></td>';
			}
			if (parseInt(Seed.citystats["city" + Cities.cities[i].id].gate) == 1) {
				 m += '<TD><CENTER><INPUT id=pbsanctuary_' + Cities.cities[i].id + ' type=submit value="VERTEIDIGUNG = AN" style="border:1px solid black; background-color:red;"></center></td>';
			}
        }
        m += '</tr><TR>';
        m += '</tr></table></div>';
...
// George will fix this when tower tab is refactored for Power Tools update      
    ...
    toggleAlertState: function(obj){
        ...
        if (t.alertState.running == true) {
            ...
            t.saveAlertState();
            obj.value = "Audio ALARM = ZU";
        }
        else {
            t.alertState.running = true;
            t.saveAlertState();
            obj.value = "Audio ALARM = OFFEN";
        }
    },
	show: function(){
		var t = Tabs.tower;
...
Tabs.build = {
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbBuildDivF class=ptstat>BAU VORHABEN</div><TABLE id=pbbuildfunctions width=100% height=0% class=pbTab><TR>';
        if (t.buildStates.running == false) {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Auto BAU = zu"></td>';
        }
        else {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Auto BAU = OFFEN"></td>';
        }
		m += '<TD><INPUT id=pbBuildMode type=submit value="Bau Modus = ZU"></td>';
		m += '<TD>Build Type: <SELECT id="pbBuildType">\
				<OPTION value=build>einfache Bau</option>\
				<OPTION value=max>Bau max.</option>\
				<OPTION value=destruct>Abbrieß</option>\
				</select></td>';
		m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>BITTEN UM HILFE?</td>';
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=ptstat>BAU ÜBERSICHT</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
    ...
    doOne : function (bQi){ 
		...
		if (typeof Seed.buildings['city' + currentcityid]["pos" + citpos][0] != 'undefined') {	
			...
			if (curlvl > 8 && mode == 'build') { 
				...
				actionLog("BAU BEGIN HÖCHST LEVEL!!!");
				return;
			};
			if (isNaN(curlvl)) {
				actionLog("kein richtiger Level Gefunden für das gegenwärtige Gebäude!!!!");
				return;
			}
			if (l_bdgid != bdgid) {
				actionLog("Das Bauen des Typs passt nicht zusammen!!!!");
				return;
			}
			if (l_bid != bid) {
				actionLog("Das Bauen des ID passt nicht zusammen!!!!");
				return;
			}
			if (l_curlvl < curlvl) {
					t.cancelQueueElement(0, currentcityid, time, false);
			...
			if (mode == 'destruct') {
				...
				new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/destruct.php" + unsafeWindow.g_ajaxsuffix, {
					...
					onFailure: function(){
						document.getElementById('pbbuildError').innerHTML = "WEGEN VERBINDUNGS PROBLEME KONNTE IHR WUNCH NICHT BETÄTIGT WERDEN..BITTE VERSUCHEN SIE ERNEUT!!!";
					}
				})
			}
			if (mode == 'build') {
				...
				if (invalid == false) {							
					...
					new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/construct.php" + unsafeWindow.g_ajaxsuffix, {
						...
						onSuccess: function(rslt){
							if (rslt.ok) {
								...
								if (rslt.error_code == 103) { // building has already the target level => just  delete
									t.cancelQueueElement(0, currentcityid, time, false);
									actionLog("Bau versuch fehlgeschlagen: Bau level ist schon verbaut oder ist schon im Bau!");
								} else {
									t.requeueQueueElement(bQi);
						...
						onFailure: function(){
							document.getElementById('pbbuildError').innerHTML = "VERBINDUNGS PROBLEM,VERSUCHEN SIE ES BITTE ERNEUT!!!";
						}
					});
				} else {
    ...
    modalmessage: function(message){
	    var t = Tabs.build;
        var timeout = 10000;
        var content = "AUTOMATISCHE SCHLIESSUNG INERHALB 10 SEK....<br><br>"
        content += message;
    ...
    _addTab: function(queueId, cityId, buildingType, buildingTime, buildingLevel, buildingAttempts, buildingMode){
        ...
        row.insertCell(6).innerHTML = '<a class="button20" id="queuecancel_' + queueId + '"><span>ABBRUCH</span></a>';
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>Stadt wo Gebaut wird ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="ONACH DER ZEIT AUFLISTEN"></td>';
    ...
    toggleStateMode: function(obj){
		var t = Tabs.build;
        if (obj.value == 'BAU MODUS = ZU') {
			unsafeWindow.buildslot = t.bot_buildslot; // overwrite original koc function
            obj.value = "BAU MODUS = OFFEN";
        }
        else {
			unsafeWindow.buildslot = t.koc_buildslot; // restore original koc function
			obj.value = "BAU MODUS = ZU";
        }
    },
  ...
  init : function (div){
    ...
    div.innerHTML = '\
        ...
        <SELECT id="pasrcType">\
          ...
          <OPTION value=1>WILDNIESE</option>\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>AUSGANGS PUNKT: &nbsp; X: </td><TD class=xtab>\
...
for (var i in Provinces) {
	m += '<option value="'+i+'">'+Provinces[i].name+'</option>';
      ...
      m += '</td></tr><TR><TD class=xtab align=right>Max. Distanz: </td><TD class=xtab><INPUT id=pasrcDist size=4 value=10 /></td></tr>';
	 
      m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=pasrcStart type=submit value="STARTE SUCHE"/></td></tr>';
      m += '</table>';
  ...
  clickedSearch : function (){
    ...
    if (t.searchRunning){
      t.stopSearch ('ABBRUCH DER SUCHE!!');
      return;
    }
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X KÄSTCHEN VON 0 BIS 749 EINGEBEN!<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y KÄSTCHEN VON 0 BIS 749 EINGEBEN!<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>100)
      errMsg += "MaX. EINGABE IST 1 bis 100!!<BR>";
    if (errMsg != ''){
    ...
    m = '<CENTER><B>WIRD GESUCHT '+ typeName +'<BR>\
        Ausgangs punkt: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distanz: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>OPTION AUSWAHL:</b><BR></td></tr>';
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. der Level:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. der Level:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>WILDNIES TYP:</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'Wisse/See', 3:'Walt', 4:'Hügel', 5:'Berg', 6:'Ebene', 0:'ALLES'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Ohne Besitzer:</td><TD class=xtab><INPUT id=pafilUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Sortiere nach:</td><TD class=xtab><SELECT id=pafilSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
			...
			<TR><TD class=xtab align=right>Nur die Koordinaten Anzeigen:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
    ...
    document.getElementById ('pastatStatus').innerHTML = 'SUCHE NACH '+ xxx +','+ yyy;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    if (dat.length == 0){
      m = '<BR><CENTER>Kein Treffer</center>';
    } else {
      dat.sort(mySort);
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    if (t.opt.searchType==0 && document.getElementById('KOCAttackToggle')!=null){    
      document.getElementById ('pbSrcExp').innerHTML = '<CENTER>'+ strButton20('Exportiere Resultate', 'id=pbSrcDoExp') +'</center>'; 
...
//logit (" 3 mapCallback()");
    ...
    document.getElementById('pastatSearched').innerHTML = 'Gesucht: '+ t.tilesSearched;
    t.dispMapTable();
...
var exportToKOCattack = {
  ...
  doExport : function (coordList, city){
    ...
    var m = '<DIV class=ptstat>Daten zu KOC ATTACK Senden</div><BR><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabPadNW>\
      <TR style="font-weight:bold; background-color:white"><TD>ZIELTYP</td><TD style="padding:1px" align=center>#<BR>ZIELE</td><TD width=15></td>';
    ...
    if (isKOCattack){
      m += '<BR><CENTER>'+ strButton20('SENDE KOORDINATEN ZU KOC Attack', 'id=pbSrcDoBA') +'</center>';
    } else {
    ...
    popExp.getTopDiv().innerHTML = '<CENTER><B>KONTROLL.PANEL</b></center>';
    if (isKOCattack)    
...
Tabs.sample = {
  ...
  init : function (div){    // called once, upon script startup
    ...
    div.innerHTML = '<CENTER><BR>This is a sample tab implementation<BR><BR>NAHRUNG '+ cityName +' : <SPAN id=pbSampleFood>0</span>\
        <BR><BR>(Nahrungstand,jeder 5 sek. erneuert)</center>';
  },
...
Tabs.Test = {
  ...
  createFakeAttack : function (cityNum, isScout, isWild, isFalse, secs, numMilitia){
    ...
    march.players.u1234567.n = 'Hakan-MM-';
...
Tabs.ActionLog = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<DIV class=ptstat>ACTION TABELE</div><DIV style="height:535px; max-height:535px; overflow-y:auto">\
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>KONTROL PANEL OPTIONS:</b></td></tr>\
        <TR><TD><INPUT id=pballowWinMove type=checkbox /></td><TD>Kontroll.PANEL verschiebbar</td></tr>\
        <TR><TD><INPUT id=pbTrackWinOpen type=checkbox /></td><TD>Erinnert sich an Fenster, nach neu laden</td></tr>\
        <TR><TD><INPUT id=pbHideOnGoto type=checkbox /></td><TD>Hide window when clicking on map coordinates</td></tr>\
        <TR><TD colspan=2><BR><B>EINSTELLUNGEN:</b></td></tr>\
        <TR><TD><INPUT id=pbFairie type=checkbox /></td><TD>verhindert öffnen von popup Fenster </td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Breitwand (alle Gebiete,größere spielfeld)</td></tr>\
        <TR><TD><INPUT id=pbWatchEnable type=checkbox '+ (GlobalOptions.pbWatchdog?'CHECKED ':'') +'/></td><TD>Ladet neu wenn innerhalb von 1 Minute,wenn nicht KOC Ladet (alle Gebiete)</td></tr>\
        <TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>in der angegebenen min. neu laden des spiels <INPUT id=pbeverymins type=text size=2 maxlength=3 \> minuten</td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Stellt Chat auf dem Recht Bildschirm seite </td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>vergrößert sich die karte!!</td></tr>\
        <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>Bei erreichen des angegebenen % wird Gold eingetrieben!! <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Automatisch wird beim Angegriffen eine benachrichtigung an die alli im alli chat gesendet!!.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Angriffs benachrichtigung: &nbsp; </td><TD><INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD align=right>Alamiert bei Spionage versuch: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Alamiert wenn Wildnies Angegriffen wird: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Minimum # an Truppen: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
            </table></td></tr>\
...
Tabs.Gifts = {
  ...
  init : function (div){
    var t = Tabs.Gifts;
    t.myDiv = div;    
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD width=200></td><TD align=center><INPUT id="pasubGifts" type=submit value="GESCHENKE ÜBERPRÜFEN" \></td><TD width=200 align=right><INPUT id=paGiftHelp type=submit value=HELP></td></tr></table><HR>\
  ...
  helpPop : function (){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>KONTROLL.PANEL HILFE</b>: GESCHENKE ANNEHMEN</center>';
    pop.show (true);
  },
  ...
  e_clickedGifts : function  (){     // (also cancel accepting)
    ...
    document.getElementById('giftDiv').innerHTML = 'Verbiendungs AUFBAU zu Facebook Geschenke Seite ...';
...
//logit (inspect (t.gifts, 8, 1));     
      var m = '<DIV class=ptstat><CENTER>KOC GESCHENKE &nbsp; &nbsp; &nbsp; ('+ t.gifts.length +' Gefunden)</center></div>';
      if (t.gifts.length<1){
        document.getElementById('giftDiv').innerHTML = m + '<BR><BR><CENTER>GESCHENKE nicht gefunden!</center>';
        return;
      }
      m += '<TABLE class=pbTab align=center><TR><TD align=right>GESCHENKE ANNAHME IN GEBIET (SERVER): </td><TD>'
        + htmlSelector (Options.giftDomains.list, getServerId(), 'id=pbGiftServers') +'</td></tr>\
          <TR><TD align=right>Lösche Benachrichtigungen nach annahme der Geschenke</td><TD>'
        + htmlSelector ({y:'Always', e:'Only if Error', n:'Never'}, Options.giftDelete, 'id=pbGiftDel')
        + '</td></tr><TR><TD>Klicken Sie die Geschenke an die sie annehmen wollen!: </td><TD width=250><INPUT type=submit id=pbGiftDo value="GESCHENKE ANNEHMEN">\
        ...
        <TR style="font-weight:bold; background:white"><TD>GESCHENKE</td><TD>DATUM</td><TD>HERHALTEN VON(server)</td><TD width=20></td></tr>';
  ...
  getErDone : function (){ 
    ...
    if (t.doList.length==0){
      document.getElementById('pbGiftNone').innerHTML = 'Bitte annehmen!';
      return;
    }
    t.doServer = document.getElementById('pbGiftServers').value;
    t.accepting = true;
    document.getElementById('pasubGifts').value = 'Annahme Stoppen'; 
    document.getElementById('giftDiv').innerHTML = '<DIV id=acpDiv style="height:400px; max-height:400px; overflow-y:auto"><B>Angenohmen '+ t.doList.length +' GESCHENKE:</b><BR></div>';    
    t.acceptNext ();
  },
  ...
  acceptNext : function (){
    ...
    if (gift == null){
      t.allDone ('GESCHENKE SIND ANGENOHMEN.'); 
      return;
    }
