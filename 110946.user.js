// ==UserScript==
// @name             |DeLiFiseK|_S_A_N_C_A_K_ Oto Saldırı
// @namespace        DeLiFiseK-_S_A_N_C_A_K_
// @description      KoC Saldırmayı Kolaylaştıran Oto Saldırı..
// @homepage       http://userscripts.org/scripts/show/110946
// @include          *apps.facebook.com/kingdomsofcamelot*
// @include          *kingdomsofcamelot.com/*main_src.php*
// @include          *kabam.com/games/kingdoms-of-camelot/*game*
...
// @icon           http://b1108.hizliresim.com/11/8/20/12750.bmp
// @require        http://tomchapin.me/auto-updater.php?id=103835

// ==/UserScript==

var KOCAversion = '_S_A_N_C_A_K_';
...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		var m = '<a id="ImportBoxCancel" class="button20"><span>ÇIK</span></a>';
		m += '<a id="ImportData" class="button20"><span>Taşıma</span></a>';
		m += '<a id="ExportData" class="button20"><span>Aktarma</span></a>&nbsp;';
		...
		citysel += '<option value=options>Seçenekler</option>';		
		citysel += '</select>';
		...
		//WinLog.write(inspect(cities));
		var citysel ='<select id=srcCity>';
		citysel += '<option value=All>Tüm Şehirler</option>';
		for(var c=0; c<cities.length; c++) {
		...
		levelsel += '<option value="All">Tüm Seviyeler</option>';
		for(var c=1; c<=11; c++) {
			levelsel += '<option value="'+c+'">Seviye '+c+'</option>';
		}
		levelsel += '</select>';

		var typesel ='<select id=srcAttackType>';
		typesel += '<option value="All">Tümü</option>';
		typesel += '<option value="normal">Saldırılar</option>';
		typesel += '<option value="transport">Taşımalar</option>';
		typesel += '<option value="bulkadded">Eklenen Koord.</option>';
		typesel += '</select>';

		div.style.display='block';
		div.innerHTML='';
		var m = '<DIV id="srcAttackOpts" style="height:30px">\
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Arama Seçenekleri: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Hepsi</option>\
		<OPTION value=Camp>Barbar Kampları</option>\
		<OPTION value=Wilderness>Bozkırlar</option>\
		<OPTION value=Grassland>Çayır</option>\
		<OPTION value=Lake>Göl</option>\
		<OPTION value=Mountains>Dağ</option>\
		<OPTION value=Forest>Orman</option>\
		<OPTION value=Hills>Tepe</option>\
		<OPTION value=Plain>Ova</option>\
		<OPTION value=City>Şehir</option>\
		</select></td>\
		<td class=xtab width=100 align=center>Şehir: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Seviye: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>İçerik: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>ÇIK</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Saldırı Listesi</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Listeyi Temizle</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Seçileni SİL</span></a>\
		<a id="KOCAttackViewAttacksIgnore" class="button20"><span>Gizle/Göster</span></a>\
		<a id="KOCAttackViewAttacksImportExport" class="button20"><span>Taşıma/Aktarma</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Şehir</td><td>Koordinat</td><td nowrap>İçerik</td>';
		  h += '<td>Type</td><td>Asker Sayısı</td><td></td><td>Zamanı</td><td>GİZLE</td></tr>';
		  var tableRows= '';
		  var count = 1
	  ...
	  tableRows += '</td>';
		  ...
		  ById('KOCAttackViewAttacksDelete').addEventListener('click',function() {
			...
			window.alert(deletes+' Koordinatlar Silindi');
	...
	ReloadWindow:function() {
		...
		var t = '<FORM target="_top" action="'+ goto +'" method=post><INPUT id=xxButReload type=submit value=Yükleniyor..><input type=hidden name=s value="'+ m +'"</form>';
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			"<tr><td valign='top' align='center'><span style='color:red'><img src='img/units/unit_6_50.jpg' /></td><td valign='top'>"+
			"<span style='color:darkgreen'>Saldırı Şekli:</span> <select id='KOCAttackOrder'><option value='mostTroops'><span style='color:black'>Asker Sayısına Göre</option><option value='closest'>Önce Yakın Hedeflere</option></select><br />"+
			"<span style='color:black'>Saldırı İçeriği:</span> <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'><span style='color:lightgreen'>Barbar Kampı</span> "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'><span style='color:blue'>Bozkırlar</span> "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'><span style='color:#46C2C0'>Şehir</span> "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'><span style='color:black'>Taşıma</span><br> "+
			"</span><br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /><span style='color:darkgreen'> Saniyede Bir Saldır.</span><br />"+
			"<span style='color:black'>Saldırı Zamanlamaları</span><br />"+
			"<div style='margin-left: 40px'>"+
			"<span style='color:lightgreen'>Barbar Saldırı Zamanı:</span><input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' /><span style='color:green'>Saat<br />"+
			"<span style='color:blue'>Bozkır Saldırı Zamanı:</span><input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' /><span style='color:black'>Saat<br />"+
			"<span style='color:#46C2C0'>Şehir Saldırı Zamanı:</span><input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' /><span style='color:green'>Saat<br />"+
			"<span style='color:black'>Taşıma Zamanı:</span><input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' /><span style='color:black'>Dakika"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /><span style='color:green'>Maksimum Saldırılan Mesafe</span><br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /><span style='color:black'>Eklenen Kordinatları Sadece Eklediğim Şehirden Saldır.<br />(Önemli)</span><br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /><span style='color:green'>Şovalyeleri Manuel Yürüyüş Yapılıken Seçme.</span></font> <br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /><span style='color:black'> Burası,Oyun da Aşırı Trafikden Doğan Hatalar İçindir...</span><br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> <span style='color:green'>Saldırı Anında Oto Saldırıyı Durdur(Saldırılar Bitince Tekrardan Başlamaz)</span><br />"+			
			"<span style='color:black'>Girilen url adresi yeni bir sekmede açılır..</span><br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+

			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /><span style='color:green'> Saldırı raporlarını Otomatik Olarak Sil</span><br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /><span style='color:black'> Tneden Fazla Rapor Oluştugunda Silemye Başla</span><br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /><span style='color:green'> Saldırı Raporlarının Görüntülenmesini Devre Dışı Bırak.</span><br />"+

			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' /><span style='color:black'>% ye Geldiğinde MUTLULUĞU Arttır</span><br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' /><span style='color:green'>% de Mutluluk Olduğunda ALTINI Arttır</span><br />"+
			"</td></tr>"+

			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> <span style='color:black'>Oyun Sırasında Oluşan Açılır Pencereleri Engelle</span><br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /><span style='color:green'>Yardım İSteklerini Otomatik Olarak Yayınla</span><br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /><span style='color:black'>İttifak ChatBox da Oluşan Yardım İsteklerini GİZLER..</span><br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /><span style='color:green'>Yapılan İnşaat ve Araştırmaları Otomatik Olarak Yayınlar.</span><br />"+
			"<span style='color:black'>Yapılan İnşaat ve Araştırmaları Otomatik Olarak Yayınlar.</span><select id='KOCAttackAutoPublishPrivacy'><option value='80'>Herkes</span></option><option value='50'><span style='color:blue'>Arkadaşlarım ve Arkadaşları</span></option><option value='40'><span style='color:blue'>Yanlızca Arkadaşlarım</span></option><option value='10'><span style='color:blue'>Yanlızca Kendime</span></option></select><br />"+
			"</td></tr>"+

			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /><span style='color:green'>Bakım ve Bağlantı Hataların da Alanı Yeniden Yükler..</span><br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /><span style='color:black'>Firefox dan Kaynaklı Doğacak Olan Hataları Raporlayarak İlgili Yerlere İletir..(GEREKSİZDİR..!)</span><br />"+
			"</td></tr>"+

			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /><span style='color:green'>  Saniyede Bir Şehirleri Gez..</span><br />"+
			"<span style='color:black'>Bu Şehirden:</span> <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /><span style='color:black'> saniye beklenir ve </span>"+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /><span style='color:green'>Saniye sonrada sayfa yenilenir...</span><br />"+
			"<span style='color:black'>Saldırı Yapılacak Şehirler: </span>"+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' /><span style='color:green'>% Rastgele Gecikme Ayarları..</span><br />"+
			"<span style='color:black'>Taşıma/Aktarma: Komutları ve Koordinatları Buraya Yapıştırmalısınız..</span><br /><textarea id='KOCAttackImport'></textarea>"+
			"<a class='button20' id='KOCAttackImportButton'><span>Taşıma</span></a> <a class='button20' id='KOCAttackExportButton'><span>Aktarma</span></a><br />"+
			"</td></tr>"+

			"</table>"+

			"<TABLE width=100%><TR><TD>"+
			"<a id='KOCAttackOptionsSave' class='button20'><span>Kaydet</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Çıkış</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Seçenekleri Sıfırla</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Fabrika Ayarlarına Dön</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Kayıtlı Saldırıları Sil</span></a>"+
			"<a id='KOCAttackDeleteTransports' class='button20'><span>Kayıtlı Nakliyeleri Sil</span></a>"+
	...
	AddViewAttacksLink:function() {
		...
		a=this.AddTabLink('Saldırıları Grntüle');
		if(!a) {
			window.setTimeout(function() {
	...
	*/
	...
	SetAttackStatusMessage:function() {
		...
		if(!toggle) {
			var t=this;
			toggle=this.AddTabLink('OTO-SALDIRI');
			if(!toggle) {
				window.setTimeout(function() {
					t.SetAttackStatusMessage();
				},t.GetRandTime(250));
		...
		if(autoAttack) {
			toggle.innerHTML='<span>Oto Saldırı--Açık</span>';
		} else {
			toggle.innerHTML='<span>Oto Saldırı--Kapalı</span>';
		}
	},
	...
	*/	
	...
	GetOptions:function() {
		...
		var defOptions={"attackDelay":5,
			...
			"noViewReports":false,
			"chromeKeepReports":2,
			"percentOfPopToTrain":200,
			"autoGoldHappiness":200,
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"<span style='color:green'>Lord</span><span style='color:black'> DeLiFiseK</span><span style='color:green'> Oto Saldırı Scriptidir</span><br /><input id='KOCAttackBulkAddForce' type='checkbox' /><span style='color:black'>Yanlış Kordinat Giriş Hatalarını Engelle.(Önemli)</u></span><br />");


		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"<span style='color:green'>Saldırısı Yapılacak Olan Lokasyonlar:</span><br />");
		var arrData = [["Camp", "Barbar Kampı"],["", "Şehir"],["Transport", "Taşıma"],["Wilderness", "Bozkır"]];
		...
		AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /><span style='color:green'>Oto Bozkır İçin Önden Yollanan Saldırıdır.</span><br />");
		...
		bulkAdd.innerHTML='<span>Kordinatları Kaydet.</span>';
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert('♦♦♦Eklenen Kordinat: '+added+' Tane♦♦♦');
		},false);
		AddHtml(bulkAddDiv,"<br />");

		var bulkAddAttackLink=document.createElement('a');
		bulkAddAttackLink.className='buttonDown20';
		bulkAddAttackLink.innerHTML='<span>Çoklu Kordinat Ekle</span>';
	...
	return;
	  ...
	  a.innerHTML = 'Saldırı İtemlerini Gizle/Göster';   
	  //inp.type='checkbox';
	  span.appendChild (a);
	...
	HandleDomainLogin:function() {
		...
		if (t.currentPage == "domain_selection" && t.options.autoLogBackIn && !t.domainLoginActionTaken) {
			...
			if(playButtons && playButtons.snapshotLength>0){
				...
				statusDiv.innerHTML='<center>OTO-Saldırı:Otomatik Olarak Yönlendirileceksiniz '+timeDifference+' Saniye İçinde...</center>';
				if(timeDifference==0){
					...
					statusDiv.innerHTML='<center>OTO-Saldırı: Otomatik Olarak Yönlendirileceksiniz ŞİMDi..!</center>';
					var functionCall = {
						'action':'load_url',
				...
				statusDiv.innerHTML='<center>OTO-Saldırı: Otomatik Olarak Yönlendirileceksiniz '+timeDifference+' Saniye İçinde...</center>';
				if(timeDifference==0){
					statusDiv.innerHTML='<center>OTO-Saldırı: Otomatik Olarak Yönlendirileceksiniz ŞİMDİ.!!</center>';
					t.ReloadWindow();
					t.domainLoginActionTaken=true;
				}
			}
	...
	OnCastleBoxAppear:function(box) {
		...
		function SetAutoGoldA() {
			var str=t.GetAutoGold()?'OTO-Açık':'OTO-Kapalı';
		...
		function SetAbandonWildsA() {
			var str=t.GetAbandonWilds()?'Bozkırlarını TERKET.!':'Bozkırlarını TUT.!';
			b.innerHTML='<span>'+str+'</span>';
		}
		b.addEventListener('click',function() {
			var abandonWilds=t.GetAbandonWilds();
			if(!abandonWilds){
				var abandonWildsConfirm = confirm("Bozkırlarınızı Terketmek İstediğinizden Eminmisiniz..??")
	...
	OnAttackBoxAppear:function(box) {
		...
		addBtn.innerHTML='<span>Saldırı Ekle</span>';
		...
		if(attack) {
			...
			if(attack.time) {
				AddHtml(div,'Son Saldırı: '+SecsToStr(nowSecs-attack.time)+' Önce Gerçekleşti <br />');
			}
			if(attack.comment){
	...
	//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
			...
			deleteBtn.innerHTML='<span>Saldırıyı SİL!</span>';
	...
	*/
	...
	AddCheckBarbarians:function() {
		...
		a.innerHTML='<span>RAPORLARI SİL!</span>';
		...
		a.className='buttonDown20';
		//a.style.paddingLeft='30px';
		a.innerHTML='<span>Raporları SİLME</span>';
		if(msgBody.nextSibling) {
			msgBody.nextSibling.insertBefore(a,msgBody.nextSibling.childNodes[0]);
			//msgBody.nextSibling.appendChild(a);
		} else {
	...
	AddTrainTroopsLink:function() {
		...
		var setTrainTroopsA=function() {
			...
			a.innerHTML='<span>'+(trainTroops[t.GetCurrentCityId()]==type?'OTO-Eğitim-Açık':'OTO-Eğitim-Kapalı')+'</span>';
		}
		a.addEventListener('click',function() {
			var trainTroops=JSON2.parse(t.GetValue('TrainTroops','{}'));
	...
	*/
	...
	DeleteAllStoredAttacks:function() {
		...
		window.alert("Kayıtlı Olan Bütün Koordinatlar SİLİNDİ..!! Şimdi Sayfa Yeniden Yüklenecek  ...");
	},
	...
	SetupMenu:function() {
		var t=this;
		GM_registerMenuCommand('OTOSaldırı-Seçenekleri',function() {
			t.ShowOptionsDialog();
		});
		GM_registerMenuCommand('OTOSaldırı-Saldırıları İZLE',function() {
			t.ShowViewAttacksDialog();
		});
		GM_registerMenuCommand('OTOSaldırı-TAŞIMA',function() {
			t.ShowImportExportBox();
		});

		GM_registerMenuCommand('OTOSaldırı-OTOSaldırı',function() {
			t.ToggleAutoAttack();
		});
		GM_registerMenuCommand('OTOSaldırı-Fabrika Ayarlarına DÖN..!!',function() {
			t.FactoryReset();
		});
	},
	...
	*/
	...
	Listen:function() {
		...
		if(t.currentPage == "koc_game"){
			//Check DeLiFiseKYenileme error
			setTimeout(function(){DeLiFiseKYenileme();},7000);

			window.setTimeout(function() {
				if(!t.pageLoaded && t.GetAutoAttack() && !t.IsMapperRunning()) {
					GM_log("Amaninnn Hata Verdi..! Lütfen 60 saniye Bekleyiniz.. :)");
					t.SetValuesCache();
					t.ReloadWindow();
				}
...
function checkWhiteScreen (){
  ...
  function DeLiFiseKYenileme (){
GM_log("DeLiFiseK Sayfa Yenileme");
 if (!document.getElementById("kochead")){
	window.setTimeout ( function() { GM_log ("DeLiFiseK Yenileme"); KOCAttack.ReloadWindow(); }, 3000);
	popup (100,100,500,275, "<BR><CENTER>Sayfa Yenileme <BR><BR>Sayfa 5sn İçinde Yenilencek..<BR><BR><BR><BR><BR><BR><BR><BR>Lord DeLiFiseK<BR><BR>");
  }  
}
