...
EmpireBoard.Ikariam.Init = function(parent)
	...
	// We need the _DefaultLocalizationStrings if we can't read the real ones Ikariam provides.  
        // This happens when we're running under Blank Canvas in Chrome.  Too bad these are always 
        // English, but its the best we can do.
	this._DefaultLocalizationStrings['timeunits'] = {};
	this._DefaultLocalizationStrings['timeunits']['short'] = {};
	this._DefaultLocalizationStrings['timeunits']['short']['day'] = 'd';
	this._DefaultLocalizationStrings['timeunits']['short']['hour'] = 'h';
	this._DefaultLocalizationStrings['timeunits']['short']['minute'] = 'm';
	this._DefaultLocalizationStrings['timeunits']['short']['second'] = 's';
	this._DefaultLocalizationStrings['language']                     = 'us';
	this._DefaultLocalizationStrings['decimalPoint']               = '.';
	this._DefaultLocalizationStrings['thousandSeperator']     = ',';
		
	this._DefaultLocalizationStrings['resources'] = {};
	this._DefaultLocalizationStrings['resources']['wood'] = 'Building material';
	this._DefaultLocalizationStrings['resources']['wine'] = 'Wine';
	this._DefaultLocalizationStrings['resources']['marble'] = 'Marble';
	this._DefaultLocalizationStrings['resources']['crystal'] = 'Crystal Glass';
	this._DefaultLocalizationStrings['resources']['sulfur'] = 'Sulfur';
	this._DefaultLocalizationStrings['resources'][0] = this._DefaultLocalizationStrings['resources']['wood'];
	this._DefaultLocalizationStrings['resources'][1] = this._DefaultLocalizationStrings['resources']['wine'];
	this._DefaultLocalizationStrings['resources'][2] = this._DefaultLocalizationStrings['resources']['marble'];
	this._DefaultLocalizationStrings['resources'][3] = this._DefaultLocalizationStrings['resources']['crystal'];
	this._DefaultLocalizationStrings['resources'][4] = this._DefaultLocalizationStrings['resources']['sulfur'];
	};
...
EmpireBoard.Ikariam.Fetch_FleetMovements = function(database)
	...
	if (resMi.snapshotLength > 0)
		{
		console.log('Found '+resMi.snapshotLength+' fleets');
		...
		for (var i=0; i < resMi.snapshotLength; i++)
			{
			var tr = resMi.snapshotItem(i).parentNode.parentNode;
			// Does not work in Chrome because it selects all nodes of type td, including 
			// nested content.  (As the standard says it should.
			//var tds = tr.getElementsByTagName("td");
			...
			if (fleetId != '')
				...
				var oPlayerName = this._Parent.Str.Trim(tds[3].childNodes[1].nodeValue);
...
EmpireBoard.Ikariam.LocalizationStrings = function(valueName,sectionName,subSectionName)
	...
	if (this._LocalizationStrings == null)
		...
		if (unsafeWindow.LocalizationStrings != undefined)
			...
			{
			this._LocalizationStrings = this._DefaultLocalizationStrings;
			}
		}
...
EmpireBoard.Intl.Load_Texts_CZ = function()
	...
	buildings = {
		...
		"winegrower"   : ["Vinarství", "Vinarství"],
		"alchemist"    : ["Vež alchymisty", "Vež alchymisty"],
		"architect"    : ["Pracovna architekta", "Architekt"],
		"optician"     : ["Optik", "Optik"],
		"vineyard"     : ["Vinný sklep", "Vinný sklep"],
		"fireworker"   : ["Zkušebna ohnostroje", "Zkušebna ohnostroje"]
		};
	texts = {
		"cityName"          : "Mesto",
		...
		"Population"        : "Populace",
		"finishedBuilding"  : "Dokoncené stavby",
		"Incomes"           : "Príjmy",
		"Trading"           : "Obchod",
		"Wood"              : "Drevo",
		"Wine"              : "Víno",
...
EmpireBoard.Intl.Load_Texts_GR = function()
	...
	buildings = {  
		"townHall" : ["??µa??e??", "??µa??e??"],  
		"temple"      : ["?a??", "?a??"],  
		"academy" : ["??ad?µ?a", "??ad?µ?a"],  
		"port" : ["?µp?????? ??µ??a?", "?µp?????? ??µ??a?"],  
		"shipyard" : ["?a?p??e??", "?a?p??e??"],  
		"warehouse" : ["?p????? ?µp??e?µ?t??", "?p????? ?µp??e?µ?t??"],  
		"wall" : ["?e??? t?? ?????", "?e??? t?? ?????"],  
		"tavern" : ["?aß???a", "?aß???a"],  
		"museum" : ["???se??", "???se??"],  
		"palace" : ["?a??t?", "?a??t?"],  
		"palaceColony" : ["? ?at????a t?? ??ße???t?", "? ?at????a t?? ??ße???t?"],  
		"embassy" : ["??esße?a", "??esße?a"],  
		"branchOffice" : ["T?s? ?µp?????? S??a??a???", "T?s? ?µp?????? S??a??a???"],  
		"safehouse" : ["???sf??et?", "???sf??et?"],  
		"barracks" : ["St?at??e?", "St?at??e?"],  
		"workshop" : ["???ast????", "???ast????"],  
		"carpentering" : ["?????????", "?????????"],  
		"forester" : ["Sp?t? ??????p??", "Sp?t? ??????p??"],  
		"stonemason" : ["?t???? ?at?µe???", "?t???? ?at?µe???"],  
		"glassblowing" : ["?a?????e??", "?a?????e??"],  
		"winegrower" : ["?p?sta?t????", "?p?sta?t????"],  
		"alchemist" : ["?????? ????µ?st?", "?????? ????µ?st?"],  
		"architect" : ["????te?t????? G?afe??", "????te?t????? G?afe??"],  
		"optician" : ["?pt????", "?pt????"],  
		"vineyard" : ["??est???? Staf?????", "??est???? Staf?????"],  
		"fireworker" : ["?e????? ????µ?? ????te???µ?t??", "?e????? ????µ?? ????te???µ?t??"]  
		};  
	texts = {  
		"Upkeep":"S??t???s?",
		"cityName": "???µa ?????", "currentlyBuilding": "??aßa?µ??ete t??a", "summary": "S?????",  
		"hide_settings": "????e ???µ?se??", "show_settings": "?µf???se ???µ?se??",  
		"Population": "?????sµ??",  
		"Research": "??e??e?",  
		"finishedBuilding": "????????µ??? ??aßa?µ?s?","Incomes":"??s?d?µata","Trading":"?µp????",  
		"Wood": "????d?µ??? ?????", "Wine": "??as?", "Marble": "???µa??", "Crystal": "???sta???", "Sulfur": "Te??"  
		};  
	};
...
EmpireBoard.Intl.Load_Texts_HU = function()
	...
	texts = {
		...
		"Wood": "Építoanyag", "Wine": "Bor", "Marble": "Márvány", "Crystal": "Kristály", "Sulfur": "Kénpor"
		};
	};
...
EmpireBoard.Intl.Load_Texts_LV = function()
	...
	langtype = "";
	buildings = {
		"townHall" : ["Ratsnams", "Ratsnams"],
		"temple" : ["Templis", "Templis"],
		"academy" : ["Akademija", "Akademija"],
		"port" : ["Osta", "Osta"],
		"shipyard" : ["Kugu buvetava", "Kugu buvetava"],
		"warehouse" : ["Noliktava", "Noliktava"],
		"wall" : ["Muris", "Muris"],
		"tavern" : ["Krogs", "Krogs"],
		"museum" : ["Muzejs", "Muzejs"],
		"palace" : ["Pils", "Pils"],
		"palaceColony" : ["Gubernatora rezidence", "Gubernatora rezidence"],
		"embassy" : ["Vestnieciba", "Vestnieciba"],
		"branchOffice" : ["Tirgus", "Tirgus"],
		"safehouse" : ["Pasleptuve", "Pasleptuve"],
		"barracks" : ["Kazarmas", "Kazarmas"],
		"workshop" : ["Darbnica", "Darbnica"],
		"carpentering" : ["Namdaris", "Namdaris"],
		"forester" : ["Mežsarga maja", "Mežsargs"],
		"stonemason" : ["Akmenkalis", "Akmenkalis"],
		"glassblowing" : ["Stikla putejs", "Stiklinieks"],
		"winegrower" : ["Vina audzetajs", "Vina audzetajs"],
		"alchemist" : ["Alhimika Tornis", "Alhimika Tornis"],
		"architect" : ["Arhitekta Ofiss", "Arhitekts"],
		"optician" : ["Optikis", "Optikis"],
		"vineyard" : ["Vina Gatavotajs", "Vina Gatavotajs"],
		"fireworker" : ["Ugunošanas izmeginajumu apgabals", "Ugunošanas apg."]
		};
	texts = {
		"Upkeep" :"Uzturešana",
		"cityName" : "Pilseta",
		"currentlyBuilding" : "Šobrid celas",
		"summary" : "Kopa",
		"hide_settings" : "Slept iestatijumus",
		"show_settings" : "Radit iestatijumus",
		"Population" : "Apdzivotiba",
		"Research": "Izpete",
		"finishedBuilding" : "Pabeigta celtnieciba",
		"Incomes" : "Zelts",
		"Trading" : "Tirznieciba",
		"Wood" : "Koks",
		"Wine" : "Vins",
...
EmpireBoard.Intl.Load_Texts_PL = function()
	...
	buildings = {
		...
		"carpentering" : ["Warsztat Ciesli", "Warsztat Ciesli"],
		"forester" : ["Lesniczówka", "Lesniczówka"],
		"stonemason" : ["Kamieniarz", "Kamieniarz"],
		"glassblowing" : ["Huta Szkla", "Huta Szkla"],
		"winegrower" : ["Winnica", "Winnica"],
		"alchemist" : ["Wieza Alchemika", "Wieza Alchemika"],
		"architect" : ["Biuro Architekta", "Biuro Architekta"],
		"optician" : ["Optyk", "Optyk"],
		"vineyard" : ["Winiarz", "Winiarz"],
		"fireworker" : ["Zaklad Pirotechnika", "Zaklad Pirotechnika"]
		};
	texts = {
		"Upkeep"			:"Koszty utrzymania",
		"cityName": "Nazwa", "currentlyBuilding": "W budowie", "summary": "Suma",
		"hide_settings": "Ukryj ustawieni", "show_settings": "Pokaz ustawienia",
		"Population": "Populacja",
		"Research": "Badania",
		"finishedBuilding": "Budowa zakonczona","Incomes":"Bilans zlota","Trading":"Handel",
		"Wood": "Drewno", "Wine": "Wino", "Marble": "Marmur", "Crystal": "Krysztal", "Sulfur": "Siarka"
		}; 
	};
...
EmpireBoard.Intl.Load_Texts_SL = function()
	...
	buildings = {
		...
		"glassblowing" : ["Steklopihac", "Steklopihac"],
		...
		"fireworker" : ["Testno obmocje ognjemetov", "Testno obmocje ognjemetov"]
		};
	texts = {
		"cityName" : "Ime mesta",
		...
		"Research": "Raziskave",
		"finishedBuilding" : "Zgradba koncana",
...
EmpireBoard.Intl.Load_Texts_TR = function()
	...
	texts = {
		...
		"Population": "Nufus","finishedBuilding": "Insaa Bitti","Incomes":"Gelir","Trading":"Ticaret",
...
EmpireBoard.Intl.Load_Texts_VN = function()
	...
	langtype = ""; 
	buildings = {
		"townHall"      : ["Tòa th? chính", "Tòa T.Chính"],
		"temple"      : ["Temple", "Temple"],
		"academy"       : ["H?c vi?n", "H?c vi?n"],
		"port"          : ["C?ng giao d?ch", "C?ng GD"],
		"shipyard"      : ["Xu?ng dóng tàu", "Xu?ng tàu"],
		"warehouse"     : ["Kho hàng", "Kho"],
		"wall"          : ["Tu?ng thành", "Tu?ng"],
		"tavern"        : ["Quán ru?u", "Quán ru?u"],
		"museum"        : ["Vi?n b?o tàng", "V.B.Tàng"],
		"palace"        : ["Cung di?n", "Cung di?n"],
		"palaceColony"  : ["Ph? th? hi?n", "Ph?"],
		"embassy"       : ["Tòa d?i s?", "Tòa Ð.S?"],
		"branchOffice"  : ["Tr?m giao d?ch", "Tr?m GD"],
		"safehouse"     : ["Noi ?n náu", "Noi ?n náu"],
		"barracks"      : ["Tr?i lính", "Tr?i lính"],
		"workshop" 	  : ["Xu?ng", "Xu?ng"],
		"carpentering" : ["Th? m?c", "Th? m?c"],
		"forester" : ["Nhà ki?m lâm", "Ki?m lâm"],
		"stonemason" : ["Th? xây dá", "Th? dá"],
		"glassblowing" : ["Ngu?i th?i th?y tinh", "Th?i TT"],
		"winegrower" : ["Máy ép nho", "Ép nho"],
		"alchemist" : ["Gi? kim", "Gi? kim"],
		"architect" : ["Tòa ki?n trúc", "Ki?n trúc"],
		"optician" : ["Th? kính", "Th? kính"],
		"vineyard" : ["Vu?n nho", "V.Nho"],
		"fireworker" : ["Th? thu?c súng", "Thu?c súng"]
		};
	texts = {
		"cityName": "Thành ph?", "currentlyBuilding": "Ðang xây d?ng", "summary": "T?ng",
		"hide_settings": "?n thi?t l?p", "show_settings": "Hi?n th? thi?t l?p",
		"Population": "Dân s?",
		"finishedBuilding": "Công trình hoàn t?t","Incomes":"Thu nh?p","Trading":"Trao d?i",
		"Wood": "G?", "Wine": "Ru?u", "Marble": "C?m th?ch", "Crystal": "Pha lê", "Sulfur": "Luu hu?nh"
		};
	};
...
function digProducedResources(res) 
	{
        ...
        var findResourceCounterData = function(name)
            {
        ...
        var sWood = findResourceCounterData('wood');
        var startResourcesDelta = null;
        if (sWood != null) {
            startResourcesDelta = /production: *([0-9.\E\-]+)/.exec(sWood);
        }
        ...
        var sTradeGood = findResourceCounterData('tradegood');
        var startTradegoodDelta = null
        if (sTradeGood != null) {
            startTradegoodDelta = /production: *([0-9.\E\-]+)/.exec(sTradeGood);
        }
	...
	if (EmpireBoard.Ikariam.Is_Version_033x())
		...
		if (ucList.snapshotLength >= 1)
			...
			//var startdate = unsafeWindow.tmppbar['startdate'];
				...
				var nScript = scripts[j];
				var sCode = nScript.innerHTML;
				if (sCode.indexOf('var tmppbar = getProgressBar') > 0)
					{
					var enddate = parseInt(/enddate: (\d+)/.exec(sCode)[1]);
					var currentdate = parseInt(/currentdate: (\d+)/.exec(sCode)[1]);
