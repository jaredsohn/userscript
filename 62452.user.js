...
// @description An Update for the TC Script and translate to english
...
theCrims = function() {
  ...
  this.getInformation = function() {
	...
	switch (armorName.toLowerCase()) {
	  ...
	  case 'nano fiber combat jacket':{
		this.armor = 5;
		break;
	  }
	  case 'nomex plated armor':{
		this.armor = 6;
		break;
	  }
  ...
  this.calcWhatRobb = function(yourPower) {
	...
	var places = new Array('Shoplift','Old lady','Car break-in','Taxi','Carding',
	                       'House','Pizza place','Gas station','The Cinema','Grocery store',
						   '7-Eleven','Kidnapping','Jewellery','Safety deposit',
						   'Little City Bank','Maffia Boss','Car Saloon','Paypal',
						   'Local bastards','Local dealer','Computer store','Rave party','Hypermarket',
						   'National museum', 'Casino', 'Russian drug king');
  ...
  this.calcWhatGG = function(yourPower) {
    ...
    var places = new Array('Forex','Bank','Value transport','Federal reserve',
                           'Steven Seagull','Manipulate stock','Al Capone','Drug factory',
                           'Fuckingham Palace','Casino Royal');
  ...
  this.getAllRobbInfo = function(ev) {
    ...
    if (!document.getElementById('robbInfoBlock')) {
      ...
      var infoOutput = '<table id="robbTable">'+
                       ...
                       '  <td rowspan="4">Difficulty</td>'+
                       '  <td rowspan="4">Estamina</td>'+
                       '  <td colspan="4"><strong>Single-user crimes</strong></td>'+
                       '</tr>'+
                       '<tr>'+
                       '  <td>Safe</td>'+
                       '  <td>Normal</td>'+
  ...
  this.output = function() {
    ...
    header.innerHTML = 'Information About User '+this.username;
    ...
    conteiner.innerHTML = '- Maximum Building you can have: <span class="menuyellowtext">'+this.calcMaxBuilding()+'</span><br />'+
                          '- Bribe in Prison: <span class="menuyellowtext">$'+this.calcBribe()+'</span><br />'+
						  '<div class="robbBlock">'+
                          '  - Single-user crimes<br/>'+
						  '  &nbsp;&nbsp;&nbsp;(Safe): '+
                          ...
                          '</div>'+
                          '<div id="robbButton" style="clear: both; margin-left: 10px;"><span style="cursor: pointer; border: 1px solid #383838; background-color: #000; color: #fff; padding: 1px;">+ All Robberies</span></div>'+
                          '<div class="robbMessage">To upgrade your power, go to <span style="color: #fff">Arms dealer</span></div><div class="robbMessage">The Revenge Gang is back<span style="color: #fff">[ToP4FilM.CoM]</span></div>';
