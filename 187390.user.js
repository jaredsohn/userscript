...
// @updateURL       http://userscripts.org/scripts/source/187390.user.js
// @downloadURL     http://userscripts.org/scripts/source/187390.user.js
// ==/UserScript==
...
extraPanel.innerHTML = '<a id="getPriceButton" class=\"btn_green_white_innerfade\"><span>Get/Set Price</span></a><span id=\"scriptStatus\">Ready</span>';
...
function GetLowestPrice() {
	...
	GM_xmlhttpRequest({
		...
		onload: function (response) {
			...
			if(result2 != "Unknown") {
				set_result2 = result2.match(/\d+[\.,]\d+/g, ' ');
			}
			...
			SetLowestPrice(set_result, set_result2);
		}
	});
}
// function to set the price
function SetLowestPrice(result, result2) {
	document.querySelector("#market_sell_buyercurrency_input").value = result;
	document.querySelector("#market_sell_currency_input").value = result2;
}