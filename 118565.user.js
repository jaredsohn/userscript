// ==UserScript==
// @name           Google Translator Tooltip[modified]
// @version        v20120104
// @author         Je
// @namespace      Je@user.js
...
var used=GM_getValue('enable')?GM_getValue('enable'):"do";
...
//	divLookup = createElement('div', {id: 'divLookup', style: 'top:'+(evt.clientY+window.pageYOffset+10)+'px; left:'+(evt.clientX + window.pageXOffset + 10) + 'px;'});
	divLookup = createElement('div', {id:'divLookup', style:'background-color:#D0E8F9 !important; color:#000000; position:absolute; top:'+(evt.clientY+window.pageYOffset+2)+'px; left:'+(evt.clientX+window.pageXOffset)+'px; padding:0px; z-index:10000;'});
...
//	divDic = createElement('div', {id:'divDic'});
	divDic = createElement('div', {id:'divDic', style:'background-color:#D6E9F8 !important; border:1px solid #0066CC; color:#000000; position:absolute; top:'+top+'; left:'+left+'px; min-width:250px; min-height:50px; max-width:60%; padding:1px; font-size:small; text-align:left; z-index:10000;'});
...
//		tl = GM_getValue('to') ? GM_getValue('to') : "zh-CN";
//		if (sl == 'auto'){sl = '';}
//		if (tl == 'auto'){tl = 'en';}

		lang = sl + "|" + tl;
//		currentURL = "http://www.google.com/translate_t?text=" + txtSel + "&langpair=" + lang;
		currentURL = "http://translate.google.com/?text=" + encodeURIComponent(txtSel) + "&langpair=" + lang;
		GM_xmlhttpRequest({
			method: 'GET',
			url: currentURL,
...
//	currentURL = "http://www.google.com/translate_t?text=" + txtSel + "&langpair=" + getId('optSelLangFrom').value + "|" + getId('optSelLangTo').value;
	currentURL = "http://translate.google.com/?text=" + encodeURIComponent(txtSel) + "&langpair=" + getId('optSelLangFrom').value + "|" + getId('optSelLangTo').value;
...
//	getId('divResult').innerHTML = '<a class="gootranslink" href="'+currentURL+'" target="_blank">' + translation + '</a>';
	getId('divResult').innerHTML = translation;
//	if(dict)
//	{
