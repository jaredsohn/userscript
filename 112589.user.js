...
var Version = 'ver 1.1 ESPAÑOL by GOGOL[VillenaFamily]';
var Versiondate = '20.05.2011';
var Scriptname = 'Torre de Vigilancia';
var ScriptTabName = 'Torre de Vigilancia';
...
Tabs.Test = {
  ...
  init : function (div){
    var t = Tabs.Test;
    t.myDiv = div;
    var m = '<TABLE><TR><TD align=right>Exploracion: </td><TD><INPUT type=checkbox id=pcfakeIsScout></td></tr>\
        <TR><TD align=right>Tierras virgenes: </td><TD><INPUT type=checkbox id=pcfakeIsWild></td></tr>\
        <TR><TD align=right>Falso informet: </td><TD><INPUT type=checkbox id=pcfakeFalse></td></tr>\
        <TR><TD align=right>Segundos: </td><TD><INPUT type=text size=4 value=300 id=pcfakeSeconds></td></tr>\
        <TR><TD align=right># de milicianos (soldados simples): </td><TD><INPUT type=text size=6 value=5000 id=pcfakeMilitia></td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=pctestSendMarch type=submit value="Attiva Attacco Falso" \></td></tr></table>\
        <INPUT id=pcReloadKOC type=submit value="Refresca KOC" \>\
        <BR>Fuerzar errores ajax : <INPUT type=checkbox id=pcajaxErr>\
  ...
  createFakeAttack : function (cityNum, isScout, isWild, isFalse, secs, numMilitia){
    ...
    march.players.u1234567.n = 'GOGOL[SPA]';
...
Tabs.Options = {
  ...
  init : function (div){
    ...
    m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pcTabLined cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Posta in chat alleanza automaticamente</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
        <TR><TD align=right>Avisa cuando te exploran: &nbsp; </td><TD><INPUT id=pcalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
        <TR><TD align=right>Avisa sobre un ataque a una tierra virgen: &nbsp; </td><TD><INPUT id=pcalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
        <TR><TD align=right>Minimo numero tropas: &nbsp; </td><TD><INPUT id=pcalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pcalerterr></span></td></tr>\
        <TR><TD align=right>Muestra las tropas en la embajada: &nbsp; </td><TD><INPUT id=pcalertEmbassy type=checkbox '+ (Options.alertConfig.embassy?'CHECKED ':'') +'/></td></tr>\
        <TR><TD align=right>Muestra mis tropas: &nbsp; </td><TD><INPUT id=pcalertMytroops type=checkbox '+ (Options.alertConfig.mytroops?'CHECKED ':'') +'/></td></tr>\
        <TR><TD align=right>Muestra el alimento utilizado: &nbsp; </td><TD><INPUT id=pcalertFood type=checkbox '+ (Options.alertConfig.food?'CHECKED ':'') +' /></td></tr>\
        <TR><TD align=right>Muestra mis defensas: &nbsp; </td><TD><INPUT id=pcalertDefense type=checkbox '+ (Options.alertConfig.defense?'CHECKED ':'') +' /></td></tr>\
        <TR><TD align=right>Mi sede: &nbsp; </td><TD>'+ citySelect +'&nbsp; &nbsp;</td></tr>\
        <TR><TD align=right>Enviar estas noticias a el chat de la alianzaalleanza: &nbsp; </td><TD><INPUT id=pcalertSendToAlly type=checkbox '+ (Options.alertConfig.sendtoAlly?'CHECKED ':'') +' /></td></tr>\
        <TR><TD align=right>Enviarme los ataques como susurro: &nbsp; </td><TD><INPUT id=pcalertSendAsWhisper type=checkbox '+ (Options.alertConfig.sendasWhisper?'CHECKED ':'') +' /></td></tr>\
	    </table></td></tr>\
        </table><BR><BR><HR>Nota si una casilla no se puede activar, quiere decir que Kabam ha disactivado la funcion attivare, vuol dire che Kabam ha disattivato quella funzione</div>';
    t.myDiv.innerHTML = m;
...
var TowerAlerts = {
  ...
  e_eachSecond : function (){   // check for incoming marches
	...
	postToChat: function(m, force){
		...
		var troopsnames = ['Tropas. Suministro', 'Milicianos', 'Explorador', 'Piqueros', 'Espadachines', 'Arquero', 'Cab. Ligera', 'Cab. pesada', 'Carreta de Suministro', 'Ballista', 'Ariete', 'Catapulta'];
		var tot = [];
		...
		if (DEBUG_TRACE) {
			logit("checkTower(): RUMBO A " + unixTime() + ": \n" + inspect(m, 8, 1));
		}	
		...
		msg += 'Mi Ciudad: ' + aCity + '. ';
		msg += 'objetivo: ' + target + '. ';
		msg += 'Atacante: ' + who + ' (' + addCommas(attackermight) + ', ' + diplomacy + ').';
		...
		msg = msg.slice(0, -2);
		msg += ' (llegan en ' + unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) + ' -> ' + arrivingDatetime +').' ;
		...
		if (city.tileId == m.toTileId) {
			...
			if (emb.count > 0) {
				...
				msg += 'Mi embajada (' + availSlots + '/' + emb.maxLevel + ')';
        ...
        if (postOptions.food) {
            ...
            msg += ' ** ALIMENTO **:';
            msg += ' '+addCommas(food)+ ' (' +addCommas(foodIncome)+') -> tiempo restante: '+timeLeftShort+'. ';
        }
		var msgpart5 =msg;
		if (DEBUG_TRACE) {
		...
		if (Options.alertConfig.defense) {
			...
			fortifications['53'] = 'Ballestas';
          	fortifications['55'] = 'Fundibulo';
          	fortifications['60'] = 'Trampas';
          	fortifications['61'] = 'Abrojos';
          	fortifications['62'] = 'Barreras de Picos';
