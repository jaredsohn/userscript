// ==UserScript==
// @name           Better Orkut Manager V 0.1.11 with new Options
// @author             K2 also called as Script Mechanic, Brunolm, Azeem
// @namespace      http://userscripts.org/users/55281
// @description    Better Orkut manager with all new feautures - Hey all please see the discription
// @copyright        K2
// @version           0.1.11
// @include        http://*.orkut.*
// @exclude        http://*.orkut.*/Edit*
// @exclude       http://*.orkut.*/AdsIframe.aspx*
// ==/UserScript==

//////////////
//
script_version = '0.1.11';
userscripts_id = 39720;
//
//////////////
...
function lastPageLinks(page) {
	$('.displaytable a[href^=/Main#CommMsgs.aspx]').each(function(i) {
		var count;
		if(page == 'Community')
...
function forumLinks(page) {
	if(page == 'Communities') {
		$('.displaytable a').each(function() {
			...
			$(this).before('<a style="float:right;font-size:85%" href="/Main#CommTopics.aspx?cmm='+comm_id+'">(forum)</a>');
		});
	}
	else if(page == 'Home') {
		$('tbody:has(.topl_g h2:contains("my communities")) .boxmid .boxgrid .thumbbox').each(function(i) {
			...
			$(this).append('<a style="padding:2 2 2 2;float:right;font-size:85%" href="/Main#CommTopics.aspx?cmm='+comm_id+'">(forum)</a>');
		});
	}
}
...
function qreplyAndQuote(comm_id, thread_id, quote_template) {
	...
	$('#bo-post-link').click(function() {
		var body = encodeURIComponent($('#bo-qreply-txtbox').val()),
			frm = $('form').get(1);
			frm.action = '/CommMsgPost.aspx?cmm=' + comm_id + '&tid=' + thread_id + '&bodyText=' + body + '&Action.submit';
			frm.submit();
	});
...
function myCommunitiesBox(user) {
	...
	$('tbody:has(.topl_g h2:contains("my communities")) .boxmid .boxgrid .thumbbox:lt('+list.length+')').each(function(i) {
		$(this).find('.thumb a').attr('href', '/Main#Community.aspx?cmm='+list[i][0]);
		$(this).find('.thumb img').attr({src:info[list[i][0]]['image'], title:info[list[i][0]]['name']}).css({width:'90%', height:'90%'});
		$(this).find('.uname a').attr('href', '/Main#Community.aspx?cmm='+list[i][0]).text(info[list[i][0]]['name']+' ['+info[list[i][0]]['visits']+']');
	});
}

function settingsEditor(json_form, settings, page_url, user) {
	$('.intabs')
	.append('<li><span class="ltab"></span><a href="javascript:;">Better Orkut Manager</a></li>')
	.find('a:last')
	.click(function() {
		$('.intabs .sel')
		...
		$('#bo-save-button').click(function() {
			...
			if(!values['feed_app_count'].match(/^[0-9]+$/) || values['feed_app_count'] == 0) {
				$('#bo-notif')
				.html('<b>Feed Entry Count should be a non zero positive integer</b>')
				.show().stop().fadeTo(0, 1).fadeOut(5000);
				return;
			}
...
function feedApplication(count) {
	if($('#lbox a[href^=/Main#ViewFeed.aspx]:not(:has(img))').length == 0)
		return;

	$('#mbox .module:eq(1)').after(makeOrkutModule('bo-feed-app'));
	$('#bo-feed-app .topl_g').append('<h2 style="padding-left:20px;background:transparent url(http://img4.orkut.com/img/castro/i_feed.gif) no-repeat scroll center left">his feeds</h2>');
	
	$('#bo-feed-app .boxmidlrg').append('<div id="bo-feed-list"></div>');
	$('#lbox a[href^=/Main#ViewFeed.aspx]:not(:has(img))').each(function() {
		var feed_url = $(this).attr('href').match(/feedurl=(.+)&/)[1];
		$('#bo-feed-list')
		.append('<div style="margin:2px;padding:3px;text-align:center;background-color:#EFF7FF;" class="bo-feedlist-title"><a style="font-weight:bold" href="javascript:;" title="'+$(this).attr('href')+'">'+$(this).attr('title')+'</a></div><div class="bo-feedlist-content"><p style="padding:5px;text-align:center">Loading...</p></div>');
	});

	$('#bo-feed-list .bo-feedlist-content').hide();
	$('#bo-feed-list .bo-feedlist-title a').click(function() {
		var feed_url = $(this).attr('title').match(/feedurl=(.+)&/)[1],
			feed_container = $(this).parent().next();
		if(feed_container.text() == 'Loading...') {
			GM_xmlhttpRequest({
				method:'GET',
				url:'http://pipes.yahoo.com/azeem/8c2fa39dab0896b391fad6cb11e2215e?feed_url='+feed_url+'&show='+count+'&_render=json',
				headers:{'User-Agent':'Mozilla/5.0', 'Accept':'text/xml'},
				onload:function(response) {
					var feed = JSON.parse(response.responseText),
					out_html = '', i;
					out_html += '<ul style="padding-left:20px;list-style:disc">';
					for(i in feed.value.items) {
						out_html += '<li style="margin-top:3px;float:none;"><a style="text-decoration:none" target="_blank" href="'+feed.value.items[i].link+'">'+feed.value.items[i].title+'</a></li>';
					}
					out_html += '</ul>';
					feed_container.html(out_html);
				}
			});
		}
		$(this).parent()
		.next('div.bo-feedlist-content')
		.slideToggle('fast')
		.siblings('div.bo-feedlist-content:visible')
		.slideUp('fast');
	})
	.eq(0).trigger('click');
}
...
function makeScrapBox(user) {
	...
	$('#bo-scrap-button').click(function() {
		var post_token = $('#lowBandwidthForm input[name=POST_TOKEN]').val(),
			...
			url  = 'http://'+window.location.hostname+'/Scrapbook.aspx?uid='+user;
		
		//$(this).replaceWith('Posting Scrap...');
		if($('#bo-captcha').val() != '')
		...
		GM_xmlhttpRequest({
			...
			onload:function(response) {
				...
				if(response.responseText.indexOf('captcha') == 0) {
					captcha_img = response.responseText.match(/(\/CaptchaImage\.aspx\?xid=[0-9]+)/)[1];
					...
					msg = '<b>Success!</b> Your message was successfully sent.';
				else
...
function scrapBox() {
	...
	$('a[href^=/Main#Profile.aspx]:not(:has(img))').hoverIntent(
	{
		interval:500,
		over:function(e) {
			$('#bo-scrap-trigger')
...
function main() {
	var page_url = window.location.href,
		page = page_url.match(/\/([a-zA-z]+)\.aspx/)[1],
		args = getArgs(page_url),
		user = getArgs($('.menu a[href^=/Main#Profile.aspx]').attr('href'), 'uid'),
		settings_form = {
			mycomm_box     : {type:'checkbox', value:false, label:'Enable Better My Communities', no_divi:true},
			clear_stats    : {type:'div', label:'Clear Stats', id:'bo-clear-stats'},
			check_updates  : {type:'checkbox', value:true, label:'Automatically check for script updates'},
			feed_app       : {type:'checkbox', value:true, label:'Show feeds application in user profiles', no_divi:true},
			feed_app_count : {type:'text',     value:'10', label:'Feed entry count'},
			...
			ad_block       : {type:'checkbox', value:true, label:'Block Ads in Home page'}
		},
		settings;
	...
	if(page == 'Profile') {
		if(settings.feed_app)
			feedApplication(settings.feed_app_count);
	}
	if(page == 'Home') {
...
var FreplyCount = 0;
var FreplyHtmlAccept = new Array();
...
var htmlQuoteB = "<div style='background: rgb(200, 225, 255); border: 2px solid LightSkyBlue; color: black; font-size: 90%; margin-left: 20px; margin-right: 20px; padding: 2px 3px 2px 3px'>";
var htmlQuoteE = "</div>";
...
/* VARS:: QuickReply */
FreplyAccept("22674184", "Linkin Park");
/* /VARS:: QuickReply */
...
/// <summary>
/// Add a id to the Freply list
/// </summary>
function FreplyAccept(id, desc)
{
    FreplyHtmlAccept[FreplyCount] = id;
    ++FreplyCount;
}
...
function isQuickReplyHTML()
{
    ...
    if (!x) return false;
    x = x[1];
    return (FreplyHtmlAccept.indexOf(x) != -1 ? true : false);
}
...
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "0", "Comunidade");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249189027809069222", "Mapa da comunidade");

AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "0", "Breath of Fire IV");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "2523060932985547978", "Dúvidas");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249161948040267942", "7 Dragons");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249162416191703206", "Dragons");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249163335314704550", "Best Equipment");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249164108408817830", "Detonado ");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249172724113213606", "Magic Combos");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249173183674714278", "Max Combos");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249173475732490406", "Max Damage");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249174613898823846", "Mestres");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249177890958870694", "Pescaria");
AddBookmarkTopic("13766660", "Breath of Fire Brasil®", "5249180330500294822", "Plains");
...
function BuildLink(count, imgBookmark, textEl, addText, InsertLastLink)
{
    ...
    if (InsertLastLink)
    {
        ...
        LastPage.innerHTML = "&nbsp;<small><last></last></small>";
...
function CreateChatBox()
{
    ...
    var ChatFreply = document.createElement("a");
    ChatFreply.innerHTML = "<small>Freply</small>";
    ChatFreply.addEventListener(
        "click",
        ChatQuickReply
        ,
        false);
    ...
    ChatHeader.appendChild(ChatFreply);
    ChatHeader.appendChild(document.createTextNode(" | "));
