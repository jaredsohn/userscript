// ==UserScript==
// @name          blackjack
// @description   forumwarz automatic blackjack script v1.05, by Neona
...
var minflezz = 10000;		// script will stop playing if flezz would drop below this level
var maxwins = 5;			// how many times you want to win. usually 20, if you're aiming for your daily antifreeze
var betamount = 1;			// how much to bet. usually 10000, again for antifreeze
var debug = false;			// set to true to see debug output in error log
var betmod = false			// you should just leave this false.. really.
...
function takeChoice(text) {
	if(document.getElementById("choices").innerHTML.indexOf(text) == -1) {
	...
	for each (Choice in Choices) {
		if(Choice.innerHTML.indexOf(text) != -1) {
			try {
...
function Finish(chat) {
	...
	if(wins <= maxwins && (flezz - betamount) >= minflezz) {
		if(!betmod) {
			...
			takeChoice("another bet");
			waitForChat(botname, function(text) { Play(text); });
		}
	} else {
		if(debug) GM_log("Lost " + lose + " times.");
		chat.innerHTML = chat.innerHTML + "<br /> Done!<br /><br />Flezz change: " + (flezz - startflezz) + "<br />Win/loss ratio: " + (wins/lose) + "<br />Total rounds: " + (wins+lose);
...
function Scroll()
{
	chat = document.getElementById('chat_history');
