...
function runScript(){
	...
	var stylesheet = "<style type='text/css'>" + 
		...
		(settings.truncateTags ? 
			...
			".s-tag.s-good a, .s-tag.s-useless a, .s-tag.s-terrible a{max-width:140px;}" +
			".s-tag.s-bad a {max-width:120px;}" +
			".s-tag.s-staff a{max-width:64px;}" +
			".s-tag.s-good.s-staff a, .s-tag.s-useless.s-staff a, .s-tag.s-terrible.s-staff a{max-width:104px;}" +
		...
		rows.each(function(i, row){
			...
			if(settings.useTorrentColoring){
				...
				if(color && !row.hasClass("redbar")){
					row.css({"background-color" : "rgba("+color[0]+","+color[1]+","+color[2]+",0.4)"});
				}
			}
		});
	}
