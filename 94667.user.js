// ==UserScript==
// @name           RTM Gmail Gadget Auto-login
// @description    Remember The Milk Gmail Gadget, auto-login based on currently signed in Gmail A/C
// @autor          1nfected
// @version        0.1
// @namespace      1nfected
// @license        CC by-nc-sa http://creativecommons.org/licenses/by-nc-sa/3.0/
// @include        http://www.rememberthemilk.com/services/modules/gmail/*
// @include        https://www.rememberthemilk.com/services/modules/gmail/*
...
function checkiFrame() {
	if(gmail.URL == 'about:blank')  {
		...
		gmail = $('#canvas_frame').contentDocument;
	}
}
...
var RTMG = {
	...
	masterList: null,
	init: function() {
		this.cuser = GM_getValue('currentUser',null);
		if(!this.cuser) { log('No user detected...'); return; }
		
		this.masterList = jParse(GM_getValue('masterList',"{}"));
		
		if($('#tl')) { // Logged in already.
			// Save temporary Login Credentials.
			var temp = jParse(GM_getValue('tempLogin',null));
			if(temp) { log('temp found');
				this.masterList[this.cuser] = {
					id   : temp.id,
					pass : temp.pass
				}
				RTMG.saveList();
				GM_deleteValue('tempLogin');
			}
			// Clear login credentials on Logout.
			$('//td/a[contains(@href,"/auth.rtm?logout=1")]',document,true).addEventListener('click',function(){ RTMG.clearLogin() },false);
			return;
		}
		
		if(!GM_getValue('key')) GM_setValue('key',RIJ.H2BA(RIJ.genkey()).join(','));
		this.key = GM_getValue('key').split(',');
		
		GM_addStyle('.o img{display:block!important;margin:0 auto!important}.f{padding:3px 0 5px!important}.f label[for=username],.f label[for=password]{display:none}.f br{display:none}#username,#password{margin:3px 0!important;padding:0!important;height:22px!important;width:99%!important}#autologin,.f label[for=remember]{margin:3px!important;vertical-align:middle!important}input.s[value=Login]{display:block;margin:8px auto 0!important}div.n{display:none}');
		
		if(this.masterList && this.masterList[this.cuser]) RTMG.login(this.masterList[this.cuser]);
		else RTMG.modLogin();
	},
	saveList: function() { GM_setValue('masterList',jStringify(this.masterList)); },
	login: function(user) {
		$('#username').value = user.id;
		$('#password').value = RIJ.BA2S(RIJ.decrypt(RIJ.H2BA(user.pass),this.key,'ECB'));
	...
	modLogin: function() {
		...
		$('//label[@for="remember"]',document,true).textContent = 'RTMG Auto-Login';
		
		var autologin = document.createElement('input');
		autologin.id = 'autologin';
		autologin.checked = false; // For security reasons, RTMG Auto-login needs to be disabled by default.
		autologin.type = 'checkbox';
		autologin.name = 'autologin';
		remBox.parentNode.insertBefore(autologin,remBox);
		
		$('//input[@type="submit" and @value="Login"]',document,true).addEventListener('click',function(){ RTMG.saveLogin() },false);
	},
	saveLogin: function() {
		if(!$('#autologin').checked) return; // RTMG Auto-login disabled.
		
		// Save login credentials temporarily.
		var temp = {
			id   : $('#username').value,
			pass : RIJ.BA2H(RIJ.encrypt($('#password').value,this.key,'ECB'))
		}
		GM_setValue('tempLogin',jStringify(temp));
	},
	clearLogin: function() {
		if(this.masterList && this.masterList[this.cuser]) {
			delete this.masterList[this.cuser];
			RTMG.saveList();
		}
	}
};
...
if(url.match(/^https?:\/\/mail\.google\.com\/mail/)) {
	var canvas = $('#canvas_frame');
	if(canvas && canvas.contentDocument) var gmail = $('#canvas_frame').contentDocument;
	else return;
	var t = 1;
	var interval = window.setInterval(function() {
		if(t > 30) { log('Unable to get Username.'); window.clearInterval(interval); return; }
		checkiFrame();
		var user = $('//div[@id="guser"]//b',gmail,true);
		if(user) {
			user  = user.textContent;
			window.clearInterval(interval);
			log('Current User: '+user);
			GM_setValue('currentUser',user);
		} else ++t;
	},100);
}
//else { document.addEventListener('load',function() { RTMG.init() },false); }
else { RTMG.init(); }
