...
function bhStats(att, def) {
  ...
  $j('#BHTableL').remove();
  $j('div#myStatBoxL').html('<div id="BHTableL">' + '<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>Citizen</th><th>Kills</th><th>Influence</th></tr>' + att + '</table></div>' + '</div>');
  $j('#BHTableR').remove();
  $j('div#myStatBoxR').html('<div id="BHTableR">' + '<div class="BHTable" style="float:right"><table class="BHTable"><tr><th>Citizen</th><th>Kills</th><th>Influence</th></tr>' + def + '</table></div>' + '</div>');
...
function histStats(hist) {
  ...
  $j('#OOTable').remove();
  $j('div#myOverBox').html('<div id="OOTable">' + '<div class="CHTable" style="float:left"><table class="BHTable"><tr><th>Citizen</th><th>Kills</th><th>Influence</th></tr>' + hist + '</table></div>' + '<div style="clear:both;line-height:0px;height:0px;"></div></div>');
  ...
  $j("body").ajaxSuccess(function (e, res, opt) {
    ...
    if (opt.url.indexOf('/battle-stats/') > -1 && unsafeWindow.SERVER_DATA.onlySpectator != 1) {
      ...
      for (var i = 0; i < history.length; i++) {
        ...
        top5HIST += '<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/' + bh.fightersData[history[i].citizen_id].id + '">' + bh.fightersData[history[i].citizen_id].name + '</a></td><td>' + history[i].kills + '</td><td><strong>' + digits(history[i].damage) + '</strong></td></tr>';
