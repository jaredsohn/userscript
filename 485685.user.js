...
//1.6.0.3x -- Triton added and modified for uni3 (by TimeLord)
...
function fAddShipyardCompletionTimer() {
 ...
 for(Q=0;Q<VAR2.length;Q++){
  if (/ (ship|defense)_template item/.test(VAR2[Q].getAttribute('class'))) {
   ...
   var VAR5=((/extreme\.com/i.test(document.location.href))? 2 : 1)*((/nova/i.test(document.location.href))?1.3333333333:1)*((/tournament|conquest|uni3/i.test(document.location.href))?8/3:1);
...
function GetShipIndex(ShipName) {
 ...
 switch (vNormalShipName) {
  ...
  case 'triton_class_recycler': return 35; break;
  case 'triton_recycler': return 35; break;
  case 'triton_class': return 35; break;       
 }
}
...
function OreCost(ShipIndex) {
 switch (ShipIndex) {
  ...
  case 35: return 30000; break; //Triton       
}} 
function CrysCost(ShipIndex) {
 switch (ShipIndex) {
  ...
  case 35: return 20000; break; //Triton        
}}
