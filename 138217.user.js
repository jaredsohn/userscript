...
// @version        047.0
// @icon           http://s3.picofile.com/file/7433992903/Game_Stats_Icon.png
...
function getPlayerURL() {
	...
	if (probe=='ika-world') {
		return 'http://www.ika-world.com/details/player.html?land='+lang+'&welt='+server+'&id=0&player_name=';
	}
	if (probe=='ikascore') {
...
function getInfos() {
	...
	var matches = /m(\d+)\.([a-z]+)\..*?\.com\//i.exec(url);
	if (matches != null) {
		server = matches[1];
		lang = matches[2];
	} else { 
