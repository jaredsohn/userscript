// Amazon - Arapahoe Lookup 1.2.6
// Sean LeBlanc
// Modified Josh Staiger's (excellent) code
// to adapt to Arapahoe County-Prospector-Worldcat-CD+DVD+VHS Lookup.

// TODO: Add in "other editions" search?
// TODO: General clean-up of unused code.

// Currently, this script will search for the book in the Colorado Arapahoe County library.
// If not found there, it will search on Prospector, and if not there, Worldcat.
// It will also insert a search link for CD, DVDs and VHS.
...
// select "Arapahoe County-Prospector-Worldcat-CD+DVD+VHS Lookup", and click Uninstall.
//
...
//
// ==UserScript==
// @name          Amazon - Arapahoe Lookup 1.2.6
// @namespace     http://sean-leblanc.blogspot.com/
// @description	  While viewing a book on Amazon, this script will search for the book in the Colorado Arapahoe County library. If not found there, it will search on Prospector, then Worldcat. Will also put search links into Arapahoe for CD,DVD & VHS items. Based on Josh Staiger's Wake County Library Lookup.
// @include       http://*.amazon.*
// @include       http://amazon.*
// ==/UserScript==
// @Version       1.2.6
...
// CHANGES
// 1.2.4 - Small fix for Amazon page changes.

// 1.2.6 - Changed for Amazon changes - getTitleLocation() changed.


var dbg = false;
var libraryName = 'Arapahoe County Public Library';
...
function libSearch(isbn, newDiv) {
	var xmlUrlArapCo = "http://65.114.255.136/search/i?SEARCH=" + isbn + "&sortdropdown=-&submit=Submit";
    var successText = "requestblu.gif";

	// Search Arapahoe Co, get redirect info:
	GM_xmlhttpRequest({
     headers: [{'User-Agent': 'dpgm'}],
     method:"GET",
     url:xmlUrlArapCo,
     onload:function(res) {
	  	...
	  	if (successStart >= 0) {
	  		...
	  		getDisplayItemInfoFunction(newDiv, libraryName, xmlUrlArapCo);
	  	}
	  	else {
...
if (isDVD() || isCD() || isVHS()) {
	...
	if (titleLocation) {
		...
		//if (dbg) GM_log("new title: '" + title + "'.");
		//if (title) {
    	// Remove junk from title as best we can:
    	title = parseTitle(title);
    	var url1 = "http://aspen.ald.lib.co.us/search/X?t:(";
    	var url2 = ")&Da=&Db=&m=";
    	var url3 = "&SORT=D";
		var itemType = "";
    	...
    	if (isDVD()) {
    		slog("Is DVD!");
    		url = url1 + title + url2 + "x" + url3;
    		itemType = "DVD";
    	}
		else if (isCD()) {
			slog("Is CD!");
			url = url1 + title + url2 + "d" + url3;
			itemType = "CD";
		}
		else if (isVHS()) {
			slog("Is VHS!");
			url = url1 + title + url2 + "v" + url3;
			itemType = "VHS";
		}
    	insertItemSearchText(itemAvailabilityDiv, "Arapahoe Library", url, itemType);
    }
    else {
