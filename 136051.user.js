...
var c_reg1 = /profile.html?id=(\d*)">/;
var c_reg2 = /\/newspaper\/(.*)-(\d*)\/1" title="/;
var c_reg3 = /"bg-btn-profilemenu">(\d*)<\/span><img/;
var c_reg4 = /goright" href="#">Sub<\/a>/;
var c_reg5 = /display:none" href="#">Unsub<\/a>/;
var c_reg6 = /id="_token" value="(.*)" \/>/;
var c_reg7 = /href="\/en\/newspaper\/(.*)-(\d*)\/1">/;
var c_reg8 = /http:\/\/www.e-sim.org\/\D{2}(.*)/;
var v_speed = 500;
var kullaniciId, gazeteId, v_acsubs, v_progress, v_countM, v_countS, v_subArray, v_errCount, time, v_box, v_sleep=0;
...
function CQmain() {
    ...
    if(BoxCheck() && v_sleep==0){$j(c_htmlinsert).insertAfter('#topArticles')};
    
    $j("div.plate").html().match(c_reg1);
    kullaniciId = RegExp.$1;
    ...
    if(((time-CacheGet('cqmm_time'))/1000)>360000){
        GM_xmlhttpRequest({
          method: "GET",
          url: "http://e-sim.org/profile.html?id="+kullaniciId,
          headers: {
            ...
            onload: function(response) {
                ...
                gazeteId = RegExp.$2;
                if (gazeteId != undefined){
                    GM_xmlhttpRequest({
                      method: "GET",
                      url: "http://www.e-sim.org/newspaper.html?id="+gazeteId,
                      headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
                      },
                        onload: function(response) {
...
function GetData() {
    GM_xmlhttpRequest({
      ...
      data: "eid="+kullaniciId+"&pid="+gazeteId+"&sub="+v_acsubs+"&ver="+c_ver+"&progress="+v_progress,
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      onload: function(response) {
        ...
        CacheSet('cqmm_data', (gazeteId+';'+Resp1[1]+';'+Resp1[2]+';'+Resp1[0]+';'+c_ver));
