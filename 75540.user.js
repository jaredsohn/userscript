// Version 1.1
// ==UserScript==
// @name           Beta Code Filler
// @description    Gets codes
// @include        *expertzone.microsoft.com/*
// ==/UserScript==
...
function fillTextBox(input) {
	...
	else if (match(input,/zip/i) || input.getAttribute("maxlength")

==5) //zip code
		input.value=randomInt(90000)+10000;
	else if (match(input,/code|ima?ge?/i)) {//CAPTCHA
		input.focus(); captcha=true;}
	else if (match(input,/year/i) || input.getAttribute

("maxlength")==4) //year of birth
		input.value=1926+randomInt(60);
	else if (match(input,/day|date/i) || input.getAttribute

("maxlength")==2) //date of birth
...
if (GM_getValue("form filler email")!=undefined && GM_getValue("form 

filler email")!="") {
	var a=document.createElement("a");
	a.setAttribute("href","http://mailinator.com/maildir.jsp?

email="+GM_getValue("form filler email")+"");
	a.appendChild(document.createTextNode("Check your email for 
...
div.appendChild(document.createTextNode("Press CTRL+SHIFT+F"));
