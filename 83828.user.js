...
function farmBuilderElements(a) {trace(a)
    JQ.get("/berichte.php", {
        id: a
    },
    function (d) {
	   ...
	   var b = new Array(d.find(".units:eq(0) tr:eq(1) td:eq(1)").text(), d.find(".units:eq(0) tr:eq(1) td:eq(3)").text()),
        ...
        JQ("#m" + a.split('|')[0]).html('<table id="report_surround">' + d.html() + "</table>");
        JQ("#m" + a.split('|')[0]).append('<br><span class="aller" id="s' + a + '">' + b + "</span>");
        JQ("#m" + a.split('|')[0]).find("a").click(function () {
            JQ(this).parent().find(".add").toggle();
            return false
        });
        JQ("#btn_" + a.split('|')[0]).attr("rel", c);
        JQ("#btn_" + a.split('|')[0]).attr("num", a);
        JQ("#btn_" + a.split('|')[0]).one("click", function () {
            var e = JQ("#t1_" + a).attr("value"),
                f = JQ("#t3_" + a).attr("value"),
                g = JQ("#t6_" + a).attr("value"),
                h = JQ("#t2_" + a).attr("value"),
                j = JQ("#t4_" + a).attr("value"),
                k = JQ("#t5_" + a).attr("value"),
                l = JQ("#t7_" + a).attr("value"),
                m = JQ("#t11_" + a).attr("value");
            data_arr[a] = {};
            data_arr[a].t1 = e;
            data_arr[a].t3 = f;
            data_arr[a].t6 = g;
...
function getFarmForm(a, d, b) {
    ...
    case 2:
        JQ.post("/a2b.php", data_arr[b], function (c) {
            ...
            if (f.t1 * 1 + f.t3 * 1 + f.t6 * 1 + f.t2 * 1 + f.t4 * 1 + f.t5 * 1 + f.t7 * 1 + f.t11 * 1 != data_arr[b].t1 * 1 + data_arr[b].t3 * 1 + data_arr[b].t6 * 1 + data_arr[b].t2 * 1 + data_arr[b].t4 * 1 + data_arr[b].t5 * 1 + data_arr[b].t7 * 1 + data_arr[b].t11 * 1) {
                JQ("#s" + b).parent().parent().prev().find("td:eq(1)").css({
                    background: "#ffc9c9"
                });
                JQ("#s" + b).parent().parent().prev().css({
                    background: "#ffc9c9"
                });
                JQ("#s" + b).html('<b style="color:red">\u043d\u0435\u0434\u043e\u0441\u0442\u0430\u0442\u043e\u0447\u043d\u043e \u0432\u043e\u0439\u0441\u043a! \u0430\u0442\u0430\u043a\u0430 \u043d\u0435 \u043e\u0442\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u0430</b>');
                data_arr[b] = null
                ...
                JQ("#s" + b).html('<b style="color:red">\u043f\u0435\u0440\u0435\u043f\u0440\u044b\u0433\u043d\u0443\u043b\u0438 \u043d\u0430 \u0434\u0440\u0443\u0433\u043e\u0435 \u0441\u0435\u043b\u043e :(</b>');
                data_arr[b] = null
            } else {
                data_arr[b] = null;
                data_arr[b] = f;
    ...
    case 3:
        JQ.post("/a2b.php", data_arr[b], function () {
            JQ("#m" + b).toggle();
            var c = JQ("#s" + b).parent().parent().prev();
            c.find("td:eq(1)").css({
                background: "#c9fffe"
            });
            c.css({
