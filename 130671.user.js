...
for(i=0,j=0;i<moveto.length&&j<kmap.length;i++){
	...
	if(moveto[i].parentNode.id=="attmode"||moveto[i].parentNode.id=="pose"||moveto[i].parentNode.id=="tactic"||moveto[i].innerHTML.indexOf("■")!=-1)continue;
	shortcut(kmap[j],function(e){
		...
		ss.text=$("key_"+(e.altKey?"Alt+":"")+String.fromCharCode(e.keyCode)).parentNode.getAttribute('onchange').replace("this.disabled=true;","");
		document.getElementsByTagName("body")[0].appendChild(ss);
		//eval($("key_"+(e.altKey?"Alt+":"")+String.fromCharCode(e.keyCode)).parentNode.getAttribute('onchange').replace("this.disabled=true;",""));
	});
	j++;
}
...
shortcut("space",function(e){
		...
		ss=document.createElement("script");
		ss.setAttribute("language","javascript");
		ss.text=document.getElementsByTagName("submit")[0].onclick.replace("this.disabled=true;","");
		document.getElementsByTagName("body")[0].appendChild(ss);
});
...
for(i=0;i<=9;i++){
	shortcut(i+"",function(e){
		...
		document.querySelectorAll('.cmdbutton')[(parseInt(String.fromCharCode(e.keyCode))-1)%10].checked=true;
		ss=document.createElement("script");
		ss.setAttribute("language","javascript");
		ss.text=document.querySelectorAll('.cmdbutton')[(parseInt(String.fromCharCode(e.keyCode))-1)%10].getAttribute('onclick').replace("this.disabled=true;","");
		document.getElementsByTagName("body")[0].appendChild(ss);
		//$("submit").click();
	});
}
