// ==UserScript==
// @name		Harvest Detail Time Report Rounder
// @description	Adds a form to the Detailed Time Report that allows toggling between rounded time and actual time.
// @include     https://*.harvestapp.com/reports/detailed/*
// @include     https://*.harvestapp.com/detailed_reports*
// @exclude		https://*.harvestapp.com/detailed_reports/filter
// @require		https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js
// @author		Tim DeMoss
// @version		0.1.3
// ==/UserScript==	

// This script accounts for HH:MM time formatting even though reports do not contain that format.
// If Harvest ever changes their mind and offers that option, we'll be ready!
...
var ButtonHTMLRound = '<div class="customrounder rounderform" style="width:150px;margin-bottom:15px;"><input type="submit" id="roundtimes" value="Round Times"></div>';
...
var ACTUALBUTTONHTML = '<div class="customrounder actualbutton" style="width:150px;margin-bottom:15px;"><input type="submit" id="actualtimes" value="Actual Times"></div>';
...
var NumberTextColor = $(".number").css('color');
var TotalTextColor = $(".total_label + .dt-hours").css('color');
...
$(document).ready(function(){
	...
	if (!$(".rounderform").length) {
		$("#page-top").before(ROUNDINGFORMHTML);
	}
...
function roundTimes() {
	...
	$(".total_label + .dt-hours").text(Total);
	...
	if (!$(".actualbutton").length) {
		$("#page-top").before(ACTUALBUTTONHTML);
	}
}
...
function addTime(Total,Time) {
	...
	if (isDecimal) {
		...
		TotalHours = parseInt(listFirst(Total,":"));
		TotalMinutes = parseInt(listLast(Total,":"));
		TimeHours = parseInt(listFirst(Time,":"));
		TimeMinutes = parseInt(listLast(Time,":"));
...
function roundTime(Time) {
	...
	if (isDecimal) {
		...
		// First, let's get integers for our hours and minutes
		Hours = parseInt(listFirst(Time,":"));
		Minutes = parseInt(listLast(Time,":"));
...
function restoreActualTimes(aTimes) {
	...
	for (var TimerID in aTimes) {
		// Total has class only, not ID
		if (TimerID !== "total_amount") {
			$("#"+TimerID+" > td.number").text(aTimes[TimerID]);
		}
	}
	
	// Update the total
	$(".total_label + .dt-hours").text(aTimes["total_amount"]);
...
function cacheActualTimes() {
	...
	aTimes["total_amount"] = $(".total_label + .dt-hours").text();
	
	// Store the time entries' information
	$(".number:visible").each(function() {
		TimerID = $(this).parent().attr("id");
...
function isDecimalTime() {
	var TimeText = $(".total_label + .dt-hours").text();
	var isDecimal = true;
...
function setTimerColors(style) {
	...
	if (style == 'rounded') {
		...
		$(".total_label + .dt-hours").css('color','red');
	} else if (style == 'actual') {
		// Show original timer colors
		$(".number").css('color',NumberTextColor);
		$(".total_label + .dt-hours").css('color',TotalTextColor);
	}
}
