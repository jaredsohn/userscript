...
/*VERSION*/var Version = '2.0.5';/*VERSION*/
...
(function(){
    try{
        ...
        if(url.indexOf('.ogame.fr') > - 1)
            ...
            srv = 'org';
        else if(url.indexOf('.ogame.com.es') > -1)
            srv = 'es';                        
        ...
        function addToGalaxy(key, where, user, days_ago) {
            try{
                ...
                var texto_dif = '';
                /*if(olderPoints != null) {
                    if(parseInt(olderPoints[0].replace('.', '')) > parseInt(obj.points.replace('.', ''))) {
                        charop = '<span style="color:red">&darr;</span>';
                        texto_dif = LangLabes.lost+' '+(parseInt(olderPoints[0].replace('.', '')) - parseInt(obj.points.replace('.', '')))+ ' '+LangLabes.inthelast+' '+olderPoints[1]+ ' '+LangLabes.days;
                    }
                    if(parseInt(olderPoints[0].replace('.')) < parseInt(obj.points.replace('.', ''))) {
                        charop = '<span style="color:green">&uarr;</span>';
                        texto_dif = LangLabes.won+' '+(parseInt(obj.points.replace('.', '')) - parseInt(olderPoints[0].replace('.', '')))+ ' '+LangLabes.inthelast+' '+olderPoints[1]+ ' '+LangLabes.days;
                    }                  
                }     */     
