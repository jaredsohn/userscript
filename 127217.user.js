// ==UserScript==
// @name          Ceriwis Quick Reply
// @icon          http://code.google.com/p/dev-ceriwis-quick-reply/logo?cct=110309324
// @namespace     http://userscripts.org/scripts/show/80409
// @include       http://www.ceriwis.us/showthread.php?*
// @version       3.2.8
// @dtversion     120203328
...
//   Fix capcat-ceriwis; +minor css.
//   Adapt capcat-ceriwis -red.
...
const OPTIONS_BOX = {
 ...
 ,KEY_SAVE_WIDE_THREAD:  ['1'] // initial state of thread, wider by Ceriwis Fixups - chaox
...
function init(){
  ...
  // initialize gvar..
  gvar.domain= 'http://'+'www.ceriwis.us/';  
  gvar.isNotForum = (!location.href.match(/^http:\/\/w{3}\.ceriwis\.us\/.*/));
  if(gvar.isNotForum) return outSideForumTreat();
  
  gvar.domainstatic= 'http://'+'static.ceriwis.us/';
  gvar.avatarLink= gvar.domainstatic + 'customavatars/';
  gvar.titlename= 'Quick Reply'+(isQR_PLUS!==0?'+':'');
  gvar.fullname= 'Ceriwis '+gvar.titlename;
  gvar.scriptId= '80409';
  ...
  GM_addGlobalStyle('','css_fixups', true); // blank style tag for ceriwis fixups, should be on body instead of head
  GM_addGlobalStyle('','css_qqr', true); // blank style tag for qqr toggle
  
  GM_addGlobalScript('','kaskus_capcay_trigger', true); // blank for trigger ceriwis capcay
...
// outside forum like u.ceriwis.us || imageshack.us
...
function getSettings(){
  ...
  */
  ...
  // pick between capcay google or original ceriwis
...
function getUploaderSetting(){
  ...
  gvar.uploader={
     ceriwis:{
        src:'u.ceriwis.us'
       ,post:'u.ceriwis.us/upload/do_upload'
       ,ifile:'userfile'
       ,hids:{
         referer:'http://'+'u.ceriwis.us'
       }
     }
    ,imgdum:{
  ...
  try{
    ...
    gvar.upload_tipe='ceriwis';
  }catch(e){gvar.upload_tipe='ceriwis';}
}
// end getSettings
...
function create_kaskus_capcat(){
	...
	if( !gvar.soal_cacats ){
		alert('Fail to get soal_cacats, or ceriwis is not using this capcay method');
...
function qr_ajax_post(reply_html){
  if(isUndefined(reply_html)){ // is there ret from XHR :: reply_html
    ...
    var parse_ajax_post = function(html){
        ...
        }else if( cucok = html.match(/TITLE>Ceriwis\sKepenuhan\b[^<]+/i) ){

            r.msg = '"Ceriwis Kepenuhan", post may has been posted. Try to <a href="javascript:location.reload(false);">reload this page</a> and check if it has been posted.';
...
// load and prep paired ceriwis smiley to do quick-quote parsing 
...
function appendAvatar(){
    ...
    if(isDefined(gvar.user.id)){
       ...
       _o('click',$D('#refetch'),function(){
         ...
         el=createEl('a',{href:'http:/'+'/www.ceriwis.us/member.php?u='+gvar.user.id,'class':'cleanlink'},'<b><small>'+gvar.user.name+'</small>'+(gvar.user.isDonatur ? ' <span style="color:red;">[$]</span>':'')+'</b>');
...
var Avatar = {
  ...
  init:function(user){
    Avatar.user = user;
    Avatar.uri='http:/'+'/www.ceriwis.us/member.php?u='+Avatar.user.id;
  },
...
var QRdp = {
 ...
 ,updLast:function(x){ cK.s(QRdp.key, String(x), '/', 'www.ceriwis.us'); QRdp._updLast( String(x) )}
 ...
 ,custom:{
    initCustom: function(){
      ...
      alert( ''
        ...
        +'\n eg.\ncheers|http:/'+'/static.ceriwis.us/images/smilies/sumbangan/smiley_beer.gif'
...
var ST = {
 ...
 ,reset_setting: function(){
    var home=['http:/'+'/www.ceriwis.us/showthread.php?t=3170414','http:/'+'/userscripts.org/topics/58227'];
    var space = '';
...
Format will be valid like this:
 ...
 ':yoyocici|http://foo'
*/
  //var sample = 'lopeh|http://static.ceriwis.us/images/smilies/sumbangan/001.gif,nangis|http://static.ceriwis.us/images/smilies/sumbangan/06.gif';
  gvar.smiliecustom = {};
...
,[H+'iluvkaskuss.gif', ':ilovekaskuss', 'I Love Ceriwis (S)']
...
,[H+s+'kaskuslove.gif', ':ck', 'Ceriwis Lovers']
,[H+'s_sm_ilovekaskus.gif', ':ilovekaskus', 'I Love Ceriwis']
...
,[H+'kaskus_radio.gif', ':kr', 'Ceriwis Radio']
...
,[H+'fd_4.gif', ':kbgt:', 'Ceriwis Banget']
...
,[H+'fd_2.gif', ':cd:', 'Cape deeehh']
 ...
 ,getTPL: function(){
  ...
  +'</tr></table>'
    ...
    */
    ...
    +'<input id="chk_fixups" tabindex="7" type="checkbox" '+(gvar.settings.widethread ? 'checked="checked"':'')+'/><a href="javascript:;"><label title="Wider Thread with Ceriwis Fixups" for="chk_fixups">Expand</label></a>'
    +'</div>'
 ...
 ,getBOT_greet: function(min, max){
    ...
    cK.s('last_greet', String(lastgreet), '/', 'www.ceriwis.us');
    return c[lastgreet];
  }
 ...
 ,getTPL_Settings_About: function(){
    var E={
       'Quick Reply+':{t:6616714,tt:'Add-ons Ceriwis Quick Reply + [QR]',tsl:1323912,ts:'Piluze'}
    ...
    return (''
     ...
     +'<a href="http://code.google.com/p/dev-ceriwis-quick-reply/" target="_blank"><img height="33" src="http://ssl.gstatic.com/codesite/ph/images/defaultlogo.png" border="0" title="dev-ceriwis-quick-reply - '+gvar.fullname+' on Google Code"/></a>&nbsp;'+HtmlUnicodeDecode('&#183;')+'&nbsp;<a href="http://creativecommons.org/licenses/by-nc-sa/3.0" target="_blank"><img src="http://i.creativecommons.org/l/by-nc-sa/3.0/88x31.png" border="0"/></a><br>'
     ...
     +'<div style="height:7px;"></div>'
     +'KASKUS brand is a registered trademark of www.ceriwis.us<br>'
     +gvar.fullname+' (QR) is not related to or endorsed by www.ceriwis.us in any way.<br>'
