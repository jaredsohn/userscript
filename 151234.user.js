...
function main() {
	...
	if (recentImg == undefined) {
		...
		recentImg.push('http://imgsrc.baidu.com/forum/pic/item/0824ab18972bd40775c447547b899e510fb30913.jpg');
		recentImg.push('http://g.hiphotos.baidu.com/album//sign=cdd2fe3dfc039245a5b5e00fb7afd5ff/6a600c338744ebf814506ccfd9f9d72a6059a718.jpg');
	}
	if (usersign.length == 0) {
		usersign[0] = new Object();
		usersign[0].name = '签名范例1';
		usersign[0].imgUrl = 'http://imgsrc.baidu.com/forum/pic/item/9e3df8dcd100baa1c8b4cd164710b912c9fc2e86.jpg';
		usersign[1] = new Object();
		usersign[1].name = '签名范例2';
		usersign[1].imgUrl = 'http://hiphotos.baidu.com/baich7/pic/item/80e426175815391820a4e9ce.jpg';
	}
	if (tailsign.length == 0) {
		tailsign[0] = new Object();
		tailsign[0].name = '女生自拍吧欢迎您';
		tailsign[0].value = '———女生自拍吧欢迎您!卡哇伊吧也欢迎您! o(∩_∩)o';
                tailsign[1] = new Object();
		tailsign[1].name = '来自白痴七的YY之光';
		tailsign[1].value = '———来自白痴七的YY之光 =.=';
		tailsign[2] = new Object();
		tailsign[2].name = '装B尽在掌握';
		tailsign[2].value = '———来自YY之光，装逼大成，优越随心，装逼尽在掌握！';
	}
	if (!getConfig(12))
		setConfig(12, []);
	...
	var line = 'http://e.hiphotos.baidu.com/album/s%3D1400%3Bq%3D90/sign=fe7475f24bfbfbedd859327b48c0cc47/30adcbef76094b360874ff20a3cc7cd98c109d1d.jpg';
	var split = '<br>———————————————————————————<br>';
	$ = unsafeWindow.$;
	setTimeout(function() {
	...
	function displayOptionDialog() {
		...
		var st12 = "<li>关闭模块：&emsp;";
		st12 += "<input type='checkbox' id='st12_1'/>&nbsp;UNICODE&emsp;&emsp;";
		...
		st12 += "<input type='checkbox' id='st12_5'/>&nbsp;YY之光&emsp;&emsp;&emsp;";
		...
		div += st8;
		var st9 = "<li>YY之光 (尾巴名称:尾巴内容 , 换行分隔多个尾巴)<textarea rows=2 style='resize: none;width:100%' id='st9'/> </li>";
		div += st9;
	...
	function flushConfig() {
		...
		if (st12[1])
			$("#st12_1").attr("checked", true);
		if (st12[2])
			$("#st12_2").attr("checked", true);
		if (st12[3])
			$("#st12_3").attr("checked", true);
		if (st12[4])
			$("#st12_4").attr("checked", true);
		if (st12[5])
			$("#st12_5").attr("checked", true);
		if (st12[6])
			$("#st12_6").attr("checked", true);
		if (st12[7])
			$("#st12_7").attr("checked", true);
		if (st12[8])
			$("#st12_8").attr("checked", true);
		if (st12[9])
			$("#st12_9").attr("checked", true);
		if (st12[10])
			$("#st12_10").attr("checked", true);
		if (st12[11])
			$("#st12_11").attr("checked", true);
		if (st12[12])
			$("#st12_12").attr("checked", true);
		if (st12[13])
			$("#st12_13").attr("checked", true);
		if (st12[14])
			$("#st12_14").attr("checked", true);
	}
	...
	function displayPostArea() {
		...
		if (getConfig(12)[7] != 1) {
			...
			musicUrl += '&nbsp;链接 : <input value="http://www.baidu.com/p/%E7%BB%9D%E5%A4%A9%E5%B0%8F%E4%B8%83?from=tieba" style="width:300px" id="musicUrl">';
	...
	function displayUserSignArea() {
		...
		if (getConfig(12)[5] != 1) {
			...
			div2 += "<td></td><td><input type='checkbox' id='doTailSign'/>使用YY之光&emsp;&emsp;&emsp;&emsp;<select id='TailSignValue'>";
			div2 += "<option oid = 0>随机YY之光</option>";
	...
	function submitWithUserSign() {
		...
		if ($("input#doTailSign")[0] && $("input#doTailSign")[0].checked) {
			...
			if (optionIndex2 == 0) {
				var len = $("select#TailSignValue").find("option").length - 1;
				optionIndex2 = Math.floor(Math.random() * len + 1);
			}
            
			var value = "";
			if (optionIndex2 == 1)
				value = "&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;————" + navigator.userAgent;
			else
				value = tailsign[optionIndex2 - 2].value;
            
			var len = 40 - value.length;
			value = value.replace(/\s/gi, '&emsp;');
			for (var i = 0; i < len; i++)
				value = "&emsp;" + value;
