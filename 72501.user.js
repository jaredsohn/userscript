// ==UserScript==
// @name			Ikariam Upgrade Watcher (Fixed 0.3.2)
// @version			1.2.3
...
//
//		v1.0.2 (17. May 2009)
//			- Fix to support different languages (thanks to oliezekat "http://userscripts.org/users/78627").
	...
	if (buildingList) {
		...
		for (var i = 0; i < spot.length; i++) {
			...
			if (isDefined(building, 0) && building[0].className != 'flag') {		
				...
				var redSumWood = 100 - redAll*100 + 100 - redWood*100;
				redSumWood = (100 - redSumWood)/100;
				var redSumMarble = 100 - redAll*100 + 100 - redMarble*100;
				redSumMarble = (100 - redSumMarble)/100;
				var redSumWine = 100 - redAll*100 + 100 - redWine*100;
				redSumWine = (100 - redSumWine)/100;						
				var redSumCrystal = 100 - redAll*100 + 100 - redCrystal*100;
				redSumCrystal = (100 - redSumCrystal)/100;						
				var redSumSulfur = 100 - redAll*100 + 100 - redSulfur*100;
				redSumSulfur = (100 - redSumSulfur)/100;						
				...
				switch (buildingType) {
					case 'palace':						
						...
						if (isDefined(BUILDINGS[buildingType][upLevel]) && BUILDINGS[buildingType][upLevel]["wood"] != 0) {
							...
							reqWood = Math.floor(BUILDINGS[buildingType][upLevel]["wood"] * redSumWood);
							// reqMarble = Math.floor(BUILDINGS[buildingType][upLevel]["marble"] * redAll * redMarble);
							reqMarble = Math.floor(BUILDINGS[buildingType][upLevel]["marble"] * redSumMarble);
							
							// reqWine = Math.floor(BUILDINGS[buildingType][upLevel]["wine"] * redAll * redWine);
							reqWine = Math.floor(BUILDINGS[buildingType][upLevel]["wine"] * redSumWine);
							
							// reqCrystal = Math.floor(BUILDINGS[buildingType][upLevel]["crystal"] * redAll * redCrystal);
							reqCrystal = Math.floor(BUILDINGS[buildingType][upLevel]["crystal"] * redSumCrystal);
							// reqSulfur = Math.floor(BUILDINGS[buildingType][upLevel]["sulfur"] * redAll * redSulfur);
							reqSulfur = Math.floor(BUILDINGS[buildingType][upLevel]["sulfur"] * redSumSulfur);
