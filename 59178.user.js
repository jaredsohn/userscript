// ==UserScript==
// @name            menueweb.hofmann-menue.de
// @namespace       Hofmann
...
function KalorienRechnen() {
   for (var i = 2; i <= 200; i++) {
      var Energie = document.getElementById('grdListe__ctl' + i + '_divBrennwert').innerHTML;
      var Menge = document.getElementById('grdListe__ctl' + i + '_Div1').innerHTML;
      var Preis = document.getElementById('grdListe__ctl' + i + '_Div5').innerHTML;
      var Preis = Preis.replace(/,/g, ".");
      var Anfang = Energie.indexOf("(") + 1;
      var Ende = Energie.indexOf("kcal");
      var Energie = Energie.slice(Anfang, Ende);
      var Strom = Menge * Energie / 100;
      var kcalproEUR = Math.round((Menge * Energie / Preis)) / 100;
      var EURprokcal = Math.round((Preis / Menge * Energie) * 100) / 100 / 100;
	  var i2 = i - 1;
      html_insert_it(window.document,document.evaluate('/HTML[1]/BODY[1]/FORM[1]/DIV[2]/TABLE[1]/TBODY[1]/TR[' + i2 + ']/TD[2]/DIV[1]/DIV[4]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue,'<center><b>' + Strom + ' kcal für ' + Preis + ' EUR bzw. ' + kcalproEUR + ' kcal/EUR</b>',false,true);
   }
};

function TextKuerzen() {
for (var x = 2; x <= 200; x++) {
  do_modify_html_it(window.document,document.evaluate('/HTML[1]/BODY[1]/FORM[1]/DIV[2]/DIV[1]/DIV[4]/TABLE[1]/TBODY[1]/TR[' + x + ']/TD[2]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue,/Betriebsrestaurant: Frühschicht/,'Kantine',null);
}
};
