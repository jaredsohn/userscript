...
$('th.m_buy').append('<small style="cursor:pointer"><acronym title="Κάνε κλικ για να ανοίξουν οι ρυθμίσεις του eRepublik Market">» Ρυθμίσεις</acronym></small>');
$('.price_sorted').parent().before('<div id="tools_settings" style="margin:10px 0;display:none;line-height:20px"><h3>Ρυθμίσεις του eRepublik Market <div id="close_settings" style="float:right;color:red;padding:0 10px;cursor:pointer;background-color:antiqueWhite;">x</div></h3><div style="float:left;width:340px;"><input type="checkbox" id="tools_set0" /> Να φαίνεται η αντιστοιχία τιμή/ζωή στο φαγητό <br /><input type="checkbox" id="tools_set1" /> Να φαίνεται η αντιστοιχία τιμή/ευτυχία στο φαγητό <br /><input type="checkbox" id="tools_set2" /> Να φαίνεται η αντιστοιχία τιμή/μονάδα(ζωή+ευτυχία) στο φαγητό <br /><input type="checkbox" id="tools_set4" /> Να φαίνεται συνολικά πόση ζωή και πόση ευτυχία δίνει το κάθε σπίτι στις ημέρες που θα το έχεις</div><div style="float:left;width:340px;"> Επέλεξε ακρίβεια μέτρησης <select id="tools_set3"><option value="1">0.1</option><option value="2">0.01</option><option value="3">0.001</option><option value="4">0.0001</option><option value="5">0.00001</option></select></div><br /><a class="f_light_blue_big" style="clear:both;margin-left:300px" href="javascript:;"><span>Αποθήκευση</span></a><div style="clear:both;text-align:center"><strong id="tools_msg" style="color:red;cursor:pointer;display:none;">Η σελίδα πρέπει να επαναφορτωθεί (πάτα F5 ή κάνε κλικ <span style="color:blue" onClick="location.reload()">εδώ</span>)</strong></div></div>');
...
$('.price_sorted > tr').each(function(index){
  ...
  if (url[6] == 1) {
    ...
    if (settings.values[0]) {
      ...
      showTxt += '<br><small style="color:green">'+ratio1+' '+currency+'/ζωή</small>';
    }
