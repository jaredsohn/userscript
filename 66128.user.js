// ==UserScript==
// @name           Klanlar Modları
// @version        5.0
// @namespace      Hcakkuzu
// @description    7x7 --> 30x30 a kadar harita görünümü. Klanlar Kaynak Durumu. Klanlar Hızlı Erişim Çubuğu.
// @include        http://tr*.klanlar.org/*
// @include        http://en*.tribalwars.net/*
// @copyright      Copyright (c) 2008, Edit By Hcakkuzu İLERİ
// ==/UserScript==

// ======== 7x7 --> 30x30 a kadar harita görünümü. ========
// ======== Language Dictionary ========
...
function init_game_info(){
	...
	if(TW_Use_Cache === false){
		...
		tw_alert("HCAKKUZU: Önbellek güncellendi!" , info);
	}
}
...
function enhance_report_screen(){
	...
	for(kk=0; kk<tds.length; kk++){
		if(tds[kk].getAttribute("height") == "160"){
			...
			var xhtml = '<table class="vis" width="100%"><tr><th>Saldırı Seçenekleri:</td></tr>';
...
var TWR_dicts = {
  "en" : {"storage" : "Kaynak Durumu(By Hcakkuzu)", "today" : "Bugün:", "tomorrow" : "Yarın:"},
  "de" : {"storage" : "Vorrat", "today" : "heute um", "tomorrow" : "morgen um"},
  "sk" : {"storage" : "Sklad", "today" : "Dnes o", "tomorrow" : "Zajtra o"},
  "cs" : {"storage" : "Skladište", "today" : "Dnes v", "tomorrow" : "Zítra v"}
  }
var TWR_dict;	
...
window.newShortcut = function(imgSrc, aHref, text){
	...
	if(tdIdIndex == 0){
		newTd.setAttribute("style", "border-width:1px; border-style:solid; border-right-style:none; background-color:#f7eed3; padding-left:4px; padding-right:4px; border-spacing:1px;"); 
	}
	else if(tdIdIndex == 10){
		newTd.setAttribute("style", "border-width:1px; border-style:solid; border-left-style:none; background-color:#f7eed3; padding-left:4px; padding-right:4px; border-spacing:1px;"); 
	}
	else{
		newTd.setAttribute("style", "border-width:1px; border-style:solid; border-left-style:none; border-right-style:none; background-color:#f7eed3; padding-left:4px; padding-right:4px; border-spacing:1px;");
	} 
...
window.createQuickbarShortcuts = function() {
	"<tr>   " + 
	newShortcut("http://ro5.triburile.ro/graphic/buildings/main.png", "/game.php?village="+VillId+"&screen=main", "Ana Bina   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/barracks.png", "/game.php?village=+VillId+&screen=barracks", "Kışla   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/stable.png", "/game.php?village="+VillId+"&screen=stable", "Ahır");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/garage.png", "/game.php?village="+VillId+"&screen=garage", "Atölye   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/snob.png", "/game.php?village="+VillId+"&screen=snob", "Saray   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/smith.png", "/game.php?village="+VillId+"&screen=smith", "Demirci   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/place.png", "/game.php?village="+VillId+"&screen=place", "İçtima Meydanı   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/market.png", "/game.php?village="+VillId+"&screen=market", "Market");
	newShortcut("http://tr1.klanlar.org/graphic/face.png?1", "/game.php?village="+VillId+"&screen=ally&mode=members", "Klan Üyeleri   ");
	newShortcut("http://tr1.klanlar.org/graphic/ally_forum.png?1", "/game.php?village="+VillId+"&screen=ally&mode=forum", "Klan Forumu   ");
	newShortcut("http://ro5.triburile.ro/graphic/buildings/main.png", "/game.php?village="+VillId+"&screen=main", "   Phoenix   ");
	tableIdIndex++;
	trIdIndex++;
}