// ==UserScript==
// @name          edit for traaviaan
// @namespace      http://userscripts.org/scripts/show/75431
// @description    Shows travian resources
// @author         Serj_LV
// @license        GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html

// @include        http://w*.t.traaviaan.*
// @include        http://w*.t.traaviaan.*
// @include        http://w*.t.traaviaan*/*
// @include        http://w*.t.traaviaan/*
// @include        http://w*.t.traaviaan.*
// @include        http://w*.t.traaviaan.*
// @exclude     http://w*.t.traaviaan*.*/hilfe.php*
// @exclude     http://w*.t.traaviaan*.*/log*.php*
// @exclude     http://w*.t.traaviaan*.*/index.php*
// @exclude     http://w*.t.traaviaan*.*/anleitung.php*
// @exclude     http://w*.t.traaviaan*.*/impressum.php*
// @exclude     http://w*.t.traaviaan*.*/anmelden.php*
// @exclude     http://w*.t.traaviaan*.*/gutscheine.php*
// @exclude     http://w*.t.traaviaan*.*/spieler.php*
// @exclude     http://w*.t.traaviaan*.*/links.php*
// @exclude     http://w*.t.traaviaan*.*/spielregeln.php*
// @exclude     http://w*.t.traaviaan*.*/tutorial.php*
// @exclude     http://w*.t.traaviaan*.*/manual.php*
// @exclude     http://w*.t.traaviaan*.*/manual.php*
// @exclude     http://w*.t.traaviaan*.*/ajax.php*
// @exclude     http://w*.t.traaviaan*.*/ad/*
// @exclude     http://w*.t.traaviaan.*/chat/*
// @exclude     http://forum.traaviaan*.*
// @exclude     http://board.traaviaan*.*
// @exclude     http://shop.traaviaan*.*
// @exclude     http://w*.t.traaviaan.ir/activate.php*
...
var langs = ['auto','English (en)','???? (ar)','????????? (bg)','Bosanski (bs)','Deutsch (de)','???????? (el)','Español (es)','????? (fa)','Français (fr)','Hrvatski (hr)','Magyar (hu)','Italiano (it)','Nederlands (nl)','Polski (pl)','Português (pt)','Român? (ro)','??????? (ru)','?????? (sr)','Svenska (sv)','Türkçe (tr)','?????????? (ua)','Ti?ng Vi?t (vi)','?? (zh)'];
...
var pageElem = ver4FL ? ( ver42FL ? pageElem42.slice(): pageElem4.slice() ): pageElem3.slice();
...
DICT = {
	...
	hu: { // Hungarian language. thx mrzed :)
		...
		showres : "Nyersanyagkijelz? külön ablakban",
		redbl : "piros (órában)",
		yellowbl: "sárga (órában)",
		marketpl : "Piactér",
		mfilter : "sz?r?",
		mfiltero : ['nincs','csak arány','csak sz?r?','teljes'],
		rpandmp : "Gyülekez?tér és piactér",
		incomres : "Információ az érkez? nyersanyagokról",
 ...
 ar: { // Arabic, thx ww_start_t
	...
	hr: { // Croatian translation, thx semiRocket
		...
		consnegat : "Potrošnja hrane u naselju je negativna. Koliko minuta želite pri?uve?",
		searchlast : "Pretraži za posljednji",
		bmove : "Premještanje gra?evina",
		neighbors : "susjedi",
		// settings
		none : 'Ništa',
		auto : "automatsko",
		...
		cropfind : "Ugra?ena tražilica žitnih (crop) polja",
		adetect : "Detektiranje napada",
		adetecto : ['Isklju?eno','Bez zamrzavanja','Uklju?eno'],
		adetecth : "Radi skrivene zahtjeve prema poslužitelju krše?i pravila. Može dovesti do kazni.",
		...
		notesize : "Veli?ina prozora za zabilješke",
		...
		saveddh : 'Uklju?uje linkove i druge nazive naselja. Korisno ako se ra?un briše ili ne koristi na Vašem ra?unalu.',
		savedelall : "Izbriši sve pohranjene podatke",
		savedelallh : 'Jeste li sigurni da želite izbrisati sve pohranjene podatke, uklju?uju?i linkove i druge nazive naselja?',
	...
	pl: { // Polish translation, thx aren
		...
		builtinh : 'dla normalizacji produkcji: kolor czerwony - najwi?cej ?rodków potrzeba, zielony - potrzeba mniej zasobów',
		...
		dontwraph : 'Je?eli lista osad przeskocz w dól',
		...
		troopsIo : ['off','on','scan help'],
		defRP : "domy?lna akcja dla Miejsca zbiórki",
		showls : "Pokaz linki",
		showlso : ['off','on','w oknie'],
		savedls : "zapisane linki",
		savedd : "zapisane dane",
		saveddh : 'w tym Linki i Druga nazwa. Je?li konto usuni?te, albo nie twój komputer.',
	...
	tr: { // Turkish language. xpugur
		...
		consnegat : "Bu köyde tüketim eksilerde. Kaç dakika gerekiyor rezervlerin dolmas? için?",
		searchlast : "Sonuncuyu aray?n",
		bmove : "Binalar? ta??",
		// settings
		none : "Yok",
		auto : "otomatik",
		...
		yourrace : "Irk?n",
		speedart : "H?z eseri",
		racelist : ['Romanl?lar','Cermenler','Galyal?lar'],
		cranny : "s???nak sar? seviyesi (yüzde)",
		crannyh : 'normal 80, klasik veya eser ya?mas? için 70 ',
		builtin : "Entegre araçlar",
		builtinh : 'Üretimi normalle?tirmek için: k?rm?z? renk - en gerekli kaynak, ye?il - en az gerekli kaynak',
		normalize : 'Üretimi normalle?tirmek için',
		normal : "normal",
		...
		buildhint : "in?aat ipuçlar?",
		onallp : "Tüm sayfalar",
		dontwrap : "Sayfalar? birle?tirme",
		dontwraph : 'köy listeleri sayfa atlarsa',
		buildand : "in?aat gerisay?m?n? ve sald?r?lar? göster",
		buildandh : 'Köy listelerinde',
		sendres : "i?aretleri göster: ?send resource/troops? ",
		sendmess : "i?aretleri göster: ?send message? ",
		...
		opennote : "Otomatik olarak notlar? aç",
		notesize : "Notlar için pencere boyutu",
		openoview : "Otomatik olarak köylerin genel görü?ünü aç",
		resbar : "Kaynaklar çubu?u",
		showres : "Kaynaklar çubu?unu pencerede göster",
		...
		bidinch : 'travian4 aç?k-artt?rmalar? için',
		show3x : "yanl?? veri gösterebilir",
		show3xh : '3x ve 2x ile gönderilen kaynak ak???n? tahmin etmeye çal??',
		...
		troopsIo : ['kapa','aç','yard?m ara'],
		defRP : "git-gel noktas? için standart hareket",
		showls : "Ba?lant?alr? göster",
		...
		saveddh : 'ba?lant?lar ve ikinci ismi içeren. Hesap silindiyse, yada kendi bilgisayar?n?z de?ilse.',
		savedelall : "kaydedilmi? tüm verileri sil",
		savedelallh : 'ikinci isim ve ba?lant?lar? içeren, tüm verileri silmekte emin misiniz?',
		scrlang : "Kod dili",
	...
	vi: { // VietNamese translation by Nguy?n Duy Thanh
		...
		cancel : "Hu? b?",
		close : "?óng",
		overview : "t?ng quan làng",
		svers : "Phiên b?n scripts(translator thanhgola):",
		settings : "Cài ??t",
		notes : "ghi chú",
		res90 : "Tài nguyên c?n ?? ??y %",
		refresh : "làm t??i",
		warehouse : "nhà kho",
		resources : "Tài nguyên",
		troops : "Lính",
		links : "Liên k?t",
		linkname : "tên liên k?t",
		linkdel : "xoá liên k?t",
		name2 : "tên ph?",
		archive : "L?u tr?",
		arena : "Võ ?ài",
		addcur : "thêm liên k?t hi?n t?i",
		del : "xoá",
		edit : "s?a",
		unfix : "g?",
		fix : "s?a",
		...
		consnegat : "M?c tiêu th? âm. C?n bao nhiêu phút ?? d? tr??",
		searchlast : "Tìm ki?m l?n cu?i",
		bmove : "D?i công trình",
		// settings
		none : "Không",
		auto : "t? ??ng",
		info : "Thông tin",
		...
		racelist : ['Romans','Teutons','Gauls'],
		cranny : "Báo màu vàng khi ?ng tài nguyên ??t (%)",
		crannyh : 'thông th??ng là 80, 70 cho làng bình th??ng ho?c làng có Bình r??u c?a Dionysus',
		builtin : "Công c? công trình",
		builtinh : 'Chu?n hoá s?n l??ng tài nguyên: màu ?? -lo?i tài nguyên c?n nh?t, màu xanh - lo?i tài nguyên ít c?n',
		normalize : 'Chu?n hoá s?n l??ng tài nguyên',
		normal : "chu?n",
		banalyze : "Ch?c n?ng - phân tích tr?n chi?n",
		cropfind : "Ch?c n?ng - tìm làng nhi?u ru?ng lúa",
		adetect : "Phát hi?n t?n công",
		adetecto : ['t?t','w/o m?c ??nh','m?'],
		adetecth : "?n các hành vi có th? d?n ??n hình phát",
		adetectt : "Th?i gian phát hi?n t?n công",
		buildhint : "M?o xây d?ng",
		onallp : "T?t c? trang",
		dontwrap : "??ng ng?t trang",
		dontwraph : 'n?u danh sách trang quá dài',
		buildand : "Hi?n th? th?i gian xây d?ng và t?n công",
		buildandh : 'Trong danh sách làng',
		sendres : "Hi?n bi?u t??ng «G?i tài nguyên/G?i lính»",
		sendmess : "Hi?n bi?u t??ng <<g?i>>",
		analyzer : "Phân tích th? gi?i",
		bigicon : "Hi?n bi?u t??ng BIG",
		addvtable : "Hi?n th? danh sách làng b? sung",
		addvtableo : ['t?t','m?','dính'],
		addvtableh : 'Cho travian 4',
		opennote : "T? ??ng m? ghi chú",
		notesize : "Kích th??c c?a s? ghi chú",
		openoview : "T? ??ng m? t?ng quát làng",
		resbar : "Thanh tài nguyên",
		showres : "Hi?n thanh tài nguyên trên c?a s?",
		redbl : "?? (gi?)",
		yellowbl: "Vàng (gi?)",
		marketpl : "Khu ch?",
		mfilter : "B? l?c",
		...
		npcsumh : 'ch? và công trình',
		bidinc : "T?ng ??u giá",
		bidinch : '??u giá c?a travian4',
		show3x : "Có th? hi?n th? d? l?u không chính xác",
		show3xh : 'c? g?n d? ?oán các lu?n tài nguyên sever 2x và 3x ?ang g?i',
		rpandmp : "Binh tr??ng và ch?",
		incomres : "Thông tin tài nguyên ?ang ??n",
		incomreso : ['t?t','m?','v?i tóm t?t','ch? ?? WW'],
		troopsI : "Thông tin v? quân ??i",
		troopsIo : ['t?t','m?','tìm tr? giúp'],
		defRP : "Hành ??ng m?t ??nh khi g?i lính",
		showls : "Hi?n liên k?t",
		showAsSN : "S? d?ng liên k?t nh? tên ph?",
		showlso : ['t?t','m?','trong c?a s?'],
		savedls : "L?u liên k?t",
		savedd : "Liên k?t ?ã l?u",
		saveddh : 'g?m liên k?t và liên k?t ph?. N?u tài kho?ng xoá ho?c ?ây không ph?i máy tính c?a b?n.',
		savedelall : "xoá t?t c? d? li?u ?ã l?u",
		savedelallh : 'b?n có ch?c mu?n xoá h?t d? li?u, bao g?m c? liên k?t và tên ph? cu? nó?',
		...
		audiotest : "Th? nh?c",
		neighbors : "Tìm ki?m",
		colorCustomize : "Cài ??t màu s?c",
		colorHint : "?? tr?ng ?? cho màu m?c ??nh",
		color0 : " S?n sàn nân c?p ",
		color1 : "Nân c?p thông qua NPC",
		color2 : "Không th? nân c?p<br>(do không ?? tài nguyên)",
		color3 : "không th? nân c?p<br>(do nhà kho/ kho lúa không ?? s?c ch?a)",
		color4 : "Hoàn thành nân c?p"
	},
...
function marketSend () {
	...
	function rEL () {
		...
		if( mcFL ) {
			(ver42FL?$gc('merchantsAvailable')[0]:$g('merchantsAvailable')).addEventListener('DOMNodeInserted',reloadMerchants,false);
			mcFL = false;
		}
...
function addARLinks(myVid, aDirect) {
	...
	if( aDirect < 2 ) {
		var ref = $ee('a',trImg(allIDs[33]),[['href',' build.php?id=22 ' +(RB.dictFL[23]==0?'':'&t=5')],['onClick','return false;']]);
...
function addlinkT4() {
	...
	if( vilView ) {
		...
		if( h1 ) {
			...
			newLink = 'd='+xy;
			var newName = (ver42FL? h1.innerHTML.firstText(): spn[1].innerHTML) +' '+ printCoords(xy);
			newName = prompt(gtext("linkname"), newName);
			if( newName == null ) return;
			var newOneLink = newLink + "\/@_" + esc(newName);
			RB.ln3.push(newOneLink);
	...
	if( h1 ) {
		...
		if( ln.length > 0 ) {
			...
			if( ver42FL )
				...
				spanLast.parentNode.insertBefore(nLink,spanLast);
		}
	}
}
...
//	alert(aText.innerHTML);
	if( aText ) eval(aText.innerHTML.replace(/window./,''));
//	alert(ajaxToken);
	return ajaxToken;
}
...
//	getAjaxToken();
	...
	function findAnim (x) {
		...
		if( typeof chkOasisFL[vid] != "undefined" ) {
			...
			ajaxRequest(fullName+'ajax.php?cmd=viewTileDetails', 'POST', 'cmd=viewTileDetails&x='+x[0]+'&y='+x[1]+(ver42FL?'&ajaxToken='+getAjaxToken():''), function(ajaxResp) {
	...
	function cropFindGetMap ( a ) {
		if( ver4FL ) {
			ajaxRequest(fullName+'ajax.php?cmd=mapPositionData', 'POST', 'cmd=mapPositionData&data[x]='+a.rX+'&data[y]='+a.rY+'&data[zoomLevel]=2&'+(ver42FL?'ajaxToken='+getAjaxToken():''), function(ajaxResp) {
				var mapData = eval('(' + ajaxResp.responseText + ')');
				if(ver42FL) mapData = mapData.response;
...
function TM_ShowMainBuildingNumbers(){
	...
	for (var i = 0; i < countArray; i++) {
		...
		if( bldArr[i+mapOffset] ) {
			...
			if( parseInt(BuildingLevel[0])==0 && ver4FL ) smallDIV.innerHTML='0 ';
...
function goldClubInfo () {
	...
	function findAnimA (x) {
		...
		var xy = id2xy(vid);
		ajaxRequest(fullName+'ajax.php?cmd=viewTileDetails', 'POST', 'cmd=viewTileDetails&x='+xy[0]+'&y='+xy[1]+(ver42FL?'&ajaxToken='+getAjaxToken():''), function(ajaxResp) {
			var mapData = eval('(' + ajaxResp.responseText + ')');
			if(ver42FL) mapData = mapData.response;
			var adv = $ee('DIV',mapData.data.html,[['style','display:none;']]);
			ad = $xf('.//table[@id="troop_info"]','f',adv);
