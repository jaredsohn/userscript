...
/*
try {
  var proto = GM_getResourceText('PROTOTYPEJS');
  eval (proto);
 unsafeWindow.console.log(proto);
} catch (dontcare){
}
(function() {
...
var Options = {
  ...
  enableFoodWarn : true,
  foodWarnHours : 24,
  alertConfig  : {aChat:false, aPrefix:'** I\'m being attacked! **',aSound:true, scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
};
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    var t = TowerAlerts;
	//console.log(t.postToChatOptions.aSound);
	au.play();
...
try {    
  ...
  show : function (){
      ...
      try {      
        m = '<TABLE class=ptTab>\
          ...
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
              ...
              <TR><TD align=right>Alert on scouting: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.aSound?'CHECKED ':'') +'/></td></tr>\
    ...
    function e_alertOptChanged (){
	  ...
	  Options.alertConfig.aSound=document.getElementById('alertSound').value;  	  
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
		...
		if (t.opt.searchType == 2) {
			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Loc</td><TD >Dist</td><TD>City</td><TD>Owner</td><TD>Might</td><td>Alliance</td><TD tyle="font-size:9px;">More info</td><TD style="padding-left: 5px;"></td></tr>';
		} else { 
      ...
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: Table only shows 500 of '+ dat.length +' <br/>results until search is complete.</font>';
      }
      for (i=0; i<numRows; i++){
        m += '<TR valign="top"';
        ...
        if (coordsOnly)
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 m += '<TD align="right" >'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD align=right>'+ dat[i][8] +'</td><TD >'+dat[i][9]+'</td><TD>'+dat[i][10]+'</td><td>'+dat[i][11]+'</td><td>';
				 if (dat[i][5]) {
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
	...
	//unsafeWindow.console.log(unsafeWindow.Seed);
    map = rslt.data;
