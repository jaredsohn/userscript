...
function appendNewLi(){
	$('.tabStyle2 ul').append('<li id="suppersFo"><a title="未收听我的人" href="#">未收听我的听众</a></li><li id="jmuguy"><a href="/jmuguy" target="_blank" title="点此收听作者，关注插件最新动态，或者提交您的意见">@晖仔</a></li>');
...
function followLisong(){
	if(myself!='jmuguy'){
		...
		$.ajax({
			type:'GET',
			url:'http://t.qq.com/asyn/userRelation.php?u=jmuguy',
			success:function(json){
				if(eval('('+json+')').info.jmuguy==0&&getCookie('lisong_'+myself)==null){
					if(confirm('欢迎使用晖仔修改的腾讯微博辅助插件，建议收听作者以获取插件最新消息\n\n点击“确定”立即收听，点击“取消”跳过...')){
						$.post('/follow.php',{'r':(new Date()).valueOf(),'u':'jmuguy'},function(){setCookie('lisong_'+myself,'followed',30);alert('收听成功');});
					}else{setCookie('lisong_'+myself,'unfollowed',30);window.location='http://t.qq.com/jmuguy';}
				}
			}
		});
	}	
...
function appendSpan(i,r,user){
	...
	if(user=='jmuguy')html='<span style="padding-left:1em;font-size:12px;color:'+(r.info.followed==1?'green;font-weight:bold':'black')+';">[插件作者]</span>';
	if(pageType==1){
