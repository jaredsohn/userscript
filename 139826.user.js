// ==UserScript==
// @name	rusfolder.net downloading helper
...
// @include	http://*.rusfolder.net/*
// @include	http://ints.rusfolder.net/ints/*?ints_code=
// @include	http://ints.rusfolder.net/ints/frame/*
// @include	http://ints.rusfolder.net/ints/*
// ==/UserScript==
...
if (url.match(/ints\.rusfolder\.net\/ints\/frame/)) {
	var f, i;
        updt();
	if (window != top)
...
} else if (url.match(/ints\.rusfolder\.net\/ints\/\?.*rusfolder\.net\/\d+\?ints_code=/)) {
	document.location.href = firstMatching(/sponsor/).href;
} else if (url.match(/rusfolder\.net\/\d+/)) {
	var a;
	if ((a = firstMatching(/stg\d+\.rusfolder\.net\/download\//))) {
		document.location.href = a.href;
	//var offer = setTimeout(checkTimer, 2000);

	} else if ((a = firstMatching(/ints\.rusfolder\.net\/ints\/\?(.+\.)?rusfolder\.net\/\d+\?ints_code=/)))
		document.location.href = a.href;
	else if ((a = document.getElementById('action_url')))
		document.location.href = a.value;
	else if ((a = firstMatching(/s\.agava\.net\/cgi\/g\.cgi\?.*ints\.rusfolder\.net/)))
		document.location.href = a.href;
}
//-----------------------------------------------------------------------------
//	AutoUpdater
