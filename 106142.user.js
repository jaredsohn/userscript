// ==UserScript==
// @name           Ogame Fast Login (modified)
...
if (sel) { // needed this because this script is sometimes executed on wrong pages
		...
		var Manager = {	
			...
			show: function () {
				...
				if (div) {
					...
					box.setAttribute('style','position:absolute;z-index:10000;padding: 0 10px;background:url(img/inner-content-bg.png) repeat-x #1E2B39;border:1px solid #000;left:50%;width:500px;margin-left:-150px;top:50%;height:400px;margin-top:-200px;');
					box.setAttribute('id', 'ocalcfastlogin');
					var h2 = document.createElement('h2');
					h2.setAttribute('style', 'color:#619FC8;text-shadow:1px 2px #000000');
					h2.innerHTML = 'Управление аккаунтами';
					...
					info.innerHTML = 'Введите данные ваших аккаунтов. У вас может быть <span style="color:red;">три</span> избранных аккаунта, которые вы сможете выбрать прямо с формы.<br><br><b>ИЗБ</b> - добавить/удалить аккаунт из избранных<br><b>↑</b> - переместить аккаунт вверх<br><b>↓</b> - переместить аккаунт вниз<br><b>X</b> - удалить аккаунт';
			...
			save: function () {
				...
				var pass = document.getElementsByName('ocalcfastloginpass');
				...
				for (; i < name.length; i++) {
					...
					var item = name[i].value + "+" + pass[i].value + "+" + sel[i].value + "+" + (favCount++ < 3 && fav[i].checked ? '1' : '') + "+" + abbr;
			...
			initForm: function () {
				...
				form.innerHTML = '<div><span style="width: 144px;display:inline-block;">логин</span><span style="width: 144px;display:inline-block;">пароль</span><span style="width: 122px;display:inline-block;">вселенная</span><span title="ИЗБ: пометить избранным - ↑: передвинуть вверх - ↓: передвинуть вниз - X: удалить">изб/↑/↓/x</span></div>';
				var i = 0;
				var account;
				...
				while(account = GM_getValue('acc' + domain + i)) {
					...
					input2.setAttribute('value', data[1]);
					input2.setAttribute('style', 'width:125px;margin-right:5px;height:18px;');
					input2.setAttribute('name', 'ocalcfastloginpass');
					...
					if (data[3] == '1') check.setAttribute('checked', 'checked');
	...
	var FastLogin = {
		...
		addButton: function() {
			...
			manageButton.appendChild(document.createTextNode('ред.'));
			...
			while (account = GM_getValue('acc' + domain + accCount)) {
				...
				if (data[3] == 1 && favCount++ < 3) {
					...
					fav.setAttribute('onclick', 'window.ocalcfastloginselect("' + data[2] + '", "' + data[0] + '", "' + data[1] + '")');
					fav.innerHTML = data[4] + ": " + data[0];
					
					manageWrap.appendChild(fav);
					var rep = document.createElement('br');
					manageWrap.appendChild(rep);
				}
				accCount++;
			}
			...
			this.selectAccount(favUrl, favAcc, favPass);
		},
		...
		modifySelect: function (data) {
			...
			for (var i = data.length - 1; i >= 0; i--) {
				...
				option.setAttribute('value', data[i][2]);
				option.setAttribute('onclick', 'window.ocalcfastloginselect("", "' + data[i][0] + '", "' + data[i][1] + '")');
				option.setAttribute('onblur', 'alert("test")');
				option.appendChild(document.createTextNode(data[i][4] + ' - ' + data[i][0]));
		...
		selectAccount: function(uni, accountName, accountPassword) {
			if (uni && uni.length > 0) {
				var select = document.getElementById('serverLogin');
				var options = select.getElementsByTagName('option');
			...
			if (accountPassword != '' && accountPassword != undefined) setTimeout("document.getElementById('passwordLogin').value = '" + accountPassword + "'", 1);
			setTimeout("var e=document.getElementById('passwordLogin');e.focus();",1);
		},
