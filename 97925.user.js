// ==UserScript==
// @name           Power Tools - LA ROJA
// @namespace      noobs
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include	   http://corpr8.co.uk/newalert/html
// @description    Version 20110222x del Power Tools POC3 en español por Pulp Fiction
// @require        http://sizzlemctwizzle.com/updater.php?id=89356
// ==/UserScript==

var Version = '20110227';
var Title = 'KOC Power Tools - La Roja';
...
var Options = {
  ...
  lastVersion : null,
  gmtClock : true,
  alertConfig  : {aChat:false, aPrefix:'** ME ATACAN !!!! **', scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
};
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('LA ROJA', eventHideShow, mouseMainTab);
}
...
//    t.show ();
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('RESETEAR', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Tierras: '+ totw +' of '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];

      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Tipo de Tierra</td><TD></td><TD align=left>Coords</td><TD>Trampas</td><TD align=left>Mercenarios</td>\
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Hacer Trampas:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Costo: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("PONER DEFENSAS", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Oro: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
//logit (inspect (rslt, 1, 1));        
    ...
    function displayReports (ar, playerNames, allianceNames, cityNames, totalPages){
      ...
      if (matTypeof(ar) != 'array') {
        ...
        msg.push("<thead><tr><td width=105>Fecha</td><td width=40>Type</td><td width=150>Attacker</td><td>Objetivo</td><td>Ver</td></tr></thead><tbody>");
...
var TowerAlerts = {
  ...
  viewImpending_hook : function (atkinc){    
    ...
    if (!isFalse){
      ...
      d.innerHTML = '<BR><TABLE><TR><TD><a id=towerPostToChat class=button20><span>Enviar al Chat de la Alianza</span></a></td></tr></table>';
      div.appendChild (d);
  ...
  postToChat : function (m, force){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' My embassy has '+ availSlots +' de '+ emb.maxLevel +' slots available.';
      }
    }
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Introducir nombre del jugador: &nbsp;</td>\
        ...
        <TR><TD class=xtab>O: </td><TD class=xtab> Introducir nombre de la alianza: &nbsp;</td>\
		...
		<TR><TD class=xtab>O: </td><TD class=xtab align=right> Listar allianzas: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="Mi Alianza"/>\
            <INPUT align=left id=allListSubmit type=submit value="Listar todos" /></td></tr>\
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Showing players matching <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Poderio</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Detalles </td></tr>';
    var row=0;
    var cl='';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>detalles</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>clasificacion</a></span> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>status online</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>ultimo Login</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerDetail : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "acualizando detalles ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Clasificacion: </b></td><TD colspan=2> Poderio: '+ p.might  +' &nbsp; Allianza: '+ an +'</td></tr>'; 
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Detalles:</b> &nbsp; </td><TD>Allianza: '+ a +' &nbsp; Ciudades: '
  ...
  gotPlayerStatus : function (rslt, span,uid){
    ...
    if (p[uid] == true) {
      m = '<span style="color:green"><b>online!</b></span>';
       ...
       m = '<span style="color:red"><b>puede no estar online</b></span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">Ultimo login: '+lastLogin+'</span>';
    } else {
       m = '<span style="color:red">Fecha de login no encontrada: '+lastLogin+'</span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Allianzas coincidentes <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Nombre de Allianza</td><TD class=xtab>Ranking</td><TD class=xtab>Miembros</td>\
        <TD align=right class=xtab>Poderio</td><TD class=xtab>Diplomacia</td><TD class=xtab></td></tr>';
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 || val > t.MaxPage || val < 1) {
      document.getElementById('allListOut').innerHTML = 'Numero de pagina fuera de rango';
      return;
    }
    t.curPage = val;
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Buscando ...</center>';
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    var m = '<div style="overflow:auto; height:556px;width:564px;"><TABLE><thead><TR style="font-weight:bold"> \
        <th class=xtab>Nombre de Allianza</th><th class=xtab>Ranking</th><th class=xtab>Miembros</th>\
        <th align=right class=xtab>Poderio</th><th class=xtab>Diplomacia</th><th class=xtab></th></tr></thead><tbody>';
    document.getElementById('allListOut').innerHTML = m;
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Ver Miembros</a></td></tr>';
    }
    ...
    m += ' &nbsp; pagina <INPUT align=right id=idPageNum type="text" size=4 value='+ t.curPage + ' /> de '+t.MaxPage;
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      ...
      m += '<TR style="max-height:30px"><TD class='+ tabclass +'>'+ t.dat[i][0] +'</td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
			...
			<td class='+ tabclass +'><SPAN onclick="PCpo(this, \''+t.dat[i][9] +'\')"><A>status online</a></span></td>\
			<td class='+ tabclass +'><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>Ultimo Login</a></span><td> </tr>';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Distancia desde <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' jugadores encontrados &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Jugador</div></a></td>\
        ...
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Ranking</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Nombre de ciudad</a></div></td>\
		...
		<TD class=clickable><A><DIV>Ultimo Login</a></div></td></tr></thead>\
      ...
      <DIV style="top:670px; left:0px; position:static; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Los datos son de la clasificacion y pueden no estar actualizados!</td>\
          <TD class=xtab align=right>Clickar en la cabecera de la columna para ordenar</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Ver distancia desde: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; O, elegir ciudad: <span id=dmcoords></span></td></tr></table>';
...
Tabs.Test = {
  ...
  init : function (div){
	...
	var citySelect = '   <SELECT id=fakeCity>';
	    ...
	    citySelect += '</select>';
    var m = '<TABLE><TR><TD align=right>Explorer: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        <TR><TD align=right>Tierra: </td><TD><INPUT type=checkbox id=fakeIsWild></td></tr>\
        <TR><TD align=right>Informe Falso: </td><TD><INPUT type=checkbox disabled id=fakeFalse></td></tr>\
        <TR><TD align=right>Segundos: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
        <TR><TD align=right># de Suministro: </td><TD><INPUT type=text size=6 value=0 id=fakeSupply></td></tr>\
		<TR><TD align=right># de Miliciano: </td><TD><INPUT type=text size=6 value=5000 id=fakeMilitia></td></tr>\
		<TR><TD align=right># de Explorer: </td><TD><INPUT type=text size=6 value=0 id=fakeScout></td></tr>\
		  <TR><TD align=right># de Pikeros: </td><TD><INPUT type=text size=6 value=0 id=fakePike></td></tr>\
		  <TR><TD align=right># de Espadas: </td><TD><INPUT type=text size=6 value=0 id=fakeSword></td></tr>\
		  <TR><TD align=right># de Arqueros: </td><TD><INPUT type=text size=6 value=0 id=fakeArcher></td></tr>\
		  <TR><TD align=right># de Caballeria: </td><TD><INPUT type=text size=6 value=0 id=fakeCav></td></tr>\
		  <TR><TD align=right># de Pesada: </td><TD><INPUT type=text size=6 value=0 id=fakeHeavy></td></tr>\
		  <TR><TD align=right># de Carretas: </td><TD><INPUT type=text size=6 value=0 id=fakeWagon></td></tr>\
		  <TR><TD align=right># de Ballistas: </td><TD><INPUT type=text size=6 value=0 id=fakeBallista></td></tr>\
		  <TR><TD align=right># de Arietes: </td><TD><INPUT type=text size=6 value=0 id=fakeRam></td></tr>\
		  <TR><TD align=right># de Catapultas: </td><TD><INPUT type=text size=6 value=0 id=fakeCat></td></tr>\
		  <TR><TD align=right>Nombre Falso a usar: </td><TD><INPUT type=text size=13 value=ESTO ES UN TEST id=fakeName></td></tr>\
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>INFORMACION DE UNIDADES</div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>COSTO PARA CONSTRUIR</b></td><TD class=xtabHL colspan=7><B>ESTADISTICAS</b></td><TD class=xtabHL><B>Upkeep</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Comida</td><TD class=xtabH>Madera</td><TD class=xtabH>Piedra</td>\
        <TD class=xtabH>Mineral</td><TD class=xtabH>Pob</td><TD class=xtabHL>Poder</td><TD class=xtabH>Vida</td><TD class=xtabH>Atq</td><TD class=xtabH>Def</td><TD class=xtabH>Vel</td><TD class=xtabH>Rango</td><TD class=xtabH>Carga</td>\
        <TD class=xtabHL>Comida</td></tr>\
    ...
    m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';
    m += '</table>';
    m += '<BR><DIV class=ptstat>CALCULO DE DISTANCIA</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Desde: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> O, elegir ciudad: <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>Hasta: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> O, elegir ciudad: <SPAN id=ptloc2></span></td></tr></table>\
      ...
      <BR><BR><DIV class=ptstat>MAPA DE PROVINCIAS</div><DIV id=ptProvMap><IMG width=680 height=654 SRC="'+ URL_PROVINCE_MAP +'"></div></center></div>';
    t.cont.innerHTML = m;
...
Tabs.Opciones = {
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        <TR><TD colspan=2><B>Configurar:</b></td></tr>\
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Permitir movimiento de ventana (arrastrandola con el raton)</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Ver \'comida restante\' en ROJO si el tiempo restante es inferior a \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> horas</td></tr>\
        <TR><TD colspan=2><P><B>Caracteristicas de KofC:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Ver numero de mensajes e informes separados en el icono de mensajes.</td></tr>';
		if (ENABLE_ALERT_TO_CHAT){
	 m += '<TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Enviar automaticamente alerta de ataques al chat de la allianza.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
          <TR><TD align=right>Mensaje: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
          <TR><TD align=right>Alerta de exploracion: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Alerta de ataque a tierras: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Mostrar allianza del atacante: &nbsp; </td><TD><INPUT id=alertAlliance type=checkbox '+ (Options.alertConfig.includealliance?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Mostrar poderio del atacante: &nbsp; </td><TD><INPUT id=alertMight type=checkbox '+ (Options.alertConfig.includemight?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Sonido de alerta: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.sound?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Minimo de tropas: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
          </table></td></tr>';
		  }
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Permitir Informes de Allianza.</td></tr>\
        <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Permitir a otros scripst cambiar el formato de Informes de la Alianza.</td></tr>\
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Permitir botones de "siguiente" y "todos los oficiales" en los mensajes.</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Permitir navegacion de paginas para mensajes e informes. <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Alarma si se intenta marchar a 0,0. <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Permitir reloj GMT junto a "Hora de Camelot" <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>KofC Correccion de errores:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Arreglar error de la torre para ver objetivo exacto (ciudad, tierra o invalido)</td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Arreglar mapa para ver distancias desde la ciudad elegida, en lugar de siempre al 1ª ciudad.</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Arreglar falsas alarmas de ataque creadas por exploraciones.</td></tr>\
        </table><BR><BR><HR>Notar que si la eleccion esta en gris hay probablemente un cambio en el codigo de KofC haciendo la opcion inoperable.';
      t.cont.innerHTML = m;
...
Tabs.Buscar = {
  ...
  init : function (div){
    ...
    this.cont = div;
    this.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Buscar: </td><TD>\
      <SELECT id="srcType">\
        <OPTION value=0>Barbaros</option>\
        ...
        <tr><td width=100  align=right >Cuadrante: </td><td><select id="idQuadSelect">\
        <option value="188,188"> NorOeste </option>\
        <option value="563,188"> NorEste </option>\
        <option value="188,563"> SurOeste </option>\
        <option value="563,563"> SurEste </option>\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Centro: &nbsp; X: </td><TD class=xtab>\
        ...
        </td></tr><TR><TD class=xtab align=right>Max. Distancia: </td><TD style="width: 200px;" class=xtab><INPUT id=srcDist size=3 value=10 /></td>\
        <td></td>\
        </tr>';
...
clickedPlayerDetail : function (span, uid){
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ officerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Poderio: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
...
//End edit city search
  ...
  srchTypeHandler : function (){
    ...
    var m = '<CENTER><B>Buscar '+ typeName +'<BR>\
        Centro: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distancia: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>LISTAR OPCIONES:</b><BR></td></tr>';
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     m += '<TR><TD class=xtab align=right>Min. nivel:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. nivel:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      ...
      m += '<TR><TD class=xtab align=right>Pradera/Lago<INPUT  id=foodWild type=CHECKBOX '+ (Options.foodWild?' CHECKED':'') +'></td>\
	       <TD class=xtab align=right>Montaña<INPUT id=mtnWild type=CHECKBOX '+ (Options.mtnWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Planicie<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD class=xtab align=right>Colina<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Solo Libres:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Orden:</td><TD class=xtab><SELECT id=filSortBy>\
        ...
        m+= '<TR><TD class=xtab align=right >Ver:</td><TD class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
             ...
             <OPTION value=1>Hostiles</option>\
	     <OPTION value=2>En niebla</option>\
	     <OPTION value=3>Aliadas</option>\
	     <OPTION value=4>Amigables</option>\
	     <OPTION value=5>Neutrales</option>\
	     <OPTION value=6>Sin alinear</option>\
             </select></td></tr>';

		m+= '<TR><TD class=xtab align=right >Orden:</td><TD class=xtab><SELECT style="width: 135px" id=filSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Poderio</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right >Ver solo coordenadas:</td><TD class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
  ...
  clickedSearch : function (){
    ...
    document.getElementById ('srcStart').value = 'Stop';
    ...
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Tipo de Tierra:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Pradera/Lago', 3:'Bosque', 4:'Colina', 5:'Montaña', 6:'Planicie', 0:'TODAS'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Solo Libres:</td>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     ...
     m += htmlOptions ( {1:'Todas', 2:'Aliada', 3:'Amigable', 4:'Hostil', 5:'Neutral', 6:'Sin Alianza', 7:'En Niebla'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Orden:</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Poderio</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
           </select></td></tr>\
		   <TR><TD class=xtab align=right>Min poderio:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>Ver solo coordenadas:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
...
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Barbaros', 'Pradera', 'Lago', 'Bosque', 'Colina', 'Montaña', 'Planicie', 'Ciudad' ];
    var t = Tabs.Buscar;
...
if (DEBUG_TRACE) DebugTimer.start();
    ...
    for (i=0; i<t.mapDat.length; i++){
	  ...
	  if (t.opt.searchType == 2 && type == 7 ) {
			if (!(Options.cityType == 2) || t.mapDat[i][12] == 'aliado')
			if (!(Options.cityType == 3) || t.mapDat[i][12] == 'amigable') 
			if (!(Options.cityType == 4) || t.mapDat[i][12] == 'hostil')
			if (!(Options.cityType == 5) || t.mapDat[i][12] == 'neutral') 
			if (!(Options.cityType == 6) || t.mapDat[i][12] == 'sin alinear') 
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    document.getElementById('statFound').innerHTML = 'Encontrado: '+ dat.length;
    if (dat.length == 0){
      m = '<BR><CENTER>Ninguna encontrada</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Localizacion</td></tr>';
      else {
        if (t.opt.searchType == 0) // Barbs


			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Localizacion</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nivel</td><TD width=80%> &nbsp; Tipo</td><TD style=""></td></tr>';

		else if (t.opt.searchType == 1) // Wilds
			//m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Localizacion</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nivel</td><TD style="padding-left: 10px;> &nbsp; Tipo</td><TD width=80%>Propietario</td><TD style=""></td></tr>';
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Localizacion</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nivel</td><TD width=10%> &nbsp; Tipo</td><TD width=70%> &nbsp; Propietario</td><TD style=""></td></tr>';
		else // Cities
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Localizacion</td><TD >Dist</td><TD>Ciudad</td><TD>Propietario</td><TD>Poderio</td><td>Alianza               </td><TD width=80% style="font-size:9px;">Mas info</td><TD style="padding-left: 10px;"></td></tr>';
			
	  }
      var numRows = dat.length;
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTA: La tabla muestra solo 500 de '+ dat.length +' resultados hasta que la busqueda se completa.</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A style="font-size:9px;" >Detalles</a></div>\
					<DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Clasificacion</a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Online status</a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Ultimo Login</a></div>';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Buscando: '+ t.tilesSearched;
    t.dispMapTable();
...
Tabs.Tropas = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Entrenar tropas y defensas de muro y campo</div><DIV style='height:10px'></div><DIV class=ptentry>\
      <DIV style='text-align:center; margin-bottom:10px;'>Elegir Ciudad: &nbsp; <span id=ptspeedcity></span></div>\
      <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
      <TABLE align=center><TR><TD align=right>Tropas: </td><TD colspan=2>\
      <SELECT id=ptttType>\
        <option value='1'>Suministro</option>\
        <option value='2'>Milicia</option>\
        <option value='3'>Explorer</option>\
        <option value='4'>Piquero</option>\
        <option value='5'>Espada</option>\
        <option value='6'>Arquero</option>\
        <option value='7'>Caballeria</option>\
        <option value='8'>Pesada</option>\
        <option value='9'>Carreta</option>\
        <option value='10'>Ballista</option>\
        <option value='11'>Ariete</option>\
        <option value='12'>Catapulta</option>\
      </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
      <TR><TD align=right># por slot: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
      ...
      <TR><TD align=right valign=top>Poner trabajadores como ociosos: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Permite crear mas tropas. Puede poner la poblacion en negativo temporalmente.</span></td></tr>\
      ...
      <TABLE align=center><TR><TD align=right>Defensas: </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Ballesta</option>\
        <option value='55'>Fundibulo</option>\
        <option value='60'>Trampa</option>\
        <option value='61'>Abrojo</option>\
        <option value='62'>Barrera de picos</option>\
      </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
      <TR><TD align=right># por slot: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
      ...
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Crear Defensas'\></td></tr></table>\
      </td></tr></table></div></div>\
      ...
      <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Cola de Tropas&nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
        <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Cola de Defensas &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
      </div>";
    t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero de tropas por slot debe ser mayor que 0.</font>';
      return;
    }
    ...
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero invalido de slots.</font>';
      return;
    }
    t.setBusy(true);
    var que = [];
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; tienes:'+ t.stats.defOwned;   
    ...
    t.TDspSpace.innerHTML = 'Nivel de Muro: <B>'+ t.stats.wallLevel +'</b><BR>Espacio de Muro: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        Espacio de Campo: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero de unidades por slot debe ser mayor que 0.</font>';
      return;
    }
    ...
    if (numSlots<1 || numSlots > t.stats.wallLevel-t.stats.Dqueued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero de slots invalido.</font>';
      return;
    }
    t.setBusy(true);
    var que = [];
    ...
    try {
      ...
      if (cmd[0] == 'T'){
        ...
        doDefTrain ( cityId, cmd[1], cmd[2], 
          function(errMsg){
            setTimeout(function (){Tabs.Tropas.doDefQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500);
          } );
      }
    } catch (err) {
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B>Suministro:</b></td><TD width=8%><B>Miliciano:</b></td><TD width=8%><B>Explorer:</b></td>\
        <TD width=8%><B>Piquero:</b></td><TD width=8%><B>Espada:</b></td><TD width=8%><B>Arquero:</b></td>\
        <TD width=8%><B>Caballeria:</b></td><TD width=8%><B>Pesada:</b></td><TD width=8%><B>Carreta:</b></td>\
 ...
 '<TD width=8%>'+Seed.units['city'+cityId]['unt11']+'</td><TD width=8%>'+Seed.units['city'+cityId]['unt12']+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><B>Comida:</b></td><TD width=16%><B>Madera:</b></td><TD width=16%><B>Piedra:</b></td>\
        <TD width=16%><B>Mineral:</b></td><TD width=16%><B>Oro:</b></td><TD width=16%><B>Poblacion ociosa:</b></td></tr>\
    ...
    try {
      ...
      if (cmd[0] == 'T'){
        ...
        doTrain (cityId, cmd[1], cmd[2], 
          function(errMsg){
           setTimeout(function (){Tabs.Tropas.doQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500 );
          }
        );
      }
    } catch (err) {
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Activo desde:</span> '+ dt.toLocaleDateString() +'</td>\
...
//**********************************
      ...
      str += _row ('Suministro', rows[1]);      
      str += _row ('Entrenando', t_rows[2]);
      str += _row ('Milicia', rows[2]);
      str += _row ('Entrenando', t_rows[4]);
      str += _row ('Explorer', rows[3]);
      str += _row ('Entrenando', t_rows[6]);
      str += _row ('Piquero', rows[4]);
      str += _row ('Entrenando', t_rows[8]);
      str += _row ('Espada', rows[5]);
      str += _row ('Entrenando', t_rows[10]);
      str += _row ('Arquero', rows[6]);
      str += _row ('Entrenando', t_rows[12]);
      str += _row ('Caballeria', rows[7]);
      str += _row ('Entrenando', t_rows[14]);
      str += _row ('Pesada', rows[8]);
      str += _row ('Entrenando', t_rows[16]);
      str += _row ('Carreta', rows[9]);
      str += _row ('Entrenando', t_rows[18]);
      str += _row ('Ballista', rows[10]);
      str += _row ('Entrenando', t_rows[20]);
      str += _row ('Ariete', rows[11]);
      str += _row ('Entrenando', t_rows[22]);
      str += _row ('Catapulta', rows[12]);
      str += _row ('Entrenando', t_rows[24]);
      ...
      str += _row ('Cola de Tropas', row, true);
      ...
      str += _row ('Cola del Muro', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Incluir Tropas en marcha y Recursos</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
...
//DebugTimer.display ('Draw General');    
    } catch (e){
...
//logit (inspect (enc, 6, 1));    
    ...
    s = '<div class=ptstat>Tropas acampadas en cada embajada.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>No hay tropas acampadas.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Jugador (knight)</b></td>';
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Ver Tropas Originales';
    s += '<BR><BR><DIV style="font-size: 10px">NOTA: Se necesita refrescar KofC para ver nuevas tropas o las restantes despues de una batalla.</div>';
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Ha ocurrido un ERROR</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Automaticamente reintentando en <SPAN id=paretrySeconds></b></span> segundos ...<BR><BR><INPUT id=paretryCancel type=submit value="CANCELAR Reintentar" \>';
