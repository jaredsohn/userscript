// ==UserScript==
// @name           Feuerwache.net Fahrzeugvorschlaege by Tfm94 AAO extern
...
// und erneut veröffentlichst. S.a. Beschreibung unter http://userscripts.org/scripts/show/63127
// ******************************************************************************************
...
var Einsatzklassen = {
  ...
  //   http://userscripts.org/scripts/review/72806
};
...
{ // Fahrzeug bearbeiten
  ...
  H += "  var FN=I.value;\n";
  H += "  if (FN.substr(0,4).toUpperCase()=='X ')\n";
  ...
  { 
    ...
    { var TRs = TBs[i].getElementsByTagName("tr");
      ...
      { var FZName = TRs[j].getElementsByTagName("td")[2].innerHTML;
        ...
        if (FZStat=="Einsatzbereit auf Wache" && Funkname.substr(0,1).toUpperCase()=="X") FZStat = "Außer Dienst";
        switch (FZStat)
...
{ 
  ...
  switch (Position) {
  case "Andere Stadt -                         83 - 180":
    PosBem = "Flughafen Einsatz";
	Einsatzklasse = "FL"
    break;
  case "Andere Stadt -                         84 - 179":
    PosBem = "Flughafen Einsatz";
	Einsatzklasse = "FL"
    break;
  case "Andere Stadt -                         84 - 180":
    PosBem = "Flughafen Einsatz";
	Einsatzklasse = "FL"
    break;
  case "Andere Stadt -                         83 - 179":
    PosBem = "Flughafen Einsatz";
    Einsatzklasse = "FL"
    break;
  case "Flughafen Einsatz":
    PosBem = "Flughafen Einsatz";
	Einsatzklasse = "FL"
    break;	
}    
  ...
  { for each (TD in document.getElementsByTagName("td"))
    { for each (A in TD.getElementsByTagName("a"))
      { if ( A.href.indexOf("http://www.feuerwache.net/feuerwehr-einsaetze/") == 0)
        { TD.innerHTML += "<span style='float:right'><font color='red'>(" + getEinsatzKlasse(A.innerHTML) + ")</font></span>";
        }
      }
    }
  }
...
{ 
  ...
  H += GM_getValue("AAO_Source","http://userscripts.org/scripts/review/72806") + "'> \n";
  H += "<input type='button' value='einlesen' id='AAO_downloadButton'><br>\n";
  H += "<font size='-1'>z.B.: http://userscripts.org/scripts/review/72806</font><br>\n";
...
{ mylog("doDownload");
  ...
  if (!AAOURL) AAOURL="http://userscripts.org/scripts/review/72806";
