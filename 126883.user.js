...
function hover(event) { 
	...
	for (var i=0; i<linkListText[this.id].length; i++) {
		if (linkListText[this.id][i] != "...") {
			...
			{this.innerHTML = this.innerHTML.concat("<a target='_blank' href=http://", linkListURL[this.id][i], ">",linkListText[this.id][i],"/</a>");}
			else if ((i==0) && (boldRoot==1)) 
			{this.innerHTML = this.innerHTML.concat("<em><a target='_blank' href=http://", linkListURL[this.id][i], " style=color:", linkColor, ">",linkListText[this.id][i],"/</a></em>");}
			else 
			{this.innerHTML = this.innerHTML.concat("<a target='_blank' href=http://", linkListURL[this.id][i], " style=color:", linkColor, ">",linkListText[this.id][i],"/</a>");}
	...
	for (var i=0; i<linkList.length; i++) {
		...
		for (var j=0; j<linkListText[linkList[i].id].length; j++) {
			if (linkListText[linkList[i].id][j] != "...") {
				if (linkColor == "default")	
				{linkList[i].innerHTML = linkList[i].innerHTML.concat("<a target='_blank' href=http://", linkListURL[linkList[i].id][j], ">",linkListText[linkList[i].id][j],"/</a>");}
				else if ((j==0) && (boldRoot==1)) 
				{linkList[i].innerHTML = linkList[i].innerHTML.concat("<em><a target='_blank' href=http://", linkListURL[linkList[i].id][j], " style=color:", linkColor, ">",linkListText[linkList[i].id][j],"/</a></em>");}
				else 
				{linkList[i].innerHTML = linkList[i].innerHTML.concat("<a target='_blank' href=http://", linkListURL[linkList[i].id][j], " style=color:", linkColor, ">",linkListText[linkList[i].id][j],"/</a>");}
