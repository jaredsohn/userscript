...
// @description     Lista de Coordenadas Personalizada. Traduccion del Script de LordEvil.
...
function addCoords(event) {
	...
	else coords = prompt('Ingrese coordenadas X:XXX:XX' );
	var desc = prompt('Nombre personalizado.');
	var typ = prompt('Tipo:, 1 - Planeta, 2 -TF, 3 - Luna','1');
...
if( result[2].indexOf('galaxy')!=-1 )
{
	...
	function delAll()
	{
		if (confirm ('Borrar toda la lista de coordenadas personalizada?')) {
			GM_setValue('coordList_'+server,'');
			alert('Todas las coordenadas borradas');
			if (typeof(ramka)!='indefinido') zarzadzaj();
		}
	}
