...
// @version       0.5
// @include       http://ro.grepostats.com/world/ro*/player/*/towns*
// ==/UserScript==
...
allAPlayer=document.evaluate(
	"//h2/a[contains(@href,'/world/ro') and contains(@href,'/player/')]",
	document,
    null,
    XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    null);
...
text=document.createTextNode("Lista de orașe [player]"+player+"[/player]:");
...
allTowns=document.evaluate(
	"//a[contains(@href,'/world/ro') and contains(@href,'/town/')]",
	document,
    null,
    XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    null);
...
if (allTowns.snapshotLength>0) 
{
	...
	textElement=document.createTextNode("Alte formate:");
	tableContent.appendChild(textElement);
	br=document.createElement("br");
	tableContent.appendChild(br);
	br=document.createElement("br");
