...
// @author      PhasmaExMachina  - Updates by Panagiotis Papazoglou -updated rada974
// @website     http://userscripts.org/scripts/show/75000
// @website 
// @version     0.153
//
//
...
// @require   http://userscripts.org/scripts/source/129360.user.js
...
//
// @history		0.153 Fixed images
// @history		0.152 Update for v5.0 Mobile version
// @history		0.151 Update for v0.4.5 (thanks to update of Army Helper)
// @history		0.150 Bugfix of pillage helper
// @history		0.149 Add sums in military view for army and navy in each city and total
// @history		0.148 Buffix with tooltiped names in military overview
// @history		0.147 Visual separation of army anf navy in military overview
// @history		0.146 Remove Triton Engines icon from Merchant Fleet page
// @history		0.145 Add travel time in island view and world view from your current city
// @history		0.144 Bugfix with Trading Post tooltip in resources
// @history		0.143 Bugfix when more than 1 million in resources
// @history		0.142 Update hebrew language
// @history		0.141 Bugfix image wrap when creating new units or ships
// @history		0.140 Add some extra removals in antiplus 
// @history		0.139 Update code for 0.4.4
// @history		0.138 Update code for 0.4.3
// @history		0.137 Expand the translated strings everywhere in the script
// @history		0.136 Bugfix at Military Overview
// @history		0.135 No more updates for now
...
ScriptUpdater.check(129360, "0.153");
  ...
  "About":{
    label:IkaTools.getText('optionsLabels', 'about', "About"),
  	html:'	<p><a href="http://userscripts.org/scripts/show/' + ScriptUpdater.scriptId + '" target="_blank" style="font-weight:bold !important;">Ikariam ExMachina v' + ScriptUpdater.scriptCurrentVersion +
  			'</a> by <a href="http://userscripts.org/users/PhasmaExMachina" target="_blank">PhasmaExMachina</a> - Updates by <a href="http://userscripts.org/users/242290" target="_blank">Panagiotis Papazoglou</a>' +
  			'<p>This is a collection of Ikariam hacks by PhasmaExMachina - Updates by Panagiotis Papazoglou</p>',
  	fields:{
  		debugMode:{
  			type:'checkbox',
...
var ExMachina = {
			...
			function resourceDetails() {
				...
				rNav.forEach(function(item) 
				{
					if (item.className != 'population')
					{
  					...
  					var onHand = parseInt(qty.text().replace('k', '000').replace(/[^\d]/g, ''));
  					var tooltip = $(IkaTools.querySelector('div.tooltip', item));
					var currGood = IkaTools.cityGetTradegoodType(IkaTools.getCurrentCity());
					var numpos = item.className == currGood || item.className == 'wood' ? 1 : 0;
					// get capacity
					var spans = tooltip.text().match(/((\d|,|\.)+k?).*/g);
					var capacity = spans[numpos].replace('k', '000').replace(/[^\d]/g, '');
					var tradepost = spans.length == numpos+2 ? spans[(numpos+1)].replace('k','000').replace(/[^\d]/g,'') : '-';
					var capacityPercent = Math.round(onHand / capacity * 100);
  					...
  					if (unsafe > 0) {
						...
						ttHtml += spans.length == numpos+2 ? '<tr><td>'+IkaTools.getText('interface', 'TradingPost', "Trading Post")+':  </td><td> ' + IkaTools.addCommas(tradepost) + '</td></tr>' : '';
  					}
  					if (change != 0) {
  						// display income
	...
	updateMilitaryOverview:function() 
	{
		...
		var html = '<table cellspacing="0" cellpadding="0" border="0" style="margin:0; width:100%;">\
				<tr valign="middle" style="background-image:url(/skin/input/button.png); height:14px;">\
					<th colspan=2><strong>'+IkaTools.getText('interface', 'City', "City")+'</strong></th>';
		// get movements
      ...
      if (totalUnitsInTraining[unitType] || (totalUnits[unitType] > 0))
			  html += '<th class="' + unitType + '"' + (unitType=="medic" ? ' style="border-right:1px solid #000000;"' : '') + ' title="'+IkaTools.getText('interface', unitType, IkaTools.getUnitName(unitType))+'"> </th>';	
		});
		html += '<th style="font-weight:bold;text-align:center;border-right:1px solid #000000;">&Sigma;</td>';
		IkaTools.shipTypes.forEach(function(shipType)
		{
      if (totalShipsInTraining[shipType] || (totalShips[shipType] > 0))
        html += '<th class="ship_' + shipType + '" title="'+IkaTools.getText('interface', 'ship_'+shipType, IkaTools.getShipName(shipType))+'"> </th>';	
		});
		html += '<th style="font-weight:bold;text-align:center;border-right:1px solid #000000;border-left:1px solid #000000;">&Sigma;</td>';
		...
		cities.forEach(function(city,i)
		{
    		...
    		html += '<tr' + (even ? ' class="even"' : '') + '>\
    			<td style="font-weight:' + (city.id == ExMachina.currentCityId ? 'bold' : 'normal') + ';">' + (i+1) + '</td><td style="width:230px;">';
    		...
    		IkaTools.unitTypes.forEach(function(unitType)
    		{
    			if (totalUnitsInTraining[unitType] || (totalUnits[unitType] > 0)) 
    			{
    				...
    				var qty = (city.owned) ? IkaTools.cityGetUnitQty(unitType, city) : 0;
    				html += '<td align="center"' + (unitType=="medic" ? ' style="border-right:1px solid #000000;"' : '') + '>' + (qty > 0 ? IkaTools.addCommas(qty) : ((inTransit || inTraining.qty > 0) ? '' : '-')) + 
    							(inTransit ? ' <span title="' + IkaTools.getText('interface', unitType, inTransit) + ' '+IkaTools.getText('interface', 'inTransitTo', "in transit towards")+' ' + IkaTools.cityGetSimpleName(city) + '" style="font-size:.8em !important;">(+' + IkaTools.addCommas(inTransit) + ')</span>' : '') +
    							(inTraining.qty > 0 ? ' <span title="' + IkaTools.addCommas(inTraining.qty) + ' '+IkaTools.getText('interface', 'trainingBarracks', "training in barracks")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(inTraining.qty) + ')</span>' : '') +
    						'</td>';
					sumqty += qty == 0 || isNaN(qty) ? 0 : qty;
					suminTraining += inTraining.qty;
					suminTransit += inTransit;
    			}
    		});
			html += '<td align="center" style="border-right:1px solid #000000;font-weight:bold">' + (sumqty == 0 ? ((suminTransit || suminTraining == 0) ? '-' : '') : IkaTools.addCommas(sumqty)) + (suminTransit ? ' <span style="font-size:.8em !important;">(+' + IkaTools.addCommas(suminTransit) + ')</span>' : '') + (suminTraining > 0 ? ' <span title="' + IkaTools.addCommas(suminTraining) + ' '+IkaTools.getText('interface', 'trainingBarracks', "training in barracks")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(suminTraining) + ')</span>' : '') + '</td>';
			var sumqty = 0;
			var suminTraining = 0;
			var suminTransit = 0;
    		IkaTools.shipTypes.forEach(function(shipType)
    		{
    			if (totalShipsInTraining[shipType] || totalShips[shipType] > 0) 
    			{
    				...
    				html += '<td align="center"' + (shipType=="submarine" ? ' style="border-right:1px solid #000000;"' : '') + '>' + (qty > 0 ? IkaTools.addCommas(qty) : ((inTransit || inTraining.qty > 0) ? '' : '-')) + (inTransit ? ' <span title="' + IkaTools.getText('interface', 'ship_'+shipType, inTransit) + ' '+IkaTools.getText('interface', 'inTransitTo', "in transit towards")+' ' + IkaTools.cityGetSimpleName(city) + '" style="font-size:.8em !important;">(+' + IkaTools.addCommas(inTransit) + ')</span>' : '') + (inTraining.qty > 0 ? ' <span title="' + IkaTools.addCommas(inTraining.qty) + ' '+IkaTools.getText('interface', 'trainingShipyard', "training in shipyard")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(inTraining.qty) + ')</span>' : '') +
    						'</td>';
					sumqty += qty == 0 || isNaN(qty) ? 0 : qty;
					suminTraining += inTraining.qty;
					suminTransit += inTransit;
			...
			html += '<td align="center" style="border-right:1px solid #000000;font-weight:bold">' + (sumqty == 0 ? ((suminTransit || suminTraining == 0) ? '-' : '') : IkaTools.addCommas(sumqty)) + (suminTransit ? ' <span style="font-size:.8em !important;">(+' + IkaTools.addCommas(suminTransit) + ')</span>' : '') + (suminTraining > 0 ? ' <span title="' + IkaTools.addCommas(suminTraining) + ' '+IkaTools.getText('interface', 'trainingShipyard', "training in shipyard")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(suminTraining) + ')</span>' : '') + '</td>';
		...
		// draw totals
		var sumtotalUnits = 0;
		var sumtotalUnitsInTraining = 0;
		html += '<tr style="background-color:#F7E4AF;"><td colspan=2> </td>'
		IkaTools.unitTypes.forEach(function(unitType)
		{
			if(totalUnitsInTraining[unitType] || (typeof(totalUnits[unitType]) != 'undefined' && totalUnits[unitType] > 0)) {
				html += '<td style="text-align:center;' + (unitType=="medic" ? 'border-right:1px solid #000000;' : '') + '">' + 
							totalUnits[unitType] +
						...
						'</td>';
				sumtotalUnits += totalUnits[unitType];
				sumtotalUnitsInTraining += totalUnitsInTraining[unitType];
			}
		});
		html += '<td style="text-align:center;border-right:1px solid #000000;font-weight:bold">' + 
							IkaTools.addCommas(sumtotalUnits) +
							(sumtotalUnitsInTraining > 0 ? ' <span title="' + IkaTools.addCommas(sumtotalUnits) + ' + ' + IkaTools.addCommas(sumtotalUnitsInTraining) + ' '+IkaTools.getText('interface', 'training', "training")+' = ' + IkaTools.addCommas(sumtotalUnits + sumtotalUnitsInTraining) + ' '+IkaTools.getText('interface', 'total', "total")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(sumtotalUnitsInTraining) + ')</span>' : '') +  
						'</td>';
		var sumtotalShips = 0;
		var sumtotalShipsInTraining = 0;
		IkaTools.shipTypes.forEach(function(shipType)
		{
			if(totalShipsInTraining[shipType] || (typeof(totalShips[shipType]) != 'undefined' && totalShips[shipType] > 0)) {
				html += '<td style="text-align:center;' + (shipType=="submarine" ? 'border-right:1px solid #000000;' : '') + '">' + 
							totalShips[shipType] +
						...
						'</td>';
				sumtotalShips += totalShips[shipType];
				sumtotalShipsInTraining += totalShipsInTraining[shipType];
			}
		});
		html += '<td style="text-align:center;border-right:1px solid #000000;font-weight:bold">' + 
							IkaTools.addCommas(sumtotalShips) +
							(sumtotalShipsInTraining > 0 ? ' <span title="' + IkaTools.addCommas(sumtotalShips) + ' + ' + IkaTools.addCommas(sumtotalShipsInTraining) + ' '+IkaTools.getText('interface', 'training', "training")+' = ' + IkaTools.addCommas(sumtotalShips + sumtotalShipsInTraining) + ' '+IkaTools.getText('interface', 'total', "total")+'" style="font-size:.8em !important; cursor:help;">(+' + IkaTools.addCommas(sumtotalShipsInTraining) + ')</span>' : '') +  
						'</td>';
		html += '<td></td></tr></table>';
	...
	drawBuildingOverview:function() {
		...
		if(!IkaTools.buildingsOverview) {
			...
			var html = '<tr valign="middle" style="background-image:url(/skin/input/button.png);"><td colspan="2"><div style="width:80px; padding-'+IkaRTL.txtLeft+':1em;"></div></td>';
			...
			cities.forEach(function(city, i)
			{
				...
				html += '<tr style="border-top:1px dotted #9D836A; background-color:' + (i % 2 != 0 ? "#FDF7DD" : "inherit") +
				        '" valign="middle"><td style="border-'+IkaRTL.txtRight+':1px dotted #9D836A;font-weight:'+(ExMachina.currentCityId==city.id ? 'bold' : 'normal')+'">'+(i+1)+'</td><td style="padding:.5em"><a href="http://' + document.domain + '/index.php?view=city&id=' + city.id + '" style="font-weight:bold;">' + city.name + ' (' + city.level + ')</a></td>';
				// buildings
    		numPorts = 0; 
    		numWarehouses = 0; 
				ownedBuildings.forEach(function(buildingType)
	...
	drawEmpireOverview:function() {
		if(!ExMachina.worldOverview) {
			...
			ExMachina.worldOverview.setAttribute('style', 'display:none; background-color:#FEE8C8; border:1px solid #9D836A; text-align:'+IkaRTL.txtLeft+'; width:950px; margin-'+IkaRTL.txtLeft+':-320px; position:relative;'); 
				...
				table.style.width = "100%";
				table.innerHTML = '<tr valign="bottom" style="background-image:url(/skin/input/button.png)"><td colspan="2"></td> <td style="text-align:center; padding:0 .05em;"><img src="http://' + document.domain + '/skin/img/city/constructionSite.png" style="height:12px; margin-top:5px;" title="'+IkaTools.getText('interface', 'UnderConstrution', 'Building under construction')+'"/></td>\
...
/*			ExMachina.buttonWorldView.addEventListener('mouseover', function() {
	...
	appendBuildingList:function(cityId, target, left, top) {
		...
		div.setAttribute('style', 'display:none; background-color:#FEE8C8; border:1px solid #9D836A; text-align:'+IkaRTL.txtLeft+'; width:auto; padding:1em 0 .2em 1em; position:absolute; z-index:1000;'); 
		...
		if(city.buildings) {
			...
			buildings.forEach(function(building)
			{
				if (building.position) {
					var a = document.createElement('a');
					a.setAttribute('style', 'display:block; margin-bottom:.8em; font-size:.9em; cursor:pointer; white-space:nowrap; height:auto; background:none; line-height:1em; padding:0; width:100%;');
	...
	drawMovements:function(movements) {
		...
		movements.forEach(function(movement, i) {
			...
			movementsHtml += '<tr style="' + (i != movements.length - 1 ? 'border-bottom: 1px dotted rgb(157, 131, 106);' : '') + '">'+
				...
				'<td><img src="' + (movement.mission == 'occupy' ? 'skin/interface/mission_occupy.jpg' : 'skin/interface/mission_' + movement.mission + '.png') + '" title="' + movement.description + '"/></td>'+
    ...
    cities.forEach(function(city){
			...
			if(IkaTools.cityGetBuildBuilding(city) && elem) {
				...
				// http://s2.en.ikariam.com/index.php?view=barracks&id=288422&position=5 
				elem.innerHTML = '<a href="/index.php?view=' + building.type + '&id=' + building.cityId + '&position=' + building.position + '" ' +
	...
	getCityListForInfoBox:function(cities) {
		...
		cities.forEach(function(city) {
			...
			html += '<td style="font-size:11px;">' +
						'<a href="http://' + document.domain + '/index.php?view=island&id=' + city.islandId + '&selectCity=' + city.id + '" title="('+convertMsToTimeStr(IkaTools.getTravelTime(IkaTools.cityGetIslandX(IkaTools.getCurrentCityId),IkaTools.cityGetIslandY(IkaTools.getCurrentCityId),city.islandX,city.islandY))+') '+IkaTools.getText('interface', 'ViewCityIsland', 'View ' + city.name + ' on its island')+'">' +
    ...
    {	
      ...
      {
        event.preventDefault();
      	var off = IkaTools.querySelectorAsArray('.offline, .vacation, .inactive', document.querySelector('#'+containerId+' .contentBox01h:nth-child(3)'));
      	var i;
      	var nodo;
...
ExMachina.views.merchantNavy = function(){
	//--------------------------------- Anti Plus --------------------------
	if(Config.get('antiPlus')) { 
		GM_addStyle('.premiumJet { display:none !important; }');
	}
}
...
//			try { target.playerName = $('.owner', this).html().replace(/(<[^d].*?\/>|<[^d].*?<\/[^\d]+?>)/g, '').match(/^\s*([^\s].+)\s*$/)[1]; } catch(e) { alert(e); alert($('.owner', this).html().replace(/(<[^d].*?\/>|<[^d].*?<\/[^\d]+?>)/g, ''));}
			...
			var html = '<div class="pillageHelperInfoWrapper"><p style="margin-top:1.5em;"><strong>'+IkaTools.getText('pillageHelper', 'PillageHelper', 'Pillage Helper')+':</strong><br />\
...
{          
	...
	if(Config.get('antiPlus')) 
	{
		GM_addStyle('#reportInboxLeft + div.dynamic { display:none !important; }');
		var isReports = document.location.toString().match(/tab=reports/);
		if (!isReports)
		{
		  try
		  {
		    isReports = $('#mainview .yui-navset .yui-nav .selected a').attr('href').toString().match(/tab=reports/);
		  }
		  catch (e)
		  {
		  }
...
ExMachina.views.transport = function() {
	//--------------------------------- Anti Plus --------------------------
	if(Config.get('antiPlus')) { 
		$('#setPremiumTransports').hide();
		$('#setPremiumJetPropulsion').hide();
		$('hr:eq(1)').hide();
	}
}
...
ExMachina.views.townHall = function() {
	...
	if(Config.get('antiPlus')) { 
		...
		GM_addStyle('#orderMouseoverTownHall { display:none !important; }');
	}
	...
	function durationHMS(seconds,depth){
	  var temp = {'day':IkaTools.getText('interface','dayshort','d'), 'hour':IkaTools.getText('interface','hourshort','h'), 'minute':IkaTools.getText('interface','minuteshort','m'), 'second':IkaTools.getText('interface','secondshort','s')}, ret = [], prefix = '';
...
Treaty = {
	...
	icons:{
		ct:'<img src="skin/museum/icon32_culturalgood.png" width="16" height="16" align="absmiddle" title="'+IkaTools.getText('interface', 'HaveACulturalTreaty', 'You cave a cultural treaty with this player')+'" style="cursor:help;" />',
		ctPending:'<img src="skin/museum/icon32_culturalgood.png" width="16" height="16" align="absmiddle" style="opacity:.4; cursor:help;" title="'+IkaTools.getText('interface', 'RequestedCulturalTreaty', "You have requested a cultural treaty with this player")+'" />',
	},
...
//					try { var targetId = $(this).html().match(/id=(\d+)/)[1]; } catch(e) { var targetId = false; }
					try { var targetId = this.innerHTML.match(/view=city&id=(\d+)/)[0]; } catch(e) { var targetId = false; }
					if(targetId) {
						// reload spy data
//						var spyId = $('.missionButton a', this).attr('href').match(/spy=(\d+)/)[1];
  					...
  					try { spy.count = Number(trim($('li', this)[1].innerHTML).split(' ')[0]); } catch(e) { spy.count = 1; }
						...
						target.islandX = tmp.match(/(\d+):(\d+)/)[0].split(':')[0];;
						target.islandY = tmp.match(/(\d+):(\d+)/)[0].split(':')[1];;
						target.cityName = $.trim(tmp.match(/.*[^\[(\d+):(\d+)\]]/)[0]);
...
*/
	...
	missionGarrison:function(spyId, callback) {
		...
		if(!hideout) 
			alert("Can't find a hideout in " + IkaTools.getCityById(cityId).name + '!');
		else {
	...
	targetGetResourcesLastUpdatedString:function(target) {
		if(typeof(target.resources) != 'undefined' && typeof(target.resources.lastUpdated) != 'undefined') {
			try {
				...
				return str.match(/^-/) ? '0'+IkaTools.getText('interface','secondshort','s') : str;
...
//						var spyId = $('.missionButton', this).html().match(/spy=(\d+)/)[1];
  					...
  					try { spy.count = Number(trim($('li', this)[1].innerHTML).split(' ')[0]); } catch(e) { spy.count = 1; }
						...
						if(playerName) {
							...
							nameLink.innerHTML += " - " + $.trim(playerName);
						}
						var allianceName = Pillage.targetGetAllianceName(target);
						if(allianceName) {
							$('.city a', this).first().after(" [" + allianceName + "]");
						}
						...
						$('.city', this)[0].innerHTML += '   <a href="http://' + document.domain + '/index.php?view=island&cityId=' + target.id + '" title="'+IkaTools.getText('pillageHelper', 'ViewcityonIsland', "View city on island")+'"><img src="http://' + document.domain + '/skin/layout/icon-island.png" style="vertical-align:middle;"/></a>';
						var div = document.createElement('div');
...
*/								
						var html = '<p align="'+IkaRTL.txtRight+'">\
								...
								<span title="'+IkaTools.getText('pillageHelper', 'CityLevel', "City Level")+'"><img src="http://' + document.domain + '/skin/img/city/building_townhall.png" style="height:25px; margin-'+IkaRTL.txtLeft+':-20px; vertical-align:middle"/> ' + Pillage.targetGetCityLevel(target) + '</span>\
								<span title="'+IkaTools.getText('pillageHelper', 'Wall', "Wall Level")+'"><img src="http://' + document.domain + '/skin/layout/stadtmauer_icon.png" style="height:25px; vertical-align:middle"/> ' + Pillage.targetGetWallLevel(target) + '</span>\
								...
								<span title="'+IkaTools.getText('pillageHelper', 'Port', "Port Level")+'"><img src="http://' + document.domain + '/skin/img/city/building_port.png" style="height:25px; margin-'+IkaRTL.txtLeft+':10px; vertical-align:middle"/> ' + Pillage.targetGetPortLevel(target) + '</span>\
								';
			...
			content.innerHTML = '<a name="autoBuild"></a><h3><a href="http://userscripts.org/scripts/show/80545" target="_blank">Ikariam Auto Build' + 
								...
								'<table cellspacing="0" cellpadding="0">\
									<tbody><tr>\
									  ...
									  '+ IkaTools.getText('interface','to','to') + '<select id="autoBuildOptionMaxDelay">' + delayOptions + '<option value="21600000">6 ' + IAB.getText('options').hours + '</option></select></td>\
									</tr>\
...
var IkaSearch = {
	...
	drawWorldOverview:function(callback) {
		...
		var infoBox = document.createElement('div');
			...
			var html = '<h3 class="header">' + IkaTools.getText('interface','worldsearch','World Search') + '</h3><div class="content" style="padding:.5em 10px;"><table><tr><td style="padding-'+IkaRTL.txtLeft+':.8em;text-align:right">' + IkaTools.getText('interface','Player','Player') + '</td><td>:</td><td><input type="text" style="width:100px;" id="ikaSearchPlayerName"/></td><td style="width:50px"><img src="' + ExMachina.icons.loading + '" style="vertical-align:middle; display:none; margin-top:-2px;" id="playerSearchLoading"/></td></tr><tr><td style="padding-'+IkaRTL.txtLeft+':.8em;text-align:right">' + IkaTools.getText('interface','Alliance','Alliance') + '</td><td>:</td><td><input type="text" style="width:100px;" id="ikaSearchAlliance"/></td><td style="width:50px">&nbsp;</td></tr></table><p style="text-align:center;"><input id="ikaSearchWorldSubmit" value="' + IkaTools.getText('optionsLabels','search','Search') + '" class="button" style="width:75px; display:inline;"/></p></div><div class="footer"/>';
			...
			html += '<tr>';
				for(var x = 1; x <= 100; x++) {
					...
					html += '<td id="ikaSearchIsland_' + id + '" style="background-color:#' + color + ';" ' + 
								(island ? 'title="' + name + ' [' + x + ':' + y + '] ('+convertMsToTimeStr(IkaTools.getTravelTime(IkaTools.cityGetIslandX(IkaTools.getCurrentCityId),IkaTools.cityGetIslandY(IkaTools.getCurrentCityId),x,y))+') - ' + IkaTools.getText('interface', type, type) + ', ' + IkaTools.getText('interface','woodlevel','Wood Level') + ' ' + woodLevel + ' (' + numCities + (island[7] == 1 ? ' ' + IkaTools.getText('interface','citysmall','city') : ' ' + IkaTools.getText('interface','citiessmall','cities')) + ')"' : '') +
	...
	performSearch:function(player, alliance, callback) {
		...
		IkaSearch.showWorld(function() {
			...
			function showCitiesOnMap(cities) {
				...
				if (cities.length == 0) 
					$('#playerCityListSearchResult').html('<div style="text-align:center;"><i>' + IkaTools.getText('interface', 'NoResultsFound', 'No Results Found') + '</i></div>');
...
ExternalScripts.ArmyHelper = function () {
  ...
  const languages = {
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "arabic":       { by: "By", garrison: " ", advisor: "", cityLevel: "  ", support: "",
					  ...
					  ship_rocketship: "Rocket Ship", ship_submarine: "", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "bosnian":      { by: "By", garrison: "Garnizon", advisor: "Savjetnik", cityLevel: "Gradski Level", support: "Podrka", 
                      artillery: "Artiljerija", air: "Zrane jedinice", line1: "Prva borbena linija", line2: "Linija borbe sa distance", 
                      flankLeft: "Lijevi bok", flankRight: "Desni bok", submarine: "Pomorska", reserve: "Rezerva", 
                      phalanx: "Kopljanik", steamgiant: "eljezni div", spearman: "Baca koplja", swordsman: "Maevalac", slinger: "Prakar",
                      archer: "Strijelac", marksman: "Sumforni Muketar", ram: "Ovan", catapult: "Katapult", mortar: "Minobaca", 
					  ...
					  ship_rocketship: "Rocket Ship", ship_submarine: "Podmornica", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
  ...
  //  "chinese":      { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "czech":        { by: "By", garrison: "Posdka", advisor: "Poradce", cityLevel: "rove radnice", support: "Podpora",
                      artillery: "Dlostelectvo", air: "Letectvo", line1: "Prvn linie", line2: "Druh linie",
                      flankLeft: "Lev kdlo", flankRight: "Prav kdlo", submarine: "Ponorka", reserve: "Rezerva",
                      phalanx: "Hoplit", steamgiant: "Parn obr", spearman: "Kopitnk", swordsman: "erm", slinger: "Stelec s prakem",
                      archer: "Lukostelec", marksman: "Stelec", ram: "Beranidlo", catapult: "Katapult", mortar: "Dlo",
                      gyrocopter: "Gyrokoptra", bombardier: "Balnov bombardr", cook: "Kucha", medic: "Doktor",
                      ship_ram: "Beranidlov lo", ship_flamethrower: "Plamenometn lo", ship_steamboat: "Parn beranidlo",
                      ship_ballista: "Balistov lo", ship_catapult: "Katapultov lo", ship_mortar: "Dlov lo",
                      ship_rocketship: "Rocket Ship", ship_submarine: "Ponorka", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Pidat jednotky do jednoho pole", removeSlot: "Odebrat jednotky z jednoho pole", fillRow: "Naplnit jednu linii jednotkami",
                      selectUnits: "Vybrat jednotky", assignNone: "dn", assignAll: "Vechny", assignField: "Naplnit bojit",
                      presetsTitle: "ablony", presetsAdd: "Pidat ablonu", presetsRemove: "Odebrat ablonu", presetsNewName: "Nzev ablony",
                      optShowTitles: "Zobrazit nzvy lini" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  //  "danish":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "dutch":        { by: "By", garrison: "Garnizoen", advisor: "Adviseur", cityLevel: "Stadsgrootte", support: "Ondersteuning", 
					  ...
					  ship_rocketship: "Rocket Ship", ship_submarine: "Onderzeer", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "english":      { by: "By", garrison: "Garrison", advisor: "Advisor", cityLevel: "Town level", support: "Support", 
                      ...
                      ship_flamethrower: "Fire Ship", ship_steamboat: "Paddle-Wheel-Ram", ship_ram: "Ram-Ship",
                      ship_catapult: "Catapult Ship", ship_ballista: "Ballista Ship", ship_mortar: "Mortar Ship", 
                      ship_rocketship: "Rocket Ship", ship_submarine: "Diving Boat", ship_paddlespeedship: "Paddle Speedboat",
					  ship_ballooncarrier: "Balloon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
                      optShowTitles: "Show battlefield row titles" },
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "french":       { by: "By", garrison: "Garnison", advisor: "Conseiller", cityLevel: "Niveau ville ", support: "Support", 
                      artillery: "Artillerie", air: "Unit arienne", line1: "Ligne de front ", line2: "Ligne de bataille de longue porte", 
                      flankLeft: "Flanc gauche", flankRight: "Flanc droit", submarine: "Submarine", reserve: "Rserve", 
                      phalanx: "Phalange", steamgiant: "Gant  vapeur", spearman: "Lancier", swordsman: "piste", slinger: "Lance pierre",
                      archer: "Archer", marksman: "Tireur d'lite", ram: "Blier", catapult: "Catapulte", mortar: "Mortier", 
                      gyrocopter: "Gyrocoptre", bombardier: "Bombardier", cook: "Cuisiner", medic: "Mdecin",
                      ...
                      ship_rocketship: "Rocket Ship", ship_submarine: "Diving Boat", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
     "german":       { by: "Von", garrison: "Garnison", advisor: "Berater", cityLevel: "Stadtgre", support: "Untersttzung",
                      ...
                      phalanx: "Hoplit", steamgiant: "Dampfgigant", spearman: "Speertrger", swordsman: "Schwertkmpfer", slinger: "Steinschleuderer",
                      archer: "Bogenschtze", marksman: "Schwefelbchsen-Schtze", ram: "Ramme", catapult: "Katapult", mortar: "Mrser",
                      ...
                      ship_ballista: "Ballistaschiff", ship_catapult: "Katapultschiff", ship_mortar: "Mrserschiff",
					  ship_rocketship: "Rocket Ship", ship_submarine: "Tauchboot", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "greek":        { by: " ", garrison: "", advisor: "", cityLevel: " ", support: " ",
                      artillery: " ", air: " ", line1: "  ", line2: "   ", sea: " ",
                      flankLeft: " ", flankRight: " ", submarine: " ", reserve: "",
                      phalanx: "", steamgiant: " ", spearman: " ", swordsman: "", slinger: "",
                      archer: "", marksman: " ", ram: "", catapult: "", mortar: "",
                      gyrocopter: "", bombardier: "", cook: "", medic: "",
                      ship_flamethrower: "(-)", ship_steamboat: "(-) --", ship_ram: "(-) ", 
                      ship_catapult: "(-) ", ship_ballista: "(-) ", ship_mortar: "(-) ", 
					  ship_rocketship: "", ship_submarine: " ", ship_paddlespeedboat: " ",
					  ship_ballooncarrier: "", ship_tender: " ",
                      addSlot: "    ", removeSlot: "    ", fillRow: "    ",
                      selectUnits: " ", assignNone: "", assignAll: "", assignField: "Fill battlefield",
                      presetsTitle: "", presetsAdd: "  ", presetsRemove: " ", presetsNewName: "      ",
                      optShowTitles: "    " },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  //  "hebrew":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "hungarian":    { by: "By", garrison: "Helyrsg", advisor: "Tancsad", cityLevel: "Vros mret", support: "Tmogats",
                      artillery: "Tzrsgi", air: "Lgi", line1: "Frontvonalban", line2: "Lvsz vonalban",
                      flankLeft: "Bal szrnyon", flankRight: "Jobb szrnyon", submarine: "Bvrhaj", reserve: "Foglalt",
                      phalanx: "Hoplita", steamgiant: "Gzris", spearman: "Lndzss", swordsman: "Kardos", slinger: "Parittys",
                      archer: "jsz", marksman: "Lvsz", ram: "Faltrkos", catapult: "Katapult", mortar: "gy",
                      gyrocopter: "Gyrocopter", bombardier: "Ballonos bombz", cook: "Sf", medic: "Felcser",
                      ship_ram: "Trhaj", ship_flamethrower: "Lnghaj", ship_steamboat: "Evezkerk haj",
                      ship_ballista: "Balliszta", ship_catapult: "Katapult haj", ship_mortar: "gyhaj",
                      ship_rocketship: "Rocket Ship", ship_submarine: "Bvrhaj", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "italian":      { by: "By", garrison: "Guarnigione", advisor: "Consulente", cityLevel: "Livello Municipio", support: "Supporto", 
					  ...
					  ship_rocketship: "Rocket Ship", ship_submarine: "Sottomarino", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
  ...
  //  "korean":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "latvian":      { by: "By", garrison: "Garnizons", advisor: "Konsultants", cityLevel: "Pilstas lmenis", support: "Paldzba",
                      artillery: "Artilrija", air: "Gaisa spki", line1: "Priekj lnija", line2: "Tls kaujas lnija",
                      flankLeft: "Kreisais flangs", flankRight: "Labais flangs", submarine: "Zemdene", reserve: "Rezerv",
                      phalanx: "pnesis", steamgiant: "Tvaika milzis", spearman: "Pnesis", swordsman: "Paukotjs", slinger: "Metjs",
                      archer: "Lokvjs", marksman: "vjs", ram: "Tarns", catapult: "Katapulta", mortar: "Mnmetjs",
                      gyrocopter: "Helikopters", bombardier: "Balons bombardieris", cook: "Pavrs", medic: "rsts",
                      ship_ram: "Tarna kuis", ship_flamethrower: "Ugunsmetjs", ship_steamboat: "Dzenrata kuis",
                      ship_ballista: "Ballistiskais kuis", ship_catapult: "Katapultas kuis", ship_mortar: "Mnmetja kuis",
					  ship_rocketship: "Rocket Ship", ship_submarine: "Zemdene", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Pievienot vienbas uz vienu lauciu", removeSlot: "Noemt vienbas no viena laucia", fillRow: "Aizpildt vienu kaujas rindu ar vienbm",
                      selectUnits: "Izvlies vienbas", assignNone: "Nevienu", assignAll: "Stt visu", assignField: "Aizpildt kaujas lauku",
                      presetsTitle: "Iepriekplnots sastvs", presetsAdd: "Pievienot jaunu sastvu", presetsRemove: "Dzst sastvu", presetsNewName: "Nordiet nosaukumu kaujas sastvam lai to saglabtu",
                      optShowTitles: "Rdt kaujas rindu virsrakstus" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "lithuanian":   { by: "By", garrison: "gula", advisor: "Patarjas", cityLevel: "Miesto lygis", support: "Palaikymas",
                      artillery: "Artilerija", air: "Oro gynyba", line1: "Priekin mio linija", line2: "Ilgo-diapazono kariai",
                      flankLeft: "Kairysis flangas", flankRight: "Deinysis flangas", submarine: "Povandeninis laivas", reserve: "Rezervas",
					  ...
					  ship_rocketship: "Rocket Ship", ship_submarine: "Krovininis laivas", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Upildyti laukelius", removeSlot: "Paalinti karinius vienetus i laukeli", fillRow: "Upildyti vien eilut kariniais vienetais",
                      selectUnits: "Pasirinkti vienetus", assignNone: "N vieno", assignAll: "Visi", assignField: "Upildyti mio laukus",
                      presetsTitle: "Iankstinis nustatymas", presetsAdd: "vesti nauj iankstin nustatym", presetsRemove: "Itrinti iankstin nustatym", presetsNewName: "Nurodykite iankstinio nustatymo pavadinim isaugojimui",
                      optShowTitles: "Rodyti mio eilutje pavadinimus" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  //  "pinoy":        { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "polish":       { by: "By", garrison: "Garnizon", advisor: "Pole bitwy", cityLevel: "Poziom ratusza", support: "Wsparcie",
                      artillery: "Artyleria", air: "Lotnictwo", line1: "Linia frontu", line2: "Linia walki na odlego",
                      flankLeft: "Lewa flanka", flankRight: "Prawa flanka", submarine: "Okrty podwodne", reserve: "Rezerwa",
                      phalanx: "Hoplita", steamgiant: "Gigant", spearman: "Wcznik", swordsman: "Wojownik", slinger: "Procarz",
                      archer: "ucznik", marksman: "Strzelec", ram: "Taran", catapult: "Katapulta", mortar: "Modzierz",
                      gyrocopter: "yrokopter", bombardier: "Bombardier", cook: "Kucharz", medic: "Medyk",
                      ship_ram: "Okrt z taranem", ship_flamethrower: "Okrt z miotaczem ognia", ship_steamboat: "Okrt parowy z taranem",
                      ship_ballista: "Balista", ship_catapult: "Okrt z katapult", ship_mortar: "Okrt z modzierzem",
					  ship_rocketship: "Rocket Ship", ship_submarine: "Okrt podwodny", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Wypenij jeden slot", removeSlot: "Oprnij jeden slot", fillRow: "Wypenij ca lini",
                      selectUnits: "Wybierz jednostki", assignNone: "Nic", assignAll: "Wszystko", assignField: "Wypenij pole bitwy",
                      presetsTitle: "Ustawienia", presetsAdd: "Dodaj ustawienie", presetsRemove: "Usu ustawienie", presetsNewName: "Podaj nazw ustawienia",
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "portuguese":   { by: "By", garrison: "Guarnio", advisor: "Orientador", cityLevel: "Nvel da Cmara", support: "Suporte",
                      artillery: "Artilharia", air: "Areo", line1: "Linha de Frente", line2: "Linha de longo alcance", 
                      flankLeft: "Flanco Esquerdo", flankRight: "Flanco Direito", submarine: "Submergvel", reserve: "Reserva", 
                      phalanx: "Hoplita", steamgiant: "Gigantes a Vapor", spearman: "Lanceiro", swordsman: "Espadachim", slinger: "Fundeiro",
                      archer: "Arqueiro", marksman: "Fuzileiros", ram: "Arete", catapult: "Catapulta", mortar: "Morteiro", 
                      gyrocopter: "Giro-cptero", bombardier: "Balo-Bombardeiro", cook: "Cozinheiro", medic: "Mdico", 
                      ship_ram: "Trireme", ship_flamethrower: "Lana-Chamas", ship_steamboat: "Abalroador a Vapor", 
                      ship_ballista: "Barco Balista", ship_catapult: "Barco Catapulta", ship_mortar: "Barco Morteiro", 
					  ship_rocketship: "Rocket Ship", ship_submarine: "Submergvel", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "romanian":     { by: "By", garrison: "Cazarma", advisor: "Asezarea trupelor pe campul de lupta", cityLevel: "Nivel Oras", support: "Unitati Ajutoare",
                      ...
                      ship_rocketship: "Rocket Ship", ship_submarine: "Submarin", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "russian":      { by: "By", garrison: "", advisor: "", cityLevel: " ", support: "", 
                      artillery: "", air: "", line1: " ", line2: " ", 
                      flankLeft: " ", flankRight: " ", submarine: " ", reserve: "",
                      phalanx: "", steamgiant: " ", spearman: "", swordsman: "", slinger: "",
                      archer: "", marksman: "", ram: "", catapult: "", mortar: "",
  ...
  //  "slovene":      { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "spanish":      { by: "By", garrison: "Guarnicin", advisor: "Asesor", cityLevel: "Nivel de la ciudad", support: "Apoyo", 
                      artillery: "Artillera", air: "Aire", line1: "Lnea de combate principal", line2: "Lnea de combate a larga distancia", 
                      flankLeft: "Flanco izquierdo", flankRight: "Flanco derecho", submarine: "Submarino", reserve: "Reserva", 
                      phalanx: "Hoplita", steamgiant: "Gigante a vapor", spearman: "Lancero", swordsman: "Espadachn", slinger: "Hondero",
                      archer: "Arquero", marksman: "Fusilero", ram: "Ariete", catapult: "Catapulta", mortar: "Mortero", 
                      gyrocopter: "Girocptero", bombardier: "Bombardero", cook: "Cocinero", medic: "Mdico", 
                      ship_ram: "Barco-Espoln", ship_flamethrower: "Barco-Lanzallamas", ship_steamboat: "Barco-Espoln a vapor", 
                      ship_ballista: "Barco-Ballesta", ship_catapult: "Barco-Catapulta", ship_mortar: "Barco-Mortero", 
                      ship_rocketship: "Rocket Ship", ship_submarine: "Submarino", ship_paddlespeedship: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "swedish":      { by: "By", garrison: "Garnison", advisor: "Rdgivare", cityLevel: "Stadsniv", support: "Support",
                      artillery: "Artilleri", air: "Flyg", line1: "Frontlinje", line2: "Distanslinje",
                      flankLeft: "Vnster flank", flankRight: "Hger flank", submarine: "Ubt", reserve: "Reserv",
                      phalanx: "Hoplit", steamgiant: "ng-jtte", spearman: "Spjutkastare", swordsman: "Svrdsman", slinger: "Slungare",
                      archer: "Bgskytt", marksman: "Karabinjr", ram: "Murbrcka", catapult: "Katapult", mortar: "Mrsare",
                      ...
                      ship_ballista: "Ballistskepp", ship_catapult: "Katapultskepp", ship_mortar: "Mrsarskepp",
                      ship_rocketship: "Rocket Ship", ship_submarine: "Ubt", ship_paddlespeedboat: "Paddle Speedboat",
					  ship_ballooncarrier: "Ballon Carrier", ship_tender: "Tender",
                      addSlot: "Add units to one slot", removeSlot: "Remove units from one slot", fillRow: "Fill out one row with units",
                      selectUnits: "Select units", assignNone: "None", assignAll: "All", assignField: "Fill battlefield",
                      presetsTitle: "Presets", presetsAdd: "Add new preset", presetsRemove: "Delete preset", presetsNewName: "Specify the name of preset to save",
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "turkish":      { by: "By", garrison: "Garnizon", advisor: "Danman", cityLevel: "ehir Boyutu", support: "Destek",
                      artillery: "Topu", air: "Hava", line1: "n Cephe", line2: "Uzun Menzilli Sava Hatt",
                      flankLeft: "Sol Kanat", flankRight: "Sa Kanat", submarine: "Submarine", reserve: "Rezerv",
                      phalanx: "Phalanx", steamgiant: "Buhar Devi", spearman: "Mzrak", swordsman: "Kl", slinger: "Ta",
                      archer: "Oku", marksman: "Tfeki", ram: "ahmerdan", catapult: "Mancnk", mortar: "Topu",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "ukrainian":      { by: "By", garrison: "", advisor: "", cityLevel: " ", support: "", 
                      artillery: "", air: "", line1: " ", line2: "", 
                      flankLeft: " ", flankRight: " ", submarine: " ", reserve: "",
                      phalanx: "", steamgiant: " ", spearman: "", swordsman: "", slinger: "",
                      archer: "", marksman: "", ram: "", catapult: "", mortar: "",
  ...
  const unitsList = {
    ...
    fleet:  [["ship_flamethrower", "ship_steamboat", "ship_ram", "ship_catapult", "ship_ballista", "ship_mortar", "ship_rocketship"],
           ["ship_submarine", "ship_paddlespeedship", "ship_ballooncarrier", "ship_tender"]]
  };
...
//    var lang = 'english';
  ...
  function appendGarrisonInfo() {
    ...
    html  = '<div id="armyPreview" style="top:180px; '+IkaRTL.txtLeft+':230px; width: 365px; height: 275px; z-index: 9999; position: absolute; text-align: justify; background-color: #FCF4DE; padding: 1px 5px 5px 5px; display: none;">';
    html += "<table>\n";
