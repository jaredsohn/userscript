...
// ver2.47 2011.04.16 ドワクエ Craford マップパーツを以前のバージョンにもどせるようにした
// ver2.48 2014.02.20 Sol　21×21メンテ対応　
...
function disp_AreaIcon(cx,cy,num,viewSize,group_flg)
{
	...
	function setArea(act, x, y, num, viewSize, group_flg)
	{
		...
		var old = false;
		var ck = $e('//*[@id="OldDesign"]');
		if( (check51_51mode() == true) && (ck.checked == true) ){
			old = true;
		}
		...
		if( (group_flg % 100) > 0 ){
			...
			if( viewSize == 15 ){
				...
				if( (hosttype != "1") && (hosttype != "l") && (old == true) ){
					img2.style.left = x;
				...
				img2.style.zIndex = 442;

				if( (hosttype != "1") && (old == true) ){
...
function getViewSize()
{
	...
	else if( $x("//div[@id=\"changemapscale\"]/ul/li[@class=\"sort21 now\"]") ){
		viewSize = 21;	// 20x20
	}
	else if( $x("//div[@id=\"change-map-scale\"]/ul/li[@class=\"sort15 now\"]") ){
		viewSize = 15;	// 15x15 本鯖の仕様変更対応
	}
	else if( $x("//div[@id=\"change-map-scale\"]/ul/li[@class=\"sort21 now\"]") ){
		viewSize = 21;	// 20x20 本鯖の仕様変更対応
	}
