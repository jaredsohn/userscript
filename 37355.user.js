// ==UserScript==
// @name           Create Football Wars game invites.
// @namespace      Football Wars friends.
// @description    Automatically invites people to your clan that you find on the fight page. Brandy altered the  the original code  from Andy Calderbanks  "Create dragon wars invites" http://userscripts.org/scripts/show/31912; Altered from Frisno's Vampire invite script.
// @include        http://apps.facebook.com/footballwars/fight.php
// @include        http://apps.facebook.com/footballwars/recruit.php
// @include        http://apps.new.facebook.com/footballwars/fight.php
// @include        http://apps.new.facebook.com/footballwars/recruit.php
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/footballwars/recruit.php';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/footballwars/stats.php?user=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/footballwars/recruit.php?action=create',
