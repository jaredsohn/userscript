// ==UserScript==
// @name        Ikariam ExMachina 0.4.5 movil
// @namespace   PhasmaExMachina
// @description A collection of mods, tools, and hacks for Ikariam by PhasmaExMachina
// @author      PhasmaExMachina  - Updates by Tuskin - updates by Panagiotis - update by zardav - Modify por Alex Corsario
// @website     http://userscripts.org/scripts/show/102851
...
// @require   http://userscripts.org/scripts/source/132833.user.js
...
ScriptUpdater.check(102851, "0.135");
  ...
  "About":{
    label:IkaTools.getText('optionsLabels', 'about', "About"),
  	html:'	<p><a href="http://userscripts.org/scripts/show/' + ScriptUpdater.scriptId + '" target="_blank" style="font-weight:bold !important;">Ikariam ExMachina v' + ScriptUpdater.scriptCurrentVersion +
  			'</a> by <a href="http://userscripts.org/users/PhasmaExMachina" target="_blank">PhasmaExMachina</a> - Updates by <a href="http://userscripts.org/users/332655" target="_blank">Tuskin</a>' +
  			'<p>This is a collection of Ikariam hacks by PhasmaExMachina - Updates by Tuskin</p>',
  	fields:{
  		debugMode:{
  			type:'checkbox',
...
var ExMachina = {
      ...
      if (Config.get('resourceLinks') || Config.get('resourceDetails'))
      {
				...
				rNav.forEach(function(item) 
				{
					if (item.className != 'population')
					{
  					...
  					if (change != 0) {
  						...
  						ttHtml += '<tr><td' + fullColor + '>' + (change > 0 ? IkaTools.getText('interface', 'Full', 'Full') : IkaTools.getText('interface', 'Empty', 'Empty')) + ':  </td><td' + fullColor + '>' + (days > 0 ? days + IkaTools.getText('interface', 'dayshort', 'd') + ' ' : '') + (hours > 0 ? hours + IkaTools.getText('interface', 'hourshort', 'h') + ' ' : '') + (minutes > 0 ? minutes + IkaTools.getText('interface', 'minuteshort', 'm') + ' ' : '') + '</td></tr>';
  					}
			...
			function movementsOverview() {
        		...
        		GM_addStyle('#ikaEmpireTradeMovements th { text-align:center; font-weight:bold; padding:.5em 0; } \
  					...
  					ExMachina.militaryOverview.innerHTML = '<div id="ikaEmpireMilitaryOverviewTransport"><div style="font-size:2em; padding:2em; text-align:center;">'+ IkaTools.getText('interface', 'loading', "loading") + ' ...</div></div>';
	...
	updateMilitaryOverview:function() 
	{
		...
		IkaTools.shipTypes.forEach(function(shipType)
		{
			...
			totalShipsInTraining[shipType] = 0;
		...
		cities.forEach(function(city)
		{
			...
			IkaTools.shipTypes.forEach(function(shipType)
			{
				...
				if (city.owned)
				  totalShipsInTraining[shipType] += IkaTools.cityGetShipInTraining(shipType, city).qty; 
			});
		});
      ...
      if (totalUnitsInTraining[unitType] || (totalUnits[unitType] > 0))
			  html += '<th class="' + unitType + '" title="'+IkaTools.getText('military', unitType, IkaTools.getUnitName(unitType))+'"> </th>';	
		});	
		IkaTools.shipTypes.forEach(function(shipType)
		{
      if (totalShipsInTraining[shipType] || (totalShips[shipType] > 0))
        html += '<th class="ship_' + shipType + '" title="'+IkaTools.getText('military', 'ship_'+shipType, IkaTools.getShipName(shipType))+'"> </th>';	
		});
		...
		cities.forEach(function(city)
		{
    		...
    		IkaTools.unitTypes.forEach(function(unitType)
    		{
    			if (totalUnitsInTraining[unitType] || (totalUnits[unitType] > 0)) 
    			{
    				...
    				html += '<td align="center">' + (qty > 0 ? qty : ((inTransit || inTraining.qty > 0) ? '' : '-')) + 
    							(inTransit ? ' <span title="' + IkaTools.getText('military', unitType, inTransit) + ' '+IkaTools.getText('interface', 'inTransitTo', "in transit towards")+' ' + IkaTools.cityGetSimpleName(city) + '" style="font-size:.8em !important;">(+' + inTransit + ')</span>' : '') +
    		...
    		IkaTools.shipTypes.forEach(function(shipType)
    		{
    			if (totalShipsInTraining[shipType] || totalShips[shipType] > 0) 
    			{
    				var inTraining = (city.owned) ? IkaTools.cityGetShipInTraining(shipType, city) : 0;
    				...
    				html += '<td align="center">' + (qty > 0 ? qty : ((inTransit || inTraining.qty > 0) ? '' : '-')) + (inTransit ? ' <span title="' + IkaTools.getText('military', 'ship_'+shipType, inTransit) + ' '+IkaTools.getText('interface', 'inTransitTo', "in transit towards")+' ' + IkaTools.cityGetSimpleName(city) + '" style="font-size:.8em !important;">(+' + inTransit + ')</span>' : '') + (inTraining.qty > 0 ? ' <span title="' + inTraining.qty + ' '+IkaTools.getText('interface', 'trainingShipyard', "training in shipyard")+'" style="font-size:.8em !important; cursor:help;">(+' + inTraining.qty + ')</span>' : '') +
    						'</td>';
    			}
    		});
		...
		IkaTools.shipTypes.forEach(function(shipType)
		{
			if(totalShipsInTraining[shipType] || (typeof(totalShips[shipType]) != 'undefined' && totalShips[shipType] > 0)) {
				html += '<td style="text-align:center;">' + 
							totalShips[shipType] +
							(totalShipsInTraining[shipType] > 0 ? ' <span title="' + totalShips[shipType] + ' + ' + totalShipsInTraining[shipType] + ' '+IkaTools.getText('interface', 'training', "training")+' = ' + (totalShips[shipType] + totalShipsInTraining[shipType]) + ' '+IkaTools.getText('interface', 'total', "total")+'" style="font-size:.8em !important; cursor:help;">(+' + totalShipsInTraining[shipType] + ')</span>' : '') +  
						'</td>';
			}
		});
		html += '<td></td></tr></table>';
			...
			$('td', IkaTools.buildingsOverview).mouseover(function() {
				try {
					...
					if(building && !IkaTools.buildingIsMaxed(building)) {
						...
						var html = '<p style="margin-bottom:.5em;' + (hasResources ? ' color:#009900; font-weight:bold;' : '') + '">' + IkaTools.getText('buildings', building.type, building.name) + ' (' + (parseInt(building.level) + 1) + ')</p>';
	...
	drawEmpireOverview:function() {
		if(!ExMachina.worldOverview) {
			...
			ExMachina.worldOverview.setAttribute('style', 'display:none; background-color:#FEE8C8; border:1px solid #9D836A; text-align:'+IkaRTL.txtLeft+'; width:950px; margin-'+IkaRTL.txtLeft+':-320px; position:relative;'); 
				...
				table.style.width = "100%";
				table.innerHTML = '<tr valign="bottom" style="background-image:url(/skin/input/button.gif)"><td colspan="2"></td> <td style="text-align:center; padding:0 .05em;"><img src="http://' + document.domain + '/skin/img/city/constructionSite.gif" style="height:12px; margin-top:5px;" title="'+IkaTools.getText('interface', 'UnderConstrution', 'Building under construction')+'"/></td>\
				...
				cities.forEach(function(city, i)
				{
					if(!city.type || /tradegood/.test(city.type)) {
						...
						tr.className = "ikaEmpireOverviewRow" + (i % 2 != 0 ? "Uneven" : "");
							...
							td.style.fontWeight = isCurrent ? 'bold' : '';
							td.innerHTML = i + 1;
							tr.appendChild(td);
							var td = document.createElement('td');
							td.className = "ikaEmpireOverviewCell";
								...
								var rTD = document.createElement('td');
								  ...
								  else
								  {
									...
									if(type != "gold") {
										...
										if(type == 'wine') {
											rTD.title += " - "+IkaTools.getText('interface', 'wineconsumption', 'Wine consumption')+": " + IkaTools.cityGetWineConsumption(city) + " "+IkaTools.getText('interface', 'perhour', 'per hour');
										}
									...
									if (type != 'gold' && type != 'research') {
										...
										var hours = Math.floor(hoursTotal % 24);
										var minutes = Math.floor(minutesTotal % 60);
										var untilFullText = (days > 0 ? days + IkaTools.getText('interface', 'dayshort', 'd')+' ' : '') + (hours > 0 ? hours + IkaTools.getText('interface', 'hourshort', 'h')+' ' : '') + (minutes > 0 ? minutes + IkaTools.getText('interface', 'minuteshort', 'm')+' ' : '');
		...
		if(city.buildings) {
			...
			buildings.forEach(function(building)
			{
				if (building.position) {
					...
					var parsedName = IkaTools.getText('buildings', building.type, building.name).length > 14 ? IkaTools.getText('buildings', building.type, building.name).replace(/[^\s]+\s*$/, '') + '...' : IkaTools.getText('buildings', building.type, building.name);
  				...
  				resourceNames.forEach(function(resourceName)
					...
					a.title = IkaTools.getText('buildings', building.type, building.name) + ' (' + building.level + ')';
					a.style.fontWeight = ((construction && construction.position.toString() == building.position.toString()) ? 'bold' : 'normal');
					div.appendChild(a);
				}
			});
		}
	...
	drawMovements:function(movements) {
		...
		movements.forEach(function(movement, i) {
			...
			for(var x = 0; x < units.length; x++) {
				iconRowHtml += '<td style="padding:0 .5em .2em 0;" title="' + IkaTools.getText('interface', units[x].type, units[x].name) + '"><img src="' + units[x].iconSrc + '" style="height:14px;"/></td>'; 
    ...
    IkaTools.querySelectorAsArray('tr.entry:not(.exMachinaParsed)', msgCnt).forEach(function(item) {
          ...
          var tblRep = IkaTools.querySelector('#tbl_reply' + messageId, msgCnt);
				...
				if (Config.get('searchIconInbox')) {
					...
					$(IkaTools.querySelectorAll('nobr', playerCell[0])).append(' <img class="ikaSearchIcon" title="'+ IkaTools.sprintf(IkaTools.getText('interface','ShowPlayerCitiesMap','Show %s\'s cities on map'),playerName) +'" src="' + IkaSearch.icons.magnifier + '" style="cursor:pointer; display:inline; vertical-align:middle;"/>');
    ...
    cities.forEach(function(city){
			...
			if(IkaTools.cityGetBuildBuilding(city) && elem) {
				...
				elem.innerHTML = '<a href="javascript:ikaEmpireGoTo(\'/index.php?view=' + building.type + '&id=' + building.cityId + '&position=' + building.position + '\', ' + building.cityId + ')" ' +
									' style="background:none; padding:0; display:inline; height:auto; width:auto;">' + IkaTools.getText('buildings', IkaTools.cityGetBuildBuilding(city).type, IkaTools.cityGetBuildBuilding(city).name) + '</a>' +
...
ExMachina.views.townHall = function() {
	...
	function durationHMS(seconds,depth){
	  var temp = {'day':IkaTools.getText('interface', 'dayshort', 'd'), 'hour':IkaTools.getText('interface', 'hourshort', 'h'), 'minute':IkaTools.getText('interface', 'minuteshort', 'm'), 'second':IkaTools.getText('interface', 'secshort', 's')}, ret = [], prefix = '';
	  if (seconds == 0) { return IkaTools.getText('interface', 'Now', 'Now'); }
...
function sortBuildingsByName(array) {
	...
	for(var i = 0; i < array.length; i++) {
		var empty = false;
		var nameLevel = IkaTools.getText('buildings', array[i].type, array[i].name) + array[i].level;
		while(!empty) {
	...
	views:{
		...
		sendIKMessage:function() {
			...
			if (elems) {
				...
				catEnabled = false;
				for (var i = 0; i < elems.length; i++) {
					if (elems[i].value.toString() == '79') 
						catEnabled = true;
				}
				if(!catEnabled) {
					var option = document.createElement('option');
					option.value = '79';
					option.innerHTML = IkaTools.getText('interface', 'AcceptCT', 'Accept cultural goods treaty') + ' (ExMachina)';
					selector.appendChild(option);
				}
			}
		}
...
//				numSpiesTotal++;
			...
			function convertMsToTimeStr(ms) {
				...
				minutes -= hours * 60;
				return (hours > 0 ? hours + IkaTools.getText('interface', 'hourshort', 'h') + ' ' : '') + (minutes > 0 ? minutes + IkaTools.getText('interface', 'minuteshort', 'm') + ' ' : '');
			}
...
*/
			...
			// remove city from target list 
			
			//    N O T      W O R K I N G ! ! ! ! missing what should actually do ... 
			
			html += numSpiesTotal == 0 ? 
					'<img src="' + Pillage.icons.trash + '" title="'+IkaTools.getText('pillageHelper', 'RemoveCity', "Remove city from target list")+' (' + target.cityName + ')" style="cursor:pointer;" class="pillageHelperTargetOverviewDeleteImg" name="' + target.cityName + '" />' : 
	...
	getTimeSinceString:function(timeStart) {
		...
		if(diff < 60) 
			...
			return (days > 0 ? days + IkaTools.getText('interface', 'dayshort', 'd') + ' ' : '').toString() + (hours > 0 ? hours + IkaTools.getText('interface', 'hourshort', 'h') : '').toString() + (minutes > 0 ? minutes + IkaTools.getText('interface', 'minuteshort', 'm') : '').toString();		
	},
			...
			function drawResearches() {
				$('#mainview .contentBox01h .content ul li', root).each(function() {													 
					...
					if(research = Librarian.getResearchByName(name)) {
						...
						$('a', this)[0].innerHTML = ('\
							<div style="width: 170px; float:'+IkaRTL.txtLeft+'; margin-'+IkaRTL.txtRight+':2em;">' + html + '</div>\
							<div style="width: 70px; float:'+IkaRTL.txtLeft+'; margin-'+IkaRTL.txtRight+':2em;">' + (this.className == 'explored' ? '&nbsp;' : research.completeIn)+ '</div>\
							<div style="width: 318px; float:'+IkaRTL.txtLeft+'; margin-'+IkaRTL.txtRight+':2em;">' + research.effect + '</div>\
						');
					}
				});
			}
...
IAB = {
	...
	drawQueueBuildings:function(buildings, queueBox, isUpgrades) {
		...
		buildings.forEach(function(b, i)
		{
			if(IkaTools.getVal('autoBuildFilterByCity').toString() != 'yes' || b.cityId == ExMachina.currentCityId) {
				...
				var truncatedName = IkaTools.getText('buildings', b.type, b.name).length > 14 ? IkaTools.getText('buildings', b.type, b.name).replace(/\s*[^\s]+\s*$/, '')+'...' : IkaTools.getText('buildings', b.type, b.name);
				var deleteTitle = isUpgrades ? IkaTools.getText('autoBuild', 'RemoveAutoUpgrade', 'Remove from auto upgrades')+" (" + IkaTools.getText('buildings', b.type, b.name) + ") " : IAB.getText('queue').remove + ' ' + IkaTools.getText('buildings', b.type, b.name) + ' ' + IAB.getText('queue').fromQueue;		
				...
				html += '<table cellspacing="0" border="0" cellpadding="0" style="width:auto; margin:0; border-collapse:none !important;"><tr >\
						  <td style="vertical-align:top; padding:0 .5em 0 0;" valign="top">\
						  	<a id="'+linkId+i+'" href="javascript:void(0);" name="'+b.type+'_'+b.cityId+'_'+b.position+'" title="'+IkaTools.getCityById(b.cityId).name+' - '+IkaTools.getText('buildings', b.type, b.name)+' ('+(parseInt(b.level)+1)+')"'+
	...
	views:{
			...
			if(!buildingQueued) {
				...
				for(var i = 0; i < buildings.size(); i++) {
					...
					if(type == queuedBuilding.type) {
						...
						notice.innerHTML = IkaTools.getText('interface', 'buildinginqueue', "Building in queue");
		...
		options:function() {	
			...
			var delayOptionsI= '<option value="60001">1.1 ' + IAB.getText('options').minutes + '<options>\
								<option value="300000">5 ' + IAB.getText('options').minutes + '<options>\
								<option value="300001">5.1 ' + IAB.getText('options').minutes + '<options>\
								<option value="600000">10 ' + IAB.getText('options').minutes + '<options>\
								<option value="610000">10.1 ' + IAB.getText('options').minutes + '<options>\
								<option value="900000">15 ' + IAB.getText('options').minutes + '<options>\
								<option value="900001">15.1 ' + IAB.getText('options').minutes + '<options>\
								<option value="1200000">20 ' + IAB.getText('options').minutes + '<options>\
								<option value="1200001">20.1 ' + IAB.getText('options').minutes + '<options>\
								<option value="1800000">30 ' + IAB.getText('options').minutes + '<options>\
								<option value="2700000">45 ' + IAB.getText('options').minutes + '<options>\
								<option value="3600000">1 ' + IAB.getText('options').hour + '<options>\
								<option value="7200000">2 ' + IAB.getText('options').hours + '<options>\
			...
			content.innerHTML = '<a name="autoBuild"></a><h3><a href="http://userscripts.org/scripts/show/80545" target="_blank">Ikariam Auto Build' + 
								...
								'<table cellspacing="0" cellpadding="0">\
									<tbody><tr>\
									  ...
									  '+ IkaTools.getText('interface','to','to') + '<select id="autoBuildOptionMaxDelay">' + delayOptionsI + '<option value="21600000">6 ' + IAB.getText('options').hours + '</option></select></td>\
									</tr>\
									...
									<tr><th> </th><td>\
										<input type="button" class="button" id="autoBuildScriptUpdates" value="' + IkaTools.getText('interface','updatesAndHistory','Script Updates & History') + '"/>\
									</td></tr>\
								  </tbody></table>'; 
...
IkaNotify = {
	...
	// notice types
	events:{
		attack:IkaTools.getText('interface','incomingattack','Incoming Attack'),
		movements:IkaTools.getText('interface','troopresourcemovements','Troop/Resource Movements'),
		advCities:IkaTools.getText('interface','townadvisor','Town Adviser'),
		advMilitary:IkaTools.getText('interface','militaryadvisor','Military Adviser'),
		advResearch:IkaTools.getText('interface','researchadvisor','Research Adviser'),
		advDiplomacy:IkaTools.getText('interface','diplomacyadvisor','Diplomacy Adviser'),
		construction:IkaTools.getText('interface','buildingcompletion','Building Completion'),
	},
	notices:{
		growl:{
			...
			test:function() {
				Growler.growl('test', IkaTools.getText('interface','testmessage','Test Message') + " - " + document.domain.replace(/\.ikariam\..+$/, ''), isChrome ? IkaTools.getText('interface','testmessage1','Desktop notification is working correctly!') :  IkaTools.getText('interface','testmessage2','Growl is working correctly!') );
			},
		...
		popup:{
			...
			checkedByDefault:false,
			test:function() {
				alert(IkaTools.getText('interface','testpopup','This is a test popup notification'));
			},
			send:function(eventType, message) {
	...
	checkAdviser:function(type, root) {
		if($('#' + type + ' a', root).attr('class') != 'normal') {
			if(typeof(IkaTools.getVal(type + '_isActive')) != 'undefined' && IkaTools.getVal(type + '_isActive').toString() != "yes") {
				IkaNotify.sendNotice(type, IkaTools.sprintf(IkaTools.getText('interface','advisorhassthtosay','The %s has something to say!'),IkaNotify.events[type]));	
			...
			if(delay % 30 == 0) {
				for(var i = 0; i < IkaNotify.constructions.length; i++) {
					if(d.getTime() >= IkaNotify.constructions[i].endTime) {
						...
						var city = IkaTools.getCityById(building.cityId);
						IkaNotify.sendNotice('construction', IkaTools.sprintf(IkaTools.getText('interface','buildingcompletionnotice','Your %s in %s is now level %s'),IkaTools.getText('buildings', building.type, building.name),city.name,(parseInt(building.level) + 1)));	
	...
	checkForEmailKeyError:function(txt) {
		if(txt == 'invalidEmailKey') {
			var c = confirm(IkaTools.sprintf(IkaTools.getText('interface','invalidemailkey','The Ikariam Notify key you are using appears to be invalid.\n\nSend a new one to %s?'),IkaNotify.email));	
	...
	sayServerError:function(text) {
		alert(IkaTools.getText('interface','invalidemailkey','Ikariam Notifier is having trouble accessing the mail server.\n\nYou may want to disable it for a bit while things get worked out.\n\nResponse from server:\n\n') + (typeof(text) != 'undefined' ? text : ''));
	},
	sendKey:function() {
		...
		GM_xmlhttpRequest ({
			...
			onload: function (response){
				if(response.responseText != "getEmailKey") {
					...
					alert(IkaTools.sprintf(IkaTools.getText('interface','sendnewkey','A new key has been sent to %s.\n\nPlease check your email and enter the key on the Ikariam\nOptions page under the Ikariam Notifier section.'),IkaNotify.email));
				}
		...
		GM_xmlhttpRequest ({
			...
			onload: function (response){
				if(!IkaNotify.checkForEmailKeyError(response.responseText) && response.responseText != "test") {
					...
					alert(IkaTools.sprintf(IkaTools.getText('interface','testmailsent','A test email has been sent to %s'),IkaNotify.email));
				}
			}
		})
	},
	...
	views:{
		options:function() {
			var content = document.createElement('div');
			var delayOptions = '<option value="300000">5 ' + IkaTools.getText('interface','minutes','minutes') + '<options>\
								<option value="600000">10 ' + IkaTools.getText('interface','minutes','minutes') + '<options>\
								<option value="900000">15 ' + IkaTools.getText('interface','minutes','minutes') + '<options>\
								<option value="1200000">20 ' + IkaTools.getText('interface','minutes','minutes') + '<options>\
								<option value="1800000">30 ' + IkaTools.getText('interface','minutes','minutes') + '<options>\
								<option value="3600000">1 ' + IkaTools.getText('interface','hour','hour') + '<options>\
								<option value="7200000">2 ' + IkaTools.getText('interface','hours','hours') + '<options>\
								<option value="14400000">4 ' + IkaTools.getText('interface','hours','hours') + '<options>';
			var validEmailInlineDisplay = IkaNotify.validateEmail(IkaNotify.email) ? 'inline' : 'none';

			var html = '<a name="ikaNotify"></a><h3><a href="http://userscripts.org/scripts/show/80545" target="_blank" title="' + IkaTools.getText('interface','notifiertitle','More information about this script') + '">' + IkaTools.getText('interface','ikariamnotifier','Ikariam Notifier') + '</a> \
							<span style="font-weight:normal">' + IkaTools.getText('interface','of','by') + ' <a href="http://userscripts.org/users/PhasmaExMachina" target="_blank">PhasmaExMachina</a></span></h3>\
							<table cellspacing="0" cellpadding="0"><tbody>\
								<tr><th>' + IkaTools.getText('interface','checkeventsevery','Check for events every') + '</th>\
								  <td><select id="ikaNotifyMinDelay"><option value="60000">1 ' + IkaTools.getText('interface','minute','minute') + '</option>' + delayOptions + '</select>\
								  ' + IkaTools.getText('interface','to','to') + ' <select id="ikaNotifyMaxDelay">' + delayOptions + '<option value="21600000">6 ' + IkaTools.getText('interface','hours','hours') + '</option></select></td>\
								</tr>\
								<tr valign="top"  style="border-top:1px dotted #F1D198"><th style="padding-top:15px;">' + IkaTools.getText('interface','emailaddress','Email Address') + '</th><td><input type="text" id="ikaNotifyEmail" value="' + IkaNotify.email + '" style="width:200px;" title="' + IkaTools.getText('interface','emailinputtitle','All email alerts will be sent to this address') + '"/>\
									  [ <a href="javascript:void(0)" id="ikaNotifySendTest" style="display:' + ((IkaNotify.emailKey != "" && IkaNotify.validateEmail(IkaNotify.email)) ? 'inline' : 'none') + '">' + IkaTools.getText('interface','test','test') + '</a> ]\
									<br/><span style="font-style:italic">' + IkaTools.getText('interface','emailcaption','emails will only be sent for new attacks') + '</span>\
								</td></tr>\
								<tr><th>' + IkaTools.getText('interface','emailkey','Email Key') + '</th><td><input type="text" id="ikaNotifyEmailKey" value="' + IkaNotify.emailKey + '" style="width:100px;" title="' + IkaTools.getText('interface','emailkeytitle','This key is used to prevent abuse of the notifier') + '"/>\
									  <span class="ikaNotifySendingKeyNotice" style="display:none;">' + IkaTools.getText('interface','sendkeynotice','sending key ... please wait') + '</span><a href="javascript:void(0)" id="ikaNotifyResendKey" style="display:' + validEmailInlineDisplay + '">'
									  + IkaTools.sprintf(IkaTools.getText('interface','getresendkey','%s Key'),(IkaNotify.emailKey != "" ? IkaTools.getText('interface','resend','Resend') : IkaTools.getText('interface','get','Get'))) + '</a>\
								</td></tr>';
				for(var notice in IkaNotify.notices) {
					...
					if(typeof(IkaNotify.notices[notice].test) == 'function')
						html += '[ <a id="ikaNotift_noticeTest_' + notice + '" style="cursor:pointer">' + IkaTools.getText('interface','test','test') + '</a> ]';	
					html += '<br><br>';
					for(var e in IkaNotify.events)
				...
				html += '<tr><th> </th><td>\
										<input type="button" class="button" id="ikaNotifyScriptUpdates" value="' + IkaTools.getText('interface','updatesAndHistory','Script Updates & History') + '"/>\
									</td></tr>';
				html +=	'</tbody></table>';
			...
			if (IkaTools.addOptionBlock(content))
			{
  			...
  			$('#ikaNotifyEmailKey')[0].addEventListener('keyup', function() { 
  				...
  				$('#ikaNotifySendTest')[0].style.display = ($('#ikaNotifyEmailKey')[0].value != "" && IkaNotify.validateEmail($('#ikaNotifyEmail')[0].value)) ? "inline" : "none";
				$('#ikaNotifyResendKey')[0].innerHTML = IkaTools.sprintf(IkaTools.getText('interface','getresendkey','%s Key'),(this.value == "" ? IkaTools.getText('interface','get','Get') : IkaTools.getText('interface','resend','Resend')));	
		...
		militaryAdvisorMilitaryMovements:function() {
			var div = document.createElement('div');
				...
				div.innerHTML = '<a href="http://' + document.domain + '/index.php?view=options#ikaNotify">' + IkaTools.getText('interface','notifieroptions','Notifier Options') + '</a>';
...
var IkaSearch = {
	init:function() {
		...
		if(document.location.toString().match(/ika-world/)) {
			...
			buttonClose.innerHTML = IkaTools.getText('interface','close','close');
			...
			if(Config.get('worldOverview')) {
				...
				imgWorld.title = IkaTools.getText('interface','worldmapoverview','World Map Overview');
	...
	drawWorldOverview:function(callback) {
		...
		var infoBox = document.createElement('div');
			...
			var html = '<h3 class="header">' + IkaTools.getText('interface','worldsearch','World Search') + '</h3><div class="content" style="padding:.5em 10px;">\
							<p style="padding-'+IkaRTL.txtLeft+':.8em;text-align:right">' + IkaTools.getText('interface','Player','Player') + ': \
							...
							<p style="padding-'+IkaRTL.txtLeft+':.8em;text-align:right">' + IkaTools.getText('interface','Alliance','Alliance') + ': <input tyle="text" style="width:100px;" id="ikaSearchAlliance"/></p>\
							<p style="text-align:center;"><input id="ikaSearchWorldSubmit" value="' + IkaTools.getText('optionsLabels','search','Search') + '" class="button" style="width:75px; display:inline;"/></p>\
			...
			html += '<tr>';
				for(var x = 1; x <= 100; x++) {
					...
					html += '<td id="ikaSearchIsland_' + id + '" style="background-color:#' + color + ';" ' + 
								(island ? 'title="' + name + ' [' + x + ',' + y + '] - ' + IkaTools.getText('interface', type, type) + ', ' + IkaTools.getText('interface','woodlevel','Wood Level') + ' ' + woodLevel + ' (' + numCities + (island[7] == 1 ? ' ' + IkaTools.getText('interface','citysmall','city') : ' ' + IkaTools.getText('interface','citiessmall','cities')) + ')"' : '') +
...
var ExternalScripts = {}
//----------------------------- Ikariam Army Helper v0.32 - http://userscripts.org/scripts/show/94360 
ExternalScripts.ArmyHelper = function () {
  //-----------------------------------------------------------------------------
  const languages = {
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "arabic":       { by: "By", garrison: "القوات العسكرية", advisor: "المستشار", cityLevel: "مستوى دار البلدية", support: "مساعدة",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "bosnian":      { by: "By", garrison: "Garnizon", advisor: "Savjetnik", cityLevel: "Gradski Level", support: "Podrška", 
  ...
  //  "chinese":      { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "czech":        { by: "By", garrison: "Posádka", advisor: "Poradce", cityLevel: "Úroveň radnice", support: "Podpora",
  ...
  //  "danish":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "dutch":        { by: "By", garrison: "Garnizoen", advisor: "Adviseur", cityLevel: "Stadsgrootte", support: "Ondersteuning", 
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "english":      { by: "By", garrison: "Garrison", advisor: "Advisor", cityLevel: "Town level", support: "Support", 
  ...
  //  "finish":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "french":       { by: "By", garrison: "Garnison", advisor: "Conseiller", cityLevel: "Niveau ville ", support: "Support", 
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "german":       { by: "Von", garrison: "Garnison", advisor: "Berater", cityLevel: "Stadtgröße", support: "Unterstützung",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "greek":        { by: "Από το", garrison: "Φρουρά", advisor: "Σύμβουλος", cityLevel: "Επίπεδο Πόλης", support: "Μονάδες Υποστήριξης",
                      ...
                      addSlot: "Προσθήκη μονάδων σε μια θέση", removeSlot: "Απομάκρυνση μονάδων από μια θέση", fillRow: "Συμπλήρωση μιας γραμμής με μονάδες",
                      selectUnits: "Επιλογή μονάδων", assignNone: "Καμία", assignAll: "Όλες", assignField: "Fill battlefield",
                      presetsTitle: "Πρότυπα", presetsAdd: "Προσθήκη νέου προτύπου", presetsRemove: "Διαγραφή προτύπου", presetsNewName: "Ορίστε το όνομα του προτύπου για αποθήκευση",
                      optShowTitles: "Προβολή τίτλων γραμμών πεδίου μάχης" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "hebrew":       { by: "מאת", garrison: "יחידות בעיר", advisor: "תצוגת שדה הקרב", cityLevel: "שלב עיר", support: "תמיכה", 
                      artillery: "ארטילריה", air: "אוויר", line1: "קו החזית", line2: "ירי לרחוק", 
                      flankLeft: "אגף ימין", flankRight: "אגף שמאל", submarine: "אגף צוללות", reserve: "עתודה",
                      phalanx: "פלנקס", steamgiant: "ענק קיטור", spearman: "נושא חנית", swordsman: "נושא חרב", slinger: "קלע",
                      archer: "קשת", marksman: "רובאי", ram: "איל-ברזל", catapult: "קטפולטה", mortar: "מרגמה",
                      gyrocopter: "גירוקופטר", bombardier: "מפציץ", cook: "טבח", medic: "רופא",
                      ship_ram: "ספינת אי-ברזל", ship_flamethrower: "להביור", ship_steamboat: "איל-ברזל מושט", 
                      ship_ballista: "ספינת בליסטרה", ship_catapult: "ספינת קטפולטה", ship_mortar: "ספינת מרגמה", 
                      ship_submarine: "צוללת",
                      addSlot: "הוסף יחידות של אגף אחד", removeSlot: "הסר יחידות של אגף אחד", fillRow: "השלם שורה שלמה עם יחידות אלו",
                      selectUnits: "בחר יחידות", assignNone: "כלום", assignAll: "הכל", assignField: "השלם שדה קרב",
                      presetsTitle: "פריסות", presetsAdd: "הוסף פריסה חדשה", presetsRemove: "מחק פריסה", presetsNewName: "תן שם לפריסה בשביל לשמור",
                      optShowTitles: "הראה כותרות שורות שדה קרב"  },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "hungarian":    { by: "By", garrison: "Helyőrség", advisor: "Tanácsadó", cityLevel: "Város méret", support: "Támogatás",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "italian":      { by: "By", garrison: "Guarnigione", advisor: "Consulente", cityLevel: "Livello Municipio", support: "Supporto", 
  ...
  //  "korean":       { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "latvian":      { by: "By", garrison: "Garnizons", advisor: "Konsultants", cityLevel: "Pilsētas līmenis", support: "Palīdzība",
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "lithuanian":   { by: "By", garrison: "Įgula", advisor: "Patarėjas", cityLevel: "Miesto lygis", support: "Palaikymas",
  ...
  //  "pinoy":        { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "polish":       { by: "By", garrison: "Garnizon", advisor: "Pole bitwy", cityLevel: "Poziom ratusza", support: "Wsparcie",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "portuguese":   { by: "By", garrison: "Guarnição", advisor: "Orientador", cityLevel: "Nível da Câmara", support: "Suporte",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "romanian":     { by: "By", garrison: "Cazarma", advisor: "Asezarea trupelor pe campul de lupta", cityLevel: "Nivel Oras", support: "Unitati Ajutoare",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "russian":      { by: "By", garrison: "Гарнизон", advisor: "Помощник", cityLevel: "Уровень города", support: "Поддержка", 
  ...
  //  "slovene":      { },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "spanish":      { by: "By", garrison: "Guarnición", advisor: "Asesor", cityLevel: "Nivel de la ciudad", support: "Apoyo", 
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "swedish":      { by: "By", garrison: "Garnison", advisor: "Rådgivare", cityLevel: "Stadsnivå", support: "Support",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "turkish":      { by: "By", garrison: "Garnizon", advisor: "Danışman", cityLevel: "Şehir Boyutu", support: "Destek",
                      ...
                      optShowTitles: "Show battlefield row titles" },
  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    "ukrainian":      { by: "By", garrison: "Гарнізон", advisor: "Помічник", cityLevel: "Рівень міста", support: "Підтримка", 
...
//    var lang = 'english';
    $("script").each( function() {
      var langMatch = IkaTools.config.language;
      if ( langMatch ) {
        lang = {
        ...
        de:  "german",     el:  "greek",    it:  "italian",    hk:  "chinese",
        hu:  "hungarian",  il:  "hebrew",	he:	 "hebrew",	   kr:  "korean",     lv:  "latvian",
  ...
  function appendGarrisonInfo() {
    ...
    html  = '<div id="armyPreview" style="top:180px; '+IkaRTL.txtLeft+':230px; width: 362px; height: 205px; z-index: 9999; position: absolute; text-align: justify; background-color: #FCF4DE; padding: 1px 5px 5px 5px; display: none;">';
    html += "<table>\n";
    ...
    html += '</tr>\n';
      ...
      html += "</table>";
    html += "<div style='position: absolute; top: -10px; '+IkaRTL.txtLeft+': -5px; width: 365px; height: 5px;'><img width='365px' src='" + images.hintTop + "'/></div>" +
            ...
            "<div style='position: absolute; bottom: -5px; '+IkaRTL.txtLeft+': -5px; width: 365px; height: 5px;'><img width='365px' src='" + images.hintBottom + "'/></div>";
    html += '</div>';
  ...
  function drawUnitButtons(type, targetBattlefieldProperties, draw) {
    GM_addStyle(
      'input.fillRow { position: absolute; top: 10px; '+IkaRTL.txtLeft+': 473px; width: 15px; margin: 9px; padding: 2px 0px; } ' +
      'input.fillRow:active { padding: 3px 0px 1px 0px; } ' +
      'input.addSlot { position: absolute; top: 10px; '+IkaRTL.txtLeft+': 605px; width: 15px; margin: 9px; padding: 2px 0px; } ' +
      'input.addSlot:active { padding: 3px 0px 1px 0px; } ' +
      'input.removeSlot { position: absolute; top: 10px; '+IkaRTL.txtLeft+': 511px; width: 15px; margin: 9px; padding: 2px 0px; } ' +
  ...
  function drawAttackHelper(type, draw) {
    ...
    if (draw) {
      ...
      html += '<div style="margin: -18px 10px 8px 5px; text-align: '+IkaRTL.txtRight+';">'+ language.by + ' <a target="_blank" href="http://userscripts.org/scripts/show/94360">Ikariam Army Helper</a>.</div>\n';
  ...
  function drawPresetsBox(type, targetBattlefieldProperties, draw) {
    ...
    html += '<div style="text-align: '+IkaRTL.txtLeft+'; margin-'+IkaRTL.txtLeft+': 5px;"><a href="" clsss="" id="ikaArmyPresetsAddNew">'+language.presetsAdd+'</a></div>';
    div.innerHTML = html;
  ...
  function drawOptions() {
    var div = document.createElement('div');
    div.setAttribute('class', "content");
    var html = '<h3><a href="http://userscripts.org/scripts/show/94360" target="_blank">Ikariam Army Helper </a> <span style="font-weight: normal;"> ' + IkaTools.getText('interface','of','by') + ' <a href="http://userscripts.org/users/273982" target="_blank">Dino.pl</a></span></h3>'
