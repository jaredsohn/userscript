...
// 04/20/2011 Given to Mr. Wizard for hosting.  All Future updates will be by Mr. Wizard.  -- Thank you GuessX for the support.
// 04/20/2011 Removed useless code on live version.  Should help speed up loading time.
...
javascript: (function () {
	...
	function Process_Attack(b) {
		...
		if (/You won/i.test(b)) {
			...
			if (X == true) {
				d += ' <img src="http://guessx.x10.mx/kill.png" alt="Kill"></img>'
			}
			try {
	...
	function Add_to_loot(b, c, d) {
		if (document.getElementById('Log_Loot').checked) {
			...
			var z = /Idle Hands|SlipStream|Talon|Windswept/;
            ...
            if (b.search(e) > -1) {
				b = b + '<span class="more_in">[WD]</span>'
			}
			if (b.search(BPC) > -1) {
				b = b + '<span class="more_in">[BRAZIL]</span>'
			}			
			if (b.search(j) > -1) {
				b = b + '<span class="more_in">[CS]</span>'
			}
			if (b.search(h) > -1) {
				b = b + '<span class="more_in">[FHEL1]</span>'
			}
			if (b.search(i) > -1) {
				b = b + '<span class="more_in">[FHEL2]</span>'
			}
			if (b.search(o) > -1) {
				b = b + '<span class="more_in">[FHEL3]</span>'
			}
			if (b.search(p) > -1) {
				b = b + '<span class="more_in">[FHEL4]</span>'
			}
			if (b.search(z) > -1) {
				b = b + '<span class="energy_highlight">[FHEL5]</span>'
			}
			if (b.search(f) > -1) {
				b = b + '<span class="more_in">[Vegas]</span>'
			}
			if (b.search(m) > -1) {
				b = b + '<span class="more_in">[BHEL]</span>'
			}
			if (b.search(g) > -1) {
				b = b + '<span class="more_in">[MHEL]</span>'
			}
			if (b.search(q) > -1) {
				b = b + '<span class="more_in">[LVHEL]</span>'
			}
			if (b.search(r) > -1) {
				b = b + '<span class="more_in">[IHEL]</span>'
			}
			if (b.search(n) > -1) {
				b = b + '<span class="more_in">[Armory]</span>'
			}
	...
	function load() {
		try {
			var a = _gat._getTracker("UA-20419663-2");
			a._trackPageview("/Brawler-Lite")
		} catch(err) {}
	}
})();
function Trace(a) {
