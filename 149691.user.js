...
function yod_doRT(screen_name, content) {
  return function (e) {
    ...
    content = escape(ytrim(unescape(content)).replace(pattcontent, my_screen_name));
    s_gm.type = 'text/javascript'; s_gm.id = s_gmbinder;
    s_gm.innerHTML = "yodShowTweetBox('" + escape(ytrim(unescape(screen_name))) + "','" + content + "%yod%', '" + valRT + "');";
...
function translate_link(tt, shortlink) {
  ...
  var content = yodRTDiv.innerHTML.replace(/<\/?[^>]+>/g, '').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, '\'')/*.replace(/"/g, '')*/;
  yodRTDiv.innerHTML = "";
  return content;
}
...
function yod_make_RT(entry, newtweet) {
  ...
  if (!hasClass(entry, "yodDone2")) {
    if (newtweet) {
      var pattcontent = new RegExp("@" + my_screen_name, "gmi");
      //var pattcontent = new RegExp("in", "gmi");
      if (tt.textContent.match(pattcontent)) {
        playsound();
...
function attachScroll(div, title, px, fn) {
  var spritemap = 'data:image/gif;base64,';
  var style = 'background-image: url(\'' + spritemap + '\'); background-repeat: no-repeat; display: inline-block; height: 15px; width: 10px; float:left;';
  var el = document.createElement('div');
  el.setAttribute('style', style + 'background-position: left ' + px + ';');
...
function doAdvTop() {
  if (logo = g("logo") || g("global-actions")) {
    ...
    div.setAttribute('style', 'width: 10px; margin: 5px auto; padding-right: 5px; cursor: pointer; float: left;');
    attachScroll(div, 'Top', 'top', function(){scroll(0, 0);});
    attachScroll(div, 'Bottom', '-15px', function(){scroll(0, document.body.scrollHeight);});
...
var yod_runScript_RT = function() {
  ...
  yodHailTweetBox = function(txa, txt) {
    ...
    if (txa) {
      txa.click();
      if (txt) txa.val(yodcleanSpace(unescape(txt)));
      txa.mouseup().focus();
    }
  }

  yodShowTweetBox = function(s,c,yodrt) {
    var content = "@" + s + ": " + yodcleanSpace(unescape(c)).replace(/%yod%/ig, "");
    if (typeof twttr !== 'undefined') {
      ...
      // Must be delayed, otherwise it doesn't work
      setTimeout(function() {
...
.yodLegend legend{margin:auto!important;line-height:inherit!important;font-size:12px!important;cursor:pointer!important;font-weight:bold!important;text-align: center!important; padding: 0 5px!important; width: auto!important;border:none !important;}\
...
#ip_yodRTOpt_RTTxt {box-shadow:none!important;-webkit-box-shadow:none!important;-moz-box-shadow:none!important;border:1px solid #CCC!important;width:50px!important;padding:0 3px!important}\
...
function watchreply(e) {
  ...
  if (elwr = c1('.//*[contains(@class,"js-tweet-text")]', p)) {
    ...
    a.addEventListener('click', function(){
      var txt = txa.value.replace(/^(reply[^@]+)/i, "").trim();
      txt = txt + ": " + elwr.textContent.trim();
      HailTxtBox(txa.id, txt);
      return false;
    }, false);
    var div3 = document.createElement("div");
    div3.appendChild(a); div2.appendChild(div3);
...
function yod_goDiag(e) {
  ...
  if (elx.tagName) {
    ...
    if (target) {
      ...
      if (el = g("yodRTEmo")) {
        var el2 = c1('.//legend', el);
        el2.addEventListener('click', function(){
          toggleShow(this.nextElementSibling, el.id);
          return false;
        }, false);
      }
    ...
    document.addEventListener('DOMNodeInserted', function (event) {
      ...
      if (cname = elmt.className) {
        if (
          ...
          yod_frun++;
          yod_render();
        } else if (/js-conversation-replies/.test(cname)) {
          yodInlineReply(elmt);
          yod_render();
        } else {
          // User Gallery
