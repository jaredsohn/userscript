...
wparent.location.href.indexOf('ads.php') != -1
	...
	div.css({zIndex:100000000,textAlign:'center',padding:5,position:'fixed',width:299,height:20,background:'#AFFFAF',border:'2px solid green',bottom:10,right:10})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px arial',zIndex:1000000000,textAlign:'left',padding:5,position:'fixed',width:299,height:50,background:'#AFFFAF',border:'2px solid green',bottom:54,right:10})
	jQuery.ajax({
		dataType: 'jsonp',
			success: function(res){
				$('.titlePTCJungle').text(res[0].post_title).after(res[0].post_content)
			}
	})
	div2.append(
		$('<h3 class="titlePTCJungle">').text('PTC - XXI')		
	)	
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			if(typeof adsWind == 'undefined')
				adsWind = wparent.open(arr[ctr].href,"xxi","width=100,height=50,top=1,left=1");
			else	
...
if(wparent.location.href.indexOf('progrid.php') != -1){
	...
	var clickNum = ctr + 1 ;
	var loading = 0;
	div.css({zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:299,height:20,background:'#AFFFAF',border:'2px solid green',bottom:10,right:10})
		.text('');
	$('html').css({position:'relative'}).append(div);
