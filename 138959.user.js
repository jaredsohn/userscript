// ==UserScript==
// @name          JV Carton
// @namespace     http://perdu.com/jvcarton
// @description   Le carton pour tous !
// @include       http://www.jeuxvideo.com/profil/*
// @version       1.0
// ==/UserScript==

var nbPoints = 0;
var urlForm = 'http://www.jeuxvideo.com/cgi-bin/admin/addavis_new.cgi?jeu=2';
...
// Récupérer le token qui servira à supprimer l'avis.
...
function recupererToken()
{
	...
	xhr.onload = function()
	{
		...
		// A-t-on-déjà un avis qui peut servir pour le déboost ?
		
		if(resp.indexOf('rel="avis ') !== -1)
		{
			var dataSuppr = 'donnees=avis+' + resp.split('rel="avis ')[1].split('"')[0];
			dataSuppr += '&hash_cdv=' + resp.split('hash_cdv="')[1].split('"')[0];
			dataSuppr += '&time_cdv=' + resp.split('time_md5="')[1].split('"')[0];
			textNode.nodeValue = 'Cartonnage...';
			supprimerAvis(dataSuppr);
		}
		else
		{
			envoyerAvis();
...
// Fonction récursive pour supprimer un avis à l'infini.

function supprimerAvis(dataSuppr)
{
	var xhr = new XMLHttpRequest();
	xhr.open('POST', 'http://www.jeuxvideo.com/profil/ajax_contrib.php', true);
	...
	xhr.onreadystatechange = function()
	{
		if(xhr.readyState === 1)
		{
			...
			// setTimeout pour éviter le freeze du navigateur.
...
if(document.getElementById('msg_visible') !== null)
{
	...
	var textNode = document.createTextNode('Cartonner');
	...
	lienBooster.onclick = function(e)
	{
		...
		alert('Si vous quittez ou rechargez cet onglet, ou bien si vous vous déconnectez ou vous changez de compte, le cartonnage s\'arrêtera.');
		
		ongletBooster.removeChild(lienBooster);
		textNode.nodeValue = 'Initialisation...';
