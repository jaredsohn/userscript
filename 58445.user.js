// ==UserScript==
// @name          inYOf4ceBook 2
// @namespace     SwornPacifist
// @description   Displays the pictures larger when you roll over smaller ones on facebook.com, with the image text title displayed below
...
// Further modification by SwornPacifist.
          ...
          globalTimer = window.setTimeout(
            function (thisImage) {return function(result) {
              ...
              if (title) {
              	var strTitleHTML = "";
              	title = trim(title);
              	if (title.length > 0) strTitleHTML = '<p style="text-align: center">' + title + "</p>";
              	newDiv.innerHTML = "<a href="+thisImage.parentNode.href+"><img title = '"+title +"' style = 'max-height: "+ (parseInt(window.innerHeight) - 40) +"px; display:block; cursor: crosshair;' src='" + fullsize + "'></a>" + strTitleHTML + "</div>";
              }
              else {
              	newDiv.innerHTML = "<a href="+thisImage.parentNode.href+"><img style = 'max-height: "+ (parseInt(window.innerHeight) - 40) +"px; display:block; cursor: crosshair;' src='" + fullsize + "'></a></div>";
              }
...
// *************
// The following code generously provided by 
