...
function send(uid, num, gift) {
    if(num && keepGoing) {
        ...
        alert('Semua gift telah terkirim.. Di refresh donk..');
...
function receive(uid, num, gift) {
    if(num--)
        $.get("http://apps.facebook.com/castle_age/army.php?act=acpt&rqtp=gift&uid=" + uid, function() {
            if(display)
                get_sub_panel('ca_gift').text(num + " sisa gift untuk dikirim.");
            send(uid, num, gift);
        });
}
...
function gift() {
    var ca_gift = get_sub_panel('ca_gift'),
        ...
        inputID    = $("<input></input>"),
        buttonSub  = $("<button >Send!!>"),
        gifts      = ['Random Soldier', 'Mystery Artifact - Sophias Battlegarb', 'Volcanic Egg - Shield of Dante', 'Mystery Ice Relic 2 - Ragnarok, The Ice Elemental', 'Mystery Earth - Earth Orb', 'Mystery Relic 2 - Drake Helm', 'Mystery Item - Battle Of Dark Legion', 'Dragan Set - Dragan Blade', 'Mystery Relic - Serpentine Shield', 'Mystery Treasure - Poseidons Horn', 'Serpent Egg - Sea Serpent', 'Dragon Egg - Dragon', 'Mystery Druid Item - Whisper Bow', 'Mystery Armor - Golden Hand', 'Mystery Frost Item - Frost Tear Dagger', 'Mystery Artifact - Morning Star', 'Mystery Armor 2 - Mystic Armor', 'Mystery Frost Relic - Glacial Blade', 'Mystery Fire Relic - Ring Of Ember',],
    ...
    buttonSub.click(function() {
        $("<div></div>").load("party.php span.linkwhite a", function() {
            if(/id=(\d+)/.test($(this).children().attr("href"))) {
                ...
                ca_gift.html("Menyiapkan gifts...");
                display = true;
            } else {
                alert("Error.. Silahkan coba sekali lagi..");
                remove_sub_panel('ca_gift');
            }
        });
    });

    ca_gift.html("Pilih jenis gift dan jumlah..<br/>");
...
function do_alch(form, num) {
    if(num > 0 && form.size()) {
        ...
        if(display)
            get_sub_panel('ca_alch').text(num + " item lagi..");
        ...
        $("<div></div>").load("alchemy.php div.results span.result_body, #"+id, data, function(responseText, textStatus, XMLHttpRequest) {
            ...
            if(/You have created/.test(txt)) {
                ...
                alert('Kurang bahan untuk combine ' +num+ ' item..');
                remove_sub_panel('ca_alch');
            }
        });
    } else {
        alert('Semua item selesai di combine..');
        remove_sub_panel('ca_alch');
    }
}
...
function alchemy() {
    ...
    divs.load("alchemy.php div.statsT2 table div.alchemyRecipeBack", function(responseText, textStatus, XMLHttpRequest) {
        ...
        buttonSub.click(function() {
            ...
            ca_alch.html("Menyiapkan bahan combine.. Nanti dikasih tau kalo sudah selesai..");
            display = true;
        });
        ca_alch.html("Pilih item dan jumlah..<br/>");
    ...
    GM_xmlhttpRequest({
        ...
        onload : function(response) {
            ...
            if(currentVersion > remoteVersion) {
                if(confirm("Script baru udah ada.. mau update?")) {
