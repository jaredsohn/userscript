...
// @version     1.8.7 redesigned by Atomic2
// ==/UserScript==
 ...
 *********************/
if (init) {
    var sCurrentVersion = "1.8.7";  //Version number with which we need to run the update fu
    ...
    //Styles
    var cssStyle = "";
cssStyle += "#ttq_tasklist, #ttq_history {position:absolute; background-color: #ffffff ;  color:#000000; border-left:1px dashed #C0C0C0 ; border-right:1px dashed #C0C0C0 ;padding:1px 7px; z-index:100;}"; 
cssStyle += "#ttq_history {background-color:#ffffff;border-left:1px dashed #C0C0C0 ; border-right:1px dashed #C0C0C0 ;;max-width:300px; }";



cssStyle += ".ttq_history_row {padding:3px 1px;}";
cssStyle += ".ttq_village_name {font-weight:bold;font-size:110% ;color:#71D000;}";
cssStyle += ".ttq_draghandle { font-size: 103%; font-weight:bold; color:black;text-align:center;background-image:url(img/un/a/c2.gif);background-repeat:repeat-x; border:1px solid #C0C0C0;height:20px;repeat-x;}";
cssStyle += ".ttq_time_village_wrapper {font-style:bold; font-size:80%; display:block;}";
cssStyle += ".ttq_close_btn {float:right; padding:1px 2px; color:white; margin:-5px -15px 0 0;}";

cssStyle +=	"#timerForm {padding:10px 20px; }";
cssStyle += "#timerform_wrapper {position:absolute;  width:550px !important; margin:0; background-image:url(http://img692.imageshack.us/img692/4007/ttqneu.png);background-repeat:no-repeat; background-position:center center;background-color:#FFFFFF; color:black;font-weight:bold; border:1px #59D72F solid;  z-index:100; -moz-border-radius:8px;}";
cssStyle += "#timerform_wrapper p {}";
cssStyle +=	"#ttq_message {position:absolute; z-index:100; border:1px solid #71D000; padding:10px 20px; color:black; width:335px}";
...
cssStyle += "#flushAllTask{cursor:pointer;font-weight:bold;border-top:1px dashed #000000;}";
	...
	* -> Please post all translations here: http://userscripts.org/topics/27738
	...
	case "hk":
		...
		aLangMenuOptions = ["Travian ", "使用伺服器的時間", "使用本地時間", "設定你的種族", "工作記錄", "檢查更新", "\n要保留多少項工作記錄？\n(要停用工作記錄請輸入 0 ) \n現在的設定值︰", "\n你在這個伺服器是甚麼種族？\n(0 是羅馬, 1 是條頓, 2 是高盧)\n現在的設定值︰"];
		break;
    ...
    case "ro":  // by Atomic
	    aLangBuildings = ["", "Cherestea", "Puţ de lut", "Mina de fier", "Lan de grâu", "Fabrica de cherestea", "Fabrica de caramidă", "Topitorie", "Moara", "Brutărie", "Hambar", "Grânar", "Fierărie", "Armurier", "Arena", "Primărie", "Adunare", "Târg", "Ambasada", "Cazarma", "Grajd", "Atelier", "Academie", "Beci", "Casa de cultură", "Vila", "Palat", "Trezorerie", "Oficiu de comerţ", "Cazarma extinsă", "Grajd extins", "Zid", "Metereze", "Palisada", "Arhitect", "Berarie", "Temnita", "Reşedinţa eroului", "Hambar extins", "Grânar extins", "Minunea Lumii"];
	    aLangTasks = ["Clădire", "Upgrade", "Atacă", "cercetează", "Instruieşte", "petrecere /"];
	    aLangStrings = ["Construieşte mai târziu", "Upgrade mai târziu", "Atacă mai târziu","Cercetează ulterior", "Programează acestă actiune pentru mai târziu", "Construcţia pornită ", "A fost incercată cu rezultate necunoscute", "Nivel", " Nu poate fi construită","Upgrade nu este posibil", "Actiunea a fost programată", "Producţia curentă:","Programarea acţiunii nu e posibilă", "Programarea acţiunii nu este disponibilă!", "Acţiuni Programate", "Şterge", "Trimite mai târziu", "Nici o unitate a fost selectată.","Trupele tale au fost trimise la :", "Trupele tale nu au putut fi trimise la:", "Suport","Atac", "Raid", "Catapultează pe la","Aleator", "la", "sau după","secunde", "minute", "ore","zile", "Spionează resurse si unitaţi", "Spionează fortificaţii si trupe", "plecate", "Atacul nu poate fi programat! Destinatia nun a fost selectată.", "pe locul (ID) nr.", "Sortează pe:", "«tipul» ", "«timp» ", "«ţinta» ", "opţiuni ", "«sate» " ,"Acţiuni derulate","închide","opţiuni ", "Cercetarea pornită","îmbunatăţeste ulterior","Cercetarea nu este posibilă", "Antrenează mai târziu", "trupe", "Antrenează","Antrenamentul pornit"," Antrenamentul nu este posibil", " ține mai târziu","ori, ","cu pauze de: ","Închide lista","Au fost trimise","Programare automată","Destinaţia","Nu au fost selectate resurse.","Introduceţi numele satului sau coordonatele","Curierii au fost trimişi","Mai multe informatii"," la: "];
		aLangTroops[0] = ["Legionar", "Pretorian", "Imperian", "Equites Legati", "Equites Imperatoris", "Equites Caesaris", "Brerbec", "Catapulta", "Senator", "Colonist", "Erou"];  //Romans
		aLangTroops[1] = ["Măciucar", "Lancier", "Executor", "Spion", "Paladin", "Cavaler teuton", "Berbec", "Catapulta", "Căpetenie", "Colonist", "Erou"];  //Teutons
		aLangTroops[2] = ["Scutier", "Pedestru", "Iscoada", "Călareț Fulger", "Druidieri", "Tarabostes", "Berbec", "Catapulta", "General", "Colonist", "Erou"];  //Gauls
	    aLangTransModel= ["MixedModel", "FoodOnly", "ResourceOnly", "FixedModel"];
		aLangStringt   = ["RecruitHome", "AutoTransport", "AutoTrade", "Businessmen", "From", "To", "TransportBy", "RepeatTime", "minutes", "seconds", "TransportTimes:", "times", "The Time Of Next Transport:", "has been added.", "has been started.", "has not enough businessmen.", "transport error.", "Schedule later after", "Corn Resource: ", "Autotrade has done.", "Autotrade failed.", "Target Resource:", "Wood", "Clay", "Iron", "Crop", "Division Equal", "Division Model:", "WARN: Chang source village is not recommend.", "Keep ZERO if you want to division equal among wood, clay and iron. fill the number otherwise.", "This is the server time.", "This is your local time.","Force Trade On Scheduled Time (no delay check)"];
		aLangUpdate = ["Your version is greater then current version: ", "Your current version is up to date: ", "There is an update available", "Do you want to update now? "];
		aLangMenuOptions = ["Travian ", "Use server time", "Use local time", "Set your race", "Task History", "Check Updates", "\nHow many past tasks do we keep in history?\n(Type 0 to disable task history.) \nCurrently: ", "\nWhat is your race on this server?\n(Type 0 for Romans, 1 for Teutons, 2 for Gauls.) \nCurrently: "];
...
function createResearchLinks() {
	...
	var Imgs = xpath("id('content')/form/table[1]/tbody/tr/td[1]/table/tbody/tr[1]/td[1]/img | //form/table/tbody/tr/td/table/tr/td/img | id('build')/table[1]/tbody/tr/td[1]/div/img");
    ...
    if (Levels.snapshotLength > 0){
  		...
  		var re = new RegExp("\\(" + aLangStrings[7] + "\\s([0-9]{1,3})\\)$", "i");
  		var aLevel = thisLevel.match(re);
  		var iLevel=aLevel[1];
		}
		if(iLevel == 20) { continue; }
...
function displayTimerForm(iTask, target, options, timestamp) {
	...
	oTitle.innerHTML = sLinkClose + "&nbsp;";
	oTitle.style.margin="10px 20px 0";
	...
	//[QFIX-F] makeDraggable changed to "title area" - now textboxes function normally
...
function onLoad() {
	...
	oTTQTime.innerHTML = " [TTQ v" + sCurrentVersion + " - Script load: <span style=\"font-weight: bold\">" + processtime + "</span> ms] ";
