...
// @namespace      Syrnia
// @author         Ciaran (friendz list by Luxember)
...
//segment for friends display
var Lux = [
'<br><font color="lime"><b>Luxember: <b>',
];
...
function addChatButtons() {
	...
	for (i=0;i<unicodeSymbols.length;i++) {
		...
		var insSymLnk = document.createElement('a');
		insSymLnk.innerHTML = Lux;
		DisCht.parentNode.insertBefore(insSymLnk,DisCht.previousSibling);
	}
		for (i=0;i<Lux1.length;i++) {
		var insSymLnk = document.createElement('a');
		insSymLnk.setAttribute('onclick','chatAppendText(\'' + 'Luxember@' + '\'); return false;');
		insSymLnk.setAttribute('href','#');
		insSymLnk.innerHTML = Lux1;
		DisCht.parentNode.insertBefore(insSymLnk,DisCht.previousSibling);
	}	
//"brk1" needs to be shown twice due to display problems
		for (i=0;i<brk1.length;i++) {
		var insSymLnk = document.createElement('a');
		insSymLnk.innerHTML = brk1;
		DisCht.parentNode.insertBefore(insSymLnk,DisCht.previousSibling);
	}
		for (i=0;i<brk1.length;i++) {
		var insSymLnk = document.createElement('a');
		...
		for (i=0;i<Lux2.length;i++) {
		insSymLnk.setAttribute('onclick','enterWindow=window.open(\'http://www.syrnia.com/theGame/includes2/messages.php?p=messages&stuur=1&name=Luxember' + '&topic=\',\'Messages\',\'width=600,height=600,top=5,left=5,scrollbars=yes,resizable=yes\'); return false;');
		insSymLnk.innerHTML = Lux2;
		DisCht.parentNode.insertBefore(insSymLnk,DisCht.previousSibling);
	}
...
This script is MOSTLY Ciaran's and NebuneX's work, I have simply added a "friends list" to the script.
I was going to have it as a type of "patch" instead of resubmiting the entire code, but shall (hopefully) get time to do that later. 
-as such the other script may have to be uninstalled :(

Friends have to be added <b>into the script</b>. (search for "player name" in the script)

Again, huge thanks Ciaran and NebuneX for all the work you've put into ths script :)
