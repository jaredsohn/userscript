// ==UserScript==
// @name        Tiberium Alliances CustomFlunik Trekker AutoUpgrade
// @namespace   CustomFlunikTrekkerAutoUpgrade
// @description Intelligently uses the AutoUpgrade Feature For C&C Tiberium Alliances
// @include     http*://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @author      RobertT Flunik dbendure KRS_L Trekker9876
// @version     3-20-2013-e
// ==/UserScript==

/*
NOTE: If conditions match for items 7-15 then script will wait until there is enough resources to perform the upgrade 
	unless tiberium exceeds 10%. When tiberium exceeds 10% it will check the conditions in order until an upgrade is found.
...
4. Upgrade any production buildings < level 12 
5. if crystal is more than 50% full and your offence is maxed out try to upgrade CC 
6. if #4 is true but you cant upgrade your CC (or you dont have one) and defence is maxed out try to upgrade DHQ 
7. if your CY < level 26 upgrade CY 
8. 
...
14. if repair time > 5 hours upgrade repair structure 
15. if repair time > 4 hours and repair structure level < CC and base level < 20 upgrade repair structure 
16. if lowest building level is below (base level+5) and we have at least 20% tiberium upgrade lowest building 
17. if lowest silo is below base level and we have at least 5% tiberium upgrade lowest silo 
...
19. if tiberium is > 5% and nothing was upgraded above then upgrade lowest level Silo/Harvester/Refinery/Power Plant/Accumulator to base level +5
...
Original Flunik tools would upgrade buildings randomly. I have tried to make the upgrading more intelligent. 
*/
...
//							bottom: 80
						...
						get_IsFull: function (city, type) {
							if (city.GetResourceCount(type) < (city.GetResourceMaxStorage(type)*0.10)) {
								return false;
							} else {
								return true;
							}
...
//								FLUNIK: The Silo building has a level of: 15 (program):234
								...
								if (currentcrypct>50) {
									//			console.debug("FLUNIK: Crystal is near full - checking if CC or DHQ upgrades is required");
									var tryDHQ=true;
									if (CC != null) {
										var tryDHQ=false;
								...
								if (CY != null) { 
									if (CY.get_CurrentLevel() < 26) {
										if (CY.CanUpgrade()) {
											//console.debug("FLUNIK: %d The CY building level %d is lower than 25 - Upgrading",cityname, CY.get_CurrentLevel());
											console.debug(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Upg: CY<26");
											CY.Upgrade();
											return;
										} else {
											var infolineSkipped=infolineSkipped+"CY<26,";
								...
								*/
								...
								if (REPAIR != null) {
									if (maxRT>18000) { // Always try to get time below 5:50 hours (21000 seconds)
										//console.debug("FLUNIK: %d Repair info in seconds: Max %d AIR %d VEH %d INF %d",cityname, maxRT, airRT, vehRT, infRT);
										if (REPAIR.CanUpgrade()) {
											...
											var infolineSkipped=infolineSkipped+repairname+" "+maxRT+">18000,";
								...
								if (lowestbuilding != null) { 
									if (lowestbuildinglevel<baselvl+5) {
										//console.debug("FLUNIK: %d Default upgrade - lowest building is %d level %d",cityname, lowestbuildingname, lowestbuildinglevel);
										console.debug(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Upg: lowest<0.8*baselvl "+lowestbuildingname+" lvl: "+lowestbuildinglevel)
										lowestbuilding.Upgrade();
										return;
									}
								}
								...
								if (lowestsilo != null) { 
									if (lowestsilolevel<baselvl + 5) {
										//console.debug("FLUNIK: %d Default upgrade - lowest building is %d level %d",cityname, lowestbuildingname, lowestbuildinglevel);
										console.debug(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Upg: lowestsilo<baselvl&tib>5 lvl: "+lowestsilolevel)
										lowestsilo.Upgrade();
										return;
									}
								}
...
//										console.debug(FlunikTools.Main.prototype.print_r(mlist));  
								...
								if (lowestbuilding != null) { 
									if (lowestbuilding.CanUpgrade() && currenttibpct>5) {
										//console.debug("FLUNIK: %d Default upgrade - lowest building is %d level %d",cityname, lowestbuildingname, lowestbuildinglevel);
