...
function httpAjaxRequest(options, callback) 
{
    ...
    try {
        ...
        var updateFromJSON = function(data) {
            ...
            options.success && options.success(data, options.id);
        };
        var updateFromHTML = function(response) {
            ...
            options.success && options.success(response, options.id);
        };
        // send request
        httpRequest({
            url       : options.url,
...
function PageGift() 
{
    ...
    function publishLinks(Links, target_id, message) {
        ...
        if (Links.length > 1) {
            $.each(Links, function(index, gift) {
                var idx = index+1;
                var link_id = '#'+((idx>9)?'':'0')+idx+' '+Util.formatText(gift.title);
                properties[link_id] = {
            ...
            publish_config = {
                'target'      : target_id,
                'message'     : message,
                'name'        : '{*actor*} has a ' + Util.formatText(Links[0].name),
                'description' : '{*actor*} wants to gift you a ' + Util.formatText(Links[0].name) + ', be sure to collect it now.',
                'pic'         : Links[0].img,
    ...
    function filterWar(htmlText) {
        ...
        try {
            ...
            if ( options.get('warUseNameFilter') && String(options.get('warNameFilter')).length > 1 ) {
                var sn = new RegExp(String(options.get('warNameFilter')).replace(/,/g,'|'),'i');
                if ( !sn.test(text) ) {
...
function Battlefieldv2() 
{
    ...
    var CSOpponent = function() {
        ...
        this.anchor = function() {            
            if (!this.isNPC) {
                return this.clan() + ' <a href="'+MW.getProfileLink(this.id)+'" target="_black">'+this.name+'</a>';
            }
            else {
    ...
    var CSStolenIce = function(fight_data) {
        ...
        this.name     = $('<div>'+fight_data.thief_name+'</div>').text();
    ...
    function isValidFamily(opp) {
        if (!opp.clanId || !opp.clanName) {
            return true;
        }
        if ( PlayerList.clanList.exists(opp.clanId) ) {
            return ( options.get('clanListUsage') !== 0 );
        }
        var text = String(options.get('clanTagFilterVal'));
        var clearedTags = new Array();
        
        if ( text.length < 1 || !(options.get('skipFilteredClans') || options.get('addToClanList')) ) {
            return true;
        }
        $.each(text.split(/\n/), function(index, tag) {
    ...
    function genEnemyListDom() {
        ...
        PlayerList.each(function(index, pl) {
            ...
            .append(c$('div', 'class:update_icon').append(
                ...
                .html(( pl.title?pl.title+'<br>':'' )+ pl.clan() + ' ' + pl.anchor() + '<br>Level: ' + pl.level)
    ...
    function addStolenIceToLog(thief, bAuto) {
        var message = 'ICE STOLEN BY: <a href="%HREF%" target="_black">'+thief.name+'</a>';
        ...
        addToLog( message.replace('%HREF%',MW.getProfileLink(thief.id))+'<br>'+result, 'iced', thief.pic, thief.id );
    }
    ...
    function AttackNewOpponent() {
        ...
        if (!isValidFamily(PlayerList.current)) {
            AttackNewOpponent();
            return;
        }
        // check faction
    ...
    function preAttack() {
        ...
        if (op('useAttackCount') && op('maximumAttacks') < opp.fights_for_ice) {
        ...
        if (op('usePowerAttack') && opp.pwratk) {
            ...
            if ( op('useRapidFire') ) {
                if ( opp.fights_for_ice < op('rapidFireWhen') && opp.fights_for_ice > 4 ) {
        ...
        switch(true) {
            ...
            case ( bDoManualHeal || (op('useHeal') && (!op('noHealIfAttacking')||opp.fights===0) && fs.health < op('healWhen')) ):
                if (!op('noHealIfLowSta') || fs.stamina >= op('minStaForHeal') ) {
