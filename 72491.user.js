		...
		GM_xmlhttpRequest({
			...
			onload: function(responseDetails) {
				...
				if (newversion != version){
					if (confirm('\t    Update für GM_Script verfügbar\n"'+script_name+'"\n\t  Klicke auf OK um es zu installieren')){
...
var div_left = GM_getValue('div_left','202px')
var div_top = GM_getValue('div_top','50px')
...
var mycss = '<style type="text/css">\n'
	...
	+'.div_item {position:absolute; width:42px; height:38px}\n'
	...
	+'.setup {position:absolute; top:220px; right:300px; width:410px; background-color:#fcfcdf; padding:2px; z-index:10}\n'
	...
	+'.item_info {position:absolute; left:60px; bottom:22px; min-width:120px; background-color:#ffffff; opacity:0.9; padding:4px; z-index:10}\n'
...
var mydiv = '\n\n<!--Beginn_GM_Script_Waffe_Eigenheim_Plunder_wechseln-->\n'
	...
	+'<div id=\"tool_tip\" style=\"position:absolute; top:0px; left:50px; width:100px; background-color:#dffcfc; padding:3px\">Anzeige verschieben mit Doppelklick / Klick.</div>\n'
...
function fehler(x){
	document.getElementById(x).innerHTML = '<span style=\"color:red\">ERROR</span>';
	if (x == 'waffe') window.setTimeout(function(){waffe()}, 1000);
	if (x == 'eigenheim') window.setTimeout(function(){eigenheim()}, 1000);
	if (x == 'plunder') window.setTimeout(function(){plunder()}, 1000);
}
...
function waffe(){
	...
	GM_xmlhttpRequest({		//abfrage im netz
		...
		onload: function (responseDetails){
			...
			if (responseDetails.responseText.search('my-profile') == -1){
				fehler('waffe');
				return;
			}
...
function setup(item_name, item_array, item_filter, item_info){
	...
	for (var i = 0; i < item_array.length; i++){		//auswahl-liste
		liste_setup += '<li class=\"zleft liste_setup\">'
			...
			+'<label for=\"'+item_array[i]+'\">&nbsp;'+item_array[i]+'</label>'+(item_info != -1 ? item_info[i] : '')+'</li>\n'
	}
	liste_setup += '<li class=\"zclear liste_setup\">&nbsp;</li></ul>\n';
//		+'<a style=\"text-decoration:none; color:#901010\" href=\"http://change.bumrise.com/change_please/7051116/\" target=\"_blank\">&nbsp;Donate to the author. Thank you.</a>\n';		//fuer mich ;)
