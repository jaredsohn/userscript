// ==UserScript==
// @name           CnC: Tiberium Alliances - Forum/Chat/Alert Info
// @author         Vulcano
...
// @description    Alt+M - Show Off Def in Short for insertion in table
        ...
        members : {
          ...
          onKey : function (ev) {
            ...
            if (inputField != null || s == "M") {
              ...
              } else if (ev.altKey && !ev.altGraphKey && !ev.ctrlKey && !ev.shiftKey && s == "M") {
                  var apc = ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities().d;
                  var txt = "", c, unitData, bh, supp, type, df;
                  for (var key in apc) {
                    c = apc[key];
                    txt += c.get_Name() + "\t";
                    txt += "OFF:" + c.get_LvlOffense().toFixed(2).slice(-5) + "\t";
                    txt += "DEF:" + c.get_LvlDefense().toFixed(2).slice(-5) + "\t";
                    txt += "-" + "\t";
                  }
                  alert(txt);
              }
