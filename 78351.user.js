...
// @homepage    https://userscripts.org/scripts/show/78351
...
// == [CSS] =====================================
var CSS = [
  '#entries .entry-title img { border: none; vertical-align: text-top; }',
  '#entries .entry-title span a { margin: 0 3px; }',
  '#entries .entry-body .clear { font-size: inherit !important; height: auto !Important; width: auto !important; }',
  '.gm_fullfeed_loading {  }',
...
FullFeed.prototype.request = function(m) {
  ...
  var opt = {
    ...
    headers: {
      'User-Agent': navigator.userAgent + ' Greasemonkey (GRFFmod)'
    },
    onerror: function() {
      state = null;
...
FullFeed.checkScroll = function() {
  ...
  scrollInterval = window.setTimeout(function() {
    var e1 = $id('viewer-entries-container');
            ...
            if (!bCache) {
              ...
              try {
                ...
                if (!bList) {
                  ...
                  localStorage.setItem('GRFFmod_userCacheListAP', JSON.stringify(userCacheListAP));
                }
              } catch(er) {}
            }
          }
...
FullFeed.removeCache = function() {
  if (window.confirm(loc[16])) {
    ...
    localStorage.removeItem('GoogleReaderFullFeed_userCacheList');
    localStorage.removeItem('GoogleReaderFullFeed_userCacheListAP');
    ...
    localStorage.removeItem('GRFFmod_userCacheList');
    localStorage.removeItem('GRFFmod_userCacheListAP');
...
FullFeed.createSettings = function() {
  ...
  if ($id('settings-button-menu')) {
    ...
    if (d1) {
      ...
      d2.innerHTML = '<div id="gm_fullfeed_settings-menu2child" class="goog-menuitem-content">Full Feed Mod ' + loc[21] + '</div>';
      d1.appendChild(d2);
    }
...
FullFeed.registerSocialIcons = function() {
  ...
  var inter, repeat = 0, check = function() {
    ...
    if (f) window.clearTimeout(inter);
  };
...
var get_active_item = function() {
  ...
  try {
    item.url =  getFirstElementByXPath(exp).href;
...
var get_active_feed = function() {
  ...
  feed.url = ''; feed.title = '';
  try {
    var est = getFirstElementByXPath('id("current-entry")//a[contains(concat(" ", @class, " "), " entry-source-title ")]');
    if (est) {
  ...
  if (flag === 'full') {
    if (!bCache && itemSiteInfo.length > 0) {
      ...
      if (!bList) {
        ...
        try {
          localStorage.setItem('GRFFmod_userCacheList', JSON.stringify(userCacheList));
        } catch(er3) {}
      }
    }
  }
    ...
    for(var i=0; i<rep.length-1; i=i+2) {
      if (rep[i] && new RegExp(rep[i]).test(c.itemURL)) {
        if (rep[i + 1] && /^http/.test(rep[i + 1])) {
          ...
          try {
            var u = getElementsByXPath('id("current-entry")//div[@class="item-body"]//a');
            if (u) excludeUrl(u);
          } catch(er) {}
        }
        break;
...
var loadingStyle = function(flag, elm) {
  ...
  if (e2 && e2.id !== 'current-entry') {
    if (flag === 'remove') {
      var s = '//div[contains(@class, "';
      var e = getElementsByXPath(s + s1 + '")]', $id('entries'));
      ...
      e = getElementsByXPath(s + s3 + '")]', $id('entries'));
      if (e) e.forEach(function(i) {
        removeClass(i, s3);
      });
      e = getElementsByXPath(s + s4 + '")]', $id('entries'));
      if (e) e.forEach(function(i) {
        removeClass(i, s4);
      });
      e = getElementsByXPath(s + s5 + '")]', $id('entries'));
      if (e) e.forEach(function(i) {
        removeClass(i, s5);
      });
      return;
  ...
  if (!e2) return;
  if (hasClass(e2.firstChild, 'card-common')) {
    e3 = e2.firstChild;
    if (flag === 'add') {
  ...
  } else if (hasClass(e2.firstChild, 'collapsed')) {
    if (flag === 'add') {
      if (e1) addClass(e1, s1);
...
var debugLog = false;
var itemSiteInfo = [];
var messageInterval, scrollInterval;
...
var userCacheList = FullFeed.getCache('userCacheList');
var userCacheListAP = FullFeed.getCache('userCacheListAP');
...
if (typeof GM_registerMenuCommand === 'function') {
  ...
  GM_registerMenuCommand('Google Reader Full Feed Mod - delete cache', function() {
...
document.addEventListener('keyup', function(e) {
  ...
  if (e.target.id === 'gm_fullfeed_settings-general_key') {
    ...
    if (e.altKey && e.ctrlKey && e.shiftKey) {
      ...
      el2 = getElementsByXPath('id("current-entry")//span[@class=" gm_fullfeed_checked"]', document);
      if (el2) {
...
if ($id('viewer-entries-container')) {
  $id('viewer-entries-container').addEventListener('scroll', function() {
...
function relativeToAbsolutePath(htmldoc, base) {
  ...
  Array.prototype.forEach.call(htmldoc.getElementsByTagName('a'), function(a) {
    if (a.getAttribute('href'))
      a.href = _rel2abs(a.getAttribute('href'), top, current, base);
  });
  Array.prototype.forEach.call(htmldoc.getElementsByTagName('img'), function(img) {
    if (img.getAttribute('src'))
      img.src = _rel2abs(img.getAttribute('src'), top, current, base);
  });
}
...
function message(mes, dur, typ) {
  var mac = $id('message-area-container'), ma = $id('message-area');
  if (!mac || !ma) return;
  var type = 'jfk-butterBar-' + ((typ) ? typ : (dur < 0) ? 'promo' : 'info');
  var dura = (dur < 0) ? 300000 : (!dur) ? 1500 : dur;
  ma.innerHTML = mes;
  removeClass(ma, 'jfk-butterBar-error');
  removeClass(ma, 'jfk-butterBar-info');
  removeClass(ma, 'jfk-butterBar-promo');
  removeClass(ma, 'jfk-butterBar-warning');
  addClass(ma, type);
  addClass(ma, 'jfk-butterBar-shown');
  window.clearTimeout(messageInterval);
  messageInterval = window.setTimeout(function() {
    removeClass(ma, 'jfk-butterBar-shown');
    if (typ || dur < 0) removeClass(ma, type);
  }, dura);
}
