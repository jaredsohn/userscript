...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  .xtabBR {padding-right: 5px; border:none; background:none;}\
    ...
    .ptChatAlliance {font-weight:bold}\
	  .ptChatScripter {color:#A56631; font-weight:bold;text-shadow: 1px 1px 1px #AAA; background-color:#E0E0E0;}\
    ...
    .ptChatNotice {color: #000; background: #CCF; font-weight:bold; text-decoration:blink;}\
	  .ptChatOfficers {color:#000; font-weight:bold; background-color:#E0E0E0;}\
...
// "Report No: 5867445"  --->  see unsafeWindow.modal_alliance_report_view()
 ...
 chatDivContentHook : function (msg){
	...
	var scripters = ["7552815","10681588","1747877","2865067","10153485","15182839","1550996","1617431819","9688786","9751486"];
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    this.cont = div;
    this.cont.innerHTML = '\
      <DIV element_class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD element_class=xtab width=100 align=right>Search for: </td><TD>\
      <SELECT id="srcType">\
      ...
      m = '<TABLE><TR valign=middle><TD element_class=xtab width=100 align=right>Center: &nbsp; X: </td><TD element_class=xtab>\
        <INPUT id=srchX type=text > &nbsp; Y: <INPUT id=srchY type=text > &nbsp; <SPAN id=spInXYold></span>\
        </td></tr><TR><TD element_class=xtab align=right>Max. Distance: </td><TD style="width: 200px;" element_class=xtab><INPUT id=srcDist size=3 value=10 /></td>\
        <td></td>\
        </tr>';
      m += '<TR><TD element_class=xtab></td><TD element_class=xtab><INPUT id=srcStart type=submit value="Start Search" /></td></tr>';
      m += '</table>';
...
clickedPlayerDetail : function (span, uid){
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ officerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 element_class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Might: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 element_class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; Cities: '
          + u.cities +' &nbsp; Population: '+ u.population +'</td></tr><TR><TD></td><TD>Provinces: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
...
//End edit city search
  ...
  srchTypeHandler : function (){
    ...
    var m = '<CENTER><B>Search for '+ typeName +'<BR>\
        ...
        <DIV element_class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD element_class=xtab colspan=10><B>LIST OPTIONS:</b><BR></td></tr>';
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     m += '<TR><TD element_class=xtab align=right>Min. level to show:</td><TD element_class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD element_class=xtab align=right>Max. level to show:</td><TD element_class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD element_class=xtab align=right>Wilderness Type:</td><TD element_class=xtab align=right>\
            Woodlands<INPUT id=woodWild type=CHECKBOX'+ (Options.woodWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD element_class=xtab align=right>Grassland/Lake<INPUT  id=foodWild type=CHECKBOX '+ (Options.foodWild?' CHECKED':'') +'></td>\
	       <TD element_class=xtab align=right>Mountain<INPUT id=mtnWild type=CHECKBOX '+ (Options.mtnWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD element_class=xtab align=right>Plain<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD element_class=xtab align=right>Hills<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD element_class=xtab align=right>Unowned Only:</td><TD element_class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD element_class=xtab align=right>Sort By:</td><TD element_class=xtab><SELECT id=filSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
			...
			<TR><TD element_class=xtab align=right>Show coordinates only:</td><TD element_class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
			</table></div><BR><SPAN id=srchSizeWarn></span>';
    } else {
        m+= '<TR><TD element_class=xtab align=right >Show:</td><TD element_class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
		...
		m+= '<TR><TD element_class=xtab align=right >Sort By:</td><TD element_class=xtab><SELECT style="width: 135px" id=filSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Might</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
        </select></td></tr>\
        <TR><TD element_class=xtab align=right >Show coords only:</td><TD element_class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
  ...
  clickedSearch : function (){
    ...
    m = '<DIV element_class=ptstat><TABLE width=100% cellspacing=0><TR><TD element_class=xtab width=125><DIV id=statSearched></div></td>\
        <TD element_class=xtab align=center><SPAN id=statStatus></span></td>\
        <TD element_class=xtab align=right width=125><DIV id=statFound></div></td></tr></table></div>\
    ...
    if (t.opt.searchType == 1){
      m += '<TR><TD element_class=xtab align=right>Wilderness Type:</td><TD element_class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Glassland/Lake', 3:'Woodlands', 4:'Hills', 5:'Mountain', 6:'Plain', 0:'ALL'}, Options.wildType );
      m += '</select></td></tr><TR><TD element_class=xtab align=right>Unowned Only:</td>\
			<TD element_class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD element_class=xtab align=right>Sort By:</td><TD element_class=xtab><SELECT id=filSortBy>\
				<OPTION value="level" '+ (Options.srcSortBy=='level'?'SELECTED':'')  +'>Level</option>\
	 ...
	 m += '<TR><TD element_class=xtab align=right>City Type:</td><TD element_class=xtab><SELECT id=filCities>';
	 ...
	 m += '<TR><TD element_class=xtab align=right>Sort By:</td><TD element_class=xtab><SELECT id=filSortBy>\
		   ...
		   <TR><TD element_class=xtab align=right>Min might to show:</td><TD element_class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD element_class=xtab align=right>Show coordinates only:</td><TD element_class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      ...
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
		 if (dat[i][12]) m += 'element_class="'+dat[i][12]+'"';
