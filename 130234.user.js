...
var voteId;
var npCleanup = '';
var adCleanup = new Array()
adCleanup[0] = 'http://a.alimama.cn/inf.js';
adCleanup[1] = 'http://pagead2.googlesyndication.com/pagead/show_ads.js';
adCleanup[2] = 'http://cpro.baidu.com/cpro/ui/c.js';
...
function is_seaMore(){
    $('ol.commentlist li p img').each(function(){
        ...
        $(this).click(function(event){
	        ...
	        t_showImgbox(getImgsrc($(imgList.get(curImg))),t_getNavimgInfo(false),false,false);
        });
    });
    $('div#sidebar div.hotcomment div.acv_comment img').each(function(){
        ...
        $(this).click(function(event){
            ...
            curImg = imgList.index($(this));
            t_showImgbox(getImgsrc($(imgList.get(curImg))),t_getNavimgInfo(true),false,true);
        });
    });
    
}

function getImgsrc(sImgItem){
	if(typeof(sImgItem.attr('org_src'))=='undefined'){
		return sImgItem.attr('src');
	}else{
		return sImgItem.attr('org_src');
	}
}
    ...
    if($(imgList.get(curImg)).parents('div.t_pagebox').length>0){
    	...
    	$('#isea_imgpage').show().click(function(){
	    	...
	    	if(curUrl.indexOf('#comments')>-1){
	    		if((curUrl.indexOf('pic#comments')>-1) || (curUrl.indexOf('ooxx#comments')>-1)){
	    			var tmps = curUrl.indexOf('#comments');
	    			nextUrl = curUrl.substring(0,tmps) + '/page-' + loadPage +'#comments';
	    		}else{
...
function t_setNaviAction(isHot){
    if(curImg>0){
        $('div.t_imgprev').unbind('click').bind('click',function(){
            ...
            curImg--;
            t_showImgbox(getImgsrc($(imgList.get(curImg))),t_getNavimgInfo(isHot),true,isHot);
        }).mouseenter(function(){
	    			...
	    			if(tpageExtend!='checked'){
					    ...
					    GM_xmlhttpRequest({
							...
							onload: function(result) {
								...
								chkhtml = chkhtml.replace(/([\s\S]*)<ol class="commentlist" style="list-style-type: none;">/,'').replace(/<\/ol><div class="comments"><p class="cp-pagenavi">([\s\S]*)/,'');
								var iad;
								for (iad in adCleanup)
								{
									var tmppos = chkhtml.indexOf(adCleanup[iad]);
									if(tmppos>-1){
										var tmpstart = chkhtml.lastIndexOf('<li>',tmppos);
										var tmpend = chkhtml.indexOf('</li>',tmppos);
										npCleanup = chkhtml.substring(tmpstart,tmpend+5);
										chkhtml = chkhtml.replace(npCleanup,'');
									}
								}
...
function t_getNavimgInfo(isHot){
    ...
    if(isHot){
        ...
        imgInfo = imgInfo + curItem.clone().children().remove().end().text();
    }else{
        curItem = $(imgList.get(curImg)).parent().parent();         
        imgInfo = '<b>' + curItem.find('b').eq(0).text() + '</b> ' + curItem.find('span.time:first').text()+'<br>';
        imgInfo = imgInfo + curItem.find('p:first').clone().children().remove().end().text();
    }
    return imgInfo;
}
...
function t_Settings(){
    ...
    $('#isea_help').click(function(){
       window.open('http://userscripts.org/scripts/show/130234');
    });
}
