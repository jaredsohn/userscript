// ==UserScript==
// @name		What.CD :: Requests Created Comments Notifier
// @namespace	http://userscripts.org/scripts/show/166166
// @description	Will notify you when you have unread comments on your requests created
...
(function() {
	...
	if ( (document.URL.indexOf("comments.php?action=requests&type=created") > "-1") && (document.URL.indexOf("action=requests&type=created") > "-1") && (document.URL.indexOf("page=") == "-1") ) {
	...
	if ( !lastChecked || (current.getTime() - lastChecked) > check_interval ) {
		...
		GM_xmlhttpRequest({
			method: 'GET',
			url: whatcd_base_url + '/comments.php?action=requests&type=created',
			headers: {
	...
	if ( alert_style <= 2 ) {
		if ( unviewedComments == true ) {
			target[1].innerHTML += ' <a href="/comments.php?action=requests&type=created"><img src="' + comment_alert_image + '" style="height: 11px; width: 10px;" title="You have ' + unviewedCount + ' new comments left on your requests" /></a>';
		} else {
			if ( alert_style == 1 ) { target[1].innerHTML += ' <a href="/comments.php?action=requests&type=created"><img src="' + comment_image + '" style="height: 11px; width: 10px;" title="View comments left on your requests" /></a>'; }
		}
		...
		if ( unviewedComments == true ) {
			if ( document.getElementById('alerts') != null ) {
				...
				new_alertbar.innerHTML = '<a href="/comments.php?action=requests&type=created">' + unviewedCount + ' new comments (requests)</a>';
	...
	var SUC_script_num = 166166; // Change this to the number given to the script by userscripts.org (check the address bar)
