...
var attack_unit_SAMElite      = "modal_attack_unit_ipt_20";  // "AttackSAMElite"
var attack_unit_GunshipElite  = "modal_attack_unit_ipt_21";  // "AttackGunshipElite"
...
var DEFAULT_ALERT_SOUND_URL = 'http://www.KCG-serve.com/gw/alarm.mp3';
var SWF_PLAYER_URL = 'http://www.KCG-serve.com/gw/alarmplayer.swf';
...
// TODO: Make sure WinPos is visible on-screen ?
 ...
 infoPop = new CPopup ('inf', Options.infWinPos.x, Options.infWinPos.y, 540,800, Options.infWinDrag,
  ...
  AddMainTabLink('GW-LT', eventHideShow, mouseMainTab);
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>800)
      errMsg = "X must be between 0 and 800<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>800)
      errMsg += "Y must be between 0 and 800<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>500)
      errMsg += "Max Distance must be between 1 and 500<BR>";
    if (errMsg != ''){
...
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['T Camp', 'Grassland', 'River', 'Oil Field', 'Hills', 'Mountain', 'Plain', 'City' ];
    var t = Tabs.Search;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 2) {
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD width="10%"><b>Location</b></td><TD width="20%"><b>Owner</b></td><TD width="20%"><b>City</b></td><TD width="12%"><b>Power</b></td><td width="15%"><b>Alliance</b></td><TD width="0%" style="font-size:9px;"><b>More info</b></td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			if(Options.unownedOnly || t.opt.searchType == 0){
			  ...
			  m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD align="center" width="10%">Location</td><TD width="10%">Dist</td><TD align="center" width="5%">Lvl</td><TD align="center" width="10%">Type</td><TD align="center" width="15%">Owner</td><TD width="12%">Power</td><TD align="center" width="15%">Alliance</td><TD width=0% align="center" style="font-size:9px;">More info</td></tr>';
			}
		}
	  }
      var numRows = dat.length;
      if (numRows > 5000 && t.searchRunning){
        numRows = 5000;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: Table only shows 5000 of '+ dat.length +' results until search is complete.</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
...
if (t.opt.searchType == 2) { 
				 m += '<TD valign="top" class=search1 >'+dat[i][9]+'</td><TD align=left class=search1 >'+ dat[i][8] +'</td><TD valign="top" class=search1 >'+dat[i][10]+'</td><td class=search1 >'+dat[i][11]+'</td><td class=search1 >';
				 if (dat[i][5]) {
					m += '<DIV onclick="PTscout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A style="font-size:9px;" >Scout</a></div>';
				} else 
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A class=loc2></a></div>\
					<DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A class=loc2></a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][7] +')"><A class=loc2></a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A class=loc2></a></div>';
			  ...
			  if(!dat[i][5] || t.opt.searchType == 0){
			    ...
			    m += '<TD align="right" valign="top" class=search1>'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD valign="top" class=search1 >'+ dat[i][4] +'</td><TD valign="top" class=search1 >'+ tileNames[dat[i][3]]+'</td><TD valign="top" class=search1 >'+ dat[i][9] +'</td><TD valign="top" class=search1 >'+dat[i][10]+'</td><TD valign="top" class=search1 >'+dat[i][11]+'</td><td class=search1 >';
				  if (dat[i][6]) {
					...
					m += '<DIV onclick="PTpd(this, '+ dat[i][8] +')"><A class=loc2 ></a></div>\
					<DIV style="" onclick="PTpd2(this, '+ dat[i][8] +')"><A class=loc2></a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][8] +')"><A class=loc2></a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][8] +')"><A class=loc2></a></div>';
				  }
				m += '</td></tr>';
			  }
			}
...
if (DEBUG_TRACE) logit (" 0 mapCallback()... setTimeout ..:" + x +' , '+ y +' , '+ t.mapCallback.toString().substr (0,50));
    setTimeout (function(){Map.request (x, y, 50, t.mapCallback)}, MAP_DELAY);
  },
};
...
Tabs.PodTools = {
    ...
    init : function (div){
        ...
        try {
            ...
            <TR>\
                ...
                <TD><INPUT id=AttackSAMElite     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; SAM Elite</td>\
                <TD><INPUT id=AttackGunshipElite     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Gunship Elite</td>\
            </tr>\
            <TR><TD><BR> </td></tr>';
    ...
    LaunchAttack : function () {
        ...
        if(generals && t.Attacking){
            ...
            if(document.getElementById("AttackSAMElite") && document.getElementById(attack_unit_SAMElite)) {
                document.getElementById(attack_unit_SAMElite).value=document.getElementById("AttackSAMElite").value;
            }
            if(document.getElementById("AttackGunshipElite") && document.getElementById(attack_unit_GunshipElite)) {
                document.getElementById(attack_unit_GunshipElite).value=document.getElementById("GunshipElite").value;
            }
...
var CollectRS = {
  ...
  init : function (){
    var t = CollectRS;
    t.lastCollect['c'+ Cities.cities[5].id] = 0;
	t.setEnable (Options.pbRSenable);
  },
  ...
  tick : function (){
    var t = CollectRS;
	if(!Cities.cities[5]) return;
    var city = Cities.cities[5];
    var since = unixTime() - t.lastCollect['c'+city.id];
    if (since>6*60*60){
        t.lastCollect['c'+city.id] = unixTime();
        t.colCityName = city.name;
        t.ajaxCollectRS (city, t.e_ajaxDone);
    }
    t.timer = setTimeout (t.tick, 15000);    
  },
  ...
  e_ajaxDone : function (rslt){
    var t = CollectRS;
    if (rslt.ok)
      actionLog ('COLLECTED '+ rslt.gained.resource2 +' RADAR STATION '+ t.colCityName, true);
    else
      actionLog ('ERROR COLLECTING RADAR STATION '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
  
  ajaxCollectRS : function (city, notify){
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    params.cid = city.id;
	params.eventid = 1;
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "missionControlEvent.php" + unsafeWindow.g_ajaxsuffix, {
