...
function Autocomplete(inputField, box, autocompleteStore, callback) {
	...
	this.box = box;
	this.autocompleteList = new AutocompleteList(inputField, autocompleteStore,
			this);
...
AutocompleteList.prototype.createOutput = function() {
	...
	this.inputField.parentNode.insertBefore(this.output,
...
AutocompleteList.prototype.highlightNextCompletion = function() {
	if (this.completions && this.completions.length > 0
			&& this.position < this.completions.length - 1) {
		++this.position;
...
AutocompleteList.prototype.update = function() {
	if (this.inputField.value.length > 0) {
		this.completions = this.autocompleteStore
				.getCompletions(this.inputField.value);
		if (this.completions && this.completions.length > 0) {
			this.clearOutput();
...
ListAutocompleteStore.prototype.getCompletions = function(text) {
	if (window.wrappedJSObject.listTabs
			&& window.wrappedJSObject.listTabs.entries
			&& window.wrappedJSObject.listTabs.entries.length > 0) {
		var completions = new Array();

		for ( var i = 0; i < window.wrappedJSObject.listTabs.entries.length; ++i) {
			if (window.wrappedJSObject.listTabs.entries[i].toLowerCase()
...
var createAutoComplete = function(name, callback) {
	...
	autocompleteBox.innerHTML = '<div class="white_rbroundbox"> <div class="white_rbtop"> <div> <div></div> </div> </div> <div class="white_rbcontentwrap"> <div class="white_rbcontent"> <div class="taskcloudcontent" style="padding: 0px 5px 0px 5px; height: 17px;" id="listtabscontainer"><div style="width: 70px; font-weight: bold; float: left; height: 17px; padding-top: 1px;">'
			+ name
			+ '</div><div style="text-align: right; float: right; width: 155px; padding-right: 2px;"><input type="text" id="autocompleteInputField_'
			+ name
			+ '" name="text" style="width: 151px;  ";/></div> </div> </div> </div> <div class="white_rbbot"> <div><div></div> </div> </div> </div> ';

	document.body.appendChild(autocompleteBox);
	var autocomplete = new Autocomplete(document
			.getElementById("autocompleteInputField_" + name), document
			.getElementById("autocompleteBox_" + name),
			new ListAutocompleteStore(), callback);

	function centerAutoCompleteBox() {
		var left = window.wrappedJSObject.innerWidth / 2 - 120
				+ window.wrappedJSObject.scrollX;
		var top = window.wrappedJSObject.innerHeight / 2 - 10
				+ window.wrappedJSObject.scrollY;
...
var createAutoCompletes = function() {
	window.wrappedJSObject.autocompletes["goTo"] = createAutoComplete(
			"GO TO: ", selectListByName);
	window.wrappedJSObject.autocompletes["moveTo"] = createAutoComplete(
			"MOVE TO: ", moveToListByName);
}
...
var moveToListByName = function(text) {
	...
	for ( var i = 0; i < window.wrappedJSObject.listTabs.entries.length; ++i) {
		if (window.wrappedJSObject.listTabs.entries[i].toLowerCase() == text
				.toLowerCase())
			window.wrappedJSObject.control.tasksSelectionChanged("", [
					"",
					"tasks.moveTo."
							+ window.wrappedJSObject.listTabs.data[i][1] ]);
	}
}
...
var showLeftColumn = function() {
	...
	if (leftColumn) {
		...
		leftColumn.style.width = Math
				.round(listTabsContainer.clientWidth * 1.14)
				+ "px";
...
var moveTabsToTheLeft = function() {
	...
	if (window.wrappedJSObject.messageBus)
		window.wrappedJSObject.messageBus.subscribe(handleViewChanged,
				window.wrappedJSObject.view.getUniqueMessageBusName()
						+ "viewChanged");
...
var handleKeyPressEvent = function(ev, ignoreCombo) {
	...
	if (target != null
			&& target.type != null
			&& (target.type == "textarea" || target.type == "input"
					|| target.type.indexOf("select") == 0
					|| target.type == "button" || target.type === "submit"
					|| target.type == "text" || target.type == "password" || (target.id != null && target.id == "map")))
		return true
	...
	if (tabs) {
		...
		case 103:
			if (ev.ctrlKey) {
				hideAutocompletes();
				window.wrappedJSObject.autocompletes["goTo"]
						&& window.wrappedJSObject.autocompletes["goTo"].show();
				if (window.wrappedJSObject.utility)
					window.wrappedJSObject.utility.stopEvent(ev);
		...
		case 109:
			if (ev.ctrlKey) {
				hideAutocompletes();
				window.wrappedJSObject.autocompletes["moveTo"]
						&& window.wrappedJSObject.autocompletes["moveTo"]
								.show();
				if (window.wrappedJSObject.utility)
					window.wrappedJSObject.utility.stopEvent(ev);
...
var overrideBodyKeyPressHandler = function() {
	if (window.wrappedJSObject.eventMgr) {
		...
		window.wrappedJSObject.eventMgr.bodyKeyPressHandler = function(ev,
				ignoreCombo) {
			if (handleKeyPressEvent(ev, ignoreCombo))
				return oldBodyKeyPressHandler.call(
						window.wrappedJSObject.eventMgr, ev, ignoreCombo);
...
var hideLists = function() {
	if (window.wrappedJSObject.listTabs) {
		var listItems = window.wrappedJSObject.listTabs.div
				.getElementsByTagName("li");

		for ( var i = 0; i < window.wrappedJSObject.listTabs.data.length; ++i) {
			if (window.wrappedJSObject.listTabs.data[i]
					&& isListHidden(window.wrappedJSObject.listTabs.data[i][1]))
...
var isListArchived = function(listID) {
	if (window.wrappedJSObject.stateMgr.lists[listID]
			&& window.wrappedJSObject.stateMgr.lists[listID].archived)
		return true;
...
var overrideListListUpdateEntry = function() {
	if (window.wrappedJSObject.listList) {
		...
		window.wrappedJSObject.listList.list.updateEntry = function(entry, D) {
			oldListListUpdateEntry.call(window.wrappedJSObject.listList.list,
					entry, D);
			...
			row.entry = entry;
			row.rowText.innerHTML = "<div id='listName_"
					+ entry[0]
					+ "' style='float: left;'>"
					+ entry[1]
					+ "</div><div style='align: right; float: right;'><a href=\"# \" id='displayListLink_"
					+ entry[0] + "'></a></div>";

			var displayListLink = document
					.getElementById('displayListLink_' + entry[0]);
