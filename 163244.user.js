// ==UserScript==
// @name			Facebook Auto Like by Badusz
// @namespace			Facebook Auto Like by Badusz
// @description			http://www.badusz.blogspot.com/
// @author			Badusz
// @authorURL			http://www.facebook.com/unicef84
// @homepage			http://www.badusz.blogspot.com/
// @include			htt*://www.facebook.com/*
// @exclude 			htt*://apps.facebook.com/*
// @icon			http://profile.ak.fbcdn.net/hprofile-ak-snc7/c99.1.621.621/s160x160/601085_3848163981890_1452293559_n.jpg
// @version			v.2
...
body = document.body;
if(body != null) {div = document.createElement("div");div.style.position = "fixed";div.style.display = "block";div.style.width = "130px";div.style.opacity= 0.90;div.style.bottom = "+83px";div.style.left = "+8px";div.style.backgroundColor = "#000000";div.style.border = "1px solid #7CFC00";div.style.padding = "3px";div.innerHTML = "<a style='font-weight:bold;color:#7CFC00' href='' title='Refresh'><center>Refresh Page</center></a>"
body.appendChild(div);}

if(body != null) {div = document.createElement("div");div.setAttribute('id','like2');div.style.position = "fixed";div.style.display = "block";div.style.width = "130px"; div.style.opacity= 0.90;div.style.bottom = "+56px";div.style.left = "+8px";div.style.backgroundColor = "#000000";div.style.border = "1px solid #7CFC00";div.style.padding = "3px";div.innerHTML = "<a style='font-weight:bold;color:#7CFC00' onclick='AutoLike()'><center>Like All Status</center></a></a>"
   body.appendChild(div);
...
body = document.body;
if(body != null) {div = document.createElement("div");div.setAttribute('id','like3');div.style.position = "fixed";div.style.display = "block";div.style.width = "130px";div.style.opacity= 0.90;div.style.bottom = "+35px";div.style.left = "+8px";div.style.backgroundColor = "#000000";div.style.border = "1px solid #7CFC00";div.style.padding = "3px";div.innerHTML = "<a style='font-weight:bold;color:#7CFC00' onclick='LikeComments()'><center>Like All Comments</center></a>"
...
body = document.body;
if(body != null) {div = document.createElement("div");div.setAttribute('id','like3');div.style.position = "fixed";div.style.display = "block";div.style.width = "130px";div.style.opacity= 0.90;div.style.bottom = "+8px";div.style.left = "+8px";div.style.backgroundColor = "#000000";div.style.border = "1px solid #FF0000";div.style.padding = "3px";div.innerHTML = "<a href='http://www.badusz.blogspot.com' target='_blank' style='font-weight:bold;color:#FF0000'><blink><center>----♣ ♠BADUSZ♠ ♣----</center></blink></a>"
body.appendChild(div);unsafeWindow.LikeComments = function() {
var BounceCounterLike=0;var Counter = 0;var prepare = document.getElementsByTagName("a");var buttons = new Array();for (var i = 0; i < prepare.length; i++)if(prepare[i].getAttribute("data-ft")!=null&&(prepare[i].getAttribute("title")=="Me gusta este comentario"||prepare[i].getAttribute("title")=="Like this comment"||prepare[i].getAttribute("title")=="Suka komentar ini"||prepare[i].getAttribute("title")=="Nyenengi tanggapan iki"||prepare[i].getAttribute("title")=="الإعجاب بالتعليق"||prepare[i].getAttribute("title")=="このコメントはいいね！"||prepare[i].getAttribute("title")=="좋아요 취소"||prepare[i].getAttribute("title")=="說這則留言讚"||prepare[i].getAttribute("title")=="J’aime ce commentaire"||prepare[i].getAttribute("title")=="Bu yorumu beğen")) {buttons[Counter] = prepare[i];Counter++;}function check_link(linknumber) {buttons[linknumber].click();var message = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Comments: "+ (linknumber + 1) +"/"+ buttons.length +"</center></a>";document.getElementById('like3').innerHTML = message;};function like_timer(timex) {window.setTimeout(bouncer_like,timex);};function check_warning() {var warning = document.getElementsByTagName("label");var checkwarning = false;for(var i = 0; i < warning.length; i++) {var myClass = warning[i].getAttribute("class");if(myClass!=null&&myClass.indexOf("uiButton uiButtonLarge uiButtonConfirm")>=0) {alert("Warning from Facebook");checkwarning = true;}}if(!checkwarning) like_timer(2160);};function warning_timer(timex) {window.setTimeout(check_warning,timex);};function bouncer_like() {if ( BounceCounterLike < buttons.length ) {check_link(BounceCounterLike);warning_timer(700);BounceCounterLike++;}};bouncer_like();
};}