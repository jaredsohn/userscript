...
// @description    	BAN QUYEN CUA A collection of mods, tools, and hacks for Ikariam by PhasmaExMachina
// @author			SUA CHUA BOI QUYNH PhasmaExMachina
...
console.log("Tooltip html: " + tooltip.html());
						var capacity = tooltip.html().match(/((\d|,|\.)+k?)[^\d]*$/i)[1];
		...
		if(!IkaTools.buildingsOverview) {
			...
			for(var i = 0; i < preferredBuildingOrder.length; i++) {
				for (var j = 0; j < ownedBuildings[preferredBuildingOrder[i]]; j++) {
					...
					html.push('.gif" style="max-height:21px; max-width:25px; margin-top:5px;"/></td>\</td>');
				}
			}
			html.push('</tr>');
		...
		if(Config.get('playerCitiesInIslandView')) {
			...
			function drawScore(label, type) {
				...
				$('#exMachinaInlineScore_' + type).html('<span style="width:70px; float:left;">loading ...</span> ...');
                                console.log('Loading player score: ' + type + ' for ' + '"' + player + '"');
...
Treaty = {
	...
	icons:{
		ct:'<img src="skin/museum/icon32_culturalgood.gif" width="16" height="16" align="absmiddle" title="You have a cultural treaty with this player" style="cursor:help;" />',
...
ExternalScripts.ArmyHelper = function () {
	const languages = {
		...
		"hungarian": {
			...
			ship_steamboat: "Evez?kerék hajó",
		...
		"latvian": {
			...
			presetsTitle: "Iepriekšpl?nots sast?vs",
		...
		"polish": {
			...
			removeSlot: "Opró?nij jeden slot",
