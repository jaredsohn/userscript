// ==UserScript==
// @name           InfoCompte International (Spanish Fix)
// @author         originally Vulca, now maintained by Fim (Spanish fixed by Saelos)
// @date           21-03-2009
// @version        1.3.10
...
const langue = new Array('de','org','us','se','com.es','com.ar','com.pt','nl','ba','com.br','gr','onet.pl','bg','mx');
...
if (Langue=='de')
{
	...
	var BBcode_debut="[quote][align=center][b]"+Pointdetails+":[/b]\n\nGesamtpunkte: ";
...
//
/* ******************************Spanish ********************************/
if (Langue=='com.es' || Langue=='mx' || Langue=='com.ar')
{
	var Mines='Minas';
	var Other_structure='Edificios';
	var Structure = 'Edificios';
	var Technology='Investigaciones';
	var Fleet ='Flota';
	var Defense = 'Defensas';
	var Progression = 'Progreso' ;
	var Moyenne = 'Promedio';
	var Production = 'Producción';
	var Indestructible = 'Indestructibles';
	var this_Planet='Éste planeta';
	var Depuis = 'desde el';
	var Points = 'Puntos';
  ...
  var construction_time = 'Tiempo de producción:';
	...
	var BBcode_debut="[quote][align=center][b]"+Pointdetails+":[/b]\n\nPuntos Totales : ";
	var BBcode_mine="Puntos en Minas: ";
	var BBcode_bat="Puntos en otros Edificios: ";
	var BBcode_batT="Puntos en Edificios: ";
	var BBcode_fin1 = "Puntos en Investigaciones: ";
	var	Bcode_fin2 = "Puntos en Flotas: ";
	...
	var sur_lune='en lunas';
	var en_vol='en vuelo';
	var Avertissement ='¿Estás seguro de querer reiniciar el conteo de su progreso?';
	var restart = 'Haga clic aquí para reiniciar su progreso.';
	var AffBBcode = 'Haga clic aquí para ver BBcode (para el foro)';
	
	var done ='Guardar cambios';
	var ICoption = 'Opciones InfoCompte:';
	
	var option1 ='<tr><th>Color de los gráficos';
	var option2 ='<tr><th>Porcentaje de puntos en edificios';
	var option3 ='<tr><th>Porcentaje de puntos indestructibles';
	var option4 ='<tr><th>Porcentaje de puntos tecnologías';
	var option5 ='<tr><th>Porcentaje de puntos en flota';
	var option6 ='<tr><th>Porcentaje de puntos defensas';
	var option7 ='<tr><th>Porcentaje de puntos de flota en vuelo';
	var option8 ='<tr><th>Porcentaje de puntos en la luna';
	var option9 ='<tr><th>Todos los porcentajes juntos (Para la flota en vuelo y puntos de la luna)';
	var option10 ='<tr><th>Porcentaje del progreso';
	var option11 ='<tr><th>Compruebar si son varios en el mismo equipo o incluso universo';
	var option12 ='<tr><th>Ver en color en función de los progresos';
	var option13 ='<tr><th>Mostrar los puntos por día';
	...
	var option16 ='<tr><th>Mostrar la información de depuración';
...
if (Langue != '')	
{
	...
	var ogame_url='.ogame.'+Langue; if (Langue=='ba' || Langue=='bg') { ogame_url='.'+Langue+'.ogame.org'; } 
  ...
  var IDMatch; var CoordMatch; var PlanetOption; var PIDerror='';
  for(var i = 0; i < selectnode[0].options.length; i++) 
  {
    ...
    if (PlanetOption==undefined) { if(i==0) { PIDerror='NO_OPTIONS<BR\>'; break; } else { PIDerror+='NO_OPTION_AT_INDEX_'+i+'<BR\>'; continue; } }
  	...
  	else { PIDerror+='NO_ID_MATCH_AT_INDEX_'+i+'<BR\>'; }
  ...
  // this part is supposed to detect Nanites level if robotics has been deconstructed. Let's hope it works (it definitely didn't before 1.3.9)
   ...
   if (debug_points==true) {
      ...
      if (sats>0) { PointsSatsThis=Math.floor(sats*2.5); }
		...
		for (var i = 0; i < 2*nombreplanetesetlunes; i++) 
		{
			if (i%2 == 0) // Si i est pair c'est une mine
			{
				...
				if (Math.floor(i/2)==numeroplanete) { PointsMinesThis=parseInt(BatPlanete[i]); }
			} 
				
			if(i%2 == 1) // si i est impair
				{
					PointsBatimentsTotal += parseInt(BatPlanete[i]);
					if (Math.floor(i/2)==numeroplanete) { PointsBatimentsThis=parseInt(BatPlanete[i]); }
				} // c'est un bat
	...
	if (debug_points==true) {
		...
		{
			nbAfficher++;
			BBcode+=BBcode_mine+"[b][color=#339966]"+addPoints(PointsMinesTotal)+"[/color][/b] ("+soit+" [b][color=#339966]"+pourcent(PointsMinesTotal)+"[/color][/b]%)\n";
		}
		if(AutreBat) 
		{
			nbAfficher++;
			BBcode+=BBcode_bat+"[b][color=#3399FF]"+addPoints(PointsBatimentsTotal)+"[/color][/b] ("+soit+" [b][color=#3399FF]"+pourcent(PointsBatimentsTotal)+"[/color][/b]%)\n";
		}
		if(BatTotal) 
		{
			nbAfficher++;
		...
		BBcode+=BBcode_fin1+"[b][color=#00CCFF]"+addPoints(PointsTechno)+"[/color][/b] ("+soit+" [b][color=#00CCFF]"+pourcent(PointsTechno)+"[/color][/b]%)\n";;
		BBcode+=Bcode_fin2+"[b][color=#FF9900]"+addPoints(PointsFlotteTotal)+"[/color][/b] ("+soit+" [b][color=#FF9900]"+pourcent(PointsFlotteTotal)+"[/color][/b]%) \n";
		BBcode+=BBcode_fin3+="[b][color=#FFFF66]"+addPoints(PointsDefTotal)+"[/color][/b] ("+soit+" [b][color=#FFFF66]"+pourcent(PointsDefTotal)+"[/color][/b]%) \n\n";
		BBcode+=BBcode_fin4+="[b][color=#FFFFFF]"+addPoints(PointsMinesTotal+PointsBatimentsTotal+PointsTechno)+"[/color][/b] ("+soit+" [b][color=#FFFFFF]"+pourcent(PointsMinesTotal+PointsBatimentsTotal+PointsTechno)+"[/color][/b]%)";
		BBcode+=" "+BBcode_fin5;	
		...
		BBcode+="Uni: [b][color=#ff0000]"+ uni+"[/color][/b]."+Langue+"[/align][/quote]";
