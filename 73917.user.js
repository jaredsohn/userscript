...
for (i = 0; i < divs.length; i++) {
    if (divs[i].className.split('post tree').length > 1) {
        surface = divs[i].getElementsByTagName('div');
        
        nick = surface[2].getElementsByTagName('a')[1].innerHTML;
        surface[1].style.width = 'auto';
        ...
        b_color = '#';
        na = [];
        for (j = 0; j < 3; j++) na[j] = parseInt(r.substr(j * 2, 2), 16);
        for (j = 0; j < 3; j++) {
            n0 = Math.round((na[j] / 255) * 35);console.log(n0);
            n1 = Math.max.apply(Math, na) == na[j] ? 220 + n0 : 200 + n0;
            n2 = Math.max.apply(Math, na) == na[j] ? 180 + n0 : 160 + n0;
            color +=  n1.toString(16);
            b_color += n2.toString(16);
        }
        color = color.substr(0, 7);
        b_color = b_color.substr(0, 7);
        
        for (j = 0; j < 3; j++) surface[j].style.background = color;
        if (divs[i].className.split('new').length > 1) {
            surface[0].style.borderLeft = b_color + ' 10px solid';
            surface[2].style.borderLeft = b_color + ' 10px solid';
        }
        
        surface[0].style.padding = '5px';
        surface[2].style.padding = '3px';
    }
}