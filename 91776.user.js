// ==UserScript==
// @name           Imperion Mods with Warlock8's addons
// @namespace      PhasmaExMachina & Warlock8
// @description    Misc improvements for Imperion
// @include        http://*.imperion.*/*
// @include        http://beta.imperion.*/*
// @require        http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js
// @author         PhasmaExMachina & Warlock8
...
// @version        0.39.6
//
// @history        0.39.5 Fixed glitch 
// @history        0.39.4 Improved algorithm of auto harvest comets
// @history        0.39.3 Fixed glitch 
// @history        0.39.2 Titans now have a config tab that lets you choose which transport to use (large by default)
// @history        0.39.2 Send transports from espionage report clicking on green arrow in report view
// @history        0.39.1 Added links for terrans Recyclers & LargeRecyclers in comet/debris/asteroid view
...
ScriptUpdater.check(74036, '0.39.6');

Config.scriptName = "Imperion Mods by PhasmaExMachina with Warlock8's addons";
...
Config.tabs = {
	...
	"Harvest":{
		html:'<p>The following options are related to the autoharvest</p>',
		fields:{
			autoHarvest:{
				type:'checkbox',
				label:'Auto-Harvesting',
				text:'Wlaczanie/ Wylaczanie; Domyslnie Tryb Nocny',
				value:true
			},
			max_Dist:{
				type:'text',
				label:'Distance',
				text:'max distance to nearby comets',
				value:15
			},
			max_Harv:{
				type:'text',
				label:'Harvesters',
				text:'How many harvesters to send nearby comets',
				value:150
			},
			oneminuteTimer:{
				type:'checkbox',
				label:'Timer',
				text:'Wlaczanie Trybu Fast',
				value:true
			}
		}
	},
	...
	"Titans":{
		html:'<p>The following options only apply to Titans players',
		fields:{
			usingLargeTransports:{
				type:'checkbox',
				label:'Large Transports',
				text:'use large transports in calculations',
				value:true
			}
		}
	},
	...
	"About":{
		html:'<p>This script performs some basic modifications of the game.</p><p><strong>Author:</strong> <a href="http://userscripts.org/users/PhasmaExMachina">PhasmaExMachina</a></p><p><strong>Addons author:</strong> <a href="http://userscripts.org/users/148474">Warlock8</a></p>'	}
};
...
Imperion = {
	...
	init:function() {
		...
		if(Config.get('autoHarvest')) {
			...
			function getRandomHarvestInterval() {
				var variance = Math.floor(Math.random()*15);	// 15 minute variance
				if(Config.get('oneminuteTimer')) {
					return 5000;
				}
				return(12000 * (5 + variance));
			}
	...
	getShipPayload:function(shipId) {
		return parseFloat($('.interface_icon_attrbiutes_payload', GetClassicDom("/help/ship/" + shipId)).text().replace(/[^\d]/g, ''));
	},
	...
	views:{
		...
		map:function() {			
			...
			function getMapObject() {
				var map = {
					...
					getCometsNearby:function() {
						var cometsFound = [];
						var maxDistance = parseInt(Config.get('max_Dist'));
						var coords = map.getCurrentSystemCoords();
						for(var x = coords.x - maxDistance; x < coords.x + maxDistance; x++) {
							for(var y = coords.y - maxDistance; y < coords.y + maxDistance; y++) {
								...
								for(var i = 0; i < comets.length; i++) {
									...
									if(typeof(comets[i]['11']) == 'undefined') {
										cometsFound.push(comets[i]);
									}
								}
							}
						}
						cometsFound.sort(function(a, b){
					...
					getClosestOpenComet:function(maxDist) {
						...
						for(var i = 0; i < comets.length; i++) {
							var total = parseInt(comets[i].r1) + parseInt(comets[i].r2) + parseInt(comets[i].r3);
							if(total >= Imperion.getShipPayload(getHarvesterId())/2  && (comets[i].distance <= maxDist) && (typeof(comets[i]['11']) == 'undefined')) {
								closestComet = comets[i];
							}
							if(closestComet) break;
						}
						return closestComet;
					}
				}
			...
			if(Imperion.getVal('performAutoHarvestNow')) {
				Imperion.setVal('performAutoHarvestNow', false);
				var maxHarvesters = parseInt(Config.get('max_Harv'));
				var maxDistance = parseInt(Config.get('max_Dist'));
				var numRecyclersAvailable = Imperion.getVal('numRecyclersAvailable');
				numRecyclersAvailable = numRecyclersAvailable < maxHarvesters ? numRecyclersAvailable : maxHarvesters;
				// check closest comet
				var closestComet  = map.getClosestOpenComet(maxDistance);
				if(closestComet) {
					var total = closestComet.r1 + closestComet.r2 + closestComet.r3;
					var numHarvestersNeeded = Math.round(total / Imperion.getShipPayload(getHarvesterId()));
			...
			function systemView() {
						...
						if(systemId) {
							...
							if(!error && confirm('Are you sure you want to survey ' + planetsToSurvey.length + ' empty planets?' +
								...
								for(var i = 0; i < planetsToSurvey.length; i++) {
									...
									var fleetBaseUrl = 'http://' + document.domain + '/fleetBase/mission/1/planetId/' + planetId + '/m/304/ships/,1#survey';
				...
				$('#phasmaSurveySystemButton').attr('title', 'Surves unoccupied planets in this system');
			...
			function getRecyclerHtml(total, type) {
				var recycleUrl0 = $('#recycle').attr('href') + (Config.get('usingLargeRecylers') ? '/ships/,,,' : '/ships/,,,,');
				var recycleUrl1 = $('#recycle').attr('href') + (Config.get('usingLargeRecylers') ? '/ships/,,,,' : '/ships/,,,');
				var numHarvesters = Math.ceil(total / Imperion.getShipPayload(getHarvesterId()));
				var TerransAnotherRecycler = Config.get('usingLargeRecylers') ? 4 : 3;
				var numAnotherRecycler = Math.ceil(total / Imperion.getShipPayload(TerransAnotherRecycler));
				var harvesterStr = '<tr><td>Total:</td><td id="ImperionMods' + type + 'Total">' + (Config.get('addMapCommas') ? addCommas(total) : total) + '</td></tr>' +
						'<tr class="fontColorRace"><td><a href="' + recycleUrl0 + numHarvesters  + '">' + Imperion.getShipName(getHarvesterId()) + ':</a></td>'+
						'<td id="ImperionMods' + type + 'RecyclerCount">' + numHarvesters + '</td></tr>';
				if(Imperion.getRace() == 'terrans') {
					harvesterStr = harvesterStr + '<tr class="fontColorRace"><td><a href="' + recycleUrl1 + numAnotherRecycler + '">' + Imperion.getShipName(TerransAnotherRecycler) + ':</a></td>'+
							'<td id="ImperionMods' + type + 'RecyclerCount">' + numAnotherRecycler + '</td></tr>';
				}
					...
					if(reportLink.size() == 1) {
						...
						switch(report.type) {
							...
							case 'espionage':
								...
								//Terran LT24 2 ,,
								//Titan Transport 22 ,,
								//Titan Large Transport 23 ,,,
								var localrace = Imperion.getRace();
								var recycleUrl0 = '/fleetBase/mission/1/planetId/' + report.targetId + '/m/301/ships/,,';
								var total = report.resourcesAvailable.metal + report.resourcesAvailable.crystal + report.resourcesAvailable.fuel;
								if( localrace == "xen") {
									var numTransports = Math.ceil(total / Imperion.getShipPayload(45));
									recycleUrl0 += parseInt(numTransports);
								}
								if( localrace == "terrans") {
									var numTransports = Math.ceil(total / Imperion.getShipPayload(2));
									recycleUrl0 += parseInt(numTransports);
								}
								if( localrace == "titans") {
									var TitanTransportId = Config.get('usingLargeTransports') ? 23 : 22;
									var numTransports = Math.ceil(total / Imperion.getShipPayload(TitanTransportId));
									recycleUrl0 += Config.get('usingLargeTransports') ? ',' : '';
									recycleUrl0 += parseInt(numTransports)
								}
								reportLink.after(
									(typeof(report.targetHref) != 'undefined' ? 
										' <a href="' + report.targetHref + '" title="View target on map" class="colorDarkGrey">' + targetText + '</a> ' +
										' <a href="' + recycleUrl0 + '" title="Send fleet"><img src="' + icons.go + '" class="reportIcon"/</a> ' 
										: '') +
