// ==UserScript==
// @name           Dream World Library this for normal bosses
...
function fightBoss()
{
	...
	if (!executeCommandsInQueue())
	{
		addToQueue(["fightWithSkillOrRefillSkillPoints(-1)","clickButton('Attack Again')"]);
	}
}
...
function autoFight()
{
	...
	if (!executeCommandsInQueue())
	{
		...
		else if ($inc1("enctext"))
		{
			...
			{
				case "A Beggar":
					queue=["getEnergyOrNothing()",exploreCommand];
					break;
				case "A Puzzle Box":
					queue=["clickButton('Open')","solvePuzzleBox()",exploreCommand];
					break;
				case "A Treasure Chest":
					queue=["clickButton('Open')","solveTreasureChestAndDisposeItem()",exploreCommand];
					break;
				case "A Treasure Map":
					queue=["clickButton('Find')","getTreasureMapResultAndDisposeItem()",exploreCommand];
					break;
				case "Dust Merchant":
					queue=["clickButton('Fight')",fightCommand,exploreCommand];
					break;
				case "Girl in Red":
					queue=["clickButton('Accept')","getGirlInRedQuest()",exploreCommand];
					break;
				case "Guardian of Dreams":
					queue=["clickButton('Fight')","fightWithSkill(0)","getGuardianOfDreamsResultAndDisposeItem()",exploreCommand];
					break;
				case "Little Imp":
					queue=["clickButton('Fight')",fightCommand,exploreCommand];
					break;
				case "Magic Elf":
					queue=["clickButton('Attack')","fightWithSkill(0)","getFightResultAndDisposeItem()",exploreCommand];
					break;
				// case "New Ally":
					// break;
				...
				case "The Gambler":
					...
					break;
				case "The Wiseman":
					queue=["killOrAccept()",exploreCommand];
					break;
				case "Wishing Well":
		...
		else if ($inc1("mobnamespan")) queue=[fightCommand,exploreCommand];
		else 
		{
...
{
	addToQueue(["clickButton('Open')","clickButton('Attack')","tryClickButton('Attack')"]);
}
...
function freeInventoryFromUselessItems(text)
{
	...
	//else addToQueue(["tryClickButton('Attack')",exploreCommand]);
}
...
function openInventoryAndUseItem(item,addCommands)
{
	...
	queue.push("clickButton('Attack',1)");
...
function useWandSkill(skill) { return equipWand()?useSkill(skill):true; }
...
function maxMoney()
{
	return addToQueue(["clickButton('Home')","tryClickButton('Open')","emptyTurn()","tryClickButton('Attack')","clickPlayNow()","clickButton('Guardian')","emptyTurn()","buyMoney()","clickButton('Attack')","changeStateAction('betAllOnWishingWells')"]);
}
...
function getFightResult() { return parseMessageBoxContent([/You found an(.*)!/,/You received(.*)coins!\s*You received(.*)experience!/,/You received(.*)!/,/You were knocked(.*)/], true); }
