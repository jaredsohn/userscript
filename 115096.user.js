// ==UserScript==
// @name			OGame - Fast Login - mod
// @namespace		Gollum
// @description		Simplified connection to ogame ! (V. 0.82)
// @include			http://*ogame.*
...
/*** Version ***/
	function Mise_a_jour(Version, VA) {
		...
		Code += '<tr><th><a href="http://userscripts.org/scripts/source/115096.user.js" id="lien_mise_a_jour">'+Tra(17, Na)+'</a></th></tr>';
		Code += '<tr><th class="b"></th></tr>';
	...
	if ((Date_ - Anci) > 86400*3) {
		GM_xmlhttpRequest({
			method: "GET",
			url: 'http://userscripts.org/scripts/show/115096',
...
/*** Functions : ***/
	...
	//function Options_(Select) {var txt = ''; for (i in Select.options) {txt += '<option value="'+Select.options[i].value+'">'+Select.options[i].innerHTML.replace(/ /g,'').replace(/\./g,'. ')+'</option>\n';} return txt;}
		function Options_(Select) {var txt = ''; for (i in Select.options) {if(typeof Select.options[i].value != "undefined"){txt += '<option value="'+Select.options[i].value+'">'+Select.options[i].innerHTML.replace(/ /g,'').replace(/\./g,'. ')+'</option>\n';}} return txt;}
	...
	uW.Charger = function (uni, NUni, NJ, MP) {
		...
		Select_(id_("serverLogin"), uni); 
		//uW.setUniID(NUni); 
		uW.setUniUrl(uni);
		id_("usernameLogin").value = NJ; 
		id_("passwordLogin").value = MP;
//alert(NJ);
