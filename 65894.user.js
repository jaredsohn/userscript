// ==UserScript==
// @name			books_recommend_NPU
// @namespace		books_recommend_NPU
// @version			v0.25
...
// @director        Northwestern  Polytechnical  University  Library /System Development Team
...
var _extLinkTpl = 'http://202.117.255.187:8080/opac/openlink.php?strText=${isbn0}&strSearchType=isbn';
var url_by_title= 'http://202.117.255.187:8080/opac/openlink.php?strText=${title0}&strSearchType=title';
var booklist = [];
var finallink;

var DOUBAN = 0, AMAZON = 1, AMAZON_CN = 2, GOOGLE_BOOK = 3, ERR_SITE = 4, DANGDANG = 5;
var now_site = DOUBAN;

var not_found_msg = "本馆没有您检索的馆藏书目";
...
{ 
	...
	} else if (now_site_url.substr(0, 28) == "http://product.dangdang.com/") {
		now_site = DANGDANG;
		if ($('#twotabtop .middleon div a').text()	== '图书') 
		{
			title = $('h1').text();
			title = title.replace(/(\(|（).*(\)|）)/g, " ").split(":")[0];
			$('#Preferences .Left .dark').each(function() {
				if ($(this).text().substr(0, 7) == "I S B N") {
					isbn = $(this)[0].nextSibling.nodeValue; 
					isbn = isbn.split("/")[0];
				}
			})
	...
	if ((now_site == DOUBAN && $('#nav	a.now span').text()	== '读书') || (now_site == AMAZON_CN && $('#twotabtop .middleon div a').text()	== '图书')|| (now_site == DANGDANG && $('#twotabtop .middleon div a').text()	== '图书') || now_site == AMAZON || now_site == GOOGLE_BOOK) 
	{	
		if(isbn!='')
		{
		setTimeout(function()
	...
	function myextract(res)
	{
        ...
        if (now_site == DOUBAN) {
        	...
        	var htmlStr = '<tr><td><a href="'+finallink+'" target="_blank">到西工大图书馆看看？</a></td></tr>';
    ...
    function InsertBookList() 
    {
    	if (now_site == AMAZON) {
    		var _itemTpl = ['<li>',
                    ...
                    '</li>'].join('');
				var s = [];
				s.push('<h2>Want To Borrow Books? Welcome To Northwestern Polytechnical University Library! </h2>');
        ...
        s.push('</div></br>');
        s.push('<hr>');
    	} else if (now_site == AMAZON_CN || now_site == DOUBAN || now_site == DANGDANG) {
        var _itemTpl = ['<li>',
        ...
        s.push('</div></br>');
        if (now_site == AMAZON_CN || now_site == DANGDANG) {
        	s.push('<hr>');
        }
      }
        return s.join('');
	    ...
	    function _analyse_t(res) 
    {
        if(res.responseText.indexOf(not_found_msg)==-1)
        {
            ...
            if (now_site == DOUBAN) {
							var htmlStr = '<h2>想借书? 到西北工业大学图书馆看看!</h2>';
