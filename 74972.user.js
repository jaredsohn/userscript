// ==UserScript==
// @name           Translate
// @namespace      nazariuskappertaal
// @description    Translates selected text into a tooltip.
// @include        http://*
// ==/UserScript==
const HREF_NO = 'javascript:void(0)';
var  languagesGoogle = '<option value="auto">Detect language</option>\
...
function showLookupIcon(evt)
{
	...
	//div container
	divLookup = createElement('div', {id:'divLookup', style:'background-color:#FFFF77; color:#000000; position:absolute; top:'+(evt.clientY+window.pageYOffset+10)+'px; left:'+(evt.clientX+window.pageXOffset+10)+'px; padding:3px; z-index:10000; -moz-border-radius:3px;'});
...
function lookup(evt)
{
	...
	//div container
	divDic = createElement('div', {id:'divDic', style:'background-color:#FFFF77; color:#000000; position:absolute; top:'+top+'; left:'+left+'; min-width:250px; min-height:50px; max-width:50%; padding:5px; font-size:small; text-align:left; z-index:10000; -moz-border-radius:3px;'});
	...
	//options link
	divDic.appendChild(createElement('a', {class:"gootranslink", id:'optionsLink', href:HREF_NO, style:'position:absolute; bottom:3px; right:3px; font-size:small; text-decoration:none;'}, 'click options false', 'Language Options'));
...
function extractResult(html)
{
	...
	if(dict)
		getId('divResult').innerHTML += '<br><br><div id="dict" style="background-color:#FFFFAA; color:#000000; padding:5px; -moz-border-radius:3px; margin-bottom:10px; max-height:180px; overflow-y:auto; overflow-x:hidden; font-size:small;">'+dict+'</div>';
...
function css()
{
	var style = createElement('style',{type:"text/css"},null,""+	
		...
		'#dict table {font-size:13px; line-height:1.5em; margin-left:5px; border:0px; background-color:#FFFFAA; color:black; }'+
		'#dict td {padding-right:29px; vertical-align:top; border:0px; color:black; background-color:#FFFFAA;}'
	);
