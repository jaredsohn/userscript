// ==UserScript==
// @name             KOCAttack modif.!
// @version          0.0.0¡2
// @namespace        KOCAttack-Mod
// @homepage         http://userscripts.org/scripts/show/89473
// @description      El original Modificado
...
function inspect(obj, maxLevels, level){
  var str = '', type, msg;
    ...
    if(maxLevels < 1)     
        return '<font color="red">Error: El nivel debe ser  > 0</font>';

    // We start with a non null object
    if(obj == null)
    return '<font color="red">Error: Objeto <b>NULO</b></font>';
...
var KOCAttack={
	...
	DoUnsafeWindow:function(func, execute_by_embed) {
		if(this.isChrome || execute_by_embed) {
			...
			try {  
				...
				this.Log("Ha ocurrido un error de javaScript. Descripcion del error: "+error.description);
			}
		}
	},
	...
	ShowImportExportBox:function() {
		...
		var m = '<a id="ImportBoxCancel" class="button20"><span>Cancelar</span></a>';
		m += '<a id="ImportData" class="button20"><span>Importar</span></a>';
		m += '<a id="ExportData" class="button20"><span>Exportar</span></a>&nbsp;';
		...
		citysel += '<option value=options>Opciones</option>';		
		citysel += '</select>';
		...
		//WinLog.write(inspect(cities));
		var citysel ='<select id=srcCity>';
		citysel += '<option value=All>Todas las ciudades</option>';
		for(var c=0; c<cities.length; c++) {
		...
		levelsel += '<option value="All">Todos los niveles</option>';
		for(var c=1; c<=10; c++) {
		...
		typesel += '<option value="All">Todo</option>';
		typesel += '<option value="normal">Ataques</option>';
		typesel += '<option value="transport">Transportes</option>';
		typesel += '<option value="bulkadded">Añadir en bloque</option>';
		typesel += '</select>';
		
		div.style.display='block';
		div.innerHTML='';
		var m = '<DIV id="srcAttackOpts" style="height:30px">\
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Buscar: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Todo</option>\
		<OPTION value=Camp>Barbaros</option>\
		<OPTION value=Wilderness>Tierras Virgenes</option>\
		...
		<td class=xtab width=100 align=center>Sel. ciudad: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Sel. Nivel: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Sel. Tipo: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>Cerrar</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Lista de ataques</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Borrar lista</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Borrar seleccionados</span></a>\
		<a id="KOCAttackViewAttacksIgnore" class="button20"><span>Ignorar/no ignorar</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>City</td><td>Coordenadas</td><td nowrap>What</td>';
		  h += '<td>Type</td><td>Tropas</td><td>1er ataque/Recursos</td><td>Tiempo</td><td>Ignorar</td></tr>';
		  var tableRows= '';
		  var count = 1
		  for(var a=0; a<t.attacks.length; a++) {
			...
			tableRows += '<td id=coords'+count+'><DIV onclick="PTgotoMap('+ t.attacks[a].xy[0] +','+ t.attacks[a].xy[1] +')"><A>'+ t.attacks[a].xy[0] +','+ t.attacks[a].xy[1] +'</a></div></td>';
			...
			if(typeof(t.attacks[a].troops)=="object") {
			  ...
			  for(var i=1; i<t.attacks[a].troops.length; i++) {
					switch(i){
						case 1:tipo='T. suministro';
						  break;
						case 2:tipo='Milicianos';
						  break;
						case 3:tipo='Exploradores';
						  break;
						case 4:tipo='Piqueros';
						  break;
						case 5:tipo='Espadas';
						  break;
						case 6:tipo='Arqueros';
						  break;
						case 7:tipo='Caballeria';
						  break;
						case 8:tipo='Cab. pesada';
						  break;
						case 9:tipo='Carretas';
						  break;
						case 10:tipo='Balistas';
						  break;
						case 11:tipo='Arietes';
						  break;
						case 12:tipo='Catapultas';
						  break;
					}
				if(i==2 || i==6){
			    troops += '<b>'+tipo+': </b>'+t.attacks[a].troops[i]+'<br/>';
				}
   			}
			  tableRows += troops;
  		}
      ...
      tableRows += '<td>';
			 if(!t.attacks[a].suicidewave && !t.attacks[a].resources){
				 var suicide='<span style=color:#cc0000><b>Desconfigurado</b></span>';
				 tableRows += suicide;
				 }
			if(typeof(t.attacks[a].suicidewave)=="object") {
			  var tropa='';
			  if(t.attacks[a].suicidewave.length>0 ){
				tropa=t.attacks[a].suicidewave[2];
				  suicide='<span style=color:#215c00><b>Correcto</b> <br/>'+tropa+' milicianos</span>';
		  }
			  ...
			  for(var i=1; i<t.attacks[a].resources.length; i++) {
			    ...
			    resources += '<b>'+tipo+':</b>'+t.attacks[a].resources[i]+'<br/>';
   			}
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Orden de ataque: <select id='KOCAttackOrder'><option value='mostTroops'>Mayor cantidad de tropas primero</option><option value='closest'>Mas cercanos primero</option></select><br />"+
			"Tipo de ataque: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbaros "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Tierras virgenes "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>ciudades "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transportar<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> Segundos que transcurren entre cada ataque<br />"+
			"Tiempo que transcurrira entre cada misión...<br />"+
			"<div style='margin-left: 40px'>"+
			"Ataque a barbaros:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />hrs<br />"+
			"Ataque a tierra virgen:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />hrs<br />"+
			" Ataque a ciudades:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />hrs<br />"+
			" Transportes:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />mins"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> Maxima distancia desde la ciudad al objetivo.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Lanzar los ataques  desdes la ciudad donde se lanzaron por primera vez<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Intentar la marcha si hay un error por exceso de trafico (recergar la página al solicitar esta opcion)<br />"+
			"Abrir esta URL en una pestaña cuando se sea atacado...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Borrar automaticamente ataque a barbaros y tierras virgenes<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Numero de informes de ataque en la ventana de marchas<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> No ver ningún informe. Tampoco se mostraran los de la página de marcha<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de población sin trabajo disponible antes de entrenar automaticamente.<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% felicidad de la poblacion antes de auto cobrar oro.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> Desactivar la molesta ventana de invitar amigos<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> ayudar automaticamente desde el chat a los miembros de la alianza en sus construcciones.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> esconder automaticamente los mensajes de ayuda en el chat (despues de haber ayudado)<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> publicar automaticamente ayudas en tu muro de facebook.<br />"+
			"si esta seleccionado y se pide confidencialidad, Sel.? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Todos</option><option value='50'>Amigos de amigos</option><option value='40'>Solo amigos</option><option value='10'>Solo yo</option></select><br />"+
			"</td></tr>"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Loguearse automaticamente en caso del que el servidor este caido o tras un largo tiempo en mantenimiento<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Permitir la consola de errores de firefox en caso de que se encuentre un fallo<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> segundos que transcurriran entre cambio de ciudades<br />"+
			"Realizar <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> ciclos y esperar  "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />segundos antes de refrescar.<br />"+
			"Sel. las ciudades desde donde lanzar ataques: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% ajuste de tardanza entre ataques (para que parezca mas humano).<br />"+
			"Importar/Exportar: Copia o pega aquí...<br /><textarea id='KOCAttackImport'></textarea>"+
			...
			"<a id='KOCAttackOptionsSave' class='button20'><span>Guardar</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Cancelar</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Borrar opciones</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Borrar todo!</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Borrar todos los ataques</span></a><a id='KOCAttackDeleteTransports' class='button20'><span>Borarr los transportes</span></a><a id='KOCAttackListAttacks' class='button20'><span>Lista de ataques</span></a>"+
			"<a id='KOCAttackListTransports' class='button20'><span>Lista de transportes</span></a></td><TD align=right>"+ KOCAversion +"</td></tr></form>";
		var t=this;
		...
		ById('KOCAttackOptionsSave').addEventListener('click',function() {
			...
			if(prev_disableInviteFriends != t.options.disableInviteFriends){
				alert("Para cambiar la opcion de invitar amigos debes refrescar la pagina!!!");
			}
...
// 		resources[4] = parseInt(Seed.resources['city'+cityID]['rec'+4][0] / 3600);
	...
	*/
	...
	SetupMenu:function() {
		...
		GM_registerMenuCommand('KOCAttack - Ver ataques',function() {
			t.ShowViewAttacksDialog();
		});
