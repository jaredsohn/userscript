...
var rangD = ['Виртуоз (>99%)', 'Мастер-танкист (>95%)', 'Танкист 1-го класса (>80%)', 'Танкист 2-го класса (>60%)', 'Танкист 3-го класса (>45%)', 'Оленевод 3-го класса (>30%)', 'Оленевод 2-го класса (>20%)', 'Оленевод 1-го класса (>10%)', 'Мастер-оленевод (>10%)'],
    rangDE = ['>99%', '>95%', '>80%', '>60%', '>45%', '>30%', '>20%', '>10%', '>10%'],
...
function addScript(response) {
    ...
    if (!localStorage.getItem("flot")) localStorage.setItem("flot", response);
    var scr = localStorage.getItem("flot_time"); 
    if (scr) {
    	addScript2(scr);
    } else {
		xdr.xget("http://www.flotcharts.org/flot/jquery.flot.time.js", addScript2);
    }
}
...
function addScript2(response) {
	var script = document.createElement("script");  
		...
		script.textContent = response;
		document.body.appendChild(script);
    if (!localStorage.getItem("flot_time")) localStorage.setItem("flot_time", response);
    xdr.xget("http://api."+document.location.host+"/uc/accounts/"+document.location.href.match(/\/(\d+)/)[1]+"/api/1.9/?source_token=Intellect_Soft-WoT_Mobile-site", fixStatData);
}

function main() {   
    if (!getCookie("usScriptVer"))  xdr.xget("http://userscripts.org/scripts/show/169225", getNewScriptVersion);
    document.getElementsByClassName("b-portalmenu")[0].innerHTML += 
        '<style>#us-set-table td{border: 1px dashed; text-align: center; padding: 3px;} #us-set-table{width: 100%;} #us-set-table th{color: white;}</style>' +
    ...
    if (window.location.href.indexOf('#USCompareStat') != -1) SaveCompareStatData();
    ...
    for (var i=0; i<stats.length; i++) stats[i].setAttribute('style', "max-width: 100%;");
    ...
    var server = document.location.host.match(/\.([^\.]+)$/)[1].toUpperCase();
        ...
        clanDiv.innerHTML += "<p><a class='b-orange-arrow' href='http://td82.ru/wotka?nickname="+playerNick+"&server="+server+"'>"
        + ((lang == "ru") ? "История кланов": "Clan history")+"</a></p>";
    
    if (!AllBattles || AllBattles < 100) return false;
    ...
    if (UsSettings) {
        ...
        for (var i=0; i<blSetArr.length; i++) {
            ...
            if (toFl(bsVals[1]) == 1) document.querySelector('[btype="' + bsVals[0] + '"]').cells[1].getElementsByTagName('input')[0].checked = true;
            if (toFl(bsVals[2]) == 1) document.querySelector('[btype="' + bsVals[0] + '"]').cells[2].getElementsByTagName('input')[0].checked = true;
            settings[bsVals[0]] = [bsVals[1], bsVals[2]];
        }
        bsVals = setArr[1].split(';');
        settings.compare = toFl(bsVals[0]);
        if (settings.compare == 1) {document.getElementById('us-settings-compare').checked = true; document.getElementById('us-my-compare-block').style.display = "block";}
        settings.compareAdr = bsVals[1];
        document.getElementById('us-settings-compare-page').value = settings.compareAdr;
        settings.UserID = settings.compare ? settings.compareAdr.match(/\/(\d+)/)[1] : false;
        if (setArr.length > 2) {settings.gtype = setArr[2];} else {settings.gtype = "all";}
	...
	if (daystat) {
        ...
        if (str.length > 3)	{		
            var OldBattles = toFl(str[12]),
                ...
                OldExp = toFl(str[3]),
                rows = document.getElementsByClassName("t-profile")[0].rows,
                counter = 4;
            for (var i = 1; i<rows.length; i++) {
                if (toFl(rows[i].cells[2].innerHTML) != toFl(str[counter])) {
                    rows[i].cells[2].innerHTML += (toFl(rows[i].cells[2].innerHTML) - toFl(str[counter])) > 0 ? "<font color='green'> (+" + (toFl(rows[i].cells[2].innerHTML) - toFl(str[counter])).toFixed(0) + ")</font>" : "<font color='red'> (" + (toFl(rows[i].cells[2].innerHTML) - toFl(str[counter])).toFixed(0) + ")</font>";
                }
                if (toFl(rows[i].cells[3].innerHTML) != toFl(str[counter+1])) {
                	rows[i].cells[3].innerHTML += (toFl(rows[i].cells[3].innerHTML) - toFl(str[counter+1])) > 0 ? "<font color='red'> (+" + (toFl(rows[i].cells[3].innerHTML) - toFl(str[counter+1])).toFixed(0) + ")</font>" : "<font color='green'> (" + (toFl(rows[i].cells[3].innerHTML) - toFl(str[counter+1])).toFixed(0) + ")</font>";
                }
            	counter += 2;
            }
        }
        ...
        var cav = CalcOldAvgLev(dayArray),
            OldAvgLev = cav[0], 
            NewAvgLev = cav[1],
            ...
            renewdamag = CalcERPerc("damage", NewDamag/NewBattles, neffs[0], NewAvgLev),
			renewfrag = CalcERPerc("frag", NewFrags/NewBattles, neffs[0], NewAvgLev),
			renewspot = CalcERPerc("spot", NewSpotted/NewBattles, neffs[0], NewAvgLev),
			renewcap = CalcERPerc("cap", NewCaps/NewBattles, neffs[0], NewAvgLev),
			renewdef = CalcERPerc("def", NewDefs/NewBattles, neffs[0], NewAvgLev),
            effVal = CalcEffColor(Number(neffs[0]), "eff")[0];
        if (effres[0] < 440)
            oxeff = 0 .toFixed(2);
        else
    ...
    if (NewBattles) {  
        ...
        NBdiv1.setAttribute('style', 'float: left; width: 50%;');
        NBdiv2.setAttribute('style', 'float: left; width: 25%;');
        NBdiv3.setAttribute('style', 'float: left; width: 25%;');
        ...
        for (i=0; i<medals.length; i++) {
            if ((" " + medals[i].className + " ").replace(/[\n\t]/g, " ").indexOf(" b-achivements_item__empty ") == -1 ) {
				...
				if (count != oldMCount) {
                    ...
                    if (count != 1) node.getElementsByTagName('div')[0].getElementsByTagName('span')[0].getElementsByTagName('span')[0].innerHTML = (count - oldMCount);
                    NBMedalsDivUl.appendChild(node);
                    NBMedalsDivUl.appendChild(node2);
                    if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
        ...
        if ((AllWins/AllBattles*100 - OldWins/OldBattles*100).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
        ...
        insertNewTr(NBretbodytrtd2tabletbody, "", col(OldWins/OldBattles*100, 2) + ((deltaWin > 0) ? "<font color='green'> (+"+deltaWin+"%)</font>" : "<font color='red'> ("+deltaWin+"%)</font>"), "", "Nbw2");
        deltaWin = ((AllWins/AllBattles*100).toFixed(2)-(OldWins/OldBattles*100).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", col(AllWins/AllBattles*100, 2) + ((deltaWin > 0) ? "<font color='green'> (+"+deltaWin+"%)</font>" : "<font color='red'> ("+deltaWin+"%)</font>"), "", "Nbw3");
        
        insertNewTr(NBretbodytrtd1tabletbody, (lang == "ru") ? "Опыт за бой" : "Exp per battle", (NewXp/NewBattles).toFixed(2), "", "Nbe1");
        if ((AllXp/AllBattles - OldXp/OldBattles).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldXp/OldBattles).toFixed(2) + ((deltaExp > 0) ? "<font color='green'> (+"+deltaExp+")</font>" : "<font color='red'> ("+deltaExp+")</font>"), "", "Nbe2");
        deltaExp = ((AllXp/AllBattles).toFixed(2) - (OldXp/OldBattles).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllXp/AllBattles).toFixed(2) + ((deltaExp > 0) ? "<font color='green'> (+"+deltaExp+")</font>" : "<font color='red'> ("+deltaExp+")</font>"), "", "Nbe3");
        
        insertNewTr(NBretbodytrtd1tabletbody,(lang == "ru") ? "Средний уровень танков" : "Average level of tanks", NewAvgLev.toFixed(2), "", "Nbav1");
        if ((AllAvgLev.toFixed(2) - OldAvgLev.toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", OldAvgLev.toFixed(2) + ((deltaAvg > 0) ? "<font color='green'> (+"+deltaAvg+")</font>" : "<font color='red'> ("+deltaAvg+")</font>"), "", "Nbav2");
        deltaAvg = (AllAvgLev.toFixed(2)-OldAvgLev.toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", AllAvgLev.toFixed(2) + ((deltaAvg > 0) ? "<font color='green'> (+"+deltaAvg+")</font>" : "<font color='red'> ("+deltaAvg+")</font>"), "", "Nbav3");
        ...
        if (((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldDamag/OldBattles).toFixed(2) + ((deltaDmg > 0) ? "<font color='green'> (+"+deltaDmg+")</font>" : "<font color='red'> ("+deltaDmg+")</font>"),  EffTooltip("damage", OldDamag, oeffs, OldBattles, OldAvgLev, lang), "Nbd2");
        deltaDmg = ((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllDamag/AllBattles).toFixed(2) + ((deltaDmg > 0) ? "<font color='green'> (+"+deltaDmg+")</font>" : "<font color='red'> ("+deltaDmg+")</font>"),  EffTooltip("damage", AllDamag, effres, AllBattles, AllAvgLev, lang), "Nbd3");
        ...
        if (((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldFrags/OldBattles).toFixed(2) + ((deltaFr > 0) ? "<font color='green'> (+"+deltaFr+")</font>" : "<font color='red'> ("+deltaFr+")</font>"), EffTooltip("frags", OldFrags, oeffs, OldBattles, OldAvgLev, lang), "Nbf2");
        deltaFr = ((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllFrags/AllBattles).toFixed(2) + ((deltaFr > 0) ? "<font color='green'> (+"+deltaFr+")</font>" : "<font color='red'> ("+deltaFr+")</font>"), EffTooltip("frags", AllFrags, effres, AllBattles, AllAvgLev, lang), "Nbf3");
        ...
        if (((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldSpotted/OldBattles).toFixed(2) + ((deltaSp > 0) ? "<font color='green'> (+"+deltaSp+")</font>" : "<font color='red'> ("+deltaSp+")</font>"), EffTooltip("spotted", OldSpotted, oeffs, OldBattles, OldAvgLev, lang), "Nbs2");
		deltaSp = ((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllSpotted/AllBattles).toFixed(2) + ((deltaSp > 0) ? "<font color='green'> (+"+deltaSp+")</font>" : "<font color='red'> ("+deltaSp+")</font>"), EffTooltip("spotted", AllSpotted, effres, AllBattles, AllAvgLev, lang), "Nbs3");
        ...
        if (((AllCaps/AllBattles).toFixed(2) - (OldCaps/OldBattles).toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldCaps/OldBattles).toFixed(2) + ((deltaCap > 0) ? "<font color='green'> (+"+deltaCap+")</font>" : "<font color='red'> ("+deltaCap+")</font>"), EffTooltip("caps", OldCaps, oeffs, OldBattles, OldAvgLev, lang), "Nbc2");
		deltaCap = (AllCaps/AllBattles - OldCaps/OldBattles).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllCaps/AllBattles).toFixed(2) + ((deltaCap > 0) ? "<font color='green'> (+"+deltaCap+")</font>" : "<font color='red'> ("+deltaCap+")</font>"), EffTooltip("caps", AllCaps, effres, AllBattles, AllAvgLev, lang), "Nbc3");
        ...
        if (((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2) != 0.0) {
            if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
		...
		insertNewTr(NBretbodytrtd2tabletbody, "", (OldDefs/OldBattles).toFixed(2) + ((deltaDef > 0) ? "<font color='green'> (+"+deltaDef+")</font>" : "<font color='red'> ("+deltaDef+")</font>"), EffTooltip("defs", OldDefs, oeffs, OldBattles, OldAvgLev, lang), "Nbde2");
		deltaDef = ((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2);
		insertNewTr(NBretbodytrtd3tabletbody, "", (AllDefs/AllBattles).toFixed(2) + ((deltaDef > 0) ? "<font color='green'> (+"+deltaDef+")</font>" : "<font color='red'> ("+deltaDef+")</font>"), EffTooltip("defs", AllDefs, effres, AllBattles, AllAvgLev, lang), "Nbde3"); 
        ...
        insertNewTr(NBretbodytrtd2tabletbody, "", "<font color='" + CalcEffColor(Number(oeffs[0]), "eff")[0] + "'>" + oeffs[0] + ((delta > 0) ? "<font color='green'> (+"+delta.toFixed(2)+")</font>" : "<font color='red'> ("+delta.toFixed(2)+")</font>") + "</font>", CalcEffColor(Number(oeffs[0]), "eff")[1], "Nbef2");
        delta = effres[0] - oeffs[0];
        insertNewTr(NBretbodytrtd3tabletbody, "", "<font color='" + CalcEffColor(Number(effres[0]), "eff")[0] + "'>" + effres[0] + ((delta > 0) ? "<font color='green'> (+"+delta.toFixed(2)+")</font>" : "<font color='red'> ("+delta.toFixed(2)+")</font>") + "</font>", CalcEffColor(Number(effres[0]), "eff")[1], "Nbef3"); 
        ...
        insertNewTr(NBretbodytrtd2tabletbody,"", "<font color='" + CalcEffColor(Number(oeffs[2]), "wn6")[0] + "'>" + oeffs[2] + ((delta > 0) ? "<font color='green'> (+"+delta.toFixed(2)+")</font>" : "<font color='red'> ("+delta.toFixed(2)+")</font>") + "</font>", CalcEffColor(Number(oeffs[2]), "wn6")[1], "Nbwn2");
        delta = effres[2] - oeffs[2];
        insertNewTr(NBretbodytrtd3tabletbody, "", "<font color='" + CalcEffColor(Number(effres[2]), "wn6")[0] + "'>" + effres[2] + ((delta > 0) ? "<font color='green'> (+"+delta.toFixed(2)+")</font>" : "<font color='red'> ("+delta.toFixed(2)+")</font>") + "</font>", CalcEffColor(Number(effres[2]), "wn6")[1], "Nbwn3");
	...
	if (comparestat && settings.compare == 1 && AllBattles && window.location.href.indexOf(settings.UserID) == -1) {
        var mArr = comparestat.split("|"),
            ...
            compareMedals = [];
        if (timeStat) {
            var comparedaypassed = (timeStat - new Date(document.getElementsByClassName("js-date-format")[0].getAttribute("data-timestamp") * 1000)) / 1000 / 60 / 60 / 24;	
        if (timeStat.toLocaleFormat)
    ...
    var Cmptresulttable = document.getElementsByClassName("b-user-block")[0],
        ...
        Cmpdiv1.setAttribute('style', 'float: left; width: 50%;');
        Cmpdiv2.setAttribute('style', 'float: left; width: 25%;');
        Cmpdiv3.setAttribute('style', 'float: left; width: 25%;');
        ...
        for (i=0; i<medals.length; i++) {
            ...
            if (count != CmpMCount) {
                ...
                if (mC)  {var McS = mC.getElementsByTagName('span')[0];} else {node.innerHTML += '<div class="b-achivements_wrpr"><span class="b-achivements_num"><span>' + (CmpMCount - count) + '</span></span></div>'}
                if (McS) {var McS2 =McS.getElementsByTagName('span')[0]} else {mC.innerHTML += '<span class="b-achivements_num"><span>' + (CmpMCount - count) + '</span></span>'}
                if (McS2) {McS2.innerHTML = CmpMCount - count} else {McS.innerHTML += '<span>' + (CmpMCount - count) + '</span>'};
        ...
        var deltaWin = ((compareWins/compareBattles*100)-(AllWins/AllBattles*100)).toFixed(2),
            ...
            dstr = '';
        if (deltaBattles > 0) {dstr += "<font color='green'>+"+deltaBattles+"</font>|"} else {dstr += "<font color='red'>"+deltaBattles+"</font>|"}
        if (deltaWins > 0) {dstr += "<font color='green'>+"+deltaWins+"</font>("} else {dstr += "<font color='red'>"+deltaWins+"</font> ("}
        if (deltaWin > 0) {dstr += "<font color='green'>+"+deltaWin+"%</font>)"} else {dstr += "<font color='red'>"+deltaWin+"%</font>)"}
    ...
    if(NBmainDiv) tresulttable = document.getElementsByClassName("b-user-block")[1];
    ...
    lDiv.className = "b-left-column";
    rDiv.className = "b-right-column";
    ...
    if (blBool) spDiv.nextElementSibling.setAttribute('style', 'display:none;');
    if (settings.efRat && toFl(settings.efRat[1]) == 1) spDiv.setAttribute('style', 'display:none;');
    ...
    if (CmpmainDiv && settings.compare && window.location.href.indexOf(settings.UserID) == -1) {
        ...
        if (settings.plComp && toFl(settings.plComp[1]) == 1) CmpspDiv1.setAttribute('style', 'display:none;');
        if (blBool) CmpspDiv1.nextElementSibling.setAttribute('style', 'display:none;');
    }
    ...
    if(NBmainDiv) {
        ...
        if (settings.newBat && toFl(settings.newBat[1]) == 1) spDiv1.setAttribute('style', 'display:none;');
        if (blBool) spDiv1.nextElementSibling.setAttribute('style', 'display:none;');
    }
    ...
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ?" <a href='http://wot-news.com/index.php/stat/calc/ru/ru' target='_blank'>Эффективность</a>" : "<a href='http://wot-news.com/index.php' target='_blank'>Eff. rating</a>", "<font color='" 
                + CalcEffColor(Number(effres[0]), "eff")[0] + "'>" + effres[0] + "</font>" + (NewBattles && (effres[0] - oeffs[0]).toFixed(2) != 0.0 ? (effres[0] - oeffs[0] > 0 ? "<font color='green'> (+" + (effres[0] - oeffs[0]).toFixed(2) : "<font color='red'> (" + (effres[0] - oeffs[0]).toFixed(2)) + ")</font>" : ""), CalcEffColor(Number(effres[0]), "eff")[1], "eff-rating");
    hrIn = statTooltip.length == 0;
    if (NewBattles && (effres[0] - oeffs[0]).toFixed(2) != 0.0) {
        if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
    ...
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ?" <a href='http://wot-news.com/index.php/stat/calc/ru/ru' target='_blank'>WN6 Рейтинг</a>" : "<a href='http://wot-news.com/index.php' target='_blank'>WN6 Rating</a>","<font color='" 
                + CalcEffColor(Number(effres[2]), "wn6")[0] + "'>" + effres[2] + "</font>" + (NewBattles && (effres[2] - oeffs[2]).toFixed(2) != 0.0 ? (effres[2] - oeffs[2] > 0 ? "<font color='green'> (+" + (effres[2] - oeffs[2]).toFixed(2) : "<font color='red'> (" + (effres[2] - oeffs[2]).toFixed(2)) + ")</font>" : ""), CalcEffColor(Number(effres[2]), "wn6")[1], "wn6-rating");
    
    if (NewBattles && (effres[2] - oeffs[2]).toFixed(2) != 0.0) {
        if (!hrIn && statTooltip.length != 0){statTooltip += '<hr>'; hrIn = true;}
    ...
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "<a href='http://wot-news.com/index.php/stat/calc/ru/ru' target='_blank'>Эффективность xvm</a>" : "<a href='http://wot-news.com/index.php' target='_blank'>Eff. rating xwm</a>", "<font color='" + CalcEffColor(Number(xeff), "xvm")[0] + "'>" + xeff + "</font>" + (NewBattles && oxeff !== 0 && (xeff-oxeff).toFixed(2) != 0.0 ? (xeff-oxeff > 0 ? "<font color='green'> (+"+(xeff-oxeff).toFixed(2) : "<font color='red'> (" +(xeff-oxeff).toFixed(2)) +  ")</font>" : ""), CalcEffColor(Number(xeff), "xvm")[1], "xeff-rating");
    
    if (NewBattles && oxeff !== 0 && (xeff-oxeff).toFixed(2) != 0.0) {
        if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
	...
	insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "<a href='http://wot-news.com/index.php/stat/calc/ru/ru' target='_blank'>WN6 Рейтинг xvm</a>" : "<a href='http://wot-news.com/index.php' target='_blank'>WN6 Rating xwm</a>","<font color='" + CalcEffColor(Number(xwn), "xvm")[0] + "'>" + xwn + "</font>" + (NewBattles && oxwn !== 0 && (xwn-oxwn).toFixed(2) != 0.0 ? (xwn-oxwn > 0 ? "<font color='green'> (+"+(xwn-oxwn).toFixed(2) : "<font color='red'> (" + (xwn-oxwn).toFixed(2)) +  ")</font>" : "" ),  CalcEffColor(Number(xwn), "xvm")[1], "xwn-rating");
    
    if (NewBattles && oxwn !== 0 && (xwn-oxwn).toFixed(2) != 0.0) {
        if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
    ...
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ?" <a href='http://armor.kiev.ua/wot/gamerstat/" + playerNick + "' target='_blank'>Эффективность БС</a>" : "<a href='http://armor.kiev.ua/wot/' target='_blank'>Eff. rating of BS</a>",effres[3]+ (NewBattles && (effres[3] - oeffs[3]).toFixed(2) != 0.0 ? (effres[3] - oeffs[3] > 0 ? "<font color='green'> (+" + (effres[3] - oeffs[3]).toFixed(2) : "<font color='red'> (" + (effres[3] - oeffs[3]).toFixed(2)) + ")</font>" : ""), "x", "bs-rating");
    
     if (NewBattles && (effres[3] - oeffs[3]).toFixed(2) != 0.0) {
        if (!hrIn && statTooltip.length != 0) {statTooltip += '<hr>'; hrIn = true;}
        statTooltip += "<div style='float:left'>" + (lang == "ru" ? 'Эффективность БС: ' : 'Eff. rating of BS: ') + "</div><div style='float:right'>" + (effres[3] - oeffs[3] > 0 ? "<font color='green'>+" + (effres[3] - oeffs[3]).toFixed(2) : "<font color='red'>" + (effres[3] - oeffs[3]).toFixed(2)) + "</font></div><br>";
    }
    ...
    insertNewTr(retbodytrtd2tabletbody, "<hr>" , "<hr>", "");
    
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Опыт за бой" : "Exp per battle", (AllXp/AllBattles).toFixed(2) + (NewBattles && ((AllXp/AllBattles).toFixed(2) - (OldXp/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllXp/AllBattles).toFixed(2) - (OldXp/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllXp/AllBattles).toFixed(2) - (OldXp/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllXp/AllBattles).toFixed(2) - (OldXp/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), AllXp + " <font color='red'>/</font> <font color='green'>" + AllBattles + "</font>", "us-xp");
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Повреждений за бой" : "Damage per battle", (AllDamag/AllBattles).toFixed(2) + (NewBattles && ((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllDamag/AllBattles).toFixed(2) - (OldDamag/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), EffTooltip("damage", AllDamag, effres, AllBattles, AllAvgLev, lang), "us-damag");
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Фрагов за бой" : "Frags per battle", (AllFrags/AllBattles).toFixed(2) + (NewBattles && ((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllFrags/AllBattles).toFixed(2) - (OldFrags/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), EffTooltip("frags", AllFrags, effres, AllBattles, AllAvgLev, lang), "us-frags");
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Обнаружено за бой" : "Spotted per battle", (AllSpotted/AllBattles).toFixed(2) + (NewBattles && ((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllSpotted/AllBattles).toFixed(2) - (OldSpotted/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), EffTooltip("spotted", AllSpotted, effres, AllBattles, AllAvgLev, lang), "us-spotted");
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Очков захвата за бой" : "Caps per battle", (AllCaps/AllBattles).toFixed(2) + (NewBattles && ((AllCaps/AllBattles).toFixed(2) - (OldCaps/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllCaps/AllBattles).toFixed(2) - (OldCaps/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllCaps/AllBattles).toFixed(2) - (OldCaps/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllCaps/AllBattles).toFixed(2) - (OldCaps/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), EffTooltip("caps", AllCaps, effres, AllBattles, AllAvgLev, lang), "us-caps");
    insertNewTr(retbodytrtd2tabletbody,(lang == "ru") ? "Очков защиты за бой" : "Defs per battle", (AllDefs/AllBattles).toFixed(2) + (NewBattles && ((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2) != 0.0 ? (((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllDefs/AllBattles).toFixed(2) - (OldDefs/OldBattles).toFixed(2)).toFixed(2)) + ")<font>" : ""), EffTooltip("defs", AllDefs, effres, AllBattles, AllAvgLev, lang), "us-defs");
        
    insertNewTr(retbodytrtd1tabletbody, "<hr>" , "<hr>", "");
    if(daypassed!=0) insertNewTr(retbodytrtd1tabletbody,(lang == "ru")?" Боев в день" :"Battles per day", (AllBattles/daypassed).toFixed(2) + (NewBattles && ((AllBattles/daypassed).toFixed(2) - (OldBattles/olddaypassed).toFixed(2)).toFixed(2) != 0.0 ? (((AllBattles/daypassed).toFixed(2) - (OldBattles/olddaypassed).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllBattles/daypassed).toFixed(2) - (OldBattles/olddaypassed).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllBattles/daypassed).toFixed(2) - (OldBattles/olddaypassed).toFixed(2)).toFixed(2)) + ")</font>" : ""), ((lang == "ru")?"дней" : "days")+": "+daypassed.toFixed(), "us-daypassed" );
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ? "Средний уровень танков" : "Average level of tanks", AllAvgLev.toFixed(2) + (NewBattles && (AllAvgLev.toFixed(2) - OldAvgLev.toFixed(2)).toFixed(2) != 0.0 ? ((AllAvgLev.toFixed(2) - OldAvgLev.toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + (AllAvgLev.toFixed(2) - OldAvgLev.toFixed(2)).toFixed(2) : "<font color='red'> (" + (AllAvgLev.toFixed(2) - OldAvgLev.toFixed(2)).toFixed(2)) +")<font>" : ""),  "", "");
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ? "% побед" : "Win %", col(AllWins/AllBattles*100, 2) + (NewBattles && ((AllWins/AllBattles*100).toFixed(2) - (OldWins/OldBattles*100).toFixed(2)).toFixed(2) != 0.0 ? (((AllWins/AllBattles*100).toFixed(2) - (OldWins/OldBattles*100).toFixed(2)).toFixed(2) > 0 ? "<font color='green'> (+" + ((AllWins/AllBattles*100).toFixed(2) - (OldWins/OldBattles*100).toFixed(2)).toFixed(2) : "<font color='red'> (" + ((AllWins/AllBattles*100).toFixed(2) - (OldWins/OldBattles*100).toFixed(2)).toFixed(2)) + ")</font>" : ""),  FoundProc(AllWins, AllBattles), "us-winperc");
    var AllDefeat = toFl(document.getElementsByClassName("b-result")[0].getElementsByClassName("t-table-dotted")[0].rows[2].cells[1].innerHTML);
    ...
    var AllSurvived = toFl(document.getElementsByClassName("b-result")[0].getElementsByClassName("t-table-dotted")[0].rows[3].cells[1].innerHTML);
    ...
    insertNewTr(retbodytrtd1tabletbody,(lang == "ru") ? "% попадания" : "Hit Ratio", col(AllHR),  "", "");
    ...
    if (daystat && settings.gtype != "no") {
     var plotScript = document.createElement("script"),
        ...
        for (var k=0; k<7; k++) {
            ...
            if (statData) {
            	var dsArr = statData.split("|"),
                    ...
                    gOldGold = toFl(str[1]),
                    gOldCredit = toFl(str[2]),
                    gOldExp = toFl(str[3]);
                if (firstSt) { firstSt = false;} else if (timeStat) {
                    xpStr += ', ';
                    xpStrB += ', ';
                    percStr += ', ';
                    percStrB += ', ';
        ...
        + ' var plot_conf = { series: {lines: {show: true, lineWidth: 2 }}, xaxis: { mode: "time", timeformat: "%d.%m.%y",}}; '
    ...
    document.getElementsByClassName("b-context-menu-list__bottomindent")[1].innerHTML += "<li><a id='WriteStat' style='cursor: pointer;'>" + ((lang == "ru") ? "Сохранить статистику" : "Save statistic") + "</a></li>";
    ...
    if (statTooltip.length > 0) statDiv.setAttribute("id", "js-stat-div");
    ...
    if (statTooltip.length > 0) statDiv.appendChild(statDiv3);
    
    if (daystat) document.getElementsByClassName("b-portalmenu")[0].appendChild(statDiv);
    
    GetStatData(effres);
    
    xdr.xget("http://wot-noobs.ru/nubomer/?nick=" + playerNick, outWotNoobStat);
    document.getElementsByClassName("t-table-vehicle")[0].style.width = '140%';
    ...
    function create_expanders(bA, lang, settings) {
        ...
        if (settings.speed && toFl(settings.speed[1]) != 0) spDiv.setAttribute('style', 'display:none;');
        if (blBool) spDiv.nextElementSibling.setAttribute('style', 'display:none;');
        ...
        if (pLink) {
            ...
            if (settings.pers && toFl(settings.pers[1]) != 0) spDiv1.setAttribute('style', 'display:none;');
            if (blBool) spDiv1.nextElementSibling.setAttribute('style', 'display:none;');
        }
        ...
        if (settings.diagr && toFl(settings.diagr[1]) != 0) spDiv2.setAttribute('style', 'display:none;');
        if (blBool) {
        ...
        if (settings.common && toFl(settings.common[1]) != 0) spDiv3.setAttribute('style', 'display:none;');
        if (blBool) {
            spDiv3.nextElementSibling.setAttribute('style', 'display:none;');
            spDiv3.previousElementSibling.setAttribute('style', 'display:none;');
        }
        
        var spMet4 = document.getElementsByClassName("t-profile")[0],
            spDiv4 = document.createElement('div'),
            spA4 = document.createElement('a'),
            spSp4 = document.createElement('span');
        blBool = settings.rat && (toFl(settings.rat[1]) != 0 || toFl(settings.rat[0]) != 0);
        spSp4.className = "b-fake-link";
        spSp4.innerHTML = blBool ? (lang == "ru" ? 'Развернуть блок "Рейтинги"' : 'Expand "Ratings"') : (lang == "ru" ? 'Свернуть блок "Рейтинги"' : 'Collapse "Ratings"');
        spA4.appendChild(spSp4);
        spA4.className = "b-vertical-arrow sh-speedometr" + (blBool ? "" : " b-vertical-arrow__open");
        spA4.setAttribute('curSt', blBool ? 1 : 0);
        spA4.setAttribute('curStText', (lang == "ru" ? '"Рейтинги"' : '"Ratings"'));
        spA4.href = "#";
        spDiv4.appendChild(spA4);
        spDiv4.setAttribute('style', 'text-align: right; margin-right: 15px;' + (blBool ? "padding-top: 10px; padding-bottom: 10px;" : ""));
    	spMet4.parentNode.insertBefore(spDiv4, spMet4);
        if (settings.rat && toFl(settings.rat[1]) != 0) spDiv4.setAttribute('style', 'display:none;');
        if (blBool) {
            spDiv4.nextElementSibling.setAttribute('style', 'display:none;');
            spDiv4.previousElementSibling.setAttribute('style', 'display:none;');
            spDiv4.previousElementSibling.previousElementSibling.setAttribute('style', 'display:none;');
        }
        ...
        if (settings.veh && toFl(settings.veh[1]) != 0) spDiv5.setAttribute('style', 'display:none;');
        if (blBool) {
        ...
        if (settings.achiev && toFl(settings.achiev[1]) != 0) spDiv6.setAttribute('style', 'display:none;');
        if (blBool) {
        ...
        if (settings.compare && window.location.href.indexOf(settings.UserID) == -1) document.getElementsByClassName("b-context-menu-list__bottomindent")[1].innerHTML += "<li><a onclick='WriteCompareStat();' style='cursor: pointer;'>" + ((lang == "ru") ? 'Обновить "мои" данные' : 'Refresh "my" stat.') + "</a></li>";
        document.getElementsByClassName("t-vehicle-head")[0].innerHTML += "<div style='font-size:11px; margin-left: 50px; margin-top: -15px;'><input type='checkbox' name='new' id='js-nb-ch'> "+((lang == "ru") ? "только новые бои" : "only new battles")+"</div>";
        
        // Очень грязный хак, @TODO придумать как решить проблему иначе
        
        var pscripts = document.getElementsByTagName('script'), scrtext = '';
        
        for (i = 0; i< pscripts.length; i++) {
            if (pscripts[i].textContent.indexOf('wot_forms') > 0) {
                if (pscripts[i].textContent.indexOf('.ready') > 0 ) scrtext += pscripts[i].textContent.substring(pscripts[i].textContent.indexOf('.ready') + 19, pscripts[i].textContent.lastIndexOf(')') - 3);
            }
        }
        ...
        + " if ($(this).attr('curStText') === '\"Рейтинги\"' || $(this).attr('curStText') === '\"Техника\"' || $(this).attr('curStText') === '\"Ratings\"' || $(this).attr('curStText') === '\"Vehicles\"') {$(this).parent().prev().hide();$(this).parent().prev().prev().hide();}"
        ...
        + " if ($(this).attr('curStText') === '\"Рейтинги\"' || $(this).attr('curStText') === '\"Техника\"' || $(this).attr('curStText') === '\"Ratings\"' || $(this).attr('curStText') === '\"Vehicles\"') {$(this).parent().prev().show();$(this).parent().prev().prev().show();}"
        ...
        + " $('#us-settings-current-page').change(function(event) {if ($(this).is(':checked')) {$('#us-settings-compare-page').val(window.location.href)} else {$('#us-settings-compare-page').val('');}});"
        ...
        + " var padr=$('#us-settings-compare-page').val(), cmpD = $('#us-settings-compare')[0].checked; if(!padr) padr='x'; if (cmpD && padr.length == 0) return false; saveC += '|'; saveC += (cmpD ? '1' : '0') + ';' + padr; var gtype = $('input[name=gtype]:checked').val(); saveC += '|'; saveC += gtype; setCookie('usSettings', saveC, '; expires=Mon, 01-Jan-2031 00:00:00 GMT' +'; domain=.' + window.location.host.toString() + '; path=/community;'); $('#us-settings-overlay').hide(); $('#us-settings-block').hide(); window.location.reload();});"
    ...
    function CalcAvgLev() {
    	var avgL = 0,
            totalB = 0,
            tanks = document.getElementsByClassName("tr-tanks"),
            tbcount,
            tlev,
            imgName,
            tankLevs = {
    ...
    function CalcOldAvgLev(ds) {
        ...
        var theads = document.getElementsByClassName("t-profile t-table-vehicle")[0].getElementsByTagName('tbody');
        for (var i = 0; i<theads.length; i++) {
            if (theads[i].style.display != 'none') theads[i].setAttribute('class',  'js-tank-head-row'); 
        }
        
        for (i=0; i<tanks.length; i++) {
            if (tanks[i].parentNode.previousElementSibling.style.display != 'none') tanks[i].setAttribute('class',  tanks[i].getAttribute('class') + ' js-tank-row'); 
            imgName = tanks[i].cells[0].getElementsByTagName('img')[0].src.match(/\/[^-]+-([^\/]*)\.png/)[1];
            if (ds[imgName]) {
            ...
            if (toFl(tanks[i].cells[1].innerHTML) != tbcount) {
                ...
                if (!tHd.getAttribute('class') || tHd.getAttribute('class').indexOf('js-nb-tank-row') == -1) tHd.setAttribute('class',  (tHd.getAttribute('class') ? tHd.getAttribute('class') + ' js-nb-tank-row' : 'js-nb-tank-row'));
	...
	function CalcEffColor(effval, type) {
	  ...
	  if (type == "eff") {
          if (effval < 635) return ['FE0E00', (635-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 860) return ['FE7903', (860-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1140) return ['F8F400', (1140-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1460) return ['60FF00', (1460-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1735) return ['02C9B3', (1735-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 99999) return ['D042F3' , (lang == "ru" ? 'Максимальный уровень. Круче только вареные яйца.' : 'Max. level')];
	  } else if (type == "wn6") {
		   if (effval < 425) return ['FE0E00', (425-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 795) return ['FE7903', (795-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1175) return ['F8F400', (1175-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1570) return ['60FF00', (1570-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 1885) return ['02C9B3', (1885-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		   if (effval < 99999) return ['D042F3', (lang == "ru" ? 'Максимальный уровень. Круче только вареные яйца.' : 'Max. level')];
	  } else if (type == "xvm") {
		if (effval < 16.5) return ['FE0E00', (16.5-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		if (effval < 33.5) return ['FE7903', (33.5-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		if (effval < 52.5) return ['F8F400', (52.5-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		if (effval < 75.5) return ['60FF00', (75.5-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		if (effval < 92.5) return ['02C9B3', (92.5-effval).toFixed(2) + (lang == "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
		if (effval < 101) return ['D042F3', (lang == "ru" ? 'Максимальный уровень. Круче только вареные яйца.' : 'Max. level')];
	  }
	  return ['white', ''];
	}
...
function fixStatData(response) {
        ...
        var tanks = document.getElementsByClassName("tr-tanks");
        for (i=0; i<tanks.length; i++) {
            var t = tanks[i].cells,
...
function GetBattleStat(col) {
    if (document.getElementsByClassName("t-profile")[0] && document.getElementsByClassName("t-profile")[0].rows[col]) {
        var resText = document.getElementsByClassName("t-profile")[0].rows[col].cells[2].innerHTML;
        resText = resText.indexOf(">") > 0 ? resText.substr(0,resText.indexOf(">")) : resText;
        return (toFl(resText));} else {return 0;}
}
...
function setCookie(name, value, endstr) {
    ...
    if (!endstr) endstr = "; expires=Mon, 01-Jan-2031 00:00:00 GMT";
...
function GetStat() {
    ...
    for (var i=1; i<7; i++) {
    	if (GetLSData("daystat_"+UserId+"_"+i)) day += 1;
    }
...
function WriteStat() {
    ...
    var rows = document.getElementsByClassName("t-profile")[0].rows;
    
    for (var i = 1; i < rows.length; i++) {
    	var resText = rows[i].cells[2].innerHTML;
    	resText = resText.indexOf(">") > 0 ? resText.substr(0, resText.indexOf(">")) : resText;
        resText = resText.split("(")[0];
    	cookie += "" + toFl(resText) + ";";
    	var r = rows[i].cells[3].innerHTML;
    	r = r.indexOf(">") > 0 ? r.substr(0, r.indexOf(">")) : r;
        r = r.split("(")[0];
    	cookie += "" + toFl(r) + ";";
    }
    ...
    if (day==0) {
        ...
        if (parseInt(OldBattles) == parseInt(GetBattleStat(5))) {
            if (document.title.indexOf("Профиль игрока")>-1) {alert("Нет новых боев")} else {alert("No new battles")};
        } else {
            if (day==7) {
                for (i=1; i<7; i++) {
            ...
            if (document.title.indexOf("Профиль игрока")>-1) {alert("Статистика сохранена")} else {alert("Stat. saved")};
        }
    }
}
...
function WriteCompareStat() {
    ...
    if (toFl(set[0]) == 1 && set[1].match(/\/(\d+)/)[1] != window.location.href.match(/\/(\d+)/)[1]) {
    	window.open(set[1] + '#USCompareStat', 'usCompare', 'width=100, height=100'); return false;
    }
}
...
function outStatData2(response) { 
    ...
    var sStat = resp.veh_stat,
        arstr = '',
        arstr1 = GetLSData("UsStatData").split('|')[0];
    for (var sc in sStat) {
    ...
    time += 3600 * 24 * 1000;
    now.setTime(time);
    SetLSData("UsStatData", arstr, now.toGMTString());
    GetStatData()
}
...
function OutUnoffStat() {
    var stData = GetLSData("UsStatData");
    if (stData) {
       	var StArr = stData.split("|"),
			UnOffStat = StArr[1].split("/");        
        if (UnOffStat) {
                var unOffStatArr = [];
                ...
                var vTh = document.getElementsByClassName("t-table-vehicle")[0].getElementsByTagName('th')[3],
                    nTh = document.createElement('th');
                document.getElementsByClassName("t-table-vehicle")[0].getElementsByTagName('th')[0].width = "288";
                nTh.innerHTML = '<span class="t-vehicle-head">' + (lang == "ru" ? "%-ср%" : "%-avg%") + "</span>";
                nTh.className ="t-center";
                vTh.parentNode.insertBefore(nTh, vTh);
                var tTypes = document.getElementsByClassName("js-tank-head-row");
                for (i=0; i<tTypes.length; i++) {
					var nrth = tTypes[i].rows[0].getElementsByClassName('t-center');
					if (nrth.length) {
						nrth[0].width = "288";
						var fTh = document.createElement('td');
						fTh.innerHTML = '-';
						fTh.className = "t-center";
						tTypes[i].rows[0].cells[3].parentNode.insertBefore(fTh, tTypes[i].rows[0].cells[3]);
					}
                }
                var tanks = document.getElementsByClassName("tr-tanks");
                for (i=0; i<tanks.length; i++) {
                    ...
                    sTd.className = "t-right";
                    var med = unOffStatArr[imgName];
                    if (med) {sTd.innerHTML = col(med) + " (" + col2(parseFloat(t[2].getElementsByTagName('font')[0].innerHTML) - med) + ")";} else {sTd.innerHTML = "x"}
                    t[3].parentNode.insertBefore(sTd, t[3]);
                }
        }
    }
    main();
...
function GetStatData(effs) {
    var stData = GetLSData("UsStatData");
    var lang;
    if (document.title.indexOf("Профиль игрока")>-1)	lang="ru";
    if (stData) {
       	var StArr = stData.split("|"),
            bsRangs = StArr[0].split(";");
        if (StArr.length < 2) xdr.xget("http://wot-crabe.ru/api/server_stats.json", outStatData2);
        var UnOffStat = StArr[1].split("/");
        if (bsRangs) {
            var rcr = 0,
                rcrv = 0;
            for (var i=bsRangs.length-1; i>-1; i--) {
                if (Number(effs[3])>Number(bsRangs[i])) {
                    rcr = i;
                    if (i>0) {
                        rcrv = (Number(bsRangs[i-1]) - Number(effs[3])).toFixed(2) + (lang == "ru" ? " баллов до следующего ранга" : " balls to next level");
                    } else {
