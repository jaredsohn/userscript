// ==UserScript==
// @name           AUTO_ATK
// @namespace      ultimo script de gw lazy
// @description    no se 
...
var attack_unit_Truck         = "modal_attack_unit_ipt_1";
var attack_unit_Infantry      = "modal_attack_unit_ipt_5";
var attack_unit_Sniper        = "modal_attack_unit_ipt_6";
var attack_unit_Antitank      = "modal_attack_unit_ipt_4";
var attack_unit_SpecialForces = "modal_attack_unit_ipt_18";
var attack_unit_MobileSAM     = "modal_attack_unit_ipt_7";
var attack_unit_Tank          = "modal_attack_unit_ipt_8";
var attack_unit_Predator      = "modal_attack_unit_ipt_17";
var attack_unit_SupplyChopper = "modal_attack_unit_ipt_9";
var attack_unit_Gunship       = "modal_attack_unit_ipt_11";
var attack_unit_Fighter       = "modal_attack_unit_ipt_10";
var attack_unit_Bomber        = "modal_attack_unit_ipt_12";
var attack_unit_CargoJet      = "modal_attack_unit_ipt_19";
var attack_unit_Hellfire      = "modal_attack_unit_ipt_16";
var attack_unit_Stealth       = "modal_attack_unit_ipt_13";

// wild-guess default values
var rallypoints=9;
var trucks=0;
var troops=0;
var snipers=0;
...
// Attack code: periodically looks for the info box from a map tile click
window.setInterval(
function (){
	...
	if(document.getElementById("Attack").checked==true){
		...
		if(document.getElementById("marchBtn") && !document.getElementById("generalsContainer")){
			attacking=1;
			fireEvent(document.getElementById("marchBtn"),'click');
		}
		...
		if(generals){
			...
			// Determine attacking troops via wild-ass guess
			if(document.getElementById(attack_unit_SupplyChopper)){
				document.getElementById(attack_unit_SupplyChopper).value=Math.round(choppers/rallypoints);
			}
			if(document.getElementById(attack_unit_Fighter)){
				document.getElementById(attack_unit_Fighter).value=Math.round(fighters/rallypoints);
			}
            ...
            if(document.getElementById("SupplyChoppers") && document.getElementById(attack_unit_SupplyChopper)) {
                document.getElementById(attack_unit_SupplyChopper).value=document.getElementById("SupplyChoppers").value;
            }
            if(document.getElementById("SupplyTrucks") && document.getElementById(attack_unit_Truck)) {
                document.getElementById(attack_unit_Truck).value=document.getElementById("SupplyTrucks").value;
            }
            if(document.getElementById("AttackChoppers") && document.getElementById(attack_unit_Gunship)) {
                document.getElementById(attack_unit_Gunship).value=document.getElementById("AttackChoppers").value;
            }
            if(document.getElementById("AttackFighters") && document.getElementById(attack_unit_Fighter)) {
                document.getElementById(attack_unit_Fighter).value=document.getElementById("AttackFighters").value;
            }
            if(document.getElementById("AttackStealths") && document.getElementById(attack_unit_Stealth)) {
                document.getElementById(attack_unit_Stealth).value=document.getElementById("AttackStealths").value;
            }
            ...
            if(document.getElementById("AttackCargoJet") && document.getElementById(attack_unit_CargoJet)) {
                document.getElementById(attack_unit_CargoJet).value=document.getElementById("AttackCargoJet").value;
            }
            ...
            for (var maxenergy=4; maxenergy>0; maxenergy--) {
                for(kk=0;kk< list.length;kk++) {
                    ...
                    if(count == maxenergy){
                        ...
                        fireEvent(list[kk],'click');
                        fireEvent(document.getElementById("march_button"),'click');
                        kk=list.length+1;
...
function ptStartup (){
  ...
  var styles = '.ptTabs {color:black; font-size:12px}\
    ...
    table.ptTabResumen tr td {border-left:1px solid #ccc; white-space:nowrap; padding: 1px;}\
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('HERRAMIENTAS', eventHideShow, mouseMainTab);
...
Tabs.Resumen = {
  ...
  init : function (div){
    this.cont = div;
    if (Options.ResumenAllowOverflow)
      this.cont.style.overflowX = 'visible';
  },

  hide : function (){
    clearTimeout (Tabs.Resumen.displayTimer);
  },
    ...
    try {
     ...
     // <TR align=left><TD><SPAN class=ptStatLight>Joined on:</span> '+ dt.toLocaleDateString() +'</td>
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class="ptTab ptStat" width=97% align=center>\
        <TD><SPAN class=ptStatLight>Poder:</span> ' + addCommas(Seed.player.might) +'</td>\
      ...
      str += "<DIV id=overMainDiv style='font-size:"+ Options.ResumenFontSize +"px'><TABLE class=ptTabResumen cellpadding=0 cellspacing=0><TR valign=top align=right><TD width=65></td><TD width=88 style='background: #ffc'><B>TOTAL</b></td>";
	  ...
	  str += '<TR valign=top align=right><TD></td><TD style=\'background: #ffc\'></td>';
		...
		if(Gate == 0)
		  ...
		  str += '<TD><SPAN class=boldRed>Defendindo</span></td>';
		}
      ...
      for (r=1; r<9; r++){
        ...
        for(i=0; i<Cities.numCities; i++) {
          ...
          if ( r == 8 )
             rows[r][i] = parseInt(Seed.citystats[cityID]["pop"][0]) - parseInt(Seed.citystats[cityID]["pop"][3]);  // Labor force  FOOS
         }
      }
...
//      str += _row ('Diamonds', rows[8]);
      ...
      str += _row ('Oro', rows[0]);
      str += _row ('Alimentos', rows[1]);
      str += _row ('Petroleo', rows[2]);
      str += _row ('Piedra', rows[3]);
      str += _row ('Acero', rows[4]);
      str += _row ('Titanio', rows[5]);
      str += _row ('Grafeno', rows[6]);
      str += _row ('Uranio', rows[7]);
      str += _row ('Poblacion', rows[8]);
...
//      str += _row ('Stealth', rows[13]);
      str += _row ('Camiones', rows[1]);
      str += _row ('Infanteria', rows[5]);
      str += _row ('Francos', rows[6]);
      str += _row ('AntiTan...', rows[4]);
      str += _row ('F. E.', rows[18]);
      str += _row ('SAM', rows[7]);
      str += _row ('Tanques', rows[8]);
      str += _row ('Drones', rows[17]);
      str += _row ('Chinook', rows[9]);
      str += _row ('Helicop...', rows[11]);
      str += _row ('Cazas', rows[10]);
      str += _row ('Bombar..', rows[12]);
      str += _row ('Hercules', rows[19]);
      str += _row ('Hellfire', rows[16]);
      str += _row ('Stealth', rows[13]);
      str += _row ('Ojivas ', rows[15]);  
    str += '<TR><TD colspan=12><BR></td></tr>';
...
//      str += _row ('Food +/-', row, true);
      str += _row ('Costo ', row3, true);
      str += _row ('Prod.', row2, true);      
      str += _row ('Balance +/-', row, true);
...
//      str += _row ('Food left', row, true);
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.ResumenAllowOverflow?' CHECKED':'') +'>Allow width overflow \
         &nbsp; &nbsp; FONT SIZE: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12}, Options.ResumenFontSize, 'id=ptoverfont') +'</td></tr>';
      str += "</table></div>";
	  str+= 'GW Lazy Tools Version : ' + Version;
      Tabs.Resumen.cont.innerHTML = str;
...
//DebugTimer.display ('Draw Resumen');    
    } catch (e){
      Tabs.Resumen.cont.innerHTML = '<PRE>'+ e.name +' : '+ e.message +'</pre>';
    }   
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
		...
		m += '</table><BR><BR><HR>Note that if a checkbox is greyed out there has probably been a change of KofC\'s code.';
      div.innerHTML = m;
	  t.cont.innerHTML = m;
...
Tabs.Auto_Ataque = {
  ...
  init : function (div){
    var t = Tabs.Auto_Ataque;
    t.cont = div;
    
    try {      
      m = '<TABLE class=ptTab>\
        <TR><TD colspan=2><B>Auto Attack:</b></td></tr>\
        <TR><TD><INPUT id=Attack type=checkbox /></td><TD>Enable Attack</td></tr>\
        <TR>\
            <TD><INPUT id=AttackInfantry type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Infantry</td>\
            <td style="width:5em"></td>\
            <TD><INPUT id=AttackCargoJet  type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; hercules</td>\
        </tr>\
        <TR>\
            <TD><INPUT id=SupplyChoppers type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Supply Helicopters</td>\
            <td></td>\
            <TD><INPUT id=SupplyTrucks   type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Supply Trucks</td>\
        </tr>\
        <TR>\
            <TD><INPUT id=AttackFighters type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Fighters</td>\
            <td></td>\
            <TD><INPUT id=AttackChoppers type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Gunships</td>\
        </tr>\
        <TR>\
            <TD><INPUT id=AttackStealths type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Stealth Bombers</td>\
            <td></td>\
            <TD><INPUT id=AttackHellfire type=text size=2 maxlength=5 value=0 /></td><TD> &nbsp; Hellfire Tanks</td>\
        </tr>\
		<TR><TD><BR> </td></tr>';
      m += '</table><BR><BR><HR>Click on any map cell to launch an attack with the above options!';
      t.cont.innerHTML = m;

      // Not sure how to save the above options yet.  Dont want to save the AutoAttack value anyway (Gus 8/14)
      // t.togOpt ('ptAllowWinMove', 'ptWinDrag', mainPop.setEnableDrag);
	  // t.togOpt ('ptgoldenable', 'pbgoldenable', CollectGold.setEnable);
	  // t.togOpt ('ptoilenable', 'pboilenable', CollectOil.setEnable);
      // t.changeOpt ('ptgoldLimit', 'pbGoldLimit');
