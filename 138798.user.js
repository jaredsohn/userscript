// ==UserScript==
// @name			Shangu Package
// @author			Laoujin / De Goede Fee / Edit: Patricier
// @namespace			
...
function sangu_ready()
{
    ...
    */
    ...
    $.extend(user_data, {
        ...
        displayDays: true, /* true: display (walking)times in days when > 24 hours. false: always displays in hours */
        ...
        mainTagger:
		{
		    ...
		    otherDescriptions:
				...
				{ name: "DODGE NOK", prefix: true },
				{ name: "DODGE OK", prefix: false },
				{ name: "SNIPE NOK", prefix: true },
				{ name: "SNIPE OK", prefix: false },
				],
		    prefix: "-----------------------------------------",
		...
		{
		    active: true,
		    off_link: "&group=1052&unit=2&amount=6000&sort=true&changeSpeed=ram",
		    def_link: "&group=1051&unit=0&amount=3000&sort=true&changeSpeed=spear"
		},
        ...
        customPlaceLinks:
			    [
				    ...
				    { active: true, type: 'def', name: '5 Ver', spy: 5, sendAlong: 0 },
				    { active: false, type: 'def', name: '4deZc', spear: 2000, heavy: 500, sendAlong: 0 },
				    { active: true, type: 'def', name: 'HelftZw', spear: 5000, sword: 5000, archer: 5000, sendAlong: 500 },
				    { active: false, type: 'def', name: '3deZw', spear: 3300, sword: 3300, sendAlong: 0 },
				    { active: false, type: 'def', name: '4deZw', spear: 2500, sword: 2500, sendAlong: 0 },
					{ active: true, type: 'def', name: 'Divide', spear: 1000, sword: 1000, sendAlong: 0 }
			    ...
			    profile:
					{
					    ...
					    externalPage: "(Externe pagina)",
					    internalPage: "(Interne pagina)",
					    conquers: "Overnames",
					    villages: "Dorpen:",
					    graphPoints: "Punten",
    ...
    $("#storage").parent().after("<td class='icon-box' nowrap><a id=sanguChangeStatus href=" + location.href.replace("&changeStatus=" + isSanguActive, "") + "&changeStatus=" + (!isSanguActive) + "><img src='graphic/dots/" + (isSanguActive ? 'green' : 'red') + ".png' title='" + (!isSanguActive ? trans.sp.sp.activatePackage : trans.sp.sp.deactivatePackage) + " (v" + sangu_versie + ")' /></a>&nbsp;</td>");
    if (isSanguActive)
    {
        // Config
        var world_data = {};
	...
	["wall", [8, 10, 12, 14, 17, 20, 24, 29, 34, 41, 50, 59, 71, 86, 103, 123, 148, 177, 213, 256]]
        ...
        switch (world_data.number)
        {
            ...
            case 18: case 19: case 20: case 21: case 22: case 23: case 25:case 26: case 27: case 28: case 29:
                if (world_data.number != 26 && world_data.number != 27 && world_data.number != 29) world_data.hasArchers = true;

                world_data.smithyLevels = false;
                if (world_data.number != 27) world_data.hasKnight = true;
		...
		function ()
		{
            ...
            if (incomingTable.size() == 1 || outgoingTable.size() == 1)
            {
                if (incomingTable.size() == 1)
                {
                    ...
                    if (user_data.mainTagger.active && incomingTable.has("img[src$='attack.png?901ab']").size() != 0)
                    {
					...
					function ()
					{
					    ...
					    //			if (firstCell.has("img[src='http://cdn.tribalwars.net/graphic/command/attack.png?901ab']").size() == 1)
					    //			{
        ...
        else if ((location.href.indexOf('screen=info_') > -1 && location.href.indexOf('screen=info_member') == -1) || location.href.indexOf('screen=ally&mode=profile') > -1)
        {
            ...
            if (user_data.profile.show && (location.href.indexOf('screen=info_village') == -1 || user_data.showPlayerProfileOnVillage))
            {
				...
				if (screen == 'tribe') {
					hislink = "<tr><td colspan=2><a href='http://" + game_data.world + ".tribalwarsmap.com/nl/history/tribe/" + id + "'>&raquo; TWmap Historie</a>(Externe pagina)</td></tr> ";
					
					infoTable.find("tr:last").before(hislink);
				}
                else {
					var hislink = "<tr><td colspan=2><a href='http://" + game_data.world + ".tribalwarsmap.com/nl/history/player/" + id + "'>&raquo; TWmap Historie</a>(Externe pagina)</td></tr> ";
					infoTable.find("tr:last").after(hislink);
				}
            }
		...
		function ()
		{
			...
			/*if (location.href.indexOf('screen=ranking') > -1)
			{
			   var table = $('table#player_ranking_table');
				$("table#player_ranking_table tbody tr:first th:eq(0)").attr('id', 'rank');
				$("table#player_ranking_table tbody tr:first th:eq(1)").attr('id', 'naam');
				$("table#player_ranking_table tbody tr:first th:eq(2)").attr('id', 'stam');
				$("table#player_ranking_table tbody tr:first th:eq(3)").attr('id', 'punten');
				$("table#player_ranking_table tbody tr:first th:eq(4)").attr('id', 'dorpen');
				$("table#player_ranking_table tbody tr:first th:eq(5)").attr('id', 'puntgem');
			$('#rang, #naam, #stam, #punten, #dorpen, #puntgem')
...
*/
            ...
            if (location.href.indexOf('screen=overview_villages') > -1 && location.href.indexOf('mode=prod') > -1)
            {
				...
				var a = $("table#production_table tbody tr"); 
				for (var q = 1; q <= a.length; q++){
					var b = $(a).eq(q).children("td:eq(6)").text();
					var c = b.split("/");
					var d = Math.round(parseInt(c[0]) / parseInt(c[1]) * 100);
					$(a).eq(q).children("td:eq(6)").append(" = " +d + "%");
				}
                $("#resFilter").change(
		function ()
		{
			...
			&& (location.href.indexOf('type=own_home') > -1 || location.href.indexOf('type=there') > -1))
            {
                ...
                var mod = "row_a";
                theUnits = $("table#units_table tbody");
                ...
                theUnits.each(function ()
                {
                    ...
                    cells.each(function (index, element)
                    {
                        ...
                        else if (index == rowSize)
                        {
							if (world_data.hasArchers) {
							var TDa = $(element);}
							else { var TDa = $(element).next();}
							//console.log($(TDa));
                            //alert(index + "==" + rowSize);
                            //alert("COMMANDS:" + world_data.hasMilitia + ":" + $(this).html());
							var deletevillage = 'javascript: $(this).closest("tr").remove(); void(0)';
                            newRow += "<td>";
                            newRow += "<a href='" + deletevillage + "'><img src='/graphic/dots/red.png' title='" + trans.sp.troopOverview.removeVillage + "' /> </a>";
                            newRow += "<a href='" + $("a", TDa).attr('href').replace("mode=units", "") + "'>";
				...
				function ()
				{
				    ...
				    if (firstCellText.indexOf(trans.tw.command.returnFull) != 0 && firstCellText.indexOf(trans.tw.command.sentBackBy) != 0 && firstCellText.indexOf(trans.tw.command.returnFrom) != 0)
				    {
				        ...
				        if (village.isValid)
				        {
				            ...
				            else isOs = cells.first().has("img[src='http://cdn.tribalwars.net/graphic/command/support.png?']").size() == 1;
		...
		function ()
		{
            ...
            else if (location.href.indexOf('screen=overview_villages') > -1 && location.href.indexOf('mode=incomings') > -1)
            {
				...
				menu += "<br /><input type=text id=filterAttacker value='Speler'>";
				menu += "<input type=button onclick=filterAanvaller() value='Filter Speler'>";
				menu += " | <input type=text id=filterName value='Naam van aanval bevat'>";
				menu += "<input type=button onclick=filterNaam() value='Filter Naam'>";
				menu += " | <input type=text id=filterTarget value='Target'>";
				menu += "<input type=button onclick=filterDoel() value='Filter Target'>";
				menu += "<br />";
				menu += "<input type=button onclick=renameOk() id=OK value='OK'>";
				menu += "<input type=button onclick=renameFake() id=Fake value='FAKE'>"
				menu += "<input type=button onclick=renameNacht() id=nBonus value='" + user_data.mainTagger.otherDescriptions[0].name + "'>";
				menu += "<input type=button onclick=renameStack() id=checkStack value='" + user_data.mainTagger.otherDescriptions[1].name + "'>";
				menu += " <input type=button onclick=renameEdel() id=edel value='" + user_data.mainTagger.otherDescriptions[3].name + "'>";
				menu += " <input type=button onclick=renameDnok() id=dNOK value='" + user_data.mainTagger.otherDescriptions[4].name + "'>";
				menu += "<input type=button onclick=renameDok() id=dOK value='" + user_data.mainTagger.otherDescriptions[5].name + "'>";
				menu += " <input type=button onclick=renameSnok() id=sNOK value='" + user_data.mainTagger.otherDescriptions[6].name + "'>";
				menu += "<input type=button onclick=renameSok() id=sOK value='" + user_data.mainTagger.otherDescriptions[7].name + "'>";
                menu += "</th></tr>";
                menu += "</table>";
				        ...
				        function (villageDetails)
				        {
				            ...
				            else
				            {
				                ...
				                if (incomingTable.size() > 0)
				                {
				                    ...
				                    var incomingRows = $("tr:has(img[src$='attack.png?901ab'])", incomingTable);
...
function renameNacht() {

	$("input[type='checkbox']:not([id])").each(function () {
		if ($(this).attr("checked")){
			var button = $(this).next().next().find("input[type='button']");	                
			button.prev().val("NACHTBONUS"); 
			button.click();
		}
	});
}
...
function filterAanvaller() {
	var a = $("table#incomings_table tbody tr"); 
	var aanvaller = $("#filterAttacker").val();
	for (var q = 1; q <= a.length; q++){
		var b = $(a).eq(q).children("td:eq(2)").text();
		if(b.indexOf(aanvaller) == -1){
			$(a).eq(q).css("display", "none");
		}
	}
...
function filterNaam() {
	var a = $("table#incomings_table tbody tr"); 
	var aanvaller = $("#filterName").val();
	for (var q = 1; q <= a.length; q++){
		var b = $(a).eq(q).children("td:eq(0)").text();
		if(b.indexOf(aanvaller) == -1){
			$(a).eq(q).css("display", "none");
		}
	}
...
function filterDoel() {
	var a = $("table#incomings_table tbody tr"); 
	var aanvaller = $("#filterTarget").val();
	for (var q = 1; q <= a.length; q++){
		var b = $(a).eq(q).children("td:eq(0)").text();
		if(b.indexOf(aanvaller) == -1){
			$(a).eq(q).css("display", "none");
