// ==UserScript==
// @name           CSM - urlReplacer
// @namespace      None
// @description    Replaces admin links
// @include        http://*.cs-manager.com/csm/other/?p=other_admin_notes&u=*
// @include        http://*.cs-manager.com/csm/?p=manager_mail&s=read&m=*
...
gb = document.getElementById('guestbook');

var shortUrl = gb.innerHTML.match('(<p>|<br>| )/(csm|forum|admin)/(.*?)(</p>|<br>)', 'gi');

if (shortUrl != null) {
	for (var a = 0; a < shortUrl.length; a++) {
		gb.innerHTML = gb.innerHTML.replace(shortUrl[a], '<a style="display:block;" href="http://www.cs-manager.com' + strip_tags(shortUrl[a]) + '" target="_blank">' + strip_tags(shortUrl[a]) + '</a>');
	}
}
