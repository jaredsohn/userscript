// ==UserScript==
// @name           Highscore v1.4.6 Poprawki + tlumaczenie + rozszerzenie by pepies + update by VILQ
// @namespace      
// @description    Pokazuje punkty bandy, pieniadze danego gracza (wieksza ilosc pieniedzy zakolorowuje na zielono (ponad 1000) lub czerwono (ponad 10000), date rejestracji. Wykorzystano dodatkowo "Menelgame bez reklam by Shorty." bo reklamy nachodziĹ�y na ramki.
...
newth_2.innerHTML = 'PieniÄ�dze/ <br /> Data Rejestracji';
...
function Geldladen(id,x) {
   ...
   alert ('Na placu:' + x + ' mial ' + name + ' wiecej niz 150000 zl w reku.');
			...
			if(cash.length >= 9)
			{
			 newtd.innerHTML = cash.substring(0,cash.length-8) + "." + cash.substring(cash.length-8,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + "zĹ�" + '<br />' + reg + pskript;
			}
			else if (cash.length>=6)
			{
			newtd.innerHTML = cash.substring(0,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + "zĹ�" + '<br />' + reg + pskript;
			}
			else if(cash.length>2)
			{
			newtd.innerHTML = cash.substring(0,cash.length-2) + "," + cash.substring(cash.length-2, cash.length) + "zĹ�" + '<br />' + reg + pskript;
			}
			else if(cash.length==2)
			{
			newtd.innerHTML =  "zĹ� 0," + cash + '<br />' + reg + pskript;
			}
			else
			{
