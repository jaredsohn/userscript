// ==UserScript==
// @name        Komica Uiharu Package
// @namespace   http://userscripts.org/users/514090
// @include     http://eden.komica.org/00/*
// @exclude	http://eden.komica.org/00/del.php*
// @version     10
// ==/UserScript==
...
if(conf_flag & 256){
	...
	for(var i in k){
		if(k[i].content == "3;URL=index.htm" || k[i].content == "0;URL=index.htm")
			window.location = "http://eden.komica.org/00/index.htm";
	}
}
...
if((conf_flag & 16) && !(window.location.href.match(/.*res=\d*/))){
	...
	if(window.location == "http://eden.komica.org/00/index.htm"){
		tmp.previousSibling.innerHTML = tmp.previousSibling.innerHTML.replace("以下省略","<a href='http://eden.komica.org/00/10.htm'>10</a>");
	}else if(tmp.action == "http://eden.komica.org/00/8.htm"){
		var page_10 = "window.location='http://eden.komica.org/00/10.htm'";
		tmp.nextSibling.nextSibling.innerHTML = tmp.nextSibling.nextSibling.innerHTML.replace("以下省略","<a href='http://eden.komica.org/00/10.htm'>10</a>");
		tmp.nextSibling.nextSibling.nextSibling.innerHTML = '<input type="button" value="下一頁" onclick="'+page_10+'">';
	}else if(tmp.action == "http://eden.komica.org/00/9.htm"){
...
if(conf_flag & 128){
	...
	for(var i in k){
		if(k[i].color == "#707070"){
			k[i].innerHTML = k[i].innerHTML.replace("要閱讀","").replace("下返信連結","<a href='http://eden.komica.org/00/index.php?res=" + k[i].previousSibling.id.replace(/.*p/,"") + "'>這裡</a>閱讀");
		}
	}
}
...
function pre_fetch_iframe(e){
	if(!window.location.href.match(/res=/)){
		var ext_url = "http://eden.komica.org/00/index.php?res="+e.target.parentNode.id.replace(/.*p/,"");
