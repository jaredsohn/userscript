...
(function() {
	...
	var dsrbAddButtons = function(node) {
		...
		var overview = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.target + '&screen=overview', icons.overview, 'Overview');
		if(coords) {
			var center = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=map&x=' + coords[0] + '&y=' + coords[1], icons.center, 'Centreaza Harta');
		}
		var troops = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=place&mode=command&target=' + vilid.target, icons.troops, 'Trimite trupe');
