// ==UserScript==	DEVO Alliance Member Enhancements
// @name DeVo Alliance Member +
// @version		0.2
// @description		Message or buddylist multiple people in evolution
// @include		http://ev5-dev.neondragon.net/*
// @author		podd
// ==/UserScript==
...
unsafeWindow.sendpm = function (url,map) {
	...
	method: 'POST',
	url: 'http://ev5-dev.neondragon.net/send_message',
...
unsafeWindow.addbuddy = function (url,map) {
	...
	method: 'POST',
	url: 'http://ev5-dev.neondragon.net/buddies/add',
	headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        data: url,
	onload:  function(responseDetails) {
		GM_log('Success');
...
var nick = nickname.substring(nickname.indexOf(" "))



url = "x=" + coordx + "&y=" + coordy + "&z=" + coordz + "&c=" + coordc + "&nickname=" + nick + "&label=Friends&othertext=";
GM_log(url);
setTimeout('addbuddy("' + url + '",' + selectedArraymap[j]  + ')', (7800 * j));
j++;
}
setTimeout('showsubmit()', (7800 * j + 1000));
...
formhtml +='</select></form></td><td style="padding:10px;"><form name="messageform" id="messageform" action="" onsubmit="return false;" method="post"><input type="text" size="50" style="width:100%;"name="msgsubject" value="" /><BR><textarea rows="12" cols="40" style="width: 100%;"name="message" id="messagebox"></textarea></form><form name="submitform" id="submitform" onsubmit="return false;"><input type="submit" onclick="sendmasspm();" value="Send Message" /><input type="submit" onclick="sendaddbuddy();" value="Add Buddy" /></form></td></tr></table><div id="confirmation" name="confirmation"></div>';
