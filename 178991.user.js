// ==UserScript==
// @name           LMRBot Assistant
// @namespace      LMRBotAssist
// @version        1.0.1
...
(
    ...
    var CT_mainFunction = function() {
       ...
       function createCtTweak() {
           qx.Class.define("LbotClaimThis.main", {
               type: "singleton",
               ...
               members: {
                   ...
                   createClaimButtons: function() {
					...
					claimThis: function(v) {
						...
						this.chat.chatLine.setValue("/w NinjaSpy "+botAction+posX+":"+posY);
       ...
       function CT_checkIfLoaded() {
           try {
               if (typeof qx != 'undefined') {
                   ...
                   if (a && c && ch && wdst) {
                       createCtTweak();
                       window.LbotClaimThis.main.getInstance().initialize();
                   } else
    ...
    var LbotClaimThisScript = document.createElement("script");
    txt = CT_mainFunction.toString();
    if (window.opera != undefined)
       txt = txt.replace(/</g, "&lt;"); // rofl Opera
    LbotClaimThisScript.innerHTML = "(" + txt + ")();";
    LbotClaimThisScript.type = "text/javascript";
    if (/lordofultima\.com/i.test(document.domain))
       document.getElementsByTagName("head")[0].appendChild(LbotClaimThisScript);
})();
