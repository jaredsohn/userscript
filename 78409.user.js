...
function checkTank(tankId,owner){
	GM_xmlhttpRequest({
		method: "GET",
		url: 'http://www.gaiaonline.com/chat/gsi/index.php?'+'v=json&m=[[6500%2C[1]]%2C[6510%2C["'+tankId+'"%2C0%2C1]]%2C[6511%2C["'+tankId+'"%2C0]]]&X='+(new Date().getTime().toString().substring(0, 10)),
		onload: function(r){
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						GM_xmlhttpRequest({
							...
							url: 'http://absols-web-apps.info/gtools/background/glowRecorder.php',
							onload: function(r){}
						});
					}
				}
...
GM_registerMenuCommand("Is it Glowing: Help", firstTimePage);