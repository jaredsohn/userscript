// ==UserScript==
// @name Ogame SaldiriyaGerekliNakliye v0.2
// @author OgameCLUB from Karamba.
// @description: Sonda raporu altina gerekli nakliye eklentisi
...
// ==/UserScript==
	...
	var spycargo_nome = new Array('Small Cargo', 'Large Cargo', 'Light Fighter' , 'Heavy Figther', 'Cruiser', 'Battleship', 'Colony Ship', 'Recycler', 'Espionage Probe', 'Bomber', 'Solar Satellite', 'Destroyer'  , 'Deathstar'      , 'Battlecruiser');
	...
	for (i = 9; i < spycargo_tr.length; i++) { 
		...
		if (spycargo_tr[i].getElementsByTagName('th')[3] && spycargo_tr[i].getElementsByTagName('th')[3].getElementsByTagName('span')[0]) {
			if (spycargo_tr[i].getElementsByTagName('span')[0].className == 'espionagereport'){
				...
				spycargo_resource = '<br><table width="400"><tbody><tr><td class="c" colspan="4">Maksimum kazanç</td></tr><tr><td>Toplam hammaddesi:</td><td>' + spycargo_adddot(spycargo_tot_res) + '</td><td>ALINABiLiR maden:</td><td>' + spycargo_adddot(spycargo_metal + spycargo_crist + spycargo_deute) + '</td></tr><tr><td>B. Nakliye:</td><td>' + spycargo_adddot(spycargo_CP_num) + '</td><td>K. Nakliye:</td><td>' + spycargo_adddot(spycargo_CL_num) + '</td></tr>';
				if ((spycargo_recy_met + spycargo_recy_cri) > 0) spycargo_resource += '<tr><td class="c" colspan="4">Debris Field</td></tr><tr><td>Metal:</td><td>' + spycargo_adddot(spycargo_recy_met) + '</td><td>Crystal:</td><td>' + spycargo_adddot(spycargo_recy_cri) + '</td></tr><tr><td>Total:</td><td>' + spycargo_adddot(spycargo_recy_met + spycargo_recy_cri) + '</td><td>Recyclers:</td><td>' + spycargo_adddot(Math.floor((spycargo_recy_met + spycargo_recy_cri + 19999)/20000)) + '</td></tr>';
				if (spycargo_spionaggio > 1 && spycargo_spionaggio < 7) spycargo_resource += '<tr><td class="c" colspan="4">Probes Needed</td></tr><tr><td>Probes Used:</td><td><input type="text" id="spycargo' + i + '" onKeyUp="if (this.value != ' + "''" + ') document.getElementById(' + "'spycargo_sond" + i + "'" + ').innerHTML = Math.max(7 + ( ((Number(this.value) - ' + spycargo_spionaggio + ' + 0.1) / Math.abs(Number(this.value) - ' + spycargo_spionaggio + ' + 0.1)) * Math.pow(Math.ceil(Math.sqrt(Math.abs(Number(this.value) - ' + spycargo_spionaggio + '))), 2)), 1)' + '" size="3" maxlength="2" style="text-align:center"></td><td>Probes Needed:</td><td id="spycargo_sond' + i + '>&nbsp;</td></tr>';
