...
// ==UserScript==
// @name           LoU BoS [Fortune Teller Fix]
// @description    [Fortune Teller Fix] - Additions to Lord of Ultima: summary, food calculator, recruitment speed calculator, combat calculator and various other features
// @namespace      BoS
// @author         Urthadar, Edit by Ventrix
...
/*
 * Version 1.5.4b - Added extended tooptips to show needed TS on Dungeons and Bosses.
 */
...
qx.Class.define("bos.gui.MassRecruitmentPage", {
	...
	members: {
		...
		_treeWidthChanged: function(e) {
			var ed = e.getData();
			if (ed.col == 1 && ed.new____Width != 75) {
				var tcm = this.tree.getTableColumnModel();
		...
		_calculateUnitDetails: function(unit, storedCity) {
			...
			resources[0] = 1000000;//webfrontend.data.Player.getInstance().getGold(); // 100000;//
			var row = [];
			var summary = getSummaryWidget();			
...
qx.Class.define("bos.gui.CastlesPage", {
	...
	construct: function() {
		...
		var columnNames;
		if( locale == "de") {
			columnNames = [ "Id", "Name", "Pos", "Nahrung", "Nahrung: LagerkapazitÃ¤t", "Keine Nahrung mehr am", "Einheiten", "Rekutierungsliste", "Orders", "TS der Einheiten die z.Z. verfÃ¼gbar sind", "Holz: LagerkapazitÃ¤t", "Eisen: LagerkapazitÃ¤t", "Verteidiger (Doppelklicken zum AuswÃ¤hlen)"];
		} else {
	...
	members: {
		...
		_createToolBar: function() {
			...
			btnHelp.addListener("execute", function(evt) {
				var dialog = new webfrontend.gui.ConfirmationWidget();
				if (locale == "de"){
					dialog.showGenericNotice("Summary Hilfe", "Die StÃ¤dte werden nach speziellen Zeichen in den Referenzen sortiert.Diese Zeichen werden durch *Zeichen* makiert und kÃ¶nnen an einer beliebigen Stelle in der Referenz stehen. Als Beispiel: 'Kont 23_3 *CM*'wÃ¼rde eine Burg darstellen, welche auch Mondsteine herstellen kann.", "C - Burg (Castle), M - Mondstein, W - Lager(Warehouse), B - In Bau/Aufbau, D - Defensive, G - Gold", "webfrontend/ui/bgr_popup_survey.gif");
				}
				else {
...
qx.Class.define("bos.gui.OptionsPage", {
	...
	construct: function() {
		...
		var btnAddCustomCityType = new qx.ui.form.Button(locale == "de" ? "HinzufÃ¼gen" :"Add");
		btnAddCustomCityType.addListener("execute", this._addCustomCityType, this);
		container.add(btnAddCustomCityType);
		
		var btnRemoveCustomCityType = new qx.ui.form.Button(locale == "de" ? "LÃ¶schen" :"Remove");
...
function exportSelectedReports() {
	...
	if (ids.length > 5) {
		if (locale == "de"){
			bos.Utils.handleWarning("Bitte wÃ¤hle nicht mehr als 5 Berichte aus");
		}
		else{
	...
	if (locale == "en") {
		...
		if (name != null && name.length > 0 && count > 1) {
			switch (name) {
				...
				name = "Kundschafter";
				break;
				case "armbrustschÃ¼tze":
				name = "ArmbrustschÃ¼tze(n)";
				break;
				case "paladin":
				name = "Paladin(e)";
				break;
...
/*
	...
	members: {
		...
		createFooter: function() {
			...
			if (locale == "de") {
				this.sbCombatType.add(new qx.ui.form.ListItem("Ãœberfall", null, "Assault"));
				this.sbCombatType.add(new qx.ui.form.ListItem("Belagerung", null, "Siege"));
				this.sbCombatType.add(new qx.ui.form.ListItem("PlÃ¼nderung", null, "Plunder"));
			} else {
			...
			this.btnClearAll = new qx.ui.form.Button(locale == "de" ? "Alles LÃ¶schen" : "Clear All");
			...
			this.btnAdd = new qx.ui.form.Button(locale == "de" ? "HinzufÃ¼gen" : "Add");
		...
		addReport: function(addDefenders) {
			...
			if (rep == null) {
				if (locale == "de"){
					bos.Utils.handleWarning("Der Reportframe ist nicht geÃ¶ffnet, bitte klicke auf den Report-Button");
				} else {
...
function jumpCoordsDialog() {
	var wdg = new webfrontend.gui.ConfirmationWidget();
		wdg.askCoords = function() {
			...
			var lc = new qx.ui.basic.Label(locale == "de" ? "Koordinaten(0-699) mÃ¼ssen mit einem Doppelpunkt getrennt werden! <br />Beispiel: 432:231 " : "Values (0-699) should be separated by a colon<br />Example: 432:231");
			lc.setRich(true);
	...
	return o.join("");
		...
		function extendedTT() {
			try {
				...
				const weakT = 'Weakness:' + sRow;
				const progressT = 'TS + %:' + sRow;
				// const zergT = r.units["6"].dn + ':' + sRow;
				const zergT = 'TS at 0%:' + sRow;
						...
						if(mode == 'c' || mode == 'd') {
							...
							if(text != null || text.length > pId) {
								...
								if(type == nameC) { // Name:
									...
									var progress = webfrontend.gui.Util.formatNumbers(parseInt((progress * 0.0175 + 1.0000) * dungeonKill[ parseInt(level) - 1]));
