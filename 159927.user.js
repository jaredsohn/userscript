...
// @name        jssbux
// @namespace   jssbux
// @include     *://*jssbux.com/surf-ads
// @include     *://*jssbux.com/v?h=*
...
if(wparent.location.href.indexOf('v?h=') != -1 && self == top){
	$.each($('iframe'),function(){
        if($(this).attr('src')){
            if($(this).attr('src').indexOf('cashnhits')>=0 ||$(this).attr('src').indexOf('resence')>=0){
	...
	var div = $('<div>');
	div.css({color:'#000',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:85,height:20,background:'#AFFFAF',border:'2px solid green',bottom:10,right:10})
		.text('');
