// ==UserScript==
// @name           Lou Ataque Programado
// @description    Planifica los ataques en español
// @namespace      TKS Fake Attack Planner Working Spanish
// @author         Cafebcs (Original by Cemmac)
...
LouTKSMassAttack.prototype.init = function() {
   ...
   // ***** City Layout Button ***** //
   var layoutButton = new qx.ui.form.Button("ATK PRG");
   layoutButton.set({appearance: "button-text-small", toolTipText: "Da click aqui­ para enviar ataque masivo."});
   ...
   var ExecuteButton = new qx.ui.form.Button("Importar Plan de Ataque");
   ...
   var CityIntelButton = new qx.ui.form.Button("Obtener Inteligencia");
   CityIntelButton.set({appearance: "button-text-small", toolTipText: "Crear un reporte de inteligencia"});
...
function LoadNextReport()
{
  ...
  if (cityIntelIndex > 0) {
    ...
    continue;
      ...
      {
    if (rowInfo.s.search(": Asaltada por ") >= 0 ||
        rowInfo.s.search(": Saqueada por ") >= 0 ||
        rowInfo.s.search(": Asediada por ") >= 0)
    {
      cityIntelIndex = i;
   ...
   if (cityIntelIndex >= 0) {
      ...
      if (cityIntelArray.length > 0) {
         ...
         qx.core.Init.getApplication().getDesktop().add(exportwin, {left: 0, right: 0, top: 0, bottom: 0});
            ...
            dialog.showGenericNotice("Gather Intel", "No hubo inteligencia de valor para obtener", "", "webfrontend/ui/bgr_popup_survey.gif");
...
function SetRealTarget()
{
   if (qx.core.Init.getApplication().cityDetailView && qx.core.Init.getApplication().cityDetailView.city) {
         ...
         qx.core.Init.getApplication().cityDetailView.cityImage.getSource().indexOf("_military_") >= 0)
      {
         ...
         var y=cx>>16;
		var z="0";
		if (y>=100) z=""; //agregada para arreglar las coordenadas
         RealTarget.setValue(x+":"+z+y);
      }
   }
}
...
function PrepareEmail()
{
   ...
   emailString += "[b]Ataques Normales[/b]: "+m+" "+d+" "+y+" @ "+h+" : "+n+" : "+s+" ("+attackTime+")\n";
   emailString += "[b]Ataques de Asedio[/b]: Retardo de 1 Minuto ("+(attackTime+60*1000)+")\n\n";
   emailString += "Coordenadas: "+RealTarget.getValue()+"\n\n";
   emailString += "[b]Ataques Falsos[/b]:\n";
   ...
   if(qx.core.Init.getApplication().sendMail != null && qx.core.Init.getApplication().currentOverlay==qx.core.Init.getApplication().sendMail && qx.core.Init.getApplication().sendMail.hasUnsaved()) {
      ...
      qx.core.Init.getApplication().sendMail.subject.setValue("Ataque Programado");
   }
}
...
function ExecuteAttackPlan() {
   ...
   if (qx.core.Init.getApplication().getMailPage().__Ou.$$user_value.indexOf("Ataque Programado") != 0) {
      err = 1;
      console.log("PS Error: start of ExecuteAttackPlan: Line 261");
   } else {
   
      var msg = qx.core.Init.getApplication().getMailPage().__OD.$$user_value;
      var sTroopAttackTick = ExtractString(msg, "<b>Ataques Normales</b>", "<br />", "(", ")");
      var sWGAttackTick = ExtractString(msg, "<b>Ataques de Asedio</b>", "<br />", "(", ")");
      var sAttackTarget = ExtractString(msg, "Coordenadas: ", "/a><br />", ">", "<");
      var decoyString = "";
      if (sTroopAttackTick == "" || sWGAttackTick == "") {
         err = true;
      } else {
         var searchString = "<b>Ataques Falsos</b>:<br />";
...
console.log("PS Error: inicio de ExecuteAttackPlan Error at line 283: " + err);
   if (!err) {
      ShowMassScheduler();
     ...
     decoyUnitTypeChanged();
      ...
      var dialog = new webfrontend.gui.ConfirmationWidget();
      dialog.showGenericNotice("Execute Attack Plan", "Este no es un plan de ataque válido", "", "webfrontend/ui/bgr_popup_survey.gif");
      qx.core.Init.getApplication().getDesktop().add(dialog, {left: 0, right: 0, top: 0, bottom: 0});
   }
}
...
function showExportWin(text) {
   ...
   wdg.showExportWin = function() {
      ...
      var la = new qx.ui.basic.Label("Inteligencia Obtenida");
...
function ShowMassScheduler() {
   if (!opts) {   // Don't reload the page, otherwise you lose the settings
      ...
      opts.setTitle("LOU Ataque Programado - Versión 5.1.1");
   ...
   // Save Button
      ...
      saveButton = new qx.ui.form.Button("Planificar").set({width: 120});
      saveButton.addListener("click", function(){ScheduleAttacks_Step1();qx.core.Init.getApplication().switchOverlay(null);}, false);
      saveButton.setEnabled(false);

      mailPlanButton = new qx.ui.form.Button("Compartir").set({width: 120});
   ...
   // Unit Type
      var UnitLabel = new qx.ui.basic.Label("Tipo de Unidad").set({width:85});
      ...
      resetUnits();
      UnitDrop.setToolTipText("Selecciona el tipo de unidad a enviar en ataque masivo");
   ...
   // Attack Type
      var AttackLabel = new qx.ui.basic.Label("Tipo de Ataque").set({width:85});
      ...
      AttackDrop.add(new qx.ui.form.ListItem("Explora",null,1));
      AttackDrop.add(new qx.ui.form.ListItem("Saqueo",null,2));
      AttackDrop.add(new qx.ui.form.ListItem("Asalto",null,3));
      AttackDrop.add(new qx.ui.form.ListItem("Apoyo",null,4));
      AttackDrop.add(new qx.ui.form.ListItem("Asedio",null,5));
   ...
   // Unit Count
      var NumOfUnitsToSendLabel = new qx.ui.basic.Label("Unidad Falsa").set({ width:85 });
   ...
   // Date
      ...
      var DateLabel = new qx.ui.basic.Label("Tiempo de Arribo:").set({width:85});
   ...
   // Time
      ...
      var TimeLabel = new qx.ui.basic.Label("Hora de Arribo:").set({width:85});
   ...
   // City box
      ...
      var CityLabel = new qx.ui.basic.Label("Coordenadas Falsas (Li­mite 16 Li­neas)");
   ...
   // Add to page
      ...
      var decoyGroup = new qx.ui.groupbox.GroupBox("Ataques Falsos");
      ...
      var RealGroup = new qx.ui.groupbox.GroupBox("Ataque Real");
...
function AddTroopInfo() {
   ...
   RealAttackDrop.add(new qx.ui.form.ListItem("Explora",null,1));
   RealAttackDrop.add(new qx.ui.form.ListItem("Saqueo",null,2));
   RealAttackDrop.add(new qx.ui.form.ListItem("Asalto",null,3));
   RealAttackDrop.add(new qx.ui.form.ListItem("Apoyo",null,4));
   RealAttackDrop.add(new qx.ui.form.ListItem("Asedio",null,5));
   RealAttackDrop.setSelection([RealAttackDrop.getChildren()[4]]);

   var row = new qx.ui.container.Composite(new qx.ui.layout.HBox());
   row.setAlignY("bottom");
   RealTargetLabel = new qx.ui.basic.Label("Coordenadas").set({width:85});
