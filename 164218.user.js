// ==UserScript==
// @name	SteamPunk autobot
// @namespace	net.gfsrv.mobilesteampunk.bot
// @include	http://live.mobilesteampunk.gfsrv.net/*
// @version	1.41
// @grant	none
...
var mobcodes = {
	codelist: JSON.parse(GM_getValue("steampunkgameallycodes",'{}')), //existing
	...
	isValid: function(code){
        ...
        var CheckCode = /^\d{3}.?\d{3}.?\d{3}$/;
        return CheckCode.test(code);
	},
	...
	save: function () { 
		...
		GM_setValue("steampunkgameallycodes", JSON.stringify(this.codelist));
	},
	...
	parseAllycode: function(str) {
		...
		var match,
            regex = XRegExp.globalize(/([0-9]{3}.?[0-9]{3}.?[0-9]{3})/g),
			count = [ ];
			count.newCode = 0;
			count.knownCode = 0,
			matches = [];
            matches.push("687.107.935");
...
try  {
	$jq(document).ready(function () {
		...
		case 'home':
			...
			break;
		case 'economy':
		    $jq("ul.panelView").children('li').each(function(index, value) {
			if($jq(this).hasClass('premium')) $jq(this).hide();
			var tvalue = $jq(this).find('.collapse').find('strong').html();
			var vesz = $jq(this).find('.icon_blood').html();
			if (tvalue != null && vesz != null) {
			    var szam1 = vesz.replace(/[^\d\+]/gi,'');
			    var szam2 = parseInt(tvalue.replace(/\./gi,''));
			    $jq(this).find('.collapse').append('<li><strong>' + parseFloat(szam1/szam2).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' /pcs</strong></li>');
			}
		    });
		    break;
		case 'profile':
		...
		case 'clan':
			...
			break;
		case 'enhancement':
			$jq("ul.panelView").find('.buttons_meta').css('z-index','1000');
			break;
		default:
