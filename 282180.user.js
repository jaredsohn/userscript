...
// @version       0.14.1.2
// @description   Download Album Facebook & Instagram dengan satu klik.
// @namespace     http://userscripts.org/users/83150
// @updateURL     https://userscripts.org/scripts/source/282180.meta.js
// @downloadURL   https://userscripts.org/scripts/source/282180.user.js
...
unsafeWindow.dFAcore = function(setup) {
  ...
  if(location.href.match(/.*facebook.com/)){
    ...
    try{
      var s=qSA("script"), env;
      for(var i=0,t; t=s[i].textContent, i<s.length; i++){
        if(t.match(/require\("ServerJS"\)/)){
          env = JSON.parse(t.slice(t.indexOf("handle(")+7,-1)).define; break;
        }
      }
      for(i=0;i<env.length;i++){
        if(env[i][0]=="CurrentUserInitialData")g.Env.user = env[i][2].id;
      }
    }catch(e){alert('Cannot load required variable');}
