// ==UserScript==
// @name           [IBFSU] Ikariam - voir les temps de recherche partout
// @namespace      Ikariam IBFSU
// @description    Voir les temps de recherches où l'on veut sur ikariam
...
{
	var version = '0.1.0';
	var script_name = '[IBFSU] Ikariam - voir les temps de recherche partout';
	...
	{
		var url = 'http://blackfantome.free.fr/script/ikariam/ibfsu.latest.user.js';
		alert ('Vous devez installer le script [IBFSU] BlackFantome Script Utilities for Ikariam\n'
				+ 'Ce script est disponible à cette adresse: ' + url + '\n'
				+ 'Une fois ceci fait, placez le script au dessus de celui-ci dans la liste');
		return;
	}
	mywindow.Version.check (script_name, version);
	...
	var Actions =
	{
		...
		{			
			...
			{
				if (split[i] == Options.current_page.toLowerCase ())
					continue;
				new_value += split[i] + '|';
			}
			Tools.setValue ('ResearchShowPage', new_value);
	...
	var Research =
	{
		...
		addCookie: function ()
		{
			...
			Cookies.addCookie ('ResearchShowPage', '|city|researchAdvisor|academy|');
		...
		createInfo: function ()
		{
			...
			const show = Tools.getValue ('ResearchShowPage', '').indexOf (Options.current_page.toLowerCase ()) != -1;
