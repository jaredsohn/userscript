...
function init() {
	MoCheck.resourceBundle = {
		...
		'dialog.tab.work.nothingSelected.1':'Вами не выбрана не одна работа!',
		'dialog.tab.work.nothingSelected.2':'Для занесения в список, откройте необходимую вам работу и добавьте её',
		'dialog.tab.work.tableHeader.work':'Работа',
		'dialog.tab.work.tableHeader.points':'ТО',
		'dialog.tab.work.tableHeader.points.popup':'Отображённые здесь трудовые очки, не изменяются при<br /> переодевании, для измения перезапустите окно Мотивация',
...
function addMotivationButton() {
	...
	moBtn.innerHTML = '<span class=\"button_right\" style="display:inline; width: 128px; height:25px; margin-left:-2px; margin-top:-1px; float:left; background:url(http://s002.radikal.ru/i199/1002/52/bf4f1b34e2b7.png);"></span>'; +
		...
		if($ES('a', 'right_menu').length > 8) {
			if($ES('a', 'right_menu').length > 9) {
				if($ES('a', 'right_menu').length > 10) {
					$('abdorment_right').setStyle('top', parseInt($('abdorment_right').getStyle('top').substring(0, $('abdorment_right').getStyle('top').indexOf("px"))) + 106);
					 } else {
				$('abdorment_right').setStyle('top', parseInt($('abdorment_right').getStyle('top').substring(0, $('abdorment_right').getStyle('top').indexOf("px"))) + 78);
				} } else {
			$('abdorment_right').setStyle('top', parseInt($('abdorment_right').getStyle('top').substring(0, $('abdorment_right').getStyle('top').indexOf("px"))) + 51);
			} } else {
		$('abdorment_right').setStyle('top', parseInt($('abdorment_right').getStyle('top').substring(0, $('abdorment_right').getStyle('top').indexOf("px"))) + 26);
	} } 
