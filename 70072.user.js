// ==UserScript==
// @name           Trizzay Check for glowing tanks.
// @description    Checks tanks Trizzay wanted for if they are glowing to help her stalk her tank.
// @include        *
// ==/UserScript==
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								...
								if(open===true){
									...
									GM_openInTab('http://absol.site88.net/gtools/glowTime.php?id='+tankId);
									GM_openInTab('http://www.gaiaonline.com/forum/t.58871515/');
								}
							}
							else{
								//alert('over');
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(3466057,false);//Trizzay
	}
}
checkTanks();
