...
(function () {
	...
	var posts = [], pageBuild = [], comments = [], depth=[], newComments = 0;
	pageBuild[0] = Layer1.innerHTML.split(/<div class\="date_header_text">Comments<\/div>/)[0];
	pageBuild[1] = '';  // ?? Comments (?? new) populated after numbers are determined
	...
	for (var i=0; i<spacers.length; i++) {
		if (spacers[i] && spacers[i].src.indexOf('/images/spacer.gif') > -1) {
            ...
            if (newComment) newComments++;
		}
	}
    pageBuild[1] = '<div class="date_header_text">' + posts.length + ' comments'
        + ((newComments > 0) ? ' (' + newComments + ' new)</div>' : '</div>');
