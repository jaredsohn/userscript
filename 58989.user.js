// ==UserScript==
// @name           Favorites Spy Reports RU 1.0 by trijin
...
// @include        http://*.ogame.ru/game/index.php?page=messages&session=*
// @version 1.1
...
var Favors = {
	createButtons: function(e) {
		...
		var a = unsafeWindow.$('<a>').attr({
			...
			'id': 'ogc20ShowFavor',
			'title': '|Избранное',
		}).addClass('tipsStandard').addClass('aFavorButton').append('<img height="16" width="16" src="img/icons/info.gif"/>');
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Этот доклад уже в избранном');
				return false;
			}
	...
	Fsave: function(obj) {
		try {
			...
			unsafeWindow.fadeBox('Изменения сохранены');
		} catch (e) {}
	},
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">От</th><th class="subject">Тема</th><th class="date">Дата</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Командование флотом</td><td class="subject">' +
				...
				'<a title="|Удалить из избранного" class="ogc20Fdel tips" rel="'+el.id+
			...
			unsafeWindow.fadeBox('нет сохраненных докладов');
		}
		return false;
	},
