...
// @version        1.6.3
// @description    (v1.6.3) Bookmark your preferred villages in a draggable table available on every page! Fast links to your farms, safehouses, targets etc. x T 3.5
// @author         dbKiller
...
function main()
{
	...
	//alert('arrivato a lancio creazione lista');
	//create list
	createVMList();
}
...
function createVMLink()
{
	...
	if (!tbody) tbody = dom.xs('//div[@id="map_details_actions"]/table/tbody');
...
function getOwnerId()
{
//	var uIdLink = find("a[contains(@href, 'spieler.php')]", XPFirst);
//	if (uIdLink == null) uIdLink = find("a[contains(@href, 'spieler.php')]", XPFirst);
//	var id =  (uIdLink) ? uIdLink.href.split("uid=")[1] : '';
//	return id;	
	var user = dom.xs('//a[contains(@href, "spieler.php?uid=")]');
	//alert('user: ' + user);
...
{
	var activeVillageLink = dom.xo('//tr[@class="sel"]/td');
	//alert('activeVillageLink: ' + activeVillageLink.snapshotLength);
	if (activeVillageLink.snapshotLength > 0) {
		//multi village account
		//alert('x:' + activeVillageLink.snapshotItem(2).innerHTML);
		//alert('y:'+ activeVillageLink.snapshotItem(4).innerHTML);
		var x = parseInt(activeVillageLink.snapshotItem(2).innerHTML.replace("(", ""));
		var y = parseInt(activeVillageLink.snapshotItem(4).innerHTML.replace(")", ""));
...
{
	var html = dom.xs('//a[starts-with(@href, "karte.php?z=")]');
...
{
	var typeObject = dom.xs('//img[starts-with(@id, "f")]');
	if (typeObject) {
...
{
	/*
	var mapDetailsRight = dom.xs('//span[@class="b"]');
	if (mapDetailsRight==null) mapDetailsRight = dom.xs('//div[@id="lmid2"]/div[@class="map_details_right"]');
...
{
	//alert("getVillageName");

	var villageName = dom.xs('//div[@id="map_details_info"]/div').innerHTML;

	//alert('villageName: ' + villageName);
	
	if (!villageName) {
		title = dom.xs('//h1').innerHTML;
...
function getPlayerName()
{
	...
	if (user==null)
		user = dom.xs('//div[@id="lmid2"]//a[starts-with(@href, "spieler.php?uid=")]');
...
{
	var mapDetailsRight = dom.xs('//div[@id="map_details_info"]');
	if (mapDetailsRight == null) { //abandoned area/empty oasis
		var playerTribe = '';
	} else {
		var playerTribe = dom.xs('//div[@id="map_details_info"]/table/tbody/tr/td[2]/b').textContent;
	}
...
{
	var user = dom.xs('//a[starts-with(@href, "spieler.php?uid=")]');
...
{
	var ally = dom.xs('//a[starts-with(@href, "allianz.php?aid=")]');
