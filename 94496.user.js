...
// ==UserScript==
// @name          HOMMK++
// @author        Julien Eyries and + Zahn
// @namespace     http://www.jeyries.fr/hommk
...
var version = '1.0.2'; // also, check meta-data field @version
...
Message.prototype.from_HOMMK = function( content ) {
    ...
    case 'TROOP_SCOUTING':
        ...
        if (contentJSON.targetedPlayerName)
            str.push( 'Player: ' + contentJSON.targetedPlayerName);
        ...
        foreach( contentJSON.regionUnitStackList, function(unit, i) {
            ...
            if (i==0) { str.push( '' ); str.push( 'Troops:' ); }
        ...
        foreach( contentJSON.heroList, function(hero) {
                ...
                if (tmp.length) str.push( 'Spells: '+ tmp.join(', ') );
                ...
                if (tmp.length) str.push( 'Equipment: '+ tmp.join(', ') );
...
function build( panel ){
   ...
   // build request for player names 
    ...
    unique(playerId).forEach(function(item){
        elParamList.push({"elementType":"ProfileFrame","elementId":item});  
    });


    do_request(
 
    {"elParamList":elParamList}
    ...
    ,function(json) {
        ...
        moves.forEach( function(obj){
            ...
            var playerName = json.d["ProfileFrame"+obj.masterHeroMove.playerId].playerName;

            var row = table.insertRow(-1);
            row.style.border = "1px solid gray";
            row.style.borderCollapse = "collapse";
            //row.style.backgroundColor = (k % 2 == 0) ? "black" : "gray";
            k ++;
...
function init() { 
	$("<link rel='stylesheet' type='text/css' />").attr("href", 'http://www.gilgarcia.org/hommk.css').appendTo("head");
    ...
    var $expander = $('<a href="#" class="ui-state-default"  style="width: 100%; display: block; text-align: center;" ></a>')
    ...
    $('<button id="hommk-user-jactari-button" style="margin-top:0px; width: 100%; display:block;">Jactari</button>')
    ...
    $('<button id="hommk-user-scribe-button" style="margin-top:5px; width: 100%; display:block;">Scribe</button>')
        .button({disabled:true})
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<button style="margin-top:5px; width: 100%; display:block;">Watcher</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<button style="margin-top:5px; width: 100%; display:block;">Cartographer</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<input type="text" id="coordinates" size=7 style="margin-top:5px; display:block;" value=xxx,yyy />')
        .appendTo($panel_extended);

    $('<button style="margin-top:5px; width: 100%; display:block;">Jump to</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<button style="margin-top:5px; width: 100%; display:block;">About</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
            launch_about();
