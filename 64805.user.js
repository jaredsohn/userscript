...
// ==UserScript==
// @name            experienceproject-msg
// @namespace       Platypus
// @include         http://www.experienceproject.com/member_mail.php?mmrt*
// @description     Uses wide layout for experienceproject.com messages.  Changes "Inbox" action to go to Sent folder.  Changes "Delete" action to skip confirmation dialog.  Changes "Block" action button to compose NewMsg action.  Adds other user's username to the window/tab title.  Edit the script and set my_username to your EP username to get the window/tab title hack working.
// @date            04 January 2010
// @author          http://NewChrissy.hi5.com
// @discussion      http://NewChrissy.hi5.com
// ==/UserScript==
function do_platypus_script() {
window.status = "Platypus running";

var my_username = "NewChrissy";  // Change this value to your EP username

// add other user's name to the Window/Tab Title
// change "Inbox" action to go to Sent folder
// change "Delete" action to skip confirmation dialog
// change "Block" action button to compose "NewMsg" action
var anchorTags = document.getElementsByTagName('A');
var msg_html = "none";
var msg_username = my_username;
var other_userID = "";
var other_username = "";
var found
for(var i=0; i<anchorTags.length; i++) {
   if (anchorTags[i].className == "msg_sender_link") {
      if (anchorTags[i].innerHTML != my_username) {
         other_userID = anchorTags[i].href.replace("http://www.experienceproject.com/profile.php?m=","");
         other_username = anchorTags[i].innerHTML;
         // alert(anchorTags[i].href + " " + other_userID + " " + other_username);
   }; };
   if (anchorTags[i].className == "mail_action_link") {
      if (anchorTags[i].innerHTML == "Inbox") {
         anchorTags[i].innerHTML = "Sent";
         anchorTags[i].href = "http://www.experienceproject.com/member_mail.php?mmsent=true&read=&mmstart=0";
      };
      // The following attempts to disable/skip the onClick confirmation dialog, but it fails.
      // A work-around is dragging the "Delete" link onto the address bar.
      if (anchorTags[i].innerHTML == "Delete") {
         // alert(anchorTags[i].innerHTML);
         // The next two fail because the onclick property gives a permission error
         // anchorTags[i].innerHTML = '<a href="' + anchorTags[i].href + '" onDblClick="return true;">DELETE</a>';
         // anchorTags[i].innerHTML = '<a href="' + anchorTags[i].href +    '" onClick="return true;">DELETE</a>';
            anchorTags[i].innerHTML = '<a href="' + anchorTags[i].href +                           '">DELETE</a>';
         // alert(anchorTags[i].innerHTML);
      };
      if (anchorTags[i].innerHTML == "Block") {
         anchorTags[i].innerHTML = "NewMsg";
         anchorTags[i].href = "http://www.experienceproject.com/member_mail.php?mmcomp=t&mmm=" + other_userID;
}; }; };
document.title = other_username + " | " + document.title;

// Remove background:
fix_page_it(window.document,document.evaluate('/HTML[1]/BODY[1]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue);

// var main_copy = document.getElementById('main-copy');  // EP may change this id from time to time!  
// location when last checked: /HTML[1]/BODY[1]/DIV[1]/DIV[1]/DIV[13]

isolate(window.document,document.getElementById('focus_pane'),null,null,null);
relax(window.document,document.getElementById('focus_pane'),null,null,null);
set_style_script(window.document,document.getElementById('message'),"height: 450px;width: 1190px;",null,null);

// add a space at the beginning of each message body to make it easier to select with the mouse:
do_modify_html_it(window.document,getElementById('focus_pane'),/<div class=\"msg_body\">/g,'<div class="msg_body">&nbsp;',null);
