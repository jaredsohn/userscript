...
//Im using data-links for images. they don't refer to a server but are the image itself! if you want to use other pictures, put in the url or use e.g. this page to transform into data-links: http://software.hixie.ch/utilities/cgi/data/data  
var devico = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8%2F9hAAAABGdBTUEAALGPC%2FxhBQAAABl0RVh0U29mdHdhcmUAUGFpbnQuTkVUIHYzLjUuNUmK%2FOAAAALbSURBVDhPjZNrSFNhGMcXkaYOzSltWRhIhBGEu7rN6dzlHHM6nc5NttRMXd6yC0RhloQRRhcjsCL90IcuFEgo5Icoc6Y5r1PD1EyULCkL%2BtIn%2B%2FLvOcdlBEEe%2BPGc97zv83%2F%2Fz%2FNwNugtFsns1MT4ys8VsWCdT7goYmxvfLy1s61zSSCJ2%2FH9aL0Z3lkVumcT4X2vRs%2BcFr0LOvTOJ%2FHxFa175jR0Rs2fc1ekI0Isekj3BQlEMdHwvlOCsVnB2h2wHiyGs6oapbW1KKurg4dwHzsOW6kH%2B%2FNdyHJnoLHFjNDIMC8J7OYFXs6ooLdYYbRmw2jLg6OyBK6achSdPIKCE1XIry5DTmkBMt0O6MyZvEBQcNAbEjCsCWhp48ELLfzfpMg%2FnAFroRvZh1zIKXEis8CBdKcDaXYbtEZycMeETZuD35IAIxDvjCEHSiSzFhTXMBj5LMPQEoccvkU5%2Bj9Q%2FCjH4CcZmh8ZwOaacPG2ESERoZNrAl1TSjS26jBACa%2Fn5RigBHtRGq7dS8GVu3qYs63IKbKgYzSRFz%2FXRAJbwsZIQMM76BxVYeSLDOdvmOD0sHSbHC1Ptbj%2FTA3%2FVynKa43QMSwqTptoMgqcvWpASGTYOAkoVwX8KviXpdAY06hGFu2DiRhdlsHiYjBBPRmmsqZ%2FSNG%2FqMDNtiTUX08lB0KuiQEHJMDV7DmlR2u7Dr4FOd%2BLhlvJONOkx2Uqz%2B4xQ53CQqZhcaFZTw6EnAPVmgPOdh8ldk0r0PxEh7wyhkaWBoMlQDoLA6HUMb8F%2FCSg4AWeTyrhoAQmOx1mqwWmTIKL%2F0CZ%2FJcDtSB2Vxy6ZxQwZzHgNv9HYaUZDX9K0Aj2JOzrsB0w8bbWQ0NzCjVbCXHsNh%2BVkMT9fxujtm99HBYV3hcaKRwK4AsVCfvpfSCwHqTIMUzNGxNJonsp7xIh4wUICZHAj0UgUPPzXUVLpARu0lFMJVgiN%2FA98hdY3sC7dJ2NBQAAAABJRU5ErkJggg%3D%3D";
var logobild="<a id=\"devlogo\" href=\"http://my.deviantart.com/messages/\"><img src=\""+ devico +"\" alt=\"\"/></a>";//HTML-Logo
				...
				onload: function(responseDetails){
					if(responseDetails.status==403){
						if(GM_getValue('devLogin'+x)!=1){GM_setValue('devLogin'+x,1);}
							...
							var patternfav=/\d+","icon":"<img src=\\"http:\\\/\\\/i\.deviantart\.net\\\/icons\\\/activity\\\/favgold\.gif/ig;
							var patternwatch=/\d+","icon":"<img src=\\"http:\\\/\\\/i\.deviantart\.net\\\/icons\\\/userpage\\\/friends\.gif/ig;
							...
							var amountfav = addid(texx.match(patternfav))+0
							var amountwatch=addid(texx.match(patternwatch))+0
							...
							if(amountwatch==0){amountwatch=texx.split("display_class\":[\"watched,\"acitivity").length-1;}
							if(amountpoll==0){amountpoll=texx.split("display_class\":[\"poll").length-1;}
							var amount = new Array(new Array(amountdev,"Deviation","messages/#view=deviations"),new Array(amountjorn,"Journal","messages/#view=journals"),new Array(amountpoll,"Poll","messages/#view=polls"),new Array(amountcom,"Comment","messages/#view=feedback"),new Array(amountnote,"Note","notes/"),new Array(amountbadge,"Badge","messages/#view=activity"),new Array(amountcont, "Contest","messages/#view=contests"),new Array(amountnews, "Hottopic","messages/#view=hottopics"),new Array(amountfav, "Favorite","messages/#view=activity"),new Array(amountwatch, "Watch","messages/#view=activity"));
							var texxt = logobild + messer(amount);
...
function messer(ress){//sums the ressource-array up in a nice text
	var raus = "";
	var plur = "";
	var mehr = "";
	var zwiar=new Array()
...
function layouter(){ //Toolbarappearence is defined.
	...
	var devbarhint = "url(data:image/gif,GIF87a%01%00%14%00%84%13%00o%86rs%8Awx%8D%7B%7C%92%7F%81%96%84%86%9A%88%8A%9E%8C%8F%A2%90%94%A6%95%98%AA%99%9E%AE%9E%A1%B3%A2%A7%B6%A6%AB%BB%AB%B0%BF%AF%B5%C3%B3%BA%C7%B8%BE%CB%BD%C2%CF%C0%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%2C%00%00%00%00%01%00%14%00%00%05%11%A0%24E%D0%E34%CC%A2%24%C8a%14%C4%20%04%40%08%00%3B)"; //background
	neustil="#devlogo img{border:none;} #devlogo{float:left;text-decoration:none;margin:2px 5px 2px -7px;} #devinfo{float:left;font:normal 14px Helvetica;height:20px;background-image:"+ devbarhint +";color:black;line-height:20px;";
	switch (anz) {
		case 1:
			if(location.href.indexOf('deviantart')!=-1){
				...
				inhalt="<div id='devinfo'>"+logobild+"Please login first!</div>";
				neustil+="padding-left:10px;}"; //#speiid{cursor:pointer;float:left;margin-left:5px;}";
				//A Link to save the cookie and a text.
			}else{
				//Somewhere else
				inhalt+="<div id='devinfo'>"+logobild+"Please visit <a href='http://www.deviantart.com/users/login'>Deviantart</a> to login!</div>";
				neustil+="padding-left:10px;}";
				//Link to Deviantart
			}
			break;
		case 0:
			inhalt+="<div id='devinfo'>"+logobild+"Loading. Please wait.</div>";
			neustil+="padding-left:10px;}";
			break;
		default:
			inhalt+="<div id='devinfo'>"+logobild+"Error!</div>";
			neustil+="padding-left:10px;}";
			break;
	}
	neustil+="#devtoolbar{height:20px;margin:0px;paddingBottom:0px;position:fixed;bottom:0px;left:0px;z-index:999999;}"; //ToolbarCSS
	neustil+="body{height:100%;}";//To set the toolbar at the bottom
	neustil+="#devtoolbar a:link,#devtoolbar a:visited,#devtoolbar a:active{color:#1c4b11;font:normal 14px Helvetica;text-decoration:underline;}"+
				"#devtoolbar a:hover{color:black;}";
