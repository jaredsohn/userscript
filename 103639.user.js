...
/******************
Current:
*) Fixed march time (marches taking longer than they should)
*) auto-build: check city before building lvl6+ (prevent CG usage from user building manually)
*) Fixed problem which caused camp attack to stop, requiring refresh to restart it
*) Added max marches config to ant attack

TODO: 
wave: option to log attacks
auto-build: check city before building lvl5+ (prevent CG usage from user building manually)
...
var OptionsDefaults = {
  ...
  autoTrain   : {enabled:false, trainingEnable:[] },
...
//  WinLog.writeText (scripts[i].src +':\n'+ code);
    var i1=0, i2=0;
    if (code.indexOf('sessionId') >= 0){
      while ((i1=code.indexOf('/*', 0)) >= 0){
        i2 = code.indexOf('*/', i1);
        code = code.substr(0, i1-1) + code.substr(i2+2);
      }
      var regexp = /(.*)C.attrs.(.*)\s*=(.*)|(.*)C.attr\s*\((.*),(.*)\)/g;
...
logit ('dtStartup');    
  ...
  //return;
    ...
    document.getElementById('cn').style.margin = '';   
  ...
  //C.attrs.sessionId = 'xd234';  //test session id failure
    Seed.init(function(rslt){
      if (rslt.ok){
        ...
        dialogFatal ('Unable to start DOA Power Tools:<BR>'+ rslt.errmsg);
        return;
      }
    });
...
function dialogFatal (msg){
  pop = new CPopup ('dtfatal', Data.options.ptWinPos.x, Data.options.ptWinPos.y, 350,300, Data.options.ptWinDrag); 
...
function checkVersion (){
  ...
  if (Data.options.version.lastCheck > now-(VERSION_CHECK_ITER))
    return;
  logit ('CHECKING VERSION.....');   
  var timer = setTimeout (checkVersion, VERSION_CHECK_ITER*1000);
  GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://userscripts.org/scripts/review/102481',
    onload : function(r){
...
//Data.options.waves.runTime = 3600;
    ...
    if (Data.options.waves == null){
      Data.options.waves = {enabled:false, timeLimit:240,  terationMin:10, iterationMax:13, stopOnLoss:true, deleteReports:false, curTarget:0, targets:[], tsStarted:serverTime(), runTime:0};
      for (var i=0; i<5; i++)
...
//logit (inspect (rpt, 8, 1));
  ...
  // Data.options.campAttack {enabled:false, maxDist:7, repeatTime:3660, delayMin:10, delayMax:25, levelEnable:[]}
...
t.addMarch (rslt.dat.result.job);        
 ...
 // Data.options.campAttack {enabled:false, maxDist:7, repeatTime:3660, delayMin:10, delayMax:25, levelEnable:[], levelDist:[]}
  ...
  tabConfigGeneral : function (){
    ...
    var m = '<DIV class=pbTitle>Auto-attack Configuration</div>\
      ...
      <TR><TD class=pbTabLeft>Maximum marches:</td><TD><INPUT class=short id=pbaacfgMM maxlength=2 type=text value="'+ Data.options.campAttack.maxMarches +'"\></td></tr>\
      </table>';
    document.getElementById('pbatContent').innerHTML = m;
...
Tabs.Train = {
  ...
  trainTimer : null,
  statTimer : null,
  troopsCapital : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'Swift Strike Dragon', 'Battle Dragon', 'Armored Transport', 'Giant', 'Fire Mirror'],
  troopsOutpost1 : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'Swift Strike Dragon', 'Battle Dragon', 'Armored Transport', 'Giant', 'Fire Mirror', 'Fangtooth'],
  
  // Data.options.autoTrain {enabled:false, trainingEnable:[] }
  init : function (div){
    var t = Tabs.Train;
    t.cont = div;
    for (var i=0; i<Seed.s.cities.length; i++)
      if (!Data.options.autoTrain.trainingEnable[i])
        Data.options.autoTrain.trainingEnable[i] = {};
      var m = '<DIV class=pbTitle>Auto Train</div>\
        <DIV style="height:100px" class=pbStatBox><CENTER><INPUT id=pbtrnOnOff type=submit\></center>\
        <DIV id=pbbldBldStat></div> <BR> <DIV id=pbbldFeedback style="font-weight:bold; border: 1px solid green; height:17px"></div>  </div>\
        <DIV id=pbbldConfig class=pbInput>';
//logit ('TRAIN OPTS:\n'+ inspect (Data.options.autoTrain, 5, 1));        
    for (var i=0; i<Seed.s.cities.length; i++){
      var list =  (i==0? t.troopsCapital : t.troopsOutpost1);
      var city = Seed.s.cities[i];
      m += '<DIV class=pbSubtitle>City #'+ (i+1) +' ('+ city.type  +')</div><TABLE class=pbTab>';
      for (var ii=0; ii<list.length; ii++)
        m += '<TR><TD><INPUT type=checkbox id="pbbldcb_'+ i +'_'+ list[ii] +'" '+ (Data.options.autoTrain.trainingEnable[i][list[ii]]?'CHECKED':'') +' /></td><TD>'+ list[ii] +'</td></tr>';  
      m += '</table>';
    }    
    m += '</div>';
    div.innerHTML = m;
    ...
    function checked (evt){
      var id = evt.target.id.split ('_');
      var cityId = Seed.s.cities[id[1]].id;
      Data.options.autoTrain.trainingEnable[id[1]][id[2]] = evt.target.checked;
    }
  },
    ...
    Seed.notifyOnUpdate(function(){
        ...
        for (var ic=0; ic<Seed.s.cities.length; ic++ ){
          ...
          if (getBuildJob (ic) == null){     // city not currently building
            // find lowest level eligible building ...
            var bl = [];
            for (var p in Data.options.autoTrain.trainingEnable[ic]){
              if (Data.options.autoTrain.trainingEnable[ic][p])
                bl = bl.concat (Buildings.getList (ic, p));
            }
            var building = null;  
            var lowest = 9; 
            ...
            if (building != null){
              if (building.level>5 && !t.reChecked){ 
logit ('TRAIN: rechecking city');
                t.reChecked = true;
                Seed.fetchCity (city.id, 1000);
                t.trainTimer = setTimeout (t.buildTick, 500);
              } else {
                t.reChecked = false;
                t.doBuild (building, city);
              }
...
t.reChecked = false;        
        if (nothingToDo){
          t.dispFeedback ('Nothing to do, disabling auto-train.');
          t.setEnable (false);
          return;
        }
        t.trainTimer = setTimeout (t.buildTick, 8000);
    }); 
  },
  ...
  doBuild : function (building, city){
    ...
    t.dispFeedback (msg);
    Ajax.buildingUpgrade (city.id, building.id, function (rslt){
//  logit ('TRAIN RESULT: '+ inspect (rslt, 7, 1)); 
      t.statTick();
      if (rslt.ok){
        t.errorCount = 0;
        actionLog (msg);
        ...
        t.trainTimer = setTimeout (t.buildTick, 20000);
        return;
      }
    });
  },
