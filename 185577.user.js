...
function CCoordlist() {
	...
	this.handlePressedKeys = function(e) {
		...
		if(pressedChar == coordList.activationKey.toLowerCase()) {
			if(busy === false) {
				display.show("DS maak BB CoordLijst op de kaart: Lijst maken");
...
function CInlinePopup() {
	...
	this.init = function(message) {
		...
		closer.appendChild(document.createTextNode("Sluiten"));
		...
		description.appendChild(document.createTextNode("Hieronder is de lijst die u zojuist heeft gemaakt. Als u alleen de coördinaten wilt zonder BBCodes, verwijdert u het vinkje bij 'toe te voegen BBCodes"));
		...
		label.appendChild(document.createTextNode("BBCodes toegevoegd"));
