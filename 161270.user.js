...
// @include     *://*/view.php?id=*
// @require     http://userscripts.org/scripts/source/161119.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link.com/*
// @include     *://*df.ly/*
...
self == top){
	...
	$('frameset frame:first').error(function(){wparent.opener.success('fail');})
	if($('frame:last').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('betterbux')!=-1)cheatCheck = [3];
		var inters = setInterval(function(){
		...
		var inters = setInterval(function(){
			...
			$('#buttons',doc).show();
			div.text('wait: '+ (fulltimer-=2)+ ' - timeout: '+ iframeLoad)
			if($('#timer',doc).text().indexOf('Click') >= 0){
				...
				if(primeImg.size()){
					...
					primeImg.load(function() {
						gameClick();
					}).ready(function(){gameClick();}).error(function() { wparent.opener.success('fail'); })
