// ==UserScript==
// @name             ESTROUTREJEN ATACK
// @version          1.2.1
// @namespace        ESTROUTREJEN ATACK
// @homepage         http://userscripts.org/scripts/review/105333
...
// @require             http://userscripts.org/scripts/review/105333
...
//
//Este codigo muestra los valores de la pantalla  de ver ataques 
//
...
ShowViewAttacksDialog:function() {
		...
		typesel += '<option value="bulkadded">Coordenadas agregadas</option>';
		typesel += '</select>';
		
		div.style.display='block';
		div.innerHTML='';
		var m = '<DIV id="srcAttackOpts" style="height:30px">\
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Buscar por: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Todos</option>\
		<OPTION value=Camp>Barbaros</option>\
		<OPTION value=Wilderness>Tierras virgenes</option>\
		<OPTION value=Grassland>Llanuras</option>\
		<OPTION value=Lake>Lagos</option>\
		<OPTION value=Mountains>Montanias</option>\
		...
		<a id="KOCAttackViewAttacksList" class="button20"><span>Lista ataques</span></a>\
		...
		//<a id="KOCAttackViewAttacksImportExport" class="button20"><span>Importar / Exportar</span></a>\	
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Ciudad</td><td>Coordenadas</td><td nowrap>What</td>';
		  h += '<td>Type</td><td>Attack Troops</td><td>tropas suicidas/Recursos</td><td>Distancia</td><td>Time</td><td>Ignore</td></tr>';
		  var tableRows= '';
		  var count = 1
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Orden de ataque: <select id='KOCAttackOrder'><option value='mostTroops'>Usar el mayor numero de tropas primero</option><option value='closest'>Objetivos cercanos</option></select><br />"+
			"Attack Type: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbaros "+
			...
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transportes<br> "+
			"Prioritise attacks for : "+attackfirst+
			"<br><br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> segundos de espera desde cada ataque<br />"+
			...
			"Attacking wilderness:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60))+"' size='3' />mins<br />"+
			" ataca ciudades:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60))+"' size='3' />mins<br />"+
			...
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> maxima distancia desde la ciudad al ataque o transporte<br />"+
