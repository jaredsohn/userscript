scr_meta=<><![CDATA[
// ==UserScript==
// @name           Vérifier les attaques sur CdR
// @namespace    Vérifier les attaques sur CdR
// @description   permet d'empêcher les attaques entres français en demandant confirmation avant l'attaque d'un français et en enlevant le lien permettant d'attaquer en cliquant sur un perso français
// @include        http://*campagne-de-russie.com/jeu*
// @version        1.1.1
// ==/UserScript==
]]></>.toString();

var classe = "attaque_fra";
initialise_attaque();
...
function verifie_attaque()
{
	...
	{
		var confirmation = prompt("Êtes-vous bien sûr de vouloir attaquer une unité française ? Si vous le voulez vraiment, tapez 'oui' mais ce choix pourrait vous faire avoir des problèmes avec la justice...");
		if (confirmation == "oui")
		{
			if (confirm("Vraiment, vous voulez vraiment attaquer un français ?!? Cliquez donc sur 'Annuler', il n'est pas encore trop tard !"))
			{
				alert("Humm, j'espère que vous aviez de bonnes raisons de tirer sur nos troupes.");
				form.submit();
			}
...
function initialise_attaque()
{
	...
	var i =0;
	var source = document.getElementsByClassName("case")[4].innerHTML;
	//alert(source);
	while (allies[i])
	{
	...
	document.getElementsByClassName("case")[4].innerHTML = unescape(source);
...
var AnotherAutoUpdater = {
 ...
 id: '80809', // Script id on Userscripts.org
