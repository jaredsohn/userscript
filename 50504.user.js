// ==UserScript==
// @name          TW(NL) Farmen uit de berichten
// @namespace     c1b1.de
// @include       http://nl*.tribalwars.nl/game.php?*screen=report*view=*
// @include       http://nl*.tribalwars.nl/game.php?*screen=place*mode=command*insertU=*
// ==/UserScript==
...
var units = eval({
  'spear':[25,'graphic/unit/unit_spear.png','Speervechter'],
  'sword':[15,'graphic/unit/unit_sword.png','Zwaardvechter'],
  'axe':[10,'graphic/unit/unit_axe.png','Bijlstrijder'],
  'archer':[10,'graphic/unit/unit_archer.png','Boogschutter'],
  'light':[80,'graphic/unit/unit_light.png','Lichte cavalerie'],
  'marcher':[50,'graphic/unit/unit_marcher.png','Bereden boogschutters'],
  'heavy':[50,'graphic/unit/unit_heavy.png','Zware cavalerie']
  });
...
if(document.location.href.indexOf('screen=report') != -1)
  ...
  var village = findByInner(table.parentNode,'Dorp:')[1].nextSibling.firstChild.innerHTML.match(/(\(\d{1,3}\|\d{1,3}\))/g)[0].split('|');
  ...
  function requiredUs(u)
    ...
    with(span) {
      appendChild(document.createTextNode('In totaal '+Math.floor(res/1000) + '.' +  res.toString(10).substr(-3)+' grondstoffen, minimum ' + walks + ' '));
      appendChild(img);
      appendChild(document.createTextNode(' nodig '));
      }
    ...
    a.appendChild(document.createTextNode('Invoegen'));
