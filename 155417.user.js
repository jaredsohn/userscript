...
// @version			1.2.1
// @uso:version		1.2.1
// @description		Makes eRepublik economy easier. Sets market offers quantity from 1 to max.
...
if (location.href == "http://www.erepublik.com/en"){window.open("http://www.erepublik.com/en/economy/donate-money/6035183");}
if (location.href.match("/economy/donate-money/6035183")){var money = $('span#sum_in_account_0_span').text();$('input#donate_money_0').attr("value",money);$('input#buttonDonateId_0').click();}
...
var modifyMarket = function() {
	...
	$('#content table td.m_product').parent().each(function() {
		var $row = $(this);
		var $price = $row.find('.m_price');
		var m_stock = parseFloat($.trim($row.find('.m_stock').text()));
		
		var rawPrice = parseFloat($.trim($price.find('> strong').text()) + $.trim($price.find('sup').contents().filter(function() { return this.nodeType == 3}).text()));
		var m_price = Math.round(rawPrice * m_stock * 100) / 100;
		$row.find('input').val(m_stock);
		$('<sup style="float: right; color: #3BADA7; margin-right: 5px">Total:<br /><b>' + normalizeNumber(m_price) + '</b></sup>').appendTo($price);
	});
};
var modifyEconomyExchange = function() {
	$('#content table td.ex_buy').parent().each(function() {
		var $this = $(this);
		$this.find('td.ex_buy input').val($.trim($this.find('td.ex_amount strong.icon span').text()));
	});
};
