// ==UserScript==
// @author      Risi (edited for 3.5 by Nevam) (edited for firefox 3.5 by _Bernard )
 ...
 *********************/
if (init) {
    var sCurrentVersion = "1.5.7";  //Version number with which we need to run the update fu
	...
	* -> Please post all translations here: http://userscripts.org/topics/27738
	...
	case "ba":  //by bhcrow
		...
		aLangTasks = ["Izgradi", "Unaprijedi", "Napad", "IstraÃ…Â¾i", "ObuÃ„Âi"];
		aLangStrings = ["Gradi poslije", "Unaprijedi poslije", "Napadni poslije", "IstraÃ…Â¾i poslije", "Isplaniraj ovaj zadatak za poslije.", "PoÃ„Âela je gradnja ", " pokuÃ…Â¡ano je s nepoznatim rezultatom.", "stepen", " ne moÃ…Â¾e biti izgraÃ„â€˜eno.", " ne moÃ…Â¾e se unaprijediti.", "Isplaniran je zadatak.", "Aktualna produkcija:", "Ne moÃ…Â¾e se isplanirati ovaj zadatak sada.", "Planirani zadatak nije dostupan!", "Planirani zadaci", "izbriÃ…Â¡i", "PoÃ…Â¡alji poslije", "Trupe nisu odabrane.", "VaÃ…Â¡a vojska je poslana na", "VaÃ…Â¡a vojska ne moÃ…Â¾e biti poslana na", "PodrÃ…Â¡ka", "Napad", "PljaÃ„Âka", "Katapulti Ã„â€¡e ruÃ…Â¡iti", "sluÃ„Âajno", "u", "ili nakon", "sekundi", "minuta", "sahati", "dana", "Ã…Â pijuniraj resourse i trupe", "Ã…Â pijuniraj trupe i odbranu", "away", "Napad ne moÃ…Â¾e biti isplaniran jer destinacija nije odreÃ„â€˜ena.", "na stranici br.", "Sort by:", "type ", "time ", "target ", "options ", "village "];
	...
	case "fi":  // Ei vielä valmis, auttakaas !
		...
		aLangMenuOptions = ["Travian Task Queue: ", "Käytä palvelimen aikaa", "Käytä paikallista aikaa", "Aseta heimo", "Tehtävähistoria", "Check Updates", "\nHow many past tasks do we keep in history?\n(Type 0 to disable task history.) \nCurrently: ", "\nMinkä heimon valitsit tälle serverille?\n(Vastaa 0 Roomalaiset, 1 Teutonit, 2 Gallialaiset.) \nNykyinen: "];
		break;
	...
	case "ir":
		aLangBuildings = ["", "هيزم شکن", "آجرسازي", "معدن آهن", "گندم زار", "چوب بري", "آجرپزي", "ذوب آهن", "آسياب", "نانوايي", "انبار", "انبار غذا", "اسلحه سازي", "زره سازي", "ميدان تمرين", "ساختمان اصلي", "اردوگاه", "بازار", "سفارت", "سربازخانه", "اصطبل", "کارگاه", "دارالفنون", "مخفيگاه", "تالار شهر", "اقامتگاه", "قصر", "خزانه", "تجارت خانه", "سربازخانه بزرگ", "اصطبل بزرگ", "ديوار شهر", "ديوار گلي", "پرچين", "سنگبري", "آّبجو سازي", "تله ساز", "عمارت قهرمان", "انبار بزرگ", "انبار غذاي بزرگ", "عمارت جادو", "Treasure Chamber", "Horse Drinking Pool"];
		aLangTasks = ["ساختن", "ارتقا", "حمله", "تحقيق", "آموزش"];
		aLangStrings = ["بعداً بساز", "بعداً ارتقا بده", "بعداً حمله کن", "بعداً تحقيق کن", "اين وظيفه را زمان بندي کن", "ساختن را شروع کرديم", " يک مشکل نا معلوم", "سطح", " ساخته نميشود", " ارتقا داده نمي شود", "زمان بندي شد", "توليد فعلي:", "نممي توانم زمان بندي کنم", "زمان بندي امکان پذير نيست", "وظايف زمان بندي شده", "پاک کن", "بعداً بفرست", "سپاهي انتخاب نشده", "سپاه شما فرستاده شدند به", "Your troops could not be sent to", "پشتيباني", "حمله", "غارت", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "away", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ", "Task History", "flush history", "We started researching ", " cannot be researched."];
		aLangTroops[0] = ["سرباز لژيون", "محافظ", "شمشيرزن", "خبرچين", "شواليه", "شواليه سزار", "دژکوب", "منجنيق آتشين", "سناتور", "مهاجر", "قهرمان"]; //Romans
		aLangTroops[1] = ["گرزدار", "نيزه دار", "تبرزن", "جاسوس", "دلاور", "شواليه توتن", "دژکوب", "منجنيق", "رئيس", "مهاجر", "قهرمان"]; //Teutons
		aLangTroops[2] = ["سرباز پياده", "شمشيرزن", "رد ياب", "رعد", "کاهن سواره", "شواليه گول", "دژکوب", "منجنيق", "رئيس قبيله", "مهاجر", "قهرمان"]; //Gauls
		aLangTransModel= ["MixedModel", "FoodOnly", "ResourceOnly", "FixedModel"];
		aLangStringt   = ["RecruitHome", "AutoTransport", "AutoTrade", "Businessmen", "From", "To", "TransportBy", "RepeatTime", "minutes", "seconds", "TransportTimes:", "times", "The Time Of Next Transport:", "has been added.", "has been started.", "has not enough businessmen.", "transport error.", "Schedule later after", "Corn Resource: ", "Autotrade has done.", "Autotrade failed.", "Target Resource:", "Wood", "Clay", "Iron", "Crop", "Division Equal", "Division Model:", "WARN: Chang source village is not recommend.", "Keep ZERO if you want to division equal among wood, clay and iron. fill the number otherwise.", "This is the server time.", "This is your local time.","Force Trade On Scheduled Time (no delay check)"];
		aLangUpdate = ["Your version is greater then current version: ", "Your current version is up to date: ", "There is an update available", "Do you want to update now? "];
		aLangMenuOptions = ["Travian Task Queue: ", "Use server time", "Use local time", "Set your race", "Task History", "Check Updates", "\nHow many past tasks do we keep in history?\n(Type 0 to disable task history.) \nCurrently: ", "\nWhat is your race on this server?\n(Type 0 for Romans, 1 for Teutons, 2 for Gauls.) \nCurrently: "];
	...
	case "jp":  //by so-men & miyako
		...
		aLangMenuOptions = ["Travian Task Queue: ", "サーバ時間を使用する。：日本ならば、GMT標準時刻＋9時間を設定してください。", "PCの時計を使用する。", "種族の設定", "タスク履歴", "アップデートチェック", "\nタスク履歴はいくつまでリストに残しますか?\n(0に設定すると、タスク履歴は使用しません。) \n現在の設定値: ", "\nあなたの種族は?\n(ローマ：0, チュートン：1, ガウル：2) \n現在の設定値: "];
		break;
	...
	case "kr": //by Kimmo
		...
		aLangStrings = ["건설 예약", "업그레이드 예약", "공격 예약", "연구 예약", "작업을 나중으로 예약.", "건설을 시작: ", " 은(는) 알 수 없는 결과를 가져오는 시도입니다.", "레벨", "을(를) 건설할수 없음.", "은(는) 업그레이드 할수 없음.", "작업이 예약되었습니다.", "현재 생산량:", "이 작업을 지금 시작 할 수 없습니다.", "예약된 작업이 불가능합니다!", "예정된 작업", "삭제", "보내기 예약", "선택된 병사가 없습니다.", "병력을 보냄: ", "병력을 보낼수 없음: ", "지원", "공격", "약탈", "투석기가 겨냥중: ", "임 의", "시각", "혹은 다음에", "초", "분", "시", "일", "병력과 자원을 염탐", "병력과 방어를 염탐", "송 환", "이 공격은 목적지가 지정되지 않아 불가능 합니다.", "지역 번호.", "정렬:", "유형 ", "시간 ", "대상 ", "설정 ", "마을 ", "작업 대기열", "기록 지우기", "연구를 시작: ", "은(는) 연구되지 않았습니다."];
	...
	case "lv": //by sultāns
		aLangBuildings = ["", "Cirsma", "Māla Karjers", "Dzelzs Raktuve", "Labības Lauks", "Kokzāģētava", "Ķieģeļu Fabrika", "Dzelzs Lietuve", "Dzirnavas", "Maiznīca", "Noliktava", "Klēts", "Ieroču kaltuve", "Bruņu kaltuve", "Turnītru laukums", "Galvebā Ēka", "Pulcēšanās Vieta", "Tirgus Laukums", "Vēstniecība", "Kazarmas", "Stallis", "Darbnīca", "Akadēmija", "Paslēptuve", "Rātsnams", "Rezidence", "Pils", "Dārgumu glabātuve", "Tirdzniecības Birojs", "Lielās Kazarmas", "Lielais Stallis", "Pilsētas Mūris", "Zemes Mūris", "Palisāde", "Akmeņlauztuve", "Alus Darītava", "Mednieku māja", "Varoņu Savrupmāja", "Lielā Noliktava", "Lielā Klēts", "Pasaules Brīnums"];
	...
	case "pl":  //by Oskar
		...
		aLangTroops[2] = ["Falanga", "Miecznik", "Tropiciel", "Grom Teutatesa", "JeÅºdziec Druidzki", "Haeduan", "Taran", "trebeusz", "Herszt", "Osadnik", "Bohater"]; //Gauls
		aLangTransModel= ["MixedModel", "FoodOnly", "ResourceOnly", "FixedModel"];
		aLangStringt   = ["RecruitHome", "AutoTransport", "AutoTrade", "Businessmen", "From", "To", "TransportBy", "RepeatTime", "minutes", "seconds", "TransportTimes:", "times", "The Time Of Next Transport:", "has been added.", "has been started.", "has not enough businessmen.", "transport error.", "Schedule later after", "Corn Resource: ", "Autotrade has done.", "Autotrade failed.", "Target Resource:", "Wood", "Clay", "Iron", "Crop", "Division Equal", "Division Model:", "WARN: Chang source village is not recommend.", "Keep ZERO if you want to division equal among wood, clay and iron. fill the number otherwise.", "This is the server time.", "This is your local time.","Force Trade On Scheduled Time (no delay check)"];
		aLangUpdate = ["Your version is greater then current version: ", "Your current version is up to date: ", "There is an update available", "Do you want to update now? "];
		aLangMenuOptions = ["Travian Task Queue: ", "Use server time", "Use local time", "Set your race", "Task History", "Check Updates", "\nHow many past tasks do we keep in history?\n(Type 0 to disable task history.) \nCurrently: ", "\nWhat is your race on this server?\n(Type 0 for Romans, 1 for Teutons, 2 for Gauls.) \nCurrently: "];
	...
	case "us":  //by m4rtini
		aLangBuildings = ["", "Woodcutter", "Clay Pit", "Iron Mine", "Wheat Field", "Sawmill", "Brickworks", "Iron Foundry", "Flour Mill", "Bakery", "Warehouse", "Granary", "Blacksmith", "Armory", "Tournament Square", "Main Building", "Rally Point", "Marketplace", "Embassy", "Barracks", "Stable", "Siege Workshop", "Academy", "Cranny", "City Hall", "Residence", "Palace", "Treasury", "Trade Office", "Great Barracks", "Great Stable", "City Wall", "Earth Wall", "Palisade", "Stonemason", "Brewery", "Trapper", "Hero's Mansion", "Great Warehouse", "Great Granary", "Wonder of the World", "Horse Drinking Trough", "Treasure Chamber"]; 
		aLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train"];
	...
	default: // default is english
		aLangBuildings = ["", "Woodcutter", "Clay Pit", "Iron Mine", "Cropland", "Sawmill", "Brickworks", "Iron Foundry", "Grain Mill", "Bakery", "Warehouse", "Granary", "Blacksmith", "Armoury", "Tournament Square", "Main Building", "Rally point", "Marketplace", "Embassy", "Barracks", "Stable", "Workshop", "Academy", "Cranny", "Townhall", "Residence", "Palace", "Treasury", "Trade Office", "Great Barracks", "Great Stable", "City Wall", "Earth Wall", "Palisade", "Stonemason", "Brewery", "Trapper", "Hero's Mansion", "Great Warehouse", "Great Granary", "Wonder", "Treasure Chamber", "Horse Drinking Pool"];
		aLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train"];
		aLangStrings = ["Build later", "Upgrade later", "Attack later", "Research later", "Schedule this Task for Later", "We started builing ", " was attempted with unknown result.", "level", " cannot be built.", " cannot be upgraded.", "The task was scheduled.", "Current production:", "We can't schedule this task right now.", "Task scheduling is not available!", "Scheduled Tasks", "Delete", "Send later", "No troops were selected.", "Your troops were sent to", "Your troops could not be sent to", "Support", "Attack", "Raid", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "away", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ", "Task History", "flush history", "We started researching ", " cannot be researched.", "Enhance later", "Spy", "train later", "troops.", "Train", "We started training ", " cannot be trained."];
		aLangTroops[0] = ["Legionnaire", "Praetorian", "Imperian", "Equites Legati", "Equites Imperatoris", "Equites Caesaris", "Battering Ram", "Fire Catapult", "Senator", "Settler", "Hero"];  //Romans
		aLangTroops[1] = ["Clubswinger", "Spearman", "Axeman", "Scout", "Paladin", "Teutonic Knight", "Ram", "Catapult", "Chief", "Settler", "Hero"];  //Teutons
...
function initialize() {
    ...
    if (server) {
        ...
        _log(1, "Task Queue: using settings for server '" + server + "'");
        return true;
    } else {
         _log(0, "Task Queue: ERROR, unknown Travian server!");
        return false;
    }    
}
...
function checkSetTasks() {
		_log(2, "Task Queue: Checking set tasks...");
		_log(3, "Task Queue: oIntervalReference " + oIntervalReference);
	
	if(bLocked) {
			_log(3, "Task Queue: The TTQ_TASKS variables is locked. We are not able to write it.");
		return false;
	}
	...
	if(bUseServerTime) {
		...
		if(iServerTimestamp == false) {  //error
				_log(2, "Task Queue: Unable to determine server's time. We can't trigger any tasks without this. Consider switching to using local time.");
			return false;
		}
	...
	for(var i = 0; i < aTasks.length; i++) {
		...
		if(aThisTask[1] <= oDate.getTime()/1000) {
				_log(2, "Task Queue: Triggering task: " + aTasks[i]);
			...
			bTaskDeleted = true;
				...
				_log(2, "Task Queue: Some building/upgrading task is set, but it is not the time yet. It is time to preload the code though.");
				getCode(aThisTask[2], aThisTask[4]);
			
		} else {
				_log(2, "Task Queue: Some task is set, but it is not the time yet.");
			...
			if(sLastRefreshed != '' && (iSessionRefreshRate > 0) && (sLastRefreshed + (iSessionRefreshRate * 60000) + iRandomMilliSeconds )  < oDate.getTime() ) {
					_log(2, "Task Queue: Refreshing the session...");
	...
	if(bTaskDeleted) {
		refreshTaskList(aTasks);
			_log(3, "Task Queue: Rewriting task list");
		data = aTasks.join("|"); 
			_log(3, "Task Queue: New task list: " + data);
...
function deleteTask(e) {
	...
	var iTaskIndex = e.target.parentNode.getAttribute("itaskindex");
		_log(2, "Task Queue: Deleting task "+iTaskIndex);	
...
function setTask(iTask, iWhen, target, options) {
	...
	if(iVillageId) {
		...
		_log(2, "Task Queue: Writing task list: "+newValue);
	...
	if(!oIntervalReference) {
		...
		_log(2, "Task Queue: Started checking for the set tasks...");
	}
		_log(3, "End setTask()");
}
...
function triggerTask(aTask) {
	...
	switch(aTask[0]) {
		...
		default:
			//do nothing
				_log(3, "Task Queue: Can't trigger an unknown task.");
				break;
	}
...
function build(aTask) {
	...
	if(sVariable != '') {
			_log(3, "Task Queue: Building code found (TTQ_CODE_0)");
		...
		if(iIndexOfVillageId > -1) {  //the village id found
			...
			_log(3, "Task Queue: No building code available (TTQ_CODE_0)");
	}
	...
	if(sCode == '') {  // no code - no building possible
		_log(1, "Task Queue: No code found. Building this building is not possible.");
...
function upgrade(aTask) {
	...
	if(sVariable != '') {
			_log(3, "Task Queue: Upgrading code found (TTQ_CODE_1)");
		var aVariable = sVariable.split(",");
		var iIndexOfVillageId = aVariable.indexOf(aTask[4]);
		if(iIndexOfVillageId > -1) {  //the village id found
			sCode = aVariable[iIndexOfVillageId + 1];
		...
		_log(3, "Task Queue: No upgrading code found (TTQ_CODE_1)");
	}	
	
	if(sCode == '') {  // no code - no building possible
		_log(1, "Task Queue: No code found. Upgrading this building is not possible.");
...
function attack(aTask) {
	...
	if(aTroops[iScoutUnit] > 0 && onlySpies(aTroops) && iAttackType > 2) {  
		...
		_log(3, "Task Queue: sParams\n"+sParams);	
		
	if(aTask[4] != 'null') {  //multiple villages
		//we need to switch village
			_log(2, "Task Queue: Switching to village:" +aTask[4]);
		var currentActiveVillage = getActiveVillage();
		var myOptions = [aTask, currentActiveVillage];
		...
		httpRequest.onreadystatechange = function() {
			if (httpRequest.readyState == 4) {
				if (httpRequest.status == 200) { // ok
						_log(2, "Task Queue: Village switched to " +aTask[4]);
					post("a2b.php", sParams, handleRequestAttack, myOptions);
						_log(2, "Task Queue: The attack was requested.");
				}
			}
		};
...
function train(aTask) {
	...
	if(aTroops.length > 1) {
		...
		for(var i = 1; i < 11; i++) {
			...
			_log(0, "Task Queue: No troops specified. Exiting function.");
		return;
	}
		_log(0, sParams);
	...
	if(aTask[4] != 'null') {  //multiple villages
		//we need to switch village
			_log(0, "Task Queue 1530: Switching to village:" +aTask[4]);
		//var currentActiveVillage = getActiveVillage();
		var httpRequest = new XMLHttpRequest();
		...
		httpRequest.onreadystatechange = function() {
			if (httpRequest.readyState == 4) { //complete
				if (httpRequest.status == 200) { // ok
						_log(0, "Task Queue: Village switched to " +aTask[4]);
					post("build.php", sParams, handleRequestTrain, myOptions);
						_log(0, "Task Queue: The training was requested.");
				}
			}
		};
		httpRequest.send(null);		
		...
		post("build.php", sParams, handleRequestTrain, myOptions);
			_log(0, "Task Queue: Training was requested.\n" + sParams);
	}
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) { // ok				
			...
			if(sResponse.match(re)) {
				...
				_log(2, "Task Queue: HTTP request status: " + httpRequest.status);
		}
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) { // ok				
			var sResponse = httpRequest.responseText;
				_log(3, sResponse);
			if(!sResponse) {  // error retrieving the response				
					_log(2, "Task Queue: We didn't get any response. Impossible to determine whether the attack was sent.");
				return;
			}			
			...
			if(re.test(sResponse)) {
					_log(1, "Task Queue: It seems your attack was successfully sent.");
				printMsg(aLangStrings[18] + " " + sPlaceName);
				addToHistory(aTask, true);
			} else {
					_log(1, "Task Queue: Your attack could not be sent.");
...
function handleRequestResearch(httpRequest, options) {
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) {
			...
			xpath("//form/table[2]//td[1]/img[contains(@class,'unit')]");
...
function handleRequestTrain(httpRequest, options) {
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) {
			var sResponse = httpRequest.responseText;
				_log(0, "Task Queue 1678: " + sResponse);
			if(!sResponse) { // error retrieving the response				
			...
			if(sResponse.match(re)) {
				...
				_log(0, "Task Queue: HTTP request status: " + httpRequest.status);
		}
		if(isInt(activateVillageDid)) switchActiveVillage(activateVillageDid);
	}
		_log(0, "End 1699 handleRequestTrain("+httpRequest+", "+options+")");
}
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) { // ok				
			var sResponse = httpRequest.responseText;
				_log(3, "Task Queue: " + sResponse);
			if(!sResponse) {
					_log(2, "Task Queue: We didn't get any response. Impossible to determine the code.");
				return false;
			}
...
function switchActiveVillage(did) {
		_log(2, "Task Queue: Switching your village back to " +did);
	if(!isInt(did)) {return;	}
...
function addToHistory(aTask, bSuccess) {
	...
	newValue += ',' + bSuccess;
		_log(2, "Task Queue: Writing var TTQ_HISTORY: "+newValue);
	if(!setVariable("TTQ_HISTORY", newValue)) {
			_log(2, "Task Queue: Failed logging to history.")
	}
...
function createBuildLinks() {
	...
	if(xpathRes.snapshotLength > 0) {  //standing building
		...
		if(xpathBuildingNames.snapshotLength > 0) {   //empty building site or error
			_log(3, "Task Queue: This is an empty building site.");
			...
			_log(3, "Task Queue: Regular expressions (new site):\n" + re + "\n" + re2);
		} else {
			_log(3, "Task Queue: This is an existing building.");
			iTask = 1;
			...
			_log(3, "Task Queue: Regular expressions (Can be built):\n" + re + "\n" + re2);
	}
	for (var i = 0; i < xpathBuildingNames.snapshotLength; ++i) {
		//search for building id
			_log(3, "Task Queue: Searching for building ID...");
		...
		if(aMatches) {  //Regular building
			...
			var sBuildingId = aLangBuildings.indexOf(sBuildingName);
				_log(3, "Task Queue: Building or resource site name found: \"" + sBuildingName +"\" \n"+ sBuildingId);
		} else if(aMatches = sBuildingName.match(re2)) {  // Cranny has different format (e.g. "3. Cranny")
			sBuildingName = aMatches[1];
			var sBuildingId = aLangBuildings.indexOf(sBuildingName);
				_log(3, "Task Queue: Cranny name found: " + sBuildingName +" \n"+ sBuildingId);
		}
		if(sBuildingId > 0) {
			...
			_log(3, "Task Queue: Building name found, but it was not identified in the building list.\n"+sBuildingName+"\n"+re);
		}
	}
...
function createAttackLinks() {
	...
	if(xpathResult.snapshotLength < 1) {
			_log(3, "Task Queue: We are not creating the 'Send later' button here.");
		return false;
	}
...
function createResearchLinks() {
	...
	if(buildingName.snapshotLength < 1) {
			_log(2, "Task Queue: Building name not found.")
		return;
	}
	...
	switch(buildingName[1]) {
		...
		case aLangBuildings[13]:  //armory
			...
			break;
		default:
			_log(2, "Task Queue: No research links needed.");
			return;			
	}
	
	//build links
		_log(2, "Task Queue: Adding research later links...");
	var Imgs = xpath("id('content')/form/table[1]/tbody/tr/td[1]/table/tbody/tr[1]/td[1] | //form/table/tbody/tr/td/table/tr/td/img | id('build')/table/tbody/tr/td/img");
	var Cells = xpath("//form/table[1]/tbody/tr/td[2]/div | //form/table[1]/tbody/tr/td[2]/a | id('build')/table/tbody/tr/td[@class='act']/a");
...
function createTrainLinks() {
	...
	switch(buildingName[1]) {
		...
		case aLangBuildings[26]: //palace
			...
			break;
		default:
			_log(2, "Task Queue: No train links needed.");
			return;			
	}
	if(bIsResidence) {
			_log(2, "Task Queue: Adding train later links for residence/palace...");
		...
		if(trainBtn.snapshotLength > 0) {  //we need to build only the button
			...
			if(iCode.snapshotLength < 1) {
					_log(3, "Task Queue: No code available. No train later link available.");
				return false;
			}
			...
			if(oParent.snapshotLength < 1) {
					_log(3, "Task Queue: Don't know where to attach the button. Exiting function...");
				return;				
			}
				_log(2, "Task Queue: Appending textbox and button...");
			oParent.snapshotItem(0).appendChild(oDiv);
			$("ttq_settler_submit_btn").addEventListener("click", scheduleTraining, false);
		}
		
	} else {	
			_log(2, "Task Queue: Adding train later links for barracks/stables/workshop...");
...
function scheduleAttack(e) {
	...
	if(iVillageId == null) {
			_log(2, "Task Queue: Target village ID not found.");
	...
	if(xpathRes.snapshotLength > 0) {
		for (var i = 0; i < xpathRes.snapshotLength; i++) {
			...
			_log(2, "Task Queue: The type of attack was not determined. Unable to schedule the attack.");
		return false;
	}
	...
	if(xpathRes.snapshotLength > 0) {		
		for (var i = 0; i < xpathRes.snapshotLength; i++) {
			...
			_log(2, "Task Queue: No info about troops found. Unable to schedule the attack.");
		return false;
	}
		_log(3, "Task Queue: Troops:\n" + aTroops);
	
	if(bNoTroops) {
			_log(2, "Task Queue: No troops were selected. Unable to schedule the attack.");
...
function scheduleTraining(e) {
	...
	if(Inputs.snapshotLength < 1 ) {
			_log(3, "Task Queue: No textboxes with troop numbers found.");
		return false;
	}
	...
	if(bNoTroops) {
			_log(2, "Task Queue: No troops were selected. Unable to schedule training.");
		printMsg(aLangStrings[17] , true);
			return false;
	}
	...
	if(iCode.snapshotLength > 0) { 
		aTroops[0] = iCode.snapshotItem(0).value;
			...
			_log(3, "Task Queue: No code available. Exiting.");
		return false;
	}
...
function displayTimerForm(iTask, target, options, timestamp) {
	...
	// For build and upgrade, we need to extract arguments from the event object	 
	 if(iTask != 2 && iTask != 4  && target == null) {  //if params are supplied, we do not extract them from the event object target (link)
		...
		if(iTask == undefined || target == undefined || options == undefined) {
				_log(2, "Task Queue: Missing arguments:\niTask="+iTask+"\ntarget="+target+"\noptions="+options);
			return false;
		}
	}
	var sTask = ''; 
	...
	/*  I remove the code of the 1.5.7 version and put the modified code of the 1.6.0 version with my path */
	/* I have add onchange="document.getElementById(\'form_ */

	/* I add the em section for stocked the info, we can hidde this after the test */
        oTimerFormadd= "<span style='visibility:hidden'><em id='form_at'>"+sTime+"</em><em id='form_after'></em><em id='form_timeunit'>60</em><em id='form_timertask'>" +iTask+ "</em><em id='form_timertarget'>" +target+ "</em><em id='form_timeroptions'>"+options+"</em>";	
	...
	if(iTask == 2 && iAttackType > 20 && bCatapultPresent) {
		...
		sCataTargets = '<select name="kata" size="" class="f8" onchange="document.getElementById(\'form_kata\').innerHTML=this.value"><option value="99">' +aLangStrings[24]+ '</option>' + sCataOptions + '</select>';
		sCataTargets += '<select name="kata2" size="" class="f8" onchange="document.getElementById(\'form_kata2\').innerHTML=this.value"><option value="99">' +aLangStrings[24]+ '</option>' + sCataOptions + '</select>';
		oTimerFormadd+="<em id='form_kata'>99</em><em id='form_kata2'>99</em>";
	}
	...
	if(iTask == 2 && iAttackType > 20 && bOnlySpies) {
		sSpyMode = '<input type="radio" name="spy" value="1" checked onclick="document.getElementById(\'form_spy\').innerHTML=1">' +aLangStrings[31]+ ' <input type="radio" name="spy" value="2" onclick="document.getElementById(\'form_spy\').innerHTML=2">' +aLangStrings[32];
		oTimerFormadd+="<em id='form_spy'>1</em>";
	}
	oTimerFormadd+="</span><br />";
	...
	+ ' <input name="at" onmousedown="function(e){alert(\'oleeee\');/*e.stopPropagation();*//*" type="text" id="at" value="' +sTime
	...
	*/
        ...
        /* The code of the 1.6.0 */
	oTimerForm.innerHTML = sLinkClose + '<input type="hidden" name="timerTask" value="' +iTask
	+ '" /><input type="hidden" name="timerTarget" value="' +target+ '" /><input type="hidden" name="timerOptions" value="' 
	+options+ '" /><p>' +sTask+ ' ' +sWhat+ '<br/>' + aLangStrings[25] 
	
	
	+ ' <input name="at" type="text" id="at" value="' +sTime 
	+ '" onchange="document.getElementById(\'form_at\').innerHTML=this.value" onfocus="document.getElementById(\'form_at\').innerHTML=\'' +sTime+ '\';document.getElementById(\'after\').value = \'\'; this.value=\'' +sTime+ '\';document.getElementById(\'form_after\').innerHTML=\'\'" title="' +sTimeType+ '" /> ' + aLangStrings[26] 
	+ ' <input name="after" type="text" id="after" onchange="document.getElementById(\'form_after\').innerHTML=this.value" onfocus="document.getElementById(\'at\').value = \'\';document.getElementById(\'form_at\').innerHTML=\'\'" /><select name="timeUnit" onchange="document.getElementById(\'form_timeunit\').innerHTML=this.value" ><option value="1">' + aLangStrings[27] 
	+ '</option><option value="60" selected="selected">' + aLangStrings[28] 
	+ '</option><option value="3600">' + aLangStrings[29] 
	+ '</option><option value="86400">' + aLangStrings[30] 
	+ '</option></select><br/><span style="font-size:75%; cursor:default;">' +sMoreInfo+ '</span></p>';
	...
	if(sSpyMode != '') {
        ...
        /* rajout personnel pour ff 3.5 */
	oTimerForm.innerHTML += oTimerFormadd;	
...
function handleTimerForm(oForm) {
	...
	/* For FireFox 3.5 : we read the info in the <em id="form_... */
	var at = document.getElementById("form_at").innerHTML; 
	var timertask=document.getElementById("form_timertask").innerHTML
	var timeroptions=document.getElementById("form_timeroptions").innerHTML
	var timertarget=document.getElementById("form_timertarget").innerHTML

	/* var at = oForm.at.value; the last version */
	if(at == '') {
		/*var after = oForm.after.value;
		var timeUnit = oForm.timeUnit.value; the last version */
		var after = document.getElementById("form_after").innerHTML;
		var timeUnit = document.getElementById("form_timeunit").innerHTML;
		...
		if(!aMatch) {
			_log(1, "Task Queue: You entered invalid format of date!");
			return;
		}
		...
		if(bUseServerTime) {  //server time
			...
			if(iServerTimeOffset == false) {  //problem. do nothing.
					_log(2, "Task Queue: We could not schedule this task, because we were unable to determine server's timezone.");
	...
	document.body.removeChild($('timerform_wrapper'));
		_log(2, "Task Queue: Task will be scheduled for " +iTaskTime);  // The stored time is the absolute Unix GMT time.	
	
	
	if(document.getElementById("form_kata")) { //store catapults targets
		timeroptions += "_" +document.getElementById("form_kata").innerHTML;
	} 
	
	if(document.getElementById("form_kata2")) { //store catapults targets
		timeroptions += "_" +document.getElementById("form_kata2").innerHTML;
	} 
	
	if(document.getElementById("form_spy")) {  //spying mission
		/*for(var i = 0; i < oForm.spy.length; i++) {
			if(oForm.spy[i].checked) {timeroptions += "_" + oForm.spy[i].value;}
		}*/
		timeroptions += "_" +document.getElementById("form_spy").innerHTML
	}
	setTask(timertask, iTaskTime, timertarget, timeroptions);
		_log(3, "Task Queue: End handleTimerForm()");
}
...
function onlySpies(aTroops) {
		_log(3,"Task Queue: Begin onlySpies()");
		
	var iScoutUnit = getOption("SCOUT_UNIT", false, "integer");
	if(iScoutUnit != 3 && iScoutUnit != 4) {  //3 or 4 are the only valid values
			_log(2, "Task Queue: Unknown iScoutUnit. Unable to determine if this is a spying mission.");
			return false;
	}
	
	if(aTroops[iScoutUnit] < 1) { //no spies
			_log(3, "Task Queue: No spies.");
		return false;  
	}
	for(var i=1; i <= 11; i++) { 
		if(i != iScoutUnit && parseInt(aTroops[i]) > 0) { //at least one other troop		
				_log(3, "Task Queue: Troops other than spies are present.");
			return false;
		}
	}	
		_log(3, "Task Queue: This is a spying mission.");
	return true;  
...
function printMsg(sMsg,bError) {
	...
	if(oOldMessage) {
			_log(3, "Task Queue: Removing the old message." +oOldMessage);
...
function findCode(sPage, iNewdid) {
	...
	if(aMatch0) {
		_log(3, "Task Queue: Code for building new stuff found.");
		sCode = aMatch0[1];
		iMode = 0;
	} else if(aMatch1) {
		_log(3, "Task Queue: Code for upgrading found.");
		sCode = aMatch1[1];	
		iMode = 1;
	} else {
		_log(3, "Task Queue: Code not found");
		return;
	}
    ...
    // deprecated!
    if(bLockedCode) {
			_log(3, "Task Queue: The TTQ_CODE_" + iMode + " variables is locked. We were not able to write it.");
		return false;
	}
	if(sCode != '') {
		...
		sVariable = aVariable.join(",");
			_log(3, "Task Queue: Writing TTQ_CODE_"+iMode+": " + sVariable);
		setVariable('TTQ_CODE_'+iMode, sVariable);
		bLockedCode = false;
	} else {
			_log(2, "Task Queue: We didn't find any code. Either there is not enough resources for this task, or another building is being built/upgraded.");
		return false;
	}
		_log(3, "End findCode()");
}
...
function handleGetPlaceName(httpRequest, iPlaceId) {
	...
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) { // ok		
				_log(3, "Task Queue: HTTP response retrieved.");
			...
			if(sResponse) {					
				...
				var iCoordY = coordZToY(iPlaceId);
					_log(3, "Task Queue: Coordinates for " +iPlaceId+ ": " +iCoordX+ "|" +iCoordY);
				var re = new RegExp("onmouseover=\"map\\('([^']*)','([^']*)','[^']*','[^']*','" +iCoordX+ "','" +iCoordY+ "'\\)\"", "i");
				var aMatch = sResponse.match(re);
				if(aMatch && aMatch[1]) { 
						_log(2, "Task Queue: The village name found:"+aMatch[1]);
					cachePlaceName(iPlaceId, aMatch[1]);
					injectPlaceName(aMatch[1], iPlaceId);
				} else {
						_log(2, "Task Queue: The village name not found.");
		...
		for(var i = 0; i < xpathResult.snapshotLength; i++) {
			if (xpathResult.snapshotItem(i).textContent == aLangTroops[0][10]) {
				...
				if (inArray(aLangTroops[2], xpathResult.snapshotItem(i).textContent)){
					...
					_log(2, "Task Queue: Race not needed in this location. End getRace()");
					return false;
 				}
			}
		}
	}
	_log(2, "Task Queue: Information not found. Could not determine the race. End getRace()");
	return false;
} 
	...
	var iSiteId = iDHref.match(/&id=([0-9]*)/i);
		if(iSiteId[1] != null) {
			return parseInt(iSiteId[1]);
				...
				_log(2, "Task Queue: Building site ID not found");
			return false;
		}
	} else {
...
function getActiveVillage() {
	...
	if(oActiveLink.snapshotLength > 0) {
			_log(2, "Task Queue: Active village link found.");
		var sHref = oActiveLink.snapshotItem(0).href;
		var aMatch = sHref.match(/newdid=([0-9]*)/i);
		if(!aMatch) {
				_log(2, "Task Queue: Active village id could not be found.");
			return false;
		} else {
				_log(3, "Task Queue: Active village id was found: " +aMatch[1]);				
			return aMatch[1];
		}
	} else {
			_log(2, "Task Queue: Active village could not be found.");
		return false;
	}
...
function getServerTimeOffset() {
	...
	if(iServerTimeOffset != false) {  //no automatic detection
			_log(3, "Task Queue: Returning the predefined iServerTimeZoneOffset.");
		...
		if(iOffset.snapshotLength < 1) {  //not found. Unknown offset.
			...
			if(!aMatch) {
					_log(3, "Task Queue: No server time zone recognized, although it seems to be displayed.");
				return false;
			}
...
function getServerTime(bReturnTimestamp) {
	...
	if(sTime.snapshotLength < 1) {
			_log(3, "Task Queue: No server time found.");
		return false;
	}
	...
	if(!aMatch) {
			_log(3, "Task Queue: No server time found. Server date could not be determined.");
		return false;
	}
	...
	if(hh == UTCHoursServer) {  //the local UTC time is similar to server's UTC time. Good!
		...
		if (sTimes >= 1) {  //the server is ahead of us
			...
			_log(2, "Task Queue: Warning! The local time (as UTC) differs from the server time (as UTC) by more than 1 hour. Your local time is incorrect or you specified wrong timezone for your server. We can't calculate server's date.");
			return false;
		}
	}
 ...
 ***************************************************************************/
if (init) {
        _log(0, "Task Queue: TTQ started");
    window.addEventListener('load', onLoad, false);
} else {
        _log(0, "Task Queue: Initializacion failed");
