...
// @description   Modified version for La Vagie by Leon
...
var version = '1.0.6'; // also, check meta-data field @version
...
function ctime(t){
	...
	return dateFormat(d, "ddd, mm/dd/yyyy, HH:MM:ss Z")
}
...
function build( panel ){
   ...
   // build request for player names 
    ...
    unique(playerId).forEach(function(item){
		...
		player_profile['p'+item.toString()]=json;
    });
	...
	moves.forEach(function(item){
		...
		var json = player_profile['p'+item.masterHeroMove.playerId]; 
...
function init() { 
	...
	$('<button id="hommk-user-jactari-button-defender" style="width:120;margin-top:5px; display:block;">Jactari ! as Defender</button>')
		.button({disabled:true})
		.appendTo($panel_extended)
		.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
	...
	//HOMMK.player.content.worldId;
	//Search Player by Leon
	$('<input type="text" id="player_name" style="width:150px;height:20px;margin-top:5px; display:block;"></input>')
		.appendTo($panel_extended)
		.autocomplete({source:function(t,a)
			{
				if (t.term.length>2)
				{
					if (http_get)
					...
					http_get=$.ajax({
						type:"GET",
						url:"http://mightandmagicheroeskingdoms.ubi.com/ajaxRequest/playerNameAutocompletion",
						data:"worldId="+HOMMK.player.content.worldId.toString()+"&start="+$('<div/>').text(t.term).html(),
	...
	$('<button id="search_player_button" style="margin-top:5px; display:block;">Search Player</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" ).button('disable');
			//this.disabled=true;
			...
			parm.push({
				...
				"elementId":HOMMK.player.content.id,
				"rankingCategory":"BY_PLAYERS",
				"rankingType":"DOMINATION",
				"searchType":"SEARCH_BY_NAME",
				"searchParam":$("<div/>").text(pname).html(),
				"sortField":"position"});  
			do_request({"elParamList":parm},function(json){
					if (json)
					{
						var objs=json.d["RankingFrame"+HOMMK.player.content.id].rankingBY_PLAYERS_DOMINATIONList;
						objs.forEach(function(obj) {
							if (obj.playerName==pname)
							{
								var parm2=[];
								parm2.push({"elementType":"ProfileFrame","elementId":obj.id}); 
								do_request({"elParamList":parm2},function(r){toggle_player(r.d["ProfileFrame"+obj.id]);$("#search_player_button").button('enable');});
							}
						})
					}
					else
					{
						$("#search_player_button").button('enable');
					}
				});
            return false;
	    });
