// ==UserScript==
// @name        Fitocracy Bulk CSV
// @namespace   com.jsmackin.fitocracy.bulkcsv
// @description Allows downloading of performance csvs in bulk.
// @include     https://www.fitocracy.com/profile/*/?performance*
// @version     2
// @grant       none
// ==/UserScript==

$(function(){
	$("#performance-charts-container").prepend($("<a href='#' id='bulk_csv'>Bulk CSV</a>"));
	$("#bulk_csv").live('click',function(){
		var zip = new JSZip();
		var dir = zip.folder("bulkcsv");
		var deferreds = [];
		$("#history_activity_chooser option").each(function(){
			var action_id = $(this).val();
			var action_name = $("#option_"+action_id).html();
			deferreds.push(load_chart(action_id, action_name));
		});
		...
		$.when.apply(window, deferreds).then(function(){
			...
			$("#history_activity_chooser option").each(function(){
				var action_id = $(this).val();
				var action_name = $("#option_"+action_id).html();
				if (ref_history[action_id]['csv']) {
					var csv = Base64.encode(ref_history[action_id]['csv']);
					dir.file(action_name+".csv", csv, {base64: true});
				}
			});
			  try {
				location.href=window.URL.createObjectURL(zip.generate({type:"blob"}));;
			} catch(e) {
