...
ListPush:function(listName,pushItem,maxItems) {
	list = GM.GetList(listName,'')
...
SelectGeneral:function(whichGeneral) {
	...
	if (/under level 4/i.test(general)) {
		if (!GM.GetList('LevelUpGenerals','')) return this.ClearGeneral(whichGeneral);
...
SetControls:function(force) {
	...
	var XBattleInstructions="Start battleing if stamina is above this points";
	...
	if (!(generalList = GM.GetList("AllGenerals"))) generalList = [];
	generalList.unshift('Use Current');
	generalList.unshift('Under Level 4');
	generalList.push('Get General List');
	
	var crossList = function(checkItem) { return (generalList.indexOf(checkItem)>=0); }
	var generalIncomeList= ['Mercedes','Cid','Use Current','Get General List'].filter(crossList);
	var generalBankingList= ['Aeris','Use Current','Get General List'].filter(crossList);
	...
	autoZyngaRestart.addEventListener('click',function(e) {
		...
		document.getElementById("AutoZynga_div").style.opacity = 0.5;
	...
	controlDiv.addEventListener('mousedown',function(e) {
		document.getElementById("AutoZynga_div").style.opacity = 1;
		nHtml.clearTimeouts();
...
*/
		if (GM.GetValue('BattlesWonList','').indexOf("`"+userId+"`") == -1) {
			...
			newelement = now + '`' + userId + '`' + userName + '`' + wins + '`' + bpnum + '`' + goldnum;
			GM.ListPush('BattlesWonList',newelement,100);
		}	
	}

	if (nHtml.FindByAttrContains(document.body,"img","src",'battle_defeat.gif')) {
		nameLink=nHtml.FindByAttrContains(document.body,"a","href","keep.php?user=");
		userId = nameLink.href.match(/user=\d+/i);
		userId = String(userId).substr(5);
		userName = nHtml.GetText(nameLink).trim();
		...
		if (GM.GetValue('BattlesLostList','').indexOf("`"+userId+"`") == -1) {
			now = (new Date().getTime()).toString();
			newelement = now + '`' + userId + '`' + userName 
...
Quests:function() {
	...
	if(GM.GetValue('WhenQuest','') == 'Not Fortifying') {
		...
		//Check to see if we are even battleing a monster that needs defense.  if not health will be -1
...
BattleUserId:function(userid) {
	...
	if(battleForm && (userid > 0)) {
		...
		var button=nHtml.FindByAttrXPath(battleForm,"input","@type='image'");
		if(button) {
			GM.Log('Battle user button:'+button.name);
...
battleRaidRe:new RegExp('Level ([0-9]+) ([^0-9]+) ([0-9]+)','i'),
BattleRaid:function() {
try{
...
//		GM.Log("Army Ratio:" + armyRatio + " Level:" + level + " Army: " + army)
		...
		var thisScore = (levelMultiplier*(this.stats.army/army));
	...
	//}
	if(!this.lastBattleID || this.lastBattleID != safeTargets[0].id) val = 0;
	bestButton = safeTargets[val].button;
	if (bestButton != null) {
		GM.Log('Found Raid score: ' + safeTargets[val].score + ' id: ' + safeTargets[val].id +' Number: '+safeTargets[val].targetNumber);
		//GM.Log('   Out of these targets:' + safeTargets);
		GM.SetValue('LastAction','Idle');
		this.Click(bestButton);
		this.lastBattleID = safeTargets[0].id;
		return true;
	}

	GM.Log('No safe targets.');
	this.NavigateTo('raid');
	GM.SetValue('LastAction','Idle');
...
Battle:function(mode) {
	...
	if(GM.GetValue('TargetType','Freshmeat') == "Raid") return this.BattleRaid();
...
GetCurrentBattleTarget:function() {
	...
	if (GM.GetValue('TargetType','') == 'Raid') return 'raid';
		...
		if (webSlice && GM.GetValue('AchievementMode',false)) {
			...
			if (webSlice) {
				...
				if (webSlice) {
					...
					var achMonstList = GM.GetList('MonstersOverAchDam',' ');
					if (damDone>achLevel && achMonstList.indexOf(this.monster.name)<0) {
...
AcceptGiftOnFB:function() {
	...
	if (window.location.href.indexOf('facebook.com/reqs.php') >= 0) {
		...
		for(var s=0; s<ss.snapshotLength; s++) {
			...
			if (GM.GetValue('ReceivedList',' ').indexOf(giftEntry)<0) GM.ListPush('ReceivedList',giftEntry + '`' + giftType);
...
Idle:function() { 
	try{
		if(GM.GetValue('FillArmy',false)){
			if(!this.CheckForImage('giftpage_all_friends_on.gif')){
				this.NavigateTo('army,gift','giftpage_all_friends_off.gif');
			}else{
				var IdsList = nHtml.FindByAttrContains(document,"div","class",'condensed_multi_friend_selector').parentNode.attributes[0].textContent;
				var Ids = IdsList.split(",");
				var count = 0;
				var ID = GM.GetValue("ArmyCount",0);
				if(ID == 0)GM.Log("Ids.length = "+Ids.length);

				//GM.Log("Filling Army, Please wait..."+ID+"/"+Ids.length);
				this.SetDivContent('army_mess','Filling Army, Please wait...'+ID+"/"+Ids.length);
...
//	this.AddBattleLinks();
	...
	var actionsList = ['AutoIncome','AutoElite','Heal','ImmediateBanking','MaxEnergyQuest','DemiPoints','Monsters','Battle','Quests','Properties','Bank','PassiveGeneral','AutoBless','AutoStat','AutoGift','Idle'];
