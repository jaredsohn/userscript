...
// @description Allows you to adjust the width of the body tag.
// @include     *
// ==/UserScript==

// PROBLEMS:
//   - doesnt work on some pages
//   - you have to edit prefs.js to edit or delete entries
//what happens to urls with spaces

////////////////////////
//take from greasemonkey
...
function saveWidth() {
	...
	var pattern = prompt("Enter included URLs: (use * as a wildcard)", document.location.href);
	
	var patterns = GM_getValue("patterns")
	var widths = GM_getValue("widths")
	if(patterns) {
		patterns += " "+pattern;
		widths += " "+width;
	}
...
if (window == top) {
	GM_registerMenuCommand("Save Body Width...", saveWidth);
}

document.addEventListener("keydown", keydown, false);

// check every pattern against the url
var patterns = GM_getValue("patterns")
if(patterns) {
	patterns = patterns.split(" ");
	var widths = GM_getValue("widths").split(" ");
	for(var i=0; i<patterns.length; i++) {
		var re = convert2RegExp(patterns[i]);
		if(re.test(document.location.href)) {
			updateWidth(widths[i]);
		}
	}
}
