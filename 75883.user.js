...
function getTinyAndInsert_BitLy(event) {
  var thelongurl = url;
  urlinput.value = "making url Bitly..";
  GM_xmlhttpRequest({
		method: 'GET',
		url: "http://api.bit.ly/v3/shorten?login=twitthis&apiKey=R_f0b6b5e3a4c028b3ec97119e4f3ce16c&format=xml&longUrl=" + thelongurl,
		onload: function(responseDetails) {
				var tinydom = new DOMParser().parseFromString(responseDetails.responseText, "application/xml");
				urlinput.value = tinydom.getElementsByTagName('long_url')[0].textContent;
				countWord(event);
		}
	});
}
