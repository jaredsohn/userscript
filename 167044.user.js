...
// @namespace   http://userscripts.org/scripts/review/165498
// @icon        http://s3.amazonaws.com/uso_ss/icon/165498/large.png?1365299642
// @updateURL   https://userscripts.org/scripts/source/165498.meta.js
// @downloadURL https://userscripts.org/scripts/source/165498.user.js
// @homepageURL https://userscripts.org/scripts/show/165498
...
var updateTip = 'Checks for Scam Filter updates a maximum of 3 times per day. Minimal bandwidth usage.';
...
var thisScriptURL = location.protocol + '//userscripts.org/scripts/show/165498'; 

// Tell auto-updater this script's meta data URL, for checking the script's latest version number
var thisScriptMetaURL = location.protocol + '//userscripts.org/scripts/source/165498.meta.js'; 
...
var scammers = [
	505511,509293,509379,510850,513541,510672,480097,510116,500038,512571,442265,513430,507472,513409,505429,
	...
	500010,501553,441113,477783,477939,469787,480448,490444,507032,507280,506308,511070,414793,512674,510630,
	...
	495995,490808,476102,476758,514045,505250,514143,514332,507958
];
...
var useless = [
	513573,484405,513621,422292,478814,513746,494425,508578,498070,500894,514046,514258,512116
];
...
var suspicious = [
	...
	500679,506741,506998,506512,505247,505401,505047,513812,234423,513836
];
...
var caution = '<span style="line-height:100%;color:darkred;font-weight:bold;margin-bottom:-10px;display:block;">' + 
	'Warning: This script is a suspected scam. Use caution before installing it.' + 
...
$('tr[id^="scripts-"]:not(.checked)').each(function(i,val){
	...
	if (pranks.indexOf(parseInt(id)) > -1){
		tagScam('prank',id);
	} else {
		...
		getMeta(id).success(function(data){
			...
			if ((cacheVersion != usoVersion) || (cache == 0)){
				...
				function handOff2(data,id){
					...
					checkScript(data,true,id,usoVersion);
				}
...
/*function checkScript(data,isList,id,usoVersion){
	...
	if ( (data.search(/facebook/i) > -1) || (data.search(/ultoo/i) > -1) || (data.search(/ask\.fm/i))){
		...
		if (data.search(/&action\=subscribe/i) > -1){
			if (isList){var reason='&action=subscribe';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/\/ajax\/friends\/lists\/subscribe/i) > -1){
			if (isList){var reason='/ajax/friends/lists/subscribe';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/\/ajax\/follow\/follow_profile\.php/i) > -1){
			if (isList){var reason='/ajax/follow/follow_profile.php';tagScam(reason, id)} else{tagScamPage()}
		} else if ((data.search(/p,a,c,k,e,(d|r)/i) > -1) && (data.search(/eval/i) > -1)){
			if (isList){var reason='packer';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/facebook.com\/plugins\/(like|follow)\.php?href\=/i) > -1){
			if (isList){var reason='facebook.com/plugins/like.php?href=';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/(&|\?)action\=add_friend/i) > -1) {
			if (isList){var reason='paramsAdd';tagScam(reason, id)} else{tagScamPage()}
		} else if ( (data.search(/document\.getElementById\('MobileNos_'\)/i) > -1) && (data.search(/ultoo/i) > -1) ){
			if (isList){var reason='ultooNumber';tagScam(reason, id)} else{tagScamPage()}
		} else if ( (data.search(/document\.getElementsByName\('PollUserName'\)\[0\]/i) > -1) && (data.search(/ultoo/i) > -1) ){
			if (isList){var reason='ultooName';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/\/ajax\/groups\/members\/add_post.php/i) > -1) {
			if (isList){var reason='facebook post';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/iframe.*facebook\.com/i) > -1){
			if (isList){var reason='facebook iframe';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/ask\.fm\/likes.*\/add/i) > -1){ 
			if (isList){var reason='ask.fm /add';tagScam(reason, id)} else{tagScamPage()}
		} else if (data.search(/(facebook|twitter)\.com.*target\=/i) > -1){
			if (isList){var reason='ask.fm spam';tagScam(reason, id)} else{tagScamPage()}
...
function tagScam(reason,id){
	...
	if (reason == '&action=subscribe'){
		...
		var reason = 'This script was published by a known scam or spam author.';
