// ==UserScript==
// @name       perawa Photo Theater Killer
// @namespace  http://perawan.us/
// @version    2.1
// @description  Kills the annoying (IMHO) new photo theater on perawan. Instead loads the old photo viewer, as long as perawan keeps it running. This has been tested in Chrome with the Tampermonkey extension. Feel free to test and modify to make it work as need be.
...
// @include    http://*perawan.us/*
// @include    https://*perawan.us/*
// @include    http://*perawan.us/*
// @include    https://*perawan.us/*
...
(function () {
        ...
        function checkForPhotoTheater() {
            ...
            if (fbPhotoTheater != null && window.getComputedStyle(fbPhotoTheater, null).display == "block")
            {
                ...
                {
                    window.location.replace(window.location.href.replace(/(\?theater$)|(&theater$)|((&)theater&)/, "$4").replace(/(perawan\.us\/).*#!\//i, "$1"));
                }
                else
