// ==UserScript==
// @name          Play YouTube on XBMC4XBOX
// @namespace     http://userscripts.org/users/278527/scripts
// @description   Adds a link to play videos from YouTube in XBMC4XBOX
// @date          2011-12-12
// @creator       Stefan B.
// @include       *youtube.*/*
// ==/UserScript==
// This script were suposed to have easier noticable button to send to XBMC
// My plan were also to not rely on the Youtube plugin,
// but when they removed the format parameter in Descember 2010 the plans changed.
// So sadly, this doesn't add much but it was already kind of finished.

// XBMC4XBOX-FIX INCLUDED
	...
	if (video_url != undefined) {
		...
		setTimeout(
				function() {
					GM_xmlhttpRequest( {
						method: "HEAD",
                        url : "http://" + xbmc_address + "/xbmcCmds/xbmcHttp?command=ExecBuiltIn(PlayMedia(plugin://video/youtube/?path=/root%26action=play_video%26videoid=" + clip + "))"
                    })
				}, 250);
	}
}
...
function pause_movie() {
	setTimeout(
			function() {
				GM_xmlhttpRequest( {
					method: "HEAD",
					url : "http://" + xbmc_address + "/xbmcCmds/xbmcHttp?command=Pause"
                })
			}, 250);
}
...
function stop_movie() {
	setTimeout(function() {
		GM_xmlhttpRequest( {
			method : 'HEAD',
			url : "http://" + xbmc_address + "/xbmcCmds/xbmcHttp?command=Stop"
			})
	}, 250);
}
...
if (clip != undefined) {
	...
	xbmc_play.addEventListener('click', function(){play_movie(clip)}, false);
