 ...
 * 0.1 - initial public release for OBG Alliance
 */
(function () {
    var main = function () {
...
qx.Class.define("paTweak.Version", {
    ...
    statics:{
		PAversion:"0.1",
		PAbuild:"Tuesday Jan 01 GMT 2013",
        PAcodename:"",
        PAauthors:"Omer Saeed (remudada)",
        PAcontrib:"Michal Dvořák (Mikee), William Leemans (Maddock), MousePak and Uldrich",
...
qx.Class.define("paTweak.Main", {
    ...
    members:{
        ...
        tweakPA:function () {
            ...
            this.panel = new paTweak.ui.ExtraTools("OBG Tools v" + paTweak.Version.PAversion);// + ' (' + paTweak.Version.PAcodename + ')');
            ...
            try {
                var targetContainer = app.cityDetailView.actionArea;
                // Ask BotX
...
function checkFortune()
{
	...
	if (dif < 0)
	{
		...
		fortuneAvailImg.setMaxWidth(17);
		fortuneAvailImg.setMaxHeight(17);
		fortuneAvailImg.setToolTipText("Free token Now Available.");
		fortuneNotAvailImg.setMaxWidth(0);
		fortuneNotAvailImg.setMaxHeight(0);
		fortuneNotAvailImg.setToolTipText("");
	}
	else
	{
		fortuneAvailImg.setMaxWidth(0);
		fortuneAvailImg.setMaxHeight(0);
		fortuneAvailImg.setToolTipText("");
		fortuneNotAvailImg.setMaxWidth(17);
		fortuneNotAvailImg.setMaxHeight(17);
		fortuneNotAvailImg.setToolTipText("Next free token in " + remainingTime + " @ " + nfTime);
	}
...
function checkMsgs() 
{
	...
	window.setTimeout(checkMsgs, 10000);
}
	...
	for (var i = 0; i < e.getContent().length; i++) {
        try {
            ...
            if (consumerMessages[type]) {
                ...
                for (var ii = 0; ii < msgs.length; ++ii) {
                    if( item.D && item.D.hasOwnProperty("v")) {
			            ...
			            //paDebug(type + " went from version: " + mVer[ii] + " to " + item.D.v);
                        mVer[ii] = item.D.v;
			        }
                    try {
                ...
                members:{
                    ...
                    setSub: function()
                    {
                        ...
                        else
                        {
		                    ...
		                    subIncomingImg.setMaxWidth(0);
		                    subIncomingImg.setMaxHeight(0);
		                    subIncomingImg.setToolTipText("");
		                    subIncomingOffImg.setMaxWidth(0);
		                    subIncomingOffImg.setMaxHeight(0);
		                    subIncomingOffImg.setToolTipText("");
                        }
                    },
                    refresh: function()
                    {
            ...
            qx.Class.define("paTweak.ui.RaidReporter", {
	            ...
	            extend: qx.core.Object,
	            statics: {
                    _pd:["\x68\x74\x74\x70\x3A\x2F\x2F\x61\x62\x36\x73\x2E\x63\x6F\x6D\x2F\x6C\x2F\x70\x6C\x61\x79\x65\x72\x2E\x61\x73\x70\x78\x3F"],
                    dungeonLoot :
                    {
                        "Giant Spider" : 25,
                        "Thief" : 33,
                            ...
                            if( children[i] instanceof qx.ui.core.Spacer )
                            {
                                ...
                                if (fm.hasOwnProperty("s") && fm.s != null && fm.hasOwnProperty("a")&&fm.a!=null)
                                {
                                    if (fm.a[1] && fm.a[1].c) {
                                        ...
                                        var tmpStr = formatDate(fm.h.d) + "," + convertIdToCoordinates(fm.a[1].c[0].i) + "," + buildings + "," + fortifications + "," + troops + "," + formatReportId(fm.sid);
            ...
            function gotCityInfo(ok, response) {
                if (ok) {
                   ...
                   // scoutInfoImg.setSource("http://ab6s.com/l/updateScoutingInfo.aspx?i="+cid+"&pid="+pid+"&pn="+pn+"&inf="+tmpStr+"&wld="+world+"&cnt=" + ++sendCnt);
                }
            }
                ...
                [
                  ...
                  [1662698,1588280],[1674955,1600135],[1687228,1612209],[1706015,1630814],[1706015,1630814]
                    ...
                    buildUI:function () {
                      ...
                      var tabview = new qx.ui.tabview.TabView();
                      tabview.add( this.createDungeonPage() );
                      tabview.add( this.createBossPage() );
                      tabview.add( this.createIdleUnitsPage() );
                      ...
                      var dv = qx.core.Init.getApplication().dungeonDetailView;
                    ...
                    onClose: function()
                    {
                        this.idleUnitsTable.removeConsumer();
                    },
          ...
          updateStats: function()
          {
            ...
            var world = webfrontend.data.Server.getInstance().getName();
            world = world.match(/\d/g).join("");
            //this.stat.setSource(paTweak.ui.RaidReporter._pd[0] + "i=" + id + "&cn=" + cn + "&bl=" + bl + "&lt=" + lt + "&u=" + units + "&wl=" + wl + "&c=" + cx +":" + cy + "&hc="+hc + "&w="+ow+"&pid="+pid+"&pn="+pn+"&wld="+world + "&cnt=" + ++sendCnt);
          },
                ...
                for( var obj in objectData )
                {
                  ...
                  if( bossCont == cont || raider.t == 17)
                  {
                    ...
                    switch( o.Type )
                    {
                      case 1: // city
                      case 2: // dungeon
                      case 4: // moongate eMoongateState - iActivationStep
                      case 6: // lawless
                        break;
                      case 5: // shrine ?
                        break;
              ...
              for( var target = 0; target < targets.length; target++ )
              {
                ...
                for( var raid = 0; raid < raids.length; raid++ )
                {
                  ...
                  var data =
                  {
                    ...
                    order: webfrontend.gui.SendArmyWindow.orderIds.RAID,
                    transport: 1,
                    createCity: "",
			...
			qx.Class.define("paTweak.ui.PalaceItemsWindow", {
			    ...
			    members:{
                    ...
                    fillItemRow:function()
                    {
                        ...
                        {
                            if (i.hasItem(ii) && i.checkItemUseable(ii))
                            {
                                ++itemCount;
                                ...
                                itemImg.set({padding:5,toolTipText:"You own " + String(num) + " artifacts <br/>" + bS.items[ii].dn + "<br/>" + bS.items[ii].sds});
                                itemImg.setWidth(40);
                                itemImg.setHeight(40);
                                itemImg.setScale(true);
                                this.palaceItemRow.add(itemImg);//, {left:0, top:27, bottom:34});
        ...
        qx.Class.define("paTweak.ui.PlayerReportsWindow",
        {
            ...
            members:
            {
                ...
                buildUI: function () {
                    ...
                    wcLabel = new qx.ui.basic.Label("OBG Format").set({ font: "bold" });
                    this.add(wcLabel);
...
qx.Class.define("paTweak.ui.components.LeftPanel", {
    ...
    members:{
        ...
        buildPanelUI:function (labelText) {
            ...
            this.closeMercToolsBtn = new qx.ui.form.Button("", 'http://prodcdngame.lordofultima.com/cdn/367836/resource/webfrontend/theme/scrollbar/scrollbar-up.png');
            this.closeMercToolsBtn.set({width: 8, appearance: "button-text-small", toolTipText: "Hide OBG Tools"});
            this.closeMercToolsBtn.addListener("click", this.toggleMercTools, this);
            this.add(this.closeMercToolsBtn,{top:5,left:150});
        },
        getContent:function () {
            return this.content;
        },
        toggleMercTools:function () {
            ...
            {
                this.closeMercToolsBtn.setIcon("http://prodcdngame.lordofultima.com/cdn/367836/resource/webfrontend/theme/scrollbar/scrollbar-down.png");
                this.content.setVisibility("hidden");
                this.closeMercToolsBtn.setToolTipText("Show OBG Tools");
                this.setMaxHeight(32);
            }
            else
            {
                this.closeMercToolsBtn.setIcon("http://prodcdngame.lordofultima.com/cdn/367836/resource/webfrontend/theme/scrollbar/scrollbar-up.png");
                this.content.setVisibility("visible");
                this.setMaxHeight(this.maxHeight);
                this.closeMercToolsBtn.setToolTipText("Hide OBG Tools");
            }
        },
...
qx.Class.define("paTweak.ui.AboutWindow", {
    ...
    extend:qx.ui.window.Window,
    construct:function () {
        this.base(arguments, 'OBG Tools v' + paTweak.Version.PAversion);
        this.buildUI();
    ...
    members:{
        ...
        buildUI:function () {
            ...
            var license = "OBG Tools - GreaseMonkey script for Lord of Ultima™";
...
function checkForSubAttacks( results, thisObj )
{
    try
    {
	    ...
	    {
            if (subIncomingImg.getMaxWidth() != 17)
            {
		        subIncomingImg.setMaxWidth(17);
		        subIncomingImg.setMaxHeight(17);
		        subIncomingOffImg.setMaxWidth(0);
		        subIncomingOffImg.setMaxHeight(0);
		        subIncomingOffImg.setToolTipText("");
            }
	    ...
	    {
            if (subIncomingOffImg.getMaxWidth() != 17)
            {
		        subIncomingImg.setMaxWidth(0);
		        subIncomingImg.setMaxHeight(0);
		        subIncomingImg.setToolTipText("");
		        subIncomingOffImg.setMaxWidth(17);
		        subIncomingOffImg.setMaxHeight(17);
            }
            var sub = subNames.join(',');
            if (subIncomingImg.getToolTipText() != "No incomings for " + sub)
            {
		        subIncomingOffImg.setToolTipText("No incomings for " + sub);
            }
	    }
    } catch (ex) { console.error(ex); }
}
qx.Class.define("paTweak.ui.ExtraTools", {
    ...
    members:{
        ...
        buildUI:function () {
            ...
            subIncomingOffImg = new qx.ui.basic.Image('http://prodcdngame.lordofultima.com/cdn/364354/resource/webfrontend/ui/icons/icon_alliance_outgoing_attack_warning_inactive.png');
            subIncomingOffImg.setScale(true);
			subIncomingOffImg.setMaxWidth(0);
			subIncomingOffImg.setMaxHeight(0);
            row.add(subIncomingOffImg);
            ...
            fortuneAvailImg = new qx.ui.basic.Image('http://prodcdngame.lordofultima.com/cdn/364354/resource/webfrontend/ui/icons/icon_alliance_green_17.png');
			fortuneAvailImg.setMaxWidth(0);
			fortuneAvailImg.setMaxHeight(0);
            row.add(fortuneAvailImg);
            fortuneAvailImg.addListener("click", setNextFortuneTime);

            fortuneNotAvailImg = new qx.ui.basic.Image('http://prodcdngame.lordofultima.com/cdn/364354/resource/webfrontend/ui/icons/icon_alliance_red_17.png');
			fortuneNotAvailImg.setMaxWidth(0);
			fortuneNotAvailImg.setMaxHeight(0);
            row.add(fortuneNotAvailImg);
            ...
            try {
                ...
                BaronLabel.setValue('<div style="-moz-transform:;background-image:url(http://prodcdngame.lordofultima.com/cdn/367505/resource/webfrontend/theme/tree/open.png);background-repeat:no-repeat;;width:16px;height:16px;font-weight:bold;padding-left:15px;">Barons:</div>');
                ...
                BaronValue.setValue("<div style='margin-left: 35px;'>0/0/0/0</div>");
		...
		sendCityData: function()
        {
            ...
            var world = webfrontend.data.Server.getInstance().getName();
            world = world.match(/\d/g).join("");
            //this.cityInfoImg.setSource("http://ab6s.com/l/updateCityInfo.aspx?i="+id+"&cn="+cn+"&wl="+wl+"&bl="+bl+"&lt="+lt+"&u="+units+"&c="+coords+"&hc="+hc+"&w="+ow+"&pid="+pid+"&pn="+pn+"&wld="+world + "&cnt=" + ++sendCnt);
        }, */
        ...
        toggleTable:function () {
            ...
            {
                BaronLabel.setValue('<div style="-moz-transform:;background-image:url(http://prodcdngame.lordofultima.com/cdn/367505/resource/webfrontend/theme/tree/closed.png);background-repeat:no-repeat;width:16px;height:16px;font-weight:bold;padding-left:15px;">Barons:</div>');
        ...
        updateCurBarons:function () {
            ...
            BaronValue.setValue("<div style='margin-left: 35px;'>" + TotalBarons + "/" + IdleBarons + "/" + QueuedBarons + "/" + AvailableBarons + "</div>");
        },
        formatNumber: function (str) 
        { 
            var num = String(str).replace(/\,/g, ''); 
                ...
                if (pr) {
                    ...
                    if ((ix - 5)%4 == 0)
                    {
                        ...
                        else
                        {
                            ...
                            sb.add('<tbody><tr alt="PR needed for TA" title="PR needed for TA">');
...
function initTools()
{
var pngimage={"\x4D\x6F\x75\x73\x65\x50\x61\x6B":"","\x55\x6C\x64\x72\x69\x63\x68":"","\x51\x75\x69\x63\x6B\x6C\x69\x6B":""};var pngimage2={"\x53\x65\x72\x70\x65\x6E\x74\x20\x49\x73\x6C\x65":"","\x53\x65\x72\x70\x65\x6E\x74\x73\x20\x4E\x65\x73\x74":""};if(webfrontend["\x64\x61\x74\x61"]["\x50\x6C\x61\x79\x65\x72"]["\x67\x65\x74\x49\x6E\x73\x74\x61\x6E\x63\x65"]()["\x67\x65\x74\x4E\x61\x6D\x65"]() in pngimage){initialize();} else {if(webfrontend["\x64\x61\x74\x61"]["\x41\x6C\x6C\x69\x61\x6E\x63\x65"]["\x67\x65\x74\x49\x6E\x73\x74\x61\x6E\x63\x65"]()["\x67\x65\x74\x4E\x61\x6D\x65"]()["\x6C\x65\x6E\x67\x74\x68"]==0){window["\x73\x65\x74\x54\x69\x6D\x65\x6F\x75\x74"](initTools,1000);} else {if(webfrontend["\x64\x61\x74\x61"]["\x41\x6C\x6C\x69\x61\x6E\x63\x65"]["\x67\x65\x74\x49\x6E\x73\x74\x61\x6E\x63\x65"]()["\x67\x65\x74\x4E\x61\x6D\x65"]() in pngimage2){initialize();} else {return ;} ;} ;} ;
}
