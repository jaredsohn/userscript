// ==UserScript==
// @name          Bokus credit card companion
// @namespace     http://henrik.nyh.se
// @description   Makes buying from Bokus.com with a credit card simpler: auto-selects the credit card payment option and auto-fills (after prompting for a password) credit card details (encrypted with Blowfish). In Swedish. Use the menu command (Tools > Greasemonkey > User Script Commands…) to store/change the credit card details to use. 
// @include       http*://*bokus.com/*
// ==/UserScript==


function init() {
    
    GM_registerMenuCommand("Bokus: Ändra lagrad kreditkortsinfo", function() {
        var password = prompt('Lösenord att använda:');
        if (!password) return;
        var blowfish = new Blowfish(password);
        blowfish_set(blowfish, 'checksum', 'checksum');
        promptAndStore(blowfish, 'cardtype', 'Typ av kort (Visa/Mastercard/Amex)');
        promptAndStore(blowfish, 'cardno', 'Kortnummer');
        promptAndStore(blowfish, 'expmon', 'Giltigt till månad (t.ex. 02 för 02/09)');
        promptAndStore(blowfish, 'expyear', 'Giltigt till år (t.ex. 09 för 02/09)');
        promptAndStore(blowfish, 'cvc', 'CVV-kod (t.ex. 123)');

    });

    if (location.href == 'https://www.bokus.com/cgi-bin/order_send_in.cgi') {

        $('creditcard').checked = true;

    } else if (location.href == 'https://www.bokus.com/cgi-bin/labyrinth.cgi') {

        if (!GM_getValue('checksum')) return;

        var incorrectPassword = false;
        while (true) {
            var password = prompt((incorrectPassword ? 'Felaktigt lösenord! Försök igen' : 'Lösenord') + ':');
            if (!password) break;
            var blowfish = new Blowfish(password);
        
            if (blowfish_get(blowfish, 'checksum')=='checksum') {
                populateForm(blowfish, 'cardtype');
                populateForm(blowfish, 'cardno');
                populateForm(blowfish, 'expmon');
                populateForm(blowfish, 'expyear');
                populateForm(blowfish, 'cvc');
                break;
            } else {
...
function blowfish_set(blowfish, key, value) {
    GM_setValue(key, blowfish.encrypt(value));    
}

function blowfish_get(blowfish, key) {
    var value = blowfish.decrypt(GM_getValue(key, ''));
    return value.replace(/\x00+/, '');  // Some Blowfish artefact?
}

function promptAndStore(blowfish, key, label) {
    var value = prompt(label+':', blowfish_get(blowfish, key));
    blowfish_set(blowfish, key, value);
}

function populateForm(blowfish, key) {
    var element = document.forms[0].elements.namedItem(key);
    var value = blowfish_get(blowfish, key);
    if (['cardtype', 'expmon', 'expyear'].indexOf(key) != -1) {  // Select boxes
        element.selectedIndex = [].slice.call(element.options).map(function(o) { return o.value }).indexOf(value);
    } else {  // Text fields
        element.value = value;
    }
