// ==UserScript==
// @name        MTurk Great HIT Export With Hearts
// @namespace   localhost
...
(function () {
    ...
    DEFAULT_TEMPLATE = '[b]Title:[/b] [url={link}][COLOR=#FA58F4]{title}[/COLOR][/url]\n';
    DEFAULT_TEMPLATE += '[b]Requester:[/b] [url=https://www.mturk.com/mturk/searchbar?selectedSearchType=hitgroups&requesterId={requesterId}][COLOR=#FA58F4]{requester}[/COLOR][/url]';
    DEFAULT_TEMPLATE += ' [{requesterId}] ([url='+TO_BASE+'{requesterId}][COLOR=#FA58F4]TO[/COLOR][/url])';
    ...
    if (linkElements != null)
            for (var i = 0; i < linkElements.length; i++) {
                if (linkElements[i] != null && linkElements[i].textContent == 'View a HIT in this group' || linkElements[i].textContent == 'Preview this HIT' || linkElements[i].textContent == '(Why?)') {
                    var url = EASYLINK + linkElements[i].href.replace('notqualified?hitId','preview?groupId');
                    hit_data.adfly = url;
                    hit_data.link = linkElements[i].href.replace('notqualified?hitId','preview?groupId');
                    break;
                }
            }
