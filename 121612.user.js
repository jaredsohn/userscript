// ==UserScript==
// @name           Facebook Add Ons Beta by  Faridzhuan Firdaus
// @namespace      Faridzhuan Firdaus
// @description    Add ons for Facebook, including a 'Like All' button, a select all option, for inviting friends, and an autopoker! 
 ...
 * jQuery FB-Auopoke
 *
 * Copyright 2011, Sascha Heldt
 * License: Creative Commons Attribution-NonCommercial-NoDerivs 3.0
 */

(function ($) {
    setInterval(function () {
        var poke_uids = $('body').html().match(/poke_([0-9]+)/g),
            i = 0;

        if (!poke_uids || $('.autopoke').length >= 1) {
            return;
        }
        ...
        for (i; i <= poke_uids.length - 1; i += 1) {
            ...
            // Orig
            $('#pagelet_pokes a').each(function () {
                $(this).next('a').after('<span class="autopoke">Poked!</span>').remove();
            });
            
            // http://userscripts.org/topics/88645
            $("div:contains('Pokes')").next().each(function () {
                $('a[ajaxify*=poke_inline]', this).each(function () {
                    $(this).replaceWith('<span class="autopoke">Poked!</span>');
                });
                $('a[ajaxify*=poke_hide]', this).each(function () {
                    $(this).remove();
                });
            });
        };
