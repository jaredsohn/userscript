...
// @description    Check for script updates really easy. DO NOT INSTALL!
// @exclude        *
// @version        1.04
// @history        1.04 A fix for Firefox 4.0+
// @history        1.03 The DOM storage wrapper class can now store objects
// @history        1.02 Fixed problems with script versions
// @history        1.01 Fixed problems with arrays (going through arrays would include non string values as well)
// @history        1.01 Version must match the following format: v?[0-9.]+[a-z]? (samples: v1.00, 1.03, 1.03a, v1.0.0c etc.)
...
ScriptUpdater = {
	...
	filterMeta: function(element, index, array) {
		return /\/\/ @/.test(element);
	},
	...
	parseMetaTags: function(metaTags) {
		...
		var lines = metaTags.split(/\n/).filter(ScriptUpdater.filterMeta);
		for(var i in lines) {
	...
	getLastCheck: function() {
		return ScriptUpdater.getValue("lastCheck", 60);
	},

	getInterval: function() {
		var interval = ScriptUpdater.getValue("interval", 60);
		return (typeof(interval) == "undefined" || !interval.toString().match(/^\d+$/)) ? 60 : parseInt(interval.toString());
	},
	...
	showNotice: function() {
		if(ScriptUpdater.scriptMetaTags.name && ScriptUpdater.scriptMetaTags.version) {
			...
			var html = new Array();
			html.push(["<h1><img id=\"ScriptUpdater", ScriptUpdater.scriptId, "Close\" src=\"", ScriptUpdater.icons.close, "\" title=\"Close\"/><img src=\"", ScriptUpdater.icons.uso, "\" align=\"absmiddle\" style=\"margin-top:-2px;\"/><a href=\"http://userscripts.org/scripts/show/", ScriptUpdater.id, "\" target=\"_blank\" title=\"About the Userscripts.org Script Updater v", ScriptUpdater.version, "\">Userscripts.org Updater</a></h1>"].join(""));
