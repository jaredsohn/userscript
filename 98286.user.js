...
// @description    ブラウザ三国志,一騎当千 地図ツール by　NicL （浮浪プログラマ(どらごら改造バージョン)を適応）
// ==/UserScript==
...
// Ver 2.38.dN1 2011/03/04　by NicL
//　　・ブラウザ一騎当千に適応
//**************
// <><><><><><><><><><><> attention(注意) <><><><><><><><><><><>
...
function addHeadHtml(parentElem) {
	...
	addElem.innerHTML = 
		...
		"<tr>"+
			...
			"<td style='color:white; background-color:black; padding:3px 4px; "+
				"text-align:center;'>"+
				"<a id='maptoolExGetMap5959' href='javascript:void(0);'>59x59取得</a>"+
			"</td>"+
			"<td style='color:white; background-color:black; padding:3px 4px; "+
				"text-align:center;'>"+
...
function genPopupText(data, x, y) {
	...
	//資源
	} else if (this.power != undefined && this.power != "" && this.npcFlg != "1") {
		this.popupText += this.material + "<br/>";
	//人口
	} else if (this.population != undefined && this.population != "") {
...
function getMap5959() {
	...
	if( confirm("59x59の範囲を一気に取得するためサーバに負荷をかけます。\n" +
							"何度も実行するとDOS攻撃と同じなので、実行には注意して下さい\n" +
							"※取得中はあまり他の動作を行わないようにしてください。") == false ) return;
	
	GET_MAP_LIST.push(new Array(CENTER_X     , CENTER_Y + 20));	// 北
	GET_MAP_LIST.push(new Array(CENTER_X - 20, CENTER_Y + 20));	// 北西
	GET_MAP_LIST.push(new Array(CENTER_X - 20, CENTER_Y     ));	// 　西
	GET_MAP_LIST.push(new Array(CENTER_X - 20, CENTER_Y - 20));	// 南西
	GET_MAP_LIST.push(new Array(CENTER_X     , CENTER_Y - 20));	// 南　
	GET_MAP_LIST.push(new Array(CENTER_X + 20, CENTER_Y - 20));	// 南東
	GET_MAP_LIST.push(new Array(CENTER_X + 20, CENTER_Y     ));	// 　東
	GET_MAP_LIST.push(new Array(CENTER_X + 20, CENTER_Y + 20));	// 北東
	GET_MAP_LIST.push(new Array(CENTER_X     , CENTER_Y     ));	// 真ん中
...
function GetMapSleep(){
	window.status = "マップ情報取得中!(" + (MAX_MAP_LIST - GET_MAP_LIST_COUNTER) + "/" + MAX_MAP_LIST +")";
...
function getMaps() {
	...
	cajaxRequest(getMapUrl(mapPosi[0], mapPosi[1]) + "&type=3", "GET", "", function(req) {
		GM_log(getMapUrl(mapPosi[0], mapPosi[1]));
