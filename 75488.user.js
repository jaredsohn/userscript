...
// @description    Buys one new all black equipment set with one attribute and no upgrades
	...
	function doPurchase(){
		...
		*/
		...
		for (var i=0;i<equipids[upgradecolor].length;i++) {
			var upgradeData = 'action=Upgrade&id=&purchase='+equipids[upgradecolor][i]+'&player_id=' + playerid +'&upgrade=' + upgradestat;
			$.ajax({
				 async: false,
				 type: 'POST',
				 url: "http://goallineblitz.com/game/upgrade_equipment.pl?player_id="+playerid+"&purchase="+equipids[upgradecolor][i],
			     data: encodeURI(upgradeData),
