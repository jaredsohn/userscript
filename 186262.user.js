// ==UserScript==
// @name        MyFreeFarm Dutch
// @namespace   http://userscripts.org/scripts/show/186262
...
try{
	...
	texte["msgMarketsaleContent"]="(.*) heeft op de marktplaats\\s*(\\d+)\\s*x\\s*(.*?) van jou gekocht voor (.*?)\\s*aD\\."; // The text where the information is stated. The information has to be replaced by "(.*?)".
	// For the following you have to take a message sent if you sell something via contract
	texte["msgContractsaleContent"]="(.*) heeft een contract geaccepteerd!<br><br>\\s*De volgende producten zijn verkocht:<br>([\\S\\s]*)\\s*<br>\\s*Het bedrag (.*?) aD is bijgeschreven op je rekening\\."; // The text where the general information is stated. The information has to be replaced by "(.*?)".
	...
	texte["msgFriend"]="(.+) wil je als vriend toevoegen"; // The subject. The person has to be replaced by "(.+)"
