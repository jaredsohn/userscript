...
// @name           Pur Black cam
// @version 4.3
// @namespace      pbk_goes_droopy
// @description    pbk-add free
// @include        http://pbkcam.com/*
// @include        http://*.pbkcam.com/*
...
// advanced search
	...
	if (document.getElementsByClassName('c-1 endless_page_template')[0]){
		...
		check=loc[3]+loc[4];
		gen="";
		if(check.indexOf("student") != -1){gen="male","female"}
		if(check.indexOf("lecturers") != -1){gen="female","male"}
		if(check.indexOf("Religion") != -1){gen="female","male"}
		if(check.indexOf("gossip") != -1){gen="female","male"}
		re=/XX/gi
...
// read non-broadcast flash player version, every 5 sec. if not yet read
	...
	if (version){createCookie("CBversion",version.getAttribute("data"),1,"pbkcam.com");clearInterval(t)}
	},5000)
	}}
	}
...
//update check
	function update(){
	metalink = "http://userscripts.org/scripts/source/653246.meta.js";
	scriptlink = "http://userscripts.org/scripts/source/653246.user.js";
	GM_xmlhttpRequest({
	method: 'GET',
	url: metalink,
	onload: function(response) {
	data = response.responseText;
	createCookie("updatecheck","1",1,"pbkcam.com");
