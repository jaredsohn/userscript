...
// @description    West: Артур, ПоросячийКот
// @description    Вест на весь экран, Иконки зданий города, Дополнительные пункты меню, TW Bag Info - Полная инфа по багажу, Лучшие_вещи - ver2
...
add_link_button = function (){
	...
	addFooterIcon('javascript:if(Character.home_town != null) AjaxWindow.show(\'building_tailor\',{town_id:Character.home_town.town_id},Character.home_town.town_id);','footer_building_tailor','Портняжка');
	addFooterIcon('javascript:if(Character.home_town != null) AjaxWindow.show(\'building_general\',{town_id:Character.home_town.town_id},Character.home_town.town_id);','footer_building_general','Магазинчег');
...
addGlobalStyle('#footer_menu_left{width: 810px !important; top:-30px !important; left: 130px !important;}');
addGlobalStyle('#footer_menu_right{width:260px !important; top: -30px !important; right: 130px !important;}');
...
// скорость =ОКРУГЛ(100/(100+%);4)

//ширина окон.
aWindow.bi2_w0=350;
aWindow.bi2_w1=1120; //920
...
addFooterIcon('javascript:bi2_show_panel();','footer_building_BEST_ITEMS','Переодевалка');
...
aWindow.bi2_title_h_mid=310;     //высота нормального окна настроек переодевалки
aWindow.bi2_title_h_max=438;     //высота развёрнугото окна настроек переодевалки 275

aWindow.bi2_window_h_min=25;
aWindow.bi2_window_h_max=639;    //высота окна данных переодевалки

aWindow.bi2_tlink=' style=\"color:white; display:block; width:20px; height:20px; float:left;\" ';
aWindow.bi2_vblock=' style=\"border:1px solid black; padding:10x; marging:1px;\" ';
...
//addGlobalStyle('#window_inventory_content {height:460px;}');
//rezhik('background-image:url(images/inventory/west_inventar_2.jpg');

//window_inventory_title
//addGlobalStyle('#window_inventory_title {margin-top:50px;}');
//rezhik('window_inventory_title');
...
winfo='';\
\
bi2_types=['right_arm', 'left_arm', 'head', 'body', 'foot', 'neck', 'animal', 'yield', 'belt', 'pants'];\
...
raboty[106] = {rus_name:'Сплав по бурному потоку', name:'wildwater', malus:425, navyki:{reflex:2,swim:2,tactic:1}, resultaty:{dengi:84, opyt:74, vezenie:30, boom:57, produkty:{}}};\
...
raboty[108] = {rus_name:'Отлов гремучих змей', name:'rattlesnake', malus:440, navyki:{reflex:2,hide:1,pitfall:1,animal:1}, resultaty:{dengi:72, opyt:46, vezenie:71, boom:73, produkty:{}}};\
...
raboty[121] = {rus_name:'Борьба с бандитизмом', name:'shatter_gang', malus:725, navyki:{endurance:1,hide:2,pitfall:1,tactic:1}, resultaty:{dengi:84, opyt:70, vezenie:89, boom:99, produkty:{}}};\
...
raboty[123] = {rus_name:'Освобождение рабов', name:'free_slaves', malus:750, navyki:{swim:1,shot:1,leadership:1,tactic:1,appearance:1}, resultaty:{dengi:84, opyt:93, vezenie:28, boom:92, produkty:{}}};\
...
aWindow.bi2_res2html = function (){
    ...
    while (count_rab < 255){
        ...
        ihtm += '<td rowspan=\"6\">';
        ihtm += '<table><tbody><tr><th colspan=\"2\">'+rabota.rus_name+'</th></tr>';
        if ((count_rab > 150)&&(count_rab <= 170)){
            ihtm += '<tr><td colspan=\"2\"><a href=\"javascript:bi2_show_shmot('+ count_rab +');\" >';
        ...
        for (jj in aWindow.rabnavyki[count_rab]){
            for (aa = aWindow.rabnavyki[count_rab][jj].mul; aa > 0; --aa){
                ...
                ihtm += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; margin-right: -2px; margin-top: -4px; background: transparent url(/images/skill/skills_'+aWindow.bi2_s2a[jj];
...
aWindow.bi2_sortir = function (tip, minto){
    ...
    for (irab in aWindow.bi2_htmlrab){
        ...
        case 'boom':
            ...
            break;
        case 'name':
            aWindow.bi2_sortrab[ind_arr].ves= (irab > 130) ? 'я ' : '';
    ...
    for (count_rab in aWindow.porabotaj){
        ...
        for (jj in aWindow.rabnavyki[count_rab]){
            for (aa = aWindow.rabnavyki[count_rab][jj].mul; aa > 0; --aa){
                ...
                vrvr += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; margin-right: -2px; margin-top: -4px; background: transparent url(/images/skill/skills_'+aWindow.bi2_s2a[jj];
        ...
        {\
		    ...
		    if (++ii == nn){\
	    		...
	    		for (inabor = nabory.length - 1; inabor >= 0; --inabor) \
		    		if (bi2_predmetov[nabory[inabor]] > 0) {\
...
bi2_odev_spam = function(){\
 ...
 wear_spam.setAttribute('style', 'position: absolute; top: 2px; left: 5px; padding: 3px; margin: 0px;font-size:75%;');\
 var wear_spam2 = document.createElement('div');\
 wear_spam2.setAttribute('id', 'bi2_wear_spam2');\
 wear_spam2.setAttribute('style', 'position: absolute; top: 0px; left: 215px; padding: 3px; margin: 0px;font-size:95%;');\
 ...
 store_set_link.setAttribute('style', 'color: red');\
	store_set_link.textContent = 'Сохранить наборчик.';\
	...
	store_set_value.setAttribute('style', '-moz-user-select: text;font-size:100%;');\
 ...
 store_rewrite_link.setAttribute('style', 'color: red');\
	store_rewrite_link.textContent = 'Переименовать набор';\
	...
	equip_select.setAttribute('style', 'width: 160px; max-width: 165px; margin: 0px 5px;font-size:100%;');\
 ...
 equip_link.setAttribute('style', 'color: red');\
	equip_link.textContent = 'НАЦЕПИТЬ';\
...
aWindow.bi2_worker2 = function(schet){
	...
	if (sslot){
		...
		aWindow.bi2_slots.belt =document.getElementById('bi2_belt').checked;
		aWindow.bi2_slots.pants =document.getElementById('bi2_pants').checked;
...
aWindow.bi2_slot_selector = function(v_slot){
	...
	document.getElementById('bi2_belt').checked = (v_slot=='belt');
	document.getElementById('bi2_pants').checked = (v_slot=='pants');
...
html0 += '</select></div></div>\
	...
	<div' +
		...
		<input name=\"bi2_slot_selecter\" type=\"radio\" style=\"margin:auto auto auto 23px;\" onchange=\"javascript:bi2_slot_selector(\'belt\');void(0);\"/>\
		<input id=\"bi2_belt\" type=\"checkbox\" checked=\"1\" style=\"margin:auto 23px auto 21px;\" />Пояс<br />\
		<input name=\"bi2_slot_selecter\" type=\"radio\" style=\"margin:auto auto auto 23px;\" onchange=\"javascript:bi2_slot_selector(\'pants\');void(0);\"/>\
		<input id=\"bi2_pants\" type=\"checkbox\" checked=\"1\" style=\"margin:auto 23px auto 21px;\" />Штаны<br />\
