...
(function() {
  var SITEINFO_URL = 'http://wedata.net/databases/Text%20Conversion%20Services/items.json';
  ...
  var buttonContainer = $x('//div[@class="status-btn"]')[0];
  var span = document.createElement('span');

  buttonContainer.insertBefore(span, submit);
  span.appendChild(submit);
  buttonContainer.insertBefore(select, span);

  var convertText = function(callback) {
    Cache.set('lastSelectedIndex', select.selectedIndex, 24 * 60 * 60 * 1000 * 30);
    var site = siteInfo.entries[select.selectedIndex].data;
    ...
    GM_xmlhttpRequest({
      method : "GET",
      url : site.url.replace(/%s/, (charsets[site.charset][1])(encodeURIComponent(status.value))),
      overrideMimeType: "text/html; charset=" + charsets[site.charset][0],
      onload : function(response) {
        var result;
        if(site.xpath) {
          var doc = createDocumentFromString(response.responseText);
          result = $x(site.xpath, doc)[0].textContent.replace(/\n+/, '');
        } else {
          result = response.responseText;
        }

        status.value = actions[site.action ? site.action : 'replace'](status.value, result);
        callback();
      }
    });
  };

  submit.parentNode.addEventListener('click', function(e) {
  ...
  SiteInfo.prototype.get = function(url, callback) { 
    ...
    if(cacheInfo = Cache.get('cacheInfo')) {
      ...
      GM_xmlhttpRequest({
        ...
        url : url,
        onload : function(res) {
          self.entries = eval('(' + res.responseText + ')');
          callback(self.entries);
...
// http://nurucom-archives.hp.infoseek.co.jp/digital/ecl.js.txt
  ...
  var charsets = {
    sjis: ['Shift_JIS', EscapeSJIS],
    euc: ['EUC-JP', EscapeEUCJP],
    jis: ['iso-2022-jp', 'JIS7'],
    utf8: ['utf-8', function(str) { return str; }]
  };
  ...
  siteInfo.get(SITEINFO_URL, function(entries) {
    ...
    try {
      select.selectedIndex = Cache.get('lastSelectedIndex');
    } catch(e) {
      Cache.set('lastSelectedIndex', null, 0);
    }
  });

  GM_registerMenuCommand('Twitter Text Converter - clear cache', function() {
