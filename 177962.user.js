...
// @grant			unsafeWindow
// @releasenotes	<p><strong>Bug-Fix!</strong></p><p>There was a potential problem where this script could have interfered with communications with the server, resulting in unpredictable results including inaccurate resource amounts in cities, missing tower alerts in other bots, etc. This update will eliminate that problem.</p>  
// ==/UserScript==
...
//	│   																									 │
//	│   August 2013 Barbarossa69 (https://userscripts.org/users/272942)										 │
//	└────────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
uW.btMapMonitorTR = function(e) {if (e.user.id != "0") {fetchPlayerInfo (e.user.id, eventLoadMonitor); }}
...
function btStartup (){
  ...
  if (Options.WinPos==null || Options.WinPos.x==null|| Options.WinPos.x=='' || isNaN(Options.WinPos.x)){
    var c = getClientCoords (document.getElementById('main_engagement_tabs'));
    Options.WinPos.x = c.x+4;
    Options.WinPos.y = c.y+c.height;
    saveOptions ();
  }
  
  if (Options.MonPos==null || Options.MonPos.x==null|| Options.MonPos.x=='' || isNaN(Options.MonPos.x)){
    var c = getClientCoords (document.getElementById('main_engagement_tabs'));
    Options.MonPos.x = c.x+4;
    Options.MonPos.y = c.y+c.height;
    saveOptions ();
  }

  if (Options.DefPos==null || Options.DefPos.x==null|| Options.DefPos.x=='' || isNaN(Options.DefPos.x)){
    var c = getClientCoords (document.getElementById('main_engagement_tabs'));
    Options.DefPos.x = c.x+4;
    Options.DefPos.y = c.y+c.height;
    saveOptions ();
  }

  if (Options.IncPos==null || Options.IncPos.x==null|| Options.IncPos.x=='' || isNaN(Options.IncPos.x)){
    var c = getClientCoords (document.getElementById('main_engagement_tabs'));
    Options.IncPos.x = c.x+4;
    Options.IncPos.y = c.y+c.height;
    saveOptions ();
  }
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
				...
				.btPopMain       {background: url("' + PanelBG + '") no-repeat scroll -10px -50px transparent; border:1px solid #000000; -moz-box-shadow:inset 0px 0px 10px #6a6a6a; -moz-border-radius-bottomright: 20px; -moz-border-radius-bottomleft: 20px; border-bottom-right-radius: 20px; border-bottom-left-radius: 20px;}\
				.btPopup         {border:5px ridge #666; opacity:0.9; -moz-border-radius:25px; border-radius:25px; -moz-box-shadow: 1px 1px 5px #000000;}\
				.btMonitor_btPopMain  {font-size:11px; }\
				.btIncoming_btPopMain  {font-size:11px; }\
				.btDefence_btPopMain  {font-size:11px; }\
  ...
  if (CITYDEFENCE)
	m += '<div align="center"><TABLE><TR><TD class=xtab><a id=btCityDefenceButton class="inlineButton btButton blue20"><span style="width:150px"><center>City Defence</center></span></a></td><TD class=xtab><a id=btIncomingButton class="inlineButton btButton blue20"><span style="width:150px"><center>Incoming Marches</center></span></a></td></tr></table></div>';
  else	
    m += '<div align="center"><TABLE><TR><TD class=xtab><a id=btIncomingButton class="inlineButton btButton blue20"><span style="width:150px"><center>Incoming Marches</center></span></a></td></tr></table></div>';
  ...
  m += '<TR><TD class=xtab>&nbsp;</td><td class=xtab><INPUT id=MoveFurnitureChk type=checkbox /></td><td class=xtab>Rearrange throne room items for better visibility</td></tr>';
...
// show monitor if was monitoring before refresh

  if (Options.MonitorStartState && (Options.LastMonitored != "")) {
	   MonitorTRClick();
    }
...
// monitor incoming attacks
  ...
  EverySecond ();
  SecondTimer = setInterval(EverySecond,1000);

// show incoming/city defence if was being shown before refresh

  
  if (Options.IncomingStartState) {
		ToggleIncomingMarches();
    }

  if (Options.DefenceStartState) {
	   ToggleCityDefence(Options.CurrentCity);
    }
...
//				delete seed.ss; -- don't do this!!!!!!!

//	    		unsafeWindow.seed = seed; // do it all!
...
function MapToggle () {
  ...
  if ((Options.ShowProvinceOnMap || Options.ShowLevelOnMap || Options.ShowMistedOnMap) && Options.OverrideMapDisplay) {
    MapTimer = setInterval(ShowMapInformation,1250);
  }	
  else if (MapTimer) clearInterval (MapTimer);
  uW.g_mapObject.getMoreSlots(); 
}
...
var safecall = ["10229400","2092477","2179932","6001304","8465111","2095918","195515","6046539"];
...
// KoC timestamps are 8 hours behind UTC...?
  return parseInt (datestr.getTime() / 1000) + (AddHours*3600) + uW.g_timeoff;
}

function getLastLogDuration (datestr,AddHours){
...
// weird bug?!!!?
    var dur = getDuration(truceExpireTimestamp,8);
	var d = '';
	if (dur != "") {d = ' ('+dur+')';}
...
function MonitorTRClick() {
  ...
  if (name.toUpperCase() == Seed.player.name.toUpperCase()) {
	  fetchPlayerInfo (uW.tvuid, eventLoadMonitor)
    return;
  }
...
// Retrieve TR of enemy using uid and loop around

  fetchPlayerInfo (uid, eventLoadMonitor);
}

function TRStats (userInfo,loop){
  	var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
		...
		onSuccess: function (transport) {
			var rslt = eval("(" + transport.responseText + ")");
				if(rslt.ok){
                    ...
                    if (loop) { PaintTRMonitor(userInfo); }					
			   } else setError('AJAX error');
		},
		onFailure: function () {
			setError('AJAX error (server not responding)');
		},
...
function PaintTRMonitor (userInfo){

    LastUser = "";
	LastTR = "";
    
    if (!Refreshing) {
		m = '<div id="btMonitor_content"><div id=btMonitorDiv><TABLE><TD class=xtab><br><B>&nbsp;&nbsp;&nbsp;Loading...</b></td></tr></table></div><div id=btButtonDiv></div></div>';

		popMon = new CPopup('btMonitor', Options.MonPos.x, Options.MonPos.y, 300, 550, true, function (){saveOptions();});
	...
	MonitorTimer = setInterval(function(){MonitorTRLoop(userInfo,popMon)},1000);
	
    popMon.onClose = function(){StopMonitoring();}
}
...
function MonitorTRLoop (userInfo,popm){

    MonitorLooper = MonitorLooper+1;
	if (MonitorLooper > 30) {
      clearInterval(MonitorTimer);	
	  Refreshing = true;
      fetchPlayerInfo (userInfo.userId, eventRefreshMonitor);
	  return
	}  

// check for 5 minute monitor timeout
	
	if (!trusted && !MonitoringPaused) {MonitorCountDown = MonitorCountDown - 1;
	...
	if (MonitoringPaused) {
		if (MonitorTimedOut)
			{ popm.getTopDiv().innerHTML = '<DIV align=center><B>&nbsp;&nbsp;&nbsp;Monitoring Timed Out</B></DIV>'; }
		else
			{ popm.getTopDiv().innerHTML = '<DIV align=center><B>&nbsp;&nbsp;&nbsp;Monitoring Paused</B></DIV>'; }
	}	
	else	
		{ popm.getTopDiv().innerHTML = '<DIV align=center><B>&nbsp;&nbsp;&nbsp;'+dots+'&nbsp;Monitoring&nbsp;'+dots+'</B></DIV>'; }
	...
	if (userInfo.online) o = ' <span style="color:#f00;">(ONLINE)</span>';
  	...
  	m += '<TABLE><tr><td class=xtabBR align="center" colspan="3"><B>' + userInfo.name + o +'</b></td></tr>';

	if (!userInfo.online)
	  m+= ' <tr><TD class=xtabBR align="center" colspan="3">(Last Login '+ getLastLogDuration(userInfo.lastLogin,8) +' ago)</td></tr>';
	if (userInfo.misted) 
  	  m += '<tr><TD class=xtabBR align="center" colspan="3"><B>*** MISTED (' + getDuration(userInfo.fogExpireTimestamp,8) + ') ***</b></td></tr>';
	...
	if (userInfo.allianceName) {
  	...
  	m += '<tr><TD class=xtab align="center" colspan="3">&nbsp;</td></tr>';
	for (k=1;k<HisStatEffects.length;k++) {
		var HisContent = "";
		var LineStyle = '';
		var EndStyle = '';
		...
		if (Options.MonitorColours) {	
			...
			if (AttackEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#800;">';
			if (DefenceEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#008;">';
			if (LifeEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#088;">';
			if (RangeEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#080;">';
			if (SpeedEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#000;">';
			if (AccuracyEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#f80;">';
			if (OtherCombatEffects.indexOf(k) > -1)
				LineStyle = '<span style="color:#808;">';
			if (GlobalEffects.indexOf(k) > -1) {
				LineStyle = LineStyle + '<strong>';
				EndStyle = '</strong>' + EndStyle;
			}
			if (DebuffEffects.indexOf(k) > -1) {
				LineStyle = LineStyle + '<i>';
				EndStyle = '</i>' + EndStyle;
			}
		}	
		if (HisStatEffects[k] != 0) HisContent = (Math.round(HisStatEffects[k]*100)/100) + '% ' + uW.cm.thronestats["effects"][k]["1"];
		if (HisContent != "") { m +='<TR><TD  width="25px" class=xtab></td><TD class=xtab>' + LineStyle + HisContent + EndStyle +'</span></td><TD  width="50px" class=xtab></td></tr>'; cText += LineStyle + HisContent + EndStyle + "||"; }
	}
    cText = cText.replace(/\|\|\s*$/, "");
    cText = ":::. |" +title + "|| "+ cText;
	m +='</table>';
	document.getElementById('btMonitorDiv').innerHTML = m;
...
// show buttons ...
	...
	document.getElementById('btPostToChat').addEventListener ('click', function(){sendChat(cText)}, false);
	document.getElementById('btOpenTR').addEventListener ('click', function(){showTR(userInfo)}, false);
...
// if changed while monitoring play a sound...
    ...
    if ((LastUser == userInfo.name) && (LastTR != cText) && Options.MonitorSound) {
...
function fetchPlayerStatus (rsltInfo, notify){
    ...
    new MyAjaxRequest(uW.g_ajaxpath + "ajax/getOnline.php" + uW.g_ajaxsuffix, {
      ...
      parameters: params,
      onSuccess: function (rslt) {
	    rsltInfo.userInfo[0].online = rslt.data[uid];
...
function fetchCourtInfo (rsltInfo, notify){
    ...
    new MyAjaxRequest(uW.g_ajaxpath + "ajax/viewCourt.php" + uW.g_ajaxsuffix, {
      ...
      onSuccess: function (rslt) {
	    u = unixTime();
		f = convertTime(new Date(rslt.playerInfo.fogExpireTimestamp.replace(" ","T")),8);
	    rsltInfo.userInfo[0].misted = (f >= u);
	    rsltInfo.userInfo[0].fogExpireTimestamp = rslt.playerInfo.fogExpireTimestamp;
	    rsltInfo.userInfo[0].warStatus = rslt.playerInfo.warStatus;
	    rsltInfo.userInfo[0].truceExpireTimestamp = rslt.playerInfo.truceExpireTimestamp;
	    rsltInfo.userInfo[0].lastLogin = rslt.playerInfo.lastLogin;
	    rsltInfo.userInfo[0].cityCount = rslt.playerInfo.cityCount;
        notify (rsltInfo);
      },
      onFailure: function (rslt) {
        setError ('AJAX error (server not responding)');
...
function eventLoadMonitor (rslt){
  if (!rslt.ok){
    return;
  }
  
  Options.LastMonitored = rslt.userInfo[0].name;
  Options.MonitorStartState = true;
  saveOptions ();
  ...
  TRStats(rslt.userInfo[0],true);
}

function eventRefreshMonitor (rslt){
  if (!rslt.ok){
    return;
  }
  if (MonitoringPaused)
	{PaintTRMonitor(rslt.userInfo[0]);}
  else	
	{TRStats(rslt.userInfo[0],true);}
}
...
function ToggleIncomingMarches (){
	...
	else
	{
		...
		popInc = new CPopup('btIncoming', Options.IncPos.x, Options.IncPos.y, 700, 200, true, function (){Options.IncomingStartState = false;saveOptions();});
...
function ToggleCityDefence (Curr){
	...
	else
	{
		...
		popDef = new CPopup('btDefence', Options.DefPos.x, Options.DefPos.y, 500, 100, true, function (){Options.DefenceStartState = false;Options.CurrentCity = -1;saveOptions();popDef = null});
...
function CheckForHTMLChange(div,newHTML,wait) {
	var oldHTML = document.getElementById(div).innerHTML;
