...
if (itemNb != -1)	// We know the total number of pages, use the combo box
{
	...
	device.setAttribute("style", "margin-left: 50px; margin-right: 45px; margin-top: 10px;");
...
else	// Use the text field
{
	...
	device.setAttribute("style", "margin-left: 50px; margin-top: 10px;");
	...
	if (bShowGoButton)
	{
		...
		device.setAttribute("style", "margin-top: 10px;");
		wrapperSpan.appendChild(device);
	}

	device = document.createElement("i");
	device.setAttribute("style", "margin-right: 45px; font-size: 0.8em;");
	device.innerHTML = " (p. " + Math.floor(1 + urlInfo.offset / urlInfo.pageSize) + ")";
	wrapperSpan.appendChild(device);
}
