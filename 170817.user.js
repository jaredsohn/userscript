// ==UserScript==
// @name           adf.ly && lienscash.com && adfoc.us && bc.vc bypassers
	...
	var loc = document.location.href, interval, secinterval, bypassers = {
		adfly: function() {
			...
			else if (document.head.innerHTML.match (tehregxp))
			{
				...
				bypassers._inject (function() {
					...
					if (_target.match (rxpb64))
					{
						...
						document.body.innerHTML += "<br />Multiple bad-sites bypasser is brought to you by Qisti. Always check <a href='http://userscripts.org/scripts/show/122331'>this link</a> for updates and to report issues!";
			...
			else if (loc.match (reg5))
			{
				...
				errorMsg    += "Thanks, Qisti (the script author)";
				alert (errorMsg);
			}
			else {
				clearInterval(interval);
