...
// ==UserScript==
// @name          StumbleUpon Smarter Interface
// @description   This will be a collection of fixes for SU interface quirks.
// @include       http://www.stumbleupon.com*
// @include       https://www.stumbleupon.com*
// @version       0.1
...
function UpdateChecker(scriptId, thisVersion) {
    ...
    this.checkForUpdate = function(overlay, callback) {
        var self = this;
        var infoPageUrl = 'http://userscripts.org/scripts/source/' + this.scriptId + '.meta.js';
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                var currentVersion = (/@version\s+(.*)/).exec(response.responseText)[1];
                var scriptName = (/@name\s+(.*)/).exec(response.responseText)[1];
...
function start() {
    ...
    if(cb = document.getElementById('cc')) {
        if(cb.nodeName.toLowerCase() == 'input') {
            if(GM_getValue('inbox_cc')) {
                cb.checked = true;
            }
        }
        ...
        cb.addEventListener('click', function() {
            if(cb.checked) {
                GM_setValue('inbox_cc', true);
            } else {
                GM_deleteValue('inbox_cc');
            }
        }, false);
    }
}
...
if(document.body && top && (window.location.href == top.location.href)) {
    var updateChecker = new UpdateChecker(75898, '0.1');
