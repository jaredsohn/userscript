// ==UserScript==
// @name			Troll faces facebook on facebook Chat
// @description			Adds an troll faces bar to Facebook chat
...
//
	...
	var version, HttpsOn, ResourcesURL, storage, trollfaces, headTag, styleTag, ArrowStyleUp, ArrowStyleDown, fEmotBarDom, fEmotsListDom, fArrow;
...
/* START: I didn't write this code but i edited it from original script: http://userscripts.org/scripts/show/50826 */
		...
		if(parseInt(getValue('LastUpdate', '0')) + 86400000 <= (new Date().getTime())) {
			try {
				xmlhttpRequest( { method: 'GET',
								  url: 'http://userscripts.org/scripts/source/121343.meta.js?' + new Date().getTime(),
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@versionnumber\s+(\d+\.\d+)/)[1] > version) {
			if(confirm("There's an update available for 'Troll faces facebook on facebook Chat'.\nDo you wish to install it?")) openInTab('http://userscripts.org/scripts/source/121343.user.js');
		}
	}
...
/* END */
	...
	trollfaces = ['171108522930776', '164413893600463', '218595638164996', '129627277060203', '227644903931785', '100002752520227', '105387672833401', '100002727365206', '224812970902314', 'ForeverAloneComics', '164413893600463', '149333368464171', '116616581692281', '145768898802324', '168456309878025', '189637151067601', '106043532814443', '214457085240151', '160723207280093', '100002752520227', '224812970902314', '192644604154319', '170815706323196', '142670085793927', '168040846586189'];
    ...
    if (headTag) {
		...
		styleTag.innerHTML =
			...
			'.chatstylesbut {width: 22px; height:22px; background-image: url("' + ResourcesURL + 'zx/r/FbCyXQSrD4-.png"); cursor: pointer; border-color: rgb(153, 153, 153) rgb(153, 153, 153) rgb(136, 136, 136); border-style: solid; border-width: 1px; }'+
	...
	for(i=0;i<trollfaces.length;i+=1) {
		var fEmotsDom = document.createElement('img');
		fEmotsDom.setAttribute('alt','[['+ trollfaces[i] +']]');
		fEmotsDom.setAttribute('src','http://graph.facebook.com/'+ trollfaces[i] +'/picture');
		fEmotsDom.setAttribute('class','chatstylesbut');	
		//fEmotsDom.setAttribute('style','width: 22px; height:22px; cursor: pointer;');
