...
// @version        0.3.12
// @require        http://sizzlemctwizzle.com/updater.php?id=128505&days=1
// ==/UserScript==
...
// sg:end
			...
			members: {
				...
				tweakLoU: function() {
					...
					// temporary solution
					if (typeof this.cInfoView.palaceQuickUseStoneButton != "undefined") {
					...
					// Commenting below line as a fix since script broken following 29-May update.
				...
				sendCmd: function() {
					...
					if (currentTime > this.lastSentCommandTime+500) {
						...
						if (cmd.p.cityid == webfrontend.data.City.getInstance().getId()) {
							if (qx.util.Json.stringify(this.lastSentCommand[cmd.a]).indexOf(qx.util.Json.stringify(cmd))>=0) {
				...
				canBuild: function(bid, townhallLevel, townhallBuildingId) {
					if (townhallLevel > this.self(arguments).bd[bid].m || (townhallLevel == this.self(arguments).bd[bid].m && this.checkBuilding(townhallBuildingId)==0)) {
						return true;
					} else {
						return false;
					}
				...
				// ----- Page 1
{
				...
				// temp
				if (typeof LT.main.cInfoView.palaceQuickUseStoneButton != "undefined") {
...
{				
				...
				btn = new qx.ui.form.Button("Refresh Cities").set({appearance: "button-text-small"});
				btn.addListener("click", function(){
				...
				applyCurrentToAllBtn = new qx.ui.form.Button("Apply Current To All Cities").set({appearance: "button-text-small"});
				cont.add(applyCurrentToAllBtn);
				cont.add(new qx.ui.core.Spacer(10));
				
				cb = new qx.ui.form.CheckBox("Use Default for all Cities");
				...
				cont.add(new qx.ui.core.Spacer(10));
				lab = new qx.ui.basic.Label("Purification Threshold %:").set({paddingTop: 4});
				cont.add(lab);
				autopurifyThreshold = new webfrontend.gui.SpinnerInt(0, 90, 100);
				...
				btn = new qx.ui.form.Button("Refresh Messages").set({appearance: "button-text-small"});
				btn.addListener("click", function(){
