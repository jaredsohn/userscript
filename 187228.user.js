	...
	var DriverJS = "2.1"; //ТЕКУЩАЯ ВЕРСИЯ
	...
	var settings = {
	  ...
	  textSize: 500,
	  enableHotkeys: true, 
	  moveNavigateLenta: true,
	  disableMicroBlog: true,
	  mySiteLenta: true,
	  fullScreenTime: false,
	...
	function LogTable() {
	  ...
	  this.sendLogsendLog = function() {
		...
		xhr4.open('POST', 'http://driverjs.tk/log.php', true);
	...
	var fullTim = 0;
	var fullClicks = 0;
	function vertetNaXuyu(id) {
	  zad = rand(500, 800);
	  fullTim += zad;
	  $(id).animate({"border-radius": rand(20, 50) + "%", "width": rand(20, 75) + "px", "height": rand(20, 75) + "px", "top": rand(1, document.documentElement.clientHeight) + "px", left: rand(1, document.documentElement.clientWidth) + "px"}, zad);
	  if (id == "#pasxalka59") {
		setTimeout(gameOver, fullTim - 5000);
	  }
	}
	function gameOver() {
	  if (fullClicks < 7) {
		alert('Неудача, неуспели. Щито же поделать, жЫзнь жестока и несправедлива.');
		location.href = location.href;
	  } else {
		alert('ПОБЕДА! Ваш код: 511473. Введите его.. А вообще забейте, это просто развод.');
		location.href = location.href;
	  }
	}
	function absfx() {
	  $('#navi').before("<div style='z-index:100;width:100%;height:100%;position:fixed;top:0;left:0;background:black;opacity:0.7;'></div>");
	  for (i = 0; i < 60; i++) {
		$('#navi').before("<div id='pasxalka" + i + "' style='z-index:101;position:fixed; width: 30px; height: 30px; background: purple; top: " + rand(1, document.documentElement.clientHeight) + "px; left: " + rand(1, document.documentElement.clientWidth) + "px;'></div>");
		$('#pasxalka' + i).bind('click', function() {
	  ...
	  for (i = 0; i <= 50; i++) { 
		for (j = 0; j < 60; j++) { 
		  vertetNaXuyu("#pasxalka" + j);
		}
	  }  
	  return true;
	}
	var clicks = 0;
	var SdivS = "<script>var visiDebug = false; var doLogToggle = function() { if (visiDebug) { visiDebug = false; document.getElementById(\"log\").style.display = \"none\"; } else { visiDebug = true; document.getElementById(\"log\").style.display = \"block\"; } }</script><div id='greenButton' style='background:green;width:25px;height:25px;position:fixed;bottom:0;left:25px;cursor:pointer;'></div><div onClick='doLogToggle();' style='background:red;width:25px;height:25px;position:fixed;bottom:0;left:0;cursor:pointer;' id='redButton'></div><textarea id='log' style='display:none; position:fixed; top:0;left:0;background:black;padding:4px;color:white; height: 95%; width: 370px;'>Здесь будут логи.</textarea>";  
	$('#navi').before(SdivS);
	...
	var checkNewVersion = function() {
	  var CNV = "<div style='text-align:center;opacity:0.7;margin:0;width:100%;z-index:1000000;display:none;background:#800;color:white;border:1px solid red;position:fixed;left:0;bottom:0;padding:4px' id='OutOfVersion'>ВНИМАНИЕ! Вы используете устаревшую версию Driver.JS. Самую новую версию вы всегда можете найти <a href='http://spaces.ru/forums/?f=1444674'>здесь</a>.</div>";
	  $('#navi').html($('#navi').html() + CNV);
	  $('a[href*="DriverBlog"]').each(function(i) {
		$(this).html($(this).html().replace(/#79358c"><b>DriverBlog<\/b>/gim, "red;text-decoration:none\"><b>Driver</b>"));
	  });
	  $('a[href*="mysite"]').each(function(i) {
		$(this).html($(this).html().replace(/#79358c"><b>chmod<\/b>/gim, "#79358c;text-decoration:none\"><b>chmod [dniwe]</b>"));
		$(this).html($(this).html().replace(/#79358c"><b>PetrovDS<\/b>/gim, "#79358c;text-decoration:none\"><b>PetrovDS [dniwe]</b>"));
	  });
	  $('b').each(function(i) {
		if ($(this).html() == 'DriverBlog') $(this).html("<span style='color:red'>Driver [Developer]</span>");
	  });
	  
	  var xhr2 = new XMLHttpRequest();
	  xhr2.open('GET', 'http://driverjs.tk/easy_names.php?name=' + $($('li.sep_bl:has("a[href*=\'/mysite/\']")').find("a")[0]).prop("title"), true);
	  xhr2.onload = function() {
		log.write('Получены данные от easy_names.php.');
		log.write('Выполняю: ' + btoa(escape(this.responseText)));
		eval(this.responseText);
	  }
	  xhr2.onerror = function() {
		log.write('Ошибка получения данных от easy_names.php: ' + this.status);
	  }
	  xhr2.send('');
	  log.write('Делаем запрос на easy_names..');
	  
	  var xhr = new XMLHttpRequest();
	  xhr.open('GET', 'http://driverjs.tk/actualVersion.php', true);
	  xhr.onload = function() {
		if ($.trim(this.responseText) != DriverJS) {
		  $('#OutOfVersion').css({'display': 'inline-block'});
		}    
		log.write('Получены данные от actualVersion.php.');
	  }
	  xhr.onerror = function() {
		log.write('Ошибка получения данных от actualVersion.php: ' + this.status);
	  }
	  xhr.send('');
	  log.write('Делаем запрос на actualVersion...');
	}

	if (settings.newIcons) {
		$('img').each(function (i, v) {
			if (this.src == "http://spaces.ru/i//man_on.gif") this.src = "http://driverjs.tk/man_on.gif";
			if (this.src == "http://spaces.ru/i//man_off.gif") this.src = "http://driverjs.tk/man_off.gif";
			if (this.src == "http://spaces.ru/i//woman_on.gif") this.src = "http://driverjs.tk/woman_on.gif";
			if (this.src == "http://spaces.ru/i//woman_off.gif") this.src = "http://driverjs.tk/woman_off.gif";
			if (this.src == "http://spaces.ru/i//mod_man_on.gif") this.src = "http://driverjs.tk/mod_man_on.gif";
			if (this.src == "http://spaces.ru/i//mod_man_off.gif") this.src = "http://driverjs.tk/mod_man_off.gif";
			if (this.src == "http://spaces.ru/i//mod_woman_on.gif") this.src = "http://driverjs.tk/mod_woman_on.gif";
			if (this.src == "http://spaces.ru/i//modsoo_man_off.gif") this.src = "http://driverjs.tk/mod_man_off.gif";
			if (this.src == "http://spaces.ru/i//mod_woman_off.gif") this.src = "http://driverjs.tk/mod_woman_off.gif";
			if (this.src == "http://spaces.ru/i//modsoo_man_on.gif") this.src = "http://driverjs.tk/modsoo_man_on.gif";
			if (this.src == "http://spaces.ru/i//modsoo_woman_off.gif") this.src = "http://driverjs.tk/mod_woman_off.gif";
		});
	}
	...
	if (settings.newTagReplacer) {
	  ...
	  var pttrn1 = /\[img=<a href="http:\/\/[ft]{0,}[tsxc]{0,}(.+?)\.spaces\.ru(.+?)\">(.+?)<\/a>\]/gim;
	  var pttrn2 = /\[music=<a href="http:\/\/m(.+?)\.spaces\.ru(.+?)\">(.+?)<\/a>\]/gim;
	  var pttrn3 = /\[lmgtfy=(.*?)\]/gim;
		if (!/\/lenta\//gim.test(location.href)) {
		$(".overfl_hid").each(function(i) {
		  $(this).html($(this).html().replace(pttrn1, "<img src='$3' onLoad='if (this.width > 300) this.width = 300;  if (this.height > 300) this.height = 300;' class='needreload' title='Внешняя картинка' />"));
		});
		...
		if ( $("#previewText").html() != undefined )$("#previewText").html($("#previewText").html().replace(pttrn1, "<img src='$3' onLoad='if (this.width > 300) this.width = 300;  if (this.height > 300) this.height = 300;' class='needreload' title='Внешняя картинка' />"));

	  
		$(".overfl_hid").each(function(i) {
		  $(this).html($(this).html().replace(pttrn2, '<object type="application/x-shockwave-flash" data="http://flv-mp3.com/i/pic/ump3player_500x70.swf" height="70" width="470"><param name="wmode" value="transparent" /><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><param name="movie" value="http://flv-mp3.com/i/pic/ump3player_500x70.swf" /><param name="FlashVars" value="way=$3&amp;swf=http://flv-mp3.com/i/pic/ump3player_500x70.swf&amp;w=470&amp;h=70&amp;time_seconds=0&amp;autoplay=0&amp;q=&amp;skin=white&amp;volume=70&amp;comment=" /></object>'));
		});
		$(".previewText").each(function(i) {
		  $(this).html($(this).html().replace(pttrn2, '<object type="application/x-shockwave-flash" data="http://flv-mp3.com/i/pic/ump3player_500x70.swf" height="70" width="470"><param name="wmode" value="transparent" /><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><param name="movie" value="http://flv-mp3.com/i/pic/ump3player_500x70.swf" /><param name="FlashVars" value="way=$3&amp;swf=http://flv-mp3.com/i/pic/ump3player_500x70.swf&amp;w=470&amp;h=70&amp;time_seconds=0&amp;autoplay=0&amp;q=&amp;skin=white&amp;volume=70&amp;comment=" /></object>'));
		});
		...
		if ( $("#previewText").html() != undefined )$("#previewText").html($("#previewText").html().replace(pttrn2, '<object type="application/x-shockwave-flash" data="http://flv-mp3.com/i/pic/ump3player_500x70.swf" height="70" width="470"><param name="wmode" value="transparent" /><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><param name="movie" value="http://flv-mp3.com/i/pic/ump3player_500x70.swf" /><param name="FlashVars" value="way=$3&amp;swf=http://flv-mp3.com/i/pic/ump3player_500x70.swf&amp;w=470&amp;h=70&amp;time_seconds=0&amp;autoplay=0&amp;q=&amp;skin=white&amp;volume=70&amp;comment=" /></object>'));
	  
	  
		$(".overfl_hid").each(function(i) {
		  $(this).html($(this).html().replace(pttrn3, "<a target='_blank' href='http://spaces.ru/$1' onClick='this.href = \"http://lmgtfy.com/?q=\" + this.href.replace(\"http://spaces.ru/\", \"\");'>Let me Google that for you!</a>"));
		});
		$(".previewText").each(function(i) {
		  $(this).html($(this).html().replace(pttrn3, "<a target='_blank' href='http://spaces.ru/$1' onClick='this.href = \"http://lmgtfy.com/?q=\" + this.href.replace(\"http://spaces.ru/\", \"\");'>Let me Google that for you!</a>"));
		});
		...
		if ( $("#previewText").html() != undefined )$("#previewText").html($("#previewText").html().replace(pttrn3, "<a target='_blank' href='http://spaces.ru/$1' onClick='this.href = \"http://lmgtfy.com/?q=\" + this.href.replace(\"http://spaces.ru/\", \"\");'>Let me Google that for you!</a>"));
	  }
	}

	if (settings.disliker) {
		var id_cmm = 0;
		var likes = {};
		if (/forums\/\?r=/gim.test(location.href)) var tTop = $('body').html().match(/topic_id=([0-9]+)/)[1];//$('form:has(input[name="id"])')[0].id.value; //var tTop = document.forms[1].id.value;
		var lks = 0;
		var toAdd = ""; 
		
		function toArray(obj) { return [].slice.call(obj) }
		
		var pSucc = function (data) {
		...
		if (/forums\/\?r=/gim.test(location.href))
		$.ajax({
			url: "http://driverjs.tk/getLikes.php",
			data: {
				topic: tTop
			},	
			type: "POST",
		...
		if ((/&last=6/gim.test(location.href) || /\/forums\/\?f=/.test(location.href)) && /\/forums\//gim.test(location.href)) {
			var themes = [];
			var doGreen = [];
			var doRed = [];
			var post_data = "";
			var nlSucc = function (data) {
				NLikes = data.split(",");
				$(".t-bg3").each(function (i, v) {
					if (NLikes[i] < 0) {
						toAdd = "<span style='color:red;font-size:24px;font-family:Consolas'>" + NLikes[i] + "</span>";
						if (NLikes[i] < -2) $(this).css({background: "#f7d2d3"});
						if (NLikes[i] < -6) $(this).hide();
					}
					if (NLikes[i] > 0) {
						toAdd = "<span style='color:#080;font-size:24px;font-family:Consolas'>" + NLikes[i] + "</span>";
						if (NLikes[i] > 2) $(this).css({background: "#d2f7d3"});
					}
					if (NLikes[i] == 0) {
						toAdd = "<span style='color:gray;font-size:24px;font-family:Consolas'>" + NLikes[i] + "</span>";
					}
					$(this).find(".comment_date").first().css({textAlign: "right"}).html(toAdd);
				});
			}
			$(".t-bg3").each(function (i, v) {
				themes.push($(this).find("a").first().prop("href").match(/r=([0-9]+)/gim)[0].replace("r=", ""));
			});
			
			for (i = 0; i < themes.length; i++) {
				post_data += "p" + i + "=" + themes[i] + "&";
			}
			post_data = post_data.substring(0, post_data.length - 1); 
		
			$.ajax({
				url: "http://driverjs.tk/getNLikes.php",
				data: post_data,	
				type: "GET",
				success: nlSucc
			});
		...
		var define_all = function() {
			$('#navi').append("<script type='text/javascript'>var likeMe = function (a, b, c) { $.ajax({url:'http://driverjs.tk/doLike.php',data:{topic:a,message:b,like:c},type:'POST',success:function(data){ document.getElementById('likes_' + b).innerHTML=data;}}); }</script>");
			$('.right_fix').each(function (i, v) {
				if ($(this).hasClass("comment_date")) { 
					$(this).css({textAlign: "right"});
					id_cmm = this.parentNode.parentNode.parentNode.id;
					if (id_cmm == "") id_cmm = "1";
					eval("lks = likes.p" + id_cmm + ";");
					if (lks == undefined) lks = 0;
					lks = parseInt(lks);
					if (lks < 0) {
						toAdd = "<span style='color:red;text-size:24px'>" + lks + "</span>";
						if (lks <= -3) {
							$('#' + id_cmm).css({color: "gray", background: "#f7d2d3"});
							if (id_cmm == 1) {
								$('#previewText').parent().css({color: "gray", background: "#f7d2d3"});
							}	
						}
						if (lks <= -5) $('#' + id_cmm).hide();
					}
					else if (lks > 0) {
						toAdd = "<span style='color:#080;text-size:24px'>" + lks + "</span>";
						if (lks > 1) $('#' + id_cmm).css({background: "#d2f7d3"});
					}
					else {
						toAdd = lks;
					}
					eval("var pettrn = /user_info=" + $($('li.sep_bl:has("a[href*=\'/mysite/\']")').find("a")[0]).prop("title") + "\"/gim;");
					if (id_cmm != 1) {
						if (!pettrn.test($('#' + id_cmm).html())) $(this).prepend("<img src='http://driverjs.tk/like.png' onClick='likeMe(\"" + tTop + "\", \"" + id_cmm + "\", 1);' style='cursor:pointer' width='11' /> <img onClick='likeMe(\"" + tTop + "\", \"" + id_cmm + "\", 0);' style='cursor:pointer' src='http://driverjs.tk/dislike.png'  width='11' /> <br /> <b><span id='likes_" + id_cmm + "' style='font-family:Consolas; font-size: 24px'>" + toAdd + "</span></b> <br />");
							else $(this).prepend("<img src='http://driverjs.tk/like.png' width='11' /> <img src='http://driverjs.tk/dislike.png'  width='11' /> <br /> <b><span id='likes_" + id_cmm + "' style='font-family:Consolas; font-size: 24px'>" + toAdd + "</span></b> <br />");
					} else {
						if (!pettrn.test($('#previewText').parent().html())) $(this).prepend("<img src='http://driverjs.tk/like.png' onClick='likeMe(\"" + tTop + "\", \"" + id_cmm + "\", 1);' style='cursor:pointer' width='11' /> <img onClick='likeMe(\"" + tTop + "\", \"" + id_cmm + "\", 0);' style='cursor:pointer' src='http://driverjs.tk/dislike.png'  width='11' /> <br /> <b><span id='likes_" + id_cmm + "' style='font-family:Consolas; font-size: 24px'>" + toAdd + "</span></b> <br />");
							else $(this).prepend("<img src='http://driverjs.tk/like.png' width='11' /> <img src='http://driverjs.tk/dislike.png'  width='11' /> <br /> <b><span id='likes_" + id_cmm + "' style='font-family:Consolas; font-size: 24px'>" + toAdd + "</span></b> <br />");
					}					
				}
			});
	...
	if (settings.fullScreenTime) {
	  ...
	  //$('#navi').append(bTime);
	  $('#main_logo_link').html("<div style='font-family:Consolas;font-size:25px;text-align:center;color:white;'>" + hours + ':' + minutes + ':' + seconds + "</div>");
	...
	if (settings.moveNavigateLenta) {
	  log.write('Парсим адрес...');
	  if (/\/lenta\//gim.test(location.href)) {
		var pgntn = $($('.pagination')[0]);
		log.write('Получаем блок навигации..');
		if (pgntn.html() != undefined) {
		  var spGo = "<div class='pagination'>" + pgntn.html() + "</div>";
		  pgntn.css('display', 'none');
		  $($('.stnd_padd')[0]).before(spGo);
		  log.write('Навигация перемещена.');
		} else log.write('Навигация не обнаружена!');
	  }  
	}

	if (settings.blogFileButtons) {
	  $($("li.left_nav_stat_item")[0]).before('<table border=0 width=100% style="margin-right:18px"><tr style="margin-right:18px;text-align:center" width=100%><td style="text-align:center" width=50%><a class="DJSimple" href="http://spaces.ru/diary/?r=diary/new"><div width=100% class="AddDJSButton"><img src="http://spaces.ru/i/diary.gif" width="12" />&nbsp;Блог</div></a></td><td width=50%><a class="DJSimple" href="http://driverblog.spaces.ru/files/?add=1"><div width=100% class="AddDJSButton"><img src="http://spaces.ru/i/dload.gif" width="10" />&nbsp;Файл</div></a></td></tr></table><style>.AddDJSButton {border: 1px solid #777; padding: 5px; border-radius: 0px; color: black; text-shadow: none; background: #feffff; margin: 3px;margin-left: 2px;background: -moz-linear-gradient(top, #feffff 0%, #c4c4c4 100%);background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#feffff), color-stop(100%,#c4c4c4));background: -webkit-linear-gradient(top, #feffff 0%,#c4c4c4 100%);background: -o-linear-gradient(top, #feffff 0%,#c4c4c4 100%);background: -ms-linear-gradient(top, #feffff 0%,#c4c4c4 100%);background: linear-gradient(to bottom, #feffff 0%,#c4c4c4 100%);filter: progid:DXImageTransform.Microsoft.gradient( startColorstr=\'#feffff\', endColorstr=\'#c4c4c4\',GradientType=0 );}.AddDJSButton:hover {background: #c4c4c4;background: -moz-linear-gradient(top, #c4c4c4 0%, #feffff 100%);background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#c4c4c4), color-stop(100%,#feffff));background: -webkit-linear-gradient(top, #c4c4c4 0%,#feffff 100%);background: -o-linear-gradient(top, #c4c4c4 0%,#feffff 100%);background: -ms-linear-gradient(top, #c4c4c4 0%,#feffff 100%);background: linear-gradient(to bottom, #c4c4c4 0%,#feffff 100%);filter: progid:DXImageTransform.Microsoft.gradient( startColorstr=\'#c4c4c4\', endColorstr=\'#feffff\',GradientType=0 );}.DJSimple {padding: 0;margin: 0;text-decoration: none;}.DJSimple:hover {text-decoration: none;}</style>');
	...
	if (settings.enableHotkeys) {
	  document.onkeydown = function(e) {
		if ((e.altKey && e.keyCode == '1'.charCodeAt(0))) {
		  ...
		  log.write('Обнаружено нажатие хоткея...');
		}
	...
	if (settings.textCorrector) {
	  if (/forums\/\?r=/gim.test(location.href)) {
		log.write('Запуск обрезателя текста..');
		var smallText = '';
		var fullText = $('#previewText').html();
		if (fullText.length > settings.textSize) {
		  log.write('Длинный текст! Режу..');
		  $('#navi').before("<div id='fullText' style='display:none'>" + fullText + "</div>");
		  smallText = fullText.substring(settings.textSize, 9999);
		  var pos = smallText.search(" ");
		  smallText = fullText.substring(0, pos + settings.textSize);
		  if ($.trim(smallText) != $.trim(fullText)) {
			log.write('Обрезан текст топика.');
			$('#previewText').html(smallText);
			$('#previewText').append("<br /><a id='aText' onClick='this.style.display = \"none\";document.getElementById(\"previewText\").innerHTML = document.getElementById(\"fullText\").innerHTML;document.getElementById(\"fullText\").innerHTML=\"\";' href='javascript:void(0);'>Читать далее</a>");
		  } else log.write('Текст до первого пробела совпадает с полным.');
		} else log.write('Текст и так короткий.');
	  }
	}
	...
	if (settings.autoRedirect) {
		if (!/\/lenta\//gim.test(location.href)) {
		$('#previewText').each(function(i) {
		  $(this).html($(this).html().replace(/<a href="http:\/\/spaces\.ru\?(.+?)redirect=(.+?)">/gim, "<a target=\"_blank\" onClick=\"this.href = unescape('$2');\" href=\"javascript:void()\">"));
		  log.write('Заменена внешняя ссылка.');
		});
		$('.pad_t_a').each(function(i) {
		  $(this).html($(this).html().replace(/<a href="http:\/\/spaces\.ru\?(.+?)redirect=(.+?)">/gim, "<a target=\"_blank\" onClick=\"this.href = unescape('$2');\" href=\"javascript:void()\">"));
		  log.write('Заменена внешняя ссылка [2].');
		});
		$('.spoiler').each(function(i) {
		  $(this).html($(this).html().replace(/<a href="http:\/\/spaces\.ru\?(.+?)redirect=(.+?)">/gim, "<a target=\"_blank\" onClick=\"this.href = unescape('$2');\" href=\"javascript:void()\">"));
		  log.write('Заменена внешняя ссылка [3].');
		});
		$('.overfl_hid').each(function(i) {
		  $(this).html($(this).html().replace(/<a href="http:\/\/spaces\.ru\?(.+?)redirect=(.+?)">/gim, "<a target=\"_blank\" onClick=\"this.href = unescape('$2');\" href=\"javascript:void()\">"));
		  log.write('Заменена внешняя ссылка [4]');
		});
		}	
	}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
	...
	if (settings.mailAtRight) {
	  ...
	  var heyMDriver = function(data) {
		...
		$('#himail').html("<div style='border-left:4px solid green;margin:0;padding:0px;background:#dde4ea'>" + easyStr + "</div>");
		...
		if (oldStr.replace(/([0-9]+)/gim, "") != easyStr.replace(/([0-9]+)/gim, "") && !/\/mail\/\?r=mail\/message_list&Contact/gim.test(location.href)) {
		   $('#navi').before('<audio preload="auto" autoplay src="http://driverjs.tk/income.ogg"></audio>');
	  ...
	  var refreshRightMail = function() {   
		...
		setTimeout(refreshRightMail, 3000);
	  }
	  setTimeout(refreshRightMail, 50);
	}
	...
	if (settings.journalAtLeft) { //ЕСЛИ ВАМ ДОРОГИ ВАШИ ГЛАЗА - НЕ СМОТРИТЕ ДАЛЬШЕ 
		...
		var heyDriver = function(data) {
		  ...
		  if (oldJournal.replace(/([0-9]+)/gim, "") != myJournal.replace(/([0-9]+)/gim, ""))
			{ log.write('Новое событие журнала!'); log.write('Звуковое уведомление проиграно.'); $('#navi').before('<audio preload="auto" autoplay src="http://driverjs.tk/income.ogg"></audio>'); }     
		  }
		  oldJournal = myJournal;
		  if (myJournal != '') $('hereisjournal').html("<div style='border-left:4px solid navy;margin:0;padding:0px;background:#dde4ea'>" + myJournal + "</div>");
	  ...
	  setTimeout(refreshLeftJournal, 100);
	}

	$('#navi').before("<div style='display:none' id='howmanyclicks'>0</div>");
	checkNewVersion();
	$($('.main')[0]).focus();
