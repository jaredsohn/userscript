// ==UserScript==
// @author         Ahileus
// @name           Ahi testing script
// @namespace      Ahitestingscript
// @description    Ahi testing script
// @version        1.66
...
if(nickname==sef1 || nickname==sef2 || nickname==sef3) {
jQuery("#large_sidebar").append("<br /><b>ADMIN MENU</b><br />* <a href='http://www.erepublik.com/en/article/2145355/1/20#comments'>Dodaj zapovjed</a>");
   }  
...
if(nickname==sef1 || nickname==sef2 || nickname==sef3) 
{
  jQuery("#menuText").append("<div style='margin:20px 0 -5px 0px; text-align:center; z-index:500; background-color:none; width:950px; font:normal 14px/16px Verdana;'><a href='http://www.erepublik.com/en/country/society/Egypt'>Country Egypt</a>   ||   <a href='http://www.erepublik.com/en/economy/citizen-accounts/4343681'>Egypt bank</a>   ||   <a href='http://egov4you.info/unit/overview/290'>LNL on eGOV</a>   ||   <a href='http://www.gvozdano.net/projekti/LNL/LnLdamage.php'>LNL damage report</a></table></div>");
}
if(nickname=="nickst07")
{
  jQuery("#menuText").append("<div style='margin:20px 0 -5px 0px; text-align:center; z-index:500; background-color:none; width:950px; font:normal 14px/16px Verdana;'><a href='http://www.erepublik.com/en/economy/job-market/63/1/desc'>HR JOB MARKET</a>   ||   <a href='http://www.erepublik.com/en/economy/market/63/12/1/citizen/0/price_asc/1'>HR WRM MARKET</a>   ||   <a href='http://www.erepublik.com/hr/economy/inventory'>STORAGE</a> || <a href='http://egov4you.info/unit/overview/290'>LNL on eGOV</a>   ||   <a href='http://www.gvozdano.net/projekti/edomovina/LnLdamage_skripta.php?show'>LNL damage report</a></table></div>");
}
}
// end create menu
...
function getData()
{
		path = parent.document.location.pathname.toString();
		erep_dan = jQuery("#header .header_info .eday strong").text();
		nick = jQuery('.user_info a').html();
		time = jQuery("#battle_countdown").text();
		damage = parseInt(jQuery("#total_damage strong").text().replace(/[^\d.]/g, ""));
		battle = jQuery("#pvp_header h2").text()+' ('+path.split('/')[4]+')';
		wall = selectFirst("#blue_domination_f", "#blue_domination").text();
		donatelink = "http://www.erepublik.com/en/economy/donate-items/" + profileUrl.substr(profileUrl.lastIndexOf("/") + 1);
		hits = 0;
		strenght = parseFloat(jQuery('#fighter_skill').text().trim().replace(/,/g,''));
		side = jQuery("#pvp_header .country h3:first").text();
		region = jQuery("#pvp_header h2").text();
		version: VERSION;
		division = 0;
		round = unsafeWindow.SERVER_DATA['zoneId'];
		level = jQuery("#large_sidebar .user_info .user_level b").text();
		profilelink = userLink.attr("href");
		battleid: jQuery(path.split('/')[4]);
		try{
