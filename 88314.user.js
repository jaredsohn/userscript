// ==UserScript==
// @name           Convertor duel
// @namespace      convertor duel by w00w
// @description    O noua versiune de convertire a duelurilor
		...
		switch(sLanguage) {
			case 'en':
				oReturn = {
					...
					rank_match: /Level (\d+)/,
					duel_rank_match: /Dueling level (\d+)/,
					no_hit_match: /No hit/,
					hit_replace: /^Strike: /,
					hit_match: /- (\d+) HP/,
			...
			case 'pt':
				oReturn = {
					...
					header_match: /^A (.+ às .+)/,
					header_today_match: /Hoje (às .+)/,
					values_match: /Local do Duelo/,
					rounds_match: /Total de pontos/,
					rank_match: /Nível (\d+)/,
					duel_rank_match: /Nível de duelo (\d+)/,
					no_hit_match: /Não atingiu/,
					hit_replace: /^Disparar: /,
					hit_match: /- (\d+) HP/,
					total_hit_match: /Total de pontos de saúde/,
					summary_match: /ganhou o duelo/,
					hit_h: 'Cabeça',
					hit_ls: 'Ombro esquerdo',
					hit_rs: 'Ombro direito',
					hit_la: 'Braço esquerdo',
					hit_ra: 'Braço direito'
				};
				break;
			case 'de':
			default:
		...
		switch (sLanguage) {
			...
			case 'pt':
				oReturn = {
					...
					sChallenger: 'D',
					sTotalHP: 'Total HP',
					show_title: 'relatório formatado'
				};
				break;
			case 'de':
			default:
	...
	addReportButton: function(div) {
		...
		for (var i = 0; i < allDivs.length; i++) {
			...
			if (0 < myID) {
				...
				if (myTitle.innerHTML.match(/Duel/)) {
