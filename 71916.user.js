...
//-----------------------------------------collect the prefix of the server---------------------------
...
//-----------------End of collection of prefix of server---------------
...
ClickNoWait:function(obj) {					//------mouse events
...
JustDidIt:function(name) {				//time count function
	var now = (new Date().getTime());
...
VisitUrl:function(href) {
	this.waitMilliSecs=3000+Math.random()*2000;
	nHtml.VisitUrl(href);
},  //------------------end of wait section-----------------------------
...
SetControls:function() {
	...
	//-------------nothing to do with the other stufff... it's just showing on the bot menu the version of bot------------
...
//--------------------Resources data (food,iron,wood,stone)
...
waitMilliSecs:5000,  //-----------------------another spent of time---------------------------------
...
//--------------Capcha w8ing stuff...
	...
	//---------------Main functions, like recruiting,building and so on...------------------------
	if (GM.getGValue("WaitForCaptcha",'') == '') {
	...
	this.WaitMainLoop(); //---------------------main w8 from b4
	return;
},
//----------------Function dat close everything u open--------------------
CloseFloat:function () {
	// close all pops
...
MainDailyHero:function() {
	...
	if (GM.getLValue("CurrentHermitHero","") == "") {
		if (this.PickArea('city', "governor")) return true;			//---example of fuction to move on pages---
		if (!this.navOK) return false;
...
MainDailyQuiz:function() {
	// once per hour
	if (!this.WhileSinceDidIt("AutoQuizQuest",60*60+Math.random()*60*5)) 
		return false;
	if (this.DailyQuestNav('3kingdoms quiz')) return true;
	if (!this.navOK) return false;
	this.JustDidIt("AutoQuizQuest");
//----------------------------------------------------------------------------------------------------------------------
	if (this.questOperate) {
		this.Click(this.questOperate);
		return true;
...
MainCamp:function() {
	...
	if (popName && (popName == GM.getLValue("CampFacLast",''))) {
		...
		if (!el_max) {
			...
			GM.setLValue('NeedHeroLevel', true);		//if the lvl of the hero isnt enough for autolooot camp then lvlup
			return this.CloseFloat();
		}
	...
	if (popName == 'Faction Map') {
		...
		rcit = (Math.random()*cit.length); //random choosing the faction camp to loot.
...
,
//---------------PLunder function, dat doesnt work atm------------------------------
...
MainRecruit:function() {
	...
	if (this.needFood)
		return false;
	//--------------move to what u going to recruit. It depends from faction quests request-----------------
	...
	//----Now start to recruit the troops-----
	...
	nHtml.LoopByXPath(sw, ".//div[@class='construct']//ul/li",function(li){
		...
		if (info) {
			...
			if ((pref=='') || name.indexOf(pref) >= 0) {
				...
				var avail=nHtml.GetText(info);		//decide how many troops to recruit.(half of available)
				...
				if (inp && avail>1 && but) {
					...
					okdid = true;
				} else {			//if dont have enough resoruces to recruit more then go out
...
MainBuild:function() {
	...
	if (ci) {
		...
		if (ci) {
			nHtml.LoopByXPath(ci, ".//li", function (li) {
				...
				if (b.match(/Lv\.\d/i)) {
					//blist = blist + "," + b;
					for(var i=0; i<b.length-2; i++){	//----remove the 1st "Lv." dat finds in b
						if(b[i]+b[i+1] +b[i+2] == "Lv."){
							b[i]='';
							b[i+1]='';
							b[i+2]='';
							break;
						}
					}		//Now it's possible to check for next "Lv."
					if (b.match(/Lv\.\d/i)) {
						blist = blist + "," + b;
						}
				}
	...
	if (this.WhileSinceDidIt('AutoResource',300)) {
		if (this.needFood) {		//main priority if there's no food build farmland
			GM.log('Food is low try build farmland');
			did = this.Build('resource', 'farmland');
		} else { //else check the lower production.
...
//End of checkign what should build. Now start to build.
...
//reload the web page from time to time.
