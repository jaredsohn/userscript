...
// @descriptionColorizes facebook based on the user's photo.
...
// ----------------------====================[ Help ]====================----------------------
//
//    If you are reading this, you need to use either Mozilla Firefox or Google Chrome.
//
//    If you are using Mozilla Firefox you also need "GreaseMonkey".
//
//    After you install either Firefox&GreaseMonkey OR Chrome, then click on this file again.
//
// ----------------------====================[ Help ]====================----------------------
...
//
// (c) 2010 William Bixler.  All rights reserved.

// v2.7.0 Support for Google Chrome. Support for secure connections.



thisScriptVersion = new Array(2,9,3);
...
function StoreItem(name, valueString) {
	if ((typeof GM_setValue != 'undefined') && (GM_setValue.toString().toLowerCase().indexOf('not supported') == -1)) { GM_setValue(name, valueString); return 1; }
	if (typeof localStorage != 'undefined') { localStorage.setItem(name,valueString); return 1; }
	return 0;
}

function GetItem(name, defaultValue) {
	if ((typeof GM_getValue != 'undefined') && (GM_getValue.toString().toLowerCase().indexOf('not supported') == -1)) { return GM_getValue(name, defaultValue); }
...
function DeleteItem(name) {
	/* untested */
	if ((typeof GM_deleteValue != 'undefined') && (GM_deleteValue.toString().toLowerCase().indexOf('not supported') == -1)) { GM_deleteValue(name); return 1; }
...
function ScrambleText(domNode, currentDepth) {
	...
	if (domNode.nodeType == 3 ) { /* A text node */
		var newStr = "";
...
function InsertCSS(colorArray, tweaks) {
	...
	.webComposerFluidInputTable .fluidInput { background: white; } \
	\
	/* Right Column */ \
	...
	.fcg {color: " + ForceContrast(colorArray[8], colorArray[9], 0.15) + ";} \
	...
	.profileImageUnknownMale50x50, .profileImageUnknownMale200x126{background:" + ForceContrast(MixTwoHexColors(colorArray[6],colorArray[9],0.7),"#FFFFFF",0.10) + ";} \
...
function ReplaceSilhouette(hexColor) {
	try {
		...
		for (var i=0; i<myImages.length; i++) {
			...
			if (className.indexOf('profileImageUnknown')>-1) continue; // This image was already processed in a previous run.
			
			addClassName = "";
			if (className.indexOf('UIProfileImage')>-1 || (myImages[i].id == "profile_pic")) {
					...
					case '.gif': // Normal users have .gif replacements.
						...
						switch(myImages[i].src.substr(-15)) {
							...
							case 'IB7NOFmPw2a.gif': // 50x50 female
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/q_silhouette-trans-white-female.png";
							...
							case 'UlIqmHJn-SK.gif': // 50x50 male
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/q_silhouette-trans-white-male.png";
								addClassName = "profileImageUnknownMale50x50";
								break;
								
							case 'yDnr5YfbJCH.gif': // 200x126 female 
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/l_silhouette-trans-white-female-swirl.png";
							...
							default: // Exact image unknown, gender unknown.
								...
								if (width == 50 && height == 50) {
									...
									myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/s_silhouette-trans-white-male-swirl.png";
					...
					case '.png': // 'Like' pages use png as the unknown image placeholder.
						...
						switch(myImages[i].src.substr(-15)) {
							...
							case 'SakaC0tDjfm.png': // 50x50 gadget
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/q_silhouette-trans-white-gadget.png";
								addClassName = "profileImageUnknownGadget50x50";
								break;
								
							case 'LKW-XRdGm6b.png': // 100x63 gadget
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/s_silhouette-trans-white-gadget.png";
								addClassName = "profileImageUnknownGadget100x63";
								//myImages[i].setAttribute('style',"background: " + hexColor + ";");
								break;
								
							case 'GylKztrlHCg.png': // 200x126 gadget
								myImages[i].src = "https://mywebspace.wisc.edu/ormont/web/facebook/l_silhouette-trans-white-gadget-swirl.png";
...
function AjaxGet(settings) {
	if ((typeof GM_xmlhttpRequest != 'undefined') && (GM_xmlhttpRequest.toString().toLowerCase().indexOf('not supported') == -1)) GM_xmlhttpRequest(settings);
