...
var Version = '20101205e';
var MAP_DELAY = 1500;
var ENABLE_TEST_TAB = false;

//var URL_CASTLE_BUT 'http://kocpower.googlecode.com/files/CastleBut2.png';
//var URL_CASTLE_BUT_SEL 'http://kocpower.googlecode.com/files/CastleBut3.png';
...
//logit ('*** STARTUP RETRY! ');
    window.setTimeout (ptStartup, 1000);
    return;
  }
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  GM_addStyle ('.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    a.ptButton20 {color:#ffff80}#srcOutTab td { vertical-align:top; text-align:left; }#srcOutTab td:nth-child(odd) { background:#eee; }#srcOutTab tr:nth-child(even) td { background:#ccc; }#srcOutTab td:nth-child(6) div { width:360px !important; overflow:auto;}\
    .matTab {}\
...
my.AllianceList = {
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<STYLE>.oddrow {background-color:#eee}</style><DIV class=ptstat>Showing players matching <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE class=ptTab cellspacing=0><TR style="font-weight:bold"><TD>Name</td>\
...
t.makeCityImg (0, eMap);      
t.makeCityImg (1, eMap);      
t.makeCityImg (2, eMap);      
t.makeCityImg (3, eMap);      
***/
      t.state = 1;
    }
  },
...
logit ('city.x: '+ city.x +' , x: '+ x);
    x -= provMapCoords.imgWidth;
    x += provMapCoords.leftMargin;
    var ce = document.createElement ('div');
    ce.style.backgroundColor = '#fff';
    ce.style.position='relative';
    ce.style.display='in-line';
//ce.style.clear = 'both';    
//ce.style.float = 'left';    
    ce.style.width='16px';
...
try {    
  ...
  togOpt : function (checkboxId, optionName, callOnChange){
    ...
    checkbox.addEventListener ('change', new eventToggle(checkboxId, optionName, callOnChange).handler, false);
...
//DebugTimer.start(); 
    ...
    for(i=0; i<Cities.numCities; i++) {
      str += "<TD width=81><B>"+ Cities.cities[i].name.substring(0,10) +'</b><BR>(<a onclick="ptGotoMap('+Cities.cities[i].x +','+ Cities.cities[i].y  +');">'+
          Cities.cities[i].x +','+ Cities.cities[i].y  +"</a>)</td>";
    }
    if (Options.includeMarching)
...
function getWallInfo (cityId, objOut){
  var spots = 0;
  objOut.wallLevel = parseInt(Seed.buildings["city" + cityId].pos1[1]);
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  var m = '<TABLE cellspacing=0 width=100% height=100%><TR id="'+ prefix +'_bar" class="CPopupTop"><TD id="'+ prefix +'_top" width=99%><DIV id=spanTest></div></td>\
...
//this.divLeft = parseInt(movingDiv.style.left);
//this.divTop = parseInt(movingDiv.style.top);
