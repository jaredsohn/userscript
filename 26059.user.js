// ==UserScript==
// @name          Travian: Attack builder SC
...
function fixcords(c, r) {
	...
	for (var i=0; i < ca1.length; i++) {
		...
		if (ca1[i]) {
			...
			var cc = ca2[i][0] + cordsSplit + ca2[i][1];

//			alert(cc);

			if (ca2[i][0] && ca2[i][1] && !in_array(cc, ca3)) {
				ca3[j] = cc;
				j++;
			}
		}
...
//	ca1 = array_unique(ca1);

	return (r ? ca3.join(targetSplit) : ca3);
}

function array_unique( array1 ) {
...
function splitCords(c) {
	...
	if (c && c.match(/(\-?[0-9]{1,3})\s?\|\s?(\-?[0-9]{1,3})/g)) {
		x = parseInt(RegExp.$1);
		y = parseInt(RegExp.$2);
...
//			attack();
			...
			setTimeout(function(){attack()}, timebetweenwaves > 300 ? timebetweenwaves : 300);
		}
	}
}
	...
	GM_xmlhttpRequest({
		...
		onload: function(responseDetails) {
			...
			if (numberattacks >= totalattacks)
			{
				...
				addCount(".");
				addCount("<a href='build.php?id=39' target='_blank'>"+text[8]+" ("+aMatch[0]+") x "+numberattacks+"("+totalattacks+")</a>");  //legg inn delay her
				abort();
			}else{
				addCount(".");
			}
