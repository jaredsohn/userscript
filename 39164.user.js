// ==UserScript==
// @author		Original author: usr8472@gmail.com
...
/*
Original Scripts from userscripts.org: http://userscripts.org/scripts/show/7409

ported to IE by laoyu20032003@hotmail.com
*/
...
eventSource.attachEvent('onload', function(e) { onLoad(); });
...
function onLoad(){ // runs the differnt functions and procedures, 
	...
	switch(lang){//load only the lang you need into memory
		...
		case '.bg':
	       ...
	       break;
	   case '.ae':
		langfile[lang]=['يحتاج الى التطوير:','الفائض', 'متعادل','مستوى المصادر','قياس المصادر', 
		' الساعة' ,'المصدر سيكون متاح اليوم عند '," ", "المصدر يتاح في ", 
		" يوم عند " , " اطول وقت للانتظار عند: " , "المخزون: ","وقت الانتظار: ","النقص" ,"الفائض"," الانتظار الاقصى: ","متوازن",
		"اضافة الاضافية تنتهي في: ","تطوير "," المصادر الناتجة.","المصادر متعادلة.", "اضافة المواد" , "اضافة الحبوب"];					
		break;
...
function rowOpera(i,leftval){
	...
	simg.className='res';
	//simg.style.clear="both";
	simg.style.styleFloat="left";
	...
	if(leftval>=90){
		value.style.textDecoration = 'blink';
...
function getResourceInfo(){ //using the resource values on the page, return the production, store, and capacity values as an array
	...
	for(var i=1;i<=4;i++) {
		...
		resource.push( [parseInt(rtd.title),parseInt(rtd.innerText.match(/\-?(\d+)\//)),parseInt(rtd.innerText.replace(/(\d+)\//,""))] );
	}
...
function getTime(){ // gets the time from the page or use java time
	...
	var servertime=returnObjById("tp1").innerText;
...
function countdown(){//updates all countdown values and displays
	...
	for(var i=0; i<4; i++){
		...
		if(overflow[i][0]>0){
			...
			if(old.innerText.indexOf("0:00:00 h")==-1){    
				//old.textContent="0:00:00 h";
				old.innerText="0:00:00 h";
				old.style.color='red';
				old=returnObjById("resbar"+i);
...
*/
	...
	var need = getElementsByClassName(document, 'table', 'f10');
	...
	for(var i=0; i<need.length; i++){
		...
		for(var k=0; k<tdcells.length; k++){
			...
			if(tdcells[k].className == 's7'){cout++;}
		}	
	
		if(cout==0){
			//var c=current.textContent;
			var c=current.innerText;
	...
	for(var i=0; i<end; i++){
		...
		var current=resourceBlock[i].innerText.split(" | ");
		...
		holdtable.className = "tbg";
		holdtable.style.width = "500px";
		holdtable.setAttribute('cellspacing', 1);
		holdtable.setAttribute('cellpadding', 1);
		var tbody;
		var w=false;
		var g=false;
		var blink=false;
		...
		if(maxtime>0){
			...
			holdtable.appendChild(tbody);

			//resourceBlock[i].appendChild( holdtable );
			resourceBlock[i].parentNode.insertBefore(holdtable, resourceBlock[i].nextSibling);
	}
}
