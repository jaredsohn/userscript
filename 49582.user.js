// ==UserScript==
// @name           Better WildGuns
// @namespace      http://www.defhoboz.biz/
// @description    Make WildGuns Better
// @include        http://*.wildguns.org/*
// ==/UserScript==
...
if (document.location.href.match("user.php")) {
  function displayBuildQueue(){
    $('vSelectVisible').style.width="568px";
    $('vSelect').style.width="550px";
    ...
    for(var server in buildQueue) {
      ...
      for(var cord in buildQueue[server]) {
        ...
        var villages=$x("//ul[@id='vSelect']/li",document.getElementById('villageContent'))
        for(var coord in villages) {
          ...
          if (villages[coord].childNodes[0].innerHTML.match(cordTxt)) {
            ...
            try {
              var offensetxt= "<span style='color:#FC5580;font-size:1.4em'>"+comma(t["Military"]["Offensive"])+"</span>"
              var defensetxt= "<span style='color:#8EE3F7;font-size:1.4em'>"+comma(t["Military"]["Defensive"])+"</span>"
    ...
    totalLi.innerHTML="<span style='"+regt+"'>Totals</span> - <span style='color:#BF8D4E;"+regt+"'>"+comma(totals[0])+"</span> --- <span style='color:#CA7166;"+regt+"'>"+comma(totals[1])+"</span> --- <span style='color:#756B61;"+regt+"'>"+comma(totals[2])+"</span> --- <span style='color:#D2B04B;"+regt+"'>"+comma(totals[3])
    ...
    if ($('attacks').innerHTML.match(/No troop movements/)) {
      $('attacks').style.height="46px"
      expand++;
    }
    if (expand==2) {
  ...
  function startitup() {
    ...
    var curTownNode=$x("//ul[@id='vSelect']/li[@class='vselectActive']")[0]
    ...
    var curTownMilitary = $x("//table[@id='militaryInVillage']/tbody/tr");
    var curTownWood = $x("//div[@id='ressources']/span[@id='ressourceWood']")[0];
    var curTownBrick = $x("//div[@id='ressources']/span[@id='ressourceBrick']")[0];
    var curTownOre = $x("//div[@id='ressources']/span[@id='ressourceOre']")[0];
    var curTownFood = $x("//div[@id='ressources']/span[@id='ressourceFood']")[0];
    ...
    try {
      ...
      for(var m in curTownMilitary) {
        var amt=parseInt($x("//td[2]",curTownMilitary[m])[m].innerHTML)
        var txt=$x("//td[3]",curTownMilitary[m])[m].innerHTML
        // 1 offensive (girl)
        if (txt.match(/Archer|Archer|Archer/)) off=off+amt;
        // 2 defensive
        if (txt.match(/Rifleman|Rifleman|Rifleman/)) def=def+amt;
        // 3 offensive
        if (txt.match(/Tomahawk fighter|Tomahawk fighter|Tomahawk fighter/)) off=off+amt;
        // 4 info don't add for this
        // 5 defensive
        if (txt.match(/Mounted archer|Mounted archer|Mounted archer/)) def=def+amt;
        // 6 offensive
        if (txt.match(/Blunderbuss|Blunderbuss|Blunderbuss/)) off=off+amt;
        // 7 offensive
        if (txt.match(/Fire fiend|Fire fiend|Fire fiend/)) off=off+amt;
        // 8 offensive
        if (txt.match(/Burning arrow|Burning arrow|Burning arrow/)) off=off+amt;
        // 9 offensive
        if (txt.match(/Firewater trek|Firewater trek|Firewater trek/)) off=off+amt;
  ...
  for(var i=0;i<list.length;i++) {
    try {
      ...
      if (content.match(/[0-9]+\|[0-9]+/)) {
        ...
        newlink.href = "http://s1.wildguns.org/user.php?action=map&mapY="+cords[0]+"&mapX="+cords[1]
        ...
        newlink.innerHTML="Open map to "+content+" (about " + dis + " spaces away)"
...
if (document.location.href.match("action=mail&mailaction=inbox&showmail=[0-9]+")) {
	if (top.location != location) {
		...
		var uname=$x("//div/div/div[3]/div/form/input[1]",document)[0].value
		var subj=$x("//div/div/div[3]/div/form/input[2]",document)[0].value
		...
		f.innerHTML+="<input type='submit' value='Reply' name='reply'/>&nbsp;"
		f.innerHTML+="<a class='button lbOn' href='lightbox_content/lb_main.php?action=squeal&igm_id="+m_id+"'>Report message</a>"
		document.body.innerHTML=document.getElementById('mailviewInner').innerHTML
		document.body.appendChild(f)
		document.body.style.textAlign="left"
		document.body.style.background="#F8E1C2"
	}
}
...
if (document.location.href.match("action=map")) {
 ...
 b.innerHTML = "Set these coordinates as Home"
 b.href="javascript:// Set these coordinates as home for distance calculations"
