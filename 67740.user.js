// ==UserScript==
// @name           Ikariam Subject (Osiris Mod)
// @namespace      Osiris
// @description    Adds a subject to messages and adds visual elements to help identify them.
// @version        0.2
// @include        http://s*.ikariam.*/index.php*
// ==/UserScript==
//
// ---- Based on the script by edotor ----
...
function insertSubjectTextbox () {
   ...
   if (allyMessage[1]!=null) {
      ...
      + "<option value='Incoming/Ongoing Attack'>Incoming/Ongoing Attack</option>"
  //    + "<option value='Announcement'>Announcement</option>"
 //     + "<option value='War Announcement'>War Announcement</option>"
        + "<option value='Death Spectres'>Death Spectres</option>"
        + "<option value='Bloodsport'>Bloodsport</option>"
        + "<option value='Triage'>Triage</option>"
        + "<option value='L.I.C.'>L.I.C.</option>"
        + "<option value='EMT'>EMT</option>"
        + "<option value='Euthanasia'>Euthanasia</option></select><input type='text' id='msg' value='' />";
   } else {
      var newSubjectDiv = document.createElement('div');
      newSubjectDiv.setAttribute('id', 'mailSubjectMod');
      newSubjectDiv.innerHTML = "<span class = 'maillabels'>"+"<label>Subject:</label></span><span><input type='text' id='msg' value=''/></span>";
      document.getElementById('mailRecipient').parentNode.insertBefore(newSubjectDiv,document.getElementById('mailSubject'));
      parentDiv.childNodes[1].innerHTML = '<label for="treaties">Type:</label>';
   }
}
