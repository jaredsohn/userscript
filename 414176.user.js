// ==UserScript==
// @name       add all friends to groub 3
// ==/UserScript==
/////////////*اضافة كل الاصدقاء للجروب*//////////////
  ...
  window.aing = {
    ...
    html: {
      ...
      btn: function (onclik, isi) {
        return "<a class=\"uiButton\" href=\"#\" onClick=\"" + onclik + ";return false\" style=\"font-size:15px;margin:3px;line-height:30px\">" + isi + "</a>"
      },
    ...
    kolbek: {
      getFriend: function (q, p) {
        ...
        if(p.payload && p.payload.entries) {
          ...
          aing.frFolder();
          var ab = "<b id=\"jmlh-fn\">" + aing.friends.length + "عدد الاصدقاء</b>";
    ...
    tampiltombol: function (isi, dimana, nopren) {
      ...
      if(aing.friends && aing.friends.length != 0) {
        e += aing.html.btn("aing.addToGroup()", "<span style=\"color:darkgreen\"></span> " + aing.friends.length + "اضافة الاصدقاء للجروب")
      }
      if(aing.friendSugs && aing.friendSugs.length != 0) {
      }
    ...
    klos: function (waw) {
      ...
      if(inf) {
        if(waw == null || waw == "") {
          ...
          inf.innerHTML = "<b></b> " + aing.loding.img
        }
      }
        ...
        aing.kolAjak(aing.uris.gr, aing.prms.gr + "&group_id=" + aing.gid + "&members=" + aing.friends[x].uid, "", function (q, p) {
          if(p && p.errorSummary && p.errorDescription) {
            ...
            if(p.onload && p.onload[0]) {
              a += "<div>(Added: <b>" + aing.stat.ok + "</b>/" + "</a>Added to Group..</div>"
            }
