// ==UserScript==
// @name           	Ikariam Gelişmiş Eklenti
// @namespace      	PhasmaExMachina
// @description    	İkariam hakkında bütün eklentilerin bir araya getirildigi bir scripter (www.ikariam.forumm.biz)
...
ScriptUpdater.check(80545, "0.88");
...
ExMachina.views.city = function(root) {
	...
	function redrawDotForBuilding(building) {
		if (root == document && Config.get('showBuildingLevels')) {
			...
			var hasResources = (IkaTools.buildingGetResourceMissingTotal(building) == 0);
			if (hasResources && building.level != IkaTools.maxBuildingLevel) 
...
Pillage = {
	...
	drawTargetsOverview:function() {
		...
		for(var x in targets) {
			...
			sortBucket[Pillage.targetGetPlayerName(targets[x]) + targets[x].cityName] = targets[x];
		}
