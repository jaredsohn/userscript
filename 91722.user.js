// ==UserScript==
// @name           Buscador de Ricos em SP
// @namespace      Buscador de Ricos em SP
// @description    Localiza os ricos ao seu alcance
// @author         James Maxwel
// @version        1.0
// @include        *sampa.faveladogame.com.br/fight/
// ==/UserScript==
...
function populateVector(vect, pantalla) 
{
	GM_xmlhttpRequest(
		{
		   	method: 'GET',
		   	url: 'http://www.sampa.faveladogame.com.br/fight/',
		   	onload: function(responseDetails) 
			{
				...
				for (i=0; i<numPantallas; i++) 
				{
					...
					var myurl = 'http://www.sampa.faveladogame.com.br/highscore/user/' + count + '/?min=' + ptsMin + '&max=' + ptsMax;
									...
									for (j=1; j<splits.length; j++) 
									{
										...
										if (buscar)
										{
											GM_xmlhttpRequest(
													{
													   	method: 'GET',
													   	url: 'http://www.sampa.faveladogame.com.br/dev/api/user.' + userid + '.xml',
													    onload: function(responseDetails) 
													    {
...
function crearTabla(vect) 
{
	...
	for (i=0; i<vect.length; i++) 
	{
		...
		insertGang(td3, vect[i][4], vect[i][5]);
...
function insertGang(mytd, gangid, gang)
{
...
{
	mytd.innerHTML = points + '&nbsp;&nbsp;<a href="http://www.sampa.faveladogame.com.br/fight/?to=' + name + '"><img src="http://media.pennergame.de/img/att.gif"></a>';
}
