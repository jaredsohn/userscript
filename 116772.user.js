...
var legends = [
        'Arceus','Articuno','Azelf','Celebi','Cobalion','Cresselia','Darkrai','Darkrown','Deoxys',
        'Dialga','Entei','Genesect','Giratina','Groudon','Heatran','Ho-oh','Jirachi','Keldeo','Kyogre',
        'Kyurem','Landorus','Latias','Latios','Lugia','Manaphy','Mesprit','Mew','Mewtwo','Moltres','Palkia',
        'Phione','Raikou','Rayquaza','Regice','Regigigas','Regirock','Registeel','Reshiram','Rotom','Shaymin',
        'Shaymin (Sky)','Suicune','Terrakion','Thundurus','Tornadus','Uxie','Victini','Virizion','Zapdos','Zekrom'
];
var Starters = [
	//FirstGen
...
var settings = {
	'keys': ['autoBattle', 'findRare', 'findLegendary', 'findStarter'],
	'data': {},
	'save': function() {
...
function moveAround() {
	...
	var xpos = parseInt(Math.sin(movPos * (Math.PI / 170))*10);
...
function findStarter() {
	...
	for (var i = 0; i < Starters.length; i++) {
		if (wildText.match(Starters[i])) {
			unsafeWindow.console.info('Starter found ', wildText);
			return;
		}
	}
...
function autoCatch() {
	try {
		var btnContinue = unsafeWindow.document.querySelector('#battleForm input[type="submit"][value="Battle!"]');
		if (btnContinue) {
			btnContinue.click();
		}
		...
		var btnContinue3 = unsafeWindow.document.querySelector('#battleForm input[type="submit"][value="Continue!"]');
		if (btnContinue3) {
			btnContinue3.click();
		}
	
		var btnAttack = unsafeWindow.document.querySelector('#itemForm input[type="submit"][value="Use Item"]');
		if (btnAttack) {
			btnAttack.click();
		}
...
function autoContinue() {
	...
	if (settings.data.findLegendary) {
        ...
        if (settings.data.findStarter) {
		findStarter();
	}
        if (settings.data.autoCatch) {
		autoCatch();
	}
}
...
function init() {
	...
	createToggler(container, 'Find Legendary', 'findLegendary');
        createToggler(container, 'Find Starter', 'findStarter');
	createToggler(container, 'Auto-catch:', 'autoCatch');
	autoContinue();
}
