// ==UserScript==
// @namespace       heroeswm
// @name            HWM Check Auction
// @description     Позволяет быстро проверять состояние рынка.
// @version         1.9.8
// @grant           GM_setValue
// @grant           GM_getValue
...
// @exclude         http://www.heroeswm.ru/index.php
...
/** Загрузка и парсинг инвентаря
*
* @param Function onLoad    Обработчик
*/
Inventory.loadParse = function(onLoad) {
    //GreaseMonkey.log('Inventory.loadParse()');
    
    /** Встроенный обработчик
    * @param Document doc       Загруженный документ
    */
    function onRawLoad(doc) {
        var inventory = Inventory.parse(doc);
        if (onLoad !== undefined)
            onLoad(inventory);
    }
    
    Inventory.load(onRawLoad);
...
* @param Function onLoad    Обработчик
*/
Inventory.load = function(onLoad) {
    //GreaseMonkey.log('Inventory.load()');
    
    var url = 'http://' + document.location.hostname + '/inventory.php';
...
/** Парсинг текущего набора
* @param Document doc
* @return Array
*/
Inventory.parseCurrentSet = function(doc) {
    var set = [];
...
* @return Boolean
*/
Inventory.useSet = function(invSet, name, onLoad) {
...
* @return Boolean
*/
Inventory.useItem = function(index, invSet, onLoad) {
    
    function onRawLoad() { 
        Inventory.useItem(index + 1, invSet, onLoad); 
    }
    
    var href = undefined;
    if (index < 0) {
        href = 'http://' + document.location.hostname + '/inventory.php?all_off=1';
    } else {
        if (invSet[index] !== undefined) {
            href = 'http://' + document.location.hostname + '/inventory.php?dress=' + invSet[index].id;
        }
    }
    
    if (href !== undefined) {
        GreaseMonkey.loadDoc(href, onRawLoad);
    } else {
        if (onLoad !== undefined) {
            onLoad();
        }
...
Player.parseTableSkills = function(player, table) {
    ...
    Player.parsePlayerResources(player);
...
* @param Player player      Игрок
*/
Player.parsePlayerResources = function(player) {
    
    var nodes = player.nodes.resources.getElementsByTagName('B');
    player.resources = {};
    for (var i = 0; i < nodes.length; i++) {
        
        var title = String.trim(nodes[i].textContent);
        var text = nodes[i].nextSibling.textContent;
        var count = Number(text.replace(/\D/ig, ''));
        ...
        player.resources[title] = resource;        
        //GreaseMonkey.log(resource.title + ': ' + resource.count);
...
Player.parsePlayerGuilds = function(player) {
    ...
    var text = $(player.nodes.guilds).text();
    player.guilds = {};
    
    // Гильдия рабочих
    var re = new RegExp("Гильдия\\s*Рабочих:\\s*(\\d+)\\s*\\((\\S+)\\)\\s*\\+(\\S+)", 'g');
    var res = re.exec(text);
    player.guilds.workersLevel = Number(res[1]);
    player.guilds.workersValue = Number(res[2]);
    
    // Гильдия кузнецов
    var re = new RegExp("Гильдия\\s*Кузнецов:\\s*(\\d+)\\s*\\((\\S+)\\)\\s*\\+(\\S+)", 'g');
    var res = re.exec(text);
    player.guilds.blacksmithsLevel = Number(res[1]);
    player.guilds.blacksmithsValue = Number(res[2]);
...
if (window === top) {
    ...
    var conf = {
        ...
        'showMinMax': {'type': 'bool', 'title': 'Мин/макс цены', 'def': true, 'desc': 'Отображение минимальной и максимальной цен.'},
        'showDates': {'type': 'bool', 'title': 'Показывать даты', 'def': false, 'desc': 'Отображение дат регистрации минимальной и максимальной цен.'},
        'showDif': {'type': 'bool', 'title': 'Показывать разницу', 'def': true, 'desc': 'Отображение разницы цен.'},
        'trackElements': {'type': 'bool', 'title': 'Отслеживать элементы', 'def': true, 'desc': 'Отслеживание цен на элементы гильдии наемников.'},        
        'maxPrices': {'title': 'Кол-во цен', 'def': 5, 'desc': 'Отображаемое количество цен для каждого отслеживаемого артефакта.'}
    }
...
function handleData() {
    ...
    // Максимальное количество цен на арт
    var maxPrices = GM_getValue('maxPrices', 5);
    
    // Отображение дат
    var showMinMax = GM_getValue('showMinMax', true);
    
    // Отображение дат
    var showDates = GM_getValue('showDates', false);
    
    // Отображение разницы
    var showDif = GM_getValue('showDif', true);
    
    
    // Заголовки таблицы
    var titles = {title: 'Артефакт', hero: 'У вас', priceMin: 'Мин.' + (showDates ? ' цена' : ''), priceMax: 'Макс.' + (showDates ? ' цена' : ''), offers: 'Объявл.'};
    var hline = {title: '------'}
    
    // Выравнивание колонок
    var aligns = {title: 'right', hero: 'right', priceMin: 'right', priceMax: 'right', offers: 'right'};
    
    
    // Удаляем заголовк и выравнивания для мин. и макс. цен
    if (!showMinMax) {
        delete titles.priceMin; delete aligns.priceMin;
        delete titles.priceMax; delete aligns.priceMax;            
    }
    ...
    for (var i in titles)
        html += '<th>' + titles[i] + '</th>';
    for (var i = 0; i < maxPrices; i++)
        html += '<td></td>';
    html = '<tr>' + html + '</tr>';
    
    // Формируем общий массив
    var arts = [];
    for (var i = 0; i < gameArts.length; i++) {
        var art = gameArts[i];
        
        // Устанавливаем количество у героя
        delete art.hero;
        if (heroArts[art.title] !== undefined)
            art.hero = heroArts[art.title];
        if (playerInfo !== null)
            if (playerInfo.resources[art.title] !== undefined)
                art.hero = playerInfo.resources[art.title].count;
        
        // Пропускаем не интересные артефакты
        if (art.prices === undefined)
            continue;
        
        // Подготавливаем цены артефакта
        art['pricesLine'] = '|';
        var count = Math.min(art.prices.length, maxPrices);
        for (var j = 0; j < count; j++) {
            if ((j > 0) && (showDif)) {
                art['price' + j] = '+' + (art.prices[j].cost - art.prices[0].cost);
            } else {
                art['price' + j] = art.prices[j].cost;
            }
            aligns['price' + j] = 'right';
        }
        
        // Ссылка на страницу арта
        art.infoUrl = Art.url(art.id, art.uid);
        if (art.cat == 'elements')
            delete art.infoUrl;
        ...
        // Ячейка с нзванием арта
        htmlCells.title = '<td><a>' + art.title + '</a></td>';
        if (art.infoUrl != undefined)
            htmlCells.title = '<td><a href="' + art.infoUrl + '" title="Информация об артефакте">' + art.title + '</a></td>';
        
        // Ячейка с количеством у героя
        htmlCells.hero = '<td>&nbsp;</td>';
        if (art.hero != undefined)
            if (art.prices.length > 0)
                htmlCells.hero = '<td><a href="http://www.heroeswm.ru/auction_new_lot.php" title="Выставить на продажу" artTitle="' + art.title + '" artPrice="' + art.prices[0].cost + '">' + art.hero + '</a></td>';
        
        // Ячейки с мин. и макс. ценами
        htmlCells.minPrice = '<td title="' + Time.toDateString(Time.fromNumber(art.priceMinTime)) + '">' + art.priceMin + '</td>';
        htmlCells.maxPrice = '<td title="' + Time.toDateString(Time.fromNumber(art.priceMaxTime)) + '">' + art.priceMax + '</td>';
        
        // Ячейка с количеством объявлений
        htmlCells.offers = '<td><a href="' + art.url + '" title="Смотреть обявления">' + art.offers + '</a></td>';
        
        // Ячейки с ценами
        for (var j = 0; j < maxPrices; j++) {
            htmlCells['price' + j] = '<td></td>';
            if (art['price' + j] != undefined)
                    htmlCells['price' + j] = '<td>' + art['price' + j] + '</td>';
        }
        
        // Строка таблицы в HTML
        var htmlRow = '';
        for (var j in htmlCells)
            htmlRow += htmlCells[j];        
        html += '<tr>' + htmlRow + '</tr>';
        
        // Конвертируем даты
        if (showMinMax && showDates) {
            art.priceMin += ' (' + Time.toDateString(Time.fromNumber(art.priceMinTime)) + ')';
            art.priceMax += ' (' + Time.toDateString(Time.fromNumber(art.priceMaxTime)) + ')';
        }
        ...
        delete art.infoUrl;
        delete art.prices;
        delete art.priceMinTime;
        delete art.priceMaxTime;
        
        // Удаляем мин. и макс. цены
        if (!showMinMax) {
            delete art.priceMin;
            delete art.priceMax;
        }
        
        // Копируем интересные артефакты в результирующий массив
        arts[arts.length] = art;
    ...
    arts.unshift(hline);
    arts.unshift(titles);
    var str = String.fromObjects(arts, aligns);
    GreaseMonkey.log('Текущее состояние рынка:' + "\n\n" + str);
    
    var path = [gui.scriptName, 'AuctionState'];
    var pre = '<pre>' + str + '</pre>';
    html = '<table>' + html + '</table>';
    if (gui.hasWindow(path)) {
        var win = gui.updateWindow(path, html);
    } else {
        var win = gui.createWindow(path, 'Текущее состояние рынка', html).css({'width': 'auto'});
        gui.createWindowButton(path.concat(['Update']), 'Обновить').click( function(){ gui.toggleWindow(path); checkAuction(); } );
        gui.createWindowButton(path.concat(['Close']), 'Закрыть').click( function(){ gui.toggleWindow(path); } );
        // Обновление кнопки "Проверить рынок"
        bCheck.unbind();
        bCheck.click(function() { gui.toggleMenu(); gui.toggleWindow(path); });
    }
    $('td', win).css({'color': '#000000', 'font-size': '10px', 'text-align': 'right'});
    $('th', win).css({'color': '#000000', 'font-size': '10px'});
    $('td a[title="Выставить на продажу"]', win).click(
        function () {
            GM_setValue('art', $(this).attr('artTitle'));
            GM_setValue('price', $(this).attr('artPrice'));
        }
    );
    gui.toggleWindow(path);
...
* @param Object Art
*/
function checkAuctionPrices(art) {
    
    
    /** Обработчик загрузки цен артефакта
    * @param Array prices
    */
    function onLoadArtPrices(prices) {
        
        requests--;
        art.prices = prices;
        art.offers = prices.length;
        Auction.saveMaxMinPrices({id: art.id}, prices);
    ...
    // Загружаем цены артефакта
    requests++;
    Auction.loadPricesList(art, document, onLoadArtPrices);
...
function checkAuction() {
    ...
    // Номер игрока
    var playerId = GM_getValue('playerId', null);
    
    // Отслеживание элементов
    var trackElements = GM_getValue('trackElements', true);
    
    
    /** Обработчик загрузки информации об игроке
    * @param Object player
    */
    function onLoadPlayer(player) {
        ...
        // Загрузка списка артефактов героя
        Inventory.loadParse(onLoadInventory);
        
    }
    
    
    /** Обработчик загрузки инвентаря
    * @param Array arts
    */
    function onLoadInventory(arts) {
        
        /** Обработчик загрузки общего списка артефактов
        * @param Array arts
        */
        function onLoadArtsList(arts) {
            
            gameArts = arts;
            
            for (var i = 0; i < arts.length; i++) {
                //GreaseMonkey.log(arts[i].title);
                
                if (
                    (trackedArts[arts[i].id] !== undefined) ||
                    ((trackElements) && (arts[i].cat == 'elements'))
                ) {
                    //GreaseMonkey.log('+' + arts[i].title);
                    checkAuctionPrices(arts[i]);
                }
            }
        ...
        var data = [];
        for (var i = 0; i < arts.length; i++) {
            //GreaseMonkey.log('Найдено у героя: "' + arts[i].title + '".');
            if (data[arts[i].title] === undefined)
                data[arts[i].title] = 0;
            data[arts[i].title]++;
        }
        ...
        // Загрузка общего списка артефактов
        Auction.loadArtsList(document, onLoadArtsList);
    ...
    if (trackElements) {
        ...
        Player.loadParse(playerId, onLoadPlayer);
...
/** Переключение режима отслеживания артефакта
* @param String artId
*/
function toggleTrack(artId) {
    
    if (trackedArts[artId] === undefined) {
        trackedArts[artId] = true;
    } else {
        delete trackedArts[artId];
    }
    
    GreaseMonkey.saveObject('trackedArts', trackedArts);
    bTrack.css({'color': (trackedArts[artId] !== undefined) ? 'black' : 'silver'});
...
if (!disabled) {
    ...
    if (url.attr('path') == '/home.php') {
        ...
        GM_setValue('playerId', home.playerId);
        
    }
    
    
    // На странице информации об артефакте
    if (url.attr('path') == '/art_info.php') {
        
        // Номер текущего артефакта
        var artId = url.param('id');
        
        // Массив отслеживаемых артефактов
        var trackedArts = GreaseMonkey.loadObject('trackedArts', {});
        
        // Кнопка "Отслеживать"
        var bTrack = gui.createMenuButton(['Track'], 'Отслеживать цены');
        bTrack.click(function() { toggleTrack(artId); gui.toggleMenu(); });
        bTrack.css({'color': (trackedArts[artId] !== undefined) ? 'black' : 'silver'});
        
    }
    
    
    // На странице создания нового лота аукциона
    if (url.attr('path') == '/auction_new_lot.php') {
        
        var art = GM_getValue('art', null);
        var price = GM_getValue('price', 0);
        GM_deleteValue('art');
        GM_deleteValue('price');        
        
        if (art != null)
            AuctionNewLot.setNewLot(document, art, price);
    ...
    if (window === top) {
        ...
        // Общий массив артефактов
        var gameArts = null;
        
        // Массив артефактов героя
        var heroArts = null;
        ...
        // Кнопка "Проверить рынок"
        var bCheck = gui.createMenuButton(['CheckAuction'], 'Рынок');
        bCheck.click(function() { checkAuction(); gui.toggleMenu(); });
