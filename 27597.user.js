...
boxScan.innerHTML = 
     ...
     ' <td class="header">'+
      ' <table class="header">'+
       ...
       ' </tr>'+
       ' <tr>'+
       ' <td class="header" align="right"><SELECT id="selOP"><OPTION value="na" selected >nome alleanza :</OPTION><OPTION value="ng">nome giocatore :</OPTION></SELECT></td><td class="header"><input id="allyTag" value="" size="17" maxlength="20" type="text"></td>'+
       ' </tr>'+
       ' <tr>'+
...
var v5    = an.value;
var selValue = selOP.options[selOP.selectedIndex].value;
...
for (iGal=(v1);iGal<=(v2);iGal++){
    for (iSys=(v3);iSys<=(v4);iSys++){
     ...
     //apro
  setTimeout("go("+iGal+","+iSys+",'"+ogaserver+"','"+sess+"','"+v5+"','"+selValue+"')",100);
 //chiudo
...
unsafeWindow.go = function(iGal,iSys,ogaserver,sess,ns,st){
    ...
    onload:  function(responseDetails) {
        ...
        var i2 = null;
             ...
             htmlString = htmlString.substring(htmlString.indexOf('tabindex=') +10);
                       ...
                       case 'na' : if (ally.indexOf(' '+ns+' ')!=-1) showResultNode.innerHTML+= '<TR><TD align="left">' +planet.substring(planet.indexOf(' '))+'</TD><TD align ="left">'+player.substring(player.indexOf(' '))+'</TD></TR>'; break; 
 case 'ng' : if (player.indexOf(' '+ns+' ')!=-1) showResultNode.innerHTML+= '<TR><TD align="left">' +planet.substring(planet.indexOf(' '))+'</TD><TD align ="left">'+player.substring(player.indexOf(' '))+'</TD></TR>'; break; 
