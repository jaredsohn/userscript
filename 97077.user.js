...
cleanButton.value ="Clean Up (BB Hatchies)";
...
function cleanCodes(){
	...
	var resArray = mystring.match(/\[url=http:\/\/magistream.com\/creature.+?url\]/gi);
	var id;
	...
	for(var i=0; i<resArray.length; i++){
			...
			if(requestString.match(/^Adult/)
                                || requestString.match(/A magic and mysterious thingthing/)
				|| requestString.match(/^Frozen/)
                                || requestString.match(/Hatchling/)
