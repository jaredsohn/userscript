// ==UserScript==
// @name           3gokushi-Beyond_Formosa
// @namespace      3gokushi-hatt
// @description    ブラウザ三国志用便利機能色々ごった煮(中華繁体字対応) by radedance
...
var VERSION_NAME = "ブラウザ三国志Beyond Ver2.0 by hasekun";
...
function initResources()
{
    ...
    for(var i=0 ; i<spns.length ; i++) {
        var str = spns[i].previousSibling.nodeValue.match(/(木|石|鐵|糧)\s+(-?\d+)/);
        if( str ) {
            var name = "";
            switch( str[1] ) {
            case "木":
...
function loadOptions()
{
    ...
    OPT_MEMO_WIDTH = cloadData( "OPT_MEMO_WIDTH", "40" );
...
//  OPT_DECK = cloadData( "OPT_DECK", 1 );
    ...
    OPT_TTDISTANCE_ITEMS = cloadData("OPT_DISTANCE_ITEMS", "[\"矛槍兵(10)\", \"弩兵(8)\", \"近衛騎兵(15)\", \"投石機(6)\", \"剣兵(6)\", \"槍兵(7)\", \"弓兵(5)\", \"騎兵(12)\", \"衝車(3)\"]", true, true);
...
//  OPT_REPORT_NP = cgetCheckBoxValue("OPT_REPORT_NP");
    ...
    ADV_DEL = cgetCheckBoxValue("ADV_DEL");
...
//  ccreateCheckBox(td3, "OPT_REPORT_NP", OPT_REPORT_NP, "同盟ログ前後表示","同盟ログの詳細に、[前のログ]・[次のログ]を追加します（限定的）",0);
    ...
    ccreateCheckBox(td3, "OPT_PIKA_TRDHELP", OPT_PIKA_TRDHELP, "トレード画面入力改善","4桁数字を入力するとカードNoとして検索します",0);
    ccreateCheckBox(td3, "OPT_PIKA_BLINKBLD", OPT_PIKA_BLINKBLD, "都市画面改善","建設中を点滅させます",0);
    ccreateCheckBox(td3, "OPT_SUZAN_SEISAN", OPT_SUZAN_SEISAN, "拠点生産量表示","拠点の生産量を表示します",0);
    ...
    ccreateCheckBox(td3, "OPT_STYLE", OPT_STYLE, "スタイル変更","スタイルを旧スタイルに近い形に変更します",0);
    ccreateCheckBox(td3, "ADV_DEL", ADV_DEL, "ページ上部の広告削除","ページ上部に広告がある時にチェックすると消える",20);

   var fs = d.createElement("fieldset0");
   var lg = d.createElement("legend0");
   lg.appendChild(d.createTextNode("　ページ上部に広告が無い時は絶対にチェックを入れないように。　") );
   fs.appendChild(lg);
   td3.appendChild(fs);

   var fs2 = d.createElement("fieldset");
   var lg2 = d.createElement("legend");
   lg2.appendChild(d.createTextNode("　広告が無いのにチェックを入れるとレイアウトが崩れます。　　　　") );
   fs2.appendChild(lg2);
   td3.appendChild(fs2);


/*
   td4.appendChild(d.createElement("br"));
   var fs = d.createElement("fieldset");
   var lg = d.createElement("legend");
   lg.appendChild(d.createTextNode("　プレゼンツ by ALMIC　") );
   fs.appendChild(lg);
   td4.appendChild(fs);
   ccreateCheckBox(fs, "OPT_STYLE", OPT_STYLE, "スタイル変更","スタイルを旧スタイルに近い形に変更します",0);
*/
...
function disp_village() {
	...
	costs["監視台"] = cost_mihari;
	costs["修行所"] = cost_syuugyousyo;
	costs["城寨"] = cost_toride;
	costs["村莊"] = cost_mura;
    ...
    var allbld = $a("//li/span[@class=\"buildStatus\" and contains(text(),\"建造\")]/a");
    ...
    allbld = $a("//li[contains(text(), \"刪除中\")]/span[@class=\"buildStatus\"]/a");
    ...
    for(var i=0 ; i<maplinks ; i++) {
        ...
        if( tmp != "" ) {
            ...
            if( data.length == 3 ) {
                ...
                var link1 = d.createElement("a");
                link1.href = caddSes
                sionId("/user/index.php?user_id=" + data[0] + "#ptop");
                link1.title = data[3];
...
function disp_XYLink()
{
    ...
    if( location.pathname == "/alliance/info.php" ) {
        targetPath.push( "//tr[th[contains(text(),\"留言\")]]/td" );
    }
    if( location.pathname == "/message/detail.php" ) {
        targetPath.push( "//tr[th[contains(text(),\"主旨\")]]/td" );
...
/// check here by hajime 2011/05/15
//////////////////////
//兵力合計
//////////////////////
...
//                      cdeleteUserXY(aid);
    ...
    {
        var xy = html.match(/<\/a>\(都城\)<\/td>[^<]*<td[^>]*>([\-0-9]+),([\-0-9]+)<\/td>/);
        if( xy ) {
    ...
    {
        var ret = [];
        var tmp = html.match(/<td[^>]*>名次<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["all_rank"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>綜合<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["all_point"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>總人口<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["jinko"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>攻擊<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["attack"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>防禦<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["defence"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>擊破分數<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["attack_score"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<td[^>]*>防衛分數<\/td>[^<]*<td[^>]*>([0-9]+)<\/td>/);
        if( !tmp ) return null;
        ret["defence_score"] = parseInt(tmp[1] ,10);

        tmp = html.match(/<\/a>\(都城\)<\/td>[^<]*<td[^>]*>([\-0-9]+),([\-0-9]+)<\/td>/);
        if ( !tmp ) return null;
    ...
    function getXYListfromUserHTML(html)
    {
        ...
        tmp = html.match(/<td[^>]*>聯盟<\/td>[^<]*<td[^>]*><a href="[^"]*">([^<]+)<\/a><\/td>/);
        if( !tmp ) return;
        var ally_name = tmp[1];
...
function disp_RemoveList()
{
    ...
    if( location.pathname == "/land.php" && URL_PARAM.x && URL_PARAM.y ) {
        ...
        var rmtime = d.body.innerHTML.match(/(正在放棄領地中|村莊建造中|城寨建造中).*\n(\d+-\d+-\d+ \d+:\d+:\d+)/ );
        if( rmtime ) {
            if( rmtime[1] == "正在放棄領地中" ) {
                addList(rmtime[2], 0, URL_PARAM.x, URL_PARAM.y );
            }else if( rmtime[1] == "村莊建造中" ) {
                addList(rmtime[2], 3, URL_PARAM.x, URL_PARAM.y );
            }else if( rmtime[1] == "城寨建造中" ) {
                addList(rmtime[2], 4, URL_PARAM.x, URL_PARAM.y );
            }
        }else{
            rmtime = d.body.innerHTML.match(/目前領地處於升級中.*\n(\d+-\d+-\d+ \d+:\d+:\d+)完成。/ );
            if( rmtime ) {
    ...
    if( location.pathname == "/facility/castle.php" ) {
        ...
        var rmtime = d.body.innerHTML.match(/(村莊刪除中。|城寨刪除中。)[^\d]*(\d+-\d+-\d+ \d+:\d+:\d+)時完成。/);
        if( rmtime ) {
            if( rmtime[1] == "村莊刪除中。" ) {
                addList(rmtime[2], 1, xy.x, xy.y );
            }else if( rmtime[1] == "城寨刪除中。" ) {
...
function disp_UserLevel()
{
    ...
    if( location.pathname == "/land.php" || location.pathname == "/village.php" ) {
        ...
        var tmp = spnxy.innerHTML.match(/等級(\d+)/);
        if( tmp ) {
...
function disp_TSendTime()
{
    ...
    var tim = td.textContent.match(/距到達[:|：][\s|　]*(\d+):(\d+):(\d+)[\s|　]*到達時間/);
    if( !tim ) return;
    ...
    div.appendChild(d.createTextNode("到達時間："));
...
function Suzan_Seisan(inner)
{
    ...
    var areas = $a('//area[contains(@alt,"田") or contains(@alt,"穀") or contains(@alt,"伐") or contains(@alt,"森") or contains(@alt,"石") or contains(@alt,"岩") or contains(@alt,"製") or contains(@alt,"礦")]');
    ...
    text += '<li><img align="middle" alt="鐵" src="' + IMG_DIR + 'common/ico_iron2.gif"> 鐵  ' + seisan.i + '</li>';
...
#mixi_btn {display:none !important;}
...
{
	GM_addStyle(<><![CDATA[
#supportNavi a.snavi_friends {display:none !important;}
...
#mixi_btn{display:none !important;}
#supportNavi {position:absolute; top:10px; left:687px;}
#adsHeader {visibility: hidden;}
#profileBtn {position:absolute; top:0px; left:760px;}
...
#lodgment {top:148px !important; left:770px !important; background-image:none !important;}
...
/* 設定画面で「ページ上部の広告削除」にチェックがあったらレイアウト調整をする */
if (ADV_DEL == 1) {
GM_addStyle(<><![CDATA[
#header {position:absolute; top:4px !important; left:0px !important; z-index:999999 !important; background-color:#FFDE49 !important; height:75px !important;}
]]></>);
}
