...
// @updateURL		https://userscripts.org/scripts/source/155932.meta.js
// @downloadURL		https://userscripts.org/scripts/source/155932.user.js
...
function GM_setValue(a,b)//GM API模拟
{
 localStorage.setItem(a,b);
}
function GM_getValue(c)
{
var t=localStorage.getItem(c);
return t;
}
function GM_addStyle(s)
{var st=document.createElement('style');st.innerHTML=s;
...
GM_addStyle('#other li:hover{background:gray;}');
GM_addStyle('#other li{border:1px solid gray;background:rgb(65,137,244);}');
GM_addStyle('#other {position:relative;bottom:260px;left:920px;z-index:19999}');
GM_addStyle('td .tb-editor-editarea img{vertical-align:sub!important;}');
GM_addStyle('#fontchange,#cochange{background:rgb(65,137,244)!important;}');
GM_addStyle('td .tb-editor-editarea{max-height:193px!important;width:559px!important;}');
...
$('.pt_submit').append('<select class="subbtn_bg" id="cochange"><option style="background:#333333">#333333</option><option style="background:red">red</option><option style="background:blue">blue</option><option style="background:orange">orange</option><option style="background:purple">purple</option><option style="background:gray">gray</option><option style="background:pink">pink</option><option style="background:black">black</option></select>');//换色按钮
$('.pt_submit').append('<select id="fontchange" class="subbtn_bg" ><option selected="selected">'+GM_getValue('family')+'</option></select>');
$('.pt_submit').append('<input id="fontSize" style="width:30px!important;height:25px!important;"  type="text"  value="14" /><span id="px">px</span>');
$(document.body).append('<ul id="other" style=" visibility:hidden;;display:inline-block"><li id="diy">添加字体</li><li id="move">删除字体</li><li id="size">自定义默认大小</li><li id="morenziti">自定义默认字体</li></ul>');
...
$('#px').toggle(function(){$('#other').css( 'visibility','visible');},
					function(){$('#other').css( 'visibility','hidden');});
document.getElementById('fontSize').addEventListener('mousewheel',function(e)
																			{	e.preventDefault();
																				e.stopPropagation();
...
$('#only').click(function()
				{
					...
					toIGM(document.getElementById('fontSize').value,color,document.querySelector('td .tb-editor-editarea').innerHTML,true,arr);
					}
...
if(!(oldfont.match(newfont))&&newfont)
	...
	$('#fontchange').append('<option>'+newfont+'</option>');
		} 
...
{if(GM_getValue('allfont'))
	...
	for(x in font)
		{	if(font[x].length>0)
			...
			{
			$('#fontchange').append('<option>'+font[x]+'</option>')
			}
		}
	}
};
...
$('#cochange').change(function(){color=this.value;$('.tb-editor-editarea ').css('color',this.value);}),
document.getElementById('cochange').addEventListener('DOMMouseScroll',function(e)
										{ 
											e.preventDefault();
											e.stopPropagation();
										  if(e.detail>0)
										  ...
										  this.selectedIndex=(this.selectedIndex%8);
										 color=this.value;$('.tb-editor-editarea ').css('color',this.value);
...
{		
		var half=Math.round(font_size/2);
			var  tb=neirong+'|';
		  var line=tb.replace(/<span>/g,'').replace(/<br>/g,'|').replace(/\|{2,}/g,'|').replace(/&nbsp;/g,' ').replace(/<img.*">/g,'');
		  var  width=0;
		  var maxwidth=0;
		  var str='';
		  var  man=0;
		...
		{	
			...
			if(width>558)
			{str+=("|"+line[i]);width=full;man++;maxwidth=560;max=true;}
			else
...
canvas.width = maxwidth;//设置允许的最大图片宽度
canvas.height = (man)*lineHeight;//此处不再限制长度了，听天由命吧，不要太长哦
 var cxt=canvas.getContext('2d');
	for(x in brNums)
		{	
			cxt.fillStyle=fontcolor;
			cxt.font=font_size+'px'+' '+document.getElementById('fontchange').value;
			if(font_size==14)
				 cxt.fillText(brNums[x],0,19+21*x)	
			else cxt.fillText(brNums[x],1,lineHeight*x+full);
			
		}
			var src=canvas.toDataURL();
			try{upload(src,bull,node);}catch(err){alert(err)};

}

  function upload(f,bu,nodes) {//f是base64数据
			...
			function uploadedHandler(a, d)
				{try{
                        ...
                        if (c.error_code != 0) 
						{
								...
								if(bu)
									{
									document.querySelector('td .tb-editor-editarea').innerHTML='';
