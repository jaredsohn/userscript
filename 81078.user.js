...
function showTotal()
{
		...
		td1 = document.createElement("td");
		td1.setAttribute("class","job");
		td1.innerHTML = 'Ниво на склада  :';

		td2 = document.createElement("td");
		 var wareDiv = <>
								<input type="text" style="text-align: right;" size="8"  id="warehouseLevelId" value="0"/> Пример: 3 или 1,2,3 ако има повече от един склад (въведете само най-високите нива на складовете, но не повече от 4 склада)  .
							</>;
		...
		td1 = document.createElement("td");
		td1.setAttribute("class","job");
		td1.innerHTML = 'Количество за грабеж:';
		...
		td2.innerHTML = <>
								<a
									href={backURL}
									title="Обратно към скривалището"> <img
									src="skin/buildings/y100/safehouse.gif"
									width="160" height="100" /> <span class="textLabel">&lt;&lt; Обратно към скривалището </span>
								</a>
								</>;
		...
		td2 = document.createElement("td");
		td2.innerHTML = 'Enjoy!!  (Kabji Lambda Ikariam.org)(Edited for 0.3.4 from Zigi)';
...
if (snp_LevelSplit.length > 4)
{
					alert('Моля, въведете не повече от 4 склада.');
					level.value='0';
					level.focus();
					return false;
				}
		...
		for (var i = 0; i < snp_LevelSplit.length; i++)
		{
			...
			{
				if(parseInt(snp_LevelSplit[i]) > 32 || parseInt(snp_LevelSplit[i]) < 0)
				{
					alert('Моля, въведете само числа(0-32).');
					level.value='0';
					level.focus();
					return false;
				}
...
function calculateShip()
{
		...
		//find safe amt
			...
			b = woodAmt - snp_sumB + snp_LevelSplit.length * 100 - 100;
			w = wineAmt - snp_sumW + snp_LevelSplit.length * 100 - 100;
			m = marbleAmt - snp_sumM + snp_LevelSplit.length * 100 - 100;
			c = crystalAmt - snp_sumC + snp_LevelSplit.length * 100 - 100;
			s = sulphurAmt - snp_sumS + snp_LevelSplit.length * 100 - 100;
