// ==UserScript==
// @name		Facebook Hesap Kontrolü
// @namespace	Flei
// @auther		http://www.leirap.com
// @description	Facebook hesabınıza hızlı girin
...
function delValue(key){
	switch (storage) {
		case 'greasemonkey':GM_deleteValue('lei_'+key);break;
		case 'localstorage':delete localStorage['lei_'+key];break;
	}
}
function setValue(key, value) {
	switch (storage) {
		case 'greasemonkey':GM_setValue('lei_'+key, value);break;
		case 'localstorage':localStorage['lei_'+key] = value;break;
	}
}
function getValue(key, value) {
	switch (storage) {
		case 'greasemonkey':return GM_getValue('lei_'+key, value);
		case 'localstorage':var val = localStorage['lei_'+key];
...
FB_addStyle(
		'#lei-account { position:relative; display:list-item; }'+
		'#lei-account:hover ul, #lei-account:hover li { display:block; }'+
		'#lei-account ul a, #lei-account ul a:focus { background:white; }'+
		'#lei-account-list { overflow:auto; }'+
		'#lei-account img { background:url(/rsrc.php/z8S5R/hash/ez3x5cuc.png) no-repeat -137px 0; height:4px; left:5px; position:relative; top:-2px; width:7px; }'+
		'#lei-account ul { background:#fff;border:1px solid #333;border-bottom:2px solid #2d4486;display:none;margin-right:-1px;margin-top:-1px;min-width:200px;padding:10px 0 5px;position:absolute;right:0;_right:-1px;top:100%;*width:100%;_width:200px;z-index:1 }'+
		'#lei-account ul a {color:#3a579a;display:block;font-weight:normal;height:auto;_margin-right:-25px;padding:4px 10px 5px;white-space:nowrap;*white-space:auto;_white-space:nowrap}'+
		'#lei-account ul a:hover {background:#6d84b4;border-bottom:1px solid #3b5998;border-top:1px solid #3b5998;color:#fff;padding:3px 10px 4px}'+
		'#lei-account li {display:block;float:none}'+
		'#lei-account:hover lei-account-link {background-color:#fff;color:#333;height:22px;position:relative;z-index:3}'+
...
FB_addScriptUrl('http://leirap.googlecode.com/files/json2.js');
FB_addScriptUrl('http://leirap.googlecode.com/files/lei_setting.js');

//FB_addScriptUrl('http://127.0.0.1/lei_setting.js');
//============================================================================================================================
// Start Defind
//============================================================================================================================
var IMG = {
...
var set_default = {"sel_language":"tr_tr","chk_update":true,"chk_login_home":false,"chk_login_auto":false,"ipt_lock_pass":"39bc1c02846bc8749a79ee2b65d51e5f","ipt_custom_title":"Switch","chk_use_custom_title":false,"chk_show_user_name":true}
...
var TIME = {},$lei = {},lei = new Object(), fuid_uarr = new Array(),language_select,noLogin = false,lei_json = {"language_select":{"tr_tr":"Türkçe"}},fblocation = getValue('fblocation', WURL);
var face_lang = {
'tr_tr':{
...
if( document.getElementsByTagName('body')[0].className.indexOf('Locale') != -1 ){
	...
	$lei['fb_pn'].append('<li id="lei-account"><a href="#" onclick="return false;" id="lei-account-link" style="padding-right:16px;"><span>'+ $T('TITLE') +'</span><img src="http://static.ak.fbcdn.net/rsrc.php/z12E0/hash/8q2anwu7.gif" class="img" style="background:url(\'/rsrc.php/z8S5R/hash/ez3x5cuc.png\') no-repeat scroll -137px 0 transparent; height:4px; left:5px; position:relative; top:-2px; width:7px;"></a>'+
		'<ul id="lei-account-list"></ul></li>');
	$lei["account"] = $('#lei-account-list');
	$lei["title"] = $('#lei-account-link').find('span');
    $lei['lei-account'] = $('#lei-account');
	lei.Methods = {
		...
		CreatAccount:function(){
			...
			$.each(useraccount,function(index,acc){
				...
				list += '<li><a class="lei_BTN_LOGIN_ACCOUNT" id="user_'+index+'">'+userview+'</a></li>';
			})
			$lei['account'].append(list);
		},
		CreatMenuList:function(){
            $('a[id^="lei_BTN"]').unbind('click');
			$lei['account'].append(
				'<li><div style="padding: 2px 0pt 0pt; margin: 2px 5px 0pt; border-top: 1px solid #E0E0E0;"></div></li>'+
				'<li><a onclick="return false" id="lei_BTN_ADD_ACCOUNT">'+$T('BTN_ADD_ACCOUNT')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_CLS_ACCOUNT">'+$T('BTN_CLS_ACCOUNT')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_EDIT_ACCOUNT">'+$T('BTN_EDT_ACCOUNT')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_EXPORT">'+$T('BTN_EXPORT_ACCOUNT')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_IMPORT">'+$T('BTN_IMPORT_ACCOUNT')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_SETTING">'+$T('BTN_SETTING')+'</a></li>'+
				'<li><a onclick="return false" id="lei_BTN_ABOUT">'+$T('ABOUT')+'</a></li>');
		},
		...
		LoginFacebook:function(id){
			...
			var Content = '<form method="POST" action="https://www.facebook.com/login.php?login_attempt=1" id="switch_login_form">'+
						...
						'</div></form>';
			lei.DialogBox( $T('ALT_SWITCHING') ,Content,function(){})
			$(".dialog_buttons").hide();
		...
		About:function(){
			...
			'<div style="width:420px;" id="version">'+$T('LAB_NEW_VERSION',lei_json.version) +'<br />'+ $T('LAB_NEW_UPLOAD',lei_json.time)+'</div>'+
			'<div style="width:420px;">Script Page：<a href="http://userscripts.org/scripts/show/105308">http://userscripts.org/scripts/show/105308</a><br/>'+
			'<p>'+$('#data_lei_ABout').html()+'</p></div>';
			lei.DialogBox( $T('ABOUT') ,Content,function(){ $('#div_Box').remove(); },true)
		}
	}
	$.extend(lei, lei.Methods);
	lei.AccountLoad();
	...
	TIME['load'] = window.setInterval(function(){
		if( $('#data_lei_json').size() > 0 || TIME['load_timeout'] > 10){
			lei_json =  $.parseJSON( $('#data_lei_json').html() );
			face_lang = $.parseJSON( $('#data_face_lang').html() );
			lei.LanguageChange(language)
			timeout = 0;
			$.each( lei_json['language_select'] ,function(lan,lanf){
	...
	TIME['fuid'] =  window.setInterval(function(){
		if( $('#data_Env').size() > 0 || TIME['fuid_timeout'] > 10){
            if( $('#data_Env').html() ){ 
                ...
                lei.FacebokUid( parseInt(FacebookEnv.user,10) ); 
            }
    ...
    $("#lei_BTN_ABOUT",$lei['fb_pn']).live('click',function(){ lei.About(); },false);
	$("#lei_BTN_ADD_ACCOUNT",$lei['fb_pn']).live('click',function(){
		var Content = '<table>'+
			'<tr><td>'+$T('LAB_USERNICK')+'</td><td><input id="lei-usernicks" type="text" autocomplete="off" size="20" />'+$T('LAB_NONEED')+'</td></tr>'+
			'<tr><td>'+$T('LAB_USERNAME')+'</td><td><input id="lei-usernames" type="text" autocomplete="off" size="20" /></td></tr>'+
			'<tr><td>'+$T('LAB_USERPASS')+'</td><td><input id="lei-passwords" type="password" size="20" /></td></tr>'+
		'</table>';
		lei.DialogBox( $T('BTN_ADD_ACCOUNT') ,Content,function(){
			var n = $('#lei-usernicks').val();
			var u = $('#lei-usernames').val();
			var p = $('#lei-passwords').val();
			if (u != "" && p != ""){
	...
	$("#lei_BTN_EDIT_ACCOUNT",$lei['fb_pn']).live('click',function(){
		...
		$.each(useraccount,function(index,acc){
			...
			Content += '<tr style="padding:0">'+
					'<td style="text-align:center;"><a href="javascript:void(0)" class="lei_BTN_MOVE_ACCOUNT" id="up_'+ index +'"><img src="data:image/png;base64,'+IMG['UP']+'" border="0" /></a></td>'+
					'<td style="text-align:center;"><a href="javascript:void(0)" class="lei_BTN_MOVE_ACCOUNT" id="down_'+ index +'"><img src="data:image/png;base64,'+IMG['DOWN']+'" border="0" /></a></td>'+
					...
					'<td style="text-align:center;"><a href="javascript:void(0)" class="lei_BTN_DEL_ACCOUNT" id="del_'+ index +'"><img src="data:image/png;base64,'+IMG['DEL']+'" border="0" /></a></td>'+
					'</tr>';
		})
		Content += '</tbody></table>';
		lei.DialogBox( $T('BTN_EDT_ACCOUNT') ,Content,function(){
	...
	$("a.lei_BTN_MOVE_ACCOUNT",$('#pop_content')).live('click',function(){
	...
	$("a.lei_BTN_DEL_ACCOUNT",$('#pop_content')).live('click',function(){
		var id = $(this).attr("id").replace('del_','');
		if(confirm( $T('ALT_CONFIRM', $T('ALT_DELACCOUNT')) )){
			useraccount.splice(id,1);
			$("#lei_BTN_EDIT_ACCOUNT").click();
		}
	})
	$("#lei_BTN_CLS_ACCOUNT",$lei['fb_pn']).live('click',function(){
		if(confirm( $T('ALT_CONFIRM', $T('BTN_CLS_ACCOUNT') ) ) ){
			useraccount = new Array();
			lei.AccountLoad();
		}
	},false)
	$("#lei_BTN_SETTING",$lei['fb_pn']).live('click',function(){
		var Content = '<table>'+
			'<tr><td>'+$T('LAB_UPDATE')+'</td><td><input type="checkbox" id="lei_CHK_UPDATE" /></td></tr>'+
			'<tr><td>'+$T('LAB_LOGIN_HOME')+'</td><td><input type="checkbox" id="lei_CHK_LOGIN_HOME" /></td></tr>'+
			'<tr><td>'+$T('LAB_LOGIN_AUTO')+'</td><td><input type="checkbox" id="lei_CHK_LOGIN_AUTO" /></td></tr>'+
			'<tr><td>'+$T('LAB_LOCK_PASS')+'</td><td><input type="hidden" id="lei_IPT_LOCK_PASS" value="" /><input class="uiButton uiButtonLarge uiButtonConfirm" type="button" id="BTN_SET_PASS" value="'+$T('BTN_LOCK_PASS')+'" /></td></tr>'+
			'<tr><td width="100">'+$T('LAB_LANGUAGE')+'</td><td><select id="lei_SEL_LANGUAGE">'+language_select+'</select></td></tr>'+
            '<tr><td>'+$T('LAB_USE_CUSTOM_TITLE')+'</td><td><input type="checkbox" id="lei_CHK_USE_CUSTOM_TITLE" /></td></tr>'+
            '<tr><td>'+$T('LAB_CUSTOM_TITLE')+'</td><td><input type="text" id="lei_IPT_CUSTOM_TITLE" value="" /></td></tr>'+
            '<tr><td>'+$T('LAB_SHOW_NAME')+'</td><td><input type="checkbox" id="lei_CHK_SHOW_USER_NAME" /></td></tr>'+
		'</table>';
		lei.DialogBox( $T('BTN_SETTING') ,Content,function(){
			if(! $("#lei_CHK_USE_CUSTOM_TITLE").attr("checked")  ){
				$("#lei_IPT_CUSTOM_TITLE" ).val( $T('TITLE') );
			}
			$.each(set_var, function(i,x){
				var key = x.toUpperCase();
				switch (key.substr(0,3)){
					case "SEL": setting[x] = $ById('lei_'+key).value;break;
					case "CHK":	setting[x] = $("#lei_"+key ).attr("checked");break;
					case "IPT": setting[x] = $("#lei_"+key ).val();break;
				}
			})
			setValue('setting', $.toJSON(setting) );
			lei.LanguageChange( setting.sel_language);
			$('#div_Box').remove();
		})
		$.each(setting, function(key,val){
			var key = key.toUpperCase();
			switch (key.substr(0,3)){
				case "SEL": $ById('lei_'+key).value = val;break;
				case "CHK":	$("#lei_"+key ).attr("checked",val);break;
				case "IPT": $("#lei_"+key ).val(val);break;
			}
		})
		$('#lei_SEL_LANGUAGE').val(language);
		$("#BTN_SET_PASS").click(function(){
			if( lei.LockPassCheck() ){
				...
				if(PASS1 != null){
					...
					if(PASS1 == PASS2){
						...
						$("#lei_IPT_LOCK_PASS").val(setting.ipt_lock_pass);
						setValue('setting', $.toJSON(setting) );
	...
	$("#lei_BTN_EXPORT",$lei['fb_pn']).live('click',function(){
		if( lei.LockPassCheck() ){
			var Content = $T('LAB_EXPORT') +'<br/><textarea cols="70" rows="7">'+ $.toJSON(useraccount) +'</textarea>';
			lei.DialogBox( $T('BTN_EXPORT_ACCOUNT') ,Content,function(){
				$('#div_Box').remove();
			},true)
		}
	})
	$("#lei_BTN_IMPORT",$lei['fb_pn']).live('click',function(){
		if( lei.LockPassCheck() ){
			var Content =  $T('LAB_IMPORT') +'<br/><input type="text" id="lei_IPT_IMPORT" size="40" />';
			lei.DialogBox( $T('BTN_IMPORT_ACCOUNT') ,Content,function(){
				var test =  $.parseJSON( $('#lei_IPT_IMPORT').val())
				if( test != null){
					useraccount = test;
					lei.AccountLoad();
				}
	...
	$("a.lei_BTN_LOGIN_ACCOUNT",$lei['fb_pn']).live('click',function(){
		var id = $(this).attr('id').replace('user_','');
		lei.LoginFacebook(id);
	});

}
function checkUpdate(){
	if(setting.chk_update && parseFloat(version)*10 < parseFloat(lei_json.version)*10 ){
		var Content = $T('LAB_NEW_UPDATA')+'<br><a href="http://userscripts.org/scripts/show/105308">http://userscripts.org/scripts/show/105308</a>';
		lei.DialogBox( $T('LAB_NEW_VERSION', $T('LAB_VERSION',lei_json.version)) ,Content,function(){
			window.top.location.href = 'http://userscripts.org/scripts/show/105308';
		})
	}
}
