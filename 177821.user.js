...
// @name		2HDD
// @version		2013.09.15
// @description		Easy videos downloads
// @author		2HDD
// @namespace		http://userscripts.org/users/532224
// @downloadURL		https://userscripts.org/scripts/source/177821.user.js
// @updateURL		https://userscripts.org/scripts/source/177821.meta.js
	...
	if (ytVideoCode) {
	  ...
	  if (myVideoCode) {
	    ...
	    if (ytVideo.match(/&sig=/)) ytVideo = ytVideo.replace (/&sig=/, '&signature=');
	      ...
	      if (ytSig) {
		var s = ytSig[1].split('');
		s = s[40] + s.slice(44, 83).reverse().join('') + s[22] + s[42] + s[41] + s[83] + s.slice(23, 40).reverse().join('') +
		      s[0] + s.slice(6, 22).reverse().join('') +s[2] + s[1] + s[3];
      ...
      if (ytVideoFound) {
	/* Create Saver */
	var ytDefaultVideo = 'Low Definition MP4aaa';
...
// =====DailyMotion===== //
  ...
  if (!dmPlayerWindow) {
    ...
    if (dmVideosContent) {
	  ...
	  'stream_h264_hq_url': 'Standard Definition MP4', 'stream_h264_url': 'Low Definition MP4aaa', 'stream_h264_ld_url': 'Very Low Definition MP4aaa'};
      var dmVideoList = {};
	...
	if (dmVideo) {
      ...
      if (dmVideoFound) {
	/* Create Saver */
	var dmDefaultVideo = 'Low Definition MP4aaa';
...
// =====Vimeo===== //
  ...
  if (!viPlayerWindow) {
    ...
    /* Get Videos */
    if (viVideosContent) {
      var viVideoFormats = {'hd': 'High Definition MP4', 'sd': 'Low Definition MP4aaa', 'mobile': 'Very Low Definition MP4'};
      var viVideoList = {};
	...
	for (var viVideoCode in viVideoFormats) {
      ...
      if (viVideoFound) {
	/* Create Saver */
	var viDefaultVideo = 'Low Definition MP4aaa';
...
// =====MetaCafe===== //
  ...
  if (!mcPlayerWindow) {
	...
	var mcVideoFormats = {'highDefinitionMP4': 'High Definition MP4', 'MP4': 'Low Definition MP4aaa', 'flv': 'Low Definition FLV'};
	...
	mcVideoList['Low Definition MP4aaa'] = mcVideoH5;
	mcVideoFound = true;
	...
	feature['container'] = false;
      ...
      if (mcVideoFound) {
	/* Create Saver */
	var mcDefaultVideo = (mcVideoList['Low Definition MP4aaa']) ? 'Low Definition MP4aaa' : 'Low Definition FLV';
...
// =====Break===== //
  ...
  if (!brPlayerWindow) {
	...
	if (brVideoPath.match(/.wmv$/)) {
	  brVideoFormats = {'.flv': 'Low Definition FLV', '.mp4': 'Low Definition MP4aaa'};
	  brVideoPath = brVideoPath.replace(/.wmv$/, '');
	}
	else if (brVideoPath.match(/.flv$/)) {
	  brVideoFormats = {'6.mp4': 'Very Low Definition MP4', '1.flv': 'Low Definition FLV', '1.mp4': 'Low Definition MP4aaa', '2.mp4': 'Standard Definition MP4', '3.mp4': 'High Definition MP4'};
	  brVideoPath = brVideoPath.replace(/1.flv$/, '');
	}
	else if (brVideoPath.match(/.mp4$/)) {
	  brVideoFormats = {'240.mp4': 'Very Low Definition MP4aaa', '360.mp4': 'Low Definition MP4aaa', '480.mp4': 'Standard Definition MP4', '720.mp4': 'High Definition MP4',};
	...
	for (var brVideoCode in brVideoFormats) {
      ...
      if (brVideoFound) {
	/* Create Saver */
	var brDefaultVideo = 'Low Definition MP4aaa';
...
// =====FunnyOrDie===== //
  ...
  if (!fodPlayerWindow) {
    ...
    /* Get Videos */
    if (fodVideosContent) {
      var fodVideoFormats = {'2500': 'High Definition MP4', '1800': 'Standard Definition MP4', '600': 'Low Definition MP4aaa', '110': 'Very Low Definition MP4'};
      var fodVideoList = {};
	...
	for (var fodVideoCode in fodVideoFormats) {
      ...
      if (fodVideoFound) {
	/* Create Saver */
	fodDefaultVideo = 'Low Definition MP4aaa';
...
// =====Videojug===== //
  ...
  if (!vjPlayerWindow) {
    ...
    if (vjVideosContent) {
      ...
      var vjVideoFormats = {'VJ480PENG.mp4': 'Standard Definition MP4', 'VJ360PENG.mp4': 'Low Definition MP4aaa', 'PHOENG.mp4': 'Very Low Definition MP4', 'FW8ENG.flv': 'Low Definition FLV', 'FS8ENG.flv': 'Very Low Definition FLV'};
      var vjVideoList = {};
...
// =====Mevio===== //
  ...
  if (!mePlayerWindow) {
    ...
    /* Get Videos */
    if (meVideosContent) {
      var meVideoFormats = {'mp4': 'High Definition MP4', 'm4v': 'High Definition M4V',  'mov': 'High Definition MOV', 'h264': 'Low Definition MP4aaa', 'flv': 'Low Definition FLV'};
      var meVideoList = {};
...
// =====Blip===== //
  ...
  if (!blipPlayerWindow) {
    ...
    if (blipVideosContent) {
      ...
      var blipDefaultVideo = 'Low Definition MP4aaaa';
...
// =====Veoh===== //
  ...
  if (!vePlayerWindow) {
    ...
    /* Get Videos */
    if (veVideosContent) {
      var veVideoFormats = {'fullPreviewHashLowPath': 'Very Low Definition MP4', 'fullPreviewHashHighPath': 'Low Definition MP4aaa'};
      var veVideoList = {};
