// ==UserScript==
// @name		Flickr Storybook Challenge Group Admin Tool
// @namespace		http://www.flickr.com/groups/the-storybook-challenge-group/
// @description		An admin tool for The Storybook Challenge Group on Flickr - V0.1
// @version		0.3
// @identifier	
// @date		03/09/2009
// @creator		Allan Bruce (charminbayurr)
// @modified date	04/06/2011
// @modified by		Kate Goyette (i_need_my_blankie)
// 
...
// @include		http://flickr.com/groups/the-storybook-challenge-group/discuss/
// @include		http://www.flickr.com/groups_newtopic.gne?id=1205360@N21
// @include		http://*flickr.com/photos/*/*
// @include		http://*flickr.com//photos/*/*
// @include		http://www.flickr.com/groups/the-storybook-challenge-group/discuss/*/edit/
// @exclude		http://www.flickr.com/groups/the-storybook-challenge-group/discuss/*/lock/
//
// ==/UserScript==
// --------------------------------------------------------------------
//
...
(function superadmintool( ) 
{
	...
	var supertoolversion = "V0.3";
	var supertoolDBversion = "V0.2";
	...
	var challengetxtval = [ '',											// 0
				...
				'Error uploading header from the back room, please use copy paste to update.' ];	// 3
	...
	var val = [ '',															//0
		...
		'<a href="http://www.flickr.com/groups/the-storybook-challenge-group/">'
		+ '<img src="http://farm4.static.flickr.com/3543/3835904612_7f13744377_o.jpg">'
		+ '</a>'
		+ '\n'
		+ '\n<b><i>Well done! You have made it Over the River in <a href="http://www.flickr.com/groups/the-storybook-challenge-group/">The Storybook Challenge Group</a>! Please post your picture to our group pool!</i>'
		+ '\n'
		+ '\nYou won: </b> ____ (##)'
		+ '\n'
		+ '\nNow all that lies between you and Grandma&#39;s House are the Woods. You can now go ahead and&nbsp;<u>enter the Through the Woods Challenge (challenge 15)</u> with this photo. Just watch out for that Big Bad Wolf! '
		+ 'He&#39;s a doozie!',													//2

		'<a href="http://www.flickr.com/groups/the-storybook-challenge-group/">'
		+ '<img src="http://farm4.static.flickr.com/3480/3835112909_8cec71ffe3_o.jpg" >'
		+ '</a>'
		+ '\n<i><b>GEE WILLIKERS! You made it through the Woods in <a href="http://www.flickr.com/groups/the-storybook-challenge-group/">The Storybook Challenge Group</a>! Please now add your picture to the <a href="http://www.flickr.com/groups/1276576@N23/">BBQ at Grandma&#39;s House</a> group pool!</b></i>'
		+ '\n'
		+ '\nNow that you made it safely through the woods, your picture is eligible for the Monthly Beanstalk Climb-Off! If you are the chosen one, you&#39;ll be entered in the Gold Coin Contest and are one step closer to the Golden Egg!']					//3
	...
	Award = 
	{
		init: function Awardinit()
		{
			if (GM_getValue("super.postedcomment")=="true")
			{
				//alert('in set tag');
				var tagtext = "";
				if ( GM_getValue( "super.postedmedal" ) == "2" ) tagtext = "StorybookWinner"; //currently no tags set yet
				if ( GM_getValue( "super.postedmedal" ) == "3" ) tagtext = "StorybookTTWwinner";
			...
			if ( ( document.referrer.match( "the-storybook-challenge-group/discuss" ) == "the-storybook-challenge-group/discuss" ) ||
				 ( document.referrer.match( "the-storybook-challenge-group/discuss" ) == "the-storybook-challenge-group/discuss" ) ||
				 ( GM_getValue( "super.paginate" ) == "true" )	) 
			{
				...
				// Check for medals
				chcktxt = '/groups/the-storybook';
				chcktxt21 = 'You have beaten two other Medal Winners and earned:';
				chcktxt22 = 'Your photo finished second in Storybook';
				chcktxt3 = 'Your photo won unanimously in Storybook'; //newer platinum
				alerttxt  ="";
				...
				n.innerHTML = '<option value="0"></option>'
					...
					+ '<option value="2">OTR medal</option>'
					+ '<option value="3">TTW medal</option>';		
				...
				this.textarea.parentNode.insertBefore( document.createTextNode( 'Storybook Awards: ' ), this.textarea );
	...
	function loadthemelist( )
	{
		...
		GM_log( "storybook: Started loading new themelist (in background)" );

		// this is (probably) the SuperHeroes themelist
		...
		GM_xmlhttpRequest(
			{
				...
				onload:function( responseDetails ) 
				{
					...
					GM_log( "storybook: Loading new themelist complete" );
				}
			} );
	}
	...
	function loadchlgheader( )
	{
		...
		GM_log( "storybook: Started loading new challengeheaders (in background)" );

		// this is (probably) the SuperHeroes headerlist
		...
		onload:function( responseDetails ) 
		{
			...
			GM_log( "storybook: Loading new challengeheaders complete" );
...
GM_log( "Here - displayfulldata" );
	...
	{
		GM_log( "storybook: init data started" );
...
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "super.lastwinner" ) ) );
		...
		GM_log( "storybook: init data complete" );
	...
	addsuperheader = function addsuperheader( )
	{
		...
		if ( unsafeWindow.global_name == 'KVangeel' || unsafeWindow.global_name == 'charminbayurr' )
			...
			tds[ 1 ].innerHTML = "storybook Admin tool " + supertoolversion + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + tds[1].innerHTML;
	}
	...
	function ProcessMainDoc( )
	{
		...
		myanchor = thisdocument.createElement( 'a' );
		myanchor.innerHTML = "Challenge-Info";
		myanchor.href = "#";
		myanchor.setAttribute( 'onClick', 'return false;' );
		...
		if ( thislocation.href.match( "the-storybook-challenge-group/discuss/page" ) != "the-storybook-challenge-group/discuss/page" )
		{
			var activethemes = "";
			for ( i = 1; i < 13; i++ )
			{
...
GM_log( "Here - ProcessDetailDoc" );
		...
		for ( i = 2; i < tds.length - 1 + endofbrackets; i++)
		{
			...
			if ( txt.match( "says" ) == "says" )
			{
				...
				if ( ( tdnr != i ) && ( ( photonumber == 3 ) || ( status.match( "VOTE" ) == "VOTE" ) || inbrackets ) )
				{
			...
			if ( medalform.elements[ i ].checked )
			{
				if ( medalform.elements[ i ].name == "lastwinner" ) 
				{
					if ( winner == " " )
						...
						alert( "You entered more than 1 winner. Please check." );
				}
			}
		}
...
//				newtxt = newtxt + "/" + lastsecond2;
	...
	Editchallengeinline = function Editchallengeinline( link,e ) //initially taken from inline forum edit post by Steeev
	{
		...
		if ( mode == 'firstpost' )
		{
			...
			if ( ( titlearea.match( "VOTE" ) == "VOTE" ) && ( lastwinner != " " ) && !supermotm )
			{
				newtxt = "AWARDED (" + lastwinner;
	...
	runsuperTool = function runsuperTool( )
	{
		...
		else if ( thislocation == "http://www.flickr.com/groups_newtopic.gne?id=1205360@N21" )
		{
			// Createnewchallenge( ); skip: themelist is fetched from superheroes backroom
		}
		...
		GM_registerMenuCommand( "Initialise storybook Admin data (themes, headers, ...)", initdatafrommenu );
	}
