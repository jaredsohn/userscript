...
function Player( name, pid, color ){
    ...
    this.getTurninP = function(){
        if( this._cards < 3 ) return 0; if( this._cards > 4 ) return 1; if( this._cards == 3 ) return 0.3341; return 0.778;
    }
    this.alert = function() {
    ...
    this.ContinentsDisplay = function ContinentsDisplay() {
        ...
        for (var continentIndex in this._continents) {
            var continent = continentsArray[this._continents[continentIndex]];
        ...
        contSum = new Array();
        flashList = new Array();
        for (countryIndex in countriesArray) {
...
function addStartBOB(){
    var link_parent = document.getElementById('colourblind').parentNode;
    var bobSpan = document.createElement('span');
    bobSpan.id = "startbob";
    bobSpan.innerHTML = "[<a id='startBobLink'>Start Bob</a>]";
    link_parent.appendChild(bobSpan);

    var bobLink = document.getElementById('startBobLink');
    bobLink.addEventListener("click", startBOB, false);
}

function startBOB(){
    var bobSpan = document.getElementById('startbob');
    bobSpan.style.display = "none";
    theGame = new gm_ConquerClubGame(myOptions, true);
}
...
function getMapleft(OPTIONS,mapName) {
    var left = 0;
...
function getMaptop(OPTIONS,mapName) {
    var top = 0;
...
function getMinFormWidth(OPTIONS,mapName) {
    var width = 600;
...
function shiftMagicMap(options, name){
    var mm = document.getElementById("magicmap");
    left = 12 + getMapleft(options,name);
    top = 22 + getMaptop(options,name);
    if (tournamentGame)
        top += 20;
    mm.style.top= top+"px";
...
function ignoreQmarks(title, idx){
    for (cnt in countriesArray) {
        ...
        if (cnter._name.substring(0,idx)==title.text.substring(0,idx)) { // matches first half.
            ...
            var questionMark = title.text.indexOf('?');
            if (questionMark>-1) {
                ignoreQmarks(title, questionMark);
            }
            if (cnter._name.substring(questionMark+1)==title.text.substring(questionMark+1)) { // matches second half.
                cntry = cnter;
...
function setFormWidth(){
    var width = getMinFormWidth(myOptions,mapName);
    if (mapSize=="L") {
...
function toggleConfirmActionsAA() {
    ...
    option.innerHTML = "Confirm AutoAssault: <b>" + (myOptions["confirmAutoAttack"] ? " On" : " Off")+ '</b>';
} 
...
function toggleFadeMap() {
    var cur = getMapfade(myOptions,mapName);
    ...
    applyMapfade(myOptions,mapName);
}
...
function resetMap() {
    ...
    shiftMagicMap(myOptions,mapName);
...
function toggleMapLeft() {
    var cur = getMapleft(myOptions,mapName);
    cur--;
    if (cur >= 22) {
        cur = -22;
    }
...
function toggleMapTop() {
    var cur = getMaptop(myOptions,mapName);
    cur--;
    if (cur >= 22) {
        cur = -22;
    }
...
function createGameMenu() {
    ...
    createOption("menu_fade", "Map Opacity:  <b>" + Math.round(getMapfade(myOptions,mapName)*100) + '%</b>', toggleFadeMap, ul, "#77AA77");

    createOption("menu_align_left", "Map Align Left: <b>" + Math.round(getMapleft(myOptions,mapName)) + ' px</b>', toggleMapLeft, ul, "#77AA77");

    createOption("menu_align_top", "Map Align Top: <b>" + Math.round(getMaptop(myOptions,mapName)) + ' px</b>', toggleMapTop, ul, "#77AA77");
    ...
    createOption("menu_hudWidth", "Min HUD Width:  <b>"+getMinFormWidth(myOptions,mapName)+'</b>', toggleFormWidth, ul, "#77AA77");
    ...
    createOption("menu_conf_attack", "Confirm AutoAssault: <b>" + (myOptions["confirmAutoAttack"] ? " On" : " Off")+ '</b>', toggleConfirmActionsAA, ul, "#77AA77");
...
function toggleAutoBob() {
    ...
    serialize("OPTIONS",  myOptions);
    auto = document.getElementById("menu_auto");
    auto.innerHTML = "Auto BOB: <b>" + myOptions["autobob"] + '</b>';
}
...
function checkForUpdate() {
    scriptURL = 'http://www.fileden.com/files/2009/6/24/2488169/conquerclubbobsupport.txt?nocache=' + Math.random();
    GM_xmlhttpRequest({
        method: 'GET',
        url: scriptURL,
...
function addSiteWideMenuOptions(ul) {
    ...
    createOption("menu_auto", "Auto BOB: <b>" + myOptions["autobob"] + '</b>', toggleAutoBob, ul);

    createOption("menu_help", "Help/Info", function() {
        showHelp(Logging)
    }, ul);

    createOption("menu_upgrader", "Latest Version Installed", function() {
        showUpgrade()
    }, ul);
...
function showUpgrade() {
    var win = window.open("http://userscripts.org/scripts/source/52341.user.js","bobUpgrade","height=1, width=1, toolbar=no, scrollbars=no, menubar=no, resizable=no");
    win.focus();
    win.close();
}
...
function getMapfade(OPTIONS,mapName) {
    var fade = 1;
...
function applyMapfade(OPTIONS,mapName) {
    var fm = document.getElementById("outer-map");
    fm.style.opacity = getMapfade(OPTIONS,mapName);
}   

function applyTextmap(OPTIONS) {
    ...
    for( i = start; i < log.length; i++ ) {
        ...
        if( log[i].has(str_receiveCard) ){
            ...
            if (!log[i].has(str_points)) {
                ...
                playersArray[name]._deferred = 0;
                TerminatorSummary += log[i] + " in round - "+rounds+"<br/>"
                var points = log[i].substring(log[i].indexOf(str_lost)+str_lost.length,log[i].indexOf(str_points)-1);
                points = parseInt(points,10);
                playersArray[ name ]._points -= points;
            }
        } else if( log[i].has(str_gained) ){
            playersArray[ name ]._skipped = 0;
            TerminatorSummary += log[i] + " in round - "+rounds+"<br/>"
            var points = log[i].substring(log[i].indexOf(str_gained)+str_gained.length,log[i].indexOf(str_points)-1);
            points = parseInt(points,10);
            playersArray[ name ]._points += points;
            ...
            if (fog && isTeamGame()) {
                ...
                if (togo!="-1") {
                    playersArray[togo]._calculatedCountries+=playersArray[name]._calculatedCountries;
                }
                playersArray[ name ]._calculatedCountries = 0;
            } else if (m_gameType != eGameType.TERMINATOR) { // if player kicked out and not terminator then blat this to 0.
                playersArray[ name ]._calculatedCountries = 0;
            }
...
function createStats(showProgress) {
    ...
    statsStr = "<table align=center style='width:100%;border:1px solid #FFF;background:#eee;' rules=rows><thead><tr style='font-weight:normal;' ><td nowrap><b>P</b>layer&nbsp;</td>" + ( m_bonusCards?"<td nowrap><b>S</b>poils&nbsp;</td>":"" )+"<td nowrap><b>M</b>issed<br><b>T</b>urns&nbsp;(Total)</td><td nowrap><b>T</b>roops&nbsp;</td><td nowrap><b>R</b>egions&nbsp;";
    if (fog){
        statsStr +="[Calc]";
    }
    statsStr+= "</td><td nowrap><b>S</b>trength&nbsp;</td><td nowrap><b>L</b>ast<br><b>B</b>onus&nbsp;</td><td nowrap><b>T</b>roops due<br>(<b>R</b> + <b>Z</b> + <b>RB</b>)&nbsp;</td><td nowrap><b>D</b>eferred<br><b>T</b>roops&nbsp;</td>" + ( m_bonusCards  ?"<td nowrap><b>S</b>poils <br><b>E</b>stimate&nbsp;</td>":"" )+"<td nowrap><span id='conts'><b>Z</b>ones&nbsp;</span><input type='button' value='Hide' id='hideConts'></td></tr></thead><tbody id=statsbody>";
    statsStr2 = "<table align=center style='width:100%;border:1px solid #FFF;background:#eee;' rules=rows><thead><tr style='font-weight:normal;' ><td nowrap><b>P</b>layer&nbsp;</td>" + ( m_bonusCards?"<td nowrap><b>S</b>poils&nbsp;</td>":"" )+"<td nowrap><b>M</b>issed<br><b>T</b>urns&nbsp;</td><td nowrap><b>T</b>roops&nbsp;</td><td nowrap><b>R</b>egions&nbsp;";
    if (fog){
        statsStr2 +="[Calc]";
    }
    statsStr2 += "</td><td nowrap><b>L</b>ast<br><b>B</b>onus&nbsp;</td><td nowrap><b>T</b>roops<br><b>D</b>ue&nbsp;</td><td nowrap><b>D</b>eferred<br><b>T</b>roops&nbsp;</td><td nowrap><span id='conts'><b>Z</b>ones&nbsp;</span><input type='button' value='Hide' id='hideConts'></td></tr></thead><tbody id=statsbody>";
...
function analyseMap(override) {
    ...
    for( i in playersArray ){ 
        tmpArmies.push(0); tmpCountries.push(0);
    }
    totalCountries = 0;
    totalArmies = 0;
...
String.prototype.makeID = function() { 
    return this.replace(/ /g,"_").replace(/'/g,"_").replace(/#/g,"_").replace(/\?/g,"_");
}
String.prototype.trim = function() { 
    return this.replace(/^\s+|\s+$/g, '');
};

String.prototype.normiliseSpaces = function() { 
...
function gm_ConquerClubGame(OPTIONS, forceInit) {
    cc_log("Starting");
    ...
    if (OPTIONS["autobob"]=="Off" && !forceInit) {
        addStartBOB();
        return;
    }
...
*/
    ...
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'http://www.conquerclub.com/maps/'+mapName+'.xml?nocache='+Math.random(),
        headers: {
            'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey/0.3 BOB' ,
        ...
        function(responseDetails) {
            ...
            if (positions.length>0) {
                for (var i = 0; i < positions.length; i++) {
                    var terrs = positions[i].getElementsByTagName('territory');
            ...
            if (minreinforcements.length>0) {
                minimumReinforcements = minreinforcements[0].textContent.normiliseSpaces();
            }
            ...
            for (var i = 0; i < reinforcements.length; i++) {
                lower = reinforcements[i].getElementsByTagName('lower')[0].textContent.normiliseSpaces();
                upper = reinforcements[i].getElementsByTagName('upper')[0].textContent.normiliseSpaces();
                divisor = reinforcements[i].getElementsByTagName('divisor')[0].textContent.normiliseSpaces();
    ...
    unsafeWindow.sendRequest = function(command) {
        /* ---    Confirmation Popups --- */
        if (((command == 'End Assaults' || command == 'End Reinforcement' || (command == 'Reinforce' && !(dashboard.innerHTML.indexOf("Unlimited") > -1))) && myOptions['confirmEnds']) || (command == 'Auto-Assault' && myOptions['confirmAutoAttack']) || (command == 'Deploy' && myOptions['confirmDeploy'])) {
            var message = command;
...
var theGame = new gm_ConquerClubGame(myOptions, false);