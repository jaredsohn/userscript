...
function sup_page_topics(idRecherche) {
	...
	if (_Iaurent78_)
	{
		...
		document.getElementById('menu_actions').innerHTML = document.getElementById('menu_actions').innerHTML.replace(/<\/ul>/, "<li><a href=\"#\" onclick=\"var root_confirmation = confirm('Etes vous sûr(e) de vouloir effacer TOUTE votre sélection ?'); for (var i = 1; document.getElementById('msg_' + i) && root_confirmation; i++) { if (document.getElementById('msg_' + i).checked) {window.open(document.getElementById('msg_' + i).value, 'popup_' + i)}} return false;\">Effacer (s&eacute;lection)</a></li>\r\n<li><a href=\"#\" onclick=\"var root_confirmation = confirm('Etes vous sûr(e) de vouloir effacer la TOTALITÉ des topics/messages de cette page ?'); for (var i = 1; document.getElementById('msg_' + i) && root_confirmation; i++) {window.open(document.getElementById('msg_' + i).value, 'popup_' + i)} return false;\">Effacer (page)</a></li>\r\n</ul>");	
	}
	else
	{
		document.getElementById('menu_interactif').innerHTML = document.getElementById('menu_interactif').innerHTML.replace(/<\/ul>(\r?\n?)$/, "</ul>$1</div>\r\n<div id=\"menu_actions\">\r\n<h3 style=\"background: url(http://jvcprono.free.fr/root/img/t_sommaire_actions.gif);\"><span>Actions</span></h3>\r\n<ul>\r\n<li><a href=\"#\" onclick=\"for (var i = 1; document.getElementById('msg_' + i); i++) {document.getElementById('msg_' + i).checked = true;} return false;\">Tout cocher</a></li>\r\n<li><a href=\"#\" onclick=\"for (var i = 1; document.getElementById('msg_' + i); i++) {document.getElementById('msg_' + i).checked = false;} return false;\">Tout d&eacute;cocher</a></li>\r\n</ul>\r\n");
		document.getElementById('menu_actions').innerHTML = document.getElementById('menu_actions').innerHTML.replace(/<\/ul>/, "<li><a href=\"#\" onclick=\"var root_confirmation = confirm('Etes vous sûr(e) de vouloir effacer TOUTE votre sélection ?'); for (var i = 1; document.getElementById('msg_' + i) && root_confirmation; i++) { if (document.getElementById('msg_' + i).checked) {window.open(document.getElementById('msg_' + i).value, 'popup_' + i)}} return false;\">Effacer (s&eacute;lection)</a></li>\r\n<li><a href=\"#\" onclick=\"var root_confirmation = confirm('Etes vous sûr(e) de vouloir effacer la TOTALITÉ des topics de cette page ?'); for (var i = 1; document.getElementById('msg_' + i) && root_confirmation; i++) {window.open(document.getElementById('msg_' + i).value, 'popup_' + i)} return false;\">Effacer (page)</a></li>\r\n</ul>");
	}
...
function init() {
	...
	else
	{
		...
		if (lineMsgs[9].getElementsByTagName('a')[1].className == 'bloquer')
		{
			sup_page_messages();
		}
		else if (lineMsgs[9].getElementsByTagName('a')[0].className == 'debloquer')
		{
			sup_page_messages();
		}
	}
