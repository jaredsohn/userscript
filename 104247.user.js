...
var WoDTool = {
	...
	getAttr : function ( context ) {
		...
		WoDTool.walker( 'table.content_table td', function ( td ) {
			if ( td.innerHTML.sindexOf( ['英雄的等级', '体力', '生命值再生', '法力', '法力回复', '每回合的行动次数', '先攻权'] ) != -1 ) {
				var attr = [];
				if ( td.childNodes[1] ) {
				...
				if ( attr[0].indexOf( '法力回复' ) == -1 ) {
					attr[2] = td.parentNode.childNodes[3].innerHTML.txt();
				} else {
					attr[2] = td.innerHTML.txt().substr( td.innerHTML.txt().indexOf( '法力回复' ) + 5 );
				}
