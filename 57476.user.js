...
//2.0 (9/10/2009) - Fixed abandoned script to work again
//
// ==UserScript==
// @name          Amazon TotalUsed 2.0
// @namespace     http://www.tapuri.org
// @description   Add shipping cost to used Amazon products in order to get accurate pricing. Version 2.0
...
function getShippingCost() {
	...
	var temp=xpath('//div[@id="navCategoryBtn"]');
...
function insertA(allDivs,choice)
{
	for (var i = 0; i < allDivs.snapshotLength; i++)
	{
		...
		for(tempLocation = 0; tempLocation < temp.length; tempLocation++)
		{
			if(temp[tempLocation].indexOf('condition=used') > 0)
			{
				tempLocation++;
				temp[tempLocation]=temp[tempLocation].split('<');
				totalCost=getTotalCost(finalPrice,temp[tempLocation][0]);
				break;
			}
		}
		...
		if((choice==0) && (totalCost > 0))
		{
			...
			if(mainPrice.snapshotItem(0)!=null && mainPrice.snapshotItem(0).innerHTML.indexOf('$')>=0)
			{
				...
				{
					temp[tempLocation][0]=temp[tempLocation][0]+' + <i>'+finalPrice+'</i> = <b>$'+totalCost+'</b> ('+percentageChange+'% savings)<';
				}
				else //if the used price+shipping is more expensive than the new price
				{
					percentageChange*=-1;
					temp[tempLocation][0]=temp[tempLocation][0]+' + <i>'+finalPrice+'</i> = <b>$'+totalCost+'</b> ('+percentageChange+'% extra)<';
				}
			}
			else
			{
				temp[tempLocation][0]=temp[tempLocation][0]+' + <i>'+finalPrice+'</i> = <b>$'+totalCost+'</b><';
			}
			
			var strNewThisDiv = ""
			for(var index = 0; index < tempLocation; index++)
			{
				strNewThisDiv += temp[index]
			}
			
			strNewThisDiv += temp[tempLocation][0]+temp[tempLocation][1]
			
			thisDiv.innerHTML=strNewThisDiv;
		}
		//Small space prevents us from showing the full calculation on the sidebar
		else if(choice==1)
		{
			temp[tempLocation][0]=temp[tempLocation - 1]+'$'+totalCost+' (inc. shipping)<'+temp[tempLocation][1];
			thisDiv.innerHTML=temp[tempLocation][0];
		}
	}
}