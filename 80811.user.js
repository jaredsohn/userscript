...
var onlyAtCamp = true;           // Play turn only if user is at Camp
...
function checkPage() {
	if(document.getElementById('app377144924760_playCounter') == null) {
		if(stagnantTime < stagnantThreshold) {
			...
			setTimeout('window.location = "http://apps.facebook.com/levynlight/";', 1000*60*10);
			// Show time
			var time = new Date();
		...
		if(onlyAtCamp && !isAtCamp()) {
				setTimeout('window.location = "http://apps.facebook.com/levynlight/";', 1000*60*15);
				// Show time
				var time = new Date();
				var time_hour = time.getHours();
