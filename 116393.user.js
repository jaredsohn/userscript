...
javascript:(function(){
	...
	var Diamonds = ['Hope Diamond','Koh-I-Noor Diamond','Great Star of Africa Diamond','Pair of The Orloff Diamond','Pair of The Sancy Diamond','Pair of The Idol\'s Eye Diamond','Pair of The Regent Diamond'];
	var Precious = ['Aquamarine','Brazilian Emerald','Gold Nugget','Imperial Topaz','Pink Diamond','Raw Diamond','Tourmaline'];
	...
	function update_loot_log() {
		...
		for (x in looted) {
			...
			lootoutput += '<span class="good">&times;'+looted[x].quantity+'</span> '+looted[x].name+' '+attack+' '+defense+' <span class="more_in">('+parseFloat(looted[x].quantity/stats.lootcount*100).toFixed(1)+'%)</span><br />';
		}
		$('#'+spocklet+'_loot_stats').html('Loot stats: '+stats.lootcount+'/'+stats.robs_done+' <span class="more_in">('+parseFloat(stats.lootcount/stats.robs_done*100).toFixed(1)+'%)</span>');
					...
					if (/index_controller/.test(response)) {
						...
						if (Util.isset(object.slot)) {
							...
							if (Util.isset(object.popup)) {
								...
								//look for diamonds, they are forever
								$(object.popup).find('img').each(function(){
									if (Diamonds.indexOf($(this).attr('alt')) > -1) {
										var lootname = '<span class="good">Diamond!</span> '+$(this).attr('alt');
										add_loot(false,lootname,quantity);
										loot += lootname+' ';
									}
									if (Precious.indexOf($(this).attr('alt')) > -1) {
										var lootname = '<span class="good">Precious!</span> '+$(this).attr('alt');
										add_loot(false,lootname,quantity);
										loot += lootname+' ';
									}
								});
							...
							//look for diamonds, they are forever
							$(robResult).find('img').each(function(){
								if (Diamonds.indexOf($(this).attr('alt')) > -1) {
									var lootname = '<span class="good">Diamond!</span> '+$(this).attr('alt');
									add_loot(false,lootname,quantity);
									loot += lootname+' ';
	...
	try {
		...
		pageTracker._trackPageview("/script/RobberBG");
	}
	catch(err) {}
	//end analytics
})()