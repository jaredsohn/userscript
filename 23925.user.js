...
// @version        2.3.0
// @namespace      QP
// @description    Simplifies lots of Travian tasks - v2.3.0
...
/** Presentation info
----- Small description -----
<i>Quick</i> Scout / Fake / Hero / Farm<br>Delayed Send Troops<br><i>Plus</i> features<br>Message Autolinks<br><i>Enhanced</i> Rally Point / MarketPlace / Inner Village<br>1-click Delete All Trade Msgs<br>..<br>All langs & servers
----- Full description -----
<h3>Features</h3>
...
<ul>
	...
	<li>Auto delete all trade messages - 1 click deletes all trade messages</li>
</ul>
...
<ul>
	...
	<li>Features to be added
		<ul>
			...
			<li>Allow all kinds of automatic report deletion (reinfs, misc, ...)</li>
...
function main() {
	...
	if (isThisPageAnyIGMPage()) {
		...
		if (isThisPageAnyReportList()) {		debug(DBG_HIGHEST, "[-][isPageAnyReportList]");
			...
			if (isToDeleteTradeReports()) {
				actionPageReportsTradeList_deleteAllTradeReports();
			}
			transformPageReportList_addDeleteTradeButton();
			transformGeneric_addAction_spaceShortcutKeyGoesToNextPage();
		}
...
// 1 - appends title of current page; 2 - shortens real title 1st word to 1st letter (eg.: "Travian" -> "T")
var DEF_CONFIG_TITLEFIX = 2;
/** transformGenericPage_fixTitle */
function transformGenericPage_fixTitle() {
	if ((DEF_CONFIG_TITLEFIX != 1) && (DEF_CONFIG_TITLEFIX != 2)) { return; }
...
function transformPageWWStatistics_addCoordsForWWVillages() {
	...
	var wwVillagesLinks = xpathEvaluate('//div[@id="lmid2"]/table/tbody/tr/td[@class="s7"]/a');
...
/** isTravianTeamMessagePage */
function isTravianTeamMessagePage() {
...
function createPermanentKeyForArrivingMerchantsTitle() {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + DEF_PARTIALPERMANENTMKEY_MERCHANTSTITLE_ARRIVING;
}
...
function createPermanentKeyForOwnMerchantsTitle() {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + DEF_PARTIALPERMANENTMKEY_MERCHANTSTITLE_OWNMERCHANTS;
}
...
function addListenerMarketplaceSendResources_updateResourceQuatity(currRowNode, parentNode, times, quantity, inner) {
	...
	link.addEventListener('click',	function() {
		debug(DBG_HIGHEST, "[addListenerMarketplaceSendResources_updateResourceQuatity] currRowNode.innerHTML " + currRowNode.innerHTML);
		if (times == 0) {
			resInput.value = '';
		} else {
			var resNewValue = resInput.value;
			debug(DBG_HIGHEST, "[addListenerMarketplaceSendResources_updateResourceQuatity] resNewValue " + resNewValue);
			resNewValue = (isNaN(parseInt(resNewValue))) ? 0 : parseInt(resNewValue);
			debug(DBG_HIGHEST, "[addListenerMarketplaceSendResources_updateResourceQuatity] resNewValue " + resNewValue);
			resInput.value = resNewValue + (times * parseInt(quantity));
			debug(DBG_HIGHEST, "[addListenerMarketplaceSendResources_updateResourceQuatity] times " + times + " quantity " + quantity);
		}
		updateMerchantsUsed();
	}, true);
	return link;
...
function updateMerchantsUsed() {
	...
	debug(DBG_NORMAL, "[transformPageMarketplaceSendResources_addMerchantsUsed][updateMerchantsUsed] ");
...
//			debug(DBG_NORMAL, "[transformPageMarketplaceSendResources_addMerchantsUsed][updateMerchantsUsed] if lastMercExcessLoad " + lastMercExcessLoad);
		txt.innerHTML = totResources +" = "+ mercs + ((lastMercExcessLoad==0) ? " (0)" : (" (+"+ lastMercExcessLoad +")"));
		txt.style.color = "red";
	} else {
//			debug(DBG_NORMAL, "[transformPageMarketplaceSendResources_addMerchantsUsed][updateMerchantsUsed] else lastMercAvailableLoad " + lastMercAvailableLoad);
...
function loadPermanentMyTribe() {
	var myTribe = GM_getValue(DEF_PARTIALPERMANENTMKEY_PREFIX + "myTribe");
	return myTribe;
}
function savePermanentMyTribe() {
	var tribe = getTribeBySendTroopsPage();
	GM_setValue(DEF_PARTIALPERMANENTMKEY_PREFIX + "myTribe", tribe);
}
...
function createPermanentKeyForSingleVillageInfo() {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + DEF_PARTIALPERMANENTMKEY_SINGLEVILLAGEINFO;
}
...
function resetPermanentVillageReportInfo(villageId) {
	savePermanentReportsAction(villageId, "");
}
...
function createPermanentKeyForVillageReportInfo(villageId) {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + villageId + DEF_PARTIALPERMANENTMKEY_VILLAGEREPORTINFO;
}
...
function transformGeneric_findTargetsToCreateTargetLinks() {
	...
	for(var i=0; i<pageSendTroopsNodeA.snapshotLength; i++) {
		debug(DBG_LOW, "[transformGeneric_findTargetsToCreateTargetLinks] pageSendTroopsNodeA - i " + i);
		debug(DBG_LOW, "[transformGeneric_findTargetsToCreateTargetLinks] pageSendTroopsNodeA.snapshotItem(i): " + pageSendTroopsNodeA.snapshotItem(i).href);
		transformGeneric_addListenerHeroAttack(pageSendTroopsNodeA.snapshotItem(i));
		transformGeneric_addListenerFakeAttack(pageSendTroopsNodeA.snapshotItem(i));
		transformGeneric_addListenerScoutAttack(pageSendTroopsNodeA.snapshotItem(i));
	}
	for(var i=0; i<pageVillagesNodeA.snapshotLength; i++) {
		debug(DBG_LOW, "[transformGeneric_findTargetsToCreateTargetLinks] pageVillagesNodeA - i " + i);
		debug(DBG_LOW, "[transformGeneric_findTargetsToCreateTargetLinks] pageVillagesNodeA.snapshotItem(i): " + pageVillagesNodeA.snapshotItem(i).href);
		transformGeneric_addListenerFakeAttack(pageVillagesNodeA.snapshotItem(i));
		transformGeneric_addListenerScoutAttack(pageVillagesNodeA.snapshotItem(i));
		transformGeneric_addListenerVillageReportInfo(pageVillagesNodeA.snapshotItem(i));
	}
}
...
//	var travelTime = xpathEvaluateInContext(sendTable, 'tr[4]/td[2]/table/tbody/tr/td[1]').snapshotItem(0).innerHTML;
	...
	var actionButton = createElemAppendAndSetInner("a", actionCell, "<img src='img/un/a/clock.gif'/>");
...
function createPermanentKeyForQPClipboard() {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + DEF_PARTIALPERMANENTMKEY_QP_CLIPBOARD;
}
...
* actionPageReportsTradeList_deleteAllTradeReports
*/
function actionPageReportsTradeList_deleteAllTradeReports(param) {
...
* transformPageReportList_addDeleteTradeButton
*/
function transformPageReportList_addDeleteTradeButton() {
	var deleteButton = xpathEvaluate('//div[@id="lmid2"]/form/table/tbody/tr/td/input[@class="std"][1]');
	var tradeLink = xpathEvaluate('//div[@id="lmid2"]/p[@class="txt_menue"]/a[contains(@href,"t=2")]');

	var deleteLang = deleteButton.snapshotItem(0).value;
	var tradeLang = tradeLink.snapshotItem(0).innerHTML;

	var buttonsCell = xpathEvaluate('//div[@id="lmid2"]/form/table/tbody/tr[last()]/td[@class="s7"]');

	var inputDelAllTrade = document.createElement('input');
	inputDelAllTrade.type = "button";
	inputDelAllTrade.value = deleteLang + " " + tradeLang;
	inputDelAllTrade.className = "std";

	debug(DBG_NORMAL, "[transformPageReportList_addDeleteTradeButton] adding del all listener");
	inputDelAllTrade.addEventListener('click',	function() {
		
		debug(DBG_NORMAL, "[transformPageReportList_addDeleteTradeButton] listener 1");
		savePermanentReportsAction(DEF_ACTION_DELETE_TRADE);
		debug(DBG_NORMAL, "[transformPageReportList_addDeleteTradeButton] tradeLink.snapshotItem(0).href " + tradeLink.snapshotItem(0).href);
		
		document.location.href = tradeLink.snapshotItem(0).href;
	}, true);
	buttonsCell.snapshotItem(0).appendChild(inputDelAllTrade);
}



/**
* isToDeleteTradeReports
*/
function isToDeleteTradeReports() {
	debug(DBG_NORMAL, "[isToDeleteTradeReports] loadPermanentReportsAction() " + loadPermanentReportsAction() + " DEF_ACTION_DELETE_TRADE " + DEF_ACTION_DELETE_TRADE);
	if (loadPermanentReportsAction() == DEF_ACTION_DELETE_TRADE) {
		debug(DBG_NORMAL, "[isToDeleteTradeReports] true");
		return true;
	}
	debug(DBG_NORMAL, "[isToDeleteTradeReports] false");
...
/**
* savePermanentReportsAction
*/
function savePermanentReportsAction(reportAction) {
	debug(DBG_LOW, "[savePermanentReportsAction] reportAction " + reportAction);
	var key = createPermanentKeyForReportsAction();
	debug(DBG_LOW, "[savePermanentReportsAction] key " + key );
	GM_setValue(key, reportAction);
	debug(DBG_LOW, "[savePermanentReportsAction] key " + key + " reportAction " + reportAction);
}

/**
* resetPermanentReportsAction
*/
function resetPermanentReportsAction() {
	savePermanentReportsAction("");
}

/**
* loadPermanentReportsAction
*/
function loadPermanentReportsAction() {
	var key = createPermanentKeyForReportsAction();
	return GM_getValue(key);
}

/**
* createPermanentKeyForReportsAction
* Creates a key for permanent storing reports actions.
* The key is of this format: <server>_<userId>_reportsAction
*/
function createPermanentKeyForReportsAction() {
	return DEF_PARTIALPERMANENTMKEY_PREFIX + DEF_PARTIALPERMANENTMKEY_REPORTSACTION;
}
...
function createPermanentKeyForInstantTroopsMove(destinationVillageCoordZ) {
	return DEF_PARTIALPERMANENTMKEY_PREFIX +
...
*/
function isToMoveTroopsToThisVillage(url) {
...
function isThisPageMarketPlaceSendResources() {
...
function isThisPageMarketPlaceSendResourcesConfirmation(isToSelf) {
	if (isThisPageAnyMarketPlacePage()) {
...
function isThisPageMarketPlaceBuy() {
	if (isThisPageAnyMarketPlacePage()) {
