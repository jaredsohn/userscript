...
(function() {
    var callback = function($) {
        ...
        var ajaxLoad = function(data) {
            ...
            $('.message_ajax_container').slideUp('fast', function() {
                $('.message_ajax_container').html(data);
                $('.message_ajax_container').slideDown('fast', function() {
                    if ($('.message_unread:first').length != 0) {
                        $(window).scrollTop($j('.message_unread:first').offset().top);
                    }
                });

                $('#content form').submit(function(ev) {
                    ev.preventDefault();
                ...
                $('#citizen_subject').val(query.subject);
                $('#citizen_message').val(query.content);
        ...
        if (window.location.hash) {
            var base64 = decodeURIComponent(window.location.hash.substr(1));
            var json = decodeURIComponent(escape(Base64.decode(base64)));
            var query = JSON.parse(json);
            
            $.get('/en/main/messages-compose/' + query.id, ajaxLoad);
        }
    };
