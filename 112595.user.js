...
// @description    xfdt-pro (versione 1.0-yakkomod)
// @include        http://*.forumdeitroll.it/*
// ==/UserScript==

// FDT
// Copyright (C) 2008,2009,2010 Wakko Warner, Achille/Penthotal, Sarrusofono
//               (http://thewakkowarner.netsons.org/,
//                http://www.forumdeitroll.it/,
//                http://ravanator.acmetoy.com/)
// Modified in 2009,2010 (C) Yakko Warner
//
...
// 
// Yakko Warner: I've commented Youtube-section 'cause isn't useful anymore.
// And I've also added a link for FDT-Chat with TOR mode (IRC client required).
// 

// Ed. Master e` il mio signore e padrone.


//Gestione albero della cuccagna
function toggle_albero_cuccagna() {
	...
	if (albero.style['display'] == 'none')
	  Set_Cookie('hideCuccagna', 'true');
	else
	  Set_Cookie('hideCuccagna', 'false');
}
...
function belink_albero_cuccagna() {
  ...
  if (tdcontent != null) { //Se al passo precedente l'ho trovato...
      if (tdaction != null) {
	  tdaction.innerHTML += "| <a href='javascript:void(0);' class='textfolder' id='belink_albero'>Mostra/Nascondi Albero della Cuccagna</a> | ";
	  tdaction.innerHTML += "<a href='javascript:void(0);' class='textfolder' id='fdtchat1'>Elenco utenti in chat</a> | ";
	  tdaction.innerHTML += "<a href='http://ravanator.acmetoy.com' target='_blank' class=\"textfolder\">Homepage dei Troll</a> | <a href='irc://mejokbp2brhw4omd.onion:6667/#fdt' target='_blank' class=\"textfolder\">Entra in Chat (client IRC + TOR)</a> | <a href='http://irc.freenode.org/#fdt' target='_blank' class=\"textfolder\">Entra in chat (client IRC)</a> | <a href='http://embed.mibbit.com/?server=irc.freenode.org&channel=%23%23fdt' target='_blank' class=\"textfolder\">Chat web</a> | <a href='http://ravanator.acmetoy.com/motorino/search' target='_blank' class=\"textfolder\">Motorino versione 2010 (Proott)</a> | <a href='http://3vtoga7czue3lskl.onion/search.cgi.pl' target='_blank' class=\"textfolder\">Motorino versione 2009 (TOR)</a> | <a href='http://fdt.freehostia.com/carburatore/wakaba/wakaba.html' target='_blank' class=\"textfolder\">Motorino versione 2008</a> | ";
...
//var checkFdtBox = false;
var hidePedoes = Get_Cookie('hidePedoes');
if (hidePedoes == null) {
  hidePedoes = "false";
  Set_Cookie("hidePedoes", "false");
}
var hideCuccagna = Get_Cookie('hideCuccagna');
if (hideCuccagna == null) {
  hideCuccagna = "false";
  Set_Cookie("hideCuccagna", "false");
}
var checkFdtBox = 'true' == Get_Cookie('checkFdtBox');
...
if (pageUrl.indexOf('r.aspx') != -1 || pageUrl.indexOf('p.aspx') != -1) {
  ...
  for (c=0; c<(div.length); c++) {
    if (div[c].getAttribute('class')=='emotitoli' && div[c].innerHTML=='Inserimento') {
      ...
      a.innerHTML='<img border="0" height="20" alt="Barrato" style="" src="http://ravanator.acmetoy.com/xfdt/strike.gif"/>';
...
//Colorize by wakko - mod_aqua
//TODO: 'sto metodo fa di tutto, sarà il caso di cambiare nome
function colorize() {
  //link nickname2wiki
  ...
  if (colorizeEnabled) {
     ...
     for (c=0; c<(div.length); c++) {
	  ...
	  for (var i = 0; i < div[c].childNodes.length; ++i) {
		 ...
		 var matched = href.match(/^http:\/\/(www|it)\.youtube\.com\/watch\?v=(.{11})/);
...
//	  forumPanelDiv.innerHTML += '<br/><img class="dotto" alt=" " src="images/trasp.gif"><img class="dotto" alt=" " src="images/trasp.gif"><label class="textfolderblu"><input type="checkbox" '+((hideCuccagna == 'true')?'checked="true"':'')+' id="chkCuccagna"> Nascondi Automaticamente Albero</label>';
  ...
  document.getElementById("chkProcura").addEventListener("click", function(event) {
    ...
    Set_Cookie("hidePedoes", hidePedoesStr);
...
/*  document.getElementById("chkCuccagna").addEventListener("click", function(event) {
  ...
  document.getElementById("chkFdtBox").addEventListener("click", function(event) {
    ...
    Set_Cookie("checkFdtBox", checkFdtBoxStr);
		...
		document.getElementById('quota').addEventListener('click', function(event){
			...
			GM_xmlhttpRequest({
				...
				onload: function(xhr) {
					...
					txInner += '<img src="JpegImage.aspx" id="imgcaptcha"><input type="text" name="txtCodeNumber" tabindex="5"><br>';
...
//function chat(sid,server,nick,chan)
//{
//    function connect()
//    {
...
//                for (var i=0;i<responses.length && e==0;i++)
//                    if (responses[i]!='')
//                    {
//                        var response=eval('(' + responses[i] + ')');
//                        if (response.cmd=='nicklist')
//                        {
//                            e=1;
//                            var nicks=get_nicks(response);
//                            disconnect();
//                            show(nicks);
//                        }
//                    }
...
//            }
//        }
//        function get_nicks(nicklist)
//        {
//            var nicks=new Array(nicklist.nicks.length-1);
//            var j=0;
//            for (var i=0;i<nicklist.nicks.length;i++)
//                if (nicklist.nicks[i].nick!=nick)
//                    nicks[j++]=((typeof(nicklist.nicks[i].op)!='undefined' && nicklist.nicks[i].op==true)?'@':'') + nicklist.nicks[i].nick;//
//            return nicks;
//        }
//        function show(nicks)
//        {
//            var txt='<div class="textfolder"><a href="http://embed.mibbit.com/?server=irc.freenode.org&channel=%23%23fdt" target="_blank">##fdt</a>: ';
...
//        var details={'method':'POST',
//                     'url':url_s,
//                     'data':'{"channel":"IRCClient","cmd":"embed","referrer":""}\n{"channel":"LoginManager","cmd":"guest","version":1648,"uagent":"Mozilla/5.0"}\n{"channel":"IRCClient","cmd":"connect","data":"' + server + '","nick":"' + nick + '","pass":"","authmethod":"nickserv","joinchannels":"' + chan + '","charset":"UTF-8","forced":true}',
...
//        GM_xmlhttpRequest(details);
//    }
//    var url_s='http://embed.mibbit.com/j/mibbit-s-' + sid;
//    var url_r='http://embed.mibbit.com/j/mibbit-r-' + sid;
//    connect();
//}

//versone corrente (non commentata)
...
//            chat(response.sessionid,'irc.freenode.org',get_nick(),'##fdt');
//        }
//    }
//    var details={'method':'POST',
//                 'url':'http://embed.mibbit.com/j/mibbit-r',
//                 'data':'{}',
//                 'onreadystatechange':responseHandler};
//    GM_xmlhttpRequest(details);
//}

//versione corrente (non commentata)
function init()
{
    var nick=get_nick();
    function responseHandler(xhr)
...
//        if (tr[i].getAttribute('class')=='paginazione')
//        {
//            var bar=tr[i].firstChild.nextSibling.nextSibling.nextSibling;
//            bar.innerHTML='<a id="fdtchat1" class="textfolder" href="javascript:void(0)">Elenco utenti in chat</a>&nbsp;|&nbsp;' + bar.innerHTML;
//            document.getElementById('fdtchat1').addEventListener('click',preinit2,false);
//            break;
//        }
//}
//
...
//            td2.innerHTML='<div class="textfolder"><img src="images/clessidra.gif"/>&nbsp;Attendere prego...</div></td>';
...
// Mostra feed di Punto Informatico
function loadPIFeed() {
