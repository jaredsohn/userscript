...
if(links[i].innerHTML=="Drammen") {
links[i].innerHTML="Født i";
links[i].style.color="#FF7F00";
sted++;}


if(links[i].innerHTML=="Buskerud"){
links[i].innerHTML="Drammen";
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Ignorer") {
  links[i].innerHTML="Si Jeg Hater Deg";
  links[i].style.color="#ff0000"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Bli venn med") {
  links[i].innerHTML="Være Venner Eller?";
  links[i].style.color="#8B7D7B"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Skriv en hilsen") {
  links[i].innerHTML="Skriv i Gjestebok="#8B7D7B"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Postkasse") {
  links[i].innerHTML="Mine Post";
...
for(q=0;q<fakta.length;q++){
	if(fakta[q].innerHTML=="Fakta")
		{fakta[q].innerHTML="Statestikk KJEDELIG !:";
		break;
	}
}
...
for(q=0;q<imON.length;q++){
	if(imON[q].innerHTML=="Pålogget")
		//{imON[q].innerHTML="<center><blink>Jeg er Vist Online Nå </blink></center>";
