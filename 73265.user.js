// ==UserScript==
// @name           Aquila Gifter & Alchemy
// @namespace      Aquila Gift
...
function send(uid, num, gift) {
    if(num && keepGoing) {
        $.post("http://apps.facebook.com/castle_age/army.php?act=create&gift=" + gift, {'ids[]': uid}, 
...
function gift() {
    var ca_gift = get_sub_panel('ca_gift'),
        ...
        inputID    = $("<input></input>"),
        buttonSub  = $("<button >GO!>"),
        gifts      = ['Random Soldier', 'Mystery Heirloom - Titania Bow/Elven Crown', 'Volcanic Egg - 

Shield of Dante', 'Mystery Ice Relic 2 - Ragnarok, The Ice Elemental', 'Mystery Earth - Earth Orb', 

'Mystery Relic 2 - Drake Helm', 'Mystery Item - Battle Of Dark Legion', 'Dragan Set - Dragan Blade', 

'Mystery Relic - Serpentine Shield', 'Mystery Treasure - Poseidons Horn', 'Serpent Egg - Sea Serpent', 

'Dragon Egg - Dragon', 'Mystery Druid Item - Whisper Bow', 'Mystery Armor - Golden Hand', 'Mystery Frost 

Item - Frost Tear Dagger', 'Mystery Artifact - Morning Star', 'Mystery Armor 2 - Mystic Armor', 'Mystery 

Frost Relic - Glacial Blade', 'Mystery Fire Relic - Ring Of Ember',],
    ...
    buttonSub.click(function() {
        $("<div></div>").load("party.php span.linkwhite a", function() {
            if(/id=(\d+)/.test($(this).children().attr("href"))) {
                ...
                alert("Cannot find your user ID.  CA servers are possibly busy.  Please try again 
...
function do_alch(form, num) {
    if(num > 0 && form.size()) {
        ...
        $("<div></div>").load("alchemy.php div.results span.result_body, #"+id, data, 
...
function(responseText, textStatus, XMLHttpRequest) {
            ...
            if(/You have created/.test(txt)) {
                ...
                alert('All items could not be combined.  You do not have sufficient materials to combine ' 
...
function alchemy() {
    ...
    divs.load("alchemy.php div.statsT2 table div.alchemyRecipeBack", function(responseText, textStatus, 
...
XMLHttpRequest) {
        ...
        divs.children().each(function(idx) {
            selectReci.append("<option value='"+$("form",$(this)).attr("id")+"'>"+$("div.recipeTitle", 

$(this)).text().replace(/RECIPES: Create | to join your army!/g,'')+"</option>");
        });
    ...
    GM_xmlhttpRequest({
        ...
        onload : function(response) {
            ...
            if(currentVersion < remoteVersion) {
                if(confirm("There is a newer version of this script available.  Would you like to 

update?")) {
                    setTimeout(function() {unsafeWindow.location.href = 

"http://userscripts.org/scripts/source/"+num+".user.js";}, 3000);
                }
            }
        }
    });
...
GM_registerMenuCommand("CA - Aquila Gifter", gift );
GM_registerMenuCommand("CA - Aquila Auto Alchemy", alchemy);

$(document).ready(function() {
    check_update(73265, '1.21', true);
});