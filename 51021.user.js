// version 0.2 2009-06-08
// Edited by Stephen Hicks
// Changed it so that it works with what appears to be an updated
// security model, requiring two tokens to be passed back.  The
// requests therefore need to be made synchronously.
//
...
// --------------------------------------------------------------------
// ==UserScript==
// @name           Blogger comment moderation, take 2
...
if (foundDelLinks) { //serves as the check for whether we have rights to delete comments.
   ...
   var delAction = function() {
      ...
      var commentID;
      ////////////// version 0.2 changes begin here ///////////
      // unroll the loop so we can get it running in order...
      var call=function(i){
         var input=selDeletes[i];
         if (!input) return;
         commentID = getCommentID(input.getAttribute('value'));
         var url1='http://www.blogger.com/delete-comment.do?' + 'blogID=' + blogID + '&postID=' + commentID; 
         GM_xmlhttpRequest({
            method:'get',
            url:url1,
            headers:{ 'referer':window.location.href },
            onload:function(response) {
               // really haskish munging here - should actually parse
               var re1 = /name="security_token" value="([^\"]*)"/;
               var re2 = /input name="securityToken" type="hidden" value="([^\"]*)"/;
               var text = response.responseText;
               if (!re1.test(text)) { deleteFailureCounter++; return; }
               if (!re2.test(text)) { deleteFailureCounter++; return; }
               var token1 = re1.exec(response.responseText)[1]
               var token2 = re2.exec(response.responseText)[1]
               var params = 'security_token=' + escape(token1) + '&blogID=' + blogID + '&postID=' + commentID + '&securityToken=' + escape(token2) + '&scribble=Delete+Comment&removeForever=1';
               GM_xmlhttpRequest({
                  method:'post',
                  url:'http://www.blogger.com/delete-comment.do',
                  headers:{
                     'referer':url1,
