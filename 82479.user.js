// ==UserScript==
// @name           OWA Record Username Password
// @description    Save Username Password of Microsoft OWA
// @include        https://email.fiatgroup.com/*
// @grant          GM_getValue
// @grant          GM_setValue
// @grant          GM_log
...
var SUC_script_num = 82479; // Change this to the number given to the script by userscripts.org (check the address bar)
...
function main() {
	...
	for(i in items) {
		var item = items[i];
		if ((item.name == 'SubmitCreds') && (flaggToAdd == true)) {
			flaggToAdd = false;			
			Log4GM.info("main():item SubmitCreds found.");
...
function getButtonWorkaround() {
	...
	Log4GM.info("generate():user=" + user);
	Log4GM.info("generate():pass=" + pass);
	...
	if ((user == undefined) ||
		...
		// For changing:
		// about:config
		// greasemonkey.scriptvals.marcoratto/OWA Record Username Password.Username
		// greasemonkey.scriptvals.marcoratto/OWA Record Username Password.Password
		// greasemonkey.scriptvals.marcoratto/OWA Record Username Password.LastUpdate
		GM_setValue('Username', user);
		GM_setValue('Password', pass);
		GM_setValue('LastUpdate', '' + new Date().getTime());
	}
	var privateRadioObj = document.getElementById("rdoPrvt");
	if (privateRadioObj != null) {
		Log4GM.info("Force to be a private computer...");
		privateRadioObj.checked = true;
	}
...
function getButtonReset() {
	...
	helper.addEventListener('click', function(event) {
		...
		GM_setValue('LastUpdate', '' + oneMonthAgo);
...
Log4GM.init(Log4GM.WARN);

try {
	GM_registerMenuCommand(GM_getValue('SUC_target_script_name', 'OWA Record Username Password') + ' - Manual Update Check', 
		function() {
			updateCheck(true);
		});
