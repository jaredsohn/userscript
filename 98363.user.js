...
// @include        http://facebook.com/*
// @include        https://facebook.com/*
// @copyright      JoeSimmons (modified from the original script by riscy, but all copyright remains with JoeSimmons)
// @version        1.1.18
...
1.1.18 - added bricks, nails, & boards, and improved help requests to include more types of them
*/
...
var buttons = $g(".//ul[contains(@class, 'uiList')]/li[contains(@class, 'uiListItem')"+(type != "all" ? "and contains(., '"+type+"')" : "")+"]//ancestor::ul", {type:7, node:div.snapshotItem(i)});
for(var x=0,button; (button=buttons.snapshotItem(x)); x++) if(button && button.nodeType == 1) {
var check = $g(".//ancestor::li[contains(@class, 'uiListItem')]//input[@type='checkbox' and starts-with(@id,'mass_accept_')]", {type:9, node:button});
check.checked = true;
...
document.body.appendChild(main.create("div", {id:"mass_accept_div",style:"position:fixed; bottom:26px; right:0px;"},new Array(
main.create('select', {id:"accept_select_type", size:"4", style:"width: 160px; display:"+(box===true ? " block;" : " none;"), ondblclick:main.checkAccept}, new Array(
main.create("option", {value:"all", textContent:"All Types From TM", selected:"yes"})
)),
main.create('select', {id:"accept_select", size:"4", style:"width: 160px; display:"+(box===true ? " block;" : " none;"), ondblclick:main.checkAccept}, new Array(
main.create("option", {value:"all", textContent:"All", selected:"yes"}),
main.create("option", {value:"confirm_29518083188", textContent:"TreasureMadness"})
)),
...
debugMode = GM_config.get("debug") === true, // debug mode, showing the reqs frame
...
div = $g("//div"+(id != "all" ? "[contains(@id, '"+id+"')]" : "[@id='contentArea']"), {type:7});
for(var i=0, len=div.snapshotLength; i < len; i++) {
var buttons = $g(".//ul[contains(@class, 'uiList')]/li[contains(@class, 'uiListItem')"+(type != "all" ? "and contains(., '"+type+"')" : "")+"]/form", {type:7, node:div.snapshotItem(i)});
