...
// @description    Show All Notes on flickr photos
...
['toString', 'toLocaleString', 'valueOf', 'toSource', 'watch', 'unwatch', 'hasOwnProperty', 'isPrototypeOf', 'propertyIsEnumerable'].each(function (method) {
    ...
    var api_key = '8112ab1d8a23e420d3c40ec3abeef8fa';

    var noteCount = $('noteCount');
    try {
        var notes = $('notes').getElements('li.box'); // new layout
    } catch (e) {
        // ignore in old layout
    }
    if ($chk(noteCount) || ($chk(notes) && notes.length > 0)) {
        var showNotes = new Element('a', {
            html: ' (show notes)',
            styles: {
                cursor: 'pointer'
            },
            events: {
                click: function (evt) {
                    evt.preventDefault();
                    var mustShow = this.get('html').match('show');
                    // the note rectangles:
                    try {
                        $('photo_notes').style.visibility = (mustShow ? 'visible' : 'hidden');
                        $$('div.note-text').each( function (note) {
                            if (mustShow) {
                                note.addClass('td_note_yeller');
                            } else {
                                note.removeClass('td_note_yeller');
                            }
                            note.style.display = mustShow ? 'block' : 'none';
                            note.style.visibility = mustShow ? 'visible' : 'hidden';
                        });
                    } catch (e) { // new layout
                        $$('span.note-text, span.box-stroke').each( function (note) {
                            note.style.visibility = mustShow ? 'visible' : 'hidden';
                        });
                    }
                    // the 'show notes' label
                    this.set('html', mustShow ? ' (hide notes)' : ' (show notes)');
                }
            }
        }).inject($chk(noteCount) ? noteCount : $('meta'));
    }
})();
