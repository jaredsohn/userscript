// ==UserScript==
// @name           PDO+ Script
// @namespace      http://userscripts.org/scripts/show/99153
// @author         Joseph F.
// @description    Script para facilitar o trabalho da moderacao da PDO+
// @version        4.0
...
/*-------------------------------------------------------------------*\
|			Script criado por Joseph F.		      |
|			www.portaltec.org			      |
|								      |
|			Visite a DgoH Codes no Orkut:		      |
|			cmm=9531162				      |
\*-------------------------------------------------------------------*/

var colocaNaPagina = function(){

tpc="\x63\x6f\x6d\x6d\x6d\x73\x67\x70\x6f\x73\x74\x3f\x63\x6d\x6d\x3d\x39\x32\x35\x38\x34\x31\x26\x74\x69\x64\x3d\x35\x35\x33\x37\x31\x32\x36\x37\x38\x37\x38\x36\x36\x30\x38\x31\x38\x35\x39";
...
function online()
	{
		...
		var breakLine = "\n";
		var texto = String("[b]Estou [green]ONLINE![/green][/b]").concat("\n\n[silver]",breakLine,breakLine,String(Math.random()),"[/silver]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
...
function offline()
	{
		...
		var breakLine = "\n";
		var texto = String("[b]Estou [red]OFFLINE![/red][/b]").concat("\n\n[silver]",breakLine,breakLine,String(Math.random()),"[/silver]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
				"subjectText" : encodeURIComponent(""),
...
function confirmaDeletarTopicos()
	{
		...
		if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML))
		{
			...
			"prompt('Digite o motivo de estar deletando este topico:\\n','T\\xf3pico de spam'));" + 
...
function confirmaBaneMembro()
	{
		...
		if (new RegExp(/ban/i).test(tagBody.innerHTML))
		{
			...
			"prompt('Digite o motivo de estar banindo este membro:\\n','Fez spam'));" + 
