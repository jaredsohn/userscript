// ==UserScript==
// @name            PowerBot FFA
// @description     Kabazzled PowerBot FFA
// @include         *
// ==/UserScript==
...
var game_iframe_id = 'game_frame';
if (document.getElementById(game_iframe_id) != null) {
    var timeout = 60000;
    ...
    setTimeout(make_space_for_scriptUI, timeout);
    return;
}
function make_space_for_scriptUI() {
    var content = document.getElementById('content');
    content.style.height = (parseInt(content.style.height) + 350) + "px";
    var frame = document.getElementById(game_iframe_id);
    frame.style.height = (parseInt(frame.style.height) + 350) + "px";

    var positionUIButton = document.getElementById('PositionUI');
    if (!is_defined(positionUIButton)) {
        positionUIButton = document.createElement('input');
        positionUIButton.id = "PositionUI";
        positionUIButton.type = 'button';
        positionUIButton.value = 'Make Space for Script';
        positionUIButton.style.top = "125px";
        positionUIButton.style.left = (parseInt(content.style.width) - 170) + "px";
        positionUIButton.style.height = "30px";
        positionUIButton.style.position = 'absolute';
        positionUIButton.addEventListener('click', make_space_for_scriptUI, false);
    
        content.insertBefore(positionUIButton, content.firstChild);
    }
}
...
// XXX: Dragon types to attack. Dragon selection for auto-attack.
...
function get_log_messages(from_index, level, isExact) {
    ...
    for (var i = 0; i < num_stored_objs(); i++) {
        ...
        if (!is_defined(object) || !object || (object.type != LOG_OBJECT_TYPE)) { continue; }
...
function get_element_or_log_if_absent(element_id, module, level, doc) {
    ...
    if (!is_defined(doc) || !doc) { doc = document; }
...
<table width="100%" cellspacing="0"><tr><td>\
	...
	<li><a href="#" width="10%" id="show_TON_ui_4">Auto attack</a></li>\
	<li><a href="#" width="8%" id="show_TON_ui_5">Donate</a></li>\
	<li><a href="#" width="4%" id="show_TON_ui_6">Log</a></li>\
</td><td width="36%" align="center" style="background-color:black;" id="Script_Name">\
<marquee><li style="color:white;">Thirst of Night Power tools</li></marquee>\
</td></tr></table>\
</ul>\
</div>\
<div id="ui_shell_content" style="background-image:url(http://i.imgur.com/tMUO5.png);background-repeat:no-repeat;">\
...
function createUI() {
    ...
    var script_ui = new PopUp('TON_Script', ui_shell_content, 0, 0, dimensions.width - UI_HORIZONTAL_MARGIN, 175, true, null, document_to_add_to);
    ...
    ui_content_shell.setAttribute('style', shell_style + ';height:145px');

    add_style(ui_tab_style, document_to_add_to);
    
    get_element_or_log_if_absent('show_TON_ui_0', UI_MODULE, ERROR, document_to_add_to).addEventListener('click', function() { show_TON_ui(0); }, false);
    get_element_or_log_if_absent('show_TON_ui_1', UI_MODULE, ERROR, document_to_add_to).addEventListener('click', function() { show_TON_ui(1); }, false);
    get_element_or_log_if_absent('show_TON_ui_2', UI_MODULE, ERROR, document_to_add_to).addEventListener('click', function() { show_TON_ui(2); }, false);
    get_element_or_log_if_absent('show_TON_ui_3', UI_MODULE, ERROR, document_to_add_to).addEventListener('click', function() { show_TON_ui(3); }, false);
    get_element_or_log_if_absent('show_TON_ui_4', UI_MODULE, ERROR, document_to_add_to).addEventListener('click', function() { show_TON_ui(4); }, false);
    ...
    get_element_or_log_if_absent('show_TON_ui_6', UI_MODULE, WARNING, document_to_add_to).addEventListener('click', function() { show_TON_ui(6); }, false);
...
'<table width="100%" border="0" cellspacing="0" cellpadding="0">\
  ...
  <tr>\
    <td style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Alchemists</td>\
	<td><input id="Change_Alchemists" type="text" value="0" size="18" maxlength="5"/></td>\
  </tr>\
</table>', 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">\
  ...
  <tr>\
    ...
    <td style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Reapers</td>\
	<td><input id="Attack_Reapers" type="text" value="0" size="7" maxlength="5"/></td>\
  </tr>\
  <tr>\
    <td style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Alchemists</td>\
	<td><input id="Attack_Alchemists" type="text" value="0" size="7" maxlength="5"/></td>\
  </tr>\
</table>',
'<table width="100%" border="0" cellspacing="0" cellpadding="0">\
  <tr style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">\
    <td width="5%">Enabled:</td>\
    <td width="5%"><input type="checkbox" id="AutoAttack_Enabled" /></td>\
    <td width="10%">\
    <span title="Enter Shanks base co-ordinates, if you want auto-attack to avoid them"><strong>[Shanks base]:</strong></span></td>\
    <td width="2%"><div align="right">x:</div></td>\
    <td width="3%"><input type="text" maxlength="3" size="3" id="AutoAttack_Shank_X"/></td>\
    <td width="2%"><div align="right">y:</div></td>\
    <td width="10%"><input type="text" maxlength="3" size="3" id="AutoAttack_Shank_Y"/></td>\
    <td width="10%">\
    <span title="Enter Pythus base co-ordinates, if you want auto-attack to avoid them"><strong>[Pythus base]:</strong></span></td>\
    <td width="2%"><div align="right">x:</div></td>\
    <td width="3%"><input type="text" maxlength="3" size="3" id="AutoAttack_Pythus_X"/></td>\
    <td width="2%"><div align="right">y:</div></td>\
    <td width="46%"><input type="text" maxlength="3" size="3" id="AutoAttack_Pythus_Y"/></td>\
  </tr>\
</table>\
<table width="100%" border="0" cellspacing="0" cellpadding="0">\
  <tr>\
  <td width="5%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Radius:</td>\
  <td width="2%"><input type="text" id="AutoAttack_Distance" maxlength="3" size="3"/></td>\
  <td width="3%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Type:</td>\
  <td width="14%">\
  <select name="select" id="AutoAttack_Type">\
    <option value="0">Any</option>\
    <option value="1">Waste Land</option>\
    <option value="2">Chasm</option>\
    <option value="3">Mountain</option>\
    <option value="4">Hill</option>\
    <option value="5">Prairie</option>\
    <option value="6">Lake</option>\
    <option value="8">Red Blood Camp</option>\
  </select>\
  </td>\
  <td width="4%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Level:</td>\
  <td width="5%"><select id="AutoAttack_Level">\
		<option value="1">1</option>\
	    <option value="2">2</option>\
	    <option value="3">3</option>\
	    <option value="4">4</option>\
  ...
  <td width="15%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Include lower levels:</td>\
  <td width="2%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;"><input type="checkbox" id="AutoAttack_IncludeLower" /></td>\
  <td width="6%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">General:</td>\
  <td width="10%">\
      <select id="AutoAttack_List_General">\
      </select>\
  </td>\
  <td width="7%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Gargoyle:</td>\
   <td width="12%">\
      <select id="AutoAttack_List_Gargoyle">\
    </select>  </td> \
  <td width="3%"><img src="http://4.bp.blogspot.com/-p0Z6x-8GGiQ/T6ILrD1C9UI/AAAAAAAAAaI/Uso68Y_55Jw/s1600/old_go_first.png" /></td>\
  <td width="3%"><img src="http://2.bp.blogspot.com/-KWPpMoxvgjA/T6ILqY2K4EI/AAAAAAAAAaE/EUnM1wZJmlw/s1600/8.png" /></td>\
  <td width="3%"><img src="http://4.bp.blogspot.com/-K21rCTyHcY0/T6ILoZVsDJI/AAAAAAAAAZ8/XvVn1TNTMp4/s1600/7.png" /></td>\
  <td width="3%"><img src="http://3.bp.blogspot.com/-jR9mDKVIzdk/T6ILr11kZtI/AAAAAAAAAaU/mP7OQhy1Z0k/s1600/old_go_last.png" /></td>\
  <td width="3%"><img src="http://1.bp.blogspot.com/-09hASl_Pl_E/T6I0BImzerI/AAAAAAAAAas/SGcNUS0352I/s1600/media_playback_stop.png" /></td>\
</tr>\
</table>\
<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">\
<tr>\
  <td>Grunts:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Grunts"/></td>\
  <td>Trucks:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Trucks"/></td>\
  <td>Slashers:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Slashers"/></td>\
  <td>Sigbins:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Sigbins"/></td>\
  <td>Flickerforms:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Flickerforms"/></td>\
  <td>Snipers:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Snipers"/></td>\
</tr>\
<tr>\
  <td>Vampire Bats:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_VampireBats"/></td>\
  <td>Nemesis Bats:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_NemesisBats"/></td>\
  <td>Armored Trucks:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_ArmoredTrucks"/></td>\
  <td>Nosferatu:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Nosferatu"/></td>\
  <td>Impalers:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Impalers"/></td>\
  <td>Reapers:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Reapers"/></td>\
</tr>\
<tr>\
  <td>Alchemists:</td>\
  <td><input type="text" maxlength="5" size="5" id="AutoAttack_List_Alchemists"/></td>\
</tr>\
</table>',
...
<table id="Log_Msgs_Table" width="100%" border="1" style="background-color:#FF9900;font-family:\'Arial Narrow\',sans-serif;font-size:16px;" cellspacing="0" cellpadding="0">\
  <tr>\
    <th width="6%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Id</th>\
    <th width="20%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Module</th>\
    <th width="7%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Level</th>\
    <th width="67%" style="font-family:\'Arial Narrow\',sans-serif;font-size:16px;">Message</th>\
  </tr>\
</table>\
</div>'];
...
var ui_descriptions = ["Upgrade/remove buildings in a city", 
					...
					"Automatically attack repeatedly, with pre-set troops",
...
function auto_attack_manage() {
    var auto_attack_cb = document.getElementById('AutoAttack_Enabled');
    ...
    var object = retrieve_object(game_setting_ids[AUTO_ATTACK_STATE]);    
    object.value = auto_attack_cb.checked;
    
    game_setting[AUTO_ATTACK_STATE] = object;
    update_object(game_setting_ids[AUTO_ATTACK_STATE], object);
}
...
function set_generals_dropdown(city_index) {
    if ((ui_tab_type != ATTACK_TAB) && (ui_tab_type != AUTO_ATTACK_TAB)) { return; }
	...
	if (ui_tab_type == ATTACK_TAB) {
    	generals_dd = get_element_or_log_if_absent('Selected_General', UI_MODULE, ERROR);
    } else {
        generals_dd = get_element_or_log_if_absent('AutoAttack_List_General', UI_MODULE, ERROR);
    }
...
function show_buildings_dropdown() {
    if (ui_tab_type != BUILDINGS_TAB) { return; }
    var city_index = get_element_or_log_if_absent('cities', UI_MODULE, ERROR).value;
    ...
    var buildings_dd = get_element_or_log_if_absent('Selected_Building', UI_MODULE, ERROR);
    var building_types = "";
...
function show_TON_ui(type) {
    ...
    if ((type < BUILDINGS_TAB) || (type > LOGGING_TAB)) {
	...
	for (i = BUILDINGS_TAB; i <= LOGGING_TAB; i++) {
    ...
    if ((ui_tab_type == RESEARCH_TAB) || (ui_tab_type == ATTACK_TAB) || (ui_tab_type == AUTO_ATTACK_TAB)) {
        cities_dropdown.value = capital_index;
        cities_dropdown.disabled = true;
    }
    if ((ui_tab_type == ATTACK_TAB) || (ui_tab_type == AUTO_ATTACK_TAB)) {
	    set_generals_dropdown(capital_index);
    }

	if (ui_tab_type == AUTO_ATTACK_TAB) {
	    var aa_element = get_element_or_log_if_absent('AutoAttack_Enabled', UI_MODULE, ERROR);
	    aa_element.checked = (game_setting[AUTO_ATTACK_STATE].value ? true : false);
	    aa_element.addEventListener('click', auto_attack_manage, false);
	    
	    var shank_x = get_element_or_log_if_absent('AutoAttack_Shank_X', UI_MODULE, ERROR);
	    var shank_y = get_element_or_log_if_absent('AutoAttack_Shank_Y', UI_MODULE, ERROR);
	    
	    shank_x.value = game_setting[AUTO_ATTACK_SHANK_LOCATION].value.x;
	    shank_y.value = game_setting[AUTO_ATTACK_SHANK_LOCATION].value.y;
	    
	    var pythus_x = get_element_or_log_if_absent('AutoAttack_Pythus_X', UI_MODULE, ERROR);
	    var pythus_y = get_element_or_log_if_absent('AutoAttack_Pythus_Y', UI_MODULE, ERROR);
	    
	    pythus_x.value = game_setting[AUTO_ATTACK_PYTHUS_LOCATION].value.x;
	    pythus_y.value = game_setting[AUTO_ATTACK_PYTHUS_LOCATION].value.y;
	} else if (ui_tab_type == BUILDINGS_TAB) {
...
function check_and_research(city_id, technology, final_level) {
    ...
    if (curr_level >= final_level) {
        ...
        add_log_message(USER_MODULE, INFO, 'Chose a very high level (' + final_level + ') to upgrade (' +
            technology + ') to, should choose within 2 - 10. Defaulting to 10.');
        final_level = 10;
    }
...
function process_TON_action_reqs() {
	...
	case 2:
		...
		var alchemists_to_change = get_element_or_log_if_absent('Change_Alchemists', UI_MODULE, WARNING); 
	...
	case 3:
        ...
        var attacking_reapers = get_element_or_log_if_absent('Attack_Reapers', UI_MODULE, WARNING);
        var attacking_alchemists = get_element_or_log_if_absent('Attack_Alchemists', UI_MODULE, WARNING);
        ...
        var gargoyle = (gargoyle_to_send ? parseInt(gargoyle_to_send.value) : 0);
	...
	case 4:
	    ...
	    game_setting[AUTO_ATTACK_SHANK_LOCATION].value.x = shank_x.value;
	    game_setting[AUTO_ATTACK_SHANK_LOCATION].value.y = shank_y.value;
	    update_object(game_setting_ids[AUTO_ATTACK_SHANK_LOCATION], game_setting[AUTO_ATTACK_SHANK_LOCATION]);
	    
	    var pythus_x = get_element_or_log_if_absent('AutoAttack_Pythus_X', UI_MODULE, ERROR);
	    var pythus_y = get_element_or_log_if_absent('AutoAttack_Pythus_Y', UI_MODULE, ERROR);
	    
	    game_setting[AUTO_ATTACK_PYTHUS_LOCATION].value.x = pythus_x.value;
	    game_setting[AUTO_ATTACK_PYTHUS_LOCATION].value.y = pythus_y.value;
	    update_object(game_setting_ids[AUTO_ATTACK_PYTHUS_LOCATION], game_setting[AUTO_ATTACK_PYTHUS_LOCATION]);
	    break;
	default:
		break;
	}
...
var AUTO_ATTACK_SHANK_LOCATION = "AutoAttackShankBaseToAvoid";
var AUTO_ATTACK_PYTHUS_LOCATION = "AutoAttackPythusBaseToAvoid";
...
function restore_setting() {
    ...
    if (!game_setting.hasOwnProperty(AUTO_ATTACK_SHANK_LOCATION)) {
        var auto_attack_shank_loc = {type:GAME_SETTING_TYPE, name: AUTO_ATTACK_SHANK_LOCATION, value: {x:-1, y:-1}};
        var id = store_object(auto_attack_shank_loc);
        
        game_setting[AUTO_ATTACK_SHANK_LOCATION] = auto_attack_shank_loc;
        game_setting_ids[AUTO_ATTACK_SHANK_LOCATION] = id;
    }
    if (!game_setting.hasOwnProperty(AUTO_ATTACK_PYTHUS_LOCATION)) {
        var auto_attack_pythus_loc = {type:GAME_SETTING_TYPE, name: AUTO_ATTACK_PYTHUS_LOCATION, value: {x:-1, y:-1}};
        id = store_object(auto_attack_pythus_loc);
        
        game_setting[AUTO_ATTACK_PYTHUS_LOCATION] = auto_attack_pythus_loc;
        game_setting_ids[AUTO_ATTACK_PYTHUS_LOCATION] = id;
    }
    if (!game_setting.hasOwnProperty(AUTO_ATTACK_STATE)) {
        var auto_attack_state = {type:GAME_SETTING_TYPE, name: AUTO_ATTACK_STATE, value: 0};
        id = store_object(auto_attack_state);
        
        game_setting[AUTO_ATTACK_STATE] = auto_attack_state;
        game_setting_ids[AUTO_ATTACK_STATE] = id;
    }
...
var DEFAULT_REPORTS_NUM = 30;
function get_reports(count, action_cb, context) {
	if (!is_defined(count) || !(count = parseInt(count))) {
		add_log_message(DATA_MODULE, WARNING, "Given number of reports to retrieve, is invalid. " + 
			"Using default value (" + DEFAULT_REPORTS_NUM + ")");
		count = DEFAULT_REPORTS_NUM;
	}
    ...
    params['category'] = 'all';
    params['page'] = 1;
    params['_eventType'] = 'msgs_req';
    
    var baseMsg = "Getting (" + count + ") reports ";
...
var CellTypes = ['Invalid', 'WasteLand', 'Chasm', 'Mountain', 'Hill', 'Prairie', 'Lake', 'Marsh', 'RedBloodCamp', 'CapitalCity', 'Outpost'];
var MAP_LENGTH = 750;
var MAP_WIDTH = 750;
var cell_info = {
    player_id: '',
...
var alliances_by_id = {};
var alliance_id_by_name = {};

function map_data_at(x, y, offset) {
    var url_params = {};
    url_params['x'] = x;
    url_params['y'] = y;
    url_params['offset'] = offset;
    url_params['_eventType'] = 'map_req';
    
    url_params['_session_id'] = sessionId;
    url_params['gangster'] = gangster;
    
    var details = {};
    details.method = 'GET';
    details.url = apiServer + "/map.json?" + toQueryString(url_params);    
    details.onerror = mapDetails_Fetch_error;
    details.onload = mapDetails_Fetch_completed;

    function mapDetails_Fetch_error(response) {
        add_log_message(DATA_MODULE, ERROR, "Fetching map details for square (" + 
            ((x - offset) % MAP_LENGTH) + ", " + ((y - offset) % MAP_WIDTH) + ") (" + 
            ((x + offset) % MAP_LENGTH) + ", " + ((y + offset) % MAP_WIDTH) + ") " + 
            "failed. Status is (" + response.statusText + ")");
    }
    function mapDetails_Fetch_completed(response) {
        var last_updated_TS = (new Date().getTime() / 1000);

        var response_JSON = JSON.parse(response.responseText);
        for (var i = 0; i < response_JSON.map_cities.length; i++) {
            var city = response_JSON.map_cities[i];
            
            map_data[city.x][city.y].cell_name = city.name;
            map_data[city.x][city.y].cell_type = (city.type == "Capital") ? 9 : 10;
            map_data[city.x][city.y].cell_level = city.level;
            map_data[city.x][city.y].player_id = city.player.id;
            map_data[city.x][city.y].cell_last_updated = last_updated_TS;

            if (player_data[city.player.id] === undefined) {
                player_data[city.player.id] = {};
            }
            player_data[city.player.id].name = city.player.name;
            player_data[city.player.id].level = city.player.level;
            player_data[city.player.id].power = city.player.might;
            player_data[city.player.id].alliance = city.player.alliance;
            player_data[city.player.id].protected_till = city.player.protected_until;
            player_data[city.player.id].last_updated = last_updated_TS;
        }
        for (i = 0; i < response_JSON.city_wildernesses.length; i++) {
            var occupied_territory = response_JSON.city_wildernesses[i];
            
            map_data[occupied_territory.x][occupied_territory.y].player_id = 
                occupied_territory.player.id;
            map_data[occupied_territory.x][occupied_territory.y].cell_last_updated = 
                last_updated_TS;
                
            if (player_data[occupied_territory.player.id] === undefined) {
                player_data[occupied_territory.player.id] = {};
            }
            player_data[occupied_territory.player.id].name = occupied_territory.player.name;
            player_data[occupied_territory.player.id].level = occupied_territory.player.level;
            player_data[occupied_territory.player.id].power = occupied_territory.player.might;
            player_data[occupied_territory.player.id].last_updated = last_updated_TS;            
        }
    }
...
/*
var WAIT_TIME = 5 * 60 * 1000;

var MAP_FETCH_SIZE = 15;    // (2 * Offset + 1)
var MAP_FETCH_INTERVAL = 40;
function get_players_occupations_data(x, y) {
    var isError = false;
    ...
    if (x == 0) return;
    if ((x_end >= (MAP_LENGTH - 1)) && (x == 0)) { return; }
    
    setTimeout(function() { get_players_occupations_data(x, y); }, MAP_FETCH_INTERVAL);
}
*/

function get_dynamic_map_data() {
/*
	get_players_occupations_data();
*/
}
function get_map_data() {
    ...
    */
    get_static_map_data();
    get_dynamic_map_data();

    /*
    var time_estimate = WAIT_TIME;
    setTimeout(function() {
        for (var i = 0; i < MAP_LENGTH; i++) {
            for (var j = 0; j < MAP_WIDTH; j++) {
                if (is_defined(map_data[i][j].player_id)) {
                    var player = map_data[i][j].player_id;
                    
                    if (player_data[player].owned_cells === undefined) {
                        player_data[player].owned_cells = [];
                    }
                    player_data[player].owned_cells.push({x:i, y:j});
                }
            }
        }

        for (player_id in player_data) {
            var player_name = player_data[player_id].name;
            
            var stored_id = player_id_by_name[player_name];
            if (is_defined(stored_id) && (stored_id != player_id)) {
                add_log_message(DATA_MODULE, ERROR, "Player (" + player_name + ") is associated with " + 
                    "more than one id (" + stored_id + ", " + player_id + ")");
                continue;
            }
            player_id_by_name[player_name] = player_id;
            
            var player_alliance = player_data[player_id].alliance;
            if (is_defined(player_alliance) && player_alliance) {
                var orig_alliance_id = alliance_id_by_name[player_alliance.name];

                if (is_defined(orig_alliance_id) && (orig_alliance_id != player_alliance.id)) {
                    add_log_message(DATA_MODULE, ERROR, "Possible loss of information. Two alliances (" + 
                        orig_alliance_id + ", " + player_alliance.id + ") have the same name (" + 
                        player_alliance.name + "). Increase wait-time for fetching the map.");
                } else if (!is_defined(orig_alliance_id)) {
                    alliances_by_id[player_alliance.id] = {name:player_alliance.name, members:[player_id]};
                } else {
                    alliances_by_id[player_alliance.id].members.push(player_id);
                }
                alliance_id_by_name[player_alliance.name] = player_alliance.id;      
                
                player_data[player_id].alliance_id = player_alliance.id;
            }
            player_data[player_id].alliance = null;            
        }
    }, time_estimate);
    */
}
function map_fetcher_thread() {
    get_map_data();
    setTimeout(map_fetcher_thread, 60 * 60 * 1000);
}
...
//    attack_units['RedBloodCamp'][1][REAPERS] = {type:TROOP, qty:25};
    attack_units['RedBloodCamp'][1][SNIPERS] = {type:TROOP, qty:15000};
    attack_units['RedBloodCamp'][1][ARMORED_TRUCKS] = {type:TROOP, qty:50};
...
//    attack_units['RedBloodCamp'][2][VAMPIRE_BATS] = {type:TROOP, qty:10000};
    attack_units['RedBloodCamp'][2][SNIPERS] = {type:TROOP, qty:22000};
//    attack_units['RedBloodCamp'][2][ARMORED_TRUCKS] = {type:TROOP, qty:50};
...
//    attack_units['RedBloodCamp'][3][VAMPIRE_BATS] = {type:TROOP, qty:10000};
    attack_units['RedBloodCamp'][3][SNIPERS] = {type:TROOP, qty:25000};
...
//  attack_units['RedBloodCamp'][4][NEMESIS_BATS] = {type:TROOP, qty:3000};
    attack_units['RedBloodCamp'][4][SNIPERS] = {type:TROOP, qty:28000};
    ...
    attack_units['RedBloodCamp'][5][SNIPERS] = {type:TROOP, qty:36000};
    ...
    attack_units['RedBloodCamp'][6][SNIPERS] = {type:TROOP, qty:43000};
    ...
    attack_units['RedBloodCamp'][7][SNIPERS] = {type:TROOP, qty:59000};
    ...
    attack_units['RedBloodCamp'][8][SNIPERS] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][7][ARMORED_TRUCKS] = {type:TROOP, qty:5000};
    ...
    attack_units['RedBloodCamp'][9][NEMESIS_BATS] = {type:TROOP, qty:100000};
    ...
    attack_units['RedBloodCamp'][11][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][12][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][13][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][14][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][15][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][16][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][17][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][18][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][19][NOSFERATU] = {type:TROOP, qty:100000};
    attack_units['RedBloodCamp'][20][NOSFERATU] = {type:TROOP, qty:100000};
    MARCH_DISTANCE = 30;
}
...
function auto_attack_desired(x, y, cell_data) {
    ...
    var shank_loc = game_setting[AUTO_ATTACK_SHANK_LOCATION].value;
    if ((x == shank_loc['x']) && (y == shank_loc['y'])) {
    	return false;
    }
    var pythus_loc = game_setting[AUTO_ATTACK_PYTHUS_LOCATION].value;
    if ((x == pythus_loc['x']) && (y == pythus_loc['y'])) {
    	return false;
    }
...
function auto_attack_thread() {
    if (game_setting[AUTO_ATTACK_STATE].value) {
        getPlayerInfo();
                ...
                for (var y = city.y - MARCH_DISTANCE; y < city.y + MARCH_DISTANCE; y++) {
                    ...
                    if (auto_attack_desired(x, y, cell_data)) {
                        ...
                        if (auto_attack_id_by_location.hasOwnProperty(location_string)) {
                            ...
                            if (attack_object.status == ERROR_IN_REQUEST) {
                                continue;
                            }
        ...
        for (location in auto_attack_id_by_location) {
            ...
            if (attack_object && (attack_object.status != TO_SCHEDULE) && 
                (attack_object.status != RUNNING) && 
                (attack_object.status != COMPLETE) &&
                (attack_object.status != ERROR_IN_REQUEST)) {
