...
var config = {
	auto_hunt: true,			// Vadászat automata folytatasa
	autohunt_wait: 1,			// Vadászat folytatása előtti max várakozás (perc)
	...
	autorandom_idle_time: 1,	// ido (mp) a random valaszolas elott autorandom eseten
	formsubmit_mintime: 1,		// ido (mp) kuldes elott
	formsubmit_rnd: 1			// ido (mp) kuldes elott
};
...
function submit_form(){ 
	if (document.getElementById('vadaszat_kvizform')) {
		document.getElementById('vadaszat_kvizform').submit();
	} else if (getByClass('form', 'kerdoiv_form').length>0) {
		getByClass('form', 'kerdoiv_form')[0].submit();
	}
}
...
vladcaosudu_msg = {
	nothunting: 'Momentálne nie si na love',
	correct: 'Odpoveď je správna',
	hunting: 'Vyberieš sa na lov'
};

soviet_msg = {
	nothunting: 'Сейчас ты не охотишься',
	correct: 'Правильно!',
	hunting: 'Ты на охоте'
};
