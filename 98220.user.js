// ==UserScript==
// @name           LeproTotalCommentsExtended
...
var modes = [
    {
        ...
        name: 'repliesmine',
        title: 'Мои с ответами',
        criteria: isReplyMine
    }
];
...
tbl = document.getElementsByClassName('category')[0].tBodies[0];
...
function sortComments() {
    ...
    for (i=0; i<nodes.length; i++) {
        ...
        if (parent_link.className == 'show_parent') {
			...
			comment.parent_id = parent_link.href.split("#")[1];
        };
        for (j=0; j<modes.length; j++) {
            mode = modes[j];
            if (!counter[mode.name]) counter[mode.name] = 0;
            if (mode.criteria) {
                comment[mode.name] = (mode.criteria)(comment.dom, comment);
...
// Содержит ссылку
function isLinks(node) {
    var content = getCommentText(node);
    return content.match(/<a\shref=("|')/) ? 1 : 0;
}
...
function isVideo(node) {
	...
	'video\\.google\\.com',
	'video\\.yandex\\.ru',
	'(vk\\.com|vkontakte\\.ru)\\/video',
	'smotri\\.com'
	];
    regexp = new RegExp('http\\:\\/(www\\.)?' + implode('|', videos));
    var content = getCommentText(node);
    return content.match(regexp) ? 1 : 0;
}

function isReply(node, comment)
{
	if (node.className.match(/mine/))
	{
		return 0;
	}
	...
	{
		if (document.getElementById(comment.parent_id))
		{
			return document.getElementById(comment.parent_id).className.match(/mine/) ? 1 : 0;
		}
	}
	return 0;
}

function isReplyMine(node, comment)
{
	if (node.className.match(/mine/))
	{
		return 1;
...
// Сам дурак. DileSoft.
function isNotFuckingInboxRequest(comment) { return true;
//!isFuckingInboxRequest(comment) 
}
