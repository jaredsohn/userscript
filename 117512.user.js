...
// @name        LPuNKTKit
// @namespace   lpunktkit
// @description Kit de utilidades para oGame Redesign 4
...
// Mostrar reloj en la barra de links
// Corregido funcionamiento de Smiles y BBCode
// Mostrar calculadora de comercio solo donde corresponde
// Mostrar advertencia del comerciante donde corresponde

// ToDo:
// Corregir resources in flight


const DEBUG_MODE = 0; //0: none; 1: basic; 2: extended; 3: language texts; !=0: errors

const VERSION_LPUNKTKIT = "4.0.0";
...
var color_Metal;                const COLOR_METAL                = '#00FF7F';
var color_Cristal;              const COLOR_CRISTAL              = '#00FF7F';
...
var usarCRFriki;                const USAR_CR_FRIKI              = false;
  ...
  try {
      ...
      else
      {
        ...
        if (comienzos[comienzos.length-1] == "index.php")
          strPaginaActual = partes[0].substring(partes[0].indexOf('=')+1);
...
function loadProduction()
{
  try {
      ...
      var theLi = document.getElementById('metal_box');
      pMetal = theLi.getAttribute('title').replace(/\./g,"");
      pMetal = re.exec(pMetal);
      pMetal = pMetal/60; //produccion por minuto

      var theLi = document.getElementById('crystal_box');
      pCristal = theLi.getAttribute('title').replace(/\./g,"");
      pCristal = re.exec(pCristal);
      pCristal = pCristal/60; //produccion por minuto

      var theLi = document.getElementById('deuterium_box');
      pDeuterio = theLi.getAttribute('title').replace(/\./g,"");
         ...
         */
         function loadWarehouses()
         {
              try {
                   ...
                   var theMetal = document.getElementById('metal_box').getAttribute('title').replace(/\./g,"").split("<br>");
                   aMetal = theMetal[1].substring(theMetal[1].indexOf('>')+1).replace("</span>",'');
                   aMetal = parseInt(aMetal);

                   var theCristal = document.getElementById('crystal_box').getAttribute('title').replace(/\./g,"").split("<br>");
                   aCristal = theCristal[1].substring(theCristal[1].indexOf('>')+1).replace("</span>",'');
                   aCristal = parseInt(aCristal);

                   var theDeuterio = document.getElementById('deuterium_box').getAttribute('title').replace(/\./g,"").split("<br>");
                   aDeuterio = theDeuterio[1].substring(theDeuterio[1].indexOf('>')+1).replace("</span>",'');
...
function addProductionRatio()
{
  try {
       ...
       var pCristalIdeal = (pMetalIdeal/3)*2;
       var pDeuterioIdeal = pMetalIdeal/3;
       ...
       var mySpan1 = document.createElement('span');
           ...
           mySpan1.appendChild(document.createTextNode(puntos(Math.ceil(pMetalIdeal*60))));
       ...
       var mySpan2 = document.createElement('span');
           ...
           mySpan2.appendChild(document.createTextNode(puntos(Math.ceil(pCristalIdeal*60))));
       ...
       var mySpan3 = document.createElement('span');
           ...
           mySpan3.appendChild(document.createTextNode(puntos(Math.ceil(pDeuterioIdeal*60))));
...
//                            var myDiv = document.getElementById('unidadesDiarias');
                            ...
                            for( i = 0; i < listaObjetos.length; i++)
                            {
                                ...
                                var myTd = myTd16.cloneNode(false);
                                    myTd.appendChild(document.createTextNode(puntos(metalAlDia-obj.totalMetalDia)));

                                    myTr5.appendChild(myTd);

                                var myTd = myTd16.cloneNode(false);
                                    myTd.appendChild(document.createTextNode(puntos(cristalAlDia-obj.totalCristalDia)));

                                    myTr5.appendChild(myTd);

                                var myTd = myTd16.cloneNode(false);
                                    myTd.appendChild(document.createTextNode(puntos(deuterioAlDia-obj.totalDeuterioDia)));
   ...
   try {
        ...
        if (existe.length>0)
        {
            ...
            $('#costs #resources li.metal').each(function() {
                ...
                if (DEBUG_MODE > 1) GM_log('calculateTime: [ palabra[palabra.length-1] = ' +
...
function showOptions()
{
  try {
      ...
      var contenido =  '<table width=100% height=100%  >' +
      ...
      contenido +=     '<div id=hdrSection2>' + LANG.MISC.txt_links + ' </div>' + divClear +
                       ...
                       ';padding:1px;width:' + intAnchoLinks2 + 'px;' + strFontSize + '">' +
                       '</input></td></tr>' +
                       '<tr><td></td><td colspan=2 align=left>' + divClear + divClear +
                       '<input type="checkbox" name="set_Clock_Links" ' +
                       'id="set_Clock_Links" ' + '> ' +
                       LANG.OPTION.txt_setClockLinks +
                       '</td></tr>' +
                       '</table>' + hr;
      ...
      contenido +=     '<td><input type="checkbox" name="show_Confirm_Trader" ' +
                       'id="show_Confirm_Trader" '  + '> ' +
      ...
      contenido +=     '<td width=49%><input type="checkbox" name="show_Planet_Nav_Keys" ' +
                       'id="show_Planet_Nav_Keys" '  + '> ' +
                       LANG.OPTION.txt_showPlanetNavKeys + '</td>';

      contenido +=     '<td width=49%><input type="checkbox" name="show_Moons_Right" ' +
                       'id="show_Moons_Right"> ' +
      ...
      contenido +=       '<td width=50%>' +
                         ...
                         'width:150px;color:' + strColor_LPuNKTKit + ';">' +
      ...
      contenido +=     '<td><input type="checkbox" name="reply_CC" ' +
                       'id="reply_CC" '  + '> ' +
      ...
      contenido +=     '<td><input type="checkbox" name="show_Chat" ' +
                       'id="show_Chat" '  + '> ' +
                       LANG.OPTION.txt_chkChat + '</td>';

      contenido +=     '<td>' + LANG.OPTION.txt_idChat +
      ...
      contenido +=     '<td colspan=2><div id=hdrNoEscape><input type="checkbox" name="show_No_Escape" ' +
                       'id="show_No_Escape" '  + '> ' +
      ...
      contenido +=     '<td><input type="checkbox" name="show_Auto_Expo_Fleet" ' +
      ...
      contenido +=    '<tr valign=top><td>' +
                      ...
                      LANG.MISC.txt_keySalto + divClear +
                           '<table border=0 cellpadding=2 cellspacing=0 width=100% ' +
                           'style="color:white;font-size:smaller;">' +
                           '<tr style="color:yellow;"><td width=2%></td>' +
                           '<td width=' + ANCHO1 + '%>' + LANG.MISC.txt_tecla + '</td>' +
                           ...
                           '<table border=0 cellpadding=0 cellspacing=0 width=100% ' +
                             ...
                             '<tr valign=bottom>' +
                             '<td width=100% height=100% align=justify style="font-size:smaller;font-style:italic;">' +
                             '** ' + LANG.MISC.txt_errorKey + '</td>' +
                           '</tr></table>';
      ...
      contenido +=    '<table border=0 cellspaccing=0 cellpadding=0 width=100%>' +
                      ...
                      LANG.MISC.txt_keyMailbox + divClear +
                           ...
                           '<td>N</td><td>' + LANG.MISC.txt_borrarSel + '</td>' +
                           '</tr><tr valign=top><td></td>' +
                           '<td>' + LANG.MISC.txt_borrar + '</td><td>' + LANG.MISC.txt_borrarMens + '</td>' +
      ...
      contenido +=    '<table border=0 cellspaccing=0 cellpadding=0 width=100%>' +
                      ...
                      LANG.MISC.txt_key3Flota + divClear +
                           ...
                           '<td>N</td><td>' + LANG.MISC.txt_borrarSel + '</td>' +
                           '</tr><tr valign=top><td></td>' +
                           '<td>V</td><td>' + LANG.MISC.txt_recInversos + ' (D, C, M)</td>' +
      ...
      if (show_Compactador_Batallas) {
          ...
          LoadColoresCR(usarCRFriki && availCRFriki);

          LoadDatosFlota( usarCRFriki && availCRFriki );
          ...
          if (availCRFriki) {
              ...
              contenido +=     '<input type="radio" name="usarCRFriki" ' +
                              'id="usarCRFriki" disabled style="' + strColorSize + '"> ' +
      ...
      var linkScript = document.createElement('a');
          ...
          linkScript.setAttribute('href', URL_SCRIPT);
...
function saveOptions()
{
  try {
        ...
        if (show_Key_Everywhere || show_Key_Mailbox || show_Key_Fleet)
...
function showLinks()
{
  try {
         ...
         function MostrarHora () {
             try {
                  var theLi = document.getElementById('OGameClock');
                  var theSpan = theLi.getElementsByTagName('span');

                  var mySpan = document.getElementById('lpk-clock');
                  if (mySpan == null)
                      anuncio.append('<span style=\'color:' + strColor_LPuNKTKit + ';\'>' +
                                     ' .: <span id=\'lpk-clock\' style=\'color:' +
                                     strColor_LPuNKTKit + ';\'> ' +
                                     theSpan[0].innerHTML + ' </span> :. </span>')
                  else
                      mySpan.innerHTML = '<span id=\'lpk-clock\' style=\'color:' +
                                         strColor_LPuNKTKit + ';\'> ' +
...
function ResourcesInFlight() {
  try {
         ...
         if (document.location.href.indexOf('/game/index.php?page=movement') >= 0)
         {
                 ...
                 function doTable ()
                 {
                   if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable: ' + strPaginaActual);

                   function createFontElement (strText, strColor)
                   {
                     if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable >> createFontElement: ' + strPaginaActual);
                   ...
                   {
                     if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable >> createCell: ' + strPaginaActual);
                   ...
                   {
                     if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable >> addEvent: ' + strPaginaActual);
                 ...
                 {
                   if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable >> redrawTable: ' + strPaginaActual);
                 ...
                 {
                   if (DEBUG_MODE > 1) GM_log('ResourcesInFlight >> doTable >> getResName: ' + strPaginaActual);
           ...
           $ (document).keydown (function (e)
           {
             ...
             switch (e.keyCode)
             {
               ...
               case 39:  // "->"
                 ...
                 break;
               case 66:  // "b"
                 if (e.shiftKey || e.ctrlKey || e.altKey || (! show_Key_Everywhere))
                   break;
               ...
               case 72:  // "h"
                 ...
                 if (e.shiftKey)
                 {
                       ...
                       (show_Key_Fleet))
                   {
                     ...
                     if (show_Key_Everywhere) $ ("a[href*='page=search']").click ();
                 return false;
                 break;
               case 75:  // "k"
                 if (e.ctrlKey || e.altKey)
               ...
               case 79:  // "o"
                 if (e.ctrlKey || e.altKey || (! show_Key_Everywhere))
                   break;
...
function BBCode() {
  ...
  try {
     if ((strPaginaActual != 'writemessage') &&
         (strPaginaActual != 'showmessage') &&
         (strPaginaActual != 'alliance') &&
         (strPaginaActual != 'networkkommunikation'))
         return;

     if (! show_BBCode) return;

     //No mostrarlos en reportes de espionaje
     if (strPaginaActual == 'showmessage')
         if (document.getElementsByClassName('material spy').length > 0) {
            if (DEBUG_MODE > 0) GM_log('BBCode [Spy Report]: ' + strPaginaActual);
...
/*     script.text =
     ...
     if (DEBUG_MODE > 1) GM_log('BBCode: [ script added ] ' + strPaginaActual);

     function funcBBCode() {
       if (DEBUG_MODE > 1) GM_log('BBCode >> funcBBCode: ' + strPaginaActual);
...
//         messageBox.style.height = "508px";
     ...
     if (document.location.href.indexOf("page=alliance") != -1) {
         ...
         if (document.location.href.indexOf("page=alliance&tab=broadcast") != -1) {
             var $;
             try { $ = unsafeWindow.$; }
             catch(e) { $ = window.$; }
...
/*     script.text =
     ...
     if (DEBUG_MODE > 1) GM_log('NewSmiles: [ SCRIPT ADDED ] ' + strPaginaActual);

     function hasClass (element, className) {
       if (DEBUG_MODE > 1) GM_log('NewSmiles >> hassClass: ' + strPaginaActual);
       return ( (" " + element.className + " ").replace(/[\n\t]/g, " ").indexOf(" " + className + " ") > -1);
     }

     function funcSmilies() {

       if (DEBUG_MODE > 1) GM_log('NewSmiles >> funcSmiles: ' + strPaginaActual);

       var form = document.getElementsByTagName("form")[0];
       if (! form) return;

       if (DEBUG_MODE > 1) GM_log('NewSmiles >> funcSmiles: [ form FOUND ] ' + strPaginaActual);
       ...
       for (var i = 0; i < smilies.length; i++) {
         ...
         if (DEBUG_MODE > 2) GM_log('NewSmiles >> funcSmiles: [ innerHTML = ' + div.innerHTML + ' ] ' + strPaginaActual);
     ...
     if (document.location.href.indexOf("page=showmessage") != -1) {
         ...
         if (note && document.getElementById("melden")) {
             ...
             function rep_smilies(value,index) {
                      if (DEBUG_MODE > 1) GM_log('NewSmiles >> rep_smilies: ' + strPaginaActual);
                      ...
                      for (var i = index; i < smilies.length; i++) {
                           ...
                           if (pos != -1) {
                               ...
                               text = rep_smilies(part1,i) +
                                      "<img src=\"" + smilies[i][1] + "\" alt=\"" +
             ...
             function sort_smilies(a,b) {
                      if (DEBUG_MODE > 1) GM_log('NewSmiles >> sort_smilies: ' + strPaginaActual);
             ...
             $(".note .tipsPlayer").cluetip("destroy").cluetip(
                 { splitTitle:"|", showTitle:true, cluetipClass:"player",
                   positionBy:"mouse", leftOffset:15, topOffset:10,
                   cluezIndex:9997, width:"auto", sticky:true, delayedClose:500,
                   hoverIntent:false, clickThrough:true, mouseOutClose:true});
         }
     }
...
function CompactadorBatallas()
{
   try {
         ...
         function getMensajeConclusion() {
             ...
             if(str.indexOf(LANG.SERVER.txt_CR_Defender.toLowerCase()) != -1)
                ret = LANG.CR.txt_DefWin;

             if(str.indexOf(LANG.CR.txt_Draw) != -1)
         ...
         patron += '{COLOR_IN}{SIZE_PEQ}' + LANG.CR.txt_battleDay + ' ' + getFecha() +
                   '{/SIZE}{/COLOR_O}{NL}{NL}';
         ...
         for(var i = 0; i < lstAtaq.length(); i++){
             ...
             for(var j = 0; j < lstAtaq.getFlotas(i).length(); j++) {
                 ...
                 patron += nombre + " {COLOR_A1}" + unidades +
                           "{/COLOR} {COLOR_A2}" + LANG.CR.txt_lost + " " + perdidas +
                           "{/COLOR}{NL}";
             }
             ...
             //if(lstAtaq.getCostePerdidas(i)[3] != 0) {
                ...
                if (usarCRFriki) {
                  patron += '({I}{COLOR_R2} ' + coste[0] +
                          ' {/COLOR} ' + LANG.CR_FRIKI.txt_RES_metal + ',  {COLOR_R2} ' + coste[1] +
                          ' {/COLOR} ' + LANG.CR_FRIKI.txt_RES_cristal + ', {COLOR_R2} ' + coste[2] +
                          ' {/COLOR} ' + LANG.CR_FRIKI.txt_RES_deuterio + '{/I}){NL}{NL}';
                } else {
                  patron += '({I}{COLOR_R2} ' + coste[0] +
                          ' {/COLOR} ' + LANG.SERVER.txt_RES_metal + ',  {COLOR_R2} ' + coste[1] +
                          ' {/COLOR} ' + LANG.SERVER.txt_RES_cristal + ', {COLOR_R2} ' + coste[2] +
                          ' {/COLOR} ' + LANG.SERVER.txt_RES_deuterio + '{/I}){NL}{NL}';
                }
             //}
         }
         ...
         for(var i = 0; i < lstDef.length(); i++){
             ...
             for(var j = 0; j < lstDef.getFlotas(i).length(); j++) {
                 ...
                 patron += nombre + " {COLOR_D1} " +
                           unidades + " {/COLOR} {COLOR_D2}" + LANG.CR.txt_lost + " " +
                           perdidas +  "{/COLOR}{NL}";
             }
             ...
             //if(lstDef.getCostePerdidas(i)[3] != 0) {
                ...
                if (usarCRFriki) {
                   patron += '({I}{COLOR_R2} ' + coste[0] +
                          ' {/COLOR} {COLOR_IN}' + LANG.CR_FRIKI.txt_RES_metal + ', {/COLOR_O}{COLOR_R2} ' +
                          coste[1] + ' {/COLOR} {COLOR_IN}' + LANG.CR_FRIKI.txt_RES_cristal +
                          ', {/COLOR_O}{COLOR_R2} ' + coste[2] + ' {/COLOR} {COLOR_IN}' +
                          LANG.CR_FRIKI.txt_RES_deuterio + '{/COLOR_O}{/I}){NL}{NL}';
                } else {
                   patron += '({I}{COLOR_R2} ' + coste[0] +
                          ' {/COLOR} {COLOR_IN}' + LANG.SERVER.txt_RES_metal + ', {/COLOR_O}{COLOR_R2} ' +
                          coste[1] + ' {/COLOR} {COLOR_IN}' + LANG.SERVER.txt_RES_cristal +
                          ', {/COLOR_O}{COLOR_R2} ' + coste[2] + ' {/COLOR} {COLOR_IN}' +
                          LANG.SERVER.txt_RES_deuterio + '{/COLOR_O}{/I}){NL}{NL}';
                }
             //}
         }
         ...
         if (usarCRFriki) {
            patron += '{B}{COLOR_T1}{SIZE_MED}' + LANG.CR.txt_stolen +
                   ':{/SIZE}{/COLOR_T}{/B}{NL}{NL}{COLOR_R4}{SIZE_MED} ' +
                   N_captura[0] + ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR_FRIKI.txt_RES_metal +
                   ', {/COLOR_O}{COLOR_R4}{SIZE_MED} ' + N_captura[1] +
                   ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR_FRIKI.txt_RES_cristal + ' ' +
                   LANG.SERVER.txt_CR_and + ' {/COLOR_O}{COLOR_R4}{SIZE_MED} ' + N_captura[2] +
                   ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR_FRIKI.txt_RES_deuterio + '{/COLOR_O}{NL}{NL}{NL}';
         } else {
            patron += '{B}{COLOR_T1}{SIZE_MED}' + LANG.CR.txt_stolen +
                   ':{/SIZE}{/COLOR_T}{/B}{NL}{NL}{COLOR_R4}{SIZE_MED} ' +
                   N_captura[0] + ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.SERVER.txt_RES_metal +
                   ', {/COLOR_O}{COLOR_R4}{SIZE_MED} ' + N_captura[1] +
                   ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.SERVER.txt_RES_cristal + ' ' +
                   LANG.SERVER.txt_CR_and + ' {/COLOR_O}{COLOR_R4}{SIZE_MED} ' + N_captura[2] +
                   ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.SERVER.txt_RES_deuterio + '{/COLOR_O}{NL}{NL}{NL}';
         }

         patron += '{COLOR_T1}{SIZE_MED}{B}' + LANG.CR.txt_loses +
                   ': {/B}{/SIZE}{/COLOR_T}{NL}{NL}{COLOR_T1}' + LANG.CR.txt_attFleet +
                   ':{/COLOR_T} {COLOR_A1}{SIZE_MED} ' + N_perdidasAtaq[3] +
                   ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR.txt_units + '.{/COLOR_O}{NL}';

         patron += '{COLOR_T1}' + LANG.CR.txt_defFleet + ':{/COLOR_T} {COLOR_D1}{SIZE_MED} ' +
                   N_perdidasDef[3] + ' {/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR.txt_units +
                   '.{/COLOR_O}{NL}{NL}';

         patron += '{COLOR_T1}' + LANG.CR.txt_totLoses + ':{/COLOR_T} {B}{COLOR_R1}{SIZE_MED} ' +
                   N(perdidasTotales[3]) + ' {/SIZE}{/COLOR}{/B}{COLOR_IN} ' + LANG.CR.txt_units +
                   '.{/COLOR_O}{NL}{NL}{NL}';

         if (usarCRFriki) {
            patron += '{COLOR_T1}{SIZE_MED}{B}' + LANG.CR.txt_debris + ': {/B}{/SIZE}{/COLOR_T}{NL}{NL}' +
                   '{COLOR_R3}{SIZE_GRA}{B} ' + N_escombros[0] +
                   ' {/B}{/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR_FRIKI.txt_RES_metal + ' ' + LANG.SERVER.txt_CR_and +
                   ' {/COLOR_O}{COLOR_R3}{SIZE_GRA}{B} ' + N_escombros[1] +
                   ' {/B}{/SIZE}{/COLOR}{COLOR_IN} ' + LANG.CR_FRIKI.txt_RES_cristal + ' [ {/COLOR_O}{COLOR_R3} ' +
                   N(calcularRecicladores(escombros[2])) + " {/COLOR}{COLOR_IN} " +
                   LANG.CR_FRIKI.txt_recys + " ]{/COLOR_O}{NL}";
         } else {
            patron += '{COLOR_T1}{SIZE_MED}{B}' + LANG.CR.txt_debris + ': {/B}{/SIZE}{/COLOR_T}{NL}{NL}' +
                   '{COLOR_R3}{SIZE_GRA}{B} ' + N_escombros[0] +
                   ' {/B}{/SIZE}{/COLOR}{COLOR_IN} ' + LANG.SERVER.txt_RES_metal + ' ' + LANG.SERVER.txt_CR_and +
                   ' {/COLOR_O}{COLOR_R3}{SIZE_GRA}{B} ' + N_escombros[1] +
                   ' {/B}{/SIZE}{/COLOR}{COLOR_IN} ' + LANG.SERVER.txt_RES_cristal + ' [ {/COLOR_O}{COLOR_R3} ' +
                   N(calcularRecicladores(escombros[2])) + " {/COLOR}{COLOR_IN} " +
                   LANG.CR.txt_recys + " ]{/COLOR_O}{NL}";
         }

         patron += '{NL}{NL}';

         if(escombros[2] > 0) {
            patron += '{COLOR_T1}{SIZE_MED}{B}' + LANG.CR.txt_profit +
                      ': {/B}{/SIZE}{/COLOR_T}{NL}{NL} {COLOR_T1}{B}' + LANG.CR.txt_attHarvest +
                      '{/B}:{/COLOR_T} {COLOR_RA1}{B} ' + N_renta_ataq_conReci[3] + ' {/B} [' +
                      p_renta_ataq_conReci[3] + '%]{/COLOR}{NL}';

            if (usarCRFriki) {
              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_metal + ':{/COLOR_T} {COLOR_RAM1} ' +
                      N_renta_ataq_conReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_cristal + ':{/COLOR_T} {COLOR_RAC1} ' +
                      N_renta_ataq_conReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RAD1} ' +
                      N_renta_ataq_conReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';

              patron += '{COLOR_T1}{B}' + LANG.CR.txt_attNoHarvest +
                      '{/B}:{/COLOR_T} {COLOR_RA2}{B} ' + N_renta_ataq_sinReci[3] +
                      ' {/B} [' + p_renta_ataq_sinReci[3] + '%]{/COLOR}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_metal + ':{/COLOR_T} {COLOR_RAM2} ' +
                      N_renta_ataq_sinReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_cristal + ':{/COLOR_T} {COLOR_RAC2} ' +
                      N_renta_ataq_sinReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RAD2} ' +
                      N_renta_ataq_sinReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';

              patron += '{COLOR_T1}{B}' + LANG.CR.txt_defHarvest + '{/B}:{/COLOR_T} {COLOR_RD}{B} ' +
                      N_renta_def_conReci[3] + ' {/B} [' + p_renta_def_conReci[3] +
                      '%]{/COLOR}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_metal + ':{/COLOR_T} {COLOR_RDM} ' +
                      N_renta_def_conReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_cristal + ':{/COLOR_T} {COLOR_RDC} ' +
                      N_renta_def_conReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.CR_FRIKI.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RDD} ' +
                      N_renta_def_conReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';
            } else {
              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_metal + ':{/COLOR_T} {COLOR_RAM1} ' +
                      N_renta_ataq_conReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_cristal + ':{/COLOR_T} {COLOR_RAC1} ' +
                      N_renta_ataq_conReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RAD1} ' +
                      N_renta_ataq_conReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';
              ...
              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_metal + ':{/COLOR_T} {COLOR_RAM2} ' +
                      N_renta_ataq_sinReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_cristal + ':{/COLOR_T} {COLOR_RAC2} ' +
                      N_renta_ataq_sinReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RAD2} ' +
                      N_renta_ataq_sinReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';
              ...
              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_metal + ':{/COLOR_T} {COLOR_RDM} ' +
                      N_renta_def_conReci[0] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_cristal + ':{/COLOR_T} {COLOR_RDC} ' +
                      N_renta_def_conReci[1] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}';

              patron += '{COLOR_T1}' + LANG.SERVER.txt_RES_deuterio + ':{/COLOR_T} {COLOR_RDD} ' +
                      N_renta_def_conReci[2] + ' {/COLOR}{COLOR_IN} ' +
                      LANG.CR.txt_units + '.{/COLOR_O}{NL}{NL}';
            }
...
function MissingSatsInprove(tech)
{
  try {
         ...
         function showMissingSats ()
         {
           ...
           if (theA.getAttribute('onclick').indexOf('techID=' + tech) < 0)
               return;
           ...
           $('#costs #resources li.metal').each(function() {
                ...
                switch (palabra[palabra.length-1]) {
                   ...
                   default: energyNeeded = palabra[0].substring(1,
                                            palabra[0].length).replace(/\./g,"");
...
//             (url.indexOf ("/game/index.php?page=traderlayer")       >= 0) ||
         ...
         function showResInfo (resName)
         {
             ...
             if (resName == "energy") {
                 mySpan.style.color = color_Res_Prod; //"#99CC00";
                 mySpan.setAttribute("Id","energy_produced");
             }
             else {
                 mySpan.style.color = color_Res_Almacen; //"#DAA520"; //"rgb(153, 153, 153)";
             ...
             if ((resName != "energy") && (parseInt(getVersionOgame()[0]) == 3) &&
                (myMatch [5]>0) ) {
                  ...
                  mySpan = document.createElement ("span");
                  mySpan.style.color = color_Res_Den; //'rgb(135, 206, 250)';      //#87CEFA

                  if (myMatch [5].length > 7)
                     mySpan.style.fontSize = "0.9em";
             ...
             if (resName=="energy") {
                 mySpan.style.color = color_Energy_Used; //"crimson";
                 mySpan.className = "";
             }
...
function ResourceLoadButtons()
{
  try {
      ...
      myDiv.setAttribute('title', LANG.MISC.txt_recInversos);
      ...
      myDiv.setAttribute('title', LANG.MISC.txt_borrarSel);
...
function ConfirmTrader()
{
  try {
       // The following "if" is not really necessary but with it this script
       // will work for Opera too
       if ((document.location.href.indexOf ("/game/index.php?page=traderOverview") == -1) ||
           (document.location.href.indexOf ('page=traderResources') == -1))
         return;
...
//       var buttonzDiv = document.getElementById ("buttonz");
       var buttonzDiv = document.getElementById ("div_traderResources");

       if (buttonzDiv == null)
         return;

       var costsDiv = getElementsByClass('costs',buttonzDiv,'div');

       if (costsDiv.length < 1)
         return;

       var warningText = costsDiv[0].innerHTML + '!';
       ...
       {
         try {
              if (DEBUG_MODE > 1) GM_log('ConfirmTrader >> fixPopUp: ' + strPaginaActual);
              ...
              if (DEBUG_MODE != 0) GM_log('ConfirmTrader >> fixPopUp [ERROR]: <' + e + '> ' + strPaginaActual);
         }
  ...
  try {
       if (typeof GM_setValue == 'undefined') {
            if (
              ...
              && location.href.indexOf('/game/index.php?page=traderOverview') == -1
       ...
       var borderImageBold = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAJMA" +
                             "AAA0CAYAAACO/ApDAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjw" +
                             "v8YQUAAAAgY0hSTQAAeiYAAICEAAD6AAAAgOgAAHUwAADqYAAA" +
                             "OpgAABdwnLpRPAAAAR9JREFUeF7t0rsNhDAABUGo6ejparqejp" +
                             "r4BDhH3nCcgCV4wWrW3/9zLNf5bvt6Px0F3hZ4DC33y7i8XfG9" +
                             "AleBYQgmHmYLwDRb0P+jAEwwZAVgylIagomBrABMWUpDMDGQFY" +
                             "ApS2kIJgayAjBlKQ3BxEBWAKYspSGYGMgKwJSlNAQTA1kBmLKU" +
                             "hmBiICsAU5bSEEwMZAVgylIagomBrABMWUpDMDGQFYApS2kIJg" +
                             "ayAjBlKQ3BxEBWAKYspSGYGMgKwJSlNAQTA1kBmLKUhmBiICsA" +
                             "U5bSEEwMZAVgylIagomBrABMWUpDMDGQFYApS2kIJgayAjBlKQ" +
                             "3BxEBWAKYspSGYGMgKwJSlNAQTA1kBmLKUhmBiICsAU5bS0IPp" +
                             "BGCvFwvXG6NqAAAAAElFTkSuQmCC";
       ...
       setStyleSet ("div#cutty " + (myPlanets ? "#myPlanets" : "#myWorlds") + " .smallplanet {height:38px;}" +
                    ...
                    "div#cutty " + (myPlanets ? "#myPlanets" : "#myWorlds") + " .smallplanet a.constructionIcon.moon{top:25px;}" +
                    "div#cutty " + (myPlanets ? "#myPlanets" : "#myWorlds") + " .smallplanet:hover {" +
                        "background-image:url('" + borderImage + "'),url('" + borderImage + "');" +
                        "background-position:left top, left bottom;}" +
//                        "border:1px dashed rgb(131,145,156);}" +
                    ...
                    "#rechts #cutty " + (myPlanets ? "#myPlanets" : "#myWorlds") + " .smallplanet a.active:link{" +
                        "background-image:url('" + borderImageBold + "'), url('" + borderImageBold + "');" +
                        "background-position:left top, left bottom;}" +
  ...
  try {
       ...
       if (myDiv == null) {
           ...
           var strContenido =
             ...
             '                  </td>' +
             '                  <td align=right id="lpunktkit-resMetal"><span>0</span></td>' +
             '                  <td align=right id="lpunktkit-resCristal"><span>0</span></td>' +
             '                  <td align=right id="lpunktkit-resDuty"><span>0</span></td>' +
             '              </tr>' +
             '              </table>' +
             '          </td>' +
             '      </tr>' +
             '      <tr valign=top>' +
             '          <td width=100% id="lpunktkit-traderButton" align=center></td>' +
             '      </tr>' +
             '      </table>' +
             '   </td>' +
             '   <td width=8%></td>' +
             ...
             '        <table border=0 width=100%>' +
             '        <tr valign=top>' +
             '            <td width=70% id="lpunktkit-error" align=left></td>' +
...
var LANG_ES = {
  ...
  MISC : {
          ...
          txt_keySalto:'En la p\u00e1gina de Salto cu\u00e1ntico',
          ...
          txt_errorKey:'El script no funciona correctamente en estos casos. La p\u00e1' +
  ...
  OPTION : {
          ...
          txt_chkChat:'Mostrar Chat de Xat.com',
...
var LANG_EN = {
  ...
  MISC : {
          ...
          txt_errorKey:'The script does not work properly in these cases. The correspo' +
  ...
  OPTION : {
        ...
        txt_useSmiles:'Smiles',
        txt_chkChat:'Show Xat.com\'s chat',
        txt_idChat:'Id chat',
...
var LANG_FR = {
  ...
  MISC : {
        ...
        txt_keySalto:'Sur la page de Jumpgate',
        ...
        txt_errorKey: 'Le script ne fonctionne pas correctement dans ces cas. La page' +
  ...
  OPTION : {
        ...
        txt_useSmiles:'Smiles',
        txt_chkChat:'Montrer Xat.com discussion',
        txt_idChat:'Id chat',
