...
// JavaScript Document
// ==UserScript==
// @name           Ikariam Arkadaş Listesi
// @autor          roselan
// @version        0.1
// @license        GNU GPL v3 (http://www.gnu.org/copyleft/gpl.html)
// @namespace      ikariamScript
// @description    İkariam arkadaş listesinde arkadaşlarınızı nasıl görmek isterseniz.
...
$(document).ready(function(){
	...
	function optionPlant () {
		...
		options += "<h3>İkariam Arkadaş Listesi (www.ikariam.forumm.biz)</h3>";
		...
		$("#friends > .friends > .friend").each( function () {
			...
			console.log(options);
			options += "<a class=atextlabel size=4>kullan</a>";
			options += "</td>";
			options += "</tr>";
		});
		...
		options += "<th>"+"Boyut";
		options += "<td>";
		options += "<input id=r1 type=radio name=friendFaceStyle value=large " + ((style == "buyuk") ? "checked" : "") + ">buyuk"+"</input>";
		options += "&nbsp;&nbsp;&nbsp;"
		options += "<input id=r2 type=radio name=friendFaceStyle value=small " + ((style == "kucuk") ? "checked" : "") + ">kucuk"+"</input>";
		options += "</td>";
		options += "</tr>";
