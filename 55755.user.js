...
window.dowork = function() {
    ...
    if ( !elements[0].className.match( 'downloadable' ) )
        for ( i=0; i<elements.length; i++ )
        {
            ...
            if (!r)
            {
                r = e.getAttribute('onclick').match( /ate(?:Wall)?\((\d+)\,'([^']+)/ );
                if ( !r ) continue;
            }
            c = e.parentNode.parentNode.childNodes[2] ? 2 : 1; //opera hack
            if (r.length == 3)
                e.parentNode.parentNode.childNodes[c].innerHTML += '<br/><a href="'+r[2]+'">'+window.download_i18n+'</a>';
            else
