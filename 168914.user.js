...
var attackSpeed   = parseInt(GM_getValue("attackSpeed", 1000));
var numberAttacks = parseInt(GM_getValue("numberAttacks", 0));
...
<input type=\"checkbox\" id=\"autoattack\" "+aachecked+" /> Auto attack player <input type=\"text\" style=\"border: 1px solid blue; width: 4em; color: #5858FA; background: black; text-align: center;\" id=\"numberAttacks\" value=\""+numberAttacks+"\" /> times with <input type=\"text\" style=\"border: 1px solid blue; width: 4em; color: #5858FA; background: black; text-align: center;\" id=\"attackSpeed\" value=\""+attackSpeed+"\" /> ms between each attack<br>\
    \
...
function checkOptions()
{
	...
	//autoAttack
	if (document.getElementById('numberAttacks').checked)
			GM_setValue("autoattack", true);
	else
			GM_setValue("autoattack", false);

	var oldNumAttacks = numberAttacks;
	var newNumAtacks = document.getElementById('numberAttacks').value;
	if (oldNumAttacks != newNumAtacks)
	{
		numberAttacks = newNumAtacks;
		GM_setValue("numberAttacks",parseInt(numberAttacks));
	}
	var oldAttackSpeed = attackSpeed;
	var newAttackSpeed = document.getElementById('attackSpeed').value;
	if (oldAttackSpeed != newAttackSpeed)
	{
		attackSpeed = newAttackSpeed;
		GM_setValue("attackSpeed",parseInt(attackSpeed));
...
function click_list()
{
	click(document.getElementById('bountyForm').getElementsByClassName('btnMed.attackOptionBtn')[0]);
	click(document.getElementById('bountyForm').getElementsByClassName('btnMed.attackOptionBtn')[0]);
	click(document.getElementById('bountyForm').getElementsByClassName('btnMed.attackOptionBtn')[0]);
	click(document.getElementById('bountyForm').getElementsByClassName('btnMed.attackOptionBtn')[0]);
	click(document.getElementById('bountyForm').getElementsByClassName('btnMed.attackOptionBtn')[0]);
...
if (autoattack)
	if (page != home)
		document.location = 'http://rl.storm8.com/home.php';
	//click on race
	click(getElementsByClassName('btnMed.attackOptionBtn'));
