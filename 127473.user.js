// ==UserScript==
// @name         The West - Market Helper PL
...
var dyMarketHelperCode = function (currentVersion, scriptWebsite) {
	...
	function initializeParams() {
		var initTown = {"name": "Pokaż moje położenie", "x": 0, "y": 0, "extraClass": prefix + "Current", "id": "currentLocation"},
			initCoordsX = 0,
			initCoordsY = 0;
		lastPageParsed = -1;
		townInfo = [];
	...
	function createTabHelpContent() {
		...
		if ($helpContent.length === 0) {
			helpContentHtml = '<b>OGÓLNIE</b><br/>' +
				'WERSJA: ' + currentVersion + '<br/>' +
				'STRONA: <a href="' + scriptWebsite + '" target="_blank" title="' + scriptWebsite + ', abrir em nova aba">' + scriptWebsite + '</a><br/>'
				'<br/>';
	...
	function marketHelperTabClick(tabObj, tabId) {
		...
		if (tabId === prefix) {
			setMessage("Miasta, w których masz przedmioty do odebrania");
		} else {
			displayHelpData = true;
			setMessage("O skrypcie");
		}
		...
		if (location.items) {
			...
			if (displayHeadingInfo) {
				...
				if (playerAtLocation) {
					...
					itemCollectHtml = "Aby odebrać wszystkie przedmioty musisz być w mieście: " + locationName + " ";
				}
			...
			for (itemId in locationItems) {
				...
				if (!isNaN(itemId) && itemId > 0) {
					...
					locationItemTitle = locationItemTitle + '<br/> Ilość: ' + + locationItemCountToCollect + ', ' +
						locationItemCountInAuction + ' jest jeszcze na aukcjach';
	...
	function townPurchasedItemsWindow(location) {
		...
		$tabHeader = jQuery("<h2>").attr({"class": prefix + "Message"}).html("Pozycje zakupione w mieście " + locationName);
	...
	function paintTown(location) {
		...
		$townEl = jQuery("<div>")
			.attr({"id": baseLocationId + "OnMap", "class": setClass, "style": townStyle, "title": "Pokaż na mapie: " + locationName})
			.click(function () {
		...
		$locationTownCenter = jQuery("<div>")
			.attr({"class": prefix + "TownCenter", "title": "Pokaż na mapie: " + locationName})
			.click(function () {
				WMap.scroll_map_to_pos(locationX, locationY);
			});
		...
		$locationCollect = jQuery("<div>")
			.attr({"class": baseId + "Collect", "title": "Kliknij, aby zobaczyć jakie przedmioty możesz odebrać z miasta " + locationName})
			.click(function () {
				townPurchasedItemsWindow(location);
			});
		if (location.itemsCount && location.itemsCount > 0) {
			$locationTown
				.attr({"title": "Pokaż miasto " + locationName})
				.click(function () {
	...
	function populateFiltersBlock($filters) {
		...
		purchasedItemsAllLabel = '<label for="' + prefix + 'PurchasedItemsFilterAll" ' +
			'title="Pokaż wszystkie przedmioty, które zakupiłeś">Wszystko</label>';
		//finish
		...
		purchasedItemsFinishLabel = '<label for="' + prefix + 'PurchasedItemsFilterFinish" ' +
			'title="Pokaż tylko przedmioty, które zostały juz kupione.">Natychmiastowy zakup</label>';
		//progress
		...
		purchasedItemsProgressLabel = '<label for="' + prefix + 'PurchasedItemsFilterProgress" ' +
			'title="Pokaż tylko aukcje, w których bierzesz udział">Aukcje</label>';
	...
	function plotData() {
		...
		setMessage("Miasta,z którymi nawiązałeś kontakty handlowe");
