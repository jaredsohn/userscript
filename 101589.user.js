...
// @name           Boo
// @version        1
// @namespace      http://userscripts.org/scripts/show/101589
// @description    Nothing
...
function updateTitle(Rmins, Rsecs){
   ...
   if(firstPage==1){
   	...
   	if (parseInt(GM_getValue('titleLastUpdated', '0')) + 5000 <= (new Date().getTime())){
      	GM_setValue('titleLastUpdated', new Date().getTime()+'');
         if(GM_getValue('alertDisabled', '0')==0)
 				setTimeout(function() {window.open('http://apps.facebook.com/mousehunt/turn.php'} , ((Rmins*60000)+(Rsecs*1000)+1000));
   		firstPage=1;
   	}
   }
   if(runOnce==0){
...
if(timervalue > 0)
{
   ...
   if(firstPage==1){
      ...
      if(GM_getValue('alertDisabled', '0')==0)
   		setTimeout(function() {window.open('http://apps.facebook.com/mousehunt/turn.php'} , timeoutvalue);
   }
}
else{
   getHornTime();
