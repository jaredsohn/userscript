// ==UserScript==
// @author cleverguide
// @name         DS Produktions-Info
// @description  V0.03 | PA-Only: Zeigt auf der Produktionsseite, das Datum der Rekrutierung an. Fügt eine Zeile mit den Summen aller Rohstoffe der angezeigten Dörfer, der freien Rohstoffmenge, der freien Bauernhofplätze, der nächst endende Bau- und Rekrutierungsauftrag an.
// @namespace none
...
function overview_production_table(){
	...
	if (!selTab) GM_log("overview-id nicht vorhanden!")
	if (selTab != 'prod') { GM_log("Produktions-Tab nicht aktiv"); return }
	...
	if (!table) {
		GM_log("Tabelle mit Dorf-Zelle nicht vorhanden")
		return
	}
	if (table.rows.length <= 1) {
		GM_log("Dorf-Tabelle hat zuwenig Einträge")
	}
...
function overview_table_update(table){
	...
	for (var r=0;r<table.rows.length;r++){
		...
		if (coords){
			...
			var res = cell.innerHTML.match(/((?:heute|morgen|am)[^>]*)<br>/)
...
*/
	...
	cell.innerHTML = villageCount+"-Dörfer"
  ...
  cell.innerHTML =
	...
	cell = row.appendChild(document.createElement("td"));
  cell.innerHTML = formatNumber(sumAll[3]) +' frei'

	cell = row.appendChild(document.createElement("td"));
  cell.innerHTML = sumAll[4] +' frei'
...
function hvmShortcuts(villageID) {
	return	'<a href="/game.php?village=' + villageID + '&screen=main">H</a>'
					+	'<a href="/game.php?village=' + villageID + '&screen=place">V</a>'
					+ '<a href="/game.php?village=' + villageID + '&screen=market">M</a> '
}
...
function formatNumber(nr, color){
  ...
  if( nr == 0 )
    return "0";
		...
		return ""+(Math.round(nr/100000)/10) +" Mio."
	}
  var ret = ""
  do
  {
