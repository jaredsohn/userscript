...
// @description     Super Golden is a combination of several Greasemonkey scripts that makes your HKGolden experience more enjoyable...
// @include         http://forum*.hkgolden.com/*
// ==/UserScript==

var SuperGolden = {

   /** Change the title to nicer format. Credit: !![$_$]!! **/
      ...
      for (var i=0; i < tdList.length; i++) {
         if (tdList[i].getAttribute('style') != null){
            if (tdList[i].getAttribute('style').indexOf('font-weight: bold;')>=0 && tdList[i].getAttribute('style').indexOf('150px;')>=0) {
               temp = unescape(tdList[i+1].innerHTML);
               temp = '[HKG]' + temp;
               document.title = temp;
               break;
            }		
   ...
   /** Auto redirect away from default.asp. Credit: !![$_$]!! **/
   autoRedirect : function() {
      loc = document.location;
      if (loc.hostname == 'forum.hkgolden.com' || loc.hostname == 'forum2.hkgolden.com') {
         var tmp = 'http://' + loc.hostname + '/topics.asp?type=BW';
         location.replace(tmp);
      }
   },

   /** Ban/Unban user, in ease. Partial credit: !![$_$]!! **/
   banUsers : function() {
      var userList;
      var cookieArr = document.cookie.split(';');
      for (var i = 0; i < cookieArr.length; i++) {
         if (cookieArr[i].indexOf('banned=') >= 0) {         
            userList = cookieArr[i].slice(8).split(',');
            break;
         }
      }
      if (!userList)
      ...
      var message = 'Hidden by Super Golden. ';
      var tdList = document.getElementsByTagName("td");
      for (var i = 0; i < tdList.length; i++) {         
         var mystyle = '' + tdList[i].getAttribute('style');         
         for (var j = 0; j < userList.length; j++) {
            if (userList[j] == '0')
               continue;           
            if (userList[j] && mystyle.indexOf('120px') >= 0 && tdList[i].innerHTML.indexOf('id='+userList[j]) > 0) {
               var blockedId = Math.floor(Math.random()*100000);
               var html = '<hr color="#888" size="1">' + tdList[i+4].innerHTML + '<hr color="#888" size="1">';
               html = message + '<a href="#" onclick="' +
                  "var blocked = document.getElementById('blocked"+blockedId+"');" +
                  "if (blocked.style.display == 'none'){ " +
                  ...
                  "} else {" +
                     ...
                     "this.innerHTML = '[show]';" +
                  "} return false; " +
               '">[show]</a><br><span style="display:none" id="blocked'+blockedId+'">'+html+'</span>';
               tdList[i+4].innerHTML = html;
               tdList[i+4].setAttribute('valign', 'top');
            } 
            if (mystyle == 'width: 100%; text-align: right;') {
               tdList[i].setAttribute('valign', 'bottom');
               tdList[i].setAttribute('height', '10px');
            }
         }
         if (mystyle.indexOf('120px') >= 0 && tdList[i].innerHTML.indexOf('id=') > 0) {
            var startPos = tdList[i].innerHTML.indexOf('id=');
            var endPos = tdList[i].innerHTML.indexOf("')", startPos);
            var userId = tdList[i].innerHTML.substring(startPos, endPos);
            userId = userId.replace('id=', '');
            var randomId = Math.floor(Math.random()*100000);
            var html = tdList[i].innerHTML;
            html = html.replace('</a>', '</a> <a href="#" onclick="' + 
               "var cookieArr = document.cookie.split(';');" +
               "var cc; var msg;" +
               "for (var j=0; j < cookieArr.length; j++) {" +
                  "if (cookieArr[j].indexOf('banned=') >= 0) {" +
                     "if (cookieArr[j].indexOf('" + userId + "') >= 0) { " +
                        "cookieArr[j] = cookieArr[j].replace('" + userId + "', '0'); " +
                        "msg = 'unblocked!';" +
                     "} else {" +
                        "cookieArr[j] = cookieArr[j] + '," + userId + "';" +
                        "msg = 'blocked!';" +
                     "} " +
                     "cc = cookieArr[j];" +
                     "break;" +
               ...
               "}" +
               "cc = cc.replace('0,', '');" + 
               "var expires = new Date(); expires.setTime((new Date()).getTime() + 3 * 30 * 24 * 60 * 60 * 1000);" +
               "document.cookie = cc + ';expires=' + expires.toGMTString(); " +
               "document.getElementById('msg" + randomId + "').innerHTML = msg;" +
            ' return false;">[X]</a><br/><span style="font-size:9pt;background-color:lightblue" id="msg'+randomId+'"></span>');
            tdList[i].innerHTML = html;
         }
      }
	},
   
   /** Change the layout of the forum to a nicer format. **/
   niceLayout : function() {
      var resultLinks = document.evaluate('/html/body/table/tbody/tr/td/table/tbody/tr[3]/td/table/tbody/tr/td[2]/table/tbody/tr[2]/td[2]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
      var main;
      ...
      resultLinks = document.evaluate('/html/body/table/tbody/tr/td[3]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
      if ( (res = resultLinks.snapshotItem(0) ) !=null ) { 
         res.style.display = 'none';
      }
      if (location.pathname == '/topics.asp') {
         resultLinks = document.evaluate('/html/body/table[6]/tbody', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
         if ( (res = resultLinks.snapshotItem(0) ) !=null ) { 
            res.style.display = 'none';
         }
      }
   ...
   /** Attempt to remove the mine. **/
   mineRemover : function() {
      var allImg = document.getElementsByTagName('IMG');
      for (var i = 0; i < allImg.length; i++) {
         var img = allImg[i];                
         if (img.src.indexOf('logout') >= 0) {
            img.src = '#';
            img.alt = '我係仆街我放雷，我係賤人我抵屌。';
            continue;
         }
      }
   },
   
   /** Auto mask all images, unmask only when clicked. **/
   imgImprover : function() {
      var allImg = document.getElementsByTagName('IMG');
      for (var i = 0; i < allImg.length; i++) {
         var img = allImg[i];                
         if (img.src.indexOf('hkgolden.com') == -1 && img.width > 30 && img.height > 30) {
            var randomId = Math.floor(Math.random()*100000);
            img.removeAttribute('onload');
            if (GM_getValue('defaultimg') != 'show') {
               img.alt = img.src;
               img.src = 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';
            } else {
               img.alt = 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';               
            }
            img.id = 'img' + randomId;
            img.style.border = '1px dotted #000';
            ...
            lnk.addEventListener('click', function() { 
               var iimg = document.getElementById('img' + this.id);
               var tmp = iimg.src;
               var w; var h;
               if (iimg.src.indexOf('data:image/gif') == -1) {
                  w = iimg.width;
                  h = iimg.height;
               }               
               iimg.src = iimg.alt;
   ...
   /** Quick reply box. Credit: jackyct1 **/
   quickReply : function() {
      var tdList = document.getElementsByTagName('td');
   	...
   	for (var i=0; i<tdList.length; i++) {
   		if(tdList[i].getAttribute("style")!=null){
   			...
   			if (tdList[i].getAttribute("style").indexOf("background-color: rgb(243, 242, 241); height: 100%;")>=0){
   				...
   				msgQuote="&nbsp;<a style='cursor: pointer;' onclick=\"javascript:document.getElementById('messagetext').focus();document.getElementById('messagetext').value='"+htb+"'\">[引]</a>";
   		...
   		for (var i=0; i<tdList.length; i++) {
   			if(tdList[i].getAttribute("style")!=null){
   				...
   				if (tdList[i].getAttribute("style").indexOf("width: 57%;")>=0 && tdList[i].getAttribute("style").indexOf("text-align: right;")<0){
   					...
   					temp += "<textarea wrap=soft rows='8' name='messagetext' cols='80' style='width: 534; word-break: break-all;' id='messagetext'></textarea><br />";
   ...
   defaultImg : function() {
      var tmp = GM_getValue('defaultimg');
      if (!tmp)
         tmp = 'hide';
      GM_setValue('defaultimg', window.prompt('Show or hide image by default?', tmp).toLowerCase());
   },
   
   defaultQuick : function() {
      var tmp = GM_getValue('defaultqr');
      if (!tmp)
         tmp = 'off';
      GM_setValue('defaultqr', window.prompt('Turn on or off Quick Reply?', tmp).toLowerCase());
   },
   
   about : function() {
      alert('Super Golden ' + SuperGolden.version + '\n\n謹獻給高登歷代烈士#adore#');
   },
...
switch (location.pathname) {
   case '/default.asp':
      SuperGolden.autoRedirect();
      break;
   case '/view.asp':
      SuperGolden.mineRemover();
      SuperGolden.titleChanger();
      SuperGolden.banUsers();
      SuperGolden.niceLayout();
      SuperGolden.imgImprover();
      if (GM_getValue('defaultqr') == 'on')
         SuperGolden.quickReply();
      break;
   case '/topics.asp':
      SuperGolden.niceLayout();
      break;
...
GM_registerMenuCommand('Change default image visibility...', SuperGolden.defaultImg);
GM_registerMenuCommand('Turn on/off quick reply...', SuperGolden.defaultQuick);
GM_registerMenuCommand('About...', SuperGolden.about);
