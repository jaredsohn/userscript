...
// @author          Seyguai ( MAJ par XxwolfxX )
...
if (ScanType) {
	if (system * 1 < 100) document.getElementsByName('systemRight')[0].click();
			...
			if (ScanType[1] != galaxy) {
				...
				alert('Scan Univers ' + document.URL.match(/uni(\d+)\.ogame/)[1] + ' terminÃ©.');
				window.setTimeout("location.replace('" + url + "');", 1500);
			}
		} else {
			GM_setValue('ScanGalaType', false);
			alert('Scan Galaxie ' + galaxy + ' terminÃ©.');
			window.setTimeout("location.replace('" + url + "');", 3257);
		}
} else {
	...
	for (var i = 0; i < td.length; i++) {
		if (td[i].innerHTML.search(/<input.+value="Afficher"/) + 1) {
			...
			td[i].innerHTML += ' <input id="G2" type="button" value="Scanner la G' + galaxy + ' Ã  partir d\'ici" onclick="location.replace(\'' + url + '&amp;' + galaxy + '&amp;system=' + (system * 1 + 1) + '\');">';
			...
			td[i].innerHTML += ' <input id="U2" type="button" value="Scanner Tout Ã  partir d\'ici" onclick="location.replace(\'' + url + '&amp;' + galaxy + '&amp;system=' + (system * 1 + 1) + '\');">';
		}
	}
