...
function TTQ_showMenuCommand() {
	...
	tA.setAttribute("style", "position:absolute;top:"+(parseInt(getPosition($id("wrapper")).y)+24)+"px;"+docDir[0]+":50%;background-color:#AAAAAA;padding:0px 2px 0px 2px;margin:0px "+(ltr?0:320)+"px 0px "+(ltr?320:0)+"px;white-space:nowrap;color:black;font-family:'Lucida Sans Unicode','Comic Sans MS';font-size:10px;-moz-border-radius:6px;border-radius:6px;border-width:2px;border-style:outset;border-color:black;z-index:501;cursor:pointer;");
...
//-- определение координат, активного здания и создание SPAN с кликабельными ссылками на деревню.
		...
		vName = "<span class ='ttq_village_name' onclick='window.location = \""+tA.href+"\";return false;' title='"+aLangStrings[80]+"'>" + tA.innerHTML + "</span>&nbsp;<span class ='ttq_village_name ttq_village_coords' title='"+aLangStrings[81]+"' onclick='window.location = \"position_details.php?x="+tX+"&y="+tY+"\";return false;' >(" + tX + "|" + tY + ")</span>";
...
function getActiveVillage (el) {
	var reqVID = el.getElementsByClassName("entry active");
	if ( reqVID.length == 1 ) {
...
function displayTimerForm(iTask, target, options, timestamp, taskindex, villagedid) {
	...
	switch(iTask) {
		...
		case 9: // Send troops through Gold-Club
			...
			if( sTask == '' ) {
				sTask = $gc('tabItem',$gc('active',$gc('tabNavi')[0])[0])[0].innerHTML;
