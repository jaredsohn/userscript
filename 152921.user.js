...
// @downloadURL    http://userscripts.org/scripts/source/152921.user.js
// @updateURL      http://userscripts.org/scripts/source/152921.meta.js
...
var main = function () {
	...
	// CONSTANTS
	var VERSION = 									"E.D. 0.7.1.2a";
	var URLSCRIPT = 								"http://userscripts.org/scripts/show/152921";
	// CUSTOM IMAGE LINKS
	...
	function initialize() {
		...
		if( $( "form[action='login.html']" ).length == 0 ) {
			...
			addConfigurationUI();
			addPurgeDiv();
			if( getValue( "configMoveNotifications" ) == "true" ) { rellocateMessages2(); }
			if( getValue( "configHideFightButtons" ) == "true" ) { hideFightButtons(); }
			...
			if( getValue( "configHideMedkitButtons" ) == "true" ) { hideMedkitButtons(); }
			//changeProfile();
		}
	...
	function loadConfiguration() {
		...
		// calin
		//if( !getValue( "configCalinSelection" ) ) { setValue( "configCalinSelection", "true" ); })
		if( !getValue( "configHideFightButtons" ) ) { setValue( "configHideFightButtons", "false" ); }
		if( !getValue( "configHideMedkitButtons" ) ) { setValue( "configHideMedkitButtons", "true" ); }
	...
	function addFastLinks() {
		...
		var content = $( "<div class='plate' style='display:block; font-weight:bold; font-size:11px; height:20px;'></div>" );
	...
	function addConfigurationUI() {
		...
		var configHideFightButtons = createCheckBox( "Hide Fight buttons", "configHideFightButtons" );
		globalBlock.append( configHideFightButtons );
		var configHideMedkitButtons = createCheckBox( "Hide Medkit buttons", "configHideMedkitButtons" );
		globalBlock.append( configHideMedkitButtons );
		...
		if( localUrl.indexOf( URLBattle, 0 ) >= 0 ) {
			...
			if( plateList.length == 5 ) {
				...
				if( isOrgAccount() ) {
					...
					if( getValue( "configHideFightButtons" ) == "true" ) { 
	...
	function checkWorkResults( idComp, idForm, data ) {
		...
		for( var i=1; i<rows.length; i++ ) {
			...
			if( cols.length > 0 ) {
				...
				var pos = $( idForm ).find( "a[name='"+ $( cols[0] ).find( "a" ).text() +"']" );
				...
				if( cols.length == 12 ) {
					...
					posSkill.bind( "click", function() {
						var v = $(this).text();
						$( ".skill" ).each( function() {
							if( v !== $(this).text() ) { $(this).parent().find( ".receipments" ).removeAttr( "checked" ); }
						});
						setCounterText(' ( Skill: ' +v+' )');
					});
					...
					for( var j=0; j<7; j++ ) {
						...
						day.bind( "click", function() {
							...
							setCounterText( ' ( Day: -'+ $(this).attr( "day" ) +' )');
						});
...
//			'.aniIcon:hover{opacity:1;-moz-opacity:1;-khtml-opacity:1;filter:alpha(opacity=100);cursor:pointer}'+
	...
	function addPurgeDiv() {
		var localUrl = new String( window.location );

		//Mu Homepage
		if( localUrl.indexOf( URLMU, 0 ) >= 0 || localUrl.indexOf( URLMUHOMEPAGE, 0 ) >= 0 ) {

			$( "#contentRow td > .testDivblue:eq(4)" ).css({ "clear" : "none" });
			$( "#contentRow td > .testDivblue:eq(2)" ).find( "center b:contains('Members')" ).append( " ("+ $( ".smallAvatar" ).length +")");
	...
	function rellocateMessages2() {
		// Relloacte messages
		var plateList = $( "#userMenu" ).children( ".plate" );
			plateList.last().insertAfter( $( "#userName" ).parent().parent() );
	}
		...
		if( localUrl.indexOf( URLBattle, 0 ) >= 0 ) {
			...
			if( plateList.length == 5 ) {
				...
				if( isOrgAccount() ) {
					...
					plateList.eq(1).find( ".fightIcon" ).parent().remove();
					if( plateList.eq(1).children( "div" ).children( ".button" ).length == 0 ) {	plateList.eq(1).remove() };
				}
			}
		}
	}

	function hideMedkitButtons() {
		$("#stats > form[action='medkit.html']").remove();
	}

	function addShoutImgLink() {
		$("head").append("<style>.miniContent .shoutImage:hover{cursor:pointer}</style>");
	...
	function highlightStockCompany() {
		$( ".dataTable:first tbody tr:not(:first)").each( function(){
			var cell = $(this).find( "td:first" );
			var img = cell.children( ".smallAvatar" ).eq(0);
			img.css({ "width":"30px", "height":"30px", "margin":"2px 10px","float":"left"});

			if ( 0 > cell.children( "a" )[0].href.indexOf( "profile.html") ) {
				cell.css({ "background-color" : "#FDFF73" });
				cell.append( "<div><b style='color:#111'>Stock company</div>" );
			}
		})
	}
