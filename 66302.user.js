...
// ==UserScript==
// @name        MW Autoplayer - Fight 100% working
// @namespace   mafiawars
...
var SCRIPT = {
  url: 'http://userscripts.org/scripts/source/66302.user.js',
  version: '0.1.00',
  build: '1',
  name: 'inthemafia',
  appID: 'app10979261223',
  ajaxPage: 'inner2',
  presentationurl: 'http://userscripts.org/scripts/show/66302',
...
if (window.location.href.match(/facebook/))  {
  ...
  var iFrameCanvas = xpathFirst('//iframe[contains(@src,"zynga.com")]');
  if (iFrameCanvas)
...
function getDisplayedOpponents(element, forceRefresh) {
  ...
  for (var i = 0, iLength=links.length; i < iLength; i++) {
    ...
    opponent.id      = linkElt.getAttribute('onclick').match(/opponent_id=(\w+)/)[1];
  ...
  if (fight && !getDisplayedOpponents.opponents.length) {
    ...
    for (var i = 0, iLength=links.length; i < iLength; ++i) {
      ...
      opponent.profile = nameAndLevel.getElementsByTagName('a')[0];
      if (!opponent.profile) continue;
      opponent.id      = opponent.profile.href.split('user=')[1];
