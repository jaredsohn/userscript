// ==UserScript==
// @name            Battle Stars
// @description     Script statistiques de BDF (Script Estadísticas de Batallas de Fuertes)
// @include         http://*.the-west.*/game.php*
// @include         http://*.the-west.*/forum.php*
// @version     1.0.9
// @history     1.0.9 correction (correción)
// @history     1.0.8 formatage pour forum (formato para el foro)
// @history     1.0.7 ajout fenetre de resultat et compatibilité 2.0.5 (adición ventana de resultados y compatibilidad con 2.0.5)
// @history     1.0.6 passage en 2.0.4 (paso a la 2.0.4)
// @history     1.0.5 compatibilite Chrome (compatibilidad con Chrome)
// @history     1.0.4 bug maj (error mayor)
// @history     1.0.3 corrections (correcciones)
// @history     1.0.2 ajout stats globales (adición estadísticas globales)
// @history     1.0.1 debut correction bugs (primera corrección de errores)
// @history     1.0.0 initial (versión inicial)
// @grant       none 
// ==/UserScript==
(function(fn) {
			...
			Stats = {
					...
					try {
						...
						Stats.formules
								.push(new calcul(
										'heros',
										'Héroe',
										'Tasa de tiros absorbidos (recibidos + eludidos) sobre los PV inicial',
              ...
              */
                ...
                .push(new calcul(
                    'survivant',
                    'Superviviente',
                    'El que termina con menos puntos de vida final',
                    'val.finishedhp', [ 'PVf' ],
                    [ 'finishedhp' ], '  val.finishedhp  > 0', '0', '<', true, true, false, false));

						Stats.formules
								.push(new calcul(
										'hitmachine',
										'Hit Machine',
										'Porcentaje de aciertos realizados sobre el total de tiros realizados (aciertos + fallos)',
										'(val.hitcount /(val.hitcount + val.misscount))*100',
										[ 'Tasa(%)', 'Aciertos', 'Fallos' ],
						...
						Stats.formules
								.push(new calcul(
										'matrix',
										'Matrix',
										'Porcentaje de tiros eludidos sobre el total de tiros recibidos con ponderación -(1/eludidos)',
						...
						Stats.formules
								.push(new calcul(
										'headshot',
										'HeadShot',
										'Porcentaje de Kos conseguidos sobre aciertos realizados',
										'(val.ko_shots.length / val.hitcount)*100',
										[ 'Tasa(%)', 'Kos efectuados', 'Aciertos' ],

										[ 'headshot', 'ko_shots.length', 'hitcount' ],
				...
				getFormule : function(type) {
					...
					throw ("No hay fórmula correspondiente a " + type);
				},
				getAll : function(type) {
					var ligTot = '';
					$('#route', CemeteryWindow.DOM).text('detalles');
				...
				openWindow : function(original){
				 ...
				 var statWindow =	wman.open(
							'window_Stats',
							'Estadísticas de la Batalla').setSize(
							650, 400);
					...
					var verif="<form><textarea style='height: 250px;width: 600px;'>[code]Estadística\t\t\tAtaque\t\tDefensa\t\tDiferencia\n--------------------------------------------------------------------------\n";
					...
					.each(
							...
							var affButton = new west.gui.Button("Formato Foro", function () {
                                 ...
                                 if(cur.indexOf("[code]")>-1){
                                       ...
                                       affButton.setCaption("Formato Foro");  
				...
				createStarsTable : function() {
					...
					Stats.table.createEmptyMessage('Battle Stars').addColumn(
						...
						sortBy : 'starthp'
							...
							'Tít. de Estrella').appendToThCell("head",
							"battle_tow", 'Nom', 'Nombre').appendToThCell("head",
							"stat_dtl", 'Détails', 'Detalles');
					...
					$('.footer', CemeteryWindow.DOM).empty();
						var titleAtt = 'Total PV Ataque - Inicial : '
							+ Stats.original.startAttPV
							+ ' - Final : '
							+ Stats.original.finishedAttPV
							+ ' - Muertos : '
							+ (Stats.original.startAttPV - Stats.original.finishedAttPV) +' - PV promedio : ' + Stats.original.moyatt.toFixed(2) 
							;
					var titleDef = 'Total PV Defensa - Inicial : '
							+ Stats.original.startDefPV
							+ ' - Final : '
							+ Stats.original.finishedDefPV
							+ ' - Muertos : '
							+ (Stats.original.startDefPV - Stats.original.finishedDefPV)  +' - PV promedio : ' + Stats.original.moydef.toFixed(2) 
							;
				...
				calculStars : function() {
					...
					$
							.each(
									...
									Stats.results.push({titre : "Total PV Inicial\t\t\t",attack:Stats.original.startAttPV, defend:Stats.original.startDefPV});
									 
									Stats.results.push({titre : 'Total PV Final\t\t\t',attack:Stats.original.finishedAttPV, defend:Stats.original.finishedDefPV});
									Stats.results.push({titre : 'Total PV Muertos\t\t\t',attack: (Stats.original.startAttPV - Stats.original.finishedAttPV) , defend:(Stats.original.startDefPV - Stats.original.finishedDefPV)});
									Stats.results.push({titre : 'PV promedio\t\t\t' ,attack: Stats.original.moyatt.toFixed(2) , defend:Stats.original.moydef.toFixed(2) });
								 
									Stats.results.push({titre : "Promedio daño\t\t\t" ,attack: (Stats.original.degAtt).toFixed(2) , defend:(Stats.original.degDef).toFixed(2)});
									Stats.results.push({titre : "Promedio eludidos\t\t",attack:	 (Stats.original.att.dodgecount / Stats.original.countAtt).toFixed(2), 
										defend:(Stats.original.def.dodgecount / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Promedio acertados\t\t" ,attack:(Stats.original.att.hitcount / Stats.original.countAtt).toFixed(2),
										defend:(Stats.original.def.hitcount / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Promedio fallados\t\t" ,attack: (Stats.original.att.misscount / Stats.original.countAtt).toFixed(2),
										defend:(Stats.original.def.misscount / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Promedio recibidos\t\t" ,attack:(Stats.original.att.takenhits / Stats.original.countAtt).toFixed(2), 
										defend:(Stats.original.def.takenhits / Stats.original.countDef).toFixed(2)});
							    	Stats.results.push({titre : "Daño medio\t\t\t" ,attack:(Stats.original.att.avg_damage / Stats.original.countAtt).toFixed(2), 
									defend:	(Stats.original.def.avg_damage / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Daño max. medio\t\t" ,attack: (Stats.original.att.maxdamage / Stats.original.countAtt).toFixed(2), 
										defend:(Stats.original.def.maxdamage / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Daño por arma min.\t\t" ,attack:(Stats.original.att.weaponmindmg / Stats.original.countAtt).toFixed(2), 
										defend:(Stats.original.def.weaponmindmg / Stats.original.countDef).toFixed(2)});
									Stats.results.push({titre : "Daño por arma max.\t\t" ,attack:(Stats.original.att.weaponmaxdmg / Stats.original.countAtt).toFixed(2) ,
