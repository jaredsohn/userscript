// ==UserScript==
// @name          Asel Warez VBulletin Rep-Giver Pro 2
// @include       http*://*.warezturkey.com/*
// @namespace     http://userscripts.org/scripts/show/180102
// @include       *.warezturkey.com//usercp.php
// @include       *.warezturkey.com/usercp.php
// @include       *.warezturkey.com//usercp.php
// @version       1.24
// @dtversion     12052724
// @timestamp     13
// @description   (Asel Warez Forum) automatically get (a|some) reputation(s) giver's link 
// @author        idx (http://userscripts.org/users/535545)
//
// Rootwarez VBulletin Rep-Giver
// Mod :: Proxlive
...
//
// mod.R.23 : 2012-04-22
// Fix bypass *llegal\sfunction for user_id
//
// mod.R.22 : 2011-05-19
...
// missed destroy_column(el_sender) -RootWarezDonat.
// 
// mod.R.16 : 2011-02-13
// Fix some regex parseIt()
...
gvar.sversion = 'R' + '24';
gvar.scriptMeta = {
  timestamp: 13 // version.timestamp
...
function init(){
  ...
  gvar.titleName = 'Root Warez vBulletin Reputation Giver';
  ...
  gvar.f = ['uid','user_id','tag_id','location','join_date','total_post','avatar_url','rep_count','rep_filename','rep_rank_title'];
...
function start_Main() {
    ...
    if( gvar._SIMULATE_ ){
      ...
      // selected user just to simulate error
      ...
      // selected user just to simulate error from (killua)
...
function parseIt(page){
  ...
  if(ret) result[gvar.f[1]] = get_user_id(page);
...
function createSender(data, x){
  ...
  data["user_id"] = filterChar( data["user_id"] );
  var is_donat = ( /\[\$\]$/.test(data["user_id"]) ? '&nbsp;<b style="color:red;">[$]</b>':'');
  if(is_donat.length>0) data["user_id"]=data["user_id"].replace(/(\[\$\])$/,'');
  
  //overflow:wrap;  //Root Warez Donator
  var inner = ''
  ...
  +'<div><b>'+( data["user_id"] )+'</b>'+(is_donat)+'</div>'
  +'<div>'+data["tag_id"]+'</div>'
  ...
  // last act, attaching..  
  var tgt = $D("_ucnd"+x);
  tgt.innerHTML = inner + '<a class="smallfont" href="' + gvar.uri['user_link'] + data["uid"] +'"><b>' + data["user_id"] + '</b></a>'
