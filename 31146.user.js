...
var priority = [10,11,6,1];
var autobuild = 0;
var buildavailable = 0;
var deleterep = 0;
var lowestlvl = 10;
var dorf1menit = '00:05:00';


buildavailable = GM_getValue('buildavailable', null);
autobuild = GM_getValue('AutoBuild', null);
deleterep = GM_getValue('DeleteAll', null);


if (autobuild == 1)
{
	Textmenu = "Travian Beyond: NonActivate AutoBuild";
}
else
{
	Textmenu = "Travian Beyond: Activate AutoBuild";
}
GM_registerMenuCommand(Textmenu , promptbuild);

if (buildavailable == 1)
{
	Textmenu = "Travian Beyond: NonActivate AutoAvailableBuild";
}
else
{
	Textmenu = "Travian Beyond: Activate AutoAvailableBuild";
}
GM_registerMenuCommand(Textmenu , promptavail);

if (deleterep == 1)
{
	Textmenu = "Travian Beyond: NonActivate Auto Delete Battle Report";
}
else
{
	Textmenu = "Travian Beyond: Activate Auto Delete Battle Report";
}
GM_registerMenuCommand(Textmenu , promptdelete);

function promptavail()
{
    if (GM_getValue('buildavailable', null)==1)
    {
	GM_setValue('buildavailable', 0);
	buildavailable = 0;
	alert ('Build Available Resources Building Deactivate');
    }
    else
    {
	GM_setValue('buildavailable', 1);
	buildavailable = 1;
	alert ('Build Available Resources Building activated');
    }
}

function promptbuild()
{
    if (GM_getValue('AutoBuild', null)==1)
    {
	GM_setValue('AutoBuild', 0);
	autobuild = 0;
	alert ('auto build resources deactivated');
    }
    else
    {
	GM_setValue('AutoBuild', 1);
	autobuild = 1;
	alert ('auto build resources activated');
    }
}

function promptdelete()
{
    if (GM_getValue('DeleteAll', null)==1)
    {
	GM_setValue('DeleteAll', 0);
	deleterep = 0;
	alert ('auto delete report deactivated');
    }
    else
    {
	GM_setValue('DeleteAll', 1);
	deleterep = 1;
	alert ('auto delete report activated');
    }
}

if (document.body.innerHTML.search(/firewall/)>=0) { 
urlna = document.location.href.split("/");
loading = 1;
document.location.href = xserver + urlna[3]; 
}
if (document.body.innerHTML.search(/retrieved/)>=0) { 
urlna = document.location.href.split("/");
loading = 1;
document.location.href = xserver + urlna[3]; 
}
...
function funcionPrincipal(e){
	...
	var lang_ru = new Array();
        ...
        lang_dk['MARK']         = 'V&Atilde;&brvbar;lg alle';
        lang_dk['PERDIDAS']     = 'Tab';
        lang_dk['RENT']         = 'Profit';
        lang_dk['SUBIR_NIVEL']  = 'Udvidelse tilg&Atilde;&brvbar;ngelig';
        ...
        lang_dk['COMERCIAR']    = 'Send r&Atilde;&yen;stoffer';
        ...
        lang_dk['TALLER']       = 'V&Atilde;&brvbar;rksted';
        lang_dk['ENVIAR']       = 'Send r&Atilde;&yen;stoffer';
        lang_dk['COMPRAR']      = 'K&Atilde;&cedil;b';
        lang_dk['VENDER']       = 'S&Atilde;&brvbar;lg';
        lang_dk['ENVIAR_IGM']   = 'Send IGM';
        lang_dk['LISTO']        = 'Udvidelse tilg&Atilde;&brvbar;ngelig';
        lang_dk['EL']           = 'p&Atilde;&yen;';
        lang_dk['A_LAS']        = 'ved';
        lang_dk['EFICIENCIA']   = 'Effektivitet';
        lang_dk['NUNCA']        = 'Aldrig';
        lang_dk['PC']           = 'kultur points';
        lang_dk['FUNDAR']       = 'Du kan grundl&Atilde;&brvbar;gge eller overtage en ny by';
        lang_dk['ALDEAS']       = 'By(er)';
        lang_dk['ENV_TROPAS']   = 'Send tropper';
        lang_dk['RECURSO1']     = 'Tr&Atilde;&brvbar;';
        ...
        lang_dk['BUSCO']        = 'S&Atilde;&cedil;ger';
        lang_dk['TIPO']         = 'Type';
        lang_dk['DISPONIBLE']   = 'Kun tilg&Atilde;&brvbar;ngelig';
        lang_dk['CUALQUIERA']   = 'Alle';
        lang_dk['SI']           = 'Ja';
        lang_dk['NO']           = 'Nej';
        lang_dk['MARCADORES']   = 'Bogm&Atilde;&brvbar;rker';
        lang_dk['ANYADIR']      = 'Tilf&Atilde;&cedil;j';
        lang_dk['ENLACE']       = 'Nyt bogm&Atilde;&brvbar;rke URL';
        lang_dk['TEXTO']        = 'Nyt bogm&Atilde;&brvbar;rke tekst';
...
*/
    ...
    var xserver = '';
    var Next_to_build = '';
	var reloading = dorf1menit;
	var loading = 0;
	//toopen
	var build = 1;
	var crany_keep = [100,130,170,220,280,360,460,600,770,1000]; // gaul 2x.
	var granary_keep = [1200,1700,2300,3100,4000,5000,6300,7800,9600,	11800,14400,17600,21400,25900,31300,37900,45700,55100,66400,80000];
	var warehouse_keep = [1200,1700,2300,3100,4000,5000,6300,7800,9600,11800,14400,17600,21400,25900,31300,37900,45700,55100,66400,80000];
	// Lenyador
	var ca = location.href.split('/')
	xserver = 'http://' + ca[2] + '/';
	var lenyadorCost = [
		[0, 0, 0, 0],
		[40, 100, 50, 60],
	...
	function calculateBuildTime(){
		...
		for(j = tablas.snapshotLength - 1; j >= 0; j--) {
			...
			if (b){
				...
				if (a != null){
				    a = a + " <br>Will Reloading at <span id='timeout'> " + reloading + " </span>";
...
//		find("//script[@type='text/javascript']", XPFirst).src.search(/\/([^\/]+)?3.js$/);
		...
		pack_grafico = find("//img[@class='logo']", XPFirst).src;
		pack_grafico = pack_grafico.substring(0,pack_grafico.length-21);
...
//				[T('CALC'), "http://www.denibol.com/~victor/travian_calc/", "_blank"],
	...
	function preCalculate1(){
		...
		var existingbuild = new Array(19);
		for(i = 1; i <= 18; i ++) {
			existingbuild[i] = new Array(2);
			for(j = 1; j <= 2; j++) {
				//j -> 0 jenis, 1 lvl, 2 available
				existingbuild[i][j] = 0; 
			}
		}
		...
		for (var i = 1; i <= 18; i++){
			...
			if (a){
				...
				existingbuild[i][1]=RegExp.$1;
			}else{
				grid[dist[tipo - 1][i - 1]][0]++;
				existingbuild[i][1]=0;
			}
			existingbuild[i][0] = dist[tipo - 1][i - 1];
			existingbuild[i][2] = 0;
		}

		// i 0 wood, 1 clay, 2 iron, 3 crops
		// j lvl
		// var actual = new Array(4); actual resources
		// var total = new Array(4);	total resources
		// var produccion = new Array(4); production resources
						
		// find out that if there's no building are builded
		var inbuilding = find("//div[@id='lbau1']//table[@class='f10']", XPFirst);
		if (inbuilding==null)
		{ 
			buildingok = 1;
		}
		...
		{
			buildingok = inbuilding.innerHTML.search(/Ladang/) +
						 inbuilding.innerHTML.search(/Penggalian/) +
						 inbuilding.innerHTML.search(/Penebang/) +
						 inbuilding.innerHTML.search(/Tambang/) +
inbuilding.innerHTML.search(/Bedeng/) +
 inbuilding.innerHTML.search(/Kastil/) +
 inbuilding.innerHTML.search(/Akademi/) +
 inbuilding.innerHTML.search(/Tukang Besi/) +
 inbuilding.innerHTML.search(/Gudang Senjata/) +
 inbuilding.innerHTML.search(/Istal/) +
 inbuilding.innerHTML.search(/Lumbung/) +
 inbuilding.innerHTML.search(/Titik Temu/) +
 inbuilding.innerHTML.search(/Gudang/) +
inbuilding.innerHTML.search(/Barak/) +
inbuilding.innerHTML.search(/Cropland/) +
		inbuilding.innerHTML.search(/Clay Pit/) +
		inbuilding.innerHTML.search(/Woodcutter/) +
		inbuilding.innerHTML.search(/Iron Mine/) +

						 inbuilding.innerHTML.search(/Cranny/)	;
			if (buildingok<=0) 
			{ buildingok = 1; }
			else
			{ buildingok = -1;}
		...
		//if (buildingok) 
		//{
			//there's no building are builded... HERE WE ARE.... BUILDDD!!!!
			// determine that what building should build
			average = Math.round((produccion[0] + produccion[1] + produccion[2])*3600 / 8);
				// find out the minimum amount of resource(s) production
				prodmin = 0;
				for (var i = 1; i<4 ; i++)
				{
					if ((produccion[i]/priority[i])<(produccion[prodmin]/priority[prodmin]))
					{
						prodmin = i;
					}
				}
				...
				for (var i = 1; i<4 ; i++)
				{
					if (produccion[i]<produccion[prodmin])
					{
						prodmin = i;
					}
				}*/
			
			if (prodmin == 0) { nextbuild = '"Wood Cutter"';}
			if (prodmin == 1) { nextbuild = '"Claypit"';}
			if (prodmin == 2) { nextbuild = '"Iron Mine"';}
			if (prodmin == 3) { nextbuild = '"Crop Land"';}
			// end of determine that what building should build
		//}
		...
		fila0.setAttribute("class", "rbg");	
		fila1.setAttribute("class", "rbg");	
		table.appendChild(fila0);
		table.appendChild(fila1);
		table.appendChild(fila2);
		var td0 = elem("TD", "Next Building To Build will be " + nextbuild + " Reload in <span id='timeout'> " + reloading + " </span>");
		td0.setAttribute("colspan", "4");	
		td0.setAttribute("align", "left");
		fila0.appendChild(td0);
		for (var i = 0; i < 4; i++){
			...
			for (var j = 0; j < 25; j++){
				if (grid[i][j] > 0 && buildingCost[i][j+1] != null){
					...
					for (var k = 1; k <= 18; k++)
					{
						if (existingbuild[k][0]==i && existingbuild[k][1]==j && can_build==1) 
						{
							existingbuild[k][2] = 1;
						}
					}
		...
		table.style.top = 550 + longitudPantalla() + 'px';
		if (datos == 1) document.body.appendChild(table);
		if (buildingok==1) // BUILDDDD
		{
			lowestlvl = 10;
			for (var i=1;i<=18;i++)
			{
				if (existingbuild[i][0]==prodmin && existingbuild[i][2] == 1)
				{
					if (existingbuild[i][1]<lowestlvl)
					{
						
						field_to_build = i;
						lowestlvl = existingbuild[i][1];
					}
				}
			}
			if (field_to_build!=0)
			{
			    //toopen
				if (autobuild) { location.href = xserver + 'build.php?id=' + field_to_build; }
			 	loading = 1;
			}
			else
			if (buildavailable==1)
	...
	function preCalculate2(){
		...
		//next to build / upgrade -> for Granary, Warehouse and Cranny
		Cranny_capacity = 0;
		Warehouse_Capacity = 0;
		Granary_Capacity = 0;
		active_cranny_link = "";
		active_cranny_lvl  = "";
		active_warehouse_link = "";
		active_warehouse_lvl  = "";
		active_granary_link = "";
		active_granary_lvl  = "";
		for(var i = 0; i < buildingsDescs.length - 3; i++) {
				//alert(buildingsDescs[i].nodeValue + " " + buildingsLinks[i].nodeValue );
				if(buildingsDescs[i] != null && basename(buildingsImages[i].nodeValue) != 'iso.gif') 
				{
					...
					//alert (buildingsDescs[i].nodeValue + " " + buildingCode + " " + buildingLevel); 
					if (buildingCode==23) {
						//Cranny_Capacity = Cranny_Capacity + ;
					}
					if (buildingCode==10) {
...
//				buildingCode = parseInt(buildingCode[0].substring(1, buildingCode[0].length));
				...
				if (buildingCost[buildingCode] != null && buildingCost[buildingCode][buildingLevel+1] != null){
					...
					// Town Construction availabity
					if (restante != null) td3.innerHTML = restante;
					else td3.innerHTML = T('SUBIR_NIVEL') + 'test';
				}
			}
		}
	...
	*/ 
	 ...
	 */
		...
		divna = find("//div[@class='b f135']", XPFirst);
		divna.innerHTML = divna.innerHTML + '</div><div class="b f135"><span id="timeout">00:00:30</span>';
        }
	...
	function crearTemporizadorRecurso(i){
		return function(){
			...
			var j = 0;
			while ( j < recursos.snapshotLength)
			{
				var cantidad = recursos.snapshotItem(j).innerHTML - 10;
				if (cantidad >= 0) recursos.snapshotItem(j).innerHTML = cantidad;
				else 
				{ 
					j =  recursos.snapshotLength;
					if (loading==0) 
					{	
							urlna = document.location.href.split("/");
							loading = 1;
	...
	function setTimers(){
		...
		for (var i = 0; i < 4; i++){
			...
			setInterval(crearTemporizadorRecurso(i), Math.floor(frecuencia[i])*10);
		}
		...
		MyInt1 = setInterval(function () {
			 ...
			 */
			var relojes = find("//*[@id='timeout' or @id='timeouta' or @id='reloader']", XPList);
			var i = 0
			while (i < relojes.snapshotLength)
			{
				...
				else 
				{
					...
					{ 
						if (loading==0) 
						{
							...
							loading = 1;
							document.location.href = xserver + urlna[3]; 
							i = relojes.snapshotLength;
						}
					}
					else
					if (relojes.snapshotItem(i).id == 'reloader') 
					{
						if (loading==0) 
						{
							urlna = document.location.href.split("/");
							...
							loading = 1;
							document.location.href = xserver + urlna[3]; 
							tiempo = calcular_segundos(relojes.snapshotItem(i).innerHTML) + 60;
...
//	if (location.href.match(/karte2.php($|\?z=)/)){		desplazarMapa(); return; }
	...
	//autorelog
	if (document.body.innerHTML.search(/login automatically in the future/)>=0) 
	{ //@class='fm fm110' and @type='text'
		var userid = find("//input[@class='fm fm110' and @type='text']", XPFirst );
		var password = find("//input[@class='fm fm110' and @type='password']", XPFirst );
		var loginbutton = find("//input[@type='image' and @value='login']", XPFirst );
		alert(loginbutton.OnClick);
	}
	...
	if (location.href.indexOf('build.php') != -1){ 		
		/*
		if (document.body.innerHTML.search(/The cranny/)>=0) 
		{ 
			if (find("//a[@href and ancestor::div[@id='lmid2']]", XPFirst))
			{ location.href = find("//a[@href and ancestor::div[@id='lmid2']]", XPFirst); }
		}
		*/
		calculateBuildTime(); 
...
if (deleterep==1)
{
	var recursos = find("//input[@type='checkbox']", XPList)
	var j = 0;
	while ( j < recursos.snapshotLength)
	{
		recursos.snapshotItem(j).checked=true;
		j=j+1;
	}
	if (j<=0)
	{
		var mybutton = find("//tr[@class='rbg']", XPFirst);
		mybutton.innerHTML='<td colspan="2">Subjek:<div id=timeout>00:05:00</div></td><td>Terkirim</td>';
	}
	else
	{	
		var mybutton = find("//input[@class='std' and @name='del']", XPFirst);
        	mybutton.click();
	}
}
	...
	if (autobuild)
	{
		var inbuilding = find("//div[@id='lmid2']", XPFirst);
                  // alert(inbuilding.innerHTML);
		buildingok = inbuilding.innerHTML.search(/Ladang/) +
		inbuilding.innerHTML.search(/Penggalian/) +
		inbuilding.innerHTML.search(/Penebang/) +
		inbuilding.innerHTML.search(/Tambang/) +
		...
		if ((build==1) && (buildingok>-8))
		{
			if (location.href.match(/build.php\?id=1/) || 
			location.href.match(/build.php\?id=2/)     ||
			location.href.match(/build.php\?id=3/)     || 
			location.href.match(/build.php\?id=4/)     ||
			location.href.match(/build.php\?id=5/)     ||
			location.href.match(/build.php\?id=6/)     ||
			location.href.match(/build.php\?id=7/)     ||
			location.href.match(/build.php\?id=8/)     ||
			location.href.match(/build.php\?id=9/)     ||
			location.href.match(/build.php\?id=10/)    ||
			location.href.match(/build.php\?id=11/)    ||
			location.href.match(/build.php\?id=12/)    ||
			location.href.match(/build.php\?id=13/)    ||
			location.href.match(/build.php\?id=14/)    ||
			location.href.match(/build.php\?id=15/)    ||
			location.href.match(/build.php\?id=16/)    ||
			location.href.match(/build.php\?id=17/)    ||
			location.href.match(/build.php\?id=18/))
			{
...
TODO:
	...
	- Cola de espera
	- Estudiar la posibilidad de dividir el script en varios ficheros (idiomas como extensiones)
	...
	- Traduccion a hungaro, noruego, hongkones, eslovaco, esloveno y bosnio (please, use Unicode HTML entities like &#334; For example, use this converter http://konieczny.be/unicode.html )
