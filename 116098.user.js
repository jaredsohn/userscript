...
var e,m,mc,mcb,i,l,j,t,version='Robber Redux v1.06 experimental',
...
var properties = {
	...
	"6" : [
		...
		{"name":"nightclub","fullname":"Nightclub"}
	],
	"7" : [
		{"name":"hq","fullname":"Headquarters"},
		{"name":"refinery","fullname":"Refinery"},
		{"name":"barracks","fullname":"Barracks"}
	],
	"8" : [
		{"name":"truckshop","fullname":"Truck Shop"},
		{"name":"speakeasy","fullname":"Speakeasy"},
		{"name":"warehouse","fullname":"Warehouse"}
	]
};
var http = 'http://';
...
function initialize() {
	...
	for(var i=0; i<as.length; i++) {
		...
		if(/html_server.php\?xw_controller=stats.*?xw_action=view/.test(as[i].href)) {
	...
	if (run) {
		...
		if (profilenum < profiles.length) {
			if (robquick) {
				...
				//console.log(mwid);
				var robtmp = '';
				var url = http+'facebook.mafiawars.zynga.com/mwfb/remote/html_server.php?xw_controller=robbingtarget&xw_action=view&xw_city='+city+'&tmp='+robtmp+'&target=p%7C'+mwid.substr(2);
	...
	$.ajax({
		...
		success: function (response){
			...
			if (roburl=/xw_controller=robbingtarget&xw_action=view&xw_city=(\d+).*?tmp=([a-f0-9]+).*?target=p%7C(\d+)/.exec(response)){
				var url = http+'facebook.mafiawars.zynga.com/mwfb/remote/html_server.php?xw_controller=robbingtarget&xw_action=view&xw_city='+roburl[1]+'&tmp='+roburl[2]+'&target=p%7C'+roburl[3];
				robtmp = roburl[2]
	...
	$.ajax({
		...
		success: function (response){
			...
			if (/has no properties available to rob/.test(response)) {
				...
				var broken = [];
				//while( (timers = /rob_protected_timer_(\d+)/g.exec(response) != null) && (loopfix < 10)) {
				var brokenregex = /id="rob_protected_timer_(\d+)"/g;
				while((timers = brokenregex.exec(response)) != null) {
						...
						if ((broken.indexOf(m[1]) > -1)) {
							...
							switch(rob_city) {
								case 1: //New York
									...
									rob_url = http+'facebook.mafiawars.zynga.com/mwfb/remote/html_server.php?xw_controller=robbing&xw_action=rob_user&xw_city='+m[6]+'&tmp='+m[7]+'&cb='+m[8]+'&xw_person='+m[9]+'&rob_user=p%7C'+m[10]+'&rob_prop='+m[11]+'&rob_city='+m[6];
									var stamina = parseInt(m[5]);
									break;
								case 2: //Cuba
									rob_url = http+'facebook.mafiawars.zynga.com/mwfb/remote/html_server.php?xw_controller=robbing&xw_action=rob_user&xw_city='+m[4]+'&tmp='+m[5]+'&cb='+m[6]+'&xw_person='+m[7]+'&rob_user=p%7C'+m[8]+'&rob_prop='+m[9]+'&rob_city='+m[4];
...
function load_rob_result(url,sta) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			if (/Robbery Success/i.test(response)) {
	...
	function stats_update(obj) {
		...
		document.getElementById('user_cash_brazil').innerHTML = obj['user_fields']['user_cash_brazil'];
		document.getElementById('user_cash_chicago').innerHTML = obj['user_fields']['user_cash_chicago'];
...
function stats_ratios(s) {
	...
	document.getElementById('user_cash_brazil').innerHTML = /user_cash_brazil.*?"(.*?)"/.exec(stats)[1];
	document.getElementById('user_cash_chicago').innerHTML = /user_cash_chicago.*?"(.*?)"/.exec(stats)[1];
