...
// @include			http://www.jeuxvideo.com/rerreychat*
// ==/UserScript==

/* Variables */
window._rerreychat = window._rerreychat || {};
var i, elems, elem;
...
if (location.pathname == '/rerreychat') {
	/* On est sur le module Rerrey Chat */

	/* Chargement de la dernière version */
	elem = document.createElement('script');
	elem.src = '//'+(document.getElementById('_jvchat_localhost') ? '127.0.0.1' : 'jvflux.com')+'/jvchat/jvchat.js';
	...
	if (url_mode === 0 && url_index === 1) { // première page uniquement
		...
		for (i in elems) {
			elems[i].innerHTML = '<a target="rerreychat" class="rerreychat" href="/rerreychat#'+url+'">Rerrey Chat</a>';
		}
	}
	...
	if (url_mode === 1 && document.getElementsByClassName('bt_repondre').length > 0) { // pas sur topics bloqués
		...
		for (i in elems) {
			...
			if (typeof elems[i] == 'object') {
				...
				elem.setAttribute('target', 'rerreychat');
				elem.setAttribute('class', 'rerreychat');
				elem.setAttribute('href', '/rerreychat#'+url);
				elem.appendChild(document.createTextNode('Rerrey Chat'));
	...
	if (url_mode === 3 
		...
		elems[0].innerHTML += ' | <a target="rerreychat" class="rerreychat" href="/rerreychat#'+url+'">Rerrey Chat</a>';
	}
	...
	if (0) {
		...
		if (elem !== null) {
			elem.innerHTML += '<li><a target="rerreychat" class="rerreychat" href="/rerreychat#">Rerrey Chat</a><li>';
		}
	}
	...
	elem.innerHTML = '/* CSS généré par Rerrey Chat */ \
