...
// @version        4.0
// @history        4.0   Fix function obt_barcos (by Kaphook)
// @history        3.9   Remove autoupdate because it is malicious script
// @history        3.8   Include zebralines (by Kaphook)
// @history        3.7   Include Script Updater Check
...
tabla += '<tr height="22"><td>Nombre</td><td>Recurso</td><td>Bienes Min.</td><td>Bienes Max.</td><td>Stock Min.</td><td title="(Minutes)">Repetir</td>';
for (var i = 0; i < cityId.length; i++) {
...
tabla += '<td><select id="f_resource"><option value="wood">Madera</option><option value="wine">Vino</option><option value="marble">Marmol</option><option value="crystal">Cristal</option><option value="sulfur">Azufre</option></select></td>';
...
function add(){
	...
	if(isNaN(minimo)){
		alert("'Bienes Min.' debe ser un número");
		return false;
	}
	var maximo = parseInt($("f_maximo").value);
	if(isNaN(maximo)){
		alert("'Bienes Max.' debe ser un número");
		return false;
	}
	if(minimo>=maximo){
		alert("'Bienes Max.' debe ser mayor que 'Bienes Min.'");
		return false;
	}
	var min_dep = parseInt($("f_min_dep").value);
	if(isNaN(min_dep)){
		alert("'Stock Min.' debe ser un número");
		return false;
	}
	var repeat = parseInt($("f_repeat").value);
	if(isNaN(repeat)){
		alert("'Repeat' debe ser un número");
		return false;
	}
	var hay_target = 0;
	var hay_source = 0;
	...
	if(hay_target == 0){
		alert("No hay una ciudad de destino[T]");
		return false;
	}
	if(hay_source == 0){
		alert("No hay una ciudad de origen [S]");
		return false;
	}
...
function cargar_dealers(){
	...
	if(dealer_name.length-1>0){
		...
		tabla += '<tr><td></td><td></td><td></td><td>Nombre</td><td>Recurso</td><td>Bienes Min.</td><td>Bienes Max.</td><td>Stock Min.</td><td title="(Minutes)">Repetir</td>';
		for (var i = 0; i < cityId.length; i++) {
			tabla += '<td title="'+cityNombre[i]+'">C'+(i+1)+'<img height="14" src="skin/resources/icon_'+cityRec[i]+'.gif"/></td>';
		}
		tabla += '<td>Siguiente ejecución</td><td colspan="3">Acciones</td><td width="68">Estado</td></tr>';
...
if(getVar("s_h_deals")==0){
	h.innerHTML = 'Mostrar Rutas';
	$("dealers").style.display="none";
}else{
	h.innerHTML = 'Ocultar Rutas';
...
if(getVar("s_h_inform")==0){
	h.innerHTML = 'Mostrar Informacion';
	$("dealer_inform").style.display="none";
}else{
	h.innerHTML = 'Ocultar Informacion';
...
if(getVar("s_h_image")==0){
	h.innerHTML = 'Mostrar Imagen';
}else{
	h.innerHTML = 'Ocultar Imagen';
}
		...
		if (nodeRef.childNodes[i].nodeType == document.ELEMENT_NODE) {
			...
			if ((tag=="OPTION")&&((cls=="coords")||(cls.substring(0,cls.length-1)=="tradegood"))){
				...
				if(cls=="coords"){
					...
					switch(cls){
						case "tradegood1": cityRec[ind_cityId] = "Vino"; break;
...
function obt_resource(text){
	...
	//return parseInt((text.substring(ini2,fin).replace(",",""))/cap);
}
