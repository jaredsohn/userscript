// ==UserScript==
// @name		Ikariam Manager (mobile version)
...
// @version		1.0.0
// @icon		http://s1.picofile.com/file/7430639351/Ikariam_Manager_Icon.jpg
// @include		http://m*.ikariam.*/*
// @include		http://m666*.*.ikariam.*/*
...
var IKM =
{
	...
	icon_army				: '/skin/img/city/building_barracks.png',
	icon_fleet				: '/skin/img/city/building_shipyard.png',
	icon_move_army			: '/skin/actions/move_army.png',
	icon_move_fleet			: '/skin/actions/move_fleet.png',

	icon_move_army_disabled	: '/skin/actions/move_army_disabled.png',
	icon_move_fleet_disabled: '/skin/actions/move_fleet_disabled.png',
	icon_transport			: '/skin/actions/transport.png',

//	icon_transport			: '<img src="/skin/characters/fleet/40x40/ship_transport_r_40x40.png" align="absmiddle" alt="ship" height="20" />', // '<img class="Action medium" src="/skin/actions/transport.png" align="absmiddle" />',
	icon_transport_disabled	: '<img src="/skin/actions/transport_disabled.png" align="absmiddle" width="20" vspace="5" />',

	icon_safe_res			: '/skin/layout/icon-wall.png',
...
*/
	Renders :
	{
		...
		icon_to_safehouse_reports : function(currentCityId,Title)
		{
			...
			// skin/img/city/building_safehouse.png
			return '<a view="safehouse" tab="reports" cityid="' + sCityId + '" position="' + sCityPos + '" href="?view=safehouse&id=' + sCityId + '&position=' + sCityPos + '&tab=reports" title="' + Title + '"><img align="absmiddle" src="skin/buildings/x40_y40/safehouse.png"/></a>';
		},
		...
		icon_to_research_advisor : function(Title)
		{
			...
			return '<a view="researchAdvisor" href="?view=researchAdvisor" title="' + Title + '"><img align="absmiddle" src="skin/resources/icon_scientist.png"/></a>';
		},
		...
		icon_to_research_overview : function(currentCityId,Title)
		{
			...
			// skin/icons/researchbonus_30x30.png
			// skin/img/city/building_academy.png
			return '<a view="researchOverview" cityid="' + currentCityId + '" href="?view=researchOverview&id=' + currentCityId + '" title="' + Title + '"><img align="absmiddle" src="skin/buildings/x40_y40/academy.png"/></a>';
		},
...
#IkariamManager TABLE.Overview thead { background: #E7C680 url(skin/input/button.png) repeat-x scroll 0 0; }
...
#IkariamManager TABLE.Overview TR.underOccupation TD.city_name { background: #FCDDD2 url(skin/icons/occupation_warning.png) top right no-repeat;}
...
#IkariamManager .moveIcon.army { background:url(skin/actions/move_army.png) center center no-repeat;}
#IkariamManager .moveIcon.fleet { background:url(skin/actions/move_fleet.png) center center no-repeat;}

.unitIcon { display:inline-block; width:35px; height:35px; }
.unitIcon.Navy { background:url(skin/characters/fleet/unitfleets_x34_y34.png) 0 0 no-repeat; }
.unitIcon.Army { background:url(skin/characters/military/unitsprites_x34_y35.png) 0 0 no-repeat; }
...
.unitIcon.Army.Large { width:48px; height:48px; background:url(skin/characters/military/buttons_sprite.png) 0 0 no-repeat; border-radius:7px; }
...
.unitIcon.Navy.Large { width:48px; height:48px; background:url(skin/characters/fleet/buttons_sprite.png) 0 0 no-repeat; border-radius:7px; }
...
*/
	Ika :
	{
		...
		{
			// Values: deployarmy, deployfleet, plunder, blockade, defend, defend_port, trade, transport, occupy
			return 'skin/interface/mission_' + mission + '.png';
		},
		...
		Get_Happiness_ImgSrc : function(growth)
		{
			...
			//tag = 'skin/smilies/' + imagen + '.png';
			//tag = 'skin/smilies/' + imagen + '_x32.png';
			return '/skin/smilies/' + imagen + '_x32.png';
		},
...
var Link =
{
	...
	finance_navy_views : function()
	{
		...
		rHTML += '<a href="?view=merchantNavy" title="View merchant navy"><img align="absmiddle" src="skin/img/city/building_port.png" /></a>';
		rHTML += (Tools.View.report_to_survey ('merchantNavy') == IKM.icon_require_attention) ? Tools.require_attention () : '&nbsp;';
		rHTML += '<a href="?view=finances" title="View finances"><img align="absmiddle" src="skin/img/city/building_townhall.png" /></a>';
	...
	agora : function(city_id)
	{
		...
		{
			return '<a href="?view=islandBoard&id=' + res.island_id + '" title="View island agora"><img hspace="3" height="12" src="skin/board/schriftrolle_offen2.png" align="absmiddle" /></a>';
		}
		return '';
	},
	...
	{
		var rHTML = '<a href="?view=city&cityId=' + city_id + '" class="changeCity" cityid="' + city_id + '" title="View city"><img align="absmiddle" src="skin/layout/icon-city2.png" /></a>';
...
//		var text = '<img align="absmiddle" src="' + IKM.icon_army + '" />';
	...
	map : function (city_id)
	{
		...
		if ((res.island_id != undefined))
		{
			...
			{
				rHTML += '<a class="linkToMap" href="?view=island&id=' + res.island_id + '&selectCity=' + city_id + '" title="View island"><img align="absmiddle" src="skin/layout/icon-island.png" /></a>';
			}
		}
	...
	resources : function (city_id, prodgood)
	{
		...
		var rHTML = '';
		if (prodgood != 1)	{ rHTML += '<a class="linkToResource changeCity" cityid="' + city_id + '" href="?view=resource&type=resource&id=' + res.island_id + '" title="' + _t('wood') + '"><img height="' + IKM.icon_res_size + '" align="absmiddle" src="skin/resources/icon_wood.png" alt="' + _t('wood') + '" /></a>&nbsp;'; }
		if (prodgood != 0)	{ rHTML += '<a class="linkToResource changeCity" cityid="' + city_id + '" href="?view=tradegood&type=tradegood&id=' + res.island_id + '" title="' + _t(res.prodgood) + '"><img height="' + IKM.icon_res_size + '" align="absmiddle" src="skin/resources/icon_' + res.prodgood + '.png" alt="' + _t(res.prodgood) + '" /></a>&nbsp;'; }
		return rHTML;
	},
...
*/
	...
	Income :
	{
		...
		create : function (prodPerHour, extraTooltip, classname)
		{
			...
			{
				var tooltip = "<img src='/skin/resources/icon_gold.png' align='absmiddle' title='" + _t('Upkeep') + "' /> &nbsp; "
...
if (current_city.main_id > 0)
{
	...
	else if (IM.View.is (['cityMilitary-army', 'cityMilitary-fleet']))
	{
		...
		if (IM.Ika.view() == "cityMilitary-army")
		{
			...
			if (has_cooks)
			{
				$('#mainview .buildingDescription').append ('<p style="font-size:32px; color:red;"><img src="/skin/characters/military/x60_y60/y60_cook_faceright.png" align="absmiddle"> ' + has_cooks + '</p>');
			}
		}
	...
	else if (IM.View.is('safehouse') && IM.Ika.Tab() == 'reports')
	{
		$('TABLE.reportTable').each ( function (k, v)
		{
			...
			$('TH', r1).each (function (key, value)
			{
				...
				{
					var url_unit = $('img', this).attr('src').replace(/(.+)\/y[0-9]+_(.+)_faceright.png/gi, '$2');

					// it's not unit, it's a ship :D
					if (url_unit.indexOf('/') > -1)
					{
						url_unit = $('img', this).attr('src').replace(/(.+)\/ship_(.+)_r_40x40.png/gi, 'ship_$2');
					}
				...
				if (payload.length > 0)
				{
					...
					for (var j = 0; j < payload.length; j++)
					{
						if (payload[j].src.indexOf('resources/') > 0)
						{
							...
							if (/icon_(wood|wine|marble|glass|sulfur)\.png/.exec(payload[j].src) != null)
							{
	...
	if (IM.preference ('TABLE_BUILDINGS', true))
	{
		...
		for (key in orderedBuildings)
		{
			if (buildingsCount[key] > 0)
			{
				...
				var img = "<p align='center' class='pad2'><img src='/skin/img/city/building_" + (key == 'townHall' ? 'townhall' : key) + ".png' class='buildingImage' style='max-width:60px;'></p>";
						...
						if (res.buildings[key] != undefined)
						{
							...
							for (position in res.buildings[key].levels)
							{
								...
								var img = IM.preference ('buildingTooltip')
									? "<span class='myttip'><h6>" + buildings[key][0] + " [" + level + "]</h6><span class='acenter'><img src='/skin/img/city/building_" + (key == 'townHall' ? 'townhall' : key) + ".png' class='buildingImage' style='max-width:100px;'></span></span>"
									: '';
...
//		s += "<p class='Caption'>(<span class=Green>1-14</span>) " + _t('availFS') + ". (<span class=Red>" + IKM.icon_require_attention + "</span>) " + _t('requireAtt') + ".</p>";
	...
	if (IM.preference ('TABLE_RESOURCES', true))
	{
		...
		s += "<th class='city_name' nowrap>"
			...
			+ "</th>"
			+ "<th colspan='3' class='lf population'><a href=''><img src='/skin/characters/40h/citizen_r.png' alt='" + _t('Population') + "' /></a></th>"
			+ "<th class='lf research'><a href='?view=researchAdvisor'><img src='/skin/characters/40h/scientist_r.png' alt='" + _t('Research') + "' /></a></th>"
			+ "<th class='research'><img src='/skin/characters/40h/templer_r.png' alt='' /></th>"
			+ "<th class='lf incomes'><img src='skin/resources/icon_gold.png' alt='" + _t('Incomes') + "' /><br />" + _t('Incomes') + "</th>"
			+ "<th colspan='2' class='lf wood' title='" + _t('wood') + "'><img src='skin/resources/icon_wood.png' alt='" + _t('wood') + "' /><br />" + _t('wood') + "</th>"
			+ "<th colspan='3' class='lf wine' title='" + _t('wine') + "'><img src='skin/resources/icon_wine.png' alt='" + _t('wine') + "' /><br />" + _t('wine') + "</th>"
			+ "<th colspan='2' class='lf marble' title='" + _t('marble') + "'><img src='skin/resources/icon_marble.png' alt='" + _t('marble') + "' /><br />" + _t('marble') + "</th>"
			+ "<th colspan='2' class='lf crystal' title='" + _t('crystal') + "'><img src='skin/resources/icon_glass.png' alt='" + _t('crystal') + "' /><br />" + _t('crystal') + "</th>"
			+ "<th colspan='2' class='lf sulfur' title='" + _t('sulfur') + "'><img src='skin/resources/icon_sulfur.png' alt='" + _t('sulfur') + "' /><br />" + _t('sulfur') + "</th>"
			+ '';
		s += "</tr></thead>";
			...
			if (Tools.Building.level(CityId, "temple", 0) > 0)
			{
				...
				if (priests != IKM.uknown)
				{
					...
					priests_string = Tooltip.create ('<a href=\'' + link + '\'>' + priests + '</a>' + '<font class="More">' + Tools.Array.value (res.buildings["temple"].wonder, "belief", IKM.uknown) + '%</font>',
						'<table><tr>'
						+ '<td nowrap><img src=\'/skin/wonder2/wonder' + Tools.Array.value (res.buildings["temple"].wonder, "id", IKM.uknown) + '.png\' height=\'80\' hspace=10 /></td>'
...
//			var city_linkP = Link.townHall ((population > 0 ? IM.Str.number_format(population) : IKM.uknown), CityId);
		...
		s += "<td class='city_name'><img class='fright' vspace='2' hspace='5' src='skin/layout/sigma.png'></td>"
			...
			for (key in unit_types)
			{
				...
				if (IKM.show_all_units || usedIndexes[key] == 1)
				{
					...
					var img = (type == 'military')
						? 'skin/characters/military/x60_y60/y60_' + un + '_faceright.png'
						: 'skin/characters/fleet/60x60/' + un + '_faceright.png';
...
//		s += "<th class='upkeep lf' nowrap title='" + _t("Upkeep") + "'>" + _t("Upkeep") + "</th>";
		...
		for (CityId in Cities)
		{
			...
			else if (theCity.own == true)
			{
						...
						if (IKM.show_all_units || usedIndexes[key] == 1)
						{
							...
							if (ggg > 0)
							{
								s += Tooltip.create_attribute (
									"<img src='/skin/relatedCities/general.png' align='absmiddle' title='" + _t('generals') + "' /> &nbsp; " + ggg + '<br />' +
									"<img src='/skin/resources/icon_upkeep.png' align='absmiddle' title='" + _t('Upkeep') + "' /> &nbsp; " + uuu
									);
							}
				...
				s += "<td class='details lf advanced'><b " + Tooltip.create_attribute (an.units + ' ' + _t('units') + " <img src='/skin/resources/icon_upkeep.png' align='absmiddle' title='" + _t('Upkeep') + "' /> &nbsp; " + IM.Str.number_format(an.upkeep_army) + " (" + an.upkeep_indestructible + ")") + ">"
					+ an.army + "</b>"
					"</td>";
				s += "<td class='details lf advanced'>"
					+ "<b " + Tooltip.create_attribute (an.ships + ' ' + _t('units') + " <img src='/skin/resources/icon_upkeep.png' align='absmiddle' title='" + _t('Upkeep') + "' /> &nbsp; " + IM.Str.number_format(an.upkeep_navy) + " (" + an.upkeep_navy_indestructible + ")") + ">"
					+ an.navy + "</b>"
					"</td>";
				s += "</tr>";
			}
		...
		s += "</tbody><tfoot class='Summary'><tr class='aright'><td class='city_name'><img class='fright' vspace=2 hspace=5 src='skin/layout/sigma.png'></td>";
...
//		s += "<td class='details upkeep lf'>" + IM.Str.number_format(-1 * Math.round(sumUpkeep), true) + "<font class='More'>" + IM.Str.number_format(-1 * Math.round(sumConstructionUpkeep), true) + "</font></td>";
		...
		s += "<tr class='Units'>";
		s += "<td class='city_name'><span class='fright'><img vspace=2 hspace=5 src='/skin/relatedCities/general.png' alt='generals score'><span class='advanced'><br /><img vspace=2 hspace=5 src='/skin/resources/icon_upkeep.png' alt='upkeep'></span></span></td>";
