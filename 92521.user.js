...
var DOWNFIGHTDE_LEGIONPAGE_URL = "http://www.downfight.de/?seite=listestufe5highscore";
...
} else if (location.toString().indexOf("sampa") != -1) {
	var TOWNBASE_URL = 'http://sampa.faveladogame.com.br/';
	var ZONEBASE_URL = ''; 
	var TOWNEXTENSION = 'MD';
	var DFTownCode = "";
	var lang = 4;
// Wenn in Rio de Janeiro gespielt wird
} else if (location.toString().indexOf("www.faveladogame.com.br") != -1) {
...
function InsertInfoInFirstTable(table, inGang) {
	...
	for (var x = 1; x <= trs.length - 2; x++) {	
		...
		tds[inGang+3].id = 'userprofileid:' + userid + "_" + (GetNrOfIDs(document, 'userprofileid:' + userid) + 1);
		tds[inGang+3].getElementsByTagName("a")[0].setAttribute('style', 'color: #33cc66');
		tds[inGang+3].getElementsByTagName("a")[0].setAttribute('target', '_blank');
...
// ***********************************************************************************************
// ***********************************************************************************************
function ProcessLogPage(table, ProgressTable, LogPageNr, showLosers, inGang, gangFilter) {
	...
	if (LogPageNr <= Number(document.getElementById("seitebis").value)) {
		...
		GM_xmlhttpRequest({method: 'GET', url: FIGHTLOG_URL + LogPageNr + "/",onload: function(logresponseDetails) {
			...
			ProcessLogPage(table, ProgressTable, LogPageNr + 1, showLosers, inGang, gangFilter)
		}
		});
	// sonst: Die letzte abzurufende Seite wurde erreicht
		...
		InsertAttackableInFirstTable(document.getElementById("content").getElementsByTagName("table")[2], inGang, true, false, showLosers);
	...
	{ 
		...
		if (gangFilter != "") {
			...
			for (var x = trs.length - 2; x >= 8; x--) {
				var gangname = trs[x].getElementsByTagName("td")[5].innerHTML;
				if (gangname.indexOf(">") == -1)
					gangname = "";
				else
					gangname = trimString(gangname.split(">")[1].split("<")[0]).toLowerCase();
				if (!filter(gangname, gangFilter))
						trs[x].parentNode.removeChild(trs[x]);
				}
		} else if (showLosers) {
		...
		compress = false;
		ProcessLogPage(table, ProgressTable, startPage, showLosers, inGang, gangFilter);
	}, false);
}
...
// ***********************************************************************************************
// ***********************************************************************************************
function ReformatFightSearchTable(trs, inGang) {
	...
	for (var x = 1; x <= NrOfCols; x++) {	
		...
		if (tds[inGang+2].innerHTML.indexOf(ICON_FIGHTCOMMENT) != -1) {
//			alert("comment: " + tds[2].getElementsByTagName("img")[2].id);
			// Fightkommenar aus der id auslesen
			var fightcomment = tds[inGang+2].getElementsByTagName("img")[(ZONEBASE_URL == "")?1:2].id;
			// Neue Zeile erzeugen
			var newtr = document.createElement("tr");
...
function HandleSearchpage(firsttable, inGang) {
	...
	ReformatFightSearchTable(trs, inGang);
...
function InsertCheaterTable() {
	...
	GM_xmlhttpRequest({method: 'GET', url: DOWNFIGHTDE_CHEATERPAGE_URL, onload: function(responseDetails) {
		...
		if (responseDetails.status == 200) {
			...
			// ***********************************************************************************************
			function GetUserData(username, dftrs, i, newtable, x, days) {
				// ***********************************************************************************************
			...
			for (var i = 1; i <= dftrs.length - 1; i++) {
				...
				GetUserData(username, dftrs, i, newtable, nrofcheaters, fakerdays);
...
// ***********************************************************************************************
// ***********************************************************************************************
// Funktion ruft die downfight.de-Legionäre ab und stellt sie in der Legiontabelle dar
// ***********************************************************************************************
// ***********************************************************************************************
function InsertLegionTable() {

	// Wartecursor
	CursorWait(document.getElementById("LegionCheckbox"), true);

	// ***********************************************************************************************
	// Abrufen der Legionärsliste von downfight.de
	// ***********************************************************************************************
	GM_xmlhttpRequest({method: 'GET', url: DOWNFIGHTDE_LEGIONPAGE_URL, onload: function(responseDetails) {
		// Wenn die Seite erfolgreich abgerufen werden konnte
		if (responseDetails.status == 200) {
			// ***********************************************************************************************
			// Funktion fügt einen Header in die Tabelle table ein
			...
			function InsertHead(table, nrofcolumns) {
				...
				newtr.getElementsByTagName("th")[2].innerHTML = "Legionär";   // Username
				newtr.getElementsByTagName("th")[2].setAttribute('style', 'width: 175px');
				...
				newtr.getElementsByTagName("th")[5].innerHTML = "Kills";      // Kills
				newtr.getElementsByTagName("th")[5].setAttribute('style', 'width: 40px; text-align: right; padding-right: 10px');
			...
			// ***********************************************************************************************
			function GetUserNameFromDFDE(content) {
				return trimString(content.split('target="_blank">')[1].split('</a>')[0]);
			}
			...
			function GetUserData(username, dftrs, i, newtable, x) {
				...
				GM_xmlhttpRequest({method: 'GET', url: API_URL + 'getname.xml?name=' + username, onload: function(responseDetails) {
					...
					GM_xmlhttpRequest({method: 'GET', url: PROFILE_URL + userid + "/", onload: function(responseDetails) {
						var profilecontent = responseDetails.responseText;
						// Zelle mit Bandenbeschreibung des Users ermitteln
						currenttd = document.getElementById("legionid" + userid).getElementsByTagName("td")[3];
						// Wenn der User gelöscht oder gebannt ist
						if (profilecontent.indexOf('"Der Spieler wurde gel&ouml;scht oder vom Spiel verbannt!"') != -1) {
							currenttd.innerHTML = '<b>GEBANNT/GELÖSCHT!</b>';
							currenttd.setAttribute('style', 'color:#ff6666');
			...
			// Neue Tabelle erzeugen
			var newtable = document.createElement("table");
			document.getElementById("legiontablehost").appendChild(newtable);
			...
			var nroflegions = 0;
			for (var i = 1; i <= dftrs.length - 1; i++) {
				if (dftrs[i].getElementsByTagName("td")[4].innerHTML.indexOf(TOWN_URL) == -1)
					continue;
				if (dftrs[i].getElementsByTagName("td")[5].innerHTML != townname)
					continue;	
				nroflegions++;
				// Username ermitteln
				var username = GetUserNameFromDFDE(dftrs[i].getElementsByTagName("td")[4].innerHTML);
				...
				// Neue Zeile erzeugen
				var newtr = InsertRow(newtable);
				GetUserData(username, dftrs, i, newtable, nroflegions);
		
			}
			// Info über die Anzahl der Legionäre über Tabelle schreiben
			document.getElementById("legioninfo").innerHTML = '<b><font color="#FFFFFF">Derzeit befinden sich ' + nroflegions + ' Penner auf der Legionärsliste (' + townname + ') von <a href="http://www.downfight.de" target="_blank">downfight.de</a>:</font></b><br /><br />';
				
			// Wartecursor
			CursorWait(document.getElementById("LegionCheckbox"), false);
		}
	}
	});
}
...
// **********************************************************************************
// **********************************************************************************
// Funktion enthält Code zum Handeln der Legionlisten-Anzeige
// **********************************************************************************
// **********************************************************************************
function HandleLegionCheckbox() {
	var legioncheckbox = document.getElementById("LegionCheckbox");

	// Click-Handler hinzufügen
	legioncheckbox.addEventListener("click", function(event) { 
		// Klickstatus speichern
		GM_setValue("ShowLegionlist" + m_ownuserid + TOWNEXTENSION, this.checked);
		
		// Wenn das Häkchen gesetzt ist
		if (this.checked) {
			// Legionliste abrufen und darstellen
			InsertLegionTable();
		// sonst: Das Häkchen ist nicht gesetzt
		} else {
			// Info über Anzahl der Legionäre und die Legionärstabelle selbst löschen
			document.getElementById("legioninfo").innerHTML = "&nbsp;";
			document.getElementById("legiontablehost").innerHTML = "&nbsp;";
		}
	}, false);
}
...
// **********************************************************************************
// **********************************************************************************
// Funktion bereitet die Tabelle auf das Einfügen der Legion-Tabelle vor
// **********************************************************************************
// **********************************************************************************
function PrepareLegionTable(table) {
	
	var newtr = document.createElement("tr");
	var newtd = document.createElement("td");
	newtd.setAttribute('style', 'padding-top: 10px; padding-bottom: 10px');
	newtd.innerHTML = "1";
	newtd.id = "LegionCheckboxTD";
	newtd.innerHTML = '<input name="LegionCheckbox" id="LegionCheckbox" type="checkbox"><span style="vertical-align: bottom">&nbsp;Legionäre von downfight.de anzeigen</span>';
	newtr.appendChild(newtd);
	table.getElementsByTagName("tbody")[0].appendChild(newtr);

	// Zeile einfügen, in der später die Legionärsanzahl steht
	newtr = document.createElement("tr");
	newtd = document.createElement("td");
	newtd.setAttribute("colspan", 2);
	newtd.id = "legioninfo";
	...
	// Zeile einfügen, in der später die Legionärstabelle steht
	newtr = document.createElement("tr");
	newtd = document.createElement("td");
	newtd.id = "legiontablehost";
	newtd.setAttribute("colspan", 2);
	newtr.appendChild(newtd);
	table.getElementsByTagName("tbody")[0].appendChild(newtr);

	// Code für Click hinterlegen
	HandleLegionCheckbox();
	
	// Wenn die letzte Einstellung der Legion-Checkbox "aktiv" war
	if (GM_getValue("ShowLegionlist" + m_ownuserid + TOWNEXTENSION, true)) {
		// Häkchen setzen
		document.getElementById("LegionCheckbox").checked = true;
		// Legionliste abrufen und darstellen
		InsertLegionTable();
	}
}
					...
					// ***********************************************************************************************
						...
						for (var i = 0; i < trs.length; i++) {
							...
							if (inGang && document.getElementById("content").getElementsByTagName("table")[1].getElementsByTagName("tr").length > 2) {
								trs[i].innerHTML = '<td style=width:50%>' + text + '&nbsp;&nbsp; <input name="postVals" id="postVals" type="checkbox" onclick="document.getElementById(\'bbcode\').style.visibility=\'hidden\'"> &nbsp;&nbsp;' + TxtPostFight[myLang] + '</td><td name="Formular2" action=""><input type="button" value="BBCode" id="bbcodeanzeigen" title="' + TxtShowBBCode[myLang] + '"></form>&nbsp;<textarea rows="1" style="vertical-align:middle" name="bbcode" action="" type="text" id="bbcode" onclick="this.focus(); this.select();"></textarea></td>';
								bbcodeEinAus();
							}
							break;
						}
