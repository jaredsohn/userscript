// ==UserScript==
// @name 	Ikariam:  Inspekcija 
// @author 	Original:  Phate, Serbian edit:  forest_van_deagle
// @version	4.06
// @description	A script for Ikariam v0.4.4 that colors leechers 
...
//
// @history	4.06 I improved the code for deletion of missing players.
// @history	4.06 FixBug: In the table of donations was missing a player's name.
// @history	4.05 I recommend the reset data. Modified saved data. I do not use names of the players but players ID.
// @history	4.05 FixBug: error if delete inactive player (aryTr[page].key is undefined ).
// @history	4.04 FixBug: I write better code for resource.
// @history	4.04 FixBug: I write better code for island view.
// @history	4.03 Modified the code to adapt it to version 0.4.2.4.
...
// ==/UserScript==
const lversion = '4.06';
ScriptUpdater.check(52047,lversion);
...
{ 	//LANGUAGES
	...
	{ // English traslate by Paul93
		...
		infotxt2: "Donatija = (Plata * broj radnika) + (Prilog * nivo gradske kuce)",
		infotxt3: "Plata",
		infotxt4: " = Plata po radniku | ",
		infotxt5: "Prilog",
		infotxt6: " = Prilog za svaki nivo grada",
		infotxt7: "Legends:",
		info1ico: "Ulagač!!",
		info2ico: "Parazit, ali 10% jos treba da se uloži.",
		info3ico: "Pravi parazit.",
		scorewrk: "Donacija po radniku",
		scorelvl: "Donacija za svaki nivo gradske kuce",
		chkboxvl: " Uračunaj oba ostrvska dobra",
		stat1:	"U cilju da se dostigne nivo",
		stat2:	" potrebno je još",
		stat31:	" da se priloži.",
...
try
{
	...
	if (page == 'tradegood' || page == 'resource')// Check if you are at a resource deposit
	{
		...
		$('tr.avatar').each(function(id) {$(this).bind('mouseenter',function(){openRtdWindow (tb[id].id,tb[id].name,maxDonate,id)})});	
		// view history button
	...
	{	
		...
		var myisle = false;
		if ($('#cities div.ownCityImg').length >0) myisle=true;
		...
		{
			$("#cities li.city").each(function()
			{
				var ply = $.trim($(this).find('tr.owner td.nameValue').next('td').text());
				var ally = $.trim($(this).find('tr.ally a:first').text());
		...
		if (myisle) // reset old item of inactive and vacation player
		{
			...
			$("#cities li.city:has(span.inactivity)").each(function()
			{
				ply = parseInt($(this).find('a.sendMessage').attr('href').split('receiverId=')[1])
				//ply = $.trim($(this).find('tr.owner td.nameValue').next('td').text());
				if ($.inArray(ply,islePar.island.inactive)<0) islePar.island.inactive.push(ply);
			});
			//Find vacation player
			$("#cities li.city:has(span.vacation)").each(function()
			{
				ply = parseInt($(this).find('a.sendMessage').attr('href').split('receiverId=')[1])
				//ply = $.trim($(this).find('tr.owner td.nameValue').next('td').text());
...
function doLeecherChecker(recall)	// Colors players in the donation list accroding to their leeching
{
	...
	if (recall == false)		// when table recall, not delete
	{
		...
		$('td.ownerName').each(function(index)	// check deleted line
		{
			...
			{
				line.attr('name','ISC_Delete');
				line.find('td').attr('name','ISC_Delete');
			}
			else
			{
				line.attr('name','ISC_row')
			}
		});
		$('td.cityName').each(function()	// check warker
		{
			if ($(this).attr('name') == 'ISC_Delete')
				$(this).parent().prevAll('tr[name=ISC_row]:first').find('td.cityName').append('<div>'+ $(this).text()+'</div>');
			else $(this).html('<div>' +$(this).text()+'</div>');
		});
		$('td.cityWorkers').each(function()	// check warker
		{
			if ($(this).attr('name') == 'ISC_Delete')
				$(this).parent().prevAll('tr[name=ISC_row]:first').find('td.cityWorkers').append($('<div>'+ $(this).text()+'</div>'));
			else $(this).html('<div>' +$(this).text()+'</div>');
		});
		$('td.cityLevel').each(function()	// check level city
		{
			if ($(this).attr('name') == 'ISC_Delete')
				$(this).parent().prevAll('tr[name=ISC_row]:first').find('td.cityLevel').append($('<div>'+ $(this).text()+'</div>'));
			else $(this).html('<div>' +$(this).text()+'</div>');
		});
		...
		// class rows
		$('#resourceUsers tr[name=ISC_row]').filter(':odd').attr('class','avatar').end().filter(':even').attr('class','alt avatar');
		// css style cells
	...
	if (recall == false)		// when table recall, not insert new HTML
	{
		...
		{
			$(this).attr('name',$(this).text());	//name for real time trend
			var idName = $(this).parent().find('a').attr('href')
			if (idName) idName = parseInt(idName.split('receiverId=')[1]);
			else	idName = 0
			$(this).attr('value',idName);	//name for real time trend
	...
	$(tb).each(function(id)
	{
		...
		this.leec = this.don - score;
		// get other donation in tradegood or resource
		var othDon = (islePar.allDon && !isNaN(islePar[othpage].player[this.id]))? islePar[othpage].player[this.id]: 0;
		//Add value leecher
		var don = this.don + othDon - score;
		// if recall control, not color inactive or vacation player
		if ($.inArray(this.id,islePar.island.inactive) < 0 && $.inArray(this.id,islePar.island.vacation) < 0)
			{
				// color row
				$('#resourceUsers tr[name=ISC_row]').eq(id).css('color',function()	
				{
		...
		else if ($.inArray(this.id,islePar.island.inactive)<0) this.I = true;
		else if ($.inArray(this.id,islePar.island.vacation)<0) this.V = true;
		...
		if (islePar.allDon)	//tooltip all donation
		{
			...
			html +=	"<table class='tooltip'>"+
						"<tbody>"+
							"<tr style='color:"+ color1 +"'>"+
								"<td style='text-align: right'>" + islePar[page].typ + "</td><td> = </td><td style='text-align: right'>" + replaceNum(this.don - score) + "</td>"+
							"</tr>"+
							"<tr style='color:"+ color2 +"'>"+
								"<td style='text-align: right'>" + islePar[othpage].typ + "</td><td> = </td><td style='text-align: right'>" + replaceNum(othDon) + "</td>"+
							"</tr>"+
						"</tbody>"+
					"</table>";
		}
	...
	$('td.ownerLeecher table.tooltip, td.ownerLeecher table.tooltip tbody')
		.css({'position':'absolute','right':'100px','width':'auto','background':'#fbfafd','border-radius':'5px','border':'1px solid #767676'});
	$('td.ownerLeecher table.tooltip td').css({'padding':'1px 4px'});
	$('td.ownerLeecher').hover(function (){$(this).find('table.tooltip').show()},function (){$(this).find('table.tooltip').hide()});
			...
			do		// Search new share for deposit Upgrading
			{
				...
				$(tb).each(function()
				{
					if(this.I != true)
					{
						...
						var othDon = (islePar.allDon)? islePar[othpage].player[this.id]: 0;
	...
	$(islePar.island.inactive).each(function(id,name) 
	{
		$('#resourceUsers tbody tr:has(td.ownerName[value='+ name +'])').css('color','gray')
			.find('td.ownerName').append(' (i)')
	});
	$(islePar.island.vacation).each(function(id,name) 
	{
		$('#resourceUsers tbody tr:has(td.ownerName[value='+ name +'])').css('color','royalblue')
			.find('td.ownerName').append(' (v)')
	});
}
...
{
	var Par = eval(localStorage.getItem(idisle + suf));	// Donation trend
...
try
{
	...
	var aryTr = getDataTrend(false,typ);
	...
	{
		if(this.data.length > nItem) 
		{
			delData = this.data[0][0]
			return false;
		}
	});
	if (delData > 0)	// delete old value
	{
		$.each(aryTr[page], function()		{if(this.data[0][0] <= delData) this.data.shift()});	//delete first item
		newPly={};
		$.each(aryTr[page], function(id)	{if(this.data.length > 0) newPly[id] = this});			//delete old players
		aryTr[page]= newPly
	}
	$('#resourceUsers tbody tr td.ownerName').each(function()	//player list
	{
		var idPly = $(this).attr('value')
		if (!aryTr[page][idPly])	//inizialize array real time trend
		{
			aryTr[page][idPly]={};
			aryTr[page][idPly]['data']=[];
		}
		aryTr[page][idPly]['label']= $(this).attr('name');	// name Player
		aryTr[page][idPly]['data'].push([today,$(this).nextAll('td.ownerDonation').text().replace(/\D/g,'') *1]);	// donation player
	});
...
{	
	...
	$.each(Hy[page], function(key, val) {			
		$("#choises").append(
			'<div>'+
				'<span class="ISC_border">'+
					'<div class="ISC_box" style="background-color:'+ lineColor[c] +';"></div>'+
					'<input type="checkbox" name="'+ val.label +'" checked="checked" id="'+ key +'id">'+
				'</span>'+
				'<label for="'+ key +'id">'+ val.label +'</label>'+
			'</div>');
			c++;
		});
		
	$("#choises span.ISC_border").css({'display':'inline-block','border':'1px solid rgb(204, 204, 204)','padding':'1px','height':'13px','margin':'1px 0px'});
	$("#choises input").css({'margin-left':'1px'});
	$("#choises div.ISC_box").css({'display':'inline-block','width':'13px','height':'13px'});
	$("#choises label").css({'padding-left':'2px','font-size':'9px','position':'relative','top':'-3px'});
	
	$("#choises input").bind('click',function()
	{
...
try
{
	...
	$("#choises").find("input:checked").each(function () 
	{
		...
		var html='<table class="ISC_dataTrend">'+
					'<thead>'+
					'<tr>'+
						'<th colspan="3">'+ $(this).attr("name") +'</th>'+
					'</tr><tr>'+
...
function openRtdWindow (idName,name,scale,paint)	// plot real time trend
{
try
{
	// title header
	...
	if (rtdPar[page][idName])
	{
		...
		$(tb).each(function()
		{
			if (this.id == idName)
			{
				...
				rtdPar[page][idName].data.push([date,this.don]);
				//Calculate time and donation
				var incDon =  Math.round((this.don - rtdPar[page][idName].data[0][1])/1000);
				var incDay = Math.ceil((date - rtdPar[page][idName].data[0][0])/(1000*60*60*24));
...
function saveIsleParPlayer()
{
	...
	$(tb).each(function() { islePar[page].player[this.id] = this.leec });	// How player  donated in this resource
	//save parameter
	localStorage.setItem(idisle + "_isle",uneval(islePar));
}
