// ==UserScript==
// @name           Toy Box Point Totaller
// @namespace      http://userscripts.org/users/192956
// @description    Version 1.5 Organizes raid logs to a readable format - modified for Toy Box by Thagorn (thanks to Slyz and Dr.Evi1)
...
// ==/UserScript==
// --------------------------------------------------------------------------------------
// Modified Dr3v1's Raid Log Manager (Modified again from Slyz's version)
// --------------------------------------------------------------------------------------
// 08/06/2010 Version 1.5 - Added table to EE displaying diverts and broken pipes
// 08/03/2010 Version 1.4 - Fixed minor bug introduced by 1.3
// 08/03/2010 Version 1.3 - Modified the Sewer Tunnel table to include grates and valves
// 07/20/2010 Version 1.2 - Modified the included pages to properly work outside of the relay browser
// 07/15/2010 Version 1.1 - Modified the Points Totals box to display players who earned points in the sewer but not in Hobopolis proper 
// 07/15/2010 Version 1.0 - Modified the point system to match the Toy Box
// --------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
// slyz's SPNG hack - Dec 18, 2008 (Modified for Toy Box - July 15, 2010)
...
var twn_Ruined = 0 ;
var twn_Ruined_2 = 0 ;
// First 15 advs at Richard's : 1 pt, then 0, then -0.75
var nb_Richard = 0 ;
var twn_Richard = 0 ;
var nb_Richard_2 = 0 ;
var twn_Richard_2 = 0 ;
...
var exp_Broke = 1  ;
var exp_Water_Out = 1 ;
var exp_Water_Burnbarrel = 0 ;
var exp_Yodeled_Little = 1 ;
var exp_Yodeled_Bit = 1 ;
...
var burial_Hobo = 1 ;
var burial_Zombo = -5 ;
var burial_Defeats = 0 ;
var burial_Flowers = 0 ;
var burial_Raided = 0 ;
var burial_Dance = 1  ;
...
var purple_Hobo = 1 ;
var purple_Chester = 0 ;
var purple_Defeats = 0 ;
var purple_Dumpster = 0 ;
var purple_Trash = 0 ;
var purple_Bamboozeled = 0  ;
var purple_Flimflammed = 1 ;
var purple_Danced = 0 ;
var purple_Barfight = 1 ;
...
if ((window.location.pathname == "/choice.php"))
{
    ...
    if (document.body.innerHTML.indexOf("3pipes.gif") >= 0 )
    {
        GM_addStyle(css);
        var EETable = "";
        if (GM_getValue("option_sewer","ON") == "ON")
            EETable = getEETableDef();
            
        document.body.innerHTML = EETable + document.body.innerHTML;
        if (GM_getValue("option_EE","ON") == "ON")
            getEECount();
    }
...
if ((window.location.pathname == "/clan_hobopolis.php"))
{
	var imgstr = document.body.innerHTML.match(/exposureesplanade\d{1,2}.gif/);
    if (document.body.innerHTML.indexOf(imgstr) >= 0)
    {
        if (GM_getValue("option_EE","ON") == "ON")
        {
            GM_addStyle(css);
            var EETable = getEETableDef();
...
if ((window.location.pathname == "/clan_raidlogs.php"))
{
    ...
    var option_EE = "ON";
    if (GM_getValue("option_EE","ON") == "OFF")
      option_EE = "OFF";
    ...
    document.body.innerHTML = "<div style='font-size: 10px;' align='center'>"+
                              ...
                              "             <div id='options_EE' style='font-size: 11px;'>Load EE noncombat table&nbsp;&nbsp;&nbsp;</div>"+
...
//    document.body.innerHTML += slime_boss_table;
    ...
    // EE option log button
    var EEoptionButton = document.createElement("input");
    with(EEoptionButton)
    {
          type = "button";
          value = option_EE;
          ...
          optionButton.addEventListener('click',function(evt) 
          {
              ...
              if (GM_getValue("option_EE","ON") == "OFF")
                option_EE = "ON";
              GM_setValue("option_EE",option_EE);
              value = option_EE;
          },false);
    }
    document.getElementById("options_EE").appendChild(EEoptionButton);
        ...
        var sewerTable = '<center>' +
                         '<table class="normal" width="410">'+
                             '<tr align="center">' +
                                 ...
                                 '<td width="50" class="normal_shaded">sewer grates</td>' +
                                 '<td width="50" class="normal_shaded">sewer valves</td>' + 
                             '</tr>' +
                             '<tr align="center">' +
                                 ...
                                 '<td class="normal_shaded"><img width="30" height="30" src="http://images.kingdomofloathing.com/adventureimages/gate.gif"></td>' +
                                 '<td class="normal_shaded"><img width="30" height="30" src="http://images.kingdomofloathing.com/adventureimages/sewervalve.gif"></td>' +
                             ...
                             '<tr align="center">' +
                                 ...
                                 '<td id="grates" class="normal_total"></td>' +
                                 '<td id="valves" class="normal_total"></td>' +
                             '</tr>' +
                             '</table></center>';
...
function getSewerItems()
{
    ...
    function findStatus(response) 
    {
          ...
          var pageText = response;
          startIndex = 0;
          while (pageText.indexOf("sewer grate", (startIndex+15)) >=0)
          {
          	  startIndex = pageText.indexOf("sewer grate", (startIndex+15));
              var m = pageText.substring(startIndex-5,startIndex).match(/\d{1,2}/);
              if (m != null)
              {
                var n = m[0].match(/\d{1,2}/);
                grates += parseInt(n[0]);
              }
              else
                grates += 1;
          ...
          startIndex = 0;
          while (pageText.indexOf("water level", (startIndex+10)) >=0)
          {
          	  startIndex = pageText.indexOf("water level", (startIndex+10));
              var m = pageText.substring(startIndex,startIndex+20).match(/\d{1,2}/);
              if (m != null)
              {
                var n = m[0].match(/\d{1,2}/);
                valves += parseInt(n[0]);
         ...
         // Apply returnVal to div layer to show results
          document.getElementById("grates").innerHTML = grates;
          document.getElementById("valves").innerHTML = valves;
	});
}
...
function getEETableDef()
{
        var EETable = '<center>' +
                         '<table class="normal" width="160">'+
                             '<tr align="center">' +
                                 '<td width="50" class="normal_shaded">Diversions</td>' + 
                                 "<td width='50' class='normal_shaded'>Broken pipes</td>" +
                             '</tr>' +
                             '<tr align="center">' +
                                 '<td id="diverts" class="normal_total"></td>' +
                                 '<td id="CLUEs" class="normal_total"></td>' +
                             '</tr>' +
                             '</table></center>';
  return EETable;
}
function getEECount() 
{
    ...
    function findStatus(response) 
    {
          ...
          var pageText = response;
          startIndex = 0;
          while (pageText.indexOf("diverted some cold water", (startIndex+15)) >=0)
          {
          	  startIndex = pageText.indexOf("diverted some cold water", (startIndex+15));
              var m = pageText.substring(startIndex,startIndex+55).match(/\d{1,2}/);
              if (m != null)
              {
                var n = m[0].match(/\d{1,2}/);
                diverts += parseInt(n[0]);
              }
              else
                diverts += 1;
          }
          startIndex = 0;
          while (pageText.indexOf("water pipe", (startIndex+10)) >=0)
          {
          	  startIndex = pageText.indexOf("water pipe", (startIndex+10));
              var m = pageText.substring(startIndex,startIndex+20).match(/\d{1,2}/);
              if (m != null)
              {
                var n = m[0].match(/\d{1,2}/);
         ...
         // Apply returnVal to div layer to show results
          document.getElementById("diverts").innerHTML = diverts;
...
function getTotalPoints()
{
    ...
    {	
          ...
          if ((ptsArray[z][9]+swr_char_total_pts[swrIndex]) >= 0)
          {
			  var shaded = "";
			  if (isEven(counter))
				shaded = "";
