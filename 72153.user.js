...
{ 
    var VERSION = '0.25b';
    var url = 'http://docs.google.com/View?docID=0ARKfKdQfUv5wZDh6aHFodl8yOGR4cjI5ZGZy&revision=_latest&hgd=1';
    var updateUrl = 'http://userscripts.org/scripts/show/72153';
...
displayErrFrame("Tvoja verzija skripte je zastarela. (Trenutna: "+VERSION+" | aktualna: "+ver+")", splitLinks("Aktualiziraj!|"+uurl));
	    ...
	    orders = orders.match(/\{([1-5])\}:\s*(.+?)##/)[2];
            mon = '<p><a href="'+mon+'">Vodstvo elite Karantanci</a></p>';
        ...
        {
            displayErrFrame("Zagnana je nova skripta za ukaze! O njej se pozanimajte na forumu!!");
            throw e;
        }
    }
    ...
    function loadURL_GM(url, xhr)
    {
        ...
        xhr({
            ...
            {
                displayErrFrame("(GM) Prišlo je do napake! "+response.status+" ("+response.statusText+") Vodstvo eEU army bomo skušali napako čimhitreje odpraviti!");
            }
        });
    }
    ...
    function loadURL_XHR(url, xhr)
    {
        ...
        xhr.onreadystatechange = function()
        {
            if(xhr.readyState == 4)
            {
                ...
                {
                    displayErrFrame("Prišlo je do napake! "+xhr.status+" ("+xhr.statusText+") Vodstvo elite Karantanci bomo skušali napako čimhitreje odpraviti!");
                }
            }
        };
        xhr.timeout = 15000;
        ...
        {
            displayErrFrame("Stran za elito Karantanci je nedosegljiva.");
        };
