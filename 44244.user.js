// ==UserScript==
// @name           Infinity Attacker
// @description      2.3
// @namespace      Infinity Attacker
// @author         Infinity Travian Plus
// @e-mail         travian@infinity-game.ru
// @version        2.3
...
switch(sLang)
{
	...
	default:
		...
		text = ["兵力調動中","無效的攻擊類型!","未指定攻擊目標!","沒有足夠的軍隊! (軍隊#","沒有設定攻擊軍隊","正在派遣軍隊","錯誤的攻擊目標","沒有偵察軍種","完成派兵","攻擊設定","新增攻擊","還原設定","攻擊設定:","攻擊類型:","正常攻擊","增援","搶奪攻擊","偵察資源/軍隊","偵察防禦/軍隊","攻擊目標","立即派遣軍隊","軍隊到達時間","到達目標於:","設定攻擊到達時間","攻擊的波數","用 <span style='color: red; font-weight: bold'>" + targetSplit + "</span> 分隔多村、 <span style='color: red; font-weight: bold'>" + cordsSplit + "</span> 分隔坐標，如 0" + cordsSplit + "0" + targetSplit + "1" + cordsSplit + "1","錯誤:","無法取得目標村莊的資訊","軍隊到達時間:","延遲時間(毫秒):"];
...
//		if ($('timebetween').value && isInt($('timebetween').value) && $('timebetween').value != timebetweenwaves) {
//			timebetweenwaves = parseInt($('timebetween').value);
//			if (timebetweenwaves <= 0) {
//				timebetweenwaves = 100;
//			}
//			$('timebetween').value = timebetweenwaves;
//		}
...
attackInterface.innerHTML = '<fieldset><legend>'+text[12]+'</legend><div id=start>' +
	...
	'</tr>' +
	'<tr><td colspan="3">'+
		' <input type=\"text\" value="' +getCords() +  '" name=\"cords\" id=\"cords\" class=\"fm\" style=\"width: 98%\" />'+
//	'</td></tr><tr><td>' + 
//		text[29] +'<input class=\"fm\" id=\"timebetween\" type=\"text\" value=\"0\" size=\"4\" />' +
	'</td></tr><tr><td height=50 colspan="3">'+
		'<button id=\"arrivalTime\">'+text[21]+'</button> </td><td> <div id=\"arrivalTimeDiv\"></div> '+
	'</td></tr><tr><td>'+
		'<button id=\"myimbabutton\" >'+text[20]+'</button>' +
	'</td></tr><tr><td colspan="3">'+
		'<button id=\"timedArrivalButton">'+text[23]+'</button> <input id=\"timedArrivalInput\" value=\"hh:mm:ss" class=\"fm\">'+
	'</td></tr></table>';
...
function switchActiveVillage(did) {
	...
	GM_xmlhttpRequest({
		method: "GET",
		url: 'http://' + document.domain + "/a2b.php?"+did,
		headers:{'Content-type':'application/x-www-form-urlencoded'}
	});
	return;
}
