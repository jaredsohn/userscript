...
BulkDeletesNext:function() {
	...
	if(maxObj && maxObj.value!="" && this.bulkDeletesProgress.total>=parseInt(maxObj.value)) {
		GM_log('deletes finished, max limit reached'+maxObj.value);
...
BulkDeletes:function(toggle,total) {
	...
	this.bulkDeletesProgress={u:0,waitForConfirm:false,'toggle':toggle,'total':total?total:0};
...
FindFriendLists:function()
{
	...
	for ( var i = 0; i < friendListsNodes.snapshotLength; i++) 
	{
		...
		var subNode = this.FindByXPath(aList, "a/div/i[contains(@class,'sx_app_icons_friend_list')]");
		if(subNode != null)
		{
