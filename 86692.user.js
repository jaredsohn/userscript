...
//            '</select>'+
            ...
            '<td width="10%">Cash</td>'+
            '<td width="1%">:</td>'+
            '<td width="53%"><span id="cash"><span class="good"><img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_16x16_01.gif"></img> $0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cubanpeso_16x11_01.gif"></img>&nbsp;C$0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_moscow_16x16_01.gif"></img>&nbsp;R$0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_bangkok_16x16_01.gif"></img>&nbsp;B$0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/vegas-chip.png"></img>&nbsp;V$0</span></span></td>'+
            '<td width="1%">&nbsp;</td>'+
...
//            '<td width="25%"><input type="radio" id="Spammers" name="fight_choice">Add Me Spammers</input></td>'+
            ...
            '<td width="40%" id="ice_posts_log" valign="top" style="display:none">Ice Count - Name - Posts<br></td>'+
            '<td width="49%" id="stash_posts_log" valign="top" style="display:none">Finder - Stash Post<br></td>'+
            '</tr>'+
...
//        if(fight_city == 5){
//            var deposit=document.evaluate( "//span[@class='bank_deposit']//a[@class='bank_deposit']", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
...
//                alert(fightlist_faction[fightlist_faction.length - 1]);
        ...
        fighttable = document.evaluate( "//table[@class=\"main_table fight_table\"]//tbody//tr//td", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
        if(fight_city==4){
...
//            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=attack&xw_city=1&tmp='+tmp_key+'&cb='+cb+'&opponent_id='+user_list[m]+'&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
            ...
            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=power_attack&xw_city='+fight_city+'&cb='+cb+'&opponent_id='+user_list[m]+'&origin=profile&xw_client_id=8';
...
//        document.getElementById('user_experience').innerHTML = /user_fields\['user_experience'\] = parseInt\("([0-9]+)/.exec(attack_results)[1];
        ...
        try{
            ...
            if(/(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/.test(attack_results)){
                temp_loot = attack_results.match(/<div class="fightres_bonus_message".+?<img src="(.+?)".+?(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/g);
                for(z=0;z<temp_loot.length;z++){
                    temp = /<div class="fightres_bonus_message".+?<img src="(.+?)".+?(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/.exec(temp_loot[z]);
                    loot_img = temp[1];
...
//                document.getElementById('stash_post'+(stash_posts.length-1)).onclick = new Function(this.id,publish_stash);
        ...
        if(/iced_pop_text/.test(attack_results)){
            ...
            script= script.replace(/http:\/\/mwfb\.static\.zynga\.com\/mwfb\/graphics\/.*\.gif/,'http://photos-h.ak.fbcdn.net/hphotos-ak-snc4/hs657.snc4/61748_157030770993717_100000603857192_404454_5850421_s.jpg');
...
//            document.getElementById('ice_post'+(iced_posts.length-1)).onclick = new Function(this.id,publish_ice);
        ...
        if(/You win!/.test(attack_results)){
            ...
            if(/Attack Again/.test(attack_results)){
                if(/Power Attack/.test(attack_results)){
...
//            cash = cash.replace(/\n/g,'');
            ...
            if(parseInt(/(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_vegas_cash|sexy_new_york_cash) good"\>[^\$]+\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')) < parseInt(cash_limit)){
...
//                document.getElementById('logger').innerHTML = 'You Lost ! Cash Lost - '+cash; 
            cash = /(sexy_cuba_cash|sexy_bangkok_cash|sexy_moscow_cash|sexy_vegas_cash|sexy_new_york_cash) (bad|good)"\>(\n|\f|\r)([^<]+)/.exec(attack_results)[4];
...
//                return;
    ...
    function process_user_attack(attack_results){
        ...
        if(!((/You win/.test(attack_results)) || (/You lose/.test(attack_results)))){
            ...
            var profile_link = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&xw_city='+fight_city+'&cb='+userid+UnixTS()+'&user='+user_list[j]+'&xw_client_id=8';
            ...
            $.ajax({type: "POST",data: params, dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8",
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|V|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Resuming attacks..','attack_log');
                    }
            ...
            $.ajax({type: "POST",data: params, dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8",
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|V|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','status');
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|V|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','attack_log');
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    }
                    else if(msg.user_fields.user_health == msg.user_fields.user_max_health){
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
...
//                                                       /mwfb/remote/html_server.php?xw_controller=propertyV2&xw_action=doaction&xw_city=5&tmp=e0ecab892fecbc344351f807376ec37f&cb=cc723350afe511dfa2e935a8557f41cb&doaction=ActionBankDeposit&amount=525&city=5&building_type=6&xw_client_id=8 
            $.ajax({type: "POST", data: params, 
...
//    	do_ajax('','remote/html_server.php?xw_controller=bank&xw_action=deposit&xw_city='+cash_city+'&cb='+userid+UnixTS()+'&amount='+amount,1,0,0,0);
//    }
//    
...
//        ignore_chars = (document.getElementById('SpecialChars').value=='')?' ':ignore_chars;
        ...
        var cookieStuff = wait1+'|'+wait2+'|'+heal_city+'|'+heal_city_text+'|'+levelup_halt+'|'+levelup_text+'| |'+
                        ...
                        triad_fac+'|'+yakuza_fac+'|'+faction_bal+'|'+ice_check_enable+'|'+power_attack_enable+'|'+user_power_attack_enable;
