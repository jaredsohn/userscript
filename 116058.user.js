// ==UserScript==
// @name           TITAN Estadisticas
// @version        0.0.1
// @namespace      TITAN Estadisticas
// @description    TITAN Estadisticas - By OLID
...
// @require 	   http://www.casadealabanza.mex.tl/488383_116058.html
// ==/UserScript==
var Version = '0.0.1';
var miseajour="Version "+Version +" by OLID. GRACIAS POR PROBAR ESTE SCRIPT";
...
function ptStartup() {
 ...
 GM_addStyle (styles); 
  ...
  AddMainTabLink("Tabla", eventHideShow, mouseMainTab);
  CollectGold.init();
...
var TowerAlerts = {
  ...
  postToChat: function(f, c, force){
    ...
    var mess = "Mi Ciudad esta siendo Atacada " + a +" por " + " " +attaquant + " " + msg;
    var lancement=0;
    sendChat("/a " + mess);
 ...
 my.Hud = {
   ...
   show : function () {
     var t = my.Hud;
       ...
       t.cont.innerHTML = '\
           <DIV class=ptstat>Busqueda de los ataques de los informes de alianza.</div>\
           ...
           <TD class=xtab><INPUT id="idHudSearch" type=submit value="Iniciar la busqueda" />\
  	     ...
  	     if (rslt['winner']==1 || rslt['winner']==2) m+='<TR><TD style="background-color:white;"><FONT color="green" size="3px"><b>Winner<br><br></font></td></tr></table>';
               ...
               if (rslt['fght'] != undefined){
  	  			m+='<TABLE style="float:left;width:45%;" class=ptTab><tr><td colspan=3><b>Attacker : '+Nom1+' <a href="javascript:void(0)" onclick="KB.Controllers.MapHelper.gotoCoord('+ Coord1 +')" class="coordinateLink">('+ Coord1 +')</a></b>\
  	  			    <br>General : '+ rslt['s1KCombatLv'] +'<br>Number of turn(s) : '+rslt['rnds']+'<br>Bonus : '+ parseInt(rslt['s1atkBoost']*100)  +' % Attack - '+ parseInt(rslt['s1defBoost']*100) +' % Defense</td></tr>\
  	  			    <TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Troops</td><TD align="center">Fought</td><TD align="center">Survived</td></tr>'; 
  	  			if (rslt['fght']["s1"] != undefined) {
  	  					for (var i=1;i<=12;i++) {
  	  						if (rslt['fght']["s1"]['u'+i] != undefined) {
  	  			...
  	  			m+='Bonus : '+ parseInt(rslt['s0atkBoost']*100)  +' % Attack - '+ parseInt(rslt['s0defBoost']*100) +' % Defense</td></tr>';	  	
  	  		  	if (rslt['fght']["s0"] != undefined) {
  	  				  	m+='<TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Troops</td><TD align="center">Fought</td><TD align="center">A Survived</td></tr>';
  	  				 	for (var i=60;i<=63;i++) {
  							if (rslt['fght']["s0"]['f'+i] != undefined) {
  							if (rslt['fght']["s0"]['f'+i][0] > rslt['fght']["s0"]['f'+i][1]) m+='<TR><TD align="center"><div  class="pic px30 units unit_'+i+'"></td><TD align="center">'+rslt['fght']["s0"]['f'+i][0]+'</td><TD align="center"><FONT color="#CC0000">'+rslt['fght']["s0"]['f'+i][1]+'</font></td></tr>';
  							else m+='<TR><TD align="center"><div  class="pic px30 units unit_'+i+'"></td><TD align="center">'+rslt['fght']["s0"]['f'+i][0]+'</td><TD align="center">'+rslt['fght']["s0"]['f'+i][1]+'</td></tr>';
  	  	...
  	  	if (rslt['pop'] != undefined) {
  	  	  ...
  	  	  if ((rslt['unts']!=undefined) || (rslt['frt']!=undefined)) {
		    	  		  	    if (rslt['frt']!=undefined) {
		    	  		  	       m+='<TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Fortifications</td><TD align="center">Numbers</td></tr>';
		    				   	for (var i=60;i<=63;i++) {
  	  	   ...
  	  	   m+='</table>';
    	        ...
    	        if (rslt['tch'] != undefined) {
    	           ...
    	           m+='<table style="float:right;width:45%;" class=ptTab><TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Tech</td><TD align="center">Nivel</td></tr>';
   	           for (var i=1;i<=16;i++) {
    ...
    DisplayReports : function (){
      ...
      if(!t.data.length) {
         results.innerHTML = '<center><b>No hay informe.</b></center>';
         return;
      }
      var m = '<center><table width=100% cellspacing=0 cellpadding=3><thead><th>Pag</th><th>R.</th><th>Dato</th><th colspan=3>Agresor</th><th>Tipo</th><th colspan=4>Destino</th></thead>';
      m += '<tbody>';
    ...
    t.data.push(rpt); 
         ...
         if (parseInt(page)+1 <= t.maxPages) {
            var results=document.getElementById("hudResultsDiv");
            results.innerHTML = '<center><b>...Busqueda en curso : '+(parseInt(page)+1)+'...</b></center>';
    ...
    totalPages:0,
      ...
      if ( t.maxPages==99999)
         t.maxPages=t.totalPages;
      results.innerHTML = '<center><b>...Busqueda en curso '+t.maxPages+' pages...</b></center>';
      t.data=[];
        ...
        //row[i] = rp[1] - usage;
     	var timeLeft = parseInt(Cities.cities[i].c.resources[1].count)  / (0-usage) * 3600;
          ...
          if (timeLeft<0){
                 ...
                 msg += '**ATENCION** Mi Ciudad:  ' + Cities.cities[i].c.name.substring(0,10) + ' en las coords' + ' (' +
                       Cities.cities[i].c.x +','+ Cities.cities[i].c.y + ') ';
                msg += ' URGENTE!!! necesito comida. Me queda : '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+').   Production : '+addCommas(usage)+'/hr';
                sendChat ("/a " + msg);
                //alert(msg);
          }
      }  
...
my.Reassign = {
 ...
 init : function (){
  ...
  show : function (){  
    ...
    clearTimeout (t.displayTimer);
        if (t.state == null) {  
          m = "<DIV class=ptstat>"+unsafeWindow.arStrings.Common.Reassign+" tropas de una villa a otra</div>";
          m +="<div id='statpourREA'></div>";
          m += "<TABLE width='450px' class=ptTab border=0 align=left cellpadding=2>\
            <tr align=center valign=middle><td colspan=1 width=100><b><u>Salida</b></u><br><span id=REAsrcRptspeedcity></span></td>\
...
unitTotal+=parseInt(Cities.cities[i].c.troops[r].count());
  ...
  var t = my.Reassign;
	...
	unsafeWindow.Chrome.City.switchTo(SourceId);
     ...
     t.statutREA.innerHTML = "<i><b>En curso espere....</b></i>";
	setTimeout(function() {
...
document.getElementById("modal_attack_unit_ipt_"+r).value=parseInt(document.getElementById("REAnbunit"+r).value);
      ...
      t.statutREA.innerHTML ="<font color=red size='3px'><b>Hecho<b></font>";
      t.actionREA.disabled=false; 
  },600);
},100);
  },
...
my.TranspAuto = {
 ...
 init : function (){
  ...
  show : function (){  
   ...
   var ModelCity = {};
     if (t.state == null) {  
      m = "<DIV class=ptstat>Transporte de mercancias de una ciudad a otra</div>";
      ...
      m += "<TABLE width=100% class=ptTab border=0 cellpadding=3>\
       ...
       A estas Coordenadas <br>X: <input type=text size=4 id=typetrpx value=0>&nbsp;Y: <input type=text size=4 id=typetrpy value=0><br><br><INPUT id='ptttButTransport' type=submit value='Transportar' style='font-weight:bold'>\
       </td></tr>\
       <tr align=center><td colspan=1>Unidad : <select id=typetrp><option value='1'>"+unsafeWindow.arStrings.unitName["u1"]+"</option><option value='2'>"+unsafeWindow.arStrings.unitName["u2"]+"</option><option value='4'>"+unsafeWindow.arStrings.unitName["u4"]+"</option><option value='5'>"+unsafeWindow.arStrings.unitName["u5"]+"</option><option value='6'>"+unsafeWindow.arStrings.unitName["u6"]+"</option><option value='7'>"+unsafeWindow.arStrings.unitName["u7"]+"</option><option value='8'>"+unsafeWindow.arStrings.unitName["u8"]+"</option><option selected value='9'>"+unsafeWindow.arStrings.unitName["u9"]+"</option><option value='10'>"+unsafeWindow.arStrings.unitName["u10"]+"</option><option value='11'>"+unsafeWindow.arStrings.unitName["u11"]+"</option><option value='12'>"+unsafeWindow.arStrings.unitName["u12"]+"</option></select>\
       ...
       <br><b>Tipo de recurso :</b><br><input type=radio id='ChoixRess0' name='ChoixRess' value='gold'> " + unsafeWindow.arStrings.ResourceName[0] + "\
...
unitTotal+=parseInt(Cities.cities[i].c.troops[9].count());
  ...
  estimerRes: function() {
   ...
   if (document.getElementById("ChoixRess3").checked) { ic_ty="rec3";ic=3;ic_text="stone";resact = parseInt(Cities.cities[t.sourceCity.idx].c.resources[3].count); }
   ...
   if (resact < esti) {
     ...
     //t.estimationRes.innerHTML += "<br><font color=red><b>Insufficient resource available :</b></font> You must <a id='TRPclickunit' href='#'>" + uniteness + "</a> ";
  ...
  clickTransportDo: function() {   // function to transport
   var t = my.TranspAuto;
   var SourceId = t.sourceCity.c.id;
    var DestinationId = t.destinationCity.c.id;
	unsafeWindow.KB.AudioManager.playSound('on_click');
   ...
   if (x == 0 || y == 0) {
      t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Caution you have selected coordinates 0,0 to transport too !.</font>';
     return;
   }
   t.TTbutTransport.disabled=true;
   var c={};
  ...
  t.divTranportStatus.innerHTML = "<i><b>En curso la peticion....</b></i>";
  setTimeout(function() {
  unsafeWindow.Modal.hideModalAll();
...
document.getElementById("modal_attack_unit_ipt_"+t.typetrp.value).value=t.Choixnbwagon.value;
   ...
   t.divTranportStatus.innerHTML ="<font color=red size='3px'><b>Finalizado<b></font>";
   t.TTbutTransport.disabled=false; 
  },600);
 },100);
  },
    ...
    var t = my.Options;
      try {      
        m = '<DIV style="height:580px; max-height:580px; overflow-y:auto"><TABLE class=ptTab>\
          <TR><TD colspan=2><font size=4><B><u>Configuracion de Utilidades :</u></b></font><br></td></tr>\
          <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Permitir movimiento de ventana</td></tr>\
         <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>Organizar automaticamente el dia de impuestos (si felicidad >= <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%)</td></tr>\
         <TR><TD><INPUT id=pbRessEnable type=checkbox /></td><TD>Auto recoleccion de todos los recursos <INPUT id=pbLimitRess type=text size=2 maxlength=3 \> minutos</td></tr>\
        <TR><TD>&nbsp;</td><td>Mostrar la autonomia en rojo a partir de <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> horas</td></tr>\
        <TR><TD><INPUT id=ptAllowFB '+ (GlobalOptions.autoPublishGamePopups?'CHECKED ':'') +' type=checkbox /></td><TD>Enable publishing automatically to Facebook '+ htmlSelector({0:'----', 80:'Todos', 50:'Amigos de Amigos', 40:'Solo Amigos', 10:'Solo Yo'},GlobalOptions.autoPublishPrivacySetting,'id=selectprivacymode') +'</td></tr>\
		<TR><TD colspan=2><B>Configuracion del Chat :</b></td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Desactivar chat</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarnTchat type=checkbox /></td><TD>Habilitar post en el chat de alianza si autonomia esta en rojo</td></tr>\
        <TR><TD><INPUT id=HelpReq type=checkbox /></td><TD>Click automatico en las solicitudes de ayudas</td></tr>\
        <TR><TD><INPUT id=DelReq type=checkbox /></td><TD>Suprimir automaticamente las solicitudes de ayudas</td></tr>\
        <TR><TD colspan=2><BR><B>Configuracion de Torre de Vigilancia :</b></td></tr>\
	<TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Postea automaticamente los ataques entrantes en le chat de alianza.</td></tr>\
	<TR><TD><INPUT id=pcalertAudioEnable type=checkbox '+ (Options.alertConfig.aAudio?'CHECKED ':'') +'/></td><TD>Emit an audible alarm. (<a href="http://www.videolan.org/" target=_blank>Need plugin VLC</a>)</td></tr>\
  ...
  clickedSearch : function (){
    ...
    if (t.opt.maxDistance<=t.opt.maxDistanceA)
     ...
     if(t.opt.maxDistanceA > 375)
           errMsg += "The maximum distance can not exceed 375 ! the risk of search failing<BR>";
        if (errMsg != ''){
   ...
   if (t.opt.searchType == 0)
     ...
     m = '<CENTER><B>Search of '+ typeName +'<BR>\
         Center : '+ t.opt.startX +','+ t.opt.startY +'  Max Distance : '+ t.opt.maxDistanceA +' to '+ t.opt.maxDistance +'<BR></center>\
     ...
     if (t.opt.searchType == 1 || t.opt.searchType == 0) {
         ...
         <tr><TD class=xtab align=right>Min Glory :</td><TD class=xtab><select id=filPuissance>\
          ...
          <option value="2500" '+ (Options.filPuissance=='2500'?'SELECTED':'')  +'>2500</option>\
          ...
          <option value="2500" '+ (Options.filPuissanceMax=='2500'?'SELECTED':'')  +'>2500</option>\
    ...
    dispMapTable : function (){
      ...
      if (dat.length == 0){
        ...
        if (coordsOnly)
          ...
          if (t.opt.searchType == 0) {
  			m = '<TABLE id=srcOutTab cellpadding=0 cellspacing=0 style="padding:2px"><TR style="font-weight: bold"><TD style="padding:2px">Co-Ord</td><TD style="padding:2px;padding-left: 10px">Dist</td><TD style="padding-left: 10px;">Lev</td><TD style="padding-left: 10px;">Type</td></tr>';
  		}
  		if (t.opt.searchType == 1) {
  			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2 style="padding:2px"><TR style="font-weight: bold"><TD style="padding:2px">Co-Ords</td><TD style="padding:2px;padding-left: 10px">Dist</td><TD style="padding-left: 10px;">Lev</td><TD style="padding-left: 10px;">Type</td><TD style="padding-left: 10px;">Player</td><td style="padding-left: 10px;">Glory</td><td style="padding-left: 10px;">Alliance</td><td style="padding-left: 10px;" colspan=2>More Info</td></tr>';
  		}
  		if (t.opt.searchType == 2) {
  			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2 style="padding:2px"><TR style="font-weight: bold"><TD style="padding:2px" width=10%>Co-ords</td><TD style="padding:2px;padding-left: 10px">Dist</td><TD  style="padding-left: 10px;">City</td><TD style="padding-left: 10px;">Player</td><TD style="padding-left: 10px;">Glory</td><td style="padding-left: 10px;">Alliance </td><td style="padding-left: 10px;" colspan=2>More info	</td></tr>';
  		}
  	  }
  ...
  show : function (){
    ...
    clearTimeout (t.displayTimer);
      ...
      str = '<div style="height:600px;max-height:600px;overflow-y:auto"><DIV class=ptstat style="margin-top:2px; margin-bottom:2px; "><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD>' + unsafeWindow.player.name + ' <SPAN class=ptStatLight>1er Conex:</span> '+ dt.toLocaleDateString() +'</td>\
        ...
        <TD align=right><SPAN class=ptStatLight>Dominio :</span> ' + unsafeWindow.domainName +'</td></tr></table></div><span id="debugtest"></span>';
   ...
   var m="";
	...
	var entTotal=0;
 	for(i=0; i<Cities.numCities; i++) {
        ...
        var m="";
	 	var entTotal=0;
	 	var variiiable="Script Beta Auto" + " Bot modificado "+ "By"+"magoblanco";
	  	for(i=0; i<Cities.numCities; i++) {
	  	       color='black';
	  	       if ( parseInt(Cities.cities[i].c.resources[1].hourlyTotalRate()) < parseInt(Cities.cities[i].c.upkeep()) ) {
	  	        // maintaining higher production
...
unitWTotal+=parseInt(Cities.cities[i].c.troops[nbu].wounded());
	...
	str += "<tr><td style='background: #e8e8e8' align=right><b>Generals</td><td style='background: #e8e8e8' align=right>"+genTotal+""+m+"</tr>";
	str += "<tr><td><br></td></tr>";
	        var m="";
			var wildTotal=0;
		 	for(i=0; i<Cities.numCities; i++) {
    ...
    if (t.state == null){
     var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
              <TR><TD class=xtab align=right></td><TD class=xtab>Nick Jugador : &nbsp;</td>\
                <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Buscar" /></td>\
 ...
 eventSubmit : function (){
  ...
  eventGotAllianceList : function (rslt){
      ...
      for (k in rslt.alliancesMatched){
        ...
        if (all.ranking!=null) {
         m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
          ...
          <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Show</a></td></tr>';
        }
      }
    ...
    eventGetMembers : function (aid){
      var t = my.AllianceList;
      document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Searching for members...</center>';
  ...
  displayMembers : function (allName, numPlayers){
      ...
      var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
        ...
        <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
          <TD class=xtab width=80% align=center>Distance from <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' Players found </td></tr></table></div>';
       m += '<div style="top:190px;left:0px;width:100%; position:absolute;max-height:475px;height:470px;overflow:auto;">\
        ...
        <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
          ...
          <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Glory</a></div></td>\
          ...
          <TD class=clickable><A><DIV>Login</div></a></td></tr></thead>\
        ...
        <DIV style="top:670px; left:0px; position:absolute; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
          <TABLE width=100%><TR><TD class=xtab>Data from Leaderboard ! (24h between updates)</td>\
          </tr></table></div>';        
      document.getElementById('allListOut').innerHTML = m;
      document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
          <TD class=xtab>Show the distance from : &nbsp; X: <INPUT size=2 type=text id="plyrX" /> Y: <INPUT size=2 type=text id="plyrY" /> Or, choose a city : <span id=dmcoords></span></td></tr></table>';
  ...
  // sort and display
    reDisp : function (){
      ...
      for (var i=0; i<t.dat.length; i++){ //
          ...
          m += '<TR style="max-height:30px"><TD class=xxtab><a onclick=PTDme("' +  t.dat[i][0] +'");><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/avatars/25/'+t.dat[i][12]+''+t.dat[i][11]+'.jpg" align=absmiddle></a>&nbsp;<a onclick=getInfoForAnUser("'+ t.dat[i][10] +'");>'+ t.dat[i][0] +'</a></td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
  	                <TD class='+ tabclass +'><span title="Classement : '+t.dat[i][13]+' player">'+ officerId2String(t.dat[i][2]) +'</span></td><TD class=xxtab>'+ (t.dat[i][14]?'<SPAN class=boldDarkRed>ONLINE</span>':'') +'</td><TD class='+ tabclass +'>'+ t.dat[i][7] +'</td><TD align=right class='+ tabclass +'>'+ t.dat[i][4] +'</td>\
      ...
      m += '<tr><td colspan=11><textarea cols="55" rows="12" onclick="this.focus();this.select();" id="cutAndPaste" name="csv">Player;Glory;X;Y;Level;Distance;City\n'+csvXL+'</textarea><br><b>Export XLS (for you public:</b><bR>Copiez le contenu de la zone, coller ensuite le contenu dans blocnote et enregistrer le fichier en .cvs</tr>';
       var tbody = document.getElementById('allBody');
      tbody.style.maxHeight = '';
      tbody.innerHTML = m;
   ...
   eventPlayerSubmit : function (){
      ...
      t.pName = name;
      if (name.length < 3){
        document.getElementById('ptplayErr').innerHTML = 'Al menos 3 caracateres';
        return;
      }
      document.getElementById('altInput').innerHTML = '';
      document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Busqueda en curso...</center>';
  ...
  gotPlayerLeaderboard : function (rslt, span){
      ...
      for (var i=0; i<p.cities.length; i++){
        ...
        +' <a href="javascript:void(0)" onclick="KB.Controllers.MapHelper.gotoCoord('+ c.xCoord +',' +c.yCoord+ ')" class="coordinateLink">('+ c.xCoord +',' +c.yCoord+ ')</a></td><TD width=75%> &nbsp; Level : '
  
          + c.tileLevel +' &nbsp; &nbsp; status: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; cr&eacute;de : ' + c.dateCreated.substr(0,10) +'</td></tr>';
      }  */
      //span.innerHTML = m + '</table>';
      span.innerHTML = "<B>Tableau d'honneur :</b> Construccion en curso<br>Nombre de villes :"+p.numOfCities+"<br>Nombre de Terres : "+p.numOfWilds;
  },
  ...
  fetchPlayerStatus : function (uidArray, notify){
   ...
   gotPlayerDetail : function (rslt, span){
      ...
      var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Detalles:</b> &nbsp; </td><TD>Alianza : '+ a +' &nbsp; Ciudades : '
            + u.cities +' &nbsp; Population : '+ u.population +'</td></tr><TR><TD></td><TD>Provincias : ';
   ...
   eventGotPlayerOnlineList : function (rslt){
      ...
      var m = '<DIV class=ptstat>Resultado Correspondiente a --> <B>"'+ t.pName +'"</b></div>\
        ...
        <TD align=right>Gloria &nbsp;&nbsp;&nbsp;&nbsp;</td><TD>&nbsp;</td><TD width=60%>Informaciones suplementarias</td></tr>';
      var row=0;
      var cl='';
      for (k in t.playerList){
        ...
        m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'<br>'+alliancenammme+'<br><A target="_tab" href="http://www.facebook.com/profile.php?id='+ u.fbuid +'">Perfil Jugador</a></td><TD align=center>&nbsp;'+ addCommasInt(u.might) +'&nbsp;</td>\
            <TD>'+ (rslt.data[u.userId]?"&nbsp;<SPAN class=boldDarkRed>En linea</span>":"") +'</td>\
           <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>Detalles</a></span><br><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>Ultimo ingreso</a></span></td></tr>';
      }
      m += '</table></div>';
      document.getElementById('allListOut').innerHTML = m;
  },
