// Ad Blocker Modified For allblog simplification
// version 1.0 modified for allblog simplification
// 2005-06-22
// Copyright (c) 2005, Reify modified by gerecter at gerecter.com
//
// --------------------------------------------------------------------
//
...
//
// ==UserScript==
// @name          Ad Blocker Modified For allblog simplification
...
window.reify_adBlocker_removeAds = function()
{
	...
	if (parent.frames.length>1) {
		for(jjj=0;jjj<5;jjj++){
		var frameset_pd = parent;
		try
		{
		frameset_pd.document.getElementsByTagName("frameset")[0].setAttribute("cols","*,1");
		frameset_pd = top;
		break;
		} catch (e) { }
		}
...
function blockGeneric()
{
	for (var i = 0; i < AdElements.adTagNames.length; i++)
	{
		...
		for (var j = 0; j < els.length; j++)
		{
			...
			{
				try{
				if(tagName=="td" && el.width=="280") removeMiscAd(el);
				if(tagName=="table" && el.getElementsByTagName("td").length==2 && el.getElementsByTagName("tr").length==1 && el.getElementsByTagName("iframe").length==0) removeMiscAd(el);
				} catch(e) {}
...
var my_n = top.document.links.length;
var my_i = 0;
var puttingtitle="";
for(my_i=0;my_i<my_n;my_i++){
	var anc = top.document.links[my_i];
	if(anc.href.indexOf("/GoPage/")>1) {
		var url=""+anc.href;
		url = url.substring(anc.href.lastIndexOf("?http")+1);
		url = unescape(url);
		top.document.links[my_i].href = url;
		if(puttingtitle=="" && (""+anc.title).indexOf(" / ")>0)  {puttingtitle=anc.title;}
	}
}

if(puttingtitle.length>2) {
	puttingtitle = puttingtitle.substring( puttingtitle.lastIndexOf(" : ")+3 );
	top.document.title=puttingtitle;
}
