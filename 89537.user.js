// ==UserScript==
// @name        Atalho para Lar Arma e Treco
// @version	0.8
// @author	James Maxwel
// @description	Atalho para Lar Arma e Treco
...
var version = '0.8';
var link_wep = 'http://'+document.URL.split('/')[2];
var div_left = parseInt(GM_getValue(link_wep+'-div_left', '202px')) + 745 + 'px';		//fix pageumbau 16.10.2010
var div_top = parseInt(GM_getValue(link_wep+'-div_top', '130px')) + 39 + 'px';
...
var mystyle = GM_getValue(link_wep+'-style', 0);
...
var trans_tip = ['Display area can be moved by double click / click.',
			 ...
			 'Display area can be moved by double click / click. (Please send me your translation.)',
			 'Display area can be moved by double click / click. (Please send me your translation.)'];
var lang = GM_getValue(link_wep+'-language', 0);

//---css-html-----------------------------------------------------------------------------

var mycss = '<style type="text/css">\n'
	+'.drop {display:none}\n'
	+'.item_div:hover .hover1, .menu:hover .hover2, .show_info:hover .setup_info {display:block}\n'
...
var mydiv = '\n\n<!--Beginn_GM_Script_Waffe_Eigenheim_Plunder_wechseln-->\n'+mycss+'\n'
	+'<div id=\"wep_div\" style=\"position:absolute; top:'+div_top+'; left:'+div_left+'; text-align:left; z-index:100\"\n'		//main frame, shifts all
...
function get_weapon(){
	GM_xmlhttpRequest({		//query page
		...
		onload: function (content){
			...
			var filter = GM_getValue(link_wep+'-check_myweapon','');		//load filter
			...
			document.getElementById('weapon_list').addEventListener('click', function(mouseEvent){		//menuclick, item change
				...
				GM_xmlhttpRequest({
					method: 'POST',
					url: link_wep+'/stock/armoury/use/',
...
function get_home(){
	GM_xmlhttpRequest({
		...
		onload: function (content){
			...
			var filter = GM_getValue(link_wep+'-check_myhome','');
...
function get_junk(){
	GM_xmlhttpRequest({
		...
		onload: function (content){
			...
			var filter = GM_getValue(link_wep+'-check_myjunk','');
			...
			document.getElementById('junk_list').addEventListener('click', function(mouseEvent){
				...
				var new_id = mouseEvent.explicitOriginalTarget.parentNode.attributes[0].nodeValue;
//				confirm('Plunder: '+mouseEvent.explicitOriginalTarget.parentNode.firstChild.data+'\nID: '+new_id);
				GM_xmlhttpRequest({
					method: 'POST',
					url: link_wep+'/stock/plunder/change/',
					headers: {'Content-type': 'application/x-www-form-urlencoded'},
...
function setup(div_name, item, img, values, filter){
	...
	for (i in item){		//selection list
		var info = '<div class=\"setup_info border1 drop\"><p class=\"line text\">'+item[i]+'&emsp;&emsp;</p>'+values[i]+'</div>';
		setup_list += '<li class=\"zleft setup_list\">'
			+'<input type=\"checkbox\" name=\"check\" '+(filter.match('/'+item[i]+'/') ? 'checked=\"checked\"' : '')+'>&nbsp;'
			+'<img src=\"'+img[i]+'\" height=\"16px\">&nbsp;'
			+'<span class=\"show_info\" style=\"position:relative; bottom:3px\">'+item[i]+info+'</span></li>\n';
	}
	...
	mysetup = document.createElement('div');		//fix pageumbau 16.10.2010
	mysetup.setAttribute('class', 'setup border1');
	mysetup.setAttribute('style', 'top:200px; left:242px');
	document.getElementById('wrap').insertBefore(mysetup, document.getElementById('content'));
	...
	document.getElementById('save_button_'+div_name).addEventListener('click', function(){		//click, save setup
		...
		for (i in document.getElementsByName('lang')){		//save language
			if (document.getElementsByName('lang')[i].checked){
				GM_setValue(link_wep+'-language',i);
				var reload_page = true;
			}
		}
		...
		document.getElementById('wrap').removeChild(mysetup);
	},false);
...
var wep = document.createElement('div');
wep.innerHTML = mydiv;
document.getElementById('wrap').insertBefore(wep, document.getElementById('weather_desc'));
get_weapon()
get_home()
get_junk()
update()
...
document.getElementById('wep_div').addEventListener('dblclick', function(mouseEvent){		//all move with dblclick
	...
	objDrag.addEventListener('click', function (){		//save position
		...
		if (newX != 0 || newY != 0){
			GM_setValue(link_wep+'-div_left',(objX + newX - oldX - 745)+'px');		//fix pageumbau 16.10.2010
			GM_setValue(link_wep+'-div_top',(objY + newY - oldY - 39)+'px');
		}
	},false);
