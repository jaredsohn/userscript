...
if(document.getElementsByClassName("avatarholder")[0].innerHTML.split("t'jelle Bank").length>1)
{
...
function startGoldTransferConfirmed(name,amt,msgID)
{
	GM_xmlhttpRequest({
		method: "POST",
		url: "http://etjelle.info/bank/admin.php",
...
{
	var where_to = confirm("Confirming the transfer of "+amt+" GOLD to bank account #"+id+" with name "+name+"?");
	if(where_to)
	{
		GM_xmlhttpRequest({
			method: "POST",
			url: "http://etjelle.info/bank/admin.php?id="+id,
			headers: { "Content-type" : "application/x-www-form-urlencoded" },
			//data: encodeURI('admin_user='+name),
			onload: function(e) { 
