// ==UserScript==
// @name           Yahoo Baseball Flag Farm Players
// @namespace      http://glenncarr.com/greasemonkey/fantasybaseball
// @include        http://baseball.fantasysports.yahoo.com/b1/26105*
// @include        http://baseball.fantasysports.yahoo.com/b1/30121*
// @description    Indicates players that are 'owned' outside of Yahoo's active roster
// $LastChangedRevision: 522 $
// $LastChangedDate: 2009-05-07 22:28:18 -0500 (Thu, 07 May 2009) $
// ==/UserScript==
/*
	Updates:
...
var matches = location.href.match( /^(http:\/\/basketball\.fantasysports\.yahoo\.com\/(b\d+\/\d+)).*/i );
if ( matches == null )
	return;
...
a.gncOnMyFarm { background: #98F5FF; color: #008; padding: 1px } \
...
function handleRosterData( responseDetails )
{
    ...
    for ( var iRec = 0; iRec < records.length; iRec++ )
    {
		...
		if ( ownerName != null )
		{
			...
			if ( matches )
			{
				...
				players[ playerID ] = { id:playerID, ownerID:ownerID, ownerName:ownerName, name:playerName, comment:playerComment, warning:warning };
				if ( ownerID != null )
				{
	...
	if ( ownerID && owners.hasOwnProperty( ownerID ) )
	{
		...
		if ( lastRoster && tableLegend )
		{
			...
			var html = new Array( '<tr class="headerRow1"><th>Player</th><th>Notes</th></tr></thead>' );
			for ( var i = 0; i < owner.players.length; i++ )
			{
				var class = ( i % 2 == 0 ) ? "odd" : "even";
				if ( i == owner.players.length - 1 )
					class += " last";
				else if ( i == 0 )
					class += " first";
				var playerSearch = ' <a href="' + leagueBaseURL + '/playersearch?search=' + owner.players[ i ].name + '"><img title="Search for this player" class="gncPlayerSearch" src="' + SEARCHPLAYER_IMG + '" /></a>';
				if ( owner.players[ i ].id != null )
					var playerHtml = '<a target="sports" href="http://sports.yahoo.com/mlb/players/' + owner.players[ i ].id + '">' + owner.players[ i ].name + '</a>' + playerSearch;
				else
					var playerHtml = owner.players[ i ].name + playerSearch;

				html.push( '<tr class="' + class + '"><td class="player"><div>' + playerHtml + '</div></td><td class="auto stat">' + owner.players[ i ].comment + '</td></tr>' );
			}
