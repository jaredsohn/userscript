...
javascript:(function(){
	...
	var trimdlolkey;
	var cheeta = 0;
	var weratioing = false;
	var tulos;
	var NeededRatio = 0;
	var opponentLevelMax = 25000;
	var opponentMafiaMax = 501; 
...
'</style><div id="wrapper"><img id="prevs" src="http://www.creallaborate.com/guessx/probsm.png"></div>';
	...
	var version='<a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw">Drone.&#12324;</a> - <a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw/drone.html">FAQ</a> - <a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw/donate.html">Donate</a> - v2.85';
	...
	var config_html =
		...
		'<span id="fobz-row" style=""><table width="99%"><tbody><tr><td>Mafia Size&nbsp;<input type="integer" value="501" style="resize:none;width:25px;" id="postformid12">&nbsp;&nbsp;Mafia Level&nbsp;<input type="integer" id="postformid13" style="resize:none;width:40px;" value="25000">&nbsp;</td></tr></tbody></table></span>'+
//&nbsp;<font size="1">*Currently Disabled till i have working code</font>  disabled=""
		'<span id="fightrobz-row" style="display:none">'+		
		'<td>Rob untill stamina ratio is under &nbsp;<input type="integer" value="2.21" style="resize:none;width:39px;" id="postformid8">&nbsp;&nbsp;Then fight Target p|&nbsp;<input type="integer" value="41236137" style="resize:none;width:65px;" id="postformid9">&nbsp;&nbsp;in&nbsp;<a class="sexy_button_new short green black_white_border sexy_travel_new" id="FICN" href="#"><span><span>Brazil</span></span></a><BR>Heal threshold is&nbsp;<input type="integer" value="500" style="resize:none;width:35px;" id="postformid6"></td>'+
		'</span>'+		
...
/*	document.getElementById('EnAlways').onchange=function() {
	...
	document.getElementById("jobbrz").onclick = function () {
		    if (whichjob == true){
                ...
                whichjob = 6;
                document.getElementById("jobbrz").setAttribute("class", "sexy_button_new short white sexy_energy_new");
                jobbing_text = document.getElementById("jobbrz").innerHTML = "<span><span>Case Warehouses on North Side [2.135]</span></span>";
            } else if (whichjob == 6) {
                whichjob = false;
                document.getElementById("jobbrz").setAttribute("class", "sexy_button_new short orange sexy_energy_new");
	...
	document.getElementById("FobOrRob").onclick = function () {  
		    if (RobOrFoB == true){
				...
				RobOrFoB = 6;
				weratioing = true;
				document.getElementById("fightrobz-row").style.display = ''	
                ...
                jobbing_text = document.getElementById("FobOrRob").innerHTML = "<span><span>Fight & Rob</span></span>";
			} else if (RobOrFoB == 6) {
                RobOrFoB = false;
				RobOn = false;
				weratioing = false;				
				document.getElementById("fightrobz-row").style.display = 'none'
...
// Old Styling->document.getElementById("FobOrRob").setAttribute("class", "sexy_button_new short red sexy_energy_new");				
	...
	document.getElementById("whatfinish").onclick = function () {
		...
		function GoTime() {
			...
			ratiochan = parseFloat(document.getElementById("postformid8").value);			
			targetid = "p|" + parseInt(document.getElementById("postformid9").value);
			opponentMafiaMax = parseInt(document.getElementById("postformid12").value);
			opponentLevelMax = parseInt(document.getElementById("postformid13").value);
			}catch (err){}
//			alert(ratiochan);
//			return;
...
//				do_ajax('mainDiv','remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city=7&from=job&zone=1&destination=7&tab=103');
		...
		function LetThereBeRobberies() {
			...
			Ratio(); // Credit to Sakari Hiltunen for ratio changing
		...
		function DoFobFight() { 
			...
			if ((EnAlways == 'checked') && (DoesIHaveEn > howmuchy)) {
				...
				doireallynothaveenorstam = 0;
			Ratio();
			// Im really sorry i didnt get your name for this part...i'll update it when i do tho
targetElts = $x('.//a[contains(@onclick,"doFightJob(72")]');
				if (RobOrFoB == true) {
					targetElts = $x('.//a[contains(@onclick,"doFightJob(82")]');
				} 
				else if (RobOrFoB == 2) {
					targetElts = $x('.//a[contains(@onclick,"doFightJob(54")]');
				}
...
// untag() is generating errors i removed it and it gives me NaN for mw size/lvl
...
// this opponent is not valid, either because of tmp keys or Zynga.
//alert('step 1'); 
}
}
if(foundOpp){
...
//no valid opponents - refresh job page
//alert('step 2'); // It alerts this because it only
	do_ajax('mainDiv','remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city=7&from=job&zone=1&destination=5&tab=8');
	setTimeout(DoFobFight,5500);
	return
}
...
// this is my last method before upgrading to the above way with mw level and size feature.		
/*		function DoFobFight() {
			var DoesIHaveStamina = parseInt(document.getElementById('user_stamina').innerHTML);
			var DoesIHaveEn = parseInt(document.getElementById('user_energy').innerHTML);
			var gotskills = parseInt($('#user_skill').text());
		...
		function DoAJob() {
			...
			Ratio();
				...
				if (whichjob == true) {
						...
						evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
						clickMe.dispatchEvent(evt);	
					} else if (whichjob == 6){ // d2 Chicago lower ratio job
						var clickMe = document.getElementById("btn_dojob_12"); 
						var evt = document.createEvent("MouseEvents");
						if (clickMe == null){
						setTimeout(JobTravelr,3500);
						return;
...
//		}
		...
		if (health < healmeat) {
			...
			Ratio();
			if (weratioing == true && NeededRatio > ratiochan){ // Credit to Sakari Hiltunen for ratio changing
					RobOrFoB = 4;
					RobOn = true;
			}
			if ((DoesIHaveStamina < stamy) && (DoesIHaveEn < howmuchy)) {
