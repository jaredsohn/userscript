...
//
// @history        3.08 BR: Traduzido para portugues os textos mais importantes
...
// @history        3.07 Bugfix: Problem in the type of battlefield parsed in detailed view which led to 

"undefined" units.
// @history        3.06 Feature Removal: Removed garrison icon on island view (I think there was a bug here 

anyway).
// @history        3.05 Feature: Added unit names to event actions (PlayerX has joined the battle with...) 

(thanks hotcarbu)
// @history        3.04 Bugfix: Problem with determining land vs sea battle (thanks Martynius).
// @history        3.03 Bugfix: Gameforge changed how '&' were stored, which broke reports sent in-game with 
...
// @history        3.00 Feature: Show garrison button added for towns with deployed troops/fleet (thanks 

Wiisley).
// @history        2.8.5 Bugfix: Images not being shown properly in game messages.
// @history        2.8.4 Feature: Put back in functionality of formatted/colored CR reports within game 

messages (thanks to Martynius).
// @history        2.8.3 Feature Removal: Some idiots on the German server were abusing the nice coloring used 

in circular messages (now removed).
// @history        2.8.2 Feature: Initial support for v.0.3.4
// @history        2.8.1 Bugfix: Victors line was not removing excess white space, which was causing DOM 

error(s) in the ajaxchat option.
...
// @history        2.6.0 Feature: Script now has external message formatter for displaying html in in-game 
...
// @history        2.3.5 Bugfix: Sometimes the value of units lost is parsed incorrect due to an in-game 
...
// @history        2.3.1 Bugfix: "Instant" battles (those resulting in "missing" troops) excluded from 
...
ScriptUpdater.check(50784, "3.08 BR");
...
Config.tabs = {
	"Converter":{
		html:'<p>Combat Report Converter: Configurações.</p>',
		fields:{
			convertFormat:{
				type:"select",
				label:"Format",
				...
				text:'que formato você deseja?',
				value:"plaintext",
			},
			enableColor:{
				type:'checkbox',
				label:'Cor',
				text:'diferenciar por cores?',
				value:true,
			},
			enableEvents:{
				type:'checkbox',
			...
			crColorsScene:{
				type:"text",
				label:"Scene > Color",
				text:'qual cor para o cabeçalho?',
				value:"0000CC",
			},
			crColorsAttackers:{
				type:"text",
				label:"Attackers > Color",
				text:'qual cor para as unidades do atacante?',
				value:"C00000",
			},
			crColorsDefenders:{
				type:"text",
				label:"Defenders > Color",
				text:'qual cor para as unidades do defensor?',
				value:"00CC00",
			},
			crColorsLeft:{
				type:"text",
			...
			crColorsLost:{
				...
				text:'qual cor para as unidades que forme perdidas?',
				value:"FF0000",
			},
			crColorsVictor:{
				type:"text",
				label:"Victor > Color",
				text:'qual cor para o(s) vitorioso(s)?',
				value:"0000CC",
			},
			crColorsEvents:{
				type:"text",
				label:"Events > Color",
				text:'qual cor para o evento?',
				value:"444444",
			},
		}
	},
