...
// ==UserScript==
// @name Vendetta extra buttons
// @author berkley
// @description: Adds building move buttons, and recruit maximums + math buttons
// @include     http://s2.vendetta1923.com/*
// ==/UserScript==
...
(function(){
	...
	if (location.pathname.search('nav.php') != -1 ) {		
		...
		for (var i = 0; i < page.length; i++) {
			...
			if (page[i].selected) {
				...
				if (last!='') {
					newDiv = document.createElement("span");
					newDiv.innerHTML =  '<a href="javascript:void(0);" onclick="parent.w.location= chto+\'?q='+id+'x'+last+'\';location=\'nav.php?q='+id+'x'+last+'&w=\'+chto"><img src="http://www.berkley.axspace.com/arrowL.gif" border="0"></a>&nbsp;';
					var parentDiv = page.parentNode;
					parentDiv.insertBefore(newDiv, page);
					var last='';
				}
				...
				if (i < page.length) {
					newDiv = document.createElement("span");
					newDiv.innerHTML =  '&nbsp;<a href="javascript:void(0);" onclick="parent.w.location= chto+\'?q='+id+'x'+page[i+1].value+'\';location=\'nav.php?q='+id+'x'+page[i+1].value+'&w=\'+chto"><img src="http://www.berkley.axspace.com/arrowR.gif" border="0"></a>';
					var parentDiv = page.parentNode;
	...
	if (location.pathname.search('flotte.php') != -1 ) {		
		...
		if (text.indexOf('Dollarz',0) != -1 && text.indexOf('(Dollar)',0) != 8){
			var parentDiv = anchorTags[i].parentNode;
