...
function funcionPrincipal(e){
	...
	lang_lt['PERFIL'] 	= 'Naudotojo nustaymai';
	lang_lt['SIM'] 		= 'Simuliatorius';
	lang_lt['CALC'] 	= 'Skaičiuoklė';
	lang_lt['SEGURO'] 	= 'Ar esate įsitikinę?';
	lang_lt['MARK'] 	= 'Pažymeti viską';
	lang_lt['PERDIDAS'] 	= 'Praradimas';
	lang_lt['RENT'] 	= 'Nauda';
	lang_lt['SUBIR_NIVEL'] 	= 'Plėsti galima';
	lang_lt['JUGADOR'] 	= 'Žaidėjas';
	lang_lt['ALDEA'] 	= 'Kaimas';
	...
	lang_lt['GUARDADO'] 	= 'Saugumas';
	lang_lt['DESP_ABR'] 	= 'Žingsnis:';
	lang_lt['FALTA'] 	= 'Trūksta';
	...
	lang_lt['LISTO']	= 'Paruošta:';
	lang_lt['EL']		= 'Nuo';
	lang_lt['A_LAS']	= 'apie';
	lang_lt['EFICIENCIA']	= 'Efektyvumas';
	lang_lt['NUNCA']	= 'Niekada';
	lang_lt['PC']		= 'Kultūros taškai';
	lang_lt['FUNDAR']	= 'Galima ieškoti arba užkariauti kaimus';
	lang_lt['ALDEAS']	= 'Kaimai';
	...
	lang_lt['COMP']		= 'Nuoroda į kažką :D';
	lang_lt['STAT']		= 'Statistika';
	lang_lt['OFREZCO']	= 'Siūloma';
	lang_lt['BUSCO']	= 'Ieškoma';
	lang_lt['TIPO']		= 'Filtras';
	lang_lt['CUALQUIERA']	= 'Bet kas';
	lang_lt['DETALLES']	= 'Detalės';
	lang_lt['MAP_EXT']	= 'Žemėlapio legenda';
	lang_lt['DISPONIBLE']	= 'Rodyti tik galimus';
	lang_lt['SI']		= 'Taip';
	lang_lt['NO']		= 'Ne';
	...
	lang_lt['ENLACE']	= 'URL';
	lang_lt['TEXTO']	= 'Tekstas';
	lang_lt['ELIMINAR']	= 'Ištrinti';
	lang_lt['MAPA']		= 'Žemėlapis';
	lang_lt['VERSION']	= 'Naujos versijos paieška';
	lang_lt['MAXTIME']	= 'Maksimalus laikas';
	lang_lt['CHECK']	= 'Nauja versija';
	lang_lt['MAT_PRIMAS']	= 'Ištekliai?';
	lang_lt['ATAQUES']	= 'Ataka?';
	lang_lt['CONSTR']	= 'Konstr.';
	lang_lt['TROPAS']	= 'Kariai?';
	lang_lt['ACTUALIZAR']	= 'Aktualijos?';
	lang_lt['ARCHIVE']	= 'Archyvas';
	lang_lt['RESUMEN']	= 'Bendrai';
	lang_lt['IResurs']  = 'Ieškomas resursas';
	...
	function img(ref, lang_dependant){ return "img/un/" + ref; }
	...
	function T(texto){
		...
		try{
			...
			eval('var language = lang_lt');
		}
		// Si una cadena concreta no esta traducida en el idioma, utiliza por defecto el castellano
		if (language[texto] == undefined) return lang_lt[texto]; else return language[texto];
	}
	...
	function F(texto, args){
		...
		catch(e){ eval('var language = lang_lt'); }
		if (language[texto] == undefined) texto = lang_lt[texto]; else texto = language[texto];
...
//				["Travian Beyond", "http://www.denibol.com/proyectos/travian_beyond/", "_blank"]
	...
	function reportBatallaSim(){ 
		var t = find("//table[@class='tbg']", XPList); 
		if (t.snapshotLength < 2) return;
		// Encuentra y suma todas las cantidades del botin
		var a = find("//tr[@class='cbg1']", XPList);
		
		var perds = new Array();
		// By each participant in the battle (attacking, defending and possible supports)
		for(var g = 0; g < 2; g++){ 
			var tt = t.snapshotItem(g); 
			var num_elementos = tt.rows[1].cells.length - 1;
			for(var j = 1; j < 11; j++){ 
				// It recovers the amounts of troops of each type that have taken part
				var u = uc[tt.rows[0].cells[j].getElementsByTagName('img')[0].src.replace(/.*\/.*\//,'').replace(/\..*/,'')]; 
				var p = tt.rows[2] ? tt.rows[2].cells[j].innerHTML : 0; 
				// Being based on the cost by unit and its capacity, they calculate lost and the total lifting capacity
				var ptu = arrayByN(u, p);
				perds[g] = arrayAdd(perds[g], ptu.slice(0, 4)); 
			}
			// Anyade the new information like an additional row in each table
			var informe = document.createElement("TD");
			for (var i = 0; i < 4; i++){
				informe.innerHTML += '<img src="' + img('r/' + (i + 1) + '.gif') + '" width="18" height="12" border="0" title="' + T('RECURSO' + (i+1)) + '">';
				informe.innerHTML += perds[g][i];
	...
	function preCalculate1(){
		...
		table.style.top = 630 + longitudPantalla() + 'px';
...
//				buildingCode = parseInt(buildingCode[0].substring(1, buildingCode[0].length));
	...
	function preCalculate3(){
		...
		var etiquetas_tabla = ["JUGADOR", "ALIANZA", "ALDEA", "HAB", /*"IResurs",*/ "COORD", "ACCION"];
		for (var i = 0; i < 6; i++){
			var td = elem('TD', T(etiquetas_tabla[i]));
			if (i < 4){
				switch(i){
					case 4: td.addEventListener("click", sortTable('tabla_mapa', i, 'int'), 0); break;
...
//	if (location.href.match(/karte2.php($|\?z=)/)){		desplazarMapa(); return; }
	...
	if (location.href.indexOf('warsim.php') != -1)	reportBatallaSim();
