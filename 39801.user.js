...
function fieldGraphicsUpdate() {
	if (enableFieldLayer == true) { //1080x480
        ...
        field.style.backgroundImage = "url(http://img152.imageshack.us/img152/4800/wildcatsua6.jpg)";
	}
...
function restructurePage() {
    ...
    if (alwaysShowPlayText == false) {
    	dsc.setAttribute("style","text-align:center; visibility: hidden");
    }
    else {
    	dsc.style.textAlign = "center";
...
function changeInfo(page) {
	...
	var div = newinfo[1].parentNode.removeChild(newinfo[1]);
    if (alwaysShowPlayText == false) {
	...
	info[0].parentNode.insertBefore(div,info[0].nextSibling);
...
//	console.log("isPlayFinished : "+unsafeWindow.currentFrame+" --- mcf="+currentFrame);
	if (playPaused == true) {
		return;
	}
	if (playDone == true) {
...
function render() {
	...
	if (currentFrame > play_data.length-1) {
		...
		playDone = true;
		playFinishedTimer = setTimeout(isPlayFinished,postDelay);
	}
	else {
		frameTimer = setTimeout(render,0);
	}
