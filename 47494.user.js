...
if(site==brutesite)	{
	...
	if (n[0]){
		...
		//Española elbruto.es  'Ha ocurrido un error... ¡brutal!'    'Mantenimiento en curso...'
		//Française labrute.fr                                     'Maintenance technique en cours...'
		if(n[0].innerHTML=='Ha ocurrido un error... ¡brutal!' || n[0].innerHTML=='Mantenimiento en curso...'){
			pagesuccess=false;
			if(n[0].innerHTML=='Ha ocurrido un error... ¡brutal!'){
				brutalerror=true;
			}
		}
	}
...
if(page=='cellule' && self==top && pagesuccess){
	...
	if(managed || arched || command=='?add' || command=='?ead'){
		...
		if(n) { var thisname=n[0].innerHTML;
			...
			//Española elbruto.es   'Ha ocurrido un error... ¡brutal!'   'Mantenimiento en curso...'
			//Française labrute.fr                                     Maintenance technique en cours...
			if(thisname=='Ha ocurrido un error... ¡brutal!' || thisname=='Mantenimiento en curso...')
				thisname=' ';
...
if(config_quick_nav==1 && readytofight && cell_fights==0 && brutenum<numbrutes-1){
	...
	//Española elbruto.es El Bruto Manager - Lista de Brutos y Conf
	...
	//Española elbruto.es "Quitar '+thisbrute+' de Mis Brutos"
	...
	//Española elbruto.es "Añadir '+thisbrute+' a Mis Brutos"
	//Française labrute.fr "Ajoutez '+thisbrute+' à votre liste brute"
	var menuadd='<div id="mbmadd" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/log_win.gif);background-position:-5px -5px;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?add\',\'_self\')" title="Añadir '+thisbrute+' a Mis Brutos"></div>';
	//English mybrute.com  "Attack '+thisbrute+' with '+GM_getValue('active_current_brute','')+'"
	//Española elbruto.es "Atacar a '+thisbrute+' con '+GM_getValue('active_current_brute','')+'"
	...
	//Española elbruto.es "Añadir '+thisbrute+' a Mis Enemigos"
	//Française labrute.fr "Ajoutez '+thisbrute+' à votre liste d\'ennemis"
	var menuenemyadd='<div id="mbmnmeadd" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/lvl_2.gif);background-position:-1px -1px;background-repeat:no-repeat;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?ead\',\'_self\')" title="Añadir '+thisbrute+' a Mis Enemigos"></div>';
	//English mybrute.com  "Remove '+thisbrute+' from your list of enemies"
	//Española elbruto.es "Quitar '+thisbrute+' de mi Mis Enemigos"
	...
	if(managed && config_show_quick_fight==1 && cell_fights>0){
		...
		if(quick_opp!='' && quick_opp.length>1){
			if(readytofight){
				...
				//Española elbruto.es 'Tomando '+quick_opp+'('+quick_level+') ...'
			...
			if(qfcount>0){
				...
				battlebutton=''+qfcount+' enemies, encontrados, buscando '+(cell_fights-qfcount)+' más oponentes al azar '+quick_level+'';
			}
			else{
	...
	if(config_arch_enemies==1 && numenemies>0){
		...
		//Española elbruto.es Buscando a
		...
		//Española elbruto.es Mis Enemigos
	...
	controller=controller+'</map>';
		...
		//Española elbruto.es Volver a la celda de '+GM_getValue('active_current_brute','')+'
			...
			if(config_arena_bonus_preview==1){
			   var hoveraction="";
				...
				if(config_arch_enemies==1){
					...
					//Española elbruto.es "Añadir '+opp+' a Mis Enemigos"
					//Française labrute.fr "Ajoutez '+opp+' à votre liste d\'ennemis"
					enemybutton='<div id="mbmnmearena'+(i+1)+'" style="width:25px;height:15px;background-color:#FF0000;text-align:center;vertical-align:middle;float:right;overflow:hidden;font-size:xx-small;color:#000000;" onclick="window.open(\'http://'+opp+'.'+brutesite+'/cellule/?ead\',\''+config_arena_enemy_target+'\'); return false;" title="Añadir '+opp+' a Mis Enemigos">ARCH</div>';
               	}
	...
	if(numbrutes>0){
		//English mybrute.com Your Brutes
		//Española elbruto.es Mis Brutos
		//Française labrute.fr Vos Brutes
		dropdown='<form id="bruteform" name="bruteform" method="post" action="">	<select name="brutechooser" style="width: 125px;" onchange="javascript:var brute=0; brute=document.bruteform.brutechooser.options[document.bruteform.brutechooser.selectedIndex].value; if(brute!=0){ location.href=\'http://\'+brute+\'.'+brutesite+'/'+page+'/\'; }">    <option value="0">Mis Brutos</option>';
		var alias=' ';
		for(i=0;i<numbrutes;i++){
			...
			if(config_arch_enemies==1){
				for(j=1;j<=6;j++){
					if(enemies[i]==arenaopps[j]){
						...
						//Española elbruto.es "Quitar "+opp+" de mi Mis Enemigos"
		...
		if(!pagesuccess){
			...
			if(active_quick_fight_retry>0){
				...
				if(ql[0]){
					...
					//Española elbruto.es  'No se pudo encontrar a nadie<br />probando de nuevo...'
	...
	if(changes){
		...
		message='¡El Bruto Manager se ha actualizado satisfactoriamente! :)';
	...
	if(pagesuccess){
		...
		//Española elbruto.es  Bonus de '+thisname+' Victorias
