// ==UserScript==
// @name        MyFreeFarm Spanish
// @namespace   http://userscripts.org/scripts/show/186267
...
try{
	...
	texte["msgMarketsaleContent"]="(.*) te ha comprado\\s*(\\d+) (.*?)\\s*<br>\\s*por (.*?) MG en el mercado\\."; // The text where the information is stated. The information has to be replaced by "(.*?)".
	// For the following you have to take a message sent if you sell something via contract
	texte["msgContractsaleContent"]=invert_exclamation+"(.*) ha firmado un contrato tuyo!<br><br>\\s*Los siguientes productos se han vendido:<br>([\\S\\s]*)\\s*<br>\\s*El importe del contrato de (.*?) MG se ha sumado a tu cuenta\\."; // The text where the general information is stated. The information has to be replaced by "(.*?)".
