// ==UserScript==
// @name           Google Reader Sanity
// @namespace      http://userscripts.org/scripts/show/117298
// @author         cqrt
// @description    Restore your Google Reader interface to its former glory and restore your sanity. -=Redesigned minimalist interface=- -=Inline Post Previews=- -=Reduced memory footprint=- -=Keyboard Shortcuts=- -=Collapsible Interface=- -=Auto Update=- -=True unread counts=- -=Enclosures restored=-
...
// @version        2.4
// @history        (2.4) Cross browser update notification, Row highlight on hover, Keyboard shortcut help, Design refreshes, Hide parts of side navigation, Adjustable side navigation width, plus more...
...
.folder .folder .name-text { max-width: 63%; } \
.folder .name-text, #reading-list-selector .label { max-width: 63%; } \
...
.lhn-hidden #entries .entry-body { max-width: 90% !important; } \
...
#lhn-add-subscription { width: 60px; margin-left :auto; margin-right: auto; } \
...
#lhn-subscriptions { margin-top: 5px; } \
#recommendations-tree .lhn-section-primary { height: 25px; } \
...
#logo-section + #lhn-add-subscription-section #lhn-add-subscription { top: 15px !important; left: 68%; z-index: 2; } \
...
#sections-header .contents { top: 65%; } \
...
#viewer-top-controls { padding: 1px 3px 1px 45px; } \
#lhn-add-subscription { font-size: .8em; } \
#viewer-translated-by { margin-left: 1em; margin-top: .1em; line-height: 10px; font-size: .8em; width: 120px; } \
#viewer-translate-revert-link { position: relative; left: 18px; } \
...
GM_config.init("Google Reader Sanity Options", {
    ...
    hideGoogHead: {
        label: "Google Search Header",
        type: "checkbox",
        defval: true
    },
    hideNavHomeSection: {
        label: "Navigation 'Home' Section",
        type: "checkbox",
        defval: false
    },
    hideNavAllItemsSection: {
        label: "Navigation 'All Items' Section",
        type: "checkbox",
        defval: false
    },
    hideNavExploreSection: {
        label: "Navigation 'Explore' Section",
        type: "checkbox",
        defval: false
    },
    ...
    enableHighlightRows: {
        label: "Enable List View Hover Highlights",
        type: "checkbox",
        defval: true
    },
    enableDimRead: {
        label: "Enable Dim Read Subscriptions",
        type: "checkbox",
        defval: true
    },
    enableEnclosures: {
    ...
    enablePostPreviews: {
        label: "Enable Post Previews (perfect for YouTube RSS Feeds)",
        type: "checkbox",
        defval: true
    },
    enablePurgePost: {
    ...
    keyboardShortcuts: {
        ...
        label: "",
        type: "textarea",
        defval: "F  -  Hide All Navigation\nU  -  Hide Side Navigation\nB  -  Open post in Background Tab\nShift V  -  Toggle Preview Post\nShift Z  -  Open this Config Box"
    }
}, " \
...
textarea { position: absolute; background-color: #708AC7; color: #000000; font-weight: bold; font-size: 8pt; border: #708AC7; width: 200px; height: 95px; line-height: 150%; resize: none; } \
textarea:focus {outline: none;} \
input[type='text'] { width: 50px !important; } \
...
function applyFeatures() {
    var sideNavWidth = GM_config.get("resizeSideNav") || "270";
    if (sideNavWidth <= 210) {
        GM_addStyle(".folder .folder .name-text { max-width: 55% !important; } ");
    } else if (sideNavWidth >= 250) {
        GM_addStyle(".folder .folder .name-text { max-width: 68% !important; } ");
    }
    ...
    if (GM_config.get("hideGoogNav")) {
        ...
        GM_addStyle(".gbemi#gb #gbx3, .gbem#gbx3 { height: 27px; } \
.gbts { color: #D4D4D4; } \
.gbts:hover { color: #FFF; } \
#gbx3 { border-color: #2C4572; background: #1E2F4D url(data:image/png;base64,) repeat-x !important; }");
                    }
...
#gbqfqw { -webkit-border-radius: 3px; border-radius: 3px; height: 26px; }");
    ...
    if (GM_config.get("hideNavHomeSection")) {
        GM_addStyle("#home-section { display: none !important; }");
    }
    if (GM_config.get("hideNavAllItemsSection")) {
        GM_addStyle("#lhn-selectors { display: none !important; }");
    }
    if (GM_config.get("hideNavExploreSection")) {
    ...
    if (GM_config.get("enableHighlightRows")) {
        GM_addStyle(cssHighlightRows);
    }
...
var cssDefault = "#GM_config { width: 430px !important; height: 530px !important; -webkit-border-radius: 35px; -moz-border-radius: 35px; border-radius: 35px; -moz-box-shadow: 4px 4px 14px #000; -webkit-box-shadow: 4px 4px 14px #000; box-shadow: 4px 4px 14px #000; }";
...
.samedir #entries.list .collapsed .entry-secondary { margin: 0 8em 0 14em; } \
...
var cssHighlightRows = "#current-entry .collapsed { background: none !important; } \
#current-entry .collapsed * { color: #000 !important; text-shadow: 0 1px 0 #fff !important; text-decoration: underline !important; } \
#current-entry.read .collapsed { background: none repeat scroll 0 0 #eee !important; } \
.collapsed:hover, #current-entry .collapsed:hover { background-color: #ffffcc !important; } \
.read .collapsed:hover, #current-entry.read .collapsed:hover { background-color: #dbe8ff !important; }";
var cssSideNavSize = "#nav, #logo-container, #lhn-add-subscription-section, #scrollable-sections-top-shadow, \
#scrollable-sections-bottom-shadow { max-width: side-nav-sizepx; width: side-nav-sizepx; min-width: 180px; }";
applyFeatures();

// List View Favicons for Firefox browser only
