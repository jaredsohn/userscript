...
- 2007-11-08: version 1.2 - Added new regexp, showing now 'Abandoned walley' instead of 'Site: ##' + some bugs repaired
*/
		...
		if (req.status == 200) {
			...
			do {
				if (counter <= (myAreas.length - 1)){
					...
					found = true;
					myInfo('<br><b>انتهى:</b>' + getDateString() + '<br>');
				}
				counter++;
			} while (found == false)
		} else {
...
if (myMap) {
	...
	addInfoDiv();
	myInfo('<b>يبحث:</b>' + getDateString() + '<br>');
