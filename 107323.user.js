...
rZu9PUbBYmEvEItmJdAT/j/fiC//yD8gBzZDxgE6wfkyH4gCQTjB34FgvGDgAI79QMfAckTd+0HPGtuIt86zz8oP5CuJ0V4E/nP3I79QK1WixvHNy6RGRP0Bor97RF/+sEvjIol6KoTQG0AAAAASUVORK5CYII=',
...
set_var = ['ipt_css','sel_language','chk_commtoggle','chk_autohide','chk_update','chk_messrep','chk_mute','chk_remove'],
set_default = {"ipt_css":"","sel_language":"en","chk_update":true,"chk_commtoggle":true,"chk_autohide":true,"chk_messrep":true,"chk_mute":true,"chk_remove":true},
gpr_setting = $.parseJSON(getValue('gpr_setting',$.toJSON(set_default))),
gpr_json =  {"version":version,"language_select":{"en":"English"} },
language = gpr_setting['sel_language'],
TIME = {"toggle_comm":true,"toggle_mess":true}, g = {},
messages = [];
for(i in set_var){
	var x = set_var[i];
...
gpr_addScript('http://mesak-project.googlecode.com/files/json2.js');
gpr_addScript('https://sites.google.com/a/wablog.info/mesak-project/documents/gpr_setting.js?'+ version );
...
'.hCJnsb .gpr_tools { width:28px;margin-left:-32px;margin-top:2px;float:left;clear:left;}'
);
if( typeof(gpr_setting['ipt_css']) != 'undefined'){
	if( gpr_setting.ipt_css != '') gpr_addStyleUrl(gpr_setting.ipt_css);
}
if(GPU.indexOf('plus.google.com/settings/') != -1 || GPU.indexOf('plus.google.com/u/0/settings/') != -1){
	...
	$('#nav-gprsettingpanel').click(function(){
		...
		var sellan = '';
		for(x in  gpr_json.language_select){
			sellan +='<option value="'+x+'">'+gpr_json.language_select[x]+'</option>';
		}
		var setting_form ='<div class="a-E-F-U d-s-r">'+T('SETTING_GNERAL')+'</div>' + 
			gpr_form_row( T('SET_CHK_UPDATE'), '<input type="checkbox" id="GPR_CHK_UPDATE" />')+
			gpr_form_row( T('SET_CHK_COMMTOGGLE'), '<input type="checkbox" id="GPR_CHK_COMMTOGGLE" />')+
			gpr_form_row( T('SET_CHK_AUTOHIDE'), '<input type="checkbox" id="GPR_CHK_AUTOHIDE" />')+
			gpr_form_row( T('SET_CHK_MESSREP'), '<input type="checkbox" id="GPR_CHK_MESSREP" />')+
			gpr_form_row( T('SET_CHK_MUTE'), '<input type="checkbox" id="GPR_CHK_MUTE" />')+
			gpr_form_row( T('SET_CHK_REMOVE'), '<input type="checkbox" id="GPR_CHK_REMOVE" />')+
			gpr_form_row( T('SET_IPT_CSS'), '<input type="text" size="30" id="GPR_IPT_CSS" />')+
			gpr_form_row( T('SET_SEL_LANGUAGE'), '<select id="GPR_SEL_LANGUAGE">'+sellan+'</select>')+
...
function gpr_form_row( title, content){
	return '<div><table class="a-E-Cf-F-qb"><tbody><tr><td class="a-E-Cf-Gv-U">'+title+'</td><td class="a-E-Cf-Gv-lf">'+content+'</td></tr></tbody></table></div>';
}
...
function gpr_chkupdate(){
	if(gpr_setting.chk_update && parseFloat(version)*10 < parseFloat(gpr_json.version)*10 ){
...
function gpr_reload_language(){
	$('span.gpr[role="button"]').html( T('REPLY') ).attr('title', T('REPLY') );
}
//******************** replay ********************//
...
if( gpr_setting.chk_commtoggle ){
	function gpr_toggle_comm_create(){
		//$('body').append( $('div.a-b-f-i-Xb:not([tg])').size() +',')
		TIME['toggle_comm'] = false;
		$('div.a-b-f-i-Xb:not([tg])').attr('tg','false');
		$('div.a-b-f-i-Xb[tg!="true"]').each(function(i,n){
			var $this = $(n),
				button_block = $this.parent();
			if( button_block.find('.gpr_toggle_comm').length == 0 ){
				button_block.append('<div class="gpr_bar"><div class="gpr_toggle_comm" style="background:url('+IMG['TP_UP']+') no-repeat left center;"></div></div>')
				gpr_toggle_comm_reload( $this );
				$this.attr('tg','true');
				$this.children('div.a-b-f-i-Xb-oa').bind('DOMNodeInserted',function(){
					var p = $(this).parent();
					gpr_toggle_comm_reload( p );
				})
		...
		setTimeout(function() {	TIME['toggle_comm'] = true; },800);
	}
		...
		var comment_show = $this.find('.a-f-i-W-r').size(),
			...
			if(comments > 0){
				...
				if(comments != comments_data){
					gpr_toggle_comm.css({'background-color': '#FFCC00'}).animate({backgroundColor: "#FFF" }, 500 );
...
if( gpr_setting.chk_messrep ){
	function gpr_toggle_mess_create(){
		TIME['toggle_mess'] = false;
		$('div.ea-S-C:not([rp])').each(function(i,n){
			var $mes = $(n),
				index = $.inArray( $mes.attr('data-content-url') , messages );
				//$('#debug').append('index = <b>'+index + '</b> messages = <b>'+ messages.length +'</b><br>')
			if( index == -1){
				...
				if( $d.find('.gpr_btn_toggle_mess').size() == 0){
					//$m.hide();
					//$m.find('.a-b-f-i-u-ki').size()
					$m.find('.a-b-f-S-oa').hide();
					$d.append('<div class="gpr_btn_toggle_mess gpr_btn_tog1" title="'+T('ALT_BTN_MESS_REP')+'">&nbsp;</div>');
				}
				$mes.attr('rp', index );
			}
		})
		setTimeout(function() {	TIME['toggle_mess'] = true; },800);
	}
}
//******************** tools ********************//
function gpr_tools_create(){
	$('div.a-f-i-p-U:not([tb])').each(function(i,n){
		var $tb = $(n), btn = '';
		if( $tb.find('.gpr_tools').size() == 0){
			var user_oid = $tb.parent().find('a:first');
			if(gpr_setting.chk_mute && gpr_setting.oid != user_oid.attr('oid') ){
				btn +='<div class="gpr_btn_mute" title="'+  $tb.parents('a-b-f-i:first').find('.a-b-f-i-Fb-C').text() +'">&nbsp;</div>';
			}
			...
			$tb.attr('tb','true').append('<div class="gpr_tools">'+btn+'</div>');
		}
	})
}
$('div.gpr_tools > div').live('click',function(){
	...
	if( $j.hasClass('gpr_btn_mute') ) {
		var mu = $j.parents('div.a-b-f-i-p:first').next().find('.a-b-f-i-Fb-C')[0];
...
function gpr_init(){
	gpr_tools_create();
	if(gpr_setting.chk_commtoggle && TIME['toggle_comm']){
			gpr_toggle_comm_create();
	}
	if(gpr_setting.chk_messrep  && TIME['toggle_mess'] ){
			gpr_toggle_mess_create();
	}
	if( g.c.children('div').size() > 1){
		messages = [];
	}
}
//******************** init object ********************//
if(typeof(gpr_setting['oid']) == "undefined" &&  $('a.a-Rf-dB').size() != 0 ){
	gpr_setting['oid'] = $('a.a-Rf-dB').attr('href').replace('/u/0/','').replace('/','');
	setValue('gpr_setting', $.toJSON(gpr_setting) );
}
...
//alert( $('.a-U-bg-T').size() )
if( $('#debug').size() == 0 ){
	$('body').prepend('<div id="debug" class="gpr_debug" style="display:none"><br><br></div>');
}
g.d = $('#debug');
function log(content){
	g.d.append(content + '<br />');
	g.d.show();
}
//******************** load setting ********************//
TIME['load_timeout'] = 0;
TIME['load'] = window.setInterval(function(){
	if( $('#data_gpr_json').size() > 0 || TIME['load_timeout'] > 10){
		clearInterval( TIME['load'] );
		var tmp_gpr_json = $.parseJSON( $('#data_gpr_json').html() );$('#data_gpr_json').remove();
		if(typeof(tmp_gpr_json) =='object'){
			gpr_json = tmp_gpr_json;
		}
		var tmp_gpr_lang = $.parseJSON( $('#data_gpr_lang').html() );$('#data_gpr_lang').remove();
		if(typeof(tmp_gpr_lang) =='object'){
			for(x in tmp_gpr_lang){	gpr_lang[x] = tmp_gpr_lang[x];}
		}
		gpr_chkupdate();
		gpr_reload_language();
		gpr_tools_create();
		if(g.c[0] != null){	g.c.bind('DOMNodeInserted',gpr_init);}
	}else{
		TIME['load_timeout']++;
	}
}, 800);
