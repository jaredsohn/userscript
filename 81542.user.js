// ==UserScript==
// @name           	Bulk Offer from Marketplace
// @namespace      	garrettFoster
// @description    	Allows you to make the same offer to multiple players that meet your marketplace criteria.
...
function main(){
	...
	'<div style = "float: left; width: 200px;">'+
		'<div class="field_container">'+             
			...
			'<br />'+
			'<select id="offerType" class="field" style="width: 128px;">'+
		...
		'<div class="field_container">'+
			...
			'<select id="no_trade" class="field" style="width: 128px;">'+
	...
	'<div style = "float: left; width: 200px;">'+
		'<div class="field_container">'+
			...
			'<select id="contract_type" class="field" style="width: 128px;">'+
		...
		'<div class="field_container">'+
			...
			'<select id="duration" class="field" style="width: 128px;">'+
	...
	'<div style = "float: left; width: 200px;">'+	
		...
		'</div>'+
		'<div class="field_container">'+
			'<div class="fieldheading">Team Id</div>'+
			'<input type="text" value="" id="teamId">'+
		'</div>'+ 
	'</div>'+
	'<div style="clear:both">'+
		'<input id="bulkOffer-go" type="button" value="Go" />'+
	'</div>';
	
	//place options on page
	id('bulkOffer-options').innerHTML = optionHTML;

	//add event listeners to options
	id('note').addEventListener('keyup',updateNoteLength,false);
	id('bulkOffer-go').addEventListener('click',sendOffers,false);
}       
...
function sendOffers(){
    ...
    //hide options
    toggleOption('bulkOffer-options');    

    //get user input
    var teamId = id('teamId').value;
	while(teamId == ''){
		teamId = prompt('Please enter a valid team id number:','');
	}
	...
	var playerRow = document.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
	for(var i=1;i<playerRow.length;i++){
		if(playerRow[i].style.visibility != 'hidden'){	
			playerId.push(playerRow[i].childNodes[3].firstChild.href.split('=')[1]);
		}
	}
	...
	id('greaseStatus').innerHTML = '<span style="color: green; padding: 2px;">0 offer(s) left to prepare</span>';
    //console.log(desired);   
    ...
    if(confirm(playerId.length + ' offers totaling $' + addCommas(totalSalary*40) + ' will be sent. Would you like to continue?')){ 
	    //send offers
	    for(var i=0;i<playerId.length;i++){
