// ==UserScript==
// @name           YouTube Auto Buffer & Auto HD Pro + Last Update
...
// @include        https://youtube.com/*
// @exclude        http://*.youtube.com/results?*
// @exclude        http://youtube.com/results?*
...
var navID = "watch7-user-header"; // change the ID if you'd like. the option button will be added to this element
...
function msg(t, h, c) {
	...
	var box = document.createElement("div"),
		...
		exist = document.getElementById(box_id_name);
	...
	if(exist === null) {
		...
		box.setAttribute("style", "position: fixed; z-index: 99999; top: 25%; left: 25%; width: 50%; height: 25%; padding: 50px; background-color: #E9E9E9; border: 3px double #006195;");
		...
		header.appendChild( document.createTextNode((typeof h !== "undefined" && h !== "" ? h : "Message Box by JoeSimmons. Double-click to close.")) );
		text.setAttribute("style", (c === true ? "width: 100%; height: 100%; text-align: center; " : "") + "color: #000000; font-size: 13px; font-family: sans-serif, arial; font-weight: normal; text-shadow: 0 0 8px #AEAEAE;");
...
function main(GM_config) {
	...
	GM_config.init("YouTube Auto Buffer & Auto HD Options", {
		autoBuffer : {
			...
			options : {
				"buffer" : "Auto Buffer",
				"play" : "Auto Play",
				"none" : "None (don't autobuffer or autoplay)"
			},
			"default" : "buffer"
		},
		autoHD : {
	...
	if(GM_config.get("footer") === true) navID = "footer-main";

	var nav = $g("//*[@id='" +navID + "'] | //div[@class='primary-pane']//div[@class='module-view featured-video-view-module'] | //div[@id='gh-overviewtab']/div[@class='c4-spotlight-module']", {type: 9});
	...
	nav.appendChild(create("button", {id: "autobuffer-options", style: "border: 1px solid #CCCCCC; border-radius: 2px 2px 2px 2px; font-weight: bold; vertical-align: middle; cursor: pointer; background: transparent !important;", className:"yt-uix-button yt-uix-button-text yt-uix-tooltip", type: "button", onclick:GM_config.open},
		...
		if(h5e !== null) {
			if(GM_config.get("autoBuffer") !== "play") {
			...
			var mp = $("movie_player"),
				...
				onPlaylist = window.location.href.find("list="),
				fv = mpC.getAttribute("flashvars").setVar("autoplay", ((GM_config.get("autoBuffer") !== "none" || (onPlaylist && GM_config.get("autoplayplaylists") === true)) ? "1" : "0")).setVar("enablejsapi", "1").setVar("fs", "1").setVar("iv_load_policy", (GM_config.get("hideAnnotations") === true ? "3" : "1"));
			...
			switch(opHD) {
				...
				case "highres": fv = fv.setVar("vq", "highres"); break;
			...
			function stateChange() {
				...
				switch(state) {
					case 1: // 1 = playing
						if(alreadyBuffered === false && autobuffer === "buffer" && !(onPlaylist && autoplayplaylists)) {
							var mp=document.getElementById("movie_player"),
			...
			addScript("var alreadyBuffered = false, onPlaylist = "+onPlaylist+", autoplayplaylists = " + (GM_config.get("autoplayplaylists") === true) + ", volume = "+volume+", autobuffer = \""+GM_config.get("autoBuffer")+"\";\n\n"+onYouTubePlayerReady+"\n\n"+stateChange, "stateChange");
...
if(url.find("#t=")) window.location.href = url.replace("#t=", "&t=");


// This function waits for the navbar and movie player to
// be ready before starting
function waitForReady(GMC) {
	if($g("//embed[@id='movie_player'] | //video[contains(@class, 'html5-main-video') and @src and @style]", {type: 9}) !== null) {
		main(GMC);
	} else {
		sec++;
...
if($("autobuffer-options") === null) {
	window.setTimeout(function() {waitForReady(GM_config);}, 100), sec=0;
	if(typeof GM_registerMenuCommand !== "undefined") GM_registerMenuCommand("YouTube Auto Buffer & Auto HD Options", GM_config.open);
}
