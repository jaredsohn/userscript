// ==UserScript==
// @name           blogspot enlarge
// @namespace      znerp
// @description    Displays fullsize pictures larger when you roll over smaller ones on blogspot blogs.
...
newDiv.addEventListener(
  ...
  true);
var allImages = document.evaluate("//a[contains(@href, '.jpg')]/img|"+
                                  "//a[contains(@href, '.gif')]/img",
...
for (var i=allImages.snapshotLength-1; i>=0; i--){
  ...
  thisImage.addEventListener(
    ...
    function(event) {
      ...
      fullsize = this.parentNode.href;
      thisImage = this;
      globalTimer = window.setTimeout(
        function (thisImage) {return function(result) {
          newDiv.setAttribute('style', 'background:url('+loading+') no-repeat;min-height:32px;min-width:32px;position:fixed;z-index:100;top:20px;left:20px;background-color:#F7F7F7;border:1px solid #3B5998;');
