...
// @namespace      http://saksham-nanda.com/
...
executeOnPage(function(checkUpdates, updateNeeded) {
	...
	window.ytd = {formats: {
			...
			17: { fmt: 17, quality: { level: 2, recdl: false, description: "Low Quality 3GPP"  }, mobile: true, format: "3GPP", mres: { width: 176, height: 144 }, wmres: { width: 176, height: 144 }, aformat: "44kHz, mono"  , acodec: "AAC" , vcodec: "MPEG-4" },
			...
			36: { fmt: 36, quality: { level: 3, recdl: true , description: "High Quality 3GPP" }, mobile: true, format: "3GPP", mres: { width: 320, height: 240 }, wmres: { width: 320, height: 240 }, aformat: "44kHz, mono"  , acodec: "AAC" , vcodec: "MPEG-4" },
	...
	if (!fmtStreamMap)
	{
		if (typeof videoPlayer != "undefined" && videoPlayer && videoPlayer.fa && videoPlayer.fa[0] && videoPlayer.fa[0].xa)
		{
			var formats5 = videoPlayer.fa;
			var lastQLevel = 99;
			for (var i = 0; i < formats5.length; i ++)
			{
				var args = formats5[i].xa.substring(formats5[i].xa.indexOf("?") + 1).split("&");
				var fmt;
				...
				if (ytd.formats[fmt])
				{
					...
					ytd.formats[fmt].url = formats5[i].xa + "&title=" + (title ?  title : "video");
					ytd.formats[fmt].guessed = false;
				}
				else
					ytd.formats[fmt] = { fmt: fmt, quality: { level: lastQLevel - 0.5, description: formats5[i].p }, url: formats5[i].xa + "&title=" + (title ?  title : "video") };
			}
		}
		else
			compatError();
