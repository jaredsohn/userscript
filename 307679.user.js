...
{
    method: 'GET',
    url: 'http://userscripts.org/scripts/source/'+numberUserscript+'.meta.js',/*la je vais sur la page ou il y a les données du script (auteur, version, etc)*/

    onload: function(response) 
    {
        var PageUserScript = response.responseText;/*la je chope le text sur la page web*/
        var Derniere_Version = trim(PageUserScript.split('@version')[1].split('// @uso:script')[0]);/*la je met la valeur de la dernière version dans Dernière_version*/
