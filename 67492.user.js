...
// ==/UserScript==
var twitterTarget = "DM_orders";
var twitterName = "com_gran_cap"; // ID твиттера
	...
	onload:function(response){
		...
		function relative_time(time_value) {
			...
			if (delta < 60) {
				...
				r = (parseInt(delta / 60)).toString() + ' minuto(s) hace';
			} else if(delta < (2*90*60)) { // 2* because sometimes read 1 hours ago
				r = ' chas nazad';
			} else if(delta < (24*60*60)) {
				r = 'okolo ' + (parseInt(delta / 3600)).toString() + ' hora(s) hace';
			} else if(delta < (48*60*60)) {
				r = 'vchera';
			} else {
				r = (parseInt(delta / 86400)).toString() + ' dia(s) hace';
			}
