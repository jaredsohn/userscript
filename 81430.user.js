...
// @description    Manages farmville wall posts; acepta bonuses, canastos, adoptar animales, recoger huevos, y mas....
...
var version = "1.2.18042"; // cant use cdata+regexp to grab version dynamically because of webkit
...
logo : "http://i43.tinypic.com/2v9y7uh.png",
...
// Fade by Txuso. Fade in/out by id and choose speed: slow, medium, or fast
...
// $g by Txuso. Supports ID, Class, and XPath (full with types) in one query
...
reqTO : 30000,
profile : "",
acceptedText : "OBTENIDO!!!!!",
failText : "YA LO HAS COGIDO O HA EXPIRADO!",
...
xpQueryTextExcludes : ["Fertilize their", "Lend a Hand", "Send Materials", "is looking for something", "wants to send a big", "wants to share", "found some", "has found a", "found a", "found some fuel", "is trying to", "could really use some help", "new FarmVille puppy is hungry", "wants YOU to help", "just completed level", "was working in the stables", "finished growing", "has completed a", "finished a job", "is hosting a barn raising", "is working hard on", "on up to", "Send doggie treats", "Send puppy kibble", "Join their Co-op", "Play FarmVille Now", "Play FarmVille now", "Feed their chickens", "Send Ingredients"],

// empty options object for later modification
opts : {},

// all regexps are stored here
whichRegex : /(give item to|gnome|bushel|catch a|reward|an egg|eggs|decoration|bonus|hatch|adopt|bouquet|perfect bunch|present|fuel|help|collectible|flag|get materials|cow|valentine|claim|some|gold|bamboo|reed thatch|bottle|blanket|brick|nail|wooden board|horseshoe|harness|horse|baby animal|floral bracket|green beam|glass sheet|white trellis|irrigation pipe|honeybee|queen bee)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
colorRegex : /(red brown|simmental|breton|party|belted|haflinger|cream draft|groovy|holi?stein|mouflon|mustang|tuscan|longhorn|appaloosa|neapolitan|boer|chocolate|clydesdale|percheron|pinto|silver|blue pony|purple pony|pink pony|pink patch|kelly green|purple|pony|green|pink|brown|black|baby|white|gr[ae]y)/,
calfRegex : /found a (simmental|fan|neapolitan|holstein|groovy|longhorn|belted|chocolate) calf/,
animalRegex : /(llama|goat|cow|turtle|turkey|sheep|kitten|kitty|duckling|duck|rabbit|bull|stallion|calf?|penguin|horse|pony foal?|foal?|pony)/,
...
accTextRegex : /(congratulations|would you like|you've been awarded|you've taken in|yee-haw|congrats|hooray|wants you to have this|your gift box|you've already accepted|you have already|you already have|do you want to)/,
failTextRegex : /(sorry|exceeded your limit|expired|whoa|woah|come back later|had enough help|someone already|only available for|try again|no room|select a free gift|slow down|thanks for trying to help)/,
...
case "collectible": w += (
				...
				text.find("Button Collection") ? "button":
					...
					""); break;
case "some": if((text.find("Favor")) || (text.find("Tuscan Wedding") && text.find("extras"))) w="favor";
...
switch(w) {
case "holistein": w="holstein"; break;
}
if(w.find("gray")) w = w.replace("gray", "grey");
if(main.calfRegex.test(text.toLowerCase())) w = text.toLowerCase().match(main.calfRegex)[1];
return w;
},
...
switch(w) {
case "foa": case "ponyfoal": case "ponyfoa": w="foal"; break;
...
toHomepage : function() { window.location.replace("http://www.facebook.com/home.php"); },
toFilterpage : function() { window.location.replace("http://www.facebook.com/home.php?filter=app_"+main.gameID+"&show_hidden=true&ignore_self=true&sk=lf"); },
...
if(main.currReqs == 0) { // if the # of requests are at 0
		...
		$(main.gameAcronym.toLowerCase()+"_msgbox").style.display = "none"; // hide msg box
	}
	else {
...
if(!box) $("contentArea").insertBefore(box=main.create("div", {id:main.gameAcronym.toLowerCase()+"_msgbox",style:"background:#CAEEFF; border:2px solid #4AC5FF; z-index:998; padding:20px; -moz-border-radius:6px; -moz-appearance:none; display:none;"}, new Array(
...
onGiveItemFrameLoad : function(e) {
	var doc=e.target.contentDocument, w=e.target.getAttribute("which"), key=e.target.getAttribute("id"),
		...
		sendButton=$g(".//div[@class='gift_box_holder']/div[@class='recipient']//form[starts-with(@id, 'req_form_')]/input", {doc:doc, node:doc, type:9}), text=$("app_content_"+main.gameID, doc);
	...
	if(text.find("Sorry") || text.find("sorry")) {
	...
	if(sendButton) main.click(sendButton);
...
if(gotItem==true && (yes || no)) {
	...
	switch(w) {
		...
		case "collectiblebutton": whichCollectibleOffered = main.checkCollectible(doc, text, main.whichButtonRegex); break;
	}
...
if(w == "giveitemto") $("silent_req_holder").appendChild(main.create("iframe", {src:url, which:w, id:key, style:"height:100%; width:100%; z-index:9995; border:0;", onload:main.onGiveItemFrameLoad}));
...
GM_config.init("<img src='"+imgs.logo+"'> v"+version, {
	...
	raising : {
		label : "Ayudar en los graneros?",
		type : "checkbox",
		"default" : true
	},
	giveitemto : {
		label : "Enviar regalos con los links items?",
		type : "checkbox",
		"default" : true
	},
	hatchwhite : {
		label : "Recoger Huevo Blanco?",
		type: "checkbox",
		"default" : false,
		kids : {
			hatchbrown : {
				label : "Recoger Huevo marron?",
				type : "checkbox",
				"default" : false
			},
			hatchblack : {
				label : "Recoger Huevo negro?",
				type : "checkbox",
				"default" : false
			},
			hatchgold : {
				label : "Recoger Huevo de Oro?",
				type : "checkbox",
				"default" : false
			},
			hatchrhodeislandred : {
				label : "Recoger huevo Rhode Island Red?",
				type : "checkbox",
				"default" : false
			},
			hatchscotsgrey : {
				label : "Recoger Huevo Scots Grey?",
				type : "checkbox",
				"default" : false
			},
			hatchcornish : {
				label : "Recoger Huevo Cornish?",
				type : "checkbox",
				"default" : false
			}
		}
	...
	adopt : {
		label : "Adoptar animales? (opcion master)",
		type: "checkbox",
		"default" : true
	},
	unknown : {
		label : "Adoptar animales desorganizados?",
		type: "checkbox",
		"default" : true
	},
	collectible : {
		label : "Recoger coleccionables? (opcion master)",
		type : "checkbox",
		"default" : true
	},
	bouquet : {
		label : "Obtener bouquets?",
		type: "checkbox",
		"default" : false,
		kids : {
			perfectbunch : {
				label : "Obtener flores perfectas?",
				type : "checkbox",
				"default" : false
			}
		}
	...
	bushelmain : {
		label : "Obtener canastos? (opcion master)",
		type : "checkbox",
		"default" : true
	},
	fuel : {
		label : "Obtener fuel gratis?",
		type: "checkbox",
		"default" : false
	},
	stallion : {
		label : "Ayudar en los caballos Stallion?",
		type : "checkbox",
		"default" : true
	},
	farmhands :  {
		label : "Obtener recoge animales?",
		type: "checkbox",
		"default" : false
	},
	arborists :  {
		label : "Obtener recoge arboles?",
		type: "checkbox",
		"default" : false
	},
	checkallmain : {
	...
	brick : {
		section : [ "MATERIALES E ITEMS DE DECORACION" ],
		label : "Get Bricks?",
		type: "checkbox",
		"default" : false
	},
	...
	materialsstable : {
		...
		kids : {
			...
			materialsjapanesebarn : {
				label : "Get japanese barn materials?",
				type: "checkbox",
				"default" : false
			},
			materialsbeehive : {
				label : "Get beehive materials?",
				type: "checkbox",
				"default" : false
			}
		},
	...
	decorationjapanesebarn : {
		label : "Get japanese barn decoration items?",
		type : "checkbox",
		"default" : false
	},
	...
	claimfavor : {
		label : "Claim Tuscan Wedding items? (master)",
		type : "checkbox",
		"default" : true,
		kids : {
			claimweddingcake : {
	...
	calfbaby : {
		...
		kids : {
			...
			calfsimmental : {
				label : "Simmental Calf",
				type : "checkbox",
				"default" : false
			},
			calfredbrown : {
				label : "Red Brown Calf",
				type : "checkbox",
				"default" : false
			}
		}
	...
	foalbrown : {
		...
		kids : {
			...
			foalsilver : {
				label : "Silver Pony Foal",
				type : "checkbox",
				"default" : false 
			},
			foalblackpony : {
				label : "Black Pony Foal",
				type : "checkbox",
				"default" : false 
			},
			foalappaloosa : {
...
"span.config_var {display:inline !important; margin-left:12px !important;}\n"+
