// ==UserScript==
// @name           Multi login no orkut by Trunkszin
// @namespace      Modificado por Trunkszin
// @description    Multiplos login no orkut
// @include        http://*.orkut.*/*
/*
 *Criado por Andre 
 *Modificado por Trunkszin
 */
// ==/UserScript==
...
var passdata = [
	...
	function save(){
		...
		Account6 = [Email6,Senha6].join('|');
		
		Final = [Account1,Account2,Account3,Account4,Account5,Account6].join('][');
...
NewDiv.setAttribute('style', 'width: 380px; vertical-align:middle; height: 315px; background-color: #FFFFCC; border: 1px solid #ED9B34; position: fixed; top: 80px; padding: 3; display: none;');
NewDiv.innerHTML='<div style="color: navy;">Editar Emails</div><br>'+
				 ...
				 'Email &nbsp;5:<input name="email" style="position: relative; left: 22px;" type="text"></input><a style="position: relative; left: 25px;" 
href="javascript: void(0);">Blank</a><br>'+
				 'Senha 5:<input name="senha" style="position: relative; left: 19px;" type="password"></input><br><hr>'+
				 'Email &nbsp;6:<input name="email" style="position: relative; left: 22px;" type="text"></input><a style="position: relative; left: 25px;" 
href="javascript: void(0);">Blank</a><br>'+
				 'Senha 6:<input name="senha" style="position: relative; left: 19px;" type="password"></input><br><hr>'+ 
				 '<div style="float:left; position: relative; bottom: 5px; font-size: 9px; color: red;">OBS.: Inserir Login e senha validos<br>Trunkszin</div>' +
...
Newform.setAttribute('style', 'width: 250px; vertical-align:middle; height: 120px; background-color: #FFFFCC; border: 1px solid #ED9B34; position: fixed; top: 80px; padding: 3; display: none;');
Newform.innerHTML = '<div style="color: red;"><center>Fazer Login</center></div>' +
					'<hr></hr>'+
	...
	div.innerHTML = '<select style="height: 17px; font-size: 10px; color: black;">' + 
				'<option selected="true" style="font-size: 10px; font-color: black;">'+ email + '</option>' +
				'<option style="font-size: 10px;">----------------</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[0] + '</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[1] + '</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[2] +'</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[3] +'</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[4] +'</option>' +
				'<option style="font-size: 10px; color: red;">' + emaildata[5] +'</option>' +
				'<option style="font-size: 10px;">----------------</option>' +
				'<option style="font-size: 10px; color: blue;">Adcionar emails</option>' +
				'<option style="font-size: 10px;">----------------</option:>' +
				'<option style="font-size: 10px; color: black;>Area de Login</option>' +
				'</select>';
...
data[5].addEventListener('click', function(){this.parentNode.getElementsByTagName('input')[10].value='- not inserted -';this.parentNode.getElementsByTagName('input')[11].value='';}, true);
