// ==UserScript==
// @name	The West - ColorMe [SOM] (Beta) fixed by Oldingyombingyom
// @namespace	http://userscripts.org/scripts/show/95518
// @include	http://*.the-west.*/game.php*
// @include	http://userscripts.org/scripts/review/95518*
// @include	http://userscripts.org/scripts/source/95518.meta.js*
// @description	enhancer of The West chat with integrated features 
...
*/
	...
	this.twTag ={
	"[Players][/Players]": "<img src='images/transparent.png' style='background-image:url(../images/bbcodes.png);background-position: -80px;height:20px;width:20px;margin:6px 1px;'>"/*,
...
function c(tag, from) {
	...
	case 'purple':
		...
		break;
	case 'tellName':
		this.tellName = prompt("Add meg egy játékos nevét, akivel suttogni szeretnél");
	...
	case 'custColor':
		...
		if(from == 'tell') {
			...
			this.custColor ='/'+ prompt("Add meg a használni kíván egyéni színkódot");
		}
...
function infoScript() {
	...
	var somMsg;
	if ((window.location.hostname.substr(0, window.location.hostname.search(/\./)))=='fr6') {
		hrefStr = 'javascript:AjaxWindow.show(\'profile\',{char_id:578826},\'578826\');';
	}else{
		hrefStr = 'mailto:knownas.y@gmail.com';
	}
	var Author = '<a href=\"' + hrefStr + '\">Y.</a>';
...
function d() {
	...
	for (var i = 0; i <= lastRow; i++) {
		...
		//players<a href="javascript:parent.AjaxWindow.show('profile',{player_name:encodeURIComponent(&quot;yannph&quot;)},&quot;yannph&quot;);">yannph</a>
		openP = colorTxt.substr_count(curTxt, '[Players]');
		closeP = colorTxt.substr_count(curTxt, '[/Players]');
		if ((openP != '0') && (closeP != '0')) {
			for (var j = 0; j < openP; j++) {
				label = curTxt.substring(curTxt.indexOf("[Players]") + 9, curTxt.indexOf("[/Players]"));
				curTxt = curTxt.substring(0, curTxt.indexOf("[Players]")) + '<a href="javascript:parent.AjaxWindow.show(\'profile\',{player_name:encodeURIComponent(&quot;' + label + '&quot;)},&quot;' + label + '&quot;);">' + label + '</a>' + curTxt.substring(curTxt.indexOf("[/Players]") + 10, curTxt.length);
			}
		}
/*
...
*/
		...
		if ((openR != '0') && (closeR != '0')) {
			for (var k = 0; k < openR; k++) {
				pos1 = parseInt(curTxt.indexOf("[report=") + 8); 
pos2 = curTxt.substring(pos1).indexOf("]"); 
id1 = curTxt.substr(pos1,pos2-10); 
id2 = curTxt.substr(pos1+pos2-10,10); 
label = curTxt.substring(pos1+pos2+1, curTxt.indexOf("[/report]"));
