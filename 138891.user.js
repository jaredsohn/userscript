// ==UserScript==
// @name           GoR Power Carpe_Diem
// @version        6.3
// @namespace      Napoleone
// @description    GoR Power Tools By Carpe_Diem
...
var Version = '6.2';
var Autore = 'W Carpe_Diem';
var ScriptName = 'GoR Carpe_Diem';
var miseajour=ScriptName+" "+Version +" - by "+Autore;
var sitesupport="http://www.facebook.com/groups/378717735516648/";
var pubblicity="<http://www.facebook.com/groups/378717735516648/ width=320 marginwidth=0 marginheight=0 height=50 scrolling=no frameborder=0></iframe>";
...
var translateITAArray = {
	...
	//Tower Alerts
   	...
   	//AutoAttack
   		...
   		'Check cities you want to attack from':'Seleziona le città da cui vuoi far partire l\'attacco',	
   		'TROOPS':'TRUPPE',
	...
	//Options
        ...
        'Allow to post attack alerts in chat':'Posta in chat ally gli avvisi di attacco',
...
function facebookInstance (){
  ...
  var e = document.getElementById('mainContainer');
 	if(e){
 		document.getElementById('content').style.minWidth = '1280px';
...
.hostile td { background:crimson; }.friendly td{background:lightblue; }.ally td{background:royalblue; }\
  ...
  AddMainTabLink("Carpe_Diem", eventHideShow, mouseMainTab);
...
var ChatStuff = {
  ...
  chatDivContentHook : function (msg){
      ...
      if (m[0].indexOf("** ARRIVEE **") >= 0 || m[0].indexOf("** ARRIVAL **") >= 0 || m[0].indexOf("** ARRIVANO **") >= 0) {
    ...
    if (m[0].indexOf("** ARRIVEE **") >= 0 || m[0].indexOf("** ARRIVAL **") >= 0 || m[0].indexOf("** ARRIVANO **") >= 0 && Options.Attackenabled) {
    ...
    if (suid) 
     if (scripters.indexOf(suid[1]) > -1 && m[0].indexOf("sussurra")<0) 
     ...
     for (k in Smileys) {
        ...
        if (k=="(massage)")
	     ...
	     msg=msg.replace(k, '<img style="width:66px !important;height:66px !important" class=emoicon src=\"'+Smileys[k]+'\">');
          ...
          msg=msg.replace(k, '<img style="width:85px !important;height:52px !important" class=emoicon src=\"'+Smileys[k]+'\">');
          ...
          msg=msg.replace(k, '<img style="width:120px !important;height:90px !important" class=emoicon src=\"'+Smileys[k]+'\">');
	else if (k=="(SpiderPig)")
		  msg=msg.replace(k, '<img style="width:265px !important;height:110px !important" class=emoicon src=\"'+Smileys[k]+'\">');
	else if (k=="(Wolf)")
		  msg=msg.replace(k, '<img style="width:192px !important;height:122px !important" class=emoicon src=\"'+Smileys[k]+'\">');
	else
      ...
      for(i=0; i < Cities.numCities; i++) {
    	...
    	var timeLeft = parseInt(Cities.cities[i].c.resources[1].count)  / (0-usage) * 3600;
          ...
          if (timeLeft<0){
              ...
              msg += ' '+translate('is running out of food')+', '+translate('so please send some')+'? '+translate('Current stock')+': '+addCommasWhole(foodleft).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ')+' ('+timestrShort(timeLeft)+') '+translate('Production')+': '+addCommas(usage).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ');
                sendChat ("/a " + msg);
                //alert(msg);
          }
      }  
 ...
 init : function (){
  ...
  show : function (){  
   ...
   var ModelCity = {};
    ...
    t.cont.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=bopbSubtab ID=BoTrpSubM type=submit value="Manuel"></td>\
             <TD><INPUT class=bopbSubtab ID=BoTrpSubA type=submit  value="Automatique"></td><TD><INPUT class=bopbSubtab ID=BoTrpSubR type=submit DISABLED  value="Approvisionner"></td></tr></table>\
         <DIV id="BoTrpOutput" style="margin-top:5px; background-color:white; height:'+(Options.HauteurBoite-65)+'px;max-height:600px;overflow-y:auto"></div>';
  ...
  showAuto: function() {
     var t = my.TranspAuto;
      var m = '<DIV id=pbTowrtDivF class=ptstat>TRANSPORT AUTOMATIQUE - CONFIGURATION</div><TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center">';
          ...
          if (t.traderState.running == false) {
           ...
           m += '<TD><INPUT id=pbShowRoutes type=submit value="Voir les Routes"></td>';
           m += '</tr></table></div>';
           m += '<DIV id=pbTraderDivDRoute class=ptstat>OPTIONS DU TRANSPORT</div>';
           m += '<TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="left">';
     	 m += '<TD colspan=2>Envoyer les transports toutes les <INPUT id=pbtransportinterval type=text size=4 value="'+Options.transportinterval+'"\> minutes</td>';
           m += '<TD colspan=4>Envoyer les transports avec un minumum de <INPUT id=pbminwagons type=text size=2 value="'+Options.minwagons+'"\> troupes</td></tr></table>';
           m += '</table>';
           m += '<DIV id=pbTraderDivDRoute class=ptstat>AJOUTER UN TRANSPORT</div>';
           m += '<TABLE id=pbaddtraderoute width=95% height=0% class=pbTab><TR align="left">';
           m += '<TR align="left"><TD>Source :</td> <TD colspan=4><DIV style="margin-bottom:10px;"><span id=ptrescity></span></div></td></tr>';
           ...
           m += '<TD width=75px>troupes :</td><TD width=150px><SELECT id="TransportTroop">';
           unsafeWindow.Barracks.allUnitIds.each(function(r){
	             if (unsafeWindow.arStrings.unitName["u"+r]) 
           ...
           m+='</select></td><TD width=75px>Dispo :&nbsp;</td><TD id=TroopAmount align=left width=75px></td>';
           m+='<TD width=75px>Estimation ::&nbsp;</td><TD id=CarryAmount align=left width=75px></td>';
           ...
           m += '<TD width=180px  align=left>Garder : <INPUT id=pbtargetamountFood type=text size=11 maxlength=12 value="0" disabled=true\></td>';
           ...
           m += '<TD width=180px align=left>Garder : <INPUT id=pbtargetamountWood type=text size=11 maxlength=12 value="0" disabled=true\></td>';
           ...
           m += '<TD width=180px align=left>Garder : <INPUT id=pbtargetamountStone type=text size=11 maxlength=12 value="0" disabled=true\></td>';
           ...
           m += '<TD width=180px align=left>Garder : <INPUT id=pbtargetamountOre type=text size=11 maxlength=12 value="0" disabled=true\></td>';
           ...
           m += '<TD width=180px align=left>Garder : <INPUT id=pbtargetamountGold type=text size=11 maxlength=12 value="0" disabled=true\></td>';
           //m += '<TD width=100px>Transporter: <INPUT id=pbtradeamountGold type=text size=11 maxlength=12 value="0"\></td>';
           m += '<TD width=50px></td></tr>';
     
           m += '</table>';
     
           m += '<DIV style="text-align:center; margin-top:15px"><INPUT id=pbSaveRoute type=submit value="Ajouter une Route"></div>';
      ...
      ById('TransportTroop').value = 'u9';
         ...
         updateTroops : function (city){
          ...
          var LoadUnit=0; //parseInt(parseInt(uW.unitstats[untid][5])*(1+loadBoost));
         	...
         	ById('Calc').innerHTML = 'Ressources: ' +  addCommas(t.Food + t.Wood + t.Stone + t.Ore + t.Gold  + t.Astone) + ' / ' + addCommas(t.MaxLoad) + '&nbsp;&nbsp;(Troupes n&eacute;cessaires : <FONT color='+fontcolor+'>' + addCommas(t.TroopsNeeded) + '</font> )' ;
         },    
        ...
        e_tradeRoutes: function(){
     	...
     	showTradeRoutes: function () {
     		...
     		t.popTradeRoutes.getTopDiv().innerHTML = '<TD><B>Les routes du Transport Automatique : </td>';
     		t.paintTradeRoutes();
     	...
     	paintTradeRoutes: function(){
     		...
     		for (var i=0;i<(r.length);i++) {
     			  citynameTo = null;
     				...
     				if (r[i].route_state) var status = '<FONT color=green>Activ&eacute;</font>';
             ...
             m +='<TR><TD width=20px>'+(i+1)+'</td><TD width=175px>Depuis:&nbsp;&nbsp;'+ cityname +'</TD><TD width=175px>Vers:&nbsp;&nbsp;'+ TO +'</td><TD width=175px>'+status+'</td>';
             m +='<TD width=60px><A onclick="traceEdit('+queueId+')">Editer</a></td><TD width=60px><A onclick="traceDelete('+queueId+')">Supprimer</a></td></tr>';
             m += '<TR><TD></td><TD>Troops:&nbsp;&nbsp;</td></tr>';
             if (r[i].ship_Food) m += '<TR><TD></td><TD align=center><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png"></td><TD>Target: '+ addCommas(r[i].target_Food) +'</td>';
     				if (r[i].ship_Wood) m += '<TR><TD></td><TD align=center><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/wood_30.png"></td><TD>Target: '+ addCommas(r[i].target_Wood) +'</td>';
     				if (r[i].ship_Stone) m += '<TR><TD></td><TD align=center><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/stone_30.png"></td><TD>Target: '+ addCommas(r[i].target_Stone) +'</td>';
     				if (r[i].ship_Ore) m += '<TR><TD></td><TD align=center><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/iron_30.png"></td><TD>Target: '+ addCommas(r[i].target_Ore) +'</td>';
     				if (r[i].ship_Gold) m += '<TR><TD></td><TD align=center><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/gold_30.png"></td><TD>Target: '+ addCommas(r[i].target_Gold) +'</td>';
            }
     	     m +='</table>';
     	 ...
     	 editQueueElement: function(queueId){
            ...
            var n = '<TABLE id=editRoutes class=pbTab>';
     	     n +='<TD>Depuis :&nbsp;'+ cityname +'</td><TD>Vers :&nbsp;'+ TO +'</td>';
     	     n +='<TD><INPUT id=TradeStatus type=checkbox>&nbsp;Activ&eacute; la route</td>';
     	     n += '<TD width=150px>Troupes :<SELECT id="pbbTransportTroop">';
             unsafeWindow.Barracks.allUnitIds.each(function(r){
	    	             if (unsafeWindow.arStrings.unitName["u"+r]) 
            ...
            for (var i=0;i<Types.length;i++){
              ...
              n += '<TD width=125px>Garde : <INPUT id=pbbtargetamount'+Types[i]+' type=text size=11 maxlength=12 value="0"></td>';
            }
            n+='</table><BR><TABLE id=editRoutes class=pbTab><TR><TD><a class="button20" id="Cancel"><span>Annuler</span></a></td>';
            n+='<TD><a class="button20" id="Save"><span>Enregistrer</span></a></td></tr>';
            n +='</table>';
 ...
 getContent : function (){
  ...
  show : function (){ 
      ...
      var SP = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
         ...
         var m = '<DIV class=ptstat>AUTO ATTACK - by Tiestoale</div><table width=100%><tr><td width="70%" style="vertical-align: middle;"><center>';
         if (Options.AttackAutoTest) {
     ...
     m+=translate('Attack Mode')+': '+ htmlSelector({1:'Single Coord', 2:'Multi Coords'},Options.AttackCoordsMode,'id=BOselectAttackMode')+'<br><br><br>';
     m+=SP+"Single Coord : X:<input type=text size=3 value='"+Options.AttackAutoTestX+"' id=TestX>  Y:<input type=text size=3 value='"+Options.AttackAutoTestY+"' id=TestY><br><br>";
     m+=SP+'Multi Coords : <i>x,y</i><br>';
     m+=SP+SP+'<textarea name=coordlist id=coordlist rows=10 cols=20></textarea><br>';
     m+=SP+SP+'<input id="ptartkcheck" type="submit" value="'+translate('Check Coordinates List')+'">&nbsp;<span id="ptcheckinfo"></span><br><br>';
	 m+='<input type=checkbox id="BoStopRoute" ' +(Options.AdvAtk.StopCicle?'CHECKED':'')+'>'+translate('AutoAttack stops when coordinates will be finished')+'<br><center><i>('+translate('do not be resumed')+')</i></center><br>';
	 ...
	 m+='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+translate('Resume the attacks after')+" <input type=text size=2 id=BoTempoAtk value='"+Options.AdvAtk.TempoAtk+"'> "+ htmlSelector({1:translate('seconds'), 2:translate('minutes'), 3:translate('hours')},Options.AdvAtk.TempoMode,'id=BOselectTempoMode');
     ...
     m+='<input id="ptrestimnow" type="submit" value="'+translate('Reset Timer Now')+'"></center></td></tr></table><br>';
 	 ...
 	 m+="<DIV class=ptstat>"+translate('Check cities you want to attack from')+" :</div>";
      for (var c=0; c<Cities.numCities; c++) {
      ...
      m+='<br><span id="prova"></span>';
      m+='<br><br><DIV class=ptstat>'+translate('TROOPS')+'</div><table width=100%>';
      m+='<tr><td>'+uW.arStrings.unitName["u1"]+' <input type=text size=3 id=Attacku1 value="'+Options.AttackUnit1+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u2"]+' <input type=text size=3 id=Attacku2 value="'+Options.AttackUnit2+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u6"]+' <input type=text size=3 id=Attacku6 value="'+Options.AttackUnit6+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u7"]+' <input type=text size=3 id=Attacku7 value="'+Options.AttackUnit7+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u4"]+' <input type=text size=3 id=Attacku4 value="'+Options.AttackUnit4+'"></td>';
      m+='</tr><tr>';
      m+='<td>'+uW.arStrings.unitName["u10"]+' <input type=text size=3 id=Attacku10 value="'+Options.AttackUnit10+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u9"]+' <input type=text size=3 id=Attacku9 value="'+Options.AttackUnit9+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u8"]+' <input type=text size=3 id=Attacku8 value="'+Options.AttackUnit8+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u5"]+' <input type=text size=3 id=Attacku5 value="'+Options.AttackUnit5+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u11"]+' <input type=text size=3 id=Attacku11 value="'+Options.AttackUnit11+'"></td>';
      m+='</tr><td>';
      m+='<td>'+uW.arStrings.unitName["u12"]+' <input type=text size=3 id=Attacku12 value="'+Options.AttackUnit12+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u31"]+' <input type=text size=3 id=Attacku31 value="'+Options.AttackUnit31+'"></td>';
      m+='<td>'+uW.arStrings.unitName["u32"]+' <input type=text size=3 id=Attacku32 value="'+Options.AttackUnit32+'"></td>';      
      m+='</tr></table>';
      
      m+='<br><br><b><center>'+translate('W Carpe_Diem')+'!</center></b><br><center>'+pubblicity;
    ...
    ById('Attacku1').addEventListener ('change', function () {
    	if(ById('Attacku1').value == '') ById('Attacku1').value = 0;
    	if(ById('Attacku1').value < 0) ById('Attacku1').value = 0;
		Options.AttackUnit1 = ById('Attacku1').value;
       saveOptions();
	}, false);
    ById('Attacku2').addEventListener ('change', function () {
    	if(ById('Attacku2').value == '') ById('Attacku2').value = 0;
    	if(ById('Attacku2').value < 0) ById('Attacku2').value = 0;
		Options.AttackUnit2 = ById('Attacku2').value;
       saveOptions();
	}, false);
    ById('Attacku6').addEventListener ('change', function () {
    	if(ById('Attacku6').value == '') ById('Attacku6').value = 0;
    	if(ById('Attacku6').value < 0) ById('Attacku6').value = 0;
		Options.AttackUnit6 = ById('Attacku6').value;
       saveOptions();
	}, false);	
    ById('Attacku7').addEventListener ('change', function () {
    	if(ById('Attacku7').value == '') ById('Attacku7').value = 0;
    	if(ById('Attacku7').value < 0) ById('Attacku7').value = 0;
		Options.AttackUnit7 = ById('Attacku7').value;
       saveOptions();
	}, false);	
    ById('Attacku4').addEventListener ('change', function () {
    	if(ById('Attacku4').value == '') ById('Attacku4').value = 0;
    	if(ById('Attacku4').value < 0) ById('Attacku4').value = 0;
		Options.AttackUnit4 = ById('Attacku4').value;
       saveOptions();
	}, false);
    ById('Attacku10').addEventListener ('change', function () {
    	if(ById('Attacku10').value == '') ById('Attacku10').value = 0;
    	if(ById('Attacku10').value < 0) ById('Attacku10').value = 0;
		Options.AttackUnit10 = ById('Attacku10').value;
       saveOptions();
	}, false);
    ById('Attacku9').addEventListener ('change', function () {
    	if(ById('Attacku9').value == '') ById('Attacku9').value = 0;
    	if(ById('Attacku9').value < 0) ById('Attacku9').value = 0;
		Options.AttackUnit9 = ById('Attacku9').value;
       saveOptions();
	}, false);
    ById('Attacku8').addEventListener ('change', function () {
    	if(ById('Attacku8').value == '') ById('Attacku8').value = 0;
    	if(ById('Attacku8').value < 0) ById('Attacku8').value = 0;
		Options.AttackUnit8 = ById('Attacku8').value;
       saveOptions();
	}, false);	
    ById('Attacku5').addEventListener ('change', function () {
    	if(ById('Attacku5').value == '') ById('Attacku5').value = 0;
    	if(ById('Attacku5').value < 0) ById('Attacku5').value = 0;
		Options.AttackUnit5 = ById('Attacku5').value;
       saveOptions();
	}, false);	
    ById('Attacku11').addEventListener ('change', function () {
    	if(ById('Attacku11').value == '') ById('Attacku11').value = 0;
    	if(ById('Attacku11').value < 0) ById('Attacku11').value = 0;
		Options.AttackUnit11 = ById('Attacku11').value;
       saveOptions();
	}, false);		
     ById('Attacku12').addEventListener ('change', function () {
    	if(ById('Attacku12').value == '') ById('Attacku12').value = 0;
    	if(ById('Attacku12').value < 0) ById('Attacku12').value = 0;
		Options.AttackUnit12 = ById('Attacku12').value;
       saveOptions();
	}, false);	
    ById('Attacku31').addEventListener ('change', function () {
    	if(ById('Attacku31').value == '') ById('Attacku31').value = 0;
    	if(ById('Attacku31').value < 0) ById('Attacku31').value = 0;
		Options.AttackUnit31 = ById('Attacku31').value;
       saveOptions();
	}, false);	
    ById('Attacku32').addEventListener ('change', function () {
    	if(ById('Attacku32').value == '') ById('Attacku32').value = 0;
    	if(ById('Attacku32').value < 0) ById('Attacku32').value = 0;
		Options.AttackUnit32 = ById('Attacku32').value;
       saveOptions();
	}, false);	
	  
    document.getElementById('coordlist').value = Options.AttackAutoCoords;
	document.getElementById('ptartkcheck').addEventListener ('click', function () {
		t.klistcheck();
	}, false);
    document.getElementById('coordlist').addEventListener ('change', function () {
		Options.AttackAutoCoords = document.getElementById('coordlist').value;
       saveOptions();
	}, false);	
    document.getElementById('BOselectAttackMode').addEventListener ('change', function () {
		Options.AttackCoordsMode = document.getElementById('BOselectAttackMode').value;
		if(Options.AttackCoordsMode == 1){
		ById('ptartkcheck').disabled = true;
		ById('coordlist').disabled = true;
		ById('TestX').disabled = false;
		ById('TestY').disabled = false;
		} else {
		ById('ptartkcheck').disabled = false;
		ById('coordlist').disabled = false;
		ById('TestX').disabled = true;
		ById('TestY').disabled = true;
		}
       saveOptions();
	}, false);		
  ...
  klistcheck : function() {
		...
		var counter = 0;
		t.koordlist = [];
		if (Options.AttackAutoCoords != ""){
		var u_koords = Options.AttackAutoCoords.split("\n");
		...
		Options.AttackAutoCoords = t.koordlist.join("\n");
		saveOptions();
		document.getElementById('ptcheckinfo').innerHTML = counter + ' '+translate('valid coordinates found');
		document.getElementById('coordlist').value = Options.AttackAutoCoords;
		if (counter < 0) {
			return;
		}
		} else {
		document.getElementById('ptcheckinfo').innerHTML = '<font color=red>'+translate('No coordinated inserted')+'</font>';
		}
	},
  klistchecka : function() {
		var t = my.testattack;
		var counter = 0;
		t.koordlist = [];
		var u_koords = Options.AttackAutoCoords.split("\n");
  ...
  start:function() {
   ...
   if(Options.AttackCoordsMode == 1){
      var cooX = Options.AttackAutoTestX;
   	  var cooY = Options.AttackAutoTestY; 
  		...
  		if(t.koordlist[t.koordlistpointer] == undefined){
   ...
   var unit32 = 32;
     ...
     if (Cities.byID[id].c.canMarch() && Cities.byID[id].c.generalsCount()!=0 && selectedKnight!=0) {
       ...
       if(Options.AttackUnit1 >0) a.units[unit1].sent=Options.AttackUnit1;
       if(Options.AttackUnit2 >0) a.units[unit2].sent=Options.AttackUnit2;
       if(Options.AttackUnit6 >0) a.units[unit6].sent=Options.AttackUnit6;
       if(Options.AttackUnit7 >0) a.units[unit7].sent=Options.AttackUnit7;
       if(Options.AttackUnit4 >0) a.units[unit4].sent=Options.AttackUnit4;
       if(Options.AttackUnit10 >0) a.units[unit10].sent=Options.AttackUnit10;
       if(Options.AttackUnit9 >0) a.units[unit9].sent=Options.AttackUnit9;
       if(Options.AttackUnit8 >0) a.units[unit8].sent=Options.AttackUnit8;
       if(Options.AttackUnit5 >0) a.units[unit5].sent=Options.AttackUnit5;
       if(Options.AttackUnit11 >0) a.units[unit11].sent=Options.AttackUnit11;
       if(Options.AttackUnit12 >0) a.units[unit12].sent=Options.AttackUnit12;
       if(Options.AttackUnit31 >0) a.units[unit31].sent=Options.AttackUnit31;
       if(Options.AttackUnit32 >0) a.units[unit32].sent=Options.AttackUnit32;
       if(Options.AttackUnit1 >0) g["u"+unit1]=Options.AttackUnit1;
       if(Options.AttackUnit2 >0) g["u"+unit2]=Options.AttackUnit2;
       if(Options.AttackUnit6 >0) g["u"+unit6]=Options.AttackUnit6;
       if(Options.AttackUnit7 >0) g["u"+unit7]=Options.AttackUnit7;
       if(Options.AttackUnit4 >0) g["u"+unit4]=Options.AttackUnit4;
       if(Options.AttackUnit10 >0) g["u"+unit10]=Options.AttackUnit10;
       if(Options.AttackUnit9 >0) g["u"+unit9]=Options.AttackUnit9;
       if(Options.AttackUnit8 >0) g["u"+unit8]=Options.AttackUnit8;
       if(Options.AttackUnit5 >0) g["u"+unit5]=Options.AttackUnit5;
       if(Options.AttackUnit11 >0) g["u"+unit11]=Options.AttackUnit11;
       if(Options.AttackUnit12 >0) g["u"+unit12]=Options.AttackUnit12;
       if(Options.AttackUnit31 >0) g["u"+unit31]=Options.AttackUnit31;
       if(Options.AttackUnit32 >0) g["u"+unit32]=Options.AttackUnit32;
...
<br><br><button  id="BOAcceptAllKDO" class="button25" type="button"><span>'+translate('Accept all gifts')+'</span></button><br><br><hr><b><center>'+translate('W Carpe_Diem')+'!</center></b><br><center>'+pubblicity+'</div>';
...
function reloadKOC (){
  ...
  actionLog('Reload','Refresh now!');
  setTimeout (function (){ById('xxpbButReload').click();}, 0);
}

/*********************************** Carpe_Diem Group Tab ***********************************/
my.perso = {
  cont : null,
  timer : null,
  ...
  show : function (){
    ...
    t.cont.innerHTML = '<CENTER><iframe src="http://www.f.com" width="750" height="690" id="BOFrame" border=0></iframe><BR></center>'; 
 	} catch (e) {
...
my.ActionLog = {
  ...
  show : function (div){
    var t = my.ActionLog;
       ...
       <select id=BOSuiviFiltre><option value="">All</option><option value="Reload">Reload</option></select>\
       &nbsp;<input type=button value="Clear" id=BOLogClean><br><TABLE cellpadding=0 cellspacing=0 id=BOactionlog class=pbTabLined></table></div>';
...
my.Options = {
  ...
  show : function (){
    var t = my.Options;   
      try {      
        m = '<DIV style="height:670px; max-height:670px; overflow-y:auto"><TABLE class=ptTab>\
          ...
          m+='<TR><TD>&nbsp;</td><TD>'+translate('Language')+': '+ htmlSelector({1:'EN', 2:'IT'},Options.Langue,'id=BOLangue') +'</td></tr>\
         ...
         <TR><TD><INPUT id=pbRessEnable type=checkbox /></td><TD>'+translate('Auto-collect resources every')+' <INPUT id=pbLimitRess type=text size=2 maxlength=3 \> '+translate('minutes')+'</td></tr>\
        ...
        <TR><TD><INPUT id=ptAllowFB '+ (GlobalOptions.autoPublishGamePopups?'CHECKED ':'') +' type=checkbox /></td><TD>'+translate('Allow to publish your helpings automatically to Facebook')+' '+ htmlSelector({0:'----', 80:'Everyone', 50:'Friends of friends', 40:'Friends only', 10:'Me only'},GlobalOptions.autoPublishPrivacySetting,'id=selectprivacymode') +'</td></tr>\
        <TR><TD colspan=2><hr><b><br><br><center>'+translate('W Carpe_Diem')+'! </center></b><br><center>'+pubblicity+'</center><hr><DIV class=ptstat>'+translate('Configuration of chat')+'</div></td></tr>\
	...
	<TR><TD></td><TD><INPUT type=submit value="'+translate('Tester')+'" id=boPlayNow> &nbsp; <INPUT id=boSoundStop type=submit value="'+translate('Stop Sound Alert')+'"></td></tr></table></div></td></tr>\
        </table></table><BR><BR><HR><center><b>'+ miseajour +'</b></center><br><br><hr><b><br><br><center>'+translate('W Carpe_Diem')+'!</center></b><br><center>'+pubblicity+'</div>'
        t.cont.innerHTML = m;
    ...
    ById("BOLangue").addEventListener('change', function(){
   	 Options.Langue = ById('BOLangue').value;
   	 reloadKOC();
	 saveOptions();
	 } , false);
   ...
   if (t.opt.searchType == 0)
     ...
     if (t.opt.searchType == 1 || t.opt.searchType == 0) {
 		...
 		m+= '<TR><TD class=xtab align=right >Filter :</td><TD class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
 	     ...
 	     <OPTION value=2>'+translate('Misty cities only')+' (KoC feature)</option>\
  ...
  show : function (){
    ...
    clearTimeout (t.displayTimer);
      ...
      str = '<div style="height:670px;max-height:670px;overflow-y:auto"><DIV class=ptstat style="margin-top:2px; margin-bottom:2px; "><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><b>' + unsafeWindow.player.name + '</>&nbsp;&nbsp;<SPAN class=ptStatLight>'+translate('Joined on')+' :</span> '+ dt.toLocaleDateString() +'</td>\
        <TD><SPAN class=ptStatLight>'+translate('Glory')+' : </span> ' + addCommasInt(unsafeWindow.player.might()) +'</td>\
        <TD><SPAN class=ptStatLight>'+translate('Alliance')+' : </span> ' + getMyAlliance()[1] + '</td>\
        <TD align=right><SPAN class=ptStatLight>'+translate('World')+' :</span> ' + unsafeWindow.domainName +'</td></tr></table></div><span id="debugtest"></span>';
...
getContent : function (){
  ...
  show : function (){  
    ...
    if (TrainOptions.Running == false) {
    	       m += '<TD><INPUT id=autoFormationtoggle type=submit value="'+translate('AUTO-TRAIN')+' = OFF"></td>';
    	   } else {
    	       m += '<TD><INPUT id=autoFormationtoggle type=submit value="'+translate('AUTO-TRAIN')+' = ON"></td>';
	   }  
    m+="</tr></table>";
  ...
  toggleautoFormationState : function(obj) {
     ...
     if (TrainOptions.Running == true) {
              ...
              obj.value = translate('AUTO-TRAIN')+" = OFF";
              saveTrainingOptions();
      }  else {
              TrainOptions.Running = true;
              obj.value = translate('AUTO-TRAIN')+" = ON";
              saveTrainingOptions();
      }
   },
  ...
  if(tabs) {
    ...
    if (mouseListener != null)
      a.addEventListener('mousedown',mouseListener, true);
       ...
       GM_xmlhttpRequest({method:"GET",url:"http://www.facebook.com/groups/378717735516648/",onload:function(a){var b=/\/\/\s*@version\s+(.+)\s*\n/i.exec(a.responseText);
       var c=Version;if(b){if(b[1]>c){
       var eNewMAJ=createButton ("New version "+b[1]+"!");
       eNewMAJ.className='tab';
       eNewMAJ.id="BOUpdate";
       eNewMAJ.title="Please update the GoR Power Bot!";
       tabs.insertBefore(eNewMAJ,tabs.firstChild);
       eNewMAJ.addEventListener('click',function() {
               window.open(sitesupport); //redirection sur mon site
       }, false);
