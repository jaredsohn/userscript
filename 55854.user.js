// ==UserScript==
// @name           Travian: جابه‌جا کنندهء ساختمان‌ها
// @namespace      Travian: جابه‌جا کنندهء ساختمان‌ها
// @description    با داشتن این اسکریپت می‌توانید ساختمان‌های مرکز دهکده را آنگونه که مایلید جا‌به‌جا کنید. 
// @version        1.5.1
...
function reset_vil(){
    if (!window.confirm("آیا مایلید ساختمان‌ها به موقعیت پیشفرض بازگردند؟")) return;
    delete mapping[did];
...
GM_registerMenuCommand("BM: Reset all buildings", function(){
        if (!window.confirm("آیا مایلید ساختمان‌های همهء دهکده‌ها به موقعیت پیشفرض بازگردند؟")) return;
        GM_setValue('mapping', '({})');
        location.reload();
    });
...
div.addEventListener('click', function(){
        ...
        truck_stage++;
        div.childNodes[0].title = 'بازگشت ساختمان‌ها به موقعیت اولیه!';
        ...
        for (var i in poly) poly[i].addEventListener('click', function(e){
                ...
                // Error conditions
                if (dest == '21'){
                    notify(' <b>شما نمی‌توانید <font color=red>اردوگاه</font> را جابه جا کنید.</b> ');
                    return;
                }
                if (dest == '22'){
                    notify(' <b>شما نمی‌توانید <font color=red>دیوار</font> را جابه جا کنید.</b> ');
                    return;
                }
        ...
        notify('<b>بر روی ساختمان اول کلیک کنید...</b>');
    }, false);