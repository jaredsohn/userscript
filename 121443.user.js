...
“https://docs.google.com/spreadsheet/viewform?hl=en_US&formkey=dEtMMnhMazBhY0VlOW44

cE1rLTdxdUE6MQ#gid=0";
var FORM_KEY = FORM_URL.substring(FORM_URL.lastIndexOf("formkey=") + 8, 

FORM_URL.lastIndexOf("#"));
...
var LOCALE = {
	en:function() {
		return {
			locale: "en_US",
			no_damage: "You must deal at least 1 damage before sending 

a report. Press F5 if the counter is bugged.",
			send_report: "Do you want to send a report?",
			report_sent: "You report is being sent. Please, don't close 

the new window until it has finished loading!",
			round_end: "Round has finished. Do you want to send a 

report? (if you don't, it will be lost!)",
			report: "FAA"
		};
	},
	bg: function() {
		return {
			locale: "es_ES",
			no_damage: "Tenes que hacer daño antes de enviar un 

reporte. Apreta F5 si el contador esta buggeado.",
			send_report: "Estas listo para enviar el reporte a FAA?",
			report_sent: "El reporte se esta enviando. Por favor, no 

cierres la ventana hasta que se termine de cargar!",
			round_end: "La ronda termino. Queres enviar tu reporte? (si 
...
(function() {
	...
	// We use this to wrap DOM handlers. This will allow us to fix problems at 
...
one place only.
	function PageElement(els) {
		...
		this.append = function(content) { els[0].innerHTML += content; 

return self; };
		this.bind = function(eventType, handler) { 

els[0].addEventListener(eventType, handler, false); return self; };
...
element.getElementById(sel.substr(1));
						break;
						
					case ".":
						match = 

element.getElementsByClassName(sel.substr(1));
						element = match.length ? match[0] : 
...
element.getElementsByTagName(sel);
						element = match.length ? match[0] : 
...
"https://spreadsheets.google.com/formResponse?hl=en_US&formkey=" + FORM_KEY + 
...
"&ifq";
		...
		submitButton = createInput(reportForm, "submit", "submit", 
...
http://www.erepublik.com/images/modules/sidebar/sidebar_buttons_wide.png?1321873582
	...
	this.createReportObject = function() {
		...
		var data = {
			...
			damage: parseInt($.find("#total_damage strong").text()), 

//innerHTML
			battle: $.find(".regionLink").attr("title"),
			eday: $.find("#clock .eday strong").text(),
			time: $.find("#battle_countdown").text(),
			wall: selectFirst("#blue_domination_f", 

"#blue_domination").text(),
			donate: "http://www.erepublik.com/en/economy/donate-items/" 

+ profileUrl.substr(profileUrl.lastIndexOf("/") + 1),
			round: 1
		};
...
document.getElementsByClassName('action_holder')[0].appendChild(reportButton);
	...
	newReportEnd.setAttribute('style','position: relative; left: 240px; top: 
...
65px;');	
	...
	// Confirmation popup
	/*$.find("#content").append('\
	<div class="pop enemy_defeated" style="display: none; font-size: 14px; 

font-weight: bold; height: 280px" id="bs_report_pop">   \
		<div class="heading">                                               
...
src="http://www.erepublik.com/images/modules/pvp/war_effort.png" class="symbol">    

                       \
			<strong>Influencia: <span 

id="bs_report_pop_dmg"></span></strong>                                             
		...
		<span style="height: 160px; float: left; display: block">           
                                                       ...
                                                       \
			<p style="padding-top: 1em">El reporte se esta 

enviando.</p>                                                           \
			<p style="padding-top: 1em">Por favor, no cierres la 

ventana hasta que se termine de cargar!</p>                                         
 ...
 \
                                                   ...
                                                   \
	<a title="Reportar a FAA" href="javascript:;" 

id="bs_report_pop_send">Reportar a FAA</a>                                         
