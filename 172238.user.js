...
// @homepage    https://userscripts.org/scripts/show/172238
// @updateURL    https://userscripts.org/scripts/source/172238.meta.js
...
var CSS = [
  ...
  '.gm_fullfeed_entry_url a { padding-left: 12px; }',
  ...
  '#gm_fullfeed_settings input[type="text"], #gm_fullfeed_settings textarea { color: black; padding: 2px; }',
...
var FullFeed = function(info, c, flag) {
  ...
  this.itemInfo.item_body = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_content ")]');
...
FullFeed.prototype.request = function(m, c) {
  ...
  state = 'request';
  var self = this, retry = 0, mes = '', xhrID;
  var message2 = function(m, d, t) {
    mes = m;
    message(m, d, t);
  };
  var gmRequest = function() {
    var xhr = GM_xmlhttpRequest(opt);
    xhrID = window.setTimeout(function() {
      window.clearTimeout(xhrID);
      xhr.abort();
      state = null;
      self.requestError.apply(self, ['Request abort']);
    }, 10000);
  };
  var opt = {
    method: m,
  ...
  if (opt.method === 'HEAD') message2(loc[54], -1);
...
FullFeed.prototype.requestEnd = function(c, as, halt) {
  ...
  var el = getFirstElementByXPath(returnXpathEntry() + '//span[contains(concat(" ", normalize-space(@class), " "), " gm_fullfeed_checked_icon ")]');
...
FullFeed.checkScroll = function() {
  ...
  scrollInterval = window.setTimeout(function() {
    var remain, s = 'reader_pane_view_style_';
    var e1 = $id('reader_pane'), btn = $id('view_style_img');
    ...
    } else if (e1.classList.contains(s + '3')) {
...
FullFeed.resetCache = function() {
  ...
  var getSiteinfo = function(data, flag) {
    var nCache = 0;
    var nCaches = (flag) ? siteInfo.length : siteInfoAP.length;
...
FullFeed.setCache = function(res, ctx, item, flag) {
  ...
  if (info.length > 0) {
    ...
    if (flag) {
      ...
      var hasAP = (localStorage.getItem('InoReaderFullFeed_cacheAP')) ? true : false;
...
FullFeed.getCache = function(key) {
  var js = '{}';
  if (localStorage.getItem('InoReaderFullFeed_' + key))
    js = localStorage.getItem('InoReaderFullFeed_' + key);
...
FullFeed.loadSettings = function(data) {
  if (data === 'reset') st = {};
    ...
    st = {};
    try {
      if (localStorage.getItem('InoReaderFullFeed_settings'))
        st = JSON.parse(localStorage.getItem('InoReaderFullFeed_settings'));
    } catch(er1) {}
  }
  ...
  if (notType('Boo', st.cantdisplay)) st.cantdisplay = false;
  ...
  if (notType('Boo', st.socialfc2)) st.socialfc2 = false;
  if (notType('Boo', st.socialhatena)) st.socialhatena = false;
  if (notType('Boo', st.sociallivedoor)) st.sociallivedoor = false;
  if (notType('Boo', st.twitter)) st.socialtwitter = true;
...
FullFeed.createSettings = function() {
  ...
  item.id = 'gm_fullfeed_settings-menu';
  item.innerHTML = 'Full Feed ' + loc[21];
  if (menu) {
...
FullFeed.viewSettings = function(id) {
  ...
  var eCurrent = getFirstElementByXPath(returnXpathEntry());
  var feedTitle = '';
  ...
  if (feedTitle) {
    ...
    $ids('autoload_whitelist_title').value = '^' + feedTitle + '$';
    $ids('autoload_blacklist_title').value = '^' + feedTitle + '$';
  }
...
FullFeed.registerWidgets = function() {
  ...
  el = getElementsByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_title ")]/span');
  if (el) {
    el.some(function(i) {
      if (i.classList.contains('gm_fullfeed_checked')) {
        flag = true;
        return true;
      }
    });
  ...
  if (flag) return;
  var container = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_title ")]');
  if (!container) return;
  itemSiteInfo = [];
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(res) {
        if (res.status === 200 && res.finalUrl) {
          ...
          var atl = getFirstElementByXPath(returnXpathEntry() + '//a[contains(concat(" ", normalize-space(@class), " "), " article_title_link ")]');
          if (atl && atl.href) atl.href = res.finalUrl;
          var hbl = getFirstElementByXPath(returnXpathEntry() + '//a[starts-with(@id, "aurl_")]');
...
FullFeed.registerSocialIcons = function() {
  ...
  if (!c.innerContents) return;
  el = getElementsByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_title ")]/span[@class="gm_fullfeed_socialicon"]');
  if (el) return;
  var container = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_title ")]');
  ...
  [sT, sD, sF, sH, sL, sY, sB, sC].forEach(function(i) {
    i.className = 'gm_fullfeed_socialicon';
  });
  ...
  var inter, repeat = 0, check = function() {
    var e = getElementsByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_title ")]/span[@class="gm_fullfeed_socialicon"]/a/img'), f = true;
    repeat ++;
    if (e) {
...
var get_active_item = function() {
  var item = {};
  var exp = getFirstElementByXPath(returnXpathEntry() + '//a[contains(concat(" ", normalize-space(@class), " "), " article_title_link ")]');
  if (exp) {
...
var GetCurrentItem = function() {
  ...
  this.articleContainer = getFirstElementByXPath(returnXpathEntry());
  this.innerContents = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_content ")]');
  this.found = false;
};
...
var initFullFeed = function(scroll) {
  var c = new GetCurrentItem();
  if ((state && (state !== 'loaded' && state !== 'wait')) || !c.item.title || !c.item.url || !c.innerContents) return;
  message(loc[54], -1);
  if (st.notread) {
    ...
    for(var i = 0, j = rep.length - 1; i < j; i = i + 2) {
      if (rep[i] && new RegExp(rep[i]).test(c.itemURL)) {
        if (rep[i + 1] && /^http/.test(rep[i + 1])) {
          ...
          try {
            var u = getElementsByXPath(returnXpathEntry() + '//div[contains(concat(" ", normalize-space(@class), " "), " article_content ")]//a');
            if (u) excludeUrl(u);
          } catch(er) {}
        }
        break;
...
var createMessageBox = function() {
  ...
  div.className = 'ui-state-highlight gm_fullfeed_hidden';
  div.innerHTML = '';
  document.body.appendChild(div);
};
...
var init = function() {
  ...
  if (GM_registerMenuCommand && typeof GM_registerMenuCommand === 'function') {
    GM_registerMenuCommand('InoReader Full Feed - reset cache', function() {
      if (window.confirm(loc[15])) FullFeed.resetCache();
    });
    GM_registerMenuCommand('InoReader Full Feed - delete cache', function() {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (e.button === 0 && e.target.id && /^gm_fullfeed_settings-/.test(e.target.id)) {
      ...
      if (e.target.id === 'gm_fullfeed_settings-menu') {
        ...
        getElementsByXPath('//span[contains(concat(" ", normalize-space(@class), " "), " gm_fullfeed_settings-siteinfo_link ")]', document).forEach(function(i) {
        ...
        if (!problem) {
          $id('gm_fullfeed_settings-ok').blur();
          set.style.display = 'none';
          FullFeed.saveSettings();
        }
      } else if (e.target.id === 'gm_fullfeed_settings-cancel') {
        $id('gm_fullfeed_settings-cancel').blur();
        set.style.display = 'none';
      }
  ...
  $id('reader_pane').addEventListener('scroll', function() {
    var btn = $id('view_style_img'), rp = $id('reader_pane'), s = 'reader_pane_view_style_';
    if ((rp && rp.classList.contains(s + '1')) || (btn && btn.hasAttribute('src') && /page-break\.png$/.test(btn.getAttribute('src')))) {
      window.setTimeout(function() {
        if (st.autopagerize) FullFeed.checkScroll();
        FullFeed.checkRegister();
      }, 1000);
    } else if (((rp && rp.classList.contains(s + '0')) || btn) && st.autopagerize) {
      var c = new GetCurrentItem();
      if (c.innerContents && c.innerContents.classList.contains('gm_fullfeed_loaded')) FullFeed.checkScroll();
    }
  }, false);
  $id('three_way_contents').addEventListener('scroll', function() {
    if (st.autopagerize) FullFeed.checkScroll();
  }, false);
};
...
function removeXSSRisks(htmldoc) {
  ...
  var embed = htmldoc.getElementsByTagName('embed');
  var param = htmldoc.getElementsByTagName('param');
  for (var i1 = 0, j1 = embed.length; i1 < j1; i1++) {
    if (embed[i1].hasAttribute(attr)) embed[i1].setAttribute(attr, 'never');
  }
  for (var i2 = 0, j2 = param.length; i2 < j2; i2++) {
    if (param[i2].hasAttribute('name') && param[i2].getAttribute('name') === attr)
      param[i2].setAttribute('value', 'never');
  }
}
...
function relativeToAbsolutePath(htmldoc, base) {
  ...
  var a = htmldoc.getElementsByTagName('a');
  var img = htmldoc.getElementsByTagName('img');
  for (var i1 = 0, j1 = a.length; i1 < j1; i1++) {
    url = a[i1].getAttribute('href');
    if (url) {
      if (url.indexOf(' ') === 0) url = url.replace(/^\s+(.+)/, '$1');
      a[i1].href = _rel2abs(url, top, current, base);
    }
  }
  for (var i2 = 0, j2 = img.length; i2 < j2; i2++) {
    url = img[i2].getAttribute('src');
    if (url) {
      if (url.indexOf(' ') === 0) url = url.replace(/^\s+(.+)/, '$1');
      img[i2].src = _rel2abs(url, top, current, base);
    }
  }
}
