// ==UserScript==
// @name           2ch: profit!
// @description    Some tweaks for 02ch.su imageboard
// @namespace      http://02ch.su/
...
var url = location.pathname.substr(1), url_ = url.split('/'), elt, delform = $('delform'),
	host_strings = {name:	{'02ch.su': 'akane',		'iichan.ru': 'nya1'},
					email:	{'02ch.su': 'nabiki',	'iichan.ru': 'nya2'},
					topic:	{'02ch.su': 'kasumi',	'iichan.ru': 'nya3'},
					message:{'02ch.su': 'shampoo',	'iichan.ru': 'nya4'},
					capurl:	{'02ch.su': '',			'iichan.ru': '/'},
					gb2:	{'02ch.su': 'gb2" value="thread', 'iichan.ru': 'postredir" value="1'}};
...
// ADDING DIV THREAD IDENTIFIERS
var hrs = delform.innerHTML.split('<br clear="left"><hr>');
var cpy, id_thread;
delform.innerHTML = '';

for (var i = 0; i < hrs.length - 1; i++)
{
	//	в hrs[i] хранится тред целиком, последний hrs[i] - форма удаления
	//cpy = hrs[i].split('<a href="/'+board[0]+'/res/');
	cpy = hrs[i].split('>№');
//	id_thread = cpy[1].split('.html');
id_thread = cpy[1].split('<');
	delform.innerHTML = delform.innerHTML + '<div id="thread-' + id_thread[0] + '">  ' + hrs[i] + '</div> <br clear="left"><hr>';
}
delform.innerHTML = delform.innerHTML + hrs[hrs.length - 1];
...
with (document.body.appendChild(document.createElement('div')))
	style.cssText = 'position:fixed;z-index:99;top:2px;left:50px;background-color:#000;color:#fff;-moz-border-radius:15px;padding:0 10px 10px 10px;display:none',
...
if (config[17]) {
	var storebm = function() {
		...
		if (vis === true) {
			...
			newdiv.innerHTML = '<a href="http://02ch.su/' + bo + '/res/' + id + '.html">' + id + '</a> (<a href="#" id="' + bo + '_' + id + '">x</a>) - ' + preview;
	...
	for (var i = 0, l = subs1.length; i < l; i++) {
		...
		for (var j = 1; j < subsect.length; j++)  {
			...
			sect += '<div><a href="http://02ch.su/' + subsect[0] + '/res/' + nfo[0] + '.html">' + nfo[0] + '</a> (<a href="#" id="' + subsect[0] + '_' + nfo[0] + '">x</a>) - ' + unescape(nfo[1]) + '</div>';
		}
...
if (elt = $n('captcha')) {
	/*if (config[1] && location.host == '02ch.su') {
...
function mainLoop(container) {
	...
	if (config[9] && (url_[0] == 'mu')) {
		for (var xp = document.evaluate('.//img[@src="/mu/icons/audio-mp3.png"]', container, null, 6, null), mp3, i = 0; mp3 = xp.snapshotItem(i); i++) {
			...
			with (pl)
				src = 'http://02ch.su/mu/mediaplayer.swf',
...
if (config[15] || config[23] || config[24] || config[29]) var threads = {}, serialize = function(tid, text) {
	...
	// div adding start //
	var thrd = text.split(' method="post">'); // тред начинается в thrd[1]
	var thrdfull = thrd[1].split('<br clear="left"><hr>'); // Весь тред целиком в thrdfull[0]
	var icpy, iid_thread;

	icpy = thrdfull[0].split('>№');
	iid_thread = icpy[1].split('<');
	text = thrd[0] + ' method="post"> <div id="thread-' + iid_thread[0] + '">  ' + thrdfull[0] + '</div> <br clear="left"><hr>' + thrdfull[1];
	// div adding end //
