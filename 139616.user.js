// ==UserScript==
// @name        Travian4 AutoMator ** تطوير ww_start_t من منتدى ترافيان العربي
// @author      ww_start_t (Serj_LV, Pimp Trizkit (based on TTQ by Risi and further edited by Nevam and then Pimp Trizkit))
// @description Auto Build, Auto Attack, Auto Train. ww_start_t من منتدى ترافيان العربي
// @include		http://*.travian.*/*
// @version     1.0.0.0
// ==/UserScript==

/*authors: ww_start_t (Serj_LV, Pimp Trizkit (based on TTQ by Risi and further edited by Nevam and then Pimp Trizkit))*/

(function () {

    function allInOneTAM() {
        notRunYet = false;
        var sCurrentVersion = "1.0.0.0";
        ...
        function detectServer() {
            var lang = GM_getValue(CS + 'lang', '0');
        ...
        function processSortedTaskList(element) { $id("tam_tasklist").appendChild(element[1]); }
        function processSortedHistory(element) { $id("tam_history").appendChild(element[1]); }
        function trim(str) {
        ...
        function tamTrimData(data, maxlength, asString, token) {
        ...
        function setOption(key, value) {
            var options = getVariable('TAM_OPTIONS', '');
            ...
            setVariable('TAM_OPTIONS', options.join(","));
        }
        ...
        function initialize() {
            ...
            if (server) {
                ...
                if (myPlayerID.length < 1) {
                    _log(0, "Init> Unknown page. TAM is not running. Possible Login screen. Attempting Auto-Login in a few seconds...");
                    return false;
                }
		...
		"jp", "kr", "lt", "lv", "mx", "my", "net", "nl", "pt", "pl", "ro", "ru", "se", "rs", "ser", "si", "sk", "asia", "th", "tr", "tw", "ua", "vn"];
            ...
            var nLangBuildings = ["", "الحطاب", "مصنع الطوب", "منجم حديد", "حقل القمح", "معمل النشارة", "مصنع االطوب", "حديد مسبك", "المطاحن", "المخبز", "المخزن", "مخزن حبوب", "<لايوجد مبنى>", "الحداد", "ساحة البطولة", "المبنى الرئيسي", "نقطة التجمع", "السوق", "السفارة", "الثكنة", "الاسطبل", "المصانع الحربية", "الاكاديمية", "المخبأ", "البلدية", "السكن", "القصر", "الخزنة", "المكتب التجاري", "الثكنة الكبيرة", "الاسطبل الكبير", "جدار المدينة", "الحاجز الارضي", "الحاجز", "الحجار", "المقهى", "الصياد", "قصر الابطال", "المخزن الكبير", "مخزن حبوب كبير", "معجزة العالم", "ساقية الخيول"];
            var nLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train", "Party", "Demolish", "Send Merchants", "Send Back/Withdraw"];
            var nLangStrings = ["Build later", "Upgrade later", "Unknown Town", "Research later", "Schedule this Task for Later", "We started building ", "<center>HALT!</center><br>Please wait, TAM is processing this task!<br>Step", "Traps", " build request sent. However, it appears that the building is not building.", "was attempted but the server redirected us.", "The task was scheduled.", "Redirected", "We can't schedule this task right now.", "Error", "Scheduled Tasks", "Delete", "Send later", "No troops were selected.", "Your troops were sent to", "Your troops could not be sent to", "Support", "Attack", "Raid", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "بعيد", "The attack cannot be scheduled because no destination was specified.", "الصفحة رقم.", "فرز حسب:", "النوع ", "الوقت ", "الهدف ", "الأعدادات ", "القرية ", "تاريخ المهام", "مسح المحفوظات", "تم بدأ البحث في الأكاديمية ", " لايمكن بدأ البحث في الأكاديمية.", "فشل الصفحة", "تجسس", "تدريب لاحقاً", "troops.", "... May have not happened!", "We started training ", " cannot be trained.", "الأحتفال لاحقاً", " لكن ليس اليوم.", "تم بدأ ", "إغلاق", "أضافة أو تعديل مهام", "عدل ثم أغلق", "أضف ثم أغلق", "أضف", "Are you sure you want to [s1] [s2]?", "الهدم لاحقاً", "يهدم", "لايمكن الهدم", "الأحداثيات خاطئة أو لايوجد موارد محدده.", "أستعمال وقت الجهاز", "أستعمال وقت السيرفر", " was attempted but we could not find the link.", " was attempted but failed. Reason: ", "لايوجد رابط", " was attempted but the building was not found.", "لايوجد مبنى", " was attempted but the server returned an error.", "السيرفر:", "Confirmation Failed", "Sorry, I <b>may</b> have built the building in the wrong town.", "Misbuild:", "Sent Back/Withdrew troops.<br>Troops are going home to:", "Sent Back/Withdrew troops Failed (I think).<br>Troops were supposed to go home to: ", "أضغط هنا للدخول الى هذه القرية.", "Click to see this Village Details screen.", "أنقطع الوقت أو السكربت أخطأ في أيجاد الصفحة"];
            var nLangMenuOptions = ["TAM: ", "أستخدام وقت السيرفر", "أستخدام وقت الجهاز", "أختر قبيلتك", "تاريخ المهام", "أعادة", "\nكم عدد مهام تريد ان يحفظها السكربت؟?\n(ضع الرقم 0 لتعطيل حفظ المهام.) \nحالياً: ", " ماهي قبيلتك في هذا السيرفر؟\nضع الرقم 0 للرومان\n, أو الرقم 1 الرقم للجرمان,\n أو الرقم 2 للأغريق.\n أو ضع الرقم 100 للتفعيل خاصية التعرف على القبيلة آلياً\nحالياً: ", "هل أنت متأكد من أعادة ضبط السكربت؟"];
            var nLangTroops = new Array();
            nLangTroops.push(["جندي أول", "حراس الأمبراطور", "جندي مهاجم", "فرقة تجسس", "سلاح الفرسان", "فرسان القيصر", "الكبش", "مقلاع ناري", "حيكم", "مستوطن", "بطل", nLangStrings[7]]);
            nLangTroops.push(["مقاتل بهراوة", "مقاتل برمح", "مقاتل بفأس", "الكشاف", "مقاتل القيصر", "فرسان الجرمان", "محطمة الأبواب", "المقلاع", "الزعيم", "مستوطن", "بطل", nLangStrings[7]]);
            nLangTroops.push(["الكتيبة", "مبارز", "المستكشف", "رعد الجرمان", "فرسان السلت", "فرسان الهيدوانر", "محطمة الابواب الخشبية", "المقلاع الحربي", "الرئيس", "مستوطن", "بطل", nLangStrings[7]]);
            var aLangResources = ["Lumber", "Clay", "Iron", "Crop"];
            switch (sLang) {
                case "ae": //by Fahad (updated by Pimp Trizkit) edited by armanda
                case "eg":
                case "sa":
                    aLangTasks = ["بناء", "تطوير", "هجوم", "فتح قسم", "تدريب", "احتفل", "دمر", "ارسال التجار"];
                    aLangStrings = ["البناء لاحقا", "تطوير لاحقا", 0, "فتح القسم لاحقا", "جدولة هذا العمل لاحقا", "لقد بداءالبناء ", 0, 0, " لا يمكن ان يبناء.", 0, "هذا العمل مجدول", 0, "لا يمكن ادراج هذه العملية لان.", 0, "المهام المجدولة", "حذف", "ارسال لاحقا", "لم يتم اختيار الجنود.", "الجنود متوجهين الى", "جيوشك لا يمكن ارسالها الى", "مساندة", "هجوم", "نهب", "تصويب المقلاع نحو", "عشوائي", "عند", "او بعد", "ثانية", "دقيقة", "ساعة", "يوم", "التجسس على الجيوش والموارد", "التجسس على الجيوش والتحصينات", "بعيد", "لا يمكن جدولة هذا الهجوم لان الهدف غير محدد ", "البناء عند:", "فرز بواسطة:", "النوع ", "الوقت ", "الهدف ", "الخيارات ", "القرية ", "مهام محفوظه", "حذف المحفوظات الحالية", "بداية عملية البحث ", " لا تستطيع اعادة البحث", 0, "تجسس", "تدريب لاحقا", "جنود", 0, "تم بدء التدريب", "لا تستطيع التدريب"];
                    break;
            ...
            var TAM_registeredMenu = $e('UL', [['style', 'font-size:12px;']]);
            var TAM_registeredMenuFL = true;
            function TAM_registerMenuCommand(t, f) {
                var newLI = $ee('LI', t);
                tamAddEventListener(newLI, 'click', f, false);
                TAM_registeredMenu.appendChild(newLI);
            }
            function TAM_showMenuCommand() {
                if (TAM_registeredMenuFL) {
                    $id('tamPanel').appendChild(TAM_registeredMenu);
                    TAM_registeredMenuFL = false;
                } else {
                    $id('tamPanel').removeChild(TAM_registeredMenu);
                    TAM_registeredMenuFL = true;
                }
            }
            ...
            tA.setAttribute("id", "tamPanel");
            tA.setAttribute("style", "position:absolute;top:" + (parseInt(getPosition($id("wrapper")).y) + 24) + "px;" + docDir[0] + ":50%;background-color:rgba(200, 200, 250, 0.8);padding:0px 2px 0px 2px;margin:0px " + (ltr ? 0 : 320) + "px 0px " + (ltr ? 320 : 0) + "px;white-space:nowrap;color:black;font-family:segoe ui;font-size:10px;-moz-border-radius:4px;border-radius:4px;border:1px solid black;z-index:10000;cursor:pointer;");
            tA.innerHTML = "<span id='tamPanelSpan'><b>TAM</b>-T4 v" + sCurrentVersion + " -<span style='display: none;'><span id='tamLoad' style='font-weight: bold' >0</span> ms - <span id='tamReloadTimer'>0</span></span> ww_start_t</span>";
            tamAddEventListener(tA, "click", TAM_showMenuCommand, false);
            ...
            for (n8 = 0, m8 = 0, l8 = iMyRace.length; m8 < l8; ++m8) {
                ...
                vName = "<span class ='tam_village_name' onclick='window.location = \"" + tA.href + "\";return false;' title='" + aLangStrings[80] + "'>" + tA.innerHTML + "</span>&nbsp;<span class ='tam_village_name tam_village_coords' title='" + aLangStrings[81] + "' onclick='window.location = \"position_details.php?x=" + tX + "&y=" + tY + "\";return false;' >(" + tX + "|" + tY + ")</span>";
            ...
            var otherPlaceNames = tamTrimData(getVariable("OTHER_PLACE_NAMES", ""), MXPN, false, "↨⌂₧☻");
            ...
            //Styles
            var cssStyle = "";
            cssStyle += "#tam_tasklist, #tam_history {border-radius: 5px;box-shadow: 0px 0px 5px black; position:absolute; background-color: rgba(255, 255, 255, 0.90); max-width: 800px; color: black; border:1px solid #C0C0C0;padding:0 0 2px 0; z-index:501; -moz-border-radius:0px 0px 10px 10px;}";
            cssStyle += "#tam_flush_history {border-radius: 5px;   text-align:center;color:black;cursor:pointer;font-weight:bold;font-size:14px;font-family:'Lucida Sans Unicode','Arial';} #tam_flush_history:hover {color:black;}";
            cssStyle += ".tam_tasklist_row {border-radius: 5px;   padding:1px 10px;}";
            cssStyle += ".tam_history_row {border-radius: 5px;  padding:1px 5px 1px 10px;border-style:dashed;border-color:black;border-width:0px 0px 1px 0px;}";
            cssStyle += ".tam_village_name {border-radius: 5px;  font-weight:bold;cursor:pointer;font-size:11px;font-family:'Lucida Sans Unicode','Arial'} .tam_village_name:hover {color:white}";
            cssStyle += ".tam_village_coords {border-radius: 5px;  unicode-bidi:bidi-override;direction:ltr;} .tam_village_coords:hover {color:white}";
            cssStyle += ".tam_draghandle {border-radius: 5px;   font-size: 103%; font-weight:bold; color:black; background-color: rgba(255, 255, 255, 0.90);background-repeat:repeat-x; border-bottom:1px solid #C0C0C0;height:20px;repeat-x; padding-left: 11px;}";
            cssStyle += ".tam_time_village_wrapper {border-radius: 5px;  font-style:bold; font-size:80%; float:left;} .tam_time_village_wrapper:hover {color:white}";
            cssStyle += ".tam_close_btn {border-radius: 5px;  float:right; padding:2px 4px; color:black; margin:-5px -15px 0 0;cursor:pointer}";
            cssStyle += "#timerForm {border-radius: 5px;  padding:0px 0px 10px 20px; }";
            cssStyle += "#timerform_wrapper {border-radius: 5px;box-shadow: 0px 0px 5px black; position:absolute;  width:550px !important; margin:0; background-repeat:no-repeat; background-position:center center; background-color: rgba(255, 255, 255, 0.90); color: black; border: 1px #59D72F solid;  z-index: 502; -moz-border-radius:8px;}";
            cssStyle += "#timerform_wrapper p {border-radius: 5px;  }";
            cssStyle += "#tam_message {border-radius: 5px; position:absolute; z-index:503; border:1px solid #71D000; padding:10px 20px; color:black; width:335px; -moz-border-radius:10px;}";
            cssStyle += ".handle {border-radius: 5px; cursor: move;}";
            cssStyle += "#tam_tasklist_sortlinks {border-radius: 5px; border-bottom:1px solid #C0C0C0; margin-bottom: 5px; padding:0;}";
            cssStyle += ".tam_tasklist_sortlinks_header { padding-left: 10px; background-color: rgba(255, 255, 255, 0.90);}";
            cssStyle += ".tam_tasklist_sortlinks_child { border-left: 1px solid #C0C0C0; text-align: center; background-color: rgba(255, 255, 255, 0.90); cursor: pointer;}";
            cssStyle += ".tam_tasklist_sortlinks_child:hover { border-left: 1px solid #C0C0C0; text-align: center; background-color: rgba(255, 255, 255, 0.90); cursor: pointer;}";
            cssStyle += ".tam_tasklist_sortlinks_child_active { border-left: 1px solid #C0C0C0; text-align: center; background-color: rgba(255, 255, 255, 0.90); cursor: pointer;}";
            cssStyle += ".tam_sort_header {border-radius: 2px 2px 0px 0px; color:black; padding: 2px;}";
            cssStyle += ".tam_research_later { display:block;}";
            cssStyle += "#sortLinkWrapper{border-radius: 2px 2px 0px 0px; border-bottom:1px dashed #000000;}";
            GM_addStyle(cssStyle);
        }
        function checkSetTasks() {
            _log(1, "CheckSetTasks> Begin. (tab ID = " + myID + ")");
            var aThisTask, aTasks = getVariable("TAM_TASKS");
            var oDate = Math.floor(((new Date()).getTime()) / 1000); // local time
            tamUpdatePanel(aTasks, oDate);

            if (bLocked) {
                _log(1, "CheckSetTasks> The TAM_TASKS variables is locked. We are not able to write it. Canceling checkSetTasks...");
                return false;
            }
            bLocked = true;
            var data = getVariable("TAM_TABID", 0);
            if (data == 0) {
            ...
            if (tamBusyTask != 0) {
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(parseInt(tamBusyTask[4])) + '</span>' + getTaskDetails(tamBusyTask) + " " + aLangStrings[50] + " " + aLangStrings[69] + " (" + aLangStrings[82] + ")", true); // Your task may have not been built, it appeared to timeout or crash.
                addToHistory(tamBusyTask, false, aLangStrings[82]);
                tamBusyTask = 0;
            }
            ...
            if (aTasks == '') {  // no tasks are set
                ...
                if (oIntervalReference) {
                    ...
                    setVariable("TAM_TABID", 0);
                    $id("tamPanel").style.backgroundColor = "rgba(200, 200, 250, 0.8)";
                    var tamTimer = $id("tamReloadTimer");
                    if (tamTimer) tamTimer.innerHTML = '';
                }
            ...
            } else setOption('RELOAD_AT', Math.floor((oDate * 1000 + Math.round(tamRandomNumber() * 60000)) / 1000));
        ...
        function refreshTaskList(aTasks) {
            ...
            var oOldTaskList = $id("tam_tasklist");
            ...
            oTaskList.id = "tam_tasklist";
            oTaskList.innerHTML = "<div align='center' id='tam_draghandle' class='handle tam_draghandle' onmousedown='return false;'>" + aLangStrings[14] + "</div>";
            tamAddEventListener(oTaskList, "dblclick", doMinimize, false);
            //Sort links
            var currentSort = getOption("TASKLIST_SORT", 1, "integer");
            var sortLinkWrapper = document.createElement("div");

            sortLinkWrapper.id = "tam_tasklist_sortlinks";
            ...
            sortLinkChild.className = "tam_tasklist_sortlinks_header";
            sortLinkChild.innerHTML += "<span class='tam_sort_header'>" + aLangStrings[36] + "</span>";
            ...
            sortKeys.forEach(function (el) {
                var sortLinkChild = document.createElement("td");
                sortLinkChild.className = (currentSort == el) ? "tam_tasklist_sortlinks_child_active" : "tam_tasklist_sortlinks_child";
                sortLinkChild.innerHTML = aLangStrings[el];
                tamAddEventListener(sortLinkChild, "click", function (ev) {
                    orderList(el, "tam_task_row");
                    setOption("TASKLIST_SORT", el);
                    var siblings = ev.target.parentNode.childNodes;
                    for (var j = 0; j < siblings.length; ++j)
                        if (siblings[j].className != "tam_tasklist_sortlinks_header" && siblings[j].nodeName == "TD") siblings[j].className = "tam_tasklist_sortlinks_child";
                    ev.target.className = "tam_tasklist_sortlinks_child_active";
                }, false);
                sortLinkTableTr.appendChild(sortLinkChild);
                sortLinkChild = null;
            }
            ...
            makeDraggable($id('tam_draghandle'));
            ...
            for (var i = 0; i < aTasks.length; ++i) {
                ...
                if (bUseServerTime) {
                    ...
                    sTime = "<span style='cursor:pointer;' id='tam_tasktime_" + i + "' title='This is the server time. Click to add/edit new task.' tam_taskid='" + i + "' >" + sTime + "</span>";
                    ...
                    oDate = oDate.toString().split(" GMT");
                    var sTime = "<span style='cursor:pointer;float:left;margin:0px 2px 0px 0px;' id='tam_tasktime_" + i + "' title='This is your local time. Click to add/edit new task.' tam_taskid='" + i + "' >" + oDate[0] + "</span>";
                ...
                tamAddEventListener(oDeleteLink, 'click', deleteTask, false);

                var oTaskRow = document.createElement("div");
                oTaskRow.id = "tam_task_row_" + i;
                oTaskRow.className = "tam_tasklist_row";
                ...
                switch (aThisTask[0]) {
                    ...
                    case "1":  //upgrade
                        ...
                        break;
                    case "2":  //attack
                        sTaskSubject = '>> <span id="tam_placename_' + aThisTask[2] + '">' + getVillageNameZ(aThisTask[2]) + '</span>';
                ...
                oTaskRow.innerHTML = "<span class='tam_time_village_wrapper' >" + sTime + "</span><span style='float:left'>&nbsp;&mdash;&nbsp;</span>" + getVillageName(aThisTask[4]) + " : <span title='" + sTaskMoreInfo + "' style='cursor:help;' >" + sTask + " " + sTaskSubject + " </span></span>";

                oTaskRow.appendChild(oDeleteLink);
                oTaskList.appendChild(oTaskRow);
                //add listener for editing times in the task list
                var oTaskTimeSpan = $id("tam_tasktime_" + i);
                tamAddEventListener(oTaskTimeSpan, "click", editTime, false);
                oDeleteLink = null;
                oTaskRow = null;
                oDate = null;
            }
            $id('tam_draghandle').innerHTML += " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font size=1>" + timeOffsetString + "</font>";
            orderList(currentSort, "tam_task_row");
        ...
        function deleteTask(e) {
            ...
            if (bLocked) {
                _log(3, "The TAM_TASKS variables is locked. We are not able to write it.");
                printMsg("Delete Failed! TAM_TASKS is locked!", true);
                return false;
            }
            bLocked = true;
            if (isTask) var data = getVariable("TAM_TASKS");
            else var data = getVariable("TAM_HISTORY");
            if (data == '') {
            ...
            data = aTasks.join("|");
            if (isTask) {
                setVariable("TAM_TASKS", data);
                tamUpdatePanel(data);
            } else setVariable("TAM_HISTORY", data);
            bLocked = false;
        ...
        function addToHistory(aTask, bSuccess, tMsg) {
            ...
            var oldValue = tamTrimData(getVariable("TAM_HISTORY", ""), iHistoryLength - 1, true);
            ...
            _log(2, "Writing var TAM_HISTORY: " + newValue);
            if (!setVariable("TAM_HISTORY", newValue)) _log(1, "Failed logging to history.")
        ...
        function refreshHistory(aTasks) {
            ...
            // Remove old history
            var oOldHistory = $id("tam_history");
            ...
            oHistory.id = "tam_history";
            oHistory.innerHTML = "<div align='center' id='tam_history_draghandle' class='handle tam_draghandle' onmousedown='return false;'>" + aLangStrings[42] + "</div>";
            tamAddEventListener(oHistory, "dblclick", doMinimize, false);
            ...
            makeDraggable($id('tam_history_draghandle'));

            for (var i = 0; i < aTasks.length; ++i) {
                var aThisTask = aTasks[i].split(",");
                oHistory.appendChild(makeHistoryRow(aThisTask, i/*, iServerTimeOffset*/));
                var oTaskTimeSpan = $id("tam_history_tasktime_" + i);
                if (oTaskTimeSpan) { tamAddEventListener(oTaskTimeSpan, "click", editTime, false); }
            }

            orderList(38, "tam_history_row");

            //flush link
            var oFlushLink = document.createElement('div');
            var xFlushLink = document.createElement('a');
            xFlushLink.innerHTML = aLangStrings[43];
            oFlushLink.id = 'tam_flush_history';
            oFlushLink.align = 'center';
            oFlushLink.appendChild(xFlushLink);
            oHistory.appendChild(oFlushLink);
            tamAddEventListener(xFlushLink, 'click', flushHistory, false);
        }

        function makeHistoryRow(aTask, index/*, iServerTimeOffset*/) {
            _log(3, "Begin makeHistoryRow()");
            var oDate = new Date(parseInt(aTask[1]) * 1000);
            var sTime = "<span style=' cursor:pointer;' id='tam_history_tasktime_" + index + "' title='This is your local time. Click to add new task.' tam_taskid='" + index + "' >" + oDate.toLocaleString() + "</span>";

            var oHistoryRow = document.createElement("div");
            oHistoryRow.id = "tam_history_row_" + index;
            oHistoryRow.className = "tam_history_row";
            ...
            switch (aTask[0]) {
                ...
                case "1":  //upgrade
                    ...
                    break;
                case "2":  //attack
                    sTaskSubject = ' >> <span id="tam_placename_history_' + aTask[2] + '">' + getVillageNameZ(aTask[2]) + '</span>';
            ...
            var sBgColor = isError ? "rgba(100, 100, 100, 0.8);" : "rgba(100, 100, 100, 0.8);";
            oHistoryRow.style.backgroundColor = sBgColor;

            oHistoryRow.innerHTML = getVillageName(aTask[4]) + ": <span title='" + sTaskMoreInfo + "' style='cursor:help;' >" + sTask + " " + sTaskSubject + " </span><br><span class='tam_time_village_wrapper' >" + sTime + "</span>";
            oDate = null;
        ...
        function createBuildLinks() {
            ...
            for (var i = 0, j = xpBuildDesc.snapshotLength; i < j; ++i) {
                ...
                tamAddEventListener(oLink, 'click', displayTimerForm, false);
            ...
            httpRequest.onreadystatechange = function () {
                if (httpRequest.readyState == 4) { //complete
                    ...
                    if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                        ...
                        if (myBuilds.length > 0 && myContracts.length > 0 && oldVID == reqVID) {
                            ...
                            for (ii = 0, j = myBuilds.length; ii < j; ++ii) {
                                ...
                                if (tmp.length > 0) {
                                    ...
                                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + " " + aLangStrings[68] + " (" + aLangStrings[70] + " 1)", true); // Your building can't be built. because we cant find the link
                                ...
                                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + " " + aLangStrings[68] + " (" + aLangStrings[70] + " 2: " + tmp + ")", true); // Your building can't be built. Because there was no button and a reason provided.
                            ...
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + "</span>" + buildingName + " " + aLangStrings[71] + " (" + aLangStrings[72] + ")", true); // Your building can't be built. Because there was no building description. Building not found.
                        ...
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' (' + aLangStrings[74] + " " + aLangStrings[11] + " 1)", true); // Your building can't be built. Because there was no building description. Building not found.
                    ...
                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' ' + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 1)", true); // Your building can't be built. Because there was a bad response from the server.
        ...
        function handleRequestBuild(httpRequest, aTask) {
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) { // ok				
                    ...
                    if (thisNewdid == oldVID) {
                        if (httpRequest.responseText.match(re)) {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangStrings[5] + ' ' + buildingName);  //Your building is being built.
                            addToHistory(aTask, true);
                        } else {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' ' + aLangStrings[8] + " (" + aLangStrings[75] + ")", true); // Your building can't be built.
                            addToHistory(aTask, false, aLangStrings[75]);
                        }
                    } else {
                        if (httpRequest.responseText.match(re)) {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' ' + aLangStrings[76] + " (" + aLangStrings[77] + " " + getVillageName(thisNewdid) + ")", true); // Your building was probably misbuilt
                            ...
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' (' + aLangStrings[74] + " " + aLangStrings[11] + " 2)", true); // Your building can't be built. Because there was no building description. Building not found.
                ...
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + buildingName + ' ' + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 2)", true); // Your building can't be built. Because there was a bad response from the server.
        ...
        function createResearchLinks(buildingID) {
            ...
            for (var i = 0, j = tContract.length; i < j; ++i) {
                ...
                oLink.id = "tam_research_later" + i;
                oLink.className = "tam_research_later";
        ...
        function handleRequestResearch(httpRequest, aTask) {
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) {
                    ...
                    if (reqVID == oldVID && (holder.getElementsByClassName("gid22").length == 1 || holder.getElementsByClassName("gid13").length == 1)) {
                        if (currResearch.length > 0) {
                            ...
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + "</span> [" + aTask[3].split("_")[2] + "] " + aLangStrings[69] + " (" + aLangStrings[70] + " 1: " + tReason + ")", true); // Your building can't be built. Because there was no building description. Building not found.
                        ...
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + "</span> [" + aTask[3].split("_")[2] + "] " + aLangStrings[68] + " (" + aLangStrings[70] + " 2)", true); // Your building can't be built. Because there was no building description. Building not found.
                    ...
                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + "</span> [" + aTask[3].split("_")[2] + "] " + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[11] + " 1)", true); // Your building can't be built. Because there was no building description. Building not found.
                    addToHistory(aTask, false, aLangStrings[74] + " " + aLangStrings[11] + " 1");
                    return;
                }
                switchActiveVillage(currentActiveVillage);
                ...
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + "</span> [" + aTask[3].split("_")[2] + "] " + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 1)", true); // Your research did not confirm.
                addToHistory(aTask, false, aLangStrings[74] + " " + aLangStrings[46] + " 1");
            }
        }
        ...
        function handleRequestResearchConfirmation(httpRequest, aTask) {
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) {
                    ...
                    if (reqVID == oldVID && (holder.getElementsByClassName("gid22").length == 1 || holder.getElementsByClassName("gid13").length == 1)) {
                        ...
                        if (ti.length > 0) {
                            ...
                            if (ti.length == 1) {
                                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangStrings[44] + " " + aTask[3].split("_")[2]);
                                addToHistory(aTask, true);
                                return;
                            }
                        }
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span> [' + aTask[3].split("_")[2] + "] " + aLangStrings[45] + " (" + aLangStrings[75] + ")", true);
                        addToHistory(aTask, false, aLangStrings[75]);
                        return;
                    }
                    ...
                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span> [' + aTask[3].split("_")[2] + "] " + aLangStrings[73] + " " + aLangStrings[50] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[11] + " 2)", true); // Your building can't be built. Because there was no building description. Building not found.
                ...
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span> [' + aTask[3].split("_")[2] + "] " + aLangStrings[73] + " " + aLangStrings[50] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[46] + " 2)", true);
        ...
        function createPartyLinks(/*sBuildingId*/) {
            ...
            for (var i = 0; i < xpathStart.length; ++i) {
                ...
                pLink.id = "tam_research_later" + i;
                pLink.className = "tam_research_later";
                ...
                tamAddEventListener(pLink, 'click', displayTimerForm, false);
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200) {
                    ...
                    if (sResponse.match(re)) {
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(aTask[4]) + '</span>' + '' + aLangStrings[55] + aLangStrings[53]);
                        addToHistory(aTask, true);
                    } else {
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(aTask[4]) + '</span>' + aLangStrings[53] + '' + aLangStrings[54], true);
                        addToHistory(aTask, false);
                    }
                    return;
                }
        ...
        function scheduleAttack(e) {
            ...
            var iVillageId = window.location.href.match(/.*build\.php\?tt=2\&(newdid=[0-9]*&)?z=([0-9]*)/);  // target village
            if (iVillageId != null) {
        ...
        function attack(aTask) {
            ...
            if (aTask[4] != 'null') {  //multiple villages
                ...
                get("build.php?tt=2&newdid=" + aTask[4] + "&z=" + aTask[2] + "&gid=16", attack2, aTask);
            } else {  //only 1 village. Perform attack immediately
                post("build.php?tt=2&gid=16", "z=" + aTask[2], attack2, aTask);
        ...
        function attack2(httpRequest, aTask) {
            if (httpRequest.readyState == 4) {
                ...
                var oldVID = parseInt(aTask[4]);
                if (isNaN(oldVID)) oldVID = -2;
                var sPlaceName = '<span id="tam_placename_' + aTask[2] + '">' + getVillageNameZ(aTask[2]) + '</span><br>' + getTroopsInfo(aTask[3].split("_"));
                var sFromName = '<span class="tam_village_name" style="display:block;" id="tam_placename_' + oldVID + '">' + getVillageName(oldVID) + ':</span><br>';
                if (httpRequest.status == 200 && httpRequest.responseText) {
                    var holder = document.createElement('div');
                    holder.innerHTML = httpRequest.responseText;
                    ...
                    if (oldVID == reqVID && holder.getElementsByClassName("a2b").length == 1 && k > 15) {
                        ...
                        post('build.php?tt=2&gid=16', sParams, attack3, aTask);
                        return;
                    }
                    if (reqVID != currentActiveVillage) switchActiveVillage(currentActiveVillage);
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                    ...
                    if (reqVID == oldVID && holder.getElementsByClassName("a2b").length == 1 && k > 15) {
                        ...
                        post('build.php?tt=2&gid=16', sParams, handleRequestAttack, aTask);
                        return;
                    }
                    if (reqVID != currentActiveVillage) switchActiveVillage(currentActiveVillage);
        ...
        // farm from gold-club
        // writed by ww_start_t
        function createGoldClubBtn() {
            if (window.location.href.indexOf("tt=99") == -1) return;
                ...
                if (httpRequest.readyState == 4) { //complete
                    ...
                    if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                        ...
                        if (reqVID == oldVID && holder.getElementsByClassName("gid" + oldGid).length == 1) {
                            ...
                            printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(oldVID) + "</span>" + troopsInfo + " " + aLangStrings[52] + " " + aLangStrings[68] + " (" + aLangStrings[70] + ")", true);
                        ...
                        printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(oldVID) + "</span>" + troopsInfo + " " + aLangStrings[52] + " " + aLangStrings[9] + " (" + aLangStrings[74] + " " + aLangStrings[11] + " 1)", true);
                        addToHistory(aTask, false, aLangStrings[74] + " " + aLangStrings[11] + " 1");
                        return;
                    }
                    switchActiveVillage(currentActiveVillage);
                    ...
                    printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(oldVID) + "</span>" + troopsInfo + " " + aLangStrings[52] + " " + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 1)", true);
                    addToHistory(aTask, false, aLangStrings[74] + " " + aLangStrings[46] + " 1");
                }
            };
            httpRequest.send(null);
        ...
        function handleRequestTrain(httpRequest, aTask) {
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) {
                    ...
                    if (reqVID == oldVID && holder.getElementsByClassName("gid" + oldGid).length == 1 && k > 0) {
                        ...
                        if (tie.nextSibling) {
                            ...
                            if (k == parseInt(tie.className.replace("unit u", "")) && tie.parentNode.className == "desc" && buildAmount == troopsInfoN) {
                                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + ' ' + aLangStrings[51] + troopsInfo);
                                addToHistory(aTask, true);
                            } else {
                                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + troopsInfo + ' ' + aLangStrings[52] + " (" + aLangStrings[75] + " 1)", true);
                                addToHistory(aTask, false, aLangStrings[75] + " 1");
                            }
                        } else {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + troopsInfo + ' ' + aLangStrings[52] + " (" + aLangStrings[75] + " 2)", true);
                    ...
                    printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(oldVID) + "</span>" + troopsInfo + " " + aLangStrings[52] + " " + aLangStrings[50] + " " + aLangStrings[9] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[11] + " 2)", true);
                    addToHistory(aTask, false, aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[11] + " 2");
                    return;
                }
                switchActiveVillage(currentActiveVillage);
                ...
                printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(oldVID) + "</span>" + troopsInfo + " " + aLangStrings[52] + " " + aLangStrings[50] + " " + aLangStrings[73] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[46] + " 2)", true);
                addToHistory(aTask, false, aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[46] + " 2");
            }
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                    ...
                    if (tInputs.length > 3 && holder.getElementsByClassName("gid17").length == 1 && reqVID == oldVID) {
                        ...
                        if (tX) maxM = parseInt(tX.innerHTML.match(/\S+\s+(\d+)\s*?\//)[1]);
                        tX = $gc('max', holder)[0];
                    ...
                    if (reqVID != currentActiveVillage) switchActiveVillage(currentActiveVillage);
                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[11] + " 1)", true); //Your merchants didnt send. (Server: Redirected)
                ...
                switchActiveVillage(currentActiveVillage);
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 1)", true); //Your merchants didnt send. (Server: Page Failed)
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                    ...
                    if (tTime.length > 0) {
                        ...
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(aTask[4]) + '</span>' + aLangTasks[7] + " >> " + getVillageNameZ(parseInt(aTask[2])) + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br> " + aLangStrings[69] + " (" + tTime + ")", true); //Your merchants didnt send. (Server: Redirected)
                    ...
                    if (reqVID != currentActiveVillage) switchActiveVillage(currentActiveVillage);
                    printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + options[2] + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[11] + " 2)", true); //Your merchants didnt send. (Server: Redirected)
                ...
                switchActiveVillage(currentActiveVillage);
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[73] + " (" + aLangStrings[74] + " " + aLangStrings[46] + " 2)", true); //Your merchants didnt send. (Server: Page Failed)
            ...
            if (httpRequest.readyState == 4) {
                ...
                if (httpRequest.status == 200 && httpRequest.responseText) { // ok
                    ...
                    if (reqVID == oldVID && holder.getElementsByClassName("gid17").length == 1) {
                        ...
                        if (ii == jj + 1) {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3])); //Your merchants were sent.
                            _log(1, "Your merchants were sent. From: " + getVillageName(oldVID) + "   To: " + oldName + " " + oldCoords + " carrying " + getMerchantInfo(aTask[3]));
                            addToHistory(aTask, true);
                        } else {
                            printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[50] + " (" + aLangStrings[75] + ")", true); //Your merchants didnt send. Confirmation failed
                        ...
                        printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[73] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[11] + " 3)", true); //Your merchants didnt send. (Server: Redirected)
                ...
                switchActiveVillage(currentActiveVillage);
                printMsg('<span class="tam_village_name" style="display:block;">' + getVillageName(oldVID) + '</span>' + aLangTasks[7] + " >> " + oldName + " " + oldCoords + "<br>" + getMerchantInfo(aTask[3]) + "<br>" + aLangStrings[50] + " " + aLangStrings[73] + " (" + aLangStrings[75] + ", " + aLangStrings[74] + " " + aLangStrings[46] + " 3)", true); //Your merchants didnt send. (Server: Page Failed)
        ...
        function displayTimerForm(iTask, target, options, timestamp, taskindex, villagedid) {
            ...
            var sWhat = '';
            var sMoreInfo = '';
            var sWho = '<b><span id="tam_placename_' + iVillageId + '">' + getVillageName(iVillageId) + '</span>:</b> ';
            ...
            switch (iTask) {
                ...
                case 2:  //Attack, Raid, Support
                    ...
                    sWhat = (bLetsSpy ? aLangStrings[47] : aLangStrings[iAttackType]) + ' >> <span id="tam_placename_' + target + '">' + getVillageNameZ(target) + '</span>';
            ...
            var sLinkClose = "<img src='" + sCloseBtn + "' alt='[" + aLangStrings[56] + "]' title='" + aLangStrings[56] + "' id='tam_close_btn' class='tam_close_btn' onclick='document.body.removeChild(document.getElementById(\"timerform_wrapper\"));' />";
	...
	+ '</option></select><br /><span id="timerMoreInfo" style="font-size:85%;color:red; cursor:default;">' + sMoreInfo + '</span><br />';
            ...
            else sSubmitButtonLabel = "موافق";
            var oSubmitBtn = $e("input", [['name', "submitBtn"], ['id', "submitBtn"], ['value', sSubmitButtonLabel], ['type', "submit"]]);
            tamAddEventListener(oSubmitBtn, 'click', function () { handleTimerForm(this.form, 1, taskindex, iVillageId) }, true);
            ...
            if (typeof (taskindex) != 'undefined') {
                ...
                tamAddEventListener(oAddCloseBtn, 'click', function () { handleTimerForm(this.form, 2, taskindex, iVillageId) }, true);
                oTimerForm.appendChild(oAddCloseBtn);

                var oAddBtn = $e("input", [['name', "AddBtn"], ['value', aLangStrings[60]], ['type', "button"]]);
                tamAddEventListener(oAddBtn, 'click', function () { handleTimerForm(this.form, 3, taskindex, iVillageId) }, true);
        ...
        function setTask(iTask, iWhen, target, options, taskindex, villagedid) {
            var iVillageId = typeof (villagedid) != 'undefined' ? villagedid : currentActiveVillage;
            if (bLocked) {
                _log(3, "The TAM_TASKS variables is locked. We are not able to write it. The Task could not be scheduled.");
                printMsg("<span class='tam_village_name' style='display:block;'>" + getVillageName(iVillageId) + "</span>" + aLangStrings[12], true);
                return false;
            }

            bLocked = true;
            var data = getVariable("TAM_TASKS");
            var aTasks = data.split("|");
            ...
            switch (iTask) {
                ...
                case "1":  //upgrade
                    ...
                    break;
                case "2":  //attack
                    sTaskSubject = ' >> <span id="tam_placename_' + target + '">' + getVillageNameZ(target) + '</span>';
            ...
            if (!oIntervalReference) {
                oIntervalReference = window.setInterval(checkSetTasks, CTE * 1000);  //start checking if there is any task to trigger
        ...
        // *** Begin Drag n Drop (move TAM windows) Block ***
        ...
        function mouseDown(ev) {
            var mousePos = mouseCoords(ev);
            var target = ev.target;
            if (target.id == 'tam_close_btn') { dragObject = null; return false; }
            iMouseDown = true;
        ...
        function makeDraggable(item, noParent) {
            if (!item) return;
            tamAddEventListener(item, "mousedown", function (ev) {
                dragObject = (noParent ? this : this.parentNode);
                mouseOffset = getMouseOffset((noParent ? this : this.parentNode), ev);
                return false;
            }, false);
        }
        // *** End Drag n Drop (move TAM windows) Block ***

        // *** Begin TAM "get" Functions  Block ***
        function getSiteId() {
            ...
            if (xpathVillage.snapshotLength > 0 || href.indexOf("nachrichten") > -1 || href.indexOf("berichte") > -1 || href.indexOf("hero") > -1 || href.indexOf("details") > -1 || href.indexOf("build.php?tt=2") > -1 || href.indexOf("karte") > -1 || href.indexOf("statistiken") > -1) {
        ...
        function getVillageNameXY(iVillageX, iVillageY) {
            ...
            var nV, tStr1 = "<span class ='tam_village_name' title='" + aLangStrings[81] + "' onclick='window.location = \"position_details.php?x=" + iVillageX + "&y=" + iVillageY + "\";return false;' >";
        ...
        function getMerchantInfo(aMerchants, isLong) {
            ...
            if (isLong) {
                sMerchantInfo = aLangResources[0] + ":" + aMerchants[2] + ", " + aLangResources[1] + ":" + aMerchants[3] + ", " + aLangResources[2] + ":" + aMerchants[4] + ", " + aLangResources[3] + ":" + aMerchants[5];
            } else {
        ...
        function generateButton(title, callback) {
            ...
            tamAddEventListener(oBtn, "click", callback, false);
        ...
        function printMsg(sMsg, bError) {
            ...
            _log(1, sWhen + sMsg);
            var oOldMessage = $id("tam_message");
            if (oOldMessage) {
            ...
            var sLinkClose = "<img src='" + sCloseBtn + "' alt='[" + aLangStrings[56] + "]' title='" + aLangStrings[56] + "' id='tam_close_btn' class='tam_close_btn' onclick='document.body.removeChild(document.getElementById(\"tam_message\"));' />";
            var sBgColor = (bError) ? "rgba(100, 100, 100, 0.8)" : "rgba(100, 100, 100, 0.8);";
            var oMsgBox = document.createElement("div");
            oMsgBox.innerHTML = "<div id='tam_draghandle_msg' class='handle'>" + sLinkClose + sMsg + "</div>";
            oMsgBox.style.backgroundColor = 'rgba(255, 255, 255, 0.8)';
            ...
            oMsgBox.id = "tam_message";
            document.body.appendChild(oMsgBox);
            makeDraggable($id('tam_draghandle_msg'));
            _log(3, "End printMsg()");
        }

        function tamRandomNumber() {
            var aleat = Math.random() * (MAXRM - MINRM);
            aleat = Math.round(aleat);
            return parseInt(MINRM) + aleat;
        }

        function tamAddEventListener(obj, str, handler, boole) {
            if (obj && str && handler) {
        ...
        function tamAniShadePanelYellowToRed(tNode) {  //This all assumes that CTE is set to the default 10 seconds.
            var tGreen = 255;
            var tBlue = 255;
            return function () {
        ...
        function tamAniShadePanelGreen(tNode) {  //This all assumes that CTE is set to the default 10 seconds.
            var tRed = 255;
            var tBlue = 255;
        ...
        function tamUpdatePanel(aTasks, tTime) {
            if (oAnimateTimerIR) {
            ...
            if (tamTimer) {
                ...
                tamTimer.innerHTML = vName;
                if (!isTroopsLoaded || !isTAMLoaded) return; // Its grey while something (getTroopNames) is running, so we let it stay grey
                tamTimer = tamTimer.parentNode.parentNode;
                if (!aTasks) aTasks = getVariable("TAM_TASKS", "");
                if (!tTime) tTime = tA;
                tX = 0;
                tY = 0;
                if (aTasks != "") {
                    ...
                    for (tY = tA.length; tX < tY; ++tX) {
                        ...
                        if (vName <= tTime + 60) {
                            if (vName <= tTime + CTE) {
                                if (vName <= tTime) {
                                    tamTimer.style.backgroundColor = "#EE8787";
                                } else {
                                    tamTimer.style.backgroundColor = "#FFFF00";
                                    oAnimateTimerIR = window.setInterval(tamAniShadePanelYellowToRed(tamTimer), CTE * 99);
                                }
                            } else {
                                tamTimer.style.backgroundColor = "#BBFFBB";
                                oAnimateTimerIR = window.setInterval(tamAniShadePanelGreen(tamTimer), CTE * 99);
                            }
                            tX = tY;
                        }
                    }
                }
                if (tX != tY + 1) tamTimer.style.backgroundColor = "#FFFFFF";
            }
        }
        ...
        function promptReset() {
            if (confirm(aLangMenuOptions[0] + "\n" + aLangMenuOptions[8])) {
                // rewritten by ww_start_t
                unLoad();
                var allkey = ['OTHER_PLACE_NAMES', 'TROOP_NAMES', 'TAM_HISTORY', 'TAM_OPTIONS', 'TAM_TABID', 'TAM_TASKS', 'vlist'];
                for (var i = 0; i < allkey.length; i++) allkey[i] = CS + myPlayerID + "_" + allkey[i];
                allkey.push(CS + 'login', CS + 'lang');
                for (i = 0; i < allkey.length; i++) GM_deleteValue(allkey[i]);
                var tamPanel = $id("tamPanel");
                if (tamPanel) {
                    tamPanel.style.backgroundColor = "rgba(200, 200, 250, 0.8)";
                    tamPanel.innerHTML = 'TAM - goodbye';
                }
                allkey = ["tam_message", "timerform_wrapper", "tam_history", "tam_tasklist"];
        ...
        function promptLang() {
            ...
            while (true) {
                ...
                if (!aLang) return;
                if (aLang == 0) {
                    GM_deleteValue(CS + 'lang');
                    break;
                } else {
                    if (("," + allLangs.join(',') + ",").indexOf("," + aLang + ",") != -1) {
                        GM_setValue(CS + 'lang', aLang);
                        break;
                    }
                }
            }
        ...
        function onLoad() {
            ...
            TAM_registerMenuCommand(aLangMenuOptions[3], promptRace);
            TAM_registerMenuCommand(aLangMenuOptions[4], promptHistory);
            TAM_registerMenuCommand(aLangMenuOptions[5], promptReset);
            TAM_registerMenuCommand("اللغة", promptLang);
            ...
            if (document.getElementById('marketSend')) createMarketLinks();
            /* قيد التطوير
            tA = /build\.php\b[^>]*tt\=2/i;
            if (tA.test(window.location.href)) createAttackLinks();
            */
            vName = getVariable("TAM_TASKS");
            if (vName != '') refreshTaskList(vName.split("|"));

            tA = getVariable("TAM_HISTORY");
            if (iHistoryLength > 0 && tA != '') refreshHistory(tamTrimData(tA, iHistoryLength, false));
            ...
            isTAMLoaded = true;
            tamUpdatePanel(vName, Math.floor(tX / 1000));
            _log(1, "End onLoad()");
        }
        ...
        function doMinimize(evt) {
            ...
            if (tD != null) {
                switch (tD.id) {
                    case "tam_tasklist": isMinimized = !isMinimized;
                        isMin = isMinimized;
                        setOption("LIST_MINIMIZED", isMinimized);
                        break;
                    case "tam_history": isHistoryMinimized = !isHistoryMinimized;
        ...
        if (init) {
            tamAddEventListener(document, "mousemove", mouseMove, false);
            tamAddEventListener(document, "mousedown", mouseDown, false);
            tamAddEventListener(document, "mouseup", mouseUp, false);
            //	tamAddEventListener ( window,   "load",      onLoad,    false );
            tamAddEventListener(window, "unload", unLoad, false);
            var inittime = new Date();
            _log(0, "TAM starting... (" + inittime + ")");

            inittime = inittime.getTime();
            var tmp = Math.round(tamRandomNumber() * 60000);
            setOption('RELOAD_AT', Math.floor((tmp + inittime) / 1000));
            inittime -= starttime;
            _log(1, "CheckSetTasks> Begin. (tab ID = " + myID + " / " + getVariable("TAM_TABID", 0) + ")");
            //	checkSetTasks();
            if (!oIntervalReference) {
                _log(3, "setInterval()");
                oIntervalReference = window.setInterval(checkSetTasks, CTE * 1000);
            }
            onLoad();
        } else {
            var oLogout = xpath("//div[@class='logout']");
            if (oLogout.snapshotLength > 0) GM_setValue(CS + 'login', '0');
            ...
            if (oLoginBtn.snapshotLength < 1 && (oLogout.snapshotLength > 0 || oSysMsg.snapshotLength > 0)) {
                _log(0, "Error screen or something. Game is not loaded. Did not start TAM.");
                ...
                // writed by ww_start_t
                var logPas = GM_getValue(CS + 'login', '0');
                if (logPas != '0') {
                    logPas = logPas.split('/');
                    ...
                    oLoginBtn.snapshotItem(0).addEventListener('click', function () {
                        if (oLogin.value.length > 0 && oPassword.value.length > 0)
                            GM_setValue(CS + 'login', oLogin.value + '/' + oPassword.value);
                    }, false);
                }
                if (loginFL) setTimeout("document.getElementById('s1').click();", Math.round(tamRandomNumber() * 111)); // 333 - roughly 1.6 to 3.3 with default random min/max settings
                else _log(0, "Auto-Login failed. You must have FireFox store the username and password. TAM does not.");
            } else {
    ...
    var notRunYet = true;
    if (/Chrome/.test(navigator.appVersion)) allInOneTAM();
    else if (window.addEventListener) window.addEventListener("load", function () { if (notRunYet) allInOneTAM(); }, false);
