// ==UserScript==
// @name           Combat Report Auto-Converter Modificado BETA
// @autor          WTF
...
//@history	2.4.5 Se agregan nuevos barcos de guerra para la version 0.4.5 de Ikariam
//@history	2.4.4 BETA Se añadió un boton para ingresar los datos del reporte de manera Manual.
//@history	2.4.3 El boton de postear batalla ahora tambien funciona para los servidores de Chile, Colombia y Mexico.


// ==/UserScript==

const VERSION = '2.4.5 BETA';
const scriptlink = 'http://userscripts.org/scripts/show/121292'
...
ar: { ships: "Barcos", cargo: "Barcos mercantes", ram: "Barco-espolón", ballista: "Barco-ballesta", flame: "Barco-lanza llamas", catapult: "Barco-catapulta", paddle: "Barco-espolón a vapor", mortar: "Barco-mortero", diving: "Submarino", rocketship: "Barco lanzamisiles", paddlespeedship: "Lancha de palas", ballooncarrier: "Portaglobos", tender: "Barco de mantenimiento"  },
...
es: { ships: "Barcos", cargo: "Barcos mercantes", ram: "Barco-espolón", ballista: "Barco-ballesta", flame: "Barco-lanza llamas", catapult: "Barco-catapulta", paddle: "Barco-espolón a vapor", mortar: "Barco-mortero", diving: "Submarino", rocketship: "Barco lanzamisiles", paddlespeedship: "Lancha de palas", ballooncarrier: "Portaglobos", tender: "Barco de mantenimiento"  },
...
mx: { ships: "Barcos", cargo: "Barcos mercantes", ram: "Barco-espolón", ballista: "Barco-ballesta", flame: "Barco-lanza llamas", catapult: "Barco-catapulta", paddle: "Barco-espolón a vapor", mortar: "Barco-mortero", diving: "Submarino", rocketship: "Barco lanzamisiles", paddlespeedship: "Lancha de palas", ballooncarrier: "Portaglobos", tender: "Barco de mantenimiento"  },
...
ve: { ships: "Barcos", cargo: "Barcos mercantes", ram: "Barco-espolón", ballista: "Barco-ballesta", flame: "Barco-lanza llamas", catapult: "Barco-catapulta", paddle: "Barco-espolón a vapor", mortar: "Barco-mortero", diving: "Submarino", rocketship: "Barco lanzamisiles", paddlespeedship: "Lancha de palas", ballooncarrier: "Portaglobos", tender: "Barco de mantenimiento"  },
...
const S_MAP = {
	name: { cargo: "fleet s201", ram: "fleet s210", ballista: "fleet s213", flame: "fleet s211", catapult: "fleet s214", paddle: "fleet s216", mortar: "fleet s215", diving: "fleet s212", rocketship: "fleet s217", paddlespeedship: "fleet s218", ballooncarrier: "fleet s219", tender: "fleet s220" },
	clas: { "fleet s201": "cargo", "fleet s210": "ram", "fleet s213": "ballista", "fleet s211": "flame", "fleet s214": "catapult", "fleet s216": "paddle", "fleet s215": "mortar", "fleet s212": "diving", "fleet s217": "rocketship", "fleet s218": "paddlespeedship", "fleet s219": "ballooncarrier", "fleet s220": "tender" }
};
...
function ManualParseReport() {
	
	if (confirm('Para batalla terrestre, presione ACEPTAR\nPara batalla maritima, presione CANCELAR') == 1 ) {
		L_clas = L_UNITS;
		clasType = U_MAP.clas;
	}
	else {
	...
	alert('INSTRUCCIONES:\n- Solo se aceptan la entrada de números\n- Si se insertan letras o se deja el espacio en blanco, la unidad en cuestión será totalmente descartada del reporte\n- Si presionan ESC o CANCELAR saldran total o parcialmente del proceso\n\n A continuacion se le pedirá que ingrese el reporte del lado ATACANTE');
	$.each(clasType, function() {
		unitName = this;
		if (unitName != null && unitName != 'cargo' && unitName != 'barbarian') {
			unitsleft = prompt('NOTA: para descartar esta unidad/barco deje el espacio en blanco\n\n' + L_clas[unitName] + ' vivos =');
			if (unitsleft == null) {salir = 1; return false}
			unitslost = prompt('NOTA: para descartar esta unidad/barco deje el espacio en blanco\n\n' + L_clas[unitName] + ' muertos =');
			if (unitslost == null) {salir = 1; return false}
			//alert(unitsleft + ' y ' + unitslost + ' y ' + unitName);
			unitsleft = parseInt(unitsleft);
			unitslost = parseInt(unitslost);
			if ((unitsleft >= 0 && unitslost >= 0) && (unitsleft != 0 || unitslost != 0))  {
				//alert('1 - ' + IKCRC_REPORT.battle.attackers[unitName])
				IKCRC_REPORT.battle.attackers[unitName] = {left:0, lost:0};
				IKCRC_REPORT.battle.attackers[unitName].left += unitsleft;
				IKCRC_REPORT.battle.attackers[unitName].lost += unitslost;
			}
			else {
			//alert('2 - ' + IKCRC_REPORT.battle.attackers[unitName])
			IKCRC_REPORT.battle.attackers[unitName]= null;
			}
		}
	});
	if (salir == 1) {return false}
	alert('A continuacion se le pedirá que ingrese el reporte del lado DEFENSOR')
	$.each(clasType, function() {
		unitName = this;
		if (unitName != null && unitName != 'cargo' && unitName != 'barbarian') {
			unitsleft = prompt('NOTA: para descartar esta unidad/barco deje el espacio en blanco\n\n' + L_clas[unitName] + ' vivos =');
			if (unitsleft == null) {return false}
			unitslost = prompt('NOTA: para descartar esta unidad/barco deje el espacio en blanco\n\n' + L_clas[unitName] + ' muertos =');
			if (unitslost == null) {return false}
			//alert(unitsleft + ' y ' + unitslost);
			unitsleft = parseInt(unitsleft);
			unitslost = parseInt(unitslost);		
			if ((unitsleft >= 0 && unitslost >= 0) && (unitsleft != 0 || unitslost != 0))  {
				//alert('1 - ' + IKCRC_REPORT.battle.attackers[unitName])
				IKCRC_REPORT.battle.defenders[unitName] = {left:0, lost:0};
				IKCRC_REPORT.battle.defenders[unitName].left += unitsleft;
				IKCRC_REPORT.battle.defenders[unitName].lost += unitslost;
			}
			else {
			//alert('2 - ' + IKCRC_REPORT.battle.attackers[unitName])
			IKCRC_REPORT.battle.defenders[unitName]= null;
			}
		}
	});

	$('div.contentBox01h:eq(2)').remove()
	$('div.contentBox01h:eq(1)').remove()
	BuildReport();
}
...
var allUnitCitizen = IKCRC_REPORT.info.type == "army"
		...
		var allUnits = IKCRC_REPORT.info.type == "army"
			...
			: ["ram", "ballista", "flame", "catapult", "paddle", "mortar", "diving", "rocketship", "paddlespeedship", "ballooncarrier", "tender"];
...
if(deffenders.length>1) {results += 'es'; }
	...
	report += '<br/>[b][i]Compactado con: [url=' + scriptlink + ']Combat Report Auto-Converter[/url] v' + VERSION + ' © by [url=http://board.ikariam.es/index.php?page=User&userID=47984]WTF[/url], Modificado por [url=http://board.es.ikariam.com/index.php?page=User&userID=10651]fom78[/url][/i][/b]';
...
if(GM_getValue('forumclass')=='plain') {
	...
	final_text = final_text.replace('[img]http://'+server+'/skin/resources/icon_sulfur.gif[/img]','Azufre:');
	final_text = final_text.replace('[img]http://'+server+'/skin/resources/icon_citizen.gif[/img]','Burgueses:');
	final_text = final_text.replace('[b][i]Compactado con: [url=' + scriptlink + ']Combat Report Auto-Converter[/url] v1.1 © by [url=http://board.ikariam.es/index.php?page=User&userID=47984]WTF[/url][/i][/b]','Compactado con: Combat Report Auto-Converter v1.1 © by WTF');
...
var new_mainview =  '<div class="footer"></div></div><div class="contentBox01h"><h3 class="header"><a href="' + scriptlink + '" target="_blank">Combat Report Auto-Converter</a> v' + VERSION + ' © by <a href="http://board.ikariam.es/index.php?page=User&userID=47984" target="_blank">WTF</a>, Modificado por <a href="http://board.es.ikariam.com/index.php?page=User&userID=10651" target="_blank">fom78</a></b></h3><div class="content"><table style="border:none;"><tr><td style="border:none; height:8px;"></td></tr><tr><td style="vertical-align:middle; text-align:center; border:none;">Doble click sobre la parte que quieras y ya estará copiada en tu portapapeles! (luego pegala con CTRL+V)';
...
new_mainview += '</td></tr><tr style="border:none;"><td style="border:none;"><table style="border:none;"><tr style="border:none;"><td style="width:20px; border:none;"></td><td style="vertical-align:middle; border:none;"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="121" height="27" id="copysubject">  <param name="movie" value="http://madacol.atspace.co.uk/copysubject.swf?textocopy=' + final_subject + '"><param name="quality" value="high"><param name="wmode" value="transparent"><param name="swfversion" value="6.0.65.0"><!-- La siguiente etiqueta object es para navegadores distintos de IE. Ocúltela a IE mediante IECC. --><!--[if !IE]>--><object type="application/x-shockwave-flash" data="http://madacol.atspace.co.uk/copysubject.swf?textocopy=' + final_subject + '" width="121" height="27"><!--<![endif]--><param name="quality" value="high"><param name="wmode" value="transparent"><!--[if !IE]>--></object><!--<![endif]--></object></td><td style="width:20px; border:none;"></td><td style="vertical-align:middle; border:none;" id="copytext_td"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="205" height="27" id="copytext">  <param name="movie" value="http://madacol.atspace.co.uk/copytext.swf?textocopy=' + final_text + '"><param name="quality" value="high"><param name="wmode" value="transparent"><param name="swfversion" value="6.0.65.0"><!-- La siguiente etiqueta object es para navegadores distintos de IE. Ocúltela a IE mediante IECC. --><!--[if !IE]>--><object type="application/x-shockwave-flash" data="http://madacol.atspace.co.uk/copytext.swf?textocopy=' + final_text + '" width="205" height="27"><!--<![endif]--><param name="quality" value="high"><param name="wmode" value="transparent"><!--[if !IE]>--></object><!--<![endif]--></object></td><td style="width:20px; border:none;"></td><td style="vertical-align:middle; border:none;">';
...
new_mainview += '<button class="button" id="CRAC_ManualPost">Generar reporte Manualmente</button>';
...
var addonclick4 = document.getElementById('CRAC_ManualPost');
addonclick4.addEventListener("click", ManualParseReport, true);
