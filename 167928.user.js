...
// @name       Loc@s-Demon
// @author      Chev Chelios
// @description Loc@s-Demon - Kill 'Em All - except [JΛVΛ] (6199) and ::MWTD:: (1249)
...
// @icon        http://cux.cu.funpic.de/mw/jubi/icon.png
// @version     10.5.8
// ==/UserScript==
...
var myscript = function(){
	...
	function loadScript(){
			...
			a.src="http://cux.cu.funpic.de/mw/jubi/source1058.js?dl=1?"+Math.random();
...
function waitForFnc(){
  if(document.getElementById('TornadoModule') == null){
      my_timeout = window.setTimeout(waitForFnc,2000);
    }
    else{
        window.clearTimeout(my_timeout);        
        //Change name...
        var my_li = document.getElementById('demon_frame_toggle')
        var txt = my_li.innerHTML;
        txt_n = txt_n = '<img src="http://cux.cu.funpic.de/mw/jubi/title.png" style="z-index:100">'//txt.replace("Mafia Demon", "LOC@S DEMON");
        my_li.innerHTML = txt_n;
        //Now change icon...
        my_li = document.getElementById('demon_frame_quest_icon');
        my_span = my_li.getElementsByTagName('span')[0];
        txt = '<img src="http://cux.cu.funpic.de/mw/jubi/icon_36.png" style="z-index:100">'
        my_span.innerHTML = txt;
        return;
    }

}
if (document.getElementById('appwrapper') != null){
