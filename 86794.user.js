...
// @name           Funny OGame
// @namespace      Gollum
// @description    Membre principal de la suite « Funny OGame » (V. 2.02).
...
// ==/UserScript==
	...
	if (tps) {
		...
		URL_S = url.match(/session=([0-9a-z]{12})/)[1];
	} else {
	...
	Galaxie = function (G, S, P, Sup) {return URL_D+'/game/index.php?page=galaxy&amp;session='+URL_S+'&amp;galaxy='+G+'&amp;system='+S+'&amp;position='+P+(Sup ? Sup.replace(/&/, '&amp;') : '');}
	Mission = function (G, S, P, T, M, Sup) {return URL_D+'/game/index.php?page=fleet1&session='+URL_S+'&galaxy='+G+'&system='+S+'&position='+P+'&type='+T+'&mission='+M+(Sup ? Sup.replace(/&/, '&') : '');}
...
/* Options */
	...
	uw.Menu_mise_a_jour = function (maj) {
		...
		if (maj) {
			...
			Code += '<a href="http://userscripts.org/scripts/source/86794.user.js">Mettre à jour maintenant !</a>';
			Code += '<br /><br /><br />';
		}
		Code += '<h5>Options - Funny OGame</h5>';
	...
	function Mise_a_jour(Menu) {
		GM_xmlhttpRequest({
				method: "GET",
				url: 'http://userscripts.org/scripts/show/86794',
				onload: function(responseDetails) {
					var NV = parseFloat(responseDetails.responseText.match(/\(V\. ([0-9]*\.[0-9]{1,2})\)/)[1]);
					if (NV > Version) {if (Menu) {uw.Menu_mise_a_jour(true);} else {Ajout_bouton(true);}} else {if (Menu) {uw.Menu_mise_a_jour(false);} else {GM_setValue('Anci', Date.toString()); Ajout_bouton(false);}}
				}
		});
	}
	...
	if (URL_P == 'overview') {
		...
		if (Tps == '' || Tps == '({})') {
			addElm(id_('boxBG'), 'iframe', {src:'index.php?page=shipyard&session='+URL_S, style:'display:none !important;'}, '');
			setTimeout(function () {addElm(id_('boxBG'), 'iframe', {src:'index.php?page=defense&session='+URL_S, style:'display:none !important;'}, '');}, 800);
			setTimeout(function () {addElm(id_('boxBG'), 'iframe', {src:'index.php?page=research&session='+URL_S, style:'display:none !important;'}, '');}, 1600);
		}
	}
...
/*** Funny Galaxy ***/
	...
	uw.PM_Joueur = function (url, type) {
		window.setTimeout(function() {
			...
			url: url,
				onload: function(responseDetails) {
					Lien_PM = responseDetails.responseText.match(/index\.php\?page=galaxy&galaxy=[0-9]{1,2}&system=[0-9]{1,3}&position=[0-9]{1,2}&session=[a-z0-9]{12}/);
					...
					if (Lien_PM && type == 'PM') {location.href = Lien_PM;} if (Infos && type == 'Joueur') {Info_Joueur(Infos);}
				}
			});
		}, 0);
	};
	Info_Joueur = function (Infos) {
		name = Infos[1]; Coor = [Infos[2], Infos[3], Infos[4]];
		window.setTimeout(function() {
			if (name) { IMG = '<img src="'+URL_D+'/game/img/galaxy/ajax_indicator.gif" alt="Chargement..." id="Chargement">';
				if (!id_('WR')) {Table = addElm(id_('links'), 'table', {id:'WR'}, IMG);} else {Table = id_('WR'); Table.innerHTML = IMG;}
				var Link = 'http://www.war-riders.de/'+URL_N+'/'+Uni()+'/details/player/'+name.replace(/ /g, '%20');
				//var Link = 'http://www.war-riders.de/?lang='+URL_N+'&uni='+Uni()+'&page=search&type=player&name='+name.replace(/ /g, '%20')+'&submit=Rechercher&post=1';
				GM_xmlhttpRequest({
					method: "GET",
					url: Link,
	...
	function Game_Stat(Code, Name, Link, session, Coor) {
		if (!IndexOf(Code, '<font class="red">No results</font>') && Code.match(/<tr><td class=s>([^<]*)<\/td>/)) {
			...
			Tableau_Classement = '<b id="title"><u>Classements de '+Name+'&nbsp;:</u></b><table style=width:100%;><tr><th style=width:50%;>Points&nbsp;:</th><th style=width:50%;>'+Classement_1+'&nbsp;('+Sep(Points_1)+')</th></tr> <tr><th>Flotte&nbsp;:</th><th>'+Classement_2+'&nbsp;('+Sep(Points_2)+')</th></tr> <tr><th>Recherches&nbsp;:</th><th>'+Classement_3+'&nbsp;('+Sep(Points_3)+')</th></tr><tr><td colspan="2" id="click"><i>(Cliquez pour accèder à la page de War-Rider.)</i></td></tr> </table>';
			Object = '<object'+GFT(Code, '<object', '</object>')+'</object>';
			
			Code_1 = '<tr><td colspan="3"><a href="'+Link+'" target="_blank" id="pop_up_"><b>'+Name+'</b></a></td></tr>';
			...
			Code = Code.replace(/([1-5]?[1-9]):([1-4]?[0-9]?[0-9]):(1[0-6]|[0-9])/g, '<a onclick="Surlignement(\'$1:$2:$3\');canLoadContent($1, $2);" class="planete_link" target="_parent">[$1:$2:$3]</a>');
			...
			id_('WR').setAttribute('class', '');
			id_('WR').innerHTML = Code;
	...
	function Galaxie_Auto(R) {
		...
		var table = id_('galaxytable'); if (table) {
			if (table.getAttribute('funny_galaxy') == 'done') {setTimeout(function() {Galaxie_Auto(true);}, 500); return;}
			...
			if (Options.fg.pm || Options.fg.info || Options.fg.espio) {
				...
				for (i = 0; i < Div.length; i++) {
					...
					if (IndexOf(Div[i].innerHTML, 'writemessage') && (Options.fg.pm || Options.fg.info)) {
						...
						if (Player_name && Options.fg.pm) {addElm(addElm(Div[i].getElementsByTagName('ul')[0], 'li', {}, ''), 'a', {href:'#',onclick:'PM_Joueur("'+tag_('a', Div[i])[0].href+'", "PM");',id:'lien_'+i}, 'Planète mère de '+Player_name[1]);}
						if (Player_name && Options.fg.info) {addElm(addElm(Div[i].getElementsByTagName('ul')[0], 'li', {}, ''), 'a', {href:'#',onclick:'PM_Joueur("'+tag_('a', Div[i])[0].href+'", "Joueur");'}, 'Autres informations sur '+Player_name[1]);}
					}
					// Lien pour Espionner... :
					if (Options.fg.espio) {
			...
			for (P = 1; P <= 15; P++) {
				...
				if (Options.fg.cdd) {
					...
					Position.innerHTML = '<a class="cdd'+((C[1] == G && C[2] == S && C[3] == P) ? 'p' : '')+'" href="'+URL_D+'/game/index.php?page=fleet1&session='+URL_S+'&galaxy='+G+'&system='+S+'&position='+P+'&type=2&mission=8" title="Recycler en ['+G+':'+S+':'+P+']">'+P+'</a>';
				}
				...
				if (class_('planetname', Row)[0]) {
					...
					if (id_('debris'+P) && !CC && Options.fg.cc) {
						...
						Debris.setAttribute('href', URL_D+'/game/index.php?page=fleet1&session='+URL_S+'&galaxy='+G+'&system='+S+'&position='+P+'&type=2&mission=8&REC='+STI(S_(tag_('li', id_('debris'+P)), -2).innerHTML));
					}
	...
	if (URL_P == 'galaxy') {
		...
		PRank = tag_('li', id_('bar'))[2].innerHTML; //alert(PRank);
		if (PRank.match(/\(([0-9]*)\)/)) {PRank = PRank.match(/\(([0-9]*)\)/)[1]} else {PRank = 350;} //alert(PRank);
		Pseudo = class_('textBeefy', id_('playerName'))[0].innerHTML; //alert(Pseudo);
		...
		Img.setAttribute('onclick', 'location.href = "'+URL_D+'/game/index.php?page=messages&session='+URL_S+'";');
	...
	if (URL_P == 'phalanx' && Options.fg.phalange) {
		Coor = url.match(/galaxy=([1-9]*)&system=([1-9]*)&position=([1-9]*)/); D = new Date;
		document.title = 'Phalange en [' + Coor[1] + ':' + Coor[2] + ':' + Coor[3] + '] à ' + D.getHours() + 'h ' + D.getMinutes();
	}
	
	if (URL_P == 'eventList') {
		//Cell = id_('eventContent');
		//if (Cell) {Cell.innerHTML = Cell.innerHTML.replace(/galaxy=[0-9]{1,2}&amp;system=[0-9]{1,3}" target="_top">\[([0-9]{1,2})\:([0-9]{1,3})\:([0-9]{1,2})\]/g, 'galaxy=$1&amp;system=$2&amp;planet=$3" target="_top">[$1:$2:$3]');}
	}
	
	if (URL_P == 'movement') {
		//Cell = id_('inhalt');
		//Cell.innerHTML = Cell.innerHTML.replace(/galaxy=[0-9]{1,2}&amp;system=[0-9]{1,3}">\[([0-9]{1,2})\:([0-9]{1,3})\:([0-9]{1,2})\]/g, 'galaxy=$1&amp;system=$2&amp;planet=$3">[$1:$2:$3]');
	}
...
/*** Funny Report ***/
	...
	function shipCount (m, c, d, cap) {
		return Math.ceil ((Math.ceil (Math.max (m + c + d, Math.min (0.75 * (m * 2 + c + d), m * 2 + d))) / 2) / cap);
	}
	...
	function SpeedSim(Code) {return Code.replace(/ /g, '%20').replace(/\/n/g, '%20').replace(/\ /g, '%20');}//.replace(/\/n/g, '%0D%0A');}
	...
	if (URL_P == 'showmessage' && Find('//table[contains(@class,"material")]')) {
		...
		var Matos_org = Find('//table[contains(@class,"material")]');
			...
			Player_name = Matos_txt.match(/ '([^']{1,20})'/)[1];
		...
		// Calculs :
		var Pillage = Math.floor(Ress.M.r/2) + Math.floor(Ress.C.r/2) + Math.floor(Ress.D.r/2);
		var PT = shipCount(Ress.M.r, Ress.C.r, Ress.D.r, 5000);
		var GT = shipCount(Ress.M.r, Ress.C.r, Ress.D.r, 25000);
		...
		var Matos_org = Find('//table[contains(@class,"material")]');
			var Matos_txt = Matos_org.getElementsByClassName('area')[0].innerHTML;
				...
				addElm(Table_info, 'tr', {}, '').innerHTML =  '<th colspan="4" class="area" style="text-align:left;">Ressources à piller :</th>';
				addElm(Table_info, 'tr', {}, '').innerHTML =  '<td>'+Ress.M.t+'</td><td>'+Sep(Math.floor(Ress.M.r/2))+'</td><td>'+Ress.C.t+'</td><td>'+Sep(Math.floor(Ress.C.r/2))+'</td>';
				addElm(Table_info, 'tr', {}, '').innerHTML =  '<td>'+Ress.D.t+'</td><td>'+Sep(Math.floor(Ress.D.r/2))+'</td><td><b>Total</b></td><td><b>'+Sep(Pillage)+'</b></td>';
...
/*** Funny Fleet ***/
	...
	if (URL_P == 'fleet1' && Options.ff.flotte_) {
		...
		if (IndexOf(url, 'mission=8')) {Load(name_('am209')[0], url.match(/&REC=([0-9]*)/));}
	...
	if (URL_P == 'fleet2' && Options.ff.flotte__) {Options = tag_('option', id_('speed'));	for (i in Options) {Options[i].setAttribute('onmousemove', 'updateVariables()');}}
...
/*** Funny Constructions ***/
	...
	if (URL_P == 'resources' || URL_P == 'shipyard' || URL_P == 'station' || URL_P == 'defense') {
		...
		if ((Options.fc.cs == true && URL_P == 'shipyard') || (Options.fc.def == true && URL_P == 'defense')) {Interval = setInterval('Modification()', 250);}
	}
