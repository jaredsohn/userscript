...
// @description   Version 2.62 - modifed from slyz version of DrEvi1's Raid Log Manager 
...
// --------------------------------------------------------------------------------------
// 04/16/2011 version 2.62 - Disabled auto-update checker
// 02/16/2011 version 2.61 - Changed point deductions for bosses and introduced scaling mechanism for normal hobos.
// 12/30/2010 version 2.51 - Well that didn't work as expected.  Modified check for Elf Alley and put Hodg back into the mix.
// 12/29/2010 Version 2.5 - Changed script to only output Elf Alley if it existed in the log
...
var twn_Hobo = 1 ;
var twn_Hobo_bonus = 0 ;
var twn_Hobo_pct = 1;
var twn_Hobo_turns = 0;
var twn_Defeats = -2 ;
...
// - Temporary disabling of auto-update checker
    ...
    if ((GM_getValue("option_hobo","ON") == "ON") && (dungeon_type != "The Slime Tube"))
    {
        ...
        for (var r=0; r < swr_chars.length; r++ )
        {
          advArray[r] = new Array(swr_chars[r],0,0,0,0,0,0,0,0);
        	...
        	// Check to see if Elf Alley is in this log

		    var pageData = document.body.innerHTML;   
		    var startIndex = pageData.indexOf("defeated  Hobelf");
		    if (startIndex >= 0 )
		          alley_exists = true ;
...
twn_Hobo_pct = GM_getValue("Hobo_pct");
		...
		{ 
		  getTSHobos(twn_data, twn_total, twn);
		  if (twn_Hobo_pct == 0)
			twn_Hobo_bonus = 1;
		  else
			twn_Hobo_bonus = Math.floor(twn_Hobo_pct/100 * ((3000 - twn_Hobo_turns)/400));
		  page += "Due to the nature of GM, you will have to reload this page to get the current scaling points for normal hobo kills" + "<br />";

		  var twn_data = new Array();
		  var twn_total = zeroArray(twn.length+2);
		  var twn_points = new Array("("+twn_Hobo_bonus+" pt)","("+twn_Defeats+" pt)","("+twn_Marketplace+" pt)","("+twn_Richard+" pt)","("+twn_Richard_2+" pt)","("+twn_Richard_3+" pt)","("+twn_Stage+" pt)","("+twn_Hat+" pt)","("+twn_Mosh+" pt)","("+twn_Ruined+" pt)","","");
		  var twn_clan = new Array(twn_Hobo_bonus,twn_Defeats,twn_Marketplace,twn_Richard,twn_Richard,twn_Richard,twn_Stage,twn_Hat,twn_Mosh,twn_Ruined,0);
		  parseSection("Town Square:", twn, twn_clan, twn_data, twn_chars, "Hobopolis")
		...
		var misc_clan = new Array(twn_Hodg,twn_Defeats,0);
		var misc_chars = setupCharacters("Miscellaneous", "Hobopolis");
		var misc_data = new Array();
		var misc_total = zeroArray(misc.length+1);
		parseSection("Miscellaneous", misc, misc_clan, misc_data, misc_chars, "Hobopolis");
...
//		}
        ...
        boss_table = '<center>' +
					...
					if (alley_exists)
					{
					boss_table += '<td width="80" class="alley_total">Uncle Crimbo</td>';
					}
				 boss_table  += '</tr>' +
                             '<tr align="center">' +
					...
					if (alley_exists) 
					{
						boss_table +='<td class="alley_shaded" onmouseover="style.cursor=\'pointer\';" onclick=window.location=\'clan_hobopolis.php?place=9\'><img width="80" height="80" src="http://images.kingdomofloathing.com/adventureimages/unclehobo.gif"></td>';
					}
				 boss_table += '</tr>' +
                             '<tr align="center">';
...
function createLootTable(lootArray, dungeon)
{
    ...
    if (dungeon == "Hobopolis")
    {
        dataTable += '<table width="100%" class="normal_table">'+
						...
						if (alley_exists)
						{
							dataTable += '<td colspan="6" class="alley_header">Uncle Hobo</td>';
						}
                                               dataTable += '<td rowspan="2" class="normal_total" style="font-size: 9px;line-height:9px;">T<br>o<br>t</td>'+
                                            '</tr>';
    }
    else
    {
...
function getZoneStatus(section)
{
	...
	else
	{
   		...
   		var totalArray = new Array(0,0,25,0,10,10,10,10,10,0);
	}
    GM_get("clan_hobopolis.php?place="+section,
        function findStatus(response) 
        {
                  ...
                  if (section == 2)
		  {
		    ...
		    GM_setValue("Hobo_pct",percentage);
...
function getTotalPoints()
{
    ...
    var dataTable = '<center>' +
					...
					if (alley_exists)
					{
						dataTable += '<td class="alley_total">Elf Alley</td>';
					}
				       dataTable += '<td class="normal_total">Total</td>' + 
                                   '</tr>';
    var counter = 0;
    ...
    {	
          ...
          if (ptsArray[z][9] != 0)
          {
			  ...
			  dataTable += '<td class="sewer' + shaded + '">' + swr_char_total_pts[swrIndex] + '</td>'+
						   ...
						   '<td class="purple' + shaded + '">' + ptsArray[z][6] + '</td>';
						    if (alley_exists) 
						    {
							dataTable += '<td class="alley' + shaded + '">' + ptsArray[z][7] + '</td>';
						    }
						   dataTable += '<td class="normal' + shaded + '">' + ptsArray[z][8] + '</td>'+
						   '</tr>';
      ...
      dataTable += '<tr>'+
		    ...
		    if (alley_exists)
		    {
			dataTable += '<td class="alley_total">' + totalArray[7] + '</td>';
		    }
		   dataTable += '<td class="normal_total">' + totalArray[8] + '</td>'+
                   '</tr>';
    ...
    for (var z=0; z < swr_chars.length; z++ )   // For each character
    {
        ...
        //{
          ...
          dataTable += '<tr>'+
			...
			if (alley_exists)
			{
				dataTable += '<td class="alley' + shaded + '">' + advArray[z][7] + '</td>';
			}
		       dataTable += '<td class="normal' + shaded + '">' + advArray[z][8] + '&nbsp;&nbsp;(' + (parseInt(advArray[z][8]) + parseInt(swr_char_total_num[swrIndex])) + ')</td>'+
                       '</tr>';
      ...
      dataTable += '<tr>'+
			...
			if (alley_exists) 
			{
			dataTable += '<td class="alley_total">' + totalArray[6] + '</td>';
			}
		   dataTable += '<td class="normal_total">' + totalArray[7] + '&nbsp;&nbsp;(' + totalArray[8] + ')</td>'+
                   '</tr>';
...
//
function getTSHobos(dataArray, totalArray, messageArray)
{
    ...
    twn_Hobo_turns = parseInt(totalArray[1]);
}
//
...
//    var startIndex = pageData.indexOf("<b>" + dungeon + ":</b>");
    var startIndex = pageData.indexOf("<b>" + dungeon);
    if (startIndex >=0 )
    {
...
{
      var player = theString.substring(0,theString.indexOf(") ")+1).replace("<b>","").replace("</b>","").replace(/^.*>/,"");
      return player;
}
