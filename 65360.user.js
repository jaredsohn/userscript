// ==UserScript==
// @name           This is a testy-type-thing! Not working! Don't install.
// @description    Checks for glowing
// @include        *
// ==/UserScript==
			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' ('+tankId+') Shouko has been glowing for '+seconds2HMS(gaiaT-glowT)+'.');
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/54509127/');
								}
							}
							else{
								//alert('over');
							...
							alert(aquaN+' ('+tankId+') Shouko will glow in '+seconds2HMS(glowT-gaiaT));
							if(open===true){
								GM_openInTab('http://www.gaiaonline.com/landing/flashaquarium/?userid='+json[1][2][tankId]['user_id']);
							}
							else{
								GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/54509127/');
								GM_openInTab('http://www.gaiaonline.com/profiles/?mode=addcomment&u=79541');
							}
						}
					}
				}
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(3435829,false);//Shouko
	}
}
checkTanks();
setTimeout(checkTanks,100);