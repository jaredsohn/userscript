// ==UserScript==
// @name           Mermaider's Tanks to Stalk[#3]
			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' ('+tankId+') [Sharee] has been glowing for '+seconds2HMS(gaiaT-glowT)+'.');
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/55007403/');
							...
							alert(aquaN+' ('+tankId+') [Sharee] will glow in '+seconds2HMS(glowT-gaiaT));
							if(open===true){
