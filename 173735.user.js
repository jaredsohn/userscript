// ==UserScript==
// @name            HF Easy PM
// @author          Helios (http://www.hackforums.net/member.php?action=profile&uid=679252)
// @description     Allows you to send PMs in an easy and fast way. Also Included Multi-PM for upgraded Members
// @require         http://ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js
// @include         *hackforums.net/*
// @version         1.2
// @changelog       Added MultiPM option in private messages. Firefox bugs have been fixed. This userscript can be used in any MyBB forum, just change the "@include" to your desired forum.
// ==/UserScript==
/*
 *@changelog 
 *
...
var popUpPM_style = "position:absolute;width:320px;height:390px;display:none;background:#333333;box-shadow: 2px 2px 20px 0px black;-moz-border-radius:7px;-webkit-border-radius:7px;-khtml-border-radius:7px;border-radius:7px;";
...
if(document.location.toString().indexOf("private.php") != -1)
    $('td[class="trow1"][align="right"][style="white-space: nowrap"]').each( function(){
        var author = $(this).prev().text();
        $(this).prepend('<a author="' + author + '" class="bitButton MultiPM" style="cursor:pointer;font-size:11px;float:left;" title="Multi PM">Multi PM</a>');
    });
...
$(".MultiPM").click( function(){
	...
	var myList = document.getElementsByClassName('MultiPM');
	for(var i=0; i<myList.length; i++) { 
		if(myList[i].style.backgroundColor == "gray")
		    count++;
	}
    if(count > 1)
...
$("#multiPM_click").click(function(e){
	...
	for(var i=0; i<myList.length; i++) { 
		if(myList[i].style.backgroundColor == "gray"){
			if(f){
			    $("#PM_rec").val(myList[i].getAttribute("author"));
				f = false;
			}else{
			    $("#PM_rec").val($("#PM_rec").val() + "," + myList[i].getAttribute("author"));
			}
		}
	}
	$("#popUpPM").css({
...
$("#multiPM_cancel").click(function(){
	...
	for(var i=0; i<myList.length; i++) { 
		if(myList[i].style.backgroundColor == "gray")
		    myList[i].style.backgroundColor = "#531F3E";
	}
});