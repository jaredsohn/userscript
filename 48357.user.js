// ==UserScript==
// @name           Farming Machine for Travian 3.5 (Arabic)
...
var SCRIPT = {
    url : 'http://userscripts.org/scripts/source/48357.user.js',
    version : '1.2.2' //same value as @version
};

// @author Hudhaifa Shatnawi (hudhaifa@gmail.com)
// Change some of function names to be more understood
// and write a doc for some functions also
// new features:
//  - add start farming button in the left side, so you can start it any time
//  - you can mark a player as friend
//  - you can mark farms that have many Crannies, so you dont need to attack them

//adapt these variables to your personal need:
//--------------------------------------------
//Message Window config
var messageWindowTop = 240;		//0 means top of the browser window
 ...
 * Comment
 */
function showStartFarmingButton() {
    var startButton = createInputButton("submit", T('START_FARMING'), startEngine);
   // dom.get('lright1').insertBefore(startButton, dom.get('lright1').firstChild);;
}
...
function attacktable(){ //analyse report table
    try {
        ...
        if (rescell != null){
            if(tm!=null){
                if(tab.snapshotItem(0).getElementsByTagName("tr")[0].innerHTML.indexOf("unit uhero")>0) {
                    statushero=1;ats[5]=1;
                }
...
function setLanguage() { //define language elements
    ...
    if (ext == "ae") { //Language:arabic, Translator : kmmad1
        lang = {
            ...
            'HALT_FARMING' : "توقف",
            'MACHINE_RUNNING' : "بدء حصد المزارع",
