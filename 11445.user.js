// ==UserScript==
// @name           BBC Facebook Style
// @namespace      Stefan Gilbert
// @description    A Style for BBC Network
// @version	0.3
...
/*
Copyright (C) 2007 by Alan Kelly, Modified for Bethany Bible College by Stefan Gilbert.
Contact: stefan.gilbert@gmail.com
...
if(pageLinks) {
    ...
    var pages = listItems.length - 1; // - 1 because there's the 'next' link
    // start on 1 because 0 is for page 1, and we're already there
    for(var i = 1; i < pages; ++i) {
        var nextPageUrl = listItems[i].getElementsByTagName("a")[0].href;
        GM_xmlhttpRequest({
            method: "GET",
            url: nextPageUrl,
...
var reload = 60;// Seconds
var check = false;//Auto Reload Unchecked

function refresh_page() {
if (document.getElementById('auto_reload').checked == true) {
...
if (client.readyState==4)
  ...
  if (client.status==200)
    {
myframe.contentDocument.getElementById('wall_posts').innerHTML = client.responseText;
if (document.getElementById('wall_posts')) {
...
if (document.getElementById('content').className == 'clearfix') {
document.getElementById('content').innerHTML = myframe.contentDocument.getElementById('content').innerHTML;}
...
document.getElementById('navigator').innerHTML = myframe.contentDocument.getElementById('navigator').innerHTML;}
  }
  }
};
...
var on_off = document.createElement('input');
...
on_off.id = "auto_reload";
var holder = document.createElement('div');
holder.style.position = "absolute";
holder.style.top = "0";
holder.style.left = "5";
ch_text = document.createTextNode("Auto Reload News Feed");
holder.appendChild(on_off);
holder.appendChild(ch_text);
