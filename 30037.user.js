// ==UserScript==
// @name           Tokyo Toshokan Cleaner v1.1
...
function build_table() {
	...
	for each(d in data) {
		nt += "<tr>";
		nt += "<td><a class='c' style='background-color: " + d.category_style + "' href='" + d.category_url + "'>" + d.category_name + "</a></td>";
		
		nt += "<td class='torrent'>"
		if(d.torrent.parsed) {
			nt += "<a class='torrent' title='" + d.torrent_original_name + "' href='" + d.torrent_url + "'>" + d.torrent.parsed;
			if(d.torrent.ext) nt += "<span class='ext'>" + d.torrent.ext + "</span>";
			nt += "</a>";
			if(d.torrent.search) {
				var links = d.torrent.search.split("%");
				for each(l in links) nt += "<sup class='search'><a href='/search.php?terms=" + l + "'>SEARCH</a></sup>";
			}
			if(d.torrent.meta) nt += "<br/><span class='meta'>" + d.torrent.meta + "</span>";
			
		}
		else {
			nt += "<a class='torrent' href='" + d.torrent_url + "'>" + d.torrent_original_name + "</a>";
			if(d.torrent.search) {
				var links = d.torrent.search.split("%");
				for each(l in links) nt += "<sup class='search'><a href='/search.php?terms=" + l + "'>SEARCH</a></sup>";
			}
		...
		nt += "<td class='__s'>" + d.s + "</td>";
		nt += "<td class='__l'>" + d.l + "</td>";
		nt += "<td class='size'>" + d.size + "</td>";
		nt += "<td class='size_desc'>" + d.size_desc + "</td>";
		
		nt += "<td class='links'>";
		if(d.website_url) nt += "<a href='" + d.website_url + "'>WEBSITE</a><br/>";
		nt += "<a href='"+ d.details_url +"'>DETAILS</a></td>";
		
		nt += "<td class='time'><nobr>" + d.formatted_date + "</nobr></td>";
		
		nt += "</tr>";
		
		if(last_sort == "time") if(d.is_marker) nt += "<tr><td class='marker' colspan='8'></td></tr>"
	}
...
function highlight(ns) {
	...
	for each(h in parsed_highls) {
		...
		for each(w in h) {
			...
			if(ind > -1) {
				hits++;
				s = s.substring(0, ind) + "<b>" + s.substr(ind, w.length) + "</b>" + s.substr(ind + w.length, s.length);
				if(!search) search = w;
				else search += "+" + w;
			}
			if(m_ind > -1) {
				hits++;
				m = m.substring(0, m_ind) + "<big>" + m.substr(m_ind, w.length) + "</big>" + m.substr(m_ind + w.length, m.length);
				if(!search) search = w;
				else search += "+" + w;
			}
		}
...
function listHighls() {
	...
	if(hlsA = getHighls()) for each(h in hlsA) {
		HighlsDiv.innerHTML += "<a href='/search.php?terms=" + h.replace(/\s/g, "+") + "'>" + h/*.replace(/\s/g, "&nbsp;")*/ + "</a><sup class='del_clickable'>DEL</sup>";
	}
}
