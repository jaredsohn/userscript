...
// @version     0.38
// @updateURL   http://sublyme.net/site_media/userscripts/HFES.user.js
...
function getThreadTitle(htmlCode, aurl){
    ...
    for(i = 0; i < navArray.length; i++){
        ...
        {
            threadTitle = navArray[i].split('class="active">')[1];
            threadTitle = threadTitle.split('</span>')[0];
            threadTitle = threadTitle.replace('&nbsp;',' ');
            threadTitle = threadTitle.replace('&quot;','"');
            threadString = "[url="+aurl+"][b]"+threadTitle+"[/b][/url]";
            threadString = decodeHtmlNumeric(threadString);
            return threadString;
        }
    }
}

function wrongPageCite(){ // HOLDER3
    ...
    GM_xmlhttpRequest({
        method: "GET",
        url: realURL,
        overrideMimeType: "text/html; UTF-8",
        onload: function(response){
...
function profileTagger(){
    ...
    //$('.largetext strong span').append('&nbsp&nbsp</strong><span class="tag_bubble" style="font-style:normal;" id="profileTag"><small>'+tag+'</small></span><strong> ');
...
function startFavorites(){
    ...
    if(document.URL.indexOf('nsfw.') != -1){
        try{
            GM_xmlhttpRequest({
                method: "GET",
                url: "http://sublyme.net/site_media/HFESfavoritesNSFW.html",
                onload: function(response){
        ...
        try{
            GM_xmlhttpRequest({
                method: "GET",
                url: "http://sublyme.net/site_media/HFESfavorites.html",
                onload: function(response){
...
function addRemoveFavForum(){
    var favFidsKeys, fid, fname;
    fid = document.URL.split('fid=')[1].split('#')[0];
    fname = getThreadTitle(document.getElementsByClassName('navigation')[0].innerHTML, document.URL).split('[b]')[1].split('[/b]')[0];
...
function addToMenu(){
    ...
    menuFavorites = '<li> <a href="/favorites.php" class="navButton">Favorites</a></li>';
        ...
        for(i=0; i<postData.length; i++){
            if(postData[i].outerHTML.indexOf('id="post_meta_') != -1){                
                ...
                if(multiDataKeys.indexOf(pid) == -1){
                    button = '<a href="javascript:void(0);" class="bitButton" id="'+id+'" title="[HFES] Multiquoting">+ Multiquote</a>';
                }else{
                    button = '<a href="javascript:void(0);" class="bitButton" id="'+id+'" title="[HFES] Multiquoting">- Multiquote</a>';
                }
...
function addDonorPerksProfile(donorList){
    ...
    if(donorKeys.indexOf(uid) != -1){
        ...
        }else if(donorList[uid][3] =='gilded'){
            var topbar;
