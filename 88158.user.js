...
// @version			0.62
// 
// @history			0.62 Support 0.4.5 new ships.
// @history			0.61 Supports ships in training (same as ground units in training).
// @history			0.60 Better implementation of reloadMilitary that refreshes troops/ships in deployed towns.
...
// @history			0.34 City names are now read as text instead of HTML (strips out Ãƒâ€š  etc.)
...
var IkaTools = {
	...
	shipTypes:['ram', 'flamethrower', 'steamboat', 'ballista', 'catapult', 'mortar', 'submarine', 'rocketship', 'paddlespeedship', 'ballooncarrier', 'tender'],
	...
	cityGetShipInTraining:function(type, city) {
		city = typeof(city) == 'object' ? city : IkaTools.getCurrentCity();
		var none = {qty:0};
		if(typeof(city.shipsInTraining) == 'undefined')
			return none;
		ret = typeof(city.shipsInTraining[type]) == 'undefined' ? none : city.shipsInTraining[type];
		return ret;
	},
        ...
        IkaTools.getRemoteDocument('http://' + IkaTools.getDomain() + '/index.php?view=militaryAdvisorMilitaryMovements', 
            function(doc, documentText) {
                ...
                if(typeof(callbackFunction) == 'function') {
                    callbackFunction(IkaTools.getVal('movements'), doc, documentText);
                }
            });
    },
	...
	reloadAllMilitary:function(callback, progressCallback) {
		var initialCityId = IkaTools.getCurrentCityId();
		
		var finished = function() {
			IkaTools.setActiveCity(initialCityId, function() {}, true);
		...
		var activatedRelatedCity = function() {
			IkaTools.getRemoteDocument('http://' + document.domain + '/index.php?view=relatedCities&id=' + cities[nextCityIndex].id, function(root) {
		...
		var loadRelatedCities = function() {
			nextCityIndex++;
			while (nextCityIndex < cities.length) {
				if(!IkaTools.cityIsOwn(cities[nextCityIndex])) {
					progressCallback(nextCityIndex, cities.length);
					IkaTools.setActiveCity(cities[nextCityIndex].id, activatedRelatedCity);
					return;
				}
				nextCityIndex++;
			}
				...
				if(IkaTools.cityIsOwn(cities[i])) {
					...
					if(barracks) {
						IkaTools.getRemoteDocument('http://' + document.domain + '/index.php?view=barracks&id=' + cities[i].id + '&position=' + barracks.position, function(root) {
							...
							progressCallback(Math.round(numUpdated / 2), cities.length);
					...
					if (shipyard) {
						IkaTools.getRemoteDocument('http://' + document.domain + '/index.php?view=shipyard&id=' + cities[i].id + '&position=' + shipyard.position, function(root) {
							numUpdated++;
							IkaTools.views.shipyard(root);
							progressCallback(Math.round(numUpdated / 2), cities.length);
							if(numUpdated == numToUpdate)
								loadRelatedCities();
						});
	...
	setActiveCity:function(cityId, callback, force) {
		if(typeof(cityId) != 'undefined' && ((cityId != IkaTools.getCurrentCityId()) || force)) {
			var postdata = "";
	...
	goTo:function(url, cityId) {
		...
		IkaTools.setActiveCity(cityId, function() {
...
IkaTools.views["cityMilitary-fleet"] = function(root, cityId) {
	...
	city.ships.paddlespeedship = parseInt(u[8].innerHTML);
            ...
            //for(var i = 0; i < tds.length; i++) {
                    ...
                    if (!cargoShipsFound) {
                        ...
                        if (match) {
                            ...
                            if (movement.mission && (movement.mission == 'blockade' || movement.mission == 'deployfleet' || movement.mission == 'defend_port')) {
                                movement.units.push({
                                  name: 'Unknown Ships', 
                                  qty: ships,
					...
					movement.direction = ($('img', tds[6 + offset]).size() == 1 ? 'right' : ($('img', tds[4 + offset]).size() == 1  ? 'left' : false));
...
IkaTools.views.relatedCities = function(root, cityId) {
	...
	city.units = {};
	city.ships = {};
	var militaryNode = $('div#selectMilitary', root)[0];
	$('div.content table.army div.armybutton', militaryNode).each(function(i) {
		var type = this.className.match(/\s*([^\s]+)$/)[1];
		city.units[type] = parseInt(this.innerHTML.match(/\d+/)[0]);
	});
	$('div.content table.army div.fleetbutton', militaryNode).each(function(i) {
		var type = this.className.match(/\s*([^\s]+)$/)[1];
		if (type.indexOf("ship_") == 0) {
...
IkaTools.views.shipyard = function(root) {
	...
	$('#unitConstructionList .army', root).each(function(i) {
		var type = IkaTools.getShipTypeById(this.className.match(/\d+/)[0]);
		...
		city.shipsInTraining[type].qty += parseInt($('.unitcounttextlabel', this).text().replace(/[^\d]/g, ''));
	});
	IkaTools.saveCity(city);
}
...
IkaTools.getCurrentCityResource = function(type) {
	...
	for(var i = 0; i < resourceLis.size(); i++) {
		if(resourceLis[i].className == type) {
			...
			return parseInt($('.#value_' + (type == 'glass' ? 'crystal' : type), resourceLis[i]).text().replace(/[^\d]/g, ''));
		}
	}
	return 0;
}
