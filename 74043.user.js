// Modified to work on the Hacettepe University libraries system by Müge Akbulut, mugeakbulut@gmail.com
...
// ==UserScript==
// @name              Hacettepe Üniversitesi, Amazon
// @description	  Hacettepe Universitesi kütüphanesi kataloğu Amazon'dan taranabilir. 
// @include       http://*.amazon.*
// ==/UserScript==
...
(function(){
//	GM_log('Hacettepe Üniversitesi, Amazon');	

	var libraryIsbnUrlPattern = 'http://katalog.hacettepe.edu.tr/search/i?SEARCH='
	var libraryTitleUrlPattern = 'http://katalog.hacettepe.edu.tr/search*tur/?searchtype=Y&searcharg='
	...
	if (isbn!=0){
		...
		updateStatusHTML('Aranıyor...' + libraryName + '...');
...
function getBookStatuses(){
	...
	if (isbnsIndex < isbns.length){
		...
		updateStatusHTML( libraryName + ' kütüphanesinde ISBN '+ isbns[isbnsIndex] + ' aranıyor...');
...
function createStatusAndLibraryHTML() {
	...
	//yazının boyutu
	splStatusDiv.style.fontSize = '80%';
...
function setStatusNoneFound() {
	...
	var label = document.createTextNode('Bulunamadı. ' + libraryName + ' kütüphanesinde başlık olarak aramayı dene.  '  );
...
var label2 = document.createTextNode(' ya da ' + libraryName + ' kütüphanesinde ISBN olarak ara.'  );
