// ==UserScript==
// @name           Ikariam Upgare Serassasin
// @version        3.0
// @namespace      ikariam
// @description    Ikariam Upgare modfied bY serassasin
// @author         Apollo ?
...
//
// @include        http://s*.ikariam.com/index.php?action=loginAvatar&*
// @include        http://s*.ikariam.com/index.php?view=city&*
// @include        http://s*.ikariam.com/index.php?*&function=changeCurrentCity&*
//
...
//
//		v2.3.1 (déc 17, 2012)
//			- Fixed : Hanged when reloading the view before spirit was researched.
//
//		v2.3 (déc 16, 2012)
//			- Fixed : icon on position 16 can be hovered again.
//
//		v2.2 (déc 15, 2012)
//			- Fixed : building updates when changing from enemy to own city and vice-versa.
//
//		v2.1 (sept 10, 2012)
//			- Fixed : reduction buildings detected even when they are in construction.
//			- Fixed : CSS issue with firefox 15, the tooltip appeared as if it had no background.
//
//		v2.0.6 (july 31, 2012)
//			- Fixed : the port appeared not-upgradeable when busy.
//
//		v2.0.5 (june 24, 2012)
//			- massive update of the costs, thanks to Woeka and ManicMiner.
//			- fixed detection of construction site (the GF changed it...)
//			- jQuery is now used in noConflict mode, to allow the use of several scripts in Chrome/Tampermonkey
//
//		v2.0.4 (may 25, 2012)
...
//
//		v2.0.2 (may 5, 2012)
//			- Displays also on the first city view after you login !
//			- Proper display for small buildings using only wood.
//
//		v2.0.1 (may 5, 2012)
//			- Thousands separator
//
...
//			- Removed ajax use to find research levels : No net activity whatsoever.
...
var IUW = new function() {
	// Server host and hostname
	this.host = top.location.host.replace(/\.ikariam\.com/, '');

	// Materials reduction from researches
	this.pulley = GM_getValue(this.host + "_iuw_pulley", false);
	this.geometry = GM_getValue(this.host + "_iuw_geometry", false);
	this.spirit = GM_getValue(this.host + "_iuw_spirit", false);

	// Var for showing info box or not, default true
	this.showInfoBox = GM_getValue(this.host + "_iuw_infobox", true);

	this.icons = [];
	this.redCoef = null;			// reduction coefficients
	...
	this.init = function() {
		this.screen = unsafeWindow.ikariam.getScreen();
		this.spot = this.screen.data.position;

		// Create all 18 icons.
		for(var i = 0; i < this.spot.length; i++) {
			IUW.icons.push( new IUWIcon(i) );
		}
	...
	this.checkResearch = function(buildingUgradeDiv, buildingType) {
		// Buliding level
		...
		if(!wood) return;
		wood = parseInt(wood.replace(/[^0-9]/g, ''));
		...
		var theoretical = Math.floor(baseWood * IUW.redCoef.wood);
		...
		if(theoretical > wood) {		// theoretical price is never underestimated, since all you get in the game are reductions
			...
			setTimeout(IUW.saveResearchLevels, 0);		
		}
	}

	this.saveResearchLevels = function() {
		GM_setValue(IUW.host + "_iuw_pulley", IUW.pulley);
		GM_setValue(IUW.host + "_iuw_geometry", IUW.geometry);
		GM_setValue(IUW.host + "_iuw_spirit", IUW.spirit);
	}
	...
	this.getReductions = function() {
		...
		if (this.pulley)   { redAll = 2;          }
		if (this.geometry) { redAll = redAll + 4; }
		if (this.spirit)   { redAll = redAll + 8; }
		...
		for (var i = 0; i < this.spot.length; i++) {
			...
			var buildingType = this.spot[i].building.split(/ /)[0];
			switch (buildingType) {
				case 'architect':
					redMarble = parseInt(this.spot[i].level);
					break;
				case 'optician':
					redCrystal = parseInt(this.spot[i].level);
					break;
				case 'carpentering':
					redWood = parseInt(this.spot[i].level);
					break;
				case 'vineyard':
					redWine = parseInt(this.spot[i].level);
					break;
				case 'fireworker':
					redSulfur = parseInt(this.spot[i].level);
					break;
			}
		}
	...
	this.updateIcons = function() {
		// Bureaucracy
		var lockedPosition = IUW.screen.data.lockedPosition ? IUW.screen.data.lockedPosition[0] : -1;
		...
		for (var i = 0; i < this.spot.length; i++) {
			...
			level = this.spot[i].level;
			underConstruction = false;		// detected later
			var buildingType = this.spot[i].building;
			// Busy buildings : port, workshop, shipyard, barracks, hideout(?)
			busy = this.spot[i].isBusy && (buildingType != 'port');		// 'port constructionSite' ? busy too because cityBusy = true
			busy = busy || cityBusy;
			...
			} else if (isDefined(IUW.POSITIONS[buildingType]) && isDefined(IUW.BUILDINGS[buildingType])) {		// test bizarre
	...
	//			else ???
			...
			if(IUW.ownCity) {
				if (isDefined(IUW.BUILDINGS[buildingType][upLevel]) && IUW.BUILDINGS[buildingType][upLevel]["wood"] != 0) {

					// Get basic resource requirements in hte DB
					var basicRequirements = IUW.BUILDINGS[buildingType][upLevel];

					// And apply deductions
					requirements = {
						wood    : Math.floor( basicRequirements.wood    * IUW.redCoef.wood   ),		// all buildings require wood
						wine    : Math.floor( basicRequirements.wine    * IUW.redCoef.wine   ),		// NaN for unused materials
						marble  : Math.floor( basicRequirements.marble  * IUW.redCoef.marble ),
						crystal : Math.floor( basicRequirements.crystal * IUW.redCoef.crystal),
						sulfur  : Math.floor( basicRequirements.sulfur  * IUW.redCoef.sulfur )
					}
				}
			}

			// Updates the icon
			IUW.icons[i].update(posTop, posRight, busy, level, requirements);
	...
	this.reload = function(){
		...
		// Is this our city ?
		this.ownCity = (this.screen.data.ownerId == unsafeWindow.ikariam.model.avatarId);
...
IUW.$(function(){
	...
	IUW.screen.IUW_Update = IUW.screen.update;
	IUW.screen.update = function(data) {
		IUW.screen.IUW_Update(data);
		IUW.reload();
	}
...
IUWIcon.prototype.update = function(top, right, busy, level, req) {
	...
	this.requirements = req;		// { wood, wine, marble, wrystal, sulfur }
	this.top = top;
	this.right = right;
...
IUW.BUILDINGS = {
	...
	alchemist:[
		...
		{ 'wood' : 590392, 'marble' : 327069}, // level 27
		{ 'wood' : 767620, 'marble' : 425294}, // level 28
		{ 'wood' : 998018, 'marble' : 55298}, // level 29
	...
	architect:[
		...
		{ 'wood' : 14424, 'marble' : 6777}, // level 22
		...
		{ 'wood' : 35825, 'marble' : 16017}, // level 28
		{ 'wood' : 41631, 'marble' : 18450}, // level 29
		{ 'wood' : 48371, 'marble' : 21245}, // level 30
		{ 'wood' : 56185, 'marble' : 24454}, // level 31
	...
	carpentering:[
		...
		{ 'wood' : 18167, 'marble' : 11273}, // level 26
		{ 'wood' : 21299, 'marble' : 13543}, // level 27
		{ 'wood' : 24946, 'marble' : 16263}, // level 28
	...
	dump:[
		...
		{ 'wood' : 2504, 'marble' : 2051, 'crystal' : 2278, 'sulfur' : 2061}, // level 4
		...
		{ 'wood' : 20905, 'marble' : 15691, 'crystal' : 15324, 'sulfur' : 18623}, // level 13
	...
	embassy:[
		...
		{ 'wood' : 15610, 'marble' : 19824}, // level 17
		{ 'wood' : 18498, 'marble' : 24339}, // level 18
		{ 'wood' : 22457, 'marble' : 29846}, // level 19
		{ 'wood' : 27074, 'marble' : 36564}, // level 20
		{ 'wood' : 32290, 'marble' : 45216}, // level 21
		{ 'wood' : 39261, 'marble' : 54769}, // level 22
		{ 'wood' : 47240, 'marble' : 66733}, // level 23
		{ 'wood' : 56812, 'marble' : 81859}, // level 24
		{ 'wood' : 70157, 'marble' : 104537}, // level 25
		{ 'wood' : 84318, 'marble' : 129580}, // level 26
		{ 'wood' : 101310, 'marble' : 158759}, // level 27
		{ 'wood' : 121979, 'marble' : 193849}, // level 28
		{ 'wood' : 146503, 'marble' : 236659}, // level 29
		{ 'wood' : 175932, 'marble' : 288888}, // level 30
		{ 'wood' : 222202, 'marble' : 358869}, // level 31
		{ 'wood' : 266778, 'marble' : 437985}, // level 32
	],

	// fireworker or Firework Test Area, wood, marble
	fireworker:[
	...
	// forester or Forester House, wood, marble
	forester:[
		...
		{ 'wood' : 17823, 'marble' : 10422}, // level 14
		...
		{ 'wood' : 192360, 'marble' : 109337}, // level 23
		{ 'wood' : 250173, 'marble' : 142266}, // level 24
		{ 'wood' : 325258, 'marble' : 185046}, // level 25
	...
	glassblowing:[
		...
		{ 'wood' : 122021, 'marble' : 67485}, // level 21
		{ 'wood' : 158740, 'marble' : 87835}, // level 22
		{ 'wood' : 206471, 'marble' : 114290}, // level 23
		{ 'wood' : 268524, 'marble' : 148680}, // level 24
		{ 'wood' : 349194, 'marble' : 193389}, // level 25
		{ 'wood' : 454063, 'marble' : 251512}, // level 26
		{ 'wood' : 590392, 'marble' : 327069}, // level 27
	...
	museum:[
		...
		{ 'wood' : 5818008, 'marble' : 7582730}, // level 21
		{ 'wood' : 8727680, 'marble' : 11526750}, // level 22
		{ 'wood' : 13092008, 'marble' : 17521067}, // level 23
		{ 'wood' : 19638608, 'marble' : 26632786}, // level 24
	],
	...
	palace:[
		{ 'wood' : 5823, 'marble' : 1433}, // level 2
		{ 'wood' : 16048, 'marble' : 4546, 'sulfur' : 3088}, // level 3
		{ 'wood' : 36496, 'wine' : 10898, 'marble' : 10770, 'sulfur' : 10300}, // level 4
		...
		{ 'wood' : 4743517, 'wine' : 1434821, 'marble' : 1591666, 'crystal' : 1357543, 'sulfur' : 1842149}, // level 11 max
		{ 'wood' : 10464974, 'wine' : 2869957, 'marble' : 3185009, 'crystal' : 2715112, 'sulfur' : 3688421}, // level 12 not possible
	],
	
	// palaceColony, wood, wine, marble, crystal, sulfur
	palaceColony:[
		{ 'wood' : 5823, 'marble' : 1434}, // level 2
		{ 'wood' : 16048, 'marble' : 4546, 'sulfur' : 3088}, // level 3
		{ 'wood' : 36496, 'wine' : 10898, 'marble' : 10770, 'sulfur' : 10300}, // level 4
		{ 'wood' : 77392, 'wine' : 22110, 'marble' : 23218, 'crystal' : 21188, 'sulfur' : 24725}, // level 5
		{ 'wood' : 159184, 'wine' : 44534, 'marble' : 48114, 'crystal' : 42400, 'sulfur' : 53573}, // level 6
	...
	port:[
		...
		{ 'wood' : 8850, 'marble' : 5051}, // level 15
		...
		{ 'wood' : 87627, 'marble' : 76366}, // level 26
		{ 'wood' : 94250, 'marble' : 85042}, // level 27
		{ 'wood' : 130776, 'marble' : 122156}, // level 28
		{ 'wood' : 159019, 'marble' : 153753}, // level 29
		{ 'wood' : 193937, 'marble' : 194089}, // level 30
		{ 'wood' : 235849, 'marble' : 244300}, // level 31
		{ 'wood' : 286514, 'marble' : 307174}, // level 32
		...
		{ 'wood' : 3541580, 'marble' : 5959026}, // level 45
		{ 'wood' : 4291523, 'marble' : 7478200}, // level 46
		{ 'wood' : 5199343, 'marble' : 9383420}, // level 47
		{ 'wood' : 6296109, 'marble' : 11768772}, // level 48
	],

	// safehouse or Hideout, wood, marble
	safehouse:[
		{ 'wood' : 248}, // level 2
		{ 'wood' : 402}, // level 3
	...
	shipyard:[
		...
		{ 'wood' : 3485, 'marble' : 3070}, // level 11
		{ 'wood' : 4459, 'marble' : 3941}, // level 12
		{ 'wood' : 5688, 'marble' : 5037}, // level 13
	...
	tavern:[
		...
		{ 'wood' : 88413, 'marble' : 1538791}, // level 41
		...
		{ 'wood' : 3184649, 'marble' : 9655691}, // level 48
	],
	...
	townHall:[
		{ 'wood' : 158,'sulfur' : 0}, // level 2
		{ 'wood' : 335,'sulfur' : 0}, // level 3
		{ 'wood' : 622,'sulfur' : 0}, // level 4
		{ 'wood' : 923,'marble' : 285}, // level 5
		{ 'wood' : 1390,'marble' : 550}, // level 6
		...
		{ 'wood' : 258875,'marble' : 403824}, // level 28
		{ 'wood' : 314902,'marble' : 515592}, // level 29
		{ 'wood' : 387655,'marble' : 666227}, // level 30
		{ 'wood' : 471194,'marble' : 850031}, // level 31
		{ 'wood' : 572580,'marble' : 1084292}, // level 32
		{ 'wood' : 695615,'marble' : 1382826}, // level 33
		{ 'wood' : 854728,'marble' : 1783721}, // level 34
		{ 'wood' : 1037814,'marble' : 2273685}, // level 35
		{ 'wood' : 1274043,'marble' : 2930330}, // level 36
		{ 'wood' : 1714396,'marble' : 3692589}, // level 37
		{ 'wood' : 1876185,'marble' : 4756439}, // level 38
	...
	// vineyard or Wine Press, wood, marble
	vineyard:[
	...
	wall:[
		...
		{ 'wood' : 420689, 'marble' : 444998}, // level 33
	...
	// winegrower or Winery, wood, marble
	winegrower:[
		{ 'wood' : 467, 'marble' : 116}, // level 2
		{ 'wood' : 718, 'marble' : 255}, // level 3
		{ 'wood' : 1045, 'marble' : 436}, // level 4
		...
		{ 'wood' : 349194, 'marble' : 193389}, // level 25
		{ 'wood' : 454063, 'marble' : 251512}, // level 26
		{ 'wood' : 590392, 'marble' : 520887}, // level 27
		{ 'wood' : 767620, 'marble' : 425294}, // level 28
		{ 'wood' : 998018, 'marble' : 552986}, // level 29
		{ 'wood' : 1297536, 'marble' : 718988}, // level 30
		{ 'wood' : 1686906, 'marble' : 934789}, // level 31
	...
	workshop:[
		...
		{ 'wood' : 19481, 'marble' : 11477}, // level 23
