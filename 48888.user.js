...
// @description    
// @include        https://www.palmbux.com/ads.php
...
view_page = /\/ads.php/.test(lhr);
ads_page = /\/cks.php?k=/.test(lhr);
ad_page_url = '?k=';
...
ad_wait_time = 75 + Math.ceil(Math.random()*5);
foundAd = false;
...
// @namespace      http://userscripts.org/users/23652
// @description    Auto clicks ads for you on NeoBux.com. Auto-reloads every 1min. & 15sec. minutes if there are no ads. By RaiN
...
view_page = /\/ads.php\/\?l=/.test(lhr);
ads_page = /\/ads.php/.test(lhr);
ad_page_url = 'ads.php';
logout_url = '?l0';
ads.php = xp("/html/body/table/tbody/tr[2]", 9);
url = 'https://' + document.domain + '/';
logoutexist = xp("//a[contains(@href, '?l0')]", 9).href.length>0;
...
setTimeout(function(){window.location.reload();}, Math.floor(1200000+Math.random()*120));
