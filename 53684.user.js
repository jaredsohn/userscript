// ==UserScript==
// @name           Modyfikacja DS_Sounds
// @namespace      none
// @description    Mod wersji 1.60.22 w języku polskim dla Plemion
// @author         Peety (thx Heinzel), mod by Paweł Woroniecki
...
(function(){
var lang = GM_getValue("ds_lang", "pl");
if (lang  == "pl") {
	var Text1 = " Opcje dźwięku "; 	
	var Text2 = " Dźwięk włączony/wyłączony ";
	var Text3 = " Potwórz co: (w minutach) ";
	var Text4 = " Głośność: ";
	var Text9 = " - Adres dźwięku wiadomości: ";
	var Text10 = " - Adres dźwięku raportu: ";
	var Text11 = " - Adres dźwięku forum: ";
	var Text12 = " Adres forum: ";
}

//Tutaj możesz wrzucić swoje nowe tłumaczenie:

else if (lang == "new") { // <-- nie zmieniaj tego "new", tłumacz tylko teksty od Text1 do Text12;
*/
...
// ------------------------------------------------------------------------	
//+++ Nie zmieniaj NIC od tego momentu:+++
//-------------------------------------------------------------------------
...
var dsphpbb = false;
try {
	var wert = document.body.innerHTML.match(/action=is_newer_post/)[0]; if (wert) dsphpbb = true;
	...
	if ((jetzt - zeit1) > (60 * zeit)) {
		...
		if (wert) {
			if ((jetzt - delay) < 3) {	// falls innerhalb von 3 Sek schon ein Sound gespielt wurde
				if (zeit > 2) {
					interval = window.setTimeout("location.reload()", (delay + 3 - jetzt) * 1000);
				}
			}
			else {
...
// Sprache einstellen
	...
	check15.addEventListener("change",function(){check_lang(this.value);},false);
	td12.appendChild(check15);
	td12.appendChild(document.createTextNode("nederlands "));
	
	var check16 = document.createElement("input");
	check16.setAttribute("type", "radio");
	check16.setAttribute("name", "lang");
	check16.setAttribute("value", "ar");
	check16.addEventListener("change",function(){check_lang(this.value);},false);
	td12.appendChild(check16);
	td12.appendChild(document.createTextNode("arabic "));
	
	if (translation != "") {
		var check17 = document.createElement("input");
		check17.setAttribute("type", "radio");
		check17.setAttribute("name", "lang");
		check17.setAttribute("value", "new");
		check17.addEventListener("change",function(){check_lang(this.value);},false);
		td12.appendChild(check17);
		td12.appendChild(document.createTextNode(translation));
	}
	...
	if (lang == "new" && translation != "") check17.checked = true;
...
function grafik() {
	var test = document.getElementsByClassName("navi-border"); 	// DS-Version >= 5.x?
