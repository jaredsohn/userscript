...
*/
var cuCol = [];
cuCol[0] = GM_getValue('t_mbg',defCol[0]);
cuCol[1] = GM_getValue('t_mtc',defCol[1]);
cuCol[2] = GM_getValue('t_mbgh',defCol[2]);
cuCol[3] = GM_getValue('t_mtch',defCol[3]);
cuCol[4] = GM_getValue('t_rbf',defCol[4]);
var firstRun = parseInt(GM_getValue('tnm_fr','1'));
var asetTrs = ['t163m_setScstep','t163m_setScmin','t163m_setScmax','t163m_setZtop','t163m_setcfh'];
var defTset = ['0.2','0.2','2','10001','680']; //缩放变动值,最小缩放倍数,最大缩放倍数,查看框Z-Index,评论框高
var cuTset = [];
t_getSettings();
var isCancelimg = false;
...
var timgActio; //直接点击图片触发 0-否, 1-是
var timgShowNav; //显示图片导航键 0-否 1-是
var thideRec; //隐藏个性推荐 0-否 1-是
var cUpinfo;
var oriImgW = 0;
var oriImgH = 0;
var rtId = '';
...
var curImg;
var twImgid=0;
var oimgletter = ["a","b","c"];
function t_getSettings(){
	cuTset[0] = GM_getValue('tnm_scstep',defTset[0]);
	cuTset[1] = GM_getValue('tnm_scmin',defTset[1]);
	cuTset[2] = GM_getValue('tnm_scmax',defTset[2]);
	cuTset[3] = GM_getValue('tnm_ztop',defTset[3]);
	cuTset[4] = GM_getValue('tnm_cfh',defTset[4]);
	tScaleStep = parseFloat(cuTset[0]);
	tScaleMin = parseFloat(cuTset[1]);
	tScaleMax = parseInt(cuTset[2]);
	zTop = parseInt(cuTset[3]);
	timgLarge = GM_getValue('tnm_imglarge','checked');
	timgTitle = GM_getValue('tnm_imgtitle', 'checked');
	timgLock = GM_getValue('tnm_imglock','checked');
	timgAction = parseInt(GM_getValue('tnm_imgact','1'));
	cUpinfo = GM_getValue('t_cupinfo','checked');
	timgShowNav = parseInt(GM_getValue('tnm_imgnav','1'));
	thideRec = parseInt(GM_getValue('tnm_hiderec','0'));
}
...
/*! jQuery Plugin for Watch Element Content Change
...
t_custom_style += '\n' + '.t_customRt{background: '+ cuCol[4] + ' !important}';
t_custom_style += '\n' + '.t_customCf{height: '+ cuTset[4] + 'px !important}';
...
t_set_html='<div id="t163m_setdiv"class="t_showframe"><div class="t_setdiv"align="center"><div style="padding-top:5px;padding-bottom:5px;"><b>t163more 设置</b></div><table class="t_set_tb"border="0"cellspacing="0"cellpadding="0"width="90%"><thead><tr><td colspan="2"align="left"><b>图片查看</b></td></tr></thead><tr><td style="width:12%;">图片尺寸</td><td style="width:88%;"><input type="radio"class="t_rtjdchk"id="t163m_imgLarge"name="simgsize"/>原图大小　<input type="radio"class="t_rtjdchk"id="t163m_imgMid"name="simgsize"/>中等大小　　点击图片打开查看: <input type="checkbox" class="t_rtjdchk" id="t163m_setimgAct" />　　图片导航键: <input type="checkbox" class="t_rtjdchk" id="t163m_setimgNav" /></td></tr><tr><td>图片缩放</td><td>缩放变动值:<input id="t163m_setScstep"class="t_setfloat"type="text"style="width:30px;"/><span id="t163m_trs1"class="t_setbtn t_settrs">重置</span>　最小缩放倍数:<input id="t163m_setScmin"class="t_setfloat"type="text"style="width:30px;"/><span id="t163m_trs2"class="t_setbtn t_settrs">重置</span>　最大缩放倍数:<input id="t163m_setScmax"class="t_setnum"type="text"style="width:30px;"/><span id="t163m_trs3"class="t_setbtn t_settrs">重置</span></td></tr><tr><td>查看框</td><td>相对固定:<input type="checkbox"class="t_rtjdchk"id="t163m_setimgLock"/>　顶部显示原图切换键:<input type="checkbox"class="t_rtjdchk"id="t163m_setimgTitle"/>　Z-Index:<input id="t163m_setZtop"class="t_setnum"type="text"style="width:45px;"/><span id="t163m_trs4"class="t_setbtn t_settrs">重置</span></td></tr><tfoot><tr><td colspan="2"style="line-height:15px">说明：【图片尺寸】 选择图片框默认显示图片的尺寸<br/>　　　【点击图片打开查看】 通过点击缩略图打开图片框<br/>　　　【图片缩放】 图片缩放的步进变动值与可缩放的最小、最大倍数<br/>　　　【查看框】　 查看框被拖动前是否相对固定不动，经常看长图片建议关闭<br/>　　　　　　　　　 在查看中等大小图片时顶部是否显示原图切换键，也可双击图片切换到原图<br/>　　　　　　　　　 当查看框被网页内其他元素遮挡时可通过增大Z-Index值解决</td></tr></tfoot></table><br/><table class="t_set_tb"border="0"cellspacing="0"cellpadding="0"width="90%"><thead><tr><td colspan="2"align="left"><b>显示效果</b></td></tr></thead><tr><td>评论框高度 <input id="t163m_setcfh"class="t_setnum"type="text" style="width:30px;" /> <span id="t163m_trs7" class="t_setbtn t_settrs">重置</span>　　<span>自动隐藏资讯: <input type="checkbox" id="t163m_setcui" /></span>　　<span>隐藏个性推荐提醒数字: <input type="checkbox" id="t163m_sethiderec" /></span></td></tr></tr><tfoot><tr><td colspan="2">说明：设置视频框和评论框的大小，请不要超过浏览器可视范围<br/>　　　自动隐藏微博列表上面的今日热点资讯</td></tr></tfoot></table><div class="t_set_ft"style="width:95%"><table cellspacing="0"cellpadding="0"style="width: 100%;"><tr><td style="width: 50%;text-align:left;"><a href="http://t.163.com/rok"target="_blank"></a></td><td style="width: 50%;text-align:right;" class="t_setftbtn"><span id="t163m_setsave"class="t_setbtn t_savbtn">保存</span>　<span id="t163m_setcancel"class="t_setbtn">取消</span>　<span id="t163m_sethelp"class="t_setbtn">帮助</span>　<span id="t163m_setud"class="t_setbtn">检查更新</span></td></tr></table></div></div></div>';

var iRad=$('div.roll-ad');
if(iRad.length>0){
    iRad.hide();
}
...
function t_Menu(){
	if($('#t163m_setting').length<=0){
		$('<li><a id="t163m_setting" style="cursor:pointer;">t163More</a></li>').insertAfter($('#headNav ul.navi-right li:first ul.navi-sublist-list li:last'));
		$('#t163m_setting').click(function(){t_Settings();});
	}
}
...
function t_closeUpInfo(){
    var iUpInfo=$('#wrapper .conL div.promoteBox');
    if(iUpInfo.length>0){
        setTimeout(function(){iUpInfo.slideUp();},3500);
    }
    
}

var trcClear = 0;
function t_clearRCount(){
	if(trcClear == 1){
		return;
	}
	trcClear =1;
	
	var recItem = $('#moduleNavi li.feedTimeline-trigger span.feed-counter');
	if(recItem.length>0){
		var fCount = recItem.text().replace(/(^\s*)|(\s*$)/g, '');
		if(fCount != ''){
			var fLnk = recItem.parent('a').attr('href');
			if(fLnk.indexOf('http://')==-1){
			        fLnk = 'http://t.163.com' + fLnk;
			    }    		
		    var cframe = $('<iframe style="top:5px;width:5px;" src="'+fLnk+'" ></iframe>').appendTo('body').hide();
		    $(cframe).one('load',function(){
		    	cframe.remove();
		    });
		    recItem.text('');
			var recMenu = $('#headNav ul.navi-right li.item-info ul.navi-sublist-list li').eq(3);
			if(recMenu.length>0){
				var acSpot = $('#headNav ul.navi-right li.item-info .js-total');
				if(acSpot.length>0){
					aCount=parseInt(acSpot.text());
					if(isNaN(aCount)){
						aCount = 0;
					}
					var fcSpot = recMenu.find('.nav-num')
					if(fcSpot.length>0){
						fCount=parseInt(fcSpot.text());
						if(isNaN(fCount)){
							fCount = 0;
						};
						fcSpot.text('');
					}
					aCount = aCount - fCount
					if(aCount == 0){
						acSpot.parent().removeClass('notice-info-show');
					}
					acSpot.text(aCount);
				}
			}
...
function t_hideRec(){
	...
	t_clearRCount();
	$('#moduleNavi li.feedTimeline-trigger span.feed-counter').contentChange(t_clearRCount);
}

function t_More(){
        
        $(document).on('mouseenter','div.mainBox div.mainBox-bd li',function(){
        	t_bePrepare($(this));
        });
        $(document).on('mouseleave','div.mainBox div.mainBox-bd li',function(){
        	$('#t163m_more').remove();
    	});

        $(document).on('mouseenter','#singleTweet',function(){
        	t_bePrepare($(this));
        });
        $(document).on('mouseleave','#singleTweet',function(){
        	$('#t163m_more').remove();
    	});
}

function t_bePrepare(sItem){
	var tShow = false;
	var frOp = sItem.find('div.js-foot div.tweet-option');
	if(frOp.length==0){return};

    t_removeMore();
    t_setMore();

	var tmpItem = sItem.find('div.js-tweet');
	var tmpId = tmpItem.attr('data-tweet-id');
    rtId = tmpId;
	
	$('#t163m_reply').hide();
	
	$('#t163m_img').hide();
    var imgItem = t_chkImg(sItem);
    if(imgItem.len>0){
        $('#t163m_img').show();
        tShow=true;
    }
    $('#t163m_chat').hide();
    if(frOp.text().indexOf('评论(')!=-1){
        $('#t163m_chat').show();
        tShow=true;
    }
	
    $('#t163m_ccmt').hide();
    var tCft = 'div.tweet-root div.tweet-ft div.tweet-option';
    var tCft = sItem.find(tCft);
    if(tCft.length>0){
        if(tCft.text().indexOf('原文评论(')!=-1){
            $('#t163m_ccmt').show();
	        tShow=true;
        }
    }
    if(tShow){
		$('#t163m_more').prependTo(frOp).show();
		t_setMenu(imgItem,sItem);
	}
}

function t_chkImg(sItem){
	var isrc = '';
	var isrc2 = '';
	var t_pimg=sItem.find('div.tweet-preview-pic img');
	isrc = 'src';
	isrc2 = 'data-realurl';
    if(t_pimg.length==0){
    ...
    var t_img=[];
    var tpStart,tpEnd;
    for(k=0;k<t_pimg.length;k++){
    	t_img[k] = t_pimg.eq(k).attr(isrc);
    	if(typeof t_img[k] == 'undefined' || t_img[k] == false){
    		t_img[k] = t_pimg.eq(k).parent().attr(isrc2);
    	}else{
	    	tpStart = t_img[k].indexOf('url=http');
	    	tpEnd = t_img[k].indexOf('&gif=');
	    	t_img[k] = decodeURIComponent(t_img[k].substring(tpStart+4,tpEnd));
    	}
	}
    if(timgLarge != 'checked'){
    	for(k=0;k<t_img.length;k++){
	    	t_img[k] = 'http://timge' + (parseInt(Math.random()*8)+1) + '.126.net/image?w=440&url=' + encodeURIComponent(t_img[k]) + '&gif=0&quality=85';
    	}
    }
	return {url:t_img,item:t_pimg,len:t_pimg.length};
}

function t_setMore(){
    $('<span id="t163m_more"><ul class="t_topnav t_customCss"><li><a id="t163m_img">图</a></li><li><a id="t163m_reply">RT</a></li><li><a id="t163m_ccmt">原评</a></li><li><a id="t163m_chat">评</a></li></ul></span>').appendTo('body').hide();
}

function t_removeMore(){
	if($('#t163m_more').length>0){
		$('#t163m_more').remove();
	}
}

function t_setMenu(sImg,sItem){
	if(sImg.len>0){
	    $('#t163m_img').bind('click',function(){
	        var imgInfo = t_getimgInfo(sItem);
        	imgList = $('li.liItem div.tweet-preview-pic img');
	        curImg = imgList.index(sImg.item.eq(0));
	        t_showImgbox(sImg.url[0],imgInfo,false);
	    });
	    if(timgAction==1){
	    	sImg.item.parents('div.tweet-preview-pic').mouseenter(function(){
	    		
	    		$(this).click(function(){
	    			sItem.find('div.tweet-media-detail').remove();
	    			$(this).parent().addClass('t_showon');
			    	twImgid = sImg.item.index($(this).find('img'));
		            var imginfo = t_getimgInfo(sItem);
		        	imgList = $('li.liItem div.tweet-preview-pic img');
		            curImg = imgList.index(sImg.item.eq(twImgid));
		            t_showImgbox(sImg.url[twImgid],imginfo,false);
	    		});
	     	});
	    	sImg.item.parents('div.tweet-preview-pic').mouseleave(function(){
	    		if(sItem.find('div.tweet-media-detail').length<=0){
	    			$('<div class="tweet-media-detail js-detail"> </div>').insertAfter($(this).parent());
		    	}
	    		$(this).unbind('click');
	    	});
	    }else{
	    	//sImg.item.parents('div.tweet-media-preview').unbind('mouseenter').unbind('mouseleave');
	    }
	}
    
    $('#t163m_ccmt').bind('click',function(){
        t_showOriCmt(sItem);        
    });
    
    $('#t163m_chat').bind('click',function(){
        t_showCurCmt(sItem); 
    });
}

function t_getimgInfo(sItem){
	var rtspot = 'p.tweet-message:first';
	var qtspot = 'div.tweet-root p.tweet-message';
	var rtuser = sItem.find('div.tweet-hd:first span a');
	var qtuser = sItem.find('div.tweet-root div.tweet-hd span a');
    var imgInfo = '<a class="link-la" href="'+ rtuser.attr('href') +'" target="_blank">@' + rtuser.text() + '</a>：' + sItem.find(rtspot).html();
    var qtMsg = sItem.find(qtspot);
    if(qtMsg.length>0){
        imgInfo = imgInfo + ' <b>回复：</b>' + '<a class="link-la" href="'+ qtuser.attr('href') +'" target="_blank">@' + qtuser.text() + '</a>：' + qtMsg.html();
    }
    return imgInfo;
}

function t_getfdItem(sImg){
	return sImg.parents('li.liItem');
}

function t_showImgbox(sImg,sInfo,isNav){
    isCancelimg = false;
    tframeLoaded = 0;
    var sleft = (document.documentElement.clientWidth - 550 ) /2;
    $.blockUI({
        message: '<div id="t163m_imgloading" class="t_showframe t_txtshow t_loadshow"><div id="t163m_loadtxt"><img src="'+ loadpic +'" />　载入中,若图片太大需时读入,请稍候.若长时间无响应则可能图片已被删除<br>双击中间定位查看,点击左、右两边切换上、下一张图或点击背景返回</div><div class="t_imgprev"></div><div class="t_imgnext"></div></div>',
        css: {
                border: '',
                width: '520px',
                left: sleft+'px',
    ...
    if(isNav){
        setTimeout(function(){
            t_scrollToLoading(t_getfdItem($(imgList.get(curImg))));
        },500);
    }else{
        t_scrollToLoading(t_getfdItem($(imgList.get(curImg))));
    }
...
function t_scrollToLoading(sItem){
	$('#t163m_loadtxt').dblclick(function(){
		$('html,body').animate({scrollTop: sItem.offset().top-50}, 500);
	});
}

function t_showImg(sImg,sW,sH,sInfo){
    if(tframeLoaded==1){
        return;
    }
    ...
    var imgInfo = sInfo;
    $.blockUI({ 
       message: '<div id="t163m_imgframed"><div class="t_imgtitle"><ul class="t_topnav t_navflnone t_customCss"><li><a id="t163m_imgla"class="t_imgbtn">原图</a></li></ul></div><div id="t163m_imgarea"><canvas id="t163m_canvas"style="display:none;"></canvas><img id="t163m_showimg"src="'+ sImg + '"title="'+sHint+'"/><div class="t_imgprev"><span></span></div><div class="t_imgnext"><span></span></div><span id="t163m_turnspan"><a id="turnLsign"class="t_turnsign"><img src="'+ tleftimg +'"title="向左转"/></a>&nbsp;<a id="turnRsign"class="t_turnsign"><img src="'+ trightimg +'"title="向右转"/></a></span><span id="t163m_zoomspan"><a id="tZoomori"class="t_turnsign"><img src="'+ tzoriimg +'"title="还原"/></a>&nbsp;<a id="tZoomout"class="t_turnsign"><img src="'+ tzoutimg +'"title="缩小"/></a>&nbsp;<a id="tZoomin"class="t_turnsign"><img src="'+ tzinimg +'"title="放大"/></a></span></div><div class="t_imginfo">'+ imgInfo + '<div><span id="t163m_imglocate"class="t_imgurl"><b><a>定位</a></b></span> | <span class="t_imgurl"><b><a target=_blank href='+sImg+'>另存</a></b></span></div><span style="float:right"><ul class="t_topnav t_customCss"><li><a id="t163m_imgrt"class="t_imgbtn">返回</a></li></ul></span></div></div>',
       css: {
            top: stop + 'px', 
            left: sleft + 'px',
            width: sW+20+'px',
    ...
    $('#t163m_imgrt').bind('click',function(){
        $.unblockUI();        
    });
    $('#t163m_imglocate').click(function(){
		$('html,body').animate({scrollTop: t_getfdItem($(imgList.get(curImg))).offset().top-50}, 500);
		$.unblockUI();
    });
    ...
    $('#t163m_imgla').click(function(){
		var i = sImg.indexOf('url=');
		var j = sImg.indexOf('&gif=');
		t_showImgbox(decodeURIComponent(sImg.substring(i+4,j)),sInfo,true);
    });
    if(sImg.indexOf('http://timge')>-1){
    	if(timgTitle=='checked'){
    		$('#t163m_imgframed div.t_imgtitle').show();
    	}
    	$('#t163m_imgarea').dblclick(function(){
    		$('#t163m_imgla').click();
    	});
    }else{
    	$('#t163m_imgarea').dblclick(t_toogleImgSize);
	}
    $('#t163m_imgframed').hover(function(){
            $('#t163m_turnspan').css('display','block');
            $('#t163m_zoomspan').css('display','block');
        },
        function(){
            $('#t163m_turnspan').css('display','none');
            $('#t163m_zoomspan').css('display','none');
        }
    )
    
    $('.blockPage').css('background-color','transparent');
    $('.blockPage .t_imginfo a').not('#t163m_imgrt').attr('target','_blank');
...
function t_setNaviAction(){
    if(curImg>0){
        $('div.t_imgprev').bind('click',function(){
	       	$.unblockUI();
            var fdItem = t_getfdItem($(imgList.get(curImg)));
            var tNavimg = t_chkImg(fdItem);
            curImg--;
            if(twImgid>0){
            	twImgid--
	        }else{
	        	fdItem = t_getfdItem($(imgList.get(curImg)));
	        	tNavimg = t_chkImg(fdItem);
	        	if(tNavimg.len>1){
	        		twImgid = tNavimg.len - 1;
	        	}else{
	        		twImgid = 0;
	        	}
	        }
            var imginfo = t_getimgInfo(fdItem);
            $.unblockUI();
            t_showImgbox(t_chkImg(fdItem).url[twImgid],imginfo,true);
        }).mouseenter(function(){
    ...
    if(curImg<imgList.length-1){
        $('div.t_imgnext').bind('click',function(){
            ...
            var fdItem = t_getfdItem($(imgList.get(curImg)));
            curImg++;
            if(twImgid<t_chkImg(fdItem).len-1){
            	twImgid++;
            }else{
            	fdItem = t_getfdItem($(imgList.get(curImg)));
            	twImgid = 0;
		...
		$('div.t_imgnext').bind('click',function(){
			if($('div.mainBox-page ul li.js-next').length>0){
				var nextp = $('div.mainBox-page ul li.js-next');
				nextp.click();
				t_rtHint(4);
				$.unblockUI();
			}else{
				var nextp = $('div.mainBox-page a span');
				if(nextp.length>0){
					nextp.click();
				}
				$('html,body').animate({scrollTop: $('div.mainBox-page').offset().top}, 500);
				t_rtHint(3);
				setTimeout(function(){
			        imgList = $('li.liItem div.tweet-preview-pic img');
		            var fdItem = t_getfdItem($(imgList.get(curImg)));
		            var imginfo = t_getimgInfo(fdItem);
		            $.unblockUI();
		            t_showImgbox(t_chkImg(fdItem).url[0],imginfo,true);
		            t_rtHintOut();
	            },2500);
	        }
		});
...
function t_imgFrametune(){
    var cvW = $('#t163m_canvas').width();
    $('.blockPage').css('width',cvW+20+'px');
    
    var cvH = $('#t163m_canvas').height();
...
function t_imgZoom(imode){
    ...
    rotate(document.getElementById("t163m_canvas"),document.getElementById("t163m_showimg"));
    t_imgFrametune();    
}
...
function t_toogleImgSize(){
    ...
    var oriImgW = $('#t163m_showimg').width();
    var oriImgH = $('#t163m_showimg').height();
    if(rot!=0 && rot!=180){
        var tmp = oriImgH;
        oriImgH = oriImgW;
        oriImgW = tmp;
    ...
    var cvH = $('#t163m_canvas').height();
    var cvShow = 1;
    if($('#t163m_canvas').css('display')=='none'){
        cvW = oriImgW;
        cvH = oriImgH;
        cvShow = 0;        
    };
...
function t_showCurCmt(sItem){
	var uLnk = 'div.tweet-hd:first a.tweet-time';
    var uLnk = sItem.find(uLnk).attr('href');
    t_popupChat(uLnk);        
}

function t_showOriCmt(sItem){
	var uLnk = 'div.tweet-root div.tweet-hd a.tweet-time';
    uLnk = sItem.find(uLnk).attr('href');
    t_popupChat(uLnk);        
}
...
function t_popupChat(sLnk){
    ...
    var sleft = (document.documentElement.clientWidth - 550 ) /2;
    $.blockUI({
        message: '<div id="t163m_imgloading" class="t_showframe t_txtshow t_loadshow"><img src="'+ loadpic +'" />　评论加载中,请稍候,若无响应请点击背景返回再试一次</div>', 
        css: {
                border: '',
                width: '520px',
                left: sleft+'px',
    ...
    $(cframe).one('load',function(){
        ...
        cframe.appendTo($('#t163m_chatdiv')).show();
	    $('.blockOverlay').css('z-index',zTop-100);
    	$('.blockPage').css('z-index',zTop);
        $('.blockPage').css('background-color','transparent');
        $('.blockOverlay').click(function(){
...
function t_rtHintpos(){
    var iW = $('#t163m_rthint').width() +20;
    var iH = $('#t163m_rthint').height() +10;
    var ileft = (document.documentElement.clientWidth - iW)/2;
    $('#t163m_rthint').css('top',(document.documentElement.clientHeight - iH -200)/2+'px').css('left',ileft+'px');
}
...
function t_rtHint(iMode){
    ...
    switch(iMode){
        case 0:
        case -1:
            if(iMode==0){hint='发送'}else{hint='评论加载'}; 
            hint +='失败，请重试';
            break;
        case 1:
            hint = '发送成功';
        ...
        case 5:
        	...
        	break;
        case -2:
            hint = '发送内容为空或超出163个字符，请检查';
            break;
        default:
            break;
    }
    if($('#t163m_rthint').length==0){
        $('<div id="t163m_rthint" class="t_rthint_n">'+ hint +'</div>').appendTo('body').hide();
        t_rtHintpos();        
    }
    if(iMode == 3 ){
        $('#t163m_rthint').show();
    }else{
        $('#t163m_rthint').fadeOut(100,function(){
            $('#t163m_rthint').html(hint);
            t_rtHintpos();
            var hold = 1500;
            if(iMode<=0){
                $('#t163m_rthint').removeClass('t_rthint_n').addClass('t_rthint_f');
                hold = 2500;
            }        
            $('#t163m_rthint').fadeIn(200,function(){
...
function t_rtHintOut(){
    $('#t163m_rthint').fadeOut(200,function(){
        $('#t163m_rthint').remove();
    });
}

function t_Settings(){
    var stop = (document.documentElement.clientHeight - 450-150) /2;
    ...
    $('.blockPage').css('z-index',zTop);
    $('.t_set_ft a:first').text('t163more v'+ t163m_version + ' - forumz @ ' + t_rdate);
    $('.blockPage').css('background-color','transparent');
    if(timgLarge == 'checked'){
    	$('#t163m_imgLarge').attr('checked','checked');
    }else{
    	$('#t163m_imgMid').attr('checked','checked');
    }
    if(timgTitle == 'checked'){
    	$('#t163m_setimgTitle').attr('checked','checked');
    }
    if(timgLock == 'checked'){
    	$('#t163m_setimgLock').attr('checked','checked');
    }
    if(timgAction==1){
    	$('#t163m_setimgAct').attr('checked','checked');
    }
    if(timgShowNav==1){
    	$('#t163m_setimgNav').attr('checked','checked');
    }
    if(thideRec==1){
    	$('#t163m_sethiderec').attr('checked','checked');
    }
    for(k=0;k<asetTrs.length;k++){
        $('#'+asetTrs[k]).val(cuTset[k]);
    }
    if(cUpinfo=='checked'){
        $('#t163m_setcui').attr('checked','checked');
    }
    $('#t163m_setdiv input:radio').checkbox({cls:'t_jradbox',empty:cbemptyimg});
    $('#t163m_setdiv input:checkbox').checkbox({cls:'t_jchkbox',empty:cbemptyimg});

    $('.t_settrs').click(function(){
        var cid = $(this).attr('id');
        cid = cid.substring(cid.length-1)-1;
        $('#'+asetTrs[cid]).val(defTset[cid]);
    })
    ...
    $('#t163m_setud').click(function(){
        update(true);
    });
    $('#t163m_sethelp').click(function(){
       window.open('http://userscripts.org/scripts/show/82498');
    });
    $('#t163m_setsave').click(function(){
    	if($('#t163m_imgLarge').attr('checked')=='checked'){
    		GM_setValue('tnm_imglarge','checked');
    	}else{
    		GM_setValue('tnm_imglarge',false);
    	}
    	if($('#t163m_setimgTitle').attr('checked')=='checked'){
    		GM_setValue('tnm_imgtitle','checked');
    	}else{
    		GM_setValue('tnm_imgtitle',false);
    	}
    	if($('#t163m_setimgLock').attr('checked')=='checked'){
    		GM_setValue('tnm_imglock','checked');
    	}else{
    		GM_setValue('tnm_imglock',false);
    	}
    	if($('#t163m_setimgAct').attr('checked')=='checked'){
    		GM_setValue('tnm_imgact','1');
    	}else{
    		GM_setValue('tnm_imgact','0');
    	}
    	if($('#t163m_setimgNav').attr('checked')=='checked'){
    		GM_setValue('tnm_imgnav','1');
    	}else{
    		GM_setValue('tnm_imgnav','0');
    	}
        if($('#t163m_setcui').attr('checked')=='checked'){
            GM_setValue('t_cupinfo','checked');
        }else{
            GM_setValue('t_cupinfo',false);
        }
    	if($('#t163m_sethiderec').attr('checked')=='checked'){
    		GM_setValue('tnm_hiderec','1');
    	}else{
    		GM_setValue('tnm_hiderec','0');
    	}
		if(parseInt($('#'+asetTrs[3]).val())<=1000){$('#'+asetTrs[3]).val('10001');}
    	GM_setValue('tnm_scstep',$('#'+asetTrs[0]).val());
    	GM_setValue('tnm_scmin',$('#'+asetTrs[1]).val());
    	GM_setValue('tnm_scmax',$('#'+asetTrs[2]).val());
    	GM_setValue('tnm_ztop',$('#'+asetTrs[3]).val());
		GM_setValue('tnm_cfh',$('#'+asetTrs[4]).val());
		t_getSettings();
    	$.unblockUI();
    	location.reload();
    });
    $('#t163m_setcancel').click($.unblockUI);
    $('.blockOverlay').click(function(){
        $.unblockUI();
    });
}
...
function t_FRshow(){
	if(firstRun>0){
		var tboxleft=0;
		var accitem= $('#headNav ul.navi-right li:first');
		if(accitem.length>0){
			tboxleft = accitem.offset().left;
		}
		if(tboxleft>0){
			$('<div id="t163m_tbox" class="t_frshow"><div class="t_txtshow t_tbox">感谢使用 <b>t163more</b><br />更多功能选项可通过菜单【t163more】进行设置<br /><s><i></i></s></div></div>').appendTo('body').hide();
			$('#t163m_tbox').css('left',tboxleft-155).show().mouseenter(function(){
				GM_setValue('tnm_fr','0');
				$(this).remove();
			});
		}
	}
...
{
  autoUpdateFromUserscriptsDotOrg({
    url: 'http://userscripts.org/scripts/source/82498.user.js',
    version: t163m_version,
    forceUpdate: forceUpdate,
    web: 'http://userscripts.org/scripts/show/82498'
...
if(GM_registerMenuCommand){
	GM_registerMenuCommand('从 userscript.org 更新 t163more',function(){update(true)});
}
