// ==UserScript==
// @name           Imperion Mod von PhasmaExMachina Deutsche Version von MpShake
// @namespace      MpShake
	...
	"Generell":{
		...
		fields:{
			planetList:{
				type:'checkbox',
				label:'Planetenliste',
				text:'Ersetzt die Planetenlsite durch eine ausführlichere Version',
				value:true
			},
			resourceDetails:{
				type:'checkbox',
				label:'Ressourcendetails',
				text:'Zeigt die Ressourcenproduktion pro Stunde an',
				value:true
			},
			resourceCapacity:{
				type:'checkbox',
				label:'Ressourcenkapazität',
				text:'Zeigt die verbleibende Ressourcenkapazität der einzelnen Lager an',
				value:false
			},
			useResourceIcon:{
				type:'checkbox',
				label:'Ressourcenbilder',
				text:'Ersetzt einige Ressourcennamen durch Bilder',
				value:true
			},
			colorizeTimers:{
				type:'checkbox',
				label:'Farbige Zeitangaben',
				text:'Färbt die einige Zeitangaben in die Farbe der Rasse ein',
				value:true
			},
			trackEvents:{
				type:'checkbox',
				label:'Ereignisliste',
				text:'Zeigt eine Liste mit Ereignissen an und erstellt einen Button',
				value:true
			}
		}
	},
	"Gebäudeliste":{
		...
		fields:{
			buildingList:{
				type:'checkbox',
				label:'Freigabe',
				text:'Zeigt eine Gebäudeliste von dem jeweiligen Planeten an',
				value:true
			},
			buildingListIcons:{
				type:'checkbox',
				label:'Bilder der Gebäude',
				text:'Zeigt die einzelnen Bilder der Gebäude in der Liste',
				value:true
			},
			buildingListMissingResources:{
				type:'checkbox',
				label:'Verbleibende Ressourcen',
				text:'Zeigt die Menge der Ressourcen an bis das Gebäude ausgebaut werden kann',
				value:true
			}
		}
	},
	"Karte":{
		html:'<p>Diese optionen verändern die Anzeigen der Karte.</p>',
		fields:{
			bigMap:{
				type:'checkbox',
				label:'Große Sternenkarte',
				text:'Vergrößert die Sternenkarte',
				value:true
			},
			addMapCommas:{
				type:'checkbox',
				label:'Kommasetzung',
				text:'Setzt Kommas in den Zahlenwerten der Karte',
				value:true
			},
			autoHarvest:{
				type:'checkbox',
				label:'Auto Recyceln',
				text:'Recycelt automatisch Kometen in näherem Umfeld',
				value:false
			}
		}
	},
	"Anderes":{
		html:'<p>Diese Optionen beziehen sich auf bestimmte Gebäude oder Anzeigen.</p>',
		fields:{
			fleetBaseSummary:{
				type:'checkbox',
				label:'Flottenübersicht',
				text:'Zeigt eine Gesammtansicht der Flotte von jedem Planeten im FLottenstützpunkt',
				value:true
			},
			fleetBaseFullPercentage:{
				type:'checkbox',
				label:'Anteil der Fracht',
				text:'Zeigt den Anteil in Prozent den die Beute in den Frachträumen der Flotte verbraucht',
				value:true
			},
			planetBuildingLevels:{
				type:'checkbox',
				label:'Gebäudestufen',
				text:'Zeigt die Gebäudestufen auf der Planetenübersicht an',
				value:true
			}
		}
	},
	"Terraner":{
		html:'<p>Diese option ist nur für Terraner.',
		fields:{
			usingLargeRecylers:{
				type:'checkbox',
				label:'Große Recycler',
				text:'Der Frachtraum der großen Recycler wird beim Berechnen der Recycleranzahl beachtet',
				value:true
			}
		}
	},
	"Xen":{
		html:'<p>Diese option ist nur für Xen.',
		fields:{
			reportRefreshLink:{
				type:'checkbox',
				label:'Aktualisierungsbutton',
				text:'Fügt einen Aktualisierungsbutton im Brain hinzu um die Spionageberichte zu aktualisieren',
				value:true
			}
		}
	},
	"About":{
		html:'<p>Dieses Script erstellt verschiedene Basismodifikationen.<Br/>Zum Speichern der Einstellung auf Save Settings drücken.</p><p><strong>Autor:</strong> <a href="http://userscripts.org/users/PhasmaExMachina">PhasmaExMachina</a></p>'	}
};
		...
		brain:function() {
			...
			if(loc.match(/brain\/report\/\d+\/target\/\d+\//)) {
				...
				if(Config.get('reportRefreshLink')) {
					...
					button.innerHTML = '<a href="#">[Aktualisieren]</a>';
		...
		fleetBase:function() {
			...
			if($('ul.tabs:eq(0) li:first a.active').size() == 1) {
				...
				if(Config.get('fleetBaseSummary')) {
					...
					h.innerHTML = '<br/>Flottenübersicht';
