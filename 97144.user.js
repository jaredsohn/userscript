...
// select "VideoGamer - EZ Download", and click Uninstall.
//
// --------------------------------------------------------------------
//
// ==UserScript==
// @name          VideoGamer - EZ Download
// @namespace     http://www.Daem0nX.com (ByronRogers@gmail.com)
// @description   Adds menu (bottom right) with support for SD/HD video file downloads.
// @version		  0.1
// @notes		  Initial release.
// @include       http*://www.videogamer.com/videos/*
// ==/UserScript==
...
var scriptName = 'VideoGamer - EZ Download';
var scriptId = '97144';
var scriptVersion = '0.1';
var scriptUpdateText = 'Initial release.';
// === Stop editing here. ===

var debugUpdate = false;
var debugSettings = false;
var donationLink = 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=6C6Z4Y9P462P4';
...
+'#gmmnu #gmmnu_title {text-align:center;font-weight:bold;font-size:9pt;color:#333;}'
...
//alert(curwindow);
//alert($ljq("#playerRow script:contains('vidId')").html());
Pageid = $ljq("#playerRow script:contains('vidId')").html();
pattern = /.*?vidId\s?=\s?([0-9]+)/ig;
...
$ljq("#gmmnu").html(""
		+"\n<div id='gmmnu_title'>VideoGamer Download</div>\n"
		+"<div id='gmmnu_menu'>\n"
		//+"<div class='header'>Menu</div>\n"
		//+"<div>1st | 2nd</div>\n"
			+"<div class='menubar'>\n"
				+"<div id='update_icon'>"
					+"<a href='#update' class='update_grey' title='No Updates'></a>"
					//+"<a href='#update' class='update' title='Update'></a>"
				+"</div>\n"
				+"<div id='donate_icon'><a href='"+donationLink+"' class='donate' title='Donate! =D' target='_blank'></a></div>\n"
				+"<div id='home_icon'><a href='http://userscripts.org/scripts/show/"+scriptId+"' class='home' title='Script Homepage' target='_blank'></a></div>\n"
			+"</div>\n"
		+"</div>\n"
...
$ljq(document).ready(function(){
	...
	if (Pageid !== '') {
		if ($ljq("a[class='streamHD']").length > 0) {
			//alert("SD/HD");
			getSDfile();
			getHDfile();
		} else {
	...
	$ljq("#gmmnu_menu .menubar div a").hover(
		function () {
			...
			$ljq("#gmmnu #gmmnu_title").html($ljq(this).attr("title"));
		},
		function () {
			$ljq("#gmmnu #gmmnu_title").html('VideoGamer Download');
		}
	);
...
function getSDfile() {
//alert('http://feeds.videogamer.com/siteflash/videodata.xml?type=hi&vid='+Pageid);
$ljq("#gmmnu_links_sd .status").html("Loading...");
	GM_xmlhttpRequest({
    	method: "GET",
    	url: 'http://feeds.videogamer.com/siteflash/videodata.xml?type=hi&vid='+Pageid+'',
    	headers: {'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey','Accept': 'text/plain',},
    	onload: function(res) {
    		...
    		if (status == '200') {
    			...
    			try {
					...
					var SDLink = matches[0].replace(pattern, "$1");
					//alert(SDLink);
...
function getHDfile() {
//alert('http://feeds.videogamer.com/siteflash/videodata.xml?type=hd&vid='+Pageid);
$ljq("#gmmnu_links_hd .status").html("Loading...");
	GM_xmlhttpRequest({
    	method: "GET",
    	url: 'http://feeds.videogamer.com/siteflash/videodata.xml?type=hd&vid='+Pageid+'',
    	headers: {'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey','Accept': 'text/plain',},
    	onload: function(res) {
    		var status = res.status;
    		//+"\n"+res.responseHeaders;
    		...
    		if (status == '200') {
    			...
    			try {
					...
					matches = text.match(pattern);
					//alert(matches[0].replace(pattern, "$2"));
					var HDLink = matches[0].replace(pattern, "$1");
					//alert(SDLink);
