// ==UserScript==
// @name           Fenix Helper
// @namespace      http://heiho.wxw.dk/heihohelper/
// @description    Interface Enhancement of Astroempires
// @include        http://fenix.astroempires.com/*
// @include        http://heiho.wxw.dk/*
// @exclude 	     http://heiho.wxw.dk/menu-frame.php
// @exclude 	     http://heiho.wxw.dk/menu.php
// @exclude        http://heiho.wxw.dk/heihohelper/scoutframe.php*
// @exclude 	     http://heiho.wxw.dk/*noshow
// @exclude 	     http://*.astroempires.com/home.aspx*
// @exclude 	     http://*.astroempires.com/login.aspx*
// @exclude        http://heiho.wxw.dk/forum/*
...
function initScoutingInterface() {
	...
	function closeScoutFrame() {
		...
		var form = '<form id="closescoutframe" method="GET" action="'+dbase_url+'heihohelper/scoutframe.php" target="_top">';
    ...
    for (var i=0; i<tables.length; i++) {
      ...
      if (isownfleet && tables[i].firstChild && tables[i].firstChild.firstChild && tables[i].firstChild.firstChild.firstChild && tables[i].firstChild.firstChild.firstChild.childNodes[1] && tables[i].firstChild.firstChild.firstChild.childNodes[1].nodeValue=='VERVIEW') {
        ...
        if (unitlist=='Scout Ship' || unitlist=='Corvette') {
          var html = '<form target="_top" action="'+dbase_url+'heihohelper/scoutframe.php">';
...
function startRegionScanners() {
  ...
  window.setTimeout(function() {
    ...
    if (syslocs_str) {
      ...
      setState('scanner_report_url',dbase_url+'index.php'+"?loc="+currentloc.slice(0,6));
		...
		if (t.firstChild) {
			...
			if (t.firstChild.firstChild.firstChild.nodeValue=='Structures') {
				...
				for (var j=0; j<buildingnames.length; j++) {
					if (buildingnames[j] == 'Command Centers')
						b.defences += 'Command Centers\t'+buildingnumbers[j]+'<br>';
					if (buildingnames[j] == 'Jump Gate')
...
function soundAlarm() {
	...
	moveOnAlarm();
	if (alarmmakenoise) {
		alarmiframe = node({html: '<iframe src="'+dbase_url+'heihohelper/alarm.html" width="1" height="1">', style: {visibility: 'hidden', position: 'absolute', left: '0px', top: '0px'},
      append: document.body});
...
function loadSystemScript() {
    ...
    GM_xmlhttpRequest({
        method: 'POST',
        url: dbase_url+'aej_loadsystem.php',
        headers: {'Content-type':'application/x-www-form-urlencoded'},
...
function initRightMenu() {
  ...
  function deleteBookmark() {
	...
	var l=createMenuLink(dbase_url+'forum/','Forum',0);
	w.appendChild(l);
	var query = currentloc ? '?loc='+currentloc.slice(0,6) : '';
	w.appendChild(createMenuLink(dbase_url+query,'Database',0));
...
function loadSystemPopup() {
  ...
  if (res && res[4] && res[4].length==9) {
    ...
    popup({ url: dbase_url+'aej_loadlocinfo.php',
...
if (isScoutFrame) {
	...
	if (!isWindowScanning())
   if (getState('o_showRightSideBar',0)) { 
	initRightMenu();
}
	initSaveDataNotify();
	if (pagetype=='mapRegion')
