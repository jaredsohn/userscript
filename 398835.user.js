...
(function () {
    ...
    var rVi = /[a-z0-9-]+\.ytimg\.com\/vi\/([^\/]+)\//i,
        ...
        menulist, __preview_timeout, __animation_interval;
    ...
    function handleHoverLogic(urlIdPrefix) {
        ...
        JSL.ajax([
            ...
            onload : function (resp) {
                if (resp.status === 200) {
                    JSL('#hover_img_2').attribute('src', resp.url);
                    JSL.ajax().clear();
                }
            }
        });
    ...
    function show(event) {
        ...
        if ( elem.is('img') && !elem.is('#hover_img, img[id^="hover_img_"]') && urlIdPrefix.getMatch(rVi) ) {
            if (isHoverEnabled) {
    ...
    function hide(event) {
        ...
        // filter out the preview image of this script and thumbnails
        if ( !elem.is('#hover_img, img[id^="hover_img_"]') ) {
            preview.hide();
        }

        // stop animating the current thumbnail
        if ( elem.is('img') && !elem.is('#hover_img, img[id^="hover_img_"]') && elem.attribute('src').getMatch(rWhichImage) ) {
            JSL.clearInterval(__animation_interval);
            elem.attribute( 'src', elem.attribute('src').replace(rWhichImage, '$1mqdefault$3') );
        }
    }
    ...
    JSL.runAt('interactive', function () {
        ...
        // temporary fix for GM_config not returning the correct value
        switch (thumbSize) {
            case 'small': {
                thumbSize = '90'; break;
            }
