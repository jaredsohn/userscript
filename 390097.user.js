...
0.3.0.4
    ...
    - Fix bug in profession end time parsing.
0.3.0.3
    - Profession lists are parsed for time to wait to finish any task in any character and the script will sleep for the shortest found time, but not longer then 2h.
0.3.0.2
...
(function() {
    ...
    function page_PROFESSIONS() {
        ...
        WaitForState("").done(function() {
            $("div.page-professions-overview div.bar-text").each(function(){
                try {
                    [source,h,m,s] = this.innerHTML.match(/(?:(\d+)h)?\s*(?:(\d+)m)?\s*(?:(\d+)s)?/).map(function(e){ return Number(e) || 0});
                   var t = (h*60*60+m*60+s)*1000; //in ms
                   min_task_wait_time = min_task_wait_time > t ? t : min_task_wait_time;
    ...
    function switchChar() {
        ...
        if (++charcurrent >= settings["charcount"]) {
            charcurrent = 0; 
            var sleep_notify = function(){                
                min_task_wait_time -= 60*1000;
                if (min_task_wait_time > 0) {
                    console.log('sleeping for ', min_task_wait_time);
                    $('#sleeping_notify')[0].innerHTML = 'sleeping for ' + min_task_wait_time / 1000 / 60 + 'm';
                    window.setTimeout(sleep_notify, 60*1000);
                }
            };
            sleep_notify();
            window.setTimeout(switchChar, min_task_wait_time);
            return;
        }
    ...
    function addSettings() {
        ...
        AddCss("\
            ...
            #sleeping_notify{font-size: 12px;border-bottom: 1px solid rgb(102, 102, 102); border-right: 1px solid rgb(102, 102, 102); background: none repeat scroll 0% 0% rgb(238, 238, 238); display: block; position: fixed; overflow: auto; right: 46px; top: 0px; padding: 3px; z-index: 1000;}\
        ...
        // Add sleeping notify div to page
        $("body").append('<div id="sleeping_notify"></div>');
