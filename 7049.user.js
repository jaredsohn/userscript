...
// These arrays define what elements will get removed.  You can comment/uncomment 
// what you want or don't want to see, or hunt down and add the ID tags or 
// class name of anything else you want removed.  Comma separated.  Be careful, especially with class names!
var homepageRemove = new Array (
//	"home_profileInfo",
//	"home_messages",
//	"home_bulletins",
//	"home_friends",
	...
	"squareAd",
	"ctl00_Main_ctl00_ProfileHome_gads",  // Google ads
	"home_setHomePage",
	"home_schools",
	"home_infoBar",
	"home_greybox",
...
var mainpageRemove = new Array (
	"splash_coolNewPeople",
	"splash_getStarted",
//	"ctl00_Main_SplashDisplay_userVideos_VideoToday",  // header for videotoday
//	"splash_videotoday",
	"splash_greybox",
//	"splash_music",   //controls both MySpace Music and MySpace Specials
//	"ctl00_Main_SplashDisplay_featuredVideos_CMS_videos",
	"ctl00_Main_SplashDisplay_gads_CMS_Splash_Gads" //randomly displayed google ads
	);
var profileRemove = new Array ( 
//	"Table2"   // Profile pic and all interest contents
//	"mini"      // the Flash music player
//	"Table1"   // Interests (both title and contents) and Extend Network box (will hide the other toggles of this script)
//  "GeneralRow", "ProfileGeneral"  // (or "Television" "Heroes", etc.)  Individual rows or content of the interests section.
//	"contactTable"
//  Find more yourself!
	);
var groupsRemove = new Array ( 
//	"header_search"
	);
var removeOASads = true;   // Nearly all the ads are tied to this
var removeBannerAd = true;  //  The banner ad on some pages needs a different approach
var maxImageWidth = "250";  //  This is the arbitrary image size you can set for the profile annoyance settings.
...
images = document.getElementsByTagName("img");
for (i = images.length - 1; i >= 0; i--) images[i].style.maxWidth=maxImageWidth;
}
...
if (prefs.indexOf("imgwidth") > -1) resizeImages();
}
...
if (document.getElementById("check_width").checked) valueToSet += "imgwidth";
...
if (isBlocked("imgwidth")) document.getElementById("check_width").click();
}
...
document.body.innerHTML += '<div id="obnoxious" style="background-color:#FFFFCC;position:absolute;top:0px;right:0px;width:350px;height:auto;z-index:100001"><table><tr><td align="center" valign="top"><font color="black"><b>Profile Annoyance Options</b></font></td><td align="right" valign="top"><img src='+closebutton+' style="cursor:pointer;" onClick="document.getElementById(\'obnoxious\').parentNode.removeChild(document.getElementById(\'obnoxious\'))"></td></tr><tr><td align="left" valign="top" colspan="2"><font size="1" color="black">Is this profile annyoing you?  Is it time to zap something?  Let\' do it! Just check everything you want to block. To unblock an item, simply return to this panel, and leave that item unblocked.  After unblocking items, you will not see them again until you reload the page.  Note that you may have to block Audio/Video for some tricky flash elements.</font><br><br><center><table border="0"><tr><td><input type="checkbox" id="check_css"></td><td>CSS</td></tr><tr><td><input type="checkbox" id="check_images"></td><td>Images</td></tr><tr><td><input type="checkbox" id="check_flash"></td><td>Flash</td></tr><tr><td><input type="checkbox" id="check_av"></td><td>Audio/Video</td></tr><tr><td><input type="checkbox" id="check_width"></td><td>Image Width</td></tr></table></center><br><center><input type="button" value="Fix It!" id="fixitbutton"></center></td></tr></table></div>';
if (isBlocked("css")) document.getElementById("check_css").click();
if (isBlocked("images")) document.getElementById("check_images").click();
if (isBlocked("flash")) document.getElementById("check_flash").click();
if (isBlocked("av")) document.getElementById("check_av").click();
...
function hideElements (y) {
	if (y.length > 0) {
		var all = document.getElementsByTagName("*");
		for (var k in all)  //(var k=0; k<all.length; k++)
			if (inArray(all[k].id, y) || inArray(all[k].className, y)) 
...
if (unsafeWindow.parent == unsafeWindow){
	...
	if (footer != undefined) 
		footer.innerHTML += "<center><div id=\"adrmvr\" style=\"width:640px;background-color:#FFFFCC;\" align=\"left\">This page was processed by the Myspace Annoyance Removal userscript for Greasemonkey.  If you have any comments or questions, head over to <a href=\"http://userscripts.org/scripts/show/3332\">http://userscripts.org/scripts/show/3332</a> or send an e-mail to danielteichman[xATx]gmail[xDOTx]com  You can help out by reporting pages that get around this script.</div></center><br>";	
