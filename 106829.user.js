// ==UserScript==
// @name       AutoBuild
// @namespace  http://userscripts.org/scripts/show/106829
// @version    2.7.1
// @description  GW Powerbot
...
unsafeWindow.AutoBuilder = {
    ...
    STRTIME: ["","Constr","Invest","Entren","Defens"],
    ...
    CreateInterface: function () {
        ...
        this.UpdateDiv("ENCENDIDO....CORRECTO",1);
    ...
    Collect: function (cty) {
        ...
        setTimeout("Modal.hideModalAll();",4000);
        this.UpdateDiv("Cobro de dinero...",cty);
    },
    CollectInt: function () {
        for(var i = 1; i <= this.NumCity; i++)
        {
    ...
    Build: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('contruccion evento en: ' + AutoBuilder.DisplayTime(Building.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.BuildTimer[cty]);
            ...
            else
            {
                ...
                {
                    clearTimeout(this.BuildTimer[cty]);
                    this.UpdateDiv("Construccion desactivada.",cty);
                    return;
                }
                this.UpdateDiv(arStrings.buildingName["b" + minT] + " Level " + minL + " en " + minS,cty);
    ...
    Train: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('entrenamiento evento en: ' + AutoBuilder.DisplayTime(Barracks.Queue.timeRemaining())," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.TrainTimer[cty]);
    ...
    Stop: function (go, bra) {
        ...
        if(go)
        {
           ...
           this.UpdateDiv("PARADO " + this.STRTIME[bra] + "....",cty);
        }
        else
        {
            this.UpdateDiv("ENCENDIDO " + this.STRTIME[bra] + "....",cty);
            switch (bra)
            {
                case 1:
