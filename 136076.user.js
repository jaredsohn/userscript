// ==UserScript==
// @name           Flickr Quit Multiple Groups
// @namespace      http://www.flickr.com/alesadam
// @description    Allows to quit mutliple groups in one action, from your Groups page.
// @creator        Aleas Dam (http://flickr.com/alesadam/)
// @date           14/06/2012
// @version        2.2
// @modified	   Sep. 20, 2013
//
...
// @match	   http://www.flickr.com/groups*
//
// @downloadURL	   https://userscripts.org/scripts/source/136076.user.js
// @updateURL	   https://userscripts.org/scripts/source/136076.meta.js
// @run-at	   document-end
// ==/UserScript==
//
...
if (!document.location.href.match(/www.flickr.com\/groups\/?(?:\?.*)?$/)) {
	return;
}

var images = {
//    error: 'http://l.yimg.com/g/images/icon_error_x_small.png',
    error: 'http://www.flickr.com/images/icon_error_x_small.png',
    success: 'http://l.yimg.com/g/images/icon_check_small.png',
    updating: 'http://www.flickr.com/images/pulser2.gif'
};

var scriptNumber = 136076;
var installPage = 'http://userscripts.org/scripts/show/' + scriptNumber;
...
if (typeof(GM_log) == "undefined") {
    ...
    var keyPrefix = 'FlickrQuitMultipleGroups.';

    GM_log = function (message) {
        if (Browser.Engine.webkit) {
            console.info("[FQMG] " + message);
        } else {
...
function showUpdateNotification(data) {
    ...
    new Element('a', {
        html: 'Flickr Quit Multiple Groups: ' + (beta ? 'beta ' + version : onlineVersion + ' available'),
        href: installPage,
        target: '_blank',
        title: 'to the ' + (beta ? 'stable version' : 'script\'s install page') + ' (opens in new tab)',
        styles: {
            'color': color,
    ...
    new Element('a', {
        ...
        href: 'http://www.flickr.com/groups/flickrhacks/discuss/72157630064427661/',
        styles: {
            'text-decoration': 'none'
        },
        target: '_blank'
...
function checkVersion(version) {
  try {
    ...
    if (!lastVersionCheckTime || elapsedtime / 1000 > 60 * 60 * 12) { //more then 12h ago
    	getThreadInfo({ threadId: '72157630064427661', callback: function (retval) {
...
if ($chk(lastVersion) && lastVersion != FQMGversion) {
	// cleanup
	GM_deleteValue("version");
	GM_deleteValue("lastVersionCheckTime");
	GM_deleteValue("onlineVersion");
}
checkVersion(FQMGversion);
GM_setValue("version", FQMGversion);


// the real code
var injectionPoint = $('groups-list');
var injectionDirection = 'top';
var nothingSelectedTitle = "first select the groups you want to quit";
var somethingSelectedTitle = "click to quit the selected groups";

new Element('button', {
	'class': 'CancelButt',
	id: 'FQMG-quit-button',
	html: 'Quit selected groups',
	title: nothingSelectedTitle,
	styles: {
		display: 'none',
		visibility: 'hidden'
	},
	events: {
		'update-state': function (evt) {
			var button = $('FQMG-quit-button');
			if ($$('input[id^=FQMG-checkbox]:checked').length > 0) {
				button.addClass('DeleteButt').removeClass('CancelButt');
				button.set('title', somethingSelectedTitle);
			} else {
				button.addClass('CancelButt').removeClass('DeleteButt');
				button.set('title', nothingSelectedTitle);
			}
		},
		'click': function (evt) {
			if (this.hasClass('CancelButt')) return;
			// then, do the quiting
			$$('input[id^=FQMG-checkbox]:checked').each(function (checkbox) {
				var groupId = checkbox.get('id').replace(/^FQMG-checkbox-/, '');
				new Element('img', {
					src: images.updating,
					id: 'FQMG-spinner-' + groupId,
					title: 'quiting group',
					height: '9px;'
				}).replaces(checkbox);
				
				var deletePhotos = $('FQMG-delete-checkbox-' + groupId).checked;
				$('FQMG-delete-checkbox-' + groupId).dispose();
				try {
					$('FQMG-delete-checkbox-label-' + groupId).dispose();
				} catch (e) {
					// ignore
				}
				...
				new Request({
						...
						if (result.stat === 'ok') {
						    try {
							$('FQMG-spinner-' + groupId).getParent('div.group-badge').setStyle('opacity', '0.4');
						    } catch (e) {
						    	// ignore
						    }
						}
						new Element('img', {
							src: result.stat === 'fail' ? images.error : images.success,
							title: result.stat === 'fail' ? "failed: " + result.message : 'done',
							height: '9px'
						}).replaces($('FQMG-spinner-' + groupId));
					    } catch (e) {
					    	GM_log("ERROR processing result: " + e);
						new Element('img', {
							src: images.error,
...
function createCheckbox(data) {
	...
	var parentElement = data.element;

	var checkbox = new Element('input', {
		type: 'checkbox',
		'class': 'FQMG-checkbox',
		id: 'FQMG-checkbox-' + groupId,
		...
		events: {
			change: function (evt) {
				...
				$('FQMG-delete-checkbox-' + groupId).setStyles({ display: checked ? 'inline' : 'hidden', visibility: checked ? 'visible' : 'hidden' });
				$('FQMG-delete-checkbox-label-' + groupId).setStyles({ display: checked ? 'inline' : 'hidden', visibility: checked ? 'visible' : 'hidden' });
				$('FQMG-quit-button').fireEvent('update-state');
			}
		}
	}).inject(parentElement);
	var leaveLabel = new Element('label', {
		html: 'leave this group',
		'for': 'FQMG-checkbox-' + groupId,
		id: 'FQMG-checkbox-label-' + groupId,
		title: 'leave this group?',
		styles: {
			'margin-left': '5px'
		}
	...
	// remove photos
	var removeCheckbox = new Element('input', {
		type: 'checkbox',
		id: 'FQMG-delete-checkbox-' + groupId,
		title: 'delete all photos from group pool?',
		styles: {
			display: 'none',
			visibility: 'hidden',
			'margin-left': '10px'
		}
	}).inject(leaveLabel, 'after');
	new Element('label', {
		'for': 'FQMG-delete-checkbox-' + groupId,
		html: 'remove photos from pool',
		id: 'FQMG-delete-checkbox-label-' + groupId,
		title: 'delete all photos from group pool?',
		styles: {
			display: 'none',
			visibility: 'hidden',
	...
	}).inject(removeCheckbox, 'after');
	return checkbox;
}

$$('li.group-overview').each(function(groupBadge) {
	try {
		var groupId = groupBadge.getElement('img.buddy-icon').get('src').match(/flickr.com\/\d+\/buddyicons\/(\d+@\w\d{2})/)[1];
	} catch (e) {
		// no buddy icon?
		try {
			groupId = groupBadge.getElement('img.buddy-icon').get('src').match(/images\/buddyicon.*(?:.gif|png|jpg)#(\d+@\w\d{2})/)[1];
		} catch (f) {
			GM_log("unable to extract group id: " + e + " - ignoring");
			return;
		}
	}
	var groupMeta = groupBadge.getElement('span span').getParent('span');
	new Element('span', {
		'class': 'divider',
		html: '|'
	}).inject(groupMeta);
	var span = new Element('span', {
		'class': 'first group-count pool-pending',
		nowrap: 'nowrap',
		styles: {
			'white-space': 'nowrap'
		}
	}).inject(groupMeta);
	var checkbox = createCheckbox({ groupId: groupId, element: span });
	// make the quit button available
	$('FQMG-quit-button').setStyles({ display: 'inline', visibility: 'visible' });
