// ==UserScript==
// @name           	Планировщик (Town Planner)
// @namespace      	ikariamLibrary
// @autor           Nick Sewell (перевод на Русский язык by Vit'OS hakergtr@yandex.ru)
// @description    	Позволяет менять расположение зданий не перестраивая их.
// @homepage        http://ika-info.ucoz.ru
// @version         1.2.3
// @history         1.2.3 Информация не известна 
...
ScriptUpdater.check(65507, '1.2.3');
// --/ScriptUpdater--
...
console.log("Town Planner: IslandID = "+islandId);
	...
	GM_registerMenuCommand("Планировщик: " + (islandEdit(null) == false ? "Включить" : "Выключить") + " режим редактирования острова", islandEdit);
	GM_registerMenuCommand("Планировщик: Поменять изображение острова", changeImage);
	GM_registerMenuCommand("Планировщик: Отменить все изменения", resetIsland);
...
console.log("Town Planner: CityID = "+cityId);
	...
	// register editMode Menu Command to enable/disable edit mode
	GM_registerMenuCommand("Планировщик: " + (cityEdit(null) == false ? "Включить" : "Выключить") + " режим редактирования города", cityEdit);
	GM_registerMenuCommand("Планировщик: Отменить все изменения", resetCity);
