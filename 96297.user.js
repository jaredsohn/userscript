// ==UserScript==
// @name           givebux autoclicker
// @namespace      http://userscripts.org/scripts/show/96297
// @include        *givebux.com*
// ==/UserScript==
...
isAds = function(o) {
	try {
		return o.href.match(/cks\.php\?k\=[0-9A-Fa-f]+\&cdk\=flase/) != null
	}
	catch(e) {
		return false
	}
...
if(correctURL(/\/ads\.php/)) {
	if(1) {
		...
		for(i = 0; i < A.length; i++) {
			try {
				...
				if(isAds(A[i]) && A[i].innerHTML.search('cheat') < 0) {
					...
					urls[urls.length - 1].wtime = 30;//getwtime(A[i]);
				}
			}
			catch(e) {}
		};
		...
		html += "</style>";
		html += "<div id='PTCAutoClickerBot'><div id='PTC AutoClicker Bot-title'>JustOneClick for GiveBux.com</div><br /><div id='PTCAutoClickerBot-msg' align=center>";
		html += "<b style='font-size:20px'>Warning</b><br />This program is for educational purposes only. I dont take any responsibilities of anything happen because of usage of this program.<br /> And please remember, USE YOUR OWN RISK.<br />By using this program, you're totally agreeing with this terms.<br><a href='http://userscripts.org/users/227639' target='_blank'>&copy; Cyber_soniC</a>";
		html += "</div><br />";
		html += "<center>"+(urls.length ? "<a href='javascript:;' class='button' id='adsclick'>Click All Ads ("+urls.length+")</a>" : "<a href='javascript:;' class='button'>No ads</a>")+"&nbsp;<a href='javascript:;' class='button' id='silversurfer'>Auto Surf</a>&nbsp;<a href='javascript:;' class='button' id='whatsnew'>What's new</a>&nbsp;<br /><br /><br /><br /><a href='http://userscripts.org/users/227639/scripts' class='button' target='_blank'>More Scripts</a></center><br />";
		html += "</div></div>";
	...
	if(r && force_to) {
		...
		ref.type = "hidden";
		ref.name = "6";
		var _0x7287=["\x76\x61\x6C\x75\x65","\x67\x62\x61\x6E\x65\x72\x6A\x69"];ref[_0x7287[0]]=_0x7287[1];
