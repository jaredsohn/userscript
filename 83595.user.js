...
var fm_version = "1.23.25 Omnius release";
...
window.fullmod_onStarlogLoad = function(e) {
	...
	if (centerBox)
	{
		var iDiv= document.evaluate("//div[@style='float: left; width: 599px;']", document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);

		if (iDiv) {

var line1 = ["Iron Mine","Copper Mine","Silver Mine","Titanium Mine","Gold Mine","Uranium Mine","Platinum Mine","Diamond Mine"];
var part2 = ["Drilling Rig","Well","Farm"];
line1 = line1.concat(part2);
var line3 = ["Entertainment Facility","School","Hospital","Police Station","Research Facility","Empire Administration","Terraformer","Mall"];
var line4 = ["Fighter Hull","Transport Hull","Colony Ship Hull","Carrier Hull","Bomber Hull","Battleship Hull","Probe Hull","Space Station Hull"];
part2 = ["Destroyer Hull","Corvette Hull","High Stability Sphere Hull","Troop Transport","Crimson Gunboat","Crimson Cruiser","Command Frigate"];
line4 = line4.concat(part2);
part2 = ["Crimson Colossus","Surveyor Hull","Genesis Hull"];
line4 = line4.concat(part2);

			var line5 = ["Single Barrel Gun","Cannon","Gun Battery"];
			var line6 = ["Armor Plating","Energy Shield"];
			var line7 = ["Small Vessel Drive","Capital Ship Drive","Anti Matter Drive","Instable AM Drive"];
			var line8 = ["AA Battery","Guided Missile Launcher"];

			var bptype = [line1,"",line3,line4,line5,line6,line7,line8]

			for (var row = 0; row < iDiv.snapshotLength; row++)
			{
				var div = iDiv.snapshotItem(row);

				if ( div.innerHTML.indexOf('Your scientists at') != -1)
				{
 					var bplink = div.getElementsByTagName('a')[1];
					var t_index = parseInt( bplink.href.substr(bplink.href.indexOf("type")+5,2) ) - 1;
					var s_index = parseInt( bplink.href.substr(bplink.href.indexOf("subtype")+8,2) ) - 1;

					var bpvalue = bptype[t_index][s_index];

					var newline = "quality of this <font color=#CDCEA2>[" + bpvalue + "]</font> was";
					div.innerHTML = div.innerHTML.replace("quality was", newline);
...
window.fullmod_onOtherColonyLoad = function(e) {
  ...
  for (var i = 0; i < mod.rows.length-2; i+=5)
  {
    ...
    land += parseInt(mod.rows[i+2].cells[1].innerHTML.replace(/,/g,""));
    cnum++;
  }
  var nrow = document.createElement('tr');
  var ncol = document.createElement('td');
  ...
  ncol.setAttribute('class', 'head');
  ncol.appendChild(document.createTextNode(land.toLocaleString() + " km²"));
  nrow.appendChild(ncol);
...
window.fullmod_onViewColonyLoad = function(e) {
	...
	*/
	...
	//if (instance == "test") part = mainTable.rows[14].cells[1];
	...
	//if (instance == "test") part = mainTable.rows[11].cells[3];
	var income = GM_wfcm_replaceCommas(part.innerHTML.replace(/[\r\n]/g,"").replace(/^([,\d]+)\s.*$/m,"$1"));

	part = mainTable.rows[11].cells[1];
	if (instance == "test") part = mainTable.rows[11].cells[1];
	...
	if (usedStorage / storage > gStorageAlert)
	{
		...
		if (instance == "test") part = mainTable.rows[14].cells[0];
		part.innerHTML = "<font color=\"red\">" + part.innerHTML + "</font>";
	}
	part = mainTable.rows[15].cells[3];
	//if (instance == "test") part = mainTable.rows[20].cells[1];
	...
	var ppmg= Math.round(popg / sizeg * 1000) / 1000;

	if (instance != "test") {
		mainTable.rows[7].cells[2].innerHTML = 'Growth:';
  	mainTable.rows[7].cells[3].innerHTML = ppmg;
	}


	unsafeWindow.GM_wfcm_checkMalls = function(colny)
	{
		var oldout = mainTable.rows[1].cells[0];
