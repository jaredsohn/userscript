...
// @version 		0.3.7.1
// @description 	Inclui barra de atalho nas fotos dos usuarios, comunidades e topicos
...
//addEventListener("load",function(){
	...
	foto.link		= "http://www.orkut.com/Album.aspx?";
	...
	xereta.title	= "Ver perfil"
	scrap.title		= "Ver recados";
	msgs.title		= "Enviar mensagem";
	foto.title		= "Ver fotos";
	video.title		= "Ver Videos";
	add.title		= "Adicionar Amigo";
	
	forum.title		= "Ver Forum";
	forumnew.title	= "Postar novo Topico";
	join.title		= "Participar da Comunidade";
	part.title		= "Sair da Comunidade";
	members.title	= "Ver Membros";

	last.title		= "Ultima Pagina";
	reply.title		= "Responder Topico";
			...
			if (obj.innerHTML.indexOf(".gif") == -1) {
				if (obj.parentNode != xeretastd) {
					obj.addEventListener('mouseover', teste = function(event){
						imgfoto = new String(this.innerHTML.match(/(https?:\/\/(.+)small(.+)\.jpg)/gim,"$1"));
						if (Simplificado == 0)
