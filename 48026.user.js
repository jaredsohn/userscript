// ==UserScript==
// @name           mafia wars Invite Mobsters
// @namespace      mafiawarsInviteMobsters
// @include        http://apps.facebook.com/inthemafia/index.php
// ==/UserScript==

if (document.location == "http://apps.facebook.com/inthemafia/index.php"){
	try{
		var div = document.getElementById("app8743457343_req_form");
		div.innerHTML = "";
	...
	} catch(e){GM_log("Error in Invite Mobsters mob page main:" + e.message);}
	...
	} catch(e){GM_log("Error in Invite Mobsters fight page main:" + e.message);}
}
	...
	try{
		...
		for (var i in checkboxes){
			if (checkboxes[i].checked){
				requestsPending.push("http://apps.facebook.com/inthemafia/index.php?join_id=" + checkboxes[i].value);
			}
		}
		button.value = "Sending: " + requestsPending.length + " requests left.";
		sendNextRequest();
	} catch(e){GM_log("Error in Invite Mobsters sendInvites:" + e.message);}
}
...
function sendNextRequest(){
	...
	} catch(e){GM_log("Error in Invite Mobsters sendNextRequest:" + e.message);}
}
...
function checkAll(){
	...
	} catch(e){GM_log("Error in Invite Mobsters checkAll:" + e.message);}
}
...
function remove(){
	...
	} catch(e){GM_log("Error in Invite Mobsters remove:" + e.message);}
}
...
function captcha(url){
	try{
		...
		div.appendChild(button);
	} catch(e){GM_log("Error in Invite Mobsters captcha:" + e.message);}
}
...
function closeCaptcha(){
	...
	} catch(e){GM_log("Error in Invite Mobsters closeCaptcha:" + e.message);}
}
...
function addFriend(){
	try{
		...
		if (this.childNodes[2] != null){
			...
			document.getElementsByName("tdkAddMeFrame")[0].src = "http://apps.facebook.com/inthemafia/index.php?id=" + addingID;
			document.getElementsByName("tdkAddMeFrame")[0].addEventListener("load", addFriendSubmit, true);
		} else setFriendAdded();
	} catch(e){GM_log("Error in Invite Mobsters addFriend:" + e.message);}
}
...
function addFriendSubmit(){
	...
	} catch(e){GM_log("Error in Invite Mobsters addFriendSubmit:" + e.message);}
}
...
function setFriendAdded(){
	...
	} catch(e){GM_log("Error in Invite Mobsters setFriendAdded:" + e.message);}
}