// ==UserScript==
// @name           KOC Power Tools - Master Hunter
// @namespace      mat
...
//Most of this code is by George Jetson.
var Version = 'a52_mh20110531a';
//Added code to update the marches in real time.
//Added code to display scouting marches separately.
//Added the message separator.
//Added city buttons to overview.
//Added defensive stance to overview.
//Added buttons to toggle defensive stance.
//Added incoming attack counts to overview.
//Added march counts to overview.
//Added code to display incoming attacks. In progess.
//Added link for page number in HUD. 
//Added fix for late marches.
//Fix for incoming attacks display.
...
var Options = {
  ...
  MonitorMarchCity_0 : true,
  MonitorMarchCity_1 : true,
  MonitorMarchCity_2 : true,
  MonitorMarchCity_3 : true,
  MonitorMarchCity_4 : true,
  MonitorMarchCity_5 : true,
  MonitorMarchCity_6 : true,
  MonitorMarchCity_7 : true,
  fixLateMarches : true,
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    .Late td { background:yellow; }.LateL2 td { background:red; }\
...
// "Report No: 5867445"  --->  see unsafeWindow.modal_alliance_report_view()
  ...
  chatDivContentHook : function (msg){
    ...
    if (m[0].indexOf('whispers') >= 0)
      ptclass = 'ptChatWhisper';
    else if (m[0].indexOf('to the alliance') >= 0)
      ptclass = 'ptChatAlliance'; 
    else  
      ptclass = 'ptChatGlobal'; 
    msg = msg.replace ("class='content'", "class='content "+ ptclass +"'");
  ...
  show : function (){
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        ...
        <option value="1,100" > Ballista </option>\
  ...
  init : function (div){
    ...
    try {      
      ...
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Enable enhanced Alliance Reports.</td></tr>\
        ...
        <TR><TD><INPUT id=togForceMarchUpdates type=checkbox /></td><TD>Force march updates. <SPAN class=boldRed>(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togfixLateMarches type=checkbox /></td><TD>Fix late marches. <SPAN class=boldRed>(NEW)</span></td></tr>\
...
t.cont.innerHTML = m;
      ...
      t.togOpt ('togfixLateMarches', 'fixLateMarches', Tabs.Marches.setEnableFixLateMarches);
  ...
  setOption : function (optionName, optValue) {
...
//DebugTimer.start(); 
    try {
      ...
      Tabs.Marches.getIncomingCounts(0);
      ...
      str += "</tr>";
      str += '<tr align=right><TD width=88 ><B>Defending</b> &nbsp;</td><TD></td>';
      for(i=0; i<Cities.numCities; i++) {
          var mode = Seed.citystats["city" + Cities.cities[i].id].gate;
          str+='<TD><INPUT id="idToggleDef_'+i+ '" type=submit value='+((mode==1)?'yes':'no')+' /></td>';
      }
      str += "</tr>";
      str += '<tr align=right style = "background: #e8e8e8"><TD width=88 style = "background: #e8e8e8"><B>Incoming</b> &nbsp;</td><TD style = "background: #e8e8e8"></td>';
      for(i=0; i<Cities.numCities; i++) {
          var incoming = 'A'+Tabs.Marches.attacks[i]+'|S'+Tabs.Marches.scouts[i];
          if ( incoming == 'A0|S0') 
             str+='<TD style = "background: #e8e8e8">'+incoming+'</td>';
          else
             str+='<TD style = "background: #ff0000" >'+incoming+'</td>';
      }
      str += "</tr>";
      str += '<tr align=right><TD width=88 ><B>Marches</b> &nbsp;</td><TD></td>';
      for(i=0; i<Cities.numCities; i++) {
          str+='<TD>'+Tabs.Marches.counts[i]+'</td>';
      }
      str += "</tr>";
      rows = [];
      rows[0] = [];
...
Tabs.Marches = {
  ...
  init : function (div){
    ...
    //unsafeWindow.pr57Recall = t.butMarchRecall;
    unsafeWindow.r8x6Home = t.butSendHome;
    unsafeWindow.ptShowTroops = t.showTroops;
    unsafeWindow.ptShowLoot = t.showLoot;
    ...
    t.setEnableFixLateMarches(Options.fixLateMarches);

    //logit ("Seed: "+JSON2.stringify(unsafeWindow.seed));
  ...
  setEnable : function (tf){
    var t = Tabs.Marches;
    //logit ("Tabs.Marches.setEnable: "+tf);
    clearInterval (t.reportTimer);
    if (tf){
      t.reportTimer = setInterval (function(){Tabs.Marches.getBattleReports(1);}, 5*1000);
    } 
  },
  setEnableFixLateMarches : function (tf){
    var t = Tabs.Marches;
    clearInterval (t.repairTimer);
    if (tf){
      t.repairTimer = setInterval (function(){Tabs.Marches.fixLateMarches();}, 5*1000);
    } 
  },    
  ...
  scouts:[],  
  /***   ATTACKS SUBTAB  ***/
  getIncomingCounts : function (cityNum) {
    var t = Tabs.Marches;
    var now = unixTime();
    var ret = {};
    ret.scouts = 0;
    ...
    if (matTypeof(Seed.queue_atkinc) != 'array') {
       for (var k in Seed.queue_atkinc) {
           ...
           for (var c=0; c<Cities.numCities; c++){
               //get citing index
               if (parseInt(m.toCityId) == Cities.cities[c].id)
                  idx = c;
           }
    ...
    ret.scouts = t.scouts[cityNum];
    ret.attacks = t.attacks[cityNum];
    return ret;
  },
  getAttackTroops : function (attack) {
    var ret = ((attack.unts['u1'])?attack.unts['u1']:'0');
    for ( var i=2; i<13; i++) {
        //---
        ret += ','+((attack.unts['u'+i])?attack.unts['u'+i]:'0');
    }
    return ret;
  },
  showAttacks : function (){
    ...
    var str = '';
    var count = 0;
    str+= '<div class=ptstat>Incoming Attacks</div><BR>';
    str += '<center><table><thead><th>#</th><th>Target</th><th>Loc</th><th>Who</th><th>Deed</th><th>Troops</th><th>ETA</th></thead>\
             <tbody>';
    if (matTypeof(Seed.queue_atkinc) != 'array') {
       for (var k in Seed.queue_atkinc) {
           var m = Seed.queue_atkinc[k];
           if ((m.marchType == 3 || m.marchType == 4) && parseIntNan(m.arrivalTime) > now) {
              try {
                var who = unsafeWindow.seed.players['u'+m.pid].n;
              }catch (err){
                 var who = 'na';
              }
              str+='<TR>';
              count+=1;
              str+='<TD>'+count+'</td>';
              str+='<TD>'+Cities.byID[m.toCityId].name+'</td>';
              str+='<TD>'+Cities.byID[m.toCityId].x+','+Cities.byID[m.toCityId].y+'</td>';
              str+='<TD>'+who+'</td>';
              str+='<TD>'+((m.marchType == 4)?'Attacking':'Scouting')+'</td>';
              str+='<TD>'+t.getAttackTroops(m)+'</td>';
              str+='<TD>'+timestr(m.arrivalTime-now)+'</td>';
             str+='</tr>';
           }
       }
    }
    ...
    str+='</tbody>';
    t.marchDiv.innerHTML = str;
    if (count==0) 
       t.marchDiv.innerHTML = '<center><b>Nothing to report</b></center>';
...
***/
  ...
  getMarchTroop2:function(march) {
    var ret = [];
    for (var i=1; i<13; i++){
        ret.push(march['unit'+ i +'Count']);
    }   
    return ret;
  },

  getReturnTroop:function(march) {
    var ret = march['unit1Return'];//The 0th unit is not used.
    for (var ii=2; ii<13; ii++){
        ret += ',' + march['unit'+ ii +'Return'];
    }   
    return ret;
  },

  getScoutedTroops:function(report) {
    if (report.winner == 0) 
        return 'NA';
    var ret = ((report.unts['u1'])?report.unts['u1']:'0');//The 0th unit is not used.
    for (var i=2; i<13; i++){
        ret += ',' + ((report.unts['u'+ i ])?report.unts['u'+ i ]:'0');
    }   
    return ret;
  },
  ...
  getScoutedRes:function(report) {
    if (report.winner == 0) 
        return 'NA';
    var ret = parseInt(report.rsc.r1);
    for (var i=2; i<5; i++){
        ret += ',' + parseInt(report.rsc['r'+ i ]);
    } 
    return ret;
  },

/*
 frt.f53-Wall-Mounted Crossbows	
 frt.f55-Defensive Trebuchet	
 frt.f60-Trap
 frt.f61-Caltrops	
 frt.f62-Spiked Barrier
*/
  getScoutedWall:function(report) {
    if (report.winner == 0) 
        return 'failed';
    var ret = ((report.frt.f53)?report.frt.f53:'0');
    ret += ',' + ((report.frt.f55)?report.frt.f55:'0');
    ret += ',' + ((report.frt.f60)?report.frt.f60:'0');
    ret += ',' + ((report.frt.f61)?report.frt.f61:'0');
    ret += ',' + ((report.frt.f62)?report.frt.f62:'0');
    return ret;
  },
      ...
      for (var k in Seed.queue_atkp[cityId]){   
        ...
        if (typeof (march) == 'object'){
          ...
          if (march.returnUnixTime) {
             ...
             if (eta<-15) {
               //logit ("Eta: "+eta+','+march.toXCoord+','+march.toYCoord);
               t.fixLateMarch(march);
             }
          }
        }
      }
    }
    t.checkKnightStatus();//And check on KABAM's normal return of knights to make sure they have not f'd up.  (fouled:D)
  },

//march.marchType: 1-transport, 2-reinforce, 3-scout, 4-attack, 5-reassign
  fixLateMarch : function (march) {
     var t = Tabs.Marches;
     if ( march.marchType == 2 || march.marchType == 5)// Don't do reinf, reassign to simplify for now. We don't want to do reinf anyhow. Duh.
        return;
    ...
    for(var i=0; i<Cities.numCities; i++) {   
      var acityId = 'city'+ Cities.cities[i].id;
      for (var k in Seed.queue_atkp[acityId]){   
        var amarch = Seed.queue_atkp[acityId][k];
        if (amarch.returnUnixTime == march.returnUnixTime) {
           cityId = acityId;
     ...
     //1. return knight
     if (march.knightId) {//A march may not have a knight.
       //Find Knight and set status to 1. 
       if ( march.marchType != 5) { //Not doing reassignments yet.  
           for (k in unsafeWindow.seed.knights[cityId]){
              if (unsafeWindow.seed.knights[cityId][k].knightId == march.knightId) {
                 unsafeWindow.seed.knights[cityId][k].knightStatus = '1';
              }
           }
       }
     }
     //2. return troops ( if reassign must handle differently)
       var returnTroops = t.getReturnTroop (march);
       var troops = t.getMarchTroop (march);
       var returnTroopsCount = t.returningTroops (march);//The troops returning may not be updated;
       if (returnTroopsCount) {
          //Repair troop levels
          //Want this path for attacks, scouts, if possible.
          var m = returnTroops.split(',');    
          for (i=1; i<13; i++) {
              if (parseInt(m[i]) > 0) {
                //logit ("bef troops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+i]);
                unsafeWindow.seed.units[cityId]['unt'+i] = parseInt(unsafeWindow.seed.units[cityId]['unt'+i])+parseInt(m[i]);
                 //logit ("aft troops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+i]);
             }
          }
       }
       else {
         if (unsafeWindow.seed.units[cityId]) {
          //This okay for transport, reassign.
          //Repair to target city for reassign. TBD.
          //Repair troop levels(possibly inaccurately, if there was a troop loss attacking, or scouting.). 
          var m = troops.split(',');
          for (i=1; i<13; i++) {
              if (parseInt(m[i]) > 0) {
                //logit ("bef troops: "+i+', '+m[i]+', '+unsafeWindow.seed.units[cityId]['unt'+i]);
    ...
    //3. delete march
    if ( unsafeWindow.seed.queue_atkp[cityId] ) {
      for (var kk in unsafeWindow.seed.queue_atkp[cityId]){   
        if (typeof (unsafeWindow.seed.queue_atkp[cityId][kk]) == 'object'){
           if ( unsafeWindow.seed.queue_atkp[cityId][kk].marchUnixTime == march.marchUnixTime ) {
               delete unsafeWindow.seed.queue_atkp[cityId][kk];
            }
              
        }
      }
    }
    //4. remove waiting reports - These go away automatically when the march is deleted.

  },
  checkKnightStatus : function () {
    var t = Tabs.Marches;
    for(var i=0; i<Cities.numCities; i++) {   
      cityId = 'city'+ Cities.cities[i].id;
      for (var j in unsafeWindow.seed.knights[cityId]){   
        var knight = unsafeWindow.seed.knights[cityId][j];
        if (typeof (knight) == 'object'){
          if ( knight.knightStatus == 10) {
            var found = false;
            for (var k in unsafeWindow.seed.queue_atkp[cityId]){   
                 march = unsafeWindow.seed.queue_atkp[cityId][k];
                 if (typeof (march) == 'object'){
                    if ( march.knightId == knight.knightId){
                       found = true;
                       break;
                    }
                 }
            ...
            if (!found) {
               //logit ("checkKnightStatus - Knight marching, but the march not found. "+ JSON2.stringify(knight));
               knight.knightStatus = 1;
            }
          }
        }
      }
  ...
  showMarches : function (){
    ...
    var m = '';
    //m+='<div><INPUT id=idUpdateMarches type=submit value="Update" /></div>;
    m+='<div class=ptstat>Note: Target and Resource data are not always accurately updated.</div><BR>';
    m+='<div class=ptentry><center><b>Monitor marches for selected cities: </b><BR>';//Options div
    for(var i=0; i<Cities.numCities; i++) {   
      m += '<INPUT type=CHECKBOX id=idCheckMonitorCity_'+i+' '+ (Options['MonitorMarchCity_'+i]?' CHECKED':' ')+'><b>' + Cities.cities[i].name + '</b> ';
    }

    m += '</center></div><BR>';
    m += '<center><table id=idMarchTable ><thead><th>#</th><th>City</th><th>Type</th><th>Target</th><th>Lvl</th><th>Loc</th><th>Troops</th><th>Loot</th><th>Ago</th><th>ETA</th><th>Status</th><th>Late</th></thead>\
             <tbody>';
    var count=0;
    var lateCount=0;
    var scoutCount=0;
    ...
    for(var i=0; i<Cities.numCities; i++) {   
      cityId = 'city'+ Cities.cities[i].id;
      t.counts[i] = 0;
      if (!Options['MonitorMarchCity_'+i])
          continue;
      for (var k in Seed.queue_atkp[cityId]){   
        march = Seed.queue_atkp[cityId][k];
        if (typeof (march) == 'object'){
          ...
          if ( t.marchTypeToStr(march.marchType) == 'scout' && now>=march.destinationUnixTime){
             //logit ("Scout Found");
            var report = t.getScoutingReport(march);
            if ( report != 'undefined') {
             scoutCount++;
             continue;
            }
          ...
          if (march.toPlayerId || march.toCityId) {
              if (march.toPlayerId>0 && targType=='city,barb')
                 targType='city';
              else if (march.toCityId && targType=='city,barb' && march.toCityId > 0) {
                 targType='city';
              }else{
                 targType='camp';
              }
          ...
          var troops = t.getMarchTroop (march);
          var returnTroops = t.getReturnTroop (march);
          var dispTroops = troops;
          var loot = t.getMarchRes (march);
          if ( status == 'Marching' && t.marchTypeToStr(march.marchType) != 'transport') {
              loot = '';
          }
          if ( status == 'Returning') {
              troops = troops + '<br>' + returnTroops;
              dispTroops = returnTroops;
          }
          ...
          m += '<tr align=right class="'+className+'"><td>' + count + '</td>';
          m += '<td>' + Cities.cities[i].name + '</td>';
          m += '<td>'+ t.marchTypeToStr(march.marchType) +'</td>\
                <td>'+targType+'</td>\
                <td>'+march.toTileLevel+'</td>\
                <td><A onclick="ptGotoMap('+march.toXCoord +','+ march.toYCoord+')">'+ march.toXCoord +','+ march.toYCoord +'</a></td>\
                <td><A onclick="ptShowTroops(['+dispTroops+'])" >'+troops+'</a></td>';
          if (loot != '')
            m += '<td><A onclick="ptShowLoot(['+loot+'])" >'+loot+'</a></td>';
          else
            m += '<td>'+loot+'</td>';
    ...
    if ( scoutCount > 0 ) {
    //Scouting marches returning table
       m += '<center><table><thead><th>#</th><th>City</th><th>Type</th><th>Target</th><th>Lvl</th><th>Loc</th>\
             <th>LL</th><th>Wall<br>(cb,tr,t,c,sb)</th><th>Troops</th><th>Resources</th><th>Ago</th><th>ETA</th><th>Status</th><th>Late</th></thead>\
             <tbody>';
    for(i=0; i<Cities.numCities; i++) {   
      cityId = 'city'+ Cities.cities[i].id;
      if (!Options['MonitorMarchCity_'+i])
      ...
      for (k in Seed.queue_atkp[cityId]){   
        ...
        if (typeof (march) == 'object'){
          ...
          if(march.returnUnixTime) {
            eta=march.returnUnixTime-now;
            if (eta < 0) etaStr = '<td>-' + timestr(-1*(eta)) +'</td>';
            else etaStr = '<td>' + timestr(eta) +'</td>';
          }
          var late = false;
          var className='';
          if (eta<0 && march.marchType!=2) {
          ...
          if( t.marchTypeToStr(march.marchType) == 'scout') {
            var targType = t.tile2Str(march.toTileType);
            if (march.toPlayerId || march.toCityId) {
              if (march.toPlayerId>0 && targType=='city,barb')
                 targType='city';
            ...
            var troops = t.getScoutedTroops (report);
            var res = t.getScoutedRes (report);
            var wall = t.getScoutedWall (report);
            ...
            m += '<td>'+ t.marchTypeToStr(march.marchType) +'</td>\
                <td>'+targType+'</td>\
                <td>'+march.toTileLevel+'</td>';
            m += '<td>'+ march.toXCoord +','+ march.toYCoord +'</td>';
            if (status == 'Returning' && report.lstlgn) {  
                m += '<td>'+timestr(now-report.lstlgn,1)+'</td>';
            }
            else
                m += '<td>'+'NA'+'</td>';
            m += '<td>'+wall+'</td>\
    ...
    // fixup empty table
    if ( count == 0) {
       var table = document.getElementById('idMarchTable');
       if ( table )
          table.innerHTML = '<cente><b>No marches to report.</b></center>';
    }

    //Add option check box event handler
    for(var i=0; i<Cities.numCities; i++) {   
        document.getElementById('idCheckMonitorCity_'+i).addEventListener('click', t.handleCityCheck, false);
    }

    t.displayTimer = setTimeout (t.showMarches, 5000);
  },
  handleCityCheck : function (e) {
    var t = Tabs.Marches;
    if (e) {
      ...
      if (chkBox) {
         ...
         var cityNum = m[1];
         var optionName = 'MonitorMarchCity_'+cityNum;
         Tabs.Options.setOption(optionName,chkBox.checked);
      }
    }
  },
...
var MessageCounts = {//Source: totino - http://userscripts.org/topics/71368
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    m += '<TD class=spacer  width=90% align=right><span id=pthead_time>GMT</span>&nbsp;</td></tr></table>';
...
function CPopupDisplayData (Id,Height,Heading,DataType, Data) {
  var pop = new CPopup (Id, 0, 0, 400,Height, true);

  pop.centerMe(mainPop.getMainDiv());
  pop.getTopDiv().innerHTML = '<CENTER><B>'+Heading+'</b></center>';

  var m = '';
  switch (DataType) {
    case 'troops':
      m += '<TABLE width="100%">';
      m += '<TR align=left ><TH></th>';
      m += '<TH>Type</th>';
      m += '<TH>'+'Amount'+'</th></tr>';
      
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_30.jpg?6545"></td>';
      m += '<TD>Supply Troop</td>';
      m += '<TD>'+addCommasInt(Data[0])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_3.jpg?6545"></td>';
      m += '<TD>Militiaman</td>';
      m += '<TD>'+addCommasInt(Data[1])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_3_30.jpg?6545"></td>';
      m += '<TD>Scout</td>';
      m += '<TD>'+addCommasInt(Data[2])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_4_30.jpg?6545"></td>';
      m += '<TD>Pikeman</td>';
      m += '<TD>'+addCommasInt(Data[3])+'</td></tr>';
      
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_5_30.jpg?6545"></td>';
      m += '<TD>Swordsman</td>';
      m += '<TD>'+addCommasInt(Data[4])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_6_30.jpg?6545"></td>';
      m += '<TD>Archer</td>';
      m += '<TD>'+addCommasInt(Data[5])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_7_30.jpg?6545"></td>';
      m += '<TD>Cavalry</td>';
      m += '<TD>'+addCommasInt(Data[6])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_8_30.jpg?6545"></td>';
      m += '<TD>Heavy Cavalry</td>';
      m += '<TD>'+addCommasInt(Data[7])+'</td></tr>';
      
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_9_30.jpg?6545"></td>';
      m += '<TD>Supply Wagon</td>';
      m += '<TD>'+addCommasInt(Data[8])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_10_30.jpg?6545"></td>';
      m += '<TD>Ballista</td>';
      m += '<TD>'+addCommasInt(Data[9])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_11_30.jpg?6545"></td>';
      m += '<TD>Battering Ram</td>';
      m += '<TD>'+addCommasInt(Data[10])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_12_30.jpg?6545"></td>';
      m += '<TD>Catapult</td>';
      m += '<TD>'+addCommasInt(Data[11])+'</td></tr>';
    ...
    case 'loot':
      ...
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/gold_30.png"></td>';
      m += '<TD>Gold</td>';
      m += '<TD>'+addCommasInt(Data[0])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png"></td>';
      m += '<TD>Food</td>';
      m += '<TD>'+addCommasInt(Data[1])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/wood_30.png"></td>';
      m += '<TD>Wood</td>';
      m += '<TD>'+addCommasInt(Data[2])+'</td></tr>';

      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/stone_30.png"></td>';
      m += '<TD>Stone</td>';
      m += '<TD>'+addCommasInt(Data[3])+'</td></tr>';
      
      m += '<TR align=left ><TD><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/iron_30.png"></td>';
      m += '<TD>Ore</td>';
      m += '<TD>'+addCommasInt(Data[4])+'</td></tr>';
...
unsafeWindow.ptAllianceReports = function (page) {
   hideMe ();
   setTimeout (function (){ 
      unsafeWindow.modal_alliance();
      unsafeWindow.allianceReports(page);
      //unsafeWindow.modal_alliance_changetab(4);
      logit ( "ptAllianceReports Page: " + page);
    }, 0);
};
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  this.BASE_ZINDEX = 150000030;//111121;
  ...
  DisplayReports : function (){
    ...
    for ( var i=0; i<t.data.length;i++) {
       ...
       if (rpt.side0Name=='undefined') 
          continue;
       m += '<tr><td><SPAN onclick="ptAllianceReports('+rpt.page+')"> <a>'+rpt.page+'</a></span></td>\
