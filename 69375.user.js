// ==UserScript==
// @name           PlanetStormBot
// @namespace      plantetstorm
// @copyright      © Layane
// @license        Creative Commons Attribution-Noncommercial-Share Alike 3.0 Germany License
// @version        2.4
// @include        http://www.plantetstorm.com/challenges*
// @include        http://www.plantetstorm.com/battle/winnings*
// @include        http://www.plantetstorm.com/battle/view/*
// ==/UserScript==
...
var version = '2.4';
var updateURL = 'http://userscripts.org/scripts/source/69375.user.js';
var webPage = 'http://userscripts.org/scripts/show/69375';
//--------------------------------------------------------------------------------\\

var maxDesk = GM_getValue("MAX_SELECTED_DESKS", MAX_SELECTED_DESKS);

GM_registerMenuCommand("[PS] Use 1 squad",  function (e) { changeMaxDesk(1); });
GM_registerMenuCommand("[PS] Use 2 squads", function (e) { changeMaxDesk(2); });
GM_registerMenuCommand("[PS] Use 3 squads", function (e) { changeMaxDesk(3); });
GM_registerMenuCommand("[PS] Use 4 squads", function (e) { changeMaxDesk(4); });

//--------------------------------------------------------------------------------\\
// Bot

if (document.location.href == 'http://www.plantetstorm.com/battle/winnings') {
	setTimeout("document.location.href  = 'http://www.plantetstorm.com/challenges'", THINK_INTERVAL);
	return;
}

if (document.location.href.match('^http:\/\/www\.plantetstorm\.com\/battle\/view\/*') != null) {
	setTimeout("document.location.href  = 'http://www.plantetstorm.com/challenges'", THINK_INTERVAL);
	return;
}
...
function showRemain() {
	...
	GM_log("PsBot [" +  (remainTime / 60000).toFixed(1) + "m]");
}
