// ==UserScript==
// @name           Mermaider's Tanks to Stalk[#1]
			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' ('+tankId+') [Mermaider] has been glowing for '+seconds2HMS(gaiaT-glowT)+'.');
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/54209047/');
								}
							}
							else{
								//alert('over');
							...
							alert(aquaN+' ('+tankId+') [Mermaider] will glow in '+seconds2HMS(glowT-gaiaT));
							if(open===true){
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(1054649,false);//Mermaider
	}
}
checkTanks();
