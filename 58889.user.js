...
var scriptUrl = "http://userscripts.org/scripts/show/58889";
...
function do_main(){
	...
	if (GM_getValue("valUpdate",true)) {
		...
		if(valLastUpdate=="") {
			GM_xmlhttpRequest({
				method: "GET",
				url: "http://userscripts.org/scripts/source/58889.meta.js",
				...
				onload: function(response) {
					...
					if (valLastUpdate!=serverversion) {
						...
						if(confirm(texte["confirmUpdate"])) {
							document.location.href = "http://userscripts.org/scripts/source/58889.meta.js";
						}
					}
				}
			});
	...
	// Werbung
	if (werbecontainer) {
	//	werbecontainer.style.display = "none";
	...
	*/
	...
	var beraterDiseaseBubble = createElement("div",{id:"beraterDiseaseBubble",style:"z-index:2;position:absolute;top:0px;left:0px;background-color:#CCC;-moz-border-radius:10px;border:2px solid black;padding:3px;margin-left:40px;"},all);
	hiddenPatientDiv = createElement("div",{id:"hiddenPatientDiv",style:"display:none;"},all);
	$("garten_komplett").addEventListener("mouseover",function(event){
