// ==UserScript==
// @name           AMarker
// @namespace      AMarker_ChowMein
// @include        *
// @history        "<p><h4>v0.1</h4></p>"
// @version        0.1
// ==/UserScript==
...
var methods = {
	...
	destroy: function() {
		return this.each( function() {
			$( this )
				.one( "izCloseEvent.izWindow", function( event ) {
					if ( !event.veto ) {
						$( this ).remove();
					}
					return false;
				} )
				.trigger( "izCloseEvent.izWindow" );
		} );
	},

	create: function( options ) {
		return this.each( function() {
			var $w = $( this ), settings = $.extend( {}, $.fn.izWindow.defaults, options || {} );

			$w.hide().addClass( "izWindow" );

			if ( settings.caption ) {
				var $titlebar = $( "<div class='izWindow-Titlebar'></div>" ).prependTo( $w ),
					$titleText = $( "<span class='izWindow-TitleText'></span>" ).text( $w.attr( "title" ) ).appendTo( $titlebar ),
					$btnBox = $( "<span class='izWindow-ButtonBox'></span>" ).appendTo( $titlebar );
				...
				if ( settings.minButton ) {
						$( "<input type='button' class='izWindow-Titlebar-MinButton' value='▁' />" )
							.bind( "mousedown", function() {
								return false;
							} )
							.bind( "click", function() {
								$w.css( { height: "1em" } );
								console.log( $w.css( "height" ) );

								$w.css( { overflow: "hidden", height: $titlebar.height() + 2 } );
								return false;
							} )
							.appendTo( $btnBox );
				}

				if ( settings.maxButton ) {
						$( "<input type='button' class='izWindow-Titlebar-MaxButton' value='□' />" )
							.bind( "mousedown", function() {
								return false;
							} )
							.bind( "click", function() {
								var b = parseInt( $w.css( 'padding-top' ) ) + parseInt( $w.css( 'border-top-width' ) );

								$w.css( { position: 'fixed', top: 0, left: 0, height: window.innerHeight - b * 2, width: window.innerWidth - b * 2 } );
								$( document.body ).css( { overflow: 'hidden', margin: 0, padding: 0} );
								return false;
							} )
							.appendTo( $btnBox );
				}
				...
				if ( settings.closeButton ) {
						$( "<input type='button' class='izWindow-Titlebar-CloseButton' value='╳' />" )
							.bind( "mousedown", function() {
								return false;
							} )
							.bind( "click", function() {
			...
			var $client = $w.find( ".izWindow-ClientArea" );
			if ( !$client.length ) {
				$client = $( "<div class='izWindow-ClientArea'></div>" ).appendTo( $w );
			}

			if ( settings.resizable ) {
				$w.bind( "mousedown", function( event ) {
					$w.izResizable( event, {
						client: $client[0],
						titleText: settings.caption ? $titleText[0] : null,
						titlebarButtonsWidth: settings.caption && $btnBox ? $btnBox.width() : 0
					} );
				} );
			}
		} );
...
$.fn.izWindow = function( opt ) {
	if ( methods[opt] ) {
		return methods[opt].apply( this, Array.prototype.slice.call( arguments, 1 ) );
	} else if ( typeof opt === 'object' || !opt ) {
		return methods.create.apply( this, arguments );
	} else {
		$.error( 'Method ' + opt + ' does not exist on jQuery.izWindow' );
	}
};

$.fn.izWindow.version = "0.2";

$.fn.izWindow.defaults = {
	caption: true,
	minButton: false,
	maxButton: false,
	closeButton: false,
...
*/
		return this.each( function() {
			var $d = $( this ), settings = $.extend( {}, $.fn.izDialogBox.defaults, options || {} );

			$d
				.wrapInner(
					$( "<div class='izWindow-ClientArea'></div>" ).css( { height: 'auto', margin: '5px' } )
				)
				.izWindow( {
					caption: $d.attr( "title" ),
				...
				.attr( 'id', settings.id )
				.attr( "title", "" )
				.addClass( "izDialogBox" )
				.css( { position : ( settings.position !== "fixed") ? "absolute" : "fixed" } )
				.offset( { top: curTop, left: curLeft } );

			// buttonPane
			if ( settings.buttons.length ) {
				var $buttonPane = $( "<div class='izDialogBox-ButtonPane'></div>" ).appendTo( $d );

				for ( var btn, $btn, i = 0, btnlen = settings.buttons.length; btnlen > i; ++i ) {
					btn = settings.buttons[i];
					$btn = $( "<input type='button' class='" + ( btn.classname ? btn.classname : "" )  + "' value='" + btn.name + "' />" );
					if ( btn.click ) {
...
$.fn.izDialogBox.version = "0.2";

$.fn.izDialogBox.defaults = {
	id: "",
	position: "fixed",
	closeButton: true,
	buttons: [],
...
var methods = {
	...
	insert: function( rows, index ) {
		return this.each( function() {
			...
			} else if ( index === undefined || index < -1 || index >= this.rows.length ) {
			...
			if ( settings.toolbar && settings.buttons.length ) {
				...
				for ( var i = 0, btnlen = settings.buttons.length; btnlen > i; ++i ) {
					...
					if ( btn.separator ) {
						...
						if ( btn.dblclick ) {
							$btn.bind( "dblclick", btn.dblclick );
							if ( settings.checkbox ) {
...
var Markers = function() {
	...
	$( document.body ).css( { "margin-bottom": "1000px" } );

	var that = this;

//	this.url = location.href.replace( /#izMarker\-[\d]+/, "" );
	this.url = location.href.replace( /#[^\?]+/, "" );
//	console.log( this.url );

	this.$grid = $( ".izMarkerData" ).izGrid( {
			id: 'izMarkerInfo',
			classname: "",
			captionText: "Markers",
			toolbar: true,
			buttons: [{ name: 'Toggle', classname: 'izMarkerInfo-ToggleBtn', click: function() {
						var $this = $( this );

						if ( $this.is( ".izEnabled") ) {
							$this.removeClass( "izEnabled" ).text( "Toggle" );
							$( document.body ).css( { cursor: "" } )
							$( document ).unbind( "click.izMarkers-Enable" );
						} else {
							$this.addClass( "izEnabled" ).text( "Stop" );
							$( document.body ).css( { cursor: "copy" } )

							$( document ).bind( "click.izMarkers-Enable", function( event ) {
									var m;
									if ( event.target.classList.contains( "izMarker" ) ) {
										if ( m = that.unmark( event.target ) ) {
												that.delRow( m );
												$this.trigger( "click" );
										}
									} else {
										if ( m = that.mark( event.target ) ) {
											$this.trigger( "click" );
											that.addRow( m );
										}
									}
				...
				{ separator: true },
				{ name: 'Remove', classname: 'izMarkerInfo-RemoveBtn', click: function() {
						var rows = $( this ).parents( ".izGrid" ).find( ".izGrid-Selected" );
						
						if ( rows ) {
							var len = rows.length;
							while ( len-- ) {
								that.unmarkByMarker( rows[len].prop_Marker );
							}

							that.$grid.izGrid( "delete", rows );
						}
						return false;
					} },
				{ separator: true },
				{ name: 'Clear', classname: 'izMarkerInfo-ClearBtn', click: function() {
						that.clear();
						return false;
					} },
				{ separator: true },
				{ name: 'Load', classname: 'izMarkerInfo-LoadBtn', click: function() {
						that.clear();
						that.load();
						return false;
					} },
				{ separator: true },
				{ name: 'Save', classname: 'izMarkerInfo-SaveBtn', click: function() {
						that.save();
						return false;
					} },
				{ separator: true }],
			...
			height: 200,
			width: 365,
			columnsInfo: [ { minWidth: "3em", editable: false }, { minWidth: "auto", editable: true }]
		} );
		
		if ( !$( "#izStyle-Markers" ).length ) {
			$( "<style id='izStyle-Markers' type='text/css'></style>" ).text(
				'.izMarker{font-size:smaller;vertical-align:super;}' 
			).appendTo( document.head );
		}
...
Markers.Relationship_Prev = 1;
Markers.Relationship_Parent = 2;
...
Markers.prototype = {
	...
	////
	buildXPath: function ( elt ) {
		var path = '', expr, idx;
			for ( ; elt && elt.nodeType === 1; elt = elt.parentNode ) {
				expr = "";
				idx = 1;

				for ( var e = elt.parentNode.firstElementChild; e; e = e.nextElementSibling ) {
					if ( e === elt ) {
						break;
					}

					if ( e.nodeType === 1 && e.tagName.toLowerCase() === elt.tagName.toLowerCase() ) {
						++idx;
					}
				}
				...
				if ( elt.className ) {
					expr += "[@class='" + elt.className + "']";
				}

				path = '/' + elt.tagName.toLowerCase() + expr + path;
			}

		return path;
	},

	buildMarker: function( m ) {
	},

	restore: function( m ) {
		var rel = document.evaluate( m.xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null ).singleNodeValue;

		if ( rel ) {
			var $a = $( "<a id='izMarker-" + m.index + "' class='izMarker sup' title='" + m.title + "'>[" + m.index + "]</a>" );
			$a.css( { "background": "#0ff", color: m.color, "-moz-user-select": "-moz-none" } );
			$a[0].prop_Marker = m;
			...
			try {
				switch ( m.relationship ) {
					case Markers.Relationship_Prev:
						range.setStart( rel.nextSibling, m.offset );
					break;

					case Markers.Relationship_Parent:
						range.setStart( rel.firstChild, m.offset );
					break;
				}
				...
				console.log( "restore[ " + "izMarkers-" + m.index + " ](E1): " + e );
			}
	...
	mark: function ( elt ) {
		if ( elt.tagName.toLowerCase() === "html" ||	elt.classList.contains( "izMarker" ) )	{
			return false;
		}
		...
		if ( container.parentNode !== elt ) {
			return false;
		}

		var n, m = {};

		m.index = ++this.lastIndex;
		m.offset = range.startOffset;

		for ( n = container.previousSibling; n; n = n.previousSibling ) {
			if ( n.nodeType == 1 ) {
				if ( n.classList.contains( "izMarker" ) ) {
					m.offset += n.prop_Marker.offset;
				}
				break;
			}
		}
		...
		for ( ; n; n = n.previousSibling ) {
			if ( n.nodeType == 1 && !n.classList.contains( "izMarker" ) ) {
				m.xpath = this.buildXPath( n );
				m.relationship = Markers.Relationship_Prev;
				break;
			}
		}

		if ( !n ) {
			m.xpath = this.buildXPath( container.parentNode );
			m.relationship = Markers.Relationship_Parent;
		}
		...
		var $a = $( "<a id='izMarker-" + m.index + "' class='izMarker' title='" + m.title + "'>[" + m.index + "]</a>" );
		$a.css( { "background": m.bgColor, color: m.color, "-moz-user-select": "-moz-none" } );
		$a[0].prop_Marker = m;
		range.insertNode( $a[0] );
/*
console.log(
...
*/
	...
	unmark: function( elt ) {
		if ( elt.tagName.toLowerCase() !== "html" )	{
			if ( elt.parentNode.classList.contains( "izMarker" ) ) {
				elt = elt.parentNode;
			}

			if ( elt.previousSibling && elt.previousSibling.nodeType == 3 && elt.nextSibling && elt.nextSibling.nodeType == 3 ) {
				elt.previousSibling.textContent += elt.nextSibling.textContent;
				elt.nextSibling.parentNode.removeChild( elt.nextSibling );
			}

			var m = elt.prop_Marker
			$( elt ).remove();
	...
	unmarkByMarker: function( m ) {
		var em, ems = $( ".izMarker" ).get(), len = ems.length, i = m.index;
		while ( len-- ) {
			em = ems[len];
			if ( em.prop_Marker.index === i ) {
				this.unmark( em );
				break;
			}
		}
	...
	////
	addRow: function( m, index ) {
		var tr = document.createElement( "tr" );

		tr.prop_Marker = m;
		tr.innerHTML = "<tr><td>" + "<a href='#izMarker-" + m.index + "' target='_self'>[" + m.index + "]</a></td><td>" + m.note + "</td></tr>";

		$( tr ).find( "td:last" ).bind( "izChangedEvent.izEditable", function() {
			this.parentNode.prop_Marker.note = this.innerHTML;
		} );

		this.$grid.izGrid( "insert", tr, index );
	},

	delRow: function( m ) {
		var tr, rows = this.$grid.find( "tr" ).get(), len = rows.length, i = m.index;
		while ( len-- ) {
			tr = rows[len];
			if ( tr.prop_Marker.index === i ) {
	...
	////
	load: function() {
		var o = JSON.parse( localStorage.getItem( "izMarkers-" + this.url ) );

		if ( o ) {
			this.lastIndex = o.lastIndex;
			this.lastModified = o.lastModified;

			var oms = o.markers, len = oms.length;
			while ( len-- ) {
				this.restore( oms[len] );
				this.addRow( oms[len], -1 );
			}
		}
	},

	save: function() {
		var ems = $( ".izMarker" ).get(), markers = [];

		if ( !ems.length ) {
			localStorage.removeItem( "izMarkers-" + this.url );
			return;
		}

		for ( var i = 0, len = ems.length; len > i; ++i ) {
			markers.push( ems[i].prop_Marker );
		}

		try {
			localStorage.setItem( "izMarkers-" + this.url, JSON.stringify( {
				lastIndex: this.lastIndex,
				lastModified: +new Date,
				markers: markers
			} ) );
		} catch ( e ) {
			console.log( "localStorage[" + "izMarkers-" + this.url + "](E2): " + e );
		}
	},

	clear: function() {
		var ms = $( ".izMarker" ).get(), len = ms.length;

		while ( len-- ) {
			this.unmark( ms[len] );
		}

		this.$grid.izGrid( "clear" );
		this.lastIndex = 0;
	}
};
//--
...
var load = function() {
	if ( $( "#izMarkerInfo" ).length ) {
		return;
	}

	$( '<table class="izMarkerData"><thead><tr><th>Index</th><th>Note</th></tr></thead><tbody></tbody></table>' ).appendTo( document.body );
...
if ( localStorage.getItem( "izMarkers-" + location.href.replace( /#[^\?]+/, "" ) ) ) {
	load();
}

GM_registerMenuCommand( "AMarker", load );
