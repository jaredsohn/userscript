// ==UserScript==
// @name           KOC Power Bot Extra 
// @version        30111217a
// @namespace      Rob
// @homepage       http://userscripts.org/scripts/review/121962
...
function pbStartup (){
  ...
  if (metc.width==null || metc.width==0){
    pbStartupTimer = setTimeout (pbStartup, 1400);
    return;
  }
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    table.pbMainTab {empty-cells:show; margin-top:3px }\
    ...
    table.pbMainTab tr td.sel    {font-weight:bold; font-size:15px; border: 1px solid; border-style: solid solid solid solid; background-color:#eed;}\
    table.pbMainTab tr td.notSel {font-weight:bold; font-size:15px; border: 1px solid; border-style: solid solid solid solid; background-color:#00a044; color:white; border-color:black;}\
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  mainPop = new CPopup ('pb', Options.pbWinPos.x, Options.pbWinPos.y, 785,700, Options.pbWinDrag,
      function (){
        tabManager.hideTab();
...
/*********************************  Cresting Tab ***********************************/
 Tabs.Crest = {
  ...
  helpPop : function (){
    ...
    helpText += 'It will attack a wild in 2 waves.<BR>';
    helpText += 'For timing on Level 2 - 7 add 1 MM to your 2nd wave so that your 2nd wave doesn\'t arrive first.<BR>';
    ...
    helpText += '<TABLE width=100%><TR><TD>Level</td><TD>Wave 1</td><TD>Wave 2</td><TD>Troop losses</td><TD>Min. Fletching</td></tr>';
    ...
    helpText += '<TR><TD>2</td><TD>5 MM</td><TD>130 archers + 1 MM</td><TD>1st Wave</td><TD>2+</td></tr>';
    helpText += '<TR><TD>3</td><TD>10 MM</td><TD>520 archers + 1 MM</td><TD>1st Wave</td><TD>3+</td></tr>';
    helpText += '<TR><TD>4</td><TD>20 MM</td><TD>1600 archers + 1 MM</td><TD>1st Wave</td><TD>4+</td></tr>';
    helpText += '<TR><TD>5</td><TD>50 MM</td><TD>2200 archers + 1 MM</td><TD>1st Wave</td><TD>6+</td></tr>';
    helpText += '<TR><TD>6</td><TD>100 MM</td><TD>3000 archers + 1 MM</td><TD>1st Wave</td><TD>7+</td></tr>';
    helpText += '<TR><TD>7</td><TD>150 MM</td><TD>6000 archers + 1 MM</td><TD>1st Wave</td><TD>8+</td></tr>';
    ...
    var pop = new CPopup ('giftHelp', 0, 0, 600, 400, true);
    pop.centerMe (mainPop.getMainDiv());  
    pop.getMainDiv().innerHTML = helpText;
	 ...
	 var troopDef = [
	  ...
	  ['Cats', 12],
  	 ...
  	 if(t.troopselect == null)	
		t.troopselect = new CPopup ('pbtroopselect', 0, 0, 850, 350, true, function(){t.saveTroops();});
...
Tabs.AutoTrain = {
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>AUTO TRAIN</div><TABLE width=100% height=0% class=pbTab><TR><TD width=200></td>';
        m += '<TD align=left><INPUT id=pbAutoTrainState type=submit value="AutoTrain = '+ (TrainOptions.Running?'ON':'OFF')+'"></td>';
	/*	m += '<TD align=center><INPUT id=pbShowTraining type=submit value="Training Queue"></td>';  */
...
/*	document.getElementById('pbShowTraining').addEventListener('click', function(){
		t.trainPop(); 
	}, false); */
  ...
  // Shows training queues... 
/*  trainPop : function (){
	  
		var trainPop = new CPopup('pbShowTraining', 0, 0, 750, 500, true, function() {clearTimeout (1000);});
		var m = '<DIV style="max-height:460px; height:460px; overflow-y:auto"><TABLE align=center cellpadding=0 cellspacing=0 width=100% class="pbTab" id="pbShowTraining">';       
		trainPop.getMainDiv().innerHTML = '</table></div>' + m;
		trainPop.getTopDiv().innerHTML = '<TD><B>Training Queues:</td>';
		trainPop.show(true);
  }, */
  ...
  helpPop : function (){
    ...
    var pop = new CPopup ('giftHelp', 0, 0, 550, 240, true);
    pop.centerMe (mainPop.getMainDiv());  
    pop.getMainDiv().innerHTML = helpText;
...
/*******************   Chat tab ****************/
Tabs.Chat = {
  tabOrder : 145,                    // order to place tab in top bar
  //tabDisabled : !ENABLE_SAMPLE_TAB, // if true, tab will not be added or initialized
  //tabLabel : 'Click Me',            // label to show in main window tabs
  myDiv : null,
  timer : null,  
...
var AutoUpdater_121962 = {
    ...
    call: function(response, secure) {
	    ...
	    onload: function(xpr) {AutoUpdater_121962.compare(xpr, response);},
            onerror: function(xpr) {if (secure) AutoUpdater_121962.call(response, false);}
        });
    },
    enable: function() {
        GM_registerMenuCommand("Enable "+this.name+" updates", function() {
            GM_setValue('updated_121962', new Date().getTime()+'');
            AutoUpdater_121962.call(true, true)
        });
    },
    ...
    compare: function(xpr,response) {
        ...
        if ( (this.xversion) && (this.xname[1] == this.name) ) {
            ...
            if ( (xpr.responseText.match("the page you requested doesn't exist")) || (this.xname[1] != this.name) ) {
            	//GM_setValue('updated_121962', 'off');
            }
            return false;
        }
        ...
        if ( updated ) {
            ...
            display_confirm('A new version of '+this.xname+' is available.\nDo you wish to install the latest version?',
                ...
                function(){
                    try { 
                        location.href = AutoUpdater_121962.beta ? AutoUpdater_121962.betaUrl :  'http://userscripts.org/scripts/show/121962'; 
                    } catch(e) {}
                },
                // Cancel
                function(){
                    if ( AutoUpdater_121962.xversion ) {
                        if(confirm('Do you want to turn off auto updating for this script?')) {
                            //GM_setValue('updated_121962', 'off');
							GlobalOptions.pbupdate = false;
							GM_setValue ('Options_??', JSON2.stringify(GlobalOptions));
                            AutoUpdater_121962.enable();
    ...
    check: function(tf) {
        if (!tf){
            ...
            if (+this.time > (+GM_getValue('updated_121962', 0) + 1000*60*60*24*this.days)) {
                GM_setValue('updated_121962', this.time+'');
...
if (typeof(GM_xmlhttpRequest) !== 'undefined' && typeof(GM_updatingEnabled) === 'undefined') { // has an updater?
    try {
        AutoUpdater_121962.check(GlobalOptions.pbupdate);
    } catch(e) {
        AutoUpdater_121962.check(GlobalOptions.pbupdate);
    }
}
...
Tabs.Knights = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '<STYLE>table.ptTabPad tr.ptwpad{background-color:#ffffff; padding-left:15px}</style>\
       <DIV id=ptknightdiv style="max-height:620px; height:620px; overflow-y:auto">';
	   t.show();
  },
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        ...
        if (knight.knightStatus == 10){
          ...
          if (unpoints > 0){
            //unpoints = '<SPAN class="boldRed">'+knight.knightStatus+','+ unpoints +'</span>';
