...
// @description    Comptage de deut dépensé et de rentabilité de raide v3.0.4
...
function listederoulante(nombreJoursdepuis1970, type, mission_type_detail_g){
...
{////*********************************************/ PARTIE mise ajours variable /*********************************************************	* /
	...
	var jours_dernier_vsitie = Math.floor(fff/(1000*24*3600));
	if(jours_dernier_vsitie != nombreJoursdepuis1970){
		if(fff.getMonth() != date.getMonth()){//nouveau mois donc on met a 0 le nouveau mois.
...
// ************************************** ************************ RAPPORT De combat : **************************************************/
	...
	var option_total = '<div id="option" style="width:100%;background-color:transparent;color:#999999;text-align:center;display:none;" >'
		...
		if (!document.getElementById('warning')){var menu_lien = document.getElementById('buttonz'); /*Lieu où on veut afficher (A remplacer par ce que vous voulez 	*/}
		...
		/* ************ Element cliquable *************/
			...
			document.getElementById("jours_s").addEventListener("focus", function(event){var liste_jours_set = setInterval(listederoulante(nombreJoursdepuis1970, 0, mission_type_detail_g), 500);}, true);
			document.getElementById("jours_s").addEventListener("blur", function(event){clearInterval(liste_jours_set);}, true);

			document.getElementById("mois_s").addEventListener("focus", function(event){var liste_mois_set = setInterval(listederoulante(nombreJoursdepuis1970, 1, mission_type_detail_g), 500);}, true);
