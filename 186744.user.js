(function () {
// ==UserScript==
// @name          Anti Adfly Dan sejenisnya. [soecity.net]
// @namespace     http://www.soecity.net/
// @author        Uchiha Ricki
// @description   Cara Cepat Hindari kejenuhan.
// @version       6.3
...
// ===========================================================
// @match         http://*.zz.na.tl/*
// @match         http://*.dl.anggelz.net/*
// @match         http://*.go.soecity.net/*
// @match         http://*.ad7.biz/*
// @match         http://*.acb.im/*
...
/*
    ...
    stamp : function() {
      ...
      Extra.doc.title = "soEcity.net " + s;
    },

    countDown : function() {
      var title = "soEcity.net";
      Extra.cdwn -= 1000;
    ...
    services : {
      ...
      adfly : {
        ...
        hosts : "adf.ly | 9.bb | u.bb | j.gs | q.gs | go.soecity.net | acb.im | zz.na.tl | apkpro.net | dl.anggelz.net | d0wn.us | deskanime.net | evozi.com | extremefile.com | facedasgostosas.com | freealbumdownload.net | freepremiumnow.com | gamecopyworld.com | hamdi.web.id | idws.im | isoforest.net | itsrinaldo.net | ksn.mx | marvelavengersfreegifts.com | marvelavengersrewards.com | mediasharingcenter.net | melonescomputer.com | nggablog.com | paxii.de | phpnulledscripts.com | sazlina.com",
        addon : true,
        vars : "jQuery",
      ...
      linkbucks : {
        ...
        fn : function () {
          ...
          else if ((l = YOD.$W.Lbjs) && l.TargetUrl) {
            //l.IsClick = 1;
            Extra.go(l.TargetUrl);
          }
        }
      },
...
function doStuff(init) {
  ...
  for (i in Extra.services) {
    ...
    if (!init || doc) {
      ...
      if (service.addon) {
        // get
        var func = 'if(s=YOD.$W.eu){z=f="";s=s.toString();P=s.indexOf("!HiTommy");if(P!=-1){s=s.substr(0,P);}for(l=0;l<s.length;l++){if((l%2)==0){f+=s.charAt(l);}else{z=s.charAt(l)+z;}}s=f+z;s=atob(s);s=s.substring(2,s.length);if(isUrl(s)){Extra.passed=1;return Extra.go(s);}}';
