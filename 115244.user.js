// ==UserScript==
// @name         bro3_Auto_Bilder_kor
// @namespace    http://at-n2.net/
// @description  일본 nottisan님 버전 개조 by 1서버 좌자
// @include      http://*.nexon.com/*
// ==/UserScript==
//2011.10.10 sd삼국지버전 by 1서버 조은
//2011.09.10 sd삼국지버전 by 1서버 좌자
//2010.10.25 「?究中」「建物削除中」の時建設されないバグを修正
//2010.10.28 「ALLクリア」機能??
//2010.10.28 市場糧自動?換機能???
//2010.10.29 城?砦?村がレベルアップされないバグを修正
//2010.10.29 市場糧自動?換機能バグ取り完了
//2010.10.31 糧村化機能??
//2010.12.24 ?箇所調整+放置ミス?策して勝手に本家として公開ｗ by nottisan
//           30分以上?点?面以外であれば自動で?点?面へ （自動出兵ツ?ルを使ってる人が居るかもしれないことがあるので、
//           出兵選??面?出兵寸前の?面(/facility/castle_send_troop.php)では動作しません。）
//2010.12.31 糧の?換パタ?ンを追加
//           平均?換：糧が指定した量になると、３資源で一番少ない物を指定値?換します。
//           一括?換：糧が指定した量になると、各指定値で?換します。
//           自動?政?動機能を追加（x知識、x技術、?の治世、王佐の才）のみスキル使用します。
//                                  ?点表示時にスキルの使用をチェックします。
//2010.01.02 自動?政?動機能にあった、スキル3つ持ちの武?でないと?動しないバグを修正
//           http://krote.blog21.fc2.com/blog-entry-54.htmlのブログを?考にchromeに??？
...
//市場用
var OPT_RISE_MAX = 30000; //市場?換開始する糧の量
...
var OPT_BPNOGADA = 0; //자동BP노가다옵션
var OPT_TORIDE = 0; //自動砦化オプション
var OPT_SOUKO_MAX = 1; //倉庫の最大?
var OPT_SUKSA_MAX = 6; //숙사의 최대

//?政用 by nottisan
var OPT_DOME = 0;

//グロ?バル??
var VERSION = "1.0.0β";  //バ?ジョン情報
...
var IDX2_TYPE = 2; //種別 C:都市?面、D:?政スキル、Fxy:施設座標
...
var OPT_FUC_NAME = ["거점","벌채소","채석장","제철소","밭","창고",
                    "동작대","대장간","방어구공장","연병소","창병사","궁병사",
                    "마굿간","숙사","무기공장","시장","훈련소","수차","공장",
                    "연구소","대숙사","원정훈련소","감시탑","수행소"];

//市場?換?理用
var OPT_ICHIBA = 0;
var OPT_ICHIBA_PA = 0;
var OPT_ICHIBA_PATS = ["평균적변환","일괄변환"];
//自動寄付用
var OPT_KIFU = 0;
...
function setVillageFacility() {
	...
	for (var i = 0; i < actionsElem.snapshotLength; i++) {
		...
		if (buildStatusElem.snapshotLength > 0) {
			//建物削除等に?? 2010.10.25 byおぜがづ
			for(var j=0; j<buildStatusElem.snapshotLength;j++){
				if(buildStatusElem.snapshotItem(j).parentNode.innerHTML.match(RegExp("(건설중|건설준비중)"))) {
					//施設建設?
					cnt++;
				}
			}
...
//		alert("平地:" + heichi + "\n畑:" + hatake + "\n倉庫:" + souko + "\n雀:" + suzume);
//		return;
//建てられるスペ?スがなければ通常の?理を?ける
	} else if(OPT_BPNOGADA == 1) {
		var area_all = new Array();
		area_all = get_area_all();
		var suksa = 0; //숙사
		var ybso = 0; //연병소
		...
		for(var i=0;i < area_all.length;i++){
			if(area_all[i].name == "평지"){heichi++;n=i;}
			else if(area_all[i].name.match(/숙사\s.*?(\d+)/)){
				hatake++;
				if(parseInt(RegExp.$1)>=OPT_SUKSA_MAX){
					delete_Flag=1;
					//지우기 스크립트 추가.
				}
			}
			else if(area_all[i].name.match(/창고/)){souko++;}
			else if(area_all[i].name.match(/연병소/)){ybso++;}
		}
...
//		alert("平地:" + heichi + "\n畑:" + hatake + "\n倉庫:" + souko + "\n雀:" + suzume + "\n畑が建てられる平地:" + (tmp - hatake));
		if(heichi>0){ //平地が余っていたら
			var tmp = heichi;
			if(souko == 0){ //창고가 없으므로 창고 만들기.
				if(Chek_Sigen(new lv_sort("창고",0,"")) != 1){ //資源チェック
					createFacility(SOUKO, area_all); //倉庫を建てる
					Reload_Flg = 0;
				}
				return;
			} else if(ybso == 0){ //연병소가 없으므로 연병소 만들기.
				if(Chek_Sigen(new lv_sort("연병소",0,"")) != 1){ //資源チェック
					createFacility(YBSO, area_all); //연병소만들기.
					Reload_Flg = 0;
				}
				return;
			}
...
//			if(tmp-hatake > 0){ //それでも平地が余っていれば
			if(tmp > 0){ //それでも平地が余っていれば
				if(Chek_Sigen(new lv_sort("숙사",0,"")) != 1){ //資源チェック
					createFacility(SUKSA, area_all); //畑を建てる
					Reload_Flg = 0;
					return;
				};
			}
...
//建てられるスペ?スがなければ通常の?理を?ける
	...
	for(i=0;i<area.length;i++){
		...
		if(parseInt(area[i].lv) >= parseInt(OPT_MAX_LV)){break;} //指定Lv以上ならメインに?る
		//建築物名分回す
...
//	alert("createFacility");
	...
	for(var i=0;i<area.length;i++){
		if(area[i].name == "평지"){ //一番最初に見つかった平地に建設
			var mURL = CREATELINK;
			mURL = mURL.replace(URL_SITE,HOST);
			var Temp = area[i].xy.split(",");
			mURL = mURL.replace(URL_X,Temp[0]);
...
//		var tid=setTimeout(function(){location.href = vcURL;},12000);
		...
		//サ?バ?への負荷を考え時間を延長
		var tid=setTimeout(function(){location.href = vcURL;},18000);
	}
}
...
//	var sidebar = d.evaluate('//*[@id="statusIcon"]', d, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	var sidebar = d.evaluate('//*[@id="mapboxInner"]',d, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
*/	
	...
	ccreateCheckBox(tdA, "OPT_CHKBOX_AVC", OPT_CHKBOX_AVC, "거점돌기","5분마다 거점들을 돌게 할 경우 체크해주세요",0);
	trA.appendChild(tdA);
	...
	ccreateButton(tdB, "저장", "설정을 저장합니다", function() {saveAVCBox()});
	trB.appendChild(tdB);
	tbl3.appendChild(trB);
	
	tr3.appendChild(td4);
	tbl3.appendChild(tr3);
	ccreateButton(td4, "닫기", "창을닫습니다", function() {closeIniBilderBox()});
...
//	var popupTop = 150;
	...
	ccreateCheckBox(td1, "OPT_CHKBOX0", OPT_CHKBOX[0], "거점", "중앙의 성, 마을, 요새의 레벨을 올립니다", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX1", OPT_CHKBOX[1], "벌채소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX2", OPT_CHKBOX[2], "채석장", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX3", OPT_CHKBOX[3], "제철소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX4", OPT_CHKBOX[4], "밭", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX5", OPT_CHKBOX[5], "창고", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX6", OPT_CHKBOX[6], "동작대", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX7", OPT_CHKBOX[7], "대장간", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td1, "OPT_CHKBOX8", OPT_CHKBOX[8], "방어구공장", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX9", OPT_CHKBOX[9], "연병소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX10", OPT_CHKBOX[10], "창병사", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX11", OPT_CHKBOX[11], "궁병사", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX12", OPT_CHKBOX[12], "마굿간", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX13", OPT_CHKBOX[13], "숙사", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX14", OPT_CHKBOX[14], "무기공장", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX15", OPT_CHKBOX[15], "시장", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX16", OPT_CHKBOX[16], "훈련소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td2, "OPT_CHKBOX17", OPT_CHKBOX[17], "수차", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td3, "OPT_CHKBOX18", OPT_CHKBOX[18], "공장", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 

	ccreateCheckBox(td3, "OPT_CHKBOX19", OPT_CHKBOX[19], "연구소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td3, "OPT_CHKBOX20", OPT_CHKBOX[20], "대숙사", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td3, "OPT_CHKBOX21", OPT_CHKBOX[21], "원정훈련소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td3, "OPT_CHKBOX22", OPT_CHKBOX[22], "감시탑", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0); 
	ccreateCheckBox(td3, "OPT_CHKBOX23", OPT_CHKBOX[23], "수행소", "자동으로 Lv 증가하는 건축물에 체크를하십시오.", 0);

	ccreateTextBox(td1, "OPT_MAX_LV", OPT_MAX_LV, "최대레벨", "자동으로 LV를 올리는 최대 수준을 지정합니다.", 5, 5);
	...
	//糧村化
	ccreateCheckBox(td11, "OPT_KATEMURA", OPT_KATEMURA, "쌀마을화", "이 거점을 쌀 마을 화합니다. 평균적으로 밭, 창고 4 개, 동작 대를 만듭니다", 0);
	ccreateTextBox(td11, "OPT_SOUKO_MAX", OPT_SOUKO_MAX, "설치할 창고?", "설치 창고?를 지정하십시오.", 1,1);
	
	//노가다
	ccreateCheckBox(td11, "OPT_BPNOGADA", OPT_BPNOGADA, "BP노가다", "이 거점을 BP노가다 화합니다. 평균적으로 숙사, 창고, 연병소를 만듭니다", 0);
	ccreateTextBox(td11, "OPT_SUKSA_MAX", OPT_SUKSA_MAX, "최대레벨", "숙사 LV를 지정하십시오.(최대레벨 후 파괴)", 1,1);

	//市場変換処理用
	ccreateCheckBox(td11, "OPT_ICHIBA", OPT_ICHIBA, "시장 자동? 전환", "이 도시에서 음식 시장 자동? 전환합니다.", 0);
	ccreateComboBox(td11, "OPT_ICHIBA_PA", OPT_ICHIBA_PATS, OPT_ICHIBA_PA, "? 전환 파타? 없습니다", "평균? 전환 : 양식이 일정량 된 때? 전환 지정하는 가장 적은 자원을? 변환합니다. 대량? 전환 : 양식이 일정량 된 때 지정되어있는 자원을 지정 값? 전환합니다. ", 5);
	ccreateTextBox(td11, "OPT_RISE_MAX", OPT_RISE_MAX, "음식이 적당한? 양에되면? 전환한다", "자동으로 음식을 다른 자원에? 변환 시작 양을 지정합니다.", 10, 5);
	ccreateTextBox(td11, "OPT_TO_WOOD", OPT_TO_WOOD, "나무? 전환하는 음식의 양", "자동으로 나무에? 전환하는 음식의 양을 지정합니다.", 10, 5);
	ccreateTextBox(td11, "OPT_TO_STONE", OPT_TO_STONE, "돌에? 전환하는 음식의 양", "자동으로 돌에? 전환하는 음식의 양을 지정합니다.", 10, 5);
	ccreateTextBox(td11, "OPT_TO_IRON", OPT_TO_IRON, "철에? 전환하는 음식의 양", "자동으로?에? 전환하는 음식의 양을 지정합니다.", 10, 5);

	//自動寄付用
	ccreateCheckBox(td11, "OPT_KIFU", OPT_KIFU, "자동 기부", "이 도시에?되면 자동으로 기부합니다.", 0);
	ccreateTextBox(td11, "OPT_RISE_KIFU_MAX", OPT_RISE_KIFU_MAX, "음식이 적당한? 양에되면 기부", "자동으로 음식을 기부 시작 양을 지정합니다.", 10, 5);
	ccreateTextBox(td11, "OPT_RISE_KIFU", OPT_RISE_KIFU, "자동으로 음식을 기부하는 양", "자동으로 음식을 기부 양을 지정합니다.", 10, 5);

	//自動内政用
	ccreateCheckBox (td11, "OPT_DOME", OPT_DOME, "자동 내정", "이 도시에?되면 자동으로? 정부 스킬을? 시작합니다.", 0);
	...
	tr3.appendChild(td4);
	tbl3.appendChild(tr3);
	ccreateButton(td4, "저장", "설정? 내용을 저장합니다", function() {SaveInifacBox(ABfacContainer.getAttribute ('vId'))});
	ccreateButton(td4, "닫기", "설정? 내용을 저장하지 않고 닫습니다", function() {closeInifacBox()});
//	ccreateButton(td4, "ALL???", "設定?容を消去します。", function() {closeInifacBox()});
	ccreateButton(td4, "초기화","설정? 내용을 지웁니다. ", function() {clearInifacBox()});
	ABfacContainer.appendChild(tbl3);
}
//?点?位の設定の保存（XY MAX_LV CheckData)
function SaveInifacBox(vId){
	...
	//市場?換?理用
	strSave += cgetCheckBoxValue($("OPT_ICHIBA")) + DELIMIT2; //市場で?換するかどうかのフラグ
	...
	strSave += cgetTextBoxValue($("OPT_RISE_KIFU")) + DELIMIT2; //自動?政用に修正
	
	//自動?政用 by nottisan ここから追加
	strSave += cgetCheckBoxValue($("OPT_DOME")) + DELIMIT2; //?政使用するかのフラグ
	strSave += cgetComboBoxValue($("OPT_ICHIBA_PA")) + DELIMIT2; //市場での?換パタ?ンフラグ
	//ここまで
	
	//BP화
	strSave += cgetCheckBoxValue($("OPT_BPNOGADA")) + DELIMIT2;
	strSave += cgetTextBoxValue($("OPT_SUKSA_MAX"));
...
//	for(i=0; i<=Temp2.length;i++){
	...
	//糧村化
	if(Temp2[36] == ""){return;}
	OPT_BPNOGADA = parseInt(Temp2[36]);
	var tmpNaN = parseInt(Temp2[37]);
	if(isNaN(tmpNaN)){return;}
	OPT_SUKSA_MAX = tmpNaN;
...
//		return; ///// んょ
	...
	// 資源のバランスが?い場合、少ない資源を優先的に?換する?理
	// 基本的には?換後も現在の最大量を超えない場合に?換を行う。
	// すなわち、量が最大の資源は?換を行わない。
	...
	var tid=setTimeout(function(){
		GM_xmlhttpRequest({
			...
			onload:function(x){
				...
				if (!x.responseXML) {
					x.responseXML = new DOMParser().parseFromString(x.responseText, "text/xml");
				}
				//alert("test::[" + x.responseXML + "]");
				var skillElem = document.evaluate('//*[@class="skill"]',x.responseXML, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
				//alert(skillElem);
				if(skillElem.snapshotLength < 1){return;}
				
				//alert(skillElem.snapshotLength);
				for(i=1;i<skillElem.snapshotLength;i++){
					var skillTag = trim(skillElem.snapshotItem(i).innerHTML);
					if((skillTag.indexOf("지식",0) > 1 )||(skillTag.indexOf("기술",0) > 1 )||(skillTag.indexOf("오의치세",0) > 1 )||(skillTag.indexOf("왕좌",0) > 1 ) ){
						//alert("test:" + skillTag);
