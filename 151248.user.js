...
.ptstat {border:1px solid; border-color:#000000; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#00468a; -moz-border-radius:5px;}\
...
table.ptMainTab tr td.notSel {font-weight:bold; font-size:13px; color: #ffffff; border: 1px solid #000000; background: -moz-linear-gradient(top, #cfeef7 0%, #73cee7 24%, #00468a 44%, #53c2e1 79%, #88d5ea 100%);}\
...
//CITTA'
//BOT It
if (m[0].indexOf('La mia ambasciata ha (') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
if (m[0].indexOf('Senza Ambasciata') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
//BOT
if (m[0].indexOf('My embassy') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
//BAO, Dracul Power Orginal e Multilang It
if (m[0].indexOf('ambasciata (') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
//BOA En
if (m[0].indexOf('ambassy (') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
//Multilang EN
if (m[0].indexOf('embassy slots (') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';

//TERRE
//BOT It
if (m[0].indexOf('La mia terra a') >= 0 && Options.chatAttack)
element_class = ' ptChatAttack';
//BOT
...
//CITTA'
//BOT It
if (whisp.indexOf('La mia ambasciata ha (') >= 0 && Options.enableTowerAlert) {
AudioManager.setSource(SOUND_FILES.alert);
AudioManager.play();
setTimeout(function(){AudioManager.stop();}, 8000);
}
if (whisp.indexOf('Senza Ambasciata') >= 0 && Options.enableTowerAlert) {
AudioManager.setSource(SOUND_FILES.alert);
AudioManager.play();
setTimeout(function(){AudioManager.stop();}, 8000);
}
...
//BOA En
if (whisp.indexOf('ambassy (') >= 0 && Options.enableTowerAlert) {
AudioManager.setSource(SOUND_FILES.alert);
AudioManager.play();
setTimeout(function(){AudioManager.stop();}, 8000);
}
//Multilang EN
if (whisp.indexOf('embassy slots (') >= 0 && Options.enableTowerAlert) {
AudioManager.setSource(SOUND_FILES.alert);
AudioManager.play();
setTimeout(function(){AudioManager.stop();}, 8000);
}
...
//TERRE
//BOT It
if (whisp.indexOf('La mia terra a') >= 0 && Options.enableTowerAlert) {
...
var mhtl = "<DIV class=ptstat>INFORMAZIONI POPOLAZIONE E PRODUZIONE</div><table width=100% class=pdxTab><tr><td colspan=8>&nbsp;</td></tr><tr><td></td>";
for(var i=0; i<Cities.numCities; i++) {
mhtl += "<TD align=center valign=bottom width=60px><B>" + Cities.cities[i].name + "</B></TD>";
}
mhtl += "</tr><tr><td><img height=18 src=http://koc-power-pdx.googlecode.com/svn/trunk/img/troops/unit_2_50_s34.jpg title=Militiaman> Miliziani/h</td>";
var temps=[];
for(var i=0; i<Cities.numCities; i++) {
temps[i]=((Cities.cities[i]['Troop2Time'] > 0)?(3600 / Cities.cities[i]['Troop2Time']):0);
mhtl += "<td>" + addCommas(parseInt(temps[i])) +"</td>";
}
mhtl += "</tr><tr><td><img height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/population_40.png title=Population> Popolazione/h</td>";
var pop=[];
for(var i=0; i<Cities.numCities; i++) {
...
mhtl += "</tr><tr><td><b>Differenza</b></td>";
var diff=0;
for(var i=0; i<Cities.numCities; i++) {
...
mhtl += "</tr><tr><td><img height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/happiness.png title=happiness> felecità</td>";
for(var i=0; i<Cities.numCities; i++) {
cityID = 'city'+ Cities.cities[i].id;
...
m += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToWilds" target="_blank">AIUTO</a></td></tr>';
m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name
+' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Terre: '+ totw +' di '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
var row = 0;
var sortem = [];

if (matTypeof(cWilds) != 'array') {
m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wild Type</td><TD></td><TD align=left>Coord.</td><TD>Trap.</td><TD align=left>Merc.</td><TD>Abbandona</td>\
...
<TD></td><TD align=left class=ptentry><B>Costr. Trap.:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i
...
m += '<TD class=ptentry> &nbsp; &nbsp; <B>Merc.:</b> ' + htmlSelector(mercNames, wildDef.mercLevel, 'id=ptwm_'+ c +'_'+ i) +' &nbsp; &nbsp; </td></tr>';
}
...
<TD width=10%>'+ strButton20("SETTA DIFESE", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Gold: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
} else {
...
ass = '<TD class=ptentry align=left colspan=4>Marcia</td>';
} else {
if (unpoints > 0){
...
m += '<TR><TD colspan=15><DIV class=ptstat>Metodo Assegnamento - <input style="height:20px;font-size:9px;" type=button value="Add Default Skill" id="ptknight_def"><input style="height:20px;font-size:9px;" type=button value="Add Combat Skill" id="ptknight_com"></div></td></tr>';
for (var c=0; c<Cities.numCities; c++) {
var cid = Cities.cities[c].id;
m += '<TR><TD colspan=15><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
<TR class=ptwpad style="font-weight:bold" align=right><TD width=70>Ruolo</td><TD width=140 align=center>Nome</td><TD widith=70>Dimetti</td><TD width=26>Pol</td><TD width=26>Com</td>\
<TD width=26>Int</td><TD width=26>Res</td><TD width=90 align=center colspan=5>--- Unassigned ---</td><TD width=35>Level</td><td>Lealtà</td><TD width=40 align=right> Salario </td></tr>';
totSalary = 0;
var did = {};
var row = 0;
...
m += '<TR align=right><TD colspan=13><B>Totale Salari:</b></td><TD>'+ addCommas(totSalary) +'</td></tr>';
}
...
.clickableSel{background-color:#ffffcc;}\
.xxtab{background-color:none; padding-left:5px; padding-right:5px;} </style>\
<DIV class=ptstat ><TABLE id=tabAllMembers cellpadding=0 width=100%><TR font-weight:bold"><TD class=xtab>Alleanza: '+ allName +'</td>\
...
<TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Città</a></div></td>\
...
<TD id=clickCol10 onclick="PTalClickSort(this)" class=clickable><A><DIV>Tempo</a></div></td>\
<TD class=clickable><A><DIV>'+uW.g_js_strings.modal_messages_viewreports_view.lastlogin+'</a></div></td></tr></thead>\
<TBODY id=allBody style="background-color:#ffffff;"></tbody></table></div>\
...
document.getElementById('allListOut').innerHTML = m; //style="top:670px; left:0px; position:absolute;
document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
<TD class=xtab>Show distance from: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; O, seleziona città: <span id=dmcoords></span></td></tr></table>';
document.getElementById('clickCol'+t.sortColNum).className = 'clickable clickableSel';
...
var m = '<DIV style="max-height:735px; height:735px;"><DIV class=ptstat>MAPPA PROVINCIE</div><DIV id=ptProvMap style="height:'+ provMapCoords.imgHeight +'px; width:'+ provMapCoords.imgWidth +'px; background-repeat:no-repeat; background-image:url(\''+ URL_PROVINCE_MAP +'\')"></div>';
...
<TR><TD class=xtab align=left><B>Prima Posizione: </b></td><TD class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> O, scegli città: <SPAN id=ptloc1></span></td></tr>\
<TR><TD class=xtab align=left><B>Seconda Posizione: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> O, scegli città: <SPAN id=ptloc2></span></td></tr></table>\
...
var main = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubU type=submit value="Opzioni"></td>';
main +='<TD><INPUT class=pbSubtab ID=ptmrchSubV type=submit value="Layout"></td></tr></table><HR class=ptThin>';
main += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToOptions" target="_blank">AIUTO</a></td></tr>';
...
m+='<TR><TD><INPUT id=ptHideOnGoto type=checkbox /></td><TD>Chiudi la finestra quando clicchi sulle coordinate nella mappa.</td></tr>';
m+='<TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Mostra cibo in rosso se in esaurimento in meno di';
m+='<INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> ore, non influisce sul allarme cibo!</td></tr>';
m+='<TR><TD><INPUT id=ptEnableFoodTower type=checkbox /></td><TD>Abilita Torre allarme cibo. (Set Attenzione a 6 ore, controllati ogni 30 min.)</td></tr>';
m+='<TR><TD><INPUT id=ptEnableWisperAlert type=checkbox /></td><TD>Abilita allarme bisbigli</td></tr>';
m+='<TR><TD><INPUT id=ptEnableTowerAlert type=checkbox /></td><TD>Attiva segnale audio per attacchi/esplorazioni in chat.</td></tr>';
m+='<TR><TD><INPUT id=ptupdate type=checkbox '+ (GlobalOptions.ptupdate?'CHECKED ':'') +'/></td><TD>Scarica aggiornamenti da: '+ htmlSelector({0:'Userscripts', 1:'Google Code'},GlobalOptions.ptupdatebeta,'id=ptupdatebeta') +' (all domains) &nbsp; &nbsp; <INPUT id=ptupdatenow type=submit value="Scarica Ora" /></td></tr>';
m+='<TR><TD colspan=2><B>KofC Features:</b></td></tr>';
m+='<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Abilita report alleanza avanzati.</td></tr>';
m+='<TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Lasciare altri script per cambiare il formato dei report alleanza.</td></tr>';
m+='<TR><TD><INPUT id=togAllMembers type=checkbox /></td><TD>Abilita visione avanzata membri alleanza.</td></tr>';
m+='<TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Pagina di navigazione avanzata per i messaggi e report.</td></tr>';
m+='<TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Abilita orologio GMT accanto a "Tempo di Camelot"</td></tr>';
m+='<TR><TD><INPUT id=togAttackPicker type=checkbox /></td><TD>Abilita selezione città di destinazione nella finestra di dialogo (rafforza, riassegna e trasporta).</td></tr>';
m+='<TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Mostra nei report di battaglia il numero dei round.</td></tr>';
m+='<TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>Abilita pulsante elimina quando guardo i report di battaglia.</td></tr>';
m+='<TR><TD><INPUT id=togRptGift type=checkbox /></td><TD>Abilita pulsante elimina regali in posta in arrivo</td></tr>';
m+='<TR><TD colspan=2><BR><BR><B>KofC Bug Fixes:</b></td></tr>';
m+='<TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Fissare report torre per mostrare destinazione esatta (città, terre o non valido)</td></tr>';
m+='<TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD>Non selezionare automaticamente un cavaliere quando si cambia tipo di marcia di esporazione, trasporto o riassegno</td></tr>';
m+='<TR><TD><INPUT id=togCoordBox type=checkbox /></td><TD>Mantenere box coordinate mappa/contrassegni in cima di attività delle truppe</td></tr>';
m+='<TR><TD><INPUT id=togMapInfo type=checkbox /></td><TD>Fissare pulsante Riassegna nel maptile info</td></tr>';
m+='<TR><TD><INPUT id=togMapInfo2 type=checkbox /></td><TD>Aggiungi il pulsante Riassegna quando si fa clic sulla propria città.</td></tr>';
m+='<TR><TD colspan=2><B>Auto Training:</b></td></tr>';
m+='<TR><TD></TD><TD><INPUT id=optAutoTrainMins type=text size=1 value="'+ parseInt(AutoTrainOptions.intervalSecs/60) +'"> minuti tra auto-training..</td></tr>';
m+='</table><BR><BR><HR>Se un checkbox è grigia probabilmente è stata una modifica del codice di KoC, che rendende inutilizzabile l\'opzione.';
t.Overv.innerHTML = m;
...
m+='<TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Abilita miglioramenti chat (coordinate cliccabili, clicca sull\'icona per bisbigliare, colori).</td></tr>';
m+='<TR><TD><INPUT id=togChatGlobal type=checkbox /></td><TD>Abilita sfondo chat globale.</td></tr>';
m+='<TR><TD><INPUT id=togChatAlliance type=checkbox /></td><TD>Abilita sfondo chat alleanza.</td></tr>';
m+='<TR><TD><INPUT id=togChatWhisper type=checkbox /></td><TD>Abilita bisbigli in grassetto.</td></tr>';
m+='<TR><TD><INPUT id=togChatBold type=checkbox /></td><TD>Abilita chat in grassetto.</td></tr>';
m+='<TR><TD><INPUT id=togChatAttack type=checkbox /></td><TD>Abilita sfondo torre d\'allerta.</td></tr>';
m+='<TR><TD><INPUT id=togChatLead type=checkbox /></td><TD>Abilita sfondo chat per i leaders.</td></tr></table>';

m+='<TABLE class=ptTab><BR><TR><TD colspan=2><U><B>Colori: &nbsp;(Refresh per fare effetto)</b></u></td></tr>';
...
m+='<TR><TD>Chat Color - Bisbigli: </td><TD><INPUT id=togWhisper type=text size=7 maxlength=7 value="'+Colors.ChatWhisper+'"></td>&nbsp;<TD style="background-color:'+Colors.ChatWhisper+'" width=30px>&nbsp;</td></tr>';
m+='<TR><TD>Chat Color - Cancelliere: </td><TD><INPUT id=togChatC type=text size=7 maxlength=7 value="'+Colors.ChatChancy+'"></td>&nbsp;<TD style="background-color:'+Colors.ChatChancy+'" width=30px>&nbsp;</td></tr>';
...
m+='<TR><TD>Chat Color - Ufficiale: </td><TD><INPUT id=togChatLeaders type=text size=7 maxlength=7 value="'+Colors.ChatLeaders+'"></td>&nbsp;<TD style="background-color:'+Colors.ChatLeaders+'" width=30px>&nbsp;</td></tr>';
...
main +='<TR align=left><TD><SPAN class=ptStatLight>Creato:</span> '+ dt.toLocaleDateString() +'</td>';
main +='<TD><SPAN class=ptStatLight>Potere:</span> ' + addCommas(Seed.player.might) +'</td>';
main +='<TD><SPAN class=ptStatLight>Allenza:</span> ' + getMyAlliance()[1] +'</td>';
...
main += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToOverview" target="_blank">AIUTO</a></td></tr>';
...
//Useful links
var u='<DIV class=ptstat>LINK UTILI</div><DIV id=ptLinks><TABLE align=center cellpadding=1 cellspacing=0><TR>';
u+='<TD width="300px" ; border:none">Scripts</td><TD width="300px" ; border:none">Information sites</td></tr>';
u+='<TR><TD width="300px" ; border:none"><a href="http://userscripts.org/scripts/show/131451" target="_blank">Power Tools It</a></td>';
...
u+='<TD width="300px" ; border:none"><a href="http://darthsith.altervista.org/" target="_blank">Darth Sith Web Sith</a></td></tr>';
u+='<TR><TD width="100px" ; border:none"><a href="http://userscripts.org/scripts/show/131152" target="_blank">Power Bot It</a></td>';
...
u+='<TD width="100px" ; border:none"><a href="https://addons.mozilla.org/en/firefox/addon/scriptish/" target="_blank">Scriptish</a></td>';
u+='</tr></table></div><BR>';
...
u+='<DIV class=ptstat>INFO STEMMI & SIGILLI</div><DIV id=ptLinks><TABLE align=center cellpadding=1 cellspacing=0><TR>';
...
<DIV style="overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>INFO UNITA\'</div><TABLE align=center cellpadding=1 cellspacing=0>\
<TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>COSTO ARRUOLAMENTO</b></td><TD class=xtabHL colspan=7><B>STATS</b></td><TD class=xtabHL><B>Mantenimento</b></td></tr>\
<TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Cibo</td><TD class=xtabH>Legno</td><TD class=xtabH>Stone</td>\
<TD class=xtabH>Mine</td><TD class=xtabH>Pop</td><TD class=xtabHL>Potere</td><TD class=xtabH>Life</td><TD class=xtabH>Att.</td><TD class=xtabH>Dif.</td><TD class=xtabH>Speed</td><TD class=xtabH>Gittata</td><TD class=xtabH>Carico</td>\
...
u += '<DIV class=ptstat>STIMA TEMPI DI ARRUALAMENTO</div><TABLE align=center cellpadding=1 cellspacing=0><TR align=right><TD></td>';
infoRows = [];
for (r=0; r<25; r++)
infoRows[r] = [];
for(i=0; i<Cities.numCities; i++) {
...
var ModelCity = {};
if (t.state == null) {
m = "<DIV class=ptstat><b>STRUMENTO MARCE RAPIDE</b></div>";
m +="<div id='statpourRAA'></div>";
m += "<TABLE width=600 class=ptTab border=0 align=center>\
<tr><td><INPUT type=checkbox id=ptmarch_autoknight "+(Options.marchautoknight?'CHECKED':'')+" /> Auto seleziona cavaliere</td></tr>\
<tr><td colspan=4 align=center><input type=button id=REEaction value='Esplora'>&nbsp;<input type=button id=RAAaction value='Attacca'>&nbsp<input type=button id=RARaction value='Riassegna'>&nbsp;<input type=button id=RENaction value='Rinforza'>&nbsp;<input type=button id=RENBaction value='Rinforza + Max Cibo'></td></tr><tr align=center valign=top><td width=130><b><u>Sorgente</b></u><br><span id=RAAsrcRptspeedcity></span></td>\
<td><b><u>Destination</b></u><br>X:<input type=text id=RAAtypetrpx size=3>&nbsp;Y:<input type=text id=RAAtypetrpy size=3><br><a href='javascript:void(0);' id='BOchargelistelieux'>Lista Membri</a> : <select id='listeFavori'></select></td>\
<td><b><u>Distance</u></b><br><span id='BOEstimationD'>&nbsp;</span><td><b><u>Closest City</u></b><br><span id=BOVilleProche></span>\
</tr><tr align=center valign=top>\
<td colspan=4 align=left><table border=0 bordercolor=black cellspacing=0 cellpadding=0 width=100% style='text-align:center'><tr><td rowspan=13><div id=RAAstatsource></div></td><td colspan=2><a href='javascript:void(0)' id=BO_RAZ_Units title='Clear' >Unità Selezionate</a></td><td>Tempo Attacco</td><td>Tempo Rinforzo</td></tr>";
for (r=1; r<13; r++){
...
t.BOCompAttack.innerHTML='<center><font color=red>LANCIO ATTACCO EFFETTUATO</font></center>';
t.destinationCityx.value = Options.AttackCibleX;
t.destinationCityy.value = Options.AttackCibleY;
ById("RAApiKnight").value=Options.AttackKnight;
nHtml.Click(ById("ptRAA0_"+Cities.byID[Options.AttackFromCity].idx));
...
t.BOCompAttack.innerHTML='<center><font color=red>IMPOSSIBILE</font></center>';
clearTimeout (t.BOAttackTimer);
Options.AttackGoHorloge=null;
saveOptions();
return false;
...
var tc = '<DIV class=ptstat>CERCA IN POSTA IN ARRIVO, POSTA IN USCITA, REPORT PERSONALI E DELL\'ALLEANZA</DIV><DIV class=ptentry><TABLE><TR align=center valign=center>';
tc += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToMessages" target="_blank">AIUTO</a></td></tr>';
...
tc += '<OPTION value="player" ' + (Options.rptType=='player'?'SELECTED':'') + '>Report Personali</OPTION>';
...
tc += '<BR />Pagine:&nbsp;<INPUT id="idRptPageFrom" size=1 value="' + Options.arPageFrom + '">–<INPUT id="idRptPageTo" size=1 value="' + Options.arPageTo + '"></TD>';
tc += '<TD class=xtab align=right>Attaccanti:&nbsp;<SELECT id="idRptAttacker">'; // Options.arPageFrom - Options.arPageTo
tc += '<OPTION value="Them" ' + (Options.arAttacker=='Them'?'SELECTED':'') + '>Loro</OPTION>';
tc += '<OPTION value="Us" ' + (Options.arAttacker=='Us'?'SELECTED':'') + '>Noi</OPTION>';
tc += '<OPTION value="Both" ' + (Options.arAttacker=='Both'?'SELECTED':'') + '>Tutti</OPTION></SELECT>';
tc += '<BR />Obbiettivo:&nbsp;<SELECT id="idRptTarget">';
tc += '<OPTION value="Them" ' + (Options.arTarget=='Them'?'SELECTED':'') + '>Loro</OPTION>';
tc += '<OPTION value="Us" ' + (Options.arTarget=='Us'?'SELECTED':'') + '>Noi</OPTION>';
tc += '<OPTION value="Both" ' + (Options.arTarget=='Both'?'SELECTED':'') + '>Tutti</OPTION></SELECT></TD>';
tc += '<TD class=xtab align=right>Contiene:&nbsp;<INPUT id=idRptWhat type=text size=11 maxlength=50 value=""><BR />';
tc += 'Ma no:&nbsp;<INPUT id=idRptWhatNot type=text size=11 maxlength=50 value=""></TD>';
...
tc += '<TD class=xtab align=left><INPUT id=idRptSearch type=submit value="Inizia Ricerca" /></TD></TR></TABLE></DIV>';
...
var m = '<DIV class=ptstat>FUNZIONI ALLEANZA</div><TABLE align=center cellpadding=1 cellspacing=0></table>';
...
m +='<TABLE class=ptTab><TD width=200px>Lista Membri Alleanza</td><TD>Ordina Per: '+ htmlSelector({Name:'Nome',Might:'Potere',glory:'Gloria',Cities:'Città',Position:'Posizione',dip:'Giorni In Posizione',uid:'User Id',fbuid:'Facebook id'},null,'id=searchAlli') +'</td>';
m += '<TD><INPUT id=alList type=submit value="Lista"></td>';
m += '<TD id=progress></td>';
m += '<TR><TD width=200px>Mosta Diplomazia Alleanza</td><TD><INPUT id=aldiplo type=submit value="Lista diplomazie"></td></tr></table>';

m+='<DIV class=ptstat>PANORAMICA</div><TABLE align=center cellpadding=1 cellspacing=0></table>';
...
row.insertCell(2).innerHTML = "<u><b>Nome</b></u>";
row.insertCell(3).innerHTML = "<u><b>Potere</b></u>";
row.insertCell(4).innerHTML = "<u><b>Gloria</b></u>";
row.insertCell(5).innerHTML = "<u><b>Città</b></u>";
row.insertCell(6).innerHTML = "<u><b>Posizione</b></u>";
row.insertCell(7).innerHTML = "<u><b>Giorni In Posizione</b></u>";
row.insertCell(8).innerHTML = "<u><b>Ultimo Login</b></u>";
row.insertCell(9).innerHTML = "<u><b>Entrato</b></u>";
},
...
else m += '<TABLE class=xtab><TR><TD>Nome Alleanza</td><TD>Membri</td></tr>';
for (k in Seed.allianceDiplomacies['friendly']){
...
try {
m = '<TABLE class=ptTab><TR><TD colspan=2><B>Scanner Report Alleanza:</b></td></tr>';
m += '<TR><TD><INPUT id=togEnhanceAR type=checkbox /></td><TD>Abilita post dei report alleanza in chat alleanza.</td></tr>\
<TR><TD></td><TD><TABLE>\
<TR><TD>Intervallo scanner: <INPUT id=ptalertinterval type=text size=3 value='+Options.alertinterval+' /> secondi\
<TR><TD><INPUT id=ptalerttext type=checkbox '+ (Options.celltext.enable?'CHECKED ':'') +'/> Invia testo in allarme</td></tr>\
<TR><TD colspan=2><table><tr><td align=left>Testo messaggio a: <INPUT id=ptnum1 type=text size=4 maxlength=4 value="'+ Options.celltext.num1 +'" '+(Options.celltext.provider==0?'DISABLED':'')+'\> &nbsp;<INPUT id=ptnum2 type=text size=3 maxlength=3 value="'+ Options.celltext.num2 +'" '+(Options.celltext.provider==0?'DISABLED':'')+'\> &nbsp;<INPUT id=ptnum3 type=text size=4 maxlength=4 value="'+ Options.celltext.num3 +'" '+(Options.celltext.provider==0?'DISABLED':'')+'\></td></tr>\
<TR><TD align=left>Paese: <select id="ptfrmcountry">';
