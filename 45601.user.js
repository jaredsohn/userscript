...
function findName(test) {
    if (test.innerHTML.indexOf('home.pl?user_id=131165', 0)>=0) return 1;
  return 0;
}
...
for(var i=0,j=els.length; i<j; i++) {
	if (findName(els[i])){
		var online = getElementsByClassName('Turned On', els[i])
		var offline = getElementsByClassName('Getting Off To Pr0n', els[i])
		if (online.length > 0){els[i].removeChild(online[0])}
		if (offline.length > 0){els[i].removeChild(offline[0])}
		els[i].innerHTML = els[i].innerHTML + '<b>New</b><br/>'
