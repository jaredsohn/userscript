// ==UserScript==
// @name	Facebook Sohbet Penceresi İfadeleri
// @namespace	Facebook Sohbet Penceresi İfadeleri
// @version           1.0
...
/* START: Desing By "DoctoR.Ossi" (Osman Aplak) */
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@versionnumber\s+(\d+\.\d+)/)[1] > version) {
			if(confirm("'Facebook Sohbet İfadeleri İçin Yeni Güncelleem Mevcut.Yüklemek İstermisiniz?")) openInTab('http://userscripts.org/scripts/source/109117.user.js');
		}
	}
...
/* END */
    ...
    if (headTag) {
		...
		styleTag.innerHTML =
			'.chat_tab_emot_bar {padding-top: 2px; padding-bottom: 6px; line-height: 17px; padding-left: 2px; background:#EEEEEE none repeat scroll 0 0; border-style: solid; border-width: 0px 0px 1px 0px; border-color: #C9D0DA; position: static; }'+
			'.chatstylesbut {width: 14px; height:14px; background-image: url("' + ResourcesURL + 'v1/yM/r/VnpYaU1mIhm.gif"); cursor: pointer; border-color: rgb(153, 153, 153) rgb(153, 153, 153) rgb(136, 136, 136); }'+
	...
	var fEmotsDom = document.createElement('img');
	fEmotsDom.setAttribute('alt',' Yapımcı: http://www.facebook.com/evrenser ');
	fEmotsDom.setAttribute('src',ImagesURL + 'blank.gif');
	fEmotsDom.setAttribute('class','chatstylesbut');	
	fEmotsDom.setAttribute('style','background-position: -1px -1px;');
	fEmotsListDom.appendChild(fEmotsDom);
