// ==UserScript==
// @name           KOC Power Tools Español 1.9
// @namespace      Traduccido por Pedro Vico
...
var Version = '20101207a';
var MAP_DELAY = 1500;
var CANCEL_TRAINING_DELAY = 1500;   // without a delay, some (recoverable) 'algo salió mal' errors occur. The server is probably delaying SQL updates on cancels.
...
var Options = {
  ...
  allowAlterAR : true,
  enhanceMsging : true,
  alertConfig  : {aChat:false, aPrefix:'*** ALERTA DE ATAQUE!!!!!!! ***(edita aquí tu propio mensaje)', scouting:false, wilds:false, minTroops:1, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  readOptions();
  AddMainTabLink('MENÚ', eventHideShow);
  setCities();
...
function setupTowerAlert (){
  if ( !alterUwFunction ('attack_viewimpending_view', [['Modal.showModal', 'eventViewingImpending(a, e); Modal.showModal']])){
    logit ("No se puede conectar a la función de vision de la torre de alerta!");
    return;
  }
...
logit ("Tower report (m"+ m.mid +") already posted, ignoring!");
  ...
  if ( city.tileId == m.toTileId )
    ...
    for (k in Seed.wilderness['city'+m.toCityId]){
      if (Seed.wilderness['city'+m.toCityId][k].tileId == m.toTileId){
        target += ' desde '+ Seed.wilderness['city'+m.toCityId][k].xCoord +','+ Seed.wilderness['city'+m.toCityId][k].yCoord;
        break; 
      }
    } 
  }  
  ...
  msg += 'Mi '+ target +' es '+ atkType  +' por '+ who +'. Tropas en camino (llegaran en '+ 
  ...
  if ( city.tileId == m.toTileId ){
    ...
    if (emb.count > 0){
      ...
      msg += ' Mi embajada tiene '+ availSlots +' plazas libres y está actualizada a nivel  '+ emb.maxLevel +'  :D.';
    }
  }
...
my.AllianceList = {
  ...
  show : function (){
    ...
    if (t.state == null){
      if (getMyAlliance()[0] == 0) {
        t.cont.innerHTML = '<BR><BR><CENTER>Tu necesitas estar en una alianza para usar esta opción.</center>';
        t.state = 1;
        return;
      }
  ...
  eventPlayerSubmit : function (){
    ...
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Búscando ...</center>';
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Jugadores encontrados que coinciden con <B>"'+ t.pName +'"</b></div>\
      ...
      <TABLE cellpadding=0><TR style="font-weight:bold"><TD class=xtab>Nombre</td><TD class=xtab width=20></td>\
      <TD align=right class=xtab>Poderio</td><TD class=xtab>Facebook &nbsp; </td><TD class=xtab>Detalles &nbsp; &nbsp; </td></tr>';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR valign=top><TD>'+ u.genderAndName +'</td><TD></td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><DIV onclick="PTpd(this, '+ u.userId +')"><A>detalles</a></div><DIV onclick="PTpd2(this, '+ u.userId +')"><A>sus ciudades</a></div></td></tr>';
    }
    m += '</table></div>';
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Mostrando alianza encontradas <B>"'+ t.aName +'"</b></div>\
...
//new CtableToText('tabAllMembers').toText();     
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Distancia desde <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' encotrados &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Jugador</div></a></td>\
        ...
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Estatus</a></div></td>\
  ...
  clickedPlayerDetail : function (div, uid){
    var t = my.AllianceList;
    div.innerHTML = "búscando detalles ..."; 
    t.pdDiv = div;
  ...
  gotPlayerDetail : function (rslt){
    ...
    var m = '<B>Alianza:</b> '+ a +' &nbsp; <B>Ciudades:</b> '+ u.cities +' &nbsp; <B>Población:</b> '+ u.population +'<BR><B>Provincias:</b> ';
    t.pdDiv.onclick = "";
  ...
  gotPlayerLBDetail : function (rslt){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<B>Alianza:</b> '+ a +' <B>Ciudades:</b><br> ';
    t.pdDiv.onclick = "";
	var cis = [];
...
my.Test = {
  ...
  show : function (){
    function xyNotify(city, x, y){
      var m = '[ Notific: '+ (city?city.name:'null') +', x='+ x +', y='+ y +' ]';
  ...
  show : function (){
    ...
    if (t.state == null){
      m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
          ...
          <DIV class=ptstat>VALORES DE TROPAS</div><TABLE align=center cellpadding=1 cellspacing=0>\
      ...
      m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';             
      m += '</table>';
      m += '<BR><DIV class=ptstat>CALCULADOR DE DISTANCIAS</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR><TD class=xtab align=right><B>Primera ubicacion: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> o elige ciudad: <SPAN id=ptloc1></span></td></tr>\
        <TR><TD class=xtab><B>Segunda ubicacion: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> o elige ciudad: <SPAN id=ptloc2></span></td></tr></table></div>\
...
my.Options = {
  ...
  show : function (){
    ...
    if (t.state != 1){
      m = '<BR><TABLE>\
        <TR><TD class=xtab colspan=2><B>CARACTERÍSTICAS:</b></td></tr>\
        <TR><TD class=xtab><INPUT id=togMsgCountFix type=checkbox /></td><TD class=xtab>Mostrar el número de nuevos mensajes separado del número de informes sobre el icono de mensajes.</td></tr>\
        <TR><TD class=xtab><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD class=xtab>Publicar automáticamente los ataques entrantes en el chat de la alianza.</td></tr>\
        <TR><TD class=xtab></td><TD class=xtab><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD class=xtab align=right>Mensaje Prefijado: &nbsp; </td><TD class=xtab><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD class=xtab align=right>Alertar si te exploran: &nbsp; </td><TD class=xtab><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD class=xtab align=right>Alertar si atacan tierras : &nbsp; </td><TD class=xtab><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD class=xtab align=right>Cuando vengan un minimo de: &nbsp; </td><TD class=xtab><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
        </table></td></tr>\
        <TR><TD class=xtab><INPUT id=togAllRpts type=checkbox /></td><TD class=xtab>Activar informes mejorados de la Alianza.</td></tr>\
        <TR><TD class=xtab><INPUT id=togAllowAlter type=checkbox /></td><TD class=xtab>Permitir que otros scripts cambien el formato de los informes de la alianza.</td></tr>\
        <TR><TD class=xtab><INPUT id=togEnhanceMsging type=checkbox /></td><TD class=xtab>Habilitar mensajería mejorada  (botones "reenviar" y "todos los oficiales").</td></tr>\
        <TR><TD class=xtab colspan=2><BR><BR><B>Corrección de Errores de Kingdoms of Camelot:</b></td></tr>\
        <TR><TD class=xtab><INPUT id=togTowerFix type=checkbox /></td><TD class=xtab>Fijar informe de la torre para mostrar el destino exacto (de la ciudad, tierra o no válido)</td></tr>\
        <TR><TD class=xtab><INPUT id=togMapDistFix type=checkbox /></td><TD class=xtab>Fijar el mapa para mostrar distancias de la ciudad seleccionada, en lugar de estar siempre en la primera ciudad.</td></tr>\
        <TR><TD class=xtab><INPUT id=togTowerFix2 type=checkbox /></td><TD class=xtab>Fijar falsas alertas de ataque creadas a partir de misiones propias de exploración.</td></tr>\
        </table><BR><BR><HR>Nota: Tenga en cuenta que si una casilla esta de color gris probablemente haya habido un cambio de código de KofC, lo que hace que  la opción deje de funcionar.';
      t.cont.innerHTML = m;
...
my.Search = {
  ...
  show : function (cont){
    ...
    if (t.state == null){
      this.cont.innerHTML = '\
        <DIV class=ptentry><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Búsqueda para: </td><TD>\
        <SELECT id="srcType">\
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X puede ser entre 0 y 749<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y puede ser entre 0 y 749<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>75)
      errMsg += "La Máxima Distancia debe ser entre 1 y 75<BR>";
    if (errMsg != ''){
    ...
    if (t.opt.searchType == 0)
      typeName = 'Campamentos Bárbaros';
    else
      typeName = 'Tierras Vírgenes';
      
    m = '<CENTER><B>Explorados: '+ typeName +'<BR>\
        ...
        <TR><TD class=xtab align=right>Min. nivel a mostrar:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
    ...
    if (t.opt.searchType == 1){
      ...
      m += htmlOptions ( {1:'Praderas/Lagos', 3:'Bosques', 4:'Colinas', 5:'Montañas', 6:'Planicies', 0:'Todos'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Solo sin Dueño:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
    ...
    document.getElementById ('statStatus').innerHTML = 'Examinando '+ xxx +','+ yyy;
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);    
  ...
  dispMapTable : function (){
    var tileNames = ['Campamento Barbaros', 'Pradera', 'Lago', 'Bosque', 'Colina', 'Montaña', 'Planicie' ];
    var t = my.Search;
    var coordsOnly = document.getElementById('coordsOnly').checked;
if (DEBUG_TRACE) DebugTimer.start();
    function mySort(a, b){
      if (Options.srcSortBy == 'Nivel'){
        if ((x = a[4] - b[4]) != 0)
          return x;
      }
      return a[2] - b[2];
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=srcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Posición</td></tr>';
      else
        m = '<TABLE id=srcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Posición</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nvl</td><TD width=80%> &nbsp; Tipo</td><TD style="padding-left: 10px;"></td></tr>';
      var numRows = dat.length;
      if (numRows > 500 && t.searchRunning){
        numRows = 500;        
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: La tabla solo muestra 500 de '+ dat.length +' resultados hasta que  la búsqueda se complete.</font>';        
      }  
      for (i=0; i<numRows; i++){
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById ('statStatus').innerHTML = 'Analizando '+ x +','+ y;
if (DEBUG_TRACE) logit (" 0 mapCallback()... calculando tiempo de espera ..:" + x +' , '+ y +' , '+ t.mapCallback.toString().substr (0,50));    
  ...
  show : function (){
    ...
    if (t.state == null){
      s = "<DIV id=trainTopSelect style='height: 148px;'>\
        <DIV class=ptstat>Altos Hornos!!!</div>\
        ...
        Poner los trabajadores a ralentí. &nbsp; (Le permite entrenar más soldados, si es posible, a costa de tener la  población inactiva en negativo temporalmente.)</td></tr>\
        <TR><TD class=xtab align=right>Ciudad:</td><TD class=xtab></td><TD class=xtab><SPAN id=ptspeedcity></span></td>";
      s += "</select></td>\
        </tr><TR><TD class=xtab align=right>Tipo de tropas:</td><TD class=xtab></td><TD class=xtab>\
        ...
        <TR><TD class=xtab align=right>  a entrenar:</td><TD class=xtab></td>\
        <TD class=xtab><INPUT id='trainNum' size=5 type='text' value='0'\>\
        &nbsp; <INPUT id='trainMax' type=submit value='máximo'\>\
        &nbsp; &nbsp; (max <span id=spanMaxTroops>0</span>)</td><TD class=xtab>\
        <INPUT id='trainDo' type=submit value='Entrenar Tropas'\>\
        </td></tr></table><BR></div>\
...
t.trainSpeed = trainSpeed;    // for debug trace
    if (t.stats.barracks < trainSpeed*2){
      document.getElementById ('trainStatus').innerHTML = "<FONT COLOR=#550000>Necesitarás por lo menos "+ (trainSpeed*2) +" cuarteles en esta ciudad para entrenar a "+ trainSpeed +"x de velocidad.</font>";
      return;
    }
    if (isNaN(numUnits) || numUnits<200 || numUnits>t.stats.MaxTrain){
      if (t.stats.MaxTrain < 200)
        document.getElementById ('trainStatus').innerHTML = "<FONT COLOR=#550000>No se pueden entrenar las unidades en este momento (max = "+ t.stats.MaxTrain +")</font>";
      else
        document.getElementById ('trainStatus').innerHTML = "<FONT COLOR=#550000>No puedes entrenar "+ numUnits +" unidades! &nbsp; (Elije un número entre 200 y "+ t.stats.MaxTrain +")</font>";
      return;
    }
    /*if (t.stats.queued > 0){
...
/**/
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE width=100%><TR align=center>\
      <TD class=xtab width=18%><B>Alimentos:</b></td>\
      <TD class=xtab width=16%><B>Madera:</b></td>\
      <TD class=xtab width=16%><B>Piedra:</b></td>\
      <TD class=xtab width=16%><B>Mineral:</b></td>\
      <TD class=xtab width=16%><B>Oro:</b></td>\
      <TD class=xtab width=16%><B>Pob. Inactiva:</b></td></tr>\
      ...
      <DIV style="overflow-y: auto; height:210px; max-height:210px"><table align=center><TR><TD class=xtab align=center colspan=4><B>Cola de entrenamiento &nbsp; ('+ t.stats.barracks +' cuarteles)</b><BR><HR></td></tr>';
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      t.displayCityStats();
      if (errMsg){
        if (errMsg == "La cancelación no se puede hacer en este momento"){
      ...
      cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<HR><B>ENTRENAMIENTO DE TROPAS TERMINADO!!!.</b> &nbsp; &nbsp; Ahora ya no hace milicianos ni hay que esperar a que las colas estén vacías.</b> &nbsp; &nbsp; <HR>');
        document.getElementById('trainDo').disabled = false;
        return;
      }
      if (cmd[0] == 'T'){
...
//      return;
    ...
    if (Options.includeMarching)
      str += '<TD width=85><B>En marcha</b></td>';
...
// TODO: food +/- ??
    ...
    rownum = 0;
    str += _row ('Colas', row, true);

    str += '<TR><TD></td><TD colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Incluir tropas y recursos en marcha</td></tr>';
    str += '<TR><TD></td><TD colspan=4>Incluir colas de entrenamiento (muy pronto!)</td></tr>';
    str += "</table>";
    dt = new Date ();
    dt.setTime (Seed.player.datejoinUnixTime * 1000);
    str += '<BR>Has estado jugando desde el '+ dt.toLocaleDateString() +'<BR>';
    str += 'Poderio: ' + addCommas(Seed.player.might) +'<BR>';
    str += 'Alianza: ' + getMyAlliance()[1] +' &nbsp; Dominio: '+ unsafeWindow.domainName +'<BR>';
...
my.Encamped = {
  ...
  show : function (cont){
    ...
    if (numSlots == 0){
      ...
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Jugador (nivca)</b></td>';
...
//logit (inspect (enc, 8,1, 2));
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Mostrar tropas de origen';
    s += '<BR><BR><DIV style="font-size: 10px">NOTA: Tendrás que volver a refrescar KoC para mostrar nuevos acampados o las tropas que quedan después de una batalla.</div>';
    t.cont.innerHTML = s;
    checkBox = document.getElementById('idCheck2');
...
function DialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  if (!rdiv){
    ...
    rdiv.innerHTML = '<DIV style="background: #cccccc"><CENTER>KOC Power Tools</div>\
      <CENTER><BR><FONT COLOR=#550000><B>Ha ocurrido un error:</b></font><BR><BR><DIV id=retryErrMsg></div>\
      <BR><BR><B>Reintentar automáticamente  en <SPAN id=retrySeconds></b></span> segundos ...<BR><BR><INPUT id=retryCancel type=submit value="CANCELAR Reintentar" \>';
