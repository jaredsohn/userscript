...
// ==UserScript==
// @name          hwmtakeoffon
// @homepage      http://hwm.xo4yxa.ru/js/takeoffonext/
// @version       0.14.4
    ...
    for( var i = 0; i < inputs.length; i++ )
    {
        ...
        if( inp.type == "checkbox" && inp.name == "dress_id" && inp.checked )
        {
            ...
            ({
                ...
                data: 'id='+art_id+'&nick='+urlencode($('trans_nick').value)+'&gold=1&wood=0&ore=0&mercury=0&sulphur=0&crystal=0&gem=0&sendtype=2&dtime=0.01&bcount=0&rep=on&art_id=&sign='+sign ,
                onload:function(res)
                {
                    inp.type = "radio" ;
                    inp.selected = true ;
