...
// @require        http://sizzlemctwizzle.com/updater.php?id=95835&days=4
// ==/UserScript==

var Version = '2.1.5.';
var Title = 'KOC PwrTools w whisp v2';
...
/***************
2.1.2. Added troop numbers to train tab
2.1.3. Added Keep map coordinate box/bookmarks on top of 
       troop activity (NEW) from latest powertools
2.1.4. added crest info to info tab
2.1.5. changed some of the food alert code
********/
...
var Options = {
  ...
  enableFoodWarn : true,
  foodWarnHours : 12,
  enableFoodChatWarn : true,    //mar25
  foodChatWarnHours : 4,          //mar25
  lastVersion : null,
  gmtClock : true,
  chatEnhance : true,
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    table.ptTabPad tr td.ptentry {background-color:#558FE6; padding-left: 8px;}\
    ...
    .ptentry {padding: 7px; border:1px solid; border-color:#000000; background-color:#558FE6; white-space:nowrap;}\
	...
	.ptChatWhisper {}\
    .ptChatAlliance {}\
    .ptChatGlobal {background-color: #FEF4E4}\
  ...
  logit ("* KOCPwrTools v"+ Version +" Loaded");
  setCities();
...
// TODO: Seed.wildDef is null if just aquired     
  ...
  e_inpTraps : function (e){
    ...
    if (isNaN(e.value) || e.value<0 || e.value>t.wildList[c][w][1]){
      e.value = '';
      e.style.backgroundColor = '#558FE6';
    } else
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
			...
			.xxtab_even{background-color:#CFDEF4; padding-left:5px; padding-right:5px;} </style>\
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    var t = Tabs.Search;
    document.getElementById ('statStatus').innerHTML = '<FONT color=#558FE6>'+ msg +'</font>';
...
Tabs.Overview = {
  ...
  show : function (){
	...
	//changed line3701 from background: #e8e8e8 pale beige to #ffc0cb pink
    function _row (name, row, noTotal){
      if (rownum++ % 2)
        style = '';
      else
        style = ' style = "background: #ffc0cb"';
      var tot = 0;
      var m = [];
    ...
    function CboxHandler (t){
      ...
      function eventChange (){
        ...
        if (isNaN(x) || x<0 || x>750){
          that.coordBoxX.style.backgroundColor = '#558FE6';
          return;
        }
        if (isNaN(y) || y<0 || y>750){
          that.coordBoxY.style.backgroundColor = '#558FE6';
          return;
        }
        that.coordBoxX.style.backgroundColor = '#558FE6';
        that.coordBoxY.style.backgroundColor = '#558FE6';
...
var WhisperAllianceForStringDefaultStatus = true;
...
function SlideControlPanelInTimer(){
    slideInTimer = window.setInterval(slideControlPanelIn, 3);
}
