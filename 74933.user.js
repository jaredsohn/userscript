...
// @author         Stasik0
// @namespace      Nothing
// @description    Replaces some broken search features by google custom search
// @include        http://dirty.ru/*
// @include        http://*.dirty.ru/*
// ==/UserScript==
	var comments_id = "001646679617833491186:ewuutccd_ie";
	var posts_comments_id = "001646679617833491186:lyu-opfxtms";
	var posts_id = "001646679617833491186:xlv3at4lv1g";
	
	function build_url(id, query){
		return "http://www.google.com/cse?cx="+id+"&ie=UTF-8&q="+query;
	}
	...
	if(exp.test(document.location.href)){
		...
		for(var i=0;i<links.length;i++){
			...
			if(exp.test(links[i].href)){
				...
				links[i].href=build_url(comments_id, "\"Написала "+match[1]+"\" OR \"Написал "+match[1]+"\"");
			}			
		}
	}
