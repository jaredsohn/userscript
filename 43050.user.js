...
// @original by   Pierre Andrews (mortimer.pa@free.fr) http://6v8.gamboni.org/IMG/js/flickrrefercomment.user.js
// @history        [2009-02-25] supported Opera. 1.8 forked from original version 1.8 (2008-12-16)
				...
				if(referrer.indexOf(document.location.pathname) >= 0) return //no need to add the comment if you just commented...
					...
					if(referrer.indexOf('/discuss/') >= 0 && referrer.indexOf('/groups/') >= 0) {
						...
						if(matches[2] == '/map') {
							...
							var restr = "/photos/("+matches[1]+"|"+w.nextprev_currentContextID.replace('stream','')+")/";
	...
	// Modifications by: Marcos Kuhns (http://www.kuhnsfam.com/)
			  ...
			  //Fix by Marco Bernardini
					...
					if(matches[2] == '/map') {
						...
						var re = new RegExp("/photos/("+matches[1]+"|"+w.nextprev_currentContextID.replace('stream','')+")/");
		...
		resolveUserName: function(type,userurl,referrer) {
			...
			w.F.API.callMethod('flickr.urls.lookupUser', {
					url:userurl
						}, listener);
		},
		...
		resolveGroupName: function(type,groupurl,referrer) {
			...
			switch(type) {
				case RANDOM:
					w.F.API.callMethod('flickr.groups.getInfo', {
							group_id:groupurl
														  }, listener);
					break;
				case DISCUSS:
				...
				case FELLOW:
				case PHOTOPHLOW:
					w.F.API.callMethod('flickr.urls.lookupGroup', {
							url:groupurl
														  }, listener);
			}
		...
		resolveSetName: function(id,referrer,type) {
			...
			w.F.API.callMethod('flickr.photosets.getInfo', {
				photoset_id:id
			}, listener);
