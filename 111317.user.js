...
// ==UserScript==
// @name           Obmen resursov skor
// @author         VelMizar
// @date           19-05-2011
// @namespace      resursy
// @description    Probnyy variant botta
// @include        http://speed.lordgalaxy.ru//game.php?page=trader*
...
}else{window.open(posylannya[8][0],'_self');};	
}
...
'<div id="dell0" st; padding-top:0px; width:222px; text-align:center;">Menyu botta:</h3>'+
  '</div></div>'+
...
function addEvt(a,b,i){  
  if(a.addEventListener){a.
    ...
    window.open(posylannya_speed[k][0],'_self');
	};
}


function activButtonDalhe(){ 
		var forma=document.getElementById("trader");
		var infoTagInput = document.getElementsByTagName('input');
if (forma == null) {                                                         
for (i = 0; i < infoTagInput.length; i++) {
			//Знаходимо і натискаэмо кнопку "Вызвать скупщика"
