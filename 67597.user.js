// ==UserScript==
// @name			Accept All Gifts1
// @namespace		        Facebook gifts
// @description		        Aceppt all gifts
...
unsafeWindow._getCountryLifeGift = safeWrap(_getCountryLifeGift);
...
_appSettings = {
	...
	'app_26947445683_0':{
		'name':'Country Life',
		'functionName': 'unsafeWindow._getCountryLifeGift(\'app_26947445683_0\')'
	},
	'app_102452128776_1':{
		'name':'Farmville',
...
function _getCountryLifeGift(_str){ //done
	var all		=	count(_str);
	var temp	=	0;
	if ($("#giftCountryLifeMain").html() == null) $("#imgList").append("<div id='giftCountryLifeMain'>Accepting Country Life request...<div id='giftCountryLifeBody' style='padding-left:10px'></div>Accepted <span id='giftCountryLifeSum'>0</span> gift and/or <span id='neibCountryLifeSum'>0</span> neighbor.</div>");
	for (var i in _request){
		if(_request[i].appName == _str) {
			...
			GM_xmlhttpRequest({ url: _link, method:'get', data: i, 
				onload: function(resp){
					...
					if(_isGift == 1){
						if($('.giftConfirm_name > span',resp.responseText).html()) 
							$("#giftCountryLifeBody").append("<div> Accept: <b>" + $('.giftConfirm_name > span',resp.responseText).html() + "</b></div>");
							$("#giftCountryLifeSum").html(parseInt($("#giftCountryLifeSum").html(),10)+1);
							_accept(this.data);
					}
					else{
						$("#giftCountryLifeBody").append("<div>Added: <b>"+_request[this.data].userName+"</b> to neighbor.</div>");
						$("#neibCountryLifeSum").html(parseInt($("#neibCountryLifeSum").html(),10)+1);
						_accept(this.data);
					}
					if(temp == all){
						$('[appBlock="'+_str+'"]').hide();
