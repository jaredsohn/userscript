// ==UserScript==
// @name           Travian Ultimate toolkit
// @description    Travian Ultimate toolkit
// @author         Ph34k H4d35
// @version        1.0.1
// @license        GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html

// @include     http://*.travian.*
// @include     http://t*travian*/dorf1*
// @include     http://t*travian*/dorf2*
// @include	    http://t*travian*/build.php?newdid*
...
[84935,45735,71870,58800,115,71]//20
	...
	var map1Element;
		...
		if (document.getElementsByName('clickareas')[0]) {
		  map1Element = document.getElementsByName('clickareas')[0];
			countArray = 22;
			top = 10;
			left = 15;
			dtop = 10;
	...
	if (document.getElementsByName('rx')[0]) {
    ...
    var container = document.getElementById('village_map');	
      ...
      BuildingLevel = -1;
			...
			if (BuildingLevel){
			  ...
			  if (dorf == 2) {
					...
					switch(imgId) {
						case 21:
							gid = parseInt(/(\d+)/.exec(imageElements[imgId].getAttribute('class')));
							break;
						case 22:
							gid = 0;
							break;
				...
				if (dorf == 1){
          if (areaElements[i].alt.indexOf('Woodcutter')!= -1){
              gid = 1;
          }else if (areaElements[i].alt.indexOf('Clay')!= -1){
              gid = 2;
          }else if (areaElements[i].alt.indexOf('Iron')!= -1){
              gid = 3;
          }else if (areaElements[i].alt.indexOf('Cropland')!= -1){
              gid = 4;
          }
          
          if (areaElements[i].alt.indexOf('Bosco')!= -1){
              gid = 1;
          }else if (areaElements[i].alt.indexOf('argilla')!= -1){
              gid = 2;
          }else if (areaElements[i].alt.indexOf('ferro')!= -1){
              gid = 3;
          }else if (areaElements[i].alt.indexOf('grano')!= -1){
              gid = 4;
          }
        ...
        if (dorf == 2){
          smallDIV = addDiv('TMbuildingtag' + i,'TMbuildingtags',BuildingLevel[0],'village_map',BuildingURL);
				...
				smallDIV.style.left = parseInt(coords[0]) + left +'px';
				smallDIV.style.visibility = "visible";
				

				var resneed = bCost[gid][parseInt(BuildingLevel)+1];
        
        var reswood = parseInt(document.getElementById('l1').firstChild.nodeValue.split('/')[0]);
        var resclay = parseInt(document.getElementById('l2').firstChild.nodeValue.split('/')[0]);
        var resiron = parseInt(document.getElementById('l3').firstChild.nodeValue.split('/')[0]);
        var rescrop = parseInt(document.getElementById('l4').firstChild.nodeValue.split('/')[0]);

				if (BuildingLevel[0] == getMaxLevel(parseInt(gid))) {
					smallDIV.style.backgroundColor = '#80F080';
				}else if( parseInt(reswood+resclay+resiron+rescrop) >= parseInt(resneed[0]+resneed[1]+resneed[2]+resneed[3]) ){
				  
				  if(reswood >= resneed[0] && resclay >= resneed[1] && resiron >= resneed[2] && rescrop >= resneed[3]){
            smallDIV.style.backgroundColor = '#FDF8C1';
          }else{
            smallDIV.style.backgroundColor = 'orange';
          }
				  ...
				  smallDIV.style.backgroundColor = '#FF5050';
        }
...
notRunYet = false;
var namespace = 'http://userscripts.org/scripts/show/100278';
...
var langs = ['auto','English (en)','عربي (ar)','Български (bg)','Bosanski (bs)','Deutsch (de)','فارسی (fa)','Français (fr)','Hrvatski (hr)','Magyar (hu)','Italiano (it)','Polski (pl)','Português (pt)','Română (ro)','Русский (ru)','Српски (sr)','Svenska (sv)','Slovakia (sk)','Українська (ua)'];
var allCookies = ['vPPH','mf','next','Dorf1','Dorf2','Dorf11','Dorf12','Dorf13','RBSetup','xy','VV','OV','Mem','Dict','DictFL','DictTR','ln','ln2','src','vHint','tropsI','tropsDic','vList','Att','trFL','AC','AS','bodyH'];
...
DICT = {
	...
	sk: { // Slovak translate Ph34k H4d35
		// ingame messages
		ok : "Ok",
		cancel : "Zrušiť",
		close : "Zavrieť",
		...
		consnegat : "Produkcia obilia v dedine je mínusová. Na koľko minút sú rezervy?",
		// settings
		none : "Žiadne",
		auto : "auto",
		info : "Informacie",
		yourrace : "Tvoj národ",
		speedart : "Rýchlosť artefaktu",
		racelist : ['Ríman','Germán','Gál'],
		cranny : "úroveň úkrytu (percent)",
		crannyh : 'normal 80, 70 pre classic alebo lúpež artefaktu',
		builtin : "Vstavané nástroje",
		builtinh : 'k normalizácii produkcie: červená-najviac potrebné zdroje, zelena-menej potrebné zdroje',
		normalize : 'k normalizácii produkcie',
		normal : "normalne",
		banalyze : "vstavaný analyzér útokov",
		cropfind : "vstavaný vyhľadávač MC",
		adetect : "odhaliť útok",
		adetecto : ['off','w/o freeze','on'],
		adetecth : "je skrytý pred požiadavkami serveru.porušenie pravidiel môže mať za následok trest.",
		adetectt : "interval odhalenia útoku",
		onallp : "Všetky stránky",
		dontwrap : "Neskryť stránku",
		dontwraph : 'Ak zoznam dedín skočí nadol',
		buildand : "Zobraziť odpočítavanie stavieb a útokov",
		buildandh : 'V zozname dedín',
		sendres : "Ukázať &laquo;poslať zdroje/jednotky&raquo; ikon",
		sendmess : "Zobraziť &laquo;paslať správu&raquo; ikony",
		analyzer : "Analyzér sveta",
		bigicon : "Zobraziť VELKÉ ikony",
		addvtable : "Zobraziť ďalšie tabuľky dediny",
		addvtableh : 'pre travian4',
		paranoid : "Paranoidný režim",
		paranoidh : 'Zatajiť prácu, ak zistí akekoľvek zmeny v stiahnutých scriptov zo serveru Travian.',
		opennote : "Automaticky otvoriť záznamy",
		resbar : "Zdroje surovín",
		showres : "Ukázať zdroje surovín v okne",
		redbl : "červená (za hodinu)",
		yellowbl: "žltá (za hodinu)",
		marketpl : "Trhovisko",
		mfilter : "filter",
		mfiltero : ['vypnúť','len kurz','len filter','úplný'],
		npcsum : "zhrnutie pre NPC",
		npcsumh : 'na trhu a budov',
		bidinc : "prihodenia",
		bidinch : 'na aukcie travian4',
		rpandmp : "Zhromaždisko a Trhovisko",
		incomres : "prichodzí zdroj informácií",
		incomreso : ['vypnúť','zapnúť','zo súhrnnými','WW režim'],
		troopsI : "Informácie o vojakoch",
		troopsIo : ['vypnúť','zapnúť','prejsť nápovedu'],
		defRP : "predvolené akcie pre zhromaždisko",
		showls : "Zobraziť odkazy",
		showlso : ['vypnúť','zapnúť','v okne'],
		savedls : "uložiť odkazy",
		savedd : "uložených dát",
		saveddh : 'vrátane odkazov a druhého názvu. Ak bol účet zmazaný, alebo to nie je Váš počítač',
		savedelall : "vymazať všetky uložené dáta",
		savedelallh : 'Naozaj chcete vymazať všetky uložené dáta, vrátane odkazov a druhého názvu?',
		scrlang : "Jazyk scriptu",
		youlang : "váš jazyk",
		notifi : "oznámenia",
		notification : "oznámenie po dokončení stavby",
		method : "spôsob",
		audiourl : "URL zvukových súborov",
...
function getVidFromCoords ( txt ) {
	var xy = txt.match(/\((-?\d{1,3})\D+?(-?\d{1,3})\)/);
...
acss =	"table#"+allIDs[0]+" {width:100%; border-collapse:collapse; font-size:8pt; text-align:center; background-color:white; padding:2px; margin:1px;}" +
	...
	"."+allIDs[4]+" {position:absolute;border:1px solid silver;text-align:center;background-color:yellow;}" +
...
if( ver4FL ) acss += "table#vlist td{padding:0;line-height:14px;text-align:"+docDir[0]+";white-space:nowrap;}table#vlist thead td{background-color:#FFF;height:22px;text-align:center;padding:0 3px;}" +
	...
	"div#build.gid17 table.send_res td {padding:2px 3px;}";// .textWithCoords span.text {float: none;} .textWithCoords span.coords {float: none;}";
...
function sendResTropAdd ( aLink, aType ) {
	...
	if( sp.length > 0 ) {
//		$at(sp[0],[['style','float:none;']]);
//	var sp = $gc('textWithCoords',aLink);
//	if( sp.length > 0 ) {
//		var cl = $gc('clear',sp[0]);
//		if( cl.length > 0 ) sp[0].insertBefore(addARLinks(vId, aType),cl[0]);
//			else sp[0].appendChild(addARLinks(vId, aType));
	} else {
...
function vlist_addButtonsT4() {
	...
	hideP.addEventListener('click', function (x) { return function() { bodyHide(x); }}([vilB,1]), false);
...
function progressbar_init() {
	...
	var cell = $em('TH',[alink,alink2,aImg2,aImg,alink3],[['class','rb_head']]);
...
function showLinks () {
	...
	function removeLink ( num ) {
		if( ! confirm(gtext("linkdel")+': '+(alinks[num].split("\/@_")[1])+' ?') ) return;
		alinks.splice(num, 1);
		saveLink( alinks );
	}
...
function addlinkT4() {
	...
	if( vilView ) {
		...
		if( h1 ) {
			var spn = $gt('SPAN',h1);
			var xy = getVidFromCoords(spn[2].innerHTML);
			var ln_cookie = RB_getValue(GMcookieID + "ln", "");
			var alinks = ln_cookie.split("@@_");
			newLink = 'd='+xy;
...
function detectNameAttaker() {
	...
	function nameAttaker( num ) {
		...
		if( ! tname ){
			// ver4FL /position_details.php?x=xxx&y=yyy
...
function viewMessageIW() {
	...
	for( var i = 0; i < allRows.snapshotLength; i++ ) {
		...
		if( ver4FL && /nachrichten/.test(crtPath) )
			$at(newImg,[['style','position:relative;float:'+docDir[0]+';'+docDir[0]+':-18px;']]);
	...
	if( vilView ) {
		...
		if( tipE.length > 0 ) {
			...
			if( tipC > 0 ) {
				...
				$at(dTTK, [['style','background-color:white;']]); // #FFFFE0;
				windowID[2] = makeFloat( dTTK, (ltr?contRight+5:contRight-185), contTop-topRS[RB.Setup[2]]);
			}
		}
	}
...
function returnQuickHelp () {
	...
	if( qh ) $at($gt('A',qh)[0],[['href','#'],['onclick','return Travian.iPopup(0,0);']]);
	else $g('mid').parentNode.appendChild($ee('div', $e('A',[['href','#'],['target','_blank'],['onclick','return Travian.iPopup(0,0);']]), [['id',"anwersQuestionMark"]]));
}
