...
function get_fnr() {
  ...
  var fnr = prompt("Hvilket fÃ¸dselsnummer vil du fylle inn?", fnr);
  GM_setValue('fnr', fnr);
  set_fnr(fnr);
}

function set_pin(pin) {
  var box = document.getElementById('ctl00_MainContentPlaceHolder_boxLogin_txtPassword');
  if (box && pin)
    box.value = pin;
}
function get_pin() {
  var pin = GM_getValue('pin', '');
  pin = prompt("Hvilken passord vil du fylle inn?", pin);
  GM_setValue('pin', pin);
  set_pin(pin);
}

GM_registerMenuCommand('Sett fÃ¸dseslsnummer', get_fnr);
GM_registerMenuCommand('Sett passord', get_pin);
