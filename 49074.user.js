// ==UserScript==
// @name           Quick2
// @version	   2.0.5
// @description	   Adds quick links to bungie.net + New Games Pages
// @Author	   K1RK
// @contributor    PKF 647 - Games Links
// @namespace      http://www.bungie.net/
// @include        http://www.bungie.net/*
// @include        http://*.bungie.net/*
// @include        http://*.bungie.net/Games/*
// ==/UserScript==
var forums, newElement;
forums = document.getElementById('aspnetForm');
if (forums) {
    newElement = document.createElement('div');
    newElement.innerHTML='<div style="margin: 0 auto 0 auto;' +
    'border-bottom: 0px solid #bbbbbb; margin-bottom: 0px;margin-left: 43px;margin-right:53px;' +
    'font-size: small; background-color: #242223; ' +
    'color: #ffffff;"><p style="margin: 0px 0 0px 0;"> ' +
    '<a href="http://www.bungie.net/Forums/topics.aspx?forumID=105242" style="color:white">Halo 3</a> |  <a href="http://www.bungie.net/Forums/default.aspx?mode=halo3" style="color:white">Halo 3 Files</a>  |  <a href="http://www.bungie.net/Forums/topics.aspx?forumID=5576" style="color:white">Optimatch</a>  |  <a href ="http://www.bungie.net/Forums/topics.aspx?forumID=1" style="color:white">Underground</a>  |  <a href ="http://www.bungie.net/Forums/topics.aspx?forumID=3" style="color:white">Septagon</a>  |  <a href="http://www.bungie.net/Forums/topics.aspx?forumID=10" style="color:white">Flood</a>  |  <a href="http://www.bungie.net/Forums/default.aspx?mode=other" style="color:white">All the rest</a>  |  <a href="http://www.bungie.net/Games/" style="color:#07a5d9">Games Page</a>  |  <a href="http://www.bungie.net/Games/MP/" style="color:#07a5d9">Multiplayer Games</a>' +
    '</p></div>';
    newElement.style.textAlign="center";
    forums.parentNode.insertBefore(newElement, forums);
}