...
'</div>'
	...
	var scriptUploadGambar = function uploadGambar()
	{
		...
		$.ajaxFileUpload(
		{
		url:'http://kaskus.co.id/misc/upload_image',
		secureuri:false,
	...
	function Mainkan(){
		...
		$("#postreply").submit(function(){
			...
			$("#ajaxStat").show();
			datum = "message="+minimalChars+"&securitytoken="+jimat+"&recaptcha_challenge_field="+celeng+"&recaptcha_response_field="+respong;
			GM_log("data:"+datum);
			GM_xmlhttpRequest({
  				method: "POST",
  				url: postURL,
