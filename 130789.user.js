...
//
// @history        0.5.1 09/12/2013 new gameforge servers
// @history        0.5   08/12/2013 Fix native combat reports with decimal scores. Fix adding emoticons. Suggestions for new emoticons.
// @history        0.4.1 08/12/2013 also works in archived messages.
// @history        0.4   08/10/2013 also works in outbox messages.
// @history        0.3.1 07/24/2013 better island coordinates detection in messages.
// @history        0.3   07/24/2013 languages fixed, portuguese added. Removed "Reply all button" since ikariam v0.5.6 has one. GM/jQuery upgrades.
// @history        0.2.9 06/24/2012 jQuery is now used in noConflict mode, to allow the use of several scripts in Chrome/Tampermonkey
...
GM_addStyle(".emotiworldIcon { float:left; line-height:1.5em } \
	...
	#emotiworldSuggestions { margin-top: 1.5em; } \
	.emotiworldList { margin: 0; list-style: none; } \
	.emotiworldList li { display: inline-block; padding: 0 0.5em; cursor: default; } \
	.emotiworldList li:hover { background-color:rgba(0,0,0,0.15) } \
...
var Emotiworld = new function() {
	...
	this.languageData = {
		'en':{
			...
			add: 'Add',
			pasteHere: "Paste here the emoticon to add :",
		...
		'fr':{
			...
			add: 'Ajouter',
			pasteHere: "Coller ici l'émoticône à ajouter :",
		...
		'de':{
			...
			add: 'Hinzufügen',
			pasteHere: "Einfügen Sie hier das Emoticon hinzufügen :",
		...
		'es':{
			...
			add: 'Añadir',
			pasteHere: "Pegue aquí el emoticono para agregar :",
			findIcons: 'Buscar otros emoticonos en [link]',
		...
		'it':{
			...
			add: 'Aggiungere',
			pasteHere: "Incollare qui l'emoticon per aggiungere :",
			findIcons: 'Trova nuove emoticon su [link]',
		...
		'pt': {
			manage: 'Gerenciar seus emoticons',
			add: 'Adicionar',
			pasteHere: "Cole aqui o emoticon para adicionar :",
			findIcons: 'Encontrar novos emoticons em [link]',
			suggestions: 'Sugestões:',
			noSuggestions: 'Sem sugestões.',
			close: 'Fechar',
			CRUndoTitle: 'Emotiworld encontrou um relatório de combate. Clique aqui para voltar a mensagem básica.'
		},
		'ro':{
			manage:'Administreaza Emoticons',
			add:'Adăuga',
			pasteHere: "Lipiți aici emoticon pentru a adăuga :",
			findIcons:'Găsiţi alte emoticoane pe [link]',
			suggestions: 'Sugestii:',
			noSuggestions: 'Nu sugestii.',
			close:'Aproape',
		...
		'ru':{
			...
			add:'Добавить',
			pasteHere: "Скопируйте сюда, чтобы добавить смайлик :",
	...
	this.ikariamInit = function() {
		...
		unsafeWindow.ajax.Responder.changeView = function(params) {
			...
			if((id == 'sendIKMessage') || (id == 'diplomacyIslandBoard')) {
				...
				setTimeout(function(){
					...
					if(!Emotiworld.configPanelPresent) {
						Emotiworld.$('body').append( Emotiworld.configPanelHTML() );
						Emotiworld.configPanelPresent = true;
					}
					Emotiworld.textarea = Emotiworld.$('#js_msgTextConfirm, #js_islandMsgTextInput');
			...
			unsafeWindow.ajax.Responder.Emotiworld_IkaChangeView(params);
			if(id === 'diplomacyAdvisor' || id === 'diplomacyAdvisorOutBox' || id === 'diplomacyAdvisorArchive' || id === 'diplomacyAdvisorArchiveOutBox') {
				var diplo = Emotiworld.$('#tab_diplomacyAdvisor');
	...
	this.configPanelHTML = function() {
		...
		html += '<div id="emotiworldPanelRight"><h4>'+lang['manage']+'</h4><br/>';
		html += '<a id="emotiworldAdd" href="" onclick="return false;">'+lang['add']+'</a><br/><br/>';
		html += lang['findIcons'].replace(/\[link\]/, '<a href="http://emotiworld.com" target="_blank" style="font-weight:bold">emotiworld.com</a>')+' ㋡</div>';
		html += '<div id="emotiworldSuggestions">'+Emotiworld.suggestions()+'</div>';
	...
	/** Detects Combat Reports from Ikariam CR Converter & ikariam native CR converter (v0.5.7) */
	this.detectCR = function(diplo) {
		Emotiworld.$('td.msgText', diplo).each(function(index) {
			var content = Emotiworld.$(this).html();
			content = content.replace(/-{67,}\s*<br\s?\/?>\s*([\s\S]+?)-{67,}\s*<br\s?\/?>([\s\S]+?)-{67,}/g, Emotiworld.CRBlock);
			Emotiworld.$(this).html(content);
		});
		Emotiworld.$('.emotiworldUndo', diplo).click(function() {
			Emotiworld.$(this).siblings().toggle();
		});
	}
	...
	this.localeString = function(number) {
		if(!number) return '';
		return number.replace(/[,\d.]+/, function(n){ return parseFloat(n.replace(/,/, '')).toLocaleString() } );
	}

	/** Detect Island coordinates */
	this.addLinks = function(diplo) {
		Emotiworld.$('tr:not(.globalmessage) td.msgText', diplo).each(function(index) {
			var content = Emotiworld.$(this).html();
			content = content
				.replace(/\[(\d+):(\d+)]/g, Emotiworld.islandLink)
				.replace(/(\d+):(\d+)/g, Emotiworld.islandLink)
	...
	this.islandLink = function(str, x, y) {
		// '20:30' is interpreted as a time, use [20:30] if you mean the island.
		// but '50:50' is obviously an island, not a time.
		if(str.charAt(0) !== '[' && parseInt(x) < 24 && parseInt(y) < 60) return str;
	...
	this.postInit = function() {
		Emotiworld.$('#emotiworldIcons').delegate('.emotiworldIcon', 'click', Emotiworld.insertIcon);
		var bgcolor = Emotiworld.textarea.css('background-color');
		Emotiworld.$('#emotiworldConfig').click(function() {
			if(Emotiworld.$('#emotiworldPanel').css('display') == 'none') {
				if(Emotiworld.overlay) {
					var docHeight = Emotiworld.$(document).height();
					Emotiworld.$("#emotiworldOverlay").css({opacity:0,display:'block'}).animate({opacity:0.4});
				}
				Emotiworld.$('#emotiworldPanel')
					.width(Emotiworld.textarea.width())
					.css('top', Emotiworld.$('#emotiworldConfig').offset().top + 28)
					.css('left', Emotiworld.textarea.offset().left);
				Emotiworld.$('#emotiworldPanel').slideDown('fast');
			}
			else Emotiworld.hideConfigPanel();
		});
		Emotiworld.$('#emotiworldAdd').click(function() {
			var lang = Emotiworld.languageData[Emotiworld.lang];
			var pasteHere = lang.pasteHere;
			var icon = Emotiworld.$.trim( prompt(pasteHere) );
			if(icon) Emotiworld.addIcon(icon);
		});
		Emotiworld.$('#emotiworldClose').click( Emotiworld.hideConfigPanel );
		Emotiworld.$( "#emotiworldTrash" ).sortable({
			tolerance: 'pointer',
			containment: '#emotiworldPanel',
			connectWith:Emotiworld.$('#emotiworldSortable')
		});
		Emotiworld.$( "#emotiworldSortable" ).sortable({
			tolerance: 'pointer',
			containment: '#emotiworldPanel',
			connectWith:Emotiworld.$('#emotiworldTrash')
		});
		Emotiworld.$( ".emotiworldList" ).delegate('li', 'click', function() {
			var $this = Emotiworld.$(this);
			Emotiworld.addIcon($this.text());
			$this.remove();
		});
		Emotiworld.$('#emotiworldPanel').fixedHeight();
//		Emotiworld.$( "#emotiworldSortable" ).disableSelection();	// ne marche pas
	};

	this.hideConfigPanel = function() {
		Emotiworld.saveIcons();
		Emotiworld.$('#emotiworldPanel').slideUp('fast');
		if(Emotiworld.overlay)
			Emotiworld.$("#emotiworldOverlay").fadeOut('fast');
	}

	/** Suggère d'ajouter les emoticones par défaut qui ne sont pas déjà présentes. */
	this.suggestions = function() {
		var lang = Emotiworld.languageData[Emotiworld.lang];
		var current = Emotiworld.icons.join(' '), diff = [];
		for(var i = 0; i < Emotiworld.defaultIcons.length; i++) {
			if(current.indexOf(Emotiworld.defaultIcons[i]) < 0) {
				diff[diff.length]  = Emotiworld.defaultIcons[i];
			}
		}
		if(diff.length > 0) {
			return lang.suggestions + '<ul class="emotiworldList"><li>'+diff.join('</li><li>')+'</ul>';
		}
		else return lang.noSuggestions;
	}

	/** Insertion de l'émoticône cliquée dans le message. */
	this.insertIcon = function() {
		Emotiworld.textarea.insertAtCaret( Emotiworld.$(this).text() );
	};

	/** Renvoie la liste d'émoticônes affichée dans le panneau de config. */
	this.getCurrentList = function() {
		var icons = [];
		Emotiworld.$('#emotiworldSortable li').each(function() {
			icons.push(Emotiworld.$(this).text());
		});
		return icons;
	};

	/** Enregistre la nouvelle liste d'émoticônes. Méthode appelée à la fermeture du panneau de config.	 */
	this.saveIcons = function() {
		Emotiworld.icons = Emotiworld.getCurrentList();
		GM_setValue("emoticons", JSON.stringify(Emotiworld.icons));
		Emotiworld.$('#emotiworldIcons').html(Emotiworld.toolbar(Emotiworld.icons, false));
	};
	...
	this.addIcon = function(icon) {
		...
		newIcons.push(icon);
		Emotiworld.$('#emotiworldSortable').css('height', 'auto');
		Emotiworld.$('#emotiworldPanel').css('height', 'auto');
		Emotiworld.$('#emotiworldSortable').append('<li>'+icon+'</li>');
		Emotiworld.$('#emotiworldPanel').fixedHeight();
		Emotiworld.$('#emotiworldSortable').fixedHeight();
	};
	...
	this.defaultIcons = ['ツ', '(•‿•)', '( ͡• ͜ʖ ͡• )', '(˘◡˘)', '◤(¬‿¬)◥', '┌∩┐(◣_◢)┌∩┐', '(´ᗣ｀)', '(º_•)', '⊙_⊙', '(╯°□°）╯︵ ┻━┻', 'ƪ(ړײ)‎ƪ​​'];
}

Emotiworld.$ = jQuery.noConflict(true);
Emotiworld.$(function() {
	// Icons
	Emotiworld.icons = JSON.parse( GM_getValue("emoticons", '[]') );
	if(!Emotiworld.icons || !Emotiworld.icons.length)
		Emotiworld.icons = Emotiworld.defaultIcons;
