...
var Collection = function(array_object) {
    ...
    var Items = new Array();
    var Keys = new Array();
    var move_callback, end_callback;
    ...
    this.MoveNext = function() {
        if ( Util.isSet(Items[currentPos+1]) ) {
            currentPos++;
            MoveEvent();
            return currentItem();
        }
    ...
    for (var i in array_object) {
        Keys.push(i);
        Items.push(array_object[i]);
    }
    log$('Collection loaded: '+Items.length);
    return this;
};
...
var defaults = {
    ...
    'feedstream': {
        ...
        rewardFilter               : false,
        rewardFilterText           : '',
        socialHelpFilter           : false,
        socialHelpFilterText       : '',
        propertyHelpFilter         : false,
        propertyHelpFilterText     : '',
...
var Util = {
    ...
    parseParam: function(text) {
        return $.parseJSON( String(text).replace('+',' ') );
    },
    /**
     * @param {Number} num
     * @param {Number} digits
     ...
     uSplit: function(url) {
        ...
        try {
            if(!this.isString(url)) {
                throw ReferenceError('url argument must be a string');
            }
            url = String( url.split('?')[1] ).replace(/&amp;/g, '&');
...
function loadingOverlay(message) {
    ...
    c$('div', 'id:overlay_pop_bg,class:pop_bg').appendTo(divElt).show();
    c$('div', 'id:overlay_pop_box,class:socialMissionTryAgain')
...
function showGroupSelection(defaultGroup, callback, defaultMessage) 
{
    var Events = {
        select_click: function() {
            ...
            setTimeout(function() {
                popupElt.close();
                callback($('#op_groups option[selected]', popupElt.content).val(), 
    ...
    function updateGroups() {
        var selectElt = $('#op_groups', popupElt.content).empty();
        $.each(global.options.get('groups'), function(id, name) {
            selectElt.append(c$('option', 'value:'+id).text(name));
        });
        $('option[value='+defaultGroup+']', selectElt).attr('selected','selected');
        return false;
    }
    ...
    .append(c$('select', 'id:op_groups,class:black').css('margin-right',5).width(400))
    .append(c$('a', 'href:#').text('Refresh').click(Events.refresh_click));
    
    b$('Select Target', 'class:medium white').appendTo(popupElt.content).click(Events.select_click);
...
function getUnshortUrl(shortURL, success, error) {
    ...
    try {
        ...
        httpXDRequest({
            method: 'GET',
            url: 'http://api.unshort.me/?t=json&r=' + escape(shortURL),
            onload: function(responseDetails) {
                try {
                    var jsonData = $.parseJSON(responseDetails.responseText);
                    if (jsonData.success) {
                        success(jsonData.resolvedURL);
                    }
                    else {
                        error && error('Server response error.');   
                    }
                }
                catch(err) {
                    errorHandler(err);
            ...
            onerror: function(responseDetails) {
                errorHandler(Error(responseDetails.statusText));
            }
        });
    }
    catch(err) {
...
var facebook = {
    ...
    // PUBLISHING METHODS
    streamPublish: function(options, callback, bForceDialog, bSilectMode) {
        ...
        this.needAppPermission('publish_stream', function(success) {
            ...
            if (bSilectMode !== true && (bForceDialog === true || global.options.get('publishPreview') === true)) {
                 MWFB.ui({
...
function modMenu()
{
    ...
    var menuOptions = [
        ...
        //{name:'News Helper',              'click': NewsHelper,           'timer':'newsHelper'       },
        {name:'Collect All Cities',       'click': CollectAllCities,     'timer':'collectAllCities' },
        {name:'Battlefield!',             'click': Battlefield                                      },
...
function PageStats() {
    if (e$('#addtowhitelistbutton') !== null) {
        return false; 
    }
    var eMember = e$('a:regex(href,xw_controller=group&xw_action=remove),.tutorial_text');
    var eCenter = c$('center', 'addtowhitelistbutton').insertAfter('.stats_title:first');
    
    if (eMember == null) {
        b$('Add this user to WhiteList!', 'class:medium green').appendTo(eCenter).css('margin', 5)
        .click(function() { Battlefield($('#inner_page'), 'whiteList'); }); 
        
        b$('Add this user to BlackList!', 'class:medium red').appendTo(eCenter).css('margin', 5)
        .click(function() { Battlefield($('#inner_page'), 'blackList'); }); 
    }
}
...
function PageGift() 
{
    ...
    var Gift = {
        ...
        getCat: function(giftID) {
            var cat = 1;
            try {
                cat = unsafeWindow.self.allCats[giftID] || 1;
            } 
            catch (e) {
                return 1;
            }
    ...
    function genGiftDom() 
    {
        ...
        for (var id in fixedGifts) 
        {
            ...
            try {
                ...
                c$('div', 'giftcat:'+gift.title+',id:'+giftId)
...
function OperationsCenter() {
    ...
    var Events = {
        ...
        setDefault_click: function() {
            options.set('defaultGroup', $('option[selected]', '#op_groups').val());
            options.save();
            showHelpPopup({
                icon: 'info',
                title: 'Default group',
    ...
    function updateMasteryBar(data, id, callback) {
        ...
        var mastery_bar, target_progress = data.progress;
        
        if ((mastery_bar = $('li[instance='+id+'] .task_mastery div')).length < 1) {
            return;
        }
        if (data.is_task_mastered) {
            target_progress = 100;
        }
        mastery_bar[0].mastery_percentage = Math.ceil(parseInt(mastery_bar.css('width')));
    ...
    function updateGroups() {
        var elt = $('#op_groups').empty();
        $.each(global.options.get('groups'), function(id, name) {
            elt.append(c$('option', 'value:'+id).text(name));
        }); 
        $('option[value='+options.get('defaultGroup')+']', '#op_groups')
    ...
    function publishOperation(mission, callback) {
        var target = $('#op_groups option[selected]', popupElt.content).val();
    ...
    function genMainDom() {
        ...
        .append(c$('label','for:op_groups').css('margin-right', 5).text('Publish in:'))
        .append(c$('select', 'id:op_groups,class:black').css('margin-right',5).width(400))
...
function InventoryAnalizer() {
    ...
    var Events = {
        ...
        item_click: function() {
            var sName   = String($('#type_filter option[selected]', popupElt.content).val());
            var sFilter = String($('#show_filter option[selected]', popupElt.content).val());
...
function FreeGiftsCenter() 
{
    ...
    var searchLinksRegex = /^([^:=]+)[:=]>?\s*(http:\/\/(?:bit.ly|tiny|spockon|apps).*)$/i;
    ...
    var CSCollector = function() {
        ...
        /**
         * Values:<br><br>
         * name {String} gift name<br>
         * links {Array} array of short links
         */
        this.gifts = new Object();
        /**
         * @param {String} text
         * @return {Object}
         */
        this.addLinks = function(text) {
            var nLinks = 0, nGifts = 0;
            var bUseFilter = $('#freegifts_usefilter').attr('checked');
            var rFilter = new RegExp(String($('#freegifts_filter').val()).replace(/\s*[^\w\s]\s*/g,'|'), 'i');
            
            $.each(text.split(/[\n\r]/), function(n, line) {
                var id, link, rgx = Util.doRgx(searchLinksRegex, line);
                
                if ( rgx.$1 && rgx.$2 && ( bUseFilter == false || rFilter.test($.trim(rgx.$1)) ) ) {
                    id = $.trim(rgx.$1).toLowerCase().replace(/\s/g,'_');
                    
                    if ( !Util.isSet(self.gifts[id]) ) {
                        self.gifts[id] = {'name':$.trim(rgx.$1), 'links':new Array()};
                        self.totalGifts++;
                        nGifts++;
                    }
                    self.gifts[id].links.push($.trim(rgx.$2));
                    self.totalLinks++;
                    nLinks++;
                }
            });
            ...
            return {
                'totalLinksAdded': self.totalLinks,
                'totalGiftsAdded': self.totalGifts,
                'linksAdded': nLinks,
                'giftsAdded': nGifts
            };
        };
         ...
         * @return {CSLink}
         */
        this.link = function(gift_id, link_id) {
            return self.gifts[gift_id][link_id];
        };
        
        this.each = function(callback) {
            $.each(self.gifts, callback);
        };
        
        this.clear = function() {
            delete self.gifts;
            self.gifts = new Object();
            self.totalGifts = 0;
            self.totalLinks = 0;
        };
    ...
    var Events = {
        ...
        searchInGroup_click: function() {
            ...
            var id = $('#groups option[selected]', popupElt.content).val();
            var nLimit = (parseInt($('#scan_limit').val()) || 100);
            var nDays = (parseFloat($('#scan_days').val()) || 1) * 24 * 60 * 60;
            var params = {
                'limit'      : nLimit, 
                'start_time' : parseInt((new Date()).getTime()/1000) - nDays 
            };
            
            if (parseInt(id)>0) {
                params['source_ids'] = id;
            }
            else {
                params['filter_key'] = 'nf';
            }
            log$('time: '+ params.start_time);
            
            facebook.streamGet(function(data) {
                log$('data.posts.length:'+Util.length(data.posts));
                $(self).removeAttr('disabled').css('opacity', 1);
                var outText = '';
                $.each(data.posts, function(index, post) {            
                    ...
                    if (Util.isArray(prop) && prop.length > 0) {
                        $.each(prop, function(idx,link) {
                            outText += (link.text + ' => ' + link.href + '\n');
                        });
                    }
                });
                addMassiveLinks(outText, $('#massive_input', popupElt.content));
            }, params);
        ...
        addMassLinks_click: function() {
            var elem = $('#massive_input', popupElt.content);
            var text = String(elem.val());
            
            if (text.indexOf('http') !== -1) {
                elem.val('');
                addMassiveLinks(text, elem);
            }
            return false;
        },
        massStart_click: function() {
            ...
            $('#masslinkslist > li', popupElt.content).each(function(n, elem) {
                ...
                if (count > 0) {
                    g = Collector.gifts[ $(elem).attr('gift') ];
                    g.amount = count;
                    gifts.push(g);
                    bCountAdded = true;
                }
    ...
    function collectGift(link, nSendType, callback) 
    {
        ...
        if( (bExcluded = (link.user_id === -1)) ) {
            item_message = 'I can\'t do this request.<br>'
                + 'You can accept it in a new tab <a href="'
                + link.url + '" target="_black">click here</a>';
            finish();
            return;
        }
        if (nSendType !== SEND_FBR && nSendType !== SEND_ZMC) { nSendType = -1; }
        // accept gift
    ...
    function addMassiveLinks(text, output) {
        var sHtml = '', sortArray = new Array();

        var result = Collector.addLinks(text);
        
        Collector.each(function(name, gift) { sortArray.push(name); });
        ...
        $.each(sortArray, function(index, name) {
            var gift = Collector.gifts[name];
            sHtml += '<li gift="'+name+'">';
            sHtml += '<div style="width:200px;cursor:pointer;" onclick="';
            sHtml += '$(this).parent().find(\'input\').select();">'+gift.name+':</div>';
            sHtml += '<div><input type="text" class="black_box" value="0" onclick="';
            sHtml += '$(this).select();"> / '+gift.links.length+'</div>';
            sHtml += '<a href="#select_all" onclick="';
            sHtml += '$(this).parent().find(\'input\').val('+gift.links.length+');return false;">';
            sHtml += 'ALL</a></li>';
        });
        
        $('#masslinkslist', popupElt.content).html(sHtml);
        
        output.val( 'All links added successfully.\n\n'
         + '- Gifts added: ' + result.giftsAdded + '\n'
         + '- Links added: ' + result.linksAdded + '\n\n'
         + '- Total gifts added: ' + result.totalGiftsAdded + '\n'
         + '- Total links added: ' + result.totalLinksAdded + '\n\n\n'
         + 'Set the amount of links you want to collect for every gift in the list.\n'
         + 'Gifts with 0 value will not be processed.\n\n'
         + 'Collector will open the correct link for every person.\n\n'
         + 'You can still adding more links, just paste here all links and click Add again.\n\n'
         + 'To start collecting, click Start.');
    }
    ...
    function startCollector(gifts) {
        ...
        var excl_ids   = new Object();
        var mwRegex    = /facebook.mafiawars.com|apps.facebook.com/;
        var limitRegex = new RegExp(options.get('excludedPattern'), 'i');
        var clcLog     = $('#collecting_log', popupElt.content);
        var msgLog     = $('#messages_log', popupElt.content);
        var sMessage   = '', nSuccess = 0, current, giftNameElt, messageElt;
        
        $('#massive_msgs', popupElt.content).empty()
        .append(giftNameElt = c$('span'))
        .append(messageElt = c$('span').css('margin-left', 5));
        
        function doNextAction(fnAction, message) {
            if (cancel_process === false) {
                messageElt.html((message || '')+' Searching a valid link...');
                fnAction();
            }
        }
        
        function next_gift() {
            if (gifts.length < 1 || cancel_process === true) {
                finish();
                return;
            }
            current = gifts.shift();
            if (!current.amount || current.amount < 1) {
                next_gift();
            }
            current.total = current.amount;
            current.count = 0;
            
            giftNameElt.html('Collecting '+Util.setColor(current.name, 'green'));
            doNextAction(next_link); 
        }
        
        function next_link() {
            current.count++;
            if (current.amount < 1 || current.links.length < 1) {
                doNextAction(next_gift);
                return;
            }
            var thisLink = current.links.shift();
            
            if (mwRegex.test(thisLink)) {
                parseAndOpen(thisLink);
            }
            else {
                getUnshortUrl(thisLink, parseAndOpen, function(msg) {
                    logErr$(Error(msg));
                    doNextAction(next_link); 
                });
            }
            
            function parseAndOpen(longUrl) {
                try {
                    ...
                    if(options.get('ignoreLimits') !== true && excluded.is(link.gift_id)) {
                        addMessageLog('Skipped. You\'ve reached the limit of this gift.');
                        doNextAction(next_gift);
                        return;
                    }
                    if (excl_ids[link.user_id] === true) {
                        doNextAction(next_link, 'Already collected from '+link.user_id);
                        return;
                    }
                    
                    messageElt.text('Collecting from '+link.user_id);
                    ...
                    collectGift(link, nSendBack, function(data) {
                        if (data.success) {
                            ...
                            addMessageLog(data.message, 'error', link.user_id);
                        }
                        if (options.get('skipSameUser') && data.crimespree !== true) { 
                            excl_ids[link.user_id] = true;
                        }
                        if (limitRegex.test(data.message) && !excluded.is(link.gift_id)) {
                            excluded.add(link.gift_id);
                            addMessageLog('Limit reached. Gift added to exclude list.');
                        }
                        doNextAction(next_link);
                    });
                }
        ...
        function addCollectLog(data) {
            var sTitle = '# '+current.name+' ('+(current.total-current.amount)+'/'+current.total+'):';
            var sImgSrc = data.image || global.zGraphicsURL+'v3/icon_gift_27x28_01.png';
            
            c$('li').append(c$('h4').text(sTitle))
            .append(c$('img').attr('src', sImgSrc))
            .append(c$('p').html(data.message))
            .prependTo(clcLog);
        }
        
        function addMessageLog(message, icon, user_id) {
            var userProfile = '';
            ...
            if (user_id) {
                userProfile += '# <a href="'+MW.getProfileLink(user_id);
                userProfile += '" target="_black">'+user_id+'</a>: ';
            }
            c$('li').append(c$('img').attr('src', sImgSrc))
        ...
        function finish() {
            ...
            giftNameElt.html('Finished.');
            messageElt.html(nSuccess+' links collected.');
            addMessageLog('Process finished at '+(new Date()).toUTCString(), 'info');
            b$('OK','class:short green').css('float','right')
            .appendTo(messageElt.parent()).click(
                function() {
                    $(this).unbind();
                    showDiv('start', 'mass');
                }
    ...
    function genMainDom() {
        ...
        // c$('li','tb:auto,class:automatic').appendTo(tabBar).click(Events.tab_click)
        // .append(c$('a','href:#').html('<span><em>Automatic
        // Mode</em></span>'));
        ...
        // var autoBody = c$('div',
        // 'auto_body').height(640).appendTo(popupElt.content);
        ...
        .append(' | ')
        .append(c$('a', 'href:#').text('Clear').click(Events.clearFilter_click))
        .appendTo(reqsPanel);
        ...
        var sLimitTitle = 'Set the limit of posts you want to scan.' ;
        var sDayTitle = 'Set how many days old you want to start scanning.' ;
        
        c$('div','start_mass').css('margin',5).height(600).append(massElt).appendTo(massBody);
        
        c$('div').css({'float':'left','width':'49%','height':600,'text-align':'center'}).append(
            c$('div', 'class:frame_box').width(340).css('margin',0)
            .append(c$('label','for:groups').text('Scan In:').css('color','#F4C506'))
            .append(c$('select','id:groups,class:black_box').width('80%'))
            .append('<br />')
            .append(c$('label','for:scan_limit').css('margin-left',5).text('Limit: '))
            .append(c$('input:text','id:scan_limit,class:black_box,title:'+sLimitTitle).width(80).val(100))
            .append(c$('label','for:scan_days').css('margin-left',5).text('Days: '))
            .append(c$('input:text','id:scan_days,class:black_box,title:'+sDayTitle).width(40).val(1))
            .append('<br /><br />')
            .append(b$('Refresh','class:short white').click(Events.refreshGroups_click))
            .append(b$('Scan & Add','class:short green').css('float','right').click(Events.searchInGroup_click))
        )
        .append(c$('textarea', 'id:massive_input,class:black_box').css({'width':'100%','height':300}))
        .append(c$('div').css({'clear':'both','text-align':'left'})
        ...
        c$('div').width('49%').css({'float':'left','margin-left':10}).append(
            ...
            .append(b$('Add','class:medium white').click(Events.addMassLinks_click))
            .append(b$('Clear all', 'class:medium white').css('margin-left',5).click(Events.clearCollector_click))
            .append(b$('Start','class:medium green').css('float','right').click(Events.massStart_click))
        )
        .append(c$('ul', 'id:masslinkslist,class:mass_gift_list').height(545))
        .appendTo(massElt);
        
        c$('div','log_mass').appendTo(massBody)
        .append(c$('div','id:massive_msgs,class:frame_box'))
    ...
    function restoreCollectorScreen() {
        ...
        $('#massive_input', popupElt.content).val(
           'Paste here all links and click Add.\n\nYou can filter gift\'s name and add only those you want to get.\n'
           + 'To active filter, check "Filter gifts" and add gift\'s name (partial or complete) in CSV format (Comma Separated Values).\n\n'
           + 'If you don\'t like the gifts added, click "Clear all" to reset Collector and change your filter.'
       );
    }
    ...
    function addFinishButtons(cQry, id, callback) {
        ...
        .click(Events.clearRequest_click);
        if (callback) {
        ...
        $.each(Requests, function(index, req) {
            ...
            c$('li', 'class:gift,id:req_'+index).appendTo(ulElt).append(
                ...
                .append(eImg.append(c$('span'))).append(
                    ...
                    .append(c$('a', 'href:#,class:ignore,req:'+index).text('ignore').click(Events.ignoreRequest_click))
                )
    ...
    function updateGroups() {
        var elt = $('#groups', popupElt.content).empty();
        $.each(global.options.get('groups'), function(id, name) {
            elt.append(c$('option', 'value:'+id).text(name));
        });
    }
    ...
    popupElt.addBase64Style(
        ...
        'cHVwIHVsI2NvbGxlY3RpbmdfbG9nIGxpIGltZyB7DQoJaGVpZ2h0OiAzMHB4Ow0KCXdpZHRoOiAzMHB4Ow0KCWZsb2F0OiBsZWZ0'+
        'Ow0KfQ0KI2ZyZWVnaWZ0c2NlbnRlcl9wb3B1cCB1bCNtZXNzYWdlc19sb2cgbGkgew0KCW1heC1oZWlnaHQ6IDQwcHg7DQoJcGFk'+
        'ZGluZzogNXB4IDIwcHggMHB4IDE1cHg7DQoJYm9yZGVyOiAxcHggc29saWQgIzIyMjsNCgltYXJnaW4tYm90dG9tOiA1cHg7DQp9'+
        'DQojZnJlZWdpZnRzY2VudGVyX3BvcHVwIHVsI21lc3NhZ2VzX2xvZyBsaSBwIHsNCglmb250LXNpemU6IDEycHg7DQoJaGVpZ2h0'+
        'OiAyMHB4Ow0KCXBhZGRpbmctbGVmdDogMzBweDsNCgltYXJnaW4tYm90dG9tOiAxcHg7DQp9DQojZnJlZWdpZnRzY2VudGVyX3Bv'+
        'cHVwIHVsI21lc3NhZ2VzX2xvZyBsaSBpbWcgew0KCWhlaWdodDogMTZweDsNCgl3aWR0aDogMTZweDsNCglmbG9hdDogbGVmdDsN'+
        'Cn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwubWFzc19naWZ0X2xpc3Qgew0KCWhlaWdodDogNDc0cHg7DQogICAgYm9yZGVy'+
        'OiAxcHggc29saWQgI0FBQTsNCgl0ZXh0LWFsaWduOiBsZWZ0Ow0KfQ0KI2ZyZWVnaWZ0c2NlbnRlcl9wb3B1cCB1bC5tYXNzX2dp'+
        'ZnRfbGlzdCBsaSB7DQoJaGVpZ2h0OiAyMHB4Ow0KCXBhZGRpbmc6IDVweCA1cHggMHB4IDVweDsNCgl0ZXh0LWRlY29yYXRpb246'+
        'IG5vbmU7DQp9DQojZnJlZWdpZnRzY2VudGVyX3BvcHVwIHVsLm1hc3NfZ2lmdF9saXN0IGxpIGRpdiB7DQoJZmxvYXQ6IGxlZnQ7'+
        'DQp9DQojZnJlZWdpZnRzY2VudGVyX3BvcHVwIHVsLm1hc3NfZ2lmdF9saXN0IGxpIGEgew0KCWZsb2F0OiByaWdodDsNCn0NCiNm'+
        'cmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwubWFzc19naWZ0X2xpc3QgbGkgaW5wdXQgew0KCXdpZHRoOiA0MHB4Ow0KCWhlaWdodDog'+
        'MTVweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwucmVxX2JveCBkaXYubGlfd3JhcHBlciB7DQoJYm9yZGVyLXRvcDog'+
        'MXB4IHNvbGlkICMyMjI7DQoJcGFkZGluZzogNnB4IDE0cHggMCA1MnB4Ow0KfQ0KI2ZyZWVnaWZ0c2NlbnRlcl9wb3B1cCB1bC5y'+
        'ZXFfYm94IGxpOmZpcnN0LWNoaWxkIHsNCglib3JkZXItd2lkdGg6IDBweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwu'+
        'cmVxX2JveCBsaSB7DQoJbWluLWhlaWdodDogNzZweDsNCgltYXgtaGVpZ2h0OiAxNjBweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJf'+
        'cG9wdXAgdWwucmVxX2JveCBsaS5naWZ0IHsNCgliYWNrZ3JvdW5kOiB1cmwoImh0dHA6Ly9td2ZiLnN0YXRpYy56Z25jZG4uY29t'+
        'L213ZmIvZ3JhcGhpY3MvdjMvaWNvbl9naWZ0XzI3eDI4XzAxLnBuZyIpIDE3cHggMjBweCBuby1yZXBlYXQ7DQp9DQojZnJlZWdp'+
        'ZnRzY2VudGVyX3BvcHVwIHVsLnJlcV9ib3ggbGkgZGl2LmJ1dHRvbnMgew0KCWZsb2F0OiByaWdodDsNCglmb250LXNpemU6IDEw'+
        'cHg7DQoJZm9udC13ZWlnaHQ6IGJvbGQ7DQoJbWFyZ2luOiAzcHggMHB4IDBweCAxNHB4Ow0KCXRleHQtYWxpZ246IHJpZ2h0Ow0K'+
        'CXdpZHRoOiA3MHB4Ow0KfQ0KI2ZyZWVnaWZ0c2NlbnRlcl9wb3B1cCB1bC5yZXFfYm94IGxpIGRpdi5ib2R5IHsNCgltYXJnaW4t'+
        'cmlnaHQ6IDEwMHB4Ow0KCW1hcmdpbi1sZWZ0OiA2NXB4Ow0KCW1pbi1oZWlnaHQ6IDYwcHg7DQp9DQojZnJlZWdpZnRzY2VudGVy'+
        'X3BvcHVwIHVsLnJlcV9ib3ggbGkgaDQgew0KCWZvbnQtc2l6ZTogMTRweDsNCglsaW5lLWhlaWdodDogMThweDsNCgltYXJnaW4t'+
        'Ym90dG9tOiA0cHg7DQoJcGFkZGluZy10b3A6IDJweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwucmVxX2JveCBsaSBw'+
        'IHsNCglmb250LXNpemU6IDEycHg7DQoJbGluZS1oZWlnaHQ6IDE1cHg7DQoJbWFyZ2luLWJvdHRvbTogNXB4Ow0KfQ0KI2ZyZWVn'+
        'aWZ0c2NlbnRlcl9wb3B1cCB1bC5yZXFfYm94IGxpIHAgaW1nIHsNCgl3aWR0aDogNDVweDsNCgloZWlnaHQ6IDQ1cHg7DQoJZmxv'+
        'YXQ6IGxlZnQ7DQoJbWFyZ2luLXJpZ2h0OiA4cHg7DQp9DQojZnJlZWdpZnRzY2VudGVyX3BvcHVwIHVsLnJlcV9ib3ggbGkgZGl2'+
        'LmJ1dHRvbnMgYS5zZXh5X2J1dHRvbl9uZXcgew0KCW1hcmdpbi1ib3R0b206IDZweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9w'+
        'dXAgdWwucmVxX2JveCBsaSBkaXYuYnV0dG9ucyBhLmlnbm9yZSB7DQoJdGV4dC10cmFuc2Zvcm06IHVwcGVyY2FzZTsNCn0NCiNm'+
        'cmVlZ2lmdHNjZW50ZXJfcG9wdXAgdWwsICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgbGksICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAg'+
        'ZGwsICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgZHQsICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgZGQsICNmcmVlZ2lmdHNjZW50ZXJf'+
        'cG9wdXAgaDIsICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgaDMsICNmcmVlZ2lmdHNjZW50ZXJfcG9wdXAgaDQsICNmcmVlZ2lmdHNj'+
        'ZW50ZXJfcG9wdXAgcCB7DQoJbWFyZ2luOiAwcHg7DQoJcGFkZGluZzogMHB4Ow0KfQ0K'
    );
...
function HomeFeedCenter() 
{
    ...
    var CSStream = function(feed) {
        ...
        try {
            ...
            if (cAction && cAction.group) {
                sGrpName = 'do'+ groupNames[cAction.group].replace(/\s/g, '');
            }
        }
        catch(err) {
            isValid = false;
        ...
        switch (sGrpName) {
            case 'doGiveSocialHelp'      : cFilter = (new CSFilter('socialHelpFilter')).test;   break;
            case 'doCollectLoot'         : cFilter = (new CSFilter('collectLootFilter')).test;  break;
            case 'doPropertyHelp'        : cFilter = (new CSFilter('propertyHelpFilter')).test; break;
            case 'doClaimBonusAndReward' : cFilter = (new CSFilter('rewardFilter')).test;       break;
        }
        ...
        /** @type {Function} */ this.filter       = cFilter;
     ...
     */
    var CSFilter = function(filter_name) {
        ...
        this.test = function(text) {
            ...
            return 'This help has been filtered.';
        };
    ...
    var Events = {
        helpFeed_click: function() {
            ...
            var addClearButton = function() {
                if (chkIsChecked('#feedstream_autoclose')) {
                    setTimeout(function() {
            ...
            if (cHelp.action && chkIsChecked(cHelp.action.enable)) {
    ...
    function cleanData(data) {
        ...
        $('img, .sexy_button_new, br, div:empty', obj).remove();
        return obj.html();
    }
    /**
     ...
     * @return {Boolean}
     */
    function chkIsChecked(selector, context) {
    ...
    function addDefaultActions() {
        ...
        Actions.add('war_share_reward_feed_click', {
            ...
            group      : 5,
            success    : /(<div\s*style=\\"color:\s*white;\s*font-size:\s*13px\\">[^<]+<\/div>|This\s*reward\s*is\s*invalid)/i
        });
        ...
        Actions.add('bossfightv2_ask_feed_click',      { name: 'Collect a Boss Fight item',    group: 5 });
        Actions.add('quest_questFeedReward',           { name: 'Claim quest reward',           group: 5 });
        ...
        Actions.add('map_mapboss_reward_claim',        { name: 'Claim boss reward',            group: 5 });
    ...
    function filterWar(htmlText) {
        ...
        try {
            ...
            if ((new CSFilter('warFilter')).test(text) !== false) {
    ...
    function doOperation(htmlText, callback) 
    {
        ...
        try {
            ...
            if ((new CSFilter('missionFilter')).test(sName) !== false) {
    ...
    function autoMode() {
        ...
        function closeAndNext(message, delay) {
            ...
            $('#total_feeds', '#feed_center_header').html(--nTotalHelps);
            
            cStream = feedStream.shift();
            
            if (nTotalHelps < 1 || feedStream.length() < 1) {
                var refreshDelay = options.get('refreshDelay')*0;
                messageTimer.start('All helps finished!. Refresh and continue in %N% seconds.',refreshDelay, refreshList);
                return;
            }           
            if (!delay) {
                doNewHelp();
        ...
        function doNewHelp() {
            ...
            if (Util.isSet(cStream.filter)) {
                if ((filterResult = cStream.filter(cStream.feed.attachment.name)) !== false) {
                    genHelpLogDom(cStream, filterResult);
                    nSkipped++;
                    closeAndNext();
                    return;
                }
            ...
            else if (options.get(cStream.groupName)) 
            {
                ...
                doHelp(toInternalUrl(cStream.url), cStream.action, 
                    function(data) 
                    {
        ...
        function refreshList() {
            ...
            facebook.queryFeed(options.get('feedsLimit'), function(fs) {
                // catch error
                if (Util.length(fs) < 1) {
                    messageTimer.start('Error loading posts, try again in %N%...',60,refreshList);
                    return;
                }
                ;
                if ((feedStream = new CSStreamCollection(fs, true)).length() < 1) {
                    messageTimer.start('No new posts, refresh again in %N%...', 0, refreshList);
                    return;
                }
                nTotalHelps = feedStream.length();
                $('#total_feeds', '#feed_center_header').html(nTotalHelps);
                doNewHelp();
            });
        }
    ...
    function genMainDom() {
        ...
        c$('div', 'class:config_tab,id:gotowar_config').appendTo(configDiv)
        .append(genConfigFilter('warfilter'));
        
        c$('div', 'class:config_tab,id:givesocialhelp_config').appendTo(configDiv)
        .append(genConfigFilter('socialhelpfilter'));
        ...
        .append(genConfigFilter('rewardfilter')).appendTo(configDiv);
        
        c$('div', 'class:config_tab,id:collectloot_config').appendTo(configDiv)
        .append(genConfigFilter('collectlootfilter'));
        
        c$('div', 'class:config_tab,id:propertyhelp_config').appendTo(configDiv)
        .append(genConfigFilter('propertyhelpfilter'));
        ...
        function genGeneralConfig() {
            ...
            .append(c$('div','class:clearfix'))
            .append(c$('label', 'for:feedstream_refreshdelay').text('Refresh delay (min.): '))
        ...
        popupElt.applyOptions({
            ...
            'feedstream_joinmissionafter'  : {0:'None', 1:'Energy', 2:'Stamina', 3:'Both'},
            'feedstream_maxfreeslots'      : {100:'Any slots', 1:'1 slot', 2:'2 slots', 3:'3 slots', 4:'4 slots'},
            'feedstream_helpdelay'         : {0:'0', 1:'1', 2:'2', 3:'3', 5:'5'},
            'feedstream_refreshdelay'      : {1:'1', 3:'3', 5:'5', 15:'15', 30:'30', 60:'60'}
        });
    }
...
function MultiGifter(select_gift_cat, select_gift_id) 
{
    ...
    var Events = {
        ...
        selectAll_click: function() {
            $('.checkboxlist[name='+$(this).attr('name')+'] input:checkbox').attr('checked', 'checked');
            return false;
        },
        selectNone_click: function() {
            $('.checkboxlist[name='+$(this).attr('name')+'] input:checkbox').attr('checked', '');
            return false;
        },
    ...
    function genMainDom() {
        ...
        .append(
            c$('input:text', 'class:black,id:search_user').width(150)
        ...
        .append(
            c$('div').css('float', 'right')
            .append(
                c$('a', 'href:#,title:Clear all selected friends.')
    ...
    function showPage(listName, page) {
        ...
        var elem = {
            ...
            'gift': function(item) {
                if ((item.amount < 1) && options.get('hideZeroAmount')) return '';
...
/**
 * Show a new popup which start a massive multi-attack mode.
 * 
 * @param {String}_sHtmlText
 * @param {String}_sList
 * @param {Function}_callback
 */
function Battlefield(_sHtmlText, _sList, _callback) 
{
    const ERROR_SUCCESS = 0;
    const ERROR_BAD_RESPONSE = 1;
    const ERROR_NO_FIGHT_RESULT = 2;
                          
    var statusTimer = new timerMessage('#status_log'); 
    var StartCity = MW.currentCity();
    var CurrentCity = 0;
    var abort_process = false;
    var bfirstRun = true;
    var bAttackWhiteList = false;
    //var bAutoPublishLimitReached = false;
    var fightTimeouts = {
        maxfightTime: 0,
        maxfightCityTime: 0
    };
    ...
    var fightStats = {
        ...
        timeToPause    : function() { return Util.toDateString(fightTimeouts.maxfightTime - (new Date()).getTime()); },
        timeToTravel   : function() { return Util.toDateString(fightTimeouts.maxfightCityTime - (new Date()).getTime()); },
        ...
        healthpercent  : function() { 
            var n = (fightStats.health * 100) / unsafeWindow.User.max_health;
            return isNaN(n) ? 0 : n; 
        },
     ...
     */
    var opponentClass = function(id, title, name, level, mafia, profile, url, iced, faction) {
        
        this.id      = Util.parseNum(id);
        this.title   = title;
        this.name    = name;
    ...
    var List = {
         ...
         * @return {Array}
         */
        toArray: function(listName) {
            var new_array = [];
            $.each(options.get(listName), function(id, name) {
         ...
         * @return {Boolean}
         */
        opponentListed: function(list, id) {
            if (parseInt(id) === 0) {
                return false;
            }
            return typeof(options.get(list)[id]) !== 'undefined';
        },
        /**
         * Add opponent to list
         * @param {String} list
         * @param {opponentClass} opp
         * @return {Boolean}
         */
        addToList: function(list, opp) {
            var time = (new Date()).getUTCDate(); 
            try {
                if (parseInt(opp.id) === 0) {
                    throw Error('User id is 0');
                }
                if (!this.opponentListed(list, opp.id)) {
                    log$('added to ' + list + ': ' + opp.id);
                }
                else {
                    log$('opponent exists in ' + list + ': ' + opp.id);
                }
                options.get(list)[opp.id] = opp.name + '  level ' + opp.level + ' at '+time;
                return true;
            }
            catch(err) {
                logErr$(err);
        ...
        addCurrentToBlackList: function() {
            var bNew = !this.opponentListed('blackList', this.current.id);
        ...
        addCurrentToWhiteList: function() {
            var bNew = !this.opponentListed('whiteList', this.current.id);
    ...
    var Events = {
        saveSession_click: function() {
            var outText = 'GENERAL:\n';
            $('#bottom_fixed .stats dl', popupElt.content).each(function(index, elem) {
                var dt = $('dt', elem), dd = $('dd', elem);
                if (index > 6 && !dt.hasClass('cash')) {
                    var st = String(dt.text()); 
                    outText += st + ((st.length>15)?'\t':(st.length>7?'\t\t':'\t\t\t')) + dd.text() + '\n';
                }
            });
            outText += 'CASH:\n';
            outText += fightStats.allCitiesCash(false) + '\n';
            
            $.each(['loot', 'iced', 'kill'], function(i1, name) {
                var listElement = $('#'+name+'_logs li', popupElt.content);
                if (listElement.length > 0) {
                    outText += '----------\n\n'+String(name).toUpperCase()+'\n\n';
                    listElement.each(function(i2, elem) { outText += $(elem).text() + '\n'; });
                }
            });
        ...
        addNewToList_click: function() {
            ...
            var profileUrl = showPromptPopup(
                ...
                function(text) { addMWProfiles(listName, text); }
            );
            return false;
        },
        ...
        mwProfile_click: function() {
            var name = $(this).attr('name').toLowerCase();
            var url = 'http://facebook.mafiawars.com/mwfb/remote/'+MW.getIntURL('stats');
            ...
            if (selectedElts.length < 2 || confirm('Are you sure to open ALL selected users?')) {
                selectedElts.each(function(index, elem) {
                    unsafeWindow.open(url + '&user=p|' + elem.value);
                });
            }
            return false;
        },
        ...
        sort_click: function() {
            List.sort(this.id.match(/sort_by_(\w+)/)[1]);
            genEnemyListDom();
            return false;
        },
        refresh_click: function() {
            showDiv('status', '_panel');
            $('#opponents_table').empty();
            autoModeMessage('Refreshing target list...', true, true);
            options.fromDomElements();
            setTimeout(function() {
                List.clear();
                refreshPlayerList(function() {
                    if (List.length() > 0) {
                        ...
                        $('#opponents_table').append(
                            c$('center').css({'padding':40, 'font-weight':'bold', 'font-size':14})
                            .text('No players found. Try change filters and click "Refresh".')
                        );
                    }
                    showDiv('start', '_panel', true);
                });
            }, 500);
            return false;
        },
        attack_click: function() {
            ...
            var url = List.setCurrent(id).attack;
            updateNewOpponent();
            showDiv('ajaxloader', '_panel');
            ...
            reqSurvey(function() {manualAttack(url);});
            return false;
        },
        powerAttack_click: function() {
            updateStats();
            showDiv('ajaxloader', '_panel');
            manualAttack(List.current.pwratk);
            return false;
        },
        attackAgain_click: function() {
            updateStats();
            showDiv('ajaxloader', '_panel');
            manualAttack(List.current.attack);
            return false;
        },
        runAway_click: function() {
            ...
            options.save();
            showDiv('', '_panel');
            if (List.length() > 0) genEnemyListDom();
            $('#timer_timetotravel, #timer_timetopause, #massacre_health_sign').hide();
            $('.leaders_rewards', '#fight_div')
            .children().stop().css('opacity', 1).hide();
            showDiv('start', '_panel', true);
            showDiv('opponents', '_list');
            showDiv('options', '_div');
            return false;
        },
        autoMode_click: function() {
            abort_process = false;
            $('#attack_again_button, #power_attack_button').hide();
            $('#stop_button', '#status_panel').show();
            options.fromDomElements();
            options.save();
            List.setRandomCurrent();
            updateNewOpponent();
            showDiv('status', '_panel', true);
            showDiv('events', '_list');
            showDiv('fight', '_div');
            $('#opponents_table').empty();
            if (options.get('fightTime')) {
                resetTimeout('fightTime');
                $('#timer_timetopause').show();
            }else{
                $('#timer_timetopause').hide();
            }
            if (options.get('fightCityTime')) {
                resetTimeout('fightCityTime');
                $('#timer_timetotravel').show();
            }else{
                $('#timer_timetotravel').hide();
            }
            if (options.get('forceStartingCity')) {
                StartCity = options.get('startingCity');
            }
            reqSurvey(autoAttackNew);
            return false;
        },
    ...
    var popupElt = new domPopupObject('battlefield_popup', {
        ...
        onclose: function() {
            ...
            options.save();
            List.clear();
            delete List, Events, Regex, options, queryResult, statusTimer;
        }
    });
    // Regex Expresions
    var Regex = {
    ...
    var fightDialog = {
        'WON' : [
            {'me':'I\'m Max-Pain<br>slow... killer!'     ,'he':'No more pain<br>please!.'}
        ],
        'LOST': [
            {'me':'I\'m sorry but, i need to go.'        ,'he':'I\'ll see my log<br>and you\ll die.'}
        ],
        // power attack is used!
        'POWERATTACK': [
            {'me':'Today you\'ll know<br>my power!!'     ,'he':'What the hell?<br>Do I deserve it?'},
            {'me':'More, more...<br>more blood!!!'       ,'he':'More, more...<br>more blaaah!'},
            {'me':'I\'ll kill you<br>very FAST!!'        ,'he':'Not if I\'m healing myself!'},
            {'me':'Die! Die! Die! Muahaha!'              ,'he':'Great! Time to wash my panties.'},
            {'me':'You\'re a slacker, so die!'           ,'he':'You\'re a sucker, so I\'ll be back!'}
        ],
        // you iced your opponent!
        'ICED': [
            {'me':'Great battle!<br>but you\'re iced!'   ,'he':'You\'re very strong...'},
            {'me':'Like a little "ice" with that?'       ,'he':'Ice is for drinks, not me!'},
            {'me':'And... iced.<br>good day...'          ,'he':'Someone will catch you!'},
            {'me':'Iced so soon?,<br>so Sad!'            ,'he':'Soon?? Sad??<br>Bull shit!!'}
        ],
        // you killed your opponent!
        '#ffffff_WON': [
            {'me':'It\'s my job.<br>Nothing personal.'   ,'he':'**CRUSH**'},
            {'me':'Killed...<br>i\'ll pray for you.'     ,'he':'.........'}
        ],
        // It was close, but you defeated your opponent!
        '#ffff00_WON': [ 
            {'me':'It was close,<br>but i\'m better!'    ,'he':'You\'re just<br>a lucky man!'}
        ],
        // You dominated your opponent!
        '#00ff00_WON': [
            {'me':'You\'re very weak,<br>anyway...'      ,'he':'I\'ll tell my mafia<br>to attack you!'},
            {'me':'Easy...<br>very easy...'              ,'he':'Let me change<br>my account...'},
            {'me':'I\'ll win with my eyes closed.'       ,'he':'Close your eyes then... Away.'},
            {'me':'Bored, bored,<br>bored...'            ,'he':'Find another<br>strong player.'},
            {'me':'Farmville is best place for you.'     ,'he':'I\'m not married to PC like you!'},
            {'me': 'Couldn\'t you TRY to fight me?'      ,'he':'Grrrr... Give me a break!'}
        ],
        // You narrowly defeated your opponent!
        '#ff9900_WON' : [
            {'me':'Yeah, closed but i win!!!'            ,'he':'Next time man<br>next time...'},
            {'me':'You\'re strong but I\'m stronger!'    ,'he':'Now... but we\'ll see next time!'}
        ],
        // You were dominated by your opponent!
        '#ff0000_LOST': [
            {'me':'Oh!, i\'ll forgot you...'             ,'he':'That\'s fine,<br>run! run! Forest.'}
        ],
        // It was close, but you were defeated by your opponent!
        '#ff9900_LOST': [ 
            {'me':'Shit!, i need more power...'          ,'he':'Try tomorrow<br>maybe? hahaha!'}
        ]
    };
    ...
    function genMainDom() {
        ...
        var battle_div = c$('div', 'class:battleground').appendTo(popupElt.content);
        var fightStatus = c$('div', 'class:war_status clearfix')
        .appendTo(
            c$('div', 'declare_war')
            .appendTo(c$('div', 'fight_div').appendTo(battle_div))
        );
        
        c$('div', 'id:fight_side_left,class:side left').appendTo(fightStatus)
        .append(c$('h3'))
        .append(
             c$('ul', 'class:leader_info')
            .append(c$('li','class:leader_name').text('???'))
            .append(
                c$('li').append(
                    c$('img','width:50,height:50')
                    .attr('src',global.zGraphicsURL+'character_silhouette.jpg')
                )
                .append(c$('div','class:war_x').hide())
            )
            .append(c$('li').append('<strong>Level: </strong>???'))
            .append(c$('li').append(c$('span','class:mafia_size').text('???')))
            .append(c$('li').append(c$('span','class:mafia_attack').text('???')))
        )
        .clone().attr('id', 'fight_side_right').appendTo(fightStatus)
        .removeClass('left').addClass('right')
        .find('.mafia_attack').removeClass('mafia_attack').addClass('mafia_defense');
        
        c$('div', 'class:war_rewards').appendTo(fightStatus)
        .append(c$('h3'))
        .append(
            c$('div', 'class:leaders_rewards')
            .append(c$('dl', 'id:left_side_text'))
            .append(c$('div', 'class:left'))
            .append(c$('dl', 'id:right_side_text'))
            .append(c$('div', 'class:right'))
        );
        
        var tabs = new domTabObject(
            c$('div', 'options_div').appendTo(battle_div), 'fightOpt', 
            ['Config Page 1','Config Page 2','BlackList','WhiteList'], 190, 'auto', 
        ...
        // ATTACK DELAY
        c$('li').appendTo(divListElt)
        .append(cb('bfopt_useattackdelay', 'Attack delay (Automatic Mode):'))
        ...
        // STOP ON LEVELUP
        c$('li').appendTo(divListElt)
        .append(cb('bfopt_stopbeforelvlup', 'Stop before level-up when remain:'))
        ...
        .append(c$('label','for:bfopt_skiplevel').text(' than:'))                           
        ...
        .append(c$('label','for:bfopt_skipmafia').text(' than:'))                           
        .append(c$('input:text', 'id:bfopt_skipmafiaof,size:5,class:right'));
        // SHOW DIALOGS
        c$('li').appendTo(divListElt)
        .append(cb('bfopt_showdialogs', 'Show fight dialogs.'));
        ...
        .append(c$('span').text('Show: ').css('margin-left',4))
        .append(cb('bfopt_showloot',        'Loot'))
        .append(cb('bfopt_showstash',       'Stash'))
        .append(cb('bfopt_showcoins',       'Coins'))
        .append(cb('bfopt_showhelp',        'Troops'))
        .append(cb('bfopt_showblacklisted', 'Blacklisted'))
        .append(cb('bfopt_showwhitelisted', 'Whitelisted'));
        ...
        .append(cb('bfopt_fightcitytime', 'Travel to different city after:'))
        ...
        c$('div').css({'float':'left','text-align':'left','margin':5,'width':400}).appendTo(listTab)
        ...
        .append(c$('br'))
        .append(c$('select', 'id:bfopt_blacklist,multiple:multiple').width(400).height(160));
        
        var tb = new domTableObject(c$('div','blackList_table').appendTo(listTab), 6, 1, {
            'width': 300, 
            'float': 'left'
        });
        
        tb.cell(0,0).append(c$('span').html('To add, delete or clear the list:'));
        tb.cell(1,0).append(b$('Add New', 'id:add_new_list,name:blackList,class:short white'));
        tb.cell(1,0).append(b$('Delete', 'id:delete_item_list,name:blackList,class:short white'));
        tb.cell(1,0).append(b$('Clear List', 'id:clear_list,name:blackList,class:short white'));
        tb.cell(2,0).append(c$('span').html('To see selected user\'s MW profiles:'));
        tb.cell(3,0).append(b$('MW Profile', 'id:go_profile,name:blackList,class:short white'));
        tb.cell(4,0).append(c$('span').html('To save/load a list in txt format:'));
        tb.cell(5,0).append(b$('Load List', 'id:add_from_list,name:blackList,class:short white'));
        tb.cell(5,0).append(b$('Save List', 'id:get_current_list,name:blackList,class:short white'));
        ...
        .append(c$('br'))
        .append(c$('select', 'id:bfopt_whitelist,multiple:multiple').width(400).height(160));
        
        tb.table.clone()
        .appendTo(c$('div','whiteList_table').appendTo(listTab))
        .find('a').each(function(index, elem) {
            $(elem).attr('name', 'whiteList');
        });
        
        // Assign click events
        $('#add_new_list', '#options_div').click(Events.addNewToList_click);
        $('#delete_item_list', '#options_div').click(Events.deleteSelected_click)
        $('#clear_list', '#options_div').click(Events.clearList_click)
        $('#go_profile', '#options_div').click(Events.mwProfile_click)
        $('#add_from_list', '#options_div').click(Events.addFromList_click)
        $('#get_current_list', '#options_div').click(Events.getList_click)
        ...
        // USER PANELS
        // ---------------
        var panels = c$('div', 'class:panels').appendTo(popupElt.content);
        
        // AJAX LOADER
        c$('div', 'ajaxloader_panel').appendTo(panels).hide()
        .append(c$('img').attr('src', global.resource.ajax_loader))
        .css({
            'width': 'auto',
            'text-align': 'center'
        ...
        // START PANEL
        c$('div', 'start_panel').appendTo(panels).css({
            'width': 'auto',
            'text-align': 'center'
        });
        c$('span').appendTo('#start_panel').css('margin-right', '10pt')
        .append(b$('Random Attacks!', 'class:short red').click(Events.autoMode_click))
        .append(
            b$('Attack WhiteList!', 'class:short red')
            .css('margin-left', 5).click(Events.attackWhiteList_click)
        );
        // ATTACK BUTTONS
        c$('div', 'buttons_panel').appendTo(panels).hide().css({
            'width': 'auto',
            'text-align': 'center'
        })
        .append(b$('Attack again', 'attack_again_button')
            .addClass('short red').click(Events.attackAgain_click))
        .append(b$('Power Attack', 'power_attack_button').css('margin-left', '10pt')
            .addClass('short red').click(Events.powerAttack_click))
        .append(b$('Run Away', 'run_away_button')
            .css('margin-left', '10pt').click(Events.runAway_click)
        );
        
        // ACTIVE STATUS
        c$('div', 'status_log')
        .appendTo(c$('div', 'status_panel').appendTo(panels).hide()).css({
            'margin-left': 30,
            'float': 'left',
            'width': 'auto'
        });
        
        c$('span').attr('style', 'margin-right: 15pt; float: right;').appendTo('#status_panel')
        .append(b$('Stop').attr('id', 'stop_button').click(Events.stop_click));
        
        // BOTTOM PART
        var bottomElt = c$('div', 'id:bottom_fixed,class:bottom').height(450).appendTo(popupElt.content);
        
        // STATS
        var statsPanel = c$('div', 'class:stats').appendTo(bottomElt);

        c$('div').height(30).appendTo(statsPanel)
        .append(
            c$('div', 'class:bossfight_health_bar').width(140)
            .append(
                c$('div', 'class:bossfight_current_health_med')
                .css('width', global.userStats.healthpercent() + '%')
            )
        )
        .append(
            c$('span', 'id:massacre_health_sign,class:health_sign').text('HEALING').hide()
        );
        
        var statsArray = [
            {name:'health:',            cls:'health',              id:['maxhealth','health']               },
            {name:'Stamina:',           cls:'stamina',             id:['maxstamina','stamina']             },
            ...
            {name:'Victory Coins:',     cls:'stat_victory',        id:'victorycoins'                       },
        ...
        // generate stats
        $.each(statsArray, function(index, stat) {
            var span = c$('span').text(stat.name);
            var dl = c$('dl').appendTo(statsPanel);
        ...
        c$('center').appendTo(statsPanel)
        .append(c$('a', 'href:#').text('Save Session Stats').click(Events.saveSession_click));
        
        
        // PANELS
        var listPanel = c$('div', 'class:list').appendTo(bottomElt);
        
        // AUTOMATIC LOG
        c$('div', 'events_list').appendTo(listPanel)
        .append(c$('div','class:player_updates').height('20%')
            .append(c$('div', 'id:fightlogs,class:update_item').height('80%'))
        )
        .append(c$('div', 'class:buttons')
            ...
            .append(c$('a', 'href:#kill').css('opacity', 0.5).text('Kills').click(Events.showLog_click))
        )
        .append(c$('div', 'id:items_logs,class:player_updates').height('73%'))
        .append(c$('ul',  'id:loot_logs').height('73%'))
        .append(c$('ul',  'id:iced_logs').height('73%'))
        .append(c$('ul',  'id:kill_logs').height('73%'));
        
        // MANUAL LIST OF FIGHTERS
        c$('div', 'opponents_list').appendTo(listPanel)
        .append(
            c$('div', 'class:header')
    ...
    // show specified div element hidded all of same category
    ...
    // show fight dialogs.
    function showDialog(id, win) {
        var speech1 = c$('span').css('color', win ? 'green' : 'red');
        var speech2 = c$('span').css('color', win ? 'red' : 'green');
        var leaderElt = $('.leaders_rewards', '#fight_div');
        var dlg = (fightDialog[id] || fightDialog[win?'WON':'LOST']);
        
        leaderElt.children().stop().css('opacity', 1).hide();
        
        if (dlg.length == 1) {
            speech1.html(dlg[0].me);
            speech2.html(dlg[0].he);
        }
        else if (dlg.length > 1) {
            var index = Math.floor(Math.random() * dlg.length);
            speech1.html(dlg[index].me);
            speech2.html(dlg[index].he);
        }
        
        setTimeout(function() {
            $('.left', leaderElt).show();
            $('#left_side_text', leaderElt).empty().append(speech1).fadeIn(400);
        }, 500);
        
        setTimeout(function() {
            $('.right', leaderElt).show();
            $('#right_side_text', leaderElt).empty().append(speech2).fadeIn(400);
        }, 1000);
    }
    ...
    function setAttackTimer(fn) {
        ...
        statusTimer.start('Ready to attack '+List.current.anchor+' in %N% seconds...', delay, fn);
    }
    
    // update new opponent "right side" info
    function updateNewOpponent() {
        if (typeof(List.current) == 'undefined') {
            List.current = {name:'???', level:'???', mafia:'???'};
        }
        
        $('.leaders_rewards', '#fight_div').children().stop().css('opacity', 1).hide();
        
        $('#fight_side_left, #fight_side_right', '#fight_div')
        .toggleClass('losing winning', false);
        
        $('h3', '#fight_div').empty();
        $('.war_x', '#fight_div').hide();
        
        var rightSide = $('#fight_div #fight_side_right');
        $('.leader_name', rightSide).text(List.current.name);
        $('li:eq(2)', rightSide).html('<strong>Level: </strong>'+List.current.level);
        $('.mafia_size', rightSide).html(List.current.mafia);
        $('.mafia_defense', rightSide).html('???');
        $('li > img', rightSide).attr('src', global.zGraphicsURL+'character_silhouette.jpg');
    }
    ...
    function addMWProfiles(listName, profilesText) {
        ...
        function addProfile() {
            ...
            try {
                ...
                if (List.opponentListed(listName, Util.parseNum(id))) {
                    return;
                }
    ...
    function getOpponentFromProfile(data) {
        ...
        try {
            ...
            url        = Util.doRgx(/['"](remote[^"']*)/, attack).$1;
            id         = Util.uSplit(url).opponent_id;
            sText      = Util.htmlDecode(eQry.find('.stats_title_text:first').text());
            name       = Util.substr(sText, '"', '"', 1);
            level      = Util.parseNum(sText.substr(sText.lastIndexOf('"') + 1));
        }
        catch(err) {
            logErr$(err);
            return {error_msg:err.message}; 
        }
        return new opponentClass(id, '', name, level, 0, MW.getProfileLink(id),  url);
    }
    ...
    function addPlayerToWhiteList(page, listName) {
        ...
        if (player.error_msg) {
            showHelpPopup({
                icon: 'error',
                title: 'Error adding player to list',
                message: player.error_msg
            });
            return false;
        }
        else {
            return List.addToList(listName, player);
        }
    }
    
    function fromRandomWhiteList() {
        var p, lst = List.toArray('whiteList');
        ...
        if (List.opponentListed('blackList', p.id)) {
            log$('skiping blacklisted id: ' + p.id);
            delete options.get('whiteList')[p.id];
            fromRandomWhiteList();
            return;
        }
        ...
        httpAjaxRequest({
            ...
            success: function(htmlText) 
            {
                List.current = getOpponentFromProfile(htmlText);
                updateNewOpponent();
                autoAttack();
            }
        });
    ...
    function isValidOpponent(opp, iced) {
        var n1 = options.get('skipMafiaOf'), n2 = options.get('skipLevelOf');
        var o1 = options.get('skipMafiaWay'), o2 = options.get('skipLevelWay');
        var i = 0, c = '', v;
        ...
        if (List.opponentListed('blackList', opp.id)) {
    ...
    function parseOpponentTable(htmlText) {
        ...
        if (CurrentCity === 4 && (faction = jQry.find('.zy_progress_bar_faction_text')).length === 2) {
            fightStats.yakuza = Regex.factionProgress(faction.eq(0).text());
            fightStats.triad  = Regex.factionProgress(faction.eq(1).text());
            updateStats();
        }
        // fighters table
        jQry.find('.fight_table tr:has(a)').each(function(index, element) 
        {
            ...
            try {
                ...
                if ((elem = e$('a:regex(href,action=attack)', element))) {
                    nId = Util.parseNum(sId = Util.uSplit(sUrl = elem.attr('href')).opponent_id);
                    sProfile = MW.getProfileLink(sId);
                }
                sName    = $.trim($('td:eq(0) a', element).text());
                nLevel   = Util.parseNum(Util.textNodes($('td:eq(0)', element)));
                bIced    = ($('img:regex(src,iced)', element).length > 0);
                sTitle   = $.trim($('td:eq(0) span', element).text());
                nMafia   = Util.parseNum($('td:eq(1)', element).text());
                
                if (sUrl && sName && sProfile && nLevel && nId) {
                    var opp = new opponentClass(nId, sTitle, sName, nLevel, nMafia, sProfile, sUrl, bIced, sFaction);
                    if (isValidOpponent(opp)) List.add(opp);
                }
            }
            catch(err) {
                logErr$(err);
        ...
        if (!options.get('skipIced')) List.sort('iced');
        log$('UserList parsed: new fighters total: ' + List.length());
    }
    ...
    function genEnemyListDom() {
        ...
        List.each(function(index, pl) {
            ...
            .append(
                ...
                c$('span', 'id:player_name,class:update_txt').width(260)
                .html(pl.title + '<br>' + pl.name + '<br>Level: ' + pl.level)
            ).append(
                c$('span', 'id:player_mafia,class:update_txt')
                .width(50).css('padding',5).text(pl.mafia)
            ).append(
                b$('Attack', 'class:short red,id:attack_id_'+ index)
    ...
    function genLogDom(message, icon, pic, opp) {
        ...
        if (icon == 'fight') {
            updateItem = $('#fightlogs', '#events_list').empty();
        }
        else {
            updateItem = c$('div', 'class:update_item,id:'+icon+'_item').prependTo('#items_logs');
        }
        c$('div').addClass('update_timestamp').appendTo(updateItem).html((new Date()).toUTCString());
        c$('div').addClass('update_icon').appendTo(updateItem).append(updateIcon);
        if (pic) {
        ...
        if (icon == 'loot' || icon == 'stash') {
            updateItem.addClass(icon);
        }
    ...
    function addLootToLog(message, pic) {
        ...
        var rgx = message.match(/\s(\d+)\s([^!]*)!/);
        
        if (rgx) {  
            var item_id =  Util.doRgx(/graphics\/(\w+)/, pic.attr('src')).$1;
            var count = parseInt(rgx[1]); 
            var elt = e$('#loot_'+item_id, '#loot_logs');
            if (elt !== null) {
                ...
                var stats = rgx[2];
                if ((rgx = message.match(/\((\d+)\s*(\d+)\)/))) {
                    stats += ' (A:'+rgx[1]+' D:'+rgx[2]+')';
                }
                c$('li', 'loot_'+item_id).prependTo('#loot_logs')
                  .append(pic.css('vertical-align', 'middle'))
                  .append(c$('span').css('margin-left',10).text('Found '))
                  .append(c$('span','item_count').css('color','green').text(count))
                  .append(c$('span').text(' '+stats+'.'));
            }
        }
    }
    ...
    function updateStats() {
        ...
        $('.bossfight_current_health_med').css('width', fightStats.healthpercent() + '%');
        
        if (elt = e$('.stats dl[city]', popupElt.content)) {
            if ( parseInt(elt.attr('city')) !== StartCity ) {
                elt.hide();
            }
            else {
                elt.show();
    ...
    function autoModeMessage(message, hasAjax, hideStopButton) {
        if (hasAjax === true) {
            ...
            $('#status_log').html(message);
        }
        if (hideStopButton === true)
            $('#stop_button', '#status_panel').hide();
        else
            $('#stop_button', '#status_panel').show();
    }
    ...
    function travelTo(toCity, callback) {
        ...
        autoModeMessage('Traveling to ' + global.cities[toCity] + '...', true);        
    ...
    function healPlayer(callback) {
        ...
        $('#massacre_health_sign').show();
        
        function Heal(url) {
            autoModeMessage('Healing at ' + global.cities[city] + '...', true);
    ...
    function manualAttack(url) {
        ...
        httpAjaxRequest({
            ...
            success: function(htmlText)
            {
                ...
                switch( parseAttackResponse(htmlText) ) {
                    ...
                    case ERROR_NO_FIGHT_RESULT:
                        if (fightStats.health < options.get('healWhen')) {
                            $('#attack_again_button, #power_attack_button').hide();
                            if (options.get('useHeal')) {
                                healPlayer(function() {
                                    $('#attack_again_button').show();
                                    showDiv('buttons', '_panel', true); 
                                });
                                return;
                            }
                    ...
                    case ERROR_SUCCESS:
                        checkBankMoney(function() { showDiv('buttons', '_panel', true); });
                        break;
                }
            }
        });
    ...
    function resetTimeout(timeName) {
        var nTime = (new Date()).getTime();
        fightTimeouts['max'+timeName] = nTime + (options.get(timeName+'out') * 60 * 1000);
        return true;
    }
    function checkCityTimeout() {
        var nTime = (new Date()).getTime();
        if (options.get('fightCityTime') && options.get('fightCityTimeout') > 0) {
            if (nTime > fightTimeouts['maxfightCityTime']) {
                resetTimeout('fightCityTime');
                setNewCurrentCity();
                return true;
            }
    ...
    function checkFightTimeout() {
        var nTime = (new Date()).getTime();
        if (options.get('fightTime') && options.get('fightTimeout') > 0) {
            if (nTime > fightTimeouts['maxfightTime']) {
                statusTimer.start('Battlefield is paused, continue in %N% seconds.',
                    options.get('fightTimeoutResume')*60, function() {
                        fightTimeouts.maxfightCityTime += options.get('fightTimeoutResume')*60*1000;
                        resetTimeout('fightTime');
                        updateNewOpponent();
                        reqSurvey();
                    });
    ...
    function autoAttackNew() {
        ...
        if (!Util.isSet(List.current)) {
            autoRefreshList();
            return;
        }
        if (List.opponentListed('blackList', List.current.id)) {
            log$('skipping blacklisted id: ' + List.current.id);
            autoAttackNew();
            return;
        }
        if (!isValidFaction(List.current.faction)) {
            log$('skipping "'+List.current.name+'" faction: '+List.current.faction);
            autoAttackNew();
            return;
        }
        if (!checkFightTimeout()) {
    ...
    function autoRefreshList() {
        ...
        refreshPlayerList(function() {
            if (List.length() > 0) {
                ...
                setTimeout(autoRefreshList, 5000);
            }
        });
    }
    function autoAttack() {
        var op = options.get, fs = fightStats;
        var reDelay = parseInt(op('resumeDelay')) > 0 ? parseInt(op('resumeDelay')) * 60 : 300;
        ...
        try {
            ...
            if (fs.stamina < staToSpend() || (op('stopWhenStamina') && fs.stamina < op('staminaToKeep'))) {
                checkBankMoney();
                if (op('keepFigthing')) {
                    statusTimer.start('No stamina left, continue in %N% seconds.',reDelay,reqSurvey);
                    $('#stop_button', '#status_panel').show();
                }
                else {
                    autoModeMessage('Auto-Attack deactivated, No stamina left.');
                    abort_process = true;
                }
                return;
            }
            if (fs.health < op('healWhen')) {
                if (!op('useHeal')) {
                    checkBankMoney();
                    if (op('keepFigthing')) {
                        statusTimer.start('Healing is set off, continue in %N% seconds.',reDelay,reqSurvey);
                        $('#stop_button', '#status_panel').show();
                    }
                    else {
                        autoModeMessage('Auto-Attack deactivated, Healing is set off.');
                        abort_process = true;
                    }
                }
                else {
            ...
            var url = List.current.attack; 
            var counter = op('attackCounter') - List.current.fights;
            
            if (op('useAttackCount') && counter < 1) {
                log$('skiping '+List.current.name+' - due max. attacks, count: '+List.current.fights);
                checkBankMoney(autoAttackNew);
                return;
            }
            if (op('usePowerAttack') && List.current.pwratk) {
                if (counter > 4 || !op('useAttackCount')) url = List.current.pwratk;
            }
            if (!/xw_controller=fight/i.test(url)) {
                autoAttackNew();
                return; 
            }
            autoModeMessage('Attacking ' + List.current.anchor + '...', true);
            ...
            httpAjaxRequest({
                ...
                success: function(htmlText) 
                {
                    ...
                    if (List.length() < 6 && !bAttackWhiteList) {
                        parseOpponentTable(htmlText);
                    }
                    switch( parseAttackResponse(htmlText, true) ) {
                        case ERROR_BAD_RESPONSE:
                            statusTimer.start('Some error in server response, try again in %N% seconds.',15, autoAttack);
                            break;
                        ...
                        case ERROR_SUCCESS:
                            if (List.current.alive && List.current.win) {
    ...
    function parseAttackResponse(htmlText, autoMode) {
        ...
        if (!Regex.fight_results(htmlText)) {
            return ERROR_NO_FIGHT_RESULT;
        }
        
        obj = new queryResult(htmlText);
        log$('CASH: '+obj.cash.text());
        var rgx, s_dialog, s_outText = '';
        var opponent = List.current;
        var n_cash = Util.parseNum(obj.cash.text().replace(',',''));
        var n_done = Util.parseNum(obj.damage.text());
        var n_take = Util.parseNum(obj.health.text());
        var n_expe = Util.parseNum(obj.experience.text());
        var n_won = 0, n_lost = 0;
        var s_cashfrom   = String(obj.cash.attr('class')).replace(/sexy_|_cash|good|bad|\s+/g, '');
        var b_goodCash   = (s_cashfrom  === cityIds[StartCity]);
        var n_grpAttack  = Util.parseNum(obj.player.groupAttack);
        var n_grpDefense = Util.parseNum(obj.player.groupDefense);

        function addStashCount(html, count) {
        ...
        function addIcedCount(html, count) {
        ...
        function addCoinsCount(html, count) {
        ...
        function addIcedToLog(script) {
            var sHtml = '', bHtml = '';
            var n_count = Regex.icedCount(script);
            ...
            sHtml += addIcedCount(script, fightStats.iced);
            sHtml += bHtml + '</div>';
            ...
            .text('#'+n_count+'  "' + opponent.name + '"  level '+opponent.level + ':          ')
            .append(c$('br'))
            .append(c$('a', 'target:_black').attr('href',opponent.profile).text(opponent.profile));
        ...
        function addCoinsToLog() {
            try {
                ...
                if (id.length > 2 && e$('#' + id) == null) 
                {
                    ...
                    genLogDom(addCoinsCount(sHtml, fightStats.coins), 'asn', icon);
                    fightStats.coins++;
                    
                    //autoPublish(sHtml, Util.doRgx(/onclick="postW2Wback\('(\d+)'/, sHtml).$1);
                }
                else {
                    $('#' + id).append(addCoinsCount(sHtml, fightStats.coins));
                }
            } catch (err) {
                logErr$(err);
            }
        ...
        function addKillToLog(count) {
            ...
            c$('li').prependTo('#kill_logs').css('margin-top', 10)
            .text('#'+count+'  "' + opponent.name + '"   level '+opponent.level + ':          ')
            .append(c$('br'))
            .append(c$('a', 'target:_black').attr('href',opponent.profile).text(opponent.profile));  
        }
        
        if (StartCity === 4) {
            fightStats.yakuza = Regex.factionProgress(obj.yakuza);
            fightStats.triad  = Regex.factionProgress(obj.triad);
        }
        
        // update opponent image and mafia
        $('#fight_side_right li > img', '#fight_div').attr('src', obj.opponent.image.attr('src'));
        $('#fight_side_right .mafia_size', '#fight_div').text(obj.opponent.groupSize);
        $('#fight_side_right .mafia_defense', '#fight_div').text(obj.opponent.groupDefense);
        ...
        // first run get my name and image
        if (bfirstRun) {
            bfirstRun = false;
            $('#fight_side_left li > img', '#fight_div').attr('src', obj.player.image.attr('src'));  
            $('#fight_side_left .leader_name', '#fight_div').text(obj.player.name);
        }
        $('#fight_side_left .mafia_attack', '#fight_div').text(obj.player.groupAttack);

        if (obj.hint.length > 1)
        {
            s_dialog = 'POWERATTACK';
            
            n_won  = Util.parseNum(obj.hint.eq(0).text());
            n_lost = Util.parseNum(obj.hint.eq(1).text());
            
            s_outText = 'You fought '+ (n_lost + n_won) +' times. '
        ...
        {
            var nKillCount = 0;
            s_dialog = Util.rgbToHex(obj.hint.css('color'));
            
            if (s_dialog == '#ffffff' && (nKillCount = Util.parseNum(obj.hint.text())) > 0) {
        ...
        fightStats.staminaSpend += (n_lost + n_won) * staToSpend();
        
        if (b_goodCash) {
            fightStats.cashWon[CurrentCity] += (obj.cash.hasClass('good') ? n_cash : n_cash * -1);
        }
        ...
        {
            opponent.alive = true;
            if (options.get('usePowerAttack') && obj.won) 
                $('#power_attack_button').show();
        }
        if (opponent.attack) 
        {
            opponent.alive = true;
        ...
        {
            opponent.win = true;
            $('#fight_side_left', '#fight_div').addClass('winning');
            $('#fight_side_left h3', '#fight_div').text('WON');
            $('#fight_side_right', '#fight_div').addClass('losing');
            $('#fight_side_right h3', '#fight_div').text('LOST');
            
            // add enemy to cash list
            if (b_goodCash && obj.cash.hasClass('good') && options.get('addUserToWList')) {
                if (n_cash >= options.get('cashNeedToGain')) 
                {
                    if (List.addCurrentToWhiteList()) {
                        fightStats.whitelisted++;
                        if (options.get('showWhiteListed'))
                            genLogDom( 
                                '"The bank" ' + List.current.anchor + 
            ...
            // opponent dead
            if (!opponent.alive) {
                $('#fight_side_right .war_x', '#fight_div').show();
            }
            else if (autoMode) {
        ...
        {
            opponent.win = false;
            $('#fight_side_left', '#fight_div').addClass('losing');
            $('#fight_side_left h3', '#fight_div').text('LOST');
            $('#fight_side_right', '#fight_div').addClass('winning');
            $('#fight_side_right h3', '#fight_div').text('WON');
            
            // add enemy to blacklist
            if (options.get('useBlacklist')) {
                if (List.addCurrentToBlackList()) {
                    fightStats.blacklisted++;
                    if (options.get('showBlackListed'))
                        genLogDom(
                            '"The beast" ' + List.current.anchor + 
        ...
        // VICTORY POINTS
        if (obj.vCoin.length) {
            fightStats.victorycoins += Util.parseNum(obj.vCoin.text());
        }
        
        if (options.get('showcoins') && obj.share_asn.length > 0) {
            addCoinsToLog();
        }
        ...
        $('.fightres_bonus', obj.bonuses).each(function(index, element)
        {
            ...
            if (s_bonus == 'stash') {
                genLogDom(addStashCount(elt.html(), fightStats.stash), s_bonus,  imgUrl);
                fightStats.stash++;
            }
            else {
        ...
        if (!autoMode || (options.get('useAttackDelay') && options.get('attackDelay') >= 1)) {
            if (options.get('showDialogs')) showDialog(s_dialog, obj.won);
        }
        
        List.current = opponent;
    ...
    function Initialize() {
        ...
        $('.leaders_rewards', '#fight_div').children().hide();
        showDiv('status', '_panel');
        showDiv('opponents', '_list');
        showDiv('items', '_logs');
        ...
        function finish() {
            ...
            if ($('#inner_page').attr('class') == 'fight_controller') {
                setTimeout(function(){
                    parseOpponentTable($('#inner_page').html());
                    if (List.length() > 0) {
                        genEnemyListDom();
                    }
                    showDiv('start', '_panel', true);
    ...
    popupElt.addBase64Style(
        ...
        'DQojYmF0dGxlZmllbGRfcG9wdXAgLmJhdHRsZWdyb3VuZCB7DQoJYmFja2dyb3VuZDogYmxhY2sgdXJsKGh0dHA6Ly9td2ZiLnN0'+
        'YXRpYy56eW5nYS5jb20vbXdmYi9ncmFwaGljcy9zb2NpYWxtaXNzaW9ucy9iZ190ZXh0dXJlX21vZHVsZS5qcGcpIG5vLXJlcGVh'+
        'dCBzY3JvbGwgNTAlIDAlOw0KCWJvcmRlci1ib3R0b206IDFweCBzb2xpZCAjM0MzQzNDOw0KCWhlaWdodDogMjMwcHg7DQoJd2lk'+
        'dGg6IGF1dG87DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvc3NmaWdodF9oZWFsdGhfYmFyIHsNCglmbG9hdDogbGVmdDsNCn0N'+
        'CiNiYXR0bGVmaWVsZF9wb3B1cCAucmlnaHQgew0KCWZsb2F0OiByaWdodDsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuaGVhbHRo'+
        'X3NpZ24gew0KCWZsb2F0OiByaWdodDsNCglmb250LXdlaWdodDogYm9sZDsNCgljb2xvcjogZ3JlZW47DQoJdGV4dC1kZWNvcmF0'+
        'aW9uOiBibGluazsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuc3RhdF92aWN0b3J5IHsNCgliYWNrZ3JvdW5kOiB1cmwoaHR0cDov'+
        'L213ZmIuc3RhdGljLnp5bmdhLmNvbS9td2ZiL2dyYXBoaWNzL3ZpY3RvcnlfaWNvbi5naWYpIG5vLXJlcGVhdCAtMnB4IDUwJTsN'+
        'CglwYWRkaW5nLWxlZnQ6IDE5cHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLnN0YXRfaWNlZCB7DQoJYmFja2dyb3VuZDogdXJs'+
        'KGh0dHA6Ly9td2ZiLnN0YXRpYy56Z25jZG4uY29tL213ZmIvZ3JhcGhpY3MvbWFwX2Jhc2VkX2pvYnMvbWFzdGVyeV9zdGFyc19t'+
        'ZWRfODF4MzBfMDIucG5nKSBuby1yZXBlYXQgLTY0cHggLTEzcHg7DQoJcGFkZGluZy1sZWZ0OiAxOXB4Ow0KfQ0KI2JhdHRsZWZp'+
        'ZWxkX3BvcHVwIC5zdGF0X2tpbGwgew0KCWJhY2tncm91bmQ6IHVybChodHRwOi8vbXdmYi5zdGF0aWMuemduY2RuLmNvbS9td2Zi'+
        'L2dyYXBoaWNzL21hcF9iYXNlZF9qb2JzL21hc3Rlcnlfc3RhcnNfbWVkXzgxeDMwXzAyLnBuZykgbm8tcmVwZWF0IC00N3B4IDJw'+
        'eDsNCglwYWRkaW5nLWxlZnQ6IDE5cHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmxpc3RlZCB7DQoJYmFja2dyb3VuZDogdHJh'+
        'bnNwYXJlbnQgdXJsKGh0dHA6Ly9td2ZiLnN0YXRpYy56Z25jZG4uY29tL213ZmIvZ3JhcGhpY3MvdjMvaWNvbl93aXNobGlzdF9h'+
        'ZGRfMTl4MTRfMDEuZ2lmKSBuby1yZXBlYXQgLTFweCA1MCU7DQoJcGFkZGluZy1sZWZ0OiAxOXB4Ow0KfQ0KI2JhdHRsZWZpZWxk'+
        'X3BvcHVwICNkZWNsYXJlX3dhciB7DQoJYmFja2dyb3VuZDogbm9uZTsNCgltYXJnaW46IDBweCA0NHB4Ow0KfQ0KI2JhdHRsZWZp'+
        'ZWxkX3BvcHVwICNkZWNsYXJlX3dhciAud2FyX3Jld2FyZHMgew0KCXRvcDogMjBweDsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAj'+
        'ZGVjbGFyZV93YXIgLndhcl9yZXdhcmRzICNsZWZ0X3NpZGVfdGV4dCB7DQoJaGVpZ2h0OiAzNXB4Ow0KCXBhZGRpbmc6IDBweCAx'+
        'NHB4Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwICNkZWNsYXJlX3dhciAud2FyX3Jld2FyZHMgI3JpZ2h0X3NpZGVfdGV4dCB7DQoJ'+
        'cGFkZGluZzogMHB4IDE0cHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgI2RlY2xhcmVfd2FyIC53YXJfcmV3YXJkcyAubGVmdCB7'+
        'DQoJdG9wOiAyMHB4Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwICNkZWNsYXJlX3dhciAud2FyX3Jld2FyZHMgLnJpZ2h0IHsNCgl0'+
        'b3A6IDcwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgI2RlY2xhcmVfd2FyIC53YXJfc3RhdHVzIHsNCgloZWlnaHQ6IDIzMHB4'+
        'Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwICNkZWNsYXJlX3dhciAud2FyX3N0YXR1cyAuc2lkZSB7DQoJYmFja2dyb3VuZC1wb3Np'+
        'dGlvbjogMHB4IDU1cHg7DQoJbWluLWhlaWdodDogMjMwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgI2ZpZ2h0T3B0IHsNCglt'+
        'YXJnaW46IDBweDsNCgl3aWR0aDogYXV0bzsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAjZmlnaHRPcHQgdWwgew0KCWxpc3Qtc3R5'+
        'bGUtdHlwZTogbm9uZTsNCgloZWlnaHQ6IDEwMCU7DQoJd2lkdGg6IDEwMCU7DQoJbWFyZ2luOiAxMHB4IDBweCAwcHg7DQoJcGFk'+
        'ZGluZzogMHB4Ow0KCXRleHQtYWxpZ246IGxlZnQ7DQoJb3ZlcmZsb3c6IGF1dG87DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgI2Zp'+
        'Z2h0T3B0IHVsIGxpIHsNCglkaXNwbGF5OiBibG9jazsNCglmbG9hdDogbGVmdDsNCgltYXJnaW46IDBweCAwcHggMHB4IDVweDsN'+
        'CglwYWRkaW5nOiAwcHg7DQoJaGVpZ2h0OiAyNXB4Ow0KCW1heC1oZWlnaHQ6IDI1cHg7DQoJd2lkdGg6IDQ4JTsNCglvdmVyZmxv'+
        'dzogaGlkZGVuOw0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5wYW5lbHMgew0KCWJhY2tncm91bmQtY29sb3I6ICMxMTE7DQoJYm9y'+
        'ZGVyOiAxcHggc29saWQgIzNDM0MzQzsNCgloZWlnaHQ6IDMwcHg7DQoJcGFkZGluZzogMTVweDsNCgl3aWR0aDogYXV0bzsNCn0N'+
        'CiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIHsNCgliYWNrZ3JvdW5kLWNvbG9yOiBibGFjazsNCgliYWNrZ3JvdW5kLWltYWdl'+
        'OiB1cmwoaHR0cDovL213ZmIuc3RhdGljLnp5bmdhLmNvbS9td2ZiL2dyYXBoaWNzL2l0YWx5X2Q0LmpwZyk7DQoJYmFja2dyb3Vu'+
        'ZC1wb3NpdGlvbjogNTAlIDAlOw0KCWJhY2tncm91bmQtcmVwZWF0OiBuby1yZXBlYXQ7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAg'+
        'LmJvdHRvbSAuc3RhdHMgew0KCWJvcmRlci1sZWZ0OiAxcHggc29saWQgIzNDM0MzQzsNCglmbG9hdDogcmlnaHQ7DQoJaGVpZ2h0'+
        'OiA5NyU7DQoJcGFkZGluZzogNXB4Ow0KCXRleHQtYWxpZ246IGxlZnQ7DQoJd2lkdGg6IDI4JTsNCn0NCiNiYXR0bGVmaWVsZF9w'+
        'b3B1cCAuYm90dG9tIC5zdGF0cyBkbCB7DQoJaGVpZ2h0OiAyMHB4Ow0KCW1hcmdpbjogMHB4Ow0KfQ0KI2JhdHRsZWZpZWxkX3Bv'+
        'cHVwIC5ib3R0b20gLnN0YXRzIGRsIGltZyB7DQoJZmxvYXQ6IGxlZnQ7DQoJaGVpZ2h0OiAxOXB4Ow0KCXdpZHRoOiAxOXB4Ow0K'+
        'CW1hcmdpbi1yaWdodDogMnB4Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5ib3R0b20gLnN0YXRzIGRsIGR0IHsNCglmbG9hdDog'+
        'bGVmdDsNCgltYXJnaW46IDBweDsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5zdGF0cyBkbCBkZCB7DQoJZmxvYXQ6'+
        'IHJpZ2h0Ow0KCW1hcmdpbjogMHB4Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5ib3R0b20gLnN0YXRzIC5udW1iZXJzIHsNCglm'+
        'bG9hdDogcmlnaHQ7DQoJZm9udC13ZWlnaHQ6IGJvbGQ7DQoJbWFyZ2luOiAwcHg7DQoJb3ZlcmZsb3c6IGhpZGRlbjsNCglwYWRk'+
        'aW5nOiAwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCB7DQoJZmxvYXQ6IGxlZnQ7DQoJaGVpZ2h0OiA5'+
        'OSU7DQoJd2lkdGg6IDcwJTsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNldmVudHNfbGlzdCB7DQoJaGVp'+
        'Z2h0OiBhdXRvOw0KCW92ZXJmbG93OiBoaWRkZW47DQoJdGV4dC1hbGlnbjogbGVmdDsNCgloZWlnaHQ6IDEwMCU7DQp9DQojYmF0'+
        'dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjZXZlbnRzX2xpc3QgI2ZpZ2h0bG9ncyB7DQoJYm9yZGVyOiAwcHg7DQp9DQoj'+
        'YmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjZXZlbnRzX2xpc3QgLmJ1dHRvbnMgew0KCWJvcmRlci1ib3R0b206IDFw'+
        'eCBzb2xpZCAjMzMzOw0KCWJvcmRlci10b3A6IDFweCBzb2xpZCAjMzMzOw0KCW1hcmdpbi1ib3R0b206IDVweDsNCgltYXJnaW4t'+
        'bGVmdDogMTBweDsNCn0JDQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjZXZlbnRzX2xpc3QgLmJ1dHRvbnMgYSB7'+
        'DQoJYm9yZGVyLWxlZnQ6IDFweCBzb2xpZCAjMzMzOw0KCWJvcmRlci1yaWdodDogMXB4IHNvbGlkICMzMzM7DQoJcGFkZGluZy1s'+
        'ZWZ0OiAxMHB4Ow0KCXBhZGRpbmctcmlnaHQ6IDEwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjZXZl'+
        'bnRzX2xpc3QgLmJ1dHRvbnMgYTpmaXJzdC1jaGlsZCB7DQoJbWFyZ2luLWxlZnQ6IDIwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9w'+
        'dXAgLmJvdHRvbSAubGlzdCAjZXZlbnRzX2xpc3QgdWwgew0KCWxpc3Qtc3R5bGUtdHlwZTogbm9uZTsNCgltYXJnaW46IDBweDsN'+
        'CglvdmVyZmxvdzogYXV0bzsNCglwYWRkaW5nOiAwcHg7DQoJdGV4dC1hbGlnbjogbGVmdDsNCgl3aWR0aDogMTAwJTsNCn0NCiNi'+
        'YXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNldmVudHNfbGlzdCB1bCBsaSB7DQoJbWFyZ2luOiAxMHB4IDBweCAwcHgg'+
        'MjBweDsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNldmVudHNfbGlzdCBkaXYjaXRlbXNfbG9ncyB7DQoJ'+
        'aGVpZ2h0OiA3NSU7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjb3Bwb25lbnRzX2xpc3Qgew0KCWhlaWdo'+
        'dDogMTAwJTsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNvcHBvbmVudHNfbGlzdCAuaGVhZGVyIHsNCglo'+
        'ZWlnaHQ6IDUlOw0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5ib3R0b20gLmxpc3QgI29wcG9uZW50c19saXN0IC5oZWFkZXIgYSB7'+
        'DQoJZmxvYXQ6IGxlZnQ7DQoJbWFyZ2luLWxlZnQ6IDIwcHg7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAj'+
        'b3Bwb25lbnRzX2xpc3QgI29wcG9uZW50c190YWJsZSB7DQoJaGVpZ2h0OiA5NSU7DQp9DQojYmF0dGxlZmllbGRfcG9wdXAgLmJv'+
        'dHRvbSAubGlzdCAucGxheWVyX3VwZGF0ZXMgew0KICAgIG92ZXJmbG93LXg6IGhpZGRlbjsNCiAgICB0ZXh0LWFsaWduOiBsZWZ0'+
        'Ow0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5ib3R0b20gLmxpc3QgLnVwZGF0ZV90eHQgew0KCXdpZHRoOiAzNjBweDsNCglmb250'+
        'LWZhbWlseTogdHJlYnVjaGV0IE1TOw0KCWZvbnQtd2VpZ2h0OiBib2xkOw0KCWNvbG9yOiB3aGl0ZTsNCgljbGVhcjogbm9uZTsN'+
        'Cn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNpdGVtc19sb2dzIC5sb290IHsNCgliYWNrZ3JvdW5kOiBibGFj'+
        'ayB1cmwoaHR0cDovL213ZmIuc3RhdGljLnp5bmdhLmNvbS9td2ZiL2dyYXBoaWNzL2ZpZ2h0X2JvbnVzZXMvc2lkZV9ib251c2xv'+
        'b3QucG5nKSBuby1yZXBlYXQgc2Nyb2xsIDEwMCUgNTAlOw0KfQ0KI2JhdHRsZWZpZWxkX3BvcHVwIC5ib3R0b20gLmxpc3QgI2l0'+
        'ZW1zX2xvZ3MgLnN0YXNoIHsNCgliYWNrZ3JvdW5kOiBibGFjayB1cmwoaHR0cDovL213ZmIuc3RhdGljLnp5bmdhLmNvbS9td2Zi'+
        'L2dyYXBoaWNzL2ZpZ2h0X2JvbnVzZXMvc2lkZV9zZWNyZXRzdGFzaC5wbmcpIG5vLXJlcGVhdCBzY3JvbGwgMTAwJSA1MCU7DQp9'+
        'DQojYmF0dGxlZmllbGRfcG9wdXAgLmJvdHRvbSAubGlzdCAjaXRlbXNfbG9ncyAuc3Rhc2ggLnVwZGF0ZV90eHQgew0KCWJhY2tn'+
        'cm91bmQtY29sb3I6IGJsYWNrOw0KCW9wYWNpdHk6IDAuOTsNCn0NCiNiYXR0bGVmaWVsZF9wb3B1cCAuYm90dG9tIC5saXN0ICNp'+
        'dGVtc19sb2dzIC5sb290IC51cGRhdGVfdHh0IHsNCgliYWNrZ3JvdW5kLWNvbG9yOiBibGFjazsNCglvcGFjaXR5OiAwLjk7DQp9'
    );
    
    // load options and Initialize
    options.load(function() {
        if ( !(_sList && _sHtmlText) ) {
            Initialize();
            return;
        }    
        if (addPlayerToWhiteList(_sHtmlText, _sList)) {    
            if (Util.isFunc(_callback)) {
                _callback('The user has been added to ' + _sList + '.');
                popupElt.close();
                return false;
            }
            showHelpPopup({
                icon: 'info',
                title: 'Player added!',
                message: 'The user has been added to ' + _sList + '.',
                buttons: [{
                    label: 'Open Battlefield!',
                    addClass: 'short white',
                    onclick: Initialize
                }, {
                    label: 'Close',
                    onclick: popupElt.close
                }] 
            });
        }
        else {
...
function PluginManager() {
    ...
    var Events = {
        ...
        saveApp_click: function() {
            ...
            options.save(function() {$.each(options.get('all'), addNewApp);selected.click();});
        ...
        removeApp_click: function() {
            ...
            $('#app_list',popupElt.content).empty();
            $('#app_code, #app_name').val('');
            options.save(function() {$.each(options.get('all'), addNewApp);});
    ...
    function addNewApp(index, app) {
        c$('li', 'app_id:'+index).prependTo($('#app_list',popupElt.content))
    ...
    function genMainDom() {
        ...
        c$('div', 'class:frame_box').height(350)
        .append(c$('ul', 'app_list').css('float','left').width(240))
        .append(c$('div','info_panel').css('float','right').width(460))
        .appendTo(popupElt.content);
        
        $.each(options.get('all'), addNewApp);
        
        $('#info_panel').append(c$('p').css('margin-bottom',5).text('Edit selected Plug-In:'))
        ...
        .append(c$('textarea', 'id:app_code,class:black').width(380).height(250))
        .append('<br />')
        .append(c$('center')
            .append(b$('Save Changes', 'class:short green').click(Events.saveApp_click))
            .append(b$('Remove App', 'class:short red').click(Events.removeApp_click)
                .css('margin-left',5))
        );
    }
    ...
    popupElt.addBase64Style(
        ...
        'OiAyNnB4Ow0KCXBhZGRpbmc6IDVweDsNCgl0ZXh0LWFsaWduOiBsZWZ0Ow0KfQ0KI3BsdWdpbm1hbmFnZXJfcG9wdXAgdWwgew0K'+
        'CWJvcmRlcjogMXB4IHNvbGlkICM5OTk7DQoJaGVpZ2h0OiAzNTBweDsNCglsaXN0LXN0eWxlLXR5cGU6IG5vbmU7DQoJbWFyZ2lu'+
        'OiAwcHg7DQoJb3ZlcmZsb3cteDogaGlkZGVuOw0KCW92ZXJmbG93LXk6IGF1dG87DQoJcGFkZGluZzogMHB4Ow0KfQ0KI3BsdWdp'+
        'bm1hbmFnZXJfcG9wdXAgdWwgbGkgew0KCWJvcmRlcjogMXB4IHNvbGlkICMzMzM7DQoJY3Vyc29yOiBwb2ludGVyOw0KCWhlaWdo'+
        'dDogMjJweDsNCgltYXJnaW46IDFweDsNCglvdmVyZmxvdzogaGlkZGVuOw0KCXBhZGRpbmc6IDFweDsNCn0NCiNwbHVnaW5tYW5h'+
        'Z2VyX3BvcHVwIHVsIGxpLnNlbGVjdGVkIHsNCglib3JkZXI6IDFweCBzb2xpZCAjQ0MwOw0KfQ=='
    );
