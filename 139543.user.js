// ==UserScript==
// @author          blackca [türkçeleştiren torlax]
// @name            eRepublik Sağlık Limiti Hesaplayıcı
...
var HLTooptipInsert = function($, window, undefined) {
	$(document).ready(function () {
		...
		if (foodNow!=foodMax&&0==$('p#foodLimitInfo').length) {
			...
			var nextMin=now.getMinutes()+timeMin, nextSec=now.getSeconds()+timeSec;
			59<nextSec&&(nextSec-=60,nextMin+=1); 59<nextMin&&(nextMin-=60,nextHour+=1); 23<nextHour&&(nextHour-=24);
			10>nextSec&&(nextSec='0'+nextSec); 10>nextMin&&(nextMin='0'+nextMin); 10>nextHour&&(nextHour='0'+nextHour);
			$('#eatFoodTooltip p[style="margin:0"]').append('<p id="foodLimitInfo">sağlık limitinin hazır olacağı saat: <big>'+nextHour+':'+nextMin+':'+nextSec+'</big> (+'+nextFood+')</p>')
		}
	})
}
// Script Insert
