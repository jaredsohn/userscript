...
WICHTIGE HINWEISE:
* Diese Version sendet nur Spios, Speerträger und Schwertkämpfer.
...
* Jeder Angriff erfolgt mit 1 Spaeher, etwa 200 Speertraegern und etwas 200 
  Schwertkämpfern. Durch Klicken auf den Link "Einstellen" neben "Autofarm 
  Starten" können diese Zahlen verändert werden. Die angegebene Anzahl an
  Deffeinheiten ist nur ein Idealwert. Der Algorithmus weicht davon ggf. um
  Faktor 0,5 bis 1,75 ab, um möglichst alle Einheiten zu beschäftigen.
...
var spysPerAttack = parseInt(GM_getValue("autfarm2-spys", 1));
var spearPerAttack = parseInt(GM_getValue("autfarm-spear", 200));
var swordPerAttack = parseInt(GM_getValue("autfarm-sword", 200));
if (document.cookie.indexOf("autofarm_deff=setup") >= 0) {
	document.cookie = 'autofarm_deff=off';
	document.cookie = 'autofarm_run=0';
	var x = prompt("Anzahl Spaeher pro Angriff:", spysPerAttack);
	if (x) {
		var y = prompt("Ideale Anzahl Speerträger pro Angriff:", spearPerAttack);
		var z = prompt("Ideale Anzahl Swertkämpfer pro Angriff:", swordPerAttack);
		if (y && z) {
			x = parseInt(x); if (x < 0) x = 0;
			y = parseInt(y); if (y < 0) y = 0;
			z = parseInt(z); if (z < 0) z = 0;
			if (!x && !y && !z) x = 1;
			GM_setValue("autfarm2-spys", x);
			GM_setValue("autfarm-spear", y);
			GM_setValue("autfarm-sword", z);
			location.reload();
		}
	}
}
...
if (document.cookie.indexOf("autofarm_deff=on") >= 0 &&
	(document.cookie.indexOf("autofarm_run=0") >= 0 || document.cookie.indexOf("autofarm_run=deff") >= 0)) {
	autofarm_on = true;
	document.cookie = 'autofarm_run=deff';
}
...
if (
	...
	newElement.innerHTML =
		'<A HREF="javascript:document.cookie=\'autofarm_deff=on\';' +
		'document.cookie=\'autofarm_run=0\';'+
		'location.href=\'staemme.php?village=' +
		village_id +
		'&screen=overview_villages&mode=combined\';"' +
		'><FONT color=#00ff00>Autofarm Starten (Deff)</FONT></A>&nbsp;&nbsp;' +
		'<A HREF="javascript:document.cookie=\'autofarm_deff=setup\';' +
	...
	document.cookie.indexOf("autofarm_run=deff") >= 0
) {
	var newElement = document.createElement('p');
	newElement.innerHTML =
		'<A HREF="javascript:document.cookie=\'autofarm_deff=off\';' +
		'location.reload()"><FONT color=#ff0000>Autofarm Stoppen (Deff)</FONT></A>'
	;
	table_cb.parentNode.insertBefore(newElement, table_cb);
	if (
		...
		if (dtbl != null) {
			...
			for (var i = 0; i < ths.length; i++) {
				...
				if (ths[i].innerHTML.indexOf('unit_spear.png') >= 0) {
					spear_idx = i;
				}
				if (ths[i].innerHTML.indexOf('unit_sword.png') >= 0) {
					sword_idx = i;
				}
			}
			if (spys_idx >= 0 && spear_idx >= 0 && sword_idx >= 0) {
				for (var i = 1; i < trs.length; i++) {
					var vllg = trs[i].getElementsByTagName('span')[0].id.substr(6);
					var tds = trs[i].getElementsByTagName('td');
					var spys = parseInt(tds[spys_idx].innerHTML);
					var spear = parseInt(tds[spear_idx].innerHTML);
					var sword = parseInt(tds[sword_idx].innerHTML);
					if (spys >= spysPerAttack && spear >= spearPerAttack / 2 && sword >= swordPerAttack / 2) {
						window.setInterval(
		...
		for (var i = 0; i < as.length; i++) {
			...
			var y = /insertUnit\(document\.forms\[0\]\.spear,%20(\d+)\)/.exec(as[i].href);
			var z = /insertUnit\(document\.forms\[0\]\.sword,%20(\d+)\)/.exec(as[i].href);
			if (x && x.length == 2) spys = parseInt(x[1]);
			if (y && y.length == 2) spear = parseInt(y[1]);
			if (z && z.length == 2) sword = parseInt(z[1]);
		}
		if (spys >= spysPerAttack && spear >= spearPerAttack / 2 && sword >= swordPerAttack / 2) {
			if (
				...
				var spearToSend = spearPerAttack;
				if (spear <= (1.5 + 0.25 * Math.random()) * spearPerAttack) {
					spearToSend = spear;
				}
				var swordToSend = swordPerAttack;
				if (sword <= (1.5 + 0.25 * Math.random()) * swordPerAttack) {
					swordToSend = sword;
				}
				var url =
					"javascript:" +
					"insertUnit(document.forms[0].spy," + spysPerAttack + ");" +
					"insertUnit(document.forms[0].spear," + spearToSend + ");" +
					"insertUnit(document.forms[0].sword," + swordToSend + ");" +
