...
// Version 1.1
// 2009-09-08
// 2009 by Chris Welfare
// File naming/org structure of CS image storage completely changed,
// updated original code by M.Schwarz to bring into line with this
// Version 1.0
// 2008-11-24
...
// @description    Displays the pictures larger when you roll over smaller ones on couchsurfing.org
...
function showPopup(imageId, callId) {
	...
	if (global.countCalls == callId) {
		...
		var srcBegin = src.substr(0,47);
		
		var IOPos = src.indexOf("_t_")

		if (IOPos > 0) {
		var srcleft = src.substr(0,IOPos );
		var srcright = src.substr(IOPos + 3,src.length);

		/*change _l_ to _m_ to use medium sized images instead of large */ 		
		var LImg = (srcleft + "_l_" + srcright);
		}

		if ((srcBegin == 'http://images.couchsurfing.us.s3.amazonaws.com/' && zoom_thumbnail_images )||(srcBegin == 'http://images.couchsurfing.us.s3.amazonaws.com/' && zoom_medium_images)) {
			/* set new image src */
			src = LImg;
...
function loadEventHandlers() {
	...
	for (var i = 0; i < images.length; i++) {
		...
		if (src &&		( src.substr(0,29) == 'http://images.couchsurfing.us')		/* all CS pics*/
					) {
