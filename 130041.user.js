...
// @namespace	DSLife
// @description	Das Skript erweitert den Marktplatz um neue Funktionen und gibt Vorschlaege zum Speicherausgleich, benoetigt Version 8.0
// @version		3.0
// @author		knish3r
// @copyright	2009 - 2012, T. Fiesinger
...
var gui = {
	...
	de: {
		...
		dssortgroups: "Funktion \"DSSortGroups\":",
		...
		popupDefinition2: "ausgeblendet werden. Sind beide Dörfer aktiv, werden Dörfer gefiltert, für die eine der Eigenschaften zutrifft.",
		...
		target_attack:  "OK",
		lastVillage: "&raquo; Letztes",
		lastVillageLink: "Funktion \"Letztes Dorf\" im VP:",
		bbcode: "BB Code",
...
var defaultFilterSettings = {minRes: [50000,56000,60000,50000], minResPerCent: [60,56,60,50], maxResPerCent: [90,80,85,75], maxRes: [350000,300000,350000,280000], 
    ...
    fontSize:"0.9em",imgSize:"13px",marginTop:"5px", timeStor: ["10","00","00",0]};
var defaultCheckboxSettings = {filterCheckbox: false, readInCheckbox: false, disableMinRes: false, 
	...
	disableLastVillageLink: false, disableMapRedirTargets: false, uniMinRes: false, uniMaxRes: false, disableHotkeys: false, reportGroups: false,
	...
	selectedGroups: [0,0,0,0,0,0,0,0], disableShowGroupsMain: false, disableShowGroupsMap: false, disableShowGroupsCombined: false, disableDSSortGroups: false, 
	...
	disableConTimeP: false,disableVpDistance: false,}
	...
	var saveOrderVillage = function() {
		if( document.getElementById("menu_row2") != null && !checkboxes.disableSaveOrder) {
			...
			if( this.list != undefined ) {
				...
				this.a.className = "village_switch_link";
				this.div = this.a.appendChild(document.createElement("span"));
				this.div.className = "arrowRight";
				
				/*if( document.getElementById("dsfm_controls") != null ) {
					document.getElementById("dsfm_controls").getElementsByTagName("a")[0].addEventListener("click", function() {
						lib.storage.deleteValue("SaveOrderVillages_player.id"+lib.game_data.player.id);
						if( document.getElementById("dssb_deleteSavedVillageOrder") != null )
							document.getElementById("dssb_deleteSavedVillageOrder").parentNode.parentNode.removeChild( document.getElementById("dssb_deleteSavedVillageOrder").parentNode );
						if( document.getElementById("dssb_jump_link") != null )
							document.getElementById("dssb_jump_link").parentNode.parentNode.removeChild( document.getElementById("dssb_jump_link").parentNode );
						if( document.getElementById("dssb_arrowLeft") != null )
							document.getElementById("dssb_arrowLeft").parentNode.parentNode.removeChild( document.getElementById("dssb_arrowLeft").parentNode );
						if( document.getElementById("dssb_arrowRight") != null )
							document.getElementById("dssb_arrowRight").parentNode.parentNode.removeChild( document.getElementById("dssb_arrowRight").parentNode );
					}, false);
				}*/
			}
		}
	...
	var newShowGroups = function( obj, id, newLine, overview ) {
		if( !checkboxes.disableGroupLinks ) {
			...
			if( !overview ) {
				...
				newGroupsA.href = "javascript:popup_scroll(\"groups.php?&mode=village&village_id="+id+(lib.game_data.player.sitter_id!=0?"&t="+lib.game_data.player.id:"")+"\", 300, 550);"
				...
				if( (location.href.match(/screen=main/) && !checkboxes.disableShowGroupsMain) || (location.href.match(/screen=map/) && !checkboxes.disableShowGroupsMap) ) {
					...
					obj.innerHTML+="<span id='showVillageGroups"+id+"'>- "+(this.groups != ""?this.groups:"<i>"+gui[lib.lang].showGroups.noGroupsSaved+"</i>")+" -</span>";
				} return;
			} else {
				var newGroupsA = obj.appendChild( document.createElement("a") );
				...
				if( this.groups != "" ) {
					if((document.getElementById("combined_table")&&!checkboxes.disableShowGroupsCombined)||(document.getElementById("production_table")&&!checkboxes.disableShowGroupsProduction)) 
						villageGroupsA.parentNode.innerHTML+="<br/><small id='showVillageGroups"+id+"'>- "+(this.groups != ""?this.groups:gui[lib.lang].showGroups.noGroupsSaved)+" -</small>";				
				}
			}
		}
	};
...
if( !checkboxes.disableDSLoyalty ) {
	...
	var updateLoyalty = function(id) {
		if( lib.storage.getValue("groupsAndLoyaltyData_"+id, ",,").split(",")[2] != "" ) {
	        var values = lib.storage.getValue("groupsAndLoyaltyData_"+id, ",,").split(",");
			var newTime = lib.getServerTime().getTime()/1000;
			var difference = Math.round((newTime*1-values[1]*1)/3600);
	        var loyal = parseInt(values[2],10)+parseInt(difference);

			if(loyal > 100) loyal = 100;	
			if( !checkboxes.disableLoyaltyColor ) {
				if(loyal <= 100 && loyal > 70 ) loyal = "<b style='color:green'>" + loyal + "</b>";
				else if(loyal < 100 && loyal > 40 ) loyal = "<b style='color:orange'>" + loyal + "</b>";
				else if(loyal < 100 && loyal <= 40 ) loyal = "<b style='color:red'>" + loyal + "</b>";		
			} else if( loyal < 100 ) loyal = "<b style='color:red'>" + loyal + "</b>";
		} else loyal = "<span class='grey' style='cursor:help;' title='"+gui[lib.lang].dsLoyalty.noLoyaltySaved+"'>???<span>";
		return loyal;
	};
	...
	var appendLoyaltyLine = function() {
		...
		var newTh = table.getElementsByTagName("tr")[0].appendChild(document.createElement("th"));
		var villageHrefTd = 0;
		...
		for(var i=1; i<trLength; i++) {
			if( table.getElementsByTagName("tr")[i].getElementsByTagName("th").length > 0 ) continue;
			var villageId = table.getElementsByTagName("tr")[i].getElementsByTagName("td")[villageHrefTd].getElementsByTagName("a")[0].href.split("village=")[1].split("screen=")[0].replace(/[^0-9]/g,"");
			table.getElementsByTagName("tr")[i].innerHTML += "<td id='DSLoyalty_" + villageId + "' style='text-align:right;'>" + updateLoyalty(villageId) + "</td>";
		}
			...
			if( !checkboxes.disableCombineUnitImgs ) {
				if( cells[recruitCell].getElementsByTagName("img") ) {
					...
					for(var j = 0; j < images.length; j++) {
						...
						var date = images[j].title.split(" - ")[1];
						...
						cells[recruitCell].removeChild(images[j--]);
					}
					cells[recruitCell].innerHTML = "";
					for(var j = 0; j < units.length; j++) {
						var image = document.createElement("img");
						image.src = "http://" + location.host + "/graphic/unit/unit_" + units[j].split(":")[0] + ".png";
						image.title = units[j].split(":")[1]+" "+lib.createTextNode( gui[lib.lang].pieces ).textContent+"- "+unescape(units[j].split(":")[2]);
						cells[recruitCell].appendChild(image);
					}
				}
			}
			...
			var readInVillages = function(node) {
				...
				for(this.i = 1; this.i < this.rows.length; this.i++) {
					...
					if(!this.rows[this.i].innerHTML.match(/id="label_\d+"/) || this.rows[this.i].style.display == "none" || this.rows[this.i].title=="dssb_filtered" )
						continue;
					if( node.id == "dssb_saveOrderLink" ) {
						this.traders = parseInt(this.cells[tradersCell].textContent.split("/")[0].replace(/\.|\s$|^\s/g, ""),10);
						if( (this.traders-(checkboxes.disableOfferBorder ? 0 : parseInt(filter.offerBorder,10))) <= 0 ) continue;
						this.ressis=this.cells[resCell].textContent.replace(/\.|\s$|^\s/g, "").split(" ");
						this.storage = parseInt(this.cells[storageCell].textContent.replace(/\.|\s$|^\s/g, ""),10);
						if( !checkboxes.disableMinRes ) {
							if( !checkboxes.minResPerCent ) {
								if( parseInt(filter.minRes,10) >= this.storage ) continue;
								if( checkboxes.disableMinSendBorder ) filter.minSendBorder = 0;
									if( ((parseInt(this.ressis[0],10)-parseInt(filter.minRes,10)) <= 0 ? 0 : ((parseInt(this.ressis[0],10)-parseInt(filter.minRes,10))))
										+((parseInt(this.ressis[1],10)-parseInt(filter.minRes,10)) <= 0 ? 0 : ((parseInt(this.ressis[1],10)-parseInt(filter.minRes,10))))
										+((parseInt(this.ressis[2],10)-parseInt(filter.minRes,10)) <= 0 ? 0 : ((parseInt(this.ressis[2],10)-parseInt(filter.minRes,10)))) < parseInt(filter.minSendBorder,10) ) continue;
							} else {
								this.stor = parseInt(filter.minResPerCent,10)/100*this.storage;
								if( (this.stor >= parseInt(this.ressis[0],10)) && (this.stor >= parseInt(this.ressis[1],10)) && (this.stor >= parseInt(this.ressis[2],10)) ) 
									continue;
								else {
									if( checkboxes.disableMinSendBorder ) filter.minSendBorder = 0;
									if( ((parseInt(this.ressis[0],10)-this.stor) <= 0 ? 0 : ((parseInt(this.ressis[0],10)-this.stor)))
										+((parseInt(this.ressis[1],10)-this.stor) <= 0 ? 0 : ((parseInt(this.ressis[1],10)-this.stor)))
										+((parseInt(this.ressis[2],10)-this.stor) <= 0 ? 0 : ((parseInt(this.ressis[2],10)-this.stor))) < parseInt(filter.minSendBorder,10) ) continue;
								}
							}
						}
						if( checkboxes.disableMinSendBorder ) filter.minSendBorder = 0;
						if( (parseInt(this.ressis[0],10)+parseInt(this.ressis[1],10)+parseInt(this.ressis[2],10)) < parseInt(filter.minSendBorder,10) ) continue;
					}
...
if( document.getElementById("buildings_table") && !checkboxes.disableDSLoyalty && !checkboxes.disableShowLoyalty[2] ) {
	new appendLoyaltyLine();
}
...
if(location.href.match(/screen=main/) ) {
	...
	if( !checkboxes.disableBuildVars ) {
		...
		if( document.getElementById("hide_completed").checked ) lib.fireEvent(document.getElementById("hide_completed"),"click"); 
		...
		var updateMain = function(id) {
			...
			if( id != 0 ) {
				...
				for( var e=1 ; e<this.row.length ; e++ ) {
					this.bui = this.row[e].getElementsByTagName("a")[0].href.match(/screen=[a-z]+/g)[0].split("screen=")[1];
		...
		var Main = function(id) {
			...
			for( var j=1 ; j<mainBuild.length ; j++ ) {
				var buildName = mainBuild[j].getElementsByTagName("img")[0].src.match(/buildings\/([^\.]+)\.png/)[1];
		...
		a.addEventListener("click", function(event) {
			...
			var removeRow = function(tr) {
				if( !isNaN(tr.id) ) {
					...
					for( var e=0 ; e<this.selected.length ; e++ ) {
						if( this.selected.options[e].value == tr.id ) this.selected.options[e] = null;
					}
			...
			var editVariant = function(tr,boolean) {
				...
				this.button.addEventListener("click",function() {
					...
					var id=1;
					if( this.val.length == 0 ) { alert(gui[lib.lang].buildingVars.pleaseSetName); return;}
					...
					lib.storage.setValue("dssbBV_variants_player.id"+lib.game_data.player.id,variants);
					document.getElementById("dssbBV_variant").options.add( new Option(this.val,id,false,false) );
					...
					if( x != Map.mocX || y != Map.mocY ) {
						...
						if( village ) {
							switch( Map.redirTarget ) {
								...
								case "groupLinks": Map.lnk.href="javascript:popup_scroll(\"groups.php?&mode=village&village_id="+village.id+(lib.game_data.player.sitter_id!=0?"&t="+lib.game_data.player.id:"")+"\", 300, 550);"; break;				
...
if (location.href.match("screen=settings&mode=settings")) {
	...
	save.addEventListener("click", function(){
		...
		values.push("disableMaxButtons","disableFadeOutOffersEnemies","disableUseOfTroops","disableUseOfRessis","disableNeighbourVillages","disableLastVillageLink");
		...
		values.push("disableShowGroupsCombined","disableShowGroupsProduction","disableDSSortGroups","disableDSTroopsCalc","disableNewIncs","disableBashpoints");
		values.push("disableAddStandingUnits","disableAddInfosStorage","disableBuildVars","disableConTimeM","disableConTimeP","disableVpDistance");
		...
		for( var key in gui[lib.lang].settingsTabTitle ) {
   			if( key == "tabSendResources" ) {
				...
				var disableLinkToMap = appendCheckbox(tdTab[12],tdTab[13],gui[lib.lang].disableLinkToMap,"disableLinkToMap",this.checkboxes.settingsDsRealCharts[5]);			
					...
					var trTab = new Array(5);
				   	for( var i=0 ; i<trTab.length ; i++ )
			   			trTab[i] = document.getElementById("dssb_"+key+"_tab").appendChild( document.createElement("tr") );
			   	   	var tdTab = new Array(10);
			   	   	for( var i=0 ; i<tdTab.length ; i++ )
			   	   		tdTab[i] = trTab[Math.floor(i/2)].appendChild( document.createElement("td") );
					...
					var disableGroupLinksMain = appendCheckbox(tdTab[2],tdTab[3],gui[lib.lang].showGroups.groupLinksMain,"disableGroupLinksMain",this.checkboxes.disableGroupLinksMain);
					var disableShowGroupsMain = tabGroupsCheckbox( tdTab[3],gui[lib.lang].showGroups.disableShowGroups,"disableShowGroupsMain",this.checkboxes.disableShowGroupsMain);
					var disableGroupLinksMap = appendCheckbox(tdTab[4],tdTab[5],gui[lib.lang].showGroups.groupLinksMap,"disableGroupLinksMap",this.checkboxes.disableGroupLinksMap);					
					var disableShowGroupsMap = tabGroupsCheckbox( tdTab[5],gui[lib.lang].showGroups.disableShowGroups,"disableShowGroupsMap",this.checkboxes.disableShowGroupsMap);
					var disableGroupLinksCombined = appendCheckbox(tdTab[6],tdTab[7],gui[lib.lang].showGroups.groupLinksCombined,"disableGroupLinksCombined",this.checkboxes.disableGroupLinksCombined);				
					var disableShowGroupsCombined = tabGroupsCheckbox( tdTab[7],gui[lib.lang].showGroups.disableShowGroups,"disableShowGroupsCombined",this.checkboxes.disableShowGroupsCombined);
					var disableGroupLinksProduction = appendCheckbox(tdTab[8],tdTab[9],gui[lib.lang].showGroups.groupLinksProduction,"disableGroupLinksProduction",this.checkboxes.disableGroupLinksProduction);
					var disableShowGroupsProduction = tabGroupsCheckbox( tdTab[9],gui[lib.lang].showGroups.disableShowGroups,"disableShowGroupsProduction",this.checkboxes.disableShowGroupsProduction);
			} else if( key == "tabOther" ) {
					if(  name =="tabOther" ) document.getElementById("dssb_tabOther_tab").style.display = "block";
					var trTab = new Array(14);
				   	for( var i=0 ; i<trTab.length ; i++ )
			   			trTab[i] = document.getElementById("dssb_"+key+"_tab").appendChild( document.createElement("tr") );
			   	   	var tdTab = new Array(28);
			   	   	for( var i=0 ; i<tdTab.length ; i++ )
			   	   		tdTab[i] = trTab[Math.floor(i/2)].appendChild( document.createElement("td") );
					...
					var disableLastVillageLink = appendCheckbox(tdTab[2],tdTab[3],gui[lib.lang].lastVillageLink,"disableLastVillageLink",this.checkboxes.disableLastVillageLink);	
					...
					var dsSortGroups = appendCheckbox(tdTab[8],tdTab[9],gui[lib.lang].dssortgroups,"disableDSSortGroups",this.checkboxes.disableDSSortGroups);
					var dsTroopsCalc = appendCheckbox(tdTab[10],tdTab[11],gui[lib.lang].dstroopscalc,"disableDSTroopsCalc",this.checkboxes.disableDSTroopsCalc);
					var newIncsButton = appendCheckbox(tdTab[12],tdTab[13],gui[lib.lang].newIncs.newIncsButton,"disableNewIncs",this.checkboxes.disableNewIncs);
					var bashpoints = appendCheckbox(tdTab[14],tdTab[15],gui[lib.lang].bashpoints,"disableBashpoints",this.checkboxes.disableBashpoints);
					var addStandingUnits = appendCheckbox(tdTab[16],tdTab[17],gui[lib.lang].addStandingUnits,"disableAddStandingUnits",this.checkboxes.disableAddStandingUnits);
					var addInfosStorage = appendCheckbox(tdTab[18],tdTab[19],gui[lib.lang].addInfosStorage.addInfosStorage,"disableAddInfosStorage",this.checkboxes.disableAddInfosStorage);					
					var disableMassRecruit = appendCheckbox(tdTab[20],tdTab[21],gui[lib.lang].massRecruit,"disableMassRecruit",this.checkboxes.disableMassRecruit);
					var disableBuildVars = appendCheckbox(tdTab[22],tdTab[23],gui[lib.lang].buildingVars.buildVars,"disableBuildVars",this.checkboxes.disableBuildVars);
					var disableAGCounter = appendCheckbox(tdTab[24],tdTab[25],gui[lib.lang].dsAGCounter.dsAGCounter,"disableAGCounter",this.checkboxes.disableAGCounter);
					
					var span = tdTab[25].appendChild(document.createElement("span"));
					span.style.paddingRight = "40px";
					var resetAGCounter = tdTab[25].appendChild(document.createElement("a"));
					...
					tdTab[26].innerHTML = gui[lib.lang].paypalButton;
					var img = tdTab[27].appendChild( document.createElement("img") );
...
if (location.href.match(/screen=market/)) {
	...
	if( location.href.match(/screen=market/) && location.href.match(/try=confirm_send/) && !document.getElementById("inputx") ) {
			...
			submit.addEventListener("click",function() {
				...
				if( lib.pa ) {
					...
					if( lib.game_data.player.name == destinationPlayer ) {
						...
						for( var r=0 ; r<resName.length ; r++ ) {
							if( resName[r].title == gui[lib.lang].wood )
								dummyTrades[lib.game_data.village.id][villId].wood.push( parseInt(resValue[r].replace(".",""),10 ) );
							if( resName[r].title == gui[lib.lang].clay )
								dummyTrades[lib.game_data.village.id][villId].clay.push( parseInt(resValue[r].replace(".",""),10 ) );
							if( resName[r].title == gui[lib.lang].iron ) 
								dummyTrades[lib.game_data.village.id][villId].iron.push( parseInt(resValue[r].replace(".",""),10 ) );
        	           	}
			...
			if( !checkboxes.disableConTimeM ) {
				...
				document.getElementById("serverTime").addEventListener("DOMSubtreeModified",function() {			
			...
			if( !checkboxes.disableMaxButtons ) {
				...
				if (trader >= 0 && tab) {
					...
					for( var i = 1; i < tab.rows.length; i++ ) {
						...
						var available = document.getElementById(res[tab.rows[i].cells[1].innerHTML.replace(/-cs5/g,"").match(/graphic\/([^\.]+)\.png/)[1]]).innerHTML;    
...
/* dssortgroups */
if( location.href.match(/screen=overview_villages/) && location.href.match(/edit_group/) && !checkboxes.disableDSSortGroups ) {
	var lnks = document.getElementsByTagName("a");
	var lnk = null;
	for(var i=0; i<lnks.length; i++){
		if(lnks[i].textContent.indexOf( regExp[lib.lang].editGroups ) != -1){
			lnk = lnks[i];
			break;
		}
	}

	var a = lnk.parentNode.appendChild( document.createElement("a") );
	a.href = "javascript:;";
	a.innerHTML = gui[lib.lang].sortGroups.readOutAndChange;
	a.addEventListener("click",function() {
		if( lib.storage.getValue("toReadIn_player.id"+lib.game_data.player.id,"") != "" ) {
			alert(lib.createTextNode( gui[lib.lang].sortGroups.checksAreRead ).textContent); return;}
		var rows = document.getElementsByClassName("vis overview_table")[0].getElementsByTagName("tr"), values=[];
		for(var i = 1; i < rows.length; i++) {
			if(!rows[i].innerHTML.match(/id="label_\d+"/) || rows[i].style.display == "none" || !rows[i].getElementsByClassName("check")[0].checked ) continue;
			var href = rows[i].getElementsByTagName("td")[1].getElementsByTagName("a")[0].href.split("village=")[1].split("&")[0];
			values.push( href );		
			rows[i].getElementsByClassName("check")[0].checked = false;
		}
		lib.storage.setValue("toReadIn_player.id"+lib.game_data.player.id,values);
		alert( lib.createTextNode( gui[lib.lang].sortGroups.pleaseConfirm ).textContent );	
	},false);
	lnk.parentNode.appendChild( document.createElement("br") );

	var b = lnk.parentNode.appendChild( document.createElement("a") );
	b.href = "javascript:;";
	b.innerHTML = gui[lib.lang].sortGroups.pasteChanges;
	b.addEventListener("click",function() {
		if( lib.storage.getValue("toReadIn_player.id"+lib.game_data.player.id,"") == "" ) {
			alert( lib.createTextNode( gui[lib.lang].sortGroups.firstReadChecks ).textContent ); return;}
		var rows = document.getElementsByClassName("vis overview_table")[0].getElementsByTagName("tr");
		var values = lib.storage.getValue("toReadIn_player.id"+lib.game_data.player.id,[]);

		for(var i = 1; i < rows.length; i++) {
			if(!rows[i].innerHTML.match(/id="label_\d+"/) ) continue;
			var href = rows[i].getElementsByTagName("td")[1].getElementsByTagName("a")[0].href.split("village=")[1].split("&")[0];
			if( lib.contains( href, values ) ) {
				rows[i].getElementsByClassName("check")[0].checked = true;
			}
		}
		lib.storage.deleteValue("toReadIn_player.id"+lib.game_data.player.id);
		alert( lib.createTextNode( gui[lib.lang].sortGroups.pleaseConfirm ).textContent );
	},false);
}
...
if( location.href.match(/screen=place/) && !checkboxes.disableNeighbourVillages && lib.pa ) {	
	if( location.href.match(/&mode=neighbor/) ) {
    	/* add elements */
	...
	/* time counter */
	if( location.href.match(/try=confirm/) && !checkboxes.disableConTimeP ) {
	...
	if( location.href.match(/mode=units/) && !checkboxes.disableVpDistance ) {
		document.getElementById("units_home").style.whiteSpace = "nowrap";
		var tr = document.getElementById("units_home").getElementsByTagName("tr"), td, elem, sumDis=0, count=0;
		var up = lib.createTextNode( String.fromCharCode(9650) );
		up.id = "dssb_up";
		var down = lib.createTextNode( String.fromCharCode(9660) );
		down.id = "dssb_down";
		...
		for( var x=0 ; x<tr.length ; x++ ) {
			if( tr[x].getElementsByTagName("td").length > 0 ) {
				...
				elem = td[1].parentNode.insertBefore( document.createElement("td"), td[1] );
				
				if( td[0].getElementsByTagName("input").length > 0 ) {
					var coords = td[0].getElementsByTagName("a")[0].textContent.match(/\((\d{1,3})\|(\d{1,3})\) K(\d+)$/);
					var sourceCoords = lib.game_data.village.coord.split("|");
				...
				if( x==0 ) {
					...
					a.addEventListener("click",function() {
						...
						for( this.i=2 ; this.i<this.tr.length ; this.i++ ) {
							if( this.tr[this.i].getElementsByTagName("th").length > 0 ) continue;
							this.toSort.push( [this.tr[this.i].getElementsByTagName("td")[1].textContent, this.tr[this.i]] );
						}						
						if( document.getElementById("dssb_up") ) {
							this.parentNode.removeChild( document.getElementById("dssb_up") );
							this.parentNode.appendChild( down );
							this.toSort.sort(function(a,b) {return b[0]-a[0]});
						} else {
							if( document.getElementById("dssb_down") ) this.parentNode.removeChild( document.getElementById("dssb_down") );
							this.parentNode.appendChild( up );							
...
if( location.href.match(/screen=place/) && location.href.match(/mode=sim/) && !checkboxes.disableDSTroopsCalc && lib.pa ) {
	...
	input.addEventListener("click",function() {
		...
		this.inputs[this.inputs.length-2].click();
	}, false);	
...
if( !lib.storage.getValue("DsAccDualForum_disabled",false) ) {
	if ( location.href.match(/screenmode=view_forum&forum_id/) || location.href.match(/screen=view_forum&forum_id/) || location.href.match("forum.php?") || location.href.match(/screen=forum/) ) {  
		...
		markAll[1].addEventListener("click",function() {
			if( confirm(gui[lib.lang].dsForumAllReaded) ) {
				this.values = lib.storage.listValues();
				for( var i=0 ; i<this.values.length ; i++ ) {
...
if( document.getElementById("target_attack") ) {
	...
	/* last village link */
	if( !checkboxes.disableLastVillageLink )  {
		...
		var onclick = a[3].getAttribute("onclick");
		var br = a[3].parentNode.getElementsByTagName("br");
		var last = a[3].parentNode.appendChild( document.createElement("a") );
		a[3].parentNode.removeChild( a[3] );
		br[0].parentNode.removeChild( br[0] );
		last.href = "javascript:;";
		last.innerHTML = gui[lib.lang].lastVillage+lib.showHotkey( hotkeys.keys.keysPlace.lastVillage );
		last.id = "lastVillage";
		last.addEventListener("click",function() {
			var coords = onclick.match(/\([\d]+\)/g);
			var x = coords[0].match(/[\d]+/g)[0];
			var y = coords[1].match(/[\d]+/g)[0];
			
			if( x != document.getElementById("inputx").value || y != document.getElementById("inputy").value ) {
	...
	if( !checkboxes.disableDsMoveReports && !location.href.match(/view=/) && lib.pa ) {
		...
		var DSMoveReportsFunctions = function(objId) {
			...
			if( objId == "moveBB" ) {
				...
				for(this.x = 0; this.x < this.allReports.length; this.x++) {
					if( this.allReports[this.x].blue ) this.setCheck(this.allReports[this.x].id);
					else this.removeCheck(this.allReports[this.x].id);
					for( this.f=0 ; this.f<this.moveBBIDs.length ; this.f++ ) {
						if( this.moveBBIDs[this.f].match(this.allReports[this.x].id) ) this.removeCheck(this.allReports[this.x].id);}
					for( this.f=0 ; this.f<this.moveAttsIDs.length ; this.f++ ) {
				...
				if( document.getElementById("attack_info_def").getElementsByTagName("a").length > 1) {
					if( document.getElementById("attack_info_def").getElementsByTagName("a")[0].textContent == lib.game_data.player.name ) {
						if( document.getElementById("attack_results").textContent.match(regExp[lib.lang].loyalty) ) {
							...
							var date = new Date();
							date.setFullYear("20"+time[0].split(".")[2]); date.setMonth(parseInt(time[0].split(".")[1]-1));
		...
		if( !hotkeys.disableAll.keysReports ) {
			...
			for( var i = 0; i < Math.max( reportA[0].length,reportA[1].length ); i++ ) {
				for( var j=0 ; j<reportA.length-1 ; j++ ) {
					...
					else if( reportA[j][i].href.indexOf("mode=move")>-1 ) id="move";
...
if( !checkboxes.disableDsRealCharts && (lib.lang == "de" || lib.lang == "ch") ) {
	...
	if( location.href.match(/screen=info_player/) ) {
		var playerId = location.href.match(/screen=info_player[&=A-Za-z0-9]*\&id=(\d+)/)[1]; 
		...
		if( tab == undefined ) {  
			...
			for( var i=0 ; i<document.links.length ; i++ ) {
				if (link[i].href.search(/village=[0-9]+&screen=mail&mode=new[&=A-Za-z0-9]*&player=([0-9]+)/) > -1) {
		...
		for (var i=0 ; i<document.links.length ; i++) {
			...
			if (link.href.search(/village=[0-9]+&screen=mail&mode=new[&=A-Za-z0-9]*&player=([0-9]+)/) > -1) {  
				var tr = new Array(6);
...
if( location.href.match(/screen=train&mode=mass/) && lib.pa &&  !checkboxes.disableMassRecruit ) {
...
function Knish3rDSLib(prefix) {
        ...
        keyUpHandler : function(e) {
            ...
            if( e.target.nodeName.toUpperCase() != "TEXTAREA" ) {
                for( var i = 0; i < self.hotkeys.keys.length; i++ ) {
                    if( self.hotkeys.keys[i].key[0] == e.keyCode && !self.hotkeys.keys[i].key[2] && !hotkeys.disableAll[self.hotkeys.keys[i].key[3]] ) {
	...
	this.popup = function(id, x, y, html, draggable, width, height) {	
		if( document.getElementById( id ) ) {
			...
			if( draggable ) {
				...
				this.script.innerHTML = "$(document).ready(function(){ UI.Draggable($('#inline_popup')); });"	
			}
		}	
	...
	this.showHotkey = function(show) {
		if( !show[2] && !hotkeys.disableAll[show[3]] ) return " ["+show[1]+"]";
		else return "";
	};
