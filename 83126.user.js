...
// @exclude        http://*.ogame.gameforge.com/game*
// @version        1.0.1
// @downloadURL    http://userscripts.org/scripts/source/83126.user.js
// @updateURL      https://userscripts.org/scripts/source/83126.meta.js
// ==/UserScript==
...
if (sel) { // needed this because this script is sometimes executed on wrong pages
	...
	if (manage) {
		...
		var Manager = {	
			...
			getAbbr: function (select) {
				...
				var uniNumber = parseInt(value.substring(1, value.indexOf('-')));
	...
	var FastLogin = {
		...
		versionCheck: function () {
			...
			if (isNaN(installedVersion) || installedVersion < this.lastCompatibleVersion) {
		...
		getDomain: function () {
			if (!this.domain) {
				...
				var options = select.getElementsByTagName('option');
				this.domain = '.' + options[0].value.substring(options[0].value.indexOf('-') + 1, options[0].value.length);
			}
			return this.domain;
		},
