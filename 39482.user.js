// ==UserScript==
// @name           Terpcenter.com Youtube Embed
// @namespace      Forest21
// @include        http://www.terpcenter.com/forums/showthread.php?*
// ==/UserScript==

// ==UserScript==
// @name           Scout.com Youtube Embed
// @namespace      Forest21
...
var movieCounter = 1;
	...
	for(var j=0; j<links.length; j++)
	{
		...
		if(link.indexOf("youtube.com/watch")!=-1)
		{
			...
			showLink.innerHTML = "(View)";
			showLink.id = movieCounter+"Link";
			showLink.href="#Link"+movieCounter;
			showLink.name="Link"+movieCounter;
			showLink.style.color="gray";
	
			parent.insertBefore(showLink,links[j].nextSibling);
	
			showLink = document.getElementById(movieCounter+"Link");
			showLink.addEventListener("click",toggle,true);
			movieCounter++;
			
			parent.insertBefore(space.cloneNode(true),links[j].nextSibling);
...
{
var id = parseInt(this.id);
var hdlink = document.getElementById(id+"HDLink");
var box;
