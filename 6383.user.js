// ==UserScript==
// @name Ogame FR : Standard messages.php (Messages)
// @author Izcelion
// @description Ogame FR : Standard messages.php (Messages)
// @language FR 
...
if (isFinite(GetUniverse())){
	...
	while (i<trnode.length) {
		...
		var expthcolspan4 = new RegExp("<th colspan=\"4\">","g");
		var exptypesubmit = new RegExp("type=\"submit\"","g");
		var exptitle = new RegExp("<td colspan=\"4\" class=\"c\">Messages</td>","g");
	
		if (expthcolspan4.test(trnodesentence) && !exptypesubmit.test(trnodesentence) && !exptitle.test(trnodesentence)) {
	...
	//alert("Standard messages.php (Messages)");
