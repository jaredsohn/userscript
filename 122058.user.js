// ==UserScript==
// @name            TC - Casper's Reconlogger
// @namespace       http://*ruinsofchaos.com/*
// @description     Logs recons and sabs.
// @version         2.2
// @include         http://*ruinsofchaos.com/*
// @exclude         http://*beta.ruinsofchaos.com/*
// @exclude         http://*ruinsofchaos.com/recruiter*.php
// @require         http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.js
// ==/UserScript==

// Author : Casper
// Special thanks to Jerre, Deloo and PHNX for helping with certain parts!
// Updated for Age 8 by Jerre
...
// Check if the user is allowed to use the script
//-----------------------------------------------
...
function CheckAllowed() {
    ...
    var user = alltables[1].rows[0].cells[0].getElementsByTagName("a")[0].innerHTML;
 
    var d=new Date();

    var hour = d.getHours();
    var lastchecked = GM_getValue("LastAllowed");
    ...
    GM_xmlhttpRequest({
    method: "GET",
    url: "http://www.phnxforums.com/jerre/allowed.php?user=" + user,
    onload: function(responseDetails) {
    // GM_setValue("LastAllowed",hour); // Reset the last "updated time".
    var allowed = responseDetails.responseText;
    //allowed = allowed.toString();
    allowed = parseInt(allowed);
    DisplayMessage(allowed);
    if (allowed == "1"){
    DisplayMessage("It's a 1, whiiii");
    }
    else {
    //	DisplayMessage("2 it is, damnit, mothafuckers");
    }
...
function Version() {
    var ThisVersion = '2.2';
    var tempURL = 'http://www.phnxforums.com/jerre/version.php';
    GM_xmlhttpRequest({
        method: 'GET',
        url: tempURL,
        ...
        onload: function (responseDetails) {
            ...
            version = response;
            downloadURL = 'http://www.phnxforums.com/jerre/Jerre_reconlogger.user.js';
            
            if (version != ThisVersion) {
                prompt('You are running version ' + ThisVersion + ', please look for version ' + version +
				' here. (The download will start automaticly if you press Ok)', downloadURL);
                window.location.href = downloadURL;
            }
            else {
                var dourl = 'http://www.phnxforums.com/jerre/usingversion.php?version=' + ThisVersion + '&me=' + GM_getValue('myID');
                // Update version used in database
                GM_xmlhttpRequest({
                    method: 'GET',
                    url: dourl,
                    onload: function (responseDetails) {
                        //alert("Return: " + responseDetails.responseText);
                        // document.body.innerHTML = document.body.innerHTML + responseDetails.responseText;
                        //DisplayMessage(dourl);
                    },
                    onerror: function (responseDetails) {
                        alert('Request for contact resulted in error code: ' + responseDetails.status);
                    }
...
// Show a clock and warn when there's a turn coming
//-------------------------------------------------

if (GM_getValue('Show_Clock', 'Yes') == 'Yes') {
    updateClock();
}
...
function updateClock() {
    ...
    if (((currentMinutes == '59') || (currentMinutes == 29)) && (currentSeconds > 30)) {
    ...
    if (!clock) {
        if (turnsoon == 0) {
            addCSS(
                '#_md_clock {position:fixed;top:10px;left:10px;background-color:#121212;}'
             );
        }
        else {
            addCSS(
    ...
    clock.innerHTML = '<span style="font-size: 20pt;color:red;" title="Does NOT (YET) run together as server!!">' + currentTimeString + '</span>';
    clock.style.display = '';
...
//================================================================
// CODE THAT IS EXECUTED ON THE BASE PAGE
//================================================================

function Basepage() 
{
	
// Show the settings in the top-right corner
//------------------------------------------
    ...
    function ShowSettings_Small(event) {
		...
		{
            addCSS(
             '#_md_settings {position:fixed;top:10px;right:10px;background-color:#121212;border:2px solid #000000;padding:5px;text-align:center;}'
          	);
        ...
        var s = '<div>';
        s += '&nbsp;<input type="button" id="expand" value="General Settings" /><div style="float:left; width: 15%;"></div>';
    ...
    function ShowSettings_Big(event) {
        ...
        s += '<tr>';
        s += '<th colspan="2">General Settings</th>';
        s += '</tr>';

        var option_yes = '';
        var option_no = '';
        if (GM_getValue('SendStats', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendStats', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your own stats when you are on the Base Page" style="cursor: help;">Send stats? </td>';
        s += '<td><select><option id="Stats_Yes"' + option_yes + 'value="Yes">Yes</option>';
        ...
        s += '<tr>';
        s += '<td title="Send your sabs to our server and Viking" style="cursor: help;">Send Sabs? </td>';
        s += '<td><select><option id="Sabs_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';
		
		option_yes = '';
        option_no = '';
        if (GM_getValue('SendRecons', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendRecons', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your recons to our server and Viking" style="cursor: help;">Send Recons? </td>';
        s += '<td><select><option id="Recons_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

		option_yes = '';
        option_no = '';
        if (GM_getValue('SendSabdamage', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendSabdamage', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your Sabdamage when you are on the Base Page" style="cursor: help;">Send Sabdamage? </td>';
        s += '<td><select><option id="Sabdamage_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

		option_yes = '';
        option_no = '';
        if (GM_getValue('SendPillaged', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendPillaged', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your Pillaged when you are on the Base Page" style="cursor: help;">Send Pillaged? </td>';
        s += '<td><select><option id="Pillaged_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

        option_yes = '';
        option_no = '';
        if (GM_getValue('ShowAAT', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('ShowAAT', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Show AAT at your own Armory Page" style="cursor: help;">Show AAT? </td>';
        s += '<td><select><option id="AAT_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

		option_yes = '';
        option_no = '';
        if (GM_getValue('Show_Clock', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('Show_Clock', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Show clock at top of every RoC page" style="cursor: help;">Show Clock? </td>';
        s += '<td><select><option id="Clock_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

        option_yes = '';
        option_no = '';
        if (GM_getValue('SendArmory', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendArmory', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your armory to our server when you go to Armory Page" style="cursor: help;">Send Armory? </td>';
        s += '<td><select><option id="Armory_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

        option_yes = '';
        option_no = '';
        if (GM_getValue('SendTrain', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('SendTrain', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Send your soldiers to our server when you go to Train Page" style="cursor: help;">Send Train? </td>';
        s += '<td><select><option id="Train_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

        option_yes = '';
        option_no = '';
        if (GM_getValue('OrderBattlefield', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('OrderBattlefield', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="The battlefield will automatically be ordered by gold" style="cursor: help;">Order BF by gold?</td>';
        s += '<td><select><option id="Order_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

		option_yes = '';
        option_no = '';
        if (GM_getValue('Startpage1', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('Startpage1', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="If you press battlefield you will go to page 1" style="cursor: help;">BF startpage 1?</td>';
        s += '<td><select><option id="Orderpage1_Yes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';

		option_yes = '';
        option_no = '';
        if (GM_getValue('ShowUpgradehelper', 'Yes') == 'Yes') option_yes = 'selected="selected"';
        if (GM_getValue('ShowUpgradehelper', 'Yes') == 'No') option_no = 'selected="selected"';

        s += '<tr>';
        s += '<td title="Show upgradehelper on Armory Page" style="cursor: help;">Show Upgradehelper?</td>';
        s += '<td><select><option id="UpgradeYes"' + option_yes + 'value="Yes">Yes</option>'
		s += '<option ' + option_no + ' value="No">No</option></select></td>';
        s += '</tr>';
    ...
    function Save_Settings(event) {
        ...
        stats = allselected[0].innerHTML;
        sabs = allselected[1].innerHTML;
        recons = allselected[2].innerHTML;
        sabdamage = allselected[3].innerHTML;
        pillaged = allselected[4].innerHTML;
        aat = allselected[5].innerHTML;
        clock = allselected[6].innerHTML;
        armory = allselected[7].innerHTML;
        train = allselected[8].innerHTML;
        order = allselected[9].innerHTML;
        startpage = allselected[10].innerHTML;
        showupgradehelper = allselected[11].innerHTML;

        GM_setValue('SendStats', stats);
        GM_setValue('SendSabs', sabs);
        GM_setValue('SendRecons', recons);
        GM_setValue('SendSabdamage', sabdamage);
        GM_setValue('SendPillaged', pillaged);
        GM_setValue('ShowAAT', aat);
        GM_setValue('Show_Clock', clock);
        GM_setValue('SendArmory', armory);
        GM_setValue('SendTrain', train);
        GM_setValue('OrderBattlefield', order);
        GM_setValue('Startpage1', startpage);
        GM_setValue('ShowUpgradehelper', showupgradehelper);

        window.location.reload();
    }


// Send my stats to the PHNX server
//---------------------------------
    ...
    var gold = document.getElementById('s_gold').innerHTML.replace(/,/g, '');
    var strike = alltables[i].rows[1].cells[1].innerHTML.replace(/,/g, '');
    var defense = alltables[i].rows[2].cells[1].innerHTML.replace(/,/g, '');
    var spy = alltables[i].rows[3].cells[1].innerHTML.replace(/,/g, '');
    var sentry = alltables[i].rows[4].cells[1].innerHTML.replace(/,/g, '');

    var nostats = 0;
    if (GM_getValue('SendStats', 'Yes') == 'No') 
	{
        var nostats = 1;
    }

    i = i + 2; 
    if (GM_getValue('SendPillaged', 'Yes') == 'Yes') 
	{
        var pillaged = alltables[i].rows[2].cells[1].innerHTML.replace(/,/g, '');
    }
    else 
	{ 
		var pillaged = '0'; 
    ...
    if (GM_getValue('SendSabdamage', 'Yes') == 'Yes') 
	{
        var sabotagedamage = alltables[i].rows[5].cells[1].getElementsByTagName("a")[0].innerHTML.replace(/,/g, '');
    }
    else 
	{ 
		var sabotagedamage = '0'; 
	}

    MakeRequest('basepage.php?user=' + me + '&strike=' + strike + '&defense=' + defense + '&spy=' + spy + '&sentry=' + sentry + '&pillaged=' +
	pillaged + '&sabotagedamage=' + sabotagedamage + '&amount=' + gold + '&nostats=' + nostats);
...
//================================================================
    ...
    function Armorypage()
        ...
        // Send your amount of weapons and sov to the PHNX server
        //-------------------------------------------------------
           
            if (GM_getValue('SendArmory', 'Yes') == 'Yes')
           {
                MakeRequest('armorypage.php?user=' + me + '&daggers=' + daggers + '&mauls=' + mauls + '&blades=' + blades + '&excals=' + excals +
              '&sais=' + sais + '&shields=' + shields + '&mithrils=' + mithrils + '&skins=' + skins + '&cloaks=' + cloaks + '&hooks=' + hooks +
              '&picks=' + pickaxes + '&horns=' + horns + '&dogs=' + dogs + '&torches=' + torches + '&sov=' + SOV);
            }
...
function IntelPage() 
{
    ...
    if (InStr(strHTML, '<td>Strike: </td>') == true) 
	{
        ...
        // Send the info from your recon/sab to the PHNX server
		//-------------------------------------------------
...
function Trainpage() 
{
	// Send the amount of soldiers to the PHNX server
	//-----------------------------------------------
...
function Battlefieldpage() {
    ...
    for (i = 2; i < rows.length - 1; i++) 
	{
        ...
        if (GM_getValue("ShowGold", "Yes") == "Yes") 
		{
			...
			{
                //Didn't get GOLD
                var tempURL = "http://www.phnxforums.com/jerre/gold.php?id=" + ID;
                newgold(tempURL, i);
            }
        }
...
function MakeRequest(url) 
{
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'http://www.phnxforums.com/jerre/' + url,
        onload: function (responseDetails) {
...
function sendgold(ID, gold_num) 
{
    ...
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'http://www.phnxforums.com/jerre/sendgold.php?id=' + ID + '&gold=' + gold_num,
        onload: function (responseDetails) {
        },
        onerror: function (responseDetails) {
            alert("Request for contact resulted in error code: " + responseDetails.status);
...
function showdefense(ID, res) 
{
    ...
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'http://www.phnxforums.com/jerre/defense.php?id=' + ID,
        onload: function (responseDetails) {
            var response = "";
            response = responseDetails.responseText;
