...
// parametre
var debug = true;
var snd_construct_finished = new Audio("http://ogame.tsuna.fr/sound/contruct_finiched.wav");
var snd_new_msg = new Audio("http://ogame.tsuna.fr/sound/new_msg.wav");
var snd_attack = new Audio("http://ogame.tsuna.fr/sound/Attack.wav");
var refresh_time = 2000;
var auto_change_page_min = 20000;   // en miliseconde
var auto_change_page_max = 15000;   // en miliseconde
var next_page="";
...
var min_pb = 40;
var min_gb = 200;
var obj_def = my_load("obj_def",true,0);
var used_nb_lm = my_load("obj_def",true,0);

var nb_construction = 0;        // nombre de construction en cours
var old_nb_construction = 0;
var auto_refresh = false;
var refresh_asked = false;
...
function load_settings(){
        ...
        // niveau des recherche
        search_nrj = my_load("search_nrj",false,0);
        //my_info("Recherche énergie niveau "+search_nrj);
        search_laser =my_load("search_laser",false,0);
        search_ions = my_load("search_ions",false,0);
        search_thyper = my_load("search_thyper",false,0);
        search_plasma = my_load("search_plasma",false,0);
        search_comb = my_load("search_comb",false,0);
        search_imp = my_load("search_imp",false,0);
        search_phyper = my_load("search_phyper",false,0);
        search_espio = my_load("search_espio",false,0);
        search_ordi = my_load("search_ordi",false,0);
        search_astro = my_load("search_astro",false,0);
        search_rsx = my_load("search_rsx",false,0);
        search_graviton = my_load("search_graviton",false,0);
        search_arme = my_load("search_arme",false,0);
        search_bouclier = my_load("search_bouclier",false,0);
        search_protec = my_load("search_protec",false,0);

        // paramètre du script
        auto_refresh = my_load("auto_refresh",false,false);
}
...
function load_time(page){
    ...
    if (ttime >= (60*60*24*365)){
        // plus d'une journé
        stime += Math.floor(ttime/(60*60*24*365))+" années ";
        ttime = ttime%(60*60*24*365);
    }
    if (ttime >= (60*60*24*30)){
        // plus d'une journé
        stime += Math.floor(ttime/(60*60*24*30))+" mois ";
        ttime = ttime%(60*60*24*30);
    }
    if (ttime >= (60*60*24)){
        // plus d'une journé
        stime += Math.floor(ttime/(60*60*24))+" jour ";
        ttime = ttime%(60*60*24);
    }
    if (ttime >= (60*60)) {
        stime += Math.floor(ttime/(60*60))+" heure ";
        ttime = ttime%(60*60);
    }
    if (ttime >= (60)) {
        stime += Math.floor(ttime/(60))+" minutes ";
        ttime = ttime%(60);
    }
...
function write_settings(){
        ...
        // recherche
        my_write("search_nrj",false,search_nrj);
        //my_info("write : energie = "+search_nrj);
        my_write("search_laser",false,search_laser);
        my_write("search_ions",false,search_ions);
        my_write("search_thyper",false,search_thyper);
        my_write("search_plasma",false,search_plasma);
        my_write("search_comb",false,search_comb);
        my_write("search_imp",false,search_imp);
        my_write("search_phyper",false,search_phyper);
        my_write("search_espio",false,search_espio);
        my_write("search_ordi",false,search_ordi);
        my_write("search_astro",false,search_astro);
        my_write("search_rsx",false,search_rsx);
        my_write("search_graviton",false,search_graviton);
        my_write("search_arme",false,search_arme);
        my_write("search_bouclier",false,search_bouclier);
        my_write("search_protec",false,search_protec);

        // paramètre
        my_write("auto_refresh",false,auto_refresh);
}
...
function found_planete_active(){
	...
	var myworld = get_worlds();
	var reg = new RegExp("<B>(.*)</B>","");
	if (myworld.children[1].children.length > 2){
		for (i = 0; i < myworld.children[1].children.length; i++){
			var planete = myworld.children[1].children[i];
			var toto = planete.children[0].getAttribute('class').match('active');
			if ( toto != null){
				// planete active;
				
				var test = myworld.children[1].children[i].children[0].title.match(reg);
				temp_max = myworld.children[1].children[i].children[0].title.split("<BR>")[2].split("à")[1].replace(" ","").replace("°C","");
				test = test[1].split(' ')[0];
				planete_active = test;
...
function udt_nb_to_build(id, needed, only_one){
    ...
    var need = 0;
    if (only_one){
        if (used_nb_lm >= needed)
            need = 1; 
    }
    else{
        need = Math.floor(needed * used_nb_lm);
    }
    docId("details"+id).getElementsByClassName("level")[0].innerHTML = info+"/"+need;
}
function chk_used_nb_lm(){
    //my_info("Récupération du nombre de lm utilisé pour les calcule");
    var reg = /([0-9]+)/;
    var dnb_lm = 0;

    if (docId("chk_obj_def")!= null){
        my_write("chk_obj_def",true,docId("chk_obj_def").checked);
    }

    if(docId("obj_def")!= null){
        obj_def = docId("obj_def").value;
        // my_info(n_obj_def_val);
        my_write("obj_def",true,obj_def);
    }
    else{
        obj_def = my_load("obj_def",true,0);
    }

    if(docId("chk_obj_def")!= null && !docId("chk_obj_def").checked){
        dnb_lm = docId("details401").getElementsByClassName("ecke")[0].children[0].innerHTML.replace(".","");
        dnb_lm = parseInt(reg.exec(dnb_lm)[0]);

    }
    else{
        if (docId("obj_def") != null){
            dnb_lm = docId("obj_def").value;
        }
    }
    used_nb_lm=dnb_lm;
...
function nb_def_needed(){
    ...
    setTimeout(nb_def_needed, refresh_time/2);
}
function table_param_ratio(){
    ...
    var info = "<table><tr>";
        ...
        info += "</tr><tr><td colspan=2>Objectif en lanceur de missile</td>";
        info += "</tr><tr><td><input type='text' style='width:118px' id='obj_def' value='"+obj_def+"'/></td><td><input type='checkbox' id='chk_obj_def'";
        if (my_load("chk_obj_def",true,"false") == "true") info += " checked='checked'";
        info += "/>Activé l'objectif</td>";
        info += "</tr></table>";
    return info;
}
function udt_ratio(){
...
function check_price(){
    ...
    setTimeout(check_price,refresh_time);
}
...
function chk_account_saved(){
    ...
    for (var i = 0; i < nb_pla_colo; i++){
        my_world = get_worlds();
        GM_setValue("exist_pla_nom"+i+id_account,my_world.children[1].children[i].children[0].getElementsByClassName('planet-name')[0].innerHTML)
    }
}
...
function write_on_empire_page(){
    ...
    // parametrage
    var page = "<fieldset><legend>Paramètres</legend><input type='checkbox' id='auto_refresh' ";
    if (auto_refresh){
        page += "checked='checked'";
    }
    page += "/> Mise à jour automatique des informations (prise de controle de la page ogame ouverte)</fieldset><br/>";

    // bloque des messages
    page += "<fieldset><legend>Messagerie</legend>";
    var nb_msg = my_load("nb_msg",false,0);
    if (nb_msg > 0){
        page+= "Vous avez "+nb_msg+" message";
        if (nb_msg > 1) page+="s";
        page+= " non lu";
        if (nb_msg > 1) page+="s";
    }
    else{
        page += "Vous n'avez aucun message non lu";
    }

    page += "</fieldset><br/>";

    // ressource :
    page += "<table class='block_empire'><tr><th></th><th collspan='4'>Ressources</th></tr>";
    page += "<tr><td></td><td>Métal</td><td>Cristal</td><td>Deutérium</td><td>Energie</td></tr>";
    ...
    page += "</table><br/>";
    // vue de l'empire
    page += "<table class='block_empire'>"+"<tr id='empire_title'>";

    // affichage des titres des colones
    var title = new Array("","Mines","Hangar","Central","Usines","Installations");
    var colspan = new Array(0,3,3,2,2,5);
    ...
    for (var i = 1; i <= nb_planete; i++){
        ...
        if (i == 1){
            ...
            title = new Array("&nbsp;","Metal","Cristal","Deutérium","Metal","Cristal","Deutérium","Solaire","Fusion","Robots","Nanite","Chantier Spatial","Laboratoire","Dépot","Silo","Terraformer");
            classe= new Array("","td_mines","td_mines","td_mines","td_usines")
            page += "<tr>";
    ...
    page += "</table><br/>";
    // block Recherche
    page += "<table class='block_empire'>"+"<tr>";
    title = new Array("Recherche fondamentale","propulsion","avancée","Combat");
    colspan = new Array(5,3,5,3);
    for(var i = 0; i < title.length; i++){
        page += "<td colspan='"+colspan[i]+"' onclick='javascript:masquer_col(\""+classe[i]+"\")'>"+title[i]+"</td>";
    }
    page += "</tr>";
    title = new Array("Energie","Laser","Ions","Hyperespace","Plasma","Combustion","Impulsion","Hyperespace","Espionage","Ordinateur","Astrophysique","Réseaux de recherche","Graviton","Armes","Boucliers","Protection");
    page += "<tr>";
    for(var i = 0; i < title.length; i++){
        page += "<td>"+title[i]+"</td>";
    }
    ...
    page += "</table><br/><br/><br/>";
    page += "Prochaine mise à jour : ";
    page += "<table class='block_empire'>";
...
function check_param(){
    check_auto_refresh = docId("auto_refresh");
    //my_info(check_auto_refresh);
    if (check_auto_refresh != null){
        auto_refresh = check_auto_refresh.checked;
        my_write("auto_refresh",false,auto_refresh);
    }
}
...
function load_working(){
    ...
    var listes = new Array("mine_metal","mine_cristal","mine_deut","central_solaire","central_fusion","hangar_metal","hangar_cristal","hangar_deut","cache_metal","cache_cristal","cache_deut","usine_robot","chantier_spatial","labo"   ,"depot"  ,"silo"   ,"nanite" ,"terraformer","search_espio");
    var pages = new Array( "resources" ,"resources"   ,"resources","resources"      ,"resources"     ,"resources"   ,"resources"     ,"resources"  ,"resources"  ,"resources"    ,"resources" ,"station"    ,"station"         ,"station","station","station","station","station"    ,"research");
...
function auto_update_page(){
    auto_refresh = my_load("auto_refresh",false,false);
    //my_info("Debug : autorefresh="+auto_refresh+" refresh_asked="+refresh_asked);
    if (auto_refresh && !refresh_asked){
        choose_next_page();
        change_page();
    }
    else if (refresh_asked && !auto_refresh){
        // refresh annulé
        my_info("Désactivation du refresh")
        //suppression du masque
        document.getElementsByTagName("body")[0].removeChild(docId("Locked"));
        refresh_asked = false;
    }
    setTimeout(auto_update_page,refresh_time);
}
function choose_next_page(){
    // recherche de la page mise à jour en dernière
    var vals=[];
    for each (var val in GM_listValues()){
        if (is_needed_page(val)){
            vals.push([GM_getValue(val),val]);
        }
    }
    vals = vals.sort();
    //my_info(vals.sort());
    // trié par ordre croissant, du coup le plus ancien est le premier
    var next = vals[0][1];

    // génération du lien en fonction du nom
    var splitted = next.split("_");
    next_page = "index.php?page="+splitted[2];

    // récupération de l'id de la planète
    var id_pla = get_pla_id(splitted[3]);
    
    next_page += "&cp="+id_pla;

    my_info("La page "+splitted[2]+" de la planete "+splitted[3]+"("+id_pla+") à besoins d'une mise à jour");

}
function get_worlds(){
    // renvoie l'élement contenant la liste des planètes
...
function get_pla_id(nm_pla){
    var myworlds = get_worlds();
    var reg = new RegExp("<B>(.*)</B>","");
    for (var i = 1; i< myworlds.children.length; i++){
        var test = myworlds.children[i].children[0].title.match(reg);
        var nom = test[1].split(' ')[0];
        var id_pla = 0;
        if (nom == nm_pla){
            // nous avons trouvé la bonne planète
            var test = myworlds.children[i].children[0].href.split("?")[1].split("&");
            for each( var param in test){
...
function is_needed_page(page){
    var retour = false;
    if (page.indexOf("last_update_") != -1 ){    
        if (page.indexOf("resources") != -1){
            retour = true;
        }
        else if (page.indexOf("station") != -1){
            retour = true;
        }
        else if (page.indexOf("research") != -1){
            retour = true;
        }
    }
    return retour;
...
function change_page(){
    var diff= auto_change_page_max - auto_change_page_min;
    var time = Math.floor(Math.random() * diff) + auto_change_page_min;
    my_info("Chargement de la page "+next_page+" dans "+Math.round(time/1000)+" secondes");
    var udt_timestamp = new Date().getTime();
    udt_timestamp += time;
    my_write("nx_page_time",false,time);
    my_write("nx_page_url",false,next_page);
    setTimeout(set_current_page, time);
    refresh_asked = true;
    // masquage de l'écran, plus un bouton pour désactiver la prise de contrôle
    var alpha = 65;

    var test_node = document.createElement("div");
    test_node.style.width="100%";
    test_node.style.height="100%";
    test_node.style.backgroundColor='black';
    test_node.style.zIndex='5000';
    test_node.style.position="fixed";
    test_node.style.top="0px";
    test_node.style.left="0px";
    test_node.style.filter = 'alpha(opacity: '+alpha+')';
    test_node.style.opacity = alpha / 100;
    test_node.style.verticalAlign="middle";
    test_node.style.textAlign="center";
    test_node.style.paddingTop="10%"

    test_node.id='Locked';
    test_node.innerHTML="<span style='opacity:100'>La page est contrôlé par le script Ogame Tool.<br/>Décochez la case sur la page d'empire pour reprendre le contrôle manuel</span>";
    

    //my_info(test_node);
    document.getElementsByTagName("body")[0].appendChild(test_node)    

}
function set_current_page(){
    if (auto_refresh){
        window.location.href=next_page;
        my_info("Chargement de la page "+next_page+" maintenant");
    }
}

function check_new_message(){
    var msg_box = docId("message_alert_box");
    if (msg_box != null){
        my_info("Vous avez un nouveau message");
        var nb_old_msg = my_load("nb_msg",false,0);
        var nb_msg = parseInt(msg_box.children[0].innerHTML);
        if (nb_msg > nb_old_msg){
            snd_new_msg.play();
            my_write("nb_msg",false,nb_msg);
        }


    }
    else{
        my_info("Vous n'avez pas de nouveau message");
        my_write("nb_msg",false,0);
    }
}

function check_attack(){
    // vérifie si une attaque est en cours
    if (document.getElementById("attack_alert").style.visibility != "hidden"){
        // champs d'attaque en cours visible, alert sonore;
        my_info("Vous êtes attaquez");
        my_write("under_attack",false, true);
    }
    else{
        my_write("under_attack",false, false);
    }
}

function chk_attack_empire(){
    var under_attack = my_load("under_attack",false,false);
    if (under_attack){
        snd_attack.play();
        setTimeout(chk_attack_empire,refresh_time*60);
    }
    else
        setTimeout(chk_attack_empire,refresh_time);
}
...
if (location.href.split('/')[2].split('.')[1] != "tsuna"){
    ...
    load_ress_place();
    load_prod();
    setTimeout(update_info,500);
    setTimeout(update_ressource_dispo,250);
    check_price();
    check_new_message();
    check_attack();
    setTimeout(auto_update_page,100);
}
else{
