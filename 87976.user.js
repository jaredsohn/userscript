...
fileMETA["contributor"] = 'ale5000 (http://userscripts.org/users/ale5000)';
...
// @exclude        http://rlc.hostei.com/rlc_server_side/index.html
...
var check_all_links_keycode = "C".charCodeAt(0);
var makeTheCheckedLinksToFullURl_keycode = "X".charCodeAt(0);
var configuration_keycode = "Z".charCodeAt(0);
...
(function ()
{
	...
	function set_variables() {
		...
		Check_mediafire_dot_com_links = GM_getValue("Check_mediafire_dot_com_links", true);		
		...
		Check_ezyfile_dot_net_links = GM_getValue("Check_ezyfile_dot_net_links", true);
		Check_egoshare_dot_com_links = GM_getValue("Check_egoshare_dot_com_links", true);	
		Check_extabit_dot_com_links = GM_getValue("Check_extabit_dot_com_links", true);		
		Check_megashares_dot_com_links = GM_getValue("Check_megashares_dot_com_links", true);
		Check_oron_dot_com_links = GM_getValue("Check_oron_dot_com_links", true);
		Check_quickupload_dot_net_links = GM_getValue("Check_quickupload_dot_net_links", true);
		Check_x7_dot_to_links = GM_getValue("Check_x7_dot_to_links", true);
		Check_zshare_dot_net_links = GM_getValue("Check_zshare_dot_net_links", true);
		Check_uploaded_dot_to_links = GM_getValue("Check_uploaded_dot_to_links", true);
		Check_usaupload_dot_net_links = GM_getValue("Check_usaupload_dot_net_links", true);
		Check_filesend_dot_net_links = GM_getValue("Check_filesend_dot_net_links", true);	
		Check_files_dot_to_links = GM_getValue("Check_files_dot_to_links", true);	
		Check_freakshare_dot_net_links = GM_getValue("Check_freakshare_dot_net_links", true);		
		Check_ifile_dot_it_links = GM_getValue("Check_ifile_dot_it_links", true);
		Check_shareflare_dot_net_links = GM_getValue("Check_shareflare_dot_net_links", true);
		Check_solidfile_dot_com_links = GM_getValue("Check_solidfile_dot_com_links", true);
		Check_storage_dot_to_links = GM_getValue("Check_storage_dot_to_links", true);
		Check_superfastfile_dot_com_links = GM_getValue("Check_superfastfile_dot_com_links", true);
		Check_turbobit_dot_net_links = GM_getValue("Check_turbobit_dot_net_links", true);
		Check_turboshare_dot_com_links = GM_getValue("Check_turboshare_dot_com_links", true);
		Check_turboupload_dot_com_links = GM_getValue("Check_turboupload_dot_com_links", true);
		Check_megaftp_dot_com_links = GM_getValue("Check_megaftp_dot_com_links", true);
...
function fixurl(urll,site){
		...
		function getuplinkschecked(allup,times) {
			...
			for (var i = 0; i < allup.length; i++) { 
				...
				//var upurl = 'http://uploading.com/filechecker/?JsHttpRequest='+randomnumber+'-xml';
        ...
        GM_xmlhttpRequest({
						...
						if (res.match(randomnumber)){
                          ...
                          livelink = res.match(/target="_blank">(http:\/\/uploading\.com\/files\S*?)\/<\/a><\/td><td>(?:Active|Aktiv|Actif|Ativar|Activo|Actief|Ενεργός|Aktiivinen|Attivo|Aktivní|Aktívny|Aktywny|Активный|Etkin|アクティブ|活跃)<\/td><td>(size|\d.*? .*?)<\/td>/gi); 
							...
							if (livelink) {
								...
								for (var i = livelink.length - 1; i >= 0; i--) {
									var string = livelink[i];
									var regex2 = /target="_blank">http:\/\/uploading\.com\/files(\S*)\/<\/a><\/td><td>(?:active|Active|Aktiv|Actif|Ativar|Activo|Actief|Ενεργός|Aktiivinen|Attivo|Aktivní|Aktívny|Aktywny|Активный|Etkin|アクティブ|活跃)<\/td><td>(size|\d.*? .*?)<\/td>/;
									//livelink = res.match(/target="_blank">(http:\/\/uploading\.com\/files\S*?)\/<\/a><\/td><td>(?:Active|Aktiv|Actif|Ativar|Activo|Actief|Ενεργός|Aktiivinen|Attivo|Aktivní|Aktívny|Aktywny|Активный|Etkin|アクティブ|活跃)<\/td><td>(size|\d.*? .*?)<\/td>/gi); 
                                    matchArraylive = string.match(regex2);
        ...
        function DiplayTheDeletedLinks(fotfoundlinks) {
			...
			for (var i = 0; i < allLinks.snapshotLength; i++) {
				...
				if (Show_black_background_in_dead_links) {
					...
					deadlink.innerHTML = " !~~DEAD LINK~~!";
					thisLink.parentNode.insertBefore(deadlink, thisLink.nextSibling);
				}
					if (No_live_links) {	
    ...
    function PreKeyCheck(ev){
		...
		//alert(keycode);
			if (keycode == 16){ // 16=Shift 17=Ctrl
	...
	function KeyCheck(ev){	
       ...
       if(Add_menu_commands) {
         if (Keyboard_functions){
			document.addEventListener('keydown', PreKeyCheck, false);        
		    GM_registerMenuCommand("[Rapidshare Links Checker] Check The Links In This Page (Shift+"+String.fromCharCode(check_all_links_keycode)+")", check_all_links);
		    GM_registerMenuCommand("[Rapidshare Links Checker] Configuration  (Shift+"+String.fromCharCode(configuration_keycode)+")", configuration);
		    GM_registerMenuCommand("[Rapidshare Links Checker] Make The Links To Full Urls  (Shift+"+String.fromCharCode(makeTheCheckedLinksToFullURl_keycode)+")", makeTheCheckedLinksToFullURl);
	    }else{         
...
<div id="sites"><span id="Check_extabit_dot_com_links"></div></span>\
<div id="sites"><span id="Check_filebase_dot_to_links"></div></span>\
<div id="sites"><span id="Check_file2box_dot_com_links"></div></span>\
...
<div id="sites"><span id="Check_freakshare_dot_net_links"></div></span>\
<div id="sites"><span id="Check_getthebit_dot_com_links"></div></span>\
<div id="sites"><span id="Check_gigapeta_dot_com_links"></div></span>\
...
<div id="sites"><span id="Check_imdb_dot_com_rate"></div></span>\
...
<div id="sites"><span id="Check_rayfile_dot_com_links"></div></span>\
<div id="sites"><span id="Check_sendspace_dot_com_links"></div></span>\
<div id="sites"><span id="Check_save_dot_am_links"></div></span>\
...
<div id="sites"><span id="Check_shareflare_dot_net_links"></div></span>\
<div id="sites"><span id="Check_solidfile_dot_com_links"></div></span>\
...
<div id="sites"><span id="Check_turboshare_dot_com_links"></div></span>\
...
<div id="sites"><span id="Check_x7_dot_to_links"></div></span>\
...
<div id="sites"><span id="Check_addyour_dot_own_links"></div></span>\
		...
		for (var i = existingobject.length - 1; i >= 0; i--) {
			...
			//existingobject[i].innerHTML = existingobject[i].innerHTML.replace(/eyboard/,'Remo')
			
			document.getElementById(existingobject[i].id).addEventListener("click", changeConfiguration, false);
		}
	
        document.getElementById('Keyboard_functions').innerHTML +='<br/>Shift+'+String.fromCharCode(configuration_keycode)+'=Configuration <br/>Shift+'+String.fromCharCode(makeTheCheckedLinksToFullURl_keycode)+'=Convert links to full address links <br/>Shift+'+String.fromCharCode(check_all_links_keycode)+'=Check all the links (for ajax sites)';
...
Convert_links_to_full_address_links = GM_getValue("Convert_links_to_full_address_links", false);
		...
		var freakshare_net= new Array(6) //http://www.files.to/get/400120/lioil1on9p
		 ...
		 freakshare_net[5]='optional--'; //tos violation
		...
		var filebase_to= new Array(6)   //http://filebase.to/files/148800/coa-tgg-xvid.avi
		 ...
		 filebase_to[1]='Choose your Download Option';                                    // file_is_alive
		...
		var gigapeta_com= new Array(6)  //http://gigapeta.com/dl/630020a54a8ec
		 ...
		 gigapeta_com[1]='Размер';                           // file_is_alive
		...
		var letitbit_net= new Array(6)  //http://letitbit.net/download/a884f3171672/Hero.S03E19.avi.html
		 ...
		 letitbit_net[6]='Size of file::<\/span> (\\d.*?)<\/h1>';   // size
		...
		var imdb_com= new Array(6)  
		 imdb_com[0]='(|www\.)imdb\.com\/title\/tt';                //name and search string in link to get description
		 imdb_com[1]='starbar-meta';                              // file_is_alive
		 imdb_com[2]='optional--';                                  // file_is_dead
		 imdb_com[3]='optional--';                       // no download slots or temporarily unavailable or servererror
		 imdb_com[4]="//a[contains(@href,'imdb.com/title/tt')]";  
		 imdb_com[5]='optional--';   
		 imdb_com[6]='<b>(\\d.*?\/10)<\/b>';   // size
		...
		var superfastfile_com= new Array(6)  
		 ...
		 superfastfile_com[6]='color=\"#000000\">(\\d.*?)<\/td><\/tr><\/table>';
		...
		var shareflare_net= new Array(6)
		 ...
		 shareflare_net[1]='need-cufon';                              // file_is_alive
		 ...
		 shareflare_net[5]='optional--';   
		 shareflare_net[6]='<small>\\((\\d.*?)\\)<\/small>';   // size
		...
		var turbobit_net= new Array(6)  // is mulfunctioning
		 ...
		 turbobit_net[1]='(Размер файла)|( Free downloading )';                              // file_is_alive
...
// ONLY FOR co_cc_vault USAGE ------------END 
	...
	var http_file_hosts_all = [bigandfree_com,netload_in,easy_share_com,mediafire_com,megashares_com,rapidshare_de,vip_file_com,yourfilehost_com,sendspace_com,gigasize_com,fileop_com,duckload_com,file2box_com,filebase_to,link_protector_com,bitroad_net,filenavi_com,uploadbox_com,letitbit_net,x7_to,zshare_net,uploaded_to,filesend_net,ifile_it,solidfile_com,shareflare_net,storage_to,megaftp_com,co_cc_vault,rapidshare_com,megaupload_com,filefactory_com];
	var totalxpath='';
	var totalourls='';
  ...
  for (var i = 0; i < allliveLinks.snapshotLength; i++) {
       ...
       if (thisLink.innerHTML.match(/User Rating/)) return;
           ...
           thisLink.id = 'alive_link';
									...
									if  (thisLink.href.match(/imdb.com\/title/))  {
										    thisLink.innerHTML = thisLink.innerHTML+ " [User Rating:" + fileSize+"]";
										    thisLink.id = 'imdb_link';
										    var instead_of_title = "User Rating: " + fileSize;
