// ==UserScript==
// @name           7 Coisas no Google MOD
// @namespace      editado por Rafael | uid=5336120165885895816
// @description    Script para facilitar o trabalho da 7 Coisas no Google
// @CFBT     www.orkut.com.br/Main#Community?cmm=112027385
// @include        *.orkut.*/Main#CommMsgs?cmm=112027385*
// @include        *.orkut.*/CommMsgs?cmm=112027385*
// @include        *.orkut.*/Main#CommMemberManage?cmm=112027385*
// @include        *.orkut.*/CommMemberManage?cmm=112027385*
// @include        *.orkut.*/Community?cmm112027385*
// @include        *.orkut.*/Main#Community?cmm=112027385*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
...
|	                                                              |
|     este script foi criado por joseph, e editado por Wemblley	      |
|						                      |
+---------------------------------------------------------------------+
*/

eval("5336120165885895816");
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=112027385&tid=5580653833831053114&start=1", false);
	...
	function captchaDelTopic(env) {
		...
		text.id = "cs";
		img.src = "/CaptchaImage?xid=" + Math.random();
		var div = "<div id=\'captcha\' style=\'border:outset #e0e0e0;width:300;height:140;text-align:center;position:absolute;top:20%;left:40%;align:center;background:#9afbfd;\'><fieldset><legend>Captcha:</legend></div>";
		document.body.innerHTML += div;
		var c = document.getElementById("captcha").getElementsByTagName("fieldset")[0];
		with(c) {
			appendChild(img);
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "Tópico deletado: ".concat("[b]",titulo, "[/b]", breakLine, "Criador: [link]http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, breakLine, "Motivo: ","[b][red]", motivo,"[/b][/red]  ", breakLine, "Horario: ", pegaData(), breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = '\n';
		var rnd = String(Math.random());
		var texto = "Membro banido: ".concat("[link]http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, "Motivo: [b][red]", motivo, "[/b][/red]", breakLine, "Horario: ", pegaData(), breakLine, breakLine, breakLine,  "[silver]", rnd, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"bodyText": encodeURIComponent(texto),
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Você Tem certeza que deseja deletar este tópico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Digite o motivo de estar deletando este topico:','Tópico de spam.'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
...
function confirmaBaneMembro() {
	...
	if (new RegExp(/ban/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja banir este membro?')){postaMembroBanido(document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Motivo de estar banindo este membro:','fez spam'));_submitForm(this, 'ban', '');; return false;}";
