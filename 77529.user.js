// ==UserScript== 
// @name        Facebook Autopoke Pirate Edition
// @author      Michael Soh (Pirate Language modifications by Dan Cottam)
// @namespace   autopoke_5200 
// @description Automatically pokes back pokers listed on your home page. This script was inspired by Lukas Fragodt's Auto-Poke and EZ-Poke. This script is based on Michael Soh's Facebook Autopoke. (modified to work on Pirate language facebook 
// @version     3.5 Pirate Edition (31 Oct 2010)
// @license     GPL 3.0 
	  ...
	  data:poke_post_data,
          onload: function(response) { 
               if (response.status == 200) { 
                    if (response.responseText.indexOf('Ye be about to skewer') != -1) { 
          ...
          onload: function (response) { 
               if (response.status == 200) { 
                    ...
                    if (response.responseText.indexOf('has not received your last poke yet') != -1) { 
                         poke_node.removeAttribute('href'); 
                         poke_node.innerHTML = 'Already poked!'; 
                    } else if (response.responseText.indexOf('Ye be skewerin') != -1) { 
                         poke_node.removeAttribute('href'); 
