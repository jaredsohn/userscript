// ==UserScript==
// @name           Crissbraga Facebook: Convida Todos
// @name           Botão Adicionar todos Amigos
// @namespace      Aspi
// @description    Adiciona um Botão para adicionar todos os amigos a um evento do Facebook.
...
(function () {
	...
	function addButton(inviteBoxParameter) {
		...
		if (inviteBox !== undefined && !executing && !document.getElementById('selectAllButton')) {
			...
			function autoScroll(alignToTop, callback) {
			...
			// Create it's label.
			selectSpanTypeCheckBoxLabel = createLabel('Type?', selectSpanTypeCheckBox);
