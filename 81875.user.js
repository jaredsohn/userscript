// ==UserScript==
// @name           Gangwars Endless2 Bullen
// @namespace      http://www2.gangwars.de
// @include        http://www2.gangwars.de/gangwars/gebiete.php
// @include        http://www2.gangwars.de/gangwars/gebiete.php*
...
DataStorage = {
	...
	init: function() {
		if (DataStorage.hasLocalSQLiteStorage()) {
			try {
				DataStorage.database = openDatabase("gangwars_storage", "1.0", "Gangwars Endless 2 Storages");
				if (!DataStorage.database) {
					alert("Failed to open the database on disk. This is probably because the version was bad or there is not enough space left in this domain's quota");
				} else {
					DataStorage.database.transaction(function(tx) {
						tx.executeSql("SELECT COUNT(*) FROM gangwars_storage", [], function(result) {}, function(tx, error) {
							tx.executeSql("CREATE TABLE gangwars_storage (id TEXT UNIQUE, value TEXT, unixtime REAL)", [], function(result) {});
						});
					});
				}
			} catch (err) {}
	...
	set: function(key, value) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			...
			DataStorage.database.transaction(function(tx){
				tx.executeSql('INSERT INTO gangwars_storage (id, value, unixtime) VALUES (?, ?, ?)', [key, value.toSource(), 0]);
			});
		} else {
	...
	get: function(key) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			var value = null;
			DataStorage.database.transaction(function(tx){
				tx.executeSql('SELECT * FROM gangwars_storage WHERE id = ?', [key], function(result){
	...
	setCache: function(key, value, unixtime) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			DataStorage.remove(key);
			DataStorage.database.transaction(function(tx){
				tx.executeSql('INSERT INTO gangwars_storage (id, value, unixtime) VALUES (?, ?, ?)', [key, value.toSource(), parseInt(unixtime)]);
			});
		} else {
			unsafeWindow.sessionStorage.setItem(DataStorage.getHostname() + '::' + key, value.toSource());
	...
	remove: function(key) {
		if (DataStorage.hasLocalSQLiteStorage()) {
			DataStorage.database.transaction(function(tx){
				tx.executeSql('DELETE FROM gangwars_storage WHERE id = ?', [key]);
			});
		} else {
...
var gw_urls = {
	police: 'http://www2.gangwars.de/gangwars/police.php'
};
