...
/*	#planet .slot {
	...
	div_parent.style.right = "5px";
	div_parent.style.bottom = (20 + 50*nb_divs) + "px";
	document.getElementById("planet").appendChild(div_parent);

	var div1 = document.createElement("div");
	div1.className = "fleft tips";
//	.fleft {    float:left;}
	div1.style.paddingLeft = "9px";
	div1.title = "|Small cargos necessary to empty the colony";
	div1.innerHTML = format(sc, 0, '.') + " Cargo leggeri";// + " small cargo" + (sc>1?"s":"");
...
//	.fright {   float:right;}
	div2.style.paddingRight = "12px";
	div2.title = "|Large cargos necessary to empty the colony";
	div2.innerHTML = format(lc, 0, '.') + " Cargo Pesanti";// + " large cargo" + (lc>1?"s":"");
	...
	script.text = "$('#planet .slot .tips').cluetip({splitTitle:'|',showTitle:false,width:300,positionBy:'auto',leftOffset:80,topOffset:60,cluezIndex:9997,hoverIntent:{sensitivity:1,interval:250,timeout:400}});";
