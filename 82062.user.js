// ==UserScript==
// @name           JustOneClick for GagaBux
// @namespace      http://www.youtube.com/user/bath4rakala
// @description    An Auto-click for GagaBux, makes you validated without viewing the ads. Ads loaded faster, because this script ignores all tags. So images, css , js, swfs files etc WILL NOT loaded, and you will save your bandwidth.
// @include        *://*gagabux.com/*
// @copyright      PTCSPY
// ==/UserScript==
...
nocaptcha = function(){
	...
	if(div) {
		try {
			...
			alert('Failed to bypass captcha !');
			return
		};
		div2 = document.createElement('div');
		div2.innerHTML = "Bypassing captcha, please wait...";
		div.parentNode.parentNode.replaceChild(div2, div.parentNode);
		ajx.onreadystatechange = function() {
			try {
				if(ajx.readyState == 4) {
					if(ajx.status == 200) {
						div2.innerHTML = "Captcha bypassed, you dont need that anymore";
						...
						div2.innerHTML = "Connection Error, retrying...";
						nocaptcha()
					}
				}
			}
...
autosurf = false;
if(correctURL(/gagabux\.com\/ads\.php/)) {
	if(login) {
		...
		html += "</style>";
		html += "<div id='JustOneClick'><div id='JustOneClick-title'>JustOneClick for GagaBux</div><br /><div id='JustOneClick-msg' align=center>";
...
else if(correctURL(/gagabux\.com\/register\.php/)) {
...
else if(correctURL(/gagabux\.com\/prefs\.php/)) {
	try {
		byName('apemail')[0].readOnly = false;
		byName('ppemail')[0].readOnly = false
	}
	catch(e){}
}
else if(correctURL(/gagabux\.com\/forum/)) {
	try {
