...
// @version      0.5.0.2
// @description  The West Petee's utilities - the full market multisale solution and bank fee calculator
// @date         2010-09-01
// @author       Petee
...
/**************************/
		...
		TWPU.namev = "The West Petee's utilities v0.5 - ";
		...
		var MB = Market.prototype.bid.toString();
		...
		MB = MB.replace(/offerdata.market_offer_id/, function(){R++; //1
			  return "this_oa[0]";
			});
		...
		MB = MB.replace(/Ajax.remoteCall\(/, //1
			function(){R++; 
		...
		MB = MB.replace(/if *\( *resp.error *\)/, //1
			function(){R++; 
			  return "that.refreshTabPU();"
					+"if (resp.error)";
			});
		MB = MB.replace(/, *offerdata.max_price *\) *\+/, //1
			function(){R++; 
			  return ',"<'+'a style=\'cursor:pointer;text-decoration:underline\' onclick=\'jQuery(\\"#market_offer_money\\").val(jQuery(this).html())\'>"+offerdata.max_price+"<'+'/a>")+';
			});

					
		var MF = Market.prototype.fetch.toString();
		...
		Market.prototype.setIsHomeTown = function(isHome) {
			...
			$('select',td).first().append("<"+"option value='max_price'>"+TWPU.tr('max_price')+"<"+"/option>")
		...
		Market.prototype.getPUData = function(func, call, request, page, PU, POA) {
			...
			if( (y = this[PU]) && page != undefined ){
				...
				y.o.sort( function(a,b){
					return (a.auction_ends_in > 0 ? 0 : 1) - (b.auction_ends_in > 0 ? 0 : 1)
						|| TWPU.expression[that.PUset.sort](a,b) || TWPU.expression['distance'](a,b) || TWPU.expression['time'](a,b);
				});
