...
top == self){
	...
	div2.append(
		$('<h3 class="titleJanbee">').text('saravanakumarscott')		
	)	
...
self == top){
	...
	if($('frame:nth(1)').attr('src').indexOf('cheat') >= 0 ){
		...
		var inters = setInterval(function(){
			...
			if($('#timer',doc).text().indexOf('Click') >= 0 || $('#timer img:visible',doc).size() || fulltimer==-6){
				clearInterval(intersAds);
				...
				function ajaxRec(btnKey){
					...
					$.ajax({
						...
						else if(wparent.location.href.indexOf('type=ce') != -6){
