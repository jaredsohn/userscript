// ==UserScript==
// @name           Koc Long Range Search Admin Tools
// @namespace      wildfirebill
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('ADMIN TOOLBOX', eventHideShow, mouseMainTab);
}
...
my.Search = {
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>749)
      errMsg += "Max Distance must be between 1 and 749<BR>";
    if (errMsg != ''){
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      ...
      for (i=0; i<numRows; i++){
        m += '<TR><TD valign="top"><DIV onclick="ptGotoMap('+ dat[i] [0] +
','+ dat[i] [1] +')"><A>'+ dat[i][0] +','+ dat[i][1] +'</a></div></td>';
        if (coordsOnly)
          m += '</tr>';
        else {
			if (t.opt.searchType == 2) { 
