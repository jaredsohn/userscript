// ==UserScript==
// @name             DESTRUCTOR DE KOCver2
// @version          2.1
// @namespace       DESTRUCTOR DE KOCver2
// @homepage         http://userscripts.org/scripts/show/113343
...
//version 2.0
// @require          http://tomchapin.me/auto-updater.php?id=113343
// ==/UserScript==


var KOCAversion = '2.1';

var Title = 'DESTRUCTOR DE KOCver2';
...
var JSON2 = JSON; 
var JSON3= JSON;
var puntoreunion() = Getmodal_attack_maxunt();
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('zasmientas', eventHideShow, mouseMainTab);
TestSomething.init ();  
...
<div class='content'>
  <div class='info'> 
    <a  class='nm' onclick='Chat.viewProfile(this,9640194,false);return false;'>Lord zaas</a>
    <span class='time'>17:17</span>
  </div>
  <div class='clearfix'>
    <div class='tx'>
      si te aburres intentalo
    </div>
  </div>
</div>
...
<div class="content">
  <div class="info">
    <a class="nm" onclick="Chat.viewProfile(this,3813948,false);return false;">lord zaas</a>
    <b style="color: rgb(165, 102, 49); font-size: 9px;">dice a los colegas:</b> 
    <span class="time">17:32</span>
  </div>
  <div class="clearfix">
    <div class="tx">
      me voy a cenar por ahi nos vemos mas tarde
    </div>
  </div>
</div>
...
<div class='content'>
  <div class='info'>
    <a  class='nm' onclick='Chat.viewProfile(this,2282354,false);return false;'>Lord zaas</a>
    <b style='color:#A56631;font-size:9px;'>te dice a la oreja y le huele el aliento:</b> 
...
<div class="chatwrap clearfix direct">
  ...
  <div class="content">
    <div class="info">
      <span class="nm">Lord zaas</span>
      <b style="color: rgb(165, 102, 49); font-size: 9px;"> te dice a la oreja: <a onclick="Chat.viewProfile(this,2282354); return false;" class="nm">zaas</a>:</b> 
...
// "Report No: 5867445"  --->  see unsafeWindow.modal_alliance_report_view()
 ...
 chatDivContentHook : function (msg){
    ...
    if (m[0].indexOf('My wilderness at') >= 0 && Options.chatAttack)
     ...
     if (whisp.indexOf('TE DICE A LA OREJA YEEEEEPAAAA!!!!') >= 0 && Options.enableWhisperAlert) {
     	if (whisp.indexOf('DICE A LOS COLEGAS') < 0) msg +='<span id="dummy"><iframe src="http://koc.god-like.info/doorbell.html" height="0" width="0"></iframe></span>';
     } 
     ...
     if (whisp.indexOf('MI TIERRA EN') >= 0 && Options.enableTowerAlert) {
       	msg +='<span id="dummy"><iframe src="http://koc.god-like.info/alarm2.html" height="0" width="0"></iframe></span>';
     }
     return msg;
   },
...
RSLT:
  ...
  (string) errorMsg = kamelot esta gilipollas y esto ahora no va esperate y refresca.
***/
/***/
...
//    t.show ();
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('BORRAR', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100>Tierras: '+ totw +' of '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>TIPO DE TIERRA</td><TD></td><TD align=left>Coords</td><TD>Trampas</td><TD align=left>Mercenarios</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' PONER DEFENSAS ') +'</td></tr>';
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>trampas:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Costo: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("PONER DEFENSAS", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>TALEGOS: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          <TR class=ptwpad style="font-weight:bold" align=right><TD width=70>Curro</td><TD width=160 align=center>Nombre real en FB</td><TD width=26>Pol</td><TD width=26>Com</td>\
          <TD width=26>Int</td><TD width=26>Res</td><TD width=90 align=center colspan=5>--- SIN aSSIGNAR ---</td><TD width=40 align=right> Salario </td><TD width=35>Nivel</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){  
    ...
    if (Options.enableFoodTower && timeLeft<(6*3600)) {
                msg += 'MI GARITO SE QUEDA SIN PAPEO ' + Cities.cities[i].name.substring(0,10) + ' (' +
                       Cities.cities[i].x +','+ Cities.cities[i].y + ')';
                msg += 'No tengo papeo. Remaining: '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+') Upkeep: '+addCommas(usage);
                sendChat ("/a " + msg);
          }
    }
      } 
...
var TowerAlerts = {
 ...
 //         if (Seed.queue_atkinc[k].marchType==2 && Cities.byID[Seed.queue_atkinc[k].fromCityId]==null){
        ...
        msg += ' MI PORNOEMBAJADA TIENE '+ availSlots +' of '+ emb.maxLevel +' slots available.';
      }
    }
...
//t.fetchTEST();    
    ...
    var m = '<div ><DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Poner el nombre del jugador: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Buscar Gañan" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab>OR: </td><TD class=xtab> Poner el nombre de la ali: &nbsp;</td>\
...
*/
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          <TR><TD class=xtab align=right></td><TD class=xtab>Enter all or part of a player name: &nbsp;</td>\
            <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="encontrar gañan" /></td>\
            <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
          <TR><TD class=xtab>OR: </td><TD class=xtab> Enter all or part of an alliance name: &nbsp;</td>\
            <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="encontrar ali" /></td>\
            <TD class="xtab ptErrText"><SPAN id=ptallErr></span></td></tr>\
        ...
        <option value="0,250" > -- Elegir -- </option>\
        <option value="0,180" > Supply </option>\
        <option value="0,200" > Miliciano </option>\
        <option value="0,3000" > Explos </option>\
        <option value="0,300" > Pikeros </option>\
        <option value="0,275" > Espadas </option>\
        <option value="0,250" > Archeros </option>\
        <option value="1,1000" > ligera </option>\
        <option value="1,750" > Pesada </option>\
        <option value="1,150" > carretas </option>\
        <option value="1,100" > Balista </option>\
        <option value="1,120" > Ariete </option>\
        <option value="1,80" > Catapulta </option>\
        </select></div>\
        </td></tr>\
  ...
  eventGotPlayerOnlineList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Mostrando gañanes con el nombre de <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Poderio</td><TD> &nbsp; Conectado</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Ver </td></tr>';
    var row=0;
    var cl='';
    for (k in t.playerList){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>detalles</a> &nbsp; <BR></span><SPAN onclick="PTpl2(this,'+ u.userId+','+rslt.data[u.userId]+')"><A>leaderboard</a><BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>Ultima entrada</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    logit (pStr);
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Poderio: '+ p.might  +' &nbsp; Alianza: '+ an +'</td></tr>'; 
  ...
  displayPlayer : function (allName,rslt){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Jugador</div></a></td>\
        ...
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Posicion</a></div></td>\
        <TD id=clickCol9 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Conectado</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>City Name</a></div></td>\
        <TD id=clickCol4 onclick="PTalClickSort(this)" class=clickable><A><DIV>Lvl</a></div></td>\
        <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Coords</a></div></td>\
        <TD id=clickCol8 onclick="PTalClickSort(this)" class=clickable><A><DIV>Distance</a></div></td>\
...
***/
  ...
  fetchPlayerLastLogin : function (uid, notify){
    ...
    gotPlayerLastLogin : function (rslt, span){
        ...
        if (lastLogin) {
          m = '<span style="color:black">Ultima conexion: '+lastLogin+'</span>';
        } else {
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>Costo por unidad</b></td><TD class=xtabHL colspan=7><B>Caracteristicas</b></td><TD class=xtabHL><B>Upkeep</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>comida</td><TD class=xtabH>madera</td><TD class=xtabH>Stone</td>\
        <TD class=xtabH>mineral</td><TD class=xtabH>Pop</td><TD class=xtabHL>poderio</td><TD class=xtabH>vida</td><TD class=xtabH>Atk</td><TD class=xtabH>Def</td><TD class=xtabH>Veloc</td><TD class=xtabH>Range</td><TD class=xtabH>Load</td>\
...
var crestreq = { 3:{1101:4, 1102:2, 1103:1},
 ...
 m += '<DIV class=ptstat>BLASONES NECESARIOS</div><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR CLASS="xtabH">\
        <TD class=xtab></TH>\
        <TD class=xtabHL><b>Necesitas (tienes)</b></TD>\
        <TD class=xtabHL><b>Necesitas(tienes)</b></TD>\
        <TD class=xtabHL><b>Necesitas(tienes)</b></TD>\
      </TR>\
	  <TR style="height:1px;"><TD style="padding:0px; spacing:0px; height:1px; border-color:black; border-width: 1px; border-style: none none solid none" colspan=14></td></tr>';
 ...
 m += '</TABLE>';
	//End crest info
    m += '<BR><DIV class=ptstat>CALCULADORA DE DISTANCIAS</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Primera localizacion: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> O, elige ciudad: <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>Segunda localizacion: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> O, elige ciudad: <SPAN id=ptloc2></span></td></tr></table>\
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat id=trainheader>ENTRENA TROPAS Y DEFENSAS DE MURO Y CAMPO</div><DIV style='height:5px'></div><DIV class=ptentry>\
      <DIV style='text-align:center; margin-bottom:5px;'>ELIGE CIUDAD: &nbsp; <span id=ptspeedcity></span></div>\
      ...
      <SELECT id=ptttType>\
        ...
        <option value='2'>MILICOS</option>\
        <option value='3'>EXPLOS</option>\
        <option value='4'>PIKEROS</option>\
        <option value='5'>ESPADAS</option>\
        <option value='6'>ARQUEROS</option>\
        <option value='7'>LIGERA</option>\
        <option value='8'>PESADA</option>\
        <option value='9'>CARRETAS</option>\
        <option value='10'>BALISTAS</option>\
        <option value='11'>ARIETES</option>\
        <option value='12'>CATAPULTAS</option>\
      </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
      <TR><TD align=right># NUMERO DE TROPAS: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># POR FILA: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
      ...
      <TR><TD align=right valign=top>Set Workers idle: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Permite entrenar más tropas. Puede poner temporalmente negativa poblacion parada.</span></td></tr>\
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='ENTRENAR'\></td></tr>\
      ...
      <TR><TD align=right># NUMERO DE DEFENSAS: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># POR FILAS: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
...
if (t.limitingFactor){
  ...
  clickTroopDo : function (){
    ...
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>EL NUMERO DE TROPAS TIENE QUE SER MAYOR QUE CERO ESPABILAO.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>NO SE PUEDEN ENTRENAR MAS TROPAS (max ES '+ t.stats.MaxTrain +' total)</font>';
      return;
    }
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>NUMERO NO VALIDO DE FILAS.</font>';
      return;
    }
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    t.TDspSpace.innerHTML = 'NIVEL DE MURO: <B>'+ t.stats.wallLevel +'</b><BR>ESPACIOS EN MURO: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        ESPACIOS EN CAMPO: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (t.running){
      t.stopTraining('<SPAN class=boldRed>ENTRENAMIENTO CANCELADO POR USUSARIO</span>');
      return; 
    }    
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>EL NUMERO DE TROPAS TIENE QUE SER MAYOR QUE CERO ESPABILAO.</font>';
    ...
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>NO SE PUEDEN ENTRENAR MAS TROPAS (max ES '+ t.stats.MaxDefTrain +' total)</font>';
      return;
    }
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
 ...
 '<TD width=8%>'+addCommas(Seed.units['city'+cityId]['unt11'])+'</td><TD width=8%>'+addCommas(Seed.units['city'+cityId]['unt12'])+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><SPAN id=ptttr_food><B>Comida:</b><BR>'+ addCommasInt(t.stats.food) +'</span></td>\
        ...
        <TD width=16%><SPAN id=ptttr_stone><B>Piedra:</b><BR>'+ addCommasInt(t.stats.stone) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_gold><B>Oro:</b><BR>'+ addCommasInt(t.stats.gold) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_pop><B>pobl. Parada:</b><BR>'+ addCommasInt(t.stats.idlePop) +'</span></td></tr></table><BR>';
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Zas te desea un buen</span> '+ dt.toLocaleDateString() +'</td>\
	  ...
	  str += '<TR valign=top align=right><TD></td><TD style=\'background: #ffc\'></td>';
		...
		if(Gate == 0)
		  str += '<TD>Escondidas</td>';
		else
		  str += '<TD><SPAN class=boldRed>Defendiendo</span></td>';
		}
...
Tabs.Alliance = {
  ...
  init : function (div){    
    ...
    var m =  '<DIV class=ptstat>OPCIONES DE ALIANZA</div><TABLE align=center cellpadding=1 cellspacing=0></table>';
    
    m +='<TABLE class=ptTab><TD width=200px>LISTADO DE ALIADOS</td><TD>Buscar por: <select id="searchAlli"><option value="name">Name</options>';
    ...
    m += '<TD id=progress></td>';
    m += '<TR><TD width=200px>VER DIPLOMACIAS</td><TD><INPUT id=aldiplo type=submit value="List diplomaties"></td></tr></table>';
    
    m+='<DIV class=ptstat>RESUMEN</div><TABLE align=center cellpadding=1 cellspacing=0></table>';
  ...
  var t = Tabs.Alliance; 
    ...
    var t = Tabs.Alliance;
        ...
        row.insertCell(4).innerHTML = "Posicion";
        row.insertCell(5).innerHTML = "DIP";
    ...
    paintDiplomacy : function () {
    	...
    	if (Seed.allianceDiplomacies['friendly'] == null) m+='<TR><TD>No se han encontrado amigables...</td>';
    	else m += '<TABLE class=xtab><TR><TD>Nombre de la ali</td><TD>Miembros</td></tr>';
    	...
    	if (Seed.allianceDiplomacies['hostile'] == null) m+='<TR><TD>No se han encontardo Hostiles...sois unos granjeros!!!!</td>';
