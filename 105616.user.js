...
// @include        http://*.nexon.com/big_map.php*
...
var FLAG3_1 = "010000악마님 사랑해요";
var FLAG3_2 = "010000악마님 존경해요";
var FLAG3_3 = "010000악마님 님좀짱인";
var FLAG4   = "";
...
// 検색옵션(특수1)
var FLAG2_EMPTY = 0;            // 빈영지検색
var FLAG2_LAND = 1;             // 영토検색
var FLAG2_ANALYZE = 2;          // 자원판별検색
var FLAG2_RADER = 3;            // レーダーの表示
var FLAG2_OWNER = 4;            // 本拠地一覧の表示
var FLAG2_EMPTY_DRAW = 5;       // 빈영지색칠
var FLAG2_EMPTY_DRAW_COLOR = 6; // 빈영지枠색
var FLAG2_STRONG_AREANO = 7;    // 強조표시영지리스트차례号(3桁)
var FLAG2_STRONG_AREANO2 = 10;  // 強조표시영지리스트차례号(3桁)
var FLAG2_STRONG_AREANO3 = 13;  // 強조표시영지리스트차례号(3桁)
var FLAG2_MENU2 = 16;           // 메뉴ー클로즈ー스위치2
var FLAG2_MENU4 = 17;           // 메뉴ー클로즈ー스위치4
var FLAG2_HFUNC = 18;           // 
var FLAG2_SEARCH_LEVEL = 19;    // ルート탐색：탐색레벨
var FLAG2_SKIP_RES = 20;        // ルート탐색：회피자원타입
var FLAG2_SKIP_USER = 21;       // 개인영지를통과
var FLAG2_SKIP_GROUP = 22;      // グルー그룹영지통과
...
// 개인/동맹検색결과표시색
var cname = ["빨강","보라","분홍","주황","노랑","녹색","청록","파랑","회색","흰색"];
...
// ルート探索条件
var croutename = ["☆1만","☆2이하","☆3이하","☆4이하","☆5이하","☆6이하","☆7이하","☆8이하","☆무관"];
var croutelim = ["없음","★2숲","★2나무","★2철"];
var groupname = ["무관","No.1","No.2","No.3"];
...
function getNPCCastleInfo() {
	...
	for (var i=0; i<areas.snapshotLength; i++) {
		...
		var linkText = '/' + pictImg + '<a href="' + rowText2 +  '" style="text-decoration: none"><font color="#00BFFF"><b>' + valueA[3] + '</b></font></a>' + '/거리 ' + valueA[6];
				...
				if(    ((chkflg1.charAt(FLAG1_LV1) == '1') && (landLevel == 1))
					...
					if( valueA[5] != "" ){
						if( ikibaku_flg == 1 ){
							...
							putText = putText + groupTxt + '영토/' + userTxt + '/' + valueA[0] + linkText + '/' + valueA[5] + ' 숲' + valueA[7] + ' / 바위' + valueA[8] + ' / 철' + valueA[9] + ' / 곡물' + valueA[10];
						}
					}
					else{
		...
		if( (chkflg2.charAt(FLAG2_EMPTY) == '1') && (valueA[1] == '') ){
			...
			if(    ((chkflg1.charAt(FLAG1_LV1) == '1') && (landLevel == 1))
				...
				if( ikibaku_flg == 1 ){
					...
					putText = putText + '공터★' + landLevel + linkText + ' / 숲' + valueA[7] + ' / 나무' + valueA[8] + ' / 철' + valueA[9] + ' / 곡물' + valueA[10];
				}
	...
	if( chkflg2.charAt(FLAG2_OWNER) == '1' ){
		...
		if( outText != '' ){
			baseText.snapshotItem(0).innerHTML = '<font color="brown"><b>본거지 목록</b></font><br>' + outText;
		}
	}
	else{
		var baseinfodv = $e('//*[@id="baseinfo_dv"]');
...
function addHtml() {
	...
	textLabel.innerHTML = "<br><b>NPC요새, 영토정보검색툴 Ver." + VERSION + "</b>";
	...
	textLabel.id = "test";
	textLabel.innerHTML = "<font color=\"red\"><b>검색대상</b></font>";
	...
	textLabel.style.fontSize = "14px";
	textLabel.innerHTML = "<font color=\"blue\"><b>보조옵션표시</b></font>";
	optionPre.appendChild(textLabel);
	...
	textLabel.style.marginLeft = "4px";
	textLabel.style.fontSize = "14px";
	textLabel.innerHTML = "<font color=\"blue\"><b>동맹옵션표시</b></font><br>";
	optionPre.appendChild(textLabel);
...
//	}
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "<font color=\"green\"><b>루트구축모드（</b></font>";
	optionPre.appendChild(ckLabel);
...
//	}
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "<font color=\"green\"><b>자원정보없음</b></font>";
	optionPre.appendChild(ckLabel);
...
//	}
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "<font color=\"green\"><b>루트자동구축)</b></font>";
	optionPre.appendChild(ckLabel);
	...
	textLabel.innerHTML = "<font color=\"green\"><b>루트자동구축조건설정</b></font><br>";
	optionPre.appendChild(textLabel);

	// 探査レベル
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "15px";
	ckLabel.innerHTML = "검색레벨";
	optionPre.appendChild(ckLabel);
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "개인영지를 통과（";
	optionPre.appendChild(ckLabel);

	// 通過同盟指定
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "개인/동맹";
	optionPre.appendChild(ckLabel);
	...
	textLabel.style.fontSize = "14px";
	textLabel.innerHTML = "<font color=\"red\"><b>보조표시</b></font><br>";
	optionPre.appendChild(textLabel);
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "51x51에서 사용하기";
	...
	textLabel.style.fontSize = "14px";
	textLabel.style.marginTop = "2px";
	textLabel.innerHTML = "<font color=\"red\"><b>영토보기의 종류</b></font><br>";
	optionPre.appendChild(textLabel);
	...
	textLabel = document.createElement("pre");
	textLabel.style.fontSize = "14px";
	textLabel.innerHTML = "<font color=\"red\"><b>개인연립검색</b></font><br>";
	...
	textLabel.style.fontSize = "14px";
	textLabel.style.marginTop = "2px";
	textLabel.innerHTML = "<font color=\"green\"><b>출병정보 공유설정</b></font><br>";
	...
	ckLabel = document.createElement("span");
	ckLabel.style.marginLeft = "4px";
	ckLabel.innerHTML = "<font color=\"blue\"><b>루트구축보기</b></font>";
					...
					if( (list2 != null) && (list2 != undefined) ){
						...
						if( (autoroute == AUTO_ROUTE_EDIT_LAST) && (ex.snapshotLength > 0)
							...
							if( autoroute == AUTO_ROUTE_EDIT_FIRST ){
								...
								// 確認
								if( window.confirm("이 좌표에서 루트를 설정합니다") ){
									// ルート構築
									makeAutoRoute("0");
								}
								else{
...
function autoRouteModeChecked() {
	...
	if( (checkBox.snapshotItem(0).checked == true) && (checkBox2.snapshotItem(0).checked == true) ){
		if( window.confirm("작성중인 루트가 소멸하지만 괜찮겠습니까?") ){
			routeClearButtonClicked();
			autoroute = AUTO_ROUTE_EDIT_FIRST;
		}
		else{
...
function autoRouteInfo(){
	...
	if( location.pathname != "/big_map.php" ){
		if( autoroute == AUTO_ROUTE_EDIT_FIRST ){
			routeField.snapshotItem(0).innerHTML = '<font color="red"><b>루트의 시작점을<br>오른클릭 해주세요</b></font>';
		}
		else if( autoroute == AUTO_ROUTE_EDIT_LAST ){
			routeField.snapshotItem(0).innerHTML = '<font color="red"><b>루트의 종점을<br>오른클릭 해주세요</b></font>';
		}
		else if( autoroute == AUTO_ROUTE_EDIT_END ){
			routeField.snapshotItem(0).innerHTML = '<font color="red"><b>루트의 종점을<br>오른클릭으로 없애고 다시설정가능</b></font>';
		}
		else{
			routeField.snapshotItem(0).innerHTML = '';
		}
