// ==UserScript==
// @name		eRepublik Plus ROMANIA
// @version		1.00
// @description	eRepublik script that simplifies common tasks
// @author		eCitizen Cristy2005ro , PeeKaBooh
// @namespace		eCitizenCristy2005ro
...
var INDUSTRIES = ['', 'Mancare', 'Cadou', 'Arma', 'Bilet Avion'];
...
var STRINGS = [	'Schimba Valuta',
				...
				'Te rugam introdu ',
				'Linkuri Rapide&nbsp;&nbsp;&nbsp;',
				'Toate comentariile traduse in&nbsp;&nbsp;&nbsp;',
				'Harta interactiva',
				'Descriere stiri',
				'"Third Party Tools" and "National Pages and Forums"',
				'Calculator wellness in profil',
				'Editor articole ziar si forum',
				'Pe pagina Army e aratat damage produs cu arme pe level',
				'Filtru cautare Job ',
				'Schimba valuta la piata monetara',
				'Pret unitar la produse si materiale',
				'Wellness al angajatilor',
				'Salveaza linkuri&nbsp;&nbsp;&nbsp;',
				'Activat',
				'Dezactivat',
				'Num',
				...
				'Antrenament pana la medalia urmatoare: '
];
...
function DisplayDamage() {
	...
	htmlDamage += '</span></span></a><strong>' + Math.ceil(BaseDamage / 2) + ' ('+ (Math.ceil(500 * BaseDamage) / 1000) +')</strong><br/>';
	...
	for (i=1; i<=5; i++)
	{
		...
		htmlDamage += '</span></span></a><strong>' + Math.ceil(BaseDamage * (i / 5 + 1)) + ' ('+ (Math.ceil(1000 * BaseDamage * (i / 5 + 1)) / 1000) + ')</strong><br/>';
	}
...
function AddNewsTitle() {
	...
	try {
		var flashVars = $("h1 embed").attr("flashvars");
...
function getWellnessByID(citizenID) {
	var sReq = "http://api.erepublik.com/v1/feeds/citizens/";

	GM_xmlhttpRequest({
		method: 'GET',
		url: sReq + citizenID + '.json',
		onload: function (json) {
			eval("jsonObj = " + json.responseText);
			$("#employee_wellness_" + citizenID).replaceWith(jsonObj["wellness"]);
		},
	});
}
...
function EmployeeWellness() {
	...
	$("table.allemployees th:eq(5)").attr('width', '40px');
	$("table.allemployees tr").each(function(i) {
		if (i == 0)
...
function addThirdPartyTools() {
	var thirdPartyTools = [
		{aname: 'Cristy2005ro',	aid: '1515392',	name: 'eRepublik Plus Romania',					url: 'http://www.erepublik.com/' + LOCALE + 'article/erepublik-plus-grand-tutorial-807662/1/20', img: 'http://static.erepublik.com/uploads/avatars/Newspapers/2009/05/15/81f97d040a2b499d539e0a7f61ce3788_55x55.jpg'},
		...
		{aname: 'carbon',		aid: '1124061',	name: 'eRepublik Tools (eRepTools)',	url: 'http://www.ereptools.pl/', 					img: 'http://ereptools.pl/images/logo_ereptools.png'},
...
function addNationalForums() {
	var nationalForums = [
		...
		{country: 'Singapore',		 	url: 'http://esingapore.freeforums.org/', 			name: 'Forum of eSingapore &bull; Index page'},
		{country: 'Serbia',			 	url: 'http://www.eserbia.iz.rs/index.php', 			name: 'eSerbia | The new world...'},
		{country: 'Russia', 			url: 'http://erepublic.ru/', 						name: 'eRussia - Главная страница'},
		{country: 'Romania', 			url: 'http://www.romaniavirtuala.info/', 					name: 'romaniavirtuala.info :: Comunitatea romanilor din eRepublik - Index'},
		...
		{country: 'Iran', 				url: 'http://forum.iranerepublik.ir/', 				name: 'انجمن غیرفعال است'},
		...
		{country: 'Estonia', 			url: 'http://estonia.forumotion.com/',				name: 'eEstonia'},
		...
		{country: 'China',				url: 'http://echina.hostei.com/forum/index.php',	name: 'e中国 &bull; Index page'},
		{country: 'Chile',	 			url: 'http://echile.foroactivo.net/', 				name: 'Foro gratis : foro de todos los echile de erepubli'},
		{country: 'Canada', 			url: 'http://ecanada.ws/forums/index.php', 			name: 'eCanada Forums &bull; Index page'},
		{country: 'Bulgaria',			url: 'http://ebulgaria-forum.info',					name: 'ebulgaria-forum.info - Индек'},
...
function addeRepPlusSettings() {
	...
	eRepPlusSettings.forEach(function(p) {
		...
		if (p.pos == 12) {
			htmlSettings += '<table class="offers"><thead><tr><th style="text-align: center;" width="40">Num</th><th width="150">Caption</th><th width="300">Link</th><th style="text-align: center" width="120">Fereastra noua</th></tr></thead>';
			htmlSettings += '<tbody>';
			for (var i=1; i<=10; i++)
			{
...
function saveLink(obj, what) {
	...
	if (GM_getValue(cID + 'eR_QL') != undefined)
		...
		lLinks = ['Gol 1', '', '', 'Gol 2', '', '', 'Gol 3', '', '', 'Gol 4', '', '', 'Gol', '', '', 'Gol 6', '', '', 'Gol 7', '', '', 'Gol 8', '', '', 'Doneaza la BNR', 'http://www2.erepublik.com/en' + LOCALE + 'organization/donate/items/10647', 'false', 'Kardinal Vest', 'http://www2.erepublik.com/en' + LOCALE + 'newspaper/kardinal-vest-189626/1', 'false'];
...
function getLink(index, what) {
	...
	if (GM_getValue(cID + 'eR_QL') != undefined)
		...
		lLinks = ['Gol 1', '', '', 'Gol 2', '', '', 'Gol 3', '', '', 'Gol 4', '', '', 'Gol 5', '', '', 'Gol 6', '', '', 'Gol 7', '', '', 'Gol 8', '', '', 'Doneaza la BNR', 'http://www2.erepublik.com/en' + LOCALE + 'organization/donate/items/10647', 'false', 'Kardinal Vest', 'http://www2.erepublik.com/en' + LOCALE + 'newspaper/kardinal-vest-189626/1', 'false'];
...
function addAllCommentsAndTranslate() {
	...
	$("div.articlecontent div.largepadded a.dotted").each(function(i) {
		...
		$(this).parent().append('<a class="dotted" id="translate_news" href="#">' + STRINGS[37] + '</a>');
		$("#translate_news").click(function() {
			$("#translate_news").text(" Anuleaza traducerea");
			$("#translate_news").click(function() { location.reload(); });
			tr = new Translator(doTranslate);
			tr.sync = false;
			tr.traverse($("div.article div.articlecontent")[0]);
			tr.traverse($("#comments_div")[0]);
		});
	});
}
...
function showQuickLinks() {
	var htmlQuickLinks = '<ul class="quick_links" style="height: auto !important;">';
	
	for (var i=1; i<=10; i++) {
	
		if (getLink(i, 1).substr(0, 11) == 'javascript:')
			htmlQuickLinks += '<li><a href="#" onclick="' + getLink(i, 1) + '">' + getLink(i, 0) + '</a></li>';
		else
			if (getLink(i, 2) == 'true')
				htmlQuickLinks += '<li><a href="' + getLink(i, 1) + '" target="_blank">' + getLink(i, 0) + '</a></li>';
			else
				htmlQuickLinks += '<li><a href="' + getLink(i, 1) + '">' + getLink(i, 0) + '</a></li>';
	}
	htmlQuickLinks += '</ul>';
	$("#menu").append(htmlQuickLinks);
	GM_addStyle("ul.quick_links li a {width: 85px !important; height: auto !important; float: left; background-color: #E9F5FA; padding: 5px 5px 5px 5px !important; }");
	GM_addStyle("ul.quick_links li a:hover {width: 85px !important; float: left; color: white; background-color: #53B3D3; padding: 5px 5px 5px 5px !important; }");
}
...
function Main(e) {
	...
	var pagesFunctions = [
		...
		{p: 'company-employees/',	s:9, 	f: EmployeeWellness},
	...
	$("#logo a").attr("title", "eRepublik Plus ROMANIA v" + VERSION);
};
...
/**
 * Translator
 * Copyright (c) 2009 Cristy2005ro - cristy2005ro(at)yahoo(dot)com 
