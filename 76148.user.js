// ==UserScript==
// @name		Save The Coratee :: Upload Comments Notifier
// @namespace	http://www.savethecoratee.org
// @description	Will notify you when you have unread comments on your uploads
// @include	http*://*savethecoratee.org*
// @version	1.6
// ==/UserScript==
...
(function() {
	...
	var savethecoratee_base_url = document.URL.match(/^(https:\/\/ssl\.savethecoratee\.org|http:\/\/savethecoratee\.org)/)[1];
	...
	if ( !lastChecked || (current.getTime() - lastChecked) > check_interval ) {
		...
		GM_xmlhttpRequest({
			method: 'GET',
			url: savethecoratee_base_url + '/comments.php?action=my_torrents',
			headers: {
	...
	if ( alert_style <= 2 ) {
		if ( unviewedComments == true ) {
			target[1].innerHTML += ' <a href="/comments.php?action=my_torrents"><img src="' + comment_alert_image + '" style="height: 11px; width: 10px;" title="You have ' + unviewedCount + ' new comments left on your uploads" /></a>';
		} else {
			if ( alert_style == 1 ) { target[1].innerHTML += ' <a href="/comments.php?action=my_torrents"><img src="' + comment_image + '" style="height: 11px; width: 10px;" title="View comments left on your uploads" /></a>'; }
		}
	...
	var SUC_script_num = 65658; // Change this to the number given to the script by userscripts.org (check the address bar)
