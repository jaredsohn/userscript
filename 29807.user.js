// ==UserScript==
// @name           pbr Game Scout
// @description    modification of tciss(?)'s game scout script for 216.245.193.2
...
/*
 *
 * based on code by tciss from www.216.245.193.2
...
// ---------------------------------------------------------------------
    ...
    function getPassingTargetTable(index, rows) {
	    var title = "Receivers";
	    var columns = ["Targets","Catches","Drops","Catch Pct.","Yards","YPC","Long"];
	...
	var title = "Defenders";
	var columns = ["Tackles","Missed","Rushing","Missed","Passing","Missed"];
...
function fillTables() {
    ...
    b1.addEventListener('click', function() { changeVisibility(0,0); }, true);
    ...
    b3.setAttribute("id","gsbar1");
    b3.addEventListener('click', function() { changeVisibility(1,3); }, true);
    ...
    b5.setAttribute("id","gsbar4");
    b5.addEventListener('click', function() { changeVisibility(4,7); }, true);
    ...
    b7.setAttribute("id","gsbar8");
    b7.addEventListener('click', function() { changeVisibility(8,12); }, true);
    ...
    b9.setAttribute("id","gsbar13");
    b9.addEventListener('click', function() { changeVisibility(13,13); }, true);
    ...
    function Play() {
        ...
        this.toString = function() {
            return this.quarter+" : "+this.team+" - "+this.timeRemaining+" - "+this.marker+" - "+this.down+"&"+this.togo;
        }
    }
	...
	if (s1 != -1) {
	    ...
	    //console.log(s);
	    if (playerDefensiveName[ct] == null) {
		playerDefensiveName[ct] = new Array();
		playerDefensiveStats[ct] = new Array();
	    }
	    playerIndex = playerDefensiveName[ct].indexOf(s);
	    if (playerIndex == -1) {
		playerIndex = playerDefensiveName[ct].length;
		playerDefensiveName[ct].push(s);
		playerDefensiveStats[ct].push([0,0,0,0,0,0,0]);
	    }
...
//        console.log("playHandler loop: "+pages+" - "+p.replay);
	...
	//console.log("fg "+playText.slice(0,40));
		...
		var playerIndex = -1;	
		if (playerKickingName[ct] != null) {
		    playerIndex = playerKickingName[ct].indexOf(s);
		    if (playerIndex == -1) {
			playerIndex = playerKickingName[ct].length;
			playerKickingName[ct].push(s);
			playerKickingStats[ct].push([0,0,0,0]); //k-y-l
		    }
		}
		else {
		    playerIndex = 0;
		    playerKickingName[ct] = new Array();
		    playerKickingStats[ct] = new Array();
		    playerKickingName[ct].push(s);
		    playerKickingStats[ct].push([0,0,0,0]);
		}
		...
		var s = playText.slice(s1,s1+s2);
		var playerIndex = -1;	
		if (playerPuntingName[ct] != null) {
		    playerIndex = playerPuntingName[ct].indexOf(s);
		    if (playerIndex == -1) {
			playerIndex = playerPuntingName[ct].length;
			playerPuntingName[ct].push(s);
			playerPuntingStats[ct].push([0,0,0,0]); //p-y-l
		    }
		}
		else {
		    playerIndex = 0;
		    playerPuntingName[ct] = new Array();
		    playerPuntingStats[ct] = new Array();
		    playerPuntingName[ct].push(s);
		    playerPuntingStats[ct].push([0,0,0,0]);
		}
...
//                    console.log("start running getPage("+p+")");
//                    getPage(p);
//                    console.log("end running getPage("+p+")");
                    playHandler(p);
                }
