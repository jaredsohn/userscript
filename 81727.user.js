...
var Pref = { _:null
		...
		// 分栏数量，默认 2
		,columns				: getValue('numcol',			2)
		
		// 搜索结果的排列方向：1-横向；2-纵向（默认）
		,direction				: getValue('direction',			2)

		// 显示模式：1-紧凑；2-对齐，但比较松散（默认）；3-对齐，但比较松散；4-对齐，但比较松散
		,displayMode			: getValue('mode',				2)

		// 搜索框浮动，1-顶部；2-底部
		,floatInput				: getValue('floatInput',		false)
		,floatInputPos			: getValue('floatInputPos',			1)

		// 是否自动加载下一页，默认 关闭
		,AutoNextPage			: getValue('AutoNextPage',		false)

		// 是否隐藏自动翻页分隔符，默认 不隐藏
		,HideAutoNextPage		: getValue('HideAutoNextPage',		false)

		// 让搜索结果适合屏幕宽度，默认 打开
		,fitwidth				: getValue('fitwidth',			true)
		
		// 移除页面右侧的广告，默认 关闭
		,removeAd				: getValue('removeAd',			false)

		// 给搜索结果条目添加序号，默认 关闭
		,addResultNum			: getValue('addResultNums',		false)

		// 添加缩略图预览，默认 关闭
		,addPreview				: getValue('addPreviews',		false)
		
		// 缩略图大小，默认 100%
		,PreviewSize			: getValue('PreviewSize',		100)

		// 添加 Favicon，默认 关闭
		,addFavicon				: getValue('addFavicons',		false)

		// 若没有 Favicon 则显示空白，默认 否
		,addFavicon_h			: getValue('addFavicons_h',		false)

		// 添加“在此站点中搜索”
		,searchSite				: getValue('searchSite',		false)

		// 禁止百度记录我的点击情况，默认 关闭
		,DisableTracking		: getValue('DisableTracking',	false)
		
		// 快捷键定位到搜索框
		,Focus2Keyword			: getValue('Focus2Keyword',		false)
		,Focus2KeywordKeys		: getValue('Focus2KeywordKeys',	'alt+83')
		// 快捷键定位到搜索框后的动作：1-清除全部文字（默认）；2-紧跟文字之后；3-选中全部文字
		,Focus2KeywordAction	: getValue('Focus2KeywordAction',	1)

		// 在页面上部添加其他搜索引擎
		,EnableSearchInOtherEngines		: getValue('SearchInOtherEngines',		true)
		// 其他搜索引擎地址
		...
		// 跟随搜索框浮动
		,OtherEnginsFloat		: getValue('OtherEnginsFloat',	false)
		...
		// 自动检查更新
		,lastCheck				: getValue('lastCheck',			0)
		,skipVersion			: getValue('skipVersion',		0)
		,newVersion				: getValue('newVersion',		0)
};
...
switch (matchNode('//br[preceding-sibling::table[tbody/tr/td/@class="f"]]').snapshotLength > 0) {
    ...
    case true:
        ...
        setTimeout(function(){
        	...
        	if (Pref.AutoNextPage) {
    			NextPageLink = matchNode('//a[parent::div[@class="p"] and font[text() = "下一页"]]').snapshotItem(0);

            	// 删除第一页的页面导航条
            	var nav = matchNode('//div[@class="p"]').snapshotItem(0);
            	if (nav) 
    ...
    case false:
        ...
        setTimeout(function(){
	        ...
	        biDiv.setAttribute('class', 'bi');
	        biDiv.setAttribute('style', 'background-color: rgb(217, 225, 247); font-size: 9pt; height: 20px; line-height: 18px; clear: both;');
	        var biTable = matchNode('//table[@class="bi"]').snapshotItem(0);
	        var biTds = matchNode('//table[@class="bi"]/descendant::td');
	        var bileft = document.createElement('div');
	        var biright = document.createElement('div');
	        try {
	            with (bileft) {
	                setAttribute('id', 'bm_bileft');
	                setAttribute('style', 'float: left;');
	                innerHTML = biTds.snapshotItem(0).innerHTML;
	            }
	            with (biright) {
	                setAttribute('id', 'bm_biright');
	                setAttribute('style', 'float: right;');
	                innerHTML = biTds.snapshotItem(1).innerHTML;
	            }
	        } 
	        catch (e) {
	        }
	        biDiv.appendChild(bileft);
	        biDiv.appendChild(biright);
	
			// 清除“共有xx条搜索结果”条目向右浮动对后面元素的影响
	        var divClear = document.createElement('div');
	        divClear.setAttribute('style', 'clear: both');
	        biTable.parentNode.replaceChild(biDiv, biTable);
	        biDiv.parentNode.insertBefore(divClear, biDiv.nextSibling);
...
function doit(){
	var css = '\
				/* 添加圆角效果等 */ \
				.rest {-moz-border-radius: 0.75em;} .bm_MultiColDiv {padding-right: 0.5em;}\
				/* 添加推广链接内容框的样式 */ \
				#popDiv {border: 1px solid red; -moz-column-count: ' + Pref.columns + '; -moz-column-gap: 0.5em; margin-top: 1em; -moz-border-radius: 0.75em;}\
			    /* 让搜索结果条目顶部对齐 */ \
			  ...
			  +
			  // 添加分栏样式
			  ' .bm_MultiColDiv {-moz-column-count: ' + Pref.columns + '; -moz-column-gap: 0.2em;}'
			  +
			  // 隐藏自动翻页分隔符
			  (Pref.HideAutoNextPage?' .p {display: none !important;}':'')
			  +
			  // 添加翻页分隔符的样式
			  (Pref.AutoNextPage?' .p {font-size: small; background: rgb(230, 230, 230) none repeat scroll 0% 0%; clear: both; line-height: 20px; text-align: center; margin-top: 0.7em;}':'')
    ...
    // 去除右侧的广告
    if (Pref.removeAd) {
        var adTable = matchNode('//table[descendant::div[@id="ScriptDiv" or @class="r"]]').snapshotItem(0);
        if (adTable) {
...
function addPreferences(){
    var titDiv = matchNode('//div[@class="Tit"]').snapshotItem(0);
    if(titDiv){
...
// 浮动搜索框
function floatInput(){
    var css, header = matchNode('//table[descendant::div[@class="Tit"]]').snapshotItem(0);
	...
	with (headerContainer) {
		...
		switch (Pref.floatInputPos) {
			case 1: // 顶部
				...
				document.body.insertBefore(rplheader, headerContainer);
				appendChild(header);

				if (Pref.OtherEnginsFloat) {
					var bi = matchNode('//table[@class="bi"]').snapshotItem(0);
					appendChild(bi);
			...
			case 2: // 底部
				var rplheader = document.createElement('div');
				document.body.insertBefore(rplheader, matchNode('//table[@style="margin-left: 18px; height: 60px;"]').snapshotItem(0).nextSibling);
...
function addSearchInOtherEngines(){
    var div = document.getElementById('bm_bileft');
	if (div) {
...
function addResultNums(doc){
    var resTds = matchNode('//td[@class="f"][not(descendant::a[text() = "推广"])]', doc);
...
function addPreviews(doc){
    var resLinks = matchNode('//td[@class="f"][not(descendant::a[text() = "推广"])]/a', doc);
...
function addFavicons(doc){
    var resLinks = matchNode('//td[@class="f"][not(descendant::a[text() = "推广"])]/a[not(descendant::img)]', doc);
	var i, len = resLinks.snapshotLength;
    for (i = 0; i < len; i++) {
		var resLink = resLinks.snapshotItem(i);
...
function prefOnChange(){
	...
	bm_pref_container.setAttribute('style','-moz-column-count: ' + bm_pref_Pref.columns + '; -moz-column-gap: 0.2em;');
                        ...
                        <table width="100%">\
                            <tr id="bm_pref_gui_title">\
                            	<td colspan="2">\
                            		...
                            		<div style="float: right; padding-right: 0.8em; font-size: small;">欢迎来<a href="http://mozilla.sociz.com/viewthread.php?tid=25917" target="_blank">这里</a>发表任何意见或建议。</div>\
                            	</td>\
                            </tr>\
                            <tr>\
                                <td colspan="2">\
...
';
    ...
    document.getElementById('save_button').addEventListener('click',function(){
		...
		GM_setValue('numcol', Number(tmp.options[tmp.selectedIndex].value));

        // 保存搜索结果的排列方向
		tmp = document.getElementById('direction');
		GM_setValue('direction', Number(tmp.options[tmp.selectedIndex].value));
                
        // 保存排版模式
		tmp = document.getElementById('mode');
		GM_setValue('mode', Number(tmp.options[tmp.selectedIndex].value));

		// 浮动搜索框
		GM_setValue('floatInput', document.getElementById('floatInput').checked);
		tmp = document.getElementById('floatInputPos');
		GM_setValue('floatInputPos', Number(tmp.options[tmp.selectedIndex].value));
		      
        // 让搜索结果适合屏幕宽度
        GM_setValue('fitwidth', document.getElementById('fitwidth').checked);
		      
        // 移除右侧的广告
        GM_setValue('removeAd', document.getElementById('removeAd').checked);
		
        // 给搜索结果标序号
        GM_setValue('addResultNums', document.getElementById('addResultNums').checked);
        
        // 添加缩略图预览
        GM_setValue('addPreviews', document.getElementById('addPreviews').checked);
        
		// 缩略图大小。如果包含非数字，则替换成默认值 100
        GM_setValue('PreviewSize', isNaN(document.getElementById('PreviewSize').value)?'100':document.getElementById('PreviewSize').value);
        
        // 添加站点的 Favicon
        GM_setValue('addFavicons', document.getElementById('addFavicons').checked);
        
        // 若没有 Favicon 则显示空白
        GM_setValue('addFavicons_h', document.getElementById('addFavicons_h').checked);
        
        // 在页面上部添加其他搜索引擎，默认【在Google中搜索xx】
        GM_setValue('SearchInOtherEngines', document.getElementById('SearchInOtherEngines').checked);
        GM_setValue('OtherEngines', document.getElementById('OtherEngines').value.replace(/\'/ig,'"').replace(/^\s*|\s*$/ig,'').replace(/\s*[\n\r]+\s*/ig,'\n').replace(/\s*\|\s*http/ig,'|http'));
        GM_setValue('OtherEnginsFloat', document.getElementById('OtherEnginsFloat').checked);
		tmp = document.getElementById('OtherEnginesTarget');
		GM_setValue('OtherEnginesTarget', tmp.options[tmp.selectedIndex].value);
        
        // 自动加载下一页
        GM_setValue('AutoNextPage', document.getElementById('AutoNextPage').checked);
    	GM_setValue('HideAutoNextPage', document.getElementById('HideAutoNextPage').checked);
            
        // 快捷键定位到搜索框
        GM_setValue('Focus2Keyword', document.getElementById('Focus2Keyword').checked);
		tmp = document.getElementById('Focus2KeywordAction');
		GM_setValue('Focus2KeywordAction', Number(tmp.options[tmp.selectedIndex].value));
        
        // 禁止Baidu记录我的点击情况
        GM_setValue('DisableTracking', document.getElementById('DisableTracking').checked);

		// 添加“在此站点中搜索”
        GM_setValue('searchSite', document.getElementById('searchSite').checked);
		...
		// 保存自定义的 CSS
		GM_setValue('customizeCSS', document.getElementById('customizeCSS').value);
	...
	document.getElementById('Focus2KeywordKeys').addEventListener('keydown',function(event){
			...
			GM_setValue('Focus2KeywordKeys', Keys); // 立即保存快捷键
			event.preventDefault();
			event.stopPropagation();
	},false);
}
...
function loadNextPage(){
    ...
    if (!IsLoadingNext && NextPageLink && document.body.scrollHeight - window.scrollY < window.innerHeight * 3 && !document.getElementById('preferences')) {
		...
		splitDiv.innerHTML = '<img style="vertical-align: middle;" src="data:image/gif;base64,">\
							BaiduMonkey 正在加载下一页 ...';
        var clr = matchNode('//table[@style="margin-left: 18px; height: 60px;"]').snapshotItem(0);
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(d){
                ...
                NextPageLink = matchNode('//a[parent::div[@class="p"] and font[text() = "下一页"]]', ContainerDiv).snapshotItem(0);
                if(!NextPageLink){
					// 如果没有下一页了则移除滚动监视
				...
				// 取得页面导航条
				var splitDiv = matchNode('//div[@class="p"]', ContainerDiv).snapshotItem(0);
	...
	if (Number(Pref.newVersion)) { // 如果已经检查到新版本
		...
		getNewVersion(function(version){
			if(Number(version)>Number(scriptVersion) && version != Pref.skipVersion){ // 如果最新版大于现有版本，并且不等于跳过的版本
				GM_setValue('newVersion',version);
				showUpdateMessage(version);
			}
			else {
				GM_setValue('newVersion',0);
			}
		})
	}
}
...
function showUpdateMessage(newVersion){
	...
	GM_addStyle('.gbh {display: none;} \
				 #bm_UpdateMsg {background-color: rgb(240, 247, 249); margin: 1em; padding: 3px; text-align: center; -moz-border-radius: 5px;} \
				');
}
...
function skipUpdate(newVersion){
	hideUpdateMessage();
	GM_setValue('skipVersion',newVersion);
	GM_setValue('newVersion',0);
}
