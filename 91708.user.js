...
// @description   BB-code la mesaje si va arata cele mai bune iteme pentru munca selectata
...
n.innerHTML="<br/><br/>Mesaje pt jucÄƒtori";
...
function twpro_insertList(twpro_data)
{
  ...
  var twpro_xhtml = '<table id="twpro_jobDisplay" style="display: inline;position: absolute; visibility:hidden; margin-left:1px;"><tr><td>';
  ...
  twpro_xhtml += '<a href="javascript:undefined;" onmouseup="TWPro.twpro_sortList(\'comb\');" id="twpro_jobsort_link_comb"><img id="twpro_jobsort_comb" alt="" src="images/transparent.png" style="display:none;height: 17px;width: 21px;background-image: url(http://www.tw-pro.de/img/sort.png);background-position: '+((TWPro.twpro_jobsort == 'comb')?('-61px 0px'):('-61px -17px'))+';" onmouseover="this.style.backgroundPosition=\'-61px 1px\';" onmouseout="this.style.backgroundPosition=((TWPro.twpro_jobsort == \'comb\')?(\'-61px 0px\'):(\'-61px -17px\'));"/></a>';

  if(TWPro.Hide_unjobs==true){checkednojob="checked"}else{checkednojob=""}
  twpro_xhtml += '<span id="twpro_aktuelleap" style="position:absolute;right:99px;visibility:hidden;"><span id="twpro_aktuelleapvalue" style="border-style:solid;border-width:2px;padding:2px;"></span> </span></td></tr><tr><td><select id="twpro_jobList" size="1" onchange="TWPro.twpro_changeJob()" onclick="TWPro.twpro_clickList()" style="background-color: rgb(207, 195, 166); font-size: 11px; width:237px;"><option value="-1" id="twpro_wait" style="background-color: rgb(207, 195, 166);">ALEGETI O MUNCA DIN LISTA</option></select>'; //edit by miodrag HHH
...
function twpro_sortList(twpro_jobSortItem)
{
  ...
  {
    if(document.getElementById('twpro_wait').text == 'Alege o muncÄƒ')
    {
      document.getElementById('twpro_wait').text = 'VÄƒ rog sÄƒ asteptaÅ£i';
      ...
      document.getElementById('twpro_wait').text = 'Alege o muncÄƒ';
    }
    else
    {
      TWPro.twpro_jobs.sort(TWPro.twpro_sortJobs);
...
function twpro_showList()
{
  ...
  if(TWPro.twpro_failure) return;
  $('twpro_jobsort_link_name').addMousePopup(new MousePopup('Sortare dupÄƒ <b>Nume</b>',100,{opacity:0.95}));
  $('twpro_jobsort_link_ervaring').addMousePopup(new MousePopup('Sortare dupÄƒ <b>ExperienÅ£Äƒ</b>',100,{opacity:0.95}));
  $('twpro_jobsort_link_loon').addMousePopup(new MousePopup('Sortare dupÄƒ <b>Salariu</b>',100,{opacity:0.95}));
  $('twpro_jobsort_link_geluk').addMousePopup(new MousePopup('Sortare dupÄƒ <b>Noroc</b>',100,{opacity:0.95}));
...
function twpro_getBagPopup()
{
  ...
  twpro_xhtml+='<tr><td class="item_popup_trader_price">Valoare totalÄƒ:&nbsp;&nbsp;</td>';
  twpro_xhtml+='<td class="item_popup_trader_price">'+(TWPro.twpro_bag.twpro_priceWear+TWPro.twpro_bag.twpro_priceBag)+' $</td></tr>';
  twpro_xhtml+='</table>';
  twpro_xhtml+='<DIV class="popup_yield_divider"></DIV><span class="item_popup_requirement_text">NumÄƒrul obiectelor</span>';
  twpro_xhtml+='<table>';
  twpro_xhtml+='<tr><td class="item_popup_trader_price">PÄƒlÄƒrii:&nbsp;&nbsp;</td>';
  ...
  twpro_xhtml+='<tr><td class="item_popup_trader_price">ÃŽmbrÄƒcÄƒminte:&nbsp;&nbsp;</td>';
  twpro_xhtml+='<td class="item_popup_trader_price">'+TWPro.twpro_bag.twpro_countType['body']+'</td></tr>';
  twpro_xhtml+='<tr><td class="item_popup_trader_price">ÃŽncÄƒlÅ£Äƒminte:&nbsp;&nbsp;</td>';
...
{
  if(TWPro.twpro_failure) return;
  if(document.getElementById('twpro_wait').text != 'VÄƒ rog sÄƒ asteptaÅ£i' && document.getElementById('twpro_wait').text != 'Alege o muncÄƒ')
  {
    document.getElementById('twpro_wait').text = 'VÄƒ rog sÄƒ asteptaÅ£i';
...
function twpro_updateList()
{
  ...
  if(!TWPro.twpro_calculated)
  {
    ...
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].malus=0;
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].name='Duel (Pistol vs SÄƒbii) ';
    ...
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].malus=0;
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].name='Duel (SÄƒbii)';
    ...
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].malus=0;
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].name='Duel (SÄƒbii vs Pistoale)';
    ...
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].malus=0;
    TWPro.twpro_jobs[TWPro.twpro_jobs.length-1].name='Duel (SÄƒbii vs SÄƒbii)';
...
GM_addStyle('.profile_bb_code_image {background-image:url(../images/bbcodes.png);height: 22px;margin: 2px 2px;width: 22px;}');
