...
if (next == null)
{
	next = document.getElementById("no-thanks");
}
if (next == null)
{
...
if(next == null)
{
	for(i=0;i<inputs.length;i++)
	{
		if((inputs[i].type=="skip") && (inputs[i].value.toLowerCase()=="skip"))
		{
			next = inputs[i];
			break;
		}
...
if(next==null)
{
	for(i=0;i<inputs.length;i++)
		{
			if((inputs[i].value.toLowerCase()=="skip") || (inputs[i].name.toLowerCase()=="skip"))
			{
				next = inputs[i];
				break;
			}
