...
// @match       http://*.douban.com/group/topic/*
...
(function () {
    var topic = document.querySelector('.topic-content .topic-content');
    if(!topic) {
        return;
    }
    var topicContent = topic.innerText;
    var regMark = /^@markdown/;
    var converter;
    var css = '.markdown-body ul, .markdown-body ol { padding-left: 30px;}'
            ...
            + '.markdown-body code, .markdown-body tt {margin: 0 2px;padding: 0 5px;border: 1px solid #EAEAEA;background-color: #F8F8F8;border-radius: 3px;}'
            + '.markdown-body pre code, .markdown-body pre tt {background-color: transparent;border: none;}'
            ;
    if(regMark.test(topicContent)) {
        topicContent = topicContent.replace(regMark, '');
        converter = new Showdown.converter();
        topic.innerHTML = '<article class="markdown-body">' + converter.makeHtml(topicContent) + '</article>';
        addCssByStyle(css);
    }
