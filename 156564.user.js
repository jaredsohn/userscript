...
if (page.url.indexOf('youtube.com/watch') != -1) {
  ...
  /* Get Player Window (New Layout) */
  if (!ytPlayerWindow) ytPlayerWindow = getMyElement ('', 'div', 'id', 'watch7-video', -1, false);
...
// =====Blip===== //
  ...
  if (!blipPlayerWindow) {
    ...
    styleMyElement (blipSaverSocket, {width: '960px', textAlign: 'center', margin: '0px auto'});
	...
	if (blipMimeTypes[blipVideoMime]) {
      ...
      if (blipVideoFound) {
	/* Create Saver */
	saver = {'saverSocket': blipSaverSocket, 'videoList': blipVideoList, 'videoSave': blipDefaultVideo, 'saverWidth': 960};
	option['definitions'] = ['High Definition', 'Standard Definition', 'Low Definition'];
	option['containers'] = ['MP4', 'M4V', 'MOV', 'FLV', 'Any'];
	createMySaver ();
      }
      else {
	saver = {'saverSocket': blipPlayerWindow, 'saverWidth': 960, 'warnMess': '!videos'};
	createMySaver ();
      }
    }
    else {
      saver = {'saverSocket': blipPlayerWindow, 'saverWidth': 960, 'warnMess': '!content'};
      createMySaver ();
    }
  }
