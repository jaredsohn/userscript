...
var config = {
  ...
  autorandom_idle_time: 60	// várakozás (másodperc) a tippelés előtt autorandom eseten
};
...
settings = {
	version: "4.3",
	server_url: "http://fedore461.netii.net/",
	talalat: false
};
...
if (csomag.vanvalasz) {
	...
	if (mehet) {
		...
		gmpost(settings.server_url, '&upload=1&kerdes='+csomag.kerdes+'&jovalasz='+jovalasz+'&opt1='+csomag.opt1+'&opt2='+csomag.opt2+'&opt3='+csomag.opt3+'&opt4='+csomag.opt4+'&invalid='+invalid+'&talalat='+talalat+'&bes1='+csomag.bes1+'&bes2='+csomag.bes2+'&bes3='+csomag.bes3, 
    function(data){
    });
	}
}
...
if (strpos(msg.nothunting, jobbcontent.innerHTML)) { 
	gmpost('https://sites.google.com/site/vukviz/', '' , function(data){
    var patt=/#url#(.+)#url#/im;
    var url=patt.exec(data.responseText);
    if (url[1]) gmpost(url[1], '&update=1' , function(data){
