// ==UserScript==
// @name           Spam automatico Taringa! By AngelKrak
// @namespace      Angel Ramirez
// @author         AngelKrak
// @description    Spam automatico por MP a Taringa!
	...
	if(/^\/(index\.php)?$/.test(location.pathname)){
		...
		$('#new-message-to').blur(function(){
			if(storage.get($(this).val().toLowerCase())){
				...
				$('#new-message-to-name').html('<strong>ATENCI&Oacute;N:</strong> ya hab&iacute;as enviado un MP a este usuario antes o est&aacute; en la lista negra'); // por AngelKrak		}else{
