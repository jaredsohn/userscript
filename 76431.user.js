// ==UserScript==
// @name        Facebook: Clear Wall
// @namespace   Si Forster - modded from Ichau
...
function removeRecentActivities()
{
    ...
    $('div.UIStory').each(    
        function()
        {
            var activity = $(this).find('a[class~=uiButtonSuppressed]').each(
            function()
            {
                var entry = $(this);
                if (entry)
                {
                    ...
                    var story_key   = url.match(/story_key=(\d+)&/i)[1];
                    var story_type  = url.match(/story_type=(\d+)&/i)[1];
                    var target_id   = url.match(/profile_id=(\d+)&/i)[1];
...
function createButton()
{
    if (!button)
    {
        ...
        button.innerHTML = '<center><b><a href="#" class="uiButton uiButtonDefault uiButtonMedium">' +
        ...
        $(button).insertBefore($('#profile_blurb'));
    }
}
