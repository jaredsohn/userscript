...
// @name           ssTOimdb
// @namespace      localhost
// @description    links from studiosystem to imdb pro
// @include        http://studiosystem.com/
// @include        http://*.studiosystem.com/
// ==/UserScript==
...
if (action.match("person") != null) { //person
	ptype = PERSON;
	iptname = "nm";
} else if (action.match("project") != null) {
	ptype = PROJECT;
	iptname = "tt";
} else if (action.match("company") != null) {
	ptype = COMPANY;
	iptname = "co";
} else {
	ptype = NONE;
...
if (ptype != NONE){
	//alert("hi");
	var htd = $("td.sectionHeader[@width=510]").eq(0);
	var name = htd.text();
	var qstr = "http://pro.imdb.com/find?q=" + name + "&" + iptname + "=on";
	var html = "<span style=\"text-size:small;\">&nbsp;&nbsp; &nbsp;  <a href=\""+ qstr +"\">Search for "+ name +" on IMDBpro</a></span>";
	$(html).appendTo(htd);
}
