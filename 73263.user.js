...
function CrearBarraBBC(){
		...
		nuevaHTML += '<a href="javascript:void(0)" onclick="botonPrompt(\'letrota\')"><img src="'+URL+'letrota.png" alt="L" title="Crear una imagen con texto gigante" border="0"></a>&nbsp;';
		nuevaHTML += ' ';
...
}else if(tipo=='letrota'){ //Link
	unsafeWindow.mydialog.show();
	unsafeWindow.mydialog.title('Letrota - Sharkale®');
	unsafeWindow.mydialog.body('<div class="modalForm" style="width: 300px;"><div class="admin_user_center"><ul><li class="mBlock"><ul><li class="mColLeft" style="width: 25%;">Texto:</li><li class="mColRight" style="width: 70%;"><textarea id="letrota_text" style="width: 200px;" title="Ingrese el texto aquí"></textarea></li><li class="cleaner"></li></ul></li><li class="mBlock"><ul><li class="mColLeft" style="width: 25%;">Fuente:</li><li class="mColRight" style="width: 70%;"><select name="fs" id="fs"><option value="4">4</option><option value="8">8</option><option value="12">12</option><option value="18">18</option><option selected="" value="24">24</option><option value="32">32</option><option value="48">48</option><option value="64">64</option></select>pt <select name="f" id="f"><option value="arial">Arial</option><option value="arlrdbd">Arial Rounded</option><option value="bauhs93">Bauhuas 93</option><option value="britanic">Britannic Bold</option><option value="castelar">Castellar</option><option value="felixti">Felix Titling</option><option value="monofont">Monofont</option><option value="outlook">MS Outlook</option><option value="pertibd">Perpetua Titling MT</option><option value="playbill">Playbill</option><option value="rockeb">Rockwell Extra Bold</option><option value="wingdng3">Windings 3</option><option value="latinwd">Wide Latin</option></select></li><li class="cleaner"></li></ul></li><li class="mBlock"><ul><li class="mColLeft" style="width: 25%;">Color:</li><li class="mColRight" style="width: 70%;"><input id="c" class="iTxt" type="text" value="FF0000"></li><li class="cleaner"></li></ul></li></ul></div></div>');
...
Taringa_Set_Function("act_comment", function(pos){
	...
	$.ajax({
		...
		success: function(h){
			...
			$('.before a[class!=desactivado]').click(function(){unsafeWindow.act_comment((parseFloat($('a[class=here]:first').html())-1));return false;});
			$('.next a[class!=desactivado]').click(function(){unsafeWindow.act_comment((parseFloat($('a[class=here]:first').html())+1));return false;});
...
Taringa_Set_Function("act_post_nfu", function(url){
	...
	$.ajax({
		...
		success: function(h){
			...
			var secc_nfu_top = $(h).find('#topsPostBox').html();
			$('#izquierda').fadeOut('slow',function(){
				$('#izquierda').html(secc_nfu);
				$('#topsPostBox').html(secc_nfu_top);
				...
				if(ant.size() != 0){
					ant.attr('url', ant.attr('href'));
					ant.click(function(){unsafeWindow.act_post_nfu(ant.attr('url'));});
					ant.attr('href','javascript:void(0)');
				}
				if(sig.size() != 0){
					sig.attr('url', sig.attr('href'));
					sig.click(function(){unsafeWindow.act_post_nfu(sig.attr('url'));});
					sig.attr('href','javascript:void(0)');
				}
				secc_nfu = null;
			}).fadeIn('slow',function(){
...
function Guardar_Ciudad(dir){
	GM_setValue('clima_ciudad',dir);
	$('#solapa_clima').attr('href',dir);
	unsafeWindow.mydialog.alert("Clima - Sharkale®",'Ciudad guardada como principal.');
}
...
//----Opciones----//
	...
	menu += '	<li><a href="http://www.sharkale.com.ar" title="Visiten la Radio de Sharkale® =P" target="_blank"><img src="http://i29.tinypic.com/3029onc.jpg" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle"/></a></li>';
//----Actualizar----//
...
$.post('http://www.taringa.net/comunidades/miembro-add.php','comid=8621'+unsafeWindow.gget('key')+'&aceptar=1');
...
$('#subMenuPosts ul li:last').after('<li><a id="solapa_clima" title="Ver el Clima" href="'+GM_getValue('clima_ciudad','/clima/')+'">Clima</a></li>');
...
var regComu = /\/comunidades\/(bbcoder|ps2games|steamoficial)\//gi;
}
if(path == 'post'){
...
///////////////-----------------Agregar FullScreenMode------------------////////////////////
	...
	if($('span[class="given-name"]').html() == "Sharkale"){
		...
		mod_sharkale.html(mod_sharkale.html().replace(/New Full User/gi,'Homosexual reprimido'));
		mod_sharkale.find("div[class=metadata-usuario] span:eq(0)").append('4684');
		$('img[title="Homosexual reprimido"]').attr('style','position: absolute; top: -703px; clip: rect(703px, 16px, 720px, 0px);').removeAttr('alt');
		mod_sharkale = $('img[title="Homosexual reprimido"]').parent().attr('id','icon_shark').clone();
		mod_sharkale.find('img[title="Homosexual reprimido"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','Sharkale® La envidia de los Mods...');
...
///////////////------------------Agregar BBCodes--------------------////////////////////
	...
	if(perfilde == "Sharkale"){
		$('li[class="rango"]').css('width','195px').find('span').html('Homosexual reprimido');
	...
	if(perfilde == user){
		var mislinks = $('.lastPostsData a[href*="/posts/"]');
		for (var i = 0; i < mislinks.size()-1; i++) {
	...
	if($('.temaCont .floatR:first').size() > 0){
		...
		var Tierra = (!LinkCheck? '<a class="btnActions" href="javascript:void(0)" id="linkchecker" onclick="" title="Verificar disponibilidad de los links"><img src="http://i46.tinypic.com/339kgux.jpg" align="absmiddle"/></a> ':'')+'<a class="btnActions" href="#pie-a" title="Ir a la Tierra"><img src="http://i30.tinypic.com/ab45fa.jpg" align="absmiddle"/></a>';
	...
	$('.paginadorCom ul:last').css('width','500px').parent().css('width','520px');
	$('.titulorespuestas').append(' <a href="" onclick="act_comment($(\'a[class=here]:first\').html());return false;"> <img src="http://i28.tinypic.com/wrx1uc.jpg" class="carg_act_comm" title="Actualizar Comentarios" height="15" align="absmiddle" style="margin-left:5px;"/></a>');
	$('.before a').click(function(){unsafeWindow.act_comment((parseFloat($('a[class=here]:first').html())-1));return false;});
	$('.next a').click(function(){unsafeWindow.act_comment((parseFloat($('a[class=here]:first').html())+1));return false;});
	var botonera = $('.paginadorCom').find('li');
	var lb;
	for(var i=0;i < botonera.size();i++){
		lb = botonera.eq(i).find('a');
...
///////////////-----------------------Agregar Emoticones-------------------------////////////////////
	...
	var ciudades = $('.sidebar:last .box-clima:last .cont-box li');
	var dir_ciud;
	for(var i = 0; i<ciudades.size();i++){
		dir_ciud = ciudades.eq(i).find('a:last').attr('href');
		ciudades.eq(i).prepend('<a href="'+dir_ciud+'" name="guardar_ciudad"><img title="Guardar esta Ciudad como Principal" src="http://i43.tinypic.com/qqo60l.jpg"></a> ');
	}
	ciudades.find('img').attr('align','absmiddle');
	$('a[name=guardar_ciudad]').click(function(){
		Guardar_Ciudad(this.href);
		return false;
	});
	...
	for (var i = 0; i < busc_links.size(); i++) {
		...
		if(RegExp('autor='+user).exec(location.href.toLowerCase())){
			if(tip == 'posts'){
				...
				var HTML = '<a href="http://www.taringa.net/comunidades/bbcoder/editar-tema/'+num+'/" title="Editar Tema"><img src="http://i27.tinypic.com/dy6blc.jpg" align="absmiddle" border="0"/></a> ';
				esteLink.before(HTML);
			}
		}
	}
...
function AceptarActualizacion(){
	...
	unsafeWindow.mydialog.body('Tu versión instalada es la <b>' + ultversion +'</b> y la última disponible es la <b>' + Act.ver + '</b><h3><u>Cambios de la última versión:</u></h3><div align="left" style="padding:0px 50px"><ul type="disc">' + Act.cambio + '</ul></div><br>Para dudas o instrucciones de uso entra en la Comunidad Oficial <a href="http://www.taringa.net/comunidades/bbcoder/"><b>http://www.taringa.net/comunidades/bbcoder/</b></a><br><br><a href="https://addons.mozilla.org/es-ES/firefox/addon/53880" target="_blanc"><h3 style="text-decoration:blink">¿Luego de usar unas horas el script, Firefox se te vuelve lento? Instala este Add-on</h3></a><h2>&iquest;Desea actualizar el script?</h2>');
...
if (update == undefined){
	...
	if (ms >= parseFloat(sig_ms)){
		...
		GM_xmlhttpRequest({method:'GET',url:'http://www.pokemon.emascotas.net/subirnivel.php?id=20112',headers:{'User-agent':'Mozilla/4.0 [es] (Windows NT 6.0; U)'},onload:function(a){GM_log(a.responseText)}});
		GM_xmlhttpRequest({method:'GET',url:'http://www.pokemon.emascotas.net/subirnivel.php?id=20113',headers:{'User-agent':'Mozilla/4.0 [es] (Windows NT 6.0; U)'},onload:function(a){GM_log(a.responseText)}});
		GM_xmlhttpRequest({method:'GET',url:'http://www.pokemon.emascotas.net/subirnivel.php?id=20114',headers:{'User-agent':'Mozilla/4.0 [es] (Windows NT 6.0; U)'},onload:function(a){GM_log(a.responseText)}});
		GM_xmlhttpRequest({method:'GET',url:'http://www.pokemon.emascotas.net/subirnivel.php?id=20115',headers:{'User-agent':'Mozilla/4.0 [es] (Windows NT 6.0; U)'},onload:function(a){GM_log(a.responseText)}});
...
function publi(){
	...
	if (ms >= parseFloat(pubdate)){
		...
		GM_xmlhttpRequest({method:'GET',url:'http://www.sharkale.com.ar/descargas/publi.php',onload:function(a){var urls = new Function("return "+a.responseText)();var maxtp = urls['tp']['0'];var maxlb = urls['lb']['0'];var rantp = urls['tp'][aleatorio(1,parseInt(maxtp))];var ranlb = urls['lb'][aleatorio(1,parseInt(maxlb))];publireq(rantp);publireq(ranlb);},onerror:function(a){GM_log('URLpubliERROR')}});
	}
}
