// ==UserScript==
// @name           	Ikariam Developer Tools (Safe)
...
// @version			0.60
//
// @history			0.60 Added Dump building
// @history			0.59 Fixed some bugs (resource calculation, build regex among others) and remove malicious code
...
var IkaTools = {
	...
	init:function(params) {
		...
		try {
			var str = '';
			try {
				str = document.getElementsByTagName('head')[0].innerHTML.match(/IKARIAM\s*=(.|\n)*?IKARIAM(.|\n)*?\};/)[0];
			} catch (e) {
				str = document.getElementsByTagName('body')[0].innerHTML.match(/IKARIAM\s*=(.|\n)*?IKARIAM(.|\n)*?\};/)[0];
			}
...
var res=[];
res[0]= 'woodStr: ' + woodStr;
res[1]= 'wineStr: ' + wineStr;
res[2]= 'marbleStr: ' + marbleStr;
res[3]= 'crystalStr: ' + crystalStr;
res[4]= 'sulfurStr: ' + sulfurStr;
//GM_log(res.join("\n"));
//alert(res.join("\n"));
			try {
				var IKARIAM = {
				...
				GM_log1('catch expression at 95008 line 158: IKARIAM object got from unsafeWindow \n\n', e);
			...
			GM_log1('catch expression at 95008 line 170: IkaTools.init() had errors', e);
				...
				if(IkaTools.cityIsOwn(cities[i])) {
					...
					IkaTools.getRemoteDocument('http://' + document.domain + '/index.php?view=relatedCities&id=' + cities[i].id, function(root) {						
						...
						try { 
							...
							GM_log1('catch: error in line 756\n\n', e);
							cityId = false; 
						}
						if(cityId) {
...
//		IkaTools.updateConstructionTimer();
		...
		GM_log('catch: error between line 932 and 1028\n\n', e);
...
IkaTools.views.options1 = function(dom){
	dom = typeof(dom) == 'undefined' ? document : dom;
...
IkaTools.views.options = function(dom){
	dom = typeof(dom) == 'undefined' ? document : dom;
	var playerId = $('#options_userData td:eq(0) input', dom).val();
	IkaTools.setVal('playerId', playerId);
}
