// ==UserScript==
// @name       better douban group
// @namespace  http://ktmud.com/
// @version    0.1
// @description  sortable my group topics and more
// @match      http://*.douban.com/group/
// @match      http://*.douban.com/group/?start=*
...
function init() {
 ...
 Do(function(){
   var tb = $('table.olt tbody').addClass('list');
   if (!tb.length) return;
   
   var thead = $('<thead><tr><th class="sortable" data-sort="td-subject">话题</th><th class="sortable asc" data-sort="td-reply">回应</th><th class="sortable asc" data-sort="td-time">时间</th><th class="sortable" data-sort="td-group">小组</th></tr></thead>').css('display', 'table-header-group').css('cursor', 'pointer');
   tb.before(thead).children().eq(0).css('border-bottom', '1px solid #eee').end().end().prev().prev('thead').remove();
   tb.find('td:last-child').addClass('td-group');
      tb.find('td:nth-child(1)').addClass('td-subject');
   tb.find('td:nth-child(2)').addClass('td-reply');
   
   var table = tb.parent().attr('id', 'topics');
   var topic_list = new List(table[0], { valueNames: ['td-subject', 'td-reply', 'td-time', 'td-group'] });
   ...
   thead.delegate('th', 'click', function(e) {
     ...
     if (n == 'td-time') {
       ...
       var f = function(a, b) {
         a = a.values()[n];
         b = b.values()[n];
         var u = compareUnit(a, b);

         if (u === 0) {
           var is_back = a.indexOf('前') > 0;
           if (!a) return 1;
           if (!b) return -1;
           a = reg_t.test(a) ? a.match(reg_t)[0] : a;
           b = reg_t.test(b) ? b.match(reg_t)[0] : b;
           a = parseFloat(a.replace(/[\:\-]/, ''), 10);
           b = parseFloat(b.replace(/[\:\-]/, ''), 10);
           if (is_back) return b-a;
           return a-b;
         }
         return u;
       }
     ...
     topic_list.sort(e, {
      ...
      if (is_asc) node.removeClass('asc');
   });
   
   function compareUnit(a, b) {
     if ((a + b).split('分钟').length == 3 ||
     (a + b).split('秒').length == 3 ||
     (a + b).split('小时').length == 3
        ) {
       return 0;
     }
     return tunit(a) - tunit(b);
   }
   function tunit(s) {
     if (s.indexOf('昨天')>-1) {
       return -1;
