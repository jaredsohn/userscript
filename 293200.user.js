...
/* 
Features:
 - Add last chapter & search link for scanlator
...
//alert(groups['999']);
	...
	var $name,$ch,$scan_node,$scan;
    var	$str = "http://www.mangaupdates.com/groups.html?id=";	

    var	$name_column = $('[class="lcol_nopri text"]');
    if($name_column.length==0)
     $name_column = $('[class="lcol3 text"]');
	$name_column.each(function(  index,element  ) {
		$.post( $(this).find('a').attr('href'), function( data ) {
			html = jQuery(data);
			$tmp=html.find('div[class="sCat"]:contains("Latest Release")').next();
			$scan_node=$tmp.find('a');
			$scan=$scan_node.html();
			$scan_url=groups[$scan_node.attr('href').substr($str.length)];
			$ch = $tmp.find('i:nth-child(2)').html();
			if($ch==null)
				$ch = $tmp.find('i').html();

			if($scan_url == null)
			     $name = '<td>'+'<a href="https://www.google.com/search?q='+$scan+'+scans">'+'c.'+$ch+" by "+$scan+'</td>';
    	     else
        	     $name = '<td>'+'<a href="'+$scan_url+'">'+'c.'+$ch+" by "+$scan+'</td>';
            if($ch.indexOf('(end)') >= 0)
                $(element).css('background-color','#E9967A');
			$(element).append($name);
		});
	});
