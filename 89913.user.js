...
// @include        http://hi.baidu.com/pub/show/modifytext*
// @include        http://hi.baidu.com/pub/show/createtext
// ==/UserScript==


// ============================================
// name        hotkeybloghighlightcode
// author      FreezeSoul
...
//Highlight.target = document.getElementById("baidu_editor_0").contentWindow;
Highlight.targetId = "baidu_editor_0"
Highlight.target = null;

var highLightDiv = document.createElement('div');
var highLightDivStyle = 'width:131px;position:fixed;bottom:0;right:0;';
var highLightCSS = document.createElement('style');
highLightCSS.innerHTML='#highLightOuter li{width:121px;height:30px;padding-left:10px;background:url(http://img.baidu.com/hi/neweditor/neweditor-btn-bg.png) 0 -93px no-repeat;}.liright{display:block;height:30px;line-height:30px;text-align:center;font-size:14px;color:#717171;padding-right:10px;background:url(http://img.baidu.com/hi/neweditor/neweditor-btn-bg.png) right -93px no-repeat;}#highLightOuter li:hover{background:url(http://img.baidu.com/hi/neweditor/neweditor-btn-bg.png) 0 0 no-repeat;cursor:pointer;}#highLightOuter li:hover .liright{background:url(http://img.baidu.com/hi/neweditor/neweditor-btn-bg.png) right 0 no-repeat;color:#fff;font-weight:bold;}#highlightboxwrap{padding:20px;position:absolute;z-index:99999;}#highlightbox{}';
window.document.body.appendChild(highLightDiv);

window.document.body.appendChild(highLightCSS);

highLightDiv.setAttribute("style",highLightDivStyle);
highLightDiv.setAttribute("id","highLightOuter");
highLightDiv = document.getElementById("highLightOuter");
highLightDiv.appendChild(document.createElement('ul'));
highLightUl = highLightDiv.getElementsByTagName('ul')[0];
var liright1 = '<span class="liright">';
var liright2 = '</span>';

highLightUl.appendChild(document.createElement('li'));//高亮代码
//highLightUl.appendChild(document.createElement('li'));//添加提示

highLightLi=highLightUl.getElementsByTagName('li');
highLightLi[0].innerHTML=liright1+'高亮代码'+liright2;
//highLightLi[1].innerHTML=liright1+'添加提示'+liright2;
//highLightLi[2].innerHTML=liright1+'增加文章'+liright2;

var temp = document.createElement('textarea');
var tempdiv =  document.createElement('div');
temp.id='highlightbox'
temp.rows='5'
temp.cols='100'

tempdiv.id = 'highlightboxwrap';
tempdiv.style.display = 'none'



tempdiv.appendChild(temp)
tempdiv.innerHTML +="<br><input type='button' value='确定' id='highlightok' style='margin-right:100px;'/><input type='button' value='取消' id='highlightclose' />"
document.body.appendChild(tempdiv)
tempdiv.style.left = document.documentElement.scrollLeft + (document.documentElement.offsetWidth-document.getElementById('highlightbox').offsetWidth)/2 + 'px';
tempdiv.style.top = document.documentElement.scrollTop + (document.documentElement.offsetHeight-document.getElementById('highlightbox').offsetHeight)/2 + 'px';
temp = tempdiv = null;

document.getElementById('highlightok').addEventListener("click",function(){
	...
	r.deleteContents();
	var temp = Highlight.target.document.createElement("div");
	var newP = Highlight.target.document.createElement("p");
	temp.setAttribute("style", 'background-color:rgb(235,235,235)');
	newP.setAttribute('style','background-color:rgb(235,235,235);margin-left:10pt;');
	temp.appendChild(newP);
	newP.appendChild(cs);
	newP.innerHTML = Highlight.highlighter.HighlightAll(document.getElementById('highlightbox').value);
	r.insertNode(temp);
	document.getElementById('highlightboxwrap').style.display = 'none'
	document.getElementById('highlightbox').value=''
},false);

document.getElementById('highlightclose').addEventListener("click",function(){
	document.getElementById('highlightboxwrap').style.display = 'none'
},false);

highLightLi[0].addEventListener("click",function(){
	/*
		Highlight.highlighter = new Highlighter.CSS();
		Highlight.codeHtml();
	...
	*/
	document.getElementById('highlightbox').value = '';
	if (!Highlight.target) {
		Highlight.target = document.getElementById(Highlight.targetId).contentWindow;
	}
	var temp = document.getElementById('highlightboxwrap')
	temp.style.display = (temp.style.display == 'none') ?'block':'none'
	temp.style.left = document.documentElement.scrollLeft + (document.documentElement.offsetWidth-document.getElementById('highlightbox').offsetWidth)/2 + 'px';
	temp.style.top = document.documentElement.scrollTop + (document.documentElement.offsetHeight-document.getElementById('highlightbox').offsetHeight)/2 + 'px';
	
},false); 
/*
highLightLi[1].addEventListener("click",function(){
	if (!Highlight.target) {
		Highlight.target = document.getElementById(Highlight.targetId).contentWindow;
	}
		Highlight.codeHtml1();
...
//bgcolor
/*
Highlight.outerStyle = "color:rgb(255,0,0)";
...
//comments outerStyle
Highlight.cmtStyle="display:block;color:green;border:1px #F8F8F8 solid;padding:8px;margin:10px;line-height:15px;font-size:14px;font-family:'微软雅黑','雅黑','宋体',sans-serif;font-style:normal;font-weight:normal;";
...
Highlight.codeHtml = function(){
	...
	if (document.all) {
		...
		if (arguments.length == 0){
			r.pasteHTML("<div style='" + Highlight.outerStyle + "'>" + Highlight.highlighter.HighlightAll(s) + "</div>");
		}else{
			r.pasteHTML("<div style='" + Highlight.outerStyle + "'>" + s + "</div>");
		}
	} else {
		...
		var temp = Highlight.target.document.createElement("div");
		var newP = Highlight.target.document.createElement("p");
		newP.setAttribute("style", 'background-color:rgb(200,200,200);margin-left:10pt;');
		newP.appendChild(cs);
		if (arguments.length == 0){
...
Highlight.codeHtml1 = function(){
	...
	if (document.all) {
		...
		r = Highlight.target.document.selection.createRange();
		if (arguments.length == 0){
			r.pasteHTML("<var style='" + Highlight.cmtStyle + ">/***  提 示  ***/<br/>" + s + "</var>");
		}else{
			r.pasteHTML("<var style='" + Highlight.cmtStyle + "'>/***  提 示  ***/<br/>" + s + "</var>");
		}
	} else {
		...
		var cs = r.cloneContents();
		r.deleteContents();
		var newP = Highlight.target.document.createElement("var");
		newP.setAttribute("style", Highlight.cmtStyle);
		newP.appendChild(cs);
		if (arguments.length == 0){
			newP.innerHTML = "/***  提 示  ***/<br/>"+newP.innerHTML;
		}else{
			newP.appendChild(cs);
		}
		r.insertNode(newP);
...
{
	return str.replace(/ /gi,'&nbsp;').replace(/\t/gi,'&nbsp;&nbsp;&nbsp;&nbsp;').replace(/<br\s*\/?>|&lt;br\s*\/?&gt;/gi, '\n').replace(/</gi, '&lt;').replace(/>/gi, '&gt;');
}
...
Highlighter.CSS = function()
{
	...
	this.regexList = [
		...
		{ regex: new RegExp(Highlighter.GetKeywords(funcs), 'gmi'),		css: '' },//func
