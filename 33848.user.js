// ==UserScript==
// @name           amex trying beta
// @namespace      http://userscripts.org/scripts/show/33848
	...
	*/
   	this.insertPowerIntoPage = function(profile) {
		...
		trSolo.appendChild(tdSolo);
      		...
      		divSolo.textContent = "BendraGalia" + ":";
...
function Profile() {	
	...
	this.calculatePower = function(newWeaponDamage, newArmor) {
		...
		var weaponPower = (((10 * weaponDamage) / 10 + armorTolerance[armor]) * 0.9);

		if(newWeaponDamage || newArmor)
			return  Math.round((playerPower + weaponPower));
   		
		this.safeSoloPower = Math.round((playerPower + weaponPower) * 0.9);
                this.normalSoloPower = Math.round((playerPower + weaponPower) * 1.29);
		this.riskySoloPower = Math.round((playerPower + weaponPower) * 1.3 );
		this.dangerSoloPower = Math.round((playerPower + weaponPower) * 1.4);
