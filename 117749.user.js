// ==UserScript==
// @name           Battlelog: Show platoon members on servers
// @namespace      olemartinorg
...
var addPlatoonButton = function () {
	...
	<div id="comcenter-platoons" class="comcenter-preview-area " style="display: none;">\
		<div class="comcenter-notification-title comcenter-notification-notification-title">\
			...
			<div class="comcenter-contract" id="comcenter-platoons-close">&nbsp;</div>\
			<div class="base-clear"></div>\
		</div>\
		<div class="comcenter-notification-content">\
			<div class="comcenter-notification-list" style="max-height: 500px; overflow-y: hidden;">\
	...
	var fillPlatoon = function () {
		...
		$.each(data.context.platoon.members, function (index, player) {
			if (player.persona.user.presence.isOnline &&
				player.persona.user.presence.isPlaying &&
				player.persona.user.presence.serverGuid){
				var guid = player.persona.user.presence.serverGuid;
				if ( guid in servers ){
		...
		$.each(servers2, function (index, playerlist) {
			var guid = playerlist[0].persona.user.presence.serverGuid;
			...
			$.each(playerlist, function (index, player) {
				battlebubble += player.persona.user.username;
		...
		$('#base-dropdown-menu-your-platoons').children().slice(1).each(function () {
					...
					<div class="comcenter-notification">\
						<div class="battlereport-minicard battlereport-minicard-anchor" style="height:auto;">\
							<div class="battlereport-minicard-map" style="width:60px;height:60px;">\
								<img alt="platoonimg" src="http://static.cdn.ea.com/battlelog/prod/emblems/26/17/2832655241008752017.jpeg?v=1320150127.64" style="display:block;margin:auto;padding-top:10px;">\
							</div>\
							<div class="battlereport-minicard-body">\
							...
							<div class="base-clear"></div>\
							<a class="battlereport-minicard-link" id="p-'+id+'-link" href="'+href+'"></a>\
						</div>\
					</div>\
				</surf:container>');
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('#platoon-'+id).find('img').first().attr('src', 'http://static.cdn.ea.com/battlelog/prod/emblems/60/'+id+'.jpeg').attr('style', '');
					...
					$.each(data.context.platoon.members, function (index, player) {
						if (player.persona.user.presence.isOnline &&
							...
							player.persona.user.presence.serverGuid){
							playing++;
						}else if (player.persona.user.presence.isOnline){
							online++;
						}
					});
