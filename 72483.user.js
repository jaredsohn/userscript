// ==UserScript==
// @name           Santos FC Moderacao
// @namespace      by Jp
// @description    Script para facilitar o trabalho da moderacao da Santos FC
// @santosfc      orkut.com.br/Community?cmm=110917399
...
/*
+---------------------------------------------------------------------+
| 		Visite a comunidade Santos FC no Orkut 	              |
|		http://www.orkut.com.br/Community?cmm=110917399	      |
|		Forum Santos Fc: 				      |
|		/CommTopics?cmm=110917399	             	      |
+---------------------------------------------------------------------+
*/
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=110917399&tid=5571578451601059025", false);
	...
	function captchaONOFF(env, status) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgs?cmm=110917399&tid=5571578919752494289", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function captchaDelTopic(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgPost?cmm=110917399&tid=5571354962977811665", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[b]Titulo[/b]: ".concat(titulo, breakLine, "[b]Motivo[/b]: ", motivo, breakLine, "[b]Horario[/b]: ", pegaData(), breakLine, breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
...
("\n\n[silver]", String(Math.random()), "[/silver]");
		...
		var ajax = new XMLHttpRequest;
		ajax.open("POST", "CommMsgPost?cmm=110917399&tid=5571578919752494289", false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
		ajax.send(array_values(envia));
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Você Tem certeza que deseja deletar este t\ópico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('digite o motivo de estar deletando este topico:\\n','Fora de formato'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
