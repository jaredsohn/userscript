(function () {
// ==UserScript==
// @name           Sobat Ganool V.1 [Edited By Kaum Ndaloe]
// @namespace      http://kaumndaloe.blogspot.com
// @author         Kaum Ndaloe
// @description    ganool.com auto Links! [Tinypaste.com]
// @version        V.1
// @icon           http://www.gravatar.com/avatar/bdb861650cd3b4a7be02052005302d6b.png
// @updateURL      http://userscripts.org/scripts/review/187283
...
const yodUpdate = {
  ...
  script_name : 'Sobat Ganool v.1 [Edited By Kaum Ndaloe]'
}
...
function doExec(e) {
  ...
  for (i=0; i<parafs.length; i++) {
    ...
    var isTP = c1(".//a[contains(@href,'tinypaste.com')]", p3.parentNode);
    if(!isTP && crp_related) isTP = c1(".//a[contains(@href,'tinypaste.com')]", crp_related.previousElementSibling);
...
';
    ...
    if (isTP) {
      if (hash = regexx(isTP.href, /tinypaste\.com\/([^'"]+)/)) {
