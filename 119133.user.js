...
if(urlVisitada[0]==urlEquipoVecino)
{
	...
	for (var i=1; i<allHTMLTags.length; i++) 
	{
		...
		if(allHTMLTags[i].innerHTML=="Plantilla" && allHTMLTags[i].tagName=="H2")
			indice=i;
		if (allHTMLTags[i].className=="textonoticiasmini"  && allHTMLTags[i].rows.length>5 && allHTMLTags[i-1].innerHTML=="Plantilla") 
		{
			var aux=0;
			...
			for (var j=1;j<allHTMLTags[i].rows.length;j++)
			{
				...
				var columnaMR=0;
				for (var k=0;k<6;k++){
					columnaMR=columnaMR+parseInt(fila.cells[6+k+aux].firstChild.nodeValue);
				}
				columnaMR=columnaMR/6;
				var columnaME=fila.cells[13+aux];
				...
				allHTMLTags[i].rows[j].cells[12+aux].innerHTML="<b>"+columnaMR.toFixed(2)+"</b>";
				MR11[j-1]=columnaMR;
