// ==UserScript==
// @name          عارض التقرير بشكل إحترافي [ Just.Smile ]
// @namespace      tribalwars
// @author	   Marc H. (Salandora)
// @description    يعرض التقرير عند تقريب الماوس إيه 
...
if (location.href.match(/screen=settings&mode=settings/))
{
	...
	spalten[ispalte].innerHTML = "[ Just.Smile ]";
	...
	berichtelink.href ="#";
	berichtelink.innerHTML = "<br>[ Just.Smile ]";
	...
	spalten[ispalte].innerHTML = "<b style='padding-right:222px;'>إظهار الحظ:</b>" + zeigeGlueck + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:133px;'>إظهار وحدات المهاجم:</b>" + zeigeAngreiferEinheiten + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:14px;'>إظهار خسائر المهاجم:</b>" + zeigeAngreiferEinheitenVerluste + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:121px;'>إظهار قوات المدافع:</b>" + zeigeVerteidigerEinheiten + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:2px;'>إظهار خسائر المدافع:</b>" + zeigeVerteidigerEinheitenVerluste + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:17px;'>عرض القوات خارج القرية:</b>" + zeigeEinheitenAusserhalb + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:135px;'>إظهار التجسس على الموارد:</b>" + zeigeErspaehteRohstoffe + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:140px;'>إظهار المباني:</b>" + zeigeErspaehteGebaeude + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:222px;'>إظهار الغنائم:</b>" + zeigeBeute + "<br>";
	spalten[ispalte].innerHTML += "<b style='padding-right:105px;'>إظهار ولاء القرية:</b>" + zeigeZustimmung + "<br>";
...
{
	if (confirm("للإستمرار إضغط موافق لتعديل الخيارات [ Just.Smile ]"))
	{
		Konfigurieren();
		if (confirm("أطيب التمنيات من [ Just.Smile ] مع تمنياتي لكم باللعب الممتع .."))
		{
		}
	}
}
...
function Konfigurieren()
{
	...
	if (confirm("اذا كان المدافع يملك وحدات هل تريد إظهارها ؟"))
	{
		GM_setValue("ZeigeVerteidigerEinheiten_" + welt, "تم التفعيل");
		if (confirm("هل تريد إظهار خسار المدافع?"))
	...
	if (confirm("إذا كان هناك وحدات خارج القرية .. هل تريد إظهارها ؟"))
		GM_setValue("ZeigeEinheitenAusserhalb_" + welt, "تم التفعيل");
	else
		GM_setValue("ZeigeEinheitenAusserhalb_" + welt, "غير مفعل");

	if (confirm("هل تريد إظهار الموارد الموجودة في القرية ؟"))
...
function ResetteBerichte()
{
	...
	if (confirm("تم تفريغ ذاكرة السيكربت !"))
	{
	}
}
...
{	
	...
	spalte1.innerHTML = "<b>" + (erfassteunterstuetzungsberichte.indexOf(berichtsid) != -1 ? "Unterstützter Spieler:" : "المهاجم([ Just.Smile ]:") + "</b>";
...
function ErstelleVerteidigerTabelle(berichtsid)
{
	...
	spalte1.innerHTML = "<b>" + (erfassteunterstuetzungsberichte.indexOf(berichtsid) != -1 ? "Einheiten:" : "المدافع[ Just.Smile ]:") + "</b>";
	spalte2.innerHTML = "<b>" + GM_getValue(gmbase + "Name", "") + "</b>";
