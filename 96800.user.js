...
if(document.getElementsByClassName(tablename)[0])
{
	...
	function processTop20(idx)
	{
		...
		function giveConv()
		{
			...
			{
				textFinal += "[b][color=white]"+rank[i]+"[/color] -- [color=#33FF33]"+prog[i]+"[/color] "+getTirets(prog[0].length,prog[i].length)+" [color=#00CCCC]"+name[i]+"[/color] "+getTirets( maxLength(name), String(name[i]).length)+" [color=#33FF33]"+ally[i]+"[/color][/b]\n";
			}
			textFinal+="[/font]";
