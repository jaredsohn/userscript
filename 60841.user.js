...
function canMission() {
  ...
  if (isChecked('multipleJobs') &&
    ...
    // Dn't do expBurners or biggest exp job if energy can be salvaged
  ...
  if (energy <= SpendEnergy.floor && !SpendEnergy.canBurn) {
    DEBUG('Not spending energy: energy=' + energy +
          ', floor=' + SpendEnergy.floor + ', burn=' + SpendEnergy.canBurn);
    return false;
  }
...
function checkLanguage() {
  ...
  /*if (document.location.href.match(/fb_sig_locale=(\w+)/)) return;

  DEBUG('Language is "' + RegExp.$1 + '".');
...
function customizeMasthead() {
  ...
  /*if (isChecked('fbwindowtitle')) {
    ...
    if (fbName && fbName.firstChild)
      document.title = document.getElementById("fb_menu_account").firstChild.innerHTML;
  }
  ...
  makeElement('div', mastheadElt, {'style':'position: absolute; top: 24px; right: 17px; text-align: left; font-size: 10px; font-weight: bold; color: #FFFFFF'}).appendChild(document.createTextNode(mwapTitle));
  ...
  if (isChecked('multipleJobs')) {
    ...
    for (var i=0, iLength=multiple_jobs_list.length; i < iLength; ++i) {
      ...
      if (availableJobs[mission[4]][mission[3]] != null &&
          availableJobs[mission[4]][mission[3]].indexOf(parseInt(job)) == -1) {
        mission[7] = 0;
        addToLog('warning Icon', 'WHY! set jobratio to 0, job: ' + missions[job][0] + ', ratio: ' + missions[job][7] + ', evil: ' + availableJobs[mission[4]][mission[3]]);
      }
...
function goProfileNav(player) {
  ...
  if (!elt) {
    ...
    if (elt && elt.getAttribute('onclick').match(/user=(\d+)/)) {
