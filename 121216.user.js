// ==UserScript==
// @name           headbanger++
// @description    işte özellikler falan
// @namespace      capua genclik
// @version        0.2.2
// @author         
...
// ==/UserScript==
var mevcutVersion = "0.2.2";
if(localStorage.getItem("headbangerReserved")==null) localStorage.setItem("headbangerReserved", "|headbanger plus plus|");	

//ilk kurulum ayari
if(localStorage.getItem("headbangerposer")==null) localStorage.setItem("headbangerposer", "|yokullah|");		//ilk 

kurulum ayari

if(localStorage.getItem("headbangerReserved").substr(0, 1)!="|" && localStorage.getItem("headbangerReserved").length>0)
localStorage.setItem("headbangerReserved", "|" + localStorage.getItem("headbangerReserved"));

if(localStorage.getItem("headbangerReserved").substr(localStorage.getItem("headbangerReserved").length-1, 1)!="|" && 

localStorage.getItem("headbangerReserved").length>0)
localStorage.setItem("headbangerReserved", localStorage.getItem("headbangerReserved") + "|");

var tarih = new Date();
var bugun = tarih.getTime();
var miniPopup = 0;
sayfaList = localStorage.getItem("headbangerReserved");
...
// eğer buraya girildiyse orta frame yüklenmiş demektir
	baslikAdi = "";
	baslikAdi = document.title.replace("headbanger sozluk - ", "");
	document.title = baslikAdi + " - headbanger sozluk";
...
//	if(gelenAdres!="http://headbanger.sozlukspot.com/" && gelenAdres!="http://www.headbangersozluk.com")	

funCerceveGetir();

	funAntiposer();			// poser listesindeki yazarların entrylerini gizle		
	funEtGetir();				// başlıklarda @'lere verilen cevaplarda @ tıklandığında mini popup 
...
if(typeof(unsafeWindow)=='undefined')
{
	...
	{
		var poserListesi = localStorage.getItem("headbangerposer");
		...
		for(var i=0; i <= kacTaneposerEklenecek; i++)
		{
			...
			{
				siradakiposer = eklenecekposerListesi[i].replace(/^\s+|\s+$/g,""); // replace kısmı trim 

yapıyor
				if(poserListesi.indexOf("|" + siradakiposer + "|")==-1)
				{
					poserListesi += siradakiposer + "|";
					localStorage.setItem("headbangerposer", poserListesi);
					poserListesiGetir();
				}
			}
		}
	...
	function reserveBosalt()
	{
		...
		{
			localStorage.setItem("headbangerReserved", "");
	...
	function poserBosalt()
	{
		...
		{
			localStorage.setItem("headbangerposer", "")
			document.body.innerHTML = "bu liste bomboş.";
		}
	}	
	...
	function etGoster(gosterilecekEt, hangiBaslik)
	{
		...
		kacheadbangerSayfa = ((gosterilecekEt - (gosterilecekEt % 25)) / 25) + 1;
                if(gosterilecekEt % 25==0) kacheadbangerSayfa--;
		adres = "http://" + location.hostname + "/w/" + hangiBaslik.replace(/ /g, "-") + "/";
		adres += kacheadbangerSayfa + "/";
		...
		if(!miniPopup)
		{
			...
			icten.style.backgroundColor = window.getComputedStyle(document.body, 
		...
		sorgu.onreadystatechange = function(){
			if(sorgu.readyState == 4)
			{
				if(sorgu.status == 200)
				{
					...
					{
						yazarTarih = gelenCevap.split(' et="' + gosterilecekEt + '" 

style="margin-left:8px;"  >')[1].split('class="yazar_tarih">')[1].split('<script language')[0];
						gelenCevap = gelenCevap.split(' et="' + gosterilecekEt + '" 

style="margin-left:8px;"  >')[1].split('<div')[0];
						gelenCevap = "<table><tr width='80%'><td>bu başlıkta @" + gosterilecekEt + " 

demiş ki; <br /> <br />" + gelenCevap + "</td></tr>";
	...
	function swfGizleGoster(nereye, neyi)
	{
		if(document.getElementById(nereye).getElementsByTagName("object").length<=0)
		{
			...
			cont  += '<object type="application/x-shockwave-flash" data="' + neyi + '" width="640" 
...
height="480">';
	...
	function poserListesiGetir()
	{
		...
		for(var i=0; i<=poserSayisi; i++)
		{
			...
			{
				poserSayfasi += poserListesi.split("|")[i] + ' - <a href="#" 

onclick="javacript:funposerSil(\'' + poserListesi.split("|")[i] + '\');">[kaldır]</a><br>';
			}
		}
			...
			poserSayfasi += "<br><input type=\"button\" 

onclick=\"topluposerEkle(document.getElementById('yeniposerListesi').value)\" id=\"poserListesiniEkle\" value=\"ekle\" 
...
/></div>";
	...
	function funposerSil(kim)
	{
		...
		{
			var eskiposerList = localStorage.getItem("headbangerposer");
			var yeniposerList = eskiposerList.replace(kim + "|", "");
			if(yeniposerList.substr(-1)!= "|") yeniposerList += "|";
	
			localStorage.setItem("headbangerposer", yeniposerList);
	...
	function funposerYap(kim)
	{
		...
		{
			if(localStorage.getItem("headbangerposer").length > 0)
			{
				var eskiposerList = localStorage.getItem("headbangerposer");
				localStorage.setItem("headbangerposer", eskiposerList + kim + "|");
				parent.frames[2].location.reload();
			}else{
				localStorage.setItem("headbangerposer", "|" + kim + "|");
				parent.frames[2].location.reload();
			}
		}
	}
	...
	function reservedSayfalariEkle(sayfaList)
	{
		...
		var sayfaBolunmus = localStorage.getItem("headbangerReserved").split("|");
		var temaKonumu = parent.frames[0].document.head.innerHTML.indexOf("<link rel=\"stylesheet\"");
		var iconKonumu = parent.frames[0].document.head.innerHTML.indexOf('<script type=\"text/javascript\"');
		...
		for(var i=0; i<=localStorage.getItem("headbangerReserved").split("|").length-1; i++)
		{
			if(sayfaBolunmus[i].length > 0)
			{
				if(sayfaBolunmus[i].indexOf("(")>-1)
				{
					...
					reservedSayfalar += "<a href=\"http://" + location.hostname + "/w/" + 

sayfaBolunmus[i].replace(/ /g, "-") + "/" + gidilecekSayfa + "/\" target=\"sportakisim\">" + sayfaBolunmus[i] + " (" + 

gidilecekSayfa + ")" + "</a> - <a href=\"#\" onclick=\"toggleReserve('" + sayfaBolunmus[i] + " (" + gidilecekSayfa + ")" + 

"', '1')\">[kaldır]</a><br>";
				} else {
				reservedSayfalar += "<a href=\"http://" + location.hostname + "/w/" + 

sayfaBolunmus[i].replace(/ /g, "-") + "\" target=\"sportakisim\">" + sayfaBolunmus[i] + "</a> - <a href=\"#\" 

onclick=\"toggleReserve('" + sayfaBolunmus[i] + "', '1');\">[kaldır]</a><br>";
				}
			}
		}
	...
	{
		if(baslik.indexOf("(")>-1) baslikTemp = baslik.split("(")[0].replace(/\s+$/,""); // parantez içini ve sondaki 

boşluğu sil
		
		if(localStorage.getItem("headbangerReserved").length != 0)
		{
			if(sayfaList.indexOf(baslik + "|")==-1)
			{
				sayfaList += baslik + "|";
				localStorage.setItem("headbangerReserved", sayfaList);
...
{	
	var sayfaList = localStorage.getItem("headbangerReserved");
...
{
	var tableSayisi = document.body.innerHTML.split(/<table/g).length-1;		// sayfada kaç tane entry olduğunu 
...
öğren
	...
	for(k=0;k<tableSayisi;k++)
	{
		...
		{
			var yazaradi = (document.getElementById("li_" + 

myId.replace("t","")).getElementsByClassName('yazar_tarih')[0].getElementsByTagName('a')[1].getAttribute("href")).replace("/"

,"@");			// yazaradi = "@yanak kafasi"; seklinde
...
"");		}
			...
			{
				basliktakiYazarLink = basliktakiYazarLink.replace(basliktakiYazarLink.split("/")[3] + "/" + 

basliktakiYazarLink.split("/")[4] + "/", "w/" + baslikAdi.replace(" ","-") + "/");
			}
			if(basliktakiYazarLink.indexOf("&p=")>-1) 
...
basliktakiYazarLink=basliktakiYazarLink.replace(basliktakiYazarLink.split("/")[5], "");
			...
			yazarbtn.innerHTML = "<a href=\"javascript:location='" + basliktakiYazarLink + "'\">baslikta 
...
{
	tempList = sayfaList + "|";
	if(tempList.indexOf("|" + baslikAdi + "|")==-1 && tempList.indexOf("|" + baslikAdi + " (" + acikSayfa + ")")==-1)	

				// eğer reserved başlık listesinde adı yoksa
	{
	...
	togglebtn.innerHTML = "<tr><td onclick=\"toggleReserve('" + baslikAdi + " (" + acikSayfa + ")');\" 

onmousedown=\"this.id='butDown'\" onmouseout=\"this.id=''\" onmousemove=\"this.id='butOver'\" class=\"linkler\"><a href=\"#\" 

id=\"reservetxt\">" + reservedText + "</a>&nbsp;</td></tr>";
...
function funResimGom()
{
	...
	for(i=0;i<linkSayisi;i++)
	{
		...
		if((mevcutLink.href.indexOf(".png")>-1 || mevcutLink.href.indexOf(".jpg")>-1 || 

mevcutLink.href.indexOf(".jpeg")>-1 || mevcutLink.href.indexOf(".gif")>-1) && (linkListesi[i-1].id!=rastgele && 

mevcutLink.href.substr(-1)!="/") && mevcutLink.href.indexOf("headbangercaps.in")==-1)
		{
			...
			gomuluResim = mevcutLink.href + '<a href=\'javascript:return void(0);\' onclick="gizlegoster(\'' + 

rastgele +'\')">' +
			"<img src=\"http://www.sozlukspot.com/res/img.png\" height=\"10\" width=\"10\" border=\"0\" /></a>" +
			"<div id='" + rastgele + "' style='display:none;overflow:hidden;'><img src='" + mevcutLink.href + "' 

style='max-width=790px' /></div>";
...
document.getElementById(entryID).getElementsByClassName('yazar_tarih')[0].getElementsByTagName('a')[1].getAttribute("href");
		yazarAdi = yazarAdi.replace("/","").replace(/-/g, " ");			// yazaradi = "yanak kafasi"; 
...
{
	poserList = localStorage.getItem("headbangerposer");
...
,"").replace(/-/g, " ");			// yazaradi = "yanak kafasi"; seklinde

		//alert("id="+myId+" / adi="+yazarAdi);
			var yazarposerbtn = document.createElement("td");			// yazar entryleri icin td'yi 
...
function funSwfGom()
{
	...
	{
		mevcutLink = linkListesi[i];
		if(mevcutLink.href.indexOf(".swf")>-1  && linkListesi[i-1].id!=rastgele && mevcutLink.href.substr(-1)!="/" && 

mevcutLink.href.indexOf(location.hostname)==-1)
		{
			rastgele=Math.floor(Math.random()*9999);
			mevcutLink.id = rastgele;
			mevcutLink.target = "blank";
			mevcutLink.rel = "nofollow";
			gomuluResim = mevcutLink.href + '<a href="javascript:swfGizleGoster(\'' + rastgele +'\', \'' + 
...
//	if(bugun - localStorage.getItem("headbangerUpdate") > 86400000 || localStorage.getItem("headbangerUpdate")==null)	

// 86400000 = 24 * 60 * 60 * 1000ms = 1 gun
//	{
	var tarayici = detectBrowser();
	if(tarayici!="opera")									// chrome veya firefox ise
	{
		localStorage.setItem("headbangerUpdate", bugun);
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://userscripts.org/scripts/source/114075.user.js',
			headers: {
			...
			onload: function(responseDetails) {
				if(responseDetails.status == "200")
				{
					...
					var guncelVersion = gelenCevap.split("@version")[1].split("@author")[0].replace("//", 

"").replace("\r\n", "").replace(/ /g, "");
					if(guncelVersion != mevcutVersion)
					{
						baslikAdres = "http://userscripts.org/scripts/source/114075";
						//alert("Sizin surumunuz " + mevcutVersion + " ve son surum " + 
...
guncelVersion);
						...
						guncellebtn.innerHTML="<a href='" + baslikAdres + "' target='sportakisim' 

id='guncelleme'>güncelleme var</a>";
...
function funBaslikSabitle()
{
		...
		if (bg.match(/(transparent|rgba)/i)) {
			setTimeout(function() { 
				sabitBaslik.style.backgroundColor = window.getComputedStyle(document.body, 
...
function funEtGetir()
{
	...
	var sayfaBaslik  = document.title.replace("headbanger sozluk - ", "").replace(" - headbanger sozluk", "");
	for(var i=0; i<entrySayisi; i++)
	{
		if(entryListesi[i].innerHTML.indexOf("@")>-1)
		{
			...
			{
				entryListesi[i].innerHTML = entryListesi[i].innerHTML.replace(etList[k], '<a 

href="javascript:etGoster(\'' + etList[k].replace("@","") + '\', \'' + sayfaBaslik + '\');">__AT__' + 
