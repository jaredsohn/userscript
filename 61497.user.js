...
(function() {
    ...
    //var LABEL_POFF = ["ON", "OFF"];
    //var LABEL_SOFF = ["ON", "OFF"];
    //var LABEL_ROFF = ["ON", "OFF"];
    //var LABEL_DOFF = ["ON", "OFF"];
    ...
    var jpflg = (location.hostname == "www.amazon.co.jp");
    var i;
    ...
    var ASSOCIATE_ID, AMAZON_URL;
    if (jpflg){
        ASSOCIATE_ID = "associate_ID-22";
        AMAZON_URL = "http://www.amazon.co.jp/gp/product/";
    } else {
        ASSOCIATE_ID = "associate_ID-21";
        AMAZON_URL = "http://www.amazon.de/gp/product/";
    }

    /*  商品情報取得  */
    asin = $xi(xpath['ASIN'],0).value;
    url = AMAZON_URL + asin + "/ref=nosim?ie=UTF8&tag=" + getCookie("associateID", ASSOCIATE_ID);
    url = checkUrl(url);
    ...
    function appendProductInfo(poff,star,review,rank,detail) {
        ...
        if (rank)
        {
            ...
            if (jpflg) divRank.innerHTML += ":" + $xi(xpath['RANK'],0).textContent.match(/([0-9]+\u4f4d) ─/)[1] + " (" + getToday() + ")";　//\u4f4d：位
            else       divRank.innerHTML += ":" + $xi(xpath['RANK'],0).textContent.match(/(Nr\. *[0-9]+) *in/)[1] + " (" + getToday() + ")";　//\u4f4d：位
        }
        if (detail)
        {
    ...
    //radioPOff = new Array(2);
    //labelPOff = new Array(2);
    //radioSOff = new Array(2);
    //labelSOff = new Array(2);
    //radioROff = new Array(2);
    //labelROff = new Array(2);
    //radioDOff = new Array(2);
    //labelDOff = new Array(2);
    ...
    textAmaID.addEventListener(
        ...
        function() {
            ...
            url = AMAZON_URL + asin + "/ref=nosim?ie=UTF8&tag=" + this.value;
            url = checkUrl(url);
