// ==UserScript==
// @name           spash extract sukebei-nyaa
...
(function() {
	...
	/* ======= content */
	{
		...
		// <img  src="http://t2.imgchili.com/6423/6423170_dv_1379.jpg">
				/img src="[^"]+" alt="Image"/gi), new RegExp(
				/\/attachments[^"]+/gi));
		...
		function getThreadLinks() {
			...
			// /html/body/div[2]/div[3]/table[4]/tbody/tr[2]/td[2]/a
			// /html/body/div[2]/div[3]/table[4]/tbody/tr[3]/td[2]/a
			// /html/body/div[2]/div[3]/table[4]/tbody/tr[12]/td[2]/a
			var linkList = x(
					'//div[2]/div[3]/table[4]/tbody/tr/td[2]/a',
					'linkList');
					
			if(linkList == null){
				linkList = x(
					'//div[2]/div[3]/table[3]/tbody/tr/td[2]/a',
					'linkList');
			}
		...
		for ( var i in linkList) {
			if (linkList[i].href.match(/tid/i)) {
					...
					.addFunction(function() {
						...
						if (this.pb) {
							torrent_urls = this.pb
									.match(/page=download&amp;tid=[0-9]+[^txt"]+"/g);
						}
						...
						if (torrent_urls && torrent_urls.length > 0) {
							...
							for ( var j = 0; j < torrent_urls.length; j++) {
								...
								torrent_url = '?'+new String(torrent_url).replace('"','').replace('&amp;','&');
	...
	function x(_xpath, msg) {
		...
		if (nodesArray.length < 1) {
			log.warn(msg + ": node array is NULL." + nodesArray.length + " return null");
			return null;
		} else {
			return nodesArray;
		}
