// ==UserScript==
// @name          Raycore Wiki Spam Bot
// @namespace     OhMurr.com
// @include       http://raycore.wikia.com/*
// ==/UserScript==
...
function Bot()
{
	(document.onload = function Loop() {
		if(window.location.toString() == "http://raycore.wikia.com/wiki/Rayville_Wiki")
		{
			window.location = "http://raycore.wikia.com/index.php?title="+makeid()+"&action=edit";
		}
		if(window.location.toString().toLowerCase().indexOf("http://raycore.wikia.com/index.php?title=".toLowerCase()) != -1)
		{
				if(document.getElementById("csCategoryInput") != null && document.getElementById("csCategoryInput").getAttribute('disabled') == "")
				{
					setTimeout(function(){RonPaul();}, 600);
					setTimeout(function(){save();}, 2000);
				}
				else
				{
					//CKEDITOR.tools.callFunction(47, this);
					document.getElementById("cke_21_label").click();
				}
		}
		if(window.location.toString().toLowerCase().indexOf("http://raycore.wikia.com/wiki".toLowerCase()) != -1)
		{
			NewRandomPage()
		}
	}());
...
function RonPaul()
{
		unsafeWindow.window.focus();
		var rand2 = Math.floor((Math.random()*36));
		unsafeWindow.insertTags(WORDS[rand2],'','');
		unsafeWindow.window.focus();
		insertTags(WORDS[rand2],'','');
}
