    // ==UserScript==
    // @name                        Facebook Animated Chat Pictures - Chatbox by Lanjelin
    // @description                 Facebook Animated Chat Pictures - Chatbox by Lanjelin
	...
	if (header_tag) {
		...
		style_tag.innerHTML =
			'.char_ani_bar {padding-top:2px;padding-bottom:6px;line-height:16px;padding-left:2px;background:#EEEEEE none repeat scroll 0 0;border-style:solid;border-width:0px 0px 1px 0px;border-color:#C9D0DA;position:static; }'+
	...
	fp_pointer_up = 'cursor: pointer; position: absolute; right: 2px; -moz-transform: rotate(180deg); -webkit-transform: rotate(180deg);'
	...
	function fHideShowEmotBar(event){
		...
		if(fb_char_bar[0].getAttribute('style') == 'display: none;') {
			for(i=0;i<fb_char_bar.length;i++) {
				...
				document.getElementsByClassName('fbMercuryChatTab')[i].style.top = '1px';
			}
		} else {
			for(i=0;i<fb_char_bar.length;i++) {
				fb_char_bar[i].setAttribute('style','display: none;');
				fb_char_bar[i].parentNode.childNodes[1].setAttribute('style',fp_pointer_down);
				document.getElementsByClassName('fbMercuryChatTab')[i].style.top = '28px';
			}
		}
