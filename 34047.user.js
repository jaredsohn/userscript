...
*/
	...
	{
		var to = setTimeout( showFixturesWhenScoreClicked, 1500);
	...
	if (document.URL.match("http://fantasy.premierleague.com/transfers/"))
	{
		var to = setTimeout( addSortableStats, 1500); 
	}
	...
	if (document.URL.match("http://fantasy.premierleague.com/transfers/"))
	{
		var to = setTimeout( addMyTeam, 1000); 
	}
...
function prepareLeagueTableRetrieval ()
{
	...
	$('body').append('<div id="dialog" title="Please Wait"><p>The script is collecting and storing the needed info please wait up to 1 minute.  This dialogue box will close automatically when done.</p></div>');
	$('#dialog').dialog({autoOpen : false});
	$('.ismSection3').after("<br/><a href='http://Statto.com'>Fixture info from Statto.com</a>");
	$('.ismSection3').after("<button id='addStattoInfo' class='AddInfoButton'>Add Player Fixture Info</button>");
	$('.ismSection3').after("<br/><a href='http://www.fiso.co.uk/crackthecode.php'>CracktheCode from Fiso</a>");
	$('.ismSection3').after("<button id='addCTCInfo' class='AddInfoButton'>Add Player Crack The Code Info</button>");
...
function DefineExternalData()
{
	...
	MyTeamAbbreviations["Arsenal"] = ["Arsenal","ARS",1];
	MyTeamAbbreviations["Aston Villa"] = ["Aston Villa","AVL",2];
	MyTeamAbbreviations["Blackburn Rovers"] = ["Blackburn","BLA",4];
	MyTeamAbbreviations["Bolton Wanderers"] = ["Bolton","BOL",6];
	MyTeamAbbreviations["Chelsea"] = ["Chelsea","CHE",7];
	MyTeamAbbreviations["Everton"] = ["Everton","EVE",8];
	MyTeamAbbreviations["Fulham"] = ["Fulham","FUL",9];
	MyTeamAbbreviations["Liverpool"] = ["Liverpool","LIV",10];
	MyTeamAbbreviations["Manchester City"] = ["Man City","MCI",11];
	MyTeamAbbreviations["Manchester United"] = ["Man Utd","MUN",12];
	MyTeamAbbreviations["Newcastle United"] = ["Newcastle","NEW",13];
	MyTeamAbbreviations["Stoke City"] = ["Stoke City","STO",14];
	MyTeamAbbreviations["Sunderland"] = ["Sunderland","SUN",15];
	MyTeamAbbreviations["Tottenham Hotspur"] = ["Tottenham","TOT",16];
	MyTeamAbbreviations["West Bromwich Albion"] = ["West Brom","WBA",17];
	MyTeamAbbreviations["Wigan Athletic"] = ["Wigan","WIG",19];
	MyTeamAbbreviations["Wolverhampton Wndrs"] = ["Wolves","WOL",20];
	MyTeamAbbreviations["Norwich City"] = ["Norwich","NOR",21];
	MyTeamAbbreviations["Queens Park Rangers"] = ["QPR","QPR",22];
	MyTeamAbbreviations["Swansea City"] = ["Swansea","SWA",23];
	MyTeamAbbreviations["Birmingham City"] = ["Birmingham","BC",3];
	MyTeamAbbreviations["Blackpool"] = ["Blackpool","BL",5];
	MyTeamAbbreviations["West Ham United"] = ["West Ham","WH",18];
}
...
function getNextFixturesDifficulties()
{
	if (arguments.length == 0)
	{
		$.each( ismData.elInfo, function (index, value) {
			$.ajax({
				...
				dataType: 'json',
				success: getNextFixturesDifficulties
			});
		});
	}
	else if (arguments[0].first_name)
	{
		var Player = arguments[0];
		var FixtureDifficulty = 0;
		var TeamToPlay = Player.fixtures.summary[0][1].substr(0,3);
		var LeagueTableToUse = Player.fixtures.summary[0][1].substr(5,1) == "H" ? StattoStats.LastYear.True.Home : StattoStats.LastYear.True.Away;
		
		FixtureDifficulty = (getNextFixturesDifficulties(TeamToPlay, LeagueTableToUse, Player) * 100 | 0) / 100;
		/*
		...
		*/
		...
		localStorage.setItem(Player.id, FixtureDifficulty + "," + FixtureDifficulty * ismData.elInfo[Player.id][ismData.elStat.RazzaRating]);
		ismData.elInfo[Player.id][ismData.elStat.NextGamesDifficulty] = FixtureDifficulty;
		ismData.elInfo[Player.id][ismData.elStat.RazzaRatingFixtures] = FixtureDifficulty * ismData.elInfo[Player.id][ismData.elStat.RazzaRating];
	}
	else if (arguments.length == 3)
	{
		...
		var LastSeasonFull = localStorage.getItem(TeamToPlay + "," + StattoStats.LastYear.True.Full).split(",");
		
		// Higher is better attack
		function AttackMarksOutOf10()
		{
			// Work out opponent average values
			var AverageFailedToScore = LastSeasonFull[StattoStats.FailedToScore] / LastSeasonFull[StattoStats.Played];
			var AverageGoalsFor = LastSeasonFull[StattoStats.For] / LastSeasonFull[StattoStats.Played];
		...
		{
			// Work out opponent average values
			var AverageCleanSheets = LastSeasonFull[StattoStats.CleanSheet] / LastSeasonFull[StattoStats.Played];
			var AverageGoalsAgainst = LastSeasonFull[StattoStats.Against] / LastSeasonFull[StattoStats.Played];
			// Opponent Attack Marks out of 10 (ish)
...
function showFixturesWhenScoreClicked()
{
	...
	if (arguments[0]  && arguments[0].first_name)
	{
		var player = arguments[0];
		var contents = "";
	...
	else
	{
		...
		$('#ismElementDataTable tr').each(function(index, element) {
			if ($(this).children('td')[4])
			{
				...
				var playerScore = playerInfo[statId.total_points]
...
function InsertJSIntoPage()
{
	...
	script.src =  chrome.extension.getURL('script.js'); // "http://localhost/34049.user.js"; // 'http://userscripts.org/scripts/source/34047.user.js' - Use this to always use latest version
