...
// @description    updated with Q6 support by hesar - mod by S, added used Q6 weapon in battle window calculated with max current damage (only finished kills without NE) - edited by J, BH & CH top 5 fixed
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'<h4 style="margin-left:45px">' + dmgCalc(rank[mRank], str, 120, 1, 1) + '</h4>' +
			...
			' q4: ' + dmgCalc(rank[mRank], str, 80, 1, 1) + ' q5: ' + dmgCalc(rank[mRank], str, 100, 1, 1) +
                        '<br />q6: ' + dmgCalc(rank[mRank], str, 120, 1, 1) +
			'</small></div></div>' +
			...
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 120, 94, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 120, 94, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	var content = $j('div#pvp_battle_area');
        ...
        var militaryRank = rank[rankString];
         ...
         var max_hit = dmgCalc(militaryRank, strength, 120, 1, 1);
        $j('#total_damage').prepend('<tr><td><div>'+
        '<div><b >&nbsp;</b><small>Hit / Q6:</small>'+
        '<p id="pCurrentHit">'+ max_hit +' / ' + (current_damage/(max_hit*6)).toFixed(2) +'</p></div></td></tr>');
        $j('#total_damage').css('margin-top','-80px');
