...
var td = document.evaluate("//*[contains(@class,'vspib')]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
(function() {
  ...
  var scriptUrl = 'http://userscripts.org/scripts/show/105480';
  var installUrl = 'http://userscripts.org/scripts/source/105480.user.js';
  /***********************************************************************************
   * 语言支持
   ***********************************************************************************/
  var Lng;
...
/* #gkk_oe a {margin-right: 3px;} */';
  ...
  function doit() {
      var weblan = (/hl=([\w-]+)&/.exec(location.href)||["",navigator.language])[1];
      var imgSearch;
      switch(weblan){
         case 'zh-CN':
              imgSearch = '图片';
     ...
     // alert(getSearchName()+"|"+Lng.imgSearch)
    if(getSearchName() == imgSearch) return;
    ...
    // 添加翻页分隔符的样式
    (Pref.AutoNextPage ? ' #nav td {padding: 0 0.2em 0 0.2em !important;} #nav {margin-bottom: 0 !important;} #nav a {display: inline !important;}' : '') + Pref.CSS + Pref.customizeCSS);
    // 判断是否关闭左侧的百宝箱
    ...
    case true:
      ...
      setTimeout(function() {
        ...
        if(Pref.AutoNextPage) {
          ...
          // 删除第一页的页面导航条
          var navTable;
          if(navTable = document.getElementById('nav')) navTable.style.display = "none";
          ...
          window.addEventListener('keydown', function(event) {
            if((event.altKey == alt_Next) && (event.ctrlKey == ctrl_Next) && (event.shiftKey == shift_Next) && (event.keyCode == keycode_Next) && (event.target.localName != 'INPUT') && !document.getElementById('preferences')) {
              ...
              var i, len = nextSplit.snapshotLength,
                topOffset = document.getElementById('center_col').parentNode.offsetTop,
                headerOffset = (Pref.floatInput && Pref.floatInputPos == 1) ? document.getElementById('gkk_header').offsetHeight : 0;
              for(i = 0; i < len; i++) {
  ...
  function addPreviews(doc) {
    ...
    for(i = 0; i < len; i++) {
      ...
      with(img) {
        ...
        setAttribute('align', 'left');
        setAttribute('src', 'http://open.thumbshots.org/image.pxf?url=' + link.href);
  ...
  function addPreferences() {
    try {
      ...
      var genuserOp = matchNode('//div[@id="ab_options"]/ul').snapshotItem(0);
  ...
  function addSearchWithOtherEngines() {
    var div = document.getElementById('resultStats');
    if(div) {
      var gbEngines = [],
        gbLinks = [];
  ...
  function addseatchSite(doc) {
    ...
    for(i = 0; i < len; i++) {
      ...
      while((span != null && span.nodeType != 1) || (span != null && span.className != 'gl')) {
  ...
  function moveSearchTip2Top(marginLeft, doc) {
    ...
    var len = relDivs.snapshotLength,
      relDiv;
    if(len > 0) {
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(resp) {
        ...
        if(relDiv) { // 百度的相关搜索如果存在
          ...
          if(document.getElementById('preferences')) relDiv.style.display = 'none';
...
';
  ...
  function setPreferences() {
    ...
    for(i = 0; i < len; i++) {
      if(allElements[i] != null && allElements[i].nodeType == 1 && getComputedStyle(allElements[i], '').display != 'none') {
      ...
      innerHTML = '<div style="display: table-cell; text-align: center; vertical-align: middle; padding: 1em;">\
                <div id="gkk_pref_gui">\
                        ...
                        <table width="100%">\
                            <tr id="gkk_pref_gui_title">\
                              <td colspan="2">\
                                ...
                                <div style="float: right; padding-right: 0.8em; font-size: small;">' + Lng.gui_welcome1 + '<a href="http://board.mozest.com/viewthread.php?tid=26585" target="_blank">' + Lng.gui_welcome2 + '<a href="javascript:void(0);" id="check_update_a" >检查更新</a></div>\
                              </td>\
                            </tr>\
                            <tr>\
                                <td colspan="2">\
                  ...
                  <div id="gkk_pref_buttons" style="float: right;padding: 1em 1em 0 0;">\
                    ...
                    <div>\
' + addCssTable('holder', 'border: 1px solid rgb(170, 170, 170);', Pref.CSS.match(/\.g\s\{[^\}]*\}/i)) + addCssTable('counter', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\.gkk_num\s\{[^\}]*\}/i)) + addCssTable('title', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\.l\s\{[^\}]*\}/i)) + addCssTable('desc', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\.s\s\{[^\}]*\}/i)) + addCssTable('keyword', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\em\s\{[^\}]*\}/i)) + addCssTable('url', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\cite\s\{[^\}]*\}/i)) + addCssTable('reflinks', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\.gl\s\{[^\}]*\}/i)) + addCssTable('related', 'border: 1px solid rgb(170, 170, 170); display: none;', Pref.CSS.match(/\#gkk_rel\s\{[^\}]*\}/i)) + '                 </div>\
                    <div>\
...
';
  ...
  function loadNextPage() {
    ...
    if(!IsLoadingNext && NextPageLink && document.body.scrollHeight - window.scrollY < window.innerHeight * 3 && !document.getElementById('preferences')) {
      ...
      GM_xmlhttpRequest({
        ...
        //http://www.google.com.hk/search?q=123&hl=zh-CN&newwindow=1&safe=strict&biw=1280&bih=709&noj=1&prmd=ivns&ei=hkIFTsCiGeGJmQWg8omuDQ&start=10&sa=N
        overrideMimeType: 'text/html; charset=' + document.characterSet,
        onerror: function(d) {
          //alert(1);
          IsLoadingNext = false;
          document.getElementById('isLoading').parentNode.removeChild(document.getElementById('isLoading'));
          /*  var ld = document.getElementById('isLoading');
        ld.innerHTML="";
        var reload = document.createElement('a');
        reload.setAttribute('href', 'javascript: void(0);');
        reload.addEventListener('click', function(){
          document.getElementById('isLoading').parentNode.removeChild(document.getElementById('isLoading'));
          loadNextPage();
        });
        reload.innerHTML="加载失败,单击重新加载下一页";
        ld.appendChild(reload);*/
          loadNextPage();
          //if (navTable = document.getElementById('nav')) 
          //  navTable.style.display = "";
        },
        onload: function(d) {
          // 如果已打开设置界面，那么不加载下一页
  ...
  function checkUpdate(bforce) {
    // alert(Pref.skipVersion);
    if(document.getElementById('gkk_UpdateMsg')) return;
    var now = Math.round(new Date().getTime() / 1000);
    if(Number(Pref.newVersion) > Number(scriptVersion)) { // 如果已经检查到新版本
      showUpdateMessage(Pref.newVersion);
    } else if(now - Pref.lastCheck > 86400 || bforce) { // 如果上一次没有检查到新版本，并且上一次检查距今超过24小时，那么现在检查新版本
  ...
  function showUpdateMessage(newVersion) {
    ...
    document.getElementById('resultStats').parentNode.insertBefore(msg, document.getElementById('resultStats'));
    var css;
  ...
  function hideUpdateMessage() {
    document.getElementById('gkk_UpdateMsg').parentNode.removeChild(document.getElementById('gkk_UpdateMsg'));
  }
  // 暂时忽略
  ...
  function getNewVersion(funcbind) {
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'http://userscripts.org/scripts/source/105480.meta.js',
      overrideMimeType: 'text/html; charset=' + document.characterSet,
      onload: function(d) {
  ...
  function getSearchName() {
    var sSearchName = "";
    var ms = xpath("//div[@id='ms']/ul/li/div[@class='kls']");
    if(ms.snapshotLength >0) {
      sSearchName = ms.snapshotItem(0).innerText;
    }else{ 
    ms = xpath("//div[@id='hdtb_msb']/div[@class='hdtb_mitem hdtb_msel']");
        if(ms.snapshotLength >0) {
          sSearchName = ms.snapshotItem(0).innerText;
        }
    }
    var img =  xpath("//div[@id='topstuff']/div/a/img");
    if(img.snapshotLength>0)
        sSearchName = sSearchName+img.snapshotLength;
    return sSearchName;
  }
  /*********************************************************************************************
   * 各功能函数 结束
