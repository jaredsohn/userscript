// ==UserScript==
// @name           Ikariam Araştırma Göstergesi
// @version        0.6
// @author         HomerOfXi & PhasmaExMachina
// @description    Araştırma sürelerini görsel ve % olarak görmek isteyenlere (www.ikariam.forumm.biz)
	...
	views:{
      ...
      var showTime = IkaTools.getVal('showTime');   
		  //create the settings panel
			...
			settingsPanel.innerHTML = '<h3 class="header"><span class="textLabel">Ikariam Araştırma Süresi</span></h3><div class="content"><form action="index.php" method="GET"><input type=hidden name=view value="options"><table cellpadding="0" cellspacing="0">'
			    + '<tr><th>Akademi Araştırma Göstergesi</th><td>' 
			    + '<input id="showAcademyResearch" type="checkbox" name="sar" ' + (showAcademyResearch ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Araştırmaları Yüzdeli Göster</th><td>' 
			    + '<input id="showPercent" type="checkbox" name="sp" ' + (showPercent ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Araştırma Kalan Süre</th><td>' 
			    + '<input id="showTime" type="checkbox" name="st" ' + (showTime ? 'checked' : '')
					+ '></td></tr>'
					+ '</table><div class="centerButton"><input id="rt_save" class="button" type="submit" value="Ayarları Kaydet" "/></div></form></div><div class="footer"></div></div>';
