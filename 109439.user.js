...
var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4693677';
var postId = '42839667';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4675876&page=1';
// var postId = '42839692';
var forumAccessURL = 'http://goallineblitz.com/game/forum_private_access.pl?forum_id=6438';
...
var listVersion = 0;
var listNames = ['family', 'apprentice', 'formeraddict', 'waitlist', 'hitlist'];
var list = {
	family: {
		member: {},
		title: 'GLB Addicts',
	...
	formeraddict: {
		member: {},
		title: 'FORMER ADDICTS',
		style: 'color: red;',
		agentTitle: function(id, name) { return 'Former Addict'; },
		forumAccess: 1,
	},
	waitlist: {
		member: {},
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			$('#content div.content_container tbody:eq(0) tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				if (list.hitlist.member[id]) {
					...
					delete names[id];
					names.num--;
				} else if (list.formeraddict.member[id] || list.apprentice.member[id] || list.family.member[id]) {
					if (list.family.member[id]) {
						++membersWithTitles;
					} else if (list.apprentice.member[id]) {
						++membersApprentice;
					} else if (list.formeraddict.member[id]) {
						++membersformeraddict;
					}
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			if (membersformeraddict) {
				statusMsg += prefix + '<b style="color: red;">' + membersformeraddict + ' without</b>';
				prefix = ', ';
			}
			if (membersWaitList) {
