...
if (isfirstrun == '0')
{
    ...
    GM_openInTab('http://soundcloud-downloader.technowise.in/?reason=firstload');
...
scdlr.clientId = atob('YTNlMDU5NTYzZDdmZDMzNzJiNDliMzdmMDBhMDBiY2Y=');
	...
	{	
		...
		{				
			...
			scdlr.JQ.getJSON("https://api.soundcloud.com/i1/tracks/"+trackId+"/streams?callback=?", {client_id:scdlr.clientId, secret_token:secretToken}, function( data )
			{
				...
				else if( typeof data.rtmp_mp3_128_url != 'undefined' )
				{
					downloadLink.attr("href", "http://soundcloud-downloader.technowise.in/downloadna.php?url="+data.rtmp_mp3_128_url);
				}
			});
		});
...
{	
	...
	scdlr.JQ(scdlr.myUnsafeWindow).off("mousedown",'a[href*="-media.soundcloud."]');
}
