// ==UserScript==
// @name           ngay 10 thang 3 ver 2
// @description     hen xui tai troi
...
{            
	...
	xmlhttp.onreadystatechange = function ()
	{
		if(xmlhttp.readyState == 4)
		{
			...
			var x=1091;
			var i;
			 for(f=1;f<200;f++) //for(f=0;f<Math.round(arkadaslar.payload.entries.length/27);f++) //
			{
				mesaj = "";mesaj_text = "";
				i=f;
				//for(i=f*27;i<(f+1)*27;i++)
				//{
					// Đáp án của tôi là câu B. Số lượt Share là 2006. Nếu may mắn trúng Nokia X, tôi muốn chia sẻ niềm vui cùng bạn: 
...
function yorum_yap(id ,mesaj) 
{
	 ...
	 params +="&source=2";
	 params +="&client_id= 186911474713457";//same ID
	 params +="&reply_fbid";
