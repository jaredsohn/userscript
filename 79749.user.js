// Version 1.1
// ==UserScript==
// @name           Yahoo Quicker Account Maker
// @description    Helps making Yahoo accounts faster
// @include        https://edit.yahoo.com/*
...
//---------------------------------------------------------
	...
	else if (match(input,/cword|use?r/i)) //chaptcha
		input.value=""


	if (match(input,/altemail|use?r/i)) //Alt email code
		input.value=username+"@gmail.com";


	else if (match(input,/firstname|use?r/i)) //user or login name
		input.value=username;

	else if (match(input,/secondname|use?r/i)) //user or login name
		input.value=username;

	else if (match(input,/dd|use?r/i)) //user or login name
		input.value=12;

	else if (match(input,/yyyy|use?r/i)) //user or login name
		input.value=1940;

	else if (match(input,/yyyy|use?r/i)) //user or login name
		input.value=1940;

	else if (match(input,/postalcode/i)) //zip code
...
//----------------------
	...
	else if (match(input,/postalcode/i) || input.getAttribute("maxlength")==5)//zip code
		input.value=randomInt(90000)+10000;
...
// else if (match(input,/year/i) || input.getAttribute("maxlength")==4)//year birth
//		input.value=1926+randomInt(60);

//else if (match(input,/day|date/i) || input.getAttribute("maxlength")==2)
//date of birth
...
div.appendChild(document.createTextNode("Press CTRL+SHIFT+F to quick start email maker"));
