// ==UserScript==
// @name        Uiqueblhats Probux Fucker - Clicks Ads + Playprogrid
// @namespace   Uiqueblhats Probux Fucker - Clicks Ads + Playprogrid
// @include     *://*/*ads.php*
// @include     *://*/?x=*
...
// @include     *://*/*earn.php*
// @include     *://*/v/index.php*
// @require     http://userscripts.org/scripts/source/170910.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link*.com/*
// @include     *://*ysear.ch/*
...
wparent.location.href.indexOf('ads.php') != -1
	...
	div.css({zIndex:100000000,textAlign:'center',padding:5,position:'fixed',width:200,height:20,background:'#90d2f0',border:'2px solid blue',bottom:1,right:1})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px arial',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:200,height:30,background:'#90d2f0',border:'2px solid blue',bottom:40,right:1})
	jQuery.ajax({
	...
	div2.append(
		$('<h3 class="titleJanbee">').text('Uiqueblhats Probux Fucker')		
	)	
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			...
			if(adsWind == null){
				 div.css({background:'#dbf400',border:'2px solid red'}).text('Allow Popup Blocker !!!')
			}else{
...
wparent.location.href.indexOf('gridview.php?') != -1) && self == top){
	var div = $('<div>');
	var ajaxCall = false;
	div.css({font:'bold 8px arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:60,height:10,background:'#90d2f0',border:'2px solid blue',top:1,left:1})
		.text('');
...
if(wparent.location.href.indexOf('progrid.php') != -1){
	...
	var clickNum = ctr + 1 ;
	var loading = 0;
	div.css({zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:70,height:20,background:'#90d2f0',border:'2px solid blue',bottom:1,right:1})
		.text('');
	$('html').css({position:'relative'}).append(div);
