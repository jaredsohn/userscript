...
/*
 ...
 v2.12 ????	   Added Aleas patch for update check exception handler
                   ...
                   Fixed detection of image url for pool photos aftter recent flickr change
...
const PoolAwardXPath = "//p[@class='flickraward']";
...
{
var Id = '';
var allPhotoDivs = document.evaluate(
    ".//div[@class='photoImgDiv']/img",
    document,
    null,
    XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    null);
if (allPhotoDivs.snapshotLength === 0) { // new layout
    allPhotoDivs = document.evaluate(
    ".//div[@class='photo-div']/img",
    document,
    null,
    XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    null);
    }
for (var i = 0; i < allPhotoDivs.snapshotLength; i++) {
    thisElement = allPhotoDivs.snapshotItem(i);
    Id = GetImageId(thisElement.src);
    break;
    }
return Id;
}
...
if (AwardDivOuter)
  ...
  if (Visible)
    ...
    //AwardDivOuter.parentNode.style.width = '150px';
    AwardDivOuter.parentNode.style.height = (AwardDivOuter.parentNode.clientHeight + AwardDivOuter.clientHeight + 10) + 'px';
    }
    else
    {
    AwardDivToHide.style.visibility = 'hidden';
    AwardDivToHide.style.display = 'none';
    AwardDivOuter.parentNode.style.height = '145px';
    //AwardDivOuter.parentNode.style.width = '110px';
    }
  }
}
...
for (var i = 0; i < allImgs.snapshotLength; i++) {
    ...
    if (thisElement.src.match('^http:\/\/(.*)static\.flickr\.com\/(.*)$'))
      {
      var buf = GetImageId(thisElement.src);
      return buf;
      }
    if (thisElement.src.match('^http:\/\/(.*)\.staticflickr\.com\/(.*)$'))
      {
      var buf = GetImageId(thisElement.src);
      return buf;
      }
...
function ReplacePoolListWithTable()
{
var holdPhotos = FindFirstElement(document,"//div[@class='HoldPhotos']");
...
var allElements = document.evaluate(
    ...
    null);
var numCols = 5;
var numRows = Math.ceil(allElements.snapshotLength / numCols);
AddGlobalStyle('.PoolList { font-size: 11px; padding: 0px; margin-right: 10px; margin-bottom: 0px; float: left; text-align: center; width: 100%; height: 100%; ! important; }');
//AddGlobalStyle('.AwardCounterPoolList { font-size: 11px; padding: 0px; vertical-align:top; text-align: center; ! important; }');
AddGlobalStyle('.AwardCounterPoolTableCell { vertical-align:top; ! important; }');
var thisElement;
var div = document.createElement("div");
div.setAttribute('class','HoldPhotos');
...
for (var j = 0; j < numRows; j++)
  ...
  for (var i = 0; i < numCols; i++)
    ...
    cell.setAttribute('class','AwardCounterPoolTableCell');
    if (item < allElements.snapshotLength)
      {
        cell.innerHTML = '<div><p class="PoolList">' + allElements.snapshotItem(item).innerHTML + '</p></div>';
      }
    item++;
...
div.appendChild(tbl);
holdPhotos.parentNode.insertBefore(div,holdPhotos);
holdPhotos.parentNode.removeChild(holdPhotos);
}
...
if (discussPane)
  {
  //AddCommentCountLinks("//div[@id='photo']",true);
...
var thisElement = FindFirstElement(document, "//div[@class='HoldPhotos']");
CreateCountAllDiv(thisElement);
ReplacePoolListWithTable();
AddCommentCountLinks("//p[@class='PoolList']",true);
//ToggleAll(false);
}
