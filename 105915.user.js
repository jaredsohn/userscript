// ==UserScript==
// @name		Просмотр уровня зданий в Икариам (MObile)
// @version		1.4.2
// @namespace		http://ikariam.org/
// @description		Показывает уровни зданий на главной странице и кол-во ресурсов, которые нужно потратить.
// @author		Anh Tuan Nguyen Dao (http://www.atdao.dk)()
...
// @exclude		http://support.*.ikariam.*/*
// @require             http://userscripts.org/scripts/source/105916.user.js
// @history 1.4.2 (8 Сентября 2010) - Исправлена ошибка недочета одной единицы ресурса
// @history 1.4.1 (4 Сентября 2010) - Исправлена ошибка с хижиной лесничего 21 уровня
// @history 1.4.0 (1 Сентября 2010) - Адаптация под версию игры 0.4.0, изменение алгоритмов вычисления, удаление поддержки версии 0.3.1 за ненадобностью
// @history 1.3.3 (28 Марта 2010) - Дополнены база данных по цене зданий
// @history 1.3.2 (26 Октября 2009) - Fixed icon and tooltip positioning for temple (thanks to Lordshinjo "http://userscripts.org/users/114555" for pointing out the problem).
// @history 1.3.1 (16 Октября 2009) - Fixed the building cost calculation has been revised in Ikariam version 0.32 which I did not know, so the calculation are all wrong. This is now fixed (thanks to RandomMan "http://userscripts.org/users/91016" for pointing out the problem). Fixed a bug with dot-notation problem for servers which uses such notation (thanks to malus "http://userscripts.org/users/109688" for posting the bug). For servers which are still at version 0.31, the "old" building cost calculation is still supported.
// @history 1.3.0 (15 Октября 2009) - Building cost database updated. Images are now linked in place. Ready for Ikariam version 0.32. Fixed a bug where the info box is not positioning correctly on palace or residence construction spot. Fixed a bug for servers which use dot-notation as thousand seperator in stead of comma (thanks to Carpediem "http://userscripts.org/users/100184" for pointing out the bug).
// @history 1.2.2 (27 Июня 2009) - Now supporting CreaseKit and Fluid (thanks to Kahil Young "http://userscripts.org/users/69426" for the suggestion and testing the mod in Fluid).
// @history 1.2.1 (8 Июня 2009) - Fixed a bug which caused no resource reduction when a reduction building is being upgrade. Improved positioning of the icons. Icon and info box at construction spot are now correctly positioned.
// @history 1.2.0 (2 Июня 2009) - New: Added icon and info box to construction spot (thanks to giangkid "http://userscripts.org/users/86129" for the suggestion).
// @history 1.1.1 (26 Мая 2009) - Fixed a bug when NOT using Black Canvas add-on to Firefox, the level indication is not centralized correctly.
// @history 1.1.0 (25 Мая 2009) - New: Added a info box which shows up when mouse over the icon, the box can be turned off in the options page. Fixed a bug where in a rare case, an icon is not shown on the wall. Fixed a bug which caused no sulfur reduction bonus.
// @history 1.0.3 (17 Мая 2009) - Fixed first time running check which caused unnecessarily research checks.
// @history 1.0.2 (17 Мая 2009) - Fixed the parsing of the building level correctly for different languages (thanks to oliezekat "http://userscripts.org/users/78627"). Improved bureaucracy-spot check.
// @history 1.0.1 (17 Мая 2009) - Fixed bureaucracy-spot, not showing if you are not done with that research. Removed alert boxes which shows up at the start.
// @history 1.0.0 (16 Мая 2009) - Релиз
// ==/UserScript==
...
ScriptUpdater.check(51635, '1.4.2')
...
imgScrollLeft_3		= "http://ika-info.ucoz.ru/scripts/img/iuw/info_l.png";
imgScrollRight_3	= "http://ika-info.ucoz.ru/scripts/img/iuw/info_r.png";
imgScrollMiddle_3 = "http://ika-info.ucoz.ru/scripts/img/iuw/info-m.png";
...
if (view == "options") {
    ...
    child = '<h3 class="header"><span class="textLabel">Скрипт Просмотр уровня зданий в Икариам</span></h3><div class="content"><form action="index.php" method="GET"><input type=hidden name=view value="options"><table cellpadding="0" cellspacing="0"><tr><th>Показывать всплывающее окно с ресурсами</th><td><input type="radio" name="iuwInfoBox" value="true" '+checkedYes+'> Да<br><input type="radio" name="iuwInfoBox" value="false" '+checkedNo+'> Нет</td></tr></table><div class="centerButton"><input class="button" type="submit" value="Сохранить" /></div></form></div><div class="footer"></div></div>';
...
if (view == "city") {
	...
	if (buildingList) {
		...
		redWood = redWood / 100;
		redWine = redWine / 100;
		redMarble = redMarble / 100;
		redCrystal = redCrystal / 100;
		redSulfur = redSulfur / 100;
				...
				switch (buildingType) {
					case 'palace':
						...
						if (isDefined(BUILDINGS[buildingType][upLevel]) && BUILDINGS[buildingType][upLevel]["wood"] != 0) {
								...
								reqWood = Math.ceil(BUILDINGS[buildingType][upLevel]["wood"] * (1 - (redAll + redWood)));
								reqMarble = Math.ceil(BUILDINGS[buildingType][upLevel]["marble"] * (1 - (redAll + redMarble)));
								reqWine = Math.ceil(BUILDINGS[buildingType][upLevel]["wine"] * (1 - (redAll + redWine)));
								reqCrystal = Math.ceil(BUILDINGS[buildingType][upLevel]["crystal"] * (1 - (redAll + redCrystal)));
								reqSulfur = Math.ceil(BUILDINGS[buildingType][upLevel]["sulfur"] * (1 - (redAll + redSulfur)));
					...
					case 'palaceColony':
						...
						if (isDefined(BUILDINGS[buildingType][upLevel]) && BUILDINGS[buildingType][upLevel]["wood"] != 0) {
								...
								reqWood = Math.round(BUILDINGS[buildingType][upLevel]["wood"] * (1 - (redAll + redWood)));
								reqMarble = Math.round(BUILDINGS[buildingType][upLevel]["marble"] * (1 - (redAll + redMarble)));
								reqWine = Math.round(BUILDINGS[buildingType][upLevel]["wine"] * (1 - (redAll + redWine)));
								reqCrystal = Math.round(BUILDINGS[buildingType][upLevel]["crystal"] * (1 - (redAll + redCrystal)));
								reqSulfur = Math.round(BUILDINGS[buildingType][upLevel]["sulfur"] * (1 - (redAll + redSulfur)));
...
/**
 * Creates a info box and attach it to an event to an icon (Dump)
 * @param string id
 * @param object target
 * @param string building type
 * @param integer wood
...
//			curWine = document.getElementById('value_wine').textContent.replace(",","");
	//		curWine = curWine.replace(".","");
		...
		// Nasty, ugly HTML for the info box
		div.innerHTML = '<div class="before" style="background-image: url(\'' + imgScrollLeft_3 + '\'); height: 80px; float: left;"></div><div style="float: left;"><div style="color: ' + colorWood + '; text-align: left;"><img style="vertical-align: middle; margin-right: 2px;" src="' + imgWood + '" /> '+ estWood +'</div><div style="margin-top: -5px; color: ' + colorMarble + '; text-align: left;"><img style="vertical-align: middle; margin-right: 2px;" src="' + imgMarble + '" /> '+ estMarble +'</div><div style="margin-top: -5px; color: ' + colorCrystal + '; text-align: left;"><img style="vertical-align: middle; margin-right: 2px;" src="' + imgCrystal + '" /> '+ estCrystal +'</div><div style="margin-top: -5px; color: ' + colorSulfur + '; text-align: left;"><img style="vertical-align: middle; margin-right: 2px;" src="' + imgSulfur + '" /> '+ estSulfur +'</div></div><div class="after" style="background-image: url(\'' + imgScrollRight_3 + '\'); height: 80px; float: left;"></div>';
...
function loadDB() {
	...
	BUILDINGS = {
		...
		"academy" : [
			...
			{ "wood" : 536309, "crystal" : 1006646 },
			{ "wood" : 743789, "crystal" : 1449741 },
			{ "wood" : 1449741, "crystal" : 2079650 },
			{ "wood" : 1257244, "crystal" : 2642546 },
			{ "wood" : 1736681, "crystal" : 3790581 }       // level 32 (lvl 27-32 by ika-info)
		],
		...
		"temple" : [
			...
			{ "wood" : 68528, "crystal" : 205093 } // level 32 (lvl 29-32 by ika-info) 
		...
		"townHall" : [
			...
			{ "wood" : 1876185, "marble" : 4756439 }    // level 38 (lvl 34-38 by ika-info)
		],
		...
		"wall" : [
			...
			{ "wood" : 1510294, "marble" : 1598031 },
			{ "wood" : 1812577, "marble" : 1917913 }  // level 41 (lvl 35-41 by ika-info)
		],
		...
		"port" : [
			...
			{ "wood" : 1353517, "marble" : 1913072 } // level 40 (lvl 33, 35-40 by ika-info)
		],
		...
		"glassblowing" : [
			...
			{ "wood" : 590392, "marble" : 327069 },
			{ "wood" : 767620, "marble" : 425294 },
			{ "wood" : 998018, "marble" : 552985 },
			{ "wood" : 1297535, "marble" : 718987 },
			{ "wood" : 1686905, "marble" : 934788 },
			{ "wood" : 2193088, "marble" : 1215329 } // level 32 (lvl 27-32 by ika-info)
		],
		...
		"museum" : [
			...
			{ "wood" : 1723016, "marble" : 2158157 } // level 18 (lvl 17,18 by ika-info)
		],
		...
		"forester" : [
			...
			{ "wood" : 550048, "marble" : 312964 } // level 27
		],
		...
		"barracks" : [
			...
			{ "wood" : 1196557, "marble" : 828807 },
			{ "wood" : 1483783, "marble" : 1027931 } // level 41 (lvl 37-41 by ika-info)
		],
		...
		"stonemason" : [
			...
			{ "wood" : 1297535, "marble" : 718987 },
			{ "wood" : 1686905, "marble" : 934788 },
			{ "wood" : 2193088, "marble" : 1215329 }  // level 32 (lvl 26-32 by ika-info)
		],
		...
		"tavern" : [
			...
			{ "wood" : 1066196, "marble" : 2000427 }   // level 42 
		],
		...
		"branchOffice" : [
			...
			{ "wood" : 233530, "marble" : 121067 } // level 26
				...
				// dump, wood, marble, crystal, sulfur (support for 0.4.0 added by http://ika-info.ucoz.ru/ )
		"dump" : [
 	 	  ...
 	 	  { "wood" : 1766, "marble" : 1445, "crystal" : 1668, "sulfur" : 1398 },
 			...
 			{ "wood" : 3388, "marble" : 2762, "crystal" : 2991, "sulfur" : 2858 }, //level 5	
 			...
 			{ "wood" : 20905, "marble" : 15691, "crystal" : 15325, "sulfur" : 18623 },	
 			...
 			{ "wood" : 96224, "marble" : 64279, "crystal" : 58635, "sulfur" : 86410 },	
 			{ "wood" : 115853, "marble" : 76195, "crystal" : 68930, "sulfur" : 104076 },	
 			{ "wood" : 139408, "marble" : 90256, "crystal" : 80974, "sulfur" : 125275 },
 			{ "wood" : 167673, "marble" : 106847, "crystal" : 95066, "sulfur" : 150714 },
 			{ "wood" : 201592, "marble" : 126425, "crystal" : 111554, "sulfur" : 181241 },	
 			{ "wood" : 242294, "marble" : 149528, "crystal" : 130844, "sulfur" : 217873 },	
 			{ "wood" : 291137, "marble" : 176788, "crystal" : 153414, "sulfur" : 261831 },
 			{ "wood" : 349749, "marble" : 208956, "crystal" : 179821, "sulfur" : 314582 },	
 			{ "wood" : 420082, "marble" : 246913, "crystal" : 201717, "sulfur" : 377882 },
 			{ "wood" : 504483, "marble" : 291703, "crystal" : 246865, "sulfur" : 453843 }, //level 30
 			{ "wood" : 605763, "marble" : 344555, "crystal" : 289158, "sulfur" : 544995 },
 			{ "wood" : 727300, "marble" : 406921, "crystal" : 338642, "sulfur" : 654378 },	
 			{ "wood" : 873144, "marble" : 480512, "crystal" : 396537, "sulfur" : 785638 },	
 			{ "wood" : 1048157, "marble" : 567350, "crystal" : 464275, "sulfur" : 943149 },
 			{ "wood" : 1258172, "marble" : 669818, "crystal" : 543528, "sulfur" : 1132163 },
 			{ "wood" : 1510191, "marble" : 790731, "crystal" : 636254, "sulfur" : 1358980 },	
 			{ "wood" : 1812613, "marble" : 933409, "crystal" : 744743, "sulfur" : 1631160 },	
 			{ "wood" : 2175519, "marble" : 1101768, "crystal" : 871676, "sulfur" : 1957775 },	
 			{ "wood" : 2611007, "marble" : 1300432, "crystal" : 1020188, "sulfur" : 2349715 },	
 			{ "wood" : 3133593, "marble" : 1534855, "crystal" : 1193946, "sulfur" : 2820041 }    //level 40
		]
	};
}