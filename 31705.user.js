// version 1.5 13 Août 2008
// ==UserScript==
// @name Vendetta
// @author Mosaik, Alex, BoB_leklair, kemike
// @descrizione: 
...
(function(){
	...
	// suppression de la pub, d calage 1pixel ressources ....
	...
	if (location.pathname.search('prod.php') != -1 ) {		
		...
		for (var i = 0; i < page.length; i++) {
			...
			switch (pid[i].value){
				...
				break;
				case '11'://D bloqueur de situation
					var couts_armes=5000;
					var couts_munitions=1000;
					var couts_dollars=4000;
				break;
				case '12'://Tireur d  lite
					var couts_armes=4000;
	...
	// boutons cocher d cocher pour les messages
	...
	// mettre le rapport dans le logiciel http://www.justalex.org/hall/
	if (location.pathname.search('bericht.php') != -1 ) {		
		var page = document.getElementsByTagName("table").item(2);
		newDiv = document.createElement("span");
		newDiv.innerHTML =  '<form action="http://www.justalex.org/hall/" method="post" name="rapport" target="_blank" onsubmit="return valider()"><input name="html" id="html" value="" type="hidden"><input name="source_dest" id="source_dest" value="ok" type="hidden"><input name="template" id="template" value="Vendetta" type="hidden"><input name="date" id="date" value="ok" type="hidden"><input name="ressources" id="ressources" value="ok" type="hidden"><input name="rounds" id="rounds" value="ok" disabled="disabled" type="hidden"><input name="puissance" id="puissance" value="ok" type="hidden"><input name="stats" id="stats" value="ok" type="hidden"><input name="espion_ressources" id="espion_ressources" value="ok" type="hidden"><input name="espion_bat" id="espion_bat" value="ok" type="hidden"><input name="envoye" id="envoye" value="IMG" src="http://www.justalex.com/vendetta_user_js/image.gif" title="Faire une image de ce rapport" onclick="document.rapport.html.value=document.getElementsByTagName(\'body\').item(0).innerHTML;" alt="Faire une image de ce rapport" type="image"></form>';
		var parentDiv = page.parentNode;
		parentDiv.insertBefore(newDiv, page);
	}
	...
	if (location.pathname.search('nav.php') != -1 ) {		
		...
		for (var i = 0; i < page.length; i++) {
			...
			if (page[i].selected) {
				...
				if (last!='') {
					newDiv = document.createElement("span");
					newDiv.innerHTML =  '<a href="javascript:void(0);" onclick="parent.w.location= chto+\'?q='+id+'x'+last+'\';location=\'nav.php?q='+id+'x'+last+'&w=\'+chto"><img src="http://img390.imageshack.us/img390/6600/prcdent1lx4.png" border="0"></a>&nbsp;';
					var parentDiv = page.parentNode;
					parentDiv.insertBefore(newDiv, page);
					var last='';
				}
				...
				if (i < page.length) {
					newDiv = document.createElement("span");
					newDiv.innerHTML =  '&nbsp;<a href="javascript:void(0);" onclick="parent.w.location= chto+\'?q='+id+'x'+page[i+1].value+'\';location=\'nav.php?q='+id+'x'+page[i+1].value+'&w=\'+chto"><img src="http://img390.imageshack.us/img390/3461/suivant1jw5.png" border="0"></a>';
					var parentDiv = page.parentNode;
