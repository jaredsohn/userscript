...
(function () {
  try {
    ...
    alert ('请勿当成脚本插入到网页之中，该脚本需要 GM 系列函数权限进行跨域操作!');
    return;
  }
  ...
  var addressToCheck = [
    { // 百度盘
      urlSyntax: /^pan\.baidu\.com\/(share\/(link|init)|netdisk\/singlepublic)\?/i,
    ...
    { // Kuaipan
      ...
      checkFunc: function (r, d, f, e) {
        var typeCode = (d.querySelector('span.mtf3.dib[id]') ? __FILE_WORKING : 
                        __FILE_NOT_EXIST);
  ...
  ['DOMNodeInserted', 'DOMSubtreeModified', 'DOMAttrModified'].forEach (function (e){
    addEventListener (e, main, false)
  });
})();