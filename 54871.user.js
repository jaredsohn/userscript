...
// @name           Heringer
// @namespace      Heringer
// @autor          Henrique Heringer
// @description    Basicamente exibe algumas informaÃƒÂ§ÃƒÂµes de forma amigÃƒÂ¡vel e tem o recurso AutoPlay que joga para vocÃƒÂª. [ Script em desenvolvimento ]
// @include        http://*reidocrime.com/*
// ==/UserScript==

/*

Este script estÃƒÂ¡ em desenvolvimento mas jÃƒÂ¡ tem algumas coisas legais:

1) Mostra a hora no Brasil que vocÃƒÂª vai sair do Hospital ou da Cadeia

2) Recursos AutoPlay que joga pra vocÃƒÂª
- Recarrega stamina automaticamente
- Faz os principais roubos (que dÃƒÂ£o mais lucro) atÃƒÂ© chegar no limite do risco
...
- Quando acabar as opÃƒÂ§ÃƒÂµes de aÃƒÂ§ÃƒÂ£o (roubo/ajuda), mudar de mapa

-----------------------------------------------------
TO-DO

- Montar tela de configuraÃƒÂ§ÃƒÂ£o na pÃƒÂ¡gina do profile
. Intervalo entre as aÃƒÂ§ÃƒÂµes
. Editar lista das aÃƒÂ§ÃƒÂµes de risco
. Editar lista das aÃƒÂ§ÃƒÂµes sem risco
. Editar link do cafÃƒÂ©
...
// Flag se AutPlay estÃƒÂ¡ ativo
var autoPlay;

// Intervalo entre uma jogada e outra (milisegundos)
var autoPlayInterval = 2000;

// Nivel minimo para tomar cafÃƒÂ©
var autoPlayMinStamina = 30;

// Nivel maximo e mÃƒÂ­nimo do risco para comeÃƒÂ§ar a ajudar ou roubar as pessoas
var autoPlayUpRiskLimit = 200;
var autoPlayDownRiskLimit = 100;

// DireÃƒÂ§ÃƒÂ£o do AutoPlay (upRisk = Roubar / downRisk = Ajudar)
var autoPlayDirection;

// Id do elemento HTML que exibe o log
var autoPlayLogId;

// Link ativo que serÃƒÂ¡ disparado
...
// Lista de direÃƒÂ§ÃƒÂ£o para percorrer os mapas
...
function getDate( sDate ) {
    ...
    */
    ...
    var nMon = parseInt(sDate.substr(3,2)) - 1; // os meses no javascript comeÃƒÂ§am no 0
    var nYea = parseInt(sDate.substr(6,4));

    // A hora do jogo estÃƒÂ¡ em +2 .. 
    //     sÃƒÂ³ que no Brasil ÃƒÂ© -3 .. 
...
function autoPlayOn()
{
   if( autoPlay )
   {
      // Se tem um link definido, entÃƒÂ£o roda ele ...
      if( autoPlayActiveLink ) activeLink( autoPlayActiveLink );
      
      // ******************************************
      //
      // Se nÃƒÂ£o tem link definido ... entÃƒÂ£o analisa cenÃƒÂ¡rio para on AutoPlay
      //
      
      var oLog = document.getElementById(autoPlayLogId);
      var success = false;
      
      // verifica se estÃƒÂ¡ na tela da cidade
      
      if( !document.getElementById('map') )
      {
          oLog.innerHTML = "AutoPlay ON (NÃƒÂ£o estÃƒÂ¡ na cidade!)";
          autoPlayActiveLink = getUserMapLink();
          autoPlayInterval = 60000; // espera 1 minuto para tentar novamente

          setCookie("AutoPlayMapIdx__",0); // zera a direÃƒÂ§ÃƒÂ£o do mapa
      }
      ...
      // verifica se precisa de cafÃƒÂ©
      
      else if( userStamina < autoPlayMinStamina )
      {
         oLog.innerHTML = "AutoPlay ON (CafÃƒÂ©!)";
         autoPlayActiveLink = getActionLink(autoPlayCoffee);
      }
      
      // verifica se ÃƒÂ© para abaixar o risco
	 ...
	 {	
      ...
      // se passou por tudo e chegou atÃƒÂ© aqui, entÃƒÂ£o alguma coisa deu errado
      ...
      // Aguarda o tempo definido para refazer a anÃƒÂ¡lise
...
function getStatus() {
  ...
  if( aA.length > 0 )
  {
    ...
    if( href.match("hospital") )
    {
      ...
      newText.innerHTML = "No hospital atÃƒÂ© " + getDate(tempo);
    }
    else if( href.match("jail") )
    {
      var tempo = aA[0].getElementsByTagName('span')[0].getAttribute('title');
      newText.innerHTML = "Na cadeia atÃƒÂ© " + getDate(tempo);
    }
  ...
  if( !success ) newText.innerHTML = "Status nÃƒÂ£o identificado";
...
function getNextMapDirection() {
  ...
  var newText = document.createElement('span');
      newText.innerHTML = "PrÃƒÂ³xima direÃƒÂ§ÃƒÂ£o do mapa: " + autoPlayMapOrder[autoPlayMapOrderIdx];
...
function init()
{
	...
	{
	   // SenÃƒÂ£o, considera o AutoPlay desligado (zero)
