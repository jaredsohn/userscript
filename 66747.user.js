// ==UserScript==
// @name           TehConnection Advanced Buffer Calculator
// @namespace      http://userscripts.org/scripts/show/66747
// @description    Show buffer in page header and on user pages based on jonls's What.CD Advanced Buffer Calculator.
// @author         jonls & kungfudrew
...
(function() {
	...
	GM_registerMenuCommand('TehConnection Advanced Buffer Calculator: '+(display_in_header ? 'Hide buffer in header' : 'Show buffer in header'), function() {
	...
	GM_registerMenuCommand('TehConnection Advanced Buffer Calculator: '+(use_ratio_coloring ? 'Use simple ratio coloring' : 'Use advanced ratio coloring'), function() {
	...
	function insert_buffer(parent, up, down, save_name) {
		...
		/* Append buffer stats */
		if (parent.attr('class') == 'toolbox') var buffer_item = $('<span class="buffer">| Buffer: </span>');
		else var buffer_item = $('<li>Buffer: </li>');
	...
	var header_req_elm;
	header_up_string = $('span.up').text();
	header_down_string = $('span.down').text();
	header_ratio_span = $('span.ratio');

	/* Parse size strings */
	var header_up = parse_binary_size(header_up_string);
	var header_down = parse_binary_size(header_down_string);
	
	//Find the ratio required based TehConnection rules
	function setRequired (down)
	{
			if (down < parse_binary_size("10 GB")) return 0;  
			else if (down < parse_binary_size("25 GB")) return 0.25;
			else if (down < parse_binary_size("50 GB")) return 0.3;
			else if (down < parse_binary_size("75 GB")) return 0.4;
			else if (down < parse_binary_size("100 GB")) return 0.5;
			else if (down < parse_binary_size("150 GB")) return 0.6;
			else if (down > parse_binary_size("150 GB")) return 0.75;		
	}
	
	//Finds your required ratio based on how much you have downloaded and the rules of TehConnection
	header_req = setRequired(header_down);

	/* Insert buffer in header */
	if (display_in_header) {
		/* Insert buffer */
		insert_buffer($('.toolbox'), header_up, header_down, 'header');
	}
	...
	$('span.ratio').attr({	
		title: "Ratio required: "+header_req,
	});
	$('span.ratio').css({'cursor' : 'help'})
	//header_req_elm.remove();
	

	if (/tehconnection\.eu\/user\.php\?id=/.test(document.URL)) {
		/* Find stats box */
		...
		if (user_up < 10*GiB) {
			append_transition_date(10*GiB, 'Member');
		}
		else if (user_up < 50*GiB) append_transition_date(50*GiB, 'Teh Fanatic');
		else if (user_up < 100*GiB) append_transition_date(100*GiB, 'Teh Actor');
		else if (user_up < 500*GiB) append_transition_date(500*GiB, 'Teh Director');
		else if (user_up < 1*TiB) append_transition_date(1*TiB, 'Teh Producer');
	}
