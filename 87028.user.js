//--------------------------------------------------------------------
// hacked by HCL: (http://www.flickr.com/people/yo70)
// build upon >> Flickr Publish Now! 
// by Vincenzo  Visciano (http://www.flickr.com/people/liberodicrederci/)
// and Alesa Dam (http://flickr.com/alesadam/): 
//      made it run on the new photo page layout + support for Google Chrome, and Opera
// first version by extranoise (Till Krech): http://www.flickr.com/people/extranoise
//--------------------------------------------------------------------
//
// A Greasemonkey user script for the Guess Where Berlin flickr group.
// http://www.flickr.com/groups/gwb
//
// It tags a guessed photo belonging to its owner
// in this group according to the rules:
//
// 4a. Tag your image "guesswhereberlin" until it is guessed, then replace it with "guessedberlin"
// 4b. Guessers are also asked to tag any guessed photo with "gwb username" 
//
//     You're invited to use this phrase in the descripton of your photo:
//     Guessed in the <a href="http://www.flickr.com/groups/gwb">Guess Where Berlin Group</a> by <b>username</b>!
//
//
// PublishNow! it's a simplified way to publish your photos on your photostream. Upload your photo as private, tag it and geotag it. When you are done, click on the Publishnow! link and your photo will appear on flickr as posted that very moment. You can simultaneously choose the privacy level. This script is also useful for reordering photos on your photostream.
//--------------------------------------------------------------------

// ==UserScript==
// @name          GWB Gummipunkt V2 by hcl
// @namespace     http://www.flickr.com/people/yo70
// @description   Simplifies the distribution of "Gummipunkts" in the  Guess Where Berlin flickr group
...
function showUpdateNotification() {
    ...
    new Element('a', {
        html: 'Gummipunkt!: update available',
...
function updatePrivacy(photoId) {
    new Request({
        ...
        onSuccess: function (responseText, responseXML) {
            try {
                ...
                if (result.stat === 'fail') {
                    $('FPNpublic').getParent().getElements('img').each(function (img) {
                        img.src = images.error;
                    });
                    $('FPNprivate').getParent().getElements('img').each(function (img) {
                        img.src = images.error;
                    });
                    $('FPNdialog').adopt(new Element('span', { html: result.message}));
                    return;
                }
                $('FPNpublic').getParent().getElements('img').each(function (img) {
                    img.src = images.ok;
                ...
                $('FPNdialog').adopt(new Element('span', { html: e }));
                return;
            }
        },
        onFailure: function () {
            ...
            $('FPNdialog').adopt(new Element('span', {
                html: "error getting photo info using id " + photoId
            }));
        }
    }).get('/services/rest', {
            ...
            nojsoncallback: 1,
            photo_id: photoId,
            is_public: ($('FPNpublic').checked ? 1 : 0),
            is_friend: ($('FPNfriends').checked ? 1 : 0),
            is_family: ($('FPNfamily').checked ? 1 : 0)
    });
}

function updatePostedDate(photoId) {
    new Request({
        url: 'http://api.flickr.com',
        onSuccess: function (responseText, responseXML) {
            try {
                ...
                if (result.stat === 'fail') {
                    $('FPNdate').getElement('img').src = images.error;
                    $('FPNdialog').adopt(new Element('span', { html: result.message}));
                    return;
                }
                $('FPNdate').getElements('img').each(function (img) {
                    img.src = images.ok;
                });
                reloadPage();
            } catch (e) {
...
function publishNOW(photoId) {
    var publicVisibility = $('FPNpublic').checked;
    var friends = $('FPNfriends').checked;
    var family = $('FPNfamily').checked;

    new Element('img', { src: images.updating }).inject(
        publicVisibility ? $('FPNpublic').getParent() : $('FPNprivate').getParent()
    );

    new Element('img', { src: images.updating }).inject($('FPNdate'));
            ...
            try {
                ...
                if (publicVisibility) {
                    if (photo.visibility.ispublic === 1) { // nothing to change
                        $('FPNpublic').getParent().getElement('img').src = images.ok;
                        reloadPage();
                    } else { // make public
                        updatePrivacy(photoId);
                    }
                } else {
                    if (photo.visibility.ispublic === 0) { // nothing changed?
                        if (idem('FPNfriends', photo, 'isfriends') && 
                            idem('FPNfamily', photo, 'isfamily')) { // nothing changed
                            $('FPNprivate').getParent().getElement('img').src = images.ok;
                            reloadPage();
                        } else {
                            updatePrivacy(photoId);
                        }
    ...
    updatePostedDate(photoId);
}

function togglePublishDialog(evt) {
    var publishDialog = $('FPNdialog');
            ...
            new Element('ul').adopt(
                ...
                new Element('li').adopt(
                    new Element('input', {
                        ...
                        events: {
                            click: function () {
                                $('FPNfriends').set('checked', false);
                                $('FPNfamily').set('checked', false);
                            }
                        }
                    }),
                    new Element('label', {
...
// the real code
var discussPhoto = document.getElementById('DiscussPhoto');
var newLayout = false;

if (discussPhoto === null) { // new layout?
    discussPhoto = document.getElementById('comments');
    newLayout = true;
}
...
$('photo-sidebar-photo-owner').adopt(
    new Element('a', {
        html: 'Publish',
