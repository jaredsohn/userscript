// ==UserScript==
// @name           Power Tools 2 - Deutsch
// @namespace      PDX
// @version 	   20110221a2PDX
// @include        *apps.facebook.com/kingdomsofcamelot*
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include	   http://dl.god-like.org/alert.html
// @description    KoC Power Tools 2 - German Version by PDX (Ohne Warteschlangen in der Übersicht)
// @require        http://sizzlemctwizzle.com/updater.php?id=98437
// ==/UserScript==

var Version = '20110221a2PDX';
...
var Options = {
  ...
  lastVersion : null,
  gmtClock : true,
  alertConfig  : {aChat:true, aPrefix:'-= ACHTUNG = ANGRIFF =-', scouting:true, wilds:true, minTroops:1, spamLimit:10 },
};
...
var knightRoles = [
  ['Meister', 'politics', 'Politik'],
  ['Marschall', 'combat', 'Schlacht'],
  ['Alchemistisch', 'intelligence', 'Intelligenz'],
  ['Verwalter', 'resourcefulness', 'Einfallsreichtum'],
];
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wildniss</td><TD></td><TD align=left>Koordinaten</td><TD>Fallen</td><TD align=left>Söldner</td>\
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Fallen Bauen:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Kosten: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("Bauen/Einstellen", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Gold: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
// TODO: Seed.wildDef is null if just aquired     
    ...
    popDiv.innerHTML = '<TABLE class=ptTab width=100% height=100%><TR><TD>\
          <DIV class=ptstat>Setze Wildniss Verteidigung</div>\
  ...
  processQue : function (errMsg){
    ...
    if (what[0] == 'T'){
      div.innerHTML += 'Baue '+ what[2] +' Fallen für '+ Cities.byID[t.buildList.cityId].name +'\'s Wildniss bei '+ wild.xCoord +','+ wild.yCoord +' ... ';
      t.postBuyTraps (t.buildList.cityId, what[1], what[2], t.processQue);
    } else {
      div.innerHTML += 'Setzte Wildniss Verteidigung auf '+ mercNames[what[2]] +' für '+ Cities.byID[t.buildList.cityId].name +'\'s Wildniss bei '+ wild.xCoord +','+ wild.yCoord +' ... ';
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          ...
          <TD width=26>Intelligenz</td><TD width=26>Einfallsreichtum</td><TD width=90 align=center colspan=5>-= Ritter Punkte =-</td><TD width=40 align=right> Kosten </td><TD width=35>Level</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    if (m.marchType == null)      // bogus march (returning scouts)
      return;
    if (ENABLE_TEST_TAB) Tabs.Fake.addDiv ("Incoming!<BR><PRE style='margin:0px;'>" + inspect (m, 8, 1) +'</pre>');
    if (m.marchType == 3){
    ...
    if (ENABLE_TEST_TAB) Tabs.Fake.addDiv (msg);
...
Tabs.Wappen = {
  ...
  show : function (){ 
	...
	var bor,ector,kay,bedivere,gawain,percival,galahad,lancelot,arthur,morgana,mordred,king,erde,wasser;
	...
	if (Seed.items.i1112){king=Seed.items.i1112}else{king=0};
	if (Seed.items.i1113){erde=Seed.items.i1113}else{erde=0};
	if (Seed.items.i1114){wasser=Seed.items.i1114}else{wasser=0};
    if (t.state == null){
      var m = '<style>\
CAPTION.MYTABLE\
  {\
...
<TABLE CLASS="MYTABLE" CELLPADDING=0 CELLSPACING=0>\
    <CAPTION CLASS="MYTABLE">Städte Voraussetzung</CAPTION>\
    \
    <THEAD >\
      <TR CLASS="MYTABLE">\
        <TH CLASS="MYTABLE">Stadt</TH>\
        <TH CLASS="MYTABLE">Voraussetzung 1</TH>\
        <TH CLASS="MYTABLE">Voraussetzung 2</TH>\
        <TH CLASS="MYTABLE">Voraussetzung 3</TH>\
      </TR>\
    </THEAD>\
    \
    <TBODY>\
      <TR CLASS="MYTABLE">  \
        <TD CLASS="MYTABLE">Stadt 2</TD>\
        <TD CLASS="MYTABLE">Level 7 ('+Seed.player.title+')</TD>\
        <TD CLASS="MYTABLE">10 Freunde ('+Seed.appFriends.length+')</TD>\
        <TD CLASS="MYTABLE">&nbsp;</TD>\
      </TR>\
   \
      <TR CLASS="MYTABLE">  \
        <TD CLASS="MYTABLE">Stadt 3</TD>\
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    if (getMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER>Du musst in der Allianz sein!</center>';
      t.state = 1;
      return;
    }
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Spieler Name &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Spieler Suchen" /></td>\
		...
		<TR><TD class=xtab></td><TD class=xtab align=right>Allianz Liste: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="Deine Allianz"/>\
            <INPUT align=left id=allListSubmit type=submit value="Alle Auflisten" /></td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'Mind. 3 Zeichen Eingeben!';
      return;
    }
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Durchsuche ...</center>';
  ...
  eventGotPlayerList : function (rslt){
    ...
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>Leaderboard</a></span> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>Online Status</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>Letzter Login</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerGetLastLogin : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "suche login datum ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Leaderboard:</b> Nicht gefunden... :/ evtl. mal im Nebel suchen ;)';
      return;
    }
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">Letzter Login: '+lastLogin+'</span>';
       ...
       m = '<span style="color:red">Kein Login gefunden: '+lastLogin+'</span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  eventGotAllianceList : function (rslt){
    ...
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Allianz Name</td><TD class=xtab>Rank</td><TD class=xtab>Members</td>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Members Anzeigen</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
  ...
  eventListSubmit : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'Du bist nich in der Allianz!';
    }
  },
  ...
  showMyAlliance : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'Du musst in der Allianz sein!.';
    }
  },
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 ) {
      document.getElementById('allListOut').innerHTML = 'Allianzen als erstes Anzeigen.';
      return;
    }
    if (t.MaxPage < 0 || val > t.MaxPage || val < 1) {
      document.getElementById('allListOut').innerHTML = 'Seiten Nummer gibt es nicht!';
      return;
    }
    t.curPage = val;
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Durchsuche...</center>';
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Members Anzeigen</a></td></tr>';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Entfernung ist von <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' spieler gefunden &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Spieler</div></a></td>\
        ...
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Städte</a></div></td>\
        ...
        <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Koordinate</a></div></td>\
        <TD id=clickCol8 onclick="PTalClickSort(this)" class=clickable><A><DIV>Entfernung</a></div></td>\
      ...
      <DIV style="top:670px; left:0px; position:static; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Daten sind vom Leaderboard und können 24 Stunden alt sein!</td>\
          <TD class=xtab align=right>Auf den Spaltenkopf klicken um ihn zu Sortieren</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>KOSTEN</b></td><TD class=xtabHL colspan=7><B>STATISTIK</b></td><TD class=xtabHL><B>VERBRAUCH</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Nahrung</td><TD class=xtabH>Holz</td><TD class=xtabH>Stein</td>\
        <TD class=xtabH>Erz</td><TD class=xtabH>Volk</td><TD class=xtabHL>Macht</td><TD class=xtabH>Leben</td><TD class=xtabH>Angriff</td><TD class=xtabH>Def</td><TD class=xtabH>Speed</td><TD class=xtabH>Weite</td><TD class=xtabH>Load</td>\
    ...
    m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';
    m += '</table>';
    m += '<BR><DIV class=ptstat>ENTFERNUNGS RECHNER</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Startpunk: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> <SPAN id=ptloc1></span></td></tr>\
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = 'Die Entfernung von '+ x1 +','+ y1 +' nach '+ x2 +','+ y2 +' ist: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
...
Tabs.Einstellung = {
  ...
  init : function (div){
    var t = Tabs.Einstellung;
    t.cont = div;
    
    try {      
      m = '<TABLE class=ptTab>\
        <TR><TD colspan=2><B>Einstellung:</b></td></tr>\
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Fenster verschieben erlauben.</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Zeige \'Nahrung Verbleibend\' in ROT an wenn sie für weniger als \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> Stunden ausreicht!</td></tr>\
        <TR><TD colspan=2><P><B>KoF Einstellung:</b></td></tr>\
	 ...
	 m += '<TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Automatisch Angriffe im Allianz Chat posten.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
          <TR><TD align=right>Nachrichten Prefix: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
          <TR><TD align=right>Bei Spähen: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Bei Wildniss Angriffe: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Angreifer Allianz Anzeigen: &nbsp; </td><TD><INPUT id=alertAlliance type=checkbox '+ (Options.alertConfig.includealliance?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Angreifer Macht Anzeigen: &nbsp; </td><TD><INPUT id=alertMight type=checkbox '+ (Options.alertConfig.includemight?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Sound Alarm: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.sound?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Mind. Truppenstärke: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
          </table></td></tr>';
		  }
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Erweiterte Allianz Berichte.</td></tr>\
        ...
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Erweiterte Nachrichten ("Senden" und "Alle Offiziere" Buttons).</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Erweiterte Seiten Navigation für die Berichte. <SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Warnen falls du nach 0,0 Maschieren möchtest. <SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>GMT Uhrzeit neben der Camelot Zeit (Um Angriffe zu planen)<SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>KofC Bug Fixes:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Tower Berichte Fix (stadt, wildnis oder ungültig)</td></tr>\
        ...
        </table><BR><BR><HR>Hinweis: Wenn eine Box grau unterlegt ist hat KoC mal wieder eine änderung vorgenommen!<BR>KoC Power Tools - Deutsch<BR>German Version by PDX';
      t.cont.innerHTML = m;
  ...
  init : function (div){
    ...
    this.cont.innerHTML = '\
      ...
      <SELECT id="srcType">\
        ...
        <option value="188,563"> SüdWest </option>\
        <option value="563,563"> SüdOst </option>\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Startpunkt: &nbsp; X: </td><TD class=xtab>\
        ...
        </td></tr><TR><TD class=xtab align=right>Max. Entfernung: </td><TD style="width: 200px;" class=xtab><INPUT id=srcDist size=3 value=10 /></td>\
        <td></td>\
        </tr>';
      m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Suchen" /></td></tr>';
      m += '</table>';
...
clickedPlayerDetail : function (span, uid){
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Allianz: '+ a +' &nbsp; Stadt: '
          + u.cities +' &nbsp; Population: '+ u.population +'</td></tr><TR><TD></td><TD>Provinzen: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
  ...
  eventSubmit : function (){
    ...
    t.aName = document.getElementById('allAllName').value;
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'Mehr als 3 Zeichen eingeben!';
      return;
    }
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
...
//End edit city search
  ...
  srchTypeHandler : function (){
    ...
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Wildniss Typ:</td><TD class=xtab align=right>\
            Wälder<INPUT id=woodWild type=CHECKBOX'+ (Options.woodWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Grassland/See<INPUT  id=foodWild type=CHECKBOX '+ (Options.foodWild?' CHECKED':'') +'></td>\
      ...
      m += '<TR><TD class=xtab align=right>Ebene<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD class=xtab align=right>Hügel<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Frei:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Sortieren:</td><TD class=xtab><SELECT id=filSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Entfernung</option>\
			...
			<TR><TD class=xtab align=right>Nur Koodinaten Anzeigen:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
        ...
        m+= '<TR><TD class=xtab align=right >Zeige:</td><TD class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
             ...
             <OPTION value=1>Feindlich</option>\
	     <OPTION value=2>Im Nebel</option>\
	     <OPTION value=3>Alliierte</option>\
	     <OPTION value=4>Freundlich</option>\
	     <OPTION value=5>Neutral</option>\
	     <OPTION value=6>Allianzlose</option>\
             </select></td></tr>';

		m+= '<TR><TD class=xtab align=right >Sortieren:</td><TD class=xtab><SELECT style="width: 135px" id=filSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Macht</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Entfernung</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right >Nur Koordinaten Anzeigen:</td><TD class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X muss zwischen 0 und 749 liegen!<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y muss zwischen 0 und 749 liegen!<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>750)
      errMsg += "Max. Entfernung muss zwischen 1 und 750 liegen!<BR>";
    if (errMsg != ''){
    ...
    document.getElementById ('srcStart').value = 'Suche Anhalten';
    ...
    m = '<CENTER><B>Suchen nach '+ typeName +'<BR>\
        Startpunkt: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Entfernung: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>EINSTELLUNG:</b><BR></td></tr>';
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. Level:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
    ...
    if (t.opt.searchType == 1){
      ...
      m += htmlOptions ( {1:'Glassland/See', 3:'Wälder', 4:'Hügel', 5:'Berg', 6:'Ebene', 0:'Alles'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Unbekannte:</td>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD class=xtab align=right>Sortieren:</td><TD class=xtab><SELECT id=filSortBy>\
            ...
            <TR><TD class=xtab align=right>Nur Koordinaten Anzeigen:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
	} else {
	 m += '<TR><TD class=xtab align=right>City Type:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'Alle', 2:'Alliierte', 3:'Freundlich', 4:'Freindlich', 5:'Neutral', 6:'Allianzlose', 7:'Im Nebel'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Sortieren:</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Macht</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Entfernung</option>\
...
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Barbaren Lager', 'Grassland', 'See', 'Wälder', 'Hügel', 'Berg', 'Ebene', 'Städte' ];
    var t = Tabs.Suchen;
...
if (DEBUG_TRACE) DebugTimer.start();
    ...
    for (i=0; i<t.mapDat.length; i++){
	  ...
	  if (t.opt.searchType == 2 && type == 7 ) {
			if (!(Options.cityType == 2) || t.mapDat[i][12] == 'Alliierte')
			if (!(Options.cityType == 3) || t.mapDat[i][12] == 'Freundlich') 
			if (!(Options.cityType == 4) || t.mapDat[i][12] == 'Feindlich')
			if (!(Options.cityType == 5) || t.mapDat[i][12] == 'Neutral') 
			if (!(Options.cityType == 6) || t.mapDat[i][12] == 'Allianzlose') 
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 0) // Barbs
			...
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Entfernung</td><TD style="padding-left: 10px;">Level</td><TD width=80%> &nbsp; Typ</td><TD style=""></td></tr>';

		else if (t.opt.searchType == 1) // Wilds
			//m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Entfernung</td><TD style="padding-left: 10px;">Lvl</td><TD style="padding-left: 10px;> &nbsp; Type</td><TD width=80%>Owner</td><TD style=""></td></tr>';
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Entfernung</td><TD style="padding-left: 10px;">Level</td><TD width=10%> &nbsp; Typ</td><TD width=70%> &nbsp; Owner</td><TD style=""></td></tr>';
		else // Cities
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD >Dist</td><TD>City</td><TD>Owner</td><TD>Macht</td><td>Alliance               </td><TD width=80% style="font-size:9px;">More info</td><TD style="padding-left: 10px;"></td></tr>';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'Suchen';
...
Tabs.Ausbilden = {
  ...
  init : function (div){
    var t = Tabs.Ausbilden;
    t.cont = div;
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Hier kannst du Truppen Ausbilden und Mauerverteidigung bauen!</div><DIV style='height:10px'></div><DIV class=ptentry>\
      ...
      <TR><TD align=right valign=top>Arbeiter Entlassen: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Erlaubt dir die Feld Arbeiter Auszubilden. (Prod. wird aber vorübergehend eingestellt!)</span></td></tr>\
      ...
      <TABLE align=center><TR><TD align=right>Mauerverteidigung: </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Fest montierte Armbr</option>\
      ...
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Bauen'\></td></tr></table>\
      </td></tr></table></div></div>\
      ...
      <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Warteschlange &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
        <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Bau Aufträge &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
      </div>";
    t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Du willst 0 Truppen Ausbilden ?</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Kann nicht so viele Truppen Ausbilden! (max. ist '+ t.stats.MaxTrain +')</font>';
      return;
    }
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; Du Besitz:'+ t.stats.defOwned;   
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Kein Slot ? mind. 1 eingeben ;)</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Truppen können nicht ausgebildet werden! (max. ist '+ t.stats.MaxDefTrain +')</font>';
      return;
    }
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Aufträge für den Mauerbau wurden gesendet!</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Es werden '+ cmd[2] +' '+  fortNamesShort[cmd[1]] +' Ausgebildet<BR>');
        doDefTrain ( cityId, cmd[1], cmd[2], 
          function(errMsg){
            setTimeout(function (){Tabs.Ausbilden.doDefQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500);
          } );
      }
    } catch (err) {
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B>Versorger</b></td><TD width=8%><B>Miliz</b></td><TD width=8%><B>Späher</b></td>\
        <TD width=8%><B>Pike</b></td><TD width=8%><B>Schwerter</b></td><TD width=8%><B>Bogis</b></td>\
        <TD width=8%><B>Kav</b></td><TD width=8%><B>SKav:</b></td><TD width=8%><B>Wagen</b></td>\
        <TD width=8%><B>Ballis</b></td><TD width=8%><B>Ramböcke</b></td><TD width=8%><B>Steinschleuder</b></td><tr>';
 ...
 '<TD width=8%>'+Seed.units['city'+cityId]['unt11']+'</td><TD width=8%>'+Seed.units['city'+cityId]['unt12']+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        ...
        <TD width=16%><B>Erz</b></td><TD width=16%><B>Gold</b></td><TD width=16%><B>Müßiges Volk</b></td></tr>\
...
// defense queue ....
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Truppen werden Ausgebildet</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Es werden '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +' Ausgebildet<BR>');
        doTrain (cityId, cmd[1], cmd[2], 
          function(errMsg){
           setTimeout(function (){Tabs.Ausbilden.doQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500 );
          }
        );
      }
    } catch (err) {
...
var GMTclock = {
  ...
  everySecond : function (){
    ...
    GMTclock.span.innerHTML = ' &nbsp; GMT: '+ now.toLocaleFormat('%H:%M:%S') +'';
  },
}
...
Tabs.Übersicht = {
  ...
  hide : function (){
    clearTimeout (Tabs.Übersicht.displayTimer);
  },

  show : function (){
    var rownum = 0;
    var t = Tabs.Übersicht;
    ...
    function _row (name, row, noTotal){
      ...
      var tot = 0;
      var m = [];
      m.push ('<TR style="background: #141516" align=right');
      m.push (style);
      m.push ('><TD');
      m.push (style);
      m.push ('><B>');
...
//DebugTimer.start(); 
    try {
      ...
      str += "<TABLE class=ptTabLined cellspacing=0><TR valign=top align=right><TD width=65></td><TD width=88 style='background: #ffc'><B>GESAMT</b></td>";
      ...
      if (Options.includeMarching)
        str += '<TD width=81><B>Maschieren</b></td>';
      str += "</tr>";
...
//**********************************
      ...
      str += _row ('Versorger<BR>', rows[1]);    
      str += _row ('Milizsoldat<BR>', rows[2]);
      str += _row ('Späher<BR>', rows[3]);
      str += _row ('Lanzenträger<BR>', rows[4]);
      str += _row ('Schwertkämpfer<BR>', rows[5]);
      str += _row ('Bogenschütze<BR>', rows[6]);
      str += _row ('Kavallerie<BR>', rows[7]);
      str += _row ('Schwere Kavallerie<BR>', rows[8]);
      str += _row ('Versorgungswagen', rows[9]);
      str += _row ('Ballisten<BR>', rows[10]);
      str += _row ('Rammbock<BR>', rows[11]);
      str += _row ('Steinschleuder<BR>', rows[12]);
      ...
      str += _row ('Nahrung<BR>Verbleibend', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Marschierende Truppen/Ressourcen</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
        str += '<BR><BR><INPUT id=subSeed type=submit name="SEED" value="Fehler beseitigen">';
      Tabs.Übersicht.cont.innerHTML = str;
...
//DebugTimer.display ('Draw Overview');    
      ...
      Tabs.Übersicht.cont.innerHTML = '<PRE>'+ e.name +' : '+ e.message +'</pre>';
    }   
    t.displayTimer = setTimeout (t.show, 5000);
  },
};
...
Tabs.Märsche = {
  ...
  /***   ATTACKS SUBTAB  ***/
  showAngriffe : function (){
    var t = Tabs.Märsche;
    t.marchDiv.innerHTML = 'Eingehende Angriffe... ist in Arbeit!';;
  },
  
  
  /***   MARCHES SUBTAB  ***/
  showMärsche : function (){
    var t = Tabs.Märsche;
    t.marchDiv.innerHTML = 'Ausgehende Märsche... ist in Arbeit!';
  },
  
  
  
  /***  REINFORCEMENTS SUBTAB  ***/
  showBotschaft : function (){
    var rownum = 0;
    var names = ['Vers.', 'Miliz', 'Späher', 'Pike', 'Schwerter', 'Bogis', 'Kav', 'SKav', 'Wagen', 'Balli', 'Ram', 'Kat'];
    var t = Tabs.Märsche;
    clearTimeout (t.displayTimer);
...
//logit (inspect (enc, 6, 1));    
    ...
    s = '<div class=ptstat>Zeigt die Lagernde Truppen in deiner Botschaft an.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Es hat dir keiner Verstärkung geschickt... :/</b></center>';
    } else {
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Truppen Anzeigen';
    s += '<BR><BR><DIV style="font-size: 10px">Hinweis: Du musst Refreshen damit es wirksam wird! KEIN Auto Refresh, also immer F5...</div>';
...
var debugWin = {
  ...
  doit : function (){ 
    ...
    if (t.popDebug == null){  
      ...
      t.popDebug.getMainDiv().innerHTML = '<DIV><INPUT type=submit id=dbsuball value=Alles> &nbsp; <INPUT type=submit id=dbsubnone value=Keine> &nbsp; \
        <INPUT type=submit id=dbdefaults value=Standart> &nbsp; <INPUT type=submit id=dbsubdo value=Anzeigen></div>\
