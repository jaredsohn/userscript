// ==UserScript==
// @name           StumbleUpon Enchanced Forum
...
btn = $('input.bold[type="submit"]').hide();
...
function insertModEditBtn( modLink ) {
    ...
    if( modLink.prev().attr('href') == myname && editNum!=num ) {
...
function insertEditBtn( editLink ) {
    ...
    //console.log( 'num= ' + num )
    if( aPost.find('a[target="_top"]' ).attr('href') == myname  && editNum!=num ) {
...
function editPost( num ) {
  ...
  $( '.aPost[num="'+num+'"]' ).css( { 'background':'rgba(255, 255, 204, 0.5)','border' : '1px dashed pink' } );
     ...
     $('#postEditForm textarea' ).val( postBody.html() );
     $( '#postID').val( num );
