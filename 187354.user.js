// ==UserScript==
// @name           Universal Ads Skipper V.1 [Kaum Ndaloe 2014]
...
// @updateURL      https://userscripts.org/scripts/source/187354.meta.js
// @downloadURL    https://userscripts.org/scripts/source/187354.user.js
// @description    Bypasses Universal Ads adf.ly, lienscash.com, adfoc.us and bc.vc.
// @grant          none
// @include        http://q.gs/*
// @include        http://9.bb/*
// @include        http://u.bb/*
	...
	var loc = document.location.href, interval, secinterval, bypassers = {
		adfly: function() {
			...
			else if (document.head.innerHTML.match (tehregxp))
			{
				...
				bypassers._inject (function() {
					...
					/*if (document.head.innerHTML.match (/view6\.js/))
						...
						neededVar = zzz;*/
					if (easyUrl == 'true') {
						document.head.innerHTML = "<title>** KAUM NDALOE **</title>";
				
						document.body.innerHTML += "<br />Visit To My Blog <a href='http://www.kaumndaloe.blogspot.com'>By KAUM NDALOE</a>. And Always check <a href='http://userscripts.org/scripts/show/187354'>This Link</a> for updates and to report issues!";
			...
			else if (loc.match (reg5))
			{
				...
				errorMsg    += "Please report the problem on the following url: http://userscripts.org/scripts/discuss/187354 and INCLUDE the adfly link.\n";
				errorMsg    += "Thanks, Kaum Ndaloe (the script author - and note that I'm NOT an Arabian guy or something)";
				alert (errorMsg);
			}
			else {
				clearInterval(interval);
				alert ("Something went wrong! Please report the problem on the following url: http://userscripts.org/scripts/discuss/187354 and INCLUDE the adfly link!");
			}
		},
		lienscash: function() {
		...
		bcvc: function() {
			...
			bypassers._inject (function() {
				...
				if (document.body.innerHTML.match (ajaxrx))
				{
					...
					document.body.innerHTML = "I'm going to bypass this bad site, please wait a few seconds..<br />If the script freezes in this phase please report the problem to <a href='http://userscripts.org/scripts/discuss/187354'>this URL</a> and include the problematic link.<br /><strong style='font-size:18px;' id='sReplace'>0</strong>";
					//alert (_a.opt);
					//return;
