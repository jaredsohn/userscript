// ==UserScript==
// @name	         *Show Biz Challenges (Flickr- Checkplay)
// @namespace		http://www.flickr.com/groups/showbizchallenges/discuss/
// @date		        01.June.2008
...
// @modified by	  Kristy (Bear and the Moon- Show Biz Challenges)
...
var CPtoolversion="SBC-CP";
var CPtoolDBversion="SBC-CP";
var chlgsplit;
...
function skipchallenge(name)
	...
	if (name.match("SWEEP","i")=="SWEEP") return true; 
	if (name.match("WAITING","i")=="WAITING") return false;
	if (name.match("OPEN","i")=="OPEN") return false;
	if (name.match("VOTE","i")=="VOTE") return false;
	if (name.match("SET","i")=="SET") return true;
...
function nonchallenge(name)
	{
   if (name.match("The Green Room")=="Green Room") return true;
	}
...
function mustread(name)	
	{
	if (name.match("VOTING IS MANDATORY")=="VOTING IS MANDATORY") return true; 
	}
...
if ((newchlgstatus=="NOMINEE")&&(value=="voter")) return "Voted"; 
...
function setchlgstatuscolor(chlgstatus)
{
	if (chlgstatus=="VOTE") return "Blue";
        if (chlgstatus=="Voted") return "Grey";
	...
	if (chlgstatus=="CHAT") return "Pink";
        if (chlgstatus=="SET") return "YELLOW";
	if (chlgstatus=="MUST READ") return "Black";
	if (chlgstatus=="NOMINEE") return "Orange";
...
function setchlgstatustitle(chlgstatus)
{
	...
	if (chlgstatus=="OPEN") return "OPEN- accepting photo submissions.";
	if (chlgstatus=="EXCLUDED") return "You are excluded from entering this challenge, see rules for more information.";
	if (chlgstatus=="NOMINEE") return "You are a nominee in this event. Good luck!";
	if (chlgstatus=="Voted") return "Thanks for voting :)";
	if (chlgstatus=="ERREXCLPLAY") return "You are excluded from this challenge but have submitted a photo. Please have your entry removed.";
	...
	if (chlgstatus=="FILLED") return "Three photos have already been submitted.You may vote in it while we wait for an Agent to change the status to VOTE.";
	if (chlgstatus=="UPDATING") return "Challenge information is being updated behind the scenes";
	if (chlgstatus=="CHAT") return "Come on in :)";
	if (chlgstatus=="MUST READ") return "Please read this discussion prior to playing.";
	if (chlgstatus=="SET") return "Come on in :)";
}
...
anchor.style.textDecoration='none';
   ...
   if (newchlgstatus=="SET") anchor.innerHTML='On the Set';
...
for (i=1;i<trs.length;i++) {
  ...
  if (SET(chlgname))  chlgstatus="MEET";
  ...
  if (chlgstatus=="NOMINEE"&&!forcereload) playernumber++; //when forcereload number gets added in loadthread
