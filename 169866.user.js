// ==UserScript==
// @name       		Guardians Thread-sorter
// @author			Doctor Blue
// @namespace  		http://hackforums.net/
// @version    		0.2.1
// @description		This userscript will sort all threads in the Guardians subforum and group the sub-threads into different sections according to their prefixes
// @require 		http://ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js
// @include      	http://*.hackforums.net/forumdisplay.php?fid=239*
// @copyright  		2013
// ==/UserScript==
...
var threads = {official: [], computing: [], gaming: [], programming: [], graphics: [], srpp: [], marketing: [], miscellaneous: []};
...
$('tr:has(td.forumdisplay_regular,.forumdisplay_sticky)').map(function(){
    ...
    console.log("Found thread with title: " + prefix + " " + title);

    // Sort into arrays
    if(prefix.indexOf("[Official]") !== -1 || prefix.indexOf("[Announcement]") !== -1 || sticky == true){
        threads.official.push($(this));
	}else if(title.indexOf("***") !== -1){
        threads.computing.push($(this));
    }else if(title.indexOf("@@") !== -1){
...
if(threads.computing.length){
    $last = $('<tr id="computing_header"><td class="trow_sep" colspan="7">Computing</td></tr>').insertAfter('tr:has(.forumdisplay_regular):last');
    
    for (var i = 0; i < threads.computing.length; i++){
        $last = threads.computing[i].insertAfter($last).addClass("computing");
    }
}
...
if(threads.srpp.length){
    $last = $('<tr id="srpp_header"><td class="trow_sep" colspan="7">Science, Religion, Politics and Philosophy</td></tr>').insertAfter('tr:has(.forumdisplay_regular):last');
    
    for (var i = 0; i < threads.srpp.length; i++){
        $last = threads.srpp[i].insertAfter($last).addClass("srpp");
    }
}
