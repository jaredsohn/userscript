...
----------------------------------- USED TRACKING OBJECTS: -----------------------------------
	Rootstorage = {}				// {serverID: Server}
	Server = {};					// {Server: Data}
	Data: {
		cities: [CityID] - {
			queue: []				// Array of Building objects
			enabled: 1,
			supply:{
				w: 0,				// Min Wood to keep (this amount won't be touched by Auto Supplier)
				p: 0				// Min island resource
				self: 1 			// if 0 city won't be supplied by other cities.
				minRes: {			// Auto Supplier will make sure that the city resources are always above minRes
						wood: 0,
						glass: 0,
						marble: 0,
						sulfur: 0,
		...
		ASC	:{						// Scheduler
				enabled: 0,
				tasks: [0] - {
					type:1 			// 1: Trade, 2: Pillage
					id: task ID,
					data:{
							see tamplates in ASC
						}
					}
				}
		}
	...
	DEF = {
		...
		cities: [id] - {
							...
							donateWonder: 1, // 0 = disabled
							defendableEnemySize: 0, // 0 = always defend,		>0 = only defend if enemy size is larger that defendableEnemySize
							maxIslandRes: 0		// maximum stock for island resource; 0 = off
					   }
	}
...
AB = {
	...
	updateOccupiedCities: function(){
		...
		var e, t, ids = {}, occ = XP('option[contains(@class, "occupiedCities")]',1, $('citySelect'));
		if(occ) for each(e in occ) ids[Number(e.value)] = trim((t=e.textContent).slice((t=t.lastIndexOf(" "))<0?0:t));
		for(e in this.occupiedCities) if(!ids[e]) delete this.occupiedCities[e];
		for(e in ids) if(!this.occupiedCities[e]) this.occupiedCities[e] = {name: ids[e]};
	...
	updateCityTroopsFromPage: function(date){
		...
		if(view == 1 || view == 3){
			...
			for each(var e in unitCounts){	
				...
				AB.Cities[CID].units[unit].count = count;
			}
		}
		else if(view == 2){
			...
			for(var i in unitCounts){
				...
				AB.Cities[CID].units["unit "+unit].count = count;
			}
		}
		else if(view == 4){
			...
			for(var i in unitCounts){
				count = parseInt(unitCounts[i].textContent, 10)||0;
				unit = (t=(t=units[i].src).slice(0, t.lastIndexOf("_"))).slice(t.lastIndexOf("_")+1)
				AB.Cities[CID].units["unit ship_"+unit].count = count;
			}
		}
		...
		if(this.position === false) return;
		 ...
		 if(upgradeBox){
			...
			if(r) for (var i = 0; i < r.snapshotLength; ++i){
				...
				if(e.className && res){ 
					if((t=this.updatedCityResources[this.cityId][res]-(c=parseInt(removeCommas(e.lastChild.data), 10)))<=0 || 1) Add("&nbsp;&nbsp;&nbsp;(<b>"+addCommas(t)+"</b>)<br />",e);
...
\
	...
	showOptionsBox: function(){
		if(!this.optionsBoxObj){
			...
			$("gm_ABNotifier_minInterval").addEventListener("focus", function(e){try{AB.keyShortcutsEnabled = false;}catch(e){AB.error(e)}}, false);
			$("gm_ABNotifier_maxInterval").addEventListener("focus", function(e){try{AB.keyShortcutsEnabled = false;}catch(e){AB.error(e)}}, false);
			$("gm_ABNotifier_popupURI").addEventListener("focus", function(e){try{AB.keyShortcutsEnabled = false;}catch(e){AB.error(e)}}, false);
			...
			$("gm_ABSupplier_resTolerance").addEventListener("focus", function(e){try{AB.keyShortcutsEnabled = false;}catch(e){AB.error(e)}}, false);
			...
			for(cid in AB.fleetMovements){
				...
				htm += "<b>"+(date.getHours()<10?"0"+date.getHours():date.getHours())+":"+(date.getMinutes()<10?"0"+date.getMinutes():date.getMinutes())+'</b></td>\
				  <td style="text-align: left;">\
					...
					else if(AB.fleetMovements[cid][sid].type == 3 || AB.fleetMovements[cid][sid].type == 6 || AB.fleetMovements[cid][sid].type == 7){ // Pillage, Occupation
						if(AB.fleetMovements[cid][sid].type !== 7)
							htm += '<td title="Trade Ships" style="padding: 0pt 0.5em 0.2em 0pt;"><img style="height: 14px;" src="skin/characters/fleet/40x40/ship_transport_r_40x40.gif" /></td>';
				  ...
				  <td>';
				  	if(AB.fleetMovements[cid][sid].type == 3 || AB.fleetMovements[cid][sid].type == 6 || AB.fleetMovements[cid][sid].type == 7){ // pillage, occupy, block
						htm += '<a target="_blank" href="http://s16.en.ikariam.com/index.php?view=island&amp;cityId='+AB.fleetMovements[cid][sid].originId+'">\
							  '+AB.Cities[AB.fleetMovements[cid][sid].originId].city_name+'\
							</a>';
				  	}
				 	else{
						htm += '<a target="_blank" href="http://s16.en.ikariam.com/index.php?view=island&amp;cityId='+AB.fleetMovements[cid][sid].originId+'">\
	...
	visualImprovements: function(){
		...
		GM_addStyle("* .ambrosia, .premium, .plusteaser, .costAmbrosia, #trader, #moveCity, #viewCityImperium, #viewMilitaryImperium, #viewResearchImperium, #viewDiplomacyImperium, #setPremiumTransports, #banner_container, div.premiumExchange, a.premiumExchange{ display:none !important; }");
...
registerKeyEvents_ = (Timestamp()-registerKeyEvents_Start)/1000;		
	...
	Supplier:{
		...
		removeJob: function (city_id, resName){ // removes current supply job, moves to next
...
function initASC(){
	...
	ASC = {
		...
		getStatusImg: function(status, task){
			...
			// blue
			if(status === 3) return '<img style="cursor:pointer;" height="10" title="scheduled'+(task.data.conditions.time?' for '+this.getFormatedTimeString(new Date(task.data.conditions.time))+' (in '+this.getFormatedTimeDifference(task.data.conditions.time)+')':'')+" "+info+'" src="'+this.getStatusImgSrc(status)+'" align="absmiddle"/>';
			// gray
		...
		updateList: function(){
			...
			for each(var task in AB.Data.ASC.tasks){
				...
				cid = task.data.from;
				fromName = AB.Cities[cid].city_name;
				fromCoords = AB.Cities[cid].city_coord;
						  ...
						  <table cellspacing="0" cellpadding="0" border="0" style="width:100%; margin: 0pt;">\
							<tbody>\
							  <tr>\
								...
								<td width="9%" rowspan="1" status="1" type="1" i="'+i+'" style="text-align:right;">'+this.getStatusImg(task.enabled?active?pending?3:1:4:2, task)+'</td>\
								</tr>\
   								<tr>\
								<td colspan="7" valign="top" style="text-align:center;">\
								  <a target="_blank" style="color: rgb(0, 102, 0);" title="'+fromCoords+'&nbsp;'+fromName+'" href="?view=city&id='+task.data.from+'">\
			...
			if(!AB.Data.ASC.tasks.length){
				...
				function changeState(e){
					...
					var type = parseInt(parent.getAttribute("type"), 10);
					var task = AB.Data.ASC.tasks[i];
					AB.Data.ASC.tasks[i].enabled = Number(!AB.Data.ASC.tasks[i].enabled);
					AB.saveData();
					
					var active = true;
					if(type === 1 && (!task.data.conditions.toAccumulate && !task.data.conditions.times || task.data.finished)) active = false;
					if(type === 2 && (!task.data.conditions.empty && !task.data.conditions.times || task.data.finished)) active = false;
					...
					if(this.tasksSession[task.id].sid){
						for(cid in AB.fleetMovements){
							for(sid in AB.fleetMovements[cid]){
								...
								// loot
								var cargo = 0;
								if(task.data.targetCoords != AB.Cities[task.data.from].city_coord){
									for(unit in AB.fleetMovements[cid][sid].troops){
					...
					if(this.tasksSession[task.id].sid && !found){
						if(this.tasksSession[task.id].checked){
							...
							task.data.statuscode = 0;
								...
								// so we finish the task and we set the info in statuscode
							task.data.finished = 1;
		...
		check: function(){
			...
			taskloop:for (var i in AB.Data.ASC.tasks){
				...
				if(AB.Cities[data.from].actions == 0) {AB.log("no action points in "+AB.Cities[data.from].city_name); continue;}
				...
				if(task.type == 1){			// transport task
					if(pending || data.finished) continue;
					if(AB.ships < data.ships) continue;
					...
					for(res in data.res){
						if(AB.updatedCityResources[data.from][res] < data.res[res]+usedRes[res]) continue taskloop;
					}
			...
			else if(nextScheduledTask && nextScheduledTask < AB.ts+AB.checkInterval*60000){		// required precision
			...
			setTimeout(function(){try{
				...
				if(state === 0){
					...
					if(!data.conditions.toAccumulate) --data.conditions.times;
						...
						if(goalreached) data.finished = 1;
					}
					if(data.conditions.times <= 0) data.finished = 1;
					if(!data.finished){
		...
		executeAttackTask: function(i){
			...
			setTimeout(function(){try{
				...
				AB.displayWaitBox("Scheduled", (task.type==2?"Pillaging":"Occupying")+" <b style=\"color:red;\">"+AB.getResourceImg(data.targetRes)+" "+data.targetName+" ("+data.targetPlayer+")</b> with "+info+" from <b><i>"+AB.Cities[task.data.from].city_name+"</i></b>");
		...
		executeBlockadeTask: function(i){
			...
			setTimeout(function(){try{
				...
				AB.displayWaitBox("Scheduled", "Blocking Port of <b style=\"color:red;\">"+AB.getResourceImg(data.targetRes)+" "+data.targetName+" ("+data.targetPlayer+")</b> with "+info+" from <b><i>"+AB.Cities[task.data.from].city_name+"</i></b>");
		...
		addGeneralsScore: function(player){
			function getMilitaryScore(e){
				...
				GM_xmlhttpRequest ({
						...
						onload: function (response){
							try{
								var t = response.responseText.indexOf('<td class="score">');
								if(t === -1) return;
								var score = response.responseText.slice(t+18, response.responseText.indexOf('</', t));
								var place = response.responseText.slice(t=response.responseText.indexOf('<td class="place">')+18, response.responseText.indexOf('</', t));
		...
		drawTaskContent: function(type, obj, edit){
			...
			if(type==1){
				...
				var city, cities = [];
				for(city in AB.Cities) {
					cities.push({id:city, name:AB.Cities[city].city_name, res: AB.Cities[city].prodgood});
				}
				var htm = '<table><tr>\
					<td colspan="2"><b>from</b> \
					<select id="ASC_selectOriginCity">\
					  <option> --- Select --- </option>';
				for each(city in cities) htm += '<option'+(obj.from&&obj.from==city.id?' selected="selected"':'')+'>'+AB.getResourceImg(city.res)+" "+city.name+'</option>';
				htm += '</select> <b>to</b> <select id="ASC_selectTargetCity">\
					  <option> --- Select --- </option>';
				for each(city in cities) htm += '<option'+(obj.to&&obj.to==city.id?' selected="selected"':'')+'>'+AB.getResourceImg(city.res)+" "+city.name+'</option>';  
				htm += '</select></td>\
				  </tr>\
				...
				for(var res in obj.res) {
					...
					htm += ' <input style="width:45px; height:13; font-size:x-small; padding:0px;" title="'+res+'" res="'+res+'" type="text" onkeypress="event.keyCode==0xd && this.blur()" onfocus="this.select()" class="AS_inp" value="'+obj.res[res]+'" /> '+AB.getResourceImg(res);
				}
				htm += '</td>\
				  </tr>\
				  <tr>\
				...
				var city, cities = [];
				for(city in AB.Cities) {
					cities.push({id:city, name:AB.Cities[city].city_name, occupied:false});
				}
				for(city in AB.occupiedCities) {
					cities.push({id:city, name:AB.occupiedCities[city].name, occupied:true});
				}
				var htm = '<table><tr>\
					<td><b>Origin:</b></td>\
					<td><select id="ASC_selectOriginCity">\
					  <option> --- Select --- </option>';
				for each(city in cities) htm += '<option'+(obj.from&&obj.from==city.id?' selected="selected"':'')+' '+(city.occupied?'style="background-color:#FFDED8;"':'')+'>'+city.name+'</option>';
				htm += '</select></td>\
				  </tr><tr>\
					<td><b>Target:</b></td>\
				...
				function setOriginCity(e, i){
					if(i == 0){
						...
						for each(var Unit in unitInputs){
							...
							max = occupied? cityUnits[unit]||0 : cityUnits["unit "+unit].count;
				...
				function setUnits(e){
					...
					if(!isNaN(val) && val >= 0){
						var temp;
						if(AB.occupiedCities[obj.from]) temp = AB.occupiedCities[obj.from].units[unit]||0;
				...
				function setOriginCity(e, i){
					if(i == 0){
						...
						obj.from = cities[i-1].id;
						// correct unit numbers
						var unit, max, cityUnits = AB.Cities[obj.from].units; //["unit "+unit].count
						var unitInputs = XP("input[@unit]",1,tdUnits);
						for each(var Unit in unitInputs){
							unit = Unit.getAttribute("unit");
							max = cityUnits["unit ship_"+unit].count;
							if(obj.units[unit] > max){
								obj.units[unit] = max;
								Unit.value = obj.units[unit];
							}
				...
				function setUnits(e){
					...
					if(!isNaN(val) && val >= 0){
						var temp;
						if(AB.occupiedCities[obj.from]) temp = AB.occupiedCities[obj.from].ships[unit]||0;
		...
		processResult: function(response){
			...
			if(tRows){
				...
				for(var i = tRows.length; tRow = tRows[--i];){
					...
					else if(t == 'plunder' && (direction == 2 || direction == 0)) {	// own pillage attack return
					...
					underway = direction;
					if(type !== 3 && type !== 6 && type !== 7 && !AB.Cities[idB]) continue;
					if(type === 2 && !AB.Cities[idB]) continue; // occupied city not supported yet
					
					if(type === 1 || type === 2){
						if(!(unitBoxes = XP("td[3]//div[@class='unitBox'][div/img[contains(@src, 'resources')]]", 1, tRow))) AB.log("Notifier - Update Arrivetimes: cannot find resource unit boxes!")
			...
			if(AB.Data.Notifier.checkForAttacks && txt.indexOf("hostile") !== -1){
				...
				if(attacks.length && AB.state == 0){
					...
					// Defender
					if(attacks[0].units > DEFENDER.data.cities[targetID].defendableEnemySize){
	...
	DEFENDER = {
		...
		getData: function(){
			...
			if(!this.Data[AB.server]){
				...
				this.data = this.Data[AB.server];
				if(!this.data.cities[AB.cityId]) this.addCity(this.data, AB.cityId);
			}
		},
		saveData: function(){
		...
		sendResources:function (){
				...
				if(!(b = XP('//*[@id="submit"]'))) throw new Error("Fetching Error! No button found!");
				this.Job.jobs.splice(0,1);	// remove the city from job
				session.set('DEF_ResJob', uneval(this.Job)); // IMPORTANT - save Job object in session
				b.click(); // :D
				return true;
			},
		...
		updateTableHead: function(){
			...
			for(var col in this.data.columns){
				...
				switch(this.data.columns[col]){
					case 1:		// Position
						th.title = "Index (sort by time when added)";
					...
					case 22:		// Player Status
						th.innerHTML = '<img height="16" title="Attacks in last 24 hours" src="'+FL.images.lightbulbOn+'">';
						this.makeColumnSortable(th);
						break;
					case 11:		// Player
			...
			for(var col in this.data.columns){
				...
				switch(this.data.columns[col]){
					...
					case 7:		// required siege units
						...
						break;
					case 8:		// travel time
						var min = AB.getTradeTravelTime(null, null, AB.Cities[AB.cityId].city_coord, e.coords);
					...
					case 22:		// Player Status
						...
						break;
					case 11:		// Player
						td.innerHTML = '<a href="?view=highscore&amp;searchUser=MIB" title="View Score (MIB is inactive)" class="playerStatus inactive"><b>'+e.player+'</b></a>';
						break;
					case 23:		// Alliance
...
// Farm List add on:
 GM_addStyle("#FarmList_targetList tbody>tr:hover{border:2px solid gray}") // (too slow)
//  (developing)
