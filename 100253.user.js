...
// @description    handy scripts for t.sina.com.cn and weibo.com
// @version        1.20
// @author         forumz
...
var tMenuBtn; //显示菜单按钮 0-否 1-是
var otimgShowNav; //多图时用于临时保存原有导航状态
var rtjdImg = '';
...
function t_getSettings(){
	...
	tScaleMax = parseFloat(cuimgSet[2]);
	...
	timgShowNav = parseInt(GM_getValue('tsm_imgnav','1'));
	tMenuBtn = parseInt(GM_getValue('tsm_menubtn','1'));
	otimgShowNav = timgShowNav;
}
...
function t_Menu(){
	if($('#tsinam_setting').length<=0){
		$('<li><a id="tsinam_setting" style="cursor:pointer;">tsinaMore</a></li>').insertBefore($('div.header ul.person li[node-type=account] ul li:last'));
		$('<li><a id="tsinam_setting" style="cursor:pointer;">tsinaMore</a></li>').insertAfter($('div.WB_global_nav div.gn_person div[node-type=account] ul:last li:last'));
...
function preloadImgs(preimgList,idx){
	for(var i=idx;i<preimgList.length;i++){
		...
		if(timgLarge == 'checked'){
			...
			img.src = $(preimgList.get(i)).attr('src').replace('/square/','/large/');
 	   	}else{
			img.src = $(preimgList.get(i)).attr('src').replace('/thumbnail/','/bmiddle/');
			img.src = $(preimgList.get(i)).attr('src').replace('/square/','/bmiddle/');
    	}
	}
}
...
function t_More(){
        $(document).on('mouseenter','div.feed_lists dl.feed_list',function(){
            t_bePrepare($(this),false,false);
        });        
        $(document).on('mouseleave','div.feed_lists dl.feed_list',function(){
        	$('#tsinam_more').remove();
        	timgShowNav = otimgShowNav;

        });

        $(document).on('mouseenter','div.feed_lists div.feed_list',function(){
            t_bePrepare($(this),false,false);
        });        
        $(document).on('mouseleave','div.feed_lists div.feed_list',function(){
        	$('#tsinam_more').remove();
        	timgShowNav = otimgShowNav;
        });

        $(document).on('mouseenter','div.WB_feed div.WB_feed_type',function(){
            t_bePrepare($(this),false,true);
        });
        $(document).on('mouseleave','div.WB_feed div.WB_feed_type',function(){
        	$('#tsinam_more').remove();
        	timgShowNav = otimgShowNav;
        });

        $(document).on('mouseenter','ul.list_feed li.list_feed_li',function(){
            t_bePrepare($(this),false,false);
        });
        $(document).on('mouseleave','ul.list_feed li.list_feed_li',function(){
        	$('#tsinam_more').remove();
        	timgShowNav = otimgShowNav;
        });

        $(document).on('mouseenter','ul.MIB_feed li',function(){
            t_bePrepare($(this),true,false);
        });
...
function t_bePrepare(sItem,isEP,isNew){
	t_Menu();
    tSQIndex = 0;
    var isImg = false;
    var isCmt = false;
    var isChat = false;
    var imgItem = t_chkImg(sItem,isEP,isNew,tSQIndex);
    if(imgItem.url!=''){
        isImg = true;
    }
    if(isEP){
    	...
    	if(isNew){
	    	var ftOp = sItem.find('div.WB_func div.WB_handle');
	    	if(ftOp.length>1){
	    		ftOp = ftOp.eq(1);
	    	}
	    	var tCmt = sItem.find('div.WB_media_expand div.WB_handle');
    	}else if(document.domain=='hot.weibo.com'){
    		var ftOp = sItem.find('div.footer div.right');
    		var tCmt = sItem.find('dl.comment:first dd.info span');
    	}else if(document.domain=='huati.weibo.com'){
    		var ftOp = sItem.find('div.con_opt p.fr');
    		var tCmt = sItem.find('dl.comment:first dd.info span');
    	}else{
    ...
    if(isImg || isCmt || isChat){
        ...
        t_setMenu(imgItem,sItem,isEP,isNew);
    }
}
...
function t_setMenu(sImg,sItem,isEP,isNew){
    if(sImg.url!=''){
        $('#tsinam_img').bind('click',function(){
            var imginfo = t_getimgInfo(sItem,isEP,isNew);
            if(isEP){
            	imgList = $('ul.MIB_feed div.feed_img img.imgicon');
            }else{
				if(document.domain=='hot.weibo.com'){
					imgList = $('div.media span.thumb img[action-type=feed_list_media_img]');
				}else{
					imgList = $('img.bigcursor');
				}
        	}
            curImg = imgList.index(sImg.item);
            preloadImgs(imgList,curImg);
            t_showImgbox(sImg.url,imginfo,false,isEP,isNew);
        });
        if(timgAction==1){
    		sImg.item.parent().attr('action-type','').click(function(){
    			if(sImg.item.attr('action-type')=='fl_pics'){
        	...
        	sImg.item.parent().unbind('mouseleave');
        }
    }
    
    $('#tsinam_ccmt').bind('click',function(){
        t_showOriCmt(sItem,isNew);
    });
    
    $('#tsinam_chat').bind('click',function(){
        t_showCurCmt(sItem,isNew); 
    });
}
...
function t_getfdItem(sImg,isEP,isNew){
	var sItem;
	if(isEP){
		sItem = sImg.parents('li.MIB_linedot_l');
		if(sItem.length==0){
			sItem = sImg.parents('li.MIB_linedot2');
		}
	}else{
		if(isNew){
			sItem = sImg.parents('div.WB_feed_type');
		}else if(document.domain=='hot.weibo.com'){
			sItem = sImg.parents('div.feed_list');
		}else if(document.domain=='huati.weibo.com'){
			sItem = sImg.parents('li.list_feed_li');
		}else{
			sItem = sImg.parents('dl.feed_list');
		}
	}
	return sItem;
}

function t_getimgInfo(sItem,isEP,isNew){
	var isQ = false;
	var iUser = '';
	var qtUser = '';
	...
	if(isEP){
		...
		if(isNew){
			var imgInfo = sItem.find('div.WB_text[node-type=feed_list_content]');
			var qtMsg = sItem.find('div.WB_text[node-type=feed_list_reason]');
			iUser = sItem.find('div.WB_info');
			if(iUser.length>1){
				iUser = iUser.eq(0).html().replace(/(^\s*)|(\s*$)/g, '')+'：';
			}else{
				iUser = '';
			}
		}
			...
			if(isQ){
    			...
    			imgInfo = sItem.find('div.content p.maintext');
    		}else if(document.domain=='huati.weibo.com'){
    			imgInfo = sItem.find('div.content p.con_txt');
    		}
	...
	if(imgInfo.length>0){
	    if(imgInfo.find('em').length==0){
	    	if(isEP){
	    		...
	    		if(isNew){
    				rtjdTxt = '@' + sItem.find('div.WB_info').eq(0).text().replace(/(^\s*)|(\s*$)/g, '') + '：';
	    		}else{
		    		if(isQ){
	    ...
	    if(qtMsg.length>0){
	    	if(isNew){
				qtUser = sItem.find('div[node-type=feed_list_forwardContent] div.WB_info');
				rtjdTxt += '//' + qtUser.text().replace(/(^\s*)|(\s*$)/g, '') + qtMsg.text().replace(/(^\s*)|(\s*$)/g, '');
				qtUser = qtUser.html().replace(/(^\s*)|(\s*$)/g, '')+"：";
	    	}else{
	        ...
	        imgInfo = imgInfo + ' <b>回复：</b>' + qtUser + qtMsg.html().replace(/(^\s*)|(\s*$)/g, '');
	    }
	}else{
		imgInfo = '';
	}
...
function t_chkImg(sItem,isEP,isNew,SQIndex){
	var t_imgact = false;
	var imgIndex = 0;
	if(isEP){
		...
		if(document.domain=='hot.weibo.com'){
			var t_pimg = sItem.find('div.media span.thumb img:first');
		}else{
			var t_pimg = sItem.find('img.bigcursor');
		}
	}
    if(t_pimg.length==0){
    ...
    if(t_img.indexOf('/thumbnail/')<0 && t_img.indexOf('/square/')<0){
    return {url:t_img,item:t_pimg,isact:t_imgact};
    }
    timgShowNav = otimgShowNav;
    if(t_img.indexOf('/square/')>=0){
		otimgShowNav = timgShowNav;
		timgShowNav = 1;
		tSQLen = t_pimg.length;
		SQIndex = Math.abs(SQIndex);
    }else{
    	SQIndex = -1;
    }
	imgIndex = SQIndex;
	...
	t_img = t_pimg.eq(imgIndex).attr('src');
    if(timgLarge == 'checked'){
    	t_img = t_img.replace('/thumbnail/','/large/');
    	t_img = t_img.replace('/square/','/large/');
    }else{
    	t_img = t_img.replace('/thumbnail/','/bmiddle/');
    	t_img = t_img.replace('/square/','/bmiddle/');
    }
...
function t_showImgbox(sImg,sInfo,isNav,isEP,isNew){
	var fdItem = t_getfdItem($(imgList.get(curImg)),isEP,isNew);
	$('.t_inshow').removeClass('t_inshow');
	fdItem.addClass('t_inshow');
    isCancelimg = false;
    tframeLoaded = 0;
    ...
    if(isNav){
        setTimeout(function(){
            t_scrollToLoading(t_getfdItem($(imgList.get(curImg)),isEP,isNew));
        },500);
    }else{
        t_scrollToLoading(t_getfdItem($(imgList.get(curImg)),isEP,isNew));
    }
    ...
    imgReady(sImg,function(sW,sH){
        if(isNav){
            setTimeout(function(){
                t_showImg(sImg,sW,sH,sInfo,isEP,isNew);
            },500);
        }else{
            t_showImg(sImg,sW,sH,sInfo,isEP,isNew);
        }
...
function t_showImg(sImg,sW,sH,sInfo,isEP,isNew){
    if(tframeLoaded==1){
        return;
    }
    ...
    var imgInfo = sInfo;
    $.blockUI({ 
        message: '<div id="tsinam_imgframed"><div class="t_imgtitle"><ul class="t_topnav t_customCss"><li><a id="tsinam_imgla"class="t_imgbtn">原图</a></li></ul></div><div id="tsinam_imgarea"><canvas id="tsinam_canvas"style="display:none;"></canvas><img id="tsinam_showimg"src="'+ sImg + '"title="'+sHint+'"/><div class="t_imgprev"><span></span></div><div class="t_imgnext"><span></span></div><span id="tsinam_turnspan"><a id="turnLsign"class="t_turnsign"><img src="'+ tleftimg +'"title="向左转"/></a>&nbsp;<a id="turnRsign"class="t_turnsign"><img src="'+ trightimg +'"title="向右转"/></a></span><span id="tsinam_zoomspan"><a id="tZoomori"class="t_turnsign"><img src="'+ tzoriimg +'"title="还原"/></a>&nbsp;<a id="tZoomout"class="t_turnsign"><img src="'+ tzoutimg +'"title="缩小"/></a>&nbsp;<a id="tZoomin"class="t_turnsign"><img src="'+ tzinimg +'"title="放大"/></a></span></div><div class="t_imginfo">'+ imgInfo + '<div><span class="t_signprev">&lt;&lt;</span>　<span id="tsinam_imglocate"class="t_imgurl"><b><a>定位</a></b></span> | <span class="t_imgurl"><b><a target=_blank href='+sImg+'>另存</a></b></span> | <span id="tjd_rtjd"class="t_imgurl"><b><a>转煎蛋</a></b></span>　<span class="t_signnext">&gt;&gt;</span></div><span style="float:right;"><ul class="t_topnav t_customCss"><li><a id="tsinam_imgrt"class="t_imgbtn">返回</a></li></ul></span></div></div>',
        css: {
            top: stop + 'px', 
            left: sleft + 'px',
            width: sW+20+'px',
    ...
    $('#tjd_rtjd').click(function(){
    	$('html,body').animate({scrollTop: t_getfdItem($(imgList.get(curImg)),isEP,isNew).offset().top-50}, 500);
        rtjdImg = sImg.replace('/large/','/bmiddle/');
        $.unblockUI();
        t_Rtjd(); 
    });
    $('#tsinam_imgla').click(function(){
    	t_showImgbox(sImg.replace('/bmiddle/','/large/'),sInfo,true,isEP,isNew);
    });
    ...
    if(timgShowNav==1){
		...
		$('.t_signprev, .t_signnext').hide();
	}
    tframeLoaded = 1;
    event.preventDefault();
}

function t_setNaviAction(isEP,isNew){
    if(curImg>0){
        if(otimgShowNav==0 && tSQIndex==0){
        	$('span.t_signprev, div.t_imgprev').hide();
        }
    	$('span.t_signprev').attr('title','上一张');
        $('span.t_signprev, div.t_imgprev').bind('click',function(){
	       	$.unblockUI();
            curImg--;
            var fdItem = t_getfdItem($(imgList.get(curImg)),isEP,isNew);
            var imginfo = t_getimgInfo(fdItem,isEP,isNew);
            $.unblockUI();
            if(!fdItem.hasClass('t_inshow')){
            	tSQIndex = 0;
            }else{
            	tSQIndex--;
            }
            t_showImgbox(t_chkImg(fdItem,isEP,isNew,tSQIndex).url,imginfo,true,isEP,isNew);
        }).mouseenter(function(){
    ...
    if(curImg<imgList.length-1){
        if(otimgShowNav==0 && tSQIndex==tSQLen-1){
        	$('span.t_signnext, div.t_imgnext').hide();
        }
    	$('span.t_signnext').attr('title','下一张');
        $('span.t_signnext, div.t_imgnext').bind('click',function(){
       		$.unblockUI();
            curImg++;
            var fdItem = t_getfdItem($(imgList.get(curImg)),isEP,isNew);
            var imginfo = t_getimgInfo(fdItem,isEP,isNew);
    	...
    	if(document.domain=='huati.weibo.com'){
    		return;
    	}
        if(otimgShowNav==0 && tSQIndex==tSQLen-1){
        	$('span.t_signnext, div.t_imgnext').hide();
        }
    	$('span.t_signnext').attr('title','更多');
		$('span.t_signnext, div.t_imgnext').css('cursor','se-resize');
		$('span.t_signnext, div.t_imgnext').bind('click',function(){
			if(isEP){
				...
				if($('div.W_pages').length>0){
					...
					t_rtHint(4);
					location.href = nextp;
				}else if($('div.pager_normal').length>0){
					var nextp = $('div.pager_normal a.btn_page:last').attr('href');
					if(nextp.indexOf('http://')<0){
						nextp = 'http://' + document.domain + nextp;
					}
					t_rtHint(4);
...
function t_showCurCmt(sItem,isNew){
	if(isNew){
		var uLnk = sItem.find('div.WB_from a.S_link2').attr('href');
	}else if(document.domain=='hot.weibo.com'){
		var uLnk = sItem.find('div.footer div.left a').attr('href');
	}else if(document.domain=='huati.weibo.com'){
		var uLnk = sItem.find('div.con_opt span.time a').attr('href');
	}else{
...
function t_showOriCmt(sItem,isNew){
	if(isNew){
		var uLnk = sItem.find('div.WB_media_expand div.WB_from a.S_func2').attr('href');
	}
	else{
...
function t_Rtjd(){
        ...
        $('.t_set_ft a:first').text('tsinamore v'+ tsinam_version + ' - forumz @ ' + t_rdate);
        $('.blockPage').css('background-color','transparent');
        $('.blockPage').css('z-index',zTop);
		$('.blockPage').drag(function( ev, dd ){
			var dtop = $(document).scrollTop();
        ...
        $('#tjd_send').click(function(){
            ...
            if($('#tjd_2pic').attr('checked')=='checked'){
                var jdplib = '26402';
		    }else if($('#tjd_2ox').attr('checked')=='checked'){
                var jdplib = '21183';
	        }else{
	            var jdplib = '17829';
	        }
...
function t_rtjdSend(sUser,sMail,sText,sImg,sPlib,isTxtup){
    ...
    GM_xmlhttpRequest({
      method: 'POST',
      url: 'http://jandan.net/wp-content/themes/egg/comments-ajax.php',
      data: rtMsg,
      headers: {
      ...
      onload: function(response){
        if(response.responseText.indexOf('Error: please fill the required fields (name, email)')>-1){
            ...
            if(rtjdChk=='checked'){
                switch(sPlib){
                    case '26402':
                        window.open('http://jandan.net/pic');
                        break;
                    case '21183':
                        window.open('http://jandan.net/ooxx');
                        break;
                    case '17829':
                    	window.open('http://jandan.net/gif');
                    default:
                        break;
                }
            }
...
function t_Settings(){
    ...
    if(timgShowNav==1){
    	$('#tsinam_setimgNav').attr('checked','checked');
    }
    if(tMenuBtn==1){
    	$('#tsinam_setmenubtn').attr('checked','checked');
    }
    ...
    $('#tsinam_setsave').click(function(){
    	...
    	if($('#tsinam_setimgNav').attr('checked')=='checked'){
    		GM_setValue('tsm_imgnav','1');
    	}else{
    		GM_setValue('tsm_imgnav','0');
    	}
    	if($('#tsinam_setmenubtn').attr('checked')=='checked'){
    		GM_setValue('tsm_menubtn','1');
    	}else{
    		GM_setValue('tsm_menubtn','0');
    	}
...
function t_FRshow(){
	if(firstRun>0 && document.domain=='weibo.com'){
		var tboxleft=0;
		var accitem1= $('div.header ul.person li[node-type=account]');
		var accitem2 = $('div.WB_global_nav div.gn_person div[node-type=account]');
		if(accitem1.length>0){
			tboxleft = accitem1.offset().left;
		}else if(accitem2.length>0){
			tboxleft = accitem2.offset().left;
		}
		if(tboxleft>0){
			$('<div id="tsinam_tbox" class="t_frshow"><div class="t_txtshow t_tbox">感谢使用 <b>tsinamore</b><br />更多功能选项可通过菜单【tsinamore】进行设置<br /><s><i></i></s></div></div>').appendTo('body').hide();
			$('#tsinam_tbox').css('left',tboxleft-150).show().mouseenter(function(){
...
if(GM_registerMenuCommand){
	GM_registerMenuCommand('从 userscript.org 更新 tsinamore',function(){update(true)});
}
