...
//var elements = ['watch-related-container', 'watch-discussion', 'watch-sidebar', 'ppv-placeholder', 'fyf_container', 'ad_creative_3']; // the ads.
var elements = ['watch-discussion'];
...
// Width of YouTube watch pages content (in pixels)
var YT_CONTENT_WIDTH = 970;

// Id of the player node (Flash and HTML5) in YouTube watch and channel pages
var YT_PLAYER_ID_FLASH = "movie_player";
var YT_PLAYER_ID_HTML5 = "movie_player-html5";

// Ids of the player's div node (player's parent node) in YouTube watch and channel pages
var YT_PLAYER_DIV_ID = "watch-player";
var YT_PLAYER_DIV_CHANNEL_ID = "playnav-player";

// Class of the player's root node (the player is inserted as a child of this node) and ids of the player node (Flash and HTML5) in YouTube channel pages (new layout)
var YT_PLAYER_ROOT_CHANNEL_NEW_CLASS = "channels-video-player";
var YT_PLAYER_CHANNEL_NEW_ID_FLASH = "movie_player-flash";
var YT_PLAYER_CHANNEL_NEW_ID_HTML5 = "movie_player-html5";

// Height of the player control bar
var YT_PLAYER_CONTROLBAR_HEIGHT = 35;

// Id of YouTube watch pages playlist
var YT_PLAYLIST_ID = "playlist-bar";

// Id of YouTube watch pages watch panel
...
// Creates a dropdown button (with the passed id, title and state) with an associated menu (buttonMenuId). The button management will be done by YouTube's code (data-button-menu-id attribute)
// The button will have a child span (buttonContentId) to hold its text (buttonText) and a arrow image for decoration
function createDropdownYTButton(buttonId, buttonText, buttonTitle, buttonContentId, buttonMenuId, buttonIsDisabled) {

	var dropButton = createNode("button", {"aria-pressed": false, class: YT_BUTTON_CLASSES + " " + YT_BUTTON_TOOLTIP_CLASS + " " + YT_BUTTON_TOOLTIP_REVERSE_CLASS, "data-button-action": "", "data-button-menu-id": buttonMenuId, id: buttonId, role: "button", title: buttonTitle, type: "button"}, {disabled: !!buttonIsDisabled});
...
function getYTConfig(prefName, prefDefaultValue) {
	...
	try {
		...
		GM_log("Error trying to get a YouTube config (" + prefName + "): " + err.toString());
	}
...
function setYTConfig(prefName, prefNewValue) {
	...
	try {
		...
		GM_log("Error trying to set a YouTube config (" + prefName + ") to " + prefNewValue + ": " + err.toString());
	}
...
//'
var videoFormatsLibrary = new AsocArray(5, new VideoFormatInfo(1, "Low Quality (legacy)", "LQ", "FLV", "Sorenson Spark", false, "MP3", 22.05),
																				18, new VideoFormatInfo(5, "Low Definition", "LD", "MP4", "H.264", false, "AAC", 44.1),
																				22, new VideoFormatInfo(14, "High Definition", "HD", "MP4", "H.264", false, "AAC", 44.1),
																				34, new VideoFormatInfo(6, "Low Definition", "FLV LD", "FLV", "H.264", false, "AAC", 44.1),
																				35, new VideoFormatInfo(10, "Standard Definition", "FLV SD", "FLV", "H.264", false, "AAC", 44.1),
																				37, new VideoFormatInfo(17, "Full High Definition", "Full HD", "MP4", "H.264", false, "AAC", 44.1),
																				38, new VideoFormatInfo(18, "4K Original Definition", "4K HD", "MP4", "H.264", false, "AAC", 48),
																				43, new VideoFormatInfo(3, "Low Definition", "WebM LD", "WebM", "VP8", false, "Vorbis", 44.1),
																				44, new VideoFormatInfo(9, "Standard Definition", "WebM SD", "WebM", "VP8", false, "Vorbis", 44.1),
																				45, new VideoFormatInfo(12, "High Definition", "WebM HD", "WebM", "VP8", false, "Vorbis", 44.1),
																				46, new VideoFormatInfo(15, "Full High Definition", "WebM Full HD", "WebM", "VP8", false, "Vorbis", 44.1),
																				82, new VideoFormatInfo(4, "Low Definition 3D", "3D LD", "MP4", "H.264", true, "AAC", 44.1),
																				83, new VideoFormatInfo(7, "Standard Definition 3D", "3D SD", "MP4", "H.264", true, "AAC", 44.1),
																				84, new VideoFormatInfo(13, "High Definition 3D", "3D HD", "MP4", "H.264", true, "AAC", 44.1),
																				85, new VideoFormatInfo(16, "Full High Definition 3D", "3D Full HD", "MP4", "H.264", true, "AAC", 44.1),
																				100, new VideoFormatInfo(2, "Low Definition 3D", "3D WebM LD", "WebM", "VP8", true, "Vorbis", 44.1),
																				101, new VideoFormatInfo(8, "Standard Definition 3D", "3D WebM SD", "WebM", "VP8", true, "Vorbis", 44.1),
																				102, new VideoFormatInfo(11, "High Definition 3D", "3D WebM HD", "WebM", "VP8", true, "Vorbis", 44.1)
);
...
function VideoAdapter(playerId, cbAPIFunc, defaultVideoName, defaultVideoUsername, doReloadPlayer) {
	...
	if ((this.videoFormats === null) || (this.videoFormats.length === 0)) {
		GM_log("[VideoAdapter]: No video formats were found");
	...
	if (vFeat.hasOwnProperty("autoplayMode")) {
		...
		registerAPIReadyAction("autoplayMode", function(autoplayMode) {
			...
			switch(autoplayMode) {
				...
				default:
					GM_log("[VideoAdapter.initFeatures]: Invalid autoplay mode");
					break;
...
VideoAdapter.parseURLMapGen = function(fmtList, streamMap) {
	...
	fmtList.forEach(function(fList) {
		...
		var formatObj = new VideoFormat(parseInt(matchfList[1], 10), formatStreamInfo.dUrl, parseInt(matchfList[2], 10), parseInt(matchfList[3], 10));
...
VideoAdapter.prototype.expandPlayer = function() {
	try {
		unsafeWindow.ytPlayerSIZE_CLICKED2(true);
	}
	catch(err) {
		GM_log("[VideoAdapter.expandPlayer]: Error trying to expand the video: " + err.toString());
	}
};

//'
function VideoAdapterHTML5(defaultVideoName, defaultVideoUsername) {

	// Gets the video Id
	this.vId = getYTConfig("VIDEO_ID", null);
	if ((typeof this.vId !== "string") || (this.vId === "")) {
	...
	this.videoFormats = this.parseURLMap();
	if ((this.videoFormats === null) || (this.videoFormats.length === 0)) {
		GM_log("[VideoAdapterHTML5]: No video formats were found");
		this.videoFormats = new AsocArray(); // Makes sure this.videoFormats contains a valid (empty) AsocArray if no video formats are found
	}
...
VideoAdapterHTML5.prototype.getFlashVar = function(varName) {
	...
	var argsObj = null;
	try {
		argsObj = unsafeWindow.yt.playerConfig.args;
	}
	catch(err) {
		GM_log("Error trying to get a HTML5 FlashVar (" + varName + "): " + err.toString());
	}
	if ((argsObj === null) || (typeof argsObj !== "object")) return null;

	return (argsObj.hasOwnProperty(varName)) ? argsObj[varName] : null;
...
var resizeNamedSizes = new AsocArray(//'
																		 ...
																		 function(playerExpandedSize, viewportSize) {
																			 return {width: YT_CONTENT_WIDTH,
																							 height: YT_CONTENT_WIDTH * (playerExpandedSize.height / playerExpandedSize.width)};
																		 },
...
GM_addStyle([
	...
	"#ytf-main-div {margin-bottom: 10px}",
	...
	".ytf-main-download-warning {color: red}",
	...
	"#ytf-conf-dialog {position: fixed; margin: auto; left: 0px; right: 0px; top: 0px; bottom: 0px; width: 475px; max-height: 590px; z-index: 1001; " +
		"overflow: auto; padding: 5px; color: black; background-color: #EEEEEE; -moz-border-radius: 15px; border-radius: 15px}",
	...
	"#ytf-conf-content li > input[type='text'], #ytf-conf-content li > select {margin-left: 5px; max-width: 200px}",
...
// controlsAutohideMode should be an integer number between -1 and 3
scriptConfig.controlsAutohideMode = Math.round(scriptConfig.controlsAutohideMode);
if ((isNaN(scriptConfig.controlsAutohideMode)) || (scriptConfig.controlsAutohideMode < -1) || (scriptConfig.controlsAutohideMode > 3)) controlsAutohideMode = -1;
	...
	function createDialog() {
		...
		confDialog = createNode("div", {id: "ytf-conf-dialog"},
			{innerHTML: "<div id='ytf-conf-title'>YousableTubeFix Configuration</div>" +
				...
				"<option id='ytf-conf-controls-autohide-mode-opt-controls' value='3'" + ((scriptConfig.controlsAutohideMode === 3) ? " selected='selected'" : "") + ">Only the controls bar</option>" +
				"</select>" +
				"</li>" +
				"<li>" +
...
var scriptVersion = 1334582218706; // 16 Apr 2012
...
if (ytPath === "/") {
...
else if ((/^\/user\//i.test(ytPath)) || (/^\/profile(?:\.php)?$/i.test(ytPath))) {
	...
	// Calls the script function for the user channel page if the channel player div or the player's root node (new channel layout) are found
	if (($(YT_PLAYER_DIV_CHANNEL_ID)) || ($x1("//div[starts-with(@class, '" + YT_PLAYER_ROOT_CHANNEL_NEW_CLASS + "')]"))) scriptChannelMain();
...
function scriptWatchMain() {
	...
	if ($x1("//embed[@id='" + YT_PLAYER_ID_FLASH + "']")) {
		...
		try {
			watchVA = new VideoAdapter(YT_PLAYER_ID_FLASH, scriptWatchAPI, YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME, false);
	...
	else if ($x1("//div[@id='" + YT_PLAYER_ID_HTML5 + "']")) {

		GM_log("HTML5 player support is incomplete yet");

		try {
			watchVA = new VideoAdapterHTML5(YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME);
		}
		catch(err) {
			GM_log(err.toString());
			return;
	...
	// No known player has been detected
		...
		GM_log("Player not found or of unknown type");
		return;
	}
	...
	function scriptWatchAPI() {
		...
		// Enables the resize button if it has already been added to the page (it is created in a disabled state if the API isn't ready at that time, because the API is needed to expand the player before resizing it)
	...
	function resizePlayer(sizeValue, doScroll) {
		...
		// Gets the player div and the video div (the player div parent)
		var playerDiv = $(YT_PLAYER_DIV_ID), videoDiv = $("watch-video");
		if ((!playerDiv) || (!videoDiv)) return;

		// Gets the headline div (the div with the video title). The player div is used if it isn't found (it doesn't matter much because is only used to scroll to it)
		...
		// The page is also scrolled to the headline div (if configured to do so) and the resize button is updated
		if (sizeValue === "default") {
			playerDiv.style.width = "";
			playerDiv.style.height = "";
			videoDiv.style.width = "";
			if (doScroll) scrollToNode(headlineDiv);
		...
		//'
		if (typeof meSt.playerExpandedSize === "undefined") meSt.playerExpandedSize = {width: playerDiv.offsetWidth, height: playerDiv.offsetHeight};
		...
		// Gets the playlist (if it exists in the page) and, if so, subtracts its height from the viewport height (the video and the playlist shouldn't overlap)
		var playlistDiv = $(YT_PLAYLIST_ID);
		if (playlistDiv) viewportSize.height = Math.clamp(playlistDiv.getBoundingClientRect().top, 0, viewportSize.height);
		...
		if (typeof resizeValueFunc === "function") {
			...
			newSize = resizeValueFunc(meSt.playerExpandedSize, viewportSize);
		}
		else {
			...
			newSize = {width: meSt.playerExpandedSize.width * sizeValueFloat,
								 height: meSt.playerExpandedSize.height * sizeValueFloat};
		}
		...
		videoDiv.style.width = "100%"; // Overrides the fixed YT_CONTENT_WIDTH width of the video div in YouTube's CSS to allow the player div to expand beyond that
		playerDiv.style.width = newSize.width.toCSS();
		playerDiv.style.height = newSize.height.toCSS();

		// The page is scrolled to the player div (or the headline div if the passed video size is "controls" or "fill") if configured to do so. The resize button is also updated
		if (doScroll) scrollToNode(((sizeValue === "controls") || (sizeValue === "fill")) ? headlineDiv : playerDiv);
		updateResizeButtonContent();
	...
	// Sets a listener to restore the last used video size if the playlist is minimized/maximized (the playlist uses vertical space, so it's taken into account when some video sizes are calculated)
	var playlistDiv = $(YT_PLAYLIST_ID);
	if (playlistDiv) {
		playlistDiv.addEventListener("DOMAttrModified", function(evt) {
			if (evt.attrName === "class") setTimeout(updateResizePlayer, 250); // A delay of 250 miliseconds is used to wait for the CSS transition used by the playlist to complete
		}, false);
	}
	...
	if (scriptConfig.removeRelatedVideos) delNode("watch-sidebar");
	...
	if (scriptConfig.removeSubscriptionReminder) {
		try {
			...
			GM_log("Error blanking promoteSubscribe: " + err.toString());
		}
		setYTConfig("SHOW_SUBSCRIBE_UPSELL", false); // Video end popup
	}
	...
	// Creates the script main div and its buttons and inserts it in the watch panel
	var watchPanel = $(YT_WATCH_PANEL_ID);
	if (watchPanel) {
		...
		var mainResizeMenu = createNode("div", {class: YT_MENU_CLASS + " " + YT_HIDDEN_CLASS, id: "ytf-main-resize-menu"}, null,
			[["click", function(evt) {
		...
		// The button is initially disabled if the YouTube API is not ready (it needs the API to work properly). The scriptWatchAPI function will enable it when the API is ready in that case
		var mainResizeButton = createDropdownYTButton("ytf-main-resize-button", "Resize", "Resize the video player", "ytf-main-resize-button-content", "ytf-main-resize-menu", !watchVA.isAPIReady);
		mainDiv.appendChild(mainResizeButton);

		// Creates the download menu div and appends it to the main div
		var mainDownloadMenu = createNode("div", {class: YT_MENU_CLASS + " " + YT_HIDDEN_CLASS, id: "ytf-main-download-menu"});
		if (watchVA.videoFormats.length !== 0) { // Adds the links to the menu if there is at least one VideoFormat in the VideoAdapter
			watchVA.videoFormats.values.sort(VideoFormat.qualitySorter()).forEach(function(vf) { // The VideoFormat objects are sorted in descending order (from highest to lowest quality)
		...
		var mainDownloadButton = createDropdownYTButton("ytf-main-download-button", "Download", "Download this video", "ytf-main-download-button-content", "ytf-main-download-menu", false);
		...
		// Inserts the main div as the first child element of the watch panel
		watchPanel.insertBefore(mainDiv, watchPanel.firstChild);

		// Restores the last used video size (without scrolling) if the API is ready to make sure the resize button caption is updated from its initial generic value (scriptWatchAPI will update it if the API isn't ready now)
		if (watchVA.isAPIReady) resizePlayer(null, false);

	}
	else {
		GM_log("Watch panel not found");
	}
...
function scriptChannelMain() {
	...
	// Tries to get the player's div or, if that fails, the player's root node (new channel layout)
	var playerDiv = $(YT_PLAYER_DIV_CHANNEL_ID);
	if (!playerDiv) {
		playerDiv = $x1("//div[starts-with(@class, '" + YT_PLAYER_ROOT_CHANNEL_NEW_CLASS + "')]");
		if (!playerDiv) {
			GM_log("Channel player div not found");
			return;
		}
	}

	// Adds a listener to call channelPlayerInsertion if the player is inserted in the page
	playerDiv.addEventListener("DOMNodeInserted", function(evt) {
		var targetId = evt.target.id;
		if ((targetId === YT_PLAYER_ID_FLASH) || (targetId === YT_PLAYER_CHANNEL_NEW_ID_FLASH) || (targetId === YT_PLAYER_CHANNEL_NEW_ID_HTML5)) channelPlayerInsertion(targetId);
	}, false);

	// Call chanelPlayerInsertion if the player is already in the page
	if ($(YT_PLAYER_ID_FLASH)) {
		channelPlayerInsertion(YT_PLAYER_ID_FLASH);
	}
	else if ($(YT_PLAYER_CHANNEL_NEW_ID_FLASH)) {
		channelPlayerInsertion(YT_PLAYER_CHANNEL_NEW_ID_FLASH);
	}
	else if ($(YT_PLAYER_CHANNEL_NEW_ID_HTML5)) {
		channelPlayerInsertion(YT_PLAYER_CHANNEL_NEW_ID_HTML5);
	}
	...
	function channelPlayerInsertion(playerId) {
		...
		switch(playerId) {
			...
			// Initializes the VideoAdapter object for the Flash player
			case YT_PLAYER_ID_FLASH:
			case YT_PLAYER_CHANNEL_NEW_ID_FLASH:

				// Creates a VideoAdapter object for the video. The relevant script configuration settings are passed to the function to be applied
				try {
					channelVA = new VideoAdapter(playerId, null, YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME, false);
			...
			// Initializes the VideoAdapter object for the HML5 player (incomplete support yet)
			case YT_PLAYER_CHANNEL_NEW_ID_HTML5:

				GM_log("HTML5 player support is incomplete yet");

				try {
					channelVA = new VideoAdapterHTML5(YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME);
				}
				catch(err) {
					GM_log(err.toString());
					return;
			...
			// Any other player id is invalid
			default:
				GM_log("Invalid player Id");
				break;
...
function scriptSearchMain() {
	...
	if (scriptConfig.downloadFromSearch) {
		...
		var videosDiv = $x("id('search-results')//div[@class='result-item-main-content'][not(ul[@class='playlist-videos'])]");
		...
		videosDiv.forEach(function(videoDiv) {
			...
			videoInfoXhr.onload = function(evt) {
				...
				var responseMatch = this.responseText.match(/^\s*yt\.playerConfig = ({.*});\s*$/im);
				...
				var badgeUL = $x1("./ul[@class='" + YT_SEARCH_BADGE_UL_CLASS + "']", videoDiv);
				if (!badgeUL) badgeUL = videoDiv.appendChild(createNode("ul", {class: YT_SEARCH_BADGE_UL_CLASS}));

				videoFormats.values.sort(VideoFormat.qualitySorter()).forEach(function(vf) {
