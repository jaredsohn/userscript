...
/*
 * evofr+
 * 2006/07/28 Updated for french interface by Toukan
 *
...
const scriptversionID = 'evofr+ ' + scriptversion;
...
function evoCreate(tableID) {
	...
	if( showstats ) {
		// column headers
		addHeader(4, 'Attaque/<br /><strong>Attaque<sup>2</sup></strong>/<br /><span title="Att2 pour 100,000 ressources (m\u00e9tal + min\u00e9ral)">pour 100K</span>');
		addHeader(5, 'Defence/<br /><strong>Defence<sup>2</sup></strong>/<br /><span title="Def2 pour 100,000 ressources (m\u00e9tal + min\u00e9ral)">pour 100K</span>');
		addHeader(6, 'Total/<br /><strong>Total<sup>2</sup></strong>/<br /><span title="Att2+Def2 pour 100,000 ressources (m\u00e9tal + min\u00e9ral)">pour 100K</span>');
	}
	...
	var youSure = function(e)
		{
			if(! confirm('Etes-vous sur de vouloir produire ces \u00e9l\u00e9ments/cr\u00e9atures?'))
				e.preventDefault();
		}
...
function evoOverview() {
	...
	node = document.evaluate(".//span[preceding-sibling::span[@class='t_medium b' and text()='Statut du continent']]",
	...
	if( node != null && (match = node.innerHTML.match(/(\d*?),(\d*?),(\d*?):\w/)) ) {
		...
		GM_log("Impossible de trouver vos coordonn\u00e9es :(", 1);

	// look for the fleets status table and show the ETA
	node = document.evaluate('.//div[@class="'+searchKey+'" and contains(., "Statut des flottes")]/following-sibling::div[1]/table', contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	if( node ) {
	...
	// Same for the R&D
	var nodes = document.evaluate(".//div[@class='alt2 t_little' and preceding-sibling::div[@class='"+searchKey+"' and starts-with(.,'D\u00e9veloppement')]]",
			contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	if( !dEvo ) {
		for( i = 0; node = nodes.snapshotItem(i); i++ ) {
			node.innerHTML = node.innerHTML.replace(/(\d+)% \((\d+)\/(\d+)\) termin.\./,
					function(str, p1, p2, p3, offset, s) {
						var minutes = (Number(p3) - Number(p2)) * tick;
						var eta = new Date(now.valueOf() + (minutes * 60000));
						return p1 + '% (' + p2 + '/' + p3 + ') termin\u00e9 - ETA: ' + evoFromatNumberZ(eta.getUTCHours(),2) + ":" + evoFromatNumberZ(eta.getUTCMinutes(),2) + " GMT" + (minutes > 1440 ? ' (+' + Math.floor(minutes/1440) + ' day' + ( minutes >= 2880 ? 's' : '' ) + ')' : '');
					});
		}
	}
	...
	for( i = 0; node = nodes.snapshotItem(i); i++ ) {
		// add a column
		node.insertCell(3);
		if( match = /\d+% termin.s, il reste (\d+) ticks/.exec(node.cells[2].textContent) ) {
	...
	var landtable = document.evaluate(".//table[@class='t_little' and parent::div[@class='alt2' and preceding-sibling::div[@class='separator title' and child::span[text()='Terrain']]]]", contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	...
	var per = Math.round(alloc/total*10000);
	per = (10000 - per)/100; // We just worked out the percentage allocated, so let's take it away from 100. This avoids a /0

	var elm = document.createElement('DIV');
	elm.className = 'separator title';
	elm.innerHTML = 'Vous avez actuellement '+per+'% de Terrain Inutilis\u00e9s';
...
var nbr =0; var qrty=0;
	...
	newCell.textContent = (nbr == 0) ? 'Il n\'y a aucun terrain \u00e0 allouer' :'Vous pouvez allouer un maximum de '+nbr+' terrain(s)';
	...
	if( resourcetable ) {
		...
		showAllocCost = function(e) {
			...
			elm.innerHTML = 'Les valeurs en <SPAN class="red">Rouge</SPAN> sont les couts en m\u00e9tal pour allouer le nombre de terrains definit sur l\'axe des x.';
			parent.appendChild(elm);
		};
		...
		elm.className = "title";
		span.innerHTML = 'Couts d\'allocation des Terrains inutilis\u00e9s';
...
function evoAllianceMembers() {
	...
	var memberTables = document.evaluate(".//div[@id='alliancememberlist']/table", contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	
	// if we have rank edit powers.. we need another snapshot
	if (memberTables.snapshotItem(0) == null){
var memberTables = document.evaluate(".//div[@id='alliancememberlist']/form/table", contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	}


	var nbr = 0; var snbr = 0;
	var totscore = 0; var stotscore = 0;
	// get the alliance's tag
	...
	for (var i = 0; memberTable = memberTables.snapshotItem(i); i++) {
		...
		for (var j = 1; j < memberTable.rows.length; j++){
			...
			if (isPrimary == true){ totscore = totscore + target; }else{ stotscore = stotscore + target;} 
			if( target < smin ){score.style.color = '#CC4411';}
			else if( target > tmax ){score.style.color ='#0000FF'; if (isPrimary == true){ nbr++; }else{ snbr++;} }
			else{score.style.color = '#00FF00'; if (isPrimary == true){ nbr++; }else{ snbr++;} }
	...
	div.innerHTML = '<div class="separator title">Il y a <span style="color:#00ff00;">'+nbr+'</span> Membres Primaire dans cette alliance ('+evoNumber2String(totscore)+')</div><div class="separator title">Il y a <span style="color:#00aaff;">'+snbr+'</span> Membres Secondaires dans cette alliance ('+evoNumber2String(stotscore)+')</div><div class="separator title">Un Total de '+evoNumber2String(nbr+snbr)+' Membres ('+evoNumber2String(totscore+stotscore)+')</div>' + div.innerHTML;
	...
	function onBuddy(e) {
		...
		if( count++ ) {
			alert("Wow... tranquille! Un seul ami \u00e0 la fois, svp?");
			--count;
			return;
		}
...
function evoUniverse() {
	...
	if( table != null ) {
		...
		var git = document.evaluate(".//table[tbody/tr[1]/td[1][text() = 'Clef']]", contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
		if( git != null ) {
			var d = git.insertRow(4);
			d.className = "lightblue_bg_row2";
			d.innerHTML = '<td>Couleurs des Textes:</td><td colspan="3" align="center"><font color="#FFFFFF">Les noms en Blanc</font> indique que la personne est actuellement conn\u00e9ct\u00e9<BR><font color="#C4C4C4">Les noms en Gris</font> indique que la personne est d\u00e9conn\u00e9ct\u00e9<BR><font color="#646464">Les noms en Gris Fonc\u00e9</font> indique que la personne ne s\'est pas conn\u00e9ct\u00e9 depuis au moins une semaine</td>';
			var c = git.insertRow(5);
			c.className = "lightblue_bg_row1";
			c.innerHTML = '<td>Couleurs des Score:</td><td colspan="3" align="center">Les joueurs avec ce <font color="#CC4411">score</font> peuvent vous attaquer, mais vous ne pouvez pas<BR>Les joueurs avec ce <font color="#00FF00">score</font> peuvent vous attaquer, et vous le pouvez aussi<BR>Les joueurs avec ce <font color="#0000FF">score</font> ne peuvent pas vous attaquer, mais vous pouvez :D</td>';
		}
	}
}
...
var land = document.getElementById('panelinfo').textContent.match(/Terrain:\s(\d+)/);
...
scantable.rows[3].cells[2].innerHTML = scantable.rows[3].cells[2].innerHTML + '<br /><span style="text-size:0.8em;color:#00ff00">'+ratio+'</span>';
...
function evoFleets(){
	...
	function updateFleetsAddRow(fleetsTable, isAttack){
		...
		updateFleetsAddCell(row, 0, "alt1 b", (isAttack ? "Attaque" : "Defense") + '<br /><span style="font-size:smaller; font-weight: normal">(estimation avec max boost)</span>');
...
//		row.cells[0].rowSpan = 12;
	...
	if( dEvo )
		fleetsTable = document.evaluate(".//table[preceding-sibling::h2[@class='forum' and text()='Organisation de la flotte']][1]",contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	else
		fleetsTable = document.evaluate(".//table[preceding-sibling::h2[@class='title' and text()='Organisation de la flotte']][1]",
										 contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	// add row for fleet counts
	var sumRow = document.evaluate(".//tr[td[position()=1 and text()='D\u00e9placer vers']]", fleetsTable, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
...
function evoNews() {
	...
	var RegExpTerm = (dEvo?dEvoExpTerm:".//h3[@class='cnewstitle' and child::span[starts-with(.,'Battle Report')]]/following-sibling::table");
...
function evoContRankings(){
	...
	var rankTable = document.evaluate(".//table[@class='t_little grey_bg']", contents, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
		...
		for (var j = 1; j < rankTable.rows.length; j++){
			...
			target = evoString2Number(score.textContent);
			if( target < smin ){score.style.color = '#CC4411';}
			else if( target > tmax ){score.style.color ='#0000FF';}
			else{score.style.color = '#00FF00';}
...
function evoNotepad() {
	...
	function clearNotepad() {
		...
		if (confirm('Vider le Pense-b\u00eate?')) {
			np.value = '';
			GM_setValue('notepad', '');
		}
	}
	...
	hDiv.style.backgroundColor = '#112288';
	hDiv.style.color = '#FFFFFF';
	hDiv.style.cursor = 'pointer';
	hDiv.innerHTML = 'Note rapide';
...
GM_registerMenuCommand( "Configurer les Boosts...", evoConfigBoosts, null, null, 'b' );
function evoConfigBoosts() {
	var n;
	if( (n = prompt("Boost pour les Cr\u00e9atures Naturelles ? (e.g. 39.46625)", boosts[UT_NATURAL]*100)) != null && !isNaN(n = Number(n)) )
		GM_setValue('boostNat', String(n/100));
	if( (n = prompt("Boost pour les Cr\u00e9atures Arch\u00e9ologiques ? (e.g. 33.126875)", boosts[UT_ENG]*100)) != null && !isNaN(n = Number(n)) )
		GM_setValue('boostEng', String(n/100));
	alert("Recharger la page pour que les changements s'effectuent.");
}
...
(function () {
	...
	// Initialization
        ...
        menuCC = document.getElementById("commandHeader");
        if( menuCC != null ) {
            s = menuCC.textContent;
            s = s.replace("Centre de commande"," Actions");
            menuCC.textContent = s;
        };

        menuCC = document.getElementById("expansionHeader");
        if( menuCC != null ) {
            s = menuCC.textContent;
            s = s.replace("Extension"," Croissance");
            menuCC.textContent = s;
        };
	...
	if( panel != null ) {
		match = panel.textContent.match(/M.tal:\s+([\d,]+)\s+Min.ral:\s+([\d,]+)\s+Nourriture:\s+([\d,]+)\s+Or:\s+([\d,]+)\s+Score:\s+([\d,]+)/);
	...
	if( panel != null ) {
		if( match = /Classement\s+actuel\s:\s+([,0-9]+)\s+sur\s+([,0-9]+)/.exec(panel.textContent) ) {
			pRank = match[1];
	...
	addItemToContent(node, 'Classement: ', 't_normal b', pRank + "/" + totalPlayers, true);
	addItemToContent(node, 'Score Min. Cible: ', 't_normal b', evoNumber2String(Math.ceil(pScore * 0.35)), true);
	addItemToContent(node, 'Score Max. Attaquant: ', 't_normal b', evoNumber2String(Math.floor(pScore / 0.35)), false);
...
function evoLayoutChanged() {
	alert('Oops.. La structure de page est inconnue, Neon a peut \u00eatre chang\u00e9 la page :(');
}
