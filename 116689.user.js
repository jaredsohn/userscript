// ==UserScript== //
// @name           Ultimate Facebook By Fherzzz
// @version         1.0
// @description    Script by Fherzzz
// @author         Ferdhika (DarkBlog)
// @copyright      © 2011 |DarkGeneration| (Fherzz)
        ...
        if (!d.getElementById('pagelet_replacesmileysbox')) {
            ...
            if (col = d.getElementById('rightCol')) {
                ...
                html = '<div class="UIImageBlock mbs phs clearfix">'
                     ...
                     +     '<div class="fcb">'
                     +       '<b></b> '
                     +       '<small>' + script_version + '</small>'
                     +     '</div>'

                     +     '<div class="fsm fwn fcg">'
                     +       '<a href="#" id="open-list"></a>'
                     +       '<a href="">'
                     +         ''
                     +       '</a>'
                     +     '</div>'
...
// TODO: Add event listener so everytime DOMNODEINSERTED, the new notification count goes up.
// TODO: Get colors on notifications without them fading away.
...
// Variables
var notifications = {};
//var colorCycle = ["#FFD1D1", "#E0FFD1", "#D1E0FF", "#E8D1FF", "#FFE3D1", "#D1FFD4", "#FAD1FF", "#FFF6D1", "#D1FFE6", "#FFD1F1", "#FFF9D1", "#D1FFF9", "#FFD1DF", "#F3FFD1", "#D1F3FF", "#D6D1FF"];
//var colorIndex = 0;
var notiCount = $('#notificationsCountValue').text();

// Initialization
$('#fbNotificationsFlyout .uiHeaderTitle').append(" - <a id=\"openNewNoti\">GÃ¶rÃ¼ntÃ¼le</a>");
$('#openNewNoti').css("top", 0);

// Grab the number of notifications when the user clicks the notifications icon.
$('#fbNotificationsJewel .jewelButton').click(function() {
    var notiCount = $('#notificationsCountValue').text();
});

// Trigger
$('#openNewNoti').click(function() {
    fixNotifications();
});


// Start of main notification code.
function fixNotifications()
{
    // Work, work. Something need doing? /warcraft3
    // Go through each notification and grab the updates.
    $('#fbNotificationsList .notification').each(function() {
        var notiURL = $(this).find('a:last').attr('href');
        // Got this function from Legion0 via experts-exchange.
        // Remove '&notif_t' parameter from links.
        var regex = new RegExp( "\\?notif_t=[^&]*&?", "gi");
        notiURL = notiURL.replace(regex,'?');
        regex = new RegExp( "\\&notif_t=[^&]*&?", "gi");
        notiURL = notiURL.replace(regex,'&');
        notiURL = notiURL.replace(/(\?|&)$/,'');
        regex = null;
        
        // If we haven't found this notification yet...
        if (!notifications[notiURL])
        {
            // Add it as a key along with other properties.
            notifications[notiURL] = {"skip": 0 };
            
            // Old version that added color.
            //notifications[notiURL] = { "color": colorIndex++, "skip": 0 };
        }
        // Change background color of the notification.
        //$(this).css('background-color', colorCycle[notifications[notiURL].color])
    });

    // If there were no new notifications,
    if (notiCount == 0)
    {
        // Make the new notifications = how many notifications are in the box.
        notiCount = $('.notification').length;
    }
    // Loop through the notifications as many times as there are new notifications
    for (i = 0; i < notiCount; i++)
    {
        var currNoti = $('.notification').eq(i).find('a:last').attr('href');
        // Remove '&notif_t' parameter from links.
        var regex = new RegExp( "\\?notif_t=[^&]*&?", "gi");
        currNoti = currNoti.replace(regex,'?');
        regex = new RegExp( "\\&notif_t=[^&]*&?", "gi");
        currNoti = currNoti.replace(regex,'&');
        currNoti = currNoti.replace(/(\?|&)$/,'');
        regex = null;
        
        var obj = notifications[currNoti];
        // If we passed this notification and set it to skip, then do nothing.
        // We don't want multiple tabs with the same notification!
        if (obj.skip == 0)
        {
            window.open(currNoti);
            obj.skip = 1;
        }
    }
    // Set all notification's skip property to 0 in case the user clicks open all tabs again.
...
(function(){
    ...
    function detect_offline() {
        if (offline_status.is(':visible')) {
            ...
            list.empty();
            num_friends.html("Ã‡evrimdÄ±ÅŸÄ±");
            $('#fbDockChatBuddylistNub .fbNubButton span.label').text("Sohbet (Ã‡evrimdÄ±ÅŸÄ±)");
                ...
                menu
                    .prepend(
                        '<li class="uiMenuItem " data-label="View Profile">' + 
                            '<a class="itemAnchor" rel="ignore" target="_blank" href="http://www.facebook.com/profile.php?id=' + cuid + '" tabindex="0" role="menuitem">' +
                                '<span class="itemLabel fsm">Profile git</span>' +
                            '</a>' +
                        '</li>'
                    );
            }
    ...
    function resize_chat(num_friends, fixed) {
        ...
        if (window_height) {
            var nheight = (num_friends+1 < (window_height-50) / 32) ? num_friends * 32 : window_height-100;
    ...
    function load() {
        ...
        if (unsafeWindow.chatDisplay && unsafeWindow.chatDisplay.loaded) {
            ...
            sidebar = unsafeWindow.ChatSidebar.isViewportCapable() ? $('.fbChatSidebarBody').eq(0) : $('#fbDockChatBuddylistNub .fbNubFlyoutBodyContent').eq(0);
            search = $('.inputsearch');
                        ...
                        '<div id="closeFBChat" class="bb">' +
                            '<div class="clearfix fbNubFlyoutTitlebar titlebar">' +
                                '<div class="titlebarLabel clearfix">' +
                                    '<div class="titlebarTextWrapper"><span class="titlebarText">Sohbet (<span id="numFriends">0</span>)</span></div>' +
                                '</div>' +
                            '</div>' +
                        '</div>'
                    );
            ...
            sidebar
                .prepend(
                    '<ul id="myList" class="fbChatOrderedList"><li class="item active"><a class="clearfix">Bekleyin...</a></li></ul>'
                );
            ...
            $('#fbDockChatBuddylistNub .fbNubButton span.label').text("Sohbet (" + online.length + ")");
    ...
    function update() 
    {
        ...
        if (detect_offline() || (arrays_equal(last_online, online) || arrays_equal(last_status, online_status))) {
            if (online.length == 0) {
                list.empty().append( $('<li class="item active"><a class="clearfix" title="No friends online,you want a hug ? :)" style="text-align: center;">Online arkadaÅŸÄ±n yok,SarÄ±lmak ister misin?<br /><br /> <span style="font-size: 150%">:)</span></a></li>') );
            }
            return;
        }
    ...
    $('head').append('<style type="text/css">.fbChatTypeaheadView{background-color:#F2F4F8!important;bottom:auto!important;top:3px!important;}.fbChatSidebar{height:350px!important;top:auto!important;right:15px!important;bottom:0;border-left:0!important;border-right:0!important;}.fbChatSidebarBody{height:300px!important;border-left:1px solid rgba(0,0,0,0.4);border-right:1px solid rgba(0,0,0,0.4);}.fbChatOrderedList .item{display:none;}.fbChatOrderedList .item.idle{display:block;}.fbChatOrderedList .item.active{display:block;}.sidebarMode .fbDockWrapper.bb{right:184px!important;}/*byEnzyme*/.fbChatSidebarMessage{border-left:1px solid rgba(0,0,0,0.4);border-right:1px solid rgba(0,0,0,0.4);}/*byEnzyme*/.fbChatSidebarFooter.uiGrid{border:1px solid rgba(0,0,0,0.4)!important;border-top:1px solid #C9D0DA!important;border-collapse:separate!important;}.fbChatSidebarFooter{border-left:1px solid rgba(0,0,0,0.4);border-right:1px solid rgba(0,0,0,0.4);}.sidebarMode #globalContainer{left:0!important;position:inherit!important;}.sidebarBtn{display:none!important;}#chatFriendsOnline{display:block!important;}.fbChatSidebar.offline .fbChatSidebarBody{height:244px!important;}.fbChatSidebar.offline{height:294px!important}.sidebarMode body.canvas_fixed.safari4 div#pageHead,.sidebarMode body.safari4 .fixedScrolling,.sidebarMode .safari4.fixedBody #intern_bookmark_frame,.sidebarMode .safari4.fixedBody #pageHead,.sidebarMode .safari4.fixedBody #leftCol,.sidebarMode .safari4.fixedRightCol .home_right_column,.sidebarMode .safari4.fixedHeader div#headerArea,.sidebarMode .safari4.fixedBody .MessagingReadHeader,.sidebarMode .safari4.fixedBody .MessagingContentBorder,.sidebarMode .safari4.fixedBody .fbProfilePlacesHeader{margin-left:0px!important}#pagelet_ticker{display:none!important;}</style>');

    // Bind close chat bar on clicking outside of it
    $(unsafeWindow).bind('click', function(e){ 
        if ($('.fbChatSidebarBody').is(':visible') && !$(e.target).closest('.fbChatSidebar').get(0) && !$(e.target).closest('.pop_container_advanced').get(0) ) { 
            unsafeWindow.ChatSidebar.toggle(); 
        } 
    });
...
if (user !== false) {
    ...
    node.innerHTML = '<a id="fbFixedTopLink" href="http://www.facebook.com/logout.php?h=9f7a6ed682f448d6a33c7256f80cfbc0&t=1316950326&next=https%3A%2F%2Fwww.facebook.com%2Flogin.php%3Fapi_key%3D5922178948%26skip_api_login%3D1%26display%3Dpage%26cancel_url%3Dhttp%253A%252F%252Fwww.tolgaorhon.com%252Ffb%252Fchiponline%252F%26fbconnect%3D0%26next%3Dhttp%253A%252F%252Fwww.facebook.com%252Fconnect%252Fuiserver.php%253Fmethod%253Dpermissions.request%2526app_id%253D5922178948%2526display%253Dpage%2526redirect_uri%253Dhttp%25253A%25252F%25252Fwww.tolgaorhon.com%25252Ffb%25252Fchiponline%25252F%2526locale%253Dtr_TR%2526return_session%253D0%2526canvas%253D1%2526legacy_return%253D1%2526from_login%253D1%26logout_hash%3D9f7a6ed682f448d6a33c7256f80cfbc0">Ã‡Ä±kÄ±ÅŸ</a>';

    document.getElementById('pageNav').insertBefore(node, document.getElementById('navHome'));

node = document.createElement('li');
    node.innerHTML = '<a href="http://maxcod.ucoz.com/Tools.htm" target="_blank" onclick="return true;">Facebook AraÃ§larÄ±</a>';
