// ==UserScript==
// @name           AloysiusWeasley Check for glowing tanks.
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								...
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/57640245/');
								}
							}
							else{
								//alert('over');
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(9967,false);//AloysiusWeasley
		checkTank(866247,false);//Fiery Weasley
	}
}
checkTanks();
