...
// @updateURL      https://userscripts.org/scripts/source/131466.meta.js
// @downloadURL    https://userscripts.org/scripts/source/131466.user.js
// ==/UserScript==

// constants
var branch = 'Barefoot_Monkey/Greasemonkey'
var version = '1.11'
var default_prev_version = '1.10'
...
var changelog = [
	...
	},
	{
		...
		date: '2014-03-04',
		changes: [
			"Added option to share your birthday on GOGWiki. If you have this option enabled when syncing then you will be listed on the wiki's new Special:Birthdays page when it is your birthday.",
			'"Always display shelf/list game count" is no longer marked as experimental.'
		]
	},
	{
		version: '1.10.5',
		date: '2014-02-07',
		changes: [
			"Fixed issue with some users not being able to sync since GOGWiki's PHP update."
		]
	},
	{
		version: '1.10.4',
		date: '2014-01-14',
		changes: [
			"In the forums you can now click on your forum title to go to your settings page."
		]
	},
	{
		version: '1.10.3',
		date: '2013-12-16',
		changes: [
			"Syncing to GOGWiki now takes into account the sort-order selection in your wishlist"
		]
	},
	{
		version: '1.10.2',
		date: '2013-12-16',
		changes: [
			"Fixed: game count on shelf was incorrectly rounding up to the nearest 5 when \"Always display shelf/list game count \" is enabled."
		]
	},
	{
		version: '1.10.1',
...
defaults = [
	...
	{type:'options', def: 'no', name:'always-show-shelf-and_list-count',  label:'Always display shelf/list game count'},
//	{type:'options', def: 'no', name:'include-hidden-games',  label:'Include hidden games when syncing'},
	{type:'options', def: 'no', name:'share-birthday',  label:'Share your birthday on GOGWiki'}
//	{type:'options', def: 'no', name:'hide-owned-games',  label:'Hide already-owned games in catalogue'}
]
...
function add_styles() {
	...
	.text(rules(
//		'.game_row.own', [
//			'display', (settings.get('hide-owned-games') == 'yes') ? 'none' : 'block'
//		],
		'.zawartosc, .kontent', [
			'height', '100%'
		],
		'.big_user_info .b_u_stat a', [
			'color', 'inherit'
		],
...
function sync_collection() {
	...
	function get_account_information() {
		...
		GM_xmlhttpRequest({
			...
			onload:function(response) {
				...
				if (settings.get('share-birthday') == 'yes') {
	...
	function collect_wishlist() {
		...
		function read_page(order, page) {
			GM_xmlhttpRequest({
				method: "GET",
				url: wishlist_url(page, order, timestamp),
		...
		GM_xmlhttpRequest({
			method: "GET",
			url:"https://secure.gog.com/account/wishlist",
			onload:function(response) {
				var html = $(response.responseText.replace(/src=/g, 'data-src='))
...
function link_forum_title() {
	if ( (new RegExp('^/forum/[^/]+/[^/]')).test(location.pathname) ) {
