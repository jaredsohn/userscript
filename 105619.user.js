// ==UserScript==
// @name            Language pack - Sample - Russian
...
twpro_lp_custom = {
	info: ['PrAIZeR', 'mailto:praizer@ya.ru', 4343, '.w1.'],     /*['Your name', 'http://your_url OR mailto:your_email', char_id (the character_id can be seen in your browser status bar by pointing your name on the ranking pages), '.fr1.w1.en12.de3. (the subdomain(s) of the world(s) you play. Each subdomain must be always preceded AND followed by a dot)']. Email/url and char_id+worlds are optional. If you mention your email/url address, other players will be able to see it on worlds you don't play. If you mention your char_id & world subdomains, other players will be able to open your InGame profile on worlds you play. Just leave these parameters blank if you don't want to use them.*/
		...
		SORTBYCOMB: 'Сортировать по <b>рабочему рангу</b>',     /*"Order activity list by job rank" button at the Inventory. The player can define the importance of experience, wages, luck and danger, which is called "Job rank"*/
		...
		FILTERJOBS: "Скрыть работы которые не доступны",     /*This message is shown in a tooltip when hovering over a checkbox which is available in the "Hide activities and itemsets" window. This checkbox is used to toggle the setting for hiding activities which the player cannot do (labor points is negative)*/
		FILTERCLOTHING: 'Показывать лучшую одежду для этой работы',     /*This text is shown in a tooltip when hovering over a checkbox at the Inventory. If checked, the best items for the selected activity is shown with all other items hidden*/
		CHOOSEJOB: 'Выберите работу...',     /*If TW Pro has calculated the activity list, this text is shown when no activity is selected*/
		CALCJOB: 'Пересчет очков, подождите...',     /*If TW Pro is calculating the activity list, this text will be shown in the activity list drop-down box*/
		...
		FBHEALTHPRIORITYZERO: "0| None",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYLOW: "1| Low",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYMEDIUM: "2| Medium",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYHIGH: "3| High",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBHEALTHPRIORITYAUTO: "4| Auto",     /*Visible in the "Preferences" tab. Applies to the "Health points priority" menu.*/
		FBBATTLEUNIT: "Battle unit type",     /*Visible in the "Preferences" tab.*/
		FBBATTLEUNITSKIRMISHER: "1| Skirmisher (dodging) | Front line",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
		FBBATTLEUNITLINEINFANTRYMAN: "2| Line infantryman (polyvalent) | Middle line",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
		FBBATTLEUNITMARKSMAN: "3| Marksman (aiming) | Rear line",     /*Visible in the "Preferences" tab. Applies to the "Battle unit type" menu.*/
		...
		CACHEEMPTYSLOWDESC : "You can still play during the process.",     /*Popup visible in the prompt box displayed when user wants to delete the Webstorage cache.*/
