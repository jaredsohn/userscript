...
var userId=url.split('?uid=')[1];
	...
	if(url.indexOf('CommMembers?')>=0){
		...
		for (var xi = 0; xi < ancrs.length ; xi++){
			if(ancrs[xi].href.indexOf("Main#Profile?uid=")>=0&&ancrs[xi].innerHTML.indexOf("<img")==-1){
					prfls[j]=ancrs[xi].href.split('?uid=')[1];
					console.log(ancrs[xi].href.split('?uid=')[1]);
					window.open("http://www.orkut.com.br/Main#FriendAdd?rl=lo&uid="+ancrs[xi].href.split('?uid=')[1]);
			}
			j++;
