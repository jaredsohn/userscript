// ==UserScript==
// @name          Twitter Friends Bio at a Glance
// @namespace     http://www.imusicmash.com
// @description   See complete friends and follower bio on the Twitter following/follows list
...
// 2.6 updated 11/28/09 - added time and Tweet source
// 2.5 update 11/21/09 - fixed most recent tweet regexp
...
function getBio(userTwitterURL, userSpanElem) {
    ...
    // recent tweet, regexp updated 6/27/09, 9/6/09, 10/31/09, 11/21/09    
    var recentTweetRegExp = /<span class="entry-content">(.*?)<\/span>/; 
    var recentTweetTimeRegExp = /<span class="published timestamp".*?">(.*?)<\/span>/;
    var recentTweetFromRegExp = /<span>from <a.*?">(.*?)<\/a><\/span>/;  
    ...
    onload: function(results) {
        ...
        if ( recentTweetRegExp.test(page) ) {        
            ...
            recentTweetText = '<br><b>Most Recent Tweet</b><br>' + recentTweetMatch[1];                 
        }
        
        if ( recentTweetTimeRegExp.test(page) ) {
            time = page.match(recentTweetTimeRegExp)[1];            
            if (recentTweetFromRegExp.test(page)) {
                from = page.match(recentTweetFromRegExp)[1];
            } else {
            	from = "web";
            }           	
            recentTweetTimeFrom = "<span style='font-size:x-small;'><br><b>Posted</b> " + time + " from " + from + "</span";
        }
                
        userSpanElem.setAttribute("style","padding-bottom:12px;padding-top:5px;line-height:1.5; color:black; font-size:small");      	        	
        userSpanElem.innerHTML = bioText + followerText + followingText + recentTweetText + recentTweetTimeFrom;
    }
    });
}	
