...
// @name		Travian: Resource++ v4 Cakens Version
// @namespace	http://userscripts.org/
// @description	Travian: Resource++ v4 Cakens version upravuje veci dle Cakena  
...
function onLoad(){ // runs the differnt functions and procedures, 
	...
	if(side){
		var opt=[];
		opt.push( makeEventlink('++ Nastavení', '#', showSettings) );
...
function toolMenu(vis){
		...
		con.style.height='15px';
		con.style.cssFloat='left';
		con.appendChild( document.createTextNode(  'Vyhledávání vesnic/hráčů' ) );
...
var button=document.createElement( 'input' );
	...
	formpost.appendChild( document.createTextNode( 'Souřadnice: ')  );
...
var button=document.createElement( 'input' );
	...
	button.name='s1';
	button.style.marginLeft='10px';
	formpost.appendChild( document.createTextNode( 'Jméno hráče: ')  );
...
function quickLinks(vis){
	...
	var con=document.createElement('div');
		...
		con.style.height='15px';
		con.style.cssFloat='left';
		con.appendChild( document.createTextNode(  'Navigátor' ) );
	...
	var onSelect=document.createElement('select');
		...
		onSelect.appendChild(makeOption('Armáda','#'));
		onSelect.appendChild(makeOption('Shromaždiště','build.php?id=39'));
		onSelect.appendChild(makeOption('Poslat jednotky','a2b.php?s=4'));
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change',function(e){jump(e);},true);
		onSelect.appendChild(makeOption('Budovy','#'));
		onSelect.appendChild(makeOption('Tržiště','build.php?gid=17'));
		onSelect.appendChild( makeOption('Kasárna','build.php?gid=19' ));
		onSelect.appendChild( makeOption('Stáje','build.php?gid=20' ));
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Aliance','#' )      );
		onSelect.appendChild( makeOption('Přehled','allianz.php' ) );
		...
		onSelect.appendChild( makeOption('Správa','allianz.php?s=5' ) );
...
function pop(){
	...
	row=document.createElement('tr');
	cell=document.createElement('td');
	cell.appendChild(makeEventlink('Skryj nastavení', '#', toggle));
	cell.setAttribute('colspan',2);
	row.appendChild( cell );
	fulltable.appendChild( row );
