// ==UserScript==
// @name          tribalwars-do7an.ucozreport- www.do7an.ucoz.com
...
if (location.href.match(/screen=settings&mode=settings/))
{
	...
	spalten[ispalte].innerHTML = "حرب قبائل - do7an.ucozreport- www.do7an.ucoz.com-abkarino";
	...
	berichtelink.href ="#";
	berichtelink.innerHTML = "<br>do7an.ucozreportبداية جديدة لي";
...
{
	if (confirm("يجب اعدد خيارت  do7an.ucozreport الان يرجاء الضغط على ok الى استمرار"))
	{
		Konfigurieren();
		if (confirm("مبارك تم اكتمال تهيات do7an.ucozreport اطيب التمنيات مني انا (abkarino)و من فريق عمل منتديات دخان :) http://do7an.ucoz.com/forum.php"))
		{
		}
	}
}
...
{	
	...
	spalte1.innerHTML = "<b>" + (erfassteunterstuetzungsberichte.indexOf(berichtsid) != -1 ? "Unterstützter Spieler:" : "المهاجم(do7an.ucoz.com):") + "</b>";
...
function ErstelleVerteidigerTabelle(berichtsid)
{
	...
	spalte1.innerHTML = "<b>" + (erfassteunterstuetzungsberichte.indexOf(berichtsid) != -1 ? "Einheiten:" : "المدافع(do7an.ucoz.com):") + "</b>";
	spalte2.innerHTML = "<b>" + GM_getValue(gmbase + "Name", "") + "</b>";
