// ==UserScript==
// @name           برنامه تجهیز خودکار
// @namespace      تجهیز
// @description    به سفارش برادر عزیزم امید جون! 
// @grant		   none
...
function main() {
    if (window.location.hash) {
        ...
        // #i2q5a10
        var pattern = /^#i([0-9]+)q([0-9]+)a([0-9]+)$/
        var result = hash.match(pattern)
        if (result !== null) {
            var industry = result[1]
            var quality = result[2]
            var amount = result[3]
            var input = jQ(document).find('input[industry="'+industry+'"][quality="'+quality+'"]')
            if (input.length === 1) {
