// IMDB Jefferson Lookup 1.0
// Sean LeBlanc

// This puts a link into a search on Jefferson lib from the movie page.
...
// select "IMDB - Jefferson Lookup 1.0", and click Uninstall.
// It also tries to detect when looking at a CD or DVD, and inserts a search link into Jefferson library for the title text.
//
// ==UserScript==
// @name          IMDB - Jefferson Lookup 1.0
// @namespace     http://sean-leblanc.blogspot.com
// @description	  While viewing a title on IMDB, it will place a link to a search for said title in Jefferson.
...
var dbg = false;
//var jeffcoLibraryName = 'Jefferson Public Library';
//var shortLibName = "Jefferson";
...
if (!GM_xmlhttpRequest || !GM_log) {
    alert('The Jefferson County Public Library Lookup script requires Greasemonkey 0.5.3 or higher.  Please upgrade to the latest version of Greasemonkey.');
    return;
}
...
if (titleLocation) {
	...
	url = mediaUrl + title + dvdTypeUrlAppend;
	insertItemSearchText(itemAvailabilityDiv, "Jefferson Library", url, itemType);
}
else {
