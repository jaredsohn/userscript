...
var ScriptAutor='msimic';
var ScriptVersion='5.1.2 +';
var ScriptLink='http://userscripts.org/scripts/show/33894';
...
switch(lang){
	...
	case ".it":
		...
		break; 
	case ".com.hr":
		langfile=["Napad","Gubitci","Obrana od pješadije",
							"Obrana od konjice","Statistike","Razlika gubitaka","Potrošnja",
							"XP heroja","Kapacitet","Gubitak u hrani","Potrošnja hrane",
							"Bez heroja","Heroj","Plijen","Nosivost",
							"Napadač","Branitelj","Dobitak","Efikasnost"];
		break; 
	case ".com.tr":
...
function main(){
	...
	// End Warsim
		...
		//
		saveBeta = tab.snapshotItem(0).childNodes[0].childNodes[1].innerHTML + "\t" + tab.snapshotItem(0).childNodes[0].childNodes[3].innerHTML;
		saveBeta += "\n";
		saveBeta += tab.snapshotItem(0).childNodes[2].childNodes[1].innerHTML + "\t" + tab.snapshotItem(0).childNodes[2].childNodes[3].textContent;
		saveBeta += "\n";
		saveBeta += "\n";
		
		saveBeta += tab.snapshotItem(1).childNodes[0].childNodes[1].innerHTML + "\t" + tab.snapshotItem(1).childNodes[0].childNodes[3].textContent;
		saveBeta += "\n";
		
		var tr_units=find("//tr[@class='unit']",XPList);
		for( var i = 2; i < tr_units.snapshotItem(0).childNodes.length; i++) {
			  if (tr_units.snapshotItem(0).childNodes[i].nodeName == "TD") {
			  	
					var un_node = tr_units.snapshotItem(0).childNodes[i];
					if (un_node != null) {
						for (var x = 0; x<un_node.childNodes.length; x++) {
							var un_n_html = un_node.innerHTML;
							var un_start = un_n_html.indexOf("title=\"");
							var un_end = un_n_html.lastIndexOf("\"");
							saveBeta += "\t[" + un_n_html.substring(un_start+7,un_end) + "]";
						}
					}
			...
			saveBeta += "\n";
			for( var i = 0; i < tab.snapshotItem(1).childNodes[3].childNodes.length; i++) {
				if (isNaN(tab.snapshotItem(1).childNodes[3].childNodes[i].innerHTML)) {
					saveBeta += tab.snapshotItem(1).childNodes[3].childNodes[i].innerHTML;
				}
				else {
					saveBeta += "\t" + tab.snapshotItem(1).childNodes[3].childNodes[i].innerHTML;
				}
			...
			saveBeta += "\n";
			for( var i = 0; i < tab.snapshotItem(1).childNodes[4].childNodes.length; i++) {
				if (isNaN(tab.snapshotItem(1).childNodes[4].childNodes[i].innerHTML)) {
					saveBeta += tab.snapshotItem(1).childNodes[4].childNodes[i].innerHTML;
				}
				else {
					saveBeta += "\t" + tab.snapshotItem(1).childNodes[4].childNodes[i].innerHTML;
				}
			...
			saveBeta += "\n";
			for (var y = 5; y<tab.snapshotItem(1).childNodes.length; y++) {
				for( var i = 0; i < tab.snapshotItem(1).childNodes[y].childNodes.length; i++) {
						saveBeta += tab.snapshotItem(1).childNodes[y].childNodes[i].textContent;
				}
				saveBeta += "\n";
			}
		...
		var tr_units=find("//tr[@class='unit']",XPList);
		//nazivi vojske
		for (var si = 1; si < tr_units.snapshotLength; si++) {

			saveBeta += tab.snapshotItem(si+1).childNodes[0].childNodes[1].innerHTML + "\t" + tab.snapshotItem(si+1).childNodes[0].childNodes[3].textContent;
			saveBeta += "\n";

			for( var i = 2; i < tr_units.snapshotItem(si).childNodes.length; i++) {
				
				  if (tr_units.snapshotItem(si).childNodes[i].nodeName == "TD") {
				  	
						var un_node = tr_units.snapshotItem(si).childNodes[i];
						if (un_node != null) {
							for (var x = 0; x<un_node.childNodes.length; x++) {
								var un_n_html = un_node.innerHTML;
								var un_start = un_n_html.indexOf("title=\"");
				...
				saveBeta += "\n";
				// deff
				for( var i = 0; i < tab.snapshotItem(si+1).childNodes[3].childNodes.length; i++) {
					if (isNaN(tab.snapshotItem(si+1).childNodes[3].childNodes[i].innerHTML)) {
						saveBeta += tab.snapshotItem(si+1).childNodes[3].childNodes[i].innerHTML;
					}
					else {
						saveBeta += "\t" + tab.snapshotItem(si+1).childNodes[3].childNodes[i].innerHTML;
					}
				...
				saveBeta += "\n";
				//gubici
				if (tab.snapshotItem(si+1).childNodes[4]) {
					for( var i = 0; i < tab.snapshotItem(si+1).childNodes[4].childNodes.length; i++) {
						if (isNaN(tab.snapshotItem(si+1).childNodes[4].childNodes[i].innerHTML)) {
							saveBeta += tab.snapshotItem(si+1).childNodes[4].childNodes[i].innerHTML;
						}
						else {
							saveBeta += "\t" + tab.snapshotItem(si+1).childNodes[4].childNodes[i].innerHTML;
						}
