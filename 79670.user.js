// ==UserScript==
// @name          pseudopod wikify
// @namespace     *
// @description   creates an alert with the entry for http://en.wikipedia.org/wiki/List_of_Pseudopod_episodes
// @include       http://pseudopod.org/*/*/*/*/
// ==/UserScript==
...
function stripHTML (input){
	...
	input = input.slice(1, input.search('</a>'));
	...
	var html = editHTML.cut('<a class="post_title" href="', '"', DELETE_START);
 	var number = editHTML.cut('>Pseudopod ', ':', DELETE_START);
 	var title = editHTML.cut(1, '</a>', DELETE_START);
	var author = editHTML.cut('By ', /<\/[s,b]/, DELETE_START);
	var narrator = editHTML.cut('Read by ', '</p>', DELETE_START);

	// cannot find 'Standard Podcast [', perhapse because of invisible characters.  cut characters after '&nbsp;' instead.
  editHTML.cut('&nbsp;', 24);
	var time = editHTML.cut(0, 5);

  var wiki = '|-\n| ' + number + ' || [' + html + ' ' + title + '] || [[' + stripHTML(author) + ']] || [[' + stripHTML(narrator) + ']] || ' + time + ' || [ forum]\n';
