 ...
 ********************************************************************************/
// Script information
var Version = '20111015a_BETA'; // Year, Month, Day, Revision, Maturity (e.g. YYYMMDDa_BETA)
...
/********************************************************************************
 * Util Tab                                                                     *
 * - Scan other players coordenates                                             *
 ********************************************************************************/
...
Tabs.Utiles = {
  ...
  init : function (div){
    ...
    t.cont = div;
    try {      
      m = '<DIV class=pbTitle style="margin-bottom:10px">Scanning function</div>\
      <TABLE width=100%>\
...
/***************************************************************************************************************/
  ...
  TropasDefiendan : function (){
    ...
    new MyAjaxRequest ('cities/'+ Seed.s.cities[0].id +'.json', {callback:'function%20Function%28%29%20%7B%7D', '%5Fmethod':'put', 'user%5Fid':C.attrs.userId, 'dragon%5Fheart':C.attrs.dragonHeart, '%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, timestamp:parseInt(serverTime()), defended:1}, function () {}, true);
  },

  TropasSantuario : function (){
    Tabs.Utiles.log('Tropas mandadas al Santuario');
    new MyAjaxRequest ('cities/'+ Seed.s.cities[0].id +'.json', {callback:'function%20Function%28%29%20%7B%7D', '%5Fmethod':'put', 'user%5Fid':C.attrs.userId, 'dragon%5Fheart':C.attrs.dragonHeart, '%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, timestamp:parseInt(serverTime()), defended:0}, function () {}, true);
  },
...
/***************************************************************************************************************/
  ...
  scanMapCircNew : function (x, y, radius, callback, doDetail){
    ...
    new MyAjaxRequest ('map.json', { 'user%5Fid':C.attrs.userId, x:t.firstX, y:t.firstY, timestamp:parseInt(serverTime()), '%5Fsession%5Fid':C.attrs.sessionId, 'dragon%5Fheart':C.attrs.dragonHeart, version:getVersion }, t.gotnew, false);
...
//  setTimeout (function(){new MyAjaxRequest ('map.json', {x:t.normalize(t.firstX+(t.curIX*15)), y:t.normalize(t.firstY+(t.curIY*15))}, t.gotnew);}, MAP_DELAY);
    setTimeout (function(){new MyAjaxRequest ('map.json', { 'user%5Fid':C.attrs.userId, x:t.normalize(t.firstX+(t.curIX*15)), y:t.normalize(t.firstY+(t.curIY*15)), timestamp:parseInt(serverTime()), '%5Fsession%5Fid':C.attrs.sessionId, 'dragon%5Fheart':C.attrs.dragonHeart, version:getVersion }, t.gotnew, false);}, (MAP_DELAY + + (Math.random() * 100)));
