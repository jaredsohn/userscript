...
END LICENSE BLOCK */
    ...
    var toCells;
    toCells = doc.evaluate("//td[@colspan='15']", doc.body, null,
                                   XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    if (live) {
          toCells = doc.evaluate("//td[@colspan=14]", doc.body, null,
                                       XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    }

    var toRegEx = /Total Team Turnovers \(Points off turnovers\):\s+(\d+)/

    var toHolderA = toCells.snapshotItem(0).textContent.match(toRegEx);
...
// 2013-11-11 - 0.8.3 - Updated to 2013 format, and to calculate FT/FG by attempts rather than made.