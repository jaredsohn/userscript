// ==UserScript==
// @name           Opera. luchshie veshi - ver2
...
window.bi2_l0 = (window.innerWidth) ? (window.innerWidth-window.bi2_w0)/2 : (1024-window.bi2_w0) /2 ;
window.bi2_t0=-5;
window.bi2_l1 = (window.innerWidth) ? (window.innerWidth-window.bi2_w1)/2 : (1024-window.bi2_w1) /2 ;
window.bi2_t1=140;


//высота окон
window.bi2_title_h_min=25;
window.bi2_title_h_mid=70;
window.bi2_title_h_max=275;
window.bi2_window_h_min=25;
window.bi2_window_h_max=400;
...
items[132] = {item_id:132, nshort:'bear', name:'Медвежонок', type:'left_arm', level:45, price:2600, image:'images/items/left_arm/bear.png', image_mini:'images/items/left_arm/mini/bear.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:'set_sleeper', name:'Набор сони'}, shop:'drop'};
...
bi2_vybvesch = function () {
	for (irabota in porabotaj) {
		...
		if (!rabota)
			{continue; }		if ((irabota>102)&&(irabota<120))
		    continue;
		...
		for (i = bi2_aktiv.length-1; i >= 0; --i) {
    		...
    		if (ochki > 0){
			    ...
			    if (iz) {
					...
					vyborka[ctype].simple.n += 1;
    				while (hm.length > 0){
    				    zh = hm.pop();    				    vyborka[ctype].simple.spisok[zh].tid = -1;
    				}
			        for (ll = vyborka[ctype].simple.n-1;ll>=0; ){
			            if (vyborka[ctype].simple.spisok[ll].tid== -1){
    		    			for (kk = ll; kk < vyborka[ctype].simple.n - 1; ++kk) {
    ...
    for (ii = ii_rekur; ii >= 0; )
    {
        ...
        if (deneg_ushlo <= bablo)
        {
		    ...
		    if (++ii == nn){
			    ...
			    --ii;
                deneg_ushlo -= ic_obj[ii].price;
                bi2_to -= ic_obj[ii].bon;    	    	if (ic_obj[ii].nabor) {
	    	    	bi2_predmetov[ic_obj[ii].nabor] -= 1;
	    	    }
	    	    i_slot[ii]+=1;
                continue;
...
my_name_is = function (){
	if (Character&&Character.name){
		...
		setTimeout(function() {if (bi2_abyrvalg.indexOf('window.')==0) {bi2_abyrvalg=bi2_abyrvalg.slice(8)};eval(bi2_abyrvalg)},500);
	}
	else{
...
window.bi2_tlink=' style=\"color:white; display:block; width:25px; height:20px; float:left;\" ';
window.bi2_vblock=' style=\"border:1px solid black; padding:1px; marging:1px;\" ';
window.bi2_title_flag=0;
window.bi2_title_flag2=1;
window.bi2_window_flag2=1;
window.bi2_odevalo4ka = false;
...
window.assign_citem = function (tid, obj){
	window.items[tid] = {item_id:tid, nshort:obj.short, name:obj.name, type:obj.type, level:obj.level, price:obj.price, image:obj.image, image_mini:obj.image_mini, characterClass:obj.characterClass, characterSex:obj.characterSex, speed:obj.speed, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};
    for (zz = window.bi2_vse_navyki.length-1;zz >=0;--zz){
        ss = window.bi2_vse_navyki[zz];
	    if (obj.bonus.skills[ss])
	        window.items[tid].bonus.skills[ss]=obj.bonus.skills[ss];
	}
    for (zz = window.bi2_vse_kharakteristiki.length-1;zz >=0;--zz){
        aa = window.bi2_vse_kharakteristiki[zz];
        if (obj.bonus.attributes[aa])
	        window.items[tid].bonus.attributes[aa]=obj.bonus.attributes[aa];
	}
	if (obj.set){
	    window.items[tid].set.key = obj.set.key;
	    window.items[tid].set.name = obj.set.name;
	}
}

window.compare_citem = function (tid, item){
	soft = true;
	hard = true;
	if (window.items[tid].item_id!=item.item_id) return;
	if (window.items[tid].nshort!=item.short){hard=false;window.items[tid].nshort=item.short};
	if (window.items[tid].name!=item.name){soft=false;window.items[tid].name=item.name};
	if (window.items[tid].type!=item.type){hard=false;window.items[tid].type=item.type}
	if (window.items[tid].level!=item.level){hard=false;window.items[tid].level=item.level}
	if (window.items[tid].price!=item.price){hard=false;window.items[tid].price=item.price}
	if (window.items[tid].image!=item.image){hard=false;window.items[tid].image=item.image}
	if (window.items[tid].image_mini!=item.image_mini){hard=false;window.items[tid].image_mini=item.image_mini}
	if (window.items[tid].characterClass!=item.characterClass){hard=false;window.items[tid].characterClass=item.characterClass}
	if (window.items[tid].characterSex!=item.characterSex){hard=false;window.items[tid].characterSex=item.characterSex}
	if (window.items[tid].speed!=item.speed){hard=false;window.items[tid].speed=item.speed}
    
    for (zz = window.bi2_vse_navyki.length-1;zz >=0;--zz){
        num_index = window.bi2_vse_navyki[zz];
        if (item.bonus.skills[num_index]&&window.items[tid].bonus.skills[num_index]){
            if (item.bonus.skills[num_index]!=window.items[tid].bonus.skills[num_index]){
                hard=false;
                break;
            }
        }
        else if (item.bonus.skills[num_index]||window.items[tid].bonus.skills[num_index]){
            hard=false;
            break;
        }
    }    
    for (zz = window.bi2_vse_kharakteristiki.length-1;zz >=0;--zz){
        num_index = window.bi2_vse_kharakteristiki[zz];
        if (item.bonus.attributes[num_index]&&window.items[tid].bonus.attributes[num_index]){
            if (item.bonus.attributes[num_index]!=window.items[tid].bonus.attributes[num_index]){
                hard=false;
                break;
            }
        }
        else if (item.bonus.attributes[num_index]||window.items[tid].bonus.attributes[num_index]){
            hard=false;
            break;
        }
    }    
	...
	if (!item.set){
	    if (window.items[tid].set.key){ hard=false;}
	}
	else{
	    if (item.set.key!=window.items[tid].set.key){
	        hard=false;
	    }
	    if (item.set.name!=window.items[tid].set.name){
	        soft=false;
	    }
	}
	res={h:hard,s:soft};
...
window.print_citem = function (tid){
	result='';
	result += 'items[' + window.items[tid].item_id + '] = {item_id:' + window.items[tid].item_id + ', nshort:\'' + window.items[tid].nshort;
	result += '\', name:\'' + window.items[tid].name + '\', type:\'' + window.items[tid].type + '\', level:' + window.items[tid].level;
	result += ', price:' + window.items[tid].price + ', image:\'' + window.items[tid].image + '\', image_mini:\'' + window.items[tid].image_mini + '\', characterClass:';
	cc = window.items[tid].characterClass ? '\'' + window.items[tid].characterClass + '\'' : null;
	result += cc + ', characterSex:';
	cs = window.items[tid].characterSex ? '\'' + window.items[tid].characterSex + '\'' : null;
	result += cs + ', speed:' + window.items[tid].speed;
	if (window.items[tid].bonus) {
		result += ', bonus:{skills:';
		ww = false;
		for (zz = window.bi2_vse_navyki.length-1; zz >=0; --zz ){
		    if (window.items[tid].bonus.skills[window.bi2_vse_navyki[zz]]) {
				if (ww) {
					result += ', '
				}
				else {
				...
				result += window.bi2_vse_navyki[zz] + ':' + window.items[tid].bonus.skills[window.bi2_vse_navyki[zz]];
			}
		}
		if (ww){
   			result += '}, '
		...
		ww = false;
		for (zz = window.bi2_vse_kharakteristiki.length-1; zz >=0; --zz ){
		    if (window.items[tid].bonus.attributes[window.bi2_vse_kharakteristiki[zz]]){
				if (ww) {
					result += ', '
				}
				else {
				...
				result += window.bi2_vse_kharakteristiki[zz] + ':' + window.items[tid].bonus.attributes[window.bi2_vse_kharakteristiki[zz]];
			}
		}
		if (ww){
		    result += '}'
	...
	if (window.items[tid].set.key) {
		result += 'set:{key:\'' + window.items[tid].set.key + '\', name:\'' + window.items[tid].set.name + '\'}'
	}
	else {
		result += 'set:{key:null, name:null}'
	}
	result += ', shop:\''+window.items[tid].shop+'\'};';
	return result;
}



window.bi2_iimport = function(){
	bagazh=window.Bag.getInstance();
	odezhda=window.Wear.wear;
	for(vv in bagazh.items){
	window.bi2_inv_imported=true;
		cobj = bagazh.items[vv].obj;
		tid = cobj.item_id;
		if (!window.bi2_uchet[tid]){
			window.bi2_uchet[tid]=true;
			if (window.items[tid]){
				var cres={h:null,s:null};
				cres=window.compare_citem(tid, cobj);
				if (!cres.h)	{
					window.einfo+='Часть данных о предмете неверна:\n';
					window.assign_citem(tid, cobj);
					window.einfo+=window.print_citem(tid)+'\n';
				}
				else if(!cres.s){
					window.winfo+='Неправильное название у предмета:\n';
					window.assign_citem(tid, cobj);
					window.winfo+=window.print_citem(tid)+'\n';
				}
			}
			else{
				window.items[tid] = {item_id:tid, nshort:'nothing', name:'Заглушка', type:'yield', level:0, price:0, image:'images/items/right_arm/clayjug.png', image_mini:'images/items/right_arm/mini/clayjug.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'nil'};
				window.assign_citem(tid, cobj);
				window.einfo+='Отсутствует описание предмета:\n';
				window.einfo+=window.print_citem(tid)+'\n';
			}
		}
	}
	for(vv in window.Wear.wear){
		if (!window.Wear.wear[vv])
			continue;
		cobj = window.Wear.wear[vv].obj;
		tid = cobj.item_id;
		window.bi2_equipment[vv]=tid;
		if (!window.bi2_uchet[tid]){
			window.bi2_uchet[tid]=true;
			if (window.items[tid]){
				var cres={h:null,s:null};
				cres=window.compare_citem(tid, cobj);
				if (!cres.h)	{
					window.einfo+='Часть данных у предмента неверна:\n';
					window.assign_citem(tid, cobj);
					window.einfo+=window.print_citem(tid)+'\n';
				}
				else if(!cres.s){
...
window.bi2_mimport = function(){
	magaz=window.TraderInventory.getInstance('bi2',0);
	if (!magaz) return;
	for(vv in magaz.items){
		cobj = magaz.items[vv].obj;
		tid = cobj.item_id;
		if (!window.bi2_khochuka[tid]){
		    if (!window.bi2_uchet[tid]){
		        window.bi2_khochuka[tid]=true;
		    }
	    	if (window.items[tid]){
				var cres={h:null,s:null};
				cres=window.compare_citem(tid, cobj);
...
window.bi2_podschet = function  (vse_veschi, iz_magazinov, plus_level, pers){
	window.bi2_aktiv=null;
	window.bi2_aktiv=[];
	for (vv in window.items){
		if (isNaN(vv)) continue;
		vesch=window.items[vv];
		check=true;
		...
		if (!window.bi2_uchet[vesch.item_id]&&!window.bi2_khochuka[vesch.item_id])
		{
			if (!vse_veschi){
				if (!iz_magazinov)
					check=false;
			...
			if ((vesch.shop=='shop')&&(vesch.price > window.bablo)) check=false;
		}
		...
		if (window.bi2_slots && window.bi2_slots[vesch.type]&&!(window.bi2_equipment[vesch.type]==vv)) check=false;
		if (check) window.bi2_aktiv.push(vesch.item_id);
	}
}

window.bi2_ocenka_khlama = function(){
    window.bi2_nenuzhnoe=[];
    if (!window.bi2_khlam)
        return;
    ispolz=[];
    for (irab=200; irab>0;irab--){
        if (window.raboty[irab]){
            if (window.resultaty[irab]){
                for (ee = 0; ee < window.bi2_types.length; ++ee){
                    sid = window.resultaty[irab].items[ee].tid;
                    ispolz[sid]=true;
                }
            }
            if (window.resultaty_z[irab]){
                for (ee = 0; ee < window.bi2_types.length; ++ee){
                    sid = window.resultaty_z[irab].items[ee].tid;
                    ispolz[sid]=true;
                }
            }
            if (window.resultaty_r[irab]){
                for (ee = 0; ee < window.bi2_types.length; ++ee){
                    sid = window.resultaty_r[irab].items[ee].tid;
                    ispolz[sid]=true;
                }
            }
        }
    ...
    for (tid in window.bi2_uchet){
        if ((tid < 600)&&(tid >=200)&&(!ispolz[tid])){
            window.bi2_nenuzhnoe[tid]=true;
        }
    }
}

window.bi2_res2html = function (){
    count_rab=0;
    window.bi2_htmlrab=[];
    while (count_rab < 255){
        if (!(window.porabotaj[count_rab]&&window.resultaty[count_rab])){
            ++count_rab;
            continue;
        }
        ihtm = '';
        ihtm += '<tr>';
        rabota = window.raboty[count_rab];
        ihtm += '<td rowspan=\"6\">';
        ...
        for (ri in rres.produkty){
            ...
            ihtm+='title=\"'+window.items[ri].name+'\" alt=\"'+window.items[ri].name+'\" ';
            ihtm+='src=\"'+window.items[ri].image_mini+'\" /></div><div class=\"jy_bi2\">'+rres.produkty[ri]+'%</div>';
            ihtm+='</div>';
        }
        ihtm += '</td></tr>';
        ihtm += '<tr><td>';
        ...
        if ((count_rab<=101)||(count_rab>111)){
		    ...
		    ihtm += (window.resultaty[count_rab].to+window.raboty[count_rab].malus-window.resultaty[count_rab].ton)+'</span></span>';
            ihtm += '<span title=\"Очки от комплектов\" class="calculation_visualisation img_plus" style=\"margin-left:0px;\">';
	    	ihtm += '<span class="skill_box_value green_text" style="text-align:center;">'+window.resultaty[count_rab].ton+'</span></span>';
		    ihtm += '<span title=\"Сложность работы\" class="calculation_visualisation img_minus"  style=\"margin-left:0px;\">';
    		ihtm += '<span class="skill_box_value" style="text-align:center;">'+window.raboty[count_rab].malus+'</span></span>';
    		...
    		ihtm += (window.resultaty[count_rab].to > 0) ? 'rgb(0,255,0)' : 'rgb(255,0,0)';
	    	ihtm += '">'+window.resultaty[count_rab].to+'</span></span>';
	    }
        else if (count_rab!=111){
			...
			var vvv = window.resultaty[count_rab].to-window.resultaty[count_rab].ton;
		...
		//ihtm += '<span title=\"Скорость от комплектов\" class="calculation_visualisation img_plus" style=\"margin-left:0px;\">';
	    	//ihtm += '<span class="skill_box_value green_text" style="text-align:center;">х'+window.resultaty[count_rab].ton+'</span></span>';
	    	ihtm += '<span title=\"Скорость\" class="calculation_visualisation img_equal" style="margin-right:10px; margin-left:0px;">';
		ihtm += '<span class="skill_box_value" style="text-align:center; ">'+Math.round(window.resultaty[count_rab].to)+'%</span></span>';
	    }
        ihtm += '</td><td>';
        ...
        brbr = 0;
        ihtm += '<table><tbody><tr>';
        for (jj in window.rabnavyki[count_rab]){
            for (aa = window.rabnavyki[count_rab][jj].mul; aa > 0; --aa){
                if (++brbr==8) {ihtm+='</tr><tr>'; brbr=1};
                ihtm += '<td><a class=\"tt3\" href=\"#\" ><span>'+window.pers.skill_titles[jj]+':'+window.rabnavyki[count_rab][jj].znach+'</span>';
                ihtm += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; background: transparent url(/images/skill/skills_'+window.bi2_s2a[jj];
                ihtm += '.png) repeat scroll '+window.bi2_s2px[jj]+'px 0px; color: rgb(255, 255, 255); ';
                ...
                ihtm += window.rabnavyki[count_rab][jj].znach+'</div>';
                ihtm += '</a></td>';
            }
        }
        ihtm += '</tr></tbody></table>';
	...
	for (ee = 0; ee < window.bi2_types.length; ++ee){
            sid = window.resultaty[count_rab].items[ee].tid;
            if (sid){
                ihtm+='<div style=\"display:inline; float:left;\">';
                vesch = window.items[sid];
                ihtm+='<a class=\"tt2\" href=\"#\" ><span><b>'+vesch.name+':</b>&nbsp;'+window.resultaty[count_rab].items[ee].bon;
                if (vesch.set.key){
                    ihtm += '<br /><em>'+vesch.set.name+'</em>';
                }
                for (ind in vesch.bonus.attributes){
                    ihtm += window.bi2_a_print(ind, vesch.bonus.attributes[ind]);
                }
                for (ind in vesch.bonus.skills){
                    ihtm += window.bi2_s_print(ind, vesch.bonus.skills[ind]);
                }
                ihtm += '</span>'
                ihtm+='<div class=\"bag_item\" ><img src=\"'+vesch.image_mini+'\" />';
                ihtm+='<div class="bag_item_count"><p style="text-align:center;">'+window.resultaty[count_rab].items[ee].bon+'</p></div></div>'
                ihtm+='</a>'
                if (window.resultaty[count_rab].items[ee].price > 0){
                    ihtm+='<br />';
                    ihtm +='<span style=\"text-align:center;\">'+window.resultaty[count_rab].items[ee].price+'&nbsp;$</span>';
                }
                ihtm +='</div>';
            }
        }
        ...
        if (window.resultaty_z[count_rab]){
            ihtm+= '<tr><td>';
            ihtm += '<span title=\"+навыки\" class="calculation_visualisation img_plus">';
	    	ihtm += '<span class="skill_box_value" style="text-align:center;">';
		    ihtm += window.resultaty_z[count_rab].zas+'</span></span>';
    		ihtm += '<span title=\"ТО\" class="calculation_visualisation img_equal" style="margin-right:10px;">';
	    	ihtm += '<span class="skill_box_value" style="text-align:center; color:';
		    ihtm += (window.resultaty_z[count_rab].to > 0) ? 'rgb(0,255,0)' : 'rgb(255,0,0)';
    		ihtm += '">'+window.resultaty_z[count_rab].to+'</span></span>';
            ihtm += '</td><td>';
            brbr=0;
            ihtm += '<table><tbody><tr>';
            for (jj in window.rabnavyki_z[count_rab]){
                for (aa = window.rabnavyki_z[count_rab][jj].mul; aa > 0; --aa){
                    if (++brbr==8) {ihtm+='</tr><tr>'; brbr=1};
                    ihtm += '<td>';
                    ihtm += '<a class=\"tt3\" href=\"#\" ><span>'+window.pers.skill_titles[jj]+':'+window.rabnavyki_z[count_rab][jj].znach+'</span>';
                    ihtm += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; background: transparent url(/images/skill/skills_'+window.bi2_s2a[jj];
                    ihtm += '.png) repeat scroll '+window.bi2_s2px[jj]+'px 0px; color: rgb(255, 255, 255); ';
                    ihtm += 'width: 25px; height: 14px; -moz-background-clip: border; -moz-background-origin: padding; ';
                    ihtm += '-moz-background-inline-policy: continuous; float: left; font-weight: bold; text-align:center;" >';
                    ihtm += window.rabnavyki_z[count_rab][jj].znach+'</div>';
                    ihtm += '</a></td>';
                }
            }
            ihtm += '</tr></tbody></table>';
		...
		ihtm += '</table></div>';
            ...
            for (ee = 0; ee < window.bi2_types.length; ++ee){
                sid = window.resultaty_z[count_rab].items[ee].tid;
                if (sid){
                    ihtm+='<div style=\"display:inline; float:left;\">';
			vesch = window.items[sid];
                    ihtm+='<a class=\"tt2\" href=\"#\" ><span><b>'+vesch.name+':</b>&nbsp;'+window.resultaty_z[count_rab].items[ee].bon;
                    if (vesch.set.key){
                        ihtm += '<br /><em>'+vesch.set.name+'</em>';
                    }
                    for (ind in vesch.bonus.attributes){
                    ...
                    ihtm += '</span>';
                    ihtm+='<div class=\"bag_item\" ><img src=\"'+vesch.image_mini+'\" />';
                    ihtm+='<div class="bag_item_count"><p style="text-align:center;">'+window.resultaty_z[count_rab].items[ee].bon+'</p></div></div>';
                    ihtm+='</a>';
                    if (window.resultaty_z[count_rab].items[ee].price > 0){
                        ihtm+='<br />';
                        ihtm +='<span style=\"text-align:center;\">'+window.resultaty_z[count_rab].items[ee].price+'&nbsp;$</span>';
                    }
                    ihtm +='</div>';
                }
            }
        ...
        if (window.resultaty_r[count_rab]){
            ihtm+= '<tr><td>';
            ihtm += '<span title=\"+навыки\" class="calculation_visualisation img_plus">';
	    	ihtm += '<span class="skill_box_value" style="text-align:center;">';
		    ihtm += window.resultaty_r[count_rab].speed+'%</span></span>';
    		ihtm += '<span title=\"ТО\" class="calculation_visualisation img_equal" style="margin-right:10px;">';
	    	ihtm += '<span class="skill_box_value" style="text-align:center; color:';
		    ihtm += (window.resultaty_r[count_rab].to > 0) ? 'rgb(0,255,0)' : 'rgb(255,0,0)';
    		ihtm += '">'+window.resultaty_r[count_rab].to+'</span></span>';
            ihtm += '</td><td>';
            brbr=0;
            ihtm += '<table><tbody><tr>';
            for (jj in window.rabnavyki_r[count_rab]){
                for (aa = window.rabnavyki_r[count_rab][jj].mul; aa > 0; --aa){
                    if (++brbr==8) {ihtm+='</tr><tr>'; brbr=1};
                    ihtm += '<td>';
                    ihtm += '<a class=\"tt3\" href=\"#\" ><span>'+window.pers.skill_titles[jj]+':'+window.rabnavyki_r[count_rab][jj].znach+'</span>';
                    ihtm += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; background: transparent url(/images/skill/skills_'+window.bi2_s2a[jj];
                    ihtm += '.png) repeat scroll '+window.bi2_s2px[jj]+'px 0px; color: rgb(255, 255, 255); ';
                    ihtm += 'width: 25px; height: 14px; -moz-background-clip: border; -moz-background-origin: padding; ';
                    ihtm += '-moz-background-inline-policy: continuous; float: left; font-weight: bold; text-align:center;" >';
                    ihtm += window.rabnavyki_r[count_rab][jj].znach+'</div>';
                    ihtm += '</a></td>';
                }
            }
            ihtm += '</tr></tbody></table>';
		...
		ihtm += '</table></div>';
            ...
            for (ee = 0; ee < window.bi2_types.length; ++ee){
                sid = window.resultaty_r[count_rab].items[ee].tid;
                if (sid){
                    ihtm+='<div style=\"display:inline; float:left;\">';
			vesch = window.items[sid];
                    ihtm+='<a class=\"tt2\" href=\"#\" ><span><b>'+vesch.name+':</b>&nbsp;'+window.resultaty_r[count_rab].items[ee].bon;
                    if (vesch.set.key){
                        ihtm += '<br /><em>'+vesch.set.name+'</em>';
                    }
                    for (ind in vesch.bonus.attributes){
                    ...
                    ihtm += '</span>';
                    ihtm+='<div class=\"bag_item\" ><img src=\"'+vesch.image_mini+'\" />';
                    ihtm+='<div class="bag_item_count"><p style="text-align:center;">'+window.resultaty_r[count_rab].items[ee].bon+'</p></div></div>';
                    ihtm+='</a>';
                    if (window.resultaty_r[count_rab].items[ee].price > 0){
                        ihtm+='<br />';
                        ihtm +='<span style=\"text-align:center;\">'+window.resultaty_r[count_rab].items[ee].price+'&nbsp;$</span>';
                    }
                    ihtm +='</div>';
                }
            }
        ...
        window.bi2_htmlrab[count_rab]=ihtm;
        ++count_rab;
    }
}

window.bi2_sortir = function (tip, minto){
    ind_arr = 0;
    window.bi2_sortrab = [];
    for (irab in window.bi2_htmlrab){
        if (window.bi2_vse_raboty&&(window.resultaty[irab].to <= -minto))
            continue;
        window.bi2_sortrab[ind_arr] = {};
        window.bi2_sortrab[ind_arr].index = irab;
        switch (tip){
        case 'd0':
            window.bi2_sortrab[ind_arr].ves = -window.raboty[irab].resultaty.dengi;
            break;
        case 'o0':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.opyt;
            break;
        case 'v0':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.vezenie;
            break;
        case 'boom':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.boom;
            break;
        case 'name':
            window.bi2_sortrab[ind_arr].ves= (irab > 100) ? 'я ' : '';
            window.bi2_sortrab[ind_arr].ves += window.raboty[irab].rus_name;
            break;
        case 'malus':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].malus;
            break;
        case 'to':
            window.bi2_sortrab[ind_arr].ves = 0-window.resultaty[irab].to;
            break;
        case 'dt':
		dt = window.resultaty[irab].to;
		if (dt){
			tmp = (1-.5/dt) * (1+dt/256);
			window.bi2_sortrab[ind_arr].ves = 0 - window.raboty[irab].resultaty.dengi*tmp;
		}
		else
			window.bi2_sortrab[ind_arr].ves = 0;
            break;
        case 'do':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.dengi - window.raboty[irab].resultaty.opyt;
            break;
        case 'dv':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.dengi - window.raboty[irab].resultaty.vezenie;
            break;
        case 'ov':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.vezenie - window.raboty[irab].resultaty.opyt;
            break;
        case 'dov':
            window.bi2_sortrab[ind_arr].ves = 0-window.raboty[irab].resultaty.dengi - window.raboty[irab].resultaty.opyt - window.raboty[irab].resultaty.vezenie;
            break;
        default:
            window.bi2_sortrab[ind_arr].ves = irab;
        }
        ++ind_arr;
    }
    window.qsort(window.bi2_sortrab,0,ind_arr);
    window.bi2_reporter2();
}

//window.bi2_setbezto = function(checked){
    //window.bi2_bezto = !checked;
//}

window.bi2_vesch_polezna = function(value){
    for (kh in window.bi2_khochuka)
        window.bi2_khochuka[kh] = false;
    if (value > 0)
        window.bi2_khochuka[value] = true;
    window.bi2_hideraboty(value);
}

window.bi2_reporter2 = function(){
    grgr = '';
    window.bi2_process=false;
    //new window.HumanMessage('Начинаем вывод данных', {type: 'success'});
    ...
    grsort += '<td style=\"width:2px; background-color:#321; padding: auto 3px;\" />';
    grsort += '<td><a title=\"Заработок с ТО\" href=\"javascript:bi2_sortir(\'dt\', bi2_bezto);\"><img style=\"width:23px; height:23px;\" src=\"images/job/dollar_bonus.png\" /></a></td>';
    grsort += '<td style=\"width:2px; background-color:#321; padding: auto 3px;\" />';
    ...
    if (window.bi2_khochuka.length > 0){
        ...
        if (window.bi2_khochuka.length > 1){
            ...
            for (kh in window.bi2_khochuka){
                grgr += '<option value=\"'+kh+'\">'+window.items[kh].name+'</option>';
            }
            grgr += '</select>';
        }
        for (kh in window.bi2_khochuka){
        if (window.bi2_khochuka[kh]){
            grgr += '<table><tr><td>';
            grgr+='<div style=\"display:inline; float:left;\">';
            vesch = window.items[kh];
            grgr+='<a class=\"tt2\" href=\"javascript:bi2_hideraboty('+kh+');\" ><span><b>'+vesch.name+':</b>';
            if (vesch.set.key){
                grgr += '<br /><em>'+vesch.set.name+'</em>';
            }
            for (ind in vesch.bonus.attributes){
                grgr += window.bi2_a_print(ind, vesch.bonus.attributes[ind]);
            }
            for (ind in vesch.bonus.skills){
                grgr += window.bi2_s_print(ind, vesch.bonus.skills[ind]);
            }
            grgr += '</span>'
            ...
            grgr +='Всего доступных  работ:' + window.chislo_rabot_to + '; &laquo;'+vesch.name+rere+ window.khoroshi_to[kh] +' раз.<br />';
            grgr +='Всего работ:' + window.chislo_rabot + '; &laquo;'+vesch.name+rere+ window.khoroshi[kh] +' раз.';
            grgr += '</td></tr></table>';
        }}
        grgr += '<hr>';
        grgr += '</td></tr>';
    ...
    for (ii = 0; ii < window.bi2_sortrab.length; ++ii){
        if (!window.bi2_hiderab[window.bi2_sortrab[ii].index]){
            if (ii>0) grgr+='<tr><td colspan=\"3\"><hr></td></tr>';
            grgr += window.bi2_htmlrab[window.bi2_sortrab[ii].index];
        }
    }
    grgr += '</table>';
    ...
    if (window.bi2_khlam){
        ...
        babosy=0;
        tdcount=0;
        for (tid in window.bi2_nenuzhnoe){
            grgr+='<td>';
            vesch = window.items[tid];
...
window.bi2_vreporter = function () {
    window.bi2_show_window();
    vrvr = '<table>';
    for (count_rab in window.porabotaj){
        if (!window.prosto_veschi[count_rab])
            continue;
        vrvr += '<tr>';
        rabota = window.raboty[count_rab];
        vrvr += '<td rowspan=\"2\">';
        ...
        for (ri in rres.produkty){
            ...
            vrvr+='title=\"'+window.items[ri].name+'\" alt=\"'+window.items[ri].name+'\" ';
            vrvr+='src=\"'+window.items[ri].image_mini+'\" /></div><div class=\"jy_bi2\">'+rres.produkty[ri]+'%</div>';
            vrvr+='</div>';
        }
        vrvr += '</td></tr>';
        vrvr += '<tr><td>';
        ...
        if (count_rab!=111){
		    ...
		    vrvr += (window.resultaty[count_rab].to+window.raboty[count_rab].malus-window.resultaty[count_rab].ton)+'</span></span>';
            vrvr += '<span title=\"Очки от комплектов\" class="calculation_visualisation img_plus" style=\"margin-left:0px;\">';
	    	vrvr += '<span class="skill_box_value green_text" style="text-align:center;">'+window.resultaty[count_rab].ton+'</span></span>';
		    vrvr += '<span title=\"Сложность работы\" class="calculation_visualisation img_minus"  style=\"margin-left:0px;\">';
    		vrvr += '<span class="skill_box_value" style="text-align:center;">'+window.raboty[count_rab].malus+'</span></span>';
    		...
    		vrvr += (window.resultaty[count_rab].to > 0) ? 'rgb(0,255,0)' : 'rgb(255,0,0)';
	    	vrvr += '">'+window.resultaty[count_rab].to+'</span></span>';
	    }
        vrvr += '</td><td>';
        ...
        for (jj in window.rabnavyki[count_rab]){
            for (aa = window.rabnavyki[count_rab][jj].mul; aa > 0; --aa){
                ...
                vrvr += '<td><a class=\"tt3\" href=\"#\" ><span>'+window.pers.skill_titles[jj]+':'+window.rabnavyki[count_rab][jj].znach+'</span>';
                vrvr += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; background: transparent url(/images/skill/skills_'+window.bi2_s2a[jj];
                vrvr += '.png) repeat scroll '+window.bi2_s2px[jj]+'px 0px; color: rgb(255, 255, 255); ';
                ...
                vrvr += window.rabnavyki[count_rab][jj].znach+'</div>';
                vrvr += '</a></td>';
            }
        }
        vrvr += '</tr></tbody></table>';
        ...
        for (ee = 0; ee < window.bi2_types.length; ++ee){
            ctype = window.bi2_types[ee];
            vrvr += '<tr><td>';
            for (vv = window.prosto_veschi[count_rab][ctype].length-1; vv >= 0;  --vv){
                sid = window.prosto_veschi[count_rab][ctype][vv].tid;
                vrvr+='<div style=\"display:inline; float:left;\">';
                vesch = window.items[sid];
                vrvr+='<a class=\"tt2\" href=\"#\" ><span><b>'+vesch.name+':</b>&nbsp;'+window.prosto_veschi[count_rab][ctype][vv].bon;
                if (vesch.set.key){
                    vrvr += '<br /><em>'+vesch.set.name+'</em>';
                }
                for (ind in vesch.bonus.attributes){
                    vrvr += window.bi2_a_print(ind, vesch.bonus.attributes[ind]);
                }
                for (ind in vesch.bonus.skills){
                    vrvr += window.bi2_s_print(ind, vesch.bonus.skills[ind]);
                }
                vrvr += '</span>'
                vrvr+='<div class=\"bag_item\" ><img src=\"'+vesch.image_mini+'\" />';
                vrvr+='<div class="bag_item_count"><p style="text-align:center;">'+window.prosto_veschi[count_rab][ctype][vv].bon+'</p></div></div>'
                vrvr+='</a>'
                if (window.prosto_veschi[count_rab][ctype][vv].price > 0){
                    vrvr+='<br />';
                    vrvr +='<span style=\"text-align:center;\">'+window.prosto_veschi[count_rab][ctype][vv].price+'&nbsp;$</span>';
                }
                vrvr +='</div>';
            }
            vrvr += '</td></tr>'
...
window.bi2_reporter = function () {
//    new window.HumanMessage('Начинаем вывод полученных данных', {type: 'success'});
    grgr='';
    window.bi2_ocenka_khlama();
    count_rab=0;
    window.bi2_show_window();
    window.bi2_res2html();
    
    if (window.bi2_khochuka.length > 0){
        window.chislo_rabot = 0;
        window.chislo_rabot_to = 0;
        window.khoroshi = [];
        window.khoroshi_to = [];
        for (kh in window.bi2_khochuka){
            window.khoroshi[kh] = 0;
            window.khoroshi_to[kh] = 0;
            window.bi2_khochuka[kh]=false;
        }
        window.bi2_khochuka[kh]=true; // last item;
        for (rr in window.porabotaj){
            if(window.resultaty[rr]){
                ++window.chislo_rabot;
                if (window.resultaty[rr].to > 0) ++window.chislo_rabot_to;
                for (ii = window.bi2_types.length-1; ii >= 0; --ii){
                    for (kh in window.bi2_khochuka){
                        if (window.resultaty[rr].items[ii].tid == kh){
                            window.khoroshi[kh] += 1;
                            if (window.resultaty[rr].to > 0) window.khoroshi_to[kh] += 1;
                        }
                    }
                }
            }
            if(window.resultaty_z[rr]){
                ++window.chislo_rabot;
                ++window.chislo_rabot_to;
                for (ii = window.bi2_types.length-1; ii >= 0; --ii){
                    for (kh in window.bi2_khochuka){
                        if (window.resultaty_z[rr].items[ii].tid == kh){
                            window.khoroshi[kh] += 1;
                            window.khoroshi_to[kh] += 1;
                        }
                    }
                }
            }
            if(window.resultaty_r[rr]){
                ++window.chislo_rabot;
                ++window.chislo_rabot_to;
                for (ii = window.bi2_types.length-1; ii >= 0; --ii){
                    for (kh in window.bi2_khochuka){
                        if (window.resultaty_r[rr].items[ii].tid == kh){
                            window.khoroshi[kh] += 1;
                            window.khoroshi_to[kh] += 1;
                        }
                    }
    ...
    window.bi2_process=false;
    window.bi2_sortir('name', window.bi2_bezto);
}
...
window.bi2_hideraboty = function(tid){
    window.bi2_hiderab=[];
    vtype = window.items[tid].type;
    kk = 0;
    for (; vtype != window.bi2_types[kk]; ++kk) {};
    for (irab in window.porabotaj){
        nea = true;
        if (window.resultaty[irab]&&(window.resultaty[irab].items[kk].tid==tid)){
            nea = false;
        }
        if (window.resultaty_z[irab]&&(window.resultaty_z[irab].items[kk].tid==tid)){
            nea = false;
        }
        if (window.resultaty_r[irab]&&(window.resultaty_r[irab].items[kk].tid==tid)){
            nea = false;
        }
        if (nea){
            window.bi2_hiderab[irab]=true;
        }
    }
    window.bi2_reporter2();
}
...
window.bi2_s_print = function(nav, val){
    /*
    kha = window.bi2_s2a[nav];
    ...
    result+='\" >'+window.pers.skill_titles[nav]+':'+val+'</div>';
    */
    result='<div>'+window.pers.skill_titles[nav]+':'+val+'</div>';
    return result;
}
window.bi2_a_print = function(kha, val){
/*
...
*/
    result = '<div style=\"font-weight:bold;\" >'+window.pers.attribute_titles[kha]+':'+val+'</div>';
    return result;
}
...
window.bi2_minimize_title = function(){
	if (window.bi2_title_flag2 == 1) {
    	window.bi2_title_flag2 = 0;
		...
		document.getElementById('bi2_form0').style.width = window.bi2_w0+'px';
	}
}

window.bi2_stretch_title = function(){
    var nv;
    if (window.bi2_title_flag == 1) {
        window.bi2_title_flag = 0;
        nv = window.bi2_title_h_mid + 'px';
    }
    else {
        window.bi2_title_flag = 1
        nv = window.bi2_title_h_max + 'px';
    }
    document.getElementById('bi2_title_content').style.height = nv;
}


window.bi2_close_title = function(){
	document.getElementById('bi2_title').style.display='none';
}

window.bi2_close_shmot = function(){
    rm = document.getElementById('bi2_shmot');
    document.body.removeChild(rm);
}

window.bi2_vselect = function (chk){
	if (chk) {
		document.getElementById('bi2_vselect').innerHTML=window.bi2_mulselect+window.bi2_conselect;
		/*document.getElementById('bi2_sk_rabot').innerHTML='Несколько работ';*/
	}
	else{
		document.getElementById('bi2_vselect').innerHTML=window.bi2_simselect+window.bi2_conselect;
		/*document.getElementById('bi2_sk_rabot').innerHTML='Одна работа&nbsp;&nbsp;&nbsp;&nbsp;';*/
	}
}

window.bi2_minimize_window = function(){
	if (window.bi2_window_flag2 == 1) {
		window.bi2_window_flag2 = 0;
		...
		window.bi2_window_flag2 = 1;
		document.getElementById('bi2_win1').style.width = window.bi2_w1+'px';
...
window.bi2_close_window = function(){
	document.getElementById('bi2_window').style.display='none';
}

window.bi2_error_window = function(err){
	document.getElementById('bi2_window_content').style.height = parseInt((window.bi2_window_h_max*3)/5, 10) + 'px';
	bi2_err = document.getElementById('bi2_window_error');
	bi2_err.style.height = parseInt((window.bi2_window_h_max*2)/5, 10) + 'px';
...
window.bi2_setValue = function(key,value) {
...
window.bi2_getValue = function(key) {
	window.setTimeout(function() {	window.bi2_abyrvalg = GM_getValue(key);}, 1 );
};
...
window.bi2_worker2 = function(schet){
    window.bi2_process=true;
    window.resultaty=[];
    window.resultaty_z=[];
    window.resultaty_r=[];
    window.zaschita=null;
    window.ezda = false;
    window.rabnavyki=[];
    window.rabnavyki_z=[];
    window.bi2_khochuka = [];
	window.pers = window.Character;
	...
	window.bi2_vse_raboty = vse_vse||vse_rab;
	vyb_rab = document.getElementById('bi2_rabota');
	window.porabotaj=null;
	window.porabotaj=[];
	if (vse_vse){
		for (r in window.raboty){
			if ((r>0)&&(r<=111))
				window.porabotaj[r]=true;
		}
	}
	else if (vse_rab){
		for (r in window.raboty){
		...
		ss = window.bi2_vse_navyki[ss];
		for (r in window.raboty){
			if ((r>0)&&(r<100)){
				for (jj in window.raboty[r].navyki)
					if (ss == jj)
						window.porabotaj[r]=true;
			}
		}
		...
		for (r in window.raboty){
			if ((r>0)&&(r<100)){
				for (jj in window.raboty[r].resultaty.produkty)
					if (ii==jj)
						window.porabotaj[r]=true;
			}
		}
		...
		for (r in vyb_rab.options){
			if (vyb_rab.options[r].selected){
				if (vyb_rab.options[r].value > 0) {
					window.porabotaj[vyb_rab.options[r].value] = true;
				}
			}
		}
	}
	min_hp=parseInt(document.getElementById('bi2_fort_hp').value,10);
	window.bi2_zdorov = (min_hp-100-(window.pers.level-1)*window.pers.lifePointPerHealthSkill)/(window.pers.lifePointPerHealthSkill+window.pers.lifePointPerHealthSkillBonus);
	    ...
	    for (v in vyb_vesch.options){
	        if (vyb_vesch.options[v].selected){
	            if (vyb_vesch.options[v].value > 0){
	                window.bi2_khochuka[vyb_vesch.options[v].value] = true;
	            }
	        }
	    }
	}
	window.bi2_khlam = document.getElementById('bi2_khlam').checked;
	iz_magazinov = document.getElementById('bi2_pokupka').checked;
	vse_veschi= document.getElementById('bi2_vse_vse').checked;
	window.bablo = parseInt(document.getElementById('bi2_bablo').value,10);
	window.bi2_millioner = document.getElementById('bi2_milion').checked;
	if (window.bi2_millioner)
	    window.bablo=1000000;
	plus_level = parseInt(document.getElementById('bi2_uroven').value,10);
	window.ezda = document.getElementById('bi2_skorost').checked
	s_zaschitoj=document.getElementById('bi2_zaschita').checked;
	e_nov_rabota=document.getElementById('bi2_navyki').checked;
	nov_index=window.raboty.length;
	if (e_nov_rabota){
	...
	if (e_nov_rabota){
		if (vse_vse || vse_rab || nesk_rab) {
			window.porabotaj[nov_index] = true;
		}
		else{
			window.porabotaj=[];
			window.porabotaj[nov_index] = true;
	...
	if (sslot){
		window.bi2_slots={};
		window.bi2_slots.flag=true;
		window.bi2_slots.head =document.getElementById('bi2_head').checked;
		window.bi2_slots.body =document.getElementById('bi2_body').checked;
		window.bi2_slots.foot =document.getElementById('bi2_foot').checked;
		window.bi2_slots.neck =document.getElementById('bi2_neck').checked;
		window.bi2_slots.right_arm =document.getElementById('bi2_right_arm').checked;
		window.bi2_slots.left_arm =document.getElementById('bi2_left_arm').checked;
		window.bi2_slots.yield =document.getElementById('bi2_yield').checked;
		window.bi2_slots.animal =document.getElementById('bi2_animal').checked;
	}
	else{
		window.bi2_slots=null;
	}
	//if (!window.bi2_inv_imported){
	    window.bi2_iimport();
	    if (!window.bi2_inv_imported){
	        new window.HumanMessage('Предварительно необходимо импортировать багаж. Откройте, и дождитесь загрузки.<br />После полной загрузки багажа, его можно свернуть или закрыть.');
	        window.bi2_process=false;
	        return;
	    }
	//}
	...
	if (window.bi2_inv_imported)
	{
        window.bi2_podschet(vse_veschi, iz_magazinov, plus_level, window.pers);
    }
       
    if (window.einfo!=''){
        window.bi2_show_window();
        window.bi2_error_window(window.einfo+'\n'+window.winfo);
    }
    else if (window.winfo!=''){
        window.bi2_show_window();
        window.bi2_error_window(window.winfo);
    }
    
    window.all_def={navyki:{}};
	if (window.zaschita){
		for (z in window.zaschita.navyki) window.all_def.navyki[z]=window.zaschita.navyki[z];
	}
	else	window.all_def.navyki = {aim:2,dodge:2,tactic:2,tough:1,reflex:1,health:1};
	if (schet){
	    window.rekurs_time = 50000;
	    window.rekurs_step = 0;
        window.bi2_vybzap();
    }
    else{
        window.bi2_vybvesch();
...
window.bi2_simselect='<select class=\"bi2_sel\" id=\"bi2_rabota\" size=\"1\" onchange=\"javascript:$(\'bi2_skol_rabot_o\').checked=true;\">';
window.bi2_mulselect='<select title=\"Выбор нескольких работ &mdash; с нажатой клавишей Ctrl\" class=\"bi2_sel\" multiple=\"multiple\" id=\"bi2_rabota\" size=\"6\" onchange=\"javascript:$(\'bi2_skol_rabot_n\').checked=true;\">';
window.bi2_conselect='\n\
...
window.bi2_slot_selector = function(v_slot){
...
window.bi2_show_shmot = function(irab){
    vv = null;
    if (window.resultaty_r[irab]){
        vv = window.resultaty_r[irab];
    }
    else if (window.resultaty_z[irab]){
        vv = window.resultaty_z[irab];
    }
    else if (window.resultaty[irab]){
        vv = window.resultaty[irab];
    }
    if (!vv) return;
    hti = '<table>';
    for (ii = 0; ii < vv.items.length; ++ii){
        ...
        if (sid){
            ...
            for (ind in vesch.bonus.attributes){
                hti += window.bi2_a_print(ind, vesch.bonus.attributes[ind]);
            }
            for (ind in vesch.bonus.skills){
                hti += window.bi2_s_print(ind, vesch.bonus.skills[ind]);
            }
            hti += '</span>';
    ...
    if (!bi2_shmot){
		...
		html2 += '<span>';
		html2 += '<a href=\"javascript:bi2_close_shmot();\"' + window.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html2 += '</span>';
...
window.bi2_show_window = function(){
    bi2_window = document.getElementById('bi2_window');
    html1='';
    
    if (!bi2_window){
		html1 += '<div id=\"bi2_win1\" style=\"width:' + window.bi2_w1 + 'px; text-align:left;\">\n';
		...
		html1 += '<span>';
		html1 += '<a href=\"javascript:bi2_minimize_window();\"' + window.bi2_tlink + ' title=\"Свернуть\">&nbsp;_&nbsp;</a>&nbsp;';
		html1 += '<a href=\"javascript:bi2_stretch_window();\"' + window.bi2_tlink + ' title=\"-\">&nbsp;^&nbsp;</a>&nbsp;';
		html1 += '<a href=\"javascript:bi2_close_window();\"' + window.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html1 += '</span>';
		html1 += '<span id=\"bi2_window_cap\">Результаты</span>';
		html1 += '</td><td class=\"gran_p\" />\n'
		html1 += '</tr><tr id=\"bi2_window_content_row\">\n';
		html1 += '<td class=\"gran_l\" />\n';
		html1 += '<td id=\"bi2_window_content0\" class=\"shadow_content\" style=\"width:' + (window.bi2_w1 - 12) + 'px;\" >';
		html1 += '<div id=\"bi2_window_content\" style=\"overflow: auto; height: ' + window.bi2_window_h_max + 'px;\">';
		...
		bi2_window.setAttribute('style', 'position: absolute; left: ' + window.bi2_l1 + 'px; top: ' + window.bi2_t1 + 'px; z-index:250');
		document.body.appendChild(bi2_window);
	}
	bi2_window.style.display = 'block';
	if (window.bi2_window_flag2 == 0){
	    window.bi2_minimize_window();
	}
}

window.bi2_show_panel = function(){
	bi2_title = document.getElementById('bi2_title');
	html0 = '';
	
	if (!bi2_title) {
		html0 += '<div id=\"bi2_form0\" style=\"width:' + window.bi2_w0 + 'px; text-align:left;\">\n';
		...
		html0 += '<span>';
		html0 += '<a href=\"javascript:bi2_minimize_title();\"' + window.bi2_tlink + ' title=\"Свернуть\">&nbsp;_&nbsp;</a>&nbsp;';
		html0 += '<a href=\"javascript:bi2_stretch_title();\"' + window.bi2_tlink + ' title=\"Развернуть\">&nbsp;^&nbsp;</a>&nbsp;';
		html0 += '<a href=\"javascript:bi2_close_title();\"' + window.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html0 += '</span>';
		...
		html0 += '<td class=\"gran_l\" />\n';
		html0 += '<td id=\"bi2_title_content0\" class=\"shadow_content\" style=\"width:' + (window.bi2_w0 - 12) + 'px;\" >';
		html0 += '<div id=\"bi2_title_content\" style=\"overflow: auto; height: ' + window.bi2_title_h_mid + 'px;\">';
		
		html0 += '\n\
<form id=\"bi2_form\">\n\
	<div id=\"bi2_vselect\">';
		html0 += window.bi2_simselect;
		html0 += window.bi2_conselect;
		
		html0 += '</div>\n\
	<div' + window.bi2_vblock + '>\n\
	...
	for (ii=0;ii<window.bi2_vse_navyki.length;++ii)
	{
		html0 += '<option value=\"'+(ii+1)+'\">	'+ window.Character.skill_titles[window.bi2_vse_navyki[ii]]+'</option>';
	};
		html0 += '\n\
	</select><br />\n\
	...
	for (ii=700;ii<800;++ii){
		tmp[ii]={};
		tmp[ii].ves = window.items[ii] ? window.items[ii].name : '-';
		tmp[ii].id = ii;
	}
	window.qsort(tmp,700,799);
	for (ii=700;ii<800;++ii)
	{
		if (tmp[ii].ves !== '-')
    ...
    for (vv = 1; vv < 900; ++vv){
        ...
        if (window.items[vv]){
            html0 +='<option value=\"'+vv+'\">	'+window.items[vv].name+'	</option>';
        }
    }
    html0 += '</optgroup><optgroup title=\"Остатки комплектов\" label=\"Остатки комплектов\">';
    // добавим сетовых ручками
    html0 += '<option value=\"792\">	'+window.items[792].name+'	</option>';
    html0 += '<option value=\"794\">	'+window.items[794].name+'	</option>';
    html0 += '<option value=\"797\">	'+window.items[797].name+'	</option>';
    html0 += '<option value=\"768\">	'+window.items[768].name+'	</option>';
    html0 += '<option value=\"723\">	'+window.items[723].name+'	</option>';
    html0 += '<option value=\"1715\">	'+window.items[1715].name+'	</option>';
    html0 += '<option value=\"854\">	'+window.items[854].name+'	</option>';		
    html0 += '</optgroup>';
html0 += '</select></div></div>\n\
	...
	<div style=\"color:red; font-weight:bold;\">\n\
		...
		bi2_title.setAttribute('style', 'position: absolute; left: ' + window.bi2_l0 + 'px; top: ' + window.bi2_t0 + 'px; z-index:202');
...
//window.bi2_getValue(window.bi2_pre+'odev_list');
//window.setTimeout(function() {eval(window.bi2_abyrvalg)},500);
//window.my_name_is();
...
add_bi2_menu_link = function(){
	...
	if (bi2_menu_inv) {
		...
		setTimeout(function(){if ($('bi2_footer_link2')) $('bi2_footer_link2').addMousePopup('<b>Подбор экипировки</b>')},2500);
		return;
	}
