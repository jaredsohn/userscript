...
(function(){
	...
	var lang = {
		...
		'pl': {
			...
			'troupsizedesc': 'Oczekiwana ilość jednostek we wioskach (nie ilość, która ma być dobudowana).',
			'openend': 'Kontynuować rekrutacje:',
			'openenddesc': 'Zaznacz oddziały, które będą rekrutowane po osiągnięciu ustalonych limitów.',
			'farmfree': 'Wolne miejsca w zagrodzie:',
			'farmfreedesc': 'Ile zostawić miejsc wolnych w zagrodzie (np. 100 na szlachcica).',
			'resfree': 'Zachowaj surowce:',
			'resfreedesc': 'Ile surowców zostawić w rezerwie (np. paczka na szlachtę - 28k/30k/25k).',
			'save': 'Zapisz',
			'fill': 'Wypełnij',
			'recruit': 'Rekrutuj',
			'running': 'czekaj...',
			'troupneeds': '(Desired troup needs %1 workers)',
			'filled': 'Wypełniono: %1 w %2 wioskach',
			'settoobig': 'Massrecruitment settings are too big to be saved in a cookie. To solve this problem delete some of your groups.'
		}
	};
	...
	if(/en\d+\.tribalwars\.net/.test(location.href)) {
		...
		curlang = 'de';
	} else if(/pl\d+\.plemiona\.pl/.test(location.href)) {
		curlang = 'pl';
	}
