// ==UserScript==
// @name          Livebeta.kaskus VBulletin Rep-Giver
// @namespace     http://userscripts.org/scripts/show/65502
// @include       *.livebeta.kaskus.co.id/usercp.php
// @include       *.livebeta.kaskus.us/usercp.php
// @include       *.livebeta.kaskus.com/usercp.php
// @version       1.24
// @dtversion     12052724
// @timestamp     1335047043780
// @description   (Kaskus Forum) automatically get (a|some) reputation(s) giver's link 
// @author        RSquare (http://userscripts.org/users/R.Square)
//
...
//
// mod.R.23 : 2012-04-22
// Fix bypass *llegal\sfunction for livebeta.kaskus_id
//
// mod.R.22 : 2011-05-19
...
function init(){
  ...
  gvar.f = ['uid','livebeta_kaskus_id','tag_id','location','join_date','total_post','avatar_url','rep_count','rep_filename','rep_rank_title'];
...
function parseIt(page){
  ...
  var get_livebeta_kaskus_id = function(x){
  ...
  if(ret) result[gvar.f[1]] = get_livebeta_kaskus_id(page);
...
function createSender(data, x){
  ...
  // safe proc, 
  if(isUndefined(data["livebeta_kaskus_id"])) {
  ...
  data["livebeta_kaskus_id"] = filterChar( data["livebeta_kaskus_id"] );
  var is_donat = ( /\[\$\]$/.test(data["livebeta_kaskus_id"]) ? '&nbsp;<b style="color:red;">[$]</b>':'');
  if(is_donat.length>0) data["livebeta_kaskus_id"]=data["livebeta_kaskus_id"].replace(/(\[\$\])$/,'');
  ...
  +'<div><b>'+( data["livebeta_kaskus_id"] )+'</b>'+(is_donat)+'</div>'
  +'<div>'+data["tag_id"]+'</div>'
  ...
  // last act, attaching..  
  var tgt = $D("_ucnd"+x);
  tgt.innerHTML = inner + '<a class="smallfont" href="' + gvar.uri['user_link'] + data["uid"] +'"><b>' + data["livebeta_kaskus_id"] + '</b></a>'
