// ==UserScript==
// @name           Hide Men's Health Trolls
// @namespace      net.mindsoup.gs
// @description    Hide posts by trolls on the menshealth.co.uk forums
// @include        http://www.menshealth.co.uk/community/forums/*
...
if(main_url.toLowerCase().indexOf("thread") != -1) {
	...
	$('.user_info').append('<br /><a href="#" class="link1 hidemenshealthlink">Hide user</a>');
} 


// bind block onclick function to link
$('.hidemenshealthlink').bind('click', function(e) { blockUser($(this)) });

// bind unblock onclick function to link
$('.unhidemenshealthlink').bind('click', function(e) { unblockUser($(this)) });

function appendBlockUser(name) {
	$('#useronline').append('<a href="#" class="link1 unhidemenshealthlink">' + name + '</a> ');
}
...
function setCookie(names) {
	...
	document.cookie = "hidemenshealthcookie" + "=" + namestring + ";expires=" + expires.toUTCString() + 

";path=/community/forums;domain=menshealth.co.uk";
}