...
menu.innerHTML = "\
<input type=\"checkbox\" id=\"autobank\" "+bchecked+" /> AB \
<input type=\"checkbox\" id=\"autobuildInc\" "+ichecked+" /> AI \
<input type=\"checkbox\" id=\"autobuildDef\" "+dchecked+" /> AD \
...
AM <input type=\"text\" style=\"border: 1px solid green; width: 2em; color: #00ff00; background: black; text-align: center;\" id=\"missionId\" value=\""+missionId+"\" /> <input type=\"checkbox\" id=\"refillEnergy\" "+rchecked+" /> HP <input type=\"checkbox\" id=\"missionDelay\" "+MDchked+" /> Delay <br>\
...
// refill Energy with HP
			...
			else
			{
				...
				setTimeout('document.location="/missions.php?cat=1";', randomTimer);
			}
		}
		else
		{
			...
			for (i=0; i<missions.length; i++)
			{
				...
				if (masteryBarProgress != null)
					...
					(missionId == '*' && progressPercent < 100))
					{
						if (missionDelay)
						{
							var randomDelay=Math.round(Math.random()*2+1)*1000;
