...
function TW_DOM_script() {
	...
	AEG.addEquipLink = function() {
		...
		if ( wear_group == null) {
			...
			store_set_link.setAttribute('title', 'Сохранить одетые вещи как набор');
			...
			delete_link.setAttribute('title', 'Удалить выбраный набор');
	...
	AEG.equip = function() {
		AEG.setEquipLinkText('Одеваемся'); //change link text so it's showing that it is busy
		...
		(function(){
			...
			if ( AEG.equipItems != undefined && AEG.equipItems.length > 0 ) {
				...
				AEG.setEquipLinkText('Одеваемся'+progress_dots);
				...
				if ( bag != null ) {
					...
					new HumanMessage("Вещь в инвентаре не найдена!");
					AEG.setEquipLinkText('Одеть'); //reset link text
				}
				...
				new HumanMessage("Не найдено сохранённых вещей, кликните на калькулятор работ!");
				AEG.setEquipLinkText('Одеть');
			}
	...
	AEG.storeSet = function() {
		...
		var sets = AEG.getSets();
		var set_name = prompt(" Назовите набор:\n"+ set_items_string);
		while ( sets[set_name] != undefined ) {
			if( !confirm("Это название набора уже используется, перезаписать?") ) {
				var set_name = prompt(" Назовите набор:\n"+ set_items_string);
			} else {
				break;
			}
	...
	AEG.deleteSet = function() {
		...
		switch ( dropdown_value ) {
			case '':
				new HumanMessage("Набор не выбран!");
			break;
			case '_TWIji':
				new HumanMessage("Невозможно удалить!");
			break;
			default:
				if ( confirm('Вы действительно желаете удалить набор "'+ dropdown_value +'"?') ) {
...
function JC_DOM_script() {
	...
	for ( ind=0; ind < table_rows.length; ind++ ) {
		...
		anchor.setAttribute('title', 'Сохранить вещи для ускоренного надевания в Весте');
	...
	AEG.itemsClick = function(best_items_cell) {
		...
		AEG.popupMessage("&nbsp;&nbsp;Сохранено<br /><br />"+ items_string +"<br /><br />&nbsp;&nbsp;для ускоренного надевания", "green");
	}
...
if ( location.href.indexOf('the-west.ru/') >= 0 ) {
	TW_DOM_script();
