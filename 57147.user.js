...
// @namespace      http://www.sdragnev.com
// @description    Counts the number of characters used in an email. Useful when sending SMS using Gmail. The script is just a modified version of Kenny Linsky's Gmail Word Count greasemonkey script -- http://www.kennyocracy.com
...
function createWcButton() {
    ...
    span.innerHTML = "Char Count";
    span.addEventListener('click', countChars, false);
    return span;
}
...
function countChars() {
    ...
    if(cleanStr.length) {
		getNode('wcButton').removeEventListener('click', countChars, false);
        var numChars = cleanStr.length-1;
        var span = createWcSpan(numChars);
...
function createWcSpan(numChars) {
    var span;
...
function wcReset() {
    ...
    if(wc) {
        ...
        getNode('wcButton').addEventListener('click', countChars, false);
    }
}
