// SPNG Raid Manager for pgrmdave
// Questions, comments, bug reports or donations to: slyz (yep that last character is a one)
//
// ==UserScript==
// @name          SPNG - hobopolis manager for pgrmdave
// @namespace     http://userscripts.org/scripts/source/60501.user.js
// @description   Version 2.2 - Organizes raid logs to a readable format - modified for SPNG by slyz
...
// --------------------------------------------------------------------------------------
// 25/10/2009 version 2.2 - Change value of advs spend at Richard's to 1 pt
// 25/10/2009 version 2.1 - Show player # in the loot table
// 25/10/2009 version 2   - Show player # in the clan point
...
var scriptURL = "http://userscripts.org/scripts/source/60501.user.js";
var scriptName = "SPNG - hobopolis manager for pgrmdave";
...
function createLootTable(lootArray, dungeon)
{
    ...
    for (var i=0; i < charArray.length; i++ )  // Loops through characters
    {
        ...
        var characterLink = playerLink2(charArray[i]);
        if (isEven(i))
          row_shaded = "";
        else
          row_shaded = "_shaded";
...
function playerLink2(character)
{
    var index = character.indexOf("#");
    var charLink = character.substring(index+1,character.length-1);
    return ('&nbsp;<a href="/showplayer.php?who=' + charLink + '">' + character + '</a>');
}
...
function getTotalPoints()
{
    ...
    {	
          ...
          if (ptsArray[z][9] != 0)
          {
			  ...
			  {
				dataTable += '<td style="text-align: left;" class="normal' + shaded + '">' + playerLink2(ptsArray[z][0]) + ' <font color="red"><b>(X)</b></font></td>\n';
			  }
			  else
			  {
				dataTable += '<td style="text-align: left;" class="normal' + shaded + '">' + playerLink2(ptsArray[z][0]) + '</td>\n';
			  }
