...
// Since Zynga put in a check to deter insertion of custom IDs, we have to use what's on the screen.
var elt = xpathFirst('.//table[@class="main_table fight_table"]//a[contains(@onclick, "reg_fight_view_attack") and contains(@onclick, "'+id+'")]', innerPageElt);
if (elt) {
opponent.attack = elt;
    }
  }
...
function createSocialTab() {
  ...
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Automatically help on jobs'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = 'Automatically call for help if someone starts a war against you';
  id = 'autoWarResponse';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' Call for war back up:'));
  ...
  id = 'autoWarPublish';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' Publish war against '));
  // War - autowar target
  id = 'autoWarTarget';
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = 'Automatically reward those who helped win your war';
  id = 'autoWarReward';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' Reward friends in war: '));
  // War - reward published
  title = 'Automatically publish that you rewarded your mafia for helping';
  id = 'autoWarRewardPublish';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' Publish the reward'));
...
function createAboutTab () {
  ...
  var devNames = ['StevenD', 'CharlesD', 'Eric Ortego', 'Jeremy', 'Liquidor', 'AK17710N', 'Fragger',
                 ...
                 'Cam', 'ChrisMSB', 'vmzildjian', 'Scrotal', 'Bushdaka'];
