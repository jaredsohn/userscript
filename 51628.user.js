// ==UserScript==
// @name           	GLB Power Rankings V2
// @namespace      	goallineblitz.com
// @description    	This is the second version of my power ranking script for GLB. This version gives the user more control over how the rankings are calculated.
...
//	}
	...
	for (var i=0; i<32; i++){
		...
		for (var j = 0; j < 32; j++) {
			if (results[i][0] == results[j][0] && j != 31) {
				j++;
			...
			onload: function(response){
				...
				if(userInput[0] == 1){					
					...
					for (var i=0; i<userInput[12]; i++){
						...
						if (gameId != ''){ 					
							var team2Id = location.parentNode.parentNode.getElementsByTagName('td')[1].getElementsByTagName('a')[0].href.split('=', 2)[1];
