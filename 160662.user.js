// ==UserScript==
// @name           KOC Power Bot
// @version        20130424-lordie
// @namespace      mat
// @homepage       http://userscripts.org/scripts/show/160662
  ...
  postToChat : function (m){
   ...
   if(Options.alertConfig.email) {
      ...
      handleTowerData: function(m){
        ...
        for (k in m.unts) {
            ...
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Catapult')
                units[12] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Bloodthorn')
                units[13] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Executioner')
                units[14] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Siege Wall')
                units[15] = m.unts[k];
        }
    ...
    addCityData2Pop: function(cityId){
        var t = Tabs.tower;
        var rownum = 0;
        var names = ['ST', 'MM', 'SC', 'PK', 'SW', 'AR', 'LC', 'HC', 'WG', 'BA', 'RM', 'CT', 'BT', 'EX', 'SG'];
        enc = {};
        numSlots = 0;
...
Tabs.AutoCraft = {
    ...
    init: function(div){
       ...
       m+="</table><b>Note:</b> If you complete more than one Item, the creation will be done randomly. <BR> <b>Important: Min. 2500 Aethestones and Refresh to Update the Inventory!</b> ";
  ...
  init : function (div){
    ...
    try {      
      ...
      <TR><TD><INPUT id=pblogperms type=checkbox '+ (Options.plog?'CHECKED ':'') +'/></td><TD>'+translate("Occasional logging of data to help with script development")+'</td></tr>\
        ...
        <TR><TD><INPUT id=pbCrestBut type=checkbox '+ (Options.crestbtns?'CHECKED ':'') +'/></td><TD>'+translate("Attack toggle button on top of screen")+'</td></tr>\
...
function AutoUpdater (prom) {
	var userscripts = 'http://userscripts.org/scripts/source/160662.user.js';
                ...
                function() {
                	...
                	if(lobalOptions.pbupdate) {
                        if(confirm('Do you want to turn off auto updating for this script?')) {
                            //GM_setValue('updated_160662', 'off');
