...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((showTooltip == true) && (dailyFact != 0)) {
    tooltip = mynumberformat(intfactPerHour, true)+" / 小时, "+mynumberformat(dailyFact, true)+" / 天";
  }
  var res;
  if (factPerHour != 0) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>"+floatFormat(remhour, 1) + " 小时装满";
    } else if (factPerHour < 0) {
      remaining = "<br>"+floatFormat(curres / -factPerHour, 1) + " 小时耗尽";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + " 总量<br>" + perc+"% 已使用" + remaining)+">"+
...
function getGrowthRemainingHours(population, maxPopulation, startTime, happiness) {
  ...
  return floatFormat((time - Number(startTime)) / 1000 / 3600, 1) + " 小时";
}
...
function createLastUpdateAsTooltip(content, time) {
  return createTooltip(content, "最后更新于 "+smartDateFormat(time, true));
}
...
if (city_idmainView > 0) {
  ...
  //finacen
  if(/view=finances/.test(document.URL)){
    var income = Number(getNodeValue("//td[@class='value res']","0")) ;
  ...
  if(/view=townHall/.test(document.URL)||/index\.php/.test(document.URL)) {
  city_id = getNode_value("//form[@id='upgradeForm']//input[@type='hidden' and @name='id']");
  city_id = getNode_value("//form[@id='changeCityForm']//input[@type='hidden' and @name='id']");
   res = getCity(city_id);
    if(getNodeValue('//li[contains(@class,"incomegold")]/span[@class="value"]')!=undefined)
         {res.income =  parseInt(getNodeValue('//li[contains(@class,"incomegold")]/span[@class="value"]'));}
    
    if(getNodeValue("//span[@class='production']")!=undefined)
        { res.production =  parseInt(getIntValue(getNodeValue("//span[@class='production']")));   }
}
  ...
  if(/view=transport/.test(document.URL)||/index\.php/.test(document.URL)) {
  	...
  	var destId = getNode_value("//form[@id='transport']/input[@type='hidden' and @name='destinationCityId']");
		...
		if((journeyTime.indexOf(" + ") > 0) && (journeyTime.indexOf(" = ") > 0))
  		...
  		var addname = prompt("请给目的地输入一个名称",destName+"["+journeyTime+"]");
  ...
  if (TABLE_RESOURCES) {
    ...
    s += "<th class='table_header' colspan=2>"+texts["cityName"]+"</th>"+
         "<th colspan=4 class='lf table_header'>"+populationName+"</th>"+
          ...
          "<th colspan=3 class='lf table_header'>收入</th>"+
    ...
    for(var i = 0; i < nodes.snapshotLength; i++) {
      ...
      res.output = parseInt(res.production) - parseInt(res.income);
      ...
      sumres.income +=res.income;
      sumres.production +=res.production;
      sumres.output +=res.output;
      ...
      if (wineUsage > 0) {
        wineRemainingHours = floatFormat(getCurrentResourceAmount(currenttime, res.prodtime, res.wine, res.prodwine - wineUsage) / wineUsage, 1) + " 小时";
      }
...
//        growthRemainingHours = floatFormat((parseInt(spacetotal) + parseInt(bonusspace) - parseInt(population)) / (0.02 * happiness), 1) + " h";
      ...
      s += "<td class='"+cs+"'>"+createLinkToCity(city.innerHTML, city.value, i)+"</td>"+
           ...
           "<td class='"+lfcs+"'>"
               ...
               +res.production+"</td>"+
              "<td class ='"+cs+"'>-"+res.output+"</td>"+
              "<td class ='"+cs+"'>"+res.income+"</td>"+
            "<td class='"+lfcs+"'>"
    ...
    s += "<td class='table_footer' colspan=2>"+texts["summary"]+"</td>"+
         ...
         "<td class='table_footer'></td>"+
         "<td class='table_footer'></td>"+
          "<td class='table_footer lf'>"+sumres.production+"</td>"+
             "<td class='table_footer'>-"+sumres.output+"</td>"+
                "<td class='table_footer'>"+sumres.income+"</td>"+
      ...
      var t = res.incomingTransporters.length;
			if(t > 0)
				for(var j = 0; j < t; j++) {
			      ...
			      s += "<td class='lf'>"+createTooltip(transporter.res["wood"], "到达时:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["wood"]+res.wood, res.prodwood))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["wine"], "到达时:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["wine"] + res.wine, res.prodwine - wineUsage))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["marble"], "到达时:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["marble"] + res.marble, res.prodmarble))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["glass"], "到达时:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["glass"] + res.glass, res.prodglass))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["sulfur"], "到达时:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["sulfur"] + res.sulfur, res.prodsulfur))+"</td>";
  ...
  if (true) {
    function reset_all_data() {
      var answer = confirm("您确定要清除所有保存的数据?");
      if (answer) {
        setVar("config", "");
        setVar("players", "");
    ...
    t.appendChild(createRowChk("声音报警:", "ALERT_SOUNDS", ALERT_SOUNDS));
    t.appendChild(createRowInput("报警音量 (0 - 100):", "ALERT_VOLUME", ALERT_VOLUME));
    t.appendChild(createRowInput("警告音量 (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("自动刷新:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("自动刷新下限(秒):", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("自动刷新上限(秒):", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("显示资源信息:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("显示来访运输船信息", "TABLE_TRANSPORTERS", TABLE_TRANSPORTERS));
    t.appendChild(createRowChk("显示建筑信息:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("显示军队信息:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("显示研究信息:", "TABLE_RESEARCH", TABLE_RESEARCH));
    t.appendChild(createRowChk("信件通道已研究:", "LETTERCHANNEL", "LETTERCHANNEL"));
    t.appendChild(createRowChk("显示玩家和城市信息 (开发中):", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("资源条模式:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "关闭", time: "基于剩余时间", percent: "基于容量"}));
    t.appendChild(createRowSlct("语言:", "LANGUAGE", language, {"": "由服务器名确定", cn: "简体中文" ,tw: "TChinese",en: "English", hu: "Magyar", de: "German", cz: "Czech", tr: "Turkish", es: "Spanish", ba: "Bosnian", it: "Italian", pt: "Portuguese", fr: "French"}));
    t.appendChild(createRowInput("联盟(缩写):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("友好联盟 (缩写, 逗号分割):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("对战联盟 (缩写, 逗号分割):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("CSS:", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("调试日志:", "DEBUG_LOG", DEBUG_LOG));
