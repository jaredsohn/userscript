(function () {
// ==UserScript==
// @name          Recent Activities Nuller
// @namespace     http://www.krakenstein.cz.cc
// @author        daYOda (Krakenstein)
// @description   Clear Facebook (damned) Recent Activities
// @version       2.2
...
// @exclude       http://*.facebook.com/pagelet/*
...
var protocol = document.location.protocol.match(/^https/i) ? "https" : "http";
...
function hide(id, m) {
  ...
  if (el !== null && el.tagName !== 'STYLE') {
    switch (m) {
      ...
      case 3: el.innerHTML = '<!--www.krakenstein.tk-->'; break;
...
function xhr(params, el) {
  ...
  var u = protocol + "://www.facebook.com/ajax/minifeed.php?__a=1";
  GM_xmlhttpRequest({
    method: "POST",
    url: u,
    data: params,
...
function createButton(remove) {
  ...
  if (!(target = is_ownprofile())) return false;

  box = document.createElement("div");
  box.id = box_id;
  box.className = 'gray_box';
  box.setAttribute('style',"text-align: center; color: gray; background-color: #F2F2F2; border: solid 1px #bfbfbf !important; border-radius: 3px !important; padding: 10px; margin: 10px 5px;");
  ...
  str += '<div id="boxrecent" style="display: none;">\
    ...
    <b>Purge Recent Activity</b></a></div>';

  // boxrecentdisabled ; Throw No Recent at all
  str += '<div id="boxrecentdisabled"><b>No Recent activities :-)</b></div>';

  // boxrecentloading ; Loading box
  str += '<div id="boxrecentloading" style="display: none;">\
    <img src="' + load_img + '" style="border: 0; padding-right: 20px" /><b>Working ...</b></div>';
  ...
  str += '<div id="boxrecentOption" style="padding-top:10px;">\
    ...
    <legend style="text-align: left; padding-left: 3px; padding-right: 3px;">Clean Option :</legend>\
    <span style="display:block; line-height:20px;"><label style="color: gray;"><input id="yod_recent_autoclean" name="yod_autoclean" type="checkbox" style="cursor: pointer; position: relative; top: 3px; margin-right: 5px;">Auto Cleaner</label></span>\
    </fieldset>\
    <br />- ver ' + yodUpdate['script_version'] + ' -\
    <br />-[ <a href="http://krakenstein.cz.cc" target="_blank">Krakenstein</a> - <a href="http://userscripts.org/scripts/show/' + yodUpdate['script_id'] + '" target="_blank">Discuss</a> ]-\
    </div>';
...
function is_ownprofile() {
  if (!g('profile_minifeed')) return false;
...
function reBoot(ok) {
  ...
  if (myContent) {
    createButton(true);
    if (ok) {
      myContent.removeEventListener(dom, collect, false);
      return Boot();
    }
  }
}
