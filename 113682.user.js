...
// @description    Dragons of Atlantis fix Anti Bot By EasyMaster
// @version        v3.9 POWER
// ==/UserScript==
(function() {

var Version = '3.9 POWER';
var Title = 'ProEkip Tools WNA BETA';
var WebSite = 'http://www.ProEkip.com/';
var Publi = 'http://www.facebook.com/DoaPowerTools';
...
var gTower = 'Gozcu';	
var kAutoResearchOn = 'Oto Arstrma Acik';
var kAutoResearchOff = 'Oto Arstrma Kpali';

var kLevel = 'level ';
var kLevel1 = ' Level';
var kResearch = 'Arastirma';
var kResearchErr = 'Arastirma Hatasi: ';
var kTooManyResearchErrs = 'Bircok Hata Oto Arastrmayi Engelliyor.';
...
var translateSPArray = {
 'Actualizar DoA':'Sayfa Yenileme',
    ...
    'Information': 'Arastirma',
    'TestroniusPowder':'Testronius Tozu',
    ...
    'Auto Upgrade Buildings': 'Otomatik Insaat',
    'Attack One Target in Waves': 'Oto Spam Saldirisi',
    ...
    'Stop if any troops lost':'Asker Kabinda Oto Spami Durdur',
    ...
    'Enable':'Izin Ver',
    'Random delay between attacks':'Saldirilar Arasi Sure Farkli',
    ...
    'Enable window drag move window by dragging <BR>top bar with mouse':'Pencerenin Mouseyle Kaydirilmasina Izin Ver',
    ...
    'DOA Power Tools Options':'RedSoldier TGE Tools Ayarlar',
    'Auto-attack Stats':'Oto Saldiri Statuleri',
    'Clear Stats':'Statuleri Temizle',
    'Attacks':'Saldirilar',
    'Stats started at':'Statuler Baslatildi',
    'Run time':'Calisma Suresi',
    'Resources':'Kaynaklar',
    'Stats by Camp Level':'Kamp Seviyelerine Gore Statuler',
    ...
    'GD Armor-1':'Buyuk Dragon Vucut Zirhi',
    'GD Armor-2':'Buyuk Dragon Kafa Zirhi ',
    'GD Armor-3':'Buyuk Dragon Kuyruk Zirhi',
    'GD Armor-4':'Buyuk Dragon Pence Zirhi',
    ...
    'Mandrakes':'Shirli Kok',
    'Mandrake-100':'100 Shirli Kok',
    'Mandrake-500':'500 Shirli Kok',
    'Mandrake-1000':'1000 Shirli Kok',
    ...
    'WindTroopItemStack100':'100 Olum Melegi Pence Zirhi ',
    'WindTroopItemStack500':'500 Olum Melegi Pence Zirhi ',
    'WindTroopItemStak1000':'1000 Olum Melegi Pence Zirhi ',
    'Loaded':'Yuklendi',
    'd':'d',
    'h':'h',
    'm':'m',
...
var Styles = '\
	...
	div.pbTitle {font-size: 9pt; border:1px solid; border-color:#CAB16A; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#5D902A; background-color:#CAB16A}\
	div.pbSubtitle {font-size: 9pt; border:1px solid; border-color:#CAB16A; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#5D902A; background-color:#CAB16A}\
	div.pbInput {font-size: 9pt; border:2px ridge yellow; background-color:#CAB16A; padding:3px}\
	div.pbStatBox {font-size: 9pt; border:2px ridge black; background-color:#B99739; padding:2px}\
...
function initSeed () {
  ...
  // TODO: Make sure WinPos is visible on-screen ?
    ...
    function gotSeed (rslt){    // TODO: check result, retry or disable tools?
      ...
      AddMainTabLink('ProEkip BETA v3.9POWER', eventHideShow, mouseMainTab); //BiTFx
...
var Seed = {
  ...
  tick : function (){     // called once per second - to check for job completion
    ...
    for (var pm in t.marches){
      ...
      if ((march.run_at < now-30) || (march.status=='Donuyor' && march.run_at < now-2) || (march.status=='returning' && march.run_at < now-2)){
        delete (t.marches[pm]);
        --Seed.numMarches;
      }
    }
    ...
    for (var pcity in t.jobs){
      for (var pjob in t.jobs[pcity]){
        ...
        if (job.run_at < (now - 300)){
          ...
          if (march && job.queue=='march' && (march.status=='Yuruyor' || march.status=='marching')){  // march just reached target
...
/***************************************************************************************************************/
  ...
  EspiarPendiente : function (){
    ...
    Tabs.Utiles.log('Saldiri ��in KAydediler Kordinatlar Listeleniyor.');
...
//      return;
  ...
  tabConfigLevels : function (){
    ...
    var m  = '<DIV><TABLE><TR>';
        m +=        '<TD><INPUT type=submit id=pbTabCampsA value=" 1-5 Arasi "></td>';
        m +=        '<TD><INPUT type=submit id=pbTabCampsB value=" 6-10 Arasi "></td></tr>';
        m +=      '</table></div>';
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '<DIV class=pbTitle>ProEkip BETA v:  '+ Version +'<BR>'+ WebSite +'<BR>Butun Yenilikleri icin asagidaki linki ziyaret Ediniz'+'<BR>'+ Publi +'</div>\
      <TABLE width=100%><TR><TD><INPUT type=submit value=" Yenile " id=pbinfRefresh></input><INPUT id=pcReloadTools type=submit \
      value=" Sayfayi Yenile " \></td><TD align=right><SPAN id=pbinfGmt></span></td><TD align=right>\
  ...
  showStuff : function (){
    ...
    m += '<TABLE style="margin-top:3px" width=100%>\
          <TR bgcolor=#dde align=center><TD style="border-right: 1px solid; border-bottom: 1px solid;"><B>ARASTIRMA - REZERVELER</b></td></tr></table>';
...
//    for (var ig=0; ig<Seed.generals.length; ig++){
    ...
    for (var ig in Seed.generals) {
      ...
      m += (Seed.generals[ig].busy?' <SPAN class=boldRed>Mesgul</span>':'') +'</td>';
      if (col == 0)
        col = 1;
      else {
        m += '</tr>';
    ...
    function dispRecursos (){
      ...
      m += '<TR><TD class=pbTabLeft>Yiyecek:</td><TD>'+ miles(recursos.food)  +'</td>';
...
Tabs.Research = {
  tabOrder        : 7,
  tabLabel        : 'Arastirmalar',
  tabDisabled     : false,
  cont            : null,
  timer           : null,
  capitolResearch : {Agriculture:'Tarim', Woodcraft:'Ormancilik', Masonry:'Duvarcilik', Mining:'Alisimlar', Clairvoyance:'Telepati',   RapidDeployment :'Hizli Sevkiyat', Ballistics:'Silah Kalibrasyonu', Metallurgy:'Metalurji', Medicine:'Tip', Dragonry:'Dragon Bilimi',   Levitation      :'Levitasyon', Mercantilism:'Tuccarlik', AerialCombat:'Hava Carpmasi'},
  ...
  tabJobOpc : function () {
    ...
    try {      
      m = '<DIV class=pbTitle style="margin-bottom:10px">Cesitli Ayarlar</div><TABLE class=pbTab>\
        <TR valign=top><TD><B>'+translate('Config')+':</b></td></tr>\
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox>Fare ile Pencere Tasimaya Izin Ver</td></tr>\
        <TR><TD><INPUT id=ptHiddenHd type=checkbox checked>Reklamlari Gizle</td></tr></TABLE>\
        <TABLE class=pbTab><TR><TD><B><BR>'+translate( 'Features' )+':</b></td></tr></TABLE>\
        <TABLE class=pbTab>\
          <TR valign=center><TD><INPUT id=pboptACol type=checkbox> Kasabadaki malzemeleri topla\
                <INPUT id=pboptAColTime size=1 maxlength=2 type=text value="'+ Data.options.autoCollect.delay +'" />\
                <SELECT id=pboptAColUnit size=1>\
                  <OPTION value=1 '+selected[1]+'>Saniyede bir</OPTION>\
                  <OPTION value=60 '+selected[2]+'>Dakikada bir</OPTION>\
                  <OPTION value=3600 '+selected[3]+'>Saatte bir</OPTION>\
                  <OPTION value=86400 '+selected[4]+'>Gunde bir</OPTION>\
                </SELECT></TD></TR>\
        </TABLE><BR><HR>';
  ...
  tabJobResearch : function (){
    var t = Tabs.Research;
    var n = '<DIV class=pbSubtitle> Otomatik Arastirma</div>\
    ...
    m += '<DIV class=pbSubtitle>Arastirmalar</div><TABLE class=pbTab><TR valign=top><TD><TABLE class=pbTab>';
...
Tabs.Azar = {
  ...
  init : function (div){
    ...
    try {      
      ...
      htmIFrame += "<div align='center'>Istenilen Odulu Almak iCin Isaretle</div>";
      htmIFrame += "Fortuna sansinizi Arttirmak iCin bu sekmeyi Kullanin.<br>";
      htmIFrame += "Not: (En Fazla 5 Oge SeCebilirsiniz)<br>";
    ...
    //htmIFrame += "Nº de intentos :<span id=NumAzar>0</span><br>";
      ...
      m = '<DIV class=pbTitle style="margin-bottom:10px"><table width="100%"><tr><td align=left>Fortuna BaslaticiO</td></tr></table></div>' + htmIFrame;
      t.cont.innerHTML = m;
  ...
  Ejecuta : function (){  
    ...
    document.getElementById('divAzar').innerHTML = 'Girisimler: ' + nAzar + '<br>';
