...
var URL_EMBED	  = "http://konghack.com/embed/game_display.php?mode=userscript&version="+version+"&game_name=";
/*
 *kong constants
 */ 
...
const NEWG_TITLE_LOCATION		= "og:title";
const NEWG_TITLE_IDENTIFIER		= "property";
const NEWG_HACKPANEL_TITLE_ID 	= "api_label";
const NEWG_ADD_AFTER_ELEMENT_A  = "embed_podbot";//id
const NEWG_ADD_AFTER_ELEMENT_B  = "html5_maincontent";//id

const NEWG_HACKPANEL_ID 		= "podcontent";
const NEWG_HACKPANEL_CLASS		= "flash_data_pod";//"maincontent-wide";//
...
const NITRO_TITLE_LOCATION		= 'og:title';
const NITRO_TITLE_IDENTIFIER		='itemprop';//<meta itemprop="name" content="Swindler">
const NITRO_ADD_BEFORE_ELEMENT	= "game_container";
...
addNitroIframe=function(){
	...
	try{
		...
		var gameTitle = FPXGetTagWithAttributeValue('meta','property',NITRO_TITLE_LOCATION)[0].getAttribute("content");		
	}catch(err){
		jOut(ERROR_TITLE+err.toSource());
		return false;
	}
...
addBeforeAuthorCommentsAndMedals=function(){
	...
	try{
		if(FPXGetTagWithAttributeValue('meta','property',NEWG_TYPE_IDENTIFIER)[0].getAttribute("content") != NEWG_TYPE_GAME){jOut("not a game, quitting newgrounds");return false;}
		var idToAddAfter = (document.getElementById(''+NEWG_ADD_AFTER_ELEMENT_A) != null) ? ''+NEWG_ADD_AFTER_ELEMENT_A : ''+NEWG_ADD_AFTER_ELEMENT_B;;	

		var gameTitle = FPXGetTagWithAttributeValue('meta','property',NEWG_TITLE_LOCATION)[0].getAttribute("content");
        jOut("game title::"+gameTitle);
        jOut("id to add script to::"+idToAddAfter);
	}catch(err){
		jOut(ERROR_TITLE+err.toSource());
		return false;
	}
