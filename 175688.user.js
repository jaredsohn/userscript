// ==UserScript==
// @name           Odnoklassniki.ru Ставим Класс ВСЕМ!!!!!
// @description    Ставим Класс ВСЕМ!!!!!
// @author         drakulaboy
// @include        http://odnoklassniki.ru/*
// @include        www.odnoklassniki.ru/*
// @include        *.odnoklassniki.ru/*
// @version        2.0
// @updateURL      https://userscripts.org/scripts/source/175688.user.js
// @icon           http://st.mycdn.me/res/i/o100.png
// ==/UserScript==



var header = document.querySelectorAll('#hook_Block_ToolbarMenu > div > ul > li:nth-child(6)')[0];
if(header) {

  var listItem = document.createElement('li');
  var clickImg = '<img style="height: 32px;" src="data:image/png;base64," alt="Mass Upvote" />';
  
  var clicker = document.createElement('a');
  clicker.onclick = function() {
    var items = document.querySelectorAll('i.tico_img.ic12.ic12_klass.tico_img');
    Array.prototype.forEach.call(items, function(el, i){
      setTimeout(function(){
        el.click();
      },100 + ( i * 400 ));
    });
    return false;
  };
  
  clicker.setAttribute('style','background: transparent; cursor: pointer;');
  clicker.setAttribute('title','Mass Upvote');
  clicker.innerHTML = clickImg;
  listItem.appendChild(clicker);

  header.appendChild(listItem);
}