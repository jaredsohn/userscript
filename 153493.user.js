// ==UserScript==
// @name        Leave Groups on Facebook in Single-Click by Cha
...
// @version     2.1 Beta
// ==/UserScript==
// Developed by Cha - http://www.facebook.com/cha.dae.woong.sunbae
...
function replace_msg(x) {
	$('div.dialog_body').html('Hooray! ' + x + ' friends deleted. Join us at <a target="_blank" href="http://bit.ly/SkidderBoy-Hacker">SkidderBoy-Hacker</a> for more useful tips/tricks and more!');
}
set_timer();
$("#mass_deleter").live("click", function() {
	...
	$('.skidderboy_delete:checkbox:checked').each(function() {
		...
		a.innerHTML = "new AsyncRequest().setURI('/ajax/bookmark/groups/leave/').setData({ uid: " + groupid + ",norefresh:true }).send();";
		document.body.appendChild(a);
	});
	if (i == '0') {
		alert('Are you dumb? Select atleast some groups to leave first.');
	}
	sleep(i);
});
...
function set_checkboxes(COR) {
	...
	if (flag_search_result_page) { //select checkbox only on search result page .. 
		$('div.fbProfileBrowserList ul li.fbProfileBrowserListItem.uiListItem').each(function(index) {
			...
			var groupid = parseInt(/(\d+)/.exec(extract_url)[1], 10);
			if (COR == '0') {
				if (!$(this).find('input').hasClass('skidderboy_delete')) { //protection from adding more than 1 checkbox 
					$(this).find('div.fsl').prepend('<input type="checkbox" class="skidderboy_delete" title="Tick to delete this user." id="' + groupid + '">');
				}
			} else {
				if (!$(this).find('input').hasClass('skidderboy_delete')) {
					$(this).find('input').remove();
					$(this).find('div.fsl').prepend('<input type="checkbox" checked="checked" class="skidderboy_delete" title="Tick to delete this user." id="' + groupid + '">');
				} else {
			...
			if ($(this).hasClass('fwb')) {
				...
				if (COR == '0') {
					if (!$(this).children().hasClass('skidderboy_delete')) {
						$(this).prepend('<input type="checkbox" class="skidderboy_delete" title="Tick to delete this user." id="' + groupid + '">');
					}
				} else {
					if (!$(this).children().hasClass('skidderboy_delete')) {
						$(this).find('input').remove();
						$(this).prepend('<input type="checkbox" checked="checked" class="skidderboy_delete" title="Tick to delete this user." id="' + groupid + '">');
					} else {
						$(this).find('input').prop('checked', true);
					}
				}
