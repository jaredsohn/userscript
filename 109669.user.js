// ==UserScript==
// @name           Player of the Game (Custom Team Version)
...
function Player(id, html, scoreDiff, teamID) {
	this.id = id;
	this.name = replaceHTML(html).trim();
	this.html = html;
...
var currentTeamName = GM_getValue("PotG-TeamName");
if(currentTeamName === undefined) {
	currentTeamName = prompt("Please enter your team name:");
	GM_setValue("PotG-TeamName", currentTeamName);
}
...
var dateXPath = document.evaluate(
	'//div[@class="tableHeader"]',
	document,
	null,
	XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,
	null);
var date = dateXPath.snapshotItem(0).textContent.replace(/^\s+|\s+$/g, "");
...
playerofgame.innerHTML = pogHTML + teamPotGRanks();
boxscore.parentNode.insertBefore(playerofgame, boxscore.nextSibling);

var pogbutton = document.createElement('input');
pogbutton.setAttribute('type', 'button');
pogbutton.setAttribute('value', 'PotG Scores');
pogbutton.setAttribute('id', 'pogbutton');
...
var teamNameButton = document.createElement('input');
teamNameButton.setAttribute('type', 'button');
teamNameButton.setAttribute('value', 'Set Team Name');
pogbutton.parentNode.insertBefore(teamNameButton, pogbutton.nextSibling);
teamNameButton.addEventListener('click', function() {
	currentTeamName = prompt("Please enter your team name:");
	GM_setValue("PotG-TeamName", currentTeamName);
...
//
//Example:
// [b]08/25/2011:[/b] TBA 118, Asian Sensation 94
// Player of the Game: M. Abinijhabadaba: 25/5/1, 2 stl
// Honorable Mention: L. James: 2/1/7, 1 stl, 0 TOs; M. Fazio: 6/9/0, 2 stl, 1 blk, 0 TOs
// ********************************************************************	
function teamPotGRanks() {
	var teamPotG = "<div><strong>" + currentTeamName + " PotG Ranks: </strong><br />";
	teamPotG += "[b]" + date + ":[/b] ";
	
	var teamRanks;
	if(replaceHTML(team1) == currentTeamName) {
		teamRanks = team1Scores;
	} else {
		teamRanks = team2Scores;
	}

	teamPotG += getTeamScores() + "<br />";
	teamPotG += "Player of the Game: " + teamStatLine(teamRanks[0]) + "<br />";
	teamPotG += "Honorable Mention: " + teamStatLine(teamRanks[1]);
	teamPotG += "; " + teamStatLine(teamRanks[2]);
	
	var extraPotGNum = 3;
	var morePotG = teamRanks[extraPotGNum];
	while(playerPoGRating(morePotG) == playerPoGRating(teamRanks[2])) {
		teamPotG += "; " + teamStatLine(morePotG);
		extraPotGNum++;
		morePotG = teamRanks[extraPotGNum];
	}
...
function getTeamScores() {
	...
	if(parseInt(score1) > parseInt(score2)) {
		winner = replaceHTML(team1) + " " + score1;
		loser = replaceHTML(team2) + " " + score2;
	} else {
		winner = replaceHTML(team2) + " " + score2;
		loser = replaceHTML(team1) + " " + score1;
	}
	
	return winner + ", " + loser;
}

function teamStatLine(player) {
	var statLine = player.name + ": ";
	statLine += player.pts + "/" + player.reb + "/" + player.ast;
	if(player.stl > 0) {
		statLine += ", " + player.stl + " stl";
	}
	if(player.blk > 0) {
		statLine += ", " + player.blk + " blk";
	}
	if(player.to == 0) {
		statLine += ", " + player.to + " TOs";
	}
