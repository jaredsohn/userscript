// ==UserScript==
// @name        CKEditor for POEditor
// @namespace   http://www.greenweez.com/
// @description CKEditor for POEditor
...
$(function() {
	...
	body.html-editor #main iframe {\
		display: block;\
	}\
	body.html-editor #main .no-html-editor {\
		display: none;\
	}";
	...
	script.onload = function() {
		...
		var toggleHTML = $('<a class="btn btn-primary">HTML editor</a>')[interactive ? 'insertBefore' : 'insertAfter']
			($(document.querySelector('.row-fluid .link-only,.interactive-buttons .btn')));
		...
		[].forEach.call(references, function(el) {
			...
			iframe.id = el.id+'ifr';
			iframe.src = 'data:text/html;charset=utf-8, <style>html, body {margin: 0; padding: 0 2px;font-size: 13px;}</style>'
		...
		function updateTextaras() {
			...
			[].forEach.call(els, function(el) {
				...
				el.iframe.src = 'data:text/html;charset=utf-8, <script>window.onload = function() {document.body.onclick = function() {parent.iframe_click("'+el.id+'");}; parent.iframe_resize(\''+el.iframe.id+'\', document.body.offsetHeight);};</script><style>html, body {margin: 0; padding: 0 2px;font-size: 13px;}</style>' + encodeURIComponent(el.textContent);
