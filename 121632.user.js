// ==UserScript==
// @name          Newzbin IMDB Movie Rating and Filter
// @description   Shows IMDB rating as number and descriptions. Its nzbstatus compatible and minimalistic. A hacked together update of http://userscripts.org/scripts/show/33755. Now also allows you to filter good and/or bad movies. Settings are also saved.
// @include       http*://*.newzbin2.es/*
// @version       3.HACK fixed for new imdb layout and changes to Newzbin.  RegEx will eventually break again.
// ==/UserScript==
...
//var ratingRegex = new RegExp(/<span class="rating-rating"><span class="value">(\d.\d)<\/span>/);
var ratingRegex = new RegExp(/<span itemprop="ratingValue">(\d.\d)<\/span>/);
var descRegex = new RegExp("name=\"description\" content=\"[^\.]+\.[^\.]+\.(.*?)/>");
var newzBinTitleRegex = new RegExp("^http[s]*:\/\/(v3|www)\.newzbin2\.es\/browse\/post\/[0-9]+\/$");
...
function do_parsing() 
{
  for (var i = 0; i < movies.snapshotLength; i++)
  {
	...
	movieBodyHTML = unescape(movieBody.innerHTML); 
	console.log(movieBodyHTML);
	var imdbLink = movieBodyHTML.match(imdbLinkRegex);
