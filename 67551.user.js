...
function saver(){
	...
	GM_setValue((server+"colorc"), (document.getElementById('Kristalcolor').value));
	GM_setValue((server+"colord"), (document.getElementById('Deuterijcolor').value));
	GM_setValue((server+"colore"), (document.getElementById('Energijacolor').value));
...
switch(langstr){
	...
	break;
	case "de":
		langloca = "2"; X_mlg = /(Metall: )<b>([\.0-9]+)/; X_clg = /(Kristall: )<b>([\.0-9]+)/; X_dlg = /(Deuterij: )<b>([\.0-9]+)/; X_elg = /(Energie: )<b>([\.0-9]+)/; X_lvl = /\(Stufe (\d+)/;
	break;
	case "es":
	...
	break;
	case "nl":
		langloca = "9"; X_mlg = /(Metaal: )<b>([\.0-9]+)/; X_clg = /(Kristal: )<b>([\.0-9]+)/; X_dlg = /(Deuterij: )<b>([\.0-9]+)/; X_elg = /(Energie: )<b>([\.0-9]+)/; X_lvl = /\(Niveau (\d+)/;
	break;
	case "it":
	...
	break;
	case "tr":
		langloca = "13"; X_mlg = /(Metal: )<b>([\.0-9]+)/; X_clg = /(Kristal: )<b>([\.0-9]+)/; X_dlg = /(Deuterij: )<b>([\.0-9]+)/; X_elg = /(Enerji: )<b>([\.0-9]+)/; X_lvl = /\(Kademe (\d+)/;
	break;
	case "ro":
	...
	break;
	case "dk":
		langloca = "16"; X_mlg = /(Metal: )<b>([\.0-9]+)/; X_clg = /(Krystal: )<b>([\.0-9]+)/; X_dlg = /(Deuterij: )<b>([\.0-9]+)/; X_elg = /(Energi: )<b>([\.0-9]+)/; X_lvl = /\(Level (\d+)/;
	break;
	default:
		langloca = checker((ogserver+"langloca"),"0");
		notdetected = true;
		X_mlg = /(Metal: )<b>([\.0-9]+)/; X_clg = /(Kristal: )<b>([\.0-9]+)/; X_dlg = /(Deuterij: )<b>([\.0-9]+)/; X_elg = /(Energija: )<b>([\.0-9]+)/; X_lvl = /\(level (\d+)/;
}
...
if((/page.buildings.+mode.Flotte/.test(ogtitle) == true) || (/page.buildings.+mode.Verteidigung/.test(ogtitle) == true) || (/mode.Forschung/.test(ogtitle) == true) || (/page.b_building/.test(ogtitle) == true)){
	if((readytime == "1") || (maxships == "1") || (relvl == "1") || (color_m.length > 0) || (color_c.length > 0) || (color_d.length > 0) || (color_e.length > 0)){

		var allcurres = xpath("//font[contains(@color,'#')]/parent::td/parent::tr/td[contains(@class,'header')]");
...
if(/page.flotten/.test(ogtitle) == true){
if(calcships == "1"){
	...
	var fuelname = new Array(); //32.
		fuelname[0] = "Deuterij consumption"; //ogame.org
		fuelname[1] = "Consumo de deut&eacute;rio"; //ogame.com.br
		fuelname[2] = "Deuterijverbrauch"; //ogame.de
		...
		fuelname[7] = "??????"; //ogame.pl
		fuelname[8] = "??????"; //ogame.ru
		fuelname[9] = "Deuterij verbruik"; //ogame.nl
...
if(advstor == "1"){
	...
	var resname = xpath("//font/parent::b/parent::i/parent::td[@class='header']/i/b/font");
	GM_log(resname.snapshotItem(0).innerHTML);
	var metname = resname.snapshotItem(0).innerHTML;
	var cryname = resname.snapshotItem(1).innerHTML;
	var deuname = resname.snapshotItem(2).innerHTML;
	...
	var stormet = Math.floor(curmet / (parseInt(storxp.snapshotItem(0).innerHTML)*1000)*100);
	...
	GM_log("Metal: "+stormet+" "+parseInt(storxp.snapshotItem(0).innerHTML)+" "+curmet);
...
if(/page.options/.test(ogtitle) == true){
	...
	var L_cc = new Array(); //4.
		L_cc[0] = "Kristal color"; //ogame.org
	...
	var L_dc = new Array(); //5.
		L_dc[0] = "Deuterij color"; //ogame.org
		L_dc[1] = "Cor do deut&eacute;rio"; //ogame.com.br
		L_dc[2] = "Farbe Deuterij"; //ogame.de
	...
	var optionszone = xpath("//body/div[@id='content']/center").snapshotItem(0);
	var optionstable = document.createElement('table');
		...
		"<tr><th>"+L_cc[langloca]+"</th><th><input type=\"button\" value=\"&reg;\" style=\"cursor:help;\" title=\""+R_rt[langloca]+"\" onclick=\"resetbut('Kristalcolor','#54B0DC')\" ondblclick=\"resetbut('Kristalcolor','')\"><input type=\"text\" onkeyup=\"changer('Kristalcolor')\" style=\"color:"+color_c+";\" id=\"Kristalcolor\" maxlength=\"18\" size =\"20\" value=\""+color_c+"\"></th></tr>"+
		"<tr><th>"+L_dc[langloca]+"</th><th><input type=\"button\" value=\"&reg;\" style=\"cursor:help;\" title=\""+R_rt[langloca]+"\" onclick=\"resetbut('Deuterijcolor','#9AACCB')\" ondblclick=\"resetbut('Deuterijcolor','')\"><input type=\"text\" onkeyup=\"changer('Deuterijcolor')\" style=\"color:"+color_d+";\" id=\"Deuterijcolor\" maxlength=\"18\" size =\"20\" value=\""+color_d+"\"></th></tr>"+
		"<tr><th>"+L_ec[langloca]+"</th><th><input type=\"button\" value=\"&reg;\" style=\"cursor:help;\" title=\""+R_rt[langloca]+"\" onclick=\"resetbut('Energijacolor','#F2D99D')\" ondblclick=\"resetbut('Energijacolor','')\"><input type=\"text\" onkeyup=\"changer('Energijacolor')\" style=\"color:"+color_e+";\" id=\"Energijacolor\" maxlength=\"18\" size =\"20\" value=\""+color_e+"\"></th></tr>"+
...
if(cilink == "1"){
	del("//div[@id='content']//a[@id='combox']/parent::div[@id='combox_container']");
}
if(oclink == "1"){
	del("//div[@id='menu']//tr/td[@align='center']/a[contains(@href,'micropayment')]/../..");
}
if(topicons == "1"){
	del("//div[@id='header_top']//center/table[@class='header']/tbody/tr[@class='header']/td[3]/table/..");
}
//---------del ads end