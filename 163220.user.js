...
function clickTheConfirms() {
    numDeleted = 0;
    $('button[type="submit"][class*="layerConfirm"]').each(function() {
        $(this).click();
        numDeleted++;
    });
    setTimeout(function () { hideTheOks(numDeleted); }, 3000);
}

function hideTheOks(nDeleted) {
    $('div[class="_1yv"]').each(function() {
        $(this).hide();
    });   
    alert('Done!\n' + nDeleted + ' friend(s) deleted.');
...
$("#mass_deleter").live("click", function() {
	...
	//sleep(i);
	setTimeout(function () { clickTheConfirms(); },3000 );
});
