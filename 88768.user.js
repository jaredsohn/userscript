...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=925841&tid=5495926433638612582", false);
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[b]Titulo:[/b] ".concat(titulo, breakLine, "[b]Criador:[/b] ", uid, breakLine, "[b]Motivo:[/b] ", motivo, breakLine, "[b]Horario:[/b] ", pegaData(), breakLine, breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b]Uid do membro:[/b] ".concat(uid, breakLine, "[b]Motivo:[/b] ", motivo, breakLine, "[b]Horario:[/b] ", pegaData(), breakLine, breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
