// ==UserScript==
// @name           Ikariam Poruke Naslov (Ćirilica)
// @namespace      Naslov
// @description    Dodaje naslov u vaše poruke
// @version        1.1
...
function insertSubjectTextbox () {
   ...
   if (allyMessage[1]!=null) {
      ...
      + "<option value='Обична' selected>Обична</option>"
      + "<option value='ВАЖНО ОБАВЕШТЕЊЕ'>ВАЖНО ОБАВЕШТЕЊЕ</option>"
      + "<option value='УПУТСТВО'>УПУТСТВО</option>"
      + "<option value='Напад'>Напад</option>"
      + "<option value='Извештај борбе'>Извештај борбе</option>"
      + "<option value='Шпијунажа'>Шпијунажа</option>"
      + "<option value='Културна размена'>Културна размена</option>"
      + "<option value='Размена ресурса'>Размена ресурса</option>"
      + "<option value='Трговина'>Трговина</option>"
      + "<option value='Ћаскање'>Ћаскање</option>"
      + "<option value='Вицеви и забава'>Вицеви и забава</option></select><input type='text' id='msg' value='' />";
   } else {
      var newSubjectDiv = document.createElement('div');
      newSubjectDiv.setAttribute('id', 'mailSubjectMod');
      newSubjectDiv.innerHTML = "<span class = 'maillabels'>"+"<label>Наслов:</label></span><span><input type='text' id='msg' value=''/></span>";
      document.getElementById('mailRecipient').parentNode.insertBefore(newSubjectDiv,document.getElementById('mailSubject'));
      parentDiv.childNodes[1].innerHTML = '<label for="treaties">Врста поруке :</label>';
   }
}
...
for (var i=0; i<soggetti.length; i++) {
   ...
   if (inn.substring(0,1)=="[") {
	 ...
	 if (res.indexOf("Обична")!=-1){
         ndiv.innerHTML="<img title=\"Обична\" src=\"skin/board/schriftrolle_closed2.gif\" height=\"12\" width=\"12\" /> "+res;
         }
         else if (res.indexOf("ВАЖНО ОБАВЕШТЕЊЕ")!=-1){
         ndiv.innerHTML="<img title=\"Културна размена\" src=\"skin/relatedCities/general.gif\" height=\"12\" width=\"12\" /> "+res;
         tr.setAttribute("style","background-color:#FFFF66");
         }
         else if (res.indexOf("УПУТСТВО")!=-1){
         ndiv.innerHTML="<img title=\"Културна размена\" src=\"skin/board/schriftrolle_offen2.gif\" height=\"12\" width=\"12\" /> "+res;
         tr.setAttribute("style","background-color:#B8DDF1");
         }
         else if (res.indexOf("Напад")!=-1){
         ndiv.innerHTML="<img title=\"Напад\" src=\"skin/advisors/military/icon-attack.gif\" height=\"12\" width=\"12\" /> "+res;
         tr.setAttribute("style","background-color:#FF9292");
         }
         else if (res.indexOf("Извештај борбе")!=-1){
         ndiv.innerHTML="<img title=\"Извештај борбе\" src=\"skin/layout/medallie32x32_gold.gif\" height=\"12\" width=\"12\" /> "+res;
	 tr.setAttribute("style","background-color:#AEFF82");
         }
         else if (res.indexOf("Шпијунажа")!=-1){
         ndiv.innerHTML="<img title=\"Шпијунажа\" src=\"skin/layout/icon-status-small.gif\" height=\"12\" width=\"12\" /> "+res;
	 tr.setAttribute("style","background-color:#CCCCCC");
         }
         else if (res.indexOf("Културна размена")!=-1){
         ndiv.innerHTML="<img title=\"Културна размена\" src=\"skin/museum/icon32_culturalgood.gif\" height=\"12\" width=\"12\" /> "+res;
         }
         else if (res.indexOf("Размена ресурса")!=-1){
         ndiv.innerHTML="<img title=\"Размена ресурса\" src=\"skin/wonder2/multi_marble.gif\" height=\"12\" width=\"12\" /> "+res;
         }
	 else if (res.indexOf("Трговина")!=-1){
         ndiv.innerHTML="<img title=\"Трговина\" src=\"skin/resources/icon_gold.gif\" height=\"12\" width=\"12\" /> "+res;
         }
	 else if (res.indexOf("Ћаскање")!=-1){
         ndiv.innerHTML="<img title=\"Ћасканје\" src=\"skin/resources/icon_population.gif\" height=\"12\" width=\"12\" /> "+res;
         }
	 else if (res.indexOf("Вицеви и забава")!=-1){
         ndiv.innerHTML="<img title=\"Ћасканје\" src=\"skin/smilies/happy.gif\" height=\"12\" width=\"12\" /> "+res;
         }
         else{         
...
if(tmp/1000 - parseInt(tm) > 90000 ){ // This part checks whether check was performed in last 25 hours. You can set your own timing by chaining value 90000 to something else. Note that this value is in seconds.
    ...
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'http://userscripts.org/scripts/source/86940.meta.js', //Here you will change number 86940 to number of your script
      headers: {
      ...
      vers = vs[4].substring(12); // Here we obtain version of the script if by some case you have your @version tag in some other row then fifth then change number 4 to some other value that responds to your code
        ...
        if(version != parseFloat(vers)){ // We check if new version is out
          alert("Nova verzija Ikariam Poruke Naslov je izašla!");// This is notification to user that new version is available
          window.location = "http://userscripts.org/scripts/source/86940.user.js"; //If new version is out install new version of the script. NOTE: that you have to change number 86940 to the number of your script on userscripts.
