...
window.addEventListener('load', function(){
	if(window.location.href.indexOf('http://translate.google.com/translate_n?') == 0 && window.parent != window.self)window.parent.document.body.rows = '0,*';
}, false);
...
window.navigator.ujs_createWindow = function(strContent, strStatus, strTitle, strId, pos, size){
	var wId = 'ujs_window'+(strId || ''), win = document.getElementById(wId);
	var keyDown = function(e){if(!e.shiftKey && !e.ctrlKey && !e.altKey && e.keyCode == 27)document.getElementById(wId).closeWin()};
	if(win)win.closeWin();
	...
	img.title = (window.navigator.language.indexOf('ru') == 0) ? '\u0417\u0430\u043A\u0440\u044B\u0442\u044C' : 'Close';
	...
	win.addEventListener('mousedown', function(e){
		if(e.target == win){
			...
			document.addEventListener('mouseup', function(){document.removeEventListener('mousemove', mouseMove, false)}, false);
		}
	}, false);
...
window.addEventListener('message', function(e){
	...
	if(e.data && e.data.indexOf('google_translate') == 0){
		...
		if(origin == 'http://gtl-proxy.appspot.com' || origin == 'http://m.translate.ru'){
			...
			window.navigator.ujs_createWindow(decodeURI(msg[0]), decodeURI(msg[1]), '<a href="'+decodeURI(msg[2])+'" target="_blank" style="display:inline;padding:0;margin:0;text-decoration:none;border:none;color:#009;font:16px Times New Roman;">Google Translate</a>', '_gt', window.navigator.lastClick);
