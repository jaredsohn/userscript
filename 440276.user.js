...
aWindow.dwnld_sel_start = function(elem){
	...
	if(sel_arr.length){
		for(var j = 0; j <= sel_arr.length; j++){
			dwnld_2arr(seldiv.parentNode.getElementsByClassName("play_new down_btn")[sel_arr[j]]);
		}
	}
}
...
aWindow.dwnld_start = function(i){
	...
	//i.getElementsByClassName("play_new down_btn")[j].click();
		dwnld_2arr(	i.getElementsByClassName("play_new down_btn")[j] );
	}
}
