// ==UserScript==
// @name           HighscoreScript v2.0
// @namespace      
// @description    HighscoreScript v2.0 by xXx_ptown67_xXx
   	...
   	url: 'http://berlin.pennergame.de/dev/api/user.' + id + '.xml',
        ...
        onload: function(responseDetails) {
			...
			if (id_gang != 0 && id_gang != 'None') {
				GM_xmlhttpRequest({
					method: 'GET',
					url: 'http://berlin.pennergame.de/dev/api/gang.' + id_gang +'.xml',
					onload: function(responseDetails) {
			...
			GM_xmlhttpRequest({
				...
				onload: function(responseDetails) {
					...
					try {
						var side_split_3 = side_split_2[0].split('<img style="margin-top:3px" src="http://berlin.pennergame.de/headline/');
					...
					try {
					    ...
					    var haustier = ' <img style="margin-top:3px" src="http://berlin.pennergame.de/headline/' + tier_ueberschrift +'/?size=28" /><br />'+ haustier_bild + '<br />' + tier_beschreib + tier_tip;
