// ==UserScript==
// @name           Yahoo Hockey Farm Players DFHL
// @namespace      http://glenncarr.com/greasemonkey
// @include        http://hockey.fantasysports.yahoo.com/hockey/5405*
// @description    Indicates players that are 'owned' outside of Yahoo's active roster
// $LastChangedRevision: 581 $
// $LastChangedDate: 2010-08-28 21:50:51 -0500 (Sat, 28 Aug 2010) $
...
var ELIGIBILITY_FOOTNOTE = '*Exceeds rookie eligibility requirements for players (>200 GP) and goalies (>100 GP)';

// If you set this value to true, you'll run the risk for having Yahoo disabled for a period of time.
// Yahoo detects repeated requests of the player stats (every time you visit your team page) and will
// start returning the status: 999 - Sorry, Unable To Process Request At This Time.  After a few hours
// it will allow requests again, but it's a pain.
var GET_STATS_AUTOMATICALLY = true;

function RookieEligibility( gamesStarted, gamesPlayed )
{
	...
	this.isEligible = function()
	{
		if ( this.gamesStarted ) 
			return ( this.gamesStarted <= 200 );
		else
			return ( this.gamesPlayed <= 100 );
	}
}

var matches = location.href.match( /^(http:\/\/hockey\.fantasysports\.yahoo\.com\/hockey\/(\d+)).*/i );
if ( matches == null )
	return;
...
var FARM_ROSTERS_URL = "http://docs.google.com/spreadsheet/pub?key=0AouyF0VNVxlHdGdRZzhRMU1SZjlaNm5wai04LVVBQ3c&output=txt";
...
//var FARM_ROSTERS_DATA_URL = FARM_ROSTERS_URL.replace( /\/pub/i, '/ccc' ).replace( /&output=txt/i, '' );
var FARM_ROSTERS_DATA_URL = FARM_ROSTERS_URL.replace( /\/pub/i, '/ccc' ).replace( /&output=txt/i, '' );
//var FARM_ROSTERS_DATA_URL = "http://docs.google.com/spreadsheet/pub?key=0AouyF0VNVxlHdGdRZzhRMU1SZjlaNm5wai04LVVBQ3c&output=txt";
...
//alert("for loop complete");
	var playerLinks = document.evaluate( '//a[contains(@href,"http://sports.yahoo.com/nhl/players/")]', document.body, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
	...
	//the next line is my attempt to make the farm team display at the bottom of the page
	var matches = location.href.match( /\d+/g);
//	alert(matches[1]);

//original line of code that was keeping the farm team from displaying at the bottom of the page	
//	var matches = location.href.match( /.*\/b\d+\/\d+\/(\d+).*/ );
	...
	if ( ownerID && owners.hasOwnProperty( ownerID ) )
	{
		...
		var tableLegend = document.getElementById( 'team-roster' );         
		...
		if ( lastRoster && tableLegend )
		{
            ...
            farmRoster.setAttribute("style","padding: 0px 0px 0px 0px; width: 750px;"); //change style so table does not inherit table properties from roster
			farmRoster.id = "gncFarmRoster";
            
//			tableLegend.parentNode.insertBefore( farmRoster, tableLegend ); 
            tableLegend.parentNode.insertBefore( farmRoster, tableLegend.nextSibling ); //moved tableLegend to different node and used nextSibling to insert farm table in empty space
            
			var header = document.createElement( 'h4' );
            header.setAttribute( "style", "margin: 0px 0px 0px 0px; padding: 0px 10px; background-color:#FFFFFF; width: 730px" ); //inserted more style adjustments to fit make the farm table look clean
            ...
            var html = new Array( '<thead><tr class="headerRow1"><th style="text-align:left; width: 150px; padding: 0px, 6px;">Player</th><th style="text-align:left; width: 150px; padding: 0px 0px;">Notes</th><th style="text-align:left; width: 150px; padding: 0px 0px;"><a id="gncGetCareerStats" href="#">Career Games/2013 Stats</a></th><th style="text-align:right; width: 75px; padding: 0px 0px;">GP</th><th style="text-align:right; width: 75px; padding: 0px 0px;">G/W</th><th style="text-align:right; width:75px; padding: 0px 0px;">A/GAA</th><th style="text-align:right; width: 75px; padding: 0px 6px 0px 0px;">PTS/SO</th></tr></thead>' );			
			for ( var iPlayer = 0; iPlayer < owner.players.length; iPlayer++ )
			{
				var cls = ( iPlayer % 2 == 0 ) ? "odd" : "even";
                var bg = ( iPlayer % 2 == 0 ) ? "FFFFFF" : "EDEDED";
				...
				{
					var playerHtml = '<a target="sports" href="http://sports.yahoo.com/nhl/players/' + owner.players[ iPlayer ].id + '">' + owner.players[ iPlayer ].name + '</a>' + playerSearch;
				}
				else
					var playerHtml = owner.players[ iPlayer ].name + playerSearch;

                html.push( '<tr class="' + cls + '" style="background-color:#' + bg + ';"><td class="player" style="width: 150px; padding: 0px 0px 0px 6px;"><div style="font-weight: normal; font-size: 12px;">' + playerHtml + '</div></td>' + 
                              '<td class="auto stat" style="font-weight: normal; font-size: 12px; width: 150px; padding: 0px 0px 0px 0px;">' + owner.players[ iPlayer ].comment + '</td>' + 
                              '<td class="gncFarmPlayerData" style="font-weight: normal; font-size: 12px; width: 150px; padding: 0px 0px 0px 0px; "><span id="' + owner.players[ iPlayer ].id + '"></span></td>' +
                          	  '<td class="gncGames" style="text-align:right; font-weight: normal; font-size: 12px; width: 75px; padding: 0px 0px 0px 0px; "><span id="gp' + owner.players[ iPlayer ].id + '"></span></td>' +
                              '<td class="gncGoals" style="text-align:right; font-weight: normal; font-size: 12px; width: 75px; padding: 0px 0px 0px 0px; "><span id="g' + owner.players[ iPlayer ].id + '"></span></td>' +
                              '<td class="gncAssists" style="text-align:right; font-weight: normal; font-size: 12px; width: 75px; padding: 0px 0px 0px 0px; "><span id="a' + owner.players[ iPlayer ].id + '"></span></td>' +
                              '<td class="gncPoints" style="text-align:right; font-weight: normal; font-size: 12px; width: 75px; padding: 0px 6px 0px 0px; "><span id="p' + owner.players[ iPlayer ].id + '"></span></td>' +
                              '</tr>' 
                             );

			}
			if ( iPlayer > 0 )
				html.push( '<tr><td colspan="100%" class="gncNotARookie" style="font-size:70%">' + ELIGIBILITY_FOOTNOTE + '</td></tr>' );
...
//farm data start
					...
					for ( var iPlayer = 0; iPlayer < playerElements.snapshotLength; iPlayer++ )
					{
                        ...
                        var playergncID = owner.players[ iPlayer ].id;
						...
						if ( !isNaN( parseInt( owner.players[ iPlayer ].id, 10 ) ) )           
						{
                            ...
                            var gpcell = document.getElementById('gp' + playergncID);
                            gpcell.innerHTML = '<img src="' + WORKING_IMG_URL + '" />';
                            var gcell = document.getElementById('g' + playergncID);
                            gcell.innerHTML = '<img src="' + WORKING_IMG_URL + '" />';                           
                            var acell = document.getElementById('a' + playergncID);
                            acell.innerHTML = '<img src="' + WORKING_IMG_URL + '" />';
                            var pcell = document.getElementById('p' + playergncID);
                            pcell.innerHTML = '<img src="' + WORKING_IMG_URL + '" />';                            
                            //busy gif for cells end
                            
                            
							GM_xmlhttpRequest({
								method: 'GET',
								url: 'http://sports.yahoo.com/nhl/players/' + owner.players[ iPlayer ].id,
								onload: getPlayerDataHandler( handlePlayerData, players[ owner.players[ iPlayer ].id ], playergncID ),
								});
						}
					}
//farm data end 
...
function getCareerStats( ownerID )  //this function does not seem to get called or actually do anything
{
//    console.dir(ownerID);
    var owner = owners[ ownerID ];
	var playerElements = document.evaluate( '//td[contains(@class,"gncFarmPlayerData")]', document.body, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
	...
	for ( var iPlayer = 0; iPlayer < playerElements.snapshotLength; iPlayer++ )
	{
		...
		player.dataCell = el.firstChild;
		player.dataCell.style.whiteSpace = 'nowrap';
		if ( !isNaN( parseInt( owner.players[ iPlayer ].id, 10 ) ) )
		{
			player.dataCell.innerHTML = '<img src="' + WORKING_IMG_URL + '" />';
			GM_xmlhttpRequest({
...
function getPlayerDataHandler( responseHandler, player, playergncID )
{
    return function ( responseDetails )
    {
        if ( responseDetails.status == 200 )
            responseHandler( responseDetails.responseText, player, playergncID );
    }
}

function handlePlayerData( responseText, player, playergncID )
{
    console.dir(playergncID);
    var spIDg = "g" + playergncID;
    var spIDa = "a" + playergncID;
    var spIDp = "p" + playergncID;
    var spIDgp = "gp" + playergncID;

    //clear gif from cells
        var tableGames = document.getElementById(spIDgp);                            
        tableGames.innerHTML = '';
        var tableGoals = document.getElementById(spIDg);                            
        tableGoals.innerHTML = ''
        var tableAssists = document.getElementById(spIDa);                            
        tableAssists.innerHTML = ''  
        var tablePoints = document.getElementById(spIDp);                            
        tablePoints.innerHTML = ''        
        
	    player.dataCell.innerHTML = '';
    	//end clear gif from cells
	...
	for ( var iTable = 0; iTable < tables.length; iTable++ )
	{
        ...
        console.dir(table);
//		var matches = table.rows[ 0 ].cells[ 0 ].innerHTML.match( /^(?:&nbsp;)*(Totals)$/i );  //old line that was not returning anything, now yahoo reverted back to old format, hopefully this works again
//		var matches = table.innerHTML.match( /;Year/i );
//      var matches = table.innerHTML.match(/Totals/i);     //this need to match totals but if player is active this season he will have 2 tables with totals due to new yahoo format inserting game log on player page
		var matches = table.innerHTML.match(/Year/i); 	//matching the header year to make sure the totals that pulls later is in the right table
//		console.dir(matches);
...
//            console.dir(row);
			...
			{
				var caption = row.cells[ iCell ].innerHTML.replace( /\b([a-z])\b/i, '$1' ).toLowerCase(); 		//converts all stat categories to lower case
				dataIndexes[ caption ] = iCell; //stores lower case inner html into a new array
			}

            for ( var iRow = 0; iRow < table.rows.length; iRow++ ) //changed iRow = 0, it was 2
			{
                ...
                else if ( /Totals/i.test( row.cells[ 0 ].innerHTML ) && getDataValue( row, 'gp' ) )  
				{
					var gamesPlayed = null;
					var gamesStarted = getDataValue( row, 'gp' );
					if ( gamesStarted )
					{
						gamesStarted = parseInt( gamesStarted, 10 ); 		
						player.dataCell.innerHTML = '<p style="text-align:left;">' + gamesStarted + ' career games</p>';
                    }
					else
					{
                        var gamesPlayed = getDataValue( row, 'gp' );
						if ( gamesPlayed )
						{
							gamesPlayed = parseInt( gamesPlayed, 10 );
							player.dataCell.innerHTML = '<p style="text-align:left;">' + gamesPlayed + ' career games</p>';
						}
					}

					var eligibility = new RookieEligibility( gamesStarted, gamesPlayed );
...
// reinserting for loop to do again to test if i can fill in stats this way *************************************************************************************************
            
  for ( var iTable = 0; iTable < tables.length; iTable++ )
	{
		var table = tables[ iTable ];
		var matches = table.innerHTML.match(/Year/i); 	//matching the header year to make sure the totals that pulls later is in the right table
		var offense = table.innerHTML.match(/offense/i);
        ...
        if ( matches )
		{
            ...
            for ( var iRow = 0; iRow < table.rows.length; iRow++ ) //changed iRow = 0, is was 2
			{
				var row = table.rows[ iRow ];
                
                if (offense)
                {    
						if ( /2013/.test( row.cells[ 0 ].innerHTML ) && getDataValue( row, 'gp' ) )  //season skater stats
                           {
                               
                            var seasonGames = getDataValue( row, 'gp' );                           
                            seasonGames = parseInt (seasonGames, 10);                            
                            tableGames.innerHTML = seasonGames + ' GP';   
                               
                            var seasonGoals = getDataValue( row, 'g' );                           
                            seasonGoals = parseInt (seasonGoals, 10);                            
                            tableGoals.innerHTML = seasonGoals + ' G';
                                                            
                            var seasonAssists = getDataValue( row, 'a' );
                            seasonAssists = parseInt (seasonAssists, 10);
                            tableAssists.innerHTML = seasonAssists + ' A';
                                
                            var seasonPoints = getDataValue( row, 'pts' );
                            seasonPoints = parseInt (seasonPoints, 10);
                            tablePoints.innerHTML = seasonPoints + ' PTS';
                            }
                }
                
               	else 
                {    
                            if ( /2013/.test( row.cells[ 0 ].innerHTML ) && getDataValue( row, 'w' ) )  //goalie stats
                            {
                            var seasonGames = getDataValue( row, 'gp' );                           
                            seasonGames = parseInt (seasonGames, 10);
                            tableGames.innerHTML = seasonGames + ' GP';     
                                
                            var seasonGoals = getDataValue( row, 'w' );
                            seasonGoals = parseInt (seasonGoals, 10);                            
                            tableGoals.innerHTML = seasonGoals + ' W';
                                
                           var seasonAssists = getDataValue( row, 'gaa' );
                           tableAssists.innerHTML = seasonAssists + ' GAA';
                               
                           var seasonPoints = getDataValue( row, 'so' );
                           seasonPoints = parseInt (seasonPoints, 10);
                           tablePoints.innerHTML = seasonPoints + ' SO';
                            }
