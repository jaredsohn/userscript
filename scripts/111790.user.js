// ==UserScript==
// @name           eRepublik battle master 汉化版
// @version        0.143 dev
// @author         eCitizen Scyld
// @namespace      eCitizenScyld
// @description    eRepublik 智能战场插件
...
// FUNKCJONALNOŚĆ
// ZhangXu @ SMTH汉化
// http://www.erepublik.com/en/citizen/profile/5107105
...
var BASE_URL	= arrURL[0] + '/' + arrURL[1] + '/' + arrURL[2] + '/' + arrURL[3] + '/';		
...
function bhStats(att, def) {
	...
	$j('div#myStatBoxL').html(
		'<div id="BHTableL">' +
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>公民</th><th>击杀数</th><th>战场影响力</th></tr>' + att + '</table></div>' +
		'</div>'
		);
	$j('#BHTableR').remove();
	$j('div#myStatBoxR').html(
		'<div id="BHTableR">' +
		'<div class="BHTable" style="float:right"><table class="BHTable"><tr><th>公民</th><th>击杀数</th><th>战场影响力</th></tr>' + def + '</table></div>' +
		'</div>'
	);
...
function histStats(hist) {
	...
	$j('div#myOverBox').html(
		'<div id="OOTable">' +
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>公民</th><th>击杀数</th><th>战场影响力</th></tr>' + hist + '</table></div>' +
		'</div>'
	);
...
function getRegionInfo(id)
{
	...
	$j('button#hospitals_get').html('请等待...').attr('disabled', 'true');
	...
	GM_xmlhttpRequest({
			...
			if (regionsData && regionsData.length > 0) {
				...
				for (var a = 0; a < regionsData.length; a++) {
					...
					if (regionsData[a].name == regionName && regionsData[a].buildings.hospitals.length > 0) {
						...
						$j('button#hospitals_get').html('无医院 - 也许需要稍等片刻').removeAttr('disabled');
		...
		onreadystatechange:	function(response) {
			...
			$j('button#hospitals_get').html('请等待...');

		},
		onerror: function(response) {

			$j('button#hospitals_get').html('出错请重试...');
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'</small></div></div>' +
			'<h3>影响力计算器' +
			'<span style="float:right">武器: <select id="dmgWeapon" size="1">' +
			...
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;# 攻击次数: <input id="dmgFights" name="dmgFights" value="60" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">go!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>60 次攻击造成的影响: </strong>' +
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 100, 60, 1) + '</h4>' +
			'<div class="stat"><small>对"天敌"影响力: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 100, 60, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	$j('div#enemy_defeated').before(
		...
		'<input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food">&nbsp;自动使用食物补充体力</label>' +
		'&nbsp;&nbsp;<input type="checkbox" id="multihit_hk" name="multihit_hk"><label for="multihit_hk">&nbsp;食物不足时,使用医疗包(0.5G/个)</label>' +
		...
		'<button id="hospitals_get">获取医院</button></div>' +
		'</div><br /><div id="bronId"></div>' +
		'<fieldset id="BLOG"><legend>v0.143 ZhangXu@SMTH 汉化版  战斗日志:&nbsp;&nbsp;<button id="battlelog_clear">清除</button></legend><div id="battlelog"></div></fieldset>'
	);
	...
	unsafeWindow.battleFX.hit = function() {
		...
		if (multiHitRunning) {
			...
			bLog('攻击完成!');
			$j('div#multihit_message').html('攻击次数: ' + multiHitDone + '&nbsp;&nbsp;杀敌数: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? '敌人已被击杀!' : '敌人依旧存活!') + ')');
	...
	unsafeWindow.battleFX.blow = function() {
		...
		if (multiHitRunning) {
			...
			bLog('ENEMY KILLED!');
			$j('div#multihit_message').html('Hits: ' + multiHitDone + '&nbsp;&nbsp;Kills: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? '敌人已被击杀!' : '敌人依旧存活!') + ')');
		}
