// ==UserScript==
// @name         The West - Market Helper
// @description  Market Helper, allows you to see towns from where you need to collect items
// @namespace    http://darkyndy.com/projects/thewestmarkethelper
// @icon         http://img854.imageshack.us/img854/4797/iconcwalter.jpg
// @license      GNU Lesser General Public License (LGPL)
// @copyright    2011, darkyndy
// @author       darkyndy
// @release	 CWalter
...
var dyMarketHelperGeneral = (function () {
	"use strict";
	var SCRIPT_VERSION = "1.2.8.3",
		SCRIPT_WEBSITE = "http://userscripts.org/scripts/show/125040",
...
var dyMarketHelperCode = function (currentVersion, scriptWebsite) {
	...
	function initializeParams() {
		var initTown = {"name": "Localização Atual", "x": 0, "y": 0, "extraClass": prefix + "Current", "id": "currentLocation"},
			initCoordsX = 0,
			initCoordsY = 0;
		lastPageParsed = -1;
		townInfo = [];
	...
	function createTabHelpContent() {
		...
		if ($helpContent.length === 0) {
			helpContentHtml = '<b>Geral</b><br/>' +
				'Versão: ' + currentVersion + '<br/>' +
				'Website: <a href="' + scriptWebsite + '" target="_blank" title="' + scriptWebsite + ', abrir em nova aba">' + scriptWebsite + '</a><br/>' +
				'<br/>';
	...
	function marketHelperTabClick(tabObj, tabId) {
		...
		if (tabId === prefix) {
			setMessage("Cidades de onde você precisa coletar itens comprados");
		} else {
	...
	function buildWindow() {
		var html = '<h2 id="' + prefix + 'Message">Carregando...</h2><br/>',
			tabName = "Compra do Mercado",
			$marketHelperTab = null;
		marketHelperTab = wman.open(prefix).setMiniTitle(tabName).setSize(840, 655)
			.addTab(tabName, prefix, marketHelperTabClick)
			.addTab("Ajuda", prefix + "Ajuda", marketHelperTabClick);
		...
		if (location.items) {
			...
			if (displayHeadingInfo) {
				...
				if (playerAtLocation) {
					...
					itemCollectHtml = '<a class="button_wrap button" href="javascript:void(0);">' +
						...
						'<span class="button_middle">Coletar Todos</span>' +
						'<span class="button_right"></span>' +
						'<span style="clear: both;"></a></span>';
				} else {
					itemCollectHtml = "Para coletar todos os itens que você precisa estar em " + locationName + " town";
				}
			...
			for (itemId in locationItems) {
				...
				if (!isNaN(itemId) && itemId > 0) {
					...
					locationItemTitle = locationItemTitle + '<br/> Você pode coletar ' + locationItemCountToCollect + ', ' +
						locationItemCountInAuction + ' ainda estão em leilão';
	...
	function townPurchasedItemsWindow(location) {
		...
		$tabHeader = jQuery("<h2>").attr({"class": prefix + "Message"}).html("Itens a serem adquiridos de " + locationName);
	...
	function paintTown(location) {
		...
		$townEl = jQuery("<div>")
			.attr({"id": baseLocationId + "OnMap", "class": setClass, "style": townStyle, "title": "Centrar no mapa: " + locationName})
			.click(function () {
		...
		$locationTownCenter = jQuery("<div>")
			.attr({"class": prefix + "TownCenter", "title": "Centrar o mapa nesta cidade: " + locationName})
			.click(function () {
				WMap.scroll_map_to_pos(locationX, locationY);
			});
		...
		$locationCollect = jQuery("<div>")
			.attr({"class": baseId + "Collect", "title": "Clique para ver os itens que podem ser coletados a partir de " + locationName})
			.click(function () {
	...
	function populateFiltersBlock($filters) {
		...
		purchasedItemsAllLabel = '<label for="' + prefix + 'PurchasedItemsFilterAll" ' +
			'title="Mostrar todas as cidades e itens">Todos</label>';
		//finish
		...
		purchasedItemsFinishLabel = '<label for="' + prefix + 'PurchasedItemsFilterFinish" ' +
			'title="Mostrar apenas as cidades e os itens que você pode coletar agora">Terminado</label>';
		//progress
		...
		purchasedItemsProgressLabel = '<label for="' + prefix + 'PurchasedItemsFilterProgress" ' +
			'title="Mostrar apenas as cidades e os itens de onde você colocou as propostas e do leilão não terminou">Progresso</label>';
	...
	function calculate() {
		if (marketHelperTabReopenEventInProgress) {
			alert("Market Helper está carregando, por favor espere...");
		} else {
			marketHelperTabReopenEventInProgress = true; 
			initializeParams();
			buildWindow();
	...
	function updateCallback(currentVersion, latestVersion) {
		var autoUpdateTab = null,
			...
			$updateHeader = null,
			contentHtml = "";
		autoUpdateTab = wman.open(updatePrefix + " nominimize noreload").setMiniTitle("Market Helper Atualização")
			.addTab("MH updater", updatePrefix, function () {});
		autoUpdateTabContent = jQuery(autoUpdateTab.divMain).find(".tw2gui_window_content_pane");
		$updateHeader = jQuery("<h2>").attr({"class": prefix + "Message"}).html("Market helper - Nova Versão Disponível");
		contentHtml = 'Você está usando a versão: ' + currentVersion + '<br/>' +
			'Última versão é: ' + latestVersion + '<br/>' +
			'Por favor, visite <a href="' + scriptWebsite + '" target="_blank">' + scriptWebsite + '</a> ' +
			'e atualizar para a ultima versão.<br/>' +
			'Após a atualização atualizar esta página, ou clique no link: ' +
