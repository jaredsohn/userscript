...
//
// ==UserScript==
// @name          Wykop, szybki zakop + liczba zakopow
// @namespace     http://diveintomark.org/projects/greasemonkey/
// @description   Wykop, szybki zakop + liczba zakopow
    ...
    function letsJQuery() {
		...
		if(a_jvote.length > 0) {
			...
			a_jvote.each( function() { 
				...
				//var wykop = $('<div></div>').load('http://www.wykop.pl/link/'+id+'/ ul.tabs');
				//var liczba_zakopow = $(wykop.find('ul li strong')[3]).html();
				var wykop = $('<div></div>').load('http://www.wykop.pl/link/'+id+'/ ul.tabs',

					function(response) {
					   var liczba_zakopow = $($(this).find('ul li strong')[3]).html();

var w_subdomain = unsafeWindow.window.location.hostname.split('.')[0];
					   //el1.append($('<div>'+liczba_zakopow+'</div>'));
					   el1.append($(' <div> <a href="http://'+w_subdomain+'.wykop.pl/link/zakopali/'+id+'/">zakopy(<span style="color:#FF772E;">'+liczba_zakopow+'</span>)</a></div>'));
					}
				);
