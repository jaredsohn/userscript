// ==UserScript==
// @name           ((DS - Speicherstände))-tribalwars arabic-www.plapl.com
// @namespace      Die Stämme
// @description	   Version 1.0.5 | Zeigt im Browsergame "Die Stämme" in der Dörferübersicht die Speicherstände der einzelnen Dörfer prozentual und farbig an
// @author         Roman S. (Zombie74)   وضع احصيات لي القريه من مورد  يستحق التجربه
...
var vers_ist = "DS - Speicherstände 1.0.5";
...
if(url.match(/screen=settings&mode=settings/)) {
	...
	if((url.match(/screen=/)) && 
		...
		if(confirm(vers_ist + " ist jetzt installiert\n\nKlicke auf OK um die Einstellungen für die Anzeige der Speicherstände vorzunehmen")) {
			document.location.href = "http://ae" + welt + "./game.php?village=" + dorf_ID + "&screen=settings&mode=settings&einstellung=speicher";
		}
		else {
...
// Nur in der Dörfübersicht anwenden:
	...
	if((welt >= 3) && (GM_getValue("Speicher-GAG-" + welt) == "an")) {
		...
		if(snob > 0) {
			...
			if(max_snob >= 1) {
				snob_gesamt_2 += snob;
				if(max_snob < snob){
					document.getElementsByClassName("box")[box].getElementsByTagName("td")[1].innerHTML += " <img src='http://de" + welt + ".die-staemme.de/graphic/unit/unit_snob.png' style='height:14px; cursor:help;' title='Du kanst in diesem Dorf " + max_snob + " von " + snob + " Adelsgeschlechtern produzieren'> <a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_ID + "&screen=snob'>" + max_snob + "</a><span class='grey'> /" + snob + "</span>&nbsp;";
				}
				else {
					document.getElementsByClassName("box")[box].getElementsByTagName("td")[1].innerHTML += " <a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_ID + "&screen=snob'> <img src='http://de" + welt + ".die-staemme.de/graphic/unit/unit_snob.png' style='height:14px; cursor:help;' title='Du kanst in diesem Dorf " + snob + " Adelsgeschlecht" + plus_snob + " produzieren'> " + snob + "</a><span class='grey'>/" + snob + "</span>&nbsp;";
				}
			}
			else {
...
if(url.match(/screen=overview_villages/)) {
	...
	// Mit PA:   
   	...
   	if(info >= 1) {
		...
		// kopf:
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'>اسم القريه</th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default;'>نقاط</th>";
		if(GM_getValue("Speicher-Zus-" + welt) == "an") {
			kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topright:10px;'><img src='http://de33.die-staemme.de/graphic/ally_rights/lead.png' title='ولاء'></th>";
		}

		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'><img src='/graphic/holz.png' title='خشب' alt='' /></th><th style='text-align:center; cursor:default; -moz-border-radius-topright:10px;'>خشب</th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'><img src='/graphic/lehm.png' title='طمي' alt='' /></th><th style='text-align:center; cursor:default; -moz-border-radius-topright:10px;'>طمي</th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'><img src='/graphic/eisen.png' title='حديد' alt='' /></th><th style='text-align:center; cursor:default; -moz-border-radius-topright:10px;'>حديد</th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'><img src='/graphic/res.png' title='المزارع' alt='' /></th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topright:10px;'>Max.</th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topleft:10px;'><img src='/graphic/face.png' title='السكان' alt='' /></th>";
		kopf += "<th style='text-align:center; padding-left:5px; padding-right:5px; cursor:default; -moz-border-radius-topright:10px;'>االتعداد السكاني الحالي</th>";
		if((welt >= 3) && (GM_getValue("Speicher-GAG-" + welt) == "an")) {
			if(welt >= 10) {
		...
		for(i=1; i<=zeilen; i++) {
			...
			zeile += "<td style='text-align:right; padding-right:5px; cursor:default;' title='Holz: " + proz_h + "%'>" + wert_h + "</td>";
			...
			zeile += "<td style='text-align:right; padding-right:5px; cursor:default;' title='Lehm: " + proz_l + "%'>" + wert_l + "</td>";
			...
			zeile += "<td style='text-align:right; padding-right:5px; cursor:default;' title='Eisen: " + proz_e + "%'>" + wert_e + "</td>";
			...
			if(GM_getValue("Speicher-Bau-" + welt) == "an") {
				zeile += "<td style='cursor:help; text-align:right; padding-left:5px; color:#fff; background-color:" + hgFarbe[pro_v] + "' title='Bauernhof: " + proz_v + "% (aktuell) | " + proz_b + "% (gesamt)'><small>" + proz_v + "%</small></td>";
				colspan = 1;
			}
			...
			if((welt >= 3) && (GM_getValue("Speicher-GAG-" + welt) == "an")) {
				...
				if(welt >= 10) {
					if(gold > 0) {
						gold_gesamt += gold;
						zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst " + gold + " Goldmünze" + plus_gold + " prägen'><a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_id[0] + "&screen=snob'>" + gold + "</a></td>";
					}
					else {
						zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst momentan keine Goldmünze prägen'><span class='grey'>" + gold + "</span></td>";
					}
				}
				else {
					if(gold > 0) {
						gold_gesamt += gold;
						zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst " + gold + " x Rohstoffe einlagern'><a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_id[0] + "&screen=snob'>" + gold + "</a></td>";
					}
					else {
						zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst momentan keine Rohstoffe einlagern'><span class='grey'>" + gold + "</span></td>";
					}
				}
				...
				if(snob > 0) {
					...
					if(max_snob >= 1) {
						snob_gesamt_2 += snob;
						if(max_snob < snob){
							zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst " + max_snob + " von " + snob + " Adelsgeschlechtern produzieren'><a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_id[0] + "&screen=snob'>" + max_snob + "</a><span class='grey'>/" + snob + "</span></td>";
						}
						else {
							zeile += "<td style='text-align:right; padding-left:5px; padding-right:5px; cursor:help;' title='Du kanst " + snob + " Adelsgeschlecht" + plus_snob + " produzieren'><a href='http://de" + welt + ".die-staemme.de/game.php?village=" + dorf_id[0] + "&screen=snob'>" + snob + "</a><span class='grey'>/" + snob + "</span></td>";
						}
					}
					else {
			...
			if(i == zeilen-1) {
				...
				inhalt += "<tr>";
				if(GM_getValue("Speicher-Zus-" + welt) == "an") {
					inhalt += "<th colspan='3' style='text-align:center; padding:2px; cursor:default; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px;'>المجموع الكلي</th>";
				}
				else {
					inhalt += "<th colspan='2' style='text-align:center; padding:2px; cursor:default; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px;'>Allgemeine Infos</th>";
				}
				...
				if(volk_gesamt_1 >= 1000000) {
					volk_gesamt_1 = (volk_gesamt_1/1000000).toFixed(2) + " <small class='grey'>Mio.</small>";
				}
