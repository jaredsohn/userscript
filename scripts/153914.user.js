 	...
 	ReportPopup: function (rslt, rpt, reportId) {
		...
		if (rpt.marchName == 'Scout' && rslt['winner']==1) {
 		...
 		if (rslt['fght']){ // not Reinforce or Transport, so we have a table with 2 columns: 1 for Attackers, 1 for Defenders
			...
			} else {m+='<TR><TD>No Troops Defended</TD></TR>'};
  ...
  show : function (){  
    ...
    if (t.state == null) {  
         ...
         m += "<TABLE width=600 class=ptTab border=0 align=center>\
           ...
           <td colspan=4 align=left><table border=0 bordercolor=black cellspacing=0 cellpadding=0 width=100% style='text-align:center'><tr><td rowspan=14><div id=RAAstatsource></div></td><td colspan=2><a href='javascript:void(0)' id=BO_RAZ_Units title='Clear' >Units Selected</a></td><td>Attack Time</td><td>Reinforce Time</td></tr>";
            for (r=1; r<14; r++){
	...
	m+='<TD></TD>Throne Room March Size Increase (%): <INPUT id=MarchSizeInc type=text size=5 value="'+ Options.MarchSizeInc +'">';
        m += "</table></td></tr>\
	...
	function metajourfavori() {
        ...
        document.getElementById('MarchSizeInc').addEventListener ('change', function () {
          var x = document.getElementById('MarchSizeInc').value; 
          Options.MarchSizeInc = x; 
          saveOptions();
        }, false);
	...
	ById("ptmarch_autoknight").addEventListener('click', function(){
  	...
  	for (r=1; r<14; r++) ById("RAAnbunit"+r).value=0; 
 	}, false); 
       ById("BO_AT_Fav_ajou").addEventListener ('click', function() {
        ...
        for (r=1; r<14; r++) lisf[r]=ById("RAAnbunit"+r).value;
    ...
    clickATTAQUEDo: function(typemarche, bouffe) {
      ...
      var e=1;
      var eTR = Options.MarchSizeInc/100 + 1;
	...
	var marchCityId = t.sourceCity.id
	if(Seed.cityData.city[marchCityId]["isPrestigeCity"] == true) {
		var pres = Seed.cityData.city[marchCityId]["prestigeInfo"]
		var preslvl = pres.prestigeLevel
		var maxtroupe=parseInt(niveauPointRall*10000*(1.5+.05*preslvl)*eTR*e);
         	if (niveauPointRall==11) maxtroupe=parseInt(150000*(1.5+.05*preslvl)*eTR*e);
         	if (niveauPointRall==12) maxtroupe=parseInt(200000*(1.5+.05*preslvl)*eTR*e);
		} else {
         	var maxtroupe=parseInt(niveauPointRall*10000*eTR*e);
         	if (niveauPointRall==11) maxtroupe=parseInt(150000*eTR*e);
         	if (niveauPointRall==12) maxtroupe=parseInt(200000*eTR*e);
		}
...
Tabs.Rpt = {
	...
	displayReportBody: function (rslt, reportId) {
		...
		if (rpt.marchName == 'Reinforce') {
			...
			t.popReport = new CPopup('pbShowOther', 0, 0, 550, 800, true, function() {clearTimeout (1000);});
			m+= '<DIV style="max-height:645px; height:645px; overflow-y:scroll">';
		}
		t.popReport.centerMe (mainPop.getMainDiv());
