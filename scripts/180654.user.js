// ==UserScript==
// @namespace       heroeswm
// @name            HWM Event Disable
// @description     Отключение блока с ивентом, который мешает корректной работе скриптов.
// @version         1.0.2
    ...
    for (var i in obj) {
        ...
        if (skipFields !== undefined)
            for (var j in skipFields)
                if (attrName == skipFields[j])
                    continue;
        // Обработка полей штампов времени
        if (timeMap[attrName])
            obj[attrName] = Time.toNumber(attrValue);
        // Значение поля
        var attrValue = obj[i];
        // Представление массива
        if ($.isArray(obj[i])) {
...
// [24.10] Экспансия на дикие земли
var node = XPath.findFirst('/html/body/center/table', document);
if ($(node).attr('style') == 'background: url(i/for_runet.jpg);')
    $(node).remove();
