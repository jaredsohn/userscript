// ==UserScript==
// @name       Acceptor & Inviter for FB FA
// @namespace  AcceptorInviterForFBFA
// @version    1.0 beta
// @description  Auto send Friend Request (FR) & Auto Accepting FR for Inviter and Acceptor team
// @include      *.facebook.com/reqs.php
// @include      *.facebook.com/pokes
...
function yogiPrepButtAsep(){
    if(typeof $('#contentArea .fbRequestList').get(0) != 'undefined'){
        ...
        asepHeadTarget.after('<a id="yogiReloadButt" class="uiButton">Refresh</a> <a id="yogiPokeBoxButt" class="uiButton" href="javascript:window.location.replace(\'/pokes\')"><img class="img mrs" src="http://static.ak.fbcdn.net/rsrc.php/v2/y2/r/W8TFwFc9d1E.gif" alt="" width="16" height="14">Goto Poke Inbox</a> <label for="yogiAsepAll" class="_11b uiButton uiButtonConfirm"><input type="submit" id="yogiAsepAll" value="Accept All '+$('.fbRequestList input[name="actions[accept]"]').length+'"></label>');
        asepHeadTarget.hide();
    }else if(/^\/reqs.php/.test(location.pathname)){
        $('#contentArea .uiHeader:first .uiHeaderTop').prepend('<div class="rfloat"><a id="yogiReloadButt" class="uiButton">Refresh</a> <a id="yogiPokeBoxButt" class="uiButton" href="javascript:window.location.replace(\'/pokes\')"><img class="img mrs" src="http://static.ak.fbcdn.net/rsrc.php/v2/y2/r/W8TFwFc9d1E.gif" alt="" width="16" height="14">Goto Poke Inbox</a></div>');
    }
}
yogiPrepButtAsep()
// ======= FR Poker =========
...
function frAllPoker(){
    ...
    // get poker list
    $('.pokesDashboard a.uiIconText[ajaxify]').each(function(index) {
        // get poker id
        var pokerId = $(this).attr('ajaxify').split('uid=');
        pokerId = pokerId[1];
        // prepare param
        var qs = '__a=1&__user='+ top_cfg.user +'&action=add_friend&ego_log_data=&fb_dtsg='+ top_cfg.fb_dtsg +'&how_found=hovercard&http_referer=&logging_location=&no_flyout_on_click=true&outgoing_id=&ref_param=none&to_friend='+pokerId;
        var phsstamp_val = generatePhstamp(qs, top_cfg.fb_dtsg);
        // send FR
        yogi_poke_butt = $(this).closest('td.prs');
        yogi_fr_submit[top_cfg.user] = $.ajax({
            type: 'POST',
            url: '/ajax/add_friend/action.php',
            data: qs+'&phstamp='+phsstamp_val
        }).done(function(msg) {
            yogi_poke_butt.html('<em style="color:#090">FR send!</em>');
        }).fail(function(jqXHR, textStatus) {
            yogi_poke_butt.html('<em style="color:#900">FR fail!</em>');
        });
        // hide poker
        $(this).trigger('click');
    });
...
function yogiPrepButtFR(){
    if(typeof $('.pokesDashboard a.uiIconText[ajaxify]').get(0) != 'undefined'){
        $('#yogiPokerAdd').live("click", function() {
            $(this).parent().hide();
            frAllPoker();
        });
        // add global button
        var pokerTarget = $('#pagelet_pokes .uiHeader').find('.rfloat .uiHeaderActions');
...
function set_timer_reqs(){
	if(typeof $('#yogiPokeBoxButt').get(0) == 'undefined'){
        yogiPrepButtAsep();
...
function set_timer_poke(){
	if(typeof $('#yogiAsepBoxButt').get(0) == 'undefined'){
        yogiPrepButtFR();
        yogiT = setTimeout(function(){ set_timer_poke() }, 1000);
    }else{
        clearTimeout(yogiT);
        console.log("Poke Inviter for FB FA - loaded");
    }
}
console.log("Acceptor & Inviter for FB FA - Loading");
if(/^\/reqs.php/.test(location.pathname)) set_timer_reqs();
if(/^\/pokes/.test(location.pathname)) set_timer_poke();