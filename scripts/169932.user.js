...
function hideUebersicht(){
	$1('#aktienuebersicht').slideUp('fast');
	GM_setValue('hideinguebersicht',true);
}

function showUebersicht(){
	$1('#aktienuebersicht').slideDown('fast');
...
// .
// ..
// stevo addon - aktienwert summe
// ..
// .


//var x = document.getElementsByClassName('tablesorter')[1];
// alert(x);
//var y = x.getElementsByTagName('tr')[1];
//alert(y.textContent);

//var x = $('.tablesorter')[1];
//alert(x);

//var trs					= $(tablesorters 'tr');

//var anzahl_trs 			= trs.length;




var tablesorters		= $('.tablesorter');
var anzahl_tablesorters = tablesorters.length;
...
// alert(anzahl_tablesorters);
var i = 1;
while (i < anzahl_tablesorters) {
 	
    var wert_damals_gesamt = parseInt(0);
    var wert_heute_gesamt = parseInt(0);
    
    var this_tablesorter = $('.tablesorter')[i];
    var trs 		= this_tablesorter.getElementsByTagName('tr');
    var anzahl_trs 	= trs.length;
    
    var z = 1;
    while (z < anzahl_trs) {
        
        var this_tr = this_tablesorter.getElementsByTagName('tr')[z];
        var wert	= this_tr.getElementsByTagName('td')[2].textContent;
        wert = wert.replace('Damals/Heute','');
        wert = wert.replace('.','');
        wert = wert.replace('.','');
        wert = wert.replace(' k','000');
        wert = wert.replace(' k','000');
        wert = wert.replace(',','.');
        wert = wert.replace(',','.');
        
        var wert_damals = wert.split(' /')[0];
        var wert_heute = wert.split('/')[1];
        
        wert_damals = parseFloat(wert_damals).toFixed(2);
        wert_heute  = parseFloat(wert_heute).toFixed(2);
       
        wert_damals = (wert_damals * 1);
        wert_heute  = (wert_heute * 1);
        
        wert_damals_gesamt = (wert_damals_gesamt + wert_damals);
        wert_heute_gesamt = (wert_heute_gesamt + wert_heute);
        
            
        var z = (z + 1);
    }
    
    var aktienname = document.getElementsByClassName('big')[i+1].textContent;
    
    wert_differenz_gesamt = (wert_heute_gesamt - wert_damals_gesamt);
    wert_differenz_gesamt = wert_differenz_gesamt.toFixed(2);
        
    if (wert_differenz_gesamt < 0) {
     	var color = '#cc0000';   
    }
    else if (wert_differenz_gesamt > 0) {
     	var color = '#00bb00';   
    }
    else {
     	var color = '#000000';   
    ...
    if (wert_heute_gesamt > 1000) {
     	wert_heute_gesamt = (wert_heute_gesamt / 1000).toFixed(0) + ' k';   
    }
    
    var div = document.createElement('div');
    div.textContent = '€ ' + wert_differenz_gesamt;
    ...
    divtext.textContent = aktienname;
    divtext.style.float = 'left';
    divtext.style.color = color;
    divtext.style.width = '120px';
    divtext.style.fontSize = '0.8em';
    divtext.title = wert_heute_gesamt;
