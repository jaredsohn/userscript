...
// @version        1.4.1.1
// @author         WildKatana | Updated by CodeEcho, PythEch, Matthias Fuchs, Enceladus, KRS_L, TheLuminary, Panavia2, Da Xue, MrHIDEn, TheStriker
    ...
    function CreateTweak() {
      ...
      qx.Class.define("TASuite.main", {
        ...
        members: {
          ...
          initialize: function () {
            this.add_ViewModeChange = (new ClientLib.Vis.ViewModeChange).EZTRJX(this, this.onViewChange); //
            this.add_ArmyChanged = (new $I.ZWQPBC).EZTRJX(this, this.onUnitMoved); //
            ...
            var battleUnitData = $I.FEUCPB.prototype;
            ...
            function () {
              try {
                ...
                for (var i in g.GetGamedata().units) { //
          ...
          getCityPreArmyUnits: function () {
            ...
            return own_city.get_CityArmyFormationsManager().GetFormationByTargetBaseId(cityId);
          },
          calculateLoot: function () {
            ...
            if (city.get_CityBuildingsData().QJACWO !== null) {
              // every building
              num = city.get_CityBuildingsData().QJACWO.l.length;
              for (var j = num; --j >= 0;) {
                var building = city.get_CityBuildingsData().QJACWO.l[j];
                // TODO: check for destroyed building
                var mod = building.get_HitpointsPercent();
                for (var i = building.LYSUOU.length; --i >= 0;) {
                  spoils[building.LYSUOU[i].Type] += mod * building.LYSUOU[i].Count;
                }
              }
            }

            // every unit
            if (city.get_CityUnitsData().NPNOZY() !== null) {
              num = city.get_CityUnitsData().NPNOZY().l.length;
              for (j = num; --j >= 0;) {
                var unit = city.get_CityUnitsData().NPNOZY().l[j];
                mod = unit.get_HitpointsPercent();
                for (i = unit.LYSUOU.length; --i >= 0;) {
                  spoils[unit.LYSUOU[i].Type] += mod * unit.LYSUOU[i].Count;
                }
              }
            }
          ...
          calculateSimResults: function () {
            ...
            while (battleground.ONNIXQ.LODJJI(false)) {}
            // DoStep$0 was renamed to LODJJI, m_Simulation was renamed to ONNIXQ
          ...
          calculateTroopStrengths: function (battleground) {
            ...
            var repair_times = own_city.get_CityUnitsData().BWBTIV.d; // m_FullRawRepairTimeForUnitGroupTypes renamed to BWBTIV
            var r_types = ClientLib.Base.EResourceType;

            var entities = battleground.DMKNSO.d; // m_Entities has been renamed to DMKNSO
            for (var i in entities) {
              var entity = entities[i];
              var i_entity = entity.YRCNTZ; // get_Entity$0() has been removed. Propery is $I.TQL - YRCNTZ
              var a_entity = entity.EONTCV; // ??? has been renamed to GUXBBT
              var current_hp = i_entity.WSPXEI; // m_iHitpointsCurrent has been renamed to WSPXEI
              var max_hp = i_entity.RHLWJC; // m_iHitpoints has been renamed to RHLWJC
              if (a_entity.SGPXHP === 2) { // ??? has been renamed to SGPXHP, Attacker is 2
                // This is one of the good guys
                end_hp += current_hp;
                total_hp += max_hp;
                switch (a_entity.QNGNTE) { // movement type has been renamed toQNGNTE
                ...
                if (i_entity.LEBSLX >= 200 && i_entity.LEBSLX <= 205) {
                  this.SupportLevel = parseInt(i_entity.LOAPZG); // m_iLevel has been renamed to LOAPZG
                  this.lastSupportPercentage = (current_hp / max_hp) * 100;
                } else {
                  switch (i_entity.LEBSLX) {
                    // m_MDCTypeId has been renamed to LEBSLX
                  case 112:
                    // GDI YARD
                  case 151:
                    // NOD CY
            ...
            this.totalSeconds = (battleground.ONNIXQ.VHMXVX * battleground.get_TimePerStep()) / 1000;
            ...
            crd.ConvertRepairCost = crd.AVTFSR; // ConvertRepairCost has been renamed to AVTFSR
          ...
          setupBattleground: function (offense_units) {
            try {
              ...
              combatData.TQBOJM = 1; // Version is TQBOJM

              var unitData = own_city.get_CityUnitsData().WFLPSU().l; // Attacker Units renamed  to WFLPSU
              if (offense_units) {
                offense_units = offense_units.get_ArmyUnits().l;
              } else {
              ...
              combatData.MPHTDS = data; // Attackers renamed to MPHTDS

              data = new Array();
              if (current_city.get_CityUnitsData().NPNOZY() !== null) { // empty defender army
                unitData = current_city.get_CityUnitsData().NPNOZY().l; // defender units
              ...
              combatData.CLMYRK = data; // Defenders renamed to CLMYRK
              ...
              combatData.QKOWUW = data; // Terrain renamed to QKOWUW

              unitData = current_city.get_CityBuildingsData().QJACWO.l; // City buildings renamed to QJACWO
              ...
              combatData.ZZSWHY = data; // Buildings renamed to ZZSWHY

              combatData.PNZZTP = null; // Support Structures renamed to PNZZTP
              combatData.SXNSJR = 8696244; // Start Step - this is just a random number - renamed to SXNSJR
              ...
              combatData.m_AttackerFaction = own_city.get_CityFaction();
              ...
              if (((combatData.m_DefenderFaction === ClientLib.Base.EFactionType.FORFaction) || (combatData.m_DefenderFaction === ClientLib.Base.EFactionType.NPCBase)) || (combatData.m_DefenderFaction === ClientLib.Base.EFactionType.NPCCamp)) {
                combatData.MHXIXA(); // This might not be needed
              }

              combatData.m_MaxDuration = 120;
              combatData.m_Complete = false;
              combatData.PSBCFR = null; // Debug renamed to PSBCFR

              var battleground = ClientLib.Vis.VisMain.GetInstance().get_Battleground();
              battleground.Reset();
              battleground.GKBWZE = combatData; // m_currentreplay
              battleground.InitBattle();
              battleground.XWJUID(combatData); // Set combat data
          ...
          restoreFormation: function (saved_units) {
            ...
            units.TFUYRR(); // RefreshData() has been renamed to TFUYRR()
          },
