...
// from 211.68.68.41 -begin-
sv=0;sv1=0;
v6='http://ip6.gw.bupt.edu.cn:9002/v6                       ';
myv6ip='                                       ';
v4serip='211.68.68.41';
m46=0;v46ip='118.229.0.0';
...
function cc(ss){document.getElementsByName("f0")[0].R1.value=ss;}
function ee(){
	var f0=document.getElementsByName("f0")[0];
	var f1=document.getElementsByName("f1")[0];
...
// from 211.68.68.41 -end-


function login(card,MD5pass){
	f0.DDDDD.value=card;
	if(ps==0){
		//f0.upass.value=xproc1(f1.upass.value);
		alert("ps=0,I don't know what to do")
		return false;
	}else{
		f0.upass.value=MD5pass;
		f0.R2.value=1;
	}
	f0.submit();
	return false;
...
function onLoad(event) {
	if(document.getElementsByName("f1")[0]){
		var card=getCookie("card");
		var pass=getCookie("MD5PassWithSalt");
		if(card!=null && pass!=null){
			login(card,pass);
			return false;
		}
		card=window.prompt("please input your card num:","2011140247");
		pass=window.prompt("please input your password:","");
		document.getElementsByName("DDDDD")[0].value=card;
		document.getElementsByName("upass")[0].value=pass;
		cc(0);
		ee();
	}
}
