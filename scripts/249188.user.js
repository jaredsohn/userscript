// ==UserScript==
// @name        Comunio - SCRIPT - Beta - kikon-v1.1
...
if( window.location.href.indexOf( "standings.phtml" ) != -1) {
	...
	//kikie//
	//console.log(tablePuntos.length);
	var trsPuntos = tablePuntos.getElementsByTagName("tr");
	//kike//
	console.log(trsPuntos.length);
	console.log(trsPuntos[3]);
	for (var i=0,len=trsPuntos.length; i<len; i++){
    trsPuntos[i].onclick = function(){
    ...
    //kike edit//
		...
		if (tdsPuntos[rowName].innerHTML.indexOf('playerInfo.phtml?pid=' + userId + '"') != -1) {
		//edit
			difPuntosTd.innerHTML = parseInt(tdsPuntos[(rowName+1)].textContent) + parseInt(ladiferencia[i]);
			...
			//kike edit//descomentar la siguiente linea y eliminar la de la diferencia//
			//var diferenciaPuntos = parseInt(puntosHTML) - misPuntos;//
			var diferenciaPuntos = parseInt(puntosHTML) + parseInt(ladiferencia[i]);
			if (diferenciaPuntos > 0) {
				difPuntosTd.style.color = "GREEN";
...
//kike edit//
divCajaClasificacion.innerHTML = "<span class=\"button02\"><a title=\"Clasificacion\" href=\"JavaScript:;\" onclick=\"window.open('https://docs.google.com/document/d/17fmb7tZJiMzYmPS9T3fbO-dVl4Rq-vXinDvG3fs5oao/pub?utm_source=scriptbeta&utm_medium=dinero&utm_campaign=scripts')\">Clasificacion</a></span>";		
