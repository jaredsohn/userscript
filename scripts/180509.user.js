// ==UserScript==
// @name        probux script 06/10/2013 WORKING!
// @namespace   probux script 06/10/2013 WORKING!
...
// @require     http://userscripts.org/scripts/source/179349.user.js
// @require     http://userscripts.org/scripts/source/179346.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link.com/*
// @include     *://*oc.us/*
...
wparent.location.href.indexOf('ads.php') != -1
	...
	div.css({zIndex:100000000,textAlign:'center',padding:5,position:'fixed',width:399,height:20,background:'#75D9FA',border:'2px solid green',bottom:10,right:10})
		.text('clicking: '+clickNum+' loading : '+ loading);
	$('body').css({position:'relative'}).append(div);
	$.each($('.anc-style'),function(k,v){
		objj[k];
		if($(v).attr('onclick')){
			if($(v).attr('onclick').indexOf('./viewad.php?') >= 0 && !$('.anuncio-clicked-title',$(v)).size()){
				var obj = {
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			if(typeof adsWind == 'undefined')
				adsWind = wparent.open(arr[ctr].href,"probux","width=100,height=50,top=1000,left=20000");
			else	
...
wparent.location.href.indexOf('viewad.php?') != -1||
...
wparent.location.href.indexOf('gridview.php?') != -1) && self == top){
	var div = $('<div>');
	var ajaxCall = false; //Changed from false
	div.css({font:'bold 10px arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:80,height:20,background:'#75D9FA',border:'2px solid green',top:10,left:10})
		.text('');
...
if(wparent.location.href.indexOf('progrid.php') != -1){
	...
	var clickNum = ctr + 1 ;
	var loading = 0;
	div.css({zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:399,height:20,background:'#75D9FA',border:'2px solid green',bottom:10,right:10})
		.text('');
	$('html').css({position:'relative'}).append(div);
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			...
			{
				adsWind = wparent.open(arr[ctr].href,"probux","width=100,height=50,top=1000,left=20000");
				//var point = getElementTopLeft("drawTable");
				//parent.open(point.top + " " + point.left);
				//document.elementFromPoint(point.left + lft * 20 - 5, point.top + rgt * 20 - 5)).click();  
			}
			else	
				adsWind.location.href = arr[ctr].href;
			(adsWind).ready(function(){adsWind.resizeTo(600,600)})	
