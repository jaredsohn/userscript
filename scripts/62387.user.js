...
var SE={
	...
	se_config : {
        ...
        "爱站":{
			"show":1,
			"百度权重":{"show":1, cfg:["http://www.aizhan.com/getbrinfo.php?url=#d", "/brs/", ".gif\" width=\"35\"", null, null, null, "utf-8"]},
			"IP":{"show":1, cfg:["http://www.aizhan.com/getbrinfo.php?url=#d", "baidu_ip').innerHTML = \"", "\";", null, null, null, "gb2312"]}
			},
		"alex":{
			"show":1,
			"昨日排名":{"show":1,
                         cfg:["http://cn.alexa.com/siteinfo/#d", "<th>昨日</th><td>", "</td>", this.domain+"该网址错误"]
                    },
			"七天平均":{"show":1,
                        cfg:["http://cn.alexa.com/siteinfo/#d", "<th>最近七天平均</th><td>", "</td>", this.domain+"该网址错误"]},
			"三月平均":{"show":1,
                        cfg:["http://www.aizhan.com/siteall/#nd","<td>三月平均：<span id=\"alexa_3months\" style=\"font-weight:bold\" class=\"orange\">", "</span>",null, null, null, "utf-8"]},
			"IPa":{"show":1, cfg:["http://www.aizhan.com/siteall/#d", "ALEXA数据预估流量：<span id=\"alexa_IPPV\">", "<\/span>", null, null, null, "utf-8"]}

			},
		"域名":{
			"show":1,
			"注册时间":{"show":1,
                        cfg:["http://www.aizhan.com/ajaxAction/whois.php?domain=#nd", "$(\"#domain_date\").html(\"", "\");<\/script>", "o match", null, null, "utf-8"]}
			}
            ,
		"雅虎":{
			"show":0,
	...
	show : function(d){
		...
		$("#se_config_link").click(function(){
			...
			//check for update
			SE_Tool.get_part(
				"http://userscripts.org/scripts/source/62387.meta.js",
				"// @uso:timestamp",
				"// @uso:installs",
				"无此字符",
				$("#se_check_version"),
				...
				function(){
					...
					if( GM_getValue('last_version')!=undefined && new Date(GM_getValue('last_version'))<new Date(s) ){
						$("#se_check_version").html("<span style='color:red'>有新版本</span>，<a href='http://userscripts.org/scripts/source/62387.user.js' id='install_it'>点击此处安装</a>");
