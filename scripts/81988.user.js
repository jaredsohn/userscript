...
var a = "\
   Authors:\
	 ...
	 New modifications by Hampage (improved to work with Farming Machine POWER).\
";
/**
 * Date:
...
var tempoAtaque = 30;
var tempoAtaqueIminente = 5;
var tempoEsperaPosAtaque = 2;
var ativaRefreshOutraVila = false;
...
//FM_log('(GM_getValue("troopTarget", "")) = '+(GM_getValue("troopTarget", "")));

var troopTarget = new Array();
if (GM_getValue("troopTarget", "") != "") {
	troopTarget[0] = GM_getValue("troopTarget", "").split(",");
	troopTarget[0][2] = 1; // 1-reinforce, 2-attack, 3-raid.
}
else
	setCoordinates();
//troopTarget[0] = [223,-214,1];
//troopTarget[1] = [-72,256,1];
//troopTarget[2] = [-71,261,1];

//GM_log("AntiFarm_EMAD - target Village = "+troopTarget[0]);
...
const tmSaveCallBackPercent = 0.70;
...
var soundUrl = "http://simplythebest.net/sounds/WAV/WAV_files/movie_WAV_files/20th_century_fox.wav";
...
// Now sub-functions will be defined
   ...
   */
  function TS_debug(text, forceDebug)
  {
    if ((logDebugMesages) || (forceDebug))
      FM_log("TS_debug - " + text);
  }
...
function checkForAttackAndInitSaveTroops(doc)
{
  ...
  if (tag.snapshotLength > 0)
  {
    ...
    if (totSeconds <= tempoAtaque*60) /* alterei para Se for menor quer o tempoAtaque */
    {
    ...
    if ((autoReload) && (reloadOn))
      str =  "auto refresh (to next village) in "+formatTimeStr(tmIdleReloadExact);
    else
...
function setCoordinates() {
	...
	troopTarget[0] = GM_getValue("troopTarget", "");
	
	if (troopTarget[0] != "")
		arr = troopTarget[0].split(",")
	else
		arr = [0, 0];
	...
	if(confirm) {
		x_coor = new_x;
		y_coor = new_y;
		GM_setValue("troopTarget", new_x + "," + new_y + ",1");
	}
...
function FM_log(level, text) {
  ...
  var logAtivado = false;
  
  if (logAtivado)
	if ((level) && (text))
		GM_log(horaAtual() + " - "+level+", "+text);
	else
		GM_log(horaAtual() + " - "+level);
		
}

GM_registerMenuCommand("Travian Antifarm POWER: Set coordinates", setCoordinates);
