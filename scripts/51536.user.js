...
var i_names = GM_getValue( "hwm_army_name" ) ;
...
for( var i = 0; i < anchors.length; i++ )
{
	...
	if( el.href == 'http://www.lordswm.com/map.php' )
	{
		...
		d.id = 'hwm_army_get' ;
		addStyle( '#hwm_army_get { position: absolute;background:#FFF;top:40px;margin-left:25px;border: 1px solid #abc;padding: 5px; }' );
		...
		d1.id = 'hwm_army_sets' ;
		addStyle( '#hwm_army_sets { padding: 5px; display:none; }' );
...
{
	div_army_sets = document.getElementById( 'hwm_army_sets' )
...
function get_set_all( t )
{
	...
	var i_name = GM_getValue( "hwm_army_name_" + t ) ;
	var i_set = GM_getValue( "hwm_army_set_" + t ) ;
...
function test()
{
	...
	var i_set = GM_getValue( "hwm_army_set_" + set ).split('|') ;
	var data = '' ;
...
function del_i_set()
{
  ...
  var i_id = '' ;
  i_name = GM_getValue( "hwm_army_name" ).split( '|' ) ;
  ...
  GM_setValue( "hwm_army_name" , i_id ) ;
  return;
}
...
function save_cur_set()
{
  ...
  i_name = GM_getValue( "hwm_army_name" ) ;
  if( !i_name )
  {
    i_name = '' ;
  }
	...
	if( set == 0 )
	{
		...
		GM_setValue( "hwm_army_name" , i_name ) ;
		GM_setValue( "hwm_army_set_" + time_cur , i_id ) ;
		GM_setValue( "hwm_army_name_" + time_cur , encodeURIComponent( str ) ) ;
		td = document.getElementById( 'armysets' )
		td.appendChild( get_set( time_cur ) ) ;
	} else
	{
		GM_setValue( "hwm_army_set_" + set , i_id ) ;
		GM_setValue( "hwm_army_name_" + set , encodeURIComponent( str ) ) ;
...
function get_set( t )
{
	...
	if( t > 0 )
	{
		i_set = GM_getValue( "hwm_army_set_" + t ).split('|') ;
		i_name = GM_getValue( "hwm_army_name_" + t ) ;
	}
