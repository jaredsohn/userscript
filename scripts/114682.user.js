// ==UserScript==
// @name          MMHK-VicesEtRales-Gnous
// @namespace     http://mightandmagicheroeskingdoms.ubi.com/play
// @description   Script Viscéral pour MMHK par l'alliance Vices et Râles (v.81) Modifié par Gnous
...
//  repeat:function(i, m) { for ( var o = []; m > 0; o[--m] = i) {} return o.join(''); },
 ...
 localStorage:
 {
  ...
  // TODO : et pourquoi elle s'appelle pas juste get() elle là ?
 ...
 // cette fonction (API.dump), empruntée au script Legends, permet d'afficher dans la "console" locale le contenu d'une variable
 // elle n'est utilisée (sauf oubli de ma part) que lors de mes tests.
 // c'est pour cette raison qu'elle est en commentaire.
 // mais par simplicité pour moi je la laisse en commentaire au lieu de la supprimer
...
//         else
//         {
//          if ( H.ArrayUtils.isArray(v) ) { li.appendChild(cE('SPAN', {className:'VRlog_array', innerHTML:'Array (éléments : ' + v.length + ')'})); }
...
// stopEvent = API.EVT.stop,
 ...
 l10n =
 {
  ...
  btnDeplacement:{FR:'Afficher/Masquer les trajets déja parcourus', EN:'Show/Hide hero moves already travelled'},
  ...
  btnReperes:{FR:'Repères', EN:'Shortcuts'},
  btnSentinelle:{FR:'Sentinelle', EN:'Sentinel'},
  btnSieges:{FR:'Sièges détectés', EN:'Seats found'},
  btnStock:{FR:'Production', EN:'Production'},
  cfgMarketLegend:{FR:'Place du marché', EN:'Market Place'},
  ...
  checkCommandUpdate:{FR:'Vices et Râles (v%d) - Vérifier la mise à jour', EN:'Vices et Râles (v%d) - Check for update'},
  checkForUpdate:{FR:'Vérifier la mise à jour', EN:'Check for update'},
  clearFilters:{FR:'Effacer les filtres', EN:'Clear filters'},
  configGenerique:{FR:'Générique', EN:'Main'},
  configMAJ:{FR:'Mises à jour', EN:'Updates'},
  configOpenOnStart:{FR:'Ouverts au démarrage', EN:'Opened at start'},
  ...
  coordsInvalides:{FR:'Coordonnées invalides', EN:'Invalid coordinates'},
  customLink:{FR:'Ajouter une url personnalisée', EN:'Add a custom url'},
  deliveryCaravan:{FR:new RegExp('La caravane de ([\u0000-\uFFFF]+) livre la cité de ([\u0000-\uFFFF]+)', 'i'),
                   EN:new RegExp("([\u0000-\uFFFF]+)'s caravan is delivering ([\u0000-\uFFFF]+) city", 'i')},
  deliveryRelay:{FR:new RegExp('Le relais de ([\u0000-\uFFFF]+) livre la cité de ([\u0000-\uFFFF]+)', 'i'),
                 EN:new RegExp("([\u0000-\uFFFF]+)'s relay is delivering ([\u0000-\uFFFF]+) city", 'i')},
  detailDistance:{FR:'%1$s est à %2$s régions au %3$s de %4$s (%5$s,%6$s)',
  ...
  disponibilites:{FR:'Disponibilités :', EN:'Availability :'},  MGuildSelectASpell:{FR:'Sélectionnez un sort dans la liste ci-contre', EN:'Please select a spell in the list'},
  distanceCalculer:{FR:'Calculer la distance en nombre de régions traversées', EN:'Calculate the distance in number of areas crossed'},
  distanceFrom:{FR:'de', EN:'from'},
  distanceTo:{FR:'vers', EN:'to'},
  errStructure:{FR:"La structure du jeu est différente. Cette version n'est plus compatible.",
                EN:'The structure of the game is different. This version is not compatible anymore.'},
  errorLocalStorage:{FR:"Vices et Râles\nlocalStorage n'est pas activé.\nDans la barre d'url tapez,  about:config\npuis placez à 'true' la clé 'dom.storage.enabled'",
                     EN:"Vices et Râles\nlocalStorage is not active.\nIn the url bar type,  about:config\nthen set to 'true' the key 'dom.storage.enabled'"},
  errorWhileCheckingVersion:{FR:'Une erreur est apparue pendant la vérification de mise à jour :\n%s', EN:'An error occurred while checking for an update:\n%s'},
  ...
  HerosEligibles:{FR:'Héros éligibles', EN:'Heroes eligible'},
  HerosEligiblesNone:{FR:'Aucun héro éligible', EN:'No hero eligible'},
  ...
  liste_diffusion_explain:{FR:"Ajoutez un ou plusieurs destinataires hors de l'alliance à votre liste en les séparant par des points virgule. Exemple : Joueur1;un autre joueur;unTroisième",
  ...
  marketPaidPrice:{FR:'Prix réel', EN:'Real price'},
  marketRetour:{FR:'Retour', EN:'Back'},
  marketShowPrice:{FR:'Prix affiché', EN:'Shown price'},
  marketYouBuySell_raise_OFFER:{FR:'Achat aux enchères', EN:'Buying at auction'},
  marketYouBuySell_raise_REQUEST:{FR:'Vente aux enchères', EN:'Selling at auction'},
  ...
  newVersionConfirmInstall:{FR:"La version %s du script Viscéral est disponible.\nPensez a recharger le jeu une fois l'installation terminée.\nVoulez-vous l'installer maintenant ?",
                            EN:"Viscéral tool version %d is available.\nDon't forget to reload the game once the installation is complete.\nWould you like to install it now ?"},
  nextBidPrice:{FR:'Prochaine enchère', EN:'Next bid'},
  noCaravanMoving:{FR:'Aucune caravane en mouvement', EN:'No caravan on the move'},
  noShortcutCreated:{FR:'Aucun repère créé', EN:'No shortcut created'},
  noSpellKnown:{FR:'Aucun sort connu', EN:'No spell known'},
  noUpdateAvailable:{FR:'Pas de mise à jour disponible.', EN:'No update available.'},
  openOnStartMisctools:{FR:'Outils', EN:'Tools'},
  openOnStartNotes:{FR:'Notes', EN:'Notes'},
  openOnStartReperes:{FR:'Repères', EN:'Shortcuts'},
  openOnStartStocks:{FR:'Stocks', EN:'Stocks'},
  openOnStartMisc:{FR:'Outils', EN:'Tools'},
  playerId:{FR:'Joueur #%d', EN:'Player #%d'},
  profilEndommage:{FR:'Profil endommagé', EN:'Profil broken'},
  profilLoad:{FR:'Charger', EN:'Load'},
  profilLoadComplete:{FR:'Chargement du profil terminé.', EN:'Profil load complete.'},
  profilLoadExplanation:{FR:'Collez votre profil dans la zone de texte ci-dessous puis appuyez sur le bouton "OK". Attention, votre profil actuel sera écrasé.',
  ...
  profilLoadNotFound:{FR:"Votre profil n'a pas été trouvé dans la base de données", EN:'Your profil can not been found in the database'},
  profilLoadServerTitle:{FR:'Restaurer votre profil depuis la base de données.', EN:'Restore your profil from the database'},
  profilSave:{FR:'Sauver', EN:'Save'},
  profilSaveComplete:{FR:'Enregistrement du profil terminé.', EN:'Profil save complete.'},
  profilSaveEmpty:{FR:'Le profil soumis est vide, rien à enregistrer.', EN:'The profil is empty, nothing to save.'},
  profilSaveLocalTitle:{FR:'Enregistrer votre profil sur votre disque dur', EN:'Save your profil to your hard drive'},
  profilSaveServerTitle:{FR:'Enregistrer votre profil dans la base de données.', EN:'Save your profil to the database'},
  remoteTimeDelta:{FR:'Vous avez un décalage de %s avec le serveur', EN:'You have a lag of %s with the server'},
  repereAdd:{FR:'Ajouter un repère', EN:'Add a shortcut'},
  repereDelete:{FR:'Supprimer ce repère', EN:'Remove shortcut'},
  repereFocusOn:{FR:'Centrer sur (%s,%s)', EN:'Focus on (%s,%s)'},
  resize:{FR:'Redimensionner', EN:'Resize'},
  sentFiltre_ally:{FR:'Alliances', EN:'Allies'},
  sentFiltre_ally_title:{FR:'Le ou les noms des alliances à filtrer, séparés par des points virgule.', EN:"Alliances's names to be filtered, separated by semicolons."},
  sentFiltre_coords:{FR:'Coordonnées', EN:'Coordinates'},
  sentFiltre_coords_title:{FR:'La ou les coordonnées à filtrer, séparées par des points virgul. 1,2 ; 3,4 ; 5,6', EN:'Coordinates, separated by semicolons 1,2 ; 3,4 ; 5,6'},
  sentFiltre_hero:{FR:'Position du héro', EN:'Hero position'},
  sentFiltre_hero_title:{FR:'La distance séparant le point actuellement centré sur la carte et la position du héro', EN:'The distance between the point now centered on the map and the hero position'},
  sentFiltre_myally:{FR:'Mon alliance', EN:'My ally'},
  sentFiltre_myally_title:{FR:"Filtrer les déplacement de l'alliance", EN:'Filter ally moves'},
  sentFiltre_myself:{FR:'Mes déplacements', EN:'My moves'},
  sentFiltre_myself_title:{FR:'Filtrer mes déplacements', EN:'Filter my moves'},
  sentFiltre_player:{FR:'Joueurs', EN:'Players'},
  sentFiltre_player_title:{FR:'Le ou les noms des joueurs à filtrer, séparés par des points virgule.', EN:"Players's names to be filtered, separated by semicolons."},
  sentFiltre_town:{FR:'Villes', EN:'Cities'},
  sentFiltre_townDest:{FR:'Ville de destination', EN:'Destination city'},
  sentFiltre_townDest_title:{FR:'La distance séparant le point actuellement centré sur la carte et la ville de destination', EN:'The distance between the point now centered on the card and the destination city'},
  sentFiltre_townImpact:{FR:'Distance à parcourir', EN:'Remaining distance'},
  sentFiltre_townImpact_title:{FR:'La distance séparant la position du héro à sa ville de destination', EN:'The distance between the position of hero and his destination city'},
  sentFiltre_town_title:{FR:'Le ou les noms des villes à filtrer, séparés par des points virgule.', EN:"Cities's names to be filtered, separated by semicolons."},
  sentinelleCloseOnClick:{FR:'Fermer sur la sélection', EN:'Close on click'},
  sentinelleDistance:{FR:'%s régions', EN:'%s areas'},
  sentinelleHistory:{FR:'Régions historisées : ', EN:'Areas archives : '},
  sentinelleNoRefresh:{FR:'0 pour aucun', EN:'0 for none'},
  sentinelleRefresh:{FR:'Rafraichissement', EN:'Refresh'},
  sentinelle_forceServerDelay:{FR:"Délai d'appel au serveur : ", EN:'Server call delay : '},
  sentinelle_forceServerDelay_warning:{FR:'Plus le délai est court plus le risque de ban est grand', EN:'The shorter the delay, the higher the risk'},
  sentinelle_forceServerRead:{FR:'Forcer la lecture complète des joueurs', EN:'Force full extraction of players'},
  sentinelle_forceServerRead_notrecommended:{FR:"Non recommandé, bien qu'un système de cache et de pool soit mis en place, cela reste des appels non autorisés au serveur",
                                             EN:'Not recommended, although a cache and pool system is established, it remains unauthorized calls to the server'},
  sentinelleHeroMoveTo:{FR:'%1$s est à %2$s regions de son objectif et se déplace en direction du %3$s de %4$s (%5$s,%6$s) vers %7$s (%8$s,%9$s)',
  ...
  siege_assiegeurs:{FR:'Assiégeants', EN:'Besiegers'},
  siege_end:{FR:'Fin', EN:'End'},
  siege_nothingFound:{FR:'Aucun siège détecté', EN:'No siege found'},
  siege_sieges:{FR:'Assiégés', EN:'Besiegeds'},
  siege_title:{FR:'Sièges', EN:'Seats'},
  ...
  titleConfig:{FR:'Viscéral - Configuration', EN:'Viscéral - Configuration'},
  ...
  titleReperes:{FR:'Repères', EN:'Shortcuts'},
  ...
  toggleSentinelle_on:{FR:"Démarrer l'actualisation automatique", EN:'Start automatic reload'},
  ...
  undefinedVariable:{FR:'La variable %s est indéfinie.\nAbandon.', EN:'Variable %d is undefined.\nAborting.'},
  UpdateAvailable:{FR:'Mise à jour v.%s disponible', EN:'Update v.%s available'},
  undefinedDatas:{FR:'Données indéfinies.', EN:'Undefined datas.'},
  unitaryPrice:{FR:'Prix unitaire', EN:'Unitary price'},
  versionInvalide:{FR:'Cette version du script Viscéral est trop ancienne pour pouvoir se synchroniser à la base de données.',
                   EN:'This version of the Viscéral script is too old to be allowed to synchronize with the database.'},
  zoomRepere13:{FR:'13x13 régions', EN:'13x13 areas'},
  zoomRepere35:{FR:'35x35 régions', EN:'35x35 areas'},
  zoomRepere35Warning:{FR:"Le format 35x35 ne déclenchera aucun recentrage si la cible (x,y) est déjà affichée sur l'interface (bug non résolu pour le moment)",
                       EN:'The format 35x35 will not trigger any focus to the (x,y) target if the area is already visible on the interface'},
  zoomRepereSize:{FR:'Format des repères', EN:'Shortcuts format'},
...
function updateCheck(forced)
{
 ...
 // Vérif une fois par jour (24 h * 60 m * 60 s * 1000 ms)
...
Window.MANAGER = (function()
{
 ...
 {
  // mon dieu que c'est laid tout ça
...
// mon alliance
//**************************
// TODO : trouver comment gérer les wings
MY_ALLY = (function(){
 ...
 function drawByTowns(spells)
 {
  ...
  for ( i = 0; i < towns.length; i++ )
  {
   ...
   if ( ecoles.length == 2 )
   {
    ...
    if ( ecoleLvl5 ) { nodeEcoles.appendChild(cTN(' spécialisation : ' + ecoleLvl5)); }
   }
  }
  if ( !found ) { nodeListe.appendChild(cTN(i18n('noSpellKnown'))); }
 }
  ...
  function drawCaravans(N/*ode*/, all)
  {
   ...
   if ( caravans.length > 0 )
   {
    ...
    for ( i = 0; i < caravans.length; i++ )
    {
     ...
     cell = row.insertCell(-1); cell.innerHTML = c.livraison ? dateH(c.livraison) : 'livré';
...
function open()
{
 function oncreate()
 {
  ...
  // ajout du bloc profil local à profil
  tmp.appendChild(div);
  ...
  // ajout du bloc profil server à profil
  tmp.appendChild(div);
...
//  x = cVX; y = cVY;
 ...
 if ( moves.length <= 0 )
 {
  ...
  cell = row.insertCell(-1);
  cell.colSpan = 15;
  cell.innerHTML = 'Aucun mouvement détecté';
 }
...
function createWin()
{
 ...
 function createFilters()
 {
  ...
  for ( i = 0; i < opts.length; i++ )
  {
   ...
   {
    case 'myself':
     node = cE('SELECT', {id:'VRSF_myself', title:title, childs:[new Option('',''), new Option('Que mes déplacements','only'), new Option('Sauf mes déplacements','except')]});
...
function cbAddToRecipient(R)
{
 ...
 {
  n.value = v+';'+n.value;
  // TODO : pas la bonne de méthode de uncheck le .recipientListCheckBox
...
function cbRemoveContentSpam(R)
{
 ...
 {
  map = [/>([0-9]{1,2}\/[0-9]{1,2} [0-9]{1,2}:[0-9]{1,2} De :[\u0000-\uFFFF]+?)(<br>À : [\u0000-\uFFFF]+?<br>Sujet :[\u0000-\uFFFF]+?)<br>/g,
...
{
 // TODO : trouver une manière moins aggressive pour les tooltips, doit bien y avoir un prototype ou un callback disponible
 function cb(retour)
 {
...
function init()
{
 ...
 function cb(retour, f,h)
 {
  var i,match,m,xy,x,y,
      ...
      regURL = /(https?:\/\/[^\s<]+)/;
  var regContent = /speakcontent/;
  if ( !f.toBePaid && typeof retour == 'string' )
  {
  var tableau = retour.split(regContent);
// console.log (retour);
// console.log (tableau[0]);
// console.log (tableau[1]);
   if ( (match=tableau[1].match(regURL)) )
   {
     tableau[1] = tableau[1].replace(match[0], '<a href="'+match[0]+'" target="_blank" class="VRchaturl">'+match[0]+'<\/a>');
   }
   else 
   {
    if ( (match=tableau[1].match(regAllCoords)) )
    {
     for ( i = 0; i < match.length; i++ )
     {
      if ( (m=match[i].match(regCoords)) )
      {
       ...
       if ( x && y ) { tableau[1] = tableau[1].replace(m[0], '<span class="VRchatcoords">' + m[0] + '</span>'); }
      }
     }
    }
   }
   retour=tableau[0]+"speakcontent"+tableau[1];
  }
  return retour;
 }
...
function enhanceMarketPlace()
{
 function cbAuction(retour)
 {
  ...
  if ( (n=this.playerNameElement) )
  {
   ...
   n.addEventListener('click', function(E) { if ( this.playerId ) { H.openPlayerProfileFrame(this.playerId); } }.bind(c), true);
   if ( c.playerId && MY_ALLY.is(c.playerId) ) { API.CSS.add('VR_inAlliance', n.parentNode); }
  }
  if ( (n=this.bestPlayerNameElement) )
  {
   n.style.cursor = 'pointer';
   n.addEventListener('click', function(E) { if ( this.bestPlayerId ) { H.openPlayerProfileFrame(this.bestPlayerId); } }.bind(c), true);
...
function init()
{
 function set(p)
 {
  ...
  if ( p && p.imageElement && (m=p.content) && (m=m.message) )
  {
   ...
   else if ( /si[è|e]ge/mi.test(m) ) { cN = 'SIEGE'; }
...
function UBIfixes()
{
 ...
 // évite les messages étranges "HOMMK.getElement error : No #189531 Region found!"
...
//**************************
// Lien vers JACTARI d'après le code de Jactari
//**************************
...
// console.log ("Test");
	switch (lang) {
		...
		case 'ru': url_combat = base_url + 'Ð±Ð¾Ð¹'; break;
		case 'fr':
	...
	style.type = 'text/css';
	style.appendChild(document.createTextNode('#permalien_jactari { position: absolute; top: 45px; left: 362px; width: 16px; height: 17px; background-image: url(' + base_url + 'images/icone-combat.png); z-index: 1000; }'));
	heredite.init();
...
// taille d'une coordonnée X ou Y
'.VR_XY { width:25px }',
...
// todo : lolo rendre les noms des clés plus cohérentes vrcfg vr_config
...
// lien jactari
'#VR_jactari { position:absolute; top:45px; left:363px; width:16px; height:17px; background-image:url('+ URL.GM.img +'jactari.png) }',
// sieges
...
// todo : trouver l'élément caché qui fait que quand le héro principal fait une attaque, il n'est plus identifié comme tel.
...
function KO(R)
{
 ...
 container.appendChild(cE('LI', {innerHTML:'Erreur de la réponse'}));
