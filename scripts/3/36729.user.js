...
// @version		5.99.10_Out_of_Nowhere
...
const scriptversion = '5.99.10_Out_of_Nowhere';
...
if (thisUpdate != scriptversion) {
	alert("New update: " + scriptversion + " Back in business for the current round of evo...");
...
function evoCreate(tableID) {
	...
	*/
	...
	if( showstats ) {
		// column headers
		addHeader(4, 'Attack/<br /><strong>Attack<sup>2</sup></strong>/<br /><span title="Attack per 1,000 resources (metal + mineral)">per 1K</span>');
		addHeader(5, 'Defense/<br /><strong>Defense<sup>2</sup></strong>/<br /><span title="Defense per 1,000 resources (metal + mineral)">per 1K</span>');
		addHeader(6, 'Total/<br /><strong>Total<sup>2</sup></strong>/<br /><span title="Attack + Defense per 1,000 resources (metal + mineral)">per 1K</span>');
	}
	...
	for( var i = 2; i < (table.rows.length - 1); i++ ) {
		...
		if( showstats ) {
			...
			addStat(row, 4, unit.attack,  att2, Math.round(att2*1000/unitCost), boost);
			addStat(row, 5, unit.defense, def2, Math.round(def2*1000/unitCost), boost);
			addStat(row, 6, (unit.defense + unit.attack), average, Math.round(average*1000/unitCost), boost);
		}
		// new UI
...
function evoOverview() {
	...
	if (cfleettable != null){
		...
		var cfp = 24 * ( 5 + l_food );
		var dif = Math.round( ( ( lfcost - cfp ) / 24) + 0.5 );
		var launchratio = 0;
	...
	var ruler = document.evaluate(".//a[@id='continfo1']", document, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue.innerHTML.substring(0,10);
	var RegExpTerm = (".//div[@id='overview_col2']/table");
	var RegExpTerm = (".//div[@id='overview_col2']//table[@class='t_little']");
...
//
function evoScans() {
	var scan = document.evaluate("id('main')/div[3]/strong", contents, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if( !scan ) {return;}
...
/// NEWS SCAN
	if (scan.indexOf("News Scan") != -1) {
		var ruler = document.evaluate("id('main')/div[3]/table/tbody/tr[1]/td/span", contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue.textContent;
...
var scanID = '';
	...
	if (scan.indexOf("Sector Scan") != -1){
		...
		// Store the string in the table ready for the 
	...
	if (scan.indexOf("Creature Scan") != -1){
		...
		// Calculate the exact number of launches and percentage fleet launch per day
			...
			var foodIncome = ((evoString2Number(foodLand[0])  + 5) * 24);
...
function evoUnitsInitialize() {
	...
	units['wolf']             = new evoUnit('Wolf'             ,UT_NATURAL ,10  ,80   ,400  ,27  ,25  ,14  ,0  ,40 ,true);
