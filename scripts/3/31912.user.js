// ==UserScript==
// @name           Create dragon wars invites.
// @namespace      Dragon wars friends.
// @description    Automatically invites people to your alliance that you find on the attack page.
// @include        http://apps.facebook.com/dragonwars/fight.php
// @include        http://apps.facebook.com/dragonwars/recruit.php
// @include        http://apps.new.facebook.com/dragonwars/fight.php
// @include        http://apps.new.facebook.com/dragonwars/recruit.php
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/dragonwars/recruit.php';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/dragonwars/stats.php?user=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/dragonwars/recruit.php?action=create',
