// Changelog
// 05.02.09. - Nach den Änderungen läuft das Script wieder
// ==UserScript==
// @name            Slideshow
// @description     Zeigt die Foto´s einer Seite in einer Slideshow an.
...
function setupSlideShow() {
  ...
  var SSdivAbout = document.createElement("div");
		SSdivAbout.id = "aboutPopup";
		SSdivAbout.className = "divPopup";
		SSdivAbout.innerHTML = "<h2>Diaschau </h2><p>Zeigt alle Bilder der aktuellen Seite in einer Slideshow!</p><strong>Tasten</strong><dl><dt>nächstes Bild</dt><dd>Pfeil rechts/unten<br>Leertaste<br>Auf Bild klicken</dd><dt>Voriges Bild</dt><dd>Pfeil links/hoch<br>b<br>Rücktaste</dd><dt>Diaschau beenden</dt><dd>Escape<br>Auf Hintergrund klicken</dd></dl><h2>Diaschau Hilfe</h2>";
  document.body.appendChild(SSdivAbout);

  var menu = document.getElementById("leftSideBox").previousSibling.previousSibling;
		var SSli = document.createElement("li");
			var SSa = document.createElement("a");
			SSa.href = "#";
			SSa.innerHTML = "SlideShow";
			SSa.title = "Starte Diaschau: Zum Navigieren Pfeiltasten benutzen, zum Beenden ESC drücken.";
			SSa.setAttribute("largeDivId", SSdivAbout.id);
			SSa.addEventListener("click", startSlideShow, true);
			SSa.addEventListener('mouseover',	mouseOver, true);
			SSa.addEventListener('mousemove',	mouseMove, true);
			SSa.addEventListener('mouseout', mouseOut, true);
		SSli.appendChild(SSa);
  menu.appendChild(SSli);
...
initSS(); // this works without any delay