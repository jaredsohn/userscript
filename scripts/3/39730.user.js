...
var g_alertsoundlink = 'http://xinhxan.net/travianalert.html';
...
{
	var pasd = find(xpath, XPList);
	if (pasd.snapshotLength > 0)
	return pasd.snapshotItem(0);
	else
	return null;
}
	...
	if (currenttype == 'troop')
	{
		...
		else
		{
			...
			if ((parseInt(lasttimecheck)-vv) <= 0 || (parseInt(lasttimecheck)-vv) > 1000)
			{
				...
				checkvillagesatk();
				createCookie(username+'atkchecklasttime', (vv+180), 10);  //3 phut nua moi check tiep
				//OpenRandomPage();
			}
			else //chua den gio...
			{
...
function CreateAttackOrder()
{
	...
	else
	{
		...
		{
			alert('NOTICE: Auto sẽ thực hiện sau (Auto will be execute after) ' + tt[2] + ' giây ');
			timeout = parseInt(tt[2])*1000;
		}
		
		if (tt[3] != null && tt[3] != '')
		{
			alert('NOTICE: Bắt đầu từ làng thứ (from the village with index of) ' + tt[3] );
			createCookie(villagekey+'currentvillage' , parseInt(tt[3]), 365);
		}
		
		if (tt[4] != null && tt[4] != '' && parseInt(tt[4]) == 1)
		{
			alert('NOTICE: Thực hiện chức năng scan map (use scan map feature -- do not really attack) ' + tt[4] );
			createCookie(villagekey+'usescanmap' , 1, 365);
		}
		else
		createCookie(villagekey+'usescanmap' , 0, 365);
		
		if (tt[5] != null && tt[5] != '' && parseInt(tt[5]) == 1)
		{
			alert('NOTICE: Số lượng lính tối thiểu (minimum troop to create an attack) ' + tt[5] );
			createCookie(villagekey+'minimumtroops' , 1, 365);
		}
		else
		createCookie(villagekey+'minimumtroops' , 0, 365);
		...
		alert('Dữ liệu đã được lưu trữ. Click Execute để chạy auto. (All data is saved, click Execute to run auto');
	}
...
function ExecuteAttackOrder()
{
	...
	{
		var v = get('atkstatus');
		v.innerHTML = 'Hệ thống đang bận xử lý sự kiện khác. (System is busy with another order) - Phase '+ g_whichphase;
	}
		...
		if (list != null && attackinfo != null)
		{
			...
			case 1:
				...
				if (t != null)
				{
					...
					xx = t.innerHTML.substring(t.innerHTML.indexOf('<h1')+4, t.innerHTML.indexOf('<h1')+6);
					if (xx.indexOf('G') == -1)
					{
						//DeactiveAttackOrder();
						createCookie(villagekey+'attackstep', 0, 365);
				...
				for(var j=0;j<tbls.snapshotLength;++j) 
				{
					...
					if (tb != null)
					{
						...
						valueid = yy.substring(yy.indexOf('>(')+2, yy.indexOf(')'));
						if (formid != '' && valueid != '')
				...
				for (pp=0;pp<mtt.length;pp++)
				{
					...
					if (tt.length>1)
					{
						...
						var mintr = readCookie(villagekey+'minimumtroops');
						if (scanmap == 1)
						mintr = 1;
			...
			case 3:
				...
				if (t != null)
				{
					...
					xx = t.innerHTML.substring(t.innerHTML.indexOf('<h1')+5, t.innerHTML.indexOf('<h1')+7);
					if (xx.indexOf('a') != -1)
					{
					...
					else
					{
						...
						if (xx.indexOf('Reinfor') == -1 && xx.indexOf('Attacking') == -1 && xx.indexOf('Raiding') == -1)
						{
							createCookie(villagekey+'attackstep', 0, 365);
				...
				for (i=0;i<vv.snapshotLength;i++)
				{
					...
					if (tt !=null && tt.innerHTML.indexOf('in') == 0)
					{
...
{ 
	...
	if (divcontent != null)
	{
		...
		var s=today.getSeconds();
		for (i=0;i<tbls.snapshotLength;i++)
		{
			var vv = tbls.snapshotItem(i);
			var vl = vv.getAttribute('onmouseover');
			if (vl != null && vl.indexOf('map') != -1)
			{
				var vs = vl.split("'");
				//map('5ludak8`s village','5ludak8','12','','142','17')"
				var vname = vs[1];
				var vlink = vv.getAttribute('href');
				var valliance = vs[7];
				var uname = vs[3];
				var pop = vs[5];
				var x = vs[9];
...
if (1) //chi la code folder cho gon
{
	...
	//=====================================================
	//Add scan map link
	if (window.location.href.match(/karte\.php/) || window.location.href.match(/karte2\.php/))
	{
...
var rightcontentm = div('<div style="position:absolute; top:0px; left: 1000px; width:800px;" id="ce1"><table><tr><td><div id="rightcontent"></div></td><td><div id="rightcontent2"></div</td></tr></table></div>');
...
rce.appendChild(document.createElement("br"));
rce.appendChild(rrr);
var holding = document.getElementById('holding');
if(rightcontent != null) 
{ 
	dcontent = '<table style="border:1px solid  #cccccc;width:300px"><Tr><Td><b>TỰ ĐỘNG XUẤT LÍNH (Auto raid)</b></td><tr>'+
	...
	dcontent = '<table style="border:1px solid  #cccccc;width:300px"><tr><td><b>TỰ ĐỘNG XUẤT LÍNH (Auto raid)</td></tr>'+
	'</table><div id="divtrooplink"></div><div id="toggleForm"></div><div id="formtroop" '+g_toggleForm+'><table>'+
	'<tr><Td >Nhập danh sách lính mặc nhiên (default troops):<br><input type="text" id="trooplist" value="'+g_trooplist+'" style="width:200px"></td></tr>'+
	'<tr><Td >Nhập cấu hình (configuration):<br><input type="text" id="attackinfo" value="'+g_attackinfo+'"  style="width:200px;"></td></tr>'+
	'<tr><td >Danh sách làng (farms list): <Br><textarea id="villist" style="width:300px;height: 300px">'+ g_villagelist+'</textarea>'+
	'</td></tr>'+
	'<tr><td><div id="vidu"></div><a href="#" onclick="var vd = document.getElementById(\'vidu\'); if (vd.innerHTML==\'\') vd.innerHTML= \'<b>(Example for the first textbox) Ô thứ nhất:</b> 1 20 4 -1<br>Nghĩa là: xuất 20 em phalax (nếu là gaul) hoặc tất cả TT<br><b>Ô thứ hai</b>: Có tất cả 5 tham số. Chỉ 2 tham số đầu là bắt buộc<Br><b>Tham số 1:Kiểu tấn công:</b>0=Reìnor, 1=Normal,2=Raid.<br><b>Tham số 2.Loại quân đang chơi:</b> 1=Teuton,2=Roman,3=Gaul<Br><b>Tham số3. Số giây chờ phát lệnh.</b> 0 nghĩa là ko chờ.<Br><b>Tham số 4.</b>Bắt đầu tấn công làng thứ mấy trong danh sách.<Br><b>Tham số 5:</b>Dùng chức năng scan map (1) hoặc không (empty)<br><b>Một ví dụ:</b>2 1 60 13<br>Giải nghĩa: Raid (2) và đang chơi quân Teuton (1), chờ 60giây mới bắt đầu thực hiện auto. Bắt đầu tấn công làng thứ 13 trở đi.<Br><b>Ô thứ ba: Cấu trúc danh sách làng: </b>Có 2 loại danh sách: 1. Dùng danh sách lính mặc nhiên. <Br>2. Dùng danh sách lính riêng.<Br>Với loại 1: chỉ cần 2 tham số: x y. Với loại 2 cần 3 tham số x y và danh sách lính theo dạng format sau: 123 123 @1 20 4 -1@langthangx. Giải nghĩa: Tấn công làng 123 123 với 20 em phalax và TT.\'; else vd.innerHTML=\'\'; return false;">Click để xem ví dụ</a></td></tr>'+
	...
	if (getNumVillage() > 1)	
	{
		var s = "<Br>Không auto nhiều làng (only execute auto within this village)" + ' '+g_automulti;
		if (g_automulti)
		s = "<Br>Đang auto nhiều làng (execute auto for all of your villages)"+ ' ' +g_automulti;
		var statlink9 = elem('a', s);
		statlink9.href = "javascript:void(0);";
		statlink9.addEventListener("click", function(){ g_automulti = !g_automulti; if (g_automulti) statlink9.innerHTML = "<Br>Đang auto nhiều làng"; else statlink9.innerHTML="<Br>Không auto nhiều làng"; createCookie(username+'automulti', g_automulti, 265); return false; }, 0);
...
function updateBuildList()
{
	...
	{
		alert(' Chua nhap du lieu kia ba (you did not input build list) !!!');
		return;
	}
	...
	deactiveBuild();
	alert("Ok. Dữ liệu xây dựng đã được lưu trữ. Nhấn Execute để thực thi (data is saved, click Execute to run)");
}
	...
	if (buildlist != null && trim(buildlist).length > 0)
	{
		...
		if (g_buildactive && alist.length > 0)
		{
			...
			if (t < vv)
			{
				...
				displayBuildStatus("Chưa đến giờ. Chờ " + (t/1000) + " giây");
				return;
			}
			...
			if (g_whichphase !='build')
			{
				...
				displayBuildStatus("Hệ thống đang bận. Chưa lượt đến xử lý Autobuild (system is busy with other actions, not build phase yet). Phase="+g_whichphase);
				return;
			}
			...
			if (alist[currentindex].indexOf('jump') != -1)
			{
				...
				displayBuildStatus("Nhảy lại dòng (jump to line) :"+currentindex);
				return;
			}
			...
			displayBuildStatus("Kien truc ID=" + bid+". Ten: "+ c_buildinglist[bid] + ' Step:'+step);
			//xay resource...
			switch (step)
			{
			...
			case 1:
				...
				if (found == 0 && bid > 53) 
				{
					cc += '<br>Không thể ra lính. Chưa xây nhà lính (you dont have barrack, cant build troops)';
					cc += '<br>Bỏ qua công trình này.. sang công trình kế tiếp (ignore this command, next one)';
			...
			case 2:
				...
				// Xay linh
				{
					...
					{
						cc+="<br>Dữ liệu không hợp lệ. Bỏ qua index này (invalid data, ignore this command)";
						displayBuildStatus(cc);
						createCookie(villagekey+'buildstep', 0, 365);
						createCookie(villagekey+"buildindex", currentindex+1, 365);
						break;
					...
					for (i=0;i<troop.length;i+=2)
					{
								...
								if (ii.innerHTML.indexOf('(') == 0 && ii.innerHTML.indexOf(')') == (ii.innerHTML.length - 1) && bo != null && bo.indexOf('t'+trooptype+'.value') != -1)
								{
									...
									if (avail > 0)
									{
										...
										else if (troopnum > avail) //chua du res...
										{
											...
											//createCookie(villagekey+'whichphase', g_whichphase, 365);
											//}
											cc += '<br>A. Chưa đủ res để xây lính. Chờ 5 phút. (not enough resource, wait 5 minutes) <br>Trở lại step 0 (back to step 0)';
											i=999999;
											submitbtn = 0;
											break;
										}										
									...
									else if (troopnum != -1)//ko available
									{
										...
										//createCookie(villagekey+'whichphase', g_whichphase, 365);
										//}
										cc += '<br>B. Chưa đủ res để xây lính. Chờ 5 phút. (not enough resource, wait 5 minutes)<br>Trở lại step 0 (back to step 0)';
										i=999999;
										submitbtn = 0;
										break;
									}	
						...
						{
							cc+='<br>Chưa nghiên cứu loại lính này (you didnt research this type of troop yet) index='+trooptype;
							cc+="<br>Dữ liệu không hợp lệ. Bỏ qua index này (invalid data, ignore this command)";
							displayBuildStatus(cc);
							createCookie(villagekey+'buildstep', 0, 365);
							createCookie(villagekey+"buildindex", currentindex+1, 365);
			...
			case 3: //da build xong...
				//go to next village
				displayBuildStatus("SUCCESS:Dang xay dung... (Built successful) <Br>Tro lai step 0 va move on next building");
				createCookie(villagekey+'buildstep', 0, 365);
				createCookie(villagekey+"buildindex", currentindex+1, 365);
				break;
			}
...
'Great Granary', 'Wonder of the World', 'Barracks (Build Troops)', 'Stable (Build Troops)']; //54 , 55
...
if(rightcontent != null) 
{
	var dv2 = '<table><tr><td><B>TỰ ĐỘNG XÂY DỰNG (Auto build)</b></td>'+
	'<tr><td><hr><div id="buildstatus"></div><Br>Công trình thứ (building index) : '+g_buildindex+'<hr></td></tr>'+
	...
	var dbuildcontent = '<table><tr><td><B>TỰ ĐỘNG XÂY DỰNG (Auto build)</b></td>'+
	'<tr><td><div id="buildlink"></div><hr></td></tr>'+
	'<tr><td>Trình tự xây (Building list) :</td></tr>'+
	'<tr><td><textarea name="tbuildlist" id="tbuildlist" style="width:300px;height:200px">'+g_buildlist+'</textarea></td></tr>'+
	'<tr><td>Chọn các công trình<br><select id="buildselect">';
...
var leftcontent = document.getElementById('lleft');	
var g_hideauto = readCookie('hideauto');
if (g_hideauto == null)
g_hideauto = 0;
...
linka.addEventListener("click", function(){ g_hideauto = !g_hideauto; if (g_hideauto) linka.innerHTML = "<Br>Show it"; else linka.innerHTML="<Br>Hide it"; createCookie('hideauto', g_hideauto, 365);  showhideAuto(); return false; }, 0);
leftcontent.appendChild(linka);
...
function checkvillagesatk()
{
	...
	if (xpathResult.snapshotLength > 0)
	{
		...
		for (var i=0;i<xpathResult.snapshotLength;i++)
		{
			...
			//g_chkatkcount[i] = 0;
			setTimeout(function () { checkvillage(); }, i*3000); //3 giay 1 lang
