...
// @namespace T3
// @version 2.9.3.27
// @description  (v 2.9.3.27) Travian v3 addons originally by Victor Garcia (aka Croc); - updated by Szabka pl51-7.June 2008; - updated by ms99 & Nux & Lux-05.October.2008
...
function functionMain(e){
	...
	var versionText = "2.9.3 - Weird Autumn";
	...
	xLang['SHOWCELLTYPEINFO']		= "Show cell type<br>while mousing over the map";
	...
	*/
	...
	/**
	 * Calcula el numero de seconds de una hora expresada en formato xx:xx:xx
	 * Params:
	 *	myElement	Texto con la hora a calcular
	 * Returns:
	 *	Numero de seconds que expresa la hora
	 */
	function ComputeSeconds(myElement) {
	...
	*/
	...
	/**
	 * Traduce una cadena de texto usando el language global detectado
	 * Params: texto: Cadena de texto a traducir
	 * Returns: Cadena de texto traducida
	 */
	function T(texto){
	...
	*/
	...
	function getGeneralData(){
		...
		} else if (strLast == detectedLanguage){
	...
	function leftMenuLinks(){
		...
		var links = [0,
				...
				0,
				//[T('MAPA'), "http://travmap.shishnet.org/?lang=" + language, "_blank"],
	...
	*/ 
	...
	function cp2villages(cp){ 
		if (document.domain.indexOf("speed")>-1) {
			...
			if (boolOldServerVersion == "0" || boolOldServerVerions == "false") {
    ...
    function cityLinks(){
		...
		cities = cities.firstChild;
		for (var i = 0; i < cities.childNodes.length; i++){
			// Utiliza el texto de las coordenadas para averiguar el ID needed para los enlaces
			var city = cities.childNodes[i];
			city.textContent.search(/\((.*)\n?\|\n?(.*)\)/);
			tmpX = RegExp.$1;
			tmpY = RegExp.$2;
			...
			var id = xy2id(tmpX, tmpY);
			city.appendChild(elem("TD", "<a href='a2b.php?z=" + id + "'><img src='data:image/gif;base64," + image["def1"] + "' width='12' border='0' title='" + T('ENV_TROPAS') + "' alt='" + T('ENV_TROPAS') + "'></a>"));
			city.appendChild(elem("TD", "<a href='build.php?z=" + id + "&gid=17'><img src='data:image/gif;base64," + image["img4"] + "' height='12' border='0' title='" + T('ENVIAR') + "' alt='" + T('ENVIAR') + "'></a>"));
	...
	function showFieldTypeInTooltip(fieldtype, newdid) {
		...
		var toolTipHTML = "<table class='f8' cellpadding='0' cellspacing='0' border='0'><tr><td colspan='2'>";
		...
		tooltipDiv.innerHTML = toolTipHTML + "<tr><td>&nbsp;</td></tr>" + tmTableHTML + "</table>";
		//alert(div.innerHTML);
    ...
    if (location.href.indexOf('berichte.php?id=') != -1)											battleReport();
