// coding: utf-8
// ==UserScript==
// @name           Ikariam Alarm and Overview Table 中文版 for v.0.2.8 (by li_you) v.2
// @namespace      psmith
// @description    This script automatically refreshes the page in Ikariam (every 5-10 minutes),
...
//
//                 Az ikariam játékban automatikusan frissíti az oldalt (5-10 percenként), és ha új
//                 üzenet jön, vagy megtámadnak, akkor hangjelzéssel figyelmeztet. Ezen kívül a
//                 falvakról összegyűjti a fontos információkat (lakosok, nyersanyagok mennyisége,
...
- 2008.11.12:
  * Updated to work with v0.2.8 based on K0R54N's Turkish only release
...
function getLocalizedTexts() {
  if (language == "hu") {
    ...
    texts = {
      "cityName": "城市名", "currentlyBuilding": "建造中", "summary": "总计:",
...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>"+floatFormat(curres / -factPerHour, 1) + " 小时用光";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + " 总容量<br>" + perc+"% 满" + remaining)+">"+
...
function createLastUpdateAsTooltip(content, time) {
  return createTooltip(content, "最后更新: "+smartDateFormat(time, true));
}
  ...
  if (TABLE_RESOURCES) {
    ...
    for(var i = 0; i < nodes.snapshotLength; i++) {
      ...
      if (Math.round(res.prodwine) > 0) {
        wineTooltip = mynumberformat(Math.round(res.prodwine), true)+" / 小时";
      }
...
//        growthRemainingHours = floatFormat((parseInt(spacetotal) + parseInt(bonusspace) - parseInt(population)) / (0.02 * happiness), 1) + " 小时";
          ...
          for(key in names) {
            if (usedIndexes[key] == 1) {
              ...
              if (pointPerUnit == 0 || level == "-") {
                ...
                citySumPoint += point;
                point = createTooltip(mynumberformat(point), level + " * " + mynumberformat(pointPerUnit) + " 点");
              }
  ...
  if (true) {
    ...
    t.appendChild(createRowInput("报警音量 (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("自动刷新:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("自动刷新最短间隔(秒):", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("自动刷新最长间隔(秒):", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("显示资源一览:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("显示建筑一览:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("显示军队一览:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("显示玩家和城市一览 (开发中):", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("资源进度条显示模式:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "关", time: "剩余时间", percent: "显示百分比"}));
    t.appendChild(createRowSlct("语言:", "LANGUAGE", language, {"": "Automatic from server name", en: "English", hu: "Magyar", cn: "简体中文", de: "German", cz: "Czech", tr: "Turkish", es: "Spanish", ba: "Bosnian", it: "Italian", pt: "Portuguese", fr: "French"}));
    t.appendChild(createRowInput("所属联盟 (缩写):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("友好联盟 (缩写, 用逗号隔开):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("敌对联盟 (缩写, 用逗号隔开):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("CSS:", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("保存debug日志:", "DEBUG_LOG", DEBUG_LOG));
