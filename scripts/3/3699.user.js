...
(function () {
	...
	var res = document.evaluate("//iframe", document, null,	XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null); 
	var i, el;
	for (i=0; el=res.snapshotItem(i); i++) {
		el.src='http://127.0.0.1/';
		el.width="600";
		el.height="0"
	}
  ...
  if (index != -1) {
   ...
   resstr = resstr.replace('http://dl3.rapidshare.de/','http://130.117.156.7/');
   resstr = resstr.replace('dl3l3.rapidshare.de','62.67.57.7');
   resstr = resstr.replace('dl3L3.rapidshare.de','62.67.57.7');
   resstr = resstr.replace('dl3CG.rapidshare.de','130.117.156.7');
   resstr = resstr.replace('dl3cg.rapidshare.de','130.117.156.7');
   alert(resstr);
