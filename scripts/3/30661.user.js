// ==UserScript==
// @name	Yet Another YouTube mplayer in HQ
// @author	mgorny
// @namespace	http://mgorny.jogger.pl/
// @description	Based on http://userscripts.org/scripts/show/22281. Replaces YT player with mplayerplug-in, selects highest quality available (based on HQTube), allows resizing, (no)autoplay and (no)looping (see config). Works on Opera.
// @ujs:download	http://userscripts.org/scripts/source/30661.user.js
// @include	http://www.youtube.com/watch*
// @include	http://youtube.com/watch*
// @include	http://*.youtube.com/w*
// @include	http://www.youtube.pl/watch*
// @include	http://youtube.pl/watch*
// @include	http://*.youtube.pl/w*
// ==/UserScript==

// oryginally written by theli @ userscripts.org
// then modified by Michał Górny (@mgorny.jogger.pl)
// with some help of Filip Cierpich (@inzaghi89.jogger.pl)
//
// changes:
// - use HQ videos (6 if possible or else 18), based on HQTube,
// - resize video
// - and don't loop it!
...
//@mgorny [use http://pasat.ocean.univ.gda.pl/~mgorny/UserCSS/youtube.css to move sidebar away]
//var videoWidth = 800;
//var videoHeight = 600;
...
var aLoader = 'R0lGODlhEAAQAPQAAP///wAAAPDw8IqKiuDg4EZGRnp6egAAAFhYWCQkJKysrL6+vhQUFJycnAQE'+
		...
		sGetter.innerHTML = "try{"+
							...
							"var fmtMap = getMp1.split('fmt_map=')[1].split('&')[0];"+
							"var srcWithT = 'http://'+document.domain+'/get_video?video_id='+vidID1+'&t='+getMp1.split('t=')[1].split('&')[0]+(fmtMap ? '&fmt=6' : '&fmt=18');"+
...
function timedCount(){
	...
	if( ( iCanHazSpanna && document.getElementById('tempLoader') )== null){
		...
		pD.innerHTML = '<OBJECT classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" width="'+videoWidth+'" height="'+videoHeight+10+'" codebase="http://www.apple.com/qtactivex/qtplugin.cab" type="video/quicktime"><param name="src" value="' + spanTex +'"><param name="autoplay" value="true"><param name="controller" value="true"><param name="loop" value="true"><EMBED src="' + spanTex + '" width="'+videoWidth+'" height="'+videoHeight+'" autoplay="'+(autoPlay ? 'true' : 'false' )+'" controller="true" loop="'+(loop ? 'true' : 'false')+'" pluginspage="http://www.apple.com/quicktime/download/" type="video/quicktime"></EMBED></OBJECT> '
