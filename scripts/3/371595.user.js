...
function IRCTCSettings()
{
	...
	mySpan.setAttribute("style", "position:fixed; width:100%; height: 100%; z-index:101; left: 0; top: 0; background-color: #000; opacity:.75; display:none;");			
	var myDiv = document.createElement('div');
	myDiv.id = 'settingsDiv';

	myDiv.setAttribute("style","position:fixed; width:650px; height: 510px; z-index: 102; background-color: #429E7D; border:8px #8590A8; text-align:center; display:none;font-family:verdana;font-weight:bold;-moz-border-radius: 10px;-webkit-border-radius: 10px;");
	document.body.appendChild(myDiv);
	myDiv.innerHTML="<span style='font-size:15px; color:#FFFFFF; line-height:15px;'>IRCTC Ticket Booking Automation</span><br/><br/>\
	<div style='padding-left:55px; font-size:10pt;line-height:22px;height:320px;'>\
		<form name='defSettingsForm' id='defSettingsForm'>\
		<div id='innerDiv1' style='float:left;text-align:left; background:#fff;color:#000;padding:16px; width:510px; height:420px;-moz-border-radius: 5px;-webkit-border-radius: 5px;'>\
		...
		<select name='classCode' id='defClass'>\
			...
			<label style='display:inline-block; width:80px; height:30px;'>Quota:</label>\ <select name='defQuota' id='defQuota'>\
			<option value='GN' id='GN'>General</option>\
			<option value='CK' id='CK'>Tatkal</option>\
			<option value='LD' id='LD'>Ladies</option>\
			<label style='display:inline-block; width:65px; height:30px;'>Train No:</label>\
...
function loadDefaultSettings() //Load default settings given to the search input form.
{
	...
	settingsLink.text("Click Here");
	settingsLink.css({'color':'#FFC000', 'font-weight':'bold', 'padding':'3px', 'text-decoration':'underline'});
	...
	jQ("select.quickBook.do option[value='"+defaultQuota+"']").attr("selected", true);//Select default quota
	
	jQ("select.quickBook.do option[value='"+defaultgatewayID+"']").attr("selected", true);//Select default defaultgatewayID
	...
	if (document.location.href.match('quickBook.do')) {
		...
		jQ("input[name='qt']").val(defaultQuota);
