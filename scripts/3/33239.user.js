...
function cleanEmpireMenus()
{
    ...
    var cleanmenus = document.evaluate("//table[contains(@class,'header')]",
...
function fixQueues(){
   try{
    ...
    } catch (e) {console.log("fixqueues: "+e);}
}
...
function getCurrentServerTime(){
    try{
      ...
      else
      {
          ...
          return '' + timerDateObject.getDate() + '-' + parseNum(timerDateObject.getMonth() + 1) + '-' + timerDateObject.getFullYear() + ' '
...
var animateTimer = function animateTimer() {
    try{
      ...
      pageDate.innerHTML = 'Current Server time: ' + zeroPad(timerDateObject.getDate()) + '-' + zeroPad(parseNum(timerDateObject.getMonth() + 1)) + '-' + timerDateObject.getFullYear() + ' '
                          + timerDateObject.getHours() + ':' + zeroPad(timerDateObject.getMinutes()) + ':' + zeroPad(timerDateObject.getSeconds());
...
setupTimer = function() {
    try{
        ...
        try{
            if (!timerHtmlElement) var timerHtmlElement = document.evaluate('//div[@id="top-header_server-time"]',
            document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        } catch (e) {
            console.log ("setuptimer error: "+e);
        }
        ...
        if (getSetting(ANIMATE_SERVER_TIME_KEY,true) && getSetting(STATIC_SERVER_TIME_KEY,true))
        {
            ...
            timerHtmlElement.setAttribute('style','float:left; padding-right:10px;');
            var copy = timerHtmlElement.cloneNode(true);

            var staticTimerHtmlElement = $('staticPageDate');
            staticTimerHtmlElement.innerHtml = "Page loaded at: "+pageDate;
