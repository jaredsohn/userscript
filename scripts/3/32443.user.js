// ==UserScript==
// @name          Ynet Links and Vids
// @namespace     ethan1701
// @description   Replaces javascript links with normal links on ynet.co.il, and enables embedded video clips in articles (v2.8). Updated from Jillian's version tp handle new videos.
...
var mainObj = {
    ...
    setVidUrls : function() {
        ...
        if (url.indexOf("castup.net") != -1) {
            ...
            this.vidUrl = url + document.getElementById("prmURL").value;
...
function isVideoFrame() {
    ...
    return ( (/mediazone/.test(document.location.hostname) && getFirstXPathResult("//obj"))
            || /castup/.test(document.location.hostname));
}

function linkifyJSLinks() {
    // Attributes to remove
    var attrs = "onclick oncontextmenu ondeactivate onblur onmouseover onmouseout target".split(" ");
