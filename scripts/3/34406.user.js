// ==UserScript==
// @name           HoboScript (Decoded)
// @namespace      hobowars
// @description    Unlike the original encoded version of Hoboscript (by CrackFiend), this version actually works on FF3.
// @author         Xyan Flux
// @author         CrackFiend
// @version        2.0.0
...
try{
	...
	if(document.location.href.match(hbwLivingUrl)){
		if(GM_getValue('displayLP',1)){
			...
			var expInfo=document.body.innerHTML.match(/Exp: ([\d,]+)\/([\d,]+)<br>/);
			var expEarned=parseInt(expInfo[1].replace(/,/g,''));
			var expNeeded=parseInt(expInfo[2].replace(/,/g,''));
			var expPercentMsg='';
			var expRemainingMsg='';
			expPercentMsg=(expEarned/expNeeded*100).toFixed(2)+'%<br />';
			expRemainingMsg='Need '+addCommas(expNeeded-expEarned)+' exp to level up.<br />';
			document.body.innerHTML=document.body.innerHTML.replace(/(Exp: [\d,]+\/[\d,]+)<br>/,"$1 "+expPercentMsg+expRemainingMsg);
		}
		var d=new Date();
		var t=d.getTime();
		var d=new Date();
