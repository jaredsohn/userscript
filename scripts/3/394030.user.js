// ==UserScript==
// @name		All video online without Flash Player
// @version		1.1 @2014
// @description		All video online without Flash Player,on html 5
// @author		◄Putri Dhelika Ahmad►
// @namespace		https://userscripts.org/users/523021
// @namespace		https://userscripts.org/users/putriahmad
// @downloadURL		https://userscripts.org/scripts/source/394030.user.js
// @updateURL		https://userscripts.org/scripts/source/394030.meta.js
// @icon		http://t3.gstatic.com/images?q=tbn:ANd9GcQOOV3Cf5EC2tfveahJMedOxSjXurJATG6GIVBnDn-1Lw71JnDriQ
...
// @include             https://www.4shared.com/video*
...
/*
  ...
  Copyright (C) 2014 ◄Putri Dhelika Ahmad►
  ...
  Group :  https://www.facebook.com/groups/BOTH.PERL/
  Contact: http://facebook.com/ratupokeran/
...
// Userscript
var userscript = 'All video online without Flash Player by ◄Putri Dhelika Ahmad►';
...
// Links
var Group = 'https://www.facebook.com/groups/BOTH.PERL/';
var contact = 'https://facebook.com/ratupokeran/';
...
function createMyElement (type, content, event, action, target) {
  ...
  if (content) {
    if (type == 'div') obj.innerHTML = content;
      ...
      obj.volume = 0.5;
      obj.innerHTML = '<br><br>Video Sedang memuat.jika tidak bisa memuat,pastikan bahwa anda mengggunakan HTML5 dan pengkode-an video terkait. jika anda yakin bahwa ini adalah masalah scriptnya silahkan hubungi kami <a href="' + contact + '"> ۝Disini۝</a>.';
    }
    else if (type == 'object') {
      obj.data = content;
      obj.innerHTML = '<br><br>Video Sedang memuat.jika tidak bisa memuat,pastikan bahwa anda mengggunakan ada sebuah plugins terbasang. jika anda yakin bahwa ini adalah masalah scriptnya silahkan hubungi kami <a href="' + contact + '">۝Disini۝</a>.<param name="scale" value="aspect"><param name="stretchtofit" value="true"><param name="autostart" value="true"><param name="autoplay" value="true">';
    }
    else if (type == 'embed') {
...
function resizeMyPlayer (size) {
  ...
  else styleMyElement (player['playerWindow'], {position: playerPosition, top: '0px', left: '0px', width: playerWidth + 'px', height: playerHeight + 'px', zIndex: 9999999999});
...
function showMyMessage (cause, content) {
  ...
  if (cause == '!player') {
    ...
    var myNoPlayerMess = 'Tidak dapat mendapatkan player element,silahkan laporkan <a href="' + contact + '">۝ Disini ۝</a>.';
    ...
    var myNoThumbMess = '<br><br>Tidak dapat mendapatkan Thumbnail,silahkan laporkan <a href="' + contact + '">۝ Disini ۝</a>.';
    modifyMyElement (player['playerContent'], 'div', myNoThumbMess, false);
  }
  else {
    appendMyElement (myPlayerWindow, myScriptLogo);
    if (cause == '!content') {
      var myNoContentMess = 'Tidak dapat mendapatkan video content,silahkan laporkan <a href="' + contact + '">here</a>.';
      modifyMyElement (myScriptMess, 'div', myNoContentMess, false);
    }
    else if (cause == '!videos') {
      var myNoVideosMess = 'Tidak dapat mendapatkan semua video,silahkan laporkan <a href="' + contact + '">here</a>.';
      ...
      var myEmbedMess = 'Ini adalah embed video. Anda bisa menontonnya <a href="' + content + '">here</a>.';
...
if (page.url.indexOf('youtube.com/watch') != -1) {
  ...
  function decryptSignature(s) {
    if (typeof ytDecrypter === 'function') {
      ...
      if (!ytDecryptFunction) ytDecryptFunction = 'a=a.split("");a=LD(a,36);a=LD(a,45);a=a.reverse();a=a.slice(2);a=a.reverse();return a.join("")';
  ...
  if (!ytPlayerWindow) {
    ...
    if (ytFeatherBeta) {
      ...
      if (ytVideoID) var ytVideoThumb = 'http://img.youtube.com/vi/' + ytVideoID[2] + '/0.jpg';
    }
    else {
	...
	var ytFncBody;
	var ytProtocol = page.win.location.protocol;
	var ytScriptURL = getMyContent(page.url, "\"js\":\\s*\"(.*?)\"", true);
	if (ytScriptURL) {
	  ytScriptURL = ytProtocol + '//' + ytScriptURL;
	  try {
	    GM_xmlhttpRequest({
	      method: 'GET',
	      url: ytScriptURL,
...
// =====DailyMotion===== //
  ...
  if (!dmPlayerWindow) {
    ...
    /* My Player Window */
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '620px', height: '372px', backgroundColor: '#F4F4F4', zIndex: '99999'});
...
// =====Vimeo===== //
  ...
  if (!viPlayerWindow) {
    ...
    /* My Player Window */
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '960px', height: '564px', backgroundColor: '#F4F4F4', zIndex: '99999'});
...
// =====FunnyOrDie===== //
  ...
  if (!fodPlayerWindow) {
    ...
    /* My Player Window */
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '970px', height: '570px', backgroundColor: '#F4F4F4', zIndex: '99999', margin: '0px auto'});
...
// =====Crackle===== //
  ...
  if (!crPlayerWindow) {
    ...
    /* My Player Window */
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '970px', height: '570px', backgroundColor: '#F4F4F4', zIndex: '99999'});
...
// =====IMDB===== //
  ...
  if (!imdbPlayerWindow) {
    ...
    /* My Player Window */ 
    var myPlayerWindow = createMyElement ('div', '', '', '', '');
    styleMyElement (myPlayerWindow, {position: 'relative', width: '670px', height: '400px', backgroundColor: '#F4F4F4', zIndex: '99999'});
