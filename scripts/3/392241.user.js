...
// @namespace   MercuryP, Updated by DragonScale 
...
function MailAttachmentAccept() { 
    ...
    $(document).ajaxStop(function()
    {
        ...
        this.acceptMailAttachment = function(id)
        {
          ...
          // Find out what kind of attachment it is so we know what currency to update.
          var attachmentImage = $(attachmentIcon.find('img')[0]);
          var attachmentType;
          if (attachmentImage.attr('src').indexOf('icon_treasure.png') > -1) {
            attachmentType = 'treasure';
          } else if (attachmentImage.attr('src').indexOf('icon_gems.png') > -1) {
            attachmentType = 'gems';
          }
          
          attachmentImage.attr('src', '/images/layout/loading.gif');
          ...
          $.ajax({
            ...
            cache:false
            }).done(function(data)
                {
                  ...
                  // After accepting, we need to update currency where relevant.
                  if (attachmentType) {
                      $.ajax({
                        type: "POST",
                        data: {currency: attachmentType},
                        url: "includes/currency_change.php",
                        cache:false
                      }).done(function(stuff){
                        $('#user_' + attachmentType).html(stuff);
                      });
                  }
                }
            );
...
function AuctionHouseEnhanced() {
    ...
    if (links.length > 7) {
        ...
        this.loadTab = function(tab, page)
        {
          ...
          this.currentTab = tab;
          setSessionValue('auctionHouseTab', tab);
        ...
        var tabSelect = /tab=([^&]+)/.exec(window.location.href);
        if (tabSelect) {
            tabSelect = tabSelect[1];
        } else {
            tabSelect = getSessionValue('auctionHouseTab', 'food');
        }
        this.loadTab(tabSelect);
    }
}
...
function FriendsBoost() {
    ...
    {    
        ...
        // Get the saved friends date and calculate the differential - we can update the cache every 12 hours to be sure.
        var friendsDate = getStoredValue('friendsDate', 0);
        var hoursElapsed = (+new Date() - friendsDate) / (1000 * 60 * 60);
        ...
        if (hoursElapsed > 12)
        {
            ...
            cache:false
            }).done(function(friendsReply)
                {
                  ...
                  setStoredValue('friendsDate', +new Date());
    ...
    this.forceSelect(document, function(sel) {
        sel.ajaxSuccess(function(e, xhr, options) {
            if (options.url.indexOf('includes/ol/friend.php') > -1)
            {
                // When an update happens, reset the time so that we update the friends list on the next load.
                setStoredValue('friendsDate', 0);
            }
        });
    });
}
...
function CrossroadsFriends()
{
    ...
    $('#btrade').click(this.initiateTrade);                
}
...
function SilentEdit2() {
    ...
    $('#message').after('<div><input id="silentedit" type="checkbox" name="silentedit" value="1" checked>Edit silently (delete this message and repost)</div>');
    ...
    $($('#submit')[0]).click(function(e){
            ...
            {
                document.SilentEdit(postID, threadID);
            }
            else
            {
                // If not, carry on.
...
/*---------------------------*/
/* STREAMLINE FESTIVE FAVORS */
/*---------------------------*/
function StreamlineFavors() {
    // Function to decrement the holiday currency total
    function RemoveCurrency(amount)
    {
        var currencyEl = $($('#super-container div div span')[2]);
        var currentTotal = parseInt(currencyEl.text().trim());
        currentTotal -= amount;
        currencyEl.text(currentTotal);
    }
    
    var clickedButtonCost;

    // Register the festive favor item costs with each corresponding button.
    var buttons = $('.mb_button').parent().parent();
    for (var i = 0; i < buttons.length; i++)
    {
        var button = $($(buttons[i]));
        var cost = $(button.find('div')[0]).text().trim();
        
        button.click(function() {
            clickedButtonCost = cost;
        });
    }
    
    // Respond to ajax requests
    $(document).ajaxSuccess(function(e, xhr, options) {
        if (options.url.indexOf('festive_buy') > -1)
        {
            // Initial dialog load
            if (options.data.indexOf('ver=con') == -1) {
                // There's absolutely no reason to refresh when we cancel...
                $('#no').off('click');
                $('#no').click(function(e){
                    $('#purchase').dialog('close');
                });
            } else {
                // If an item was bought we can deduct those tokens.  I wonder if he eats them...
                RemoveCurrency(clickedButtonCost);
                // Then skip the reload on clicking OK.
                $('#back').off('click');
...
function FlightRisingEnhancer() {
    ...
    this.moduleReference =
    {
      ...
      streamlineDialogs: 'Streamline Dialogs',
...
var pageReference =
{
    ...
    "festive": [
        {func: StreamlineFavors, key: "streamlineDialogs"}
    ],
    "": [
    ...
    "*": [
        ...
        {func: FriendsBoost, key: "friendsBoost", immediate: true},
