// ==UserScript==
// @name       CRM enhance tool
// @description	  CRM enhance tool
// @version    3.2.2
// @copyright  reizhi
// @include http://*.faces
// @include http://172.22.8.180/*
// @updateURL	http://roov.org/388863.user.js
// @downloadURL	http://roov.org/388863.user.js
// @author reizhi
// ==/UserScript==
    ...
    for (n = 0; n <= i; n++) //遍历所有label
    {
        ...
        else if (des[n].innerHTML.indexOf("用户账户余额不足") > 0 )
        {
            ...
            else if (des[n].innerHTML.indexOf("预约操作停止") > 0)
            {
                ...
                var tend = des[n].innerText.indexOf("请查收用户退还设备");
...
function autocnl() {//取消服务自动填表函数
    ...
    conval1 = setInterval(function(){cheval('1075','0')},400);
...
function autobfr() {//缓冲自动填表函数
    ...
    document.getElementById("default:iptvCloud").value = 0;
    A4J.AJAX.Submit('_viewRoot','default',event,{'similarityGroupingId':'default:j_id350','parameters':{'default:j_id350':'default:j_id350'} ,'actionUrl':'/hy_crm/rich/page/TroubleTicketManager/CreateTroubleTicketPage.faces'} );
    conval1 = setInterval(function(){cheval('673','1642')},400);
...
function autornw() {//套餐提醒填表函数
    ...
    document.getElementById("default:iptvCloud").value = 0;
    A4J.AJAX.Submit('_viewRoot','default',event,{'similarityGroupingId':'default:j_id350','parameters':{'default:j_id350':'default:j_id350'} ,'actionUrl':'/hy_crm/rich/page/TroubleTicketManager/CreateTroubleTicketPage.faces'} );
    conval1 = setInterval(function(){cheval('1179','1231')},400);
    document.getElementById('default:assign').value = 505626
...
function autocrd() {//更新资料填表函数
    ...
    conval1 = setInterval(function(){cheval('1102','1107');},400);
...
function cheval(val1,val2) {//循环设置select
    ...
    else if ( getval('default:one') == val1 )
    {
        if (val2 != 0)
        {
...
function autotme() {//插入时间函数
	...
	var ttct = prompt("请输入标题内容", "");
	document.getElementById('default:title').value = getcon(parseInt(gettext('注册渠道'))) + gettext('姓名') + '-' + ttct + '-' + localStorage.getItem('wn');
    var frameObj = document.getElementsByTagName("iframe");
...
function nametonum (wn) {//工号判断函数
    ...
    switch (wn) {
		...
		case "liju":
            {
                localStorage.setItem('wn', 1007);
                break;
            }
		case "limengxue":
            {
                localStorage.setItem('wn', 1025);
                break;
            }
    }
...
if (document.getElementById("gongdan") ) {
    ...
    {
        input.setAttribute("type","button");
        input.setAttribute("value","时间/标题");
