...
//                 Ezen kívül a falvakról összegy?ti a fontos információkat (lakosok, nyersanyagok mennyisége,
...
Version history:
- 2008.07.07:
  * Lots of little fixes to keep the script alive. Thanks Kupy123 and ocdcsv.
...
texts = {
"cityName": "城市名", "currentlyBuilding": "建築中", "summary": "總計:",
"hide_settings": "隱藏設定", "show_settings": "顯示設定",
    ...
    texts = {
      "cityName": "?ehir", "currentlyBuilding": "Yükseltilen", "summary": "Toplam:",
      "hide_settings": "Hide settings", "show_settings": "Show settings",
    };
...
"hide_settings": "隱藏設置", "show_settings": "顯示設置",
};
...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((showTooltip == true) && (dailyFact != 0)) {
    tooltip = mynumberformat(intfactPerHour, true)+" / 小時, "+mynumberformat(dailyFact, true)+" / 天";
  }
  var res;
  if (factPerHour != 0) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>"+floatFormat(remhour, 1) + " 小時裝滿";
    } else if (factPerHour < 0) {
      remaining = "<br>"+floatFormat(curres / -factPerHour, 1) + " 小時耗盡";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + " 總量<br>" + perc+"% 已使用" + remaining)+">"+
...
function createProd(prodPerHour, extraTooltip) {
  ...
  var tooltip = mynumberformat(Math.round(24 * prodPerHour), true)+" / 每天";
...
function getGrowthRemainingHours(population, maxPopulation, startTime, happiness) {
  ...
  return floatFormat((time - Number(startTime)) / 1000 / 3600, 1) + " 小時";
}
...
function createLastUpdateAsTooltip(content, time) {
  return createTooltip(content, "最後更新於 "+smartDateFormat(time, true));
}
  ...
  if(/view=port/.test(document.URL)||/index\.php/.test(document.URL)) {
	      ...
	      if(body_id="port") {
	      	//add destination part
			  	var node = getNode("//div[@class='contentBox01h'][3]/div/ul");
			  	...
			  	var transporters = xpath("//div[@class='contentBox01h'][5]/div/table/tbody/tr/td[@class='city']");
					res.incomingTransporters = [];
			  	if(transporters.snapshotLength > 0) {
			  		//record them
			  		for(var i = 0; i < transporters.snapshotLength; i++) {
        			var node = transporters.snapshotItem(i);
        			var transporter = {};
        			...
        			transporter.res = r;
        			node = getNextNode(node);
        			transporter.mission = node.textContent;
        			node = getNextNode(node);
        			transporter.arriveTime = getLocalTime(getDuration(node.textContent+""));
  ...
  if(/view=transport/.test(document.URL)||/index\.php/.test(document.URL)) {
  	...
  	var destId = getNode_value("//form[@id='transport']/input[@type='hidden' and @name='destinationCityId']");
		...
		if((journeyTime.indexOf(" + ") > 0) && (journeyTime.indexOf(" = ") > 0))
  		...
  		var addname = prompt("請給目的地輸入一個名稱",destName+"["+journeyTime+"]");
...
function renderTables() {
  ...
  var LETTERCHANNEL = getCfgValue("LETTERCHANNEL", false);
  ...
  if (TABLE_RESOURCES) {
    ...
    s += "<table border=1 class='table01'>";
    s += "<caption>資源總覽</caption>";
    ...
    for(var i = 0; i < nodes.snapshotLength; i++) {
      ...
      if (wineUsage > 0) {
        wineRemainingHours = floatFormat(getCurrentResourceAmount(currenttime, res.prodtime, res.wine, res.prodwine - wineUsage) / wineUsage, 1) + " 小時";
      }
...
//        growthRemainingHours = floatFormat((parseInt(spacetotal) + parseInt(bonusspace) - parseInt(population)) / (0.02 * happiness), 1) + " h";
  ...
  if(TABLE_TRANSPORTERS) {
  	s += "<table border=1 class='table01'>";
  	s += "<caption>來訪的運輸船</caption>";
    s += "<tr class='table_header'><th>"+texts["cityName"]+"</th>"+
    		 "<th class='lf'>出發地</th>"+
    		 "<th class='lf'>"+woodName+"</th>"+
    		 "<th>"+wineName+"</th>"+
    		 "<th>"+marbleName+"</th>"+
    		 "<th>"+glassName+"</th>"+
    		 "<th>"+sulfurName+"</th>"+
      ...
      var t = res.incomingTransporters.length;
			if(t > 0)
				for(var j = 0; j < t; j++) {
						...
						s += "<tr>";
						if(j == 0)
			      s += "<td rowspan='"+t+"' class='"+cs+"'>"+createLinkToCity(city.innerHTML, city.value, i)+"</td>";
			      s += "<td class='lf'>"+transporter.startcity+"</td>";
			      s += "<td class='lf'>"+createTooltip(transporter.res["wood"], "到達時:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["wood"]+res.wood, res.prodwood))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["wine"], "到達時:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["wine"] + res.wine, res.prodwine - wineUsage))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["marble"], "到達時:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["marble"] + res.marble, res.prodmarble))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["glass"], "到達時:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["glass"] + res.glass, res.prodglass))+"</td>";
			      s += "<td>"+createTooltip(transporter.res["sulfur"], "到達時:"+getCurrentResourceAmount(transporter.arriveTime, res.prodtime, lastship["sulfur"] + res.sulfur, res.prodsulfur))+"</td>";
  ...
  if(TABLE_RESEARCH) {
    ...
    for(var i = 0; i < nodes.snapshotLength; i++) {
      ...
      sMaxSum += currentMax
      eSum += efficiency;
      s += "<td class='"+cs+"'>"+scientists+"/"+currentMax+"</td>";
      s += "<td class='"+cs+"'>"+createProd(efficiency)+"</td>";
      var cost = 8;
      if(LETTERCHANNEL)
      	cost =7;
      s += "<td class='"+cs+"'>"+createProd(-cost*scientists)+"</td>";
      var link = getArrValue(res.buildings["academy"], "link", "-");
      var linkText = "";
      if(link!='-')
      	linkText="<a href='"+link+"'>前往"+buildings["academy"][1]+"</a>";
      s += "<td class='"+cs+"'>"+linkText+"</td>";
      s += "</tr>";
    }
    s += "<tr class='table_footer'>";
    s += "<td class='table_footer'>"+texts["summary"]+"</td>"+
    		 "<td class='lf'></td>"+
    		 "<td>"+sSum+"/"+sMaxSum+"</td>"+
    		 "<td>"+createProd(eSum)+"</td>"+
    		 "<td>"+createProd(-cost*sSum)+"</td>"+
    		 "<td></td>";
  ...
  if (TABLE_ARMYFLEET) {
    if (config["unitnames"] != undefined) {
      ...
      if (usedIndexesCount > 0) {
        s += "<table border=1 class='table01'>";
        s += "<caption>軍隊總覽</caption>";
        s += "<tr class='table_header'><th class='table_header'>"+texts["cityName"]+"</th>";
        for(key in names) {
          var name = names[key];
          if (usedIndexes[key] == 1) {
            s += "<th class='lf' colspan=2>"+name+"</th>";
          }
        }
        s += "<th class='lf' colspan=2>"+texts["summary"]+"</th>";
        s += "</tr>";
        var sum = [];
        var sumPoint = [];
        for(var i = 0; i < nodes.snapshotLength; i++) {
          ...
          s += "<td class='lf "+cs+"'>"+(citySum != 0 ? mynumberformat(citySum) : "-")+"</td>"+
               "<td class='"+cs+"'>"+(citySumPoint != 0 ? mynumberformat(citySumPoint) : "-")+"</td>";
          s += "</tr>";
        }
        s += "<tr class='table_footer'>";
        s += "<td class='table_footer'>"+texts["summary"]+"</td>";
  ...
  if (true) {
    function reset_all_data() {
      var answer = confirm("您確定要清除所有保存的數據?");
      if (answer) {
        setVar("config", "");
        setVar("players", "");
    ...
    t.appendChild(createRowChk("聲音報警:", "ALERT_SOUNDS", ALERT_SOUNDS));
    t.appendChild(createRowInput("報警音量 (0 - 100):", "ALERT_VOLUME", ALERT_VOLUME));
    t.appendChild(createRowInput("警告音量 (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("自動刷新:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("自動刷新下限(秒):", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("自動刷新上限(秒):", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("顯示資源信息:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("顯示來訪運輸船信息", "TABLE_TRANSPORTERS", TABLE_TRANSPORTERS));
    t.appendChild(createRowChk("顯示建築信息:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("顯示軍隊信息:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("顯示研究信息:", "TABLE_RESEARCH", TABLE_RESEARCH));
    t.appendChild(createRowChk("信件通道已研究:", "LETTERCHANNEL", "LETTERCHANNEL"));
    t.appendChild(createRowChk("顯示玩家和城市信息 (開發中):", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("資源條模式:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "關閉", time: "基於剩餘時間", percent: "基於容量"}));
    t.appendChild(createRowSlct("語言:", "LANGUAGE", language, {"": "由服務器名確定", cn: "簡體中文" ,tw: "TChinese",en: "English", hu: "Magyar", de: "German", cz: "Czech", tr: "Turkish", es: "Spanish", ba: "Bosnian", it: "Italian", pt: "Portuguese", fr: "French"}));
    t.appendChild(createRowInput("聯盟(縮寫):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("友好聯盟 (縮寫, 逗號分割):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("對戰聯盟 (縮寫, 逗號分割):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("CSS:", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("調試日誌:", "DEBUG_LOG", DEBUG_LOG));
