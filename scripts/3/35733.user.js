// coding: utf-8
// ==UserScript==
// @name           Ikariam 總覽插件 (0.2.8 適用)
// @namespace      newIAaOT
// @description    IKARIAM中顯示城鎮建築,軍事及島嶼資訊. 中文版由Whiskers修改.
...
//                 It also collects important information about your towns (population, amount of
//                 resources, building levels, buildings under construction), and displays it all
...
- 2008.10.22
  * Updated to be compatible with 0.2.8

- 2008.10.20
  * Added TW Translation
  * Removed broken Russian translation, must have lost it when someone saved as non-unicode.
  * Added wine usage up until level 21
...
var tavernWineUsage = [0, 3, 5, 8, 11, 14, 17, 21, 25, 29, 33, 38, 42, 47, 52, 57, 63, 68, 73, 79, 85, 91];
...
function getLocalizedTexts() {
  if (language == "hu") {
    ...
    };  } else if (language == "tw") { //by Whiskers
    buildings = {
      "townHall"      : ["市政府", "市政府"],
      "academy"       : ["學院", "學 院"],
      "port"          : ["港口", "港 口"],
    ...
    texts = {
      "cityName": "城鎮名稱", "currentlyBuilding": "目前正在建設",
      "summary" : "總計:", "hide_settings": "隱藏設置", "show_settings": "顯示設置",
    };
  } else {
    buildings = {
...
function createCounter(startTime, startAmount, factPerHour, showTooltip, maxCapacity, plusText) {
  ...
  if ((showTooltip == true) && (dailyFact != 0)) {
    tooltip = mynumberformat(intfactPerHour, true)+"/小時, "+mynumberformat(dailyFact, true)+"/日";
  }
  var res;
  if (factPerHour != 0) {
  ...
  if ((PROGRESS_BAR_MODE != "off") && (maxCapacity > 0)) {
    ...
    if (factPerHour > 0) {
      ...
      remaining = "<br>預計爆倉時間: "+floatFormat(remhour, 1) + "小時";
    } else if (factPerHour < 0) {
      remaining = "<br>預計耗完時間: "+floatFormat(curres / -factPerHour, 1) + "小時";
    }
    ...
    res +=  "<table class='myPercent'>"+
            "<tr class='myPercent' "+createTooltipAttribute(mynumberformat(maxCapacity) + "最大容量<br>約占倉庫總量: " + perc+"%" + remaining)+">"+
...
function createProd(prodPerHour, extraTooltip) {
  ...
  var tooltip = mynumberformat(Math.round(24 * prodPerHour), true)+"/日";
...
function getGrowthRemainingHours(population, maxPopulation, startTime, happiness) {
  ...
  return floatFormat((time - Number(startTime)) / 1000 / 3600, 1) + "小時後人口滿載";
}
...
function smartDateFormat(time, showElapsedTime, elapsedTimeSeparator) {
  ...
  if (showElapsedTime) {
    ...
    if (d < 3600) {
      ...
      if (d >= 86400) {
        t += " " + Math.floor(d / 86400) + "日";
      }
      t += " " + floatFormat((d % 86400) / 3600, 1) + "小時";
    }
  }
  return t;
}
...
function createLastUpdateAsTooltip(content, time) {
  return createTooltip(content, "資料時間: "+smartDateFormat(time, true));
}
...
function renderTables() {
  ...
  PROGRESS_BAR_MODE = getCfgValue("PROGRESS_BAR_MODE", "percent"); //progress bar mode for resource counters
  ...
  if (TABLE_RESOURCES) {
    ...
    for(var i = 0; i < nodes.snapshotLength; i++) {
      ...
      if (Math.round(res.prodwine) > 0) {
        wineTooltip = mynumberformat(Math.round(res.prodwine), true)+"/小時";
      }
      ...
      s += "<tr>";
      s += "<td class='"+cs+"' align=left>"+"&nbsp;&nbsp;"+createLinkToCity(city.innerHTML, city.value, i)+"</td>"+
          ...
          for(key in names) {
            if (usedIndexes[key] == 1) {
              ...
              if (pointPerUnit == 0 || level == "-") {
                ...
                citySumPoint += point;
                point = createTooltip(mynumberformat(point), level + " * " + mynumberformat(pointPerUnit) + " 點");
              }
  ...
  if (true) {
    ...
    t.appendChild(createRowChk("提醒:", "ALERT_SOUNDS", ALERT_SOUNDS));
    t.appendChild(createRowInput("提醒音量 (0 - 100):", "ALERT_VOLUME", ALERT_VOLUME));
    t.appendChild(createRowInput("報警音量 (0 - 100):", "WARNING_VOLUME", WARNING_VOLUME));
    t.appendChild(createRowChk("自動重整:", "AUTO_REFRESH", AUTO_REFRESH));
    t.appendChild(createRowInput("自動重整最短間隔(秒):", "AUTO_REFRESH_MIN_SECS", MIN));
    t.appendChild(createRowInput("自動重整最長間隔(秒):", "AUTO_REFRESH_MAX_SECS", MAX));
    t.appendChild(createRowChk("顯示資源概覽:", "TABLE_RESOURCES", TABLE_RESOURCES));
    t.appendChild(createRowChk("顯示建築概覽:", "TABLE_BUILDINGS", TABLE_BUILDINGS));
    t.appendChild(createRowChk("顯示軍隊概覽:", "TABLE_ARMYFLEET", TABLE_ARMYFLEET));
    t.appendChild(createRowChk("顯示玩家和城鎮概覽 (開發中):", "TABLE_PLAYERS", TABLE_PLAYERS));
    t.appendChild(createRowSlct("資源條顯示模式:", "PROGRESS_BAR_MODE", PROGRESS_BAR_MODE, {off: "关", time: "剩於時間", percent: "庫存量百分比"}));
    t.appendChild(createRowSlct("語言:", "LANGUAGE", language, {"": "Automatic from server name", en: "English", hu: "Magyar", tw: "正體中文", de: "German", cz: "Czech", tr: "Turkish", es: "Spanish", ba: "Bosnian", it: "Italian", pt: "Portuguese", fr: "French"}));
    t.appendChild(createRowInput("所屬聯盟 (縮寫):", "ownAlly", getCfgValue("ownAlly", "")));
    t.appendChild(createRowInput("友好聯盟 (縮寫, 以逗號分隔):", "friendlyAllies", getCfgValue("friendlyAllies", "")));
    t.appendChild(createRowInput("敵對聯盟 (縮寫, 以逗號分隔):", "hostileAllies", getCfgValue("hostileAllies", "")));
    t.appendChild(createRowTxtr("CSS:", "CSS", getCfgValueNonEmpty("CSS", default_style), 15, 70));
    t.appendChild(createRowChk("保存debug日誌:", "DEBUG_LOG", DEBUG_LOG));
