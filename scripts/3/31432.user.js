// ==UserScript==
// @name          Travian Attack Builder w/ non-linear attack delay
// @description   Travian Attack Builder by m4rtini - fixed by bluelovers, modified by wojtek858, nerdrobin and skibumstevo
// @author        m4rtini (m4rtini89@gmail.com)
...
switch(sLang)
{
	...
	case "et"://by jeje
		...
		text = ["Ataca!!! =)","Tipo de ataque invalido!","No hay coordenadas!","No hay suficientes Tropas! (Tropas #","No tropas introducidas","Empezando","Problamente malas coordenadas","No hay exploradores","Correcto-Hecho","Configurar Ataque:","Añadir nuevo ataque","Resetear","Configurar ataque:","Tipo de ataque:","Normal","Refuerzos","Atraco","Acechar Produccion/Tropas","Achechar Defensas/Tropas","Coordenadas","Ataca camarada! =)","Tiempo de Llegada","Llegara a las :","Establecer tiempo de llegada","Numero de ataques de esta estrategia","con '|' ex: 0,0|1,1","ERROR:","Incapaz de encontrar la aldea activa. Assuming one village account","Tiempo de llegada establecido a las:","Time between waves in ms: (default 10000) "];
 	...
 	case "fi"://by Christer82
		text = ["Hyökkäys =)","Väärä hyökkäystyyppi!","Ei koordinaatteja!","Ei riittävästi joukkoja! (Yhteensä #","Joukkoja ei valittu","Valmis","Luultavasti väärät koordinaatit","Ei tiedustelujoukkoja","Valmis","Hyökkäysallon asetukset:","Lisää uusi hyökkäysaalto","Nollaa","Hyökkäyksen asetukset:","Hyökkäystyyppi:","Normaali","Vahvistus","Ryöstö","Tiedustelijat Res/Troops","Tiedustelijat Def/Troops","Koordinaatit","Potki persuksille! =)","Saapumisaika","Saapuu:","Asetettu saapumisaika","Hyökkäysaaltojen lukumäärä","erottele '|' esim: 0,0|1,1","VIRHE:","Aktiivista kylää ei voida määrittää. Luultavasti yhden kylän tili","Saapumisaika asetettu:","Aika hyökkäysaaltojen välillä (ms): (oletus 500) "];
		cataText = ["Aseta kohde =)","Satunnainen","Puunhakkaaja","Savimonttu","Rautakaivos","Viljapelto","Sahaamo","Tiilentekijä","Rautavalimo","Viljamylly","Leipomo","Varasto","Viljasiilo","Aseseppä","Haarniskaseppä","Turnausareena","Päärakennus","Kokoontumispaikka","Marketti","Lähetystö","Kasarmi","Hevostalli","Työpaja","Akatemia","Kaupungintalo","Virka-asunto","Palatsi","Kauppamaja","Suuri kasarmi","Suuri hevostalli","Sankarin kartano","Suuri varasto","Suuri viljasiilo","Maailmanihme","Aarrekammio"];
break;
	...
	case "it": // Traduzione by Aries
	default:

		text = ["Fottilo ^_^","Tipo di attacco errato!","Manca coord!","Truppe insufficienti! (Truppa #","Inserire truppe","Avvio","Possibili coordinate errate","Nessun esploratore","Fatto","Imposta ondata:","Aggiungi ondata","Resetta","Imposta attacco:","Attacco Tipo:","Normale","Rinforzi","Raid","Spia Ris/Truppe","Spia Dif/Truppe","Coordinate","Fottilo! ^_^","Orario di arrivo","Arrivo a:","Imposta orario di arrivo","Numero di attacchi per quella specifica ondata","con ',' es: 0|0,1|1","ERRORE:","Impossibile trovare villaggio attivo. Probabile account con un solo villo","Orario di arrivo impostato a:","Tempo tra le ondate in ms: (default 10000) "];
		cataText = ["Select a target =)","Casuale","Segheria","Pozzo di argilla","Miniera di ferro","Campo di grano","Falegnameria","Fabbrica di mattoni","Fonderia","Mulino","Forno","Magazzino","Granaio","Fabbro","Armeria","Arena","Centro cittadino","Caserma","Mercato","Ambasciata","Campo addestramento","Scuderia","Officina","Accademia","Municipio","Residence","Castello","Ufficio commerciale","Grande caserma","Grande scuderia","Circolo eroi","Grande magazzino","Grande granaio","Meraviglia","Camera del tesoro"];
		break;
}
...
// Robin made changes here.  timebetweenwaves = document.getElementById('timebetween').value;
//		timebetweenwaves = parseFloat(document.getElementById('timebetween').value);
		timebetweenwaves = Math.ceil(parseFloat(document.getElementById('timebetween').value)*(1+Math.random()*.5));
...
attackInterface.innerHTML = '<fieldset><legend>'+text[12]+'</legend><div id=start>' +
	...
	'</tr><tr><td colspan="3">'+
		'<input type=\"text\" value="' +getCords() +  '" name=\"cords\" id=\"cords\" class=\"fm\" style=\"width: 98%\" /><br>'+ text[29] +'<input class=\"fm\" style=\"color:#777;\" id=\"timebetween\" type=\"text\" value=\"10000\" size=\"4\" />' +
	'</td>'+
	'</tr><tr><td>'+
...
table += '</tr></table><button id="newWaveButton">'+text[10]+'</button> <button id="resetButton">'+text[11]+'</button> <span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:grey;">( * - '+text[24]+' )</span>';
