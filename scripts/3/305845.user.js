...
try{
	...
	texte["msgMarketsaleContent"]="(.*) cump"+a_tilde+"r"+a_tilde+" \\s*(\\d+)x (.*?) pentru\\s*<br>\\s*(.*?) pD de la tine\\."; // The text where the information is stated. The information has to be replaced by "(.*?)".
	// For the following you have to take a message sent if you sell something via contract
	texte["msgContractsaleContent"]="(.*) a semnat unul dintre contractele tale!<br><br>\\s*Urm"+a_tilde+"toarele produse au fost v"+a_circumflex+"ndute:<br>([\\S\\s]*)\\s*<br>\\s*Cantitatea de (.*?) pD a fost ad"+a_tilde+"ugat"+a_tilde+" "+i_circumflex+"n contul t"+a_tilde+"u\\."; // The text where the general information is stated. The information has to be replaced by "(.*?)".
