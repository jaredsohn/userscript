// ==UserScript==
// @name                Google Reader + Douban
// @namespace      http://google.com/reader/userscript
// @description      Share posts to Douban from Google Reader
...
// Google Reader + Douban
// Adapted from http://userscripts.org/scripts/show/30832 by kuber
	...
	if (event.target.tagName=="DIV"){
		...
		try{
			if (event.target.className!=""){
				...
				btn.innerHTML="Recommend to Douban";
...
function postBookmark(event){
	...
	addbkmk.setAttribute('style',"top: "+(pos[1]-85)+"px; left: 28%; width: 350px !important;");
								...
								var editcontents=document.createElement("div"); 
									...
									notesinput=document.createElement("textarea");
										...
										notesinput.setAttribute("style",
											"width: 96% !important; height: 100px !important;");
...
function saveBookmark(event){	
	...
	GM_xmlhttpRequest({
		...
		data: 'title=' + encodeURIComponent(title) + '&uid=' + encodeURIComponent(url)  + '&comment=' + notes +'&type=I',
	});
