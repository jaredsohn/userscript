...
function init() {
	...
	if(window.location.href.match(/youtube.com\/watch/)) {
		...
		for (var i = 0; i < videoFormatsGroup.length; i++){
			...
			var url = videoFormatsElem[1].split(sep3)[1];
			var itag = videoFormatsElem[0].split(sep3)[1];
			var sig = videoFormatsElem[4].split(sep3)[1];
			videoURL[itag] = unescape(unescape(url)).replace(/\\\//g,'/').replace(/\\u0026/g,'&') + '&signature=' + sig;
		}
	...
	/* VIMEO */
		...
		GM_xmlhttpRequest({
			...
			onload: function(responseDetails) {
				...
				var videourl = 'http://www.vimeo.com/moogaloop/play/clip:' + vimeoid + '/' + request_signature + '/' + request_signature_expires + '/?q=' + isHD;
