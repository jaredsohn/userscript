...
// Italian (Thanks to rospo84)
} else if (lang == 'it') {

  var playMP4 = 'Riproduci MP4';
  var playFlash = 'Riproduci Flash';
  var stop = 'Stop';
  var expand = 'Espandi';
  var contract = 'Contrai';
  var hdOn = 'HD on';
  var hdOff = 'HD off';
  var download = 'Download';
  var optionsMenu = 'Opzioni';
  var optionsTooltip = 'Clicca per attivare la funzione';
  var update = 'Aggiorna Script';
  var previewsOption = 'Disabilita autoplay';
  var embedOption = 'Cambia Flash con MP4';
  var hdSupportOption = 'Attiva HD';
  var autoExpandOption = 'Espandi tutti i video';
  var correct43Option = 'Correggi video in 4:3';
  var autoLightsOutOption = 'Abilita lights out';
  var hideCommentsOption = 'Nascondi commenti';
  var hideAnnotationsOption = 'Nascondi annotazioni';
  var buttonStyleOption = 'Usa i bottoni blu';
  var autoCheckUpdatesOption = 'Verifica aggiornamenti';
  var debugModeOption = 'Abilita debug mode';
...
if (typeof(yt) != "undefined" && yt.getConfig('IS_WIDESCREEN')) {
  wideView = true;
} else if (typeof(myWindow.chrome) == "undefined" && myWindow.yt.getConfig('IS_WIDESCREEN')) {
  wideView = true;
}
...
if (typeof(yt) != "undefined" && yt.getConfig('IS_HD_AVAILABLE')) {
  hdAvailable = true;
} else if (typeof(myWindow.chrome) == "undefined" && myWindow.yt.getConfig('IS_HD_AVAILABLE')) {
  hdAvailable = true;
}

// Bad nasty redundant workaround that works (Thanks Google Chrome, I love you)
if (typeof(myWindow.chrome) != "undefined") {
  var hdTemp, wideTemp;
  hdTemp = false;
  wideTemp = false;
  for (i = 0; i < document.scripts.length && !wideTemp && !hdTemp; i++) {
    wideTemp = document.scripts[i].innerText.match("'IS_WIDESCREEN': true") != null;
    hdTemp = document.scripts[i].innerText.match("'IS_HD_AVAILABLE': true") != null;
  }
  wideView = wideTemp;
  hdAvailable = hdTemp;
}
...
if (typeof GM_getValue == 'function' && window.chrome == "undefined") {
...
function buildPlayer(player) {
  ...
  if (starTimeRE) {
    executeUnsafely('setTimeout((function() { seekTo(extractSeconds(starTimeRE[1])); }), 5000)');
  }
...
function stopPlayer() {
  toggleWidePlayer(false);
  video_player.innerHTML = '';
  video_player.appendChild(play_video_link);
  $('watch-player-div').style.height = '385px';
}

// Phisically expand the player
function toggleWidePlayer(wide) {
  if (wide) {
    executeUnsafely('yt.www.watch.player.enableWideScreen(true, "watch-wide-mode")');
  } else {
    executeUnsafely('yt.www.watch.player.enableWideScreen(false, "watch-wide-mode")');
  }
}

// To execute code in the unsafeWindow
function executeUnsafely(code) {
  // Bad nasty redundant workaround that works (Thanks Google Chrome for this, I love you)
  setTimeout("location.href = 'javascript:" + code + "'", 1);
}
...
function lightsOut() {
  ...
  var imgLightsOut = $E('img',
    { 'id': 'img-lights-out',
      'src': outImg,
      'style': 'position:fixed;top:0;left:0;width:100%;height:' +
...
if (video_player) {
  ...
  addStyle("\
    ...
    .watch-wide-mode #watch-this-vid { background: white; }\
    ...
    width: 960px !important; height: 575px !important; margin-left: 0px !important; }\
    #options-link { z-index: 50; }\
  ...
  // Avoid the WideView
  if (previews) toggleWidePlayer(false);
