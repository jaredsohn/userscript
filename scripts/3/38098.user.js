// ==UserScript==
// @name DreamF T3 AfterBeyond
// @author 2pac
// @namespace T3
// @version 3.4.5
// @description  (v 3.4.5 Travian v3 addons originally by Victor Garcia (aka Croc); - updated by Szabka pl51-7.June 2008; - updated by ms99 & Nux-04.December.2008
// @source http://userscripts.org/scripts/show/38098
// @identifier http://userscripts.org/scripts/show/38098.user.js
...
function functionMain(e) {
	...
	var SCRIPT = {
		url: 'http://userscripts.org/scripts/source/38098.user.js',
		version: '3.4.5',
		name: ':D',
		presentationurl: 'http://userscripts.org/scripts/show/38098'
	};
	...
	function switchLanguage(detectedLanguage) {
		switch (detectedLanguage) {
			...
			case "lv":
				...
				xLang['TRAVIANBEYONDSETUPLINK'] = 'Travian Beyondをセットアップ';
				...
				xLang['LOCKBOOKMARKS']		= 'ブックマークのロック<br>(削除,上移動,下移動アイコンを隠す)';
				xLang['MTCLEARALL']		= 'すべてを削除';
				xLang['UNLOCKBOOKMARKS']	= 'ブックマークのアンロック<br>(削除,上移動,下移動アイコンの表示)';
				...
				xLang['ATTACKS']		= '戦闘';
				xLang['NEWS']			= 'ニュース';
				xLang['ATTACKTYPE2']		= '派兵';
				xLang['DISTINFO']		= "あなたの村からの距離";
				xLang['TIMEINFO1']		= "必要な時間";
				xLang['TIMEINFOM']		= "商人";
				xLang['TIMEINFOT']		= "兵士";
				xLang['MAPSCAN']    		= 'マップをスキャン';
				xLang['ALLIANCEFORUMLINK']	= '外部のフォーラムへのリンク<br>(内部フォーラムを使う場合は書かないでください。)';
				xLang['NOTEBLOCK']		= 'ノートブロックを表示する';
				xLang['NBSIZE']			= 'ノートブロックのサイズ';
				xLang['NBHEIGHT']		= 'ノートブロックの高さ';
		...
		if (boolShowBookmarks == "1") {
			...
			for (var i = 0; i < marcadores.length; i++){
				...
				if (boolLockBookmarks != "1") {
					...
					var editLink = elem("A", "<img src='" + image["editbookmark"] + "' style='cursor:pointer' title='" + T('EDIT') + "' alt='" + T('EDIT') + "'>");
                ...
                for (var i = 0; i < maxCount; i++) {
                    ...
                    if (lvlBuilding){
						...
						if (!isNaN(gid)) {
							...
							} else if (lvlBuilding[0] == getBuildingMaxLevel(gid) || lvlBuilding[0] == 100) {
								aDIV.style.backgroundColor = CN_COLOR_MAX_LEVEL;
							} else {
	...
	function NPCAssistant(typeNPC, xpNeeded, multipliers) {
		...
		for (var i = 0; i < xpNeeded.snapshotLength; i++) {
			...
			if (neededTotal > 0 && r >= 0 && (time_saved > 0 || time_saved == null)) {
				...
				//log(3, "urlNPCback = " + urlNPCback);
				//log(3, "currentBuildingName = " + currentBuildingName);
				//urlNPCback = addQueryParameter(urlNPCback, NPCbacklinkName, (currentBuildingName.length > 1 ? currentBuildingName[i] : currentBuildingName[0]));
