...
// @name	HerrJeanke's Choice (not my script) : Travian: Market++ v1
...
function onLoad(){ // calls to the functions and procedure
	...
	if(!fields[lang]){//if we can't find the names use the image names
		...
		switch(lang){//only load the lang you need. Might look strange if you don't view as UTF-8
			...
			break;
			case '.nl':
			langfile[lang]=['Aanbiedingen','Bod accepteren','Te weinig grondstoffen', 'Gezocht','U zoekt','Verwijderen','Herstellen'];
			break;
			case '.ru':
			langfile[lang]=['???????????','??????? ???????????','?? ??????? ?????','?????','???????????','???????','?????'];
			break;
...
var cell = document.createElement('td');
	...
	row2.appendChild( cell );
		...
		cell = document.createElement('td');
		con.appendChild( document.createTextNode( "less" )   );
		con.href='#';
		...
		cell = document.createElement('td');
		con.appendChild( document.createTextNode( " 1:1 " )   );
		con.href='#';
		...
		con = document.createElement('a');	
		cell = document.createElement('td');
		con.appendChild( document.createTextNode( "more" )   );
		con.href='#';
