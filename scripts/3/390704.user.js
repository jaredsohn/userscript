...
// @include        http://*.live365.com/web/*
// @include        http://live365.com/stations/*
// ==/UserScript==

//v3.0.2
// ==> Script update by: CoZmic NoMad <==
// Site JavaScript is written crappy because it contains multiple id's with the same name. Therefore not all adds are removed
// Changed the delElem function to delDivElem which removes an element when it encounters it. I know its crappy but so is the site.
//
//v3.0.1 
// ==> Script update by: CoZmic NoMad <==
// No more annoying adds => The adds region is removed.
// '.pls' file included. This enables you to 'open' the stream (file) in your favourite stream player
// 
//v3.0
...
function insPlsElem(lnk) {
  if (lnk) {
    var lnk2=document.createElement("a");
    lnk2.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent("[playlist]\r\nNumberOfEntries=1\r\nFile1=" + lnk));
    lnk2.setAttribute('download', "Live\ 365\.pls");
    lnk2.textContent="Download \pls \File";
    lnk2.innerHTML=lnk2.innerHTML + "\<br\ \/\>";
    naddElem(lnk2);
  }
}
...
function delDivElem(elm) {
  var scriptTag = document.getElementsByTagName('div');
  scriptTag = scriptTag[scriptTag.length - 1];
  if (scriptTag.id = elm) { scriptTag.parentNode.removeChild(scriptTag); }
}
...
if (!l.match(/\/schedule$/i)){
	...
	if (lspln.match(/index.live/gi)) {
      ...
      delDivElem("adCol"); // Deletes annoying add
