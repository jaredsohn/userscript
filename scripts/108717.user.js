...
//logit ('g_timeoff: '+ uW.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap}\
    ...
    .ptChatAlliance {font-weight:bold}\
     ...
     .ptChatChancy {color:#000; background-color:#FC3D3D;}\
     .ptChatVCs {color:#000; background-color:#00ff00;}\
    .ptChatOfficers {color:#000; background-color:#FFEA00;}\
...
// "Report No: 5867445"  --->  see uW.modal_alliance_report_view()
 ...
 chatDivContentHook : function (msg){
	...
	if (Options.chatLeaders) {
		if (Options.AllianceVCs.indexOf(uW.tvuid) >= 0 && suid.substr(0, 3)=="div") IsMe = true;
    	if (Options.AllianceChancy.indexOf(suid) >= 0 || IsMe) element_class = 'ptChatChancy';
     if (Options.AllianceLeaders.indexOf(suid) >= 0 || IsMe) element_class = 'ptChatOfficers';
      if (Options.AllianceVCs.indexOf(suid) >= 0 || IsMe) element_class = 'ptChatVCs';
    }
	IsMe = false;
...
function getAllianceLeaders (){
    ...
    new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/allianceGetLeaders.php" + unsafeWindow.g_ajaxsuffix, {
		 ...
		 if (rslt.officers) {
                 // logit("officers: " + inspect(rslt, 8, 2));
	         Options.AllianceChancy = [];
                 Options.AllianceVCs = [];
                 Options.AllianceLeaders = [];
       for (add in rslt.officers) {
          if (rslt.officers[add].type == "CHANCELLOR")
            Options.AllianceChancy.push(rslt.officers[add]['userId']);
          if (rslt.officers[add].type == "VICE_CHANCELLOR")
            Options.AllianceVCs.push(rslt.officers[add]['userId']);
          if (rslt.officers[add].type == "OFFICER")
            Options.AllianceLeaders.push(rslt.officers[add]['userId']);

          // logit("name: " + rslt.officers[add].genderAndName);
          // logit("rank: " + rslt.officers[add].type);
       }
     } 
     else {
