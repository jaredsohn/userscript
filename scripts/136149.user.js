// ==UserScript==
// @name           Flickr Show Groups Discuss Last Post Date
// @namespace      http://www.flickr.com/alesadam
// @description    Shows the date of the last Discuss post in a group, on your Groups page.
// @creator        Aleas Dam (http://flickr.com/alesadam/)
// @date           15/06/2012
// @version        1.2
// @modified	   Mar. 01, 2013
//
...
//
// @updateURL	   https://userscripts.org/scripts/source/136149.meta.js
// @downloadURL    https://userscripts.org/scripts/source/136149.user.js
// @run-at	   document-end
// ==/UserScript==
//

(function () {
var FSGPDversion = "1.2";
...
['toString', 'toLocaleString', 'valueOf', 'toSource', 'watch', 'unwatch', 'hasOwnProperty', 'isPrototypeOf', 'propertyIsEnumerable'].each(function (method) {
	...
	var retval = getJSVariable(/\"api_key\"[ :]+\'([^\']+)\'/);
	if (!$chk(retval)) retval = getJSVariable(/global_magisterLudi[ =]+\'([^\']+)\'/);
	return retval;
    }

    GM_getAuthHash = function () {
	var retval = getJSVariable(/\"auth_hash\"[ :]+\'([^\']+)\'/);
	if (!$chk(retval)) retval = getJSVariable(/global_auth_hash[ =]+\'([^\']+)\'/);
	return retval;
    }

    GM_getAuthToken = function () {
	var retval = getJSVariable(/\auth_token\"[ :]+\'([^\']+)\'/);
	if (!$chk(retval)) retval = getJSVariable(/global_auth_token[ =]+\'([^\']+)\'/);
	return retval;
    }
...
if (Browser.Engine.webkit || // Chrome, Safari
    ...
    var keyPrefix = 'FlickrShowGroupsDiscussLastPostDate.';
...
function showUpdateNotification() {
    ...
    new Element('a', {
        html: 'Flickr Show Groups Discuss Last Post Date: update available',
    ...
    new Element('a', {
        ...
        href: 'http://www.flickr.com/groups/flickrhacks/discuss/72157630070429047/',
        styles: {
            'text-decoration': 'none'
        },
        target: '_blank'
...
function checkVersion() {
  try {
    ...
    if (!lastVersionCheckTime || elapsedtime / 1000 > 60 * 60 * 12) { //more then 12h ago
    	getThreadInfo({ threadId: '72157630070429047', callback: function(retval) {
    ...
    if ($chk(onlineVersion)) {
        ...
        var currentVersionParts = reVersionMatch.exec(FSGPDversion);
...
GM_setValue("version", FSGPDversion);

checkVersion();

// the real code
function getLastPostDate(groupId) {
			new Request({
       				url: 'http://www.flickr.com',
       				onSuccess: function (responseText, responseXML) {
			            try {
					...
					if (result.stat === 'fail') {
						new Element('img', {
							src: images.error,
							title: "failed retrieving last post date: " + result.message,
							height: '9px'
						}).replaces($('FSGPD-label-' + groupId));
					} else {
						var label = $('FSGPD-label-' + groupId);
						label.setStyles({ display: 'inline', visibility: 'visible' });
						if ($chk(result.topics.topic)) {
							var latestPost = result.topics.topic.reduce(
								function (topicA, topicB) {
									var lastA = parseInt(topicA.datelastpost,10);
									var lastB = parseInt(topicB.datelastpost,10);
									if (lastA > lastB) return topicA;
									return topicB;
								}
							);
								
							var now = new Date().getTime() / 1000; // working in seconds
							var latest = parseInt(latestPost.datelastpost,10);
							var timeago = now - latest;
							if ((now - latest) < 90) { // 90 seconds ~ 2 minutes
								timeago = Math.round(timeago) + 's';
							} else if (timeago < (60 * 60)) { // < 1h
								timeago = Math.round(timeago / 60) + 'm';
							} else if (timeago < (24 * 60 * 60)) { // 24h
								timeago = Math.round(timeago / (60*60)) + 'h';
							} else if (timeago < (30 * 24 * 60 * 60)) { // 30d ~ 1month
								timeago = Math.round(timeago / (60*60*24)) + 'D';
							} else if (timeago < (12 * 30 * 24 * 60 * 60)) { // ~ 1 year
								timeago = Math.round(timeago / (60*60*24*30)) + 'M';
							} else {
								timeago = Math.round(timeago / (60*60*24*30*12)) + 'Y';
							}
							label.set('html', timeago);
						} else {
							label.set('html', '-');
							label.set('title', 'no posts?');
						}
					}
				    } catch (e) {
				    	GM_log("ERROR processing result: " + e);
					new Element('img', {
						src: images.error,
						title: "ERROR processing result: " + e,
						height: '9px'
					}).replaces($('FSGPD-label-' + groupId));
				    }
				},
				onFailure: function (response) {
		            ...
		            group_id: groupId,
			    page: 1,
			    per_page: 21 // there could be 20 stickies, all older than the latest post
			});
}

$$('li[id^=group_]').each(function(listItem) {
	try {
		var groupId = listItem.get('id').match(/group_(\d+@\w\d{2})/)[1];
	} catch (e) {
		GM_log("unable to extract group id: " + e + " - ignoring");
		return;
	}
	var label = new Element('small', {
		id: 'FSGPD-label-' + groupId,
		styles: {
			visibility: 'hidden',
			display: 'none'
		}
	}).inject(listItem);
	getLastPostDate(groupId);
});
