// ==UserScript==
// @name          WannaRapeATitty
// @namespace     nobody in particular
...
{  // this appends the setting below the new tabbed interface -- usually. 
	...
	mH.push("<tr><td colspan='9' "+TSUI_tdhd+"'><center>Titan Stride QW  (8/23/11 18:00ST) Settings</center></td></tr>");
	...
	dEbId('FS_Stride_Save').addEventListener('click',function(){
		...
		if ( NaN != mdt ) {  // make sure the user gave us a number
			FSST_map_dt = ( mdt < 2 ) ? 2 : mdt;  // 5 times a second is not fast enough for everyone!
		}
