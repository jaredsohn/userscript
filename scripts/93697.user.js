...
unsafeWindow.culMsg = function(checkbox)
{
	var txt = document.getElementById('text');
	if(txt.value.substr(0,3)!='cul')
	{
		txt.value = 'cul '+ txt.value;
	}
	else
	{
		txt.value = txt.value.substr(4);
...
if(jsGet('view')=='sendIKMessage')
{
	...
	obj.innerHTML = '<b>WAZNE:</b> <input type="checkbox" id="importantMsg" onClick="impMsg(this);" /><b>Kulturka:</b> <input type="checkbox" id="culturalMsg" onClick="culMsg(this);" />';
...
if(jsGet('view')=='diplomacyAdvisor'||document.getElementsByClassName('entry')[0].id.substr(0,7)=='message')
{
	...
	for (var i = 0; i < msgIds.length; i++) {
		...
		if(msg.innerHTML.substr(0,3)=='cul') { var isCul = true; } else { var isCul = false; }
		if(isImp) { header.style.backgroundColor = '#F39E9E'; msg.innerHTML = msg.innerHTML.substr(4,msg.innerHTML.length); }
		else if(isCul) { header.style.backgroundColor = '#C0F7FE'; msg.innerHTML = msg.innerHTML.substr(4,msg.innerHTML.length); }
...
if(jsGet('view')=='diplomacyAdvisorOutBox')
{
	...
	for  (var i in inputs)
	{
		if(inputs[i].type == 'checkbox')
		{
			...
			if (msg.innerHTML.substr(0,3)=='cul') { var isCul = true; msg.innerHTML = msg.innerHTML.substr(4,msg.innerHTML.length); } else { var isCul = false; }	
			if(isImp) { header.style.backgroundColor = '#F39E9E'; }
			else if(isCul) { header.style.backgroundColor = '#D0BCFE'; }
