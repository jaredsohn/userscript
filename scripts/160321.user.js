...
my.Generals = {
 ...
 init : function (){
  ...
  show : function (){
    ...
    t.cont.style.maxHeight = (Options.HeightTools)-30+'px'; 
    var m = '';
 ...
 my.Crest = {
  ...
  init : function (cont){
   ...
   getContent : function (){
    ...
    m += '<br><br><b><center>'+translate('Please support the GoR BoTTols by clicking the ads on the GoR BoTTols website from time to time')+'!</b><br><br>'+pubblicity;
...
/*********************************** AutoAttack for Generals (Germania) ***********************************/
my.Germ = {
  cont : null,
  timer : null,
  pop:null,
    ...
    var t = my.Germ;   
      try {
		var m = '<DIV class=ptstat>'+translate('GERMANIA')+'</div>';
			...
			m+= '</td><td style="text-align:center; vertical-align:middle;" width=50%>'+pubblicityQuadrato+'</td></tr></table>';
			t.cont.innerHTML = m; 
			t.UpdateSelect(1);
...
my.AutoRes = {
  ...
  init : function (){
    var t = my.AutoRes;
    t.cont = document.createElement('div');
    if (Options.ActiveResearch) t.Start();
    return t.cont;
  },
    ...
    t.cont.style.maxHeight = (Options.HeightTools)-30+'px';  
      try {      
			var m = '<DIV class=ptstat>'+uW.arStrings.UserEngagement.ResearchTitle.toUpperCase()+'</div>';
				...
				m+= '<tr><td width="70%" style="text-align:center; vertical-align:middle;">';
					...
					uW.Research.orderedTechIds.each(function (r) {
						if (Options.EnResearch[r] == undefined) Options.EnResearch[r] = false;
						if (Options.SelResearch[r] == undefined) Options.SelResearch[r] = 1;
						if (Options.CityResearch[r] == undefined) Options.CityResearch[r] = 0;

						var q = uW.player.technologies[r].level();
						var f = uW.Research.checkRequirementForLevel(r, q + 1)[3].include(0)
						m+= '<tr><td><input type=checkbox class="'+r+'" id="EnRes'+r+'" '+ (Options.EnResearch[r]?'CHECKED ':'')+'> '+uW.arStrings.techName["t" + r];
						...
						for (var z=1; z <= 10; z++) {
							m+= '<option value="'+z+'" ' + (Options.SelResearch[r]==z?'SELECTED':'') + '>'+z+'</option>';
						}
						m+= '</select></center></td>';
						...
						for (var c=0; c<Cities.numCities; c++){
							...
							if (Ctr > 0) {
								m+= '<option value="'+c+'" ' + (Options.CityResearch[r]==c?'SELECTED':'') + '>'+Cities.cities[c].c.name+'</option>';
							}
						}
						m+= '</select></center></td>';
						m+= '</tr>';
				...
				m+= '<br><br><input type=checkbox id="HelpRes" '+ (Options.HelpResearch?'CHECKED ':'')+'> '+uW.arStrings.BuildingModal.RequestHelp;
				m+= '<br><br><br>'+pubblicityQuadrato+'</td></tr></table>';
    		t.cont.innerHTML = m;
    		...
    		ById("HelpRes").addEventListener('click', function(e) {
				Options.HelpResearch = ById("HelpRes").checked;
       			saveOptions();
     		}, false);    
     				
    		uW.Research.orderedTechIds.each(function (r) {
    			ById("EnRes"+r).addEventListener('click', function(e) {
					Options.EnResearch[e.target['className']] = ById("EnRes"+e.target['className']).checked;
       				saveOptions();
     			}, false);
     			
    			ById("SelectRes"+r).addEventListener('change', function(e) {
					Options.SelResearch[e.target['className']] = ById("SelectRes"+e.target['className']).value;
       				saveOptions();
     			}, false);     			

    			ById("CityRes"+r).addEventListener('change', function(e) {
					Options.CityResearch[e.target['className']] = ById("CityRes"+e.target['className']).value;
       				saveOptions();
     			}, false);
  ...
  Start : function (){
  	...
  	if (Options.ActiveResearch) {
		uW.Research.orderedTechIds.each(function (r) {
			if (Options.EnResearch[r] && !Cities.cities[Options.CityResearch[r]].c.queues.research.active()) {	// Se attiva la ricerca e la città non sta ricercando
				if (!t.otherCityIsResearching(r)) {	// Controllo se la ricerca è già avviata
					...
					if ((uW.player.technologies[r].level()+1) <= Options.SelResearch[r] || (uW.player.technologies[r].level()+1) <= 10) {
						if (!uW.Research.checkRequirementForLevel(c, b)[3].include(0)) {
							var a = {};
            					a.cid 	=	Cities.cities[Options.CityResearch[r]].c.id;
            					a.lv	=	b;
            					a.tid	=	c;
            					uW.AjaxCall.gPostRequest("research.php", a, 
            						function (d) {
										...
										Cities.byID[Cities.cities[Options.CityResearch[r]].c.id].c.queues.research.addSlot(z);
										                    					
										if (Options.HelpResearch) {
						...
						Options.EnResearch[r] = ById("EnRes"+r).checked;
						saveOptions();
  ...
  ToggleAutoResearch: function() {
	...
	if (Options.ActiveResearch == true) {
		Options.ActiveResearch = false;
		if (obj) obj.value = translate('AUTO')+' '+uW.arStrings.Common.Research.toUpperCase()+' = OFF';
		saveOptions();
	}  else {
		Options.ActiveResearch = true;
...
/***************************** AUTO ATTACK TAB - Modded by Tiestoale **************************/
my.AutoAtt = {
  cont : null,
  timer : null,
    ...
    t.cont.style.maxHeight = (Options.HeightTools)-30+'px';   
      try {      
		var m = '<DIV class=ptstat>MULTI '+uW.arStrings.Common.Attack.toUpperCase()+'</div>';
			...
			m+= '</td><td width=20%>'+pubblicity+'</td></tr></table>';
			m+= '<DIV class=ptstat>'+translate('CONFIGURATION')+'</div><table width=100%><tr>';
			m+= '<td width=50%>'+translate('Time between attacks')+": <input type=text size=3 id=BOAttackInterval value='"+ parseInt(AutoAttack.AttackInterval)+"'> "+translate('seconds')+'</td>';
			...
			if (!Cities.byID[id].c.canMarch()) {	// Non può marciare (ES. Raduno completo)
				...
				if (Cities.byID[id].c.generalsCount() == 0) {	// Nessun generale nominato
					...
					if (selectedKnight == 0) {	// Nessun generale disponibile
						actionLog('Multi'+uW.arStrings.Common.Attack,'('+CooX+','+CooY+') '+Cities.cities[c].c.name+': <font color=red>'+translate('No general available')+'</font>');
					} else {	// Errore sconosciuto
						actionLog('Multi'+uW.arStrings.Common.Attack,'('+CooX+','+CooY+') '+Cities.cities[c].c.name+': <font color=red>'+translate('Unknown error')+'</font>');
					}
				}
			}
		}
...
<br><br><button  id="BOAcceptAllKDO" class="button25" type="button"><span>'+uW.arStrings.Gift.ButtonAcceptAll+'</span></button><br><br><hr><b><center>'+translate('Please support the GoR BoTTols by clicking the ads on the GoR BoTTols website from time to time')+'!</center></b><br><center>'+pubblicity+'</div>';
...
<DIV class=ptstat>'+uW.arStrings.Common.Troops.toUpperCase()+'</div><TABLE align=center CLASS="MYTABLE" cellpadding=4 cellspacing=0><tr><td>'+uW.arStrings.Common.Troops+'</td><td>'+uW.arStrings.Common.Glory+'</td><td>'+translate('Life')+'</td><td>'+uW.arStrings.Common.Attack+'</td><td>'+uW.arStrings.Troops.Defense+'</td><td>'+translate('Speed')+'</td><td>'+translate('Range')+'</td><td>'+translate('Load')+'</td><td>'+uW.arStrings.Common.UpKeep+'</td></tr>';
...
my.News = {
  ...
  init : function (){
	...
	GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://www.kocbottols.tiestoale.com/Tools/Addon/News/GoRNews.txt',
			headers: {
...
function DashInnert (){
    ...
    var m = '<div class="ptstat">DASHBOARD</div>';
    	...
    	m+= '<br><table width="70%">';
    		...
    		m+= '</tr>';
    		m+= '<tr><td style="text-align:center; vertical-align:middle;">';
    		if (Options.ActiveResearch) m+= '<INPUT id="DAResAuto" class="'+Red+'" type=submit value="Auto'+uW.arStrings.Common.Research+' = ON"></td>';
...
my.scanmap = {
  ...
  show : function (){
    ...
    t.cont.innerHTML = '<CENTER><iframe src="http://www.kocbottols.tiestoale.com/Pubblicita/PubbliScanMap.php" width="750" height="690" id="BOFrame" border=0></iframe><BR></center>'; 
 	} catch (e) {
        t.cont.innerHTML = '<PRE>'+ e.name +' : '+ e.message +'</pre>';  
      }      
  },
...
my.Options = {
  ...
  show : function (){
    var t = my.Options;   
      ...
      try {      
        m = '<DIV style="height:670px; max-height:670px; overflow-y:auto">\
          ...
          <TR><TD>&nbsp;</td><TD>'+translate('Language')+': '+ htmlSelector({0:'...', 1:'EN', 2:'IT', 3:'FR', 4:'NL', 5:'TR', 6:'DK', 7:'GE', 8:'SP'},Options.Lingua,'id=BOLangue') +'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id=SpanLang></span></td></tr>\
         ...
         <TR><TD><INPUT id=pbRessEnable type=checkbox '+ (Options.pbRessEnable?'CHECKED ':'') +'/></td><TD>'+translate('Auto-collect resources every')+' <INPUT id=pbLimitRess type=text size=2 maxlength=3 value="'+ Options.pbRessTime +'"\> '+translate('minutes')+' &nbsp; &nbsp; &nbsp; &nbsp; <input type=button id="CollectNow" value="'+uW.arStrings.MainChrome.CollectResources+'"></td></tr>\
        ...
        <TR><TD colspan=2><hr><b><br><br><center>'+translate('Please support the GoR BoTTols by clicking the ads on the GoR BoTTols website from time to time')+'! </center></b><br><center>'+pubblicity+'</center><hr><DIV class=ptstat>'+translate('Configuration of chat')+'</div></td></tr>\
	...
	<TR><TD></td><TD><INPUT type=submit value="'+translate('Tester')+'" id=boPlayNow> &nbsp; <INPUT id=boSoundStop type=submit value="'+translate('Stop Sound Alert')+'"></td></tr></table></div></td></tr>\
        </table></table><BR><BR><HR><center><b>'+ miseajour +'</b> (Based on BeWorld\'s Script.)</center><br><br><hr><b><br><br><center>'+translate('Please support the GoR BoTTols by clicking the ads on the GoR BoTTols website from time to time')+'!</center></b><br><center>'+pubblicity+'</div>'
        t.cont.innerHTML = m;
  ...
  fetchPlayerStatus : function (uidArray, notify){
   ...
   gotPlayerDetail : function (rslt, span){
      ...
      var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>'+uW.arStrings.Common.Details+':</b> &nbsp; </td><TD>'+uW.arStrings.Common.Alliance+': '+ a +' &nbsp; '+uW.arStrings.Common.Cities+': '
            + u.cities +' &nbsp; '+uW.arStrings.Common.Population+': '+ u.population +'</td></tr><TR><TD></td><TD>'+translate('Provinces')+': ';
...
function saveAutoAttack (){
  var serverID = getServerId();
  setTimeout (function (){GM_setValue ('AutoAttack_' +serverID, JSON2.stringify(AutoAttack));}, 0);
}
...
function readAutoAttack (){
  var serverID = getServerId();
  s = GM_getValue ('AutoAttack_'+serverID);
  if (s != null){
    opts = JSON2.parse (s);
    for (k in opts){
        AutoAttack[k] = opts[k];
...
function AddMainTabLink(text, eventListener, mouseListener) {
  ...
  if(tabs) {  
		...
		for (var i = 0; i < col_wrapper.length; i++) {
    		if (Options.HideTab && (col_wrapper[i].href == "http://community.kabam.com/forums/forumdisplay.php?11-Glory-of-Rome" || col_wrapper[i].href.indexOf("salesforce/help") >= 0 || col_wrapper[i].href == "http://www.facebook.com/apps/application.php?id=140956165916773" || col_wrapper[i].href == "https://apps.facebook.com/gloryofrome/")) {
    	...
    	if (mouseListener != null)
       	...
       	GM_xmlhttpRequest({method:"GET",url:"http://www.kocbottols.tiestoale.com/Tools/GoR/bottols.user.js",onload:function(a){var b=/\/\/\s*@version\s+(.+)\s*\n/i.exec(a.responseText);
...
function alertlang () {
		...
		var m = '<center><b><i>Thank you for installing the '+ScriptName+'</i>';
			...
			m+= '<br><br>'+htmlSelector({0:'...', 1:'ENGLISH', 2:'ITALIANO', 3:'FRANÃ‡AIS', 4:'DUTCH/FLEMMISH', 5:'TURKISH', 6:'DANISH', 7:'GERMAN', 8:'ESPAÑOLAS'},Options.Lingua,'id=BOFirstLangue');
