// ==UserScript==
// @name           Wall Manager Sidekick (Cafe World)
...
(function() { 
	...
	function dock(){
	    try{
		var tests=[
            {link:["Play Game"],ret:"exclude"},

			{url:["neighbors.php","track.php","food_showdown_poll","cma_brag"],ret:"exclude"},

			{url:["mystery_spice","get_spice.php"],ret:"sendmagicspice"}, 

			{url:"FriendSpice",ret:"spices"},

			{url:["CoffeeFirstBrew","MachineMastery"],ret:"energy"},

			{url:["QuestCompleteFeed"],ret:"coins"},

		    {url:["CateringMissionReminder","CateringHelpFriend","accept_achievement_bonus.php","Collections",
                        "CheckInSystemFeed","BuildAnX","PastryStationFeed","OutsideMission","AssemblyThanks",
                        "BBQGrillFeed","VIPServed","IndoorGarden"
                       ],ret:"reward"},
			...
			{url:["QuestAskForHelp","askforhelp","Patricks2012","cafeinparis","FeedRequestDriver","questPart",
                        "FeedBoothsfeed","rock_band","door","stove_shop_feed","Memories","BabyShower","Honeymoon",
                        "Winter"
                       ],ret:"sendquest"},

			{url:"scratch_lotto.php",ret:"lotto"},

			{url:["DailyBonus","FriendMission","p=spoil_saved","accept_chef_special.php",
                        "accept_taste_testing.php","FreeGift"
                       ],ret:"food"},
		];
		...
		accTextCustoms= {
			sendspecialdelivery: "Special Delivery",sendmagicspice:"Magic Spice",coins:"Coins",
		...
		menu= {
			section_main:{type:"section",label:appName+" ("+version+")",kids:{
				...
				sep:{type:"separator",label:"Basics",kids:{
					...
					coins:{type:"checkbox",label:"Get Coins"},
				...
				helpseparator:{type:"separator",label:"Help Friends",kids:{
					sendhelp:{type:"checkbox",label:"Join Catering Crews"},
	...
	function read(){
		...
		if (text.find("Looks like all the prizes have been claimed!"))statusCode=-2;//all out
		...
		else if (text.find('You have already claimed this reward')) statusCode=-6; //already claimed
                ...
                else if (text.find("already helped the allowed number of friends")) statuscode=-2 //over limit                
                else if (text.find("reward has expired")) statuscode=-2 //expired
                else if (text.find("you still get 1000 coins")) statuscode=-2 //not yours
