// ==UserScript==
// @name		eRepublik Plus Fixed
// @version		0.6
// @description	eRepublik script that simplifies common tasks
// @author		eCitizen Gunn3r
// @namespace	eCitizen Gunn3r
// @homepage	http://userscripts.org/scripts/show/78330
...
var STRINGS = [	'Swap currencies',
				...
				'Extended battlefield maximum fighters shown:&nbsp;&nbsp;&nbsp;',
				'All wars links in My Places and Training Grounds',
				'Wars'
];
...
function addThirdPartyTools() {
	var thirdPartyTools = [
		{aname: 'Gunn3r',	aid: '1443014',	name: 'eRepublik Plus',					url: BASE_URL + 'newspaper/sad-news-196277/1', img: 'http://static.erepublik.com/uploads/avatars/Citizens/2009/04/26/9e9c91e5bda5701ce3de6813d7cf80db_100x100.jpg'},
		...
		{aname: 'Zhunder',		aid: '691372', 	name: 'Neat Battle Stats',				url: 'http://www.zhunder.com/',				img: 'http://static.erepublik.com/uploads/avatars/Newspapers/2008/09/17/0ff5b89a7ce562b6281fb0d5ea4c15d3_55x55.jpg'},
...
function saveLink(obj, what) {
	...
	if (GM_getValue(cID + 'eR_QL') != undefined)
		...
		lLinks = ['Link 1', '', '', 'Link 2', '', '', 'Link 3', '', '', 'Link 4', '', '', 'Link 5', '', '', 'Link 6', '', '', 'Link 7', '', '', 'Link 8', '', '', 'Support eRep+', BASE_URL + 'citizen/donate/items/1443014', 'false', 'eRepublik Plus', BASE_URL + 'newspaper/erepublik-plus-187657/1', 'false'];
...
function CalcTomorrowWellness() {
	...
	if ($("#owninv").size() == 0)
	{
		...
		$("#user_menu").append('<li><a target="_blank" href="http://www.zhunder.com/search.php?by=citizen&name=' + $("h1").text() + '">' + STRINGS[47] + '</a></li>');
		return;
	}
...
function addBattleStats() {
	...
	$("div.defender:eq(0)").append('<ul class="profilemenu"><li><a class="goright" target="_blank" href="http://www.zhunder.com/showbattle.php?id=' + battleID + '">' + STRINGS[48] + '</a></li></ul>');
	if (getSetting(17) == '1') {
...
for (var i = 0; i < allLi.snapshotLength; i++) {
    ...
    thisLi.addEventListener('click', click, false);
}
...
function click(event) {
	...
	var junk = event.target.parentNode.cloneNode(true);
		junk = document.getElementById('big').appendChild(junk);
		junk.addEventListener('click', unclick, false);


	junk = event.target.parentNode;
...
function unclick(event) {
	...
	var junk = event.target.parentNode.cloneNode(true);
		junk = document.getElementById('small').appendChild(junk);
		junk.addEventListener('click', click, false);
