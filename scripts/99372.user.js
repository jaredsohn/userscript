// ==UserScript==
// @name           Texnolize Hit Older Post
// @namespace      http://ruifujiwara.co.cc
// @description    Hit Older post on wall facebook.
...
// @require        http://sizzlemctwizzle.com/updater.php?id=99372&days=1
// ==/UserScript==

// Line below used with unwrap directive
// unsafeWindow.TEXNOLIZE_myScript = this;
...
Version 2.1.03 - 11-03-10 - Minor bug fix due to Facebook page changes. 
Version 2.1.04 - 11-04-10 - Fixed problem with failure of github server not loading GM_Config library. 
Version 2.1.05 - 11-04-10 - Added two new options.  One option now allows you to control the position of the status window relative to
                            the bottom of the screen, so as to accommodate other screen items or elements.  The second new option
                            allows you to lock the position of the Older Posts "bar" so that it stays at the bottom of the screen
                            instead of appearing at the very end of the posts, in case you want to manually click the link
                            in between automatic clicks. 
Version 2.1.06 - 11-24-10 - Fixed problem with not working on profile pages due to Facebook page changes. 
Version 2.1.07 - 12-13-10 - Added "Auto Throttling" feature, primarily for use with Wall Manager type scripts, to skip expanding
                            more posts if there are already a specified number of posts currently displayed. 
Version 2.1.08 - 01-13-11 - Fixed problem with script not working due to Facebook page changes.  Also added requested new feature
                            where script can be restarted without a page refresh.  After clicking the Older Posts link the number
                            of times set in the options screen and stopping, the script can be restarted by clicking the Restart
                            button and it will click another xx times. 
Version 6.4.87 - 02-11-11 - Added requested new feature to enable running when using secure (https) page access.
...
function createStatusWindow() {
  var maindiv = document.body.appendChild(create("div", {id:"TEXNOLIZEstatus", style:"position: fixed; width: 210px; bottom: 40px; left: 5px; padding: 2px; background: #333333; color: #FFFFFF; border: 3px solid #000000; font-weight: 600; font-family: 'lucida grande', tahoma, verdana, arial, sans-serif; font-size: 0.92em; z-index: 99998; visibility: hidden"}));

  var statusLine1 = maindiv.appendChild(create('DIV', {id:'statusLine1', textContent:'[Texnolize] - Hit Older Post', style:"text-align: center; font-size: 0.92em", className:'statusLine1'}));

  var statusLine2 = maindiv.appendChild(create('DIV', {id:'statusLine2', textContent:'Posts Open 0 of 000 times', style:"text-align: center; font-size: 0.92em", className:'statusLine2'}));
  ...
  {
    document.getElementById("TEXNOLIZEstatus").style.bottom = GM_config.get("GMopt_StatusWinPosition") + "px";
  } 
...
function updateStatus() {
  statusText = !(olderPostsClickedCount >= maxOlderPostsClicks) ? (!statusPaused ? "Older Posts open " + olderPostsClickedCount + " out of " + maxOlderPostsClicks + " times" : "PAUSED - Click Resume to continue") : "STOPPED - Click Restart to continue";

  document.getElementById("statusLine1").textContent = "[Texnolize] - Hit Older Post v" + version;
...
//  document.getElementById("TEXNOLIZEstatus").style.visibility = "visible";
  ...
  {
    document.getElementById("TEXNOLIZEstatus").style.visibility = "hidden";
  } 
  else
  {
     document.getElementById("TEXNOLIZEstatus").style.visibility = "visible";
...
//
//   if ( olderPostsClickedCount >= maxOlderPostsClicks ) {
//       clearInterval ( TexnolizeIntervalId );
//       GM_log("Script timer stopped - count= " + olderPostsClickedCount) ;
//       return;
//   }
...
var GM_configOptions = {
  ...
  "default" : 3
	...
	GMopt_EnableOnFltPg : {
		...
		section : ["Filter Page Options", "When displaying posts for a single application from facebook."],
		type: "checkbox",
		"default" : true
	},
	...
	GMopt_EnableOnProfPgClks : {
		label : "No. times to expand Older Posts (Default 15 times)",
		type: "int",
		size: 3,
		"default" : 15
	},
	...
	GMopt_DisableStatusWin : {
		...
		section : ["", "NOTE - If you opt to disable the display of the status window below, you will no longer be able to pause or resume TEXNOLIZE, and the only way to access this configuration screen will be from the link on the main Facebook page or via the Greasemonkey 'User Script Commands' menu. –Rui Fujiwara"],
	...
	GMopt_StatusWinPosition : {
		label : "No. of pixels from bottom of screen to position status window (Default 35 pixels)",
...
GM_config.init("Facebook Hit Older Post v" + version + " Options", GM_configOptions, 
'#GM_config .section_header { font-size: 12pt !important; background: #000000 !important; color: #FFF !important;}' +
...
'#GM_config_header { background-color: #333333 !important; color: #ffffff !important; font-weight: bold !important; font-size: 20px !important; outline: 2px solid #eae9e8 !important; height: 30px !important; text-align: center !important; padding-top: 5px !important; }', {
    open: function() {
...
GM_registerMenuCommand("Facebook Hit Older Post "+version+" Options", GM_config.open);
...
create("a", {className:"item", href:"javascript:void(0);", onclick:main.config}, new Array(
	create("span", {className:"imgWrap"}, new Array(create("img", {src:imgs.icon}))),
	create("span", {textContent:main.gameAcronym+" "+version+" Options"})))));
if(f && !$("navItem_Texnolize_1")) f.parentNode.appendChild(link1);
if(a && !$("navItem_Texnolize_2")) a.appendChild(link2);
...
if (olderPostsClickedCount < maxOlderPostsClicks)  {
  ...
  TexnolizeIntervalId = window.setInterval ( displayOlderPosts, msBetweenClicks );
