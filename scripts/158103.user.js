...
// @include        http://id*.perangkaum.net/*
// ==/UserScript==
...
function FW_Screen_Info_Village() {
	...
	if ( GM_getValue ( 'favorite_villages' ).search ( a_vName ) != -1 ) {
		data += '				Â» Remove village from favorites';
	} else {
		data += '				Â» Add village to favorites';
	}
		...
		if ( event.target.id == 'favorite' ) {
			$( '#' + event.target.id ).fadeOut('slow', function(){
				...
				if ( GM_getValue ( 'favorite_villages' ).search ( a_vName ) != -1 ) {
					...
					$( '#' + event.target.id ).html( "Â» Add village to favorites" ).fadeIn('slow');
				} else {
					GM_setValue ( 'favorite_villages' , GM_getValue ( 'favorite_villages' ) + a_vName + ' (' + a_vCoordX + '|' + a_vCoordY + ');' )
					$( '#' + event.target.id ).html( "Â» Remove village from favorites" ).fadeIn('slow');
				}
...
function FW_Screen_Favorite_Villages() {
	...
	for ( key = 0; key < favorite.length - 1; key++ ) {
		...
		data += '	<td><a href="' + gameURL + vID + '&screen=favorite_villages&mode=remove&id=' + key + '"> Â» Remove</a></td>';
		data += '</tr>';
	}
