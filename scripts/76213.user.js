...
var fixBalanced = function(text, leftStr)
{
    ...
    else { if (/&amp;laquo;$/.test(leftStr)) { index = text.indexOf("&amp;raquo;"); }   // Â«  Â»
    else { if (/&amp;#171;$/.test(leftStr)) { index = text.indexOf("&amp;#187;"); }     // Â«  Â»
    }}}}}}}
    if (index > -1)
    {
        return text.substring(0, index);
...
var textToLink = function(nodeValue)
{
    ...
    while ((matches = nodeValue.substring(fromIndex).match(uriRegExp)) !== null)
    {
        ...
        if (/^([Ã¤Ã¥Ã¦Ã¶Ã¸Ã¼]|\.\w)/i.test(nodeValue.substring(fromIndex, fromIndex + 2)) || unwantedRegExp.test(text))
        {
            continue;
        }
