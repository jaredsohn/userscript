...
menuLinkReplacements = {
	...
	"militaryUnitStatistics.html"	:["Military unit stats","Statystyki jed. wojsk."],
	...
	"partyElections.html"					:["Party elections","Wybory na szefa partii"],
	"congressElections.html"				:["Congress elections","Wybory do Kongresu"],
	"presidentalElections.html"				:["Presidential elections","Wybory na prezydenta"],
...
sideLink2Replacements = {
	...
	"http://wiki.e-sim.org/en/Health"	:["Health tutorial","Poradnik o zdrowiu"],
};
...
function replaceNoAlliesComm(obj) {
	if (obj.innerHTML.match("no allies")) {
		...
		obj.nodeValue=obj.nodeValue.replace(/(no allies)/,"bez sojusznik\u00F3w");
	}
}
...
function replaceBattleInfo(obj) {
	// no allies  
	obj.innerHTML=obj.innerHTML.replace(/(no allies)/g,"Bez sojusznik\u00F3w");
	//12 allies
	if (obj.innerHTML.match("1 ally")) {
	    obj.innerHTML=obj.innerHTML.replace(/(\d*)( allies)/g,"1 sojusznik");
    } else {
        obj.innerHTML=obj.innerHTML.replace(/(\d*)( allies)/g,"$1 sojusznik\u00F3w");
    }
	//Resistance war
	obj.innerHTML=obj.innerHTML.replace(/(Resistance war)/g,"Powstanie");
	//Subsidies:
	obj.innerHTML=obj.innerHTML.replace(/(Subsidies:)/g,"Dofinansowanie:");
}
...
function doCompany() {
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/No companies/,"Brak firm")
    tmp = allElements.children[3]
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/click here/,"kliknij tutaj")
    tmp = allElements.children[4]
...
function doPremium() {
	...
	tmp.innerHTML = tmp.innerHTML.replace(/Single payment/,"Pojedyncza p\u0142atno\u015B\u0107")

        tmp = allElements.children[15].children[11]
	tmp.innerHTML = tmp.innerHTML.replace(/Subscription/,"Subskrypcja")

        tmp = allElements.children[15].children[13]
	tmp.innerHTML = tmp.innerHTML.replace(/Payments will be automatically charged every 30 days (starting today) till you cancel the subscription./,"Płatność będzie automatycznie dokonywana co 30 dni (zaczynając od dziś) dopóki nie odwołasz subskrypcji.")

        tmp = allElements.children[15].children[23]
	tmp.innerHTML = tmp.innerHTML.replace(/Active subscriptions/,"Aktywne subskrypcje")
