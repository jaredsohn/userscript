// ==UserScript==
// @name           Ikariam Aide-Memoire Amended
// @namespace      http://userscripts.org/scripts/show/51116amended
...
// ==/UserScript==
//
// Version 1.0.0 -	Original version based on early versions of Ikariam Aide-Memoire (Generals Score)
//
// -- Resources --
...
function IkariamAideMemoire( options ) {
	...
	this.language 		= {
			"arabic":	{	ltr: false,	ds: ',',	back: '??? ?????',				prev: '?????? ???????',			next: '?????? ???????',			update: '??? ?????',				reset: '????? ???????',				msg: '????? ??? ?? ??? 15 ?????. ?????? ????????' },
			"bulgarian":	{	ltr: true,	ds: ',',	back: '????? ???<br />??????',		prev: '???????? ????????',	next: '???????? ???????? ',	update: '???????? ????????????',	reset: '???????? ????????',		msg: '??????? ???????????? ???? ?????????? 15 ??????. ???? ?????????' },
			"chinese":	{	ltr: true,	ds: ',',	back: '??',				prev: '???',			next: '???',			update: '????',			reset: '???',				msg: '????,???15??????' },
			"czech":	{	ltr: true,	ds: ',',	back: 'Zpet na<br />Island',		prev: 'Predchozí Strana',	next: 'Další Stránka',		update: 'Poslední aktualizace ',	reset: 'Reset Skóre',			msg: 'Skóre aktualizovány behem posledních 15 minut. Prosím, vyckejte' },
			...
			"greek":	{	ltr: true,	ds: ',',	back: '?p?st??f?<br />se ??s?',		prev: '???????µe?? Se??da',	next: '?p?µe?? Se??da',		update: '?e?e?ta?a e??µ???s?',		reset: '?pa?af??? ?a?µ?????e?',		msg: '?p?t??esµa e??µ???s?, ta te?e?ta?a 15 ?ept?. ?a?a?a?? pe??µ??ete' },
			"hebrew":	{	ltr: false,	ds: ',',	back: '???? ??????',				prev: '?? ????',			next: '??? ???',			update: '????? ?????',			reset: '??? ?????',				msg: '???? ????? ????? ?? 15 ????. ??? ????' },
			"hungarian":	{	ltr: true,	ds: ',',	back: 'Vissza a<br />Sziget',		prev: 'Elozo oldal',		next: 'Következo oldal',	update: 'Utolsó frissítés',		reset: 'Reset Scores',			msg: 'Pontszám frissítés az utolsó 15 perc. Kérem, várjon' },
			...
			"korean":	{	ltr: true,	ds: ',',	back: '? ????',			prev: '?? ???',		next: '?? ???',		update: '??? ????',			reset: '??? ??',			msg: '??? ?? 15 ? ??? ??? ??????. ?? ???????' },
			"latvian":	{	ltr: true,	ds: ',',	back: 'Atpakal Uz<br />Island',		prev: 'Previous Page',		next: 'Next Page',		update: 'Last update',			reset: 'Atjaunot Raditajus',		msg: 'Score atjauninata pedejo 15 minušu laika. Ludzu, gaidiet' },
			"lithuanian":	{	ltr: true,	ds: ',',	back: 'Atgal I<br />Sala',		prev: 'Ankstesnis Puslapis',	next: 'Kitas Puslapis',		update: 'Paskutini karta atnaujinta',	reset: 'Zresetuj Natos',		msg: 'Bala atnaujintas per paskutines 15 minuciu. Prašome palaukti' },
			...
			"polish":	{	ltr: true,	ds: ',',	back: 'Powrót<br />na Wyspy',		prev: 'Poprzednia Strona',	next: 'Nastepna Strona',	update: 'Ostatnia aktualizacja',	reset: 'Resetuj Wynikach',		msg: 'Ocena zaktualizowane w ciagu ostatnich 15 minut. Prosze czekac' },
			...
			"romanian":	{	ltr: true,	ds: ',',	back: 'Înapoi Pentru<br />a Island',	prev: 'Previous Page',		next: 'Next Page',		update: 'Ultima actualizare',		reset: 'Resetare Note',			msg: 'Scor actualizat în ultimele 15 de minute. Va rugam sa asteptati' },
			"russian":	{	ltr: true,	ds: ',',	back: '?????????<br />? ?????????',	prev: '?????????? ????????',	next: '????????? ????????',	update: '????????? ??????????',		reset: '????? ?????',			msg: '????? ????????? ?? ????????? 15 ?????. ??????????, ?????????' },
			"serbian":	{	ltr: true,	ds: ',',	back: '????? ??<br />??????',		prev: '???????? ????',		next: '????? ????',		update: '???? ??????',			reset: '????? ??????',			msg: '????? ????????? ? ????????? 15 ??????. ?????? ?????????' },
			"slovak":	{	ltr: true,	ds: ',',	back: 'Spät na<br />Island',		prev: 'Predchádzajúca Strana',	next: 'Dalšia Stránka',		update: 'Posledná aktualizácia',	reset: 'Reset Skóre',			msg: 'Skóre aktualizované pocas posledných 15 minút. Prosím, pockajte' },
			"slovene":	{	ltr: true,	ds: ',',	back: 'Nazaj<br />na Otok',		prev: 'Prejšnja stran',		next: 'Next Page',		update: 'Last update',			reset: 'Ponastavi Indekse',		msg: 'Ocena posodobitev v zadnjih 15 minutah. Prosimo, pocakajte' },
			...
			"turkish":	{	ltr: true,	ds: ',',	back: 'Geri Adasi<br />Için',		prev: 'Önceki Sayfa',		next: 'Sonraki Sayfa',		update: 'Son güncelleme',		reset: 'Sifirla Puanlari',		msg: 'Puan son 15 dakika içinde güncellendi. Lütfen bekleyin.' },
			"ukranian":	{	ltr: true,	ds: ',',	back: '???????????<br />?? ?????????',	prev: '????????? ????????',	next: '???????? ????????',	update: '??????? ?????????',		reset: '???????? ????',			msg: '?????? ???????? ?? ??????? 15 ??????. ???? ?????, ?????????' },
			...
			"vietnamese":	{	ltr: true,	ds: ',',	back: 'Tr? v?<br />Ð?o',			prev: 'Trang tru?c',		next: 'Trang sau',		update: 'L?n c?p nh?t m?i nh?t',		reset: 'Thi?t l?p l?i Ði?m',		msg: 'Ði?m du?c c?p nh?t trong vòng 15 phút. Xin vui lòng ch?' }
		}[this.lang];
	 ...
	 */
	new IkariamUserScriptUpdater( self.options.SCRIPT_ID, self.options.SCRIPT_NAME );
...
IkariamAideMemoire.prototype.processHighscore = function() {
	...
	$("tr.own td:last a").each( function() {
