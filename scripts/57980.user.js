// ==UserScript==
// @name           zynga_syhsieh_20090824
// @namespace      zynga_syhsieh_20090824
// @description    Auto player for zynga games on facebook: mafia wars,  castle age.
...
/* jwa */ 		if(!a) {
/* jwa */ 			questForm=nHtml.FindByAttrContains(tr,"form","id",'_doJob_');
/* jwa */ 			a=nHtml.FindByAttr(questForm,'input','name','Do Quest');
/* jwa */ 		} 
			if(!a) {
...
VisitBankPage:function() {
	...
	var useHref=true;
	if(gameName=="streetracinggame" || gameName=='castle_age' || gameName=='glamour_age' || gameName=='under_world') {
	...
	if(!bankLink) {
		// castle_age
		bankLink=nHtml.FindByAttrContains(content,'a','href','treasury.php');
	}
	if(!bankLink) {
...
VisitFightPage:function(force) {
	...
	this.GMLog('VisitFight'+fightLink.href);
	this.waitMilliSecs=12000;
	if(useHref) {
...
//	var fightForm=nHtml.FindByAttrXPath(document,"form","contains(@action,'fight.php') or contains(@onsubmit,'fight.php') or contains(@action,'Role/Fight')");
	...
	if(fightA) {
		...
		if(onclick) {
			...
			if(fightFightM) {
				var fightFightStr="_fight_fight("+fightFightM[1]+","+userid+",";
	...
	if(img) {
		...
		var onclick=fightA.getAttribute('onclick').toString().replace(battleIdRe,battleIdStr);
...
VisitHospitalPage:function() {
	...
	var gameName=this.GameName();
	if(gameName=="castle_age" || gameName=="glamour_age" || gameName=='under_world') {
		// hospital is in stats page: castle age
	...
	if(!hospitalLink) {
		// gangwars, fashionwars, heroes and villians ...
