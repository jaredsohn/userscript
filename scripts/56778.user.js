// ==UserScript==
// @name           Google Account Multi-Login (ES)
// @namespace      albertttt_
// @description    Remplaza "Salir" en las paginas de Google por un menu para seleccionar otra cuenta. Script "no original". Simplemente está traducido al español de otro script con el mismo nombre pero sin "(ES)"
...
var SUC_script_num = 56778;
var version_timestamp = 1241062573298; // This is here for the previous version of the updater script to find.
try{function updateCheck(forced){if ((forced) || (parseInt(GM_getValue('SUC_last_update', '0')) + 86400000 <= (new Date().getTime()))){try{GM_xmlhttpRequest({method: 'GET',url: 'http://userscripts.org/scripts/source/'+SUC_script_num+'.meta.js?'+new Date().getTime(),headers: {'Cache-Control': 'no-cache'},onload: function(resp){var local_version, remote_version, rt, script_name;rt=resp.responseText;GM_setValue('SUC_last_update', new Date().getTime()+'');remote_version=parseInt(/@uso:version\s*(.*?)\s*$/m.exec(rt)[1]);local_version=parseInt(GM_getValue('SUC_current_version', '-1'));if(local_version!=-1){script_name = (/@name\s*(.*?)\s*$/m.exec(rt))[1];GM_setValue('SUC_target_script_name', script_name);if (remote_version > local_version){if(confirm('Hay una nueva versión!!! "'+script_name+'."\n¿Descargar?')){GM_openInTab('http://userscripts.org/scripts/show/'+SUC_script_num);GM_setValue('SUC_current_version', remote_version);}}else if (forced)alert('No hay actualizaciones disponibles para "'+script_name+'."');}else GM_setValue('SUC_current_version', remote_version+'');}});}catch (err){if (forced)alert('Error al buscar actualizaciones:\n'+err);}}}GM_registerMenuCommand(GM_getValue('SUC_target_script_name', '???') + ' - Comprobar actualizaciones', function(){updateCheck(true);});updateCheck(false);}catch(err){}
...
if (!GM_getValue)
{
	alert('Necesitas tener la última versión de Greasemonkey para poder usar el script...');
	return;
}
...
function onSelectChange()
{
	...
	switch(this.value)
	{
		case 'change_account':
			...
			var pw = passwords[i];
			var d = /.*?@(.*?)$/.exec(un);
			var galx_url='https://www.google.com/accounts/ServiceLoginAuth';
		...
		case 'add_account':
			var u, p;
			if (((u = prompt('Nombre de usuario:')) != null) && ((p = prompt('Contraseña (¡cuidado! ¡no se oculta!):')) != null))
			{
				if ((u.indexOf(',') != -1) || (p.indexOf(',') != -1))
					alert('Los nombres de usuario y contraseñas no pueden contener comas.');
				else
				{
					var a = confirm('¿Entrar automáticamente en esta cuenta?') + '';
		...
		case 'remove_account':
			if (usernames[0] == '')
				alert('No hay cuentas para borrar.');
			else
			{
				u = parseInt(prompt('Pon el número de la cuenta que quieras borrar:' + makeUserList("\n", '', true)));
				if (isNaN(u)) {}
				...
				else
				{
					...
					if (j == -1)
						alert('La cuenta aparecerá cuando recargues la página.');
					else
					{
						un.splice(j, 1);
						pw.splice(j, 1);
...
function getSelectBox()
{
	...
	selectBox.innerHTML=
		'<form name="gmLoginForm" action="https://www.google.com/accounts/ServiceLoginAuth" method="post" style="display:inline;">'+
			'<select style="font-family:arial,san-serif; font-size:7pt; padding:0px; height:16px;" name="gmSelectLogin">'+
				'<option value="">Cambiar de usuario</option>'+
				'<option value="_accounts_begin" disabled="disabled">&#8212;&#8212;</option>'+
				makeUserList('<option value="change_account">', '</option>', false) +
				'<option disabled="disabled">&#8212;&#8212;</option>'+
				'<option value="add_account">Añadir cuenta</option>'+
				'<option value="remove_account">Borrar cuenta</option>'+
				'<option value="sign_out">Salir</option>'+
				'<option disabled="disabled">&#8212;&#8212;</option>'+
				'<option value="check_for_update">Comprob. actualiz.</option>'+
			'</select>'+
