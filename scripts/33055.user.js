...
(function(){
	...
	var getAllSubNodes = function(node) {
		if (node.nodeType == 1) {
		...
		if (node && node.nodeType == 1 && node.hasChildNodes() && !node.tagName.match(/(head|script|style|frameset|frame|iframe)/i)){
			for (var i in node.childNodes)
				getAllSubNodes(node.childNodes[i]);
		}
	};


	var existNodeNN = function() {
		//document.evaluate("//div[@id='googleResult']//div[@id='nn']", document, null, 9, null).singleNodeValue;
		var node = document.getElementById('googleResult');
		if(!node) return false;
		tmpnodes = new Array();
		getAllSubNodes(node);
		for(var i in tmpnodes){
			if(tmpnodes[i].id == 'nn') return true;
		}
		return false;
	};
	var getResultNodes = function() {
		//document.evaluate("//div[@id='googleResult']//div[@class='g']", document, null, 6, null);
		var node = document.getElementById('googleResult');
		if(!node) return false;
		tmpnodes = new Array();
		getAllSubNodes(node);
		...
		var res = [];
		for(var i in tmpnodes){
			if(tmpnodes[i].className == 'g') res.push(tmpnodes[i]);
		}
		return res;
	};
	...
	var getStorageContainer = function() {
		...
		for(var i=0; i<id.childNodes.length; i++){
			if(id.tagName && id.tagName.toLowerCase() == 'div' && id.childNodes[i].id == '') return id.childNodes[i];
		}
		return null;
	};
...
*/
	...
	var getInitNodes = function(){
		//var res = document.evaluate("//div[@id='res']//div[contains(@class,'g') and not(@style)]", document, null, 6, null);
		//equal to 
		//var res = document.evaluate("//div[contains(@class,'g')]", document, null, 6, null);
		//at first search page
		var res = [];
		var elms = document.getElementsByTagName('div');
		for(var i=0; i<elms.length; i++){
...
// init
	...
	for (var i = 0; i < res.length; i++) addCounter(res[i], i+1);
