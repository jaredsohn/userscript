...
const scriptversion = '5.xXx.1';
const scriptversionID = 'Evo+' + scriptversion;
const scriptTag = 'Beta than nothing';
...
function evoAlliances() {
	...
	for(var i = 1; row = rows.snapshotItem(i); i++) {
		row.cells[3].innerHTML = evoNumber2String(row.cells[3].textContent) + '<br /><span style="color: indigo">' + evoNumber2String(Math.ceil(evoString2Number(row.cells[3].textContent)/evoString2Number(row.cells[2].textContent))) + "</span>";
	...
	for( var i = 0; memberTable = memberTables.snapshotItem(i); i++ ) {
		...
		for(var j = 1; j < memberTable.rows.length; j++) {
			...
			if( target < smin ) {
				...
				if( target > tmax ) score.style.color ='indigo';
...
(function () {
  ...
  if( panel != null ) {
	...
	addItemToContent(node, 'X-Rated Links: ', 't_normal b', "<a href=\"http://playevo.com/_/758164-Xrated-Alliance-Forum/\">X-Rated Forum</a>", true);
        addItemToContent(node, '', 't_normal b', "<a href=\"http://www.thejackofclubs.net/evo/irc/\">Java IRC</a>", true);
        addItemToContent(node, '', 't_normal b', "<a href=\"http://playevo.com/fleets/\">Fleets</a>", true);
        addItemToContent(node, '', 't_normal b', "<a href=\"http://playevo.com/auctions/\">Auctions</a>", true);
        addItemToContent(node, '', 't_normal b', "<a href=\"http://playevo.com/resources/overview/\">Resources</a>", true);
	
        contents.parentNode.insertBefore(node, contents);

	node = document.createElement('DIV');
	node.style.color = "lawngreen";
	node.style.fontSize = "9pt"; node.style.fontWeight = "bold";
	node.style.paddingLeft = "53px"; node.style.paddingTop = "9px";
...
function evoUnitsInitialize() {
	...
	if(! dEvo ) {
		...
		units['superstealthwall'] = new evoUnit('superstealthwall' ,UT_NONE    ,21  ,15000  ,6000   ,0   ,0   ,0   ,0  ,0  ,false);
		...
		units['spy']              = new evoUnit('spy'              ,UT_NONE    ,24  ,1000000  ,500000  ,0   ,0   ,0   ,0  ,0  ,false);
		units['guerrilla ']        = new evoUnit('Guerrilla '        ,UT_NONE    ,24  ,3000000  ,4500000  ,0   ,0   ,0   ,0  ,0  ,false);
	} else {
		// DEVO Stats
