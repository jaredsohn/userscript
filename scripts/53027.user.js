...
// @description    Manage all your RapidShare accounts in one place
// @version        1.0
...
if (/checkfiles\.html\?manage/.test(document.location)) {
	...
	if (saved_data != '') {
		...
		$.each(saved_accounts, function(i, account) {
			...
			$('#accounts').append('<tr style="text-align: center" id="r_' + i + '"><td><input type="text" id="eu_' + i + '" value="' + account.split(':')[0] + '" /></td><td><input type="password" id="ep_' + i + '" value="' + account.split(':')[1] + '"></td><td><input type="checkbox" id="pr_' + i + '" ' + checked + ' /></td><td id="e_' + i + '">Checking...</td><td id="p_' + i + '">Checking...</td><td id="k_' + i + '">Checking...</td><td id="t_' + i + '">Checking...</td><td id="f_' + i + '">Checking...</td><td><a id="d_' + i + '" href="#" onclick="javascript:return false">Delete</a></td></tr>');
			if (account.split(':')[2] == 'true') {
				...
				GM_xmlhttpRequest({
					...
					onload: function(r) {
						if (/klappbox/.test(r.responseText)) { // valid
							$('#e_' + i).html(r.responseText.match(/<td>Expiration date:<\/td><td style="padding-right:20px;"><b>[A-Za-z]{3}, ([^<]*)<\/b>/)[1]);
							$('#p_' + i).html(r.responseText.match(/<td>Free RapidPoints:<\/td><td align=right style="padding-right:20px;"><b><span id="rpo">([^<]*)<\/span><\/b>/)[1]);
							$('#k_' + i).html(r.responseText.match(/<td>Premium RapidPoints:<\/td><td align=right style="padding-right:20px;"><b><span id="rppo">([^<]*)<\/span><\/b>/)[1]);
				...
				GM_xmlhttpRequest({
					...
					onload: function(r) {
						if (/klapptitel/.test(r.responseText)) { // valid
							$('#e_' + i).html('N/A');
							$('#p_' + i).html(r.responseText.match(/<td>Free RapidPoints:<\/td><td align=right style="padding-right:30px;"><b><span id="rpo">([^<]*)<\/span><\/b>/)[1]);							
							$('#k_' + i).html(r.responseText.match(/<td>Premium RapidPoints:<\/td><td align=right style="padding-right:30px;"><b><span id="rppo">([^<]*)<\/span><\/b>/)[1]);
							$('#t_' + i).html('N/A');
