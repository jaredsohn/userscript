// ==UserScript==
// @name           OGame Modified Resource Panel Alpha
// @author         Emilio, J.A. y Shuko (modified by Cotonite)
// @date           18-12-2008
// @version        1.0
// @namespace      OGame Modified Resource Panel Alpha
...
//		Cookie recursos:  contiene el valor de los recursos metal, crystal y deuterium, su produccion por hora.
...
var Crystal = T_Recursos2[11].innerHTML.replace(/\./g,'');
var Deut = T_Recursos2[12].innerHTML.replace(/\./g,'');

var PMetal,PCrystal,PDeut;
var AlmM,AlmC,AlmD;
var T_Factor;
...
if( location.href.indexOf('resources') != -1 ){
	...
	PCrystal = T_Recursos[11].innerHTML.replace(/\./g,'');
	...
	if (PCrystal.indexOf('<font color')!=-1) {
		PCrystal = PCrystal.substring(57, PCrystal.indexOf('</font'));
	}
...
if( location.href.indexOf('resources') != -1 ){
	...
	if(GetCookie("planetas") == null){
		// comprobamos que no sea una luna, metal y deuterium a cero
	...
	for (var t=1; t<planetas.length; t++){
		if(planetas[t]==planeta[1]){
			// comprobamos que no sea una luna , metal y crystal a cero
			if(recurso[1]!="0" && recurso[2]!="0"){
...
if (ResForm) {
	...
	CuentaRec.innerHTML = '<br /><table width="550">'+'<tr><td class="c" colspan="4">Extended production</td></tr>'+'<tr>'+'<td class="c">&nbsp;</td>'+'<th>Daily</th>'+'<th>Weekly</th>'+'<th>Monthly</th>'+'</tr>'+'<tr>'+'<td class="c" width="110"><img src="http://uni43.ogame.com.es/evolution/images/metall.gif" width="24" style="margin-right:15px;" /><font color="#F1531E" style="margin-right:15px;font-style:italic;">Metal</font></td>'+'<th><font color="#00ff00">'+formatNmb(XMetal[0])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XMetal[1])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XMetal[2])+'</font></th>'+'</tr>'+'<tr>'+'<td class="c" width="110"><img src="http://uni43.ogame.com.es/evolution/images/kristall.gif" width="24" style="margin-right:15px;" /><font color="#54B0DC" style="margin-right:15px;font-style:italic;">Crystal</font></td>'+'<th><font color="#00ff00">'+formatNmb(XCrystal[0])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XCrystal[1])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XCrystal[2])+'</font></th>'+'</tr>'+'<tr>'+'<td class="c" width="110"><img src="http://uni43.ogame.com.es/evolution/images/deuterium.gif" width="24" style="margin-right:15px;" /><font color="#9AACCB" style="margin-right:15px;font-style:italic;">Deut</font></td>'+'<th><font color="#00ff00">'+formatNmb(XDeut[0])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XDeut[1])+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(XDeut[2])+'</font></th>'+'</tr>'+'</table><br />';
	...
	if ((LlenoC)||(PCrystal==0)) segundosC=0
	else segundosC=(AlmC-Crystal)/CcadaSeg;
...
'<tr><td class="c" colspan="3">State of Storages</td></tr>'+'<tr>'+'<th id="alm_M">Metal ('+EAlmM+'%)</th>'+
'<th>'+(tiemMd != 0 ? tiemMd+'d': '')+' '+(tiemMh != 0 ? tiemMh+'h': '')+' '+(tiemMm != 0 ? tiemMm+'m': '')+' '+(tiemMs != 0 ? tiemMs+'s': '')+tiemMetal+' </th>'+
...
'</tr>'+
'<tr>'+
'<th id="alm_C">Crystal ('+EAlmC+'%)</th>'+
'<th>'+(tiemCd != 0 ? tiemCd+'d': '')+' '+(tiemCh != 0 ? tiemCh+'h': '')+' '+(tiemCm != 0 ? tiemCm+'m': '')+' '+(tiemCs != 0 ? tiemCs+'s': '')+tiemCrystal+' </th>'+
...
'</table><br />';
	...
	for (var i=1; i<planetas_recursos.length; i++){
		...
		if(i%3 == 2){
		planetas_recursos_crystal=planetas_recursos_crystal+parseInt(planetas_recursos[i]);
		}
		if(i%3 == 0){
	...
	CuentaRec.innerHTML += '<table width="550">'+'<tr><td class="c" colspan="4">Total production of '+(planetas_format.length-1)+' planets</td></tr><tr><td class="c" width="110" style="text-align:center;">Planets</td><td class="c" colspan="3">'+planetas_format_final+'</td></tr>'+'<tr>'+'<td class="c">&nbsp;</td>'+'<th>Daily</th>'+'<th>Weekly</th>'+'<th>Monthly</th>'+'</tr>'+'<tr>'+'<td class="c"><img src="http://uni43.ogame.com.es/evolution/images/metall.gif" width="24" style="margin-right:15px;" /><font color="#F1531E" style="margin-right:15px;font-style:italic;">Metal</font></td>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_metal*24)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_metal*168)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_metal*720)+'</font></th>'+'</tr>'+'<tr>'+'<td class="c"><img src="http://uni43.ogame.com.es/evolution/images/kristall.gif" width="24" style="margin-right:15px;" /><font color="#54B0DC" style="margin-right:15px;font-style:italic;">Crystal</font></td>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_crystal*24)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_crystal*168)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_crystal*720)+'</font></th>'+'</tr>'+'<tr>'+'<td class="c"><img src="http://uni43.ogame.com.es/evolution/images/deuterium.gif" width="24" style="margin-right:15px;" /><font color="#9AACCB" style="margin-right:15px;font-style:italic;">Deuterium</td>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_deuterium*24)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_deuterium*168)+'</font></th>'+'<th><font color="#00ff00">'+formatNmb(planetas_recursos_deuterium*720)+'</font></th>'+'</tr>'+'<tr>'+'<td class="c" style="text-align:center;"><font>Total</td>'+'<th><font color="#00ff00">'+formatNmb((planetas_recursos_metal*24)+(planetas_recursos_crystal*24)+(planetas_recursos_deuterium*24))+'</font></th>'+'<th><font color="#00ff00">'+formatNmb((planetas_recursos_metal*168)+(planetas_recursos_crystal*168)+(planetas_recursos_deuterium*168))+'</font></th>'+'<th><font color="#00ff00">'+formatNmb((planetas_recursos_metal*720)+(planetas_recursos_crystal*720)+(planetas_recursos_deuterium*720))+'</font></th>'+'</tr></table><br />';
	...
	for (var i = 0; i <= (EAlmCPorc > 250 ? 250 : EAlmCPorc); i++) {
		...
		setTimeout('document.getElementById("alm_C").innerHTML ="Crystal - '+Math.round(((100*i)/250))+' %"', 20*(i+2));
	}
	setTimeout('document.getElementById("alm_C").innerHTML ="Crystal - '+EAlmC+'%"', (EAlmCPorc > 250 ? 250 : EAlmCPorc)*20*(i+2));
...
if(GetCookie("planetas") == null){
	...
	for (var i=1; i<planetas_format.length; i++){
		...
		PCrystal = parseInt(planetas_recursos[a+2]);
...
if(PMetal!="0" && PCrystal!="0" && Energia!="<font>0</font>/0"){ // para que sea distinto de una luna

/* Producción por segundo para recursos "live" */
SMetal = PMetal/3600;
SCrystal = PCrystal/3600;
SDeut = PDeut/3600;
...
var secs = (endtime-startime)/1000;
	...
	if(Crystal<AlmC) {
		Cposeido = roundNumber((SCrystal*secs)+Crystal);
