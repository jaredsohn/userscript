// ==UserScript==
// @name           Travian Resource Bar Plus (ItravTool)
// @namespace      http://userscripts.org/scripts/show/472174
// @OriginalScript http://userscripts.org/scripts/show/75431
// @description    Shows travian resources, some enhancement and buildings shortcut
// @author         Fox, Serj_LV, 
...
var namespace = 'http://userscripts.org/scripts/show/472174';
...
var langs = ['auto','English (en)','???? (ar)','????????? (bg)','Bosanski (bs)','Deutsch (de)','???????? (el)','Español (es)','????? (fa)','Français (fr)','Hrvatski (hr)','Magyar (hu)','Italiano (it)','Nederlands (nl)','Polski (pl)','Português (pt)','Româna (ro)','??????? (ru)','?????? (sr)','Svenska (sv)','Türkçe (tr)','?????????? (ua)','Ti?ng Vi?t (vi)','?? (zh)'];
...
DICT = {
	...
	hu: { // Hungarian language. thx mrzed :)
		...
		resbar : "Nyersanyagkijelzo",
		showres : "Nyersanyagkijelzo külön ablakban",
		redbl : "piros (órában)",
		yellowbl: "sárga (órában)",
		marketpl : "Piactér",
		mfilter : "szuro",
		mfiltero : ['nincs','csak arány','csak szuro','teljes'],
		rpandmp : "Gyülekezotér és piactér",
		incomres : "Információ az érkezo nyersanyagokról",
 ...
 ar: { // Arabic, thx ww_start_t
	...
	hr: { // Croatian translation, thx semiRocket
		...
		consnegat : "Potrošnja hrane u naselju je negativna. Koliko minuta želite pricuve?",
		searchlast : "Pretraži za posljednji",
		bmove : "Premještanje gradevina",
		neighbors : "susjedi",
		// settings
		none : 'Ništa',
		auto : "automatsko",
		...
		crannyh : 'Normalno 80, 70 za klasicne pljacke ili artefakt',
		builtin : "Ugradeni alati",
		...
		normal : "Normalno",
		banalyze : "Ugradeni analizator bitke",
		cropfind : "Ugradena tražilica žitnih (crop) polja",
		adetect : "Detektiranje napada",
		adetecto : ['Iskljuceno','Bez zamrzavanja','Ukljuceno'],
		adetecth : "Radi skrivene zahtjeve prema poslužitelju kršeci pravila. Može dovesti do kazni.",
		adetectt : "Razdoblje detektiranja napada",
		buildhint : "Savjeti za gradnju",
		onallp : "Sve stranice",
		dontwrap : "Onemoguci prijelom stranica",
		...
		buildands : ['Iskljuceno','Ukljuceno','Prostrano'],
		...
		addvtableo : ['Iskljuceno','Ukljuceno','Zalijepljeno'],
		addvtableh : '(Travian 4)',
		opennote : "Automatski otvori zabilješke",
		notesize : "Velicina prozora za zabilješke",
		...
		marketpl : "Tržnica",
		mfilter : "Filter",
		mfiltero : ['Iskljuceno','Samo omjer','Samo filter','Potpuno'],
		npcsum : "Sažetak za NPC",
		npcsumh : 'Prikaz u tržnici i zgradama',
		bidinc : "Povecanje ponude",
		bidinch : 'Koristi se za aukcije (Travian 4)',
		show3x : "Predvidanje kretanja kod 2x i 3x",
		...
		incomres : "Informacije o dolazecim resursima",
		incomreso : ['Iskljuceno','Ukljuceno','S kratkim pregledom','WW mod'],
		troopsI : "Informacije o trupama",
		troopsIo : ['Iskljuceno','Ukljuceno','Skeniraj upute'],
		...
		showlso : ['Iskljuceno','Ukljuceno','U zasebnom prozoru'],
		savedls : "Spremljene poveznice",
		savedd : "Pohranjeni podaci",
		saveddh : 'Ukljucuje linkove i druge nazive naselja. Korisno ako se racun briše ili ne koristi na Vašem racunalu.',
		savedelall : "Izbriši sve pohranjene podatke",
		savedelallh : 'Jeste li sigurni da želite izbrisati sve pohranjene podatke, ukljucujuci linkove i druge nazive naselja?',
	...
	pl: { // Polish translation, thx aren
		...
		info : "Informacja",
		yourrace : "Twoja rasa",
		speedart : "Predkosc artefaktu",
		racelist : ['Rzymianie','Germanie','Galowie'],
		cranny : "zólty poziom kryjówki (procent)",
		crannyh : 'normalny 80, 70 do grabiezy na klasycznym lub artefakcie',
		builtin : "Wbudowane narzedzia",
		builtinh : 'dla normalizacji produkcji: kolor czerwony - najwiecej srodków potrzeba, zielony - potrzeba mniej zasobów',
		...
		dontwraph : 'Jezeli lista osad przeskocz w dól',
		buildand : "Pokaz odliczanie budynku i ataku",
		buildandh : 'Na liscie osad',
		sendres : "Pokaz &laquo;wyslij surowce/jednostki&raquo; ikony",
		sendmess : "Pokaz &laquo;wyslij wiadomosc&raquo; ikony",
		analyzer : "World analyzer",
		bigicon : "Pokaz DUZE ikony",
		...
		incomres : "informacja o przychodzacych surowcach",
		incomreso : ['off','on','z podsumowaniem','WW mode'],
		troopsI : "Informacja o jednostkach",
		troopsIo : ['off','on','scan help'],
		defRP : "domyslna akcja dla Miejsca zbiórki",
		showls : "Pokaz linki",
		showlso : ['off','on','w oknie'],
		savedls : "zapisane linki",
		savedd : "zapisane dane",
		saveddh : 'w tym Linki i Druga nazwa. Jesli konto usuniete, albo nie twój komputer.',
		savedelall : "usunac wszystkie zapisane dane",
		savedelallh : 'Czy na pewno chcesz usunac wszystkie dane, w tym linki i druga nazwe?',
		scrlang : "Jezyk skryptu",
		youlang : "twój jezyk",
		notifi : "powiadomienie",
		notification : "powiadomienia po zakonczeniu budowy",
		method : "metoda",
	...
	tr: { // Turkish language. xpugur
		...
		yourrace : "Irkin",
		speedart : "Hiz eseri",
		racelist : ['Romanlilar','Cermenler','Galyalilar'],
		cranny : "siginak sari seviyesi (yüzde)",
		crannyh : 'normal 80, klasik veya eser yagmasi için 70 ',
		builtin : "Entegre araçlar",
		builtinh : 'Üretimi normallestirmek için: kirmizi renk - en gerekli kaynak, yesil - en az gerekli kaynak',
		normalize : 'Üretimi normallestirmek için',
		normal : "normal",
		banalyze : "Dahili savas analizcisi",
		...
		buildhint : "insaat ipuçlari",
		onallp : "Tüm sayfalar",
		dontwrap : "Sayfalari birlestirme",
		dontwraph : 'köy listeleri sayfa atlarsa',
		buildand : "insaat gerisayimini ve saldirilari göster",
		buildandh : 'Köy listelerinde',
		sendres : "isaretleri göster: ?send resource/troops? ",
		sendmess : "isaretleri göster: ?send message? ",
		...
		openoview : "Otomatik olarak köylerin genel görüsünü aç",
		...
		show3x : "yanlis veri gösterebilir",
		show3xh : '3x ve 2x ile gönderilen kaynak akisini tahmin etmeye çalis',
		...
		savedls : "kaydedilmis baglantilar",
		savedd : "kaydedilmis veriler",
		saveddh : 'baglantilar ve ikinci ismi içeren. Hesap silindiyse, yada kendi bilgisayariniz degilse.',
		savedelall : "kaydedilmis tüm verileri sil",
		...
		notification : "insaattan sonra bilgilendirmeler",
		method : "method",
	...
	vi: { // VietNamese translation by Nguy?n Duy Thanh
		...
		consnegat : "M?c tiêu th? âm. C?n bao nhiêu phút d? d? tr??",
		...
		racelist : ['Romans','Teutons','Gauls'],
		cranny : "Báo màu vàng khi ?ng tài nguyên d?t (%)",
		crannyh : 'thông thu?ng là 80, 70 cho làng bình thu?ng ho?c làng có Bình ru?u c?a Dionysus',
		builtin : "Công c? công trình",
		builtinh : 'Chu?n hoá s?n lu?ng tài nguyên: màu d? -lo?i tài nguyên c?n nh?t, màu xanh - lo?i tài nguyên ít c?n',
		normalize : 'Chu?n hoá s?n lu?ng tài nguyên',
		normal : "chu?n",
		banalyze : "Ch?c nang - phân tích tr?n chi?n",
		cropfind : "Ch?c nang - tìm làng nhi?u ru?ng lúa",
		adetect : "Phát hi?n t?n công",
		adetecto : ['t?t','w/o m?c d?nh','m?'],
		adetecth : "?n các hành vi có th? d?n d?n hình phát",
		adetectt : "Th?i gian phát hi?n t?n công",
		buildhint : "M?o xây d?ng",
		onallp : "T?t c? trang",
		dontwrap : "Ð?ng ng?t trang",
		dontwraph : 'n?u danh sách trang quá dài',
		buildand : "Hi?n th? th?i gian xây d?ng và t?n công",
		buildandh : 'Trong danh sách làng',
		sendres : "Hi?n bi?u tu?ng «G?i tài nguyên/G?i lính»",
		sendmess : "Hi?n bi?u tu?ng <<g?i>>",
		analyzer : "Phân tích th? gi?i",
		bigicon : "Hi?n bi?u tu?ng BIG",
		addvtable : "Hi?n th? danh sách làng b? sung",
		addvtableo : ['t?t','m?','dính'],
		addvtableh : 'Cho travian 4',
		opennote : "T? d?ng m? ghi chú",
		notesize : "Kích thu?c c?a s? ghi chú",
		openoview : "T? d?ng m? t?ng quát làng",
		...
		marketpl : "Khu ch?",
		mfilter : "B? l?c",
		mfiltero : ['t?t','ch? t? l?','ch? b? l?c','d?y d?'],
		npcsum : "Tóm t?t cho NPC",
		npcsumh : 'ch? và công trình',
		bidinc : "Tang d?u giá",
		bidinch : 'd?u giá c?a travian4',
		show3x : "Có th? hi?n th? d? l?u không chính xác",
		show3xh : 'c? g?n d? doán các lu?n tài nguyên sever 2x và 3x dang g?i',
		rpandmp : "Binh tru?ng và ch?",
		incomres : "Thông tin tài nguyên dang d?n",
		incomreso : ['t?t','m?','v?i tóm t?t','ch? d? WW'],
		troopsI : "Thông tin v? quân d?i",
		troopsIo : ['t?t','m?','tìm tr? giúp'],
		defRP : "Hành d?ng m?t d?nh khi g?i lính",
		showls : "Hi?n liên k?t",
		showAsSN : "S? d?ng liên k?t nhu tên ph?",
		showlso : ['t?t','m?','trong c?a s?'],
		savedls : "Luu liên k?t",
		savedd : "Liên k?t dã luu",
		saveddh : 'g?m liên k?t và liên k?t ph?. N?u tài kho?ng xoá ho?c dây không ph?i máy tính c?a b?n.',
		savedelall : "xoá t?t c? d? li?u dã luu",
		...
		audiotest : "Th? nh?c",
		neighbors : "Tìm ki?m",
		colorCustomize : "Cài d?t màu s?c",
		colorHint : "Ð? tr?ng d? cho màu m?c d?nh",
		color0 : " S?n sàn nân c?p ",
		color1 : "Nân c?p thông qua NPC",
		color2 : "Không th? nân c?p<br>(do không d? tài nguyên)",
		color3 : "không th? nân c?p<br>(do nhà kho/ kho lúa không d? s?c ch?a)",
	...
	el: {// Greek translation by Tasos el malo
		...
		duration : "d????e?a",
		deficit : "???e?µµa",
		aclock : "????? s??a?e?µ??\nhh:mm:ss , hh:mm , mm (ap? t??a)",
		consnegat : "? ?ata????s? st? ????? e??a? a???t???. p?sa ?ept? ??e???eta? ap??eµa?",
		searchlast : "a?a??t?se t? te?e?ta??",
		bmove : "µeta????se ?t???a",
		neighbors : "?e?t??e?",
		// settings
		none : "?a???a",
		...
		info : "p????f???e?",
		yourrace : "? f??? s??",
		speedart : "ta??t?ta a?t??e?µ????",
		racelist : ['??µa???','?e?t??e?','Ga??te?'],
		cranny : "??t???? ep?ped? ??????a? (p?s?st?)",
		crannyh : '?a?????? 80, 70 ??a ??as??? ? a?t??e?µe?? ?e??as?a?',
		builtin : "??s?µat?µ??a e??a?e?a",
		builtinh : '??a ?µa??p???s? pa?a?????: ??????? ???µa - µe?a??te?? a????? p????, p??s??? - ????te?? a????? p????',
		normalize : '?µa??p???s? pa?a?????',
		normal : "?a??????",
		banalyze : "e?s?µat?µ???? a?a??t?? µ????",
		cropfind : "e?s?µat?µ??? e??es? p???s?ta???",
		adetect : "e?t?p?sµ?? ep??es??",
		adetecto : ['a?e?e???','????? a????t?p???s?','e?e???'],
		adetecth : "???e? ???f? a?t?µata st?? e??p??et?t?, pa?aß?????ta? t??? ?a???e?. µp??e? ?a ?d???se? se t?µ???a.",
		adetectt : "pe???d?? e?t?p?sµ?? ep??es??",
		buildhint : "s?µß???? ?atas?e???",
		onallp : "??e? ?? se??de?",
		dontwrap : "??? a?ad?p??se?? se??de? (a???)",
		dontwraph : '??? ? ??sta pa?a??µpte? ta ????? p??? ta ??t?',
		buildand : "eµf???s? a?t?st??f?? µ?t??s?? ?t????? ?a? ep??es??",
		buildandh : 'st?? ??sta t?? ??????',
		sendres : "eµf???se «ste??e ??e?/st?ate?µata» e?????d?a",
		sendmess : "eµf???se «ste??e µ???µa» e?????d??",
		analyzer : "World analyzer",
		bigicon : "eµf???se µe???a e?????d?a",
		addvtable : "?µf???s? ep?p???? p??a?a ??????",
		addvtableo : ['a?e?e???','e?e???','?????µ???'],
		addvtableh : '??a travian4',
		opennote : "?????e a?t?µata s?µe??µat????",
		notesize : "µ??e??? pa??????? s?µe??µat?????",
		openoview : "?????e a?t?µata t?? eµf???s? ??????",
		resbar : "µp??a p????",
		showres : "eµf???se t?? µp??a p???? se pa??????",
		redbl : "??????? (se ??e?)",
		yellowbl: "??t???? (se ??e?)",
		marketpl : "?????",
		mfilter : "F??t??",
		mfiltero : ['a?e?e???','µ??? p?s?st?','µ??? f??t??','p???e?'],
		npcsum : "s????? ??a NPC",
		npcsumh : 'st?? a???? ?a? sta ?t???a',
		bidinc : "a??ßase p??sf???",
		bidinch : '??a pe??p?te?e? travian4',
		show3x : "µp??e? ?a eµfa???e? ?a??asµ??a ded?µ??a",
		show3xh : 'p??ß?e?? ???? t?? p???? se 3x ?a? 2x ap?st???',
		rpandmp : "p?ate?a s?????t??s?? ?a? a????",
		incomres : "p????f???e? e?se???µe??? p????",
		incomreso : ['a?e?e???','e?e???','µe s?????','??a ?a?µa'],
		troopsI : "p????f???e? ??a ta st?ate?µata",
		troopsIo : ['a?e?e???','e?e???','ß???e?a s???s??'],
		defRP : "p??ep??e?µ??? e????e?a ??a t?? p?ate?a s????e?s??",
		showls : "eµf???se s??d?sµ???",
		showAsSN : "???s?µ?p???se s??d?sµ??? ?? de?te?? ???µa",
		showlso : ['a?e?e???','e?e???','se pa??????'],
		savedls : "ap????e?µ???? s??desµ??",
		savedd : "ap????e?µ??a ded?µ??a",
		saveddh : 's?µpe???aµßa??µ???? t?? s??d?sµ?? ?a? t?? de?te??? ???µat??. ?? ??a? ???a??asµ?? d?a??afe?, ? ??? ? ?p?????st?? sa?.',
		savedelall : "d?a??af? ???? t?? ap????e?µ???? ded?µ????",
		savedelallh : '??ste s??????? ?t? ???ete ?a d?a????ete ??a ta ded?µ??a, s?µpe???aµßa??µ???? t?? s??d?sµ?? ?a? t? de?te?? ???µa?',
		scrlang : "???ssa Script",
		youlang : "? ???ssa s??",
		notifi : "e?d?p???s?",
		notification : "e?d?p???s? µet? t?? ?atas?e??",
		method : "µ???d??",
		audiourl : "URL ??a a??e?? ????",
		audiotest : "d???µ? ????",
		colorCustomize : "ep?????? ???µ?t??",
		colorHint : "?f?se ?e?? ??a p??ep??e?µ??? ???µa",
		color0 : "d?a??s?µ? a?aß??µ?s?",
		color1 : "a?aß??µ?s? µes? NPC",
		color2 : "a?aß??µ?s? de? e??a? d??at? <br/>(??? a??et?? p????)",
		color3 : "a?aß??µ?s? de? e??a? d??at? <br/>(??? a??et? ????t???t?ta ap??????/s?tap??????)",
		color4 : "a??tat? ep?ped?"
	},
	nl: {
		// ingame messages
...
function marketSend () {
	...
	moC.appendChild($em('SPAN',[maxRM,' S=',maxRC],[['style','margin:0px 5px;font-size:12px;']]));
...
function vlist_addButtonsT4n () {
	...
	if( RB.Setup[21] > 0 ) {
		...
		vilT.appendChild($ee('THEAD',$em('TR',[$c(hideP),$c($gc('boxTitle',$g('sidebarBoxVillagelist'))[0].innerHTML.onlyText(),[['colspan',3],['style','font-weight:bold;']]),$c('('+villages_count+')')])));
		for( var i=0; i<villages.length; i++) {
			vLink[i] = $a($gt("div",villages[i])[0].innerHTML,[['href',linkVSwitch[i]]]);
			var cl = villages_id[i]==village_aid?"dot hl":"dot";
			//vilB.appendChild($em('TR',[$c('<a href="dorf1.php'+linkVSwitch[i]+'"><strong style="margin-right: 5px; color: blue; font-size: 14pt;">&#171;</strong></a>&#x25CF;',[['class',cl]]),$c($ee('DIV',vLink[i])),$c($a(printCoords(villages_id[i]),[['href',linkVSwitch[i]]]),[['style','direction:'+coordDir+';']]),$c(addARLinks(villages_id[i],0))]));
			var linkDorf1Baru = $a('&#171;',[['href', 'dorf1.php'+linkVSwitch[i].slice(0,-1)], ['style', 'color: blue; font-size: 14pt; margin-right: 2px;'], ['title', 'Go outside Village']]);
			var linkDorf2Baru = $a('&#187;',[['href', 'dorf2.php'+linkVSwitch[i].slice(0,-1)], ['style', 'color: blue; font-size: 14pt;'], ['title', 'Go inside Village']]);
			//var kolomDorf1Baru = $c(linkDorf1Baru);
			//var kolomDorf2Baru = $c(linkDorf2Baru);
			var kolomBaru = $c($em('SPAN', [linkDorf1Baru, linkDorf2Baru]));
			vilB.appendChild($em('TR',[kolomBaru, $c('&#x25CF;',[['class',cl]]),$c($ee('DIV',vLink[i])),$c($a(printCoords(villages_id[i]),[['href',linkVSwitch[i]]]),[['style','direction:'+coordDir+';']]),$c(addARLinks(villages_id[i],0))]));
		}
		vilT.appendChild(vilB);
		if( RB.Setup[21] == 1 ) makeFloatD(vilT,7);
		if( RB.Setup[21] == 2 ) {
...
function userActivityServers ( num, id, user ) {
	...
	{ // @ by TG @ April, 20, 2014
		sd = crtName.substr(crtName.lastIndexOf('.')+1, crtName.length);
		ss = dsrv.slice(0,-1)+crtName.split('.')[0];
		return ['itravtools','http://itravtools.com/travian-analyser/travian-history-profile/?sd='+sd+'&ss='+ss+'&'+(user?'u':'a')+'id='+id,'http://itravtools.com/travian-analyser/travian-history-profile/?sd='+sd+'&ss=###'+ss+'###&uid=...'];
  } else 
	{
...
function showDorf1 () {
	...
	for( var i = 0; i < villages_id.length; i++ ) {
		...
		// @ by TG @ April, 20, 2014
		var newdid = linkVSwitch[i].match(/newdid=\d+/i)[0];
		var mapId = villages_id[i];
		var newc = $ee('SPAN', '<a href="build.php?'+newdid+'&z='+mapId+'&gid=22" style="margin: 0px 5px; font-weight: bold; color: blue;" title="Academy">A</a>');
    newTD.appendChild(newc);		

		var newc = $ee('SPAN', '<a href="build.php?'+newdid+'&z='+mapId+'&gid=24" style="margin-right: 5px; font-weight: bold; color: DarkOrange;" title="Town Hall">TH</a>');
    newTD.appendChild(newc);		

		var newc = $ee('SPAN', '<a href="build.php?'+newdid+'&z='+mapId+'&gid=17&t=2" style="margin-right: 5px; font-weight: bold; color: Red;" title="Market Sell">M</a>');
    newTD.appendChild(newc);		
