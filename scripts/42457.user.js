...
*/
// 
var pluginMime = GM_getValue("pluginMime", "application/x-totem-plugin");

//// Linux fix
var fdiv = $('watch-player-div');

if(fdiv){
    var vid_url = "http://"+document.domain+"/get_video?";
    var vars = makeFlashVars();
    removeAllChildren(fdiv);
    //Allow automagic to do its voodoo before we start to play anything.
    var n = makePlayer('' /*vid_url + vars*/,475,385);
    n.setAttribute("flashvars",vars);
    fdiv.appendChild(n);
}

//comment out the original reloadPlayer
function reloadPlayer() {
	if (isVideoIcon()) return;
	player.src = vid_url + player.getAttribute("flashvars");
...
function makeFlashVars(){
    ...
    if(swf_args)
    {
        ['video_id', 't' /*, 'vq', 'fmt_map'*/].forEach(function(v) {
            res += v + "=" + getArg(swf_args, v) + "&";
        });
        if(window.location.href.match(/fmt=/))
            res += "fmt="+window.location.href.match(/fmt=([^&]*)/)[1];
        //else
        //    res += "fmt=" + getArg(swf_args, 'fmt', 18);
    }
    return res;
}

function getFlashVar2(varName,flashVars,def) {
	// Searchs for the varName in the flashvars
	var queryRE = new RegExp('"' + varName + '":\\s+"([^"]*)');
	var queryRet = queryRE.exec(flashVars);

	// Returns the corresponding value or null (if not found)
	return (queryRet === null) ? (def?varName+'='+def:'') : varName+"="+queryRet[1];
}

function makePlayer(url,w,h){
    var el = document.createElement("EMBED");
    with(el){
        id = "movie_player"; src = url;
        width = w; height = h;
        type = pluginMime; //"application/x-totem-plugin"; 
...
function scriptShowUpdateMessage(scriptShowMessage, scriptNewVersion) {
	...
	if (scriptShowMessage == false) {
		...
		if (messageDiv) {
			...
			// Creates the notice
			messageDiv = createNode("div", {id: "gsscriptVersionMessage", title: "A new MYousableTubeFix version is available"});
			messageDiv.innerHTML = "A new version of MYousableTubeFix (" + scriptNewVersionDate + ") is available<br><br>" +
				"<a id='gsscriptVersionMessageInstall' href='" + scriptFileURL + "' title='Install the script update'>Install</a>" +
				"<a href='" + scriptHomepageURL + "' target='_blank' title='Go to MYousableTubeFix homepage'>Go to web page</a>" +
...
var scriptVersion = 1258888529567; // 22 Nov 2009
...
var scriptFileURL = "http://userscripts.org/scripts/source/42457.user.js";
var scriptHomepageURL = "http://userscripts.org/scripts/show/42457";
...
function configureScript(e) {
	...
	function createLayers() {
		...
		dialogLayer.innerHTML = "<div id='gsconfTitle'>MYousableTubeFix Configuration</div>" +
			"<ul>" +
            ...
            "<li>" +
			"Select plugin: &nbsp;" +
			"<select id='gsconfpluginMimeSel' size='1'>" +
			"<option id='gsconfpluginMimeTotem' value='application/x-totem-plugin'" + ((pluginMime === "application/x-totem-plugin") ? " selected='selected'" : "") + ">Totem</option>" +
			"<option id='gsconfpluginMimeMplayer' value='application/x-mplayer2'" + ((pluginMime === "application/x-mplayer2") ? " selected='selected'" : "") + ">Totem / Mplayer</option>" +
            "<option id='gsconfpluginMimeQuicktime' value='application/x-quicktimeplayer'" + ((pluginMime === "application/x-quicktimeplayer") ? " selected='selected'" : "") + ">Mplayer / Quicktime</option>" +
            "<option id='gsconfpluginMimeVlc' value='application/x-vlc-plugin'" + ((pluginMime === "application/x-vlc-plugin") ? " selected='selected'" : "") + ">VLC</option>" +
            "<option id='gsconfpluginMimeXine' value='application/x-xine-plugin'" + ((pluginMime === "application/x-xine-plugin") ? " selected='selected'" : "") + ">Xine</option>" +
			"</select>" +
			"</li>" +
			"</ul>" +
			"<div id='gsconfButDiv'>" +
	...
	function saveConfiguration(evt) {
        ...
        GM_setValue("pluginMime", $("gsconfpluginMimeSel").value);
...
GM_registerMenuCommand("MYousableTubeFix Configuration", configureScript, null, null, "Y");
...
function selChangeVideoFormat(evt) {
	...
	if (videoFormatDetails !== null) {
        ...
        setFlashVar("fmt", selNewValue, true);
