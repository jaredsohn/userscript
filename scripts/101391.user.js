...
// @namespace        PDX
// @description      KoC Attack - Extra Feature! Italian Version by Greed.
// @version          0.0.0.1
...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		if(!div) {
			...
			div.style.border='3px solid #600000';
		...
		//WinLog.write(inspect(cities));
		var citysel ='<select id=srcCity>';
		citysel += '<option value=All>All</option>';
		for(var c=0; c<cities.length; c++) {
		...
		levelsel += '<option value="All">All</option>';
		for(var c=1; c<=11; c++) {
		...
		typesel += '<option value="transport">Transport</option>';
		typesel += '<option value="bulkadded">Bulk Added</option>';
		typesel += '</select>';
		...
		<OPTION value=Camp>Barbarian</option>\
		<OPTION value=Wilderness>Wildners</option>\
		...
		<td class=xtab width=100 align=center>Sanctuary: &nbsp; </td>\
		...
		<a id="KOCAttackViewAttacksList" class="button20"><span>Update</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Empty list</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Stadt</td><td>Cords</td><td nowrap>Typ</td>';
		  h += '<td>Typ</td><td>Troops</td><td>Kamikaze waves/Resources</td><td>Entf.</td><td>Time</td><td>Ignore</td></tr>';
		  var tableRows= '';
		  var count = 1
	  ...
	  var distance=t.CalcXYDist({'x':t.attacks[a].xy[0],'y':t.attacks[a].xy[0]},{'x':t.GetCityCoordsX(t.attacks[a]['fromCity']),'y':t.GetCityCoordsX(t.attacks[a]['fromCity'])});
		tableRows += '<td>'+parseInt(distance)+'</td>'; 
          ...
          ById('KOCAttackViewAttacksDelete').addEventListener('click',function() {
			...
			window.alert(deletes+' Coordinates were deleted!');
 ...
 //window.location.reload(true); 
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Attack command: <select id='KOCAttackOrder'><option value='mostTroops'>Troops</option><option value='closest'>Distances</option></select><br />"+
			"March Typ: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbarian Camp "+
			...
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> Second intervals between the attacks<br />"+
			"time in which the <u>same</u> Targets are attacked.<br />"+
			"<div style='margin-left: 40px'>"+
			"Barbarian Camp:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />Hours!<br />"+
			"Wild:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />Hours!<br />"+
			" City attack:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />Hours!<br />"+
			" Transport:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />Minutes!<br />"+
			" <input id='KOCAttackTransportReserveAmount' value='"+this.options.transportResourcesReserveAmount+"' size='10' />each resource in the city are at least as stock can!<font color=#600000>(Overproduction)</font>"+ 
			"</div>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% random delay <font color=#600000>(to act a human!)</font><br />"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> max. distance for Attack/Transport<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Was launched only from attacking the city in which the first attack.<br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> Knights only attack in the use! <font color=#600000>(will gain even with no knight automatic substitution!)</font> <br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Cancel march when: unknown / excess traffic error <font color=#600000>(required Refresh!)</font><br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Auto Attack off when you are attacked.<font color=#600000>(Auto Attack will NOT AUTO ACTIVATE after the attack!)</font><br />"+			
			"URL... <font color=#600000>(Called up to the attack)</font><br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Automatic deletion of the Barbarians/Wild reports. <font color=#600000>(required Refresh!)</font><br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Farm Reports Auto Delete.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Below are reports, the attack on the offensive side. <font color=#600000>(Send troops window)</font><br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Get reports automatically turn off. <font color=#600000>(Also the reports of the attack site off)</font><br />"+
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
 			"ab <input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% Idle people's auto start training <br /> <input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' />% max of that. possible people forming!<br />"+
 			"<span style=\"font-size:10px; color:#555; line-height:18px; \">To turn on Auto simple training in the barracks at Troop Type 'Auto Training - ON'!</span><br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Note: If you want to train 100% of available people can sometimes cause errors of resources. If the game cache.</span><br />"+ 
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gold_30.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% Auto Gold <font color=#600000>(At what % should be viewed?)</font><br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">In order to enable gold auto in the Castle 'Auto Gold - ON' want.</span><br />"+
			"</td></tr>"+
			
"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> \"Invite friends\" Block pop-ups.<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Automatic for the construction and research assistance to help.<font color=#600000>(CAUTION: When setting the Alliance Chat selected!)</font><br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Construction and research assistance in the chat does not show! <font color=#600000>(If using auto is NOT checked, they will disappear after the help!)</font> <br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Automatically publish. <font color=#600000>(post as a game request to the Chat)</font><br />"+
			"Who can see the request ? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>each</option><option value='50'>Friends of Friends</option><option value='40'>Just Friends</option><option value='10'>Just me</option></select><br />"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Automatically re-load when Facebook or Koc spins.<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Diagnostic logging in der Firefox fehler console einschalten. <font color=#600000>(only for bug reports!)</font><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> Seconds to wait until to change city.<br />"+
			"Durch alle Städte <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> <u>times</u> Switch, <u>then</u> "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /> Seconds <u>wait</u>... then <font color=#600000>Auto Refresh</font>!<br />"+
			"Cities that make attack: "+okCitiesHtml+' <font color=#600000>(Marks = Activated)</font><br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
		
			"Import/Export: <u>Settings</u> copy here...<br /><textarea id='KOCAttackImport'></textarea>"+
			...
			"<a id='KOCAttackOptionsReset' class='button20'><span>Reset (default)</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Back contact (all)</span></a><BR>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Coordinates Delete</span></a>"+
			"<a id='KOCAttackDeleteTransports' class='button20'><span>Transport Delete</span></a><BR>"+
			"<BR><HR><a id='KOCAttackOptionsSave' class='button20'><span>Save</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Abort</span></a>"+
			//"<a id='KOCAttackListAttacks' class='button20'><span>Attacks</span></a><a id='KOCAttackListTransports' class='button20'><span>Transport</span></a>"+
			"</td><TD align=right><a href='http://userscripts.org/scripts/show/98234' target='_blank'>KoC Attack - Italian</a><BR>Italian Version by <a href='http://userscripts.org/' target='_blank'>Nitocle</a><BR>Version:<span style='color:red'> "+ KOCAversion +" </span></td></tr></form>";
		var t=this;
	...
	*/
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>Attack -0n</span>';
		} else {
...
if(this.options.enableLogging){
 ...
 GM_log(str);
	...
	*/	
	...
	IsFirstAttackAtLocation:function(x,y) {
		...
		var attack=this.GetAttack(x,y);
		if(!attack){
			this.Log("No attack on the system, this is the first attack!");
			return true;
		}
 ...
 this.Log("Kamikaze wave is already longer than 30 seconds to go ... Shaft should be sent again, this is now the first attack on ("+x+","+y+")!"); 
			return true;
		}
		}
...
//attack.levelInfo.type = locationType; 
		...
		for(var r=0; r<coordRows.length; r++) {
			...
			if(attack && !force) {
				if(SuicideAttackDefined && isSuicideWave==true){
					this.Log('Kamikaz Welle(n) für '+x+','+y+' wurden bereits Hinzugefügt! versuche sie zu überschreiben...');
					continue;
	...
	AddHtml(bulkAddDiv,"<u>Hinweis</u>: Kopiere die Koordinaten einfach <u>Zeile</u> für <u>Zeile</u> in den Kasten!<br /><u>Wichtig</u>: Die Koordinaten werden mit der <b>Truppenstärke</b> die du <u>oben eingetragen</u> hast, angegriffen!<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> <label for=\"KOCAttackBulkAddForce\">Überschreibe alte Angriffe!</label><br />");
		...
		var arrData = [["Camp", "Barbaren Lager"],["City", "Städte"],["Transport", "Transport"],["Wildernesses", "Wildnisse"]];
 ...
 break;
		...
		//AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Kamikaze Welle(n) <font color=#600000>(2 Wellen Taktik)</font><br />");
 ...
 if(KOCAttackBulkAddLocationType_Camp) { KOCAttackBulkAddLocationType_Camp.checked = true; }
	...
	HandleDomainLogin:function() {
		...
		if (t.currentPage == "domain_selection" && t.options.autoLogBackIn && !t.domainLoginActionTaken) {
			...
			if(!statusDiv) {
				...
				statusDiv.style.border='3px solid #600000';
	...
	OnCastleBoxAppear:function(box) {
		...
		function SetAutoGoldA() {
			var str=t.GetAutoGold()?'Auto Gold - AN':'Auto Gold- AUS';
		...
		function SetAbandonWildsA() {
			var str=t.GetAbandonWilds()?'Wildnisse Aufgeben - AN':'Wildnisse Aufgeben - AUS';
...
var arrData = [["Camp", "Barbaren Lager"],["City", "Städte"],["Transport", "Transport"],["Wildernesses", "Wildnisse"]];
 ...
 btnMarch.parentNode.insertBefore(addAttackContainerDiv,btnMarch);
		...
		AddText(div2,'In der Angriffs Liste Ignorieren!');
...
if(supplyfilter_checkbox.checked){
 ...
 if(!resetTime){resetTime=false;} 
	...
	if(added){
     if(alert){ 
			window.alert("Angriff Hinzugefügt!");
			}
 } 
	},
	...
	OnToolTipAppear:function(box) {
		...
		if(troops && troops.length>0) {
			...
			var nowSecs=new Date().getTime()/1000;
			var lastAttackStr='Letzter Angriffen: '+SecsToStr(nowSecs-lastAttack)+' - '+(attack.ignore?' <b>(Ignorieren)</b> ':'');
...
DeleteFarmAttacks:function() {
 ...
 reportsRemoved+=this.DeleteFarmAttacks();
		...
		a.innerHTML='<span>Wild/Barb/Transp Löschen</span>';
 ...
 msgBody.appendChild(a);
	...
	AddTrainTroopsLink:function() {
		...
		var setTrainTroopsA=function() {
			...
			a.innerHTML='<span>'+(trainTroops[t.GetCurrentCityId()]==type?'Auto Ausbildung - AN':'Auto Ausbildung - AUS')+'</span>';
		}
		a.addEventListener('click',function() {
			var trainTroops=JSON2.parse(t.GetValue('TrainTroops','{}'));
 ...
 return curCityNum;
			...
			if(numInp && maxObj) {
				...
				window.setTimeout(function() {
					if(numInp.value>parseInt(maxObj.textContent)) {
						t.Log('Keine Auszubildene Truppen:'+numInp.value+'>'+maxObj.textContent);
						return;
					}
	...
	*/
	...
	SetupMenu:function() {
		...
		GM_registerMenuCommand('KoC Attack - Zurücksetzten!',function() {
			t.FactoryReset();
		});
	},
...
//t.Log('Last known attack city from :'+currentAttackCityNum);
 ...
 t.Log('Zur Letzten Stadt wechsel vor dem Refresh:'+currentAttackCityNum);
...
function checkWhiteScreen (){
 ...
 KOCAttack.ReloadWindow();
 GM_log("White Screen Error. Auto Refresh.....");
 }
 return;
 }
 checkiFrame();
...
GM_log("Check strange majic");
 if (!document.getElementById("kochead")){
    ...
    popup (100,100,500,275, "<BR><CENTER><b>KoC Attack Deutsch</b><BR>checkStrangeMajik<BR>Kingdom of Camelot wurde nicht gefunden!<BR>Auto Refresh in 5 Sekunden...<BR><BR>");
  }  
}
