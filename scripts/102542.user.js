...
// @require        http://sizzlemctwizzle.com/updater.php?id=98323
// ==/UserScript==
...
{ document.getElementById("mod_comm_list2").style.backgroundColor = '#fffbe0';}
if(document.getElementById("mod_comm_list1"))
{ document.getElementById("mod_comm_list1").style.backgroundColor = '#fffbe0';}
...
Tabs.Hud = {
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    .ptChatWhisper {}\
    .ptChatAlliance {}\
    .ptChatGlobal {background-color: #84C6DE}\
...
<div class="content">
  <div class="info">
    ...
    <b style="color: rgb(165, 102, 49); font-size: 9px;"> sagt zur Allianz:</b> 
...
// "Report No: 5867445"  --->  see unsafeWindow.modal_alliance_report_view()
  ...
  chatDivContentHook : function (msg){
    ...
    if (m[0].indexOf('whispers') >= 0)
      class = 'ptChatWhisper';
    else if (m[0].indexOf('zur Allianz') >= 0)
...
//    t.show ();
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('ZurÃ¼cksetzten', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
    for (var c=0; c<Cities.numCities; c++){
      var city = Cities.cities[c];
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wildniss</td><TD></td><TD align=left>Koordinate</td><TD>Fallen</td><TD align=left>SÃ¶ldner</td>\
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TD class=ptentry> &nbsp; &nbsp; <B>SÃ¶ldner:</b> ' + htmlSelector(mercNames, wildDef.mercLevel, 'id=ptwm_'+ c +'_'+ i) +' &nbsp; &nbsp; </td></tr>';
        }
      ...
      for(i=0; i < Cities.numCities; i++) {
		...
		if (timeLeft > 0 && (timeLeft<(Options.foodChatWarnHours*3600))) {
                msg += 'Meine Stadt ' + Cities.cities[i].name.substring(0,10) + ' (' +
                       Cities.cities[i].x +','+ Cities.cities[i].y + ')';
                msg += ' hat wenig Nahrung. Ã¼brig: '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+') Unterhalt: '+addCommas(usage);
                sendChat ("/a " + msg);
          }
      }  
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    msg += 'Meine '+ target +' wird '+ atkType  +' von '+ who +' = Truppen StÃ¤rke (Ankunft in '+
        unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +') : ';
    var totTroops = 0;
    for (k in m.unts){
      var uid = parseInt(k.substr (1));
...
Tabs.AllianceList = {
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Suchergebnisse: <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Macht</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Information </td></tr>';
    var row=0;
    var cl='';
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Allianz: '+ a +' &nbsp; #: '
          + u.cities +' &nbsp; Volk: '+ u.population +'</td></tr><TR><TD></td><TD>Provinzen: ';
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">Letzter Login:</span><span style="color:red"> '+lastLogin+'</span>';
    } else {
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Ergebnisse: <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Name</td><TD class=xtab>Rang</td><TD class=xtab>Spieler</td>\
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 ) {
      document.getElementById('allListOut').innerHTML = 'Allianzen als erstes einblenden!';
      return;
    }
    if (t.MaxPage < 0 || val > t.MaxPage || val < 1) {
      document.getElementById('allListOut').innerHTML = 'Seite Nummer exestiert nicht!';
      return;
    }
    t.curPage = val;
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Suche ...</center>';
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    var m = '<div style="overflow:auto; height:556px;width:564px;"><TABLE><thead><TR style="font-weight:bold"> \
        <th class=xtab>Name</th><th class=xtab>Rang</th><th class=xtab>Spieler</th>\
...
//new CtableToText('tabAllMembers').toText();
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("Keine Allianz gefunden!")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
...
Tabs.Fake = {
  ...
  init : function (div){
	...
	var citySelect = '   <SELECT id=fakeCity>';
	    ...
	    citySelect += '</select>';
    var m = '<TABLE><TR><TD align=right>SpÃ¤h Angriff: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
		...
		<TR><TD align=right>SpÃ¤her: </td><TD><INPUT type=text size=6 value=0 id=fakeScout></td></tr>\
		  <TR><TD align=right>LanzentrÃ¤ger: </td><TD><INPUT type=text size=6 value=0 id=fakePike></td></tr>\
		  <TR><TD align=right>SchwertkÃ¤mpfer: </td><TD><INPUT type=text size=6 value=0 id=fakeSword></td></tr>\
		  <TR><TD align=right>BogenschÃ¼tze: </td><TD><INPUT type=text size=6 value=0 id=fakeArcher></td></tr>\
...
var crestreq = { 3:{1101:4, 1102:2, 1103:1},
 ...
 m += '</TABLE>';
	//End crest info
		//Link info
    m += '<DIV class=ptstat>NÃœTZLICHE LINKS</div>';
    m += '<table width="700" border="0" cellspacing="0" cellpadding="0">';
    m += '<tr>';
    m += '<td width="57"><b>Nr.</b></td>';
    m += '<td width="343"><b><center>Thema</center></b></td>';
    m += '<td width="156"><b><center>Quelle</center></b></td>';
    m += '</tr>';
    m += '<tr>';
    m += '<td><b># 1</b></td>';
	...
	m += '<td><center><a href=http://kingdoms-of-camelot.de target=_blank>kingdoms-of-camelot.de</a></center></td>';
    m += '</tr>';
    m += '<tr>';
    m += '<td><b># 2</b></td>';
    ...
    m += '<td><center><a href="http://koc.wiki.com" target="_blank">koc.wiki.com</a></center></td>';
    m += '</tr>';
    m += '<tr>';
    m += '<td><b># 4</b></td>';
    ...
    try {      
      m = '<TABLE class=ptTab>\
       <TR><TD colspan=2><B><u>Power Tools - Einstellung</u></b></td></tr>\
        ...
        <TR><TD><INPUT id=ptHideOnGoto type=checkbox /></td><TD>Automatisch bei Klick auf Koordinaten Fenster SchlieÃŸen.</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>\'Reicht fÃ¼r\' in Rot Anzeigen wenn es noch fÃ¼r \
        <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> Stunden ausreicht! <SPAN class=boldRed>(Stats Tab)</span></td></tr>\
                     <TR><TD><INPUT id=ptEnableFoodChatWarn /></td><TD>In den Chat schreiben wenn die Nahrung nur noch fÃ¼r \
            <INPUT id=optfoodChatWarnHours type=text size=3 value="'+ Options.foodChatWarnHours +'"> std. reicht.</td></tr>\
      ...
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Erweiterte Allianz Berichte.</td></tr>\
        ...
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Warnen falls du nach 0,0 Maschieren mÃ¶chtest.</td></tr>\
        ...
        <TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Chat Erweitern <SPAN class=boldRed>(Koordianten Klickbar, pink global messages, Avatar klicken zum flÃ¼stern)</span> </td></tr>\
        <TR><TD><INPUT id=togAttackPicker type=checkbox /></td><TD>Heiligtum AuswÃ¤hler auf der Truppen Bewegungs Seite Anzeigen. <SPAN class=boldRed>(VerstÃ¤rkung, Transport etc...)</span></td></tr>\
        <TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Die Runden Anzahl einblenden</td></tr>\
        <TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>LÃ¶sch Button einblenden <SPAN class=boldRed>(Um auf der Angriffs Seite die Koordinate direkt zu LÃ¶schen)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B><u>Bug Fix</u></b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Wachturm Berichte Fix <SPAN class=boldRed>(Stadt, Wildnis oder UngÃ¼ltig)</span></td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Zeige die Entfernung von der AusgewÃ¤hlten Stadt, und nicht immer die von der ersten!</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Fake Angriffe durch SpÃ¤her Attacken vermeiden.</td></tr>\
        <TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD><u>Keinen</u> Ritter auswÃ¤hlen wenn der Marsch typ: <SPAN class=boldRed>SpÃ¤hen, Transport</span> oder <SPAN class=boldRed>VerstÃ¤rken</span> ist!</td></tr>\
        <TR><TD><INPUT id=togCoordBox type=checkbox /></td><TD>Koordinaten Box  <SPAN class=boldRed>Ã¼ber</span> der Truppenbewegung Anzeigen!</td></tr>\
        </table><BR><BR><HR><u>Hinweis</u>: Wenn eine Box nicht klickbar ist hat KoC mal wieder eine Ã¤nderung vorgenommen!<HR><BR><a href="http://userscripts.org/scripts/show/98323" target="_blank">Power Tools - Deutsch</a> - German Version by <a href="http://userscripts.org/users/297645/scripts" target="_blank">PDX</a> - Version: <SPAN class=boldRed> modded by HellHound '+ Version +' </span>';
      t.cont.innerHTML = m;
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      ...
      <SELECT id=ptttType>\
        ...
        <option value='3'>SpÃ¤her</option>\
        <option value='4'>LanzentrÃ¤ger</option>\
        <option value='5'>SchwertkÃ¤mpfer</option>\
        <option value='6'>BogenschÃ¼tze</option>\
      ...
      <TR><TD align=right>Du benÃ¶tigst mehr: </td><td colspan=2><font color=#FF0000><B> <span id=ptttLimiter></span> </b></font></TD></TR>\
      ...
      <SELECT id=pttdType>\
        <option value='53'>Fest montierte ArmbrÃ¼ste</option>\
        <option value='55'>Abwehr Trebuchet</option>\
        <option value='60'>Falle</option>\
        <option value='61'>KrÃ¤henfÃ¼ÃŸe</option>\
        <option value='62'>Eiserne ErdspieÃŸe</option>\
      ...
      <TR align=center><td align=right>Du benÃ¶tigst mehr: </td><TD colspan=2 align=left><font color=#FF0000><B> <span id=pttdLimiter></span> </b></font></td></tr>\
...
/*******  TROOPS  ******/  
  ...
  changeTroopSelect : function (){
    ...
    if ( (t.stats.idlePop / uc[6]) < max) {
      ...
      limiter = '<blink>BevÃ¶lkerung</blink>';
    }
...
/*******  DEF  ******/  
  ...
  changeDefSelect : function (){
    ...
    if (matTypeof(uc[8]) == 'object'){
      for (k in uc[8]){  // check building requirement
        ...
        if (b.maxLevel < uc[8][k][1]){
          t.stats.MaxDefTrain = 0;
          t.stats.DefLimiter = '<blink>GebÃ¤ude/Upgrade</blink>';
          break;
        }
      }
    }
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Bau AuftrÃ¤ge wurden in Auftrag gegeben!</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B><u>Versorger</u></b></td><TD width=8%><B><u>Miliz</u></b></td><TD width=8%><B><u>SpÃ¤her</u></b></td>\
 ...
 '<TD width=8%>'+Seed.units['city'+cityId]['unt11']+'</td><TD width=8%>'+Seed.units['city'+cityId]['unt12']+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        ...
        <TD width=16%><B>Erz</b></td><TD width=16%><B>Gold</b></td><TD width=16%><B>MÃ¼ÃŸige BevÃ¶lkerung</b></td></tr>\
...
//DebugTimer.start(); 
    try {
      ...
      str += "<DIV id=overMainDiv style='font-size:"+ Options.overviewFontSize +"px'><TABLE class=ptTabOverview cellpadding=0 cellspacing=0><TR valign=top align=right><TD width=65></td><TD width=88 style='background: #ffc'><B>Gesamt</b></td>";
      for(i=0; i<Cities.numCities; i++) {
      ...
      rownum = 0;
	  str += '<TR><TD class=ptstat>Truppen</td></tr><BR>';
      str += _row ('Versorger', rows[1]);
      str += _row ('Miliz', rows[2]);
      str += _row ('SpÃ¤her', rows[3]);
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Marschierende Truppen/Resourcen Anzeigen</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptoverIncTraining'+ (Options.includeTraining?' CHECKED':'') +'>Auszubildene Truppen Anzeigen/Zusammenrechnen</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Overflow erlauben \
         &nbsp; &nbsp; | SchriftgrÃ¶ÃŸe: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12 , 14:14}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr>';
      str += "</table></div>";
...
Tabs.Marches = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubA type=submit value=Angriffe></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubM type=submit value=MÃ¤rsche></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubR type=submit value=VerstÃ¤rkung></td></tr></table><HR class=ptThin>\
  ...
  showAttacks : function (){
    var t = Tabs.Marches;
    t.marchDiv.innerHTML = 'Hier werden die Angriffe auf dich Angezeigt! Diese Funktion ist noch nicht verfÃ¼gbar!!';;
  },
  ...
  showMarches : function (){
    var t = Tabs.Marches;
    t.marchDiv.innerHTML = 'Hier werden mal die MÃ¤sche Angezeigt! Diese Funktion ist noch nicht verfÃ¼gbar!';
  },
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['Versorger', 'Miliz', 'SpÃ¤her', 'Lanzen', 'Schwerter', 'Bogen', 'Kav', 'S-Kav', 'Wagen', 'Ballisten', 'Ram', 'Kat'];
    var t = Tabs.Marches;
    clearTimeout (t.displayTimer);
...
//logit ("enc: "+ inspect (enc, 6, 1));    
    ...
    if (numSlots == 0){
      s += '<BR><CENTER><B>Es hat dir keiner verstÃ¤rkung geschickt...! :/</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Spieler (Ritterpunkte)</b></td>';
