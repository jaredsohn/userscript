...
// @namespace     https://twitter.com/mcxiaoke

// @description   Expand shortened URLs use LongURL.org API.使用 LongURL.org 的API展开短链接

// @author        mcxiaoke

// @homepage      https://twitter.com/mcxiaoke
...
(function() {
			...
			ajaxRequest({
				...
				onload: function(response) {
					...
					var url = data['long-url'];
					    var result = url;
					    result += '<br /><a href='+url+' title="Goto this link" style="color:#00f;" target="_blank">新窗口打开原始链接</a>';
	...
	tooltip = function(text, e) {
		...
		if (typeof(this.tooltip_node) === 'undefined') {
			...
			this.tooltip_node.style.maxWidth = '720px';

			this.tooltip_node.style.backgroundColor = '#eeeeee';

			this.tooltip_node.style.border = '2px solid #c9c9c9';

			this.tooltip_node.style.padding = '6px';

			this.tooltip_node.style.fontSize = '12px';
			this.tooltip_node.style.letterSpacing = '0px';
			this.tooltip_node.style.color = '#000000';
