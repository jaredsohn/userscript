...
function weblancerClick() {
  ...
  try {
    ...
    xmlhttp.onreadystatechange = function() {
		...
		to = to.parentNode;
		to.className="";
		if(this.responseText.lastIndexOf('<table class="bids_list">')>0){
			to.innerHTML = "<div>"+this.responseText.match(/<td id="right_panel_2">([\s\S]*?)<table class="bids_list">\s/m)[1]+"</div>";
		} else {
			to.innerHTML = "<div>"+this.responseText.match(/<td id="right_panel_2">([\s\S]*?)<div class="text_box">/m)[1]+"</div>";
		}
	};
    xmlhttp.send(null);
  } catch(e) {
