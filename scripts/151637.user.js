// ==UserScript==
// @name 		Vplaats - laatste dorp
// @description 	Een nieuwe mogelijkheid op je verzamelplaats om het laatste doelwit te kiezen.
		...
		if(coords) {
			...
			if(td) {
				...
				if(coords.length == 3) {
					...
					link.textContent = unescape('%BB') + ' Laatste (' + coords[1] + '|' + coords[2] + ')';
				} else {
					...
					link.textContent = unescape('%BB') + ' Laatste (' + coords[1] + ':' + coords[2] + ':' + coords[3] + ')';
				}
				...
				if(coords.length == 3) {
					...
					link.title = img.alt = 'Kaart centreren';
