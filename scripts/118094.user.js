// ==UserScript==
// @name			DoA Power Tools Plus API 23 Fix
// @namespace		http://www.facebook.com/DoaTools
 ...
 ********************************************************************************/
// Script information
var Version = 'API 23 Fix'; // Year, Month, Day, Revision, Maturity (e.g. YYYMMDDa_BETA)
var Title = 'http://www.facebook.com/DoaTools'; // Removed for security purposes
var WebSite = 'http://www.facebook.com/DoaTools'; // Removed for security purposes
var getVersion = 23;
var postVersion = 23;

// Help pages
var urlError = 'http://www.facebook.com/DoaTools';

// Style - Rules: First character must be a-zA-Z, last character must be a-zA-Z0-9, middle characters must be _a-

zA-Z0-9-
var rndStyle = ['idPrefix', 'classPrefix', 'subtab_info', 'subtab_build', 'subtab_research', 'subtab_train', 

'status_ticker', 'status_feedback', 'job_header', 'job_content'];
var rndId = ['enable_drag', 'enable_verbose', 'enable_collect', 'collect_time', 'collect_unit', 'options_refresh', 

'options_clear', 'support_link'];
var rndClass = ['red_button', 'green_button', 'blue_button', 'yellow_button', 'purple_button', 'cyan_button', 
...
var children,
    i,
    iframeId,
    objectId = "castlemania_swf",
    platform;

if (window.location.href.indexOf("facebook") !== -1) {
    iframeId = "iframe_canvas";
    platform = "facebook";
} else if (window.location.href.indexOf("google") !== -1) {
    iframeId = "oz-gadgets-canvas-iframe-659749063556";
    platform = "google";
}
...
if (window.top === window.self) {
    ...
    function setHigh() {
        ...
        switch (platform) {
        case "facebook" :
            children = document.getElementById("hd").childNodes;
...
function initStyle () {
	...
	setTimeout (initScript, 1000); // NOTE: Forced a delay in an attempt to stop the styling issue (Need to 
...
var wordArr = ['Czar', 'Bright', 'Work', 'Power', 'Tools', 'Dragons', 'Fire', 'Water', 'Lava', 'Metal', 'Stone', 

'Protect', 'Branch', 'Child', 'New', 'Fangled', 'Flugel', 'Horn', 'Perfect', 'Myth', 'Jelly', 'Graph', 'Quick', 

'Thorn', 'Pitbull', 'Tech', 'Cow', 'Middle', 'Brow', 'Hammer', 'Chord', 'Dazzle', 'Elemental', 'Brillig', 'Craft', 

'Thumb', 'Print', 'Crtyp', 'Torch', 'Light', 'Bank', 'Final', 'Epic', 'Desk', 'Marble', 'Aqua', 'Phoenix', 

'Peanut', 'Halo', 'Nimbus', 'Cloud', 'Seed',];
...
var kFatalSeedMsg = 'Please disable the script and see if you are able to play the game manually. If normal play is 

possible then enable the script and try again. If the error persists please read the following post before 

submitting a report. If normal play is not possible then wait until it is and try again.';
...
*) added Maps tab - searches and displays ant camps, cities, outposts, grasslands, swamps, lakes, hills, plains, 

mountains, and forests within 35 miles
*) allow mapped objects to be attacked, changed the Ant tab name to Attack to reflect new functinality
*) user interface changes: buttons (hover, active, cursor), remember last sub-tab displayed, make sub-tabs behave 
...
//
// This is part of a sweeping change to remove the camp terminology and replace it with mapped objects (e.g. 

mountains, plains, cities, etc.)
var OptionsDefaults = {
	// Popup
	popUp			: {open:true, drag:true, x:0, y:0},
	
  objAttack     : {enabled:false, repeatTime:3660, delayMin:30, delayMax:60, levelEnable:[], levelDist:

[null,10,10,10,10,10,10,10,10,10,10], deleteObjAttacks:false, stopAttackOnLoss:false, logAttacks:true, 

maxMarches:10, troops:[], clearAllTargets:false},
  currentTab    : false,
  attackTab     : 0,
  mapTab        : 0,
  jobsTab       : 0,
  ...
  //waves         : {enabled:false, iterationMin:30, iterationMax:45, stopOnLoss:true, deleteReports:false, 

targets:[], runTime:0},
  objStats      : null,
  objMarches    : {},
  version       : {lastChecked:0, checkVersion:Version, lastVersion:Version},
  maps          : {enabled:false, repeatTime:3660, delayMin:30, delayMax:60, levelEnable:[], levelDist:
...
maxMarches:10},
  ...
  researchTimer : null,
  trainTimer    : null,
//  alertConfig  : {aChat:false, aPrefix:'** I\'m being attacked! **', scouting:false, wilds:false, 

minTroops:10000, spamLimit:10 },
	// Options Tab
...
var Styles = '\
    div.' + rndClass['tab_title'] + ' {border:1px solid; border-color:#ffffff; font-weight:bold; padding-top:2px; 

padding-bottom:2px; text-align:center; color:#ffffff; background-color:#436}\
    div.' + rndStyle['classPrefix'] + 'Subtitle {border:1px solid; border-color:#ffffff; font-weight:bold; padding

-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#444}\
	...
	div.' + rndStyle['status_feedback'] + ' {height: 34px; border:1px solid black; background-color:#ffeeee; 

padding: 2px 0px 2px 2px; text-align:left; font-weight:bold;}\
    div.short {height:7px;}\
    .hiding {background-color: #2FAC2F; color: white; padding-left: 10px; padding-right: 10px; margin-right: -

2px;}\
    .defending {background-color: #F80000; color: white; padding-left: 10px; padding-right: 10px; margin-right: -

2px;}\
    div#qTip {padding: 3px; border: 1px solid #777; border-right-width: 2px; border-bottom-width: 2px; display: 

none; background: #999; color: #fff; font: bold 9px Verdana, Arial, sans-serif; text-align: left; position: 

absolute; z-index: 1000}\
    table.' + rndStyle['classPrefix'] + 'TabPad tr td {border:none; background:none; white-space:nowrap; padding: 

1px 1px}\
    table.' + rndStyle['classPrefix'] + 'Tab tr td {border:none; background:none; white-space:nowrap; padding: 1px 

1px;}\
    table tr td.' + rndStyle['classPrefix'] + 'TabLeft {font-weight:bold; text-align:right; padding-right: 5px}\
    table.' + rndStyle['classPrefix'] + 'TabLined tr td {border-bottom:1px solid #ccc; background:none; padding: 
...
1px 4px 1px 4px;}\
    ...
    tr.' + rndStyle['classPrefix'] + 'retry_ptpoptp td { background-color:#a00; color:#fff; border:none; height: 
...
21px; padding:0px; }\
    ...
    table.' + rndStyle['classPrefix'] + 'MainTab tr td   {height:60%; empty-cells:show; padding: 0px 5px 0px 5px;  

margin-top:5px; white-space:nowrap; border: 2px solid; border-style: none none solid none; }\
    table.' + rndStyle['classPrefix'] + 'MainTab tr td.spacer {padding: 0px 3px; border:none; }\
    table.' + rndStyle['classPrefix'] + 'MainTab tr td.sel    {font-weight:bold; font-size:13px; border: 1px solid; 

border-style: solid solid none solid; background-color:#eed; cursor:hand; cursor:pointer; padding: 2px;}\
    table.' + rndStyle['classPrefix'] + 'MainTab tr td.notSel {font-weight:bold; font-size:13px; border: 1px solid; 

border-style: solid solid none solid; background-color:#0044a0; color:white; border-color:black; cursor:hand; 

cursor:pointer; padding: 2px;}\
    .' + rndStyle['classPrefix'] + 'CP .' + rndStyle['classPrefix'] + 'CPMain { background-color:#f8f8f8; 

padding:6px;}\
    input.butAttackOff {width:130px; background-color:#e80000; color:white; font-weight:bold; cursor:hand; 

cursor:pointer;}\
    input.butAttackOff:hover {width:130px; background-color:#f80000; color:white; font-weight:bold; cursor:hand; 

cursor:pointer;}\
    input.butAttackOn {width:130px; background-color:#009C1F; color:white; font-weight:bold; cursor:hand; 

cursor:pointer;}\
    input.butAttackOn:hover {width:130px; background-color:#2FAC2F; color:white; font-weight:bold; cursor:hand; 
...
cursor:pointer;}\
    ...
    input.' + rndClass['red_button'] + ', input.' + rndClass['green_button'] + ', input.' + rndClass['blue_button'] 

+ ', input.' + rndClass['yellow_button'] + ', input.' + rndClass['cyan_button'] + ', input.' + rndClass

['purple_button'] + ' {width:130px; color:white; font-weight:bold; cursor:hand; cursor:pointer;}\
    input.' + rndClass['red_button'] + ':active, input.' + rndClass['green_button'] + ':active, input.' + rndClass

['blue_button'] + ':active, input.' + rndClass['yellow_button'] + ':active, input.' + rndClass['cyan_button'] + 

':active, input.' + rndClass['purple_button'] + ':active {width:130px; background-color:black; color:white; font-

weight:bold; cursor:hand; cursor:pointer; }\
    input.' + rndClass['red_button'] + ':hover, input.' + rndClass['green_button'] + ':hover, input.' + rndClass

['blue_button'] + ':hover, input.' + rndClass['yellow_button'] + ':hover, input.' + rndClass['cyan_button'] + 

':hover, input.' + rndClass['purple_button'] + ':hover {width:130px; color:white; font-weight:bold; cursor:hand; 
...
cursor:pointer; }\
    ...
    .button {cursor:hand; cursor:pointer; border: 1px solid #006; background: #049C93; color: white; padding: 2px; 

font-weight:bold; font-size:13px; border-style: solid solid none solid;}\
//  .button:hover {background: #eed; font-weight:bold; font-size:13px; color: black; border-style: solid solid none 

solid; }\
//  .button:active {background: black; font-weight:bold; font-size:13px; color: white; border-style: none none none 

none;}\
    span.boldRed {color:#550000; font-weight:bold}\
    hr.thin {margin:0px; padding:0px}\
    #hdRnd ul.tabs li.tab a.' + rndStyle['classPrefix'] + 'toolbutOn {background-color:#900 ! important; 

color:white ! important}\
    #hdRnd ul.tabs li.tab a.' + rndStyle['classPrefix'] + 'toolbutOff {background-color:#ffc ! important; 
...
function dtStartup() {
	...
	try {
		...
		logit (inspect (C, 6, 1));
		Data.init({options:OptionsDefaults, log:[], targets:{radius:0, center:{x:0, y:0}, mapObjects:[], 

camps:[], cities:[], outposts:[], grasslands:[], swamps:[], lakes:[], hills:[], plains:[], mountains:[], forests:
		...
		Translate.init(function (rslt) {
			if (rslt.ok) {
				...
				dialogFatal('<B>' + kFatalSeedTitle + '</B><BR><BR>\
					' + kFatalSeedMsg + '<BR><BR>\
					<A id="' + rndId['support_link'] + '" href="javascript:;">Bugs and Known 
...
Issues</A><BR><BR>\
		...
		function startScript() {    // TODO: check result, retry or disable tools?
			...
			mainPop = new CPopup ('main', Data.options.popUp.x, Data.options.popUp.y, popupWidth, 800, 

Data.options.popUp.drag, function () {
				tabManager.hideTab();
			});
...
// This is very crude - does not really check for an updated version, only if the current version is different from 

the one in userscripts.org
  ...
  GM_xmlhttpRequest({
    ...
    onload : function(r){
      ...
      if (m && m[1]!=Data.options.version.checkVersion){
        clearTimeout (timer);
        pop = new CPopup ('checkversion', Data.options.popUp.x, Data.options.popUp.y, 300,200, 

Data.options.popUp.drag, remindLater); 
        // Translation
        ...
        pop.getMainDiv ().innerHTML = '<STYLE>'+ Styles +'</style><BR><CENTER>'+ kAnnounceVersion + m[1] 
            +'<BR><BR>at: <A HREF="http://'+ WebSite +'">'+ WebSite +'</a><BR><BR><INPUT id=doaVerRML type=submit 

value='+ kRemindLater +'\> &nbsp; &nbsp;<INPUT id=doaVerDR type=submit value='+ kNoReminder +'\></center>';
        pop.setLayer(20);
        pop.show(true);
...
var Messages = {
  ...
  gotBattleReport : function (rpt){
    var t = Messages;
if (DEBUG_MARCHES) WinLog.write ('Read Message: '+ rpt.report.location.terrain +' , '+ rpt.report.location.x +','+  

rpt.report.location.y +' General: '+ rpt.report.attacker.general.id );    
...
var Manifest = {
	...
	fetchManifest : function (notify) {
		var now = new Date().getTime() / 1000;
		new MyAjaxRequest ('manifest.json', { 'user%5Fid':C.attrs.userId, timestamp:parseInt(serverTime()), 

'%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, 'dragon%5Fheart':C.attrs.dragonHeart }, function (rslt) {
			if (rslt.ok && !rslt.dat.errors) {
				Manifest.m = rslt.dat; // This holds the entire Manifest JSON data parsed as an 

object 
				try {
					//var testThis = Manifest.buildings.byCityType('capital', 'all', 

{alphabetical: 'asc', buildable: false});
					//testThis = Manifest.buildings.sortBy(testThis, {alphabetical: 'desc', 
...
buildable: false});
					...
					rslt.ok = false;
					rslt.errmsg = 'fetchManifest when calling updateManifest returned this 
		...
		byCityType : function (cityType, buildable, order) {
			...
			if (buildings.length > 0) {
				for (b = 0; b < buildings.length; b = b + 1) {
					if (buildings[b].buildable === buildable || buildable.toLowerCase() === 

'all') {
						if (buildings[b].city_type.length > 0) {
							for (ct = 0; ct < buildings[b].city_type.length; ct = ct + 

1) {
								if (buildings[b].city_type[ct] === 

cityType.toLowerCase() || cityType.toLowerCase() === 'all') {
...
'all') {
						if (buildings[b].location === location.toLowerCase() || 
		...
		sortBy : function (data, order) {
			...
			function orderLocation(order) {
				if (order.toLowerCase() === 'city') {
					data.sort(function (a, b) {
						var locationA = a.location.toLowerCase(), locationB = 
...
[buildingManifest[b].levels[l].level] = {};
		}

		// 	Save building requirements
		for (var b=0; b<buildingManifest.length; b++) 
			for (var l=0; l<buildingManifest[b].levels.length; l++) 
				Seed.requirements.building[buildingManifest[b].type].level[buildingManifest

[b].levels[l].level] = buildingManifest[b].levels[l].requirements;
...
[researchManifest[r].levels[l].level] = {};
		}

		// 	Save research requirements
		for (var r=0; r<researchManifest.length; r++) 
			for (var l=0; l<researchManifest[r].levels.length; l++) 
				Seed.requirements.research[researchManifest[r].type].level[researchManifest

[r].levels[l].level] = researchManifest[r].levels[l].requirements;
...
'%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, 'dragon%5Fheart':C.attrs.dragonHeart }, function (rslt) {
	...
	fetchSeed : function (notify) {
		var t = Seed, city;
		new MyAjaxRequest ('player.json', {'user%5Fid':C.attrs.userId, 'dragon%

5Fheart':C.attrs.dragonHeart, '%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, timestamp:parseInt

(serverTime()) }, function (rslt) {
			if (rslt.ok && !rslt.dat.errors) {
				if (rslt.dat.timestamp)
	...
	fetchCity : function (cityId) {
		...
		var t = Seed;
		new MyAjaxRequest('cities/'+ cityId +'.json', { 'user%5Fid':C.attrs.userId, timestamp:parseInt

(serverTime()), '%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, 'dragon%5Fheart':C.attrs.dragonHeart }, 
		...
		for (var pcity in t.jobs) {
			for (var pjob in t.jobs[pcity]) {
				...
				if (job.run_at < (now - 300)) {
					if (!job.done) {
					//WinLog.write ('****** TIMER Seed.tick: RETAINING \'UNDONE\' JOB  (now='+ 

serverTime() +'):\n'+ inspect (job, 4, 1)); 
					//logit ('****** TIMER Seed.tick: RETAINING \'UNDONE\' JOB  (now='+ 
...
+'):\n'+ inspect (job, 4, 1)); 
					...
					if (march && job.queue=='march' && march.status=='marching') {  // march 
  ...
  checkIncomingData : function (rslt){
    ...
    for (var ij=0; ij<rslt.dat.city.jobs.length; ij++){
      ...
      if (job.queue == 'building'){
        ...
        if (!building){
          WinLog.writeText ('*********************** MISSING BUILDING! ('+ job.city_building_id +') now='+ 
  ...
  findMarch : function (mid, marches){
	...
	var Translate = {
		...
		fetchLocale : function (notify) {
			var t = Translate;
			new MyAjaxRequest ('locales/' + navigator.language.substr(0, 2) + '.xml', {'%5Fswf%

5Fsession%5Fid':C.attrs.sessionId}, function (rslt) {
				if (rslt.ok) {
					try {
					...
					new MyAjaxRequest('locales/en.xml', {'%5Fswf%5Fsession%

5Fid':C.attrs.sessionId}, function (rslt) {
						if (rslt.ok) {
							try {
								t.updateCommons(rslt.dat);
								t.updateItems(rslt.dat);
		...
		updateCommons : function (xmlString) {
			var t = Translate, xmlStart = xmlString.indexOf("<common>"), xmlFinish = xmlString.indexOf

("</common>") + 9, parser, xmlObject, commons, common, i, ii;
				...
				for (i = 0; i < common.length; i = i + 1) {
					if (common[i].firstChild) {
						if (common[i].firstChild.nodeValue) {
							t.commons[t.commons.length] = {id:common[i].nodeName, 

name:common[i].firstChild.nodeValue};
						} else if (common[i].childNodes.length > 0) {
							for (ii = 0; ii < common[i].childNodes.length; ii = ii + 1) 

{
								t.commons[t.commons.length] = {id:common

[i].childNodes[ii].nodeName, name:common[i].childNodes[ii].firstChild.nodeValue};
							}
						}
					}
				}
		...
		updateItems : function (xmlString) {
			var t = Translate, xmlStart = xmlString.indexOf("<items>"), xmlFinish = xmlString.indexOf

("</items>") + 8, parser, xmlObject, items, item, i;
			// Parse the string into an object			
			xmlObject = t.parseXML(xmlString, xmlStart, xmlFinish);
			// Traverse through xmlObject and place each item into an array of objects
			...
			if (item.length > 0) {
				for (i = 0; i < item.length; i = i + 1) {
					t.items[t.items.length] = {id:item[i].nodeName, name:item
...
[i].firstChild.firstChild.nodeValue};
		...
		updateTroops : function (xmlString) {
			var t = Translate, xmlStartOffset = xmlString.indexOf("<troops>"), xmlFinishOffset = 

xmlString.indexOf("</troops>"), xmlStart, xmlFinish, xmlObject, items, item, i;
			// As this element appears twice the result needs to be offset (note if xml structure 
...
changes this could break)
			...
			if (troop.length > 0) {
				for (i = 0; i < troop.length; i = i + 1) {
					t.troops[t.troops.length] = {id:troop[i].nodeName, name:troop
...
Tabs.Waves = {
	...
	init : function (div) {
		...
		Data.init({waves: {enabled:false, iterationMin:30, iterationMax:45, stopOnLoss:true, 

deleteReports:false, target:{troopsWave:{}, x:0, y:0, type:null, level:0, stats:{numAttacks:0, spoils:{}}}, 
...
runTime:0}});
		...
		//	for (var i=0; i<5; i++)
		//		Data.waves.targets[i] = {enabled:false, lastAttack:0, troopsWave:{}, targetX:0, 

targetY:0, terrainType:null, terrainLevel:0, stats:{numAttacks:0, spoils:{}}};
		var gensel = htmlSelector (generalList(), '', 'id=pbrptGenSel');
		var m = '<DIV class=' + rndClass['tab_title'] + '>'+kWaveTitle+'</div>\
			<DIV id=' + rndStyle['idPrefix'] + 'Status class=' + rndStyle['status_ticker'] + ' 

style="margin-bottom:5px !important">\
			<CENTER><INPUT type=submit value="OnOff" id=' + rndStyle['idPrefix'] + 

'Enable></input></center>\
			<DIV id=' + rndStyle['idPrefix'] + 'Marches style="height:140px; max-height:140px; 

overflow-y:auto;"></div>\
			<DIV id=' + rndStyle['idPrefix'] + 'Feedback class='+ rndStyle['status_feedback'] 

+'></div></div>\
			<DIV class=' + rndStyle['classPrefix'] + 'Input>\
			<DIV style="height:48px;"><B>'+ kTargetCoords +'</b> &nbsp; X:<INPUT id=' + rndStyle

['idPrefix'] + 'X size=1 maxlength=3 type=text value="'+ Data.waves.target.x +'" /> Y:<INPUT id=' + rndStyle

['idPrefix'] + 'Y size=2 maxlength=3 value="'+ Data.waves.target.y +'" type=text/> &nbsp <B>'+ kDistance1 +'</b> 

<SPAN id=' + rndStyle['idPrefix'] + 'Dist></span><BR>\
			<DIV class=' + rndStyle['status_ticker'] + ' style="margin:0px 10px !

important"><CENTER><SPAN id=' + rndStyle['idPrefix'] + 'Tile></span></center></div></div>\
			<TABLE class=' + rndStyle['classPrefix'] + 'Tab id=' + rndStyle['idPrefix'] + 'Troops><TR 

align=center class=' + rndStyle['classPrefix'] + 'TabHdr1><TD colspan=8>'+ kWaveTroops +'</td></tr></table>\
			<BR><TABLE class=' + rndStyle['classPrefix'] + 'TabPad><TR><TD class=' + rndStyle

['classPrefix'] + 'TabLeft>'+ kDeleteBattleReports +'</td><TD><INPUT id=' + rndStyle['idPrefix'] + 'DBR 

type=checkbox '+ (Data.waves.deleteReports?'CHECKED ':'') +'/></td></tr>\
			<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kStopOnLoss +'</td><TD><INPUT id=' 

+ rndStyle['idPrefix'] + 'STL type=checkbox '+ (Data.waves.stopOnLoss?'CHECKED ':'') +'/></td></tr>\
			<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kDelayBetweenAttacks 

+'</td><TD><INPUT id=' + rndStyle['idPrefix'] + 'Delay type=text size=1 maxlength=4 value="'+ 

Data.waves.iterationMin +'" \> to <SPAN id=' + rndStyle['idPrefix'] + 'DelMax>'+ Data.waves.iterationMax 

+'</span>'+ kSeconds +'</td></tr></table></div>\
			<DIV class=' + rndStyle['status_ticker'] + ' style="margin-top:10px !important">\
			<CENTER><INPUT class=' + rndClass['green_button'] + ' id=' + rndStyle['idPrefix'] + 

'ResStat type=submit value='+ kResetStats +' /></center>\
			<DIV id=' + rndStyle['idPrefix'] + 'Stats  style="height:200px; max-height:200px; overflow

-y:auto"></div>\
			<HR class=thin><DIV id=' + rndStyle['idPrefix'] + 'CurSpoil> &nbsp; </div></div>';
    t.cont.innerHTML = m;
    document.getElementById(rndStyle['idPrefix'] + 'Enable').addEventListener ('click', function(){t.setWaveEnable

(!Data.waves.enabled)}, false);
    ...
    document.getElementById(rndStyle['idPrefix'] + 'DBR').addEventListener ('click', function(e)

{Data.waves.deleteReports=e.target.checked}, false);
    document.getElementById(rndStyle['idPrefix'] + 'STL').addEventListener ('click', function(e)

{Data.waves.stopOnLoss=e.target.checked}, false);
  ...
  gotBattleReport : function (rpt){
    ...
    if (rpt.report.location.x==Data.waves.target.x && rpt.report.location.y==Data.waves.target.y){
      ...
      for (var i=0; i<rpt.report.spoils.items.length; i++){
        ...
        document.getElementById(rndStyle['idPrefix'] + 'CurSpoil').innerHTML = new Date().toTimeString().substring 

(0,8) + kGot + Names.itemAbbr(rpt.report.spoils.items[i]);
      }
      t.dispStats();
  ...
  dispStats : function (){
    ...
    var msg = '<TABLE class=' + rndStyle['classPrefix'] + 'TabPad width=100%><TR><TD class=' + rndStyle

['classPrefix'] + 'TabLeft>'+ kRunTime +'</td><TD width=90%>'+ timestr(runTime, true) +'</td></tr>\
        <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kAttack2 +'</td><TD>'+ Data.waves.numAttacks 
...
+'</td></tr>\
    ...
    for (var p in  Data.waves.target.stats.spoils){
      ...
      msg += '<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ item +':</td><TD>'+ num +' ('+ 

perHour.toFixed(2) +' per hour)</td></tr>';
    }
  ...
  onUnload : function (){
	...
	waveAttackTick : function () {
		...
		if (!Data.waves.enabled)
			return;
		targetMsg =  'a level ' + Data.waves.target.level + ' ' + Data.waves.target.type + ' at ' + 

Data.waves.target.x +'/'+ Data.waves.target.y;
		retryDelay = Math.floor(Math.random() * (5 - 7 + 1) + 7);	
		if (Ajax.marchBusy > 0) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Wave attack to ' + targetMsg + ' delayed due to pending march request: 
...
retry in ' + retryDelay + ' seconds');
		...
		if (getMusterPointSlots(0) <= 0) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Wave attack to ' + targetMsg + ' delayed due to insufficent march slots: 
...
retry in ' + retryDelay + ' seconds');
		...
		if (waveGeneral === null) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Wave attack to ' + targetMsg + ' delayed due to insufficent generals: 
...
retry in ' + retryDelay + ' seconds');
		...
		if (waveUnits !== null) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Wave attack to ' + targetMsg + ' delayed due to ' + waveUnits +': retry 

in ' + retryDelay + ' seconds');
			}
		...
		new Ajax.march (Seed.cities[0].id, Data.waves.target.x, Data.waves.target.y, waveGeneral.id, 

Data.waves.target.troopsWave, 'wave', function (rslt) {
			var t = Tabs.Waves, waveDelay, retryDelay;
			if (rslt.ok && rslt.dat.result.success) {
				t.attackErrors = 0;
				waveDelay = Math.floor(Math.random() * (Data.waves.iterationMax - 

Data.waves.iterationMin + 1) + Data.waves.iterationMin);
				if (Data.options.verboseLog.enabled) {
				...
				if (Data.options.verboseLog.enabled) {
					actionLog('Wave attack to ' + targetMsg + ' failed and returned error: ' + 

rslt.errmsg + ' - retrying in ' + retryDelay  + ' seconds');
				} else {
  ...
  eventCoords : function (e){
    ...
    document.getElementById(rndStyle['idPrefix'] + 'Dist').innerHTML = distance(Seed.cities[0].x, Seed.cities[0].y, 
...
x, y);
    ...
    function callback (rslt){
      ...
      document.getElementById(rndStyle['idPrefix'] + 'Tile').innerHTML = '<B>'+ Map.names[tile.type] +' level '+ 
...
var Data = {
	...
	onUnload : function () {
		...
		for (var i=0; i<t.names.length; i++)
			localStorage.setItem(getServerId() + '_' + C.attrs.userId + '_' + t.names[i], 

JSON.stringify(t[t.names[i]]));
	},
...
Tabs.AutoAttack = {
  ...
  MAX_DISTANCE  : 35,
  curRunStart   : 0,
  contentType   : 0, // 0 = levels, 1 = config, 2 = targets, 3 = stats, 4 = mapTypes these should be enums but 

Javascript doesn't support that type
  ...
  addMarch : function (job){
    ...
    if (march == null){
      ...
      if (DEBUG_MARCHES) WinLog.write ('Tabs.AutoAttack.addMarch: ID='+ march.id +'  ('+ march.x +','+ march.y +') 

General:'+ march.general.id);    
    }
  },
...
(Data.options.objMarches[p], 6, 1));    
  ...
  showStats : function (){
    ...
    var m = '<TABLE class=' + rndStyle['classPrefix'] + 'TabPad> <TR><TD class=' + rndStyle['classPrefix'] + 

'TabLeft>'+ kStatsStarted +'</td><TD>'+  new Date(Data.options.objStats.tsStart * 1000).myString() +'</td></tr>\
    <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kRunTime +'</td><TD>'+ timestr(runTime, true) 

+'</td></tr>\
    <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kAttack2 +'</td><TD>'+ Data.options.objStats.numAttacks 

+'</td></tr>\
    <TR valign=top><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kResources +'</td><TD><TABLE class=' + 

rndStyle['classPrefix'] + 'TabPad>';
    for (var p in Data.options.objStats.resources){
      var perHour = Data.options.objStats.resources[p] / trueRunTime;
      m += '<TR align=right><TD>'+ p +':</td><TD>'+ addCommasInt(Data.options.objStats.resources[p]) +'</td><TD>('+ 

addCommasInt(perHour) +' /hr)</td></tr>';
    }
    m += '</table></td></tr></table>';
    
    m += '<BR><DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ kStatsBy + Data.options.mapChoice + kLevel1 

+'</div><DIV style="overflow:auto"><TABLE class=' + rndStyle['classPrefix'] + 'TabPad><TR class=' + rndStyle

['classPrefix'] + 'TabHdr1 align=center><TD style="background:none !important;"></td><TD align=right colspan=10>'+ 

titleLine(kLevels) +'</td></tr><TR align=right class=' + rndStyle['classPrefix'] + 'TabHdr1><TD 

style="background:none !important;"></td>';
    for (i=1; i<11; i++)
      m += '<TD width=45>'+ i +'</td>';
    m += '</tr><TR><TD colspan=11><HR class=thin></td></tr><TR align=right><TD class=' + rndStyle['classPrefix'] + 
...
//*** Attacks Tab - Stats Sub-tab ***
  tabStats : function (){
    ...
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+kAttackStatsTitle+'</div>\
      <CENTER><INPUT class=' + rndClass['green_button'] + ' id=pmcampRS type=submit value='+ kClearStats +' 

\></center>\
      <DIV class=' + rndStyle['status_ticker'] + ' id=pbcampSO></div>';
    document.getElementById('pbatcon').innerHTML = m;
    document.getElementById('pmcampRS').addEventListener('click', function(){t.clearStats(); t.showStats();}, 
...
false);
  ...
  clearStats : function (){
	...
	checkMapData : function () {
		...
		if (t.checkMapBusy)
			return false;
		if (Data.targets.radius!=35 || Data.targets.center.x!=Seed.cities[0].x || Data.targets.center.y!

=Seed.cities[0].y) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Stored coords are ' + Data.targets.center.x + '/' + 

Data.targets.center.y);
				actionLog('Reported coords are ' + Seed.cities[0].x + '/' + Seed.cities[0].y);
			}
			Seed.fetchSeed(function (rslt) {
				if (rslt.ok) {
					if (Data.options.verboseLog.enabled) {
						actionLog('Player data was successfully requested from the 

server');
						actionLog('Stored coords are ' + Data.targets.center.x + '/' + 

Data.targets.center.y);
						actionLog('Updated coords are ' + Seed.cities[0].x + '/' + 

Seed.cities[0].y);
					}
					if (Data.targets.radius!=35 || Data.targets.center.x!=Seed.cities[0].x || 

Data.targets.center.y!=Seed.cities[0].y) {
						if (Data.options.verboseLog.enabled)
							actionLog('Rescanning map due to a coordinate mismatch');
						t.checkMapBusy = true;
						t.setAttackEnable (false);
						t.scanMap(35, function(){logit('****** Setting checkMapBusy to 

FALSE'); Tabs.AutoAttack.checkMapBusy = false});
						return false;
					}
				} else {
					if (Data.options.verboseLog.enabled)
						actionLog('The following error occured when updating Player data: ' 
...
Data.waves.target.x +'/'+ Data.waves.target.y;
		...
		if (Ajax.marchBusy > 0) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Attack to ' + targetMsg + ' delayed due to pending march request: retry 
...
in ' + retryDelay + ' seconds');
		...
		if (marchCount >= Data.options.objAttack.maxMarches) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Attack to ' + targetMsg + ' delayed due to march limit reached: retry in 
...
' + retryDelay + ' seconds');
		...
		if (getMusterPointSlots(0) <= 0) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Attack to ' + targetMsg + ' delayed due to insufficent march slots: 
...
retry in ' + retryDelay + ' seconds');
		...
		if (attackGeneral === null) {
			if (Data.options.verboseLog.enabled) {
				actionLog('Attack to ' + targetMsg + ' delayed due to insufficent generals: retry 
...
in ' + retryDelay + ' seconds');
		...
		if (t.checkTroops(0, nextAttackTarget.lvl) === null) {
			t.sendAttack(0, nextAttackTarget, attackGeneral, function (rslt) {
				...
				if (rslt) {
					attackDelay = Math.floor(Math.random() * (Data.options.objAttack.delayMax - 

Data.options.objAttack.delayMin + 1) + Data.options.objAttack.delayMin);
			...
			if (Data.options.verboseLog.enabled) {
				actionLog('Attack to ' + targetMsg + ' delayed due to insufficient troops: retry in 
...
' + retryDelay + ' seconds');
	...
	sendAttack : function (cityIdx, mapObject, gen, notify) {
		...
		var targMsg =  kAttackSent + ' ' + mapObject.lvl + ' ' + Data.options.mapChoice + ' ' + kAt + ' ' + 
...
mapObject.x +'/'+ mapObject.y;
		...
		t.attackBusy = true;
		t.lastAttack = now;
		new Ajax.march (Seed.cities[cityIdx].id, mapObject.x, mapObject.y, gen.id, 

Data.options.objAttack.troops[mapObject.lvl], 'camp', function (rslt) {
			t.attackBusy = false;
			if (rslt.ok && rslt.dat.result.success) {
				t.attackErrors = 0;
				if (Data.options.verboseLog.enabled) {
  ...
  // return the mapObject that is next to be attacked, if we are at the last object in the last, return the first 
...

{
    ...
    // This is complicated by the fact that the last attacked target in the list may not be the last physical 

entry, just the one that fits
...

{
    ...
    if (mapObject == lastMatchingTarget) {
        for (var k=0; j<objs.length; k++) {
            targetObj = objs[k];
            if (targetObj.attackable) {
                if ( (targetObj.dist <= defaultObj.levelDist[targetObj.lvl]) && defaultObj.levelEnable
...
[targetObj.lvl]) {
  ...
  getActiveObjectList : function (){
    ...
    for (var i=0; i<Data.targets.mapObjects.length; i++){
      ...
      if ((mapObject.dist<=Data.options.objAttack.levelDist[mapObject.lvl]) && Data.options.objAttack.levelEnable
...
[mapObject.lvl])
  ...
  checkAttack : function (mapObject, notify){
    ...
    var targMsg =  kManualAttackSent + ' ' + mapObject.lvl + ' ' + Data.options.mapChoice + ' ' + kAt + ' ' + 
...
levelDist:[]}
  ...
  troopTypes : [kPorter, kConscript, kSpy, kHalberdsman, kMinotaur, kLongbowman, kSwiftStrikeDragon, kBattleDragon, 

kArmoredTransport, kGiant, kFireMirror, kAquaTroop, kStoneTroop, kFireTroop, kWindTroop, kGreatDragon, 

kWaterDragon, kStoneDragon, kFireDragon, kWindDragon],
  tabConfigLevels : function (){
    ...
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+ kAutoAttack + Data.options.mapChoice + kLevels +'</div>\
        ...
        <TABLE class=' + rndStyle['classPrefix'] + 'TabPad><TR class=' + rndStyle['classPrefix'] + 'TabHdr1><TD 

style="background:none !important;"></td><TD align=center colspan=10>'+ titleLine(kLevels) +'</td></tr>\
        <TR align=center class=' + rndStyle['classPrefix'] + 'TabHdr1><TD style="background:none !

important;"></td><TD>1</td><TD>2</td><TD>3</td><TD>4</td><TD>5</td><TD>6</td><TD>7</td><TD>8</td><TD>9</td><TD>10</

td></tr>\
        </div><TR align=center><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kEnable +'</td>';
    for (var x=1; x<=10; x++)
      m += '<TD><INPUT type=checkbox id=pbatEn_'+ x +(Data.options.objAttack.levelEnable[x]?' CHECKED':'')   +' 

\></td>';
    m += '</tr><TR align=center><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kMaxDist +'</td>';
    for (var x=1; x<=10; x++)
      m += '<TD><INPUT type=text id=pbatDist_'+ x +' maxlength=2 style="width:34px" value="'+ 

Data.options.objAttack.levelDist[x] +'"\></td>';
    m += '</tr><TR><TD><DIV class=short></td></tr>';
    
    for (i=0; i<t.troopTypes.length; i++){
      m += '<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ Names.troops.byName[t.troopTypes[i]][2] 
...
+':</td>';
      for (var x=1; x<=10; x++){
        ...
        if (!num)
          num = 0;
        m += '<TD><INPUT type=text id=pbatTrp_'+ x +'_'+ i +' maxlength=6 size=2  style="width:34px" value="'+ num 
...
+'\
      <INPUT class=short id=pbaacfgRD2 maxlength=4 type=text value="'+ Data.options.objAttack.delayMax +'"/>'+ 

kSeconds +'</td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kSameTargetDelay +'</td><TD>'+ kOneHour +'</td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kLogAttacks +'</td><TD><INPUT id=pbaacfgLA '+ 

(Data.options.objAttack.logAttacks?'CHECKED ':'') +' type=checkbox \></td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kDeleteMarchReports +'</td><TD><INPUT id=pbaacfgDMR 

'+ (Data.options.objAttack.deleteObjAttacks?'CHECKED ':'') +' type=checkbox \></td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kStopOnLoss +'</td><TD><INPUT id=pbaacfgSTL '+ 

(Data.options.objAttack.stopAttackOnLoss?'CHECKED ':'') +' type=checkbox \></td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kMaxMarches +'</td><TD><INPUT class=short 

id=pbaacfgMM maxlength=2 type=text value="'+ Data.options.objAttack.maxMarches +'"\></td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kClearLast +'</td><TD><INPUT class=' + rndClass

['green_button'] + ' type=submit id=pbaacfgClr value='+ kClear +'></td></tr>\
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kClearAll +'</td><TD><INPUT id=pbaacfgCA '+ 

(Data.options.objAttack.clearAllTargets?'CHECKED ':'') +' type=checkbox \></td></tr>\
      </table>';
    document.getElementById('pbatcon').innerHTML = m;
    
    // Add event listeners
    document.getElementById('pbaacfgDMR').addEventListener('change', function (e)

{Data.options.objAttack.deleteObjAttacks = e.target.checked;}, false);
    document.getElementById('pbaacfgSTL').addEventListener('change', function (e)

{Data.options.objAttack.stopAttackOnLoss = e.target.checked;}, false);
    document.getElementById('pbaacfgLA').addEventListener('change', function (e){Data.options.objAttack.logAttacks 

= e.target.checked;}, false);
    document.getElementById('pbaacfgCA').addEventListener('change', function (e)

{Data.options.objAttack.clearAllTargets = e.target.checked;}, false);
    ...
    function delayChanged (e){
      ...
      if (min<MIN_DELAY || min>3600 || (max-min)<5){
        var dial = new ModalDialog (t.cont, 300, 150, '', true);
        dial.getContentDiv().innerHTML = '<B>'+ kInvalidDelay +'</b><BR><BR>'+ kFirstValue + MIN_DELAY + 
...
//*** Attacks Tab - Targets Sub-Tab ***
  tabTargets : function (){
    ...
    var timer = null;
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+ kAutoAttack + Data.options.mapChoice + kLevels 

+'</div><TABLE id=pbatTargTab class=' + rndStyle['classPrefix'] + 'Tab><TR class=' + rndStyle['classPrefix'] + 

'TabHdr2><TD>'+ kDistance +'</td><TD>'+ kCoords +'</td><TD>'+ kLevel +'</td><TD width=65>'+ kLastAttack 
...
//logit (inspect (Data.targets.mapObjects, 5, 1));
    ...
    for (var i=0; i<mapObjects.length; i++){
        m += '<TR id=pbatRow_'+ i +'><TD>'+ mapObjects[i].dist +'</td><TD align=center>'+ mapObjects[i].x +','+ 

mapObjects[i].y +'</td><TD align=center>'+ mapObjects[i].lvl +'</td>\
              <TD><span id=pbatList_'+ i +'> --- </span></td><TD><INPUT class=small id=pbattargAN_'+ i +' 

type=submit value=' + kAttackNow + '\></td>';
        
        // Add the skip attack button for cities and outposts
        if (t.selectedMapName == kCity || t.selectedMapName == kOutpost)
            m += '<TD><INPUT class=small id=pbskipAN_'+ i +' type=submit value=' + kSkipAttack + '\></td><TD>'+ 

mapObjects[i].playerAlliance +'</td>';
        ...
        for (var j=0;j<ownedWilderness.length; j++) {
            if (ownedWilderness[j].x == mapObjects[i].x && ownedWilderness[j].y == mapObjects[i].y) {
                document.getElementById('pbatRow_'+j).setAttribute("class", "' + rndStyle['classPrefix'] + 
...
'Owned");
    ...
    document.getElementById('pbatcon').scrollTop = gAttScrollPos;
     ...
     for (var i=0; i<mapObjects.length; i++)
        for (var j=0;j<ownedWilderness.length; j++) {
            if (ownedWilderness[j].x == mapObjects[i].x && ownedWilderness[j].y == mapObjects[i].y) {
                document.getElementById('pbatRow_'+i).setAttribute("class", "' + rndStyle['classPrefix'] + 
  ...
  scanMap : function (radius, notify){
    ...
    function callback (dat){
      ...
      for (var i=0; i<dat.tiles.length; i++){
        ...
        if (tile.type == 'A')
          Data.targets.mapObjects.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, last:null, fromCity:0, 
...
//*** Attacks Tab - Maps Sub-tab ***
  tabMaps : function(){
    ...
    var m = '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ kMapCategories +'</div>\
        ...
        <TR align=center class=' + rndStyle['classPrefix'] + 'TabHdr1><TD style="background:none !important;" 
...
colspan=2></td></tr>\
    ...
    m += '<TR><TD colspan="2"><INPUT class=' + rndClass['green_button'] + ' type=submit id=pbatMapSearch value='+ 
...
kSearch +'></td></tr>'; 
    ...
    function butSearchNow (e){
        ...
        var radius = 35;
        var t = Tabs.AutoAttack;
        Data.targets = {radius:0, center:{x:Seed.cities[0].x, y:Seed.cities[0].y}, mapObjects:[], camps:[], 

cities:[], outposts:[], grasslands:[], swamps:[], lakes:[], hills:[], plains:[], mountains:[], forests:[]};
        var dial = new ModalDialog (t.cont, 300, 165, '', false, null);
        ...
        function callback (dat){
            ...
            for (var i=0; i<dat.tiles.length; i++){
                ...
                switch (tile.type){
                    case 'A': Data.targets.camps.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'C': Data.targets.cities.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

cityName:tile.cityName, cityLvl:tile.cityLvl, playerName:tile.playerName, playerLvl:tile.playerLvl, 

playerMight:tile.playerMight, playerAlliance:tile.playerAlliance, last:null, fromCity:0, attackable:

(tile.playerAlliance == '---')}); break;
                    case 'O': Data.targets.outposts.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 
...
(tile.playerAlliance == '---')}); break;
                    case 'G': Data.targets.grasslands.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'B': Data.targets.swamps.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'L': Data.targets.lakes.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'H': Data.targets.hills.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'P': Data.targets.plains.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'M': Data.targets.mountains.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 

last:null, fromCity:0, attackable:true}); break;
                    case 'F': Data.targets.forests.push ({x:tile.x, y:tile.y, dist:tile.dist, lvl:tile.lvl, 
...
last:null, fromCity:0, attackable:true}); break;
            ...
            if (dat.done){
                ...
                //   Data.targets.hills.length + Data.targets.plains.length + Data.targets.mountains.length + 

Data.targets.forests.length;
    ...
    function enableChanged (e){
        ...
        if (Data.options.objAttack.enabled) {
            t.setAttackEnable(false); // It would be very bad to leave attack on when switching targets. Imagine 

sending the troops for a wilderness to a city or an ant camp...
  ...
  for (var c=0; c<Seed.cities.length; c++){
    ...
    for (var j=0; j<jobs.length; j++){
        ...
        if (jobs[j].queue == 'units' && jobs[j].unit_type)
            m += '<TR class='+ mtClass +'><TD>'+ kTraining +'</td><TD>'+ jobs[j].quantity +'</td><TD>'+ jobs

[j].unit_type +'</td><TD><TD>'+ time +'</td></tr>';   
    }
  }
  return m + '</table>';
}
...
function marchTable (myId){
  ...
  for (var p in Seed.marches){
    ...
    m += '<TR class='+ mtClass +'><TD>'+ kAttack1 + march.x +','+ march.y +'</td><TD>('+ march.target +'-'+ 

march.terrain_level +')</td><TD>'+ march.status +'<TD>'+ time +'</td></tr>';
  }
  return m +'</table>';
}
...
var Ajax = {
  ...
  messageList : function (cat, callback){
    if (!cat)
      cat = 'all';
    new MyAjaxRequest ('reports.json', {'user%5Fid':C.attrs.userId, 'dragon%5Fheart':C.attrs.dragonHeart, count:12, 

timestamp:parseInt(serverTime()), '%5Fsession%5Fid':C.attrs.sessionId, category:cat, page:1, version:getVersion }, 
...
mycb, false);
  ...
  messageDetail : function (id, callback){
    new MyAjaxRequest ('reports/'+ id +'.json', {'user%5Fid':C.attrs.userId, timestamp:parseInt(serverTime()), '%

5Fsession%5Fid':C.attrs.sessionId, version:getVersion, 'dragon%5Fheart':C.attrs.dragonHeart }, mycb, false);
    function mycb (rslt){
      if (rslt.ok && rslt.dat.result.success){
        if (callback)
...
var AutoCollect = {
	...
	setEnable : function (onOff){
		...
		if (onOff){
			var time = (Data.options.autoCollect.delay*Data.options.autoCollect.unit) - serverTime() + 

Data.options.autoCollect.lastTime;
			if (time <= 0)
				t.doit ();
			else
	...
	doit : function (){
		...
		t.timer = setTimeout (t.doit, ((Data.options.autoCollect.delay*Data.options.autoCollect.unit) + 
...
Tabs.Info = {
  ...
  init : function (div){
    var t = Tabs.Info;
    t.cont = div;
    div.innerHTML = '<DIV class=' + rndClass['tab_title'] + '>'+ kDOAVersionString + Version +'<BR>'+ WebSite 

+'</div>\
      <TABLE width=100%><TR><TD><INPUT class=' + rndClass['green_button'] + ' type=submit value='+ kRefresh +' 

id=pbinfRefresh></input>\
  ...
  hide : function (){
    ...
    showStuff : function (){
        ...
        m += '<TABLE style="margin-top:3px" width=100%>\
              <TR bgcolor=#dde align=center><TD style="border-right: 1px solid; border-bottom: 1px solid;"><B>'+ 
...
kTroops +'</b></td>\
        ...
        for (var i=0; i<Names.troops.names.length; i++){
            ...
            if (name==kGreatDragon || name==kWaterDragon || name==kStoneDragon || name==kFireDragon || 

name==kWindDragon) continue;
            ...
            m += '<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ name +':</td><TD align=right>'+ num 

+'</td><TD align=right>'+ marchStr +'</td></tr>';
        }
        ...
        for (var ig=0; ig<city.generals.length; ig++){
            ...
            m += '<TR><TD align=right>'+ city.generals[ig].name +' ('+ city.generals[ig].rank +')</td><TD 

width=75%>'+ (city.generals[ig].busy?loc:'') +'</td></tr>';
        }
        
        m += '</table></td></tr></table><BR><TABLE class=' + rndStyle['classPrefix'] + 'TabPad>\
              <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kMarches +'</td><TD>'+ Seed.numMarches 
...
//        + dispBuildingJob(0) + dispResearchJob(0) + dispTrainingJobs(0) + '</td></tr></table>';
        ...
        //        m += '<DIV class=short></div>'+ cityTitle(i) + '<TABLE class=' + rndStyle['classPrefix'] + 

'TabPad><TR><TD></td></tr></table>'
//      //    + dispBuildingJob(i) + dispTrainingJobs(i) + '</td></tr></table>';
        ...
        function cityTitle (cityIdx){
            ...
            if (cityIdx == 0)
                wallStatus = (Seed.cities[cityIdx].defended) ? '<font class="defending">'+ kDefending +'</font>' : 

'<font class="hiding">'+ kSanctuary +'</font>';
            else
                wallStatus = '<font class="defending">'+ kDefending +'</font>';
      
			return '<div class=' + rndStyle['classPrefix'] + 'Subtitle><TABLE class=' + rndStyle

['classPrefix'] + 'Tab><TR><TD align=left>'+ city.name +'</td><TD align=center>'+ city.x +','+ city.y + '</td><TD 

align=center width=200px><font color=yellow>' + alliance_name +'</font></td><TD width=80px align=right>'+ 

wallStatus +'</td></tr></table></div>';
        }
    },
...
Tabs.Debug = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.mouseElement = div;
    div.innerHTML = '<TEXTAREA id=pbdbgUnTxt row=3 cols=50></textarea><INPUT type=submit value="unescape" 

id=pbdbgUn></input><BR><BR>\
      ...
      <DIV style="background-color:#eee; margin:5px"><CENTER><INPUT style="width:130px" class=butAttackOff 

id=pbdbgTMonoff type=submit value="Track Mouse Off"><BR><DIV id=pbdbgCoords>&nbsp;</div></center></div>\
    ...
    document.getElementById('pbdbgResetMR').addEventListener ('click', function(){Data.options.messages.missing=0; 

t.showMissingReports()}, false);
  ...
  dispScripts : function (){
    ...
    for (var i=0; i<scripts.length; i++){
      ...
      if (code == undefined)
        ...
        m += 'Source: '+ scripts[i].src +'<BR>Length: '+ code.length +'<BR>'+ code.substr(0,1000).htmlEntities() 
...
+'<BR><HR>';
  ...
  seedMarches : function (){
    ...
    for (var p in Seed.marches){
      ...
      if (status == 'returning')
        status = 'returning ';
      msg += 'OWNER: '+  march.ownerId +' ID: '+ march.id +' '+ status +' '+ march.x +','+ march.y +' '+ 

march.run_at +'('+ (march.run_at-now)  +') '+ march.duration +'\n';
    }
    logit (msg);
  },
...
//  mouseElement = document.getElementById('castlemania_swf_container');
  ...
  moveHandler : function (me){
    var t = Tabs.Debug;
    t.mouseDispDiv.innerHTML = 'Client: '+ me.clientX +','+ me.clientY +' &nbsp; Screen: '+ me.screenX +','+ 
...
//    t.createMouseClick (t.mouseElement, 874,280,803,183);
  ...
  createMouseClick : function (e, screenX, screenY, clientX, clientY){
    ...
    evObj.initMouseEvent( 'click', true, cancellable, window, 1, screenX, screenY, clientX, clientY, false, false, 
...
C.attrs:   
  ...
  (array) playerGeneralFacebookIds = 

511766946,531413843,1408508145,1583521095,1630864998,1641056237,100000233332372,100000629563828
  (boolean) isFan = false
  (boolean) hasExtPerms = false
  (array) appFriendIds = 

1400526627,511766946,513290679,517317030,587021483,594053295,607773106,628608909,685297360,769038534,829965606,1266

963775,1311343917,1361197746,1383471858,1408508145,1408906378,1432144775,1630864998,1641056237,1655875848,166133321

9,100000232672930,100000475977050,100000569950692,100000583783925,100000629563828
  (array) nonAppFriends = .................
...
//    researchTimer   : null,
    ...
    trainTimer      : null,
    trainStatTimer  : null,
    capitolTroops   : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'SwiftStrikeDragon', 

'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror'],
    outpost1Troops  : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'SwiftStrikeDragon', 

'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror', 'AquaTroop'],
    outpost2Troops  : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'SwiftStrikeDragon', 

'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror', 'StoneTroop'],
    outpost3Troops  : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'SwiftStrikeDragon', 

'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror', 'FireTroop'],
	outpost4Troops  : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 

'SwiftStrikeDragon', 'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror', 'WindTroop'],
    allTroops       : ['Porter', 'Conscript', 'Spy', 'Halberdsman', 'Minotaur', 'Longbowman', 'SwiftStrikeDragon', 

'BattleDragon', 'ArmoredTransport', 'Giant', 'FireMirror', 'AquaTroop', 'StoneTroop', 'FireTroop', 'WindTroop'],
    selectedQ       : kMinHousing,
    trainJobs       : [],
    capitolResearch : {Agriculture:'Agriculture', Woodcraft:'Woodcraft', Masonry:'Masonry', Mining:'Alloys', 

Clairvoyance:'Clairvoyance', RapidDeployment:'Rapid Deployment', Ballistics:'Weapons Calibration', 

Metallurgy:'Metallurgy', Medicine:'Medicine', Dragonry:'Dragonry', Levitation:'Levitation', 

Mercantilism:'Mercantilism', AerialCombat:'Aerial Combat'},
    researchIdx     : {Agriculture:0, Woodcraft:1, Masonry:2, Mining:3, Clairvoyance:4, RapidDeployment:5, 

Ballistics:6, Metallurgy:7, Medicine:8, Dragonry:9, Levitation:10, Mercantilism:11, AerialCombat:12},
    capitolCity     : [kHome, kGarrison, kScienceCenter, kMetalsmith, kOfficerQuarter, kMusterPoint, kRookery, 

kStorageVault, kTheater, kSentinel, kFactory, kFortress, kDragonKeep, kWall],
	...
	capitalResearch : ['Agriculture', 'Woodcraft', 'Masonry', 'Alloys', 'Clairvoyance', 'Rapid Deployment', 

'Weapons Calibration', 'Metallurgy', 'Medicine', 'Dragonry', 'Levitation', 'Mercantilism', 'Aerial Combat'],
    contentType     : 0, // 0 = info, 1 = train, 2 = build, 3 = research, these should be enums but Javascript 
...
doesn't support that type
	...
	init : function (div){
		...
		div.innerHTML =  '<TABLE width=100% bgcolor=#ffffd0 align=center><TR><TD>\
			<INPUT class=button type=submit value="Job Info" id=' + rndStyle['subtab_info'] + 
...
'></INPUT>\
			...
			<INPUT class=button type=submit value="Research" id=' + rndStyle['subtab_research'] + 

'></INPUT>\
			</TD></TR></TABLE>\
			<DIV id=' + rndStyle['job_header'] + ' style="padding-top:5px; height:260px; max-

height:260px;"></div>\
			<DIV id=' + rndStyle['job_content'] + ' style="padding-top:5px; height:435px; max-

height:800px; overflow-y:auto;"></div>';
		...
		document.getElementById(rndStyle['subtab_research']).addEventListener ('click', t.tabJobResearch, 
...
false);
	...
	tabJobInfo : function (){
		...
		var m = '<TABLE class=' + rndStyle['classPrefix'] + 'TabPad>' + dispBuildingJob(0) + 

dispResearchJob(0) + dispTrainingJobs(0) + '</td></tr></table>';
  
		// outposts ...
		if (Seed.cities.length > 0){
			for (var i=1; i<Seed.cities.length; i++){
				m += '<DIV class=short></div>'+ cityTitle(i) + '<TABLE class=' + rndStyle

['classPrefix'] + 'TabPad>' + dispBuildingJob(i) + dispTrainingJobs(i) + '</td></tr></table>';
			}
		}
		...
		function dispBuildingJob(cityIdx) {
            ...
            // Don't show negative values - not pleasant user interface. To be truly nice, if the time is less than 

zero, we should reset 
            // the build timer. For now, that is done by the Build tab's notification process
                m += '<TD>'+ job.building.type +' level '+ job.job.level +'</td><TD>'+ timestr(job.job.run_at - 

serverTime(), true) +'</td></tr>';
            }
             else
		...
		function dispResearchJob (cityIdx){
            ...
            if (job && job.run_at > serverTime())
                m += '<TD>'+ job.research_type +' level '+ job.level +'</td><TD>'+ timestr(job.run_at - serverTime
...
(), true) +'</td></tr>';
		...
		function dispTrainingJobs (cityIdx){
            ...
            for (var i=0; i<Seed.cities[cityIdx].jobs.length; i++)
                if (Seed.cities[cityIdx].jobs[i].queue=='units' && Seed.cities[cityIdx].jobs[i].unit_type && 
...
Seed.cities[cityIdx].jobs[i].run_at > last)
            ...
            for (var i=0; i<trains.length; i++){
                ...
                m += '<TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ left +'</td><TD>'+ trains[i].quantity 

+' '+ trains[i].unit_type +' </td><TD> '
...
'<font class="hiding">'+ kSanctuary +'</font>';
            ...
            return '<div class=' + rndStyle['classPrefix'] + 'Subtitle><TABLE class=' + rndStyle['classPrefix'] + 

'Tab><TR><TD align=left>'+ city.name +'</td><TD align=center>'+ city.x +','+ city.y + '</td><TD align=center 

width=200px><font color=yellow>' + alliance_name +'</font></td><TD width=80px align=right>'+ wallStatus 
	...
	tabJobTrain : function (){
	   ...
	   // Create status ticker
        var n = '<DIV class=' + rndClass['tab_title'] + '>'+ kAutoTrain +'</div>\
                 <DIV class=' + rndStyle['status_ticker'] + ' style="margin-bottom: 5px !important"><CENTER><INPUT 

id=pbtrnOnOff type=submit\></center>\
                 ...
                 <INPUT class=button type=submit value='+ kConfig +' 

id=pbttConfigTrain></input></td></tr></table>';
		document.getElementById(rndStyle['job_header']).style.height = "258px";
        document.getElementById(rndStyle['job_header']).innerHTML = n;
        
        var m = '<DIV id=pbtrnConfig style="padding-top: 5px; overflow:auto;"class=' + rndStyle['classPrefix'] + 
...
'Input>';
 	  ...
 	  // Add event listener for auto on/off button
        document.getElementById('pbtrnOnOff').addEventListener ('click', function (){ t.setTrainEnable (!
...
Data.options.autoTrain.enabled);}, false);
	...
	tabJobBuild : function (){
        ...
        for (var i=0; i<Seed.cities.length; i++){
            ...
            // The seed object contains a wealth of information including alliance membership, number of people in 

the alliance, facebook ids of each member,
            // the ol's information (in alliances and alliance_membership), the s object contains all the buildings 

for the cities, whether or not the city is
            // on defense, the list of generals, what and where the dragon is, a list of jobs (e.g. research, 

building, troops training and pending training, current marches)
            // the marches alone say where the troops are, whether or not they are returning or attacking, general 

assigned, etc.
            var city = Seed.cities[i];
            m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ kCityNumber + (i+1) +' ('+ city.type +')

</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle

['classPrefix'] + 'Tab>';
            for (var ii=0; ii<listC.length; ii++){
                m += '<TR><TD><INPUT type=checkbox id="pbbldcb_'+ i +'_'+ listC[ii] +'" '+ 

(Data.options.autoBuild.buildingEnable[i][listC[ii]]?'CHECKED':'') +' /></td><TD>'+ listC[ii] +'</td>'+ 

buildDisplayCap(i,ii) +'</tr>';  
                el.push('pbbldcb_'+ i +'_'+ listC[ii]);
            }
            m += '</table></td><TD><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';  
            for (var ii=0; ii<listF.length; ii++){
                m += '<TR><TD><INPUT type=checkbox id="pbbldcb_'+ i +'_'+ listF[ii] +'" '+ 

(Data.options.autoBuild.buildingEnable[i][listF[ii]]?'CHECKED':'') +' /></td><TD>'+ listF[ii] +'</td>'+ 

buildDisplayCap(i,(listC.length + ii)) +'</tr>';  
        ...
        for (var i=0; i<Seed.cities.length; i++) {
            var len = (i==0) ? (t.capitolCity.length + t.capitolField.length) : (t.outpostCity.length + 

t.outpostField.length);
            for (var ii=0;ii<len; ii++) {
                ...
                try {
                    if (!Data.options.autoBuild.buildCap[i][ii]) {
                        ...
                        var lowestBuildingLevel = t.getCurrentLowestBuildingLevel(i, (i==0)? capitolB[ii] : 
...
outpostB[ii]);
        ...
        document.getElementById('pbbldOnOff').addEventListener ('click', function (){t.setBuildEnable (!

Data.options.autoBuild.enabled);}, false);
	...
	tabJobResearch : function (){
        ...
        m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ kCityNumber +'1 ('+ city.type +')</div><TABLE 

class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 

'Tab>';
        var i=0;
        for (var p in t.capitolResearch){
            m += '<TR><TD><INPUT type=checkbox id="pbrescb_'+ 0 + '_' +t.capitolResearch[p] +'" '+ 

(Data.options.autoResearch.researchEnable[0][t.capitolResearch[p]]?'CHECKED':'') +' /></td><TD>'+ 

t.capitolResearch[p] +'</td><TD>'+ researchDisplayCap(i) +'</td></tr>';  
        ...
        document.getElementById('pbresOnOff').addEventListener ('click', function (){t.setResearchEnable (!

Data.options.autoResearch.enabled);}, false);
        ...
        for (var i=0; i<Seed.cities.length; i++){
            ...
            if (job == null)
                ...
                if (timeRemaining == 0) {
                    ...
                    m += kBuilding1 +'</td><TD>'+ kLevel1 +' '+ job.level +' '+ b.type  +'</td><TD>'+ timeRemaining 
    ...
    resStatTick : function (){
        ...
        if (job == null)
            ...
            if (timeRemaining == 0) {
                ...
                m += kResearch +'</td><TD>'+ kLevel1 +' '+ job.level +' '+ t.resUITranslate (job.research_type) 

+'</td><TD>'+ timeRemaining  +'</td></tr>';
    ...
    dispFeedback : function (msg){
        ...
        if (elementId && document.getElementById(elementId))
            if (msg == '')
                ...
                document.getElementById(elementId).innerHTML = new Date().toTimeString().substring (0,8) +' '+  
    ...
    getBuildingCap : function (cityIdx, buildingType){
        ...
        for (var i=0; i < cityType.length; i++) {
            if (cityType[i] == buildingType) {
                try {
                    cap = (Data.options.autoBuild.buildCap[cityIdx][i]) ? Data.options.autoBuild.buildCap[cityIdx]
...
[i] : 0; 
    ...
    getResearchCap : function (researchType){
        ...
        for (var p in resType) {
            ...
            if (p == researchType) {
                try {
                    cap = (Data.options.autoResearch.researchCap[0][t.researchIdx[researchType]]) ? 

Data.options.autoResearch.researchCap[0][t.researchIdx[researchType]] : 0; 
                    break;
                }
                catch (e) {
                }  
    ...
    // This would be simple if only one building of each type existed, but you may build multiple 

garrisons/training camps
    // So we have to look through the entire list and use an additional parameter to specify the building level 
...
needed
    ...
    // Upkeep: 2 food
        ...
        if (ic == 0){ 
            if (t.getBuildingLevel(ic, kGarrison, garrisonLevel) == 0) m += '<TD>&nbsp;garrison '+ garrisonLevel 

+'</td>';
        }
        else if (t.getBuildingLevel(ic, kTrainingCamp, garrisonLevel) == 0) m += '<TD>&nbsp;training camp '+ 
...
garrisonLevel +'</td>';
        ...
        var availablePop = city.figures.population.current - city.figures.population.laborers - 

city.figures.population.armed_forces;
...
city.figures.population.armed_forces;
        ...
        if (availablePop < idlePop) m += '<TD>&nbsp;people ' + (idlePop - availablePop) + '</td>';
        if (t.getRemainingQueue(ic, kUnits) == 0) m += '<TD>&nbsp;training queue</td>';
        if (Seed.s.research.Clairvoyance < clairvoyanceLevel) m += '<TD>&nbsp;Clairvoyance ' + clairvoyanceLevel + 
...
city.figures.population.armed_forces;
        ...
        if (availablePop < idlePop) m += '<TD>&nbsp;people ' + (idlePop - availablePop) + '</td>';
        if (t.getRemainingQueue(ic, kUnits) == 0) m+= '<TD>&nbsp;training queue</td>';
        if (Seed.s.research.WeaponsCalibration < weaponCalibrationLevel) m += '<TD>&nbsp;Weapons Calibration ' + 

weaponCalibrationLevel +'</td>'; 
        if (m.length == 0) {
            ret.trainable = true;
...
city.figures.population.armed_forces;
        ...
        if (Seed.s.research.RapidDeployment < rapidDeploymentLevel) m += '<TD>&nbsp;Rapid Deployment ' + 

rapidDeploymentLevel +'</td>'; 
        if (m.length == 0) {
            ret.trainable = true;
...
garrisonLevel +'</td>';
        if (t.getBuildingLevel(0, kMetalsmith, metalsmithLevel) == 0) m += '<TD>&nbsp;Metalsmith '+ metalsmithLevel 
...
city.figures.population.armed_forces;
        ...
        if (t.getRemainingQueue(ic, kUnits) == 0) m+= '<td>&nbsp;training queue</td>';
        if (Seed.s.research.Metallurgy < metallurgyLevel) m += '<TD>&nbsp;Metallurgy ' + metallurgyLevel +'</td>'; 
        if (Seed.s.research.WeaponsCalibration < weaponsCalibrationLevel) m += '<TD>&nbsp;Weapons Calibration ' + 

weaponsCalibrationLevel +'</td>'; 
        if (m.length == 0) {
            ret.trainable = true;
    ...
    // Upkeep: 125 food
        ...
        // Returns zero or the building level
        if (t.getBuildingLevel(ic, kTrainingCamp, trainingCampLevel) == 0) m += '<TD>&nbsp;training camp '+ 

trainingCampLevel +'</td>';
        if (t.getBuildingLevel(0, kFactory, factoryLevel) == 0) m += '<TD>&nbsp;Factory '+ factoryLevel +'</td>';
        if (t.getBuildingLevel(0, kMetalsmith, metalsmithLevel) == 0) m += '<TD>&nbsp;Metalsmith '+ metalsmithLevel 

+'</td>';
        var availableRespirators = t.getItem(kAquaTroopRespirator);
        if (availableRespirators < respiratorQty) m += '<TD>&nbsp;Respirators '+ (respiratorQty - 

availableRespirators) +'</td>';
        if (city.resources.food < food) m += '<TD>&nbsp;food '+ (food - city.resources.food) +'</td>';
        if (city.resources.wood < lumber) m += '<TD>&nbsp;lumber '+ (lumber - city.resources.wood) +'</td>';
        if (city.resources.ore < metal) m += '<TD>&nbsp;metal '+ (metal - city.resources.ore) +'</td>';
        if (city.resources.stone < stone) m += '<TD>&nbsp;stone '+ (stone - city.resources.stone) +'</td>';
...
+'</td>';
        ...
        if (availableMandrakes < mandrakeQty) m += '<TD>&nbsp;Mandrakes '+ (mandrakeQty - availableMandrakes) 
...
+'</td>'; 
    ...
    checkFireTroopReqs : function (troopQty, ic, count, troopsLength) {
        ...
        // Returns zero or the building level
		if (Seed.s.research.Clairvoyance < clairvoyanceLevel) m += '<TD>&nbsp;Clairvoyance ' + 

clairvoyanceLevel +'</td>';
		if (city.resources.food < food) m += '<TD>&nbsp;food '+ (food - city.resources.food) +'</td>';
        var availablePop = city.figures.population.current - city.figures.population.laborers - 
...
city.figures.population.armed_forces;
        ...
        if (city.resources.wood < lumber) m += '<TD>&nbsp;lumber '+ (lumber - city.resources.wood) +'</td>';
        if (city.resources.ore < metal) m += '<TD>&nbsp;metal '+ (metal - city.resources.ore) +'</td>';
		if (t.getBuildingLevel(0, kMetalsmith, metalsmithLevel) == 0) m += '<TD>&nbsp;Metalsmith '+ 
...
trainingCampLevel +'</td>';
        ...
        if (availableRunes < volcanicRunesQty) m += '<TD>&nbsp;Runes '+ (volcanicRunesQty - availableRunes) 

+'</td>';
		if (Seed.s.research.Ballistics < weaponsCalibrationLevel) m += '<TD>&nbsp;Weapons Calibration ' + 
...
weaponsCalibrationLevel +'</td>'; 
    ...
    checkWindTroopReqs : function (troopQty, ic, count, troopsLength) {
        ...
        // Returns zero or the building level
		if (Seed.s.research.AerialCombat < aerialCombatLevel) m += '<TD>&nbsp;Aerial Combat ' + 

aerialCombatLevel +'</td>';
		if (city.resources.food < food) m += '<TD>&nbsp;food '+ (food - city.resources.food) +'</td>';
        var availablePop = city.figures.population.current - city.figures.population.laborers - 
...
city.figures.population.armed_forces;
        availablePop = (availablePop > 0) ? availablePop : 0;
        if (availablePop < idlePop) m += '<TD>&nbsp;people ' + (idlePop - availablePop) + '</td>';
		if (Seed.s.research.Levitation < levitationLevel) m += '<TD>&nbsp;Leviatation ' + levitationLevel 
...
trainingCampLevel +'</td>';
        ...
        if (availableTalons < bansheeTalonsQty) m += '<TD>&nbsp;Talons '+ (bansheeTalonsQty - availableTalons) 
...
+'</td>';
    ...
    checkAgricultureReqs : function() {
        ...
        if (city.resources.gold < gold) m += '<TD>&nbsp;gold '+ (gold - city.resources.gold) +'</td>';
        if (city.resources.ore < metal) m += '<TD>&nbsp;metal '+ (metal - city.resources.ore) +'</td>';
        if (t.getBuildingLevel(0, 'ScienceCenter', scienceCenterLevel) == 0) m += '<TD>&nbsp;Science Center '+ 

scienceCenterLevel +'</td>';
...
scienceCenterLevel +'</td>';
        if (t.getCurrentResearchLevel('Woodcraft') < woodcraftLevel) m += '<TD>&nbsp;Woodcraft '+ woodcraftLevel 
...
scienceCenterLevel +'</td>';
        if (t.getBuildingLevel(0, 'Metalsmith', metalsmithLevel) == 0) m += '<TD>&nbsp;Metalsmith '+ 

metalsmithLevel +'</td>';
        if (t.getBuildingLevel(0, 'Garrison', garrisonLevel) == 0) m += '<TD>&nbsp;Garrison '+ garrisonLevel 
...
scienceCenterLevel +'</td>';
        ...
        if (t.getCurrentResearchLevel('Levitation') < levitationLevel) m += '<TD>&nbsp;Levitation '+ 
...
levitationLevel +'</td>';
    ...
    checkAerialCombatReqs : function() {
        ...
        if (city.resources.gold < gold) m += '<TD>&nbsp;gold '+ (gold - city.resources.gold) +'</td>';
        if (city.resources.food < food) m += '<TD>&nbsp;food '+ (food - city.resources.food) +'</td>';
        if (t.getBuildingLevel(0, 'DragonsKeep', dragonskeepLevel) == 0) m += '<TD>&nbsp;Dragons Keep '+ 

dragonskeepLevel +'</td>';
        if (t.getCurrentResearchLevel('Dragonry') < dragonryLevel) m += '<TD>&nbsp;Dragonry '+ dragonryLevel 
...
+'</td>';
    ...
    // Called from setTrainEnable() with a starting city of zero (capitol), attemptTrainShortQ() uses setTimeout() 
        ...
        //Seed.notifyOnUpdate(function(){   
            for (var ic=0; ic<Seed.cities.length; ic++ ){
                ...
                if (bJob == null){     // city not currently building
                    ...
                    // Change: we want to iterate over each buildings comparing the level to the cap. If the cap 
        ...
        //Seed.notifyOnUpdate(function(){
            ...
            if (rJob == null){     // no research being done yet
                ...
                if (rJob) {
                    ...
                    for (var i=0; i<Data.options.rJobs.length; i++) {
                        // Check the rJob structure for a field called city_research_id or some such (like the 
...
building)
    ...
    // doTrain() also calls Seed.fetchCity() (before calling attemptTrainShortQ()) to ensure that the data is 

current
    // attemptTrainShortQ() will examine the training queues to determine when/if a new job should be sent to 
                        ...
                        case 1:
                            ...
                            try {
                                if (cap) {
                                    ...
                                    document.getElementById('pbtrnTrp_' + i + '_' + j).style.backgroundColor = 

"red";
                                }
                                else if (document.getElementById('pbtrnTrp_' + i + '_' + j).style.backgroundColor 
...
"red")
    ...
    // If the rslt is ok, we set the Train Tab errorCount back to zero, log the training, increment the count 

(why?)
    // and attempt to train more troops - this seems like it should come from trainTick() instead being called 

directly here
    // If the rslt is not ok, we refetch the city info, log the error, increment the Train Tab errorCount (if we 

have more than
    // three errors we disable training and show the feedback) and display the error message, reset the training 
...
time for 20 seconds
    ...
    tabTrain : function(){
        ...
        for (var c=0; c<Seed.cities.length; c++){
            ...
            m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ city.name +'</div><TABLE class=' + 

rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';
            for (tt=0; tt<troopTypes.length; tt++){
                // Use less room in the table layout
                if (tt%3 == 0) m += '<TR>';
                m += '<TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ Names.troops.byName[troopTypes[tt]][2] 

+':</td>';
                var num = Data.options.autoTrain.city[c].troopType[tt];
                if (!num || isNaN(num)) num = 0;
                m += '<TD><INPUT type=text id=pbtrnTrp_'+ c +'_'+ tt +' maxlength=6 size=2 value="'+ num 
...
important;" colspan=2></td></tr>\
        ...
        // Add the radio buttons  
        m += '<TR><TD><INPUT type=radio name=pbttQRadio value="Minimum Housing" ></td><td>'+ kMinHousing 

+'</td></tr>';
        m += '<TR><TD><INPUT type=radio name=pbttQRadio value="Minimum Resource Levels" ></td><td>'+ 

kMinResourceLevels +'</td></tr>';
        m += '</table><DIV class=short></div>'; 
        ...
        m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle style="background-color:#0044a0;">Troop Cap (Max 

Troops, 0 = no max)</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE 

class=' + rndStyle['classPrefix'] + 'Tab>';
        for (tt=0; tt<troopTypes.length; tt++){
            // Use less room in the table layout
            if (tt%3 == 0) m += '<TR>';
            m += '<TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ Names.troops.byName[troopTypes[tt]][2] 
...
Tabs.Options = {
	...
	init : function (div) {
		...
		try {      
			m = '<DIV class=' + rndClass['tab_title'] + ' style="margin-bottom:10px">'+ kOptions 

+'</div>\
				<TABLE>\
				<TR valign=top><TD><B>' + kGameOptions + ': </B></TD></TR>\
				<TR valign=top><TD><INPUT id=' + rndId['enable_collect'] + ' type=checkbox /> ' + 

kAutoCollect + ' <INPUT id=' + rndId['collect_time'] + ' size=1 maxlength=2 type=text value="' + 

Data.options.autoCollect.delay + '" />\
				...
				<TR valign=top><TD><INPUT id=' + rndId['enable_drag'] + ' type=checkbox /> ' + 

kEnableDrag + '</TD></TR>\
				<TR valign=top><TD><INPUT id=' + rndId['enable_verbose'] + ' type=checkbox /> ' + 

kEnableVerbose + '</TD></TR>\
				</TABLE><BR><HR>\
				<TABLE>\
				<TR valign=top><TD><INPUT id=' + rndId['options_refresh'] + ' class=' + rndClass

['blue_button'] + ' type=submit value=' + kRefresh + ' /></TD></TR>\
				</TABLE><BR><HR>';
				
			t.cont.innerHTML = m;
			t.togOpt(rndId['enable_collect'], Data.options.autoCollect.enabled, AutoCollect.setEnable);
			document.getElementById(rndId['collect_time']).addEventListener ('change', t.timeChanged, 

false);
			document.getElementById(rndId['collect_unit']).addEventListener ('change', t.unitChanged, 
...
false);
			...
			document.getElementById(rndId['options_refresh']).addEventListener ('click', 

t.refreshClicked, false);
		} catch (e) {
	...
	togOpt : function (checkboxId, optionVar, callEnable, callIsAvailable) {
		...
		checkbox.addEventListener ('change', new eventToggle(checkboxId, optionVar, callEnable).handler, 
...
//WinLog.writeText ('***** AJAX: '+ t.curX +' , '+ t.curY);    
    new MyAjaxRequest ('map.json', { 'user%5Fid':C.attrs.userId, x:t.firstX, y:t.firstY, timestamp:parseInt

(serverTime()), '%5Fsession%5Fid':C.attrs.sessionId, 'dragon%5Fheart':C.attrs.dragonHeart, version:getVersion }, 
...
(t.curIX*15)), y:t.normalize(t.firstY+(t.curIY*15)), timestamp:parseInt(serverTime()), '%5Fsession%

5Fid':C.attrs.sessionId, 'dragon%5Fheart':C.attrs.dragonHeart, version:getVersion }, t.got, true);}, MAP_DELAY);
 },
...
//  curtain.style.padding = curtainDiv.style.padding;
  ...
  this.div.innerHTML = '<TABLE HEIGHT=100% WIDTH=100%><TR valign=middle height=80%><TD><DIV id=MD7r8hc style="text

-align:center"></div></td></tr>\
    <TR valign=middle align=center><TD><INPUT id=MD7r8h type=submit value="CLOSE" 
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    for (var i=0; i<sorter.length; i++)
      m += '<TD class=spacer></td><TD class=notSel id='+ sorter[i][1].scrambled +' ><A><SPAN>'+ sorter[i][1].label 
...
+'</span></a></td>';
    ...
    for (k in t.tabList) {
      ...
      div.style.maxWidth = document.getElementById(t.tabList

[k].scrambled).parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.s
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
    ...
    pop.getMainDiv().innerHTML = '<TABLE class=ptTab align=center style="height: 100%"><TR align=center height=90%

><TD>'+ msg +'</td></tr>\
        <TR align=center><TD><INPUT id=ptcccancel type=submit value='+ kCancel +' \> &nbsp; &nbsp; <INPUT 

id=ptcccontin type=submit value='+ kContinue +' \></td></tr></table>';
    document.getElementById('ptcccancel').addEventListener ('click', function (){pop.show(false); if (canNotify) 

canNotify();}, false);
    document.getElementById('ptcccontin').addEventListener ('click', function (){pop.show(false); if (contNotify) 
			...
			if (r.status === 200 && r.responseText) {
				if (responseType.indexOf('application/json') !== -1) {
					try {
						...
						msg = 'The request succeeded but returned this error when being 

parsed: ' + e;
						callback({ok:false, errmsg:msg});
					}
				} else if (responseType.indexOf('application/xml') !== -1) {
					callback({ok:true, dat:r.responseText}); // Due to the inconsistent nature 

of the locale XML document parsing is not done at this point
			...
			if (r.status > 200 && r.responseText) {
				if (responseType.indexOf('application/json') !== -1) {
					try {
						...
						msg = 'The request failed but returned this error when being 

parsed: ' + e;
						callback({ok:false, errmsg:msg});
					}
				} else if (responseType.indexOf('application/xml') !== -1) {
					callback({ok:false, dat:r.responseText}); // Due to the inconsistent nature 
...
of the locale XML document parsing is not done at this point
	...
	function AjaxRequest(url, opts) {
		...
		if (opts.method === 'POST') {
			...
			headers['X-S3-AWS'] = SHA1("Dracunculiasis" + parmStr + "LandCrocodile" + url  + "Bevar-
...
Asp");
		...
		ajax.onreadystatechange = function () {
			if (ajax.readyState === 4) {
				clearTimeout(timer);  
				if (ajax.status === 200) {
					if (opts.onSuccess) opts.onSuccess({responseText:ajax.responseText, 

status:ajax.status, statusText:ajax.statusText, ajax:ajax});
				} else {
					if (opts.onFailure) opts.onFailure({responseText:ajax.responseText, 
...
status:ajax.status, statusText:ajax.statusText, ajax:ajax});
		...
		function timedOut() {
			ajax.abort();
			if (opts.onFailure) opts.onFailure({responseText:null, status:599, statusText:'Request 

Timed Out', ajax:ajax}); // CHECK: 599 is custom error code. See if better option exists.
		}
	}   
...
function logit(msg) {
	...
	var now = new Date();
	console.log(serverID + ' @ ' + now.toTimeString().substring (0,8) + '.' + now.getMilliseconds() + ': ' +  
...
Tabs.ActionLog = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<DIV class=' + rndClass['tab_title'] + '>ACTION LOG</div><DIV style="height:725px; max-

height:725px; overflow-y:auto">\
      <TABLE cellpadding=0 cellspacing=0 id=pbactionlog class=' + rndStyle['classPrefix'] + 

'TabLined><TR><TD></td><TD width=95%></td></table></div>';
...
function titleLine (msg){
  return '<TABLE cellpadding=0 cellspacing=0 width=100%><TR><TD width=50%><HR></td><TD>'+ msg +'</td><TD width=50%
...
function debugPos  (e){
  return 'client - offset: '+ e.clientRight +','+ e.clientTop +','+ e.clientWidth +','+ e.clientHeight
          +' - '+ e.offsetRight +','+ e.offsetTop +','+ e.offsetWidth +','+ e.offsetHeight +' '+ e +' --OP--> '+ 
...
function htmlTitleLine (msg){
  return '<TABLE width=100% cellspacing=0><TR><TD style="padding:0px" width=50%><HR></td><TD style="padding:0px">[ 

'+ msg +' ]</td><TD style="padding:0px" width=50%><HR></td></tr></table>';  
}
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  var m = '<TABLE cellspacing=0 width=100% height=100%>\
           ...
           <TD id='+ rndPopup['X'] +' align=right valign=middle onmouseover="this.style.cursor=\'pointer\'" 

style="color:#fff; background:#333; font-weight:bold; font-size:14px; padding:0px 4px">X</td></tr>\
  ...
  function CeventDown (that){
    ...
    function handler (me){
      if (t.bounds == null){
        ...
        t.bounds = {top:10-t.clickableRect.height, bot:t.bodyRect.height-25, left:40-t.clickableRect.width, 
...
//t.dispEvent ('eventMOVE', me);
  ...
  function debug  (msg, e){
    ...
    logit ('offsetRight, Top, Width, Height (parent): '+ e.offsetRight +','+ e.offsetTop +','+ e.offsetWidth +','+ 

e.offsetHeight +' ('+ e.offsetParent +')');
    logit ('scrollRight, Top, Width, Height: '+ e.scrollRight +','+ e.scrollTop +','+ e.scrollWidth +','+ 

e.scrollHeight);
  }

  function dispEvent (msg, me){
    logit (msg + ' Button:'+ me.button +' Screen:'+ me.screenX +','+ me.screenY +' client:'+  me.clientX +','+ 

me.clientY +' rTarget: '+ me.relatedTarget);
  }
}
  ...
  for(property in obj) {
    try {
        ...
        if (doFunctions==true && (type == 'function')){
          ...
          str += indent + '(' + type + ') ' + property + ( (obj[property]==null)?(': null'):('')) +' = '+ obj
