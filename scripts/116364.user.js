...
try { //megatry
	...
	var version = 'RepeatJob Redux v1.02 beta', jobstodo = 0, job = 0, onevent = 'Pause', wait1 = 1, wait2 = 3, bankwithdraw = 'No', combinedloot='',
	...
	log_size = 10, log_keep=/(completed|secret)/, timestamping = true,
	...
	'<div id="'+spocklet+'_main">'+
		...
		//logorow+
		'<tr><td colspan="3" align="center" style="text-align:center;font-size:1.1em;">'+logo+'<br />'+version+' - <a href="http://www.spockholm.com/mafia/donate.php?RepeatJobRedux" class="sexy_button_new short white" target="_blank" title="Save the Click Monkeys" alt="Save the Click Monkeys!"><span><span id="donate">Support Team Spockholm</span></span></a> - <a href="http://www.spockholm.com/mafia/help.php" alt="Help" target="_blank">Help</a> - '+
	...
	function pausing(seconds,message,resume_func) {
		var delay = (seconds > 0)? delay = 100 : delay = 10;
	 ...
	 function banker(cash,city,deposit) {
		...
		if (deposit) {
			...
			if (current_city == 'vegas') {
				var url = preurl+'remote/html_server.php?xw_controller=propertyV2&xw_action=doaction&doaction=ActionBankDeposit&building_type=6&city=5';
			}
		}
		else {
			...
			if (current_city == 'vegas') {
				var url = preurl+'remote/html_server.php?xw_controller=propertyV2&xw_action=doaction&doaction=ActionBankWithdrawal&building_type=6&city=5';
			}
		}
		if (banking) {
			$.ajax({
	...
	function repeat_job(jid,tid,oid,jsig) {
		if(isNaN(jid) || isNaN(tid)) {
		...
		else if (expneed < parseInt(document.getElementById('exptolevel').value) && (parseInt(document.getElementById('exptolevel').value) > 0)) {
			...
			if (p(document.getElementById(cashid).innerHTML) > bankamount && bankamount > 0 && run != 0) {
				...
				do_job(preurl+'remote/html_server.php?xw_controller=map&xw_action=dojob&xw_city='+xw_city+'&job='+jid+'&tab='+tid+'&opponent_id='+oid+'&job_sig='+jsig);
			}
		}
		else {
			msg('Pausing...');
		...
		$.ajax({
			...
			success: function (response){
				...
				if(objectdata['job_results']['success'] == '1') {
					...
					if (objectdata['job_results']['bossv2_consumable'] !== undefined) {
						parse_loot(objectdata['job_results']['bossv2_consumable'],'Boss v2');
					}
					...
					// edit
						log(timestamp()+pre+' <span class="good">'+exp+' exp</span> and '+(money<0?'spent ':'')+'<span class="'+(money_gained>0?'cur_cash':'bad')+'"><strong>'+sign+commas(money)+'</strong></span>'+(objectdata['job_results']['cash_bank_consumed']>0?' from bank.':'')+mastery+loot.slice(0,loot.length-2));
					}
					if (jobsig === 'undefined') {
	...
	function parse_loot(lootobject,loot_tag) {
		var replacetext = /(- Used in Boss Fights)|(was used to build and upgrade the)|(- Used to build and upgrade the |(was used to build armor in the))(Armory|Weapons Depot|Chop Shop)/;
		if (loot.length == 0) {
			loot = '. Loot: ';
		}
		var tag = '';
	...
	document.getElementById('donate').onmouseover = function() {
		this.innerHTML = 'Donate a Pint/Coffee';
		return false;
	};
	document.getElementById('donate').onmouseout = function() {
		this.innerHTML = 'Support Team Spockholm';
		return false;
	};
	controls_display(false,true,true);
