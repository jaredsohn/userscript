...
// @version        kite-kite, kompatible by v3.2.6
// @dtversion     99999999
// @timestamp     1319917890445
// @description   provide a quick reply feature, under circumstances capcay required.
// @author        idx(302101; http://userscripts.org/users/idx); bimatampan(founder); kucrutbeibeh (editan);
...
gvar.sversion = 'versi ' + 'kite-kite';
gvar.scriptMeta = {
  timestamp: 1319917890445 // version.timestamp

 ,dtversion: 08382962797 // version.date
...
function init(){
  ...
  gvar.silahken= 'Silahken ngepost gan';
  gvar.tooshort= 'kependekan gan.. musti 5 karakter baru bisa';
  gvar.qr_diakses= 'Quick Reply bisa dipake dengan ngeklik langsung setiap ikon ginian';
...
function start_Main(){
    ...
    if(!gvar.restart) {
     ...
     if(leng) for(var i=0; i<leng; i++){
       ...
       // qqr
       child='<img src="'+gvar.B.qquote_gif+'" alt="QQ-Reply" title="Quick Quote postingan ini" border=0 />';
...
function buildRate(){
  ...
  sel = createEl('select', {id:'sel_rating',name:'rating','class':'gbtn',tabindex:'6',title:'Rate this Thread..'});
   Dom.add(sel, par);
  el=createEl('option', {value:0}, 'pilih ratingnye');
   Dom.add(el, sel);
...
function qr_preview(reply_html){
  ...
  if(isUndefined(reply_html)){ // is there ret from XHR :: reply_html
    ...
    GM_XHR.uri = prep[1];
    GM_XHR.cached = true;
    GM_XHR.request(spost.toString(),'posting', qr_preview);
  } else {
  ...
  if(flag) { // locking ..    
      ...
      if(gvar.user.isDonatur){
        ...
        if(adv){
            ...
            ec = createEl('input', {id:'gajadi_mosting',type:'button',value:'Cancel',style:'margin:0 10px',title:'Cancel Posting'});
    ...
    if($D('#qr_prepost_submit')) $D('#qr_prepost_submit').value= (gvar.user.isDonatur ? '' : 'sebelom-') + 'Posting Quick Reply';
...
function initEventTpl(){
    ...
    if(!gvar.restart){
      ...
      _o('submit',$D('#vbform'),function(e){
        ...
        if(gvar.settings.ajaxpost && $D('#clicker').value != 'aturan lanjutan' ) {
          clog('here and aborted');
          e.preventDefault(); // return false;
          return false;
		}else{		

          if( !gvar.user.isDonatur && $D('#clicker').value != 'aturan lanjutan' ){
...
var DRAFT= {
  ...
  ,save: function(txt){
    if(isUndefined(txt)){
        ...
        $D('#save_draft').value= 'Saved';
        $D('#draft_desc').innerHTML = 'udeh kesimpen';
		DRAFT.switchClass('gdisabled');
...
,[H+'fd_2.gif', ':cd:', 'Cape deeehh']
 ...
 ,getTPL_main: function(){
  return (''
    ...
    +'<div style="float:left;">Pasang Judulnye:&nbsp;<a href="javascript:;" id="atitle" title="Optional Title Message">[+]</a>&nbsp;</div><div id="titlecont" style="display:none;"><div id="dtitle" style="float:left;margin-top:-3px;""><input id="input_title" type="text" tabindex="1" name="title" maxlength="85" class="input_title" title="Optional"/></div>&nbsp;<div class="spacer">&nbsp;</div></div>'
    +'Isi Posting:&nbsp;<a id="textarea_clear" href="javascript:;" title="Clear Editor">reset</a>'
 ...
 ,getTPL: function(){
  return ('' 
    ...
    +(gvar.settings.qrdraft ? '<div id="qrdraft" style="position:absolute;right:160px;"><span id="draft_desc">blank</span><input id="save_draft" class="lilbutton gbtn gdisabled" type="button" title="" value="Simpen" /></div>' : '')
    
    +'<div id="qrsetting" style="position:absolute;right:57px;"><a id="qr_setting_btn" href="javascript:;" style="text-decoration:none;outline:none;" title="Pengaturan '+gvar.fullname+'" ><img src="'+gvar.B.setting_gif+'" alt="S" border="0"/><div style="float:right;margin:0;margin-top:3px;padding:0 2px;">Pengaturan</div></a></div>'
    +'</td></tr></thead>'
  ...
  +'</tr></table>'
    ...
    */
    ...
    +'<input id="chk_fixups" tabindex="7" type="checkbox" '+(gvar.settings.widethread ? 'checked="checked"':'')+'/><a href="javascript:;"><label title="Wider Thread with Kaskus Fixups" for="chk_fixups">Centang</label></a>'
    +'</div>'
    ...
    +'<div style="text-align:center;width:100%;">'    
     ...
     +'&nbsp;&nbsp;'
     +'<input id="qr_preview_ajx" class="gbtn" type="button" title="(Alt + P)" name="sbutton" tabindex="4" value="liat dulu" />'
 ...
 ,getTPL_vbEditor: function(){
  ...
  if(lUse = gvar.settings.lastused){
    lF = [(lUse.font ? lUse.font:''), ''];
     lF[1] = (lF[0] ? lF[0].substring(0,11)+(lF[0].length>11?'..':''):'Font');  
    lC = [(lUse.color ? lUse.color:'Black'), ''];
     lC[1] = (lC[0] ? lC[0].toLowerCase() : '');
    lS = (lUse.size ? lUse.size:'Ukuran');
  }
  return (''
     ...
     +(gvar.settings.hidecontroll[6] != '1' ? ' <td><div class="imagebutton cdefault" id="vB_Editor_001_qrcmd_createlink"><img src="'+gvar.domainstatic+'images/editor/createlink.gif" alt="Insert Link" title="Masukan Link" /></div></td>'
     :'')
     +(gvar.settings.hidecontroll[7] != '1' ?' <td><div class="imagebutton cdefault" id="vB_Editor_001_qrcmd_insertimage"><img src="'+gvar.domainstatic+'images/editor/insertimage.gif" alt="Insert Image" title="Masukin gambar" /></div></td>'
     :'')
     ...
     +' <td><div class="imagebutton cdefault" id="vB_Editor_001_cmd_insertsmile"><img id="vB_Editor_001_cmd_insertsmile_img" src="'+gvar.B.smile_gif+'" alt="Smiles" title="Masukin smiles" /></div></td>' :'')
 ...
 ,getTPL_Preview: function(tipe){
  var prop = {
     'settings' : 'Pengaturan '+gvar.titlename+'<span id="fsize" style="display:none;"></span><span id="subtitle" style="float:right;margin-right:5px;"></span>'
 ...
 ,getTPL_Settings_About: function(){
    ...
    return (''
     ...
     +spacer
     +'<a href="http://'+ 'userscripts.org/scripts/show/'+gvar.scriptMeta.scriptID+'" target="_blank">'+gvar.fullname+'</a> is an improvement of '+HtmlUnicodeDecode('&#733;')+'kaskusquickreply'+HtmlUnicodeDecode('&#733;')+' (Firefox Add-Ons) initially founded by bimatampan and edited kite-kite by kucrutbeibeh<br>'
 ...
 ,getTPL_Settings_General: function(){
    ...
    return (''
     ...
     +'<input id="misc_autolayout_sigi" type="checkbox" '+(gvar.settings.userLayout.config[0]=='1' ? 'checked':'')+'/><label for="misc_autolayout_sigi">TandaTanganOtomatis</label>&nbsp;'
