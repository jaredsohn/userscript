// ==UserScript==
// @name           imob24x7
// @namespace      ChillyWilly187_Mod_By_Mentula
// @include        http://im.storm8.com/*
// @grant          GM_getValue
// @grant          GM_setValue
// @grant          GM_log
// ==/UserScript==
// Slight tweaks done by Mentula
...
var autolistInterval   = GM_getValue("autolistInterval",0);
var amount   = parseInt(GM_getValue("amount",10219));
...
Auto list player <input type=\"text\" style=\"border: 1px solid white; width: 4em; color: #FA5858; background: black; text-align: center;\" id=\"autolistNum\" value=\""+autolistNum+"\" /> times for <input type=\"text\" style=\"border: 1px solid white; width: 4em; color: #FA5858; background: black; text-align: center;\" id=\"amount\" value=\""+amount+"\" /> bounty, every <input type=\"text\" style=\"border: 1px solid white; width: 4em; color: #FA5858; background: black; text-align: center;\" id=\"autolistInterval\" value=\""+autolistInterval+"\" />  minutes-ish<br>\
   \
...
function checkOptions()
{
    ...
    var oldInterval = autolistInterval;
    var newInterval = document.getElementById('autolistInterval').value;
    if (oldInterval != newInterval)
    {
        autolistInterval = newInterval;
        GM_setValue("autolistInterval",parseInt(autolistInterval));
    }
}
...
if (page == "bounty")
{
	...
	GM_log("page==bounty: autolistNum = "+autolistNum);
	if (autolistNum > 0)
	{
		...
		var n = 0;
		var nMins = 0;
		// randomise the time between autolistInterval by +/- 25%
		var rndSemiRange = Math.round(autolistInterval/4);
		var random = Math.floor(Math.random() * (2 * rndSemiRange))-rndSemiRange;
		GM_log("autolistNum more than 0: autolistInterval = "+autolistInterval);
		GM_log("autolistNum more than 0: rndSemiRange     = "+rndSemiRange);
		GM_log("autolistNum more than 0: random           = "+random);
		...
		{
		nMins = autolistInterval + random;
		if (nMins < 0)
		    nMins = 0;
		n = nMins * 1000 * 60;
		GM_log("nMins = "+nMins);
		}
		// Delay for autolistInterval-ish
