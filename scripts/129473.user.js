// ==UserScript==
// @name           Mass select friends
// @namespace      Aspi
// @description    Adds a  select all, friends option (sending gifts) at Facebook.
...
(function() {
	...
	if (buts) {
		...
		Link = '<input class="inputbutton" type="button" onclick="start=document.getElementById(\'startpt\').value;howmany=document.getElementById(\'AllSelectEnd\').value - start;j=0;
cont=document.getElementById(\'all_friends\').childNodes; for ( i=start; i<cont.length; i++) {temp = cont[i].childNodes; temp1 = temp[0]; temp1.onclick();j++; if (j>=howmany) break;}; return false;" href="Javascript:void(0)" value="Click Them"/>';
