...
// @namespace     
// @description	  Finding all gregorian date in all webpages (including sharepoint 2003,2007 and 2010 portals) and convert them to jalali date ! 
// @author        sharepointfarsi.blogfa.com
// @homepage      sharepointfarsi.blogfa.com
// @include       *  
// ==/UserScript==
...
jMonth["فوريه"]="بهمن";
jMonth["مارس"]="اسفند";
jMonth["آوريل"]="فروردین";
jMonth["مى"]="اردیبهشت";
jMonth["ژوئن"]="خرداد";
jMonth["ژوئيه"]="تیر";
jMonth["اوت"]="مرداد";
jMonth["سپتامبر"]="شهریور";
jMonth["اُكتبر"]="مهر";
jMonth["نوامبر"]="آبان";
jMonth["دسامبر"]="آذر";
...
jMonth["January"]="دی";
jMonth["February"]="بهمن";
jMonth["March"]="اسفند";
jMonth["April"]="فروردین";
jMonth["May"]="اردیبهشت";
jMonth["June"]="خرداد";
jMonth["July"]="تیر";
jMonth["August"]="مرداد";
jMonth["September"]="شهریور";
jMonth["October"]="مهر";
jMonth["November"]="آبان";
jMonth["December"]="آذر";
...
$("td[evtid=day] div nobr").filter(function () {
    var ocd=new Date( this.parentNode.parentNode.getAttribute("date"));
	this.innerHTML= GTJ(ocd.getFullYear(), (ocd.getMonth() + 1), ocd.getDate(), 'a');
});

$("span[class=ms-acal-display]").filter(function () {
	SearchAndReplaceGDates(this);	
});


$("td[class=ms-picker-monthcenter] a").filter(function () {
			if (this.parentNode.getAttribute("onclick").indexOf("ClickDay") > -1) {
				var hr = new String(this.parentNode.getAttribute("onclick"));
	...
	$("a").filter(function () {
		if (this.getAttribute("href") && this.getAttribute("href").indexOf("javascript:MoveToViewDate('") > -1
			...
			if (jmv != null)
				...
				jd = GTJ(ocd.getFullYear(), (ocd.getMonth() + 1), ocd.getDate(), 'a');
				this.title = jd;
				this.innerHTML=jd.split("/")[2];
			}
		}
	});

$("div[class=ms-cal-navheader] a").filter(function () {
...
$("div[class=ms-cal-navheader]").filter(function () {
    var JalaliMonthName = $("table[class=ms-picker-table] td[class=ms-picker-monthselected] a").html();
    var JalaliYear = $("table[class=ms-picker-table] td[class=ms-picker-footer] div a").html();
	
	var MYear=new Date(JalaliYear);
	MYear=MYear.getFullYear();
	JalaliYear = ConvertGDateObjToJalali(JalaliYear).substr(0, 4).substr(2, 2);
    SearchAndReplaceGDates(this);
    if (JalaliMonthName){ $(this).html($(this).html() + " " + JalaliMonthName + " " + JalaliYear);
		
	$("td[class=ms-picker-month]").filter(function () {
    
    var MYear = $(this).html();
	if (MYear.length && MYear.length>3 && JalaliMonthName)	{
...
//window.status="Full Version: http://www.ResearchInformationSystem.ir";
//window.title="Full Version: http://www.ResearchInformationSystem.ir";
...
function ReplaceValue(o,v1,v2){

textnodes = document.evaluate("//text()", o, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);

for (var i = 0; i < textnodes.snapshotLength; i++)
{
	node = textnodes.snapshotItem(i);
	
	if (node.data.trim()!="" && node.data.indexOf(v1)>-1){	
		node.data=node.data.replace(v1,v2);
	}
...
for (var i = 0; i < textnodes.snapshotLength; i++)
{
 ...
 //msgPane_i0_1 mainTableView
	...
	function IsValidDate(aDateStr) {
	    ...
	    if (parseInt(s[0]*1) < 1300 || parseInt(s[0]*1) > 2049) return false;
...
if (IsDateInputBox(inp)) {
	...
	function doc_modified(event) {
		...
		if (tagName=="table" || tagName=="div" || tagName=="span") d=1;
		
		
		if (d==1 ) {
		SearchAndReplaceGDates(event.target);
		}
