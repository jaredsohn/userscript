...
// 08/08/2009
// ==UserScript==
// @name Vendetta Plus 1.7.4
// @author mitm[ITA], UnDeaD[GER]
// @descrizione: Vendetta All-in-one tool - Supports foreign servers; Original Script by: Mosaik(ITA), Al_Caponazzo(ITA), Alex(FRA), Platone (ITA), TGWo (Admin Vendetta1923.it), Trip (ITA);
...
(function(){
	...
	if (location.pathname.search('konst.php') != -1 ) {
		...
		newDiv.innerHTML = '<hr><input name="rst_view" value="Zurücksetzen" type="Submit" STYLE=\'width: 120px;font-size:8pt;font-family:Verdana\'>';
	...
	if (location.pathname.search('player.php') != -1 ) {		
		...
		for (var i = 0; i < link_attivi.length - 1; i++) {
			...
			range.selectNode(link_attivi[i]);
				...
				newDiv.innerHTML =  "<b STYLE='font-size:8pt;font-family:Verdana;cursor:pointer;' onclick=\"window.location.href = \'" + link + "\'; \">(Zur Farmliste Hinzufügen) </b>&nbsp;";
				newDiv.innerHTML += "<b STYLE='font-size:8pt;font-family:Verdana;cursor:pointer;' onclick=\"window.location.href = \'" + link2 + "\'; parent.frames[3].location = \'" + link_nav_flotte + "\'; \">Angriff~~></b>&nbsp;";
	...
	/* cosa fa: aggiunge, davanti alle coordinate di ogni edificio, un link per la schermata Stanze dell'edificio selezionato, un link per la propria schermata Missioni con edificio selezionato impostato come bersaglio, e uno per la schermata flotta dell'edificio selezionato */
        if (location.pathname.search('planets.php') != -1)
        {
			...
			coord =  "<b STYLE='font-size:8pt;font-family:Verdana;cursor:pointer;' onclick=\"location = \'" + link_stanze + "'; parent.frames[3].location = \'" + link_nav_stanze + "\'; \"><b>:Raum:</b></b>&nbsp;<b STYLE='font-size:8pt;font-family:Verdana;cursor:pointer;' onclick=\"window.location.href = \'" + link_bersaglio + "';\">:Einsätze:</b>&nbsp;<b STYLE='font-size:8pt;font-family:Verdana;cursor:pointer;' onclick=\"window.location.href = \'" + link_flotte + "'; parent.frames[3].location = \'" + link_nav_flotte + "\'; \">:Einsätze*:</b>&nbsp;<br>" + coord;
	...
	if (location.pathname.search('msgshow.php') != -1 ) {		
		...
		newDiv.innerHTML =  '<br> <input type="button" name="Submit" value="Alles Auswählen" onClick="var inputs = document.getElementsByTagName(\'input\');for (var i = 0; i < inputs.length; i++) {blnEtat=true;if (inputs[i].checked==false) inputs[i].checked=blnEtat;}"> <input type="button" name="Submit2" value="Alles Zurücksetzen" onClick="var inputs = document.getElementsByTagName(\'input\');for (var i = 0; i < inputs.length; i++) {blnEtat=false;if (inputs[i].checked==true) inputs[i].checked=blnEtat;}"> <input type="button" name="Submit3" value="Auswahl Umkehren" onClick="var inputs = document.getElementsByTagName(\'input\');for (var i = 0; i < inputs.length; i++) {inputs[i].checked=!inputs[i].checked;}"> <input name="submit" type=submit value="Löschen!">';
	...
	if(location.pathname.search('msgshow.php') != -1)
	{
		...
		var html = "<button type='button'  STYLE='font-size:8pt;font-family:Verdana' onclick='"+funzione+"'>Alles Auswählen</button>&nbsp;&nbsp;";
	...
	if (location.pathname.search('nav.php') != -1 ) {		
		/* mitm: bottone reset */
		var html = "<hr><button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_reset>Einsätze Zurücksetzen</button><hr>";
	...
	if (location.pathname.search('flotte.php') != -1)
	{
		...
		/* rileva le Waffen disponibile sul Edificio */
		range.selectNode(anchorTagsRis[5]);
		Waffen = range.toString();
		Waffen = Waffen.replace(/\./g, "");

		/* rileva le Munition disponibile sul Edificio */
		range.selectNode(anchorTagsRis[6]);
		Munition = range.toString();
		Munition = Munition.replace(/\./g, "");

		/* rileva gli Alkohol disponibile sul Edificio */
		range.selectNode(anchorTagsRis[7]);
		Alkohol = range.toString();
		Alkohol = Alkohol.replace(/\./g, "");

		/* rileva i Dollar disponibile sul Edificio */
		range.selectNode(anchorTagsRis[8]);
		Dollar = range.toString();
		Dollar = Dollar.replace(/\./g, "");

		/* rileva la quantita  di ogni risorsa finora immessa nei campi di trasporto */
		txtWaffen = document.getElementsByName('r1').item(0).value;
		txtMunition = document.getElementsByName('r2').item(0).value;
		txtAlkohol = document.getElementsByName('r3').item(0).value;
		txtDollar = document.getElementsByName('r4').item(0).value;
		...
		for (var i = 0; i < anchorTags.length ; i++)
		{
			...
			if (text.indexOf('( ',0) != -1) // && restore != 0)
			{
				...
				var valore = casellaTesto[z].getAttribute("value");
                                ...
                                html += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_"+nome+">Speichern</button>";
	...
	html += "\">ALLES AUSWÄHLEN</button>";
	anchorTags[2].innerHTML = "Anzahl<br><br>"+html;
	var parentDiv = casellaTesto[z].parentNode;
	var valore = coord1;
	if (restore != 0 && pers != 1)
	{
	...
	html = "<br><b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value-- \">-</b>&nbsp;&nbsp;&nbsp;<b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value++ \">+</b>&nbsp;<br><input type=text name=\'C1\' value=\'"+valore+"\' maxlength=3 size=3>&nbsp;";
	html += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_C1>Speichern</button>";
	z = z + 1;
	...
	html += "<br><b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value-- \">-</b>&nbsp;&nbsp;&nbsp;<b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value++ \">+</b>&nbsp;<br><input type=text name=\'C2\' value=\'"+valore+"\' maxlength=3 size=3>&nbsp;<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_C2>Speichern</button>";
	z = z + 1;
	...
	html += "<br><b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value-- \">-</b>&nbsp;&nbsp;&nbsp;<b style=\"font-size:8pt;cursor:pointer;\" onclick=\"document.rs." + casellaTesto[z].getAttribute("name") + ".value++ \">+</b>&nbsp;<br><input type=text name=\'C3\' value=\'"+valore+"\' maxlength=3 size=3>&nbsp;<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_C3>Speichern</button>";
	z = z + 1;
	parentDiv.innerHTML = html;
	var parentDiv = selects[0].parentNode;
	var html = parentDiv.innerHTML;
	html += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_typ>Speichern</button>";
	parentDiv.innerHTML = html;
	var missione = GM_getValue(serverID + 'h');
	if (missione != 0) document.getElementsByName('typ').item(0).value = missione;
	trasporto(anchorTags[anchorTags.length-9], "Waffen", maxcarico, Waffen, txtWaffen, "r1", "r2", "r3", "r4", "a", restore);
	trasporto(anchorTags[anchorTags.length-7], "Munition", maxcarico, Munition, txtMunition, "r2", "r1", "r3", "r4", "b", restore);
	trasporto(anchorTags[anchorTags.length-5], "Alkohol", maxcarico, Alkohol, txtAlkohol, "r3", "r1", "r2", "r4", "c", restore);
	trasporto(anchorTags[anchorTags.length-3], "Dollar", maxcarico, Dollar-Stipendio, txtDollar, "r4", "r1", "r2", "r3", "d", restore);
	
	var ancora = document.getElementsByName('S');
	newDiv = document.createElement("span");
	var valore = GM_getValue(serverID + 'ext_view')?'checked':'';
	newDiv.innerHTML = '<br><br><input name="ext_view" value="yes" ' + valore + ' type="checkbox"> Nach Unten Blättern&nbsp;&nbsp;';
	newDiv.innerHTML += "<button type=\"submit\"  STYLE='font-size:8pt;font-family:Verdana' value=\"Avvia missione\" id=previous name=\"S\"><< Zurück</button>&nbsp;&nbsp;<button type=\"submit\" STYLE='font-size:8pt;font-family:Verdana' value=\"Avvia missione\" id=next name=\"S\">Weiter >></button>";
	ancora[0].parentNode.appendChild(newDiv);
	...
	function hide_btn(maschera)
	{
		...
		for(var i = 0; i < info.length; i++) {
			...
			if (nome.search('info.php') != -1) {
				...
				newDiv = document.createElement("span");
				//newDiv.innerHTML = '<input onclick=\"this.parentNode.parentNode.parentNode.innerHTML = \'\'; \" name="stanza_' + i + '" id="stanza_' + i + '" value="Ausblenden" type="submit">&nbsp;&nbsp;';
				newDiv.innerHTML = '<input name="' + maschera + '_' + item_numb + '" id="' + maschera + '_' + item_numb + '" value="Ausblenden" type="submit">&nbsp;&nbsp;';
	...
	function trasporto(nodo, nomerisorsa, maxcarico, quanris, txtrisorsa, pozris, pozris1, pozris2, pozris3, tipo, restore)
	{
		...
		appoTH += "<th>";
		appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' onclick=\"document.rs."+pozris+".value = parseInt(document.rs."+pozris+".value) - 1000000; if(parseInt(document.rs."+pozris+".value)<0) document.rs."+pozris+".value = 0\">-1kk</button>";
		/* pulsante -500k (decrementa di 500000 la quantit  della risorsa selezionata. Se il valore va sottozero, azzera il campo) */
		appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' onclick=\"document.rs."+pozris+".value = parseInt(document.rs."+pozris+".value) - 500000; if(parseInt(document.rs."+pozris+".value)<0) document.rs."+pozris+".value = 0\">-500k</button>";
		...
		/* pulsante 0*/
		appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' onclick=\"document.rs."+pozris+".value = 0; if(parseInt(document.rs."+pozris+".value)<0) document.rs."+pozris+".value = 0\">0</button>";
		/* pulsante +10k */
		...
		/* pulsante +500k */
		appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' onclick=\"document.rs."+pozris+".value = parseInt(document.rs."+pozris+".value) + 500000; if(parseInt(document.rs."+pozris+".value)>";
		appoTH += "Math.min("+maxcarico+"-parseInt(document.rs."+pozris1+".value)-parseInt(document.rs."+pozris2+".value)-parseInt(document.rs."+pozris3+".value), "+quanris+")";
		appoTH += ") document.rs."+pozris+".value =";
		appoTH += "Math.min("+maxcarico+"-parseInt(document.rs."+pozris1+".value)-parseInt(document.rs."+pozris2+".value)-parseInt(document.rs."+pozris3+".value), "+quanris+"); ";
		appoTH += "if(parseInt(document.rs."+pozris+".value)<0) document.rs."+pozris+".value = 0\">+500k</button>";
		/* pulsante +1kk */
		appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' onclick=\"document.rs."+pozris+".value = parseInt(document.rs."+pozris+".value) + 1000000; if(parseInt(document.rs."+pozris+".value)>";
		appoTH += "Math.min("+maxcarico+"-parseInt(document.rs."+pozris1+".value)-parseInt(document.rs."+pozris2+".value)-parseInt(document.rs."+pozris3+".value), "+quanris+")";
		appoTH += ") document.rs."+pozris+".value =";
		appoTH += "Math.min("+maxcarico+"-parseInt(document.rs."+pozris1+".value)-parseInt(document.rs."+pozris2+".value)-parseInt(document.rs."+pozris3+".value), "+quanris+"); ";
		...
		/* mitm: codice pulsanti salvataggio risorse */
	        appoTH += "<button type='button' STYLE='font-size:8pt;font-family:Verdana' id=bt_"+pozris+">Speichern</button>";
