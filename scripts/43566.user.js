// ==UserScript==
// @name           EmbajadaFull FIX
// @description    Solucionado el salto de linea en la embajada, ahora se muestra correctamente.
// @autor         P4
// @version		   0.1.1
...
confCell.innerHTML = "<img style='float: left' src='http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/configuracion.gif' alt='Config' title='Configuración' id='ainfoConfig'>";
confCell.innerHTML += "<img style='float: right' src='http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/atras.gif' alt='Reset' title='Resetear' id='ainfoReset'>";
...
function showToolConfig(e)
{
  ...
  else
  {
    ...
    div.innerHTML = '<h3 class="header" style="padding-top: 8px; height: 20px; background-image: url(http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/cartel.jpg)"><b>Config</b></h3>'; 
    div.innerHTML += '<div style="margin-left: 80px; margin-top: 20px; text-align: left">';
    
    if(tpl == 0) div.innerHTML += '<input id="eToolCfText" style="position: absolute;left: 60px;" type="radio" name="garba" value="0" checked >&nbsp;Texto<br/>';
    else div.innerHTML += '<input id="eToolCfText" style="position: absolute;left: 60px;" type="radio" name="garba" value="0">&nbsp;Texto<br/>';
    
    if(tpl == 1) div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1" checked >&nbsp;Imagenes</div>';
    else div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1">&nbsp;Imagenes</div>';
    
    //GM_log(div.innerHTML);
    div.innerHTML += '<br><input id="eToolConfButton" type="button" class="button" value="Guardar">';
...
function getActualData()
{
  ...
  for(i=1; i < membersTab.rows.length - 1; i++)
  {
    ...
    if(typeof(recorded_points[pName]) === 'undefined')
	  {
	    membersTab.rows[i].cells[points_cell_idx].innerHTML += " (<font color='#F00'>Nueva entrada</font>)";
...
function template(status, rowElement, lastOnline)
{
  if(status == 'online')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML="<div style='width: 8em'><span style='float: right'><img src='http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/conectado.gif'></span></div>";
        break;
      default:
        rowElement.cells[0].innerHTML="<blink><B><font color='#008800'>En linea</FONT></B></blink>";        
        break;
    }
  }
  else if(status == 'inactive')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML = "<div style='width: 8em'><span style='float: left'>("+lastOnline + ")</span><span style='float: right'><img src='http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/inactivo.gif'></span></div>";
        break;
      default:
        rowElement.cells[0].innerHTML = "<B><font color='#708090' SIZE='1'>("+lastOnline + ")<br>Inactivo</FONT></B>";        
        break;
    }
  }
  else if(status == 'offline')
  {
    ...
    {
      case 1:
        rowElement.cells[0].innerHTML = "<div style='width: 8em'><span style='float: left;'>("+lastOnline+")</span><span style='float: right'><img src='http://www.ikariam80.net23.net/imagenes/scripts/EmbajadaFull/noconectado.gif'></span></div>";
        break;
      default:
        rowElement.cells[0].innerHTML = "<font color='#F00' SIZE='1'><b>("+lastOnline+")</b></FONT>";
        break;
    }
  }
