// ==UserScript==
// @name        Order & DMG report script for LnL
// @namespace   Order & DMG report
// @author      LnL
...
//alert("start");
var sendToSpread=1;
var orderVersionBase='1.0';
var urlOrder="https://docs.google.com/document/d/1M_gWhWZHHrUbQqqCkrV1n5zBorAygnl4in1hO35V_5o/edit";
var startTag="[order]";
var endTag="[/order]";
var startImg="[img]";
var endImg="[/img]";
...
var formKey="dGxSVjhhelpFSTdIYWUwX1N2NEgzZVE6MQ";
var defArmy=function(){return "LnL";}
...
function getData()
{
        ...
        var battleCountdown=jQuery("#battle_countdown").text();
		var rankPoints=jQuery("#rank_min").text();
        var currentHealth=jQuery("#current_health").text();
		...
		if (sendToSpread==1 && totalDamage>0)
		{
			...
			var url=URLGoogleForm+
			 ...
			 "&entry.1.single=" + region + 
			 "&entry.2.single=" + leftSide +
			 "&entry.3.single=" + totalDamage +
			 "&entry.4.single=" + battleCountdown +
			 "&entry.5.single=" + blueDomination +
			 "&entry.6.single=" + currentRound +
             "&entry.7.single=" + currentHealth +
			 "&entry.8.single=" + rankPoints +
			 "&entry.9.single=" + fighterSkill +
			 "&entry.10.single=" + locAll +
			 "&submit=Submit";
			 //alert(url);
