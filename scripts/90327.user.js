// ==UserScript==
// @name           	OBGHelper Chat - Ikariam - V 0.5
// @namespace     	http://userscripts.org/scripts/show/90327
// @description    	Jou directe link met je alliantie op de irc server van GF
// @author       	Nightlion & alboltdrogo
//
// @include        	http://s*.nl.ikariam.com/index.php?view=diplomacyAdvisorAlly*
// @include         http://s*.nl.ikariam.com/index.php?view=options*
...
ScriptUpdater.check(90327, "0.5");

var gameServer = top.location.host;
var gameServerParts = gameServer.split(".");
var serverName = gameServerParts[0];

if(gameServerParts[1] == "nl"){
var InfoChat = "Het standaard kanaal is de alliantie tag aangevuld met de server. Mocht jouw alliantie een ander kanaal gebruiken dan kun je deze hier veranderen. Wil je een Kanaalbeheer bot in het kanaal hebben dan raden wij de standaardnaam aan: '[alliantie tag]-[wereld]'. Bijvoorbeeld: test-alpha. Geen # nodig dus.";
}
if(gameServerParts[1] != "nl"){
...
function addlinks() {
		...
		td1.innerHTML = '<br />Alliantie chat:';
		td2 = document.createElement("td");
		td2.innerHTML = '<input type="button" class="button" id="btn1" name="btn1" value="Chat Nu!" onclick="chat()" />';
		...
		td3.innerHTML = '<br />Alliantie Support:';
...
if ((document.getElementsByTagName('body')[0].id=="diplomacyAdvisorAlly"))
{
	if(GM_getValue('OBGUser'+serverName,'OBGUser')=='OBGUser')
	{
		GM_xmlhttpRequest({
			...
			onload: function(response) 
			{
				...
				hiddenDiv.innerHTML = response.responseText;
				
				var xml = response.responseXML;
				string = String(hiddenDiv.getElementsByTagName('table')[0].getElementsByTagName('input')[0].value);

				GM_setValue('OBGUser' + serverName, string);
			}
		});
	}
	
	ally = document.getElementsByTagName('table')[1].getElementsByTagName('tr')[0].getElementsByTagName('td')[1].innerHTML;
	ally = ally.replace(" ","_");
		
	if((GM_getValue('OBGAlly'+serverName,'OBGAlly')=='OBGAlly') || (GM_getValue('OBGAlly'+serverName,'OBGAlly') != ally + "-" + nameserverName()))
	{	
		if (ally.length != 0)
		{
		GM_setValue('OBGAlly'+serverName, ally + "-" + nameserverName());
		GM_setValue('AllyChan'+serverName, ally + "-" + nameserverName());
		}
		else
		{
		GM_setValue('OBGAlly'+serverName, 'undefined');
		GM_setValue('AllyChan'+serverName, 'undefined');
		}
	}
	
	if (GM_getValue('AllyChan'+serverName,'AllyChan')!='undefined')
	{
	var chat = "http://widget.mibbit.com/?server=webirc.ogamenet.net:%2B8070&channel=%23trainer%2c%23ikariam.nl%2c%23" + GM_getValue('AllyChan'+serverName,'AllyChan') + "&noServerTab=false&autoConnect=true&nick=" + GM_getValue('OBGUser'+serverName,'OBGUser');
	addlinks();
	}
}
...
if (document.getElementsByTagName('body')[0].id=="options"){
	...
	if(GM_getValue('OBGAlly'+serverName,'OBGAlly')=='OBGAlly')
	{
		GM_xmlhttpRequest({
			...
			onload: function(response) 
			{
				...
				var xml = response.responseXML;
				string = String(hiddenDiv.getElementsByTagName('table')[1].getElementsByTagName('tr')[0].getElementsByTagName('td')[1].innerHTML);
				...
				if (string.length != 0)
				{
					...
					GM_setValue('AllyChan'+serverName, string + "-" + nameserverName());
				}
				else
				{
					GM_setValue('OBGAlly'+serverName, 'undefined');
			   ...
			   "<tbody>" +
				  "<tr>" +
					 "<th id='AllyChat'>Ally Chat</th>" +
						"<td><input class='textfield' style='width: 120px;' type='text' id='AllyChatValue' value='" + GM_getValue('AllyChan'+serverName,'AllyChan') + "'></td>" +
				  "</tr>" +
			   "</tbody>" +
			"</table>" +
	...
	{
	  GM_setValue('AllyChan'+serverName, document.getElementById('AllyChatValue').value);
	  //change text to option page
	  document.getElementById('AllyChatValue').innerHTML = GM_getValue('AllyChan'+serverName,'AllyChan');
