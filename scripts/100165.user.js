// ==UserScript==
// @name           PCSD SUPREMA...® (Moderadores)
// @namespace      Editado por João uid=10491237285998293568
// @description    Script para facilitar o trabalho da PCSD SUPREMA...®
// @PCSD     http://www.orkut.com.br/Main#Community?cmm=112887386
// @include        *.orkut.*/Main#CommMsgs?cmm=112887386*
// @include        *.orkut.*/CommMsgs?cmm=112887386*
// @include        *.orkut.*/Main#CommMemberManage?cmm=112887386*
// @include        *.orkut.*/CommMemberManage?cmm=112887386*
// @include        *.orkut.*/Community?cmm=112887386*
// @include        *.orkut.*/Main#Community?cmm=112887386*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
...
|	
|     este script foi criado por Kaique, e editado por João	      |
|						                      |
+---------------------------------------------------------------------+
*/

Editado_por = "Koyote/Kaique";
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgs?cmm=112887386&tid=5589359088339191725", false);
	...
	function captchaDelTopic(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgs?cmm=112887386&tid=5589358783396513709", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[teal][b]Titulo:[/b][/teal] ".concat(titulo, breakLine, "[teal][b]Criador:[/b] ", "[gray]", uid, breakLine, "[/gray]", breakLine, "[/gray][teal][b]Motivo:[/b][/teal] ", motivo, breakLine, "[teal][b]Horario:[/b][/teal] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
