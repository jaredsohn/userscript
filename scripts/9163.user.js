// ==UserScript==
// @name Ogame FR : Salto Cuantico ( max/no fleet + all/no fleet on screen + no description)
// @author Flater
// @description Ogame ES : Jumper Gate ( max/no fleet + all/no fleet on screen + no description)
// @language FR
	...
	{   
	  ...
	  for( i = 1; i < trnode.length-1; i++ ) 
	    ...
	    var tablenode = document.getElementsByTagName('form')[0].getElementsByTagName('table')[1].getElementsByTagName('tbody')[0];
	      tablenode.innerHTML = tablenode.innerHTML.substring(0, tablenode.innerHTML.length-80)+'<th colspan="2"><a href="javascript:noShipsC();">Ninguna Nave</a></th><th colspan="2"><a href="javascript:maxShipsC();">Todas las Naves</a></th>'+tablenode.innerHTML.substring(tablenode.innerHTML.length-79, tablenode.innerHTML.length-1);
