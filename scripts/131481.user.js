// ==UserScript==
// @name       Gladalahara
// @namespace   Gladalahara
// @description Gladalahara
...
var myscript = function () {
    ...
    function topshelfstuff() {
        var mlbarr = 0;
        mlbarr = window.setInterval(function () {if (document.getElementById("mw_like_button")) {$("#mw_like_button").remove();window.clearInterval;}}, 50);
        var sbarr2 = 0;
        sbarr2 = window.setInterval(function () {if (document.getElementById("zbar")) {$("#zbar").parent().remove();window.clearInterval;}}, 50);
        var mw2barr = 0;
        mw2barr = window.setInterval(function () {if (document.getElementById("mafia_two_banner")) {document.getElementById("appwrapper").removeChild(document.getElementById("mafia_two_banner"));window.clearInterval;}}, 50);
        var revolverElt = 0;
        revolverElt = window.setInterval(function () {if (document.getElementById("LoadingOverlay")) {document.getElementById("final_wrapper").removeChild(document.getElementById("LoadingOverlay"));window.clearInterval;}}, 50);
        var loadingBrackground = 0;
        loadingBrackground = window.setInterval(function () {if (document.getElementById("LoadingBackground")) {document.getElementById("final_wrapper").removeChild(document.getElementById("LoadingBackground"));window.clearInterval;}}, 50);
    }
    ...
    if (/dialog\/feed/.test(window.location.href)) {
        if (document.evaluate("//input[@type=\"submit\" and @name=\"publish\"]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
            var pubElt = document.evaluate("//input[@type=\"submit\" and @name=\"publish\"]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
            if (/Get a Boost/.test(document.body.innerHTML)) {
                var evt = document.createEvent("MouseEvents");
                evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
                pubElt.snapshotItem(0).dispatchEvent(evt);
            }
        }
        return;
    }
    ...
    var avoidlist = [];
    var rivalcycle = [];
    var familytargets = new Array;
    var familymodeOn = false;
    var familyindex = 0;
    var fighton = false;
    ...
    var icecount = 0;
    var killcount = 0;
    var heal_green_count = 0;
    var heal_red_count = 0;
    var ILVjobon = false;
    var fightwatchdogtimout;
    var icecheckfail = false;
    var fightInterval;
    var healInterval;
    var energyInterval;
	var logOn =false;
    ...
    function writeCookie() {
        var a = stamOn + "|" + healOn + "|" + postOn + "|" + fbfeedloc + "|" + jobsOn + "|" + healat + "|" + icesteal + "|" + burstOn + "|" + pauseOn + "|" + stopat + "|" + autopostOn + "|" + rref + "|" + FighterInt + "|" + autoBank + "|" + removeChat + "|" + MaxHit + "|" + autoclose + "|" + showHealth;
        createCookie("UMW", a);
    }
    ...
    function readCookieSettings() {
        try {
            ...
            if (a == null || /undefined/.test(a) || a.length < 18) {
                stamOn = false;
                postOn = false;
                healOn = false;
                fbfeedloc = "";
    ...
    function readCookie(a) {
        var i, cookie, nameEQ = a + "=", cookieArray = document.cookie.split(";");
    ...
    function toggleStam() {
        if (stamOn == true) {
            ...
            fightInterval = setInterval(Fighter, FighterInt);
    ...
    function togglePost() {
        if (postOn == true) {
            ...
            document.getElementById("PostOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.fb_icon + "\"></span></span>";
        } else {
            postOn = true;
    ...
    function toggleLog() {
        if (logOn == true) {
            ...
            document.getElementById("LogOnOff").setAttribute("class", "sexy_button_new short black");
            document.getElementById("LogOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"http://zyngapv.hs.llnwd.net/e6/mwfb/graphics/inventory/WishlistDropDown/Inventory-wishlist-icon.png\"></span></span>";
        } else {
            logOn = true;
			$( "#logDialog" ).dialog( "open" );
			document.getElementById("logData").value = toolbarLog;
            document.getElementById("LogOnOff").setAttribute("class", "sexy_button_new short green");
            document.getElementById("LogOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"http://zyngapv.hs.llnwd.net/e6/mwfb/graphics/inventory/WishlistDropDown/Inventory-wishlist-icon.png\"></span></span>";
        }
    }
    ...
    function toggleEvil() {
        if (evilOn == true) {
            ...
            document.getElementById("unlockedStatus").innerHTML = "STOPPED I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short black");
            document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
        } else {
            evilCity = /mw_city(\d)/.exec(document.getElementById("mw_city_wrapper").className)[1];
            if (/Sucker Punch/.test(document.body.innerHTML)) {
                targetid = "p|" + /user=p\|(\d+)'.+>Profile/.exec(document.body.innerHTML)[1];
                target_name = /levels">\((.*?)\)/.exec(document.body.innerHTML)[1];
                evilInterval = setInterval(PureEvil, 15000);
                evilOn = true;
                document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                PureEvil();
            } else {
                if (document.evaluate("//li[contains(@class, \"tab_on tab_middle\")]//a[contains(., \"Fighters\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                    ...
                    document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                    document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                    evilInterval = setInterval(Fightlist, 5000);
                    Fightlist();
                } else {
                    if (document.evaluate("//li[contains(@class, \"tab_on tab_middle\")]//a[contains(., \"Rivals\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                        ...
                        document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                        document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                        evilInterval = setInterval(Rivals, 5000);
                        Rivals();
                    } else {
                        if (document.evaluate("//li[contains(@class, \"tab_on\")]//a[contains(., \"Robbing\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                            evilOn = true;
                            document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                            document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                            setTimeout(LetThereBeRobberies, 1000);
                            document.getElementById("unlockedStatus").innerHTML = "Robbing in progress.....";
                        } else {
                            if (document.evaluate("//li[contains(@class, \"tab_on tab_first\")]//a[contains(., \"Families\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
                                var tabs = document.getElementsByClassName("clan_nav_tab selected");
                                if (tabs.length > 0 &&
                                    tabs[0].id == "battle_tab") {
                                    evilOn = true;
                                    document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                                    document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                                    familymodeOn = true;
                                    var members = document.getElementsByClassName("fl");
                                    for (i = 1; i < members.length; i++) {
                                        familytargets[i - 1] = new familyMember(decodeID(/user=(.+?)\"/.exec(members[i].innerHTML)[1]), "weak");
                                    }
                                    evilInterval = setInterval(Battle, 10000);
                                    document.getElementById("unlockedStatus").innerHTML = "Battle Mode Loading";
                                    Battle();
                                    return;
                                }
                                evilOn = true;
                                ...
                                var members = document.getElementsByClassName("name_n_rank");
                                for (i = 0; i < members.length; i++) {
                                    if (members[i].style.clear == "both") {
                                        familytargets[j++] = new familyMember(decodeID(/user=(.+?)\"/.exec(members[i].innerHTML)[1]), "weak");
                                    }
                                }
                                document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                                document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                                evilInterval = setInterval(FightFamily, 5000);
                                document.getElementById("unlockedStatus").innerHTML = "Fight Family Loading";
                                FightFamily();
                            } else {
                                if (document.getElementById("declare_war_head") &&
                                    /Declare war now for/.test(document.body.innerHTML)) {
                                    alert("Already at war, purchase a timer rest if you really want to declare, then retry.");
                                    return;
                                } else {
                                    if (document.getElementById("declare_war_head") &&
                                        document.getElementsByClassName("sexy_button_new medium white")) {
                                        document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short green");
                                        document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";
                                        evilOn = true;
                                        declare_war(null);
    ...
    function ExternalLinks() {
        ...
        if (toolbar) {
            ...
            if (document.getElementById("unlock_links")) {
                ...
                for (var i = 0; i < document.getElementsByClassName("unlocked_loadbm").length; i++) {
                    document.getElementsByClassName("unlocked_loadbm")[i].onclick = loadbm;
                }
            } else {
    ...
    function remove_addon_like_elt() {
        if (document.getElementById("fbmw_menu") &&
            document.getElementById("like_button")) {
            document.getElementById("fbmw_menu").removeChild(document.getElementById("like_button"));
        }
    }
    ...
    function loadbm() {
        ...
        if (/custom_bm_/.test(id)) {
            ...
            bm_url = unescape(unlocked.user_bm_locale[a]);
        } else {
            return;
        }
    ...
    function Toolbar() {
		...
		var quickheal_icon = document.getElementById("quickheal_icon");

		if(quickheal_icon){
			document.getElementById("quick_heal").addEventListener("click", quickHeal, false);
			document.getElementById("quick_powerpack").addEventListener("click",quickPowerpack , false);
		}
		else{
			var clock_health = document.getElementById("clock_health");
			
			var quickheal_icon = document.createElement('span');
			quickheal_icon.id = "quickheal_icon";
			quickheal_icon.className = "heal_link";
			
			 
			quickheal_icon.innerHTML = 	"<br>heal <a id=\"quick_heal\" class=\"heal_link\"><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/icon_health_16x16_01.gif\" title=\"Heal at Hospital\"></a>"+
										" or <a id=\"quick_powerpack\" class=\"heal_link\"><img src=\"http://zyngapv.hs.llnwd.net/e6/mwfb/graphics/HP_powerpack.png\"  title=\"Heal by Power Pack\" height=16></a>";
					
			clock_health.insertBefore(quickheal_icon, clock_health.lastChild);
		...
		if(!logDialog){
			...
			logDialog.title = "Ice & Kill Log";
			logDialog.style.fontSize = "9px";
			logDialog.innerHTML ="<textarea id=\"logData\" style=\"width:280px;height:120px;\" readonly>you have not kill anyone yet.</textarea>";
			
			document.body.insertBefore(logDialog, document.body.lastChild);
			...
			$(function() {
					...
					$( "#logDialog" ).bind('dialogclose', function(event) {
						logOn = false;
			            document.getElementById("LogOnOff").setAttribute("class", "sexy_button_new short black");
			            document.getElementById("LogOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"http://zyngapv.hs.llnwd.net/e6/mwfb/graphics/inventory/WishlistDropDown/Inventory-wishlist-icon.png\"></span></span>";
					 });
        ...
        if (toolbar) {
            ...
            if (document.getElementById("unlock_toolbar")) {
				...
				document.getElementById("LogOnOff").addEventListener("click", toggleLog, false);
                document.getElementById("EvilOnOff").addEventListener("click", toggleEvil, false);
            } else {
                toolbar_div = document.createElement("div");
                toolbar_div.id = "unlock_toolbar";
                toolbar.appendChild(toolbar_div);
                var toolbarhtml = "<div id=\"toolbar_content\" class=\"clearfix empire_module_title\" style=\"height: 30px;width:750px;\"><a href=\"http://www.facebook.com/groups/247143632716/\" id=\"logo\" target=\"_blank\" style=\"position:relative; top:-5px;\" > <img src=\"" + unlocked.images.unlocked_icon + "\"></a><a class=\"sexy_button_new short black\" id=\"BankButtonB\" style=\"position:relative; top:-15px; left:3px\"><span><span><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/icon_cash_16x16_01.gif\"></span></span></a><marquee behavior=\"scroll\" direction=\"left\" id=\"unlockedStatus\" width=\"35%\"  style=\"position:relative; top:-15px; left:120px;font-size:12px\" >I:" + icecount + "  K:" + killcount + " T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count + "</marquee><a id=\"unlockSettings\" style=\"position:relative; top:-15px; left:150px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/v3/icon_hammer_wrench.png\"></span></span></a><a id=\"EvilOnOff\" style=\"position:relative; top:-15px; left:155px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a><a id=\"PostOnOff\" style=\"position:relative; top:-15px; left:160px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.fb_icon + "\"></span></span></a><a id=\"HealOnOff\" style=\"position:relative; top:-15px; left:165px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/icon_health_16x16_01.gif\"></span></span></a><a id=\"StamOnOff\" style=\"position:relative; top:-15px; left:170px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/icon_stamina_16x16.png\"></span></span></a><a id=\"JobsOnOff\" style=\"position:relative; top:-15px; left:175px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"http://mwfb.static.zgncdn.com/mwfb/graphics/icon_energy_16x16_01.gif\"></span></span></a><a id=\"LogOnOff\" style=\"position:relative; top:-15px; left:-135px;\" class=\"sexy_button_new short black\"><span class=\"sexy_health_new\"><span><img src=\"http://zyngapv.hs.llnwd.net/e6/mwfb/graphics/inventory/WishlistDropDown/Inventory-wishlist-icon.png\"></span></span></a>";
				toolbar_div.innerHTML = toolbarhtml;
                document.getElementById("BankButtonB").addEventListener("click", Bank, false);
                document.getElementById("PostOnOff").addEventListener("click", togglePost, false);
                document.getElementById("HealOnOff").addEventListener("click", toggleHeal, false);
    ...
    function declare_war(arg) {
        ...
        if (parseInt(arg)) {
            ...
            if (!arg || arg == undefined) {
                ...
                war_target = Math.floor(Math.random() * b);
            }
        }
        var declare_url = "remote/html_server.php?xw_controller=war&xw_action=declare_war&xw_city=" + city + "&xw_person=" + /([\d]+)/.exec(xw_user)[1] + "&target_id=p%7C" + war_target + "&from=declare_war_page";
        declaring_war = true;
        do_ajax("inner_page", declare_url, 1, 1);
    }
        ...
        if (NYjobon || BKjobon || ILVjobon || Bjobon) {
            ...
            if (NYjobon) {
                ...
                if (ILVjobon) {
                    ...
                    if ([4, 6, 8, 13, 16, 25, 28, 31, 39, 42, 46, 49, 52, 59, 60, 61, 70, 74, 79, 82].indexOf(Job) !== - 1 &&
                        city == 6 ||
                        [5, 11, 12, 20, 23, 33, 41, 46, 51, 54, 56, 62, 65, 67, 72, 76, 78].indexOf(Job) !== - 1 &&
                        city == 5) {
    ...
    function Fightlist() {
        var i;
        if (fightActionRunning ||
            isFightPopOpen() ||
            parseInt(document.getElementById("user_stamina").innerHTML) <= 10 ||
            parseInt(document.getElementById("user_health").innerHTML) < healat) {
            return;
        }
        if (document.getElementsByClassName("impulse_buy_prompt contextual").length > 0 ||
            document.evaluate("//li[contains(@class, \"tab_on tab_middle\")]//a[contains(., \"Fighters\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0) {
            document.getElementById("unlockedStatus").innerHTML = "Re-Loading fightlist   ICES" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            do_ajax("inner_page", "remote/html_server.php?xw_controller=fight&xw_action=view&tab=0", 1, 1, 0, 0);
            return;
        }
        city = /mw_city(\d)/.exec(document.getElementById("mw_city_wrapper").className)[1];
        if (evilCity != city) {
            do_ajax("inner_page", "remote/html_server.php?xw_controller=travel&xw_action=travel&destination=" + evilCity + "&from=index&nextParams=&menu=travel", 1, 1, 0, 0);
        }
        fightActionRunning = true;
        if (autoBank) {
            Bank();
        }
        clearTimeout(fightwatchdogtimout);
        fightwatchdogtimout = setTimeout(resetfightaction, 15000);
        ...
        document.getElementById("unlockedStatus").innerHTML = "Fightlist\tI:" + icecount + "    K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
        var evt = document.createEvent("MouseEvents");
        evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
        fightlist[i].firstElementChild.dispatchEvent(evt);
    }

    Array.prototype.findIndex = function (value) {var ctr = "";for (var i = 0; i < this.length; i++) {if (this[i].pid == value) {return i;}}return ctr;};

    function Battle() {
        var targets;
        if (fightActionRunning ||
            isFightPopOpen() ||
            document.getElementById("user_stamina").innerHTML <= 0 ||
            parseInt(document.getElementById("user_health").innerHTML) < healat) {
            return;
        }
        if (document.getElementsByClassName("sexy_button_new short sexy_attack_new impulse_buy").length > 0) {
            var members = document.getElementsByClassName("fl");
            if (members.length - 1 > familytargets.length) {
                for (i = 1; i < members.length; i++) {
                    var pid = decodeID(/user=(.+?)\"/.exec(members[i].innerHTML)[1]);
                    if (familytargets.findIndex(pid) === "") {
                        familytargets[familytargets.length] = new familyMember(pid, "weak");
                    }
                }
            }
            targets = document.getElementsByClassName("sexy_button_new short sexy_attack_new impulse_buy");
            var targnum = Math.floor(Math.random() * targets.length);
            familyindex = familytargets.findIndex(targets[targnum].id.replace("btn_attack_p", "p|"));
            if (familytargets[familyindex].stat == "strong") {
                targets = document.getElementsByClassName("sexy_button_new sexy_page_refresh_new");
                var evt = document.createEvent("MouseEvents");
                evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
                ...
                var evt = document.createEvent("MouseEvents");
                evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
                targets[targnum].dispatchEvent(evt);
            }
        } else {
            if (document.getElementsByClassName("sexy_button_new short sexy_attack_new impulse_buy").length > 0) {
                targets = document.getElementsByClassName("sexy_button_new short sexy_attack_new impulse_buy");
    ...
    function Rivals() {
        ...
        if (document.getElementsByClassName("impulse_buy_prompt contextual").length > 0 ||
            document.evaluate("//li[contains(@class, \"tab_on tab_middle\")]//a[contains(., \"Rivals\")]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0) {
            document.getElementById("unlockedStatus").innerHTML = "Re-Loading fightlist   I" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            do_ajax("inner_page", "remote/html_server.php?xw_controller=fight&xw_action=view&tab=1", 1, 1, 0, 0);
            return;
        }
        city = /mw_city(\d)/.exec(document.getElementById("mw_city_wrapper").className)[1];
        if (evilCity != city) {
        ...
        fightwatchdogtimout = setTimeout(resetfightaction, 15000);
        var fightlist = document.getElementsByClassName("action");
        for (i = fightlist.length - 11; i > 1; i--) {
            var iced = fightlist[i].parentNode.firstElementChild.className;
            if (iced != "fight_list_player_dead") {
                if (!rivalcycle[i]) {
                    rivalcycle[i] = true;
        ...
        if (rivalcycle[1] == true) {
            for (var j = 0; j < rivalcycle.length; j++) {
                rivalcycle[j] = false;
            }
        }
        document.getElementById("unlockedStatus").innerHTML = "Attacking Rivals!!\tI:" + icecount + "    K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
        var evt = document.createEvent("MouseEvents");
        evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
        fightlist[i].firstElementChild.dispatchEvent(evt);
    }
    ...
    function FightFamily() {
        ...
        if (document.getElementsByClassName("impulse_buy_prompt contextual").length > 0 ||
            /Remove from Mafia/.test(document.body.innerHTML) ||
            familytargets[familyindex].stat == "strong") {
            document.getElementById("unlockedStatus").innerHTML = "Loading Next Family target...\tI:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            familyindex++;
            if (familyindex >= familytargets.length) {
                familyindex = 0;
            }
            loadProfile(familytargets[familyindex].pid);
            return;
        }
        ...
        if (/Sucker Punch/.test(document.body.innerHTML)) {
            ...
            target_name = /levels">\((.*?)\)/.exec(document.body.innerHTML)[1];
            if (icecheckfail) {
                document.getElementById("unlockedStatus").innerHTML = "Attacking " + target_name + "\tI:" + icecount + "    K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
                var elem = document.getElementById("btn_attack_" + familytargets[familyindex].pid.replace("|", ""));
                var evt = document.createEvent("MouseEvents");
                evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
                elem.dispatchEvent(evt);
            } else {
                document.getElementById("unlockedStatus").innerHTML = "Icechecking " + target_name + "   I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
                var params = {ajax: 1, liteload: 1, sf_xw_user_id: xw_user, sf_xw_sig: local_xw_sig};
                $.ajax({type: "POST", url: hitlist_url, timeout: 30000, data: params, success: function (msg) {if (/is already dead or too weak!/.test(msg)) {document.getElementById("unlockedStatus").innerHTML = target_name + " is dead...   I:" + icecount + "   K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;familyindex++;if (familyindex >= familytargets.length) {familyindex = 0;}loadProfile(familytargets[familyindex].pid);return;} else {if (/The action was not able to be completed/.test(msg)) {icecheckfail = true;setTimeout(clearicecheckfail, 45000);document.getElementById("unlockedStatus").innerHTML = "Zynga icecheck sucks....    I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;return;} else {if (/Sucker Punch/.test(document.body.innerHTML)) {document.getElementById("unlockedStatus").innerHTML = "Attacking " + target_name + "\tI:" + icecount + "    K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;var elem = document.getElementById("btn_attack_" + familytargets[familyindex].pid.replace("|", ""));var evt = document.createEvent("MouseEvents");evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);elem.dispatchEvent(evt);}}}}, error: function (req, status, err) {return;}});
            }
        } else {
            document.getElementById("unlockedStatus").innerHTML = "Loading next faily target...    I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            familyindex++;
            if (familyindex >= familytargets.length) {
                familyindex = 0;
            }
    ...
    function PureEvil() {
        ...
        if (document.getElementsByClassName("impulse_buy_prompt contextual").length > 0) {
            document.getElementById("unlockedStatus").innerHTML = "Re-Loading " + target_name + "\tI:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
        ...
        if (/Sucker Punch/.test(document.body.innerHTML)) {
            ...
            document.getElementById("unlockedStatus").innerHTML = "Icechecking " + target_name + "   I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            var params = {ajax: 1, liteload: 1, sf_xw_user_id: xw_user, sf_xw_sig: local_xw_sig};
            $.ajax({type: "POST", url: hitlist_url, timeout: 30000, data: params, success: function (msg) {if (/is already dead or too weak!/.test(msg)) {document.getElementById("unlockedStatus").innerHTML = target_name + " is dead...   I:" + icecount + "   K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;return;} else {if (/The action was not able to be completed/.test(msg)) {document.getElementById("unlockedStatus").innerHTML = "Zynga icecheck sucks....    I:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;return;} else {if (/Sucker Punch/.test(document.body.innerHTML)) {document.getElementById("unlockedStatus").innerHTML = "Attacking " + target_name + "\tI:" + icecount + "    K:" + killcount + "   T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;var elem = document.getElementById("btn_attack_" + targetid.replace("|", ""));var evt = document.createEvent("MouseEvents");evt.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);elem.dispatchEvent(evt);}}}}, error: function (req, status, err) {return;}});
        } else {
            document.getElementById("unlockedStatus").innerHTML = "Loading " + target_name + "\tI:" + icecount + "\tK:" + killcount + "    T:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
            setTimeout(PureEvil, 1500);
            loadProfile(targetid);
        }
    }

	function limitMaxHit(){
		if(document.getElementsByClassName("fightV2_result")[0]){
			if(/Wins: \d+/.exec(document.getElementsByClassName("fightV2_result")[0].innerHTML) && evilOn){
				var totalAttack = parseInt(/Wins: (\d+)/.exec(document.getElementsByClassName("fightV2_result")[0].innerHTML)[1]);
				var percent_hp = 100 - (parseInt(document.getElementById("defender_hp").style.width)/215*100);
				var percent_attack = totalAttack/MaxHit*100;
				
				if(MaxHit > 0 && percent_attack >= 20 && percent_hp < percent_attack){
					return false;
				}
	...
	function trappowerAttack(msg, context){
		FightV2.powerAttack(msg, context);

		var fight_data = msg.fight_result;
		if (fight_data.status == 'ok') {
			if(fight_data.attacker.health < healat)
				HealNY();	
			$('#defender_hp').html("<div style='position:absolute;top:25px;width:215px;left:6px;cursor:pointer'>"+calcuateHealth(fight_data)+"</div>");
		}
	}	
	
	var target_profile = "";
	var target_start_health_pct = 0;
	var target_prev_health_pct = 0;
	var target_hit = 0;
	var target_total_damage = 0;
	var target_total_other = 0;	
	var target_max_health = 0;	
	

	function calcuateHealth(fight_data){
		
		var current_health_pct = fight_data.defender.current_health_pct;
		var damage_dealt = fight_data.defender.damage_dealt;
		var profile_link = fight_data.defender.profile_link;
		var other_damage = fight_data.defender.other_damage;
		var this_hit = fight_data.power_attack.won;
		
		if(target_profile == profile_link){
			if(target_hit >= this_hit){
				//slow response
				if(target_max_health > 0){
					var cur_health = target_max_health*fight_data.defender.current_health_pct/100;
					return parseInt(cur_health) + "/"+ parseInt(target_max_health) + " ("+fight_data.defender.current_health_pct+"%)";	
				}
				
				return  "("+fight_data.defender.current_health_pct+"%)";
			}
			else{
				if(current_health_pct > target_prev_health_pct){
					target_start_health_pct = current_health_pct;
					target_prev_health_pct = current_health_pct;
					target_total_damage = 0;
					target_total_other = 0;
					target_hit = this_hit;
					...
					target_total_damage += damage_dealt;
					target_total_other += other_damage;
					target_hit = this_hit;
					
					var pct_damage = (target_start_health_pct - current_health_pct) + target_total_other;
					target_max_health = (100/pct_damage)* target_total_damage;
					var cur_health = target_max_health*current_health_pct/100;
					var calcualte_from_percent = damage_dealt / (target_prev_health_pct - (current_health_pct + other_damage)) * 100;
					
					target_prev_health_pct = current_health_pct;

					return parseInt(calcualte_from_percent*current_health_pct/100) + "/"+  parseInt(calcualte_from_percent) + " ("+current_health_pct+"%)";
					
					//return parseInt(cur_health) + "/"+ parseInt(target_max_health) + " ("+current_health_pct+"%)";
    ...
    function Fighter() {
        ...
        if (newfighton) {
            if (isFightPopOpen()) {
                ...
                if ((!/Lost/.test(elem.innerHTML)  || stamOn == 2) && limitMaxHit()) {
                    ...
                    if (document.getElementById("fightv2_poweratkbtn_boost_on").style.display == "block" ||
                        document.getElementById("fightv2_poweratkbtn_boost_off").style.display == "block") {
                        health = parseInt(document.getElementById("user_health").innerHTML);
                        if (health < healat) {
							HealNY();
                            return;
						...
						url = /remote\/html_server.+/.exec(url) + "";
						
						if(showHealth)
							var atkCallback = trappowerAttack;
						else
							var atkCallback = FightV2.powerAttack;
						
						do_ajax('inner_page', url, 1, 0, FightV2.prePowerAttack, atkCallback);
						return;
                    }
                }
                newfighton = false;
                if (!closing) {
	                if (icesteal) {
	                    if (document.getElementById("fv2_defender_overlay_stolen").style.display == "block") {
	                        if (document.getElementsByClassName("sexy_button_new medium red sexy_attack_new impulse_buy").length > 0) {
        ...
        if (hitlist) {
            ...
            client.open("POST", url, true);
            client.setRequestHeader("X-Requested-With", "XMLHttpRequest");
            client.setRequestHeader("Accept", "*/*"); /*Syntax Hilight Fix*/
            client.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            client.send("ajax=1&liteload=1&sf_xw_user_id=" + xw_user + "&sf_xw_sig=" + local_xw_sig + "&skip_req_frame=1");
            client.onreadystatechange = function () {if (this.readyState == 4) {response = client.responseText;document.getElementById("exp_to_next_level").innerHTML = /user_fields\['exp_to_next_level'\] = parseInt\("([\d]+)/.exec(response)[1];document.getElementById("user_cash_nyc").innerHTML = /user_cash_nyc'\] = "([$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_bangkok").innerHTML = /user_cash_bangkok'\] = "([B$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_italy").innerHTML = /'user_cash_italy'\] = "([L$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_vegas").innerHTML = /'user_cash_vegas'\] = "([V$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_brazil").innerHTML = /'user_cash_brazil'\] = "([BRL$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_chicago").innerHTML = /'user_cash_chicago'\] = "([\\u00a2\d,]+)"/.exec(response)[1];document.getElementById("user_stamina").innerHTML = /user_fields\['user_stamina'\] = parseInt\("([\d]+)/.exec(response)[1];document.getElementById("inner_page").innerHTML = response;}};
        }
    }
    ...
    function parseNewFightResults(response) {
        ...
        if (document.getElementById("fv2_defender_overlay_iced").style.display == "block") {
            if (/Get a (free )*boost', 'link': '(.+?)'/.test(response)) {
                icecount++;
		 
				eval( eval( "(" + response + ")").fight_result.feed_js.replace("MW.Feed(feed);","") );
				
				var icemsg = /(.+)( Need to whack someone| Think you can do better| Need help icing yours)/.exec(feed.description)[1];
				if(icemsg == null)
					icemsg = feed.description;
				
				toolbarLog +=icemsg + "\n";
				document.getElementById("logData").value = toolbarLog;
				
                document.getElementById("unlockedStatus").innerHTML = "I:" + icecount + "\t  K:" + killcount + "\tT:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
    ...
    function publishIce(response) {
        ...
        var mwfeed = eval("(" + /var feed = (.+?); MW\.Feed/.exec(feed)[1] + ")");
        if (autopostOn) {
            mwfeed.actionLinks[0].name = mwfeed.actionLinks[0].name.replace("Get a boost", "Get a Boost");
        }
        if (fbfeedloc != "") {
            mwfeed.targetId = fbfeedloc;
        }
        MW.Feed(mwfeed);
    }
    ...
    function DoAutoHeal() {
        ...
        client.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        client.send("ajax=1&liteload=1&sf_xw_user_id=" + xw_user + "&sf_xw_sig=" + local_xw_sig + "&skip_req_frame=1");
        client.onreadystatechange = function () {if (this.readyState == 4) {response = client.responseText;try {CurHealth = User.health = document.getElementById("user_health").innerHTML = /user_health":([\d]+)."/.exec(response)[1];} catch (err) {}}};
    ...
    function Bank() {
        ...
        client.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        client.send("ajax=1&liteload=1&sf_xw_user_id=" + xw_user + "&sf_xw_sig=" + local_xw_sig + "&skip_req_frame=1");
        client.onreadystatechange = function () {if (this.readyState == 4) {response = client.responseText;if (/user_cash_nyc":"([$\d,]+)"/.test(response)) {document.getElementById("user_cash_nyc").innerHTML = /user_cash_nyc":"([$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_bangkok").innerHTML = /user_cash_bangkok":"([B$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_italy").innerHTML = /user_cash_italy":"([L$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_vegas").innerHTML = /user_cash_vegas":"([V$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_brazil").innerHTML = /user_cash_brazil":"([BRL$\d,]+)"/.exec(response)[1];document.getElementById("user_cash_chicago").innerHTML = "\xA2" + /user_cash_chicago":"\\u00a2([\d,]+)"/.exec(response)[1];document.getElementById("user_cash_london").innerHTML = "\xA3" + /user_cash_london":"\\u00a3([\d,]+)"/.exec(response)[1];}}};
    }
    ...
    function HealNY() {
		if(!healOn)
 		...
 		function healCB(resp) {
			try {
				document.getElementById("user_health").innerHTML = resp.current_health;
				
				if(resp.hospital_success == 1){
					var healed = parseInt(/\d+ health/.exec(resp.hospital_message));
					if(resp.max_health - healed < 20)
						heal_red_count++;
					else
						heal_green_count++;
		...
		do_ajax('', 'remote/html_server.php?xw_controller=hospital&xw_action=heal&xw_city=5&tmp=51ef09aec25ceb77ce9dc33262de5105&cb=337400c06fff11e1a6b94dd4beddc7c9&xcity=1', 1, 0, null, healCB);
    }
		 ...
		 function healCB(resp) {
			try {
				...
				if(resp.hospital_success == 1){
					...
					document.getElementById("unlockedStatus").innerHTML = "Hospital heal! I:" + icecount + "\t  K:" + killcount + "\tT:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
				}
			} 
			catch (err) {}
		};
	...
	function quickPowerpack(){
		function packCB(resp){
				try {
					if(resp.hospital_message == "1"){
						document.getElementById("unlockedStatus").innerHTML = "PowerPack heal! I:" + icecount + "\t  K:" + killcount + "\tT:" + (icecount + killcount * 2) + " | GH:" +  heal_green_count + " RH:" + heal_red_count;
					}
				} 
				catch (err) {}
		};
		
		do_ajax('', 'remote/html_server.php?xw_controller=module&xw_action=usePowerPack&responseType=hospital&packType=1', 1, 0, null, packCB);
	}
    ...
    function Settings() {
        var version = 329;
        var postsetting = fbfeedloc;
        popupTitle = "Kowalski UnlockedMW Settings";
        content = "<br>Post ices to  <input value=\"" + postsetting + "\" type=\"text\" style=\"resize:none;width:90px;\" id=\"postformid\"><br><br>"+
					"<input type=\"checkbox\" id=\"autopost\" name=\"autopost\" value=\"Checked\"> Autopost Ices<br><br>"+
					"<input type=\"checkbox\" id=\"followstolen\" name=\"followstolen\" value=\"Checked\"> Attack Ice Stealers<br><br>"+
					"<input type=\"checkbox\" id=\"burst\" name=\"burst\" value=\"Checked\"> Burst Attacks<br><br>"+
					"<input type=\"checkbox\" id=\"showHealth\" name=\"showHealth\" value=\"Checked\"> Calculate Enermy Health<br><br>"+
					"<input type=\"checkbox\" id=\"autobank\" name=\"bankit\" value=\"Checked\"> Autobank when slayer running<br><br>"+
					"<div style='display:none'>"+
						"<input type=\"checkbox\" id=\"remchat\" name=\"removechat\" value=\"Checked\"> Remove Clan Chat<br><br>"+
						"<input type=\"checkbox\" id=\"pausecheck\" name=\"pausecheck\" value=\"Checked\">Pause at XP remaining <input value=\"" + stopat + "\" type=\"integer\" style= \"resize:none;width:50px;\" id=\"postformid3\"><br><br>"+
											"Robbing Refresh Rate <input value=\"" + rref / 1000 + "\" type=\"integer\" style= \"resize:none;width:35px;\" id=\"postformid5\"><br><br>"+
					"</div>"+
					"Slayer Max Hit <input value=\"" + MaxHit + "\" type=\"integer\" style= \"resize:none;width:50px;\" id=\"inputMaxHit\"><br><br>"+
					"Heal Threshold <input value=\"" + healat + "\" type=\"integer\" style= \"resize:none;width:50px;\" id=\"postformid2\"><br><br>"+
					"<input type=\"checkbox\" id=\"autoclose\" name=\"autoclose\" value=\"Checked\"> Auto Close Fight Popup<br><br>"+
					"<a id=\"SaveSettings\" class=\"sexy_button_new black\"><span><span>Save</span></span></a><br><br><a id=\"CustomBM\" class=\"sexy_button_new black\"><span><span>Customize Tools</span></span></a><br><br></div>";
        
		height = "560";
		
        myPop("UnlockedSettings", popupTitle, content, height);
        document.getElementById("SaveSettings").onclick = saveSettings;
        document.getElementById("CustomBM").onclick = display_custom_bm_array;
        if (icesteal) {
        ...
        if (showHealth) {
            document.getElementById("showHealth").checked = true;
        }
    }
    ...
    function saveSettings() {
		...
		MaxHit = parseInt(document.getElementById("inputMaxHit").value);
        ...
        if (document.getElementById("autoclose").checked == true) {
            autoclose = true;
        } else {
            autoclose = false;
        }
        if (document.getElementById("showHealth").checked == true) {
            showHealth = true;
        } else {
            showHealth = false;
        }
    ...
    function myPopupClose() {
        document.getElementById("popup_fodder").removeChild(document.getElementById("myPopup"));
    }
    ...
    function startup() {
        ...
        if (burstOn == "true" || burstOn == true) {
            burstOn = true;
        } else {
            burstOn = false;
        }
        if (autopostOn == "true" || autopostOn == true) {
            autopostOn = true;
        } else {
            autopostOn = false;
        }
        if (autoBank == "true" || autoBank == true) {
            autoBank = true;
        } else {
            autoBank = false;
        }
        if (removeChat == "true" || removeChat == true) {
            removeChat = true;
        } else {
            removeChat = false;
        }
        ...
        if (showHealth == "true" || showHealth == true) {
            showHealth = true;
        } else {
            showHealth = false;
        }
    ...
    function CloseDoopidPopup() {
        if ($(".pop_bg").length > 0) {
            $(".pop_bg").each(function () {var id = this.id;MW.Popup.hide(id.substr(id.lastIndexOf("_") + 1));});
        }
    }
    ...
    function LetThereBeRobberies() {
        ...
        if (DoesIHaveStam < 50) {
            ...
            alert("Sorry you need more stamina to clear the board!\n\nResults : \n\n Start Stamina : " + currstam + " \n\n End Stamina : " + anystam + "\n\n Stamina Spent : " + (currstam - anystam) + "\n\n Experience Gained : " + (currexp - anyexp) + "\n\n Exp Ratio achieved : " + XPratio + " \n\n***Please note if the stats above look wack its because you leveled halfwaythru and threw the hardcoded calculators outta wack***");
        } else {
            for (var i = 0; i < 10; i++) {
                do_ajax("", "remote/html_server.php?xw_controller=robbing&xw_action=rob&xw_city=" + evilCity + "&slot=" + i, 1, 0, RobbingController.preRob(i), RobbingController.rob);
            }
    ...
    $("body").ajaxComplete(function (e, xhr, settings) {
		...
		if (/local_xw_sig = '([\da-f]+)';/.test(response)) {
			local_xw_sig = /local_xw_sig = '([\da-f]+)';/.exec(response)[1];
		}
		
		if (ajax) {ajax = false;return;}
		if (fightActionRunning) {fightActionRunning = false;clearTimeout(fightwatchdogtimout);}
		if (document.getElementById("attacker_fight_status")) {parseNewFightResults(response);}
		
		if (document.evaluate("//a[@class='sexy_button_new short red sexy_attack_new'][contains(string(),'Attack Again')]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {hitlist = true;} 
		else {if (hitlist) {hitlist = false;}}
		
		if (document.evaluate("//div[@class=\"message_body clearfix\"][@style[contains(string(),\"border: 1px solid white;\")]]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {NYjobon = true;} else {if (NYjobon) {NYjobon = false;}}
		if (/Do This Job Again/.test(document.getElementById("inner_page").innerHTML)) {BKjobon = true;} else {if (BKjobon) {BKjobon = false;}}
		if (document.evaluate("//div[@class=\"job_info\"][@style[contains(string(),\"255, 255, 255\")]]", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0 && document.getElementsByClassName("buy_prompt").length == 0) {ILVjobon = true;} else {if (ILVjobon) {ILVjobon = false;}}
		if (/jobResult":{"city":7/.test(response) || /jobResult":{"city":8/.test(response) | /jobResult":{"city":9/.test(response)) {
			if (/jobResult":{"city":7/.test(response)) {Bjobid = /jobResult":{"city":7,"id":(\d+),/.exec(response)[1];}
			if (/jobResult":{"city":8/.test(response)) {Bjobid = /jobResult":{"city":8,"id":(\d+),/.exec(response)[1];}
			if (/jobResult":{"city":9/.test(response)) {Bjobid = /jobResult":{"city":9,"id":(\d+),/.exec(response)[1];}			
			Bjobon = true;} 
		else {if (Bjobon) {Bjobon = false;}}
		
		
		
		
		
		if (declaring_war == true && /<!-- Current Page: war_controller ([\d]+) -->/.test(response)) {if (/their war status/.test(response)) {setTimeout(function () {declare_war(null);}, 0);} else {if (/too low of a level to declare war/.test(response)) {setTimeout(function () {declare_war(null);}, 0);} else {if (/are already at war/.test(response)) {declaring_war = false;} else {if (/user not found/i.test(response) || /does not exist/i.test(response)) {setTimeout(function () {declare_war(null);}, 0);} else {setTimeout(function () {var pop_id = /([\d]+)/.exec(document.getElementsByClassName("sexy_button_new short white sexy_call_new")[0].getAttribute("onclick"))[1];MW.Feed(postFeedAndSendCallForHelpNew);$("#declare_war .sexy_call_new").css("display", "none");MW.Popup.hideAndRemove(pop_id);declaring_war = false;}, 1500);document.getElementById("EvilOnOff").setAttribute("class", "sexy_button_new short black");document.getElementById("EvilOnOff").innerHTML = "<span class=\"sexy_health_new\"><span><img src=\"" + unlocked.images.evil_icon + "\"\"></span></span></a>";evilOn = false;}}}}return;}});
    ...
    function add_bm_to_array() {
        if (document.getElementById("Unlocked_User_BM_locale").value.length < 1) {
            alert("Locale is empty!");
            UnlockedAddBM();
            return;
        }
        if (!/http(.+)js/.test(document.getElementById("Unlocked_User_BM_locale").value)) {
            alert("Locale is not valid!");
            UnlockedAddBM();
            return;
        }
        if (document.getElementById("Unlocked_User_BM_alias").value.length < 1) {
            alert("Alias is empty!");
            UnlockedAddBM();
            return;
        }
        locale = /http(.+)js/.exec(document.getElementById("Unlocked_User_BM_locale").value)[0];
        alias = escape(document.getElementById("Unlocked_User_BM_alias").value);
        try {
            if (document.getElementById("UnlockedAddCustomBM")) {
                document.getElementById("popup_fodder").removeChild(document.getElementById("UnlockedAddCustomBM"));
            }
        } catch (e) {
        }
        var testArray = isArray(unlocked.user_bm_locale);
        if (testArray == false) {
            unlocked.user_bm_locale = new Array;
        }
        testArray = isArray(unlocked.user_bm_alias);
        if (testArray == false) {
            unlocked.user_bm_alias = new Array;
        }
        var a = unlocked.user_bm_locale.length;
        unlocked.user_bm_locale[a] = locale;
        unlocked.user_bm_alias[a] = alias;
        save_bm_array_to_localstorage();
    }


    function save_bm_array_to_localstorage() {
        localStorage.setItem("unlocked_user_bm_length", unlocked.user_bm_locale.length);
        for (var i = 0; i < unlocked.user_bm_locale.length; i++) {
            localStorage.setItem("unlocked_user_bm_locale_" + i, unlocked.user_bm_locale[i]);
            localStorage.setItem("unlocked_user_bm_alias_" + i, unlocked.user_bm_alias[i]);
        }
    ...
    function remove_user_bm() {
        ...
        try {
            if (document.getElementById("UnlockedCustomBM")) {
                document.getElementById("popup_fodder").removeChild(document.getElementById("UnlockedCustomBM"));
            }
        } catch (e) {
        }
        localStorage.removeItem("unlocked_user_bm_locale_" + a);
        localStorage.removeItem("unlocked_user_bm_alias_" + a);
        var a = parseInt(localStorage.getItem("unlocked_user_bm_length"));
        for (var i = item_value; i < a; i++) {
            unlocked.user_bm_alias[item_value] = unlocked.user_bm_alias[item_value + 1];
            unlocked.user_bm_locale[item_value] = unlocked.user_bm_locale[item_value + 1];
            localStorage.setItem("unlocked_user_bm_alias_" + i, localStorage.getItem("unlocked_user_bm_alias_" + (i + 1)));
            localStorage.setItem("unlocked_user_bm_locale_" + i, localStorage.getItem("unlocked_user_bm_locale_" + (i + 1)));
        }
        localStorage.setItem("unlocked_user_bm_length", a - 1);
        display_custom_bm_array();
    }
    ...
    function edit_user_bm() {
        ...
        unlocked.user_bm_alias[item_value] = escape(document.getElementById("Unlocked_User_BM_alias").value);
        unlocked.user_bm_locale[item_value] = escape(document.getElementById("Unlocked_User_BM_locale").value);
        localStorage.setItem("unlocked_user_bm_alias_" + item_value, unlocked.user_bm_alias[item_value]);
        localStorage.setItem("unlocked_user_bm_locale_" + item_value, unlocked.user_bm_locale[item_value]);
        display_custom_bm_array();
    }
    ...
    function display_custom_bm_array() {
        try {
            if (document.getElementById("UnlockedSettings")) {
                document.getElementById("popup_fodder").removeChild(document.getElementById("UnlockedSettings"));
            }
        } catch (e) {
        }
        try {
        ...
        var i;
        for (i = 0; i < a; i++) {
            b += "<tr><td> " + (i + 1) + ": </td><td title=\"" + unescape(localStorage.getItem("unlocked_user_bm_locale_" + i)) + "\"> " + unescape(localStorage.getItem("unlocked_user_bm_alias_" + i)) + " </td><td width=\"40\"> <a class=\"edit_user_bm_pop\" id=\"" + i + "\" onclick=\"" + c + "\">edit?</a></td><td width=\"60\"> <a class=\"remove_user_bm\" id=\"" + i + "\" onclick=\"" + c + "\">remove?</a></td></tr>";
        }
        b += "</table></div>";
        b += "<a class=\"sexy_button_new black\" id=\"addBM\"><span><span>Add Bookmarklets</span></span></a><a class=\"sexy_button_new black\" id=\"restoreBM\"><span><span>Restore Originals</span></span></a><br><br>Copy this to a text file to save a backup:<br><textarea rows=\"8\" cols=\"40\" id=\"backupBMtext\">" + unlocked.user_bm_alias.join("|") + "##" + unlocked.user_bm_locale.join("|") + "</textarea><br><a class=\"sexy_button_new black\" id=\"backupBM\"><span><span>Restore Backup</span></span></a>";
        myPop("UnlockedCustomBM", "Current custom bookmarklets", b, d, "600");
        for (var r = 0; r < i; r++) {
        ...
        document.getElementById("addBM").onclick = UnlockedAddBM;
    ...
    function restore_custom_bms() {
        var count = parseInt(localStorage.getItem("unlocked_user_bm_length"));
        for (var i = 0; i < count; i++) {
            localStorage.removeItem("unlocked_user_bm_locale_" + i);
            localStorage.removeItem("unlocked_user_bm_alias_" + i);
        }
