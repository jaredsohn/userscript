...
function getLocation (statusPanel) {
    ...
    if (sector.childNodes[0].nodeType == 1) {
        name = sector.childNodes[0].innerHTML;
        coords = loc.childNodes[0].innerHTML;
    } else {
        name = sector.innerHTML;
        coords = loc.innerHTML;
    }
...
unsafeWindow.document.addEventListener ("keydown", function (evt){pr_hotkey(evt);}, true); // The D key
...
function markCurrentNode () {
    ...
    if (current) {
        if (!old) {
            ...
            old.appendChild(document.createTextNode (" < you're here"));
        }
...
function deleteAvailablePath () {
    ...
    for (var i=0; i<rows.childNodes.length; i++) {
        if (rows.childNodes[i].getAttribute("startloc") == userloc && rows.childNodes[i].getAttribute("startuni") == universe && rows.childNodes[i].getAttribute("filename") == filename) {
            ...
            GMcreateCookie ("pr_paths", div.innerHTML);
            readPathsHeader ();
            showAvailablePaths ();
            return;
        }
...
function showAvailablePaths (showAll) {
    ...
    // Check if there is any saved header
    var header = readCookie("pr_paths_header");
...
function readPathsHeader () {
    ...
    // Save the header
    createCookie ("pr_paths_header", div.innerHTML);
...
function buildRecorder () {
    ...
    var toptable = document.getElementById ("pr_top_table");

    // Create the recorder's content and install it
    var row = document.createElement ("tr");
    toptable.firstChild.appendChild (row);
    row.innerHTML = "<td></td>";
    var rec = row.firstChild;

    // Create the recorder content
    rec.innerHTML = "<table id=pr_recorder cellpadding=0 cellspacing=0 style=\"display: none;\">\
                        <tbody>\
                            <tr id=pr_top_menu>\
                                <td>\
                                    <table>\
...
function showLink() {
    ...
    div.innerHTML = "<tbody><tr><td></td></tr></tbody>";
    div.setAttribute ("style", "border: 2px ridge white;");
    div.setAttribute ("cellpadding", "2");
    div.setAttribute ("cellspacing", "0");
    div.id = "pr_top_table";
    var cell = div.firstChild.firstChild.firstChild;
...
//    if (unsafeWindow.ajax) {
	cell.innerHTML = "<a id=pr_link title='Open/Close the path recorder'>Path recorder +</a> (<span id=pr_current_node_index>?</span>/<span id=pr_nodes_recorded>0</span> nodes) <span><a id=pr_go_next href=\"javascript:pr_go_next()\" title='Moves your ship to the next tile in the current path. You can use the \"d\" key instead of clicking.'>Go next</a></span>";
	cell.firstChild.addEventListener("click", showRecorder, true);
//    } else {
	// Quit in case the Partial refresh is not set.
//	cell.innerHTML = "<a href='http://www.pardus.at/index.php?section=account_settings' style='background-color:yellow;color:red;font-weight:bold;'>PathRecorder can only be used with 'Partial Refresh' set to Yes in your account settins.</a>";
//	return;
//    }
    
    // Cache the saved paths into the local storage
    GMreadCookie ("pr_paths", true);
