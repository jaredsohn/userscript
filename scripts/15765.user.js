...
function post_comment(){
	...
	if (dd.getAttribute('tag_caller_entry_id') == this.id) // hide-unhide
        ...
        tags = this.getAttribute('tagList')
		dd.firstChild.value = tags
		dd.style.display = 'block'
	} 
...
function trigger_submit_on_ctrl_enter(e) {
    ...
    newTags.sort();
    GM_setValue('thisEntryTags', encodeURI(newTags.join(',')));
    ...
    reply.setAttribute('tagList', this.value);
	...
	GM_xmlhttpRequest({
		...
		GM_xmlhttpRequest({
			...
			onload: function(r) {
                var allTags = r.responseText.match(/\/tag\/[^<]*<\/a>/g)
                if (allTags != null) {
                    ...
                    newTags.sort()
                    var oldTags = decodeURI(GM_getValue('thisEntryTags'));
...
allReplies = document.evaluate(
    "//a[contains(@href, '?mode=reply') or contains(@href, '/tag/')]",
    document,
    null,
...
for (var i = 0; i < allReplies.snapshotLength; i++) {
    ...
    if (thisOne.href.indexOf('/tag/') > -1) {
        if (tagList == '')
            tagList = thisOne.innerHTML
        else
            tagList = tagList + ', ' + thisOne.innerHTML
    } else {
...
//        tags.setAttribute('value', tagList)
//        tagList = ''
        ...
        //if(journal == journal_username)
        {
            ...
            a.setAttribute('tagList', tagList)
            tagList = ''
