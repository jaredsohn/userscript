// ==UserScript==
// @name           L Pagerize
// @namespace      userL.l_pagerize
// @description    Show stories and series on single page
// @include        http://www.l*a.com/stories/showstory.php?id=*
// ==/UserScript==
//
// author: userL
// version: 0.0.2
...
if( isFirstPage(document) ) {
		...
		var temp = document.getElementById('storypager');
		if (!temp) temp=document.getElementById('sharethis').previousSibling
		processDocument(document, temp.previousSibling, true, 1);
}
  ...
  var content;
	GM_xmlhttpRequest({
		  ...
		  onload: function(response) {
				...
				if(processSeries && doc.getElementById('seriesstories')) {
				if (DEBUG) GM_log('Begin Process Series');
					var seriesLinks = doc.getElementById('seriesstories').getElementsByTagName('a');
					...
					if(seriesLinks[0].innerHTML.substr(seriesLinks[0].innerHTML.length-1,1) == '2' || (seriesLinks[0].innerHTML.substr(seriesLinks[0].innerHTML.length-1,1) == '1' && seriesLinks[1].innerHTML.substr(seriesLinks[1].innerHTML.length-1,1) == 2)) {
						for(y=seriesLinks.length-1; y>-1; y--) {
							...
							if (DEBUG) GM_log('Begin Process Chapter ' + y+2);
							newStory = document.createElement('p');
							newStory.id = 'story' + COUNTER;
							COUNTER += 1;
							newStory.innerHTML = '<br/><u><b>Ch ' + (y+2) + ' Part 1</b></u><br/><br/>';
							inStory.parentNode.insertBefore(newStory, inStory.nextSibling);
							if (DEBUG) GM_log('Requesting First Page, Chapter = ' + y+2);
							insertRequest(newStory, seriesLinks[y].toString(), false, y+2);
						}
					}
				}
				...
				for(z=0; z<divs.length; z++) divs[z].parentNode.removeChild(divs[z]);
...
function isFirstPage( inDoc ) {
	var prev = inDoc.getElementById('prevpage');
	if (!prev || prev.getElementsByTagName('a').length == 0) {
		return true;
	}
	else {
		return false;
...
function getPageNavArray( inDoc ) {
	var temp = inDoc.getElementById('page_nav')
	if(temp) return temp.getElementsByTagName('a'); else return null;
}
