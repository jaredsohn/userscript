// ==UserScript==
// @author		usr8472@gmail.com  based on luckyman script, modified by tomasz.frelik (at) enzo.pl, modified by Otyugh with resource overflow based on wannes 
// @namespace	http://userscripts.org/
// @name		Travian: Resource++ v1
// @description	Travian Page Resource++ v1
...
newmenu += "<tr class=rbg><td colspan=2>Resource Meters</td></tr>";

newmenu += "<tr><td width=270>"+out[0]+"</td><td>"+out[1]+"</td></tr><tr><td width=270>"+out[2]+"</td><td>"+out[3]+"</td></tr>";

newmenu += "<tr>";
newmenu += "<td colspan=2 class=rbg>Resource Bars</td>";
newmenu += "</tr>";
...
//if you want you can add other page specific pos info, if you want you can append to body but you would need to change top for certain pages
	    ...
	    if(store>0){ 
			if(prod > 0) {
				secFull = (3600*(storeMax-store)/prod);
		  	    out[i]="Overflow: <span id=res"+i+" >"+formatORG(Math.ceil(Math.max(0,secFull)))+" </span>";
			}  else if(prod[i] < 0) {
			    secFull = (3600*-store/prod);
				out[i]="Depleted: <span id=res"+i+" >"+formatORG(Math.ceil(Math.max(0,secFull))) +" </span>";
			} else {
