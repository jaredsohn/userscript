...
function insertTag(e, j) {
	...
	if(e.target.selectionStart || e.target.selectionStart == '0') {
		...
		var selEnd = e.target.selectionEnd;
                ...
                //Check if it's a word count and handle it
               if (shortcuts[j][0] == "Alt+C")
               {
                   var messages;
                   if (selSta == selEnd) //Count everything in box
                   {
                      messages = "Words: " + e.target.value.split(/\s+/).length + "\n" + "Characters: " + e.target.value.length;
                      alert (messages);
                      return;
                   }
                   messages = "Words: " + e.target.value.substring(selSta, selEnd).split(/\s+/).length + "\n" + "Characters: " + e.target.value.substring(selSta, selEnd).length;
                   alert (messages);
                   return;
                }
...
function addShortcuts() {
	...
	for(i = 0; i < elements.length; i++) {
		//for(var j = 0; j < shortcuts.length; j++) {
            ...
            shortcut.add(shortcuts[0][0], function(e) { insertTag(e,0); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[1][0], function(e) { insertTag(e,1); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[2][0], function(e) { insertTag(e,2); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[3][0], function(e) { insertTag(e,3); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[4][0], function(e) { insertTag(e,4); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[5][0], function(e) { insertTag(e,5); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[6][0], function(e) { insertTag(e,6); }, { 'target' : elements[i] });
             shortcut.add(shortcuts[7][0], function(e) { insertTag(e,6); }, { 'target' : elements[i] });
...
function htmlInsertInit() {
	 ...
	 * http://www.openjs.com/scripts/events/keyboard_shortcuts/
	 */
	shortcuts = [['Ctrl+B', '[B]', '[/B]'],
				 ['Ctrl+I', '[I]', '[/I]'],
				 ['Ctrl+U', '[U]', '[/U]'],
				 ['Ctrl+S', '[B]', '[/B]'],
				 ['Ctrl+Shift+M', '<a href="|">', '</a>'],
				 ['Ctrl+P', '<p>', '</p>'],
				 ['Alt+C', 'Count the words', 'Count the words'],
                                 ['Ctrl+2', '<h2>', '</h2>'],];
