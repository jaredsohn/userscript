...
// @author         No M3rcy|Chisum|Michael Richter
// @version        3.0
...
//game_data deffinieren
	var game_data = typeof unsafeWindow != 'undefined' ? unsafeWindow.game_data : window.game_data;
    var $ = typeof unsafeWindow != 'undefined' ? unsafeWindow.$ : window.$;
// DS_Version ermitteln
	var ds_version = game_data.version.match(/\d+ ((\d+)\.(\d+))/)[1]
	var api = typeof unsafeWindow != 'undefined' ? unsafeWindow.ScriptAPI : window.ScriptAPI;
			api.register( 'Massrecruitment',  (ds_version), 'No M3rcy|Chisum', 'no.m3rcy@gmx.net' );
	var UI = typeof unsafeWindow != 'undefined' ? unsafeWindow.UI : window.UI;

//Server Sprache ermitteln
	var curlang = game_data.market;
	//var curlang = 'de'; // entfernen!!!
	var ui_error = UI.ErrorMessage;

	curlang == 'de' ? main() :	ui_error('Das Script \'Massrecruitment\' ist für Deine Sprachversion gesperrt!');
...
function main(){
	...
	var lang = {
		'de': {
			...
			'farmfreedesc': 'Wie viele BH-Plätze sollen frei gelassen werden (z.B. 100 für ein AG).',
			'resfree': 'Rohstoffe übrig lassen:',
			'resfreedesc': 'Wie viele Rohstoffe sollen mindestens übrig bleiben.',
			'save': 'Speichern',
			'fill': 'Ausfüllen',
			'recruit': 'Rekrutieren',
			'running': 'läuft...',
...
// Ingame Massrek. Tabelle ausblenden
		win = typeof unsafeWindow != 'undefined' ? unsafeWindow : window;
		$('#mr_all_form').parent().hide();
...
// save settings as cookie
	var set_cookie = function(arr) {
		{
			var value = escape(JSON.stringify(arr));
			if(win.localStorage) {
			...
			var mr_buffer_pop = $('#massrecruitfree').val();
            $('[name="buffer_pop"]').val(mr_buffer_pop)
			$('#villages_selected + input').click();
		}
	};
	var get_cookie = function() {
		var massrecruiting = /massrecruiting=(.*?)(?:;|$)/.exec(document.cookie);
		if(massrecruiting) {
			var value = JSON.parse(unescape(massrecruiting[1]));
			if(win.localStorage) {
			...
			return JSON.parse(unescape(win.localStorage.massrecruiting));
		}
		return [];
	};
...
// get settings from the document (w/o saving)
	var getCurrentSettings = function(group) {
		...
		var setunits = $('[name^="massrecruitunit_"]');
		//var setopenend = $x('//input[starts-with(@name,"massrecruitopenend_")]');
		var setopenend = $('[name^="massrecruitopenend_"]');
		var cursettings = {
			group: group,
			units: [],
			openend: [],
...
// update needed workers for selected troups
	var updateWorkers = function() {
		...
		var curset = getCurrentSettings(current_group);
		var neededworkers = 0;
...
// get available groups
	current_group = game_data.village.group;
	current_group_name = $('.vis_item strong').html().replace(/\>/, "").replace(/\</, "");
			
// get settings
	var settings = get_cookie();
		
	var recform = document.getElementById("mass_train_form");
	
// get available units
	var units = [];
	var u = $x('table[1]/thead/tr[1]/th/img[contains(@src,"unit_")]', recform);
	//var u = win.$('#mass_train_table th img');
...
// settings table
	var grpidx = Array.searchgroup(settings, current_group);
	var setcon = ce('div');
	setcon.setAttribute('style', 'padding:5px;');
	var h3 = ce('h3');
	h3.textContent = lang[curlang]['settings'] + ' »'+current_group_name+'«';
...
// save settings
	btn.addEventListener('click', function(){
		var currentsettings = getCurrentSettings(current_group);
		if(grpidx < 0) {
			settings.push(currentsettings);
			grpidx = Array.searchgroup(settings, current_group);
		} else {
...
// calculate and fill
		...
		var currentsettings = getCurrentSettings(current_group);
		var rows = recform.getElementsByTagName("tr");//$x('//form[contains(@action,"screen=train") and contains(@action,"mode=success") and contains(@action,"action=train_mass") and contains(@action,"h=")]/table[1]/tbody/tr/td/parent::tr');
		//var rows = win.$('#mass_train_table tr');
		var builtsum = [];
		for(var i = 0; i < units.length; i++) {
			builtsum[i] = 0;
		}
		...
		for(var i = 0; i < rows.length; i++) {
			...
			var cells = $('#mass_train_table tr:eq(' +i+ ') td');
			if(cells.length > 0) {
// get resources
...
// click recruiting button
		var submit = $x('input[@type="submit"]', recform);
		if(submit.length > 0)
			submit[0].click();
	}, false);
	td.appendChild(btn);
	...
	//recform.parentNode.insertBefore(ce('hr'), recform);
	recform.parentNode.insertBefore(setcon, recform);
	//recform.parentNode.insertBefore(ce('hr'), recform);
	if(grpidx > 0)
		updateWorkers();
