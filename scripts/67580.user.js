...
//          which will degrade gradually the script.
...
(function() {
	...
	if(head) {
		...
		// Naming some tables (putting IDs)
		var tables=document.getElementsByTagName('table');
		// I will suppose I will get two tables
		tables[0].setAttribute('id','connectionstable');
		var connectionstable=tables[0];
		tables[1].setAttribute('id','connectionsheadtable');
		var connectionsheadtable=tables[1];
		tables=false;
		...
		var temp=connectionsheadtable;
		do{
			temp=temp.previousSibling;
		}while( temp && temp.nodeType!=1);
		// Firefox bug: Node.ELEMENT_NODE is not defined when this script is run.
		//     Node, however, is already a Node object.
		//     http://www.mozdev.org/bugs/show_bug.cgi?id=13193
		//while( temp && temp.nodeType!=Node.ELEMENT_NODE);
		if( temp && temp.tagName.toLowerCase() == 'hr' )
			temp.parentNode.removeChild(temp);

		// Trying to style each column separately
		var firstchild=connectionstable.firstChild;
		var connectionstablecolumns=new Array();
		for( var i = 0; i < 13; i++ ) {
			var temp=document.createElement('colgroup');
			connectionstablecolumns[i]=temp;
			connectionstable.insertBefore(temp,firstchild);
		}
		connectionstablecolumns[0].setAttribute('align','left'); // Srv
		connectionstablecolumns[1].setAttribute('align','right'); // PID
		connectionstablecolumns[2].setAttribute('align','center'); // Acc
		connectionstablecolumns[3].setAttribute('align','center'); // M
		connectionstablecolumns[4].setAttribute('align','right'); // CPU
		connectionstablecolumns[5].setAttribute('align','right'); // SS
		connectionstablecolumns[6].setAttribute('align','right'); // Req
		connectionstablecolumns[7].setAttribute('align','right'); // Conn
		connectionstablecolumns[8].setAttribute('align','right'); // Child
		connectionstablecolumns[9].setAttribute('align','right'); // Slot
		connectionstablecolumns[10].setAttribute('align','right'); // Client
		connectionstablecolumns[11].setAttribute('align','center'); // VHost
		connectionstablecolumns[12].setAttribute('align','left'); // Request

		// Creating/getting the THEAD and TFOOT
		var connectionstablethead=connectionstable.getElementsByTagName('thead')[0];
		if(!connectionstablethead) {
			connectionstablethead=document.createElement('thead');
			connectionstable.insertBefore(connectionstablethead,firstchild);
		}
		var connectionstabletfoot=connectionstable.getElementsByTagName('tfoot')[0];
		if(!connectionstabletfoot) {
			connectionstabletfoot=document.createElement('tfoot');
			connectionstable.insertBefore(connectionstabletfoot,firstchild);
		}
		// Moving headers to THEAD
		connectionstablethead.appendChild(connectionstable.getElementsByTagName('tr')[0]);
		// Copying headers to TFOOT
		connectionstabletfoot.appendChild(connectionstablethead.firstChild.cloneNode(true));
		// Making it sortable
		connectionstable.setAttribute('class', 'sortable');

		// Transforming a paragraph in a table
		var scoreboardtable=false;
		var scoreboard=connectionstable;
		do{
			scoreboard=scoreboard.previousSibling;
		}while( scoreboard && scoreboard.nodeType!=1);
		// Firefox bug: Node.ELEMENT_NODE is not defined when this script is run.
		//     Node, however, is already a Node object.
		//     http://www.mozdev.org/bugs/show_bug.cgi?id=13193
		//while( scoreboard && scoreboard.nodeType!=Node.ELEMENT_NODE);
		if( scoreboard && scoreboard.tagName.toLowerCase() == 'p' ) {

			var text=getInnerText(scoreboard);
			...
			regex.compile('"(.)" +(.*[^ ]) *');
			for(var i=0, s; s=elements[i]; i++) {
				var tr=document.createElement('tr');
				var th=document.createElement('th');
				var td=document.createElement('td');
				regex.exec(s);
				th.appendChild(document.createTextNode(RegExp.$1));
				td.appendChild(document.createTextNode(RegExp.$2));
				tr.appendChild(th);
				tr.appendChild(td);
