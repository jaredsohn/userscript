// ==UserScript==
// @name           KOC Power Bot ES
// @version        0.2.3
// @namespace      mat
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @description    Funciones automatizadas para Kingdoms of Camelot
// @require        http://sizzlemctwizzle.com/updater.php?id=97843
// ==/UserScript==
...
var Options = {
  ...
  pbChatOnRight: false,
  pbWideMap    : true,
  alertConfig  : {aChat:false, aPrefix:'** Estoy siendo atacado!!! **', scouting:true, wilds:true, minTroops:1, spamLimit:20, lastAttack:0 },
...
function pbStartup (){
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    input.pbDefButOn {cursor:pointer; border:1px solid black; background-color:#88F748;}\
    input.pbDefButOff {cursor:pointer; border:1px solid black; background-color:#FE6005;}\
  ...
  window.name = 'PT';
  logit ("* KOC Power Bot ES v"+ Version +" Cargado");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  actionLog ("KOC Power Bot ES v"+ Version +" Cargado  (Versión de KofC: "+ anticd.getKOCversion() +")");
...
Tabs.tower = {
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>DEFENDER CIUDADES/OCULTAR TROPAS</div><TABLE class=pbTab><TR align=center>';
    ...
    m += '</tr></table><BR><DIV><CENTER><INPUT id=pbSoundStop type=submit value= Parar Sonido de Alerta style="background-color:#F5F6CE;"></center></div><DIV id=pbSwfPlayer></div>';
    m += '<BR><DIV class=pbStat>CONFIGURACIÓN DE ALERTAS</div><TABLE class=pbTab>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Publicar automaticamente ataques entrantes en el chat de la alianza.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            ...
            <TR><TD align=right>Alertar cuando exploren: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Alertar cuando ataquen: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
        ...
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Sonar alarma cuando ataquen/exploren</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>Cargando SWF player...</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Archivo: &nbsp; </td><TD><INPUT id=pbsoundFile type=text size=55 maxlength=160 value="'+ Options.alertSound.soundUrl +'" \>\
             &nbsp; </td><TD><INPUT id=pbSoundLoad type=submit value=Cargar><INPUT id=pbSoundDefault type=submit value=Origen></td></tr>\
            ...
            <TR><TD></td><TD>Alertar durante <INPUT id=pbSoundLength type=text size=3 maxlength=5 value="'+ Options.alertSound.playLength +'"> segundos</td></tr>\
            <TR><TD></td><TD><INPUT type=submit value="Escuchar" id=pbPlayNow></td></tr></table></div></td></tr>\
        </table><BR>';
  	t.myDiv.innerHTML = m;
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y'); 
  ...
  loadUrl : function (url){
    ...
    document.getElementById('pbLoadStat').innerHTML = 'Cargando';
  },
...
Tabs.Construir = {
    ...
    init: function(div){
        ...
        if (t.buildStates.running == false) {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Auto Edificar = OFF"></td>';
        }
        else {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Auto Edificar = ON"></td>';
        }
		m += '<TD><INPUT id=pbBuildMode type=submit value="Modo Construir = OFF"></td>';
		m += '<TD>Tipo Construcción: <SELECT id="pbBuildType">\
				<OPTION value=build>Subir Nivel</option>\
				<OPTION value=max>Nivel Max</option>\
		...
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=pbStat>COLAS CONSTRUCCIÓN</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><B>' + Cities.cities[i].name + '</b></center></td>';
        }
		m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><INPUT id=pbbuild_' + Cities.cities[i].id + ' type=submit value="Mostar"></center></td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>Cola de edeificacion de ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="Optimize by Time"></td>';
    ...
    toggleStateRunning: function(obj){
        ...
        if (t.buildStates.running == true) {
            ...
            t.saveBuildStates();
            obj.value = "Auto Edificar = OFF";
        }
        else {
            t.buildStates.running = true;
            t.saveBuildStates();
            obj.value = "Auto Edificar = ON";
        }
    },
...
//				13:{'name':"-------",'x':375,'y':375},
    ...
    m += htmlSelector ({0:"Campamento Bárbaros", 1:"Tierras Vírgenes", 2:"Ciudades"}, null, 'id=pasrcType'); 
    m += '</td></tr><TR><TD class=pbDetLeft>Desde: </td><TD class=xtab>X=<INPUT id=pasrchX type=text\> &nbsp;Y=<INPUT id=pasrchY type=text\>\
      &nbsp; Radio: <INPUT id=pasrcDist size=3 value=10 /> &nbsp; <SPAN id=paspInXY></span>\
      <TR><TD class=pbDetLeft>O:</td><TD>Buscar en la Provincia entera: <select id="provinceXY"><option>--Provincias--</option>';
    for (var i in Provinces)
    	m += '<option value="'+i+'">'+Provinces[i].name+'</option>';
    m += '</select></td></tr>';
    m += '<TR><TD colspan=2 align=center><INPUT id=pasrcStart type=submit value="Buscar"/></td></tr>';
    m += '</table></div>\
  ...
  clickedSearch : function (){
    ...
    document.getElementById ('pasrcStart').value = 'Parar Búsqueda';
    m = '<DIV class=pbStat><TABLE width=100% cellspacing=0><TR><TD class=xtab width=125><DIV id=pastatSearched></div></td>\
        ...
        <TD class=xtab align=right width=125><DIV id=pastatFound></div></td></tr></table></div>\
          <TABLE width=100%><TR valign=top>\
            ...
            <TD align=center valign=middle><A id=pbAhideShow style="text-decoration:none; cursor:pointer;"><DIV style="width:1em; border:1px solid red; padding:10px 2px; background-color:#fee"><SPAN id=spanHideShow> O C U L T A R</span><BR><BR> O<BR>P<BR>C<BR>I<BR>O<BR>N<BR>E<BR>S<BR><BR>L<BR>I<BR>S<BR>T<BR>A<BR>D<BR>O<BR><BR></div></a></td>\
    ...
    if (t.opt.searchShape == 'square')
      ...
      var distName = 'Radio';
    m = '<CENTER><B>Búsqueda de '+ typeName +'<BR>\
        Desde: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; '+ distName +': '+ t.opt.maxDistance +'<BR></center>\
    ...
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. nivel a mostar:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. nivel a mostar:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Tipo Tierras:</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'Praderas/Lagos', 3:'Bosques', 4:'Colinas', 5:'Montañas', 6:'Planicies', 0:'TODO'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Solo Libres:</td><TD class=xtab><INPUT id=pafilUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
		...
		m+= '</select></td></tr><TR><TD class=xtab align=right>Solo en Niebla:</td><TD class=xtab><INPUT id=pafilMisted type=CHECKBOX '+ (Options.mistedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Solo Hostiles:</td><TD class=xtab><INPUT id=pafilHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Ordenar por:</td><TD class=xtab><SELECT id=pafilSortBy>\
    ...
    document.getElementById ('pastatStatus').innerHTML = 'Buscando.... '+ xxx +','+ yyy;
  ...
  dispMapTable : function (){
    var tileNames = ['Campamentos Bárbaros', 'Praderas', 'Lgos', 'Bosques', 'Colinas', 'Montañas', 'Planicies' ];
    var t = Tabs.Buscar;
    ...
    document.getElementById('pastatFound').innerHTML = 'Localizados: '+ dat.length;
    if (dat.length == 0){
      ...
      if (coordsOnly)
        m = '<TABLE align=center id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Situación</td></tr>';
      else {
      if (t.opt.searchType == 2) {
			 m = '<TABLE id=pasrcOutTab class=pbSrchResults cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Coord</td><TD align=right>Dist</td><TD>Jugador</td><TD align=right>Poderío</td><TD>Alianza</td><TD></td></tr>';
		} else { 
			m = '<TABLE id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Situación</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nvl</td><TD width=80%> &nbsp; Tipo</td><TD style=""></td></tr>';
		}
	}
      var numRows = dat.length;
      if (numRows > t.MAX_SHOW_WHILE_RUNNING && t.searchRunning){
        numRows = t.MAX_SHOW_WHILE_RUNNING;
        document.getElementById('pasrchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: La tabla solo muestra '+ t.MAX_SHOW_WHILE_RUNNING +' de '+ dat.length +' resultados hasta que la búsqueda se complete.</font>';
      }
      for (i=0; i<numRows; i++){
        ...
        if (coordsOnly) {
          ...
          if (t.opt.searchType == 2) { // city search
            ...
            if (dat[i][5])
              m += '<TD colspan=4>* EN NIEBLA * &nbsp; &nbsp; <SPAN onclick="pbSearchScout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A>Explorar</a></span></td></tr>';
            else{
              var allStyle = '';
              if (dat[i][12]=='f')
              ...
              m += '<TD>'+ dat[i][9]+'</td><TD align=right>'+ dat[i][10] +'</td><TD><SPAN '+ allStyle +'>'+ dat[i][11]+'</span></td><TD><A onclick="pbSearchLookup('+ dat[i][7] +')">Investigar</a></td></tr>';
            }
			} else { 
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('pasrcStart').value = 'Buscar';
    document.getElementById ('pasrchSizeWarn').innerHTML = '';
    if (t.opt.searchType==0 && document.getElementById('KOCAttackToggle')!=null){    
      document.getElementById ('pbSrcExp').innerHTML = '<CENTER>'+ strButton20('Exportar Resultados', 'id=pbSrcDoExp') +'</center>'; 
...
// TODO: save memory, remove city name ?   			
    ...
    document.getElementById('pastatSearched').innerHTML = 'Explorados: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    if (t.curX > t.lastX){
      ...
      t.curY += 15;
      if (t.curY > t.lastY){
        t.stopSearch ('<FONT color=#80FF00>¡¡¡ FIN DE EXPLORACIÓN !!!');
        return;
      }
    }
    var x = t.MapAjax.normalize(t.curX);
    var y = t.MapAjax.normalize(t.curY);
    document.getElementById ('pastatStatus').innerHTML = 'Buscando... '+ x +','+ y;
  ...
  clickedLookup : function (pid){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>Jugador Investigado</b></center>';
    pop.getMainDiv().innerHTML = '<DIV class=pbStat>Información Clasificación</div><SPAN id=pblupLB>Buscando en Clasificaciones...</span>\
      <BR><DIV class=pbStat>Datos Alianza</div><SPAN id=pblupAI>Buscando información de la Alianza...</span>';
    pop.show (true);
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Clasificación:</b> No encontrado! (en Niebla?)<BR><BR>';
      return;
    }
    ...
    var m = '<CENTER><SPAN class=boldRed>NOTA: La información de la clasificación tiene un retardo de 24 horas</span></center><TABLE class=pbTabSome>';
    m += '<TR><TD class=pbDetLeft>Nombre:</td><TD>'+ name +'</td></tr>\
      <TR><TD class=pbDetLeft>Poderío:</td><TD>'+ p.might +' (rango:'+ p.rank +')</td></tr>\
      <TR><TD class=pbDetLeft>Alianza:</td><TD>'+ aName +'</td></tr>\
      <TR valign=top><TD class=pbDetLeft>Ciudades:</td><TD><TABLE class=pbTabSome><TR style="font-weight:bold"><TD>Ciudad</td><TD>Coordenadas</td><TD>Nivel</td><TD>Estado</td><TD>Creada</td></tr>';
  ...
  gotPlayerInfo : function (rslt, span){
    ...
    m += '<TR><TD class=pbDetLeft>Jugador:</td><TD>'+ p.genderAndName +'</td></tr>\
      <TR><TD class=pbDetLeft>Poderío:</td><TD>'+ p.might +'</td></tr>\
      <TR><TD class=pbDetLeft>Página Facebook:</td><TD><A target="_tab" href="http://www.facebook.com/profile.php?id='+ p.fbuid +'">Abrir en nueva ventana</a></td></tr>\
      <TR><TD class=pbDetLeft>Alianza:</td><TD>'+ p.allianceName +'</td></tr>\
...
var exportToKOCattack = {
  ...
  doExport : function (coordList, city){
    ...
    var m = '<DIV class=pbStat>Exportar datos a KOC Attack</div><BR><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabPadNW>\
      <TR style="font-weight:bold; background-color:white"><TD>Tipo Objetivo</td><TD style="padding:1px" align=center>Cantidad</td><TD width=15></td>';
    for (var i=0; i<troopDef.length; i++)
      m += '<TD>'+ troopDef[i][0] +'</td>';
    m += '</tr>';
    for (var b=1; b<11; b++){
      m += '<TR><TD>Nivel Bárbaros '+ b +'</td><TD align=right>'+ coordList['lvl'+b].length  +'&nbsp; &nbsp;</td><TD></td>'; 
    ...
    if (isKOCattack){
      m += '<BR><CENTER>'+ strButton20('Añadir a KOC Attack', 'id=pbSrcDoBA') +'</center>';
    } else {
      m += 'KOC Attack no está funcionando, imposible exportar';
    } 
    ...
    popExp.getTopDiv().innerHTML = '<CENTER><B>AÑADIR TROPAS</b></center>';
    if (isKOCattack)    
...
// TODO: WAIT FOR City select ?
    ...
    function validate (e){
      ...
      if (isNaN(x) || x<0 || x>100000){
        ...
        document.getElementById('ptETerr_'+ b).innerHTML = 'Entrada Inválida';
        return;
      } else {
      ...
      if (tot<1 && cList['lvl'+ b].length>0 )
        document.getElementById('ptETerr_'+ b).innerHTML = 'Tropas no definidas';
      if (tot>100000)
...
// WinManager.hideall();      <=== TODO!  
    ...
    function e_attackDialog (tf){
      ...
      document.getElementById('pbSrcExpResult').innerHTML += 'Añadidos '+ list.length +' objetivos  para '+ city.name +'<BR>';
...
Tabs.Attack = {
  ...
  init : function (div){
    var t = Tabs.Attack;
    t.myDiv = div;
    t.myDiv.innerHTML = '<TABLE width=100% height=100% class=pbTab><TR><TD><INPUT id=pbBarbShow type=submit value="Mostar todos Objetivos" \> <BR>\
       Ciudad: <SPAN id=pbAtkCSS></span> &nbsp; &nbsp; &nbsp; Radius: <INPUT id=pbBarbDist size=3 type=text> &nbsp; &nbsp; <INPUT id=pbBarbScan type=submit value=Scan \></td></tr><TR><TD height=100%>\
...
DebugTimer.display ('Time to GM_getValue() '+ totTargets +' targets for all cities');    
  ...
  e_clickedScan : function (){
    ...
    if (isNaN(dist) || dist<1 || dist>35){
      t.writeDiv ("<SPAN class=boldRed>Nuh-uh, Prueba otra vez</span><BR>");
      return; 
    }
    t.writeDiv ('Escaneando mapa para  ciudad: '+ t.scanCity.name +'<BR>');
  ...
  e_clickedShow : function (){    // show all current attack data
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      for (i=0; i<atks.length; i++)
        m += '<TR><TD>Campamento Bárbaros '+ atks[i].lvl +'</td><TD>'+ atks[i].x +','+ atks[i].y +'</td><TD> &nbsp; Dist='+ atks[i].dist.toFixed(2) +'</td></tr>';
    }    
    t.popShow.getMainDiv().innerHTML = '</table></div>'+ m;
    t.popShow.getTopDiv().innerHTML = '<CENTER><B>Mostar todos los objetivos en memoria/b></center>';
    t.popShow.show(true);    
  },
  ...
  init : function (div){
    ...
    try {      
     m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>Configuración del Power Bot:</b></td></tr>\
        ...
        <TR><TD><INPUT id=pbTrackWinOpen type=checkbox /></td><TD>Recordar el estado de ventanas abiertas al refrescar</td></tr>\
        ...
        <TR><TD><INPUT id=pbFairie type=checkbox /></td><TD>Deshabilitar todas las ventanas emergentes de Feriantes</td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Pantalla panorámica (todos los dominios, requiere refrescar)</td></tr>\
        <TR><TD><INPUT id=pbWatchEnable type=checkbox '+ (GlobalOptions.pbWatchdog?'CHECKED ':'') +'/></td><TD>Refrescar si  KOC no se carga en 1 minuto (todos los dominios)</td></tr>\
        <TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>Refrescar KOC cada <INPUT id=pbeverymins type=text size=2 maxlength=3 \> minutos</td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Poner ventana de chat a la derecha (requiere pantalla panorámica)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>Usar Mapa Panorámico (requiere pantalla panorámica)</td></tr>\
        <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>Recolectar el oro cuando la felicidad llege al <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
          </table><BR><BR><HR>Tener en cuenta que si una casilla cambia a color gris probablemente haya habido un cambio en el código de KoC, quedando esa opción inservible.</div>';
      div.innerHTML = m;
...
var CollectGold = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectGold;
    if (rslt.ok)
      actionLog ('Recaudado '+ rslt.goldGained +' Oro en '+ t.colCityName +' (la felicidad llegó al '+ t.colHappy +'%)'); 
    else 
...
var FairieKiller  = {
  ...
  setEnable : function (tf){
    if (tf)
      unsafeWindow.Modal.showModalUEP = eval ('function (a,b,c) {actionLog ("Ventana toca pelotas emergente de feriantes bloqueada");}');
    else
...
Tabs.Regalos = {
  ...
  init : function (div){
    var t = Tabs.Regalos;
    t.myDiv = div;    
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD width=200></td><TD align=center><INPUT id="pasubGifts" type=submit value="Comprobar Regalos" \></td><TD width=200 align=right><INPUT id=paGiftHelp type=submit value=AYUDA style="background-color:#ECF979;"></td></tr></table><HR>\
  ...
  helpPop : function (){
    var helpText = '<BR>La ficha REGALOS te ayuda a aceptar los regalos más comodamente que tener que ir a través de la página de Facebook. Para usarlo, primero pica en el boton\"Comprobar Regalos\"\
    .  Esto los buscará en la página de regalos de facebook y hará¡ una lista de todos los disponibles.<BR><BR>\
        En la lista, marca todos los regalos que desea aceptar o presiona en \"Todo\" para seleccionarlos de una sola vez.  Asegúrate de seleccionar el \
        dominio en el que deseas aplicar los regalos. Si quieres que los regalos se eliminen de Facebook después de aceptar, cambia la opción \"Borrar Regalos\"\
        a \"Siempre\". Ahora pusa en el boton\"Aceptar Regalos\"  para recoger los seleccionados.  Este proceso llevará algúºn tiempo, ya que hay cuatro páginas web \
        a las que se accede por cada regalo!<BR><BR>\
        NOTAS:<UL><LI>La página de regalos tiene una capacidad de 100 regalos para <B>"todos"</b> tus juegos en Facebook. Esto significa que solo algunos de los regalos para KofC\
        estarán disponibles. Después de aceptar los regalos vuelve a  \"Comprobar Regalos\" otra vez para ver si los puedes volver a cobrar!<p>\
        <LI>Si  elijes no eliminar después de aceptar regalos,  "los regalos", pueden estar disponibles para cobrarlos otra vez!!! Después de que el proceso está completo, basta con pulsar en\
        \"Comprobar Regalos\" para ver cuantos regalos quedan disponibles.</ul>';
    var pop = new CPopup ('giftHelp', 0, 0, 500, 400, true);
    pop.centerMe (mainPop.getMainDiv());  
    pop.getMainDiv().innerHTML = helpText;
    pop.getTopDiv().innerHTML = '<CENTER><B>Ayuda del Power Bot</b>: Aceptando regalos </center>';
    pop.show (true);
  },
  ...
  e_clickedGifts : function  (){     // (also cancel accepting)
    var t = Tabs.Regalos;
    if (t.accepting){
      document.getElementById('pasubGifts').value = 'Comprobar Regalos';
      document.getElementById('giftDiv').innerHTML+= '<BR><SPAN class=boldRed>Cancelado.</span>';
      t.accepting = false;
      return; 
    }
    document.getElementById('giftDiv').innerHTML = 'Explorando pagina de regalos de Facebook ...';
...
//logit (inspect (t.gifts, 8, 1));     
      var m = '<DIV class=ptstat><CENTER>Regalos de Camelot &nbsp; &nbsp; &nbsp; ('+ t.gifts.length +' encontrados)</center></div>';
      if (t.gifts.length<1){
        document.getElementById('giftDiv').innerHTML = m + '<BR><BR><CENTER>No se encontraron regalos!</center>';
        return;
      }
      m += '<TABLE class=pbTab align=center><TR><TD align=right>Dominio a descargar regalos: </td><TD>'
        + htmlSelector (Options.giftDomains.list, getServerId(), 'id=pbGiftServers') +'</td></tr>\
          <TR><TD align=right>Borrar regalos después de aceptarlos</td><TD>'
        + htmlSelector ({y:'Siempre', e:'Solo si da error', n:'Nunca'}, Options.giftDelete, 'id=pbGiftDel')
        + '</td></tr><TR><TD>Seleciona los regalos que quieres aceptar y pulsa: </td><TD width=250><INPUT type=submit id=pbGiftDo value="Aceptar Regalos">\
  ...
  getErDone : function (){ 
    ...
    if (t.doList.length==0){
      document.getElementById('pbGiftNone').innerHTML = 'Membrillooo!!! Selecciona algo!!!';
      return;
    }
    t.doServer = document.getElementById('pbGiftServers').value;
    t.accepting = true;
    document.getElementById('pasubGifts').value = 'Parar Aceptar'; 
    document.getElementById('giftDiv').innerHTML = '<DIV id=acpDiv style="height:400px; max-height:400px; overflow-y:auto"><B>Recolectados '+ t.doList.length +' Regalos:</b><BR></div>';    
    t.acceptNext ();
  },
  ...
  acceptNext : function (){
    ...
    if (gift == null){
      t.allDone ('<font color=#54AE51><B>Recogida de regalos terminada.'); 
      return;
    }
    var acpDiv = document.getElementById('acpDiv');
    var curDiv = document.createElement ('div');
    acpDiv.appendChild (curDiv);
    curDiv.innerHTML = '<B>'+ gift.gift +'</b> desde '+ gift.giver +' on '+ gift.args.da.substr(0,4) +'-'+ gift.args.da.substr(4,2) +'-'+ gift.args.da.substr(6,2) +': ';    
...
//logit ("getErDone.gotGiftData ... \n"+ inspect (gift, 8, 1)); 
    ...
    function doneAccepting (rslt){
      ...
      var msg = 'OK.';
      if (rslt.ok)
        actionLog ('Regalo Aceptado:  '+ gift.gift +' desde '+ gift.giver +' en '+ gift.args.da.substr(0,4) +'-'+ gift.args.da.substr(4,2) +'-'+ gift.args.da.substr(6,2)     );
      else
...
//  parse multiple reply formats .....         
    function ver1GotPost (rslt){
      ...
      if (m)
        notify ({ok:false, msg: 'En el saco:  '+ m[1]}); 
      else
...
// sample result: .... window.location.replace("http:\/\/apps.facebook.com\/kingdomsofcamelot\/?page=claimgift&gid=1045&sid=1432568&s=250&in=1432568&si=5"); ...
      ...
      if (m == null)
        notify ({ajaxErr:'Análisis - página 1'});
      var url = m[1].replace ('\\/', '/', 'g');
      GM_AjaxGet (url, '', got2, 'Página 2');        
    }
...
// sample URL: http://www88.kingdomsofcamelot.com/fb/e2/src/claimDailyGift_src.php?sid=4411654&gid=361&standalone=0&res=1&iframe=1&wcfbuid=1400526627&fbml_sessionkey=2.wdwjP4blBLkO2wXAFqDglg__.3600.1293681600-1400526627&lang=en&in=4411654&si=9&ts=1293677199.881&page=claimdailygift&gid=361&sid=4411654&s=88&in=4411654&si=9&appBar=&kabamuid=114014&tpuid=alYJXw-Us9z9qjRn3DHChEtsFvo&fb_sig_in_iframe=1&fb_sig_base_domain=kingdomsofcamelot.com&fb_sig_locale=en_GB&fb_sig_in_new_facebook=1&fb_sig_time=1293677199.924&fb_sig_added=1&fb_sig_profile_update_time=1267240352&fb_sig_expires=1293681600&fb_sig_user=1400526627&fb_sig_session_key=2.wdwjP4blBLkO2wXAFqDglg__.3600.1293681600-1400526627&fb_sig_ss=7wEsU_e0FLqhrGxE1LAZDg__&fb_sig_cookie_sig=514b59deb303becb5c5c654c9d457732&fb_sig_ext_perms=email%2Cuser_birthday%2Cuser_religion_politics%2Cuser_relationships%2Cuser_relationship_details%2Cuser_hometown%2Cuser_location%2Cuser_likes%2Cuser_activities%2Cuser_interests%2Cuser_education_history%2Cuser_work_history%2Cuser_online_presence%2Cuser_website%2Cuser_groups%2Cuser_events%2Cuser_photos%2Cuser_videos%2Cuser_photo_video_tags%2Cuser_notes%2Cuser_about_me%2Cuser_status%2Cfriends_birthday%2Cfriends_religion_politics%2Cfriends_relationships%2Cfriends_relationship_details%2Cfriends_hometown%2Cfriends_location%2Cfriends_likes%2Cfriends_activities%2Cfriends_interests%2Cfriends_education_history%2Cfriends_work_history%2Cfriends_online_presence%2Cfriends_website%2Cfriends_groups%2Cfriends_events%2Cfriends_photos%2Cfriends_videos%2Cfriends_photo_video_tags%2Cfriends_notes%2Cfriends_about_me%2Cfriends_status&fb_sig_country=us&fb_sig_api_key=0ab5e11ff842ddbdbf51ed7938650b3f&fb_sig_app_id=130402594779&fb_sig=fca33813d9e1c9d411f0ddd04cf5d014
    function got2 (page){
      if (page == null)
        notify ({ajaxErr:'COMM Error - página 2'});
      progress ('2');
      var m = page.match (/<form action=\"(.*?)\"/im);
      if (m == null)
        notify ({ajaxErr:'Análisis - página 2'});
    ...
    function got3 (page){
      if (page == null)
        notify ({ajaxErr:'COMM Error - página 3'});
      progress ('3');
      var m = /<div class=\'regalo devuelto\'>(.*?)<\/div/im.exec(page);
      if (m != null){
      ...
      var m = /(Facebook dice que tienes amigos.*?)</im.exec(page);
      if (m != null){
        notify ({feedback:m[1], del:true}); 
        return;
      }
...
**/        
        ...
        if (m == null){
          notify ({ajaxErr:'Análisis (ver:1, regalo no encontrado) - página 3'});
          return;
        }
        gift.dat.giftId = m[1];
        gift.dat.ver = 1;
