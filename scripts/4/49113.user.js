// ==UserScript==
// @name           Darwin's Table2
// @namespace      Darwin's Table2
// @description    Tabla de Darwin's.
// @author         Monkey
// ==/UserScript==
...
function renderTables() {
	...
	PREMIUM_VIEW       = getCfgValue("PREMIUM_VIEW", false);
	BUILDINGS_IMAGES   = getCfgValue("BUILDINGS_IMAGES", true);
	ARMY_IMAGES        = getCfgValue("ARMY_IMAGES", true);
	QUICK_UPGRADE      = getCfgValue("QUICK_UPGRADE", true);
	...
	TABLE_RESEARCH     = getCfgValue("TABLE_RESEARCH", false);
    ...
    t.appendChild(createRowChk(texts["BUILDINGS_IMAGES"]+":", "BUILDINGS_IMAGES", BUILDINGS_IMAGES));
    t.appendChild(createRowChk(texts["ARMY_IMAGES"]+":", "ARMY_IMAGES", ARMY_IMAGES));
    t.appendChild(createRowChk(texts["QUICK_UPGRADE"]+":", "QUICK_UPGRADE", QUICK_UPGRADE));
    t.appendChild(createRowChk(texts["DOUBLE_MAP"]+":", "DOUBLE_MAP", DOUBLE_MAP));
    ...
    var n = document.createElement('input');
    n.type = "button";
    n.value = texts["Reset_css_data"];
    n.setAttribute("class", "button");
    n.setAttribute("style", "display: inline !important;");
    n.addEventListener("click", reset_css_data, false);
    buttonsPanel.appendChild(n);
...
function CitiesTable() {
	...
	s += "<th colspan=3 class='table_header "+langtype+"'>"+texts["Resource"]+"</th>";
	...
	var sumres = {"actions":0, "population":0, "totalspace":0, "freeworkers":0, "woodworkers":0, "specialworkers":0,
		          "scientists":0, "happiness":0, "gold":0,};
	...
	for(var i = 0; i < nodes.length; i++) {
		...
		var growthRemainingHours = "&#8734; "+texts["h"];  // 預設為 ∞ 這符號
		...
		s += "<td class='"+lfcs+"'>";
			...
			s += createTooltip(createLinkToResource(createImg2(icons["wood"], texts["wood"], 16), res.island_id, city.value, i), texts["wood"])+" ";
			s += createLinkToResource(res.woodlevel, res.island_id, city.value, i);
		s += "</td>";
		
		s += "<td class='"+cs+"'>";
				
			s += createTooltip(createLinkToTradegoodCond(true, createImg2(icons[res.type], texts[res.type], 16), res.island_id, city.value, i), texts[res.type])+" ";
			s += createLinkToTradegoodCond(true, res.tradegoodlevel, res.island_id, city.value, i);
		s += "</td>";
		
		s += "<td class='"+cs+"'>";
		s += (res.wonder != "?") ? createTooltip(createImg(wonders_img[res.wonder], "title='"+texts[res.wonder]+"'", 17, 17), texts[res.wonder]) : "?";		
		s += "</td>";
		...
		var textbuilding = (buildings[res.underConstructionName] != undefined) ? buildings[res.underConstructionName][1] : "";
		var buimages = (BUILDINGS_IMAGES) ? createTooltip(createImg(title_icons[res.underConstructionName], "", 16), textbuilding) : createTooltip(textbuilding, textbuilding);
		
		var align = (BUILDINGS_IMAGES) ? "" : "align='left'";

		s += "<td class='"+lfcs+"' '"+align+"'>"+buimages+"</td>";
		s += "<td class='"+cs+"'>"+res.underConstructionLevel+"</td>";
		s += "<td class='"+cs+"'>"+createTimeCounter(res.underConstructionTime)+"</td>";
		s += "</tr>";
		sumres.actions += SumformatInt(res.actions);   // 行動點數加總
		sumres.population += SumformatInt(population); // 人口加總
		sumres.totalspace += SumformatInt(spacetotal + bonusspace); // 房屋總空間
		sumres.freeworkers += SumformatInt(res.freeworkers); // 空閒人口
		sumres.woodworkers += SumformatInt(res.woodworkers); // 伐木人口
		sumres.specialworkers += SumformatInt(res.specialworkers); // 特產人口
		sumres.scientists += SumformatInt(res.scientists); // 特產人口
		sumres.happiness += SumformatInt(happiness);   // 民意加總
		sumres.gold += SumformatInt(res.gold);   // 收入加總
	}
    ...
    //s += "<td class='table_footer "+langtype+"' colspan=3>"+"</td>"
	s += "<td class='table_footer "+langtype+"' colspan=3>"+createLink("<strong>GigeL's Overview</strong>", scriptsite, "target='_blank'")+"</td>";
    s += "</tr>";
	s += "</table>";
	s += "<br>";
	return s;
...
function ResourcesTable() {
	...
	s += "<td class='table_footer'></td>";
	s += "<td class='table_footer "+langtype+"' colspan=1 style='font-weight: normal;'>"+createLink("<strong>GigeL's Overview</strong>", scriptsite, "target='_blank'")+"</td>";
 	s += "</tr>";
    s += "</table>";
    s += "<br>";
	return s;
	...
	for(key in buildings) {
		if (usedIndexes[key]) {
			...
			for (var i=0; i<buildingsCount[key]; i++) {
				var bimages = (BUILDINGS_IMAGES) ? createTooltip(createImg(title_icons[key], "", 30), buildings[key][1]) : createTooltip(buildings[key][0], buildings[key][1]);
				
				var qucolspan = (QUICK_UPGRADE) ? "colspan='2'" : ""; 
				s += "<th '"+qucolspan+"' class='"+firstStyle+currentBuildingStyle+" table_header'>"+bimages+"</th>";
			}
			firstStyle = "";
		}
	}
...
function ArmyTable() {
	...
	if (usedIndexesCount > 0) {
		...
		var armyimages = (ARMY_IMAGES) ? createTooltip(createImg(title_icons[key], "", 30), info, texts["unitInfo"]) : createTooltip(names[key], info, texts["unitInfo"]);
				s += "<th colspan=2 class='table_header "+langtype+"'>"+armyimages+"</th>";
			}
		}
...
function PlayersTable() {
    ...
    for(idx in playerNames) {
		...
		if (!PLAYERS_BANNED && citystatus == "banned") continue;
		if (!PLAYERS_VACATION && citystatus == "vacation") continue;
		var banner = (citystatus == "banned") ? createImg(icons["banned"]) : "";
		var vacation = (citystatus == "vacation") ? createImg(icons["vacation"]) : "";
		var inactive = (citystatus == "inactivity") ? "<font color='#C0C0C0'><b><i>(I)</i></b></font>" : "";
		var strattr = "class='deleteplayer' playername='"+plname+"' ";
		var deletebutton = createImg(icons["close"], strattr, 16);
		...
		var plname2 = (citystatus == "inactivity") ? "<font color='#999999'><i>"+plname+"</i></font>" : plname;
		var ss = "";
		var citynum = 0;
		for(id in city_ids) {
			if (city_ids[id]) {
				...
				ss += createTooltip(createIconLink("diplomacy", id, true, "", city.island_id), texts["Diplomacy"]);
		...
		s += "<td"+rowspan+">"+plname2+banner+vacation+"</td>";
...
function reset_css_data() {
	var answer = confirm(texts["RESET_CSS_CONFIRM"]);
	if (answer) {
		config.cfg["CSS"] = default_style;
		saveConfig();
		window.location.href = window.location.href;
	}
}
	...
	if(level!="-"&&link!="-") {
		...
		if (!isUpgrade) {
			...
			if (tooltip != "undefined" && tooltip != "") {
			   upgradetext = createTooltip(upgradetext, tooltip, texts["ResourceNeeds"]+" "+texts["Upgrade"]+":"+level);			//GigeL
			   levellink = createTooltip(levellink, tooltip, texts["ResourceNeeds"]+" "+texts["Upgrade"]+":"+level);
			} else {
			   upgradetext ="";
			   levellink = createTooltip(levellink,texts["unknown"]);
			}
