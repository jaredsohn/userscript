...
// @description    Seznam farem (přeložil: honzk)
...
function addCoords(event) {
	...
	else coords = prompt('Vlož souřadnice X:XXX:XX' );
	var desc = prompt('Jméno hráče (farmy)');
	var typ = prompt('1 - Planeta, 2 - DF, 3 - Měsíc','1');
...
if( result[2].indexOf('galaxy')!=-1 )
{
	...
	function delAll()
	{
		if (confirm ('Skutečně chceš smazat všechny farmy?')) {
	...
	function moveCoord(event) {
		...
		if ((poz = prompt ('Nová pozice:')) && (poz>0) && (poz<CoordsTab.length)) {
	...
	function zarzadzaj() {
		...
		td.className='c';
		td.colSpan=4;
		td.appendChild(document.createTextNode('Seznam farem'));
		...
		for( i = 0; i < len; i++ )
		{
			...
			if( x != '' )
			{
				...
				if( arr[0] != null && typeof(arr[1])!='undefined')
				{
					...
					a.appendChild(document.createTextNode('Smazat'));
					th.appendChild(a);
				}
			}
		}
