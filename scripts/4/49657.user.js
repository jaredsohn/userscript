...
function post(url) {
  GM_xmlhttpRequest({
	...
	infobar.innerHTML ='<span style="color:#FF0000"><b>' + villageNameA + ' Megtámadtak!</b>';
	if (sounds_enabled)
		infobar.innerHTML += "<embed src='"+soundUrl+"' hidden=true autostart=true loop=false>";
	infobar.innerHTML += "</span>";
	}
	if (tot == ant)
	{ant++; last(); }else{ant++;}
...
var ex = "//tr[@class='sel']//a[contains(@href,'newdid')]";
  ...
  getAttackInfo(pulled);
	...
	infobar.innerHTML ='<span style="color:#FF0000"><b>' + villageNameA + ' Under attack!</b>';
	if (sounds_enabled)
		infobar.innerHTML += "<embed src='"+soundUrl+"' hidden=true autostart=true loop=false>";
	infobar.innerHTML += "</span>";
	}else{
...
function getVilLink ()
{
//var ex = "//a[contains(@href,'newdid')][not(@class='active_vl')][count(*)=0]";
var ex = "//tr[not(@class='sel')]//a[contains(@href,'newdid')][count(*)=0]";
	tag = document.evaluate( 
  	ex,
    	document,
    	null,
...
var sounds_enabled = GM_getValue('attacksounds', true);
var timer = GM_getValue('check_timer', 1);
timer = parseInt(timer);
var now = new Date().getTime();

var exptime = (timer + (10+Math.floor(Math.random()*5))*60*1000);
now = "" + now;
...
infobar.innerHTML = '<span style="color:#00CC00"><b>O.o</b></span>';
//thisDiv = document.getElementById('lmid1');
/*thisDiv = document.getElementById('map_details_actions');
if (!thisDiv)
	thisDiv = document.getElementById('sright');
else
	infobar.style.padding = '5px';
if (!thisDiv)
...
function soundstoggle() {
	...
	GM_setValue('attacksounds', sounds_enabled);
	if (sounds_enabled) {
		this.innerHTML = 'Enabled sounds';
	} else {
		this.innerHTML = '<span style="background-color: #71D000 !important; color: #FFFFFF !important;">Enable sounds</span>';
	}
}
...
if (search.snapshotLength)
  ...
  link.href = '#';
  if (sounds_enabled) {
	link.innerHTML = 'Disable sounds';
  } else {
	link.innerHTML = '<span style="background-color: #71D000 !important; color: #FFFFFF !important;">Enable sounds</span>';
  }
