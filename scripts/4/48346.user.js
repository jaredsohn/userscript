...
// select "TribalWars Enhancer v3 (asia)", and click Uninstall.
//
// --------------------------------------------------------------------
// ==UserScript==
...
// @description   base on "TribalWars Enhancer v3". Ads Remover. 15x15 Map. Shortcuts Bar. villages-list. Language: en or zh-CN, and more!
// @include        http://en*.tribalwars.*/*
// @copyright      Copyright (c) QinJiaXian(秦 假仙)
// ==/UserScript==
...
function log(lvl, arg) {
	if (DEBUG && DEBUG !== 0) {
		if (DEBUG > 0 && lvl == 1) { console.log('[错误]: '+arg); }
		if (DEBUG > 1 && lvl == 2) { console.log('[失败]: '+arg); }
		if (DEBUG > 2 && lvl == 3) { console.log('[消息]: '+arg); }
	} else { return; }
}
...
//GM_log(villageSet[i].innerHTML+"   \n\r "+cells[0].innerHTML+" \n\r " +cells[1].innerHTML+" \n\r " +cells[2].innerHTML);
		 ...
		 var support = cells[0].innerHTML.match("support.png");
		 if (support != null) {
			support = support.length;
		} else {support = 0;}
		 var bonus = cells[0].innerHTML.split( 'span class="bonus_icon bonus_icon_')[1];
		 var attack = cells[0].innerHTML.match("attack.png");
		 if (attack != null) {
			attack = "1";
		} else {
			attack = "0";
		 ...
		 //log(1,i+":    "+attack);
		 if (bonus !=undefined) {
				var bonus_title = bonus.split('" title="')[1].split('">')[0];
				bonus = bonus.split('" title="')[0];
				//GM_log("\n\r"+bonus+"\n\r"+bonus_title);
		 } else {
				var bonus_title = "村庄纵览";
				bonus = "0";
		 }
		...
		var vcoords_from = ["609","347"];
		var vcoords_to = [tmpDetails[2],tmpDetails[3]];
		var dist = parseInt(plugins.distancecalc.get_distance_num(vcoords_from,vcoords_to)*10000000);
		//log(1,dist);
		 villages[tmpId[1]] = {id: tmpId[1], name: tmpDetails[1], x: parseInt(tmpDetails[2]), y: parseInt(tmpDetails[3]), continent: tmpDetails[4], points: tmpPoints, pointsint: tmpPointsInt,group: tmpGrp,alias: tmpAlias, resourcesHTML:cells[2].innerHTML, bonus: bonus, title: bonus_title, attack:attack, attack_1:"", attack_2:"", attack_3:"", support: support, dist: dist};
...
languages = { // Language tags
	"cn" : {
		...
		"group_action" : "动作",
		"group_store" : "更新分组",
		"group_storeDone" : "分组已更新! 重新载入村庄纵览页面.",
		"group_name" : "组名",
		"group_ingroup" : "当前组",
		"overview_nogroup" : "未分组村庄",
		"overview_village" : "村庄",
		"overview_points" : "积分",
		"overview_resources" : "资源",
		"overview_warehouse" : "仓库",
		"overview_farm" : "农场",
		"main_setaliasheader" : "更改村庄别名:",
		"main_setalias" : "保存别名",
		"main_setaliasdone" : "别名已保存!",
		"setting_show" : "TW Enhancer 设置",
...
// Params: js
// Returns: nothing
	//var newJS = "function $(id) {" + "	return document.getElementById(id);}";

/*
function xRequest (postvar) {	
	GM_xmlhttpRequest({
		method: "POST",
		url: "http://cn1.buluozhengxiong.asia/game.php",
		...
		onload: function(responseDetails) 
			{
				if (responseDetails.readyState == 4)	{
					...
					log(1,responseDetails.responseText);
				}
			}
	});
}		 
	 ...
	 setFunc("waveAttack", function() { window.setTimeout(function() {
		 var icoArray = ["spear","sword","axe","archer","marcher","heavy","ram","catapult","knight","snob"];
		 var postvar="";
		 var report;
...
log(1,"waveAttack");
		 for(var report_id in reports) {
			 ...
			 var age = (lclTime.getTime()-report.timestamp)/1000;
				 ...
				 sum = 0;
				 for(var w in resources) { sum += resources[w]; }
				 var cavReturn = 0;
				 if (sum != 0){
					  cavReturn = sum / 80;
				...
				if (cavReturn > 15) {
					postvar = "village=" +v.id +  
								   "&screen=place&action=command&h=203e&try=confirm";
					postvar += "&x="+ report.coords[0] + "&y=" +report.coords[1];
					for (var u in icoArray) {postvar += "&" + icoArray[u] + "=0";}
					postvar += "&spy=1&light=" + cavReturn;
					log(1,postvar)
					var doc = xRequest(postvar);
					//log(1,doc);
					var action_id = doc.getElementById("action_id").value;
					log(1,action_id);
					postvar = "village=" +v.id +  
								   "&screen=place&action=command&h=203e&action_id="+ action_id;
					postvar += "&x="+ report.coords[0] + "&y=" +report.coords[1];
					for (var u in icoArray) {postvar += "&" + icoArray[u] + "=0";}
					postvar += "&spy=1&light=" + cavReturn;					
					var doc = xRequest(postvar);
  ...
  'info' : {
   ...
   'dev' : "Timas"
	...
	plugins.clearPremium = { // Get rid of the "get premium" messages
		'enhance_game_main' : function() {
//log(1,'$("table.main").length='+$("table.main").length );
			if ($("table.main").length > 1) {
				//$("table.main:first,br").css({display: "none"});
				$("table.main:first,br").remove();
			}	
			$("br").each(function() {
	  ...
	  var mainTD = $('td:has(> img[src$="/graphic/big_buildings/main1.png?1"])')[0]; 
	  if (!mainTD) mainTD = $('td:has(> img[src$="/graphic/big_buildings/main2.png?1"])')[0]; 
	  if (!mainTD) mainTD = $('td:has(> img[src$="/graphic/big_buildings/main3.png?1"])')[0]; 	  
	  if(mainTD)	mainTD.parentNode.parentNode.parentNode.removeChild(mainTD.parentNode.parentNode); 
		var mainCol = $('table.vis tr td a:has(> img[src$="/graphic/buildings/main.png?1"])')[0]; 
		if (mainCol) {
			var mainTable = mainCol.parentNode.parentNode.parentNode;
			for (var i=16; i>0; i--) {
				//log(1,mainTable.childNodes[i*2].textContent);
				if (mainTable.childNodes[i*2]) 
					if (mainTable.childNodes[i*2].textContent.indexOf("建筑已完全扩建") != -1)
						mainTable.childNodes[i*2].parentNode.removeChild(mainTable.childNodes[i*2]);
			}
		}

		},
	    'enhance_game_smith' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/smith1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/smith2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/smith3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");		  
	  },
	    'enhance_game_barracks' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/barracks1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/barracks2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/barracks3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");	
		  var unit_row = $('tr.row_a'); 
		  //log(1,unit_row.length+"\n\r"+unit_row[2].cells[6].innerHTML.split("/")[1]);
		  var rallypoints = getValue("rallypoints");
		  if(!rallypoints) rallypoints = new Object();
		  var rallypoint = rallypoints[personals.getCurrentVillage()].units;
		  for (var u=0; u<unit_row.length; u++) {
			  //var title = unit_row[u].cells[0].textContent;
			  //var name = unit_row[u].cells[7].childNodes[0].name;
			  var unit_count = unit_row[u].cells[6].innerHTML.split("/")[1];
			  rallypoint[u] = unit_count;
			  setValue("rallypoints", rallypoints);
		  }
		},
	    'enhance_game_stable' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/stable1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/stable2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/stable3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");	
		  var unit_row = $('tr.row_a'); 
		  //log(1,unit_row.length+"\n\r"+unit_row[2].cells[6].innerHTML.split("/")[1]);
		  var rallypoints = getValue("rallypoints");
		  ...
		  for (var u=0; u<unit_row.length; u++) {
			  ...
			  //var name = unit_row[u].cells[7].childNodes[0].name;
			  var unit_count = unit_row[u].cells[6].innerHTML.split("/")[1];
			  rallypoint[u+4] = unit_count;
			  setValue("rallypoints", rallypoints);
		  }
		},	
	    'enhance_game_place' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/place1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/place2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/place3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");
		},	
	    'enhance_game_garage' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/garage1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/garage2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/garage3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");
		  var unit_row = $('tr.row_a'); 
		  //log(1,unit_row.length+"\n\r"+unit_row[2].cells[6].innerHTML.split("/")[1]);
		  var rallypoints = getValue("rallypoints");
		  ...
		  for (var u=0; u<unit_row.length; u++) {
			  ...
			  //var name = unit_row[u].cells[7].childNodes[0].name;
			  var unit_count = unit_row[u].cells[6].innerHTML.split("/")[1];
			  rallypoint[u+8] = unit_count;
			  setValue("rallypoints", rallypoints);
		  }
		},	
	    'enhance_game_snob' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/snob1.png?1"])')[0]; 
		  if(placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");	
		  ...
		  var unit_count = unit_row.cells[4].innerHTML.split("/")[1];
			  rallypoint[11] = unit_count;
			  setValue("rallypoints", rallypoints);
		},	
	    'enhance_game_market' : function() {
		  placeTD = $('td:has(> img[src$="/graphic/big_buildings/market1.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/market2.png?1"])')[0]; 
		  if (!placeTD) placeTD = $('td:has(> img[src$="/graphic/big_buildings/market3.png?1"])')[0]; 
		  if (placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");
		},	
	    'enhance_game_statue' : function() {
			  placeTD = $('td:has(> img[src$="/graphic/big_buildings/statue1.png?1"])')[0]; 
			  if(placeTD)   placeTD.parentNode.parentNode.setAttribute("style","display:none;");		
			  var unit_row = $('table.vis tr')[1]; 
			 // log(1,unit_row.innerHTML);
			  var rallypoints = getValue("rallypoints");
			  if(!rallypoints) rallypoints = new Object();
			  var rallypoint = rallypoints[personals.getCurrentVillage()].units;
				  //var title = unit_row[u].cells[0].textContent;
				  //var name = unit_row[u].cells[7].childNodes[0].name;
				  if (unit_row.innerHTML.match("<a")) {
				  	  rallypoint[10] = "0";
				  } else {
					  rallypoint[10] = "1";
				  }
  ...
  'enhance_game_settings' : function() {
	...
	plugins.nobleCoins = { // One click to buy X coins at once
		...
		'enhance_game_snob' : function() {
			...
			var lowest_coin = 0;
			var coin_wood = Math.floor(parseInt(gid("wood").innerHTML)/cost_wood);
			var coin_iron = Math.floor(parseInt(gid("iron").innerHTML)/cost_iron);
			var coin_clay = Math.floor(parseInt(gid("stone").innerHTML)/cost_clay);
//log(1,coin_wood & coin_iron & coin_clay);
  ...
  'info' : {
   ...
   'dev' : "Timas"
	...
	plugins.enhanceOverview = { // Enhance the village overview page
		...
		'enhance_game_overview' : function() { // Clean up and enhance the village overview
				var myvils =getValue("myVillages");
				var tdSet = $('a:has(> img[src$="/graphic/command/attack.png?1"])')[0];
				if (tdSet) {
					tdSet = tdSet.parentNode.parentNode.parentNode;
					var attack = "1";
					var attack_1 = tdSet.cells[0].textContent;
					var attack_2 = tdSet.cells[1].textContent;
					var attack_3 = tdSet.cells[2].textContent;
					myvils[twe.curVillage].attack = attack;
					myvils[twe.curVillage].attack_1 = attack_1;
					myvils[twe.curVillage].attack_2 = attack_2;
					myvils[twe.curVillage].attack_3 = attack_3;
					setValue("myVillages",myvils);
				}
   ...
   'dev' : "Timas"
	...
	plugins.addToolbar = { // Add the premium-like toolbar
		...
		'enhance_game' : function() { // Add the premium-like toolbar
			//$("hr[@size='2']")[0].style.display = "none";
			//$("hr[@size='2']")[0].parentNode.removeChild($("hr[@size='2']")[0])
 		//make sure twe.curVillage is initalized
			...
			var menu_xhtml = '';
            //log(1,"tooooooo");
			//menu_xhtml += '<td><a href="/game.php?village=' + twe.curVillage + '&screen=overview_villages'+t+'"><img src="' + personals.getImageBase() + 'buildings/statue.png?1" style="margin:0px 2px" title=" ' + twe.lang("villages") + ' " /></a></td>';
			
			// Add villages dropdown
			var vils = twe.myVillages();
			var multivillage=false;
			for(var v in vils) { if(v!=twe.curVillage) { multivillage = true; break; } }
			if(twe.myVillages().length > 0) {
...
*/
				...
				//menu_xhtml += '<td><select onchange="switch_village(this)">';
				menu_xhtml += '<td>';
...
/*
				for (x = 0; x < grps.length; x++) {
					...
					for(var ij in vils) {
						var vil = vils[ij];
						if (vil['group'] == grp['id']) {
							option += '<option value="' + vil["id"] + '"' + (twe.curVillage == vil["id"] ? ' style="color:green;" selected="selected"' : '') + '>' + (vil['alias'] != '' ? decodeURIComponent(vil['alias']) : decodeURIComponent(vil["name"])) +" ("+ vil["x"]+'|'+vil["y"] + ') [' + vil["points"] + ']'  + '</option>';
							villist.push(vil['id']);
						}
					}
...
*/
//http://cn1.buluozhengxiong.asia/game.php?village=34588&screen=overview_villages
				var v_href = location.href.replace(/screen=([^&]+)/, "screen=overview_villages");
				var s_href = '';
				var v_list = '<table class="main" height="100%">' + 
						'<tr><th style="border-bottom:1px solid #987634;"><a href="javascript:villages_sort()">»</a>' + '<a href="'+ v_href + '">我的城堡</a>:</th></tr><tr><td>';
				v_list += '<table height="100%">';
				var bonuss = [0, 0, 0, 46, 88, 74, 60, 0,0];
				for(var ij in vils) {
					var vil = vils[ij];
					if (villist.indexOf(vil['id']) == -1) {
						...
						//menu_xhtml += '<option value="' + vil["id"] + '"' + (twe.curVillage == vil["id"] ? ' selected="selected"' : '') + '>' + (vil['alias'] != '' ? decodeURIComponent(vil['alias']) : decodeURIComponent(vil["name"])) + ' [' + vil["points"] + '] ' + vil["x"]+'|'+vil["y"] + '&nbsp;</option>';
							...
							v_href = location.href.replace(/village=([^&]+)/, "village=" + vil["id"]);
							s_href = v_href.replace(/screen=([^&]+)/, "screen=map");
							v_list += '<tr>';
							var v_name = (vil['alias'] != '' ? decodeURIComponent(vil['alias']) : decodeURIComponent(vil["name"]));
							//var v_bonus = (vil.bonus != '0'?'<span class="bonus_icon bonus_icon_'+ vil.bonus + '" title=" ' + vil.title + '"></span>':'');
							var attack = "";
							var attack_title = "";
							if (vil['attack'] == '1') {
								attack = "<img src='/graphic/command/attack.png?1'>";
								attack_title = vil['attack_1'] +"\n\r"+ vil['attack_2'] +"\n\r"+ vil['attack_3'] +"\n\r";
							} else {var attack = "";}
							var support = "";
							if (vil['support'] != 0) {
								support = "<img src='/graphic/command/support.png?1'>";
							}
							var bg_style = ' background-position:0px '+ bonuss[(vil.bonus-1)]+'px;';
							//var v_bonus = (vil.bonus != '0'?'<span class="bonus_icon" style="'+bg_style+'" title=" ' + vil.title + '"></span>':'');
var v_bonus = (vil.bonus != '0'?'<span class="bonus_icon bonus_icon_'+ vil.bonus + '" title=" ' + vil.title + '"></span>':'');
//						log(1,vil.bonus+"  "+vil.title + " "+ bonuss[(vil.bonus-1)]);
							if (twe.curVillage == vil["id"] ) {
								v_list += '<th style="border-top:1px solid;border-bottom:1px solid;">' + v_bonus + '</th>' +  
								//<img src="/graphic/ally_forum.png?1" height="8" />
											 '<th nowrap style="border-top:1px solid;border-bottom:1px solid;"><a href='+v_href+' title="' + attack_title +'">' + 
											  v_name + '</a>'+attack+support+'</th>' + 
											  '<th nowrap style="text-align:right;border-top:1px solid;border-bottom:1px solid;">' + 
											 '<a href=' + s_href + 
											 ' title="中心地图">' + vil["x"]+'|'+vil["y"] + '</a></th>';

							} else {
								v_list += '<td>' + v_bonus + '</td>' + 
											 '<td nowrap><a href='+v_href+' title="' +attack_title+'">' + 
											  v_name + '</a>'+attack+support+'</td>' + 
											 '<td nowrap align="left">' + 
											 '<a href=' + s_href + 
											 ' title="中心地图">' + vil["x"]+'|'+vil["y"] + '</a></td>';
							}
							v_list += '</tr>';
						}
					}
...
*/
	 ...
	 setFunc("villages_sort", function() { window.setTimeout(function() {
//	 var reports = plugins.reportcache.getReportsByVillage();
	 var myVillages = getValue("myVillages");
	 var ret = {};
	 var temp = {};
	 var villages = twe.myVillages();
	 var v = villages[personals.getCurrentVillage()];
	 ...
	 for (var id in villages) { 
		if (id != "length") {
			...
			//var dist = village["dist"];
			var distance_x = v.x - village.x;
			var distance_y = v.y - village.y;
			var dist = parseInt(Math.sqrt(Math.pow(distance_x,2) + Math.pow(distance_y,2)) * 1000000);
			//log(1,id + " / " +dist);
			if (!temp[dist]) {
				temp[dist] = id;
			} else {
			...
			j++;
			//log(1,j+"////temp["+dist+"]= "+id);
		} else { v_len = villages[id];}
	}
	...
	for (var dt in temp) {
		...
		//log(1,k+": temp[ "+tmp+" ] = "+temp[tmp]);	
		//log(1,tmp);
		var vid = villages[temp[tmp]].id;
		ret[vid] = villages[temp[tmp]];
		temp[tmp] = "@@@";
	}
	ret["length"] = v_len;
	//log(1,"length = " + v_len);
	setValue("myVillages", ret);

 
	 }, 0)});	 
	
//			menu_xhtml += '<td id="bonus"><a href="/game.php?village=' + twe.curVillage + '&screen=overview_villages'+t+'"><img src="' + personals.getImageBase() + 'buildings/statue.png?1" style="margin:0px 2px" title=" ' + twe.lang("villages") + ' " /></a><b>' + twe.lang("villages") +'</b></td>';
//menu_xhtml += '<td id="bonus"><a href="/game.php?village=' + twe.curVillage + '&screen=overview_villages'+t+'"><span class="bonus_icon bonus_icon_'+ vils[twe.curVillage].bonus + '" title=" ' + vils[twe.curVillage].title + ' " /></a></td>';

			var TW_Image_Base = personals.getImageBase();

 
//			menu_xhtml += '<td><a href="/game.php?village=' + twe.curVillage + '&screen=map'+t+'"><img src="http://i268.photobucket.com/albums/jj27/sharkale/overview.png" style="margin:0px 2px" title=" ' + twe.lang("map") + ' " /></a></td>';
			menu_xhtml = '<td><u><a href="/game.php?village=' + twe.curVillage + '&screen=overview'+t+'"><img src="' + TW_Image_Base + 'face.png" style="margin:0px 4px" title=" ' + twe.lang("overview") + ' " /><b>'  +(vils[twe.curVillage].alias != '' ? decodeURIComponent(vils[twe.curVillage].alias) : decodeURIComponent(vils[twe.curVillage].name)) + '</b> [<b>' +vils[twe.curVillage].points+ '</b>]</a></td>';			
			menu_xhtml += '<td style="border-left:dotted 2px #876534"><a href="/game.php?village=' + twe.curVillage + '&screen=main'+t+'"><img src="' + TW_Image_Base + 'buildings/main.png" style="margin:0px 2px" title=" ' + twe.lang("main") + ' " /></a></u></td>';
			...
			menu_xhtml += '<td style="border-left:dotted 1px"><a href="/game.php?village=' + twe.curVillage + '&screen=market'+t+'"><img src="' + TW_Image_Base + 'buildings/market.png" style="margin:0px 2px" title=" ' + twe.lang("market") + ' " /></a></td>';
			//menu_xhtml += '<td><a href="/game.php?village=' + twe.curVillage + '&screen=snob'+t+'"><img src="' + TW_Image_Base + 'buildings/snob.png" style="margin:0px 2px" title=" ' + twe.lang("snob") + ' " /></a></td>';
...
*/
			...
			var div_village = document.createElement("DIV");
			//div_village.setAttribute("style","vlign:top;");
			div_village.innerHTML = v_list;
			try {
				$("#menu_row2")[0].innerHTML = menu_xhtml;
				//$("table.content-border tr")[0].appendChild(div_village);
				$("table.main tr")[0].appendChild(div_village);
/*
				var cells = $("#menu_row2 td");
 
				var twt_xhtml = '<tr><td><div id="twtools_div" style="display: none;">';
				twt_xhtml += '<table><tr>';
				twt_xhtml += '<td><a href="http://' + twe.World + '.tw-tools.com/index.php?object=main&method=worldindex&lng=en&type=html" target="_blank"> TW工具</a>:</td>';
...
*/
  ...
  'info' : {
   ...
   'dev' : "Timas"
	...
	var spArray = [18, 22, 18, 18, 9, 10, 10, 11, 30, 30, 10, 35];
	var icoArray = ["spear","sword","axe","archer","spy","light","marcher","heavy","ram","catapult","knight","snob"];
	plugins.distancecalc = { // Calculate the time to travel to a village
		get_distance_num : function(from, to) {
		...
		get_distance : function(from, to) {
			...
			var distance_y = from[1]-to[1];
			var distance = Math.sqrt(Math.pow(distance_x,2) + Math.pow(distance_y,2));
			var ret = "<table cellpadding='0' width='100%'><tr>";
//			ret += "<th> </th>";
//			for (i=0; i<spArray.length; i++) {
//				ret += "<th><img src='/graphic/unit/unit_"+ icoArray[i]+".png?1'></th>";
//			}
//			ret += "</tr><tr>";
			ret += "<th>距离: <font color='green'>" + distance.toFixed(2) +"</font> 格&nbsp;</th>";
			for (i=0; i<spArray.length; i++) {
				ret += "<th><img src='/graphic/unit/unit_"+ icoArray[i]+".png?1'>"+ parseInt((spArray[i]*distance)/60) +":" + (parseInt(spArray[i]*distance) - parseInt((spArray[i]*distance)/60)*60) + "</th>";
			}
...
//  log(1,'get_distance_table - '+target);
	  html = "<table>";
	  html += "<tr><th>村庄</th><th>距离</th>";
	  ...
	  setFunc("distancecalctoggle", function() {
		 ...
		 if ($("#distancecalclink span").html() != '显示') { $("#distancecalclink span").html('显示'); } else { $("#distancecalclink span").html('隐藏'); }
	  ...
	  html += '<tr><td colspan="14"><a id="distancecalclink" href="javascript:distancecalctoggle();"><span>显示</span>全部村庄</a></td></tr>';
	  html += "</table>";
	  return html;
  },

  'enhance_game_info_village' : function() {
	  var target = xpathGetFirst("//table[1]/tbody/tr/td/table/tbody/tr[2]/td[2]").innerHTML;
	  //GM_log(target);
	
	  target = target.replace(/(.+)\((-?\d+\|-?\d+)\)\s(.+)/gi,"$2").split("|");
	  //GM_log(target);
	 //pathGetFirst("/html/body/table[1]/tbody/tr/td").innerHTML += plugins.distancecalc.get_distance_table(target);
	  xpathGetFirst("//table[1]/tbody/tr/td/table/tbody/tr[2]/td[2]").innerHTML += plugins.distancecalc.get_distance_table(target);
	  //GM_log(plugins.distancecalc.get_distance_table(target));
  },

  'enhance_game_report' : function() {
	  if(!/view=(\d+)/.test(location.href)) return;

	  var target = xpathGetFirst("/a/table/tbody/tr[3]/td/table[4]/tbody/tr[2]/td[2]/a");
	  if(!target) return;
	  target = target.innerHTML;
	  target = target.replace(/(.+)\((-?\d+\|-?\d+)\)\s(.+)/gi,"$2").split("|");
	  var content = xpathGetFirst("//table/tbody/tr/td[2]/table/tbody/tr/td/table/tbody/tr[3]/td");
	  content.innerHTML += plugins.distancecalc.get_distance_table(target);
  },
  'info' : {
	  'mandatory' : false,
...
//	  log(1,village_id);
//log(1,"get_rallypoint_html");	  
	  if(!rallypoint) return false;

	  var html = "<form name=\"units\" action=\"game.php?village="+personals.getCurrentVillage()+"&amp;screen=place&amp;try=confirm\" method=\"post\">\n";
	  ...
	  html += "<td align=\"top\" style=\"visibility:hidden;\"><a href=\"javascript:popup_scroll('targets.php?village="+personals.getCurrentVillage()+"&amp;mode=own', 609, 347)\">二重林</a></td>\n";
	  html += "<td rowspan=\"2\"><input class=\"attack\" name=\"attack\" value=\"攻击\" style=\"font-size: 10pt;\" type=\"submit\"></td>\n";
	  html += "<td rowspan=\"2\"><input class=\"support\" name=\"support\" value=\"支援\" style=\"font-size: 10pt;\" type=\"submit\"></td>\n";
	  html += "</tr></tbody></table>";
	  html += "</form>";
	  return html;
  },
...
//log(1,"enhance_game_report");		
//log(1,'$("br").length='+$("br").length );		
		var brArray = $("br");
		//log(1,brArray.length);
		for (i=0; i<brArray.length-0; i++) {
			brArray[i].setAttribute("style","display:none;");
		}
		/*
			$("br").each(function() {
				$(this).css({display: "none"});
		...
		*/
		//if ($('table[width="450"]')[0]) $('table[width="450"]')[0].setAttribute("width","600");
//log(1,$('table.vis tr.center').length);
		var trArray = $('table.vis tr.center');
		for (i=0; i<trArray.length; i++) {
			trArray[i].cells[0].setAttribute("width", "60");
		}
		
	  resourceTD = $('td:has(> img[src$="graphic/holz.png?1"])')[0];
	  ...
	  resourceTD.innerHTML += "&nbsp;&nbsp;<img src=\"/graphic/res.png\" title=\"现有资源总量\" alt=\"\"> 资源: " + resourceTotal + "&nbsp;<a href=\"javascript:insertUnit(document.forms[0].light, "+cavReturn+")\"><img src=\"graphic/unit/unit_light.png\" title=\"搬回资源需要轻骑\" alt=\"\">&nbsp;需要轻骑: " + cavReturn+"</a>";

//	  var enemy = xpathGetFirst("//table[@class='vis']/tbody/tr[3]/td/table[4]/tbody/tr[2]/td[2]/a").innerHTML;
//log(1,enemy);
...
//	  xpathGetFirst("//td[@height='160']").innerHTML += plugins.quickattack.get_rallypoint_html(personals.getCurrentVillage(),enemy_x,enemy_y);
  ...
  enhance_game_place : function() {
	  ...
	  var rallypoints = getValue("rallypoints");
	  if(!rallypoints) rallypoints = new Object();
	  if (!rallypoints[personals.getCurrentVillage()]) {
			rallypoints[personals.getCurrentVillage()] = {'units':rallypoint,'timestamp':now,'expires':expires};
	  } else {
		  rallypoints[personals.getCurrentVillage()].timestamp = now;
		  rallypoints[personals.getCurrentVillage()].expires = expires;
	  }
	  setValue("rallypoints", rallypoints);

	  xpathGetFirst("//table[1]/tbody/tr/td/table[2]/tbody/tr/td[2]").innerHTML += plugins.quickattack.get_overview();
...
//		  input.dispatchEvent(evt);
  ...
  'get_overview' : function() {
	  ...
	  html += "<table><tr><td></td><th><a href='http://cn1.buluozhengxiong.asia/game.php?village=45696&screen=overview_villages'>村庄</a></th>\n";
	  html += plugins.quickattack.get_overview_heading();
/*
	  html += "<th>更新</th>";
	  html += "<th>剩余</th>";
*/
	  html += "<th><img src='/graphic/face.png?1'></th></tr>";

	  var now = new Date().getTime();
	  TW_Villages = twe.myVillages();
	  var count = 0;
	  var u_people = [1, 1, 1, 1, 2, 4, 5, 6, 5, 8, 10, 100, 0, 0];
	  var c = {};
	  var d = {};
	  for(var j=0;j<100;j++) { 
			c[j] = 0;
	  ...
	  for(var i in TW_Villages) {
		 ...
		 //log(1,v);
		if (v.id) {
			bg_style = ' background-position:0px '+ bonuss[(v.bonus-1)]+'px;';
			v_bonus = (v.bonus != '0'?'<span class="bonus_icon" style="'+bg_style+'" title=" ' + v.title + '"></span>':'');
		  out = (v.alias != '') ? v.alias : v.name;
		  if (bcolor%2 == 0) { style = ' style="background: #dec8a3;"'; } else { style = ' style="background: none;"'; }
		  bcolor += 1;
		  html += "<tr"+style+"><td>" + v_bonus+ "</td>" + "<td><a href=\"javascript:location.href=location.href.replace(/village=([^&]+)/, 'village=" + v.id+"')\"> "+out+"&nbsp;&nbsp;&nbsp;&nbsp;  </a></td>";
		  if(!rallypoint) {  
...
//		  html += plugins.quickattack.get_overview_row(v.id);
		  ...
		  for(var j=0;j<arr.length;j++) {
			if (parseInt(arr[j]) > 0) {
				  html += "<td>"+ arr[j] +"</td>";
				  c[j] += parseInt(arr[j]);
				  d[j] += parseInt(arr[j]) * u_people[j];
				  v_count += parseInt(arr[j]) * u_people[j];
				  //html += "<td>"+(parseInt(arr[j])>0? arr[j] : "")+"</td>";
			} else {
				html += "<td><font color='grey'> - </font></td>";
			}
		  }
		  html += "<td>"+ v_count +"</td>";
/*
		  html += "<td>"+Math.round(((now-rallypoint.timestamp)/1000)/60)+" 分钟之前,</td>";
		  if(rallypoint.expires === 0) {
			  html += "<td></td>";
		  } else {
			  html += "<td> "+Math.round(((rallypoint.expires-now)/1000)/60)+" 分钟</td>";
		  }
*/
		  }
		  html += "</tr>\n";
	  ...
	  for (k=0; k< count; k++) {
			if (parseInt(c[k]) > 0) {
				...
				html += "<th><font color='grey'> - </font></th>";
			}		
	  }
	  html += "<th>"+ cc +"</th></tr><tr><td></td><th><img src='/graphic/face.png?1'></th>";
	  cc = 0;
	  for (k=0; k< count; k++) {
			if (d[k] > 0) {
				 html += "<th>"+ d[k] +"</th>";
  ...
  'get_overview_row' : function(vid) {
	  ...
	  var html = "";
	  for(var j=0;j<arr.length;j++) {
		  html += "<td>"+(parseInt(arr[j])>0? arr[j] : "<font color='grey'>"+arr[j]+"</font>")+"</td>";
		  //html += "<td>"+(parseInt(arr[j])>0? arr[j] : "")+"</td>";
	  }
	  return html;
  },
 ...
 enhance_game_map : function() {
	 ...
	 log(3,"enhance_game_map :");
	 $("#info_content #info_extra_info").after("<tr><th></th><td id=\"info_reportcache\"></td></tr>");
...
var res = 	"<table cellspacing='0' cellpadding='0' width='100%'>" + 
				"<tr><th>最新报告：</th><th>"+report.timefull + "</th><td>&nbsp;</td>" + 
				"<th>资源库存：</th><th>" + report.resources+ "</th></tr>" + 
				"</table>" + 
				"<table cellspacing='0' cellpadding='0' width='96%'><tr>";
var cr = 0;
for (var b in report.buildings) {
	if (cr<4) {
		res += "<td nowrap>" + buildings[b] + ":</td><td nowrap>" + report.buildings[b] + "</td>&nbsp;";
		cr++;
	}
	else {
		res += "<td nowrap>" + buildings[b] + ":</td><td nowrap>" + report.buildings[b] + "</td></tr><tr>";
		cr = 0;
	}
}
res += "</tr></table>";
...
/*
				var res =  report.timefull + 
							"<br>"+ 
							report.resources + 
							"<br>" + 
							" 木:" + report.buildings['wood'] + 
							" 泥:" + report.buildings['stone'] + 
							" 铁:" + report.buildings['iron'] + 
							"<br>" + 
							(report.buildings['wall']?  report.buildings['wall'] : "&nbsp;");
*/
			  }
			 $("#info_reportcache").html(res);
		 }, 0);
	 });
 },
 _get_report_html : function(rCount) {
	 var html="";
	 var report;
	 var reports = plugins.reportcache.getReportsByVillage();
	 //var reports = plugins.reportcache.getReportsArray();
	 var villages = twe.myVillages();
	 var v = villages[personals.getCurrentVillage()];
	 var vcoords = [v.x,v.y];
	 var i=0;
	 var jj =0;
	 var iStorage = 1000;
	 var iStart = rCount -10;
	 for (var report_id in reports) { i++;}
	 if (rCount > i) {
//log(1,(rCount > reports.length)+" / iStart: "+ iStart + "  /  rCount: "+ rCount + "  /   reports.length: " + i);		 
		 rCount = i;
		 }
	 i = 0;
	 if (rCount  == 0) {
	 ...
	 for (var report_id in reports) {
		...
		if (i < iStart) { 
			i++;
			 ...
			 jj += 1;
			 report = reports[report_id];
			 //log(1,report.villageid);
				//log(1, "reportcache report="+uneval(report));
				// age in seconds
...
//			 var sSecond = parseInt(((sTime - sHour)*60 - sMinute)*60);
			...
			if (report.player != "不知名的") {
				var rplayer = report.player + "<span style='float:right;color:red;'>" + (jj+iStart) + "&nbsp;</span>";
			} else var rplayer = "<span style='float:right;color:red;'>" + (jj+iStart) + "&nbsp;</span>";
			 html += "<table class=\"report\" cellpadding='0' width='100%'>";						
			 html += "<tr><th width='130px'>玩家/村庄/日期</th><th nowrap>"+rplayer + 
						 "&nbsp;&nbsp;<a href=\"game.php?village="+personals.getCurrentVillage() +   "&amp;screen=info_village&amp;id="+report.villageid+"\" target='_blank'>["+report.villagefull+"]</a>" + 
						 "&nbsp;&nbsp;" +report.timefull+", 大约<font color='green'> "+sTime+ ":00 </font>之前" + 
						 "&nbsp;&nbsp;(<a href='game.php?village="+personals.getCurrentVillage() + 
						 "&amp;screen=report&amp;mode=attack&amp;view="+report.id+ "'" + 
						 "target='_blank'>察看完整报告</a>)" + 
						 "</th></tr>";

			 html += "<tr><th>"+v.name+"<br>("+vcoords+")</th><td>"+plugins.distancecalc.get_distance(vcoords,report.coords) + "</td></tr>";

		//	 html += "<tr><td colspan=\"0\">";
			 html += "</td></tr></table>";

			 html += "<table width='100%' cellpadding='0'>";
			 html += "<tr><th width='130px'></th><th><img src=\"/graphic/holz.png\">木材</th><th><img src=\"/graphic/lehm.png\">泥土</th><th><img src=\"/graphic/eisen.png\">铁块</th><th><img src=\"/graphic/res.png\">资源总量</th></tr>";
			 var sum = 0;
			 for(var j in report.resources) { sum += report.resources[j]; }
			 html += "<tr><th>库存资源(<font color='green'>等级</font>/产量)</th>";
			 if(report.buildings.wood||report.buildings.stone||report.buildings.iron) {			 
				  var lvl = [];
				  lvl[0] = report.buildings.wood;
				  lvl[1] = report.buildings.stone;
				  lvl[2] = report.buildings.iron;
				  for (var irs=0; irs<report.resources.length; irs++) { 
							html += "<td>" + report.resources[irs] + 
										"&nbsp;(<font color='green'><b>"+ 
										lvl[irs] + 
										"</b></font>/"+ 
										plugins.reportcache._production[lvl[irs]-1]+ 
										")</td>"; 
				 }
				 html += "<td>" +	sum+ "</td></tr>\n";
//log(1,report.buildings.wood+"\n\r"+report.buildings.stone+"\n\r"+report.buildings.iron);

				 var resources = report.resources.slice(); // slice = clone array
				 resources[0] += Math.round(plugins.reportcache._production[report.buildings.wood-1]*(age/3600));
				 resources[1] += Math.round(plugins.reportcache._production[report.buildings.stone-1]*(age/3600));
				 resources[2] += Math.round(plugins.reportcache._production[report.buildings.iron-1]*(age/3600));
//	log(1,Math.round(plugins.reportcache._production[report.buildings.wood-1])+"\n\r"+age/3600);
				var storage = 1000;
				var hide = 150;
				var iconst = 1.2295111;
				var hconst = 1.333333;
				for (ir=1; ir < parseInt(report.buildings['storage']); ir++) {
						storage = storage*iconst;
				}
				for (ir=1; ir < parseInt(report.buildings['hide']); ir++) {
						hide = hide*hconst;
				}	
				storage = parseInt(storage);	
				hide = parseInt(hide+0.5);
				sum = 0;
				for(var ires in resources) {
					sum += resources[ires]; 
				}
				var isum = sum;
				if (isum > (storage-hide)*3) isum = (storage-hide)*3;
				var cavReturn = 0;
				var cavCount = 0;
				if (isum != 0){
					  cavReturn = isum / 80;
					  cavReturn = Math.ceil(cavReturn) ;
					  cavCount = Math.ceil(Math.ceil(cavReturn/3)/100)*100;
				};				

				html += "<tr><th>预计资源(<font color='red'>库容</font>-<font color='red'>山洞</font>)</th>" + 
							"<td>"+ resources[0] + "(<font color='red'>" +(storage-hide) + "</font>)</td>" + 
							"<td>"+ resources[1] + "(<font color='red'>" +(storage-hide) + "</font>)</td>" + 
							"<td>"+ resources[2] + "(<font color='red'>" +(storage-hide) + "</font>)</td>" + 
							"<td><font color='red'><b>" +sum+ "</b></font>" + 
							"(<font color='#339989'>"+ (storage-hide)*3 + "</font>)</td>" + 
							"<td>";
// ------------------------
					html +=	 "<a href='javascript:{" + 
								 "document.forms[0].spy.value= \"1\";"+ 						 
								 //"document.forms[0].light.value= \"400\";"+ 
								 "document.forms[0].x.value= "+ report.coords[0] + ";"+ 
								 "document.forms[0].y.value= "+ report.coords[1] + ";" + 
								 "document.forms[0].attack.click();" + 
								 "document.forms[0].submit();" + 
								 "}'>&nbsp;<img src='graphic/unit/unit_spy.png?1'></a>";	
								 
				 html += "&nbsp;<a href='javascript:{"+ 
							 "insertUnit(document.forms[0].spy, \"1\");"+ 						 
							 "insertUnit(document.forms[0].light, \""+ (cavReturn+10) + "\");"+ 
							 "insertUnit(document.forms[0].x, \""+ report.coords[0] + "\");"+ 
							 "insertUnit(document.forms[0].y, \""+ report.coords[1] + "\");" + 
							 "}'>" +  
							 "&nbsp;<img src='graphic/unit/unit_light.png'></a>";
							 
				html +=  "&nbsp;<a href='javascript:{" + 
							 "document.forms[0].spy.value= \"1\";"+ 						 
							 "document.forms[0].light.value= "+ (cavReturn+10) + ";"+ 
							 "document.forms[0].x.value= "+ report.coords[0] + ";"+ 
							 "document.forms[0].y.value= "+ report.coords[1] + ";" + 
							 "document.forms[0].attack.click();" + 
							 "document.forms[0].submit();}'>" + 
							 "&nbsp;<img src='graphic/command/attack.png?1'>(" +  		
							 (cavReturn+10) + ")</a>&nbsp;";	

				if (cavReturn>cavCount*1.5) {
					html +=	 "<a href='javascript:{" + 
								 "document.forms[0].spy.value= \"1\";"+ 						 
								 "document.forms[0].light.value= "+ cavCount*1.5 + ";"+ 
								 "document.forms[0].x.value= "+ report.coords[0] + ";"+ 
								 "document.forms[0].y.value= "+ report.coords[1] + ";" + 
								 "document.forms[0].attack.click();" + 
								 "document.forms[0].submit();" + 
								 "}'>&nbsp;<img src='graphic/command/attack.png?1'>("+ cavCount*1.5 + ")</a>";
				} 

				if (cavReturn>cavCount*2) {
					html +=	 "<a href='javascript:{" + 
								 "document.forms[0].spy.value= \"1\";"+ 						 
								 "document.forms[0].light.value= "+ cavCount*2 + ";"+ 
								 "document.forms[0].x.value= "+ report.coords[0] + ";"+ 
								 "document.forms[0].y.value= "+ report.coords[1] + ";" + 
								 "document.forms[0].attack.click();" + 
								 "document.forms[0].submit();" + 
								 "}'>&nbsp;<img src='graphic/command/attack.png?1'>("+ cavCount*2 + ")</a>";
					}
					html += "</td></tr>\n";
			 }
			 ...
			 i++;
			 //log(1,"i: " + i + "   rCount: " + rCount);
			 if (i == rCount ) {
								html += "<table cellpadding='0' width='100%'><tr><th width='130px'>快捷出兵</th><td>" + 
								plugins.quickattack.get_rallypoint_html(v.id,"","") + 
								"</td></tr>" + 
								//"<tr><th></th><th>" + 
								//"<a href='javascript:void(0);' id='wAttack'>多波抢劫</a></th></tr>" +
								"</table>";	 
							}
...
//GM_log(timeTD.innerHTML);
	 ...
	 } else if(/buluozhengxiong.asia/.test(location.host)) {
			//example: 07.02.09 19:24
		 var tmp = timeTD.innerHTML.match(/(\d+)\.(\d+)\.(\d+) (\d+):(\d+)/);
		 time = new Date("20"+tmp[3], parseInt(tmp[2])-1, tmp[1], tmp[4], tmp[5]).getTime();
	 } else {
...
//		start : "----------" + report_id + "---------->",
	 ...
	 var priceArray = [0,90,130,130,190,120,475,500,950,700,820,80,140000,0];
	 var peopleArray = [ 0, 1, 1, 1, 1, 2, 4, 5, 6, 5, 8, 10, 100,0];
	 var priceTotal = 0;
	 var peopleTotal = 0;
	 var uLost = 0;
	 var target = $("table[width='100%']:has(table.vis img[src*='graphic/unit/unit_spear.png?1']):eq(0)")[0];
	// log(1,target.innerHTML);
	 var rLost = $('tr:eq(5) td', target);	
	 ...
	 peopleTotal = 0;
	 priceTotal = 0;
	 var target = $("table[width='100%']:has(table.vis img[src*='graphic/unit/unit_spear.png?1']):eq(1)")[0];
//	 log(1,target.innerHTML);
	 var rLost = $('tr:eq(5) td', target);	
	 for (r=1;r<rLost.length;r++) {
		if (parseInt(rLost[r].innerHTML) > 0) {
			...
			priceTotal += priceArray[r] *uLost;
			peopleTotal += peopleArray[r] * uLost;
//			log(1,"peopleArray[r]"+peopleArray[r]+"* uLost"+uLost+"="+ peopleArray[r] * uLost);
...
//	 var target = $("table[width='100%']:has(table.vis img[src*='graphic/unit/unit_spear.png?1']):eq(1)")[0];
	...
	// GM_log(resourceTD.innerHTML);
	 ...
	 var buildingsTD = $('tr:has(> td > img[src$="graphic/holz.png?1"]) + tr:first > td')[0];
...
// log(1,uneval(plugins.reportcache._buildings));
 ...
 enhance_game_main : function() {
	 ...
	 $('table[class=vis]:has(tr:eq(1) td:eq(6)) tr td:first-child').each(function() {
		 ...
		 if (!buildings[tmp[1]]) buildings[tmp[1]] = tmp[2];
//		 	 log(1,tmp[1]+" -- "+tmp[2]);
	 });
...
//		 log(1,"buildings["+reverse[building]+"] = "+level)
  ...
  'enhance_game_map': function(){
	  ...
	  var iHtml = '<table><tr><td colspan="2">&nbsp;</td></tr><tr><td align="right">地图大小: </td><td><select id="map_size_select" onchange="resize_map()">';
	  ...
	  iHtml += '</select></td></tr>';
	  iHtml += '<tr><td align="right">玩家标签: </td><td><input type="text" id="mark_player" size="8" /><input type="text" id="mark_tribe" size="8" /><a href="javascript:search_map();"><img src="'+TW_Image_Base+'/map/map_e.png" /></a></td></tr>';
...
// Kicker
	for(var pname in plugins) {
		...
		if (plugs.indexOf(pname) == -1) {
			...
			if(func) { func(); }
//log(1,"plugins[" +pname+"],[enhance_"+twe.Page+"_"+twe.Screen+"]");
		}
	}
	...
	var iStep = 10;
	var iCount = ((i/iStep)+1);
	var strRC = '&nbsp;';
	strRC += '<a href="javascript:report_sort()">' + 
//				'<img src="http://cn1.buluozhengxiong.asia/graphic/unit/speed.png?1" height="17">' + 
					'» ' + 
					'</a>';
	for (ii=1; ii<iCount; ii++) {
		strRC += '<a href="javascript:reportcache(' + 
					   ii * iStep + 
					  ')" title=" ' + ((ii-1)*iStep+1) + '~' + ii*iStep + ' "' + '>' + 
//					  '<img src="file://e:/six/' + ii + '.gif" height="17">' + 
					  '~' + ii + 
					  '</a>';
	}
	 $('table[@class="menu nowrap"]:first tr td:eq(8)').append(strRC);
	var strForum = '<a href="http://cn1.buluozhengxiong.asia/game.php?village=34360&screen=ally&mode=forum"><img src="http://cn1.buluozhengxiong.asia/graphic/ally_forum.png?1" height="8"></a>' ;
	var menurow = xpathGetAll("//tr[@id='menu_row']/td");
	menurow[10].parentNode.removeChild(menurow[10]);
	if (menurow[7].innerHTML.indexOf("ally_forum.png") == -1)
			menurow[7].innerHTML = strForum + menurow[7].innerHTML;
	menurow[5].parentNode.removeChild(menurow[5]);
	menurow[1].parentNode.removeChild(menurow[1]);
	menurow[0].parentNode.removeChild(menurow[0]);
	//$('table[@class="menu nowrap"]:first tr td:eq(7)').append(strForum);
	 setFunc("reportcache", function(iCount) { window.setTimeout(function() {
	 //log(1,iCount);
		 //$("table.main table:first tr:first > td:eq(1)").html(plugins.reportcache._get_report_html(iCount*10));
		 $("table.main tr:first > td").html(plugins.reportcache._get_report_html(iCount));
	 //gid("wAttack").addEventListener("click", waveAttack(), 0);		 
		//$("table.navi-border tr:first > td:eq(1)").html(plugins.reportcache._get_report_html());
		//GM_log($("table.navi-border tr:first > td:eq(1)").html(plugins.reportcache._get_report_html()));
	 }, 0)});	 
	 setFunc("report_sort", function() { window.setTimeout(function() {
//	 var reports = plugins.reportcache.getReportsByVillage();
	 var reports = getValue("reports");
	 var ret = {};
	 var temp = {};
