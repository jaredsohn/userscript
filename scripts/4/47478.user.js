scr_meta=<><![CDATA[
// ==UserScript==
// @name		Troys Twitter Script [+ Pete's Mailana link]
// @namespace	http://twitter.com/troynt
// @description	Nested Replies, RT button, Custom Search Tabs, YouTube Embed, TwitPic Embed, URL Expansion, Hash Tag Search Links [ + Pete's Mailana link ]
// @version		7.7.1
...
tnt_twitter = {
	...
	help:{
		settings:{			
            ...
            'mailana_link':'Creates a link for the Mailana graph',
	...
	init:function(){
		if(typeof unsafeWindow.jQuery == 'undefined') { window.setTimeout(this(),100); return; }
		...
		/* tab stuff */
		css += "#tabMenu li { position:relative; }";
		css += "#tabMenu .remove-tab { visibility:hidden; color:#800; position:absolute; top:0; right:0; }";
		css += "#tabMenu li:hover .remove-tab { visibility:visible; }";
		...
		css += '#tnt-settings label { padding:.5em 1em; }'
		css += '#tnt-settings label:hover { background:#ffe; }'
		css += '#tnt-settings span  { display:inline-block; width:9em; line-height:2em; height:2.3em; }'
		
		css += "ol.statuses .thumb { overflow:visible; }"
		
		/* social links */
		css += '#user_links_div img { margin:0 5px 5px 0; }'
		...
		if( tnt_twitter.can('hide_twitter_defs') )
			css += '#profile .promotion { display:none; }'
		...
		else if( $('table.doing').length > 0 )
		{
			tnt_twitter.process_friend($('.vcard'));	
		}
		else if( $('#profile .profile-head').length == 1 )
		{
			var user = $.trim($('#profile .about .fn:first').text())

			$msg_link = $('#side_base .section a[href^=/direct_messages/create/]');
			...
			if( tnt_twitter.can('mailana_link') ) tnt_twitter.profile_add_mailana_link();			
		...
		tnt_twitter.tweet_process($('.hentry,#permalink'));
		
	},
	profile_add_mentions:function(){
		var user = '@' + $('meta[name=page-user-screen_name]').attr('content');
		tnt_twitter.profile_add_custom_search_tab(user,user,false,true,true)
	},
	friend_show_icon:function(user,callback)
	{
		...
		else if( tnt_twitter.logged_in && tnt_twitter.user != user && !tnt_twitter.stranger_cache[user] )
		{
			$.get('http://twitter.com/'+user,function(resp){
				$resp = $(resp);
				$dm_link = $resp.find('#side_base .section a[href^=/direct_messages/create/]')
	...
	{		
		...
		html+= '<a target="_blank" href="http://twitter.com/troynt">@troynt</a> <a id="tnt-settings-link" onclick="$(\'#tnt-settings\').toggle(); return false;" href="#">settings</a> | <a target="_blank" href="http://getsatisfaction.com/troynt/products/troynt_twitter_greasemonkey_script">help</a>'
		...
		html = '<small style="margin-top:1em">Hover over setting for brief description.<br />Refresh after you have made your changes.</small>';
	...
	{
		$last_section = $('#side .section:last');
		$notes_section = $last_section.clone();
		$notes_section = $notes_section.attr('id','tnt-twitter-notes');
		...
		$notes_section.html('<h3>Notes</h3><pre class="notes-body">'+ notes_txt +'</pre>');
		$last_section.after($notes_section);
		tnt_twitter.notes_body_click($notes_section.find('.notes-body'));
	},
	...
	profile_add_social_links:function()
	{
		...
		if( !tnt_twitter.social_link_cache[user] )
		{		
			GM_xmlhttpRequest({
				method: "GET",
				...
				onload: function(resp)
				{
					...
					social_links = resp.match(/<div id="user_links_div">\n.*<\/div>/);
					tnt_twitter.save('social_link_cache',user,social_links,function(){
		...
		if( social_links && social_links[0].split('img').length >2 ) 
		{
			social_links = (social_links + '').replace('src="/images/icons/','src="','gi');
			for( site in social_icons )
			{
				social_links = social_links.replace(site+'.png',social_icons[site]);
			}
			var $tmp = $(social_links)
			$tmp.find('span').remove();
			//$tmp.find('a[href^=http://twitter.com],a[href^=http://www.twitter.com]').remove();
			var html = '';
			$tmp.find('div').each(function(){ html += $(this).html() });
			social_links_html = html;
		}
		
		
		if( tnt_twitter.social_graph_cache[user] )
		{
			$cur_links = $(social_links_html);
			var profile_url = '';
			...
			for( url in nodes )
			{
				if( nodes[url] > 2 && $cur_links.find('a[href='+url.replace(/\/$/,'')+']').length == 0
					&& url != profile_url && url.indexOf('mailto:') == -1
					&& url.indexOf('sgn://') == -1
				)
				{
					url = url.replace(/\/$/,'') //remove trailing slash
					social_links_html += '<a class="social-graph-link" target="_blank" href="'+url+'"><img class="links-image" align="absmiddle" alt="'+url+'" src="'+get_social_icon(url)+'" /></a>';
				}
			}
			console.log(nodes)
	...
	profile_add_mailana_link:function()
	{
		var user = tnt_twitter.user_from_url(window.location+'');
        $('#profile .about li:last').after('<li><div id="mailana_link_div"><a href="http://twitter.mailana.com/profile.php?person='+ user + '">Mailana Graph</a></div></li>')
	},
	 ...
	 */
	process_friend:function($friends)
	{
		...
		$friends.each(function(){
			...
			if( tnt_twitter.can('show_bios') )
				$t.find('.person-actions').before('<small class="tnt-bio">'+tnt_twitter.linkify_plain($user_link.attr('title'))+'</small>')
				
			var user = tnt_twitter.user_from_url($user_link.attr('href'))
			tnt_twitter.save_username(user);
			if( tnt_twitter.can('groups') )
			{
				...
				group_html += '<div class="group-add" style="display:none;"><select class="group-select"><option value="create">Create a Group</option>'+tnt_twitter.group_get_opts()+'</select><button class="tnt-okay small">OK</button>'
				group_html += ' or <a href="#" class="tnt-cancel" href="#">Cancel</a></div></div>'
				$act = $t.find('.person-actions:first').append(group_html);
				
				tnt_twitter.render_group_list(user,$t.find('.tnt-group-list'));
				
				$act.find('.tnt-cancel').click(function(){ $(this).parent().hide().parent().find('.add-group').show(); return false;})
				...
				$act.find('.add-group').click(function(){
	...
	profile_add_custom_searches:function()
	{
		$('#public_timeline_tab').parent().after('<li><a id="search-tab">Search <form><input type="text" /></form></a></li>');
		$('#search-tab form').submit(function(){
			var $input = $('#search-tab input');
			var v = $input.val();
			tnt_twitter.profile_add_custom_search_tab(v,v,true)
			$input.val('');
			return false;
		});
		$.each(tnt_twitter.custom_searches,function(label,query){ tnt_twitter.profile_add_custom_search_tab(label,query); });
	},
	profile_add_custom_search_tab:function(label,query,trigger,temp,first)
	{
		...
		if( trigger ) $('#tabMenu li:last').find('a:first').trigger('click');
	},
	group_refresh_searches:function()
	{
		$.each(tnt_twitter.group_cache,function(group,members){
			...
			if( memberArr.length > 0 ) query = '@'+memberArr.join(' OR @');
			if( query.length >= 140 )
			{
	...
	status_autocomplete:function()
	{
		...
		$(window).keyup(function(e){
			if( (e.keyCode == 9 || e.keyCode == 13 ) && $('#auto-complete:visible').length == 1 )
			{
				...
				var v = $status.val();
				$status.val(v + ' ');
				s.setSelectionRange(v.length+1, v.length+1);
				return true;	
			}
		});
		...
		$status.keyup(function(e){
			...
			if( matches && matches.length >= 3 && !tnt_twitter.username_cache[matches[2]]  )
			{
				...
				var possible_matches = tnt_twitter.possible_friend_matches(user,is_dm);
				var friend_list = '';
				var cur_match = null;
				for( idx in possible_matches )
				{
					...
					friend_list += '<li><a title="'+friend+'" href="#">'+ friend +'</a></li>'
				}
				...
				if( friend_list != '' )
				{
					...
					$status.val($status.val()+last_part_match)
					if(s.createTextRange) {
						var range = s.createTextRange();
						range.move('character', orig_length);
						range.select();
					}
					else {
						if(s.selectionStart) {
						s.focus();
						s.setSelectionRange(orig_length, $status.val().length);
					}
					else
						s.focus();
					}
					...
					$status_autocomplete.find('a').click(function(){
						tnt_twitter.status_replace(cur_match,action+$(this).attr('title')+' ')
						$status.focus();
	...
	tweet_process:function( $tweets )
	{
		...
		$tweets.each(function(){
			...
			var user = tnt_twitter.user_from_url($tweet.find('.entry-date').attr('href'));
			...
			$tweet.find('.entry-content a[rel=nofollow]').each(function(){
				$(this).text($(this).attr('href'));
			});
			tnt_twitter.tweet_add_retweet_button($tweet);
			var reply_link = $tweet.find('.entry-meta a[href^=http://twitter.com]').not('.entry-date').filter(function(){ return $(this).text().match('^in reply to') });
	...
	{
		try { var st_id = $tweet.find('.entry-content').text().match('^([0-9]+)')[1] }
		catch( err ){ return; }
		
		$tweet.find('.entry-content a[rel=nofollow]:last').remove();
	...
	{
		var $link = $tweet.find('.entry-content a[rel=nofollow]:last');
		if( $link.length == 0 ) return;
		
		var link_url = $tweet.find('.entry-content a[rel=nofollow]:last').attr('href');
		try { var id = link_url.match('http://www.fmylife.com/.*/([0-9]+)')[1]; }
		catch( err )
		{
		...
		tnt_twitter.cache_ajax({
			method: "GET",
			url: 'http://api.betacie.com/view/'+ id +'/nocomment&language=en&key=readonly',
		},
		function(resp) {
			if( resp.status != 200 ) return [];
			var txt = $(resp.responseText).find('text').text()
			if( txt == '') return;
			txt = txt.replace('FML','<a target="_blank" href="'+ link_url +'">FML</a>');
			$tweet.find('.entry-content').html(txt);
		});
	},
	tweet_add_retweet_button:function($tweet)
	{
		...
		$reply_btn = $tweet.find('.actions .reply');
		...
		link = link.split('&')
		link[0] = '/home?status='+encodeURIComponent('RT @'+user+' '+$tweet.find('.entry-content').text().replace("\n",'','g') )
		link = link.join('&');
	...
	linkify:function( $tweet )
	{
		...
		if( $content.length == 1 )
			$content.html($content.html().replace(/( |^)\#([a-zA-Z0-9.]+)/gi, ' <a rel="nofollow" class="expanded hash-tag" href="http://search.twitter.com/search?q=%23$2" title="Search for #$2" target="_blank">#$2</a>'));
		if( tnt_twitter.can('search') && $('#home').length == 1 )
		{
			$content.find('.hash-tag').click(function(){
				var label = $(this).text();
				tnt_twitter.profile_add_custom_search_tab(label,label,true);
				return false;
			});
		}
	},
	linkify_plain:function(text)
	{
		...
		text = text.replace(/(^|\s)@(\w+)/gi,'$1@<a href="http://twitter.com/$2">$2</a>')
	...
	expand_urls:function( $tweet )
	{
		$tweet.find('a[rel~=nofollow]').not('.expanded').each(function(){
			...
			else if( domain.indexOf('twitpic.com') > -1 && tnt_twitter.can('image_embed') )
			{
				$t.css('float','right');
				$t.html('<img class="tnt-image" src="http://twitpic.com/show/thumb/'+ url_parts[1] + '" />');
			}
			else if( url.indexOf('youtube.com/watch?v=') > -1  && tnt_twitter.can('video_embed') )
			{
				var video_id = url.match('=([a-zA-Z_0-9\-]+)')[1];
				var title = $t.attr('title')
				if( title ) $t.text(title);
				
				var embed_html = '<br /><object width="425" height="373"><param name="movie" value="http://www.youtube.com/v/' +
			...
			else if( domain.indexOf('movapic.com') > -1 && tnt_twitter.can('image_embed') )
			{
				$t.html('<img align="right" src="http://image.movapic.com/pic/m_' + url_parts[2] + '.jpeg" />');
			}
			...
			else if( domain == 'hellotxt.com' && url_parts[1] == 'i' && tnt_twitter.can('image_embed')  )
			{
				$t.html('<img class="tnt-image" src="http://hellotxt.com/image/'+url_parts[2]+'.m.jpg" />')
			}
			else if( domain == 'ping.fm' && url_parts[1] == 'p' && tnt_twitter.can('image_embed') )
			{
				tnt_twitter.cache_ajax({
						method: "GET",
						url: $t.attr('href')
	...
	user_from_url:function( url ){
		if( !url ) return false;
		var user = url.match('http://twitter.com/([a-zA-Z0-9_]+)');
		if( user ) user = user[1];
		return user;
	},
	...
	tweet_get:function(permalink,callback)
	{
		...
		{
			if( typeof callback == "function" ) callback(tnt_twitter.tweet_cache[ status_id ]);
		}
		else
		{
			$.get(permalink,function(resp){
				...
				var tweet = {
					...
					text: $tweet.find('.entry-content').html(),
					created_at: $tweet.find('.entry-date span').attr('title')
				}
				
				var $in_reply_to_link = $tweet.find('.entry-meta a[href^=http://twitter.com]').not('.entry-date').filter(function(){ return $(this).text().match('^in reply to') });
				...
				if( typeof callback == "function" ) callback(tweet);
			});
		}
	},
	/**
	...
	tweet_load_reply:function(reply,$tweet)
	{
		...
		reply_html += '<a rel="bookmark" class="entry-date" href="http://twitter.com/'+user+'/status/'+ reply.id +'"> <span title="'+ reply.created_at +'" class="published">#</span></a>';
		
		if( reply.in_reply_to_status_id )
			reply_html += ' <a style="display:none" href="http://twitter.com/'+ reply.in_reply_to_screen_name +'/status/'+ reply.in_reply_to_status_id  +'">in reply to '+ reply.in_reply_to_screen_name +'</a>'
	...
	load_tweet:function(tweet)
	{
		...
		{
			klass += ' reply';
			reply_meta += ' <a style="display:none" href="http://twitter.com/'+ tweet.in_reply_to_screen_name +'/status/'+ tweet.in_reply_to_status_id  +'">in reply to '+ tweet.in_reply_to_screen_name +'</a>'
		}
		
		tweet_html = '<li id="status_' + tweet.id +'" class="'+ klass +'">'
		tweet_html += '<span class="thumb vcard author">'+ img +'</span>'
		tweet_html += '<span class="status-body"><strong><a href="http://twitter.com/'+user+'">'+ user +'</a> </strong>'
		tweet_html += '<span class="entry-content">'+ tweet.text +'</span><!--/.entry-content-->'
		tweet_html += '<span class="meta entry-meta">'
		tweet_html += '<a rel="bookmark" class="entry-date" href="http://twitter.com/'+user+'/status/'+ tweet.id +'"> <span title="'+ tweet.created_at +'" class="published">#</span></a>';
	...
	search_group:function( query, callback )
	{
	...
	{
		var search_url = query.indexOf('http://search.twitter.com/search') == 0 ? query : 'http://search.twitter.com/search.json?q=' + encodeURIComponent(query)
		
		if( query.indexOf('tnt-group:') > -1 ) tnt_twitter.search_group(query, callback);
			...
			GM_xmlhttpRequest({
				...
				onload: function(resp) {
					...
					$.each(js_tweets,function(idx,js_tweet){
						...
						tnt_twitter.tweet_get('http://twitter.com/'+js_tweet.from_user+'/status/'+js_tweet.id,function(tweet){
							tweets[tweet.id] = tweet;
	...
	cache_ajax:function(options,callback)
	{
		if( tnt_twitter.ajax_cache[options.url] )
		{
			callback(tnt_twitter.ajax_cache[options.url]);
		}
		else
		{
			options.onload = function(resp){
				tnt_twitter.save('ajax_cache',options.url,resp,function(){
					tnt_twitter.cache_ajax(options,callback);
				});
			}
			window.setTimeout(function(){
