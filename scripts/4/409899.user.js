...
// @name           Land Walker 2.0 (LEFT)
// @namespace      v1: TehNoob v2: Avalon
// @description    AutowaLk to X Y coords, includes intregrated Teleport Home button
...
addGlobalJS('function landWalkerScript(x,y){ var atimer_check = document.getElementById("actiontimer").innerHTML; if(atimer_check != "0.0") { walkTimeout = setTimeout(function(){landWalkerScript(x,y);}, 2); return; } else { var currLoc = document.getElementById("loccoords").innerHTML; currLoc = currLoc.split(","); var currLocX = currLoc[0]; var currLocY = currLoc[1]; if (x == "") { return; } if (y == "") { return; } if (currLocX != x) { var Xtest = parseFloat(x-currLocX); if (Xtest < 0) { go(4); walkTimeout = setTimeout(function(){landWalkerScript(x,y);}, 2); return; } else if (Xtest > 0) { go(3); walkTimeout = setTimeout(function(){landWalkerScript(x,y);}, 2); return; } } if (currLocY != y) { var Ytest = parseFloat(y-currLocY); if (Ytest < 0) { go(2); walkTimeout = setTimeout(function(){landWalkerScript(x,y);}, 2); return; } else if (Ytest > 0) { go(1); walkTimeout = setTimeout(function(){landWalkerScript(x,y);}, 2); return; } } } }');
