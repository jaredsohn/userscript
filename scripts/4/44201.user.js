// ==UserScript==
// @name		Flickr You Rock! admin tool
// @namespace		http://www.flickr.com/groups/you_rock/
// @description		An admin tool for the Flickr You Rock! group - V0.1
// @version		0.2
// @identifier	
// @date		03/09/2009
...
// @include		http://flickr.com/groups/you_rock/discuss/
// @include		http://www.flickr.com/groups_newtopic.gne?id=1035503@N21
// @include		http://*flickr.com/photos/*/*
// @include		http://*flickr.com//photos/*/*
// @include		http://www.flickr.com/groups/you_rock/discuss/*/edit/
// @exclude		http://www.flickr.com/groups/you_rock/discuss/*/lock/
//
// ==/UserScript==
// --------------------------------------------------------------------
//
...
(function you_rockadmintool( ) 
{
	var you_rockStartTime = new Date( );
	var you_rocktoolversion = "V0.1";
	var you_rocktoolDBversion = "V0.1";
	...
	var val = [ '',															//0
		...
		'<a href="http://www.flickr.com/groups/you_rock/">'
		+ '<img src="http://farm4.static.flickr.com/3314/3295565713_bdf12384c7_o.jpg" alt="1st Place" />'
		+ '</a>'
		+ '\n~~ Congratulations! ~~'
		+ '\nYou have beaten two other Medal Winners and earned: <a href="http://www.flickr.com/groups/you_rock/">'
		+ '<b>You Rock! Challenge Groups</b></a>!'
		+ '\nPlease post it to our pool.',											//1

		'<a href="http://www.flickr.com/groups/you_rock/">'
		+ '<img src="http://farm4.static.flickr.com/3423/3295579573_b2e2701980_o.jpg" alt="2nd Place" />'
		+ '</a>'
		+ '\n~~ Congratulations! ~~'
		+ '\nYour photo finished second in <a href="http://www.flickr.com/groups/you_rock/">'
		+ '<b>You Rock! Challenge Groups</b></a>!'
		+ '\nPlease post it to our pool.',											//2

		'<a href="http://www.flickr.com/groups/you_rock/">'
		+ '<img src="http://farm4.static.flickr.com/3062/3295625147_79b3b04e5e_o.jpg" alt="Unanimous" />'
		+ '</a>'
		+ '\n~~ Congratulations! ~~'
		+ '\nYour photo won unanumously in <a href="http://www.flickr.com/groups/you_rock/">'
		+ '<b>You Rock! Challenge Groups</b></a>!'
		+ '\nPlease post it to our pool.' ];											//3

	var node;
	var processforyou_rock;
	...
	function converttotime( elaps )
	{
		...
		var mytime = new Date( );
		var t = 0;
		you_rocktime = you_rockStartTime.getTime( );
		timevalue = elaps.split( " " )[ 0 ];

		if ( elaps.match( "moment" ) == "moment" ) t = you_rocktime;
		else if ( elaps.match( "second" ) == "second" ) t = you_rocktime - timevalue * 1000;
		else if ( elaps.match( "minute" ) == "minute" ) t = you_rocktime - ( timevalue - 0.5 ) * 1000 * 60;
		else if ( elaps.match( "hour" ) == "hour" ) t = you_rocktime - ( timevalue - 0.5 ) * 1000 * 60 * 60;
		else if ( elaps.match( "day" ) == "day" ) t = you_rocktime - ( timevalue - 0.5 ) * 1000 * 60 * 60 * 24;
		else if ( elaps.match( "week" ) == "week" ) t = you_rocktime - ( timevalue - 0.5 ) * 1000 * 60 * 60 * 24 * 7;
		else if ( elaps.match( "month" ) == "month" ) t = you_rocktime - ( timevalue - 0.5 ) * 1000 * 60 * 60 * 24 * 7 * 30;
	...
	Award = 
	{
		...
		{
			if (GM_getValue("you_rock.postedcomment")=="true")
			{
				//alert('in set tag');
				var tagtext = "";
				if ( GM_getValue( "you_rock.postedmedal" ) == "1" ) tagtext = "You Rock 1st Place"; //currently no tags set yet
				if ( GM_getValue( "you_rock.postedmedal" ) == "2" ) tagtext = "You Rock 2nd Place";
				if ( GM_getValue( "you_rock.postedmedal" ) == "3" ) tagtext = "You Rock Unanimous";
				...
				GM_setValue( "you_rock.postedcomment", "false" );
				return;
			}

			processforyou_rock = false;
			if ( ( document.referrer.match( "you_rock/discuss" ) == "you_rock/discuss" ) ||
				 ( document.referrer.match( "you_rock/discuss" ) == "you_rock/discuss" ) ||
				 ( GM_getValue( "you_rock.paginate" ) == "true" )	) 
			{
				processforyou_rock = true;
				
				GM_setValue( "you_rock.paginate", "false" ); // switch off paginateswitch
				...
				n.innerHTML = '<option value="0"></option>'
					+ '<option value="1">1st Place medal</option>'
					+ '<option value="2">2nd Place medal</option>'		
					+ '<option value="3">Unanimous medal</option>';
				...
				this.textarea.parentNode.insertBefore( document.createTextNode( 'you_rock Awards: ' ), this.textarea );
		...
		insertAward: function Awardinsert( )
		{
			...
			GM_setValue( "you_rock.postedmedal", node.value );
		},

		clickpostcomment: function Awardclickpostcomment( )
		{
			//alert( 'in postcomment' );       
			GM_setValue( "you_rock.postedcomment", "true" );
		},

		clickme: function Awardclickme( e ) 
		{
			//GM_log( "process: " + processforyou_rock );
			if ( processforyou_rock )
			{
				var targ;
				if ( !e ) 
				{
				...
				if ( tname == "A" )
				{
					...
					{
						GM_setValue( "you_rock.paginate", "true" );
	...
	function loadthemelist( )
	{
		...
		GM_setValue( "you_rock.themelisttime", thistime );

		GM_log( "you_rock: Started loading new themelist (in background)" );

		var url = "http://www.flickr.com/groups/786556@N23/discuss/72157605521012105/";
		...
		GM_xmlhttpRequest(
			{
				...
				onload:function( responseDetails ) 
				{
					...
					GM_setValue( "you_rock.themelist", themestring );
					challengethemeval = themestring.split( "||" );

					GM_log( "you_rock: Loading new themelist complete" );
				}
			} );
	}
	...
	function loadchlgheader( )
	{
		...
		GM_log( "you_rock: Started loading new challengeheaders (in background)" );

		var url = "http://www.flickr.com/groups/786556@N23/discuss/72157605747699890/";
		...
		onload:function( responseDetails ) 
		{
			var headernormal = responseDetails.responseText.split( "<b>Regular Challenge Header</b><br />" )[ 1 ];
			...
			GM_setValue( "you_rock.normalchlgheader", headernormal );

			var headermedal = responseDetails.responseText.split( "<b>Grandmother Challenge Header</b><br />" )[ 1 ];
			...
			GM_setValue( "you_rock.medalchlgheader", headermedal );

			var headerupgrade = responseDetails.responseText.split( "<b>Upgrade Challenge Header</b><br />" )[ 1 ];
			headerupgrade = headerupgrade.split( "<small>" )[ 0 ];			
			headerupgrade = headerupgrade.replace( "\n", "", "g" ).replace("\r","","g");
			headerupgrade = headerupgrade.replace( "<br />", "" );  //remove first <br />
			headerupgrade = headerupgrade.replace( "<br />", "\n", "g" );
			headerupgrade = headerupgrade.replace( "&lt;", "<", "g" );
			headerupgrade = headerupgrade.replace( "&gt;", ">", "g" );
			headerupgrade = headerupgrade.replace( "&amp;", "&", "g" );
			headerupgrade = headerupgrade.replace( "&quot;", '"', "g" );
			headerupgrade = headerupgrade.replace( "1796", sChallengeVoteScore, "g" );
			headerupgrade = headerupgrade.replace( "1784", sChallengeVoteScore, "g" );
			GM_setValue( "you_rock.upgradechlgheader", headerupgrade );

			var headerrace = responseDetails.responseText.split( "<b>Race for the Medals Header</b><br />" )[ 1 ];
			headerrace = headerrace.split( "<small>" )[ 0 ];			
			headerrace = headerrace.replace( "\n", "", "g" ).replace("\r","","g");
			headerrace = headerrace.replace( "<br />", "" );  //remove first <br />
			headerrace = headerrace.replace( "<br />", "\n", "g" );
			headerrace = headerrace.replace( "&lt;", "<", "g" );
			headerrace = headerrace.replace( "&gt;", ">", "g" );
			headerrace = headerrace.replace( "&amp;", "&", "g" );
			headerrace = headerrace.replace( "&quot;", '"', "g" );
			headerrace = headerrace.replace( "1796", sChallengeVoteScore, "g" );
			headerrace = headerrace.replace( "1784", sChallengeVoteScore, "g" );
			GM_setValue( "you_rock.racechlgheader", headerrace );

			GM_log( "you_rock: Loading new challengeheaders complete" );
	...
	function submittext( )
	{
		try
		{
			...
			for ( i = 0; i < foundforms.length; i++ )
			{
				...
				if ( foundforms[ i ].action.match( "/groups/you_rock/" ) == "/groups/you_rock/" ) 
				{
					myform = true;
					formnumber = i;
					break;
			...
			if ( myform )
			{
				...
				GM_setValue( "you_rock.bumping", "true" );
			}
			else
			{
				alert( "Can't find a reference to you_rock group, No input form available?" );
				GM_setValue( "you_rock.bumping", "false" );
			}
		}
		catch ( err )
		{
	...
	bumpmeup = function bumpmeup( ) 
	{
		...
		if ( GM_getValue( "you_rock.bumping" ) == "finished" )
		{
			GM_setValue( "you_rock.bumping", "false" );
		...
		{
			//alert( "phase2\n" + url );
			if ( GM_getValue( "you_rock.bumping" ) == "false" )
			{
		...
		else // on a page with delete in url
		{
			...
			else
			{
				...
				try
				{
					...
					{
						unsafeWindow.document.forms[ formnumber ].submit( );
						GM_setValue( "you_rock.bumping", "finished" );       
					}
					else
					{
	...
	{
		GM_setValue( "you_rock.bumpAllMethod", "All" );
		GM_setValue( "you_rock.bumpAllNr", 1 )
		GM_setValue( "you_rock.bumpAllState", "processing" ); 
		bumpall( );
	}

	initbumpopen = function initbumpopen( )
	{
		GM_setValue( "you_rock.bumpAllMethod", "Open" );
		GM_setValue( "you_rock.bumpAllNr", 1 )
		GM_setValue( "you_rock.bumpAllState", "processing" ); 
		bumpall( );
	}
	...
	{
		if ( GM_getValue( "you_rock.bumpAllState" ) == "processing" )
		{
			var iChallenge = parseInt( GM_getValue( "you_rock.bumpAllNr" ) );
	
			for ( ; iChallenge <= 12; iChallenge++ )
			{
				GM_setValue( "you_rock.bumpAllNr", "" + ( iChallenge + 1 ) );
				var sBase = "you_rock." + ( iChallenge < 10 ? "0" : "" ) + iChallenge;
				var sKey = sBase + ".threadnr";
				var sThreadNr = GM_getValue( sKey );
				var sStatus = GM_getValue( sBase + ".chlgstatus" );

				if ( ( GM_getValue( "you_rock.bumpAllMethod" ) == "All" && ( sStatus == "VOT" || sStatus == "OK" ) ) ||
				     ( GM_getValue( "you_rock.bumpAllMethod" ) == "Open" && decodeURIComponent( GM_getValue( sBase + ".status" ) ).match( " OPEN " ) == " OPEN " ) )
				{
			...
			var dailyChatURL = GM_getValue( "you_rock.DailyChatUrl" );
			if ( dailyChatURL != "" && GM_getValue( "you_rock.bumpAllMethod" ) == "All" )
			{
				GM_setValue( "you_rock.bumping", "true" );
				GM_setValue( "you_rock.DailyChatUrl", "" );
				thislocation.replace( dailyChatURL );
				return;
			}
		}

		GM_setValue( "you_rock.bumpAllState", "done" );		
		alert( "Done Bumping " + GM_getValue( "you_rock.bumpAllMethod" ) + "!" );
		runyou_rockTool( );
	}
...
GM_log( "checking: " + challenge );
		...
		mylist[ 0 ] = "you_rock.01";
		mylist[ 1 ] = "you_rock.02";
		mylist[ 2 ] = "you_rock.03";
		mylist[ 3 ] = "you_rock.04";
		mylist[ 4 ] = "you_rock.05";
		mylist[ 5 ] = "you_rock.06";
		mylist[ 6 ] = "you_rock.07";
		mylist[ 7 ] = "you_rock.08";
		mylist[ 8 ] = "you_rock.09";
		mylist[ 9 ] = "you_rock.10";
		mylist[ 10 ] = "you_rock.11";
	...
	bumpmenow = function bumpmenow( e ) 
	{
		...
		{
			GM_setValue( "you_rock.bumping", "start" );
		}
		location.href = url;
		return true;
	}
	...
	function displayplatinumdata( e )
	{
		...
		ddthreadnr = GM_getValue( you_rockchlgnr + ".threadnr" );
		ddchlgnr = GM_getValue( you_rockchlgnr + ".challengenr" );
		ddcommentcounter = GM_getValue( you_rockchlgnr + ".commentcounter" );
		ddstatus = decodeURIComponent( GM_getValue( you_rockchlgnr + ".status" ) );
		ddphoto1 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo1" ) );
		ddphotovoter = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photovoter" ) );
...
GM_log( "Here - displaydata" );
		...
		votealert = checkvotes( you_rockchlgnr, true );

		ddthreadnr = GM_getValue( you_rockchlgnr + ".threadnr" );
		ddchlgnr = GM_getValue( you_rockchlgnr + ".challengenr" );
		ddcommentcounter = GM_getValue( you_rockchlgnr + ".commentcounter" );
		ddstatus = decodeURIComponent( GM_getValue( you_rockchlgnr + ".status" ) );
		ddwinner = decodeURIComponent( GM_getValue( you_rockchlgnr + ".winner" ) );
		ddphoto1 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo1" ) );
		ddphoto2 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo2" ) );
		ddphoto3 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo3" ) );
		ddphotovoter = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photovoter" ) );
		ddlastvote = GM_getValue( you_rockchlgnr + ".lastvote");
		ddchlgstatus = GM_getValue( you_rockchlgnr + ".chlgstatus" );
		ddtovotetime = GM_getValue( you_rockchlgnr + ".tovotetime" );
		ddphoto1time = GM_getValue( you_rockchlgnr + ".photo1time" );
		ddphoto2time = GM_getValue( you_rockchlgnr + ".photo2time" );
		ddphoto3time = GM_getValue( you_rockchlgnr + ".photo3time" );
		...
		if ( true )
		{
			...
			{
				var challengenumber = parseInt( you_rockchlgnr.split( "." )[ 1 ] );
				...
				for ( iVote = 0; iVote < 3; iVote++ )
				{
					...
					{
						winner = "" + decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo" + ( iVote + 1 ) ) ) +
...
GM_log( "Here - displayfulldata" );
		...
		if ( GM_getValue( "you_rock.brackets" ) != "false" )
		{
	...
	updatedata = function updatedata( e )
	{
		...
		url=targ.id;
		GM_setValue( "you_rock.update", "true" );
	...
	{
		GM_setValue( "you_rock.updatealldata","true" );
		var updfound = false;

		maxupdates = GM_getValue( "you_rock.maxupdate" );
		maxupdates++;
		//GM_log( maxupdates );

		if ( maxupdates > 15 ) 
		{
			GM_setValue( "you_rock.maxupdate", 0 );
			GM_setValue( "you_rock.updatealldata", "false" );
			alert( "Stopped updating: possible loop due to double challenges" );
			return;
		}
		else
		{
			GM_setValue( "you_rock.maxupdate", maxupdates );
		}
		...
		for ( i = 1; i < discusstrs.length; i++ )
		{
			...
			var you_rockchlgnr = "you_rock." + mnchlgnr;
		...
		//alert( GM_getValue( "you_rock.updatealldata" ) );
	...
	{
		GM_log( "you_rock: init data started" );
		...
		GM_setValue( "you_rock.updatealldata", "false" );
		GM_setValue( "you_rock.update", "false" );
		GM_setValue( "you_rock.maxupdate", 0 );
		GM_setValue( "you_rock.bumping", "false" );
		GM_setValue( "you_rock.bumpAllState", "none" );
		GM_setValue( "you_rock.version", you_rocktoolversion );
		GM_setValue( "you_rock.brackets", "false" );
		GM_setValue( "you_rock.lastwinner", encodeURIComponent( " " ) );
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
		GM_setValue( "you_rock.lastsecond", encodeURIComponent( " " ) );
		GM_setValue( "you_rock.lastsecond2", encodeURIComponent( " " ) );
		GM_setValue( "you_rock.lastmedalsplayers", encodeURIComponent( " " ) );
		GM_setValue( "you_rock.lastchallenge", "" );
		GM_setValue( "you_rock.currentchallenge", "" );
		GM_setValue( "you_rock.themelist", "" );
		GM_setValue( "you_rock.themelisttime", "" );
		GM_setValue( "you_rock.normalchlgheader", "Error uploading header from the back room, please use copy paste to update." );
		GM_setValue( "you_rock.medalchlgheader", "Error uploading header from the back room, please use copy paste to update." );
		GM_setValue( "you_rock.upgradechlgheader", "Error uploading header from the back room, please use copy paste to update." );
		GM_setValue( "you_rock.racechlgheader", "Error uploading header from the back room, please use copy paste to update." );
		...
		for ( x in mylist )
		{
			...
			GM_setValue( mylist[ x ] + ".status", encodeURIComponent( mylist[ x ].split( "you_rock." )[ 1 ] + " - not found or not updated with new info yet" ) );
		...
		GM_log( "you_rock: init data complete" );
	...
	addyou_rockheader = function addyou_rockheader( )
	{
		...
		var you_rockEndTime = new Date( );
		var you_rockExecutionTime = you_rockEndTime - you_rockStartTime;

		if ( unsafeWindow.global_name == 'KVangeel' || unsafeWindow.global_name == 'charminbayurr' )
			tds[ 1 ].innerHTML = "you_rock Admin tool " + you_rocktoolversion + " (" + you_rockExecutionTime + "ms)" + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + tds[ 1 ].innerHTML;
		else
			tds[ 1 ].innerHTML = "you_rock Admin tool " + you_rocktoolversion + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + tds[1].innerHTML;
	}
	...
	function ProcessMainDoc( )
	{
		...
		for ( i = 1; i < trs.length; i++ )
		{
			...
			if ( mnstatus.match( "Daily Chat" ) == "Daily Chat" )
			{
				...
				GM_setValue( "you_rock.DailyChatUrl", url );
			}

			if ( mnchlgnr.length == 2 ) //only process if chlgnr is correct format
			{
				you_rockchlgnr = "you_rock." + mnchlgnr;
				...
				ddphoto2 	= decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo2" ) );
				ddphoto3 	= decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo3" ) );
				ddphoto1time 	= decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo1time" ) );
				ddphoto2time 	= decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo2time" ) );
				ddphoto3time 	= decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo3time" ) );
				ddlastvote 	= GM_getValue( you_rockchlgnr + ".lastvote" );
				ddthreadnr 	= GM_getValue( you_rockchlgnr + ".threadnr" );

				var votetime = decodeURIComponent( GM_getValue( you_rockchlgnr + ".tovotetimesince" ) );
				var phototime1 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo1timesince" ) );
				var phototime2 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo2timesince" ) );
				var phototime3 = decodeURIComponent( GM_getValue( you_rockchlgnr + ".photo3timesince" ) );
				...
				if ( phototime3.length < 2 )
				{
					if ( phototime2.length < 2 )
					{
						...
						{
							challengeopendate = decodeURIComponent( GM_getValue( you_rockchlgnr + ".startdate" ) );
							hours = ( you_rockStartTime.getTime( ) - challengeopendate ) / ( 1000 * 60 * 60 );
				...
				GM_setValue( you_rockchlgnr + ".idleAlert", sAlert );
				...
				if ( chlgstatus == "VOT" )  
				{
					if ( mnstatus.match( "VOTE" ) == "VOTE" ) //VOTE found, so update needed
					{
						...
						GM_setValue( you_rockchlgnr + ".status", encodeURIComponent( mnstatus ) ); // write new full status
					}
				} 
				...
				if ( chlgstatus != "---" ) 
				{
					GM_setValue ( you_rockchlgnr + ".chlgstatus", chlgstatus);
		...
		update = GM_getValue( "you_rock.updatealldata" );
		if ( update.match( "true" ) == "true" )
			updatealldata( );
		else 
			GM_setValue( "you_rock.maxupdate", 0 );

		// add you_rock info & update headers
		var trs = tables[ 0 ].getElementsByTagName( "tr" );
		var tds = trs[ 0 ].getElementsByTagName( "td" );
		...
		myanchor = thisdocument.createElement( 'a' );
		myanchor.innerHTML = "you_rock-Info";
		myanchor.href = "#";
		myanchor.setAttribute( 'onClick', 'return false;' );
		...
		if ( thislocation.href.match( "you_rock/discuss/page" ) != "you_rock/discuss/page" )
		{
			var activethemes = "";
			for ( i = 1; i < 13; i++ )
			{
				...
				{
					GM_setValue( "you_rock." + chkchlg + ".status", encodeURIComponent( chkchlg + " - not found or not updated with new info yet" ) );
					GM_setValue( "you_rock." + chkchlg + ".winner", encodeURIComponent( " " ) );
					GM_setValue( "you_rock." + chkchlg + ".photo1", encodeURIComponent( " " ) );
					GM_setValue( "you_rock." + chkchlg + ".photo2", encodeURIComponent( " " ) );
					GM_setValue( "you_rock." + chkchlg + ".photo3", encodeURIComponent( " " ) );
					GM_setValue( "you_rock." + chkchlg + ".photovoter", encodeURIComponent( " " ) );
					GM_setValue( "you_rock." + chkchlg + ".commentcounter", "100" );
					GM_setValue( "you_rock." + chkchlg + ".lastvote", " " );
					GM_setValue( "you_rock." + chkchlg + ".chlgstatus", "---" );
					GM_setValue( "you_rock." + chkchlg + ".tovotetime", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo1time", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo2time", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo3time", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo1timesince", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo2timesince", " " );
					GM_setValue( "you_rock." + chkchlg + ".photo3timesince", " " );
				}
				else
				{
					activethemes = activethemes + decodeURIComponent( GM_getValue( "you_rock." + chkchlg + ".status" ) ).split( 'Theme: ' )[ 1 ] + "||";
				} 
			}

			GM_setValue( "you_rock.activethemes", encodeURIComponent( activethemes ) );
		}
...
GM_log( "Here - ProcessDetailDoc" );
		...
		{
			//add you_rock bump header & leave
			var place = thisdocument.getElementById( "Tertiary" );

			//add you_rock Bump
			place.appendChild( thisdocument.createTextNode(' / ') );
			myanchor=thisdocument.createElement( 'a' );
			myanchor.innerHTML = "you_rock-Bump";
			myanchor.href = "#";
		...
		you_rockchlgnr = "you_rock." + chlgnr;

		//alert( you_rockchlgnr );

		GM_setValue( you_rockchlgnr + ".startdate", encodeURIComponent( challengeopendate ) );
		GM_setValue( you_rockchlgnr + ".threadnr", threadnr );
		GM_setValue( you_rockchlgnr + ".challengenr", chlgnr );
		GM_setValue( you_rockchlgnr + ".commentcounter", commentcounter );
		GM_setValue( you_rockchlgnr + ".status", encodeURIComponent( status ) );
		GM_setValue( you_rockchlgnr + ".winner", encodeURIComponent( winner ) );
		GM_setValue( you_rockchlgnr + ".photo1", encodeURIComponent( photoposter[ 0 ] ) );
		GM_setValue( you_rockchlgnr + ".photo2", encodeURIComponent( photoposter[ 1 ] ) );
		GM_setValue( you_rockchlgnr + ".photo3", encodeURIComponent( photoposter[ 2 ] ) );
		GM_setValue( you_rockchlgnr + ".photovoter", encodeURIComponent( photovoter ) );

		// inside challenge - decode last vote if no spaces
		lastvote = detectNoSpaces( lastvote );

		GM_setValue( you_rockchlgnr + ".lastvote", lastvote );
		GM_setValue( you_rockchlgnr + ".chlgstatus", "OK" );
		GM_setValue( "you_rock.currentchallenge", chlgnr );

		GM_setValue( you_rockchlgnr + ".tovotetime", converttotime( votetime ) );
		GM_setValue( you_rockchlgnr + ".photo1time", converttotime( phototime[ 0 ] ) );
		GM_setValue( you_rockchlgnr + ".photo2time", converttotime( phototime[ 1 ] ) );
		GM_setValue( you_rockchlgnr + ".photo3time", converttotime( phototime[ 2 ] ) );

		GM_setValue( you_rockchlgnr + ".tovotetimesince", encodeURIComponent( votetime ) );
		GM_setValue( you_rockchlgnr + ".photo1timesince", encodeURIComponent( phototime[ 0 ] ) );
		GM_setValue( you_rockchlgnr + ".photo2timesince", encodeURIComponent( phototime[ 1 ] ) );
		GM_setValue( you_rockchlgnr + ".photo3timesince", encodeURIComponent( phototime[ 2 ] ) );

		update = GM_getValue( "you_rock.update" );

		if ( update.match( "true" ) == "true" )
		{
			GM_setValue( "you_rock.update", "false" );
			thislocation.replace( "http://www.flickr.com/groups/you_rock/discuss/" );
			return;
		}
...
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
			...
			if ( status.toUpperCase( ).match( "ROCK ON" )  == "ROCK ON" )
				GM_setValue( "you_rock.lastmedalsplayers", encodeURIComponent( photoposter[ 0 ] + ' / ' + photoposter[ 1 ] + ' / ' + photoposter[ 2 ] ) );
			else
				GM_setValue( "you_rock.lastmedalsplayers", encodeURIComponent( " " ) );

			GM_setValue( "you_rock.lastchallenge", chlgnr );
			...
			if ( ( ( vote1 == sChallengeVoteScore ) && ( !bRockOn && !bUpgrade ) ) || ( ( vote1 == sUpgradeChallengeVoteScore ) && ( bUpgrade ) ) || ( ( vote1 == sRockOnChallengeVoteScore ) && ( bRockOn ) ) ) 
			{
				...
				GM_setValue( "you_rock.lastwinner", encodeURIComponent( photoposter[ 0 ] ) );
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
			}
			else if ( ( vote1 == "" + ( iChallengeVoteScore - 1 ) ) && !bUpgrade && !bRockOn ) 
			{
				document.getElementById( "C12" ).checked = true;
				GM_setValue( "you_rock.lastsecond", encodeURIComponent( photoposter [ 0 ] ) );
				sec1check = "set";
			}
			...
			if ( ( ( vote2 == sChallengeVoteScore ) && ( !bRockOn && !bUpgrade ) ) || ( ( vote2 == sUpgradeChallengeVoteScore ) && ( bUpgrade ) ) || ( ( vote2 == sRockOnChallengeVoteScore ) && ( bRockOn ) ) ) 
			{
				...
				GM_setValue( "you_rock.lastwinner", encodeURIComponent( photoposter[ 1 ] ) );
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
			}
			else if ( ( vote2 == "" + ( iChallengeVoteScore - 1 ) ) && !bUpgrade && !bRockOn ) 
			{
				...
				if ( sec1check == "" ) 
				{
					GM_setValue( "you_rock.lastsecond", encodeURIComponent( photoposter[ 1 ] ) );
					sec1check = "set";
				}
				else 
					GM_setValue( "you_rock.lastsecond2", encodeURIComponent( photoposter[ 1 ] ) );
			}
			...
			if ( ( ( vote3 == sChallengeVoteScore ) && ( !bRockOn && !bUpgrade ) ) || ( ( vote3 == sUpgradeChallengeVoteScore ) && ( bUpgrade ) ) || ( ( vote3 == sRockOnChallengeVoteScore ) && ( bRockOn ) ) ) 
			{
				...
				GM_setValue( "you_rock.lastwinner", encodeURIComponent( photoposter[ 2 ] ) );
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
			}
			else if ( ( vote3 == "" + ( iChallengeVoteScore - 1 ) ) && !bUpgrade && !bRockOn ) 
			{
				...
				if ( sec1check == "" )
					GM_setValue( "you_rock.lastsecond", encodeURIComponent( photoposter[ 2 ] ) )
				else
					GM_setValue( "you_rock.lastsecond2", encodeURIComponent( photoposter[ 2 ] ) )
			}
...
GM_log( "Here - ProcessDetailDocInline" );
	...
	entermedal = function entermedal( ) //gets called when tickboxes are changed
	{
		...
		GM_setValue( "you_rock.lastwinner",encodeURIComponent( winner ) );
		GM_setValue( "you_rock.lastsecond",encodeURIComponent( second ) );
		GM_setValue( "you_rock.lastsecond2",encodeURIComponent( second2 ) );
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
	...
	function Editchallenge( ) //gets called after a page for editing a challenge is opened
	{
		...
		lastwinner = decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) );
		lastsecond = decodeURIComponent( GM_getValue( "you_rock.lastsecond" ) );
		lastsecond2 = decodeURIComponent( GM_getValue( "you_rock.lastsecond2" ) );
		currentchlgnr = GM_getValue( "you_rock.currentchallenge" );
		...
		if ( decodeURIComponent( GM_getValue( "you_rock." + currentchlgnr + ".photo3" ) ) != " " )
	...
	Editchallengeinline = function Editchallengeinline( link,e ) //initially taken from inline forum edit post by Steeev
	{
		...
		you_rockmotm = false;
		if ( link.getAttribute( 'href' ).match( "you_rockmotm" ) == "you_rockmotm" )
			you_rockmotm = true;
		...
		if ( mode == 'firstpost' )
		{
			...
			if ( ( titlearea.match( "TO VOTE" ) == "TO VOTE" ) && ( lastwinner != " " ) && !you_rockmotm )
			{
				newtxt = "AWARDED (" + lastwinner;
			...
			if ( decodeURIComponent( GM_getValue( "you_rock." + currentchlgnr + ".photo3" ) ) != " " )
				if ( ( titlearea.match( "OPEN" ) == "OPEN" ) && !you_rockmotm )
					titlearea = titlearea.replace( "OPEN", "TO VOTE" );

			topic_title = titlearea;
			//you_rock add ons till here
	...
	Editchallengepostitbaby = function Editchallengepostitbaby( node, editlink )
	{
		...
		{
			GM_setValue( "you_rock." + decodeURIComponent( subject ).split( " " )[ 0 ] + ".tovotetime", converttotime( "a moment" ) );

			//GM_setValue( you_rockchlgnr + ".tovotetime", converttotime( "Posted 1 second ago." ) );
		}
	...
	insertChallengetxt = function insertChallengetxt( )
	{
		...
		challengetxtval[ 1 ] = GM_getValue( "you_rock.normalchlgheader" );
		challengetxtval[ 2 ] = GM_getValue( "you_rock.medalchlgheader" );

		//check if variables are filled in + alert
		if ( decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) == " " )
		{
			if ( GM_getValue( "you_rock.lastchallenge" ) == "" )
		...
		if ( challengetxtnode.value == "3" )
			...
			this.titlearea.value = GM_getValue( "you_rock.lastchallenge" ) + " - OPEN - Theme: " + t.options[ t.selectedIndex ].text;

		//replace winner field
		if ( decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) != " " && challengetxtnode.value == 1 ) 
		{
			this.textarea.value = this.textarea.value.replace( "winner", decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
		}
		else if ( decodeURIComponent( GM_getValue( "you_rock.lastmedalsplayers" ) ) != " " )
			this.textarea.value = this.textarea.value.replace( "Challenger 1 / Challenger 2 / Challenger 3", decodeURIComponent( GM_getValue( "you_rock.lastmedalsplayers" ) ) );
	}
	...
	insertChallengetheme = function insertChallengetheme( )
	{
		...
		if ( challengetxtnode.value == "1" )
		{
			...
			{
				this.textarea.value = GM_getValue( "you_rock.racechlgheader" );
			}
			else if ( sThemeString.match( "Upgrade Challenge" ) == "Upgrade Challenge" )
			{
				this.textarea.value = GM_getValue( "you_rock.upgradechlgheader" );
			}
			else
			{
				this.textarea.value = GM_getValue( "you_rock.normalchlgheader" );
			}

			// replace winner
			this.textarea.value = this.textarea.value.replace( "Previous Winner", decodeURIComponent( GM_getValue( "you_rock.lastwinner" ) ) );
		}
	}
	...
	Createnewchallenge = function Createnewchallenge( )
	{
		...
		//myWindow.document.open( "text/html", "replace" );
		//myWindow.document.write( "<P><h1>you_rock Admin tool log</h1></p>" );

		themelist = GM_getValue( "you_rock.themelist" );
		if ( themelist != "" )
		...
		activethemes = decodeURIComponent( GM_getValue( "you_rock.activethemes" ) );
		if ( activethemes != "" )
		...
		lastchallenge = GM_getValue( "you_rock.lastchallenge" );
		...
		this.textarea.parentNode.insertBefore( document.createElement( "br" ), this.textarea );
		this.textarea.parentNode.insertBefore( document.createTextNode( 'you_rock challenge type: ' ), this.textarea );
		this.textarea.parentNode.insertBefore( n, this.textarea );
		this.textarea.parentNode.insertBefore( document.createElement( "br" ), this.textarea );
		this.textarea.parentNode.insertBefore( document.createElement( "br" ), this.textarea );
		...
		this.titlearea.parentNode.insertBefore( document.createTextNode( 'you_rock theme: ' ), this.titlearea );
	...
	function ProcessPlatinumDetailDoc( )
	{
		...
		if ( status.match( "Category" ) == "Category" ) //only process brackets
		{
			...
			GM_setValue( you_rockchlgnr + ".commentcounter", commentcounter );
			GM_setValue( you_rockchlgnr + ".status", encodeURIComponent( status ) );
			GM_setValue( you_rockchlgnr + ".photo1", encodeURIComponent( photoposter ) );
			GM_setValue( you_rockchlgnr + ".photovoter", encodeURIComponent( photovoter ) );
			GM_setValue( "you_rock.currentchallenge", chlgnr );
	...
	runyou_rockTool = function runyou_rockTool( )
	{
		// check if we have GM variables
		if ( GM_getValue( "you_rock.version" ) == undefined )
			initdata( );
	
		if ( GM_getValue( "you_rock.version" ) != you_rocktoolDBversion )
			initdata( );

		// check themelist & chlgheaders

		themelisttime = GM_getValue( "you_rock.themelisttime" );
		...
		themelisttime = GM_getValue( "you_rock.themelisttime" );
		elapstime = you_rockStartTime.getTime( ) - themelisttime;
		...
		if (GM_getValue("you_rock.brackets")!="false")
		{
			GM_setValue("you_rock.brackets","false"); //reset brackets status when we start
		}
		...
		else if ( thislocation.href.match( "/groups/you_rockplatinum/" ) == "/groups/you_rockplatinum/" )
		{
			// in you_rock platinum for brackets qualifying.
		...
		else if ( GM_getValue( "you_rock.bumping" ) != "false" )
		{
			bumpmeup( ); //if in bumping mode, go to bump processing
		}
		else if ( GM_getValue( "you_rock.bumpAllState" ) == "processing" )
		{
			bumpall( );
			return;
		}
		...
		{
			// ************************
			// main photo you_rock page
			// ************************
			//alert( "in main" );
			if ( GM_getValue( "you_rock.bumping" ) != "false" )
			{
				GM_setValue( "you_rock.bumping", "false" ); //reset bumping status if we're in discuss
			}
			GM_setValue( "you_rock.postedcomment", "false" );
		...
		GM_registerMenuCommand( "Initialise you_rock Admin data (themes, headers, ...)", initdatafrommenu );
	}
