// ==UserScript==
// @name           [PESN] Script - PES.Nyu
// @namespace      pesn
// @description    Script Greasemonkey pour la communauté PES
...
function MAJ() {
	...
	switch (maj) {
		case 0 : {
			var test = confirm("Utiliser la coloration des pseudonymes par défaut ?");
			if (test) {
				var mod = 1;
			} else {
				var mod = 0;
			}
			var test = confirm("Rappel du pseudonyme effectivement utilisé lors de l'affichage d'un groupement ?");
			if (test) {
				mod += 4;
			} else {
				mod += 2;
...
function delpseudo(pseudo) {
	...
	str = "µ" + str + "µ";
	liste = liste.replace(new RegExp(str,"gi"),"µ");
	var str = "";
...
function imgadd(initial,terminal) {
	...
	if (img != "") {
		...
		if (terminal == initial && test) {
			img = "¤"+img.replace(/\|/g,"¤")+"¤";
			img = img.replace(new RegExp("¤"+initial+"¤","gi"),"¤");
			img = img.replace(/¤/g,"|").substring(1,img.length-1);
		}
	} else {
		img = initial;
	}
...
function ListeAlternanceModo() {
	...
	addmodo.title = "Ajouter modération";
	...
	rmvmodo.title = "Retirer modération";
...
function addmod() {
	...
	if (forum != null) {
		var pseudo = null;
		while (pseudo == null) {
			pseudo = prompt("Nom du modérateur : ","");
		}
		var mail = null;
		while (mail == null) {
		...
		var mdp = null;
		while (mdp == null) {
			mdp = prompt("Mot de passe modérateur : ","");
		}
...
function moderationmanual() {
	if (document.getElementById('liste_modos').selectedIndex == 0) {
		var test = confirm("Ajouter ce groupe de modération");
		if (test) {
...
function addlink() {
	var lien = prompt("Quel est le motif à censurer ?","");
	if (lien != null && lien != "") {
		pradd(lien);
		Reload();
	}
...
function dellink() {
	var liste = GM_getValue("Protection","");
	liste = "¤"+liste.split("|").join("¤")+"¤";
	liste = liste.replace(new RegExp("¤"+this.name+"¤","gi"),"¤");
...
function rmvlink() {
	var lien = prompt("Quel est le motif à décensurer ?","");
	if (lien != null && lien != "") {
		var liste = GM_getValue("Protection","");
		var reg = new RegExp("(" + liste + ")","gi");
		...
		if (test) {
			...
			liste = liste.replace("¤" + motif + "¤","¤");
			GM_setValue("Protection",liste.replace(/¤/g,"|").substring(1,liste.length-1));
			Reload();
		}
	}
}
...
function delimg() {
	...
	var liste = "¤"+GM_getValue("img_liste","").split("|").join("¤")+"¤";
	liste = liste.replace(new RegExp("¤"+img+"¤","gi"),"¤");
	liste = liste.substring(1,liste.length-1);
	liste = liste.split("¤").join("|");
...
function AccesPC(mode) {
	if (mode>=-1) {
		...
		statlink.appendChild(document.createTextNode("PES.Nyu"));
		var stat = document.createElement('li');
		stat.appendChild(statlink);
		menu.appendChild(stat);
	}
...
function Citation(mess) {
	...
	if (pseudo != -1 && post != -1 && date != -1 && ancre != -1) {
		...
		citation += "| Posté le " + new RegExp("[0-9]{1,2}[er]* [a-zA-Zéû]* [0-9]{4} à [0-9]{2}:[0-9]{2}:[0-9]{2}").exec(element[date].innerHTML) + "\n";
		var str = element[post].innerHTML;
...
function ChangeDate(a,dif) {
	var ori = new RegExp("([0-9]{1,2})[er]* ([a-zA-Zéû]+) ([0-9]{4}) à ([0-9]{2}):([0-9]{2}):([0-9]{2})")
	...
	var str = "Posté le "+IntToStr(dat.getDate(),2)+" "+month[dat.getMonth()]+" "+dat.getFullYear()+" à "+IntToStr(dat.getHours(),2)+":"+IntToStr(dat.getMinutes(),2)+":"+IntToStr(dat.getSeconds(),2);
...
function AjoutBoutonCensure(a,messid,pseudo,modo,moderateur) {
	...
	if (GM_getValue("jvc_masque",true)) {
		...
		newImg.name = "<I><small>(Ce message a été censuré)</small></I>";
	...
	if (!modo && (((censure && !(modo)) && !cache) || (!(censure && !(modo)) && cache))) {
		if (exclusion) {
			...
			var dat = new RegExp("[0-9]{1,2}[er]* [a-zA-Zéû]* [0-9]{4} à [0-9]{2}:[0-9]{2}:[0-9]{2}").exec(document.getElementById(messid).innerHTML);
			dat = dat[0];
			...
			newImg.title = "Réafficher le message p" + dat.substring(1,dat.length);
			newImg.alt = newImg.title;
...
function AjoutBoutonModération(a,messid,pseudo,modo) {
	var date = document.getElementById(messid).innerHTML;
	date = new RegExp("[0-9]{1,2}[er]* [a-zA-Zéû]* [0-9]{4} à [0-9]{2}:[0-9]{2}:[0-9]{2}").exec(date);
	var newImg = document.createElement("img");
	newImg.style.cursor = "pointer";
	newImg.src = "http://www.noelshack.com/uploads/bt_forum_avertirmod020241.gif";
	newImg.id = "Rapport_modération_" + messid;
	...
	a.insertBefore(document.createTextNode(" : rapport de modération "),a.firstChild);;
...
function RapportModeration () {
	...
	if (rep) {
		...
		var peine = prompt("Peine demandée : ","Bannissement définitif");
		if (motif != null && motif != "") {
			alerte += "Motif : "+motif+"\n";
			minialerte += " : " + motif;
		}
		if (peine != null && peine != "") {
			alerte+="Peine demandée : "+peine+"\n";
		}
...
function Coloration() {
	...
	var newclr = prompt("Quelle couleur donner ? (code RGB hexadécimal XXXXXX )",clr);
...
function GererMode(Nom,pseudo) {
	...
	newImg.title = "Changer le mode d'écriture";
	newImg.name = pseudo;
	newImg.alt = newImg.title;
	Nom.appendChild(document.createTextNode(" "));
	Nom.appendChild(newImg);
...
function RenommageGroupement() {
	var pseudo = this.name;
	var groupe = GM_getValue("gp#"+pseudo.toLowerCase()+"#groupe",pseudo);
	var newname = prompt("Renommer tous les pseudonymes associés à ce groupement en ?",groupe);
...
function ModeEcriture() {
	var pseudo = this.name;
	var mode = prompt("Quel mode d'écriture ?",GM_getValue("gp#"+pseudo.toLowerCase()+"#mode",0));
...
function ass() {
	...
	var newgroupe = prompt("Associer le pseudonyme "+pseudo+" à quel groupement ?",groupe);
...
function Preferences(position) {
	...
	a.appendChild(document.createTextNode("Supprimer préférence"));
	a.id = "Dest_Pref";
	...
	p.className="lien_base";
	var a = document.createElement('a');
	a.appendChild(document.createTextNode("Ajouter préférence"));
	a.id = "Ajout_Pref";
	a.style.cursor = "pointer";
	p.appendChild(a);
	document.getElementById("liste_forums").parentNode.insertBefore(p,document.getElementById("liste_forums").nextSibling);
...
function AjouterLienPref(nom,ref,i) {
	...
	img.title = "Retirer préférence";
	img.style.cursor = "pointer";
	img.style.display = "none";
	img.alt = img.title;
	img.id = "Préférence_"+i;
	img.name = i;
	a.href = ref;
...
function montrerDestructLiensPref() {
	...
	if (img.length>0) {
		...
		if (img[0].style.display == disp) {
			...
			this.replaceChild(document.createTextNode("Supprimer préférence"),this.firstChild);
		}
...
function addPreference() {
	var ref = prompt("URL du lien à rajouter dans les préférences",self.location.href);
	var nom = prompt("Nom du lien à rajouter dans les préférences",ref);
...
function DeletePreference() {
	...
	document.getElementById("Préférence_"+i).parentNode.parentNode.removeChild(document.getElementById("Préférence_"+i).parentNode);
	...
	for (var j=(i+1);j<n;j++) {
		...
		document.getElementById("Préférence_"+j).name=(j-1);
		document.getElementById("Préférence_"+j).id="Préférence_"+(j-1);
...
function SmileyInCDV() {
	...
	"images smileys"; {
		var smiley = new Array (
			...
			'<img src="http://www.noelshack.com/uploads/16062009/Kira032574.gif" alt=":kira:">',
			'<img src="http://www.noelshack.com/uploads/16062009/Demon055139.gif" alt=":demon:">',
			'<img src="http://www.noelshack.com/uploads/16062009/HapMonosourcil095433.gif" alt=":monosourcil:">',
			'<img src="http://www.noelshack.com/uploads/16062009/Coange027983.gif" alt=":coange:">',
			'<img src="http://www.noelshack.com/uploads/16062009/HapMalade017807.gif" alt=":malhap:">',
			'<img src="http://www.noelshack.com/uploads/16062009/Mort2032352.gif" alt=":mort2:">',
			'<img src="http://www.noelshack.com/uploads/16062009/Snif3084718.gif" alt=":snif3:">',
			'<img src="http://www.noelshack.com/uploads/16062009/-_-099920.gif" alt=":-_-:">'
...
function RapportComplet() {
	var test = confirm("Rédiger le rapport complet ?");
	if (test) {
...
function RapportPartiel() {
	var test = confirm("Rédiger le rapport partiel ?");
	if (test) {
...
function GestionMasques() {
	...
	if (pic == "none") {
		piclink.appendChild(document.createTextNode("Désactiver masque images"));
	} else {
	...
	if (vid == "none") {
		vidlink.appendChild(document.createTextNode("Désactiver masque vidéos"));
	} else {
		vidlink.appendChild(document.createTextNode("Activer masque vidéos"));
	}
...
function MasquePic() {
	...
	if (pic != "none") {
		...
		document.getElementById("masque_pic").appendChild(document.createTextNode("Désactiver masque images"));
	} else {
		GM_setValue("msq#pic","");
...
function MasqueVid() {
	...
	if (vid != "none") {
		...
		document.getElementById("masque_vid").appendChild(document.createTextNode("Désactiver masque vidéos"));
	} else {
		GM_setValue("msq#vid","");
		document.getElementById("masque_vid").appendChild(document.createTextNode("Activer masque vidéos"));
	}
	Reload();
}
...
function RecupDonnees() {
	...
	if (GM_getValue("jvc_debug_post",false)) {
		...
		newImg.title = "Récupérer les données de debug";
		newImg.alt = newImg.title;
		prebloc.parentNode.insertBefore(document.createTextNode("				"),prebloc);
		prebloc.parentNode.insertBefore(newImg,prebloc);
	...
	if (GM_getValue("jvc_lecture",false)) {
		...
		newImg.title = "Récupérer les données contenue dans le script";
		newImg.alt = newImg.title;
		prebloc.parentNode.insertBefore(document.createTextNode("				"),prebloc);
		prebloc.parentNode.insertBefore(newImg,prebloc);
	...
	if (GM_getValue("jvc_ecriture",false)) {
		...
		newImg.title = "Ecrire les données dans le script";
		newImg.alt = newImg.title;
		prebloc.parentNode.insertBefore(document.createTextNode(" "),prebloc);
		prebloc.parentNode.insertBefore(newImg,prebloc);
...
function ecrdondbg() {
	var test = confirm("Récupérer les données de debugage ?");
	if (test) {
...
function ecrdonscr() {
	var test = confirm("Récupérer les données contenue dans le script ?");
	if (test) {
		var str = "";
		var n = GM_getValue("gp#prc_nombre",0);
		for (var i=0;i<n;i++) {
...
function lirdonscr() {
	var test = confirm("Lire les données et les écrire dans le script ?");
	if (test) {
...
function ArmeSelect() {
	...
	statlink.style.cursor = "pointer";
	statlink.appendChild(document.createTextNode("Tout sélectionner"));
	...
	statlink.style.cursor = "pointer";
	statlink.appendChild(document.createTextNode("Tout désélectionner"));
...
function TraitementMessage(a,i,param) {
	...
	if (selem.length > 3) {
		...
		if (GM_getValue("jvc_moderation",true)) {
			AjoutBoutonModération(selem[3],messid,pseudo,modo);
		}
...
switch (mode) {
case - 9 : { // Panneau de contrôle
	Menu(param[0],"")
	var bloc = document.getElementById("col1");
	bloc.innerHTML = "";
	var h2 = document.createElement('h2');
	h2.appendChild(document.createTextNode('Panneau de contrôle : ProEvolutionSoccer.Nyu'));
	bloc.appendChild(h2);
	...
	switch (param[0]) {
		case 0 : {
			...
			CreateOptionPC(tbody,param,"FirstMsg","jvc_first_page","Toujours revenir à la première page de la liste des sujets","");
			...
			CreateOptionPC(tbody,param,"Affichage","jvc_affichage","Activer l'affichage des images et vidéos dans les sujets","affichage");
			...
			CreateOptionPC(tbody,param,"Moderation","jvc_moderation","Activer l'aide à la modération","");
			...
			CreateOptionPC(tbody,param,"Preferences","jvc_preferences","Activer le menu de preferences personnalisé","");
			...
			CreateOptionPC(tbody,param,"Arme","jvc_arme_liste","Activer l'arme de modération","arme");
		break;
		}
		case 1 : {
			...
			switch (param[2]) {
				case "regroupements" : {
					...
					bloc.appendChild(document.createTextNode('Mode par défaut : '));
					...
					bloc.appendChild(document.createElement('br'));
					bloc.appendChild(document.createElement('br'));
					bloc.appendChild(document.createTextNode('-1 : Affichage par défaut'));
					bloc.appendChild(document.createElement('br'));
				...
				case "coloration" : {
					...
					bloc.appendChild(h3);
					bloc.appendChild(document.createElement('br'));
					bloc.appendChild(document.createTextNode('Pour changer de coloration il faut insérer le code hexadécimal RGB de la couleur en question'));
					bloc.appendChild(document.createElement('br'));
				...
				case "images" : {
					...
					for (var i=0;i<img.length;i++) {
						...
						tr.appendChild(td);
						var td = document.createElement('td');
						td.appendChild(document.createTextNode(" transformé en "));
						tr.appendChild(td);
						var td = document.createElement('td');
						td.style.textAlign = "center";
						td.appendChild(img2);
				...
				case "affichage" : {
					...
					tr.appendChild(td);
					var td = document.createElement('td');
					td.appendChild(document.createTextNode(" affichage par défaut des images"));
					tr.appendChild(td);
					document.getElementById(img.id).addEventListener("click", ModifPC, false);
					...
					tr.appendChild(td);
					var td = document.createElement('td');
					td.appendChild(document.createTextNode(" affichage par défaut des vidéos"));
					tr.appendChild(td);
					document.getElementById(img.id).addEventListener("click", ModifPC, false);
				break;
				}
				...
				case "kick" : {
					CreateButtonPC(tbody,"jvc_cs_last_msg"," atteindre la dernière page d'un sujet censuré",true);
					CreateButtonPC(tbody,"jvc_cs_form"," atteindre le formulaire de post d'un sujet censuré",true);
					...
					CreateButtonPC(tbody,"jvc_exc_mod_aff"," afficher les messages exclus quand connecté en tant que modérateur",true);
					CreateButtonPC(tbody,"jvc_exc_mod_tit"," ne pas censurer les titres des sujets quand connecté en tant que modérateur",true);
				break;
				}
				case "citation" : {
				...
				case "pseudos" : {
					...
					tr.appendChild(td);
					var td = document.createElement('td');
					td.appendChild(document.createTextNode(" déconnection automatique"));
					tr.appendChild(td);
					document.getElementById(img.id).addEventListener("click", ModifPC, false);
					...
					tr.appendChild(td);
					var td = document.createElement('td');
					td.appendChild(document.createTextNode(" affichage de la liste des forums modérés"));
					tr.appendChild(td);
					document.getElementById(img.id).addEventListener("click", ModifPC, false);
				...
				case "partage" : {
					CreateButtonPC(tbody,"jvc_lecture"," pour récupérer les données de groupement",false);
					CreateButtonPC(tbody,"jvc_ecriture"," pour lire les données de groupement",false);
					CreateButtonPC(tbody,"jvc_debug_post"," pour récupérer les données de post de message",false);
				break;
				}
				case "signature" : {
					...
					tr.appendChild(td);
					var td = document.createElement('td');
					td.appendChild(document.createTextNode(" activer les signatures personnalisées"));
					tr.appendChild(td);
					document.getElementById(img.id).addEventListener("click", ModifPC, false);
					...
					span.appendChild(document.createTextNode(200-textsign.length));
					span.style.color = "#FF0000";
					span.id = "carac_rest";
					bloc.appendChild(span);
					bloc.appendChild(document.createTextNode(' caractères.'));
					bloc.appendChild(document.createElement('br'));
...
//<input name="yournewmessage" type="hidden" />
				...
				case "date" : {
					...
					bloc.appendChild(document.createTextNode("Décalage par rapport à Greenwhch (en minutes : "));
					bloc.appendChild(input);
					bloc.appendChild(document.createElement('br'));
				...
				case "arme" : {
					CreateButtonPC(tbody,"jvc_arme_preselectexcl"," préselectionner les exclus dans la mire de l'arme",true);
					CreateButtonPC(tbody,"jvc_arme_lien_illicites"," préselectionner les personnes postant des liens censurés",true);
				break;
				}
			}
			bloc.appendChild(document.createElement('br'));
...
//----------------------------------------------------------------------------------------------------------
	...
	if  (moderation && GM_getValue("jvc_arme_liste",true)) {
		...
		th.id = "p_mod";
		th.className = "Suppression de la sélection#Masquer la sélection";
	...
	if (GM_getValue("jvc_post_msg",true) || GM_getValue("jvc_last_msg",true) || GM_getValue("jvc_kick",true) || GM_getValue("jvc_coloration",true) || GM_getValue("jvc_regroupement",true)||GM_getValue("jvc_date",true)) {
		for (var i = 1;i<elem.length;i++) {
			...
			if (cs && !(moderation && GM_getValue("jvc_exc_mod_tit",true))) {
				...
				italique.appendChild(document.createTextNode("sujet censuré (cliquez "));
				var fintexte = document.createTextNode(" pour y accéder)");
			...
			if (exc) {
				...
				newImg.title = "Montrer le sujet : « " + sjt + " » "+n+" message(s)";
				newImg.alt = newImg.title;
...
function Initialisation() {
	...
	"Regroupements"; {
		...
		groupe = "PaicCitron"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "AlerteCobra"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "Cola le Fou"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "Vil-e-Coyote"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "Always-Winners"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "Alberforth62"; color = "#000000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
		...
		groupe = "Lenoor"; color = "#FFA000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("2k8","",groupe,mode);
			listeadd("Agumon2003","",groupe,mode);
			listeadd("AmeHinotama","",groupe,mode);
			listeadd("Aquarinei","",groupe,mode);
			listeadd("ArseIe","",groupe,mode);
			listeadd("Arsele","",groupe,mode);
			listeadd("ArthasMenethil","",groupe,mode);
			listeadd("AurelienCapoue","",groupe,mode);
			listeadd("Balisto","",groupe,mode);
			listeadd("Batista_unlaesh ","",groupe,mode);
			listeadd("Beladonis","",groupe,mode);
			listeadd("Bij","",groupe,mode);
			listeadd("Buffalo_Usa","",groupe,mode);
			listeadd("Carloducul3","",groupe,mode);
			listeadd("Celerine","",groupe,mode);
			listeadd("ChaosGallantmon","",groupe,mode);
			listeadd("Chaptal","",groupe,mode);
			listeadd("Charlie","",groupe,mode);
			listeadd("Cherubimon","",groupe,mode);
			listeadd("Chocoboodor","",groupe,mode);
			listeadd("ChocoLenoor","",groupe,mode);
			listeadd("ChuckNorris","",groupe,mode);
			listeadd("demelza94","",groupe,mode);
			listeadd("DevilKirby","",groupe,mode);
			listeadd("De_Psycholoog","",groupe,mode);
			listeadd("Duisternis","",groupe,mode);
			listeadd("ElSolDoLo","",groupe,mode);
			listeadd("Extra-Techno","",groupe,mode);
			listeadd("Frankrijk","",groupe,mode);
			listeadd("Fricofaye","",groupe,mode);
			listeadd("Gambit2","",groupe,mode);
			listeadd("GarenZesshugeki","",groupe,mode);
			listeadd("GateGuardian","",groupe,mode);
			listeadd("Glacius","",groupe,mode);
			listeadd("GlasMelk","",groupe,mode);
			listeadd("HommeTronc","",groupe,mode);
			listeadd("Hoom","",groupe,mode);
			listeadd("Ienoor","",groupe,mode);
			listeadd("ImperialDramon2","",groupe,mode);
			listeadd("JungleFever","",groupe,mode);
			listeadd("J_aime_Ies_gens","",groupe,mode);
			listeadd("Kaabii","",groupe,mode);
			listeadd("KairyuShin","",groupe,mode);
			listeadd("Kaizerin","",groupe,mode);
			listeadd("Kazejin","",groupe,mode);
			listeadd("Kempi","",groupe,mode);
			listeadd("Kerobero","",groupe,mode);
			listeadd("Kerstmis","",groupe,mode);
			listeadd("Khing","",groupe,mode);
			listeadd("Killua19","",groupe,mode);
			listeadd("Kirby13","",groupe,mode);
			listeadd("Kirbyne","",groupe,mode);
			listeadd("LadyLike","",groupe,mode);
			listeadd("LaHaye","",groupe,mode);
			listeadd("Licht","",groupe,mode);
			listeadd("Lenoortje","",groupe,mode);
			listeadd("LIM","",groupe,mode);
			listeadd("Loise","",groupe,mode);
			listeadd("Ludo_Docx","",groupe,mode);
			listeadd("Maart","",groupe,mode);
			listeadd("Magmar","",groupe,mode);
			listeadd("MARINE5","",groupe,mode);
			listeadd("Marleen","",groupe,mode);
			listeadd("Mega-Australien","",groupe,mode);
			listeadd("Meisje","",groupe,mode);
			listeadd("Mermaidlips101","",groupe,mode);
			listeadd("MetalSeadramon","",groupe,mode);
			listeadd("Metal_Imperiale","",groupe,mode);
			listeadd("Mistec_Baltomon","",groupe,mode);
			listeadd("Moonflow","",groupe,mode);
			listeadd("Over-Extramon","",groupe,mode);
			listeadd("Rikimaro","",groupe,mode);
			listeadd("Sanga","",groupe,mode);
			listeadd("SangaDeLaFoudre","",groupe,mode);
			listeadd("SangaDuTonnere","",groupe,mode);
			listeadd("Siam","",groupe,mode);
			listeadd("SirEbrum","",groupe,mode);
			listeadd("Solar_Energy","",groupe,mode);
			listeadd("Suijin","",groupe,mode);
			listeadd("Sulfurine","",groupe,mode);
			listeadd("TalThulOrtAmn","",groupe,mode);
			listeadd("TalThul0rtAmnr","",groupe,mode);
			listeadd("TenbuHorin","",groupe,mode);
			listeadd("Tokkori","",groupe,mode);
			listeadd("TrashFever","",groupe,mode);
			listeadd("Ultraphoenix","",groupe,mode);
			listeadd("Vandaag","",groupe,mode);
			listeadd("VexOhmIstDol","",groupe,mode);
			listeadd("Vuurkorf","",groupe,mode);
			listeadd("Vuurmuur","",groupe,mode);
			listeadd("Vuurwerk","",groupe,mode);
			listeadd("Vuury","",groupe,mode);
			listeadd("YesserRoshdy","",groupe,mode);
			listeadd("Zambamon","",groupe,mode);
			listeadd("Zudomon","",groupe,mode);
			listeadd("[-]Goku[-]","",groupe,mode);
			listeadd("[JV]Lenoor","",groupe,mode);
		}
		groupe = "Lenoor"; color = "#FFA000"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("aranoch","",groupe,mode);
			listeadd("Lamantine","",groupe,mode);
		}
		groupe = "Coldlake!"; color = "#9999FF"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("1k994","",groupe,mode);
			listeadd("Aarde","",groupe,mode);
			listeadd("Amoer","",groupe,mode);
			listeadd("Audi_R-Zero","",groupe,mode);
			listeadd("Bladees","",groupe,mode);
			listeadd("Coldlake","",groupe,mode);
			listeadd("Elektriciteit","",groupe,mode);
			listeadd("Essar","",groupe,mode);
			listeadd("Frigore","",groupe,mode);
			listeadd("Heelal","",groupe,mode);
			listeadd("HeIicobide","",groupe,mode);
			listeadd("Hotlake","",groupe,mode);
			listeadd("IchLiebeLenoor","",groupe,mode);
			listeadd("IkHouVanLenoor","",groupe,mode);
			listeadd("ILenoor","",groupe,mode);
			listeadd("iLenoor","",groupe,mode);
			listeadd("ILoveLenoor","",groupe,mode);
			listeadd("Incinerateur","",groupe,mode);
			listeadd("InLenoorWeTrust","",groupe,mode);
			listeadd("Ium","",groupe,mode);
			listeadd("JeTAimeLenoor","",groupe,mode);
			listeadd("Len0or","",groupe,mode);
			listeadd("Leno0r","",groupe,mode);
			listeadd("LenoorForever","",groupe,mode);
			listeadd("LenoorIR","",groupe,mode);
			listeadd("LoveLenoor","",groupe,mode);
			listeadd("LovletHill","",groupe,mode);
			listeadd("Master-G","",groupe,mode);
			listeadd("MazdaRX-Huit","",groupe,mode);
			listeadd("Megacryometeore","",groupe,mode);
			listeadd("Merlimont","",groupe,mode);
			listeadd("Nanorobot","",groupe,mode);
			listeadd("NeerIandaises","",groupe,mode);
			listeadd("PixelCat","",groupe,mode);
			listeadd("PixelClochette","",groupe,mode);
			listeadd("PixelGribouille","",groupe,mode);
			listeadd("PixelLenoor","",groupe,mode);
			listeadd("PixelMoon","",groupe,mode);
			listeadd("Potassium","",groupe,mode);
			listeadd("Poussin_debile","",groupe,mode);
			listeadd("PurkJanSing","",groupe,mode);
			listeadd("ReineLocuste","",groupe,mode);
			listeadd("RXTSupercharged","",groupe,mode);
			listeadd("Sanatorium","",groupe,mode);
			listeadd("Sea-Doo","",groupe,mode);
			listeadd("Soprakountache","",groupe,mode);
			listeadd("TeAmoLenoor","",groupe,mode);
			listeadd("Titurium","",groupe,mode);
			listeadd("Tost1c1ty","",groupe,mode);
			listeadd("ToxineBotulique","",groupe,mode);
			listeadd("Tritherapie","",groupe,mode);
			listeadd("Ubercopieur","",groupe,mode);
			listeadd("Ununhexium","",groupe,mode);
			listeadd("Vuur","",groupe,mode);
			listeadd("VuurJeTAime","",groupe,mode);
			listeadd("Waddle-Dee","",groupe,mode);
			listeadd("WiiBryan","",groupe,mode);
			listeadd("Wind","",groupe,mode);
		}
		groupe = "Jan"; color = "#00AA00"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("Clem94","",groupe,mode);
			listeadd("Ijs","",groupe,mode);
			listeadd("Jan","",groupe,mode);
			listeadd("MarseilleParis","",groupe,mode);
			listeadd("ParisMarseille","",groupe,mode);
			listeadd("Sopracountash","",groupe,mode);
			listeadd("Soprakountash","",groupe,mode);
		}
		groupe ="Demelza"; color = "#FF53FF"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("Bloemen","",groupe,mode);
			listeadd("Boeketbloemen","",groupe,mode);
			listeadd("LibegonSauvage","",groupe,mode);
			listeadd("MetalFox","",groupe,mode);
		}
		groupe = "Yaya"; color = "#800080"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("[Romane]","",groupe,mode);
			listeadd("_[_Yann_]_","",groupe,mode);
			listeadd("_[Rachel]_","",groupe,mode);
			listeadd("_[Yann]_","",groupe,mode);
			listeadd("___Yann___","",groupe,mode);
			listeadd("Amaranth","",groupe,mode);
			listeadd("Cindye","",groupe,mode);
			listeadd("Embrace","",groupe,mode);
			listeadd("LordAres","",groupe,mode);
			listeadd("MrNety","",groupe,mode);
			listeadd("Pann","",groupe,mode);
			listeadd("ParkJinSung","",groupe,mode);
			listeadd("ParkJinSung2","",groupe,mode);
			listeadd("Patatos_2008","",groupe,mode);
			listeadd("TaggleZerch","",groupe,mode);
			listeadd("Woord","",groupe,mode);
			listeadd("Yannick3","",groupe,mode);
		}
		groupe = "Mazda!"; color = "#FF4901"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("1996","",groupe,mode);
			listeadd("1k996","",groupe,mode);
			listeadd("2k10","",groupe,mode);
			listeadd("AdaWong","",groupe,mode);
			listeadd("Alsjeblieft","",groupe,mode);
			listeadd("ArbreFritier","",groupe,mode);
			listeadd("Arctica","",groupe,mode);
			listeadd("Astral","",groupe,mode);
			listeadd("BarretteMemoire","",groupe,mode);
			listeadd("Bazin","",groupe,mode);
			listeadd("BiduleInconnu","",groupe,mode);
			listeadd("bloem","",groupe,mode);
			listeadd("CadeauxDeNoel","",groupe,mode);
			listeadd("Cible","",groupe,mode);
			listeadd("Coldfake","",groupe,mode);
			listeadd("Crazyjump","",groupe,mode);
			listeadd("Cryogenie","",groupe,mode);
			listeadd("DarkMoor","",groupe,mode);
			listeadd("Deisler","",groupe,mode);
			listeadd("DemeIza","",groupe,mode);
			listeadd("Demelza","",groupe,mode);
			listeadd("DesertStorm","",groupe,mode);
			listeadd("Drepanocytose","",groupe,mode);
			listeadd("DrLenoor","",groupe,mode);
			listeadd("Elektrizitat","",groupe,mode);
			listeadd("Endzeit","",groupe,mode);
			listeadd("Erde","",groupe,mode);
			listeadd("Fatal1ty","",groupe,mode);
			listeadd("Faus","",groupe,mode);
			listeadd("Feuer","",groupe,mode);
			listeadd("GameSpy","",groupe,mode);
			listeadd("GanondorfPower","",groupe,mode);
			listeadd("GazSarin","",groupe,mode);
			listeadd("GhostOfTheSun","",groupe,mode);
			listeadd("Growl","",groupe,mode);
			listeadd("GustaveEiffel","",groupe,mode);
			listeadd("Hoarau","",groupe,mode);
			listeadd("HubUSB","",groupe,mode);
			listeadd("Hzp","",groupe,mode);
			listeadd("Hzpower","",groupe,mode);
			listeadd("Iceworld","",groupe,mode);
			listeadd("iDemelza","",groupe,mode);
			listeadd("IkHoudVanJou","",groupe,mode);
			listeadd("IkHoudVanLenoor","",groupe,mode);
			listeadd("IkHouVanDemelza","",groupe,mode);
			listeadd("IkHouVanJou","",groupe,mode);
			listeadd("ILoveYouLenoor","",groupe,mode);
			listeadd("iMazda","",groupe,mode);
			listeadd("IntelCorei7","",groupe,mode);
			listeadd("JinSungPark","",groupe,mode);
			listeadd("Jose","",groupe,mode);
			listeadd("Katatonia-","",groupe,mode);
			listeadd("Kezman","",groupe,mode);
			listeadd("KillingSpree","",groupe,mode);
			listeadd("Kipa","",groupe,mode);
			listeadd("Lenoor","",groupe,mode);
			listeadd("Lenoor-","",groupe,mode);
			listeadd("LenoorCladMast","",groupe,mode);
			listeadd("Lenooriser","",groupe,mode);
			listeadd("LenoorMazouille","",groupe,mode);
			listeadd("LenoorNorris","",groupe,mode);
			listeadd("LesLangoliers","",groupe,mode);
			listeadd("Lesuud","",groupe,mode);
			listeadd("LogitechG5","",groupe,mode);
			listeadd("Logout","",groupe,mode);
			listeadd("Lovlet","",groupe,mode);
			listeadd("Mathijsen","",groupe,mode);
			listeadd("Mazda323","",groupe,mode);
			listeadd("MazdaDeVoyage","",groupe,mode);
			listeadd("MazdaRX-10","",groupe,mode);
			...
			listeadd("Mazouille","",groupe,mode);
			listeadd("Melodeath","",groupe,mode);
			listeadd("Mendelevium","",groupe,mode);
			listeadd("MerIimont","",groupe,mode);
			listeadd("Morpheo","",groupe,mode);
			listeadd("MyTwin","",groupe,mode);
			listeadd("Natal","",groupe,mode);
			listeadd("Nederlander","",groupe,mode);
			listeadd("Nee","",groupe,mode);
			listeadd("Neerlandais","",groupe,mode);
			listeadd("Neerlandaise","",groupe,mode);
			listeadd("NextProfundis","",groupe,mode);
			listeadd("NissanSentra","",groupe,mode);
			listeadd("Noor","",groupe,mode);
			listeadd("Nozel","",groupe,mode);
			listeadd("Nubz","",groupe,mode);
			listeadd("obZen","",groupe,mode);
			listeadd("Odinist","",groupe,mode);
			listeadd("OhMijnGod","",groupe,mode);
			listeadd("Ouch","",groupe,mode);
			listeadd("Paiement","",groupe,mode);
			listeadd("PainDeMie","",groupe,mode);
			listeadd("Pancrate","",groupe,mode);
			listeadd("Parijs","",groupe,mode);
			listeadd("Porphyrie","",groupe,mode);
			listeadd("Potion","",groupe,mode);
			listeadd("Profil","",groupe,mode);
			listeadd("Pyromanie","",groupe,mode);
			listeadd("Quintessenz","",groupe,mode);
			listeadd("ResidentEvil","",groupe,mode);
			listeadd("RivetCity","",groupe,mode);
			listeadd("RoyalFIush","",groupe,mode);
			listeadd("SanctusIgnis","",groupe,mode);
			listeadd("ShadowClad","",groupe,mode);
			listeadd("ShauniEricsson","",groupe,mode);
			listeadd("Slyin","",groupe,mode);
			listeadd("SonyCubeX","",groupe,mode);
			listeadd("Spok","",groupe,mode);
			listeadd("Stari","",groupe,mode);
			listeadd("StephanForte","",groupe,mode);
			listeadd("Stratocaster","",groupe,mode);
			listeadd("Suupos","",groupe,mode);
			listeadd("Syles","",groupe,mode);
			listeadd("Telepathe","",groupe,mode);
			listeadd("Telepathie","",groupe,mode);
			listeadd("TexasHoldEm","",groupe,mode);
			listeadd("Thrasher","",groupe,mode);
			listeadd("Thunder_","",groupe,mode);
			listeadd("Tournament","",groupe,mode);
			listeadd("Tosti","",groupe,mode);
			listeadd("Tox1c1ty","",groupe,mode);
			listeadd("Toxibear","",groupe,mode);
			listeadd("TwelveHundred","",groupe,mode);
			listeadd("UitElkaar","",groupe,mode);
			listeadd("VitalijKuprij","",groupe,mode);
			listeadd("VivaEmptiness","",groupe,mode);
			listeadd("Wishmaster","",groupe,mode);
			listeadd("Zadig","",groupe,mode);
			listeadd("Zapdos","",groupe,mode);
		}
		groupe = "Anti"; color = "#3030FF"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("Antig","",groupe,mode);
			listeadd("Antigraviotis","",groupe,mode);
			listeadd("Canartishauni","",groupe,mode);
			listeadd("DakotaFanning","",groupe,mode);
			listeadd("ElleFanning","",groupe,mode);
			listeadd("Lamantine","",groupe,mode);
			listeadd("MichaelKyle","",groupe,mode);
			listeadd("Ombrine","",groupe,mode);
			listeadd("Ritzy","",groupe,mode);
			listeadd("Schaduw","",groupe,mode);
		}
		groupe = "Rika"; color = "#FFAAC8"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("Dinorika","",groupe,mode);
			listeadd("Rika","",groupe,mode);
			listeadd("Saartje","",groupe,mode);
		}
		groupe = "Alonzo"; color = "#00BB99"; {
			GM_setValue("gp#prc#"+groupe.toLowerCase()+"#clr",color);
			listeadd("TribalWeb","",groupe,mode);
			listeadd("SpeedyGonzatest","",groupe,mode);
		}
	}
	"Images"; {
	...
	"Smileys"; {
		...
		smladd(":kira:","http://www.noelshack.com/uploads/16062009/Kira032574.gif");
		smladd(":demon:","http://www.noelshack.com/uploads/16062009/Demon055139.gif");
		smladd(":monosourcil:","http://www.noelshack.com/uploads/16062009/HapMonosourcil095433.gif");
		smladd(":coange:","http://www.noelshack.com/uploads/16062009/Coange027983.gif");
		smladd(":malhap:","http://www.noelshack.com/uploads/16062009/HapMalade017807.gif");
		smladd(":mort2:","http://www.noelshack.com/uploads/16062009/Mort2032352.gif");
		smladd(":snif3:","http://www.noelshack.com/uploads/16062009/Snif3084718.gif");
		smladd(">_<","http://www.noelshack.com/uploads/16062009/-_-099920.gif");
	}
	"Préférences"; {
		GM_setValue("nb_preferences",0);
		PreferencesAjout("http://www.jeuxvideo.com/forums/0-19584-0-1-0-1-0-0.htm","Forum PES 2010");
		PreferencesAjout("http://www.jeuxvideo.com/forums/0-15134-0-1-0-1-0-0.htm","Forum PES 2009");
		PreferencesAjout("http://www.jeuxvideo.com/forums/0-50-0-1-0-1-0-0.htm","Blabla 15-18 ans");
		PreferencesAjout("http://www.jeuxvideo.com/forums/0-15-0-1-0-1-0-0.htm","Blabla -15 ans");	
		PreferencesAjout("http://www.jeuxvideo.com/forums/0-1000017-0-1-0-1-0-0.htm","Forum Réclamations");
		PreferencesAjout("http://www.jeuxvideo.com/forums/1-19584-149393-1-0-1-0-script-pes-nyu.htm","Topic de PES.Nyu");
		PreferencesAjout("http://www.noelshack.com/","NoelShack"); 
	}
	"Protections"; {
