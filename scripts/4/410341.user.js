// ==UserScript==
// @name           Google Calendar Time Box Duration EditPage
// @namespace      http://userscripts.org/users/410341
// @description    Shows the duration of each event.
// @include        https://www.google.com/calendar*
// ==/UserScript==

var timer;
document.querySelector('#coverinner').addEventListener("DOMSubtreeModified",
    function(e)
    {
...
{
    var timeElements = document.querySelectorAll('.text.dr-date');
    start = timeElements[0].value;
    end = timeElements[1].value;
    str = start +" – "+end;
        str     = str.replace(/^(\d{2}:\d{2}) – (\d{2}:\d{2}) (.*?)$/, callbackTimeISO);
        str     = str.replace(/^(\d+)(\:?\d*?)(p?) – (\d+)(\:?\d*?)(p?) (.*?)$/, callbackTimeGoogleAMPM);;
    console.log(str)
        timeElements[1].innerHTML = str;
        window.clearTimeout(timer);
    }
}
