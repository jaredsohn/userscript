...
// @namespace	MAX
// @version		1.0
// @updateURL   http://userscripts.org/scripts/source/184630.user.js
// @downloadURL	http://userscripts.org/scripts/source/184630.user.js
// @description É uma pequena extensão para o jogo de browser Grepolis.(contador das tropas, a força das unidades, caixa de smiley, opções de comércio, WW partes e alterações na interface do usuário)
 ...
 * Changes
 * ----------------------------------------------------------------------------------------------------------------------------
 * | ● Total sum = sum of all stages, not to current stage anymore
 * ----------------------------------------------------------------------------------------------------------------------------
 *******************************************************************************************************************************/
...
if(uw.location.pathname === "/game/index"){
    $('<svg width="0%" height="0%">'+
      ...
      '<filter id="Hue1"><feColorMatrix type="hueRotate" values= "60"></filter>'+
      '<filter id="Hue2"><feColorMatrix type="hueRotate" values="150"></filter>'+
      '<filter id="Hue3"><feColorMatrix type="hueRotate" values="-60"></filter>'+
      '</svg>').appendTo('#ui_box');
}
/*******************************************************************************************************************************
...
var LANG = {
    de : {
        set : {
            txt: {
                dsc: "MAX-Tools bietet unter anderem einige Anzeigen, eine Smileyauswahlbox,<br>Handelsoptionen und einige Veränderungen des Layouts.",
            ...
            way: [ "Laufzeit",			"Zeigt im ATT/UT-Fenster die Laufzeit bei Com Movimento Acelerado an" ],
            ...
            tsk: [ "Taskleiste",		"Vergrößert die Taskleiste und minimiert alle Fenster beim Start" ],
            cht: [ "Chat",				"Blendet den Chat im Menu aus" ],
            ...
            twn: [ "Stadtliste",		"Fügt Stadttyp-Icons zur Stadtliste hinzu" ],
            
            sav_btn: "Speichern", don_btn: "http://s7.directupload.net/images/140131/ctahnu2q.png",
        },
        sml : { std: "Normal ", gre: "Grepolis ", nat: "Natur ", ppl: "Leute ", oth: "Sonstige " },
    ...
    en : {
        set : {
            txt: {
                dsc: "Maximus TOOLS - Tradução efetuada em algumas funções",
                act: "Ativar / desativar funções do conjunto de ferramentas:",
                prv: "Pré-visualização das funções individuais:",
            },
            // opt: [label, tooltip]
            bir: [ "Cont. de Birremes",	"Conta quantos birremes tem em todas as suas cidades (Informando um total num mini mapa)" ],
            sml: [ "Smileys",			"Barra com smileys para mensagens" ],
            str: [ "Força das Unidades","Gráfico com forças das unidades" ],
            trd: [ "Troca",				"Estende a janela de comércio por um percentual do comércio, um comércio e limite de recrutamento marcadores para festival urbano" ],
            cnt: [ "Revoltas",			"Contagens dos ataques / apoios na janela de revolta (apenas próprias revoltas)" ],
            way: [ "Mov. de Tropas",	"Apresenta maior velocidade tropa na janela de ataque / apoio" ],
            wwc: [ "Maravilhas",		"Compartilhar cálculo e recursos botões Anterior e Avançar na maravilhas do mundo acabados contador + (atualmente não desativável!)" ],
            sim: [ "Simulador",			"Adaptação do layout simulador e exposição permanente da caixa de alterações" ],
            spl: [ "Encantamentos",		"Caixa de encantamentos movel" ],
            mov: [ "Cx. de Atividades",	"Melhoria da exibição de caixas de comércio e atividade das tropas (Móvel)" ],
            pop: [ "Popup",				"Alteração na caixa de favores" ],
            tsk: [ "Barra de Atividade","Aumenta a barra de tarefas e minimiza todas as janelas na inicialização" ],
            cht: [ "Chat",				'Inativado o CHAT do Grepolis' ],
            bbc: [ "BBCode Campo",		"Expande o bb-bbcode num campo de defesa automáticamente" ],
            com: [ "Comparações",		"Adiciona tabelas de comparação de unidades" ],
            twn: [ "Lista de Cidades",	"Adiciona ícones de tipo de cidade para a lista de cidade" ],
            
            sav_btn: "Salvar", don_btn: "https://www.paypal.com/en_US/i/btn/btn_donate_LG.gif",
        },
        sml : { std: "Diversos ", gre: "Grepolis ", nat: "Natureza ", ppl: "Pessoais ", oth: "Outros " },
        uni : "Tropas Disponiveis",
        bbc : { 
            ttl: "Vizualização: Defesa da Cidade", inf: "Tropas da Cidade:", mov: "Ataques / Apoios:", dev: "Delays", but: "Inserir",
            sel: [ "Unidades Terrestres",  "Premium Ativo", "Prata na Gruta", "Ataques / Apoios" ] 
        },
        wwc : { leg: "WW Share", stg: "Stage", tot: "Total"},
        sim : { str: "Força das Unidades", los: "Perdas", mod: "Sem Influência no Modificador" },
        com : {
            dsc: "Comparação de Tropas", hck: "Impacto", prc: "Corte", dst: "Arremesso", sea: "Naval", att: "Ataque", def: "Defesa", spd: "Velocidade", 
            bty: "Saque (Recursos)", cap: "Capacidade de Transporte", res: "Custo (Recursos)", fav: "Favor", tim: "Tempo de Recrutamento (s)"
        },
        trd : { uni: "Proporção de recursos de um tipo de unidade", rat: "Compartilhar da capacidade de armazenamento da cidade alvo", prc: "Percentagem de Troca"}
    },
    fr : {  
        set : {  
            txt: {  
                dsc: "MAX-Tools offres certains écrans, une boîte de smiley, les options <br>commerciales, des changements à la mise en page et d'autres choses.",  
            ...
            tsk: [ "<nobr>Barre de tâches&nbsp;&nbsp;</nobr>",		"La barre de tâches augmente et minimise toutes les fenêtres lors du démarrage" ],  
            cht: [ "Chat",					"Masque le chat dans le Menu" ],  
            ...
            twn: [ "<nobr>Liste des villes</nobr>",		"Ajoutes desicônes de type de ville à la liste de ville" ],  
            
            sav_btn: "Sauver", don_btn: "http://s7.directupload.net/images/140131/ctahnu2q.png",  
        },  
        sml : { std: "Normal ", gre: "Grepolis ", nat: "Nature ", ppl: "Gens ", oth: "Autres " },  
...
if(uw.location.pathname === "/game/index"){
    ...
    options = { 
        bir: GM_getValue("set_biri", true),	sml: GM_getValue("set_smile", true),str: GM_getValue("set_str", true),	trd: GM_getValue("set_trade", true),way: GM_getValue("set_way", true), 
        cnt: GM_getValue("set_eo", true),	sim: GM_getValue("set_simu", true),	spl: GM_getValue("set_spell", true),mov: GM_getValue("set_move", true),	tsk: GM_getValue("set_task", true), 
        cht: GM_getValue("set_hide", false),pop: GM_getValue("set_pop", true),	wwc: GM_getValue("set_wwc", true),	bbc: true,	com: true, twn : true
    };
    ...
    options = JSON.parse(GM_getValue("options", JSON.stringify(options)));
...
function loadSettingImages(){
    if(firstLoad){
        $('<style type="text/css"> .dio_tools_bg { background:url(http://s7.directupload.net/images/140202/dz8ziycu.png) 320px 10px no-repeat;} </style>').appendTo('head');
...
function settings() {
    ...
    if(!$("#dio_tools").get(0)){
        $(".settings-menu ul:last").append('<li id="dio_li"><img id="dio_icon" src="http://www.greensmilies.com/smile/smiley_emoticons_smile.gif"></div> <a id="dio_tools" href="#"> MAX-Tools</a></li>');
    ...
    $("#dio_tools").click(function () { 
        ...
        if(!$('#dio_settings').get(0)){
            $('.settings-container').append(
                ...
                '<a href="http://br.grepolis.com" target="_blank" style="color:white">MAX-Tools (v'+ GM_info.script.version +')</a></div>'+
                '<p>'	+ LANG[LID].set.txt.dsc + '</p>'+
                '<p class="bold"><u>'+ LANG[LID].set.txt.act + '</u></p>'+
                '<table width="100%" style=" font-size: 1.0em;"><tr><td width="24%">'+
                ...
                '<div id="cht" class="checkbox_new"><div class="cbx_icon"></div><div class="cbx_caption">'+ LANG[LID].set.cht[0] +'</div></div><br><br>'+
                '</td></tr>'+
                '</table>'+
            ...
            $("#pop").tooltip(LANG[LID].set.pop[1]); $("#tsk").tooltip(LANG[LID].set.tsk[1]); $("#cht").tooltip(LANG[LID].set.cht[1]); $("#twn").tooltip(LANG[LID].set.twn[1]);
...
function addSettingsButton(){
    ...
    $('.dio_settings').tooltip("MAX-Tools");
    $('.dio_settings').click(function(){
        ...
        uw.Layout.wnd.Create(uw.GPWindowMgr.TYPE_PLAYER_SETTINGS,'Einstellungen');
    });
}
        ...
        if((ch.length == 5) && exc && (sum == 118)){
            switch (action) {  
                case "/data/get":
                    ...
                    if(options.cht)	{hideNavElements();}
                ...
                case "/debug/log_startup_time":
                    ...
                    if(options.tsk)	{
                        waitStartup();
                        /*
                    for(var u = 0; u < uw.GPWindowMgr.getOpen(uw.Layout.wnd.TYPE_SHOW_ON_LOGIN).length; u++){
                    	uw.GPWindowMgr.getOpen(uw.Layout.wnd.TYPE_SHOW_ON_LOGIN)[u].minimize();
                    }
                    */
                    }
                    if(options.cht)	{hideChat();}
                    ...
                    if(options.twn)	{startTownList();
                                     addTownIcon();
                                    }
                    //messageSound();
                    
                    //console.log(uw.GameData);
                    //test();
                ...
                case "/town_info/attack": case "/town_info/support":
                    if(options.way)	{TownTabHandler(action.split("/")[2]); }
                    break;
                case "/report/index":
...
function waitStartup(){
    if(!$('.ui-dialog-titlebar-minimize').get(0) && (cnt <= 10)){
        cnt++;
        setTimeout(function(){ 
            waitStartup();
        }, 500);
 ...
 * Town list
 * ----------------------------------------------------------------------------------------------------------------------------
 * | ● Adds town type to the town list
 * ----------------------------------------------------------------------------------------------------------------------------
 *******************************************************************************************************************************/
...
function startTownList(){
    ...
    $('<style type="text/css"> #town_groups_list .inner_column { border: 1px solid rgba(100, 100, 0, .3);margin: -2px 0px 0px 2px;} </style>').appendTo('head');
    ...
    var town_list_switch = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
        ...
        mutations.forEach(function(mutation) {
            if(mutation.addedNodes[0]){
                if(mutation.addedNodes[0].id === "town_groups_list"){
                    changeTownList();
                    town_list_switch.observe($('#town_groups_list').get(0), { attributes: false, childList: true, characterData: false});
                }
            }
        });    
    });
    town_list.observe($('body').get(0), { attributes: false, childList: true, characterData: false});
    
    //$('.town_name_area .caption').prepend('<div class="townicon_'+ (autoTownTypes[uw.Game.townId] || "no") +'" style="background-position:3px 3px;background-repeat: no-repeat; margin-left:2px; padding-left:0px;height:20px;width:20px; float:left">');
    //$('.town_name_area .caption').append('<div class="townicon_'+ (autoTownTypes[uw.Game.townId] || "no") +'" style="background-position:3px 3px;background-repeat: no-repeat; margin-left:2px; padding-left:0px;height:20px;width:20px; float:left">'); 
}
...
function addTownIcon(){
    ...
    $('.town_name_area').css({ zIndex: 6 }); // because of Kapsonfires Script
    ...
    var icoArray = [ 'ld', 'lo', 'bu', 'sd', 'so', 'po', 'fd', 'fo', 'no' ];
    
    // Select boxes for unit and ratio
    $('<div class="select_town_icon dropdown-list default active">'+
      '<div class="item-list">'+
      '<hr style="color:black; border:1px dashed black;margin: 0px 0px 7px 0px;position: relative;top: 3px;">'+
      '<div class="option_s auto" name="auto" style="width:75px !important; border:2px solid rgb(162, 123, 0); border-radius: 5px; height: 14px !important;line-height:1;"><b>Auto</b></div>'+
      '</div></div>').appendTo("#town_icon");
    
    for(var i in icoArray){
        if(icoArray.hasOwnProperty(i)){
            $('.select_town_icon .item-list').prepend('<div class="option_s icon_small townicon_'+ icoArray[i] +'" name="'+ icoArray[i] +'"></div>');
        }
    }
    ...
    $('.select_town_icon').css({
        ...
        boxShadow: 'rgba(0, 0, 0, 0.498039) 4px 4px 6px',
        borderRadius: '0px 10px 10px 10px',
        background: "url(http://gpde.innogamescdn.com/images/game/popup/middle_middle.png)"
    });
    $('#town_icon .item-list').css({ maxHeight: '400px', maxWidth: '200px', align: "right" });
    ...
    $('#town_icon .option_s').hover(
        ...
        function(){
            $('.option_s').css({"-webkit-filter" : "grayscale(0%) sepia(0%)  brightness(1.0)", "filter": "url(#GrayScale)"});
            $('.option_s').css({ border: "2px solid rgba(0,0,0,0.0)"});
...
function loadFilter(){
    //console.log($('#dd_filter_type .caption').get(0).innerHTML);
    if(!($('#dd_filter_type .caption').get(0).innerHTML === filter)){
        $('#dd_filter_type .caption').get(0).click();
        $('#dd_filter_type_list .item-list div:contains('+ filter +')').get(0).click();
    }
}
...
function countMovements(){
    ...
    var str = "<div style='float:left;margin-right:5px;'>Truppenbewegungen: </div>"+
    ...
    $('.gpwindow_content .tab_content .bold').get(0).innerHTML = str;
    $('.move_hr').css({
...
function addShortDuration(wndID){
    ...
    */
    ...
    $(wndID + '.short_duration').tooltip("Movimento Acelerado");
...
function addRecTrade(wndID){
    ...
    $(wndID + '.option_s').hover(
        ...
        function(){
            $('.option_s').css({"-webkit-filter" : "grayscale(100%) sepia(0%)", "filter": "url(#GrayScale)"});
...
/*******************************************************************************************************************************
 * Gesamtstärke der Einheiten (Schlag/Stich/Distanz)
 * ----------------------------------------------------------------------------------------------------------------------------
 * | ● Einheitenstärke: Einheitenmenü
 * |	- Umswitchen von Def/Off-Anzeige mit Buttons
 * |	- Auswahlmöglichkeit spezieller Einheitentypen
 * | ● Einheitenstärke: EO
 * | ● Einheitenstärke: Kaserne
 * | ● Transporterkapazität: Einheitenmenu
 * |	- Umswitchen der Transportergeschwindigkeit (+/- Große Transporter)
 * ----------------------------------------------------------------------------------------------------------------------------
 *******************************************************************************************************************************/
...
function setStrengthMenu() {
    var unitsIn = uw.ITowns.getTown(uw.Game.townId).units(),
        e, units = getSelectedUnits();
    
    //console.log(units.sword + " " + units.sword);
...
function addStrengthMenu(){
    ...
    $('#off').hover(function(){
        css = $(this).attr('style');
        console.log(css);
        $(this).css({"-webkit-filter" : "brightness(1.5)"});
    }, function(){
        $(this).attr('style') = css;
    });
    */
...
function changeSimulatorLayout(){
    ...
    $('.place_sim_select_strategies select').get(0).children[0].innerHTML ="Normal";
...
function addForm(e){
    ...
    $(bbcodeBarId + ' #dio_insert').click(function(){
        ...
        var str = '[img]'+ imgArray.bordure + '[/img]'+
            '\n\n[color=#006B00][size=14][b][u]'+ LANG[LID].bbc.ttl +' [/u]([color=#161616]©MAX-Tools[/color])[/b][/size][/color]\n\n'+
        ...
        if($('#f_mov').hasClass("checked")){ 
            ...
            $('#toolbar_activity_commands_list .content .command').each(function(){
                ...
                if((cl[cl.length-1] === "returning" || cl[cl.length-1] === "revolt_arising" || cl[cl.length-1] === "revolt_running") && ((bb_count_str + bb_count_move) < 480)) {
                    ...
                    move_table += getArrivalTime($(this).children()[1].innerHTML) + " Uhr[||]";
            ...
            move_table += (i%1) ? "[/**]" : "";
            move_table += "[*][|][color=#800000][size=10][i] ("+ LANG[LID].bbc.dev +": ±1s)[/i][/size][/color][/*][/table]\n";
        }
...
smileyArray.standard = [ 
    "smilenew", "%5E%5E", "lol", "neutral_new", "afraid", "freddus_pacman", "auslachen2", "kolobok-sanduhr", "bussi2", "winken4", "flucht2", "panik4", "ins-auge-stechen", 
...
if($(".editor_textbox_container").get(0)){
    ...
    $('.posticons table').css({ width: "360px", marginTop: "-16px"});
    ...
    $('.posticons tr:eq(2) label').css({ marginLeft: "254px" });
    $('.posticons tr:eq(2)').prependTo('.posticons table');
    $('.posticons p').remove();
...
function addSmileyBox(e){
    ...
    $('<img class="smiley_button" src="'+ smileyArray.button[1].src +'">').appendTo(bbcodeBarId + ' .bb_button_wrapper');
 ...
 * | ● Larger taskbar and minimized windows on startup (daily reward)
 * | ● Hiding menu element "Chat"
...
function initSpellBox(){
    ...
    $('.gods_spells_menu .top').css({
        background: 'url("http://de.cdn.grepolis.com/images/game/layout/layout_2.51_compressed.png") no-repeat right -232px'
    });
...
function minimizeWindows(){
    $('.ui-dialog-titlebar-minimize').each(function(){
        $(this).click();
    });
}
