// ==UserScript==
    // @name           Extended Menu (Dutch, Nederlands)
    // @namespace      LightSeven
    // @description    Menu maar dan wat uitgebreider
 ...
 var jump_gate_text = 'Sprongpoort';
     ...
     if (tables.length > 4)
    {
        ...
        var trs = tables[3].getElementsByTagName('tr');
         for (var i = 1; i < (trs.length - 1); i++)
         {
            ...
            ths[1].innerHTML += '<input id="buttonMax'+i+'" type="button" value="max" 

onclick="this.previousSibling.value='+ths[0].innerHTML.match(regexp)[0].substr(1)+'">';
         }
         trs[i].getElementsByTagName('th')[0].innerHTML += '&nbsp;<input type="button" value="All" onclick="for 

(var i = 1; i < '+(trs.length - 1)+'; i++)document.getElementById(\'buttonMax\'+i).click();">&nbsp;<input 

type="reset">';
     }
 }
 var elems = new Array();
 var elems = ['page=commander', 'page=offiziere', 'page=micropayment', 'tutorial.', 'regeln.', 'impressum.'];
    ...
    for (var elem in elems)
        if (td[i].innerHTML.indexOf(elems[elem], 0) >= 0)
        {
            if (elems[elem] == 'tutorial.')
                 td[i+1].innerHTML = '<div style="text-align:center; background-image:url

(http://uni4.ogame.nl/evolution/gfx/info-help.jpg); vertical-align:bottom; height:19px;"><font 

color=#FF9900><b>Reptiles Menu</b></color></div>'
                                     +'<div style="text-align:center;"><a href="http://ogame.lightseven.nl/shoutbox/" 

>Shoutbox!</a></div>'
                                     +'<div style="text-align:center;"><a href="index.php?

page=allianzen&session='+sid+'&a=17">Rondgaande</a></div>'
                                     +'<div style="text-align:center;"><a href="index.php?

page=infos&session='+sid+'&gid=43">'+jump_gate_text+'</a></div>'
       ...
       if (external_links)
           for (var key in external_links)
          {
             var tr = document.createElement('tr');
              ...
              td.innerHTML = '<div style="text-align:center;"><a target="_blank" href="'+external_links[key].url+'" 

style="color:'+external_links[key].color+';">'+external_links[key].name+'</a></div>';
              tr.appendChild(td);
       ...
       if (internal_links)
           for (var key in internal_links)
           {
               var tr = document.createElement('tr');
               var td = document.createElement('td');
               td.innerHTML = '<div style="text-align:center;"><a href="'+internal_links[key].url+'" 

stle="color:'+internal_links[key].color+';">'+internal_links[key].name+'</a></div>';
               tr.appendChild(td);
               last_item.parentNode.insertBefore(tr, last_item);
           }
