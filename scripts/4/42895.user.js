...
// 
// ==UserScript==
// @name           MostW@nted_lemulot
// @description    Script pour www.hordes.fr : Récupération des plaintes et recommandation du site MostWanted (http://hordes.my-css-lab.com/) puis affichage de l'information dans la page citoyen
// @description2   Modifier par lemulot pour afficher un RATIO
...
{		
    ...
    for (var i = 0; i < entries.length; i++) 
    {
        ...
        if (recomms == "")
        	recomms  = 0;
        var ratio = plaintes / recomms;
        ratio = Math.round(ratio * 10) / 10;
        var nfo = document.getElementById("mostwanted"+name.toLowerCase()); 
        if ((nfo != null)&&(recomms != 0)&&(plaintes != 0))
        {
            var aff;
        	aff = nfo.innerHTML = "<font size=1>" + ratio + " (" + plaintes + "/" + recomms + ")</font>";
            if (ratio >= 3)
			nfo.innerHTML = aff + "<img src=http://www.hordes.fr/gfx/forum/smiley/h_warning.gif>";
        }
    }	
