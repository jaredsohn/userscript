...
// @namespace      quanganhdo.rapidshare.multiacc
			...
			if (account.split(':')[2] == 'true') {
				...
				GM_xmlhttpRequest({
					...
					onload: function(r) {
						if (/klapptitel/.test(r.responseText)) { // valid
							...
							$('#p_' + i).html(r.responseText.match(/<td>RapidPoints:<\/td><td align=right style="padding-right:20px;"><b><span id="rpo">([^<]*)<\/span><\/b>/)[1]);
				...
				GM_xmlhttpRequest({
					...
					onload: function(r) {
						if (/klapptitel/.test(r.responseText)) { // valid
							$('#e_' + i).html('N/A');
							$('#p_' + i).html(r.responseText.match(/<td>RapidPoints:<\/td><td align=right style="padding-right:30px;"><b><span id="rpo">([^<]*)<\/span><\/b>/)[1]);
							$('#t_' + i).html('N/A');
