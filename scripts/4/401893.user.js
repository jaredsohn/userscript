// ==UserScript==
// @name        Furbase Enhancement Suite
// @namespace   Furbase
// @description Diverse kleine Funktionen für Furbase - vom Klaue
// @include     https://forum.furbase.de/*
// @include     http://forum.furbase.de/*
// @version     1
// @grant       GM_getValue
// @grant       GM_setValue
// @grant       GM_deleteValue
// ==/UserScript==
// ugly merged file because userscripts.org does not support  multiple files..
 ...
 * @constructor
 * @this {Tag}
 * @param {String} userId The user id whose tag this is
 * @param {string} text The text of the tag
 * @param {string} fg The foreground color as a css color tag
 * @param {string} bg The background color as a css color tag
 * @param {string} url The url info (optional)
 * @param {string} id A unique identificator
 */
function Tag(userId, text, fg, bg, url, id) {
	this.userId = userId;
	this.text = text.trim();
	if (!fg) {
		this.foregroundColor = "#000000";
	} else {
	    this.foregroundColor = fg.trim();
	}
	if (!bg) {
	    this.backgroundColor = "transparent";
	} else {
	    this.backgroundColor = bg.trim();
	}
	if (!url) {
	    this.url = "";
	} else {
...
/**
 * Transforms the tag to a String containing all data in an array as a JSON-string.
 *
 * @this {Tag}
 * @return {string} The JSONified current tag
 */
Tag.prototype.toString = function() {
	// build an array out of all text data and transform to JSON. simplest way without having to bother with escaping (JSON does it allready)
	var tags = new Array(this.userId, this.text, this.foregroundColor, this.backgroundColor, this.url, this.id);
	return JSON.stringify(tags);
}

/**
 * Saves this Tag to the greasemonkey database for the given user. Tags identical (text, bg, fg) to allready existing ones are omitted.
 *
 * @this {Tag}
 */
Tag.prototype.saveTag = function() {
    var oldTags = Tag.getTagsForUser(this.userId);
	var stringifiedArray = new Array();

	var arrayLength = oldTags.length;
	for (var i = 0; i < arrayLength; i++) {
    	var tag = oldTags[i];
		if (tag.text == this.text && tag.foregroundColor == this.foregroundColor && tag.backgroundColor == this.backgroundColor) {
			// do not save this tag
			return;
		}
		stringifiedArray[i] = tag.toString();
	}
	stringifiedArray.push(this.toString());
	
	var saveString = JSON.stringify(stringifiedArray);
	GM_setValue('FES_tags_' + this.userId, saveString);
};

/**
 * Removes this Tag from the Greasemonkey database
 *
 * @this {Tag}
 * @param {String} userid The ID of the user
 * @return {number} the number of tags still set for this user
 */
Tag.prototype.deleteCurrentTag = function() {
	return Tag.deleteTag(this.userId, this);
}

/**
 * Adds the current tag to the post
 *
 * @this {Tag}
 * @param {ImportantHTMLElements} importantHTMLElements The important elements of the post this tag should be added to
 * @param {Object} importantHTMLElementsMap All importantHtmlElements (used for the tag-delete-link)
 */
Tag.prototype.addToPost = function(importantHTMLElements, importantHTMLElementsMap) {
	Tag.addTagToPost(importantHTMLElements, this, importantHTMLElementsMap);
}

/**
 * Generates a Tag object from the given String (only works with strings built from toString())
 *
 * @static
 * @return {Tag} A tag object
 */
Tag.fromString = function(string) {
	var tags = JSON.parse(string);
	return new Tag(tags[0], tags[1], tags[2], tags[3], tags[4], tags[5]);
}

/**
 * Gets all Tag objects for the given user from the greasemonkey database
 *
 * @static
 * @param {String} userid The ID of the user
 * @return {Array} An array of Tag objects
 */
Tag.getTagsForUser = function(userid) {
	var tagObjectArray = new Array();
	var tagsArrayString = GM_getValue('FES_tags_' + userid);
	if (!tagsArrayString || /^\s*$/.test(tagsArrayString)) return tagObjectArray; // empty array
	var tagsArray = JSON.parse(tagsArrayString);

	var arrayLength = tagsArray.length;
	for (var i = 0; i < arrayLength; i++) {
    	tagObjectArray[i] = Tag.fromString(tagsArray[i]);
	}
	return tagObjectArray;
}

/**
 * Removes the tag with the given id from the Greasemonkey database
 *
 * @static
 * @param {String} userid The ID of the user
 * @param {mixed} itemToDelete The ID of the tag or the tag object itself to remove
 * @return {number} the number of tags still set for this user
 */
Tag.deleteTag = function(userid, itemToDelete) {
	var id;
	if (itemToDelete instanceof Tag) {
		id = itemToDelete.id;
	} else {
		id = itemToDelete;
	}

	var oldTags = Tag.getTagsForUser(userid);
	var stringifiedArray = new Array();
	...
	for (var i = 0; i < arrayLength; i++) {
		...
		stringifiedArray.push(tag.toString());
	}
	
	if (stringifiedArray.length == 0) {
		GM_deleteValue('FES_tags_' + userid);
	} else {
		var saveString = JSON.stringify(stringifiedArray);
		GM_setValue('FES_tags_' + userid, saveString);
	}
	return stringifiedArray.length;
}

/**
 * Adds the given tag to the html
 *
 * @static
 * @param {ImportantHTMLElements} importantHTMLElements The important elements of the post the tag should be added to
 * @param {Tag} tag The tag to add
 * @param {Object} importantHTMLElementsMap All importantHtmlElements (used for the tag-delete-link)
 */
Tag.addTagToPost = function(importantHTMLElements, tag, importantHTMLElementsMap) {
	var tagP = document.createElement('p');
	tagP.style.color = tag.foregroundColor;
	tagP.style.backgroundColor = tag.backgroundColor;
	var textNode = document.createTextNode(tag.text + ' '); // will encode string value

	if (/^\s*$/.test(tag.url)) { // url is whitespace
		tagP.appendChild(textNode);
	} else {
		//url is set, make link
		var tagLink = document.createElement('a');
		tagLink.appendChild(textNode);
		tagLink.href = tag.url;
		tagLink.style.color = tag.foregroundColor;
	...
	delLink.onclick = function(_tag, _importantHTMLElementsMap) {
		return function() {
			Tag.removeTagFromHTML(_tag, _importantHTMLElementsMap);
		}
	}(tag, importantHTMLElementsMap);
	delLink.style.color = "red";

	tagP.appendChild(delLink);

	importantHTMLElements.allTagsDiv.appendChild(tagP);
	importantHTMLElements.addTagP(tag.id, tagP);
}

/**
 * Adds all saved tags to the current page
 *
 * @static
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
Tag.addTagsToHtml = function(importantHTMLElementsMap) {
	var ignoredUserStr = GM_getValue('ignoredUsers');
	if (!ignoredUserStr) ignoredUserStr = "";
	var ignoredUsers = ignoredUserStr.split(";");
	
	for(var userId in importantHTMLElementsMap) {
		...
		var importantHTMLElementsArray = importantHTMLElementsMap[userId];
		
		var importantHTMLElementsArrayLength = importantHTMLElementsArray.length;
		for (var i = 0; i < importantHTMLElementsArrayLength; i++) {
			var importantHTMLElements = importantHTMLElementsArray[i];

			// add tags to div
			var tagTitleP = document.createElement('p');
			var tagTitle = document.createElement('b');
			tagTitle.textContent = "Tags:";
			tagTitleP.appendChild(tagTitle);
			importantHTMLElements.authorDiv.appendChild(tagTitleP);

			importantHTMLElements.allTagsDiv = document.createElement('div');
			importantHTMLElements.emptyTagId = "FES_empty" + _emptyTagId++;

			var tags = Tag.getTagsForUser(userId);
			if (tags.length == 0) {
				var tagP = document.createElement('p');
				tagP.id = importantHTMLElements.emptyTagId;
				tagP.appendChild(document.createTextNode("<Noch keine>"));
				importantHTMLElements.allTagsDiv.appendChild(tagP);
			} else {
				var tagsLength = tags.length;
				for (var j = 0; j < tagsLength; j++) {
					tags[j].addToPost(importantHTMLElements, importantHTMLElementsMap);
				}
			}

			importantHTMLElements.authorDiv.appendChild(importantHTMLElements.allTagsDiv);

			var spacer = document.createElement('p');
			spacer.innerHTML = "&nbsp";
			importantHTMLElements.authorDiv.appendChild(spacer); // spacer

			var addLink = document.createElement('a');
			var linkText = document.createTextNode("Tag hinzufügen");
			addLink.appendChild(linkText);
			addLink.href = "javascript:void(0)";
			//http://stackoverflow.com/questions/1451009/javascript-infamous-loop-issue
			addLink.onclick = function(_userId, _importantHTMLElementsMap) {
				return 	function() {
					Tag.constructNewTag(_userId, _importantHTMLElementsMap);
				};
			}(userId, importantHTMLElementsMap);

			importantHTMLElements.authorDiv.appendChild(addLink);
		}
	}
}

/**
 * Removes the tag from the whole html (all posts). if the tag was the last one, an empty tag placeholder is generated.
 *
 * @static
 * @param {Tag} tag The tag to remove
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
Tag.removeTagFromHTML = function(tag, importantHTMLElementsMap) {
	// get all elements from that user
	var importantHTMLElementsArray = importantHTMLElementsMap[tag.userId];
	if (importantHTMLElementsArray.length == 0) return;

	// delete tag from DB first in case something goes wrong below
	var tagsLeft = tag.deleteCurrentTag();

	// loop through htmlelements
	var importantHTMLElementsArrayLength = importantHTMLElementsArray.length;
	for (var i = 0; i < importantHTMLElementsArrayLength; i++) {
		var tagElem = importantHTMLElementsArray[i].getTagP(tag.id);
		if (!tagElem) {
			// if the first post of the user doesn't have that tag, none does
			break;
		}

		importantHTMLElementsArray[i].allTagsDiv.removeChild(tagElem);
		importantHTMLElementsArray[i].removeTagP(tag.id);

		if (tagsLeft == 0) {
			// make a new empty tag
			var tagP = document.createElement('p');
			tagP.id = importantHTMLElementsArray[i].emptyTagId; // this ID should always be set, so we can reuse it without making a new one
			tagP.appendChild(document.createTextNode("<Noch keine>"));
			importantHTMLElementsArray[i].allTagsDiv.appendChild(tagP);
		}
	}
...
/**
 * Generates a dialog for creating a new tag
 *
 * @static
 * @param {string} userId The user the new tag is for
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
Tag.constructNewTag = function(userId, importantHTMLElementsMap) {
	if (!(userId in importantHTMLElementsMap)) return;

	// make input dialog
	// firstly, a div that hides everything
	var overlay = document.createElement('div');
	overlay.id = "FES_dialog_overlay";
	overlay.style.backgroundColor = "rgba(0, 0, 90, 0.5)";
	...
	// add a copy of our data to that node to preserve it for the callback
	overlay.FES_userId = userId;
	overlay.FES_importantHTMLElementsMap = importantHTMLElementsMap;
	...
	var dialogContent = _tagDialogHtml;
	dialogContent = dialogContent.replace("%userid%", userId);
	dialogContent = dialogContent.replace("%currenturl%", window.location.href);
	dialog.innerHTML = dialogContent;

	overlay.appendChild(dialog);
	document.body.appendChild(overlay);

	// add onclick events to dialog
	// IMPORTANT: can't do that inside the innerhtml. GM runs sandboxed and functions on the webpage cannot access functions in GM
	// if we added that functionality using innerHtml, it would be on the website and therefore not know our functions and classes
	var fgInput = document.getElementById("FES_dialog_foreground");
	var bgInput = document.getElementById("FES_dialog_background");
	var saveBtn = document.getElementById("FES_dialog_save");
	var abortBtn = document.getElementById("FES_dialog_abort");
	
	fgInput.onclick = function(e) {
		// removing of style in first color field is because colorPicker sets style=relative on the paragraph. is the upper picker called and the lower picker is style=relative,
		// the picker is drawn, for some reason, below the input field instead of above it. removing the style before opening the picker fixes that.
		document.getElementById("FES_dialog_bgcolorp").removeAttribute("style");
		colorPicker(e);
	};
	...
	saveBtn.onclick = function(e) {
		Tag.constructNewTag_dialogCallback(false);
	};

	abortBtn.onclick = function(e) {
		Tag.constructNewTag_dialogCallback(true);
	};
}

/**
 * Creates the tag from the dialog
 *
 * @static
 * @param {boolean} abort True if abort button was hit
 */
Tag.constructNewTag_dialogCallback = function(abort) {
	var dialog = document.getElementById("FES_dialog_overlay");
	if (abort) {
		document.body.removeChild(dialog);
		return;
	}

	// get tag values from form, dialog
	var userId = dialog.FES_userId;
	var importantHTMLElementsMap = dialog.FES_importantHTMLElementsMap;
	var text = document.getElementById("FES_dialog_text").value;
	var fg = document.getElementById("FES_dialog_foreground").value;
	var bg = document.getElementById("FES_dialog_background").value;
	var url = document.getElementById("FES_dialog_url").value;

	// check tag values
	if (!userId || !importantHTMLElementsMap) {
		alert("interner fehler von Furbase Enhancement Suite, Tag kann nicht gespeichert werden. wtf.");
		document.body.removeChild(dialog);
		return;
	}
	if (!text || /^\s*$/.test(text)) {
		alert("Du hast den Tag-Text vergessen");
		return;
	}
	if (!fg || !(/^#[0-9A-F]{6}$/i.test(fg))) {
		fg = "black";
	}
	if (!bg || !(/^#[0-9A-F]{6}$/i.test(bg))) {
		bg = "transparent";
	}
	if (!url || /^\s*$/.test(url)) { // also gets whitespace
		url = "";
	}

	// we have all we need, close dialog
	document.body.removeChild(dialog);
	

	var newTag = new Tag(userId, text, fg, bg, url);
	newTag.saveTag();

	// remove "no tag" element if present
	var importantHTMLElementsArray = importantHTMLElementsMap[userId];
	var importantHTMLElementsArrayLength = importantHTMLElementsArray.length;
	for (var i = 0; i < importantHTMLElementsArrayLength; i++) {
		var emptyElem = document.getElementById(importantHTMLElementsArray[i].emptyTagId);
		if (!emptyElem) {
			// if the first post of the user has no empty elem, none has, so we can abort
			break;
		}
		importantHTMLElementsArray[i].allTagsDiv.removeChild(emptyElem);
	}

	// add tags
	for (var i = 0; i < importantHTMLElementsArrayLength; i++) {
		newTag.addToPost(importantHTMLElementsArray[i], importantHTMLElementsMap);
	}
}

/**
 * Goes through the HTML and removes all posts by ignored users
 *
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
var removeIgnoredUsers = function(importantHTMLElementsMap){
	var ignoredUserStr = GM_getValue('ignoredUsers');
	if (!ignoredUserStr || /^\s*$/.test(ignoredUserStr)) return;
	var ignoredUsers = ignoredUserStr.split(";");

	var ignoredUsersLength = ignoredUsers.length;
	for (var i = 0; i < ignoredUsersLength; i++) {
		removeIgnoredUser(ignoredUsers[i], importantHTMLElementsMap);
	}
}

/**
 * Goes through the HTML and removes all posts by the given user
 *
 * @param {String} userId The id of the user whose posts will be removed
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
var removeIgnoredUser = function(userId, importantHTMLElementsMap) {
	if (!(userId in importantHTMLElementsMap)) return; // no post of this user
	var importantHTMLElementsArray = importantHTMLElementsMap[userId];

	var importantHTMLElementsArrayLength = importantHTMLElementsArray.length;
	for (var j = 0; j < importantHTMLElementsArrayLength; j++) {
		var importantHTMLElements = importantHTMLElementsArray[j];

		// clean author div - remove everything but name
		while (importantHTMLElements.authorDiv.hasChildNodes()) {
			if (importantHTMLElements.authorDiv.lastChild.className == "userName") break;
			importantHTMLElements.authorDiv.removeChild(importantHTMLElements.authorDiv.lastChild);
		}

		// remove all other entries from sidebar, like avatar
		var userAvatar = importantHTMLElements.sidebar.getElementsByClassName("userAvatar")[0]; // there can be.. ONLY ONE!
		var userSymbols = importantHTMLElements.sidebar.getElementsByClassName("userSymbols")[0]; // there can be.. ONLY ONE!
		var userCredits = importantHTMLElements.sidebar.getElementsByClassName("userCredits")[0]; // there can be.. ONLY ONE!
		var userMessenger = importantHTMLElements.sidebar.getElementsByClassName("userMessenger")[0]; // there can be.. ONLY ONE!
		if (userAvatar) importantHTMLElements.sidebar.removeChild(userAvatar);
		if (userSymbols) importantHTMLElements.sidebar.removeChild(userSymbols);
		if (userCredits) importantHTMLElements.sidebar.removeChild(userCredits);
		if (userMessenger) importantHTMLElements.sidebar.removeChild(userMessenger);

		// remove whole post
		while (importantHTMLElements.contentArea.hasChildNodes()) {
			importantHTMLElements.contentArea.removeChild(importantHTMLElements.contentArea.lastChild);
		}

		// add deleted notice
		var delP = document.createElement('p');
		delP.style.color = "red";
		var textNode = document.createTextNode("Post entfernt von Furbase Enhancement Suite - Benutzer Ignoriert. ");
		delP.appendChild(textNode);

		var spacer = document.createElement('p');
		spacer.innerHTML = "&nbsp";
		delP.appendChild(spacer); // make room for allow-link in sidebar.. div resizing, how does it work?

		importantHTMLElements.contentArea.appendChild(delP);
	}
}

/**
 * Adds links to the sidebar of all users, either "block this user" if user is not blocked, or "re-allow this user" if he is
 *
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
var addBlockLinks = function(importantHTMLElementsMap) {
	var ignoredUserStr = GM_getValue('ignoredUsers');	
	if (!ignoredUserStr) ignoredUserStr = "";
	var ignoredUsers = ignoredUserStr.split(";");
	...
	for(var userId in importantHTMLElementsMap) {
		...
		addBlockLinksForUser(importantHTMLElementsMap, userId, ignored);
	}
}

/**
 * Adds a link to the sidebar of the given user (all posts), either "block this user" if ignored=false, or "re-allow this user" if ignored=true
 *
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 * @param {String} userId The id of the user whose posts' sidebars will have the links added
 * @param {boolean} ignored True if the link should be a re-allow one, false otherwise
 */
var addBlockLinksForUser = function(importantHTMLElementsMap, userId, ignored) {
	var importantHTMLElementsArray = importantHTMLElementsMap[userId];
	var importantHTMLElementsArrayLength = importantHTMLElementsArray.length;
	for (var i = 0; i < importantHTMLElementsArrayLength; i++) {
		var importantHTMLElements = importantHTMLElementsArray[i];

		// add block/allow link
		var link = document.createElement('a');
		if (ignored) {
			var linkText = document.createTextNode("Wieder zuslassen");
			link.appendChild(linkText);
		} else {
			var linkText = document.createTextNode("Benutzer ignorieren");
			link.appendChild(linkText);
		}
		link.href = "javascript:void(0)";
		//http://stackoverflow.com/questions/1451009/javascript-infamous-loop-issue
		link.onclick = function(_userId, _importantHTMLElementsMap, isIgnored) {
			return function() {
				if (isIgnored) {
					allowUser(_userId);
				} else {
					ignoreUser(_userId, _importantHTMLElementsMap);
				}
			}
		}(userId, importantHTMLElementsMap, ignored);
		if (ignored) {
		...
		importantHTMLElements.authorDiv.appendChild(container);
	}
}

/**
 * Ignores the given user (adds to ignore list and removes posts)
 *
 * @param {String} userId The id of the user to ignore
 * @param {Object} importantHTMLElementsMap Associative Array with mapping userId/array of ImportantHTMLElements (Posts). Should contain all post divs of all users. importantHTMLElementsMap[userId] = [ImportantHTMLElements, ImportantHTMLElements, ...]
 */
var ignoreUser = function(userId, importantHTMLElementsMap) {
	// if this fnc is called, the user is guaranteed to not be ignored, so we don't have to check that
	var ignoredUserStr = GM_getValue('ignoredUsers');
	
	if (!ignoredUserStr || /^\s*$/.test(ignoredUserStr)) { // empty or null
		GM_setValue('ignoredUsers', "" + userId);
	} else {
		GM_setValue('ignoredUsers', ignoredUserStr + ";" + userId);
	}
	
	removeIgnoredUser(userId, importantHTMLElementsMap); // will also remove block link
	addBlockLinksForUser(importantHTMLElementsMap, userId, true); // will add allow link
}

/**
 * Un-ignores the given user (removes from ignore list, to re-show the posts a reload is in order)
 *
 * @param {String} userId The id of the user to ignore
 */
var allowUser = function(userId) {
	var ignoredUserStr = GM_getValue('ignoredUsers');
	if (!ignoredUserStr || /^\s*$/.test(ignoredUserStr)) ignoredUserStr=userId; // that should never happen. setting it to the given user is just an attempt to collect the shards
	var ignoredUsers = ignoredUserStr.split(";");
	
	position = ignoredUsers.indexOf(userId);
	if ( ~position ) ignoredUsers.splice(position, 1);

	GM_setValue('ignoredUsers', ignoredUsers.join(";"));

	alert("Neu laden, um Posts des Users wieder anzuzeigen");
}
...
var _tagDialogHtml =
'<h2>Neuer Tag f&uuml;r Benutzer Nr. %userid%</h2>												\
<form>																							\
    <p>																							\
        Tag text:<br>																			\
        <input type="text" id="FES_dialog_text" />												\
    </p>																						\
    <br>																						\
    <p>																							\
        Textfarbe (leer für Schwarz):<br>														\
        <input type="text" id="FES_dialog_foreground" /> 										\
    </p>																						\
    <br>																						\
    <p id="FES_dialog_bgcolorp">																\
        Hintergrundfarbe (leer für Transparent):<br>											\
        <input type="text" id="FES_dialog_background" />										\
    </p>																						\
    <br>																						\
    <p>																							\
        Link zum Tag (optional):<br>															\
        <input type="text" id="FES_dialog_url" value="%currenturl%" style="width: 300px"/>		\
    </p>																						\
</form>																							\
<br>																							\
<button type="button" id="FES_dialog_save">Speichern</button>&nbsp;&nbsp;&nbsp;					\
<button type="button" id="FES_dialog_abort">Abbrechen</button>';
...
// looks nicer but the color picker is positioned as if x-index of the fields was 0 for some reason
/*var _tagDialogHtml =
'<h2>Neuer Tag f&uuml;r Benutzer Nr. %userid%</h2>												\
<form>																							\
    <table>																						\
        <tr>																					\
            <td>Tag text:</td>																	\
            <td><input type="text" id="FES_dialog_text" /></td>										\
        </tr>																					\
        <tr>																					\
            <td>Textfarbe:</td>																	\
            <td><input type="text" onClick="colorPicker(event)" id="FES_dialog_foreground" /></td>		\
        </tr>																					\
        <tr>																					\
            <td>Hintergrundfarbe (leer für Transparent):</td>									\
            <td><input type="text" onClick="colorPicker(event)" id="FES_dialog_background" /></td>		\
        </tr>																					\
        <tr>																					\
            <td>Link zum Tag (optional):</td>													\
            <td><input type="text" id="FES_dialog_url" value="%currenturl%" /></td>					\
        </tr>																					\
    </table>																					\
</form>																							\
<a href="javascript:void(0)" onclick="function(){%savefnc%;}">Speichern</a>&nbsp;&nbsp;&nbsp;	\
<a href="javascript:void(0)" onclick="function() {%abortfnc%;}">Abbrechen</a>';*/

/**
 * Creates an instance of TextfieldBackup.
 * It is the responsibility of the caller that tinyMCE is initialized if the textField is an TinyMCE editor field. Use TextfieldBackup.waitForTinyMCE() for that.
 *
 * @constructor
 * @this {TextfieldBackup}
 * @param {mixed} textField The textField to save, either HTMLElement or TinyMCE-Editor
 * @param {HTMLElement} divForButton The div to add the button to
 */
function TextfieldBackup(textField, divForButton) {
	this.textField = textField;
	this.buttonDiv = divForButton;

	this.isTinyMCE = ("theTextarea" in textField);
	
	this.ignoreValue = null; // leave this at null so we know below that it's the first run of save()
}

/**
 * Waits for the instance of TinyMCE to be loaded. Warning: waits endlessly, so only call on pages on which TinyMCE should be running
 *
 * @static
 * @param {method} callback The method to call once TinyMCE is available and intialized
 */
TextfieldBackup.waitForTinyMCE = function(callback) {
	if (typeof(unsafeWindow.tinyMCE) != 'undefined' && unsafeWindow.tinyMCE.initialized) {
		callback();
	} else {
		window.setTimeout(function() {
			TextfieldBackup.waitForTinyMCE(callback);
		}, 500);
	}
}

/**
 * Gets the bbcode from the text field
 *
 * @this {TextfieldBackup}
 * @return {string} The bbcode as string (empty string if null or whitespace only)
 */
TextfieldBackup.prototype.getBBCode = function() {
	var bbCode;
	if (this.isTinyMCE) {
		if (this.textField.editorIsActive) {
			// wysiwyg
			// convert to bbcode for saving
			bbCode = this.textField.getBody().innerHTML;
			bbCode = unsafeWindow.tinyMCE.htmlToBBCode(bbCode);
		} else {
			// sourcecode
			bbCode = this.textField.theTextarea.value;
		}
	} else {
		if (!this.isVisible(this.textField)) return "";
		bbCode = this.textField.value;
	}
	if (!bbCode || /^\s*$/.test(bbCode)) {
		// do not save while area is still empty, or we never have a chance to load
		return "";
	}
	return bbCode;
}
...
/**
 * Sets the bbcode to the text field (replacing existing)
 *
 * @this {TextfieldBackup}
 * @param {string} text The bbcode to set
 */
TextfieldBackup.prototype.setBBCode = function(text) {
	...
	if (this.isTinyMCE) {
		...
		if (this.textField.editorIsActive) {
			// wysiwyg
			// convert from bbcode
			var html = unsafeWindow.tinyMCE.bbCodeToHTML(text, true);
			this.textField.getBody().innerHTML = html;
		} else {
			// sourcecode
			this.textField.theTextarea.value = text
		}
	} else {
		if (!this.isVisible(this.textField)) return;
...
/**
 * Attaches the restore button to this object's button div
 *
 * @this {TextfieldBackup}
 */
TextfieldBackup.prototype.attachButton = function() {
	var button = document.createElement('button');
	button.type = "button";
	button.onclick = function(textfieldBackup) {
		return function() {
			textfieldBackup.restore();
		}
	}(this);

	var buttonText = document.createTextNode("FES - Text wiederherstellen");
	button.appendChild(buttonText);

	// add it to the top of the container, above the editor
	this.buttonDiv.insertBefore(button, this.buttonDiv.firstChild);
}

/**
 * Attaches backup thread to the given text field and adds a restore-button to the start of the given div
 *
 * @this {TextfieldBackup}
 */
TextfieldBackup.prototype.attach = function() {
	// add restore button
	this.attachButton();

	// save one time all two seconds not onkeyup
	// reason: performance. also gets other way of putting in text (like drag'n'drop, linux copy-paste etc) that onkeyup wouldn't
	//http://stackoverflow.com/questions/1451009/javascript-infamous-loop-issue
	setInterval(function(textfieldBackup) {
			return function() {
				textfieldBackup.save();
			}
		}(this),2000);
}

/**
 * Saver-thread, called all 2 secs or so
 *
 * @this {TextfieldBackup}
 */
TextfieldBackup.prototype.save = function() {
	var value = this.getBBCode();
	
	// check if ignoreValue was set. if so, check for changes (don't save if none)
	// if not, set ignoreValue and don't save (reason is that whatever was in the text field when the user opened it should not be saved or else restoring would be impossible)
	if (this.ignoreValue == null) {
		// first time save is called. set ignoreValue and exit
		this.ignoreValue = value;
	} else {
		// not the first time. check if there were any changes and if yes, save
		if (value != this.ignoreValue) {
			GM_setValue("FES_textBackup", value);
			
			// set current value to ignore value. No sense in saving if user didn't change a thing
			this.ignoreValue = value;
		}
	}
}
...
/**
 * Restores the text field
 *
 * @this {TextfieldBackup}
 */
TextfieldBackup.prototype.restore = function() {
	var text = GM_getValue("FES_textBackup");
	this.setBBCode(text);
}

// http://snippets.jc21.com/snippets/javascript/check-if-an-element-is-visible-to-the-user/ no idea how it works, but it does
TextfieldBackup.prototype.isVisible = function(e) {
...
if (window.top != window.self) { //don't run on frames or iframes (like tinyMCE-Wysiwyg-editor-window)
    return;
}

/**
 * Creates an instance of ImportantHTMLElements, a simple data store for html elements of a single post
 *
 * @constructor
 * @this {ImportantHTMLElements}
 * @param {String} userId The user
 * @param {HTMLElement} messageDiv the element of the whole post
 * @param {HTMLElement} contentArea The text area of the post, the "real" post
 * @param {HTMLElement} sidebar The sidebar of the posts (with username, avatar etc)
 * @param {HTMLElement} authorDiv a div that contains user name, title and rank
 * @param {HTMLElement} allTagsDiv The div with all the tags in it
 * @param {HTMLElement} emptyTagId The ID of the empty-tag-field (post may have none)
 */
function ImportantHTMLElements(userId, messageDiv, contentArea, sidebar, authorDiv, allTagsDiv, emptyTagId) {
	this.userId = userId;
	this.messageDiv = messageDiv;
	this.contentArea = contentArea;
	this.sidebar = sidebar;
	this.authorDiv = authorDiv;
	this.allTagsDiv = allTagsDiv;
	this.emptyTagId = emptyTagId;

	this.tagPArray = new Object(); // associative array for tag id/htmlelement
}

/**
 * Adds a tag paragraph
 *
 * @this {ImportantHTMLElements}
 * @param {String} id The ID of the tag
 * @param {HTMLElement} tagP The tag paragraph
 */
ImportantHTMLElements.prototype.addTagP = function(id, tagP) {
	this.tagPArray[id] = tagP;
}

/**
 * Removes a tag paragraph (does not remove from HTML!)
 *
 * @this {ImportantHTMLElements}
 * @param {String} id The ID of the tag
 */
ImportantHTMLElements.prototype.removeTagP = function(id) {
	delete this.tagPArray[id];
}

/**
 * Checks if this ImportantHTMLElements object has the goven id
 *
 * @this {ImportantHTMLElements}
 * @param {String} id The ID of the tag
 * @return {Boolean} true if this contains the id
 */
ImportantHTMLElements.prototype.hasTagP = function(id) {
	var strId = id + '';
	return (strId in this.tagPArray);
}
 ...
 * @this {ImportantHTMLElements}
 * @param {String} id The ID of the tag
 * @return {HTMLElement} the element of that tag
 */
ImportantHTMLElements.prototype.getTagP = function(id) {
	return this.tagPArray[id];
}

var importantHTMLElementsMap = new Object(); // associative array for user id/list of ImportantHTMLElements for posts

// Greasemonkey runs at the DOMContentLoaded event by default. This means that everything will be in the page except for possibly large
// images and stuff added by some javascripts so we have to wait for nuthin. Let's do this! LEEEEROY JEEEEEENKINS!!!!
// handle forum page only
if (window.location.href.indexOf("page=Thread") !== -1) {
	// get all the html elements we need for ignoring and tagging
	var posts = document.getElementsByClassName("message");
	var postsLength = posts.length;
	for (var i = 0; i < postsLength; i++) {
		if (!posts[i].hasChildNodes()) {
			// real posts have always exactly one child
			continue;
		}
	
		// post text area
		var contentArea = posts[i].getElementsByClassName("messageContentInner")[0]; // there can be.. ONLY ONE!
		if (!contentArea) continue;

		// author data/avatar area
		var sidebar = posts[i].getElementsByClassName("messageSidebar")[0]; // there can be.. ONLY ONE!
		if (!sidebar) continue;

		// name area with user name, title and rank
		var authorDiv = sidebar.getElementsByClassName("messageAuthor")[0]; // there can be.. ONLY ONE!
		if (!authorDiv) continue;

		// parse link to user profile in user name to get user id
		var userId;
		var userNameP = authorDiv.getElementsByClassName("userName")[0]; // there can be.. ONLY ONE!
		if (!userNameP) continue;
		var link = userNameP.getElementsByTagName('a')[0];
		if (!link) continue;
		var linkStr = link.href;
		if (linkStr.indexOf("=") == -1) continue;
		userId = linkStr.substring(linkStr.lastIndexOf("=") + 1, linkStr.length);
		if (!userId) continue; // should not happen

		var currentPostData = new ImportantHTMLElements(userId, posts[i], contentArea, sidebar, authorDiv);
		if (userId in importantHTMLElementsMap) { // user id allready exists
			importantHTMLElementsMap[userId].push(currentPostData);
		} else {
			importantHTMLElementsMap[userId] = new Array(currentPostData);
		}
	}

	removeIgnoredUsers(importantHTMLElementsMap);
	addBlockLinks(importantHTMLElementsMap);

	Tag.addTagsToHtml(importantHTMLElementsMap);

	// add backup tool to quick answer text area
	var textField = document.getElementById("text");
	var buttonDiv = textField.parentNode;
	var backupper = new TextfieldBackup(textField, buttonDiv);
	backupper.attach();

} else if (window.location.href.indexOf("form=PostQuickAdd") !== -1 || window.location.href.indexOf("form=PostAdd") !== -1 || window.location.href.indexOf("form=ThreadAdd") !== -1
		|| window.location.href.indexOf("form=PostEdit") !== -1 || window.location.href.indexOf("form=PMNew") !== -1) {
	// post write dialogs with tinymce
	TextfieldBackup.waitForTinyMCE(function() {
			var buttonDiv = document.getElementById("textDiv");
			var editor = unsafeWindow.tinyMCE.getInstanceById('mce_editor_0');
			var backupper = new TextfieldBackup(editor, buttonDiv);
			backupper.attach();
		});
}
