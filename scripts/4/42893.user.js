// ==UserScript==
// @name           سكريبت الهجمات المتتالية تعريب ALNONO
// @author         FuJis & Mars
// @description    سكريبت الهجمات المتتالية تعريب ALNONO
// @include        http://*.travian.*/* 
// @exclude        http://forum.travian.*
// @معرب السكريبت        ALNONO
// @version        V 1.7
// ==/UserScript==
...
function voegToe() {
  ...
  t10 = parseInt(document.getElementById('t10').value)
	...
	alert('تمت الاضافة: '+dorpnaam+"("+x+"|"+y+")"); //opslaan moet nog gemaakt worden. afhankelijk van coordinatan van geselecteerd dorp -> eerst coordinaten ophalen.
};
...
function addForm() {
	...
	addButton.innerHTML = "» إضافة إلى الهجمات المتتالية";
...
function geefOverzicht() {
	...
	if(GM_getValue("valaan"+document.domain,0) == 0)	onoffButton.innerHTML = "تشغيل الهجمات المتتالية";	//http://s3.travian.lt/img/un/a/del.gif
	else onoffButton.innerHTML = "إيقاف الهجمات المتتالية";
	...
	ntd1.innerHTML = "<span class='c0'><a href='http://WWW.N3NO3.COM'>معرب السكريبت</a></span>";
...
function removeFarm(itemToRemove) {
	...
	if (itemToRemove < farms.length) {
		if (confirm('حذف: ' + farms[itemToRemove])) {
...
function valAan()
{
	...
	if(GM_getValue("valaan"+tekst,0) == 1)
	{
		...
		addButton.value = "السكريبت قيد التشغيل, إضغط هنا لإيقافه!";
...
startvalue = 1+ 10*prompt('ماهي نوع قبيلتك?: الرومان=0, الجرمان=1, الاغريق=2 ')
