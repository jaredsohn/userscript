...
//document.title='Nettbys profildesignere av Mujaff ©'
//}
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Ignorer") {
  links[i].innerHTML="ignorer denne borgeren";
  links[i].style.color="#ff0000"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Bli venn med") {
  ...
  links[i].style.color="#FFD700"; {
  }
 }
}
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="Skriv en hilsen") {
  links[i].innerHTML="skriv en hilsen i gjesteboka";
  links[i].style.color="#C71585"; {
  }
 }
}
for(i=0;i<links.length;i++) {
...
akt.innerHTML = akt.innerHTML + '<div id="Profiler du kansje liker"></div>';
}
}
//CALADORION ©
for (var c = 0; c < window.document.getElementsByTagName('div').length; c++){
...
anchor.appendChild( document.createTextNode( 'designere på nettby' ) );
...
function poeng(e){try{e.innerHTML=e.innerHTML.replace(/Poeng han eller henne har skaffet idag/,"Poeng du har i dag").replace(/(\d*)\/20\)/,"--0--)").replace(/Poeng totalt/,"alle poengene mine tilsammen").replace(/\(\d*\/2000\)/,"(2001/2000)").replace(/Bling-O-Meter:/,"BLING!!!!!! :");
...
{switch(d.innerHTML){case 'Din aktivitet':case 'Finn borger':case 'Inviter en venn til og bli med på nettby':case '<span>Sist pålogget </span>':case 'Sist påloggete i byen vår':case 'Informasjon':d.className="rtl";break;case 'Aktiviteten min':poeng(window.document.getElementsByTagName('div')[i+1]);
...
{if(fonts[i].innerHTML=="Profildesigner+Borgermester"){fonts[i].innerHTML='<img src="http://img227.imageshack.us/img227/5271/borjemestergfyn2.gif">';
fonts[i].style.color="#3c3c3c";break;}}

for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="mujaff") {
  links[i].innerHTML="lederen for nettbys profildesignere";
...
for(q=0;q<innom.length;q++){
	...
	if(innom[q].innerHTML=="Sist innom")
		{innom[q].innerHTML="Disse har vært på nettby før deg profildesigner:";
		break;
	}
}

//Setter status (Jeg er på nettby:p)
var imON=window.document.getElementsByTagName('div');
for(q=0;q<imON.length;q++){
	if(imON[q].innerHTML=="Pålogget")
		//{imON[q].innerHTML="<center><blink>Jeg er på nettby:p</blink></center>";
