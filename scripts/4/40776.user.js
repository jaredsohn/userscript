// ==UserScript==
// @name           Replay Pitbulls
// @description    Adds some new features to the replay page. Changes made for Kiev Pitbulls
// @namespace      pbr
...
var enableLowerDecalLayer = true;  //end zone images
var enableLineLayer = false;        //yardlines
var enableMiddleDecalLayer = true; //field graphics
var enableUpperDecalLayer = true;  //used for shadowing, only have retractable roof right now
    ...
    if (alternatePlayers == true) {
    	...
    	for (var i=0; i<players.length; i++) {
    		...
    		if (img.src.indexOf("ball.gif") != -1) {
        		//img.src = img.src.replace("http://goallineblitz.com/images/","http://i39.tinypic.com/10cug4w.gif");
    			z++;
    		}
...
function fieldGraphicsUpdate() {
	if (enableFieldLayer == true) { //1080x480
        var fields = [];
        fields[fields.length] = 'http://i41.tinypic.com/148qpk.jpg';
    ...
    if (enableLowerDecalLayer == true) {
		...
		if (d == null) {
			...
			img.src = "http://i44.tinypic.com/1zb6um8.jpg";
			...
			var img = document.createElement("img");
			img.src = "http://i40.tinypic.com/11hyu5g.jpg";
    ...
    if (enableMiddleDecalLayer == true) {
		...
		if (d == null) {
			...
			//50 yard line image 
			var imgysize = 199; //set to the y-axis length of your image
			var imgxsize = 173; //set to the x-axis length of your image
			var img = document.createElement("img");
			img.src = "http://i41.tinypic.com/34nr7rk.png";
    ...
    if (enableUpperDecalLayer == true) { //1080x480
		...
		if (d == null) {
			...
			var img = document.createElement("img");
            //img.src = "http://i40.tinypic.com/2uhbbqr.png";
...
function drawFrame() {
	...
	for (var i = 1; i < frame.length; i++) {
		var data = frame[i];
		drawPlayer(data.id, data.x, data.y);
	}
