// ==UserScript==
// @name	        C&CTA - Formation Saver-Loader
// @description     Extension for the "C&CTA - Offline Combat Simulator"
// @version         0.00
// @author          Vulcano
// @updateURL       http://-.vulcanion.com/FormationSaver.meta.js
// @include         https://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// ==/UserScript==

(function (){
  var oTACS_main = function() {
    var windowSaver;
      
    function initialize() {
      console.log("Formation Saver/Loader Loaded");

      qx.Class.define("webfrontend.gui.PlayArea.FomationSaverLoader", {
        ...
        members: {
          ...
          init: function() {          
            ...
            var labelTitle = new qx.ui.basic.Label("<b><a href="http://Vulcanion.com"oTACS</a></b>");
            labelTitle.set({marginLeft: 4, rich: true});
            labelTitle.setTextColor("#0093EF");
            ba.add(labelTitle);
          ...
          expand: function(bs) {
            if(!bs) {
              ...
              this.containerMain.setMaxHeight(webfrontend.gui.PlayArea.FomationSaverLoader.SaverCollapsedHeight);
            } else {
              this.buttonResize.setIcon("FactionUI/icons/icon_tracker_minimise.png");
              this.containerMain.setMaxHeight(webfrontend.gui.PlayArea.FomationSaverLoader.SaverExpandedHeight);
            }
          },
          ...
          update: function() {
            ...
            for(var id in formations[cityID][ownCityID]) {
              if(id != 0) {
                ...
                var toolTipText = "<div><span style='float: left'><b>" + formation.n + "</b></span><span style='float: right'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + date.getDate() + "/" + (date.getMonth() + 1) + "/" date.getYear()  + " " + date.getHours() + ":" + (date.getMinutes() <= 9 ? "0" : "") + date.getMinutes() + "</span></div><div style='clear: both;'></div>";
                ...
                labelDelete.setTextColor("#FF0000");
                labelDelete.addListener("click", this.clickDeleteF(cityID, ownCityID, id), this);
                labelDelete.addListener("mouseover", this.mouseover(labelDelete, "#B20505"), this);
                labelDelete.addListener("mouseout", this.mouseover(labelDelete, "#FF0000"), this);
          ...
          save: function() {
            try {
              ...
              if(armyUnits == null) {
                console.log("oTACS Error: You must move a unit befor saving!");
                return;
              }
          ...
          load: function(newFormation) {
            try {
              ...
              var armyUnits = formation.get_ArmyUnits();
              if(armyUnits == null) {
                console.log("oTACS Error: You must move a unit befor loading!");
                return;
              }
              armyUnits = armyUnits.l;
      ...
      windowSaver = new webfrontend.gui.PlayArea.FomationSaverLoader();
      windowSaver.hide();
      qx.core.Init.getApplication().getPlayArea().add(windowSaver, {top: 55, right: -2});
      
      if(!ClientLib.Data.MainData.GetInstance().get_Cities().__oTACS__set_CurrentOwnCityId) {
        ClientLib.Data.MainData.GetInstance().get_Cities().__oTACS__set_CurrentOwnCityId = ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentOwnCityId;
      }
      ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentOwnCityId = function(a) {
        this.__oTACS__set_CurrentOwnCityId(a); 
        updateView();
      }
      
      if(!ClientLib.Data.MainData.GetInstance().get_Cities().__oTACS__set_CurrentCityId) {
        ClientLib.Data.MainData.GetInstance().get_Cities().__oTACS__set_CurrentCityId = ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentCityId;
      }
      ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentCityId = function(a) {
        this.__oTACS__set_CurrentCityId(a); 
        updateView();
      }
      ...
      try {
        if (typeof qx != 'undefined') {
          ...
          if (a && mb) {
            ...
            window.setTimeout(oTACS_checkIfLoaded, 1000);
        } else {
          window.setTimeout(oTACS_checkIfLoaded, 1000);
        }
      } catch (e) {
  ...
  var oTACSScript = document.createElement("script");
  oTACSScript.innerHTML = "(" + oTACS_main.toString() + ")();";
  oTACSScript.type = "text/javascript";
  if (/commandandconquer\.com/i.test(document.domain)) {
    document.getElementsByTagName("head")[0].appendChild(oTACSScript);
  }
})();