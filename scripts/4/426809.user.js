...
//2.4.6 -- added extended number formating to support Conquests numbers int he Trillions and Quadrillions - MAD MAX
//2.4.7 -- Added Planet Tagging to work with AA open parser galaxy tools
...
if(/\.com\/messages/i.test(document.location.href)){
 ...
 //var headID = document.getElementsByTagName("head")[0];
    ...
    //newScript.innerHTML = "function setCookie(c_name%2C+value%2C+expiredays)+%7B+++++var+exdate+%3D+new+Date()%3B+++++exdate.setDate(exdate.getDate()+%2B+expiredays)%3B+++++document.cookie+%3D+c_name+%2B+%22%3D%22+%2B+escape(value)+%2B+((expiredays+%3D%3D+null)+%3F+%22%22+%3A+%22%3Bexpires%3D%22+%2B+exdate.toUTCString())%3B+%7D";
    //newScript.innerHTML = "function setCookie(c_name, value, expiredays) { var exdate = new Date(); exdate.setDate(exdate.getDate() + expiredays); document.cookie = c_name + \"=\" + escape(value) + ((expiredays == null) ? \"\" : \";expires=\" + exdate.toUTCString()); window.alert(\"Planet:\" +c_name+ \" has been tagged/untagged.\");}";
    //headID.appendChild(newScript);
    //emitStatusMessage('<H3>Espionage Parser Courtesy of mvidas and JVS - Double click to parse messages</H3>');
 setInterval(ParseEspionageReports,1000);
}

function CreateTagLink(PlanetInBracket,ESP) {
    var vPlanetString=vPrefix+'.planet.'+PlanetInBracket;
	//var c_data = localStorage.getItem(vPlanetString+'.tagged');
	//var c_data = getCookie(PlanetInBracket);
    var HTMLreturn = "";
	//ESP = "BLAAA";
	//c_data = "";

    //if (c_data == "" | c_data == "false") {
        //Display Tag link
        //HTMLreturn = '<a href="#" onClick="setCookie(&quot;'+PlanetInBracket+'&quot;,true,30)">Tag-New</a>';
		HTMLreturn = '<a href="javascript:void(0);" onClick=localStorage.'+PlanetInBracket+'=\'' + encodeURIComponent(ESP).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/\*/g, '%2A') +'\'>Tag-New</a>';
		
		//localStorage.setItem(vPrefix+'.planet.'+PlanetInBracket+'.tagged',true);
    //} else {
        //display the Untag link
        //HTMLreturn = '<a href="#" onClick="setCookie("' + PlanetInBracket + '",false,30)">Tag-Replace</a>';
		//HTMLreturn = '<a href="javascript:void(0);" onClick=localStorage.setItem('+ vPlanetString+'.tagged' + ',\'' + encodeURIComponent(ESP).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/\*/g, '%2A') +'\')>Tag-Replace</a>';
 ...
 for (i=1;i<vMessages.length;i++) {
  if(/(un)?read message( alt)?/i.test(vMessages[i].getAttribute('class'))){
   if(/espionage report for /i.test(vMessages[i].innerHTML)){
    ...
    vSubjectLine.innerHTML=vWarning+espiData[vTargetTypeIdx].substr(0,1)+'&nbsp;Res:'+vRes+'&nbsp;S:'+m(vShips,2)+'&nbsp;D:'+m(vDefenses,2)+'&nbsp;Dios:'+m(vDios,3);
    ...
    vOrigMessage.innerHTML+='<br /><br />'+vToggleLink+vLinkInfo+vBattlementatLink;
	vOrigMessage.innerHTML+='&nbsp;&nbsp;|&nbsp;&nbsp;' + CreateTagLink('g'+espiData[3]+'s'+espiData[4]+'p'+espiData[5],vOrigMessage.innerHTML) + '<div id="MsgData'+i+'" style="display:none"><br />'+vOrigMessageTxt+'</div>';
...
function ParseEspiReport(vEspi) {
 ...
 vData[30]=ParseEspiField(vMainFleet,/\* Ares (Class )?Bomber: ([\d,]+)/m,2);
 vData[31]=ParseEspiField(vMainFleet,/\* Hades (Class )?Battleship: ([\d,]+)/m,2);
 vData[32]=ParseEspiField(vMainFleet,/\* Prometheus (Class )?Destroyer: ([\d,]+)/m,2);
 vData[33]=ParseEspiField(vMainFleet,/\* Thanatos (Class )?Destroyer: ([\d,]+)/m,2);
 vData[34]=ParseEspiField(vMainFleet,/\* Zeus( Class)?: ([\d,]+)/m,2);
 vData[35]=ParseEspiField(vMainFleet,/\* (Hephaestus Class|Titan) Attack Platform: ([\d,]+)/m,2);
 vData[36]=ParseEspiField(vEspi,/\* Missile Battery: ([\d,]+)/m,1);
 vData[37]=ParseEspiField(vEspi,/\* Laser Cannon: ([\d,]+)/m,1);
 vData[38]=ParseEspiField(vEspi,/\* Space Mine: ([\d,]+)/m,1);
 vData[39]=ParseEspiField(vEspi,/\* Pulse Cannon: ([\d,]+)/m,1);
 vData[40]=ParseEspiField(vEspi,/\* Particle Cannon: ([\d,]+)/m,1);
 vData[41]=ParseEspiField(vEspi,/\* Anti-Ballistic Missile: ([\d,]+)/m,1);
 vData[42]=ParseEspiField(vEspi,/\* Decoy: ([\d,]+)/m,1);
 vData[43]=ParseEspiField(vEspi,/\* Interplanetary Ballistic Missile: ([\d,]+)/m,1);
 vData[44]=ParseEspiField(vEspi,/\* Gauss Cannon: ([\d,]+)/m,1);
 vData[45]=ParseEspiField(vEspi,/\* Large Decoy: ([\d,]+)/m,1);
 vData[46]=ParseEspiField(vEspi,/\* Plasma Cannon: ([\d,]+)/m,1);
 vData[47]=ParseEspiField(vEspi,/\* Ore Warehouse: ([\d,]+)/m,1);
 vData[48]=ParseEspiField(vEspi,/\* Crystal Warehouse: ([\d,]+)/m,1);
 vData[49]=ParseEspiField(vEspi,/\* Hydrogen Storage: ([\d,]+)/m,1);
 vData[50]=ParseEspiField(vEspi,/\* Resource Den: ([\d,]+)/m,1);
 vData[51]=ParseEspiField(vEspi,/\* Ore Mine: ([\d,]+)/m,1);
 vData[52]=ParseEspiField(vEspi,/\* Crystal Mine: ([\d,]+)/m,1);
 vData[53]=ParseEspiField(vEspi,/\* Hydrogen (Synthesizer|Refinery): ([\d,]+)/m,2);
 vData[54]=ParseEspiField(vEspi,/\* Armor Tech: ([\d,]+)/m,1);
 vData[55]=ParseEspiField(vEspi,/\* Weapons Tech: ([\d,]+)/m,1);
 vData[56]=ParseEspiField(vEspi,/\* Shield Tech: ([\d,]+)/m,1);
 vData[57]=ParseEspiField(vEspi,/\* Jet Drive: ([\d,]+)/m,1);
 vData[58]=ParseEspiField(vEspi,/\* Pulse Drive: ([\d,]+)/m,1);
 vData[59]=ParseEspiField(vEspi,/\* Warp Drive: ([\d,]+)/m,1);
 vData[60]=ParseEspiField(vEspi,/\* A.I. Tech: ([\d,]+)/m,1);
 vData[61]=ParseEspiField(vEspi,/\* Espionage Tech: ([\d,]+)/m,1);
 vData[62]=ParseEspiField(vEspi,/\* Lunar Base: ([\d,]+)/m,1);
 vData[63]=ParseEspiField(vEspi,/\* Oracle: ([\d,]+)/m,1);
 vData[64]=ParseEspiField(vEspi,/\* Warp Gate: ([\d,]+)/m,1);
 vData[65]=ParseEspiField(vEspi,/(DEFENSES:)/m,1);
 vData[66]=ParseEspiField(vEspi,/(BUILDINGS:)/m,1);
 vData[67]=ParseEspiField(vEspi,/(TECHS:)/m,1);

 vData[68]=ParseEspiField(vEspi,/Diameter: (\d+)/m,1);
 vData[69]=ParseEspiField(vEspi,/\* Lunar Dock: ([\d,]+)/m,1);
 vData[70]=ParseEspiField(vEspi,/([a-z ]+show signs of leaving soon|The territory shows signs of spatial instability\.)/mi,1);
 vData[71]=ParseEspiField(vEspi,/(Holding[a-z ,]+\.)/mi,1);
 ...
 if(vDefenders==null) {
  ...
  if(vDefenders.length>1){
   for(q=1;q<vDefenders.length;q++){
    ...
    vData[vOffset+21]=ParseEspiField(vNextFleet,/\* Ares (Class )?Bomber: ([\d,]+)/m,2);
    vData[vOffset+22]=ParseEspiField(vNextFleet,/\* Hades (Class )?Battleship: ([\d,]+)/m,2);
    vData[vOffset+23]=ParseEspiField(vNextFleet,/\* Prometheus (Class )?Destroyer: ([\d,]+)/m,2);
    vData[vOffset+24]=ParseEspiField(vNextFleet,/\* Thanatos (Class )?Destroyer: ([\d,]+)/m,2);
    vData[vOffset+25]=ParseEspiField(vNextFleet,/\* Zeus( Class)?: ([\d,]+)/m,2);
    vData[vOffset+26]=ParseEspiField(vNextFleet,/\* (Hephaestus Class|Titan) Attack Platform: ([\d,]+)/m,2);
   }
  }
 }
 return vData; }
...
function FormatEspiMessage(espiData,vShips,vDefenses,vDios,vZags) {
 ...
 // } else {
  ...
  // }
 // }
 vFormattedTotalRSP = m(vTotRSP,2);
	//function m(n,d){x=(''+n).length,p=Math.pow,d=p(10,d)
	//x-=x%3
	//return Math.round(n*d/p(10,x))/d+" kMGTPE"[x/3]}
 ...
 if (espiData[vDefenderNameIdx].length==0) {
  ...
  vText+=ifShip(espiData[30],'Ares Class Bomber');
  vText+=ifShip(espiData[31],'Hades Class Battleship');
  vText+=ifShip(espiData[32],'Prometheus Class Destroyer');
  vText+=ifShip(espiData[33],'Thanatos Class Destroyer');
  vText+=ifShip(espiData[34],'Zeus Class');
  vText+=ifShip(espiData[35],'Hephaestus Class Attack Platform');
  if(espiData[vGDIdx]>1){
   for(q=1;q<espiData[vGDIdx];q++){
    var vOffset=(vGDIdx+1)+((vUBShip-vLBShip)+2)*(q-1);
    vText+='<br />';
    ...
    vText+=ifShip(espiData[vOffset+21],'Ares Class Bomber');
    vText+=ifShip(espiData[vOffset+22],'Hades Class Battleship');
    vText+=ifShip(espiData[vOffset+23],'Prometheus Class Destroyer');
    vText+=ifShip(espiData[vOffset+24],'Thanatos Class Destroyer');
    vText+=ifShip(espiData[vOffset+25],'Zeus Class');
    vText+=ifShip(espiData[vOffset+26],'Hephaestus Class Attack Platform');
   }
  }
  vText+='</span>';
 }
 ...
 if (espiData[vDEFIdx].length==0) { 
  ...
  vText+='<br />';
  vText+=ifShip(espiData[36],'Missile Battery');
  vText+=ifShip(espiData[37],'Laser Cannon');
  vText+=ifShip(espiData[38],'Space Mine');
  vText+=ifShip(espiData[39],'Pulse Cannon');
  vText+=ifShip(espiData[40],'Particle Cannon');
  vText+=ifShip(espiData[vABMIdx],'Anti-Ballistic Missile');
  vText+=ifShip(espiData[42],'Decoy');
  vText+=ifShip(espiData[vIPBMIdx],'Interplanetary Ballistic Missile');
  vText+=ifShip(espiData[44],'Gauss Cannon');
  vText+=ifShip(espiData[45],'Large Decoy');
  vText+=ifShip(espiData[46],'Plasma Cannon');
 }
...
function GetDebrisAmounts() {
 ...
 DebrisAmts[30]=22500; //ares
 DebrisAmts[31]=21000; //hades
 DebrisAmts[32]=33000; //prom
 DebrisAmts[33]=765000; //thanatos
 DebrisAmts[34]=2700000; //zeus
 DebrisAmts[35]=12000000; //heph
 return DebrisAmts;}
 ...
 DebrisAmts[30]=37500; //ares
 DebrisAmts[31]=35000; //hades
 DebrisAmts[32]=55000; //prom
 DebrisAmts[33]=1275000; //thanatos
 DebrisAmts[34]=4500000; //zeus
 DebrisAmts[35]=20000000; //heph
 DebrisAmts[36]=400; //missile
 DebrisAmts[37]=400; //laser
 DebrisAmts[38]=100; //space mine
 DebrisAmts[39]=1600; //pulse
 DebrisAmts[40]=1600; //particle
 DebrisAmts[41]=1600; //abm
 DebrisAmts[42]=4000; //decoy
 DebrisAmts[43]=3000; //nuke
 DebrisAmts[44]=7000; //gauss
 DebrisAmts[45]=20000; //l.decoy
 DebrisAmts[46]=20000; //plasma
 return DebrisAmts;}
...
function GetDSPAmounts() {
 ...
 DSPAmts[30]=90000; //ares
 DSPAmts[31]=85000; //hades
 DSPAmts[32]=125000; //prom
 DSPAmts[33]=2650000; //thanatos
 DSPAmts[34]=10000000; //zeus
 DSPAmts[35]=50000000; //heph
 return DSPAmts;}
...
function GetRSPAmounts() {
 ...
 RSPAmts[30]=90000; //ares
 RSPAmts[31]=85000; //hades
 RSPAmts[32]=125000; //prom
 RSPAmts[33]=2650000; //thanatos
 RSPAmts[34]=10000000; //zeus
 RSPAmts[35]=50000000; //heph
 return RSPAmts;}


function GetNukeAmounts() {
var NukeAmts=new Array();
NukeAmts[36]=2; //missile
NukeAmts[37]=2; //laser
NukeAmts[38]=0.5; //space mine
NukeAmts[39]=8; //pulse
NukeAmts[40]=8; //particle
NukeAmts[vABMIdx]=0; //abms
NukeAmts[42]=20; //decoy
NukeAmts[vIPBMIdx]=15; //nukes
NukeAmts[44]=35; //gauss
NukeAmts[45]=100; //large decoy
NukeAmts[46]=100; //plasma
return NukeAmts;}
