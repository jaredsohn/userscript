// ==UserScript==
// @name        Clodogame - Profil de Bande Deluxe
// @namespace   http://userscripts.org/scripts/show/102166
// @author      Memphis007
// @description Permet un affichage détaillé des caractéristiques des joueurs sur le profil de bande.
// @maj         Correction de la formule de calcul pour les joueurs attaques
// @version     3.9
...
if (url.indexOf("reloaded.clodogame")>=0) {
...
if (url.indexOf("marseille.clodogame")>=0) {
	...
	var sig = 'http://inodes.clodogame.fr/ma_FR/signaturen/';
}
if (url.indexOf("www.clodogame")>=0) {
...
document.getElementsByClassName("gang_footer")[0].style.width = '99%';

var succes = document.getElementsByClassName("gang_footer")[0].getElementsByTagName("table")[0].innerHTML;
var tr1 = succes.split('<td width="50%">')[1];
var tr2 = succes.split('<td width="50%">')[2];
tr2 = tr2.replace("<em>Aucune déclaration de guerre existante</em>","<em>Aucune déclaration de guerre existante</em><br/>");
tr2 = tr2.replace(/Krieg/g,"Guerre");
tr2 = tr2.replace(/Verbündet/g,"Alliés");
document.getElementsByClassName("gang_footer")[0].getElementsByTagName("table")[0].innerHTML = "<tr><td>"+tr1+"</td></tr><tr><td>"+tr2+"</td></tr>";

var table = document.getElementsByClassName("gang_footer")[0].getElementsByTagName("table")[1];
...
for (var x = 1; x < tr.length; x++) {
	if (x%2 == 0){
		...
		tr[x].setAttribute('onmouseout', 'this.style.backgroundColor="#222222";');
	}
	...
	newtd.setAttribute('style', 'vertical-align:middle;text-align:right;width:80px;');

	var newtd = tr[x].appendChild(document.createElement('td'));
	newtd.setAttribute('id', 'stadt'+x);
	newtd.setAttribute('style', 'vertical-align:middle;text-align:center;width:60px;');
...
function Geldladen(id,x) {
	GM_xmlhttpRequest({
		...
		onload: function(responseDetails) {
			...
			document.getElementById('nachricht'+x).innerHTML = "<a href=\"/messages/write/?to="+id+"\"><img style\"margin:5px;\" src=\"http://static.pennergame.de/img/pv4/icons/new_msg.gif\" title=\"Envoyer un message à "+username+".\"></a>";
		}
	});
...
// muenchen
					...
					case "73933": var tier_name = 'Souris dressée'; break;
...
//mendigogame clodogame reloaded
					case "73933": var tier_name = 'Souris dressée'; break;
					case "fr8596": var tier_name = 'Cafard'; break;
					case "fr8930": var tier_name = 'Poisson Rouge'; break;
					case "fr8795": var tier_name = 'Souris'; break;
					case "fr5423": var tier_name = 'Hamster'; break;
					case "s1"    : var tier_name = 'Moineau'; break;
					case "fr4591": var tier_name = 'Perruche'; break;
					case "fr1451": var tier_name = 'Pigeon'; break;
					case "fr3684": var tier_name = 'Rat'; break;
					case "fr7730": var tier_name = 'Lapin'; break;
					case "fr1903": var tier_name = 'Furet'; break;
					case "fr3735": var tier_name = 'Chat'; break;
					case "fr9386": var tier_name = 'Faucon'; break;
					case "fr1482": var tier_name = 'Serpent'; break;
					case "fr2474": var tier_name = 'Chèvre'; break;
					case "fr2758": var tier_name = 'Caniche'; break;
					case "fr4263": var tier_name = 'Aigle'; break;
					case "fr9051": var tier_name = 'Berger Allemand'; break;
					case "fr5240": var tier_name = 'Pitbull'; break;
					case "fr1456": var tier_name = 'Cocker'; break;
					case "fr7563": var tier_name = 'Chihuahua'; break;
					case "fr0385": var tier_name = 'Cheval'; break;
					case "fr2563": var tier_name = 'Girafe'; break;
					case "fr4220": var tier_name = 'Hippopotame'; break;
					case "fr4843": var tier_name = 'Crocodile'; break;
					case "fr5687": var tier_name = 'Tigre'; break;
					case "fr2536": var tier_name = 'Singe'; break;
					case "fr8569": var tier_name = 'Rhino'; break;
					case "fr1256": var tier_name = 'Elephant'; break;
					case "fr8142": var tier_name = 'Ours Polaire'; break;
					case "mammut_xmas": var tier_name = 'Mammouth'; break;
//bumrise                       
					case "cockroach": var tier_name = 'Kakerlake ATT0/DEF0'; break;
			...
			document.getElementById('stadt'+x).innerHTML = location3+'<sup style="font-size: 90%;">e</sup>';
		...
		GM_xmlhttpRequest({
			...
			onload: function(responseDetails) {
				...
				for (var x = 1; x < tr.length; x++) {
					...
					if ((minPts <= points) && (points <= maxPts) && (parseInt(points * 0.8) > pts)) {
						document.getElementById('angreifen'+x).innerHTML = '\<a href="'+link+'/fight/?to='+username+'">'+
						"<img height=\"18px\" src=\""+getPic("blue")+"\""+
						" alt=\"Angreifbar\" title=\"Tu peux attaquer ce joueur, il a plus de points mais il ne peut pas t'attaquer\"></a>";
					}else if ((minPts <= points) && (points <= maxPts) && (pts > points)) {
						document.getElementById('angreifen'+x).innerHTML = '\<a href="'+link+'/fight/?to='+username+'">'+
						"<img height=\"18px\" src=\""+getPic("green1")+"\""+
						" alt=\"Angreifbar\" title=\"Tu peux attaquer ce joueur, il a moins de points que toi.\"></a>";
					}else if ((minPts <= points) && (points <= maxPts) && (pts < points)) {
						document.getElementById('angreifen'+x).innerHTML = '\<a href="'+link+'/fight/?to='+username+'">'+
						"<img height=\"18px\" src=\""+getPic("green2")+"\""+
						" alt=\"Angreifbar\" title=\"Tu peux attaquer ce joueur, il a plus de points que toi.\"></a>";
					}else if (points == pts){
						document.getElementById('angreifen'+x).innerHTML = "<img height=\"18px\" src=\""+getPic("red")+"\""+
						" alt=\"selber\" title=\"Tu ne peux pas t'attaquer toi même !\">";
					}else if ((parseInt(points * 0.8) <= pts) && (pts <= parseInt(points + points * 0.5))){
						document.getElementById('angreifen'+x).innerHTML = "<img height=\"18px\" src=\""+getPic("yellow")+"\""+
						" alt=\"Kampfbereit\" title=\"Ce joueur a moins de points que toi..tu ne peux pas l'attaquer toi même, parce tu as trop de point mais il peut t'attaquer car tu es encore dans son interval de points..\">";
					}else{
						document.getElementById('angreifen'+x).innerHTML = "<img height=\"18px\" src=\""+getPic("red")+"\""+
						" alt=\"n.Angreifbar\" title=\"Tu ne peux pas attaquer "+username+" car il n'est pas dans ton interval de points\">";
					}
				}
...
name: 'Clodogame - Profil de Bande Deluxe', // Script Name
version: '3.9', // Version
id: '102166', // Script id on Userscripts.org
