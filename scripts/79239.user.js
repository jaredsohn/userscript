...
var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4100116&page=1';
var postId = '4100116';
...
var listNames = ['family', 'promoteme', 'waitlist', 'hitlist'];
var list = {
	family: {
		member: {},
		title: 'FAMILY',
...
function storeCache() {
	GM_setValue('expires', now + cacheTime + '');
	$.each(listNames, function() {
		GM_setValue(this, list[this].member.toSource());
	});
	GM_setValue('nameToId', nameToId.toSource());
}

function loadFromCache() {
	var tmp = GM_getValue('expires', 0);
	if (now > tmp) {
		insertTitles();
		$.each(listNames, function() {
			list[this].member = {};
...
function loadFromThisContent(data) {
	...
	var dataText = '#SL#' + $(data).html().replace(/<br>/g, "#EL##SL#").replace(/#SL#$/, '').replace(/<(\/?[bi])>/gi, function(m, tag) { return '#'+tag+'#'; });
	names = {num: 0};
	nameToId = {};
	$.each(listNames, function() {
		var l = this;
...
function loadFromCurPage() {
	...
	data.html(dataText
		.replace(/#SL##b#~~~ (\S+) ~~~#\/b##EL#(.+?)#SL##b#~~~ END ~~~#\/b##EL#(?:#SL##EL#)*/g, function(matched, ltitle, content) {
			...
			var newContent = content.replace(/(.*?)#SL##b#### (.+?)#\/b##EL#/g, function(matched, pre, title) {
				return pre.replace(/#EL#((?:#SL##EL#)*)$/, function(matched, end) { return '#EL#</fieldset>' + end; })
					+ '#SL#<fieldset id="fam_list_' + l + '_' + title.replace(/[ \t"']+/g, '_') + '" style="padding: 4px; border: 1px solid #d0d0d0;">'
					+ '<legend style="font-weight: bold;">' + title + '</legend>';
			});
			if (content != newContent) newContent += '</fieldset>';
			return '<fieldset id="fam_list_' + l + '" style="margin: 4px; padding: 4px; border: 1px solid #a0a0a0; display: ' + (ltitle.match(/ChangeLog|TODO/) ? 'block' : 'inline') + '; vertical-align: top;">'
		...
		.replace(/#(\/?[bi])#/gi, function(matched, tag) { return '<'+tag+'>'; }).replace(/#EL#/g, '<br />').replace(/#SL#/g, '')
	);
	(function() {
		var me = this;
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			$('#content div.content_container tbody:eq(0) tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				if (list.hitlist.member[id]) {
					...
					delete names[id];
					names.num--;
				} else if (list.promoteme.member[id] || list.family.member[id]) {
					if (list.family.member[id]) {
						++membersWithTitles;
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
					...
					if (unknownPanel == null) {
						$('#fam_list_family').after(
							'<fieldset id="fam_list_UNKNOWN" style="margin: 4px; padding: 4px; border: 1px solid #a0a0a0; display: inline; vertical-align: top;">'
...
function setupPrefs() {
	...
	function prefSet() {
		...
		if (pref[p].val == pref[p].dflt) {
			GM_deleteValue('cfg_' + p);
		} else {
			GM_setValue('cfg_' + p, pref[p].val);
		}
		insertTitles();
//		setupPrefs();
	}
	...
	function prefLoadAll() {
		$.each(pref, function(key, p) {
			p.val = GM_getValue('cfg_' + key);
			if (p.val == undefined) {
				p.val = p.dflt;
			} else if (p.val == p.dflt) {
				GM_deleteValue('cfg_' + key);
			}
		});
	}
	switch (PAGE_TYPE) {
...
UPDATE.check(2.5, SCRIPT.meta, SCRIPT, function(objUpdate, objScript) {
	...
	$('div.content_header').after('<div class="tip"><div class="tt_rounded"> <div class="tt_top"><div><center><p>&nbsp;</p></center></div></div> <div class="tt_content" id="update_content">'
		+ '<a style="color: #ffff00;" href="' + objScript.source + '" title="' + title + '">' + objScript.name + '</a> has been updated; <a style="color: #ffff00;" href="' + objScript.identifier + '" title="' + title + '">upgrade now</a>.'
