// ==UserScript==
// @name           KOC Power Bot
// @version        20110903a-Fr
// @namespace      mat
// @homepage       http://userscripts.org/scripts/show/104016
// @include        *.kingdomsofcamelot.com/*main_src.php*
// @include        *apps.facebook.com/kingdomsofcamelot/*
// @include        *facebook.com/connect/uiserver.php*
// @description    Automated features for Kingdoms of Camelot
// @require        http://tomchapin.me/auto-updater.php?id=104016
// ==/UserScript==


var Version = '20110903a-Fr';
...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'** Je suis Attaqué ! **', scouting:false, wilds:false, defend:true, minTroops:10000, spamLimit:10, lastAttack:0, barbautoswitch:false, raidautoswitch: {}, },
...
Tabs.tower = {
  ...
  towerMarches: [],
  Providers : {
		0: { 'country': "--Pays--", 'provider': "--Opérateur--" },
  ...
  init: function(div){
	...
	AddTowerTab('Arrêt alarme', t.stopSoundAlerts, 'pbtowertab');
    ...
    var m = '<DIV class=pbStat>ALERTES TOUR</div><TABLE class=pbTab><TR align=center>';
    ...
    m += '</tr></table><BR><DIV><CENTER><INPUT id=pbSoundStop type=submit value="Arrêt alarme"></center></div><DIV id=pbSwfPlayer></div>';
    ...
    <td align=left>SMS si une attaque arrive: <INPUT id=pbnum1 type=text size=4 maxlength=4 value="'+ Options.celltext.num1 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum2 type=text size=3 maxlength=3 value="'+ Options.celltext.num2 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum3 type=text size=4 maxlength=4 value="'+ Options.celltext.num3 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\> <span style="color:#800; font-weight:bold">(Frais standard de SMS)</span></td></tr><tr><td></td>\
    <TD align=left>Pays: <select id="pbfrmcountry">';
 ...
 if(t.Providers[i].country == t.Providers[Options.celltext.provider].country)
    ...
    m += '</select></td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Message automatique au chat alliance si arrivée d\'une attaque.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            ...
            <TR><TD align=right>Alerte éclaireur: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Alerte attaque TS: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Afficher statut de défense: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Minimum de troupe: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
            </table></td></tr>\
        <TR><TD align=right><INPUT id=pbalertraid type=checkbox '+ (Options.alertConfig.raid?'CHECKED':'') +'/></td><TD>Arrêt raids si attaque.</td></tr>\
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Alarme si arrivée attaque/éclairage</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>Loading SWF player</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Fichier son: &nbsp; </td><TD><INPUT id=pbsoundFile type=text size=55 maxlength=160 value="'+ Options.alertSound.soundUrl +'" \>\
             &nbsp; </td><TD><INPUT id=pbSoundLoad type=submit value=Charge><INPUT id=pbSoundDefault type=submit value=Défaut></td></tr>\
            ...
            <TR><TD align=right><INPUT id=pbSoundRepeat type=checkbox '+ (Options.alertSound.repeat?'CHECKED ':'') +'/></td><TD> Répéter toutes les <INPUT id=pbSoundEvery type=text size=2 maxlength=5 value="'+ Options.alertSound.repeatDelay +'"> minutes</td></tr>\
            <TR><TD></td><TD>jouer pour <INPUT id=pbSoundLength type=text size=3 maxlength=5 value="'+ Options.alertSound.playLength +'"> secondes</td></tr>\
            <TR><TD></td><TD><INPUT type=submit value="Jouer maintenant" id=pbPlayNow></td></tr></table></div></td></tr>\
        </table><BR>';
  	t.myDiv.innerHTML = m;
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
  ...
  e_soundFileLoaded : function (chan, isError){ // called by SWF when sound file finishes loading
    ...
    if (isError)  
      ...
      document.getElementById('pbLoadStat').innerHTML = 'Chargement';
  },  
  ...
  postToChat : function (m){
	...
	if(m.marchStatus == 9)
		msg += 'La '+ atkType +' sur ma '+ target +' par '+ who +' a été rappelé.';
	else
		msg += 'My '+ target +' est '+ atkType  +' par '+ who +' troupes entrantes (arrivant dans '+ unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +') : ';
    var totTroops = 0;
    for (k in m.unts){
      var uid = parseInt(k.substr (1));
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' Mon ambassade a '+ availSlots +' des '+ emb.maxLevel +' emplacements disponibles.';
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
            msg+= ' Troupes se cachent!';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            msg+= ' Troupes en DEFENSE!';
        }
      }
    }
	t.sendalert(m);
    ...
    if (SEND_ALERT_AS_WHISPER)
      ...
      handleTowerData: function(m){
        ...
        for (k in m.unts) {
            var uid = parseInt(k.substr(1));
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Unité de ravi.')
                units[1] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Milicien')
                units[2] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Eclaireur')
                units[3] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Piquier')
                units[4] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Paladin')
                units[5] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Archer')
                units[6] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Cavalerie')
                units[7] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Cavalerie Lourde')
                units[8] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Wagon de ravi.')
                units[9] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Baliste')
                units[10] = m.unts[k];
            if (unsafeWindow.unitcost['unt' + uid][0] == 'Bélier')
                units[11] = m.unts[k];
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    m = '<DIV class=ptentry><TABLE width=100% class=pbTab><TR><TD class=pbDetLeft>Recherche pour: </td><TD width=99%>';
    m += htmlSelector ({0:"Camps barbares", 1:"Terres sauvages", 2:"Villes"}, null, 'id=pasrcType');
	m += '&nbsp; &nbsp; &nbsp; <span class=pbDetLeft>Type de recherche: &nbsp;';
	m += htmlSelector({square:"Carré", circle:"Cercle"}, Options.srcdisttype, 'id=pbsrcdist');
    m += '</span></td></tr><TR><TD class=pbDetLeft>Au: </td><TD class=xtab>X=<INPUT id=pasrchX type=text\> &nbsp;Y=<INPUT id=pasrchY type=text\>\
      &nbsp; Rayon: <INPUT id=pasrcDist size=3 value=10 /> &nbsp; <SPAN id=paspInXY></span></tr>\
      <TR><TD class=pbDetLeft>Ou:</td><TD>Recherche province entière: <select id="provinceXY"><option>--provinces--</option>';
...
Tabs.transport = {
  ...
  check:false,
    ...
    t.readTraderState();
      ...
      m += '<TR align="left"><TD>De la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptrescity></span></div></td></tr>';

      m += '<TR align="left">';
      m += '<TD>Pour la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptcityTo></span></div></td>';
      m += '<TD>OR</td>';
...
************/
 ...
 Tabs.Raid = {
  ...
  init : function (div){
	...
	var m = '<DIV class=pbStat>FONCTIONS DU SYSTEME RAID</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
	    m += '<TD><INPUT id=pbRaidStart type=submit value="RAZ automatique = '+ (Options.RaidRunning?'ON':'OFF') +'" ></td>';
	    m += '<TD><INPUT id=pbsendraidreport type=checkbox '+ (Options.foodreport?'CHECKED':'') +'\> Envoyer un rapport d\'attaque toutes les ';
	    m += '<INPUT id=pbsendreportint value='+ Options.MsgInterval +' type=text size=3 \> heures </td>';
	    m += '</tr></table></div>';
	    m += '<DIV class=pbStat>ACTIVE ATTAQUES</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
	    ...
	    m += '<DIV class=pbStat>RAIDS ENREGISTRES</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
  ...
  paint : function ()	{
  	...
  	var botStat = {0:'Undefined',
  						...
  						8:'Resting'};
  	var o = '';
  	if (t.rslt.settings != undefined) o+= '<FONT size=2px><B>Minuterie RAID: '+ timestr( 86400 - ( unixTime() - t.rslt.settings.lastUpdated )) +'</b></font>';
  	document.getElementById('ptRaidTimer').innerHTML = o;
  	
  	var z ='<TABLE class=pbTab><TR><TD width=60px align=center><A onclick="pbStopAll('+t.cityId+')">STOP</a></td><TD width=70px>Temps</td><TD width=85px>Coords</td><TD width=50px>Niveau</td><TD width=50px></td><TD width=50px><A onclick="pbDeleteAll()">SUPPRIMER</a></td></TR>';
  	if (t.rslt['queue'] != ""){
  		for (y in t.rslt['queue']) {
  			if (t.rslt['queue'][y]['botMarches'] != undefined) {
  				...
  				if (botMarchStatus == 3) z+='<TD><A onclick="pbEditRaid('+ y +')">Modifier</a></td>';
	  				else z+='<TD><FONT COLOR= "CCCCCC">Modifier</font></td>';
  				...
  				z +='<TD width=25px></td><TD>Temps de repos: '+ timestr(restPeriod) +'</td>';
  				z+='</tr>';
  			}
  		}
  	}
  	z+='</table>';
  	if (t.rslt['queue'] == "") z ='<TABLE class=pbTab><TR><TD>Aucun raids dans la ville!</td></TR>';
  	...
  	if (check) a ='<TABLE class=pbTab><TR><TD>Aucun raids enregistrés dans la ville!</td></TR>';
  ...
  toggleRaidState : function (){
  	...
  	if(Options.RaidRunning){
  		...
  		t.raidtimer = null;
  		document.getElementById('pbRaidStart').value = 'RAZ automatique = OFF';
  	} else {
  		Options.RaidRunning = true;
  		t.raidtimer = setTimeout(t.checkRaids, 5000);
  		document.getElementById('pbRaidStart').value = 'RAZ automatique = ON';
  	}
  	saveOptions();
  },
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>Configuration power bot:</b></td></tr>\
        <TR><TD><INPUT id=pballowWinMove type=checkbox /></td><TD>Activer déplacement de la fenêtre (maintenir click gauche sur la barre menu de la fenêtre puis déplacer la souris)</td></tr>\
        ...
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Activer le style écran large: '+ htmlSelector({normal:'Normal', wide:'écran large', ultra:'Ultra'},GlobalOptions.pbWideScreenStyle,'id=selectScreenMode') +' (tous les domaines, il faut rafraîchir)</td>\
        <TR><TD colspan=2><BR><B>Fonctions KoC:</b></td></tr>\
        <TR><TD><INPUT id=pbFairie type=checkbox /></td><TD>Disable all Fairie popup windows</td></tr>\
        <TR><TD><INPUT id=pbWatchEnable type=checkbox '+ (GlobalOptions.pbWatchdog?'CHECKED ':'') +'/></td><TD>Actualiser si KOC pas chargé dans 1 minute (tous les domaines)</td></tr>\
        <TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>Actualiser KoC toutes les <INPUT id=pbeverymins type=text size=2 maxlength=3 \> minutes</td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Mettre le chat sur la droite (nécessite écran large)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>Carte large (nécessite écran large)</td></tr>\
        <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>Collecte automatique de l\'or quand le bonheur atteint <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
        <TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>Activer Alerte alimentaire (Pour moins de 6 heures de nourriture contrôlée toutes les heures)</td></tr>\
        <TR><TD colspan=2><BR><B>Fonctions supplémentaires:</b></td></tr>\
        <TR><TD><INPUT id=HelReq type=checkbox /></td><TD>Aide alliance construction/recherche labo</td></tr>\
        <TR><TD><INPUT id=DelReq type=checkbox /></td><TD>Masquer les demandes dans le chat alliance</td></tr>\
        <TR><TD><INPUT id=PubReq type=checkbox '+ (GlobalOptions.autoPublishGamePopups?'CHECKED ':'') +'/></td><TD>Publier automatiquement les messages sur Facebook  pour '+ htmlSelector({0:'----', 80:'Tout le monde', 50:'Amis des Amis', 40:'Amis uniquement', 10:'Moi uniquement'},GlobalOptions.autoPublishPrivacySetting,'id=selectprivacymode') +' (Pour tous les domaines)</td>\
        <TR><TD><INPUT id=MapExtra type=checkbox /></td><TD>Afficher joueur et puissance sur la carte.</td></tr>\
        <TR><TD><INPUT id=deletetoggle type=checkbox /></td><TD> Supprimer automatiquement les rapports des camps barbares</td></tr>\
        <TR><TD><INPUT id=deletes0toggle type=checkbox /></td><TD> Supprimer auomatiquement les rapports de transport</td></tr>\
        <TR><TD><INPUT id=deletes1toggle type=checkbox /></td><TD> Supprimer automatiquement les rapports de TS</td></tr>\
        </table><BR><BR><HR>Notez que si une case à cocher est grisée, il y a sans doute eu un changement de code KoC, rendant l\'option inopérable.</div>';
        m += strButton20('Rétablir toutes les options', 'id=ResetALL');
      div.innerHTML = m;
...
Tabs.Reassign = {
  ...
  check:false,
    ...
    init: function(div){
      ...
      m += '<TD width=20px>De la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptassigncity></span></div></td></tr>';

      m += '<TR align="left">';
      m += '<TD width=20px>Pour la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptassigncityTo></span></div></td>';
...
Tabs.Reinforce = {
  ...
  checkETA:null,
    ...
    init: function(div){
      ...
      m += '<TD width=20px>De la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptRfcityFrom></span></div></td></tr>';

      m += '<TR align="left">';
      m += '<TD>Pour la ville:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptRfcityTo></span></div></td>';
      m += '<TD>OR</td>';
...
Tabs.AutoTrain = {
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>FORMATION AUTOMATIQUE</div><TABLE width=100% height=0% class=pbTab><TR><TD width=200></td>';
	...
	new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/train.php" + unsafeWindow.g_ajaxsuffix, {
		...
		onSuccess: function(rslt) {
		  if (rslt.ok) {
			...
			actionLog(logcity  + ' Formation ' + num + ':  ' + troops[unitId] );
		  } else {
			setTimeout (notify, 10000);
		  }
		},
...
Tabs.Chat = {
  ...
  init : function (div){    // called once, upon script startup
    ...
    t.myDiv.innerHTML = '<DIV class=pbStat>Chat Answer/Reply Info</div><TABLE><TR>\
						<TD><input type=checkbox id=pbchatqaenable /></td><TD>Activer les fonctions de chat </td></tr>\
						<TR><TD><input type=checkbox id=pbchatpassenable /></td><TD>Activer mot de passe: <input type=text id=pbchatpass value="'+ ChatOptions.password +'"/></td>\
						<TD width=10px>&nbsp;</td><TD><input type=checkbox id=pbautoblacklist />Auto blacklist players if 1st attempt fails</td></tr>\
						<TR><TD></td><TD valign=top>Joueurs autorisés: <br><textarea cols=30 rows=1 id=allowUserBox></textarea></td>\
