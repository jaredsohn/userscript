// ==UserScript==
// @name           TencentWblogFollowerCheck for Firefox&Maxthon&Chrome
// @description    腾讯微博互听查询插件[火狐,遨游,Chrome]版
// @include			http://t.qq.com/*
// @exclude			http://t.qq.com/k/*
// @exclude			http://t.qq.com/p/*
// @exclude			http://t.qq.com/setting
// @author			http://t.qq.com/liang47
// @homepage		        http://927la.com
// @version		    1.8.3.5
// ==/UserScript==
...
function appendNewLi(){
	$('.tabStyle2 ul').append('<li id="suppersFo"><a title="未收听我的人" href="#">神奇的听众</a></li><li id="lisong"><a href="/liang47" target="_blank" title="点此收听作者，关注插件最新动态，或者提交您的意见">@海文</a></li>');
	$('#suppersFo').click(function(){
		...
		$(this).addClass('select').html('<b>神奇的听众</b>');
		$('.tabStyle2 .right').remove();
		$('span.browserMod').remove();
		$('.citySame .right,.specialFo,#pageNav,#sFoTip').remove();
		$('.listWrapper .LC').empty();
		$('.citySame .left').html('<input type="button" id="queryStart" style="display:inline;" title="" value="点此查询" />')
...
function getUser(){
	...
	if($("#LUI").size()>0){
		...
		$(".userPic a:last-child").each(function(i){
			user = $(this).attr('title').replace(/.+\(@(.+)\)/g,"$1");
...
function getSameTags(){
	$.ajax({type:'GET',url:'http://t.qq.com/asyn/userCard.php?u='+myself,success:function(rs){
		...
		for(var i=0;i<tags.length;i++){
			$('div.userTags a:contains("'+tags[i]+'")').css({'text-shadow':'red 0px 0px 1em'});
		}
	}});
}
	...
	if(pageType==1||pageType==2){
		if(!$('.listWrapper li:eq('+i+')').hasClass('pubInfo')){
			...
			if((/\d+月\d+日/gi).test(postHTML)){
				...
				postDate=new Date(today.getFullYear(),timeArray[0]-1,timeArray[1]);
			}else if((/\d+年\d+月\d+日/gi).test(postHTML)){
				timeArray=postHTML.match(/\d+/gi);
				postDate=new Date(timeArray[0],timeArray[1]-1,timeArray[2]);
			}
			interval=parseInt((today-postDate)/86400000);
			if(interval>=3){
				$('.listWrapper p.pubTime').eq(i).append('<span style="float:right;font-weight:bold;font-size:110%;color:red;">['+interval+'天前更新]</span>');
			}
		}
	}
}
function followLisong(){
	if(myself!='lisong'){
		...
		$.ajax({
			type:'GET',
			url:'http://t.qq.com/asyn/userRelation.php?u=lisong',
			success:function(json){
				if(eval('('+json+')').info.lisong==0&&getCookie('lisong_'+myself)==null){
					if(confirm('欢迎使用木公开发的腾讯微博辅助插件，建议收听作者以获取插件最新消息\n\n点击“确定”立即收听，点击“取消”跳过...')){
						$.post('/follow.php',{'r':(new Date()).valueOf(),'u':'lisong'},function(){setCookie('lisong_'+myself,'followed',30);alert('收听成功');});
					}else{setCookie('lisong_'+myself,'unfollowed',30);window.location='http://t.qq.com/lisong';}
				}
			}
		});
	}	
...
function unFoList(isFollowed,user,info){
	if(isFollowed=='0'){
		var html='<li id="li_'+user+'"><div class="userPic"><a href="/'+user+'" rel="'+info.nick+'('+user+')"><img id="face_'+user+'" src="http://mat1.gtimg.com/www/mb/images/head_50.jpg"></a></div>';
			html+='<div class="msgBox"><div class="clear"><div class="userName"><a title="'+info.nick+'(@'+user+')" href="/'+user+'" style="color:'+(info.gender?'#3333ff':'#ff66cc')+';font-weight:bold;text-shadow:#666 1px 1px 0.5em;">'+info.nick+'</a>';
			html+=(info.flag.auth==1?'<a class="vip" target="_blank" title="腾讯认证" href="/certification"></a>':'')+'</div>';
...
function appendSpan(i,r,user){
	var html='<span style="padding-left:1em;color:#999999;font-size:12px;">已收听我:<a><strong>'+(r.info.followed == '1' ? '是' : '否')+'</strong></a></span>';
	if(user=='lisong')html='<span style="padding-left:1em;font-size:12px;color:'+(r.info.followed==1?'green;font-weight:bold':'black')+';">[插件作者]</span>';
	if(pageType==1){
		$('.listWrapper .LC li:eq('+i+') .msgBox .userName a:last').after(html);
		if(r.info.followed==0) $('.listWrapper .LC li').eq(i).css('background','#FFFF00');
	}else if(pageType==3){
		$('#LUI .detail .userName').css({'color':r.info.gender?'#3333ff':'#ff66cc','text-shadow':'#999 1px 1px 0.5em'})
									.nextAll('.left').append(html);
	}
	html=null;
	if(pageType==1||pageType==2){
		$('.listWrapper .LC li:eq('+i+') .msgBox .userName a:first').css({'color':r.info.gender?'#3333ff':'#ff66cc','font-weight':'bold','text-shadow':'#666 1px 1px 0.5em'});
		$('.listWrapper .LC li:eq('+i+') .msgBox .userNums').append('<span class="cNote" style="white-space:nowrap;">广播<a href="/'+user+'"><strong>'+r.info.num[0]+'</strong></a>个</span>');
		if(r.info.num[0]==0) $('.listWrapper .LC li:eq('+i+') .msgBox .userNums').append('<span style="float:right;margin-right:0px;font-weight:bold;font-size:110%;color:red;">[从未发表广播]</span>');
	}
}