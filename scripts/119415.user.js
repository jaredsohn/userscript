// ==UserScript==
// @name	Wall Manager Sidekick (Sims Social)
// @namespace   WM_Sims_Social_Sidekick
// @description	Assists Wall Manager with 'Sims Social' posts
// @include	/^https?:\/\/simssoc\.game\.playfish\.com\/(.*)\/feed\?/
...
(function() {	// anonymous function wrapper start
	...
	var appName='Sims Social';	// Name of the Facebook Application.
	var scriptID='119415';		// Unique userscripts id.
	...
	****************************************************************************/
	function dock(){
		...
		var attachment={
			appID:appID,
			name:appName,
			thumbsSource: "app_full_proxy.php?app=144959615576466",
			flags:{},
			icon: "http://photos-f.ak.fbcdn.net/photos-ak-snc1/v43/222/144959615576466/app_2_144959615576466_4719.gif",
	...
	var main = {
		...
		run : function (){
			...
			**********************************************************************/
			...
			//accepts
			else if (text.match( /((Y|y)ou have (receive|claim)|Congratulations)/ )) {
				sendMessage("status=1"); return;

			}

			//already accepts
			else if (text.match( /((Y|y)ou('ve| have)? already (received|claimed)( this gift)?)/ )) {
				sendMessage("status=-6"); return;

			}

			//over-limits
			else if (text.match( /(handle any more gifts for the day)/ )) {
