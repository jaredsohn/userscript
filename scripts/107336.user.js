...
var SCRIPT = {
	name: 'The Family (dynamic) 3.1',
	namespace: 'vladykins',
	source: 'http://userscripts.org/scripts/show/107336',
	identifier: 'http://userscripts.org/scripts/source/107336.user.js',
	meta: 'http://userscripts.org/scripts/source/107336.meta.js',
	version: '3.1.0',
	date: '2013-02-06',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4675876';
var postId = '42839667';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4675876&page=1';
// var postId = '42839692';
...
var listVersion = 0;
var listNames = ['family', 'apprentice', 'cured', 'waitlist', 'hitlist'];
var list = {
	family: {
		member: {},
		title: 'GLB Addicts',
	...
	cured: {
		...
		title: 'CURED',
		style: 'color: black;',
		agentTitle: function(id, name) { return 'Cured'; },
		forumAccess: 1,
	},
	waitlist: {
		member: {},
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			$('#content div.content_container tbody tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				if (list.hitlist.member[id]) {
					...
					delete names[id];
					names.num--;
				} else if (list.cured.member[id] || list.apprentice.member[id] || list.family.member[id]) {
					if (list.family.member[id]) {
						++membersWithTitles;
					} else if (list.apprentice.member[id]) {
						++membersApprentice;
					} else if (list.cured.member[id]) {
						++memberscured;
					}
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			if (memberscured) {
				statusMsg += prefix + '<b style="color: red;">' + memberscured + ' without</b>';
				prefix = ', ';
			}
			if (membersWaitList) {
