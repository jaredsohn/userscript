// ==UserScript==
// @name           BrickLink Wanted List Exporter
// @namespace      http://www.smallapple.net/
// @description    Export BrickLink Wanted List
// @author         Ng Hun Yang
...
var ID_PREFIX = "ujs-bl-export-wanted-list-";
...
function loadOptions() {
  var wanted_list = localStorage[STORE_ID + "wanted_list"] || 0;

  var wanted_list_opts = doc.querySelector("#" + SEL_ID);
  wanted_list_opts.value = wanted_list;
  ...
  user_params.win_name = localStorage[STORE_ID + "win_name"] || "wanted_list" + (+new Date());
  }

function storeOptions(opts) {
  localStorage[STORE_ID + "wanted_list"] = opts.wanted_list;
...
var DIV_STYLE = "background-color: #222; border: #fff 1px solid; border-radius: 3px; color: white; font-family: arial,helvetica,sans-serif; padding: 0.3em; position: fixed; top: 0.5em; right: 0.5em";

var SEL_STYLE = "margin: 0.3em";

var BTN_TEXT = "Export Wanted List";
var BTN_STYLE = "border-radius: 3px; cursor: pointer; margin: 0.3em";

var OPTS_STYLE = "border-top: #fff 1px solid; font-size: 80%; margin: 0.5em 0.25em 0.25em";

function createSel(divNode, list) {
  ...
  for(var i = 0; i < list.length; ++i)
    s += dom.emitHtml("option", { value: list[i].value }, list[i].text);
...
function createOpts(divNode) {
  ...
  dom.html(optsNode,
    dom.emitHtml("label", { style: "display: block" }, dom.emitHtml("input", { type: "radio", name: OUT_WIN_OPT_NAME, value: "new" }) + "New") +
    dom.emitHtml("label", { style: "display: block" }, dom.emitHtml("input", { type: "radio", name: OUT_WIN_OPT_NAME, value: "replace" }) + "Replace")  +
    dom.emitHtml("label", { style: "display: block" }, dom.emitHtml("input", { type: "radio", name: OUT_WIN_OPT_NAME, value: "append" }) + "Append") 
    );
...
function createDiv() {
  ...
  createSel(divNode, getWantedLists());
...
function getWantedLists() {
  var wantedListNodes = doc.querySelectorAll("input[name=wantedMoreIDmove]");
  var wantedListNameNodes = wantedListNodes[0].parentNode.querySelectorAll("b");

  var wantedList = [];

  for(var i = 0; i < wantedListNodes.length; ++i)
    wantedList.push({ value: wantedListNodes[i].value, text: wantedListNameNodes[i].textContent });
...
function genWantedList(list, wanted_list) {
  function emit(tag, v) {
    return "<" + tag + ">" + v + "</" + tag + ">";
    }
  ...
  for(var i = 0; i < list.length; ++i) {
    var elm = list[i];
    var str = "";
    str += emit("ITEMTYPE", elm.part_type);
    str += emit("ITEMID", elm.part_id);
    str += emit("COLOR", elm.color);
    str += emit("CONDITION", elm.cond);
    str += emit("MINQTY", escTags(elm.min_qty));

    if(elm.max_price != "")
      str += emit("MAXPRICE", escTags(elm.max_price));

    if(elm.remarks != "")
      str += emit("REMARKS", escTags(elm.remarks));

    if(elm.notify != "N")
      str += emit("NOTIFY", elm.notify);

    if(wanted_list != 0)
      str += emit("WANTEDLISTID", wanted_list);

    out_list.push(emit("ITEM", str));
    }

  return emit("INVENTORY", "\n" + out_list.join("\n") + "\n");
  }
...
function emitWantedList() {
  ...
  var selNode = dom.gE(SEL_ID);
  var wanted_list = selNode.value;
  ...
  var str = genWantedList(getParts(), wanted_list);

  var preNode = new_doc.querySelector("pre");

  if(win_opt == "append" && preNode) {
    str = dom.html(preNode) + escTags(str);
    dom.html(preNode, str.replace(/&lt;\/INVENTORY&gt;\s*&lt;INVENTORY&gt;/g, ""));
    }
  else {
