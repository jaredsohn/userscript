...
// ==UserScript==
// @name          Geocaching Map Linker V2.01
// @namespace     http://cymbaline.us
// @description	 Major hack of Paul Dixon's original script just to get GoogleEarth to work
...
window.enhanceGeocacheListing=function()
{
	...
	// Changes made 05/22/07 by Cymbaline
	//Revised 06/03/07
	
	var rawlat = document.getElementById("Location").innerHTML;
	var pattern = /lat=(-?[\d\.]+).*lon=(-?[\d\.]+)/;
	var matches=rawlat.match(pattern);
    if (matches)
	{
		...
		var googleout=linkGMap+" "+linkGSat+" "+linkGEarth; 
		var LatLon = document.getElementById('lnkConversions');
		if (LatLon)
		{
			var addGoogle = document.createElement('span');
			addGoogle.innerHTML='<br>'+googleout+'<br>';
			LatLon.parentNode.insertBefore(addGoogle, LatLon.nextSibling);
		} 
	}
	else
	{
		alert('NO MATCHES for GOOGLE EARTH - check for newer greasemonkey script or disable this script!');
	}
	...
	if (spanLong)
	{
		...
		longdesc=longdesc.replace(/([ns])\s*(\d+)[°\s]+(\d+\.\d+)\s+([we])\s*(\d+)[°\s]+(\d+\.\d+)/ig,
