var meta = <><![CDATA[
// ==UserScript==
// @name           Mousehunt KR 30 Mnt - With are Script's
// @namespace      http://userscripts.org/scripts/show/81693
// @description    Longtail AutoHorn Just add from Are Script's
// @version        3.03
...
var minHornDelay = 90;   // in seconds, minimum waiting time from horn valid until we sound the horn
var maxHornDelay = 300;  // in seconds, maximum waiting time from horn valid until we sound the horn
...
if (isOnlen == -1) {
	...
	if (isKR == 'true') {
		...
		setTimeout(function() { document.location = 'http://apps.facebook.com/mousehunt/'; } , timeoutvalue * 1000);
                tulisWaktu(-1);
		soundAlarm();
	} else {
