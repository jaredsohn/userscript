...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		//citysel += '<option value=All>Tutte le Città</option>';
	...
	ShowViewAttacksDialog:function() {
		...
		levelsel += '<option value="All">Tutti i Livelli</option>';
		for(var c=1; c<=11; c++) {
		...
		typesel += '<option value="All">Tutti</option>';
		typesel += '<option value="normal">Attacchi</option>';
		typesel += '<option value="transport">Trasporti</option>';
		typesel += '<option value="bulkadded">Aggiungi tutte</option>';
		typesel += '</select>';
		...
		<OPTION value=Camp>Campi Barbari</option>\
		<OPTION value=Wilderness>Terre Selvagge</option>\
		<OPTION value=Grassland>Praterie</option>\
		<OPTION value=Lake>Laghi</option>\
		<OPTION value=Mountains>Montagne</option>\
		<OPTION value=Forest>Selve</option>\
		<OPTION value=Hills>Colline</option>\
		<OPTION value=Plain>Pianure</option>\
		<OPTION value=City>Città</option>\
		<OPTION value=Transport>Trasporti</option>\
		<OPTION value=Unknown>Sconosciuto</option>\
		</select></td>\
		<td class=xtab width=100 align=center>Seleziona Città: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Seleziona Livello: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Seleziona Tipo: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>Chiudi</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Lista Attacchi</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Svuota Lista</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Elimina Selezionati</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Città</td><td>Coordinate</td><td nowrap>What</td>';
		  h += '<td>Tipo</td><td>Truppe Attacco</td><td>Truppe Suicida/Risorse</td><td>Distanza</td><td>Time</td><td>Ignora</td></tr>';
		  var tableRows= '';
		  var count = 1
		  for(var a=0; a<t.attacks.length; a++) {
			...
			tableRows += '<td nowrap>'+type+' Livello '+levelInfo.level+'</td>';
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Ordine Attacchi: <select id='KOCAttackOrder'><option value='mostTroops'>Prima più truppe</option><option value='closest'>Prima obiettivi vicini</option></select><br />"+
			"Tipo d\'attacco: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Campi barbari "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Terre selvagge "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Città "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Trasporti<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> secondi da un attacco all\'altro<br />"+


			"Ritardo tra attacchi <u>allo stesso obiettivo</u>...<br />"+

			"<div style='margin-left: 40px'>"+
			"Attacchi ai campi: <input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />hrs<br />"+
			"Attacchi alle terre selvagge: <input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />hrs<br />"+
			" Attacchi alle città: <input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />hrs<br />"+
			" Transporti: <input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />mins<br />"+
			" Durante i trasporti, tenta di mantenere almeno <input id='KOCAttackTransportReserveAmount' value='"+this.options.transportResourcesReserveAmount+"' size='10' /> di ogni risorsa in ciascuna città <font color=#FF0000>(NEW)</font>"+
			"</div>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% aggiustamento casuale di tutti i ritardi (per apparire più umani).<br />"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distanza massima dalla città da attaccare/rifornire.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Invia gli attacchi solo dalla città da cui sono stati lanciati per la prima volta.<br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> De-seleziona il cavaliere quando non sei in modo attacco.<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Ritenta la marcia in caso di errore sconosciuto/eccesso di traffico (riavvia Camelot dopo aver modificato questa opzione).<br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Ferma gli attacchi automatici in caso di pericolo imminente.<span style='color:red'> (Gli attacchi non ripartiranno dopo l\'attacco. Usare a proprio rischio e pericolo)</span><br />"+			
			"Apri questo URL (link a file audio o video) in una nuova tab quando si è sotto attacco:<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Rimuovi automaticamente i resoconti degli attacchi a barbari e terre selvagge.<br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Rimuovi automaticamente i resoconti degli attacchi alle farm.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> numero massimo resoconti di attacchi da conservare nella lista degli attacchi.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Disabilita la visione dei resoconti, questa opzione disabilita anche la raccolta dei resoconti nella pagina degli attacchi.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% di popolazione inattiva disponibile prima di addestrare automaticamente <input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' />% della massima disponibile.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Per abilitare l\'addestramento automatico, abilitare il pulsante \'addestra auto\' sotto il tipo di truppa nelle caserme.</span><br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Nota: Nel caso si addestri il 100% della popolazione massima, è possibile incontrare errori sulle risorse legati alla gestione cache del gioco.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gold_30.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% felicità prima di aumentare oro automaticamente.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Per abilitare l\'aumento automatico dell\'oro, abilitare il pulsante \'Auto-On\' dalla finestra del riepilogo del proprio castello.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> Disabilitare il popup molesto \"Invite Friends\".<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Aiuta automaticamente i membri dell'alleanza con costruzioni e ricerche.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Nascondi le richieste e i resoconti di aiuto degli alleati dalla chat (se il precedente è abilitato, dopo aver aiutato).<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Pubblica automaticamente i popup del gioco (come le richieste d\'aiuto) sulla bacheca di facebook.<br />"+
			"Se il precedente è selezionato, che impostazioni di privacy devono essere usate? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tutti</option><option value='50'>Amici di amici</option><option value='40'>Solo amici</option><option value='10'>Soltanto io</option></select><br />"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Log In automatico nel dominio se disconnesso per manutenzione o down-time del server.<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Abilita i log diagnostici nella console dei messaggi d\'errore di Firefox (utile se si sta cercando di correggere un problema o se si sta sottoponendo dei dettagli assieme ad un bug report).<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> secondi prima di cambiare città.<br />"+
			"Cicla su tutte le città <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> volte e quindi attendi "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />secondi prima di effettuare il refresh.<br />"+
			"Città da cui attaccare: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"Importa/Esporta: Incolla o copia qui le impostazioni...<br /><textarea id='KOCAttackImport'></textarea>"+
			"<a class='button20' id='KOCAttackImportButton'><span>Importa</span></a> <a class='button20' id='KOCAttackExportButton'><span>Esporta</span></a><br />"+
			"</td></tr>"+

			"</table>"+
			
			"<TABLE width=100%><TR><TD>"+
			"<a id='KOCAttackOptionsSave' class='button20'><span>Salva</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Annulla</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Reset opzioni</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Resetta tutte!</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Elimina tutti gli attacchi salvati</span></a>"+
			"<a id='KOCAttackDeleteTransports' class='button20'><span>Elimina i trasporti</span></a>"+
			//"<a id='KOCAttackListAttacks' class='button20'><span>List attacks</span></a><a id='KOCAttackListTransports' class='button20'><span>Elenco trasporti</span></a>"+
	...
	AddViewAttacksLink:function() {
		...
		a=this.AddTabLink('Vedi Attacchi');
		if(!a) {
			window.setTimeout(function() {
	...
	*/
	...
	SetAttackStatusMessage:function() {
		...
		if(autoAttack) {
			toggle.innerHTML='<span>Attacchi Auto - On</span>';
		} else {
			toggle.innerHTML='<span>Attacchi Auto - Off</span>';
		}
	},
	...
	*/	
	...
	IsFirstAttackAtLocation:function(x,y) {
		...
		if(!attack){
			...
			this.Log("Attacco non definito nel sistema. Questo è il primo attacco!");
		...
		for (var march in current_marches) {
			if(current_marches[march]['toXCoord'] == x && current_marches[march]['toYCoord'] == y){
				...
				this.Log("Sono presenti truppe già in marcia per ("+x+","+y+") nelle attività di truppa.");
				...
				*/
				if(attack.suicidewavetime && attack.suicidewave){
					...
					if(elapsedTime > 30 && attack.time < attack.suicidewavetime){
						...
						this.Log("Attacco suicida inviato da più di 30 secondi e attacco normale non ancora inviato. L\'attacco suicida deve essere re-inviato! Questo è il primo attacco inviato a  ("+x+","+y+")!");

						return true;
					}
				}
				this.Log("Questo non è il primo attacco inviato a ("+x+","+y+").");
				return false;
				break;
			}
		}
		...
		*/
		...
		this.Log("Le truppe non stanno attualmente marciando verso ("+x+","+y+") nelle attività di truppa. Questo è il primo attacco.");
	...
	GetGuiCoords:function() {
		...
		if(!x || !y) {
			this.Log("Non riesco a trovare le coordinate");
		...
		if(x.value.length<1 || y.value.length<1){
			...
			this.Log("Una o entrambe le coordinate sono mancanti!");
	...
	SetAttackFromGuiXY:function(x,y,box,isSuicideWave,locationType,resetTime) {
		...
		if(resetTime==true){
			this.Log("Re-impostazione orari di avvio perl\'attacco a ("+x+","+y+")");
		}
		...
		if(marchType==null) {
			...
			throw("Impossibile trovare tipo di attacco");
		...
		if(totalTroops<=0) {
			...
			this.Log("Nessuna truppa, attacco non salvato");
		...
		if(marchType==0 && locationType!="Transport") {
			...
			if(isSuicideWave) {
				this.Log("Attacco suicida determinato dalla checkbox");
				this.Log("Attacco suicida:"+troops);
		...
		for(var r=0; r<coordRows.length; r++) {
			...
			if(attack && !force) {
				if(SuicideAttackDefined && isSuicideWave==true){
					...
					this.Log('Attacco suicida verso '+x+','+y+' già aggiunto. Provare a sovrascrivere.');

					continue;
				}else if(attack.troops){


					this.Log('Attacco normale verso '+x+','+y+' già aggiunto. Provare a sovrascrivere.');
			...
			if(attack) {
				...
				if(SuicideAttackDefined || isSuicideWave || previous_suicidewave){
					...
					if(previous_suicidewave && (!SuicideAttackDefined || !isSuicideWave)){
						...
						this.Log("L\'ondata suicida precedente per questo attacco è stata sovrascritta. Unisco e ripristino...");
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"Copia e incolla qui le coordinate (es. 343,434) una per riga...<br />Nota: l\'obiettivo sarà aggiunto utilizzando il numero di truppe selezionato in questa schermata.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> <label for=\"KOCAttackBulkAddForce\">Sovrascrivi attacco esistente se uno è già presente</label><br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Tipo di obiettivi (Tutte le coordinate devono essere di questo tipo):<br />");
		var arrData = [["Campo", "Campi"],["Città", "Città"],["Trasporto", "Trasporto"],["Terra Selvaggia", "Terre Selvagge"]];
		...
		//AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' /> Questa è un\'ondata suicida iniziale per distruggere le trappole in una terra selvaggia.<br />");
		...
		AddHtml(bulkAddSuicideSpan,'<label for="KOCAttackBulkAddSuicideWave">Questa è un\'ondata suicida iniziale per distruggere le trappole in una terra selvaggia.</label>');
		...
		bulkAdd.innerHTML='<span>Aggiungi Tutte</span>';
		...
		bulkAddAttackLink.innerHTML='<span>Aggiungi coord. in massa</span>';
	...
	HideAttackEfforts: function () {
		...
		a.innerHTML = 'Nascondi Attacchi/Bonus velocità';
	...
	SetResourceInput:function(num,resourceCount) {
		...
		if(parseInt(resourceCount)>0) {
			if(resource.value!=resourceCount) {
				this.Log('Impossibile impostare il numero di risorse:'+num+',desiderate:'+resourceCount+', numero:'+resource.value);
	...
	OnCastleBoxAppear:function(box) {
		...
		function SetAbandonWildsA() {
			var str=t.GetAbandonWilds()?'Abbandona Terre - On':'Abbandona Terre - Off';
			b.innerHTML='<span>'+str+'</span>';
		}
		b.addEventListener('click',function() {
			var abandonWilds=t.GetAbandonWilds();
			if(!abandonWilds){
				var abandonWildsConfirm = confirm("Sei sicuro di voler abbandonare  automaticamente tutte le terre selvagge?\n")
	...
	OnAttackBoxAppear:function(box) {
		...
		addBtn.innerHTML='<span>Aggiungi Attacco</span>';
		...
		// radio boxes for defining bulk coordinate type
		var arrData = [["Camp", "Campo"],["City", "Città"],["Transport", "Trasporto"],["Wilderness", "Terra Selvaggia"]];
		for (var i=0; i < arrData.length; i++){
			var objRadItem = document.createElement("input");
			objRadItem.type = "radio";
		...
		AddHtml(suicideSpan,'<label for="KOCAttackSuicideWaveCheckbox">Questa è un\'ondata suicida</label>');
		...
		if(attack) {
			...
			if(attack.time) {
				AddHtml(div,'Ultimo attacco: '+SecsToStr(nowSecs-attack.time)+' fa<br />');
			}
			if(attack.comment){
			...
			if(attackTypeSelected==0 && attack.type==0) { // if 'attack' mode
				...
				if(this.prevAttack) { this.Log('Attacco precedente:'+this.prevAttack.x+'=='+xy[0] +','+this.prevAttack.y+'=='+xy[1] ); }
	...
	//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
			...
			deleteBtn.innerHTML='<span>Elimina Attacco</span>';
		...
		if(autoAttack && autoAttack.x==xy[0] && autoAttack.y==xy[1] && !ignore.checked) {
			...
			&& btnMarch.className.indexOf('grey')<0
				...
				if(!this.nextAutoAttackTimeout) {
					...
					this.Log('Impossibile proseguire attacchi automatici, sto per attaccare o cambiare città');

				}
				return;
			} else {
				this.Log('Impossibile attaccare '+xy[0]+','+xy[1]+', cavalieri disp.:'+(knightSelect.options.length-1)+', Truppe insuff.: '+notFullTroops+'(need '+totalTroops+'). Risorse insuff.:'+notFullResources+' (need '+totalResources+')');
				this.DoUnsafeWindow("Modal.hideModal();");
	...
	*/
	...
	AddTrainTroopsLink:function() {
		...
		var setTrainTroopsA=function() {
			...
			a.innerHTML='<span>'+(trainTroops[t.GetCurrentCityId()]==type?'Addestra Auto - On':'Addestra Auto - Off')+'</span>';
		}
		a.addEventListener('click',function() {
			var trainTroops=JSON2.parse(t.GetValue('TrainTroops','{}'));
