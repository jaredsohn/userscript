	...
	$.on('ru',
		  function (data) {
			  return {
				  ...
				  settings: 'Настройки',
				  bookmarks: 'Закладки',
				  bookmarksTitle: 'Пеночка — Закладки',
				  settingsTitle: 'Пеночка — Настройки',
				  fromBms: 'Из закладок',
				  toBms: 'В закладки',
...
;(function ($) {
   ...
   function setVal(name, value, local) {
      var path = local ? location.pathname : '';// : '/' <-- wtf?
      if (storage) {
   ...
   function getVal(name) {
      if (storage) {
         var ret = storage.getItem(name + '') // '/') <-- wtf?
         if (!ret) {
...
;(function ($) {
   ...
   $.on(
      ...
      function (data) {
         ...
         if (moar.length) {
            moar[0].innerHTML = moar[0].innerText.replace(/\..*$/,'.')
            tflags['moar']=1
         }
         if (!trm.length) {
...
function $x(xpath, context, callback){ 
	var nodes = []; 
	try { 
		var doc = context || document; 
		var results = document.evaluate(xpath, doc, null, XPathResult.ANY_TYPE, null); 
		var node; 
		while (node = results.iterateNext()){
			callback?callback(node):nodes.push(node);
		}
	} catch (e) {  console.log([e,xpath])} 
	return nodes; 
}
function time(){
...
var size=8;
var level=8192;
var dr_canvas = document.createElement("canvas");
dr_canvas.style.width=size+"px";
dr_canvas.style.height=size+"px";
var dr_context=dr_canvas.getContext("2d");
var filters=JSON.parse(window.localStorage.img_filters||"[]");

function get_img_data(img){
	dr_context.drawImage(img,0,0,8,8);
	var data=dr_context.getImageData(0,0,8,8).data;
	var a=[];
...
function compare(a1,a2){
	var diff = 0;
	var cnt=Math.min(a1.length, a2.length);
	for(var i=0;i<cnt;i++){
...
function same_aspect(wh1,wh2){try{
	wh1=wh1.split("x");
	wh2=wh2.split("x");
	var diff=Math.abs((wh1[0]*wh2[1]-wh2[0]*wh1[1])/(wh1[0]*wh2[1]+wh2[0]*wh1[1]+1));
	if(diff<0.01){
		return 1;
	}else{
		return 0;
...
function add_filter(sz,img){
	filters.push({"wh":sz,"filter":get_img_data(img)});
	window.localStorage.img_filters=JSON.stringify(filters);
}

function img_test(imag){
	if(imag.event)imag=imag.event.target;
	if(imag.target)imag=imag.target;
	if(!$){
		imag.addEventListener("click",img_clc,0);
	}
	filters.map(function(f){ try{
		var em=$x("../../span[@class='filesize']/em",imag);
		var m=/(\d+x\d+)/.exec(em[0].textContent);
		if(m[1]!=f.wh&&!same_aspect(m[1],f.wh))return 1;
		if(compare(get_img_data(imag),f.filter) < level){
			var post=imag;
			while(post.tagName!="TABLE"&&post.tagName!="SPAN"){
				post=post.parentNode;
			}
			if($){
				$.to("toggle post",{"e":$(imag)});
			}else{
...
console.log(e);
	...
	if(e.shiftKey && e.ctrlKey){
		e.preventDefault();
		var em=$x("../../span[@class='filesize']/em",e.target);
		var m=/(\d+x\d+)/.exec(em[0].textContent);
		add_filter(m[1],e.target);
		console.log(JSON.stringify([m[1],get_img_data(e.target)]));
	}
}
...
function DO(){
	var t=time();
//	console.log(t);
	$x("//img[@class='thumb']",document).map(img_test);
	var t=time()-t;
//	console.log(t);
}
if($){
	function showconfig(){
		var cfg_area=$('<div style="position:absolute; width:500px;height:300px; top:100px;left:100px; border:1px solid black;">'
			+'<textarea rows=12 cols=50>asfsadf</textarea>'
			+'<input type="button" value="save"></div>');
		$(document.body).append(cfg_area);
		var textarea=cfg_area.find("textarea")[0];
		var button=cfg_area.find("input")[0];
		textarea.value=window.localStorage.img_filters;
		$(button).click(function(){
			window.localStorage.img_filters=textarea.value;
			cfg_area.remove()
		});
	}

	$(document).bind('keydown', 'Ctrl+Shift+h' , showconfig);
	document.addEventListener("click",function(e){try{
		if(e.target.tagName=="IMG")
			img_clc(e);
	}catch(e){console.log(e)}},1);
	$.on(["post","filter"],function(data){
		var img=data.e.find("img");
		if(img.length>0){
			e=img[0]
			if(e.complete){
				img_test(e);
   ...
   $.timer = {
	...
	$.on('loaded', function (data) {
      ...
      setTimeout(function() {
         ...
         $($.iom.footer).
            before('<a href="http://github.com/anonymous32767/Penochka/" title="version: UnStAbLe, ' + $.timer.cache + ' total: ' + $.timer.total + 'ms">penochka</a> + ')
      },0)
		return data
	})
