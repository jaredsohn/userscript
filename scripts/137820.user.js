...
var Emotiworld = new function() {
	...
	this.languageData = {
		'en':{
			manage:'Jelek kezelése',
			add:'Hozzáadás',
			findIcons:'',
			close:'Kilép',
			replyToAll:'Válasz mindenkinek',
			CRUndoTitle:'CR konvertáló'
		}
	};
	...
	this.ikariamInit = function() {
		...
		if(unsafeWindow.ajax && unsafeWindow.ajax.Responder) {
			...
			// Ikariam v0.4.5
			// toolbar insertion
			Emotiworld.textarea = Emotiworld.$("textarea#text, textarea#message");
	...
	this.setReplyToAll = function(diplo) {
		var lang = Emotiworld.languageData[Emotiworld.lang];
		Emotiworld.$('td.reply a.button[href^="?view=sendIKMessage"]', diplo).each(function(index) {
			var href = Emotiworld.$(this).attr('href').replace(/&receiverId=\d+/, '&msgType=51&allyId='+unsafeWindow.ikariam.model.avatarAllyId);
			Emotiworld.$(this).after('<a class="button" href="'+href+'" onclick="ajaxHandlerCall(this.href);return false;">'+lang.replyToAll+'</a>');
		});
	}
	...
	this.postInit = function() {
		Emotiworld.$("div.emotiworldIcon").click(Emotiworld.insert);
...
//		Emotiworld.$( "#emotiworldSortable" ).disableSelection();	// ne marche pas
	...
	this.save = function() {
		...
		Emotiworld.$('#emotiworldIcons').html(Emotiworld.toolbar(newIcons, false));
		Emotiworld.$("div.emotiworldIcon").click(Emotiworld.insert);
	};
