...
// @author 	congxz6688
// @version 	1.2.1.0
// @description This is my first script, it is unfinished. I shall update it step by step.^_^  support English/CN/TW!
...
switch (Xlang) {
	case "cn":
		...
		TaskKind = ["升级", "新建", "攻击", "研发", "训练", "运输", "平仓", "拆除", "活动"];
		...
		taskoftext = ["预定升级", "预定新建", "资源自动升级", "尚未开启", "马上开启", "已经开启", "点击关闭", "该村资源田分布", "自动运输", "自动运输尚未设定", "已设定", "运送成功", "任务列表", "运输限额", "默认", "更改", "木/泥/铁", "粮食", "预定拆除", "预定发兵", "攻击类型", "到达所需时间", "重复次数", "间隔时间", "00:00:00", "投石目标", "随机", "未知", "次", "月", "日", "部队已发出","预定训练","训练设施","训练任务已执行","定制运输","设定页面刷新间隔","页面刷新的间隔时间，是指隔多久执行一次页面的自动载入。\n此时间过短，会增加被系统侦测到的危险，过长则影响任务执行的效率。\n默认为20分钟，请输入新的时间间隔：\n\n"];
		errorText = ["资源不足", "已经有建筑在建造中", "建造完成", "将马上开始全部建造", "在开发中", "建造所需资源超过仓库容量上限,请先升级你的仓库", "建造所需资源超过粮仓容量上限,请先升级你的粮仓", "资源何时充足时间提示","粮食产量不足: 需要先建造一个农场"];
	...
	case "tw":
		allbuildwithid = ["伐木場", "泥坑", "鐵礦場", "農場", "農田", "鋸木廠", "磚廠", "鋼鐵鑄造廠", "麵粉廠", "麵包店", "倉庫", "穀倉", "鐵匠", "盔甲廠", "競技場", "村莊大樓", "集結點", "市場", "大使館", "兵營", "馬廄", "工場", "研究院", "山洞", "村會堂", "行宮", "皇宮", "寶物庫", "交易所", "大兵營", "大馬廄", "城牆", "土牆", "木牆", "石匠舖", "釀酒廠", "陷阱機", "英雄館", "大倉庫", "大穀倉", "世界奇觀", "放牧水槽"];
		addTaskText = ["添加任務", "任務類型", "所在村", "任務對象", "目標", "模式", "支援建設", "資源集中", "上移", "下移", "刪除", "任務內容", "移動", "清除所有任務"];
		TaskKind = ["升級", "新建", "攻擊", "研發", "訓練", "運輸", "平倉", "拆除", "活動"];
		...
		gametext = ["等級", "商人", "坑號", "主村", "執行時間", "該時間設置尚未啟用", "到", "村莊", "運送", "回來", "運送到", "從", "由", "資源", "建築", "建造新的建築物", "空", "等級"];
		raceName = ["羅馬人", "條頓人", "高盧人"];
		taskoftext = ["預定升級", "預定建築", "資源自動升級", "尚未開啟", "馬上開啟", "已經開啟", "點擊關閉", "該村資源分布", "自動運輸", "自動運輸尚未設定", "已設定", "運送成功", "任務列表", "運輸限額", "默認", "更改", "木/磚/鐵", "穀物", "預定拆除", "預定發兵", "攻擊類型", "到達所需時間", "重複次數", "間隔時間", "00:00:00", "投石目標", "隨機", "未知", "次", "月", "日", "部隊已發出","預定訓練","訓練設施","訓練任務已執行","定制運輸","設定頁面刷新間隔","頁面刷新的間隔時間，是指隔多久執行一次頁面的自動載入。\n此時間過短，會增加被系統偵測到的危險，過長則影響任務執行的效率。\n默認為20分鐘，請輸入新的時間間隔：\n\n"];
		errorText = ["資源不足", "已經有建築在建造中", "建造完成", "將馬上開始全部建造", "在開發中", "建造所需資源超過倉庫容量上限,請先升級你的倉庫", "建造所需資源超過糧倉容量上限,請先升級你的糧倉", "資源何時充足時間提示"];
		...
		allbuildString = "鋸木廠磚廠鋼鐵鑄造廠麵粉廠麵包店倉庫穀倉鐵匠盔甲廠競技場村莊大樓集結點市場大使館兵營馬棚工場研究院山洞城鎮廳行宮皇宮寶物庫交易所大兵營大馬廄城牆土牆木牆石匠舖釀酒廠陷阱機英雄館大倉庫大穀倉世界奇觀放牧水槽";
	...
	case "pt":
		...
		addTaskText = ["Adicionar tarefa", "Estilo", "Aldeia Activa", "Alvo", "Para", "Modo", "Ajuda na Construção", "Quantidade de recursos", "Mover para cima", "Mover para baixo", "Apagar", "&#160;&#160;&#160;Tarefa de conteúdos", "Mover ", "Eliminar todas as tarefas"];
		TaskKind = ["Evolução", "Novo construção", "Ataque", "Pesquisa", "Treino", "Transporte", "NPC", "Demolição", "Celebrção"];
		maxlevel = ["10", "10", "10", "10", "0", "5", "5", "5", "5", "5", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "10", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "20", "10", "20", "20", "20", "20", "100", "20"];
		gametext = ["Lvl", "Mercadores", "Identificação", "Capital", "Inicio", "this timeseting is unuseful now.", "Para", "Aldeia", "transporte", "de", "Transporte para", "Transporte de", "Regresso de", "Recursos", "Edifiçio", "Construir novo edifício", "Vazio", "Nivel"];
		raceName = ["Romanos", "Teutões", "Gauleses"];
		taskoftext = ["Agendar Evolução", "Agendar nova construção", "AutoResUpD", "Not_run", "Iniciar", "Iniciado", "Suspender", "A distribuição dos recursos campos desta vila é ", "Autotransporte", "Autotransporte não está aberto", "Aberto", "Transporte com sucesso", "Lista de agendamento", "Trans_limit", "Default", "Modificar", "madeira/barro/ferro", "Cereal", "Agendamento de demolição", "Agendamento de ataque", "Tipo de ataque", "Tempo de viagem", "Repetir número de vezes", "Tempo de intervalo","00:00:00","Alvo catapulta","Aleatório", "Desconhecido", "Vezes", "Mês", "Dia", "Tropas enviadas","Agendamento de treino","Train ubication","Agendamento de treino feito","Transporte personalizado"];
		errorText = ["Poucos recursos.", "Os trabalhadores já estão a trabalhar.", "Construção cpmpleta", "Inicio da construção", "Em desenvolvimento", "Seu Armazém é pequeno. Evolua o seu armazém para continuar a sua construção", "Seu Armazém é pequeno. Evolua o seu celeiro para continuar a sua construção", "Recursos suficientes"];
		otherText = ["Nota importante", "Apenas os campos de recursos da capital <br>podem ser levados a nivel 20 . A sua capital <br> nao está detectavel. Por favor visite o seu perfil.", "Atalho aqui ^_^", "Instalação concluída", "Cancelado", "Iniciar as tarefas", "Upgrade com sucesso", "Executar com sucesso", "Sua raça é desconhecida, e o seu tipo de tropa também. <br>Visite o seu perfil para determinar as raça.<br>", "Por favor visite a sua mansão do heroi para determinar<br> a velocidade e o tipo de heroi."];
		allsourceString = "BosquePoço de BarroMina de FerroCampo de Cereais"
		resources=["Madeira","Barro","Ferro","Cereal"];
		...
		troops[2] = ["Falange", "Espadachim", "Batedor", "Trovão Theutate", "Cavaleiro Druida", "Haeduano", "Aríete", "Tranbuquete", "Chefe de clã", "Colonizador", "Heroi"];
