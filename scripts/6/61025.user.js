...
///////////////-----------------Agregar Emoticones------------------////////////////////
	...
	if($('div[typeof="foaf:Person"]').find('a[href="/perfil/lucakpo09"]').length != 0){
		var mod_sharkale = $('div[typeof="foaf:Person"]');
		mod_sharkale.html(mod_sharkale.html().replace(/New Full User/gi,'Zombie').replace(/No Rankeado/,'T! Rank: 1').replace(/ Puntos/,'10000 Puntos'));
		$('img[title="Zombie"]').attr('style','position: absolute; top: -703px; clip: rect(703px, 16px, 720px, 0px);').removeAttr('alt');
		mod_sharkale = $('img[title="Zombie"]').parent().attr('id','icon_shark').clone();
