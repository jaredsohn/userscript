...
const c_AMEspPRecMin = 'Espionajes con más recursos que esta cantidad';
...
const c_autoUpdate = 'Actualizar automáticamente';
...
if (getScriptConf('specialEffects')) {
	function hide(path, list, level) {
		...
		for (var i in pos) {
			...
			object.snapshotItem(pos[i]).style['width']=String(level*100) + '%';
...
//						alert('Acceso denegado, comprueba tu nombre de usuario y tu password');
						...
						nodo.innerHTML+="<img title='Error: la página no existe' src='data:image/gif;base64,R0lGODlhDgAOAMIGAL+/vwAAAP9EH+ozIOJtWZ02Ev///////yH5BAEAAAcALAIAAgAKAAsAAAMqeBoc+oFI4RYxRIjh4p0bI2TZYDKkthWiag5soA0bzB7Ba1e4fT+4xiMBADs=' >";
...
//					alert('La url especificada no es correcta');
							...
							nodo.innerHTML+="<img title='Error: url incorrecta o código desconocido' src='data:image/gif;base64,R0lGODlhDgAOAMIGAL+/vwAAAP9EH+ozIOJtWZ02Ev///////yH5BAEAAAcALAIAAgAKAAsAAAMqeBoc+oFI4RYxRIjh4p0bI2TZYDKkthWiag5soA0bzB7Ba1e4fT+4xiMBADs=' >";
							if (getScriptConf('debugMode')) alert(details.responseText);
						}
					}
				},
...
if (location.pathname=='/game/leftmenu.php') {
	...
	unsafeWindow.makeConfig = function () {
		...
		function newNestledTable(id, depend, title, colspan, parentColspan) {
			return "<tr id='"+id+"' align='center' "+(!getScriptConf(depend)?"style='display:none'":'')+"><td colspan='"+parentColspan+"'><table width='95%' colspan="+colspan+" >" +
...
//			(table==''?" />":" onclick='if (this.checked) document.getElementById(\""+table+"\").style[\"display\"]=\"\"; else hideId(\""+table+"\")' />") +
			"</th><th>" + desc + "</th></tr>";
		}
		...
		var size = 5;
		var cad = "<br><h1>Script <a href='http://userscripts.org/scripts/show/5550' target='_blank' style='color:lightblue'>oGame++</a> by Unbrained <small title='click para buscar actualizaciones' onclick='checkUpdates(true)' style='color:orangered; cursor:pointer'> v"+getDefScriptConf('version')+"</small></h1>" +
		...
		cad += "<tr style='display:none' name='allyTag'><th><input type='button' value='-' title='Eliminar etiqueta' " +
			...
			"<th><input size='10' type='text' name='ATColor' value='' onchange=\"this.parentNode.nextSibling.firstChild.style['color']=this.value\" /></th>" + //onchange=\"this.parentNode.style['background']=this.value;this.style['color']=this.value\" /></th>" +
    ...
    setSession(location.hostname, nodo.innerHTML.substring(indSession+8, indSession+20));
		...
		coordTable.innerHTML = nodo.previousSibling.previousSibling.cloneNode('true').innerHTML +
					"<tr><td><div align='center'><font color='#ffffff'><a onclick='addCoords()' style='cursor:pointer'>Añadir coordenadas</a></font></div></td></tr>" +
					"<tr><td><div align='center'><font color='#ffffff'><a onclick='delAll()' style='cursor:pointer'>Eliminar coordenadas</a></font></div></td></tr>" +
					"<tr><td><div align='center'><font color='#ffffff'><a onclick='makeCoordListPage()' style='cursor:pointer'>Listar coordenadas</a></font></div></td></tr>";
/*		y.addEventListener('click',addCoords,true);
		x.appendChild(y);
		x.appendChild(document.createElement('BR'));
		y = document.createElement( 'INPUT' );
		y.setAttribute('type','button');
		y.value='Eliminar coordenadas';
		y.addEventListener('click',delAll,true);
		x.appendChild(y);
		x.appendChild(document.createElement('BR'));
		y = document.createElement('INPUT');
		y.setAttribute('type','button');
		y.value='Listar coordenadas';
		y.addEventListener('click',makeCoordListPage,true);
		x.appendChild(y);
*/		nodo.parentNode.parentNode.parentNode.appendChild(coordTable);
 ...
 //winref = window.open('http://www.google.es', 'coas');
...
//			GM_log(distancia);
	...
	var PMetal = T_Recursos[33].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');
	var PCristal = T_Recursos[34].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');
	var PDeut = T_Recursos[35].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');

	var AlmM = T_Recursos[28].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');
	var AlmC = T_Recursos[29].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');
	var AlmD = T_Recursos[30].innerHTML.replace(/.*>(.*)<.*/g,'$1').replace(/\./g,'');
...
//		document.getElementsByTagName('tbody')[4].setAttribute('onchange', 'alert(1)');
	...
	if (getScriptConf('galaxyTool')) {
		...
		for (var i=0; i<15; i++) {
			...
			if (pName.replace(/\s/, '')!='') { // si no hay nombre de planeta no tiene sentido comprobar el resto de campos
...
//		var content=document.getElementsByTagName("table")[5].innerHTML;
	...
	function parse(nodo) {
		...
		if (cad=='Láser pequeño') return "&numunits[1][0][l_l]="  + nodo.nextSibling.innerHTML;
		if (cad=='Láser grande') return "&numunits[1][0][s_l]="  + nodo.nextSibling.innerHTML;
	...
	element = document.getElementsByTagName('table')[5].getElementsByTagName('tr');
