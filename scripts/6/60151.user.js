// ==UserScript==
// @name           Times to Wikipedia citation
// @namespace      Wikipedia
// @include        http://*.timesonline.co.uk/tol/*.ece*
// @include        http://timesonline.typepad.com/*/*
// @include        http://timesnews.typepad.com/*/*
// ==/UserScript==

/*
Description
This Greasemonkey script takes any article page from the Times website and creates a
Wikipedia citation for it.

Edit history
2009-10-19 Creation based on similar script for The Guardian. Note that journalist's blogs are
	not part of timesonline.co.uk and the citation button is not shown on these pages.
	-23 Fix to reference names (cannot be just year)
	-26 Extended to blog pages timesonline.typepad.com and timesnews.typepad.com
...
var utitle='', uauthor='', usummary='', udate='', 
	uyear='', uurl='', uref='', upub='The Times',
	uwidth='650',blogTest=false,
...
based on the Times article you are currently viewing. You can edit the text on this page \
...
<a href=http://userscripts.org/scripts/show/60151 target=_blank>userscripts.org</a>.\
</i></p>\
...
function infoFromPaper() {
    ...
    if(h2!==null){
    	for(var i in h2){
    		...
    		//if(h2[i].className=="sub-heading padding-top-5 padding-bottom-15"){
    			case "sub-heading padding-top-5 padding-bottom-15":
    			h=h2[i].innerHTML;
    			if(h.length>1){
    				usummary=h.replace(/\s+/g," ")
    					.replace(/["]/g,"'")
    					.replace(/[^\w\s\u2019\201C\201D\-';,\.\?§!@\#\$%\^\&\*\_~`]/g,"");
    				i=h2.length;
    			}
    			break;
    			case "date-header": // for blogs this is date
    			if(udate==''){
    				udate=h2[i].innerHTML;
    			}
    			break;
    		}
    	}
    ...
    // Title on blog
    //<h3 class="entry-header">Why we're talking about abortion now</h3>
    var h3=document.getElementsByTagName('h3');
    if(h3!==null && blogTest){
    	for(var i in h3){
    		if(h3[i].className=="entry-header"){
    			h=h3[i].innerHTML;
    			if(h.length>1){
    				utitle=h.replace(/\s+/g," ")
    					.replace(/["]/g,"'")
    ...
    if(divs!==null){
    	for(var i in divs){
    		if(divs[i].className=="article-author"){
    			author=divs[i].innerHTML.replace(/<.[^>]*>/g,"");
    			i=divs.length;
    		}
    	}
    }
    ...
    if(divs!==null){
    	for(var i in divs){
    		if(divs[i].className=="small color-666"){
    			udate=divs[i].innerHTML.replace(/<.[^>]*>/g,"")
    				.replace(/^\s*|\s*$/g,"")
    				.replace(/\s/g," ");
    			i=divs.length;
    			s+=" |date="+udate;
    ...
    // Check which newspaper
    var spans=document.getElementsByTagName('span');
    if(spans!==null){
    	for(var i in spans){
    		switch(spans[i].className){
    			case "byline":
    			var p=spans[i].innerHTML.replace(/<.[^>]*>/g,"")
    				.replace(/^\s*|\s*$/g,"")
    				.replace(/\s/g," ");
    			if(p.match(/Times/)!==null){
    				upub=p;
    				GM_log("upub="+upub);
    			}else{ // Wrong 'byline', probably author
    				GM_log("suspect ="+p+"; uauthor="+uauthor);
    				if(author.length<2 && p.length>6){
    					author=p;GM_log("set author="+p);
    				}
    			}
    			break;
    			case "post-footers":
    			if(author==''){
    				author=spans[i].innerHTML.replace(/<.[^>]*>/g,"")
    				.replace(/posted by /i,"")
    				.split(/ on /)[0];
    			}
    			break;
    ...
    if(author!==null && author.length>0){
    	uauthor=cnvrt2title(author)
    		.replace(/^\s*by |,.*pondent| in .*/ig,"").replace(/\s+/g," ").replace(/^\s*|\s*$/g,"");
    	if(uauthor!==''){
   	    	if(authorSplit(uauthor,0)!=='' && uauthor.match(/Times/)==null){
    ...
    if(uref==''){uref=(authorSplit(uauthor,1)!=='' ? authorSplit(uauthor,1) : "Times" )+uyear};
    s+=" |newspaper="+upub;
    return s;
}
...
function showCitationFromInfo(info, url) {
      ...
      if(test!==null){
      	...
      	var tb = blogTest?node_after(document.getElementById('myCite'))
      		:node_after(document.getElementById('myCite').parentNode); // insert before this div
...
<option value="http://www.wired.com/search?query='+search+'&siteAlias=all">Wired News</option>\
		...
		with(cite.style){
      		...
      		padding='8px';
      		margin='2px';
			marginTop=blogTest?'0':'8em'; // A bit arbitrary, subject to reformat
			width=uwidth+'px';
		}
...
function add_link(text, title, func, id) {
    ...
    var divs = document.getElementsByTagName('div'), bar='';
    if(divs==null){return};
    for(var i in divs){
    	if(divs[i].className=='crumbtrail-prefix'){
     		bar=divs[i];i=divs.length;
     	}
    }
   	if(location.href.match(/times(news|online).typepad.com/)!=null){//trap this to blogs (using url test?)
   		//GM_log("** Testing for crumbtrail id");
   		blogTest=true; // Used by later formatting exclusive to blogs
    	if(document.getElementById('region-content')!=null){
    		bar=document.getElementById('region-content');
   		}
    }
    if(bar==''){return};
...
add_link('<input type=button style="margin-top:5px;background:transparent;" value="Show citation">', 
