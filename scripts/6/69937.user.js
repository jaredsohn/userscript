 ...
 */
	...
	IconHelper.prototype.initIcon = function() {
	    ...
	    div.style.top        = '3px'
	    div.style.right      = '3px'
	    div.style.background = COLOR['on']
	    div.style.color      = '#fff'
	    div.style.width = '10px'
	    div.style.height = '10px'
	    div.style.zIndex = '255'
	    if (this.state != 'enable') {
	        div.style.background = COLOR['off']   
	    }
 ...
 */	
    ...
    function getCustom(aObj, key) {
      ...
      for (var w in words) {
        site = null;
        if (/^\s*{\s*(\S*?)\s*}(.*)$/.test(words[w])) {
          site = RegExp.$1;
          w = RegExp.$2;
          site = site.replace(/[\/\?\.\(\)\+\-\[\]\$]/g, "\\$&").replace(/\*/g, "\.*");
        }
        words[w] = words[w].replace(/\\/g, "\\").replace(/^\s+|\s+$/g, "");
        if (words[w]) {
          if (site) {
            re = eval('/' + site + '/i');
      ...
      for (var i = 0; i < links.length; i++) {
        ...
        if (!text) {
          ...
          for (var img in link.childNodes) {
            if (link.childNodes[img].localName == "IMG") {
              text = link.childNodes[img].alt || link.title || link.childNodes[img].title;
              if (text) break;
            }
          }
          if (!text) continue;
    ...
    function onKeyDown(event) {
      // 不是Ctrl+左右方向建被按下或不到延迟时间则退出
      if (event.shiftKey || event.metaKey || event.keyCode != 37 && event.keyCode != 39 || !event.ctrlKey || delay)
        return
