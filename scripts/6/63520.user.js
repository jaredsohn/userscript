// ==UserScript==
// @name           Craque da Galera - Petkovic 3.0
// @namespace      -
...
var ancora = validacao.getElementsByClassName('botao')[0].childNodes[1]
    ancora.addEventListener('click', votar, true)
	config()
...
function votado()
{
	...
	if(exito==true) item.aumentar()
	else if(exito==false) novaImagem()
    else if(exito==null) return alert(''+
...
function config()
{
    ...
    for(var i=0; i<inp.length; i++)
        if(inp[i].name=='opt') inp[i].value = '1'
}
