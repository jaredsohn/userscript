// ==UserScript==
// @name           Arrival Time Calculator PL version
...
function main(){
  ...
  var portDiv = <>
  								Poziom portu: <input type="text" style="text-align: right;" size="8"  id="portLevelId" value="0"/>  
  							</>;
  p.innerHTML = portDiv;
...
var portlevel = document.getElementById('portLevelId').value;
  ...
  if(isNaN(portlevel)){
  alert('Error: Wpisz tylko cyfre ');
  return false;
  }

  if(portlevel >= port.length || portlevel <= 0){
  alert('Error: Wpisz poziom portu 1-26')
  return false;
  }
...
if(plunder!=-1){
  ...
  if (weight > 0){
    ...
    document.getElementById('loadingtimecalc').innerHTML = "Czas zaladunku: "+loadingtime;
    
    document.getElementById('totaltimecalc').innerHTML = "Czas podrózy: "+totaltime;
...
function updateServerTime() {
    ...
    impacttime.innerHTML = "Godzina dotarcia do celu: "+hours+":"+minutes+":"+seconds;
