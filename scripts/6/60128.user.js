// ==UserScript==
// @name           BBC News to Wikipedia citation
// @namespace      Wikipedia
// @include        http://news.bbc.co.uk/*
// @include        http://www.bbc.co.uk/*
// @description    Take any article page from the BBC website and create a Wikipedia citation for it.
// ==/UserScript==
/*
Description
This Greasemonkey script takes any article page from the BBC website and creates a
Wikipedia citation for it.

Edit history
2009-10-17 Creation based on similar script for The Guardian.
	-21 Fix firstname/lastname split, added droplist searches.
	-22 Fix 3-state ref button and remove 'less' button.
	-23 Enhance ref and layout buttons and add width to cite box.
2009-11-02 Better dates
...
// Global variables
var uwidth=966+8,	// Screen width based on current BBC css standards
	utitle='', uauthor='', usummary='', udate='', 
	uyear='', uref=''; uurl='', uref='',
	now=new Date(), monthNames=new Array(
	"January","February","March","April","May","June","July",
	"August","September","October","November","December");

var psty="style=margin-top:0.75em;font-size:10pt;font-family:'Arial,Tahoma';",
	helpText='<p style=font-size:12pt;><b>Instructions</b></p>\
<p '+psty+'>The citation above is automatically populated \
based on the BBC News or web article you are currently viewing. You can edit the text on this page \
...
<div style=margin-left:2em;margin-top:0.75em;font-size:10pt;font-family:"Arial";>\
...
<p '+psty+'margin-top:1.5em;color:grey;>Release date of this script: '+version+'</p>';
...
function infoFromPaper() { // *********************
    	...
    	for(var i in meta){
    		switch(meta[i].name){
    			...
    			case "OriginalPublicationDate":
    				...
    				" "+udate.match(/\d{4}/)[0];
    				break;
    			 case "dcterms.created" :case "created"         :case "DCTERMS.Created":
    			 case "DCTERMS.created" :case "dcterms.modified":case "modified":
    			 case "DCTERMS.Modified":case "DCTERMS.modified":
    ...
    if(udate.substr(0,1)=="0"){udate=udate.substr(1)}; // Remove leading zero
    var accessdate=now.getDate()+" "+monthNames[now.getMonth()]+
    " 20"+now.getYear().toString().substr(1);
    s+=(udate==''?'':" |date="+udate)+" |title="+utitle+
    (news()=='news'?" |newspaper=BBC News":" |publisher=BBC |accessdate="+accessdate);
...
function add_link(text, title, func, id) {
    ...
    if(document.getElementById('bbcpagesearchbox')!==null){ 
    	// used on some older pages and using 'div' displays badly
    	// e.g. http://news.bbc.co.uk/1/hi/entertainment/tv_and_radio/2545327.stm
		bar=document.getElementsByTagName('table');
		if(bar!==null){bar=bar[0]};
    }
...
add_link('<div style="margin:auto;width:'+uwidth+'px;background-color:lightblue;height:10pt;">\
