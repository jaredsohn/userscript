// ==UserScript==
// @name           Combat Layout Preview
// @namespace      overkill_gm
// @version        0.4
// @description    Gives you a preview of what your layout will be or something
...
function init(){
  ...
  function main(){
    ...
    function distributeForce(){
      ...
      if (field == "sea") // this if statement is unnecessary but i think it improves readability
      {
        ...
        if (force[213]) place(213,'main',distribution.main.length,10,false);  // ballista
        if (force[216]) place(216,'main',distribution.main.length,10,true); // pwr
        if (force[215]) place(215,'main',distribution.main.length,10,false);  // mortar
        if (force[214]) place(214,'main',distribution.main.length,10,false);  // catapult
        //long range
        if (force[214]) place(214,'longRange',distribution.longRange.length,10,true); // catapult
        // ballista
        if (force[213]) place(213,'longRange',distribution.longRange.length,10,true);
        // mortar
        if (force[215]) place(215,'longRange',distribution.longRange.length,10,true);
      }
      else // LAND BATTLE
      {
        //air - gyroscope
        if (force[312]) place(312,'air',distribution.air.length,30,true);
        //air - bomb
        if (force[309]) place(309,'air',distribution.air.length,30,true);
        //artillery
        // mortar
        ...
        // sword
        if (force[302]) place(302,'main',distribution.main.length,30,false);
        // spearman
        if (force[315]) place(315,'main',distribution.main.length,30,false);
        // guns
        if (force[304]) place(304,'main',distribution.main.length, 7,false);
        // archer
        if (force[313]) place(313,'main',distribution.main.length,30,false);
        // slinger
        if (force[301]) place(301,'main',distribution.main.length,30,false);
        // cook
        if (force[310]) place(310,'main',distribution.main.length,30,false);
        // doctor
        if (force[311]) place(311,'main',distribution.main.length,30,false);
        //long range
        // guns
  ...
  if (document.body.id == "blockade") {
    ...
    onClick(ctrl.appendChild(node('span','button','','small')),function(){field = 'small'; main(); });
    onClick(ctrl.appendChild(node('span','button','','medium')),function(){field = 'medium'; main(); });
    onClick(ctrl.appendChild(node('span','button','','large')),function(){field = 'large'; main(); });
  }
  main();
}
