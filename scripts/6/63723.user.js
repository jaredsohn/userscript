// ==UserScript==
// @name        Khan Wars Interface Tools
// @description Major revamp of Khan Wars interface
// @include     http://game*.khanwars.com*
// ==/UserScript==

// Originally created by Bagatelle for Utopia Kingdoms as "Utopia Kingdoms Interface Tools".
// Updated by schyllic for use in khan wars
// Apologies to Bagatelle for ripping his most excellent script for my purposes
// Thank you Bagatelle I stand in awe. I understood barely enough of this to make it work in khan wars -- "Maromania"

// NOTE: clan admin link currently requires editing this file to use your clan id#

// Major differences:
//
//  * building costs removed from overview
//  * no more "AttackNow" buttons on marches -- the page was too different it wouldn't work without complete rewrite
//  * scrolling message suppressed
//  * "motivation speech" suppressed
//  * added a few links
//  * moved [Release Troops] button below Order
//  * removed local time conversion from map
//  * changed color of scav on map from lightgreen to darkblue and added count of SB required

// TODO: Go thru and replace UK type names with the KW equivalents.
// TODO: Verify prices for everything
// TODO: simplify options for adding/removing things like building costs, SBM in map scav
// TODO: Create a list of features from Utopia Kingdoms Interface Tools. Verify they are all working.

// schyllic's edits -- remove the scroll thing
...
var Server = /game(\d+)\.khanwars.com/i.exec(window.location);
...
if (Node) {
  ...
  var PageMatch = new RegExp(
    "^http://game.*\\.khanwars\\.com.*/pohod\\.php(?:\\?" +
      ".*attackx=(\\d+)&attacky=(\\d+))?",
    "i"
...
/* // schyllic edit -- remove local time conversion
  var LocalSeconds =
...
function ReleaseTroops() {
  ...
  for (var N = 1; N <= 4; N++) {
    DoRequest(
      "http://game" + Server +
        ".khanwars.com/barracks.php?unit_type=" + String(N),
      N
    );
  }
  var Attempts = 0;
...
if (InsertionPoint) {
  ...
  // schyllic edit -- addded a few more items in the list below.
  LinkNode.innerHTML =
    ...
    "<a href='editclan.php?id=232'>Clan Admin</a> | " +
    "<a href='vip_search.php'>VIP Search</a>";
  InsertionPoint.insertBefore(LinkNode, InsertionPoint.firstChild);
}

var InsertionPoint = document.getElementsByClassName("blacksmith")[0]; // schyllic change barracks to blacksmith
if (InsertionPoint) {
...
var MarchInfo = new RegExp(
  "^http://game.*\\.khanwars\\.com.*/info\\.php\\?what=pohod&posted_id=" +
    "\\d+$",
  "i"
).test(window.location);
...
// schyllic edit -- this gets rid of the motivation speech after sending unmotivated march
// this has to be below the next section because window.name is cleared there
//http://game4.khanwars.com/pohod.php?sent=1&motivated=0
var PageMatch = /http:\/\/game.\.khanwars\.com.*\/pohod\.php.?sent=1&motivated=0/i.test(window.location);
if (PageMatch && window.location) {
    var msg = document.getElementsByClassName("infosmall")[0]; //[0];
    msg.innerHTML = "                March Sent Successfully!                                <br/>"
}

// redir coords on successfully sent march

var PageMatch = /^http:\/\/game.*\.khanwars\.com.*\/pohod\.php.*sent=1/i.
  test(window.location);
...
var PageMatch =
  /^http:\/\/game.*\.khanwars\.com.*\/pohod\.php/i.test(window.location);
var Formation = document.getElementById("army_orders");
if (PageMatch && ! Formation) {
  ...
  for (var N = 0; N < SelNodes.snapshotLength; N++) {
    ...
    if (Match) {
      ...
      LinkNode.innerHTML =
        "<a href='http://game" + Server +
        ".khanwars.com/pohod.php?attackx=" +
  ...
  /*
// schyllic edit -- begin comment here to remove attacks now & lower marches mods
...
var Table = document.getElementById("units_to_send");
  ...
  */ // schyllic edit -- end of comment to remove attacks now & lower marches page mods
}
...
var PageMatch =
  /^http:\/\/game.*\.khanwars\.com.*\/info\.php\?what=pohod&posted_id=\d+$/i.
    test(window.location);
if (PageMatch) {
  var PatCoords = /(\d+):(\d+)/i;
  var Nodes = document.evaluate(
...
function FormatSpyReport(Report, SavedLocal) {
  ...
  Report.innerHTML = Report.innerHTML.replace(
    ...
    function () {
      ...
      return Start + arguments[0] + "&nbsp;(<a href='" +
        "http://game" + Server + ".khanwars.com/pohod.php?attackx=" +
  ...
  if (Seconds <= 14400) {
    ...
    Colour = "darkblue"; // schyllic edit -- was lightgreen
  ...
  if (PatBuildings.test(Report.innerHTML)) {
    ...
    Report.innerHTML = Report.innerHTML.replace(
      ...
      "<a href=\"http://game" + Server + ".khanwars.com/map.php?setx=" +
...
var PageMatch = new RegExp(
  "^http://game.*\.khanwars\.com.*/(?:news|writenote)\.php" +
    "(?:.*?(?:type=(\\d+)))?",
  "i"
...
if (PageMatch) {
  ...
  var Headers = document.evaluate(
    "//h2[contains(text(), 'Khan Wars')]", document, null,
    XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  ...
  switch(PageMatch[1]) {
    ...
    // battle reports
    case ("2"):
      function CallBR(Node, URL) { // NOTE: The call to this is commented out below!
        GM_xmlhttpRequest({
          method: "GET",
          url: URL,
          onload: function (Response) {
            ...
            var StolenText = new RegExp("<li>Stolen Resource:", "i").
              test(Response.responseText);
            var TotalStolen = 0;
            if (StolenText)
              TotalStolen = ArraySum(new RegExp(
                "[\\s\\S]*?<li>Stolen Resource:[\\s\\S]*?(\\d+)[\\s\\S]*?" +
                  "(\\d+)[\\s\\S]*?(\\d+)[\\s\\S]*?(\\d+)",
                "i"
            ...
            NewNode.innerHTML = "<br />Attacker: " +
              ...
              Server + ".khanwars.com/map.php?setx=" +
      ...
      for (var N = 0; N < BattleReports.snapshotLength; N++) {
        ...
        //CallBR(URL, URL.href); // schyllic edit -- this was causing script freeze
        // because there are elements non-existent referred to in there
      }
      break;
              ...
              onload: function (Response) {
                ...
                var PatRes = new RegExp(
                  "<div class=\"box message\" id=\"msg_item_(\\d+)\">" +
                    "[\\s\\n]*?" +
                    "<p>Our armies from.*?sent to collect the cleanups around.*?" +
...
var PageMatch = /^http:\/\/game.*\.khanwars\.com.*\/vip_poleta\.php/i.
  test(window.location);
if (PageMatch) {
  var PatCoords = /\[(\d+):(\d+)\]/i;
  ...
  for (var N = 0; N < SelNodes.snapshotLength; N++) {
    ...
    if (Number(Pop) >= HealThreshold || Resources >= ScavengeThreshold) {
      Colour = "green"; // was lightgreen
    }
    var CarryString = (Colour ? "<font color='" + Colour + "'>" : "") +
      "Total: " + String(Resources) + "; " +
      String(Math.ceil(Resources / Units["u10"]["carry"])) + " LC or " +
      String(Math.ceil(Resources / Units["u19"]["carry"])) + " CA or " +
      String(Math.ceil(Resources / Units["u16"]["carry"])) + " SB" +
      (Colour ? "</font>" : "");
    var NewNode = document.createElement("td");
    ...
    function (Option) {
      ...
      eval(
        ResName + "Node = document.getElementById('send" +
          Option[1] + "');"
      ); // schyllic notes: the script dies here, when loading Battle reports
      eval(
...
var PageMatch = /http:\/\/game.*\.khanwars\.com.*\/market\.php\?action=2/i.
  test(window.location);
if (PageMatch) {
  /* // schyllic edit: there is no such thing as 'fastNav' on kw'
...
var PageMatch = new RegExp(
  "^http://game.*\\.khanwars\\.com.*/market\\.php\\?action=1", "i"
).exec(window.location);
if (PageMatch) {
...
var PageMatch = new RegExp(
  "^http://game.*\\.khanwars\\.com,*/market\\.php\\?action=4", "i"
).exec(window.location);
if (PageMatch) {
  ResList.forEach(
    function (Option) {
...
function ParseUnits(Node, Skills, Phase1, End) {
  ...
  var AttackerWin = /Attacker is the winner/i.test(document.evaluate(
    "//div[@class='infosmall']",
    Node, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue.innerHTML);
  var TotalPop = 0;
  ...
  var PatSiege = /<dt>Attack siege:<\/dt><dd>(\d+)<\/dd>/i;
  ...
  for (N = 0; N < Army.length; N++) {
    ...
    var Wizard = PatSiege.exec(Stats)[1];
...
var PageMatch = new RegExp(
  "http://game.*\\.khanwars\\.com.*/battle-report\\.php\\?id=.+" +
    "(?:&world_id=.+)?",
  "i"
).test(window.location);
if (PageMatch) {
  ...
  NewNode.innerHTML =
    ...
    String(Math.ceil(Scavenge / Units["u19"]["carry"])) + " CA/" +
    String(Math.ceil(Scavenge / Units["u19"]["carry"])) + " SB/" +  ")";
  ResultNode.appendChild(NewNode);
  var SurvivorCarry = Number(
    document.getElementById("GM_DataNode").getAttribute("SurvivorCarry")
  );
  var InsertionPoint = document.evaluate(
    "//ul[@class='battleResults']/li[contains(text(), 'Stolen Resource')" +
  ...
  for (var N = 0; N < InsertionPoint.snapshotLength; N++) {
    ...
    NewNode.innerHTML = "Actual: " + String(Actual) +
      ...
      String(Math.ceil(Actual / Units["u19"]["carry"])) + " CA/" +
      String(Math.ceil(Actual / Units["u16"]["carry"])) + " SB" +
      ")";
...
var PageMatch =
  /http:\/\/game.*\.khanwars\.com.*\/calculator\.php/.test(window.location);
if (PageMatch) {
...
//      GetEmptyGridStart("y");
      ...
      for (var N = 0; N < Nodes.snapshotLength; N++) {
        ...
        if (Number(Scavenge) > 0) {
            ...
            String(Math.ceil(Scavenge / Units["u19"]["carry"])) + " CA<br />" +
            String(Math.ceil(Scavenge / Units["u16"]["carry"])) + " SB" +
            "</b></font>";
        }
...
var PageMatch =
  /http:\/\/game.*\.khanwars\.com.*\/barracks\.php/.test(window.location);
if (PageMatch) {
  var Head = document.getElementsByTagName("head")[0];
  var NewScript = document.createElement("script");
  ...
  var PatID = /<img src=\"img\/units\/thumbs\/(\d+)\.jpg/i;
...
var PageMatch = /http:\/\/game.*\.khanwars\.com.*\/overview\.php/i.
  test(window.location);
if (PageMatch) {
  ...
  // building notes -- schyllic edit -- commented out this part
  // to remove building stuff, start comment here
  /*
  ...
  // schyllic edit -- to remove building stuff, end comment here
  */
}
...
var PageMatch = /^http:\/\/game.*\.khanwars\.com.*\/army_order\.php/i.
  test(window.location);
if (PageMatch) {
