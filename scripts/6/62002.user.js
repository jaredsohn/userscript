// ==UserScript==
// @name           Rapidshare Plus! (FREE Users) English Version
// @namespace      Juampi_Mix
// @description    Rapidshare.com redesigned web site and automates (autoclicks for free-users) downloads for you!
// @include        http://rapidshare.com/*
// @include        https://rapidshare.com/*
// @include        http://*.rapidshare.com/*
// @include        https://*.rapidshare.com/*
// @autor          Juampi_Mix
// @require        http://userscripts.org/scripts/source/62002.user.js
// @version        1.00
// @history        1.00 Fixed a detail of minimal relevance
// @history 	   0.33 Changing version to check updater functions?
// @history 	   0.32 Small touches history in the code. 
// @history 	   0.31 Returned to the previous updater (since i found that it worked. Just as the findings make each a day) (I'm looking to modify the code of automator to see if I can get download/timer checks every 1 minute) 
// @history 	   0.30 New history cleaning of code and solution for the updater. Now if you display the current version in the update notice.
// ==/UserScript==
...
if ( !disabling && (ch.nodeType == 1) && (
	...
	function RapidshareAutowait() {
		...
		if (error = getErrorType()) {
			var reload = true;
			var countdownMessage = "%s s - waiting to try again";
			var w = repeatTimeout;
			...
			switch (error) {
				...
				case "network-error":
					w = 10; 
					useFuzzyLogic = false;
					countdownMessage = "%s s - Network error, try again";
					break;
				case "server-down":
					w = 5*60; 
					countdownMessage = "%s s - Server down, try again";
					break;
				case "downloading":
					countdownMessage = "%s s - Download in progress Retry";
					break;
				/*
				case "wait":
			...
			if (undefined !== c) { // c 
				if (c > 0) {
					...
					window.setTimeout(RapidshareAutowait, c*1000+2000); // c+2 seconds (only if..) 
				} else {
					submitDownloadForm();
					document.title="Top Download successful!";
				}
			} else {
				alert("Unknown error - the site has changed?");
			}
		}
	}
...
ScriptUpdater.check(62002, '1.00');
ScriptUpdater.forceNotice(62002, '1.00');
ScriptUpdater.forceCheck(62002, '1.00');
function handleReturnedVersion(v) {
}
ScriptUpdater.check(62002, "1.00", handleReturnedVersion);