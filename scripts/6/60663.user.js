// ==UserScript==
// @name		Script actualizador 
// @namespace		Juampi_Mix
// @description		Traduccion de Script Updater (http://userscripts.org/scripts/show/57756) comprobador de secuencia de comandos de usuario de actualización, para su uso con los scripts de usuario. Incluye medio simple para mostrar el historial de la versión a través de los meta tags.
// @version			1.05
//
// @history             1.05 Retocada algunas partes sin traducir
// @history             1.04 Traduccion al español
...
ScriptUpdater = {
	...
	getLatestVersion:function(scriptId, callback) {	
		if(typeof(callback) != 'function')
			alert("ScriptUpdater error:\n\n scriptUpdater.getLatestVersion() requiere una función de devolución de llamada como el tercer argumento"); 
	...
	checkRemoteScript:function() {
		...
		// comprobar el codigo de revision en  userscripts.org 
		GM_xmlhttpRequest ({
			method: "GET",
	...
	parseHeaders:function(metadataBlock) {
		// tomado de http://wiki.greasespot.net/Metadata_block - gracias a sizzlemctwizzle por la sugerencia
		var headers = {};
		...
		if(ScriptUpdater.meta.name && ScriptUpdater.meta.version) {	
			...
			noticeWrapper.id = "ScriptUpdaterBodyWrapper";
				...
				if(!ScriptUpdater.forceNoticeEnabled) {
					html.push('<p>Hay una nueva versión de <strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="Ir a la pagina del script">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong> Disponible para la instalacion </p>');
				} else {
					html.push('<p><strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="Ir a la pagina del  script" style="margin:0; padding:0;">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong></p>');
				}				
				if(ScriptUpdater.scriptCurrentVersion) {
					html.push('<p>Su version Actual <strong>');
					html.push(ScriptUpdater.scriptCurrentVersion)
					html.push('</strong> La version disponible es  <strong>');
					html.push(ScriptUpdater.meta.version);
					html.push('</strong></p>');
				}
				if(ScriptUpdater.meta.history) {
					html.push('<h2>Historial de versiones:</h2><div id="ScriptUpdaterHistory">');
			...
			// Establecer intervalo actual en el selector
