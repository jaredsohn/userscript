...
// @name           Outwar Helper amaihero mod



// @namespace      amaihero mod
...
/* Focus on captcha forms and security prompt */
...
//pop up attack player window

if (( document.URL.indexOf("com/profile.php?id=") != -1 ) || ( document.URL.indexOf("characters") != -1 )) {

	var link = xpath(document, "//a[contains(@href,'javascript:attackWindow')]");
...
if ( document.URL.indexOf("world.php") != -1 ) {
	 ...
	 * 66th column is the one that holds the mobs and players in the room
...
masterdiv.insertBefore(searchForm,masterdiv.childNodes[56]);
...
//Add a link on hitlist to open profiles

if ( document.URL.indexOf("hitlist.php") != -1 ) {

		var link = document.createElement('a');

	link.addEventListener("click", profileTabs, false);

	link.setAttribute('title',"Open All in Tabs");

	link.appendChild(document.createTextNode('Open All in Tabs'));
	...
	var maindiv = doc.getElementsByTagName("td");

	for (var i = 0; i < maindiv.length; i++){

		if ( maindiv[i].innerHTML.indexOf("Name") != -1 ) {

		maindiv[i].appendChild(link);
...
//Add a link on Crew profiles to open all player profiles
...
function xpath(doc, query) {
	...
	var result = doc.evaluate(query, doc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null );
			...
			for (var i = 0; i < aLinks.length; i++){
...

				{
					...
					//opening the link
                    			...
                    			GM_openInTab("http://" + owServer + ".outwar.com/profile.php?id=" + pieces[3])
