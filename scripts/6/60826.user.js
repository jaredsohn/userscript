// ==UserScript==
// @name			ComunioEX
// @description		Integra las estadisticas de NoMasKeine en el juego Comunio
// @include         http://www*.comunio.es/*
// @include         http://comunio.es/*
// @version			0.6.1
// @license			http://creativecommons.org/licenses/by-nc-sa/3.0/es/
// ==/UserScript==

/* 	
	Gets XML or responseText from a URL
	
	async--> Receive it sync or async (true or false)
	response--> "responseText" or "responseXML"
	url--> url from we get the response
 */
function getURL(async, response, url) {

	try{

		var xhr = new XMLHttpRequest();
		xhr.open("GET", url, async);
		xhr.send();
		
		if( async == true ){
		
			xhr.onreadystatechange = function(){
			
			  if( xhr.readyState == 4 ){
				if( response == "responseXML" ){
					return xhr.responseXML;
				}
				else{
					return xhr.responseText;
				}
			  }
			  
			}
		}
		else{

			while( xhr.readyState != 4 );
			
			if( response == "responseXML" ){
				return xhr.responseXML;
			}
			else{
				return xhr.responseText;
			}
		
		}
	
	}
	catch(ex){
		alert(ex);
	}
	
}


/*--------------------------------------------------------------*/
// HTML TABLE SORTER
// OBJECT ORIENTED JAVASCRIPT IMPLEMENTATION OF QUICKSORT
// @author	Terrill Dent 
// @source	http://www.terrill.ca
// @date	August 28th, 2006
/*--------------------------------------------------------------*/
function TSorter(){
	var table = Object;
	var trs = Array;
	var ths = Array;
	var curSortCol = Object;
	var prevSortCol = '-1';
	var sortType = Object;

	function get(){}

	function getCell(index){
		return trs[index].cells[curSortCol] 
	}

	/*----------------------INIT------------------------------------*/
	// Initialize the variable
	// @param tableName - the name of the table to be sorted
	/*--------------------------------------------------------------*/
	/*
	this.init = function(tableName)
	{
		table = document.getElementById(tableName);
		ths = table.getElementsByTagName("th");
		for(var i = 0; i < ths.length ; i++)
		{
			ths[i].onclick = function()
			{
				sort(this);
			}
		}
		return true;
	};
	*/
	this.init = function(t){
		
		var words = loadTranslatedWords();
		
		table = t;
		trs = table.getElementsByTagName("tr");
		ths = trs[0].getElementsByTagName("td");
		
		for(var i = 0; i < ths.length ; i++){
		
			if( ths[i].textContent.indexOf("Delantero") != -1 ){
				return;
			}
		
			ths[i].innerHTML = '<a href="JavaScript:;"> ' + ths[i].textContent + '</a>'; 
		
			ths[i].addEventListener( "click",  function(event){ sort(this, words); }, true );
		}
		return true;
	};
	/*----------------------SORT------------------------------------*/
	// Sorts a particular column. If it has been sorted then call reverse
	// if not, then use quicksort to get it sorted.
	// Sets the arrow direction in the headers.
	// @param oTH - the table header cell (<th>) object that is clicked
	/*--------------------------------------------------------------*/
	function sort(oTH, words)
	{
		curSortCol = oTH.cellIndex;
		sortType = oTH.abbr;
		trs = table.getElementsByTagName("tr");
		
		
		var trs2 = new Array();
		for( var i=1; i<trs.length; i++ ){
			
			trs2[i-1] = trs[i];
		}
		trs = trs2;
		
		
		if( ths[curSortCol].textContent.toLowerCase().indexOf( words["for"] ) != -1 ||
			ths[curSortCol].textContent.indexOf( words["name"] ) != -1 ){
			
			sortType = "link_column";
		}
		else if( ths[curSortCol].textContent.toLowerCase().indexOf( words["club"] ) != -1 ){
				
			sortType = "img";
		}
		else if( ths[curSortCol].textContent.toLowerCase().indexOf( words["price"] ) != -1 || 
				 ths[curSortCol].textContent.toLowerCase().indexOf( words["quote"] ) != -1 ||
				 ths[curSortCol].textContent.toLowerCase().indexOf( words["points"] ) != -1 ){
				 
			sortType = "number";
		}
		else if( ths[curSortCol].textContent.toLowerCase().indexOf( words["placed"] ) != -1 || 
				 ths[curSortCol].textContent.toLowerCase().indexOf( words["changed"] ) != -1 ){
				 
			sortType = "date";
		}
		
		

		//set the get function
		setGet(sortType)

		// it would be nice to remove this to save time,
		// but we need to close any rows that have been expanded
		/*
		for(var j=0; j<trs.length; j++)
		{
			if(trs[j].className == 'detail_row')
			{
				closeDetails(j+2);
			}
		}
		*/

		// if already sorted just reverse
		if(prevSortCol == curSortCol)
		{
			oTH.className = (oTH.className != 'ascend' ? 'ascend' : 'descend' );
			reverseTable();
		}
		// not sorted - call quicksort
		else
		{
			oTH.className = 'ascend';
			//if(ths[prevSortCol].className != 'exc_cell'){ths[prevSortCol].className = '';}
			quicksort(0, trs.length);
		}
		prevSortCol = curSortCol;
	}
	
	function noAccent( str ){
	
		return str.replace(/Á/g, "A").replace(/É/g, "E").replace(/Í/g, "I").replace(/Ó/g, "O").replace(/Ú/g, "U")
				.replace(/á/g, "a").replace(/é/g, "e").replace(/í/g, "i").replace(/ó/g, "o").replace(/ú/g, "u");
	}
	
	/*--------------------------------------------------------------*/
	// Sets the GET function so that it doesnt need to be 
	// decided on each call to get() a value.
	// @param: colNum - the column number to be sorted
	/*--------------------------------------------------------------*/
	function setGet(sortType)
	{
		switch(sortType)
		{   
			case "link_column":
				get = function(index){
					/*return  getCell(index).firstChild.firstChild.nodeValue;*/
					return  noAccent( getCell(index).textContent.toLowerCase() );
				};
				break;
			case "img":
				get = function(index){
					//return  getCell(index).innerHTML;
					return  getCell(index).getElementsByTagName("img")[0].alt;
				};
				break;
			case "number":
				get = function(index){
					return  parseInt( getCell(index).textContent.split(".").join(""), 10 );
				};
				break;
			case "date":
				get = function(index){
					return  parseInt( getCell(index).textContent.split(".").reverse().join(""), 10 );
				};
				break;
			default:
				//get = function(index){	return getCell(index).firstChild.nodeValue;};
				get = function(index){	return  noAccent( getCell(index).textContent.toLowerCase() );};
				break;
		};	
	}

	/*-----------------------EXCHANGE-------------------------------*/
	//  A complicated way of exchanging two rows in a table.
	//  Exchanges rows at index i and j
	/*--------------------------------------------------------------*/
	/*
	function exchange(i, j)
	{
		if(i == j+1) {
			table.tBodies[0].insertBefore(trs[i], trs[j]);
		} else if(j == i+1) {
			table.tBodies[0].insertBefore(trs[j], trs[i]);
		} else {
			var tmpNode = table.tBodies[0].replaceChild(trs[i], trs[j]);
			if(typeof(trs[i]) == "undefined") {
				table.appendChild(tmpNode);
			} else {
				table.tBodies[0].insertBefore(tmpNode, trs[i]);
			}
		}
	}
	*/
	function exchange(i, j){
		var temp = trs[i].innerHTML;
		
		trs[i].innerHTML = trs[j].innerHTML;
		trs[j].innerHTML = temp;
	}
	
	/*----------------------REVERSE TABLE----------------------------*/
	//  Reverses a table ordering
	/*--------------------------------------------------------------*/
	function reverseTable()
	{
	/*
		for(var i = 1; i<trs.length; i++)
		{
			table.tBodies[0].insertBefore(trs[i], trs[0]);
		}
	*/

		for(var i = 0; i<trs.length/2; i++){
			exchange(i,trs.length-1-i);
		}
	
	}

	/*----------------------QUICKSORT-------------------------------*/
	// This quicksort implementation is a modified version of this tutorial: 
	// http://www.the-art-of-web.com/javascript/quicksort/
	// @param: lo - the low index of the array to sort
	// @param: hi - the high index of the array to sort
	/*--------------------------------------------------------------*/
	function quicksort(lo, hi)
	{
		if(hi <= lo+1) return;
		 
		if((hi - lo) == 2) {
			if(get(hi-1) > get(lo)) exchange(hi-1, lo);
			return;
		}
		
		var i = lo + 1;
		var j = hi - 1;
		
		if(get(lo) > get(i)) exchange(i, lo);
		if(get(j) > get(lo)) exchange(lo, j);
		if(get(lo) > get(i)) exchange(i, lo);
		
		var pivot = get(lo);
		
		while(true) {
			j--;
			while(pivot > get(j)) j--;
			i++;
			while(get(i) > pivot) i++;
			if(j <= i) break;
			exchange(i, j);
		}
		exchange(lo, j);
		
		if((j-lo) < (hi-j)) {
			quicksort(lo, j);
			quicksort(j+1, hi);
		} else {
			quicksort(j+1, hi);
			quicksort(lo, j);
		}
	}
}
////////////////////////////////////////////////////////////


//Trim function(Opera doesnt have it)
//http://blog.stevenlevithan.com/archives/faster-trim-javascript
function trim(str){
	return str.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
}


////////Player ID//////////////////////

/*
	Updates player ID's from an URL to localStorage
	
	localStorage--> (playersID) name//id//name2//id2...
*/
function updatePlayerIds(url){
	
	var response = getURL(false, "responseXML", url);
	var players = response.getElementsByTagName('jugador');
	
	var playersString = "";
	var id;
	
	for (var i=0; i<players.length; i++){
	
		name = players[i].getElementsByTagName("nombre")[0].childNodes[0].nodeValue;
		id = players[i].getElementsByTagName("id")[0].childNodes[0].nodeValue;
		playersString += trim( name ) + "//" + id + "//";
	}
	localStorage.setItem("playerIds", playersString);
	//alert( "Lista actualizada con exito ( " + players.length + " jugadores en la lista )" );

	return;
}

/*
	Loads player ID's from localStorage
	
	localStorage--> (playersID) name//id//name2//id2...
*/
function readPlayerIds(){

	var playersString = "";
	var playerIds = new Array();
	
	try{
		playersString = localStorage.getItem("playerIds").split("//");
	}
	catch(ex){
		updatePlayerIds('http://www.nomaskeine.com/cron/script.php');
		playersString = localStorage.getItem("playerIds").split("//");
	}
	
	
	for( var i=0; i<playersString.length; i+=2 ){
		playerIds[ playersString[i] ] = playersString[i+1];
	}
	return playerIds;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////


///////////IMAGES///////////////
/*
	Loads images in an array
*/
function loadImages(){

	var images = new Array;

	images[':D'] =  	'data:image/gif;base64,';
	images[':)'] =  	'data:image/gif;base64,';
	images[':('] =  	'data:image/gif;base64,';
	images[':o'] =  	'data:image/gif;base64,';
	images[':-?'] =     'data:image/gif;base64,R0lGODlhDwAWAJH/AP//////AAAAAP///yH5BAEAAAMALAAAAAAPABYAQAI/3GSGuSwBoWgv2tuS3jXM+EkOlilaVm7qcbVI11qvhInHfS8vc/b8b1JNhisYhiHphGYxUvOoHCqRzdRBuikAADs=';
	images['8)'] =      'data:image/gif;base64,';
	images[':lol:'] =   'data:image/gif;base64,';
	images[':x'] =      'data:image/gif;base64,';
	images[':P'] =      'data:image/gif;base64,';
	images[':oops:'] =  'data:image/gif;base64,R0lGODlhDwAPALMAAAAAAM7Ozv8AAP///////////////////////////////////////////////////yH5BAEAAAEALAAAAAAPAA8AAAQ1MEgJap04VMH5xUAnelM4jgAlmCG7at3mmSt9inKcz3iM2zQgzOMjDWdC1e0FstUyJUsKEwEAOw==';
	images[':cry:'] =   'data:image/gif;base64,';
	images[':evil:'] = 	'data:image/gif;base64,';
	images[':roll:'] = 	'data:image/gif;base64,';
	images[':wink:'] = 	'data:image/gif;base64,';
	images[':zzz:'] = 	'data:image/gif;base64,';
	images[':))'] = 	'data:image/gif;base64,';
	images[':love:'] = 	'data:image/gif;base64,';
	images[':bier:'] = 	'data:image/gif;base64,';
	images[':xx'] = 	'data:image/gif;base64,';
	images[':8))'] = 	'data:image/gif;base64,';
	images[':puke:'] = 	'data:image/gif;base64,';
	images['>:)'] = 	'data:image/gif;base64,';
	images[':clap:'] = 	'data:image/gif;base64,';
	images[':schimpf:'] = 'data:image/gif;base64,';
	images['*:('] = 	'data:image/gif;base64,';

	images['Almería'] = 'data:image/png;base64,';
	images['Atlético'] = 'data:image/png;base64,';
	images['Athletic'] = 'data:image/png;base64,';
	images['Barcelona'] = 'data:image/png;base64,';
	images['Deportivo'] = 'data:image/png;base64,';
	images['Espanyol'] = 'data:image/png;base64,';
	images['Getafe'] = 	'data:image/png;base64,';
	images['Málaga'] = 	'data:image/png;base64,';
	images['Mallorca'] = 'data:image/png;base64,';
	images['Osasuna'] = 'data:image/png;base64,';
	images['Racing'] = 	'data:image/png;base64,';
	images['Real Madrid'] = 'data:image/png;base64,';
	images['Sevilla'] = 'data:image/png;base64,';
	images['Sporting'] = 'data:image/png;base64,';
	images['Tenerife'] = 'data:image/png;base64,';
	images['Valencia'] = 'data:image/png;base64,';
	images['Valladolid'] = 'data:image/png;base64,';
	images['Villarreal'] = 'data:image/png;base64,';
	images['Zaragoza'] = 'data:image/png;base64,%3D%3D';
	images['Real Sociedad'] = 'data:image/png;base64,%3D%3D';
	images['Levante'] = 'data:image/png;base64,%3D';
	images['Hércules'] = 'data:image/png;base64,%3D';

	images['ha dejado la liga'] = 'data:image/png;base64,';
	images['imgIcon'] = 'data:image/png;base64,';
	images['httpIcon'] = 'data:image/png;base64,';

	return images;
}

////////Team URL's//////////////////////
function loadTeamUrls(){
	
	var teamUrls = new Array;
	
	teamUrls["oficial"] = new Array;
	teamUrls["oficial"]["Almería"] = 'http://www.udalmeriasad.com/udalmeria/';
	teamUrls["oficial"]["Athletic"] = 'http://www.athletic-club.net/web/main.asp?a=0&b=0&c=0&d=0&idi=1';
	teamUrls["oficial"]["Atlético"] = 'http://www.clubatleticodemadrid.com/es/index.asp';
	teamUrls["oficial"]["Barcelona"] = 'http://www.fcbarcelona.cat/web/castellano/index.html';
	teamUrls["oficial"]["Deportivo"] = 'http://www.canaldeportivo.com/';
	teamUrls["oficial"]["Espanyol"] = 'http://www.rcdespanyol.com/';
	teamUrls["oficial"]["Getafe"] = 'http://www.getafecf.com/';
	teamUrls["oficial"]["Málaga"] = 'http://www.malagacf.com/?home';
	teamUrls["oficial"]["Mallorca"] = 'http://www.rcdmallorca.es/';
	teamUrls["oficial"]["Osasuna"] = 'http://www.osasuna.es/dev/';
	teamUrls["oficial"]["Racing"] = 'http://www.realracingclub.es/';
	teamUrls["oficial"]["Real Madrid"] = 'http://www.realmadrid.com/cs/Satellite/es/Home.htm';
	teamUrls["oficial"]["Sevilla"] = 'http://www.sevillafc.es/_www/index.php';
	teamUrls["oficial"]["Sporting"] = 'http://www.realsporting.com/webrsg/';
	teamUrls["oficial"]["Tenerife"] = 'http://www.clubdeportivotenerife.es/default.asp';
	teamUrls["oficial"]["Valencia"] = 'http://www.valenciacf.com/es/Home/index.html';
	teamUrls["oficial"]["Valladolid"] = 'http://www.realvalladolid.es/';
	teamUrls["oficial"]["Villarreal"] = 'http://www.villarrealcf.es/principal.php?idioma=1';
	teamUrls["oficial"]["Xerez"] = 'http://www.xerezcd.com/index.php';
	teamUrls["oficial"]["Zaragoza"] = 'http://www.realzaragoza.com/';
	teamUrls["oficial"]["Hércules"] = 'http://www.herculescf.es/';
	teamUrls["oficial"]["Real Sociedad"] = 'http://www.realsociedad.com/caste/home/real.asp';
	teamUrls["oficial"]["Levante"] = 'http://www.levanteud.com/';
	
	teamUrls["NMK"] = new Array;
	teamUrls["NMK"]["Almería"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/19/almeria';
	teamUrls["NMK"]["Athletic"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/7/athletic';
	teamUrls["NMK"]["Atlético"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/12/atletico';
	teamUrls["NMK"]["Barcelona"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/2/barcelona';
	teamUrls["NMK"]["Deportivo"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/21/deportivo';
	teamUrls["NMK"]["Espanyol"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/13/espanyol';
	teamUrls["NMK"]["Getafe"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/3/getafe';
	teamUrls["NMK"]["Málaga"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/4/malaga';
	teamUrls["NMK"]["Mallorca"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/9/mallorca';
	teamUrls["NMK"]["Osasuna"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/5/osasuna';
	teamUrls["NMK"]["Racing"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/15/racing';
	teamUrls["NMK"]["Real Madrid"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/16/real-madrid';
	teamUrls["NMK"]["Sevilla"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/1/sevilla';
	teamUrls["NMK"]["Sporting"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/17/sporting';
	teamUrls["NMK"]["Tenerife"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/22/tenerife';
	teamUrls["NMK"]["Valencia"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/20/valencia';
	teamUrls["NMK"]["Valladolid"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/18/valladolid';
	teamUrls["NMK"]["Villarreal"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/11/villarreal';
	teamUrls["NMK"]["Xerez"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/24/xerez';
	teamUrls["NMK"]["Zaragoza"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/23/zaragoza';
	teamUrls["NMK"]["Hércules"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/66/hercules';
	teamUrls["NMK"]["Real Sociedad"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/26/real-sociedad';
	teamUrls["NMK"]["Levante"] = 'http://www.nomaskeine.com/estadisticas/comunio/equipo/25/levante';

	teamUrls["AS"] = new Array;
	teamUrls["AS"]["Almería"] = 'http://www.as.com/futbol/equipo/Almería-85';
	teamUrls["AS"]["Athletic"] = 'http://www.as.com/futbol/equipo/Athletic-5';
	teamUrls["AS"]["Atlético"] = 'http://www.as.com/futbol/equipo/Atlético-42';
	teamUrls["AS"]["Barcelona"] = 'http://www.as.com/futbol/equipo/Barcelona-3';
	teamUrls["AS"]["Deportivo"] = 'http://www.as.com/futbol/equipo/Deportivo-7';
	teamUrls["AS"]["Espanyol"] = 'http://www.as.com/futbol/equipo/Espanyol-8';
	teamUrls["AS"]["Getafe"] = 'http://www.as.com/futbol/equipo/Getafe-172';
	teamUrls["AS"]["Málaga"] = 'http://www.as.com/futbol/equipo/Málaga-10';
	teamUrls["AS"]["Mallorca"] = 'http://www.as.com/futbol/equipo/Mallorca-11';
	teamUrls["AS"]["Osasuna"] = 'http://www.as.com/futbol/equipo/Osasuna-13';
	teamUrls["AS"]["Racing"] = 'http://www.as.com/futbol/equipo/Racing-15';
	teamUrls["AS"]["Real Madrid"] = 'http://www.as.com/futbol/equipo/Real Madrid-1';
	teamUrls["AS"]["Sevilla"] = 'http://www.as.com/futbol/equipo/Sevilla-53';
	teamUrls["AS"]["Sporting"] = 'http://www.as.com/futbol/equipo/Sporting-76';
	teamUrls["AS"]["Tenerife"] = 'http://www.as.com/futbol/equipo/Tenerife-162';
	teamUrls["AS"]["Valencia"] = 'http://www.as.com/futbol/equipo/Valencia-17';
	teamUrls["AS"]["Valladolid"] = 'http://www.as.com/futbol/equipo/Valladolid-18';
	teamUrls["AS"]["Villarreal"] = 'http://www.as.com/futbol/equipo/Villarreal-19';
	teamUrls["AS"]["Xerez"] = 'http://www.as.com/futbol/equipo/Xerez-254';
	teamUrls["AS"]["Zaragoza"] = 'http://www.as.com/futbol/equipo/Zaragoza-20';
	teamUrls["AS"]["Hércules"] = 'http://www.as.com/futbol/equipo/Hercules-93';
	teamUrls["AS"]["Real Sociedad"] = 'http://www.as.com/futbol/equipo/R-Sociedad-16';
	teamUrls["AS"]["Levante"] = 'http://www.as.com/futbol/equipo/Levante-136';
	
	return teamUrls;
}

/*
	Returns language selected
	
	"en", "es", ...
*/
function getLanguage(){
	var head = document.getElementsByTagName("head")[0];
	var metas = head.getElementsByTagName("meta");
	
	var i = 0;
	while( metas[i].getAttribute("http-equiv") != "Content-Language" ){
		i++;
	}
	
	return metas[i].getAttribute("content");
}

/*
	Loads words in the selected language
*/
function loadTranslatedWords(){

	var language = getLanguage();
	var words = new Array();
	
	words["en"] = new Array();
	words["en"]["name"] = "name";
	words["en"]["club"] = "club";
	words["en"]["owner"] = "owner";
	words["en"]["from"] = "from";
	words["en"]["to"] = "to";
	words["en"]["for"] = "for";
	words["en"]["changed"] = "changed";
	words["en"]["quote"] = "quote";
	words["en"]["price"] = "price";
	words["en"]["points"] = "points";
	words["en"]["placed"] = "placed";
	
	words["de"] = new Array();
	words["de"]["name"] = "name";
	words["de"]["club"] = "verein";
	words["de"]["owner"] = "besitzer";
	words["de"]["from"] = "von";
	words["de"]["to"] = "an";
	words["de"]["for"] = "für";
	words["de"]["changed"] = "letzte";
	words["de"]["quote"] = "marktwert";
	words["de"]["price"] = "preis";
	words["de"]["points"] = "punkte";
	words["de"]["placed"] = "vom";
	
	words["es"] = new Array();
	words["es"]["name"] = "nombre";
	words["es"]["club"] = "equipo";
	words["es"]["owner"] = "propietario";
	words["es"]["from"] = "de";
	words["es"]["to"] = "para";
	words["es"]["for"] = "por";
	words["es"]["changed"] = "cambio";
	words["es"]["quote"] = "valor";
	words["es"]["price"] = "precio";
	words["es"]["points"] = "puntos";
	words["es"]["placed"] = "desde";
	
	return words[ language ];
}

/*
	returns status player ("Plus" or "basic")
*/
function getStatusPlayer(){

	var span = document.getElementsByTagName("span");
	for( var i=0; i<span.length; i++){

		//plusplayer?
		if (span[i].textContent == "Plus" ){
			return "Plus";
		}
		//basic?
		if (span[i].textContent == "basic" ){
			return "basic";
		}
	}
	return "";
}

/*
	sets status player
*/
function setStatusPlayer(status){

	var span = document.getElementsByTagName("span");
	for( var i=0; i<span.length; i++){

		//plusplayer?
		if (span[i].textContent == "Plus" ){
			span[i].textContent = status;
			break;
		}
		//basic?
		if (span[i].textContent == "basic" ){
			span[i].textContent = status;
			break;
		}
	}
}

/*
	sets script version
*/
function setScriptVersion(version){
	var playerStatus = document.getElementsByClassName("playerstatus")[0];
	
	playerStatus.parentNode.parentNode.parentNode.innerHTML += '<tr><td><span class="button01">v' + version + '</span></td></tr>';
}

//Function checkUpdate (it checks if there is a new version)
//text--> Data from the web where it is the updated script
function checkUpdates(version) {

	if (window.location.href.indexOf( "administration" ) == -1){
		return;
	}

	try{
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://userscripts.org/scripts/source/60826.meta.js',
			onload: function(xhr) {
			
					var metaText = xhr.responseText;

					var ini = metaText.indexOf("// @version");	//Search for number of version ( x.x.x )
					ini += ("// @version").length;
					var end = metaText.indexOf( "\n", ini );

					metaText = metaText.substring( ini, end );
					metaText = metaText.replace( /\s/g, "" );		//Clear white spaces
					
					if( metaText.split(".").length < 3 ){	//Error if there are not at least 3 numbers
						update = '<tr><td><span class="button01">Error al comprobar la versión</span></td></tr>';
					}
					else if( metaText == version ){				//Using the newest version
						update = '<tr><td><span class="button01">Versión actualizada</span></td></tr>';
					}
					else{	//New update!
						update = '<tr><td><span class="button01">Versión vieja</span></td></tr>' + 
								 '<tr><td><a href="http://userscripts.org/scripts/show/60826"> Actualizar!</a></td></tr>';
					}
					
					//Search for status player and change it to NoMasKeinePlayer with the number of version of the script
					var playerStatus = document.getElementsByClassName("playerstatus")[0];
					
					playerStatus.parentNode.parentNode.parentNode.innerHTML += update;
				
			}
		});
	}
	catch(ex){}
}

/*
	Changes size of principal table (%)
*/
function setSize(percent){
	var tables = document.getElementsByTagName("table");
	for( var i=0; i<tables.length; i++ ){
		if( parseInt( tables[i].style.width.replace("px", ""), 10 ) > 600 ){	//If it has an attribute called "width" with more than 600px, it must be the main table
			tables[i].style.width = percent + "%";
			break;
		}
	}
}

/*
	Adds smiles/improves editor
*/
function addSmileys(){

	var images = loadImages();
	
	//If we are writing a message...
	if ( window.location.href.indexOf( "postMessage" ) != -1 ){
		
		//Improve editor
		var newScript = document.createElement("script");
		newScript.innerHTML = "function setText( txt ){" + 
							   "	var text = document.post.message;" +
							   "	var start = text.selectionStart;" +
							   "	var end = text.selectionEnd;" +
							   "	var s1 = text.value.substr(0, start);" +
							   "	var s2 = text.value.substr(end);" +
							   "	var scrollTop = text.scrollTop;" +
							   "	text.value = s1 + txt + s2;" +
							   "	if (start == end)	{" +
							   "		text.selectionStart = s1.length + txt.length;" +
							   "		text.selectionEnd = text.selectionStart;" +
							   "	}" +
							   "	else{" +
							   "		text.selectionStart = s1.length;" +
							   "		text.selectionEnd = s1.length + txt.length;" +
							   "	}" +
							   "	if ( txt.indexOf('[/') != -1 ){" +
							   "		text.selectionStart -= txt.indexOf('[/')+1;" +
							   "		text.selectionEnd = text.selectionStart;" +
							   "	}" +
							   "	text.focus();" +
							   "	text.scrollTop = scrollTop;" +
							   "}";
		document.body.appendChild( newScript );
		
		//Smileys
		var smileyTable1 = document.createElement("table");
		smileyTable1.setAttribute( "style", "width:100%; height:24px;" );
		smileyTable1.setAttribute( "cellpadding", "3" );
		smileyTable1.setAttribute( "cellspacing", "3" );
		var smileyTable2 = smileyTable1.cloneNode(true);
		var smileyTable3 = smileyTable1.cloneNode(true);
		
		smileyTable1.innerHTML = 	'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':D\');"><img src="' + images[':D'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':)\');"><img src="' + images[':)'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':(\');"><img src="' + images[':('] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':o\');"><img src="' + images[':o'] + '" border="0" /></a></td>' +
									'</tr>' +
									
									'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':-?\');"><img src="' + images[':-?'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\'8)\');"><img src="' + images['8)'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':lol:\');"><img src="' + images[':lol:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':x\');"><img src="' + images[':x'] + '" border="0" /></a></td>' +
									'</tr>' +
									
									'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':P\');"><img src="' + images[':P'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':oops:\');"><img src="' + images[':oops:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':cry:\');"><img src="' + images[':cry:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':evil:\');"><img src="' + images[':evil:'] + '" border="0" /></a></td>' +
									'</tr>' +
									
									'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':roll:\');"><img src="' + images[':roll:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':wink:\');"><img src="' + images[':wink:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':zzz:\');"><img src="' + images[':zzz:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':))\');"><img src="' + images[':))'] + '" border="0" /></a></td>' +
									'</tr>' +
									
									'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':love:\');"><img src="' + images[':love:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\'*:(\');"><img src="' + images['*:('] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':xx\');"><img src="' + images[':xx'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':8))\');"><img src="' + images[':8))'] + '" border="0" /></a></td>' +
									'</tr>' +
									
									'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':puke:\');"><img src="' + images[':puke:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\'>:)\');"><img src="' + images['>:)'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':clap:\');"><img src="' + images[':clap:'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':schimpf:\');"><img src="' + images[':schimpf:'] + '" border="0" /></a></td>' +
									'</tr>';

		smileyTable2.innerHTML = 	'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\':bier:\');"><img src="' + images[':bier:'] + '" border="0" /></a></td>' +
									'</tr>';
									
		smileyTable3.innerHTML = 	'<tr>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\'[img][/img]\');"><img src="' + images['imgIcon'] + '" border="0" /></a></td>' +
										'<td><a href="JavaScript:;" onclick="JavaScript:setText(\'http://\');"><img src="' + images['httpIcon'] + '" border="0" /></a></td>' +
									'</tr>';
							
		//Add smileys tables to the web
		var allTables = document.getElementsByTagName("table");
		allTables[20].parentNode.insertBefore(smileyTable1, allTables[20].nextSibling);
		smileyTable1.parentNode.insertBefore(smileyTable2, smileyTable1.nextSibling);
		smileyTable2.parentNode.insertBefore(smileyTable3, smileyTable2.nextSibling);

	}
	
	//If we are at news page, but not writing a post...
	if ( window.location.href.indexOf( "news" ) != -1 && window.location.href.indexOf( "edit" ) == -1 ){
		
		//Obtain all the posts from news page
		var objects =[];
		var posts = document.getElementsByClassName("tableline02b");
		for( var i=0; i<posts.length; i++ ){
			objects.push(posts[i]);
		}
		var infoBox = document.getElementsByClassName("infoBox");
		for( var i=0; i<infoBox.length; i++ ){
			var infoRows = infoBox[i].getElementsByTagName("tr");
			for( var j=0; j< infoRows.length; j++ ){
				objects.push(infoRows[j].getElementsByTagName("td")[0]);
			}
		}

		//Process all the objects
		for( var i=0; i<objects.length; i++ ){
			
			//Replace Smileys with their images
			objects[i].innerHTML = objects[i].innerHTML.replace(/:D/g,"<img src=\"" + images[':D'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:\)\)/g,"<img src=\"" + images[':))'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/\*:\(/g,"<img src=\"" + images['\*:\('] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:oops:/g,"<img src=\"" + images[':oops:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:-\?/g,"<img src=\"" + images[':-?'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:8\)\)/g,"<img src=\"" + images[':8))'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:lol:/g,"<img src=\"" + images[':lol:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:xx/g,"<img src=\"" + images[':xx'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:P/g,"<img src=\"" + images[':P'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:o/g,"<img src=\"" + images[':o'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:cry:/g,"<img src=\"" + images[':cry:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:evil:/g,"<img src=\"" + images[':evil:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:roll:/g,"<img src=\"" + images[':roll:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:wink:/g,"<img src=\"" + images[':wink:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:zzz:/g,"<img src=\"" + images[':zzz:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/>:\)/g,"<img src=\"" + images['>:)'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:love:/g,"<img src=\"" + images[':love:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:bier:/g,"<img src=\"" + images[':bier:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:x/g,"<img src=\"" + images[':x'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/8\)/g,"<img src=\"" + images['8)'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:puke:/g,"<img src=\"" + images[':puke:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:\)/g,"<img src=\"" + images[':)'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:clap:/g,"<img src=\"" + images[':clap:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:schimpf:/g,"<img src=\"" + images[':schimpf:'] + "\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/:\(/g,"<img src=\"" + images[':('] + "\" />");

			//Web links (http://, https://, file://, ftp://)
			var exp = /[^\]"](\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
			objects[i].innerHTML = objects[i].innerHTML.replace(exp,"<a href='$1'>$1</a>"); 
			
			//Image links ( [img][/img], [img=align][/img]
			objects[i].innerHTML = objects[i].innerHTML.replace(/\[img\](.*?)\[\/img\]/gi,"<img src=\"$1\" />");
			objects[i].innerHTML = objects[i].innerHTML.replace(/\[img=(.*?)\](.*?)\[\/img\]/gi,"<img src=\"$2\" align=\"$1\" />");

		}
	}
}
	

/*
	Process tables with players, teams, etc.
	
	Adds links, images, status, sortability...
*/
function convertTables(){

	var words = loadTranslatedWords();
	var teamUrls = loadTeamUrls();
	var images = loadImages();
	var playerIds = readPlayerIds();

	var playerNameCol = 0, teamNameCol = 1, ownerCol;
	var playersUpdated = false;
	
	//Collect all kinds of tables in one
	var tables = [];
	var tables1 = document.getElementsByClassName("tablecontent03");
	var tables2 = document.getElementsByClassName("tablecontent03b");
	for( var i=0; i<tables1.length; i++ ){
		tables.push(tables1[i]);
	}
	for( var i=0; i<tables2.length; i++ ){
		tables.push(tables2[i]);
	}

	//Iterate through the tables we have collected
	for( var i=0; i<tables.length; i++ ){
	
		//If we are in the administration page, we dont touch it
		if( window.location.href.indexOf( "administration" ) != -1 ){
			break;
		}
			
		//Add sorting capability	
		var Table1Sorter = new TSorter;
		Table1Sorter.init(tables[i]);
		
		if( window.location.href.indexOf( "standings" ) != -1 ||		//Standings table--> out (only sorting)
			window.location.href.indexOf( "teamInfo.phtml" ) != -1 ||	//Table with the community players
			window.location.href.indexOf( "team_news" ) != -1) {		//News board table--> out (only sorting)
			
			break;
		}

		//Obtain the rows of the table
		var tableRows = tables[i].getElementsByTagName('tr');
		for ( var j = 0; j < tableRows.length; j++ ){
			
			//Obtain columns
			var td = tableRows[j].getElementsByTagName("td");
			
			//Row header
			if( tableRows[j].getAttribute('class') != 'tr1' && tableRows[j].getAttribute('class') != 'tr2' ){
				for( var k = 0; k < td.length; k++ ){
				
					if( td[k].textContent.toLowerCase().indexOf( words["name"] ) != -1 || 
						td[k].textContent.toLowerCase().indexOf( words["for"] ) != -1 ){
						
						playerNameCol = k;
					}
					else if( td[k].textContent.toLowerCase().indexOf( words["club"] ) != -1 ){
						teamNameCol = k;
					}
					else if( td[k].textContent.toLowerCase().indexOf( words["owner"] ) != -1 ||
							 td[k].textContent.toLowerCase().indexOf( words["to"] ) != -1 ||
							 td[k].textContent.toLowerCase().indexOf( words["from"] ) != -1 ){
						
						ownerCol = k;
					}
				}
			}
			else{	//A player

				var playerName = td[ playerNameCol ].textContent.replace( /\*/g, "" );
				var teamName = td[ teamNameCol ].textContent;
				
				//Comunio errors with spaces
				playerName = trim(playerName);
				teamName = trim(teamName);
		
				//Id number of the player
				var id = playerIds[ playerName ];
				
				//If we dont have ID's and thet were not updated...
				if( !playerIds[ playerName ] && playersUpdated == false ){
					updatePlayerIds('http://www.nomaskeine.com/cron/script.php');
					playersUpdated = true;
					playerIds = readPlayerIds();
					id = playerIds[ playerName ];
				}

				//Image and link of the team
				td[ teamNameCol ].innerHTML = 	'<a href="' + teamUrls[ "oficial" ][ teamName ] + 
												'" target="_blank"><img src="' + images[ teamName ] + '" alt=' +
												teamName +' title=' +teamName + ' border="0" /></a>';
																				
									
				//Nomaskeine link					
				td[ playerNameCol ].innerHTML = '<a href="http://www.nomaskeine.com/estadisticas/comunio/jugador/' + 
												id + '/' + playerName.replace(/"/g, "&quot;") + '" target="_blank" ">' +
												td[ playerNameCol ].textContent + '</a>';
												
				// estado de jugadores - by anthorlop - 20110301 - ini
				// nueva columna en alineacion
				if (window.location.href.indexOf( "lineup" ) != -1
					|| window.location.href.indexOf( "playerInfo" ) != -1
					|| window.location.href.indexOf( "exchangemarket" ) != -1
					&& window.location.href.indexOf( "viewoffers" ) == -1) {
					var tdNew = document.createElement("TD");
					
					//crear la nueva columna estado del jugador
					tdNew.appendChild(document.createTextNode(''));
					tdNew.align = "center";
					
					tdNew.innerHTML = '<img width="20px" height="20px" title="cargando estado..."  src="data:image/png;base64," id="estado' + playerName.replace(/"/g, "&quot;").replace( /\ /g, "_" ) + '" />';
					tableRows[j].appendChild(tdNew);
				}
				// estado de jugadores - by anthorlop - 20110301 - fin
			}
		}
	}
}


/*
	Change player links from Comunio to NoMasKeine
*/
function convertLinksNoMasKeine(){
	
	if ( window.location.href.indexOf( "news" ) != -1 && window.location.href.indexOf( "edit" ) == -1 ){
		var a = document.getElementsByTagName("a");
		for( var i=0; i<a.length; i++ ){
			
			if( a[i].href.indexOf("/primera_division") != -1 ){
			
				var player = a[i].getAttribute("href").replace( "../primera_division/", "" ).replace( ".html", "" );
				player = player.split("-");
				
				var playerName = trim( a[i].textContent );
				a[i].textContent = playerName;
				var playerId = player[0];
				
				a[i].setAttribute( "href", 'http://www.nomaskeine.com/estadisticas/comunio/jugador/' + 
											playerId + '/' + playerName );
				a[i].setAttribute( "onclick", "" );
			}
			else if( a[i].href.indexOf("tradableInfo.phtml") != -1 ){
			
				var playerId = a[i].getAttribute("href").replace( "tradableInfo.phtml?tid=", "" );
				var playerName = trim( a[i].textContent );
				a[i].textContent = playerName;
				
				a[i].setAttribute( "href", 'http://www.nomaskeine.com/estadisticas/comunio/jugador/' + 
											playerId + '/' + playerName );
				a[i].setAttribute( "onclick", "" );
			}
		}
	}
}

/*
	Adds live scores in an iframe in the lineUp web
*/
function addScores(){
	if ( window.location.href.indexOf( "lineup" ) != -1 ){
	
		var tables = document.getElementsByClassName("tableline02");
		var div = document.createElement("div");
		div.innerHTML = '<iframe frameborder="no" scrolling="no" style="border: 0px solid rgb(255, 255, 255);' +
						'height: 220px; width: 620px;" src="http://www.marcadoresonline.com/widgets/widget3.php?' +
						'mantenerse=si&liga=PRIMERA"></iframe>';
		tables[2].parentNode.insertBefore(div, tables[2]);
	}
}

/*
	Adds forum information to the lineUp webPage
*/
function addForumInfo(){

	//At points table, this function quits the player from the total sum
	function quitPlayerPoints( player ){
		
		var temp = player.textContent.split(" ");							//Obtain the points of the player spliting it ( Villa 16 )
		var points = parseInt( temp[temp.length-1], 10 );					//Parse points
		
		player.parentNode.parentNode.removeChild(player.parentNode);
	
		var totalPointsCell = document.getElementById("totalPointsCell");	//Obtain the cell with the sum
		var total = totalPointsCell.textContent.split(":")[1];				//Only the number (TOTAL:22)
		var totalInt = parseInt( total, 10 );								//Parse it
		totalInt -= points;													//Quit
		totalPointsCell.textContent = totalPointsCell.textContent.replace(/\bTOTAL:-?[0-9]+/,'TOTAL:' + totalInt);	//Change the total sum
	}
	
	//At points table, it adds a player to the total sum
	function addPlayerPoints( player ){
	
		var totalPointsTable = document.getElementById("totalPointsTable");		//Table where the sum it is
		var totalPointsCell = document.getElementById("totalPointsCell");		//Cell of the total sum
		
		var temp = player.textContent.split(" ");								//Obtain the points of the player spliting it ( Villa 16 )
		var tempInt = parseInt( temp[temp.length-1], 10 );						//Parse points
		
		if( isNaN( tempInt ) ){													//If there is a problem or the player has no points... 0
			tempInt = 0;
		}
		
		var total = totalPointsCell.textContent.split(":")[1];					//Only the number (TOTAL:22)
		var totalInt = parseInt( total, 10 );									//Parse it
		totalInt += tempInt;													//Add points
		totalPointsCell.textContent = totalPointsCell.textContent.replace(/\bTOTAL:-?[0-9]+/,'TOTAL:' + totalInt);		//Change the total sum
		
		//Add the player name to the total sum column
		var newRow = totalPointsTable.insertRow(0);
		newRow.innerHTML = '<td><a href="javascript:;">' + player.textContent + '</a></td>';
		//Put an event listener on it for quit from the sum if we want
		newRow.getElementsByTagName("a")[0].addEventListener( "click", function(){ quitPlayerPoints(this) }, false);
	}
	
	//Function that hides or shows an object
	function toggleDisplayObj( obj ){
		
		if( obj.style.display == "none" ){
			obj.style.display = "block";
		}
		else{
			obj.style.display = "none";
		}
	}
	function toggleDisplayId( id ){
		
		var obj = document.getElementById(id);
		toggleDisplayObj(obj);
	}
	
	/*
		Returns an array with each post from a thread forum
	*/
	function getPosts(text){
		var posts = new Array();
		
		var iniPost = text.indexOf('<span class="postbody">');
		while( iniPost != -1){
		
			var counter = 1;
			var index = iniPost;
			var iniSpan = 0;
			var endSpan = 0;
			
			while( counter > 0 && iniSpan >= 0 ){
				
				iniSpan = text.indexOf('<span', index + 1);
				endSpan = text.indexOf('</span', index + 1);
				
				if( endSpan > iniSpan ){	//<span before </span
					counter++;
					index = iniSpan;
				}
				else{
					counter--;
					index = endSpan;
				}
				
			}
			//Quit signatures
			if( text.indexOf('<br />_________________<br />', iniPost) != -1 ){
				endSpan = text.indexOf('<br />_________________<br />', iniPost);
			}
			posts.push( text.substring(iniPost, endSpan) );

			iniPost = text.indexOf('<span class="postbody">', iniPost + 1);		//Next post
		}
		return posts;
	}
	
	function addConvocated(text){
		
		var td = document.getElementById("convCol");
	
		var posts = getPosts(text);
		for( var i=0; i<posts.length; i++ ){
			td.innerHTML += '<table>' + posts[i] + '<br><br></table>';
		}
	}

	function addInjuried(text){

		var td = document.getElementById("injuriedCol");
	
		var posts = getPosts(text);
		for( var i=0; i<posts.length; i++ ){
			td.innerHTML += '<table>' + posts[i] + '<br><br></table>';
		}
	}

	function addPoints(text){
		
		var pointsTable = document.getElementById("pointsTable");
		var posts = getPosts(text);
		
		//posts (teams or matches)		
		for( var i=0; i<posts.length; i++ ){
		
			var lines = posts[i].replace( /<\/?span>/gi, "" ).split(/<br \/>/);
			var table = document.createElement("table");
			var first = true;
			
			//players and points
			for( var j=0; j<lines.length; j++ ){
				
				//Empty line
				if( lines[j].replace( /\s/gi, "" ) == "" ){
					continue;
				}
				
				//Header
				if( first ){
					first = false;
					
					var row = pointsTable.insertRow(-1);									//Insert row
					lines[j] = lines[j].replace( /<[^<>]+>/gi , "" );						//Quit html tags
					row.innerHTML = 	'<td align="left">' +
											'<a href="JavaScript:;" tableToShow="' + "subTablePoints" + i + '">' +
												'<span style="color: orange">' + 
													lines[j] + 
												'</span>' +
											'</a>' +
										'</td>';
					table.setAttribute("id", "subTablePoints" + i);
					table.style.display = "none";
					row.getElementsByTagName("a")[0].addEventListener( "click", function(){toggleDisplayId(this.getAttribute("tableToShow"));}, false  );
				}
				//Player with his points
				else{
					var row = table.insertRow(-1);
					row.innerHTML = '<td align="left"><a href="JavaScript:;">' + lines[j] + '</a></td>';
					row.getElementsByTagName("a")[0].addEventListener( "click", function(){ addPlayerPoints(this);}, false);
				}
			}
			var cell = pointsTable.insertRow(-1).insertCell(-1);
			cell.appendChild(table);
		}
	}
	
	/*
		Returns the HTML text from a forum topic
		
		subForumTitle--> 	Sub Forum to search
		topicTitle--> 		topic title to search
	*/
	function getForumTopic(subForumTitle, topicTitle){
	
		/*
			Returns the URL from a forum topic or a subForum
			
			text--> Source HTML text where we want to search URL's
			title--> title to search
			type--> subForum or topic
		*/
		function getForumUrl(text, title, type){
		
			var typeUrl = 'href="viewtopic';
			var typeTitle = '" class="topictitle">';
			
			var iniUrl = 0;
			var endUrl = 0;
			var iniTitle = 0;
			var endTitle = 0;
			
			if( type == "subForum" ){
				typeUrl = 'href="viewforum';
				typeTitle = '" class="forumlink">';
			}
		
			while( iniUrl != ( -1 + ('href="').length ) ){
			
				iniUrl = text.indexOf( typeUrl, endUrl ) + ('href="').length;
				endUrl = text.indexOf( '"', iniUrl );
				
				iniTitle = text.indexOf( typeTitle, endUrl ) + typeTitle.length;
				endTitle = text.indexOf( '</a>', iniTitle );
				
				
				if( iniTitle - endUrl > typeTitle.length ){
					continue;
				}
				if( text.substring( iniTitle, endTitle ).toLowerCase().indexOf( title ) != -1 ){
					return "external/phpBB2/" + text.substring( iniUrl, endUrl );
				}
			}
			alert("No se ha encontrado el post");
			return "";
		}
		
		var forumText = getURL( false, "responseText", "external/phpBB2" );
		var subForumUrl = getForumUrl(forumText, subForumTitle, "subForum");
		
		var subForumText = getURL( false, "responseText", subForumUrl );
		var topicUrl = getForumUrl(subForumText, topicTitle, "topic");
		
		return getURL( false, "responseText", topicUrl );
	}
	
	function actualizaEstado(text) {
	
		var lesionadosSancionados = '';
		var arrayPosts = getPosts(text);
		for( var i=0; i<arrayPosts.length; i++){
			lesionadosSancionados += normalize(arrayPosts[i]);
		}
		
		var images = document.getElementsByTagName("img");
		for(var i = 0; i <images.length; i++) {
			if(images[i].id.indexOf('estado') != -1) {
				var idImg = images[i].id;
				var jugadorNombre = idImg.replace('estado','').replace( /\_/g, ' ');
				var jugador = normalize(idImg.replace('estado','').replace( /\_/g, ' '));
				if (lesionadosSancionados.indexOf('SANCIONADOS') != -1 
						&& lesionadosSancionados.indexOf(jugador + ' (', lesionadosSancionados.indexOf('SANCIONADOS') ) != -1) {
					// Sancionado
					if (lesionadosSancionados.indexOf('Acumulacion') != -1 
						&& lesionadosSancionados.indexOf(jugador + ' (', lesionadosSancionados.indexOf('Acumulacion') ) != -1) {
						//ACUMULACION DE TARJETAS
						images[i].src='data:image/png;base64,';
						images[i].title='Es posible que ' + jugadorNombre 
							+ ' no pueda jugar el próximo partido. Parece ser que está sancionado por acumulación de tarjetas.';
					} else if (lesionadosSancionados.indexOf('Expulsion') != -1 
						&& lesionadosSancionados.indexOf(jugador + ' (', lesionadosSancionados.indexOf('Expulsion') ) != -1) {
						//DOBLE AMONESTACION
						images[i].src='data:image/png;base64,';
						images[i].title='Es posible que ' + jugadorNombre 
							+ ' no pueda jugar el próximo partido. Parece ser que está sancionado por expulsión por doble amonestación.';
					} else {
						//ROJA DIRECTA
						images[i].src='data:image/png;base64,';
						images[i].title='Es posible que ' + jugadorNombre 
							+ ' no pueda jugar el próximo partido. Parece ser que está sancionado por expulsión por roja directa.';
					}
					images[i].style.height="25px";
				} else if (lesionadosSancionados.indexOf('- ' + jugador + ' -') != -1
								|| lesionadosSancionados.indexOf('- ' + jugador + '-') != -1
								|| lesionadosSancionados.indexOf('- ' + jugador + '</span>') != -1
								|| lesionadosSancionados.indexOf('- ' + jugador + ' </span>') != -1) {
					//Lesionado
					var datosLesion = lesionadosSancionados.substring(lesionadosSancionados.indexOf('- ' + jugador) + jugador.length + 2, lesionadosSancionados.indexOf('<br', lesionadosSancionados.indexOf('- ' + jugador)));
					
					if (datosLesion.indexOf('</span>') != -1) {
						images[i].style.height="22px";
						images[i].style.width="22px";
						images[i].src='data:image/png;base64,';
					} else {
						images[i].src='data:image/png;base64,';	
					}
					datosLesion = datosLesion.replace('</span>', '');					
					images[i].title=datosLesion;
				} else {
					images[i].title='Parece que ' + jugadorNombre 
							+ ' no aparece en la zona de lesiones y sanciones.';
					images[i].src= 'data:image/png;base64,'
				}
			}
		}
	}
	
	function clickPoints(tableRow,link){ 
		toggleDisplayObj(tableRow);
		if(link.textContent[0]!='*'){
			return;
		}
		var pointsHtmlText = getForumTopic("estadísticas","puntos");
		addPoints(pointsHtmlText);
		link.textContent = link.textContent.replace( "*", "" );
	}
	function clickInjuried(tableRow,link){ 
		toggleDisplayObj(tableRow);
		if(link.textContent[0]!='*'){
			return;
		}
		var noPlayersHtmlText = getForumTopic("noticias de la liga","lista de lesionados");
		addInjuried(noPlayersHtmlText);
		link.textContent = link.textContent.replace( "*", "" );
	}
	function clickConvocated(tableRow,link){ 
		toggleDisplayObj(tableRow);
		if(link.textContent[0]!='*'){
			return;
		}
		var convHtmlText = getForumTopic("noticias de la liga","convocatorias de la liga bbva");
		addConvocated(convHtmlText);
		link.textContent = link.textContent.replace( "*", "" );
	}
	
	//Generate the table with the information from the forum
	if ( window.location.href.indexOf( "lineup" ) != -1 ){

		var allTables = document.getElementsByTagName('table');
		var bigTable = document.createElement('table');
		bigTable.style.fontSize = "12px";
		bigTable.style.width = "100%";
		
		bigTable.innerHTML = 	'<tr>' +
									'<td>' +
										'<table cellspacing="0" cellpadding="0">' +
											'<tr>' +
												'<td align="left">' +
													'<a id="link1" href="JavaScript:;">*Puntos' +
													'</a>' +
												'</td>' +
											'</tr>' +
											'<tr style="display:none" id="infoRow1">' +
												'<td>' +
													'<table id="pointsTable">' +
													'</table>' +
												'</td>' +
												'<td align="left">' +
													'<table id="totalPointsTable">' +
														'<tr>' +
															'<td id="totalPointsCell">TOTAL:0' +
															'</td>' +
														'</tr>' +
													'</table>' +
												'</td>' +
											'</tr>' +
										'</table>' +
									'</td>' +
								'</tr>' +
								
								 '<tr>' +
									'<td>' +
										'<table>' +
											'<tr>' +
												'<td align="left">' +
													'<a id="link2" href="JavaScript:;">*Lesionados/Sancionados' +
													'</a>' +
												'</td>' +
											'</tr>' +
											'<tr style="display:none" id="infoRow2">' +
												'<td id="injuriedCol">' +
												'</td>' +
											'</tr>' +
										'</table>' +
									'</td>' +
								'</tr>' +
								
								 '<tr>' +
									'<td>' +
										'<table>' +
											'<tr>' +
												'<td align="left">' +
													'<a id="link3" href="JavaScript:;">*Convocados' +
													'</a>' +
												'</td>' +
											'</tr>' +
											'<tr style="display:none" id="infoRow3">' +
												'<td id="convCol">' +
												'</td>' +
											'</tr>' +
										'</table>' +
									'</td>' +
								'</tr>' +
								
								'<tr>' +
									'<td>Dad las gracias a los foreros que comparten esta información (J,A,E, y Gsus77)' +
									'</td>' +
								'</tr>';
								
								
		allTables[ allTables.length-4 ].parentNode.insertBefore( bigTable, allTables[ allTables.length-4 ].nextSibling );
		
		var infoRow1 = document.getElementById("infoRow1");
		var infoRow2 = document.getElementById("infoRow2");
		var infoRow3 = document.getElementById("infoRow3");
		
		var link1 = document.getElementById("link1");
		var link2 = document.getElementById("link2");
		var link3 = document.getElementById("link3");
		
		link1.addEventListener( "click", function(){ clickPoints(infoRow1,this); }, false );
		link2.addEventListener( "click", function(){ clickInjuried(infoRow2,this); }, false );
		link3.addEventListener( "click", function(){ clickConvocated(infoRow3,this); }, false );
	}
	
	// estado de jugadores - by anthorlop - 20110301 - ini
	if (window.location.href.indexOf( "lineup" ) != -1
					|| window.location.href.indexOf( "playerInfo" ) != -1
					|| window.location.href.indexOf( "exchangemarket" ) != -1
					&& window.location.href.indexOf( "viewoffers" ) == -1) {
		var playerStatusHtmlText = getForumTopic("noticias de la liga","lista de lesionados");
		actualizaEstado( playerStatusHtmlText );
	}
	// estado de jugadores - by anthorlop - 20110301 - fin
}

// Cálculo de Saldo - by anthorlop - 20110222 - ini
function calcularSaldo(){

	//Colleccion con los valores de Saldo y valor del equipo.
	finanzas = document.getElementsByClassName("finances_right");
	if (window.location.href.indexOf( "exchangemarket.phtml" ) != -1
							 && window.location.href.indexOf( "viewoffers" ) == -1) {
		var allTablesMercado = document.getElementsByTagName('table');
		var saldoControl = document.createElement('table');
		saldoControl.setAttribute( "style", "width:100%; height:24px;" );
		saldoControl.setAttribute( "cellpadding", "3" );
		saldoControl.setAttribute( "cellspacing", "3" );
		saldoControl.innerHTML = '<tr><td><table><tr><td>&nbsp;</td></tr></table><table style="width:100%; height:20px;" border="0" cellpadding="0" cellspacing="0" class="tableline02"><tr> <td style="width:8px;" valign="top"><img src="i/i/curve_05.gif" width="8" height="20" border="0" alt="" /></td> <td class="headingtablesmall">Cálculo de Saldo</td><td style="width:8px;" valign="top"><img src="i/i/curve_06.gif" width="8" height="20" border="0" alt="" /></td></tr></table> <table style="width:100%; height:1px;" border="0" cellpadding="0" cellspacing="0" class="tableline02"><tr><td><img src="i/i/pxl_transparent.gif" width="1" height="1" border="0" alt="" /></td></tr></table><table style="width:100%; height:20px;" border="0" cellpadding="4" cellspacing="0"><tr class="tr1"><td class="text" style="white-space:nowrap;">Actual: <input size="12px" type="text" name="saldoActual" id="saldoActual" readonly="true" style="background-color:transparent;border:0px;color:white;font-size:12px;text-align:right;" /></td></tr></table><table style="width:100%; height:20px;" border="0" cellpadding="4" cellspacing="0"><tr class="tr1"><td class="text" style="white-space:nowrap;">Ofrecido:<input size="10px" type="text" name="ofrecido" id="ofrecido" readonly="true" style="background-color:transparent;border:0px;color:white;font-size:12px;text-align:right;" /></td></tr></table><table style="width:100%; height:20px;" border="0" cellpadding="4" cellspacing="0"><tr class="tr1"><td class="text" style="white-space:nowrap;">Total: <input size="13px" type="text" name="saldoTotal" id="saldoTotal" readonly="true" style="background-color:transparent;border:0px;color:white;font-size:12px;text-align:right;" /></td></tr></table></td></tr> ';
			
		allTablesMercado[allTablesMercado.length-5].parentNode.insertBefore( saldoControl, allTablesMercado[allTablesMercado.length-5].nextSibling );
		
		function addPuntosMiles(num){
			var rgx = /(\d+)(\d{3})/;
			while (rgx.test(num)) {
				num = num.replace(rgx, '$1' + '.' + '$2');
			}
			return num;
		}
		
		function recalcularSaldo(){
			document.getElementById('saldoTotal').value='';
			document.getElementById('ofrecido').value='';
			document.getElementById('saldoActual').value='';
			var ofrecido = 0;
			var inp = document.getElementsByTagName('input');
			for(var i = 0; i <inp.length; i++) {
				if(inp[i].type == 'text') { 
					inp[i].addEventListener("input", recalcularSaldo, false);
					inp[i].addEventListener("click", recalcularSaldo, false);
					if (inp[i].value != '') {
						inp[i].value = inp[i].value.replace( /\./g, "" );
						if (!isNaN(inp[i].value)) {
							ofrecido = parseInt(ofrecido,10) + parseInt(inp[i].value,10);
							inp[i].value = addPuntosMiles(inp[i].value);
						} else {
							inp[i].value = '';
						}
						
					}
				}
			}
			var saldo = finanzas[0].textContent;
			saldo = saldo.replace( /\./g, "" );
			saldo = saldo.replace(' Euro','');
			var total =  parseInt(saldo) -  parseInt(ofrecido);
			document.getElementById('saldoTotal').value=addPuntosMiles(total+'') + ' €';
			document.getElementById('ofrecido').value=addPuntosMiles(ofrecido+'') + ' €';
			document.getElementById('saldoActual').value=addPuntosMiles(saldo+'') + ' €';
		}
		
		recalcularSaldo();
	}
}
// Cálculo de Saldo - by anthorlop - 20110222 - fin

var normalize = (function() {
  var from = "ÃÀÁÄÂÈÉËÊÌÍÏÎÒÓÖÔÙÚÜÛãàáäâèéëêìíïîòóöôùúüûÑñÇç", 
      to   = "AAAAAEEEEIIIIOOOOUUUUaaaaaeeeeiiiioooouuuunncc",
      mapping = {};
 
  for(var i = 0, j = from.length; i < j; i++ )
      mapping[ from.charAt( i ) ] = to.charAt( i );
 
  return function( str ) {
      var ret = [];
      for( var i = 0, j = str.length; i < j; i++ ) {
          var c = str.charAt( i );
          if( mapping.hasOwnProperty( str.charAt( i ) ) )
              ret.push( mapping[ c ] );
          else
              ret.push( c );
      }      
      return ret.join( '' );
  }
 
})();


/*
	Main function
*/
function ComunioEX(){
	
	var version = "0.6.1";
	
	if( getStatusPlayer() == "Plus" ){
		return;
	}
	setStatusPlayer("NoMasKeine");
	setScriptVersion(version);
	checkUpdates(version);
	
	setSize(65);
	
	convertTables();
	convertLinksNoMasKeine();
	
	addSmileys();
	
	addForumInfo();
	
	addScores();
	
	calcularSaldo();
}

/*
	UserScript
*/
ComunioEX();


