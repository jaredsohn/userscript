// ==UserScript==
// @name           LinkedIn Account Creation
// @namespace      
// @description    Stuff
// @author         Brice
// @version        1.1
...
if (GM_getValue("varDomain"))
	emailTail = GM_getValue("varDomain");
else
	GM_setValue("varDomain", "@spankyspooks.com");
...
function fillPrimaryPage(){
		...
		if (prEmail != null){
			document.getElementById("firstName-registration").focus();
			document.getElementById("firstName-registration").value = firstnamegen;
			document.getElementById("lastName-registration").value = lastnamegen;
			document.getElementById("recipientEmail-registration").value = prEmail+emailTail;
			document.getElementById("password-registration").value = password;
			document.getElementById("btn-submit").click();
			GM_openInTab("http://webaccounts.miningbased.com/webaccounts/create?email="+prEmail+emailTail+"&password=gbconfirm&network_id=2&status=inactive")
		}
}

function changeDomain(){
	var newDomain = prompt("Input new email domain", emailTail);
	GM_setValue("varDomain", newDomain);
	emailTail = GM_getValue("varDomain", "@nicksnewmail.com");
	document.getElementById("recipientEmail-registration").value = emailTail;
...
function infoFill(){
	...
	if (document.title == "LinkedIn | Relationships Matter"){
		var place = document.getElementById("register");
		place.appendChild(button);
		place.appendChild(button2);
	...
	if (document.title == "Build Your Profile | LinkedIn"){
		...
		document.getElementById("postalCode-location-employedProfileForm").value = zipVar;
		document.getElementById("companyName-companyInfo-employeeCompany-employedProfileForm").value = company;
		document.getElementById("workCompanyTitle-employedProfileForm").value = jobTitle;
		document.getElementById("industryChooser-employeeCompany-employedProfileForm").value = industry;
		document.getElementById("employer-btn-submit").click();
	}
	...
	if (document.title == "Upload Webmail Contacts | LinkedIn"){
		...
		for ( var i = 0; i < links.length; i++ ) {
			element = links[ i ];
			if ( element.href.indexOf( "http://www.linkedin.com/reg/web-flow?ns=1&flow=" ) == 0 ) {
...
button2.addEventListener('click', changeDomain, false);
