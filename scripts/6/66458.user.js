// ==UserScript==
// @name           GC-Notes (beta)
// @namespace      David Wright (GC:ChilliSchaf)
...
var special_abilities = getElements("table", "class", "CacheInformationTable Spacing");
var div = special_abilities[0];

var notes = document.createElement("div");
notes.id = "GC-Notes";
notes.innerHTML = "<br><span style='font-weight:700;'>GC-Note</span> <font size='1'><a href='http://go.to/gc-note/'>go.to/gc-note</a></font><br><br>";
...
if(savedNotes) {
	...
	empty_notes.innerHTML = "Diesen Bereich kannst du nutzen um dir Notizen zu diesem Cache zu machen."
}
...
edit_note.innerHTML = "<br>Notizen &auml;ndern";
edit_note.title = "<br>Hier klicken um die Notizen zu bearbeiten";
edit_note.className = "n-edit";

var save_note = document.createElement("a");
save_note.innerHTML = "<br>Notizen speichern";
...
var css = '#player_notes {display:inline; float:left; width:90%; margin-top:0px;}' +
	'#note_pad {margin: 0 0 0 4px;}' +
	'#note_contents {background: #FFFBBE none repeat scroll 0 0; border: 1px solid #FFE53E; padding:10px 10px 0 10px; min-height: 0px;}' +
	'#note_contents textarea {background: #FEFDDE none repeat scroll 0 0; height:140px; width:100%; display:none; border: 0 none;}' +
	'#note_contents a {color: #0069AA; text-decoration:none; font-family:verdana; font-size: 77%; cursor: pointer; margin-top: 0px;}' +
	'#notestatic {min-height:100%; width:100%;}' +
	'#display a.note_button {cursor: pointer; margin: 0 5px 0 0; padding: 2px 3px; display:none; }' +
...
function toggle() {
	...
	if(textarea.style.display == "block") {		
		if(savedNotes) {
			...
			empty_notes.innerHTML = "Du hast zu diesem Cache noch keine Notizen angelegt.";
		}
