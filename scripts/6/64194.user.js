// ==UserScript==
// @name           The-West ArmorSet
// @namespace      http://www.puli.sk
// @include        http://*.the-west.*/game.php*
// ==/UserScript==

var wardrobe_text={en:'Kledij',sk:'Å atnÃ­k',cz:'SkÅ™Ã­Åˆ'};
var new_name_text={en:'Nieuwe set',sk:'NovÃ½ nÃ¡zov',cz:'NovÃ½ nÃ¡zev'};
var cancel_button_text={en:'Annuleren',sk:'ZruÅ¡iÅ¥',cz:'ZruÅ¡it'};
var delete_button_text={en:'Weg',sk:'ZmazaÅ¥',cz:'Smazat'};
var save_button_text={en:'Opslaan',sk:'UloÅ¾iÅ¥',cz:'UloÅ¾it'};
var save_message_text={en:'Set is opgeslaan',sk:'Å atnÃ­k bol uloÅ¾enÃ½',cz:'SkÅ™Ã­Åˆ byla uloÅ¾ena'};
var confirm_overwrite_text={en:'Er is al een set, toch opslaan? ',sk:'Naozaj chceÅ¡ prepÃ­saÅ¥ obleÄenie s nÃ¡zvom ',cz:'SkuteÄnÄ› chceÅ¡ pÅ™epsat obleÄenÃ­ pod nÃ¡zvem '};
var save_choose_name_error_text={en:'Error!!',sk:'Najprv musÃ­Å¡ vybraÅ¥ poloÅ¾ku "NovÃ½ nÃ¡zov" alebo uÅ¾ existujÃºcu poloÅ¾ku!',cz:'MusÃ­Å¡ nejdÅ™Ã­v vybrat poloÅ¾ku "NovÃ½ nÃ¡zev" nebo uÅ¾ existujÃ­cÃ­ poloÅ¾ku!'};
var delete_choose_name_error_text={en:'Error!!',sk:'Najprv musÃ­Å¡ vybraÅ¥ poloÅ¾ku!',cz:'MusÃ­Å¡ nejdÅ™Ã­v vybrat poloÅ¾ku!'};

var maxRetry=3;
var retryPeriod=200;
var gQueue = new Array();
var gQueueIndex = 0;
var gQueueMax = 0;
var gSelectedName='';
var gSetName,gTTU=0, gTTW=0,gActUnwearing=0, gActWearing=0;


function insertAfter( newNode, referenceNode){ referenceNode.parentNode.insertBefore( newNode, referenceNode.nextSibling ); }

function hideElement(element){ element.style.display='none'; }

function showElement(element){ element.style.display='block'; }

function toggleElement(element){ if (element.style.display=='none')showElement(element);else hideElement(element) };

function getArmorSet(setName){
  return GM_getValue(getServer(location)+'_'+unsafeWindow.Character.name+'_'+setName,'');
}

function createOptionsList(element){
  list = getListOfSets();
  list = list.substring(list.indexOf(';')+1);
  while (list.length>0) 
  {
    name = list.substring(0,list.indexOf(';'));
    if (list.indexOf(';')==-1) {
      name = list;
      list = '';
    } else
      list = list.substring(list.indexOf(';')+1);
    nNewItem= document.createElement('option');
    nNewItem.innerHTML=name;
    armorSet = getArmorSet(name);

    var funct="nNewItem.addEventListener('click', function(){setArmor(\""+armorSet+ "\")} , false);";
    eval(funct);
    element.appendChild(nNewItem);
  }
}

function isNull(variable) {
  return (typeof(variable) == "undefined" || variable===null)?  true: false;
}

function removeElement(el){
  el.parentNode.removeChild(el);
}

function changeToTextBox(select){
  gInputElement=document.createElement('input');
  gInputElement.style.width='123px';
  insertAfter(gInputElement, select);
  removeElement(select);
  gInputElement.focus();
  gArmorInputType='text';
  codb=document.getElementById('wardrobe_cancelOrDelete_button');
  if (codb)
    codb.innerHTML=cancel_button_text[gLang];
}

function getListOfSets(){
  return GM_getValue(getServer(location)+'_'+unsafeWindow.Character.name+':listOfSets','');
}

function createDropInvDropDown() {
  gInputElement= document.createElement('select');
  gInputElement.style.width='127px';
  list = getListOfSets();
  list = list.substring(list.indexOf(';')+1);
  first=true;
  while (list.length>0) 
  {
    name = list.substring(0,list.indexOf(';'));
    if (list.indexOf(';')==-1) {
      name = list;
      list = '';
    } else
      list = list.substring(list.indexOf(';')+1);
    nNewItem= document.createElement('option');
    nNewItem.innerHTML=name;
    var funct="nNewItem.addEventListener('click', function(){gSelectedName=\""+name+ "\"} , false);";
    eval(funct);
    if (first) {gSelectedName=name}
    first=false;
    gInputElement.appendChild(nNewItem);
  }
  if (first) {gSelectedName=new_name_text[gLang]}
  nNewItem= document.createElement('option');
  nNewItem.innerHTML='... '+new_name_text[gLang]+'...';
  nNewItem.addEventListener('click', function(){changeToTextBox(gInputElement)}, false);
  gInputElement.appendChild(nNewItem);
  codb=document.getElementById('wardrobe_cancelOrDelete_button');
  if (codb)
    codb.innerHTML=delete_button_text[gLang];
}

function changeToDropDown(text){
  createDropInvDropDown();
  insertAfter(gInputElement, text);
  removeElement(text);
  gArmorInputType='drop';
  codb=document.getElementById('wardrobe_cancelOrDelete_button');
  if (codb)
    codb.innerHTML=delete_button_text[gLang];
}

function trim(str) {
	return str.replace(/^\s+/, '').replace(/\s+$/, '');
}

function getServer(loc) {
  loc = loc.substring(loc.indexOf('//')+2);
  loc = loc.substring(0,loc.indexOf('/'));
  return loc;
}
function setValue(name, value){
  GM_setValue(name,value);
}

function saveArmor() {
  if (gArmorInputType=='text')
    setName=gInputElement.value;
  else {
    if (gSelectedName == new_name_text[gLang]){
      new unsafeWindow.HumanMessage(save_choose_name_error_text[gLang]);
      return;
    }
    if (!confirm(confirm_overwrite_text[gLang]+'"'+gSelectedName+'"?"')) return;
    setName=gSelectedName;
  }
  setName=trim(setName);
  if (setName=='') return;
  _animal=_body=_foot=_left_arm=_head=_yield=_right_arm=_neck='';
  if (!isNull(unsafeWindow.Wear.wear.animal))    _animal    = unsafeWindow.Wear.wear.animal.get_short();
  if (!isNull(unsafeWindow.Wear.wear.body))      _body      = unsafeWindow.Wear.wear.body.get_short();
  if (!isNull(unsafeWindow.Wear.wear.foot))      _foot      = unsafeWindow.Wear.wear.foot.get_short();
  if (!isNull(unsafeWindow.Wear.wear.left_arm))  _left_arm  = unsafeWindow.Wear.wear.left_arm.get_short();
  if (!isNull(unsafeWindow.Wear.wear.head))      _head      = unsafeWindow.Wear.wear.head.get_short();
  if (!isNull(unsafeWindow.Wear.wear.yield))     _yield     = unsafeWindow.Wear.wear.yield.get_short();
  if (!isNull(unsafeWindow.Wear.wear.right_arm)) _right_arm = unsafeWindow.Wear.wear.right_arm.get_short();
  if (!isNull(unsafeWindow.Wear.wear.neck))      _neck      = unsafeWindow.Wear.wear.neck.get_short();
  
  GM_setValue(getServer(location)+'_'+unsafeWindow.Character.name+'_'+setName,_animal+':'+_body+':'+_foot+':'+_left_arm+':'+_head+':'+_yield+':'+_right_arm+':'+_neck);
  if (gSelectedName != setName)
    GM_setValue(getServer(location)+'_'+unsafeWindow.Character.name+':listOfSets', getListOfSets()+';'+setName);
  new unsafeWindow.HumanMessage(save_message_text[gLang]);
  createDropdown();
}

function addToQueue(action) {
  gQueue[gQueueMax+1] = action;
  gQueueMax++;
}

function isUncarryAction(action){
  if (action=='head') return true;
  if (action=='left_arm') return true;
  if (action=='body') return true;
  if (action=='right_arm') return true;
  if (action=='yield') return true;
  if (action=='foot') return true;
  if (action=='neck') return true;
  if (action=='animal') return true;
  return false;
} 

function executeQueue(retry){
  if (retry==maxRetry || gQueueIndex>gQueueMax) {
    gQueueIndex++;
    if(gQueueIndex>gQueueMax) {
      gQueueIndex=0;
      gQueueMax=0;
    }
    return;
  }
  if (gQueueMax==0) return;
  if (gQueueIndex==0) gQueueIndex=1;
  ok = false;
  if (isUncarryAction(gQueue[gQueueIndex])){
    ok = unsafeWindow.Wear.uncarry(gQueue[gQueueIndex]);

    found = true;
  } else {
    items = unsafeWindow.Bag.getInstance().items;
    found = false;
    for(i in items){
      if (items[i].get_short()==gQueue[gQueueIndex]){
        found = true;
        if (!isNull(unsafeWindow.Wear.wear[items[i].get_type()]) && unsafeWindow.Wear.wear[items[i].get_type()].get_short()==items[i].get_short()) {
          ok = true;
          break;
        }
        ok = unsafeWindow.Bag.getInstance().carry(i);
      }
    }
  }
  if (ok || !found) {
    gQueueIndex++;
    if(gQueueIndex>gQueueMax) {
      gQueueIndex=0;
      gQueueMax=0;
    }
  } else {
    if (isNull(retry)) retry=0;
    setTimeout(function(){executeQueue(retry+1)},retryPeriod*(retry+1)*2);
    return;
  }
  setTimeout(function(){executeQueue()},retryPeriod);
}

function refreshGInputElement(){
  if (gArmorInputType=='text') {
    changeToDropDown(gInputElement);
    return;
  }
  changeToTextBox(gInputElement);
  changeToDropDown(gInputElement);
}

function removeArmor() {
  if (gArmorInputType=='text') {
    changeToDropDown(gInputElement);
    return;
  }
  if (gSelectedName == new_name_text[gLang]) {
    new unsafeWindow.HumanMessage(delete_choose_name_error_text[gLang]);
    return;
  }
  if (!confirm(confirm_overwrite_text[gLang]+'"'+gSelectedName+'"?"')) return;
  GM_setValue(getServer(location)+'_'+unsafeWindow.Character.name+'_'+gSelectedName,'');
  list = getListOfSets();
  list = list.split(';');
  newList = '';
  for (i in list)
    if (list[i]!=gSelectedName && list[i]!='')
      newList = newList+';'+list[i];
  if (newList==';') newList='';
  GM_setValue(getServer(location)+'_'+unsafeWindow.Character.name+':listOfSets', newList);
  refreshGInputElement();
  createDropdown();
}

function setArmor(armorSet) {
  ANIMAL=0;BODY=1;FOOT=2;LEFT_ARM=3;HEAD=4;YIELD=5;RIGHT_ARM=6;NECK=7;

  if (document.getElementById('bag'))
    unsafeWindow.AjaxWindow.maximize('inventory');
  else {
    unsafeWindow.AjaxWindow.show('inventory');
    setTimeout(function(){addInventoryButtons()},100);
    setTimeout(function(){setArmor(armorSet)},1000);
    return;
  }
  
  setArray= new Array();
  setArray = armorSet.split(':');

  gTTU=0;
  gTTW=0;
  gActUnwearing=0;
  gActWearing=0;

  //remove wrong clothes and apply right clothes  
  if (!isNull(unsafeWindow.Wear.wear.animal))
    if (setArray[ANIMAL]!=unsafeWindow.Wear.wear.animal.get_short())
      addToQueue('animal');
  if (setArray[ANIMAL])
    if (isNull(unsafeWindow.Wear.wear.animal) || unsafeWindow.Wear.wear.animal.get_short()!=setArray[ANIMAL])
      addToQueue(setArray[ANIMAL]);

  if (!isNull(unsafeWindow.Wear.wear.body))
    if (setArray[BODY]!=unsafeWindow.Wear.wear.body.get_short())
      addToQueue('body');
  if (setArray[BODY])
    if (isNull(unsafeWindow.Wear.wear.body) || unsafeWindow.Wear.wear.body.get_short()!=setArray[BODY])
      addToQueue(setArray[BODY]);

  if (!isNull(unsafeWindow.Wear.wear.foot))
    if (setArray[FOOT]!=unsafeWindow.Wear.wear.foot.get_short())
      addToQueue('foot');
  if (setArray[FOOT])
    if (isNull(unsafeWindow.Wear.wear.foot) || unsafeWindow.Wear.wear.foot.get_short()!=setArray[FOOT])
      addToQueue(setArray[FOOT]);

  if (!isNull(unsafeWindow.Wear.wear.left_arm))
    if (setArray[LEFT_ARM]!=unsafeWindow.Wear.wear.left_arm.get_short())
      addToQueue('left_arm');
  if (setArray[LEFT_ARM])
    if (isNull(unsafeWindow.Wear.wear.left_arm) || unsafeWindow.Wear.wear.left_arm.get_short()!=setArray[LEFT_ARM])
      addToQueue(setArray[LEFT_ARM]);

  if (!isNull(unsafeWindow.Wear.wear.head))
    if (setArray[HEAD]!=unsafeWindow.Wear.wear.head.get_short())
      addToQueue('head');
  if (setArray[HEAD])
    if (isNull(unsafeWindow.Wear.wear.head) || unsafeWindow.Wear.wear.head.get_short()!=setArray[HEAD])
      addToQueue(setArray[HEAD]);

  if (!isNull(unsafeWindow.Wear.wear.yield))
    if (setArray[YIELD]!=unsafeWindow.Wear.wear.yield.get_short())
      addToQueue('yield');
  if (setArray[YIELD])
    if (isNull(unsafeWindow.Wear.wear.yield) || unsafeWindow.Wear.wear.yield.get_short()!=setArray[YIELD])
      addToQueue(setArray[YIELD]);

  if (!isNull(unsafeWindow.Wear.wear.right_arm))
    if (setArray[RIGHT_ARM]!=unsafeWindow.Wear.wear.right_arm.get_short())
      addToQueue('right_arm');
  if (setArray[RIGHT_ARM])
    if (isNull(unsafeWindow.Wear.wear.right_arm) || unsafeWindow.Wear.wear.right_arm.get_short()!=setArray[RIGHT_ARM])
      addToQueue(setArray[RIGHT_ARM]);

  if (!isNull(unsafeWindow.Wear.wear.neck))
    if (setArray[NECK]!=unsafeWindow.Wear.wear.neck.get_short()) 
      addToQueue('neck');
  if (setArray[NECK])
    if (isNull(unsafeWindow.Wear.wear.neck) || unsafeWindow.Wear.wear.neck.get_short()!=setArray[NECK])
      addToQueue(setArray[NECK]);

  executeQueue();
}

function isElementHidden(element){
  return element.style.display!='block';
}

function createDropdown(){
  newLI=document.getElementById('armorset_li');
  newDropDown = document.getElementById('armorset_combobox');
  hidden = true;
  if(newDropDown) {
    hidden=isElementHidden(newDropDown);
    newDropDown.parentNode.removeChild(newDropDown);
  }
  else
    newLI.addEventListener('click', function(){toggleElement(newDropDown)}, false);

  newDropDown = document.createElement('select');
  newDropDown.setAttribute('id','armorset_combobox');
  newDropDown.style.width='127px';
  if (hidden)
    hideElement(newDropDown);

  createOptionsList(newDropDown);
  insertAfter(newDropDown,newLI);
}

function addInventoryButtons(){
  var elements=unsafeWindow.document.getElementsByTagName('TD');
  var invTargetPos=null;
  for (i=0;i<elements.length;i++) {
    styleAttr=elements[i].getAttribute('style');
    if (styleAttr && styleAttr.indexOf('images/border/border_b.png')>0 && 
        elements[i].parentNode && elements[i].parentNode.parentNode &&
        elements[i].parentNode.parentNode.parentNode &&
        elements[i].parentNode.parentNode.parentNode.parentNode &&
        elements[i].parentNode.parentNode.parentNode.parentNode.getAttribute("id")=="window_inventory_content") {
      invTargetPos=elements[i];
      break;
    }
  }
  if (isNull(invTargetPos)) {
    setTimeout(function(){addInventoryButtons()},100);
    return;
  }
  invTargetPos=document.getElementById('window_inventory_content');
  var nTable=document.createElement('table');
  nTable.setAttribute("style","position: absolute; margin-left: 10px; margin-top: -5px;");
  invTargetPos.appendChild(nTable);
  nTR= document.createElement('tr');
  nTable.appendChild(nTR);
  var nTD= document.createElement('td');
  nTR.appendChild(nTD);
  var nLabel= document.createElement('span');
  nLabel.innerHTML=wardrobe_text[gLang]+':';
  nLabel.setAttribute("style","font-weight:bold");
  nTD.appendChild(nLabel);
  nTD=document.createElement('td');
  nTR.appendChild(nTD);

  createDropInvDropDown();
  nTD.appendChild(gInputElement);
  nTD=document.createElement('td');
  nTR.appendChild(nTD);
  var nb= document.createElement('img');
  nb.setAttribute("style","cursor: pointer;");
  nb.setAttribute("src","data:image/gif;base64,"+buttonImg);
  nb.addEventListener("click",function(){saveArmor();if (gArmorInputType=='text') {changeToDropDown(gInputElement)}},false);
  nTD.appendChild(nb);
  var ns=document.createElement('span');
  ns.setAttribute("style","cursor:pointer; width: 71px; color: rgb(255, 255, 255); display: block; z-index: 200; font-size: 15px; position: relative; top: -22px; text-align: center; left: 0px; font-weight: bold;");
  ns.addEventListener("click",function(){saveArmor();if (gArmorInputType=='text') {changeToDropDown(gInputElement)}},false);
  ns.innerHTML=save_button_text[gLang];
  nTD.appendChild(ns);

  nTD=document.createElement('td');
  nTR.appendChild(nTD);
  var nb= document.createElement('img');
  nb.setAttribute("style","cursor: pointer;");
  nb.setAttribute("src","data:image/gif;base64,"+buttonImg);
  nb.addEventListener("click",function(){removeArmor();},false);
  nTD.appendChild(nb);
  var ns=document.createElement('span');
  ns.setAttribute("style","cursor:pointer; width: 71px; color: rgb(255, 255, 255); display: block; z-index: 200; font-size: 15px; position: relative; top: -22px; text-align: center; left: 0px; font-weight: bold;");
  ns.setAttribute("id","wardrobe_cancelOrDelete_button");
  ns.addEventListener("click",function(){removeArmor();},false);
  ns.innerHTML=delete_button_text[gLang];
  nTD.appendChild(ns);
}

var gLang='en';
lang = window.location.href.substring(window.location.href.indexOf("//")+2,window.location.href.indexOf("//")+4);
if (wardrobe_text[lang]) gLang=lang;

var location = window.location.href;
var gArmorInputType='drop';
var gInputElement=null;
var menuButtonImg='\nHCE0GyI0HSM2GyAyGyAyHCE0AAAA';
//var cancelButtonImg='\nGyAyGyAyIyw9FRorExgpHCE0HCI1HCI2GyAyGyAyHCE0HCE0AAAA';
var buttonImg='\nGyAyGyAyIyw9FRorExgpHCE0HCI1HCI2GyAyGyAyHCE0HCE0AAAA';
var menuFortsButton = document.getElementById('menu_forts');
var newLI = document.createElement('li');
newLI.innerHTML='<a href="#"><img src="data:image/gif;base64,'+menuButtonImg+'" style="z-index:100"/><span style="width: 102px; color: rgb(255, 255, 255); display: block; z-index: 200; font-size: 13px; position: relative; top: -20px; text-align: center; left: 24px;">'+wardrobe_text[gLang]+'</span></a>';
newLI.setAttribute("id","armorset_li");
insertAfter(newLI,menuFortsButton);
createDropdown();

//search for inventory button
var pageElements = unsafeWindow.document.getElementsByTagName('A');
var invButton;
for (var i=0; i< pageElements.length;i++) {
  invButton=pageElements[i];
  if (invButton.getAttribute('onclick')=="AjaxWindow.show('inventory');" && invButton.parentNode.id=='menu_inventory') {
    invButton.addEventListener("click",function(){setTimeout(function(){addInventoryButtons()},100);},false);
  }
}

// fix "abdorment_right"
document.getElementById('abdorment_right').style.zIndex="5";
