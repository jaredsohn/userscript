// ==UserScript==
// @name           OGame InfoCompteinter
// @namespace      Im Me
// @description    OGame InfoCompte Inter
...
var langue = new Array('org','us','se','com.es','com.pt','de', 'dk');
...
if (Langue=='com.es')
{
	...
	var nom_def = new Array('Lanzamisiles',"ser peque","ser grande","n Gauss","nico","n de plasma","Misil de intercepc","Misil interplanetario","a de protecc","pula grande de protecc");
	var nom_tech = new Array('a de espionaje',"a de computaci","a militar","a de defensa","a de blindaje","a de energ","a de hiperespacio","Motor de combust","Motor de impulso","Propulsor hiperespacial","a l"+aaigu+"ser","a i"+oaigu+"nica","a de plasma","Red de investigaci","a de expedici");
	var nom_bat = new Array('Mina de me',"Mina de cri","Sintetizador de de","Planta de energ","lanta de fusi","brica de Robots","brica de Nanobots","Hangar","Almac"+eaigu+"n de metal","Almac"+eaigu+"n de cristal","ontenedor de deuterio","Laboratorio ","Terraformer","Silo","sito de la Alianza","Base lunar","Sensor Phalanx","Salto cu");
	var nom_flotte = new Array("Nave peque"+nacc+"a de carga","Nave grande de carga","Cazador ligero ","Cazador pesado","Crucero","Nave de batalla","Colonizador","Reciclador","Sonda de espionaje","Bombardero","Destructor","Estrella de la muerte","Acorazado");
...
if (Langue != '')	
{
	...
	MatchTab = url.match('uni([0-9]{1,2}).ogame.'+Langue+'.game.index.php.page=([a-zA-Z0-9_]+)(?:&dsp=1)?(?:&no_header=1)?&session=([a-z0-9]{12})(?:&mode=([a-zA-Z0-9]+))?');
	var uni = MatchTab[1];
	...
	if ((url.indexOf('option',0))>=0)
	{
		if ((url.indexOf('&infocompte_plus=oui',0))>=0) 
		{
			...
			if (Boutton[0]) 
			{
				Boutton[0].addEventListener("click", function() 
				{
						...
						BOUTTON[0].innerHTML = '<a href="http://uni'+uni+'.ogame.'+Langue+'/game/index.php?page=options&session='+session+'&infocompte_plus=oui">'+done+'</a>';
			...
			if (Boutton[0]) 
			{
				Boutton[0].addEventListener("click", function() 
				{
					...
					BOUTTON = document.getElementsByClassName("boutton_VG");
					BOUTTON[1].innerHTML = '<a href="http://uni'+uni+'.ogame.'+Langue+'/game/index.php?page=options&session='+session+'&infocompte_plus=oui">'+done+'</a>';
				}, true);
			}
		 }
		...
		else 
		{
			if (url.indexOf('=oui',0) == -1) 
			{
				...
				New_Table.innerHTML = '<tr><td colspan="2" class="c"><a href="http://uni'+uni+'.ogame.'+Langue+'/game/index.php?page=options&session='+session+'&infocompte_plus=oui">'+ICoption+'</a></td></tr>';
					...
					{	
						...
						{ // Calcul de la production 
							...
							{ 
								...
								nombre = parseInt(nombre.replace(/\./g,'')); 
								if(isNaN(nombre)) nombre = 0;
								prod += (3-i)*10*nombre*Math.pow(1.1,nombre);	
							}			
						}
	...
	{ 
		...
		{		
			...
			tr_ProdJours.innerHTML = '<th width="60px" colspan="1" >'+Production+'</th><th colspan=\"3\" >'+addPoints(Math.round(prod))+ ' '+Point_day+'</td>';	
		}
