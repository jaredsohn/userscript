...
PATTERN['part.visual'] = 'div[@class="product"]/div[@class="info"]/div[@class="data"]/div[2]/strong/text()';
PATTERN['part.handling'] = 'div[@class="product"]/div[@class="info"]/div[@class="data"]/div[3]/strong/text()';
...
function loadMe(){
	...
	me.handling = parseInt(getPatternResult('//div[@id="sidebar"]/div[3]/div[3]/ul/li[2]//div[@class="wrap"]/strong/text()').snapshotItem(0).data);
	me.visual = parseInt(getPatternResult('//div[@id="sidebar"]/div[3]/div[3]/ul/li[3]//div[@class="wrap"]/strong/text()').snapshotItem(0).data);
...
function getTeamTableHeader(f,d){
	...
	th.appendChild(name);
	name.appendChild(document.createTextNode('Name' + ((f=='name')?dir:'')));
	...
	th.appendChild(work);
	work.appendChild(document.createTextNode('Work hours' + ((f=='work')?dir:'')));
...
function show_requestRespect(){
	...
	if (xRequestPlayer.snapshotLength == 1){
		...
		var groups = /^\s*(.*)\s+(challenges you to a race\.)\s*$/.exec(text);
...
function show_repair(){
	...
	if (xRepairs==null)
		...
		for (var i=0; i<xRepairs.snapshotLength; i++){
			...
			if ((repair.name!=null)&&(repair.performance!=null)&&(repair.damage!=null)&&(repair.cost!=null)&&(repair.button!=null)){
				var groups = /^\s*(\d+[.]\d+)[(](\d+)[)]/.exec(repair.performance);
				if ((groups==null) || (groups.length!=3))
					continue;
				repair.performance = parseFloat(groups[1]);
				repair.performanceMax = parseFloat(groups[2]);
				...
				groups = /\w*:\s*\$(\d+)\s*$/.exec(repair.cost.replace(',',''));
				if ((groups==null) || (groups.length!=2))
					continue;
				repair.cost = parseInt(groups[1]);
	...
	if ((xRepairHolder==null)||(xRepairHolder.snapshotLength!=1))
		...
		li.innerHTML = '<strong>Repair <span style="color:#D6930A">'+t.performanceDamage.toFixed(2)+
		'</span> performance points for <span style="color:'+((t.cost <= me.money)? '#8dc63f':'#9E0B0F')+'">$'+t.cost+'</span> '+
		...
		"javascript:__doPostBack('ctl00$MainContentWrapper$PageContentPlaceHolder$repairBody$ctl00$btnRepairAll','')"+'"><span>Repair All</span></a>'+
		...
		"javascript:__doPostBack('ctl00$MainContentWrapper$PageContentPlaceHolder$repairBody$ctl00$btnReplaceAll','')"+'"><span>Replace All</span></a><br/>';
		...
		trh.innerHTML = '<th title="The name of the part">Part</th><th title="Repairs left"><img alt="Repairs left" src="/images2/repair/repair_icon.png"/></th><th title="The performance points missing">P</th><th title="Cost to repair">$</th>';
...
function show_bestbuy(){
	...
	if ((xParts==null)||(xParts.snapshotLength==0))
		...
		for (var i=0; i<xParts.snapshotLength; i++){
			...
			var xVisual = getPatternResult(PATTERN['part.visual'],xPart);
			var xHandling = getPatternResult(PATTERN['part.handling'],xPart);
			...
			part.visual = ((xVisual==null)||(xVisual.snapshotLength!=1))? null : xVisual.snapshotItem(0).data;
			part.handling = ((xHandling==null)||(xHandling.snapshotLength!=1))? null : xHandling.snapshotItem(0).data;
			...
			if ((part.name != null)&&(part.performance != null)&&(part.visual != null)&&(part.handling != null)&&(part.cost != null)&&(part.buy != null)){
				...
				if ((part.visual.length>0)&&(part.visual.charAt(0)=='+'))
					part.visual = part.visual.substring(1)
				part.visual = parseInt(part.visual);

				if ((part.handling.length>0)&&(part.handling.charAt(0)=='+'))
					part.handling = part.handling.substring(1)
				part.handling = parseInt(part.handling);
				...
				part.visualRatio = (part.visual==0)? Number.MAX_VALUE :  (part.cost/part.visual);
				part.handlingRatio = (part.handling==0)? Number.MAX_VALUE :  (part.cost/part.handling);
				part.overall = part.performance + part.handling; //+part.visual
				part.overallRatio = (part.overall==0)? Number.MAX_VALUE :  (part.cost/part.overall);
				...
				total['visual']+=part.visual;
				total['handling']+=part.handling;
				total['overall']+=part.overall;
	...
	if ((xSidebar==null)||(xSidebar.snapshotLength!=1))
		...
		headerStrong.innerHTML = 'Best Buy';
		...
		if ((total['visual']>0)&&(div=getInfoDiv(parts,'visual')))
			body.appendChild(div);
		if ((total['handling']>0)&&(div=getInfoDiv(parts,'handling')))
			body.appendChild(div);
		if ((total['overall']>0)&&(div=getInfoDiv(parts,'overall')))
			body.appendChild(div);
		if ((total['performance']==0)&&(total['handling']==0)&&(total['visual']==0)&&(total['overall']==0)){
...
function getInfoDiv(parts,field)
{
	...
	var trh = document.createElement('tr');
	table.appendChild(trh);
	trh.innerHTML = '<th title="Cost per point">$&nbsp;/&nbsp;P</th><th title="Part Name">Part</th><th title="Points">P</th><th title="Part Cost">$</th>';
