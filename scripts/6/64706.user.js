...
//	gm.log("my army/rank/level:" + this.stats.army + "/" + this.stats.rank + "/" + this.stats.level);
	for(var s=0; s<ss.snapshotLength; s++) {
		...
		if (type == 'Raid') {
			...
			var rank =parseInt(levelm[1],10);
			var level=parseInt(levelm[3],10);
			var army =parseInt(levelm[5],10);
		} else {
			...
			if(gm.getValue('DemiPointsFirst','') && !gm.getValue('DemiPointsDone',true)) {
				...
				if (parseInt(demiPointList[deityNumber],10)==10 || !gm.getValue('DemiPoint'+deityNumber)) continue;
			}
			...
			var level=parseInt(levelm[1],10);
			...
			var army = parseInt(nHtml.GetText(subtd.snapshotItem(2)).trim(),10);
		}
...
//		gm.log("Army Ratio:" + armyRatio + " Level:" + level + " Rank:" + rank + " Army: " + army)
	...
	if (type == 'Raid') {
		if ( (engageButton = this.monsterEngageButtons[gm.getValue('targetFromraid','')]) ) caap.Click(engageButton);
...
CheckMonster:function(){
	...
	if (attackButton) {
		...
		if (!dam) {
			...
			this.DeceiveDidIt("NotargetFrombattle_monster");
			gm.setValue("navLink","");
...
dragObject:function(element, attachElement, lowerBound, upperBound, startCallback, moveCallback, endCallback, attachLater){
  ...
  function dragStart(eventObj)
  {
    ...
    elementStartPos = new style.Position(parseInt(element.style.left), parseInt(element.style.top));
