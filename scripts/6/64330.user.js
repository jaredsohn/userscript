// ==UserScript==
// @name           Favorites Spy Reports GR 1.0 by Dahakaa
...
// @version        1.0beta
// @include        http://*.ogame.gr/game/index.php?page=messages&session=*
// @exclude        http://uni*.ogame.gr/*
...
var Favors = {
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Αυτή η αναφορά έχει ήδη επιλεχθεί');
				return false;
			}
	...
	Fsave: function(obj) {
		try {
			...
			unsafeWindow.fadeBox('Οι αλλαγές ολοκληρώθηκαν');
		} catch (e) {}
	},
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">Από</th><th class="subject">Θέμα</th><th class="date">Ημερομηνία</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Αρχηγείο Στόλου</td><td class="subject">' +
				...
				'<a title="|Διαγραφή απο τα αποθηκευμένα" class="ogc20Fdel tips" rel="'+el.id+
			...
			unsafeWindow.fadeBox('Δεν υπάρχουν αποθηκευμένες αναφορές');
		}
		return false;
	},
