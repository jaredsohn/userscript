...
var upArrowImage = '/images/game/body/icons/button-friend-online.png';
var downArrowImage = '/images/game/body/icons/button-friend-offline.png';
...
if (loc.match(/^account/)) {
	var settingsPos = $("div.tab_container"); // "a[name='twitter']:first"
	
	// First of all insert the update div
	settingsPos.after('<div id="flash_notice2"> <a href="http://userscripts.org/scripts/show/69525" target="_blank" style="text-decoration: underline;">check here for updates</a> </div><p>&nbsp;<p>');
	// Check for script updates
	//checkUpdate(false, false);
	
	// Now the settings come
	settingsPos.after('<h2>Legends of Zork Helper</h2>'+"\n"
		+'<div><table border="0" cellpadding="5" cellspacing="5" id="lozHelper" class="inventory">'+"\n"
		+'	<th colspan="3" style="color:black;">choose which parts of the LoZ Helper you need... some may work :P</th>'+"\n"
	...
	var myCharacter = $('.row-1-data-cell-1-name-surname a').attr('href');
	...
	var myZorkmids = parseInt($('div.row-1-data-cell-zorkmids').attr('title').replace(/,/g, ''));
...
function myCharacterDataReady(myCharData) {
	...
	if (loc.match(/^arena/) && !loc.match(/^arena\/fight/) && GM_getValue('activateArena', true)) {
		...
		// ForEach <a> found 
		$('#full_opponents_list table tr td:not(.tab-table-mail)').each(function(intIndex) {
		...
		if (GM_getValue('activateAttChanger', true)) {
			$('div#recent_fights_list').before('<div class="protection" style="margin: 450px 0px; padding: 5px"><p>Your current attitude stance is: <span id="current_stance">'+myCharacterData.attitude+'</span></p><p><a href="javascript:void(0)" id="sa_click">Change to Shock & Awe</a> | <a href="javascript:void(0)" id="sm_click">Change to Smart Moves</a> | <a href="javascript:void(0)" id="hr_click">Change to Hit & Run</a> | <a href="javascript:location.reload();">Reload Page</a></p></div>');
	...
	// Are we looking at a rankings page?
		...
		// ForEach <a> found
		$('.avatar td:not(.table-avatar-cell) a').each(function(intIndex) {
			// Double check that the link we are looking at is indeed a player link
			if(!$(this).attr('href').match(/heroes\/\d+-/))
...
function parseTargetResponse(targetData){
	...
	var el = longVersion ? $('div#full_opponents_list table tr td:not(.tab-table-mail) a[href^="javascript: select(\''+targetData.id+'"]') : $('div.avatar-table tr td:not(.table-avatar-cell) a[href="/heroes/'+targetData.id+'"]');
...
function checkFocus() {
	...
	var playerId;
	$('.avatar-table table tr td:not(.table-avatar-cell)').each(function(intIndex) {
		hitCount--;
