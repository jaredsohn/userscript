...
function activeMain() {
	...
	if (GM_getValue("Active_" + suffixGlobal, -1) > -1) {
		...
		var currentVillageId = getActiveVillageId();
		        //Anubis
			//************* 
			if(d.getMinutes() >=5 && d.getMinutes() <20)
			{
			stopEngine();
			}

			if(d.getMinutes() >=25 && d.getMinutes() <40)
			{
			stopEngine();
			}

			if(d.getMinutes() >=45 && d.getMinutes() <60)
			{
			stopEngine();
			}
			...
			if (fque == "" || fque.length == 0 || doneHere == 1) {
				...
				acVillageFlag = false;
				setTimeout("window.location.replace('http://s5.travian.asia/build.php?newdid=99482&gid=16&id=39')", 3000);
				stopEngine();
			} else {
...
function isReallyRallyPoint() {
	...
	if(d.getMinutes() <25 && d.getMinutes() >20)
			{
			startEngine();
			}
	if(d.getMinutes() <45 && d.getMinutes() >40)
			{
			startEngine();
			}
	//*************
