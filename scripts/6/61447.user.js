...
// @description    Scrobbling your music to Last.fm when listening to douban.fm(豆瓣电台)
// @include        http://www.douban.com/radio/
// @include        http://douban.fm/*
// @version        0.20
// @require        http://userscripts.org/scripts/source/57756.user.js
// ==/UserScript==

ScriptUpdater.check(61447, '0.20');  

GM_registerMenuCommand('dbScrobbler - Last.fm config', userConfig); 
GM_registerMenuCommand('dbScrobbler - check update', checkUpdate); 
...
function checkUpdate() {
    ScriptUpdater.forceCheck(61447, '0.20');  
}
...
function wait_for_uw() {
	if (!unsafeWindow.DBR || !unsafeWindow.$) {
        retry_times --;
        if (retry_times < 0) {
        ...
        GM_log('established communication with uw...')
		rlog_old = unsafeWindow.DBR.rlog;
        $ = unsafeWindow.$;

        dbs_info = $('<div style="bottom: 0pt; position: absolute; ' + 
            'left:0pt; font-size: 12px; -moz-border-radius: 5px 5px 0 0; ' + 
            ' padding: 5px; background-color: #3FA156; display: none; '  + 
            'color: #FFF;" id="dbs_info"></div>');

        login_form = $('<div id="login_form"' + 
            'style="position: absolute; left: 0; bottom: 0; display: none;' + 
            'background-color: #3FA156; font-size: 12px; width: 150px;' + 
		...
		//settitle_old = unsafeWindow.DBR.settitle;
		hijack();
        if (GM_getValue('user')) {
            handshake();
        } else {
...
function get_song_from_list(s) {
    var sid = s.match(/(\d+)$/)[1];
	for (var i = 0; i < playlist.length; ++i) {
        if (playlist[i].indexOf(sid) != -1) {
            var result = playlist[i].match(/\[(.*) \/ (.*) \(\d+\)\]/);
            return {
                title: result[1],
                artist: result[2]
            }
...
function hijack() {
	...
	unsafeWindow.DBR.rlog = function(message) {
        //GM_log('message:', message);
		switch (message) {
            case "play ok":
                submit(playlist.shift());
                break;

            case "play skipped":
                if (new Date().getTime() - play_start > 20000) {
                    GM_log('skipped, scrobbling...')
                    submit(playlist.shift());
                } else {
                    GM_log('skipped, too short, don\'t scrobble...')
                }
                playlist = [];
                break;
            ...
            case "marked good":
                // todo...
                //submit(playlist.shift(), 'L');
                playlist = [];
                break;

            case "conceled good mark":
                // todo...
                playlist = [];
                break;
            ...
            case "new list loaded...":
                break;

            default:
                if (message.indexOf('now start to play:') != -1) {
                    var song = get_song_from_list(message);
                    now_playing(song.artist, song.title);
	...
	unsafeWindow.DBR.settitle = function(title) {
		settitle_old(title);
		var result = playlist[0].match(/\[(.+) \/ (.+) \(\d+\)\]/);
		var title = result[1];
		var artist = result[2];
...
function submit(song, action) {
    var title = last_song.title;
    var artist = last_song.artist;
