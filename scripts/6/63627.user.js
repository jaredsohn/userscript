...
// 
// Version 2009.12.11.1
//	- Another attempt to fix "try again later" bug. I think I got it this time.
//	- Added certain delays and randomness to the click cycle to try to obfuscate
//		the script from being detected.
//
// Version 2009.12.10.1
//	- Screen is refreshed if mission sticks on "try again later".
//
// Version 2009.12.07.1
//	- Auto-Run Limits are now persisted. Close, open, refresh browser windows without affecting
//		how many missions are set to run. Note that you still must have the Mission windows open in order
//		for the script to start the missions.
//	- Do not let "try again later" missions keep new missions from starting.
//
// Version 2009.12.06.1
//	- When on the Missions page there will be an "Auto-Start" link over each Start Mission button.
//		Clicking this link will set an internal "limiter" variable for that mission to 1. If the
//		Start button is green, the mission will be started -- once.  The "Auto-Start" link will
//		then be replaced by a <Clear> and an <Inc to 2> link. The former will clear the "limiter"
//		variable while the latter will increase it. As long as a mission is running fewer times
//		than the "limiter" setting and the Start button is green, said mission will start.
//	- Manually starting missions will not conflict with auto-run missions.
//  - There is a 4 second timer that checks for changes, so not all features react instantaneously.
//	- Missions can be auto-run on more than one planet at a time by opening the Mission page for 
//		each planet in its own tab or window. Each will operate independently.
// 	- Auto-run limits will not be saved if the browser window is refreshed or closed.
//
// TBD:
...
var ciStartScan_MinLoopDelay = 2;	
var ciStartScan_MaxLoopDelay = 8;

function MissionProp(mission_planet, mission_key, mission_title, auto_start_limit, click_text, status_ready) {
			...
			if (document.getElementById(newID)) {
				...
				if (selectFleet) {
					...
					if (pMission.auto_start_limit > 0) {
						elt.appendChild(document.createTextNode('<Inc to '+(pMission.auto_start_limit+1)+'>'));
					} else {
						elt.appendChild(document.createTextNode('Auto-Start'));
					}
					...
					if (pMission.auto_start_limit > 0) {
						...
						if (pMission.auto_start_limit > 1) {
							elt.appendChild(document.createTextNode('<Dec to '+(pMission.auto_start_limit-1)+'>'));
						} else {
							elt.appendChild(document.createTextNode('<Clear>'));
						}
...
*/
		...
		if (--ciStartScan_WaitCount <= 0) {
			...
			ciStartScan_WaitCount = Math.floor(Math.random() * (ciStartScan_MaxLoopDelay - ciStartScan_MinLoopDelay) + ciStartScan_MinLoopDelay);
	...
	window.setTimeout(MainLoop, ciScanDelay_msecs);
}
