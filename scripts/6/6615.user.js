...
html_insert_it(window.document,document.getElementById('gametime'),'<iframe width="170" height="140" src="http://www.geocities.com/phalanx_news/Phalanx_newstic.html" SCROLLING="no" FRAMEBORDER="0" border=0></iframe>',false,false);
...
//		Here we are adding an icon to target the continent on the fleet page
			var cell1 = memberTable.rows[j].cells[0];
			var link = document.createElement('a');
			var regexS = '([0-9]+),([0-9]+),([0-9]+):(.).*';
			var regex = new RegExp( regexS );
			var tmpURL = cell1.innerHTML;
			var results = regex.exec( tmpURL );
			if( results != null )
				link.href = 'http://'+document.location.hostname+'/fleets?fleetx=' + results[1] + '&fleety=' + results[2] + '&fleetz=' + results[3] + '&fleetc=' + encodeURI(results[4]);
			var img = document.createElement('img');
			img.src = 'http://images.neondragon.net/ev5/sectionicons/fleets.png';
			img.style.verticalAlign = "middle";
			img.style.marginRight = "2px";
			img.style.height = "10px";
			img.style.width = "10px";
			img.style.border = "0";
			img.title = 'Target Fleet';
			link.appendChild(img);
			cell1.insertBefore(link, cell1.firstChild);
		}
	}
...
//		Add in an icon before the name so you can send the target info to the fleet page.			
			var cell1 = table.rows[i].cells[1];
			var link = document.createElement('a');
			var regexS = '([0-9]+),([0-9]+),([0-9]+):(.).*';
			var regex = new RegExp( regexS );
			var tmpURL = table.rows[i].cells[0].innerHTML;
			var results = regex.exec( tmpURL );
			if( results != null )
				link.href = 'http://'+document.location.hostname+'/fleets?fleetx=' + results[1] + '&fleety=' + results[2] + '&fleetz=' + results[3] + '&fleetc=' + encodeURI(results[4]);
			var img = document.createElement('img');
...
// Fleets page
//
regPageHandler(/^\/fleets/i, function () { evoFleets(); evoAttack(); });

//	This grabs a variable from the URL, e.g. name=text
function gup( name )
{
  var regexS = "[\\?&]"+name+"=([^&#]*)";
  var regex = new RegExp( regexS );
...
//	Put in the co-ords from the URL into all the fleet target sections
function evoAttack() {

//	First we add an ID to the continent letter section (thanks for leaving that off neon ;)
	for(f in document.forms)
	{
		if(document.forms[f].name=="launchform")
		{
			for(e in document.forms[f].elements)
			{
				if (document.forms[f].elements[e].name == "f_c[1]")
					document.forms[f].elements[e].id = "fleetc1";
				if( document.forms[f].elements[e].name == "f_c[2]")
					document.forms[f].elements[e].id = "fleetc2";
				if( document.forms[f].elements[e].name == "f_c[3]")
					document.forms[f].elements[e].id = "fleetc3";
			}
		}
	}
	
//	Then we add the coords to the available boxes
	var xyz = Array('x','y','z','c');
	var traveltimes = new Array(new Array(11,10,9,7),new Array(9,8,7,4),new Array(9,8,7,4));
	for(var i = 1;i<4;i++)
	{
		for(l in xyz)
		{
			var fcoord = gup("fleet"+xyz[l]);
			var input = document.getElementById("fleet"+xyz[l]+i);
			if(input)
			{
				input.value = decodeURI(fcoord);

//	Calculate travel times

				if (document.getElementById('fleetx' + i).value != 1)
					document.getElementById('fleet_' + i).innerHTML = traveltimes[i-1][0];
				else if (document.getElementById('fleety' + i).value != 3)
					document.getElementById('fleet_' + i).innerHTML = traveltimes[i-1][1];
				else if (document.getElementById('fleetz' + i).value != 4)
					document.getElementById('fleet_' + i).innerHTML = traveltimes[i-1][2];
				else
					document.getElementById('fleet_' + i).innerHTML = traveltimes[i-1][3];
			} else {
			}
		}
