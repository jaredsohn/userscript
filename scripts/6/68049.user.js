...
var MY_URL = 'http://tools.iroBay.com/';

// Constants
var VERSION = '1.00';
var RELEASE_DAY = '04 Feb 2010';
var SCRIPT_URL = 'http://userscripts.org/scripts/show/68049';
var LOCALE = 'en/';
var tempList = null;
...
function calcUserProfit(){
    ...
    GM_xmlhttpRequest({
        ...
        onload: function (data) {
            ...
            $('table.employees_details thead tr').append('<th class="e_employee">سود</th>');
            ...
            $.each($('table.employees_details tbody tr'), function(i, item){
                ...
                // construct table td
                var profit_td = '<td>n:<strong style="color:'+ColorChoose(profit_obj.profit)+'" id="t_profit_'+user_id+'" title="بازدهی با سلامتی کنونی">'+profitReplacer(profit_obj.profit)+'</strong>';
                // get productivity by fact
                profit_td += '<br/>l:<strong style="color:'+ColorChoose(profit_obj.fakt_profit)+'" id="f_profit_'+user_id+'" title="بازدهی با سلامتی زده شده">'+profitReplacer(profit_obj.fakt_profit)+'</strong>';
                // get wellness by fact
                profit_td += '<br/>w:<strong id="f_wellness_'+user_id+'" title="سلامتی کار انجام شده">'+wellnessReplacer(profit_obj.fakt_wellness)+'</strong></td>';
            ...
            $('.employees_details tbody').append('<tr><td colspan="3" class="e_salary" style="text-align:left;">فروش محصول در بازار: <input type="text" id="real_price" class="sallary_field" style="float:none;"></input><span class="ecur" style="float:none;">'+ecur+'</span></td><td class="e_salary" style="text-align:left;">مواد خام در بازار: <input type="text" id="real_raw_price" class="sallary_field" style="float:none;"></input><span class="ecur" style="float:none;">'+ecur+'</span></td><td><span class="vround-btn-start"><span class="vround-btn-end"><a id="recalc" class="vround-btn-core" style="background:#E9F5FA url(/images/parts/icon-arrow.gif) no-repeat scroll 86px 50%; width:86px">تایید تغییرات</a></span></span></td><td>n:<strong style="color:'+ColorChoose(total_profit)+'" id="t_total_profit" title="مجموع بازدهی با سلامتی کنونی">'+profitReplacer(total_profit)+'</strong><br/>l:<strong style="color:'+ColorChoose(total_fakt_profit)+'" id="f_total_profit" title="مجموع بازدهی با سلامتی زده شده">'+profitReplacer(total_fakt_profit)+'</strong></td></tr>');
            $('#real_price').val(product_price.toFixed(2));
            $('#real_raw_price').val(raw_price.toFixed(2));
            var raw_td = raw_price == 0?'<td>&nbsp;</td>':'<td><span class="regular" style="font-size:12px;">مواد خام:<br/> استفاده شده: '+(total_fakt_productivity*company_quality).toFixed(2)+' ('+(total_fakt_productivity*company_quality*raw_price).toFixed(2)+' '+ecur+')<br/>مورد نیاز: '+((total_productivity-total_fakt_productivity)*company_quality).toFixed(2)+' ('+((total_productivity-total_fakt_productivity)*company_quality*raw_price).toFixed(2)+' '+ecur+')<br/>مجموع: '+ (total_productivity*company_quality).toFixed(2) + ' ('+(total_productivity*company_quality*raw_price).toFixed(2)+' '+ecur+')</span></td>';
            $('.employees_details tbody').append('<tr><td colspan="3"><span class="regular" style="font-size:12px;">تولیدات:<br/> محصول تولید شده: '+ (total_fakt_productivity/raw_parts_in_one).toFixed(2) + '<br/> مجموع: '+(total_productivity/raw_parts_in_one).toFixed(2)+'</span></td>'+raw_td+'<td colspan="2"><span class="regular" style="font-size:12px;">حقوق:<br/> پرداخت: '+worked_salary.toFixed(2) + ' ' + ecur +'<br/>مانده: '+(total_salary - worked_salary).toFixed(2) + ' ' + ecur +'<br/> مجموع: '+total_salary.toFixed(2) + ' ' + ecur +'</span></td></tr>');
        }
    });
        ...
        onload: function (data) {
            ...
            $.each($('div.infoholder div.left span.special'), function(i, item){
                if (i == 0){
                    ...
                    $(item).parent().parent().append('<div style="margin-top:7px;"><span class="regular" style="padding-left:30px;">قیمت در بازار</span><br/><br/><span class="currency" style="padding-left:20px;">'+ rprice.product +'</span>'+'<br/><span class="currency" style="padding-left:20px; font-size:10px;">'+ (rprice.gold_curr*parseFloat(rprice.product.split(' ')[0])).toFixed(3) +' G</span>'+price_vat+'<br/><span class="currency" style="padding-left:20px; font-size:10px;">'+ (rprice.gold_curr*parseFloat(rprice.product_vat.split(' ')[0])).toFixed(3) +' G</span></div>');
                    ...
                    $(item).parent().parent().append('<div style="margin-top:7px;"><span class="regular" style="padding-left:30px;">قیمت در بازار</span><br/><br/><span class="currency"  style="padding-left:20px;">'+ rprice.raw +'</span>' +'<br/><span class="currency" style="padding-left:20px; font-size:10px;">'+ (rprice.gold_curr*parseFloat(rprice.raw.split(' ')[0])).toFixed(3) +' G</span></div>');
                }
            });
            // add market button
            $('div.holder:eq(3) div.action').append('<span class="vround-btn-start"><span class="vround-btn-end"><a class="vround-btn-core" href="/'+LOCALE+'market/country-'+rprice.country_id+'-industry-'+rprice.industry_id+'-quality-'+rprice.quality+'" title="">ورود به بازار</a></span></span>');
        }
    });
}
...
//  1.00        04 Feb 2010         - Translated to Persian
