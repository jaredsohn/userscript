// ==UserScript==
// @name           BBCodes para Taringa Corregido por LeoTorreZ
// @namespace      SharkaleBBCODER Corregido por LeoTorreZ
// @description    BBCoder para T! (Crear post, comentarios, menú)
// @include        http://*taringa.net/*
// @exclude        http://br.taringa.net/*
// @version        25.0 Corregido por LeoTorreZ
// @copyright      Copyright (c) 2009, Sharkale Corregido por LeoTorreZ
...
function CrearBarraBBC(){
		GM_addStyle(".markItUpHeader {padding:5px 0;}.error {margin-top:3px;}");
			...
			$('#procesando').html(HTML).slideDown(1000, function(){
				$.ajax({
					...
					success: function(h){
						...
						var HTML = '<div class="cita" style="color:#464646;text-shadow:0 1px 0 #CCCCCC;"><img src="'+URL+'cerrar.gif" alt="[X]" title="'+tarLang["Preview"][3]+'" border="0" onclick="javascript:($(\'#procesando\').slideUp(1000))" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />'+tarLang["Preview"][4]+' - Sharkale® Corregido por LeoTorreZ</div>';
	...
	$.ajax({
		...
		GM_log(h);
			if(h.search('container400') != -1){
				...
				$('.error').hide('slow').html(error.html()).show('slow');
				error = null;
			}else{
				var html = h.replace(/^[\w\W]*\<\!\-\- Comentarios \-\-\>/i,'<html><body>').replace(/\<div[^>]*>/,'');
				html = html.replace(/\<div id="procesando[\w\W]*$/,'');
				html += '<div id="procesando" style="display:none">&nbsp;</div>';
				$('.Sharkale_cargando').hide();
		...
		error: function(h){
			$('.error').hide('slow').html('Como de costumbre Taringa no responde. =)').show('slow');
			$('.Sharkale_cargando').hide();
		}
	});
});
...
Taringa_Set_Function("add_comment", function(ult, nro){
	...
	$('#mBtn btnOk').attr('disabled', 'true');
	$('#mBtn btnOk').addClass('ui-state-disabled');
	$('.Sharkale_cargando').show();
	$('#error').show();
	$.ajax({
		type: 'POST',
		url: '/comentario2.php',
		data: 'postid=' + datospag.postid + '&comentario=' + encodeURIComponent($('#body_comm').val()) + '&ult='+ult+'&key=' + datospag.user_key + '&nro=' + nro,
		success: function(h){
			if(h.charAt(0)==0){ //Error
				...
				$('#procesando').html(h.substring(3));
				$('#procesando').slideDown('slow');
				$('#body_comm').val('');
				$('.error').hide('slow');
			}
			$('.Sharkale_cargando').hide();
			$('#error').hide();
			$('#mBtn btnOk').removeAttr('disabled');
			$('#mBtn btnOk').removeClass('ui-state-disabled');
		},
		error: function(h){
			$('.error').hide('slow').html('Como de costumbre Taringa no responde. =)').show('slow');
			$('.Sharkale_cargando').hide();
...
///////////////------------------Agregar Boton Ir Abajo--------------------////////////////////
	if($('#post-title')){
		...
		$('#post-title').html(Tierra + '<span style="vertical-align: middle;padding-top: 1px;">' + $('#post-title').html()+ '</span>');
		$('div[class="post-comentarios"]').find('div[class="floatR"]:first').prepend('<a href="#pie-a" class="size13"><strong>Ir a la Tierra</strong></a>');
	}
...
///////////////-----------------Agregar Opciones en Comentarios------------------////////////////////
		if(PunFav == true){
			if($('#dar-puntos').size() != 0 || $('#post-acciones').size() != 0){
				var stropc = '';
				if($('#dar-puntos').size() == 1){
					var opc1 = $('#dar-puntos').clone().html();
					if(opc1.match(/solidarios[\w\W]*post-acciones/))
						opc1 = '<b>Dar puntos:</b> Los posts solidarios no se pueden calificar';
					else if(opc1.match(/para dar[\w\W]*post-acciones/))
						opc1 = '<b>Dar puntos:</b> No hay puntos para dar';
					stropc = opc1;
				}else stropc = '<b>Dar puntos:</b> Usuarios Novatos no pueden calificar';
				if($('#post-acciones').size() == 1){
					var opc2 = $('#post-acciones').clone();
				...
				$('span[class="error"]:first').html('<span style="float:left;text-align:left;">Agregar un nuevo comentario</span><span style="float:right;text-align:right;font-weight:normal;width: 550px;">'+stropc+'</span>');
			}
		}
///////////////-----------------Agregar Emoticones------------------////////////////////
		...
		BotonOcultar = '<input class="mBtn btnOk" value="'+(EmotVis == true? "Ocultar Emoticones":"Mostrar Emoticones")+'" onclick="Ocultar_Emoticones(this);return false;" id="ocultar_emot" type="button" title="Mostrar/Ocultar la barra de emoticones rápidos">';
		$('#emoticons').remove();
		$('a[href="javascript:openpopup()"]').parent().remove();
		$('input[value="Enviar Comentario"]').before('<span style="float:left;">'+BotonOcultar+'</span><span id="seccion_enviar" style="float:right;"></span><br><br>'+CrearBarraEmoticones());
		$('#mBtn btnOk').attr('id','sharkborrar').clone(true).attr('id','mBtn btnOk').prependTo('#seccion_enviar');
		$('#sharkborrar').remove();
		$('#mBtn btnOk').removeAttr('disabled');
		autor = $(':button[value="Eliminar"]').is(':visible');
		GM_addStyle('.Sharkale_cargando {display:none;}');
		HTML = '<div class="commenttext" style="font-size: 11px;"><input type="button" value="Actualizar Comentarios - Sharkale®" class="button" title="Actualizar todos los comentarios. Si hay demasiados puede trabarse un momento." onclick="actualizar_comentarios_post()">&nbsp;&nbsp;<span class="Sharkale_cargando"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></span></div>';
		$('#procesando').parent().after(HTML);
		HTML = '<input type="mBtn btnOk" value="Ver Preview del Comentario" class="mBtn btnOk" title="Ver Preview del Comentario" onclick="preview()">';
	...
	if($('div[typeof="foaf:Person"]').find('a[href="/perfil/LeoTorreZ"]').length != 0){
		...
		mod_sharkale.html(mod_sharkale.html().replace(/New Full User/gi,'Ultra Mega User').replace(/No Rankeado/,'T! Rank: 1').replace(/ Puntos/,'4684 Puntos'));
		...
		mod_sharkale.find('img[title="Ultra Mega User"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','LeoTorreZ La otra envidia de los Mods...');
