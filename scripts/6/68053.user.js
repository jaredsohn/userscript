...
var discussionURL= 'http://userscripts.org/scripts/discuss/57917';
var debug=false;
...
SetControls:function(force) {
	...
	htmlCode += this.ToggleControl('Monster','MONSTER');
		...
		htmlCode += "<div id='AutoZynga_WhenMonsterHide' style='display: " + (GM.GetValue('WhenMonster',false)!='Never'?'block':'none') +"'>"
			...
			htmlCode += '<tr><td>Get Demi Points First</td><td> ' + this.MakeCheckBox('DemiPointsFirst',false,'DemiList',demiPointsFirstInstructions,true)+  '</td></tr>';
				var demiPtList = ['Am','Ma','Co','Au','Az'];
				for (var demiPtItem in demiPtList) {
					htmlCode += demiPtList[demiPtItem] + this.MakeCheckBox('DemiPoint'+demiPtItem,true);
				}
			htmlCode += "</div>";
	...
	htmlCode += this.ToggleControl('Other','OTHER OPTIONS');
		...
		htmlCode += "<tr><td>Monster delay secs</td><td>" + this.MakeNumberForm('seedTime','Max random delay to battle monsters',300,"type='text'  size='4' style='font-size: 10px'") + "</td></tr></table>";
		htmlCode += '</div>';
	htmlCode += "<hr/> </div>";
	...
	autoZyngaRestart.addEventListener('click',function(e) {
		...
		document.getElementById("AutoZynga_div").style.background='#efe';
...
//		Zynga.WaitMainLoop();
	...
	controlDiv.addEventListener('mousedown',function(e) {
		document.getElementById("AutoZynga_div").style.background='#fee';
//		nHtml.clearTimeouts();
...
CheckResults:function() {
	...
	if (!GM.GetValue('HaveGift')) {
		...
		} else if (giftDiv = nHtml.FindByAttrContains(document.body,'a','href','castle/tracker.php?n=friend_request_click')) {
			if(giftDiv.parentNode.parentNode.parentNode.style.backgroundColor !== 'rgb(255, 255, 255)'){
				GM.Log('We have a gift waiting');
				GM.SetValue('HaveGift',true);
			}
		}
...
DrawProperties:function() {
	...
	var propByName=this.IterateProperties(function(prop) {
		...
		var roi=(parseInt((prop.income/prop.totalCost)*2400000) /1000);
...
try{
	...
	if(ss.snapshotLength<=0) {
		...
		if (!this.CheckForImage('tab_raid_on.gif')) {
			return this.NavigateTo("battle,raid");
		}
...
DemiPoints:function() {
	...
	if (this.CheckTimer('DemiPointTimer')) return this.NavigateTo('battle','battle_on.gif');
...
if (GM.GetValue('LastVersion',0) != thisVersion) {
	...
	if (parseInt(GM.GetValue('LastVersion',0))<127) {
		for each(var n in GM.ListValues()){
			if (GM.GetValue(n)) GM.SetValue(n,GM.GetValue(n).toString().replace(/~/g,ls).replace(/`/g,os));
		}
	}
