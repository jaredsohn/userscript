...
function check_version(url, data) {
    GM_xmlhttpRequest({
		...
		if(testtext.test(xhr.responseText)) { 
			...
			var version2 = version1[1].split('&#x000A');
			var version3 = version2[0];
		
			if(VERSION!=version3) { if(confirm('Su script Combat Report Auto-Converter está obsoleto. ¿Desea actualizar a la última versión?')) {  openInNewTab('http://userscripts.org/scripts/source/56551.user.js','Actualizar Script');  } } else { 
updateVersionCheck();
...
window.open('http://board.ikariam.com.mx/index.php');
...
function BuildStandardReport() {
	...
	var place2 = IKCRC_REPORT.info.place.split("Batalla en ");
	var place = '';
...
var winner_attacker = 0;
	for(i=0;i<attackers.length;i++) {
		if(attackers[i]==winner) {
			winner_attacker = 1;
		}
	}
		...
		if (unitName != null) {
			...
			var defender = IKCRC_REPORT.battle.defenders[allUnits[unit]];
				...
				if (attacker != null) {
					...
					if(GM_getValue('city')==2) { report = report.replace(/la ciudad/,'el puerto'); }
...
results += '<br/>';
results += '<br/>';
results +='[size=12][b]Vencedor';

if(winner_attacker==1) { if(attackers.length>1) {results += 'es';  }  } else {  if(deffenders.length>1) { results += 'es'; }  }

results += '[/b][/size]';
results += '<br/>';
results += '[i][color=';

if(winner_attacker==1) { results += 'red'; } else { results += 'green'; }
				
results +='][size=18]';
if(winner_attacker==1) { results += attackers_string; } else { results += deffenders_string; }
...
if(deffenders.length>1) {results += 'es'; }
	...
	report += '<br/>[b][i]Compactado con: [url=http://board.ikariam.es/index.php?page=Thread&threadID=129508]Combat Report Auto-Converter[/url] v2.2.3 © by [url=http://board.ikariam.es/index.php?page=User&userID=47984]WTF[/url][/i][/b]';
