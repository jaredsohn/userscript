...
var Favors = {
	createButtons: function(e) {
		...
		unsafeWindow.$("a.ajax_thickbox[href*='cat=7']", Utils.mC).each(function(i) {
			...
			var a = unsafeWindow.$('<a>').attr({
				...
				'id': 'save' + id,
				'rel': id,
				'title': 'Spionagebericht speichern',
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Spionagebericht schon vorhanden');
				return false;
			}
	...
	Fsave: function(obj) {
		try {
			...
			unsafeWindow.fadeBox('Spionagebericht gespeichert');
		} catch (e) {}
	},
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">Absender</th><th class="subject">Betreff</th><th class="date">Datum</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Flottenkommando</td><td class="subject">' +
				...
				'<a title="Spionagebericht löschen" class="ogc20Fdel tips" rel="'+el.id+
