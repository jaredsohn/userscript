// ==UserScript==
// @name          Amazon Greater Victoria Public Library Linky
// @namespace     http://tannock.net
// @description	  v1.5.1 Search the Greater Victoria Public Library Catalog from Amazon book listings.  
// @include       http://*.amazon.*
// ==/UserScript==

// Modified to somewhat work with Greater Victoria Public Library
...
//	GM_log('Amazon Vancouver Public Library Linky');	
	...
	var libraryIsbnUrlPattern = 'http://catalogue.gvpl.ca/search/i?SEARCH='
	var libraryTitleUrlPattern = 'http://catalogue.gvpl.ca/search~S1/?searchtype=t&searcharg='
	var libraryName = 'the Greater Victoria Public Library';
...
//	GM_log("getBookStatuses"+isbnsIndex+ " " + isbns.length);
	...
	if (isbnsIndex < isbns.length){
		updateStatusHTML('Available at ' + isbns[isbnsIndex] + " in " + libraryName + '...');
		var title = getTitle();
		...
		link.setAttribute('href', libraryIsbnUrlPattern +encodeURIComponent(isbn));
		link.setAttribute('target', "_blank");
		link.style.color = "green";

		var label = document.createTextNode('Available at ' + libraryName);
