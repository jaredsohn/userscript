// ==UserScript==
// @name           Ikariam Research Times Arabic
// @version        1.0
// @author         Yazmat
// @description    Script to show research times for Ikariam 0.3.2+ in arabic  (Advisor in academy desactivated because of a probleme)
...
// A translation of the Ikariam Research Times made by HomerOfXi in arabic 
// you can download the original script at http://userscripts.org/scripts/show/57209


ScriptUpdater.check(69315, '1.0');
		...
		for(var i=0; i < researches.length; i++) {
			...
			if(hoursNeeded > 0) {	
				...
				if(showTime) { //only build string if option is true or does not exist
				  strTime = '<img src="/skin/resources/icon_time.gif" style="display:block; float:left;"/><br/>' + '    ~ ' + ResearchTimes.getRemainingTime(hoursNeeded);
				}
	...
	views:{
		academy:function() { 
			...
			if (IkaTools.getVal('showAcademyResearch')) {
				...
				#mainview .researchType label {	position:absolute; left:0px; width:1px; height:1px;	overflow:hidden; } \
				#mainview .researchType .costs h5 {	position:absolute; left:0px; width:1px; height:1px;	overflow:hidden; } \
			...
			");
				...
				GM_addStyle("#academy #mainview div#currentResearch .content { height:270px; text-align:left;}"
      ...
      var showTime = IkaTools.getVal('showTime');   
		  //create the settings panel
			...
			settingsPanel.innerHTML = '<h3 class="header"><span class="textLabel">Ikariam Research Times Arabic</span></h3><div class="content"><form action="index.php" method="GET"><input type=hidden name=view value="options"><table cellpadding="0" cellspacing="0">'
			    + '<tr><th>إظهار البحوث في الأكاديمية</th><td>' 
			    + '<input id="showAcademyResearch" type="checkbox" name="sar" ' + (showAcademyResearch ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>إظهار النسبة</th><td>' 
			    + '<input id="showPercent" type="checkbox" name="sp" ' + (showPercent ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>إظهار الوقت المتبقي</th><td>' 
			    + '<input id="showTime" type="checkbox" name="st" ' + (showTime ? 'checked' : '')
					+ '></td></tr>'
					+ '</table><div class="centerButton"><input id="rt_save" class="button" type="submit" value="تسجيل الخصائص" "/></div></form></div><div class="footer"></div></div>';
