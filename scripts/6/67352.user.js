// ==UserScript==
// @name				DSMoveReports
// @author				Heinzel, pinjam
// @description			Setzt einen Button, mit dem man saemtliche Berichte, in denen ein vom Benutzer eingegebenes Keyword enthalten ist, in eine bestimmte Gruppe verschieben kann. Zudem werden Buttons gesetzt, die  verschiedene Sorten von Berichten verschieben.
...
function getAllReports() {
	...
	for(var x = 0; x < reportElements.length; x++) {
		...
		Report.id = reportElements[x].id.replace(/labelText_(\d+)/, '$1');
		if(reportElements[x].parentNode.firstChild.nextSibling.nodeName == 'IMG'){
			if (reportElements[x].parentNode.firstChild.nextSibling.src.indexOf('green.png')>0) {Report.green = 1; };
			if (reportElements[x].parentNode.firstChild.nextSibling.src.indexOf('yellow.png')>0) {Report.yellow = 1;};
			if (reportElements[x].parentNode.firstChild.nextSibling.src.indexOf('red.png')>0) {Report.red = 1;};
			if (reportElements[x].parentNode.firstChild.nextSibling.src.indexOf('blue.png')>0) {Report.blue = 1;};
			if (reportElements[x].parentNode.firstChild.nextSibling.nextSibling.nextSibling.nodeName != 'SPAN') {
				if (reportElements[x].parentNode.firstChild.nextSibling.nextSibling.nextSibling.src.indexOf('max_loot/0.png')>0) {Report.empty = 1;};
			}
		}
		else
		{
		...
		if (reportElements[x].parentNode.parentNode.parentNode.parentNode.innerHTML.indexOf('neu') == -1) Report.read = 1;
		reports.push(Report);
	}
	return reports;
}
...
function moveReports() {
	...
	var error = false;
	if(checks === 0) {
		var error = 'Keine Berichte mit diesem Keyword vorhanden!';
	} else if(groupname === '') {
		var error = 'Sie müssen erst noch einen Gruppennamen angeben, in den die Berichte verschoben werden sollen!';
...
function createPreferencesMenu() {
	var tabs = _evaluate('//a[.="Öffentliche Berichte"]/parent::td/parent::tr/parent::tbody');
	...
	span.innerHTML += "<br />In folgende Gruppe:<br />";
