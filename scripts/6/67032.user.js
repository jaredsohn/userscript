// ==UserScript==
// @name           Adds a link to download video for XVIDEOS.COM
// @namespace      http://userscripts.org/users/76078
// @version        0.2.0
// @author         charmy
// @description    Adds a link to download video and a button to watch video for XVIDEOS.COM and XNXX.COM
...
//		autoplay_id: 'AddsALinkSetupWatchAutoplay',
		remove_sponsor_id: 'AddsALinkSetupWatchRemoveSponsor',
		title_style: 'text-align:center;margin:0 0 5px 0;font-weight:bold;',
		ul_style: ul_style,
	...
	video: {
		...
		remove_sponsor_id: 'AddsALinkSetupVideoPlayerRemoveSponsor',
...
var	max_height = 0;
var	listener_query = '//a[@class="miniature"]/ancestor::table/..';
var	listener_type = TYPE_INSERTED;
var	anchor_query = '//a[@class="miniature"]';
...
if(window.location.href.match(/\/video\d+\//)) {
...
switch(page_type) {
case WATCH_PAGE:
	anchor_query = '//div[contains(@id,"photo")]//a[contains(@href,"/video")]';
	added_query = '../..';
	cell_query =  '../..';
	break;
...
function AddDownloadLink() {
	...
	if(xp.snapshotLength) {
		...
		for (var i=0; i < xp.snapshotLength; i++) {
			...
			var title = anchor.textContent.replace(/\n/g, '');
...
// reload player for watch page
function reload_player(embed) {
...
function watch_page_proc() {
	...
	if(custom.watch.remove_sponsor) {
		var embed = document.evaluate('//div[@id="player"]/embed', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
		if(embed) {
			var flashvars = embed.getAttribute('flashvars');
			if(/&sponsor_name=.*/.test(flashvars)) {
				embed.setAttribute('flashvars', flashvars.replace(/&sponsor_name=.*/,''));
//				reload_player(embed);
	...
	var pid = ExtractPid(href);
	var title = '';
	var node = document.evaluate('//title', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(node) {
		title = node.textContent.match(/(.*?) - /)[1];
	}
	watch_page_pid = pid;
	var base = document.createElement('div');
	...
	base.setAttribute('style', style);
	var parent = document.evaluate('//div[@id="player"]/../../../../../..', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        parent.appendChild(base);
	queue[pid] = {
		title: title,
		watch_page: true,
...
// Add menu
	GM_registerMenuCommand( "##### Set up this script(XVIDEOS.COM) #####", Setup);
...
function GetEmbedCode(div) {
	var xp = document.evaluate('.//div[@id="player"]/embed[@type="application/x-shockwave-flash"]', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		var val = xp.parentNode.innerHTML;
		return GetEmbed(ConvHtml(val));
	}
	return '';
}

function GetEmbedPlayer(div) {
	var xp = document.evaluate('.//input[@name="media_embed_code"]', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		var val = xp.getAttribute('value');
		return GetEmbed(ConvHtml(val));
	}
	return '';
}
...
function GetTitle(div) {
	var xp = document.evaluate('.//title', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		var work = xp.textContent.match(/(.*)\s+-\s+.*\.COM/);
		if(work) {
			return work[1];
		}
...
function GetUploadDate(div) {
	var xp = document.evaluate('.//u[contains(.,"Upload date:")]/following-sibling::font', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		return xp.textContent;
	}
	return '';
...
function GetViews(div) {
	var xp = document.evaluate('.//u[contains(.,"Viewed:")]/following-sibling::font/text()', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		var work = xp.textContent.match(/(\d+)\stimes/);
		if(work) {
			return work[1];
		}
	}
...
function GetComments(div) {
	var xp = document.evaluate('.//span[@onclick="commentsDisplay()"]/b', div, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if(xp) {
		var work = xp.textContent.match(/(\d+)/);
		if(work) {
			return work[1];
		}
	}
...
function GetVoteInfo(text) {
	var p = text.indexOf('displayFlashVars');
	if(!p) return '';
	text = text.substr(p+16);
	p = text.indexOf('rating=');
	if(!p) return '';
	text = text.substr(p);
	p = text.indexOf("'");
	if(!p) return '';
...
function ParseSourceXml(text, pid)
{
	...
	videoInfo['upload'] = GetUploadDate(div);
	var voteInfoStr = GetVoteInfo(text);
	videoInfo['voteInfo'] = '';
	videoInfo['rating_vote'] = '';
	videoInfo['rating'] = '';
	if(voteInfoStr) {
		var work = voteInfoStr.split('&');
		var voteInfo = {};
		for(var i in work) {
		...
		videoInfo['voteInfo'] = voteInfo;
		videoInfo['rating'] = voteInfo['rating'];
		videoInfo['rating_vote_good'] = voteInfo['vgood'];
		videoInfo['rating_vote_bad'] = voteInfo['vbad'];
	}
	...
	if(videoInfo['embedCode']) {
		...
		videoInfo['flashvars_str'] = embed.getAttribute('flashvars');
//GM_log(videoInfo['flashvars_str']);
		if(videoInfo['flashvars_str']) {
			var work = videoInfo['flashvars_str'].split('&');
			videoInfo['flashvars'] = {};
			for(var i in work) {
				var keyval = work[i].split('=');
				var key = keyval[0];
				var val = keyval[1];
				videoInfo['flashvars'][key] = val;
			}
			videoInfo['video_url'] = decodeURIComponent(videoInfo['flashvars']['flv_url']);
		}
	} else 
...
// Meta data
	var info = '';
	if(custom.list.upload && queue[pid]['upload']) {
		info += "Upload : "+queue[pid]['upload'] +"<br />";
	}
	if(custom.list.comments && queue[pid]['comments']) {
		info += "Comments : "+queue[pid]['comments'] +"<br />";
	}
	if(custom.list.ratings) {
		if(queue[pid]['rating_vote_good']) {
			info += "Good : "+queue[pid]['rating_vote_good'];
			if(custom.list.good_percent) {
				if(queue[pid]['rating']) {
					info += " ("+queue[pid]['rating'] +"%)";
				}
			}
			info += "<br />";
		}
		if(queue[pid]['rating_vote_bad']) {
			info += "Bad : "+queue[pid]['rating_vote_bad'] +"<br />";
		}
	}
...
function AddButtonIntoVideoPanel(pid, parent, id, val, style, cb) {
...
function WatchVideo(id) {
	...
	if(custom.player.remove_sponsor) {
		embedCode = embedCode.replace(/&sponsor_name=.*?\"/,'"');
	}
...
//	}
	...
	AddButtonIntoVideoPanel(pid, div, video_panel.close.id, 'X', style, CloseVideoPanel)
...
function Update_Video_Panel() {
	...
	var style = buildStyle(custom.style.video_panel.download, video_panel.download.style);
	download_button.setAttribute('style', style);
	if(custom.player.download_link) {
		download_button.style.display = 'block';
	} else {
...
function Make_Html_For_List_Page() {
	var html = ''+
		...
		'<li style="'+setup_box.list.upload_date_style+'"><input type="checkbox" id="'+setup_box.list.upload_id+'">Upload</li>' +
//		'<li style="'+setup_box.list.views_style+'"><input type="checkbox" id="'+setup_box.list.views_id+'">Views</li>' +
		...
		'<input type="checkbox" id="'+setup_box.list.good_percent_id+'">Percentage of Good rating' +
		'</li>'+
		'</ul>'+
		'';
...
function Setup_List_Page() {
	...
	Setup_Set_Check_Box(setup_box.list.upload_id, custom.list.upload);
//	Setup_Set_Check_Box(setup_box.list.views_id, custom.list.views);
	...
	Setup_Set_Check_Box(setup_box.list.good_percent_id, custom.list.good_percent);
...
function Save_Options_List_Page() {
	...
	custom.list.upload = document.getElementById(setup_box.list.upload_id).checked;
//	custom.list.views = document.getElementById(setup_box.list.views_id).checked;
	...
	custom.list.good_percent = document.getElementById(setup_box.list.good_percent_id).checked;
	SetSetupDataListPage();
...
//		'<li style="'+setup_box.video.lock_ratio_style+'"><input type="checkbox" id="'+setup_box.video.lock_ratio_id+'">Lock aspect ratio</li>'+
		...
		'<li style="'+setup_box.video.remove_sponsor_style+'"><input type="checkbox" id="'+setup_box.video.remove_sponsor_id+'">Remove sponsor box</li>'+
		'</ul>'+
...
//	Setup_Set_Check_Box(setup_box.video.lock_ratio_id, custom.player.lock_ratio);
	Setup_Set_Check_Box(setup_box.video.remove_sponsor_id, custom.player.remove_sponsor);
...
//	custom.player.lock_ratio = document.getElementById(setup_box.video.lock_ratio_id).checked;
	custom.player.remove_sponsor = document.getElementById(setup_box.video.remove_sponsor_id).checked;
...
function Make_Html_For_Watch_Page() {
	var html = ''+
		...
		'<li style="'+setup_box.watch.remove_sponsor_style+'"><input type="checkbox" id="'+setup_box.watch.remove_sponsor_id+'">Remove sponsor box</li>'+
		'</ul>'+
		'';
...
function Setup_Watch_Page() {
	...
	Setup_Set_Check_Box(setup_box.watch.remove_sponsor_id, custom.watch.remove_sponsor);
...
function Save_Options_Watch_Page() {
	...
	custom.watch.remove_sponsor = document.getElementById(setup_box.watch.remove_sponsor_id).checked;
