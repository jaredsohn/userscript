// ==UserScript==
// @name           MargoMap Ukis correction
// @namespace      http://userscripts.org/users/126311
// @description    CN map
// @copyright      Ukis
// @version        1.0
...
function myScript() {
      ...
      this.activateTooltip = function(){
        ...
        $('#miniMapContainer div.point').live('mouseover', function(event){
          ...
          try{
            ...
            var klan = typeof(tmpRawElements[id].clan) != 'undefined' ? ' Klan: '+tmpRawElements[id].clan : ''
            ...
            $(miniMapTooltip.tooltip).css({'top' : event.pageY, 'left' : (event.pageX + 10)})
  ...
  k_down = function(a) {
    ...
    if (b == 69 && targ.id != 'chatIn'){
      MinimapObject.switchVisibility();
    }
    
    /*if (b == 78 && targ.id != 'chatIn'){
      //MinimapObject.switchAutoMove();
    }*/
  ...
  elements.prototype.add = function (b) {
    ...
    if (a[0] == "type=other") {
        if (a[1] != "id=" + hero.pid) {
            ...
            if (tmpObj.id == 11505){
              tmpObj.icon = 'http://game6.margonem.pl/obrazki/postacie/spec/whitekitty.gif';
            }
  ...
  elements.prototype.del = function (c, b) {
    for (var a in this.eList) {
        if ((this.eList[a].id == c) && (this.eList[a].type == b)) {
            ...
            if (this.eList[a].type == 'equip' && this.eList[a].inbag){
              delete bagFill[this.eList[a].x][this.eList[a].y];
            }
            delete this.eList[a]
        }
    }
