// ==UserScript==
// @name           Replay Rewrite - no480fields
// @namespace      pbr/rr
...
const scriptName = "Replay Rewrite - no480fields";
const scriptVersion = "09.10.18";
const scriptWebpage = "http://userscripts.org/scripts/show/60117";
...
var alwaysShowPlayText = true;//(unsafeWindow.hide_scores != 1);
...
function restructurePage() {
    ...
    area.style.backgroundImage = null;
...
function drawPlayer(id, x, y, z, p) {
    ...
    if (id == "ball") {
	    ...
	    if (parseFloat(z) > 1.125) {
		    ...
		    playerDiv["ball"].src = window.location.toString().split("game")[0]+"images/"+ballMotionImg;
		}
	    }
	    else {
   	        if (playerDiv["ball"].src.indexOf("/ball") == -1) {
		    playerDiv["ball"].src = window.location.toString().split("game")[0]+"images/ball.gif";
		}
	    }
