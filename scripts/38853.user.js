...
function sendtroops() {
	...
	if(pagina.indexOf("There isn't a Village on these coÄ‚Â¶rdinates.") > -1)
	{
...
function voegToe() {
	...
	var conf = confirm("LĂ©giĂłs? (10)");
	if (conf) {
		addList(x+","+y+"|5,0,0,0,0,0,0,0,0,0|"+dorpnaam);
	} else {
		addList(x+","+y+"|0,100,0,0,0,0,0,0,0,0|"+dorpnaam);
	}
	alert('Added: '+dorpnaam+" ("+x+"|"+y+")\nFarm whit 12 " + ((conf) ? "legionari" : "axes"));
};
...
function addForm() {
	...
	addButton.innerHTML = "Â» Add to AutoFarm";
