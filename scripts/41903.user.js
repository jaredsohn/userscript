// ==UserScript==
// @name           	Kiem Tra Dong Gop Tai Nguyen v 2.0.2
// @namespace      http://ongconoi.xm.com/
...
function getLeecherInfo(page)
{
	...
	var info =  "Có " + cities + " thành phố đang hoạt động trên hòn đảo này. Điều đó có nghĩa là tất cả mọi người phải đóng <b>"+ Math.round(10000/cities)/100 +"%</b> số gỗ cần để nâng cấp mỏ<br />"+
		"Cấp độ của mỏ là : " + lvl + " ,cần đóng góp thêm <b>"+addPointsToNumber(woodNext) +" gỗ</b> để nâng cấp mỏ.<br />"+
		"Người chơi phải đóng  <b>" + addPointsToNumber(Math.ceil(woodNext * percentage)) + " gỗ</b>. ("+addPointsToNumber(Math.ceil(wood * percentage))+" cần cho cấp độ của bạn)";
	...
	{		
		...
		div.innerHTML = 
			"<div id='leecher_checker' class='contentBox'>"+
				"<h3 class='header'><span class='textLabel'>Kiểm Tra Đóng Góp Tài Nguyên v2.0.2</span></h3>"+
				"<div class='content'>"+
					"<table>"+
						"<tr>"+
							"<td colspan='3'>"+
								"Kiểm tra sự đóng góp mỏ của người chơi.<br />"+
								"Each player has to donate a % to reach to the next level. This percentage is 100 divided by the number of cities that are donating."+
								"Adjust if if you have inactive players. This method is aimed at improving the deposit. The legenda is as follow: <br />"+
							"</td>"+
						"</tr>"+
						"<tr>"+
						...
						"</tr>"+
						"<tr>"+
							"<td style='color: green;'><center>Bạn đã đóng góp vượt chỉ tiêu mà cấp độ hiện tại của bạn cần</center></td>"+
							"<td style='color: orange;'><center>Bạn đã đóng góp đủ cho cấp độ hiện tại của bạn</center></td>"+
							"<td style='color: red;'><center>Bạn là một kẻ bóc lột công sức người khác!</center></td>"+
						"</tr>"+
						"<tr>"+
							"<td colspan='3'><hr><center>Số người chơi trên đảo<input id='param' type='textfield' class='textfield' name='param' size='3' value='"+getCityNumber()+"' />"+
		...
		doLeecherChecker(page);
				...
				{alert("Leacher Checker đã bị lỗi\nXin vui lòng gửi hình ảnh lỗi này & một ít thông tin khi bạn gặp lỗi về địa chỉ Email : lien.minh.ong.co.noi@gmail.com\n" + er)}
},
    true);