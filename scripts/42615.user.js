...
function overview_table() {
	...
	if (city_idmainView > 0) {
		...
		// 間諜屋
		if (body_id=="safehouse" || /view=safehouse/.test(document.URL)) {
			updateHideout(res);
		}
		// 學院
		...
		// 運輸船畫面
		if (body_id == "merchantNavy" || /view=merchantNavy/.test(document.URL)) {
			if (SETTING_EXPANDED) {
				updateMerchantNavy();
			}
		}
		...
		// 運輸畫面
		if(body_id=="transport" || /view=transport/.test(document.URL)) {
			updateTransport();
		}
	}
	// 島嶼畫面
