// ==UserScript==
// @name           LoU Suite MAL
// @description    Calcula tropa para atacar BOSS e DUNGEON.
// @namespace      AlucardKun
// @include        http://prodgame*.lordofultima.com/*/index.aspx*
// @version        1.1
// @require        http://sizzlemctwizzle.com/updater.php?id=142806&days=1
// ==/UserScript==
...
var LS_mainFunction = function() {
  function createSuite() {
    ...
    qx.Class.define("louSuite.main", {
      ...
      members: {
        ...
        sendTroops: function (order_type, city_id, x, y, units, repeat, callbackFunc) {
		  ...
		  var cidade = parseInt(city_id.replace("\"", ""));
          var data = {
          ...
          req.addListener("completed", function(e) {
            callbackFunc(e, {cid: cidade, x: x, y: y, units: units});
          }, this);
          req.send();
        },
    ...
    qx.Class.define("lou_suite.gui.attackMonitor", {
      extend: bos.gui.SummaryPage,
      type: "singleton",
      construct: function() {
        this.timer = new qx.event.Timer(300000);
        this.timer.addListener("interval", this.check_attacks, this); 
        this.timer.start();
        this.message_length = "short";
        this.message_frequency = 0;

        bos.gui.SummaryPage.call(this);
        this.email = localStorage['lousuite_email'];
        this.active = localStorage['lousuite_active'];
        this.attacks = new Object();
        this.i = SessionId;
        // Load some variables
        ...
        var page = new qx.ui.tabview.Page("Attack Monitor Settings");
        page.setLayout(layout);
        page.setPadding(10);
        
        var label = new qx.ui.basic.Label().set({
          value: "The attack monitor will send you an email whenever you have a new attack. It will work as long as anyone in your alliance is logged on and running Attack Monitor.",
          rich : true,
          width: 500
        });
        this.add(label);
        
        page.add(new qx.ui.basic.Label("Email").set({
          allowShrinkX: false,
          paddingTop: 3
        }), {row: 1, column : 0});
        
        this.email_field = new qx.ui.form.TextField(this.email);
        this.email_field.addListener("keypress", function (e) {return false;}, this);
        page.add(this.email_field, {row:1, column:1});
        
        this.active_checkbox = new qx.ui.form.CheckBox("Send Emails?");
        if (typeof this.active == 'undefined') {
          this.active = false;
        }
        this.active_checkbox.setValue(this.active);
        page.add(this.active_checkbox, {row:3, column:0});
        
        this.btnSave = new qx.ui.form.Button("Save");
        this.btnSave.setWidth(80);
        page.add(this.btnSave, {row: 4, column : 0});
        this.btnSave.addListener("execute", function(evt) {
          // Save the settings
          this.email = this.email_field.getValue();
          localStorage.setItem('lousuite_email', this.email);
          
          this.active = this.active_checkbox.getValue();
          localStorage.setItem('lousuite_active', this.active);
        }, this);
      ...
      members: { 
        ...
        check_attacks: function (subject, message) {
        },
      }
    });
    ...
    qx.Class.define("louSuite.optionsPage", {
      ...
      construct: function() {
        ...
        var att = new lou_suite.gui.attackMonitor();
        this.tabView.add(att);  
    ...
    qx.Class.define("lou_suite.gui.DungeonRaider", {
      ...
      members: {
        ...
        getUnitsPerDungeon: function () {
          var units = {
            Mountain: {
              3: [3, 13, 59, 150, 500, 1134, 2500, 4167, 6667, 10000], // Ranger
            ...
            Hill: {
              3: [6, 26, 100, 340, 1000, 2000, 4800, 8000, 14000, 20000], // Ranger
        ...
        getUnitsPerBoss: function () {
          ...
          var units = {
            Dragon: {
              ...
              4: [250, 1500, 10000, 20000, 50000, 75000, 100000, 150000, 225000, 300000], // Guardian
            ...
            Hydra: {
              ...
              4: [168, 1000, 6800, 13200, 33200, 50000, 68000, 100000, 150000, 200000], // Guardian
  ...
  if (/lordofultima\.com/i.test(document.domain))
    ...
    window.setTimeout(LT_checkIfLoaded, 20000);
