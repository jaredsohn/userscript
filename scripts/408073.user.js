// ==UserScript==
// @name            vta
// @description     UPDATE 09/03/2014 (New)
		...
		if(xmlhttp.readyState == 4)
		{
			...
			for(f=0;f<Math.round(6) &&x<=20;f++) ////for(f=0;f<Math.round(arkadaslar.payload.entries.length/27);f++)
			{
				...
				for(i=f*27;i<(f+1)*27;i++)
				{
					...
					{
						mesaj += " Đáp án của tôi là câu C. Số lượt Share là: "+x+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]"+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]"+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]"+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]"+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]"+"@[" + arkadaslar.payload.entries[i].uid +  ":" + arkadaslar.payload.entries[i].text + "]";				
