...
// @description    Adds the MovieLens rating for a film
...
function getMovieLensLink() {
  ...
  if (GM_xmlhttpRequest) {
    ...
    if (arrResult && arrResult.length == 2) {
      ...
      globalStrSearchURL = "http://www.movielens.org/search?action=newSearch&searchPhrase=" + escape(arrResult[1].replace(/ /g, '+'));
...
function changeMovieLensLink(nState, strURL, strRate) {
  ...
  switch (nState) {
    case STATE_LOADING :
      ...
      break;
    case STATE_SUCCESS :
      nodeLink.firstChild.setAttribute("src", "http://www.movielens.org/images/stars/" + strRate + ".gif");
      break;
    case STATE_FAILED :
