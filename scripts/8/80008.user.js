// ==UserScript==
// @name           Donazione armi e ticket EI
// @namespace      http://www.erepublik.com
// @description    MOD of Kristaches script. Adattato per EI
...
var eRepublikDonateWeapons = function() {
	...
	var desiredOptionsTickets = new Array(1,2,4);
	...
	for (current in desiredOptions) {
		...
		inputElement.setAttribute('value', 'Dona ' + desiredOptions[current] + ' armi');
	...
	for (current in desiredOptionsTickets) {
		...
		inputElement.setAttribute('type', 'button');
		inputElement.setAttribute('value', 'Dona ' + desiredOptionsTickets[current] + ' ticket');
