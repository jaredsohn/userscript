// ==UserScript==
// @name          TribalWars Navigation Toolbar v0.7PL
  ...
  //if (GM_getValue('villageDropdown') == null) {
    ...
    $.get(gameURL,
      ...
      function(data) {
        ...
        /* List of Villages */
        var dStart = '<tr><th>Wioska</th><th>P.</th><th>Surowce</th><th>Spichlerz</th><th>Zagroda</th>';
        var dEnd = '</table>';
        ...
        for (x=0; x < v; x++) {
          ...
          sDropDown += '<option value="'+tmp[1]+'"'+((tmp[1] == vID) ? ' SELECTED' : '')+'>'+tmp[2]+' ('+points+'pkt)</option>';
        }

        pTable = allTables[1].tBodies[0].rows[0].cells[0];
        pTable.innerHTML = sDropDown + '</select> - <a href="'+gameURL+'&screen=overview_villages">Wioski</a> - <a href="'+gameURL+'&screen=map">Mapa</a> - <a href="'+gameURL+'&screen=overview">Przegląd wioski</a>';
      }
    );
  //}
}
...
/* MenuBar Styles */
var newStyles = '';
newStyles += '.menu {font: 12px arial; width:840px; position:relative; margin:0px 0 0px 0; z-index:100;}';
newStyles += '.menu ul li a, .menu ul li a:visited {display:block; text-decoration:none; color:#000;width:95px; height:20px; text-align:center; color:#000; border:2px solid #804000; border-left: 0px; border-right: 0px; background:#f1ebdd; line-height:20px; padding: 4px; overflow:hidden;}';
...
/* Forum */
menuBarTop += '    <li><a class="hide" href="http://forum.plemiona.pl/">Forum</a></li>';

/* Chat */
menuBarTop += '    <li><a class="hide" href="chat.php">Czat</a></li>';
...
menuBarTop += '      	<li><a href="'+gameURL+'&screen=settings&mode=email">Adres E-Mail</a></li>';
menuBarTop += '      	<li><a href="'+gameURL+'&screen=settings&mode=settings">Ustawienia</a></li>';
...
menuBarTop += '      	<li><a href="'+gameURL+'&screen=settings">Pytanie</a></li>';
menuBarTop += '      </ul>';
menuBarTop += '    </li>';
...
menuBarTop += '      	<li><a href="'+gameURL+'&screen=ranking&mode=con_ally">K-Plemiona</a></li>';
menuBarTop += '      	<li><a href="'+gameURL+'&screen=ranking&mode=con_player">K-Gracze</a></li>';
menuBarTop += '      </ul>';
menuBarTop += '    </li>';

var newPost = '';
if (topCell.innerHTML.indexOf('ally_forum.png') > 0) {
  newPost = '<img src="'+baseURL+'/graphic/ally_forum.png" title="Nowy post na prywatnym forum" alt="">&nbsp;';
}
...
menuBarTop += '    	<ul>';
menuBarTop += '      	<li><a href="'+gameURL+'&screen=report&mode=all">Wszytkie</a></li>';
...
menuBarTop += '      	<li><a href="'+gameURL+'&screen=report&mode=forwarded">Przekazane</a></li>';
menuBarTop += '      	<li><a href="'+gameURL+'&screen=report&mode=filter">Filtr</a></li>';
menuBarTop += '      	<li><a href="'+gameURL+'&screen=report&mode=block">Zablokuj</a></li>';
menuBarTop += '      </ul>';
menuBarTop += '    </li>';
...
menuBarTop += '    <li style="border-right:2px solid #804000"><a class="hide" href="'+gameURL+'&screen=mail">'+newMail+'Wiadomości</a>';
menuBarTop += '    	<ul>';
...
menuBarTop += '      	<li><a href="'+gameURL+'&screen=mail&mode=block">Zablokuj</a></li>';
menuBarTop += '      </ul>';
menuBarTop += '    </li>';
...
menuBar += '    	<ul>';
menuBar += '      	<li><a href="'+gameURL+'&screen=market&mode=send">Wyślij surowce</a></li>';
...
if (getParam('screen') == 'overview') {
  ...
  wood.innerHTML = wood.innerHTML + '<br /><strong>' + (woodAmt * 24) + '</strong> dziennie';
  clay.innerHTML = clay.innerHTML + '<br /><strong>' + (clayAmt * 24) + '</strong> dziennie';
  iron.innerHTML = iron.innerHTML + '<br /><strong>' + (ironAmt * 24) + '</strong> dziennie';
