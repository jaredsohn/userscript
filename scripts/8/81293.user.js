// ==UserScript==
// @name           Feuerwache.net AAO DarkPrince (Komplett-Script)
// @namespace      http://userscripts.org/users/119663
// @description    schlaegt Fahrzeuge zu Einsatzstichworten gemäß AAO vor
// @include        http://www.feuerwache.net/*
// @author         DarkPrince
// @info           Überarbeitetes Originalscript von Sawos
// @version        29.05.2010 - 20:30 Uhr - Nachforderungen KW integriert
// ==/UserScript==
...
// Die Variablen UPDATEURL und INSTALLURL müssen angepasst werden, wenn Du das Script anpasst
...
var INSTALLURL="http://userscripts.org/scripts/source/63003.user.js";
  ...
  // ***************************************************************************************
  // 		AB HIER KOPIEREN und in die neue Scriptversion einfügen.
  // ***************************************************************************************
  ...
  // ***************************************************************************************
  // Einsatzklassen, Bei einem neuen Einsatz entsprechend unten stehendem Schema einfügen.
  // ***************************************************************************************
var Einsatzklassen = {
  ...
  // Einsatzklasse zuerst, dann ggf. ein Plus (+) und weiter Fahrzeugklassen, 
  // diese durch Komma (,) voneinander getrennt. Alternativen durch Schrägstrich (/) getrennt.
  ...
  'Brand im Casino'                  : 'FEU_KINO',
  'Brand in Kühlhaus'                : 'FEU_LAGER',
  'Brand in Reifenlager'             : 'FEU_LAGER',
  'Brand im Sägewerk'                : 'FEU_WERKSTATT',
  'Brand im Supermarkt'              : 'FEU_GESCHÄFT',
  'Brand in Autohaus'                : 'FEU_LAGER',
  'Brand in Briefkasten'             : 'FEU_KLEIN',
  'Brand in Druckerei'               : 'FEU_WERKSTATT',
  'Brand in KFZ-Werkstatt'           : 'FEU_WERKSTATT',
  'Brand in Kletterhalle'            : 'FEU_HALLE',
  'Brand in Lackfabrik'              : 'FEU_LAGER',
  'Brand in Schloss'                 : 'FEU_HOCHHAUS',
  'Brand in Schule'                  : 'FEU_SCHULE',
  'Brand in Spedition'               : 'FEU_LAGER',
  'Brand in Sporthalle'              : 'FEU_HALLE',
  'Brand in Zugdepot'                : 'FEU_HALLE',
  ...
  'Brennender Müllwagen'             : 'FEU_LKW',
  'Brennender PKW'                   : 'FEU_PKW',
  'Brennendes Gebüsch'               : 'FEU_KLEIN',
  ...
  // ***************************************************************************************
  // AAO/Einsatzklassen, Bei einem neuen Einsatz entsprechend unten stehendem Schema einfügen.
  // ***************************************************************************************
var Einsatzklasse_Fahrzeugzuordnung = {
  // hier wird definiert, welche Fahrzeuge standardmäßig zu den verschiedenen Einsatzklassen
  // geschickt werden. Einzelne Fahrzeuge werden durch Komma (,) getrennt. Alternativen durch (/).
  ...
  'FEU_LAGER'  :  'ELW,DL,LF,LF,LF/KLF,TLF,GW-M,GW-G,GW-W,GW-A',
  ...
  'ABC_LAND'  :  'ELW,LF,LF,LF/KLF,RW,GW-M,GW-G',
  'GAS'  :  'ELW,LF,LF,LF/KLF,RW,GW-M,GW-G',
  ...
  'P_WASSER' : 'ELW,LF,LF/KLF,GW-T',
  'FEU_KRHS' : 'ELW,DL,LF,LF,LF,LF,RW,GW-W,GW-M,GW-G,,GW-A',
  'HILFELEISTUNG' : 'LF,LF/KLF,RW,GW-T,KW',
  'FEU_ZIMMER'  :  'ELW,DL,LF,LF/KLF',
  ...
  // ***************************************************************************************
  // Fahrzeugklassen, Bei einem neuen Fahrzeug entsprechend unten stehendem Schema einfügen.
  // ***************************************************************************************
var Fahrzeugklassen = {
  // hier die verfügbaren Fahrzeugen mit ihrer Beschreibung und der Zuordnung zu einer Fahrzeugklasse
  // auflisten. In der gleichen Reihenfolge, wie hier die Fahrzeugklassen aufgeführt sind,
  ...
  // *************************************************************************************** 
  // Nachforderungen, bei neuer Nachforderung, nach unten stehenden Schema einfach einfügen.
  // ***************************************************************************************
var Nachforderungen = {
  ...
  //Falsche Nachforderung(derzeit noch aktiv)
  ...
  // ***************************************************************************************
  // 		BIS HIERHER NUR AAO-SCRIPT (IS NUR FÜR MICH; NICHT BEACHTEN^^)
  // ***************************************************************************************


  // ***************************************************************************************
  // Fahrzeuggeschwindigkeiten, Wenn ein neues Fahrzeug hinzukommt, einfach unten einfügen.
  // ***************************************************************************************
...
var Fahrzeuggeschwindigkeiten = {
  ...
  // ***************************************************************************************
  // Links zum Wikipedia, Wenn ein neuer Einsatz kommt, einfach nach Schema den Link zufügen.
  // ***************************************************************************************
var WikiLinks = {
  ...
  // ***************************************************************************************
  // 		BIS HIERHER KOPIEREN und in die neue Scriptversion einfügen.
  // ***************************************************************************************
  ...
  // ***************************************************************************************
  // 	Nach dieser Linie nichts mehr aendern !!! Ab hier ist das Script (Programm)
  // ***************************************************************************************
...
{ 
  ...
  { var TR=TRs[i];
    ...
    var H = "&nbsp;" + H1 + " / " + H2 + "&nbsp;" ;
    if (parseInt(H1) != parseInt(H2)) H = "<font color='lightgreen'>"+H+"</font>";
    ...
    H = TD.innerHTML;
    H2 = H.split("/");
    if (parseInt(H2[0]) != parseInt(H2[1])) H = "<font color='lightgreen'>"+H+"</font>";
    TD.innerHTML = H;
...
{ var DC=document.getElementById("content");
  ...
  T += "<th>FZ-Typ</th>" +
       "<th>Anzahl</th>" +
       "<th style='background-color:#0000F8;'>Frei über Funk<br>(S1)</th>" +
       ...
       "<th style='background-color:#DAD815;color:#000000;'>Patient übernommen<br>(S7)</th>" +
...
{ 
  ...
  if (ToAlarm.length>0) 
  { if (showInfoToAlarm) InfotextToAlarm = "<font color='lightgreen'>" + ToAlarm + "</font>"; }
  else
  { if (showInfoToAlarm)
    {
      InfotextToAlarm = "<font color='lightblue'>Fahrzeuge ausreichend.</font> "; 
...
{ MachSortierbar(myTB);
  ...
  { if (TR.getElementsByTagName("td").length>5)
    { var TDs = TR.getElementsByTagName("td");
      ...
      if (Mot <= 89) TDs[1].style.color = "#32CD32";
      if (Mot <= 75) TDs[1].style.color = "#66FF66";
      if (Mot <= 50) TDs[1].style.color = "#FFD700";
      ...
      if (Stat == "Beim Einsatz") { AnzDienst++; AnzEinsatz++; TDs[5].style.color="#FF6"; }
      if (Stat == "Frei - nicht im Dienst") TDs[5].style.color="#F03";
      if (Stat == "Einsatzbereit") { AnzDienst++; AnzBereit++; TDs[5].style.color="#6F0"; }
      if (Stat == "In der Feuerwehrschule") { AnzSchule++; TDs[5].style.color="#6FF"; }
  ...
  ret = "<b>" + AnzFM + " FM(SB)</b>";
  if (AnzDienst != AnzFM) 
  { ret += ", davon <font color='lightblue'>" + AnzDienst + " im Dienst</font> und <font color='#DAA520'>";
    if (AnzSchule) 
    { ret += AnzSchule + " in der Schule.</font>"; }
    else
    { ret = ret.substr(0,ret.length-5) + "."; }
  }
  ret += "<br><b><font color='lightgreen'>" + AnzBereit + " einsatzbereit</font></b> (=" + parseInt(1000*AnzBereit/AnzDienst)/10 + "%), <font color='#FF6666'>";
  if (AnzEinsatz) ret += AnzEinsatz + " im Einsatz</font> (=" + parseInt(1000*AnzEinsatz/AnzDienst)/10 + "%), ";
  ret = ret.substr(0,ret.length-2) + ".<br>";
  if (AnzGG)
  { ret += "<font color='#DEB887'>Gefahrgut: </font>" + AnzGG + " insgesamt, <font color='lightblue'>"+ AnzGGDienst + " im Dienst</font>";
  ...
  if (AnzRA)
  { ret += "<font color='#FF8C00'>RA: </font>" + AnzRA + " insgesamt, <font color='lightblue'>" + AnzRADienst + " im Dienst</font>";
  ...
  if (AnzTA)
  { ret += "<font color='#1E90FF'>Taucher: </font>" + AnzTA + " insgesamt, <font color='lightblue'>" + AnzTADienst + " im Dienst</font>";
