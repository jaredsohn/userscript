// ==UserScript==
// @name                FTD NZB download link
// @description         Adds a NZB download link to the web based FTD
// @include             *ftd.nu/*
// @version             0.12
// ==/UserScript==
...
function setNZBButton()
{
  if(header = document.getElementsByClassName('pageheader')[0])
  {
    header.childNodes[0].style.cssFloat = 'left';

    var img = document.createElement('img');
    img.src = "http://matis-digital.com/greasemonkey/nzb.png";
    img.style.border = '0px solid black';
    img.style.margin = '0px';
    img.style.padding = '0px';
    
    var nzbButton = document.createElement('a');
    nzbButton.id = "nzbbutton";
    nzbButton.style.cssFloat = 'right';
    nzbButton.style.margin = '0px';
    nzbButton.style.padding = '0px';
    nzbButton.style.height = '51px';
    
    nzbButton.setAttribute('href', nzbindexQuery());
    nzbButton.setAttribute('target', '_blank');
    nzbButton.appendChild(img);
    
    header.appendChild(nzbButton);
  }
  return null;
}
...
if(searchQuery != null)
{
    var downloadHTML = 'Bestandsnaam: ' + searchQuery + ' <br />';
    downloadHTML += 'Select: <input type="text" id="searchQuery" onClick="this.select()" value="' + searchQuery + '"><br /> Download van: ';
    // nzbindex.nl
