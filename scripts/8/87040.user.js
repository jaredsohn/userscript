...
// @description		An admin tool for the Flickr pre*Game group - V0.9
// @version		0.9
// @identifier	
// @date		
// @author		Kris Vangeel (http://www.flickr.com/people/kvgl69/)
// @copyright		2008-2011 Kris Vangeel
// @license		GPL v3+
// @licstart		The following is the entire license notice for this script.
/*
 * Copyright (C) 2008-2011 Kris Vangeel
 ...
 */
// @licend
// @contributor		Criz Stoddard (http://www.flickr.com/people/zyrc/)
// @modified by		RKho (pipeguru) for Ease Game group
// @contributor     Alesa Dam (http://flickr.com/alesadam): v0.4
// @contributor     Martin Heimburger (http://flickr.com/vispillo): admin invite, voting check, referrer
...
// v0.2: add PG-Init button to initialise PreGame Admin data
...
// v0.5: remove the 'bump' functionality as per the new Flickr behavior for 'invisibumps'
// v0.6: UCP bump messages should not be reported as votes
// v0.7: remove eventHandler conflict with FlickrNotifications
// v0.8: add version check
// v0.9: bug fix: 'theme in play' should consider more than just the first word!
//
// ==/UserScript==
// --------------------------------------------------------------------
//
...
(function PreGameadmintool( ) 
{
	...
	var PreGametoolversion = "V0.9";
	var scriptNumber = '87040';
...
function showUpdateNotification() {
    ...
    var updateLink = document.createElement('a');
        updateLink.innerHTML = 'pre*Game Admin Tool: update available';
...
function checkVersion() {
    ...
    if (onlineVersion) {
        ...
        var currentVersionParts = reVersionMatch.exec(PreGametoolversion);
...
// script code
    ...
    var nHeaders = parseInt(GM_getValue("PreGame.headers"));
    if (isNaN(nHeaders)) {
        alert("You will need to (re-)initialize the PreGame Admin tool data");
    }
...
*/
                ...
                for (var i = 1; i <= nMedals; ++i) {
                    ...
                    medalOption.value = i;
                    medalOption.innerHTML = GM_getValue("PreGame.medal." + i + ".title");
		...
		{
		  chlg_id = GM_getValue('PreGame.currentchallenge');
		  chlg_thread = 'http://www.flickr.com/groups/pregame/discuss/'+GM_getValue('PreGame.'+chlg_id+'.threadnr')+'/';
		  chlg_theme = decodeURIComponent(GM_getValue('PreGame.'+chlg_id+'.status')).split('Theme: ')[1];
		  string = '';
		  // Check if we came here from the discussion page of the 'current' challenge
		  if (document.referrer.split('/')[6] == GM_getValue('PreGame.'+chlg_id+'.threadnr')) {
  		  string = '<br />Congratulations, you won the <a href="'+chlg_thread+'">"'+chlg_theme+'"</a> challenge on <a href="http://www.flickr.com/groups/pregame/">pre*Game</a>!<br />';
  		}
			this.textarea.value = GM_getValue('PreGame.medal.'+node.value+'.medal') + string + this.textarea.value;			
		...
		clickpostcomment: function Awardclickpostcomment( )
		{
			//alert( 'in postcomment' );       
	            try {
		    	var postedMedal = GM_getValue( "PreGame.postedmedal" );
			var tagtext = GM_getValue( "PreGame.medal." + postedMedal + ".tags");
			if ( tagtext != "" ) 
			{
			    // Check if we came here from the discussion page of the 'current' challenge
			    chlg_id = GM_getValue('PreGame.currentchallenge');
			    if (document.referrer.split('/')[6] == GM_getValue('PreGame.'+chlg_id+'.threadnr')) {
  			        tagtext = tagtext + ' "'+ decodeURIComponent(GM_getValue('PreGame.'+chlg_id+'.status')).split('Theme: ')[1]+'"';
			    }
			    ...
			    try {
			    	...
			    	GM_log("unable to add tag; trying admin invite");
			    }
	 	    
		    	    var groupId = GM_getValue("PreGame.medal." + postedMedal + ".groupId");
			    groupId.split('\n').forEach( function (group) {
			    	if (group && group.length > 0 && group.match(/\d+@\w\d{2}/)) {
				    sendAdminInvite(group,photoid);
				}
	                    });
			}
	            } catch (e) {
	...
	function loadthemelist( )
	{
        ...
        var url = "http://www.flickr.com/groups/1343581@N20/discuss/72157624939924479/";
	...
	function loadchlgheader( )
	{
        ...
        var url = "http://www.flickr.com/groups/1343581@N20/discuss/72157624982220239/";
          ...
          try {
            ...
            for (var i = 0, len = replies.snapshotLength; i < len; ++i) {
                ...
                try {
                    ...
                    GM_setValue( "PreGame.header." + (nHeaders+1) + ".title", title);
                    GM_setValue( "PreGame.header." + (nHeaders+1) + ".text", header); // 1-based
                    if (reply.innerHTML.match("===Start of Theme===")) {
                        var theme = reply.innerHTML.split("===Start of Theme===")[1]
                                                   .split("===End of Theme===")[0]
                                                   .replace(/<[^>]+>/g, '');
                        GM_setValue( "PreGame.header." + (nHeaders+1) + ".theme", theme);
                    } else { // in case there was one previously
                        GM_deleteValue("PreGame.header." + (nHeaders+1) + ".theme");
                    }
            ...
            GM_setValue( "PreGame.headers", nHeaders );
	...
	function loadmedals( )
	{
		...
		GM_log( "PreGame: Started loading new medals (in background)" );

        var url = "http://www.flickr.com/groups/1343581@N20/discuss/72157625196869928/";
		...
		onload:function( responseDetails ) 
		{
          try {
		    ...
		    var groupId = reply.innerHTML.split("===Start of GroupID===")[1]
						...
						.replace(/<br>/g, '\n');
                    GM_setValue( "PreGame.medal." + (nMedals+1) + ".title", title);
                    GM_setValue( "PreGame.medal." + (nMedals+1) + ".medal", medal); // 1-based
		    GM_setValue( "PreGame.medal." + (nMedals+1) + ".tags", tags);
		    GM_setValue( "PreGame.medal." + (nMedals+1) + ".groupId", groupId);
                    ++nMedals; // in case there is a reply that throws an error
                } catch (e) {
                    GM_log("error reading medal entry - skipping (" + e + ")");
                }
            }
            GM_setValue( "PreGame.medals", nMedals );

			GM_log( "PreGame: Loading new medals complete" );
          } catch (e) {
            GM_log("error loading headers: " + e);
          }
		}} ); // end GM_xmlhttpRequest
  ...
  function loadvotetimes ( ) {
    ...
    // Extra check, in case this function is called on an individual thread page.
    if (trs) {
...
//rxk remove ddphoto3 orig 	ddphoto2 + " - " + ddphoto3 + "\n";
	...
	updatealldata = function updatealldata() 
	{
		...
		maxupdates = parseInt(GM_getValue( "PreGame.maxupdate" ));
		maxupdates++;
		//GM_log( maxupdates );
...
GM_log( "Clearing last winner: " + decodeURIComponent( GM_getValue( "PreGame.lastwinner" ) ) );
	...
	addPreGameheader = function addPreGameheader( )
	{
		...
		header.innerHTML = "pre*Game Admin " + PreGametoolversion + "&nbsp;-&nbsp;";
...
//rxk modified to fix UCI conflict
        ...
        GM_setValue( "PreGame.DailyChatUrl", url );
			...
			if ( mnchlgnr.length == 2 ) //only process if chlgnr is correct format
			{
				...
				else if ( ( ddlastvote.match( sPreGameVoteScore ) == sPreGameVoteScore ) && ( mncommentcounter == ddcommentcounter ) && ( ddphoto2 != " " ) ) chlgstatus = "FIN";
...
// rxk change ddphoto3 to ddphoto2 for 2 photos challenge
		...
		//add initialise PreGame Admin Data
...
GM_log( "Setting last winner: " + decodeURIComponent( GM_getValue( "PreGame.lastwinner" ) ) );
	...
	insertChallengetxt = function insertChallengetxt( )
	{
        ...
        var header = GM_getValue("PreGame.header." + t.value + ".text");
        this.textarea.innerHTML = "";
        if (header) {
            this.textarea.innerHTML = header;
        }
        var fixedTheme = GM_getValue("PreGame.header." + t.value + ".theme");
        if (fixedTheme) {
...
*/
        ...
        for (var i = 1; i <= nHeaders; ++i) { // 1-based
            ...
            var title = GM_getValue("PreGame.header." + i + ".title");
...
*/		
  ...
  function getJSVariable (regex) {
	...
	runPreGameTool = function runPreGameTool( )
	{
		...
		if ( GM_getValue( "PreGame.version" ) != PreGametoolversion )
			initdata( );
