// ==UserScript==
// @name           Favourite Spy Reports DK
// @namespace      ogc20-modules/
// @description    Gem dine Spionage Rapporter 
// @include        http://*.ogame.dk/game/index.php?page=messages&session=*
// @version 1.1
// @copyright 2010, trijin (Oversat til dansk) 
// @autor          Jan Kjærsgaard
// ==/UserScript==
var Utils = {
...
var Favors = {
	createButtons: function(e) {
		...
		var a = unsafeWindow.$('<a>').attr({
			...
			'id': 'ogc20ShowFavor',
			'title': 'Gemte Spionage rapporter',
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Rapporten er allerede gemt');
				return false;
			}
	...
	Fsave: function(obj) {
		try {
			...
			unsafeWindow.fadeBox('Ændringer gemt');
		} catch (e) {}
	},
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">Afsender</th><th class="subject">Angående</th><th class="date">Dato</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Flådekommandant</td><td class="subject">' +
				...
				'<a title="Slet denne meddelelse" class="ogc20Fdel tips" rel="'+el.id+
			...
			unsafeWindow.fadeBox('Ingen gemte rapporter');
		}
		return false;
	},
