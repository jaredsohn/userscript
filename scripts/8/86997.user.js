// ==UserScript==
// @name             Chrome RSS Icon
// @namespace    http://browservulsel.blogspot.com/
// @version          0.2
// @description    Based on Jasper's Google Reader subscribe - Another Google Reader subscriber. This one was designed to use as little screen size as possible. It only adds a feed icon in the top right corner. On mouse over the subscribe links are shown.
// @include          http://*/*
// @include          https://*/*
// @exclude         http://www.google.com/reader/*
// @exclude         https://www.google.com/reader/*
// @exclude         https://mail.google.com/mail/*

// ==/UserScript==

var result = document.evaluate('//link[@rel="alternate"][contains(@type, "rss") or contains(@type, "atom") or contains(@type, "rdf")]', document, null, 0, null);
const logoRssOrange = 'data:image/png;base64,';

var item;
var feeds = [];
while (item = result.iterateNext()) feeds.push(item);
if (feeds.length > 0) {

    var JGRSmain = document.createElement('div');
    JGRSmain.setAttribute('id', 'JGRSmain');
    document.body.appendChild(JGRSmain);
    JGRSmain.setAttribute( 'style', 'position: fixed; ' +
        'z-index: 32767; ' +
        'top: 0; ' +
        'right: 0; ' +
        'margin: 3px; ' +
        'height: 20px; ' +
        'width: 20px; ' +
        'background:url('+logoRssOrange+') no-repeat;' );
         
    var JGRSfeeds = document.createElement('div');
    JGRSmain.appendChild(JGRSfeeds);

    for (var i = 0, feed; feed = feeds[i]; i++) {
        JGRSfeeds.innerHTML += '<a style=\"display: block; text-decoration: none;\" href=https://www.google.com/reader/view/feed/'+ encodeURIComponent(feed.href) +' title=\"'+ feed.title +'\">&nbsp;</a>';
    }
}