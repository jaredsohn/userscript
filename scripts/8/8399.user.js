// ==UserScript==
// @name           Google Reader + Simpy
// @namespace      http://google.com/reader/simpy
// @description    Lets you bookmark items from Google Reader to Simpy
...
var DEFAULT_LABEL="Google Reader";
var NORMALIZE=true;
...
var mode;
var visibility=1; //public=1, private=0
							...
							var divedit=document.createElement("div"); divedit.className="tags-edit";
								...
								var editcontents=document.createElement("div"); 
									...
									var lbldesc=document.createElement("div"); 
										...
										lbldesc.innerHTML="Labels: (Separate with commas)";
...
function saveBookmark(event){
	...
	var sendData="";
	var tagsData=labels;
	if (tagsData.indexOf(" ")>0 && !tagsData.indexOf(",")>=0)
		tagsData+=",";
	tagsData=encodeURIComponent(tagsData);
	tagsData=tagsData.replace(/%2C/,",");
	tagsData=tagsData.replace(/%20/,"+");
	sendData="title="+encodeURIComponent(title)+
    		"&href="+encodeURIComponent(url)+
    		"&accessType="+visibility+
   			"&tags="+tagsData+
   			"&urlNickname="+encodeURIComponent(title)+
   			"&note="+encodeURIComponent(notes);
	GM_xmlhttpRequest({
    		method: 'POST',
    		url: "http://www.simpy.com/simpy/api/rest/SaveLink.do",
			headers: {'User-Agent': 'Mozilla/4.0 (compatible) Google Reader + Simpy Userscript',
