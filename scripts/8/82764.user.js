...
// @name           eRepublik WE bebe II Military Tracker by Denis Cicic
// @namespace      http://www.erepublik.com/en/citizen/profile/1709354
// @description    eRepublik WE Military Tracker.
// @version        1.0.2
// @include        http://www.erepublik.com/en
// @include        http://www.erepublik.com/sr
// @require        http://sizzlemctwizzle.com/updater.php?id=82764&days=1&show
...
function getVersion() {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://userscripts.org/scripts/show/82764',
		headers: {
			'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey/0.3',
			'Accept': 'application/atom+xml,application/xml,text/xml',
		},
		onload: function(responseDetails) {
			var currVersion = VERSION.replace('.', '');
			var newVersion = $(responseDetails.responseText).find('#summary').html().split('<b>Version:</b>')[1].trim().split('\n')[0];
			var nVersion = newVersion.replace('.', '');
			if(nVersion > currVersion) {
				$('.eCM_version').html('<a title="Update available! New version: v ' + newVersion + '" href="http://userscripts.org/scripts/show/82764" target="_blank" style="color: #ff0000; font-weight: bold;">v ' + VERSION + '</a>');
			}
		}
	});
}
...
function preFetchMembers() {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://whiteeagles.freehostia.com/members/webebeIImember',
		onload:function(response) {
...
function Main(e) {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://whiteeagles.freehostia.com/members/webebeIIusers',
		onload:function(response) {
			var statusData = "off";
