...
javascript:(function(){
    ...
    var table_html='<form id="something">'+
            ...
            '<tr>'+
            '<th width="50%" style="font-size:20px; padding-left:15px;text-align: left">Brawler v2.9</th>'+
            ...
            '<td width="10%">Cash</td>'+
            '<td width="1%">:</td>'+
            '<td width="67%"><span id="cash"><span class="good"><img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_16x16_01.gif"></img> $0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cubanpeso_16x11_01.gif"></img>&nbsp;C$0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_moscow_16x16_01.gif"></img>&nbsp;R$0&nbsp;<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_cash_bangkok_16x16_01.gif"></img>&nbsp;B$0</span></span></td>'+
            '<td width="1%">&nbsp;</td>'+
            '<td width="8%">Delay</td>'+
            '<td width="1%">:</td>'+
            ...
            '<td width="1%">:</td>'+
            '<td width="25%"><input type="radio" id="Fightlist" name="fight_choice">Fightlist</input></td>'+
            '<td width="25%"><input type="radio" id="Users" name="fight_choice" checked>Specific Users</input></td>'+
...
/*			'<tr id="FightChoice" style="display:none">'+
			'<td width="100%">'+
...
// end FightChoice here
            ...
            '<td width="69%" id="stash_posts_log" valign="top" style="display:none">Stashes<br></td>'+
            '</tr>'+
    ...
    try{
	   document.getElementById('popup_fodder').removeChild(document.getElementById('fight_attack_div'));
    }
    catch(err){}
...
/**/
    ...
    document.getElementById("heal_city_change").onclick=function(){
        heal_city++;
        heal_city=(heal_city>4)?0:heal_city;
...
//                alert(fightlist_faction[fightlist_faction.length - 1]);
        ...
        if(fightlist_match >= 3){
            logmsg('No matches on the fightlist with set criteria, trying again in 30 seconds','status');
            fightlist_match=0;
            setTimeout(function(){attack_or_not=true;loadfightpage();},30000);
            return;
        }
...
//            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=attack&xw_city=1&tmp='+tmp_key+'&cb='+cb+'&opponent_id='+user_list[m]+'&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
            ...
            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=power_attack&xw_city=1&cb='+cb+'&opponent_id='+user_list[m]+'&origin=profile&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
...
//        alert(user_urls);
    ...
    function start_attack(){
        ...
        if((exp_to_levelup <= 30) && levelup_halt){
...
//            },30000);
            ...
            var link="http://facebook.mafiawars.com/mwfb/"+fightlist_attack[i]+"&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig;
        }
        catch(err){alert(err);}
  
    
        req = $.ajax({type: "GET",url: link, timeout: 30000,
            success: function (msg){
...
//    catch(err){alert(err);}
    ...
    function process_user_attack(attack_results){
        ...
        if(!((/You win/.test(attack_results)) || (/You lose/.test(attack_results)))){
            ...
            var profile_link = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&xw_city=1&cb='+userid+UnixTS()+'&user='+user_list[j]+'&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
            
            $.ajax({type: "GET",url: profile_link,
              success: function (msg){
...
//        health_lost = /fightres_health">took ([0-9]+)/.exec(attack_results)[1];
        document.getElementById('user_health').innerHTML = /user_fields\['user_health'\] = "([0-9]+)/.exec(attack_results)[1];
        document.getElementById('user_stamina').innerHTML = /user_fields\['user_stamina'\] = "([0-9]+)/.exec(attack_results)[1];
        document.getElementById('user_experience').innerHTML = /user_fields\['user_experience'\] = "([0-9]+)/.exec(attack_results)[1];

        exp_to_levelup = parseInt(/user_fields\['exp_for_next_level'\] = "([0-9]+)/.exec(attack_results)[1]) - parseInt(document.getElementById('user_experience').innerHTML);
        ...
        mafia_attack = parseInt((/fightbar_group_stat">.+?Mafia Attack Strength.+?> (.+?)<\/div>/.exec(attack_results)[1]).replace(/,/g,''));
        if(initial_mafia_attack == 0){
            initial_mafia_attack = mafia_attack;
        }
        mafia_defense = parseInt((/fightbar_group_stat">.+?Mafia Defense Strength.+?> (.+?)<\/div>/.exec(attack_results)[1]).replace(/,/g,''));
        ...
        try{
            ...
            if(/(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/.test(attack_results)){
                temp_loot = attack_results.match(/(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/g);
                for(z=0;z<temp_loot.length;z++){
                    temp = /(found|gained|earned) (some|an|a|A) (.+?)(See|while|\.)/.exec(temp_loot[z])[3];
                    temp = temp.replace(/!/g,'');
        ...
        try{
            if(/found the location of a secret stash/.test(attack_results)){
...
//            }
//            else{
            exp_gained += parseInt(/fightres_experience good">\n(.*)\+(\d*) Experience/.exec(attack_results)[2]);
...
//            alert(/fightres_cash sexy_cash good">\n(.*)\+( *)([^<]+)/.exec(attack_results));
            cash = /fightres_cash sexy_cash good">\n([^<]+)/.exec(attack_results)[1];
            ...
            if(temp != ''){
                logtext += ' <img src="http://arun.keen-computing.co.uk/loot.png" alt="Loot"></img> '+temp;
            }
...
//            return;
            switch(true){
                case (/fightres_cash sexy_cash good">\n(.*)\+(.*)C\$/.test(attack_results)):
                    process_cash(2,(/fightres_cash sexy_cash good">\n(.*)\+(.*)C\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                break;
                case (/fightres_cash sexy_cash good">\n(.*)\+(.*)R\$/.test(attack_results)):
                    process_cash(3,(/fightres_cash sexy_cash good">\n(.*)\+(.*)R\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));                
                break;
                case (/fightres_cash sexy_cash good">\n(.*)\+(.*)B\$/.test(attack_results)):
                    process_cash(4,(/fightres_cash sexy_cash good">\n(.*)\+(.*)B\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                break;
                case (/fightres_cash sexy_cash good">\n(.*)\+(.*)\$/.test(attack_results)):
                    process_cash(1,(/fightres_cash sexy_cash good">\n(.*)\+(.*)\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                break;
            }
...
//                document.getElementById('logger').innerHTML = 'You Lost ! Cash Lost - '+cash; 
                cash = /fightres_cash sexy_cash bad">([^<]+)/.exec(attack_results)[1];
                switch(true){
                    case (/fightres_cash sexy_cash bad">\n(.*)-(.*)C\$/.test(attack_results)):
                        process_cash(2,-(/fightres_cash sexy_cash bad">\n(.*)-(.*)C\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                    break;
                    case (/fightres_cash sexy_cash bad">\n(.*)-(.*)R\$/.test(attack_results)):
                        process_cash(3,-(/fightres_cash sexy_cash bad">\n(.*)-(.*)R\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));                
                    break;
                    case (/fightres_cash sexy_cash bad">\n(.*)-(.*)B\$/.test(attack_results)):
                        process_cash(4,-(/fightres_cash sexy_cash bad">\n(.*)-(.*)B\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                    break;
                    case (/fightres_cash sexy_cash bad">\n(.*)-(.*)\$/.test(attack_results)):
                        process_cash(1,-(/fightres_cash sexy_cash bad">\n(.*)-(.*)\$([^<]+)/.exec(attack_results)[3].replace(/,/g,'')));
                    break;
                }
                if(/Attack Again!/.test(attack_results)){
                    logtext = '<img src="http://arun.keen-computing.co.uk/attack.png" alt="alive"></img> ';
...
//        health_lost = /fightres_health">took ([0-9]+)/.exec(attack_results)[1];
        ...
        try{
            if(/secret stash/.test(attack_results)){
                ...
                stash_posts[stash_posts.length] = stash_script;
                document.getElementById('stash_posts_log').innerHTML += '<a href="#" id="stash_post'+(stash_posts.length-1)+'" onclick="return false;">Stash #'+stash_posts.length+'</a><br>';
...
//            document.getElementById('ice_post'+(iced_posts.length-1)).onclick = new Function(this.id,publish_ice);
        ...
        if(check_city != fight_city){
            ...
            $.ajax({type: "GET",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+check_city+"&cb="+cb+"&destination="+fight_city+"&from=fight&nextParams=&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
                success: function (msg){
                    attack_or_not=true;
                    loadfightpage();
                    return;
...
//            alert(/fightres_cash sexy_cash good">\n(.*)\+( *)([^<]+)/.exec(attack_results));
            ...
            if(loot_log.length >= 1){
                logtext += ' <img src="http://arun.keen-computing.co.uk/loot.png" alt="Loot"></img> '+loot_log;
            }
            ...
            if(parseInt(/fightres_cash sexy_cash good">\n.*\+.*(C|B|R|)\$([^<]+)/.exec(attack_results)[2].replace(/,/g,'')) < parseInt(cash_limit)){
            ...
            if(/You lose!/.test(attack_results)){
                ...
                document.getElementById('exp_ratio').innerHTML = exp_ratio.toFixed(2);
//                document.getElementById('logger').innerHTML = 'You Lost ! Cash Lost - '+cash; 
                cash = /fightres_cash sexy_cash (bad|good)">([^<]+)/.exec(attack_results)[2];
//                document.getElementById('popup_fodder').innerHTML = attack_results;
//                return;
                try{
                    switch(true){
                        case (/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)C\$/.test(attack_results)):
                            process_cash(2,-(/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)C\$([^<]+)/.exec(attack_results)[4].replace(/,/g,'')));
                        break;
                        case (/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)R\$/.test(attack_results)):
                            process_cash(3,-(/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)R\$([^<]+)/.exec(attack_results)[4].replace(/,/g,'')));                
                        break;
                        case (/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)B\$/.test(attack_results)):
                            process_cash(4,-(/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)B\$([^<]+)/.exec(attack_results)[4].replace(/,/g,'')));
                        break;
                        case (/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)\$/.test(attack_results)):
                            process_cash(1,-(/fightres_cash sexy_cash (bad|good)">\n(.*)-(.*)\$([^<]+)/.exec(attack_results)[4].replace(/,/g,'')));
    ...
    function pre_check(){
        ...
        if((parseInt(exp_to_levelup) <= 30) && levelup_halt){
            logmsg('Could level up on next Attack, Stopping..','status');
            document.getElementById('pause').style.display = 'none';
            document.getElementById('begin').style.display = 'inline';
            return;
        ...
        if(document.getElementById('ice_check').checked){
            ...
            var hitlist_url = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hitlist&xw_action=set&xw_city='+fight_city+'&target_pid='+target_id+'&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
            $.ajax({type: "GET",url: hitlist_url, timeout: 30000,
                success: function (msg){
    ...
    function Add_to_loot(loot,count){
        total_loot+=count;
        if(loot_item.length <= 0){
                loot_item[loot_item.length]=new Array(loot,count);
            }
        else{
            for(k=0; k<loot_item.length; k++){
                if(loot.indexOf(loot_item[k][0]) != -1){
                    loot_item[k][1]+=count;
                    break;
                }
        ...
        try{
            for(l=0; l<loot_item.length; l++){
                document.getElementById('loot_log').innerHTML += '<span class="good">'+loot_item[l][1]+'x</span> '+loot_item[l][0]+'<br>';
            }
        }
        catch(err){}
...
//        document.getElementById('popup_permanence').innerHTML += 'Heal travel value - '+heal_travel+'<br>';
        ...
        if(fight_city==heal_city){
            ...
            $.ajax({type: "GET",dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Resuming attacks..','attack_log');
                    }
                    else{
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    }
            ...
            $.ajax({type: "GET",dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
                success: function (msg){
                    if(msg.hospital_message == null){
                        ...
                        process_cash(heal_city,-(/for (C|B|R|)\$([^.]+)/.exec(msg.hospital_message)[2].replace(/,/g,'')));
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','status');
                        logmsg('<img src="http://arun.keen-computing.co.uk/heal.png" alt="heal"></img> Healed <span class="bad">'+(/for (C|B|R|)\$([^.]+)/.exec(msg.hospital_message)[0])+'</span>, Travelling back..','attack_log');
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    }
                    else{
                        document.getElementById('user_health').innerHTML = msg.user_fields.user_health;
                    ...
                    $.ajax({type: "GET",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+heal_city+"&cb="+cb+"&destination="+fight_city+"&from=fight&nextParams=&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
                    success: function (msg){
            ...
            $.ajax({type: "GET",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+fight_city+"&cb="+cb+"&destination="+heal_city+"&from=fight&nextParams=&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
            success: function (msg){
                heal_travel=true;
                logmsg('Healing...','status');
                $.ajax({type: "GET",dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id="+userid+"&sf_xw_sig="+local_xw_sig,
