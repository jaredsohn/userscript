...
function my_addEventListener(el, evt, func) {
...
function insertPhotoEmbed()
{
	...
	var imageElement = document.getElementsByClassName('photo-div');
    if (!imageElement || !imageElement[0]) {
        return;
    }
    
    imageElement = imageElement[0].getElementsByTagName('img')[0];	
	...
	*************************************************************************/
	...
	var destination = document.getElementById('photo-sidebar-tags');
    if (!destination) {
        destination = document.getElementsByClassName('photo-people');
    }
    ...
    // get the 500px width version
    var imageLink = document.getElementById('share-menu-options-embed-textarea');

	// this is the html that gets added to make the text box
    var htmlEmbed = document.createElement('div');
    destination.parentNode.insertBefore(htmlEmbed, destination);
    var title = document.createElement('h4');
    title.innerHTML = 'Embeddable Image HTML';
    htmlEmbed.appendChild(title);
    var contentP = document.createElement('p');
        var textArea = document.createElement('textarea');
        textArea.name = 'textfield';
        my_addEventListener(textArea, "focus", function (e) {
            this.select();
        });
        textArea.rows = 5;
        textArea.style.width = textBoxWidth + 'px';
        textArea.wrap = "virtual";
	    textArea.innerHTML = '<center>' + imageLink.innerHTML + '</center>'; // .value keeps &lt; as such		
        contentP.appendChild(textArea);
    htmlEmbed.appendChild(contentP);
    ...
    contentP = document.createElement('p');
        ...
        textArea.style.width = textBoxWidth + 'px';
        textArea.wrap = "virtual";
        var photoPage = document.location.href.match(/.*flickr\.com\/photos\/[^\/]*\/\d+/)[0];
        var photoSrc = imageLink.innerHTML.match(/.*src=\"([^\"]*)\"/)[1];
        textArea.value = "[url=" + photoPage + "][img]" + photoSrc + "[/img][/url]";
        contentP.appendChild(textArea);
    htmlEmbed.appendChild(contentP);
}
