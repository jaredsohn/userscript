...
// @author          ollily2907
// @version         0.90.12
// @date            $LastChangedDate: 2012-07-25 11:48:01 +0200 (Mi, 25. Jul 2012) $
// @revision        $LastChangedRevision: 34 $
...
/*
 * Sample:
 * (ensure to escape a backslash if this is required, "\" -> "\\"
 * addSite(<server-pattern>, <image-pattern>, <search-pattern>, <replace-pattern-preview>, <replace-pattern-large>, <download-link> );
 *
 */
addSite(".+\\.xing\.com", "\/(?:pub|)img\/users\/.+(?:\_s[0-9]{1,}.*?|\,[0-9]{1,}.[0-9]{1,3}x[0-9]{1,3})\.(?:jpg|gif|png)", "(\\_s[0-9]{1,}.+?|\,[0-9]{1,}.[0-9]{1,3}x[0-9]{1,3})\\.", ".", ".", 0 );
addSite(".+\.net", "\/img\/users\/.+(?:\_s[0-9].*?|\,[0-9].[0-9]+?x[0-9]+?)\.(?:jpg|gif|png)", "(\\_s[0-9].+?|\,[0-9].[0-9]+?x[0-9]+?)\\.", ".", ".", 0 );
...
/**
 * Sets the elements for the search on a site, if it's known.
 *
 ...
 *
 * @param newImage - the image to inspect
 ...
 *
 * @param hDiv - the div-element
 * @param iPoint - metrics of the image [width, height]
 * @param offsetW - an offset for the width (optional)
 * @param offsetH - an offset for the height (optional)
