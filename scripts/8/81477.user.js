// ==UserScript==
// @name         (NMR111) Travian: Attack Builder (14) bu abu rheel
...
switch(sLang)
{
	...
	case "lt":
		text = ["Ataka vykdoma","Klaidingas atakos tipas","Nepasirinktos koordinates","Nepakanka kariu (Kariu skaicius","Nepasirinkti kariai","Pradedama","Klaidingos koordinates","Truksta žvalgybiniu kariu","Ivykdyta","Bangu nustatymai:","Prideti nauja banga","Pradiniai nustatymai","Ataku nustatymai:","Atakos tipas:","Ataka","Pastiprinimas","Reidas","Resursu bei pajegu žvalgyba","Gynybiniu fortifikaciju bei pajegu žvalgyba","Koordinates","Ataka","Atvykimas","Atakos laikas:","Nustatyti atakos laika","ataku skaicius","atskirti su \'|\' pvz.: 0,0|1,1","KLAIDA:","Neimanoma gauti gyvenvieciu koordinaciu. Tikriausiai yra tik viena gyvenviete.","Kariu atvykimo laikas nustatytas:","Vykdoma","Time between waves in ms: (default 500) "];

		cataText = ["Nustatyti taikini","Atsitiktinai","Medžiu kirtaviete","Molio karjeras","Geležies kasykla","Grudu ferma","Lentpjuve","Plytine","Liejykla","Malunas","Kepykla","Sandelis","Kletis","Ginklu kalve","Šarvu kalve","Arena","Gyvenamasis pastatas","Susiburimo vieta","Turgaviete","Ambasada","Kareivines","Arklide","Dirbtuves","Akademija","Rotuše","Rezidencija","Valdovo rumai","Iždine","Prekybos rumai","Didžiosios kareivines","Didžioji arklide","Karžygio namai","Didysis sandelis","Didžioji kletis","Pasaulio stebuklas","Treasury"];
		break;

	case "tr"://by kustah
		text = ["vur tekmeyi =)","yanlis atak tipi!","cordinat yok!","yeterli asker yok! (askerler #","asker seçilmedi","basliyor","yanlis kordinat","gözcü yok","Bitti","Dalga ayari:","Yeni dalga ekle","Reset","Atak ayari:","Atak tipi:","Normal","Destek","Yagmalama","Hammade gözle","Defans gözle","Kordinat","Vur tekmeyi! =)","Ulasim zamani","Varis saati:","Ulasim zamani ayari","Dalgadaki saldiri sayisi","with '|' ex: 0,0|1,1","Hata:","Hesap kapali","Ulasim zamani :","Time between waves in ms: (default 500) "];

		cataText = ["Select a target =)","Random","Orman","Tugla ocagi","Demir madeni","Tarla","Marangozhane","Tugla Firini","Demir dökümhanesi","Degirmen","Ekmek firini","Hammadde deposu","Tahil ambari","Silah dökümhanesi","Zirh dökümhanesi","Turnuva alani","Merkez binasi","Askeri üs","Pazar yeri","Elçilik","Kisla","Ahir","Tamirhane","Akademi","Belediye","Kösk","Saray","Ticaret merkezi","Büyük kisla","Büyük ahir","Kahraman kislasi","Büyük hammadde deposu","Büyük Tahil ambari","Dünya harikasi","Treasury"];
		break;
	...
	case "bg": //?? IYI-Aryan
		text = ["????, ????? ????? =)","????????? ??? ?????!","?? ??? ?????? ??????????!","???????????? ??????! (????? #","?? ??? ?????? ??????","?????????","???????? ????? ??????????","?????? ???????????","??????","????????? ?? ???????:","?????? ???? ?????","???????","????????? ?? ???????:","??? ?????:","?????????","????????????","?????","??????????? ???/??????","??????????? ??????/??????","??????????","?????? ?? ???????! =)","??? ?? ??????????","????????? ?:","??????? ?????????? ? ????? ???","??. ????????? ????? ? ???? ?????????? ?????????","????????? ???????????? ?? ????? ??????? ??? ? '|' ????????: 0,0|1,1","??????:","??????? ??? ????????? ?? ?????????? ?? ????????? ????. ???????? ???????? ? ???? ? ???? ????","???????????? ? ????? ??? ????????? ??:","Time between waves in ms: (default 500) "];

		cataText = ["?????? ??? =)","????????","??????","??????? ???????","??????","????? ????","????????????","????????","??????","???????","???????","?????","??????","????????? ?? ??????","????????? ?? ?????","?????","?????? ??????","?????? ?????","?????","?????????","???????","???????","???????????","????????","????????","??????????","??????","????????? ??????","?????? ???????","?????? ???????","???????","????? ?????","????? ??????","????","Treasury"];
		break;

	case "cz": //CeP
		text = ["Odesílám","Chybný typ útoku!","Chybí souradnice!","Nedostatek jednotek! (Jednotka #","Neurcil jsi útocné jednotky :)","Start","Asi špatné souradnice","Nemáš špehy","Hotovo","Nastavení vlny:","Pridat další vlnu","Reset","Nastavení útoku:","Typ útoku:","Útok","Podpora","Loupež","Špehovat suroviny/jednotky","Špehovat obranu/jednotky","Souradnice","Odeslat útok","Zjistit cas príchodu","Prijít v:","Nacasovat príchod","Pocet útoku ve vlne","oddelovac souradnic - ';' - stredník - napr.: -5|10;51|-110","CHYBA:","Nelze odeslat ..","Príchod nastaven na:","Time between waves in ms: (default 500) "];

		cataText = ["Vyber cíl =)","Náhodný","Drevorubec","Hlinený dul","Železný dul","Obilné pole","Pila","Cihelna","Slévárna","Mlýn","Pekárna","Sklad","Sýpka","Kovárna","Zbrojnice","Turnajové hrište","Hlavní budova","Shromaždište","Tržište","Ambasáda","Kasárny","Stáje","Dílna","Akademie","Radnice","Rezidence","Palác","Obchodní kancelár","Velké kasárny","Velké stáje","Hrdinský dvur","Velký sklad","Velká sýpka","Div sveta","Treasury"];
		break;
	...
	case "sk": //by eth4rendil
		text = ["Rozbi tú lamu =)","Chybný typ útoku!","žiadne súradnice!","Nedostatok vojakov! (Vojak #","Žiadny vojaci na utocenie :)","Zacínam","Pravdepodobne zlé súradnice","žiadny špeh","Hotovo","Nastavenie vlny:","Pridat novú vlnu","Reset","Nastavenie útoku:","Typ útoku:","Útok","Podpora","Lúpež","Špehovat suroviny a vojakov","Špehovat obranne budovy a vojakov","Súradnice","Rozbi tú lamu! =)","Cas príchodu","Príst o:","Nastavit casovaný príchod","Pocet útokov v danej vlne","with '|' ex: 0,0|1,1","Error:","Nemozno odoslat ..","Cas príchodu nastaveny na:","Time between waves in ms: (default 500) "];

		cataText = ["Vyberte si ciel =)","Náhodne","Drevorubac","Hlinená bana","Železná bana","Obilné pole","Píla","Tehelna","Zlievaren","Mlyn","Pekáren","Sklad","Sýpka","Kovácska dielna","Zbrojnica","Aréna","Hlavná budova","Bod stretunutia","Trhovisko","Ambasáda","Kasárne","Stajne","Dielna","Akadémia","Radnica","Rezidencia","Palác","Obchodná kancelária","Velké kasárne","Velké stajne","Hrdinský dvoj","Velký sklad","Velká sýpka","Div sveta","Treasury"];
		break;

	case "pl": //by robertnik
		text = ["Licze cos tam :P","Zly typ ataku!","Brak wspólrzednych!","Zle wybrales jednostki! (Atak #","Nie wybrano jednostek!","Zaplanowany ATAK","Zle wspólrzedne","Nie wybrales szpiegów","Atakuj ","Wybierz jednostki:","Dodaj nowy atak","Wyczysc","Szczególy ataku:","Typ ataku:","Atak: Normalny","Posilki","Atak: Grabiez","Obserwuj surowce","Obserwuj fortyfikacje","Wspólrzedne","Zaplanuj, pózniej odswiez ;)","Pokaz aktualny czas ataku","Przybycie o:","Zaplanuj atak o czasie dojscia","Ilosc ataków","inny cel po ',' przyklad: 0|0,1|1","BLAD: ","Niezdolny dostac aktywna wies.","Zaplanowany atak dojdzie o: ","Czas pomiedzy falami w ms: (domyslnie 500) "];

		cataText = ["Wybierz cel =)","Losowy cel","Las","Kopalnia gliny","Kopalnia zelaza","Pole","Tartak","Cegielnia","Huta zelaza","Mlyn","Piekarnia","Magazyn","Spichlerz","Kuznia","Zbrojownia","Plac turniejowy","Glówny budynek","Miejsce zbiórki","Rynek","Ambasada","Koszary","Stajnia","Warsztat","Akademia","Ratusz","Rezydencja","Palac","Targ","Duze koszary","Duza stajnia","Dwór bohaterów","Duzy magazyn","Duzy spichlerz","Cud :P","Treasury"];
		break;
	...
	case "ae": ///NMR111
text = ["?? ?????? =)","??? ?????? ????!","?????? ????????!","??? ?????? ???????? ??? ????! ?????? ???  #","?? ??? ?????? ??????","???????","???? ?????????? ?????","?? ????? ????? ?????","??????:??? ????????","??????? ??????:","????? ???? ???? ?????","??? ??????","??????? ??????:","??? ??????:","????: ????","??????","????: ?????","?????? ??? ??????? ???????","?????? ??? ????????? ???????","??????????","???? ????! =)","??? ??????","?????? ??:","????? ??? ??????","??? ???????","?????? ??? ???? ?? ???? ?? ??? ??? ??????????',' ????: 00|1,1|0*????? ??? ??????? 2000 ???? ??????? ??? ?? ???? ?? ??????","???:","???? ?????? ??? ?????? ??????. ?? ??????? ?? ???? ?? ???? ???? ????? ?????","??? ?????? ??? ???:","????? ??? ???????: (????????? 2000) "," ?? ?????:??? ??????? (NMR111)"];
...
break;
	...
	case "hu": // by Mijo
		text = ["Zúzás folyamatban =)","Érvénytelen támadási típus!","Mik a koordináták?!","nincs elég sereg! (Sereg #","Nincs sereg megadva!","Indítás","Valószínuleg hibás cél!","Nincs kémed!","Kész!","Hullámok beállítása:","Új hullám","Lenulláz","Támadás beállítása:","Támadási forma:","Normál","Támogatás","Fosztogatás","Kémkedés Nyersanyagok/Seregek","Kémkedés Védelmi Berendezések/Seregek","Koordináták","Zúzzunk oda! =)","Érkezési ido","Érkezés ekkor:","Idozített érkezés","Az adott hullám támadásainak száma","Több cél elválasztása '|'-val pl: 0,0|1,1","HIBA: ","Nem lehet meghatározni az aktuális falut! Feltételezhetoen egy falus játékos.","Idozített érkezés beállítva:","Time between waves in ms: (default 500) "];

		cataText = ["Válassz célpontot! =)","Véletlen","Favágó","Agyagbánya","Vasércbánya","Búzafarm","Furész üzem","Agyagégeto","Vasöntöde","Malom","Pékség","Raktár","Magtár","Fegyverkovács","Páncélkovács","Gyakorlótér","Foépület","Gyülekezotér","Piac","Követség","Kaszárnya","Istálló","Muhely","Akadémia","Tanácsháza","Rezidencia","Palota","Kereskedelmi központ","Nagy kaszárnya","Nagy istálló","Hosök háza","Nagy raktár","Nagy magtár","Világcsoda","Treasury"];
		break;
...
function AttackBuilder(account){
	...
	this.attack = function(){
		...
		for(var waveIndex = 0; waveIndex < _this.waves.length; waveIndex++){
			...
			for(var troopIndex = 0; troopIndex < 10; troopIndex++) {
				troopsPerAttack[troopIndex] = ((_this.spy != 0) == (troopIndex == spyIndex))? parseInt(troopsElements[troopIndex][waveIndex].value): 0;
			...
			_this.waves[waveIndex] = {
				...
				troops: {
					...
					t13: troopsPerAttack[12],

					t14: 0,
					t15: troopsPerAttack[14],
					c: _this.type,
					dname: ""
				},
				spec: {
		...
		for(var troopIndex = 0; troopIndex < 10; troopIndex++)
			if (totalTroops[troopIndex] > getTotalUnit('t' + (troopIndex + ((troopIndex == 9)? 2: 1)))) {
				errorMsg(text[4]);
				abort();
				return;
			}
	...
	this.postFirstPage = function(){
		autoPost(
			...
			function(response){
				...
				var rndsec = Math.round(_this.account.getTimePost());
				addCount("???????? "+rndsec+"mks | ");
				setTimeout(_this.setForAttackPerWave, rndsec) 
			}
		)
...
attackInterface.innerHTML = '<fieldset><legend>'+text[12]+'</legend><div id=start>' +
	...
	'<tr><td colspan="3">' +
		...
		'</select>' +
	'</td></tr><tr><td colspan="3">'+text[19]+' ' + ' <span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:#2200CC;">( '+text[25]+' )</span>' + '</td>'+
	'</tr><tr><td colspan="3">'+
		'<input type=\"text\" value="' +getCords() +  '" name=\"cords\" id=\"cords\" class=\"fm\" style=\"width: 98%\" /><br>'+ text[29] +'<input class=\"fm\" style=\"color:red;\" id=\"timebetween\" type=\"text\" value=\"2000\" size=\"4\" />' +
	'</td>'+
	'</tr><tr><td>'+
...
table += '</tr></table><button id="newWaveButton">'+text[10]+'</button> <button id="resetButton">'+text[11]+'</button> <span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:grey;">( * - '+text[24]+' )</span><span style="font: 11px/1.5em Tahoma,Verdana,Arial !important; color:#484006;">'+text[30]+'</span>';
