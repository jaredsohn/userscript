...
// @description    Dodaje opcej zapamietania wiadomosci
...
var nazwy=new Array(" RS ","Flota"," PZ ","Obrona","Sojusz","Gracze"," RW ","Inne","Wszystkie");//categories names
...
function pokaz(typw){//show category
	...
	x.className='c';
	x.innerHTML="Zachowane wiadomosci";
	w.appendChild(x);
	tbody.appendChild(w);
...
//	
	...
	w=document.createElement('tr');
	x=document.createElement('th');
	x.innerHTML="Akcja";//headers like in message table
	w.appendChild(x);
	x=document.createElement('th');
	x.innerHTML="Data";
	w.appendChild(x);
...
if(adres.href.indexOf("bericht.php")==-1){
	...
	for(i=20;i<td.length;i++){
		if(td[i].className=='b' && td[i].innerHTML==' '){
			...
			w.setAttribute('value','Zapisz'); //save button
	...
	w=document.createElement('input');
	w.setAttribute('type','button');
	w.setAttribute('value','Zapamietaj RW');//save battle report button
