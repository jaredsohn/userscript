// ==UserScript==
// @name        Lower Merion Library - LibraryLookup
// @description Check book availability in Lower Merion Library.  When you look at a book on Amazon.com, this script will check the Lower Merion Library catalog, then show you in the best link to get the book.  Written by Nick Baker, Reference and Web Services Librarian; adapted for Lower Merion Library by David Manning.  Based on adapation for Chicago Public Library by Mike Harris.
...
(
   function()
   {
      ...
      var lowermerionpub =
      {
            name: 'Lower Merion Public Library',
			image: '',

            createSearchUrl: function(isbn)
            {
	       var prefix = 'http://cat.lmls.org/polaris/search/searchresults.aspx?&type=Keyword&by=ISBN&sort=MP&limit=TOM%3d*&query=&page=0&term=';
               return prefix + isbn
            },
            ...
            keepLooking: function()
            {
               if ( 0 == extraIsbns.length && 1 == this.libraries.length )
               {
				   ...
				   var sfx = '<a target="_blank" style="text-decoration:none" ' +
				   		'href="http://www.worldcat.org/search?q=' + originalIsbn + '&=Search&qt=owc_search">' +
					   '<B><U>NO</U></B>: the library does <B><I>not</I></B> own this.  Click here to access the book on WorldCat for a intralibrary loan.</a>';
     ...
     // Change this to add, delete, or change order in which libraries are
      // checked
      libraryLookup.libraries = [lowermerionpub];
