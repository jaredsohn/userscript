// ==UserScript==
// @name           The Golden Army - Tetenek
// @namespace      N/A
// @description    The Golden Army - Tetenek
...
function rerun(){
	...
	if(data == null){
		alert('KofC data yok.Oluşturuluyor...');
	...
	for(var i=0;i<poch_mods.length;i++){
		var a = poch_mods[i];
		poclog.log('Mod yükleniyor: ' + a.name + ' v' + a.version);
		poclog.log('-> fonksiyonlar ekleniyor');
		a.functions();
		poclog.log('-> değişkenler ekleniyor');
		a.variables();
		poclog.log('-> sekmeler ekleniyor');
		document.getElementById('koc_flags').innerHTML += '<a id="flag_'+ a.ident +
							'" onclick="pocgui.tabs.onlick(this)">'+ a.tab +'</a>';
		poclog.log('-> bilgi kutusu ekleniyor');
		document.getElementById('pocinfobox').innerHTML += '<div id="info_'+ a.ident + '"></div>';
		poclog.log('-> görünüm kutusu ekleniyor');
		pocgui.getdisplay[a.ident] = a.displaybox;
		pocrepaint();
		poclog.log('-> kontroller ekleniyor');
...
function pocFunctions(){
	...
	window.pocbuild.add = function(c,a){
		...
		if(w>9){alert('Gereksinimlerinden dolayı, 9 level üstü inşaatlar \n manual yapılmalı .');return null;}
		var e=Math.pow(2,w);
		  ...
		  new Ajax.Request(g_ajaxpath + "ajax/construct.php" + g_ajaxsuffix,{
			 ...
			 onSuccess : function(transport){
				...
				if(rslt.ok){
					...
					y.push('Yapılıyor <b>' + buildingcost['bdg' + a.type ][0] + ' lvl' + q + '</b> in');
					for(var i=0; i<seed['cities'].length;i++){
	...
	window.pocbuild.display = function(a){
		...
		y.push('<a class="button20" id="queuecancel_' + a + '" onclick="pocbuild.remove(this);"><span>iptal</span></a>');
		y.push('</td></tr></table>');
		return y.join('');
	};
	...
	window.poccontrols['buildmode'] = function(a){
		if(window.mode == 1){
				a.innerHTML = '<span>İnşa Modu</span>';
				window.mode = 0;
				window.buildslot = window.old_buildslot;
			}else{
				a.innerHTML = '<span>Çık</span>';
				window.mode = 1;
	...
	window.pocgui.getdisplay['log'] = function(){
		...
		if(pocdata.log.length>0){
			...
			ret.push('<div align="center"> <b> Kayıt Yok</b></div>');
		}
		return ret.join('');
	};
	window.pocgui.getdisplay['build'] = function(){
		...
		if(a.length>0){
			...
			ret.push('<div align="center"> <b> Yapım yok</b></div>');
		}
		return ret.join('');
	}
	...
	window.pocgui.tabs.onlick = function(a){
		...
		try{
			...
			poclog.log( tabselected + ' için bilgi kutusu bulunamadı.');
		}
		pocrepaint();
		pocupdatedisplay();
	};
	...
	window.pocupdatedisplay = function(){
		...
		debug.innerHTML = pocgui.getdisplay[tabselected]();
			...
			poclog.log(tabselected + ' için görünüm bulunamadı.')
		}
		return;
	};
...
function pocVariables(){
	...
	window.pochtmls.controls['log'] = '<a id="con_clearlog" class="button20" onclick="pocgui.control.onclick(this)"><span>Kayıt Temizle</span></a>';
	window.pochtmls.controls['build'] = '<a id="con_buildmode" class="button20" onclick="pocgui.control.onclick(this)"><span>İnşa Modu</span></a><a id="con_sortBT" class="button20" onclick="pocgui.control.onclick(this)"><span>Sırala</span></a>';
	window.pochtmls.mainbody = '<table id="pochmain" style="position:fixed; top:64px; left:765px" cellspacing="0" border="0" width="450"><tr style="color:#FFFFFF; padding:0; background-image:url('+ pocimages.kochead + ')" height="93" width="450" border="0" id="koc_head"><td width="20px" ></td><td width="210" ><div style="position:relative; top:25px" id="pochHead"><div id="pochtitle" style="color:black;">'+ SCRIPT.name +'</div><div id="pochversion" style="color:black;">versiyon '+ SCRIPT.version + ' yazılım ' + SCRIPT.build +'</div></div></td><td><div style=" position:relative; top:25px; margin-left:95px"><a class="button20" onclick="poctoggletimer()"><span style="text-align: center" id="timer_control">Başlat</span></a><a class="button20" onclick="eval(prompt());"><span style="text-align: center" id="evaluate">?</span></a><a class="button20" onclick="window.details = !window.details;pocupdatedisplay();"><span>Görünüm</span></a></div></td></tr><tr height="50px" style="background-image:url('+ pocimages.koclower +')" bgcolor="#FFFFCC" id="koc_lower"><td colspan="2"><div id="koc_flags" style=" border:0; line-height:40px; left:10px"><a id="flag_log" class="sel" onclick="pocgui.tabs.onlick(this)">Kayıt</a><a id="flag_build" class="" onclick="pocgui.tabs.onlick(this)">İnşa</a></div></td><td valign="top"><div style="position:relative; top:10px" id="koc_controls"><a class="button20" onclick="controlClick(this)"><span>Kayıtları Temizle</span></a></div></td></tr><tr><td height="360px" style="background-image:url('+ pocimages.kocbody +')" bgcolor="#FFFFCC" valign="top" colspan="3" id="koc_debugmain"><div id="pocinfobox"></div></div><div id="debugmain" style=" position:relative; left:8px;width:431px;height:350px;overflow-x: hidden; overflow-y: scroll; border:1px solid #A56631"></div></td></tr><tr><td height="36px" style="background-image:url('+ pocimages.kocbottom +')" bgcolor="#FFFFCC" colspan="3" id="koc_bottom"></td></tr></table>';
