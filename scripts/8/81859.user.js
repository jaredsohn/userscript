...
function RedInMessages() {
	...
	for (var i = 0; divMessages[i]; i++)
	{
		...
		{
			divMessages[i].innerHTML = divMessages[i].innerHTML.replace(/<strong>Shivos<\/strong>/, '<strong class="moderateur">Shivos<\/strong>');
		}
	}
}
...
function RedInTopics() {
	...
	for (var i = 0; trTopics[i]; i++)
	{
		...
		{
			trTopics[i].innerHTML = trTopics[i].innerHTML.replace(/<td class="pseudo">Shivos<\/td>/, '<td class="pseudo topic_mod">Shivos<\/td>');
		}
	}
}
