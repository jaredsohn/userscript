// ==UserScript==
// @name           SACO
// @namespace      SoldatRyan
// @description    Généateur de statistique de raids
...
var title = "De SoldatRyan pour la SACO";
var sessionTitle = "Liste des raids";
var sessionTitleUp = ["&nbsp", "Pillages", "Recyclage", total];
var sessionTitleDown = ["nÂ°", "Coordonnée", ressourceNames[METAL], ressourceNames[CRISTAL], 

ressourceNames[DEUT], ressourceNames[METAL], ressourceNames[CRISTAL], "&nbsp"];
...
Raid.prototype.sum = function(){return this.ressources[0] + this.ressources[1] + this.ressources[2] + 

this.debrisField[0] + this.debrisField[1];}
...
("playerName").getElementsByTagName("span")[0].innerHTML;
	else
		playerName = getElementsByList("wrapper", ["div", "table", "tbody", "tr", "td"], 
...
function isRaidReport(){
	...
	if(!(getElementsByList("wrapper",["div", "table", "tbody", "tr", "td", "span"],[1,0,0,0,0,2]))) return 
...
function removeRaid(number){
	...
	for(var i = 0; i < raidSession.number; i++){cookies.erase(server + "_" + playerName + 
...
function initInterface(){
	...
	document.getElementById("box").insertBefore(upDateButton, document.getElementById

("inhalt"));
}
function initButtons(){
	var text = "<center><h2 id=RaidTitle><font color=orange>" + title + 

"</font><br\><br\></h2></center>"
...
function makeRaidSessionSummary(){
	var table = "<br><center>" + sinds + " " + sessionDate  + "<br><br><table bgcolor='" + 
...
tableColor + "'>";
	...
	table += "<td align=center>" + "%" + "</td>";
	table += "<td align=center>" + "[-]" + "</td>";
	table += "<td align=center>" + Math.round(raidSession.ressources[METAL]/sum*100) + "%

</td>";
	table += "<td align=center>" + Math.round(raidSession.ressources[CRISTAL]/sum*100) + "%

</td>";
	table += "<td align=center>" + Math.round(raidSession.ressources[DEUT]/sum*100) + "%

</td>";
	table += "<td align=center>" + Math.round(raidSession.debrisField[METAL]/sum*100) + "%

</td>";
	table += "<td align=center>" + Math.round(raidSession.debrisField[CRISTAL]/sum*100) + "%

</td>";
	table += "<td align=center>" + "100%" + "</td>";
	table += "</tr>";
	table += "</tbody></table>";
	table += "<br><br>" + lostText[0] + " = " + raidSession.myLost + "<br>" + lostText[1] + " = " + 

raidSession.hisLost;
	table += "<br><br><input style='width: 230px'  id='resetId' type=submit value='" + reset + 
...
function makeRaidSumSummary(sum){
	...
	table += "<caption><h2>" + sumTitle[0] + " : " + raidSum.number + " raids" + 
...
"</h2></caption>";
	...
	for(var i = 0; i < ressourceNames.length; ++i){
		...
		table += "<td align=center>" + addPoints((raidSum.debrisField[i] ? 

raidSum.debrisField[i] : "0")) + "</td>";
		var sum = (raidSum.ressources[i] + (raidSum.debrisField[i] ? raidSum.debrisField[i] 

: 0));
		table += "<td align=center>" + addPoints(sum)+ "</td>";
		table += "<td align=center>" + Math.round(sum/sumsum*100) + "</td>";
		table += "<td align=center>" + addPoints(Math.round(sum/raidSum.number)) + 
...
"</td>";
	...
	table += "<tr>";
	table += "<td align=center>" + total + "</td>";
	table += "<td align=center>" + addPoints((raidSum.ressources[0] + raidSum.ressources[1] + 

raidSum.ressources[2])) + "</td>";
	table += "<td align=center>" + addPoints((raidSum.debrisField[0] + raidSum.debrisField[1])) + 
...
"</td>";
	...
	table += "</tr>";
	table += "</tbody></table>";
	table += "<br><br>" + lostText[0] + " = " + raidSum.myLost + "<br>" + lostText[1] + " = " + 

raidSum.hisLost + "</center>";
	return table;	
}
function makeFormulaire(){
	...
	table += "<tr bgcolor='" + lineColor[1] + "'><td>" + coord + "</td><td><input id='mod" + -1 + "' 

value='[undefined]' size=8></td></tr>";
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=center>" + formTitle[1] + "</td></tr>";
	table += "<tr bgcolor='" + lineColor[0] + "'><td>" + ressourceNames[METAL] + 

"</td><td><input id='mod" + 0 + "' value='0' size=8></td></tr>";
	table += "<tr bgcolor='" + lineColor[1] + "'><td>" + ressourceNames[CRISTAL] + 

"</td><td><input id='mod" + 1 + "' value='0' size=8></td></tr>";
	table += "<tr bgcolor='" + lineColor[0] + "'><td>" + ressourceNames[DEUT] + 

"</td><td><input id='mod" + 2 + "' value='0' size=8></td></tr>";
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=center>" + formTitle[2] + "</td></tr>";
	table += "<tr bgcolor='" + lineColor[1] + "'><td>" + ressourceNames[METAL] + 

"</td><td><input id='mod" + 3 + "' value='0' size=8></td></tr>";
	table += "<tr bgcolor='" + lineColor[0] + "'><td>" + ressourceNames[CRISTAL] + 

"</td><td><input id='mod" + 4 + "' value='0' size=8></td></tr>";
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=center>" + formTitle[3] + "</td></tr>";
	table += "<tr bgcolor='" + lineColor[1] + "'><td>" + ressourceNames[METAL] + 

"</td><td><input id='mod" + 5 + "' value='0' size=8></td></tr>";
	table += "<tr bgcolor='" + lineColor[0] + "'><td>" + ressourceNames[CRISTAL] + 

"</td><td><input id='mod" + 6 + "' value='0' size=8></td></tr>";
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=center>" + formTitle[4] + "</td></tr>";
	table += "<tr bgcolor='" + lineColor[1] + "'><td>" + "nÂ°" + "</td><td><input id='" + 

"numberid" + "' value='0' size=8></td></tr>";
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=center>" + formTitle[5] + "</td></tr>";
	table += "<tr bgcolor='" + lineColor[0] + "'><td>" + add_remove[2] + "</td><td><select 
...
id='selectId'>";
	...
	table += "</select></td></tr>"
	table += "<tr><td>&nbsp</td></tr>";
	table += "<tr><td colspan=2 align=right><input style='width: 230px'  id='submitId' 
...
function submitRaid(){
	...
	if(document.getElementById("selectId").selectedIndex == 1){
		if(confirm("Etes vous sur de vouloir supprimer le raid nÂ°" + raid.number + " ?")){
...
function addRaidButton(){
	...
	buttonAccess = buttonAccess.getElementsByTagName("ul")[0].getElementsByTagName("li")

[7];
	var button = document.createElement("li");
	button.innerHTML = '<a target="_self" accesskey="" href="' + 

buttonAccess.getElementsByTagName("a")[0].href + "&Terminator" + '" class="menubutton "><span 

class="textlabel">Raid</span></a>';
	buttonAccess = document.getElementById("links").getElementsByTagName("ul")

[0].getElementsByTagName("li")[10];
...
function readReport(){
	...
	getIdentifier();	
	var pos = 1
	if(strContains(document.getElementById("combatants").getElementsByTagName("div")

[0].innerHTML, playerName))
		pos = 0;
...
""));
	...
	access = getElementsByList("shortreport", ["tbody", "tr", "td", "table", "tbody", "tr", "td"], 
...
function checkUpDate(){
	GM_xmlhttpRequest({
		...
		function(answers){
			...
			var versionOfScript = page.substring(page.indexOf('<b>Version</b> : ')
...
"<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='" + URLS + "'><font color=orange> " + 
 ...
 * @param id L'identifier de l'objet de dÃ©part
 * @param tags Liste de tags Ã suivre
...
function getElementsByList(id, tags, index){
	...
	var elem;
	for(var i = 0, elem = document.getElementById(id) ; elem && i < tags.length; elem = 

elem.getElementsByTagName(tags[i])[index[i]], ++i);
 ...
 * @param time Le temps pendant lequel la fade box est affichÃ©e
 */
...
function coockiesFunctions(isFirefox){
	if(isFirefox){
		return {
			...
			load : function (key, defaultValue){return GM_getValue(key, 
...
defaultValue);},
		...
		return {
			save : function (key, value){localStorage.setItem(namespace + key, 

value);},
			load : function (key, defaultValue){var res = localStorage.getItem

(namespace + key); return (res ? res : defaultValue); },
 ...
 * @return Indice de l'element ou -1 si il n'est pas prÃ©sent
 */
...
function addPoints(nombre){
	...
	return  signe + (((n % 3) ? str.substr(0, n % 3) + '.' : '') + str.substr(n % 3).match(new RegExp
