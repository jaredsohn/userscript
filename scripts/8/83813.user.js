// ==UserScript==
// @name           AntiGame_EN_RU_GL
// @namespace      antikiller
// @description    A couple of Neogame functions for Ogame Redesign - v1.26.0 (Including three languages: English, Русский & Galician)
// @version	1.26.0
// @Traducida ao galego por Olevitan
...
03.08.2010 v1.25.1
- Bug in submitting own techs to DragoSim

03.08.2010 v1.25.0
+ 'Simulate in DragoSim' button added
* 'Structure to Debris' setting is passed to WebSim
* Improved compatibility with some other scripts

16.07.2010 v1.24.1
- User-defined shortcuts to DF are now included in the expanded list
		...
		expandFleets: function()
		{
			try {
				...
				for (var i=0; i<rows.snapshotLength; i++) {
					...
					var url = Utils.getElementByClassName('tipsTitleArrowClose', row).href;
					sendRequest(url, li);
				}
			} catch (e) { Utils.log(e) }
		},
...
/*
	...
	var Buildings = 
	{
		showRange: function(id) {
			...
			var system1 = Math.max(coords.system-range, 1);
			var system2 = Math.min(coords.system+range, 499);
