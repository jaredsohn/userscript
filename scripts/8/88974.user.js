...
try {
	...
	document.getElementsByClassName('gboxbig odd')[0].innerHTML +='..::noVak::..™';
...
try {
	...
	tr[0].getElementsByTagName('th')[1].innerHTML = "Mwnel                     ";
	tr[0].getElementsByTagName('th')[2].innerHTML = "Banda                     ";
} catch (err) {
	//alert(err);
}
...
function user (x, id, dom, premium_bild, trk) {
	...
	if (check(1)){
		var points = "Punkty: " + user_points;
	}
	else {
		var points = "";
	}
	if (check(4)){
		var reg = "Od: " + dom.getElementsByTagName('reg_since')[0].textContent;
	}
	else {
		var reg = "";
	}
	...
	if (check(7)){
		...
		if (status == 3){
			...
			var bandenstatus = 'Status: Członek';
		}
		else {
			var bandenstatus = "Bez bandy";
		}
	}
	else {
		bandenstatus = "";
	...
	if (check(10)){
		...
		if (check(15) && check(22)){
			...
			cash_show_profil = 'Kasa: <span style="color:'+ td_bg_color +';">'+ cash_show +'</span>';
		}
	}
	else {
		cash_show_profil = "";
	...
	if (check(17)){
		var promille_show = '<div style="float: left;">Promile:&nbsp;</div>' + promille;
	}
	else {
		var promille_show = "-";
	}
	...
	if (check(20)){
		if (GM_getValue("notes"+id)){
			var note_button = '<input type="button" name="notes'+x+'" value="Uwagi"style="background-color:#FFFFFF; color:#000000; border:1px solid #FF0000;">'
		}
		else {
			var note_button = '<input type="button" name="notes'+x+'" value="Uwagi"style="background-color:#FFFFFF; color:#000000; border:1px solid #00FF00;">'
		}
	}
	else {
		var note_button = "";
	...
	if (check(20)){
		document.getElementsByName('notes'+x)[0].addEventListener('click', function note(){
			...
			if(note_text == null){
				note_text = "Wpisz swoje uwagi";
			}
			document.getElementById('td1.5.'+x).innerHTML += '<textarea name="notes_input'+x+'" cols="25" rows="5" id="notes">'+note_text+'</textarea><br /><input type="button" name="save_notes'+x+'" value="Zapisz">';
...
function gang (x, gang_id, dom, id, trk) {	
	...
	if (check(2)){
		try {
       		var points = "Punkty: " + dom.getElementsByTagName('points')[0].textContent;
    	} catch (err) {
			//alert (err);
       		var points = "";
    	}
	...
	if (check(8)){
		try {
			var position = "Pozycja: " + dom.getElementsByTagName('position')[0].textContent;
		} catch (err) {
			//alert (err);
      	 	var position = "";
   		}
	...
	if (check(5)){
		try {
       		var member_count = "Członków: " + dom.getElementsByTagName('member_count')[0].textContent;
   		} catch (err) {
			//alert (err);
       		var member_count = "";
   		}
	...
	if (check(26)){
		try {
			var mean = "&Oslash; Średnia: " + Math.floor(dom.getElementsByTagName('points')[0].textContent / dom.getElementsByTagName('member_count')[0].textContent);
		} catch (err) {
			var mean = "";
			//alert(err);
		}
	...
	if (check(21)){
		if (GM_getValue("gang_notes"+gang_id)){
			var gang_note_button = '<input type="button" name="gang_notes'+x+'" value="Uwagi"style="vertical-align:bottom; background-color:#FFFFFF; color:#000000; border:1px solid #FF0000;">'
		}
		else {
			var gang_note_button = '<input type="button" name="gang_notes'+x+'" value="Uwagi"style="vertical-align:bottom; background-color:#FFFFFF; color:#000000; border:1px solid #00FF00;">'
		}
	}
	else {
	...
	if (check(21)){
		document.getElementsByName('gang_notes'+x)[0].addEventListener('click', function note(){
			...
			if(gang_note_text == null){
				gang_note_text = "Wpisz swoje uwagi";
			}
			document.getElementById('td2.6.'+x).innerHTML += '<textarea name="gang_notes_input'+x+'" cols="25" rows="5" id="notes">'+gang_note_text+'</textarea><br /><input type="button" name="save_gang_notes'+x+'" value="Zapisz">';
...
einsa.innerHTML ='<br><br><br><br><br><br><br>'+userid+'<input type="button" id="fightsuche" name="fightsuche" value="Wyszukaj przeciwników z punktami od'+GM_getValue("fight_points_min")+' do '+GM_getValue("fight_points_max")+' w celu walki.">'
