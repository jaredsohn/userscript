// ==UserScript==
// @name           Ogame Redesign: Advanced Messages **Beta**
// @namespace      vtr
// @description    Displays advanced options in messages like: Reply by PM in circular messages. To be continued...
// @version	0.5
...
var EstimatedTimeFunc = function() {
    ...
    Lang.MessagesGR =
	{
	    Circular: 'Κυκλικό Μήνυμα',
	    Send: 'Στείλε',
	    ReplyTo: 'Απάντηση Σε'
	}
    Lang.MessagesEN =
	{
	    Circular: 'Circular Messages',
	    Send: 'Send',
	    ReplyTo: 'Reply to'
	}
    ...
    var Options =
	{
	    ...
	    initLang: function() {
	        ...
	        this.Messages = Lang['Messages' + Utils.server_lang];
	        this.copyMissingProperties(Lang.MessagesEN, this, 'Messages');
	        this.readResLabels();
	    },
	...
	{
	    Run: function() {
	        document.getElementById('messageContent').addEventListener("DOMNodeInserted", Messages.onDOMNodeInserted, false);
	    },
	    getUsers: function() {
	        var userIds = Utils.getValue('EBTcookie');
	        if (userIds) {
	            userIds = userIds.split('&');
	            for (var i = 0; i < userIds.length; i++) {
	                userIds[i] = userIds[i].split('=');
	            }
	        }
	        return userIds;
	    },
	    SaveUsersIDs: function() {
	        if (Utils.isFirefox) {
	            var $ = Utils.$;
	            var users = Messages.getUsers();
	            var str = '';
	            var newusers = '';
	            var userTRs = Utils.XPath('//table[@id="member-list"]/tbody/tr');
	            for (var i = 0; i < userTRs.snapshotLength; i++) {

	                var TR = userTRs.snapshotItem(i);
	                var TDs = Utils.XPath('td', TR);
	                var user = TDs.snapshotItem(0).innerHTML.trim();
	                var aElement = Utils.XPath('a[@title]', TDs.snapshotItem(7)).snapshotItem(0);
	                if (aElement) {
	                    var id = aElement.href.match(/\&to=([0-9]{1,20})\&/)[1];

	                    if (!Messages.GetIdByUser(user)) {
	                        if (newusers != '')
	                            newusers += '&';
	                        newusers += user + '=' + id;
	            ...
	            if (str != '')
	                setTimeout(function() { Utils.setValue('EBTcookie', str) }, 0);
	        }

	    },
	    GetIdByUser: function(user) {
	        try {
	            var userIds = Messages.getUsers();
	            for (var i = 0; i < userIds.length; i++) {
	                if (userIds[i][0] == user) {
	                    return userIds[i][1];
	                    break;
	    ...
	    InitReply: function() {
	        ...
	        if (!$('#openCloseForm')[0]) {
	            ...
	            messagebox.style.height = '620px';
	            var wrapper = $('#wrapper')[0];
	            var session = document.location.href.match(/\&session=([0-9a-fA-F]{1,12})\&/)[1];
	            var msg_id = document.location.href.match(/\&msg_id=([0-9]{1,20})\&/)[1];

	            var userrow = Utils.XPath('//div[@id="wrapper"]//div[@class="note"]').snapshotItem(0).innerHTML.trim();
	            userrow = userrow.substring(0, userrow.indexOf(':'));
	            var possibleusers = userrow.split(' ');
	            var id;
	            var user;
	            for (var i = 0; i < possibleusers.length; i++) {
	                user = possibleusers[i];
	                id = Messages.GetIdByUser(user);
	                if (id)
	                    break;
	            }
	            //alert(Utils.XPath('//div[@id="wrapper"]/div[@class="textWrapperSmall"]').snapshotItem(0));
	            Utils.XPath('//div[@id="wrapper"]/div[@class="textWrapper"]').snapshotItem(0).style.height = '470px';
	            if (id) {
	                Utils.XPath('//div[@id="wrapper"]/div[@class="textWrapper"]').snapshotItem(0).style.height = '340px';
	                var newDiv = document.createElement("div");
	                newDiv.innerHTML = '<div class="answerHeadline open" ' +
	                'onclick="if(this.className==\'answerHeadline open\')' +
	                '{this.className=\'answerHeadline\'; ' +
	                'document.getElementById(\'answerForm\').style.display=\'none\';' +
	                '$(\'.textWrapper\')[0].style.height = \'475px\';}' +
	                'else{this.className=\'answerHeadline open\';' +
	                'document.getElementById(\'answerForm\').style.display=\'inline\';'+
	                '$(\'.textWrapper\')[0].style.height = \'340px\';}">' +
	                 Options.Messages.ReplyTo + ' ' + user + '<a id="openCloseForm" href="#"></a></div>' +
	        '<div id="answerForm" class="textWrapperSmall">' +
			'<form autocomplete="off" target="_parent" method="post" action="index.php?page=messages&amp;session=' + session +
			'&amp;to=' + id + '&amp;isAnswerMessage=1&amp;relationMessageId=' + msg_id + '">' +
				'<div class="answerText">' +
					'<textarea tabindex="3" name="text" class="mailnew" onkeyup="javascript:cntchar(2000)"></textarea>' +
					'<input type="hidden" name="betreff" value="RE:' + Options.Messages.Circular + '" />' +
				'</div>' +
   				'<div class="answerText">' +
					'<div class="fleft count textBeefy">(<span id="cntChars">0</span> / 2000)</div>' +
					'<div class="fleft buttonbox">' +
						'<input tabindex="4" type="submit" class="button188" value="' + Options.Messages.Send + '" />' +
					'</div>' +
					'<br class="clearfloat" />' +
				'</div>' +
			'</form>' +
	    ...
	    ChangeHrefAttr: function(link, Attr, Value) {
	        if (link.href) {
	            var href = link.href;
	            var strAfterAttr = '';
	            var strBeforeAttrValue = '';
	            var strafterAttrValue = '';
	            if (href.indexOf(Attr + '=') > -1) {
	                strAfterAttr = href.substring(href.indexOf(Attr + '='));
	                strBeforeAttrValue = href.substring(0, href.indexOf(Attr + '=') + Attr.length + 1);

	                if (strAfterAttr.indexOf('&') > -1)
	                    strafterAttrValue = strAfterAttr.substring(strAfterAttr.indexOf('&'));
	                href = strBeforeAttrValue + Value + strafterAttrValue;
	                link.href = href;
	            }
	        }
	    },
	    onDOMNodeInserted: function(e) {
	        if (e.target.name == 'delMsg') {
	            var links = Utils.XPath('//*[@id="mailz"]/tbody//td[@class="subject"]/a');
	            var i = 0;
	            while ((link = links.snapshotItem(i)) != null) {
	                Messages.ChangeHrefAttr(link, 'height', '620');
	                i++;
	            }

	        }
	        if (e.target.id == 'allyMemberlist') {
	            Messages.SaveUsersIDs();
	        }
	    },
	    InitUsers: function() {
	        document.getElementById('eins').addEventListener("DOMNodeInserted", Messages.onDOMNodeInserted, false);
	    }
    ...
    try {
        ...
        if (Utils.isCurrentPage('showmessage'))
            setTimeout(Messages.InitReply, 0);

        if (Utils.isCurrentPage('alliance'))
            Messages.InitUsers();

        //alert(Utils.getValue('EBTcookie'));
