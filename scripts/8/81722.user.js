...
var div, spanOutdateds, spanUpdateds, a, divoptions, fadingNode;
var outdateds=0, spanCount, totalScripts, contScripts=0, notfound=0, zindex = 10000;
var imgCompare, imgUpdate, imgHide, imgRemove;
	...
	GM_xmlhttpRequest({
		...
		onload: function(resp) {
			if(resp.status==503){
				...
				//debug('queued script '+arrInfo+': '+arrScripts.join(';'));
			}else{
				if(getId('divUpdater'))//?
...
function options()
{
	...
	sel.appendChild(createElement('option', {value:12}, '', 12+' hours'));
	sel.appendChild(createElement('option', {value:24}, '', 24+' hours'));
	for(var i=2; i<8; i++){
...
function getCSS()
{
	return 'a.USO_outdatedLink:link {color: #0088FF !important; text-decoration: underline !important;}'  +  
