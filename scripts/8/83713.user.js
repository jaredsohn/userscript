...
var militaryAlert = document.evaluate(
    "//div[@id='advisors']//li[@id='advMilitary']//a[@class='premiumalert']",
    document,
    null,
...
function processAttacks(responseDetails) {
  ...
  var table = document.createElement("table");
	...
	for (var i=0; i < table.rows.length; i++) {
		...
		if (row.className.search("hostile") != -1) { // Hostile action
		  ...
		  var timeH; // = row.cells[1].innerHTML.split("h")[0];
			...
			GM_log('row ['+row.cells[2].innerHTML+']');
			...
			if (cityIDs.search(targetID) != -1) {  //own city
				GM_log('own city');
	      if (shipAmount > 1 || timeH < 1 || troopsAmount == parseInt(troopsAmount)) {
					threats++;
				}
			}
			else {
	      ...
	      /*if (shipAmount = 1) {
					GM_log('testing');	
					var fleets = document.getElementById('fleetmovements');
					GM_log('fleetmovements ['+fleets+']');	
					if (fleets) {
					    fleets.parentNode.removeChild(fleets);
					}
			}
		*/
	...
	if (threats > 0) {
		$("li#advMilitary a:first").attr("class","premiumalert");
	}
...
function GM_ready() 
{
	$("li#advMilitary a.premiumalert").attr("class","normal");
