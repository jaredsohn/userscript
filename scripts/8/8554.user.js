// ==UserScript==
// @name           Google Maps + MLS.ca Details Page
// @description    Adds a link to google maps from the Property Details Page.
...
function findParent(child, nodeName) 
{
...
var tblHeading = mainHeadings.snapshotItem(0);
...
var centeredNodes = document.evaluate(
  	"//td[@class='HeadingCenter']",
  	document,
  	null,
  	XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
 	 	null);

var addressTd = centeredNodes.snapshotItem(0);

if (img && tblHeading && addressTd){
  
  /*
    The MLS number and the price are located in this cell, along with a lot of extra markup
    1) strip off the mls number
    2) remove the HTML formatting
  */
  var price = tblHeading.rows[0].cells[0].innerHTML.replace(/MLS\S:\s?\d+\s+/g, '');
  price = price.replace(/[^\d$,]/g, '');
  
  var mls = img.alt.replace(/MLS\S/g, '').replace(trimReg, '');
  
  /*
  Split the address information into an array and rebuild it from the first
  two elements
  */
  var tmp = addressTd.innerHTML.split('<br>');
    
  var address = tmp[0].replace(trimReg, '');
  //Google Maps doesn't like CR, replace with Crescent       	
  address = address.replace(' CR', ' CRESCENT');
  
  address += ', ' + tmp[1].replace(trimReg, '');
  ...
  if (validAddress.test(address)) {  
    ...
    */    	
  	...
  	var td = tblHeading.rows[0].insertCell(tblHeading.rows[0].cells.length);
