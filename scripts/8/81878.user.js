...
// @version           	0.2
// @namespace		gretools
// @description		Some improvements and tools for the Grepolis browser game. Based on the script by Dreambreaker.
// @include		*.grepolis.*/game/*
// ==/UserScript==

/*
   Changelog 0.1 (Cfuzz)
   - Menus translated to Spanish, I might add language support later.
   - Rearranged some items. Buildings on a horizontal bar on bottom of page,
     storage times under each resource and additional bar with hourly production
     under the town name.
   - Added Building name (only spanish) and time when resources will be enough to
     upgrade in town view.

	Changelog 0.9.2 (Dreambreaker)
...
while(gtNotesInst>=1){
	...
	if(typeof(gtNotes[gtNotesInst])=='undefined'){gtNotes[gtNotesInst]='Escribe una nota...';}
	--gtNotesInst;
}
...
if(location.href.match('game/building_main')){
   ...
   GM_setValue('gtb_academy_enough_resources_at',gtb.b.academy.enough_resources_at);
   GM_setValue('gtb_barracks_enough_resources_at',gtb.b.barracks.enough_resources_at);
   GM_setValue('gtb_docks_enough_resources_at',gtb.b.docks.enough_resources_at);
   GM_setValue('gtb_farm_enough_resources_at',gtb.b.farm.enough_resources_at);
   GM_setValue('gtb_hide_enough_resources_at',gtb.b.hide.enough_resources_at);
   GM_setValue('gtb_ironer_enough_resources_at',gtb.b.ironer.enough_resources_at);
   GM_setValue('gtb_lumber_enough_resources_at',gtb.b.lumber.enough_resources_at);
   GM_setValue('gtb_main_enough_resources_at',gtb.b.main.enough_resources_at);
   GM_setValue('gtb_market_enough_resources_at',gtb.b.market.enough_resources_at);
   GM_setValue('gtb_place_enough_resources_at',gtb.b.place.enough_resources_at);
   GM_setValue('gtb_stoner_enough_resources_at',gtb.b.stoner.enough_resources_at);
   GM_setValue('gtb_storage_enough_resources_at',gtb.b.storage.enough_resources_at);
   GM_setValue('gtb_temple_enough_resources_at',gtb.b.temple.enough_resources_at);
   GM_setValue('gtb_wall_enough_resources_at',gtb.b.wall.enough_resources_at);
}
//Building Levels - Set
if(location.href.match('game/index')){
	var gtBuildings={
		main:{
         ...
         era:GM_getValue('gtb_main_enough_resources_at')
		},
		academy:{
			level:GM_getValue('gtb_academy_level'),
         era:GM_getValue('gtb_academy_enough_resources_at')
		},
		barracks:{
			level:GM_getValue('gtb_barracks_level'),
         era:GM_getValue('gtb_barracks_enough_resources_at')
		},
		docks:{
			level:GM_getValue('gtb_docks_level'),
         era:GM_getValue('gtb_docks_enough_resources_at')
		},
		farm:{
			level:GM_getValue('gtb_farm_level'),
         era:GM_getValue('gtb_farm_enough_resources_at')
		},
		hide:{
			level:GM_getValue('gtb_hide_level'),
         era:GM_getValue('gtb_hide_enough_resources_at')
		},
		ironer:{
			level:GM_getValue('gtb_ironer_level'),
         era:GM_getValue('gtb_ironer_enough_resources_at')
		},
		lumber:{
			level:GM_getValue('gtb_lumber_level'),
         era:GM_getValue('gtb_lumber_enough_resources_at')
		},
		market:{
			level:GM_getValue('gtb_market_level'),
         era:GM_getValue('gtb_market_enough_resources_at')
		},
		place:{
			level:GM_getValue('gtb_place_level'),
         era:GM_getValue('gtb_place_enough_resources_at')
		},
		stoner:{
			level:GM_getValue('gtb_stoner_level'),
         era:GM_getValue('gtb_stoner_enough_resources_at')
		},
		storage:{
			level:GM_getValue('gtb_storage_level'),
         era:GM_getValue('gtb_storage_enough_resources_at')
		},
		temple:{
			level:GM_getValue('gtb_temple_level'),
         era:GM_getValue('gtb_temple_enough_resources_at')
		},
		wall:{
			level:GM_getValue('gtb_wall_level'),
         era:GM_getValue('gtb_wall_enough_resources_at')
		}
	}
}
...
$(document).ready(function(){
	//General stuff
		...
		$('.report_translation').empty().html('<div id="gt_notepad_trigger" style="height:40px;margin:6px;position:relative;top:8px;padding-top:10px;color:#ffcc66;">Notas</div>');
	//Additional ressources info
		$('#header').append('<div id="mini_produccion" style="position:absolute; top:62px; left:15px; font-weight: bold; color: #ffcc66; height:22px; padding-left: 5px;padding-top:2px;"></div>');
      $('#mini_produccion').append('<div style="position:absolute; top: -4px; left: 3px; width: 30px; height: 30px; background: url(/images/game/res/wood.png)"></div>');
      $('#mini_produccion').append('<div style="position:absolute; top: -4px; left: 84px; width: 30px; height: 30px; background: url(/images/game/res/stone.png)"></div>');
      $('#mini_produccion').append('<div style="position:absolute; top: -4px; left: 165px; width: 30px; height: 30px; background: url(/images/game/res/iron.png)"></div>');
      $('#mini_produccion').append('<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-left: 15px; margin-right:3px;padding-top:2px;">'+res['wood-i']+'</div>');
      $('#mini_produccion').append('<div class="gt_res-inner" style="width:64px;height:22px;float:left; margin-left: 15px; margin-right:3px;padding-top:2px;">'+res['stone-i']+'</div>');
      $('#mini_produccion').append('<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-left: 15px;margin-right:3px;padding-top:2px;">'+res['iron-i']+'</div>');
      if(res['fav-i']>0){
        $('#mini_produccion').append('<div class="gt_res-inner" style="width:64px;height:22px;float:left;margin-left: 15px;margin-right:3px;padding-top:2px;">'+res['fav-i']+'</div>');
      }

      $('#res').css({'height':'75px'});
      $('#res').append('<div id="gt_res" style="position:relative;top:55px;text-align:left;height:22px;text-align:center;color:#ffcc66;"></div>');
      $('#gt_res').append('<div id="widget_storage_wood" class="gt_res-inner" style="font-size: 10px; width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;">'+trad.lumber+'</div>');
      $('#gt_res').append('<div id="widget_storage_stone" class="gt_res-inner" style="font-size: 10px;width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;">'+trad.stone+'</div>');
      $('#gt_res').append('<div id="widget_storage_iron" class="gt_res-inner" style="font-size: 10px;width:64px;height:22px;float:left;margin-right:3px;padding-top:2px;">'+trad.iron+'</div>');
      ...
      if(res['fav-i']>0){
          $('#mini_produccion').append('<div style="position:absolute; top: -4px; left: 246px; width: 30px; height: 30px; background: url(/images/game/res/favor.png)"></div>');
      ...
      $('#bar_wrapper').css({'position': 'absolute','left': '800px','top': '-20px'});
	...
	//Navigation
		//General Stuff
			...
			$('#link_ranking').next().append('<span style="color:#FFCC66;"> | </span><a href="http://forum.'+lang+'.grepolis.com" target="_blank">Foro</a>').next().remove();
			var temp=$('#link_report').next().next().html();
			$('#link_report').next().next().remove();
			$('#link_report').next().append('<span style="color:#FFCC66;"> | </span>'+temp);
			$('#links ul').append('<span style="display:block; margin-top:20px;font-size:16px;font-weight:bold;color:#ffcc66;text-decoration:underline;">Grepolis Tools</span>');
			$('#links ul').append('<li><a id="w_pref" style="font-size:10px;font-weight:bold;">Preferencias</a></li><li><a id="w_help" style="font-size:10px;font-weight:bold;">Ayuda</a></li>');
	...
	//Widgets
		//Buildings Widget
      $('#box').append('<div id="widget_buildings" class="gtwidget" style="width:790px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;padding:2px;text-align:left;font-size:10px;"></div>');
         //Links
            $('#widget_buildings').append('<a href="building_place?action=simulator&town_id='+town+'">'+trad.sim+'</a> |');
            $('#widget_buildings').append('<a href="building_main?town_id='+town+'">'+trad.main+'</a> |<a href="building_academy?town_id='+town+'">'+trad.academy+'</a> |');
            $('#widget_buildings').append('<a href="building_barracks?town_id='+town+'">'+trad.barracks+'</a> |<a href="building_docks?town_id='+town+'">'+trad.docks+'</a> |');
            $('#widget_buildings').append('<a href="building_storage?town_id='+town+'">'+trad.storage+'</a> |<a href="building_lumber?town_id='+town+'">'+trad.lumber+'</a> |<a href="building_stoner?town_id='+town+'">'+trad.stone+'</a> |<a href="building_ironer?town_id='+town+'">'+trad.iron+'</a> |');
            $('#widget_buildings').append('<a href="building_temple?town_id='+town+'">'+trad.temple+'</a> |<a href="building_market?town_id='+town+'">'+trad.market+'</a> |');
            $('#widget_buildings').append('<a href="building_wall?town_id='+town+'">'+trad.wall+'</a> |<a href="building_place?town_id='+town+'">'+trad.place+'</a>');
         $('#widget_buildings').css({'position':'absolute','top':'600px','left':'190px','color':'#ffcc66'});
	...
	//Notepad
		...
		$('#gt_notepad').append('<div style="font-size:14px;font-weight:bold;margin-bottom:15px;">Bloc de Notas<a href="#" id="gt_notepad_quit" class="main_tasks_cancel" style="float:right;position:relative;top:0px;left:0px;"></a></div>');
		...
		$('#gt_notepad').append('<a class="button"  href="#" style="float:right;" id="gt_notepad_save"><span class="left"></span><span class="middle">Guardar</span><span class="right"></span><span style="clear:both;"></span></a>');
		...
		$('#gt_notepad').append('<a href="#" style="position:relative;top:3px;width:64px;height:21px;float:right;margin-right:3px;padding-top:2px;background:url(/images/game/layout/bg_resources.png) repeat-x 0px -32px" id="gt_notepad_clear">Borrar</a>');
	...
	//Building Levels
		if(location.href.match('game/index')){
         if (gtBuildings.main.level > 0)
			$('#map_town').append('<div style="top:190px;left:360px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Senado:'+gtBuildings.main.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.main.era+'</span></div>');
			if (gtBuildings.academy.level > 0)
         $('#map_town').append('<div style="top:20px;left:250px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Academia:'+gtBuildings.academy.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.academy.era+'</span></div>');
			if (gtBuildings.barracks.level > 0)
         $('#map_town').append('<div style="top:120px;left:210px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Cuartel:'+gtBuildings.barracks.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.barracks.era+'</span></div>');
			if (gtBuildings.docks.level > 0)
         $('#map_town').append('<div style="top:430px;left:130px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Puerto:'+gtBuildings.docks.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.docks.era+'</span></div>');
			if (gtBuildings.farm.level > 0)
         $('#map_town').append('<div style="top:30px;left:500px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Granja:'+gtBuildings.farm.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.farm.era+'</span></div>');
			if (gtBuildings.hide.level > 0)
         $('#map_town').append('<div style="top:350px;left:360px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Cueva:'+gtBuildings.hide.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.hide.era+'</span></div>');
			if (gtBuildings.ironer.level > 0)
         $('#map_town').append('<div style="top:0px;left:0px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Mina:'+gtBuildings.ironer.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.ironer.era+'</span></div>');
			if (gtBuildings.lumber.level > 0)
         $('#map_town').append('<div style="top:350px;left:490px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Aserradero:'+gtBuildings.lumber.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.lumber.era+'</span></div>');
			if (gtBuildings.market.level > 0)
         $('#map_town').append('<div style="top:230px;left:510px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Mercado:'+gtBuildings.market.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.market.era+'</span></div>');
			if (gtBuildings.stoner.level > 0)
         $('#map_town').append('<div style="top:300px;left:40px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Cantera:'+gtBuildings.stoner.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.stoner.era+'</span></div>');
			if (gtBuildings.storage.level > 0)
         $('#map_town').append('<div style="top:280px;left:320px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Almacén:'+gtBuildings.storage.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.storage.era+'</span></div>');
			if (gtBuildings.temple.level > 0)
         $('#map_town').append('<div style="top:145px;left:10px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Templo:'+gtBuildings.temple.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.temple.era+'</span></div>');
			if (gtBuildings.wall.level > 0)
         $('#map_town').append('<div style="top:250px;left:200px;z-index:10;float:left;position:absolute;background-color:#FFFFFF;border:1px solid;opacity:0.6;" class="gt_bl">Muralla:'+gtBuildings.wall.level+'<br/><span style="font-size: 8px; color: red">'+gtBuildings.wall.era+'</span></div>');
