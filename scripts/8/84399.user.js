...
//
// ==UserScript==
// @name          Madden 11 Team Stats
// @namespace     http://www.mjefferson.net/
// @description   team stats for madden 11 online franchise
// @include       http://*.easports.com/onlinefranchise/individualFranchise*
// @include       http://easports.com/onlinefranchise/individualFranchise*
...
function getFranchiseIdFromURL(url) { 
var m_strOut = new String(url); 
    m_strOut = m_strOut.replace(/.*\/fId\//g, ''); 
    return m_strOut; 
}
...
var franchiseID = getFranchiseIdFromURL(window.location.href);//stripAlphaChars(getURLProperty('prefs'));

var containers = getElementsByClassName("container", "div", document.body);
var valueBeforeFranchise = "552A0001";//"792A0001";
...
if(containers.length > 0) {
	...
	if(td.length > 0) {
		...
		var baseHandleURL = "http://www.easports.com/p/web/a/easportscom/l/en_US/s/e/handles/"+commish+"/platforms/";
		var psURL = baseHandleURL+"PS3/large_avatar";
		GM_xmlhttpRequest({
		    method: 'GET',
		    url: psURL,
...
function determineSystem(xhr) {
	...
	baseURL = "http://www.easports.com/p/web/a/easportscom/l/en_US/s/g/blaze/"+valueBeforeFranchise+"/franchise/";
