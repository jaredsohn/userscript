...
function download_tag() {
	var tag = tagload();
	var flv = 'http://filmessemlimite.co.cc/watch.php?v=www'+vars.s+'.megavideo.com/files/'+decrypt(vars.un, vars.k1, vars.k2)+'/'+vars.title+'.flv';
	tag.innerHTML = '<a title="Download ('+vars.title+'; '+vars.added+')" class="tag link_tag download_link" href="'+flv+'"><img src="'+icons.download+'" width="16" height="16" />Watch with no limits (Assistir sem Limites)</a>';
}
function size_tag() {
	var tag = tagload();
