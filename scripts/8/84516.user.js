...
// @namespace     Grepolis
// @version        1.0.3
// @description  Grepolis Beyond port
// @license 	 Creative Commons Attribution-Noncommercial-Share Alike 3.0 License
// @include     http://*.grepolis.*/game/*
// @source     http://userscripts.org/scripts/show/84516
// @exclude 	*.css
// @exclude 	*.js
// ==/UserScript==
...
//
// Adaptation of Grepolis Beyond script to run on Chrome with tHema
//
// Original script :  http://userscripts.org/scripts/show/67059
//
...
function isEmpty(o){
	return (!o || ((typeof o==='object') && o.length==0));
}
...
function functionMain(e) {
	...
	//var GB = {
		version: '1.0.2',
		source: 'http://userscripts.org/scripts/source/84516.user.js?source',
		url: 'http://userscripts.org/scripts/source/84516.user.js',
		homepage: 'http://userscripts.org/scripts/show/84516',
	...
	if(typeof GB.Game.townId === 'undefined'){ return;}
	...
	" #GB_supportbar {text-align:left;position: absolute;color:#000000;z-index:10;right:0px;top:100px;width:275px;} "+
	...
	" #GB_Notepad{position:absolute;left:1000px;top:500px;z-index:10;}"+
	...
	//change from pc dependent links to user dependent links.
	...
	GB.BOrder = GB_getValue("BOrder"+GB.Game.townId);
	if (isEmpty(GB.BOrder)) { 
	 GB.BOrder = {"main" : [1,0],"storage" : [2,1],"hide" : [3,2],"market" : [4,3],"stoner" : [5,1],"wall" : [6,2],"temple" : [7,3],"farm" : [8,1],"barracks" : [9,2],"academy" : [10,3],"lumber" : [11,1],"ironer" : [12,2],"docks" : [13,3],"place" : [14,0],"statue" : [15,0],"theater" : [16,0],"library" : [17,0],"ligthouse" : [18,0],"tower" : [19,0],"oracle" : [20,0],"trade_office" : [21,0]};
	 GB_setValue("BOrder"+GB.Game.townId,GB.BOrder);
	}
	...
	{	
		...
		alock = $a('',[['href',jsVoid],['style','float: right;']]);
	...
	*/
	function buildLinks(){
		...
		aclose = $a('',[['href',jsVoid],['class','cancel'],['style','float:right;']]);
	...
	function GB_ProdInfo(){
		var wrm = (((GB.Layout.storage_volume-(GB.Layout.resources.wood+GB.Layout.resources.wood_offset))/(GB.Layout.production.wood))*3600);
		var strm = (((GB.Layout.storage_volume-(GB.Layout.resources.stone+GB.Layout.resources.stone_offset))/(GB.Layout.production.stone))*3600);
		var sirm = (((GB.Layout.storage_volume-(GB.Layout.resources.iron+GB.Layout.resources.iron_offset))/(GB.Layout.production.iron))*3600);

		if($g('GB_prodinfo')){
			$g('GB_prodinfo').innerHTML = '<table><tr>'+
			'<td id="GB_wood" >('+GB.Layout.production.wood+',<span id="GB_wood_C" >'+ft(wrm)+'</span>)</td>'+
			'<td id="GB_stone" >('+GB.Layout.production.stone+',<span id="GB_stone_C" >'+ft(strm)+'</span>)</td>'+
			'<td id="GB_silver" >('+GB.Layout.production.iron+',<span id="GB_silver_C" >'+ft(sirm)+'</span>)</td>'+
			'</tr></table>';
		}else{
			prodinfo = $d('<table><tr>'+
			'<td id="GB_wood" >('+GB.Layout.production.wood+',<span id="GB_wood_C" >'+ft(wrm)+'</span>)</td>'+
			'<td id="GB_stone" >('+GB.Layout.production.stone+',<span id="GB_stone_C" >'+ft(strm)+'</span>)</td>'+
			'<td id="GB_silver" >('+GB.Layout.production.iron+',<span id="GB_silver_C" >'+ft(sirm)+'</span>)</td>'+
			'</tr></table>',[['id','GB_prodinfo']]);
			
			$g("res").appendChild(prodinfo);
		}
		GB.counters["GB_wood_C"] =  ["GB_wood_C",wrm];
		GB.counters["GB_stone_C"] =  ["GB_stone_C",strm];
		GB.counters["GB_silver_C"] =  ["GB_silver_C",sirm];
	}
	GB_ProdInfo();
	...
	function GB_Validate()
	{
		if(GB.Validate[0] == 'attack'){
			...
			GM_setValue("MovementsN"+GB.Game.townId,GB.MovementsN);
			GB_Movements();
		}
		GB.Validate = [];
	}
	...
	if(GB.Game.controller == "map"){	
		...
		uW.TownInfo.sendUnits = function(info,type){
			...
			if(GB.Debug) { $g('GB_Update').innerHTML = info +  " | " + type + " | " + toSource(params); }
	...
	function GB_Report_UBB_Spy(){
		...
		UBB += "[b] "+L('UNITS') +": [/b]\n";
		if(toSource(units) != "({ })" ) {for(var i in units){ UBB += "[img]" +imageURL+ "units/" + i + "_50x50.png[/img] " + units[i] + " "; }} else { UBB += L('NON')+"\n";}
		UBB += "\n[b] "+L('BUILDINGS') +": [/b]\n";
		if(toSource(buildings) != "({ })" ) {for(var i in buildings){ UBB += "[img]" +imageURL+ "main/" + i.replace("building_","") + "_50x50.png[/img] " + buildings[i] + " "; }} else { UBB += L('NON')+"\n";}
		UBB += "\n[/quote]";
	...
	function SetupOpen(){
        ...
        +    '<tr><td class="game_border_left"/>'
        +    '<td>'
		+    '<div class="GB_Table_1"  id="GB_Setup_Title" >'+L('STPT')+'<a class="cancel" style="float:right;" href="#" id="GB_Setup_C">&nbsp;</a> </div>'
		...
		+    '<tr><td style="text-align:left;" >'+L('LINKC')+':</td><td style="text-align:right;" ><input type="text" onclick="this.select()" name="link" value=\''+toSource(GB.Links)+'\' /></td></tr>'
	...
	function GB_getValue(varname){
		var o = GM_getValue(varname,[]);
		return o;
	};
	function GB_setValue(varname,vardata){
