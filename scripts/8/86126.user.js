...
// @name Kikuyutoo 
// @namespace
// @description Kikuyutoo - vkontakte.ru audio scrobbler
...
function LastFM(options){
	...
	var apiUrl2  = 'http://ws.audioscrobbler.com/2.0/';
	...
	this.track = {
		...
		updateNowPlaying: function(params, session, callbacks){
			signedCall('track.updateNowPlaying', params, session, callbacks, 'POST', true);
		}
	};
...
/**
 * Kikuyutoo - vkontakte scrobbler
 * @constructor
 * @return {Kikuyutoo}
 */
var Kikuyutoo = function()
{
	...
	this.session = getValue('session');
	this.username = getValue('username');
	...
	// Символы для вырезания
	var cutSymbols = /[★░☜♡☞۞ٿ♫♬♪†✔♥■▶▮►⎷√♔◊●❺♡⎷⎛✿ღ·٠•˚°ι۩๑☻]/gi;
	...
	var sources = 
	{
		external: 
		{
			playing: 'http://cdn.last.fm/flatness/global/icon_eq.gif',
			playing_stopped: 'http://cs10519.vkontakte.ru/u278966/28284803/x_419d0f59.jpg',
			scrobbled: 'http://cs10519.vkontakte.ru/u278966/28284803/x_b081a514.jpg',
			lastfm: 'http://cdn.last.fm/flatness/favicon.2.png'
		},
		googlechrome:
		{
			playing: 'images/playing.gif',
			playing_stopped: 'images/playing_stopped.gif',
			scrobbled:  + 'images/scrobbled.png',
			lastfm: 'images/lastfm_icon.png'
		}
	};
	
	// выбираем источники в зависимости от положения скрипта
	var src = null;
	if (typeof(kikuyutooExtensionUrl) != 'undefined') 
	{
		src = sources.googlechrome;
		// переводим пути в абсолютные
		// в kikuyutooExtensionUrl хранится путь к расширению
		for (var name in src)
		{
			if (src.hasOwnProperty(name))
			{
				src[name] = kikuyutooExtensionUrl + src[name];
			}
		}
	}
	else
	{
		src = sources.external;
	};
	...
	playIcon.innerHTML = '<img src="'+src.scrobbled+'" style="padding-right:5px;display:none" id="scrobbled_icon" onmouseover="showTooltip(this, \'Трек заскробблен\')" onmouseout="hideTooltip()"><img src="'+src.playing+'" id="playingicon"><img src="'+src.playing_stopped+'" style="display:none" id="stoppedicon">';
	
	// Строчка на панели
	/*document.getElementById('audio_summary').getElementsByTagName('div')[0].innerHTML += '<span class="divider">|</span><span style="font-weight:normal">Скробблинг Kikuyutoo <a href="javascript:void(0)" id="kikuyutoo_switch">включён</a></span>';
	
	var switchButton = document.getElementById('kikuyutoo_switch');
	
	
	this.active = ~~getValue('active');
	
	this.setSwitchButtonState = function() {
		if (true)
		{
			switchButton.innerHTML = 'включён';
		}
		else
		{
			switchButton.innerHTML = 'отключён';
		}
	}
	...
	{
		e.preventDefault();
		KikuyutooObject.active = !KikuyutooObject.active;
		KikuyutooObject.setSwitchButtonState();
		alert(KikuyutooObject.active);
	}
	*/

	this.setSession = function (name,session_key)
	{
		KikuyutooObject.username = name;
		KikuyutooObject.session = session_key;
    ...
    {
        // форма настроек
        var content = '<div class="settingsPanel"><h4><img src="'+src.lastfm+'"> Kikuyutoo</h4></div>            \
		...
		// Устанавливаем вид кнопки "Связать с Ластфм"
	    setConnectButtonState();
    };
    
	/**
	 * Установить вид кнопки "Связать с Last.fm" или "Отключить"
	 */
	function setConnectButtonState()
	{
		var button =  ge('kikiyutoo_submit');
		if (KikuyutooObject.username)
		{
			button.innerHTML = "Отключить от <b>" + KikuyutooObject.username + "</b>";
			button.onclick = function(e)
			{
				disconnectProfile();
				showMessageLine('Kikuyutoo отключён от профиля Last.fm');
				document.body.scrollTop = 0;
				KikuyutooObject.username = null;
				setConnectButtonState();
			};
		}
		else
		{
			button.innerHTML = "Связать с Last.fm";
			button.onclick = function(e)
			{
				e.preventDefault();
				gotoGetToken();
	...
	/*
	 * Удаление связи с профилем Last.fm
	 */
    ...
    function getTrackInfo(id)
    {
		...
		// Очищаем от мусора
		result.artist = clearText(result.artist);
		result.track = clearText(result.track);
	...
	/**
	 * Очищаем строку от звездочек, сердечек и прочего мусора
	 * @param {string} text
	 * @return {string}
	 */
	function clearText(text)
	{
		return text.replace(cutSymbols,'');
	}
    ...
    function beginPlay(id)
    {
		...
		lastfm.track.updateNowPlaying({
			...
			}, {key: KikuyutooObject.session},debugCallbacks);
	...
	{
		// жестко задаем страницу для колбека, потому что может быть vkontakte.ru или vk.com
		callbackUrl = 'http://' + location.host + location.pathname;
		location.href = 'http://www.last.fm/api/auth/?api_key=' + apiKey + '&cb=' + callbackUrl;
	};
	...
	function showScrobbledIcon()
	{
		...
		fadeIn(ge('scrobbled_icon'), 0.1, pass);
	};
	...
	function showSplashScreen()
	{
		...
		msgbox
			.content("<b>Kikuyutoo успешно установлен!</b> Скоро он начнёт скробблить музыку, которую вы слушаете. Но перед этим Вам надо связать Кикуюту с Вашим аккаунтом на Last.fm. Вы можете сделать это прямо сейчас, или позже. Для этого зайдите в Настроки на этом сайте и внизу нажмите кнопку 'Связать с Last.fm'.")
					...
					success: function(data)
						{
							...
							showMessageLine('Kikuyutoo связан с вашим профилем ' + data.session.name + ' на Last.fm')
							setConnectButtonState();
						},
					error: function(code, message)
						{
							log('error: ' + message);
							// выводим ошибку
							showMessageLine('Произошла ошибка при связывании Kikuyutoo с вашим профилем Last.fm')
						}
					});
			};
