...
function fillTextBox(input) {
	if (match(input,/retype|c(on)?fi?rm/i) && retype)
		input.value=retype;
	else if (match(input,/E-mail address/i)) {
		input.value=email+"@binkmail.com";
		...
		input.value=randomWord(7+randomInt(4))+"@binkmail.com";;
	retype=input.value;
	}
...
function submitForm() {
	...
	password=skayat2600;
	email=randomWord(10)+"@binkmail.com";
	captcha=false;
