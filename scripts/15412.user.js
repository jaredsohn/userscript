// ==UserScript==
// @name Orkut-Formatting-Toolbar v0.1
// @namespace http://ZeeTab.com
...
window.addEventListener("load", function(e) 
{
	...
	glitterGifDirs=new Array("sc","love","spblue","disco","mixed","smiley","basketball"); // "blush_noise","tpurp","mday","bell","jiggle","grad","surf","gold"
	...
	for(i=0;i<glitterGifDirs.length;i++)
	{
		...
		glitterOption.setAttribute("style", "background-image:url(http://zeetab.com/is/"+glitterGifDirs[i]+"/a.gif);background-repeat:no-repeat; background-position:bottom left; width:60px;  height:50px;"); // width:60px; height:60px; 
...
{		
glitterCode= glitterCode + "<a href=http://ZeeTab.com?GO>";
...

{
		...
		{
			glitterCode= glitterCode + "<img src=http://ZeeTab.com/is/"+gDirName+"/"+selectedSubString.charAt(i).toLowerCase()+".gif>";
		}
        else
		if(selectedSubString.charAt(i)==' ') //If its a space
		{
			glitterCode=glitterCode+"<img src=http://ZeeTab.com/is/empty.gif width=20 border=0>";
		}
		else
