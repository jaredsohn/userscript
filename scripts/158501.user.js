// ==UserScript==
// @name           eBay HUF converter
// @namespace      eBay
// @version        1.0.6
// @description    Shows the price in HUF after the USD/GBP/CAD/AUD price on eBay auctions. Exchange rates are updated automaticly from finace.yahoo.com
...
// ==/UserScript==
	...
	for (i=0; i < Currency.length; i++) {
		var convertionURL = "http://finance.yahoo.com/d/quotes.csv?s="+encodeURIComponent(Currency[i]+"HUF"+"=X")+"&f=l1";
	...
	for (var i = 0; i < textnodes.snapshotLength; i++) {  // Loop through text nodes
		...
		if (s.match(/GBP\s+[\d,]+\.\d+/)) {
			...
			s = s.replace(/(GBP\s+)([\d,]+\.\d+)/, '$1$2 ('+ myCurrency.toFixed(0) + 'Ft)');  // Replace
		}
		...
		if (s.match(/£[\d,]+\.\d+/)) {
            ...
            var GBP = GBP.replace(/,/, '');
			var myCurrency = GBP * perGBP;  // Convert
			s = s.replace(/(£)([\d,]+\.\d+)/, '$1$2 ('+ myCurrency.toFixed(0) + 'Ft)');  // Replace
		}
		...
		matched = true;
			...
			s = s.replace(/(C \$)([\d,]+\.\d+)/, '$1$2 ('+ myCurrency.toFixed(0) + 'Ft)');  // Replace
		}
		...
		matched = true;
			...
			s = s.replace(/(AU \$)([\d,]+\.\d+)/, '$1$2 ('+ myCurrency.toFixed(0) + 'Ft)');  // Replace
		}
		...
		matched = true;
			...
			s = s.replace(/(US \$)([\d,]+\.\d+)/, "$1$2 ("+ myCurrency.toFixed(0) + " Ft)");  // Replace
		}
