// ==UserScript==
// @name           Create Might of many invites.
// @namespace      Might of many friends.
// @description    Automatically invites people to your guild that you find on the Battle page. Frisno altered the  the original code  from Andy Calderbanks  "Create mobwars invites" http://userscripts.org/scripts/show/30725
// @include        http://apps.facebook.com/mightofmany/fight/
// @include        http://apps.new.facebook.com/mightofmany/fight/
// @include        http://apps.facebook.com/mightofmany/guild/
// @include        http://apps.new.facebook.com/mightofmany/guild/
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/mightofmany/guild/';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/mightofmany/profile/?user_id=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/mightofmany/guild/do.php',
