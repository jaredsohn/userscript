...
// ==UserScript==
// @name            platypus-hi5-Inbox
// @namespace       Platypus
// @include         http://*hi5.com/friend/mail/displayInbox.do
// @include         http://*hi5.com/friend/mail/displayInbox.do?first*
// @include         http://*hi5.com/friend/mail/displayInbox.do?type=bulk*
// @description     Changes images & usernames in Inbox & Bulk folders to link to the user's profile instead of to message.  Changes "Read Message" to "Reply to Message".  Subject line remains linked to message.  See http://www.hi5.com/friend/group/displayTopic.do?groupId=1021&topicId=15398321
// @date            8 April 2009
...
// Another way to index it is using the DIV number as follows (may be either DIV[4] or DIV[5}):
// document.evaluate('/HTML[1]/BODY[1]/DIV[5]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null)

// was ./DIV[3]/DIV[1]/DIV[1]/DIV[3]/DIV[1]
// now ./DIV[3]/DIV[1]/DIV[1]/DIV[3]/DIV[2] so moved next three commands up to 
//     ./DIV[3]/DIV[1]/DIV[1]/DIV[3]

// Change image to link to user's profile for each message:
do_modify_html_it(window.document,document.evaluate('./DIV[3]/DIV[1]/DIV[1]/DIV[3]', main_div, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue,/LinkImage" href="\/friend\/mail\/displayReadMail.do\?messageId=([0-9]*)&amp;senderId=([0-9]*)&amp;[^"]*"/g,'LinkImage" href="/friend/profile/displayProfile.do?userid=$2"',null);

// Change username to link to user's profile for each message:
do_modify_html_it(window.document,document.evaluate('./DIV[3]/DIV[1]/DIV[1]/DIV[3]', main_div, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue, /LinkName" href="\/friend\/mail\/displayReadMail.do\?messageId=([0-9]*)&amp;senderId=([0-9]*)&amp;[^"]*"/g, 'LinkName" href="/friend/profile/displayProfile.do?userid=$2"',null);

// Add "Reply to Message" link for each message:
do_modify_html_it(window.document,document.evaluate('./DIV[3]/DIV[1]/DIV[1]/DIV[3]', main_div, null, XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue, /displayReadMail.do\?messageId=([0-9]*)&amp;senderId=([0-9]*)&amp;[^"]*">Read Message/g,'displayComposeMail.do?msgId=$1&amp;senderId=$2&amp;toId=$2&amp;mailOp=RE&amp;currentStatus=0">Reply to Message',null);
