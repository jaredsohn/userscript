...
switch(sLang)
{
	...
	text = ["( آرسال القطار ) =)","Invalid attack type!","No cords!","not enough troops! (Troop #","No troop input","Starting","Probably bad cords","No scout units","Done","عدد القوات :","آظافة هجمة جديدة","مسح","آنشاء قطار :","نوع الهجمه :","عادي","تعزيز","نهب","تجسس","Scout Def/Troops",""," آرسال القطار","Arrival time","Arrive at:","Set timed arrival","تم التعريب بواسطة : Dynamite_1","  آحداثيات القريه : 4|5 ","ERROR:","Unable to get active village. Assuming one village account","Timed arrival set at:","(الوقت بين الهجمات ) ",""];
	cataText = ["Select a target =)","عشوائي","حطاب","حفرة الطين","منجم حديد","حقل القمح","معمل النشارة","مصنع الطوب","مسبك حديد","المطاحن","مخبز","مخزن","مخزن حبوب","","أفران صهر الحديد","ساحة البطولة","المبنى الرئيسي","نقطة تجمع","السوق","سفارة","ثكنة","الاسطبل","المصانع الحربية","الأكادمية","البلدية","سكن","قصر","المكتب التجاري","ثكنة كبيرة","آسطبل كبير","قصر الأبطال","","","معجزة العالم","Treasury"];
	break;
...
function AttackBuilder(account){
	...
	this.onloadPostSecondPage = function(response) {
		...
		if (_this.countAttacksComplet >= _this.countAttacksPerCoord) {
			...
			addCount(".");
			addCount("<a href='build.php?tt=2&id=39'>"+text[8]+" ("+aMatch[0]+") x " + _this.countAttacksComplet + "("+_this.countAttacksPerCoord+")</a><br/>");
