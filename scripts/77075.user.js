...
{
    return '<p class="smallpadded"><b><h3>LIST '+reset_button_text+'</h3>'+GM_getValue('multi_list')+'<br><h3></h3><h3>Gold

Listed: <font color=red>'+GM_getValue('multi_gold_count')+'</font><br>All-time Gold Listed: <font color=red>'+GM_getValue

('multi_gold_count_all')+'</font></h3></b></p>';
}
...
if(document.getElementsByClassName("avatarholder")[0].innerHTML.split("ERX Terminal").length>1||true)
{
    ...
    if(amt>=0)
    {
        ...
        GM_xmlhttpRequest({
                ...
                else
                {
                    ...
                    alert("Warning: "+name+" has attempted to donate "+amt+" GOLD at level "+level+". Added to

multi suspicion list and crediting canceled.");
                    ...
                    var timestamp = currentTime.getMonth()+1+'/'+currentTime.getDate()

+'/'+currentTime.getFullYear()+' '+hours+':'+minutes;
                    multiList += name+": Level "+level+" attempted donation of "+amt+" GOLD
...
[i].parentNode.previousSibling.previousSibling.previousSibling.previousSibling.innerHTML.split('value="')[1].split('"')[0];
            ...
            {
                elements[i].innerHTML+='<input type="submit" id="'+msgID+'" name="processgold"

onclick="startGoldTransfer(\x27'+name+'\x27,'+amount+','+msgID+');return false;" value="Process Deposit" />';
                //elements[i].innerHTML+='<input type="submit" id="sac" name="setascompleted"

onclick="GM_setValueTIMEOUT('+msgID+',true);return false;" value="Set as Completed" />';
                var button=document.getElementById(msgID);
                //button.onclick = startGoldTransfer(name,amount);
                //button.addEventListener("click", startGoldTransfer(name,amount), false);
                //elements[i].innerHTML+='<a href="javascript:void(0)" OnClick="javascript:startGoldTransfer

(\x27'+name+'\x27,'+amount+','+msgID);return false;">Text to Click</a>';
            }
        }
    }   
