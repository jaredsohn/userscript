// ==UserScript==
// @name           Google search on site
// @namespace      koshiua.userscripts.org
// @description    Add an ability to invoke a 'google site search' form on every page. It allows you to do a custom site search restricted by current site domain (and optionally a site path).
// @include        *
// ==/UserScript==

// We should add @exclude http://www.google.tld/ but without it
// a script handling will be faster. And you can google itself ;-)

//*NOTE* Use hotkey ctrl+shift+9 to invoke the search form. Hide it by pressing escape within the form. Press Enter in input field or click on 'Google' button or 'on' text to do search (hold Shift key if you wish to open search in a new tab).
...
function fillBox(framebox) {
  ...
  appendHTML(bodybox, '<div id="searchBtnBox"><input id="searchButton" type="button" value="Google" /></div>');
...
function invokeWatcher(ev) {
  ...
  if (ev.keyCode == 57 && ev.ctrlKey && ! ev.altKey && ev.shiftKey) {
    ev.preventDefault();
    showBox();
  }
}
...
function go(event) {
  ...
  //alert('Go ' + request);
  request = 'http://www.google.com/search?q='+ request;
  if (event.shiftKey)
    openInTab(request);
  else
