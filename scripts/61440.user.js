...
// @copyright      2009-2013, Scott Royalty
...
window.addEventListener("load", findVotes, false);

function findVotes(e) {
	if (running)
	{
		return;
	}
	...
	// First we check for the No more votes div. If that exists, we exit since nothing to process.
	var requestsCheck = document.evaluate("//div[@id='noRequestsLeft']", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null); 
	if ( requestsCheck.snapshotLength > 0 )
	{
		return;
	}
