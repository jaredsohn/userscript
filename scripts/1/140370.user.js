// ==UserScript==
// @name           Ignore User
// @namespace      Ignore User
// @description    Allows a user to ignore other forum users.
...
function blacklist() {
	...
	//Set an arbitrary upper limit to prevent too many ignored users from causing memory issues in firefox.
	this.upperLimit = 1200;
}
...
blacklist.prototype.add = function (userID, username) {
	//Make sure we aren't adding too many settings because we don't want to slow down firefox
...
function user_post(ele) {
	...
	this.userID = this.userInfo.find('img[src^="http://www.naruto-arena.com/images/avatars/uploaded/"]');
	...
	this.username = this.userInfo.find('a[href^="http://naruto-boards.com/profile/"]').text();
	...
	if (this.userInfo.filter(':contains("Webmaster")').length > 0) {
		infoHTML = infoHTML + '<span style="color:red; font-weight:bold;">Webmaster</span><br>';
	}
	else if (this.userInfo.filter(':contains("Administrator")').length > 0) {
		infoHTML = infoHTML + '<span style="color:red; font-weight:bold;">Administrator</span>';
	}
	else if (this.userInfo.filter(':contains("Global Moderator")').length > 0) {
		infoHTML = infoHTML + '<span style="color:red; font-weight:bold;">Global Moderator</span><br>';
	}
	else if (this.userInfo.filter(':contains("Moderator")').length > 0) {
		infoHTML = infoHTML + '<span style="color:red; font-weight:bold;">Moderator</span><br>';
	}
	else if (this.userInfo.filter(':contains("Member")').length > 0) {
		infoHTML = infoHTML + '<span style="color:black; font-weight:bold;">Member</span>';
	}
...
user_post.prototype.check_mod = function () {
	...
	if (this.userInfo.filter(':contains("Moderator")').length > 0 || this.userInfo.filter(':contains("Administrator")').length > 0) {
