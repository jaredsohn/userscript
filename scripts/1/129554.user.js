// ==UserScript==
// @name           LoU Mail App PL
// @description    UI Extension for LordOfUltima. Polish version translated for www.lordofultima.tk
...
qx.Class.define("MailApp",{
	extend:webfrontend.gui.CustomWindow,construct:function(){
		webfrontend.gui.CustomWindow.call(this);
		this.ErrorReporting();
		this.set({width:1000,height:500,allowMaximize:false,allowMinimize:false,resizable:true,caption:"Mail App v1.5 Polish version translated for www.lordofultima.tk",showMaximize:false,showMinimize:false,showStatusbar:false,showClose:false,contentPadding:0,useMoveFrame:true});
	...
	// Load config options
		this.doptions = {
			"height": 500,
			"width" : qx.bom.Viewport.getWidth(window)-220,
			"x" : 410,
			"y" : 100,
		};
	...
	// Start the interface
		...
		{this.____jF=[];
		this.____jF["@In"]="Odebrane";
		this.____jF["@Out"]="Wyslane";
		...
		this.___jH.setColumns(["","Temat","Z","Odebrane"],["c","s","f","d"]);
		this.___jH.sortByColumn(3,false);
		this.___jI=new webfrontend.gui.CustomTable(this.___jH).set({columnVisibilityButtonVisible:false,statusBarVisible:false,width:420});
		...
		this.___jM = 0;
		this.___jN = 0;
		this.___jJ=new qx.ui.form.CheckBox("Zaznacz wszystko").set({font:"bold"});
		this.add(this.___jJ,{left:5,bottom:14});
		this.___jJ.addListener("click",this.___jP,this);
		this.___jK=new qx.ui.form.Button("Usun").set({width: 75, appearance: "button-text-small",enabled:false});
		this.___jK.addListener("click",this.___jT,this);
		this.add(this.___jK,{bottom:12,left:90});
		this.___jL=new qx.ui.form.Button("Zaznacz nieprzeczytane").set({width: 75, appearance: "button-text-small",enabled:false});
	...
	// Hijack some hooks so LOU uses our mail client
		...
		var PI = qx.core.Init.getApplication().getPlayerInfoPage();
		PI.sendEmailButton.removeListener("execute",PI._onSendEmail,PI);
		PI.sendEmailButton.addListener("click",this._onSendEmail,this);
	...
	members:{ error:null,___jO:0,___jM:0,___jN:0,
		loadOptions: function() {
			...
			if (_str) {
				...
				var str = qx.util.Json.stringify(this.options);
		...
		___jQ:function(r,bq,br){
			...
			if(bq!=0){
				webfrontend.gui.MessageBox.messageBox({title:"Usuanie: Blad",text:"Wystapil blad podczas usuwania wiadomosci",buttons:1});
			}
		},
		___jR:function(r,bt,bu){
			...
			this.__kc();
			if(!r){
				webfrontend.gui.MessageBox.messageBox({title:"Zaznaczanie nieprzeczytanych: Blad",text:"Wystapil blad podczas zaznaczania nieprzeczytanych wiadomosci",buttons:1});
			}
		},
		___jS:function(e) {
		...
		___jV:function(e){
			...
			if(bA[0].getUserData("name")=="@Out"){
				this.___jDD=false;
				this.___jH.setColumns(["","Temat","Do","Wyslana"],["c","s","to","d"]);
				this.___jH.setDirection(false);
			}else if(!this.___jDD){
				this.___jDD=true;
				this.___jH.setColumns(["","Temat","Do","Odebrana"],["c","s","f","d"]);
		...
		___jW:function(r,bB,bC){
			if(r==false){
				webfrontend.gui.MessageBox.messageBox({title:"Blad polaczenia",text:"Sprobuj ponownie",buttons:1});
				return;
			}
			if(bB==null||bC!=this.___jO){
				return;
			}
			var bG=new qx.ui.tree.TreeFolder("Skrzynka").set({icon:"webfrontend/ui/icons/icon_mail_folder_open.png"});
			bG.setOpen(true);
		...
		__Tt:function(cC, cD){
			...
			switch(cD){
				case "0":
					var cF=qx.bom.String.escape(qx.lang.String.trim(this.tabPages[ii].page.to.getValue()));
					break;
				case "1":
					var cF=qx.bom.String.escape(qx.lang.String.trim(this.tabPages[ii].page.cc.getValue()));
					break;
			}
			var cG="";
			if(cF.indexOf(";") != -1){
				...
				var cH = cE.length;
				for(var i = 0; i < cH; i++){
					if(qx.lang.String.trim(cE[i]) == cC){
						return;
					}
				}
			...
			switch(cD){
				case "0":
					this.tabPages[ii].page.to.setValue(this.tabPages[ii].page.to.getValue()+cG);
					break;
				case "1":
					this.tabPages[ii].page.cc.setValue(this.tabPages[ii].page.cc.getValue()+cG);
					break;
			}
		},
		__Tv:function(cI){
			switch(cI.type){
				case 0:
					this.__Tt(cI.name, cI.fieldType);
					break;
				case 1:{
					var cL=webfrontend.data.Alliance.getInstance().getOfficers();
					var cK=cL.length;

					for(var i=0;i<cK;i++){
						this.__Tt(cL[i].n,cI.fieldType);
					}
					};
				case 2:{
		...
		_onSendEmail:function(e) {
			this.createCompose(qx.core.Init.getApplication().getPlayerInfoPage().nameLabel.getValue(), 0);
			if (!this.isVisible()) {
				this.open();
			}
		...
		createTab:function(bw) {
			...
			lab = new qx.ui.basic.Label("Temat").set({paddingTop: 4, maxWidth:50});
			cont.add(lab);
			...
			// Date
			cont = new qx.ui.container.Composite(new qx.ui.layout.HBox());
			lab = new qx.ui.basic.Label("Data:").set({paddingTop: 0});
			cont.add(lab);
			...
			// From
			cont = new qx.ui.container.Composite(new qx.ui.layout.HBox());
			lab = new qx.ui.basic.Label("Od:").set({paddingTop: 0});
			cont.add(lab);
			cont.add(new qx.ui.core.Spacer(18));
			...
			// To
			cont = new qx.ui.container.Composite(new qx.ui.layout.HBox());
			lab = new qx.ui.basic.Label("Do:").set({paddingTop: 0});
			...
			// CC
			cont = new qx.ui.container.Composite(new qx.ui.layout.HBox());
			lab = new qx.ui.basic.Label("Kopia do:").set({paddingTop: 0, paddingBottom: 20});
			cont.add(lab);
			cont.add(new qx.ui.core.Spacer(30));
			...
			// Buttons
			var ReplyBtn = new qx.ui.form.Button("Odpowiedz");
			ReplyBtn.set({width: 70, appearance: "button-text-small"});
			ReplyBtn.addListener("click", this.Reply, this);
			page.add(ReplyBtn, {top: 0, right: 5});

			var ReplyAllBtn = new qx.ui.form.Button("Wszyscy");
			ReplyAllBtn.set({width: 70, appearance: "button-text-small"});
			ReplyAllBtn.addListener("click", this.ReplyAll, this);
			page.add(ReplyAllBtn, {top: 18, right: 5});

			var FwdBtn = new qx.ui.form.Button("Dalej");
			...
			var DelBtn = new qx.ui.form.Button("Usun");
			...
			var CloseBtn = new qx.ui.form.Button("Zamknij");
		...
		createCompose:function(page, type) {
			...
			this.__Tr=new webfrontend.gui.MailContacts(this.__Tv,this);
			this.receiver = "";
			this.cc = "";
			...
			// To
			cont = new qx.ui.container.Composite(new qx.ui.layout.HBox());
			var lab = new qx.ui.form.Button("Do");
			...
			var availcharsLbl=new qx.ui.basic.Label("Dostepne znaki:");
			...
			// Buttons
			var SendBtn = new qx.ui.form.Button("Wyslij");
		...
		addButton:function() {
			var MailButton = new qx.ui.form.Button("Poczta");
