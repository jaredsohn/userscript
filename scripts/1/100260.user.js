...
// @description      KoC Attack with change made to see the links to start/stop the script.
...
var KOCAttack={
	...
	ShowOptionsDialog:function() {
		...
		for(var c=1; c<=8; c++) {
			...
			var SetCity=function(target,set) {
				target.style.border=set?'2px solid #FD5501':'';
	...
	*/
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>Attack - On</span>';
		} else {
			toggle.innerHTML='<span>Attack - Off</span>';
		}
	},
