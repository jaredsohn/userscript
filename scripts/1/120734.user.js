// ==UserScript==
// @name	ModifictionOfGuessXDroneScript
// @description Pause 5 seconds to collect level up bonus small mod to GuessX Drone script
// ==/UserScript==

javascript:(function(){
	/*Energy/Stamina/End/Options Buttons*/
	...
	/*Intervals to repeat things*/
	var JobberInterval;
	var FightLVInterval;
	var onoffInterval;
	var MeepInterval;	
	/*Alllows you to pause the Drone*/
	var DroneOnOff = true;
	/*All Radio Buttons*/
	...
	var StaAlways = '';
	var SCCLUB = '';
	/*Manual User Inputs which Save to cookie*/
	var ratiochan = 2.21;
	var whotokill = 41236137
	var attemat = 777;
	var healmeat = 500;
	var rref = 7000;
	var rjref = 1000;
	...
	var opponentLevelMax = 501;
	var opponentMafiaMax = 25000;	
	/*Picks which city you want to fight in, otherwise will autodefault you to Brazil*/
	var FCityC = false;
	var FightInCityNum = 7;
	/*Stuff to do with Ratios & there changings*/
	var weratioing = false;
	var weratioingwitfobs = false;
	var tulos;
	var NeededRatio = 0;
	/*This is me using counting as a replacement for poor coding*/
	var cheater = 0;
	var Meep = 0;
	var doireallynothaveenorstam = 0;
	var cheeta = 0;
	/*Sets your Energy & Stamina Requirements*/
	var howmuchy = 321;	
	var stamy = 120;
	/*Who you end up fighting*/
	var targetid;
	/*Turns Robbing on/off & sets the current city as an option*/
	var RobOn;
	var WhereAmI = HereYouIs();
	var RobWhere = getCity();
	/*For Fighting and older cities*/
    var xw_user = /sf_xw_user_id': '(.+)'/.exec(document.body.innerHTML)[1];
	var xw_user = User.id;
	var xw_sig = local_xw_sig;	
	var startkilling;	
	/*Like My Facebook Group & My background pic*/
    ...
    function writeCookie() {
        var a = opponentLevelMax + "/" + opponentMafiaMax + "/" + rref + "/" + rjref + "/" + plref + "/" + restartref + "/" + robslowdown + "/" + healmeat + "/" + attemat + "/" + ratiochan + "/" + whotokill;
        createCookie("GXDrone", a) 
    } 
	
	function readCookieSettings() {
        try {
            var a = readCookie("GXDrone");
            if (a == null || (/undefined/.test(a)) || a.length < 12) {		
				opponentLevelMax = 501;
				opponentMafiaMax = 25000;
				rref = 7000;
				rjref = 1000;	
				plref = 4000;
				restartref = 1800000;
            ...
            a = a.split("/");
				opponentLevelMax = a[0] || 501;
				opponentMafiaMax = a[1] || 25000;
				rref = a[2] || 7000;
				rjref = a[3] || 1000;	
				plref = a[4] || 4000;
				restartref = a[5] || 1800000;
				robslowdown = a[6] || 750;
				healmeat = a[7] || 500;
				attemat = a[8] || 777;
				ratiochan = a[9] || 2.21;
				whotokill = a[10] || 41236137;
		} catch (e) {}
    }
	...
	/*Removes Zynga Stuff, Banners etc*/
	function rzc() {
		try {
	...
	/*Creates the Onscreen Display for script configeration*/
	function create_div() {
	...
	/*Reads the Cookie we baked and sets some numbers*/
	readCookieSettings();		

	/*Knows what to create for the OSD, When requested*/
	var version='<a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw">Drone.&#12324;</a> - <a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw/drone.html">FAQ</a> - <a TARGET="_blank" href="http://www.creallaborate.com/guessx/mw/donate.html">Donate</a> - v2.88';
	...
	var config_html =
		...
		'<span id="fobz-row" style=""><table width="99%"><tbody><tr><td>Mafia Size&nbsp;<input type="integer" value="'+opponentMafiaMax+'" style="resize:none;width:40px;" id="postformid12">&nbsp;&nbsp;Mafia Level&nbsp;<input type="integer" id="postformid13" style="resize:none;width:25px;" value="'+opponentLevelMax+'">&nbsp;</td></tr></tbody></table></span>'+
		'<span id="fightrobz-row" style="display:none">'+		
		'<td>Rob untill stamina ratio is under &nbsp;<input type="integer" value="'+ratiochan+'" style="resize:none;width:39px;" id="postformid8">&nbsp;&nbsp;Then fight Target p|&nbsp;<input type="integer" value="'+whotokill+'" style="resize:none;width:65px;" id="postformid9">&nbsp;&nbsp;in&nbsp;<a class="sexy_button_new short green black_white_border sexy_travel_new" id="FICN" href="#"><span><span>Brazil</span></span></a><BR>Heal threshold is&nbsp;<input type="integer" value="'+healmeat+'" style="resize:none;width:35px;" id="postformid6"></td>'+
		'</span>'+		
		'<span id="robzNfobz-row" style="display:none">'+
		'<table width="99%">'+
		'<tr>'+
		'<td>Rob untill stamina ratio is under &nbsp;<input type="integer" value="'+ratiochan+'" style="resize:none;width:39px;" id="postformid15">&nbsp;&nbsp;then do Fob. Mafia Size&nbsp;<input type="integer" value="'+opponentMafiaMax+'" style="resize:none;width:40px;" id="postformid16">&nbsp;&nbsp;Mafia Level&nbsp;<input type="integer" id="postformid17" style="resize:none;width:25px;" value="'+opponentLevelMax+'">		</td>'+
		'</tr>'+ 
		'</table>'+
		'</span>'+
		...
		'<table width="99%">'+
		'<tr>'+
		'<td>Target is p|&nbsp;<input value="'+whotokill+'" type="integer" style= "resize:none;width:65px;" id="postformid14">&nbsp;&nbsp;Heal threshold&nbsp;<input type="integer" id="postformid6" style="resize:none;width:40px;" value="'+healmeat+'">&nbsp;Fight in <a href="#" id="FICN" class="sexy_button_new short green black_white_border sexy_travel_new"><span><span>Brazil</span></span></a></td>'+
		'</tr>'+
		'</table>'+
		'</span>'+
		'<p>When im out of stamina &amp; energy: <a href="#" id="whatfinish" class="sexy_button_new short black"><span><span>' + finish_text + '</span></span></a></p>Use Energy whenever you have enough? <input id="EnAlways" type="radio" checked="CHECKED" name="zoomy" '+EnAlways+'/><br>Use Stamina whenever you have enough? <input id="StaAlways" type="radio" name="zoomy" '+StaAlways+'/><br>Neither, Just go toe to toe! <input id="SCCLUB" type="radio" name="zoomy" '+SCCLUB+'/></div>'+
		...
		'</tr>'+
		'<tr>'+/**/
		'<td><input type="radio" name="zoom" id="energy_apply">Skill points to Energy.</td><td>Rob Individual Speed&nbsp;<input value="'+robslowdown+'" type="integer" style= "resize:none;width:35px;" id="postformid5"> milli seconds.</td>'+
		'</tr>'+
		'<tr>'+
		'<td><input type="radio" name="zoom" id="stam_apply">Skill points to Stamina.</td>'+
		'<td>Attack Speed&nbsp;<input type="integer" id="postformid7" style="resize:none;width:35px;" value="'+attemat+'"> milli seconds.</td>'+
		'</tr>'+
		'</table></center>'+
		'</span><br>'+	
	...
	/*Energy/Stamina or just continue what it is doing till it switches or runs out of Attribute*/
	...
	document.getElementById('SCCLUB').onclick=function() {
		EnAlways = '';
		StaAlways = '';
	};
	
	/*Applys Skillpoints, Default is turned off and this is not saved in cookie, SO ALWAYS TURNED OFF! in order, None/Att/Def/Health/Energy/Stamina*/
	...
	/*Sets your Energy/Stamina & finishing options here*/
	...
	document.getElementById("FobOrRob").onclick = function () {  
		if (RobOrFoB == true){
            ...
            RobOrFoB = 7;
            weratioing = false;                    
            weratioingwitfobs = true;
            stamy = 50;
            document.getElementById("robzNfobz-row").style.display = ''
            document.getElementById("fightrobz-row").style.display = 'none'	
            document.getElementById("FobOrRob").setAttribute("class", "sexy_button_new short red sexy_attack_new impulse_buy");
            jobbing_text = document.getElementById("FobOrRob").innerHTML = "<span><span>Rob & Fob</span></span>";
		} else if (RobOrFoB == 7) {
            RobOrFoB = false;
            RobOn = false;
            weratioingwitfobs = false;            
            stamy = 120;
            document.getElementById("robzNfobz-row").style.display = 'none'            	
	...
	/*Ends your Energy/Stamina & finishing options*/
...
while (new Date() < ms){}
	...
	/*Spends Skillpoints if the radio button has been checked*/
	function skillspender(){
	...
	/*Upon User clicking Start this gathers values*/
	function GoTime() {
		Ratio();
		try {
			...
			if(RobOrFoB == 6){
				ratiochan = parseInt(document.getElementById("postformid15").value);
			}
			whotokill = parseInt(document.getElementById("postformid9").value);
			if(RobOrFoB == 5){
				whotokill = parseInt(document.getElementById("postformid14").value);
			}
			targetid = "p|" + whotokill;
			opponentMafiaMax = parseInt(document.getElementById("postformid12").value);
			if(RobOrFoB == 6){
				opponentMafiaMax = parseInt(document.getElementById("postformid16").value);
			}
			opponentLevelMax = parseInt(document.getElementById("postformid13").value);
			if(RobOrFoB == 6){
				opponentLevelMax = parseInt(document.getElementById("postformid17").value);
			}			
		writeCookie()
		}catch (err){}
	...
	/*Sets Ratios and is updated everytime*/
	...
	/*Places the Stop/Pause/Go button in top Right corner*/
	function offon() {
	...
	function DoAJob() {
		...
		if ((StaAlways == 'checked') && (DoesIHaveStamina > stamy)) {
			...
			Ratio();
			if (whichjob == true) {
				var clickMe = document.getElementById("btn_dojob_95"); 
	...
	function healmenow(){
		do_ajax('', 'remote/html_server.php?xw_controller=hospital&xw_action=heal&xw_city=8&xcity=1&xw_client_id=8');
	}		
	/*End Fight Code*/
    ...
    function load() {
        try
            ...
            pageTracker._trackPageview("/dronev2")
        }
        catch(err)
            {
        }
