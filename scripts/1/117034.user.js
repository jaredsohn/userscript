...
$(function() {
	...
	if (lang == 'zh-CN') {
		var follow_text = '您关注的对象';
		var shared_text = '共享条目';
		var notes_text = '备注';
		var share_text = '共享';
		var share_note_text = '共享备注';
		var like_text = '喜欢';
		var liked_text = '您喜欢的条目';
		var add_share_text = '添加到共享条目';
		var post_text = '发布备注';
		var posting_text = '发布备注中…';
		var close_text = '关闭';
	...
	var $friends = $('<div id="friends-selector" class="selector"><a href="#stream/user/-/state/com.google/broadcast-friends" class="link"><div class="selector-icon"></div><span class="text">' + follow_text + '</span></a></div>');
	$friends.insertAfter($liked).click(function() {
	...
	function get_item($button) {
		try {
			if ($button) {
				...
				var entry = $entry.length ? $entry[0] : null;
			} else {
	...
	var $like_button = $('<span class="like" title="L"><span class="link unselectable">' + like_text + '</span></span>');
	var $share_button = $('<span class="broadcast" title="Shift+F"><span class="link unselectable">' + share_text + '</span></span>');
	var $share_note_button = $('<span class="broadcast-with-note" title="Shift+D"><span class="link unselectable">' + share_note_text + '</span></span>');
	function show_button() {
			...
			for (key in unsafeWindow) {
				if (key.length == 2) {
					...
					if (obj && typeof(obj) == 'object' && !('length' in obj)) {
						...
						if (keys.length != 1 || Object.keys(obj.__proto__).length != 1) {
							continue;
						}
						key2 = keys[0];
						var obj2 = obj[key2];
						if (obj2 && typeof(obj2) == 'object' && !('length' in obj2) && obj2.__proto__ && 'abort' in obj2.__proto__ && 'reset' in obj2.__proto__) {
							data = obj;
							...
							for (var _key3 in obj2) {
								...
								if (obj3 && typeof(obj3) == 'object' && 'length' in obj3) {
									if (obj3.length) {
										var obj4 = obj3[0];
										if (obj4.__proto__ && obj4.__proto__.__proto__ && 'isReadStateLocked' in obj4.__proto__.__proto__) {
											key3 = _key3;
		...
		var $like_button_copy = $like_button.clone();
		var $share_button_copy = $share_button.clone();

		if (item.liked) {
			$like_button_copy.addClass('liked');
		}
		if (item.shared) {
			$share_button_copy.addClass('shared');
		}
		...
		$like_button_copy.insertBefore('#current-entry .item-plusone').on('click', function() {
			edit_item('like', $like_button_copy);
		});
		$share_button_copy.insertAfter($like_button_copy).on('click', function() {
			edit_item('share', $share_button_copy);
		});
		$share_note_button.clone().insertAfter($share_button_copy).click(function() {
			show_note_dialog($(this));
		})
	}
	var $entries = $('#entries').on('click', '.collapsed', show_button);

	function edit_item(state, $button) {
		...
		try {
			...
			if (change_like) {
				...
				var toggle = function() {
					...
					($button || $('#current-entry .entry-actions>.like')).toggleClass('liked');
				}
			} else {
				post_data[item.shared ? 'r' : 'a'] = 'user/-/state/com.google/broadcast';
				toggle = function() {
					item.shared = !item.shared;
					($button || $('#current-entry .entry-actions>.broadcast')).toggleClass('shared');
				}
			}
	...
	function post_note() {
		...
		try {
			var item = get_item($note_button);
			if (item) {
				options.title = item.g;
				options.content = item.sd;
				options.url = item.Hd.alternate[0].kf;
				options.srcTitle = item.Nb.g;
				options.srcUrl = item.Nb.c.streamId.substring(5);
			}
			add_note(options);
		} catch (e) {
			console.error('Reader Sharer: failed to post note due to:\n', e);
			$post_button.text(post_text);
			posting_note = false;
		}
	}
	...
	function show_note_dialog($button) {
		if ($button) {
			$note_button = $button;
		} else {
			$note_button = null;
	...
	var $post_button = $note_dialog.find('button[name=ok]').click(function() {
		post_note();
	});
...
#title-and-status-holder {
...
#entries.list .entry div.collapsed {
	cursor: pointer !important;
	margin: 0 !important;
	...
	border-width: 0 2px 1px !important;
	border-color: #C2CFF1 !important;
	border-style: solid !important;
}
#entries.list .entry.expanded .entry-container {
	border-width: 0 2px !important;
	border-color: #C2CFF1 !important;
	border-style: solid !important;
}
#entries.list .entry.expanded .entry-actions {
	border-left-width: 2px !important;
	border-right-width: 2px !important;
	border-color: #C2CFF1 !important;
	border-style: solid !important;
}
#entries.list #current-entry .entry-container, #entries.list #current-entry .entry-actions {
...
#entries.list #current-entry .collapsed, #entries.cards #current-entry .card {
