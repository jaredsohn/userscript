...
// @description      На ауте добавляет ссылки: "На себя", "Магазин лицензий". Закрытие заявки. Подбор лута. В FireFox проверяет рюкзак на наличие сломанных вещей и активных синдикатных бох.
...
// мы на ауте
if (root.location.href.indexOf('http://quest.ganjawars.ru/walk') != -1) {
    ...
    load(function() {
        ...
        if (forum) {
            ...
            root.setTimeout(function() {
                ...
                // ссылка на меня
                var span = root.document.createElement('span');
                span.innerHTML = '<a href="http://www.ganjawars.ru/info.php?id='+ getCookie('uid') +'" target="_blank">Ссылка на меня</a> | ';
                forum.parentNode.insertBefore(span, forum);
...
if (s.length) {
    ...
    for (i = 0, l = s[0].length; i < l; i++) {
        ...
        if (s[0].options[i].innerHTML.indexOf('Red point') >= 0) {
