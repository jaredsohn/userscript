...
api.register( 'DS-Alarm', [8.3, 8.4], 'Harpstennah', 'support-nur-im-forum@arcor.de' );
...
function main() {
	...
	// Sound bei Ver?nderungen
	var aDiff = aIncs - aZaehler;
	if (aDiff  > 0){
		if (aSettings.AnzeigeDauer != 0 ){
			win.UI.InfoMessage( aDiff + ' عليك هجووووم جديد', aSettings.AnzeigeDauer, true); // true = rot ; false = grün
			}
		astr.load();
		astr.play(0);
		}
