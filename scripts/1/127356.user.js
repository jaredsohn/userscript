...
function showStats() {
	...
	for (i = 0, len = key.length; i < len; i++) {
		...
		if (tmp[0].length == 1) {
			...
			stats.push('<tr><td>' + tmp[1] + '_'  + tmp[2] + '_' + value[0] + '_' + ccode + '</td><td>' + value[1] + '</td><td><a href="javascript:;" class="delete_stat" id="' + key[i] + '">X</a></td></tr>');
		} else {
			eliteFight += parseInt(value[0], 10);
			eliteInf += parseInt(value[1], 10);
			stats.push('<tr style="background-color:LightGreen;"><td>' + tmp[1] + '_'  + tmp[2] + '_' + value[0] + '_' + ccode + '</td><td>' + value[1] + '</td><td><a href="javascript:;" class="delete_stat" id="' + key[i] + '">X</a></td></tr>');
		}
	}
	if (stats.length == 1) {
		stats = '';
	...
	if (cmcInf == 0 && eliteInf == 0) {
		tstats = '<tr><td style="text-align:center;font-weight:bold;"><a href="http://userscripts.org/scripts/show/127746" target="_blank">ROCAS 申報插件 v1.0.0</a></td></tr>';
	}
		...
		if (r == true) {
			...
			if (cmcs.length > 0 && elites.length > 0) {
				...
				setTimeout(function () {
					GM_xmlhttpRequest({
						method:	'POST',
						url: 'http://seisei.no-ip.org/rocas/addapi.php',
						headers: {
								...
								setTimeout(function () {
									GM_xmlhttpRequest({
										...
										onload: function (response) {
											var data = JSON.parse(response.responseText);
											if (data.message == 'success') {
												$j('#stats_report').html('<span style="color:green;">Submitted. <br/>Check: </span><a href="http://seisei.no-ip.org/rocas/list.php" target="_blank">ROCAS list</a>');
											} else if (data.message == 'error') {
												$j('#stats_report').html('<span style="color:red;">' + data.errmsg + ' Try to submit again or access: <br/></span><a href="http://seisei.no-ip.org/rocas/formElite.php" target="_blank">ROCAS formElite</a>');
											}
										}
									});
								}, 0);
							} else if (data.message == 'error') {
								$j('#stats_report').html('<span style="color:red;">' + data.errmsg + ' Try to submit again or access: <br/></span><a href="http://seisei.no-ip.org/rocas/form.php" target="_blank">ROCAS form</a>');
							}
						}
					});
				}, 0);
				...
				setTimeout(function () {
					GM_xmlhttpRequest({
						...
						onload:	function (response) {
							var data = JSON.parse(response.responseText);
							if (data.message == 'success') {
								$j('#stats_report').html('<span style="color:green;">上傳成功. <br/>網頁確認: </span><a href="http://seisei.no-ip.org/rocas/list.php" target="_blank">ROCAS list</a>');
							} else if (data.message == 'error') {
								$j('#stats_report').html('<span style="color:red;">' + data.errmsg + ' Try to submit again or access: <br/></span><a href="http://seisei.no-ip.org/rocas/form.php" target="_blank">ROCAS form</a>');
							}
						}
