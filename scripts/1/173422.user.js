...
// @namespace   http://vladsukhoy.com/
// @description  Greasemonkey script for encrypting data in Trello.
...
/*
 * IMPORTANT: you need to change the password before using encryption for you own boards.
 * Otherwise, your boards will be encrypted with the default password "password" and you
...
function encrypt(text) {
    var textHash = CryptoJS.SHA3(text, 512);
    var pkg = textHash.toString(CryptoJS.enc.Hex) + text;
    var rv = "AES:" + CryptoJS.AES.encrypt(pkg, password).toString();
    return rv;
}

function decrypt(text) {
    if (text.indexOf('AES:') != 0)
        return null;
    var pkg = text.substring(4);
    var decrypted = CryptoJS.AES.decrypt(pkg, password).toString(CryptoJS.enc.Utf8);
    if (decrypted.length < 128)
        return null;
    var hashHex = decrypted.substring(0, 128);
    var msg = decrypted.substring(128);
    var hash = CryptoJS.SHA3(msg, 512);
    if (hash.toString(CryptoJS.enc.Hex) != hashHex) {
        //alert('failing due to mismatch')
        return null;
    }
    return msg;
}
...
function getMO(fn) {
  if (typeof MutationObserver !== 'undefined')
    ...
    return new WebKitMutationObserver(fn);
  return null;
}

var observer = getMO(function (mutations) {
    mutations.forEach(function(mutation) {
        if (mutation.type == "childList")
            if (mutation.addedNodes !== null) {
                $(mutation.addedNodes).each(function (idx, addedNode) {
                    processNode(addedNode);
                    $(addedNode).find('input').click(function (evt) {
                        $('textarea').each(function (idx, tarea) {
                            if ($(tarea).val()) {
                                $(tarea).val(encrypt($(tarea).val()));
                            }
                        });
                        return true;
                    });
...
function processNode(node) {
    for (;;) {
        var dirty = false;
        getTextNodes($(node)).each(function (idx, textNode) {
            var text = $(textNode).text();
            text = $.trim(text);
            // handle the "completed" list items
            const offset = text.indexOf("AES:");
            if (offset > -1) {
                try {
                    var decryptedText = decrypt(text.substring(offset), password);
                    if (decryptedText !== null) {
                        dirty = true;
                        textNode.nodeValue = text.substring(0, offset) + decryptedText;
                    }
                }
                catch (e) {}
            }
...
window.setInterval(function() { processNode($(document.documentElement)); }, 1000);
console.log('Trello Encryption Started...');
