...
// TODO: Make sure WinPos is visible on-screen ?
 ...
 if(!GlobalOptions.pbWideScreen && Options.pbWinPos.x > 1045){
	 var c = getClientCoords (document.getElementById('main_engagement_tabs'));
	  Options.pbWinPos.x = c.x+4;
	  saveOptions ();
	  }
  ...
  modifyTotalTime: function (cityId, type, buildingTime) {
    ...
    showBuildQueue: function(cityId, focus){
      ...
      clearTimeout (t.timer);
        ...
        var m = '<DIV style="max-height:490px; height:490px; overflow-y:auto"><TABLE align=center cellpadding=0 cellspacing=0 width=100% class="pbTabPad" id="pbCityQueueContent">';       
...
//				13:{'name':"-------",'x':375,'y':375},
	...
	m += '</table></div>\
		<DIV id="pasrcResults" style="height:490px; max-height:490px;"></div>';
    ...
    document.getElementById ('provinceXY').addEventListener ('click', function() {
    	  if (this.value >= 1) {
    		  ...
    		  document.getElementById ('pasrcDist').value = '185';
    	  }
	    }, false); 
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>185)
      errMsg += "Radius (distance) must be between 1 and 185<BR>";
    if (errMsg != ''){
    ...
    m = '<DIV class=pbStat><TABLE width=100% cellspacing=0><TR><TD class=xtab width=125><DIV id=pastatSearched></div></td>\
        ...
        <TD class=xtab align=right width=125><DIV id=pastatFound></div></td></tr></table></div>\
          <TABLE width=100%><TR valign=top>\
            <TD width=99% style="max-width:50px"><DIV id=padivOutTab style="height:490px; max-height:490px; overflow-y:auto;"></div></td>\
...
return 0;
    ...
    tbody.innerHTML = m;
    if (parseInt(tbody.clientHeight) > 490){
      tbody.style.height = '490px';
      tbody.style.maxHeight = '490px';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV style="top:670px; left:0px; position:static; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab><u>Wichtig</u>: Die daten sind vom Leaderboard und können bis zu 24 Stunden alt sein!<BR><u>Hinweis</u>: Auf die Kopfspalte klicken zum Sortierten!</td>\
...
Tabs.wikia = {
  ...
  hide : function (){         // called whenever the main window is hidden, or another tab is selected
    var t = Tabs.wikia;
    mainPop.div.style.width = 1220 + 'px';
  },
