// ==UserScript==
// @name           OGame find player details
// @namespace      -
// @author         Eleria & now benneb
// @description    finds all coordinates from players within the standard ogame search - actuality one week!
// @include        http://*.ogame.*/game/index.php?*page=*
// @version        2.3.8
// @updateURL      http://userscripts.org/scripts/source/164695.meta.js
// @downloadURL    https://userscripts.org/scripts/source/164695.user.js
// ==/UserScript==
		...
		if (settings.url.indexOf('http://' + ogameUniverse + '/game/index.php?page=search') != -1 && settings.type == 'POST') {
			...
			if (method == '2') { // 2 = player, 4 = ally, 3 = planetnames
				...
				$('.ajaxContent').width(757).css('overflow','auto').find('tr').slice(1,-2).each(function(){
					var $resultRow = $(this).clone(), url;
					if ($resultRow.find('td.action').children().size() > 0) {
						resultList = /to=(\d+)\&/.exec($resultRow.find('td.action').first().html());
						url = 'http://' + ogameUniverse + '/api/playerData.xml?id=' + resultList[1];
					} else {
						url = 'http://' + ogameUniverse + '/api/playerData.xml?id=' + ogamePlayerId;
					}
					$resultRow.find('td.position').css('margin-right','5px').empty();
					$resultRow.find('td.home').empty();
					
					$.get(url, function(data){
						var updateTimestamp = parseInt($(data).find('playerData').first().attr('timestamp'),10) * 1000;
						var update = new Date(updateTimestamp);
						var nowTimestamp = new Date().getTime();
						var difference = (nowTimestamp - updateTimestamp);
						$('<span/>').css({'display':'block','text-align':'right','font-style':'italic'}).attr('title',update.getDate() + '.' + (update.getMonth() + 1) + '.' + update.getFullYear() + ' ' + update.getHours() + ':' + update.getMinutes() + ':' + update.getSeconds()).html('update ' + formatTime(difference) + ' ago').appendTo($resultRow.find('td.position'));
						
						var coordsList = new Array();
						var planets = new Array();
						$(data).find('planet').each(function(i){
