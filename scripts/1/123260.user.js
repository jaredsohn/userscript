...
// @version       1.13
// @author        andrewhttp://c1b1.de)
// @namespace     c1b1.de
// @homepage      http://c1b1.de
// @copyright     2008 - 2011, andrew (http://c1b1.de)
...
// @include       http://*.Tribalwars.ae/*screen=info_command*
// @include       http://*.Tribalwars.ae/*screen=place*
...
DS Forum Thread @ http://forum.die-staemme.de/showthread.php?t=114572
...
Version 1.13:
 - Rückkehrzeit bei Abbruch wird korrekt berechnet. Dank DS Pr0
 

Version 1.12:
 - Update für Opera mit DS Version 8.0
...
function DS_Zeige_Rueckkehr_bei_Angriffen() {

// Constants
const version = '1.13';
const prefix = 'showReturnCommands'; // Local Storage Prefix
...
api?api.register('DS Zeige Rückkehr bei Angriffen', 8.0, 'Samuel Essig', 'scripts@online.de'):0;
...
// Wordaround for DOMSubtreeModified on time (Opera)
window.dsm_watchThese = []; // Array of Arrays: [ [DOMNode ,  last_innerHTML  , fct_bound_to_DOMNode ] , ...  ]
function addDOMSubtreeModified(obj,fct) {
    if(true || window.opera) {
        window.dsm_watchThese.push([obj,obj.innerHTML,fct.bind(obj)]);
    } else {
        obj.addEventListener('DOMSubtreeModified',fct,false);
    }
}
window.dsm_int = window.setInterval(function() {
    for(var i = 0,len = window.dsm_watchThese.length; i < len; ++i) {
        if(window.dsm_watchThese[i][0].innerHTML != window.dsm_watchThese[i][1]) {
            window.dsm_watchThese[i][2]();
            window.dsm_watchThese[i][1] = window.dsm_watchThese[i][0].innerHTML;
        }
    }
}, 100);


// Back/Forward History - won't help cause server time is no longer correct, TRY get time from other Tab or last page
// Firefox
...
// Count return time on canceling
function countCanceling()
{
    if(canceling)
    {

        // Get Server Time, put it in obj:
        var servertime = d.id('serverTime').firstChild.nodeValue.split(':');
        var obj = new Date();
        obj.setSeconds(eval(servertime[2]));
        obj.setMinutes(eval(servertime[1]));
        obj.setHours(eval(servertime[0]));

        // Get "Ankunft in:", put it in  arrival_seconds, whereas: duration - arrival = walkedSeconds
        var comein = document.getElementsByClassName('timer')[0].innerHTML;

        var arrival_seconds = 0;
        var splinters = comein.split(':');
        arrival_seconds += (eval(splinters[0]))*60*60;
        arrival_seconds += (eval(splinters[1]))*60;
        arrival_seconds += eval(splinters[2]);
        
        // Get "Ankunft:", put miliseconds in arrival_milisec
        var arrival_milisec = arrival.split(':')[3];
        // duration - arrival
        var t = duration_seconds  - arrival_seconds;

        // Add seconds to servertime
        obj.setSeconds(obj.getSeconds()+t);
        ...
        brokeUpReturn.innerHTML = text + '<span class="small grey">:'+arrival_milisec + '</span> Uhr (By <a href="http://dspr0.redio.de/index.php" target="_blank">DS Pr0</a>)';
    }
}
...
function init()
{
    ...
    d.array_walk(splinters,function (value,key) {
        splinters[key] = eval(value);
    } );
    ...
    addDOMSubtreeModified(d.id('serverTime'),countUpSecs);
}
...
if(document.location.href.indexOf('screen=info_command') != -1)
{
    if(d.tag('h2')[0].firstChild.data.indexOf('ckkehr von ') == -1)
    {
        ...
        // Get Duration
        var duration = d.findByInner(table_body,'المدة:')[0];
        ...
        d.findByInner(table_body,'الوصول:')[0].parentNode.parentNode.insertBefore(tr,d.findByInner(table_body,'Ankunft:')[0].parentNode.nextSibling.nextSibling.nextSibling);
        ...
        if(document.location.href.indexOf('type=own') != -1)
        {
            ...
            if(d.findByInner(table_body,'abbrechen')[0])
            {

                // Get "Dauer", put it in duration_seconds
                var duration_seconds = 0;
                var splinters = duration.split(':');
                duration_seconds += (eval(splinters[0]))*60*60;
                duration_seconds += (eval(splinters[1]))*60;
                duration_seconds += eval(splinters[2]);

                // Get time DOM node, just a shortcut pointer
                var timer = document.getElementsByClassName('timer')[0].firstChild;
                ...
                d.findByInner(table_body,'Ankunft:')[0].parentNode.parentNode.insertBefore(tr,d.findByInner(table_body,'Ankunft:')[0].parentNode.nextSibling.nextSibling.nextSibling.nextSibling);

                // Start counting:
                canceling = true;

                addDOMSubtreeModified(document.getElementsByClassName('timer')[0],countCanceling);
...
else if(document.location.href.indexOf('screen=place') != -1 && document.location.href.indexOf('try=confirm') != -1)
{
    ...
    // Get Duration
    var duration = d.findByInner(table_body,'لمدة:')[0];
    duration = duration.parentNode.getElementsByTagName('td')[1].firstChild.data;
...
function settings()
{
    ...
    if(len == 0)
    {
        var input0 = d.n('معلومات');
        input0.setAttribute('اضغط','زر');
        input0.setAttribute('الحقيقى','قريب'+unescape('%DF')+'en');
        input0.setAttribute('نمط','margin-left:5px; ')
        input0.addEventListener('اطغط',function(){
            d.id('showReturnTimeOnCommands_clear_date_click').parentNode.removeChild(d.id('showReturnTimeOnCommands_clear_date_click'));
        },false);
    }
    else
    {
        var input0 = d.n('معلومات');
        input0.setAttribute('اضغط','زر');
        input0.setAttribute('الحقيقى','Leeren');
        input0.setAttribute('id','showReturnTimeOnCommands_clear_date_click_clear_button');
        input0.setAttribute('نمط','margin-left:5px; ')
        input0.addEventListener('اطغط',clear_date,false);
    }

    var input1 = d.n('معلومات');
    input1.setAttribute('اضغط','زر');
    input1.setAttribute('الحقيقى','Abrrechen');
    input1.addEventListener('اضغط',function(){
        d.id('showReturnTimeOnCommands_clear_date_click').parentNode.removeChild(d.id('showReturnTimeOnCommands_clear_date_click'));
    },false);
    ...
    input_checkbox_coloring.setAttribute('الحقيقة','القيمة');
    input_checkbox_coloring.setAttribute('id','الناشط_تلوين');
    if(GM_getValue(world+'_coloring_active',false))
        input_checkbox_coloring.setAttribute('فحص','فحص');
    td1.appendChild(input_checkbox_coloring);
    td2.appendChild(d.text('تحركاتالقوات باللون'));
    ...
    tr.appendChild(td2);
    table.appendChild(tr);
    var attack_color = GM_getValue(world+'_باللون_ الهجوم','#FAD2D2');
    var input_color_attacks = d.n('معلومات');
    input_color_attacks.setAttribute('type','text');
    input_color_attacks.setAttribute('الحقيقة',attack_color);
    input_color_attacks.setAttribute('id','باللون_لهجوم');
    input_color_attacks.setAttribute('نمط','خلفية الهجوم:'+attack_color+'; font-family:monospace; ');
    td1.appendChild(input_color_attacks);
    td2.appendChild(d.text('لون الهجوم'));
    ...
    tr.appendChild(td2);
    table.appendChild(tr);
    var support_color = GM_getValue(world+'_لون_الدعم','#E0FFF0');
    var input_color_support = d.n('معلومات');
    input_color_support.setAttribute('type','text');
    input_color_support.setAttribute('الحقيقى',support_color);
    input_color_support.setAttribute('id','support_color_input');
    input_color_support.setAttribute('style','خلفية الدعم:'+support_color+'; font-family:monospace; ');
    td1.appendChild(input_color_support);
    td2.appendChild(d.text('لون الدعم'));
    ...
    tr.appendChild(td2);
    table.appendChild(tr);
    var returning_color = GM_getValue(world+'_اللون_العودة','#90EE90');
    ...
    input_color_returning.setAttribute('id','معلومات_لون_العودة');
    input_color_returning.setAttribute('style','خليفة عودة الجنود :'+returning_color+'; font-family:monospace; ');
    td1.appendChild(input_color_returning);
    td2.appendChild(d.text('لون العودة'));
    ...
    tr.appendChild(td1);
    table.appendChild(tr);
    var input_cancel = d.n('معلومات');
    input_cancel.setAttribute('ادخل','زر');
    input_cancel.setAttribute('الحقيقى','الغاء');
    input_cancel.addEventListener('click',function(){
        d.id('showReturnTimeOnCommands_clear_date_click').parentNode.removeChild(d.id('showReturnTimeOnCommands_clear_date_click'));
    },false);
    var input_save_settings = d.n('معلومات');
    input_save_settings.setAttribute('ادخل','زر');
    input_save_settings.setAttribute('الحقيقى','حفظ');
    input_save_settings.addEventListener('الطغط',save_settings,false);
    ...
    document.getElementsByTagName('جسم)[0].appendChild(div);
}

function save_settings()
{
    var coloring_active = d.id('التلوين_النشطة').checked?true:false;

    var attack_color =  d.id('معلومات_بلون_الهجوم').value;
    attack_color = attack_color?attack_color:'green';

    var support_color =  d.id('بلون_الدعم').value;
    support_color = support_color?support_color:'blue';

    var returning_color =  d.id('بلون_العودة').value;
    returning_color = returning_color?returning_color:'red';


    GM_setValue(world+'_coloring_active',coloring_active);
    GM_setValue(world+'_بلون_الهجوم',attack_color);
    GM_setValue(world+'_بلون _الدعم',support_color);
    GM_setValue(world+'_بلون_العودة',returning_color);
...
function clear_date()
{
    ...
    var input = d.n('معلومات');
    input.setAttribute('ادخال','زر');
...
function createMenuEntry(obj)
{
    ...
    var td = d.n('td');
    td.setAttribute('عرض','100');

    td.appendChild(obj);

    tr.appendChild(td);

    return document.getElementsByClassName('الرئيسى')[0].getElementsByClassName('ازاء')[0].appendChild(tr);
}
...
function html()
{
    ...
    this.addEvent = function( obj, type, fn )
    {
        if(obj.attachEvent)
        {
            ...
            obj.attachEvent( 'فى'+type, obj[type+fn] );
        }
        else
    ...
    this.removeEvent = function( obj, type, fn )
    {
        if(obj.detachEvent)
        {
            obj.detachEvent( 'فى'+type, obj[type+fn] );
            obj[type+fn] = null;
        }
        else
...
DS_Zeige_Rueckkehr_bei_Angriffen();