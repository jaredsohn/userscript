...
var ex_labels_main = function() {
	...
	var workz = function() {
		...
		ex.click(function(e){
				...
				if (st[n]) {
					$('.issue-list-group li').has('span.label[data-name="' + n + '"]').fadeOut();
					$(e.target).html('(include)');
					$(e.target).closest('li').addClass('exclude-active');
				} else {
					$('.issue-list-group li').has('span.label[data-name="' + n + '"]').fadeIn();
					$(e.target).html('(exclude)');
...
addJS_Node (null, null, ex_labels_main);
