...
'.sidebarMode #globalContainer{left:0!important;position:inherit!important;}' +
'</style>');
  ...
  function changeConversationSize() {
	...
	if( saved_con_height ) {
		if( saved_con_height == 2 ) {
			if( $(".fbNubFlyout").height() != '450' ) {
				$('.fbNubFlyoutBody').css('height', '400px');
				$('.fbNubFlyout').css('height', '450px');
			}
		}
	}
    }
	...
	if( !GM_getValue('fce_firstload') || fce_version != GM_getValue('fce_inst_version')) {
		jQuery.facebox('<center><h3>Hello, and thanks for loading the new Facebook Chat Enhancer!</h3><br>' +
				...
				'<span style="font-size:160px;">:)</span><br>You can now adjust the height of the Chatbar yourself!<br>You only have to click the little gear (wheel)' +
 				'on the bottom left inside of the chatbar and then click on "Chat-Settings"!<br>The second thing you will find there, is a point to change the size of the conversation window!<br><br>' +
				...
				'2011-07-26 (12:07 AM): Fixed the Bug, which only shows some people online!<br><br>'+
				'2011-07-26 (03:53 PM): Fixed loading Bug in applications e.g. Zynga Poker!<br><br>' +
