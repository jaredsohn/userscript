...
<ul>
	...
	<li>0.8.3 - added shortcut key "space" to go to the next page (no more looking and pointing for the tiny Ã¯Â¿Â½</li>
...
function filterbar(){// create the menu
	...
	// Action
	var actionFilterTable = document.createElement('table');	actionFilterTable.className="tbg";
	var actionFilterTableRow = document.createElement('tr');	//ratioFilterTableRow.className="cbg1";
	createActionFilterCell(25, actionFilterTableRow);
	createAutoNextPageCell(26, actionFilterTableRow);
	actionFilterTable.appendChild(actionFilterTableRow);
	cell = document.createElement('td');
	actionFilterTable.setAttribute('cellspacing', 0);
	cell.colSpan = 8;
	cell.appendChild(actionFilterTable);
	rowAction.appendChild(cell);


	// Reset
	con = document.createElement('a');	
	cell = document.createElement('td');
	cell.innerHTML = '<img src="img/un/a/del.gif">';
	cell.addEventListener('click', reset, true);
	cell.rowSpan = 5;
	cell.appendChild(con);
	rowOffer.appendChild(cell);


	rowSearch.className="cbg1";
	rowOffer.className="cbg1";
	rowRatios.className="cbg1";
	rowAlliance.className="cbg1";
	rowAction.className="cbg1";


	filterTable.appendChild(  rowOffer );			
	filterTable.appendChild(  rowSearch );
	filterTable.appendChild(  rowRatios );			
	filterTable.appendChild(  rowAlliance );			
	filterTable.appendChild(  rowAction );			


	filterTable.className="tbg";
	filterTable.setAttribute('cellspacing', 1);
	filterTable.setAttribute('cellpadding', 1);

	applyAllFilters();
	randomDelay(147, 392, action_autoGoToNextPage);
	
	return filterTable;
}

/** Indicates if the offers table, with the applied filters has no offers. */
function noValidOffers() {
	var rowsOnOffersTable = xpathEvaluate('//div[@id="lmid2"]/table/tbody/tr[@class="rbg"]/../tr');
	return (rowsOnOffersTable.snapshotLength <= 3);
}

/**
* Automatically goes to the next page if there are no offers
*/
function action_autoGoToNextPage() {
	if (qFilterState[27] > qFilterState[26]) {
		qFilterState[27] = qFilterState[26];
		qFilterCells[27].firstChild.value = qFilterState[27];
	}

	if (noValidOffers()) {
		// no offers and more auto-pages to go through
		if (qFilterState[27] > 0) {
			qFilterState[27] = "" + parseInt(qFilterState[27]) - 1;
			qFilterCells[27].firstChild.value = qFilterState[27];
			qFilterCells[27].style.backgroundColor = filterBackGroundColor;
			savePermanentMarketFilters();
			action_goToNextPage();
			return;
		}
...
function applyAllFilters() {
	loadPermanentMarketFilters();
//	window.setTimeout(function() {setFiltersOnNewPage()}, 20);
	setFiltersOnNewPage();
}
...
* setFiltersOnNewPage
*/
function setFiltersOnNewPage() {
	for(var i=0; i<qFilterState.length; i++) {

	debug(DBG_LOW, "filters value["+i+"]: " + qFilterState[i]);

		if (qFilterState[i] != "false") {
			...
			switch (i) {
				...
				case 14:
				case 15:
						var tableColumnToSearch = (i<=7) ? 0 : 2;
						var isInclude = (((i>=0) && (i<=3)) || ((i>=8) && (i<=11))) ? true : false;
						offerSearchFilterEventListener (qFilterCells[i], i, tableColumnToSearch, isInclude);
					break;
				case 16:
				case 17:
				case 18:
				case 19:
						ratioFilterEventListener(qFilterCells[i], i, i-16);
					break;
				case 20:
						qFilterCells[i].firstChild.value = qFilterState[i];
						customRatioFilterEventListener(qFilterCells[i], i, qFilterState[i], false);
					break;

					// Alliance filters (checkboxes)
				case 21:
				case 23:
						qFilterCells[i].lastChild.checked = (qFilterState[i] == "true");
						alliancePartialMatchFilterEventListener(qFilterCells[i], i);
					break;

					// Alliance filters (textboxes)
				case 22:
				case 24:
						debug(DBG_LOW, "filters value["+i+"]: " + qFilterState[i]);
						var trimmed = qFilterState[i].replace(/^\s+|\s+$/g, '');
						if (trimmed == "") { continue; }

						var allianceFilters;
						if (qFilterState[i] == "::") {
							allianceFilters = "";
						} else if (qFilterState[i] == ": :") {
							allianceFilters = " ";
						} else {
							allianceFilters = qFilterState[i];
						}
						debug(DBG_LOW, "[setFiltersOnNewPage] allianceFilters " + allianceFilters);

						qFilterCells[i].firstChild.value = allianceFilters;
						var isInclude = (i == 22);

						allianceFilterEventListener(qFilterCells[i], i, isInclude);
					break;

				case 25:	// accept offers only
						actionAcceptOfferOnlyFilterEventListener(qFilterCells[i], i);
					break;

				// auto next page
				case 26:
				case 27: qFilterCells[i].firstChild.value = qFilterState[i];
					break;
				default: debug(DBG_HIGH, "NEW FILTER LOADED - need to implement the behavior to set it on new page: " + i);
					break;
			}
		}
	}
...
/**
* savePermanentScoutVillages
*/
function savePermanentMarketFilters() {
	GM_setValue(getServerName() + "_" + getUserId() + "_TravianMarket", escape(arrayToXML(qFilterState)));
}

/**
* loadPermanentScoutVillages
*/
function loadPermanentMarketFilters() {
//	var tmp = unescape(GM_getValue(getServerName() + "_" + getUserId() + "_TravianMarket"));


	var tmp = GM_getValue(getServerName() + "_" + getUserId() + "_TravianMarket");
	debug(DBG_NORM, "[loadPermanentMarketFilters] tmp: " + tmp);

	// To create initial values when they don't exist
	if (tmp==undefined) {
		debug(DBG_NORM, "[loadPermanentMarketFilters] undefined -> reset");
		reset();
	} else {
		debug(DBG_NORM, "[loadPermanentMarketFilters] defined -> unescape");
		tmp = unescape(tmp);
		qFilterState = xmlToArray(tmp);
	}
	debug(DBG_HIGH, "load permanent: qFilterState: " + qFilterState);
}
...
//				debug(DBG_LOW, "[TMarket][xmlToArray] i["+i+"] j["+j+"] " + x.childNodes[i].childNodes[j].nodeValue);
...
//	debug(DBG_LOW, "[getParamFromUrl] url " + url + " urlParam " + urlParam);
...
/**
* isPageAnyBuildingPage
* @param {URL} url The URL text string.
*/
function isPageAnyBuildingPage(url) {
	return (url.search(/build\.php/) != -1);
}


/**
* isThisPageAnyMarketPlacePage
*/
...
/**
* isPageMarketPlaceSendResources
*/
function isPageMarketPlaceSendResources() {
	if (isThisPageAnyMarketPlacePage(document.location.href)) {
...
/**
* isPageMarketPlaceBuy
*/
function isPageMarketPlaceBuy() {
	if (isThisPageAnyMarketPlacePage(document.location.href)) {
		var tParam = getParamFromUrl(document.location.href, "t");
		return (tParam == 1);
	}
...
function debug(verbosity, txt) {
 	if (verbosity <= DEBUG_VERBOSITY) { GM_log(txt); }
}
