...
// @name            Macedonian Announces
// @namespace       Macedonian Announces
// @description     Macedonian Announces
// @version         1.0.1 
...
function announce_init_blocks() {
    if ($('#orderContainer').length > 0) {
        ...
        GM_xmlhttpRequest({
            method  : 'Get',
            url     : 'http://erep-udar.info/stats/api/blocks/index/' + unsafeWindow.flc.getVariable("citizen_id"),
...
function announce_blocks() {
    ...
    $('#announce-blocks-panel').append('<div class="announce-blocks-announces"></div>');
        ...
        $('#announce-blocks-panel').find('div.announce-blocks-announces').append('<div class="announce-annouces-content"></div>');
            $('#announce-blocks-panel').find('div.announce-annouces-content').append('<h1>' + translate('Announce') + ':</h1>');
