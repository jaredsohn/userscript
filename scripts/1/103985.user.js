// ==UserScript== 
// @name	Facebook-Autostubser
// @author      Michael
// @namespace   autopoke_5200 
// @description Stubst automatisch Leute von Facebook zurück
// @version     5.0 (1 Jun 2011)
...
function init() { 
     ...
     if (debug > 0) { 
          var poke_div = evaluate_xpath('.//h4[contains(.,"Anstupser")]'); 
	  ...
	  data:poke_post_data,
          onload: function(response) { 
               if (response.status == 200) { 
                    ...
                    if (response.responseText.indexOf('Du bist im Begriff') != -1) { 
                         poke_node.innerHTML = 'Lese Bestätigung...'; 
                         if (debug >= 2) FB_log("Text:" + response.responseText); 
                         execute_poke(poke_uid, poke_node); 
                    } else if (response.responseText.indexOf('hat deinen letzten Anstupser noch nicht erhalten') != -1) { 
                         poke_node.removeAttribute('href'); 
                         poke_node.innerHTML = 'Bereits gestupst!'; 
                    } else { 
                         poke_node.removeAttribute('href');  
                         poke_node.innerHTML = 'Auto-Stups Fehler! [1.1]';  
                    ...
                    poke_node.innerHTML = 'Auto-Stups Fehler! [1.2]'; 
     ...
     GM_xmlhttpRequest({ 
          ...
          onload: function (response) { 
               if (response.status == 200) { 
                    ...
                    if (response.responseText.indexOf('hat deinen letzten Anstupser noch nicht erhalten') != -1) { 
                         poke_node.removeAttribute('href'); 
                         poke_node.innerHTML = 'Bereits gestupst!'; 
                    } else if (response.responseText.indexOf('Du hast') != -1) { 
                         poke_node.removeAttribute('href'); 
                         poke_node.innerHTML = 'Auto-Gestupst!'; 
                    } else { 
                         poke_node.innerHTML = 'Stupsen Fehlgeschlagen! [2.1]'; 
                         FB_log("Auto-Poke failed -- Error Code 2.1: Facebook gave an unexpected response to the poke."); 
                         FB_log(response.responseText); 
                    } 
               } else { 
                    poke_node.innerHTML = 'Stupsen Fehlgeschlagen! [2.2]'; 
          ...
          onerror: function (responseDetails) { 
               poke_node.removeAttribute('href'); 
               poke_node.innerHTML = 'Stupsen Fehlgeschlagen! [2.3]'; 
