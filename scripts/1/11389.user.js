...
// @name           imdbTOss
// @namespace      localhost
// @description    imdb pro to studiosystems
...
if (action.match("name") != null) { //person
	ptype = PERSON;
	iptname = "person";
} else if (action.match("title") != null) {
	ptype = PROJECT;
	iptname = "project";
...
if (ptype != NONE){
	var htd = $("html/body/table/tbody/tr/td/font/b").eq(0);
	//alert(htd);
	var name = htd.text().replace(/\x28[\w\W\d\D\s\S]*\x29/gi,"");
	name = name.replace(/\x5B[\w\W\d\D\s\S]*\x5D/gi,"");
	var qstr = "http://studiosystem.com/index.cfm?action=basicSearch&term=" + name + "&type=" + iptname;
	var html = "<span style=\"text-size:small;\">&nbsp;&nbsp; &nbsp;  <a href=\""+ qstr +"\">Search for "+ name +" on StudioSystem</a></span>";
	$(html).appendTo(htd);
}
