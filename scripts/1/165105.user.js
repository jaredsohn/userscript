...
// @namespace      Pokemon Vortex Bot
// @author         Garrie Richmond/ IGN: 1_annihilation
...
/*
	...
	REMEMBER: save as FILE.USER.JS so greasemonkey/scriptish can install it!
	
	DONT ASK ME FOR HELP!

	Releasing into the public domain.
	at anytime they can change the id's of the elements on their site and this will stop working.

	contains lots of crap code! there is no logic, just a bunch of tests and currently only battles PsychicTraining.
	
	WHAT IT DOES:
	if you go to a map it will open a new window with a name, this name, can be checked and if found, the site
	can stop it. the parent window just monitors the inactive or hanging of the battle page.
	
	it currently navigates to the battle another player and battles PsychicTraining which all his pokemon
	have one move, Splash. so any pokemon can beat them. you can change battleId to another number.
	
	IT'S NOT CHECKING for other pages so you could get stuck in a loop or just hang firefox.
	
	on a side note, the Pokemon
