...
// @name       js简繁互转
// @version    1.2
// @description  对页面进行简繁转换, 默认繁转简
// @match      *
// @copyright  2013, qingshan
// ==/UserScript==
function convert(){	
    ...
    function s2t(data){
        ...
        for(var i=0;i<data.length;i++){
            k+=(s.indexOf(data.charAt(i))==-1)?data.charAt(i):t.charAt(s.indexOf(data.charAt(i)));
        }
        return k;
    }
    ...
    function t2s(data){
        var k='';
        for(var i=0;i<data.length;i++) {
            k+=(t.indexOf(data.charAt(i))==-1)?data.charAt(i):s.charAt(t.indexOf(data.charAt(i)));
        }
        //console.error(k);
        return k;
    }
...
document.body[eventName](prefix + "load", convert(), false);