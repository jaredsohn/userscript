 ...
 ********************************************************************************/
// Script information
var Version = 'Atlantis TURKIYE'; // Year, Month, Day, Revision, Maturity (e.g. YYYMMDDa_BETA)
...
// Help pages
var urlError = 'http://www.atlantisturkiye.com/announces/iste-yepyeni-dragon-of-atlantis-turkce-bot-t732.html';
...
var Seed = {
  ...
  jsonAddJob : function (job){  // called from various jsons (buildUpgrade) when new job rx'd 
	...
	jsonGotCity : function (dat) {  // call from various jsons when seed data rx'd
		...
		if (Data.options.verboseLog.enabled)
			actionLog('Updated coords are ' + dat.city.x + '/' + dat.city.y);
...
Tabs.Waves = {
	...
	init : function (div) {
		var t = Tabs.Waves;
		t.cont = div;
		if (Data.options.waves.targets.length == 0)
			for (var i=0; i<5; i++)
				Data.options.waves.targets[i] = {enabled:false, lastAttack:0, troopsWave:{}, targetX:0, targetY:0, terrainType:null, terrainLevel:0, stats:{numAttacks:0, spoils:{}}};
...
//    troopTable (document.getElementById(rndStyle['idPrefix'] + 'Troops'), 1, 'FW', t.eventTroops);
  ...
  waveAttackTick : function (){
    ...
    var msg;
    if ((msg = t.checkTroops (cityIdx, target.troopsWave)) != null){
      t.dispFeedback (msg);
      t.attackTimer = setTimeout (t.waveAttackTick, 5000);
      return;
    }
	...
	new Ajax.march (Seed.s.cities[cityIdx].id, target.targetX, target.targetY, gen.id, target.troopsWave, 'wave', function (rslt) {
		var t = Tabs.Waves;
		if (rslt.ok && rslt.dat.result.success) {
			if (Data.options.verboseLog.enabled)
				actionLog('Wave attack to ' + targMsg + ' succeeded');
			else
			...
			if (Data.options.verboseLog.enabled)
				actionLog('Wave attack to ' + targMsg + ' failed and returned error: ' + rslt.errmsg);
	...
	checkMapData : function () {
		...
		if (Data.targets.radius!=35 || Data.targets.center.x!=Seed.s.cities[0].x || Data.targets.center.y!=Seed.s.cities[0].y) {
			if (Data.options.verboseLog.enabled) {
				...
				actionLog('Reported coords are ' + Seed.s.cities[0].x + '/' + Seed.s.cities[0].y);
			}
			Seed.fetchSeed(function (rslt) {
				if (rslt.ok) {
					if (Data.options.verboseLog.enabled) {
						actionLog('Player data was successfully requested from the server');
						actionLog('Stored coords are ' + Data.targets.center.x + '/' + Data.targets.center.y);
						actionLog('Updated coords are ' + Seed.s.cities[0].x + '/' + Seed.s.cities[0].y);
					}
					if (Data.targets.radius!=35 || Data.targets.center.x!=Seed.s.cities[0].x || Data.targets.center.y!=Seed.s.cities[0].y) {
						if (Data.options.verboseLog.enabled)
