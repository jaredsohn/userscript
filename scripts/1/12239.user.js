...
/*
  ...
  2008/05/10 - 0.61
  - Fixed duplicate comment anchors in WP 2.5
  - Remove the different colouring for comments that aren't yours
...
function commentNinja() {
  autoUpdateFromUserscriptsDotOrg({
    name: 'WordPress Comment Ninja',
	url: 'http://userscripts.org/scripts/source/12239.user.js',
  ...
  styles += "#idt-nfo .idt-more-nfo { display: none; } #idt-nfo:hover .idt-more-nfo { display: block; }";
  styles += "#idt-nfo img { display: none; } #idt-nfo:hover img { display: block; }";
  ...
  function displayInfo() {
    ...
    if (h.length == 0) {
      // WP 2.5
      h = $x("/html/body/div/div/div[3]/div/form/h2");
      wp_ver = 2.5;
    }
    if (h.length > 0) {
  ...
  function wp25RespondLink(i) {
    i.forEach(function(r) {
		...
		var permission = $x(".//*[contains(@class,'unapprove')]", r);
		if (permission.length > 0) {
		  r.innerHTML = '<span class="idt-respond"></span><span> | </span>' + r.innerHTML;
		} else {
		  r.innerHTML = '<span class="idt-respond"></span> | ' + r.innerHTML;
		}
		var insert = $x(".//*[contains(@class,'idt-respond')]", r);
		if (insert[0]) {
		  var img = document.createElement('img');
  ...
  if (i.length > 0) {
    ...
    // WP 2.5
    i = $x(".//td[contains(@class,'action-links')]");
    wp25RespondLink(i);
  }
  return;
}
...
function clickListener(ev) {
  ...
  function wp25BuildBox(c) {
      // ANALYZE DATA AND STORE

      var c_name = $x(".//p[@class='comment-author']/strong", c)[0].textContent;

      var post_link = $x(".//p[last()]/a[last()]", c)[0];

      var c_link = post_link.href;
      if (post_link.href.match(/#/)) {
	// remove the named anchor from the post link
	c_link = post_link.href.split(/#/)[0];
      }

      var c_email = $x(".//p[@class='comment-author']/a[2]", c)[0].textContent;
	...
	insertAfter(close, post);
      ...
      //      tr.className = 'idt-comment-ninja-container';
      //      td.appendChild(ul);
      //      tr.appendChild(td);
      //      insertAfter(tr, c);
      c.childNodes[3].appendChild(ul);
  ...
  function buildBox(respond) {
    ...
    if (c.className == "idt-built-form") {
      var box = $x(".//ul[@class='idt-comment-ninja-input']", c);
      if (box.length > 0) {
  ...
  function testPostComment() {
    ...
    if (links.length == 0) {
      // WP 2.5
      links = $x("//tbody[@id='the-comment-list']/tr/td[2]/p[last()]/a");
    }
    if (links.length > 0) {
