// ==UserScript==
// @name           google extra (italian version)
// @description    Displays results for google image, video, wikipedia, wikipedia.it search and dictionary.com search alongside normal google searches.
...
var icons =   "data:image/gif;base64,R0lGODlhdAEDAfcAAPVvb4%2Bv1Mw2NNbW1rIdIFVSVbfDyIzK7OoBAerz%2BcbHx%2FbOosnq0nTL"+
	    ...
	    //edit to change number of lines for images/videos
 var image_l= "3"
 var video_l= "2"
 var image_n= 6*image_l
 var video_n= 2*video_l
	    // end edit
...
function addVideos() {
  ...
  GM_xmlhttpRequest({
      ...
      onload: function(result) {
        res = result.responseText;
        if (res.indexOf("Suggestions:") == -1) {
          videoDiv.innerHTML = '<table width="100%" cellspacing="0" cellpadding="0" border="0" class="t bt" style="margin-top: 4px;margin-bottom: 2px;" style="margin: 3px;"><td><span id="sd"><a href="'+videourl+'">Videos</a></span></td></table>'
          addTopImages("video");
...
function addWikiIt() {
  wikiurlIt = "http://it.wikipedia.org/wiki/" + href.substring(href.indexOf("q=") + 2, ((href.indexOf("q=") < href.lastIndexOf("&")) ? href.indexOf("&", href.indexOf("q=")) : href.length)).replace(/%20|\+/g, "_").replace(/%22/g, "").replace(/_[a-z]/g, toUpCase);
  if (document.getElementById("wikiIt"))
    wikiDivIT = document.getElementById("wikiIt");
  else {
    wikiDivIt = document.createElement("div");
    wikiDivIt.setAttribute("class", "wikiIt");   
    wikiDivIt.setAttribute("id", "wikiIt");
    GM_addStyle('.wikiIt {font-size:0.75em;color:#333333;font-family:"Lucida Sans Unicode","Arial Unicode MS","Lucida Sans","Lucida Grande",Verdana,Helvetica,Arial,sans-serif;}'+
                '.wikiContent {overflow:auto;max-height:300px;}');
    rightDiv.appendChild(wikiDivIt);
  }
  GM_xmlhttpRequest({
      method: 'get',
      headers: {
      ...
      onload: function(result) {
        ...
        if (res.indexOf('<p>', res.indexOf('<div id="contentSub">')) != -1) {
          wikiDivIt.innerHTML = '<table width="100%" cellspacing="0" cellpadding="0" border="0" class="t bt" style="margin-top: 4px;margin-bottom: 2px;font-family:arial,sans-serif;" style="margin: 3px;"><td><span id="sd"> Wikipedia (<a href="'+wikiurlIt+'">goto</a>) </span></td></table>'
          addTopImages("wikiIt");
          if (eval(GM_getValue("show", "[true,true,true,true,true]"))[2]) {
           ...
           wikiDivIt.innerHTML += "<div class='wikiContent'>" +
                                ((res.indexOf('class="image"') != -1) ? wikiImage : "") +
                                wiki.replace(/href=\"\//g, "href=\"http://it.wikipedia.org/") + 
                                "</div>";
          }
        }
        addEventListeners("wikiIt");
      }
    });
}
  ...
  GM_xmlhttpRequest({
      ...
      onload: function(result) {
        ...
        if (res.indexOf('<div class="luna-Ent">') != -1) {
          dictDiv.innerHTML = '<table width="100%" cellspacing="0" cellpadding="0" border="0" class="t bt" style="margin-top: 4px;margin-bottom: 2px; margin: 3px;"><td><span id="sd" style="font-size:100%;"><a href="'+dicturl+'">Dictionary.com</a></span></td></table>'
          addTopImages("dict");
...
function numberize(div) {
  switch (div) {
    ...
    case ("wikiit"): return 3; break;
    case ("dict"): return 4; break;
    case ("dictCN"): return 5; break;
  }
}
...
for (i = 0; i < 6; i++) {
  switch (eval(GM_getValue("order", "[0,1,2,3,4]"))[i]) {
    ...
    case 3: addWikiIt(); break;
    case 4: addDict(); break;
    case 5: if (GM_getValue("DictCN", false)) addDictCN(); break;
  }
}

function toggleDictCN() { 
  GM_setValue("DictCN", !(GM_getValue("DictCN", false)) ) 
}
