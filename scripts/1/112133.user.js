// ==UserScript==
// @name           Lulz Filter
// @namespace      aeosynth (lulz.net edit by An~Ok.CAT.BOY)
// @description    Regular expression, point-and-click filtering. Originally by Aeosynth, modified for Lulz.net by An~Ok.CAT.BOY.
// @include        http://lulz.net/*
// @include        http://www.lulz.net/*
// @version        1.4
...
//Javascript blows fucking dicks. What do you mean I can't do <br />?
dialog.innerHTML = '\
<div>\
	<span></span><br>\
	<span></span>\
...
if (GM_getValue('top', true))
  dialog.style.top = GM_getValue('top', 50);
else
  dialog.style.bottom = GM_getValue('bottom');
	*/

const reply = /res|\/|archive\/|thread/.test(window.location.pathname);
...
const useridx =
  ...
  server == 'dis' ? './/label' :
  "./label";
//Thanks for the lack of classes, cho0b. Take a fucking web design class.
...
for (var i = 0, l = omitted.length; i < l; i++) {//getPosts on thread collapse. Does this work w/ lulz.net x?
  omitted[i].addEventListener('DOMCharacterDataModified', function() { if (/^\d/.test(this.textContent)) getPosts() }, true);//lulz.net extension
  ...
  reset();
	for (var i in threads) {
	  if (spam(threads[i])) {
		...
		{
			//was a stub did not have sibling
		}
	  }
	}
  if (manual && stubs) {
...
function spam(el) {
  ...
  if (files) {
    ...
    if (temp)
      for (j in userids)
        if (userids[j].test(temp.textContent))
          return true;
  } if (emails) {
    temp = $('a', n);
    if (temp)
...
function hideCountF() {
  hideCount = 0;
  for (var i = 0; i < threads.length; i++)
    if (threads[i].style.display && stubs)
      hideCount++;
  if (manual && stubs) {
    for (var i in replies)
