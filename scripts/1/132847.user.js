...
var options_menu = document.getElementById('resourceUsers').childNodes[1];
	...
	if ( (page == 'tradegood') || (page == 'resource') )
	{
		...
		for (var j = 1; j < donationList.rows.length; j+=cities)
		{
			...
			if (sort_all_mills==2) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"vlctooptip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>"+Comma(Math.round(ordered_towns[j][6]))+" madera ha sido donada por la ciudad.</span></a>";
			}
			if (sort_all_mills==1) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"vlctooptip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>"+Comma(Math.round(ordered_towns[j][7]))+" madera ha sido donada por la ciudad.</span></a>";
			}
			var workersneeded = "o "+workers_at_100+" Trabajadores";
			if (method == 3) {
			workersneeded = "";
			}
			var woodneed100 = "<font color=\"orange\">100% Necesitas "+Comma(Math.round(needed_wood))+" "+workersneeded+"</font><br/>";
			var woodneed110 = "<font color=\"green\">110% Necesitas "+Comma(Math.round(needed_wood110))+"</font>";
			if (needed_wood < 1) {woodneed100 = "";}
			if (needed_wood110 < 1) {
			var over100percent = Math.round(needed_wood110)*-1;
			woodneed110 = Comma(over100percent)+" Madera sobrante";
			}
