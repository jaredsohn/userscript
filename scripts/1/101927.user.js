...
//		Inhalt der Einstellungen generieren & anzeigen		
	...
	generateContent: function(hk_content){
		...
		s_th.innerHTML = "Gespeicherte Hotkeys";
		...
		s_ex_td.innerHTML = "Hier werden alle gespeicherten Hotkeys angezeigt.";
		...
		var s_sign_th = [];
		s_sign_th[0] = $("th");
		s_sign_th[0].innerHTML = "Tastenkombination";
		s_sign_th[1] = $("th");
		s_sign_th[1].innerHTML = "Typ";
		s_sign_th[2] = $("th");
		s_sign_th[2].innerHTML = "Informationen";
		s_sign_th[3] = $("th");
		...
		for(var i=0; i<saved_keys.length; i++){
			...
			if(checkHotkeyStructure(hk)){
				...
				var hk_type_td = $("td");
				hk_type_td.innerHTML = (hk.type==1 ? "Direktlink" : (hk.type==2 ? "Schnellleistenlink" : (hk.type==3 ? "sonstiger Link" : "unbekannt")));
				
				var hk_info_td = $("td");
				switch(hk.type){
					case 1: // direkt
						hk_info_td.innerHTML = '<strong>URL:</strong> <span style="font-family: Courier New;">'+htmlspecialchars(hk.data.url.length>50 ? hk.data.url.substr(0,50)+"..." : hk.data.url)+'</span>' + (hk.data.blank ? "<br>wird in neuem Tab ge&ouml;ffnet":"");
						break;
					case 2: // direkt
					...
					case 3: // direkt
						var istr = "";
						istr += "<strong>Typ:</strong> " + (hk.data.link_type ? "Button":"Link") + "<br>";
						istr += "<strong>Methode:</strong> " + (hk.data.recog_method ? "Inhalt":"HTML-Attribut") + "<br>";
						istr += (hk.data.recog_method ? "" : "<strong>HTML-Attribut:</strong> "+hk.data.html_attr+"<br>");
						istr += "<strong>Wert:</strong> "+htmlspecialchars(hk.data.value);
		...
		if(saved_keys.length == 0){
			...
			no_row.firstChild.innerHTML = "Bisher wurde noch keine Hotkeys gespeichert!";
		...
		clear_btn.addEventListener("click",function(){if(confirm("M"+oe+"chtest du wirklich alle Hotkeys l"+oe+"schen?")){ storage.clear(/hotkey_\d+/); SettingsAPI.refreshContent(); }},false);
		clear_btn.innerHTML = "Alle Hotkeys l&ouml;schen";
		hk_content.appendChild(clear_btn);
		
		var new_block = $("p");
		var new_btn = $("button");
		new_btn.addEventListener("click",function(){newHotkey.init();},false);
		new_btn.innerHTML = "Neuen Hotkey erstellen";
		...
		global_headline.innerHTML = "Globale Konfiguration";
		...
		new_define_key_alert_lbl.innerHTML = " <b>Neuer Hotkey:</b> Info-Meldung beim Festlegen der Tastenkombination anzeigen";
		...
		open_onload_lbl.innerHTML = " Einstellungen beim Laden der Seite ausklappen";
		...
		global_save_btn.innerHTML = "Speichern";
	...
	editHotkey: function(e){
		alert("Das Bearbeiten von Hotkeys ist in dieser Version noch nicht integriert!");
	},
	
	deleteHotkey: function(e){
		if(confirm("M"+oe+"chtest du diesen Hotkey wirklich l"+oe+"schen?")){
			var ruler = e.target;
			...
			alert("Der Hotkey wurde gel"+oe+"scht!");
			this.refreshContent();
		}
	},
	...
	saveGlobalOptions: function(){
		...
		alert("Die globale Konfiguration wurde gespeichert!");
	},
	...
	toggleContent: function(e,forceState){
		...
		e.target.title = (newState ? "Einstellungen verbergen" : "Einstellungen anzeigen");
	},
...
var newHotkey = {
	...
	init: function(){
		...
		key_lbl.innerHTML = "Tastenkombination";
		...
		key_btn.innerHTML = "&raquo; Festlegen &laquo;";
		...
		quickbar_option.innerHTML = "Schnellleistenlink";
		...
		other_option.innerHTML = "sonstiger Link";
	...
	displayNewHotkey: function(hk){
		...
		gid("hotkeys_new_key_button").firstChild.nodeValue = AE+"ndern";
	},
	...
	save: function(){
		...
		if(typeof hk_key != "object"){
			alert("Du musst eine Tastenkombination festlegen!");
			return;
		}
		...
		switch(hk_type){
			case 0: // nichts
			case false:
				hk_errormsg = "Du musst einen Hotkey-Typ ausw"+ae+"hlen!";
				break;
			case 1: // Direktlink
				hk_data = new_DataGrabber.getDirect();
				hk_errormsg = "Du musst eine URL eingeben!";
				break;
			case 2: // SL-Link
				hk_data = new_DataGrabber.getQuickbar();
				hk_errormsg = "Du musst einen Schnellleistenlink ausw"+ae+"hlen!";
				break;
			case 3: // sonstiger
				hk_data = new_DataGrabber.getOther();
				hk_errormsg = "Du Einstellungen f"+ue+"r den sonstigen Link sind ung"+ue+"ltig!";
				break;
			default:
...
var HotkeyCreator = {
	...
	endCapture: function(e){
		...
		if(!e.keyCode){
			alert("Diese Taste wurde nicht erkannt und kann nicht verwendet werden!");
			return;
		}
...
var TypeManager = {
	...
	ContentFiller: {
		direct: function(t){
			...
			blank_description.innerHTML = " in neuem Tab &ouml;ffnen";
		...
		quick: function(t){
			var instructions = $("p");
			instructions.innerHTML = "W&auml;hle hier einen Link aus deiner Schnellleiste aus, der beim Dr&uuml;cken des Hotkeys aufgerufen werden soll. SL-Scripte sind auch m&ouml;glich.<br>Wenn du mehrere SL-Links mit der selben Bezeichnung hast, k&ouml;nnen sie nicht zuverl&auml;ssig als Hotkey definiert werden. Benenne sie in diesem Fall einfach um.";
			t.appendChild(instructions);
		...
		other: function(t){
			var instructions = $("p");
			instructions.innerHTML = "Mit diesem Typ kannst du alle anderen Links und Schaltfl&auml;chen im Spiel als Hotkey definieren. Zur Verf&uuml;gung stehen zwei Methoden: automatisch und manuell";
			t.appendChild(instructions);
			...
			manual_description.innerHTML = "Mit dieser Methode kannst du selbst Kriterien zum Finden des Links festlegen.<br>Anwendung f&uuml;r fortgeschrittene Benutzer empfohlen!";
			...
			manual_recog_td.innerHTML = '<input type="radio" name="recog_type" value="htmlattr"> HTML-Attribut<br><input type="radio" name="recog_type" value="content"> Inhalt';
...
var new_DataGrabber = {
	...
	getOther: function(){
		...
		if(!(f.elements.link_type[0].checked || f.elements.link_type[1].checked)){ return "Du musst einen Typ angeben!"; }
		...
		else{ return "Du musst eine Ermittlungs-Methode angeben!"; }
...
function decodeKey(key,mayAsk){
	if(String.fromCharCode(key).match(/^[A-Z0-9]$/)){
		...
		var new_code;
		if(mayAsk && (new_code = trim(window.prompt("Welche Taste wurde gedr"+ue+"ckt?\nDie gew"+ae+"hlte Taste ist zwar g"+ue+"ltig, kann aber nicht als Buchstabe oder Zahl dargestellt werden.\nBitte gib an, welche Taste du gedr"+ue+"ckt hast (die Taste wird gespeichert und in Zukunft erkannt):","")))){
