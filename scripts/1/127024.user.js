// ==UserScript==
// @name           Tudo Sobre Café Mania
// @namespace      editado por Guilherme | uid=15577794736272900305
// @description    Script para facilitar o trabalho da Comunidade Tudo Sobre Café Mania
// @TSCM     www.orkut.com.br/Main#Community?cmm=118022967
// @include        *.orkut.*/Main#CommMsgs?cmm=118022967*
// @include        *.orkut.*/CommMsgs?cmm=118022967*
// @include        *.orkut.*/Main#CommMemberManage?cmm=118022967*
// @include        *.orkut.*/CommMemberManage?cmm=118022967*
// @include        *.orkut.*/Community?cmm=118022967*
// @include        *.orkut.*/Main#Community?cmm=118022967*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
| 		Userbars PS. [UPS]     	                              |
|		http://www.orkut.com.br/Main#Community?cmm=109941116  |
|	
|     este script foi criado por joseph, e editado por Guilherme      |
|						                      |
+---------------------------------------------------------------------+
*/

Editado_por = "Guilherme";
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=118022967&tid=5569365422287419741", false);
	...
	function captchaONOFF(env, status) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgPost?cmm=118022967&tid=5570607291179875049", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[Blue][b]Titulo:[/b][/blue] ".concat(titulo, breakLine, "[green][b]Criador:[/green][/b] ", "[green]", uid, breakLine, "[/green]", breakLine, "[/green][red][b]Motivo:[/b][/red] ", motivo, breakLine, "[silver][b]Horario:[/b][/silver] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b][red]Membro:[/b][/red] ".concat("[blue]", uid, breakLine, "[/blue]", breakLine, "[/blue][b][blue]Motivo:[/b][/blue] ", motivo, breakLine, "[b][silver]Horario:[/b][/silver] ", pegaData(), breakLine, breakLine, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function online() {
		...
		var breakLine = "\n";
		var texto = String("[b]Duuu [green]ON[/green].[/b]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function offline() {
		...
		var breakLine = "\n";
		var texto = String("[b]Duuu [red]OFF[/red].[/b]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent(""),
