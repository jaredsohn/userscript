...
api.register( 'DSStorageBalancer', [8.2,8.3,8.4,8.5,8.6,8.7,8.8], 'knish3r', 'scripts@knish3r.de' );
...
if( !checkboxes.disableDsRealCharts && (lib.lang == "de" || lib.lang == "ch") ) {
	...
	if( location.href.match(/screen=info_player/) ) {
		...
		for (var i=0 ; i<document.links.length ; i++) {
			...
			if (link.href.replace(/\%20/g,"").search(/village=[0-9]+&mode=new&player=([0-9]+)[&=A-Za-z0-9]*&screen=mail/) > -1) { 
				var tr = new Array(6);
	...
	/* ally charts */
	if (document.location.href.match(/screen=info_ally/) || (document.location.href.match(/screen=ally/) && document.location.href.match(/mode=profile/) ) ) {
		for (var i=0 ; i<document.links.length ; i++) {
			var link=document.links[i];
			if (link.href.search(/village=[0-9]+[&=A-Za-z0-9]*&id=([0-9]+)&screen=info_member/) > -1) {
				console.log("yes");
				var allyId = link.href.match(/village=[0-9]+[&=A-Za-z0-9]*&id=([0-9]+)&screen=info_member/)[1];
				var tr = new Array(6);
				for( var i=0 ; i<tr.length ; i++ ) tr[i] = document.createElement("tr");
				var td = new Array(4);
				for( var i=0 ; i<td.length ; i++ ) td[i] = document.createElement("td"); 
