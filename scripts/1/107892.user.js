...
// @icon            http://www.gravatar.com/avatar.php?gravatar_id=537879f6d9eaf804cd6aa515eab9e6fa&r=PG&s=60&default=identicon
// @grant           GM_setValue
// @grant           GM_getValue
...
Auction.formPrice = function(price) {
    ...
    //GreaseMonkey.log('Auction.formPrice(' + price + ') == ' + result);
    return result;
}
...
* @return Object
*/
Auction.parsePricesList = function(doc) {
    //GreaseMonkey.log('Поиск объявлений...');

    var text = doc.body.innerHTML;

    var xpath = '/html/body/center/table/tbody/tr/td/table/tbody/tr[2]/td[2]/table';
    var table = XPath.findFirst(xpath, doc);
    if (table == null) {
        GreaseMonkey.log('Список объявлений на рынке не найден.');
        return [];
    }
    ...
    for (var i = 2; i < table.rows.length; i++) {
        ...
        // Пропускаем крафт и уникальные предметы
        if ($('img[src="http://im.heroeswm.ru/i/transparent.gif"]', row.cells[0]).length > 0)
            continue;

        // Пропускаем все сделки кроме 'Купить сразу!'
        var offerType = String.trim(row.cells[1].textContent);
        if (offerType != 'Купить сразу!')
            continue;

        var chlidTables = row.cells[2].getElementsByTagName('TABLE');
        var cost = Cost.toGold(Cost.fromNode(chlidTables[1]));
        var costHtml = Cost.toHtml({gold: cost});
        var countText = chlidTables[1].textContent;
        var count = Number(countText.replace(/\D/ig, ''));
...
/** Загрузка рыночных цен артефакта
* @param Object art
* @param Document sourceDoc
* @param Function onLoad
* @param Number max
*/
Auction.loadPricesList = function(art, sourceDoc, onLoad) {
    ...
    function onRawLoad(doc, params) {
        ...
        var prices = Auction.parsePricesList(doc);
        //GreaseMonkey.log('Загружено ' + prices.length + ' объявлений для "' + name + '".');
        if (onLoad !== undefined) {
            onLoad(prices);
        }
    }

    var url = 'http://' + sourceDoc.location.hostname + '/auction.php?cat=' + art.cat + '&art_type=' + art.id + '&sort=4&sbn=1&sau=0&snew=1';
    GreaseMonkey.loadDoc(url, onRawLoad);

}


/** Парсинг списка артефактов
* @param Document doc
*/
Auction.parseArtsList = function(doc) {
    ...
    for (var i = 0; i < Auction.treeFuncs.length; i++) {
        ...
        // function a_ring(){ document.getElementById('mark_info_ring').innerHTML = '&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=warring13"><font style="font-size:9px;">Глаз дракона (17)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=gm_rring"><font style="font-size:9px;">Заколдованное кольцо в. охотника (19)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=ring19"><font style="font-size:9px;">Кольцо бесстрашия (3)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=wwwring16"><font style="font-size:9px;">Кольцо боли (11)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=warriorring"><font style="font-size:9px;">Кольцо воина (19)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=mmmring16"><font style="font-size:9px;">Кольцо звёзд (7)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=i_ring"><font style="font-size:9px;">Кольцо ловкости (12)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=gm_sring"><font style="font-size:9px;">Кольцо ловкости в. охотника (12)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=sh_ring1"><font style="font-size:9px;">Кольцо ловкости зверобоя (14)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=hunter_ring2"><font style="font-size:9px;">Кольцо ловкости мастера-охотника (33)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=smring10"><font style="font-size:9px;">Кольцо молнии (7)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=mring19"><font style="font-size:9px;">Кольцо непрестанности (9)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=circ_ring"><font style="font-size:9px;">Кольцо отречения (10)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=hunter_ring1"><font style="font-size:9px;">Кольцо полёта мастера-охотника (23)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=powerring"><font style="font-size:9px;">Кольцо пророка (11)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=bring14"><font style="font-size:9px;">Кольцо противоречий (7)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=sring4"><font style="font-size:9px;">Кольцо силы (24)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=sh_ring2"><font style="font-size:9px;">Кольцо силы зверобоя (36)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=doubt_ring"><font style="font-size:9px;">Кольцо сомнений (19)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=rashness_ring"><font style="font-size:9px;">Кольцо стремительности (6)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=darkring"><font style="font-size:9px;">Кольцо теней (19)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=sring17"><font style="font-size:9px;">Кольцо хватки дракона (4)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=verve_ring"><font style="font-size:9px;">Перстень вдохновения (11)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=smring17"><font style="font-size:9px;">Печать единения (1)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=magring13"><font style="font-size:9px;">Печать заклинателя (14)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=ring&sort=0&art_type=sring10"><font style="font-size:9px;">Терновое кольцо (28)</font></a>';document.getElementById('mark_ring').innerHTML = '<a href="#" onclick="a2_ring(); return false">Кольца</a> <a href="/auction.php?cat=ring&sort=0">(376)</a>';}
        // function a_elements(){ document.getElementById('mark_info_elements').innerHTML = '&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=abrasive"><font style="font-size:9px;">абразив (27)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=snake_poison"><font style="font-size:9px;">змеиный яд (21)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=tiger_tusk"><font style="font-size:9px;">клык тигра (29)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=ice_crystal"><font style="font-size:9px;">ледяной кристалл (42)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=moon_stone"><font style="font-size:9px;">лунный камень (37)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=fire_crystal"><font style="font-size:9px;">огненный кристалл (34)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=meteorit"><font style="font-size:9px;">осколок метеорита (26)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=witch_flower"><font style="font-size:9px;">цветок ведьм (30)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=wind_flower"><font style="font-size:9px;">цветок ветров (28)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=fern_flower"><font style="font-size:9px;">цветок папоротника (13)</font></a><BR>&nbsp;&nbsp;&nbsp;<a href="/auction.php?cat=elements&sort=0&art_type=badgrib"><font style="font-size:9px;">ядовитый гриб (34)</font></a>';document.getElementById('mark_elements').innerHTML = '<a href="#" onclick="a2_elements(); return false">Элементы</a> <a href="/auction.php?cat=elements&sort=0">(321)</a>';}
        ...
        for (var j = 0; j < links.length; j++) {
            try {
                var link = links[j];
                // http://www.heroeswm.ru/auction.php?cat=shield&sort=0
                // http://www.heroeswm.ru/auction.php?cat=skeletons&sort=0#
                // http://www.heroeswm.ru/auction.php?cat=boots&sort=0&art_type=boots2
                var url = $.url(link.href);
                if (url.attr('path') != '/auction.php')
                    throw new Error('Непредвиденный путь');
                var cat = url.param('cat');
                if (cat === undefined)
                    throw new Error('Нет параметра cat');
                var id = url.param('art_type');
                if (id === undefined)
                    throw new Error('Нет параметра art_type');
                var title = link.firstChild.textContent;
                var matches = /(\S.*\S)\s+\(\d+\)/.exec(title);
                title = matches[1];
            } catch (e) {
                // Игнорируем все левые ссылки
                //GreaseMonkey.log(link.href + "\n" + e.message);
                continue;
            }
...
* @param Function onLoad
*/
Auction.loadArtsList = function(sourceDoc, onLoad) {

    function onRawLoad(doc, params) {
        GreaseMonkey.log('Список артефактов с рынка загружен.');
        var arts = Auction.parseArtsList(doc);
        if (onLoad !== undefined) {
            onLoad(arts);
        }
    }

    var url = 'http://' + sourceDoc.location.hostname + '/auction.php';
    GreaseMonkey.loadDoc(url, onRawLoad);

}


/** Сохранение минимальных и максимальных цен артефекта
* @param Object art
* @param Array prices
*/
Auction.saveMaxMinPrices = function(art, prices) {
...
function UserScriptGUI(context) {
    ...
    */
    this.createMenuButton = function(path, title, desc) {
        //GreaseMonkey.log('UserScriptGUI.createMenuButton([' + path.join(', ') + '], ' + title + ', ' + desc +')');
    ...
    */
    this.createMenu = function(path) {
        //GreaseMonkey.log('UserScriptGUI.createMenu([' + path.join(', ') + '])');
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
        var ppath = fpath.slice(0, fpath.length - 1);
        var pid = ppath.join('_');
    ...
    */
    this.toggleMenu = function(path) {
        //GreaseMonkey.log('UserScriptGUI.toggleMenu([' + path.join(', ') + '])');
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
        var ppath = fpath.slice(0, fpath.length - 1);
        var pid = ppath.join('_');
    ...
    */
    this.createWindowButton = function(path, title, desc) {
        //GreaseMonkey.log('UserScriptGUI.createWindowButton([' + path.join(', ') + '], ' + title + ', ' + desc +')');
        title = (title === undefined) ? path[path.length - 1] : title;
        desc = (desc === undefined) ? '' : desc;
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
    ...
    */
    this.createWindowOption = function(path, title, desc, def) {
        //GreaseMonkey.log('UserScriptGUI.createWindowOption([' + path.join(', ') + '], ' + title + ', ' + desc +')');
        title = (title === undefined) ? path[path.length - 1] : title;
        desc = (desc === undefined) ? '' : desc;
    ...
    */
    this.createWindowOptionBool = function(path, title, desc, def) {
        ...
        var ppath = fpath.slice(0, fpath.length - 1);
        var pid = ppath.join('_');
        //GreaseMonkey.log('path == [' + path.join(', ') + ']');
        //GreaseMonkey.log('fpath == [' + fpath.join(', ') + ']');
        //GreaseMonkey.log('ppath == [' + ppath.join(', ') + ']');
        if ($('#o' + fid, context).length === 0) {
    ...
    */
    this.createWindow = function(path, title, content) {
        //GreaseMonkey.log('UserScriptGUI.createWindow([' + path.join(', ') + '], ' + title + ', ' + content + ')');
    ...
    */
    this.toggleWindow = function(path) {
        //GreaseMonkey.log('UserScriptGUI.toggleWindow([' + path.join(', ') + '])');
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
    ...
    */
    this.createPopup = function(path, title, content) {
        //GreaseMonkey.log('UserScriptGUI.createPopup([' + path.join(', ') + '], ' + title + ', ' + content + ')');
        title = (title === undefined) ? '' : title;
        content = (content === undefined) ? '' : content;
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
    ...
    */
    this.getPopup = function(path) {
        //GreaseMonkey.log('UserScriptGUI.getPopup([' + path.join(', ') + '])');
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
        return $('#p' + fid, context);
    }
    ...
    */
    this.destroyPopup = function(path) {
        //GreaseMonkey.log('UserScriptGUI.destroyPopup([' + path.join(', ') + '])');
        var fpath = ['UserScriptGUI'].concat(path);
        var fid = fpath.join('_');
    ...
    */
    this.destroyPopupClass = function(popupClass) {
        //GreaseMonkey.log('UserScriptGUI.destroyPopupClass(' + className + ')');
...
function UserScriptFrameManager(context) {
    ...
    */
    this.createFrame = function(id, type, url) {
        //GreaseMonkey.log('id == ' + id + ', type == ' + type + ', url == ' + url);
        var iframe = $('#' + id, context);
        if (iframe.length === 0) {
            // Создание фрейма
    ...
    this.reloadFrame = function(id, delay) {
        if (delay === undefined) {
            ...
            try {
                ...
                GreaseMonkey.log('Ошибка перезагрузки фрейма:' + '\n' + e.message + '\n' + iframe.src);
            }
        } else {
            var self = this;
...
function UIObjectPreview(ref, autoclose) {
    ...
    */
    function togglePreview(refNode, objectId) {
        //GreaseMonkey.log('togglePreview(refNode, ' + objectId + ')');
        ...
        function previewCreate() {
            ...
            */
            ...
            function createCtrlRefresh() {
                ...
                $(result).css({'cursor': 'pointer', 'margin-left': '10px'}).text('Обновить').click(
                    function() {
            ...
            function createCtrlClose() {
                var result = refNode.ownerDocument.createElement('i');
                $(result).css({'cursor': 'pointer', 'margin-left': '10px'}).text('Закрыть').click(
                    function() {
                        togglePreview(refNode, objectId);
                    }
                );
    ...
    */
    ...
    //GreaseMonkey.log('ref.href == ' + ref.href);
    //GreaseMonkey.log('objectId == ' + objectId);
...
* @param Object def         Объект умолчанию
* @param Array timeFields   Поля штампов времени
* @return Object
*/
GreaseMonkey.loadObject = function(key, def, timeFields) {
    //GreaseMonkey.log('GreaseMonkey.loadObject(' + key + ', def, timeFields)');

    var str = GM_getValue(key, null);
    if (str === null)
        return def;

    // Карта полей штампов времени
    var timeMap = {};
    if (timeFields !== undefined)
        for (var i in timeFields)
            timeMap[timeFields[i]] = true;
    ...
    for (var i = 0; i < arr1.length; i++) {
        ...
        // Обработка полей штампов времени
        if (timeMap[attrName])
            obj[attrName] = Time.fromNumber(attrValue);
    }
...
* @param Array timeFields   Поля штампов времени
*/
GreaseMonkey.saveObject = function(key, obj, skipFields, timeFields) {
    //GreaseMonkey.log('GreaseMonkey.saveObject(' + key + ', obj, skipFields, timeFields)');

    // Карта пропускаемых полей
    var skipMap = {};
    if (skipFields !== undefined)
        for (var i in skipFields)
    ...
    for (var i in obj) {
        ...
        // Обработка полей штампов времени
        if (timeMap[attrName])
            attrValue = Time.toNumber(attrValue);
        // Представление массива
        if ($.isArray(obj[i])) {
            attrValue = '';
            for (var j in obj[i])
...
*/
Home.loadParse = function(onLoad) {
    //GreaseMonkey.log('Home.loadParse()');
...
* @param Function onLoad    Обработчик
*/
Home.load = function(onLoad) {
    //GreaseMonkey.log('Home.load()');

    var url = 'http://' + document.location.hostname + '/home.php';

    GreaseMonkey.loadDoc(url, onLoad);

}


/** Парсинг домашней страницы
*
* @param Document doc       Разбираемый документ
* @return Home
*/
Home.parse = function(doc) {
    //GreaseMonkey.log('Home.parse()');

    var home = {};
    home.nodes = {};

    // Ячейка: имя, номер, фракция
    var node = XPath.findFirst('/html/body/center/table/tbody/tr/td/table/tbody/tr[2]/td/table/tbody/tr/td/center', doc);
    if (node === null)
        throw new Error('Некорректный документ.');
    home.nodes.playerName = node;

    // Имя и номер игрока
    home.playerName = $('a', home.nodes.playerName).text();
    home.playerHref = $('a', home.nodes.playerName).attr('href');
    var res = /pl_info\.php\?id=(\d+)$/ig.exec(home.playerHref);
    home.playerId = Number(res[1]);

    // Имя и номер фракции игрока
    home.playerFracName = $('img[align=absmiddle]', home.nodes.playerName).attr('title');
    home.playerFracSrc = $('img[align=absmiddle]', home.nodes.playerName).attr('src');
    var res = /r(\d+).gif$/ig.exec(home.playerFracSrc);
    home.playerFracId = Number(res[1]);

    // Ячейка: умения, гильдии
    var node = XPath.findFirst('/html/body/center/table/tbody/tr/td/table/tbody/tr[2]/td/table/tbody/tr/td/table/tbody/tr/td[2]', doc);
    if (node === null)
        throw new Error('Некорректный документ.');
    home.nodes.playerSkills = node;
    home.playerSkills = Home.parseSkills(home.nodes.playerSkills);
    home.playerGuilds = Home.parseGuilds(home.nodes.playerSkills);
...
/** Парсинг умений
*/
Home.parseSkills = function(skillsNode) {

    var nodes = skillsNode;
    playerSkills = {};
    for (var i = 0; i < nodes.length; i++) {
        ...
        playerSkills[title] = skill;
        //GreaseMonkey.log(skill.title + ': ' + skill.value);
...
/** Парсинг гильдий
*/
Home.parseGuilds = function(guldsNode) {

    var text = $(guldsNode).text();
    playerGuilds = {};

    // Гильдия рабочих
    var re = new RegExp("Гильдия\\s*Рабочих:\\s*(\\d+)\\s*\\((\\S+)\\)\\s*\\+(\\S+)", 'g');
    var res = re.exec(text);
    playerGuilds.workersLevel = Number(res[1]);
    playerGuilds.workersValue = Number(res[2]);

    // Гильдия кузнецов
    var re = new RegExp("Гильдия\\s*Кузнецов:\\s*(\\d+)\\s*\\((\\S+)\\)\\s*\\+(\\S+)", 'g');
    var res = re.exec(text);
    playerGuilds.blacksmithsLevel = Number(res[1]);
    playerGuilds.blacksmithsValue = Number(res[2]);
...
Page.prototype.parse = function() {
    ...
    if (this.doc == null) {
        ...
        GreaseMonkey.log('this.doc.innerHTML == ' + this.doc);
        this.state = -1;
        return false;
    }
    ...
    if (this.doc.body == null) {
        GreaseMonkey.log(this + '\n' + 'Тело документа отсутствует');
        GreaseMonkey.log('this.doc.body == ' + this.doc.body);
        this.state = -1;
        return false;
    }
    ...
    if (this.doc.body.innerHTML == null) {
        GreaseMonkey.log(this + '\n' + 'Тело документа отсутствует');
        GreaseMonkey.log('this.doc.body.innerHTML == ' + this.body.doc.innerHTML);
        this.state = -1;
        return false;
    }
    ...
    try {
        ...
        if (this.doc.location !== null)
            GreaseMonkey.log('this.doc.location.href == ' + this.doc.location.href);
        GreaseMonkey.log('this.doc.body.innerHTML == ' + this.doc.body.innerHTML);
        this.state = -1;
        return false;
...
Page.prototype.parseHP = function() {
    ...
    try {
        ...
        var xpath = '//*[@id="heart"]';
        //var xpath = '/html/body/table/tbody/tr/td/table/tbody/tr/td[4]/table/tbody/tr[2]/td/div/object';
        var node = XPath.findFirst(xpath, this.doc);
        if (node == null) throw new Error('Узел по пути \'' + xpath + '\' не найден');
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            if (this.heroHP == null)
                throw new Error('Непредвиденное значение:' + '\n' + value);
            break;
        }
...
Page.prototype.parseMP = function() {
    ...
    try {
        var xpath = '//*[@id="mana"]';
        //var xpath = '/html/body/table/tbody/tr/td/table/tbody/tr/td[8]/table/tbody/tr[2]/td/div/object';
        var node = XPath.findFirst(xpath, this.doc);
        if (node == null) throw new Error('Узел по пути \'' + xpath + '\' не найден');
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            var matches = /param=(\d+(\.\d+)?)\|(\d+(\.\d+)?)\|(\d+)\|.*\|/ig.exec(value);
            if (matches === null)
                continue;
            this.heroMP = Number(matches[1]);
            this.heroMPMax = Number(matches[5]);
            if (this.hero !== null) {
...
PageMap.prototype.parse = function() {
    ...
    try {
        ...
        GreaseMonkey.log(this + '\n' + e.message);
        GreaseMonkey.log('this.doc.location.href == ' + this.doc.location.href);
        GreaseMonkey.log('this.doc.textContent == ' + this.doc.textContent);
        this.state = -1;
        return false;
...
PageObjectInfo.prototype.truncateContent = function() {
    ...
    try {
        ...
        if ((busy !== null) && (free !== null)) {
            ...
            node.appendChild(this.doc.createElement('br'));
...
PageObjectInfo.prototype.parseObjectResume = function(node) {
    ...
    // Ищем форму
    // <form method="POST" action="object_do.php" name="working">
...
var desc = {
    'details':
        ...
        +'</ul>',
    'homepage': {'title': 'userscripts.org', 'link': 'http://userscripts.org/users/hotamateurxxx'},
...
var conf = {
    ...
    'showChangeTime': {'type': 'bool', 'title': 'Показывать смену', 'def': true, 'desc': 'Отображение времени окончания смены.'},
    'showBalance': {'type': 'bool', 'title': 'Показывать баланс', 'def': true, 'desc': 'Отображение баланса предприятия.'},
    'showPlaces': {'type': 'bool', 'title': 'Показывать места', 'def': true, 'desc': 'Отображение рабочих мест предприятия.'},
    ...
    'enablePreview': {'type': 'bool', 'title': 'Предпросмотр объектов', 'def': true, 'desc': 'Возможность предпросмотра объектов с карты.'},
    'enableAutoclose': {'type': 'bool', 'title': 'Автозакрытие предпросмотра', 'def': true, 'desc': 'Автозакрытие окна предпросмотра.'},
    //'autosell': {'type': 'bool', 'title': 'Автопродажа ресурсов', 'def': false, 'desc': 'Автоматическая продажа ресурсов.'},
    //'dontSellOnWork': {'type': 'bool', 'title': 'Не продавать при работе', 'def': true, 'desc': 'Отключение автоматической продажи ресурсов при налиции формы устройства на работу.'},
    'showRealPayment': {'type': 'bool', 'title': 'Факт. зарплата', 'def': true, 'desc': 'Отображение на предприятиях фактической зарплаты (с учетом гильдии рабочих).'},
    ...
    //'sellCount': {'title': 'Количество для продажи', 'def': 1, 'desc': 'Количество ресурса при котором происходит автоматическая продажа.'},
    //'reloadTime': {'title': 'Время перезагрузки', 'def': 10, 'desc': 'Время перезагрузки страницы в секундах (0 - отключено).'}
}
...
* @return Number
*/
function calcRealPayment(payment, guildLevel) {
    //GreaseMonkey.log('calcRealPayment(' + payment + ', ' + guildLevel + ')');
    payment = Number(payment);
    guildLevel = Number(guildLevel);
    var mod = null;
    switch (guildLevel) {
        case 0: mod = 1.0; break;
        case 1: mod = 1.1; break;
        default: mod = 1.0 + 0.2 * (guildLevel - 1);
    }
    //GreaseMonkey.log('mod == ' + mod);
    var result = Math.floor(payment * mod);
    //GreaseMonkey.log('result == ' + result);
    return result;
}
...
function saveObjectInfo(objInfo) {
    GreaseMonkey.saveObject('ObjectInfo_' + objInfo.objectId, objInfo, [], ['updateTime']);
}
...
function loadObjectInfo(objectId) {
    return GreaseMonkey.loadObject('ObjectInfo_' + objectId, false, ['updateTime']);
}
...
function getChangeTimeStr(objInfo) {
    ...
    if (objInfo.changeTimeHours === undefined)
        return '';
    if (objInfo.changeTimeMins === undefined)
        return '';
...
function updateMapTable(page) {
    ...
    var cacheTime = Number(GM_getValue('cacheTime', conf.cacheTime.def));
    var actualTime = new Date(new Date() - 1000 * 60 * cacheTime);
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        var color = ((objInfo.updateTime === undefined) || (objInfo.updateTime < actualTime)) ? 'grey' : 'inherit';

        if (showChangeTime)
            $(table.rows[i].cells[1]).css({'color': color}).attr('align', 'center').html(getChangeTimeStr(objInfo));

        if (showPlaces)
            $(table.rows[i].cells[2]).css({'color': color}).attr('align', 'center').html(getPlacesStr(objInfo));

        if (showBalance)
            $(table.rows[i].cells[3]).css({'color': color}).attr('align', 'center').html(getBalanceStr(objInfo));

        var guildLevel = GM_getValue('guildLevel', 0);
        var str = '';
        // Предприятие может заплатить игроку
        var guildLevel = GM_getValue('guildLevel', 0);
        if (objInfo.balance > calcRealPayment(objInfo.payment, guildLevel))
            // На предприятии есть места
            if (Number(objInfo.placesFree) > 0)
                str = '»»»';
        $('a', table.rows[i].cells[5]).html(str);
    ...
    // Предпросмотр
    var enablePreview = GM_getValue('enablePreview', conf.enablePreview.def);
    var enableAutoclose = GM_getValue('enableAutoclose', conf.enableAutoclose.def);
    if (enablePreview) {
        var refs = table.getElementsByTagName('a');
        for (var i = 0; i < refs.length; i++)
...
function completeMap(page) {
    ...
    function objectInfoToRow(objInfo) {
        ...
        */
        ...
        var html =
            ...
            '<td class="wblight">&nbsp;<a style="text-decoration:none;" href="object-info.php?id=' + objInfo.objectId + '">' + str + '</a>&nbsp;</td>';
    ...
    */
    function appendMapFrame(div) {
        ...
        var enableCache = GM_getValue('enableCache', conf.enableCache.def);
...
function updateObjects(pageMap) {
    ...
    */
    function updateObject(list, idx) {
        ...
        */
        function onLoad(resp) {
            //GreaseMonkey.log('onLoad(resp)');

            var parser = new DOMParser();
            var doc = parser.parseFromString(resp.responseText, 'text/html');
            //GreaseMonkey.log('idx == ' + idx);
            //GreaseMonkey.log('doc.location == ' + doc.location);
            ...
            if (showChangeTime || showBalance || enableEcostat) {
                ...
                //GreaseMonkey.log('pageObj.state == ' + pageObj.state);
    ...
    //GreaseMonkey.log('list == ' + list.join(', '));
...
function updateObjectDetails(page) {
    ...
    objInfo.payment = page.objectPayment;
    objInfo.captcha = page.objectHasCaptcha ? 1 : 0;
    ...
    if (page.nodes.objectChangeTime !== null) {
        ...
        objInfo.changeTimeHours = Number(matches[1]);
        objInfo.changeTimeMins = Number(matches[2]);
        ...
        if (page.objectBalance > page.objectPayment) {
            ...
            delete objInfo.changeTimeHours;
            delete objInfo.changeTimeMins;
...
// Настройки скрипта
var enableSort = GM_getValue('enableSort', conf.enableSort.def);
var showChangeTime = GM_getValue('showChangeTime', conf.showChangeTime.def);
var showBalance = GM_getValue('showBalance', conf.showBalance.def);
var showPlaces = GM_getValue('showPlaces', conf.showPlaces.def);
var loadObjects = GM_getValue('loadObjects', conf.loadObjects.def);
var autosell = false;//GM_getValue('autosell', conf.autosell.def);
var dontSellOnWork = true;//GM_getValue('dontSellOnWork', conf.dontSellOnWork.def);
var showRealPayment = GM_getValue('showRealPayment', conf.showRealPayment.def);
var guildLevel = Number(GM_getValue('guildLevel', 0));
var sellCount = 0;//Number(GM_getValue('sellCount', conf.sellCount.def));
...
$(document).ready(
    ...
    function () {
        ...
        // Создание интерфейса
        gui = UserScriptGUI(top.document);
        ...
        if (url.attr('path') == '/home.php') {
            ...
            var home = Home.parse(document);
            GM_setValue('guildLevel', home.playerGuilds.workersLevel);
        ...
        // На странице информации об объекте
        if (url.attr('path') == '/object-info.php') {
            ...
            if (page.state === 1) {
                ...
                // Заголовок документа
                document.title = page.objectType;
                ...
                var table = page.nodes.objectOffersBuy.nextSibling;
                for (var i = table.rows.length - 1; i > 0; i--) {
                    ...
                    var title = String.trim(row.cells[0].textContent);
                    var price = Number(row.cells[2].textContent);
                    var matches = /(\d+\.\d+?)\s*\/\s*(\d+)/g.exec(row.cells[3].textContent);
                    var countReq = Math.floor(Number(matches[2]) - Number(matches[1]));
                    var countAv = Number(row.cells[5].textContent);
                    var count = Math.min(countReq, countAv);

                    if ((count * price) > (countMax * priceMax)) {
                        countMax = count;
                        priceMax = price;
                        formMax = form;
                        titleMax = title;
                ...
                if ((autosell) && (sellCount > 0) && (countMax >= sellCount)) {
                    if ((page.objectHasCaptcha) && (dontSellOnWork)) {
                        GreaseMonkey.log('Запрос на продажу: ' + titleMax + ' (' + countMax + ' ед.) отложен ради устройства на работу.');
                        document.title = 'Работа за ' + calcRealPayment(page.objectPayment, guildLevel) + ' золота';
                    } else {
                        GreaseMonkey.log('Отправлен запрос на продажу: ' + titleMax + ' (' + countMax + ' ед.).');
                        document.title = 'Продажа: ' + titleMax + ' (' + countMax + ' ед.).';
                        formMax.elements['count'].nextSibling.click();
                    }
                }
                ...
                if (window === top) {
                    ...
                    // Автоматическая продажа
                    if (conf.autosell !== undefined) {
                        var bAuto = gui.createMenuButton(['ObjectResourcesAuto'], 'Продажа ресурсов');
                        bAuto.css({'color': autosell ? 'black' : 'silver'});
                        bAuto.click(function() {
                            var autosell = GM_getValue('autosell', conf.autosell.def);
                            autosell = !autosell;
                            GM_setValue('autosell', autosell);
                            GreaseMonkey.log('Автоматическая продажа ресурсов ' + (autosell ? 'включена' : 'выключена') + '.');
                            bAuto.css({'color': autosell ? 'black' : 'silver'});
                        });
                    }

                    if (!page.objectHasCaptcha) {

                        // Обновление страницы
                        var reloadTime = 0;//Number(GM_getValue('reloadTime', conf.reloadTime.def));
                        if (reloadTime > 0) {
                            var reloadEnabled = GM_getValue('reloadEnabled', false);
                            var reloadCounter = reloadTime;
                            var reloadTimeout = undefined;
                            var reloadInterval = undefined;

                            function setReloadTimeout() {
                                if (reloadEnabled) {
                                    reloadCounter = reloadTime;
                                    reloadTimeout = setTimeout(function() { window.location.reload(true); }, reloadTime * 1000);
                                    reloadInterval = setInterval(function () {reloadCounter--; bReload.val('Обновление (' + reloadCounter + ')');}, 1000);
                                    bReload.css({'color': 'black'});
                                    bReload.val('Обновление (' + reloadCounter + ')');
                                } else {
                                    clearTimeout(reloadTimeout);
                                    clearInterval(reloadInterval);
                                    bReload.css({'color': 'silver'});
                                    bReload.val('Обновление');
                                }
                            }

                            function onButtonClick() {
                                reloadEnabled = GM_getValue('reloadEnabled', false);
                                reloadEnabled = !reloadEnabled;
                                GM_setValue('reloadEnabled', reloadEnabled);
                                GreaseMonkey.log('Собственное обновление страницы информации об объекте ' + (reloadEnabled ? 'включено' : 'выключено') + '.');
                                setReloadTimeout();
                            }

                            var bReload = gui.createMenuButton(['ObjectResourcesReload'], 'Обновление');
                            bReload.click(function() {onButtonClick();});
                            setReloadTimeout();
                ...
                // Добавление информации о фактической зарплате
                if (showRealPayment) {

                    if (Number(guildLevel) > 0) {

                        var nodePayment = page.nodes.objectPayment;
                        var realPayment = calcRealPayment(page.objectPayment, guildLevel);
                        var realPaymentStr = Auction.htmlPrice(realPayment, undefined, true);

                        var bothPayment = GM_getValue('bothPayment', false);
                        if (bothPayment) {
                            $(nodePayment.rows[0]).append(
                                '<td>&nbsp;&nbsp;&nbsp;</td>' +
                                '<td>(</td><td>' + realPaymentStr + '</td><td>)</td>'
                            );
                        } else {
                            $(nodePayment.rows[0].cells[1]).html(
                                realPaymentStr
                            );
                        }
        ...
        // На странице действия над объектом
        if (url.attr('path') == '/object_do.php') {

            // Возвращаемся на страницу объекта
            document.location.replace(
                 url.attr('protocol') + '://' + url.attr('host') + '/object-info.php?id=' + url.param('id')
            );
