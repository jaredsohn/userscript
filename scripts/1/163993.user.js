// ==UserScript==
// @name        Facebook: Hizli Mesaj Silme Plus + Last Update
// @description Mesajlar bölümündeki konusmalari kolay bir sekilde silebilirsiniz.
	...
	var run = function() {
		...
		if (document.getElementById("MessagingShelf")) {
			...
			if (!document.getElementById("QuickDelete")) {
				...
				if (tid) {
					var elem = q('<a class="uiButton uiButtonConfirm uiToolbarItem" id="QuickDelete" role="button" rel="dialog"><span class="uiButtonText">Sohbeti Sil</span></a>');
		...
		q(this).find('a[class~="archiveLink"]').each(function() {
			...
			a.find('span[class~="uiTooltipText"]').html("Sil");
			a.find('input').attr("value", "X");
