...
function handleVeryXml(xo,vc,doc,kw,fields) {
    ...
    if (l>0) {
        ...
        for (var i=0;i<10;i++) {
            ...
            t=topics.item(i);
            title=t.getElementsByTagName('title').item(0).firstChild.nodeValue;
...
function verydou() {
    ...
    if (doc.location.href.search(/douban.com\/subject\/[0-9]+\/(?:\?.*)?$/) > 0) {
        ...
        try {
            ...
            //     }
            // }
            if (doc.location.href.search(/movie/)>0) {
                        //find_cat='%E7%94%B5%E5%BD%B1';
                fields='entries';
            } else if (doc.location.href.search(/music/)>0) {
                find_cat=24;
                fields='folders';
            } else if (doc.location.href.search(/book/)>0) {
                find_cat=69;
                fields='folders';
            }
        } catch (ex) {
    ...
    //构建url
    url='http://www.verycd.com/search/'+fields+'?kw='+kw;
    if (fields=='entries'){
	    url+='&fields=cname,ename,alias&format=rss';
	} else {
		url+='&fields=title&format=rss';	
	}
