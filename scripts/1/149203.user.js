// ==UserScript==
// @name        MyEpisodes.com Links to KickAssTorrents
// @namespace   http://userscripts.org/users/416433
// @description It Creates Links next to each episode and season to search at KAT.ph
...
// let it search on KickAssTorrent
...
if(seasonHeaders.length>0){
   ...
   table.innerHTML+="<tbody><tr class='header' align='center'><td style='border-bottom: 1px solid black;'>KAT</td></tr><tr align='center'><td><a href='"+ createSimpleSearchURL(showName) +"' target='_blank' class='flatbutton'>Search</a></td></tr></tbody>";
...
function getTPBcell(linkURL, fadeLink){
   ...
   link.appendChild(document.createTextNode("KAT"));
   if(fadeLink){
...
// Create the KickAssTorrent search URL
function createSearchURL(showName, episodeNumber, episodeName){
   searchterm = showName.replace(/'/g,"") + " " + trimNumber(episodeNumber);
   return createSimpleSearchURL(searchterm);
}

function createSimpleSearchURL(searchterm){
   var url = "http://kat.ph/usearch/" + searchterm + searchAttributes + '/';
   return url;
}
