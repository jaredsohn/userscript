...
// @include        http://hockey.fantasysports.yahoo.com/hockey/91435*
...
function handleRosterData( responseDetails )
{
	...
	var matches = location.href.match( /.*\/\d+\/(\d+).*/ );
	if ( !matches )
		matches = location.href.match( /.*\/\d+\/team.*&mid=(\d+).*/ );
	...
	if ( ownerID && owners.hasOwnProperty( ownerID ) )
	{
		...
		if ( lastRoster && tableLegend )
		{
			...
			if ( GET_STATS_AUTOMATICALLY )
			{
				getCareerStats( ownerID );
			}
			else
			{
...
function getCareerStats( ownerID )
{
	var owner = owners[ ownerID ];
	...
	for ( var iPlayer = 0; iPlayer < playerElements.snapshotLength; iPlayer++ )
	{
		...
		if ( !isNaN( parseInt( owner.players[ iPlayer ].id, 10 ) ) )
		{
			...
			GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://sports.yahoo.com/nhl/players/' + owner.players[ iPlayer ].id + '/career',
...
function handlePlayerData( responseText, player )
{
	...
	for ( var iTable = 0; iTable < tables.length; iTable++ )
	{
		...
		var matches = table.rows[ 0 ].cells[ 0 ].innerHTML.match( /^(?:&nbsp;)*(stats)$/i );
		if ( matches )
		{
			var dataIndexes = {};
			...
			for ( var iRow = 2; iRow < table.rows.length; iRow++ )
			{
				...
				else if ( /career/i.test( row.cells[ 0 ].innerHTML ) )
				{
					...
					if ( gamesStarted )
					{
						...
						player.dataCell.innerHTML = gamesStarted + ' games played';
					}
					else
					{
						...
						if ( gamesPlayed )
						{
							...
							player.dataCell.innerHTML = gamesPlayed + ' games played';
						}
					}
