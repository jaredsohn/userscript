...
// @description    Adaptation userscripts.org de l'extension Google Chrome JVCPremium, Voir http://www.jeuxvideo.com/forums/1-1000021-1051825-1-0-1-0-chrome-jvcpremium-enfin-en-beta.htm
...
(function() {
	...
	s.type = 'text/css';
	s.rel = 'stylesheet';
	s.innerHTML = 'p.blocHeader a { float: right; } p.blocHeader img { vertical-align: bottom; } div.optionsForm { background: url(http://image.jeuxvideo.com/css_img/defaut/sep_999.gif) left top repeat-x; margin-bottom: 1em; margin-top: 6px; padding-top: 1em; } div.optionsForm p + p { margin-top: 1em; } div.optionsForm input { vertical-align: top; } div.optionsForm label { font-weight: normal; margin-left: 3px; vertical-align: middle; } li.pseudo strong.male { transition: color 200ms; -webkit-transition: color 200ms; -moz-transition: color 200ms; } li.pseudo strong.male:hover { color: #09F; } li.pseudo strong.female { transition: color 200ms; -webkit-transition: color 200ms; -moz-transition: color 200ms; } li.pseudo strong.female:hover { color: #F39; } li.pseudo strong.banned { transition: text-decoration 200ms; -webkit-transition: text-decoration 200ms; -moz-transition: text-decoration 200ms; } li.pseudo strong.banned:hover { text-decoration: line-through; } li.pseudo span { background-image: url(http://image.noelshack.com/fichiers/2012/19/1336747077-Rangs_Updated_By_Cartouchiere.png); display: inline-block; height: 13px; margin-left: 3px; width: 16px; } li.pseudo span.carton { background-position: 0px -248px; } li.pseudo span.bronze { background-position: 0px -278px; } li.pseudo span.argent { background-position: 0px -308px; } li.pseudo span.or { background-position: 0px -338px; } li.pseudo span.rubis { background-position: 0px -368px; } li.pseudo span.saphir { background-position: 0px -398px; } li.pseudo span.emeraude { background-position: 0px -428px; } li.pseudo span.diamant { background-position: 0px -458px; } div.msg div.profilePicture { position: absolute; right: 100%; top: -1px; transition: opacity 200ms; -webkit-transition: opacity 200ms; -moz-transition: opacity 200ms; } div.msg div.profilePicture img { border: 1px solid white; box-shadow: 0px 0px 7px black; position: relative; right: 12px; } table.lightbox { background-color: rgba(0, 0, 0, 0); height: 100%; left: 0px; position: fixed; top: 0px; transition: background-color 500ms; -webkit-transition: background-color 500ms; -moz-transition: background-color 500ms; width: 100%; z-index: -1; } table.lightbox iframe { box-shadow: 0px 0px 30px black; height: 0px; transition: all 500ms; -webkit-transition: all 500ms; -moz-transition: all 500ms; width: 0px; } #liste_topics a.lastViewLink { display: inline; font-size: 100%; font-weight: normal; } #liste_topics a.lastViewLink:visited { color: #0A77B8; } div.msg:target { background-color: #EBFAEB; }';
...
(function() {
	if(location.search == '?options') {
		...
		var pseudo = document.querySelector('td#compte a strong') ? document.querySelector('td#compte a strong').textContent : 'jeune pomme';
		...
		html += '<p class="blocHeader"><strong>Bonjour ' + pseudo + ' <img src="http://image.jeuxvideo.com/smileys_img/18.gif"></strong>';
		html += '<a href="' + href + '"><img src="http://image.noelshack.com/fichiers/2012/15/1334493015-previousPage.gif"></a></p>';
		html += '<div class="optionsForm">';
		html += '<p><input id="profileInfos" name="profileInfos" type="checkbox"' + (function() { return options.get('profileInfos') ? ' checked' : '' })() + '>';
		html += '<label for="profileInfos">Infos de profil (genre, grade, image)</label></p>';
		html += '<p><input id="lastViewLink" name="lastViewLink" type="checkbox"' + (function() { return options.get('lastViewLink') ? ' checked' : '' })() + '>';
		html += '<label for="lastViewLink">Lien du dernier post visité pour chaque topic</label></p>';
		html += '<p><input id="sameTimePosts" name="sameTimePosts" type="checkbox"' + (function() { return options.get('sameTimePosts') ? ' checked' : '' })() + '>';
			...
			if(xhr.readyState == 4 && xhr.status == 200) {
				...
				callback((function(response) {
					...
					if(status) {
						...
						var picture = response.querySelector('div#pt_avatar div.portlet-content img').src;
						picture = picture == 'http://image.jeuxvideo.com/avatars/default.jpg' ? false : picture;
						return {
							gender: gender,
							rank: rank,
							picture: picture,
...
(function() {
	if(options.get('profileInfos') && /^\/forums\/1|3/i.test(location.pathname) && location.search != '?options') {
		...
		for(var i = 0; i < pseudos.length; i++) {
			(function(i) {
				profileInfos.get('http://www.jeuxvideo.com/profil/' + pseudos[i] + '.html', function(response) {
					if(response) {
						...
						a.addEventListener('click', function() {
							...
							iframe.src = 'http://www.jeuxvideo.com/profil/' + pseudos[i] + '.html';
						...
						img.src = response.gender ? 'http://image.noelshack.com/fichiers/2012/16/1334668105-profileMale.gif' : 'http://image.noelshack.com/fichiers/2012/16/1334668105-profileFemale.gif';
						a.appendChild(img);
						lines[i].parentNode.insertBefore(a, lines[i].parentNode.querySelector('a[target="profil"]'));
					}
					else {
			...
			if(dates[i - 1].textContent == dates[i].textContent) {
				(function(posts) {
					for(var i = 0, span; i < posts.length; i++) {
						posts[i].firstChild.nodeValue = posts[i].firstChild.nodeValue.replace(/([0-9]{2}:[0-9]{2}:[0-9]{2})/i, '');
