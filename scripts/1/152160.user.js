...
// @require        http://kkcdn-static.kaskus.co.id/themes_2.0/js/editor/jquery.markitup.js
// @require        http://kkcdn-static.kaskus.co.id/themes_2.0/js/editor/kaskus_bbcode.js
...
//
// Changelog:
// 47.7.5599 (Approx. 30 Minutes After 47.6.2889)
// QR Wide Fix
// Settings And Help Fix
//              
// 47.6.2889 (December 16th, 2012)
// Settings Unlocked!
// JQuery Updated!
// New Capcay Lock
//              
// 46.9.0299 (November 13th, 2012)
// Updated JQuery (Less False Capcay)
// Crashed AJAXLOAD Fix
// Back To Old Capcay Lock
//              
// 10.11.12 (November 10th, 2012)
// Change Capcay Lock Into Bung Tomo (Heroes Day)
// Divided Into 2 Version, SD Version And HD Version
//              
// 44.4.4444 R2 (November 8th, 2012)
// Multi Quote Level Is Disabled (Thx To Adamantoi)
// Work In Kaskus.co.id (Without www)(Thx To Adamantoi)
// Repaired Default Siggy
// Remove Unsupported Font (Thx To Adamantoi)
// Repaired Gangnam Style Emoticon              
//              
// 44.4.4444 (November 5th, 2012)
// Fix Gangnam Style Colour Change
// (New!) Sleep Emoticon (See "Jumbo" Emot Button To Activate It)
//              
// 43.9.6049 (November 5th, 2012)
// Fix Colour Change
//              
// 43.9.4930 (November 5th, 2012)
// Change Segoe UI Into Comic Sans MS (Good Bye Segoe UI :()
//
// 43.9.2192 (November 4th, 2012)
// Wider text range (16:9 only)
// (New!) Touch Mode Gen 2 (more touch-friendly button)
// More fresh (With new colour button)
//              
// 42.7.8827 (November 3rd, 2012)
// More Language Updates
// (New!) Gangnam Style Emoticon (See "Jumbo" Emot Button To Activate It)
//
// 41.2.3384 (November 2nd, 2012)
// New Capcay Lock
// (New!) Touch Mode (1st Kaskus QR With Touch Mode :D)
// Internationalized Language
// Fixed Some Bugs              
// Updated Gangnam Style
//               
// 41.2.3384 (November 1st, 2012)
// Fixed Several Bug              
//              
// 40.6.4852 (November 1st, 2012)
// Redirect Last Post
// New Capcay Lock             
//
// 39.2.2248 (October 30th, 2012)
// Refubrished Engine
// ############### GM related codes ##################
...
//################ attach recapcay script #################
	...
	myscript2.setAttribute('src','http://www.google.com/recaptcha/api/js/recaptcha.js');
	document.body.appendChild(myscript2);
	
	var ajax_file_upload = document.createElement('script');
	ajax_file_upload.setAttribute('src','http://kkcdn-static.kaskus.co.id/themes_2.0/js/ajaxfileupload.js');
...
$(document).ready(function(){
	...
	if(typeof(Storage)!=="undefined")
  	...
  	{
  		alert("Your browser does not support local storage, please update it to the new version");
  	}
			...
			'<tr>'+
				'<td>'+
						'<div class="reply-message" id="pesan">'+
							...
							//############## save siggy button #################
							'<a id="btnSaveSiggy" href="javascript:void(0);" class="button small red" >Save Siggy</a>'+
					...
					'<div id="timer_div" class="timer">&nbsp</div>'+	
						'<div class="capcay_div">'+
							'<fieldset class="fieldset">'+
								...
								//##### gbr jengkol ###############
									'<div id="div_gbrCapcayLock">'+
										'<img id="gbrCapcayLock" src="http://sadpanda.us/images/1294064-60Z5YA0.jpg"><div class="clear_both"></div>'+
									'</div>'+
							  ...
							  '<input type="submit" accesskey="s" value="Check Your Post" name="preview" id="preview" class="button large blue" style="float:right;margin:0px 5px;">'+							  
							  ...
							  '<li>Klik Ryouko Asakura jika capcay tidak muncul</li><li>Save siggy dahulu sebelum melakukan preview</li><li>Buka semua spoiler hotkey CTRL+G/SHIFT+X</li>'+
							  '<li>Biasakan ganti IP sehari sekali</li><li>Jangan lupa selalu mematikan komputer sebelum tidur (Agar tidak meledug :hammer)</li></ul></div>'+
							'</fieldset>'+
						'</div>'+
					//################ end capcay ##########################
					'</td></tr>'+
...
'</div>'
	...
	'<div class="emotButtonWrapper">'+
		...
		'<a class="emotTab" id="gaya" href="javascript:void(0);">Gangnam Style</a>'+
	...
	'<b>Gangnam Style:</b>'+
	'<input type="radio" name="GangnamStyle" id="rbGangnamStyleOn"  class="rbSiggy"/><label for="rbGangnamStyleOn" class="clear_left jarak5">On</label>'+
	'<input type="radio" name="GangnamStyle" id="rbGangnamStyleOff"  class="rbSiggy"/><label for="rbGangnamStyleOff" class="clear_left jarak5">Off</label>'+
	'<br><br>'+
	...
	function setGangnamStyleStatus(status){
		localStorage.setItem("QR_postStyleStatus",status);
	}
	function getGangnamStyleStatus(){
		return localStorage.getItem("QR_postStyleStatus");
	}
	function setGangnamStyleSetting(setting){
		localStorage.setItem("QR_postStyleSetting",setting);
	}
	function getGangnamStyleSetting(){
		return localStorage.getItem("QR_postStyleSetting");
	}
	function setGangnamStyleTemplate(template){
		localStorage.setItem("QR_postStyleTemplate",template);
	}
	function getGangnamStyleTemplate(){
		return localStorage.getItem("QR_postStyleTemplate");
	}
	function setGangnamStyleSettings(){
		var styleSetting = getGangnamStyleSetting().split(",");
		if(styleSetting[0]=="0"){
	...
	function updateGangnamStyleSetting(arrNum,value){
		var oldSetting = getGangnamStyleSetting().split(",");
		...
		setGangnamStyleSetting(newSetting);
		updateGangnamStylePreview();
	}
	function updateGangnamStylePreview(){
		var setting = getGangnamStyleSetting().split(",");
		var temp = loremIpsum;
	...
	function Mainkan(){
		//############### variable setup #######################
		var DEFAULT_SIGGY = '[FONT="Comic Sans MS"][RIGHT][size="1"]Powered By Duty™ Engine 47.7.5599 (SD Version)[/size][/RIGHT][/FONT]';
		var temp_siggy = "\n";
		var use_siggy = "1";
		...
		if((getGangnamStyleStatus()==null) || (getGangnamStyleStatus()=="")){
			setGangnamStyleStatus("0");
		}
		if((getGangnamStyleSetting()==null) || (getGangnamStyleSetting()=="")){
			setGangnamStyleSetting(styleSetting);
		}
		if((getGangnamStyleTemplate()==null) || (getGangnamStyleTemplate()=="")){
			getGangnamStyleTemplate(styleTemplate);
		}
		...
		$("#gaya").toggle(function(){
			...
			$("#siggyWrapper").hide();	
			$("#styleWrapper").show();
			if(getGangnamStyleStatus()=="0"){
				$("#rbGangnamStyleOff").prop("checked",true);
				$("#div_styleContainer").hide();
			}
			else{
				$("#rbGangnamStyleOn").prop("checked",true);
				setGangnamStyleSettings();
				updateGangnamStylePreview();
	    ...
	    //############ Style related code ###############
	    $("#rbGangnamStyleOn").click(function(){
			setGangnamStyleStatus("1");
			setGangnamStyleSettings();
			$("#div_styleContainer").show();
			scrollToStyle();
		})
		
		$("#rbGangnamStyleOff").click(function(){
			setGangnamStyleStatus("0");
			$("#div_styleContainer").hide();
		})
	    ...
	    $("#txtChosenColor").blur(function(){
	    	...
	    	updateGangnamStyleSetting(5,$("#txtChosenColor").val());
	    })
	    ...
	    $("li.fontList").click(function(){
	    	...
	    	updateGangnamStyleSetting(0,$(this).text());
	    	$("#fontTypeList").hide();
	    })
	    $("#cbBold").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGangnamStyleSetting(1,"1");
	    	}
	    	else{
	    		updateGangnamStyleSetting(1,"0");
	    	}
	    })
	    $("#cbItalic").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGangnamStyleSetting(2,"1");
	    	}
	    	else{
	    		updateGangnamStyleSetting(2,"0");
	    	}
	    })
	    $("#cbUnderline").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGangnamStyleSetting(3,"1");
	    	}
	    	else{
	    		updateGangnamStyleSetting(3,"0");
	    	}
	    })
	    $("#rbAlignLeft").click(function(){
	    	updateGangnamStyleSetting(4,"0");
	    })
	    $("#rbAlignCenter").click(function(){
	    	updateGangnamStyleSetting(4,"1");
	    })
	    $("#rbAlignRight").click(function(){
	    	updateGangnamStyleSetting(4,"2");
	    })
	    //######### Preview click #########
		...
		$("#postreply").submit(function(){
			...
			if((getGangnamStyleStatus()=="1") && (postAttemp == 0)){
				var temp = getGangnamStyleTemplate();
