// ==UserScript==
// @name           BrickLink Store Links
// @description    Provides link to Catalog and PriceGuide on the BrickLink Store page
// @icon           http://icdn.pro/images/en/b/r/brick-link-icone-5096-32.png
// @namespace      http://www.lbreuss.ch/
// @author         Leo Breuss (derived from BrickLink Store Options by Ng Hun Yang)
// @match          *://*.bricklink.com/storeDetail.asp*
// @grant          none
// @version        1.0
// @updateURL      http://userscripts.org/scripts/source/173727.user.js
// ==/UserScript==
// Credits go to Ng Hun Yang, the Author of BrickLink Store Options 1.00. This script was spawned off of his work.
...
// -----------------------------------------------------------------------------
// Maybe these functions could be loaded from 
(function() {
...
// Helper function. Search the colorList for a matching color string.
...
function process_items() {
  /** Return the part type and id by looking for the first href matching a pattern like ...?P=2686c01...
   *  @return it as type=P, id=2686c01 */
  function get_part_info(row) {
    if(row.children.length < 1)
      return false;

    // In the table row, look for the first HTML anchor <a>, which is the item image link.
    // https://developer.mozilla.org/en-US/docs/Web/API/document.querySelector
    var hElm = row.children[0].querySelector("a");
    if(!hElm)
      return false;
      
    // match the href part "?P=32311"  of <a id="imgLink0" rel="blcatimg" href="/catalogItemPic.asp?P=32311&viewType=s"
    if(!hElm.getAttribute("href").match(/\?(\w)=([0-9a-zA-Z-]+)/))
      return false;
    // return it as type=P, id=32311
    return { type: RegExp.$1, id: RegExp.$2 };
    }

  /** @return "N" if the part is New, "U" for a used one. */
  function get_new_status(row, part_info) {
    // skip table rows, that have 1 or 0 columns
  ...
  function add_link(parent_node, url, text, title) {
    var link_node = dom.cE("a");
    dom.attr(link_node, "href", url);
    link_node.textContent = text;
    if(title) {
      dom.attr(link_node, "title", title + " (This link by GM BrickLink Store Links script)");
    }
    dom.append(parent_node, dom.cT("\u00a0\u00a0"));
    dom.append(parent_node, link_node);
  }

  // -------------------------------------------
  // The createDiv() script executionstarts here
  // -------------------------------------------
  // Entry point is the first anchor with the attribute rel="blcatimg" (catalog image). This is quite reliably identifiable.
  var anchor = doc.querySelector("a[rel=blcatimg]");
  // From there we navigate up until we are on the table
  var tbl = anchor.parentNode.parentNode.parentNode.parentNode;
  // Then we iterate over all table rows with class="tm"
  ...
  for(var i = 0; i < rows.length; ++i) {
    ...
    // add "N" or "U" to part_info.new_status
    get_new_status(row, part_info);

    // textContent is the description: "White Technic, Gear 24 Tooth Clutch". 
    // hlp.getColorId will try to translate the literal color to the color id.
    // Note: we need the color for the price guide.
    part_info.color = hlp.getColorId(row.children[2].textContent);

    // win.console.log(part_info);
    
    // Add the links to the links of the Description column, inside the font tag.
    var descr_font_node = row.children[2].querySelector("font.fv");    
    add_link(descr_font_node, "/catalogItem.asp?P=" + part_info.id, "(Catalog)", "View Article in Catalog");
    add_link(descr_font_node, "/catalogPG.asp?P=" + part_info.id + "&colorID=" + part_info.color, "(PG)", "View Article in Price Guide");
...
if(doc.querySelector("a[rel=blcatimg]")) {
  // the querySelector is not null, this store page has at least one item in the list.
  process_items();
}
