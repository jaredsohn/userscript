...
top == self){
	...
	setTimeout(function(){
		...
		$.each($('a'),function(k,v){
			if($(v).text().toLowerCase().indexOf('anti-cheat') >= 0 ){
...
self == top){
	...
	if($('frame:nth(1)').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('juicybux')!=-1)cheatCheck = [2];
		if(wparent.location.host.indexOf('luckyptc')!=-1)cheatCheck = [6,3];
		if(wparent.location.host.indexOf('storybux')!=-1)cheatCheck = [1,5,12,16,18,10];
		if(wparent.location.host.indexOf('clickbux')!=-1)cheatCheck = [71,75,60,45,57];
		...
		if(wparent.location.host.indexOf('idw-silverclix')!=-1)cheatCheck = [8,14,3,15,20];
		...
		if(wparent.location.host.indexOf('arrowptc')!=-1)cheatCheck=[10,4];
		if(wparent.location.host.indexOf('holidaygiftsptc')!=-1)cheatCheck=[4,8];
		...
		if(wparent.location.host.indexOf('javaklix')!=-1)cheatCheck=[];
		...
		if(wparent.location.host.indexOf('cashtravel')!=-1)cheatCheck=[23];
