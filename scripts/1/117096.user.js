// ==UserScript==
// @name			CaOs Compactador
// @namespace		http://userscripts.org/users/55448
// @description		CaOs Compactador de Batallas (y Espionajes... coming back soon)
// @version			0.9.0
// @author			ApoCaLypSe
// @creator			ApoCaLypSe
// @updateURL		http://userscripts.org/scripts/source/117096.meta.js
// @downloadURL		https://userscripts.org/scripts/source/117096.user.js
...
var SCRIPT = {
  name: "CaOs Compactador"
  ,url: "http://userscripts.org/scripts/show/117096"
  ,version: "0.9.0"
  ,funciona_ok: "5.4.99.99"
  ,alianzaimg: "http://DeAThCuLTArMaGeDDon.com/images/CaOs.Presents.png"
  ,alianzatxt: "CaOs [DeATh CuLT ArMaGeDDon]"
  ,alianzaurl: "http://DeAThCuLTArMaGeDDon.com"
  ,jugadorimg: "http://DeAThCuLTArMaGeDDon.com/images/DCA/CaOs.png"
  ,reciclaimg: "http://DeAThCuLTArMaGeDDon.com/images/CaOs.Reciclado.png"
  ,separadimg: "http://DeAThCuLTArMaGeDDon.com/images/Separators/CaOs.png"
};
...
function codificar(patron, tipo, version) {
    ...
    if(tipo == "HTML") {
        ...
        marcas = [
			[/{CAOS}/gi, '<a href="'+SCRIPT.alianzaurl+'" target="_CaOs" title="'+SCRIPT.alianzatxt+'"><img src="'+SCRIPT.alianzaimg+'" border=0 alt="'+SCRIPT.alianzatxt+'"></a>'],
			[/{CAOS_I}/gi, '<img src="'+SCRIPT.jugadorimg+'" border=0 alt="">'],
			[/{CAOS_R}/gi, '<img src="'+SCRIPT.reciclaimg+'" border=0 alt="">'],
			[/{CAOS_S}/gi, '<img src="'+SCRIPT.separadimg+'" border=0 alt="">'],
			[/{DEDICAS}/gi, '</center><b>DEDICAS:</b><br>&laquo; INSERTAR DEDICAS AQUÍ &raquo;<center>'],
			[/{HISTORIA}/gi, '</center><b>HISTORIA:</b><br>&laquo; INSERTAR HISTORIA AQUÍ &raquo;<center>'],
			[/{RECICLAJE}/gi, '</center><b>RECICLAJE:</b><br>&laquo; INSERTAR DETALLES DE RECICLAJE AQUÍ &raquo;<center>'],
            [/{B}/gi, '<b>'],
            [/{\/B}/gi, '</b>'],
            [/{I}/gi, '<i>'],
            [/{\/I}/gi, '</i>'],
    ...
    if(tipo == "OGame") {
        ...
        marcas = [
			[/{CAOS}/gi, '[url='+SCRIPT.alianzaurl+'][img]'+SCRIPT.alianzaimg+'[/img][/url]'],
			[/{CAOS_I}/gi, '[img]'+SCRIPT.jugadorimg+'[/img]'],
			[/{CAOS_R}/gi, '[img]'+SCRIPT.reciclaimg+'[/img]'],
			[/{CAOS_S}/gi, '[img]'+SCRIPT.separadimg+'[/img]'],
			[/{DEDICAS}/gi, '[/ALIGN][SPOILER=DEDICAS:]&laquo; INSERTAR DEDICAS AQUÍ &raquo;[/SPOILER][ALIGN=CENTER]'],
			[/{HISTORIA}/gi, '[/ALIGN][SPOILER=HISTORIA:]&laquo; INSERTAR HISTORIA AQUÍ &raquo;[/SPOILER][ALIGN=CENTER]'],
			[/{RECICLAJE}/gi, '[/ALIGN][SPOILER=RECICLAJE:]&laquo; INSERTAR DETALLES DE RECICLAJE AQUÍ &raquo;[/SPOILER][ALIGN=CENTER]'],
            [/{B}/gi, '[B]'],
            [/{\/B}/gi, '[/B]'],
            [/{I}/gi, '[I]'],
            [/{\/I}/gi, '[/I]'],
    ...
    if(tipo == "phpBB") {
        ...
        marcas = [
			...
			[/{CAOS_R}/gi, '[img]'+SCRIPT.reciclaimg+'[/img]'],
			[/{CAOS_S}/gi, '[img]'+SCRIPT.separadimg+'[/img]'],
			[/{DEDICAS}/gi, '[/CENTER][SPOILER=DEDICAS:]&laquo; INSERTAR DEDICAS AQUÍ &raquo;[/SPOILER][CENTER]'],
			[/{HISTORIA}/gi, '[/CENTER][SPOILER=HISTORIA:]&laquo; INSERTAR HISTORIA AQUÍ &raquo;[/SPOILER][CENTER]'],
			[/{RECICLAJE}/gi, '[/CENTER][SPOILER=RECICLAJE:]&laquo; INSERTAR DETALLES DE RECICLAJE AQUÍ &raquo;[/SPOILER][CENTER]'],
            [/{B}/gi, '[B]'],
            [/{\/B}/gi, '[/B]'],
            [/{I}/gi, '[I]'],
            [/{\/I}/gi, '[/I]'],
   ...
   if(tipo == "phpBB3") {
        ...
        patron = patron.replace(/{ENLACE_SCRIPT}/gi, SCRIPT.version);

        for(var i = 0; i < colores.length; i++)
            patron = patron.replace(colores[i][0],'');
    }
...
function getCuadrosBBCode(patron) {
   ...
   html += '</td></tr><tr><td>'
   // foro phpBB3
   html += '<b><font color=#FE9A2E>CC/MP OGame:</font></b><br>';
   html += '<textarea id="txtBB" name="txt_phpBB" style="background-color:#1F273C;width:200px;height:100px;border: 2px solid #FFFFFF;color:#FFFFFF" onFocus="javascript:this.select()">';
...
function compactar()  {
		...
		patron += '{CAOS}{NL}{NL}';
		patron += '{CAOS_I}{NL}{NL}';
		patron += '{HISTORIA}{NL}{NL}';
		patron += '{CAOS_S}{NL}{NL}';
		patron += '{SIZE_PEQ}Batalla del día ' + getFecha() + '{/SIZE}{NL}{NL}{NL}';
        ...
        // ATACANTES
        patron += '{COLOR_T1}{U}{B}{SIZE_MED}Atacantes (' + lstAtaq.length() + '):{/SIZE}{/B}{/U}{/COLOR}{NL}';
        
        for(var i = 0; i < lstAtaq.length(); i++){
             patron += '{COLOR_A1}{B}{SIZE_GRA}'+ lstAtaq.getNombre(i) + '{/SIZE}{/B}{/COLOR}{NL}';
         ...
         // DEFENSOR
        patron += '{COLOR_T1}{U}{B}{SIZE_MED}Defensores (' + lstDef.length() + '):{/SIZE}{/B}{/U}{/COLOR} {NL}';
        
        for(var i = 0; i < lstDef.length(); i++){
          patron += '{COLOR_D1}{B}{SIZE_GRA}'+ lstDef.getNombre(i) + '{/SIZE}{/B}{/COLOR}{NL}';
		...
		patron += '{CAOS_R}{NL}{NL}';
		patron += '{RECICLAJE}{NL}{NL}';
		patron += '{DEDICAS}{NL}{NL}';
