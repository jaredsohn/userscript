...
$("#villages_list").find("tr").find("td:eq(0)").prepend('<input type="radio" style="display:none;" checked>');
            ...
            if(null!==vlg){
                ...
                $("#village_table").each(function(){
                    ...
                    for (var i=0; i<10000; i++){
                        $("#village_table").find("tr:eq("+i+")").each(function(){
                            var villageID = $("#villages_list").find(":checked:eq("+i+") + span").find("a:eq(0)").attr("href").match(/\d+/g)[1];
                            var $this = $(this);
                            var t = $this.html();
                            $this.html(t.replace(/screen=overview/g,'target='+villageID+'&screen=place'));
                            $(this).find("td").css("background-color","#F5FF00");
                        });  
                    }
                });
            } 
