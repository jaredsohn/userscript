...
// @homepageURL    https://userscripts.org/scripts/show/145282
// @website        http://userscripts.org/scripts/show/145282
// @updateURL	   https://userscripts.org/scripts/source/145282.meta.js
// @installURL	   https://userscripts.org/scripts/source/145282.user.js
...
function otherSites(evt){
	...
	var sites = [
		...
		{name:'Blogs', href:'http://www.google.com.mx/search?hl=auto&q=(RAR|ISO|Zip|BDRip|MKV|FLAC|MP3|DJVU|PDF|MirrorCreator|ZippyShare|ShareBeast|PutLocker|SendSpace|ZippyShare|DivShare|SkyDrive) site:blogspot.com OR site:wordpress.com OR site:edublogs.org OR site:tumblr.com OR site:scienceblogs.com&q='+query.replace(/\s+/g,'+')},
		{name:'Cinema & TV: Vídeos, Series, Films...', href:'http://www.google.com.mx/search?hl=auto&q=(MKV|AVI|MP4|RMVB|DVDRip|BDRip|DVD|BluRay|BlogSpot) "Ge.Tt" OR "MediaFire" OR "SendSpace" OR "DepositFiles" OR "DivShare" OR "ZippyShare" OR "2Shared" OR "Mega" OR "RapidShare" OR "Share-Online" OR "MyUpload" OR "ShareBeast" OR "PutLocker" OR "BillionUploads" OR "SockShare" OR "DropBox"&q='+query.replace(/\s+/g,'+')},
		{name:'Definition, Sentences, Synonyms...', href:'http://www.google.com.mx/search?hl=auto&q=site:iate.europa.eu OR site:en.glosbe.com OR site:imtranslator.net OR site:woxikon.com OR site:bab.la OR site:wordreference.com OR site:thefreedictionary.com OR site:linguee.com OR site:tatoeba.org&q='+query.replace(/\s+/g,'+')},
		{name:'Docs, Papers, Journals, Magazines, Books...', href:'http://www.google.com.mx/search?hl=auto&q=(DJVU|CHM|CBR|SkyDrive|BlogSpot|XLS|PPT|DOC|NFO|DVI|EPUB|PDF) "MediaFire" OR "PutLocker" OR "SendSpace" OR "ZippyShare" OR "DivShare" OR "ShareBeast" OR "Mega" OR "BillionUploads" OR "DropBox" OR "2Shared" OR "Ge.Tt" OR "GDrive" OR "MyUpload" OR "SockShare" OR "SkyDrive"&q='+query.replace(/\s+/g,'+')},
		{name:'File Hosting Links', href:'http://www.google.com.mx/search?hl=auto&q="Ge.Tt" OR "MediaFire" OR "PutLocker" OR "SendSpace" OR "DepositFiles" OR "Share-Online" OR "ZippyShare" OR "DivShare" OR "FileFlyer" OR "ShareBeast" OR "Mega" OR "BillionUploads" OR "DropBox" OR "2Shared" OR "Share-Online" OR "Uploading" OR "RapidShare" OR "BlogSpot" OR "GDrive" OR "MyUpload" OR "SockShare" OR "SkyDrive" OR "RGHost"&q='+query.replace(/\s+/g,'+')},
		{name:'FTP Folder', href:'http://www.google.com.mx/search?hl=auto&q=(ISO|RAR|MP3|DOC|MKV|DJVU|7Z|FLAC|PPT|AVI|EPUB|ZIP|M4A|XLS|MP4|PDF|EXE) "Parent Directory" intitle:"index.of" "Name" "Last modified" "Size" "Description" -inurl:htm -inurl:html -inurl:php -idftp&q='+query.replace(/\s+/g,'+')},
		{name:'Music, Audio (Download Direct Links)', href:'http://www.google.com.mx/search?hl=auto&q=(SkyDrive|GDrive|MP3|FLAC|M4A|LossLess|BlogSpot) "Ge.Tt" OR "MediaFire" OR "Mega" OR "BillionUploads" OR "SendSpace" OR "DepositFiles" OR "Share-Online" OR "ZippyShare" OR "FileFlyer" OR "MyUpload" OR "PutLocker" OR "DropBox" OR "2Shared" OR "DivShare" OR "ShareBeast" OR "RapidShare"&q='+query.replace(/\s+/g,'+')},
		{name:'Scientific Search', href:'http://www.google.com.mx/search?hl=auto&q=site:redalyc.org OR site:techxtra.ac.uk OR site:tdx.cat OR site:scitation.aip.org OR site:doaj.org OR site:pesquisa.bvsalud.org OR site:ncbi.nlm.nih.gov OR site:agu.org OR site:afe.org OR site:electrochem.org OR site:springerlink.com&q='+query.replace(/\s+/g,'+')},
		{name:'Subtitles', href:'http://www.google.com.mx/search?hl=auto&q=(SRT|SUB|IDX|SSA|TXT|RT|ASS|PSB|S2K|LRC|SMI|MKS|SMIL|SSF|USF) site:all4divx.com OR site:subdivx.com OR site:subscene.com OR site:divxtitles.com OR site:divxsubtitles.net OR site:opensubtitles.com&q='+query.replace(/\s+/g,'+')},
		{name:'FileShut', href:'http://www.fileshut.com/find_files.php?submit=submit&ext=0&submitit=Search+Files&q='+query.replace(/\s+/g,'+')},
		...
		{name:'MP3Fusion', href:'http://www.mp3fusion.net/searchSuggest.php?dosearch=dosearch&cmdSearch.x=0&cmdSearch.y=0&txtSearch='+query.replace(/\s+/g,'+')},
		...
		{name:'Tesis Doctorales', href:'http://www.tesisenred.net/search?scope=%2F&ocult=0&query='+query.replace(/\s+/g,'+')},
		{name:'Twilight', href:'http://twilight.ws/downloads.php?type=&host=&q='+query.replace(/\s+/g,'+')},
		{name:'WorldWide Science', href:'http://worldwidescience.org/wws/result-list/fullRecord:'+query.replace(/\s+/g,'+')},
		{name:'YouTube', href:'http://www.youtube.com/results?search_type=search_videos&search_query='+query.replace(/\s+/g,'+')}
...
function checkupd(){
	...
	GM_xmlhttpRequest({
		...
		onload:function(result) {
			if (result.responseText.indexOf('@version        2.0') == -1) {
				Upd = true;
...
function so_captureKeyDownEvent(e) {
  ...
  switch(keyCode) {
    ...
    case 123: // F12 (F12 redirects to translate.google.com)

      var translation_url = "http://translate.google.com.mx/translate?hl=en&js=y&prev=_t&ie=UTF-8&layout=1&eotf=1&sl=auto&tl=auto&u=" + document.location.href
