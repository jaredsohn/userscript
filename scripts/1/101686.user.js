...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  readOptions();
  AddMainTabLink('FLOWER', "fbutton", function(){
...
function updatefbutton(text)
{
var a=document.getElementById("fbutton");
...
// alert("in trysendattackx " + Cities.numCities + ":" + my.Search.mapDat.length + ":" + my.Search.citiessearched);
    ...
    var indcit = Math.floor(Math.random() * Cities.numCities);
    //for (i = 0; i < Cities.numCities; i++)
    dela = 10;
    var iindcit = 0;
    while (iindcit < Cities.numCities)
    {
   ...
   //     document.getElementById("srcResults").innerHTML = "numcav is now " + numcav + ", marching: " + (march.marchUnits[7] + march.returnUnits[7]) + "<br/>" + document.getElementById("srcResults").innerHTML;
        ...
        for (ind = 0; ind < my.Search.mapDat.length; ind++) //while (ind < 15)
        {
            ...
            var numcavmarch = el.tileLevel == 3 ? 4500 : (el.tileLevel == 2 ? 1750 : 1000);

            if (((el.isbarbel && cit.isbarbcit) || (el.iscrestel && cit.iscrestcit)) && curtime - el.lastTime > 60 * 60 * 750)// && numcavmarch <= numcav)// &&*/ ((el[4] == 2 && numcav >= 2000) || (el[4] == 3 && numcav >= 4500)))
            {
...
//                        break;
   ...
   //             document.getElementById("srcResults").innerHTML = "knight " + bestknight.knightName + " found<br/>" + document.getElementById("srcResults").innerHTML;
            ...
            //    alert(curtime + (20 * 1000));
                bestknight.lastActivity = curtime + (120 * 1000); // time out this knight for 30s
    ...
    //        document.getElementById("srcResults").innerHTML = "found bestel " + bestel.x + "," + bestel.y + ", mssinceprev: " + (curtime - bestel.lastTime) + ", pos = " + bestelind + "<br/>" + document.getElementById("srcResults").innerHTML;
       ...
       //         document.getElementById("srcResults").innerHTML = "found bestel2 " + bestel.x + "," + bestel.y + ", mssinceprev: " + (curtime - bestel.lastTime) + ", pos = " + bestelind + "<br/>" + document.getElementById("srcResults").innerHTML;
              ...
              if (bestel.isbarbel)
              {                
                  var resrc = getResources(cit.id);
                  var totalrs = resrc.food + resrc.wood + resrc.stone + resrc.ore;
                  var attck = CityUnderAttack(cit.id);
                 
                if (my.Search.transportcity != null && (my.Search.transportcity[0] != cit.x || my.Search.transportcity[1] != cit.y) && (totalrs >= cit.numwagon * 5000 || (totalrs >= 2000000 && attck)) && (curtime - cit.lasttransport > 1000 * 60 * 1)  )
                {
                 //   alert("wanting to send transport from " + cit.x + " " + cit.y + ", totalrs = " + totalrs + ", atck = " + attck);
                    var max = cit.numwagon * 5000;
                    var transprsrc = {};
                    var total = 0;
                    transprsrc.stone = resrc.stone > (max - total) ? (max - total) : resrc.stone;
                    total += transprsrc.stone;
                    transprsrc.ore = resrc.ore > (max - total) ? (max - total) : resrc.ore;
                    total += transprsrc.ore;
                    transprsrc.wood = resrc.wood > (max - total) ? (max - total) : resrc.wood;
                    total += transprsrc.wood;
                    transprsrc.food = (resrc.food > max - total)  ? (max - total) : resrc.food;
                    if (transprsrc.food > resrc.food - 1200000)
                    {
                        transprsrc.food = resrc.food > 1200000 ? resrc.food - 1200000 : 0;
                    }

                    total += transprsrc.food;
                    


                    my.Search.doMarch(cit.id, bestknight.knightId, my.Search.transportcity[0], my.Search.transportcity[1], 0, 0, numcavmarch, 0, 0, cit.numwagon, dela, my.Search.marchNotify, transprsrc);
                    cit.lasttransport = curtime;
                }
                else
                {
                    this.setTileTime(bestel.x, bestel.y, bestel.lastTime + 0.35 * 60 * 1000); // move lasttime
                    var numcavmarch = bestel.tileLevel == 3 ? 4500 : (bestel.tileLevel == 2 ? 1750 : 1000);
                    my.Search.doMarch(cit.id, bestknight.knightId, bestel[0], bestel[1], 0, 0, numcavmarch, 0, 0, 0,  dela, my.Search.marchNotify);
                }
          ...
          //          setFw(bestel.x, bestel.y, false);
                ...
                {
                  //  this.setTileTime(bestel.x, bestel.y, bestel.lastTime + 0.35 * 60 * 1000); // move lasttime
                    my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, 0, 8000, 0, 800, 0, 0, dela, my.Search.marchNotify);
                }
                else
                {
                    if (my.Search.crestlvl == 6)
                        my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, 100, 0, 0, 0, 0, 0, dela, my.Search.marchNotify);
                    else
                        my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, 299, 0, 0, 1, 0, 0, dela, my.Search.marchNotify);            
                }
              }
              if (bestel.isbugel)
              {
            //  alert("sending tickle");
                        my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, my.Search.nummmtickle, 0, 0, 0, my.Search.numscoutstickle, 0, dela, my.Search.marchNotify);              
              }
...
//alert("trysendattack end");   
 ...
 checkTileTime : function (x, y)
 {
        ...
        for (ind = 0; ind < my.Search.mapDat.length; ind++) //while (ind < 15)
        {
            ...
            if (el.x == x && el.y == y && curtime - el.lastTime < 60 * 60 * 750 && (!el.iscrestel))
            {
                return false;
            }
        }
 ...
 try
  ...
  setTimeout(my.Search.callTrySendAttack, 5000);
 },
  ...
  //document.getElementById("srcResults").innerHTML = "in marchnotify<br/>" + document.getElementById("srcResults").innerHTML;
        ...
        for (ind = 0; ind < my.Search.mapDat.length; ind++) //while (ind < 15)
        {
           ...
           // dbg += el.tileId + ":" + rslt.tileId + "\r\n";
            ...
            {
            dbg += "YES";// + Seed.units['city' + el.city]['unt'+7] + "\r\n";
               ...
               // if (!el.iscrestel || el.fw)
                ...
                {
                 //   Seed.units['city' + el.city.id]['unt'+7] -= numcavmarch;
                ...
                //    dbg += rslt.eta + ":" + curtime + ":" + el.knight.lastActivity + ":" + ((curtime - rslt.eta * 1000) * 2) + "<br/>";
                    ...
                    if (el.iscrestel)
                    {
                            ...
                            var bestknight = el.knight.swk;
                             ...
                             if (my.Search.crestlvl == 6)
                                my.Search.doMarch(el.city.id, bestknight.knightId, bestel.x, bestel.y, 1, 3000, 0, 0, 0, 0, 10, function(rslt){});
                             else
                                my.Search.doMarch(el.city.id, bestknight.knightId, bestel.x, bestel.y, 0, 8000, 0, 800, 0, 0, 10, function(rslt){});
     ...
     //           Seed.queue_atkp[el.city][7] += numcavmarch;
 ...
 doMarch : function(cid, kid, x, y, nummil, numarch, numcav, numbal, numscouts, numwagons, dela, notify, res) {
  var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    params.cid = cid;
    params.xcoord = x;
    params.ycoord = y;
    ...
    {
        params.r1 = res.food; params.r2 = res.wood; params.r3 = res.stone; params.r4 = res.ore;  params.gold = 0;
    }
    params.standalone = 0;
    params.items = "";
    params.type = numwagons > 0 ? 1 : numscouts > 0 ? 3 : 4;
    params.u2 = nummil;
    params.u3 = numscouts;
    params.u6 = numarch;
    params.u7 = numcav;
 ...
 doMarch2 : function (params)
 {
    ...
    for (var i = 0; i < my.Search.marcharrcnt; i++)
    {
        if (my.Search.marcharr[i].xcoord == params.xcoord && my.Search.marcharr[i].ycoord == params.ycoord)
        {
            my.Search.marcharr[i] = params;
            return;
        }
    }
 ...
 if (my.Search.marcharrcnt == 10)
 {
    for (var i = 0; i < my.Search.marcharrcnt - 1; i++)
    {
        my.Search.marcharr[i] = my.Search.marcharr[i + 1];
    }
    my.Search.marcharr[9] = params;
     return;
 }


 my.Search.marcharr[my.Search.marcharrcnt] = params;
 my.Search.marcharrcnt++;
 // alert("doMarch2 " + my.Search.marcharrcnt + " " + params + " " + my.Search.marcharr[0]);
 },
...
my.Search.lastmarch = (new Date()).getTime();
//alert("domarch3" + my.Search.marcharrcnt);
var params = null;
if (my.Search.marcharrcnt > 0)
{
    params = my.Search.marcharr[0];
    //alert("params = " + params);
    for (var i = 0; i < my.Search.marcharrcnt - 1; i++)
    {
        my.Search.marcharr[i] = my.Search.marcharr[i + 1];
...
if (params != null && my.Search.checkTileTime(params.xcoord, params.ycoord))
{
...
try{
  ...
  setTimeout(my.Search.doMarch3, 100);
        }
    );
    }
    catch(err2)
...
minmighttotickle : 2000000,
 ...
 AutoHelp : function () {
  ...
  //  return;
    setTimeout(my.Search.AutoHelp,4000);
 },
...
return iframe;
 ...
 StartAuto : function (){
  ...
  updatefbutton(Cities.numCities);
    for (i = 0; i < Cities.numCities; i++)
    {
        var cit = Cities.cities[i];
        cit.numarch = my.Search.numarch = parseInt(Seed.units['city' + cit.id]['unt'+6]);
        cit.numbal = my.Search.numbal = parseInt(Seed.units['city' + cit.id]['unt'+10]);
        cit.nummil = my.Search.nummil = parseInt(Seed.units['city' + cit.id]['unt'+2]);
        cit.numcav = my.Search.numcav = parseInt(Seed.units['city' + cit.id]['unt'+7]);
        cit.numwagon = my.Search.numwagon = parseInt(Seed.units['city' + cit.id]['unt'+9]);
      ...
      //  var numcav = parseInt(Seed.units['city' + cit.id]['unt'+7]);
  ...
  this.GetKnights();
        ...
        my.Search.mapDat = [];
        my.Search.FindBarbs(cit.x, cit.y, 37, function() { alert("notify called"); });

 //   alert("before trysendattack");

    
    my.Search.SearchAlliance("ANADOLU");
 //   my.Search.SearchAlliance("Helms");
    setTimeout (function(){my.Search.checkstuffrunning()}, 180000);
   // setTimeout(my.Search.openWindow, 120000);
   setTimeout(my.Search.callTrySendAttack, 120000);
    setTimeout(my.Search.doMarch3, 120000);
    ...
    if (rslt.userInfo[0].allianceId)
    {
     ...
     //   alert(rslt.userInfo[0].allianceName + ":" + diplo);
        if (true)//diplo != "friendly")
        {
            ...
            if (okies && rslt.userInfo[0].allianceName != "ANADOLU")
            {
...
//      alert("distance not ok " + dist + ":" + t.opt.maxDistance);
 ...
 //        document.getElementById("srcResults").innerHTML = "lv8 wild found at " + newel.x + "," + newel.y + "<br/>" + document.getElementById("srcResults").innerHTML
  ...
  //    alert("done");
      ...
      {
            var amsg = "about to search city " + t.citiessearched ;
         //   alert(amsg);
         updatefbutton(Cities.numCities - t.citiessearched);
         try{
         document.getElementById("srcResults").innerHTML = amsg + "<br/>" + document.getElementById("srcResults").innerHTML;
            }catch(errx)
            {}
            var cit = Cities.cities[t.citiessearched];

            t.FindBarbs(cit.x, cit.y, 37, function() { alert("notify called"); });
      }
      else
      {
      t.searchdone = true;
...
//    document.getElementById ('statStatus').innerHTML = 'Searching at '+ x +','+ y;
  ...
  my.Search.reqmap = true;
  my.Search.lastmaprq = (new Date()).getTime();
    setTimeout (function(){my.Search.checkmaprequest(x, y)}, Math.floor( 120000) );
    setTimeout (function(){my.Search.MapRequest(x, y, 30, t.mapCallback2)}, MAP_DELAY);
  },

  reqmap:false,
  lastmaprq:(new Date()).getTime() - 1000 * 60,

  checkmaprequest : function (x, y)
  {
     if (my.Search.searchdone)
        return;
    if ((new Date()).getTime() - my.Search.lastmaprq > 60000)
    {
        my.Search.lastmaprq = (new Date()).getTime();
        setTimeout (function(){my.Search.checkmaprequest(x, y)}, Math.floor( 120000) );
        setTimeout (function(){my.Search.MapRequest(x, y, 30, my.Search.mapCallback2)}, MAP_DELAY);
    }
  },

  lastmarch:(new Date()).getTime() - 1000 * 60,

  checkstuffrunning : function (x, y)
  {

    if ((new Date()).getTime() - my.Search.lastmarch > 180000)
    {
        my.Search.doMarch3();
        setTimeout (function(){my.Search.checkmaprequest(x, y)}, Math.floor( 120000) );

    }        
    setTimeout (function(){my.Search.checkstuffrunning()}, 60000);
  },
...
function setCities(){
  Cities.numCities = Seed.cities.length;
      ...
      //    for (var k in Seed.cities)
      //      alert(k + "/" + Seed.cities[k]);
//  Cities.curCityID = unsafeWindow.currentcityid;
  Cities.cities = [];
  Cities.byID = {};
  for (i=0; i<Cities.numCities; i++){
    city = {};
    city.lasttransport = (new Date()).getTime() - 1000 * 60;
    city.idx = i;
  ...
  //  alert("before getresources");
  //  getResources(city.id);
  //  alert("after getresources");
  //  if (CityUnderAttack(city.id))
  //      alert("city " + i + " is under attack");
  //  else
  //      alert("city " + i + " is NOT under attack");
  }

}

function getResources(cityid)
{
        var ret = {};
        cityID = 'city'+ cityid;
        ret[0] = ret.food = parseInt(Seed.resources[cityID]['rec1'][0] / 3600);
        ret[1] = ret.wood = parseInt(Seed.resources[cityID]['rec2'][0] / 3600);
        ret[2] = ret.stone = parseInt(Seed.resources[cityID]['rec3'][0] / 3600);
        ret[3] = ret.ore = parseInt(Seed.resources[cityID]['rec4'][0] / 3600);
      //  alert("food:" + ret.food + "wood:" + ret.wood + "stone:" + ret.stone + "ore:" + ret.ore + "gold:" + ret.gold);
      return ret;
}

function CityUnderAttack(cityid)
{
    for (var k in Seed.queue_atkinc){
        var m = Seed.queue_atkinc[k];
        if (m.marchType != 4)      // skip scouting, only attacks
            continue;
        if (m.toCityId != cityid)
            continue;

        var secs = parseInt(m.arrivalTime - unixTime());
     //  alert("in " + secs + " secs");
       if (secs < 300)
            return true;
...
function AddMainTabLink(text, id, eventListener) {
