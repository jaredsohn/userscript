// ==UserScript==
// @name           Doacoes de Comunidade Elite MODE
// @namespace      editado por JoaoPaulo | uid=17288583650260853947
// @description    Script para facilitar o trabalho da Doacoes de Comunidade Elite
// @CFBT     www.orkut.com.br/Main#Community?cmm=101673534
// @include        *.orkut.*/Main#CommMsgs?cmm=101673534*
// @include        *.orkut.*/CommMsgs?cmm=101673534*
// @include        *.orkut.*/Main#CommMemberManage?cmm=101673534*
// @include        *.orkut.*/CommMemberManage?cmm=101673534*
// @include        *.orkut.*/Community?cmm=101673534*
// @include        *.orkut.*/Main#Community?cmm=101673534*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
...
|	
|     este script foi criado por joseph, e editado por Rafael	      |
|						                      |
+---------------------------------------------------------------------+
*/
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=101673534&tid=5482694550451129075", false);
	...
	function captchaONOFF(env, status) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgPost?cmm=101673534&tid=5482697939180325619", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[green][b]Topico apagado:[/b][/green] ".concat(titulo, breakLine, "[green][b]Criador:[/b][/green] ", uid, breakLine, "[green][b]Motivo:[/b][/green] ", motivo, breakLine, "[green][b]Horario:[/b][/green] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent("Topico Deletado:"),
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b][green]Membro:[/b][/green] ".concat(uid, breakLine, "[b][green]Motivo:[/b][/green] ", motivo, breakLine, "[b][green]Horario:[/b][/green] ", pegaData(), breakLine, breakLine, "[/gray]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este topico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('digite o motivo de estar deletando este topico:\\n','T\ÃƒÂ³pico de spam'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
