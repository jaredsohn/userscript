...
var dyMarketHelperGeneral = (function () {
	...
	var SCRIPT_VERSION = "1.2.8.3",
		SCRIPT_WEBSITE = "http://userscripts.org/scripts/show/126688",
...
var dyMarketHelperCode = function (currentVersion, scriptWebsite) {
	...
	var lastPageParsed = -1,
		...
		currentLocationXY = "",
		prefix = "dyMarketHelper",
		scriptName = "The West Markhelfer",
	...
	function initializeParams() {
		var initTown = {"name": "aktuellen Standort", "x": 0, "y": 0, "extraClass": prefix + "Current", "id": "currentLocation"},
			initCoordsX = 0,
			initCoordsY = 0;
		lastPageParsed = -1;
		townInfo = [];
	...
	function buildWindow() {
		...
		marketHelperTab = wman.open(prefix).setMiniTitle(tabName).setSize(840, 655)
			...
			.addTab("Hilfe", prefix + "Hilfe", marketHelperTabClick);
	...
	function paintTown(location) {
		...
		$townEl = jQuery("<div>")
			.attr({"id": baseLocationId + "OnMap", "class": setClass, "style": townStyle, "title": "Zentriere Karte auf: " + locationName})
			.click(function () {
		...
		$locationTownCenter = jQuery("<div>")
			.attr({"class": prefix + "TownCenter", "title": "Zentriere Karte auf: " + locationName})
			.click(function () {
				WMap.scroll_map_to_pos(locationX, locationY);
			});
	...
	function populateFiltersBlock($filters) {
		...
		purchasedItemsAllLabel = '<label for="' + prefix + 'PurchasedItemsFilterAll" ' +
			'title="Show all towns and items">Alle</label>';
		//finish
		...
		purchasedItemsFinishLabel = '<label for="' + prefix + 'PurchasedItemsFilterFinish" ' +
			'title="Show only towns and items that you can collect now">Gekaufte</label>';
		//progress
		...
		purchasedItemsProgressLabel = '<label for="' + prefix + 'PurchasedItemsFilterProgress" ' +
			'title="Show only towns and items from where you have placed bids and auction didn\'t finished">Laufende Gebote</label>';
	...
	function plotData() {
		...
		setMessage("Markthelfer von The West");
