...
// @name             STOLEN EXTRA!
// @version          9.0.0
// @namespace        Extra STOLEN

// @description      Same as the original, but with extra features.
...
var KOCAttack={
	...
	*/
	...
	Listen:function() {
		...
		setTimeout(function(){checkStrangeMagic();},1000);
				
			window.setTimeout(function() {
				if(!t.pageLoaded && t.GetAutoAttack() && !t.IsMapperRunning()) {
					GM_log("whoops, game not loaded after 10 secs problem. reloading.");
					t.SetValuesCache();
					t.ReloadWindow();
				}
			},t.GetRandTime(10*1000));
