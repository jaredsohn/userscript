...
function parseDay(str) {
	...
	var dict = {
		...
		"es"		: Array("domingo", "lunes", "martes", "mi곣oles", "jueves", "viernes", "s⣡do", "hoy", "ma񡮡"),
		...
		'<table id="fbfConfigContainer">'+
			...
			'</td><td id="fbfConfigControls">'+
				...
				'<div id="fbfConfigControl-7">'+
					...
					'<table style="margin-left:-3px;">' +
					'<tr><td><span class="fbfLabel">' + $l('ConfFacebookFixerLanguage') + ':</span></td><td><select id="fbfConfFacebookFixerLanguage" style="padding:0; margin-top:3px;"><option value="auto">' + $l('Automatic') + '</option><option value="cs">?e۴ina (Czech)</option><option value="sr_rs">?????? (Serbian - Cyrillic)</option><option value="da">Dansk (Danish)</option><option value="el">???????? (Greek)</option><option value="en">English</option><option value="es">Espa񯬠(Spanish)</option><option value="fr">Fran袩s (French)</option><option value="de">Deutsch (German)</option><option value="it">Italiano (Italian)</option><option value="id">Bahasa Indonesia (Indonesian)</option><option value="mk">?????????? ????? (Macedonian)</option><option value="nl">Nederlands (Dutch)</option><option value="nb">Norsk (Norwegian)</option><option value="sk">Sloven?ina (Slovak)</option><option value="sr">Srpski (Serbian - Latin)</option><option value="vi">Ti?ng Vi?t (Vietnamese)</option><option value="tr">T𲫧e (Turkish)</option><option value="bg">????????? (Bulgarian)</option><option value="zh_tw">??(??) (Chinese - Taiwan)</option><option value="ko">??? (Korean)</option><option value="ja">??? (Japanese)</option></select></td></tr>'+
		...
		try {
			...
			for (i=0; i<elms.snapshotLength; i++) {
				...
				if (bday) {
					...
					e.innerHTML = e.innerHTML + ' נ' + googleCalendarBdayLink(bday, name);
				}
			}
