...
// @name           Script LISa
// @namespace      Hrenb kakae-to =))
// @description    Lis bototo
// @include        http://ts*.travian.*/build.php?tt-=99&id-=99
// @include        http://ts*.travian.*/build.*
// @include        http://ts*.travian.*/build.php?id=39
// @include        http://ts*.travian.*/build.php?gid=16
// @include        http://ts*.travian.*/login.php
// @include        http://ts*.travian.*/dorf3.php*
// ==/UserScript==


var Troops = {};
Troops.Name = [
			[	"Legu", "Proteru", "Impu", "razvedka", "Impo konu",
				"Equites Caesaris", "Taranu", "Catapultu", "Sen9", "Poselu", "Heroi"],
			[	"Dybu", "kopi9", "Toporu", "Scout", "Paladinu", "Tefteli", 
				"Taran", "Catapult", "Govorun", "Posel", "Hero"],
			[	"PHALU", "Me4u", "Razvedka", "Gromu", "Druidu"] ,
];
...
var Doc = {
  ...
  Login : function(){
      ...
      LoginForm = LoginReq[0].getElementsByTagName("form")[0];
			LoginData = "";
			LoginForm.setAttribute('target', '_green');
...
Farm.Init = function(){
  ...
  if (ListEntries && ListEntries.length > 0){
    for (var i = 0; i < ListEntries.length ; i++){
      ...
      NameData = "<font color='green'>"+LastSent+"</font> / ";
      NameData += "<font color='red'>"+NextSent+"</font>";
