...
// @author         hotzu
// @version        1.04.03
// @lastchanges    (by:Dr.ZERO ONly for ++ UA|NW ++ :/)  Elephant finder shows animal numbers, coordinates are links, bug fix on spy send, you can save and load data for the elephant scanner, the neighbour reports now not autorefresh
...
GM_registerMenuCommand("Tools Settings",showSet);
 ...
 * Catapult, spy function added by: hotzu
 */
			...
			success: function(msg) {
				...
				$.ajax({
					...
					success: function(msg){
                                            ...
                                            {
                                                adatok2+=adatok.split('spy=')[0]+'spy='+spy+'&timestamp='+adatok.split('spy=')[1].split('&timestamp=')[1];
                                            }
                                            else
                                                adatok2=adatok;
...
function village_links()
{
    if(parseInt(getCookie("VillageLinks","1"))){
        $(".entry a").each(function(){
            ...
            gomb.setAttribute('title', texts[11]);
            ...
            gomb2.setAttribute('title', texts[12]);
            kep2=document.createElement('img');
            kep2.setAttribute('style', 'display:inline;margin-left:0px;background-image:url("http://'+document.location.href.split('/')[2]+'/gpack/travian_Travian_4.0_Fluffy/img/market/ratio.gif");width:16px;height:16px;');
