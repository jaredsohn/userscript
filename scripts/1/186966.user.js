// ==UserScript==
// @name          OGame - Expeditions statistics (by BoGnY)
// @namespace     OGame - Expeditions statistics (by BoGnY)
// @description	  OGame - Expeditions statistics (by BoGnY)
// @require       http://userscripts.org/scripts/source/118453.user.js
// @updateURL     http://userscripts.org/scripts/source/186966.meta.js
// @version       2.2.13
// @author        BoGnY
// @copyright     Copyright 2013 by BoGnY (based on the Ogame expeditions statistics v2.1.5 by Demien)
...
-    oGameVersionCheck("OGame - Expeditions statistics (by BoGnY)", "5.7.2", "http://userscripts.org/scripts/show/186966");
}
...
contentEval(function () {
    var script_verion = '2.2.13'; // z-include script, a-without include
    ...
    function setExpLanguage() {
        switch (lang) {
            ...
            case 'it': // Italiano
                ...
                'sfortunatamente siamo tornati a mani vuote',
                'ha fatto sì che la flotta volasse in tondo tutto il tempo',
                'tua spedizione si è imbattuta nel campo gravitazionale di una stella di neutroni',
                'Un problema al reattore della nave ammiraglia ha quasi distrutto l`intera spedizione',
                'alle interminabili code davanti ai bagni delle navi, la spedizione è stata annullata',
                'forma di vita di pura energia ha fatto sì che tutti i membri della spedizione rimanessero ipnotizzati dai loro schermi'];
                fleet_name = ['parte della flotta:<br>'];
                aliens_name = ['Specie sconosciuta',
                'straniere hanno attaccato la spedizione senza alcun avviso!',
                'La tua flotta in esplorazione non ha avuto un amichevole primo contatto con una specie sconosciuta',
                'sconfinato nel territorio di una razza aliena sconosciuta ma molto aggressiva',
                'specie sconosciuta sta attaccando la nostra spedizione'];
                pirates_name = ['Alcuni pirati dello spazio decisamente disperati',
                'La tua spedizione ha avuto uno spiacevole incontro',
                'da una base spaziale pirata',
                'dovuto combattere alcuni pirati',
                'stata attaccata da un piccolo gruppo di navi!',
                'Sembra che saremo presto sotto attacco',
                'barbari primitivi ci stanno attaccando con delle',
                'rivelata essere una trappola di alcuni bucanieri dello spazio',
                'imboscata tesa da alcuni pirati dello spazio!',
                'riporta feroci scontri contro delle navi pirata non identificate!'];
                ...
                'disorientata la spedizione ritorna utilizzando i motori ad impulso',
                'La tua spedizione è andata in un settore colpito da una tempesta di particelle',
                'vento-stellare di una gigante rossa ha compromesso il salto'];
                speed_name = ['Un componente inserito nel generatore di energia',
                'La flotta però ha incontrato un forte vento solare al momento del salto di ritorno',
                'nuovo e audace comandante ha usato con successo un buco nero alquanto instabile per accorciare il viaggio di ritorno'];
                buyer_name = ['La tua spedizione ha avuto contatto con un`amichevole razza aliena',
                'Dopo aver liberato il cargo, il capitano ha annunciato che la persona che li ha salvati sarà il loro solo cliente preferito'];
                pipec_name = ['unica cosa che rimane dalla spedizione è il seguente radiogramma',
                'rottura nel nucleo della nave ammiraglia ha causato una reazione a catena'];
                fake_message_text = ['informa di quanto segue'];
                strPattrFleet = 'seguenti navi sono ora parte della flotta:<br>';
                ...
                spyWarn = ['Non pensavo che ci sarebbe stato qualcun altro qui.',
                'Non siamo i primi ad essere stati qui',
                ''];
                // Plain text
                spyWarnSovet = "In questo sistema non è più produttivo inviare spedizioni. Si consiglia di cambiare sistema.";
                ...
                totInFleetText = 'Totale risorse nella flotta';
                ...
                showOutText = 'visualizza statistiche';
                showAllTimeText = 'di sempre';
                showLastMonthText = 'ultimo mese';
                showLastWeekext = 'ultima settimana';
                showTodayText = 'ultimo giorno';
                clearDataTitle = 'Cancella tutti i dati salvati delle statistiche';
                sendUndefinedMessage = 'Impossibile identificare il risultato della spedizione!';
                undefined_exp_message_confirm = 'Sei sicuro che questo è un messaggio relativo al risultato della spedizione '+
                '. Vuoi inviarlo all\'autore dello script per l\'aggiunta?';
                clearDataText = 'Elimina dati';
                lossesText = 'perdita';
                // Fleets
                fl_mt = 'Cargo leggero';
                fl_bt = 'Cargo Pesante';
    ...
    function renderInfo(logs, message) {
        ...
        +'<tr class="exp_res_row"><td class="exp_left_cols">'+tot_name+':</td><td>' +number_format(met+cris+deit, 0, ',', '.')+ '</td></tr>'
        +'<tr><td class="exp_fleet_head" colspan="2">'+expFleetHeaderText+'</td></tr>';

        // Выводим стоимость найденных флотов
        if (metInFleet>0) {
            appendText = appendText + '<tr class="exp_fleet_row"><td class="exp_left_cols">'+metInFleetText+':</td><td>' + number_format(metInFleet, 0, ',', '.')+ '</td></tr>';
        }
        if (crisInFleet>0) {
            appendText = appendText + '<tr class="exp_fleet_row"><td class="exp_left_cols">'+crisInFleetText+':</td><td>' + number_format(crisInFleet, 0, ',', '.')+ '</td></tr>';
        }
        if (deitInFleet>0) {
        ...
        if ((metInFleet + crisInFleet + deitInFleet) > 0) {
            appendText = appendText + '<tr class="exp_fleet_row"><td class="exp_left_cols">'+totInFleetText+':</td><td>' + number_format(metInFleet+crisInFleet+deitInFleet, 0, ',', '.')+ '</td></tr>';
        }
        ...
        GM_addStyle(
            ".expeditions_statistica { width: 400px; margin: 21px auto; }"+
            ".expeditions_statistica .exp_left_cols { text-align:right; padding-right:10px; width: 200px; }"+
            ".expeditions_statistica .exp_res_head { text-align:center; border: 1px solid green; cursor: pointer; width: 300px; background: url('http://gf1.geo.gfsrv.net/cdn71/fc7a8ede3499a0b19ea17613ff0cb1.gif') no-repeat scroll 374px -19px transparent;}"+
            ".expeditions_statistica .exp_other_head { text-align:center; border: 1px solid blue; cursor: pointer; width: 300px; background: url('http://gf1.geo.gfsrv.net/cdn71/fc7a8ede3499a0b19ea17613ff0cb1.gif') no-repeat scroll 374px -19px transparent;}"+
            ".expeditions_statistica .exp_fleet_head { text-align:center; border: 1px solid red; cursor: pointer; width: 300px; background: url('http://gf1.geo.gfsrv.net/cdn71/fc7a8ede3499a0b19ea17613ff0cb1.gif') no-repeat scroll 374px -19px transparent;}"+
            ...
            ".expeditions_statistica .exp_footer { text-align:center; }"
            );

        var selected = ' selected="selected"';

        appendText = appendText + '<tr><td class="exp_footer" colspan="2">'+showOutText+': ' +
        ...
        appendText = appendText + '</td></tr></table></div>';
        ...
        if (resState==0) {
            $('.exp_res_head').css('background-position','374px 0');
            $('.exp_res_row').css('display', 'none');
        }
        if (fleetState==0) {
            $('.exp_fleet_head').css('background-position','374px 0');
            $('.exp_fleet_row').css('display', 'none');
        }
        if (otherState==0) {
            $('.exp_other_head').css('background-position','374px 0');
    ...
    $(document).on('click', '.exp_res_head, .exp_other_head, .exp_fleet_head', function() {
        ...
        if (state!='none') {
            ...
            $(this_head_class).css('background-position', '374px 0');
        } else {
            $(this_row_class).css('display', 'table-row');
            $(this_head_class).css('background-position', '374px -19px');
        }
