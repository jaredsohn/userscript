...
function cc_encode(){
	...
	if(pwd!=null){
		...
		text.innerHTML='#CC_Cipher#'+result+' <br>';
		...
		ct.onclick=function(){text.innerHTML=text.innerHTML.replace('#CC_Cipher#'+result,content);ct.outerHTML=''};
...
function cc_decode(){
	try{
		...
		this.outerHTML=result.replace(/<script/gi,'&lt;script').replace(/<style/gi,'&lt;style').replace(/<link/gi,'&lt;link').replace(/<meta/gi,'&lt;meta').replace(/<iframe/gi,'&lt;iframe').replace(/<frame/gi,'&lt;frame').replace(/<object/gi,'&lt;object').replace(/<embed/gi,'&lt;embed').replace(/on(\w*?)=/gi,"on$1!=");
	}
	catch(error){
...
function cc_addvideo(){
	var url=prompt("请输入视频页地址（目前支持 YouTube、acfun、bilibili、tucao.cc、SoundCloud、虾米，尝试性支持 catfun、萌否电台）：");
	if(url!=null){
		var text=editor;
...
function cc_addmusic(){
	...
	if(url!=null){
		...
		text.innerHTML+='<img unselectable="on" class="BDE_Music" src="http://tieba.baidu.com/tb/editor/v2/music.png" title="http://box.baidu.com/widget/flash/bdspacesong.swf?from=tiebasongwidget&amp;url='+encodeURIComponent(url)+'&amp;name='+encodeURIComponent(title)+'&amp;artist='+encodeURIComponent(artist)+'&amp;extra=&amp;autoPlay=true&amp;loop=true" width="400" height="95" data-width="400" data-height="95"><br>';
	}
}
...
function add_panel(){
	...
	panel.innerHTML='<div class="cc_code_title"><span>CC Code</span></div>'
		+'<div class="cc_code_content">'
			...
			+'<span class="cc_code_addmusic">插入音乐</span>'
		+'</div>';
