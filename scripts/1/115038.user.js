...
// ==/UserScript==
    ...
    function refreshAfter5() {
        window.setTimeout("refreshPage()",5000); //5000 micro sec = 5 sec
    }
    function refreshPage() {
        window.location.replace('http://apps.facebook.com/kingdomsofcamelot/claimGiftUrl');
    }
				...
				if(claim_gift){
					...
					if(domain_selector && (next_button1 || next_button2)){
						for(var i=0; i<domain_selector.options.length; i++){
							if(domain_selector.options[i].value == "Clarent200"){
