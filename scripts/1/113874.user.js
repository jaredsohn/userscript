...
// @namespace     http://doa.tripplexwelt.de/dls
// @description   Edited Wave Tab new troops added
...
var OptionsDefaults = {
  ...
  objAttack     : {enabled:false, repeatTime:3660, delayMin:15, delayMax:30, levelEnable:[], levelDist:[null,16,16,16,16,16,16,16,16,16,16], deleteObjAttacks:true, stopAttackOnLoss:true, logAttacks:false, maxMarches:10, troops:[null,{SwiftStrikeDragon:1210},{SwiftStrikeDragon:2420},{SwiftStrikeDragon:3630},{SwiftStrikeDragon:5000},{BattleDragon:6000},{BattleDragon:10000},{BattleDragon:15000},{BattleDragon:30000},{BattleDragon:60000},{AquaTroop:28000,Longbowman:1}], clearAllTargets:false},
  currentTab    : false,
  attackTab     : 0,
  mapTab        : 0,
  jobsTab       : 0,
...
Tabs.Waves = {
	...
	cont : null,
  	troopList : [kSpy, kATrans, kBatDrg, kSSDrg, kBanshee, kLBM, kFang, kOgre, kLavaJaw, kFireM],
	enabled : false,
	attackTimer : null,
	marchTimer: null,
	attackErrors : 0,
	...
	init : function (div) {
		...
		var m = '<DIV class=' + rndClass['tab_title'] + '>'+kWaveTitle+'</div>\
			...
			<CENTER><INPUT class=' + rndClass['refresh_button'] + ' id=' + rndStyle['idPrefix'] + 'ResStat type=submit value='+ kResetStats +' /></center>\
    ...
    function troopTable (tab, rownum, prefix, listener){
      ...
      for (var i=0; i<t.troopList.length; i++){
		...
		if(i<=1){
			...
			inp.title = 'Special troops';
		}
		else if(i>1 && i<5){
			inp.style.border = '2px groove blue';
			inp.title = 'Flying troops';
		}
		else {
			inp.style.border = '2px groove red';
			inp.title = 'Walking troops';
		}
...
//*** Attacks Tab - Stats Sub-tab ***
  tabStats : function (){
    ...
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+kAttackStatsTitle+'</div>\
      <CENTER><INPUT class=' + rndClass['refresh_button'] + ' id=pmcampRS type=submit value='+ kClearStats +' \></center>\
 ...
 // Data.options.campAttack {enabled:false, maxDist:7, repeatTime:3660, delayMin:15, delayMax:25, levelEnable:[], levelDist:[]}
  ...
  troopTypes : [kPorter, kConscript, kHalberdsman, kMinotaur, kLongbowman, kSwiftStrikeDragon, kBattleDragon, kArmoredTransport, kGiant, kFireMirror, kAquaTroop, kStoneTroop, kFireTroop, kWindTroop, kGreatDragon, kWaterDragon, kStoneDragon, kFireDragon, kWindDragon],
...
//*** Attacks Tab - Config Sub-Tab ***
  tabConfigGeneral : function (){
    ...
    var m = '<DIV class=' + rndClass['tab_title'] + '>'+ kAutoAttackConfig + '</div>\
      ...
      <TR><TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ kClearLast +'</td><TD><INPUT class=' + rndClass['refresh_button'] + ' type=submit id=pbaacfgClr value='+ kClear +'></td></tr>\
...
//*** Attacks Tab - Maps Sub-tab ***
  tabMaps : function(){
    ...
    m += '<TR><TD colspan="2"><INPUT class=' + rndClass['refresh_button'] + ' type=submit id=pbatMapSearch value='+ kSearch +'></td></tr>'; 
...
Tabs.Info = {
  ...
  init : function (div){
    var t = Tabs.Info;
    t.cont = div;
    div.innerHTML = '<DIV class=' + rndClass['tab_title'] + '>'+ kDOAVersionString + Version +'<BR />Black Edition by TrippleX<BR />'+ WebSite +'</div>\
      <TABLE width=100%><TR><TD><INPUT class=' + rndClass['refresh_button'] + ' type=submit value='+ kRefresh +' id=pbinfRefresh></input>\
  ...
  hide : function (){
    ...
    showStuff : function (){
        ...
        m += '<TABLE style="margin-top:3px" width=100%>\
              <TR bgcolor=#6F6F6F align=center><TD style="border-right: 1px solid; border-bottom: 1px solid;"><B>'+ kTroops +'</b></td>\
...
*/    
        function cityTitle (cityIdx){
			...
			return '<div class=' + rndStyle['classPrefix'] + 'Subtitle><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR><TD align=left>'+ Ops[city.name] +'</td><TD align=center>'+ city.x +'\/'+ city.y + '</td><TD align=center width=200px><font color=yellow>' + alliance_name +'</font></td><TD width=80px align=right>'+ wallStatus +'</td></tr></table></div>';
        }
    },
...
//    researchTimer   : null,
	...
	tabJobInfo : function (){
        ...
        function cityTitle (cityIdx){
            ...
            return '<div class=' + rndStyle['classPrefix'] + 'Subtitle><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR><TD align=left>'+ Ops[city.name] +'</td><TD align=center>at '+ city.x +'\/'+ city.y + '</td><TD align=center width=200px><font color=yellow>' + alliance_name +'</font></td><TD width=80px align=right>'+ wallStatus +'</td></tr></table></div>';
        }
	},
	...
	tabJobBuild : function (){
        ...
        for (var i=0; i<Seed.s.cities.length; i++){
            ...
            m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ Ops[kCityNumber + (i+1)] +'</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';
	...
	tabJobResearch : function (){
        ...
        m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ Ops[kCityNumber +'1'] +' ('+ city.type +')</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';
        var i=0;
    ...
    buildStatTick : function (){
        ...
        for (var i=0; i<Seed.s.cities.length; i++){
			...
			var Ops = new Array();
				...
				Ops[kCityNumber + '4'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops[kCityNumber + '5'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
            m += '<TR><TD>'+ Ops[kCityNumber + (i+1)] +'</td><TD>';
            if (job == null)
                m += kIdle +'</td></tr>';
            else {
    ...
    resStatTick : function (){
    	...
    	var Ops = new Array();
				...
				Ops[kCityNumber + ' 4'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops[kCityNumber + ' 5'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
        m += '<TR><TD>'+ Ops[kCityNumber +' 1']+'</td><TD>';
...
//      return;
    ...
    tabTrain : function(){
        ...
        for (var c=0; c<Seed.s.cities.length; c++){
			...
			var Ops = new Array();
				...
				Ops['Outpost 3'] = "<span style=color:orange>Firedragon Outpost</span>";
				Ops['Outpost 4'] = "<span style=color:lightgoldenrodyellow>Winddragon Outpost</span>";
            m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle>'+ Ops[city.name] +'</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';
    ...
    tabConfigTrain : function(){
        ...
        m += '<DIV class=' + rndStyle['classPrefix'] + 'Subtitle style="background-color:#222222;">Troop Cap (Max Troops, 0 = no max)</div><TABLE class=' + rndStyle['classPrefix'] + 'Tab><TR valign=top><TD width=150><TABLE class=' + rndStyle['classPrefix'] + 'Tab>';
        for (tt=0; tt<troopTypes.length; tt++){
            // Use less room in the table layout
            if (tt%3 == 0) m += '<TR>';
            m += '<TD class=' + rndStyle['classPrefix'] + 'TabLeft>'+ Names.troops.byName[troopTypes[tt]][2] +':</td>';
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  var m = '<TABLE cellspacing=0 width=100% height=100%>\
           <TR id="'+ rndPopup['bar'] +'" style="background-color: #6F6F6F;">\
