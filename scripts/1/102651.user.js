// ==UserScript==
// @name           Power Tools - With Food Alert
// @namespace      Hung
...
{ document.getElementById("mod_comm_list2").style.backgroundColor = '#F5F3AE';}
if(document.getElementById("mod_comm_list1"))
{ document.getElementById("mod_comm_list1").style.backgroundColor = '#F3C2F5';}
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    .ptChatWhisper {}\
    .ptChatAlliance {}\
    .ptChatGlobal {background-color: F576A4}\
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=togWhisperOn type=checkbox /></td><TD>Turn off Audio Whisper <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
...
Tabs.Reports = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '\
        ...
        <TD class=xtab><INPUT id=idReportsSearch type=submit value="Start Search" />\
        <span id=idSpanReportsErrorMsg></span></td></tr>\
        </table></div>\
        <DIV id="ReportsResultsDiv" style="height:470px; max-height:470px;"></div>';
        document.getElementById('idReportsSearch').addEventListener ('click', t.handleReportsSearch, false);
        document.getElementById('idReportsSelect').addEventListener ('click', t.handleReportsSelect, false);
  ...
  DisplayReports : function (){
    var t = Tabs.Reports;
    var data = t.data;
    var results=document.getElementById("ReportsResultsDiv");
    if(!t.data.length) {
  ...
  handleReportsSelect : function(rslt, page) {
    var t = Tabs.Reports;
    t.maxPages=document.getElementById("idReportsSelect").value;
    if ( t.maxPages==99999)
       t.maxPages=t.totalPages;
    document.getElementById("idSpanNumPages").innerHTML = t.maxPages+'';
  },
  handleReportsSearchCB : function(rslt, page) {
    var t = Tabs.Reports;
    if (rslt) {
       if (!rslt.ok) {
          document.getElementById("idSpanReportsErrorMsg").innerHTML = rslt.errorMsg;
          return;
       }
  ...
  handleReportsSearch : function() {
    var t = Tabs.Reports;
    var results=document.getElementById("ReportsResultsDiv");
    //logit("handleReportsSearch");
    t.maxPages=document.getElementById("idReportsSelect").value;
    if ( t.maxPages==99999)
       t.maxPages=t.totalPages;
  ...
  getAllianceReports : function (pageNum){
    ...
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/listReports.php" + unsafeWindow.g_ajaxsuffix, {
      ...
      parameters: params,
      onSuccess: function (rslt) {
          Tabs.Reports.handleReportsSearchCB (rslt, pageNum);     
      },
      onFailure: function (rslt) {
          Tabs.Reports.handleReportsSearchCB (rslt, pageNum);     
      },
