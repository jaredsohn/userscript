	...
	for (i in  spans) 
	{
		...
		if(getStr != undefined)
		{
			...
			if(startPos!=-1)
			{
				...
				for each(id in ids)
				{
					imgCodes += "<img style=\'max-width:100%' src=http://www.yuchirian.com/gif/" + (id) + ".gif>" + "<br/>";
				}
				
				s.innerHTML = description + imgCodes +  '<a href ='  +  decodeURIComponent(getUrl) + ' target=_blank>' +  '<img src=http://www.yuchirian.com/yuchirianGif/yuchirian.icon.jpg> ' + decodeURIComponent(getUrl)  + '</a>';
			}
		...
		for (i in  divs) 
		{
			...
			{
				var searchResult = getContent.search("yuchirian.com/gif");
				...
				var searchResult = getContent.search("Yuchirian|Gif");
		...
		for (ui in  udivs) 
		{
			...
			{
				var usearchResult = ugetContent.search("yuchirian.com/gif");
				...
				var usearchResult = ugetContent.search("Yuchirian|Gif");
