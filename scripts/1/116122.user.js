...
// @version        3.0
// @updateURL      https://userscripts.org/scripts/source/116122.meta.js
...
#content .sticky,img[src*=\"ad_\"],*[id*=\"banner\"],*[src*=\"banner\"],a[href*=\"aff\"] img,\
...
function doExec(e) {
  ...
  for (i=0; i<parafs.length; i++) {
    ...
    if (x = c1(".//center/blink", p)) {
      x.parentNode.className = " yodHide";
    }
    ...
    if (!crp_related || (i >= 1)) continue;
    if (internal) p3 = internal.parentNode;
    var isTP = p3 ? c1(".//a[contains(@href,'tny.cz')]", p3.parentNode) : false;
    //if(!isTP && crp_related) isTP = c1(".//a[contains(@href,'tny.cz')]", crp_related.previousElementSibling);
    if (!isTP && crp_related) {
      isTP = 
      c1(".//a[contains(@href,'tny.cz')]", crp_related.previousElementSibling)
       || c1(".//a[contains(@href,'tny.cz')]", crp_related.parentNode);
    }
...
' + yodUpdate['script_name'] + ' (' + yodUpdate['script_version'] + ')</a>\
 // <a href="http://blog.krakenstein.net/" target="_blank" title="Dev Page">CecekPawon 2o11</a>\
</div>\
...
// start INJECT
var yod_frun = true;
setCookies("ads=yod;gila_tvCap=1;_g.pop=1;");
