...
function getWeibo(){
  ...
  GM_xmlhttpRequest({
    method: "GET",
    url: 'http://photo.weibo.com/photos/getWbPhotos?uid='+g.uId+(g.ajax?'&offset='+g.ajax:''),
    onload: function() {
    var photodata=g.photodata,
    res=JSON.parse(this.response),elms=res.data;
    console.log(res);
    g.ajax=elms.length==30?elms[elms.length-1].mid:null;
    for(var i=0;i<elms.length;i++){
      var j=null,url='http://ww1.sinaimg.cn/large/'+elms[i].pid[0]+'.jpg';
      if(!g.downloaded[url]){g.downloaded[url]=1;}else{continue;}
      photodata.photos.push({
      title: elms[i].text || '',
      url: url,
      href: 'http://photo.weibo.com/'+g.uId+'/talbum/detail/photo_id/'+elms[i].mid,
      date: elms[i].created_at || ''
      });
    }
    ...
    if(qS('#stopAjaxCkb')&&qS('#stopAjaxCkb').checked){output();}
    else if(g.ajax){getWeibo();}else{output();}
  }
  });
}
unsafeWindow.dFAcore = function(setup) {
  ...
  if(location.href.match(/.*facebook.com/)){
    ...
    try{
      aName='微博配圖';
      aAuth=qS('.name').textContent
    }catch(e){}
    g.downloaded = {};
    g.uId = qSA('.pf_icon a')[3].href.match(/\/u\/(\d*)/)[1];
    g.photodata = {
      aName:aName,
      aAuth:aAuth,
      aLink:location.href,
