...
//
if ( localStorage.getItem("start") != 'off' ){
localStorage.setItem("btnProcent0", 100);
localStorage.setItem("btnProcent1", 75);
localStorage.setItem("btnProcent2", 50);
localStorage.setItem("btnProcent3", 25);
...
GM_addStyle(".tbox {position:absolute; display:none; padding:14px 17px; z-index:90000}\
...
var form = document.getElementById('formOpcje');
	...
	for(var count in btn){
	
	document.getElementById('opcje_procentBTC'+count).value= localStorage.getItem('btnProcent'+count);
...
var form = document.getElementById('formOpcje');
//Tworzenie zapisu procentów
	for(var count in btn){
			localStorage.setItem('btnProcent'+count, document.getElementById('opcje_procentBTC'+count).value);
		}
...
function opOkno(){
 TINY.box.show({html:"<div id='formOpcje'>"+
                "<fieldset>"+
...
"</fieldset>"+
"</br><div align='center'><input type='button' id='oOpcjeZapiszButton' value='Zapisz'/></br>Po zapisie strona zostanie odświeżona</div>"+                
"</div>"+
	""
...
var btnOpcje = document.createElement('input');
		with(btnOpcje){
		
		addEventListener('click', opOkno,false);
		setAttribute( 'value', 'Opcje skryptu' );
		setAttribute( 'type', 'button' ); 
		setAttribute( 'style', 'width: auto; cursor: pointer; margin-top: 5px; margin-left: 400px; ' );
...
function buyBTC(){
var buyPrice = parseFloat($('#buyPrice')[0].value);
var funds = parseFloat($('#buyForm .funds')[0].textContent.replace(',', '.').replace("$",""));
var nr = parseFloat(this.id);
var kwota = funds / buyPrice*( parseFloat(btn[nr])/100.0);
document.getElementById( 'buyAmount' ).value=kwota.toFixed(8)  ;
$( '#buyAmount' ).blur();
}
...
btn[0] = localStorage.getItem("btnProcent0")+'%';
btn[1] = localStorage.getItem("btnProcent1")+'%';
btn[2] = localStorage.getItem("btnProcent2")+'%';
btn[3] = localStorage.getItem("btnProcent3")+'%';
...
for(var count in btn)
{ 
		newBtn[count] = document.createElement('input');
		with(newBtn[count]){
		
		addEventListener('click', buyBTC,false);
		setAttribute( 'value', btn[count] );
		setAttribute( 'type', 'button' ); 
	...
	$('#buyForm .verticalShadow tbody tr:eq(1) td:eq(2)').append(newBtn[count]);
    $('#buyForm .compact tbody tr:eq(1) td:eq(0)').append(newBtn[count]);
}
...
function sellBTC(){
var funds = parseFloat($('#sellForm .funds')[0].textContent.replace(',', '.'));
var nr = parseFloat(this.id);
var kwota = funds *( parseFloat(btn[nr])/100.0);
document.getElementById( 'sellAmount' ).value=kwota.toFixed(8)  ;
$( '#sellAmount' ).blur();
}


//Tworzenie oraz wyświetlanie przycisku BTC
var newBtnSell = new Array()
for(var count in btn)
{ 
		newBtnSell[count] = document.createElement('input');
		with(newBtnSell[count]){
		
		addEventListener('click', sellBTC,false);
		setAttribute( 'value', btn[count] );
		setAttribute( 'type', 'button' ); 
        setAttribute( 'class', 'buttonBlue');
		setAttribute( 'style', 'width: auto; cursor: pointer; margin-top: 5px; margin-right: 3px; ' );
		setAttribute( 'id', count+'bidilosc' );
		
		}
    
	$('#sellForm .verticalShadow tbody tr:eq(1) td:eq(2)').append(newBtnSell[count]);
    $('#sellForm .compact tbody tr:eq(1) td:eq(0)').append(newBtnSell[count]);
}
...
for(var count in btn)
		{
			...
			{
				newBtn[count].setAttribute( 'style', 'display: none;');
				newBtnSell[count].setAttribute( 'style', 'display: none;');
			}
			
			if(localStorage.getItem('btnProcent'+count)== 0 || null || "" )
			{
				newBtn[count].setAttribute( 'style', 'display: none;');
				newBtnSell[count].setAttribute( 'style', 'display: none;');
			}
