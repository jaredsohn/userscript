...
function jjuds() {
	if (BDUSS == "") {
		var noBDUSS = confirm("tiebaAllsign无法获取BDUSS！必须先退出百度帐号，然后到wap版重新登录！\r\n\r\n点击“确定”，可直接切换到http://tieba.baidu.com/mo/进行注销和重登录。\r\n点击“取消”，那就是以后再说。。。\r\n\r\n\r\n（重登录之后，再回到这里应该就能签到了。）");
		if (noBDUSS == true) {
			window.location = "http://tieba.baidu.com/mo/";
...
function gowork(allAncs, baNameF) { //以获取的地址数组和吧名数组为参数
	...
	function onebyone(gg, goorstop) { //这里的gg是从0开始的贴吧序号，goorstop用于判别是否递进执行。
		...
		var tiebaname = "<a href='http://tieba.baidu.com/mo/m?kw=" + encodeURIComponent(baNameF[gg]) + "' title='" + baNameF[gg] + "吧' target='_blank'><font color='blue'>" + baNameF[gg].reComLength() + "吧</font></a>";
