// ==UserScript==
// @name Tribalwars Shortcut near village coords
// @description Www.tw-ar.com
// @author WWW.Tw-Ar.com , GunZ
...
(function() {
	...
	var dsrbAddButtons = function(node) {
		...
		var overview = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.target + '&screen=overview', icons.overview, 'النظره العامه للقريه');
		if(coords) {
			var center = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=map&x=' + coords[0] + '&y=' + coords[1], icons.center, 'اظهار القريه على الخريطه');
		}
		var troops = dsrbCreateButton('game.php?' + uv + 'village=' + vilid.own + '&screen=place&mode=command&target=' + vilid.target, icons.troops, 'ارسال الجيش');
