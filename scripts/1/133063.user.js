	...
	function eventlist( i, galaxie , systeme, f)
	{
		...
		{		
			...
			if (nbSauv == 4 )	
			{
				...
				GM_deleteValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+i+';'+f );
			
			}
			else 
			{
				document.getElementsByClassName('row')[i].getElementsByClassName(listeLettre[f])[0].style.backgroundColor='#'+ color_activ[(nbSauv+f)%4];
				GM_setValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+i+';'+f , nbSauv+1);
...
var url = location.href;
	...
	var uneCase = false ; // Boolean(GM_getValue(nomScript+'affichage1case'+serveur,false));
				...
				if (document.getElementsByClassName('row')[i].getElementsByClassName('planetname')[0])
				{
					if (!uneCase)
					{
						...
						for (var f = 0 ; f<4; f++)
						{
							...
							//Ancienne sauvegarde
								var old= parseInt(GM_getValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+i, '4' ));
								if(old != 4)
								{
									GM_setValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+i+';'+old, '1' );
								}
								GM_deleteValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+i);
							// Fin ancienne sauvegarde
							...
							if(k != 0)
									aff += '<input class="'+listeLettre[f]+'" style="cursor:pointer;border: solid '+color_border+' 1px; width: 15px;color:#FFFFFF; background-color:#'+color_activ[(k-1+f)%5]+';" value="'+listeLettreaff[f]+'" type="button">';
			...
			document.getElementsByClassName('GalMarq'+galaxie+';'+systeme+';'+position+';'+f)[n].addEventListener("click", function(event) 
			{
				...
				for (var n2 = 0 ; n2 < nbRc2 ; n2++)
				{
					...
					if(k == 4)
					{
						...
						GM_deleteValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+position+';'+f);
								
					}
					else 	
					{
						document.getElementsByClassName('GalMarq'+galaxie+';'+systeme+';'+position+';'+f)[n2].style.backgroundColor='#'+ color_activ[(f+k)%4];
						
						
									
						GM_setValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+position+';'+f  ,k+1);
					}
				}
			}, true);
	...
	{		
		...
		for (var i =0 ; i< lieuCoord.length ; i++)
		{
			if(lieuCoord[i].getElementsByTagName('a')[0])
			{
				...
				{
					//Ancienne sauvegarde
								var old= parseInt(GM_getValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+position, '4' ));
								if(old != 4)
								{
									GM_setValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+position+';'+old, '1' );
								}
								GM_deleteValue(nomScript+'infoCoord'+serveur+';'+galaxie+';'+systeme+';'+position);
							// Fin ancienne sauvegarde
					...
					if(k != 0)
					{
						...
						newElement.innerHTML = '<input class="GalMarq'+galaxie+';'+systeme+';'+position+';'+f+'"  style="cursor:pointer;border: solid '+color_border+' 1px; width: 15px;color:#FFFFFF; background-color:#'+color_activ[(f-1+k)%4]+';" value="'+listeLettreaff[f]+'" type="button">';
