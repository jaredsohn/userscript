...
// @namespace      ikariam
// @version        0.2.8
// @downloadURL    https://userscripts.org/scripts/source/130789.user.js
// @updateURL      https://userscripts.org/scripts/source/130789.meta.js
//
...
//
// @history        0.2.8 05/10/2012 Languages : ru, CR in replies, new soldier icon in CR.
// @history        0.2.7 05/09/2012 Fixed blue buttons in outbox. Fixed messing with GF global messages.
// @history        0.2.6 05/07/2012 Converts URLs into links.
// @history        0.2.5 05/07/2012 Localised numbers in combat reports. Chrome fix.
// @history        0.2.4 05/07/2012
...
// @history        0.2.2 04/26/2012 Languages : de, es, it, ro
...
jQuery.fn.extend({
// Ã‡a marche aussi dans un text input.
...
GM_addStyle(".emotiworldIcon { float:left; line-height:1.5em } \
	...
	div.emotiworldCRIcon { position:absolute; background:url(/skin/advisors/military/bang_soldier.png); width:35px; height:38px } \
	...
	td.msgText a:not(.button) { color:#00e } \
");
...
var Emotiworld = new function() {
	...
	this.languageData = {
		...
		'fr':{
			manage:'GÃ©rer les Ã©moticÃ´nes',
			add:'Ajouter',
			findIcons:"Trouver d'autres Ã©moticÃ´nes sur [link]",
			close:'Fermer',
			replyToAll:'RÃ©pondre Ã  tous',
			CRUndoTitle:'Emotiworld a trouvÃ© un rapport de combat. Cliquez ici pour revenir au message de base.'
		},
		'de':{
			...
			CRUndoTitle:'Emotiworld hat einen Kampfbericht gefunden. Klicken Sie hier, um grundlegende Botschaft zurÃ¼ck.'
		},
		'es':{
			...
			CRUndoTitle:'Emotiworld ha encontrado un informe de batalla. Haga clic aquÃ­ para convertirla en mensaje bÃ¡sico.'
		},
		'it':{
		...
		'ro':{
			...
			add:'AdÄƒuga',
			findIcon:'GÄƒsiÅ£i alte emoticoane pe [link]',
			close:'Aproape',
			replyToAll:'RÄƒspunde la toate',
			CRUndoTitle:'Emotiworld a gÄƒsit un raport de luptÄƒ. Click aici pentru a reveni la mesajul de bazÄƒ.'
		},
		'ru':{
			manage:'Ð£Ð¿Ñ€Ð°Ð²Ð»ÐµÐ½Ð¸Ðµ ÑÐ¼Ð°Ð¹Ð»Ð¸ÐºÐ°Ð¼Ð¸',
			add:'Ð”Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ',
			findIcons:'ÐÐ°Ð¹Ñ‚Ð¸ Ð½Ð¾Ð²Ñ‹Ñ… ÑÐ¼Ð°Ð¹Ð»Ð¸ÐºÐ¾Ð² Ð¿Ð¾ [link]',
			close:'Ð—Ð°ÐºÑ€Ñ‹Ñ‚ÑŒ',
			replyToAll:'ÐžÑ‚Ð²ÐµÑ‚Ð¸Ñ‚ÑŒ Ð²ÑÐµÐ¼',
			CRUndoTitle:'ÐœÑ‹ Ð½Ð°ÑˆÐ»Ð¸ Ð½Ð¾Ð²Ñ‹Ð¹ Ð±Ð¾ÐµÐ²Ð¾Ð¹ Ð´Ð¾ÐºÐ»Ð°Ð´. ÐÐ°Ð¶Ð¼Ð¸Ñ‚Ðµ Ð·Ð´ÐµÑÑŒ Ñ‡Ñ‚Ð¾Ð± Ð²ÐµÑ€Ð½ÑƒÑ‚ÑŒÑÑ Ðº Ð¾ÑÐ½Ð¾Ð²Ð½Ð¾Ð¼Ñƒ ÑÐ¾Ð¾Ð±Ñ‰ÐµÐ½Ð¸ÑŽ.'
		}
	};
	...
	this.ikariamInit = function() {
		...
		// Ici on est sÃ»r d'Ãªtre dans ikariam.
		if(unsafeWindow.ajax && unsafeWindow.ajax.Responder) {
			...
			unsafeWindow.ajax.Responder.changeView = function(params) {
				...
				if((id == 'sendIKMessage') || (id == 'diplomacyIslandBoard')) {
					// toolbar insertion
					// On l'insÃ¨re dans le code html avant son intÃ©gration Ã  la page, pour que la hauteur de la fenÃªtre soit bien calculÃ©e par ikariam.
					setTimeout(function(){
						...
						} // else on a un problÃ¨me : revoir la rÃ©ponse json
	...
	/** DÃ©termine la langue d'aprÃ¨s le paramÃ¨tre. Si on ne trouve pas on prend l'anglais. */
	...
	/** gÃ©nÃ¨re le code html de la barre d'icÃ´nes */
	...
	/** gÃ©nÃ¨re le code html complet Ã  insÃ©rer. */
	this.configPanelHTML = function() {
		...
		html += lang['findIcons'].replace(/\[link\]/, '<a href="http://emotiworld.com" target="_blank" style="font-weight:bold">emotiworld.com</a>')+' ã‹¡</div>';
		html += '<div id="emotiworldClose"><a href="" onclick="return false;">'+lang['close']+' â˜’</a></div>';
		html += '</div>';
	...
	this.CRBlock = function(str, p1, p2) {
		return '<div class="emotiworldCR">' +
			...
			p1.replace(/(^|\n)(&gt; )+/g, '$1')
				.replace(/(?:([^.]+)\.+(\d+)(\(-\d+\)))?\.* - (?:([^.]+)\.+(\d+)(\(-\d+\)))?\.*\s*<br\s?\/?>/g, Emotiworld.CRLineLosses) +
			'</table><hr/><table class="CRSummary">' +
			p2.replace(/(^|\n)(&gt; )+/g, '$1')
				.replace(/([^.]+)\.+([-,%\d]+) - ([^.]+)\.+([-,%\d]+)\s*<br\s?\/?>/g, Emotiworld.CRLineSummary) +
	...
	this.addLinks = function(diplo) {
		$('tr:not(.globalmessage) td.msgText', diplo).each(function(index) {
			var content = $(this).html();
			content = content
				.replace(/\[(\d\d+):(\d\d+)]/g, Emotiworld.islandLink)
				// capture html <a>...</a>, generic <...> tags and http urls
				.replace(/<a.*?\/a>|<.*?>|(https?:\/\/.+?)([\s()\[\]<>]|$)/g, Emotiworld.genericLink);
			$(this).html(content);
		});
	}

	this.genericLink = function (str, url, after) {
		if(str[0] == '<') return str;				// if we captured an html tag, don't replace it.
		var pos = url.lastIndexOf('.');
		var linkClass = '';
		return '<a href="'+url+'"'+linkClass+' target="_blank">'+url+'</a>'+after;
	}
	...
	this.postInit = function() {
		...
		$('#emotiworldAdd').click(function() {
			var icon = prompt("Copiez ici l'Ã©moticÃ´ne Ã  ajouter :");
			Emotiworld.add(icon);
		});
...
//		$( "#emotiworldSortable" ).disableSelection();	// ne marche pas
	...
	/** Insertion de l'Ã©moticÃ´ne cliquÃ©e dans le message. */
	this.insert = function() {
		Emotiworld.textarea.insertAtCaret( $(this).text() );
	};

	/** Renvoie la liste d'Ã©moticÃ´nes affichÃ©e dans le panneau de config. */
	...
	/** Enregistre la nouvelle lsite d'Ã©moticÃ´nes. MÃ©thode appelÃ©e Ã  la fermeture du panneau de config.	 */
	this.save = function() {
	...
	/** Ajoute une Ã©moticÃ´ne Ã  la fin de la liste. */
...
$(function() {
	...
	if(!Emotiworld.icons)
		Emotiworld.icons = ['ãƒ„', '(â—â€¿â—)', '(Ë˜â—¡Ë˜)', 'â—¤(Â¬â€¿Â¬)â—¥', 'â”Œâˆ©â”(â—£_â—¢)â”Œâˆ©â”', '(Â´á—£ï½€)', '(Âº_â€¢)', 'âŠ™_âŠ™', '(â•¯Â°â–¡Â°ï¼‰â•¯ï¸µ â”»â”â”»', '=^..^=', 'ì›ƒ'];
