...
if (location.href.indexOf('http://www.jeuxvideo.com/forums/') == 0) {
        ...
        var TurboForum = function() {
                ...
                function readystatechange(e) {
                        ...
                        /* Arrive quand une requÃªte est complÃ©tÃ©e */
 
                        if (p[id].xhr.status != 200) {
                                if (p[id].xhr.status != 404) {
                                        window.console && console.log('p[' + id + '].xhr.status â‰  200 (' + p[id].xhr.status + ') ' + p[id].url)
                                }
                                if (p[id].xhr.status == 0) {
                                        /* Arrive alÃ©atoirement, quand Ã§a arrive lâ€™inspecteur rÃ©seau de Chrome ne montre pas de requÃªte */
 
                                        /* Quand Ã§a arrive, les requÃªtes futures vers cette page donneront la mÃªme erreur */
                                        /* Sauf si on crÃ©e une image avec comme src lâ€™url du lien, les requÃªtes futures marchent (sous Chrome du moins) */
                ...
                function preload(url) {
                        ...
                        Ou aprÃ¨s que p[p_id].state == 'displayed' dans display (quand on clique une deuxiÃ¨me fois sur un forum prÃ©fÃ©rÃ©) */
                       
                        var id = p_id
 
                        if (p[p_id].state == 'waiting') {
                                /* Arrive quand on survole un lien alors quâ€™un autre (ou le mÃªme) est dÃ©jÃ  en train de charger */
                                id ^= 1
                        }
                ...
                function display(url, t) {
                        ...
                        Ou 50ms aprÃ¨s que !p[p_id].html
                        Ou 50ms aprÃ¨s que p[p_id].state == 'displayed' */
 
                        if (p[p_id].state == 'preloading') {
                                /* PremiÃ¨re fois */
                                if (t < 1000) {
                                        /* On ne vient pas dâ€™hover un autre lien */
                                        t += 1000
                                }
                        }
                        else {
                                ...
                                if (p[p_id].state == 'waiting' && t < 1000) {
                                        /* Il nâ€™y a pas eu de preloading prÃ©alable, Ã§a arrive quand on clique sur le mÃªme lien avant que celui-ci ai Ã©tÃ© displayed, p[p_id].state est directement en waiting et cette instance de display tourne en parralÃ¨le de la premiÃ¨re */
                                       
                                        /* Ou alors on a cliquÃ© sur un autre lien pendant quâ€™on en charge un */
                        ...
                        if (p[p_id].url != url) {
                                /* Arrive quand on a swappÃ© p_id, `url` est lâ€™url prÃ©cÃ©dente */
                                return
                        }
                        if (!p[p_id].html) {
                                /* La rÃ©ponse du serveur nâ€™a pas encore Ã©tait reÃ§u, ou il y a eu un problÃ¨me avec la requÃªte */
                        ...
                        if (p[p_id].state == 'displayed') {
                                /* Ã‡a arrive quand on clique Ã  la suite sur les forums prÃ©fÃ©rÃ©s */
                                if (!col1.style.opacity) {
                                        col1.style.opacity = opacity
                                }
                                preload(url)
                        ...
                        Utilisateur() // fonction de JVC pour mettre Ã  jour les MP
                        if (p[p_id].url == no_hash(location.href)) {
                                /* Bouton rafraÃ®chir, on rajoute de lâ€™opacitÃ© un court moment pour montrer que le rafraÃ®chissement a bien eu lieu. */
                                col1.style.opacity = opacity
                        ...
                        speed_up_all() // Ã€ faire aprÃ¨s history.pushState pour faire des conditions sur location.href dans speed_up_all
                }
                ...
                function speed_up_all(forums_prefs) {
                        ...
                        for (var i = 0; i < links.length; i++) {
                                ...
                                if (a.href.indexOf('http://www.jeuxvideo.com/forums/') != 0) {
                                        /* On ne prÃ©charge pas en dehors des forums */
                                        continue
                                }
                                ...
                                if ([0, 1, 3, 26].indexOf(mode) == -1) {
                                        /* On prÃ©charge seulement liste des sujets, page dâ€™un topic, formulaire de rÃ©ponse, retour liste des sujets */
                                        continue
                                }
                                if (a.target) {
                                        continue
                                ...
                                if (a.className == 'sup_pref') {
                                        /* Bouton pour supprimer un forum prÃ©fÃ©rÃ© */
                                        continue
                                }
                        ...
                        /* Autofocus sur le champ de rÃ©ponse + affichage citation */
                ...
                function store_quote(id) {
                        if (!geid('message_' + id)) {
                                window.console && console.log('canâ€™t quote ' + id)
                                return
                        }
                ...
                function textToQuote(citation_html, lien_citation) {
                        ...
                        // var de = 'aeiou'.indexOf(pseudo[0].toLowerCase()) == -1 ? 'de ' : 'dâ€™';
                        var de = 'de '
       
                        var date = str_between(citation_html, '<li class="date">', '<a').replace(/<span>via mobile<\/span> /, '').replace(/\r?\n/g, '').replace('PostÃ© le ', '').trim();
                        var aujourdhui = ''
                        var date_a = date.split(' ')
                        var date_o = new Date()
                        date_o = [
                                date_o.getDate(),
                                'janvier,fÃ©vrier,mars,avril,mai,juin,juillet,aoÃ»t,septembre,octobre,novembre,dÃ©cembre'.split(',')[date_o.getMonth()],
                                date_o.getFullYear()
                        ]
                        ...
                        citation_texte += '| ' + pseudo + '  -  ' + (aujourdhui ? ('aujourdâ€™hui Ã  ' + date.split('Ã  ')[1].split(':')[0] + ':' + date.split('Ã  ')[1].split(':')[1]) : ('le ' + date.split('Ã ')[0])) + "\r\n"
                        if (lien_citation) {
                ...
                function add_goodies(html) {
                        /* Ignorer */
                        /* Agit sur lâ€™icÃ´ne des dossiers, Ã  faire avant Lastpage */
                        ...
                        if (html.indexOf('<li class="ancre">') != -1) {
                                var ancres = html.split('<li class="ancre">'),
                                        url_repondre = html.split('<li class="ancre"><a href="')[1].split('"')[0] // Ne pas rÃ©cupÃ©rer lâ€™URL du bouton rÃ©pondre : il nâ€™y est pas sur les topics lockÃ©s
                        ...
                        /* Liens citer - formulaire de rÃ©ponse */
                        ...
                        if (jvchat_on == 1) {
                                ...
                                /* Topic */
                                if (html.indexOf('alt="RÃ©pondre" width="80" height="17" />') != -1) {
                                        var url = str_between(html, 'alt="RÃ©pondre" width="80" height="17" />', '" class="bt_rafraichir"').split('/').pop()
                ...
                function show_update() {
                        ...
                        bar.innerHTML = '<span class="tf_update_bar_close">Ã—</span><div><b>TurboForum</b> : Une <a href="http://jvflux.com/turboforum/?from=' + turboforum_version + '" target="_blank">nouvelle version</a> est dÃ©sormais disponible.</div>'
                                ...
                                for (var i = 1; i < messages.length; i++) {
                                        ...
                                        if (has) {
                                                ...
                                                if (message.indexOf('target="kick_user">') != -1) {
                                                        message = message.replace(/(<a href="http:\/\/www\.jeuxvideo\.com\/cgi-bin\/jvforums\/kick_user\.cgi[^>]+target="kick_user">)/, '<a class="add_blacklist" href="#" title="Ne plus dÃ©finir ' + pseudo + ' comme indÃ©sirable" message=' + id + '></a> $1')
                                                }
                                                else {
                                                        message = message.replace(/<a target="avertir"/, '<a class="add_blacklist" href="#" title="Ne plus dÃ©finir ' + pseudo + ' comme indÃ©sirable" message=' + id + '></a> <a target="avertir"')
                                                }
                                        }
                                        else {
                                                if (message.indexOf('target="kick_user">') != -1) {
                                                        message = message.replace(/(<a href="http:\/\/www\.jeuxvideo\.com\/cgi-bin\/jvforums\/kick_user\.cgi[^>]+target="kick_user">)/, '<a class="add_blacklist" href="#" title="DÃ©finir ' + pseudo + ' comme indÃ©sirable" message=' + id + '></a> $1')
                                                }
                                                else {
                                                        message = message.replace(/<a target="avertir"/, '<a class="add_blacklist" href="#" title="DÃ©finir ' + pseudo + ' comme indÃ©sirable" message=' + id + '></a> <a target="avertir"')
                                                }
                                        }
                                        messages[i] = message
                                }
                ...
                function blacklist_dom() {
                        /* UtilisÃ© quand on ignore quelquâ€™un sur le formulaire de rÃ©ponse, on ne peut recharger col1 sans perdre le message */
                        ...
                        for (var i = 0; i < msgs.length; i++) {
                                ...
                                if (has) {
                                        ...
                                        button.title = 'Ne plus dÃ©finir ' + pseudo + ' comme indÃ©sirable'
                                }
                                else {
                                        msg.classList.remove('msg_blacklisted')
                                        button.title = 'DÃ©finir ' + pseudo + ' comme indÃ©sirable'
                                }
                        }
                }
                ...
                addEventListener('popstate', function() {
                        /* Arrive quand on utilise les boutons prÃ©cÃ©dent/suivant du navigateur */
                        if (current_pathname == location.pathname) {
                                /* Lien permanent */
                                return
                        }
                        if (!p_history.html[no_hash(location.href)]) {
                                /* Arrive quand on a rafraÃ®chit la page au prÃ©alable et que TurboForum nâ€™a plus ce lien en mÃ©moire */
                ...
                if (geid('fiche_infos2')) {
                        /* On retire la pub pour Amazon/Fnac sur les forums de jeux, elle est dans col2 alors quâ€™on update uniquement col1 */
                        remove_element('fiche_infos2')
                }
                if (geid('sponso_forum')) {
                        /* PrÃ©sent sur certains forums de jeux comme LoL */
