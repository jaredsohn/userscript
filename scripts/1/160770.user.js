// ==UserScript==
// @name		Ikariam General Assistant
...
//
// @history		1.1 	custom script settings
// @history		1.0.1 	new layout and options
// @history		1.0 	option to fetch island coordinates in the report
// @history		0.1.1	fixed problem with include
// ==/UserScript==
...
var languages =
{
	rs:
	{
		...
		'copy_info'				: 'Кликните на текст изнад и притисните Ctrl+C да копирате извештај',
		...
		'no_new_attacks' 		: 'Нема нових напада',
	...
	en:
	{
		...
		'settings' 				: 'Settings',
		'copy' 					: 'Copy Report',
		'copy_info'				: 'Cick in the text area above and pres Ctrl+C',
...
Ika = {
	...
	script_version	: '1.1',
	script_url		: 'http://userscripts.org/scripts/show/102032',
	...
	if (Ika.view == 'embassyGeneralAttacksToAlly')
	{
		...
		$("#mainview .table01 .rowRanks").each( function (key, v)
		{
			...
			if (t != '')
			{
				...
				d += '<span class="attackTime">' + attackedTime + '</span><br />\n';
				d += '[<span class="attackedCoordinates" id="attackedCoordinates' + cnt + '"></span>] ' + attackedType;
				d += '<br />\n';
				d += '<span class="source"><a target="attackWindow" href="' + attackedFromUrl + '">' + attackedAttacker + '</a></span> (<span class="unitsCount">' + attackedUnits + ' ' + unitsText + '</span>) '
					+ atackStatus + ' <span class="destination"><a target="attackWindow" href="' + attackedUrl + '">' + attackedDefender + '</a></span>';
//				d += '<span class="attackUrls">' + attackedFromUrl + ' ► ' + attackedUrl + '</span>';
				d += '<br />\n<br />\n</div>';
		...
		var html_content = ''
			...
			+ '</div>'
			+ '<p><br /><a href="' + Ika.script_url + '">' + Ika.script_name + ' v' + Ika.script_version + '</a></p>'
			+ '</div>'
			+ '</div>';
