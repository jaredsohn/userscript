...
// @description    Experimental 108086
...
// @require 	   	 http://sizzlemctwizzle.com/updater.php?id=108086
// ==/UserScript==

var Version = '0.0.3';
var crightmsg="Version "+Version +" by DonR 2011 Based others' scripts"
...
// End Set Wide function
   setWide();
// End Facebook Instance
}
...
function ptStartup() {
 ...
 GM_addStyle (styles); 
  ...
  mainPop = new CPopup ('idp', Options.ptWinPos.x, Options.ptWinPos.y, 655,600, true, 
      function (){
        my[currentName].hide();
  ...
  //AddMainTabLink("GoRBot", eventHideShow, mouseMainTab);
...
my.Hud = {
   ...
   show : function () {
     var t = my.Hud;
       ...
       t.cont.innerHTML = '\
           <DIV class=ptstat>Research Attacks</div>\
           ...
           <span id=idSpanHudErrorMsg></span><td><select style="display:none;" id="idHudTypeSearch"><option value="">All Reports</option><option value="0">Incoming Attacks</option><option value="1">Outbound Attacks</option><option value="2">Reinforcement</option></select>\
           <select id="idHudMedSearch"><option value="">All</option><option value=1>See My Reports</option></select></td></tr>\
           </table></div>\
    ...
    showReportBody: function (Date1,rslt,TileId,SideNum,Nom1,Coord1,Nom2,Coord2) {
      	...
      	if (rslt['winner']==0) m+='<TR><TD><FONT color="green" size="4px"><b>Victory</b><br></font></td></tr></table>';
       	if (rslt['fght'] != undefined){
    			m+='<TABLE style="float:left;width:45%;" class=ptTab><tr><td colspan=3><b>Attacker : '+Nom1+' <a href="javascript:void(0)" onclick="KB.Controllers.MapHelper.gotoCoord('+ Coord1 +');" class="coordinateLink">('+ Coord1 +')</a></b><br>Bonus : '+ parseInt(rslt['s1atkBoost']*100)  +' % attack - '+ parseInt(rslt['s1defBoost']*100) +' % defence</td></tr>\
<TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Troops</td><TD align="center">Fought</td><TD align="center">Survived</td></tr>'; 
    			...
    			m+='</table><TABLE style="float:right;width:45%;" class=ptTab><tr><td colspan=3><b>Defender : '+Nom2+' <a href="javascript:void(0)" onclick="KB.Controllers.MapHelper.gotoCoord('+ Coord2 +');" class="coordinateLink">('+ Coord2 +')</a></b><br><br><br>Bonus : '+ parseInt(rslt['s0atkBoost']*100)  +' % attack - '+ parseInt(rslt['s0defBoost']*100) +' % defence</td></tr>';	  			  	
    		  	if (rslt['fght']["s0"] != undefined) {
  					 ...
  					 if (rslt['fght']["s0"]['f'+i] != undefined) {
  					  if (rslt['fght']["s0"]['f'+i][0] > rslt['fght']["s0"]['f'+i][1]) m+='<TR><TD align="center"><div  class="pic px30 units unit_'+i+'"></td><TD align="center">'+rslt['fght']["s0"]['f'+i][0]+'</td><TD align="center"><FONT color="#CC0000">'+rslt['fght']["s0"]['f'+i][1]+'</font></td></tr>';
  				  		  	 ...
  				  		  	 m+="<tr><td><br>No Units Defended</td></tr>";
  	  		 }
    		  	m+='<TR><TD></TD></TR></table>';
    	}
      	
      	if (rslt['unts']!= undefined) { // For Reinforcements
      	    // Renfort !
    	...
    	if (rslt['rsc'] != undefined) {
    	  ...
    	  // Reinfocement !
     	   m+='<TABLE style="float:left;width:45%;" class=ptTab cellpadding=3><TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Troops</td><TD align="center">Reinforcement;</td></tr>';
  	 ...
  	 if (rslt['conquered']==1) m+='<TD><FONT color="#CC0000" size="3px">Conquered</font></td></tr>';
  	     ...
  	     if (rslt['winner']==1 || rslt['winner']==2) m+='<TR><TD style="background-color:white;"><FONT color="green" size="3px"><b>Winner<br><br></font></td></tr></table>';
  	  	...
  	  	if (rslt['pop'] != undefined) {
  	  	   ...
  	  	   m+='<table style="float:left;width:45%;"class=ptTab><TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><td align="center">City<td align="center">&nbsp;</tr><TR><TD><img width=30 height=30 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/population_30.png><TD>'+addCommas(parseInt(rslt['pop']))+'</td></tr>';
  	     ...
  	     var typebutin ='';
  	     	...
  	     	if (rslt['loot'] != undefined) {
  	     		m+='</td><tr><td style="background-color:white;"><b><u>Resource '+typebutin+' :</u></b><br>';
  	     	  		  	...
  	     	  		  	m+='<TD>Iron : </td><TD>'+addCommas(parseInt(rslt['loot'][4]))+'</td></table>';
  	  	}
  	   ...
  	   m+="</table>";
      	...
      	t.popReport.getTopDiv().innerHTML = '<TD align="center"><B>Battle Report - '+unsafeWindow.formatDateByUnixTime(Date1)+'</td>';
    ...
    DisplayReports : function (){
      ...
      var m = '<center><table width=100% cellspacing=0><thead><th>Page</th><th>R.</th><th>Date</th><th colspan=3>Attacker</th><th>Type</th><th colspan=4>Destination</th></thead>';
...
var TowerAlerts = {
  ...
  postToChat: function(f, c, force){
    ...
    if(Cities.byID[f.id].c.wilds[c.to.tileId]){
     ...
     msg = msg.slice(0, -2);
     msg += ' (arriving in ' + arrivalTime +').' ;
     
    var mess = "**ATTENTION** I am being attacked at " + a +" by " + " " +attaquant + " " + msg;
/*
        msg += '.';
        var emb = getCityBuilding(c.to.CityId, 8);
    		if (emb.count > 0){
  ...
  e_eachMinute : function (){   
    ...
    if (Options.enableFoodWarnTchat)  {
      for(i=0; i < Cities.numCities; i++) {
        ...
        var usage = parseInt(Cities.cities[i].c.upkeep());
        //row[i] = rp[1] - usage;
        
    	var timeLeft = parseInt(Cities.cities[i].c.resources[1].count)  / usage * 3600;
          var msg = '';
          if (timeLeft<0){
           }
          else if (timeLeft<(Options.foodWarnHours*3600)) {
                msg += 'My City ' + Cities.cities[i].c.name.substring(0,10) + ' (' +
                       Cities.cities[i].c.x +','+ Cities.cities[i].c.y + ') ';
                msg += ' is running short of food. Current Stock : '+addCommasWhole(foodleft).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ')+' ('+timestrShort(timeLeft)+') Production : '+addCommas(usage).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ')+'/h';
                sendChat ("/a " + msg);
                //alert(msg);
          }
      }  
 ...
 init : function (){
  ...
  var t = my.Reassign;
   ...
   nHtml.Click(document.getElementById("city_"+SourceId));
     ...
     //var rplevel=parseInt(getCityBuilding (t.sourceCity.c.id, 12).maxLevel); // 12=Point de ralliement
...
*/
       ...
       t.statutREA.innerHTML = '<FONT COLOR=#550000>Can not reassign more than '+maxtroupe+' of troops that you have!.</font>';
       return;
      }
 ...
 init : function (){
  ...
  show : function (){  
   ...
   var ModelCity = {};
    ...
    if (t.state == null) {  
      m = "<DIV class=ptstat>Transport Goods from one city to another </div>";
    	 	...
    	 	var entTotal=0;
    	  	for(i=0; i<Cities.numCities; i++) {
    	  	       ...
    	  	       if ( parseInt(Cities.cities[i].c.resources[1].hourlyTotalRate()) < parseInt(Cities.cities[i].c.upkeep()) ) {
    	  	        // entretien supÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rieur ÃƒÆ’Ã†â€™  la production
...
if (unsafeWindow.arStrings.unitName["u7"]) {
  ...
  verifierWagons: function() {
   ...
   if (saisw > maxw) {
      ...
      t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>The quantity may not exceed '+maxw+' !.</font>';
   }
   t.estimerRes();
  },
  estimerRes: function() {
   ...
   //t.estimationRes.innerHTML += "<br>Estimated Running Time : <b>" + m.friendEtaStr + "</b>" ; 
  ...
  clickTransportDo: function() {   // Transport Function
   var t = my.TranspAuto;
   var SourceId = t.sourceCity.c.id;
   ...
   if (!document.getElementById("ChoixRess0").checked && !document.getElementById("ChoixRess1").checked && !document.getElementById("ChoixRess2").checked && !document.getElementById("ChoixRess3").checked && !document.getElementById("ChoixRess4").checked) {
       t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Please Choose Resource Type !</font>';
      return;
   }
   if (t.sourceCity.c.x==t.destinationCityx.value && t.sourceCity.c.y==t.destinationCityy.value) {
      t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Cannot send to and from same city !.</font>';
     return;
   }
   if (parseInt(t.Choixnbwagon.value)=="0") {
   t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Cannot send with No transport !.</font>';
     return;
   }
   var x=t.destinationCityx.value;
   var y=t.destinationCityy.value;
   if (x == 0 || y == 0) {
      t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Cannot send to destination 0,0 !.</font>';
     return;
   }
   t.TTbutTransport.disabled=true;
  ...
  unsafeWindow.AjaxCall.gPostRequest("march.php",c,
	  ...
	  var rslt = transport;
          ...
          var timediff = parseInt(rslt.eta) - parseInt(rslt.initTS);
	                     ...
	                     t.divTranportStatus.innerHTML = "<font size='3px'><b>Transport Sent.</b>";
                   t.TTbutTransport.disabled=false;
          
          } else {
		    t.divTranportStatus.innerHTML ="<font color=red size='3px'><b>Error, Please try again !<b></font>";
		     if (rslt.msg) {
...
<TABLE CLASS="MYTABLE" CELLPADDING=3 CELLSPACING=0 align=center>\
    <CAPTION CLASS="MYTABLE"><center><b>Marks needed for new Cities</CAPTION>\
    \
    <THEAD >\
      <TR CLASS="MYTABLE">\
    ...
    <TBODY>\
      <TR CLASS="MYTABLE">\
        ...
        <TD CLASS="MYTABLE" style="background-color:'+ville2+';">Requires Level 7 (Actual Level : ' + unsafeWindow.player.level + ')\
        <br>5 Cohorts</TD>\
      </TR>\
      <TR CLASS="MYTABLE">  \
...
my.Options = {
  ...
  show : function (){
    var t = my.Options;
      try {      
        m = '<DIV style="height:580px; max-height:580px; overflow-y:auto"><TABLE class=ptTab>\
          <TR><TD colspan=2><font size=4><B><u>Toolbox Settings:</u></b></font><br></td></tr>\
          <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Allow Window Movement</td></tr>\
         <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>Automatically Organise Tax Day when Happiness >= <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%)</td></tr>\
         <TR><TD><INPUT id=pbRessEnable type=checkbox /></td><TD>AutoCollect resources every <INPUT id=pbLimitRess type=text size=2 maxlength=3 \> minutes</td></tr>\
        <TR><TD>&nbsp;</td><td>Show in red when not enough food for <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> hours</td></tr>\
        <TR><TD colspan=2><B>Chat Configuration :</b></td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Allow chat on right side</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarnTchat type=checkbox /></td><TD>Automatically post when running low on food</td></tr>\
        <TR><TD><INPUT id=HelpReq type=checkbox /></td><TD>Click automatically on help requests</td></tr>\
        <TR><TD><INPUT id=DelReq type=checkbox /></td><TD>Automatically delete help requests</td></tr>\
	...
	<TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Post notification to Alliance Chat on attack </td></tr>\
        </table></table><BR><BR><HR>'+ crightmsg+'</div>'
        t.cont.innerHTML = m;
  ...
  show : function (){
    ...
    clearTimeout (t.displayTimer);
      ...
      str = '<div style="height:550px;max-height:550px;overflow-y:auto"><DIV class=ptstat style="margin-top:2px; margin-bottom:2px; "><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD>' + unsafeWindow.player.name + ' <SPAN class=ptStatLight>Start Date :</span> '+ dt.toLocaleDateString() +'</td>\
...
var m="";
      ...
      str += "<tr><td style='background: #e8e8e8' align=right><b>Population Limit</td><td style='background: #e8e8e8' align=right>"+addCommas(popTotal)+" "+ m+"</tr>"; 
      ...
      var m="";
            ...
            str += "<tr><td style='background: #e8e8e8' align=right><b>Actual Population</td><td style='background: #e8e8e8' align=right>"+addCommas(popTotal)+" "+ m+"</tr>"; 
      ...
      str += "<tr><td style='background: #e8e8e8' align=right><b>Labour</td><td style='background: #e8e8e8' align=right>"+addCommas(popTotal)+" "+ m+"</tr>"; 
   ...
   var m="";
            ...
            str += "<tr><td style='background: #e8e8e8' align=right><b>Tax</td><td style='background: #e8e8e8' align=right>&nbsp; "+ m+"</tr>"; 
	...
	var entTotal=0;
 	for(i=0; i<Cities.numCities; i++) {
        ...
        var m="";
	 	var entTotal=0;
	 	var variiiable="Script GorBot" + " by "+ "Mark";
	  	for(i=0; i<Cities.numCities; i++) {
	  	       color='black';
	  	       if ( parseInt(Cities.cities[i].c.resources[1].hourlyTotalRate()) < parseInt(Cities.cities[i].c.upkeep()) ) {
	  	        // entretien supÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rieur ÃƒÆ’Ã†â€™  la production
...
my.AllianceList = {
  ...
  show : function (){
    ...
    if (t.state == null){
     ...
     var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
            ...
            <TR><TD class=xtab>OR </td><TD class=xtab>Alliance Name (Contains) : &nbsp;</td>\
             <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit DISABLED type=submit value="Search Alliance" /></td>\
  ...
  eventPlayerSubmit : function (){
      ...
      document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Current Research ...</center>';
      t.fetchPlayerList (name, t.eventGotPlayerList);
  },
  clickedPlayerLeaderboard : function (span, uid){
      var t = my.AllianceList;
      span.onclick = '';
      span.innerHTML = "Research on the glory roll ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
      ...
      if (rslt.totalResults == 0){
        span.innerHTML = "<B>Praise for Good Perfomance :</b> Not Found ! (in the mist ?)";
        return;
      }
      var p = rslt.data[0];
      ...
      m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Glory Table : </b></td><TD colspan=2> Glory : '+ p.might  +' &nbsp; Alliance : '+ an +'</td></tr>'; 
      for (var i=0; i<p.cities.length; i++){
        var c = p.cities[i];
        m += '<TR><TD align=right><B>City #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName 
        +' <a href="javascript:void(0)" onclick="cm.utils.CoordinateLinkController.onClick(event)" class="coordinateLink">('+ c.xCoord +',' +c.yCoord+ ')</a></td><TD width=75%> &nbsp; Level : '
  
          + c.tileLevel +' &nbsp; &nbsp; status: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; believes : ' + c.dateCreated.substr(0,10) +'</td></tr>';
      }  */
      //span.innerHTML = m + '</table>';
      span.innerHTML = "<B>Glory Roll :</b>Under Construction<br>Number of Cities :"+p.numOfCities+"<br>Number of Wilds : "+p.numOfWilds;
  },
  ...
  fetchPlayerStatus : function (uidArray, notify){
   ...
   clickedPlayerDetail : function (span, uid){
        var t = my.AllianceList;
        span.onclick = '';
        span.innerHTML = "Research details ...";
			...
			for (var i = 0; i < chatPosts.snapshotLength; i++) {
				...
				if(Options.HelpRequest){
				 if(postAuthor.snapshotItem(0)){
						...
						logit("tchat 5 : "+postAuthorName);
						logit("tchat 6 : "+DisplayName);
				...
				if(Options.DeleteRequest){
					var myregexp1 = /of 5 to help/i;
					var myregexp2 = /You have already helped with this project./i;
					var myregexp3 = /There has been an error. You cannot help with this project at this time./i;
					var myregexp4 = /5 people have already helped/i;
