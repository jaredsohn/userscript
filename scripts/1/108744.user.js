...
var HdSuite = function () {
     ...
     */
    this.version = { text:'3.5.2', rev:201207302200 };
     ...
     */
     this.tx = {
        mono:
             'display:block;font:14px/14px \'ヒラギノ角ゴ Pro W3\',\'Hiragino Kaku Gothic Pro\',\'メイリオ\',Meiryo,sans-serif;white-space:pre;padding:5px;'
                    ...
                    if ( !strong[i].innerHTML && strong[i].getAttribute('class') == vobj['video_id'] ) {
                        ...
                        for ( var j=0; j<vobj['formats'].length; j++ ) {
                            ...
                            if ( hdsuite.config.download_formats[ vobj['formats'][j]['itag'] ] ) {
                                ...
                                link.href      = vobj['formats'][j]['url']+'&title='+hdsuite.convertTitle(vobj,fmt);
                                ...
                                if ( strong[i].nextSibling.href && !strong[i].nextSibling.href.match(/#$/) && hdsuite.config.redirect ) {
                                    ...
                                    strong[i].nextSibling.href += "&hd=1";
                                }
                                break;
                            }
                        }
        ...
        for ( var i=0; i<s['formats'].length; i++ ) {
            if ( s['formats'][i]['itag'] == fmt ) {
                this.$('check_fmt_'+fmt).innerHTML
                    = '<a href="'+s['formats'][i]['url']+'&title='+hdsuite.convertTitle(s,format)+'" style="'+this.tx.dl+'border-bottom:5px solid #'+format.color+';" title="'+format.desc+'">'+format.icon+'('+format.type+')</a>';
                return true;
                break;
            }
        }
    ...
    this.checkVars = function() {
        ...
        // Remote check
        var url = 'http://'+l.host+'/watch?v='+match+'&ts='+new Date().getTime();
        var XHR = this.createXHR();
        XHR.open( 'GET', url, true );
        XHR.onreadystatechange = function() { 
            if (XHR.readyState==4) {
    ...
    this.showDownloadLinks = function() {
        if( !this.$('hdsuite_dl_div') ) {
            ...
            div.style.MmozBorderRadius    = '5px';
            div.style.WwebkitBorderRadius = '5px';
    ...
    this.checkVersion = function( force ) {
        ...
        if ( force || !last_update_check || (now - last_update_check) > (3600 * 24 * 1000) ) {
            ...
            script.src  = 'http://userscripts.org/scripts/source/92923.user.js';
