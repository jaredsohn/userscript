...
var thread_id = jq('article.msg div.msg_body h1 a').get(0).pathname.split('/').pop();
...
function letsGo() {
    ...
    jq("a.lor-report-msg").click(function() {
      ...
      if (url1.length == 0) {
          url1 = reportlink.parent().parent().parent().parent().find("div.title a.counter:first");
      }
...
function add2tracker(id){
       var links = new Array;
       // have we saved list of tracked threads?
       var getlinks = readCookie('links');
       if (!getlinks) { // we havn't
               // create new
               createCookie('links', id, 90);
       } else {
               // get saved list of tracked threads
               links = getlinks.split("_");
               var f = 1;
               for (var i=0; i<links.length; i++){
                       if (links[i] == id) f=0;
               }
               if (f) {
                       createCookie('links', getlinks + '_' + id, 90);
               }
       }
}


jq('div.title:eq(0)').append('<a href="#" id="trackit">[Add to tracker] </a>');
jq('#trackit').click(function(event) {
       add2tracker(thread_id);
...
function onAjaxSuccess(data) {
    ...
    // [newmsgs > msgs] means that here is new messages
    if (newmsgs > msgs) {
        newid.length = 0;
        if (msgs > 0) {
...
//  var subj = 'Re: ' + jq('article.msg h1').get(0).textContent.replace(/(^\s*)|(\s*)$/g, "");
    
    var repuser = jq("div.sign a:first",elem.parentNode.parentNode).text();
    //var repuser = jq("div.sign",elem.parentNode.parentNode).text().replace(/(^\s*)|(\s*)$/g, "").split(" ")[0];
    ...
    if (seltxt) {
        //seltxt = '[quote="Цитируем ' + repuser + '"]' + "\n\n"  + seltxt + "\n[/quote]\n\n";
        seltxt = repuser + '> ' + seltxt + "[br]\n";
    }
...
jq('div.messages').prepend('<div class="treebut"></div>');
jq('div.treebut').append('<input id="blayout" type="button" value="' + butText + '"/>').click(
  function() {
       var but = jq(this).find('#blayout');
       if (but.val() == 'Plain layout') {
               but.val('Tree layout');
               // do plain
               jq('article.msg').sort(function(a, b) {
               return jq(a).find("div.title a:eq(0)").text() - jq(b).find("div.title a:eq(0)").text();
               }).prependTo("div.comment");
               jq('div.comment article.msg, div.comment div.title').css('cssText', '');
       } else {
