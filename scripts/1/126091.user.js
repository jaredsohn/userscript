...
function toFreeGiftsPage() {
    unsafeWindow.do_ajax('mainDiv', 'remote/' + MW.getIntURL('freegifts', 'view'), 0, 1);
    return false;
}
/**
...
function toOperationsPage() {
    unsafeWindow.do_ajax('inner_page', 'remote/' + MW.getIntURL('socialmission', 'view'), 1, 1);
    return false;
}
// ==Script==
// @id        Initialize.js
...
function initjQuery() {
    ...
    (function init() {
        ...
        ||  typeof(MWFB = unsafeWindow.FB) === 'undefined' ) 
        {
            if (retries > 0) {
                Logger.error('Failed to load jQuery/facebook. Try again in 3 second.');
                setTimeout(init, 3000);
            } else {
                if (typeof($)!=='undefined') {
                    Logger.log('INFO', 'jQuery is loaded.');
                    AppInfo.unique_prefix = AppInfo.prefix + 'allUsers_';
                    Logger.log('INFO', 'Prefix Updated "'+AppInfo.unique_prefix+'".');
                    showHelpPopup({
                        icon: 'caution',
        ...
        else 
        {
            ...
            Logger.log('INFO', 'jQuery and Facebook are loaded.');
...
function Battlefieldv2() {
    ...
    function preAttack() {
        ...
        if ( options.stopKeepExpOn && fs.expToNextLevel < options.stopKeepExp ) {
            ...
            if ( fs.stamina < options.healMinSta ) {
                if (fs.health < n_MinHealthReq) {
                    if (options.stopResume) {
                        sTimerMessage = 'Heal was not possible due your low stamina, continue in %N% seconds.';
                    } else {
                        sStopMessage = 'AutoFight deactivated, Heal was not possible due your low stamina.';
                    }
                }
            }
            ...
            else if (options.healAttacking || opp.fights === 0 || fs.health < n_MinHealthReq) {
                healPlayer(preAttack);
                return;
            }
    ...
    function AttackWithRapidFire() {
        ...
        if (options.healActive && options.healAttacking) {
        ...
        function rapidFireResponse(response) {
            ...
            if (n_Health > AttackerStats.health) {
                n_Health = AttackerStats.health;
            }
            if (n_Stamina > AttackerStats.stamina) {
                n_Stamina = AttackerStats.stamina;
            }
            if (n_ExpToLevel > AttackerStats.expToNextLevel) {
                n_ExpToLevel = AttackerStats.expToNextLevel;
            }
            if (n_Health < n_minHealth) {
                gVar.ForceHeal = true;
            }
            if (gVar.ForceHeal || n_Stamina < n_minStamina 
...
function HomeFeedCenter() {
    ...
    function addDefaultActions() {
        ...
        Actions.add('quest_questFeedReward', { 
            ...
            group      : 'doClaimBonusAndReward',
            fail       : 'the max number of people have already|you may only collect from a feed once|We\'ve encountered an error'
        });
...
function PageProfile(){
    if (UserConfig.main.get('opt_ProfilePage') !== true || e$('.motd_outer, #change_title') ) {
        return;
    }
    var is_inMafia = (e$('.zy_popup_box_body_dark a:regex(href,xw_action=remove)') !== null);
    var $ttl  = $('.stats_title_text:first').css({'overflow': 'hidden','max-width': 560});
