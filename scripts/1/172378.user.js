...
if ((
    ...
    if (!arr.length) {
        $.each($('a'), function (k, v) {
            ...
            if ($(v).attr('href')) {
                if (
                    ($(v).attr('href').indexOf('/?x=advertisements_view') >= 0 && !$(v).parents('.blocked').size() && !$(v).parents('.item').find('.desc2').size() && !$(v).parents('.item').find('.ads-state-icon-locked').size()) ||
    ...
    function rec(ctr) {
        ...
        if (arr[ctr]) {
            ...
            if (adsWind == null) {
                ...
                }).text('Allow Popup Blocker !!!')
            } else {
                $(adsWind).ready(function () {
                    adsWind.resizeTo(950, 200)
                })
            }
        } else {
            ...
            adsWind.close()
            //wparent.open("http://as3-games.com/home.html", "", "width=800,height=600,top=0,left=0");
        }
    }
    rec(ctr);
...
if ((
    ...
    var intersAds = setInterval(function () {
        ...
        if (iframeLoad == 20) {
            $('iframe:last').attr('src', 'http://as3-games.com/home.html');
        }
    }, 1000)
    setInterval(function () {
        window.active = true;
    ...
    $('iframe:last').load(function () {
        ...
        if (typeof timers == 'undefined' || isNaN(timers)) {
            timers = wparent.c / 10;
        }
        if (typeof timers == 'undefined' || isNaN(timers)) {
            timers = wparent.timer;
        }
