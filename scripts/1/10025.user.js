// ==UserScript==
// @name           socialbookmarkers
// @namespace      http://d.hatena.ne.jp/noxy
// @description    print count of bookmarkers of delicious, hatena bookmark, livedoor clip and buzzurl.
// @include        http://*
// ==/UserScript==

new function(){

if (self.location.href!=top.location.href) return;//not display in iframes
...
var api_url = { 
		'livedoor clip' : 'http://image.clip.livedoor.com/counter/' ,
		'Hatena Bookmark' : 'http://b.hatena.ne.jp/entry/image/' ,
		'Buzzurl' : 'http://api.buzzurl.jp/api/counter/'
	};

var createImgTag = function(sbm, target_url){
	var div = document.createElement('div');
	var img = document.createElement('img');
	
	if(sbm == 'delicious'){ 
		img.src = 'http://del.icio.us/feeds/img/savedcount/' + md5_hex(target_url) + '?aggregate';
	}else{
		img.src = api_url[sbm] + target_url;
	}
	div.innerHTML = sbm + ':';
	div.appendChild(img);
	return div;
...
with(panel.style){bottom=0; left=0; padding='2px'; opacity=0.7; fontSize='x-small'; color='#000000'; backgroundColor = '#EEEEEE'; border = '1px solid #C0C0C0';  zIndex = 100;position='fixed'; }

var url = w.location.href;

panel.appendChild(createImgTag('delicious', url));
panel.appendChild(createImgTag('livedoor clip', url));
panel.appendChild(createImgTag('Hatena Bookmark', url));
panel.appendChild(createImgTag('Buzzurl', url));
panel.addEventListener('click',	function(event){this.style.display = 'none';}, true);
