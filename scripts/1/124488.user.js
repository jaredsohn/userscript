// ==UserScript==
// @name           EvrenSerdar Eski-Versiyon
// @namespace      EvrenSerdar Eski-Versiyon
// @version        EvrenSerdar Eski-Versiyon
...
var Version = 'EvrenSerdar Eski-Versiyon';
...
var attack_unit_Ion           = "modal_attack_unit_ipt_20";  // "AttackIon"
...
function ptStartup (){
  ...
  var styles = '.ptTabs {color:black; font-size:12px}\
    ...
    button::-moz-focus-inner, input[type="submit"]::-moz-focus-inner { border: none; }\
    a.loc1 { color:#0000FF; font-size:15px}\
    a.loc2 { color:#0000FF; font-size:15px}\
	td.search1 { font-size:15px }\
	a.ptButton21 {\
       color: black;\
       display: block;\
       float: left;\
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Aranılan: </td><TD>\
      <SELECT id="srcType">\
        <OPTION value=0>Trst.Kampı</option>\
        <OPTION value=1>Bozkır</option>\
		<OPTION value=2>Şehir</option>\
      </select></td></tr>\
      </table>\
    ...
    m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Aramaya BAŞLA"/></td></tr>';
    m += '</table>';
...
clickedPlayerDetail : function (span, uid){
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Sıralama:</b> Not found! (misted?)';
      return;
    }
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Sıralama: </b></td><TD colspan=2> Güç: '+ p.might  +' &nbsp; İttifak: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      m += '<TR><TD align=right><B>Şehir #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName +' '+coordLink (c.xCoord, c.yCoord)+'</td><TD width=75%> &nbsp; Seviye: '
        + c.tileLevel +' &nbsp; &nbsp; Durumu: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; Oluşumu: ' + c.dateCreated.substr(0,10) +'</td></tr>';
    }  
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>İttifak: '+ a +' &nbsp; Şehirleri: '
  ...
  gotPlayerStatus : function (rslt, span,uid){
    ...
    if (p[uid] == true) {
      m = '<span style="color:green"><b>Çevrimiçi!!</b></span>';
       ...
       m = '<span style="color:red"><b>Çevrimiçi Değil</b></span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:black">Son Giriş Tarihi: '+lastLogin+'</span>';
    } else {
       m = '<span style="color:red">Son Giriş Tarihi Bulunamadı: '+lastLogin+'</span>';
    }  
    span.innerHTML = m + '';
  },
//End edit city search
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>799)
      errMsg = "X Kutusuna 799 ile 0 arasında bir rakam girmelisiniz..!<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>799)
      errMsg += "Y Kutusuna 799 ile 0 arasında bir rakam girmelisiniz..!<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>799)
      errMsg += "Maksimum Uzaklığı 799 ile 1 arasında girmelisiniz..!<BR>";
    if (errMsg != ''){
    ...
    document.getElementById ('srcStart').value = 'Aramayı DURDUR';
    ...
    if (t.opt.searchType == 0)
      typeName = 'Terörist Kampı';
    else if (t.opt.searchType == 1)
      typeName = 'Bozkır';
	else 
	  typeName = 'Şehir';

    m = '<CENTER><B>Aranılan '+ typeName +'<BR>\
    ...
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Bozkır İçeriği:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Çayır/Göl', 3:'Petrol Kuyusu', 4:'Tepe', 5:'Dağ', 6:'Ova', 0:'Tümü', 20:'Özel'}, Options.wildType );
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD class=xtab align=right>Sırala:</td><TD class=xtab><SELECT id=filSortBy>\
	 ...
	 m += '<TR><TD class=xtab align=right>Şehir İçeriği:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'Tümü', 2:'İttifak', 3:'Dost', 4:'Düşman', 5:'Tarafsız', 6:'İttifaksız', 7:'Sisli'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Sırala:</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Güç</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Mesafe</option>\
           </select></td></tr>\
		   <TR><TD class=xtab align=right>Min.Güçlüden Başla:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>Sadece Koordinatlar:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
    ...
    document.getElementById ('statStatus').innerHTML = 'Aranılan .. '+ xxx +','+ yyy;
if (DEBUG_TRACE) actionLog (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['T.Kampı', 'Çayır', 'Göl', 'Petrol Kuyusu', 'Tepe', 'Dağ', 'Ova', 'Şehit' ];
    var t = Tabs.Search;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 2) {
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD width="10%"><b>Lokasyon</b></td><TD  width="5%"><b>Mesafe</b></td><TD width="15%"><b>Şehir</b></td><TD width="15%"><b>Oyuncu</b></td><TD width="12%"><b>Güç</b></td><td width="15%"><b>İttifak</b></td><TD width="28%" style="font-size:9px;"><b>Fazla Bilgi</b></td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			if(Options.unownedOnly || t.opt.searchType == 0){
			  m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD width="20%"><b>Lokasyon</b></td><TD width="10%"><b>Mesafe</b></td><TD width="10%"><b>Sevy.</b></td><TD width=60%><b>İçerik</b></td></tr>';
			} else {
			  m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD align="center" width="10%">Lokasyon</td><TD width="5%">Mesafe</td><TD align="center" width="5%">Sevy.</td><TD align="center" width="10%">İçerik</td><TD align="center" width="15%">Sahip</td><TD width="12%">Güç</td><TD align="center" width="15%">İttifak</td><TD width=28% align="center" style="font-size:9px;">Fazla Bilgi</td></tr>';
			}
		}
	  }
      ...
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>DİKKAT: Bu menüde En fazla 500\'den '+ dat.length +' \'a kadar sonuçlar görüntülenebilinir..!</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A class=loc2>Son Giriş Tarihi</a></div>';
			  ...
			  if(!dat[i][5] || t.opt.searchType == 0){
			    ...
			    m += '<TD align="right" valign="top" class=search1>'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD valign="top" class=search1 >'+ dat[i][4] +'</td><TD valign="top" class=search1 >'+ tileNames[dat[i][3]]+'</td><TD valign="top" class=search1 >'+ dat[i][9] +'</td><TD valign="top" class=search1 >'+dat[i][10]+'</td><TD valign="top" class=search1 >'+dat[i][11]+'</td><td class=search1 >';
				  if (dat[i][6]) {
					...
					<DIV style="" onclick="PCplo2(this, '+ dat[i][8] +')"><A class=loc2>Son Giriş Tarihi</a></div>';
				  }
				m += '</td></tr>';
			  }
			}
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'Aramaya BAŞLA';
...
if (DEBUG_TRACE) actionLog (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Aranılan: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    if (t.curX > t.lastX){
      ...
      t.curY += 15;
      if (t.curY > t.lastY){
        t.stopSearch ('İşlem Başarıyla TAMAMLANDI.!!');
        return;
      }
    }
    ...
    document.getElementById ('statStatus').innerHTML = 'Aranıyor .. '+ x +','+ y;
...
Tabs.transport = {
  ...
  check:false,
    ...
    init: function(div){
      ...
      var m = '<DIV id=pbTowrtDivF class=pbStat>Otomatik Nakliye Fonksiyonu Menüsü</div><TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center">';
      if (t.traderState.running == false) {
          m += '<TD><INPUT id=pbTraderState type=submit value="Taşıma = KAPALI"></td>';
      } else {
          m += '<TD><INPUT id=pbTraderState type=submit value="Taşıma = AÇIK"></td>';
      }
      m += '<TD><INPUT id=pbShowRoutes type=submit value="Kayıtları GÖSTER"></td>';
      m += '</tr></table></div>';
      m += '<DIV id=pbTraderDivDRoute class=pbStat>SEÇENEKLER</div>';
      m += '<TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center"><TR align="left">';
	    m += '<TD colspan=4>Nakliye Kontrolü Zaman Aralığı: <INPUT id=pbtransportinterval type=text size=2 value="'+Options.transportinterval+'"\>  Dakikada Bir Bakılır..</td></tr></table>';
      m += '<TD colspan=4><INPUT id=pbminwagons type=text size=2 value="'+Options.minwagons+'"\>En az Yandaki Kutuya Girilecek Miktarda Nakliyeye İzin Verir ..</td></tr></table>';
      m += '<DIV style="margin-top:10px;margin-bottom:5px;">En az Yandaki Kutuya Girilecek Miktarda Nakliyeye İzin Verir ...</div></table>';
    
      
      m += '<DIV id=pbTraderDivDRoute class=pbStat> NAKLİYE </div>';
      ...
      m+='<TD width=75px>Taşınabilinir Miktar:&nbsp;</td><TD id=CarryAmount align=left width=75px></td>';
      m += '<TR><TD >Taşıyacak Birim: </td><TD><INPUT id=TroopsToSend type=text size=11 maxlength=11 value="0">&nbsp;&nbsp;<INPUT id=MaxTroops type=submit value="Maks."></td>';
      ...
      m += '<TD width=180px  align=left>Taşınmayacak: <INPUT id=pbtargetamountFood type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountFood type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountWood type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountWood type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountStone type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountStone type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountOre type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountOre type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountGraph type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountGraph type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountTita type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountTita type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Taşınmayacak: <INPUT id=pbtargetamountGold type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Taşınan: <INPUT id=pbtradeamountGold type=text size=11 maxlength=11 value="0"\></td>';
      m += '<TD width=50px><INPUT id=MaxGold type=submit value="Maks."></td></tr>';

      m += '</table>';

      m += '<DIV style="text-align:center; margin-top:15px"><INPUT id=pbSaveRoute type=submit value="KAYIT EKLE"><INPUT id=pbManualSend type=submit value="ELLE NAKLİYE ET"></div>';
    ...
    updateTroops : function (city){
    	...
    	document.getElementById('Calc').innerHTML = 'Malzeme Adeti: ' +  addCommas(t.Food + t.Wood + t.Stone + t.Ore + t.Gold) + ' / ' + addCommas(t.MaxLoad) + '&nbsp;&nbsp;(İhtiyaç Duyulan Birim Adeti: <FONT color='+fontcolor+'>' + addCommas(t.TroopsNeeded) + '</font> )' ;
 ...
 e_tradeRoutes: function(){
	...
	addTradeRoute: function () {
		...
		{
			new CdialogConfirm ('<SPAN class=boldRed>DİKKAT 0-0 Koordinatına Nakliye Yapmaya Çalışıyorsunuz..!!!</span>', t.checkcoords, unsafeWindow.modal_attack_check, mainPop.getMainDiv); 
			return;
		}
	...
	paintTradeRoutes: function(){
			...
			for (var i=0;i<(r.length);i++) {
			  citynameTo = null;
				...
				if (r[i].route_state) var status = '<FONT color=green>AKTİF</font>';
        ...
        m +='<TR><TD width=20px>'+(i+1)+'</td><TD width=175px>Buradan:&nbsp;&nbsp;'+ cityname +'</TD><TD width=175px>Buraya:&nbsp;&nbsp;'+ TO +'</td><TD width=175px>'+status+'</td>';
        m +='<TD width=60px><A class="ptButton21" onclick="traceEdit('+queueId+')">Düzenle</a></td><TD width=60px><A class="ptButton21" onclick="traceDelete('+queueId+')">SİL</a></td></tr>';
        m += '<TR><TD></td><TD>Birim:&nbsp;&nbsp'+unsafeWindow.arStrings.unitName[unit]+'</td></tr>';
        if (r[i].ship_Food) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + FOOD_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Food) +'</td><TD>Taşınacak: '+ addCommas(r[i].trade_Food)+'</td>';
		if (r[i].ship_Wood) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + OIL_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Wood) +'</td><TD>Taşınacak: '+ addCommas(r[i].trade_Wood)+'</td>';
		if (r[i].ship_Stone) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + STONE_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Stone) +'</td><TD>Trade: '+ addCommas(r[i].trade_Stone)+'</td>';
		if (r[i].ship_Ore) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + STEEL_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Ore) +'</td><TD>Taşınacak: '+ addCommas(r[i].trade_Ore)+'</td>';
		if (r[i].ship_Graph) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + GRAPH_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Graph) +'</td><TD>Trade: '+ addCommas(r[i].trade_Graph)+'</td>';
		if (r[i].ship_Tita) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + TITA_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Tita) +'</td><TD>Taşınacak: '+ addCommas(r[i].trade_Tita)+'</td>';
		if (r[i].ship_Gold) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + GOLD_IMAGE +'></td><TD>Taşınamaz: '+ addCommas(r[i].target_Gold) +'</td><TD>Taşınacak: '+ addCommas(r[i].trade_Gold)+'</td>';
       }
	     m +='</table>';
	 ...
	 editQueueElement: function(queueId){
       ...
       var n = '<TABLE id=editRoutes class=pbTab>';
	     ...
	     n += '<TD width=150px>Birim İçeriği:<SELECT id="pbbTransportTroop">';
for (y in unsafeWindow.arStrings.unitName) if(y == "u2"){continue} else if(y == "u14") {continue} else { n+='<option value="'+findoptvalue(y)+'">'+unsafeWindow.arStrings.unitName[y]+'</option>' };
       n+='</select></td></table><BR><TABLE  id=editRoutes class=pbTab>';
         ...
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountfood type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamountfood type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + OIL_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipwood type=checkbox></td>';
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountwood type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamountwood type=text size=11 maxlength=11 value="0"\></td></tr>';
         ...
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountstone type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamountstone type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + STEEL_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipore type=checkbox></td>';
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountore type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>v: <INPUT id=pbbtradeamountore type=text size=11 maxlength=11 value="0"\></td></tr>';
         ...
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountgraph type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamountgraph type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + TITA_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshiptita type=checkbox></td>';
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamounttita type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamounttita type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + GOLD_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipgold type=checkbox></td>';
         n += '<TD width=125px>Taşınamaz: <INPUT id=pbbtargetamountgold type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Taşınan: <INPUT id=pbbtradeamountgold type=text size=11 maxlength=11 value="0"\></td></tr>'
       n+='</table><BR><TABLE id=editRoutes class=pbTab><TR><TD><a class="ptButton21" id="Cancel"><span>İPTAL</span></a></td>';
  ...
  doTrades: function(count){
    var t = Tabs.transport;
	  ...
	  var maxloadperwagon = (featherweight * ((Load/100)*10)) + Load;
		...
		if ( wagons_needed < Options.minwagons ) { actionLog('Nakliye Yapılamadı,Atlandı..!'); return; }
	  ...
	  case 'unt19': params.u19 = wagons_needed;break;
         ...
         new AjaxRequest(unsafeWindow.g_ajaxpath + "march.php" + unsafeWindow.g_ajaxsuffix, {
                  ...
                  if (rslt.ok) {
                  actionLog('Trade   From: ' + cityname + "   Buraya: " + xcoord + ',' + ycoord + "    \'Adetindeki Birimlerle: " + wagons_needed);
...
/****************************  Alarm Sekmesi Fonksiyonu  ******************************/
	var uu1 = 'İkmal Kamyonu';
	var uu4 = 'Orbital İyon Topu';
	var uu5 = 'Piyade';
	var uu6 = 'Keskin Nişancı';
	var uu7 = 'Mobil SAM';
	var uu8 = 'Tank';
	var uu9 = 'İkmal Helikopteri';
	var uu10 = 'Avcı Uçağı';
	var uu11 = 'Savaş Helikopteri';
	var uu12 = 'Bombardıman Uçağı';
	var uu13 = 'Görünemez Bombardıman Uçağı';
	var uu15 = 'Taktikal Nuke';
	var uu16 = 'Cehennem Ateşi Tankı';
	var uu17 = 'Tanksavar Piyade';
	var uu18 = 'Özel Kuvvetler';
	var uu19 = 'Kargo Nakliyesi';
        var uu20 = 'İnsansız Uçak';
Tabs.tower = {
  tabOrder: 22,
  tabLabel: 'ALARM',
  myDiv: null,
  ...
  Providers : {
        ...
        1: { 'country': "TÜRKİYE", 'provider': "Vodafone" },
		2: { 'country': "TÜRKİYE", 'provider': "Türkcell" },
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>ALARM SEÇENEKLERİ MENÜSÜ</div><TABLE class=pbTab><TR align=center>';
    ...
    <td align=left>SMS\'in Atılacağı Numara: <INPUT id=pbnum1 type=text size=4 maxlength=4 value="'+ Options.celltext.num1 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum2 type=text size=3 maxlength=3 value="'+ Options.celltext.num2 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum3 type=text size=4 maxlength=4 value="'+ Options.celltext.num3 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\> <span style="color:#800; font-weight:bold">(Standard SMS fiyatı ile Ücretlendirilir)</span></td></tr><tr><td></td>\
 ...
 if(t.Providers[i].country == t.Providers[Options.celltext.provider].country)
    ...
    m += '</select></td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Otomatik Olarak Saldırı Raporunu İttifak Sohbetine Gönderir..!</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Mesaj İçeriği: &nbsp; </td><TD><INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD align=right>Haberci Alarmı: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Bozkır Saldırısı Alarmı: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
        ...
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Aşağıya Girilen Linkdeki Melodi ALARM Sesi Olarak Ayarlanır.!!</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>Lütfen FlashPlayer yada SWF Player Yükleyiniz ..</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Ses Dosyası: &nbsp; </td><TD><INPUT id=pbsoundFile type=text size=55 maxlength=160 value="'+ Options.alertSound.soundUrl +'" \>\
             &nbsp; </td><TD><INPUT id=pbSoundLoad type=submit value=YÜKLE><INPUT id=pbSoundDefault type=submit value=Varsayılan></td></tr>\
            <TR><TD align=right>Ses Ayarı: &nbsp; </td><TD><TABLE cellpadding=0 cellspacing=0 class=pbTab><TR valign=middle><TD><SPAN id=pbVolSlider></span></td><TD width=15></td><TD align=right id=pbVolOut>0</td></td></table></td><TD align=center><SPAN id=pbLoadStat>xx</span></td></tr>\
            <TR><TD align=right><INPUT id=pbSoundRepeat type=checkbox '+ (Options.alertSound.repeat?'CHECKED ':'') +'/></td><TD> Tekrar zamanı: <INPUT id=pbSoundEvery type=text size=2 maxlength=5 value="'+ Options.alertSound.repeatDelay +'"> Dakika</td></tr>\
            ...
            <TR><TD></td><TD><INPUT type=submit value="DİNLE" id=pbPlayNow></td></tr></table></div></td></tr>\
        </table><BR>';
  	t.myDiv.innerHTML = m;
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
  ...
  postToCell : function (m){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        data.embassy = 'Benim Elçiliğimde Şu Anda: '+ availSlots +'of'+ emb.maxLevel;
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
            data.stat = 'Askerlerim Sığınaktadır ..';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            data.stat = 'Askerlerim Şehrimi SAVUNUYOR.!!';
        }
      }
    }
  ...
  postToChat : function (m){
    ...
    var city = Cities.byID[m.toCityId];
    if ( city.tileId == m.toTileId ) {
      target = 'Saldırı Alan Şehrimin Koordinatı: ('+ city.x +','+ city.y + ')';
	  }
    else {
      if (!Options.alertConfig.wilds)
        return;
      target = 'Benim Bozkır\'ıma';
	  alert('wild')
      for (k in Seed.wilderness['city'+m.toCityId]){
        if (Seed.wilderness['city'+m.toCityId][k].tileId == m.toTileId){
    ...
    showTowerIncoming: function(cityId){
        ...
        t.popTowerIncoming.getTopDiv().innerHTML = '<TD width="300px"><B>Saldırı Detayı Gösterilen Şehriniz: ' + cityName + '</b></td></td>';
    ...
    addCityData2Pop: function(cityId){
        var t = Tabs.tower;
        var rownum = 0;
        var names = ['İkml K.', 'Orb.İ.Tp.', 'Pyd.', 'Nşnc', 'Mbl SAM', 'Tank', 'İ.Hlkptr', 'Avc Uçğ', 'S.Hlkptr', 'Bmb.Uçğ', 'Grn.Bm.Uçğ', 'Tak.Nuke', 'Ch.Atş Tnk', 'Tnk.Pyd.', 'Özl Kvv.', 'Krg Nkly.',];
        enc = {};
        numSlots = 0;
        ...
        s3 += '<TD class="city"></td><TR><TD colspan=17><BR></td></tr><TR align=right><TD class="tot" align=left><B>Gelen Saldırı:</b></td>';
            ...
            for (k in t.towerMarches) {
                if (typeof t.towerMarches[k].atkType != 'undefined') {
                    if (t.towerMarches[k].cityId == cityId) {
                        ...
                        s3 += '<TD width=5%  ><B>İsim</b></td>';
						s3 += '<TD width=5%><B>Kaynak: </b></td>'
                        s3 += '<TD width=5%><B>Güç: </b></td>'
                        s3 += '<TD width=5% colspan=2><B>İttifak: </b></td>'
						...
						s3 += '<TD width=5%></td>'
                        s3 += '<TD><B>Hatırlatma: </b></td>'
                        s3 += '<TD><B>Ulaşmasına: </b></td></tr>';
					    s3 += '<TD width=5%></td>'
		...
		s2 += '<TR><TD colspan=16><BR></td></tr><TR align=right><TD class="attack" align=left><B>Saldıran:</b></td>';
        for (a = 1; a < 17; a++)
...
//DebugTimer.start(); 
    try {
     ...
     // <TR align=left><TD><SPAN class=ptStatLight>Joined on:</span> '+ dt.toLocaleDateString() +'</td>
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class="ptTab ptStat" width=97% align=center>\
        ...
        <TD><SPAN class=ptStatLight>İttifak:</span> ' + getMyAlliance()[1] +'</td>\
      ...
      str += _row ('Altın', rows[0]);
      str += _row ('Gıda', rows[1]);
	  ...
	  str += _row ('İkmal Kmyn.', rows[1]);
      str += _row ('Piyade', rows[5]);
      str += _row ('K.Nişancı', rows[6]);
      str += _row ('TankSavar P.', rows[4]);
      str += _row ('Özel Kuv.', rows[18]);
      str += _row ('Mobil SAM', rows[7]);
      str += _row ('Tank', rows[8]);
      str += _row ('İnsansız Uçak', rows[17]);
      str += _row ('İkmal Helkpt.', rows[9]);
      str += _row ('Savaş Helkpt.', rows[11]);
      str += _row ('Avcı Uçağı', rows[10]);
      str += _row ('Bombardıman Uçk.', rows[12]);
      str += _row ('Kargo Uçk.', rows[19]);
      str += _row ('Cehennem Tankı', rows[16]);
      str += _row ('Görünmez Bombardıman Uç.', rows[13]);
      str += _row ('Nükleer Füze', rows[15]);
	  str += _row ('İyon Topu', rows[20]);
      str += '<TR><TD colspan=12><BR></td></tr>';
      ...
      for (r=1; r<5; r++){
	    ...
	    str += _row ('Mayın:', rows[2],true);
        str += _row ('StingerFüze:', rows[1],true);
	    str += _row ('Boşluk Var:', rows[8],true,true);
        str += _row ('TOP:', rows[3],true);
        str += _row ('Uçak Savar:', rows[4],true);
        str += _row ('Raylı TOP:', rows[5],true);
        str += _row ('Raylı LazerTOP:', rows[6],true);
        str += _row ('Yapılabilinir:', rows[9],true,true);
        str += '<TR><TD colspan=12><BR></td></tr>';
    }
           row = [];
      row2 = [];
      ...
      for(i=0; i<Cities.numCities; i++) {
        ...
        row2[i] = rp[1];
			...
			str += _row ('Bakım:', row3, true);
      str += _row ('Üretim:', row2, true);      
      str += _row ('Gıda Gideri:', row, true);
      ...
      str += _row ('Savunma Eğt.:', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=ptoverOriginal'+ (Options.includeCity?' CHECKED':'') +'>Şehirdeki Askerleri Gösterir!!</td></tr>';
    ...
    //  str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptoverIncTrainingTotal'+ (Options.includeTrainingTotal?' CHECKED':'') +' DISABLED>Show troops training totals</td></tr>';
     str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Belirlenen Boyuta Ayarlar Yazıları: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12,15:15}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr>';
      str += "</table></div>";
...
//        <TD align=right><SPAN class=ptStatLight>Domain:</span> ' + unsafeWindow.domainName +'</td></tr></table></div>';
      ...
      str += "<TD width=100><B>Şehirleriniz:"
      for(i=0; i<Cities.numCities; i++) {
        str += "<TD width=100><B>"+ Cities.cities[i].name.substring(0,11) +'</b><BR>' +"</td>";
      }
      str += "</tr><TD width=81>Altın:<BR>Vergi:<BR>Gelir:<BR>Bakım:<BR><BR><B>Kaynak(1%):<BR>Ordu(0.5%):<BR>Araştırma(0.5%):<BR>İnşaat(0.5%):</B>";
      Gold=[];
      Tax=[];
      Revenue=[];
      for(i=0; i<Cities.numCities; i++) {
...
/****************************  Otomatik İnşaat Sekmesi Fonksiyonu  ******************************
 TODO:
...
Tabs.build = {
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbBuildDivF class=ptStat>İNŞAAT SEÇENEKLERİ MENÜSÜ</div><TABLE id=pbbuildfunctions width=100% height=0% class=ptTab><TR>';
		...
		m += '<TD><INPUT id=pbBuildMode type=submit value="OTO-İnş.Sırası=KAPALI"></td>';
		m += '<TD>İnşaat İçeriği: <SELECT id="pbBuildType">\
				...
				<OPTION value=destruct>Yıkım YAP</option>\
				</select></td>';
		//m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>Yardım İstensin mi??</td>';
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=ptStat>İnşaat Seçenekleri</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
        ...
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD>Sıra:</td><TD id=pbbuildcount_' + Cities.cities[i].id + '>' + t["bQ_" + Cities.cities[i].id].length + '</td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
    ...
    doOne : function (bQi){
		...
		if (Seed.buildings['city' + currentcityid]["pos" + citpos] != undefined && Seed.buildings['city' + currentcityid]["pos" + citpos][0] != undefined) {	
			...
			if (curlvl > 8 && mode == 'build') {
				...
				actionLog("Seviye 10 inşaatlar Manuel Olarak Yapılabilinir ancak.!!");
				return;
			};
			if (isNaN(curlvl)) {
				t.cancelQueueElement(0, currentcityid, time, false);
				actionLog("İnşaat İçin Gereksinimler Karşılanamadı.!!");
				return;
			}
			if (l_bdgid != bdgid) {
				t.cancelQueueElement(0, currentcityid, time, false);
			...
			if (l_curlvl < curlvl) {
					t.cancelQueueElement(0, currentcityid, time, false);
					actionLog("Sıradan Kaldırıldı.!Zaten Aynı Seviyede Olduğundan İnşa Edilemedi.!");
					return;
			}
			...
			if (mode == 'destruct') {
				...
				new MyAjaxRequest(unsafeWindow.g_ajaxpath + "destruct.php" + unsafeWindow.g_ajaxsuffix, {
					...
					onFailure: function(){
						document.getElementById('pbbuildError').innerHTML = "Bağlantı Hatası Meydana Geldi.! Yıkım İŞlemi Yapılamadı..!";
					}
				})
			}
			if (mode == 'build') {
				...
				if (invalid == false) {							
					...
					if (params.lv > 9){ //make sure that no level 10+ is built
						t.cancelQueueElement(0, currentcityid, time, false);
						actionLog("İnşaat Sıradan Kaldırıldı.. Seviye 10 ve Üzeri İnşaatları LÜTFEN MANUEL Yapınız.!!");
						return;
					}
					if (params.lv > 1) {
						params.bid = bid;
					...
					new MyAjaxRequest(unsafeWindow.g_ajaxpath + "construct.php" + unsafeWindow.g_ajaxsuffix, {
						...
						onSuccess: function(rslt){
							if (rslt.ok) {
								actionLog("İnşaa Edilen: " + unsafeWindow.buildingcost['bdg' + bdgid][0] + " Seviyesi: " + params.lv + " Şehiriniz: " + cityName, true);								
								...
								if (rslt.error_code == 103) { // building has already the target level => just  delete
									t.cancelQueueElement(0, currentcityid, time, false);
									actionLog("İnşaat Sıradan Kaldırıldı.! Zaten Şu anda Devam Ediyor İnşaatı..!");
								} else {
									t.requeueQueueElement(bQi);
						...
						onFailure: function(){
							document.getElementById('pbbuildError').innerHTML = "Bağlantı hatası Oluştu.! Lüğtfen Daha Sonra Yeniden Deneyiniz.!";
						}
					});
				} else {
			...
			t.cancelQueueElement(0, currentcityid, time, false);
			actionLog("İnşaat Sıradan Kaldırıldı.!Öğe Bulunmuyor.!!");
		}
	},
    ...
    bot_buildslot: function(c, a){
		...
		if (DEBUG_TRACE) actionLog("Pos: " + buildingPos + " İçerik: " + buildingType + " Level: " + buildingLevel + " Id: " + buildingId);
    ...
    _addTab: function(queueId, cityId, buildingType, buildingTime, buildingLevel, buildingAttempts, buildingMode){
        ...
        row.insertCell(6).innerHTML = '<input class="button30 ptButton20" type=button id="queuecancel_' + queueId + '" value=İPTAL></input>';
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>Kayıtlı İnşaatlarınız: ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="Zamana Göre Sırala"></td>';
    ...
    toggleStateMode: function(obj){
		var t = Tabs.build;
        if (obj.value == 'OTO-İnş.Sırası=KAPALI') {
			...
			obj.value = "OTO-İnş.Sırası=KAPALI";
        }
    },
...
m = '<STYLE>.xtabR {padding-right: 5px; border:none; background:none; white-space:nowrap; text-align:right;}\
        ...
        <DIV style="overflow-y:auto; overflow-x:hidden"><DIV class=ptStat>GENEL BİLGİLENDİRME </div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR style="text-align:center"><TD class=xtab></td><TD class=xtabHL colspan=9><B>GEREKSİNİMLERİ </b></td><TD class=xtabHL colspan=4><B>ÖZELLİKLERİ</b></td><TD class=xtabHL colspan=4><B>Bakım</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Gıda</td><TD class=xtabH>Petrol</td><TD class=xtabH>Taş</td><TD class=xtabH>Çelik</td>\
        <TD class=xtabH>Titanyum</td><TD class=xtabH>Grafen</td><TD class=xtabH>Elmas</td><TD class=xtabH>Uranyum</td><TD class=xtabH>Nüfus</td><TD class=xtabHL>Can</td><TD class=xtabH>Saldırı</td><TD class=xtabH>Hız</td>\
        <TD class=xtabH>Yük</td><TD class=xtabHL>Gıda</td></tr>\
...
//actionLog ("ajax/allianceGetMembersInfo.php:\n"+ inspect (rslt, 5, 1));      
  ...
  show : function (){
    ...
    if (t.state == null){
      if (getMyAlliance()[0] == 0) {
        t.cont.innerHTML = '<BR><BR><CENTER>Bu Özelliği Kullanabilmeniz İçin Bir İttifak Üyesi Olmalısınız..!</center>';
        t.state = 1;
        return;
      }
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          ...
          <TR><TD class=xtab>Yada,: </td><TD class=xtab> İttifak Adı: &nbsp;</td>\
            <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="İttifak ARA" /></td>\
        ...
        <option value="1,90" > Cehennem Tankı</option>\
        <option value="1,80" > İyon Topu </option>\
        <option value="1,380" > Nükleer Füze </option>\
         <option value="1,680" > İyon Topu</option>\
        </select></div>\
        </td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'En Az 3 Karakter Yazarak Arama Yapmalısınız..!';
      return;
    }
  ...
  eventGotPlayerOnlineList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Gösterilen Oyuncu: <B>"'+ t.pName +'"</b></div>\
	  ...
	  <TD width=25%>İsim</td><TD align=right width=15%>Güç</td><TD width=15%> &nbsp; Çevrimiçi</td><TD width=15%>Facebook Prfl.&nbsp;</td><TD width=75%>Bakış</td></tr>';
    var row=0;
    var cl='';
    for (k in t.playerList){
      ...
      if (++row % 2)
       ...
       cl = '';
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A style="color:#1D0AF2">Detaylar</a> &nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A style="color:#1D0AF2">Son Giriş Tarihi</a></span></td></tr>';
    }
    m += '</table></div>';
...
clickedPlayerLeaderboard : function (span, uid){
 ...
 gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD>İttifak: '+ a +' &nbsp; Şehirleri: '
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'En Az 3 Karakter Girişi Yaparak Arama Yapmalısınız..!';
      return;
    }
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("Herhangi Bir İttifak Üyesi Değilsiniz..")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
  ...
  fetchPlayerLastLogin : function (uid, notify){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
            ...
            <DIV class=ptstat ><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab>İttifak: '+ allName +'</td>\
            ...
            <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Sıralama</a></div></td>\
            <TD id=clickCol9 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Çevrimiçi</a></div></td>\
            <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Şehir Adı</a></div></td>\
    ...
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Mesafesi Hesaplanacak Koordinat: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; Yada,Şehir Seçiniz: <span id=dmcoords></span></td></tr></table>';
...
Tabs2.ActionLog = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<DIV class=pbStat>TAKİP - Versiyon: '+ Version+'</div><DIV style="height:535px; max-height:535px; overflow-y:auto">\
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>İşaretlendiğinde Menüyü Hareket Ettirebilirsiniz..</td></tr>\
		<TR><TD><INPUT id=ptahelpenable type=checkbox /></td><TD>İttifakda Yayınlanan Yardım İsteklerine Otomatik Yardım Eder..</td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Geniş Ekran: '+ htmlSelector({normal:'Normal', wide:'Geniş Ekran', ultra:'Çok Geniş'},GlobalOptions.pbWideScreenStyle,'id=selectScreenMode') +' </td>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Sohbet Penceresini Sağa Taşır(Geniş Ekran Seçiliyse Çalışır..)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>Haritayı Genişletir. (Geniş Ekran Seçiliyse Çalışır..)</td></tr>\
		...
		<TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>Yiyecek Alarmı..! (Otomatik Olarak 2 Saatte Bir Kontrol Edilir..)</td></tr>\
		<TR><TD><BR> </td></tr>\
		<TR><TD colspan=2><B>Otomatik Toplama:</b></td></tr>\
		<TR><TD><INPUT id=ptgoldenable type=checkbox /></td><TD>Otomatik Altın Toplar.. <INPUT id=ptgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
		...
		<TR><TD><INPUT id=ptWHenable type=checkbox /></td><TD>Otomatik Savaş Başlığı Toplar</td></tr>';
		m += '</table><BR><BR><HR>EvrenSerdar Tarafından Optimize Edilmiştir..!';
      div.innerHTML = m;
	  t.cont.innerHTML = m;
...
Tabs.PodTools = {
    ...
    init : function (div){
        ...
        try {
            m = '<TABLE class=ptTab>\
            <TR><TD colspan=2><B>Otomatik Saldırı Seçenekleri:</b></td></tr>\
            <TR>\
                <TD><INPUT id=PodAttack     type=checkbox /></td><TD>Saldırı AKTİF</td>\
                <td style="width:5em"></td>\
                <TD><INPUT id=AttackWild type=checkbox /></td><TD>Bozkır Saldırısı Yapılacağı Zaman İşaretlenir.!</td></tr>\
            <TR>\
                <TD><INPUT id=SupplyTruck   type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; İkmal Kamyonu</td>\
                <td></td>\
                <TD><INPUT id=SupplyChopper type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; İkmal Helikopteri</td>\
            </tr>\
            <TR>\
                <TD><INPUT id=AttackInfantry type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Piyade</td>\
                <td></td>\
                <TD><INPUT id=AttackChopper  type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Savaş Helikopteri</td>\
            </tr>\
            <TR>\
                <TD><INPUT id=AttackSniper  type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Keskin Nişancı</td>\
                <td></td>\
                <TD><INPUT id=AttackFighter type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Avcı Uçağı</td>\
            </tr>\
            <TR>\
                <TD><INPUT id=AttackAntitank type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Tanksavar Piyade</td>\
                <td></td>\
                <TD><INPUT id=AttackBomber   type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Bombardıman Uçağı</td>\
            </tr>\
            <TR>\
            ...
            <TR>\
                ...
                <td></td>\
                <TD><INPUT id=AttackHellfire  type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Cehennem Ateşi Tankı</td>\
            </tr>\
            <TR>\
                <TD><INPUT id=AttackTank    type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Tank</td>\
                <td></td>\
                <TD><INPUT id=AttackStealth type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Görünmez Bombardıman Uçağı</td>\
            </tr>\
            <TR>\
                <TD><INPUT id=AttackPredator type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; İnsansız Uçuk</td>\
                <td></td>\
                <TD><INPUT id=AttackNuke     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Nükleer Füze</td>\
            </tr>\
			<TD><INPUT id=AttackIon     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; İyon Topu</td>\
            </tr>\
            <TR><TD><BR> </td></tr>';
            m += '</table><BR><BR><HR>Asker sayılarını yukarıdaki kutulara girdikden sonra SADECE HARİTADAN SALDIRI YAPILACAK YERE TIKLAYIN.!!';
            t.cont.innerHTML = m;
    ...
    LaunchAttack : function () {
	    ...
	    if(document.getElementById("AttackIon") && document.getElementById(attack_unit_Ion)) {
                document.getElementById(attack_unit_Ion).value=document.getElementById("AttackIon").value;
            }
            ...
            for (var maxenergy=4; maxenergy>0; maxenergy--) {
                for(kk=0;kk< list.length;kk++) {
                    ...
                    if(count == maxenergy){
                        actionLog("Saldıran: General " + kk + " Budur " + count + " Enerjisi Maks.=" + maxenergy,true);
            ...
            if(list.length==0) {
                var text = "HATA: Kullanılabilinecek Generaliniz Bulunmamaktadır..!";
                actionLog(text,true);
                unsafeWindow.Modal.hideModalAll();
                unsafeWindow.Modal.showAlert(text);
                window.setTimeout('Modal.hideModal();', 3000);
...
var RefreshEvery  = {
  ...
  doit : function (){
      actionLog ('Yenilendi: ('+ Options.pbEveryMins +' Zaman Aralığı İçinde..)');
    reloadKOC();
  },
...
function reloadKOC (){
  ...
  var t = '<FORM target="_top" action="'+ goto +'" method=post><INPUT id=xxpbButReload type=submit value=YENİLENİYOR><INPUT type=hidden name=s value="'+ serverId +'"</form>';
  ...
  e_eachMinute : function (){  
    ...
    var now = unixTime();
      ...
      for(i=0; i < Cities.numCities; i++) {
        ...
        if (usage < 0) {
    if (Options.pbFoodAlert && timeLeft<(6*3600)) {
                msg += 'Benim Bu Şehirim de ' + Cities.cities[i].name.substring(0,10) + ' (' +
                      Cities.cities[i].x +','+ Cities.cities[i].y + ')';
                msg += ' Yiyeceğim Kritik Seviyede.. DİKKAT..!! Bana Yiyecek Gönderin ACİL..!! '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+') Giderim: '+addCommas(usage);
                sendChat ("/a " + msg);
          }
    }
      }
...
function dialogRetry (errMsg, seconds, onRetry, onCancel, errCode){
  ...
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Bilinmeyen Bir Hata Meydana Geldi:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Otomatik Olarak Sayfanız Yenilenecektir .. <SPAN id=paretrySeconds></b></span> Süre İçinde ...<BR><BR><INPUT id=paretryCancel type=submit value="İPTAL" \>';
...
var CollectWH = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectWH;
    if (rslt.ok)
      actionLog ('Yapıldı!:'+ rslt.gained.resource2 +' Savaş Başlığı '+ t.colCityName, true);
    else
      actionLog ('HATA Savaş Başlığı Yapılamadı.! '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
/************************ Fırlatma Aracı Monte Fonksiyonu ************************/
var CollectMC = {
  timer : null,
  lastCollect : {},
...
var CollectGold = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectGold;
    if (rslt.ok)
      actionLog ('Toplandı'+ rslt.gained.gold +' Altın '+ t.colCityName +' ( '+ t.colHappy +')', true);
    else
      actionLog ('HATA Altın Toplanamadı!: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectFood = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectFood;
    if (rslt.ok)
      actionLog ('Gıda '+ rslt.gained.resource1 +' Toplandı! '+ t.colCityName, true);
    else
      actionLog ('HATA Gıda Toplanamadı!:'+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectOil = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectOil;
    if (rslt.ok)
      actionLog ('Çıkarıldı '+ rslt.gained.resource2 +' Petrol '+ t.colCityName, true);
    else
      actionLog ('HATA Petrol Çıkarılamadı!: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
function CdialogConfirm (msg, canNotify, contNotify, centerElement){
  ...
  pop.getMainDiv().innerHTML = '<TABLE class=ptTab align=center style="height: 100%"><TR align=center height=90%><TD colspan=2>'+ msg +'</td></tr>\
      <TR align=center><TD><INPUT id=pbok type=submit value="TAMAM" \> &nbsp; &nbsp; </td><TD><INPUT id=pbcancel type=submit value="İPTAL" \> &nbsp; &nbsp; </td></tr></table>';
...
// @description    Global Warfare Oyunu Yardımcı Araçlar Programı
// @include        *.globalwarfaregame.com/*main_src.php*
// @include        *.globalwarfaregame.com/*main_src.php*
// @include        *apps.facebook.com/globalwarfaregame/*
// @include        *apps.facebook.com/globalwarfaregame/*
...
MarchTimer: [],
 ...
 SendIt: false,
    ...
    STRTIME: ["","İNŞAAT","ARAŞTR","EĞİT","SUR","İLERLE"],
    ...
    CreateInterface: function () {
        ...
        this.UpdateDiv("Başlıyor....Hazır",1);
...
opt.setAttribute('id', 'marchBM');
    ...
    Collect: function (cty) {
        ...
        setTimeout("Modal.hideModalAll();",4000);
        this.UpdateDiv("Altın Toplanıyor...",cty);
    },
        SendFarm: function (cty) {
        this.SwitchTo(cty);
        Modal.hideModalAll();
        setTimeout("AutoBuilder.FarmIt();",10);
        this.UpdateDiv("Gıda Toplanıyor...",cty);
    },
         FarmIt: function () {
    ...
    Build: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('İnşaatı Yapılan: ' + AutoBuilder.DisplayTime(Building.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.BuildTimer[cty]);
    ...
    Resrch: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('Araştırması Yapılan: ' + AutoBuilder.DisplayTime(Research.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.RsrchTimer[cty]);
    ...
    Train: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('Eğitimi Yapılan: ' + AutoBuilder.DisplayTime(Barracks.Queue.timeRemaining())," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.TrainTimer[cty]);
   ...
   March: function (cty,forceWait) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('İlerle: ' + AutoBuilder.DisplayTime(" + m + ")," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.MarchTimer[cty]);
    ...
    Stop: function (go, bra) {
        ...
        {
            this.UpdateDiv("Başlatıldı.. " + this.STRTIME[bra] + "....",cty);
            switch (bra)
            {
                case 1:
                    clearTimeout(this.BuildTimer[cty]);
