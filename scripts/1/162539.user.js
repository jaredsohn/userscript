...
/* TO DO 
 ...
 - Spy en auto ?
 - Overview des phares + troupes globales / par joueur / total
 - Envoi auto sur phare des troupes de déf formées (config à mettre en place. Type d'unité ? quantité ? ...)
*/
(function() {
var scriptVersion	= '2013.1114';
var scriptId		= '162539';
var client_ver		= '454';
	...
	create : function (t, user_id, notify){
		...
		function callback (res){
			if (res.done){
				...
				dial.getContentDiv().innerHTML = '<B>Ouuups, une erreur s\'est produite durant la création du spectre.</B>\n'+(res.errmsg || '');
					...
					if (VisitUsers.current.gameData && VisitUsers.current.gameData.city && VisitUsers.current.gameData.city.buildings) {
						...
						if (labo_lvl > 0 && !already_created) {	//&& !is_own_cyborg 
							Cyborgs.createForUserCmd (user_id, function (cyb_res){
								if (cyb_res.done) {
									cyborg_created = true;
									msg += '<b>Spectre créé</b>. ';
								} else 
									msg += 'Erreur lors de la création du spectre.<br>'+(cyb_res.errmsg || '');
								deal_step(current_step + 1);
							});
						} else {
							var sep = '';
							msg += 'Spectre non créé (';
							if (labo_lvl <= 0) { msg += 'pas de crypte'; sep = ', '; }
							if (is_own_cyborg) { msg += sep+'est déjà dans mes spectres'; sep = ', '; }
...
Tabs.Contacts = {
	...
	init : function (div){
		...
		var m = '<div class=bsx_title>Contacts</div>'
			+'<ul class=tabs>'
			+'	<li class="tab first"><a id=tabContactsList>Spectres</a></li>'
	...
	tabContactsList : function (){
		...
		var m = '<div id=tabContacts_Results style="height:640px">'
			...
			+'	<table class=bsx_table style="margin-top:3px" width=100%>'
			+'		<tr valign=top align=left>'
			+'			<td width=16% align=left><label>Mes spectres : </label></td>'
			...
			+'			<td width=19% align=left><label>Spectres chez amis : </label></td>'
		...
		m += '</tr></table></td>'
			...
			+'			<td colspan=4><input id=tabContacts_CB_Done type=checkbox '+ (Data.options.contacts.hide_created?'CHECKED ':'') +' /> Masquer les spectres créés</td>'
