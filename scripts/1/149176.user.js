// ==UserScript==
// @name           Script01-24092012
// @version        09102012
// @namespace      papasmurf
// @homepage       http://userscripts.org/scripts/show/149176
...
var ENABLE_CHAT_TAB = false;
var ENABLE_REINFORCE_TAB = false;
var ENABLE_COMBAT_TAB = false;
var ENABLE_EXPORT_TAB = false;
var ENABLE_THRONE_TAB = false;
var ENABLE_POPULATION_TAB = false;
var ENABLE_GIFTS_TAB = false;
var ENABLE_RESOURCES_TAB = false;
// end test switches
...
var Options = {
  ...
  pbWideMap    : true,
  pbFoodAlert  : true,
  alertConfig  : {aChat:true, aPrefix:'** I\'m being attacked! **', scouting:true, wilds:true, defend:true, minTroops:1, spamLimit:20, lastAttack:0, barbautoswitch:false, raidautoswitch: {}, alertTR:false, alertTRset:1},
  alertSound   : {enabled:true, soundUrl:DEFAULT_ALERT_SOUND_URL, repeat:true, playLength:20, repeatDelay:0.5, volume:100, alarmActive:false, expireTime:0},
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('BH BOT', eventHideShow, mouseMainTab);
  tabManager.init (mainPop.getMainDiv());
  actionLog ("BH Bot v"+ Version +" Loaded  (KofC version: "+ anticd.getKOCversion() +")");
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){  
    ...
    if (Options.pbFoodAlert && timeLeft<(2*3600)) {
                msg += translate("*** LOW FOOD WARNING*** My city")+' '+Cities.cities[i].name.substring(0,10) + ' (' +
...
Tabs.Search = {
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>250)
      errMsg += translate("Radius (distance) must be between")+" 1 +"+translate("and")+" 250<BR>";
    if (errMsg != ''){
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>'+translate("BH Bot Config:")+'</b></td></tr>\
        ...
        <TR><TD><INPUT id=pbupdate type=checkbox '+ (GlobalOptions.pbupdate?'CHECKED ':'') +'/></td><TD>'+translate("DO NOT CHECK, MANUAL UPDATE ONLY")+' '+ htmlSelector({0:'Userscripts', 1:'Google Code'},GlobalOptions.pbupdatebeta,'id=pbupdatebeta') +' '+translate("(all domains)")+' &nbsp; &nbsp; <INPUT id=pbupdatenow type=submit value="'+translate("Update Now")+'" /></td></tr>\
        ...
        <TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>'+translate("Enable Food Alert (on less than 2 Hours of food. Checked every hour)")+'</td></tr>\
...
Tabs.Reinforce = {
  tabOrder: 40,
  tabDisabled : !ENABLE_REINFORCE_TAB,
  tabLabel: 'Reinforce',
  myDiv: null,
  cityID: null,
  rallypointlevel:null,
...
Tabs.Resources = {
  tabOrder : 100,
  tabDisabled : !ENABLE_RESOURCES_TAB,
...
Tabs.popcontrol = {
  tabOrder : 850,
  tabDisabled : !ENABLE_POPULATION_TAB,
