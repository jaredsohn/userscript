...
// @name           BWPT V20120119
// @namespace      BiggyWill
// @version        BiggyWill PT V20120119
// @description    Global Warfare Powertools
...
function ptStartup (){
  ...
  var styles = '.ptTabs {color:black; font-size:12px}\
    ...
    .pbStat {border:1px solid; border-color:#ffffff; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#969696}\
    ...
    table.pbSrchResults tr td {border-left:1px solid #ccc; white-space:nowrap; padding: 1px;}\
    ...
    .ptStat {border:1px solid; border-color:#ffffff; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#969696}\
    .ptStatLight {color:#ddd}\
	.ptentry {padding: 7px; border:1px solid; border-color:#000000; background-color:#969696; white-space:nowrap;}\
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '\
      ...
      <DIV id="srcOpts" style="height:100px"></div></td><td style="visibility:hidden"><DIV id=divOutOpts style="background:#969696; padding:10px ;visibility:hidden"></div></td></tr></table></div>\
      <DIV id="srcResults" style="height:470px; max-height:1000px;"></div>';
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    m = '<DIV class=ptstat><TABLE width=100% cellspacing=0><TR><TD class=xtab width=125><DIV id=statSearched></div></td>\
      ...
      <TABLE width=100%><TR><TD><DIV id=divOutTab style="width:100%; height:470px; max-height:1000px; overflow-y:auto;"></div></td></tr></table>';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      ...
      if (numRows > 64000 && t.searchRunning){
        numRows = 64000;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: Table only shows 64,000 of '+ dat.length +' results until search is complete.</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
    ...
    var t = Tabs.transport;
	  ...
	  case 'unt19': params.u19 = wagons_needed;break;
         ...
         new AjaxRequest(unsafeWindow.g_ajaxpath + "march.php" + unsafeWindow.g_ajaxsuffix, {
                  ...
                  if (rslt.ok) {
                  actionLog('Trade   From: ' + cityname + "   To: " + xcoord + ',' + ycoord + "    ->   Cargo Transport: " + wagons_needed);
...
Tabs.PodTools = {
    ...
    init : function (div){
        ...
        try {
            ...
            <TR>\
                <TD><INPUT id=PodAttack     type=checkbox /></td><TD>Active Attack</td>\
                <td style="width:5em"></td>\
                <TD><INPUT id=AttackWild type=checkbox /></td><TD>Mark Your Time Attack</td></tr>\
            <TR>\
            ...
            <TR><TD><BR> </td></tr>';
            m += '</table><BR><BR><HR>After entering the number of troops in the boxes above, just maps ATTACK CLICK TO BE THE PLACE.!!';
            t.cont.innerHTML = m;
...
var CollectWH = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectWH;
    if (rslt.ok)
      actionLog (' Collected'+ rslt.gained.resource2 +' Warheads For:  '+ t.colCityName, true);
    else
      actionLog (' ERROR: Collecting Warheads For: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectMC = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectMC;
    if (rslt.ok)
      actionLog (' Collected '+ rslt.gained.resource2 +' Launch Vehicle For:  '+ t.colCityName, true);
    else
      actionLog (' ERROR: Collecting Launch Vechicle For: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
