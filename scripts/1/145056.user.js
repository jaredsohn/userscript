// ==UserScript==
// @name       T&P Redesigned
// @namespace  http://vickychijwani.github.com/
// @version    1.3.3
// @description  Redesign and enhancements for the IIT Kharagpur Training and Placement Notice Board.
// @match      http://tp.iitkgp.ernet.in/notice/
// @match      http://tp.iitkgp.ernet.in/notice/index.php*
// @copyright  2012, Vicky Chijwani
// ==/UserScript==
...
if ( typeof define === "function" && define.amd && define.amd.jQuery ) {
  ...
  function highlight_placements(element) {
    ...
    var words = ['placement', 'interview', 'shortlist', 'ppo', 't&p', 't & p', '4th', '5th', 'final', 'ppt', 'imp', 'urgent', 'atten', 'result'];
  ...
  function highlight_internships(element) {
    ...
    var words = ['internship', 'interview', 'shortlist', '3rd', 'summer', 'ppt', 'imp', 'urgent', 'atten', 'result'];
    $(element).find('tr').each(function() {
      var tr = this;
      _.each(words, function(word) {
  ...
  function processItems(items) {
    // notices
    var table = $('<table>').addClass('notices');
    table.css({
      'position': 'relative',
      'width': '100%',
    ...
    last_page = parseInt($(items[items.length - 1]).find('strong:last').text());
    ...
    _.each(fragmented_items, function(item, index) {
      ...
      notice.html("<td class='notice-time'><span class='notice-attachment'>" + (item[4] || '') + "</span>" + item[0] + "</td><td class='notice-title'><a target='_blank' href='" + item[2] + "' onclick='" + item[3] + "'>" + item[1] + "</a></td>");
      //console.log(notice);
      ...
      notice.find('td.notice-title a').hover(function() {
        $(this).parent().css({
          'background': '#f7f7f7',
    ...
    $('<span class="page-marker">Page ' + current_page + '</span>').css({
      'position': 'absolute',
      'bottom': '-15px',
      'left': '100%',
      'width': '100px',
  ...
  // search box
  var search_box = $('<input class="search" type="text" autofocus placeholder="Filter notices..." />');
  search_box.css({
    'display': 'inline',
    'float': 'right',
    'width': '250px',
  ...
  // fetches @num_pages pages sequentially, calls @callback after all have been fetched
  var fetchPages = function(num_pages, callback) {
    console.log(num_pages + ' pages to go...');
    callback = callback || function() {};
    num_pages = num_pages || 1;
    $(window).unbind('scroll', scrollHandler);
    $.get(href_next, {}, function(response) {
      processItems($(response).find('table > tbody > tr'));
      $(window).scroll(scrollHandler);
      if (num_pages > 1) {
        console.log('fetching next page...');
        fetchPages(num_pages - 1, callback);
      }
      else if (num_pages === 1) {
  ...
  // event handler to load the next page when the user scrolls to the bottom
  ...
  var more = $('<a class="more">&darr; More</a>');
  var pages_per_more = 10;
  more.click(function() {
    more.hide();
    spinner.show();
    fetchPages(pages_per_more, function() {
      spinner.hide();
      more.show();
    });
  });
  ...
  // spinner to display when user clicks "More"
  var spinner = $(" \
    <div id='bowlG'> \
      <div id='bowl_ringG'> \
