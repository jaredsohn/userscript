// ==UserScript==
// @name        Elderrain cashcamel
// @namespace   Elderrain cashcamel
...
// @require     http://userscripts.org/scripts/source/167388.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link.com/*
// @include     *://*oc.us/*
...
({color:'#000',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:399,height:

20,background:'#AFFFAF',border:'2px solid green',bottom:10,right:10})
	...
	setTimeout(function(){
		...
		var clickSrc = ($('.ptcWrapper li center').size()) ? $('.ptcWrapper li center') 
...
: $('.ptcWrapper td');
		...
		$.each(clickSrc,function(k,v){
			...
			if(thisItem.attr('href')){
				if( ( thisItem.attr('href').indexOf('gpt.php?v=entry&type=ptc') 

>= 0 || thisItem.attr('href').indexOf('gpt.php?v=read') >= 0 ) && thisItem.text().indexOf('Cheat 
...
Check') == -1){
		...
		if(!arr.length){
			$.each($('a,div'),function(k,v){
				...
				$
				if(thisItem.attr('href')){
					if( (thisItem.attr('href').indexOf('gpt.php?

v=entry&type=ptc') >= 0 || thisItem.attr('href').indexOf('gpt.php?v=read') >= 0 ) && 

thisItem.text().indexOf('Cheat Check') == -1){
						var obj = {
							href 	: thisItem.attr('href'),
							jObj	: thisItem
						}
					...
					if( (thisItem.attr('onclick').toString().indexOf

('entry&type=ptc') >= 0  ) && thisItem.text().indexOf('Cheat Check') == -1){
						var obj = {
							href 	: 'gpt.php?v='+thisItem.attr

('onclick').toString().split(',')[1].match(/'[^]+'/)[0].replace(/'/g,''),
							jObj	: thisItem
						}
						arr.push(obj); 
					}
		...
		function rec(ctr){
			...
			if(arr[ctr]){
				if(typeof adsWind == 'undefined')
					adsWind = wparent.open(arr

[ctr].href,"janbee","width=100,height=50,top=1000,left=20000");
				else	
					adsWind.location.href = arr[ctr].href;
				$(adsWind).ready(function(){adsWind.resizeTo(600,600)}).error

(function(){wparent.success('fail');})
				div.text('clicking : '+clickNum+' / '+ arr.length +' - loading : 
...
if(wparent.location.href.indexOf('gpt.php?v=read') != -1 && self == top){
	var div = $('<div>');
	div.css({font:'bold 10px 

arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:85,height:20,backgroun

d:'#AFFFAF',border:'2px solid green',top:10,left:10})
		.text('');
...
d:'#AFFFAF',border:'2px solid green',top:10,left:10})
		.text('');
	$('html').css({position:'relative'}).append(div);
	var thisFrame = ($('frameset frame:first').size()) ? $('frameset frame:first') : 
...
$('iframe:first')
	...
	if($('frame:last').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('cashcamel')!=-1)cheatCheck = 

[49,27,51,32,35,8,39,60,46,22,65];
...
('checked',true);
		...
		var inters = setInterval(function(){
			...
			if($('#timer',doc).text().indexOf('Click') >= 0 || $('#timer 

img:visible',doc).size()){
				clearInterval(intersAds);
				function ajaxRec(btnKey){
					$.ajax({
						url : 'gpt.php?v=verify&buttonClicked='+btnKey

+'&id='+win.id+'&type='+win.type+'&pretime='+win.pretime+'&'+win.url_variables
					}).complete(function(a,b){
						console.log(a)
						console.log(b)
						if(a.responseText.indexOf('You clicked the wrong 

number')>=0){
							btnKey++;
							ajaxRec(btnKey)
						}
						else if(wparent.location.href.indexOf('type=ce') 
...
('canvas');
							...
							// Draw image on canvas to get its pixel 
...
div',doc).attr('style').match(/url\([^]+\)/)[0].replace('url(','').replace(')',''))
							var imgobj = $('<img>').attr
...
(imgobj[0],'-'+(primeImg.width()*k)))
							})
						}
						else{
							$.each($('#buttons img',doc),function
...
(k,v){
						...
						btnKey = checker.indexOf(Math.max.apply(Math, 
...
checker)) ;
					...
					primeImg.load(function(){gameClick();}).ready(function

(){gameClick();}).error(function(){wparent.opener.success('fail');});
