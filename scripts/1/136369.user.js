...
// @description    Easier Kongregate's Dawn of the Dragons - IDIOT Edition
// @author         SReject, chairmansteve, Snowman25
// @version        0.0.11
// @date           07.16.2012
			...
			if (typeof r != 'undefined' && typeof r.diff != 'undefined' && typeof r.hash != 'undefined' && typeof r.boss != 'undefined' && typeof r.id != 'undefined') {
				...
				r.linkText = function () {
					if (SRDotDX.config.formatRaidLinks){
						...
						txt = txt.replace(/&lt;image&gt;/gi,'<image src="http://cdn2.kongregate.com/game_icons/0033/2679/i.gif" style="vertical-align: text-top; float: left;">');
						if ((!this.name?'':this.stat)=='H') {
							return "LOL! IDIOT POSTED A GUILD-RAID."
						} else {
							return txt
						}
					}
				...
				{					
					...
					if(linklist.length>10)
					{
						...
						while(link = patt.exec(linklist))
						{
							...
							timer+=ttw+Math.floor(Math.random()*2000);
							i+=1;
						}
					}
					setTimeout(function() {						
...
This is probably only useful if you have a clipboard listener like my 'DotD raid link spammer' which may (or may not) be linked in the forums.";
		...
		load: function (fails) {
			if (typeof holodeck == 'object' && typeof ChatDialogue == 'function' && typeof activateGame == 'function' && typeof document.getElementById('kong_game_ui') != 'null') {
				...
				holodeck.addChatCommand("becreepy", function(deck,text) {
                    console.log("Creep-creep!");

                    var people = [];
                    var stalker = holodeck._active_user._attributes._object.username;
                    function findStrangers(){
                    ...
                    function tentacleRape(person){
                        // console.log(person)
                        new Ajax.Request('/accounts/' + stalker + '/friends/' + person + '.chat', {asynchronous:true, evalScripts:true, method:'put', onSuccess:function(request){$('add_friend_indicator').hide(); $('remove_friend').show()}, parameters:'authenticity_token='+encodeURIComponent(active_user.formAuthenticityToken())});
                    }

                    findStrangers();
                    SRDotDX.echo('Creeping upon ' + people.length + ' innocents...')
                    people.forEach(tentacleRape);

                    return false;
                });
				holodeck.addChatCommand("uncreep", function(deck,text) {
                    console.log("Creep-creep!");
                    ...
                    function tentacleCuddle(person) {
                        // console.log(person)
                        new Ajax.Request('/accounts/' + stalker + '/friends/' + person + '.chat', {asynchronous:true, evalScripts:true, method:'delete', onSuccess:function(request){$('add_friend_indicator').hide(); $('add_friend').show()}, parameters:'authenticity_token='+encodeURIComponent(active_user.formAuthenticityToken())});
                    }

                    findFriends();
                    SRDotDX.echo('Uncreeping ' + people.length + ' innocents...')
                    people.forEach(tentacleCuddle);
		...
		raids: {
			...
			dragons_lair:{name: 'DRAGONS LAIR!!', shortname: 'DRAGONS LAIR!!',  id: 'dragons_lair', stat: 'S', size:10, duration:5, health: [100000000,500000000,1000000000,1500000000,,]},
