...
// @include /^https?://(|www\.)kaskus.co.id/classified/*
// @include http://www.kaskus.co.id/classified/*
// @license (CC) by-nc-sa 3.0
// @exclude /^https?://(|www\.)kaskus.co.id/post_reply/*/
// @exclude /^https?://(|www\.)kaskus.co.id/visitormessage/*/
// @version 1B
...
gvar.sversion = 'v' + '1B';
gvar.scriptMeta = {
titlename: 'QR PLUS++' + ( isQR_PLUS !== 0 ? '+' : '' )
,scriptID: 167990 // script-Id
...
if (window.location.href.indexOf("&text") > -1) //==>plus
{buff = buff.replace(/<(?!br)(\/?[^>(1,)]+)>/gi, '');
}//==>plus
else{buff = buff.replace(/(?:<\!-{2,}reason\s?[^>]+.)+/gi,"");}
...
if (window.location.href.indexOf("&text") > -1)
				...
				quoteS = '[QUOTE=' + _QQ.get_quotefrom(post_id) + ']';
				quoteE = '[/QUOTE]'  + "\n\n";
				}//end
if( ret ) 
_TEXT.add( quoteS+''+ ret +''+quoteE );//==>plus
}
});
},
...
if( $.inArray( A, [13, 66,73,85,88, 69,76,82] ) != -1 ){
if(A===13){
_BOX.init();
_BOX.presubmit();
}else
...
//tambahan
//plus++
$('#btn-slide-close').click();
//add delay di title
var count = 41,
udul = $('title').get(0),
judulasli = $('title').text(),
lam = location.href.indexOf('/?p') > -1,
lamG = location.href.indexOf('/group/discussion/') == -1;
if(lam && lamG ){
countdown = setInterval(function() {
 
$(udul).html( '['+count+']'+'&nbsp;&nbsp;'+ judulasli);
...
//css tombol auto copas
GM_addStyle(" #plus{color:#FFF !important;float:right;text-align:center;margin:-20px -30px 0 0;width:15px;height:16px;border-radius:3px;padding:0 2px 2px}  .cplus{background:#FA5858;} .cplus:hover{background:#FE2E2E;} .splus{background:#999;} .splus:hover{background:#777}");

	$('td.post').each(function(){
	$('div.post-title a, .sub-meta .pagination ul li a').attr('target','_blank');
		var bn = $(this).find('.pagination ul li:last').text(),
			current = 200,
			bk = window.location.href.indexOf('classified') > -1;
		if (bn > current && !bk ){
			var min = 114,
				max = 190,
				random = Math.floor(Math.random() * (max - min + 1)) + min,
				lintt = $(this).find('.link_thread_title').attr('href'),
				plax = '<a href="'+lintt+''+random+'&text" id="plus" class="cplus" target="_blank" data-original-title="Auto Copas" rel="tooltip"><i class="icon-arrow-right"></i></a>';
			
			$(this).find('.post-title').not('.subforum-table .post-title, .sticky .post-title').append(plax);
		}
	});
	$("a#plus, .post-title").each(function(){$(this).click(function(){
	$(this).not('.post-title').addClass('splus');
	$(this).removeClass('cplus');
	$(this).closest('.post-title').children('.link_thread_title:first').attr('style','font-weight:500 !important');
	})
	});
...
//auto copas
				...
				var nn = Math.floor((Math.random()*19)+1);
				
				$('.hfeed:eq('+nn+')').find('.button_qq').click();
				if( $('#'+gvar.tID).val() < -1) alert('test');
