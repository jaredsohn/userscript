// ==UserScript==
// @name           memperbesar photo in facebook
// @namespace      agungwea
// @description    Menampilkan gambar profil lengkap berukuran besar ketika Anda mengarahkan mouse ke thumbnail pada foto profil atau dinding.
    ...
    if (imagesrc.include('profile.ak.facebook.com') || imagesrc.include('profile.ak.fbcdn.net') || imagesrc.include('fbcdn-profile-a.akamaihd.net')) {
      if (imagesrc.substr(imagesrc.lastIndexOf('\/')+1,1) == 'q'
        ...
        /* old haxxor no longer needed march 10 */ 
          ...
          imgs[i].addEventListener('mouseout', function(){$ge('FB_PIC_EVO').innerHTML = '';$ge('FB_PIC_EVO').style.visibility='hidden';}, true); 
        }
      }
    }
  }
