// ==UserScript==
// @name       UnlockedMW VIP
// @namespace   mafiawars
// @description Unlocks MW
// @include     http://facebook.mafiawars.zynga.com/mwfb/remote/html_server.php*
// @include     http://mwfb.zynga.com/mwfb/remote/html_server.php*
// @include     http://facebook.mafiawars.zynga.com/mwfb/xd_receiver.htm
// @include     http://apps.facebook.com/inthemafia/*
// @include     http://apps.new.facebook.com/inthemafia/*
// @include     http://www.facebook.com/connect/uiserver*
// @include     https://www.facebook.com/dialog/feed*
// @exclude     http://mwfb.zynga.com/mwfb/*#*
// @exclude     http://facebook.mafiawars.zynga.com/mwfb/*#*
// @exclude     http://apps.facebook.com/inthemafia/sk_updater.php*
// @exclude	    http://facebook.mafiawars.zynga.com/mwfb/iframe_proxy.php*
// @include     https://facebook.mafiawars.zynga.com/mwfb/remote/html_server.php*
// @include     https://mwfb.zynga.com/mwfb/remote/html_server.php*
// @include     https://facebook.mafiawars.zynga.com/mwfb/xd_receiver.htm
// @include     https://apps.facebook.com/inthemafia/*
// @include     https://apps.new.facebook.com/inthemafia/*
// @include     https://www.facebook.com/connect/uiserver*
// @exclude     https://mwfb.zynga.com/mwfb/*#*
// @exclude     https://facebook.mafiawars.zynga.com/mwfb/*#*
// @exclude     https://apps.facebook.com/inthemafia/sk_updater.php*
// @exclude	    https://facebook.mafiawars.zynga.com/mwfb/iframe_proxy.php*
// @version     4.9.0
// ==/UserScript==

function injectScript(source) {
	// Utilities
	var isFunction = function (arg) {
		return (Object.prototype.toString.call(arg) == "[object Function]");
	};
	var jsEscape = function (str) {
		// Replaces quotes with numerical escape sequences to
		// avoid single-quote-double-quote-hell, also helps by escaping HTML special chars.
		if (!str || !str.length)
			return str;
		// use \W in the square brackets if you have trouble with any values.
		var r = /['"<>\/]/g,
		result = "",
		l = 0,
		c;
		do {
			c = r.exec(str);
			result += (c ? (str.substring(l, r.lastIndex - 1) + "\\x" + c[0].charCodeAt(0).toString(16)) : (str.substring(l)));
		} while (c && ((l = r.lastIndex) > 0))
		return (result.length ? result : str);
	};
	var bFunction = isFunction(source);
	var elem = document.createElement("script"); // create the new script element.
	var script,
	ret,
	id = "";
	if (bFunction) {
		// We're dealing with a function, prepare the arguments.
		var args = [];
		for (var i = 1; i < arguments.length; i++) {
			var raw = arguments[i];
			var arg;
			if (isFunction(raw)) // argument is a function.
				arg = "eval(\"" + jsEscape("(" + raw.toString() + ")") + "\")";
			else if (Object.prototype.toString.call(raw) == '[object Date]') // Date
				arg = "(new Date(" + raw.getTime().toString() + "))";
			else if (Object.prototype.toString.call(raw) == '[object RegExp]') // RegExp
				arg = "(new RegExp(" + raw.toString() + "))";
			else if (typeof raw === 'string' || typeof raw === 'object') // String or another object
				arg = "JSON.parse(\"" + jsEscape(JSON.stringify(raw)) + "\")";
			else
				arg = raw.toString(); // Anything else number/boolean
			args.push(arg); // push the new argument on the list
		}
		// generate a random id string for the script block
		while (id.length < 16)
			id += String.fromCharCode(((!id.length || Math.random() > 0.5) ? 0x61 + Math.floor(Math.random() * 0x19) : 0x30 + Math.floor(Math.random() * 0x9)));
		// build the final script string, wrapping the original in a boot-strapper/proxy:
		script = "(function(){var value={callResult: null, throwValue: false};try{value.callResult=((" + source.toString() + ")(" + args.join() + "));}catch(e){value.throwValue=true;value.callResult=e;};" + "document.getElementById('" + id + "').innerText=JSON.stringify(value);})();";
		elem.id = id;
	} else // plain string, just copy it over.
	{
		script = source;
	}
	elem.type = "text/javascript";
	elem.innerHTML = script;
	// insert the element into the DOM (it starts to execute instantly)
	document.head.appendChild(elem);
	if (bFunction) {
		// get the return value from our function:
		ret = JSON.parse(elem.innerText);
		// remove the now-useless clutter.
		elem.parentNode.removeChild(elem);
		// make sure the garbage collector picks it instantly. (and hope it does)
		delete(elem);
		// see if our returned value was thrown or not
		if (ret.throwValue)
			throw(ret.callResult);
		else
			return (ret.callResult);
	} else // plain text insertion, return the new script element.
		return (elem);
}
var myscript = function () {
$qXSyjankCMO3n1$DTSj=function(n){if(typeof($qXSyjankCMO3n1$DTSj.list[n])=="string")
return $qXSyjankCMO3n1$DTSj.list[n].split("").reverse().join("");return $qXSyjankCMO3n1$DTSj.list[n]};
$qXSyjankCMO3n1$DTSj.list=["repparw_tegdiw_2vf","sgnittes_dekcolnu",/ Current Page: index_controller /,"wen_llac_yxes. raw_eralced#",/does not exist/i,/are already at war/,/their war status/,/"masteryTotal":(\d+),/,/jobResult":{"city":9,"id":(\d+),/,/jobResult":{"city":7,"id":(\d+),/,/jobResult":{"city":8/,/"job_mastery_percentage":(\d+),/,/\\/g,"yub_eslupmi der trohs wen_nottub_yxes wen_nottub_yxes",/local_xw_sig = '([\da-f]+)';/," rebmem werc ","\"=crs gmi<>\";)(evomer.)\'rebmemwercetavitca-2vporp#\'($\"=kcilcno \";xp5:pot ;xp5:thgir ;etulosba:noitisop\"=elyts \"ntb-esolc\"=ssalc a<","rebmemwercetavitca-2vporp","1=tols_werc&thgif=epyt_psd_werc&porp=epyt_werc&",/https/,"9-llatcelloc-2vporp","7-llatcelloc-2vporp",">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"8\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<","3_elggoTslavir","\"=crs gmi<>naps<>naps<>\"3_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<","\"=crs gmi<>naps<>naps<>\"2_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<","1_elggoTslavir","\"=crs gmi<>naps<>naps<>\"1_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<","_elggoTtegrat\"=di \";evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<","_elggoTtegrat\"=di \";evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<","\"=crs gmi<>naps<>naps<>\"0_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<","\"=crs gmi<>naps<>naps<>\"0_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<",/targetToggle_(\d+)/,"\"=crs gmi<>naps<>naps<",/rivalsToggle_(\d+)/,"cni_tl_kta_wmu","ltt_tl_kta_wmu",/toolbar\.unlockedmw\.com/,";xp0 :pot ;evitaler :noitisop","MBmotsuCdekcolnU","\"=di \"ppa_elggot\"=ssalc \";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<",">rt/< >dt/<>a/<>\"","\"=di \"ppa_elggot\"=ssalc\";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<",">llams<>dt<>dt/<",">llams<>dt<>dt/<>\"",">\"030303#\"=rolocgb rt<","])\'0 :hserfeR\',)(gnirts(sniatnoc[naps//naps//]\"tsoc_hserfer_bor\"=di@[a//","=noitanitsed&7=ytic_wx&levart=noitca_wx&levart=rellortnoc_wx?php.revres_lmth/etomer",";)bor.rellortnoCgnibboR ,)2(boRerp.rellortnoCgnibboR ,0 ,1 ,\'","llAboRkcilCeno","gnibboR ot nruteR :reyalS",">vid/<>vid/<>retnec/<>naps/<",">\"xp5-:pot;evitaler:noitisop\"=elyts \"eltit_pupoPym\"=di naps<>\"eltit_eludom_eripme\"=ssalc \"xp02:thgieh;xp","\"=htdiw \"fig.002x067_tneidarg_gb/eripme/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>\"pop_ofni_PE_inim\"=ssalc \"99999:xedni-z\"=elyts vid<>a/<>\"))di.edoNtnerap.siht(dIyBtnemelEteg.tnemucod(dlihCevomer.)\'reddof_pupop\'(dIyBtnemelEteg.tnemucod\"=kcilcno \"esolc_pop\"=ssalc \"#\"=ferh \"esolC_poPym\"=di a<>\";999999:xedni-z;xp",":tfel;xp001:pot;etulosba:noitisop\"=elyts \" xob_pop\"=ssalc \"","!txen WMdekcolnU ot nigol ot deen lliw uoY ?dekcolnU etadpu dna egarots lla raelc ot tnaw yllaer uoy oD",/\n/g,"sgat_tsilkcalb_dekcolnu","delbasid_tsilkcalb_dekcolnu","delbasid_tsiletihw_dekcolnu",/true/,"sgnitteSevaS",">vid/<>a/<>naps/<>naps/<etadpU & egarotS raelC>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"etaDpUecroF\"=di a<>rb<>rb<>a/<>naps/<>naps/<erotS ppA>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"MBmotsuC\"=di a<;psbn&;psbn&>a/<>naps/<>naps/<evaS>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"sgnitteSevaS\"=di a<","\"=eulav tupni<;psbn&]0001-*333[ etaR kcattA thgiF","\"=eulav tupni<;psbn&etaR yaleD gnibboR/sdauqS boR>\"dekcehC\"=eulav \"dauqsbor\"=eman \"dauqsbor\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<YN ni;psbn&>\"dekcehC\"=eulav \"ynlaeh\"=eman \"ynlaeh\"=di \"xobkcehc\"=epyt tupni<>\"2dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"","\"=eulav tupni<;psbn&tegrat rep skcatta mumixaM>\"dekcehC\"=eulav \"kcehcpots\"=eman \"kcehcpots\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<stegraT yhtlaeH diovA;psbn&>\"dekcehC\"=eulav \"yhtlaeh\"=eman \"yhtlaeh\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"3dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"","\"=eulav tupni< fo stsruB esU>\"dekcehC\"=eulav \"tsrub\"=eman \"tsrub\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"dimroftsop\"=di \";xp09:htdiw;enon:eziser\"=elyts \"txet\"=epyt \"","laeh=noitca_wx&latipsoh=rellortnoc_wx?php.revres_lmth","htlaeh_xam_resu","0000000001=tnuoma&","8=di_tneilc_wx&5=ytic&6=epyt_gnidliub&",/\,/g,"tsoob eerf a teG",/Need help icing yours.+/,".slavir ruoy eci ot tsoob thgif eerf ruoy teG",/var feed = (.+?); MW\.Feed/,/msg.fight_result =/,"tnuoc_latot_wmu","tnuoc_eci_wmu","deci_yalrevo_rednefed_2vf",/user_fields\['user_stamina'\] = parseInt\("([\d]+)/,/'user_cash_london'\] = "\\u00a3([\d,]+)"/,/xw_controller=hitlist&xw_action=attack([&\w\d=]+)/,/message_body">You <strong>LOST<\/strong> the fight/,"yub_eslupmi wen_kcatta_yxes der muidem wen_nottub_yxes",/user_stamina":([\d]+)/,"ogacihc_hsac_resu",/exp_to_next_level":([\d]+),/,"no_tsoob_ntbktarewop_2vthgif","ntBkcattA2Vthgif yub_eslupmi der trohs wen_nottub_yxes","sutats_thgif_rekcatta",/The action was not able to be completed/,/Mafia you Attacked/,/Your Attackers/,/targetToggle/,"1=bat&weiv=noitca_wx&thgif=rellortnoc_wx?php.revres_lmth/etomer",/10 Attack/,"wen_hserfer_egap_yxes wen_nottub_yxes","!tsilkcalb eht no si ylimaf stegrat reyalS","p_kcatta_ntb","0=bat&weiv=noitca_wx&thgif=rellortnoc_wx?php.revres_lmth/etomer","levart=unem&=smaraPtxen&xedni=morf&","=resu&weiv=noitca_wx&stats=rellortnoc_wx?php.revres_lmth/etomer",/'user_cash_brazil'\] = "([BRL$\d,]+)"/,/'user_cash_london'\] = "([Ã‚Â£\d,]+)"/,/exp_to_next_level'\] = parseInt\("([\d]+)/,/percent_complete'\] = "([\d]+)/,/user_cash_nyc'\] = "([$\d,]+)/,/percent_complete":([\d]+),"/,/user_cash_brazil":"([BRL$\d,]+)"/,/user_cash_vegas":"([V$\d,]+)"/,/user_cash_london":"([Ã‚Â£\d,]+)"/,"cyn_hsac_resu","1=emarf_qer_piks&","dedocnelru-mrof-www-x/noitacilppa","tseuqeRpttHLMX",/'p\|([\d]+)',0,'(.+)'\);/,/job([\d]+)/,/tab=([\d]+)/,/person=([\d]+)/,/tmp=([\da-f]+)/,/([\d]+) Energy/,"level_txen_ot_pxe","C7%p=di_tegrat&","=ytic_wx&raw_eralced=noitca_wx&raw=rellortnoc_wx?php.revres_lmth/etomer",/(^|[^\/])(www\.[\S]+(\b|$))/gim,/(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim,">vid/<>a/<moc.WMdekcolnU @ erom daeR>\"knalb_\"=tegrat \"",">\" ;xp01 xp0 xp01 xp0 :nigram\"=elyts vid<>naps/<"," >\";xp51 :ezis-tnof\"=elyts \"knalb_\"=tegrat \"","01=tnuoc&stnemhcatta,stnemmoc,nialp_eltit,noitpac,sutats,sutats_tnemmoc,tnuoc_tnemmoc,rohtua,sgat,seirogetac,deifidom,epyt,guls,tnetnoc=edulcxe&stsop_tnecer_teg=nosj?/moc.wmdekcolnu.ehcac//:ptth",">retnec/< gnidaoL>\"gnidaol_deef_golb_wmu\"=di retnec<>rb<>rb<>rb<>rb<>rb<>rb<"," :pot ;xp9 :tfel ;00002 :xedni-z ;etulosba :noitisop",";enon :yalpsid","deef_golb_wmu","\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp031:tfel\"=elyts \"ffOnOtsoP\"=di a<>a/<>naps/<>naps/<>\"\"","\"=crs \"42\"=htdiw \"42\"=thgieh \";xp1 :tfel ;xp4-:pot ;evitaler:noitisop\"=elyts gmi<>naps<>naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \"ogol_wmu\"=di a<>elyts/<} ;xp5-:pot ;evitaler:noitisop { ngilAvWMU.>\"ssc/txet\"=epyt elyts<>\";xp057:htdiw;xp03 :thgieh\"=elyts \"eltit_eludom_eripme xifraelc\"=ssalc \"tnetnoc_rabloot\"=di vid<","htlaeh_kcolc","rabloot_kcolnu","tpircsavaj/txet","reniatnoc_sloot",">vid/<>vid/<","\"=di \"mbdaol_dekcolnu\"=ssalc a<","reniatnoc_gol","\"=crs \";6.0 :yticapo;)06=yticapo(ahpla :retlif ;xp9 :pot ;evitaler :noitisop\"=elyts \"retoof_gol_WMU\"=di gmi<>\"077063887536612/emaG-sraW-aifaM-ruoy-kcolnU-WMdekcolnU/segap/moc.koobecaf.www//:ptth\"=ferh \"knalb_=tegrat \"egapnaF WMdekcolnU\"=eltit a<>vid/<(: deggol sgnihtoN>\";yerg:roloc;%43:tfel;xp2:pot;evitaler:noitisop\"=elyts vid<>\";kcolb:x-wolfrevo;kcolb:y-wolfrevo;xp56:thgieh-nim;xp073:thgieh-xam;xp6:pot;xp6 :pot ;xp6 xp6 :suidar-tfel-mottob-redrob ;xp6 xp6 :suidar-thgir-mottob-redrob ;xp6   :thgirmottob-suidar-redrob-zom- ;xp6   :tfelmottob-suidar-redrob-zom-;kcalb:roloc-dnuorgkcab;xp691-:tfel;evitaler:noitisop;xp092:htdiw;xp1-:pot-nigram;enon :yalpsid ;02 :xedni-z\"=elyts \"gol-WMU\"=di vid<>a/< >naps/<naps/<goL>\";suounitnoc :ycilop-enilni-dnuorgkcab-zom- ;gniddap :nigiro-dnuorgkcab-zom- ;redrob :pilc-dnuorgkcab-zom- ;tfel :ngila-txet ;%05 xp57 llorcs taeper-on )fig.worra_levart_nwodpord/scihparg/bfwm/moc.agnyz.citats.bfwm//:ptth(lru tnerapsnart :dnuorgkcab\"=elyts naps<>naps<>\";xp59 :htdiw\"=elyts \"redrob_etihw_kcalb trohs wen_nottub_yxes\"=ssalc a<>\";xp371 :pot ;xp056 :tfel ;xp001 :htdiw;etulosba:noitisop\"=elyts \"reniatnoc_gol\"=di vid<>elyts/<};xp8- :pot ;evitaler :noitisop ;xp5 :mottob-nigram ;otua :thgir-nigram ;otua :tfel-nigram ;kcolb :yalpsid{ retoof_gol_WMU# };xp2 0 :nigram ;dlob :thgiew-tnof ;xp21 :ezis-tnof ;xp1 xp1 xp1 :htdiw-redrob ;dcdcdc# dilos xp1 :redrob{gol-WMU#>\"ssc/txet\"=epyt elyts<","retoof_gol_WMU","fws.01draobpilCoreZ/segami/moc.wmdekcolnu.ehcac//:ptth","\"=crs \";6.0 :yticapo;)06=yticapo(ahpla :retlif ;xp4 :pot ;evitaler :noitisop\"=elyts \"retoof_gol_WMU\"=di gmi<>\"077063887536612/emaG-sraW-aifaM-ruoy-kcolnU-WMdekcolnU/segap/moc.koobecaf.www//:ptth\"=ferh \"knalb_=tegrat \"egapnaF WMdekcolnU\"=eltit a<>vid/<",">ht/<>\"%52\"=htdiw ht<>ht/<>naps/<",">rt<>\"0\"=redrob elbat<",/^Slayer/,")1=yticapo(ahpla","stegraT reppoH gnihcteF reyalS",/&amp;/g,/Sucker Punch/,/Remove from Mafia/,"01dimroftsop","hcnuaLreppoH","\"=eulav tupni<;psbn& DI tekcuB kcattA>rb<>rb<>rb<!reppoh morf stegrat thgif yllacitamotua ot WMdekcolnU esu won nac uoy ,moc.stsilwm ta sdneirf ruo morf ecivres a si reppoH>rb<","eludoMetadpUreyalP",".yrter neht ,eralced ot tnaw yllaer uoy fi tser remit a esahcrup ,raw ta ydaerlA","daeh_raw_eralced","evitcA edoM elttaB reyalS","detceles bat_van_nalc","evitcA edoM gnibboR reyalS","evitcA edoM tsilthgiF reyalS",/levels">\((.*?)\)/,/Ask Mafia to Attack and Let them Get a Reward!/,"])\"gnibboR\" ,.(sniatnoc[a//])\"no_bat\" ,ssalc@(sniatnoc[il//","])\"slaviR\" ,.(sniatnoc[a//])\"elddim_bat no_bat\" ,ssalc@(sniatnoc[il//","])\"srethgiF\" ,.(sniatnoc[a//])\"elddim_bat no_bat\" ,ssalc@(sniatnoc[il//","deppotS reyalS",">naps/<>naps/<>\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<","\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<",">naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs  gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<","ngilAvWMU kcalb trohs wen_nottub_yxes","ngilAvWMU der trohs wen_nottub_yxes","eldnahtnihmetiwol",";xp521 :htdiw;xp5:pot;xp7-:tfel;evitaler:noitisop",";kcolb :yalpsid","\"=kcilcno vid<",";)(elggot.eludoMsmetItsroW.WM","2yalrevOrorrE#","dnuorgkcaBgnidaoL#","sj.redaolpgo/sg/kd.ynomis//:ptth","sj.renepoknil/telkramkoob/moc.telkcops//:ptth","sj.smeti_evitca_ym/sppa/moc.wmdekcolnu.rabloot//:ptth","smetI evitcA yM",".ay rof mekop eht syalP","sj.reyubetarc/stpircs/wm/xsseug/moc.etaroballaerc.www//:ptth","sj.xuder.bojtaeper/telkramkoob/moc.telkcops//:ptth","xudeR boJtaepeR","selttab ylimaf eht tuoba ofni tnatropmi teG","!enorD wen ruo tseT ateB","sj.renetrohs_lru/sppa/moc.wmdekcolnu.rabloot//:ptth","sj.bm_der/sppa/moc.wmdekcolnu.rabloot//:ptth","retfiG gaB yretsyM deR","ffuts yretsam ylimaf morf PX tcelloc ot siht esU","sj.sdrawerelttab/mb/moc.reficul-ufmm//:ptth","rotcelloC elttaB",".stniop lliks ruoy sngissA","sj.ssobylimaf_lu/1261201453/moc.tpircsavajruoy//:ptth","rethgiF ssoB dekcolnU",".YN ni sessob eht sthgiF","sj.ateb-tsilkcehc/telkramkoob/moc.telkcops//:ptth","sj.ofni_ylimaf/telkramkoob/moc.telkcops//:ptth","sj.skniltfigwm/mb/moc.reficul-ufmm//:ptth","sj.cpx2ne/stpircs/wm/xsseug/moc.etaroballaerc.www//:ptth","sj.enordsxg/stpircs/wm/xsseug/moc.etaroballaerc.www//:ptth","sj.rekcuhC/sg/kd.ynomis//:ptth","sj.ynitteg/mb/moc.reficul-ufmm//:ptth","sj.rezylana.yrotnevni/telkramkoob/moc.telkcops//:ptth","rezylanA metI","rethgiF ssoB reficuL",".weivrevo reisae rof meht spuorg dna stsop detaler sraW aifaM rof deef swen ruoy nacS","sj.rotcelloctfig/telkramkoob/moc.telkcops//:ptth","rotcelloC tfiG",".thgif ssoB eht ni ecno esu ,sessob ylimaf eht thgiF","sj.repuorGyrotnevnI/69noilehtoel/moc.stpeercs//:ptth","repuorg yrotnevni","reganaM ylimaF",".yalp t\'nod taht seno eht tuo kcik ,aifam ruoy eganaM","sj.stols_esool/telkramkoob/moc.telkcops//:ptth","sj.retsalbtfig/telkramkoob/moc.telkcops//:ptth","retsalB tfiG",".gnitfarc yliad ruoy od dna ,seitreporp ruoy morf tcelloC","sj.ateb-cmz/telkramkoob/moc.telkcops//:ptth","rotcelloC CMZ",".resworb ruoy ni rablooT agnyZ eht gnillatsni tuohtiw tsoob modnar eht teG",".seitreporp cificeps bor ottelkramkoob gnibboR","sj.gb.rebbor/telkramkoob/moc.telkcops.www//:ptth","sj.rotan-a-nissassa/telkramkoob/moc.telkcops//:ptth","rotaN-a-nissassA",".neercs thgif ro egap eliforp no esU .koobecaf ot sraW aifaM morf sehctiwS",".emag eht emarfnU",".ylno srebmem PIV ruo rof si loot sihT .lizarB ni smeG eht tluaV",".ylno srebmem PIV rof si loot sihT .PR gnisu gnitfarC ylraE","EC39TZqMWdprv7RA6PHmt6072LVm4gzDt",/mw_city(\d)/,"==,46esab;gnp/egami:atad","=,46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad",",46esab;gnp/egami:atad","==,46esab;gnp/egami:atad","==,46esab;gnp/egami:atad","repparw_ytic_wm","=/+9876543210zyxwvutsrqponmlkjihgfedcbaZYXWVUTSRQPONMLKJIHGFEDCBA","sj.dliubdeeps/1242551041/moc.tpircsavajruoy//:ptth","sj.megtluav/0215621541/moc.tpircsavajruoy//:ptth","sj.emarfnu/sppa/moc.wmdekcolnu.rabloot//:ptth","sj.hctiws/moc.wmdekcolnu.rabloot//:ptth",".esu erofeb relaeh WMdekcolnU ffo nruT .loot gnithgif hcir erutaeF",".egap boR eht no nur eb ot deen ton seod ,telkramkoob gnibboR weN","xudeR rebboR","sj.xuder.rebbor/telkramkoob/moc.telkcops//:ptth","sj.kcapinim/aifam/moc.mlohkcops.www//:ptth",".kcab tfig otua htiw ,retneC egasseM agnyZ eht morf stfiG tcelloC","reganaM ytreporP","sj.reganam.ytreporp/telkramkoob/moc.telkcops.www//:ptth",".aifam eritne ruoy ot stfig fo stsol tsalB",".tnemeveihca eht teg meht pleh ,stols sdneirf ruoy yalP","reganaM aifaM","sj.yrotsih_aifam/telkramkoob/moc.telkcops//:ptth","sj.yrotsih_ylimaf/telkramkoob/moc.telkcops//:ptth",".smeti depuorg fo evah uoy tool ynam woh no weiv a teg ylkciuQ","rethgiF ssoB kcopS","sj.rethgif-ssob-ylimaf/telkramkoob/moc.telkcops//:ptth",".siht htiw deen uoy tahw tcelloc ,stfig htiw aifam ruoy gnitsalb retfA","rennacS maertS","sj.rennacs-maerts/telkramkoob/moc.telkcops//:ptth","sj.5ssobsmafwm/mb/moc.reficul-ufmm//:ptth",".fo dir teg nac uoy tahw ees ,yrotnevni ruoy sezylanA",".pupop a gnisu ,lru gnol a morf LRUyniT etareneG",".ffuts elbatfig htiw skrow ,aifam ruoy ot stfig dneS",".pu gnilevel uoy peek ot cte ,sbor ,sboj snuR",".setar PX ot ygrenE seviG","aifam ruoy ot stfig dnes ot siht esU",".egap ylimaf eht no noitamrofni erom teG",".serohc ruoy od ot uoy dnimer ,tsilkcehC yliaD","rethgiF ssoB YN","sj.rethgifssobyn/telkramkoob/moc.telkcops//:ptth","loot rehto yna naht retteb sessob ylimaf eht sthgiF","nissassA tnioP llikS","sj.retsop_lliks/telkramkoob/moc.telkcops//:ptth",".egap elttab morf esu ,elttab ylimaf morf tooL tcelloC","PX tcelloC nalC","sj.pxtcellocnalc/mb/moc.reficul-ufmm//:ptth","sdneirf ruoy ot sgaB yretsyM deR tfiG","ecivres em.nokcops gnisu SLRU yniT etareneG","enorD dekcolnU","sj.enordu/selif/moc.edocelgoog.nroblliw//:ptth","sj.tluserelttab/mb/moc.reficul-ufmm//:ptth",".ay rof sboj staepeR",".ereht tuo sOH etarc uoy lla roF","rotan-a-znieH","sj.rotarekop/telkramkoob/moc.telkcops//:ptth",".YLNO gnithgif ot sdrager ni ,esnefed + kcatta rof smeti evitca lla stsiL","sknil fo not a nepo ot siht esU","efil ot kcab thguorb enord s\'ytfeL","yalrevOgnidaoL#","1yalrevOrorrE#","nottub_ekil_wm#",".esolc ot em kcilC",">vid/<>\"elddimsba\"=ngila \"61\"=thgieh \"91\"=htdiw \"fig.10_61x22_kcatta_aifam_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>\"serocS metI tsewoL\"=eltit \";xp85:pot;xp01-:tfel;evitaler:noitisop;xp02:htdiw\"=elyts \"","gnikcart_tats_wmu",">rt/<>ht/<0>\"cni_tl_fed_wmu\"=di \"doog\"=ssalc \"%04\"=htdiw ht<>ht/<000,000>\"ltt_tl_fed_wmu\"=di \"esnefed_puorg\"=ssalc \"lamron:thgiew-tnof\"=elyts \"%06\"=htdiw ht<>rt<>rt/<>ht/<0>\"cni_tl_kta_wmu\"=di \"doog\"=ssalc \"%04\"=htdiw ht<>ht/<000,000>\"ltt_tl_kta_wmu\"=di \"kcatta_puorg\"=ssalc \"lamron:thgiew-tnof\"=elyts \"%06\"=htdiw ht<>rt<",">retnec/<>b/<serocS metI tsewoL>b<>retnec<",">naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<","ngilAvWMU neerg trohs wen_nottub_yxes",">naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<",">naps/<>naps/<>\"","ngilAvWMU egnaro trohs wen_nottub_yxes",">a/<>naps/<>naps/<>\"\"","evitcA tsilthgiF ykcitS reyalS","evitcA slaviR ykcitS reyalS","evitcA edoM gnibboR ykcitS reyalS",/user=p\|(\d+)'.+>Profile/,"evitcA edoM eliforP reyalS","evitcA edoM slaviR reyalS","])\"seilimaF\" ,.(sniatnoc[a//])\"tsrif_bat no_bat\" ,ssalc@(sniatnoc[il//",/user=(.+?)\"/,"evitcA ylimaF thgiF reyalS",/Declare war now for/,"etihw muidem wen_nottub_yxes","stegraT reppoH moc.stsilwM thgiF",">rb<>rb<>a/<>naps/<>naps/<tratS>naps<>naps<>\"kcalb wen_nottub_yxes\"=ssalc \"hcnuaL\"=di a<>rb<>rb<>\"01dimroftsop\"=di \";xp051:htdiw;enon:eziser\"=elyts \"txet\"=epyt\"","reddof_pupop","lautxetnoc tpmorp_yub_eslupmi","tegrat reppoH txeN gnidaoL reyalS",/Sucker Punch.+?<a href="(.+?)".+?>Add to Hitlist/,"=tekcub?php.wmdekcolnu_evilteg/stelkraMkooB/moc.stsilwm//:ptth","gnorts oot llA ro stegraT eviL oN :reyalS",")06=yticapo(ahpla",">\";xp682:htdiw;xp051 :thgieh-nim ;xp003 :thgieh-xam;kcolb:x-wolfrevo;llorcs:y-wolfrevo\"=elyts vid<>a/<;psbn&;psbn&goL raelC;psbn&;psbn&>\"gol_raelc-WMU\"=di a<>\";2.0 :yticapo;)02=yticapo(ahpla :retlif;xp2-:pot;evitaler:noitisop;%09:htdiw\"=elyts rh<",">\";xp2-:pot;xp5:tfel;evitaler:noitisop\"=elyts naps<>\"%57\"=htdiw ht<",">\";2.0 :yticapo;)02=yticapo(ahpla :retlif;xp1-:pot;evitaler:noitisop;%09:htdiw\"=elyts rh<>elbat/<>rt/<",">vid/<>vid/<>a/<>naps/<>naps/<)!ateb( ypoC>naps<>naps<>\";xp03-:pot ;xp081:tfel ;evitaler:noitisop\"=elyts \"wen_nottub_yxes\"=ssalc \"#\"=ferh \"golelbaypoc\"=di a<>a/<>\"",">naps/<>naps/<!deipoC secI>naps<>naps<","gol_raelc-WMU",">vid/<>vid/<>a/<>\"","> \";x051:htdiw;xp2-:pot-nigram;enon :yalpsid ;02 :xedni-z\"=elyts \"uneMsloot\"=di vid<>a/< >naps/<naps/<slooT>\";suounitnoc :ycilop-enilni-dnuorgkcab-zom- ;gniddap :nigiro-dnuorgkcab-zom- ;redrob :pilc-dnuorgkcab-zom- ;tfel :ngila-txet ;%05 xp57 llorcs taeper-on )fig.worra_levart_nwodpord/scihparg/bfwm/moc.agnyz.citats.bfwm//:ptth(lru tnerapsnart :dnuorgkcab\"=elyts naps<>naps<>\";xp59 :htdiw\"=elyts \"redrob_etihw_kcalb trohs wen_nottub_yxes\"=ssalc a<>\";xp371 :pot ;xp411 :tfel ;etulosba :noitisop ;xp001 :htdiw\"=elyts \"reniatnoc_sloot\"=di vid<>elyts/<};xp2 0 :nigram ;dlob :thgiew-tnof ;xp21 :ezis-tnof ;xp1 xp1 0 :htdiw-redrob ;dcdcdc# dilos xp1 :redrob{ uneMsloot#>\"ssc/txet\"=epyt elyts<",">\"elddim noitanitsed_yxes\"=ssalc vid<>\"","sknil_kcolnu","mbdaol_dekcolnu","=ytic_wx&metIsunoBdda=noitca_wx&sunoBpUlevel=rellortnoc_wx?php.revres_lmth/etomer","sgnitteSkcolnu",">a/<!WON LAEH>\"#\"=ferh a<","\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp521:tfel\"=elyts \"ffOnOlivE\"=di a<>a/<>naps/<>naps/<>\"gnp.hcnerw_remmah_noci/3v/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp021:tfel\"=elyts \"sgnitteSkcolnu\"=di a<>a/<>naps/<>naps/<>\"fig.10_61x61_hsac_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<>\"xp01:tfel\"=elyts \"BnottuBknaB\"=di \"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc a<>vid/<>\";enon :yalpsid\"=elyts \"xob_pop\"=ssalc \"deef_golb_wmu\"=di vid<>a/<>naps/<>naps/<>\"",">vid/<>naps/<>naps/<0>\";yerg:roloc;xp04:htdiw;xp5:tfel;evitaler:noitisop\"=elyts \")slliK + secI( latoT\"=eltit \"tnuoc_latot_wmu\"=di naps<>naps/<:T >\";xp5:tfel;evitaler:noitisop\"=elyts naps<>naps/<0>\";yerg:roloc;xp04:htdiw;xp5:tfel;evitaler:noitisop\"=elyts \"slliK\"=eltit \"tnuoc_llik_wmu\"=di naps<>naps/<:K >\";xp5:tfel;evitaler:noitisop\"=elyts naps<>naps/<0>\";yerg:roloc;xp04:htdiw;evitaler:noitisop\"=elyts \"secI\"=eltit \"tnuoc_eci_wmu\"=di naps<>naps/<:I>naps<>\"sutatSdekcolnu\"=di \";xp81:ezis-tnof;xp002:htdiw;xp651:tfel;xp2-:pot;evitaler:noitisop\"=elyts naps<>a/<>naps/<>naps/<>\"fig.10_61x61_ygrene_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp541:tfel\"=elyts \"ffOnOsboJ\"=di a<>a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp041:tfel\"=elyts \"ffOnOmatS\"=di a<>a/<>naps/<>naps/<>\"fig.10_61x61_htlaeh_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>\"wen_htlaeh_yxes\"=ssalc naps<>\"ngilAvWMU kcalb trohs wen_nottub_yxes\"=ssalc \";xp531:tfel\"=elyts \"ffOnOlaeH\"=di a<>a/<>naps/<>naps/<>\"",/px/,"rabloot_mlohkcops","xp0 :thgieh ;xp0 :thgieh-nim ;xp0 :thgieh-xam ;xp0 :htdiw ;xp0 :htdiw-nim ;xp0 :htdiw-xam ;kcolb :yalpsid ;xp","gnidaol_deef_golb_wmu",">rb<>a/<>\")\';enon :yalpsid\',\'elyts\'(etubirttAtes.)\'deef_golb_wmu\'(dIyBtnemelEteg.tnemucod\"=kcilcno \"esolc_pop\"=ssalc \"#\"=ferh \"esolC_poPym\"=di a<>a/<>\"gnp.dekcolnu/segami/moc.wmdekcolnu.ehcac//:ptth\"=crs gmi<>\"knalb_\"=tegrat \"moc.wmdekcolnu//:ptth\"=ferh a<>\";xp082 :thgieh-nim ;xp717 :htdiw-nim;xp082 :thgieh-xam ;xp717 :htdiw-xam;xp082 :thgieh ;xp717 :htdiw;ezilatipac:mrofsnart-txet;xp21 :ezis-tnof;neddih :x-wolfrevo ;otua :y-wolfrevo ;xp01 xp01 xp01 xp01 :nigram\"=elyts vid<",">\";yerGmid :roloc\"=elyts naps<psbn&-;psbn&>a/<","\"=ferh a< >rb<",">\";2.0 :yticapo;)02=yticapo(ahpla :retlif;%09:htdiw\"=elyts rh<",">a/<1$>\"knalb_\"=tegrat \"1$\"=ferh a<",">a/<2$>\"knalb_\"=tegrat \"2$//:ptth\"=ferh a<1$",/([\d]+)/,"egap_raw_eralced=morf&","]])\";etihw dilos xp1 :redrob\",)(gnirts(sniatnoc[elyts@[]\"xifraelc ydob_egassem\"=ssalc@[vid//","=ytic_wx&bojod=noitca_wx&boj=rellortnoc_wx?php.revres_lmth",/cb=([\da-f]+)/,/job=([\d]+)/,"]])\"552 ,552 ,552\",)(gnirts(sniatnoc[elyts@[]\"ofni_boj\"=ssalc@[vid//",/(p\|[\d]+)/,"htiW-detseuqeR-X","epyT-tnetnoC","=di_resu_wx_fs&1=daoletil&1=xaja",/user_cash_nyc":"([$\d,]+)"/,"nodnol_hsac_resu","sagev_hsac_resu","lizarb_hsac_resu"," :htdiw ;tfel :taolf ;tfel :ngila-txet ;)94 ,202 ,14(bgr :roloc-dnuorgkcab ;neddih :y-wolfrevo ;neddih :x-wolfrevo",/"user_stamina":([\d]+),"user_max_stamina/,/user_energy'\] = parseInt\("([\d]+)/,/user_cash'\] = parseInt\("([\d]+)/,/user_cash_nyc'\] = "([$\d,]+)"/,/'user_cash_vegas'\] = "([V$\d,]+)"/," GNIPPOTS deretsam :reyalS","=noitanitsed&levart=noitca_wx&levart=rellortnoc_wx?php.revres_lmth/etomer","tsilthgif gnidaoL-eR reyalS","daed_reyalp_tsil_thgif","!daed ro gnorts oot era stegrat reyalS"," yub_eslupmi wen_kcatta_yxes trohs wen_nottub_yxes"," yub_eslupmi wen_kcatta_yxes muidem wen_nottub_yxes",".tsilthgif gnidaoL-eR reyalS",/Your Rivals/,/Family Rivals/,/btn_attack_p/,/is already dead or too weak!/,".skcus kcehceci agnyZ :reyalS",/Lost/,"ntBkcattA2Vthgif yub_hctab_eslupmi der trohs wen_nottub_yxes","ffo_tsoob_ntbktarewop_2vthgif",/user_cash_london":"\\u00a3([\d,]+)"/,/user_cash_chicago":"\\u00a2([\d,]+)"/,"nelots_yalrevo_rednefed_2vf","])\'niagA kcattA\',)(gnirts(sniatnoc[]\'wen_kcatta_yxes der trohs wen_nottub_yxes\'=ssalc@[a//","?php.revres_lmth",/user_fields\['exp_to_next_level'\] = parseInt\("([\d]+)/,/'user_cash_chicago'\] = "\\u00a2([\d,]+)"/,"tsil_diova_dekcolnu",/boost', 'link': '(.+?)'/,"tnuoc_llik_wmu","dellik_yalrevo_rednefed_2vf",/'feed_js': '(.+?)'/,".slavir ruoy eci ot tsoob thgif eerf a teG",/Think you can do better.+/,/Need to whack someone.+/,/V\$([\d,]+)/,"=tnuoma&tisopeDknaBnoitcA=noitcaod&5=ytic_wx&noitcaod=noitca_wx&2Vytreporp=rellortnoc_wx?php.revres_lmth","=ytic_wx&lla_tisoped=noitca_wx&knab=rellortnoc_wx?php.revres_lmth","8=di_tneilc_wx&yevrus_spn_wohs=noitca_wx&yevrus=rellortnoc_wx?php.revres_lmth",/,/,"\"=eulav tupni<;psbn& ot seci tsopotuA >\"dekcehC\"=eulav \"tsopotua\"=eman \"tsopotua\"=di \"xobkcehc\"=epyt tupni<>rb<","\"=eulav tupni<;psbn&gniniamer PX ta esuaP>\"dekcehC\"=eulav \"kcehcesuap\"=eman \"kcehcesuap\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<tahC ediH >\"dekcehC\"=eulav \"tahCevomer.sgnittes.dekcolnu\"=eman \"tahcmer\"=di \"xobkcehc\"=epyt tupni<;psbn&knabotuA >\"dekcehC\"=eulav \"tiknab\"=eman \"knabotua\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<srelaetS ecI >\"dekcehC\"=eulav \"nelotswollof\"=eman \"nelotswollof\"=di \"xobkcehc\"=epyt tupni<;psbn&;psbn&>\"77dimroftsop\"=di \";xp01:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"","\"=eulav tupni<;psbn&dlohserhT laeH>rb<>rb<>\"7dimroftsop\"=di \";xp05:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"",">rb<>rb<sunoB pu-leveL mialC >\"dekcehC\"=eulav \"levelmialc\"=eman \"levelmialc\"=di \"xobkcehc\"=epyt tupni<>rb<>rb<>\"5dimroftsop\"=di \";xp53:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"",">vid/<>vid/<>aeratxet/<>\";%56:htdiw;enon:eziser\"=elyts \"sgat_tsilkcalb_dekcolnu\"=di aeratxet<>rb<ffO >\"elggot_tsilkcalb_dekcolnu\"=eman \"delbasid_tsilkcalb_dekcolnu\"=di \"oidar\"=epyt tupni<nO >\"elggot_tsilkcalb_dekcolnu\"=eman \"delbane_tsilkcalb_dekcolnu\"=di \"oidar\"=epyt tupni<:tsilkcalB>\".yltcaxe gat emag-ni eht hctam TSUM deretne gat ehT\n\n)tsilthgiF & egap slaviR( .gat siht htiw sreyalp piks lliw reyalS dna ,enil rep eno ,gat ylimaf a retnE\"=eltit vid<>vid/<>aeratxet/<>\";%56:htdiw;enon:eziser\"=elyts \"sgat_tsiletihw_dekcolnu\"=di aeratxet<>rb<ffO >\"elggot_tsiletihw_dekcolnu\"=eman \"delbasid_tsiletihw_dekcolnu\"=di \"oidar\"=epyt tupni<nO >\"elggot_tsiletihw_dekcolnu\"=eman \"delbane_tsiletihw_dekcolnu\"=di \"oidar\"=epyt tupni<:tsiletihW>\".yltcaxe gat emag-ni eht hctam TSUM deretne gat ehT\n\n)ylno egap slaviR( .gat siht kcatta ylno lliw reyalS dna ,enil rep eno ,gat ylimaf a retnE\"=eltit vid<>vid<>rb<>rb<agnyZ yb dewolla deeps tsetsaf eht si 333*>rb<>\"6dimroftsop\"=di \";xp53:htdiw;enon:eziser\" =elyts \"regetni\"=epyt \"","sgnitteSdekcolnU","nelotswollof","delbane_tsiletihw_dekcolnu","delbane_tsilkcalb_dekcolnu","sgat_tsiletihw_dekcolnu",/(.+)/,"77dimroftsop","php.nigol-pw/moc.wmdekcolnu//:ptth"," :htdiw;kcolb :yalpsid;xp",":htdiw;xp5-:pot;evitaler:noitisop\"=elyts vid<>rb<>\"gnp.dekcolnu/segami/moc.wmdekcolnu.ehcac//:ptth\"=crs \";evitaler:noitisop\"=elyts gmi<>retnec<>\";xp0:tfel;xp0:pot;etulosba:noitisop;)ged081(etator:mrofsnart-tikbew-;)ged081(etator:mrofsnart-zom-;)ged081(etator:mrofsnart\"=elyts \"57\"=thgieh \"",">\"xp5:pot;evitaler:noitisop\"=elyts naps<>vid/<>naps/<","sdnoces 03 ni niaga yrt - animats fo tuo gnibboR :reyalS",/RobbingController.robAllHelper\((\d+),/,"=tols&1=ytic_wx&bor=noitca_wx&gnibbor=rellortnoc_wx?php.revres_lmth/etomer\',\'\'(xaja_od","tsoc_hserfer_bor","levart=unem&=smaraPtxen&1=enoz&gnibbor=morf&",">\"4\"=gniddapllec \"0\"=gnicapsllec \"0\"=redrob elbat<>\";llorcs:y-wolfrevo;xp004:thgieh-xam;xp044:htdiw-xam;xp024:htdiw\"=elyts vid<>rb<","\"=crs gmi<>dt<",">rb<>llams/<",">dt/< >llams/<","\"=crs gmi<>\"","\"=crs gmi<>\"knalb_\"=tegrat \"/piv-ot-edargpu/moc.wmdekcolnu//:ptth\"=ferh \"ppa_elggot\"=ssalc \";xp0:pot ;evitaler:noitisop\"=elyts a<;psbn&>\"08\"=htdiw dt<",">vid/<>elbat/<","erotS ppA telkramkooB","sppa_dekcolnu","nioj/moc.wmdekcolnu//:ptth","ltt_tl_fed_wmu","cni_tl_fed_wmu","kcalb trohs wen_nottub_yxes","neerg trohs wen_nottub_yxes",">a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<",">a/<>naps/<>naps/<>\"","0_elggoTslavir",">a/<>naps/<>naps/<>\"gnp.61x61_animats_noci/scihparg/bfwm/moc.ndcngz.citats.bfwm//:ptth\"=crs gmi<>naps<>naps<>\"","_elggoTtegrat","\"=crs gmi<>naps<>naps<>\"1_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<","\"=crs gmi<>naps<>naps<>\"2_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"neerg trohs wen_nottub_yxes\"=ssalc a<","2_elggoTslavir","\"=crs gmi<>naps<>naps<>\"3_elggoTslavir\"=di \";xp61:tfel;xp32:pot;evitaler:noitisop\"=elyts \"kcalb trohs wen_nottub_yxes\"=ssalc a<",">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"7\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<",">a/<>naps/<>naps/<x2>naps<>\";xp53 :htdiw\"=elyts naps<>\"9\"=ytic \"etamwerCetavitcA neerg trohs wen_nottub_yxes wen_nottub_yxes\"=ssalc \"#\"=ferh \";xp5:nigram;thgir:taolf\"=elyts \"rebmem werc noitcelloc ytreporp x2 etavitcA\"=eltit a<","8-llatcelloc-2vporp","etamwerCetavitcA","=ytic_werc&1=xajasi&etavitca=noitca_wx&werCytiC=rellortnoc_wx?php.revres_lmth/etomer/bfwm/moc.agnyz.srawaifam.koobecaf//:","/: nwodlooc no si",";xp006 :pot ;%94 :tfel ;%04 :htdiw ;kcolb :yalpsid",">rb<>retnec<>\"eltit-fnoc\"=ssalc vid<>a/<>\"41\"=htdiw \"41\"=thgieh \"fig.10_41x41_nottub_esolc_kcalb/scihparg/bfwm/6e/ten.dwnll.sh.vpagnyz//:",">vid/<>retnec/<>rb<>rb<","ntb-etanimile-tidnab"," ]])\";etihw dilos xp1 :redrob\",)(gnirts(sniatnoc[elyts@[]\"xifraelc ydob_egassem\"=ssalc@[vid//",/"job_mastery_percentage":/,/jobResult":{"city":7/,/jobResult":{"city":9/,/jobResult":{"city":8,"id":(\d+),/,/"masteryTotal":/,/<!-- Current Page: war_controller ([\d]+) -->/,/too low of a level to declare war/,/user not found/i,"wen_llac_yxes etihw trohs wen_nottub_yxes",/"group_atk"/,"cmz_reddof_pupop#",/[^A-Za-z0-9\+\/\=]/g];
	
localStorage.setItem("scriptumw", 1);
var g = {
  images: {
    unlocked_icon: $qXSyjankCMO3n1$DTSj(244),
    fb_icon: $qXSyjankCMO3n1$DTSj(245),
    evil_icon: $qXSyjankCMO3n1$DTSj(243),
    spock_app: $qXSyjankCMO3n1$DTSj(246),
    mmfu_app: $qXSyjankCMO3n1$DTSj(242),
    unlocked_app: $qXSyjankCMO3n1$DTSj(247),
    simony_app: $qXSyjankCMO3n1$DTSj(241),
    guessx_app: $qXSyjankCMO3n1$DTSj(248),
    installed: $qXSyjankCMO3n1$DTSj(240),
    disabled: $qXSyjankCMO3n1$DTSj(249),
    viponly: $qXSyjankCMO3n1$DTSj(239),
    autostart: $qXSyjankCMO3n1$DTSj(250)
  },
  log: new Array(),
  user: {
    firstRun: true,
    group_atk: 0,
    group_def: 0,
    start_group_atk: 0,
    start_group_def: 0,
    group_atk_inc: 0,
    group_def_inc: 0
  },
  xw_user: User.id,
  city: $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1],
  umwavoidlist: new Array(),
  rivalcycle: new Array(),
  livelinks: new Array(),
  livelinkindex: 0,
  familytargets: new Array(),
  familymodeOn: false,
  familyindex: 0,
  newfighton: false,
  settings: {
    stamOn: true,
    postOn: false,
    healOn: true,
    fbfeedloc: "",
    jobsOn: false,
    healat: 7000,
    healny: true,
    umwicesteal: false,
    burstOn: true,
    pauseOn: false,
    stopat: 500,
    autopostOn: false,
    rref: 400,
    FighterInt: 555,
    autoBank: false,
    removeChat: true,
    abortOn: false,
    abortHeal: false,
    abortAt: 150,
    hopperid: $qXSyjankCMO3n1$DTSj(237),
    userobsquads: false,
    claimlevel: false,
    slayerOn: false,
    slayerMode: 0,
    evilCity: $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1],
    count: 1,
    bursts: 4,
    customRivals: new Array(),
    Whitelist_enabled: false,
    Whitelist: new Array(),
    Blacklist_enabled: false,
    Blacklist: new Array(),
  },
  hitlist: false,
  abortCount: 0,
  CurHealth: 0,
  evilOn: false,
  NYjobon: false,
  Bjobon: false,
  popcount: 0,
  targetid: 0,
  target_name: "",
  ajax: false,
  icecount: 0,
  killcount: 0,
  Bjobid: 0,
  ILVjobon: false,
  fightwatchdogtimout: 0,
  icecheckfail: false,
  fightInterval: 0,
  healInterval: 0,
  energyInterval: 0,
  keyStr: $qXSyjankCMO3n1$DTSj(252),
  evilInterval: 0,
  closing: false,
  fightActionRunning: false,
  declaring_war: false,
  currlevel: $("#user_level").text(),
  is_vip: true,
  RHangCHKInterval: 0,
  RobbingHung: false
};
    var k = new Array({
        vendor: "Spockholms",
        name: "New NY Property parts",
        description: "New NY Property parts",
        url: "http://spocklet.com/bookmarklet/missionlink.js",
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
     }, {
        vendor: "UnlockedMW",
        name: "Speedbuild",
        description: $qXSyjankCMO3n1$DTSj(236),
        url: $qXSyjankCMO3n1$DTSj(253),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "UnlockedMW",
        name: "Vault Gems",
        description: $qXSyjankCMO3n1$DTSj(235),
        url: $qXSyjankCMO3n1$DTSj(254),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "UnlockedMW",
        name: "Unframe",
        description: $qXSyjankCMO3n1$DTSj(234),
        url: $qXSyjankCMO3n1$DTSj(255),
        image: g.images.unlocked_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: "Switch",
        description: $qXSyjankCMO3n1$DTSj(233),
        url: $qXSyjankCMO3n1$DTSj(256),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(232),
        description: $qXSyjankCMO3n1$DTSj(257),
        url: $qXSyjankCMO3n1$DTSj(231),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: "Robber BG",
        description: $qXSyjankCMO3n1$DTSj(258),
        url: $qXSyjankCMO3n1$DTSj(230),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(259),
        description: $qXSyjankCMO3n1$DTSj(229),
        url: $qXSyjankCMO3n1$DTSj(260),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: "MiniPack",
        description: $qXSyjankCMO3n1$DTSj(228),
        url: $qXSyjankCMO3n1$DTSj(261),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(227),
        description: $qXSyjankCMO3n1$DTSj(262),
        url: $qXSyjankCMO3n1$DTSj(226),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(263),
        description: $qXSyjankCMO3n1$DTSj(225),
        url: $qXSyjankCMO3n1$DTSj(264),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(224),
        description: $qXSyjankCMO3n1$DTSj(265),
        url: $qXSyjankCMO3n1$DTSj(223),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: "Loose Slots",
        description: $qXSyjankCMO3n1$DTSj(266),
        url: $qXSyjankCMO3n1$DTSj(222),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(267),
        description: $qXSyjankCMO3n1$DTSj(221),
        url: $qXSyjankCMO3n1$DTSj(268),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(220),
        description: $qXSyjankCMO3n1$DTSj(221),
        url: $qXSyjankCMO3n1$DTSj(269),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(219),
        description: $qXSyjankCMO3n1$DTSj(270),
        url: $qXSyjankCMO3n1$DTSj(218),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(271),
        description: $qXSyjankCMO3n1$DTSj(217),
        url: $qXSyjankCMO3n1$DTSj(272),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(216),
        description: $qXSyjankCMO3n1$DTSj(273),
        url: $qXSyjankCMO3n1$DTSj(215),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(274),
        description: $qXSyjankCMO3n1$DTSj(214),
        url: $qXSyjankCMO3n1$DTSj(275),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Lucifer",
        name: $qXSyjankCMO3n1$DTSj(213),
        description: $qXSyjankCMO3n1$DTSj(217),
        url: $qXSyjankCMO3n1$DTSj(276),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(212),
        description: $qXSyjankCMO3n1$DTSj(277),
        url: $qXSyjankCMO3n1$DTSj(211),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Lucifer",
        name: "Tiny Urls",
        description: $qXSyjankCMO3n1$DTSj(278),
        url: $qXSyjankCMO3n1$DTSj(210),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Simony",
        name: "Chucker",
        description: $qXSyjankCMO3n1$DTSj(279),
        url: $qXSyjankCMO3n1$DTSj(209),
        image: g.images.simony_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "GuessX",
        name: "Drone.x",
        description: $qXSyjankCMO3n1$DTSj(280),
        url: $qXSyjankCMO3n1$DTSj(208),
        image: g.images.guessx_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "GuessX",
        name: "En2XP.x",
        description: $qXSyjankCMO3n1$DTSj(281),
        url: $qXSyjankCMO3n1$DTSj(207),
        image: g.images.guessx_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "Lucifer",
        name: "Gift Sender",
        description: $qXSyjankCMO3n1$DTSj(282),
        url: $qXSyjankCMO3n1$DTSj(206),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: "Family Info",
        description: $qXSyjankCMO3n1$DTSj(283),
        url: $qXSyjankCMO3n1$DTSj(205),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: "Checklist",
        description: $qXSyjankCMO3n1$DTSj(284),
        url: $qXSyjankCMO3n1$DTSj(204),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(285),
        description: $qXSyjankCMO3n1$DTSj(203),
        url: $qXSyjankCMO3n1$DTSj(286),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "UnlockedMW",
        name: $qXSyjankCMO3n1$DTSj(202),
        description: $qXSyjankCMO3n1$DTSj(287),
        url: $qXSyjankCMO3n1$DTSj(201),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(288),
        description: $qXSyjankCMO3n1$DTSj(200),
        url: $qXSyjankCMO3n1$DTSj(289),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Lucifer",
        name: $qXSyjankCMO3n1$DTSj(199),
        description: $qXSyjankCMO3n1$DTSj(290),
        url: $qXSyjankCMO3n1$DTSj(198),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Lucifer",
        name: $qXSyjankCMO3n1$DTSj(291),
        description: $qXSyjankCMO3n1$DTSj(197),
        url: $qXSyjankCMO3n1$DTSj(292),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "UnlockedMW",
        name: $qXSyjankCMO3n1$DTSj(196),
        description: $qXSyjankCMO3n1$DTSj(293),
        url: $qXSyjankCMO3n1$DTSj(195),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "UnlockedMW",
        name: "URL Shorten",
        description: $qXSyjankCMO3n1$DTSj(294),
        url: $qXSyjankCMO3n1$DTSj(194),
        image: g.images.unlocked_app,
        isvip: false,
        isinstalled: true
    }, {
        vendor: "UnlockedMW",
        name: $qXSyjankCMO3n1$DTSj(295),
        description: $qXSyjankCMO3n1$DTSj(193),
        url: $qXSyjankCMO3n1$DTSj(296),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "Lucifer",
        name: "Battle Info",
        description: $qXSyjankCMO3n1$DTSj(192),
        url: $qXSyjankCMO3n1$DTSj(297),
        image: g.images.mmfu_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(191),
        description: $qXSyjankCMO3n1$DTSj(298),
        url: $qXSyjankCMO3n1$DTSj(190),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "GuessX",
        name: "CrateBuyer",
        description: $qXSyjankCMO3n1$DTSj(299),
        url: $qXSyjankCMO3n1$DTSj(189),
        image: g.images.guessx_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: $qXSyjankCMO3n1$DTSj(300),
        description: $qXSyjankCMO3n1$DTSj(188),
        url: $qXSyjankCMO3n1$DTSj(301),
        image: g.images.spock_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "UnlockedMW",
        name: $qXSyjankCMO3n1$DTSj(187),
        description: $qXSyjankCMO3n1$DTSj(302),
        url: $qXSyjankCMO3n1$DTSj(186),
        image: g.images.unlocked_app,
        isvip: true,
        isinstalled: false
    }, {
        vendor: "Spockholms",
        name: "LinkOpener",
        description: $qXSyjankCMO3n1$DTSj(303),
        url: $qXSyjankCMO3n1$DTSj(185),
        image: g.images.unlocked_app,
        isvip: false,
        isinstalled: false
    }, {
        vendor: "Simony",
        name: "OGP",
        description: $qXSyjankCMO3n1$DTSj(304),
        url: $qXSyjankCMO3n1$DTSj(184),
        image: g.images.simony_app,
        isvip: false,
        isinstalled: true
    });
function l() {
  if($($qXSyjankCMO3n1$DTSj(305))) {
    $($qXSyjankCMO3n1$DTSj(305)).remove();
    $($qXSyjankCMO3n1$DTSj(183)).remove();
    $($qXSyjankCMO3n1$DTSj(306)).remove();
    $($qXSyjankCMO3n1$DTSj(182)).remove();
    $($qXSyjankCMO3n1$DTSj(307)).remove();
    $("#zbar").parent().remove()
  }
  var c = document.getElementsByTagName("\x61"),
    d = $qXSyjankCMO3n1$DTSj(181);
  for(var i = 0; i < c.length; i++) {
    if(c[i].innerHTML === "\x69") {
      c[i].setAttribute("title", "\x20");
      c[i].parentNode.setAttribute("title", $qXSyjankCMO3n1$DTSj(308));
      c[i].parentNode.setAttribute("onclick", d);
      c[i].innerHTML = $qXSyjankCMO3n1$DTSj(180) + d + $qXSyjankCMO3n1$DTSj(309);
      c[i].parentNode.parentNode.setAttribute("style", $qXSyjankCMO3n1$DTSj(179));
      i == c.length
    }
  }
  var Q = document.getElementById("lowitemhint");
  Q.removeChild(Q.lastChild);
  Q.removeChild(Q.lastChild);
  Q.style.width = "230px";
  var R = document.createElement("table");
  R.id = $qXSyjankCMO3n1$DTSj(310);
  R.border = 0;
  R.setAttribute("style", $qXSyjankCMO3n1$DTSj(178));
  var S = $qXSyjankCMO3n1$DTSj(311);
  Q.appendChild(R);
  document.getElementById($qXSyjankCMO3n1$DTSj(310)).innerHTML = S;
  document.getElementById($qXSyjankCMO3n1$DTSj(177)).innerHTML = $qXSyjankCMO3n1$DTSj(312)
}

function m() {
  if(g.settings.stamOn == true) {
    g.settings.stamOn = 2;
    document.getElementById("StamOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
    document.getElementById("StamOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(313)
  }
  else {
    if(g.settings.stamOn == 2) {
      g.settings.stamOn = false;
      g.newfighton = false;
      clearInterval(g.fightInterval);
      document.getElementById("StamOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
      document.getElementById("StamOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(313)
    }
    else {
      g.fightInterval = setInterval(Fighter, g.settings.FighterInt);
      g.settings.stamOn = true;
      document.getElementById("StamOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
      document.getElementById("StamOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(313)
    }
  }
  writeSettings()
}

function n() {
  if(g.settings.healOn == true) {
    g.settings.healOn = "red";
    document.getElementById("HealOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
    document.getElementById("HealOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(174)
  }
  else {
    if(g.settings.healOn == "red") {
      g.settings.healOn = false;
      clearInterval(g.healInterval);
      document.getElementById("HealOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
      document.getElementById("HealOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(174)
    }
    else {
      g.healInterval = setInterval(DoAutoHeal, 2000);
      g.settings.healOn = true;
      document.getElementById("HealOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
      document.getElementById("HealOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(315)
    }
  }
  writeSettings()
}

function o() {
  if(g.settings.postOn == true) {
    g.settings.postOn = false;
    document.getElementById("PostOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
    document.getElementById("PostOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.fb_icon + $qXSyjankCMO3n1$DTSj(316)
  }
  else {
    g.settings.postOn = true;
    document.getElementById("PostOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
    document.getElementById("PostOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.fb_icon + $qXSyjankCMO3n1$DTSj(316)
  }
  writeSettings()
}

function p() {
  if(g.settings.jobsOn == true) {
    clearInterval(g.energyInterval);
    g.energyInterval = setInterval(Jobber, 1000);
    g.settings.jobsOn = 2;
    document.getElementById("JobsOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
    document.getElementById("JobsOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(172)
  }
  else {
    if(g.settings.jobsOn == 2) {
      g.NYjobon = false;
      g.settings.jobsOn = false;
      clearInterval(g.energyInterval);
      document.getElementById("JobsOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
      document.getElementById("JobsOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(172)
    }
    else {
      g.energyInterval = setInterval(Jobber, 1000);
      g.settings.jobsOn = true;
      document.getElementById("JobsOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(317));
      document.getElementById("JobsOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(172)
    }
  }
  writeSettings()
}

function q() {
  if(g.evilOn == true) {
    if(g.settings.slayerOn == true) {
      g.settings.slayerOn = false;
      writeSettings();
      g.evilOn = false;
      g.familymodeOn = false;
      g.livelinks = [];
      g.livelinkindex = 0;
      clearInterval(g.evilInterval);
      logEntry($qXSyjankCMO3n1$DTSj(171));
      document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
      document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318)
    }
    else {
      if(document.evaluate($qXSyjankCMO3n1$DTSj(170), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
        g.settings.slayerMode = 0;
        logEntry($qXSyjankCMO3n1$DTSj(319));
        document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
        document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
        g.settings.slayerOn = true;
        writeSettings()
      }
      else {
        if(document.evaluate($qXSyjankCMO3n1$DTSj(169), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
          g.settings.slayerMode = 1;
          logEntry($qXSyjankCMO3n1$DTSj(320));
          document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
          document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
          g.settings.slayerOn = true;
          writeSettings()
        }
        else {
          if(document.evaluate($qXSyjankCMO3n1$DTSj(168), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
            g.settings.slayerMode = 2;
            logEntry($qXSyjankCMO3n1$DTSj(321));
            document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
            document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
            g.settings.slayerOn = true;
            writeSettings()
          }
          else {
            g.evilOn = false;
            g.familymodeOn = false;
            g.livelinks = [];
            g.livelinkindex = 0;
            clearInterval(g.evilInterval);
            logEntry($qXSyjankCMO3n1$DTSj(171));
            document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
            document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318)
          }
        }
      }
    }
  }
  else {
    g.settings.evilCity = $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1];
    if($qXSyjankCMO3n1$DTSj(167).test(document.body.innerHTML)) {
      g.targetid = "p|" + ($qXSyjankCMO3n1$DTSj(322).exec(document.body.innerHTML)[1]);
      g.target_name = $qXSyjankCMO3n1$DTSj(166).exec(document.body.innerHTML)[1];
      g.evilInterval = setInterval(PureEvil, 15000);
      g.evilOn = true;
      logEntry($qXSyjankCMO3n1$DTSj(323));
      document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
      document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
      PureEvil()
    }
    else {
      if(document.evaluate($qXSyjankCMO3n1$DTSj(170), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
        g.evilOn = true;
        logEntry($qXSyjankCMO3n1$DTSj(165));
        document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
        document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
        g.evilInterval = setInterval(Fightlist, 5000);
        Fightlist()
      }
      else {
        if(document.evaluate($qXSyjankCMO3n1$DTSj(169), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
          g.evilOn = true;
          logEntry($qXSyjankCMO3n1$DTSj(324));
          document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
          document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
          g.evilInterval = setInterval(Rivals, 5000);
          Rivals()
        }
        else {
          if(document.evaluate($qXSyjankCMO3n1$DTSj(168), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
            g.evilOn = true;
            logEntry($qXSyjankCMO3n1$DTSj(164));
            document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
            document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
            setTimeout(LetThereBeRobberies, 1000)
          }
          else {
            if(document.evaluate($qXSyjankCMO3n1$DTSj(325), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
              var Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(163));
              if(Q.length > 0 && Q[0].id == "battle_tab") {
                g.evilOn = true;
                document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
                document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
                g.familymodeOn = true;
                var R = document.getElementsByClassName("fl");
                for(i = 1; i < R.length - 1; i++) {
                  var S = decodeID($qXSyjankCMO3n1$DTSj(326).exec(R[i].innerHTML)[1]);
                  if(g.umwavoidlist.isAvoid(S)) {
                    g.familytargets[i - 1] = new familyMember(S, "strong")
                  }
                  else {
                    g.familytargets[i - 1] = new familyMember(S, "weak")
                  }
                }
                g.evilInterval = setInterval(Battle, 5000);
                logEntry($qXSyjankCMO3n1$DTSj(162));
                Battle()
              }
              else {
                g.evilOn = true;
                g.familymodeOn = true;
                var j = 0;
                g.familyindex = 0;
                g.familytargets.length = 0;
                var R = document.getElementsByClassName("name_n_rank");
                for(i = 0; i < R.length; i++) {
                  if(R[i].style.clear == "both") {
                    g.familytargets[j++] = new familyMember(decodeID($qXSyjankCMO3n1$DTSj(326).exec(R[i].innerHTML)[1]), "weak")
                  }
                }
                document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
                document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
                g.evilInterval = setInterval(FightFamily, 5000);
                logEntry($qXSyjankCMO3n1$DTSj(327));
                FightFamily()
              }
            }
            else {
              if(document.getElementById($qXSyjankCMO3n1$DTSj(161)) && $qXSyjankCMO3n1$DTSj(328).test(document.body.innerHTML)) {
                alert($qXSyjankCMO3n1$DTSj(160));
                return
              }
              else {
                if(document.getElementById($qXSyjankCMO3n1$DTSj(161)) && document.getElementsByClassName($qXSyjankCMO3n1$DTSj(329))) {
                  document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
                  document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
                  g.evilOn = true;
                  declare_war(null)
                }
                else {
                  if(document.getElementById($qXSyjankCMO3n1$DTSj(159))) {
                    popupTitle = $qXSyjankCMO3n1$DTSj(330);
                    content = $qXSyjankCMO3n1$DTSj(158) + g.settings.hopperid + $qXSyjankCMO3n1$DTSj(331);
                    height = "280";
                    myPop($qXSyjankCMO3n1$DTSj(157), popupTitle, content, height);
                    document.getElementById("Launch").onclick = hopper
                  }
                }
              }
            }
          }
        }
      }
    }
  }
}

function hopper() {
  setTimeout(function () {
    document.getElementById($qXSyjankCMO3n1$DTSj(332)).removeChild(document.getElementById($qXSyjankCMO3n1$DTSj(157)))
  }, 10);
  g.settings.hopperid = document.getElementById($qXSyjankCMO3n1$DTSj(156)).value;
  writeSettings();
  document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
  document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
  g.evilOn = true;
  loadNextHopper();
  g.evilInterval = setInterval(Hopperlist, 10000)
}

function Hopperlist() {
  if(commonFightCheck()) {
    return
  }
  stickyCity();
  if(g.settings.autoBank) {
    Bank()
  }
  if((document.getElementsByClassName($qXSyjankCMO3n1$DTSj(333)).length > 0) || ($qXSyjankCMO3n1$DTSj(155).test(document.body.innerHTML))) {
    logEntry($qXSyjankCMO3n1$DTSj(334));
    g.livelinkindex++;
    loadNextHopper();
    return
  }
  if(g.settings.autoBank) {
    Bank()
  }
  if($qXSyjankCMO3n1$DTSj(154).test(document.body.innerHTML)) {
    var Q = $qXSyjankCMO3n1$DTSj(335).exec(document.body.innerHTML)[1].replace($qXSyjankCMO3n1$DTSj(153), "\x26");
    g.target_name = $qXSyjankCMO3n1$DTSj(166).exec(document.body.innerHTML)[1];
    g.targetid = g.livelinks[g.livelinkindex];
    document.getElementById("btn_attack_" + g.livelinks[g.livelinkindex].replace("\x7c", "")).click()
  }
  else {
    g.livelinkindex++;
    loadNextHopper()
  }
}

function s() {
  $.getJSON($qXSyjankCMO3n1$DTSj(336) + g.settings.hopperid + "&callback=?", function (Q) {
    var R = eval(Q);
    for(i = 0; i < R.length; i++) {
      if(R[i].mwid != "none") {
        g.livelinks[g.livelinks.length] = R[i].mwid
      }
    }
  })
}

function loadNextHopper() {
  if(g.livelinkindex >= g.livelinks.length) {
    logEntry($qXSyjankCMO3n1$DTSj(152));
    g.livelinkindex = 0;
    g.livelinks = [];
    s();
    setTimeout(loadHopperTarget, 1500)
  }
  else {
    loadHopperTarget()
  }
}

function loadHopperTarget() {
  if(g.settings.stamOn != 2) {
    for(var i = g.livelinkindex; i <= g.livelinks.length; i++) {
      if(!g.umwavoidlist.isAvoid(g.livelinks[i])) {
        break
      }
      g.livelinkindex++
    }
  }
  if(g.livelinkindex < g.livelinks.length) {
    loadProfile(g.livelinks[g.livelinkindex])
  }
  else {
    logEntry($qXSyjankCMO3n1$DTSj(337))
  }
}

function t() {
  var Q = document.getElementById("toolsMenu");
  if(Q) {
    Q.style.display = "block"
  }
}

function u() {
  var Q = document.getElementById("toolsMenu");
  if(Q) {
    Q.style.display = "none"
  }
}

function v() {
  var Q = document.getElementById("toolsMenu");
  if(Q) {
    if(Q.style.display == "none") {
      Q.style.display = "block"
    }
    if(Q.style.display == "block") {
      Q.style.display = "none"
    }
  }
}

function x() {
  if(this.id) {
    document.getElementById(this.id).style.filter = $qXSyjankCMO3n1$DTSj(151);
    document.getElementById(this.id).style.opacity = "\x31"
  }
}

function y() {
  if(this.id) {
    document.getElementById(this.id).style.filter = $qXSyjankCMO3n1$DTSj(338);
    document.getElementById(this.id).style.opacity = "0.6"
  }
}
var z;

function A() {
  var Q = document.getElementById("UMW-log");
  if(Q) {
    if(Q.style.display == "none") {
      document.getElementById("UMW-log").style.display = "block";
      UMW_update_log()
    }
    else {
      if(Q.style.display == "block") {
        document.getElementById("UMW-log").style.display = "none";
        z.destroy()
      }
    }
  }
}

function logEntry(Q) {
  if($qXSyjankCMO3n1$DTSj(150).test(Q) || g.log[g.log.length - 1] != Q) {
    g.log[g.log.length] = Q
  }
  UMW_update_log()
}

function UMW_update_log() {
  var Q = null,
    R = g.log,
    S = "";
  Q = $qXSyjankCMO3n1$DTSj(339);
  for(var i = 0; i < R.length; i++) {
    var T = $qXSyjankCMO3n1$DTSj(149);
    T += $qXSyjankCMO3n1$DTSj(340) + R[i] + $qXSyjankCMO3n1$DTSj(148);
    T += $qXSyjankCMO3n1$DTSj(341);
    Q += T;
    if(!$qXSyjankCMO3n1$DTSj(150).test(R[i])) {
      S += R[i] + "\n"
    }
  }
  Q += $qXSyjankCMO3n1$DTSj(147) + g.images.unlocked_icon + $qXSyjankCMO3n1$DTSj(342);
  document.getElementById("UMW-log").innerHTML = Q;
  ZeroClipboard.setMoviePath($qXSyjankCMO3n1$DTSj(146));
  z = new ZeroClipboard.Client();
  z.setText(S);
  z.setHandCursor(true);
  z.setCSSEffects(true);
  z.addEventListener("complete", function (U, V) {
    document.getElementById("copyablelog").innerHTML = $qXSyjankCMO3n1$DTSj(343)
  });
  z.glue("copyablelog");
  document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseover", x, false);
  document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseout", y, false);
  document.getElementById($qXSyjankCMO3n1$DTSj(344)).addEventListener("click", UMW_clear_log, false)
}

function UMW_clear_log() {
  g.log = new Array();
  UMW_update_log()
}

function B() {
  var Q = document.getElementById("mw_masthead");
  if(Q) {
    var R, S = $qXSyjankCMO3n1$DTSj(144) + g.images.unlocked_icon + $qXSyjankCMO3n1$DTSj(345);
    if(document.getElementById("unlock_log")) {
      document.getElementById($qXSyjankCMO3n1$DTSj(143)).addEventListener("click", A, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseover", x, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseout", y, false);
      document.getElementById("UMW-log").addEventListener("click", A, false)
    }
    else {
      R = document.createElement("div");
      R.id = "unlock_log";
      Q.appendChild(R);
      R.innerHTML = S;
      document.getElementById($qXSyjankCMO3n1$DTSj(143)).addEventListener("click", A, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseover", x, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(145)).addEventListener("mouseout", y, false);
      document.getElementById("UMW-log").addEventListener("click", A, false)
    }
  }
}

function C() {
  var Q = document.getElementById("mw_masthead");
  if(Q) {
    var R, S = $qXSyjankCMO3n1$DTSj(346);
    for(var i = 0; i < k.length; i++) {
      if(k[i].isinstalled) {
        S += $qXSyjankCMO3n1$DTSj(142) + i + $qXSyjankCMO3n1$DTSj(347) + k[i].name + "</div></a>"
      }
    }
    S += $qXSyjankCMO3n1$DTSj(141);
    if(document.getElementById($qXSyjankCMO3n1$DTSj(348))) {
      document.getElementById($qXSyjankCMO3n1$DTSj(348)).innerHTML = S;
      R = document.getElementById($qXSyjankCMO3n1$DTSj(348));
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("click", v, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("mouseover", t, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("mouseout", u, false);
      for(var i = 0; i < document.getElementsByClassName($qXSyjankCMO3n1$DTSj(349)).length; i++) {
        document.getElementsByClassName($qXSyjankCMO3n1$DTSj(349))[i].onclick = loadbm
      }
    }
    else {
      R = document.createElement("div");
      R.id = $qXSyjankCMO3n1$DTSj(348);
      Q.appendChild(R);
      R.innerHTML = S;
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("click", v, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("mouseover", t, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(140)).addEventListener("mouseout", u, false);
      for(var i = 0; i < document.getElementsByClassName($qXSyjankCMO3n1$DTSj(349)).length; i++) {
        document.getElementsByClassName($qXSyjankCMO3n1$DTSj(349))[i].onclick = loadbm
      }
    }
  }
}

function loadbm() {
  var Q = parseInt(this.id);
  bm_url = k[Q].url;
  var a = document.createElement("script");
  a.type = $qXSyjankCMO3n1$DTSj(139);
  a.src = bm_url + "\x3f" + Math.random();
  document.getElementsByTagName("head")[0].appendChild(a)
}

function D() {
  if(g.settings.claimlevel) {
    var Q = $("#user_level").text();
    if(Q != g.currlevel) {
      for(i = 0; i < g.settings.count; i++) {
        do_ajax("", $qXSyjankCMO3n1$DTSj(350) + g.city + "&no_load=1")
      }
      g.currlevel++
    }
  }
  var R = document.getElementById("menubar");
  if(g.settings.removeChat) {
    try {
      document.getElementById("clanChat").parentNode.removeChild(document.getElementById("clanChat"))
    }
    catch(S) {}
  }
  if(R) {
    var T;
    if(document.getElementById($qXSyjankCMO3n1$DTSj(138))) {
      T = document.getElementById($qXSyjankCMO3n1$DTSj(138));
      document.getElementById("BankButtonB").addEventListener("click", Bank, false);
      document.getElementById("PostOnOff").addEventListener("click", o, false);
      document.getElementById("HealOnOff").addEventListener("click", n, false);
      document.getElementById("StamOnOff").addEventListener("click", m, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(351)).addEventListener("click", Settings, false);
      document.getElementById("EvilOnOff").addEventListener("click", q, false)
    }
    else {
      T = document.createElement("div");
      T.id = $qXSyjankCMO3n1$DTSj(138);
      R.appendChild(T);
      var U = document.getElementById($qXSyjankCMO3n1$DTSj(137));
      healbutton_div = document.createElement("div");
      healbutton_div.id = "quickheal";
      U.appendChild(healbutton_div);
      healbutton_div.innerHTML = $qXSyjankCMO3n1$DTSj(352);
      document.getElementById("quickheal").addEventListener("click", HealNY, false);
      var V = $qXSyjankCMO3n1$DTSj(136) + g.images.unlocked_icon + $qXSyjankCMO3n1$DTSj(353) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(135) + g.images.fb_icon + $qXSyjankCMO3n1$DTSj(354);
      T.innerHTML = V;
      document.getElementById("BankButtonB").addEventListener("click", Bank, false);
      document.getElementById("PostOnOff").addEventListener("click", o, false);
      document.getElementById("HealOnOff").addEventListener("click", n, false);
      document.getElementById("StamOnOff").addEventListener("click", m, false);
      document.getElementById("JobsOnOff").addEventListener("click", p, false);
      document.getElementById($qXSyjankCMO3n1$DTSj(351)).addEventListener("click", Settings, false);
      document.getElementById("EvilOnOff").addEventListener("click", q, false);
      document.getElementById("umw_logo").addEventListener("click", toggleBlogFeed, false);
      if(g.settings.stamOn == true) {
        document.getElementById("StamOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
        document.getElementById("StamOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(313)
      }
      else {
        if(g.settings.stamOn == 2) {
          document.getElementById("StamOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
          document.getElementById("StamOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(313)
        }
      }
      if(g.settings.healOn == true) {
        document.getElementById("HealOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
        document.getElementById("HealOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(315)
      }
      else {
        if(g.settings.healOn == "red") {
          document.getElementById("HealOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
          document.getElementById("HealOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(315)
        }
      }
      if(g.settings.postOn == true) {
        document.getElementById("PostOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
        document.getElementById("PostOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.fb_icon + $qXSyjankCMO3n1$DTSj(316)
      }
      if(g.settings.jobsOn == true) {
        document.getElementById("JobsOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(317));
        document.getElementById("JobsOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(172)
      }
      else {
        if(g.settings.jobsOn == 2) {
          document.getElementById("JobsOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
          document.getElementById("JobsOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(172)
        }
      }
      if(g.settings.evilOn == true) {
        document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(314));
        document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318)
      }
      if(g.settings.slayerOn == true) {
        document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(176));
        document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318)
      }
    }
  }
  C();
  B()
}

function E(Q) {
  var a = document.getElementById($qXSyjankCMO3n1$DTSj(134)),
    w, h;
  w = parseInt(a.style.width.replace($qXSyjankCMO3n1$DTSj(355), ""));
  h = parseInt(a.style.height.replace($qXSyjankCMO3n1$DTSj(355), ""));
  if(w < 737) {
    w = w + 12;
    if(w > 737) {
      w = 737
    }
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.maxWidth = w + "px";
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.minWidth = w + "px";
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.width = w + "px"
  }
  if(h < 300) {
    h = h + 6;
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.maxHeight = h + "px";
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.minHeight = h + "px";
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.height = h + "px"
  }
  if(w < 737) {
    setTimeout(E, 1)
  }
}

function toggleBlogFeed() {
  var a = document.getElementById($qXSyjankCMO3n1$DTSj(134));
  if(a.style.display == "block") {
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).setAttribute("style", $qXSyjankCMO3n1$DTSj(133))
  }
  else {
    var Q = 205;
    if(document.getElementById($qXSyjankCMO3n1$DTSj(356))) {
      Q = 242
    }
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).setAttribute("style", $qXSyjankCMO3n1$DTSj(132) + Q + $qXSyjankCMO3n1$DTSj(357));
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).innerHTML = $qXSyjankCMO3n1$DTSj(131);
    var R = 550;
    for(var i = 1; i < 4; i++) {
      setTimeout(function () {
        if(document.getElementById($qXSyjankCMO3n1$DTSj(358))) {
          document.getElementById($qXSyjankCMO3n1$DTSj(358)).innerHTML += ". "
        }
      }, (R * i))
    }
    document.getElementById($qXSyjankCMO3n1$DTSj(134)).style.display = "block";
    E();
    $.ajax({
      type: "GET",
      url: $qXSyjankCMO3n1$DTSj(130),
      timeout: 30000,
      dataType: "jsonp",
      success: function (msg) {
        var S = parseInt(msg.count),
          T = $qXSyjankCMO3n1$DTSj(359);
        for(var i = 0; i < S; i++) {
          msg.posts[i].excerpt = linkify(msg.posts[i].excerpt);
          T += '<a href="' + msg.posts[i].url + $qXSyjankCMO3n1$DTSj(129) + msg.posts[i].title + $qXSyjankCMO3n1$DTSj(360) + msg.posts[i].date + $qXSyjankCMO3n1$DTSj(128) + msg.posts[i].excerpt + $qXSyjankCMO3n1$DTSj(361) + msg.posts[i].url + $qXSyjankCMO3n1$DTSj(127);
          if((i + 1) != S) {
            T += $qXSyjankCMO3n1$DTSj(362)
          }
        }
        document.getElementById($qXSyjankCMO3n1$DTSj(134)).innerHTML = T + "</div>"
      },
      error: function (S, status, T) {
        return
      }
    })
  }
}

function linkify(Q) {
  var R, S, T, U;
  S = $qXSyjankCMO3n1$DTSj(126);
  replacedText = Q.replace(S, $qXSyjankCMO3n1$DTSj(363));
  T = $qXSyjankCMO3n1$DTSj(125);
  replacedText = replacedText.replace(T, $qXSyjankCMO3n1$DTSj(364));
  return replacedText
}

function declare_war(Q) {
  var R = null;
  if(parseInt(Q)) {
    R = Q
  }
  else {
    if(!Q || Q == undefined) {
      var a = Math.floor(Math.random() * 11);
      if(a > 5) {
        b = 9999999
      }
      else {
        b = 999999
      }
      R = Math.floor(Math.random() * b)
    }
  }
  var S = $qXSyjankCMO3n1$DTSj(124) + g.city + "&xw_person=" + $qXSyjankCMO3n1$DTSj(365).exec(g.xw_user)[1] + $qXSyjankCMO3n1$DTSj(123) + R + $qXSyjankCMO3n1$DTSj(366);
  g.declaring_war = true;
  do_ajax("inner_page", S, 1, 1)
}

function Jobber() {
  if(g.NYjobon || g.ILVjobon || g.Bjobon) {
    if(g.settings.pauseOn) {
      if(parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML) < g.settings.stopat) {
        g.NYjobon = false;
        g.ILVjobon = false;
        g.Bjobon = false;
        return
      }
    }
    var Q, R;
    if(g.NYjobon) {
      Q = document.evaluate($qXSyjankCMO3n1$DTSj(367), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
      Q = Q.snapshotItem(0).innerHTML;
      g.city = $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1];
      if(parseInt(document.getElementById("user_energy").innerHTML) < parseInt(($qXSyjankCMO3n1$DTSj(121).exec(Q))[1])) {
        g.NYjobon = false;
        return
      }
      R = $qXSyjankCMO3n1$DTSj(368) + g.city + "&tmp=" + ($qXSyjankCMO3n1$DTSj(120).exec(Q))[1] + "&cb=" + ($qXSyjankCMO3n1$DTSj(369).exec(Q))[1] + "&xw_person=" + ($qXSyjankCMO3n1$DTSj(119).exec(Q))[1] + "&job=" + ($qXSyjankCMO3n1$DTSj(370).exec(Q))[1] + "&tab=" + ($qXSyjankCMO3n1$DTSj(118).exec(Q))[1]
    }
    else {
      if(g.ILVjobon) {
        if(document.getElementsByClassName("buy_prompt").length > 0) {
          g.ILVjobon = false;
          return
        }
        g.city = $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1];
        Q = document.evaluate($qXSyjankCMO3n1$DTSj(371), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
        Q = parseInt(($qXSyjankCMO3n1$DTSj(117).exec(Q.snapshotItem(0).id))[1]);
        if(([4, 6, 8, 13, 16, 25, 28, 31, 39, 42, 46, 49, 52, 59, 60, 61, 70, 74, 79, 82].indexOf(Q) !== -1 && g.city == 6) || ([5, 11, 12, 20, 23, 33, 41, 46, 51, 54, 56, 62, 65, 67, 72, 76, 78].indexOf(Q) !== -1 && g.city == 5)) {
          ExpertMapController.selectNode(Q);
          return MapController.doFightJob(Q, ($qXSyjankCMO3n1$DTSj(372).exec(document.getElementById("fight_list0").onclick))[1], 1, ($qXSyjankCMO3n1$DTSj(116).exec(document.getElementById("fight_list0").onclick))[2])
        }
        else {
          ExpertMapController.selectNode(Q);
          return MapController.panelButtonDoJob(Q)
        }
      }
      else {
        if(g.Bjobon) {
          document.getElementById("btn_dojob_" + g.Bjobid).click();
          return
        }
      }
    }
    var S = new XMLHttpRequest();
    S.open("POST", R, true);
    S.setRequestHeader($qXSyjankCMO3n1$DTSj(373), $qXSyjankCMO3n1$DTSj(115));
    S.setRequestHeader("Accept", "*/*");
    S.setRequestHeader($qXSyjankCMO3n1$DTSj(374), $qXSyjankCMO3n1$DTSj(114));
    S.send($qXSyjankCMO3n1$DTSj(375) + g.xw_user + "&sf_xw_sig=" + local_xw_sig + $qXSyjankCMO3n1$DTSj(113));
    S.onreadystatechange = function () {
      if(this.readyState == 4) {
        response = S.responseText;
        if(($qXSyjankCMO3n1$DTSj(376).test(response))) {
          document.getElementById($qXSyjankCMO3n1$DTSj(112)).innerHTML = ($qXSyjankCMO3n1$DTSj(376).exec(response))[1];
          document.getElementById($qXSyjankCMO3n1$DTSj(377)).innerHTML = ($qXSyjankCMO3n1$DTSj(111).exec(response))[1];
          document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML = ($qXSyjankCMO3n1$DTSj(110).exec(response))[1];
          document.getElementById($qXSyjankCMO3n1$DTSj(379)).innerHTML = ($qXSyjankCMO3n1$DTSj(109).exec(response))[1];
          document.getElementById("level_bar").setAttribute("style", $qXSyjankCMO3n1$DTSj(380) + ($qXSyjankCMO3n1$DTSj(108).exec(response))[1] + "%;");
          document.getElementById("user_stamina").innerHTML = ($qXSyjankCMO3n1$DTSj(381).exec(response))[1]
        }
        else {
          if(($qXSyjankCMO3n1$DTSj(107).test(response))) {
            document.getElementById("inner_page").innerHTML = response;
            document.getElementById("user_energy").innerHTML = ($qXSyjankCMO3n1$DTSj(382).exec(response))[1];
            document.getElementById("level_bar").setAttribute("style", $qXSyjankCMO3n1$DTSj(380) + ($qXSyjankCMO3n1$DTSj(106).exec(response))[1] + "%;");
            document.getElementsByClassName("cur_cash").innerHTML = ($qXSyjankCMO3n1$DTSj(383).exec(response))[1];
            document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML = ($qXSyjankCMO3n1$DTSj(105).exec(response))[1];
            document.getElementById($qXSyjankCMO3n1$DTSj(112)).innerHTML = ($qXSyjankCMO3n1$DTSj(384).exec(response))[1];
            document.getElementById($qXSyjankCMO3n1$DTSj(377)).innerHTML = ($qXSyjankCMO3n1$DTSj(104).exec(response))[1];
            document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML = ($qXSyjankCMO3n1$DTSj(385).exec(response))[1];
            document.getElementById($qXSyjankCMO3n1$DTSj(379)).innerHTML = ($qXSyjankCMO3n1$DTSj(103).exec(response))[1]
          }
        }
      }
    };
  }
}

function F() {
  g.NYjobon = false;
  g.ILVjobon = false;
  g.Bjobon = false;
  logEntry($qXSyjankCMO3n1$DTSj(386));
  return
}

function loadProfile(Q) {
  do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(102) + Q, 1, 1, 0, 0)
}

function commonFightCheck() {
  if(g.fightActionRunning || isFightPopOpen() || (parseInt(document.getElementById("user_stamina").innerHTML) <= 10) || (g.settings.healOn && parseInt(document.getElementById("user_health").innerHTML) < g.settings.healat) || (parseInt(document.getElementById("user_health").innerHTML < 30))) {
    return true
  }
  else {
    return false
  }
}

function stickyCity() {
  g.city = $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1];
  if(g.settings.evilCity != g.city) {
    do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(387) + g.settings.evilCity + $qXSyjankCMO3n1$DTSj(101), 1, 1, 0, 0)
  }
}

function Fightlist() {
  var i;
  if(commonFightCheck()) {
    return
  }
  if((document.getElementsByClassName($qXSyjankCMO3n1$DTSj(333)).length > 0) || (document.evaluate($qXSyjankCMO3n1$DTSj(170), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
    logEntry($qXSyjankCMO3n1$DTSj(388));
    do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(100), 1, 1, 0, 0);
    return
  }
  stickyCity();
  if(g.settings.autoBank) {
    Bank()
  }
  g.fightActionRunning = true;
  clearTimeout(g.fightwatchdogtimout);
  g.fightwatchdogtimout = setTimeout(resetfightaction, 15000);
  var Q = document.getElementsByClassName("action"),
    R = Math.floor(Math.random() * (Q.length - 1)) + 1;
  for(i = Q.length - R; i > 1; i--) {
    var S = Q[i].parentNode.firstElementChild.className;
    if((S != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].firstElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|")))) {
      break
    }
  }
  g.targetid = Q[i].firstElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
  var T;
  try {
    if(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
      T = escape(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
    }
  }
  catch(U) {
    T = 0
  }
  if(g.umwavoidlist.isAvoid(g.targetid)) {
    logEntry($qXSyjankCMO3n1$DTSj(390));
    g.fightActionRunning = false;
    do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(100), 1, 1, 0, 0);
    return
  }
  else {
    if(T && (g.settings.Blacklist_enabled == true && g.settings.Blacklist.isWhitelist(T))) {
      logEntry($qXSyjankCMO3n1$DTSj(98));
      g.fightActionRunning = false;
      do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(100), 1, 1, 0, 0);
      return
    }
  }
  Q[i].firstElementChild.click()
}
Array.prototype.findIndex = function (Q) {
  var R = "";
  for(var i = 0; i < this.length; i++) {
    if(this[i].pid == Q) {
      return i
    }
  }
  return R
};

function Battle() {
  var Q;
  if(commonFightCheck()) {
    return
  }
  stickyCity();
  if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(391)).length > 0) {
    var R = document.getElementsByClassName("fl");
    if(R.length - 1 > g.familytargets.length) {
      for(i = 1; i < R.length; i++) {
        var S = decodeID($qXSyjankCMO3n1$DTSj(326).exec(R[i].innerHTML)[1]);
        if(g.familytargets.findIndex(S) === "") {
          if(g.umwavoidlist.isAvoid(S)) {
            g.familytargets[g.familytargets.length] = new familyMember(S, "strong")
          }
          else {
            g.familytargets[g.familytargets.length] = new familyMember(S, "weak")
          }
        }
      }
    }
    Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(391));
    i = Math.floor(Math.random() * Q.length);
    if(g.familytargets[g.familytargets.findIndex(Q[i].id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))].stat == "strong") {
      for(i = 0; i < Q.length; i++) {
        if(g.familytargets[g.familytargets.findIndex(Q[i].id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))].stat == "weak") {
          break
        }
      }
    }
    if(i >= Q.length) {
      Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(97));
      Q[0].click()
    }
    g.familyindex = g.familytargets.findIndex(Q[i].id.replace($qXSyjankCMO3n1$DTSj(99), "p|"));
    if(g.familytargets[g.familyindex].stat == "strong") {
      Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(97));
      Q[0].click()
    }
    else {
      g.targetid = g.familytargets[g.familyindex].pid;
      Q[i].click()
    }
  }
  else {
    if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(392)).length > 0) {
      Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(392));
      if(!$qXSyjankCMO3n1$DTSj(96).test(Q[0].innerHTML)) {
        return
      }
      for(i = 0; i < 5; i++) {
        Q[0].click()
      }
    }
    else {
      Q = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(97));
      Q[0].click()
    }
  }
}

function Rivals() {
  var i;
  if(commonFightCheck()) {
    return
  }
  stickyCity();
  if((document.getElementsByClassName($qXSyjankCMO3n1$DTSj(333)).length > 0) || (document.evaluate($qXSyjankCMO3n1$DTSj(169), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0)) {
    logEntry($qXSyjankCMO3n1$DTSj(393));
    do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(95), 1, 1, 0, 0);
    return
  }
  if(g.settings.autoBank) {
    Bank()
  }
  g.fightActionRunning = true;
  clearTimeout(g.fightwatchdogtimout);
  g.fightwatchdogtimout = setTimeout(resetfightaction, 15000);
  var Q = document.getElementsByClassName("action");
  if(g.is_vip) {
    for(i = 0; i < Q.length; i++) {
      if($qXSyjankCMO3n1$DTSj(394).test(Q[i].parentNode.firstElementChild.innerHTML) && (g.settings.customRivals[0] == 1)) {
        var j = 10;
        i++;
        while(i < Q.length && $qXSyjankCMO3n1$DTSj(94).test(Q[i].firstElementChild.id)) {
          var R = Q[i].parentNode.firstElementChild.className,
            S = 0;
          try {
            if(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
              S = escape(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
            }
          }
          catch(T) {}
          if((g.settings.customRivals[j] == 1) && (R != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))) && (g.settings.Whitelist_enabled == false || (S && g.settings.Whitelist_enabled == true && g.settings.Whitelist.isWhitelist(S))) && (g.settings.Blacklist_enabled == false || (g.settings.Blacklist_enabled == true && !g.settings.Blacklist.isWhitelist(S)))) {
            if(!g.rivalcycle[i]) {
              g.rivalcycle[i] = true;
              g.targetid = Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
              Q[i].lastElementChild.click();
              return
            }
          }
          j++;
          i++
        }
        i--
      }
      else {
        if($qXSyjankCMO3n1$DTSj(395).test(Q[i].parentNode.firstElementChild.innerHTML) && (g.settings.customRivals[1] == 1)) {
          var j = 20;
          i++;
          while(i < Q.length && $qXSyjankCMO3n1$DTSj(94).test(Q[i].firstElementChild.id)) {
            var R = Q[i].parentNode.firstElementChild.className,
              S = 0;
            try {
              if(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                S = escape(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
              }
            }
            catch(T) {}
            if((g.settings.customRivals[j] == 1) && (R != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))) && (g.settings.Whitelist_enabled == false || (S && g.settings.Whitelist_enabled == true && g.settings.Whitelist.isWhitelist(S))) && (g.settings.Blacklist_enabled == false || (g.settings.Blacklist_enabled == true && !g.settings.Blacklist.isWhitelist(S)))) {
              if(!g.rivalcycle[i]) {
                g.rivalcycle[i] = true;
                g.targetid = Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
                Q[i].lastElementChild.click();
                return
              }
            }
            j++;
            i++
          }
          i--
        }
        else {
          if($qXSyjankCMO3n1$DTSj(93).test(Q[i].parentNode.firstElementChild.innerHTML) && g.settings.customRivals[2] == 1) {
            i++;
            while(i < Q.length && $qXSyjankCMO3n1$DTSj(396).test(Q[i].firstElementChild.id)) {
              var R = Q[i].parentNode.firstElementChild.className,
                S = 0;
              try {
                if(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                  S = escape(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                }
              }
              catch(T) {}
              if((R != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))) && (g.settings.Whitelist_enabled == false || (S && g.settings.Whitelist_enabled == true && g.settings.Whitelist.isWhitelist(S))) && (g.settings.Blacklist_enabled == false || (g.settings.Blacklist_enabled == true && !g.settings.Blacklist.isWhitelist(S)))) {
                if(!g.rivalcycle[i]) {
                  g.rivalcycle[i] = true;
                  g.targetid = Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
                  Q[i].lastElementChild.click();
                  return
                }
              }
              j++;
              i++
            }
            i--
          }
          else {
            if($qXSyjankCMO3n1$DTSj(92).test(Q[i].parentNode.firstElementChild.innerHTML) && g.settings.customRivals[3] == 1) {
              i++;
              while(i < Q.length && $qXSyjankCMO3n1$DTSj(396).test(Q[i].firstElementChild.id)) {
                var R = Q[i].parentNode.firstElementChild.className,
                  S = 0;
                try {
                  if(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].getAttribute("style") == "color:red") {
                    S = escape(Q[i].parentNode.childNodes[1].childNodes[3].childNodes[3].childNodes[0].innerHTML)
                  }
                }
                catch(T) {}
                if((R != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|"))) && (g.settings.Whitelist_enabled == false || (S && g.settings.Whitelist_enabled == true && g.settings.Whitelist.isWhitelist(S))) && (g.settings.Blacklist_enabled == false || (g.settings.Blacklist_enabled == true && !g.settings.Blacklist.isWhitelist(S)))) {
                  if(!g.rivalcycle[i]) {
                    g.rivalcycle[i] = true;
                    g.targetid = Q[i].lastElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
                    Q[i].lastElementChild.click();
                    return
                  }
                }
                j++;
                i++
              }
              i--
            }
          }
        }
      }
    }
    logEntry($qXSyjankCMO3n1$DTSj(390));
    for(var j = 0; j < g.rivalcycle.length; j++) {
      g.rivalcycle[j] = false
    }
    g.fightActionRunning = false;
    do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(95), 1, 1, 0, 0);
    return
  }
  else {
    for(i = 1; i < Q.length; i++) {
      if(!Q[i].firstElementChild) {
        break
      }
    }
    for(; i > 0; i--) {
      if(Q[i].firstElementChild) {
        var R = Q[i].parentNode.firstElementChild.className;
        if((R != $qXSyjankCMO3n1$DTSj(389)) && (g.settings.stamOn == 2 || !g.umwavoidlist.isAvoid(Q[i].firstElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|")))) {
          if(!g.rivalcycle[i]) {
            g.rivalcycle[i] = true;
            break
          }
        }
      }
    }
    if(g.rivalcycle[1] == true) {
      for(var j = 0; j < g.rivalcycle.length; j++) {
        g.rivalcycle[j] = false
      }
    }
    if(i == 0) {
      logEntry($qXSyjankCMO3n1$DTSj(390));
      for(var j = 0; j < g.rivalcycle.length; j++) {
        g.rivalcycle[j] = false
      }
      g.fightActionRunning = false;
      do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(95), 1, 1, 0, 0);
      return
    }
    g.targetid = Q[i].firstElementChild.id.replace($qXSyjankCMO3n1$DTSj(99), "p|");
    Q[i].firstElementChild.click()
  }
}

function G() {
  g.icecheckfail = false
}

function FightFamily() {
  if(commonFightCheck()) {
    return
  }
  stickyCity();
  if(g.settings.autoBank) {
    Bank()
  }
  if((document.getElementsByClassName($qXSyjankCMO3n1$DTSj(333)).length > 0) || ($qXSyjankCMO3n1$DTSj(155).test(document.body.innerHTML)) || g.familytargets[g.familyindex].stat == "strong") {
    g.familyindex++;
    if(g.familyindex >= g.familytargets.length) {
      g.familyindex = 0
    }
    if(g.settings.stamOn != 2) {
      for(i = g.familyindex; i < g.familytargets.length; i++) {
        if(!g.umwavoidlist.isAvoid(g.familytargets[g.familyindex].pid)) {
          break
        }
        g.familyindex++
      }
    }
    loadProfile(g.familytargets[g.familyindex].pid);
    return
  }
  if(g.settings.autoBank) {
    Bank()
  }
  if($qXSyjankCMO3n1$DTSj(154).test(document.body.innerHTML)) {
    var Q = $qXSyjankCMO3n1$DTSj(335).exec(document.body.innerHTML)[1].replace($qXSyjankCMO3n1$DTSj(153), "\x26");
    g.target_name = $qXSyjankCMO3n1$DTSj(166).exec(document.body.innerHTML)[1];
    if(g.icecheckfail) {
      document.getElementById("btn_attack_" + g.familytargets[g.familyindex].pid.replace("\x7c", "")).click()
    }
    else {
      var R = {
        ajax: 1,
        liteload: 1,
        sf_xw_user_id: g.xw_user,
        sf_xw_sig: local_xw_sig
      };
      $.ajax({
        type: "POST",
        url: Q,
        timeout: 30000,
        data: R,
        success: function (msg) {
          if($qXSyjankCMO3n1$DTSj(397).test(msg)) {
            logEntry("Slayer " + g.target_name + " is dead.");
            g.familyindex++;
            if(g.familyindex >= g.familytargets.length) {
              g.familyindex = 0
            }
            if(g.settings.stamOn != 2) {
              for(i = g.familyindex; i < g.familytargets.length; i++) {
                if(!g.umwavoidlist.isAvoid(g.familytargets[g.familyindex].pid)) {
                  break
                }
                g.familyindex++
              }
            }
            loadProfile(g.familytargets[g.familyindex].pid);
            return
          }
          else {
            if($qXSyjankCMO3n1$DTSj(91).test(msg)) {
              g.icecheckfail = true;
              setTimeout(G, 45000);
              logEntry($qXSyjankCMO3n1$DTSj(398));
              return
            }
            else {
              if($qXSyjankCMO3n1$DTSj(154).test(document.body.innerHTML)) {
                g.targetid = g.familytargets[g.familyindex].pid;
                document.getElementById("btn_attack_" + g.familytargets[g.familyindex].pid.replace("\x7c", "")).click()
              }
            }
          }
        },
        error: function (S, status, T) {
          return
        }
      })
    }
  }
  else {
    g.familyindex++;
    if(g.familyindex >= g.familytargets.length) {
      g.familyindex = 0
    }
    if(g.settings.stamOn != 2) {
      for(i = g.familyindex; i < g.familytargets.length; i++) {
        if(!g.umwavoidlist.isAvoid(g.familytargets[g.familyindex].pid)) {
          break
        }
        g.familyindex++
      }
    }
    loadProfile(g.familytargets[g.familyindex].pid)
  }
}

function PureEvil() {
  if(commonFightCheck()) {
    return
  }
  stickyCity();
  if(g.settings.autoBank) {
    Bank()
  }
  if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(333)).length > 0) {
    setTimeout(PureEvil, 1500);
    loadProfile(g.targetid);
    return
  }
  if($qXSyjankCMO3n1$DTSj(154).test(document.body.innerHTML)) {
    var Q = $qXSyjankCMO3n1$DTSj(335).exec(document.body.innerHTML)[1].replace($qXSyjankCMO3n1$DTSj(153), "\x26"),
      R = {
        ajax: 1,
        liteload: 1,
        sf_xw_user_id: g.xw_user,
        sf_xw_sig: local_xw_sig
      };
    $.ajax({
      type: "POST",
      url: Q,
      timeout: 30000,
      data: R,
      success: function (msg) {
        if($qXSyjankCMO3n1$DTSj(397).test(msg)) {
          logEntry("Slayer " + g.target_name + " is dead");
          return
        }
        else {
          if($qXSyjankCMO3n1$DTSj(91).test(msg)) {
            logEntry($qXSyjankCMO3n1$DTSj(398));
            return
          }
          else {
            if($qXSyjankCMO3n1$DTSj(154).test(document.body.innerHTML)) {
              document.getElementById("btn_attack_" + g.targetid.replace("\x7c", "")).click()
            }
          }
        }
      },
      error: function (S, status, T) {
        return
      }
    })
  }
  else {
    setTimeout(PureEvil, 1500);
    loadProfile(g.targetid)
  }
}

function Fighter() {
  var Q, i, R;
  if(g.newfighton) {
    if(g.settings.pauseOn) {
      if(parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML) < g.settings.stopat) {
        g.newfighton = false;
        return
      }
    }
    if(isFightPopOpen()) {
      var S = document.getElementById($qXSyjankCMO3n1$DTSj(90));
      if(!$qXSyjankCMO3n1$DTSj(399).test(S.innerHTML) || g.settings.stamOn == 2) {
        S = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(89));
        if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(400)).length > 0) {
          S = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(400))[1]
        }
        if(document.getElementById($qXSyjankCMO3n1$DTSj(88)).style.display == "block" || document.getElementById($qXSyjankCMO3n1$DTSj(401)).style.display == "block") {
          R = parseInt(document.getElementById("user_health").innerHTML);
          if((g.settings.healOn && R < g.settings.healat) || R < 30) {
            return
          }
          url = S.href.replace("remote/", "");
          g.abortCount++;
          if(g.settings.abortOn && g.abortCount > g.settings.abortAt) {
            g.newfighton = false;
            g.abortCount = 0;
            if(!g.closing) {
              setTimeout(closeFightPop, 2500)
            }
            g.closing = true;
            return
          }
          if(g.settings.burstOn) {
            url = url.replace("click_amt=1", "click_amt=" + g.settings.bursts)
          }
          if(g.settings.FighterInt >= 500) {
            bursts = 2 + g.abortCount / 11
          }
          else {
            bursts = 1
          }
          for(i = 0; i < bursts; i++) {
            var T = new XMLHttpRequest();
            T.open("POST", url, true);
            T.setRequestHeader($qXSyjankCMO3n1$DTSj(373), $qXSyjankCMO3n1$DTSj(115));
            T.setRequestHeader("Accept", "*/*");
            T.setRequestHeader($qXSyjankCMO3n1$DTSj(374), $qXSyjankCMO3n1$DTSj(114));
            T.send($qXSyjankCMO3n1$DTSj(375) + g.xw_user + "&sf_xw_sig=" + local_xw_sig + $qXSyjankCMO3n1$DTSj(113));
            T.onreadystatechange = function () {
              if(this.readyState == 4) {
                response = T.responseText;
                try {
                  var msg = JSON.parse(response)
                }
                catch(V) {
                  return
                }
                if(!msg.fight_result) {
                  return
                }
                document.getElementById("user_health").innerHTML = msg.user_fields.user_health;
                User.health = msg.user_fields.user_health;
                g.CurHealth = msg.user_fields.user_health;
                if(g.settings.abortHeal == true && g.abortCount > 3 && msg.fight_result.defender.current_health_pct >= 100) {
                  g.newfighton = false;
                  g.abortCount = 0;
                  if(!g.closing) {
                    setTimeout(closeFightPop, 2500)
                  }
                  g.closing = true;
                  return
                }
                FightV2.powerAttack(msg, this);
                parseNewFightResults(response);
                var W = ($qXSyjankCMO3n1$DTSj(108).exec(response))[1];
                document.getElementById("level_bar").setAttribute("style", $qXSyjankCMO3n1$DTSj(380) + W + "%;");
                document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML = ($qXSyjankCMO3n1$DTSj(87).exec(response))[1];
                document.getElementById($qXSyjankCMO3n1$DTSj(112)).innerHTML = ($qXSyjankCMO3n1$DTSj(376).exec(response))[1];
                document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML = ($qXSyjankCMO3n1$DTSj(110).exec(response))[1];
                document.getElementById($qXSyjankCMO3n1$DTSj(379)).innerHTML = ($qXSyjankCMO3n1$DTSj(109).exec(response))[1];
                document.getElementById($qXSyjankCMO3n1$DTSj(377)).innerHTML = "Ã‚Â£" + ($qXSyjankCMO3n1$DTSj(402).exec(response))[1];
                document.getElementById($qXSyjankCMO3n1$DTSj(86)).innerHTML = "Ã‚Â¢" + ($qXSyjankCMO3n1$DTSj(403).exec(response))[1];
                document.getElementById("user_stamina").innerHTML = ($qXSyjankCMO3n1$DTSj(85).exec(response))[1]
              }
            };
            return
          }
        }
      }
      g.newfighton = false;
      g.abortCount = 0;
      if(g.settings.umwicesteal) {
        if(document.getElementById($qXSyjankCMO3n1$DTSj(404)).style.display == "block") {
          if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(84)).length > 0) {
            document.getElementsByClassName($qXSyjankCMO3n1$DTSj(84))[0].click();
            return
          }
        }
      }
      if(!g.closing) {
        setTimeout(closeFightPop, 2500)
      }
      g.closing = true
    }
  }
  if(g.hitlist) {
    if(g.settings.pauseOn) {
      if(parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML) < g.settings.stopat) {
        g.hitlist = false;
        return
      }
    }
    element = document.evaluate($qXSyjankCMO3n1$DTSj(405), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    if(element.snapshotLength > 0) {
      element = element.snapshotItem(0).href;
      var U = document.getElementById("content_row").innerHTML;
      if(!($qXSyjankCMO3n1$DTSj(83).test(U))) {
        url = $qXSyjankCMO3n1$DTSj(406) + ($qXSyjankCMO3n1$DTSj(82).exec(element))[0]
      }
      else {
        g.hitlist = false;
        return
      }
    }
    R = parseInt(document.getElementById("user_health").innerHTML);
    if((g.settings.healOn && R < g.settings.healat) || R < 30) {
      return
    }
    var T = new XMLHttpRequest();
    T.open("POST", url, true);
    T.setRequestHeader($qXSyjankCMO3n1$DTSj(373), $qXSyjankCMO3n1$DTSj(115));
    T.setRequestHeader("Accept", "*/*");
    T.setRequestHeader($qXSyjankCMO3n1$DTSj(374), $qXSyjankCMO3n1$DTSj(114));
    T.send($qXSyjankCMO3n1$DTSj(375) + g.xw_user + "&sf_xw_sig=" + local_xw_sig + $qXSyjankCMO3n1$DTSj(113));
    T.onreadystatechange = function () {
      if(this.readyState == 4) {
        response = T.responseText;
        document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML = ($qXSyjankCMO3n1$DTSj(407).exec(response))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(112)).innerHTML = ($qXSyjankCMO3n1$DTSj(384).exec(response))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML = ($qXSyjankCMO3n1$DTSj(385).exec(response))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(379)).innerHTML = ($qXSyjankCMO3n1$DTSj(103).exec(response))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(377)).innerHTML = "Ã‚Â£" + ($qXSyjankCMO3n1$DTSj(81).exec(response))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(86)).innerHTML = "Ã‚Â¢" + ($qXSyjankCMO3n1$DTSj(408).exec(response))[1];
        document.getElementById("user_stamina").innerHTML = ($qXSyjankCMO3n1$DTSj(80).exec(response))[1];
        document.getElementById("inner_page").innerHTML = response
      }
    };
  }
}

function parseNewFightResults(Q) {
  var R = document.getElementById($qXSyjankCMO3n1$DTSj(90));
  if(!R) {
    return
  }
  var S = $qXSyjankCMO3n1$DTSj(399).test(R.innerHTML);
  if(S) {
    if(!g.umwavoidlist.isAvoid(g.targetid)) {
      g.umwavoidlist[g.umwavoidlist.length] = g.targetid;
      localStorage.setItem($qXSyjankCMO3n1$DTSj(409), JSON.stringify(g.umwavoidlist))
    }
  }
  if(g.familymodeOn && S) {
    g.familytargets[g.familyindex].stat = "strong"
  }
  if(!S || (g.settings.stamOn == 2)) {
    if(!g.newfighton) {
      g.newfighton = true
    }
  }
  else {
    g.newfighton = false;
    g.abortCount = 0;
    if(g.settings.stamOn && isFightPopOpen()) {
      if(g.settings.umwicesteal) {
        if(document.getElementById($qXSyjankCMO3n1$DTSj(404)).style.display == "block") {
          if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(84)).length > 0) {
            document.getElementsByClassName($qXSyjankCMO3n1$DTSj(84))[0].click();
            return
          }
        }
      }
      setTimeout(closeFightPop, 2500)
    }
  }
  if(document.getElementById($qXSyjankCMO3n1$DTSj(79)).style.display == "block") {
    if(($qXSyjankCMO3n1$DTSj(410).test(Q))) {
      document.getElementById($qXSyjankCMO3n1$DTSj(78)).innerHTML = ++g.icecount;
      document.getElementById($qXSyjankCMO3n1$DTSj(411)).innerHTML = g.killcount;
      document.getElementById($qXSyjankCMO3n1$DTSj(77)).innerHTML = g.icecount + g.killcount * 2;
      publishIce(Q)
    }
  }
  if(document.getElementById($qXSyjankCMO3n1$DTSj(412)).style.display == "block") {
    if(($qXSyjankCMO3n1$DTSj(410).test(Q))) {
      document.getElementById($qXSyjankCMO3n1$DTSj(78)).innerHTML = g.icecount;
      document.getElementById($qXSyjankCMO3n1$DTSj(411)).innerHTML = ++g.killcount;
      document.getElementById($qXSyjankCMO3n1$DTSj(77)).innerHTML = g.icecount + g.killcount * 2;
      publishIce(Q)
    }
  }
}

function publishIce(Q) {
  var R;
  if($qXSyjankCMO3n1$DTSj(76).test(Q)) {
    try {
      R = $qXSyjankCMO3n1$DTSj(413).exec(Q)[0]
    }
    catch(S) {
      return
    }
  }
  else {
    try {
      var msg = JSON.parse(Q);
      R = msg.fight_result.feed_js
    }
    catch(S) {
      return
    }
  }
  var T = $qXSyjankCMO3n1$DTSj(75).exec(R)[1];
  T = "\x28" + T + "\x29";
  var U = eval(T),
    V = U.description.replace($qXSyjankCMO3n1$DTSj(414), "");
  V = V.replace($qXSyjankCMO3n1$DTSj(74), "");
  V = V.replace($qXSyjankCMO3n1$DTSj(415), "");
  V = V.replace($qXSyjankCMO3n1$DTSj(73), "");
  logEntry(V.replace($qXSyjankCMO3n1$DTSj(416), ""));
  if(g.settings.fbfeedloc != "") {
    U.targetId = g.settings.fbfeedloc
  }
  if(g.settings.autopostOn) {
    U.actionLinks[0].name = U.actionLinks[0].name.replace("Get a boost", "Get a Boost");
    U.actionLinks[0].name = U.actionLinks[0].name.replace($qXSyjankCMO3n1$DTSj(72), "Get a Boost")
  }
  if(g.settings.postOn) {
    MW.Feed(U)
  }
}

function Bank() {
  g.city = $qXSyjankCMO3n1$DTSj(238).exec(document.getElementById($qXSyjankCMO3n1$DTSj(251)).className)[1];
  if(g.city == 5) {
    vegascash = $qXSyjankCMO3n1$DTSj(417).exec(document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML)[1];
    vegascash = parseInt(vegascash.replace($qXSyjankCMO3n1$DTSj(71), ""));
    url = $qXSyjankCMO3n1$DTSj(418) + vegascash + $qXSyjankCMO3n1$DTSj(70)
  }
  else {
    url = $qXSyjankCMO3n1$DTSj(419) + g.city + $qXSyjankCMO3n1$DTSj(69)
  }
  var Q = {
    ajax: 1,
    liteload: 1,
    sf_xw_user_id: g.xw_user,
    sf_xw_sig: local_xw_sig
  };
  $.ajax({
    type: "POST",
    url: url,
    timeout: 30000,
    data: Q,
    success: function (R) {
      if(($qXSyjankCMO3n1$DTSj(376).test(R))) {
        document.getElementById($qXSyjankCMO3n1$DTSj(112)).innerHTML = ($qXSyjankCMO3n1$DTSj(376).exec(R))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(377)).innerHTML = "Ã‚Â£" + ($qXSyjankCMO3n1$DTSj(402).exec(R))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(378)).innerHTML = ($qXSyjankCMO3n1$DTSj(110).exec(R))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(379)).innerHTML = ($qXSyjankCMO3n1$DTSj(109).exec(R))[1];
        document.getElementById($qXSyjankCMO3n1$DTSj(86)).innerHTML = "Ã‚Â¢" + ($qXSyjankCMO3n1$DTSj(403).exec(R))[1]
      }
    }
  })
}

function DoAutoHeal() {
  var Q = $qXSyjankCMO3n1$DTSj(420),
    R = parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(68)).innerHTML);
  g.CurHealth = parseInt(document.getElementById("user_health").innerHTML);
  var S = new XMLHttpRequest();
  S.open("POST", Q, true);
  S.setRequestHeader($qXSyjankCMO3n1$DTSj(373), $qXSyjankCMO3n1$DTSj(115));
  S.setRequestHeader("Accept", "*/*");
  S.setRequestHeader($qXSyjankCMO3n1$DTSj(374), $qXSyjankCMO3n1$DTSj(114));
  S.send($qXSyjankCMO3n1$DTSj(375) + g.xw_user + "&sf_xw_sig=" + local_xw_sig + $qXSyjankCMO3n1$DTSj(113));
  S.onreadystatechange = function () {
    if(this.readyState == 4) {
      response = S.responseText;
      try {
        var b = eval("\x28" + response + "\x29");
        document.getElementById("user_health").innerHTML = b.user_fields.user_health;
        document.getElementById("user_stamina").innerHTML = parseInt(b.user_fields.user_stamina);
        document.getElementById("user_energy").innerHTML = parseInt(b.user_fields.user_energy);
        g.CurHealth = b.user_fields.user_health;
        User.health = b.user_fields.user_health;
        g.user.group_atk = b.fightbar.group_atk;
        g.user.group_def = b.fightbar.group_def;
        if(g.user.firstRun == true) {
          g.user.start_group_atk = b.fightbar.group_atk.replace($qXSyjankCMO3n1$DTSj(421), "");
          g.user.start_group_def = b.fightbar.group_def.replace($qXSyjankCMO3n1$DTSj(421), "");
          g.user.firstRun = false
        }
        update_user_scores()
      }
      catch(T) {}
    }
  };
  if(parseInt(g.CurHealth) < g.settings.healat && g.settings.healOn == "red" && parseInt(User.stamina) > 5 && (g.evilOn || g.newfighton)) {
    HealNY()
  }
  else {
    if(parseInt(g.CurHealth) < g.settings.healat && g.settings.healOn == true) {
      HealNY()
    }
  }
}

function HealNY() {
  url = $qXSyjankCMO3n1$DTSj(67);
  if(g.settings.healny == true) {
    url += "&xcity=1"
  }
  send = $qXSyjankCMO3n1$DTSj(375) + g.xw_user + "&sf_xw_sig=" + local_xw_sig;
  var Q = new XMLHttpRequest();
  Q.open("POST", url, true);
  Q.setRequestHeader($qXSyjankCMO3n1$DTSj(373), $qXSyjankCMO3n1$DTSj(115));
  Q.setRequestHeader($qXSyjankCMO3n1$DTSj(374), $qXSyjankCMO3n1$DTSj(114));
  Q.send(send);
  Q.onreadystatechange = function () {
    if(this.readyState == 4) {
      response = Q.responseText;
      var b = eval("\x28" + response + "\x29");
      try {
        document.getElementById("user_health").innerHTML = b.user_fields.user_health;
        g.CurHealth = b.user_fields.user_health;
        User.health = b.user_fields.user_health;
        g.user.group_atk = b.fightbar.group_atk;
        g.user.group_def = b.fightbar.group_def;
        if(g.user.firstRun == true) {
          g.user.start_group_atk = b.fightbar.group_atk.replace($qXSyjankCMO3n1$DTSj(421), "");
          g.user.start_group_def = b.fightbar.group_def.replace($qXSyjankCMO3n1$DTSj(421), "");
          g.user.firstRun = false
        }
        update_user_scores()
      }
      catch(R) {}
    }
  };
}

function Settings() {
  var Q = ".9.0",
    R = g.settings.fbfeedloc;
  popupTitle = "UnlockedMW";
  if(g.is_vip) {
    popupTitle += " VIP 4"
  }
  else {
    popupTitle += " PRO 3"
  }
  popupTitle += Q;
  content = $qXSyjankCMO3n1$DTSj(422) + R + $qXSyjankCMO3n1$DTSj(66) + g.settings.bursts + $qXSyjankCMO3n1$DTSj(423) + g.settings.stopat + $qXSyjankCMO3n1$DTSj(65) + g.settings.abortAt + $qXSyjankCMO3n1$DTSj(424) + g.settings.healat + $qXSyjankCMO3n1$DTSj(64) + g.settings.rref + $qXSyjankCMO3n1$DTSj(425);
  if(g.is_vip) {
    content += $qXSyjankCMO3n1$DTSj(63) + g.settings.FighterInt + $qXSyjankCMO3n1$DTSj(426)
  }
  content += $qXSyjankCMO3n1$DTSj(62);
  height = "760";
  myPop($qXSyjankCMO3n1$DTSj(427), popupTitle, content, height);
  document.getElementById($qXSyjankCMO3n1$DTSj(61)).onclick = saveSettings;
  document.getElementById("CustomBM").onclick = display_appstore;
  document.getElementById("ForceUpDate").onclick = forceupDate;
  if(g.settings.umwicesteal) {
    document.getElementById($qXSyjankCMO3n1$DTSj(428)).checked = true
  }
  if(g.settings.burstOn) {
    document.getElementById("burst").checked = true
  }
  if(g.settings.pauseOn) {
    document.getElementById("pausecheck").checked = true
  }
  if(g.settings.abortOn) {
    document.getElementById("stopcheck").checked = true
  }
  if(g.settings.autopostOn) {
    document.getElementById("autopost").checked = true
  }
  if(g.settings.autoBank) {
    document.getElementById("autobank").checked = true
  }
  if(g.settings.removeChat) {
    document.getElementById("remchat").checked = true
  }
  if(g.settings.userobsquads) {
    document.getElementById("robsquad").checked = true
  }
  if(g.settings.healny) {
    document.getElementById("healny").checked = true
  }
  if(g.settings.abortHeal) {
    document.getElementById("healthy").checked = true
  }
  if(g.settings.claimlevel) {
    document.getElementById("claimlevel").checked = true
  }
  if($qXSyjankCMO3n1$DTSj(60).test(g.settings.Whitelist_enabled)) {
    document.getElementById($qXSyjankCMO3n1$DTSj(429)).checked = true
  }
  else {
    document.getElementById($qXSyjankCMO3n1$DTSj(59)).checked = true
  }
  if($qXSyjankCMO3n1$DTSj(60).test(g.settings.Blacklist_enabled)) {
    document.getElementById($qXSyjankCMO3n1$DTSj(430)).checked = true
  }
  else {
    document.getElementById($qXSyjankCMO3n1$DTSj(58)).checked = true
  }
  if(g.settings.Whitelist) {
    var a = unescape(g.settings.Whitelist[0]);
    for(var i = 1; i < g.settings.Whitelist.length; i++) {
      a += "\n" + unescape(g.settings.Whitelist[i])
    }
    document.getElementById($qXSyjankCMO3n1$DTSj(431)).value = a
  }
  if(g.settings.Blacklist) {
    var a = unescape(g.settings.Blacklist[0]);
    for(var i = 1; i < g.settings.Blacklist.length; i++) {
      a += "\n" + unescape(g.settings.Blacklist[i])
    }
    document.getElementById($qXSyjankCMO3n1$DTSj(57)).value = a
  }
}

function saveSettings() {
  setTimeout(function () {
    document.getElementById($qXSyjankCMO3n1$DTSj(332)).removeChild(document.getElementById($qXSyjankCMO3n1$DTSj(427)))
  }, 10);
  if(g.is_vip) {
    g.settings.FighterInt = document.getElementById("postformid6").value;
    if(document.getElementById($qXSyjankCMO3n1$DTSj(430)).checked) {
      g.settings.Blacklist_enabled = true
    }
    else {
      g.settings.Blacklist_enabled = false
    }
    if(document.getElementById($qXSyjankCMO3n1$DTSj(429)).checked) {
      g.settings.Whitelist_enabled = true
    }
    else {
      g.settings.Whitelist_enabled = false
    }
    if($qXSyjankCMO3n1$DTSj(432).test(document.getElementById($qXSyjankCMO3n1$DTSj(431)).value)) {
      var a = document.getElementById($qXSyjankCMO3n1$DTSj(431)).value.split($qXSyjankCMO3n1$DTSj(56));
      g.settings.Whitelist = new Array();
      for(var i = 0; i < a.length; i++) {
        g.settings.Whitelist[i] = escape(a[i])
      }
    }
    if($qXSyjankCMO3n1$DTSj(432).test(document.getElementById($qXSyjankCMO3n1$DTSj(57)).value)) {
      var a = document.getElementById($qXSyjankCMO3n1$DTSj(57)).value.split($qXSyjankCMO3n1$DTSj(56));
      g.settings.Blacklist = new Array();
      for(var i = 0; i < a.length; i++) {
        g.settings.Blacklist[i] = escape(a[i])
      }
    }
  }
  g.settings.rref = parseInt(document.getElementById("postformid5").value);
  g.settings.fbfeedloc = document.getElementById("postformid").value;
  g.settings.healat = parseInt(document.getElementById("postformid2").value);
  g.settings.stopat = parseInt(document.getElementById("postformid3").value);
  g.settings.abortAt = parseInt(document.getElementById("postformid7").value);
  g.settings.bursts = parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(433)).value);
  if(document.getElementById($qXSyjankCMO3n1$DTSj(428)).checked == true) {
    g.settings.umwicesteal = true
  }
  else {
    g.settings.umwicesteal = false
  }
  if(document.getElementById("autopost").checked == true) {
    g.settings.autopostOn = true
  }
  else {
    g.settings.autopostOn = false
  }
  if(document.getElementById("burst").checked == true) {
    g.settings.burstOn = true
  }
  else {
    g.settings.burstOn = false
  }
  if(document.getElementById("pausecheck").checked == true) {
    g.settings.pauseOn = true
  }
  else {
    g.settings.pauseOn = false
  }
  if(document.getElementById("stopcheck").checked == true) {
    g.settings.abortOn = true
  }
  else {
    g.settings.abortOn = false
  }
  if(document.getElementById("autobank").checked == true) {
    g.settings.autoBank = true
  }
  else {
    g.settings.autoBank = false
  }
  if(document.getElementById("robsquad").checked == true) {
    g.settings.userobsquads = true
  }
  else {
    g.settings.userobsquads = false
  }
  if(document.getElementById("remchat").checked == true) {
    g.settings.removeChat = true
  }
  else {
    g.settings.removeChat = false
  }
  if(document.getElementById("healthy").checked == true) {
    g.settings.abortHeal = true
  }
  else {
    g.settings.abortHeal = false
  }
  if(document.getElementById("healny").checked == true) {
    g.settings.healny = true
  }
  else {
    g.settings.healny = false
  }
  if(document.getElementById("claimlevel").checked == true) {
    g.settings.claimlevel = true
  }
  else {
    g.settings.claimlevel = false
  }
  writeSettings()
}

function forceupDate() {
  if(confirm($qXSyjankCMO3n1$DTSj(55))) {
    localStorage.clear();
    top.location.href = $qXSyjankCMO3n1$DTSj(434)
  }
}

function H(Q) {
  var R = null,
    i;
  for(i = 0; i < Q.length; i++) {
    if(Q[i].checked) {
      R = Q[i].value;
      break
    }
  }
  return R
}

function myPop(Q, R, S, T, U, V) {
  if(!T || T == undefined) {
    T = "600"
  }
  if(!U || U == undefined) {
    U = "330"
  }
  var W = '<div id="' + Q + $qXSyjankCMO3n1$DTSj(54) + (375 - (U / 2)) + $qXSyjankCMO3n1$DTSj(435) + U + "px;height:" + T + $qXSyjankCMO3n1$DTSj(53) + U + $qXSyjankCMO3n1$DTSj(436) + (U - 10) + $qXSyjankCMO3n1$DTSj(52) + R + $qXSyjankCMO3n1$DTSj(437) + S + $qXSyjankCMO3n1$DTSj(51);
  document.getElementById($qXSyjankCMO3n1$DTSj(332)).innerHTML += W;
  if(V) {
    setTimeout(function () {
      document.getElementById($qXSyjankCMO3n1$DTSj(332)).removeChild(document.getElementById(Q))
    }, V)
  }
}

function I() {
  document.getElementById($qXSyjankCMO3n1$DTSj(332)).removeChild(document.getElementById("myPopup"))
}

function LetThereBeRobberies() {
  var Q = parseInt(document.getElementById("user_stamina").innerHTML);
  if(!g.evilOn || g.RobbingHung) {
    return
  }
  if(g.settings.pauseOn) {
    if(parseInt(document.getElementById($qXSyjankCMO3n1$DTSj(122)).innerHTML) < g.settings.stopat) {
      g.evilOn = false;
      logEntry($qXSyjankCMO3n1$DTSj(171));
      document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
      document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
      return
    }
  }
  if(Q < 50) {
    logEntry($qXSyjankCMO3n1$DTSj(438));
    setTimeout(LetThereBeRobberies, 30000)
  }
  else {
    if(document.evaluate($qXSyjankCMO3n1$DTSj(168), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength == 0) {
      TravelRobbing();
      logEntry($qXSyjankCMO3n1$DTSj(50));
      setTimeout(LetThereBeRobberies, 3000)
    }
    else {
      if(parseInt($qXSyjankCMO3n1$DTSj(439).exec(document.getElementById($qXSyjankCMO3n1$DTSj(49)).onclick)[1]) > 0 && g.settings.userobsquads) {
        setTimeout(RefreshMeRS, (6900));
        document.getElementById($qXSyjankCMO3n1$DTSj(49)).click()
      }
      else {
        g.RobbingHung = true;
        g.RHangCHKInterval = setTimeout(RobbingWatchdog, 30000);
        robTheBoard();
        setTimeout(RefreshMe, 6900)
      }
    }
  }
}

function robTheBoard() {
  if(g.RobbingHung) {
    var Q = 8;
    for(var i = 8; i >= 0; i--) {
      setTimeout($qXSyjankCMO3n1$DTSj(440) + Q + $qXSyjankCMO3n1$DTSj(48), i * g.settings.rref);
      Q--
    }
  }
}

function RefreshMeRS() {
  CloseDoopidPopup();
  setTimeout(LetThereBeRobberies, 4000);
  document.getElementById($qXSyjankCMO3n1$DTSj(441)).click()
}

function RobbingWatchdog() {
  clearTimeout(g.RHangCHKInterval);
  g.RobbingHung = false;
  TravelRobbing();
  setTimeout(LetThereBeRobberies, 5000);
  return
}

function TravelRobbing() {
  do_ajax("inner_page", $qXSyjankCMO3n1$DTSj(47) + g.settings.evilCity + $qXSyjankCMO3n1$DTSj(442), 1, 1, 0, 0)
}

function RefreshMe() {
  try {
    var Q = document.evaluate($qXSyjankCMO3n1$DTSj(46), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    if(Q.snapshotLength >= 1) {
      clearTimeout(g.RHangCHKInterval);
      g.RobbingHung = false;
      CloseDoopidPopup();
      setTimeout(LetThereBeRobberies, 4000);
      document.getElementById($qXSyjankCMO3n1$DTSj(441)).click()
    }
    else {
      if(g.RobbingHung) {
        robTheBoard();
        setTimeout(RefreshMe, 5000)
      }
    }
  }
  catch(R) {}
}

function resetfightaction() {
  g.fightActionRunning = false;
  closeFightPop()
}

function display_appstore() {
  try {
    if(document.getElementById($qXSyjankCMO3n1$DTSj(427))) {
      document.getElementById($qXSyjankCMO3n1$DTSj(332)).removeChild(document.getElementById($qXSyjankCMO3n1$DTSj(427)))
    }
  }
  catch(e) {}
  var b = $qXSyjankCMO3n1$DTSj(443),
    d = 500,
    i;
  for(i = 0; i < k.length; i++) {
    if(i % 2 == 0) {
      b += "<tr>"
    }
    else {
      b += $qXSyjankCMO3n1$DTSj(45)
    }
    b += $qXSyjankCMO3n1$DTSj(444) + k[i].image + $qXSyjankCMO3n1$DTSj(44) + k[i].vendor + $qXSyjankCMO3n1$DTSj(445) + (i + 1) + ".&nbsp;" + k[i].name + $qXSyjankCMO3n1$DTSj(43) + k[i].description + $qXSyjankCMO3n1$DTSj(446);
    if(k[i].isinstalled == 2) {
      b += $qXSyjankCMO3n1$DTSj(42) + i + $qXSyjankCMO3n1$DTSj(447) + g.images.autostart + $qXSyjankCMO3n1$DTSj(41)
    }
    else {
      if(k[i].isinstalled == true) {
        b += $qXSyjankCMO3n1$DTSj(42) + i + $qXSyjankCMO3n1$DTSj(447) + g.images.installed + $qXSyjankCMO3n1$DTSj(41)
      }
      else {
        if(k[i].isvip && !g.is_vip) {
          b += $qXSyjankCMO3n1$DTSj(448) + g.images.viponly + $qXSyjankCMO3n1$DTSj(41)
        }
        else {
          b += $qXSyjankCMO3n1$DTSj(40) + i + $qXSyjankCMO3n1$DTSj(447) + g.images.disabled + $qXSyjankCMO3n1$DTSj(41)
        }
      }
    }
  }
  b += $qXSyjankCMO3n1$DTSj(449);
  myPop($qXSyjankCMO3n1$DTSj(39), $qXSyjankCMO3n1$DTSj(450), b, d, "440");
  var Q = document.getElementsByClassName("toggle_app");
  for(var r = 0; r < Q.length; r++) {
    document.getElementsByClassName("toggle_app")[r].onclick = toggle_app
  }
}

function toggle_app() {
  var Q = parseInt(this.id);
  if(k[Q].isvip && !g.is_vip) {
    return
  }
  if(k[Q].isinstalled == 2) {
    k[Q].isinstalled = false;
    document.getElementsByClassName("toggle_app")[Q].innerHTML = '<img src="' + g.images.disabled + '">';
    document.getElementsByClassName("toggle_app")[Q].setAttribute("style", $qXSyjankCMO3n1$DTSj(38))
  }
  else {
    if(k[Q].isinstalled == true) {
      k[Q].isinstalled = 2;
      document.getElementsByClassName("toggle_app")[Q].innerHTML = '<img src="' + g.images.autostart + '">';
      document.getElementsByClassName("toggle_app")[Q].setAttribute("style", $qXSyjankCMO3n1$DTSj(38))
    }
    else {
      k[Q].isinstalled = true;
      document.getElementsByClassName("toggle_app")[Q].innerHTML = '<img src="' + g.images.installed + '">';
      document.getElementsByClassName("toggle_app")[Q].setAttribute("style", $qXSyjankCMO3n1$DTSj(38))
    }
  }
  save_app_settings();
  C()
}

function save_app_settings() {
  var Q = new Array();
  for(var i = 0; i < k.length; i++) {
    Q[i] = k[i].isinstalled
  }
  localStorage.setItem($qXSyjankCMO3n1$DTSj(451), JSON.stringify(Q))
}

function J() {
  var i;
  if(!localStorage.getItem($qXSyjankCMO3n1$DTSj(451))) {
    save_app_settings()
  }
  else {
    var R = JSON.parse(localStorage.getItem($qXSyjankCMO3n1$DTSj(451)));
    for(i = 0; i < R.length; i++) {
      k[i].isinstalled = R[i]
    }
    if(k.length > R.length) {
      save_app_settings()
    }
  }
}

function update_user_scores() {
  if(document.getElementById($qXSyjankCMO3n1$DTSj(310))) {
    var Q = g.user;
    document.getElementById($qXSyjankCMO3n1$DTSj(36)).innerHTML = Q.group_atk;
    document.getElementById($qXSyjankCMO3n1$DTSj(453)).innerHTML = Q.group_def;
    var R = parseInt(parseInt(Q.group_atk.replace($qXSyjankCMO3n1$DTSj(421), "")) - parseInt(Q.start_group_atk.replace($qXSyjankCMO3n1$DTSj(421), ""))),
      S = document.getElementById($qXSyjankCMO3n1$DTSj(35));
    if(R >= 0) {
      S.innerHTML = "\x2b" + R;
      S.className = "good"
    }
    else {
      if(R < 0) {
        S.innerHTML = R;
        S.className = "bad"
      }
    }
    var T = parseInt(parseInt(Q.group_def.replace($qXSyjankCMO3n1$DTSj(421), "")) - parseInt(Q.start_group_def.replace($qXSyjankCMO3n1$DTSj(421), ""))),
      U = document.getElementById($qXSyjankCMO3n1$DTSj(454));
    if(T >= 0) {
      U.innerHTML = "\x2b" + T;
      U.className = "good"
    }
    else {
      if(T < 0) {
        U.innerHTML = T;
        U.className = "bad"
      }
    }
  }
}

function K() {
  var Q = this.id;
  rivalindex = $qXSyjankCMO3n1$DTSj(34).exec(Q)[1];
  if(g.settings.customRivals[rivalindex] == 1) {
    document.getElementById(Q).setAttribute("class", $qXSyjankCMO3n1$DTSj(455));
    document.getElementById(Q).innerHTML = $qXSyjankCMO3n1$DTSj(33) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
    g.settings.customRivals[rivalindex] = 0;
    writeSettings()
  }
  else {
    document.getElementById(Q).setAttribute("class", $qXSyjankCMO3n1$DTSj(456));
    document.getElementById(Q).innerHTML = $qXSyjankCMO3n1$DTSj(33) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
    g.settings.customRivals[rivalindex] = 1;
    writeSettings()
  }
}

function L() {
  var Q = this.id;
  rivalindex = $qXSyjankCMO3n1$DTSj(32).exec(Q)[1];
  if(g.settings.customRivals[rivalindex] == 1) {
    document.getElementById(Q).setAttribute("class", $qXSyjankCMO3n1$DTSj(455));
    document.getElementById(Q).innerHTML = $qXSyjankCMO3n1$DTSj(457);
    g.settings.customRivals[rivalindex] = 0;
    writeSettings()
  }
  else {
    document.getElementById(Q).setAttribute("class", $qXSyjankCMO3n1$DTSj(456));
    document.getElementById(Q).innerHTML = $qXSyjankCMO3n1$DTSj(457);
    g.settings.customRivals[rivalindex] = 1;
    writeSettings()
  }
}

function M() {
  var Q = document.getElementsByClassName("action");
  for(i = 0; i < Q.length; i++) {
    if(!Q[i].firstElementChild) {
      if($qXSyjankCMO3n1$DTSj(394).test(Q[i].parentNode.firstElementChild.innerHTML)) {
        var j = 10;
        if(g.settings.customRivals[0] == 1) {
          Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(31) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
        }
        else {
          Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(30) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
        }
        document.getElementById($qXSyjankCMO3n1$DTSj(459)).onclick = K;
        i++;
        while(i < Q.length && Q[i].firstElementChild) {
          Q[i].setAttribute("style", "width:200px");
          if(g.settings.customRivals[j] == 1) {
            Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(29) + j + $qXSyjankCMO3n1$DTSj(460) + Q[i].innerHTML
          }
          else {
            Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(28) + j + $qXSyjankCMO3n1$DTSj(460) + Q[i].innerHTML
          }
          document.getElementById($qXSyjankCMO3n1$DTSj(461) + j).onclick = L;
          j++;
          i++
        }
        i--
      }
      else {
        if($qXSyjankCMO3n1$DTSj(395).test(Q[i].parentNode.firstElementChild.innerHTML)) {
          var j = 20;
          if(g.settings.customRivals[1] == 1) {
            Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(27) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
          }
          else {
            Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(462) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
          }
          document.getElementById($qXSyjankCMO3n1$DTSj(26)).onclick = K;
          i++;
          while(i < Q.length && Q[i].firstElementChild) {
            Q[i].setAttribute("style", "width:200px");
            if(g.settings.customRivals[j] == 1) {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(29) + j + $qXSyjankCMO3n1$DTSj(460) + Q[i].innerHTML
            }
            else {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(28) + j + $qXSyjankCMO3n1$DTSj(460) + Q[i].innerHTML
            }
            document.getElementById($qXSyjankCMO3n1$DTSj(461) + j).onclick = L;
            j++;
            i++
          }
          i--
        }
        else {
          if($qXSyjankCMO3n1$DTSj(93).test(Q[i].parentNode.firstElementChild.innerHTML)) {
            if(g.settings.customRivals[2] == 1) {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(463) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
            }
            else {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(25) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
            }
            document.getElementById($qXSyjankCMO3n1$DTSj(464)).onclick = K
          }
          else {
            if(g.settings.customRivals[3] == 1) {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(24) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
            }
            else {
              Q[i].innerHTML = $qXSyjankCMO3n1$DTSj(465) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(458)
            }
            document.getElementById($qXSyjankCMO3n1$DTSj(23)).onclick = K
          }
        }
      }
    }
  }
}

function N() {
  var Q = $qXSyjankCMO3n1$DTSj(466),
    R = $qXSyjankCMO3n1$DTSj(22),
    S = $qXSyjankCMO3n1$DTSj(467);
  document.getElementById($qXSyjankCMO3n1$DTSj(21)).innerHTML += Q;
  document.getElementById($qXSyjankCMO3n1$DTSj(468)).innerHTML += R;
  document.getElementById($qXSyjankCMO3n1$DTSj(20)).innerHTML += S;
  var T = document.getElementsByClassName($qXSyjankCMO3n1$DTSj(469));
  for(var i = 0; i < T.length; i++) {
    document.getElementsByClassName($qXSyjankCMO3n1$DTSj(469))[i].onclick = ActivateCrewmate
  }
}

function ActivateCrewmate() {
  var Q = parseInt(this.getAttribute("city"));
  if(Q) {
    var R;
    if($qXSyjankCMO3n1$DTSj(19).test(window.location.href)) {
      R = "https"
    }
    else {
      R = "http"
    }
    var S = ["Brazil", "Chicago", "London"],
      T = "",
      U = R + $qXSyjankCMO3n1$DTSj(470) + Q + $qXSyjankCMO3n1$DTSj(18);
    $.ajax({
      type: "POST",
      url: U,
      timeout: 30000,
      data: T,
      success: function (msg) {
        var V = JSON.parse(msg),
          W;
        if(V.data.crewstatus == "ok") {
          W = "activated !"
        }
        else {
          if(V.data.crewmsg) {
            W = $qXSyjankCMO3n1$DTSj(471)
          }
        }
        if(W.length > 0) {
          var X = document.createElement("div");
          X.id = $qXSyjankCMO3n1$DTSj(17);
          X.setAttribute("class", "pop_box");
          X.setAttribute("style", $qXSyjankCMO3n1$DTSj(472));
          X.innerHTML = $qXSyjankCMO3n1$DTSj(16) + R + $qXSyjankCMO3n1$DTSj(473) + S[(Q - 7)] + $qXSyjankCMO3n1$DTSj(15) + W + $qXSyjankCMO3n1$DTSj(474);
          document.getElementById($qXSyjankCMO3n1$DTSj(332)).appendChild(X)
        }
      },
      error: function (V, status, W) {
        return
      }
    })
  }
}
$("body").ajaxComplete(function (e, Q, R) {
  if(g.is_vip) {
    if(document.evaluate($qXSyjankCMO3n1$DTSj(169), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
      M()
    }
  }
  var S = Q.responseText;
  if($qXSyjankCMO3n1$DTSj(14).test(S)) {
    local_xw_sig = $qXSyjankCMO3n1$DTSj(14).exec(S)[1]
  }
  if(g.ajax) {
    g.ajax = false;
    return
  }
  if(g.fightActionRunning) {
    g.fightActionRunning = false;
    clearTimeout(g.fightwatchdogtimout)
  }
  if(document.getElementById($qXSyjankCMO3n1$DTSj(90))) {
    parseNewFightResults(S)
  }
  if(document.evaluate($qXSyjankCMO3n1$DTSj(405), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
    g.hitlist = true
  }
  else {
    if(g.hitlist) {
      g.hitlist = false
    }
  }
  var T = 0;
  if(g.settings.jobsOn != false) {
    if(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(475)).length > 0) {
      document.getElementsByClassName($qXSyjankCMO3n1$DTSj(13))[0].click()
    }
    if(document.evaluate($qXSyjankCMO3n1$DTSj(476), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0) {
      g.NYjobon = true
    }
    else {
      if(g.NYjobon) {
        g.NYjobon = false
      }
    }
    if(document.evaluate($qXSyjankCMO3n1$DTSj(371), document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0 && document.getElementsByClassName("buy_prompt").length == 0) {
      g.ILVjobon = true
    }
    else {
      if(g.ILVjobon) {
        g.ILVjobon = false
      }
    }
    var U = S.replace($qXSyjankCMO3n1$DTSj(12), "");
    if($qXSyjankCMO3n1$DTSj(477).test(U)) {
      T = $qXSyjankCMO3n1$DTSj(11).exec(U)[1];
      if(T == 100 && g.settings.jobsOn != 2) {
        F()
      }
    }
    if($qXSyjankCMO3n1$DTSj(478).test(S) || $qXSyjankCMO3n1$DTSj(10).test(S) || $qXSyjankCMO3n1$DTSj(479).test(S)) {
      if($qXSyjankCMO3n1$DTSj(478).test(S)) {
        g.Bjobid = $qXSyjankCMO3n1$DTSj(9).exec(S)[1]
      }
      if($qXSyjankCMO3n1$DTSj(10).test(S)) {
        g.Bjobid = $qXSyjankCMO3n1$DTSj(480).exec(S)[1]
      }
      if($qXSyjankCMO3n1$DTSj(479).test(S)) {
        g.Bjobid = $qXSyjankCMO3n1$DTSj(8).exec(S)[1]
      }
      g.Bjobon = true
    }
    else {
      if(g.Bjobon) {
        g.Bjobon = false
      }
    }
    if($qXSyjankCMO3n1$DTSj(481).test(S)) {
      T = $qXSyjankCMO3n1$DTSj(7).exec(S)[1];
      if(T == 100 && g.settings.jobsOn != 2) {
        F()
      }
    }
  }
  if(g.declaring_war == true && $qXSyjankCMO3n1$DTSj(482).test(S)) {
    if($qXSyjankCMO3n1$DTSj(6).test(S)) {
      setTimeout(function () {
        declare_war(null)
      }, 0)
    }
    else {
      if($qXSyjankCMO3n1$DTSj(483).test(S)) {
        setTimeout(function () {
          fP(null)
        }, 0)
      }
      else {
        if($qXSyjankCMO3n1$DTSj(5).test(S)) {
          g.declaring_war = false
        }
        else {
          if($qXSyjankCMO3n1$DTSj(484).test(S) || $qXSyjankCMO3n1$DTSj(4).test(S)) {
            setTimeout(function () {
              declare_war(null)
            }, 0)
          }
          else {
            setTimeout(function () {
              var V = $qXSyjankCMO3n1$DTSj(365).exec(document.getElementsByClassName($qXSyjankCMO3n1$DTSj(485))[0].getAttribute("onclick"))[1];
              MW.Feed(postFeedAndSendCallForHelpNew);
              $($qXSyjankCMO3n1$DTSj(3)).css("display", "none");
              MW.Popup.hideAndRemove(V);
              g.declaring_war = false
            }, 1500);
            document.getElementById("EvilOnOff").setAttribute("class", $qXSyjankCMO3n1$DTSj(175));
            document.getElementById("EvilOnOff").innerHTML = $qXSyjankCMO3n1$DTSj(173) + g.images.evil_icon + $qXSyjankCMO3n1$DTSj(318);
            g.evilOn = false
          }
        }
      }
    }
    return
  }
  if(($qXSyjankCMO3n1$DTSj(486).test(S))) {
    var b = eval("\x28" + S + "\x29");
    g.user.group_atk = b.fightbar.group_atk;
    g.user.group_def = b.fightbar.group_def;
    if(g.user.firstRun == true) {
      g.user.start_group_atk = b.fightbar.group_atk.replace($qXSyjankCMO3n1$DTSj(421), "");
      g.user.start_group_def = b.fightbar.group_def.replace($qXSyjankCMO3n1$DTSj(421), "");
      g.user.firstRun = false
    }
    update_user_scores()
  }
  if($qXSyjankCMO3n1$DTSj(2).test(S)) {
    N()
  }
});

function O() {
  l();
  J();
  if(!localStorage.getItem($qXSyjankCMO3n1$DTSj(409))) {
    g.umwavoidlist[0] = "p|45293801";
    localStorage.setItem($qXSyjankCMO3n1$DTSj(409), JSON.stringify(g.umwavoidlist))
  }
  else {
    g.umwavoidlist = JSON.parse(localStorage.getItem($qXSyjankCMO3n1$DTSj(409)))
  }
  readSettings();
  D();
  if(g.settings.stamOn == true || g.settings.stamOn == 2) {
    g.fightInterval = setInterval(Fighter, g.settings.FighterInt)
  }
  if((g.settings.healOn == true) || (g.settings.healOn == "red")) {
    g.healInterval = setInterval(DoAutoHeal, 2000)
  }
  if(g.settings.jobsOn == true || g.settings.jobsOn == 2) {
    g.energyInterval = setInterval(Jobber, 1000)
  }
  if(g.settings.slayerOn == true) {
    if(g.settings.slayerMode == 0) {
      g.evilInterval = setInterval(Fightlist, 5000);
      g.evilOn = true
    }
    else {
      if(g.settings.slayerMode == 1) {
        g.evilInterval = setInterval(Rivals, 5000);
        g.evilOn = true
      }
      else {
        if(g.settings.slayerMode == 2) {
          setTimeout(LetThereBeRobberies, 3000);
          g.evilOn = true
        }
      }
    }
  }
  setInterval(D, 10000);
  setTimeout(KillStupidPop, 10000);
  for(var i = 0; i < k.length; i++) {
    if(k[i].isinstalled == 2) {
      var Q = k[i].url,
        a = document.createElement("script");
      a.type = $qXSyjankCMO3n1$DTSj(139);
      a.src = Q + "\x3f" + Math.random();
      document.getElementsByTagName("head")[0].appendChild(a)
    }
  }
  setTimeout(N, 500)
}
if(document.getElementById($qXSyjankCMO3n1$DTSj(138))) {
  return
}
else {
  O()
}

function KillStupidPop() {
  if($($qXSyjankCMO3n1$DTSj(487)).css("display") == "block") {
    $($qXSyjankCMO3n1$DTSj(487)).css("display", "none")
  }
}

    function writeSettings() {
        var Q = JSON.parse(localStorage.getItem("scriptumw"));
        if (1) {
            localStorage.setItem($qXSyjankCMO3n1$DTSj(1), JSON.stringify(g.settings))
            } else {
            g.settings.healat = 0;
            g.settings.FighterInt = 3000
        }
    }
    function readSettings() {
        if (!localStorage.getItem($qXSyjankCMO3n1$DTSj(1)) || !localStorage.getItem("scriptumw")) {
            writeSettings()
            } else {
            tempsettings = JSON.parse(localStorage.getItem($qXSyjankCMO3n1$DTSj(1)));
            if (Object.keys(tempsettings).length != Object.keys(g.settings).length) {
                writeSettings()
                } else {
                g.settings = tempsettings
            }
        }
    }

function P(Q, R) {
  R = R || document;
  return document.evaluate(Q, R, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
}
HTMLElement.prototype.click = function () {
  $(this).click()
};

function familyMember(Q, R) {
  this.pid = Q;
  this.stat = R
}

function decodeID(Q) {
  Q = unescape(Q);
  if(isNaN(Q)) {
    Q = decode64(Q)
  }
  return Q
}

function decode64(Q) {
  var R = "",
    S, T, U = "",
    V, W, X, Y = "",
    i = 0,
    Z = $qXSyjankCMO3n1$DTSj(488);
  if(Z.exec(Q)) {}
  Q = Q.replace($qXSyjankCMO3n1$DTSj(488), "");
  do {
    V = g.keyStr.indexOf(Q.charAt(i++));
    W = g.keyStr.indexOf(Q.charAt(i++));
    X = g.keyStr.indexOf(Q.charAt(i++));
    Y = g.keyStr.indexOf(Q.charAt(i++));
    S = (V << 2) | (W >> 4);
    T = ((W & 15) << 4) | (X >> 2);
    U = ((X & 3) << 6) | Y;
    R = R + String.fromCharCode(S);
    if(X != 64) {
      R = R + String.fromCharCode(T)
    }
    if(Y != 64) {
      R = R + String.fromCharCode(U)
    }
    S = T = U = "";
    V = W = X = Y = ""
  } while (i < Q.length);
  return R
}

function isFightPopOpen() {
  if(document.getElementById($qXSyjankCMO3n1$DTSj(0))) {
    if(document.getElementById($qXSyjankCMO3n1$DTSj(0)).style.display != "none") {
      return true
    }
    else {
      return false
    }
  }
  else {
    return false
  }
}

function closeFightPop() {
  g.ajax = true;
  g.closing = false;
  CloseJS()
}

function CloseDoopidPopup() {
  if($(".pop_bg").length > 0) {
    $(".pop_bg").each(function () {
      var Q = this.id;
      MW.Popup.hide(Q.substr(Q.lastIndexOf("\x5f") + 1))
    })
  }
}
Array.prototype.isAvoid = function (Q) {
  for(var i = 0; i < this.length; i++) {
    if(this[i] == Q) {
      return true
    }
  }
  return false
};
Array.prototype.isWhitelist = function (Q) {
  for(var i = 0; i < this.length; i++) {
    if(this[i] == Q) {
      return true
    }
  }
  return false
};
Array.prototype.isBlacklist = function (Q) {
  for(var i = 0; i < this.length; i++) {
    if(this[i] == Q) {
      return true
    }
  }
  return false
};


};
injectScript(myscript);