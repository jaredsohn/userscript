...
javascript: (function () {
	...
	var G = 1;
	var H = 0,  vegas_cash = 0, atlantic_cash=0;
	var I = 0;
	var J = false;
	var K = 0;
	var L;
	...
	var bw = 1, wait2 = 2, random_delay = 1;
	var bx = 1, heal_city_text = 'Atlantic city';
...
//	var bP = 'checked', LootStats_A = 0, LootStats_D = 0, Log_Main = 'checked';
	...
	if (top === self) {
		try {
			...
			} else if (/m.mafiawars.com.inthemafia/.test(document.location)) {
...
//	};
	...
	document.getElementById("heal_city_change").onclick = function () {
		...
		case 1:
			...
			break;
                          ...
                          case 2:
			heal_city_text = document.getElementById("heal_city").innerHTML = "Atlantic city";
			break;		
		}
		writeCookieStuff();
		return false
	...
	function attack_user() {
		...
		if (bd[j] == '') {
			...
			var l = 'http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&xw_city=' + bj + '&user=' + bb[j] + '&xw_client_id=8';
			$.ajax({
				type: "POST",
				url: l,
				data: k,
				success: function (d) {
					...
					bd[j] = 'http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=attack&xw_city=' + bj + '&tmp=' + be[j] + '&cb=' + cb + '&opponent_id=' + bb[j] + '&origin=fight_page&xw_client_id=8';
...
//		}
		...
		try {
			...
			var g = "http://m.mafiawars.com/mwfb/" + B[i] + "&xw_client_id=8"
		} catch(err) {
			alert(err)
		}
		var h = {
...
//			}
		...
		if (p != bj) {
			...
			$.ajax({
				type: "POST",
				data: q,
				url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city=" + p + "&cb=" + cb + "&destination=" + bj + "&from=fight&nextParams=&xw_client_id=8",
				success: function (a) {
					if (document.forms.something.Users.checked) {
						Y = true;
						attack_user();
	...
	function heal() {
		...
		if (bx != bj && bx == 1) {
			...
			$.ajax({
				...
				data: f,
				dataType: "json",
				url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&xw_city=" + bj + "&xw_person=" + bS + "&xcity=1",
				success: function (a) {
					if (a == null) {
		...
		if (bj == bx) {
			...
			$.ajax({
				...
				data: f,
				dataType: "json",
				url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb=" + cb + "&xw_client_id=8",
				success: function (a) {
					if (a == null) {
						TimedMessage('Error during healing, retrying', 'status', 2, heal);
						return
			...
			$.ajax({
				...
				success: function (b) {
					...
					$.ajax({
						type: "POST",
						data: f,
						url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city=" + bx + "&cb=" + cb + "&destination=" + bj + "&from=fight&nextParams=&xw_client_id=8",
						success: function (a) {
			...
			$.ajax({
				type: "POST",
				data: f,
				url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city=" + bj + "&cb=" + cb + "&destination=" + bx + "&from=fight&nextParams=&xw_client_id=8",
				success: function (c) {
					J = true;
	...
	function bank(d, e) {
		if (bj == 5) {
			...
			$.ajax({
				type: "POST",
				data: f,
				url: "http://m.mafiawars.com/mwfb/remote/html_server.php?xw_controller=propertyV2&xw_action=doaction&xw_city=5&doaction=ActionBankDeposit&xw_person=" + bS + "&amount=" + d + "&city=5&building_type=6&xw_client_id=8",
				success: function (a) {},
				error: function (a, b, c) {}
			});
			return
