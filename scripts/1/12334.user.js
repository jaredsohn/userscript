...
// 2008.2.8
// * 增加大小写不一样的topicDisplay.asp支持
// 2008.6.18
// * 增加Firefox 3.0支持
// 2008.12.4
// * 调整论坛新地址的图片链接
// 2008.12.7
// * 增加部分xml结构错误页面的支持（感谢heroboy帮助）


function getXMLDoc(url , callback , errorHandler) 
{
    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4) {
            if (request.status == 200) {
                ...
                xml.innerHTML=request.responseText;
                callback(xml); 
            }
            else {
                if (errorHandler) 
    ...
    while (rs.node)
    {
        if (rs("boardid") == 0)
        {
            if (iFlag)
            {
                ...
                HTML += "<a href='http://sonicbbs.eastday.com/boarddisplay.asp?BoardID=47' target='_blank'><img src ='http://58.49.58.159/images/benbenlogo.jpg' width='116' height='69' border=0></a>"
                HTML += "</td>"
                HTML += "<td>"
...
//                HTML += "本本族笔记本专卖是专门从事开发销售计算机软、硬件、网络、通讯设备的专业公司；是一定集成软硬件产品、解决方案及服务于一身的高科技企业。公司拥有一批年轻有为，朝气蓬勃的专业技术人员和管理人才。"
        ...
        else
        {
            ...
            if (unsafeWindow.toDate(unsafeWindow.GetCookie("EnterTime_" + rs("boardid"))) > unsafeWindow.toDate(unsafeWindow.lastTimeArray[rs("boardid")]))
                HTML += "<img src='http://58.49.58.159/images/off.gif' width='18' height='18'>"
            else
                HTML += "<img src='http://58.49.58.159/images/on.gif' width='18' height='18'>"
...
//        userIDArray = new Array();
        ...
        try
        {
            ...
            if (page == 1)
            {
                ...
                HTML += "<td width=50% align=right><div id = 'adXml'><img src='http://58.49.58.159/images/ajax-loader.gif' width=16 height=16/></div></td></tr></table>";
...
HTML = HTML.replace(/http:\/\/images.sonicalbum.com\/bbs_album_.{1,3}\//gi, originalPhotoPath)
HTML = HTML.replace(/images.sonicbbs.com/gi, "58.49.58.159")
HTML = HTML.replace(/images.sonicmodel.com/gi, "58.49.58.159")
...
//        xmlDocTopic.load("bbs_xml/" + folderID + "/" + id + ".xml?t=" + lastDate);
        getXMLDoc(encodeURI("bbs_xml/" + folderID + "/" + id + ".xml?t=" + lastDate), function(xDoc){xmlDocTopic = xDoc;showTopic(1);}) 
...
//        xmlDocUserDetailonreadystatechange();
        ...
        getXMLDoc(encodeURI("member_xml/" + folderID + "/" + userIDArray[i] + ".xml"), function(xDoc){xmlDocUserDetail = xDoc;xmlDocUserDetailonreadystatechange();}) 
//        xmlDocTopic.onreadystatechange = xmlDocTopiconreadystatechange;
//        xmlDocTopiconreadystatechange();
...
//            rs = xmlDoc.recordset;
            ...
            if (objNodeList.length != 0)
            {
                ...
                {
                    if (!(arrMedal[v].match(/[a-z0-9]/)) )
                        continue;
                    medal =  medal + " <img title='" + medalNameArray[arrMedal[v].substr(1) - 1] + "' src='http://58.49.58.159/images/medal/" + arrMedal[v] + ".gif'> ";
                }
            }
            //-- add by Alex End

            userIconArray[i] = "<br><br><img src='http://58.49.58.159/images/rank/rank" + rank + ".gif'><br>" + rankName + "[" + rank + "]<br>" + medal;
...
if (location.pathname == '/' || location.pathname.substr(0,6).toLowerCase() == '/index')
    ProcessIndexPage();
else if (location.pathname.substr(0,13).toLowerCase() == '/topicdisplay')
    ProcessTopicDisplayPage();
else if (location.pathname.substr(0,6).toLowerCase() == '/reply'
    || location.pathname.substr(0,10).toLowerCase() == '/topicedit'
    || location.pathname.substr(0,5).toLowerCase() == '/new.'
    )
    ProcessReplyPage();
//else if (location.pathname.substr(0,7).toLowerCase() == '/search'
//    || location.pathname.substr(0,17).toLowerCase() == '/sonicchat/search'
//    || location.pathname.substr(0,11).toLowerCase() == '/userdetail'
//        )
