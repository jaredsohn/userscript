...
// 2.2 fixed fuel when none available to buy issue
...

{
  ...
  document.getElementById('loadSF').addEventListener('click',loadSF,true);
...

{
  ...
  document.getElementById('loadRobots').addEventListener('click',loadRobots,true);
}
...

{
  ...
  if (commodity[16] < 5) {
    freeShipSpace = freeShipSpace - (5 - commodity[16]);
  }
...

{
  ...
  if (shipStock[16] > fuelAmountToPurchase) {
    sellCommodity(16, shipStock[16] - fuelAmountToPurchase);
  }
  if (shipStock[16] < fuelAmountToPurchase) {
    buyCommodity(16, fuelAmountToPurchase - shipStock[16]);
  }
