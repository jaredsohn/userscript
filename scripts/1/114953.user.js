...
//plus.google.com/games/216622099218
...
for (y in unsafeWindow.arStrings.unitName) if(y == "u2"){continue} else if(y == "u14") {continue} else { n+='<option value="'+findoptvalue(y)+'">'+unsafeWindow.arStrings.unitName[y]+'</option>' };
  ...
  doTrades: function(count){
    var t = Tabs.transport;
   	...
   	var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
		...
		if(!Cities.byID[city]) return;
   	 	var xcoord = t.tradeRoutes[count]["target_x"]; ///HERE
	...
	actionLog('Attempting Trade   From: ' + cityname + "   To: " + xcoord + ',' + ycoord + "    ->   Wagons: " + wagons_needed)
	actionLog("trade: "	+ JSON2.stringify(t.tradeRoutes[count]) + " params: " + JSON2.stringify(params));
