// ==UserScript== 
// @name          MySpace Comment Box Plus UPDATED
// @description   Modifies comment boxes in user profiles. Updated to work with MySpace now.
...
(function() {
	...
	for(var i=0; i<allTables.length; i++){
		if(allTables[i].innerHTML.indexOf('Friends Comments')<400 && allTables[i].innerHTML.indexOf('Friends Comments')>0){
			var friendHTML = '<tr><td colspan="2" class="orangetext15"><span style="text-align:left">&nbsp;&nbsp;&nbsp;Leave  '+ userName +'  a comment. </span><br><br /><form method="post" action="hhttp://comments.myspace.com/index.cfm?fuseaction=user.ConfirmComment" style="text-align:center"><input type="hidden" name="friendID" value="' +  friendID + '"/><textarea name="f_comments" cols="64" rows="5" style="background-color:FFFFFF; border-width:1px; border-style:solid; border-color:000000; color:000000;"></textarea><br /><input type="submit" value="Add Comment" /></form></td></tr>';
			
			var numComments = 10;
			var commentArr = allTables[i].getElementsByTagName('tr');
			friendHTML = friendHTML + '<tr><td class="orangetext15" colspan="2">&nbsp;&nbsp;&nbsp;' + userName + '\'s <span class="redtext">' + Math.min(numComments, commentArr.length) + '</span> latest comments (<a href="http://comments.myspace.com/index.cfm?fuseaction=user.viewComments&friendID=' +  friendID + '">View all of ' + userName +'\'s comments</a>)<br><br></td></tr>';
