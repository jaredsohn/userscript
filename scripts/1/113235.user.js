// ==UserScript==
// @name           إعداد و تعريب : كابي فرح شريط العضوية الخاصة
// @namespace      www.tribalwars.co.uk
// @description    شريط مباني العضوية الخاصة (المبنى الرئيسي, الثكنات , الإسطبل , الورشة , الأكادمية , الحداد , نقطة التجمع , السوق)  
// @include        http://*.tribalwars.*
// @include        https://*.tribalwars.*  
// @version        0.9
// ==/UserScript==

var lang = "ae";   // Language option
...
function functionMain(e) {
	...
	if (b==null) {
		...
		window.createQuickbarShortcuts = function() {
			...
			//+ "<tr>   " + 
            newShortcut("", "/redir.php?url=www.bhootnath.in", langArr[8]);
			tableIdIndex++;
			trIdIndex++;
		}
	}
