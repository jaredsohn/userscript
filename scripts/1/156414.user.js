...
GoldJobs = {
	version: VERSION,
	CHECK_FILE_URL: 0,
	FEEDBACK_URL: "http://macabre2077.koding.com/userscripts/the-west/goldjobs/sendFeedback.php",
	...
	shownImageOpacity: 1,
	GAME_VERSION_SUPPORTED_MIN: 2.03,
	GAME_VERSION_SUPPORTED_MAX: 2.04
};
...
GoldJobs.getEnqueueButton = function(jobId, x, y) {
	...
	var canDo = job.canDo();
	var duration = job.duration || 7200;
...
GoldJobs.addColumnsCss = function() {
	...
	$('.goldenJobs .row_head .jobName').css('width' , '128px');
...
GoldJobs.makeJobsTable = function(chosenJobName, showSilver, showGold, sortByName, sortByDistance) {
	...
	var arrow_desc = '&nbsp;<img src="http://www.the-west.ru/images/window/job/sortarrow_desc.png"/>';
	var arrow_asc = '&nbsp;<img src="http://www.the-west.ru/images/window/job/sortarrow_asc.png"/>';
...
GoldJobs.getMessageDialog = function(text, type, title) {
	title = title || "";
	if(window.hasOwnProperty("tw2gui")) {
		if(type == "warning") {
			type =  tw2gui.dialog.SYS_WARNING;
		}
		if(type == "question") {
			type =  tw2gui.dialog.SYS_QUESTION;
		}			
		return new tw2gui.dialog(text, title, type);	
	} else {
		if(type == "warning") {
...
GoldJobs.intro = function() {
	...
	text += '<li>v2.04 support</li>';
	text += '<li>If you have ideas for last version of this script, then send them <a href="#" onclick="javascript:GoldJobs.openFeedbackWindow();">here</a></li>';
...
GoldJobs.toggleSelectbox = function() {
	var isShown = (this.selectBox.divMain.css("display") == "block");
	if(isShown) {
...
GoldJobs.init = function() {
	...
	div.append(link);
	if(GoldJobs.getGameVersion() < 2.04) {
		div.append(selectbox.getMainDiv());
	}
...
GoldJobs.setSelectboxPosition = function(nextTo) {
	if(GoldJobs.getGameVersion() < 2.04) {
		this.selectBox.setPosition(-168,12);
	} else {
		var p = $(nextTo).offset();
        p.left -= GoldJobs.selectBox.divMain.width() + 150;
...
GoldJobs.gui.init = function() {
	if(GoldJobs.getGameVersion() < 2.04) {
		GoldJobs.gui.makeButton = function(caption, callback) {
			return new tw2gui.button(caption, callback);
		};
		GoldJobs.gui.makeTextarea = function(content,classes) {
			return new tw2gui.textarea(content,classes);
		};
		GoldJobs.gui.makeTextfield = function(id) {
			return new tw2gui.textfield(id);
		};
		GoldJobs.gui.makeTable = function() {
			return new tw2gui.table();
		};
		GoldJobs.gui.makeSelectbox = function() {
			return new tw2gui.selectbox(true);
		};
		GoldJobs.gui.makeCheckbox = function(label,groupClass,callback) {
			return  new tw2gui.checkbox(label,groupClass,callback);
		};
	} else {
		GoldJobs.gui.makeButton = function(caption, callback) {
