...
// @require        https://crypto-js.googlecode.com/svn/tags/3.0.2/build/rollups/sha1.js
// @description    Organizes,  upgrades and salvages KOC throne room items
// ==/UserScript==


var Version = '20120903_mm';
...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            // preset selector
            m += '<TABLE  width=90% class=trTabPad2><tr align=center><td width=50%><div><span>View preset: <select id="PresetList">';
            ...
            for (k= 0; k < 8; k++)
            {
                m += '<option value="local'+k+'"> Local:  '+ String.fromCharCode(65+k).toUpperCase() +'</option>';
            }
            ...
            m+= '<tr><td colspan=3><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 10px; margin-left: 40px;"> Switch to preset: </span>';
            ...
            m+= '<tr><td colspan=3><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 10px; margin-left: 40px;"> Switch to local preset: </span>';
            for (a=0; a<8; a++)
            {
               m += '<a style="font-family: serif;" class="button20 loadPreset"  id=trLoad'+ a + ' > <span> ' + Seed.throne.slotNum + '-' + String.fromCharCode(65+a).toUpperCase() + ' </span></a>';                   
            }
            m += '</td></tr>';

            m+= '<tr><td colspan=3><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 10px; margin-left: 40px;"> Save local preset: </span>';            
            for (a=0; a<8; a++)
            {
               m += '<a style="font-family: serif;" class="button20 savePreset" id=trSave'+ a + ' > <span>' + Seed.throne.slotNum + '-' + String.fromCharCode(65+a).toUpperCase() + ' </span></a>';                   
            }
            
            m += '</td></tr>';
            m += '<tr align=center><td colspan=3><div id=trSwitchStatus></div></td></tr>';
        ...
        loadLocalPreset : function (id) {
            ...
            setTimeout ( function () { 
                ...
                t.setSwitchStatus("Local preset "+ String.fromCharCode(65+id).toUpperCase() + " loaded.")
                }, c* delay*1000 + 1000);
        },
        ...
        saveLocalPreset : function (id) {            
            ...
            savePresetData();
            Tabs.organizer.setSwitchStatus("Local preset " + String.fromCharCode(65+id).toUpperCase() + " saved.");
        },
        ...
        paintTable : function() {
            ...
            $("td.tdcard").on( "mouseleave", "*", function (A) {
                var theId = $(this).parents("td.tdcard").attr("id").split("card")[1];
                if (unsafeWindow.kocThroneItems[theId])
...
Tabs.options = {
        ...
        init : function (div) {
            ...
            // header stuff
            var m = '<Div><DIV id=trOptioins class=trStat>OPTIONS</div><TABLE id=trOptionTbl width=100% height=0% class=trTab>';

            m += '<TR><TD width=100%><INPUT id=trDisableAnim type=checkbox '+ (upgradeData.disableAnim?'CHECKED ':'') +'/> Disable success/failure animation (Big red X) </TD></TR>';
...
Tabs.upgrader = {
        ...
        qualities :  [ "Simple", 
                       ...
                       init : function (div){
                           ...
                           //m += '<tr align="center"><td colspan=3><div id=trLastResult class=indent25> <br> </div></td><td><a id=trpplink><img id=trpp /></a></td></tr>';
                           m += '<tr align="center"><td colspan=3><div id=trUpgradeStatus class=indent25> <br> </div></td></tr>';
                           m += '<tr align=center><div><td><input style="float: left;" id=trClearQ type=button value="Clear Queue"/></div></td><td colspan=1><div id=trLastResult class=indent25> <br> </div></td><td colspan=2><a id=trpplink><img id=trpp /></a></td></tr>'
                               m += '</table>';
                                       ...
                                       if (upgradeData.repairAll == true)
                                       {
                                           ...
                                           for (k in queueData.list)
                                           {
                                               ...
                                               if ( (throne_item != null ) 
                                                       ...
                                                       && (queueData.list[k].status != "complete"))
                                               {
                                                   t.doRepair(throne_item.id);
                                                   clearTimeout(t.timerH);
                                                   t.timerH = setTimeout(t.doAction, retryTime*1000);
