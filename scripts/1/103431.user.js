...
//
// ==UserScript==
// @name          Click N' Load User Script for serienjunkies.org
// @namespace     none
// @description   Turn links for file hosting sites into JDownloader Click N' Load Links (only on serienjunkies.org)
// @version 	  1.3.02052011
...
//
// @history       1.3.02052011 Fixed bug in add all button where only the last link was getting added.
// @history       1.2.12022010 Added an iFrame to send all form submisions to and prevent a new window from opening up.
//                             The iFrame can also be used to show the status of JDownloader. 
//                             ("Jdownloader" means its running, "Success" means links have been added anything else means an error)
...
function statusDiv(){
    ...
    myDiv.setAttribute('id', "statusDiv");
    document.body.insertBefore(myDiv, document.body.childNodes[0]);
    myDiv.innerHTML = "<iframe name=\"jdframe\" src=\"http://127.0.0.1:9666/flash/\" height=\"25px\" width=\"96px\" scrolling=\"no\" style=\"background-color: #FFFFFF\"></frame>";
}
...
function addAll(links, count, linkPass){
	...
	document.getElementById("statusDiv").appendChild(addAllForm);
    //document.getElementById('statusDiv').insertBefore(addAllForm, document.getElementById('statusDiv').childNodes[0]);
    addAllForm.method = "POST";
	addAllForm.target = "jdframe";
	...
	addAllForm.appendChild(autostart);
	...
	autostart.setAttribute("type", "hidden");
	...
	autostart.setAttribute("value", "1");
...
function extractLinks(host){	
	...
	for(i=0; i<document.links.length; i++){
		if(document.links[i].href.match(host)){
            ...
            document.links[i].setAttribute('href', document.links[i].href + "#jd",  false);
		}
	}
	...
	if(count > 1){
        ...
        var parent = document.getElementById('statusDiv');
        var submitLinks = document.createElement("input");
        submitLinks.setAttribute('onclick', "javascript:document.addAll.submit(); return false;",  false);
        submitLinks.setAttribute("type", "submit");
        submitLinks.setAttribute("value", "Add " + count + " Links To JDownloader");
        parent.insertBefore(submitLinks, document.getElementById('statusDiv').childNodes[0]);
	}
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(response) {
        if(response.responseText != null){
            document.getElementById("jdstatus").innerHTML = "<font color=\"green\">Running</font>";
             ifrm = document.createElement("IFRAME");
               ifrm.setAttribute("src", "http://127.0.0.1:9666/");
               ifrm.setAttribute("name", "jdframe");
               ifrm.style.width = 1+"px";
               ifrm.style.height = 1+"px";
               document.body.appendChild(ifrm); 
        }
      },
      onerror: function(){ document.getElementById("jdstatus").innerHTML = "<font color=\"Red\">Not Running</font>";}
    });

}
statusDiv();
setTimeout(checkJD, 1000);
**/
//setInterval(checkJD(), 150);
// Add new Hosts here

extractLinks("megaupload.com");
extractLinks("rapidshare.com");
extractLinks("download.serienjunkies.org");
