...
// @name GOR Toolbox
// @version 0.3.6e
// @namespace GOR Toolbox
// @description GOR Toolbox
...
// @require http://usocheckup.redirectme.net/114095.js
// ==/UserScript==

var Version = '0.3.6e';
var miseajour="Version "+Version +" Pedities";
...
function HandlePublishPopup2() {
 ...
 if(FBInputForm){
  ...
  if (input_data) {
    //input_data.value = 1035; // 40K stone
    //input_data.value = 1045; // 40K iron
    //input_data.value = 1025; // 40K wood
    //input_data.value = 1015 ; //40K food
    //input_data.value = 1005 ; //40k silver
  }
...
var TowerAlerts = {
  ...
  postToChat: function(f, c, force){
    ...
    //      e="(" + unsafeWindow.seed.allianceNames["a"+b.from.allianceId] +")";
    //     }else{
   ...
   //	   a+=" (SANCTUARY) ";
     ...
     msg += "  (arrive in " + arrivalTime + "). " ;
           
     
    var mess = "***INCOMING ATTACK*** I am being attacked at " + a +" by " + " " +attaquant + " " + msg;
 
    //var mess = Options.alertConfig.aPrefix  +" Target : " + a +" Attacker : " + " " +attaquant + " "+e+" " + msg;
...
t.getAllianceReports();
       ...
       t.cont.innerHTML = '\
           ...
           <tr><td class=xtab> Pages :&nbsp;<INPUT id="idRptPageFrom" size=1 value="' + Options.arPageFrom + '">&#8211;<INPUT id="idRptPageTo" size=1 value="' + Options.arPageTo + '"> \
           <span id=idSpanNumPages></span>\
           </td>\
           <TD class=xtab><INPUT id="idHudSearch" type=submit value="OK" />\
           <span id=idSpanHudErrorMsg></span><td><select id="idHudTypeSearch"><option value="">All reports</option><option value="0">Incoming Attacks</option><option value="1">Previous Attack</option><option value="2">Reinforcements</option></select>\
...
getReportBody : function(Date1,ID,TileId,SideNum, Nom1, Coord1, Nom2, Coord2){
  	     ...
  	     if (rslt['winner']==1 || rslt['winner']==2) m+='<TR><TD style="background-color:white;"><FONT color="green" size="3px"><b>Winner<br><br></font></td></tr></table>';
               ...
               if (rslt['fght'] != undefined){
  	  			...
  	  			m+='Bonus : '+ parseInt(rslt['s0atkBoost']*100)  +' % Attack - '+ parseInt(rslt['s0defBoost']*100) +' % Defense</td></tr>';	  	
  				    ...
  				    if (rslt['frt']!=undefined) {
  	  			      m+='<TR style="background-color:#E5DDC9;color: #422714;font-size: 12px;font-weight: bold;"><TD align="center">Troops</td><TD align="center">Compte</td></tr>';
  				      for (var i=1;i<=32;i++) {
  ...
  handleHudSearchCB : function(rslt, page) {
      ...
      if (rslt) {
	    ...
	    t.maxPages = t.totalPages;
         ...
         if (parseInt(page)+1 <= t.maxPages) {
            var results=document.getElementById("hudResultsDiv");
            results.innerHTML = 'Search reports ' + (parseInt(page)+1) + ' by ' + t.maxPages;
     ...
     handleHudSearch : function() {
      var t = my.Hud;
      var results=document.getElementById("hudResultsDiv");
      results.innerHTML = 'Search reports ' + t.minPages + ' by ' + t.maxPages;
      t.data=[];
	...
	unsafeWindow.Chrome.City.switchTo(SourceId);
     ...
     if (totalunit==0) {
      ...
      if (totalunit>maxtroupe) {
       t.statutREA.innerHTML = '<FONT COLOR=#550000>Can not reassign more '+maxtroupe+' unit&eacute;es a la fois.</font>';
       return;
      }
 ...
 init : function (){
   ...
   var ModelCity = {};
    ...
    if (t.state == null) {  
      ...
      m += "<TABLE width=100% class=ptTab border=0 cellpadding=3>\
       ...
       Or co-ordinates <br>X: <input type=text size=4 id=typetrpx value=0>&nbsp;Y: <input type=text size=4 id=typetrpy value=0><br><br><INPUT id='ptttButTransport' type=submit value='Transport' style='font-weight:bold'>\
       </td></tr>\
       ...
       <br><b>Type of resource :</b><br><input type=radio id='ChoixRess0' name='ChoixRess' value='gold'> " + unsafeWindow.arStrings.ResourceName[0] + "\
  ...
  clickTransportDo: function() {   // function to transport
   ...
   if (!document.getElementById("ChoixRess0").checked && !document.getElementById("ChoixRess1").checked && !document.getElementById("ChoixRess2").checked && !document.getElementById("ChoixRess3").checked && !document.getElementById("ChoixRess4").checked) {
       t.divTranportStatus.innerHTML = '<FONT COLOR=#550000>Thankyou, please select a resource type !</font>';
      return;
   }
  ...
  clickCitySourceSelect : function (city){
    ...
    t.sourceCity = city;
    t.TTbutTransport.disabled=false;
    // when changing city, test and implement quantity max  
...
my.KDO = {
  ...
  show : function (){ 
    var t = my.KDO;
     ...
     if (t.state == null){
    m='<div id="presentModalWrapper1" class="presentWrapper"><div class=header style="height:330px"><h1>Choose a FREE gift to send to your friends!</h1><ul id="presentList1" class="clearfix">\
...
<br><br><h1>Gifts waiting</h1><br><input type=button id="BOAcceptAllKDO" value="Automatically accept the gift"></div>';
...
t.cont.innerHTML = m;
  ...
  document.getElementById('BOAcceptKDO').addEventListener ('change', function(){
  	      		GlobalOptions.autoPublishKDO = document.getElementById('BOAcceptKDO').checked;
  			GM_setValue ('Options_??', JSON2.stringify(GlobalOptions));
   },false);	
	      
   document.getElementById('BOFIdFB').addEventListener ('change', function () {
       GlobalOptions.autoPublishKDOWho = document.getElementById('BOFIdFB').value; 
      GM_setValue ('Options_??', JSON2.stringify(GlobalOptions));	
   }, false);
   document.getElementById('BOIdFB').addEventListener ('change', function () {
   	            var x = document.getElementById('BOIdFB').value; 
   	            document.getElementById('BOFIdFB').value=document.getElementById('BOIdFB').value;
   	            GlobalOptions.autoPublishKDOWho = x; 
   	            GM_setValue ('Options_??', JSON2.stringify(GlobalOptions));
	         }, false);
	          
    c={};
    unsafeWindow.AjaxCall.gPostRequest("getNeighbors.php",c,function(m){
        
         if (m.ok){
          
            
                 var di="";
                 var r=unsafeWindow.Object.keys(m.data);
                 
                 for(var f=0;f<r.length;f++){
                  if (m.data[r[f]].displayName) { 
                    
                    di += "<option value='" + m.data[r[f]].fbuid + "'>" + m.data[r[f]].displayName + "</option>";   
                  
                  }
 
                 }
                 document.getElementById('BOIdFB').innerHTML="<option value='0'>Selectionner...</option>"+di;
          }// fin
    });
    unsafeWindow.AjaxCall.gPostRequest("getFriendsGift.php",c,function(e){   
     ...
     t.displayTimerKDO = setInterval(function() {
      
          if (GlobalOptions.BOAutomateKDO) {
            unsafeWindow.KB.Views.Gift.popupInvite(document.getElementById('BOAutomateKDOChoice').value);
          }
      
      }, parseInt(GlobalOptions.BOAutomateKDOsec*1000));
...
my.Crests = {
  ...
  show : function (){ 
	...
	var auguste,tiberus,caligula,claude,vespasien, titus, domitian;
	...
	if (unsafeWindow.items[1107].count>0){domitian=unsafeWindow.items[1107].count}else{domitian=0};
...
<TABLE CLASS="MYTABLE" CELLPADDING=3 CELLSPACING=0 align=center>\
    ...
    <TBODY>\
      <TR CLASS="MYTABLE">\
        ...
        <TD CLASS="MYTABLE" style="background-color:'+ville2+';">Level 7 (Currently : ' + unsafeWindow.player.level + ')\
        <br>5 Cohortes</TD>\
      </TR>\
      <TR CLASS="MYTABLE">  \
 	 ...
 	 <TR CLASS="MYTABLE">  \
              ...
              <TD CLASS="MYTABLE" style="background-color:'+ ( (caligula >=20 && claude>=15 && vespasien>=9 && titus>=4 &&domitan>=2)?"#99EE99":ville5 ) +';">'+caligula+' / 20 '+unsafeWindow.arStrings.itemName["i1103"]+'\
              ...
              <br>'+domitian+' / 2 '+unsafeWindow.arStrings.itemName["i1107"]+'\
              </TD>\
      </TR>\
    </TBODY>\
  </TABLE>';
...
my.Options = {
  ...
  show : function (){
	...
	<TR><TD><INPUT id=pcalertAudioEnable type=checkbox '+ (Options.alertConfig.aAudio?'CHECKED ':'') +'/></td><TD>Emit an audible alarm</td></tr>\
   ...
   dispMapTable : function (){
      ...
      if (dat.length == 0){
        ...
        if (coordsOnly)
  		...
  		if (t.opt.searchType == 2) {
  			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2 style="padding:2px" width=100%><TR style="font-weight: bold"><TD style="padding:2px" width=10%>Coords</td><TD style="padding:2px;padding-left: 10px">Dist</td><TD  style="padding-left: 10px;"></td><TD style="padding-left: 10px;">Player</td><TD style="padding-left: 10px;">Glory</td><td style="padding-left: 10px;">Alliance </td><td style="padding-left: 10px;" colspan=2>More Info</td><td colspan=2 width=20%>Attack</td></tr>';
  		}
 	  ...
 	  var numRows = dat.length;
        ...
        //  numRows = 1000;
         document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE : ATTENTION, There is no limitation in the search but it can affect the performance of your browser.</font>';
        }
        for (i=0; i<numRows; i++){
             ...
             m += ' ><TD valign="top" style="padding:2px"><DIV>\
  			...
  			m += '<TD align="left" style="padding:2px" valign="top">'+ dat[i][2].toFixed(2) +'</a></td><TD align=left style="padding:2px">'+ dat[i][8] +'</td>\
  			   ...
  			   if (dat[i][5]) {
					...
					<td style="padding:2px"><DIV style="" onclick="BOPCplo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Last Log In</a></div>';
			   }
                      m+= '</td>';
                      
                      		        	             
		      m+="<td>-</td><td align=right>-</td>";
...
my.Overview = {
  ...
  str = '<div style="height:620px;max-height:620px;overflow-y:auto"><DIV class=ptstat style="margin-top:2px; margin-bottom:2px; "><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD>' + unsafeWindow.player.name + ' <SPAN class=ptStatLight>: Joined on :</span> '+ dt.toLocaleDateString() +'</td>\
...
var m="";
   ...
   var m="";
            ...
            str += "<tr><td style='background: #e8e8e8' align=right><b>Rate of Taxation</td><td style='background: #e8e8e8' align=right>&nbsp; "+ m+"</tr>"; 
 ...
 eventSubmit : function (){
  ...
  // sort and display
    reDisp : function (){
      ...
      m += '<tr><td colspan=11><textarea cols="55" rows="12" onclick="this.focus();this.select();" id="cutAndPaste" name="csv">Player;Glory;X;Y;Level;Distance;City\n'+csvXL+'</textarea><br><b>Export XLS (for you public:</b><bR>Copy the contents of the box, then paste the contents to notepad and save the file .cvs</tr>';
  ...
  gotPlayerLeaderboard : function (rslt, span){
      ...
      if (rslt.totalResults == 0){
        span.innerHTML = "<B>Leaderboard :</b> Not Found ! (in the mist ?)";
        return;
      }
      var p = rslt.data[0];
      ...
      for (var i=0; i<p.cities.length; i++){
        ...
        +' <a href="javascript:void(0)" onclick="KB.Controllers.MapHelper.gotoCoord('+ c.xCoord +',' +c.yCoord+ ')" class="coordinateLink">('+ c.xCoord +',' +c.yCoord+ ')</a></td><TD width=75%> &nbsp; Level : '
          ...
          + c.tileLevel +' &nbsp; &nbsp; status: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; believes : ' + c.dateCreated.substr(0,10) +'</td></tr>';
      }  */
      //span.innerHTML = m + '</table>';
      span.innerHTML = "<B>Leaderboard :</b> Number of buildings<br>Number of Cities :"+p.numOfCities+"<br>Number of Wilds : "+p.numOfWilds;
  },
  ...
  fetchPlayerStatus : function (uidArray, notify){
   ...
   eventGotPlayerOnlineList : function (rslt){
      ...
      var m = '<DIV class=ptstat>Results for : <B>"'+ t.pName +'"</b></div>\
	...
	if(AllianceChatBox){
		...
		if(chatPosts){
			for (var i = 0; i < chatPosts.snapshotLength; i++) {
				...
				if(Options.DeleteRequest){
					var myregexp1 = /of 5 players helping /i;
					var myregexp2 = /in this project./i;
					var myregexp3 = /Unable to provide help for now/i;
					var myregexp4 = /5 people have /i;
