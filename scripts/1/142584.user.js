...
//Create language strings
	...
	try {
		...
		ytvle.strings.language=ytvle.strings.region.split(/-|_/);
	} catch(e) {
	...
	if (ytvle.strings.language[0]=="sv") {
		...
		ytvle.strings.dlurl="Hämtning";
		ytvle.strings.dlget="Hämta";
		ytvle.strings.quality="Kvalitet";
		ytvle.strings.type="Filtyp";
		ytvle.strings.dlnow="Ladda ner videoklipp";
		ytvle.strings.dlnowA="L";
		ytvle.strings.noUrls="Det här videoklippet finns inte tillgängligt för nerladdning.";
	} else {
...
//Add event handles for the URL and download elements
	...
	ytvle.download=function(e) {
		location.href=ytvle.urls[e.dataset.ytvleUrl].url.replace("http:", location.protocol)+"&title="+encodeURIComponent(ytvle.getFileName());
	};
//Register GM command
	ytvle.downloadNow=function() {
		...
		if (best.id>-1) {
			location.href=ytvle.urls[best.id].url.replace("http:", location.protocol)+"&title="+encodeURIComponent(ytvle.getFileName());
		}
	}
	try {
