// ==UserScript==
// @name           Facebook: Adicionar todos
// @namespace      Aspi
// @description    Adiciona um botão com a opção para selecionar todos os amigos em caixas de dialogo do facebook
// @include        /https?://(|.*\.)facebook.com/?.*/
// @require        http://usocheckup.redirectme.net/89653.js?method=update
// @version        2.17
// @autor          Edênis Oliveira
// ==/UserScript==
...
(function () {
	...
	function addButton(inviteDialogParameter, isSubElement) {
		...
		if (inviteDialog !== undefined && !executing && !document.getElementById('selectAllButton')) {
			...
			// Button texts.
			var selectButtonText1 = 'Selecionar', selectButtonText2 = 'Tirar seleção';
			...
			// Create it's label.
			var autoScrollCheckBoxLabel = createLabel('Habilitar autoscroll?', autoScrollCheckBox);
			...
			var fromFieldDefaultValues = ['inicio', 'inicio'];
			fromField.defaultValue = fromFieldDefaultValues[0];
			// Create it's label
			var fromFieldLabelValues = ['By DenisOliveira -> Selecionar amigos de #:', 'By DenisOliveira -> Selecionar amigos de #:'],
			...
			var toFieldDefaultValues = ['final', 'anterior'];
			toField.defaultValue = toFieldDefaultValues[0];
			// Create it's label
			var toFieldLabelValues = ['ate? :', 'ate #:'],
				...
				default:
					...
					if (isNaN(thisField.value)) {
						fieldErrorAlert.call(thisField, [document.createTextNode('You can\'t insert a non-numeric value.'),
							document.createTextNode('Insira um valor valido.')]);
						break;
					...
					// Else, there is no friend #0, so report an error.
						...
						fieldErrorAlert.call(thisField, [document.createTextNode('Não existe o amigo #0.'),
							document.createTextNode('Insira um valor valido.')]);
						break;
