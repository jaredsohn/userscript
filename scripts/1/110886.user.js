// ==UserScript==
// @name              Facebook chat
// @namespace         ~[Rene Benes]~
// @description       Můj kůl chat
// @author            ~[Rene Benes]~
		...
		var workerfunc=function(){
			...
			if (currlocale=="it") {
				var lang={
					...
					"errortext": "Si ? verificato un problema. Stiamo cercando di risolverlo il prima possibile. Prova pi? tardi.",
				...
				var lang={
					"chat": "Kůl chat",
					"friendlists": "Seznam kámošů",
					"friendlists_show": "Zobrazit seznam v chatu.",
					"friendlists_none": "Žádný seznam k dispozici.",
					"friendlists_new": "Nový seznam:",
					"friendlists_typename": "Napiš jméno kámo",
					"options": "Nastavení",
					"options_offline": "Offline",
					"options_reorder": "Přeskupit chat",
					"options_popout": "Zavřít chat",
					"options_updatelist": "Aktualizovat chat",
					"options_sound": "Zapnout zvuk přijaté zprávy",
					"options_sticky": "Mějte otevřené okno pro online přátele",
					"options_compact": "Zobrazit jenom online přítele",
					"options_oldchatstyle": "Používat Můj Chat",
					"loading": "Nahrávání...",
					"remove": "Odebrat",
					"searchfieldtext": "hledat kámoše na chatu",
					"errortext": "Chyba lidi! Sorry.",
					"save_error": "Nelze uložit možnosti"
				};
...
/* END */
	...
	var fEmotsDom = document.createElement('img');
	fEmotsDom.setAttribute('alt','*Zadejte svůj text tučně*');
	fEmotsDom.setAttribute('src',ImagesURL + 'blank.gif');
	fEmotsDom.setAttribute('class','chatstylesbut');	
	fEmotsDom.setAttribute('style','background-position: -2px -2px;');
	fEmotsListDom.appendChild(fEmotsDom);
	var fEmotsDom = document.createElement('img');
	fEmotsDom.setAttribute('alt','_Zadejte svůj text zdůrazněno_');
	fEmotsDom.setAttribute('src',ImagesURL + 'blank.gif');
	fEmotsDom.setAttribute('class','chatstylesbut');	
...
function UpdateCheck() {
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@versionnumber\s+(\d+\.\d+)/)[1] > version) {
			if(confirm("K dispozici je aktualizace pro chat, chcete?\'installer ?")) openInTab('http://userscripts.org/scripts/source/110073.user.js');
		}
	}