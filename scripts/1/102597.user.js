...
//
// ==UserScript==
// @name          HOMMK - Jactari (ru)
// @author        Julien Eyries
// @namespace     http://www.mitbbs.com/bbsdoc/Game.html
// @description   Modified version for La Vagie by Leon,stalin@mitbbs,nickmit,etc...
...
/*
   ...
   2- ne rien mettre dans le DOM, tant que le module n'est pas active
...
extend( locale_strings_ru, {
 ...
 function launch_jactari(switch_combat_side){
	 ...
	 $("#hommk-user-info").html( '<a href="'+link+'" class="ui-state-default" target="_blank">чистый симулятор</a>' ); 
...
extend( locale_strings_ru, {
	 "VIGIE_NOTHING_HERE":   "никого не видно.",
	 "VIGIE_SEARCH_TROOPS":   "поиск активных передвижений...",
	 });

 extend( locale_strings_fr , {
	 "VIGIE_NOTHING_HERE":   "rien a l'horizon.",
...
(function() {
 ...
 extend( locale_strings_default, {
	 ...
	 extend( locale_strings_ru, {
	 "CARTO_SHARE":          "Сканировать",
	 "CARTO_SHARE_COMMENT":  "скан карты...",
	 "CARTO_PLOT":           "В большом окне",
...
extend( locale_strings_fr , {
		...
		'LINK_MINIMIZE':    'cliquer pour reduire'

		});
		
extend( locale_strings_ru , {
		'LINK_ABOUT':       'Инфо',
		'LINK_EXPAND':      'развернуть',
		'LINK_MINIMIZE':    'свернуть'
...
.appendTo($panel_extended)
	...
	$('<button id="hommk-user-jactari-button-defender" style="width:120;margin-top:5px; display:block;">Симулятор - Защита</button>')
	.button({disabled:true})
.appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
// Le Scribe
	...
	$('<button id="hommk-user-scribe-button" style="width:120;margin-top:5px; display:block;">Шпионский отчет</button>')
	.button({disabled:true})
.appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
// La Vigie
	...
	$('<button style="width:120;margin-top:5px; display:block;">Активность на карте</button>')
	.button()
.appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
// Le Cartographe
	...
	$('<button style="width:120;margin-top:5px; display:block;">Карта мира</button>')
	.button()
.appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
.appendTo($panel_extended);
	...
	$('<button style="margin-top:5px; display:block;">Центрировать карту</button>')
	.button()
.appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
