// ==UserScript==
// @name           Castle Age - Elite Guard Builder Pro
// @description    This script is a modification of MuabDib's Original Elite Guard Builder Script
...
/*
Changelog:
3.0.1 - Changed url being created to reflect Castle Ages new url - casuser not just user
3.0.2 - Fixed ParentNode issue that caused the Management Box to not display
3.0.3 - Added background images to the Management Box to make it look nicer
*/

/*
to-do:
	1. Add Names to links so they are easier to identify
*/
...
function loopRequests(x, modBox) {
	...
	url = 'http://apps.facebook.com/castle_age/party.php?twt=jneg&jneg=true&casuser=' + UserID[x] + '&lka=' + UserID[x] + '&ref=nf';	
	startXmlHttp(url);
	if (++x < UserID.length)
	{
		modBox.innerHTML = 'Working ' + x + '/' + UserID.length;
...
function massAddFav() {
	...
	{
		url = 'http://apps.facebook.com/castle_age/party.php?twt=jneg&jneg=true&casuser=' + ArmyMembers[i] + '&lka=' + ArmyMembers[i] + '&ref=nf';
		startXmlHttp(url);
	}	
...
function createJoinLink(thisPlayer) {
	...
	thisInsert.setAttribute('href', 'http://apps.facebook.com/castle_age/party.php?twt=jneg&jneg=true&casuser=' + thisPlayer + '&lka=' + thisPlayer + '&ref=nf" " target=_blank>');
...
// This is where we create the actual UI
function createManagementBox() {
	var theBox = document.createElement('div');
	theBox.setAttribute('id', 'EGM_Container');
	theBox.setAttribute('style', 'clear: both; text-align: center; -moz-user-select:none; background-image: url("http://image4.castleagegame.com/3014/graphics/bg_main_middle.jpg"); border-width: 2px; border-style: solid; margin-bottom: 5px;');
	
	var theTitle = document.createElement('div');
	theTitle.setAttribute('id', 'EGM_Title');
	theTitle.setAttribute('style', 'background-image: url("http://image4.castleagegame.com/3014/graphics/bg_jobs_tile.jpg"); background-color: #EDEFF4; padding-left: 3px; padding-right: 3px; padding-bottom: 3px; padding-top: 3px; font-weight: bold; border-bottom-style: solid;');
	...
	theScript.innerHTML = '<a href="http://userscripts.org/scripts/show/101541" target=_blank>Script Homepage</a> | <a href="http://userscripts.org/users/318796" target=_blank>Psyber\'s Scripts</a>';
	theBox.appendChild(theScript);

//The id of the parentNode could change again. This is where you fix it
...
function createFavListElement(thisPlayer) {
	...
	thisInsertParty.setAttribute('href', 'http://apps.facebook.com/castle_age/party.php?twt=jneg&jneg=true&casuser=' + thisPlayer + '&lka=' + thisPlayer + '&ref=nf');
