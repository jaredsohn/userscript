scr_meta=<><![CDATA[
// ==UserScript==
// @name           Incrasebux Referral Highlighter
// @namespace      http://www.incrasebux.com/
// @description    Incrasebux Referral Highlighter
// @include        http://www.incrasebux.com/rreferrals.php*
// @version        0.1
...
// get the <tbody> surrounding the list of referrals -- all data located within this table
	var mainTable = document.evaluate('//td[@bgcolor="#ffffff"]/ancestor::tbody[1]',
    document,
    null,
...
if(window.location.href.match('rreferrals.php')) directRefs = false; // default = true

if(directRefs) {
	var col_AVG = 7; // the average number of clicks the referral has made (per day) = COL 8
	var col_CLICKS = 4; // total number of clicks the referral has made = COL 7
	var col_LAST = 6; // date of last click = COL 6
	var col_SINCE = 2; // length of ownership of referral (today/yesterday/date) = COL 4
} else {
	var col_AVG = 7; // the average number of clicks the referral has made (per day) = COL 8
	var col_CLICKS = 4; // total number of clicks the referral has made = COL 7
	var col_LAST = 6; // date of last click = COL 6
