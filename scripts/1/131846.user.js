// ==UserScript==
// @name           KOC Taht Odası Botu (Turkısh ThroneRom Bot)
// @version        20120930
// @namespace      By_musty
// @homepage       http://userscripts.org/scripts/edit_src/131846
// @delay 1000
// @include        *.kingdomsofcamelot.com/*main_src.php*
// @include        *.kingdomsofcamelot.com/*standAlone.php*
// @include        *apps.facebook.com/kingdomsofcamelot/*
// @include        *kabam.com/kingdoms-of-camelot/play*
// @updateURL      https://userscripts.org/scripts/source/131846.meta.js
// @require        https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js
// @require        https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.21/jquery-ui.min.js
// @description    Taht Odası İtem İyileştirme ve Geliştirme Ayarları
// ==/UserScript==


var Version = '20120930_mm';
...
function attachTab()
{
    ...
    var str = unsafeWindow.cm.FETemplates.Throne.mainThrone.replace(
            ...
            //'<li id="throneStatTab" class="inactive"> Stats </li><li id="throneTest" class="inactive" onclick="hideShow()"> Ayarlar </li><li id="trexecselect" class="inactive" onclick="execSelect()">Select</li><li id="trexecupgrade" class="inactive" onclick="execUpgrade()">Upgrade</li><li id="trexecsalvage" class="inactive" onclick="execSalvage()">Salvage</li><li id="trtimerdisp" class="inactive">Timer</li>');
    '<li id="throneStatTab" class="inactive"> Stats </li><li id="throneTest" class="inactive" onclick="hideShow()"> Ayarlar </li><li id="trexecupgrade" class="inactive" onclick="execUpgrade()">Upgrade</li><li id="trexecsalvage" class="inactive" onclick="execSalvage()">Salvage</li><li id="trtimerdisp" class="inactive">Timer</li>');
...
Tabs.throneSalvage = {
        ...
        init : function (div) {
            ...
            t.myDiv = div;
            var m = '<Div><DIV id=trSalvage class=trStat>OTOMATİK SİLME</div>';
            m += "<div id='trInfoArea'>";
            m += '</div>';
            ...
            m += '<tr><th width=20%/><th width=30%/><th width=40%/><th/></tr>'
                if (salvageData.salvageActive == false) {
                    m += '<tr><TD><div><INPUT id=trSalvagerPower type=button value="Silme = KAPALI"/></div></td>';
                } else {
                    m += '<tr><TD><div><INPUT id=trSalvagerPower type=button value="Silme = AÇIK"/></div></td>';
                }


            m += '<td colspan=3><b> Eter Biriktirilcek olan Şehir: </b><div style="display: inline;" id=trSalvageCity /></td>';

            m += '<tr><td><td/><tdtyle="text-align: center;"></td>'; 
            m += '</tr></table>';

            m += '<table class=trTabPad><hr/>';
            m += '<tr><td width=35%><div> SİLME: <select id="trSalvageQuality">';
            m += '<option value="1">Yaygın</option>';
            m += '<option value="2">Alışılmamış</option>';
            m += '<option value="3">Nadir</option>';
            m += '<option value="4">Epik</option>';
            m += '<option value="5">Muhteşem</option>';
            m += '</select> ve daha üstü açık itemleri</div></td>';

            m += '<td colspan=2 width=24%>Silinmicek ilk <INPUT id=trSaveNum type=text size=3 maxlength=3 value="' + salvageData.throneSaveNum+ '"/> item</td>';
            m += "</tr></table><hr/>";
            ...
            m += '<tr><td>  <b>Silinmesini İstenmeyen İtemlerin Özeliklerini Seçiniz: </b> </td>';

            m += '<td alight="left"><div><span>İtem Cinsi: <select id="trFactionType">';
            ...
            m += '<td alight="left"><div><span>İtem Türü: <select id="trCardType">';
            m += '  <option value="any">Hepsi</option>';
            m += '  <option value="chair">Koltuk</option>';
            m += '  <option value="table">Sehpa</option>';
            m += '  <option value="window">Pencere</option>';
            m += '  <option value="banner">Bayrak</option>';
            m += '  <option value="advisor">Rahip</option>';
            m += '  <option value="trophy">Kafa</option>';
            m += '</select></span></div></td>';
            m += '<td align="right"><INPUT id=trAddRule type=button value="TÜRÜ KAYDET"/></td>';
            m += '</tr></table>';
            m += '<TABLE  class=trTabPad width=100% id="trConditionTable"  style="padding-left: 5px;">';
            m += '<tr><td align=left colspan=1><INPUT id=trAddRow type=button value="Basamak Ekle"/></td>';
            m += '<td></td><td></td><td></td><td></td><td></td></tr>';
            m += '</table>';
            m += '</div><hr/>';
            m += '<div id="trSalvStatus" style="text-align: center;" >Bekleyiniz ... </div>';
            m += '<div id="trNumSalv" style="text-align: center;">Silinen İtem Sayısı: '+ addCommas(salvageData.numSalvagedItems) + ' </div>';
            m += '<hr/>';
            m += '<div class=trRulePane>';
            m += '<div align=center> <b> Aşağıya Eklenen Özellikli İtemler Silinmicektir. </b></div>';
        ...
        buildRuleDisplay : function ()
        {
            ...
            for (i =0; i < salvageData.ruleSet.length; i++)
            {
                ...
                m += "</div></td>";
                m += "<td width=20%><INPUT id=trDelRule" + i + " type=button value='Türü Sil' /></td>";
                m += '</tr>';
            }
        ...
        doSalvage : function() {
            ...
            if (t.delItems.length > 0) {
                ...
                t.deleting = false;
                t.setStatus('Silinecek İtem Bulunmamaktadır.. Yeni İtemin Gelmesini Bekliyorum.');
            }
            t.setFullness();
        },
...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            m += "  <option value='both'>Hepsi</option>";
            m += "  <option value='buff'>Güçlendirici</option>";
            m += "  <option value='debuff'>Zayıflatıcı</option>";
            m += "</select></td>";
...
Tabs.upgrader = {
        ...
        qualities :  [ "Basit", 
                       ...
                       init : function (div){
                           ...
                           // header stuff
                           var m = '<Div id=trfield><DIV id=trUpgrader class=trStat>OTOMATİK GELİŞTİRME</div><TABLE id=trupgrader width=100% height=0% class=trTab>';
                           m+= '</table></div>';
                           ...
                           if (upgradeData.active == false) {
                               m += '<TD width=25%><div><INPUT id=trUpgraderPower type=button value="GÜÇ = KAPALI"/></div></td>';
                           } else {
                               m += '<TD width=25%><div><INPUT id=trUpgraderPower type=button value="GÜÇ = AÇIK"/></div></td>';
                           }

                           m += '<td width=25%><INPUT id=trOneItem type=checkbox '+ (queueData.oneItem ? ' CHECKED':'') +'/>  Her denemede 1 Yükseltme</td>';
                           m += '<td width=25%><INPUT id=trRepairAll type=checkbox '+ (upgradeData.repairAll?' CHECKED':'') +'/>  Arızalı İtemleri Tamir Et.</td>'; 
                           m += '<td width=25%/></tr>';

                           m += '  <tr><td colspan=3><div ><b>Eter Kullanılacak Şehir: </b><span id="trUpgradeCity"></span></div></td>';
                           ...
                           m += '<option value="upgrade">Geliştirme</option>';
                           m += '<option value="enhance">İyileştirme</option>';
                           ...
                           if (Seed.queue_throne != null && Seed.queue_throne.end != null)
                           {
                               ...
                               t.setStatus("Onarım Süresi " + n.toLocaleTimeString() + " kadar bekliyorum.  Item: " + unsafeWindow.kocThroneItems[t.repairId].name);
                       ...
                       doUpgrade : function(uItem) {
                           ...
                           t.setStatus("Yükseltme İsteği Gönderiliyor ...");
                           var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
                           params.ctrl = 'throneRoom\\ThroneRoomServiceAjax';
                       ...
                       buildQueueDisplay : function () {
                           ...
                           $("#trQueue").append("<tr><th width=5%>Kayıtlı</th><th width='8%'>Durum</th><th width='30%'>Item</th><th width='5%'>Yapılan İşlem</th><th width='5%'>Max</th><th width='45%'>Yapılan İşlem Durumu </th><th>Kaldır</th></tr>");
            ...
            .slideDown(700);
                       ...
                       togglePower: function(obj){
                           ...
                           if (upgradeData.active == true) {
                               upgradeData.active = false;
                               btn.value = "Yükseltme = KAPALI";
                               t.setStatus("Güç Kapalı");
                               
                           } else {
                               upgradeData.active = true;
                               btn.value = "Yükseltme = AÇIK";
                               t.setStatus("Güç Açık");
                           }
                           ...
                           new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                               ...
                               onSuccess: function (transport) {
                                   ...
                                   if(rslt.ok){
                                       ...
                                       var n = new Date(t.repairEnd *1000);
                                       t.setStatus("Onarım Başladı ... Onarımın Biticeği Süre " + n.toLocaleTimeString() + ". Item: "  + unsafeWindow.kocThroneItems[t.repairId].name);
                       ...
                       clearRepair : function () {
                           ...
                           if (timeUntilDone <= 0)
                           {
                               ...
                               if (t.repairId != 0 && unsafeWindow.kocThroneItems[t.repairId]  != null)
                               {
                                   ...
                                   {
                                       t.setStatus("Onarım İşlemi Tamamlandı.");
                                   }
...
function trPopup (prefix, x, y, width, height, enableDrag, onClose) {
    ...
    m += '<TR vAlign=top id="'+ prefix +'_bar" class="'+ topClass +'"><td width="95%"><div class="trTitle">ÖLÜMÜNE KANKAYIZ Arondight388 Azrail</div></td><TD><div id='+ prefix +'_X align=right valign=top onmouseover="this.style.cursor=\'pointer\'" class=trCloseSpan> </span></td></tr>';
