...
if (init) {
	...
	*  Words that are removed, and appear as the number zero (0), currently have no translation and the english version is used.
	...
	case "ua": //by Rustle rs11[@]ukr.net
		...
		aLangStrings = ["Побудувати пізніше", "Розвити пізніше", 0, "Тренувати пізніше", "Запланувати задачу.", "Ми почали будівництво ", 0, 0, " неможливо побудувати.", 0, "Задача запланована.", 0, "Ми не можемо планувати це зараз.", 0, "Заплпновані задачі", "Видалити", "Відправити пізніше", "Атака не може бути запланована, оскільки війська не вибрані.", "Ваші війська були відправлені", "Ваші війська не можуть бути відправлені", "Підкріплення", "Атакувати", "Розбійницький набіг", "Какапульти націлені на", "Випадково", "в", "чи через", "секунд", "хвилин", "годин", "днів", "Розвідати ресурси та військо супротивника", "Розвідати оборонні споруди та військо супротивника", "Відсутнє", "Атака неможе бути запланована бо немає цілі.", "Поле №.", "Сортувати:", "тип ", "час ", "ціль ", "настройки ", "селище "];
		break;
	...
	var l8, m8;  //Sorry for the names, i was just being funny.
	for ( m8 = 0, l8 = iMyRace.length ; m8 < l8 ; ++m8 ) {
		tA = iMyRace[m8];
		if ( tA.tagName != "LI" ) continue;
		tA = tA.getElementsByTagName("a")[0];
		tX = parseInt(tA.title.split("(")[1]);
		tY = parseInt(tA.title.split(")")[0].split(">").slice(-1));
	...
	var myID = myPlayerID.getElementsByClassName; // Save off into Recycled Variable for later use
	//Get Player ID  (uid)
	myPlayerID = myID("signLink")[0].href.split("=")[1];
	...
	if ( isNaN(iMyRace) || iMyRace < 0 ) {
		iMyRace = parseInt(myID("nationBig")[0].className.replace(/nationBig/g,"")) - 1;
...
function upgradebuild(aTask) {
	_log(3,"UpgradeBuild> Begin. aTask = ("+aTask.toSource()+")");
