// ==UserScript==
// @name               Sigma PV Trainer 
// @namespace      http://www.facebook.com/vortexgod Edited by: Alex "God"
...
function findLegendary() {
	var wildText = unsafeWindow.document.querySelector('#pkmnappear');
	if (!wildText) {
		moveAround();
		return;
	}
...
function findRare() {
	...
	unsafeWindow.console.info('Rare found ', wildText);
                        ...
                        function autoBattle() {
                         ...
                         var btnAttack = unsafeWindow.document.querySelector('[type="submit"]');
                         if (btnAttack) {
                         btnAttack.click();
           }
		...
		if (linkReturnToMap && linkReturnToMap.textContent.trim() == 'Return to the Map') {
			settings.data.nextBattle = new Date().getTime() + 0;
...
function init() {
	...
	createToggler(container, 'Alex ::: Auto-battle:', 'autoBattle');
	createToggler(container, 'Was ::: Find Rare:', 'findRare');
	createToggler(container, 'Here! :. Find Legendary:', 'findLegendary');
