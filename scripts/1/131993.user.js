// ==UserScript==
// @name          YousableTubeFix for Chrome
...
// The script needs a compatibility layer provided by Tampermonkey or similar addons
// Logs if unsafeWindow is available. If it isn't then Tampermonkey is probably not installed
if (typeof unsafeWindow === "undefined") {
	console.log("unsafeWindow is undefined. Is Tampermonkey installed?");
}
...
VideoAdapter.prototype.__defineGetter__("uwPlayer", function() {
	if (this.player.wrappedJSObject) {
		return this.player.wrappedJSObject;
	}
	else if (unsafeWindow.document.getElementById(this.playerId)) {
...
VideoAdapter.prototype.reloadPlayer = function() {
	this._isAPIReady = false; //'
	this.player.outerHTML = this.player.outerHTML;
...
GM_addStyle([
	...
	"#ytf-conf-dialog {position: fixed; margin: auto; left: 0px; right: 0px; top: 0px; bottom: 0px; width: 475px; max-height: 630px; z-index: 1001; " +
		"overflow: auto; padding: 5px; color: black; background-color: #EEEEEE; border-radius: 15px}",
	"#ytf-conf-dialog input[type='button'] {padding: 2px 6px}",
	...
	function createDialog() {
		...
		confDialog = createNode("div", {id: "ytf-conf-dialog"},
			{innerHTML: "<div id='ytf-conf-title'>Настройка YousableTubeFix</div>" +
				...
				"<ul>" +
				"<li>" +
				"<span>Размер видео по умолчанию:</span>" +
...
var scriptVersion = 1369330918399; // 23 May 2013
...
var scriptHomepageURL = "http://userscripts.org/scripts/show/57201";
var scriptVersionCheckJsURL = "http://dl.dropbox.com/u/3311359/YousableTubeFix/ytfChromeVersionCheck.js";
