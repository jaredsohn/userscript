// ==UserScript==
// @name           Travian4 Resource bar plus(mar_ram11)
// @namespace      mar_ram11
...
var langs = ['auto','English (en)','???? (ar)','????????? (bg)','Bosanski (bs)','Deutsch (de)','Español (es)','????? (fa)','Français (fr)','Hrvatski (hr)','Magyar (hu)','Italiano (it)','Polski (pl)','Português (pt)','Româna (ro)','??????? (ru)','?????? (sr)','Svenska (sv)','Türkçe (tr)','?????????? (ua)','Ti?ng Vi?t (vi)','?? (zh)'];
...
DICT = {
 ...
 ar: { // Arabic, thx ww_start_t
	...
	vi: { // VietNamese translation by Nguy?n Duy Thanh
		...
		savedelallh : 'b?n có ch?c mu?n xoá h?t d? li?u, bao g?m c? liên k?t và tên ph? c?u nó?',
