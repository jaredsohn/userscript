// ==UserScript==
// @name          kufotobook
// @namespace     http://userscripts.org/scripts/show/114257
// @version       0.1
// @dtversion     111002001
// @timestamp     1317675839731
// @description   theater foto effects for kufoto.com
// @author        idx(302101);
// @license       (CC) by-nc-sa 3.0
// @include       http://www.kufoto.com/*
// @include       http://kufoto.com/*
//
// -!--latestupdate
//
// v0.1.2beta - 2011-10-04 . 1317675839731
// infinite scroll proper condition
// dismiss effect by unique-class
//
// v0.1.1beta - 2011-10-02 . 1317537039071
// infinite scroll
//
// v0.1.beta - 2011-09-30 . 1317325598136
// init
//
// -/!latestupdate---
// ==/UserScript==
...
function startMain(){
	...
	var leng, node, atr, para, gotit=false, imgs = getTag('img');
	var classIdentity = 'eVF';
	leng=imgs.length;
	if( leng ) for(var i=0; i<leng; i++){
		node = imgs[i];
		//if(node.className.indexOf(classIdentity) != -1) continue;		
		atr = String(node.getAttribute('src'));
		if( atr && /\/uploads\/galleries\//i.test( atr ) && !node.getAttribute('border') ) {
			para = cariParent(node, 'A', 2);
			if( para ){
				para.setAttribute('onclick', 'return false');					
	...
	if(!gotit && !gvar.isChrome){ 
		//clog('not gotit...')
		_o("popstate", window, function(e){ location.href= location.href });
	}	
...
function parselink(lnk){	
	return lnk.replace(/\d{3}x\d{3}(\.\w+$)/i, "640x480" + "$1");
}
function findHref(e){
...
function is_denied_url(){
	var kelas = ($D('.polaroids').length + $D('.wide-polaroids').length);
	return ( kelas == 0 );
}
function click_photo(e){
	
	var tgt=e.target||e, ch, parts, ret=false, kika=false;	
	if(tgt.className.indexOf('uiButton')!=-1){
		...
		if(!parts) return;
		parts = parts.split(',');
		ret = parselink(parts[1]);
		tgt = null;
		kika = 1;
	}else if(tgt.nodeName=='IMG'){
		ret = parselink(tgt.src);
	}else{
		ch = getTag('img', tgt.parentNode); 
		tgt = ch[0];
		ret = parselink(tgt.src);
	}
	if(!kika){
	...
	if( ret ){
		var kufotolink = (tgt ? findHref(tgt) : parts[0]);
...
function is_keydown_pressed_ondocument(e){
	...
	if($D('#kfSnowBox')){
		switch(A){
			case 27: Layer.close('kfSnowBox'); 	break;
			case 37: Layer.nav_goto('prev'); 	break;
			case 39: Layer.nav_goto('next'); 	break;
		}
	}
	return false;
}
		...
		if(dest){
			...
			pos.forEach(function(no){
				if(isDefined( gvar.collections[no] )){
					el = createEl('img', { src: parselink( gvar.collections[no].src ) });
					Dom.add(el, dest);
				}
			});
		}
...
var rSRC = {
  getCSS: function(){
	...
	+'.kfSnowBox .container{background-color:#fff;box-shadow:0 0 1px rgba(0, 0, 0, .25), 0 1px 5px 3px rgba(0, 0, 0, .15), 0 5px 4px -3px rgba(0, 0, 0, .16);margin:9px auto 40px auto;max-width:960px;min-width:720px;padding:12px;width:720px}'
	+'.kfSnowBox .stageWrapper,.kfSnowBox .stagePagers{text-align:center;width:100%}'
	+'.kfSnowBox .stageWrapper{max-height:960px;min-height:402px;height:100%;cursor:default;position:relative}'
	...
	+'.kfSnowBox .stage{background:#f6f6f6;cursor:pointer;display:block;font-size:0;height:100%;max-height:960px;min-height:402px;position:relative;text-align:center;width:100%}'
 ...
 ,getTPL_layer_FB: function(){
   ...
   +'<div style="width: 720px;" class="container" id="mcontainer"><div style="height: 458px;" class="stageWrapper">'
   ...
   +'<div style="line-height: 458px;" class="stage"></div>' // main image container
   +'</div>' // .stageWrapper 
...
var AutoLo = {
	init: function(){
		...
		this.xPathNext = '//ul[@class="pagination"]/li[@class="next"]/a';
		this.state = 'enable';
	...
	,request_cb: function(res){
		...
		dest = $D('.//ul[contains(@class,"wide-polaroids")]', null, 1);
		...
		if(leng) {
			...
			for(var i=0; i<leng; i++){
				...
				atr = String(el.getAttribute('src'));
				if( atr && /\/uploads\/galleries\//i.test( atr ) && !el.getAttribute('border') ) {
					para = cariParent(el, 'A', 2);
					if( para ){
						para.setAttribute('onclick', 'return false');
