// ==UserScript==
// @name       MyEpisodesToTottrentz
// @namespace  http://www.myepisodes.com
// @version    0.1
// @description  Add a download link to Torrentz.eu while browsing MyEpisodes.com 
// @match      http://www.myepisodes.com/views.php
// @copyright  2012+, OW
// ==/UserScript==



var dlIcon = "data:image/png;base64,";


var shows = document.evaluate("//td[@class='showname']",
		document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);

for (var i = shows.snapshotLength - 1; i >= 0; i--) {
		var elm = shows.snapshotItem(i);
        var content = elm.firstChild;
        var link = document.createElement('a');
        var btn  = document.createElement('img');
        btn.src = dlIcon;
        btn.width=12;
        btn.height=12;
        link.appendChild(btn);
        var showName = content.textContent;
        showName.replace(" ", "+");
    link.href = "http://torrentz.eu/search?f=" + showName;
		content.insertBefore(link);
	}
