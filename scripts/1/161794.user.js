...
// @updateURL      https://userscripts.org/scripts/source/161794.meta.js
// @downloadURL    https://userscripts.org/scripts/source/161794.user.js
// @author         congxz6688
// @version        2013.3.18.0
// ==/UserScript==
...
replyCss += "#faceTextDiv{background-color:lightGrey; width:653px;}#faceTextDiv td{padding:0px !important;}";
...
replyCss += ".qdisplayDiv{margin-left:3px; width:137px; display:inline-block;}.face_button{display: inline-block !important;}"
...
function insertEdiror(e) {
	...
	document.getElementById("atc_content").value = document.getElementById("atc_content").value.slice(0, document.getElementById("atc_content").selectionStart) + InsertText + document.getElementById("atc_content").value.slice(document.getElementById("atc_content").selectionEnd);
	closeFaceTxttable();
}
//列表创建函数
...
function createFaceTexttable(ete) {
	if ($("#faceTextDiv").length == 0) {
		var ortt = ete.target;
		var getTop = getElementTop(ortt) - 100;
		//定位主列表位置
		var faceTextDiv = $("<div>", {
				id : "faceTextDiv"
			}).css({
