...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;# of fights: <input id="dmgFights" name="dmgFights" value="30" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">go!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>30 fights: </strong>' +
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 100, 30, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 100, 30, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	content.prepend(
		'<div id="myStatBoxL" style="position: absolute; top: 418px; left: 22px; width: 250px; opacity: 0.7;"></div>' +
		'<div id="myOverBox" style="position: absolute; top: 290px; left: 275px; width: 250px; opacity: 0.7;"></div>'
	);
	content.append(
		'<div id="myStatBoxR" style="position: absolute; top: 418px; left: 488px; width: 250px; opacity: 0.7;"></div>'
	);
	...
	// check hospital   BESTRAGIJA MENJAO
	//MENJANOsetInterval(function() {

	//MENJANO	var h = $j('#hospital_btn');

	//MENJANO	if ($j('#hospital_btn small').html() != '0' && !h.hasClass('disabled') && unsafeWindow.SERVER_DATA.onlySpectator == 0) {

	//MENJANO		bLog('Getting EXTRA wellness from hospital...');
	//MENJANO		unsafeWindow.useHospital();
