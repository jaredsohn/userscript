...
function beatmaplist(){
	...
	$.ajax({
		...
		success: function(json){
			...
			$.each(json,function(i,o){//枚举json并在在相应的div插入链接,参数i是索引,参数o是索引指向的值等同json[i]
				var c="| <a href='http://osz.wo.tc/s/" + o.id + "' target='_blank'>分流(Mediafire)</a>";
	...
	$.ajax({
		...
		success: function(json){
			//json返回的数组
			$.each(json,function(i,o){//枚举json并在在相应的div插入链接,参数i是索引,参数o是索引指向的值等同json[i]
				var c="| <a href='http://osz.wo.tc/d/" + o.id + "'>分流(osz.wo.tc)</a>";
				var t=$("#info"+o.id+" a:eq(1)");//登陆后的插入点
				if(!t.length) t=$("#info"+o.id+" a:eq(0)");//登陆前的插入点
				t.after(c);
			});
...
function detail(){
	...
	var url2="<a href='http://osz.wo.tc/s/'" + id + ">切换到从osz.wo.tc下载</a>"
	html=$(url2);
	html.click(conver);
