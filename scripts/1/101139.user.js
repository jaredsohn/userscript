// ==UserScript==
// @name           genesisnew_BOSS_AutoPlayer
// @namespace      civwar
// @description    civwar2012 Boss AutoPlay
// @version        1.09
// @include        http://2012.by.ca.cutedna.com/*
// ==/UserScript==
var thisVersion = "1.09";
/*!
...
// update script from: http://userscripts.org/scripts/review/100860
var SUC_script_num = 101139;
...
dragHandler:function(e){
	...
	var target = document.getElementById("civwarap_div");// != null ? e.target : e.srcElement;
...
SetupDivs:function() {
    ...
    //var b=nHtml.FindByAttrContains(document.body, 'div', 'id', 'main');	
	//nHtml.FindByAttr(document.body, 'div', 'className', 'UIStandardFrame_SidebarAds').style.display='none';
	//nHtml.FindByAttr(document.body, 'div', 'className', 'UIStandardFrame_SidebarAds withCanvasNav').style.display='none';
...
AppendTextToDiv:function(divName,text) {
	var d=document.getElementById('civwarap_' + divName);
	if(d) {
		d.innerHTML  += text;
		return true;
	} else return false;
...
SetControls:function(force) {
	...
	var htmlCode = '';
	htmlCode += '<hr />' + this.ToggleControl('TrainingBoss','訓練BOSS');
		htmlCode += '<table width=700 cellpadding=0 cellspacing=0>';	
				var BossList = ['1.恐龍','2.猛馬象','3.混沌獸','4.九尾狐','5.影子武士','6.圓桌騎士','7.狼人','8.黑暗女巫','9.傀儡木人','10.海盜船長','11.金剛','12.史萊克','13.死亡騎士','14.地心魔獸','15.奧特曼','16.地球超人','17.蝙蝠俠','18.黑衣人','19.閃電俠','20.忍者神龜','21.鷹女俠','22.機械鯊魚','23.綠燈俠','24.數碼精靈','25.機器俠','26.動感超人','27.機器人瓦利','28.紅蜘蛛','29.威震天','30.大黃蜂','31.擎天柱','32.神奇女俠','33.機器怪獸','34.蜘蛛人','35.鋼鐵俠','36.超人','37.克隆人','38.科學怪人','39.阿凡達','40.神聖天使獸'];
				//htmlCode += '<table width=180 cellpadding=0 cellspacing=0>';
				htmlCode += '<tr><td width=100><font color="#FFFFFF">BOSS訓練營:</font></td><td width=80>' + this.MakeDropDown('Boss',BossList,'',"style='font-size: 10px'") + "</td>";
 			  	htmlCode += '<td width=180 align="left"><font color="#FFFFFF">自動隨時滿兵(國庫). ' + this.MakeCheckBox('MoneyBuySolider',false) + '<input type="text" id="soldierNums" size="10" value=""></p></td></tr>';
		    	var PowerList = ['1','10','50','100'];				
    			htmlCode += '<tr><td width=80><font color="#FFFFFF">攻擊次數:</font></td><td>' + this.MakeDropDown('TrainingPower',PowerList,'',"style='font-size: 10px;'") + "</td>";
 			  	htmlCode += '<td width=180 align="left"><font color="#FFFFFF">低於20%自動滿兵(神域50豆豆). ' + this.MakeCheckBox('GodFillMilitaryEnergy',false) + " </td></tr>";
    			htmlCode += '<tr><td width=80><font color="#FFFFFF">訓練次數:</font></td><td>' + this.MakeNumberForm('TrainingTimes','','100',"type='text'  size='12' style='font-size: 10px'") + " </td>";		
 			  	htmlCode += '<td width=180 align="left"><font color="#FFFFFF">自動補充軍事資源包(神域200豆豆). ' + this.MakeCheckBox('GodFillMilitaryEnergy',false) + " </td></tr>";
   	    		htmlCode += "<tr><td><input type='button' id='TrainingBoss' value='開始' style='font-size: 10px; width:50; height:50'></td></tr></table>";			
	htmlCode += "<hr/> </div>";
	...
	this.AddListeners('civwarap_div');

	var TrainingBoss=document.getElementById('TrainingBoss');
	TrainingBoss.addEventListener('click',function(e){
			var CurrentBoss = gm.getValue('Boss');
			gm.setValue('CurrentBoss',CurrentBoss);
			var CurrentBossNum = CurrentBoss.substr(0,CurrentBoss.indexOf(".",0));
			gm.setValue('CurrentBossNum',CurrentBossNum);
			var CurrentTrainingPower = gm.getValue('TrainingPower');
			gm.setValue('CurrentTrainingPower',CurrentTrainingPower);
			var CurrentTrainingTimes = gm.getValue('TrainingTimes');
			gm.setValue('CurrentTrainingTimes',CurrentTrainingTimes);
			var i=0;
			civwarap.ReloadTraining(0,CurrentTrainingTimes,CurrentBossNum);
	},false);
	...
	globalContainer.addEventListener('DOMNodeInserted', function(event) {
 		...
 		if(document.querySelector('#all_faz pleft5')) {
			nHtml.setTimeout(civwarap.addExpDisplay, 0);
		}
	}, true);
	...
	globalContainer.addEventListener('click', function(event) {
		...
		if(obj && obj.href) civwarap.clickUrl = obj.href;
//		gm.log('global container ' + civwarap.clickUrl);
	}, true);
    
},

/////////////////////////////////////////////////////////////////	
ReloadTraining:function(i,CurrentTrainingTimes,CurrentBossNum) {	
	if (i < CurrentTrainingTimes){
		if ($("#soldiers_area").text().replace(/,/g,"")-2 < $("#total_population_area").text().replace(/,/g,"") && gm.getValue('MoneyBuySolider'))
		{
		    var need_soldiers = $("#total_population_area").text().replace(/,/g,"") -$("#soldiers_area").text().replace(/,/g,"") -2;
		    document.getElementById('soldierNums').value = need_soldiers;
			if (need_soldiers > 0)
				unsafeWindow.civWarCls.doBuySoldiers();
			this.waitForPageChange=true;
			nHtml.setTimeout(function() { this.waitForPageChange=false; this.doTrainingBoss(i,CurrentTrainingTimes,CurrentBossNum);},civwarap.waitMilliSecs * (1 + Math.random() * 0.5));
		}
		
		//兵力低於20%才用神域補兵
		if ($("#soldiers_area").text().replace(/,/g,"")-2 < $("#total_population_area").text().replace(/,/g,"") *0.2 && gm.getValue('GodFillMilitaryEnergy'))
		{
		...
		if ($("#military_energy_area").text()-1 < gm.getValue('CurrentTrainingPower') && gm.getValue('GodFillMilitaryEnergy'))
		{
			unsafeWindow.civWarCls.doGod('FullMilitaryEnergy');
			this.waitForPageChange=true;
			nHtml.setTimeout(function() { this.waitForPageChange=false; this.doTrainingBoss(i,CurrentTrainingTimes,CurrentBossNum); },civwarap.waitMilliSecs * (1 + Math.random() * 0.5));
		}
	    else if ($("#military_energy_area").text()-1 < gm.getValue('CurrentTrainingPower') && !gm.getValue('GodFillMilitaryEnergy'))
		{
			this.waitForPageChange=true;
			nHtml.setTimeout(function() { this.waitForPageChange=false; this.doTrainingBoss(i,CurrentTrainingTimes,CurrentBossNum); },civwarap.waitMilliSecs * (1 + Math.random() * 0.5));
		}		
		
	  this.doTrainingBoss(i,CurrentTrainingTimes,CurrentBossNum);	  	  
	}	
},

doTrainingBoss:function(i,CurrentTrainingTimes,CurrentBossNum) {
  	window.setTimeout(function() {
		i++;
		unsafeWindow.civWarCls.tBoss(CurrentBossNum,gm.getValue('CurrentTrainingPower'));
		civwarap.ReloadTraining(i,CurrentTrainingTimes,CurrentBossNum);
	  }, 1500);	
},
...
addExpDisplay:function() {
    ...
    if (/\(/.test($("#totalXpArea").text())) {
    	return false;
    }
    var arrExp = $("#totalXpArea").text().split("/");
    $("#totalXpArea").append(" (<span style='color:red'>"+(arrExp[1] - arrExp[0])+"</span>)");
},
		...
		if (inputDiv.type=='checkbox') {
			inputDiv.addEventListener('change',function(e) {
				...
				if (e.target.className) civwarap.SetDisplay(e.target.className,e.target.checked);
					...
					if (value) {
						civwarap.SetDisplay('Status_Normal',false);
						civwarap.SetDisplay('Status_Adv',true);
						for (var n=1; n<=5; n++) {
							gm.setValue('AttrValue' + n,'');
						}
					} else {
						civwarap.SetDisplay('Status_Normal',true);
						civwarap.SetDisplay('Status_Adv',false);
					}

					civwarap.SetControls(true);
				}
			},false);
...
GetStats:function() {
	...
	if (navigator.userAgent.toLowerCase().indexOf('firefox') == -1) {
		// Facebook ID
		var webSlice=nHtml.FindByAttrContains(document.body,"a","href","act=king");
		if (webSlice) {
	...
	// rank
	var attrDiv =nHtml.FindByAttrContains(document.body,"div","class",'civwar_top');
	if (attrDiv) {
...
CheckResults:function() {
	...
	if ((resultsFunction) && !civwarap.WhileSinceDidIt('SetResultsFunctionTimer',20)) civwarap[resultsFunction](resultsText);
...
WaitMainLoop:function() {
	this.waitForPageChange=true;
	nHtml.setTimeout(function() { this.waitForPageChange=false; civwarap.MainLoop(); },civwarap.waitMilliSecs * (1 + Math.random() * 0.5));
}
};

//if(gm.getValue('SetTitle')) {
//        document.title=gm.getValue('PlayerName','Genesisnew');
//}
...
$(function() {
	//gm.setValue('civwarapPause','none');
	civwarap.clickUrl=window.location.href;
	// todo figure out way to print out the querySelector value for refined function calls
	civwarap.JustDidIt('PageLoad');
	civwarap.addExpDisplay();
...
LoadMenu:function(state){
	...
	var oldColor = 0;
	if(state == 'Start')
	oldColor = document.getElementById("civwarap_StyleColorStarted").value;
	else oldColor = document.getElementById("civwarap_StyleColorStoped").value;
...
ChangeBackGround:function(){
	...
	if(state == 'Start') {
			...
			document.getElementById("civwarap_div").style.background = gm.getValue('StyleBackgroundLight','#efe');
	}else{
			...
			document.getElementById("civwarap_div").style.background = gm.getValue('StyleBackgroundDark','#fee');
	}
	civwarap.SetControls(true);
},
...
endMovement:function(){
	if(document.getElementById("civwarap_div") && gm.getValue('state'))style.ChangeBackGround(gm.getValue('state','Start'));
