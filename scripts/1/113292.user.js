// ==UserScript==
// @name           Doacoes de Comunidade Elite MOD
// @namespace      editado por JoãoPaulo | uid=17288583650260853947
...
var colocaNaPagina = function() {
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "[green][b]Titulo:[/b][/green] ".concat(titulo, breakLine, "[green][b]Criador:[/b][/green] ", uid, breakLine, "[green][b]Motivo:[/b][/green] ", motivo, breakLine, "[green][b]Horario:[/b][/green] ", pegaData(), breakLine, breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent("TÃ³pico Deletado:"),
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = "\n";
		var rnd = String(Math.random());
		var texto = "[b][green]Membro:[/b][/green] ".concat(uid, breakLine, "[b][green]Motivo:[/b][/green] ", motivo, breakLine, "[b][green]Horario:[/b][/green] ", pegaData(), breakLine, breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tÃ³pico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('digite o motivo de estar deletando este topico:\\n','T\Ã³pico de spam'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
