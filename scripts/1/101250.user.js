...
// @require        http://tomchapin.me/auto-updater.php?id=101250
// ==/UserScript==


var Version = '20110416c';
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){
    ...
    for(i=0; i < Cities.numCities; i++) { 
      ...
      row[i] = (rp[1] - usage);
      var timeLeft = parseInt(Seed.resources["city" + Cities.cities[i].id]['rec1'][0]) / 3600 / (0-totalusage) * 3600;
      var msg = '';
      if (Options.enableFoodChatWarn &&timeLeft >= 0 && timeLeft < Options.foodChatWarnHours*3600) {
        msg += 'My city at (' + Cities.cities[i].x +','+ Cities.cities[i].y + ')';
        msg += ' is short on food. I have ' + timestrShort(timeLeft) + 'left and am' + addCommas(totalusage) + ' per hour.';
        sendChat ("/a " + msg);
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=pballowWinMove type=checkbox /></td><TD>Enable window drag (move window by dragging top bar with mouse)</td></tr>\
         ...
         <TR><TD><INPUT id=pbEnableFoodChatWarn type=checkbox /></td><TD>Post to chat if food will run out in less than \
