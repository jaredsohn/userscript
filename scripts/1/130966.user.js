// ==UserScript==
// @name           Advance Expand URL - Expand short url using longurl.org service
// @description    Show preview long url when mouse over short url
// @include        http*
// @date           20120903
// @version        3.1
// @author          energetictree
// @license        GPL GNU General Public License - http://www.gnu.org/copyleft/gpl.html  
// ==/UserScript==

//
// Based in original userscript ExpandeURL
// version 2.0
// Copyright (c) 2008, Antonio Rguez. Capita
...
(function() {
	...
	modifyShortLinks = function() {
		...
		for (var x = 0; x < links.snapshotLength; x++) {            		 
			...
			// Filter by shortening regular domains
			var domain = href.match(/^https?:\/\/?(.+\.co|.+\.ly|.+\.to|.+\.in|.+\.me|.+\.kcy|.+\.as|.+\.cc|.+\.gl|.+\.mp|.+\.ws|.+\.gd|.+\.us|.+\.it|.+\.sm|.+\.me|.+\.de|.+\.be|.+\.bz|.+\.ps|.+\.gs|.+\.am|.+\.tl|.+\.tk|.+\.ph|.+\.fm|.+\.pr|.+\.tc|.+\.ac|.)\//i);				
                        if (!domain) {
                                domain = hostname.match(/(^|\.)(|4sq|tinyurl|feedly|mylikes|path|urbanup|mixtab|bitly|jav4you|rtify|alturl)\.com$/);
                        }
			
			if (domain) {													
				// Check if the link is outside the current domain
	...
	expandLink = function(anchor, e) {
		...
		tooltip('<img src="https://lh4.googleusercontent.com/-WLxQsmKfzyY/T3VpsWo-xDI/AAAAAAAAACA/1waZLlslXMs/s16/cargando.gif" style="margin:4px 0 0 10px;float:right" alt="Please wait"><strong>Expanding</strong>', e);
				...
				onload: function(response) {
					...
					if (typeof(data.messages) !== 'undefined') { // There was an error
					    ...
					    if (typeof(data['title']) !== 'undefined') {
					        result = '<div style="color:#8DC7F1;font-weight:bold;margin-bottom:10px;width:100%">'+data['title']+'</div><a href="'+data['long-url']+'" style="color:#E0E9FF;" target="_blank">'+result+'</a>';
					    }
					    result += ' <a href="http://longurl.org/expand?url='+encodeURIComponent(anchor.href)+'&amp;src=lme_gm" title="More Information click this link" style="color:#E9FE94;">[info]</a></div>';
