...
// @version        0.0.1
// @include        http://ts3.travian.asia/dorf1.php
// ==/UserScript==
...
var a = "\
   Authors:\
     ...
     small modifications by Serj_LV.\
";
/**
 * Date:
 ...
 * Changelog:
 *   24 Feb 2011, Serj_LV
 *     - small correction for support Chrome and Opera. And for Travian version 4.0
 *   15 Sep 2009, deFox:
...
troopTarget[0] = [-44,-110,2];
//troopTarget[1] = [-44,-110,2];
//troopTarget[2] = [-44,-110,2];
...
// Now sub-functions will be defined
  ...
  function prepareDivDocking()
  {
    ...
    if (divDock == null)
    {
      ...
      if (divlmidall == null)
        divlmidall = find("//div[@id='mid']/div[@id='side_info']", XPFirst);
      if (divlmidall == null)
...
//      GM_log(text);
  ...
  {
    if (logDebugMesages)
		Debug_Out('debug:'+text);
//      GM_log(text);
  }

	function Debug_Out (text) {
		var dDiv = $g('ts_debug_out');
		if( dDiv ) {
			dDiv.appendChild($ee('P',text));
		} else {
			document.body.appendChild($ee('DIV',$ee('P',text),[['id','ts_debug_out']]));
		}
	}
...
var ver4FL =  $g('side_navi') ? false: true;

//function ajaxRequest(url, aMethod, param, onSuccess, onFailure) {
	function TS_postRequest(reqUrl,reqData,reqCallback) {
		ajaxRequest(reqUrl,'POST',reqData,function(ajaxResp) {
			reqCallback(ajaxNDIV(ajaxResp));
		}, function(ajaxResp) {
			reqCallback(ajaxNDIV(ajaxResp));
		});
	}
...
*/
   ...
   */
	function TS_getRequest(reqUrl,reqCallback) {
		ajaxRequest(reqUrl,'GET',null,function(ajaxResp) {
			reqCallback(ajaxNDIV(ajaxResp));
		}, function(ajaxResp) {
			reqCallback(ajaxNDIV(ajaxResp));
		});
...
function toSeconds(hTime) {
  ...
  function formatTimeStr(tmWhole) {
	return formatTimeStrReal(Math.abs(tmWhole) + getServerTime())
  }
  function formatTimeStrReal(tmWhole)
  {
  ...
  {
    travActiveVilliage = parseInt(RB_getValue("travActiveVilliage", 0));
    travActiveVilliageStr = RB_getValue("travActiveVilliageStr", "");
  }
  ...
  function setTroopSaveState(nState,tmReloadWait)
  {
    ...
    if (troopSaveState != TSState_Monitoring)
      ...
      reloadOn = true;
    RB_setValue("troopSaveState", troopSaveState);
    troopSaveErrors = 0;
    RB_setValue("troopSaveErrors", troopSaveErrors);
    if (tmReloadWait)
    {
  ...
  {
    troopSaveState = RB_getValue("troopSaveState", TSState_Monitoring);
    if (troopSaveState != TSState_Monitoring)
      reloadOn = false;
    else
      reloadOn = true;
    troopSaveErrors = RB_getValue("troopSaveErrors", 0);
    if (autoReload)
    {
      sheduleReloadCheck(tmIdleReloadExact*1000);
    }
  ...
  function getVillagesTableNode(doc)
  {
    ...
    if (tag == null)
      tag = find(".//div[@id='villageList']", XPFirst, doc);
    if (tag == null)
      return null;
    return tag;
  }
  ...
  function getAllVillagesCount(doc)
  {
    ...
    vCount = ver4FL ? find("count(.//li/a[contains(@href,'?newdid=')])", XPNumber, tag):
		find("count(.//td//a[contains(@href,'?newdid=')])", XPNumber, tag);
  ...
  function getVillageStr(doc,nVillage)
  {
    ...
    hrefs = ver4FL ? find(".//li/a[contains(@href,'?newdid=')]", XPList, tag):
		find(".//td//a[contains(@href,'?newdid=')]", XPList, tag);
