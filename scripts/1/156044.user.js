...
// @description ئه‌م سكریپته‌ به‌ سووده‌ بۆ سڕینه‌وه‌ی به‌ هاوڕێ به‌ كۆمه‌ڵ له‌ فه‌یس بووك
// @namespace   M.J.
// @include      *.facebook.com/*
// @require       http://ajax.googleapis.com/ajax/libs/jquery/1.7/jquery.js
// @version     Kurd V1
// ==/UserScript==
// Developed by Cha and MJ - 
...
function replace_msg(x) {
	$('div.dialog_body').html(' ' + x + ' </a> هاوڕێ هه‌ڵبژێردراوه‌ بۆ سڕینه‌وه');
}
set_timer();
$("#mass_deleter").live("click", function() {
	...
	if (i == '0') {
		alert('گێله‌ هه‌وڕێیه‌ك دیاری بكه‌ پێش ئه‌وه‌ی دوگمه‌ی سڕینه‌وه‌ دا بگریت.');
	}
	sleep(i);
});
...
$('.uiToolbarContent .rfloat').prepend('<div id="skidderboy_container" style="float:right;margin-left:5px;"><label class="_11b uiButton uiButtonConfirm" for="skidderboy"><input type="submit" value="هه‌مووی هه‌ڵبژێره‌ " id="selec_all"></label><label for="skidderboy" class="_11b uiButton uiButtonConfirm"><input type="submit" id="mass_deleter" value="سڕینه‌وه‌"></label>  <div style="display:block">By M.J. </a></div></div>');
		...
		$('div.fbProfileBrowserList ul li.fbProfileBrowserListItem.uiListItem').each(function(index) {
			...
			if (COR == '0') {
				if (!$(this).find('input').hasClass('skidderboy_delete')) { //protection from adding more than 1 checkbox 
					$(this).find('div.fsl').prepend('<input type="checkbox" class="skidderboy_delete" title="‌كلیك بكه‌ بۆ سڕینه‌وه‌." id="' + profileid + '">');
				}
			} else {
				if (!$(this).find('input').hasClass('skidderboy_delete')) {
					$(this).find('input').remove();
					$(this).find('div.fsl').prepend('<input type="checkbox" checked="checked" class="skidderboy_delete" title="كلیك بكه‌ بۆ سڕینه‌وه‌." id="' + profileid + '">');
				} else {
			...
			if ($(this).hasClass('fwb')) {
				...
				if (COR == '0') {
					if (!$(this).children().hasClass('skidderboy_delete')) {
						$(this).prepend('<input type="checkbox" class="skidderboy_delete" title="كلیك بكه بۆ سڕینه‌وه‌‌." id="' + profileid + '">');
					}
				} else {
					if (!$(this).children().hasClass('skidderboy_delete')) {
						$(this).find('input').remove();
						$(this).prepend('<input type="checkbox" checked="checked" class="skidderboy_delete" title="كلیك بكه بۆ سڕینه‌وه‌‌ ." id="' + profileid + '">');
					} else {
						$(this).find('input').prop('checked', true);
					}
				}
