// ==UserScript==
// @name           Facebook: InviteThemAll 3.0
...
// ==ChangeLog==
// @history        3.00 (12.12.2013) Major update. Cleaned up and enhanced code, improved detection methods, improved cross-browser compatibility and added donation dialogs.
...
(function () {
    ...
    var
        ...
        infoAlert = function (header, elms, relativeElement, infoAlertCallback) {
            ...
            try {
                ...
                if (relativeElement) {
                    ...
                    // Remove the width and height of the container from the given positions,
                    // to make the lower left corner what the position is relative to.
                    var pixelsFromLeft = relativeElementPosition.fromLeft - offsetPixelsToRight, pixelsFromTop = relativeElementPosition.fromTop - offsetPixelsToBottom - container.offsetHeight;

                    container.style.left = pixelsFromLeft.toString() + 'px';
                    container.style.top = pixelsFromTop.toString() + 'px';
                // Else, set it to default values.
                } else {
                    container.style.left = '4em';
            ...
            // If nothing else works, fall back to window.alert.
                ...
                dbgLog('error utilizing custom error alert dialog. ' + f);

                // 
                var messageInString = '';

                for (i = 0, l = elms.length; i < l; ++i) {
                    messageInString += elms[i].textContent + '\n';
                }
        ...
        // Function to show 3.0 update message.
        show3p0UpdateDialog = function () {
            infoAlert('- Major update -', [
                'Welcome to Facebook: InviteThemAll version 3.0!',
                '',
                'First of all, I\'m truly sorry for the waiting time',
                'of this fix. I had not developed much in nearly a year,',
                'and when I started tweaking this again, I could not',
                'settle with the current state of the code.',
                'So, I opted for yet another giant rewrite.',
                'Geez, I have to stop doing that.',
                '',
                'Now, you are currently using a /much/ optimized version,',
                'performing and looking better, as well as being based on',
                'much cleaner code. I also (finally) decided to use some',
                'time on cross-browser compatibility. There you go :)',
                '',
                'Some of you have asked for a donation option earlier,',
                'but I have not been old enough to create a PayPal',
                'account before just recently. I\'ve now embedded a',
                'PayPal donation option. If you are one of the countless',
                '(well, a decent part of 444\'000) users who actually are',
                'monetarily dependent on this script, please consider',
                'sending a little thanks to a humble student developer.',
                'I would truly appreciate :)',
                '',
                generateDonationButton(true),
                '',
                'Anyways, continue having fun with this little script.',
                'What began in 2010 as a little snippet to help my sister,',
                'has evolved into a weird project spanning 450k users.',
                'Go and Invite Them All! :D']);
        },
        ...
        selectFriends = function (btn) {
            ...
            if (!flags.selectButtonClicked) {
                ...
                // Show 3.0 update msg at second usage after updating. Cannot be at first, as will always be shown if problems with persistent storage.
                if (timesSelectedFriends === 2) {
                    show3p0UpdateDialog();
                }
            // Unselect friends.
            } else {
                ...
                while (i--) {
                    ...
                    if ((pageElements.collections.friendCheckBoxes[i].checked && !pageElements.collections.friendCheckBoxes[i].disabled) || forceChoose) {
                        ...
                        // If found invite button does not have the same ID as custom invite button would have, invite button is not a custom invite button.
                        currentLink = userInput.buttons.inviteButton.id !== ids.inviteButtonCustom ?
                                // The anchors are hopefully the first link within the current friend.
        ...
        showOptionDialog = function (relativeElement) {
            ...
            document.getElementById(ids.optionDialog);
            ...
            var homePageLink = createLink(links.homePage, 'home page');
            var bugLink = createLink(links.bug, 'bug reporting');
            var suggestionLink = createLink(links.suggestion, 'script suggestions');
            var linkContainer = createContainer('div', [
            ...
            // Assign value from persistent storage.
            userInput.fields.autoscrollWaitForNewElementsInMsField.value = getPersistentValue(persistentStorage.autoscrollWaitForNewElementsInMs);
            ...
            values.fromFieldPlaceholders.span = 'first';
            values.fromFieldPlaceholders.amount = 'first';
            userInput.fields.fromField.setAttribute('placeholder', values.fromFieldPlaceholders[getPersistentValue(persistentStorage.selection.type)]);
            ...
            values.toFieldPlaceholders.span = 'last';
            values.toFieldPlaceholders.amount = 'all';
            userInput.fields.toField.setAttribute('placeholder', values.toFieldPlaceholders[getPersistentValue(persistentStorage.selection.type)]);
            // Assign value from persistent storage if present.
            ...
            userInput.sections.optionDialog = infoAlert('Options', [
                startMessageContainer,
                autoscrollContainer,
                selectSpanTypeContainer,
                selectSpanContainer,
                '',
                devSettingsMessage,
                inviteDialogCustomInputContainer,
                inviteDialogDeepSearchContainer,
                '',
                userInput.buttons.clearButton
            ], relativeElement);
        ...
        addFriendEnumeration = function () {
            ...
            dbgLog('adding friend enumeration to ' + pageElements.collections.friends.length + ' friends');
        ...
        getFriends = function () {
            ...
            var friends = pageElements.collections.friends || [],
                n = 0, friend0ParentNumbern = pageElements.collections.friendCheckBoxes[0], friend1ParentNumbern = pageElements.collections.friendCheckBoxes[1],
