...
* Clicking outside of Facebook Fixer's popups will close them.
...
var lang = {
	...
	en : {
		...
		'ConfDownloadVideo' : 'Add a link to download the videos from video pages. (You may need an <a href="http://sqoatydhent.blogspot.com/" target="_blank">FLV player</a>)',
		...
		'ConfGoogleCalendar' : 'Add links to add birthdays and events to <a href="http://sqoatydhent.blogspot.com/" target="_blank">Google Calendar</a>.',
		'ConfGoogleLanguage' : 'Language for <a href="http://sqoatydhent.blogspot.com/" target="_blank">Google Translate</a>',
		...
		'ConfiCalendar' : 'Add links to download an <a href="http://sqoatydhent.blogspot.com/" target="_blank">Calendar</a> file with all birthdays.',
		...
		'ConfUpdates' : 'Check Userscripts daily for updates to Facebook Fixer. Or <a href="#" id="fbfUpdateLink" onclick="return false;">check now</a>.',
	...
	id : {
	   ...
	   'Close' : 'Tutup',
	   'ConfigureFacebookFixer' : 'Atur Facebook Fixer',
	   ...
	   'ConfFacebookFixerLanguage' : 'Bahasa untuk Facebook Fixer',
	   'ConfFacebookTimestamps' : 'Tampilkan cap waktu Facebook (contoh "3 jam lalu").',
	   'ConfFBFTimestamps' : 'Tambahkan cap waktu Facebook Fixer setelah cap waktu Facebook (contoh "11:45").',
	   'ConfFBFTimestamps24' : 'Tampilkan cap waktu Facebook Fixer dalam format 24 jam.',
	   ...
	   'ConfHomeLink' : 'Tampilkan tautan Beranda pada menu bar atas.',
	   ...
	   'ConfSectionAbout' : 'Tentang Facebook Fixer',
	   ...
	   'ConfUpdates' : 'Cek Userscripts.org setiap hari untuk update Facebook Fixer. Atau <a href="#" id="fbfUpdateLink" onclick="return false;">cek sekarang</a>.',
	   ...
	   'FacebookFixerConflict' : 'Facebook Fixer sekarang dikenal dengan nama Facebook Fixer.<br /><br />Karena pergantian nama Anda harus menguninstal Facebook Fixer dari browser Anda, atau dua script ini akan bertentangan satu sama lain.<br /><br />Jika Anda tidak mengetahui cara untuk menguninstal script ini, <a %s>Klick disini untuk tata caranya</a>.',
	   ...
	   'UpdateAvailable1' : 'Update tersedia untuk Facebook Fixer',
...
addStyle(
	...
	'.Facebook Fixer-highlighted-story, .Facebook Fixer-highlighted-story * { font-weight:bold !important; }'
);
...
if (id!=0) {
	setTimeout(function() {
		if ($('#FBPPdiv') && (true || parseInt(getValue("LastConflictCheck", "0")) + 86400000 <= (new Date().getTime()))) {
			...
			showDialog('<div class="fbfImportant">Facebook Fixer</div><br />' + $l('FacebookFixerConflict', 'href="http://www.code-poet.net/userscripts/Facebook Fixer/upgrading-from-facebook-fixer.html" target="_blank"'), '', 'small');
		}
	}, 2000);
}
...
function showDebugInfo() {
	try {
		showDialog(
			'Facebook Fixer Debug Info:<br /><br />'+
			'<table id="ff-debug">'+
			row(['version: ',version])+
		...
		'<table id="fbfConfigContainer">'+
			...
			'</td><td id="fbfConfigControls">'+
				...
				'<div id="fbfConfigControl-7">'+
					...
					'<table style="margin-left:-3px;">' +
					'<tr><td><span class="fbfLabel">' + $l('ConfFacebookFixerLanguage') + ':</span></td><td><select id="fbfConfFacebookFixerLanguage" style="padding:0; margin-top:3px;"><option value="auto">' + $l('Automatic') + '</option><option value="en">English</option><option value="id">Indonesia </option><option </select></td></tr>'+
				...
				'<div id="fbfConfigControl-9">'+
					...
					'<br /><br />Custom Feed Modification (<a href="http://www.code-poet.net/userscripts/Facebook Fixer/Facebook Fixer-guide.html#custom-feed-modification" target="_blank">instructions</a>):<br /><textarea id="fbfConfCustomFeedModification" style="width:400px; height:150px;"></textarea><br /><input type="button" value="Save Custom Feed Modification" id="SaveCustomFeedModification" />'+
					'<br /><br />Custom CSS (<a href="http://www.code-poet.net/userscripts/Facebook Fixer/Facebook Fixer-guide.html#custom-css" target="_blank">instructions</a>):<br /><textarea id="fbfConfCustomCSS" style="width:400px; height:150px;"></textarea><br /><input type="button" value="Save Custom CSS" id="SaveCustomCSS" />'+
				'</div>'+
				'<div id="fbfConfigControl-10">'+
					'<span xmlns:dc="http://purl.org/dc/elements/1.1/" property="dc:title"><a href="http://sqoatydhent.blogspot.com/" target="_blank">Facebook Fixer</a></span> By <span xmlns:cc="http://sqoatydhent.blogspot.com/ns#" property="cc:attributionName">Veno Andrean</span> Is Licensed Under a <a rel="License" href="http://sqoatydhent.blogspot.com/">Creative Commons Attribution Share Alike 1.0.0 Unported License</a>.<br /><br />Facebook is a registered trademark of Facebook, Inc.<br />Facebook Fixer is not related to or endorsed by Facebook, Inc. in any way.<br /><br /><a rel="License" href="http://sqoatydhent.blogspot.com/"><img alt="Creative Commons License" style="border-width:0;" src="http://i.creativecommons.org/l/by-sa/3.0/88x31.png" /></a><br /><br /><b>' + $l('Translators') + ':</b><ul><li>Veno Andrean - Indonesian</li></ul>'+
				'</div>'+
			'</td></tr>'+
		'</table>'+
		'<br /><hr /><div style="float:left; padding-top:8px;"><a href="http://sqoatydhent.blogspot.com/">' + $l('UpdateHomepage') + '</a></div><div style="text-align:right;"><input type="button" value="' + $l('Refresh') + '" onclick="location.reload();" /> <input type="button" id="fbfCloseConfig" value="' + $l('Close') + '" /></div>'+
		'</div>', true
	);
			...
			if (oldSrc || newSrc) {
				...
				if (profilePixRegex.test(newSrc) ? (newSrc.indexOf('profile')!=-1 ? prefs['ProfilePicPopup'] : prefs['PhotoPopup']) : prefs['ExternalPopup']) {
					...
					showPopupPicTimeout = setTimeout(function(){
						$('#ff-popup-pic-image').innerHTML = '<a href="' + profileLink + '"><img src="' + newSrc + '" alt="Facebook Fixer - ' + $l('LoadingPic') + '" style="max-height:' + (window.innerHeight-35) + 'px;"/></a>';
				...
				if (prefs['CustomFeedModification'].length>0 || prefs['HideApplicationStories'] || prefs['HideEventStories'] || prefs['HideFriendStories'] || prefs['HideGroupStories'] || prefs['HideLikeStories'] || prefs['HideLinkStories'] || prefs['HideNoteStories'] || prefs['HidePhotoStories'] || prefs['HidePlaceStories'] || prefs['HideProfilePicStories'] || prefs['HideRelationshipStories'] || prefs['HideStatusStories'] || prefs['HideVideoStories'] || prefs['HideWallStories']) {
					...
					if (stream) {
						...
						if (highlightedStoryXPath.length>0) {
							var elms = $(".//*[contains(@class,'uiUnifiedStory') and not(contains(@class,'Facebook Fixer-highlighted-story')) and (" + highlightedStoryXPath.join(' or ') + ")]", stream);
							for (var i=0; i<elms.snapshotLength; i++) { elms.snapshotItem(i).className=elms.snapshotItem(i).className + ' Facebook Fixer-highlighted-story'; }
						}
					}
				}
...
// Edited By : Veno Andrean | www.sqoatydhent.blogspot.com |
