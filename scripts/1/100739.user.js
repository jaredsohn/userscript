// ==UserScript== 
// @name            Team-Integra Cleanup/Enhance  
// @namespace       http://www.team-integra.net/userscripts/ 
// @description     Restores some functionality of the old TI 
// @include         http://www.team-integra.net/* 
// ==/UserScript==
...
var GM_config = new GM_configStruct();
	...
	var Settings = {
		"ShowUserCP" 	: 4, 		// # of seconds to show UserCP table, set to 0 to not show at all
		"HideAds"		: true,		// true/false to hide the ads 
		"AltBanner"	: "http://dl.dropbox.com/u/1498261/Team-Integra/lined_up_468x60.png", 
	        "ShowDebug"    : true,  
		"RunningLog" 	: "http://www.team-integra.net/forum/20-your-integra/98252-running-log-morningzs-gsr.html" 
	};
	...
	if (GM_config) {
		$console.append('<div>- Found GM_Config</div>');
	}
	else {
		$console.append('<div>- Did *not* find GM_Config</div>');
	}

	if (Settings.RunningLog != "") {
		$('<a href="' + Settings.RunningLog + '">Your Running Log</a>&nbsp;&nbsp;').insertBefore($("#cse-gtsearch_form_3 input[name='q']"));
	}

	var Events = {};
	Events.HideAds = function() {
		$console.append('<div>- Hide Ads - Start</div>');
		$('.top_ads,#collapseobj_module_16').hide();
		$('.header_sponsor').css('visibility', 'hidden');
		$('#ag_n,.forumlinks').hide();
		$('tbody[id=\'collapseobj_module_22\']').closest('table').css('visibility', 'hidden');
		$('div[id^=\'google_ads_div_\']').closest('div').hide();
		$('td.tfoot:first').closest('table.page').hide();
		$('.vbmenu_popup:first').next('div').hide().next('div').find('img:first').css('visibility', 'hidden');
		$console.append('<div>- Hide Ads - End</div>');
	};
	...
	if (Settings.AltBanner != "") {
		$('.header_sponsor').find("img").attr("src", Settings.AltBanner).end().css("visibility", "visible");
	}

	Events.TopAndBottom = function() {
		// find last post id
		var FirstPostID = $('table[id^=\'post\']:first').attr('id');
		var LastPostID = $('table[id^=\'post\']:last').attr('id');
	
		// Add 'To Top' and 'To Bottom' links
		$console.append('<div>- Add Top and Bottom Links (' + FirstPostID + '/' + LastPostID + ')</div>');
		var loc = '' + window.location;
		if (loc.indexOf('#') > -1) { loc = loc.substr(0, loc.indexOf('#')); }
		$('table[id^=\'post\']').each(function() {
			$(this).find('tbody tr:first td:last').prepend('<a href=\'' + loc + '#' + FirstPostID + '\' style=\'color: #0000ff; text-decoration: none;\'>To First Post</a>&nbsp;&nbsp;&nbsp;&nbsp;' + 
				'<a href=\'' + loc + '#' + LastPostID + '\' style=\'color: #0000ff; text-decoration: none;\'>To Last Post</a>&nbsp;&nbsp;&nbsp;&nbsp;');
		});
		$console.append('<div>- Add Top and Bottom Links - End</div>');
	};
	Events.TopAndBottom();


	Events.FixUpPosts = function() {
		$console.append('<div>- Fix up posts - Start</div>');
		$('td[id^=\'td_post_\']').each(function(i) {
			var $ThisPost = $(this);
	
			// Hides the 'title' if empty 
			var $TitleDiv = $ThisPost.find('div:first');
			var $Clone = $TitleDiv.clone().find('img').remove().end();
			if ($.trim($Clone.text()) == '') { 
				$TitleDiv.hide(); $ThisPost.find('hr:first').hide(); 
			}
	
			// Add some padding to the signature div		
			$ThisPost.find('div[id^=\'post_message_\']').next("div").css('margin-top', '55px');
		});
		$console.append('<div>- Fix up posts - End</div>');
	};
	Events.FixUpPosts();

	
	Events.DressUpLinks = function() {
		var f2 = { "95" : "#0000ff", "99" : "#ff0000", "102" : "#042D7A" };
		$console.append('<div>- Dress up certain forum links - Start</div>');
		$("td.alt1 a[href*='forum/']").each(function() {
			$console.append('<div>  - Link: ' + $(this).attr("href"));
			var hit = $(this).attr("href").match(/^.+\/forum\/(\d+)-(.+)\/.+$/);
			if (hit && hit.length === 3) {
				var ForumID = "" + hit[1];
				if (f2[ForumID] != null) {
					$(this).css("color", f2[ForumID]).css("text-decoration", "none");
					$(this).closest("tr").find("td.alt2:first img").attr("src", "http:" + "//dl.dropbox.com/u/1498261/Team-Integra/iconM.gif");
				}
				var ForumName = hit[2];
				var NewForumName = [];
				$.each(ForumName.split("-"), function() {
					var x = "" + this;
					if (x.length === 1) {
						NewForumName.push(x.substr(0,1).toUpperCase());
					}
					else {
						NewForumName.push(x.substr(0,1).toUpperCase() + x.substr(1));
					}
				});
				
				// Apppend the forum name after the date time
				if ($(this).closest("table[id^='post']").length == 0) {
					var $time = $(this).closest("td").find("div:last");
					if (!$time.hasClass("FNM")) { $time.addClass("FNM").append('&nbsp;in:&nbsp;<a href="/forum/' + ForumID + '-' + ForumName + '/" style="color: #016B21; text-decoration: none; font-wright: bold;" target="_blank">' + NewForumName.join(" ") + '</a>'); }
				}
			}		
		});
		$console.append('<div>- Dress up certain forum links - End</div>');
	};
	Events.DressUpLinks();


	Events.GrowlCP = function() {
		if (Settings.ShowUserCP > 0) {
			var txt = $("span[id='notifications']:first").find("strong:first").text();
			if (txt != "0" && txt.match(/^\d+$/)) {
				$.growlUI('User CP Alert', $("div[id='notifications_menu']:first").find("table").css("margin", "0 auto").end().html(), Settings.ShowUserCP * 1000);
			}
		}
	};
	if (Settings.ShowUserCP > 0) { Events.GrowlCP(); } 
	

	Events.LoadDialog = function () {
		$console.append('<div>- LoadDialog - Start</div>');

		var $dialog = $('<div id="TIDialog" title="Team-Integra Enhancer"></div>').hide();
		...
		$console.append('<div>- LoadDialog - End</div>');
	};


	Events.LoadUI = function() {
		// Load jQuery UI css file, then JS
		$console.append('<div>- Load jQuery UI Css - Start</div>');
		$("head").append("<link>");
	    var css = $("head").children(":last");
	    css.attr({
	      rel:  "stylesheet",
	      type: "text/css",
	      href: "http:" + "//ajax.googleapis.com/ajax/libs/jqueryui/1.8.11/themes/base/jquery-ui.css"
	    });
		$console.append('<div>- Load jQuery UI Css - Done</div>');
		$console.append('<div>- Load jQuery UI Js - Start</div>');
		$.getScript("https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.11/jquery-ui.min.js", function() {
			$console.append('<div>- Load jQuery UI Js - Done</div>');
			setTimeout(function() {
				Events.LoadDialog();
			},250);
	    });
