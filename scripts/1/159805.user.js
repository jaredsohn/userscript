...
$(document).ready(function(){
    ...
    var buttonHtml = '<a id="raffekGo">Download all movies visible below.</a>'
    
	$("div#upgrade_notice").append(buttonHtml).removeClass('hidden');
      
    $("#raffekGo").click(function(event){
    	readUrl();	
    });
});
...
var readUrl = function() {
    ...
    collection.each(function(index){
        ...
        if(deffered != null) {
            deffered.then(deffered = makeAjaxCall(urlToLoad, result, link.html())); 
        } else {
            deffered = makeAjaxCall(urlToLoad, result, link.html());
        }
    });
    
    if(deffered != null) {
        deffered.then(function() {
            console.log("The read is finished. Read: " + result.length + " elements.");
            var toString = "";
            for(var i in result) {
                var obj = result[i];                
				$.fileDownload(obj.url);                
                toString += i+": " + obj.desc + ": " + obj.url + "\n";
                
            }
            
            var uriContent = "data:application/octet-stream;filename=Movie_id_to_title_mapping.txt," + 
            encodeURIComponent(toString);
            newWindow=window.open(uriContent, 'Movie_id_to_title_mapping.txt');

        });
    }
};

var makeAjaxCall = function(urlToLoad, result, desc) {
        return $.get(urlToLoad, function(data) {
            if(typeof(data) != 'string') {
                window.alert("not a string");
            }
            var match = /<source\ssrc=\"(.*)\"\stype/g.exec(data);
            var entry  = {
                desc : desc,
                url : match[1]
            };
