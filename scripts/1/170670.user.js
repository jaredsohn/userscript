...
if (autoheal)
{
   ...
   if (window.name=='HospitalWin') {
      ...
      if (healthCurrent < healthLimit && healthCurrent < healthMax) {
         if (!healing) {
            launch("http://kl.storm8.com/hospital.php", "HospitalWin", "", "MainWin");
...
//             document.location = 'http://kl.storm8.com/hitlist.php?';
           }
         }
       }
       else
...
function attackSanc(noReattack)
{
   ...
   if (document.getElementById('staminaCurrent').innerHTML == 0)
   {
       ...
       setTimeout("document.location = 'http://kl.storm8.com/hitlist.php'", Math.floor(1000*seconds));
       document.location = 'http://kl.storm8.com/hitlist.php';
       return;
   }
