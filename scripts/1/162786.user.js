// ==UserScript==
// @name           4Chan YouTube Link Title
// @description    Adds video titles, shows previews and embeds on click.
// @author         chompskyhomp (original by kuehlschrank)
// @homepage       http://userscripts.org/scripts/show/162786
// @version        0.1
// @icon           http://i.imgur.com/2RjWWlv.png
// @updateURL      http://userscripts.org/scripts/source/162786.user.js
...
function onMutations(muts) {
	for(var i = muts.length, m; i-- && (m = muts[i]);) {
		if(m.type == 'attributes') {
			delay(links.process, 100, m.target);
		} else {
...
var links = {
	...
	parseInfo: function(a) {
		...
		for(var sid in sites) {
			...
			if(info = sites[sid].parse(url)) return {sid:sid,vid:info.vid,t:info.t,url:sites[sid].patterns.some(test, a.textContent.toLowerCase())};
		}
		return null;
	},
		...
		if(a.className.indexOf('YTLT-') > -1) {
			...
			if(!this.styleAdded) {
				var css = '\
					...
					a.YTLT-text { font-weight:bold!important;font-style:normal!important;white-space:nowrap; }\
	...
	show: function(li) {
		if(!this.styleAdded) {
			GM_addStyle('\
				...
				#YTLT-player-titlebar { display:block;line-height:17px;padding:0;background:#232323;background:linear-gradient(0deg, #232323, #141414);border-bottom:2px solid black;text-align:'+(cfg.reverse_buttons?'left':'right')+'; }\
...
var sites = {
	...
	'wh': {
		patterns:["worldstarhiphop.com/video"],
		parse: function(url) { return /\?v=([a-z0-9]+)/i.exec(url) ? {vid:RegExp.$1} : null; },
		...
		request:function(vid, f) {
			net.text('http://www.worldstarhiphop.com/videos/video.php?v=' + vid, [/<title>(Video: )?(.+?)\s*(\(\*Warning|<\/title>)/i, /"image_src" href="(.+?)"/], function(code, m) {
				...
				if(title) {
					delay(f, 0, {title:title[2], preview:preview ? preview[1] : null});
				}
			});
		}
	},
