...
//***************************************************************************************************************************************
	...
	var sortBy = "created_time";
	var sortDirection = "descending"; //keeps newer on top
	...
	var wmConsole = {
		...
		init: function(params){try{
			...
			if (!($("wmTabContainer"))) {
					...
					wmConsole.pagesNode = createElement("div",{id:"wmPageContainer"},[
						...
						createElement("div",{id:"wmMainPage_Feed",href:jsVoid,className:"wm content selected"},[
							...
							forms.createCoolBar({tabs:[
								{id:"feedAll", title:"Show ALL", selected:(main.quickOpts["filterApp"]=="All"),func:wmConsole.selectFilter,kids:[
				...
				wmConsole.feedNode.parentNode.parentNode.insertBefore(
					createElement("div",{className:"toolBox medium inline",style:"display:inline-block;"},[
						createElement("div",{},[
							...
							(isChrome)?null:createElement("select",{className:"toolDropDown", title:"Sort By:", onchange:function(){main.sortPosts({by:this.value});main.redrawPosts();} },optionsFromArray(sortFields)),
							...
							createElement("div",{className:"toolButton oddOrange"},[createElement("a",{title:"Pause Automatic Collection",href:jsVoid,onclick:main.pauseCollecting},[createElement("img",{src:imgs.stop})])]),
							createElement("div",{className:"toolButton oddOrange"},[createElement("a",{title:"Pause Automatic Fetching",href:jsVoid,onclick:main.pauseFetching},[createElement("img",{src:imgs.expand2})])]),
				...
				//$("wmPageViewSelector").selectedIndex = (main.quickOpts.displayMode||0);
				var cn = wmConsole.feedNode.className;
				wmConsole.feedNode.className = (main.quickOpts.displayMode=="1" || main.quickOpts.displayMode=="3")?cn.addWord("short"):cn.removeWord("short");
		...
		selectFilter: function(){
			...
			main.quickOpts["filterApp"]=this.id.split("_feed")[1];
			setOptJSON('quickopts_'+profile,main.quickOpts);
			main.redrawPosts();
		}
	};
...
//***************************************************************************************************************************************
	var Dock = {
		...
		addMenuElements:function(app){try{	
			...
			var testms=main.quickOpts["masterSwitch"][app.appID];
			main.quickOpts.masterSwitch[app.appID]=(testms==null||testms=="undefined")?true:testms;
		...
		addSidekickElement:function(app){try{
			...
			if (sidekickNode) {
				sidekickNode.appendChild(
					createElement("div",{className:"wmSidekickEntry"+((main.quickOpts.masterSwitch[app.appID]||false)?"":" disabled")},[
					...
					createElement("a",{id:"master_"+app.appID,className:"master consoleButton right",href:jsVoid,textContent:((main.quickOpts.masterSwitch[app.appID]||false)?"Disable":"Enable"),onclick:main.toggleSidekick}),
					])
				);
			} else {
		...
		addConsoleElement:function(app){try{
			...
			if (coolBar) {
				...
				var node = coolBar.appendChild(
					forms.createCoolBarButton({id:"feed"+app.appID,title:title,icon:icon,func:wmConsole.selectFilter,selected:(main.quickOpts.filterApp==app.appID)})
				);
	...
	var Rule = function(params){try{
		...
		try{$("wmPriorityBuilder").insertBefore(
			this.node=createElement("div",{className:"listItem"+((!this.enabled)?" disabled":"")},[
				...
				this.levelDropDown=createElement("select",{className:"selectPriority",onchange:function(){rule.level=this.value; main.saveRules();}},(function(){
					var optsret=[]; 
...
//***************************************************************************************************************************************
	...
	var App = function(params){try{
		...
		this.resetCounters=function(){try{
			this.accepted=0; this.failed=0;
			this.updateCounters();
		}catch(e){log("App.resetCounters: "+e);}};
...
//***************************************************************************************************************************************
	...
	//post binary flags
	var postFlags = new EnumFlags("COLLECT","SCAM","FAILED","ACCEPTED","EXCLUDE","PAUSED","PINNED","LIKED","MYPOST","STALE","NODEF","WISHLIST");
	//var postState = new Enum("OPEN","FINISHED");
	...
	var Post = function(params){try{
		...
		this.identify=function(){try{
			...
			if(!main.quickOpts.masterSwitch[app.appID]) {
		...
		this.open=function(){try{
			...
			post.node.className = post.node.className.replaceWord("collect","working");
			post.state="working";
			main.requestsOpen++;
		...
		this.forceOpen=function(){try{
			var post=this;
			post.node.className=post.node.className
				.replaceWord("failed","collect")
				.replaceWord("timeout","collect")
				.replaceWord("accepted","collect")
				.replaceWord("excluded","collect");
			if (!post.node.className.contains("collect")) post.node.className=post.node.className.addWord("collect");
			post.open();
		...
		this.draw=function(redraw){try{
			...
			var filter=(main.quickOpts.filterApp||"All");
			...
			//create the layout
			var layout=function(){
				switch (main.quickOpts.displayMode||"0"){			
		...
		this.pause=function(){try{
			...
			if (this.node||null) this.node.className=this.node.className.addWord("paused");
		}catch(e){log("Post.pause: "+e);}};

		this.unPause=function(){try{
			this.flags &= ~postFlags.PAUSED;
			if (this.node||null) this.node.className=this.node.className.removeWord("paused");
		}catch(e){log("Post.unPause: "+e);}};

		this.exclude=function(){try{
			this.flags |= postFlags.EXCLUDED;
			if (this.node||null) this.node.className=this.node.className.addWord("excluded");
		}catch(e){log("Post.exclude: "+e);}};

		this.collect=function(){try{
			this.flags |= postFlags.COLLECT;
			if (this.node||null) this.node.className=this.node.className.addWord("collect");
		}catch(e){log("Post.collect: "+e);}};

		this.stopCollect=function(){try{
			this.flags &= ~postFlags.COLLECT;
			if (this.node||null) this.node.className=this.node.className.removeWord("collect");
		}catch(e){log("Post.collect: "+e);}};
		...
		this.pin=function(){try{
			...
			if (this.node||null) this.node.className=this.node.className.addWord("pinned");			
		...
		this.unPin=function(){try{
			this.flags &= ~postFlags.PINNED;
			if (this.node||null) this.node.className=this.node.className.removeWord("pinned");
		...
		this.accept=function(mark){try{
			...
			if (this.node||null) this.node.className=this.node.className.addWord("accepted").removeWord("failed").removeWord("working").removeWord("collect");
			if (mark) main.setAsAccepted(null, 3,this);
		}catch(e){log("Post.accept: "+e);}};

		this.fail=function(mark){try{
			this.flags |= postFlags.FAILED;
			if (this.node||null) this.node.className=this.node.className.removeWord("accepted").addWord("failed").removeWord("working").removeWord("collect");
...
//***************************************************************************************************************************************
	...
	var main = {
		...
		pauseCollecting : function(){
			...
			this.parentNode.className = (isPaused)?this.parentNode.className.replaceWord("oddOrange","oddGreen"):this.parentNode.className.replaceWord("oddGreen","oddOrange");
			this.childNodes[0].src = (isPaused)?imgs.play:imgs.stop;
			this.title = (isPaused)?"Collect":"Pause Automatic Collection";
		},
		...
		pauseFetching : function(){
			...
			this.parentNode.className = (isPaused)?this.parentNode.className.replaceWord("oddOrange","oddGreen"):this.parentNode.className.replaceWord("oddGreen","oddOrange");
			//this.childNodes[0].src = (isPaused)?imgs.play:imgs.stop;
			this.title = (isPaused)?"Fetch":"Pause Automatic Fetching";
		},
		
		sortPosts : function(params){
			params=params||{}; 
			params.direction=(sortDirection=(params.direction||sortDirection||"asc")); //default ascending
			params.by=(sortBy=(params.by||sortBy||"created_time")); //default by date
		...
		setAsAccepted : function(comment,status, post) {try{
			...
			history[post.id]={status:status, date:timeStamp(), which:post.which.removePrefix(app.appID), appID:app.appID};
			...
			if (postNode){
				...
				postNode.className=postNode.className.replaceWord("working","accepted").addWord("accepted");
			...
			//try autolike
			try{
				if (opts.autolikeall || opts.autolikeaccepted || (opts.autolikesent && w.startsWith(synApp.appID+"send")) ) {
		...
		setAsFailed : function(comment, status, post){try{
			...
			if (postNode) {
				...
				if (!opts.failontimeout && status==-14) {
					...
					postNode.className=postNode.className.replaceWord("working","timeout").addWord("timeout");
				} else {
					post.state="failed";
					...
					postNode.className=postNode.className.replaceWord("working","failed").addWord("failed");
				}
				post.status=status;
		...
		toggleSidekick : function(){
			...
			var opt = !(main.quickOpts["masterSwitch"][appID]||false); //toggle
			main.quickOpts["masterSwitch"][appID]=opt;
		...
		redrawPosts : function(){
			...
			wmConsole.feedNode.className=(main.quickOpts.displayMode=="1" || main.quickOpts.displayMode=="3")?"short":"";
		...
		cleanPosts : function () {
			for (var p in posts) {
				...
				if (!(
					(post.state=="working") ||
					(post.flags & (postFlags.PINNED | postFlags.COLLECT)) ||
					(post.state=="timeout" && !opts.cleanTimedOut)
				)) post.remove();
			}
		},
		...
		fetch : function(params) {try{
			...
			for (var f in feeds) { var feed=feeds[f]; if (feed.enabled) {
				...
				if (feed.url.contains("/me/home")) for (var a in apps) {
					...
					if (main.quickOpts.masterSwitch[a]){
						var app=apps[a];
						Graph.fetchPosts({
		...
		compileFeedSources : function(){try{
			...
			var newFeeds = {me:feeds.me}; //always put our own feed in there first
			...
			for (var f=0,feedsToAdd=feedsInput.split("\n"),toAdd; (toAdd=feedsToAdd[f]); f++) if (toAdd.trim()!="") {
				...
				for (var oldFeed in feeds) if (feeds[oldFeed].url==feed) {
					//found, copy the old version to preserve next/prev location
					newFeeds.push(feeds[oldFeed]); found=oldFeed; break;
				}
				//if not found, add it new
				if (!found) newFeeds.push({url:feed, next:"",prev:"",enabled:true,filters:{}});
			}
		...
		init : function(){try{
			...
			//get quickopts
			main.quickOpts = getOptJSON('quickopts_'+profile)||{};
			main.quickOpts["filterApp"]=(main.quickOpts["filterApp"]||"All");
			main.quickOpts["displayMode"]=(main.quickOpts["displayMode"]||"0");
			main.quickOpts["masterSwitch"]=(main.quickOpts["masterSwitch"]||{});
