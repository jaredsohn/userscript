...
var STARTUP_TIMER;
var DEBUG	= true; /* Add additional logs into the console */
...
var production_icons = {'gold':{desc:'Oro', icon:'ui/blackMarket/gold.png', treasure:'ui/achievements/25_01.png', march_icon:'ui/skin_parts/icons/iconUraniumGiant.png'},
						'iron':{desc:'Hierro', icon:'ui/blackMarket/steel.png', treasure:'ui/achievements/24_01.png', march_icon:'ui/skin_parts/icons/iconTitaniumGiant.png'},
						'food':{desc:'Comida', icon:'ui/blackMarket/meat.png', treasure:'ui/achievements/26_01.png', march_icon:'ui/skin_parts/icons/iconCreditGiant.png'}};
...
var Map = {
	...
	to_visit	: [],
	//106, 104, 102, 
	buildings_id: [109, 108, 103, 105, 107, 150, 800, 506, 139],
	block_size	: 20,
	x			: 0,
	y			: 0,
				...
				VisitUsers.visitCmd (user_id.toString(), segment_id, function (res){
					...
					if (res.done){
						...
						if (t.percent != t.old_percent) {
							progressBar.update ({ step:t.percent, stepText:'Espera ...<br>Paso '+(t.step+1)+'/'+t.steps+' : Jugador '+t.users_count+'/'+t.total_users });
	...
	updateUserEvol : function (user_id) {
		...
		if (VisitUsers && VisitUsers.current && VisitUsers.current.gameData && VisitUsers.current.gameData.account) {
			...
			for (var i=0; i<user_buildings.length; i++) {
				...
				if (t.buildings_id[j] == building.type) 
                    found = true;
				}
				if (found) {
				...
				case 3:
					if (to_collect) {
						Treasures.getTreasures (user_id, function (treas_res){
							if (treas_res.done)
								msg += '<b>'+treas_res.count+'</b> Tributos Recolectados';
							else msg += 'Error recolectando tributos.<br>'+(treas_res.errmsg || '');
							deal_step(current_step + 1);
						});
					} else  deal_step(current_step + 1);
					break;
...
Tabs.Info = {
	...
	init : function (div){
		...
		+'	<td width=25% align=center><input id=tabInfo_Toggle type=button value="Refresh Pagina"></input></td>'
		+'	<td width=25% align=center>&nbsp;</td>'
		+'	<td width=25% align=center><input id=tabInfo_Collect type=button value="Recoger tributos"></input></td>'
		+'</tr></table>'
		+'<ul class=tabs>'
	...
	tabInfoOverview : function (){
		...
		m += '<table width=100%>'
			...
			+'<tr valign=middle>';
		for (var r in production_icons) {
			m += '	<td align=center width=38px title="Tributos '+production_icons[r].desc+'">'+ vignette2(production_icons[r].treasure,25) +'</td>'
				+'	<td align=left width=50px title="Tributos '+production_icons[r].desc+' + those with friends"><div id='+r+'_treas></div></td>';
		}
		...
		for (var g=0; g<unit_groups.length; g++) {
			...
			if (g%2 == 0) {
				...
				for (var i=0; i<Seed.units_id.length; i++) {
					...
					var name = Seed.reqs[Seed.units_id[i].id].name + " "+ Seed.units_id[i].id;							
...
*/
		...
		function refreshView () {
			...
			var tj = '<table style="margin-top:3px" width=100%>', title = 'Formationssss :';
...
Tabs.Contacts = {
	...
	tabTargetList : function (){
		...
		var m = '<div id=tabTargets_Results style="height:640px">'
			...
			+'	<table class=bsx_table width=100%>'
			+'		<tr valign=top align=left>'
			+'			<td width=30%><label>min level : </label>' + levelSelect('Min', nvl(Data.options.contacts.min_level,35) ) + '</td>'
		...
		function getTargets () {
			...
			for (var f=0; f<source.length; f++) {
				...
				if (user.level < nvl(Data.options.contacts.min_level,35) || user.level > nvl(Data.options.contacts.max_level,100)) continue;
		...
		function scanMap (notify){
			var t = Tabs.Contacts;
			var x = Seed.gameData.account.x; //fernando
			var y = Seed.gameData.account.y;
			//var x = -3834;
			//var y = -500;
			var min = nvl(Data.options.contacts.min_level,35);
...
function AjaxRequest(url, segment, method_name, opts) {
	...
	if (opts.method == 'POST') {
		...
		if (opts.isBinary || opts.isFacebook) {
			...
			url += '?ver='+client_ver+'&method='+method_name+'&locale=es-ES';
