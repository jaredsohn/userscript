...
sideLink3Replacements = {
	...
	"useGiftLink":["Use gifts","Usar Regalos"]
};

sideLink4Replacements = {
	"eatButton":["Eat Food","Comer"],
	"useGiftButton":["Use gift","Usar Regalo"] 
};
...
hpTabsReplacements = {
	...
	"#substidedBattles":["Subsidized","Subvención"],
...
function replaceBattleInfo(obj) {
	...
	//Subsidies:
	obj.innerHTML=obj.innerHTML.replace(/(Subsidies:)/g,"Subvención:");
}
...
function doSide() {
	...
	for (var i = 0; i < results.snapshotLength; i++) {
		...
		if (obj.innerHTML.match("Any")) {
			...
			obj.innerHTML=obj.innerHTML.replace(/(Canada)/,"Canada");
		}
			...
			obj.innerHTML=obj.innerHTML.replace(/(Gift)/,"Regalos");
		}
...
function doProductMarkets() {
	...
	tmp.innerHTML=tmp.innerHTML.replace(/( items )/g,"Cant."); 
// check	
	replaceLinkByHref({
...
function doBattle() {
	...
	if (allElements.innerHTML.match("occupant country to")) {
		allElements.innerHTML=allElements.innerHTML.replace(/(You can't fight in this battle from your current location.)/," No se puede luchar en esta batalla desde tu ubicación actual.");
		allElements.innerHTML=allElements.innerHTML.replace(/(You must travel to the occupant country to participate in the battle.)/," Debes viajar al país de los ocupantes para participar en la batalla");
	} else if (allElements.innerHTML.match("countries participating")) {
		allElements.innerHTML=allElements.innerHTML.replace(/(You can't fight in this battle from your current location.)/," No se puede luchar en esta batalla desde tu ubicación actual.");
		allElements.innerHTML=allElements.innerHTML.replace(/(You must travel to one of the countries participating in the battle.)/," Debes viajar a uno de los países participantes en la batalla.");
...
function doProfile() {
	...
	tmp.innerHTML=tmp.innerHTML.replace(/Rank:/,"Rango:");
	tmp.innerHTML=tmp.innerHTML.replace(/Economy skill:/,"Nivel Econom.:");
	tmp.innerHTML=tmp.innerHTML.replace(/Strength:/,"Fuerza:");
	...
	if (allElements.snapshotLength > 0) {
		...
		if (tmp.innerHTML.match("Active debts")) {
			rowoffset = 2;
			tmp.innerHTML=tmp.innerHTML.replace(/Active debts/,"Deudas activas");
...
function doInboxMessages() {
	...
	replaceInputByValue({
		"Delete":["Delete","Borrar"],
		"Quick reply":["Quick reply","Respuesta Rapida"],
...
function doSubscribedNewspapers() {
	...
	replaceLinkByHref({
		"http://wiki.e-sim.org/en/Newspaper":["Newspaper tutorial on wiki","Tutorial periodico en la wiki"]
	});
}
...
function doNewspaper() {
	...
	replaceLinkByHref({
		"editNewspaper.html":["Edit newspaper","Editar periodico"],
	...
	if (allElements) {
		...
		tmp.innerHTML=tmp.innerHTML.replace(/Write new article/,"Escribir nuevo articulo");
		tmp = allElements.children[0];
		tmp.innerHTML=tmp.innerHTML.replace(/Publish in country/,"Publicar en el Pais");
		tmp = allElements.children[4];
		tmp.innerHTML=tmp.innerHTML.replace(/Article title:/,"Título articulo:");
		tmp = allElements.children[10];
		tmp.innerHTML=tmp.innerHTML.replace(/Message:/,"Mensaje:");
		tmp = allElements.children[allElements.children.length-1];
		tmp.innerHTML=tmp.innerHTML.replace(/Please save a copy of your article on a hard disc before submitting it/," Por favor, guarde una copia del articulo en su disco duro antes de enviarlo!!!");
	}
}
...
function doNewspaperEdit() {
	...
	replaceInputByValue({
		"Delete":["Edit newspaper","Editar Periodico"]
	});
}
