// ==UserScript==
// @name           Power Tools - Italian
// @namespace      PDX
// @description    KoC Power Tools - Italian Version by Greed
// @version 	   0.4.1
...
var Title = 'KoC Power Tools - Italian';
...
var Options = {
  ...
  alertConfig  : {aChat:false, aPrefix:'!!! WARNING = ATTACK !!!', scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  logit ("* KpC Power Tools - Italian - by Greed - Version: "+ Version +" caricato!");
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('Il Padrino Power', eventHideShow, mouseMainTab);
TestSomething.init ();  
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Wild: '+ totw +' von '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];

      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wild</td><TD></td><TD align=left>Cord</td><TD>Fallen</td><TD align=left>Troop</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' DEFENSE ') +'</td></tr>';
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Cost: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("Defense set", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Gold: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
if (wildDef==undefined || !wildDef)
  wildDef = {fort60Count:0, mercLevel:0};
    ...
    popDiv.innerHTML = '<TABLE class=ptTab width=100% height=100%><TR><TD>\
          <DIV class=ptstat>Wild defense are...</div>\
  ...
  processQue : function (errMsg){
    ...
    if (what[0] == 'T'){
      div.innerHTML += 'Build '+ what[2] +' fall for '+ Cities.byID[t.buildList.cityId].name +'\'s Wild on '+ wild.xCoord +','+ wild.yCoord +' ... ';
      t.postBuyTraps (t.buildList.cityId, what[1], what[2], t.processQue);
    } else {
      div.innerHTML += 'Instruct '+ mercNames[what[2]] +' for '+ Cities.byID[t.buildList.cityId].name +'\'s Wild on defense '+ wild.xCoord +','+ wild.yCoord +' ';
...
Tabs.Knight = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        m += 'Nobody Added</td><TD colspan=4></td><TD class=ptentry colspan=5></td><TD colspan=2></td></tr>';
      } else {
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          ...
          <TD width=26>Int.</td><TD width=26>Ressis</td><TD width=90 align=center colspan=5>-= Knight Punkte =-</td><TD width=40 align=right> Kosten </td><TD width=35>Level</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
...
//logit ('displayReports: '+ Options.allowAlterAR);        
        ...
        msg.push("<thead><tr><td width=105>Datum</td><td width=40>Typ</td><td width=150>Angreifer</td><td>Target</td><td>Anzeigen</td></tr></thead><tbody>");
  ...
  e_eachMinute : function (){   
    ...
    for(i=0; i < Cities.numCities; i++) {
      ...
      if (timeLeft<0){
        ...
        msg += ' has little food! ... I  '+addCommasWhole(foodleft);
        msg += ' Food ... they will provide a ' + timestrShort(timeLeft) + ' - Consumption: ' + addCommas(usage);
        sendChat ("/a " + msg);
      }
    }  
...
var TowerAlerts = {
  ...
  viewImpending_hook : function (atkinc){    
    ...
    if (t.fixTargetEnabled){ 
      ...
      div.childNodes[0].innerHTML = '<B>Time: </b>'+ target;
    }
    if (!isFalse){
      var d = document.createElement ('div');
      d.innerHTML = '<BR><TABLE><TR><TD><a id=towerPostToChat class=button20><span>The Alliance Chat Show!</span></a></td></tr></table>';
      div.appendChild (d);
  ...
  postToChat : function (m, force){
    ...
    if (DEBUG_TRACE) logit ("checkTower(): Attention on "+ unixTime()  +": \n"+ inspect (m, 8, 1));
    if (m.marchType == null)      // bogus march (returning scouts)
      return;
    if (ENABLE_TEST_TAB) Tabs.Test.addDiv ("Attention!<BR><PRE style='margin:0px;'>" + inspect (m, 8, 1) +'</pre>');
    if (m.marchType == 3){
    ...
    msg += 'My '+ target +' is '+ atkType  +' of '+ who +' = Troops (Attack in '+
 ...
 //         if (Seed.queue_atkinc[k].marchType==2 && Cities.byID[Seed.queue_atkinc[k].fromCityId]==null){
        ...
        msg += ' = My Embassy have '+ availSlots +' of '+ emb.maxLevel +' Slots!';
      }
    }
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    if (getMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER><blink>You must be in an alliance to use this feature!</blink></center>';
      t.state = 1;
      return;
    }
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Player Name: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Player Found" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab> </td><TD class=xtab> Alliance Name: &nbsp;</td>\
          <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Alliance Searchn" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptallErr></span></td></tr>\
		<TR><TD class=xtab> </td><TD class=xtab align=right> Alliance List: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="'+ getMyAlliance()[1] +'" />\
            <INPUT align=left id=allListSubmit type=submit value="All Alliance" /></td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = '<span style="color:red"><blink>Min 3 letter!</blink></span>';
      return;
    }
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Search...</center>';
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Search results: <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Might</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Information </td></tr>';
    var row=0;
    var cl='';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>Leaderboard</a></span> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>on/offline</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>Login</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "search leaderboard info";
  ...
  clickedPlayerCheckOnline : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "search online status";
  ...
  clickedPlayerGetLastLogin : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "search login date";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Leaderboard:</b> No found! Mists ?';
      return;
    }
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; #: '
          + u.cities +' &nbsp; Volk: '+ u.population +'</td></tr><TR><TD></td><TD>Province: ';
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">Last Login:</span><span style="color:red"> '+lastLogin+'</span>';
    } else {
       m = '<span style="color:red">No login data found: '+lastLogin+'</span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = '<span style="color:red"><blink>Min 3 letter!</blink></span>';
      return;
    }
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Results: <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Name</td><TD class=xtab>Rang</td><TD class=xtab>Player</td>\
        <TD align=right class=xtab>Might</td><TD class=xtab>Diplomatie</td><TD class=xtab></td></tr>';
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Player View</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
  ...
  showMyAlliance : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'You have to be in an alliance to use this feature!';
    }
  },
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 ) {
      document.getElementById('allListOut').innerHTML = 'Alliance at the first show!';
      return;
    }
    if (t.MaxPage < 0 || val > t.MaxPage || val < 1) {
      document.getElementById('allListOut').innerHTML = 'Page number does not exist!';
      return;
    }
    t.curPage = val;
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Search ...</center>';
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    var m = '<div style="overflow:auto; height:556px;width:564px;"><TABLE><thead><TR style="font-weight:bold"> \
        <th class=xtab>Name</th><th class=xtab>Rang</th><th class=xtab>Player</th>\
        <th align=right class=xtab>Might</th><th class=xtab>Diplomatie</th><th class=xtab></th></tr></thead><tbody>';
    document.getElementById('allListOut').innerHTML = m;
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Player View</a></td></tr>';
    }
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      ...
      m += '<TR style="max-height:30px"><TD class='+ tabclass +'>'+ t.dat[i][0] +'</td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
			...
			<td class='+ tabclass +'><SPAN onclick="PCpo(this, \''+t.dat[i][9] +'\')"><A>off/online</a></span></td>\
			<td class='+ tabclass +'><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>Login</a></span><td> </tr>';
    }
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Entfernung von <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' Player gefunden &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
		...
		<TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Tite</a></div></td>\
        ...
        <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Cord.</a></div></td>\
      ...
      <DIV style="top:670px; left:0px; position:static; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab><u>Important</u>: The data are from the leader board and can be up to 24 hours old!<BR><u>Note</u>: Click on the column head for sorted!</td>\
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("No alliance found!")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
  ...
  init : function (div){
    ...
    var m = '<TABLE><TR><TD align=right>Späh Angriff: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
		...
		<TR><TD align=right>Späher: </td><TD><INPUT type=text size=6 value=0 id=fakeScout></td></tr>\
		  ...
		  <TR><TD align=right>Fake Name: </td><TD><INPUT type=text size=13 value=FAKE_ATTACK id=fakeName></td></tr>\
		  <TR><TD align=right>Target: </td><TD>'+citySelect+'</td></tr>\
...
Tabs.F = {
  ...
  init : function (div){
	...
	var citySelect = '   <SELECT id=fakeCity>';
	    ...
	    citySelect += '</select>';
    var m = '<TABLE><TR><TD align=right>Fake Scout: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        <TR><TD align=right>Fake Wild: </td><TD><INPUT type=checkbox id=fakeIsWild></td></tr>\
        <TR><TD align=right>Fake Bericht: </td><TD><INPUT type=checkbox disabled id=fakeFalse></td></tr>\
        <TR><TD align=right>Seconds: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
        <TR><TD align=right>Supply: </td><TD><INPUT type=text size=6 value=0 id=fakeSupply></td></tr>\
		<TR><TD align=right>Militia: </td><TD><INPUT type=text size=6 value=5000 id=fakeMilitia></td></tr>\
		<TR><TD align=right>Scout: </td><TD><INPUT type=text size=6 value=0 id=fakeScout></td></tr>\
		  <TR><TD align=right>Pike: </td><TD><INPUT type=text size=6 value=0 id=fakePike></td></tr>\
		  <TR><TD align=right>Sword: </td><TD><INPUT type=text size=6 value=0 id=fakeSword></td></tr>\
		  <TR><TD align=right>Archer: </td><TD><INPUT type=text size=6 value=0 id=fakeArcher></td></tr>\
		  <TR><TD align=right>Cav: </td><TD><INPUT type=text size=6 value=0 id=fakeCav></td></tr>\
		  <TR><TD align=right>H-Cav: </td><TD><INPUT type=text size=6 value=0 id=fakeHeavy></td></tr>\
		  <TR><TD align=right>Wagon: </td><TD><INPUT type=text size=6 value=0 id=fakeWagon></td></tr>\
		  <TR><TD align=right>Ballista: </td><TD><INPUT type=text size=6 value=0 id=fakeBallista></td></tr>\
		  <TR><TD align=right>Ram: </td><TD><INPUT type=text size=6 value=0 id=fakeRam></td></tr>\
		  <TR><TD align=right>Catapult: </td><TD><INPUT type=text size=6 value=0 id=fakeCat></td></tr>\
		  <TR><TD align=right>Fake Name: </td><TD><INPUT type=text size=13 value=FAKE_ATTACK id=fakeName></td></tr>\
		  <TR><TD align=right>Time: </td><TD>'+citySelect+'</td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=testSendMarch type=submit value="Fake Attack" \></td></tr></table>\
        <INPUT id=ptReloadKOC type=submit value="Refresh KoC" \>\
        <BR><DIV id=testDiv style="background-color:#fffff0; maxwidth:675; max-height:430px; height:430px; overflow-y:auto;"></div>';
    t.cont.innerHTML = m;
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m += '<DIV class=ptstat>DISTANCE CALCULATOR</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Starting point: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>Target: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> <SPAN id=ptloc2></span></td></tr></table>\
...
</select><BR><BR><SPAN class=boldRed>(Starting point, Target und Truppen Auswählen!)</span></div>\
    ...
    try {      
      m = '<TABLE class=ptTab>\
       <TR><TD colspan=2><B><u>Power Tools - Einstellung</u></b></td></tr>\
		...
		<INPUT id=optFoodAlertHours type=text size=3 value="'+ Options.foodAlertHours +'"> Stunden ausreicht! <SPAN class=boldRed>(Bei Refresh und alle 15 Min.)</span></td></tr>\
      ...
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Erweiterte Allianz Berichte.</td></tr>\
        ...
        <TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Chat Erweitern <SPAN class=boldRed>(Koordianten Klickbar, pink global messages, Avatar klicken zum flüstern)</span> </td></tr>\
        ...
        <TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>Lösch Button einblenden <SPAN class=boldRed>(Um auf der Angriffs Seite die Koordinate direkt zu Löschen)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B><u>Bug Fix</u></b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Wachturm Berichte Fix <SPAN class=boldRed>(City, Wildnis oder Ungültig)</span></td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Zeige die Entfernung von der Ausgewählten City, und nicht immer die von der ersten!</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Fake Angriffe durch Späher Attacken vermeiden.</td></tr>\
        <TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD><u>Keinen</u> Knight auswählen wenn der Marsch typ: <SPAN class=boldRed>Spähen, Transport</span> oder <SPAN class=boldRed>Verstärken</span> ist!</td></tr>\
...
Tabs.Searchn = {
  ...
  init : function (div){
    ...
    this.cont = div;
    this.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Searchn: </td><TD>\
      <SELECT id="srcType">\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Starting point: &nbsp; X: </td><TD class=xtab>\
      ...
      m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Searchn" /></td></tr>';
      m += '</table>';
...
clickedPlayerDetail : function (span, uid){
  ...
  clickedPlayerGetLastLogin : function (span, uid){
	...
	var s = Tabs.Searchn;
    span.onclick = '';
    span.innerHTML = "Search login datum ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Leaderboard:</b> nicht gefunden! :/  evtl. mal im Nebel Searchn?';
      return;
    }
    var p = rslt.results[0];
    var an = p.allianceName;
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Allianz: '+ a +' &nbsp; City: '
  ...
  eventSubmit : function (){
    ...
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Searchn ...</center>';
    if (myA[0]!=0 && myA[1].toLowerCase().indexOf(t.aName.toLowerCase())>=0 )
      t.fetchAllianceList (t.aName, myA[0], t.eventGotAllianceList);
    else
      t.fetchAllianceList (t.aName, null, t.eventGotAllianceList);
...
//End edit city search
  ...
  srchTypeHandler : function (){
    ...
    var m = '<CENTER><B>Searchn nach '+ typeName +'<BR>\
        Starting point: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Entfernung: '+ t.opt.maxDistance +'<BR></center>\
    ...
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        ...
        <TR><TD class=xtab align=right >Koordinaten:</td><TD class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
  ...
  clickedSearch : function (){
    ...
    if (t.searchRunning){
      t.stopSearch ('Search ABGEBROCHEN...');
      return;
    }
    t.opt.searchType = document.getElementById ('srcType').value;
    t.opt.startX = parseInt(document.getElementById ('srchX').value);
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>750)
      errMsg = "X muss zwischen 0 und 749 liegen!<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>750)
    ...
    document.getElementById ('srcStart').value = 'Search Anhalten';
    ...
    m = '<CENTER><B>Searchn nach '+ typeName +'<BR>\
        Starting point: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Entfernung: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>EINSTELLUNG:</b><BR></td></tr>';
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. Level:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD class=xtab align=right>Sortieren:</td><TD class=xtab><SELECT id=filSortBy>\
            ...
            <TR><TD class=xtab align=right>Koordinaten:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
	} else {
    ...
    document.getElementById ('statStatus').innerHTML = 'DurchSearch... '+ xxx +','+ yyy;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 0) // Barbs
			...
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Koordinate</td><TD style="padding-left: 10px">Entfernung</td><TD style="padding-left: 10px;">Level</td><TD width=10%> &nbsp; Typ</td><TD width=70%> &nbsp; Besitzer</td><TD style=""></td></tr>';
		else // Cities
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Koordinate</td><TD >Dist</td><TD>City</td><TD>Besitzer</td><TD>Macht</td><td>Allianz               </td><TD width=80% style="font-size:9px;">Information</td><TD style="padding-left: 10px;"></td></tr>';
			
	  }
      var numRows = dat.length;
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>WICHTIG: Tablle zeigt nur 500 von '+ dat.length +' Ergebnissen an solange die Search noch nicht beendet ist!</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'Searchn';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById ('statStatus').innerHTML = 'DurchSearch... '+ x +','+ y;
...
//DebugTimer.start(); 
    try {
      ...
      str += "<DIV id=overMainDiv style='font-size:"+ Options.overviewFontSize +"px'><TABLE class=ptTabOverview cellpadding=0 cellspacing=0><TR valign=top align=right><TD width=65><b>City Name</b><br>(Koordinate)<br>Provinz</td><TD width=88 style='background: #600000;color: #FFF'><B>Gesamt</b></td>";
      for(i=0; i<Cities.numCities; i++) {
	  ...
	  str += '<TR valign=top align=right><TD style=\'background: #CCFFCC\'><b>Wachturm</b></td><TD style=\'background: #CCFFCC\'></td>';
	  for(i=0; i<Cities.numCities; i++){
     ...
     row = [];
        ...
        if (row[i] >= 0)
          ...
          if (timeLeft > 86313600)
            ...
            if (Options.enableFoodWarn && timeLeft<(Options.foodWarnHours*3600))
              row[i] = '<blink><SPAN class=whiteOnRed>'+ timestrShort(timeLeft) +'</span></blink>';
            else
      ...
      str += _row ('Rammbock', rows[11]);
      str += _row ('Katapulte', rows[12]);
	}
      	  if (Options.includeTraining) {
		str += '<TR><TD class=ptstat>Ausbildung</td></tr>';
        str += _row ('<i><span style="color:green">Versorger</span></i>', t_rows[2]);
        str += _row ('<i><span style="color:green">Miliz</span></i>', t_rows[4]);
        str += _row ('<i><span style="color:green">Späher</span></i>', t_rows[6]);
        str += _row ('<i><span style="color:green">Lanzen</span></i>', t_rows[8]);
        str += _row ('<i><span style="color:green">Schwerter</span></i>', t_rows[10]);
        str += _row ('<i><span style="color:green">Bogen</span></i>', t_rows[12]);
        str += _row ('<i><span style="color:green">Kav</span></i>', t_rows[14]);
        str += _row ('<i><span style="color:green">S-Kav</span></i>', t_rows[16]);
        str += _row ('<i><span style="color:green">Wagen</span></i>', t_rows[18]);
        str += _row ('<i><span style="color:green">Ballisten</span></i>', t_rows[20]);
        str += _row ('<i><span style="color:green">Rammbock</span></i>', t_rows[22]);
        str += _row ('<i><span style="color:green">Katapulte</span></i>', t_rows[24]);
      }
	  ...
	  str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><b><u>Ansicht Einstellung</u></b><BR><INPUT type=CHECKBOX id=ptIncWachturm'+ (Options.includeWachturm?' CHECKED':'') +'>Wachturm Anzeigen! <SPAN class=boldRed>(Verteidigung/Versteckt)</span></td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=8>Nahrung Verbrauch Anzeigen! | Oben <INPUT type=CHECKBOX id=ptIncNahrung'+ (Options.includeNahrung?' CHECKED':'') +'> Unten <INPUT type=CHECKBOX id=ptIncNahrungUnten'+ (Options.includeNahrungUnten?' CHECKED':'') +'></td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><INPUT type=CHECKBOX id=ptIncRessis'+ (Options.includeRessis?' CHECKED':'') +'>Ressourcen Anzeigen!</td></tr>';
	  str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><INPUT type=CHECKBOX id=ptIncTruppen'+ (Options.includeTruppen?' CHECKED':'') +'>Truppen Anzeigen! | Auszubildene Truppen Anzeigen/Zusammenrechnen <INPUT type=CHECKBOX id=ptoverIncTraining'+ (Options.includeTraining?' CHECKED':'') +'></td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><INPUT type=CHECKBOX id=ptIncInfos'+ (Options.includeInfos?' CHECKED':'') +'>Informationen Anzeigen! <SPAN class=boldRed>(Knight, Wildnisse, Warte und Ausbildungsschalngen)</span></td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Overflow erlauben <SPAN class=boldRed>(keine Scrollbalken!)</span> | Schriftgröße: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12 , 14:14}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr>';
	  str += '<TR><TD class=xtab></td><TD class=xtab colspan=8><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Marschierende Truppen/Resourcen Anzeigen</td></tr>';
      str += "</table></div>";
...
Tabs.Marches = {
  ...
  showAttacks : function (){
    var t = Tabs.Marches;
    t.marchDiv.innerHTML = 'Hier werden die Angriffe auf dich Angezeigt! Diese Funktion ist noch nicht verfügbar!!';;
  },
...
logit ('ajaxCancelMarch: '+ marchId); 
  ...
  showMarches : function (){
    ...
    var m = '(Truppen Bewegung ist noch in der entwicklung!)<br><div class=ptstat>Truppen Bewegung</div>';
    m += '<center><table><thead><th>#</th><th>City</th><th>Typ</th><th>Target</th><th>Level</th><th>Koordinate</th><th>Truppenstärke</th><th>Resourcen</th><th>seid</th><th>ETA</th><th>Status</th><th>zu spät</th></thead>\
             <tbody>';
    var count=0;
    for(i=0; i<Cities.numCities; i++) {   
      cityId = 'city'+ Cities.cities[i].id;
      ...
      for (k in Seed.queue_atkp[cityId]){   
        ...
        if (typeof (march) == 'object'){
          ...
          if (march.toPlayerId) {
              if (march.toPlayerId>0 && targType=='City,Barbaren')
                 targType='Lager';
              else {
                 targType='Barbaren';
              }
          } else if (targType=='City,Barbaren')
                 targType='City';
          ...
          m += etaStr;
          m += '<td>'+status+'</td><td>'+((late)?'J':'N')+'</td></tr>';
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['Versorger', 'Miliz', 'Späher', 'Lanzen', 'Schwerter', 'Bogen', 'Kav', 'S-Kav', 'Wagen', 'Ballisten', 'Ram', 'Kat'];
    var t = Tabs.Marches;
    clearTimeout (t.displayTimer);
...
//logit ("enc: "+ inspect (enc, 6, 1));    
    ...
    s = '<div class=ptstat>Truppen in deiner Botschaft</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Es hat dir keiner verstärkung geschickt...! :/</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Player (Knightpunkte)</b></td>';
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
              s += '<TR align=right><TD align=left>'+ player + knight +' <A><SPAN onclick="r8x6Home('+ march.marchId +')">Zurückrufen</span></a></td>'
              for (i=1; i<13; i++){
    ...
    s += '<BR><BR><DIV style="font-size: 10px">Hinweis: Truppen werden erst nach Refresh Angezeigt! kein auto Refresh, also immer F5!</div>';
...
function CdispCityPicker (id, span, dispName, notify, selbut){
  ...
  if (dispName)
    m += ' &nbsp; <br><b>City Name:</b> &nbsp; <SPAN style="display:inline-block; width:85px; font-weight:bold;" id='+ id +'cname' +'></span>';
  span.innerHTML = m;
