...
function processingText(urlval, rt) {
    try{
        ...
        if(urlval == url_raw_weapon) {
            ...
            val_money_market = $(rt).find(".exchange_offers > tbody > tr > td.ex_rate > strong:eq(1) > span")[0].innerHTML;
            var temp = $(rt).find(".exchange_offers > tbody > tr > td.ex_amount > strong > span");
            var maxvalue = 0;
            for(j=0;j<temp.length;j++){
                tempval = parseFloat(temp[j].innerHTML);
                if(maxvalue < tempval) {
                    maxvalue = tempval;
                }
            }
...
function viewError(){
    //console.debug("Report.. "+ new Date());
    document.getElementById('test').innerHTML = "Error.. please relogin(session closed) erepublik and refresh(F5) this page."+ new Date()+"<br/>" + document.getElementById('test').innerHTML;
}
function testReport(){
    //console.debug("Report.. "+ new Date());
    document.getElementById('test').innerHTML = "Report.. "+ new Date()+"<br/>" + document.getElementById('test').innerHTML;
}
