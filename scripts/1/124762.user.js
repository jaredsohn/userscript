...
var SCRIPT = {
	name: 'DWO Structured',
	namespace: 'crohllf',
	source: 'http://userscripts.org/scripts/show/124762',
	identifier: 'http://userscripts.org/scripts/source/124762.user.js',
	meta: 'http://userscripts.org/scripts/source/124762.meta.js',
	version: '1.0',
	date: '2012-02-01',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4847903';
var postId = '44815664';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4847903&page=2';
// var postId = '44815664';
var forumAccessURL = 'http://goallineblitz.com/game/forum_private_access.pl?forum_id=10050';
...
var list = {
	...
	apprentice: {
		member: {},
		title: 'DWO Titans',
		style: 'color: DarkBlue; background-color: LightBlue; padding: 0px 3px 0px 3px;',
		agentTitle: function(id, name) { return 'dWo Titan'; },
		forumAccess: 1,
	},
	promoteme: {
		member: {},
	...
	waitlist: {
		member: {},
		title: 'DWO Immortals',
		style: 'color: LightGoldenrodYellow; background-color: black; padding: 0px 3px 0px 3px;',
		agentTitle: function(id, name) { return 'dWo Immortal'; },
		forumAccess: 1,
	},
	hitlist: {
		member: {},
		...
		agentTitle: function(id, name) { return '!! SHIT LIST !!'; },
		forumAccess: 0,
	},
};
var listName = {};
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			$.each(names, function(id, name) {
				if (!(id = parseInt(id))) return;
					...
					++nonMemberAddicts;
					$('#fam_list_family a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=10050&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.apprentice.member[id]) {
					$('#fam_list_apprentice a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=10050&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.waitlist.member[id]) {
					$('#fam_list_waitlist a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=10050&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list[idToList[id]].forumAccess == 1) {
					$('#post_content_' + postId + ' a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=10050&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				}
			});
			var prefix = ' (';
			...
			if (membersDWO) {
				statusMsg += prefix + '<b style="color: Red; background-color: black; padding: 0px 3px 0px 3px;;">' + membersDWO + ' DWO Members </b>';
				prefix = ', ';
			}
			if (membersApprentice) {
				statusMsg += prefix + '<b style="color: darkblue; background-color: lightblue; padding: 0px 3px 0px 3px;;">' + membersApprentice + ' Titans</b>';
				prefix = ', ';
			}
			if (membersPromoteMe) {
			...
			if (membersWaitList) {
				statusMsg += prefix + '<b style="color: LightGoldenrodYellow; background-color: black; padding: 0px 3px 0px 3px;;">' + membersWaitList + ' Immortals</b>';
				prefix = ', ';
			}
			if (membersHitList) {
			...
			statusMsg += ')';
			if (nonMemberAddicts) {
				statusMsg += ', ' + nonMemberAddicts + ' DWO member has left the building';
			}
