...
var dateString = document.getElementsByClassName('infohead')[0].getElementsByTagName('tr')[3].textContent
var m = /[^0-9-]+([0-9]+)[^0-9-]+([0-9]+)[^0-9-]+([0-9]+)[^0-9-]+([0-9]+)[^0-9-]+([0-9]+)[^0-9-]+([0-9]+)[^0-9-]+/g.exec(dateString);
...
var ress = /[^0-9-]+([0-9.]+)[^0-9-]+([0-9.]+)[^0-9-]+([0-9.]+)[^0-9-]+([0-9.]+)[^0-9-]+([0-9.]+)[^0-9-]+/g.exec(document.getElementsByClassName('summary')[0].textContent);

 
	var rslt = {win_metal:RegExp.$1.replace('.',''),win_cristal:RegExp.$2.replace('.',''),win_deut:RegExp.$3.replace('.',''),a_lost:0 ,d_lost:0,deb_metal:RegExp.$4.replace('.',''),deb_cristal:RegExp.$5.replace('.','')};
	var perte = document.getElementById('shortreport').getElementsByTagName('tr')[1].getElementsByClassName('value');
	
	rslt['a_lost']=perte[0].textContent.replace( /[^0-9-]/g, "");
	rslt['d_lost']=perte[1].textContent.replace( /[^0-9-]/g, "");
	
	var player;
	var coords;
	var type ='A';
	var weap ='Armes: 100% Bouclier: 100% Coques: 100%';
	
	
	var combatants = document.getElementsByClassName('textBeefy combatants')[0].getElementsByTagName('div');
	var nbatt = combatants[0].getElementsByTagName('div').length;

	var nbdef = combatants[nbatt+2].getElementsByTagName('div').length;
	var dec=1;
	
	for(var table=0; table<nbatt+nbdef ; table++)
	{
	
		player = combatants[table+dec].getElementsByTagName('span')[combatants[table+dec].getElementsByTagName('span').length-1].textContent;
		coords=combatants[table+dec].getElementsByTagName('a')[0].textContent.replace('[','').replace(']','');
		
		dat={};
		var i='fleet';
		for(var j= 202 ; j<503 ; j++)
		{
			dat[XtenseDatabase[i][j]] = 0;
			if(j==215) {j =400; i='defense'}
			else if(j==408) j=501;
		}
		
		data[table] = {player : player, coords : coords, type : type, weapons : weap, content : dat};
		
		if(table==nbatt-1) {dec = 3; type='D'}
