...
// @include     *://*.clixsense.com/en/View_Ad*
// @version     9.6.5
// ==/UserScript==

if( wparent.location.href.indexOf('/View_Ads') != -1 && top == self){
	var arr = [],ctr = 0;
	var div = $('<div>');
	var clickNum = ctr + 1 ;
	var loading = 0;
	...
	$.each($('.anc-style'),function(k,v){
		objj[k];
		if($(v).attr('onclick')){
			if($(v).attr('onclick').indexOf('./ajax_clicks.php') >= 0 && !$('.anuncio-clicked-title',$(v)).size()){
				var obj = {
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			...
			wparent.open("http://google.com","","width=100,height=100,top=1000,left=20000") ;
		}	
	}
	rec(ctr);
...
if(wparent.location.href.indexOf('/ClixGrid') != -1||wparent.location.href.indexOf('./ajax_clicks.php') != -1){
	var div = $('<div>');
	var ajaxCall = false;
...
if(wparent.location.href.indexOf('/ClixGrid') != -1){
	var div = $('<div>');
	var arr = [],ctr = 0;
	var clickNum = ctr + 1 ;
	var loading = 0;
