// ==UserScript==
// @name           Custom Reddit Header Bar2
...
(function(){
    var ContextMenu = function(){
        ...
        function init(){
            ...
            if(!_contextMenu)
                return;
            // clear other sr-bars so the user has more space to  work with
            _srBar = document.getElementById('sr-bar');
            var srBarParent = _srBar.parentNode;
            while (_srBar.previousSibling) {
                // Remove all the original menu items and separators.
                srBarParent.removeChild(_srBar.previousSibling);
            }
        ...
        function setupEditMenu(){
            ...
            editMenu.appendChild(createElement('a', {href: 'javascript:void(0);', className: 'choice', innerHTML: 'import - export'}, 
                {fontWeight: 'bold', width: '80%', display: 'block'}, function(){
                editMenu.style.display = 'none';
                importExportItems();
            }));
        ...
        function messupStyles(){
            document.querySelector("head").appendChild(
                createElement('style', { innerHTML: 
                    '#editMenu { width: 200px; maxHeight: 450px; overflow-y: auto; overflow-x: clip; background-color: white;  '
                    + ' border: 1px solid gray; z-index: 100; position: absolute; left: ' + (document.documentElement.offsetWidth - 210) + 'px; top: 15px}\n'
                    + '#editMenu .choice{ margin: 3px; font-size: 13px }\n'
                    + '#editMenu .choice *{ display: block; }\n'
                    + '#editMenu .choice .edit:hover{ background-color: #F0F0F0}\n'
                    + '#editMenu .btn {width: 8%; text-align: center; border: 1px solid #D00; background-color: #FFFFFF; font-weight: bold;}\n'
                })
            );
        }
        ...
        function importExportItems() {
            ...
            json = window.prompt("Your saved header items. To export simply copy (Ctrl + C) the text in the window below. To import paste it back in.", json);
            
            if (!json) {
                // If the user pressed cancel, we're done here.
                return;
            }
            ...
            try {
                ...
                alert("In valid bookmark text. Please ensure you've copied the text correctly.");
            }                
        }
