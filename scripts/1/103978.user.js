// ==UserScript==
// @name           Hatena Bookmark extension + Delicious bookmark count viewer
...
// http://www.google.co.jp/#q=site:ja.wikipedia.org

// deliciousルール:
//・100件位まとめて投げても返ってくる。
//・MD5にする場合、%23は#に。
//・jsonのurlに"www."や"index.html"などがあったりなかったり。正規表現で無い場合に備えることが必要。
//・jsonのurlは&が&amp;で返ってくる。replaceの順番注意、&に直してからエスケープ。

function deli(doc) {
  setTimeout(function() {
    var hbc = doc.querySelectorAll(".hBookmark-widget-counter img");
    if (!hbc.length) return;
    var q = "http://badges.del.icio.us/feeds/json/url/blogbadge?";
    for (var i = 0, n = hbc.length; i < n; i++) {
      q += "&hash=" + md5_hex(hbc[i].src.replace(/^http:\/\/b\.st-hatena\.com\/entry\/image\//,"").replace(/%23/g,"#"));
    }
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(res) {
        ...
        for (var i = 0, m = json.length; i < m; i++) {
          ...
          if (deriData.url.trim() == "") return;

          var deliUrl = RegExp(deriData.url.replace(/^https?:\/\/(?:www\.)?|\/$/g, "").replace(/&amp;/g, "&").replace(/([\W])/g, '\\$1').replace(/(\\\/index\\..+?)$/,'(?:$1)?'));

          for (var j = 0, n = hbc.length; j < n; j++) {
            var hbcImg = hbc[j];
            var hatenaUrl = hbcImg.src.replace(/^http:\/\/b\.st-hatena\.com\/entry\/image\//, "").replace(/%23/g, "#").replace(/^https?:\/\/|\/$/g, "");

            if (deliUrl.test(hatenaUrl)) {
              var a = document.createElement('a');
              a.target = "_blank";
              a.className = "_GM_delNav " + getLevel(deriData.total_posts);
              a.href = "http://delicious.com/url/" + deriData.hash;
              a.textContent = deriData.total_posts;
             
              if (!hbcImg.parentNode.parentNode.parentNode.querySelector('a[class*="_GM_delNav"]')) {
                hbcImg.parentNode.parentNode.insertBefore(a, hbcImg.parentNode);
                break;
              }
            }
          }
...
if (!!unsafeWindow.google && /google\./.test(location.href)) {
...
function getLevel(n) {
  return n>=10000 ? "_GM_savers5" : n>=1000 ? "_GM_savers4" : n>=100 ? "_GM_savers3" : n>=10 ? "_GM_savers2" : "_GM_savers1";
}
...
._GM_savers2 {
  background-color: #5592E9 !important;
...
._GM_savers4 {
  background-color: #2261A0 !important;
  width: 28px !important;
}

/*h3などにappendされてwidthが無い場合は横に伸びるがしょうがない。
http://www.xuldev.org/blog/?p=513 */
._GM_savers5 {
  background-color: #2261A0 !important;
}

._GM_delNav {
  font: bold 13px arial,helvetica,clean,sans-serif !important;
