...
// Adds support for standard Greasemonkey functions/objects
...
// Inserts the passed string of code in the page using a script tag (the inserted code will be executed in the page context)
// The code is removed after the insertion if removeAfter is true. This is useful to keep the DOM tree clean if the code is intended to be used one time
// If errorWrapper is a string, the code is inserted within a try/catch block (jsString in the try clause and errorWrapper in the catch clause [the exception variable will be called "err"])
function evalOnContent(jsString, removeAfter, errorWrapper) {

	if (typeof errorWrapper === "string") jsString = "try {\n" + jsString + "\n}\ncatch(err) {\n" + errorWrapper + "\n}";

	var codeEl = createNode("script", {type: "text/javascript"}, {textContent: jsString});

	var pageHead = document.getElementsByTagName("head")[0];
	pageHead.appendChild(codeEl);
	if (removeAfter) pageHead.removeChild(codeEl);

}

//'
function ytfCompatFireCustomEvent(evtName, evtTarget, evtDetail) {

	if (typeof evtDetail === "undefined") evtDetail = null;

	if ((evtName) && (evtTarget)) {
		var evtObj = document.createEvent("CustomEvent");
		evtObj.initCustomEvent(evtName, false, false, evtDetail);
		return evtTarget.dispatchEvent(evtObj);
	}
	else {
		return null;
	}
...
evalOnContent(ytfCompatFireCustomEvent.toString(), false);

//'
var unsafeWindow = null;
document.body.addEventListener("ytfgetUnsafeWindow", function(evt) {
	evt.currentTarget.removeEventListener("ytfgetUnsafeWindow", arguments.callee, false);
	if ((evt.detail !== null) && (typeof evt.detail === "object")) unsafeWindow = evt.detail;
}, false);
evalOnContent("ytfCompatFireCustomEvent('ytfgetUnsafeWindow', document.body, window);", true, "window.console.log('Error trying to get unsafeWindow: ' + err.toString());");

//'
if (unsafeWindow === null) {
	GM_log("unsafeWindow is null");
}
...
GM_addStyle([
	...
	"#ytf-conf-dialog {position: fixed; margin: auto; left: 0px; right: 0px; top: 0px; bottom: 0px; width: 475px; max-height: 610px; z-index: 1001; " +
...
var scriptVersion = 1361631582686; // 23 Feb 2013
...
if (ytPath === "/") {
	...
	if ($x1("//div[starts-with(@class, '" + YT_PLAYER_ROOT_CHANNEL_CLASS + "')]")) scriptChannelMain();
...
function scriptWatchMain() {
	...
	// Waits for the player insertion if it isn't in the page yet
		...
		function playerHTML5Insertion() {
			...
			try {
				watchVA = new VideoAdapterHTML5(YT_PLAYER_ID_HTML5, scriptWatchAPI, YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME);
	...
	function resizePlayer(sizeValue, doScroll) {
		...
		if (playlistBar) { // If the playlist exists it is resized too to match the player
			playlistDiv.style.paddingLeft = videoContainerDiv.style.paddingLeft;
...
function scriptChannelMain() {
	...
	var playerDiv = $x1("//div[starts-with(@class, '" + YT_PLAYER_ROOT_CHANNEL_CLASS + "')]");
	if (!playerDiv) {
	...
	playerDiv.addEventListener("DOMNodeInserted", function(evt) {
		var meSt = arguments.callee;
		var targetId = evt.target.id;
		if (targetId === YT_PLAYER_CHANNEL_ID_FLASH) {
			if (!meSt.skipReloadNextEvent) {
				meSt.skipReloadNextEvent = true; // Skips the next player insertion because the player reload (caused by a new VideoAdapter creation in channelPlayerInsertion) will trigger the listener
				channelPlayerInsertion(targetId);
			}
			else {
				meSt.skipReloadNextEvent = false;
			}
		}
		else if (targetId === YT_PLAYER_CHANNEL_ID_HTML5) {
			channelPlayerInsertion(targetId);
		}
	}, false);

	// Call chanelPlayerInsertion if the player is already in the page
	if ($(YT_PLAYER_CHANNEL_ID_FLASH)) {
		channelPlayerInsertion(YT_PLAYER_CHANNEL_ID_FLASH);
	}
	else if ($(YT_PLAYER_CHANNEL_ID_HTML5)) {
		channelPlayerInsertion(YT_PLAYER_CHANNEL_ID_HTML5);
	}
	...
	function channelPlayerInsertion(playerId) {
		...
		switch(playerId) {
			...
			// Initializes the VideoAdapter object for the Flash player
			case YT_PLAYER_CHANNEL_ID_FLASH:
			...
			// Initializes the VideoAdapter object for the HML5 player (incomplete support yet)
			case YT_PLAYER_CHANNEL_ID_HTML5:

				console.log("HTML5 player support is incomplete yet");

				try {
					channelVA = new VideoAdapterHTML5(playerId, null, YTF_DEFAULT_VIDEO_TITLE, YTF_DEFAULT_VIDEO_USERNAME);
