// ==UserScript==
// @name			Facebook Auto Like  V 0.212 By JOKOrowoTlogoRejo
// @namespace			FALV212
// @version			0.212
// @copyright			Achep Malm Steenzy
// @description			Auto Like And Confirm (mass) | You can mass like and confirm
// @author			Achep Malm Steenzy (http://userscripts.org/users/477924)
...
// Copyright (c) 2012, JokoRowoTlogoRejo
...
if(body != null) {
	...
	div.innerHTML = "<div style='background-color: #2E5392; color: #FFFFFF; border: 1px solid #333333;'><center><a style='color: #FFFFFF;' <a href='https://www.facebook.com/Asep.nyamer' title='Click to Achep Malm Steenzy on Facebook'> Achep Malm Steenzy </a></div>"
	...
	div2.innerHTML = "<div style='background-color: #FFFFFF; color: #FF0000; border: 1px solid #333333;'><a style='color: #FFFFFF;' onclick='spoiler()' title='Click to Hidden Autolike'>&laquo;</a>&nbsp;<a href='http://minds-freaks.blogspot.com/' target='_blank' title='Autolike Version 0.212 by Achep Malm Steenzy Corporation | Copyright 2012 | All Right Reserved' style='color: #FFFFFF;'>Autolike V 0.212</a> | <a style='color: #FF0000;' onclick='thanks()'><center>BACA DULU !!!</center></a></div>"
	
	body.appendChild(div);
	body.appendChild(div2);
	
	unsafeWindow.thanks = function() {
	alert("Gunakan Dengan Bijak Baik autolike by Achep Malm Steenzy dan Akan Selalu Memberikan Yang Terbaik Untuk Sahabat Semuanya:)\n\nAchep Malm Steenzy\nCopyright (c) 2012");
	}
	...
	for(i=1;i<=20;i++) {
		...
		div2.style.width = "125px";
		div2.innerHTML = "<div style='background-color: #FFFFFF; color: #FF0000; border: 1px solid #333333;'><a style='color: #FFFFFF;' onclick='spoiler()' title='Click to Hidden 

Autolike'>&laquo;</a>&nbsp;<a style='color: #FFFFFF;' href='http://minds-freaks.blogspot.com/' target='_blank' title='Autolike Version 0.212 by Achep Malm Steenzy Corporation | Copyright 2012 | All Right Reserved'>Autolike V 
...
if(body != null) {
	...
	unsafeWindow.AutoLike = function() {
		...
		for (var i = 0; i < prepare.length; i++)
			if(prepare[i].getAttribute("id")!=null&&prepare[i].getAttribute("id").indexOf(".reactRoot")>=0&&(prepare[i].innerHTML=="Me gusta"||prepare[i].innerHTML=="Like"||prepare

[i].innerHTML=="Suka"||prepare[i].innerHTML=="Been"||prepare[i].innerHTML==""||prepare[i].innerHTML=="Seneng"||prepare[i].innerHTML=="Jaime")) {
...
div.style.position = "fixed";
	...
	div.style.border = "1px solid #94a3c4";
	div.style.padding = "2px";
div.innerHTML = "<img src='https://lh4.googleusercontent.com/-D1HYuLwPnNQ/TxPK6cm_THI/AAAAAAAAAIE/ynATGaxGbv0/s16/Facebook%252520Like%252520Small.jpg' width='16' height='14' align='absmiddle' 

/>&nbsp;&nbsp; <a onclick='LikeComments()'>Like All Comments</a>"
body.appendChild(div);
	unsafeWindow.LikeComments = function() {
		...
		for (var i = 0; i < prepare.length; i++)
			if(prepare[i].getAttribute("data-ft")!=null&&(prepare[i].getAttribute("title")=="Me gusta este comentario"||prepare[i].getAttribute("title")=="Like this comment"||prepare[i].getAttribute("title")=="Suka 

komentar ini"||prepare[i].getAttribute("title")=="Nyenengi tanggapan iki"||prepare[i].getAttribute("title")==" "||prepare[i].getAttribute("title")=="Jaime ce 

commentaire"||prepare[i].getAttribute("title")=="Bu yorumu been")) {
				buttons[Counter] = prepare[i];
				Counter++;
			}
		function check_link(linknumber) {
