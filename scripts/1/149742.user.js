...
var isConnected	= get_dom_xpath("id('navigation-top')/li/a", document, -42);
				isConnected	= (isConnected.title=="Connexion") ? false : isConnected.href;

{
		document.getElementsByTagName("body")[0].appendChild(build_node("script", ["type"], ["text/javascript"], function putLogs(uni, pseudo, pass) {document.getElementById("username").value=pseudo;document.getElementById("password").value=pass;document.getElementsByName("login")[0].click();}));
		var clicngo	= build_node("div", ["style", "id"], ["float:right;cursor:pointer;padding:10px;", "clicngo"], "<img src='"+scripts_icons+"Clic&Go/connecting_people.png'></img>");
		...
		get_dom_xpath("id('bar')", document, 0).appendChild(clicngo);
		...
		var html  = "<div onclick='document.getElementById(\"clicngo_contents\").style.display=\"none\";document.body.style.opacity=\"1\";' style='padding-bottom:5px;cursor:pointer;text-align:center;color:#A6FF94;border-bottom:1px solid white;font-weight:bold;'>Clic & Go !</div>";
			...
			html += "<div>";
			html += "<input id='add_username' onclick='this.value=\"\";'  type='text' value='Pseudo' style='border:1px solid #545454;height:15px;padding:1px;vertical-align:middle;background-color:black;border-radius:5px 5px 5px 5px;color:#CDD7F8;font:13px Times New Roman normal;margin:5px 0 1px 2px;text-align:center;'/>";
				...
				img.addEventListener("click", function(){	var index = /clicngo_(\d*)/.exec(this.name)[1];
															function login()
															{
																...
																if(window.XMLHttpRequest)		xhr_object = new XMLHttpRequest();
																			...
																			return;	}
																var username = config.ClicNGo.usernames[index];
																var password = config.ClicNGo.passwords[index];
																xhr_object.open("POST", "ucp.php?mode=login", true);
																xhr_object.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
																xhr_object.send("username="+username+"&password="+password+"&viewonline=on&redirect=.%2Fucp.php%3Fmode%3Dlogin&redirect=index.php&login=Connexion"); 
																xhr_object.onreadystatechange = function()
																{
																	if(xhr_object.readyState == 4)	window.location.href="http://www.spaceswars.com/forum/index.php";
																}
															}
															if (isConnected != false)
															{
																...
																if(window.XMLHttpRequest)		xhr_object = new XMLHttpRequest();
																			...
																			return;	}
																xhr_object.open("GET", isConnected, false);
																xhr_object.send(null);
																if(xhr_object.readyState == 4)	login();
															}
															else	login();}, false);
