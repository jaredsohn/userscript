...
var run = function() {
    ...
    var input_ef = $('<select>').append('<option value=-1>All</option>').append('<option value=10>< 100%</option>').append('<option value=100>100%</option>').append('<option value=0>0%</option>').change(function() {
        list.each(function() {
    ...
    var input = $('<input>').attr({type: 'text', value: ''}).change(function() {
        ...
        list.each(function() {
            ...
            var cols = $('td', this);
            if ($(cols[2]).text().search(needle) == -1) {
                $(this).hide();
                if ($(this).hasClass('zz')) {
                    $(this).next().hide();
                }
    ...
    container.append('Search: ').append(input).append('Efficiency Filter: ').append(input_ef);
}
