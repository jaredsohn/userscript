// ==UserScript==
// @name           KOC Organisateur Salle de Throne Fr
// @version        18122012
// @namespace      mmm & philip
// @homepage       https://userscripts.org/scripts/show/154409
// @delay 1000
...
// @updateURL      https://userscripts.org/scripts/source/154409.meta.js
...
// @description    Organises, upgrades et supprime tes items dans ta salle
// ==/UserScript==


var Version = '18122012_mm';
...
function trStartup (){
    ...
    logit ("Organisateur de throne charger");
    ...
    var tStr= '<div style="width: auto; padding: 0px;" vAlign=top id="'+ prefix +'_bar" class="'+ topClass +' trTitle">Organisateur salle de Throne</div>';
    ...
    $("#tr_main").html(
            '<div>\
            <ul>\
            <li><a href="#tabs-upgrade">Action</a></li>\
            <li><a href="#tabs-salvage">Supprime</a></li>\
            <li><a href="#tabs-organize">Organise</a></li>\
            <li><a href="#tabs-estats">Stats Amélioration</a></li>\
            <li><a href="#tabs-ustats">Stats Niveaux</a></li>\
            <li><a href="#tabs-options">Options</a></li>\
            <li><a href="#tabs-log">Journal</a></li>\
            </ul>\
...
Tabs.throneSalvage = {
        ...
        init : function (div) {
            ...
            t.myDiv = div;
            var m = '<Div><DIV  id=trSalvage class=trStat>SUPPRIME AUTOMATIQUEMENT (Salvage)</div>';
            m += "<div id='trInfoArea'>";
            m += '</div>';
            ...
            if (salvageData.salvageActive == false) {
                m += '<tr><TD><div><INPUT id=trSalvagerPower type=button value="Salvager = NON"/></div></td>';
            } else {
                m += '<tr><TD><div><INPUT id=trSalvagerPower type=button value="Salvager = OUI"/></div></td>';
            }


            m += '<td colspan=3><b> Mettre aetherstones: </b><div style="display: inline;" id=trSalvageCity /></td>';

            m += '<tr><td><td/><tdtyle="text-align: center;"></td>'; 
            m += '</tr></table>';

            m += '<table class="trTabPad trTable"><hr/>';
            m += '<tr><td width=35%><div> Garder les: <select id="trSalvageQuality">';
            m += '<option value="1">Commun</option>';
            m += '<option value="2">Uncommun</option>';
            m += '<option value="3">Rare</option>';
            m += '<option value="4">Epic</option>';
            m += '<option value="5">Superbe</option>';
            m += '</select> et au dessus</div></td>';

            m += '<td colspan=2 width=24%>Garde les <INPUT id=trSaveNum type=text size=3 maxlength=3 value="' + salvageData.throneSaveNum+ '"/> premier  items de mon throne</td>';
            m += "</tr></table><hr/>";
            ...
            m += '<tr><td>  <b>Definir les TR items à recherché: </b> </td>';

            m += '<td alight="left"><div><span>Faction: <select id="trFactionType">';
            m += '  <option value="any">Tous</option>';
            m += '  <option value="fey">Fey</option>';
            m += '  <option value="briton">Briton</option>';
            m += '  <option value="druid">Druide</option>';
            m += '</select></span></div></td>';
            m += '<td alight="left"><div><span>Type: <select id="trCardType">';
            m += '  <option value="any">Tous</option>';
            m += '  <option value="chair">Throne</option>';
            m += '  <option value="table">Table</option>';
            m += '  <option value="window">Fenetre</option>';
            m += '  <option value="banner">Banniére</option>';
            m += '  <option value="advisor">Conseillé</option>';
            m += '  <option value="trophy">Trophet</option>';
            m += '</select></span></div></td>';
            m += '<td align="right"><INPUT id=trAddRule type=button value="Creer une régle"/></td>';
            m += '</tr></table>';
            m += '<TABLE  class="trTabPad trTable" width=100% id="trConditionTable"  style="padding-left: 5px;">';
            m += '<tr><td align=left colspan=1><INPUT id=trAddRow type=button value="Ajout ligne"/></td>';
            m += '<td></td><td></td><td></td><td></td><td></td></tr>';
            m += '</table>';
            m += '</div><hr/>';
            m += '<div id="trSalvStatus" style="text-align: center;" >Chargement ... </div>';
            m += '<div id="trNumSalv" style="text-align: center;">Nombre d\' items supprimés: '+ addCommas(salvageData.numSalvagedItems) + ' </div>';
            m += '<hr/>';
            m += '<div class=trRulePane>';
            m += '<div align=center> <b> Salvage Gardera que les items contenant une de ces régles </b></div>';
        ...
        createRow : function()
        {
            ...
            var h  = "<td> <select id='" + rowId + "sel1'> <option value='true'> </option> <option value='false'>Sans</option></select></td>";
        ...
        buildRuleDisplay : function ()
        {
            ...
            for (i =0; i < salvageData.ruleSet.length; i++)
            {
                ...
                m += "</div></td>";
                m += "<td width=20%><INPUT id=trDelRule" + i + " type=button value='Jete cet régle' /></td>";
                m += '</tr>';
            }
        ...
        doSalvage : function() {
            ...
            if (t.delItems.length > 0) {
                ...
                t.deleting = false;
                t.setStatus('Aucun items a été supprimés.  Attendre les prochains.');
            }
            t.setFullness();
        },

        setFullness : function () {
            // change the color on the throne button when full -> changer la couleur du bouton trône quand il est plein
        ...
        // Cree une liste item a supprimer.
        // If 'test' is set to true, then broken/equipted items are included -> Si 'test' est sur true, les articles cassés/equiptés sont inclus.
        buildList : function(test){
            ...
            for (k in unsafeWindow.kocThroneItems) {
                ...
                // ignore these things -> ignorer ces choses
                if (throne_item.level !=0) continue;

                // in test mode, include these items -> en mode test, inclure ces éléments
                // These items are at risk if they are repaired or unequiped. -> Ces éléments sont à risque si elles sont réparées ou déquipé.
                if (test != true)
                {
            ...
            new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                ...
                onSuccess: function (transport) {
                    ...
                    if(rslt.ok) {
                        ...
                        $("#trNumSalv").html('<div style="text-align: center;">Nombres d\'items supprimés: '+ addCommas(salvageData.numSalvagedItems) + ' </div>');
...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            // setup the tab
            var m = '<Div style="width: 710px"><DIV id=trOrganizer class=trStat>ORGANISEUR</div>';
            ...
            // preset selector
            m += '<TABLE  width=100% class="trTabPad2 trTable"><tr align=center><td width=15%/><td width=30%><div><span>Vue préset: <select id="PresetList">';
            ...
            m += '</select></span></div></td>';
            m += '<TD width=20%><INPUT id=testSalvage type=button value=" Test Supprime "/></td><td id=trDelResults></td>';
            m += '</tr>';


            m+= '<tr><td colspan=1><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 4px; margin-left: 5px;"> Bascule de preset: </span>';
            m+= '</td><td colspan=3>';
            ...
            if (presetData.num_presets > 0) m +=  "Remarque: Le préréglage final est remplacer quand vous utilisez le preset local.";
            m +=  "</span>" ;
            m += '</td></tr>';

            if (presetData.num_presets > 0) {
                m+= '<tr><td colspan=1><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 4px; margin-left: 5px;"> Remplace le préréglage local: </span>';
                m+= '</td><td colspan=3>';
                ...
                m+= '<tr><td colspan=1><span style="width: 130px; float: left; margin-top: 4px;  margin-right: 4px; margin-left: 5px;"> Sauve le préréglage local: </span>';
                m+= '</td><td colspan=3>';
                for (a=0; a< presetData.num_presets; a++)
                {
            ...
            m += '<tr align=center><td colspan=2><div><span>Description: <select id="trSortList">';
            ...
            m += "  <option value='both'>LesDeux</option>";
            m += "  <option value='buff'>Buff</option>";
            m += "  <option value='debuff'>Debuff</option>";
            m += "</select></td>";

            m += '<td><INPUT id=trSortInactive type=checkbox '+ (  upgradeData.sortInactive ?' CHECKED':'') +'/> Includ Inactive</td>';
            m +='</tr></table>';

            m += "<div id='trScrollDiv' style='position: static; width: 710px; height: 500px; overflow-x: hidden; overflow-y: auto;'>";
            ...
            m += "<tr align=center><th>Thrones</th><th>Tables</th><th>Fenétres</th><th>Banneiéres</th><th>Conseillers</th><th>Trophets</th></tr>";
            m += '</table></div>';
            m += '</div>';
            m += '</div>';
            ...
            $("#trScrollDiv").css('height', upgradeData.organizeH);
        ...
        loadLocalPreset : function (id) {
            ...
            {
                t.setSwitchStatus("Réglage local en progression ....");
                return;
            }
            ...
            {
                t.setSwitchStatus("Préréglage local est vide");
                return;
            }
            ...
            if (Seed.throne.slotNum != Seed.throne.activeSlot) {
                ...
                t.setSwitchStatus("Bascule au dernier reglages activé ...");
...
Tabs.trActionLog = {
        ...
        init : function (div){
            ...
            t.myDiv.innerHTML = '<div id=logScroll style=" position: static; width: 710px; height: 500px; overflow-x: hidden; overflow-y: auto;" >\
                <DIV class=trStat>JOURNAL UPGRADE  - VERSION: '+ Version+'</div>\
...
Tabs.options = {
        ...
        init : function (div) {
            ...
            m += '<TR><TD width=100%><INPUT id=trupdate type=checkbox '+ (TRGlobalOptions.trUpdate?'CHECKED ':'') +'/>Vérifier les mise a jour du script sur userscripts.org (tout domains) &nbsp; &nbsp; <INPUT id=trupdatenow type=button value="Mettre a jour" /></TD></TR>';
            m += '<TR><TD width=100%><INPUT id=trDisableAnim type=checkbox '+ (upgradeData.disableAnim?'CHECKED ':'') +'/> Supprime animation ( X echec) </TD></TR>';

            m += '<TR><td><div  class=trStat>Options des suppréssions (Salvage)</div></td></TR>';
            m += '<tr><td><div>Charge ces paramétres pour le domaine: <INPUT id=trLoadDomain type=text size=3 maxlength=3 /><INPUT id=trLoadRules type=button value="Charge"/></div></td></tr>';
            m += '<TR><TD><div style="white-space: pre;" ><INPUT id=trSalAnyCity type=checkbox '+ (salvageData.anyCity?' CHECKED':'') +'/> Quand la ville 1 est pleine, Mettre aethers sur autre ville. ';
            m += '    Hauteur Maximum aethers: <INPUT id=trMaxStone type=text size=7 maxlength=7 value="' + salvageData.maxStones+ '"/></div></td></TR>';
            m += '<TR><TD>Méthode de partage: <select id=trOverflow><option value="order">Ordre des villes</option><option value="lowest">Moins remplies</option>  </select></TD></TR>';
            m += '<TR><TD><div style="white-space: pre;" ><INPUT id=trSalUpgradeFirst type=checkbox '+ (salvageData.upgradeFirst?' CHECKED':'') +'/> Upgrade items a +1 avant de supprimer.';
            
            m += ' Maximum qualitée: <select id="trSalUpgradeQuality">';
            ...
            m += '<TR><td><div  class=trStat>Options Upgrader (monte un niveau)</div></td></TR>';
            m += '<tr><td width=25%>intervalle (secondes): <INPUT id=trUpRefresh type=text size=3 maxlength=3 value="' +upgradeData.retryInterval+ '"/></td></tr>';
            m += '  <tr><td colspan=2><div style="white-space: pre;"><INPUT id=trAnyCity type=checkbox '+ (upgradeData.anyCity?' CHECKED':'') +'/> Lorsque la ville 1 est vide, utilisez aethers sur autre ville.  ';
            m += ' Hauteur Minimum aethers: <INPUT id=trMinStone type=text size=7 maxlength=7 value="' +upgradeData.minStones+ '"/> </div></td></tr>';
            m += '<tr><td colspan=2>  <INPUT id=trWhisperCheck type=checkbox '+ (upgradeData.whisperToMe?'CHECKED ':'') +'/> Murmure moi sur upgrades reussi</td></tr> ';
            m += '<tr><td colspan=2>  <INPUT id=trBuffsCheck type=checkbox '+ (upgradeData.buffsOff?'CHECKED ':'') +'/> Désactive les jetons Automatiquement</td></tr> ';
            m += '<tr><td colspan=2><INPUT id=trSafetyCheck type=checkbox '+ (upgradeData.safetyOn?'CHECKED ':'') +'/> Désactive les mise à niveaux upgrade si moins de <input id=trSafetyLimit size=10 maxlength=10 value="' + upgradeData.safetyLimit +'" /> aetherstones </td></tr> ';
         
            m += '<TR><td><div  class=trStat>Organiseur Options</div></td></TR>';
            m += '<TR><td><div style="text-align:center;"> Preset Noms/Descriptions</div></td></TR>';
            ...
            for (j=0; j<presetData.num_presets; j++ ) 
            {
                ...
                m += '<tr><td style="width: 650px; white-space: nowrap;">Nommer: <INPUT class=trNameEntry id=trPresetName' + j + ' type=text size=8 maxlength=8 value="' + presetData.ids[j] + '"/> Description: <INPUT class=trDescEntry id=trPresetDesc' +j+' type=text size=80 maxlength=100 value="' + presetData.desc[j] + '"/></td></tr>';
            }

            m += '<TR><TD><div style="white-space: pre;" >Nombre de presets: <INPUT id=trNumPresets type=text size=4 maxlength=2 value="' +presetData.num_presets+ '"/>  <INPUT id=trNoTooltips type=checkbox '+ (presetData.noTooltips?' CHECKED':'') +'/> Ne pas montrer l\Item en grand dans l\Onglet Organise</div></td></tr>';
            m += '</table></div>';
...
Tabs.upgrader = {
        ...
        qualities :  [ "Simple", 
                       ...
                       init : function (div){
                           ...
                           // header stuff
                           var m = '<Div id=trfield><DIV id=trUpgrader class=trStat>UPGRADER AUTOMATIQUEMENT</div><TABLE id=trupgrader width=100% height=0% class=trTab>';
                           m+= '</table></div>';
                           ...
                           if (upgradeData.active == false) {
                               m += '<TD width=25%><div><INPUT id=trUpgraderPower type=button value="Upgrade = NON"/></div></td>';
                           } else {
                               m += '<TD width=25%><div><INPUT id=trUpgraderPower type=button value="Upgrader = OUI"/></div></td>';
                           }

                           m += '<td width=25%><INPUT id=trOneItem type=checkbox '+ (queueData.oneItem ? ' CHECKED':'') +'/>  Upgrade 1 à 1</td>';
                           m += '<td width=25%><INPUT id=trRepairAll type=checkbox '+ (upgradeData.repairAll?' CHECKED':'') +'/>  Répare les items cassés</td>'; 
                           m += '<td width=25%/></tr>';

                           m += '  <tr><td colspan=3><div ><b>Prendre aethers sur: </b><span id="trUpgradeCity"></span></div></td>';
                           ...
                           m += '<option value="both">Deux</option>';
                           m += '</select></div></td>';
                           ...
                           m += '<tr align=center><div><td><input style="float: left;" id=trClearQ type=button value="Vide la file"/></div></td><td colspan=1></td><td colspan=2><a id=trpplink><img id=trpp /></a></td></tr>';
                           m += '</table>';
                           ...
                           if (Seed.queue_throne != null && Seed.queue_throne.end != null)
                           {
                               ...
                               t.setStatus("Termine à " + n.toLocaleTimeString() + " la réparation.  Item: " + unsafeWindow.kocThroneItems[t.repairId].name);
                       ...
                       addBothItem : function (id) {
                           ...
                           if (!throne_item)
                           {
                               logit("Impossible de trouver l\item.");
                               return;
                           }
                           ...
                           try {
                               ...
                               {  
                                   ...
                                   if (queueData.index <0) {
                                       t.setStatus("Fin de la file atteinte.")
                                   ...
                                   retryTime = ti + 5;
                                   var n = new Date(t.repairEnd *1000);
                                   t.setStatus("Attendre " + n.toLocaleTimeString() + " pour compléte réparation.  Item: " + unsafeWindow.kocThroneItems[t.repairId].name);
                               }
                               unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
                           } catch(e) {
                       ...
                       doEnhance : function(eItem) {
                           ...
                           try {
                               ...
                               if ( num_city < 0)
                               {
                                   t.setStatus("Manque aetherstones pour enhance.  Besoin de " + upgradeData.minStones + " au minimun.  Mise en attente ...");
                                   return;
                               }

                               var t_city = unsafeWindow.currentcityid;
                               unsafeWindow.currentcityid = Seed.cities[num_city][0];
                               var w = unsafeWindow.cm.ThronePanelController.calcCost("enhance", y, null, "Aerther");
                               unsafeWindow.currentcityid = t_city;

                               if ( (w.gems.use > 0) || (w.stones.total > Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0] ))
                               {
                                   t.setStatus("Manque d\' aetherstones pour enhance.");
                                   return; 
                               }
                               ...
                               t.setStatus("Envoi une requette pour enhance ");
                               new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                                   method: "post",
                                   parameters: params,
                                   loading: true,
                                   onSuccess: function (transport) {
                                       ...
                                       if(rslt.ok){
                                           ...
                                           if (rslt.gems > 0)
                                           {
                                               trActionLog('Upgrader accidentellement avec des  gems!  Upgrader mis hors tension');
                                               t.setStatus("Erreur ... arreter");
                                           ...
                                           if (rslt.success)
                                           {
                                               ...
                                               y.name = y.createName();
                                               Tabs.upgrader.repaint();
                                               t.setResult("Enhance réussi.  "  + addCommas(rslt.aetherstones) + " aethers utilisés.");
                                               t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                                               t.setStatus("Prochaine Tentative");
                                               ...
                                               {
                                                   var now = new Date();
                                                   qItem.lastUpgrade = "Enhance pour " + Tabs.upgrader.qualities[y.quality] + " " + now.toDateString().substring(3,10) + " " + now.toTimeString().substring(0,8)  + " sur " + qItem.triesThis + " Tenter";
                                                   if (!qItem.upgrades) qItem.upgrades = [];
                                                   qItem.upgrades.push(qItem.lastUpgrade);

                                                   var msg = 'Enhance: '+unsafeWindow.kocThroneItems[eItem].name + ' en qualitée, ' + rslt.item.quality+ " pour " + qItem.triesThis + " tentatives. " + qItem.triesTotal + " Tentatives totale pour cet item.";
                                                   trSuccessLog(msg);
                                                   if (upgradeData.whisperToMe) sendChat("/"+ Seed.player.name +' ' + msg);
                                                   
                                                   if (qItem.level <= y.quality) {
                                                       qItem.status = "compléte";
                                                       upgradeData.newUpgradeState = 2;
                                                   }
                                                   else {
                                                       var now = new Date();
                                                       qItem.status = "Partiéllement enhance";
                                           ...
                                           else
                                           {
                                               ...
                                               trActionLog('Enhance échouer de cet item: '+unsafeWindow.kocThroneItems[eItem].name);
                                               ...
                                               {
                                                   y.isBroken = true;
                                                   y.brokenType = "qualitée";
                                               }
                                               y.name = y.createName();
                                               t.setResult("Enhance Raté.  "  + addCommas(rslt.aetherstones) + " aethers utilisés");
                                               t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                                               //t.setStatus("Starting repair ...");
                                               var qItem = queueData.list[queueData.index];
                                               if (qItem)  if (qItem.status == "Attente") qItem.status = "Activer";
                                               saveQueueData();
                                               Tabs.upgrader.buildQueueDisplay();
                                               clearTimeout(Tabs.upgrader.timerH);
                                       ...
                                       {
                                           logit("enhance erreur");
                                           logit("Enhance resultat:" + inspect (rslt, 3, 1));
                                           t.setStatus("Impossible à améliorer pour le moment ... attendre le prochain tour");
                                           
                                      }
                                       return;
                                   },
                                   onFailure: function (rst) {
                                       logit("Enhance erreur de requete.  attendre le prochain tour.");
                                       t.setStatus("Impossible d\'envoyer une requette enhance.  attendre le prochain tour");
                                       logit("resultat: " + inspect(rst,3,1));
                                       // try to repair item
                                       //t.doRepair(y.id);
                       ...
                       doUpgrade : function(uItem, bypass) {
                           ...
                           if ( num_city < 0)
                           {
                               t.setStatus("Manque aetherstones pour upgrader.  Besoin de " + upgradeData.minStones + " au minimun.  Mise en attente ...");
                               return;
                           }

                           var t_city = unsafeWindow.currentcityid;
                           unsafeWindow.currentcityid = Seed.cities[num_city][0];
                           var w = unsafeWindow.cm.ThronePanelController.calcCost("upgrade", y, null, "aethers");
                           unsafeWindow.currentcityid = t_city;

                           if ( (w.gems.use > 0) || (w.stones.total > Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0] ))
                           {
                               t.setStatus("Manque aetherstones pour upgrader.");
                               return; 
                           }
                           ...
                           new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                               ...
                               onSuccess: function (transport) {
                                   ...
                                   if(rslt.ok){
                                       ...
                                       if (rslt.success)
                                       {
                                           ...
                                           if (bypass !=true)
                                           {
                                               ...
                                               t.setResult("Upgrade reussi.  "  + addCommas(rslt.aetherstones) + " aetherstones utilisés.");
                                               t.setStatus("Prochaine tentative upgrade");
                                               unsafeWindow.cm.sounds.play("tr_success_build");

                                               var qItem = queueData.list[queueData.index];
                                               if (qItem) {
                                                   var now = new Date();
                                                   qItem.lastUpgrade = "Upgrade niveau +" + y.level + " " + now.toDateString().substring(3,10) + " "+ now.toTimeString().substring(0,8) + " sur " + qItem.triesThis + " tentatives";
                                                   
                                                   if (!qItem.upgrades) qItem.upgrades = [];
                                                   qItem.upgrades.push (qItem.lastUpgrade);

                                                   var msg = 'Upgraded '+unsafeWindow.kocThroneItems[uItem].name + ' du niveau ' + rslt.item.level + " pour " + qItem.triesThis + " tentatives. " + qItem.triesTotal + " tentatives au total pour cet item.";
                                                   if (upgradeData.whisperToMe) sendChat("/"+ Seed.player.name +' ' + msg);
                                                   trSuccessLog(msg);
                                                   if (qItem.level <= y.level) {
                                                       qItem.status = "complet";
                                                       upgradeData.newUpgradeState =2;
                                                   }
                                                   else {
                                                       var now = new Date();
                                                       qItem.status = "Partiéllement upgrader";
                                                       qItem.triesLast = qItem.triesThis;
                                                       qItem.triesThis = 0;
                                                       if (upgradeData.newUpgradeState !=2) upgradeData.newUpgradeState = 1;
                                                   }
                                       ...
                                       else
                                       {
                                           ...
                                           t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                                           trActionLog('Upgrade Echoué de l\'item '+unsafeWindow.kocThroneItems[uItem].name);
                                           if (rslt["Echouer"]) {
                                              y.isBroken = true;
                                              y.brokenType = "niveaux";
                                           }
                                           y.status = rslt.item.status;
                                           y.name = y.createName();
                                           if (bypass !=true)
                                           {
                                               ...
                                               saveUpgradeStats();
                                               t.setResult("Upgrade raté.  "  + addCommas(rslt.aetherstones) + " aethers utilisés");
                                               
                                               //t.setStatus("Starting repair ... ");
                                               var qItem = queueData.list[queueData.index];
                                               if (qItem.status == "Attente") qItem.status = "démarre";
                                               saveQueueData();
                                               Tabs.upgrader.buildQueueDisplay();
                                   ...
                                   {
                                       if (bypass !=true) t.setStatus("Upgrade non accepté.  attendre le prochain tour.");
                                       logit("Upgrade résultat:" + inspect (rslt, 3, 1));
                                       
                                   }
                                   return;
                               },
                               onFailure: function (rrr) {
                                   t.setStatus("Impossible de transmettre la requette.  attendre le prochain tour.");
                                   unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
                       ...
                       buildQueueDisplay : function () {
                           ...
                           $("#trQueue").append("<tr><th width=5%>Trier</th><th width='8%'>Status</th><th width='25%'>Item</th><th width='5%'>Action</th><th width='5%'>Max</th><th width='40%'>Status/Dernière mise a niveau/Tentatives</th><th width='10%'>Retire</th></tr>");
                           ...
                           for (ii in queueData.list) {
                               ...
                               var name = "Inconnu / Item supprimé";
                               ...
                               m += " / " + q.triesThis + " éssaies pour ce niveau, " + q.triesTotal + " tentatives au total"
            ...
            .slideDown(700);
                       ...
                       updateTRTab : function() {
                           $("#trexecupgrade").html("Upgrade " + (upgradeData.active ? "OUI" : "NON"));
                       },
                       ...
                       doRepair : function( rItem) {
                           ...
                           new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                               ...
                               onSuccess: function (transport) {
                                   ...
                                   //logit("rslt: " + inspect(rslt,3,1));
                                   if(rslt.ok){
                                       trActionLog('Démarre la réparation de l\' item: '+unsafeWindow.kocThroneItems[rItem].name);
                                       ...
                                       var n = new Date(t.repairEnd *1000);
                                       t.setStatus("Réparation commencé ... La réparation sera complète a: " + n.toLocaleTimeString() + ". Item: "  + unsafeWindow.kocThroneItems[t.repairId].name);
                                   ...
                                   {
                                       logit ("Réparation échoué");
                                       if (rslt.msg == "Item est pas cassé")
                                       {
                                       ...
                                       logit("résultat:" + inspect(rslt,3,1));
                                   }
                                   return;
                               },
                               onFailure: function (ttt) {
                                   logit("erreur réparation");
...
Tabs.estats = {
        ...
        buildDisplay :function()   {
            var t = Tabs.estats;
            var m = '<DIV class=trstat style="margin-top:5px; margin-bottom:5px;"><DIV id=trStatsMain class=trStat>STATISTICS ENHANCER (Amélioration)</div>';
            ...
            m += '<TR valign=top align=center><TH colspan=6>Nombres Enhancer (succées/échoués)</TH></TR>';
            ...
            m += '<TR valign=top align=center><th> Pourcentages: </th>';
            for( q =0; q <5; q++)
            {
                m += "<td style='font-weight: bold;' class='td" + (q%2) +"'  >";
                if ( (st[q] + ft[q]) == 0 )
...
Tabs.ustats = {
        ...
        {
            var t = Tabs.ustats;
            var m = '<DIV class=trstat style="margin-top:5px; margin-bottom:5px;"><DIV id=trStatsMain2 class=trStat>STATISTICS UPGRADE (monte de niveau)</div>';
            m += '<DIV id= trStatsTabDiv2 align=left style="margin-top:10px; margin-bottom:10px; margin-left: 30px;">';
            m += '<TABLE class=trStatTab align=center cellspacing=0>';

            m += '<TR valign=top align=center><TH colspan=12>Nombres Upgrade  (succés/échécs)</TH></TR>';
...
var AutoUpdater_132329 = {
        ...
        enable: function() {
            GM_registerMenuCommand("Active "+this.name+" mise à jour", function() {
        ...
        compare: function(xpr,response) {
            ...
            if ( (this.xversion) && (this.xname[1] == this.name) ) {
                ...
                if ( (xpr.responseText.match("La page demander n\'éxiste pas")) || (this.xname[1] != this.name) ) {
            ...
            if ( updated ) {
                display_confirm('Une nouvelle version du '+this.xname+' est dispo.\nDo you wish to install the latest version?',
                        // Ok
                        function(){
                    try {
                ...
                function(){
                    if ( AutoUpdater_132329.xversion ) {
                        if(confirm('Voulez vous désactiver la MàJ de ce script?')) {
                            // GM_setValue('updated_132329', 'off');
                            TRGlobalOptions.trUpdate = false;
                            GM_setValue ('TROptions_??', JSON2.stringify(TRGlobalOptions));
                            AutoUpdater_132329.enable();
                            alert('La MàJ automatique pour ce script pourra étre réactiver dans module complementaire de votre moteur de recherche.');
                        }
                    }
                }
                );
                ...
                alert('Aucune MàJ pour '+this.name);
            }
        },
        check: function(tf) {
            if (!tf){
                ...
                GM_registerMenuCommand("Verifier "+this.name+" pour MàJ", function() {
                    GM_setValue('updated_132329', new Date().getTime()+'');
                    AutoUpdater_132329.call(true, true)
                });
            }
