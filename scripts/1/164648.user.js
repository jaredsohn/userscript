// ==UserScript==
// @name        Hachi
// @namespace   HachiTools
// @description Just a set of statistics & summaries about repair time and base resources. Mainly for internal use, but you are free to test and comment it.
// @version     2.0.0.0
// @author      DaRkSiDe Hachi
        ...
        qx.Class.define("MaelstromTools.Language", {
          ...
          members: {
            ...
            loadData: function(language) {
              ...
              this.Data["Repair all defense buildings"] = ["Alle Verteidigungsgebäude reparieren", "Reparar todos os edifícios de defesa", "Réparer tous les bâtiments de défense"][l];
              ...
              this.Data["Autorepair defense (higher prio than buildings)"] = ["Repariere Verteidigung automatisch (höhere Prio als Gebäude)", "Auto reparar defesa (maior prioridade do que os edifícios)", "réparation automatique la défense (priorité plus élevé que les bâtiments) "][l];
          ...
          members: {
            ...
            initialize: function() {
              try {
                ...
                this.createNewImage("RepairAllDefense", "ui/"+factionText+"/icons/icon_def_army_points.png", fileManager);
                ...
                this.buttonRepairAllDefense = this.createDesktopButton(Lang.gt("Repair all defense buildings"), "RepairAllDefense", true, this.desktopPosition(desktopPositionModifier));
            ...
            runAutoCollectTimer: function() {
              try {
                ...
                if(CCTAWrapperIsInstalled()) {
                  ...
                  if(this.checkRepairAllDefense() && MT_Preferences.Settings.autoRepairDefense) {
                    this.repairAllDefense();
                  }
            ...
            repairAllDefense: function() {
              try {
                ...
                console.log("MaelstromTools.repairAllDefense: ", e);
              }
            },
          ...
          members: {
            ...
            setWidgetLabels: function() {
              try {
                ...
                var chkAutoRepairDefense = new qx.ui.form.CheckBox(Lang.gt("Autorepair defense (higher prio than buildings)")).set({
                ...
                MaelstromTools.Util.addElement(this.Widget, rowIdx++, colIdx, chkAutoRepairDefense, 2);
                ...
                this.addFormElement(MaelstromTools.Preferences.AUTOREPAIRDEFENSE, chkAutoRepairDefense);
              ...
              try {
                ...
                for(var i = 0; i < cityEntities.length; i++) {
                  ...
                  for(var x = 0; x < unitLevelRequirements.length; x++) {
                    loot[unitLevelRequirements[x].t] += unitLevelRequirements[x].c * cityEntity.get_HitpointsPercent();
        ...
        qx.Class.define("MaelstromTools.Wrapper", {
          ...
          statics: {
            ...
            GetUnitLevelRequirements: function(cityEntity) {
              return (cityEntity.get_UnitLevelRequirements() != null ? cityEntity.get_UnitLevelRequirements().rer : null);
            },
            ...
            getPrioList: function (city, arTechtypes, eModPackageSize, eModProduction, bOnlyTopBuildings, bOnlyAffordableBuildings) {
              try {
                ...
                for (var d = 0; d < buildings.length; d++) {
                  ...
                  var TechLevelData = ClientLib.Base.Util.GetTechLevelData_Obj(city_building.get_CurrentLevel() + 1, city_building.get_TechGameData_Obj());
                  ...
                  for (var costtype in TechLevelData.rr) {
                    if (typeof(TechLevelData.rr[costtype]) == "function") {
                      continue;
                    }
                    if (TechLevelData.rr[costtype].t == "0") {
                      continue;
                    }
                    
                    oCosts[TechLevelData.rr[costtype].t] = TechLevelData.rr[costtype].c;
                    if (parseInt(TechLevelData.rr[costtype].c) <= 0) {
                      continue;
                    }
                    RatioPerCostType[costtype] = TechLevelData.rr[costtype].c / resbuilding["GainPerHour"];
                    if (sCosts.length > 0) {
                      sCosts = sCosts + ", ";
                    }
                    sCosts = sCosts + webfrontend.gui.Util.formatNumbersCompact(TechLevelData.rr[costtype].c) + " " + MaelstromTools.Statics.LootTypeName(TechLevelData.rr[costtype].t);
                    if (sRatio.length > 0) {
                      sRatio = sRatio + ", ";
                    }
                    // Upgrade affordable ?
                    if (city.GetResourceCount(TechLevelData.rr[costtype].t) < TechLevelData.rr[costtype].c) {
                      switch (TechLevelData.rr[costtype].t) {
                        case ClientLib.Base.EResourceType.Tiberium:
                        {
                          bHasTiberium = false;
                          if (TotalTiberium < TechLevelData.rr[costtype].c) {
                    ...
                    var techlevelData = MaelstromTools.Statics.LootTypeName(TechLevelData.rr[costtype].t);
                    ...
                    oTimes[TechLevelData.rr[costtype].t] = 0;
                    if (oCosts[TechLevelData.rr[costtype].t] > city.GetResourceCount(TechLevelData.rr[costtype].t)) {
                      oTimes[TechLevelData.rr[costtype].t] = (3600 * (oCosts[TechLevelData.rr[costtype].t] - city.GetResourceCount(TechLevelData.rr[costtype].t))) / dCityProduction;
                    }
                  }
