...
var miniURLprof = document.location.href.match(new RegExp(/http:\/\/www.fotka.pl\/profil\/\w+[\/]/));
if(!miniURLprof){
	if(u.comments){
		if(u.pager){
...
if(miniURL){
	...
	menuCommand.innerHTML = "<img src='http://teczka.szatana.eu/favicon.ico' width=\"16\" height=\"16\" /> Teczka tego zdjęcia";
...
link.innerHTML = "<img src='http://teczka.szatana.eu/favicon.ico' width=\"16\" height=\"16\" />";
link.href = "http://teczka.szatana.eu/szukaj.php?mode=autor&s=" + u.profile_login;
link.target = "_blank";
$("#obok-av h1").append(link);


function dodajLink_stat(){
	var link = document.createElement("a");
	link.href = "javascript:void(0)";
	link.innerHTML = "◄archiwizuj status►";
...
function dodajLinki_prof(e){
	...
	komcie.each(function(){
		...
		if(kom.attr("teczka") != "tak"){
			...
			if(!u.pager){
				link.attr("comment_page", Math.ceil(($('#comments-list .comments-item').length - 1 - ileZłych)/10)  );
			}else{
