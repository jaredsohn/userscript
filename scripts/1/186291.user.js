// ==UserScript==
// @name           bro3_item_collect.user.js
// @namespace      http://homepage3.nifty.com/Craford
// @include        http://*.3gokushi.jp/*
// @description    ブラウザ三国志 アイテム一括回収
// @version        1.05
// ==/UserScript==
// version date       author
// ver1.00 2012.06.23 Craford トライアル。
// ver1.01 2012.06.24 Craford アイテム回収できないときにメッセージを出すようにした。
//                            mixi以外はcssの違いかボタンが隠れてしまうので、とりあえずいまのとこ放置＞＜
// ver1.02 2012.06.27 Craford アイテム回収の厳密性アップ、および水鏡娘のようなカードも回収できるようにしてみる。
// ver1.03 2012.07.08 Craford 地味にめんどくさいのでアイテムとカードをまとめて回収するようにする。
// ver1.04 2012.10.26 Craford BPコイン一括回収ボタンを追加。
// ver1.05 2012.11.03 Craford 資源UPアイテムをそれぞれ1個づつだけ使うボタンを追加。
...
//leyon ori		j$("#collectAllItemAndCards").click(function(){collectAllItemAndCards();});
        collectAllItemAndCards();
	}
...
//leyon ori		j$("#useAllBPCoins").click(function(){useAllBPCoins();});
//leyon ori		j$("#useResources").click(function(){useResources();});
useAllBPCoins();
useResources();
	}

	else if (location.pathname == "/card/status_info.php" ) {
	
	var pos1,pos2;
	var addr;
	           reward2 = document.getElementsByClassName('status_levelup')[0].innerHTML;
       //    alert(reward2);
           					pos1 = reward2.indexOf("cid=");
           					pos2 = reward2.indexOf("&amp");
           				//	alert(pos1);
           				//	alert(pos2);
           	                addr = reward2.substring(pos1,pos2);
           	                alert("here");
			...
			function(){
				...
				if( match != null && match[1] == '2' ){
					var opt = {
						...
						url: "http://" + HOST + "/item/inbox.php",
						data: "ssid=" + ssid + "&item_id=" + match[2],
						onload: function(res) {
							itemCount++;
...
//			alert("使用できる資源アイテムはありません。");
			location.href = "http://" + HOST + "/village.php"; //leyon add test
			return;
		}

//leyon ori		if( !confirm("資源UPアイテムを1日分使いますか？") ){
// leyon ori			return;
// leyon ori		}
		...
		location.href = "http://" + HOST + "/village.php"; //leyon add
	}
