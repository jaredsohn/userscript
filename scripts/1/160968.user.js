// ==UserScript==
// @name YoGame mesenge convecter v0.2 new 20x sped uni http://xnova.danubis.eu/u1/
// @author Karamba
// @description: Inserting cargo no. forom spy repotr
// @include      */game.php?page=messages*
// ==/UserScript==
	...
	for (i = 9; i < spycargo_tr.length; i++) { 
		...
		if (spycargo_tr[i].getElementsByTagName('th')[3] && spycargo_tr[i].getElementsByTagName('th')[3].getElementsByTagName('span')[0]) {
			if (spycargo_tr[i].getElementsByTagName('span')[0].className == 'espionagereport'){
				...
				spycargo_resource = '<br><table width="400"><tbody><tr><td class="c" colspan="4">Maximum Resources</td></tr><tr><td>Total Resources:</td><td>' + spycargo_adddot(spycargo_tot_res) + '</td><td>Raid:</td><td>' + spycargo_adddot(spycargo_metal + spycargo_crist + spycargo_deute) + '</td></tr><tr><td>Large Cargo:</td><td>' + spycargo_adddot(spycargo_CP_num) + '</td><td>Small Cargo:</td><td>' + spycargo_adddot(spycargo_CL_num) + '</td></tr>';
