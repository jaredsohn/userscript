...
// To Do: 
//
//  Fix cache miss on wikipedia svg images.
//  Check last checked date to make sure it's not in the future.
//  Have check contrast function take alpha in account.
//  Create a function to make sure the DataStore is enabled and revert to other storage if needed.
//  Use chrome's auditor to determine which css rules are unused.
//  Write code to ignore current user's image when not on user's profile. 
//  Make a json doc which is a mapping from FB images that need replacing to final images.
//  Fix issue that wrong colors are displayed if the ajax requests get re-ordered. (done)
...
//
// v2.9.5 Added support for new image viewer. Fixed coloring of groups & events. Added Firefox4 support.
// v2.9.3 Additional support for secure connections.  Fixing icons at top left.
// v2.9.2 Cut CPU usages in half. Made fade between color schemes smoother & slightly longer.  
// v2.7.0 Support for Google Chrome. Support for secure connections.
