...
// @include     http://en70.tribalwars.net/*
// @grant       none
// @version     1
// ==/UserScript==
		...
		if( document.getElementById( "main_buildlink_"+next_building)){
			...
			if( wood >0 || stone>0 || iron>0){
				...
				if( get_resource){
					...
					var w= window.open("http://en70.tribalwars.net/game.php?village="+get_resource+"&screen=market", "auto_transfer_"+coord+"_"+wood+"|"+stone+"|"+iron);
					//autoreload(30*60*1000);
					window.name="auto_builder";
					window.setTimeout( function(){window.location= "http://en70.tribalwars.net/game.php?village="+myid+"&screen=market"; }, 60000);
				}
			}
		}
	}
...
if( production_table){
	...
	jQuery("#button_building_link").click(function(){
		...
		for( var i=1; i< production_table.rows.length; i++){
			...
			if( building == "easy_farmer"){
				aElem.href="http://en70.tribalwars.net/game.php?village="+vid+"&screen=map&mycmd=easy_farmer";
				...
				aElem.href="http://en70.tribalwars.net/game.php?village="+vid+"&screen=market&mode=own_offer";
			}else{
				aElem.href="http://en70.tribalwars.net/game.php?village="+vid+"&screen="+building;
			}
	...
	jQuery("#get_merchant_info").click(function(){
		...
		if( village_ids){
			...
			window.setTimeout( function(){window.location=  "http://en70.tribalwars.net/game.php?village="+id+"&screen=market";}, 5000);
	...
	jQuery("#button_academy_x").click(function(){
		...
		for( v in vs){
			var w= window.open("http://en70.tribalwars.net/game.php?village="+vs[v]+"&screen=snob", vs[v]+"_academy");
		} 
	});
	...
	jQuery("#button_transfer").click(function(){
		...
		if( wood >0 || stone >0 || iron >0){
			var w= window.open("http://en70.tribalwars.net/game.php?village="+from_village+"&screen=market", "auto_transfer_"+to_village+"_"+wood+"|"+stone+"|"+iron);
		}
...
if( "market"== screen){
	if( window.name.indexOf("auto_transfer") >= 0){
		...
		if( h3.indexOf("Incoming transports") <0) {
			window.setTimeout( function(){window.location= "http://en70.tribalwars.net/game.php?village="+myid+"&screen=main"; }, 10000);
		}else{
			var get_resource = sessionStorage.getItem("get_resource");
			var find_link= false;
	...
	if( sessionStorage.getItem("get_merchant_info")){
		window.setTimeout( function(){
			...
			if( id){
				...
				window.setTimeout( function(){window.location=  "http://en70.tribalwars.net/game.php?screen=overview_villages";}, 5000);
			}
		}, 5000);
	}else{
		//get_merchant_info();
