// ==UserScript==
// @name           Cutin.de | ExpandeURL - Erweitern Sie kurze Links mit dem LongURL Service
// @description    Zeigt Ihnen die Original-URL an, wenn Sie mit der Maus drüber gehen.
// @include        http*
// @date           20120801
// @version        3.0
// @author         Cutin.de - German Localized | Script Writer - Antonio Rguez.
...
(function() {
	...
	modifyShortLinks = function() {
		...
		for (var x = 0; x < links.snapshotLength; x++) {            		 
			...
			var domain = href.match(/^https?:\/\/?(.+\.co|.+\.ly|.+\.to|.+\.in|.+\.me|.+\.de|.+\.as|.+\.cc|.+\.gl|.+\.mp|.+\.ws|.+\.gd|.+\.us|.+\.it)\//i);				
	...
	expandLink = function(anchor, e) {
		...
		tooltip('<img src="http://i.cutin.de/laden.gif" style="margin:4px 0 0 10px;float:right" alt="Wird geladen"><strong>Wird expandiert ...</strong>', e);
			...
			ajaxRequest({
				...
				onload: function(response) {
					...
					if (typeof(data.messages) !== 'undefined') { // There was an error
					    ...
					    result += ' <a href="http://longurl.org/expand?url='+encodeURIComponent(anchor.href)+'&amp;src=lme_gm" title="Mehr Information" style="color:#E9FE94;">[info]</a></div>';
