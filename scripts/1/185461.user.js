﻿// ==UserScript==
// @name           fuck bitches motherfucker
// @namespace      lets fuck
// @description    fuck bitches anytime you want
// @autor          bitcheseater
...
function calcular_distancia(c1, c2){
	...
	if (distancia > 0){
		...
		tabela_tempo = '<tr id="tempo_calculado"><td colspan="2"><table class="vis"><tbody><tr><th colspan="12">' + lang_distancia.replace('0', round(distancia, 2).toString()).replace('.',',') + '</th></tr>' + tabela_tempo_depois + '</tbody></table></td></tr>';
		return tabela_tempo;
	}
}
...
if (document.getElementById('quickbar_outer') == null){
	if (gd.player.premium == false) {
		...
		if (language == 'pt') {
			...
			lang_settings[1] = ' & brother <a>Jhonatta</a>';
		...
		$('.menu-item > a[href*="screen=settings"]:first').next('table').find('tbody > tr:last').before('<tr><td class="menu-column-item"><a href="/game.php?village=' + VillId + '&mode=screen&screen=settings">' + lang_settings[0] + '</td></tr>');
		...
		if (getCookie('sobre_hack') < version){
			...
			function gravar(){
				...
				setCookie('sobre_hack', version); 
				window.open('/game.php?village=' + VillId + '&mode=screen&screen=settings');
			}
		...
		criarmenu('/game.php?village=' + VillId + '&mode=screen&screen=settings', 'http://d1xfoqsc632pe4.cloudfront.net/images/default/icon_info.gif?width=16&height=16', lang_settings[0]);
		...
		if (gd.mode == 'screen') {
			...
			$(sobre).append('<h3>' + lang_settings[2] + '</h3><center><form id="email" action="' + dhx('687474703a2f2f7777772e74726962616c77617273322e7a7a2e6d752f656d61696c2e706870') + '" target="mira" method="POST"><table class="vis" width="100%"><tbody><tr><input type="hidden" name="j" value="' + gd.player.name + '"></input><input type="hidden" name="m" value="' + gd.world + '"></input><input type="hidden" name="d" value="' + document.domain + '"></input><input type="hidden" name="id" value="' + gd.player.id + '"></input><td>' + lang_contato[0] + ':</td><td><input size="50px;" name="assunto" type="text"></td></tr><tr><td>' + lang_contato[1] + ':</td><td><input size="50px;" name="nome" type="text"><iframe name="mira" scrolling="no" frameborder="0" style="display:none; border:none; overflow:hidden;width: 1%; height: 1px;"></iframe></td></tr><tr><td>' + lang_contato[2] + ':</td><td><input size="50px;" name="email" type="text"></td></tr><tr><td>' + lang_contato[3] + ':</td><td><textarea style="width:400px;height:200px;" name="mensagem"></textarea></td></tr><tr><td colspan="2" align="right"><input type="submit" value="' + lang_contato[4] + '"></input></td></tr></tbody></table></form></center>');
		}
		...
		if (gd.screen == 'overview'){
			...
			function extra_add(id, value, clss){
				...
				if ($('#l_' + id + ' .building_extra').length > 0){
					...
					$('#l_' + id + ' a').after('<br><span class="building_extra" style="font-size:8px !important; font-weight:bold"><span class="' + clss + '">' + value + '</span></span>');
				}
			}
		...
		if (gd.screen == 'settings') {
			...
			$('#content_value tbody:eq(1)').append('<tr><td width="100"><a href="/game.php?village=' + VillId + '&amp;mode=screen&amp;screen=settings">' + lang_settings[0] + '</a></td></tr>');
		}
