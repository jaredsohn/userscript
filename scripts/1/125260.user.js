...
function iamafilter(){
	...
	if( reddit.post_site == 'redditoroftheday' ){
		...
		$('.thing a.author[href$="'+OP+'"]').css({backgroundColor:'#5F99CF',color:'white',padding:'0 2px','-moz-border-radius':'3px','border-radius':'3px'});
	}
	...
	// Add button actions
	$('.tabmenu li[class|=qad] a').click( function(){ $('div.thing').show(); $('.tabmenu li[class|=qad]').removeClass('selected');$(this).parent().addClass('selected') });
	$('.tabmenu li.qad-q a').click( function(){ $('div.nestedlisting div.thing div.thing').hide() });
	$('.tabmenu li.qad-a a').click( function(){ $('div.nestedlisting div.thing').not('div:has(a.author:contains("'+OP+'"))').hide() });
};

// Add script to main page scope
document.addEventListener('DOMContentLoaded',function(e){var s=document.createElement('script');s.textContent="("+iamafilter.toString()+')()';document.head.appendChild(s)});
