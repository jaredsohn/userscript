 ...
 ********************************************************************************/
// Script information
var Version = '20110918b_ALPHA'; // Year, Month, Day, Revision, Maturity (e.g. YYYMMDDa_BETA)
...
var Seed = {
	...
	fetchSeed : function (notify) {
		...
		new MyAjaxRequest ('player.json', {'user%5Fid':C.attrs.userId, 'dragon%5Fheart':C.attrs.dragonHeart, '%5Fsession%5Fid':C.attrs.sessionId, version:getVersion, timestamp:parseInt(serverTime()) }, function (rslt) {
			...
			console.log('fetchSeed complete');
			if (notify)
				notify(rslt);
		});
	},
	...
	updateCity : function (city) {
		if (Data.options.verboseLog.enabled)
			actionLog('Attempting updateCity ' + city.id);
		...
		//t.updateNotifyQueue = []; 
		if (Data.options.verboseLog.enabled)
			actionLog('Successful updateCity ' + city.id);
	},
