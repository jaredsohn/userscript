// ==UserScript==
// @name           eRepublik Japanese Translation
// @namespace      Japanese Translation
// @author         BillWilson, blackca ,yukaonly
// @description    eRepublik 日本語バージョン
// @version        0.3.2.1013j
...
//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	// header
//	$("input#search_field").attr("value","市民を検索");
	// common catalog
	$('#menu ul li#menu2 a').css('background-image','url(\"https://lh6.googleusercontent.com/-E2wrlhHYh-s/TnNT2U9JBXI/AAAAAAAABQM/DfDILikdfd4/s0/map-erepublik-logged.new_.png\")');
	$('#menu ul li#menu3 a').css('background-image','url(\"https://lh6.googleusercontent.com/-E2wrlhHYh-s/TnNT2U9JBXI/AAAAAAAABQM/DfDILikdfd4/s0/map-erepublik-logged.new_.png\")');
	$('#menu ul li#menu4 a').css('background-image','url(\"https://lh6.googleusercontent.com/-E2wrlhHYh-s/TnNT2U9JBXI/AAAAAAAABQM/DfDILikdfd4/s0/map-erepublik-logged.new_.png\")');
	$('#menu ul li#menu5 a').css('background-image','url(\"https://lh6.googleusercontent.com/-E2wrlhHYh-s/TnNT2U9JBXI/AAAAAAAABQM/DfDILikdfd4/s0/map-erepublik-logged.new_.png\")');
	...
	$("#menu > ul > li#menu4 > ul > li > a:contains('Monetary')").text('通貨市場');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Job')").text('労働市場');
	$("#menu > ul > li#menu4 > ul > li > a:contains('Companies for sale')").text('會社オークション');
	$("#menu > ul > li#menu5 > ul > li > a:contains('World Map')").text('世界地図');
	$("#menu > ul > li#menu5 > ul > li > a:contains('My Party')").text('政党');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Elections')").text('選挙センター');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Military Unit')").text('軍団');
//	$('#menu > ul > li#menu5 > ul > li:eq(5) > a').text('国会議員センター');
//	$('#menu > ul > li#menu5 > ul > li:eq(6) > a').text('ランキング');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Invite friends')").text('友達を招待');
	$("#menu > ul > li#menu5 > ul > li > a:contains('Badges')").text('ロゴス');
	$('#menu > ul > li#menu5 > ul').prepend('<li><a href=\"http://www.erepublik.com/en/news/latest/all\" target="_self">新聞</a></li>');
	...
	$('#experienceTooltip').html('<img src=\"http://www.erepublik.com/images/modules/sidebar/yellow_arrow_tip.png\" class=\"tip\" alt=\"\" />'+
		'経験等級: <strong>LV. '+Explevel+'</strong><br />経験値: <strong>'+Exppoint+'</strong><br />レベルアップに必要: <strong>'+Nextlevel+'</strong><br />');
	$('#eatFoodTooltip > p:eq(0)').text('こちらは、倉庫から食事を出し、食べることによって、できるだけ健康値を回復します。');
	$('#eatFoodTooltip > p:eq(1)').html('回復できる健康値の量 <big>'+healLimit+'</big>');
	$('#eatFoodTooltip > p:eq(1) > big').attr('class','tooltip_health_limit');
	$('#eatFoodTooltip > small').replaceText('100 more recoverable Health in','次の100健康値は回復できるまで残り時間')
	$('#sidebar_missions > .content > b:first').text('クエスト');
	$('strong#foodText').text('食事をとる');
	// others
	$('a.logout').text('ログアウト');
	$('a.prev').text('前へ');
	$('a.next').text('次へ');
	// footer
	$('#footer > p > a:eq(0)').text('フォーラム');
	$('#footer > p > a:eq(1)').text('wiki');
	$('#footer > p > a:eq(2)').text('ブログ');
	$('#footer > p > a:eq(3)').text('公式ブログ');
	$('#footer > p > a:eq(4)').text('GMの郵便受け');
	$('#footer > p > a:eq(5)').text('-');
	$('#footer > p > a:eq(6)').text('サービスの条件');
	$('#footer > p > a:eq(7)').text('プライバシーポリシー');
	$('#footer > p > a:eq(8)').text('合作夥伴');
	$('#footer > p > a:eq(9)').text('ゲームのルール');
	// remove kosovo ad
...
if (host==='www') {
	...
	if ((document.location.toString()=="http://www.erepublik.com/"+lang) || (document.location.toString().indexOf("?viewPost=")!==-1)) {
		...
		// daily order
		$('#orderContainer > h1').text('軍団クエスト');
		$('#orderContainer > div > strong').text($('#orderContainer > div > strong').text().replace('Defeat enemies from','ノックアウト')+' 戦場で敵兵士を');
		$('#orderContainer > div > a.blue_beauty').text('戦場へ');
		$('#orderContainer > div > a.green_beauty').text('受賞');
		// daily task
		$('.column > .boxes.daily_tasks > h1').text('毎日クエスト');
		$('.column > .boxes.daily_tasks > div > strong').text('毎日の働く與訓練');
		$('.column > .boxes.daily_tasks > div > a.blue_beauty').text('僕の土地');
		$('.column > .boxes.daily_tasks > div > a.green_beauty').text('受賞');
		// daily task pop-box
		$('#daily_pop > h3').text('毎日クエスト');
		$('#daily_pop > h2').text('見事達成!');
		$('#daily_pop small:eq(0)').text('力');
		$('#daily_pop small:eq(1)').text('経験値');
		$('#daily_pop > a').text('はい');
		// battle list
		$('#homepage_feed .column:eq(0) > h1.noborder').text('バトル一覧');
		$("#battle_listing > h4:exact('Campaign of the day')").text('今日のバトル');
		$("#battle_listing > h4:contains('Allies')").text('今 盟友 のバトル');
		$("#battle_listing > h4:contains('Campaigns')").text('現在 '+$("#battle_listing > h4:contains('Campaigns')").replaceText('Campaigns','のバトル').text());
		$("#battle_listing span:exact('Fight')").text('戦う');
		$('#battle_listing .rest > a:eq(0)').text('バトル一覧');
		$('#battle_listing .rest > a:eq(1)').text('最新ニュース');
		$('#battle_listing > ul.resistance_war').find('a#fundRW_btn > span').text('援助する');
		$('#battle_listing > ul.resistance_war').find('.info > small').text('に反乱する');
		// citizen feed
		$('#homepage_feed .column:eq(1) > h1:first').text('ウォール');
		$('#citizen_feed form#wall_post > textarea#shout').text('何が書く');
		$('#citizen_feed a.report').text('告発する');
		$('#citizen_feed a.post_button > b').text('投稿');
		$('#citizen_feed a.previous_comments').text('前のコメントを見る');
		$("#citizen_feed a[trigger='reply']").text('投稿');
		$("#citizen_feed a[trigger='post_like']").text('いいね！');
		$("#citizen_feed a[trigger='post_unlike']").text('いいね！を取り消す');
//		$("#citizen_feed a[trigger='comment_like']").text('いいね！');
//		$("#citizen_feed a[trigger='comment_unlike']").text('いいね！を取り消す');
		$("#citizen_feed span[trigger='add_comment']").text('コメントする');
		$('#citizen_feed textarea.comment_reply_post').text('コメントする');
		...
		$('#citizen_feed').find("div.vote_bar em:contains('voted this.')").each( function() {
			...
			$(this).replaceText('and','和').replaceText('voted this.','がいいね！と言っています。');});
		$('#citizen_feed > .previous_posts > a > span').text('前のコメントを見る');
		// news list
		$('#news.box > .title > h1:first').text('新聞の分類');
		$('#articles > div > a.mbutton:eq(0) > span').text('新人指導、教育');
		$('#articles > div > a.mbutton:eq(1) > span').text('軍命令');
		$('#articles > div > a.mbutton:eq(2) > span').text('( ﾟдﾟ)＜生存戦略ぅーーー！');
		$('#articles > div > a.mbutton:eq(3) > span').text('政治評論');
		$('#articles > div > a.mbutton:eq(4) > span').text('経済の焦点');
		$('#articles > div > a.mbutton:eq(5) > span').text('社交、娯楽');
		$('#articles > div > a.mbutton:eq(6) > span').text('サブスクリプション');
	};
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	if ((document.location.toString().indexOf("/citizen")!==-1) || (document.location.toString().indexOf("/economy/donate-")!==-1)) {
		...
		// layout menu
		$('.citizen_profile_header.auth > .citizen_edit > a > span').html('<img src="http://www.erepublik.com/images/modules/citizenprofile/edit.png">編集');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(0) > a').text('概要');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(1) > a').text('口座');
		$('.citizen_profile_header.auth > .citizen_menu > li:eq(2) > a').text('倉庫');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Dead citizen')").replaceText('Dead citizen','死んだ');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Temporary banned')").replaceText('Temporary banned','一時垢バン');
		$(".citizen_profile_header.auth > .citizen_state > div.is > span:contains('Permanent banned')").replaceText('Permanent banned','永久垢バン');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_friend.tip').attr('title','友達を招待');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_friend_remove.tip').attr('title','友達を削除');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_message.tip').attr('title','メール');
		$('.citizen_profile_header.auth > .citizen_actions > a.action_donate.tip').attr('title','寄付する');
		$('.citizen_profile_header.auth > .citizen_status > div > small').text('経験値');
		// profile sidebar
		$('.citizen_sidebar > div > small:eq(0)').html('所在地 <a href="http://www.erepublik.com/en/citizen/change-residence" title="">(移動)</a>');
		$('.citizen_sidebar > div > small:eq(1)').html('国籍 <a href="http://www.erepublik.com/en/citizenship" title="">(変更)</a>');
		$('.citizen_sidebar > div > .citizen_second > small:eq(0)').text('eRepublik 誕生日');
		$('.citizen_sidebar > div > .citizen_second > small:eq(1) > a').replaceText('National rank','国に経験等級のランキング');
		$('.citizen_sidebar > .citizen_activity > div.place:eq(0) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_political_activity.png">政党に入れない');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Party Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/party_member.png">党員');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Party President')").html('<img src="http://www.erepublik.com/images/modules/_icons/party_president.png">党首');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Congress Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/congress_member.png">国会議員');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > h3:contains('Country President')").html('<img src="http://www.erepublik.com/images/modules/_icons/country_president.png">大統領');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Far-left Wing')").text('極左政党');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center-left Wing')").text('中道左派政党');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center Wing')").text('中道政党');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Center-right Wing')").text('中道右派政党');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(0) > div > span > small:contains('Far-right Wing')").text('極右政党');
		$('.citizen_sidebar > .citizen_activity > div.place:eq(1) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_mu.png">軍団に入れない');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Recruit')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">軍団新人');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Member')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">軍団団員');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Captain')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">軍団隊長');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('2nd Commander')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">軍団副司令官');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(1) > h3:contains('Commander')").html('<img src="http://www.erepublik.com/images/modules/_icons/mu.png">軍団司令官');
		if ($('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity > a').length==1) {
			$('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_media_activity.png"><a href="http://www.erepublik.com/en/create-newspaper">新聞を創刊する</a>');
		} else {
			$('.citizen_sidebar > .citizen_activity > div.place:eq(2) > h3.noactivity').html('<img src="http://www.erepublik.com/images/modules/_icons/no_media_activity.png">新聞を創刊する');};
		$(".citizen_sidebar > .citizen_activity > div.place:eq(2) > h3:contains('Press director')").html('<img src="http://www.erepublik.com/images/modules/_icons/press_director.png">新聞社社長');
		$(".citizen_sidebar > .citizen_activity > div.place:eq(2) > h3:contains('Create newspaper')").html('<img src="http://www.erepublik.com/images/modules/_icons/press_director.png"><a href="/en/create-newspaper">新聞を創刊する</a>');
		$('.citizen_sidebar > .citizen_activity > h4.friends_title').replaceText('Friends','友達 ');
		$('.citizen_sidebar > .citizen_activity > a.view_friends').text('全部見る');
		// profile content
		var cizName = $(".citizen_profile_header > h2").text();
		$('.citizen_content > .my_land_profile > p > strong').text(cizName+' の産業:');
		$('.citizen_content > .my_land_profile > p').replaceText('buildings','建築物');
		$('.citizen_content > .my_land_profile > a > span').text("行く");
		$('.citizen_content > h3.mbot2:eq(0)').html('<br><br><br><br><br>自己紹介');
		$('.citizen_content > h3.mbot2:eq(1)').html('<br><br><br><br><br>成果の勲章');
		// achievment
		$("ul#achievment > li:eq(0) > .hinter > span > p:eq(0) > strong").text("模範的な社員");
		$("ul#achievment > li:eq(0) > .hinter > span > p:eq(1)").text("連續工作30天");
		$("ul#achievment > li:eq(1) > .hinter > span > p:eq(0) > strong").text("国会議員");
		$("ul#achievment > li:eq(1) > .hinter > span > p:eq(1)").text("国会議員選挙に勝つ");
		$("ul#achievment > li:eq(2) > .hinter > span > p:eq(0) > strong").text("大統領");
		$("ul#achievment > li:eq(2) > .hinter > span > p:eq(1)").text("大統領選挙に勝つ");
		...
		$("ul#achievment > li:eq(5) > .hinter > span > p:eq(0) > strong").text("バトル英雄");
		$("ul#achievment > li:eq(5) > .hinter > span > p:eq(1)").text("在一場バトル中達到最高的戰爭影響");
		...
		$('.citizen_content > .citizen_military:eq(0) > strong:first').text('力');
		$('.citizen_content > .citizen_military:eq(0) > div > small:first').replaceText('Super soldier:','超級大兵:');
		$('.citizen_content > .citizen_military:eq(1) > strong:first').text('軍隊階級');
		$('.citizen_content > .citizen_military:eq(1) > div > small:first').text('軍功ポイント:');
		if (document.location.toString().indexOf("/economy/")!==-1) {
			if (document.location.toString().indexOf("/donate-")!==-1) {
				// donate tab
				$('.citizen_content > div > h2.special.padded').replaceText('Donate','寄付');
				$('.citizen_content > div > h2.special.padded > img#donate_tip').attr('title','只有 食物, 武器 和 機票 能被捐贈.');
				$('.citizen_content > div > ul.tabs > li#topratedtab > a > span').text('物品');
				$('.citizen_content > div > ul.tabs > li#latesttab > a > span').text('通貨');
				$(".citizen_content > div > .donate_form > table > tbody > tr > th:contains('Your storage')").text('君の倉庫');
				$(".citizen_content > div > table > tbody > tr > th:contains('Your account')").text('君の口座');
				$(".citizen_content input[value='Donate']").attr('value','寄付');
//			} else if (document.location.toString().indexOf("/citizen-accounts/")!==-1) {
			} else {
				// account money tab
				$(".citizen_content > a.fluid_blue_light_small > span:contains('Exchange currencies')").text('両替する');
				...
				$('div#allaccounts > div:first > a.f_light_blue_small > span').text('金を買う');
			};
		} else if (document.location.toString().indexOf("/citizen-friends/")!==-1) {
			// friend tab
			$("table.warning_message > tbody > tr > td:contains('Only your first 2000 friends will see your wall posts.')").text('只有前 2000 位友達能看到你發佈在公民塗鴉牆上的訊息.');
			$("table.success_message > tbody > tr > td:contains('Your friendship request has been sent.')").text('友達邀請已經送出.');
			$('div#friends_tab_content div.dead').text('死んだ');
			...
			$('.citizen_content > h2.special').text('位置を移動する');
			$('.citizen_content > .current_location > h4').text('現在の位置');
			$('.citizen_content > form > .new__location > h4').text('新い位置');
			$('.citizen_content > form > .new__location > #selects > small:eq(0)').replaceText('Moving distance:','　距離:　');
			$('.citizen_content > form > .new__location > #selects > small:eq(1)').replaceText('Travelling cost:','　料金:　');
			$('.citizen_content > form > a#move > span').text('移動する');
		} else if (document.location.toString().indexOf("/change-password")!==-1) {
			// change password tab
			$('.citizen_sidebar > div > .citizen_second > small:eq(2)').replaceText('Forfeit points','違規點數');
			$('.citizen_content > .holder > h2.special.borderedsep').text('パスワードを変更');
			$("form.changepassword span.fieldname.goleft:contains('Current password')").text('現在のパスワード');
			$('form.changepassword span#error_for_citizen_password_twin.twin-small').text('現在のパスワードを再入力.')
			$("form.changepassword span.fieldname.goleft:exact('New password')").text('新いパスワード');
			$("form.changepassword span.fieldname.goleft:exact('New password again')").text('新いパスワードを再入力');
			$('form.changepassword > div > input.arrowbutton').attr('value','保存');
		} else if (document.location.toString().indexOf("/edit/profile")!==-1) {
			// editor tab
			$('.citizen_content > .holder > h2.special.borderedsep').text('個人情報設定');
			$("form.editprofile span.fieldname.goleft:contains('Your description')").text('自己紹介');
			$("form.editprofile span.fieldname.goleft:contains('Citizen Avatar')").text('プロフィール画像');
			$('form.editprofile span#error_for_citizen_file_twin.twin-small').html('対応フォーマットは <strong>.jpeg</strong> だ.');
			$("form.editprofile span.fieldname.goleft:contains('Your birthday')").text('誕生日');
			...
			$("form.editprofile span.fieldname.goleft:contains('Your password')").text('パスワード');
			$('form.editprofile span#error_for_password_twin.twin-small').text('如果確定更改個人資料, 請輸入你目前的密碼.');
			$('form.editprofile > .largepadded > input.arrowbutton').attr('value','保存');
			$('.citizen_content > div > a.dotted.change_password').text('パスワードを変更');
		};
	};
	if (document.location.toString().indexOf("/citizenship")!==-1) {
		...
		$("table.info_message > tbody > tr > td:contains('You are not be able to request a new citizenship while being a party member')").text('由於你目前是政党成員, 所以不能提出國藉申請.');
	};
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	if (document.location.toString().indexOf("/exchange")!==-1) {
		$('h1').css('font-family','Arial,Arial');
		$("h1:contains('Monetary Market')").text('通貨市場');
	//	$("a#buy_selector > span.selecttags").text('買う');
	//	$("a#sell_selector > span.selecttags").text('売る');
	//	$('table#table_list_offers > tbody > tr:first > th:eq(0)').text('提供者');
	//	$('table#table_list_offers > tbody > tr:first > th:eq(1)').text('量');
	//	$('table#table_list_offers > tbody > tr:first > th:eq(2)').text('為替レート');
	//	$('table#table_list_offers > tbody > tr:first > th:eq(3)').text('購入金額');
	};
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	if (document.location.toString().indexOf("/economy/inventory")!==-1) {
		...
		// storage
		$('#inventory_overview > h2:first > span').text('個人的倉庫');
		$('#inventory_overview > h2:first > img.storage_tooltip').attr('title','如果想增加你的個人倉庫容量，請擴建土地，並建造額外的倉庫建築。');
		$('#inventory_overview > .items_holder > h4:eq(0)').text('製品');
		$('#inventory_overview > .items_holder > h4:eq(1)').text('原料');
		$('#inventory_overview > .items_holder > h4:eq(2)').text('コレクション');
		$(".collection_list > ul > li:[title='Barrel']").attr('title','銃砲身');
		$(".collection_list > ul > li:[title='Scope']").attr('title','照準器');
		$(".collection_list > ul > li:[title='M6A3 Rocket']").attr('title','M6A3ロケット弾');
		$(".collection_list > ul > li:[title='Trigger Kit']").attr('title','引き金');
		$(".collection_list > ul > li:[title='Stock']").attr('title','銃床');
		$(".collection_list > ul > li:[title='Hammer']").attr('title','槌');
		$(".collection_list > ul > li:[title='Pliers']").attr('title','鉗子');
		$(".collection_list > ul > li:[title='Saw']").attr('title','電ノコ');
		$(".collection_list > ul > li:[title='Shovel']").attr('title','シャベル');
		$(".collection_list > ul > li:[title='Tape Measure']").attr('title','巻尺');
		$('.collection_list > a.assemble').text('組み立てる');
		$('.collection_list > a.assemble').attr('title','組み立てる');
		$('.collection_list > .bazooka > .details > strong').html('バズーカ'+
		...
		$('.collection_list > .bazooka > .details > small').text('一撃必殺');
		$('.collection_list > .bazooka > .details > p:eq(0)').attr('title','耐久度');
		$('.collection_list > .bazooka > .details > p:eq(1)').attr('title','火力');
		$('.collection_list > .bazooka > .details > p:eq(1) > span').text('10000 / 発');
		$('.collection_list > .toolbox > .details > strong').html('ツールボックス'+
		...
		// control module
		$('#inventory_overview > a.inventory_sell > strong').text('売る');
		$('#inventory_overview > a.inventory_sell > small').text('將物品放到市場販售');
		$('#inventory_overview > a.inventory_buy > strong').text('買う');
		$('#inventory_overview > a.inventory_buy > small').text('從市場購買東西');
		$('#sell_offers th.offers_product > strong').text('製品');
		$('#sell_offers th.offers_quantity > strong').text('量');
		$('#sell_offers th.offers_price > .relative > strong').text('毎に1つ');
		$('#sell_offers th.offers_market > .relative > strong').text('市場');
		$('#sell_offers th.offers_market > .relative > small > a#buy_license').text('交易許可を買う');
		$('#sell_offers th.offers_action > a > span').text('確定販售');
		$('#sell_offers a.delete_offer').attr('title','破棄する');
		$('.buy_market_shell > a#buy_market_license > span').text('交易許可を買う');
		$('.buy_market_shell > a#select_buy_license_country > span#buy_license_country_name').text('国を選択する');	
	};
	//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	if (document.location.toString().indexOf("/wars/")!==-1) {
		...
		// war
		$('h1:first').text('戦争');
		$('#war_type_filter > div > .core > div > h2.goleft.big:first').text('戦争の形式を選択');
		$('#war_type_filter > div > .core > ul:eq(0) > li > a').html('<img src="/images/parts/icon_warlist_war.gif" class="flag" alt="">会戦');
		$('#war_type_filter > div > .core > ul:eq(1) > li > a').html('<img src="/images/parts/icon_warlist_resistancewar.gif" class="flag" alt="">反乱');
		$('#war_type_filter > div > .core > ul:eq(2) > li > a').html('<img src="/images/parts/icon_warlist_allwars.gif" class="flag" alt="">全部');
		$('#war_status_filter > div > .core > div > h2.goleft.big:first').text('戦争状態を選択');
		$('#war_status_filter > div > .core > ul:eq(0) > li > a').html('<img src="/images/parts/icon_warlist_active.gif" class="flag" alt="">進行中');
		$('#war_status_filter > div > .core > ul:eq(1) > li > a').html('<img src="/images/parts/icon_warlist_inactive.gif" class="flag" alt="">終了');
		$('#war_status_filter > div > .core > ul:eq(2) > li > a').html('<img src="/images/parts/icon_warlist_allstates.gif" class="flag" alt="">全部');
		$('#war_country_filter > div > .core > div > h2.goleft.big:first').text('戦争国を選択');
		$.each( $(".warholder > .attacker > .nameholder:contains('Resistance Force')"), function() {
			$(this).html($(this).replaceText('Resistance Force of ','').text()+'<span id="CHT">反乱軍</span>');});
		$('.nameholder > #CHT').css('font-family','Arial,Arial');
		$('.warholder > .attacker > a').replaceText('allies','盟友').replaceText('no ','なし');;
		$('.warholder > .defender > a').replaceText('allies','盟友').replaceText('no ','なし');;
		$('.warholder > .middle > a.details').text('詳細');
		if (document.location.toString().indexOf("/wars/show/")!==-1) {
			...
			$(".war_list_header > .country.left_side > div > h3:contains('Resistance Force')").html(rCoun+' <span id="CHT">反乱軍</span>');
			$('.war_list_header > .country.left_side > div > h3 > #CHT').css('font-family','Arial,Arial');
			$('.war_list_header > .country > div > a').replaceText('allies','盟友').replaceText('no ','なし');
			$('.war_list_header > .vs > small').replaceText('Still active','戦争中');
			$('.listing > a.reversed > span').replaceText('Join','参加する');
			$(".listing > a.join[title='Join Resistance'] > span").text('反乱軍を参加する');
			$(".listing > a.join[title='Join'] > span").text('参加する');
			$('.listing > small').replaceText('started on','目前戰場開啟自: ');
			$(".listing.done > small:contains('Conquered by')").replaceText('Conquered by','占領する:');
	...
	if (document.location.toString().indexOf("/military/")!==-1) {
		if (document.location.toString().indexOf("/military/battlefield/")!==-1) {
			$('#pvp_header > .battle_hero.left_side').attr('title','僕たちの戦場ヒーロー');
			$('#pvp_header > .domination > .domibar > #left_points').attr('title','国ポイント');
			$('#pvp_header > .domination > .domibar > #right_points').attr('title','敵の国ポイント');
			$('#pvp_header > .battle_hero.right_side').attr('title','敵の戦場ヒーロー');
			...
			$('#pvp_header > .country > div > a').replaceText('allies','盟友').replaceText('no ','ない');
			$('#pvp_battle_area > table.damage_aligner > tbody > tr > td > #total_damage').attr('title','僕の累積的な影響.');
			$('#pvp_battle_area > table.damage_aligner > tbody > tr > td > #total_damage > small').text('僕の戦場影響');
			$('#pvp_battle_area > .player.left_side > a#change_weapon').attr('title','武器を切り替える');
//			$('#pvp_actions > .action_holder > a#heal_btn').attr('title','パンを食べる');
			$('#pvp_actions > .action_holder > a#fight_btn').text('攻撃する');
			$('#pvp_actions > .action_holder > a#fight_btn').attr('title','攻撃する!');
			$('#pvp > a#go_stats.battle_stats_button').attr('title','統計');
			$('#pvp > a.help_button').attr('title','ヘルプ');
			$('.battle_stats > .top > h3:first').text('バトルの統計');
			$('.battle_stats > .repeat > .content > .lister > .head > .one').text('市民');
			$('.battle_stats > .repeat > .content > .lister > .head > .two').text('擊倒數');
			$('.battle_stats > .repeat > .content > .lister > .head > .three').text('戦場影響');
			// defeated-pop-box
			$('#enemy_defeated > .heading > h2').text('擊倒敵人');
			$('#enemy_defeated > .content > div:eq(0) > strong').text('戦場影響');
			$('#enemy_defeated > .content > div#natural_enemy_bonus > strong').text('朝敵戦場ボーナス');
			$('#enemy_defeated > .content > div:eq(2) > strong').text('軍階點數');
			$('#enemy_defeated > .content > div:eq(3) > strong').text('経験値');
			...
			$('#battle_end > a.green_go').text('查看其它バトル');
			...
			$('h1:first').text('バトル列表');
			$("#battle_listing > h4:exact('Campaign of the day')").text('本日バトル');
			$("#battle_listing > h4:contains('Campaigns')").text('目前 '+$("#battle_listing > h4:contains('Campaigns')").replaceText('Campaigns','的バトル').text());
			$("#battle_listing > p.campaigns_header.allies.toggled:contains('Allies')").html('<span class="arrow"></span>目前 同盟國 的バトル');
			$("#battle_listing > p.campaigns_header.allbattles.toggled:contains('All')").html('<span class="arrow"></span>目前 全部國家 的バトル');
	...
	if (document.location.toString().indexOf("/news/")!==-1) {
		...
		// newspaper, article
		$('body#newspaper > #container > #content > h2.new_spaper_section').text('關於本報');
	...
	if (document.location.toString().indexOf("/party/")!==-1) {
		$('h2').css('font-family','Arial,Arial');
		// party
		$("h2.section:contains('Info')").text('政党資訊');
		$(".infoholder > .largepadded > span.field:exact('Members')").text('成員');
		$(".infoholder > .largepadded > span.field:exact('Orientation')").text('政党傾向');
		...
		$('.indent > .bordersep:eq(2) > .subindent > #candidate_for_congress_template > .holder').replaceText('Do you agree to represent your party in the congress election in','請問你是否同意, 代表你的政党參選').replaceText('?',' 區的議員?');
		...
		$(".indent > .bordersep > p.smallholder:contains('Party Member')").replaceText('Party Member','政党成員');
		...
		$('.holder > a.btn-arrow-left.goright').text('政党頁面');
		$('a.report.party').text('檢舉政党');
		...
		$('.infoalert > p:eq(0)').html('<strong>1.</strong> 政党成員每月的 16 號到 23 號間，可以報名參選國會議員.');
		$('.infoalert > p:eq(1)').html('<strong>2.</strong> 黨主席可以在每月的 24 號，調整最終議員參選名單.');
		$('.infoalert > p:eq(2)').html('<strong>3.</strong> 每個政党在每個區域，最多只能提名三名國會議員參選人.');
...
//		$(".bordersep > h2:exact('Country')").text("請選擇一個國家");
	...
	if (document.location.toString().indexOf("/elections/")!==-1) {
		...
		$("#party_filters > .extended-menus > .core > .bordersep > h2:contains('Parties')").text('請選擇一個政党');
		...
		if ((document.location.toString().indexOf("election-congress")!==-1) || (document.location.toString().indexOf("election-party")!==-1)){
			...
			$("table.electiondetails th:contains('Member of')").text('所屬政党');
	...
	if (document.location.toString().indexOf("/group-")!==-1) {
		...
		if (document.location.toString().indexOf("/group-home/")!==-1) {
			...
			$('#orderContainer > h3').replaceText('Order of the day','每日軍團クエスト');
			...
			$('#group_orders > h3:first').text('新兵クエスト');
			$('#group_orders > p:first').text('完成這些クエスト, 就能成為這個軍團的正式隊員.');
			...
			$('#group_orders > .requirements > ul.req_content > li:eq(2) > .details > strong').text('在 5 場不同的バトル中, 為國家和我們的同盟而戰');
...
//			$("#group_feeds > #citizen_feed a[target='reportpopup']").text('檢舉');
	...
	if ((document.location.toString().indexOf("/country")!==-1) || (document.location.toString().indexOf("/law/")!==-1)) {
		...
		if (document.location.toString().indexOf("/country/society/")!==-1) {
			...
			$("span.fakeheight:eq(2)").text("国籍申請");
...
//			$(".holder:eq(1) > .indent > p.largepadded:contains('Do you agree to transfer')").replaceText('Do you agree to transfer','請問你是否同意從國庫中捐贈').replaceText('from the country accounts to','到');
	...
	if (document.location.toString().indexOf("/tickets/report/")!==-1) {
		...
		$('body.reportAbuseBody > h3').replaceText('Report ','檢舉').replaceText('a newspaper','本報').replaceText('an article','本文').replaceText('a comment','留言').replaceText('a wall post','塗鴉牆留言').replaceText('a wall comment','塗鴉牆回應').replaceText('a party','政党');
...
//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/
	...
	if (document.location.toString().indexOf("/land/")!==-1) {
		...
		$("#work_results .wdetails_bar > .list.stats > div:last > div[title='Experience points']").attr('title','経験値');
		...
		$('#train_results .wdetails_bar > .list.friends_bonus > div:first > small').text('邀請友達獎勵');
		$('#train_results .wdetails_bar > .list.friends_bonus_off > div:first > small').text('邀請友達獎勵');
		...
		$("#train_results .wdetails_bar > .list.stats > div:last > div[title='Experience points']").attr('title','経験値');
