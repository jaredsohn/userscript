// ==UserScript==
// @name           Power Tools - Firefighter and Dragon
// @namespace      noobs
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include	   http://corpr8.co.uk/newalert/html
// @description    Enhancements and bug fixes for Kingdoms of Camelot. Komplett in Deutsch
// ==/UserScript==

var Version = '20110221b';
var Title = 'KOC ffd Tools';
...
// TODO: Futter surplus trend (% change over 30 minutes, etc.)
// TODO: Force KOC to update training queues, Marschierende, etc for non-current city ?  (fix 'waiting for reports', lingering tower alerts, etc)
***/
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('FF and Dragon', eventHideShow, mouseMainTab);
}
...
//    t.show ();
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('RUECKSETZEN', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Wildnisse: '+ totw +' of '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];

      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wildnis Art</td><TD></td><TD align=left>Koords</td><TD>Fallen</td><TD align=left>Eingestellt</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' AENDERE VERTEIDIGUNG ') +'</td></tr>';
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Baue Fallen:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
          ...
          m += '<TD class=ptentry> &nbsp; &nbsp; <B>Einst.</b> ' + htmlSelector(mercNames, wildDef.mercLevel, 'id=ptwm_'+ c +'_'+ i) +' &nbsp; &nbsp; </td></tr>';
        }
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Kosten: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("SETZE VERTEIDIGUNG", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Gold: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
Tabs.Ritter = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        ...
        if (knight.knightStatus == 10){
          ass = '<TD class=ptentry align=left colspan=4>Unterwegs</td>';
        } else {  
          if (unpoints > 0){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          <TR class=ptwpad style="font-weight:bold" align=right><TD width=70>Rolle</td><TD width=160 align=center>Name</td><TD width=26>Pol</td><TD width=26>Com</td>\
          <TD width=26>Int</td><TD width=26>Res</td><TD width=90 align=center colspan=5>--- nicht zugeteilt ---</td><TD width=40 align=right> Gehalt </td><TD width=35>Level</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
      ...
      m += '<TR align=right><TD colspan=11><B>Total Gehalt:</b></td><TD>'+ addCommas(totSalary) +'</td></tr>';        
    }
  ...
  clickedAssignPoints : function (e, cid, kid, rid){
    ...
    if (knight.knightStatus == 10){
      ...
      row.childNodes[7].innerHTML = 'Unterwegs';
      return; 
    }
    sk = [];
...
var TowerAlerts = {
  ...
  towerMarches : {},    // track all Marschierende that have been posted to alliance chat
  ...
  e_eachSecond : function (){   // check for incoming Marschierende
    var t = TowerAlerts;
    var now = unixTime();
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Geben Sie ein Ganzen oder einen Teil eines Spieler-Namens ein: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Finde Spieler" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab>Oder: </td><TD class=xtab> Gehen Sie ein Ganzen oder einen Teil der Alli ein: &nbsp;</td>\
          <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Finde Allianz" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptallErr></span></td></tr>\
		<TR><TD class=xtab>Oder: </td><TD class=xtab align=right> Liste Alli auf: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="Meine Allianz"/>\
            <INPUT align=left id=allListSubmit type=submit value="Zeige  alle" /></td></tr>\
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Ergebnis der Spieler suche <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Macht</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Lookup </td></tr>';
    var row=0;
    var cl='';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>leaderboard</a></span> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>onlinestatus</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>letzter Login</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Macht: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
  ...
  gotPlayerStatus : function (rslt, span,uid){
    ...
    if (p[uid] == true) {
      m = '<span style="color:green"><b>online!</b></span>';
       ...
       m = '<span style="color:red"><b>nicht online</b></span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">Letzter login: '+lastLogin+'</span>';
    } else {
       m = '<span style="color:red">kein login datum gefunden: '+lastLogin+'</span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  eventGotAllianceList : function (rslt){
    ...
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Alliance Name</td><TD class=xtab>Rank</td><TD class=xtab>Members</td>\
        <TD align=right class=xtab>Macht</td><TD class=xtab>Diplomacy</td><TD class=xtab></td></tr>';
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Zeige Member</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Zeige Member</a></td></tr>';
    }
    ...
    m += ' &nbsp; seite <INPUT align=right id=idPageNum type="text" size=4 value='+ t.curPage + ' /> von '+t.MaxPage;
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      ...
      m += '<TR style="max-height:30px"><TD class='+ tabclass +'>'+ t.dat[i][0] +'</td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
			...
			<td class='+ tabclass +'><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>letzter Login</a></span><td> </tr>';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Entfernung ist von <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' players found &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Spieler</div></a></td>\
        ...
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Staedte</a></div></td>\
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Rank</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Staedte Name</a></div></td>\
        <TD id=clickCol4 onclick="PTalClickSort(this)" class=clickable><A><DIV>Lvl</a></div></td>\
        <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Koords</a></div></td>\
		...
		<TD class=clickable><A><DIV>Letzter Login</a></div></td></tr></thead>\
    ...
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Zeige Entfernung von: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; Oder, waehlen die Stadt: <span id=dmcoords></span></td></tr></table>';
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>EINHEITSINFORMATION</div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>KOSTEN, UM ZU BAUEN</b></td><TD class=xtabHL colspan=7><B>STATS</b></td><TD class=xtabHL><B>Unterhalt</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Futter</td><TD class=xtabH>Holz</td><TD class=xtabH>Steine</td>\
        <TD class=xtabH>Erz</td><TD class=xtabH>Pop</td><TD class=xtabHL>Macht</td><TD class=xtabH>Life</td><TD class=xtabH>Atk</td><TD class=xtabH>Def</td><TD class=xtabH>Speed</td><TD class=xtabH>Range</td><TD class=xtabH>Load</td>\
    ...
    for (ui=1; ui<13; ui++){
      ...
      cost = unsafeWindow.unitcost['unt'+ui];     //  NAME, Futter, Holz, Steine, Erz, ?, IdlePop, Time
      stats = unsafeWindow.unitstats['unt'+ui];   //  Life, Attack, Defense, Speed, Range, Load
      Futter = unsafeWindow.unitupkeeps[ui];
      ...
      m += '<TR '+ rsty +'align=right><TD class=xtab align=left><B>'+ cost[0] +'</b></td><TD class=xtabL>'+ cost[1] +'</td><TD class=xtab>'+ cost[2] +'</td>\
          ...
          <TD class=xtab>'+ stats[4] +'</td><TD class=xtab>'+ stats[5] +'</td><TD class=xtabL>'+ Futter +'</td></tr>';
    ...
    for (k in unsafeWindow.fortcost){
      if (++rownum % 2)
        ...
        rsty = ' style="background: #e8e8e8" ';
      cost = unsafeWindow.fortcost[k];     //  NAME, Futter, Holz, Steine, Erz, ?, IdlePop, Time
      fi = k.substring(3);
    ...
    m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';
    m += '</table>';
    m += '<BR><DIV class=ptstat>ENTFERNUNGSRECHNER</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>Die erste Position: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> Oder, waehle Stadt: <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>Die zweite Position:: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> Oder, waehle Stadt: <SPAN id=ptloc2></span></td></tr></table>\
      ...
      <BR><BR><DIV class=ptstat>PROVINZ-KARTE</div><DIV id=ptProvMap><IMG width=680 height=654 SRC="'+ URL_PROVINCE_MAP +'"></div></center></div>';
    t.cont.innerHTML = m;
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = 'Die Entfernung von '+ x1 +','+ y1 +' to '+ x2 +','+ y2 +' ist: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Enable window drag (erlaubt das fenster mt der maus zu verschieben)</td></tr>\
        <TR><TD><INPUT id=ptEnableFutterWarn type=checkbox /></td><TD>Zeige \'Futter reicht\' in ROT wenn das futter die angegbene zeit unterschreitet \
            <INPUT id=optFutterHours type=text size=3 value="'+ Options.FutterWarnHours +'"> stunden</td></tr>\
        <TR><TD colspan=2><P><B>KofC Features:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Zeige Zahl von neuen Nachrichten getrennt von der Zahl von Berichten ueber die Nachrichtenikone.</td></tr>';
		if (ENABLE_ALERT_TO_CHAT){
	 m += '<TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Zeige automatisch drohende Angriffe im Allianz chat an.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
          <TR><TD align=right>Nachrichtenwahl: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
          <TR><TD align=right>Alarmsignal beim spaehen: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Alarmsignal beim Wildnis angriff: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Zeige angreigende Allianz: &nbsp; </td><TD><INPUT id=alertAlliance type=checkbox '+ (Options.alertConfig.includealliance?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Zeige Macht des Angreifenden: &nbsp; </td><TD><INPUT id=alertMight type=checkbox '+ (Options.alertConfig.includemight?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Alarmsignal bei drohendem Angriff: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.sound?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Minimum  der Truppen: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
          </table></td></tr>';
		  }
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Zeigt verbesserte Allianz Berichte.</td></tr>\
        <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Zeigt bessere Schriften, Format von Allianz Berichten.</td></tr>\
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Erlauben Sie erhoehte Nachrichtenuebermittlung ("vorwaerts" und "alle Offiziere" Knoepfe).</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Verbesserte Seitennavigation fuer Nachrichten und Berichte <SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Warnt Sie, falls versucht, zur Position 0,0 zu marschieren. <SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Zeigt die WEZ-Uhr neben der "Camelot Zeit" <SPAN class=boldRed>&nbsp;(NEU)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>KofC Programmfehler:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Repariert Turm Berichte und zeigt genaues Ziel (Stadt, wild oder ungueltig)</td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Repariert Karte , um Entfernungen von der zurzeit ausgewaehlten Stadt statt immer der ersten Stadt zu zeigen.</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Behebe falsche Alarmmeldungen hervorgerufen von Spaehmissionen</td></tr>\
        </table><BR><BR><HR>Bemerken Sie, dass, wenn ein checkbox Grau ist, wahrscheinlich eine Aenderung des Codes von KofC gewesen ist, die Auswahl nicht mehr moeglich machen.';
      t.cont.innerHTML = m;
      ...
      t.togOpt ('ptEnableFutterWarn', 'enableFutterWarn');
      ...
      document.getElementById('optFutterHours').addEventListener ('change', function () {
          var x = document.getElementById('optFutterHours').value; 
          if (isNaN(x) || x<0.01 || x>99999){
            document.getElementById('optFutterHours').value = Options.FutterWarnHours;
            return;
          }
          Options.FutterWarnHours = x; 
          saveOptions();
        }, false);
...
Tabs.Suchen = {
  ...
  init : function (div){
    ...
    this.cont = div;
    this.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Suchen nach: </td><TD>\
      <SELECT id="srcType">\
        <OPTION value=0>Barbaren</option>\
        <OPTION value=1>Wildnisse</option>\
		<OPTION value=2>Staedte</option>\
       </select></td></td> \
        <td> In Province:\
        ...
        <tr><td width=100  align=right >Im Quadrant: </td><td><select id="idQuadSelect">\
        <option value="188,188"> NordWest </option>\
        <option value="563,188"> NordOst </option>\
        <option value="188,563"> SuedWest </option>\
        <option value="563,563"> SuedOst </option>\
      ...
      m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Starte Suche" /></td></tr>';
      m += '</table>';
...
//End edit city Suchen
  ...
  srchTypeHandler : function (){
    ...
    var m = '<CENTER><B>Suchen nach '+ typeName +'<BR>\
    ...
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     m += '<TR><TD class=xtab align=right>Min. level zegen:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. level zeigen:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Wildnisse Type:</td><TD class=xtab align=right>\
            Wald<INPUT id=HolzWild type=CHECKBOX'+ (Options.HolzWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Grassland/See<INPUT  id=FutterWild type=CHECKBOX '+ (Options.FutterWild?' CHECKED':'') +'></td>\
	       <TD class=xtab align=right>Berg<INPUT id=mtnWild type=CHECKBOX '+ (Options.mtnWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Ebene<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD class=xtab align=right>Huegel<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Nur unbesetzte:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Sort nach:</td><TD class=xtab><SELECT id=filSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
			...
			<TR><TD class=xtab align=right>Zeige nur koordinaten</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
			</table></div><BR><SPAN id=srchSizeWarn></span>';
    } else {
        m+= '<TR><TD class=xtab align=right >Zeige:</td><TD class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
             <OPTION value=0>Alle Staedte</option>\
             <OPTION value=1>Nur Feindlich</option>\
	     <OPTION value=2>Nur Im Nebel</option>\
	     <OPTION value=3>Nur Verbuendete</option>\
	     <OPTION value=4>Nur Freundliche</option>\
	     <OPTION value=5>Nur Neutrale</option>\
	     <OPTION value=6>Nur Allilos</option>\
             </select></td></tr>';

		m+= '<TR><TD class=xtab align=right >Sort nach:</td><TD class=xtab><SELECT style="width: 135px" id=filSortBy>\
          <OPTION value="macht" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Macht</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right >Zeige nur koordinaten:</td><TD class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
    ...
    if (t.opt.searchType == 1){
    document.getElementById('FutterWild').addEventListener ('change', function(){
        Options.FutterWild = document.getElementById('FutterWild').checked;
        saveOptions();
        t.dispMapTable ();
        }, false);
    ...
    document.getElementById('HolzWild').addEventListener ('change', function(){
        Options.HolzWild = document.getElementById('HolzWild').checked;
        saveOptions();
        t.dispMapTable ();
        }, false);
  ...
  clickedSearch : function (){
    ...
    document.getElementById ('srcStart').value = 'Stope Suche';
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min. level zegen:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. level zegen:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Wildnis Type:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Glassland/See', 3:'Wald', 4:'Huegel', 5:'Berg', 6:'Ebene', 0:'ALLE'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Nur unbebsetzte:</td>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	 ...
	 m += '<TR><TD class=xtab align=right>Stadt Type:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'Alle', 2:'Allies', 3:'Freundlich', 4:'Feindlich', 5:'Neutral', 6:'Ohne Alli', 7:'Im Nebel'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Sort nach:</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="macht" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Macht</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
           </select></td></tr>\
		   <TR><TD class=xtab align=right>Zeige min Macht:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>Zeige nur koordinaten</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
    ...
    document.getElementById ('statStatus').innerHTML = 'Suchen bei '+ xxx +','+ yyy;
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Barbaren', 'Grassland', 'See', 'Wald', 'Huegel', 'Berg', 'Ebene', 'Stadt' ];
    var t = Tabs.Suchen;
...
if (DEBUG_TRACE) DebugTimer.start();
    ...
    for (i=0; i<t.mapDat.length; i++){
	  ...
	  if (t.opt.searchType == 2 && type == 7 ) {
			...
			if (!(Options.cityType == 3) || t.mapDat[i][12] == 'fruendlich') 
			if (!(Options.cityType == 4) || t.mapDat[i][12] == 'feindlich')
			if (!(Options.cityType == 5) || t.mapDat[i][12] == 'neutrale') 
			if (!(Options.cityType == 6) || t.mapDat[i][12] == 'ohne ally') 
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    if (dat.length == 0){
      m = '<BR><CENTER>nicht gefunden</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 0) // Barbs
			...
			//m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Distance</td><TD style="padding-left: 10px;">Lvl</td><TD style="padding-left: 10px;> &nbsp; Type</td><TD width=80%>besitzer</td><TD style=""></td></tr>';
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Location</td><TD style="padding-left: 10px">Distance</td><TD style="padding-left: 10px;">Lvl</td><TD width=10%> &nbsp; Type</td><TD width=70%> &nbsp; besitzer</td><TD style=""></td></tr>';
		else // Cities
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Ort</td><TD >Dist</td><TD>Stadt</td><TD>Besitzer</td><TD>Macht</td><td>Alliance               </td><TD width=80% style="font-size:9px;">Mehr Info</td><TD style="padding-left: 10px;"></td></tr>';
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Letzter Login</a></div>';
					m+= '</td><TD  valign="top">'+ (dat[i][5]?' im Nebel':'') +'</td></tr>';
				...
				if (dat[i][9]) { // Is Misted?
					...
					// neutral, freundlich, alli, feindlich
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'Starte Suche';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Gesucht: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('statStatus').innerHTML = 'Suchen bei '+ x +','+ y;
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Trainiere Truppen und baue Verteidigung der Mauer/Feldes</div><DIV style='height:10px'></div><DIV class=ptentry>\
      <DIV style='text-align:center; margin-bottom:10px;'>Waehle Stadt &nbsp; <span id=ptspeedcity></span></div>\
      ...
      <SELECT id=ptttType>\
        ...
        <option value='3'>Spaeher</option>\
        <option value='4'>Lanzentraeger</option>\
        <option value='5'>Schwertkaempfer</option>\
        <option value='6'>Bogenschuetze</option>\
      ...
      <TR><TD align=right># pro slot: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># bis slots: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
        <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
      <TR><TD align=right valign=top>Arbeiter laufen leer: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Erlaubt Ihnen, mehr Truppen zu trainieren. Kann muessige negative Bevoelkerung provisorisch setzen.</span></td></tr>\
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='Trainiere Truppen'\></td></tr>\
      </table></td><TD width=20></td><TD style='border-left:solid 2px;' width=50% align=center>\
      <TABLE align=center><TR><TD align=right>Verteidigungstyp: </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Armbrueste</option>\
        <option value='55'>Trebuchet</option>\
        <option value='60'>Falle</option>\
        <option value='61'>Kraehenfuesse</option>\
        <option value='62'>Eiserne Erdspiesse</option>\
      </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
      <TR><TD align=right># pro slot: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># bis slots: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
      ...
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Bilde Defensive'\></td></tr></table>\
      </td></tr></table></div></div>\
      ...
      <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Truppe-Warteschlange &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
        <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Verteidigungswarteschlange &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
      </div>";
    t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  changeTroopSelect : function (){
    ...
    // unitcost: NAME, Futter, Holz, Steine, Erz, Gold, Pop, ?
    var id = t.TTselType.value;
    t.lastTroopSelect = id;
    var uc = unsafeWindow.unitcost['unt'+id];
    var max = 9999999999;
    if ( (t.stats.Futter / uc[1]) < max)
      max = t.stats.Futter / uc[1];
    if ( (t.stats.Holz / uc[2]) < max)
      max = t.stats.Holz / uc[2];
    if ( (t.stats.Steine / uc[3]) < max)
      max = t.stats.Steine / uc[3];
    if ( (t.stats.Erz / uc[4]) < max)
      max = t.stats.Erz / uc[4];
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Die Zahl von Truppen pro Ablagefach muss groesser sein als 0.</font>';
      return;
    }
    ...
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Ungültige Zahl von Ablagefächern.</font>';
      return;
    }
    t.setBusy(true);
    var que = [];
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; besitz:'+ t.stats.defOwned;   
    ...
    t.TDspSpace.innerHTML = 'Mauer level: <B>'+ t.stats.wallLevel +'</b><BR>Mauer plaetze: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        Feld plaetze: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  displayCityStats : function (){
    var t = Tabs.Train;
    var cityId = t.selectedCity.id;
    t.stats.Futter = parseInt (Seed.resources['city'+cityId].rec1[0]/3600);
    t.stats.Holz = parseInt (Seed.resources['city'+cityId].rec2[0]/3600);
    t.stats.Steine = parseInt (Seed.resources['city'+cityId].rec3[0]/3600);
    t.stats.Erz = parseInt (Seed.resources['city'+cityId].rec4[0]/3600);
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B>Versorger:</b></td><TD width=8%><B>Miliz:</b></td><TD width=8%><B>Spaeher:</b></td>\
        <TD width=8%><B>Lanzen:</b></td><TD width=8%><B>Schwerter:</b></td><TD width=8%><B>Bogis:</b></td>\
        <TD width=8%><B>Kava:</b></td><TD width=8%><B>sKava:</b></td><TD width=8%><B>Wagen:</b></td>\
 ...
 '<TD width=8%>'+Seed.units['city'+cityId]['unt11']+'</td><TD width=8%>'+Seed.units['city'+cityId]['unt12']+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><B>Futter:</b></td><TD width=16%><B>Holz:</b></td><TD width=16%><B>Steine:</b></td>\
        <TD width=16%><B>Erz:</b></td><TD width=16%><B>Gold:</b></td><TD width=16%><B>Idle Pop:</b></td></tr>\
      <TR align=center><TD>'+ addCommasInt(t.stats.Futter) +'</td><TD>'+ addCommasInt(t.stats.Holz) +'</td><TD>'+ addCommasInt(t.stats.Steine) +'</td>\
        <TD>'+ addCommasInt(t.stats.Erz) +'</td><TD>'+ addCommasInt(t.stats.gold) +'</td>\
...
Tabs.Uebersicht = {
  ...
  hide : function (){
    clearTimeout (Tabs.Uebersicht.displayTimer);
  },

  show : function (){
    var rownum = 0;
    var t = Tabs.Uebersicht;
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Erstellt am:</span> '+ dt.toLocaleDateString() +'</td>\
      ...
      if (Options.includeMarching)
        str += '<TD width=81><B>Unterwegs</b></td>';
      str += "</tr>";
      ...
      str += _row ('Futter', rows[1]);
      str += _row ('Holz', rows[2]);
      str += _row ('Steine', rows[3]);
      str += _row ('Erz', rows[4]);
      str += '<TR><TD><BR></td></tr>';
...
//**********************************
      ...
      for(i=0; i<Cities.numCities; i++) {
        if (row[i] >= 0)
          ...
          if (timeLeft > 86313600)
            ...
            if (Options.enableFutterWarn && timeLeft<(Options.FutterWarnHours*3600))
      ...
      str += _row ('Futter reicht', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Truppen bauzeit', row, true);
      ...
      str += _row ('Mauer bauzeit', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Inklusive Marschierender Truppen/Ressourcen</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
        str += '<BR><BR><INPUT id=subSeed type=submit name="SEED" value="DEBUG">';
      Tabs.Uebersicht.cont.innerHTML = str;
...
//DebugTimer.display ('Draw Uebersicht');    
    } catch (e){
      Tabs.Uebersicht.cont.innerHTML = '<PRE>'+ e.name +' : '+ e.message +'</pre>';
    }   
    t.displayTimer = setTimeout (t.show, 5000);
  },
};
...
Tabs.Marschierende = {
  ...
  init : function (div){
    ...
    t.cont.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubA type=submit value=Angriffe></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubM type=submit value=Marschierende></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubR type=submit value=Verstaerkungen></td></tr></table><HR class=ptThin>\
  ...
  showAttacks : function (){
    var t = Tabs.Marschierende;
    t.marchDiv.innerHTML = 'INCOMING Angriffe coming soon!';;
  },
  
  
  /***   Marschierende SUBTAB  ***/
  showMarches : function (){
    var t = Tabs.Marschierende;
    t.marchDiv.innerHTML = 'OUTGOING Marschierende coming soon!';
  },
  
  
  
  /***  Verstaerkungen SUBTAB  ***/
...
//logit (inspect (enc, 6, 1));    
    ...
    s = '<div class=ptstat>Diese Vertretung von Truppen lagert an jeder Ihrer Botschaften.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Keine Truppen lagern.</b></center>';
    } else {
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Zeigen die Urspruenglichen Truppen';
    s += '<BR><BR><DIV style="font-size: 10px">BEMERKEN SIE: Sie werden KofC neu laden muessen, um neue Lager oder restliche Truppen nach einem Kampf zu zeigen.</div>';
...
// TODO: Check times for expired Marschierende !?!?!
...
// TODO: fixup completed Marschierende
