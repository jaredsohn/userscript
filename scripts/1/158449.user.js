...
// @resource  meta  http://userscripts.org/scripts/source/158449.meta.js
...
// @versioninfo     Change in default rules plus one new rule added (DeviantArt). This only works for clean installs, updaters need to manually change the rules. Look at the script Version History for changes.
// ==/UserScript==
...
Script's Homepage:              http://userscripts.org/scripts/show/158449
...
function LoadSavedData()
{
    ...
    if (d == null)
    {
        ...
        data.title = "Youtube outgoing links";
        data.url = "^https?://www\\.youtube\\.\\w+/";
        data.truncators = "";
        data.attributes = "class:yt-uix-redirect-link;";
        data.sampleURL = "http://www.youtube.com/watch?v=rerRef2rvr0";
        data.override = true;
        data.enabled = true;
        ...
        data = new RowData();
        data.title = "Deviant outgoing links";
        data.url = "^http://.+\.deviantart\.com";
        data.truncators = "http://\\w+\\.deviantart\\.com/users/outgoing\\?";
        data.attributes = "";
        data.sampleURL = "http://sandara.deviantart.com/art/hades-and-persephone-2-210920648";
        data.override = true;
        data.enabled = true;
        dataRules.push(data);

        data = new RowData();
        data.title = "DeviantScript: Downloads Same Tab (Recommend using Script 179414)";
        data.url = "^http://.+\.deviantart\.com/art/";
        data.truncators = "";
        data.attributes = "onclick:PubSub.publish(;";
        data.sampleURL = "http://sandara.deviantart.com/art/hades-and-persephone-2-210920648";
        data.override = true;
        data.enabled = true;
        dataRules.push(data);
        ...
        data = new RowData();
        data.title = "Google outgoing links";
        data.url = "^https?://\\w+\\.google\\.[\\w.]+/";
        data.attributes = "onmousedown";
        data.sampleURL = "http://www.google.co.uk/#hl=en&q=userscripts.org+scripts+show+158449";
        data.override = true;
        data.enabled = true;
        dataRules.push(data);
    }
...
'            </div>' +
'            <div style="width: 100%; background-color: #F0FFFF; display:none;">' +
'                Please improve the filtering by sharing your settings to make updating more comprehensive. ' +
'                Click <a id="copyRules" href="http://userscripts.org/scripts/discuss/158449" target="_blank">HERE</a> which copies rules into the clipboard and then opens the scripts discussion forum in new window. Create a post and paste content. Thank you :)' +
'            </div>' +
'        </div>';
