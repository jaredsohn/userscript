/*
Marking any change by Raimi
javascript:%28function%28%29%7Bvar%20a%3Ddocument.createElement%28%22script%22%29%3Ba.type%3D%22text%2Fjavascript%22%3Ba.src%3D%22http://.js%3F%22%2BMath.random%28%29%3Bdocument.getElementsByTagName%28%22head%22%29%5B0%5D.appendChild%28a%29%7D%29%28%29%3B
Hosting
javascript:%28function%28%29%7Bvar%20a%3Ddocument.createElement%28%22script%22%29%3Ba.type%3D%22text%2Fjavascript%22%3Ba.src%3D%22http://userscripts.org/scripts/source/107443.user.js%3F%22%2BMath.random%28%29%3Bdocument.getElementsByTagName%28%22head%22%29%5B0%5D.appendChild%28a%29%7D%29%28%29%3B
http://www.facebook.com/raimisyazwan
*/

// ==UserScript==
// @name           Sonata 13
// @author         Sonata 13
// @description    Sonata 13
// @credits        Sonata 13
// ==/UserScript==
...
javascript:(function(){
	...
	//initial variables
	var version = 'Assassin-a-Nator v0.35 beta',
	userid = User.id,
	...
	*/
	...
	function do_attack() {
		if (targets.length == 0 && fightlist == 'fightlist') {
			...
			pausing(0,'Ran out of targets, reloading fight list in',function(){ load_fightlist(); });
			return;
		}
		...
		if (targets.length == 0 && fightlist == 'rivals') {
			...
			pausing(2,'Ran out of targets, reloading rivals in',function(){ load_rivals(); });
			return;
		}
		if (run) {
			...
			//else if (targets[0].level > parseInt($('#'+spocklet+'_level_high').val()) || targets[0].level < parseInt($('#'+spocklet+'_level_low').val()) && targets[0].type != 'Stealer') {
				...
				log('Skipping '+targets[0].family+' '+targets[0].name+' because max attacks reached ('+maxattacks+').');
				skip_target();
				repeat_attack();
			}
			else if ((target.prev_health_pct > maxhealth_pct) && (maxhealth_pct != 0)) {
				log('Skipping '+targets[0].family+' '+targets[0].name+' because max health % detected ('+target.prev_health_pct+'% > '+maxhealth_pct+'%).');
				skip_target();
				repeat_attack();
			}
			else if ((target.health > maxhealth) && (maxhealth != 0) && (target.attacks > 2)) {
				log('Skipping '+targets[0].family+' '+targets[0].name+' because max health detected ('+target.health+' > '+maxhealth+').');
				skip_target();
				repeat_attack();
			}
	...
	function parse_attack(response) {
		...
		if (fightResult) {
			...
			if (User.health < parseInt($('#'+spocklet+'_health').val())) {
				...
				pausing(2,'No fight result to parse, retrying in ',function(){ repeat_attack(); });
			}
		}
	...
	function parse_fightlist(response,attack) {
		...
		$(response).find('tr').filter(function() {
			...
			if (name) {
				...
				var iced = $(this).find('td:first img:last').attr('src') == 'http://mwfb.static.zgncdn.com/mwfb/graphics/iced.png' ? true : false;
	...
	$('#'+spocklet+'_plugin_update').click(function(){
		var url="http://spocklet.com/bookmarklet/aan_updates.js";
	...
	$('#'+spocklet+'_plugin_stats').click(function(){
		var url="http://spocklet.com/bookmarklet/aan_stats.js";
		$(this).removeClass('green').addClass('red').unbind('click').find('span span').html('Started');
		var a=document.createElement("script");
		a.type="text/javascript";
		a.src=url+"?"+Math.random();
	...
	$('#'+spocklet+'_plugin_history').click(function(){
		var url="http://spocklet.com/bookmarklet/aan_history.js";
		$(this).removeClass('green').addClass('red').unbind('click').find('span span').html('Started');
		var a=document.createElement("script");
		a.type="text/javascript";
		a.src=url+"?"+Math.random();
	...
	if (/snapi_auth_hash/.test(document.location)) {
		log('It looks like you have unframed the Facebook wrapped game. Some popups may not work properly because of that. We recommend you to unframe <a href="http://www.mafiawars.com">http://www.mafiawars.com</a> or use the direct-unframed link <a href="http://spockon.me/mwunframed">http://spockon.me/mwunframed</a>.')
	}
