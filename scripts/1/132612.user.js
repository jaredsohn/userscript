...
function __MHA_wrapper() {
	...
	function MHA_page_process()
	{
		...
		MHA_syncTime(true);
		if (window.location.hostname.indexOf('facebook') != -1) MHA_mode = 2;
		else if (document.getElementsByClassName('ui-mobile').length == 0) MHA_mode = 0;
		else MHA_mode = 1;
		switch (MHA_mode)
		{
			case 0:MHA_standard_main();break;
			case 1:MHA_mobile_main();break;
			case 2:MHA_facebook_main();break;
		}
	}
	...
	function MHA_travelcontentLoad() {
		...
		MHA_traveltarget.onreadystatechange = function()
		{
			if (MHA_traveltarget.readyState === 4)
			{
				...
				else MHA_travelcontentChild.innerHTML = "Cannot load the page, please refresh";
			}
		};
	...
	function MHA_removeAds()
	{
		...
		if (rightCol != null)
		{
			while (rightCol.childElementCount > 1)
				if (rightCol != null) rightCol.removeChild(rightCol.lastChild);//rightCol.style.display = "none";
		}
		else
		{
		...
		//userObj.viewing_atts.hasOwnProperty('tournament');
	}
	...
	function MHA_trapcontentLoad() {
		...
		MHA_traptarget.onreadystatechange = function()
		{
			if (MHA_traptarget.readyState === 4)
			{
				...
				else MHA_trapdiv.innerHTML = "Cannot load the page, please refresh";
			}
		};
		...
		MHA_shoptarget.onreadystatechange = function()
		{
			if (MHA_shoptarget.readyState === 4)
			{
				if (MHA_shoptarget.status == 200)
				{
					...
					MHA_shopdiv.innerHTML = "";
					MHA_shopdiv.appendChild(HTMLdiv);
				}
				else MHA_shopdiv.innerHTML = "Cannot load the page, please refresh";
			}
		};
	...
	function MHA_simpleSkin()
	{
		...
		//cleanup heading
		document.getElementById('hgbar').removeChild(document.getElementById('appRow'));
		document.getElementById('hgbar').removeChild(document.getElementById('newsRowHolder'));
		...
		hgRow.removeChild(document.getElementById('appLogo'));
		hgRow.removeChild(document.getElementById('communityMenu')); 
		hgRow.removeChild(document.getElementById('supportMenu'));
		var userGreeting = document.getElementById('userGreeting');
		hgRow.removeChild(userGreeting.nextSibling); 
		hgRow.removeChild(userGreeting); 
		
		separator_right = hgRow.getElementsByClassName('hgSeparator right');
		for (i = 1;i < separator_right.length;++i)
			hgRow.removeChild(separator_right[i]); 
	...
	function MHA_defaultSkin()
	{
		...
		hgRow.removeChild(document.getElementById('appLogo')); //remove leftbanner (mousehunt)
		...
		sbplus.firstChild.firstChild.removeChild(sbplus.firstChild.firstChild.firstChild);
		...
		document.getElementById('hgbar').removeChild(document.getElementById('appRow'));//remove the appRow (Inbox,play,Free gift...)
		document.getElementById('hgbar').removeChild(document.getElementById('newsRowHolder'));//remove the News
		...
		document.getElementsByClassName('supernav')[0].removeChild(campbtn);//move the camp button to donation area for moving easier
	...
	function MHA_addControlPanel()
	{
		...
		controlpanel.firstChild.firstChild.removeChild(controlpanel.firstChild.firstChild.firstChild);
	...
	function MHA_initAndLoadSettings()
	{
		...
		if (!window.localStorage.MHA_settings)
		{
			...
			window.localStorage.MHA_aggressive = 1;
			...
			window.localStorage.MHA_KRresume = 0;
			window.localStorage.MHA_KRresumetime = 10800;
		...
		MHA_global.settings.MHA_KRresume = Number(window.localStorage.MHA_KRresume);
		MHA_global.settings.MHA_KRresumetime = Number(window.localStorage.MHA_KRresumetime);
		MHA_global.settings.MHA_scheduleTable = Number(window.localStorage.MHA_scheduleTable);
	}
	...
	function MHA_saveSettings()
	{
		...
		MHA_global.settings.MHA_KRresume = document.getElementById("MHA_KRresume").getElementsByClassName("tick")[0].value
		...
		MHA_global.settings.MHA_KRresumetime = document.getElementById("MHA_KRresumetime").value
		...
		window.localStorage.MHA_KRresume = MHA_global.settings.MHA_KRresume;
		window.localStorage.MHA_KRresumetime = MHA_global.settings.MHA_KRresumetime;
	...
	function initControlPanel()
	{
		...
		tmp = document.getElementById("MHA_KRresume").childNodes[MHA_global.settings.MHA_KRresume];tmp.setAttribute("aria-checked","true");tmp.setAttribute("origin","true");tmp.className="tick";
		...
		document.getElementById("MHA_KRresumetime").value = MHA_global.settings.MHA_KRresumetime;
		...
		i = 0;
		MHA_global.MHA_settinggrouplength[i] = 448;
		tmp[i].style.height = MHA_global.MHA_settinggrouplength[i] + "px";
		i = 1;
		MHA_global.MHA_settinggrouplength[i] = 0;
		tmp[i].style.height = MHA_global.MHA_settinggrouplength[i] + "px";		
	}
...
<div id="MHA_settting_bot" class="MHAsettingtab" style="display:none;">
	...
	<div class="MHAsettinggroup">
		...
		<div class="MHAsetting">
			<label class="MHAlabel">Play King Reward Sound</label>
			<div class="MHAsettingvalue">
		...
		<div class="MHAsetting">
			<label class="MHAlabel">King Reward Resume</label>
			<div class="MHAsettingvalue">
				<div class="MHAsettingvalue">
					<ul id="MHA_KRresume" role="radiogroup" aria-labelledby="">
						<li class="MHAsettingli" value="0" tabindex="0" aria-checked="false">ON</li>
						<li class="MHAsettingli" value="1" tabindex="-1" aria-checked="false">OFF</li>
					</ul>
				</div>
				<div>
					...
					<input id="MHA_KRresumetime" type="text">
					<label> seconds</label>
				</div>
			</div>
		</div>
...
//Todo //new setting css
//auto update
//ads => Tournament + schedules + timers

//King Reward simple mode //test
//edit button for mobile

//color HunterTimer
//problem of hornded
//mobile tabbed button css
...
play sound KR
KR resume
solve puzzle => warn on puzzle cannot solve
mobile
tournament adjustment
*/
/* Schedule
IF (
	Washed Away	/ gone to AR
	Tower Amp
	gold/point/number of bait,charm,things
	on time
	after time
