// ==UserScript==
// @name        MyFreeFarm Danish
// @namespace   http://userscripts.org/scripts/show/186265
...
try{
	...
	texte["msgMarketsaleContent"]="(.*) har ved markedspladsen k"+o_stroke+"bt\\s*(\\d+)x (.*?) af dig<br>\\s*for(.*?) kD\\."; // The text where the information is stated. The information has to be replaced by "(.*?)".
	// For the following you have to take a message sent if you sell something via contract
	texte["msgContractsaleContent"]="(.*) har underskrevet en kontrakt fra dig!<br><br>\\s*F"+o_stroke+"lgende produkter blev solgt:<br>([\\S\\s]*)\\s*<br>\\s*Kontraktsummen p"+a_ring+" (.*?) kD blev indsat p"+a_ring+" din konto\\."; // The text where the general information is stated. The information has to be replaced by "(.*?)".
	...
	texte["msgFriend"]="(.+) m"+o_dots+"chte dich als Freund hinzuf"+u_dots+"gen"; // The subject. The person has to be replaced by "(.+)"
