...
var firefoxVersion = "3.6b"; //getFirefoxVersion();
    ...
    sendReinforcmentHome: function(){ //FUNCTION NOT IN USE YET BUT SOON :-)
        ...
        new Ajax.Request(g_ajaxpath + "ajax/kickoutReinforcements.php" + g_ajaxsuffix, {
            ...
            onSuccess: function(transport){
                var rslt = TM_eval("(" + transport.responseText + ")");
                if (rslt.ok) {
...
// WinManager.hideall();      <=== TODO!  
  ...
  function searchDOM (node, condition, maxLevel, doMult){
    var found = [];
    var compFunc =  TM_eval ('(function (node) { return ('+ condition +') })');
    doOne(node, 1);
    if(!doMult){
      if (found.length==0)
        return null;
...
var FairieKiller  = {
  saveFunc : null,
  init : function (tf){
    if (firefoxVersion.substring(0,4) == '4.0b')  // bug in firefox 4.0b10 causes syntax error with: "var func = TM_eval ('function (){}');"
      return;
  ...
  setEnable : function (tf){
    if (tf)
      unsafeWindow.Modal.showModalUEP = TM_eval ('function (a,b,c) {actionLog ("Blocked Faire popup");}');
    else
...
var anticd = {
  ...
  init: function (){
    ...
    unsafeWindow.cm.cheatDetector.detect = TM_eval ('function (){}');
...
if (DEBUG_TRACE) logit (" 0 myAjaxRequest: "+ url +"\n" + inspect (o, 2, 1));
  ...
  function mySuccess (msg){
    var rslt = TM_eval("(" + msg.responseText + ")");
    var x;
