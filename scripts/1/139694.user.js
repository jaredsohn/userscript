	...
	var TAS_main = function () {
                  ...
                  // player bases info to share with others
                    ...
                    var apcl = apc.get_AllCities().d;//all player cities list
										for (var key in apcl) {
											var c = apcl[key];
											try {
												txt += "Basis \'" + c.get_Name() + "\' \n"; //m_Level
												txt += "Basis     lvl: " + c.get_LvlBase().toFixed(2).toString() + "\r\n";
												txt += "Defensive lvl: " + c.get_LvlDefense().toFixed(2).toString() + "\r\n";
												txt += "Offensive  lvl: " + c.get_LvlOffense().toFixed(2).toString() + "\r\n";
												txt += "Distanz zum Centrum: " + Math.round(ClientLib.Base.Util.CalculateDistance(ClientLib.Data.MainData.GetInstance().get_Server().get_ContinentWidth() / 2, ClientLib.Data.MainData.GetInstance().get_Server().get_ContinentHeight() / 2, c.get_PosX(), c.get_PosY())) + "\r\n";
            ...
            //button Alt+B
              ...
              // REPAIR ALL OWN ARMIES
							try {	// NOTICE Under construction
                var pc = ClientLib.Data.MainData.GetInstance().get_Cities();//player cities
                var oc = ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities().d;
			...
			// Loading
			function TAS_checkIfLoaded() {
				try {
					if (typeof qx != 'undefined') {
						ap = qx.core.Init.getApplication(); // application
						mb = qx.core.Init.getApplication().getMenuBar();
						if (ap!==null && mb!==null) {
							createInstance();
							window.TAS.main.getInstance().initialize();
						} else window.setTimeout(TAS_checkIfLoaded, 1000);
					} else {
						window.setTimeout(TAS_checkIfLoaded, 1000);
					}
				} catch (e) {
