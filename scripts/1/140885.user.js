// ==UserScript==
// @name           Travian Overview Viewer [Including DistanceCalculator]
...
// @description    To keep track on all of your villages. [New Release of 'Travian Site Update Data']
// @include        http://*t*.travian.*/*
// @include        http://traviancalc.net84.net/*
// @include        http://traviancalc.clanteam.com/*
// ==/UserScript==

var thisVersion = "1.1.1.1";
var saveDataServer = "http://traviancalc.net84.net";
...
function setUpServerChoice(){
	...
	scs.width = "24%";
	scs.height = "20%";
	scs.border = "2px outset #000";
	scs.zIndex = "400";
	scs.background = "#fff";
	document.body.appendChild(scdiv);
	
	sctext = document.createElement("div");
	sctext.style.textAlign = "center";
	sctext.style.fontSize = "16px";
	sctext.style.fontFamily = "sans-serif";
	sctext.style.padding = "5px";
	sctext.innerHTML = "Please choose the server you want to use";
	scdiv.appendChild(sctext);
	
	scdiv.appendChild(document.createElement("br"));
	
	scc = document.createElement("center");
	scdiv.appendChild(scc);
	
	scForm = document.createElement("form");
	scForm.setAttribute("name", "scForm");
	scForm.setAttribute("action", "dorf1.php");
	scForm.setAttribute("method", "GET");
	scc.appendChild(scForm);
	
	scsel = document.createElement("select");
	scsel.setAttribute("name", "serverSel");
	scsel.style.align = "center";
	scsel.style.fontSize = "17px";
		optArr = ["http://traviancalc.net84.net", "000webhost", "http://traviancalc.clanteam.com", "Zymic"];
		for(i = 0; i<optArr.length/2; i++){
			opt = document.createElement("option");
			opt.setAttribute("value", optArr[2*i]);
			opt.innerHTML = optArr[2*i+1];
			opt.style.width = "150px";
			opt.style.height = "25px";
			opt.style.fontSize = "17px";
			if(i == 0)opt.setAttribute("selected", "true");
			scsel.appendChild(opt);
		}
	scForm.appendChild(scsel);
	
	scForm.appendChild(document.createElement("br"));
	
	scbutt = document.createElement("submit");
	scbutt.innerHTML = "Choose";
	scbutt.setAttribute("name", "wes");
	scbutt.setAttribute("onMouseOver", "this.style.opacity='0.75';");
	scbutt.setAttribute("onMouseOut", "this.style.opacity='1';");
	scbutt.setAttribute("onClick", "this.parentNode.submit();");
	scbs = scbutt.style;
	scbs.padding = "0.34em 1.5em 0.4em";
	scbs.backgroundColor = "#7aa824";
	scbs.background = "-webkit-gradient(linear, left bottom, left top, color-stop(0, #77a423), color-stop(0.6, #93c436), color-stop(0.97, #aad15d), color-stop(0.99, #d4ed99))";
	scbs.background = "-moz-linear-gradient(bottom, #77a423, #93c436 60%, #aad15d 96%, #d4ed99 98%)";
	scbs.borderRadius = "3px";
	scbs.mozBorderRadius = "3px";
	scbs.webkitBorderRadius = "3px";
	scbs.border = "1px solid #81a340";
	scbs.borderBottom = "1px solid #708444";
	scbs.color = "white";
	scbs.font = "bold 15px arial,sans-serif";
	scbs.marginTop = "10px";
	scbs.cursor = "pointer";
	scbs.cursor = "hand";
	scbs.display = "inline-block";
	scForm.appendChild(scbutt);
}
	
/* ------------------------------------------- END OF FUNCTIONS ------------------------------------- */

if(getArgument("serverSel") != null){
	today = new Date();
	today.setTime(today.getTime());
	edate = new Date(today.getTime()+(1000*60*60*24*365)).toGMTString();	//1 year in the future
	ck = 'servername='+getArgument("serverSel")+';expires='+edate;
	document.cookie = ck;
}
...
if(window.location.hostname.indexOf("travian.") != -1){
	...
	if(cookie_exists("servername") && Get_Cookie("servername") != "null")saveDataServer = Get_Cookie("servername");
	else setUpServerChoice();
