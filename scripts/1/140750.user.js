...
function PhotoViewer(win, handleKeys) {
    ...
    this.shadeColor = "#FFFFFF";
    this.shadeOpacity = 0.95;
...
function showPhoto(index, cropWidth, opacity) {
    ...
    photoDiv.style.overflow = "visible";
...
function zoomPhoto(times) {
    var div = document.getElementById(VIEWER_ID_PHOTO);
    var img = div.getElementsByTagName('img')[0];
    var imgW = parseInt(img.style.width);
    var imgH = parseInt(img.style.height);
    var width =  imgW * times;
    var height = imgH * times;
    img.style.width = div.style.width = width  + 'px';
    img.style.height = div.style.height = height  + 'px';
    var winSize = getWindowSize(this.win);
    var availW = winSize.w - (this.nopadding ? this.borderWidth * 2 : 20);
    var availH = winSize.h - (this.nopadding ? this.borderWidth * 2 : 20);
    var scrollPos = getScrollPos(this.win);
    var scrollLeft = scrollPos.left;
    var scrollTop = scrollPos.top;
    var top =  (availH - height)/2 + scrollTop;
    var left =  (availW - width)/2 + scrollLeft;
    if (top < 0) top = 0;
    if (left < 0) left = 0;
    div.style.top = top + 'px';
    div.style.left = left + 'px';
    this.log('imgW:' + imgW + ' width:' + width + 'availW:' + availW + ' scrollLeft:' + scrollLeft);
}
...
function linkPhotos(viewer, node) {
    var i;
    if (node == undefined)
        return;
    if (isThumbnail(node)) {
        var photo = new PhotoImg(undefined, node.src);
        var found = false;
        for (i = 0; i < viewer.photos.length; i++) {
            if (viewer.photos[i].src == photo.src) {
                found = true;
    ...
    if (node.childNodes != undefined) {
        for (i = 0; i < node.childNodes.length; i++) {
            linkPhotos(viewer, node.childNodes[i]);
        }
    }
}
function isThumbnail(node) {
    if (node.nodeName.toLowerCase() != "img")
        return false;
    if (!isNaN(node.width) && node.width < MIN_THUMBNAIL_DIM)
        return false;
    if (!isNaN(node.height) && node.height < MIN_THUMBNAIL_DIM)
        return false;
    return true;
}
var IMAGE_FORMATS = ["jpg", "jpeg", "png", "gif", "bmp", "tif", "tiff"];
function findImageLink(link, src) {
    if (link == undefined || link.href == undefined)
        return false;
    var i;
    for (i = 0; i < IMAGE_FORMATS.length; i++) {
        if (link.href.indexOf("." + IMAGE_FORMATS[i]) != -1) {
            return true;
        }
    }
    var flickrRE = /.+static\.flickr\.com.+_.\.jpg/;
    if (flickrRE.test(src)) {
        link.href = src.replace(/_.\.jpg/, "_b.jpg");
        return true;
    }
    return false;
}
function initDefaultViewer() {
    if (defaultViewer == undefined) {
        if (document.viewer == undefined) {
            defaultViewer = new PhotoViewer();
        } else {
            defaultViewer = document.viewer;
            defaultViewer.photos = [];
            defaultViewer.index = 0;
        }
    } else {
    ...
    linkPhotos(defaultViewer, document.body);
    if (defaultViewer.photos.length > 0) {
        defaultViewer.show();
    } else {
        defaultViewer = undefined;
    }
}
document.onload = setTimeout(initDefaultViewer,1000);