// ==UserScript==
// @name           My edit to Brawler
// @namespace      http://bukanpengemis.blogspot.com
// @description    An Auto fighter on MW
// ==/UserScript==
...
javascript: (function () {
	...
	function attack_user() {
		...
		u = $.ajax({
			...
			error: function (a, b, c) {
				logmsg('Error sending request, Retrying attack..', 'status');
				setTimeout(attack_user, 2000)
			}
		})
	}
	function attack() {
	...
	function Process_Attack(b) {
		...
		try {
			if ((/secret stash/.test(b)) && (document.getElementById('Log_Ice_Posts').checked)) {
				var h = /MW.Feed(.+?);; return false;/.exec(b)[1];
				bs[bs.length] = "MW.Feed" + h + ";";
		...
		if (/You won/i.test(b)) {
			...
			if (document.forms.something.Fightlist.checked) {
				if (parseInt(/(sexy_bangkok_cash|sexy_vegas_cash|sexy_italy_cash|sexy_brazil_cash|sexy_new_york_cash) good"\>[^\$]+\$([^<]+)/.exec(b)[2].replace(/,/g, '')) < parseInt(cash_limit)) {
			...
			if (document.forms.something.Fightlist.checked) {
				V[V.length] = y[i];
				logmsg(x[i] + ' You lost.... ' + random_delay + ' seconds..', 'attack_log');
				bu++;
				if (document.getElementById('Log_Targets').checked) {
	...
	function pre_check() {
		...
		if ((document.getElementById('user_stamina').innerHTML <= 0) || ((document.getElementById('user_stamina').innerHTML < 5) && (bj == 7))) {
			logmsg('Get more stamina, you're out...', 'status');
			stop();
			return
		}
		if (bp && (parseInt(O) <= 30) && by) {
