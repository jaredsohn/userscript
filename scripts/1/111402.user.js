...
(function() {
 ...
 var version = '3.0.0'; // also, check meta-data field @version
 ...
 var $themeswitcher;
    ...
    function setState(newOn)
    {
      ...
      else
      {
        $("#cssOverride").text(
		  ...
		  //my black list
          ,".battleResultDetailedMessageShowAvatarButtonContainer" 
          ].join(", ") + " { display: none; }"
        );
      }
      /*
  ...
  worlds = (function()
  {
    ...
    function loadWorldList(ondone)
    {
      ...
      ajax.open("GET", HOMMK.BASE_URL + "selectWorld", true);
...
var spShow = function(){ 
	switcherpane.css({
top: button.offset().top + options.buttonHeight + 16, 
...
(function() {
 ...
 // build request for player names 
   ...
   row.insertCell(-1).innerHTML = "Цвет";
   row.insertCell(-1).innerHTML = "Альянс";
   row.insertCell(-1).innerHTML = "Игрок";
   row.insertCell(-1).innerHTML = "Из";
   row.insertCell(-1).innerHTML = "Текущий";
   row.insertCell(-1).innerHTML = "Цель";
	...
	if (!$dialog_about) {
		...
		$dialog_about = $('<div id="hommk-user-about"></div>')
			.html(
					...
					""
					,''
					,'RU page : <a href="http://userscripts.org/scripts/show/111402" class="ui-state-default" target="_blank">[userscript.org]</a>'
					].join("<br>")
			     )
			.dialog({
autoOpen: false,
...
title: localize('LINK_ABOUT')
	...
	$('<div style="padding-top: 6px">Хочешь передвинуть главное окошко?<br><a href="#Container" class="ui-state-default">Жми сюда ...</a></div>')
...
function init() { 
  ...
  */
  ...
  // header
  $dialog_panel.html(  "Версия " + version + " " );
...
/*
  ...
  .ready(function() { 
    // Get List of items
    console.log("worlds.count",worlds.count());
    if( worlds.count() == 0)
...
*/
  // Jactari button
	...
	$('<button id="hommk-user-jactari-button" style="width:120;margin-top:0px; display:block;">Симулятор - Атака</button>')
  ...
  .appendTo($panel_extended)
	...
	$('<button id="hommk-user-jactari-button-defender" style="width:120;margin-top:0px; display:block;">Симулятор - Защита</button>')
	.button({disabled:true})
  .appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
// Le Scribe
	...
	$('<button id="hommk-user-scribe-button" style="width:120;margin-top:5px; display:block;">Шпионский репорт</button>')
	.button({disabled:true})
  .appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
// La Vigie
/*
	$('<button style="width:120;margin-top:5px; display:block;">Часовой</button>')
	.button()
  .appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
...
*/
  ...
  .appendTo($panel_extended);
	...
	$('<button style="margin-top:5px; display:block;">Центрировать</button>')
	.button()
  .appendTo($panel_extended)
	.click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
  ...
  $('<span class="ui-state-default">Платные кнопки</span><br />')
  .appendTo($panel_extended)

  $('<div id="buyables">' + 
    '<input type="radio" id="buyoff" name="buyables" ' + (buyables.state() ? '' : ' checked="checked"') + ' /><label for="buyoff">Выкл</label>' +
    '<input type="radio" id="buyon" name="buyables"' +   (buyables.state() ? ' checked="checked"' : '') + ' /><label for="buyon">Вкл</label>' +
    '</div>'
  ).appendTo($panel_extended)
  .buttonset();
/*
...
*/
  ...
  $('<button style="margin-top: 5px; display: block;">Рунные Крепости</button>')
  .button()
  .appendTo($panel_extended)
  .click(function() { HOMMK.openRegionBuildingListFrame(); return false; } )
  ;

  $('<button id="hommk-user-jactari-button-dump" style="margin-top: 5px; display: block;">Дамп вещаков</button>')
  .button()
  .appendTo($panel_extended)
  .click(function() {
    //try
