...
// Modified by Charles Dejean, June 2011
...
// ==UserScript==
// @name          MMHK
// @author        Julien Eyries, Charles Dejean
// @namespace     
// @description   Improve HOMMK functionnality
...
var version = '1.1.1'; // also, check meta-data field @version
...
//    return d.toLocaleString();
    return d.toLocaleDateString() + " " + padZero(d.getHours()) + ":" + padZero(d.getMinutes()) + ":" + padZero(d.getSeconds());
}
...
// locale secondaire : français ou allemand
...
var donnees = {
    ...
    d:{
        ...
        fortification:0, // 1:fortin/2:citadelle/3:château
        forts:0,
        fort_principal:0,
        dolmens:0,
        ralliement:0,
...
extend( locale_strings_default, {
    ...
    "VIGIE_SEARCH_TROOPS":   "Searching troops movements...",
	"VIGIE_DONE":            "Done."
});
...
//	console.log("0004");
   ...
   // build request for player names 
	...
	if (!HOMMK_user.playerMap) {
		HOMMK_user.playerMap = new Object();
	}
	...
	fill_vigie_table(table, moves, 0);

}

function fill_vigie_table(table, moves, index) {
	// Get out of the loop
	if ( !moves || !moves.length || moves.length == 0 || index >= moves.length ) {
		return;
	}
	
	// Process
	var move = moves[index].masterHeroMove;

	var sPlayerId = move.playerId + "";
	if (!HOMMK_user.playerMap[sPlayerId]) {
		// Player not yet fetched => request
		var elParamList = [];
		elParamList.push({"elementType":"ProfileFrame","elementId":sPlayerId});
		var json = do_sync_request({"elParamList":elParamList});
//		console.log(json);
		var result = json.d["ProfileFrame" + sPlayerId];
		HOMMK_user.playerMap[sPlayerId] = {"name":result.playerName, "alliance":result.allianceName};
	}
	...
	var cell = row.insertCell(-1);          
	cell.innerHTML = HOMMK_user.playerMap[sPlayerId].name;

	var cell = row.insertCell(-1);          
	cell.innerHTML = ( HOMMK_user.playerMap[sPlayerId].alliance ? HOMMK_user.playerMap[sPlayerId].alliance : " " );
	...
	// Loop
	setTimeout(function() { fill_vigie_table(table, moves, index + 1) }, 0);
}
...
extend( locale_strings_fr , {
    ...
    'LINK_UPDATE': function(param){
                    ...
                    + ' Mettez votre script à jour <a href="'  + param[2] + '" class="ui-state-default" target="_blank">ici</a>'; }
});
...
function launch_about(){
    ...
    if (!$dialog_about) {
	    ...
	    $dialog_about = $('<div id="hommk-user-about"></div>')
		    .html(
                [
                "MMHK user script version " + version,
                "\251 jeyries 2010"
                ,'home page : <a href="http://userscripts.org/scripts/show/79617" class="ui-state-default" target="_blank">[userscript.org]</a>'
...
function init() { 
    ...
    $('<button id="hommk-user-jactari-button" style="margin-top:0px; display:block;">Jactari !</button>')
        .button({disabled:true})
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
		    HOMMK_user.jactari.launch();
            return false;
	    });
    ...
    $('<button id="hommk-user-scribe-button" style="margin-top:5px; display:block;">Le Scribe</button>')
        .button({disabled:true})
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<button style="margin-top:5px; display:block;">La Vigie</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<button style="margin-top:5px; display:block;">Le Cartographe</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $dialog_panel.dialog( "option", "show", 
        ...
        complete: function() {
            do_request_gm(
                {
                    request: "hello",
                    player: HOMMK.player.content.name,
                    world:  HOMMK.player.content.worldId,
                    world_name: HOMMK.WORLD_NAME,
                    version: version,
                },
                function(json){        
                    if (json.current_version && json.current_version!=version){
                        $("#hommk-user-info").html( 
                            localize('LINK_UPDATE', json.current_version, "http://userscripts.org/scripts/show/79617" ));
                    }
                });
        }});
...
console.log("MMHK user script loaded");
