// ==UserScript==
// @name          Tribal eXtension! v3.0.2 PL
// @namespace     http://none
// @description   poprawione spolszczenie TribalWars v3.0.2 by JonnyD
...
function twLoadingData(mode)
{
	...
	{ 
		...
		newHTML.innerHTML = '<div style="width:300px;margin-top:150px;margin-bottom:auto;background-color:#F8F4E8;border:2px solid #804000;padding:30px 30px 30px 40px;margin-left:auto; margin-right:auto;"><img src="http://i184.photobucket.com/albums/x195/twextension/loading.gif" border="0" ><br><b>Odświeżanie</b></div>';
...
function twInitialize()
{
	...
	twLanguage["es"]["mainMenu"] = new Array ("Wyloguj", "Forum", "Chat", "Pomoc", "Opcje", "Premium", "Ranking", "Plemię", "Raporty", "Wiadomości"); 
	...
	twLanguage["es"]["units"] = new Array("Pikinier", "Miecznik", "Topornik", "Łucznik", "Zwiadowca", "Lekka kawaleria", "Łucznik na koniu", "Ciężka kawaleria", "Taran", "Katapulta", "Rycerz", "Szlachcic");
	twLanguage["es"]["buildings"] = new Array ("Ratusz", "Koszary", "Stajnia", "Warsztat", "Corte Aristocr&aacute;tica", "Herrer&iacute;a", "Plac", "Plaza de Mercado", "Los Le&ntilde;adores", "Barrera", "Mina de Hierro", "La Granja", "Almac&eacute;n", "El Escondrijo", "Muralla");
	...
	twLinks["twMainMenuArray"]["forum"] = "http://board.tribalwars.es/index.php";
	...
	twLinks["twMainMenuArray"]["settings"]= new Array();
		...
		twLinks["twMainMenuArray"]["settings"][10] = "http://www.plemiona.pl/ticket_login.php";
...
function twShowInfoResources()
{
	...
	newDiv += "		<tbody>";
	newDiv += "			<tr>";
	newDiv += "				<th style='background-color:#804000;color:#fff;width:auto;height:29px;padding:2px 4px 2px;margin-right:5px'>Surowce:</th>";
...
function twShowInfoFarm()
{
	...
	newDiv += "	<tbody>";
	newDiv += "		<tr>";
	newDiv += "			<th style='background-color:#804000;color:#fff;width:auto;height:29px;padding:2px 2px 2px 8px;'>Ludność: </th>";
...
function twShowInfoArmy(village_id)
{
	...
	newDiv += "	<tbody>";
	newDiv += "		<tr>";
	newDiv += "			<th style='background-color:#804000;color:#fff;width:auto;height:28px;padding:2px 2px 2px 2px;'>Wojsko: </th>";
	for(var i=0;i<12;i++){
...
{
		var newHTML ="";
		newHTML += "<h2>Przegląd wojsk</h2>";
		newHTML += "<table id='GlobalArmyInfo' width=100% style='text-align:left;'>"
		newHTML += "	<tr style='text-align:center'><th rowspan=2>Wioski</th><th rowspan=2>Punkty</th><th colspan=12 width:80% align='center'>Wojsko</th></tr>";
		newHTML += "	<tr>";
...
{	
	try{
		...
		newDiv += '		<tbody>';
		newDiv += '			<tr>';
		newDiv += '				<td style="border-right:1px solid #804000"><a id="btnDataRefresh" href="javascript:void(0)"><img src="'+twGlobalInfo["InterfaceItems"][7]+'" title="Odśwież" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["overview"]+'"><img src="'+twGlobalInfo["InterfaceItems"][4]+'" title="Podglad wioski" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["map"]+'"><img src="'+twGlobalInfo["InterfaceItems"][5]+'" title="Mapa" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["villages"]+'"><img src="'+twGlobalInfo["InterfaceItems"][8]+'" title="Przegląd wiosek" />&nbsp;</a></td>';
		newDiv += '				<td><a id="btnShowAllArmy" href="javascript:void(0)"><img src="'+twGlobalInfo["InterfaceItems"][6]+'" title="Przeglad wojsk"/>&nbsp;</a></td>'
		newDiv += '			</tr>'
		newDiv += '		</tbody>';
		newDiv += '	</table>';
		newDiv += '</div>';
...
function twReportGenerator(reportInfo)
{
		...
		newReport[0] = '[b]Wysłane '+reportInfo['date'].substring(0,8)+' o '+reportInfo['date'].substring(14,8)+'[/b]\n';
		// Vencedor de la Batalla
		if(attackerLossesUnits == attackerTotalUnits)
			newReport[0] +='Obrońca ('+reportInfo['defender']['player']+').';
		else
			newReport[0] +='Agresor ('+reportInfo['attacker']['player']+').';

		
		newReport[1] = '\n\n[i]Sszczęście:[color=red] '+reportInfo['luck']+' [/color][/i]\n';
		newReport[2] = '[i]Morale:[color=blue] '+reportInfo['morale']+' [/color][/i]\n\n';
		

		// Datos del Atacante
		newReport[3] = '[b]Agresor: [color=purple][size=15] '+reportInfo['attacker']['player']+' [/size][/color][/b]\n'; 
		newReport[4] = '[b]Wioska: [color=navy][size=13] '+reportInfo['attacker']['village']['name']+' ['+reportInfo['attacker']['village']['x']+'|'+reportInfo['attacker']['village']['y']+'] [/size][/color][/b]\n';
		...
		newReport[5] += '\n[i][b]Straty Agresora:[/b]'; 
		newReport[5] += '\n&nbsp; Wojsko: [color=darkred]'+attackerLossesUnits+'[/color]/[b]'+attackerTotalUnits+'[/b] ([color=green]'+(attackerTotalUnits-attackerLossesUnits)+'[/color]) Jednostek ([b][color=blue]'+(Math.round((attackerLossesUnits*100/attackerTotalUnits)*10)/10)+'%[/color][/b]) Jednostek'
		newReport[5] += '\n&nbsp; Surowce:[img]'+twLinks["twOtherImg"][0]+'[/img]'+reportInfo['attacker']['lossesResources']['wood']+'[img]'+twLinks["twOtherImg"][1]+'[/img]'+reportInfo['attacker']['lossesResources']['stone']+'[img]'+twLinks["twOtherImg"][2]+'[/img]'+reportInfo['attacker']['lossesResources']['iron'];
		...
		newReport[6] = '\n[b]Obrońca: [color=purple][size=15] '+reportInfo['defender']['player']+' [/size][/color][/b]\n'; 
		newReport[7] = '[b]Wioska: [color=navy][size=13] '+reportInfo['defender']['village']['name']+' ['+reportInfo['defender']['village']['x']+'|'+reportInfo['defender']['village']['y']+'] [/size][/color][/b]\n';
		...
		if(parseInt(defenderTotalUnits) != 0)
		{
			...
			newReport[8] += '\n[i][b]Straty Obrońcy:[/b]';
			newReport[8] += '\n&nbsp;Wojsko: [color=darkred]'+defenderLossesUnits+'[/color]/[b]'+defenderTotalUnits+'[/b] ([color=green]'+(defenderTotalUnits-defenderLossesUnits)+'[/color]) Jednostek ([b][color=blue]'+(Math.round((defenderLossesUnits*100/defenderTotalUnits)*10)/10)+'%[/color][/b])';
			newReport[8] += '\n&nbsp;Surowce:[img]'+twLinks["twOtherImg"][0]+'[/img]'+reportInfo['defender']['lossesResources']['wood']+'[img]'+twLinks["twOtherImg"][1]+'[/img]'+reportInfo['defender']['lossesResources']['stone']+'[img]'+twLinks["twOtherImg"][2]+'[/img]'+reportInfo['defender']['lossesResources']['iron'];
		...
		else
		{
			if(attackerTotalUnits == attackerLossesUnits)
			{
				...
				newReport[8] +='\n\n[i][b]Straty:[/b] [b]Perdi&oacute;[/b] [color=darkred]????[/color]/[b]????[/b] Jednostek ([b][color=blue]??%[/color][/b]). [b]Przeżyło[/b]: [color=green]????[/color] Jednostek[/i].\n\n';
			}
			else
			{
				newReport[8] = '[i]Sin Tropas. El Pueblo est&aacute; vac&iacute;o[/i]';
				newReport[8] +='\n\n[i][b]Straty:[/b] [b]Perdi&oacute;[/b] [color=darkred]0[/color]/[b]0[/b] Jednostek ([b][color=blue]100%[/color][/b]). [b]Resto[/b]: [color=green]0[/color] Jednostek [/i].\n\n';
			}
		}
		...
		newReport[11] +='\n\n[b]Raport wygenerowany przy pomocy Tribal e[color=sienna]X[/color]tension [/b]';
		return newReport;
};
//__________________________________________________________________________________________________________________//
...
function twReportInterface()
{
		...
		newHTML += '<dl id="reportEffect"><dl>'
			newHTML += "<h2><b>Konwerter</b></h2>"
			newHTML += "<div><a href='"+twUrlScreen+"village="+vID+presentURL+"&view="+getParam("view")+"' >Powrót</div>"
			// opciones
			newHTML += '<dt id="twReportSettings"><a href="javascript:void(0)">Opcje</a></dt>';
			newHTML += '<dd>';
			newHTML += '		<input type="checkbox" id="coords" checked >Pokaż/Ukryj Współrzędna<br>';
			newHTML += '		<input type="checkbox" id="nombreAtc" checked >Pokaż/Ukryj Straty Agresora<br>';
			newHTML += '		<input type="checkbox" id="nombreDef" checked >Pokaż/Ukryj Straty Obrońcy<br>';
			newHTML += '		<input type="checkbox" id="ArmyAtc" checked >Pokaż/Ukryj Wojsko Agresora<br>';
			newHTML += '		<input type="checkbox" id="ArmyDef" checked >Pokaż/Ukryj Wojsko Obrońcy<br><br>';
			newHTML += '		<input type="button" id="aplicaCambios" value="Potwierdź" disabled ><br>';
			newHTML += '</dd>';
			
			// Informe Compactado
			newHTML += '<dt><a href="javascript:void(0)">Raport</a></dt>';
			newHTML += '<dd><textarea id="twReport"  name="twReport" cols=94 rows=15>'+reportWithForumTags+'</textarea></dd>';

			// Vista PrĂ©via del informe
			newHTML += '<dt id="twReportPreview"><a href="javascript:void(0)">Podgląd</a></dt>';
...
function twMainMenuNavigationBar()
{
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][0]+'">Wiadomości</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][1]+'">Wiadomość grupowa</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][2]+'">Napisz wiadomość</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][3]+'">Zablokuj nadawce</a></li>';
	codeNavBar +=				'</ul>';
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][0]+'">Wszystkie</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][1]+'">Atak</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][2]+'">Obrona</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][3]+'">Pomoc</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][4]+'">Handel</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["reports"][5]+'">Inne</a></li>';
	codeNavBar +=				'</ul>';
	codeNavBar +=			'</li>';
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][0]+'">Przeglądaj</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][1]+'">Czlonkowie</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][2]+'">Profil</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][3]+'">Dyplomacja</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][5]+'">Zaproszenia</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][6]+'">Wiadomość powitalna</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ally"][7]+'">Właściwości</a></li>';
	...
	codeNavBar +=	'<li><a href="javascript:void(0)">&nbsp;Dodatki</a>';
	codeNavBar +=		'<ul>';
	...
	codeNavBar +=					'</ul>';
	codeNavBar +=				'</li>';
	codeNavBar +=				'<li><a href="javascript:void(0)">Kalkulatory</a>';
	codeNavBar +=					'<ul>';
	codeNavBar +=						'<li><a href="'+twLinks["twUtils"][5]+'" target="_blank">Kalkulator odległości</a></li>';
	codeNavBar +=						'<li><a href="'+twLinks["twUtils"][6]+'" target="_blank">Kalkulator odległości w Excelu</a></li>';
	codeNavBar +=						'<li><a href="'+twLinks["twUtils"][7]+'" target="_blank">Kalkulator strat</a></li>';	
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][0]+'">Profil</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][1]+'">Email</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][2]+'">Ustawienia</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][3]+'">Zacznij od nowa</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][4]+'">Usuń konto</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][5]+'">Dzielić łącze</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][6]+'">Zastępstwo</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][7]+'">Logowanie</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][8]+'">Zmień hasło</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][9]+'">Ankiety</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["settings"][10]+'">Pomoc</a></li>';
	codeNavBar +=				'</ul>';
	codeNavBar +=			'</li>';
	...
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][0]+'">Gracze</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][1]+'">Plemiona</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][2]+'">Gracze na kontynencie</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][3]+'">Plemiona na kontynencie</a></li>';
	codeNavBar +=				'</ul>';
	codeNavBar +=			'</li>';
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=intro" target="_blank">Pierwsze kroki</a></li>';
	codeNavBar +=					'<li><a href="javascript:void(0);" target="_blank">Zagraniczne plemiona</a>';
	codeNavBar +=						'<ul>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.es/" target="_blank">Plemiona Hiszpańskie (es)</a></li>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.net/" target="_blank">Plemiona Angielskie (net)</a></li>';
	codeNavBar +=							'<li><a href="http://www.die-staemme.de/" target="_blank" >Plemiona Niemieckie (de)</a></li>';	
	codeNavBar +=							'<li><a href="http://www.tribalwars.it/" target="_blank">Plemiona Włoskie (it)</a></li>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.fr/" target="_blank">Plemiona Francuskie (fr)</a></li>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.nl/" target="_blank">Plemiona Holenderkie (nl)</a></li>';
	codeNavBar +=							'<li><a href="http://www.plemiona.pl/" target="_blank">Plemiona Polskie (pl)</a></li>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.ro/" target="_blank">Plemiona Rumuńskie (ro)</a></li>';
	codeNavBar +=							'<li><a href="http://www.tribalwars.se/" target="_blank">Plemiona Szweckie (se)</a></li>';
	codeNavBar +=						'</ul>';
	codeNavBar +=					'</li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=late_start" target="_blank">Późny początek</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=buildings" target="_blank">Budynki</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=units" target="_blank">Jednostki</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=fight" target="_blank">System bitwy</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=points" target="_blank">Punkty</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=reports" target="_blank">Raporty</a></li>';
...
//	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["help"]+'?mode=server_info" target="_blank">Informaci&oacute;n del Servidor</a></li>';
	...
	codeNavBar +=					'<li><a href="http://www.plemiona.pl/rules.php" target="_blank">Reguły</a></li>';
	codeNavBar +=				'</ul>';
	codeNavBar +=			'</li>';
...
function twRankingBar()
{
	...
	newHTML += '<div style="position:absolute;top:115px;left:205px;width:700px;">';
		...
		newHTML += '	<div style="float:left;"><img  src="'+twGlobalInfo["InterfaceItems"][9]+'" width="30" height="28" border="0" alt=""  /></div>';
		newHTML += '	<div style="float:left;width:141px;height:22px;margin-top:3px;background-image:url(\''+twGlobalInfo["InterfaceItems"][10]+'\');background-repeat:repeat-x"><div style="margin:3px;color:#804000;font-weight:bold">Opcja niedostępna</div></div>';
		newHTML += '	<div style="float:left;margin-top:3px"><img  src="'+twGlobalInfo["InterfaceItems"][11]+'" width="9" height="22" border="0" alt="" /></div>';
		newHTML += '</div>';
...
function twBuildingsBar()
{
	...
	//	Enlaces
	//newHTML += '<div id="mMain" class="iconBuilding" style="width:120px;top:155px;margin-left:840px;"><a href="'+twLinks["twMainScreens"][0]+'">Ratusz</a></div>';
	...
	newHTML += '	<area id="MarketIcon" title="Rynek" shape="poly" alt="" coords="0,287, 9,281, 29,281, 33,277, 40,277, 44,281, 45,288, 41,294, 36,295, 36,303, 29,310, 6,311, 0,304" href="'+twLinks["twMainScreens"][7]+'" />';
	newHTML += '	<area id="SmithIcon" title="KuĹşnia" shape="poly" alt="" coords="0,251, 9,245, 29,245, 33,241, 40,241, 44,245, 45,252, 41,258, 36,259, 36,267, 29,274, 6,275, 0,268" href="'+twLinks["twMainScreens"][5]+'" />';
	newHTML += '	<area id="PlaceIcon" title="Plac" shape="poly" alt="" coords="0,214, 9,207, 27,207, 33,202, 41,204, 44,208, 45,214, 42,219, 36,222, 36,230, 29,236, 8,236, 0,231" href="'+twLinks["twMainScreens"][6]+'" />';
	newHTML += '	<area id="SnobIcon" title="Pałac" shape="poly" alt="" coords="0,176, 9,170, 29,170, 33,166, 40,166, 44,170, 45,177, 41,183, 36,184, 36,192, 29,199, 6,200, 0,193" href="'+twLinks["twMainScreens"][4]+'" />';
	newHTML += '	<area id="GarageIcon" title="Warsztat" shape="poly" alt="" coords="0,142, 7,135, 28,136, 32,132, 37,130, 42,133, 45,140, 42,147, 36,150, 36,158, 29,165, 7,165, 0,159" href="'+twLinks["twMainScreens"][3]+'" />';
	newHTML += '	<area id="StableIcon" title="Stajnia" shape="poly" alt="" coords="0,105, 7,98, 29,98, 33,93, 40,93, 44,99, 45,106, 41,112, 36,113, 36,121, 29,128, 7,127, 0,122" href="'+twLinks["twMainScreens"][2]+'" />';
	newHTML += '	<area id="BarracksIcon" title="Koszary" shape="poly" alt="" coords="0,67, 9,61, 29,61, 33,57, 40,57, 44,61, 45,68, 41,74, 36,75, 36,83, 29,90, 6,91, 0,84" href="'+twLinks["twMainScreens"][1]+'" />';
	newHTML += '	<area id="MainIcon" title="Ratusz" shape="poly" alt="" coords="0,30, 9,24, 29,24, 33,20, 40,20, 44,24, 45,31, 41,37, 36,38, 36,46, 29,53, 6,54, 0,47" href="'+twLinks["twMainScreens"][0]+'" />';
	newHTML += '</map>';
...
function twSetReport()
{
	...
	// ComprobaciĂłn: Informe reenviado (init = 5) o Informe propio (init = 3)
		...
		newHTML += '	</tr>';
		newHTML += '	<tr>';
		newHTML += '		<td><a id="twLinkReporter" href="javascript:void(0)" >&raquo; Konwerter</a></td>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
		newHTML += '		<td><a id="twLinkSimu" href="'+baseURL+'/staemme.php?screen=place&mode=sim'+url+'" >&raquo; Symulator</a></td>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
...
function TwSetVillageInfo()
{
	try{
		...
		newinfo_village += '			<tr><th>Informacje o graczu</th></tr>';
		newinfo_village += '			<tr><td>Współrzędne: <b>'+ CoordDestino +'</b></td></tr>';
		newinfo_village += '			<tr><td>P.: <b>'+ infoPlayer["Puntos"] +'</b</td></tr>';
		newinfo_village += '			<tr><td>Gracz: <a id="btnPlayerPopUp" href="javascript:void(0);">'+ infoPlayer["NombreJugador"] +'</a></td></tr>';
		newinfo_village += '			<tr><td>Plemię: '+ infoPlayer["Tribu"] +'</td></tr>';
		newinfo_village += '			<tr><th>Akcja</th></tr>';
		newinfo_village += '			<tr><td><a 	href= "'+twLinks["twMainMenuArray"]["map"]+'&x='+infoPlayer["CoordDestino"][0]+'&y='+infoPlayer["CoordDestino"][1]+'" >&raquo; scentruj mapę</a></td></tr>';
		...
		newinfo_village += '		<tbody>';
		newinfo_village += '			<tr><th>Jednostka</th><th>Czas</th></tr>';
		...
		newinfo_village += '		<tbody>';
		newinfo_village += '			<tr><th>Zaawansowane Informacje</th></tr>';
		newinfo_village += '			<tr><td><a href="javascript:alert(\'Deshabilitado.\');">&raquo; Historia wioski</a></td></tr>';
		newinfo_village += '			<tr><td><a href="javascript:alert(\'Deshabilitado.\');">&raquo; Zmiany plemienia</a></td></tr>';
		newinfo_village += '		</tbody>';
		newinfo_village += '	</table>';	
		newinfo_village += '</div>'; 
...
function twPlayerPopUp()
{
	...
	newInfo_village += '	<table>';
	newInfo_village += '		<tbody>';
	newInfo_village += '			<tr><th>Informacje o graczu </th><td><a id="btnPlayerPopUpClose" href="javascript:void(0);" >X</a></td></tr>';
	newInfo_village += '			<tr><td colspan=2 ><a href="'+twUrlScreen+'village='+vID+'&screen=info_player&id='+infoPlayer["JugadorID"]+'">Profil</a></td></tr>';
	newInfo_village += '			<tr><td colspan=2 ><a id="btnPlayerCurriculum" href="javascript:void(0)">Statystyki Gracza</a></td></tr>';
	newInfo_village += '			<tr><td colspan=2 ><a id="btnPlayerAllies" href="javascript:void(0)">Statystyki Plemienia</a></td></tr>';
	newInfo_village += '		</tbody>';
	newInfo_village += '	</table>';
	newInfo_village += '</div>'
...
function twPlayerCurriculum()
{
	...
	newInfo_village += '	<table>';
	newInfo_village += '		<tbody>';
	newInfo_village += '			<tr><th width=100%>Statystyki</th><td><a id="btnPlayerCurriculumClose" href="javascript:void(0);" >X</a></td></tr>';
	newInfo_village += '			<tr><td><div id="twPLayerCurriculum"><div id="loadingCurriculum">Wczytywanie...</div></div></td></tr>';	
	newInfo_village += '		</tbody>';
	newInfo_village += '	</table>';
	newInfo_village += '</div>'
...
function twPlayerInfoAllies()
{
	...
	newInfo_village += '	<table>';
	newInfo_village += '		<tbody>';
	newInfo_village += '			<tr><th width=100%>Statystyki Plemienia </th><td><a id="btnPlayerCurriculumClose" href="javascript:void(0);" >X</a></td></tr>';
	newInfo_village += '			<tr><td><div id="twPLayerCurriculum"><div id="loadingCurriculum">Wczytywanie...</div></div></td></tr>';	
	newInfo_village += '		</tbody>';
	newInfo_village += '	</table>';
	newInfo_village += '</div>'
