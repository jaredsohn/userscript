...
// @include        http://archive.foolz.us/a/thread/*
// @include        https://archive.foolz.us/a/thread/*
// ==/UserScript==
...
function get_chrome(uri, tag, id, callback)
{
	...
	xhr.onload = function(e)
	{
		if (this.status == 200)
		{
			callback(findOgg(this.response, tag, id), tag);
		}
	}
	...
	GM_xmlhttpRequest(
	{
		...
		onload: function(e)
		{
			if (e.status == 200)
			{
				...
				var dicks = s2ab(text);
				callback(findOgg(dicks, tag, id), tag);
			}
		},
		...
		while (repeat)
		{
			...
			for (var j = 0; j < p.childNodes.length; j++)
			{
				...
				link.addEventListener('click', function(e)
				{
					...
					try
					{
						players[id].pause(); //FJDSKLF;JASKFDL;A
						players[id] = null;
					}
					catch (ex)
					{
...
function player(audio) {
    ...
    */
    ...
    {
        var volume = Math.log(e.layerX) /        // Need to figure out whether it's
                Math.log(e.target.offsetWidth);  // already log scaled internally
                // Also, layerX/Y will fire a billion depreciation console messages 
                // in Chrome. This should go away in future releases because apparently 
                // they changed their mind. http://code.google.com/p/chromium/issues/detail?id=101733#c41
    ...
    volumeBTN.addEventListener('mousemove', function(e)
    {
        if (!mousedown)
        {
            slider.style.left = Math.exp(Math.log(volumeSlider.offsetWidth) 
                    * audioElement.volume) + 'px'; // We don't take kindly to 
        }                                          // linear amplitudes 'round here
...
var VOLUMESLIDERBG =
'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAAZCAIAAAB/8tMoAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAIGNIUk0AAHolAACAgwAA+f8AAIDpAAB1MAAA6mAAADqYAAAXb5JfxUYAAAAkSURBVHjafMcxEQAwEMMw2/y5hGIJ9H7QILalBqSeqvM/wBsA4D4ChiE+Tt4AAAAASUVORK5CYII=';
...
'.volumeBTN:hover .volumeSlider{background:url('+VOLUMESLIDERBG+') repeat-x;border-right:1px solid #9d9d9d;display:block;height:18px;left:20px;position:relative;width:60px;}    '+
