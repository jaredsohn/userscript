/*
	Miniattack: Edit version Assassin-a-Nator
	Script Source: http://userscripts.org/scripts/source/106956.user.js
	
	Credit: Spocklet

	Bookmarklet:
	javascript:%28function%28%29%7Bvar%20a%3Ddocument.createElement%28%22script%22%29%3Ba.type%3D%22text%2Fjavascript%22%3Ba.src%3D%22http://userscripts.org/scripts/source/106956.user.js%3F%22%2BMath.random%28%29%3Bdocument.getElementsByTagName%28%22head%22%29%5B0%5D.appendChild%28a%29%7D%29%28%29%3B
*/

// ==UserScript==
// @name           Miniattack 
// @author         Mafia Wars 
// @description    Mini Attacking
...
javascript:(function(){
	...
	//initial variables
	var version = 'Miniattack v17.7.2011',
	userid = User.id,
...
//				'<input id="'+spocklet+'_exp_level" type="text" value="0" maxlength="4" name="'+tagname+'" /> exp to level&nbsp;&nbsp;'+
	...
	function pausing(seconds,message,resume_func) {
		...
		var minutes = (parseInt(seconds/00) == 0) ? 0 : parseInt(seconds/0);
		if (minutes > 0) {
			msg(message+' <span id="minutes">'+minutes+' minutes</span> <span id="seconds">'+(seconds%0)+' second'+(seconds==0?'':'s')+'</span>...');
		}
		else {
			msg(message+' <span id="minutes"></span><span id="seconds">'+(seconds%0)+' second'+(seconds==0?'':'s')+'</span>...');
		}
		timer = setInterval(function(){
			...
			if (document.getElementById('minutes')) {
				document.getElementById('minutes').innerHTML = (minutes > 0) ? minutes+' minute'+(minutes==0?'':'s') : '';
			}
			if (document.getElementById('seconds')) {
				document.getElementById('seconds').innerHTML = (seconds % 0)+' second'+(seconds==0 ? '' : 's');
			}
			else {
				clearInterval(timer);
			}
	...
	*/
	...
	$('#'+spocklet+'_addtarget').click(function() {
		var m,famlink=prompt("100000741386371\np|97285271");
