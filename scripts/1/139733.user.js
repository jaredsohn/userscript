// ==UserScript==
// @name           KOC MadMaxx(Esp)para PowerKoc
// @version        0.1
// @namespace      Vazquez
// @homepage       https://userscripts.org/scripts/show/139733
...
// @icon           http://thorastur.webege.com/public_html/powerkoc/powerkoc.jpg
// @description    Organiza,mejora y borra Objetos del salon del trono en Koc
// ==/UserScript==
...
function trStartup (){
    ...
    logit ("Organizador del trono cargado");
...
function buildPresetWidget()
{
    ...
    var C = $("<ul/>", {
        id: "t_throneStatList",
        addClass: "presetList",
        style: "padding: 0; margin: 5px; list-style: none; overflow: hidden; border: 5px outset tan;",
    });
    $.each(E, function (K, L) {
        C.append(L)
    })
...
function attachTab()
{
    ...
    '<li id="throneStatTab" class="inactive"> Estado </li><li id="throneTest" class="inactive" onclick="hideShow()"> Controles </li><li id="trexecselect" class="inactive" onclick="execSelect()">Select</li><li id="trexecupgrade" class="inactive" onclick="execUpgrade()">Upgrade</li><li id="trexecsalvage" class="inactive" onclick="execSalvage()">Salvage</li><li id="trtimerdisp" class="inactive">Timer</li>');
...
function saveUpgradeData (){
    ...
    setTimeout (function (){GM_setValue ('UpgradeDataMM2_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(upgradeData));}, 0);
}

function saveSalvageData (){
    var serverID = getServerId();
    setTimeout (function (){GM_setValue ('SalvageDataMM2_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(salvageData));}, 0);
}

function saveUpgradeStats (){
    var serverID = getServerId();
    setTimeout (function (){GM_setValue ('UpgradeStats_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(upgradeStats));}, 0);
}
...
function readUpgradeData (){
    var serverID = getServerId();
    s = GM_getValue ('UpgradeDataMM2_' + Seed.player['name'] + '_' +serverID);
    if (s != null){
        opts = JSON2.parse (s);
        for (k in opts){
...
function readSalvageData (){
    var serverID = getServerId();
    s = GM_getValue ('SalvageDataMM2_' + Seed.player['name'] + '_' +serverID);
    if (s != null){
        opts = JSON2.parse (s);
        for (k in opts){
            if (matTypeof(opts[k]) == 'object')
...
function readUpgradeStats (){
    var serverID = getServerId();
    s = GM_getValue ('UpgradeStats_' + Seed.player['name'] + '_' +serverID);
    if (s != null){
        opts = JSON2.parse (s);
        for (k in opts){
            if (matTypeof(opts[k]) == 'object')
...
Tabs.throneSalvage = {
        ...
        init : function (div) {
            ...
            t.myDiv = div;
            var m = '<Div><DIV id=trSalvage class=trStat>ELIMINACION AUTOMATICA</div>';
            m += "<div id='trInfoArea'>";
            m += '</div>';
            ...
            if (salvageData.salvageActive == false) {
                m += '<tr><TD><div><INPUT id=trSalvagerPower type=submit value="Eliminador = OFF"/></div></td>';
            } else {
                m += '<tr><TD><div><INPUT id=trSalvagerPower type=submit value="Eliminador = ON"/></div></td>';
            }
            m += '<td><b> Ciudad para dejar los aetherstones: </b></td>';
            m += "<td><div id='trSalvageCity'></td><td></td></tr>";

            m += '<tr><td><div> Conservar : <select id="trSalvageQuality">';
            ...
            m += '</select>O mejor</div></td>';

            m += '<td>Dejar los primeros <INPUT id=trSaveNum type=text size=3 maxlength=3 value="' + salvageData.throneSaveNum+ '"/> objetos</td>';
            m += '<td><div><INPUT id=trSalAnyCity type=checkbox '+ (salvageData.anyCity?' CHECKED':'') +'/> Cuando este llena, poner el aether en otra ciudad</div></td>';
            ...
            m += '<tr><td>  <b>Define los TR objetos para conservar: </b> </td>';

            m += '<td alight="left"><div><span>Faccion: <select id="trFactionType">';
            m += '  <option value="any">Cualquiera</option>';
            m += '  <option value="fey">Fey</option>';
            m += '  <option value="briton">Briton</option>';
            m += '  <option value="druid">Druid</option>';
            m += '</select></span></div></td>';
            m += '<td alight="left"><div><span>Tipo de Carta: <select id="trCardType">';
            m += '  <option value="any">Cualquiera</option>';
            ...
            m += '</select></span></div></td>';
            m += '<td align="right"><INPUT id=trAddRule type=submit value="Crear Regla"/></td>';
            m += '</tr></table>';
            m += '<TABLE  class=trTabPad width=100% id="trConditionTable"  style="padding-left: 5px;">';
            m += '<tr><td align=left colspan=1><INPUT id=trAddRow type=submit value="Agregar Linea"/></td>';
            m += '<td></td><td></td><td></td><td></td><td></td></tr>';
            m += '</table>';
            m += '</div><hr/>';
            m += '<div id="trSalvStatus" style="text-align: center;" >Cargando ... </div><hr/>';
            m += '<div class=trRulePane>';
            m += '<div align=center> <b> El eliminador conservara objetos que coincidan con alguna de estas reglas: </b></div><hr/>';
        ...
        createRow : function()
        {
            ...
            var h  = "<td> <select id='" + rowId + "sel1'> <option value='true'> </option> <option value='false'>NO</option></select></td>";
        ...
        buildRuleDisplay : function ()
        {
            ...
            for (i =0; i < salvageData.ruleSet.length; i++)
            {
                ...
                m += "</div></td>";
                m += "<td width=20%><INPUT id=trDelRule" + i + " type=submit value='Eliminar Regla' /></td>";
                m += '</tr>';
            }
        ...
        doSalvage : function() {
            ...
            if (t.delItems.length > 0) {
                ...
                t.deleting = false;
                t.setStatus('No se han eliminado objetos. Esperando para el siguiente ciclo.');
            }
        },
        ...
        doDelete : function(){
            ...
            t.setStatus('Eliminando ' + unsafeWindow.kocThroneItems[id].name);
            ...
            new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                ...
                onSuccess: function (transport) {
                    ...
                    if(rslt.ok) {
                        trSalvageLog('Eliminado objeto del trono '+ throne_item.name);
                    ...
                    {
                        logit("Fallo de borrado");
                        logit("rslt: " + inspect(rslt,3,1));
                        t.setStatus('No se puede borrar el objeto ' + throne_item.name);
                    ...
                    if (t.delItems.length > 0) { // Check if the array is
                        ...
                        t.deleting = false;
                        t.setStatus('Eliminacion completa. Esperando para el siguiente ciclo.');
                        return;
                    }
                },
                onFailure: function () {
                    t.delIems = [];
                    t.deleting = false;
                    logit("fallo al eliminar objeto " + unsafeWindow.kocThroneItems[id].name );
                    unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
                    return;
                },
            });
...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            // setup the tab
            var m = '<Div><DIV id=trOrganizer class=trStat style="width: 720px;">ORGANIZADOR</div>';
            ...
            m += '<option value="0">--Configuraciones--</option>';

            for (k= 1; k <= Seed.throne.slotNum; k++)
            {
                m += '<option value="'+k+'"> Configuración:  '+ k +'</option>';
            }
            for (k= 1; k <= 10; k++)
            {
            ...
            m += '</tr>';
            m += '<tr align=center><td><div><span>Ordenar: <select id="trSortList">';

            m += '<option value="none">--Efectos--</option>';
            for (k in effects)
            {
            ...
            m += "  <option value='both'>Ambos</option>";
            m += "  <option value='buff'>Buff</option>";
            m += "  <option value='debuff'>Debuff</option>";
            m += "</select></td>";

            m += '<td><INPUT id=trSortInactive type=checkbox '+ (  t.sortInactive ?' CHECKED':'') +'/> Incluye Inactivos</td>';
            m +='</tr></table>';
        ...
        testSalvage : function() {
            ...
            $('#trDelResults').html("<div> " + toDelete.length + " objetos marcados para borrar</div>");
...
Tabs.trActionLog = {
        ...
        init : function (div){
            ...
            t.myDiv = div;            
            var m = '<DIV class=trStat>ORGANIZADOR LOG - VERSION: '+ Version+'</div><DIV style="height:535px; max-height:535px; overflow-y:auto">';
            m += '<TABLE cellpadding=0 cellspacing=0 id=trsuccesslog class=trTabLined><TR><TD></td><TD width=95%></td></table>';
            m += '<DIV class=trStat>Action Log</div>';
            m += '<TABLE cellpadding=0 cellspacing=0 id=tractionlog class=trTabLined><TR><TD></td><TD width=95%></td></table>';
            m += '<DIV class=trStat>Salvage Log</div>';
            m += '<TABLE cellpadding=0 cellspacing=0 id=trsalvagelog class=trTabLined><TR><TD></td><TD width=95%></td></table></div>';
...
Tabs.upgrader = {
        ...
        init : function (div){
            ...
            // header stuff
            var m = '<Div id=trfield><DIV id=trUpgrader class=trStat>Actualizador Automatico</div><TABLE id=trupgrader width=100% height=0% class=trTab>';
            m   += '<TR>';
            if (upgradeData.active == false) {
                m += '<TD><div class=indent25><INPUT id=trUpgraderPower type=submit value="Actualizador = OFF"/></div></td>';
            } else {
                m += '<TD><div class=indent25><INPUT id=trUpgraderPower type=submit value="Actualizador = ON"/></div></td>';
            }
            ...
            m += '<td>Intervalo de reintentos (segundos): <INPUT id=trUpRefresh type=text size=3 maxlength=3 value="' +upgradeData.retryInterval+ '"/></td>';
            m += '</tr>';
            ...
            m += '<option value="0">--Objetos--</option>';
            for (k in unsafeWindow.kocThroneItems) {
                var throne_item = unsafeWindow.kocThroneItems[k];
            ...
            m += '</select></span></div></td>';
            m += '<td><div> Maxima calidad: <select id="trMaxQuality">';
            m += '<option value="1">Common</option>';
            m += '<option value="2">Uncommon</option>';
            m += '<option value="3">Rare</option>';
            m += '<option value="4">Epic</option>';
            ...
            m += '<tr><TD><INPUT id=trSwithUpgrade type=checkbox '+ (upgradeData.switchToUpgrade?' CHECKED':'') +'/>  Cambiar de enchance a upgrade cuando se complete</td>';
            m += '<td><INPUT id=trRepairAll type=checkbox '+ (upgradeData.repairAll?' CHECKED':'') +'/>  Reparar todas las cartas antes</td></tr>';
            m += '<tr><td colspan=3><hr></td></tr>';
            m += '  <tr><td colspan=2><div ><b>Ciudad para usar aetherstones desde: </b><span id="trUpgradeCity"></span></div></td>';
            m += '  <td><div id=trStoneRemain></div></td></td>';
            m += '  <tr><td><div><INPUT id=trAnyCity type=checkbox '+ (upgradeData.anyCity?' CHECKED':'') +'/> Si hay poco usa de cualquier ciudad</div></td></tr>';
            ...
            if (Seed.queue_throne != null && Seed.queue_throne.end != null)
            {
                ...
                t.setStatus("Esperando hasta las " + n.toLocaleTimeString() + " para la reparación completa");
        ...
        updateTRSelect : function() {
            if (upgradeData.enhanceAction == false)
            {
                $("#trexecselect").html("Actualizando")
                .css("color", "blue");
            }
            else
            {
                $("#trexecselect").html("Activando")
                .css("color", "yellow");
            }
        },
        ...
        togglePower: function(obj){
            ...
            if (upgradeData.active == true) {
                ...
                btn.value = "Actualizador = OFF";
                t.setStatus("Apagado");
            } else {
                upgradeData.active = true;
                btn.value = "Actualizador = ON";
                t.setStatus("Encendido");
            }
        ...
        doAction :function ()
        {
            ...
            try {
                ...
                if ( ti <= 0)
                {
                    ...
                    retryTime = ti + 5;
                    var n = new Date(t.repairEnd *1000);
                    t.setStatus("Acaba a las " + n.toLocaleTimeString() + " de reparar completamente");
                }
                unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
            } catch(e) {
        ...
        doEnhance : function() {
            ...
            try {
                ...
                if ( y.quality >= upgradeData.enhanceMax)
                {
                    ...
                    trActionLog('El objeto del trono '+unsafeWindow.kocThroneItems[eItem].name + ' ya tiene esta calidad ' + y.quality);
                ...
                if ( num_city < 0)
                {
                    t.setStatus("No hay suficiente aether para enhance.  Minimo 25 k. Esperando por mas.....");
                    return;
                }
                ...
                new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                    ...
                    onSuccess: function (transport) {
                        ...
                        if(rslt.ok){
                            ...
                            if (rslt.success)
                            {
                                trSuccessLog('Mejorado el objeto '+unsafeWindow.kocThroneItems[eItem].name + ' a la calidad ' + rslt.item.quality);
                                ...
                                y.name = y.createName();
                                t.repaint();
                                t.setResult("Enhance exitoso.  "  + addCommas(rslt.aetherstones) + " aetherstones usados.");
                                t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                                t.setStatus("Intentando siguiente enhancement");
                            ...
                            else
                            {
                                ...
                                y.name = y.createName();
                                t.setResult("Enhance fallo.  "  + addCommas(rslt.aetherstones) + " aetherstones usados");
                                t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                                t.setStatus("Empezando reparación ...");
                                t.doRepair(y.id);
                            }
                            if (rslt.gems > 0)
                            {
                                trActionLog('Mejorador gasto gemas accidentalmente!  Apagando mejorador off');
                        ...
                        else
                        {
                            ...
                            t.setStatus("No se puede mejorar en este momento ... esperando al proximo intento");
                        }
                        return;
                    },
                    onFailure: function () {
                        logit("Enhance error.  Esperando proximo intento.");
                        t.setStatus("No se puede enviar petición de enhance.  Esperando al proximo intento");
                        return;
                    },
                });
            } catch (e) {
        ...
        doUpgrade : function() {
            ...
            if ( num_city < 0)
            {
                t.setStatus("No hay suficiente aether para upgrade.  Minimo 100k se necesita.  Esperando por mas ...");
                return;
            }
            ...
            new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                ...
                onSuccess: function (transport) {
                    ...
                    if(rslt.ok){
                        ...
                        if (rslt.success)
                        {
                            trSuccessLog('Subido el objeto '+unsafeWindow.kocThroneItems[uItem].name + ' a nivel ' + rslt.item.level);
                            ...
                            y.name = y.createName();
                            t.repaint();
                            t.setResult("Upgrade exitoso!!.  "  + addCommas(rslt.aetherstones) + " ether usado.");
                            t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                            t.setStatus("Empezando nuevo intento");
                            // update the cost line
                            t.setUpgradeItem(upgradeData.item);
                            clearTimeout(t.timerH);
                            t.doAction();
                        ...
                        else
                        {
                            ...
                            y.name = y.createName();
                            unsafeWindow.cm.HeatUpModel.attemptCallback(+(rslt.heatupModifier));
                            t.setResult("Upgrade fallo.  "  + addCommas(rslt.aetherstones) + " aether usado");
                            t.setStones(Seed.resources["city" + Seed.cities[num_city][0]]["rec5"][0]);
                            t.setStatus("Empezando reparación ... ");
                        ...
                        if (rslt.gems > 0)
                        {
                            t.setStatus("Error .... apagando.");
                            trActionLog('Upgrader gasto gemas accidentalmente!  Apagando upgrader');
                            upgradeData.active = false;
                            saveUpgradeData();
                        }
                        return;
                    ...
                    {
                        t.setStatus("Peticion de Upgrade no aceptada.  Esperando a nuevo intento.");
                        logit("Upgrade result:" + inspect (rslt, 3, 1));
                    }
                    return;
                },
                onFailure: function () {
                    t.setStatus("No se puede transmitir la orden de upgrade.  Esperando a nuevo intento.");
                    unsafeWindow.cm.ThroneView.renderInventory(unsafeWindow.kocThroneItems);
                    return;
                },
            });
        ...
        doRepair : function( rItem) {
            ...
            new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                ...
                onSuccess: function (transport) {
                    ...
                    // logit("rslt: " + inspect(rslt,3,1));
                    if(rslt.ok){
                        trActionLog('Empezando reparacion para el objeto '+unsafeWindow.kocThroneItems[rItem].name);
                        ...
                        var n = new Date(t.repairEnd *1000);
                        t.setStatus("Reparacion empezando ... Reparacion acabara a las " + n.toLocaleTimeString());
                    ...
                    {
                        if (rslt.msg == "El objeto no esta roto")
                        {
                            t.setStatus("Objeto esta reparado.");
                            t.repairId = rItem;
                            t.repairEnd = 0;
                            t.clearRepair();
                        }
        ...
        clearRepair : function () {
            ...
            if (timeUntilDone <= 0)
            {
                ...
                if (t.repairId != 0 && unsafeWindow.kocThroneItems[t.repairId]  != null)
                {
                    ...
                    {
                        t.setStatus("Reparacion completada.");
                    }
...
Tabs.estats = {
        ...
        buildDisplay :function()   {
            var t = Tabs.estats;
            var m = '<DIV class=trstat style="margin-top:5px; margin-bottom:5px;"><DIV id=trStatsMain class=trStat>ESTADISTICAS DE MEJORAS</div>';
            ...
            m += '<TR valign=top align=center><TH colspan=6>Estadisticas de mejoras  (Exitos/fallos)</TH></TR>';
            ...
            m += '<TR valign=top align=center><th> Totales: </th>';
            for( q =0; q <5; q++)
            {
            ...
            m += '<TR valign=top align=center><th> Por 100%: </th>';
            for( q =0; q <5; q++)
            {
                m += "<td style='font-weight: bold;' class='td" + (q%2) +"'  >";
                if ( (st[q] + ft[q]) == 0 )
...
Tabs.ustats = {
        ...
        {
            var t = Tabs.ustats;
            var m = '<DIV class=trstat style="margin-top:5px; margin-bottom:5px;"><DIV id=trStatsMain2 class=trStat>SUBIR DE NIVEL ESTADISTICAS</div>';
            m += '<DIV id= trStatsTabDiv2 align=left style="margin-top:10px; margin-bottom:10px; margin-left: 30px;">';
            m += '<TABLE class=trStatTab align=center cellspacing=0>';

            m += '<TR valign=top align=center><TH colspan=12>ESTADISTICAS UPGRADE  (Exitos/fallos)</TH></TR>';
...
function trPopup (prefix, x, y, width, height, enableDrag, onClose) {
    ...
    m += '<TR vAlign=top id="'+ prefix +'_bar" class="'+ topClass +'"><td><div class="trTitle">Organizador Salon del Trono</div></td><TD><div id='+ prefix +'_X align=right valign=top onmouseover="this.style.cursor=\'pointer\'" class=trCloseSpan> </span></td></tr>';
    ...
    var build = function(){
        ...
        if(meta.depricated == null){
            ...
            var a_span = document.createElement("span");
                a_span.appendChild(document.createTextNode("Esperar a proxima actualizacion!"));
            a_hide.appendChild(a_span);
            hide.appendChild(a_hide);
        }
        var h1 = document.createElement("h1");
            h1.appendChild(hide);
            h1.appendChild(document.createTextNode(meta.depricated == null ? "Greasemonkey UserScript aviso de actualizacion!" : "Desactualizado Greasemonkey UserScript!"));
        updater.appendChild(h1);
        var p = document.createElement("p");
        if(meta.depricated == null){
            var text = "Hay una actualizacion disponible para <a href=\"http://userscripts.org/scripts/show/" + config.scriptId + "\">" + meta.name + "</a>.<br>";
                text += meta.change ? "<br>" + meta.change + "<br><br>" : "";
                text += "Actualmente estas usando la version <b>" + config.currentVersion + "</b>, la nueva version en userscripts.org es <b>" + meta.version + "</b>!<br><a href=\"http://userscripts.org/scripts/source/" + config.scriptId + ".user.js\">Actualizar a version " + meta.version + "</a>";
        } else {
            var text = "El userscript <a href=\"http://userscripts.org/scripts/show/" + config.scriptId + "\">" + meta.name + "</a> esta desactualizado.<br>";
                text += meta.depricated && meta.depricated != "true" ? "<br>" + meta.depricated + "<br><br>" : "";
                text += "Porfavor actualiza el script! Gracias por usarlo.";
        }
        p.innerHTML = text;
...
userscriptUpdater.init({
    scriptId:       "139733", // insert id of your userscript from userscripts.org!
    currentVersion: "0.1"        // insert current version number based on versioning scheme: major.minor[.bugfix]
});
