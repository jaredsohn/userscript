...
// @namespace      Takato
// @author         Takato
// @description    Settings manager for my scripts. A copy of PhasmaExMachina's script, with modifications by me.
// @version        2012.01.02
// @exclude        *
// ==/UserScript==

var ExternalScriptImported = true;
	...
	show:function(callback) {
		...
		if(typeof(Config.styleDrawn) == 'undefined') {				// apply styling
			GM_addStyle("\
					...
					#ConfigBody { width:800px; margin:auto !important; margin-left:-400px !important; top:30px; position:fixed; left:50%; text-align:left; background:#f9f9f9; border:1px outset #333; padding:0 !important; font-family:Arial; font-size:14px; cursor:default; z-index:9010; color:#333; padding-bottom:1em !important; } \
					...
					#ConfigTabs span { border:1px solid #666; padding: 2px 10px !important; position:relative; top:-2px; background-color:#ddd; cursor:pointer; }\
					...
					#ConfigFooter input { border:1px outset #666; padding:3px 5px 5px 20px !important; background:no-repeat 4px center #eee; cursor:pointer; width:80px; float:right; margin-left:.5em !important; } \
			...
			switch(fields[fieldName].type) {
				case 'checkbox': 
					Config.$('configInput_' + fieldName).addEventListener('change', function() {
						Config.tempOptions[this.id.toString().match(/configInput_(.+)$/)[1]] = this.checked ? true : false;															   
					}, false);
					break;
				case 'select': 
					Config.$('configInput_' + fieldName).addEventListener('change', function() {
			...
			switch(field.type) {
				case 'checkbox':
					if(typeof(field.value) == 'undefined' || !field.value)
						return (typeof(GM_getValue(key)) != 'undefined' && GM_getValue(key).toString() == "true") ? true : false;	// false by default
					else 
						return (typeof(GM_getValue(key)) != 'undefined' && GM_getValue(key).toString() == "false") ? false : true;	// true by default
					break;
				case 'select':
				case 'password':
				case 'text':
