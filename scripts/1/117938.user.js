// ==UserScript==
// @name         AKFB MafiaWars Addon
// @namespace    AK MAFIA WARS ADDON
// @version      0.5.3.0
...
var AppInfo = {
    ...
    name     : 'AKFB MafiaWars Addon',
    tag      : 'AKMWAddon',
    prefix   : 'AKFBMafiaWarsAddon_',
    url      : 'http://userscripts.org/scripts/show/117938',
    meta     : 'http://userscripts.org/scripts/source/117938.meta.js'
};
...
var MW = {
     ...
     * @param {Function} callback
     */
    getGift: function(giftId, giftCat, callback) {
        ...
        httpAjaxRequest({
            ...
            success  : function(htmlText) {
                ...
                if (req) {    
                    data = Util.parseJSON(unescape(req));
                }
...
var facebook = {
    ...
    sendRequest: function(args, callback, bOnlyInternally) {
        ...
        if (bOnlyInternally !== true) {
            ...
            postSend({request: (new Date()).getTime()/1000});
        }
        var retries = 3;
        function postSend(response) {
            ...
            url += '&data=' + data;
            url += '&to=' + args.to;
            httpAjaxRequest({'url': url, 'success': function(r) {
                if (r !== 'timeout') {
                    callback(r);
                } else if (retries > 0){
                    retries--;
...
function showPromptPopup(message, defaultText, callback, height) {
    ...
    var $in = c$('textarea').css({'width':600,'height':(height|100)}).val(defaultText||'');
...
function showTextPopup(message, text) {
    ...
    var $in = c$('textarea', 'readonly:readonly,rows:12,cols:72').val(text);
    var Popup = new PopupObject({
        type     : 'simple',
        zIndex   : 99999,
        buttons  : [{
    ...
    c$('div').css('margin-top', 10).append(c$('h4').css('margin', 0).text(message))
...
var UserFreeGifts = {
    content: {
        1000 : { name:'Union Worker'        , img:'huge_item_union_worker_01.png'            , title:'CHICAGO'     },
        1001 : { name:'Carpenter Nails'     , img:'huge_item_carpenter_nails_01.png'         , title:'CHICAGO'     },
        1002 : { name:'Lath Strips'         , img:'huge_item_lath_strips_01.png'             , title:'CHICAGO'     },
        1003 : { name:'Iron Cast'           , img:'huge_item_iron_cast_01.png'               , title:'CHICAGO'     },
        1004 : { name:'Douglas Fir Beams'   , img:'huge_item_douglas_fir_beams_01.png'       , title:'CHICAGO'     },
        86   : { name:'Red Mystery Bag'     , img:'item_mysterybag_red.png'                  , title:'MYSTERY'     },
        400  : { name:'Secret Drop'         , img:'icon_atk_75x75_01.png'                    , title:'MYSTERY'     },
        401  : { name:'Italian Hardwood'    , img:'huge_item_italianhardwood_01.png'         , title:'ITALY'       },
        402  : { name:'Marble Slab'         , img:'huge_item_marbleslab_01.png'              , title:'ITALY'       },
        422  : { name:'Exotic Animal Feed'  , img:'huge_item_exoticanimalfeed_01.png'        , title:'ITEM PART'   },
        444  : { name:'Set of Hidden Charges', img:'huge_item_hiddencharges_01.png'          , title:'CONSUMABLES' },
        445  : { name:'Cooked Book'         , img:'huge_item_cookedbook_01.png'              , title:'CONSUMABLES' },
        189  : { name:'Special Part'        , img:'huge_item_parts_01.png'                   , title:'ITEM PART'   },
...
function Battlefieldv2()
{
    ...
    function genMainDom() {
        ...
        .append(x$('bfopt_skipicedbyme', 'Skip targets that i iced.'))
        .append(x$('bfopt_skiphealed', 'Skip target that is Healed.'));
        ...
        c$('div').css({'float':'left','text-align':'left','margin':5,'width':500}).appendTo(divListElt)
        .append(c$('div').text('Use "Skip Name" filter in Second page to add friendly families tags.'))
        .append(c$('div').text('Add your Enemy families to attack here:'))
        .append(c$('div').css({'clear':'both','margin-top':5}))
        .append(c$('select', 'id:bfopt_clanlist,multiple:multiple').width(500).height(260))
    ...
    function getFamilyOpponents(callback) {
        var tempList = PlayerList.clanList.toArray();
    ...
    function isValidOpponent(opp) {
        ...
        if (options.levelRangeActive === true) {
            if (opp.level >= options.levelRangeMin && opp.level <= options.levelRangeMax) {
                isInRange = (options.levelRangeMethod === 'attk');
            } else {
                isInRange = (options.levelRangeMethod !== 'attk');
            }
        }
        if (options.mafiaRangeActive === true) {
            if (opp.mafia >= options.mafiaRangeMin && opp.mafia <= options.mafiaRangeMax) {
                isInRange = (options.mafiaRangeMethod === 'attk');
            } else {
                isInRange = (options.levelRangeActive === (isInRange === true)) || (options.mafiaRangeMethod !== 'attk');
            }
        }
        if (isInRange !== true) {
            Logger.debug('skipping "'+opp.name+'" because it\'s out of level/mafia range.'
            +            'level: '+opp.level+', mafia:'+opp.mafia);
            return false;
        }
        return true;
    }
    ...
    function preAttack() {
        ...
        if (options.attackPwr && opp.pwratk) {
            ...
            if (options.rapidFireActive && n_forIce > 4 && parseInt(n_forIce/5) <= options.rapidFireAttacks) {
        ...
        if ( fs.expToNextLevel < options.stopKeepExp ) {
            ...
            else if (options.healAttaking || opp.fights === 0) {
                healPlayer(preAttack);
                return;
            }
    ...
    function AttackWithRapidFire( url ) {
        ...
        var levelUp_stop = false, b_AttackAgain = true;
        ...
        sendNewRFAttack();
        options.rapidFireTiming = Math.max(options.rapidFireTiming, 200);
        ...
        function rapidFireResponse(response) {
            ...
            if ( resp.success !== ERROR_SUCCESS ) {
                ...
                if ( total_sent > 9 && parseInt((n_badResponses*100)/total_sent) > 80 ) {
                    stopRapidFire();
                }
            }
            else {
                ...
                if (bDoManualHeal || b_AttackAgain !== true || n_Health<n_minHealth || n_Stamina<n_minStamina) {
                    stopRapidFire();
                }
            }
            
            // completed
            if (requests_left < 1 && b_Stopped) {
                if (options.rapidFireAutoTiming === true && total_sent > 9) {
                    if (n_badResponses > n_goodResponses) {
                        options.rapidFireTiming += 50;
                    }
                    else if (parseInt((n_goodResponses*100)/total_sent) > 85) {
                        options.rapidFireTiming -= 50;
                    }
                }
                ...
                else if (b_AttackAgain && (total_sent < 5 || n_goodResponses > 0)) {
                    preAttack();
                }
                else {
                    addAutoControls();
    ...
    function parseAttack(htmlText, autoMode) {
        ...
        if (fight_result.isWin)
        {
            ...
            if (b_goodCash && options.addUserToWList) {
                if (n_cash >= options.cashNeedToGain) {
...
function FreeGiftsCenter() {
    ...
    var CSCollector = function() {
         ...
         */
        this.each = function(callback) {
            Util.each(reqs, function(id, amount) {
                callback(UserFreeGifts.get(id), amount);
            });
        };
    ...
    var Events = {
        ...
        editFriends_click: function() {
            ...
            var $ul = $('#send_body .snd_users');
            $ul.find('li').each(function(i, e) {
                added_users.push($(e).attr('uid'));
            });
            showFriendsSelector(function(users) {
                if (users.length > 0) {
                    $ul.empty();
                    Util.each(users, function(i, user) {
                        c$('li','uid:'+user.uid).text(user.name).appendTo($ul);
                    });
                }
            }, added_users);
            return false;
        ...
        selectGift_click: function() {
            if (!$('#send_body .snd_selectable').hasClass('disabled')) {
                $('#send_body .snd_selectable li').removeClass('selected');
                $(this).addClass('selected');
            }
            return false;
        },
        ...
        addGift_click: function() {
            var $b = $(this);
            var gift_id = $b.attr('gid');
            var gift_name = UserFreeGifts.get(gift_id).name;
            $b.hide();
            
            c$('li','gid:'+gift_id).appendTo('#mass_body .mss_selected')
            .append($b.find('img').clone())
            .append(c$('div', 'class:gift_name').text(gift_name))
            .append(c$('div', 'class:quantity').append(n$(gift_id+'_quantity', 'Quantity:', 30)));
            
            return false;
        },
        clearCollector_click: function() {
            $('#mass_body .mss_selectable li').show();
        ...
        startCollector_click: function() {
            ...
            if (!Util.isSet(Collector)) {
                Collector = new CSCollector(); 
            } else {
                Collector.clear();
            }
            $('#mass_body .mss_selected li').each(function(index, elem) {
                var count = $('input', elem).val();
                if (count && (count=parseInt(count)) > 0 ) {
                    Collector.add($(elem).attr('gid'), count);
                }
            });
        ...
        accept_click: function() {
            ...
            CollectRequest(req, options.sendBack, function(data) {
     ...
     * @param {String} s_list
     */
    function addFriends(s_list) {
        if (!Util.isSet(FriendLists['all'])) {
            return;
        }        
        if (Util.isSet(FriendLists[s_list])) {
            var $ul = $('#send_body .snd_users');
            Util.each(FriendLists[s_list], function(i, uid) {
                uid = parseInt(uid);
                if (!isNaN(uid) && Util.isSet(FriendLists['all'][uid])) {
                    ( e$('li[uid='+uid+']', $ul) ||
                      c$('li','uid:'+uid).appendTo($ul) ).text(FriendLists['all'][uid]);
                }
            });
    ...
    function loadFriendLists(callback) {
        if ($('#friendlists').hasClass('loading')) {
            return;
        } else {
            $('#friendlists').addClass('loading');
        }
        $('#snd_messages').empty()
        ...
        var active = (FriendLists['active'] = new Array());
        ...
        if (!Util.isSet(FriendLists['all'])) {
            facebook.getAppFriends(function(users) {
                ...
                if (users.error) {
                    $('#snd_messages').html('ERROR: '+users.error.message);
                    $('#friendlists').removeClass('loading');
                } else {
                ...
                'success': function(htmlText) {
                    ...
                    if (ol) {
                        ...
                        Util.each(ol, function(id, uids) {
                            var name = $j.find('#mfs_tab_'+id).text();
                            var $op = $('option[value='+id+']', '#friendlists');
                            FriendLists[id] = uids;
                            if ($op.length < 1) {
                                $('#friendlists').append(c$('option','value:'+id).text(name));
                            }
                        });
                        $('#snd_messages').empty()
                        .append(c$('span').text('Friend Lists Loaded.'));
                    }
                    $('#friendlists').removeClass('loading');
                },
                error: function(msg) {
                    $('#snd_messages').html('ERROR: '+msg);
                    $('#friendlists').removeClass('loading');
                }
            });
        }
                    ...
                    if (bSendBack === true && req.send) {
                        facebook.sendRequest(req.send, function(r) {
                            if (r && r.data && r.data.reqsentmsg) {
                                sendBackCallback($(r.data.reqsentmsg).find('div:first').html());
                            } else {
                                var error_msg = r.error ? r.error.message : 'Error sending request.';
                                sendBackCallback(Util.setColor(error_msg,'red'));
                            }
    ...
    function addActiveFriend(uid) {
        if (!isNaN(uid = parseInt(uid))) {
            Logger.debug('addActiveFriend:'+ Util.isNumber(options.activeFriends[uid]));
    ...
    function sendRequests() {
        ...
        var gift_id = $('.snd_selectable .selected').attr('gid');
        var gift = UserFreeGifts.get(gift_id);
        
        $('#send_body .start_ctrl a, #friendlists, .snd_selectable, .tab_bar', Popup.content).addClass('disabled');
        $('#snd_messages').empty();
        
        if (!gift_id || !gift) {
            addMessageLog('Select a gift to send.');
            finish();
            return;
        }
        
        $('#send_body .snd_users li').each(function(index, elem) {
            toIds.push(parseInt($(elem).attr('uid')));
        });
        ...
        while (toIds.length > 0) {
            toSend.push(toIds.splice(0,50));
        }
        
        addLoadingMessage(gift.name);        
        ...
        function startSending(send) {
            var n_total = toSend.length;
            $('#snd_messages').empty();
            addMessageLog('Total Requests To Send: '+n_total);
            function nextSend() {
                if (toSend.length < 1) {
                    finish();
                    return;
                ...
                addMessageLog('Sending request #'+(n_total-toSend.length));
                facebook.sendRequest(send, function(r) {
                    if (r && r.data && r.data.reqsentmsg) {
                        addMessageLog($(r.data.reqsentmsg).find('div:first').html());
                        nextSend();
                    } else {
                        showAskPopup('Error Sending Requests', 'Do you want to continue?', nextSend, finish);
                    }
        ...
        function addLoadingMessage(message) {
            $('#snd_messages').empty()
            .append(c$('img').attr('src', global.resource.ajax_loader))
            .append(c$('span').text('Loading '+message+'...'));
        }
        /**
         * @param {String} message
         */
    ...
    function StartCollector() {
        ...
        var p_links, c_gift = new Collection(Collector.getAll());
        var giftRequests = Requests.toArray('gift');
        ...
        c_gift.onMove(function(i1, gift_id, amount) {
            ...
            var gift = UserFreeGifts.get(gift_id);
            setGiftName(Util.setColor(gift.name, 'green')+':');
            
            if ( !Util.isSet(gift.item_id)||(!Util.isSet(sendBack[gift_id])&&options.sendBack) ) {
                sendMessage('Loading, wait a moment please...');
                MW.getGift(gift_id, gift.item_cat, function(id, cat, data, msg, title) {
                    if (id && cat) {
                        sendBack[gift_id] = {
                            to      : new Array(),
                        ...
                        gift.item_id = id;
                        gift.item_cat = cat;
                        collect(giftRequests.shift());
                    } else {
                        sendMessage('Wait a moment please...');
                        addMessageLog('Error loading "'+gift.name+'" data, going to next gift...', 'error');
                        c_gift.MoveNext();
                    }
                });
            } else {
            ...
            function collect(req) {
                ...
                if (options.ignoreLimits !== true && excluded.is(gift.item_id)) {
                    addMessageLog('"'+gift.name+'" skipped. Limits are reached.');
                    giftRequests.push(req);
                    c_gift.MoveNext();
                    return;
                }
                sendMessage('Collecting from ' + req.name);
                req.url = req.url.replace(/item_id=\d+/, 'item_id='+gift.item_id);
                req.url = req.url.replace(/item_cat=\d+/, 'item_cat='+gift.item_cat);
                
                CollectRequest(req, false, function(data){
                    Requests.remove(req.id);
                    if (bCancelled||cancel_process) {
                        return;
                    }
                    if (req.send && req.send.to && options.sendBack === true) {
                        sendBack[gift_id].to.push(String(req.send.to[0]));
                    }
                    if (data.success) {
                        n_Success++;
                        ...
                        addCollectLog(gift, req, data);
                    }
                    else {
                        addMessageLog(data.message, 'error', req);
                    }
                    if (isLimitReached(data.message) && !excluded.is(gift.item_id)) {
                        excluded.add(gift.item_id);
                        addMessageLog('Limit reached. "' + gift.name + '" added to exclude list.');
                    }
                    postCollect();
                    
                }, function(message) {
                    addMessageLog(message, 'info', req);
                });
            }
            function postCollect() {
                sendMessage('Wait a moment please...');
                if (amount > 0 && giftRequests.length > 0) {
                    collect(giftRequests.shift());
                } else {
                    setGiftName('');
                    c_gift.MoveNext();
         ...
         * @param {Object} data
         */
        function addCollectLog(gift, req, data) {
            var gift_image = data.image || global.zGraphicsURL+'v3/icon_gift_27x28_01.png';
            var title = gift.name + ' from ' + Util.setAnchor(req.profile, req.name);
        ...
        function sendRequestsBack() {
            var toSend = new Array();
            Util.each(sendBack, function(id, req) {
                var toIds = req.to;
                while (toIds.length > 0) {
                    req.to = toIds.splice(0,49);
                    toSend.push(req);
                }
            });
            Logger.debug('Requests To Send: '+toSend.length);
            function nextSend() {
                if (toSend.length < 1) {
                    finished();
                    return;
                }
                facebook.sendRequest(toSend.shift(), function(r) {
                    if (r && r.data && r.data.reqsentmsg) {
                        addMessageLog($(r.data.reqsentmsg).find('div:first').html(), 'info');
                    } else {
                        var error_msg = r.error ? r.error.message : 'Error sending request.';
                        addMessageLog(Util.setColor(error_msg,'red'), 'error');
                    }
                    nextSend();
                }, options.sendInternally);
            }
        ...
        function finish() {
            if (options.sendBack === true && Util.length(sendBack) > 0) {
                sendRequestsBack();
            } else {
                finished();
            }
        ...
        $('#mss_successlog', Popup.content).empty();
        $('#mss_messagelog', Popup.content).empty();
    ...
    function clearCollectorScreen() {
        var amount = Requests.toArray('gift').length;
        var text = 'You have ' + Util.setColor(amount,'yellow')+' requests left.';
        text += '<br>Select the gifts you want and set the amount to collect.';
        $('#request_left', Popup.content).html(text);
    ...
    function genMainDom() {
        ...
        function addColLayout($to) {
            var $frm = c$('div', 'class:frame_box').appendTo($to);
            c$('div', 'id:request_left').css('min-height',40).appendTo($frm);
            ...
            .append(c$('ul', 'class:mss_selectable').css('height',504));
            
            c$('div','class:li_wrapper').appendTo($frm)
            .append(c$('ul', 'class:mss_selected'))
            .append(c$('center', 'class:start_ctrl')
            .append(b$('CLEAR','class:medium white').css('margin-right',20).click(Events.clearCollector_click))
        ...
        function addSndLayout($to) {
            ...
            c$('div','class:li_wrapper').appendTo($frm)
            .append(c$('div','class:flist_ctrl').appendTo($frm).append(s$('friendlists', 'Select From List:', 200)))
            .append(c$('ul', 'class:snd_users').css('height',406))
            .append(c$('center', 'class:start_ctrl')
            .append(b$('CLEAR','class:medium white').css('margin-right',20).click(Events.clearFriends_click))
            .append(b$('EDIT','class:medium orange').css('margin-right',20).click(Events.editFriends_click))
        ...
        UserFreeGifts.each(function(id, gift) {
            var imgUrl = UserFreeGifts.getImageUrl(id);
            
            c$('li','gid:'+id).appendTo('.mss_selectable')
            .append(c$('img').attr('src', imgUrl))
            .append(c$('div', 'class:gift_name').text(gift.name))
            .append(c$('div', 'class:add_button').text('>>'))
            .click(Events.addGift_click);
    ...
    Popup.addBase64Style(
        ...
        'IHdpZHRoOiAxOHB4Ow0KICAgIGhlaWdodDogMThweDsNCn0NCiNmcmVlZ2lmdHNjZW50ZXJfcG9wdXA