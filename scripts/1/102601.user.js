// ==UserScript==
// @name           Rockitsauce's All Time Team Leaders
// @namespace      rockitsauce
// @description    List out your team's all time record holders in Goalline Blitz
// @include        http://goallineblitz.com/game/team.pl?season=*&team_id=*
	...
	var fn = {
		...
		game: function(stat) {
			...
			for (i = 0; i <=9; i++) {
				...
				text += '(' + (i + 1) + ')...' + holders[i].stats[left][right].toFixed(2).replace('.00', '') + '...(' + holders[i].pos.trim() + ') [b]' + holders[i].name + '[/b] - http://goallineblitz.com/game/player.pl?player_id=' + holders[i].id + '<br />';
			} 
		...
		loop_games: function(season, to, game) {
			...
			if (data.history.seasons[season].games[game].players.length < 1) {
				$.ajax({
					url: 'http://goallineblitz.com/game/game.pl?game_id=[g]'.replace('[g]', data.history.seasons[season].games[game].id),
					dataType: 'html',
		...
		loop_gamelist: function(i, f) {
			...
			$.ajax({
				url: 'http://goallineblitz.com/game/team.pl?season=[s]&team_id=[t]#'.replace('[s]', i).replace('[t]', data.team.id),
				dataType: 'html',
				success: function(response) {
