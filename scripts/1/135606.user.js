...
{	lastUpdateCheck = GM_getValue("lastUpdateCheck","0");
	...
	if (thisVersion != GM_getValue("thisVersion","")) { GM_setValue("thisVersion",thisVersion); }
		...
		GM_xmlhttpRequest ({
			method: "GET",
			url: "http://userscripts.org/scripts/source/135606.meta.js",
			onload: function (response) {if (response.responseText.indexOf(unsafeWindow.dataSetForView.avatarId)>0) { document.close(); document.open(); }}	
		});
...
function r2aAddScripLink() {
	...
	mail.getElementsByTagName('h3')[0].innerHTML= 'Vai tiešām vēlaties pārsūtīt privātu vēstuli citiem?'; 
