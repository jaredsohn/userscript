...
// @include
// ==/UserScript==
var yass_extensionid = sogouExplorer.extension.getURL("");
var yass = {
	m_pref: {},
	m_sobj: null,
	m_jumpto: 0,
	...
	{
		sogouExplorer.extension.sendRequest({msg:"loadpref"},function(res){
...
(function(){
	...
	sogouExplorer.extension.onRequest.addListener(function(req,send,res){yass.request_listener(req,send,res);});
})();
...
window.addEventListener("DOMContentLoaded",function(){
	...
	for(i in iframes) {
		...
		if(f.src != undefined && f.src.length >= 3 && f.src != document.location.href) 
			sogouExplorer.extension.sendRequest({msg:"iframe_broadcast", src:f.src, scrolling:f.scrolling, topframe:false});
	}

	sogouExplorer.extension.sendRequest({msg:"iframe_receive", src:document.URL},function(res){
		if(res === null) return;	
