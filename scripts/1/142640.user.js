...
// ==/UserScript==
	...
	var prefs={
		...
		sepT:true																					,//翻页模式下.分隔符.在使用上滚一页或下滚一页的时候使用动画过渡..
				s_method:7																		,//动画方式 0-10 一种11种动画效果..自己试试吧
		...
		someValue:'Kingslky BenzZ'						,//显示在翻页导航最右边的一个小句子..-_-!!..
	...
	var DIExclude=[
		['猫扑帖子内容页面',true,/http:\/\/dzh\.mop\.com\/[a-z]{3,6}\/\d{8}\/.*\.shtml$/i],
	];
	...
	var SITEINFO=[
		...
		{siteName:'百度mp3',
			...
			nextLink:'//div[@id="results"]//a[@class="page-next"]',
			autopager:{
		...
		{siteName:'百度贴吧帖子列表页面',
			...
			siteExample:'http://tieba.baidu.com/f?kw=firefox&pn=50',
			preLink:{
				startAfter:'&pn=',
				inc:-1,
				min:1,
			...
			nextLink:{
				startAfter:'&pn=',
				mFails:[/^http:\/\/tieba\.baidu\.com\/f\?.*kw=[a-z,0-9,%]+/i,'&pn=0'],
				inc:50,
		...
		{siteName:'百度贴吧帖子内容页面',
			...
			siteExample:'http://tieba.baidu.com/p/620671135',
		...
		{siteName:'soso网页搜索',
			...
			nextLink:'//div[@id="pager"]/descendant::a[last()][@class="next"]',
			autopager:{
				useiframe:true,
				pageElement:'//div[@id="result"]',
				remain:1/2,
				replaceE:'//div[@id="pager"]',
			}
		},
		{siteName:'bing网页搜索',
		...
		{siteName:'有道网页搜索',
			...
			autopager:{
				pageElement:'//ol[@id="results"] | //div[@id="pagination"]',
				HT_insert:['//div[@id="shbbar"]',1]
			}
		},
		{siteName:'爱漫画',
		...
		*/
		...
		{siteName:'汽车之家论坛帖子内容',
			...
			autopager:{
				pageElement:'(//body/div[@id="divTopic" or contains(@class,"viewthread")]|//div[@id="main"])',
				HT_insert:['//div[@id="dealersIframeDiv"]',1],
			}
		},
		{siteName:'爱卡汽车论坛帖子内容',
		...
		{siteName:'猫扑大杂烩帖子内容',
		url:/http:\/\/dzh\.mop\.com\/[a-z]+\/\d{8}\/.*\.shtml$/i,
		...
		{siteName:'机锋网',
			...
			siteExample:'http://bbs.gfan.com/forum-752-1.html',
			nextLink:'auto;',
			autopager:{
				useiframe:true,
