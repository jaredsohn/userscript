...
// @require	http://userscripts.org/scripts/source/100794.user.js
...
Version 2.0.2:
- Fix to detect Triton engine under v.0.4.2.2
		...
		for (var i=0; i < contentBoxes.snapshotLength; i++)
			...
			if ((includeMissions == true) && (Missions.snapshotLength >= 1))
				...
				for (var j=0; j < Missions.snapshotLength; j++)
					...
					var missionId = '';
					var ETA = tds[4].id;
					if (this._Parent.DOM.Has_ClassName(tds[5],'speed'))
						{
						var RET = tds[6].id;
						}
					else
	...
	if (EmpireBoard.Ikariam.View() == "militaryAdvisorMilitaryMovements")
		...
		if (resMi.snapshotLength > 0)
			...
			for (var i = 0; i < resMi.snapshotLength; i++)
				...
				var nTarget = tds[3];
				var nETA = tds[4];
				if (EmpireBoard.DOM.Has_ClassName(tds[5],'speed'))
					{
					var nRET = tds[6];
					var nAc = tds[7];
					}
