...
//logit ('g_timeoff: '+ uW.g_timeoff);
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap}\
    ...
    table.ptMainTab tr td.notSel {font-weight:bold; font-size:13px; border: 1px solid; border-style: solid solid none solid; background-color:'+Colors.Tabs+'; color:'+Colors.TabFont+'; border-color:black; text-shadow: 0px 0px 1px #AAA;}\
...
init : function (div){
   ...
   getContent : function (){
     ...
     show : function (){  
      var t = Tabs.Inventar;
      m = "<DIV class=ptstat>Inventory List</div>";
      m += "<div style='max-height:1000px; height:650px; overflow-y:auto' id='idInventar'></div>";
      t.cont.innerHTML = m; 
      t.showInventar();
   },
   showInventar:function() {
    var t = Tabs.Inventar;
         var  category = ['','General','Speed-Ups','Combat','Resources','Chest','Court'];
    var m = "<TABLE class=pdxTab border=0 align=center><tr><td colspan=2><b>Item List</td><td><b>Total</td><td><b>Catagory</td></tr>";
      var test = Seed.items;
...
show : function () {
  ...
  var mhtl = "<DIV class=pbStat><SPAN class=oohfancyi>POPULATION AND PRODUCTION INFORMATION</span></div><table width=100% class=pdxTab><tr><td colspan=8>&nbsp;</td></tr><tr><td></td>";
  for(var i=0; i<Cities.numCities; i++) {
   mhtl += "<TD align=center valign=bottom width=60px><B><SPAN class=oohfancy>" + Cities.cities[i].name + "</span></B></TD>";
  }
  mhtl += "</tr><tr><td><img height=18 src=http://koc-power-pdx.googlecode.com/svn/trunk/img/troops/unit_2_50_s34.jpg title=Militiaman><SPAN class=oohfancy> Militiaman/h</span></td>";
  var temps=[];
  for(var i=0; i<Cities.numCities; i++) {
   temps[i]=((Cities.cities[i]['Troop2Time'] > 0)?(3600 / Cities.cities[i]['Troop2Time']):0);
   mhtl += "<td>" + addCommas(parseInt(temps[i])) +"</td>";
  }
    mhtl += "</tr><tr><td><img height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/population_40.png title=Population><SPAN class=oohfancy> Population/h</span></td>";
	var pop=[];
  for(var i=0; i<Cities.numCities; i++) {
  ...
  mhtl += "</tr><tr><td><b><SPAN class=oohfancy>Difference</span></b></td>";
  var diff=0;
  for(var i=0; i<Cities.numCities; i++) {
   ...
   mhtl += "</tr><tr><td><img height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/happiness.png title=happiness><SPAN class=oohfancy> Happiness</span></td>";
   for(var i=0; i<Cities.numCities; i++) {
   cityID = 'city'+ Cities.cities[i].id;
  ...
  var now = unixTime();
  mhtl += "</tr><tr><td><b><SPAN class=oohfancy>Queue</span></b></td>";
    for(var i=0; i<Cities.numCities; i++) {
     cityID = 'city'+ Cities.cities[i].id;
     var totTime = 0;
  ...
  onSuccess:function(transport){
   ...
   if(rslt.ok){
	...
	t.cont.innerHTML +="<div class=pbStat><SPAN class=oohfancyi>TOURNAMENT CHECK</span></div><div class='tourny_modal_upsell'><br><center><b>"+unsafeWindow.g_js_strings.modal_tourny_changetab.notourny+"</b></center></div>";
     ...
     if(rslt.name){
	   ...
	   if(rslt.startdate&&rslt.enddate){
	   	   	    ...
	   	   	    tournyhtml.push("<table width=100% align=center class=pbTab><tr bgcolor=#FE8888><td width=40%><b><SPAN class=oohfancy>Starts</span></td><td width=40%><b><SPAN class=oohfancy>Ends</span></td><td width=20%><b><SPAN class=oohfancy>Ends</span></td></tr>");
	   	   	    dt = new Date ();
...
var DispReport = {
  ...
  parseGiftReport : function(rpts){
    ...
    for(var i=0; i<rpts.length; i++){
		...
		if(rpts[i].subject.innerHTML.indexOf('Nieuwe cadeau ontvangen!') >= 0 && rpts[i].sender.innerHTML.indexOf('Kingdoms Of Camelot') >= 0){
		...
		if(rpts[i].subject.innerHTML.indexOf('Nouveaux Cadeaux re?!') >= 0 && rpts[i].sender.innerHTML.indexOf('Kingdoms Of Camelot') >= 0){
			rpts[i].checkbox.firstChild.checked = true;
		}
...
*/
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          ...
          <TR><TD class=xtab>OR: </td><TD class=xtab> Enter all or part of an alliance name: &nbsp;</td>\
           ...
           <TR><TD class=xtab><INPUT align=left id=allListSubmit type=submit value="List Alliances" /></td>\
			...
			<TD class=xtab><INPUT align=left id=idMyTHSubmit type=submit value="Top 100"/></td>\
        ...
        <option value="1,80" > Catapult </option>\
             ...
             <TR><TD class=xtab><INPUT id=pbShowFriendlies type=CHECKBOX>Show Friendlys &nbsp;<INPUT id=pbShowHostiles type=CHECKBOX>Show Hostiles</td>\
       </td></tr>\
  ...
  eventGetUserLeaderboard: function(rslt) {
   var t = Tabs.AllianceList;
    ...
    var m = '<div style="overflow:auto; height:556px;width:100%;"><TABLE width="600"><thead><TR style="font-weight:bold"> \
          <th class=xtab>Rank</th><th class=xtab>Avatar</th><th>Name</th><th class=xtab>Might</th>\
          <th class=xtab>Alliance</th><th class=xtab>Cities</th><th class=xtab></th></tr></thead><tbody>';
    document.getElementById('allListOut').innerHTML = m;
...
return 0;        
    ...
    for (var i=0; i<t.dat.length; i++){
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ addCommasInt(t.dat[i][1]) 
         ...
         </td><TD  nowrap class=xxtab>'+ (t.dat[i][10]?'<SPAN>'+ (t.dat[i][10]>0?timestr(t.dat[i][10],1):'NA') +'</span>':'<SPAN>NA</span>') +'<td class=xxtab><SPAN onclick="PCplo(this, \''+ t.dat[i][11] +'\')"><A>last Login</a></span><td><TD class=xxtab><DIV onclick="pbAttack('+ t.dat[i][5] +','+ t.dat[i][6] +')"><A>March</td></tr>';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
        ...
        <TD class=clickable><A><DIV>Last Login</a></div></td>\
        <TD id=clickCol13 onclick="PTalClickSort(this)" class=clickable><A><DIV>March</a></div></td></tr></thead>\
...
Tabs.Info = {
	...
	init: function (div){
		...
		var m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; } ' +
			...
			'<TR align=center><TD class=xtab></td><TD class=xtabHL colspan=6><B>COST TO BUILD</b></td><TD class=xtabHL colspan=7><B>STATS</b></td><TD class=xtabHL><B>UPKEEP</b></td></tr>' +
		...
		m += '<BR /><DIV class=ptstat>RESEARCH</div><BR /><SPAN id="Research">&nbsp;</SPAN><BR><DIV class=ptstat>TROOP TRAINING & DEFENSE BUILDING ESTIMATES</div>' +
		...
		for(i=0; i<Cities.numCities; i++) {
			cityID = 'city'+ Cities.cities[i].id;
			m += "<TD align=center valign=bottom width=60px><B>" + Cities.cities[i].name.replace(/ /g, "<BR>") + "</B></TD>";
		...
		m += "</tr>";
		rownum=0;
		_displayrow ('<SPAN class=oohfancy>Number of Barracks</SPAN><img title="Number of Barracks" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/buildings/barracks_lvl10_gen_26.png>', infoRows[0]);
		_displayrow ('<SPAN class=oohfancy>Total Barracks Level</SPAN><img title="Total Barracks Level" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/buildings/barracks_lvl10_gen_26.png>', infoRows[1]);
		_displayrow ('<SPAN class=oohfancy>Foreman Base Politics</SPAN><img title="Foreman Base Politics" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/knight_foreman_100.jpg>', infoRows[2]);
		_displayrow ('<SPAN class=oohfancy>Marshall Combat</SPAN><img title="Marshall Combat" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/knight_marshal_100.jpg>', infoRows[3]);
		_displayrow ('<SPAN class=oohfancy>Stable Level</SPAN><img title="Stable Level" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/building_icons/build_stable_26.png>', infoRows[5]);
		_displayrow ('<SPAN class=oohfancy>Workshop Level</SPAN><img title="Workshop Level" height=18 src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/building_icons/build_workshop_26.png>', infoRows[6]);
		m += "<TR><TD></TD><TD nowrap align=center colspan="+(Cities.numCities)+"><B>Number able to be trained per hour</B></TD></TR>";
		_displayrow ('<SPAN class=oohfancy>Supply Troop</SPAN><img height=18 title="Supply Troop" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_30.jpg', infoRows[7]);
		_displayrow ('<SPAN class=oohfancy>Militiaman</SPAN><img height=18 title="Militiaman" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.jpg', infoRows[8]);
		...
		m += "<TR><TD></TD><TD nowrap align=center colspan="+(Cities.numCities)+"><B>Number able to be built per hour</B></TD></TR>";
		...
		m += '<BR><DIV class=ptstat>DISTANCE CALCULATOR (Also plots location on Province Map)</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>' +
		...
		m += '<DIV class=ptstat>PROVINCE MAP</div><DIV id=ptProvMap style="height:'+ t.provMapCoords.imgHeight +'px; width:'+ t.provMapCoords.imgWidth +'px; background-repeat:no-repeat; background-image:url(\''+ URL_PROVINCE_MAP +'\')"></div>';

		m += '<BR /><DIV class=ptstat>MISCELLANEOUS INFO</div>Tools version: '+ Version + '<BR />KoC client version: '+ KOCversion +'<BR />' +
			'<a href="http://koc.dunno.com/index.sjs?f=KocMapViewer&server_id='+serverID+'">KoC Map Viewer</a>, e.g. right-click and open in a new tab<BR />' +
			'<INPUT id=ptButDebug type=submit name="SEED" value="DEBUG"></div>';
	...
	show: function (){
	  ...
	  m+='<DIV class=ptstat>CREST INFO</div><DIV id=ptLinks><TABLE align=center cellpadding=1 cellspacing=0><TR>';
	  for (city in crestreq){
	  		deed = 'q800' + city;
        if (Seed.quests[deed] ==1) m+='<TD width=75px style="background:#CCFFCC">';
        else  m+='<TD width=75px ">';
        m+= uW.g_js_strings.commonstr.city+' '+city+'</td>';
	  		for (crest in crestreq[city]){
	  			owned = Seed.items['i'+crest];
	  			if (Seed.quests[deed] ==1) m+='<TD 200px style="background:#CCFFCC">';
          else  m+='<TD 200px ">';
	  			if (owned == undefined) owned=0;
	  			if (owned < crestreq[city][crest]) m+='<FONT color=red>'+owned+'/'+crestreq[city][crest]+'</font> '+uW.itemlist['i'+crest]['name']+'</td>';
	  				else  m+='<FONT color=green>'+owned+'/'+crestreq[city][crest]+'</font> '+uW.itemlist['i'+crest]['name']+'</td>'; 
	  		}
	  		m+='</tr>';
		}
		m += '</TABLE></SPAN>';
...
Tabs.Options = {
  ...
  init : function (div){
	...
	main +='<TD><INPUT class=pbSubtab ID=ptmrchSubV type=submit value="Layout"></td>';
	main +='<TD><INPUT class=pbSubtab ID=ptmrchSubX type=submit value="Smiley List"></td></tr></table><HR class=ptThin>';
   // main += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToOptions" target="_blank">HELP</a></td></tr>';
    ...
    try { 
      m = '<TABLE class=ptTab>';
	  m+='<TR><TD colspan=2><B>Smiley List:</b></td></tr>';
	  m+='<TD colspan=2><b>(salut)<br>:-)<br>:-(<br>:-D<br>(ptdr)<br>;-(<br>;-)<br>:-*<br>:^)<br>I-)<br>8-)<br>(doh)<br>(angel)<br>(bravo)<br>(bravo2)<br>(devil)<br>]:-><br>(fubar)<br>:-@<br>(bomb)<br>(headbang)<br>(punch)<br>:-&<br>]:)<br>(:I<br>:-P<br>:-$<br>(party)<br>(call)<br>(smoke)<br>(mooning)<br>(h)<br>(inlove)<br>*in love*<br>*kissing*<br>(emotlove2)<br>(U)<br>(d)<br>(cafe)<br>(m)<br>(bug)<br>(bear)<br>I()';
	  m+='<TD colspan=4><b>(f)<br>(dance)<br>(bandit)<br>(poolparty)<br>(tmi)<br>(drunk)<br>(rock)<br>(sweat)<br>(wait)<br>(makeup)<br>(rain)<br>(time)<br>(*)<br>(ninja)<br>(cake)<br>(massage)<br>(music)<br>(shark)<br>(shark2)<br>(robot)<br>(pingouin)<br>(cat)<br>(fox)<br>(sonic)<br>(heidy)<br>(caribou)<br>(renne)<br>(hamta)<br>(rat)<br>(girafe)<br>(tigre)<br>(elephant)<br>(spider)<br>(miroir)<br>(bath)<br>(couette)<br>(magebarbe)<br>(baby)<br>(fouet)<br>(concombre)<br>(autruche)<br>(cochon)<br>(crabe)';
	  m+='<TD colspan=6><b>(ours)<br>(serpent)<br>(hippo)<br>(singe)<br>(herisson)<br>(bubulle)<br>(aigle)<br>(chien)<br>(lapin)<br>(grenouille)<br>(cuicui)<br>(panda)<br>(dragon)<br>(lapin1)<br>(lapin2)<br>(lapin3)<br>(camelot)<br>(papillon)<br>(bienmal)<br>(taupe)<br>(ver)<br>(vache)<br>(tortue)<br>(chat)<br>(Italy)<br>(dindon)<br>(Jolly)<br>(SpiderPig)<br>(update)<br>(Wolf)</b></td>';
	  t.Overv.innerHTML = m;
    } catch (e) {
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight><SPAN class=oohfancy>Joined on:</span><SPAN class=oohfancyi> '+ dt.toLocaleDateString() +'</span></td>\
        <TD><SPAN class=ptStatLight><SPAN class=oohfancy>Might:</span><SPAN class=oohfancyi> ' + addCommas(Seed.player.might) +'</span></td>\
        <TD><SPAN class=ptStatLight><SPAN class=oohfancy>Alliance:</span> <SPAN class=oohfancyi>' + getMyAlliance()[1] +'</span></td>\
        <TD align=right><SPAN class=ptStatLight><SPAN class=oohfancy>Domain:</span><SPAN class=oohfancyi> ' + unsafeWindow.domainName +'</span></td></tr></table></div>';
	 ...
	 if(Gate == 0) var couleurr="#77EE77";
        ...
        str += "<TD width=81 style='background-color:"+couleurr+"'><SPAN class=oohfancy>"+ Cities.cities[i].name.substring(0,11) +'</SPAN><BR>'+ coordLink (Cities.cities[i].x, Cities.cities[i].y) +"<BR>"+ unsafeWindow.provincenames['p'+ Cities.cities[i].provId] +"</td>";
      ...
      str += _row ('<SPAN class=oohfancy>Gold</SPAN><img title="Gold" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/gold_30.png>', rows[0], false, 0);
      str += _row ('<SPAN class=oohfancy>Food</SPAN><img title="Food" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png>', rows[1], false, 0);
      str += _row ('<SPAN class=oohfancy>Wood</SPAN><img title="Wood" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/wood_30.png>', rows[2], false, 0);
      str += _row ('<SPAN class=oohfancy>Stone</SPAN><img title="Stone" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/stone_30.png>', rows[3], false, 0);
      str += _row ('<SPAN class=oohfancy>Ore</SPAN><img title="Ore" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/iron_30.png>', rows[4], false, 0);
      str += _row ('<SPAN class=oohfancy>Aetherstone</SPAN><img title="Aetherstone" height=18 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/aetherstone_30.png>', rows[5]);
...
row = [];
      ...
      str += '<TR><TD><BR></td></tr>';
      str += _row ('<SPAN class=oohfancy>Food +/-</SPAN><img height=18 title="Food +/-" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png>', row, false, 0);
      ...
      str += _row ('<SPAN class=oohfancy>Food Left</SPAN><img height=18 title="Food Left" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png>', row, true);
      str += '<TR><TD><BR></td></tr>';
...
//**********************************
      ...
      str += _row ('<SPAN class=oohfancy>SupTrp</SPAN><img height=18 title="Supply Troop" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_30.jpg', rows[1]);      
      str += _row ('<SPAN class=oohfancy>Militia</SPAN><img height=18 title="Militia" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.jpg', rows[2]);
      str += _row ('<SPAN class=oohfancy>Scout</SPAN><img height=18 title="Scout" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_3_30.jpg', rows[3]);
      str += _row ('<SPAN class=oohfancy>Pike</SPAN><img height=18 title="Pike" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_4_30.jpg', rows[4]);
      str += _row ('<SPAN class=oohfancy>Sword</SPAN><img height=18 title="Sword" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_5_30.jpg', rows[5]);
      str += _row ('<SPAN class=oohfancy>Archer</SPAN><img height=18 title="Archer" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_6_30.jpg', rows[6]);
      str += _row ('<SPAN class=oohfancy>Cavalry</SPAN><img height=18 title="Cav" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_7_30.jpg', rows[7]);
      str += _row ('<SPAN class=oohfancy>H-Cav</SPAN><img height=18 title="H-Cav" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_8_30.jpg', rows[8]);
      str += _row ('<SPAN class=oohfancy>Wagon</SPAN><img height=18 title="Wagon" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_9_30.jpg', rows[9]);
      str += _row ('<SPAN class=oohfancy>Ballista</SPAN><img height=18 title="Balls" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_10_30.jpg', rows[10]);
      str += _row ('<SPAN class=oohfancy>Ram</SPAN><img height=18 title="Rams" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_11_30.jpg', rows[11]);
      str += _row ('<SPAN class=oohfancy>Catapult</SPAN><img height=18 title="Cats" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_12_30.jpg', rows[12]);
      ...
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>SupTrp TR</SPAN><img height=18 title="Supply Troops Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_30.jpg', t_rows[2])};
      var intmax = 0;
      for (intt=0; intt<t_rows[4].length; intt++) {
         if (t_rows[4][intt] > intmax) {intmax = t_rows[4][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Militia TR</SPAN><img height=18 title="Militiaman Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.jpg', t_rows[4])};
      var intmax = 0;
      for (intt=0; intt<t_rows[6].length; intt++) {
         if (t_rows[6][intt] > intmax) {intmax = t_rows[6][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Scout TR</SPAN><img height=18 title="Scout Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_3_30.jpg', t_rows[6])};
      var intmax = 0;
      for (intt=0; intt<t_rows[8].length; intt++) {
         if (t_rows[8][intt] > intmax) {intmax = t_rows[8][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Pike TR</SPAN><img height=18 title="Pikeman Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_4_30.jpg', t_rows[8])};
      var intmax = 0;
      for (intt=0; intt<t_rows[10].length; intt++) {
         if (t_rows[10][intt] > intmax) {intmax = t_rows[10][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Sword TR</SPAN><img height=18 title="Swordsman Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_5_30.jpg', t_rows[10])};
      var intmax = 0;
      for (intt=0; intt<t_rows[12].length; intt++) {
         if (t_rows[12][intt] > intmax) {intmax = t_rows[12][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Archer TR</SPAN><img height=18 title="Archer Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_6_30.jpg', t_rows[12])};
      var intmax = 0;
      for (intt=0; intt<t_rows[14].length; intt++) {
         if (t_rows[14][intt] > intmax) {intmax = t_rows[14][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Cav TR</SPAN><img height=18 title="Cavalry Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_7_30.jpg', t_rows[14])};
      var intmax = 0;
      for (intt=0; intt<t_rows[16].length; intt++) {
         if (t_rows[16][intt] > intmax) {intmax = t_rows[16][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>H-Cav TR</SPAN><img height=18 title="Heavy Cavalry Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_8_30.jpg', t_rows[16])};
      var intmax = 0;
      for (intt=0; intt<t_rows[18].length; intt++) {
         if (t_rows[18][intt] > intmax) {intmax = t_rows[18][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Wagon TR</SPAN><img height=18 title="Supply Wagon Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_9_30.jpg', t_rows[18])};
      var intmax = 0;
      for (intt=0; intt<t_rows[20].length; intt++) {
         if (t_rows[20][intt] > intmax) {intmax = t_rows[20][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Ball TR</SPAN><img height=18 title="Ballista Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_10_30.jpg', t_rows[20])};
      var intmax = 0;
      for (intt=0; intt<t_rows[22].length; intt++) {
         if (t_rows[22][intt] > intmax) {intmax = t_rows[22][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Ram TR</SPAN><img height=18 title="Ram Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_11_30.jpg', t_rows[22])};
      var intmax = 0;
      for (intt=0; intt<t_rows[24].length; intt++) {
         if (t_rows[24][intt] > intmax) {intmax = t_rows[24][intt]};
      }
      if (intmax > 0) {str += _row ('<SPAN class=oohfancy>Cat TR</SPAN><img height=18 title="Catapult Training" src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_12_30.jpg', t_rows[24])};
      }
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('<SPAN class=oohfancy>#Wilds</SPAN><img height=18 title="Wilds" src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/buildings/mountains_lvl1.png?4568', row, true);
      ...
      str += _row ('<SPAN class=oohfancy>#Knights</SPAN><img height=18 title="Knights" src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/building_icons/build_knightshall_26.png?4568', row, true);
      ...
      for(i=0; i<Cities.numCities; i++) {
        ...
        if (totTime < 3600)
          ...
          row[i] = timestr(totTime);
            ...
            str +='<tr style="background: #fff" align=right><td><SPAN class=oohfancy>Building Q</SPAN><img height=20 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/items/70/131.jpg title="Building"></td><td>&nbsp;</td>';
            for(i=0; i<Cities.numCities; i++) {
             var totTime = 0;
	  ...
	  str +='<tr style="background: #e8e8e8" align=right><td><SPAN class=oohfancy>Research Q</SPAN><img height=20 src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/items/70/1.jpg title="Research"></b></td><td>&nbsp;</td>';
            for(i=0; i<Cities.numCities; i++) {
             var totTime = 0;
      ...
      str += _row ('<SPAN class=oohfancy>TroopQ</SPAN><img height=18 title="Troop Q" src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/building_icons/build_barracks_26.png?4568', row, true);
      ...
      str += _row ('<SPAN class=oohfancy>Wall Q</SPAN><img height=18 title="Wall Q" src=https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/building_icons/build_walls_26.png?4568', row, true);
...
Tabs.Builds = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptstat><SPAN class=oohfancy>Building and Research overview</span></div>';
    ...
    m += "<TD align=left width=85><INPUT id=showReq type=checkbox " + (t.showReq?'CHECKED ':'') +"><SPAN class=oohfancy>Show missing req.</span></td>";
    for(i=0; i<Cities.numCities; i++) {
      m += "<TD width=79><B>"+ Cities.cities[i].name.substring(0,11) +"</b><BR>"+ coordLink(Cities.cities[i].x, Cities.cities[i].y) +"<BR></td>";
    }
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Building</span></td>';	
    for(i=0; i<Seed.cities.length; i++) {
    ...
    m+='</tr>';
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Researching</span></td>';	
    for(i=0; i<Seed.cities.length; i++) {
    	city = 'city'+Seed.cities[i][0];
    	m+='<TD width=79>';
    ...
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>CrossBows</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	...
    	if (wall >=6 && blacksmith >=6 && fletching >=5 && max > 0) m+= '<br><SPAN class=oohfancy>Left: </span>' + max +'</td>';
    	else if (t.showReq){
   			if (wall < 6) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Wall: ' + wall + '(6)</font>';
   			if (blacksmith < 6) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>BlackS.: </span>' + blacksmith + '(6)</font>';
   			if (fletching < 5) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Fletch.: </span>' + fletching + '(5)</font>';
   			m+='</td>';
   		} 		
    }
    m+='</tr><TR valign=top align=right><TD width=85><SPAN class=oohfancy>Trebuchet</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    	...
    	if (wall >=8 && blacksmith >=8 && fletching >=7 && geometry>=7 && max > 0) m+= '<br><SPAN class=oohfancy>Left: </span>' + max+'</td>';
    	else if (t.showReq){
    			if (wall < 8) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Wall: </span>' + wall + '(8)</font>';
    			if (blacksmith < 8) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>BlackS.: </span>' + blacksmith + '(8)</font>';
    			if (fletching < 7) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Fletch.: </span>' + fletching + '(7)</font>';
    			if (geometry < 7) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Geomet.: </span>' + geometry + '(7)</font>';
    			m+='</td>';
    	} 	
    }
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Wall defences</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    ...
    m+='</tr><TR valign=top align=right><TD width=85><SPAN class=oohfancy>Traps</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    	...
    	if (wall >=4 && blacksmith >=4 && poisonededge>=4 && max>0) m+= '<br><SPAN class=oohfancy>Left: </span>' + max+'</td>';
    	else if (t.showReq){
    			if (wall < 4) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Wall: </span>' + wall + '(4)</font>';
    			if (blacksmith < 4) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>BlackS.: </span>' + blacksmith + '(4)</font>';
    			if (poisonededge < 4) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Poison.: </span>' + poisonededge + '(4)</font>';
    			m+='</td>';
    	} 	
    }
    m+='</tr><TR valign=top align=right><TD width=85><SPAN class=oohfancy>Caltrops</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    	...
    	if (wall >=1 && metalalloys >=1 && max>0) m+= '<br><SPAN class=oohfancy>Left: </span>' + max+'</td>';
    	else if (t.showReq){
    			if (wall < 1) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Wall: </span>' + wall + '(1)</font>';
    			if (metalalloys < 1) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Metal.: </span>' + metalalloys + '(1)</font>';
    			m+='</td>';
    	} 	
    }
    m+='</tr><TR valign=top align=right><TD><SPAN class=oohfancy>Spiked Barrier</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    	...
    	if (wall >=2 && blacksmith>=2 && logging>=2 && max>0) m+= '<br><SPAN class=oohfancy>Left: </span>' + max+'</td>';
    	else if (t.showReq){
    			if (wall < 2) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Wall: </span>' + wall + '(2)</font>';
    			if (blacksmith < 2) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>BlackS.: </span>' + blacksmith + '(2)</font>';
    			if (logging < 2) m+= '<br><FONT COLOR= "CC0000"><SPAN class=oohfancy>Logg.: </span>' + logging + '(2)</font>';
    			m+='</td>';
    	} 	
    }
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Field defences</span></td>';
    for (i=0;i<Seed.cities.length;i++){
    	city = 'city'+Seed.cities[i][0];
    	for (y in Seed.buildings[city]) {
    		if (Seed.buildings[city][y][0] == 19) wall = Seed.buildings[city][y][1];
    ...
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>City space</span></td>';
    ...
    m+='</tr>';
    m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Field space</span></td>';
    ...
    m+='</tr>';
    for (b=0;b<20;b++){
    	m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>'+buildings[b]+'</span></td>';
	...
	m+='<TR valign=top align=right><TD width=85><SPAN class=oohfancy>Guardians</span></td>';
	for(i=0; i<Seed.cities.length; i++) {
    ...
    for (i in Research) {
    	select = 'tch'+i;
    	m+='<TD border=1px style="width:150px;"><SPAN class=oohfancy>'+Research[i]+'</span></td><TD align=center style="width:50px max-width:150px;">';
    	...
    	if (t.showReq) {
    		...
    		switch (i) {
    			case '1': 
    				if (alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab+ '('+ (Seed.tech[select]) +')';
    				if (farm < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Farm </span>'+ farm +'('+ (Seed.tech[select]+1) +')</td>';
    				break;
    			case '2': 
    				if (alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab+ '('+ (Seed.tech[select]) +')';
    				if (sawmill < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Sawmill</span> '+ sawmill +'('+ (Seed.tech[select]) +')';
    				break;
    			case '3': 
    				if (alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab</span> '+ alchemylab+ '('+ (Seed.tech[select]) +')';
    				if (quarry < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Quarry </span>'+ quarry +'('+ (Seed.tech[select]) +')</td>';
    				break;
    			case '4': 
    				if (alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab+ '('+ (Seed.tech[select]) +')';
    				if (mine < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Mine </span>'+ mine +'('+ (Seed.tech[select]) +')';
    				break;
    			case '5': 
    				if (alchemylab < 3) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(3)';
    				if (alchemylab >= 3 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (workshop < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Workshop </span>' + workshop +'('+ (Seed.tech[select]) +')';
    				if (stoneworking < 2) m+='<BR><SPAN class=oohfancy>Stoneworking </span>'+ stoneworking +'(2)';
    				break;
    			case '6': 
    				if (alchemylab < 3) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(3)';
    				if (alchemylab >= 3 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				break;	
    			case '8': 
    				if (alchemylab < 2) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(2)';
    				if (alchemylab >= 2 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (barracks < 2) m+='<BR><SPAN class=oohfancy>Barracks</span> '+ barracks +'(2)';
    				break;		
    			case '9': 
    				if (alchemylab < 3) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(3)';
    				if (alchemylab >= 3 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (blacksmith < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Blacksmith </span>'+ blacksmith+'('+ (Seed.tech[select]) +')';
    				if (mining < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Mining </span>'+ mining +'('+ (Seed.tech[select]) +')';
    				break;
    			case '10': 
    				if (alchemylab < 4) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(4)';
    				if (alchemylab >= 4 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab</span> '+ alchemylab +'('+ (Seed.tech[select]) +')';
    				break;
    			case '11': 
    				if (alchemylab < 4) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(4)';
    				if (alchemylab >= 4 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				break;
    			case '12': 
    				if (alchemylab < 5) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(5)';
    				if (alchemylab >= 5 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (stable < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Stable </span>'+ stable +'('+ (Seed.tech[select]+1) +')';
    				if (metalalloys < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Metal Alloys </span>'+ metalalloys +'('+ (Seed.tech[select]+1) +')';
    				break;
    			case '13': 
    				if (alchemylab < 4) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(4)';
    				if (alchemylab >= 4 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (logging < 4) m+='<BR><SPAN class=oohfancy>Logging </span>'+ logging +'(4)';
    				break;
    			case '14': 
    				if (alchemylab < 6) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(6)';
    				if (alchemylab >= 6 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (storehouse < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Storehouse </span>'+ storehouse +'('+ (Seed.tech[select]) +')';
    				if (logging < 3) m+='<BR><SPAN class=oohfancy>Logging </span>'+ logging+'(3)';
    				break;
    			case '15': 
    				if (alchemylab < 6) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(6)';
    				if (alchemylab >= 6 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (featherweightpowder  < 3) m+='<BR><SPAN class=oohfancy>Featherweight Powder </span> '+ featherweightpowder +'(3)';
    				break;
    			case '16': 
    				if (alchemylab < 5) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'(5)';
    				if (alchemylab >= 5 && alchemylab < Seed.tech[select]) m+='<BR><SPAN class=oohfancy>Alchemy Lab </span>'+ alchemylab +'('+ (Seed.tech[select]) +')';
    				if (logging  < 5) m+='<BR><SPAN class=oohfancy>Logging </span>'+ logging +'(5)';
    				if (geometry  < 2) m+='<BR><SPAN class=oohfancy>Geometry </span> '+ geometry +'(2)';
    				break;
    		}
    		m+='</font>';
    	}
...
Tabs.Alliance = {
  ...
  init : function (div){    
	...
	m += '<TD width = "100px" ; border:none"><a href="http://code.google.com/p/koc-power-tools/wiki/HowToAlliance" target="_blank">AIUTO</a></td></tr>';
  ...
  paintMembers: function(){
   ...
   t._addTabHeader();
	...
	var row = document.getElementById('alOverviewTab').insertRow(0);
		...
		row.insertCell(6).innerHTML = "<u><b>Positio</b></u>";
		row.insertCell(7).innerHTML = "<u><b>Days In Position</b></u>";
		row.insertCell(8).innerHTML = "<u><b>Last Login</b></u>";
		row.insertCell(9).innerHTML = "<u><b>Entered Domain</b></u>";
      },
...
if (document.body.id == 'mainbody') {
	...
	div.appendChild(Builder.BlueButton("Create Message", "modal_messages();track_chrome_btn('messages_btn');modal_messages_compose();return false;"));
	//div.appendChild(Builder.BlueButton("Message Alliance", "getMessageWindow(1445,'Alliance Members','allianceall');return false;"));
	...
	div.appendChild(Builder.BlueButton("March Troops", "modal_attack(4,'','');return false;"));
	//div.appendChild(Builder.BlueButton("Scan Chat", scan_allianceChat));
	//div.appendChild(Builder.Html(" KSX Buttler v" + Version));
...
function atResourceRequestClick() {
	...
	//alert(e.id);
	Tools.type(e, "/a *Request* " + rrDestination + " " + rrType + " " + rrAmount);
	var b = e.nextSibling;
	Tools.click(b);
}
...
//		else //this should isolate the stuff in alliance chat. seltab2 = alliance.
		...
		for (var i = 0; i < div.childNodes.length; i++) {
			...
			for (var j = 0; j < msgdiv.childNodes.length; j++) {
				...
				var tDly = tReq.replace('*Request*', "*Delivery Enroute*");
				...
				actiondiv.appendChild(Builder.BlueButton("*Delivery*", cD));
