// ==UserScript==
// @name         Google Deep Web
// @author       Keyser Söze
// @description  Allows Deep Web Browsing Through Google Search Engine: Find music, videos, films, tv series, books, magazines, media files, torrents, presentations, datasheets, multiple papers, essays, dissertations, scientific publications, general information, software, etc.., Within specialist sites, forums, blogs, government institutions , file hosting (free storage servers), FTP folders, etc... All through Google, Scientific, Academic & Specialized search engines!
// @descripción  Permite Navegar La Web profunda A Través Del Motor De Búsqueda De Google: Encuentra música, videos, películas, series de televisión, libros, revistas, archivos multimedia, torrentes, presentaciones, hojas de cálculo, documentos varios, ensayos, tesis, publicaciones científicas, información general, software, etc... Dentro de páginas especializadas, foros, blogs, instituciones gubernamentales, sitios de alojamiento de archivos (servidores de almacenamiento gratuito), carpetas FTP, etc. Todo a través de Google, buscadores científicos, académicos y especializados!
// @namespace	 http://userscripts.org/users/KeyserSoze
// @homepageURL  https://userscripts.org/scripts/show/152596
// @website      http://userscripts.org/scripts/show/152596
// @updateURL    https://userscripts.org/scripts/source/152596.meta.js
// @installURL   https://userscripts.org/scripts/source/152596.user.js
// @downloadURL  https://userscripts.org/scripts/source/152596.user.js
// @icon	 http://www.forodirecto.com/customavatars/avatar51207_1.gif
// @run-at	 document-end
// @grant        GM_log
// @grant        GM_addStyle
// @grant        GM_getValue
...
// @grant        unsafeWindow
// @grant        GM_openInTab
// @grant        GM_getValueUTF8
// @grant        GM_setValueUTF8
// @grant        GM_xmlhttpRequest
// @grant        GM_registerMenuCommand
// @version      2.5.6
// @include      http://*
// @include      https://*
// @exclude      htt*://*.google.*/*tbm=isch*
// @exclude      htt*://*.google.*/images?*
// @exclude      htt*://*.google.*/imgh*
// @exclude      htt*://images.google.*/*
// @exclude      htt*://*.google.com/
// @exclude      htt*:*drive.google.*
// @exclude      htt*:*docs.google.*
// @exclude      htt*:*scholar.google.*
// @exclude      htt*:*skydrive.live.*
// @exclude      htt*://mail.google.com/*inbox
// @exclude      htt*://*.google.*/
// @exclude      htt*:*denti.li/index.php?action=post*
// @exclude      htt*:*porniteca.li/index.php?action=post*
// @exclude      htt*:*ringa.net/comunidades/*/agregar*
// @exclude      htt*://*youtube.com/analytics*
// @exclude      htt*:*.facebook.com/*
// @license      Public Domain
// Basado en	 "Click Search Now Also As", "Greased LightBox", "PicViewer", "Linkify", "TextArea Backup", "Docs Online Viewer", "Google Search Extended" & "Search Jump" ... Así que, muchas gracias a los autores originales!
// Based on 	 "Click Search Now Also As", "Greased LightBox", "PicViewer", "Linkify", "TextArea Backup", "Docs Online Viewer", "Google Search Extended" & "Search Jump" ... So, many thanks to original authors!
// ==/UserScript==
...

{
 ...
 var css = "";
if (false || (document.location.href.indexOf("http://www.google") == 0) || (document.location.href.indexOf("https://www.google") == 0) || (document.location.href.indexOf("http://accounts.google") == 0) || (document.location.href.indexOf("https://accounts.google") == 0))
...
/*
		============================================
		|| Media Player & Documents Online Viewer ||
		============================================
*/
...
var fileTypes2 = ["mp3","mp4","m4a","avi","flv","aac","mpeg","mpg","ogg","ogv","wav"];
var fileTypes3 = ["zip","rar"];
var doCheck = true;
			...
			if (supportedFileFormat == 0){
				for (var i2 = 0; i2 < fileTypes2.length; i2++) {
					...
					if (endsWith(url, '.' + fileTypes2[i2])) {
						changeLink(docLinks[i], 2);
						break;
					}
				}
			}
			if (supportedFileFormat == 0){
				for (var i2 = 0; i2 < fileTypes3.length; i2++) {
					var url = stripQuery(docLinks[i]);
					if (endsWith(url, '.' + fileTypes3[i2])) {
						changeLink(docLinks[i], 3);
						break;
					}
				}
			}
...
function changeLink(link, fileTypeCategory) { 
	...
	if(fileTypeCategory == 1){
		...
		viewLink.href = "http://thumbmaxx.com/flash/player.swf?file="+encodeURI(stripQuery(link))+"&autostart=true&stretching=fill&allowfullscreen=allways&image=http://i220.photobucket.com/albums/dd28/mten666/Interbeing_-_Alex_Grey.jpg&skin=http://thumbmaxx.com/flash/glow/glow.xml";
	}else if(fileTypeCategory == 3){
	...
	viewLink.title="View with Media & Docs Online Viewer\nURL: "+encodeURI(stripQuery(link))+"\n- Thanx 2: Deekshith";
	var ico = document.createElement("img");
	ico.src = "http://www.google.com/s2/favicons?domain=corelite.dreamwidth.org";
...
*/
	...
	for (var i=0; i<page_links.length; i++){
		if (page_links[i].href.match(/\.mp3$/i)) {
			...
			var url = "http://www.cancionesdelayer.com/forum/players/mp3player.swf?file="+escape(page_links[i].href)+"&song_title="+escape(page_links[i].innerHTML)
			var width = 100
			var height = 20
			code_str = ""
	...
	var page_links = document.links;
	for (var i=0; i<page_links.length; i++){
		if (page_links[i].href.match(/\.m4a$/i)) {
			var span = document.createElement("span");
			var url = "http://thumbmaxx.com/flash/player.swf?skin=http://thumbmaxx.com/flash/glow/glow.xml&file="+escape(page_links[i].href)+"&song_title="+escape(page_links[i].innerHTML)
			var width = 150
			var height = 25
			code_str = ""
			code_str += " <object type=\"application/x-shockwave-flash\"\n"
	...
	function selectSite(site,link,special) {
		...
		if (site == "Audio") {
			if (html5) {
				...
				span.innerHTML += '<'+embed+' src="http://beemp3.com/player/player.swf?playerID=1&bg=0xCDDFF3&leftbg=0x357DCE&lefticon=0xF2F2F2&rightbg=0x64F051&rightbghover=0x1BAD07&righticon=0xF2F2F2&righticonhover=0xFFFFFF&text=0x357DCE&slider=0x357DCE&track=0xFFFFFF&border=0xFFFFFF&loader=0xAF2910&autostart=yes&soundFile='+link.href+'" width="'+space[1]+'" height="35" frameborder="0"></'+embed+'>';
			}
		}
		if (site == "Facebook") {
		...
		if (site == "YouTube") {
			if (iframe) {
				...
				span.innerHTML += '<object width="'+space[3]+'" height="'+space[3]/(16/9)+'"><param name="movie" value="http://www.youtube.com/v/'+link.href.replace(youtubelink,"$1")+'?rel=0&start='+link.href.replace(youtubelink,"$2")+'"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'+link.href.replace(youtubelink,"$1")+'?rel=0&start='+link.href.replace(youtubelink,"$2")+'" type="application/x-shockwave-flash" width="'+space[3]+'" height="'+space[3]/(16/9)+'" autoplay="1" allowscriptaccess="always" allowfullscreen="true"></embed></object>';
			}
		}
		if (site == "YouTu.be") {
			if (iframe) {
...
SaveTextArea.prototype = {
  ...
  restore: function() {
    ...
    GM_registerMenuCommand(
      'Restaurar Respaldo Anterior En | Restore Previous Backup For '+this.ref(),
  ...
  _confirm_restore: function(buff) {
    ...
    var msg = "Existe Texto Detectado En | Existing Text Detected In '" + this.ref()
            + "', Overwrite With This Backup? | Sobreescribirlo Con Éste Respaldo?\n\n";
...
/*		=======================================
		|| Scientific Search Engines SIDEBAR ||
		=======================================
*/

{
   ...
   var lg = new Array();
   searchenginearray[0] = new Array('Wolfram|Alpha', 'i', 'wolframalpha.', 'http://www.wolframalpha.com/input/?i=--keywords--', 1, 'http://www.wolframalpha.com/favicon.ico');
   searchenginearray[1] = new Array('CiteSeerX', 'q', 'citeseer.ist.', 'http://citeseer.ist.psu.edu/search?q=--keywords--', 1, 'http://citeseer.ist.psu.edu/favicon.ico');
   searchenginearray[2] = new Array('FindEbookEe', 'q', 'findebookee.', 'http://findebookee.com/search.php?q=--keywords--&type=all', 1, 'http://www.findebookee.com/favicon.ico');
   searchenginearray[3] = new Array('BVSalud', 'q', 'pesquisa.bvsalud.', 'http://pesquisa.bvsalud.org/regional/?q=--keywords--&where=ALL&index=&lang=es&_charset_=iso-8859-1', 1, 'http://www.google.com/s2/favicons?domain=pesquisa.bvsalud.org');
   searchenginearray[4] = new Array('SciELO', 'q', 'search.scielo.', 'http://search.scielo.org/?lang=es&col=org&site=scielo&count=10&filter=&filterLabel=&qt=standard&fmt=&sort=&pageFrom=0&from=&addfilter=&backfilter=&printMode=&output=&fb=&q=--keywords--&index=&where=ORG', 1, 'https://plus.google.com/_/favicon?domain=search.scielo.org');
   searchenginearray[5] = new Array('EBooksBay', 'SearchString=', 'ebooksbay.', 'http://www.ebooksbay.org/search-new.php?SearchString=--keywords--&Search=Search', 1, 'http://www.epubbooks.com/favicon.ico');
   searchenginearray[6] = new Array('Ask', 'q', 'web.ask.', 'http://web.ask.com/web?q=--keywords--&qsrc=0&o=0', 0, 'http://www.ask.com/favicon.ico');
   searchenginearray[7] = new Array('LatIndex', 'q', 'latindex.unam.', 'http://www.latindex.unam.mx/buscador/resBus.html?palabra=--keywords--', 1, 'http://www.lexilogos.com/favicon.ico');
   searchenginearray[8] = new Array('Yahoo', 'p', 'search.yahoo.', 'http://search.yahoo.com/search;_ylt=AtMs5L7cJoJYhnOOdwJxjO2bvZx4?p=--keywords--', 0, 'http://www.yahoo.com/favicon.ico');
   searchenginearray[9] = new Array('PubMed', 'term', 'ncbi.nlm.nih.', 'http://www.ncbi.nlm.nih.gov/pubmed?term=--keywords--', 1, 'http://www.ncbi.nlm.nih.gov/favicon.ico');
  searchenginearray[10] = new Array('TDX', 'query', 'tdx.', 'http://www.tdx.cat/search?query=--keywords--', 1, 'http://www.tdxtech.com/favicon.ico');
  searchenginearray[11] = new Array('BASE', 'lookfor', 'base-search.', 'http://www.base-search.net/Search/Results?lookfor=--keywords--', 1, 'http://www.base-search.net/favicon.ico');
  searchenginearray[12] = new Array('Scirus', 'q', 'Scirus.', 'http://www.scirus.com/srsapp/search?q=--keywords--', 1, 'http://www.scirus.com/favicon.ico');
  searchenginearray[13] = new Array('BiomedCentral', 'terms', 'biomedcentral.', 'http://www.biomedcentral.com/search/results?terms=--keywords--', 1, 'http://www.biomedcentral.com/favicon.ico');
  searchenginearray[14] = new Array('DocStoc', 'search/', 'docstoc.', 'http://www.docstoc.com/search/--keywords--?catid=0', 0, 'http://www.docstoc.com/favicon.ico');
  searchenginearray[15] = new Array('AltaVista', 'q', 'altavista.', 'http://www.altavista.com/web/results?itag=ody&q=--keywords--', 0, 'http://www.altavista.com/favicon.ico');
  searchenginearray[16] = new Array('TDR', 'query', 'tesisenred.', 'http://www.tesisenred.net/search?scope=%2F&ocult=0&query=--keywords--', 0, 'http://www.google.com/s2/favicons?domain=www.tesisenred.net');
  searchenginearray[17] = new Array('Lycos', 'query', 'search.lycos.', 'http://search.lycos.com/default.asp?loc=searchbox&tab=web&query=--keywords--', 0, 'http://www.lycos.com/favicon.ico');
  searchenginearray[18] = new Array('Bing', 'q', 'bing.', 'http://www.bing.com/search?q=--keywords--', 0, 'http://www.bing.com/favicon.ico');
  searchenginearray[19] = new Array('Science', 'fullrecord:', '.edu', 'http://www.science.gov/scigov/result-list/collections:SCIGOVIMAGE-NSF,SCRNNIE,TOXNETTOXLINE,CFSAN,NISTDBS,TREESEARCH,USGSPUBS,CDER,EPRINT,CFR,SCIGOV-WRIGHT,CBER,NASA-TRS,SCIGOVIMAGE-SCICINEMA,CONGRESS-111,SCIGOV-DOEDE,USPTO,USDA-FNICWEB,CONGRESS-112,SCIGOVIMAGE-MEDLINEPLUS,NCI,INFOB-XML,SCIGOV-ESTSC,DOE-PATENT-XML,ERIC,EPA-NSCEP,SCIGOV-NISIC,NASAWEB,EPA-EIMS,DOE-RDACC,NREL-AFDC,PMC,SCIGOV-MAS,NSFPUBS,NIH-MEDLINEPLUS,DTIC-ST,TOXNETHSDB,PESTICIDES,EIAPUBS,ECD,TEKTRAN,NOAA,SCIGOVIMAGE-USGS,SCIGOVIMAGE-NASA,NSDL,NREL-WEB,PUBMED,CFR2012,FEDREG,CFR2011,SCIGOVWS,SCIGOV-HHH,NASAADS,SCIGOVIMAGE-PLANTS,CLINICALTRIALS,SCIGOV-IRIS,NTIS,SCIGOV-OPENAGRICOLA,DOTNTL/fullRecord:--keywords--&Submit.x=0&Submit.y=0', 1, 'http://www.google.com/s2/favicons?domain=www.science.gov');
  searchenginearray[20] = new Array('Scholar', 'q', 'scholar.google.', 'http://scholar.google.com.mx/scholar?q=--keywords--', 1, 'https://plus.google.com/_/favicon?domain=scholar.google.com');
  searchenginearray[21] = new Array('DOAJ', 'query', 'doaj.', 'http://www.doaj.org/doaj?func=search&template=&uiLanguage=es&query=--keywords--', 1, 'http://www.doaj.org/favicon.ico');
  searchenginearray[22] = new Array('About:DeepWeb', 'q', '.gov', 'http://www.google.com.mx/search?hl=auto&safe=off&q=--keywords--', 1, 'http://www.forodirecto.com/customavatars/avatar51207_1.gif');
  searchenginearray[23] = new Array('ScienceResearch', 'fullRecord:', 'scienceresearch.', 'http://www.scienceresearch.com/scienceresearch/result-list/collections:SCRNATGEONEWS,SCRRSP,SCRSPIR,MED-FDAD,SCRAMS,SCRNSCAN,SCRGEOSA,SCRIUPAC,SCRARST,SCRCDER,SCIDERDA,INTUTE,SCRSD,SCRTHIEME,CDER,SCRENVSE,ESN-CRS,MEDLINEPLUS-2,SCRCDC,SCRNPDB,SCRSN,SCRNIH,SCRNASWEB,SCRTRIS,DTIC-RDDS,SCRNATACS,SCRWWS,SCRPIX,SCRNGESPI,SCRFAOUN,SCRDERDP,ASMINTL,DOE-PATENT-XML,HEL-COCLIB,SCRAME,SCRELSD,NGC,EF-NSR,SCRYANWS,DTIC-SCAMPI,SCRBIOONE,SCRNSF,SCIGOV-SIMPLE,SCRCHMID,HEL-FEDST,SCRDTICST,SCRNCI,SCRAAOB,SCRHIPUBC,SCRUPTO,SCRJOURW,SCRASJOUR,SUSMS,SCRCIPO,SCROXPRESS,SCREPAOD,SCRNBIOII,SCRAAELIB,SCREPRINT,SCRBIOCEN,SCRNIHCA,SCRWINS,MED-EKNIH,MEDHM,SCRSPACE,SCRCALAS,SCRDOEIB,SCRASCE,MRS-NOPR,MED-WEB,SCRASM,SCRELESOC,SCREPAPF,SCRSPD,SCRACM,EPOJP,SCREPATIP,SCRASLOC,SCRBLD,SCRHUBST,SCRACS,SCRSCONP,SCRMMMS,SCRBAND,SCRUGCS,EPOEP,CANRESCOUNCIL,HEL-NPRATD,SCRAGRB,AMA-JAMA,SCRNNIE,SCRENHP,SCRNOAAPL,TOXNETTOXLINE,NSSN,SCREPRI,SCRAGRA,SCRBENVRAD,SCRBIJOUR,HEL-MAYO,SCRTEKTRAN,SCRESWIB,SCROEP,SCRIEA,SCRSCH,MEDPUBMED,SCRAAPHY,SRCGISDB,SCRUDLOS,SCRANNRE,EPOWIPO,SCRIAEA,SCRRSCPUB,SCRENVPA,SCRASPB,SCRUCPJ,SCRBITERA,SCRSSCTR,HEL-WHO,FDA-CBER,SIAM,DOD-WST,SCREIA,SCRSCIMAG,SCRMSE,SCRUSGSPUBS,SCRSEGEO,SCREVMGR,SCRDOAR,SCRSPEXP,SCRNTIS,SCRKARPUB,SCRIGCNT,SCRNATAP,SCRNACE,SCRWMW,SCREPASI,SCRDODBIOR,SCRCELLPR,SRBIOMB,SCRARMP,SCRNICHWBK,SCRBIOIO,SCRNASTRS,SCRSCIA,SCRAPI,SCRGPD,SCRUSECS,SCRBFIN,SCRCFSAN,SCRNASADS,SCRFERM,PUBMED,SCITOPIA,SCRMCB,AACR,SCRERSD,SCROAISTER,SCRHRD,SCRPROSOC,SCRANS,DTIC-AULIMP,SCRPLANDS,SCRESCU,SCRNISTDG,SCRNATPUGR,SCRTFGRP,DOE-RDPROD,SCRUSDEPD,CLINICALTRIALS,SCRPOP,SCISPR,HEL-IMPRO,SCRIEEE,SCRSSSAJ,NLSWED-KAROLINSKA,SCRHWPRE,SCREF/fullRecord:--keywords--', 1, 'http://scienceresearch.com/scienceresearch/images/favicon.ico');
  searchenginearray[24] = new Array('SciTation', 'possible1', 'scitation.aip.', 'http://scitation.aip.org/vsearch/servlet/VerityServlet?KEY=FREESR&possible1=--keywords--&smode=results&possible1zone=article&maxdisp=10', 1, 'http://scitation.aip.org/jhtml/css/css_images/favicon.ico');
  searchenginearray[25] = new Array('ScienceAxelerator', 'fullRecord:', 'scienceaccelerator.', 'http://www.scienceaccelerator.gov/dsa/result-list/fullRecord:--keywords--', 1, 'http://www.google.com/s2/favicons?domain=www.scienceaccelerator.gov');
  searchenginearray[26] = new Array('Hakia', 'q', 'hakia.', 'http://www.hakia.com/web?q=--keywords--', 0, 'http://www.hakia.com/favicon.ico');
  searchenginearray[27] = new Array('MS Academic', 'query', 'academic.research.microsoft.', 'http://academic.research.microsoft.com/Search?query=--keywords--', 0, 'http://www.google.com/s2/favicons?domain=www.microsoft.com');
  searchenginearray[28] = new Array('Redalyc', 'idFrmBuscaArt%3AidInputArticulo', 'redalyc.org', 'http://www.redalyc.org/homeBasic.oa?idFrmBuscaArt=idFrmBuscaArt&idFrmBuscaArt%3AidBtnBuscaArt=Search&javax.faces.ViewState=j_id9&idFrmBuscaArt%3AidInputArticulo=--keywords--', 1, 'http://www.redalyc.org/img/r.ico');
  searchenginearray[29] = new Array('SpringerLink', 'query', 'link.springer.', 'http://link.springer.com/search?query=--keywords--', 1, 'https://plus.google.com/_/favicon?domain=link.springer.com');
  searchenginearray[30] = new Array('EbookBrowse', 'q', 'ebookbrowse.', 'http://ebookbrowse.com/srch.php?q=--keywords--', 1, 'http://www.ebooks.com/favicon.ico');
  searchenginearray[31] = new Array('DocJax', 'q', 'docjax.', 'http://docjax.com/Search/index.shtml?q=--keywords--', 0, 'http://thedjlist.com/favicon.ico');
  searchenginearray[32] = new Array('Open-Video', 'terms', 'open-video.', 'http://www.open-video.org/results.php?view=bg_thumbs&search_field=all&terms=--keywords--&genre=%25&size=any&format=0&color=%25&sound=%25&submit=Search', 1, 'http://www.google.com/s2/favicons?domain=www.open-video.org');
  searchenginearray[33] = new Array('TechXtra', 'query', 'techxtra.', 'http://www.techxtra.ac.uk/index.html?action=search&query=--keywords--&search_landscape_id=100&searcher=Go&id=&search_type=basic&mainaction=search&nrresults=20', 1, 'http://www.google.com/s2/favicons?domain=www.techxtra.ac.uk');
  searchenginearray[34] = new Array('WorldWideScience', 'fullRecord', 'worldwidescience.', 'http://worldwidescience.org/wws/result-list/collections:WWS-DATACITE,WWS-CABI,ISTIC-EN,VTT-PUB-EN,IREVUES-FR,WWS-NASA,DEFFRD-EN,WWS-LSDCS-JA,WWS-AGRIS,CSIR-SA-EN,DRIVER-FR,ASIAJOL,ISTIC-ZN-CHS,WWS-KOMED-EN,UCATSCLIT-RU,DRIVER-JA,INIST-EN,TIBGERLIB-DE,VINITIEARTH-RU,WWS-SYNAPSE,NLNZ-RSNZ-EN,WWS-SOAJ,ISE-EN,LITHUANIA-VLIB-EN,DRIVER-ZH-CHS,J-STORE-EN,BLDIRECT-EN,WWS-PLEIADI,AADC-EN,WWS-CERN-MULTIMEDIA,LAJOL,WWS-SCICINEMA,TIBGERLIB-EN,DRIVER-PT,INDMEDCEN-EN,KISTI-KSART-EN,BAZTECH-PL,AJOL-SA-EN,ETD-IIS-EN,J-STAGE-EN,WWS-OPSIGLE-EN,ERFF-EN,INIST-FR,WWS-ARXIV,CZAPUBDB-EN,NORRES-EN,LENUS-EN,WWS-KAUST,WWWISTC-EN,CZASCIR-EN,SCIELO-PT,IO-PORT-DE,UKPMC-EN,WWS-TSTOR,IAEA-INISDB-EN,NDC-GR-EL,DGITALCSIC-ES,IISC-EN,WWS-MEDLINEPLUS,SCIGOV-EN,WWS-KSP-EN,WWS-LSD-JA,DRIVER-RU,SCIELO-EN,WWS-EMBL,SCIELO-ES,WWS-LSDCS-EN,VTT-RSC-EN,DRSNIO-EN,DOAJ-ART-EN,J-EAST-JA,NARCIS-EN,WWS-DNADATA,IDRC-EN,WWS-CDC,WWS-DRYAD,WASTENET,WBC-PL,WWS-ICSU,DRIVER-ES,GOOGLE-IAS-EN,INSERM-FR,J-STAGE-JA,LNBLV-NATLIB-EN,DRIVER-EN,STPUBTLIB-RU,CISTI-EN,WWS-SCPINFO,VINITIPROJECTS-RU,WWS-INTEGBIO-JA,J-EAST-EN,WWS-BIOGRID,DRIVER-DE,ARROW-EN,WWSCERN-EN,ETDEWEB-EN,WWS-NATSCIFOUND,WWS-LSD-EN/preferredLanguage:en%2Cen/fullRecord:--keywords--', 1, 'http://www.google.com/s2/favicons?domain=worldwidescience.org');
  ...
  var qvar;
  var linkstyle = '';
  var footerlinkstyle = "text-align: left; white-space: nowrap; text-decoration: bold; background: #383838; margin: 0 10px 3px 10px; padding: 3px 8px 3px 8px; display: block; color: #6cebeb; font-size: 80%;";
  ...
  if ((q.length > 0) && (e != -1)) {
    ...
    function make_boxes() {
        if ((!b) && (keywords != '')) {
            ...
            b.setAttribute("style","position: fixed; bottom: 0px; right: 0; padding: 10px 0; background: #151515; border: 1px solid #ccc; border-right: 0; width: 143px; overflow: hidden;");
        for (i = 0; i < searchenginearray.length; i++) {
          if (searchenginearray[i][4] == 1) {
            linkstyle = "text-align: left; white-space: nowrap; text-decoration: none; background:  url('" + searchenginearray[i][5] + "') #383838 1px center no-repeat; margin: 0 10px 3px 10px; padding: 3px 8px 3px 20px; display: block; color: #6cebeb; font-size: 80%;";
            ...
            b.appendChild(lg[i]);
                ...
                ls.setAttribute("style", "text-decoration: none; margin: 0 10px 3px 10px; padding: 3px 8px 3px 8px; display: block; color: #b5ed6d; font-size: 80%; overflow: hidden;");
        addtext(ls, " ");
                b.appendChild(ls);
                lc = document.createElement("a")
                lc.setAttribute("style", footerlinkstyle);
                lc.setAttribute("href","http://www.chemedia.com/index.shtml");
                lc.setAttribute("onClick","return true;");
        addtext(lc, 'Books&Docs Browser \u00BB');
                b.appendChild(lc);
...

{
  ...
  var searchenginearray = new Array();
  var lg = new Array(); 
  searchenginearray[0] = new Array('> About:DeepWeb', 'q', 'taringa.net', 'http://www.google.com.mx/search?q=(MP3|FLAC|AAC|AC3|MKV|DVD|DVDRip|BluRay|BDRip|AVI|RMVB|EXE|ISO|RAR|ZIP|PDF|EPUB|DJVU|MediaFire|DropBox|ZippyShare|2Shared|Mir.Cr|GDrive|SkyDrive) site:taringa.net&q=--keywords--', 1, 'http://www.forodirecto.com/customavatars/avatar51207_1.gif');
  searchenginearray[1] = new Array('1B Content&Date', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=23&type=cuerpo&sort=fecha&p=1', 1, 'http://docstoc.com/favicon.ico');
  searchenginearray[2] = new Array('1D Content&Date', 'q', 'tainga.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=9&type=cuerpo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.maniadownload.com');
  searchenginearray[3] = new Array('1L Content&Date', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=2&type=cuerpo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.publimetro.com.mx');
  searchenginearray[4] = new Array('1M Content&Date', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=8&type=cuerpo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.radioguide.fm');
  searchenginearray[5] = new Array('1T Content&Date', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=13&type=cuerpo&sort=fecha&p=1', 1, 'http://www.cinemorelos.com/favicon.ico');
  searchenginearray[6] = new Array('2B Content&Points', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=23&type=cuerpo&sort=puntos&p=1', 1, 'http://docstoc.com/favicon.ico');
  searchenginearray[7] = new Array('2D Content&Points', 'q', 'tainga.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=9&type=cuerpo&sort=puntos&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.maniadownload.com');
  searchenginearray[8] = new Array('2L Content&Points', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=2&type=cuerpo&sort=puntos&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.publimetro.com.mx');
  searchenginearray[9] = new Array('2M Content&Points', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=8&type=cuerpo&sort=puntos&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.radioguide.fm');
 searchenginearray[10] = new Array('2T Content&Points', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=13&type=cuerpo&sort=puntos&p=1', 1, 'http://www.cinemorelos.com/favicon.ico');
 searchenginearray[11] = new Array('3B Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=23&sort=fecha&p=1', 0, 'http://docstoc.com/favicon.ico');
 searchenginearray[12] = new Array('3D Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=9&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.maniadownload.com');
 searchenginearray[13] = new Array('3L Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=2&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.publimetro.com.mx');
 searchenginearray[14] = new Array('3M Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=8&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.radioguide.fm');
 searchenginearray[15] = new Array('3T Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=13&sort=fecha&p=1', 0, 'http://www.cinemorelos.com/favicon.ico');
 searchenginearray[16] = new Array('4B Date&Title', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=23&type=titulo&sort=fecha&p=1', 1, 'http://docstoc.com/favicon.ico');
 searchenginearray[17] = new Array('4D Date&Title', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=9&type=titulo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.maniadownload.com');
 searchenginearray[18] = new Array('4L Date&Title', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=2&type=titulo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.publimetro.com.mx');
 searchenginearray[19] = new Array('4M Date&Title', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=8&type=titulo&sort=fecha&p=1', 1, 'http://www.google.com/s2/favicons?domain=www.radioguide.fm');
 searchenginearray[20] = new Array('4T Date&Title', 'q', 'taringa.', 'http://www.taringa.net/buscar/posts/?q=--keywords--&category=13&type=titulo&sort=fecha&p=1', 1, 'http://www.cinemorelos.com/favicon.ico');
 searchenginearray[21] = new Array('5B Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=23&sort=fecha&p=1', 0, 'http://docstoc.com/favicon.ico');
 searchenginearray[22] = new Array('5D Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=9&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.maniadownload.com');
 searchenginearray[23] = new Array('5L Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=2&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.publimetro.com.mx');
 searchenginearray[24] = new Array('5M Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=8&sort=fecha&p=1', 0, 'http://www.google.com/s2/favicons?domain=www.radioguide.fm');
 searchenginearray[25] = new Array('5T Date&Tags', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=13&sort=fecha&p=1', 0, 'http://www.cinemorelos.com/favicon.ico');
 searchenginearray[26] = new Array('. Tags!Date&Title', 'q', 'taringa.', 'http://www.taringa.net/tags/--keywords--/?category=0&sort=fecha&type=titulo&p=1', 1, 'http://www.taringa.net/favicon.ico');
 searchenginearray[27] = new Array('. Subtitles', 'subtitles/', 'all4divx.', 'http://www.all4divx.com/subtitles/--keywords--/Spanish/1', 1, 'http://www.scirus.com/favicon.ico');
 searchenginearray[28] = new Array('> About:Science', 'query', 'tdx.', 'http://www.tdx.cat/search?query=--keywords--', 1, 'http://www.google.com/images/icons/product/scholar-16.png');
 searchenginearray[29] = new Array('. Online Radio', 'Internet-Radio/', 'shoutcast.', 'http://www.shoutcast.com/Internet-Radio/--keywords--', 1, 'http://storagenoticiasmvs.blob.core.windows.net/audio/player/imagenes/misc/microphone.png');
 searchenginearray.sort();
  var r = escape(document.referrer);
  var u = escape(document.location.href);
    var b = document.getElementById("searchSideBar");
  ...
  if ((q.length > 0) && (e != -1)) {
    ...
    function make_boxes() {
        if ((!b) && (keywords != '')) {
            b = document.createElement("div");
            b.setAttribute("id","searchSideBar");
            b.setAttribute("style","position: fixed; bottom: 0.5px; left: 0.5px; padding: 10px 0; background: #151515; border: 1px solid #ccc; border-left: 1px; width: 140px; overflow: hidden;");
        for (i = 0; i < searchenginearray.length; i++) {
          if (searchenginearray[i][4] == 1) {
            linkstyle = "text-align: left; white-space: nowrap; text-decoration: none; background:  url('" + searchenginearray[i][5] + "') #383838 1px center no-repeat; margin: 0 10px 3px 10px; padding: 3px 8px 3px 20px; display: block; color: #6cebeb; font-size: 80%;";
        ...
        addtext(ls, " ");
                b.appendChild(ls);
                lh = document.createElement("a")
                lh.setAttribute("style", footerlinkstyle);
                lh.setAttribute("href","http://www.taringa.net/KeyserSoxe");
                lh.setAttribute("onClick","return true;");
        addtext(lh, "[+] About:KeyserSöze \u00BB");
                b.appendChild(lh);
        lh.style.textAlign = 'left';
            document.body.appendChild(b);
      b.addEventListener("click", toggle_box, false);
            return true;
        }
...

{
   ...
   var searchenginearray = new Array();
   var lg = new Array(); 
   searchenginearray[0] = new Array('FTP Audio', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(MP3|AAC|FLAC|RA|M4A|APE|OGG|WMA|OFR|TTA|AC3|ALAC) intitle:"index.of" "Parent Directory" -asp -php -htm -html&q=--keywords--', 1, 'http://audioysonido.blogspot.com/favicon.ico');
   searchenginearray[1] = new Array('FTP Cinema&TV', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(MKV|AVI|MP4|MOV|MPEG|WMV|RMVB|VOB) intitle:"index.of" "Parent Directory" -htm -html -asp -php -musicindexof -inurl:htm -inurl:html -inurl:php&q=--keywords--', 1, 'http://www.google.com/images/icons/product/code-16.png');
   searchenginearray[2] = new Array('MultiMedia', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=filetype:rmvb OR filetype:avi OR filetype:mkv OR filetype:mov OR filetype:mp4 OR filetype:mp3 OR filetype:ra OR filetype:flv OR filetype:flac OR filetype:m4a OR filetype:alac OR filetype:ape OR filetype:ofr OR filetype:wma OR filetype:wmv OR filetype:gif OR filetype:rm OR filetype:swf&q=--keywords--', 1, 'https://plus.google.com/_/favicon?domain=play.google.com');
   searchenginearray[3] = new Array('OnLine Music', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:grooveshark.com OR site:mp3fusion.net OR site:ivoox.com OR site:goear.com OR site:beemp3.com OR site:wuzam.com OR site:findsounds.com OR site:ascoltaremusicagratis.com OR site:live365.com OR site:airmp3.me OR site:musicmp3spb.org&q=--keywords--', 1, 'https://plus.google.com/_/favicon?domain=www.radioguide.fm');
   searchenginearray[4] = new Array('FTP Folder', 'q', '.info', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(ISO|RAR|MP3|DOC|MKV|DJVU|7Z|FLAC|PPT|AVI|EPUB|ZIP|M4A|XLS|MP4|PDF) "Parent Directory" intitle:"index.of" "Name" "Last modified" "Size" "Description" -inurl:htm -inurl:html -inurl:php -idftp&q=--keywords--', 1, 'http://www.google.com/s2/favicons?domain=email.uoa.gr');
   searchenginearray[5] = new Array('Free Forums', 'q', '.co.uk', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:getmediafire.net OR site:gratisprogramas.org OR site:telechargementz.org OR site:forodirecto.com OR site:portalnet.cl OR site:avaxho.me OR site:intercambiosvirtuales.org OR site:ba-k.com OR site:jarochos.net OR site:x-caleta.com OR site:zone-telechargement.com OR site:vagoslatino.com OR site:webwarez.it OR site:downtwarez.com OR site:metalwarez.com OR site:alllossless.com OR site:metalwarez.in.ua OR site:jayfan.com OR site:pordescargadirecta.com OR site:israbox.com OR site:megafire.org OR site:heroturko.me&q=--keywords--', 1, 'http://www.f-haus.de/favicon.ico');
   searchenginearray[6] = new Array('.. Torrents', 'q', '.ca', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:torrent-finder.info OR site:thepiratebay.org OR site:isohunt.com OR site:extratorrent.com OR site:torrentportal.com OR site:btloft.org OR site:torrentbit.net OR site:torrentreactor.net OR site:torrentz.eu OR site:kat.ph OR site:btjunkie.org OR site:demonoid.me OR site:torrentdownloads.net OR site:torrentreactor.net OR site:limetorrents.com&q=--keywords--', 1, 'http://torrentz.eu/favicon.ico');
   searchenginearray[7] = new Array('PornFlakes', 'q', '.ru', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:pornbusters.org OR site:pornfactor.net OR site:fapdu.com OR site:xxxbunker.com OR site:porntube.com OR site:poringa.net OR site:4tube.com OR site:vporn.com OR site:xhamster.com OR site:pornhub.com OR site:sockshare.com OR site:putLocker.com OR site:share-online.biz OR site:webwarez.it OR site:intporn.com OR site:mywebporno.com&q=--keywords--', 1, 'https://plus.google.com/_/favicon?domain=www.ixxx.com');
   searchenginearray[8] = new Array('Music', 'q', '.co.uk', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(SkyDrive|MP3|FLAC|M4A|BlogSpot|GDrive) "Ge.Tt" OR "MediaFire" OR "Share-Online" OR "SendSpace" OR "DepositFiles" OR "ZippyShare" OR "DivShare" OR "ShareBeast" OR "PutLocker" OR "Mega" OR "DropBox" OR "MyUpload" OR "SockShare" OR "BillionUploads"&q=--keywords--', 1, 'http://files.myopera.com/TrikiTran/albums/11598242/Musicico.gif');
   searchenginearray[9] = new Array('FTP Books', 'q', '.br', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(DJVU|CHM|CBR|NFO|EPUB|PDF) "Parent Directory" intitle:"index.of" "Name" "Last modified" "Size" "Description" -inurl:htm -inurl:html -inurl:php -htm -html -php&q=--keywords--', 1, 'http://www.physics.umn.edu/favicon.ico');
  searchenginearray[10] = new Array('E-Books', 'q', '.pt', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:quedelibros.com OR site:freelibros.com OR site:ebookmeme.com OR site:librosenepub.blogspot.com OR site:epubgratis.es OR site:manybooks.net OR site:ebooksplanet.net OR site:librostown.blogspot.mx OR sie:scienceunseen.com&q=--keywords--', 1, 'http://www.ebooks.com/favicon.ico');
  searchenginearray[11] = new Array('FileHosting Links', 'q', '.de', 'http://www.google.com.mx/search?hl=auto&safe=off&q="Ge.Tt" OR "MediaFire" OR "BillionUploads" OR "SendSpace" OR "DepositFiles" OR "ShareBeast" OR "ZippyShare" OR "DivShare" OR "Share-Online" OR "PutLocker" OR "Mega" OR "DropBox" OR "2Shared" OR "MyUpload" OR "Mir.Cr" OR "BlogSpot" OR "SockShare" OR "GDrive" OR "SkyDrive"&q=--keywords--', 1, 'http://www.google.com/s2/favicons?domain=googledrive.com');
  searchenginearray[12] = new Array('Subtitles', 'q', '.fr', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(SRT|SUB|IDX|SSA|TXT|RT|ASS|PSB|S2K|LRC|SMI|MKS|SMIL|SSF|USF) site:all4divx.com OR site:subdivx.com OR site:subscene.com OR site:divxtitles.com OR site:divxsubtitles.net OR site:opensubtitles.com&q=--keywords--', 1, 'http://www.subdivx.com/favicon.ico');
  searchenginearray[13] = new Array('Documents', 'q', '.it', 'http://www.google.com.mx/search?hl=auto&safe=off&q=filetype:doc OR filetype:docx OR filetype:rtf OR filetype:txt OR filetype:odt OR filetype:djvu OR filetype:pdf OR filetype:epub&q=--keywords--', 1, 'http://www.google.com/images/icons/product/docs-16.png');
  searchenginearray[14] = new Array('Papers|Books|Journals', 'q', '.es', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(DJVU|SkyDrive|BlogSpot|EPUB|DOC|PPT|XLS|PDF|GDrive) "MediaFire" OR "SendSpace" OR "ZippyShare" OR "DivShare" OR "ShareBeast" OR "PutLocker" OR "Mir.Cr" OR "DropBox" OR "BillionUploads" OR "Mega" OR "Share-Online"&q=--keywords--', 1, 'http://www.docstoc.com/favicon.ico');
  searchenginearray[15] = new Array('Presentations', 'q', '.org', 'http://www.google.com.mx/search?hl=auto&safe=off&q=filetype:ppt OR filetype:pps OR filetype:odp OR filetype:otp OR filetype:sxi OR filetype:pot&q=--keywords--', 1, 'http://www.naturespace.com/favicon.ico');
  searchenginearray[16] = new Array('Blogs', 'q', '.ar', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(RAR|BDRip|MKV|FLAC|MP3|DJVU|PDF|MirrorCreator|ZippyShare|ShareBeast|PutLocker|SkyDrive|SendSpace|DivShare|MediaFire|Mega) site:blogspot.com OR site:wordpress.com OR site:freeblogit.com OR site:edublogs.org OR site:tumblr.com OR site:scienceblogs.com&q=--keywords--', 1, 'http://www.base-search.net/interface/images/silk/base_blog.png');
  searchenginearray[17] = new Array('DataSheets', 'q', '.mx', 'http://www.google.com.mx/search?hl=auto&safe=off&q=filetype:xls OR filetype:ods OR filetype:xlsx OR filetype:csv OR filetype:slk OR filetype:dbf&q=--keywords--', 1, 'http://www.feelnumb.com/favicon.ico');
  searchenginearray[18] = new Array('TV Series & Films', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(MKV|AVI|MP4|DVDRip|BDRip|BluRay|BlogSpot) "PutLocker" OR "SendSpace" OR "BillionUploads" OR "ZippyShare" OR "ShareBeast" OR "DivShare" OR "MyUpload" OR "Share-Online" OR "MediaFire" OR "Ge.Tt" OR "SockShare" OR "DropBox" OR "Mega"&q=--keywords--', 1, 'http://www.cinemorelos.com/favicon.ico');
  searchenginearray[19] = new Array('T! ', 'q', '.net', 'http://www.google.com.mx/search?hl=auto&safe=off&q=(MP3|FLAC|MKV|DVDRip|BluRay|BDRip|AVI|RMVB||RAR|PDF|EPUB|DJVU|MediaFire|MyUpload|DropBox|ZippyShare|2Shared|SkyDrive|ShareBeast|Mega|GDrive) site:Taringa.Net&q=--keywords--', 1, 'http://www.taringa.net/favicon.ico');
  searchenginearray[20] = new Array('Vagos', 'q', 'google.', 'http://www.google.com.mx/search?hl=auto&safe=off&q=site:vagoslatino.com&q=--keywords--', 0, 'http://www.gl.iit.edu/favicon.ico');
  searchenginearray[21] = new Array('Online Video', 'q', '.com', 'http://www.google.com.mx/search?hl=auto&safe=off&tbo=1&sout=1&prmdo=1&tbs=dur:l&tbm=vid&q=-YouTube&q=--keywords--', 1, 'http://www.filmsite.org/favicon.ico');
  searchenginearray[22] = new Array('FilesHut', '/', 'fileshut.', 'http://www.fileshut.com/find_files.php?submit=submit&q=--keywords--', 0, 'http://www.fileshut.com/favicon.ico');
  searchenginearray[23] = new Array('.. About:Science', 'query', '.ws', 'http://www.tesisenred.net/search?scope=%2F&ocult=0&query=--keywords--', 1, 'http://www.gifss.com/espacio/atomos/Atomo_21.gif');
  searchenginearray[24] = new Array('XDD', 'searchid', 'pordescargadirecta.', 'http://pordescargadirecta.com/search.php?searchid=--keywords--', 0, 'http://www.pordescargadirecta.com/favicon.ico');
  searchenginearray[25] = new Array('Ba-K', 'searchid', 'ba-k.', 'http://ba-k.com/search.php?searchid=--keywords--', 0, 'http://www.ba-k.com/favicon.ico');
  searchenginearray[26] = new Array('ForoDirecto', 'searchid', 'forodirecto.', 'http://www.forodirecto.com/search.php?searchid=--keywords--', 0, 'http://www.forodirecto.com/favicon.ico');
  searchenginearray[27] = new Array('Jarochos', 'searchid', 'jarochos.', 'http://www.jarochos.net/jportal/search.php?searchid=--keywords--', 0, 'http://www.j-lab.org/favicon.ico');
  searchenginearray[28] = new Array('FileGaga', 'q', 'filegaga.', 'http://filegaga.com/search.php?q=--keywords--', 0, 'http://filegaga.com/favicon.ico');
  searchenginearray.sort();
  var r = escape(document.referrer);
  var u = escape(document.location.href);
    var b = document.getElementById("searchSideBar");
  ...
  var qvar;
  var linkstyle = '';
  var footerlinkstyle = "text-align: left; white-space: nowrap; text-decoration: bold; background: #383838; margin: 0 10px 3px 10px; padding: 3px 8px 3px 8px; display: block; color: #151515; font-size: 80%;";
  for (i = 0; i < searchenginearray.length; i++) {
    t = searchenginearray[i][2];
    if (d.indexOf(t) != -1) {
      e = i;
  ...
  if ((q.length > 0) && (e != -1)) {
    ...
    function make_boxes() {
        if ((!b) && (keywords != '')) {
            b = document.createElement("div");
            b.setAttribute("id","searchSideBar");
            b.setAttribute("style","position: fixed; bottom: 0.5px; left: 0.5px; padding: 10px 0; background: #151515; border: 1px solid #ccc; border-left: 1px; width: 145px; overflow: hidden;");
        for (i = 0; i < searchenginearray.length; i++) {
          if (searchenginearray[i][4] == 1) {
            linkstyle = "text-align: left; white-space: nowrap; text-decoration: none; background:  url('" + searchenginearray[i][5] + "') #383838 1px center no-repeat; margin: 0 10px 3px 10px; padding: 3px 8px 3px 20px; display: block; color: #b5ed6d; font-size: 80%;";
        ...
        addtext(ls, " ");
                ...
                lc = document.createElement("a")
                lc.setAttribute("style", footerlinkstyle);
                lc.setAttribute("href","http://userscripts.org/users/426977/scripts");
                lc.setAttribute("onClick","return true;");
        addtext(lc, "[+] About:KeyserSöze\u00BB");
                b.appendChild(lc);
        lc.style.textAlign = 'left';
            document.body.appendChild(b);
      b.addEventListener("click", toggle_box, false);
...
/*		=============================
		|| Compacted Google TopBar ||
		=============================
*/

{
	
var buttonBar = ((document.getElementById("gbu") !== null) ? document.getElementById("gbu").getElementsByClassName("gbtc")[0] : null);
var tbHeight = ((document.getElementById("mngb") !== null) ? document.getElementById("mngb").offsetHeight : "");
var isFrontPage = ((document.getElementById("leftnavc") === null) ? true : false);
var css =
	//topbar
	"#mngb{ position: fixed; top: 0; width: 100%; } #main{ margin-top:" + tbHeight + "px !important; } .gssb_c{ position: fixed !important; }" +
	//appbar
	((buttonBar !== null) ? "#appbar { display: none !important; }" : "#appbar { margin-left: 204px; !important; width: auto !important;}") +
	//left navbar
	"#leftnav { position: fixed !important; top:" + tbHeight + "px !important; height:" + ((tbHeight !== "") ? window.innerHeight - tbHeight : "") + "px !important; overflow-x: hidden!important; overflow-y: auto!important; z-index: 1;}" +
	//site preview
	"#nycp { position: fixed !important; top:" + tbHeight + "px !important; left: auto !important; min-width: 54.2% !important; z-index: 4; }" +
	//padding for search results
	"#center_col { padding-top: 12px !important; }" +
	//fix image search front page
	"#body { margin-top:" + tbHeight + "px !important; }" +
	//fix low width
	((screen.width < 1000 || window.innerWidth < 1000) ? (!isFrontPage) ? "#gbu { left: auto !important; right: 0 !important; } .gbqff, #gbqf { min-width: 350px !important; max-width: 350px !important;  } #gbz .gbts { padding-left: 6px !important; padding-right: 6px !important; }" : "#gbz .gbts { padding-left: 6px !important; padding-right: 6px !important; }" : "");

//login button
(function setLButton() {
	if (window.location.toString().indexOf("https://encrypted.google.com/") === -1) {
		var topBar = document.getElementById("gb");
		var lButton = document.getElementById("gb_70");
		if (lButton !== null && topBar !== null) {
			lButton.firstChild.removeAttribute("class");
			lButton.parentNode.setAttribute("id", "gbz");
			lButton.firstChild.firstChild.setAttribute("class", "gbts");
			lButton.parentNode.setAttribute("style", "top: 2px!important; right: 0!important; left: auto!important; z-index: 1000;");
			css += "#gb_70 { font-size: 13px; }";
			topBar.appendChild(lButton.parentNode);
		}
	}
})();
//profile button
(function setPButton() {
	...
	var pButton = document.getElementById("gbg4");
	if (pButton !== null && topBar !== null) {
		topBar.appendChild(pButton.parentNode);
		pButton.parentNode.setAttribute("style", "position: fixed !important; top: 2px!important; right: 0!important; left: auto!important; height: 28px; z-index: 1000;");
		css += "#gbgs4d { color: #BBBBBB !important; height: 28px; } #gbgs4d:hover { color: #FFFFFF !important; } #gbs { top: 50px!important; }";
	}
})();
//config, safe search and search tools buttons and for google instant
(function setGIButtons() {
	var setButtons = function () {
		var ssButton = document.getElementById("ab_ctl_ss");
		var cButton = document.getElementById("ab_ctl_opt");
		var stButton = document.getElementById("hdtb_tls");
		var set = true;
		if (ssButton !== null) {
			buttonBar.appendChild(ssButton);
		} else {
			set = false;
		}
		if (cButton !== null) {
			buttonBar.appendChild(cButton);
			css += "a.ab_button { height: 28px !important; min-width: 10px !important; } .ab_ctl { margin-left: 8px!important; }";
		} else {
			set = false;
		}
		if (stButton !== null && document.getElementById("gbqfbw") !== null) {
			document.getElementById("gbqfbw").appendChild(stButton);
			css += ".hdtb-tl { background: -moz-linear-gradient(center top , #F5F5F5, #F1F1F1) repeat scroll 0 0 transparent; border: 1px solid #D7D7D7; color: #333333; } #hdtb_tls { color: #444444 !important; font-weight: bold !important; font-size: 11px !important;} .hdtb-tl-sel { background-color: #EEEEEE !important; background-image: -moz-linear-gradient(center top , #EEEEEE, #E0E0E0) !important; border: 1px solid #CCCCCC !important; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.1) inset !important; color: #222222 !important; } #top_nav { position: relative; top: -102px !important; } #hdtbMenus { position: fixed !important; } #hdtbMenus.hdtb-td-o { top:" + tbHeight + "px !important; } hdtb-mn-gp { width: 20px !important; } #center_col { margin-left: 20px !important; padding-top: 0 !important; } #hdtb-mn-gp { width: 20px !important; }#nycp { min-width: 62.1% !important; z-index: 104 !important; } .mw {  position: relative; top: -18px; }";
		} else {
			set = false;
		}
		return set;
	...
	var setAgain = function () {
		...
		if (screen.width < 1000 || window.innerWidth < 1000) {
			css += "#gbu { left: auto !important; right: 0 !important; } .gbqff, #gbqf { min-width: 350px !important; max-width: 350px !important; }";
		}
		window.removeEventListener("hashchange", setAgain, true);
	};
	if (buttonBar !== null) {
		// Set buttons, if Google instant add listener if front page and not image search
		if (!setButtons() && document.getElementById("prm") === null && isFrontPage) {
			window.addEventListener("hashchange", setAgain, true);
		}
	}
})();
...
function GetUserLanguage()
{
	...
	var languagePattern = new RegExp("intl/([a-z]{2,3}(?:-[0-9A-Z]{2,3}|-[a-z]{4,})?)/about.html", "");
	var anchors = document.getElementById("fll").getElementsByTagName("a");
	if (anchors.length > 0)
	{
		for (var key in anchors)
		{
			if (anchors[key].getAttribute("href").match(languagePattern))
			{
				language = anchors[key].getAttribute("href").match(languagePattern)[1];
				break;
			}
		}
	}	
