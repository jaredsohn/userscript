...
// @description Twitter Webに 引用URLボタンを追加します。BASED on traditional rt for twitter via http://naonie.com/projects/twitter_rt.html
// @version 0.1.1
...
var TwitterRT = {
    ...
    rt_action_template: function() {
        return ['<li class="action-quote-container">',
                '<a class="with-icn js-action-quote" title="ET" href="#">',
                '<i class="action-quote"></i>', '<b>ET</b>', '</a>',
                '</li>'].join("");
    },
    ...
    click_rt: function(e) {
        ...
        $ID_tweet = $tweet.attr("data-tweet-id");
    ...
    tweet_text: function($tweet, screen_name) {
        var $tweet_text = $tweet.find(".js-tweet-text"),
            ...
            len = children.length,
            texts = [], tmp, $child,
            components = ["" ,["https://twitter.com/",screen_name,"/status/" ,$ID_tweet].join("")];
