// ==UserScript==
// @name           KOC Organizador del Trono (Traducido por Anibal)
// @version        20121223ESP
// @namespace      mmm
// @homepage       http://userscripts.org/scripts/show/139597
// @delay 1000
...
// @updateURL      https://userscripts.org/scripts/source/139597.meta.js
// @downloadURL    http://userscripts.org/scripts/source/139597.user.js
...
function trStartup (){
   ...
   if (!unsafeWindow.cm)
    ...
    var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
        ...
        div.cardOverlay { font: cracked; width: 100%; font-size:3.5em; position: absolute; left: 0%; top: 50%; color: red; text-align: center; text-shadow: 2px 2px 4px #000;} \
    ...
    // clear some styles
    var styles2 = ".ui-widget-content { font-size: 1.0em; background: none; border: none;}" +
            " .ui-tabs .ui-tabs-nav li a { font-weight: bold; font-family: georgia,arial,sans-serif; color: white; font-size: 1.2em; background: url('https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/tab_unselected.png') no-repeat scroll 0% 0% transparent;}" +
            " .ui-tabs .ui-tabs-nav li.ui-tabs-active a {   background: url('https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/tab_selected.png') no-repeat scroll 0% 0% transparent;}" +
            ".ui-widget-content a { color: white;}" +
            "a.buttonDown20 span { color: white;}" +
    		".ui-widget {font-size: 1.0em;}" +
    	    ".ui-dialog .ui-dialog-title {margin: 0px; width: 100%;}" +
    		".ui-widget-header {background: none; border: none;}" +
    		".ui-state-deafault {background-color: none; }" +
    		".ui-widget .ui-widget {font-size: 0.9em;}" +
    		'.ui-dialog .ui-dialog-titlebar-close span { background-position: 0px 0px; background-image: url("https://kabam1-a.akamaihd.net/kingdomsofcamelot/fb/e2/src/img/close_icon.png"); background-repeat: no-repeat; height: 20px; width: 20px; }' +
    		" .ui-dialog .ui-dialog-titlebar-close { top: 30%; width: 20px; height: 20px;}" +
    		" .ui-resizable-s { background: rgba(0,0,255,0.3);}" +
    		" .ui-tabs .ui-tabs-nav { position: relative; top: 10px; left: 15px;}" +
    ...
    var tStr= '<div style="width: auto; padding: 0px;" vAlign=top id="'+ prefix +'_bar" class="'+ topClass +' trTitle">Organizador del Trono</div>';
    ...
    mainPop = $("#tr_top")[0];
    $("div.ui-dialog-titlebar").css( {background: 'none', border: 'none'} );
    ...
    $("#tr_main").html(
            '<div>\
            <ul>\
            <li><a href="#tabs-upgrade">Actualizar</a></li>\
            <li><a href="#tabs-salvage">Eliminar</a></li>\
            <li><a href="#tabs-organize">Organizar</a></li>\
            <li><a href="#tabs-estats">Enhance Stats</a></li>\
            <li><a href="#tabs-ustats">Upgrade Stats</a></li>\
            <li><a href="#tabs-options">Opciones</a></li>\
...
function installHandlerFunctions() {
    ...
    var renderPanel2 = function(v1, v2) {
        ...
        var checkAstoneLevel = function () {
            // check limit
            var stones = Seed.resources["city" + unsafeWindow.currentcityid]["rec5"][0];
...
Tabs.throneSalvage = {
        ...
        init : function (div) {
            ...
            m += '<td colspan=3><b> dejar los aetherstones en ciudad: </b><div style="display: inline;" id=trSalvageCity /></td>';
            ...
            m += '<option value="1">Simple</option>';
            m += '<option value="2">Ordinario</option>';
            m += '<option value="3">Extraordinario</option>';
            m += '<option value="4">Epico</option>';
            m += '<option value="5">Fabuloso</option>';
            m += '</select> y superior</div></td>';

            m += '<td colspan=2 width=24%>Guardar las primeras <INPUT id=trSaveNum type=text size=3 maxlength=3 value="' + salvageData.throneSaveNum+ '"/> cartas</td>';
            m += "</tr></table><hr/>";
            ...
            m += '<tr><td>  <b>Define las cartas a guardar: </b> </td>';
            ...
            m += '  <option value="druid">Druida</option>';
            m += '</select></span></div></td>';
            m += '<td alight="left"><div><span>Card type: <select id="trCardType">';
            m += '  <option value="any">Cualquiera</option>';
            m += '  <option value="chair">Trono</option>';
            m += '  <option value="table">Mesa</option>';
            m += '  <option value="window">Ventanal</option>';
            m += '  <option value="banner">Banderin</option>';
            m += '  <option value="advisor">Consejero</option>';
            m += '  <option value="trophy">Trofeo</option>';
            m += '  <option value="candelabrum">Candelabro</option>';
            m += '</select></span></div></td>';
            m += '<td align="right"><INPUT id=trAddRule type=button value="Crear Regla"/></td>';
            m += '</tr></table>';
            ...
            m += '<div id="trNumSalv" style="text-align: center;">Numero de cartas eliminadas: '+ addCommas(salvageData.numSalvagedItems) + ' </div>';
            m += '<hr/>';
...
Tabs.organizer = {
        ...
        init : function (div){
            ...
            if (presetData.num_presets > 0) m +=  "Note: Final preset is overwritten when using local presets.";
            m +=  "</span>" ;
            m += '</td></tr>';
            ...
            m += '<td><INPUT id=trSortInactive type=checkbox '+ (  upgradeData.sortInactive ?' CHECKED':'') +'/> Include Inactive</td>';
            m +='</tr></table>';
...
Tabs.options = {
        ...
        init : function (div) {
            ...
            m += '<TR><TD><div style="white-space: pre;" ><INPUT id=trSalAnyCity type=checkbox '+ (salvageData.anyCity?' CHECKED':'') +'/> Cuando la ciudad primaria esta llena, poner aetherstones en otra ciudad. ';
            m += '    Maximo de aetherstones: <INPUT id=trMaxStone type=text size=7 maxlength=7 value="' + salvageData.maxStones+ '"/></div></td></TR>';
            ...
            m += '<TR><TD><div style="white-space: pre;" ><INPUT id=trSalUpgradeFirst type=checkbox '+ (salvageData.upgradeFirst?' CHECKED':'') +'/> Actualiza los elementos a +1 antes de eliminar.';
            
            m += ' Maximum quality: <select id="trSalUpgradeQuality">';
            m += '<option value="0">Simple</option>';
            m += '<option value="1">Ordinario</option>';
            m += '<option value="2">Extraordinario</option>';
            m += '<option value="3">Excepcional</option>';
            m += '<option value="4">Epico</option>';
            m += '<option value="5">Fabuloso</option>';
            m += '</select> </div></td></tr>';
            
            m += '<TR><td><div  class=trStat>Upgrade Options</div></td></TR>';
            m += '<tr><td width=25%>intervalo de reintento (seconds): <INPUT id=trUpRefresh type=text size=3 maxlength=3 value="' +upgradeData.retryInterval+ '"/></td></tr>';
            m += '  <tr><td colspan=2><div style="white-space: pre;"><INPUT id=trAnyCity type=checkbox '+ (upgradeData.anyCity?' CHECKED':'') +'/> Cuando la ciudad primaria es baja, utilizar aetherstones de cualquier ciudad.  ';
            m += ' Minimom numero de aetherstones: <INPUT id=trMinStone type=text size=7 maxlength=7 value="' +upgradeData.minStones+ '"/> </div></td></tr>';
            m += '<tr><td colspan=2>  <INPUT id=trWhisperCheck type=checkbox '+ (upgradeData.whisperToMe?'CHECKED ':'') +'/> Susurrarme en actualizaciones con éxito</td></tr> ';
            m += '<tr><td colspan=2>  <INPUT id=trBuffsCheck type=checkbox '+ (upgradeData.buffsOff?'CHECKED ':'') +'/> deseleccionar automaticamente </td></tr> ';
            m += '<tr><td colspan=2><INPUT id=trSafetyCheck type=checkbox '+ (upgradeData.safetyOn?'CHECKED ':'') +'/> Desactivar actualizaciones manuales si es inferior a <input id=trSafetyLimit size=10 maxlength=10 value="' + upgradeData.safetyLimit +'" /> aetherstone</td></tr> ';
            ...
            $('#trSafetyLimit').change( function (){
                upgradeData.safetyLimit = $("#trSafetyLimit").val();
                saveUpgradeData();
               }
            );
...
Tabs.upgrader = {
        ...
        qualities :  [ "Simple", 
                       ...
                       init : function (div){
                           ...
                           m += '<td width=25%><INPUT id=trRepairAll type=checkbox '+ (upgradeData.repairAll?' CHECKED':'') +'/>  Reparar todas las cartas</td>'; 
                           m += '<td width=25%/></tr>';

                           m += '  <tr><td colspan=3><div ><b>Usar aetherstones de ciudad: </b><span id="trUpgradeCity"></span></div></td>';
                           ...
                           m += '<td width=20%><div id=trActionDiv>Action: <select id="trAction">';
                           m += '<option value="upgrade">Upgrade</option>';
                           m += '<option value="enhance">Enhance</option>';
                           m += '<option value="both">Both</option>';
                           m += '</select></div></td>';
                           m += '<td width=20%><div id=trMaxDiv></div></td>';

                           m += "<td width=10%><div><INPUT id=trQueueAdd type=button value='Add'/></div></td></tr>";
                           m += '<tr><td colspan=4><hr/</td></tr>';
                                       ...
                                       try {
                                           ...
                                           if(rslt.ok){
                                               ...
                                               if (rslt.success)
                                               {
                                                   ...
                                                   {
                                                       var now = new Date();
                                                       qItem.lastUpgrade = " wiiiiii (lapin3)  Activado a " + Tabs.upgrader.qualities[y.quality] + " " + now.toDateString().substring(3,10) + " " + now.toTimeString().substring(0,8)  + " en " + qItem.triesThis + " intentos";
                                                       if (!qItem.upgrades) qItem.upgrades = [];
                                                       qItem.upgrades.push(qItem.lastUpgrade);

                                                       var msg = ' 8-) Activado '+unsafeWindow.kocThroneItems[eItem].name + ' a calidad ' + rslt.item.quality+ " en " + qItem.triesThis + " intentos. " + qItem.triesTotal + " total intentos para esta carta.";
                                                       trSuccessLog(msg);
                                                       if (upgradeData.whisperToMe) sendChat("/"+ Seed.player.name +' wiiiiii (lapin3) ' + msg);
                       ...
                       doUpgrade : function(uItem, bypass) {
                           ...
                           new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                               ...
                               onSuccess: function (transport) {
                                   ...
                                   if(rslt.ok){
                                       ...
                                       if (rslt.success)
                                       {
                                           ...
                                           if (bypass !=true)
                                           {
                                               ...
                                               t.setResult("Actualizacion Conseguida.  "  + addCommas(rslt.aetherstones) + " aetherstones usados.");
                                               t.setStatus("Esperando nueva actualizacion");
                                               unsafeWindow.cm.sounds.play("tr_success_build");

                                               var qItem = queueData.list[queueData.index];
                                               if (qItem) {
                                                   var now = new Date();
                                                   qItem.lastUpgrade = "Actualizado a +" + y.level + " " + now.toDateString().substring(3,10) + " "+ now.toTimeString().substring(0,8) + " en " + qItem.triesThis + " intentos";
                                                   
                                                   if (!qItem.upgrades) qItem.upgrades = [];
                                                   qItem.upgrades.push (qItem.lastUpgrade);

                                                   var msg = '  (bravo)  Subido '+unsafeWindow.kocThroneItems[uItem].name + ' a nivel ' + rslt.item.level + " en " + qItem.triesThis + " intentos. " + qItem.triesTotal + " total intentos para esta carta.";
                                   ...
                                   {
                                       if (bypass !=true) t.setStatus("Upgrade request not accepted.  esperando para el siguiente ciclo.");
                           ...
                           new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch53.php" + unsafeWindow.g_ajaxsuffix, {
                               ...
                               onSuccess: function (transport) {
                                   ...
                                   else
                                   {
                                       ...
                                       if (Seed.queue_throne && Seed.queue_throne.Seed.queue_throne && Seed.queue_throne.Seed.queue_throne.end)
                                       {
                                           t.repairEnd = Seed.queue_throne.end;
                                           t.repairId = Seed.queue_throne.itemId;
                                       }
