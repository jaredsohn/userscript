...
// Uppgift: att rensa DSC-forumet fr?n o?nskade inl?gg + att ge forumet en ny layout med s?krader och diskuteratbox
// Version: 0.3
...
// ==UserScript==
// @name	dscShiny
// @description	Ny layout till dagensskiva.com diskussioner. Tema steel. Blockerar idioter & highlightar dina idoler. Adds search bars. Adds Kommenteratrutan.
...
(function() {
       ...
       function xpath(query) {
        ...
        // Adds support for co-comment

      var comment_user = xpath("//input[@name='AvNamn_nick-r']").snapshotItem(0);
      var comment_user_name = comment_user.getAttribute("value");
      var comment_thread = xpath("//td[@BGCOLOR='#ccccff']/b").snapshotItem(0).innerHTML;
      var comment_url = window.location.href;
      //alert('user:'+comment_url);        
      var script_var = "var blogTool  = 'WordPress';  var blogURL  = 'http://www.dagensskiva.com/';  var blogTitle  =  'dagensskiva.com';   var postURL  = '"+comment_url+"';  var postTitle  =  '"+comment_thread+"';    var commentAuthor  = '"+comment_user_name+"';   var commentFormID   =  'FORM1';       var commentTextFieldName   =  'texten';      " ;
      var co_comment_script = document.createElement("script");
      co_comment_script.setAttribute('type', 'text/javascript');
      co_comment_script.innerHTML = script_var;
      document.body.appendChild(co_comment_script);
      
         var co_comment = document.createElement("script");
        co_comment.setAttribute('src','http://www.cocomment.com/js/cocomment.js');
        document.body.appendChild(co_comment);
      
      
function cutout(text) {
        var searchstr = '<IMG SRC="bilder/kommenterat.gif" HSPACE="3" VSPACE="3" ALT="Kommenterat">';
	...
	for (var i = 0; i < divs.snapshotLength; i++) { 
      ...
      if(name.innerHTML.indexOf(puckos[j])!=-1) {
        if(dolj_helt==1) {
                ...
                var as = entry.getElementsByTagName("a"); 
  	        for (foo = 0; foo < as.length; foo++) {
                a = as[foo]; 
                a.href=a.href.replace(/^\s+/g, '').replace(/\s+$/g, '');
                }
      ...
      if(name.innerHTML.indexOf(idoler[j])!=-1) {
        ...
        GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://www.dagensskiva.com/tenlatest.asp',
    headers: {
        'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
        'Accept': 'text/html',
    },
    onload: function(responseDetails) {
            var feed = cutout(responseDetails.responseText)
        //alert('Request for Atom feed returned ' + responseDetails.status +' ' + responseDetails.statusText + '\n\n' +'Feed data:\n' + feed);
               var newfeed = document.createElement("td");               
               newfeed.setAttribute("bgcolor","#4499FF");
               newfeed.setAttribute("class","boxar");
               newfeed.innerHTML = feed;
               diskimg.parentNode.parentNode.parentNode.parentNode.appendChild(newfeed);
               var newborder = document.createElement("td");
               newborder.setAttribute("rowspan","2");
               newborder.setAttribute("bgcolor","#000000");
               newborder.innerHTML=  '<IMG SRC="bilder/boxlinevertical.gif" WIDTH="2" HEIGHT="2">';
               newfeed.parentNode.appendChild(newborder);
    }
});
