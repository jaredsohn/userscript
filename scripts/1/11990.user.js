...
// @description    Shows an imagelink to directly view script source on userscripts.org
// @include        http://userscripts.org/*
// ==/UserScript==

//=====
var plain = true; // View Source in plain text format? true=yes; false=no
//=====


var link=document.getElementsByTagName("a");
for (var i=0; i<link.length; i++) {
  if (link[i].href.match(/\/scripts\/show\/([0-9]{1,6})$/gi) && link[i].className=="title") {
    var source=link[i].href.replace(/scripts\/show/gi, "scripts/review");
    if (plain) {
      source += "?format=txt";
    }
