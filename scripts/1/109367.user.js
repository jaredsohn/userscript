// ==UserScript==
// @name		Vide - Ne pas installer RACC
// @author		ren
...
if (url.indexOf(".the-west.") != -1 && url.indexOf("game.php") != -1) {
	...
	var CADRE_nb_icone_ville	= 15 ;
	var CADRE_nb_icone_SDF		= 4 ;
	...
	function programme_principal() 
	{
		...
		if (footer_menu_left)
		{
			...
			if(Character.home_town != null)
			{
				...
				//Artisanat
                                ajoute_icone("javascript:CharacterWindow.open();CharacterWindow.showTab('crafting');",
				//ancienne version artisanat
 				//ajoute_icone("javascript:Crafting.open();",
					"icone_artisan",
					MENU_icone_artisan) ;
				ajout_commentaire_icone("icone_artisan", ALT_commentaires["crafting"]);
			}
			else
			{
				//
