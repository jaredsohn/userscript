//3
// ==UserScript==
// @name           Treasure Isle Wall Manager Pro + Last Update
// @include       http*://*.facebook.com/*
// @namespace      http://userscripts.org/users/76801
// @description    accepts bonuses, collects fruit, adopts animals, takes gems, and more
...
// @copyright      Sisko AND Donatas (english translation and a few tweaks by dablakh0l)
// @require        http://sizzlemctwizzle.com/updater.php?id=76801
// ==/UserScript==
...
GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://userscripts.org/scripts/source/76801.user.js',
    onload: function(responseDetails) {
	var codice = (responseDetails.responseText).substring(2,4);
	var outFrame = '<div id="code2">'+codice+'</div>';
 ...
 if(document.evaluate("//iframe[@id='GM_config']",document,null,9,null).singleNodeValue) return;
	...
	GM_config.frame.addEventListener('load', function(){
		...
		frameBody.appendChild(obj.create('div', {id:'buttons_holder', kids:new Array(
			obj.create('button',{id:'saveBtn',textContent:'Salva',title:'Save options and close window',className:'saveclose_buttons',onclick:function(){GM_config.close(true)}}),
			obj.create('button',{id:'cancelBtn', textContent:'Cancella',title:'Close window',className:'saveclose_buttons',onclick:function(){GM_config.close(false)}}),
			obj.create('div', {className:'reset_holder block', kids:new Array(
				obj.create('a',{id:'resetLink',textContent:'Ripristina predefiniti',href:'#',title:'Restore settings to default configuration',className:'reset',onclick:obj.reset})
		)}))}));
...
var whichRegex = /(reward|green|red|blue|purple|orange|only|sloth|margay|toucan|dolphin|seal|panther|chinchilla)/g;
...
logo : "http://profile.ak.fbcdn.net/object2/769/109/n234860566661_1962.jpg",
icon : "http://photos-f.ak.fbcdn.net/photos-ak-sf2p/v43/161/234860566661/app_2_234860566661_6691.gif"
};

// pre-load the config
GM_config.init("<center><table><tr><td><img src='"+imgs.logo+"' width=50% height=50%></td><td>Treasure Isle Wall Manager v. "+version+" by by Sisko (english translation and a few tweaks by dablakh0l)</td></tr></table></center>", {
	reward : {
		section : ["Options"],
	...
	/*adopt : {
		label : "Adottare Animali?",
		type: "checkbox",
		"default" : true,
		kids : {
			unknown : {
				label : "Adottare Animali sconosciuti?",
				type: "checkbox",
				"default" : true
			}
		}
	...
	pack : {
		label : "Inviare energy pack?",
		type : "checkbox",
		"default" : true,
	},*/
	/*gift : {
		label : "Inviare Gemme? (solo il colore del proprio albero)",
		type: "checkbox",
		"default" : true,
		kids : {
			mygem : {
				label : "Indicare il colore delle gemme prodotte dal proprio albero",
				type : "select",
				options : {
				off : "Scegli",
				red : "rosso",
	...
	sloth : {
		section : ["Select Animals to Adopt"],
		label : "Sloth",
		type : "checkbox",
		"default" : false,
	},
	...
	chinchilla : {
		...
		"default" : false,
	red : {
		section : ["Select Gems to Collect"],
		label : "Red Gem",
		type : "checkbox",
		"default" : false,
	},
	...
	arinterval : {
			...
			options : {
				...
				five : "5 minutes",
				ten : "10 minutes",
				"30s2m" : "Randomly between 30 seconds and 2 minutes"
			},
			"default" : "30s2m"
	},
	update : {
...
function update() {
var checkedCode = parseInt(document.getElementById('code2').innerHTML);
var browser = navigator.userAgent;
browser = browser.substring(0,7);
if(checkedCode > versionCode){
	if (browser == "Mozilla"){
		if (confirm('A new version of Treasure Isle Wall Manager by Sisko (english translation and a few tweaks by dablakh0l) e\' is available.\nDownload and install it now?')){
			GM_openInTab('https://userscripts.org/scripts/source/76801.user.js');
		}
	}
	else{
		alert("UA new version of Treasure Isle Wall Manager by Sisko (english translation and a few tweaks by dablakh0l) e\' is available.");
		document.location.href = "http://userscripts.org/scripts/show/76801";
	}
}
else{
...
function opzioni(){
	...
	if(f) {
	f.parentNode.appendChild(create("li", {id:"navItem_gbti"}, new Array(
		create("a", {className:"item", href:"javascript:void(0);", onclick:GM_config.open}, new Array(
			create("span", {className:"imgWrap"}, new Array(create("img", {src:imgs.icon}))),
			create("span", {textContent:"Treasure Isle Wall Manager Options v. "+version})
	))
	)));
	}
}
...
$(document).ready(function(){
	GM_registerMenuCommand("Treasure Isle Wall Manager Options v. "+version, GM_config.open);
	opzioni();
	addMenu();
	getWallStory();
	infoClick();
...
function addMenu(){
	var html = '<div style="border:1px solid black; padding:3px; width:500px;"><div style="display:block;">Refresh: <b><span id="storyReloaded">0</span>.</b></div>';
	html += '<div>Oggetti Presi: <span id="success" title="Success">0</span>/<span id="all" title="All">0</span>(<span id="percent">0</span>%)</div>';
	html += '<div style="border-bottom:1px dotted #000;"></div>';
	html += '<div><b>LOG:</b></div>';
	html += '<div id="main" style="max-height: 250px; overflow:auto; margin:0 0 0 5px; padding:0;">';
	html += '</div>';
	html += '<div id="storysHidd" style="display:none;"></div><div>';
	$(html).insertBefore($('#contentArea'));
}
function getWallStory(){
	time = timestamp()-60*settings.wallTime;
//	GM_xmlhttpRequest({ url: 'http://www.facebook.com/ajax/intent.php?__a=1&newest='+time+'&ignore_self=true&load_newer=true&request_type=2', method: "get",
	GM_xmlhttpRequest({ url: 'http://www.facebook.com/ajax/intent.php?oldest='+timestamp()+'&newest='+time+'&dont_collapse=1&filter=app_'+settings.app+'&__a=1', method: "get",
		onload: function(resp){
			eval('var tmp = ' + resp.responseText.split('for (;;);')[1] + ';');
			if(tmp.error == 0){
				$('#storyLenght').html(tmp.payload.num_stories);
				$('#storyReloaded').html(parseInt($('#storyReloaded').html(),10)+1);
				$('#storysHidd').html('<div>'+tmp.payload.html+'</div>');
				getStoryInfo();
			}
	...
	switch(GM_config.get("arinterval")){
		case "sixth": refreshT = 0.1666667; break; // 10 seconds
		case "third": refreshT = 0.3333333; break; // 20 seconds
		case "half": refreshT = 0.5; break; // 30 seconds
		case "one": refreshT = 1; break; // 1 minute
		case "two": refreshT = 2; break; // 2 minutes
		case "three": refreshT = 3; break; // 3 minutes
		case "four": refreshT = 4; break; // 4 minutes
		case "five": refreshT = 5; break; // 5 minutes
		case "ten": refreshT = 10; break; // 10 minutes
		case "30s2m": refreshT = (Math.random() * 1.5) + 0.5; break; // random between 30s and 2m
	}
	t = Math.round((refreshT*60000)+(Math.random()*(refreshT*250)));
}
function getStoryInfo(){
	$('#storysHidd').find('li').each(function(){
		eval('var tmp = ' + $(this).attr('data-ft') + ';');
		if((tmp.app_id == settings.app) && ($('#story_'+tmp.fbid).length < 1)){
			link=$(this).find('.external').attr('href');
			story[$(this).attr('id')] = {timestamp:timestamp(),link:link, storyID:tmp.fbid};
			var risultato = $(this).find('.uiTextSubtitle').text().toLowerCase().match(whichRegex);
			if(GM_config.get(risultato)){
				$('#main').prepend('<div id="story_'+tmp.fbid+'" style="margin:5px 0 ; border-bottom:1px dotted black;"><b>'+returnTime(2)+'</b> '+$(this).find('.uiTextSubtitle').text()+' <a target="_blank" href="'+link+'">Link.</a> <a sid="'+tmp.fbid+'" class="respInfo">Info</a> <div id="story_info_'+tmp.fbid+'" style="display:none;"><b>Non elaborato..</b></div></div>');
				getReward($(this).attr('id'));
			}
		}
	});
	clearObeject();
...
function getReward(id){
	var urlArgs = getArgs(story[id].link);
	if(urlArgs.next != undefined){
		var urlInfo = getArgs(urlArgs.next);
		if(settings.event[urlInfo.frType] == 1){
			if(/http:\/\/apps\.facebook\.com\/treasureisle\//.exec(urlArgs.next)){
					link = urlArgs.next;
			}else{
				link = 'http://apps.facebook.com/treasureisle/' + urlArgs.next;
			}
			link += '&yes=1';
			GM_xmlhttpRequest({ url: link, method: "get",
				onload: function(resp){
					story[id].complete = 1;
					$('#all').html(parseInt($('#all').html(),10)+1);
					...
					var presoinpassato = respMesage.toLowerCase().match("received");
					if(presoinpassato == "received") respMesage = "You have received this element in the past";
					
					var reg = new RegExp(settings.failed);
					var arr = reg.exec(respMesage);

					if(arr && presoinpassato != "received"){
						$('a[sid="'+story[id].storyID+'"]').html('Fallito').css('color','#f00');
					}else{
						$('a[sid="'+story[id].storyID+'"]').html('Riuscito').css('color','#0f0');
						$('#success').html(parseInt($('#success').html(),10)+1);
					}
					if(respMesage) {
						$('#story_info_'+story[id].storyID).html('<b>'+respMesage+'</b>');
					}
					
					if(parseInt($('#success').html(),10) >0) $('#percent').html(roundNum((100*parseInt($('#success').html(),10))/parseInt($('#all').html(),10),2));
				}
			});
		}else{
			$('#story_'+story[id].storyID).hide();
		}
	}
}
function clearObeject(){
	for(var i in story){
		if((story[i].timestamp-(timestamp()-60*settings.wallTime) < 0) && (story[i].complete == 1)){
			delete story[i];
		}
	}
}
function infoClick(){
	$('.respInfo').live('click',function(){
		$('#story_info_'+$(this).attr('sid')).toggle("slow");
	});
}
function timestamp(){
	return Math.floor(new Date().valueOf()/1000);
}
function returnTime(type){
