// ==UserScript==
// @name           Infozentrale Plus + Last Update
...
var ICON_PLNDWIWUPOWERBACK = new Array();                     // Icon Hintergrund für Plunder, WiWu und Power
    ...
    ICON_PLNDWIWUPOWERBACK[1] = 'w2r6hhv3/plndwiwupower.GIF';
...
var ICON_FIGHT = new Array();
    ...
    ICON_FIGHT[2] = 'gz9jewwq/rot3.png';
...
function ShowImg(bgrnd, imglink, imgsource, imgtitle, imgwidth, imgheight, imgleft, imgtop, imgzindex, imgid) {
    ...
    if (bgrnd == '') {
        ...
        newdiv.innerHTML += '<span id="'+imgid+'" style="z-index:'+imgzindex+';position:absolute;top:'+imgtop+'px;left:'+imgleft+'px"><div style="cursor:move;">&nbsp;</div></span>';
...
function DispZeit(jetzt, cnt, url, Txt, back, id, x, y, z, maxTime, maxwidth) {
    ...
    var text = printf(getLangTxt(Txt), zeit, enddate);
    if (!document.getElementById(back+"img"))
        return;
    var width = Math.ceil(maxwidth / maxTime * rtime);
    ShowImg (back, url, ICON_PLUNDERA, text, maxwidth, 5, x, y, z, id+'bg');
    if (width > 0)
			    ...
			    if (content.indexOf("pet_kader") == -1) {
                               ...
                               for (var b = 0; b < petbusy.length; b++) {
                                   ...
                                   if (petbusy[b].innerHTML.indexOf('counter') != -1) {
                                       DispZeit(new Date(), petbusy[b], PET_URL, TxtStreun, "SCBACK", "streunpet", 6, (skAnz==4)?42:41, 3, 5400, 25);
                                       backflg = 2;
                                       break;
                                   }
                               }
...
function getOwnUserID() {
    try {
        ...
        var ownuserid = document.getElementById('my-profile').innerHTML.split('<a href="/profil/id:')[1];
                ...
                onload: function(responseDetails) {
                    ...
                    if (!consume && responseDetails.responseText.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                        ...
                        if (responseDetails.responseText.indexOf(href) != -1) {
                            PLUNDERUSE_URL = TOWNBASE_URL + href.substr(1);
                        ...
                        for (var i = oldVersion; i < plundertrs.length; i++) {
                            ...
                            if (!benutzbar && nrOfTabs != 6) {
                                ...
                                for (var ii = 0; ii < td.length; ii++) {
                                	if (td[ii].innerHTML.indexOf('execboost') != -1 || td[ii].innerHTML.indexOf('choose_target') != -1) {
                                		benutzbar = true;
                                		break;
                                	}
                                }
