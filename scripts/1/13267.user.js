// ==UserScript==
// @name           MySpace AIL 2 Edited: Add to address book.
// @namespace      http://myspace.com/ambientsellout
// @description    Edited version of Adrian's "Myspace Add Image Links 2" to replace the function "Suscribe to Blog" with "Add to Address Book."
// @include        http://myspace.com/*
// @include        http://*.myspace.com/*
// @exclude        http://*.myspace.com/*.viewCategory*
// ==/UserScript==
// CREDITS: The original concept and design of this script was created by
// Adrian (myspace.com/adrian232). I just edited it to add Add to Address Book.



var objCSS = document.createElement('link');
objCSS.rel = 'stylesheet';
objCSS.href = 'http://x.myspace.com/modules/messaging/static/css/messaging-003.css';
...
myscript.src = 'http://x.myspace.com/modules/messaging/static/js/common-001.js';
document.body.appendChild(myscript);

var myscript = document.createElement('script');
myscript.src = 'http://x.myspace.com/modules/messaging/static/js/myspaceajax-001.js';
document.body.appendChild(myscript);


var myscript = document.createElement('script');
myscript.src = 'http://x.myspace.com/modules/messaging/static/js/addressbook-001.js';
document.body.appendChild(myscript);
...
unsafeWindow.Type.registerNamespace("MySpace");
unsafeWindow.Type.registerNamespace("MySpaceRes");

unsafeWindow.MySpaceRes.AddressBooks = {"AreYouSureYouWantToDeleteContact":"Are you sure you want to delete the selected contacts?","ConfirmContact":"Confirm Contact","ContactAddedSuccessfully":"Contact Added Successfully!","ContactBeenAdded":"The following contact has been added to your Address Book:","ThatUserDoesntExist":"That user doesn\u0027t exist!","Close":"Close","AreYouSureAdd":"Are you sure you want to add the following user to your Address Book?","ViewProfile":"View Profile","MyspaceDotCom":"myspace.com/","First":"First","Last":"Last","Confirm":"Confirm","ThatUserAlreadyIn":"That user is already in your Address Book!","ViewDetails":"View Details","Cancel":"Cancel","EditDetails":"Edit Details","MySpaceDotCom":"myspace.com/","SendMessage":"Send Message"};

unsafeWindow.MySpaceRes.Common = {"Yes":"Yes","No":"No","Ok":"Ok","Attention":"Attention"};
unsafeWindow.MySpace.ClientContext = {"UserId":26237651,"DisplayFriendId":0,"IsLoggedIn":true};




try {
    element = document.createElement("<input name='aradiobutton' type='hidden' id='addQuickInput' />");
} catch (e) {
...
try {
    element = document.createElement("<input name='aradiobutton' type='hidden' id='jsonContactObject' />");
} catch (e) {
    element = document.createElement("input");
    element.setAttribute("name", "aradiobutton");
    element.setAttribute("type", "hidden");
    element.setAttribute("id", "jsonContactObject");
}
...
try {
    element = document.createElement("<input name='aradiobutton' type='hidden' id='ElementUpdate' />");
} catch (e) {
   element = document.createElement("input");
   element.setAttribute("name", "aradiobutton");
    element.setAttribute("type", "hidden");
    element.setAttribute("id", "ElementUpdate");
}
...
function Shortcut(name) {
	...
	if (name == 'add_friend') {
		...
		this.url = "'http://www.myspace.com/index.cfm?fuseaction=invite.addfriend_verify&friendID=' + friendID + '&MyToken=' + MyToken";
		//this.url = "'#'";
	...
	if (name == 'view_pictures') {
		...
		this.url = "'http://viewmorepics.myspace.com/index.cfm?fuseaction=user.viewPicture&friendID=' + friendID + '&MyToken=' + MyToken";
	}
	...
	if (name == 'send_im') {
		...
		this.url = "'javascript: GetQuickAddContactDetail();'";
	...
	if (name == 'subscribe_blog') {
		...
		this.url = "'javascript: addQuickInput.value=' + friendID + '; GetQuickAddContactDetail();'";
	}
	
	if (name == 'add_preferred') {
		this.description = 'Add to Blog Preferred List';
		this.url = "'http://blog.myspace.com/index.cfm?fuseaction=blog.addToPrivateList&friendID=' + friendID + '&MyToken=' + MyToken";
	}
