...
function HDYV_ProcessPlaylistVerticalView()
{
	...
	var strLiNodeXPath = "//div[@id='gh-activityfeed']/ol//li";
	if (bEditMode)
		strLiNodeXPath = "//ol[@id='playlist-video-items']/li";
	...
	{
		buttonsArea = document.evaluate("//div[contains(@class, 'playlist-actions')]", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
		if (!buttonsArea)
			return false;
		...
		for(var i=0; i<liNodes.snapshotLength; i++)
		{
			...
			(function(itemNode, bLastNode, fPercent)
			{

				// Extract all <a> tags to fix them
				var strVideoURL = "";
				var aNodes = document.evaluate(".//a", itemNode, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
				if (aNodes.snapshotLength > 0)
				{
					var urlNode = aNodes.snapshotItem(0);
					strVideoURL = urlNode.href;
				}
				var urlNodes = aNodes;
				var strVideoLink_ = strVideoURL;
				...
				GM_xmlhttpRequest({
					...
					onload:function(response)
					{
						...
						{
							// Progress
							buttonNode.setAttribute("value", fPercent.toFixed(0) + "%");
						}
						else
						{
						...
						// Extract Video ID
						strVideoLink = HDYV_ExtractVideoLink(strVideoLink_);
						if (strVideoLink == "")
							return;

						var strTitle = "";
						var bMissingData = false;
						var secondaryArea = document.evaluate(".//div[contains(@class, 'video-overview')]/*[2]", itemNode, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
						for(var j=0; j<urlNodes.snapshotLength; j++)
						{
							...
							{
								if (secondaryArea)
									strTitle = "Click to retrieve references on the same uncensured video (opens a new tab)";
								else
								{
						...
						newANode.appendChild(itemNode);
						parentNode.insertBefore(newANode, parentNode.childNodes[HDYV_m_listBrokenURLs.length]);
						HDYV_m_listBrokenURLs.push(strVideoID);

						// Fix the thumbnail due to Google optimization
						var imgNode = document.evaluate(".//img[@data-group-key]", itemNode, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
						if (imgNode)
						{
							imgNode.setAttribute("src", imgNode.getAttribute("data-thumb"));
							imgNode.removeAttribute("data-group-key");
							imgNode.removeAttribute("data-thumb");
						}
			...
			}) (liNode, i==liNodes.snapshotLength-1, (i+1)/liNodes.snapshotLength*100);
		}	
	}
