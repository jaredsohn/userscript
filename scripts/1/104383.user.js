...
var MWAddonInfo = {
    version         : '0.5.0.0',
    versiona        : '0.5.0.0 Andys Mod',
    urla            : 'http://userscripts.org/scripts/show/104383',
    name            : 'FB MafiaWars Addon',
...
var Tooltips = {
    ...
    applyTo: function(contentID) {
        ...
        if ( Util.isSet(r) ) {
            ...
            $(document).bind('mouseover.'+contentID, {'content': contentID}, Tooltips._MouseOverEvent);
        }
    },
...
function getShortURL(longURL, success, error)
{
    try {
        ...
        if (UserConfig.main) {
            api = URLShortening.shortURL[UserConfig.main.get('shortServiceID')];
        }
        if (!Util.isSet(api)) {
            if (global.xd_support === true) {
                api = URLShortening.shortURL['TinyUrl'];
            } else {
...
function getUnshortUrl(shortURL, success, error) {
    ...
    try {
        ...
        if (UserConfig.main) {
            api = URLShortening.unshortURL[UserConfig.main.get('unshortServiceID')];
        }
        if (!Util.isSet(api)) {
            if (global.xd_support === true) {
                api = URLShortening.unshortURL['unshort.me'];
            } else {
    ...
    addNew: function(plugin, callback) {
        ...
        UserConfig.plugins.load(function() {
            Util.each(UserConfig.plugins.all, function(i,v) {
                ...
                if ((success = (n === String(plugin.name).toLowerCase())) === true) {
...
var MainMenu = {
    root: [{
        ...
        name: 'Check Updates (Orig.)',
        click: {'func':Updater.check, 'params':true},
        icon: global.resource.update_menu_icon,
        css: {'border-bottom':'1px solid white'}
    }, {
        name: 'Helvetica ClanPage',
        click: Helvetica,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/cp.jpg"
    }, {
        name: 'Helvetica Battlefield',
        click: HelveticaBF,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/wp.jpg"
    }, {
        name: 'Helvetica - Gift Links',
        click: HelveticaGL,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/gl.jpg",
        css: {'border-bottom':'1px solid white'}
    }, {
        name: 'Open MW Unframed',
        click: UnframeMW,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/gm.jpg"		
    }, {
        name: 'SWITCH to FB-Profile',
        click: SwitchFB,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/gm.jpg",
        css: {'border-bottom':'1px solid white'}
    }, {
        ...
        name: 'Assasins-a-Nator',
        click: Assasins,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/gm.jpg"		
    }, {
        name: 'Zynga Message Center',
        click: ZMC,
        icon: "http://helvetica.mstuff.ch/bilder/toolbar/gm.jpg"
    }, {
        name: 'Link-a-Nator',
...
function Helvetica() {
    unsafeWindow.open('http://www.facebook.com/Helvetica.Clan');
}
function HelveticaBF() {
    unsafeWindow.open('http://www.facebook.com/Helvetica.BattleField');
}
function HelveticaGL() {
    unsafeWindow.open('http://www.facebook.com/groups/helvetica.links');
}
function UnframeMW() {
    unsafeWindow.open('http://facebook.mafiawars.zynga.com/mwfb/index.php?skip_req_frame=1&mwcom=1');
}
function SwitchFB() {
    location.href="javascript:%28function%28%29%7Bvar%20a%3Ddocument.createElement%28%22script%22%29%3Ba.type%3D%22text%2Fjavascript%22%3Ba.src%3D%22http://spocklet.com/bookmarklet/switch.js%3F%22%2BMath.random%28%29%3Bdocument.getElementsByTagName%28%22head%22%29%5B0%5D.appendChild%28a%29}%29%28%29%3B;";
}
    ...
    function addVersionText() {
        ...
        if (canvas_iframe !== null) {
            ...
            divElt.innerHTML = '<span style="font-weight: bold; font-size: 14px;">'+
                MWAddonInfo.name + ' v' + MWAddonInfo.versiona + '&nbsp;|&nbsp;' +
                '<a href="'+ MWAddonInfo.url + '">Check Updates</a>' + '&nbsp;|&nbsp;' +
                '<a href="'+ MWAddonInfo.urla + '">Mod Updates</a></span>';
