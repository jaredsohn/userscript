// ==UserScript==
// @name         insertCommentsIntoDigg.user.js
// @description  Insert Comments Into Digg.com
// @include      https://digg.com/reader*
// ==/UserScript==

/*
 * reference:     163 news rss bot, http://userscripts.org/scripts/show/77899
 * created date:  20130720
...
FullFeed.getFocusedLink = function () {
	...
	var title = getNodeBySelector([".feed-item-focused .story-title a"]);
...
FullFeed.getCurrentEntry = function() {
	...
	for (var i = 0; i < SITE_INFO.length; i++) {
		...
		if (this.link.match(reg)) {
			this.request[SITE_INFO[i].caller](i, this.link, getNodeBySelector([".feed-item-focused .detail-body"]));
			break;
		}
	}
};
...
FullFeed.request["cnbeta"] = function (index, link, body) {
	...
	var ads = body.querySelectorAll("*");
	for (var i = 0; i < ads.length; i++) {
		if (ads[i].nodeName == "BR" || (ads[i].nodeName == "IMG" && ads[i].src.indexOf("feedsportal") > -1) || (ads[i].nodeName != "P" && ads[i].innerHTML.indexOf("feedsportal") > -1)) {
...
function insertComment(commentHtml, body) {
	...
	commentDiv.setAttribute("class", "detail-body");
	commentDiv.innerHTML = commentHtml;
	body.parentNode.insertBefore(commentDiv, body.nextSibling);
}
