/*
   ...
   Updates by Glenn Harris - 09/19/2007
   ...
   select "updated gmail calendar", and click Uninstall.
   --------------------------------------------------------------------
*/

// ==UserScript==
// @name             updated gmail calendar
...
// http://www.google.com/bookmarks/find?q=label:GMgcal&output=rss
XMLURL = "http://www.google.com/bookmarks/find?q=label:" + gcalLabel + "&output=rss";
...
function initialize(){
   ...
   // define global
   deflen = 20; // default length of text units
...
function getData()
{
	...
	if (parseInt(GM_getValue("last_updated", 0)) + 3600000 > d.getTime()){
		...
		var evHTML = GM_getValue("evHTML", "");
		renderCalendar(evHTML, true);
		return;
	}
   ...
   GM_xmlhttpRequest({
      ...
      onload: function(responseDetails) { parseBookmarkXML(responseDetails.responseText); },
      onerror: function(responseDetails) { gcalStatusDIV.innerHTML = "Error Loading Bookmarks"; }
   });
}

function parseBookmarkXML(feedXML)
{
	...
	if ( responseDOM == null || responseDOM.docNode == null){
		gcalStatusDIV.innerHTML =   "<FONT style='color:red;font-size:.8em;'>Bookmark response data contained invalid XML.</FONT>";
		return;
	}
	...
	for ( var i = 0 ; i < items.length ; i ++ )
	{
		...
		getFeed(link, title, setTitle);
	}
}

function getFeed(gcalURL, bookmarkTitle, setTitle){
   ...
   //console.info("loading " + bookmarkTitle + " (" + gcalURL + ")");
   ...
   GM_xmlhttpRequest({
      ...
      onload: function(responseDetails) {
		//console.info("got response ; title: " + bookmarkTitle);
		parseCalendarXML(responseDetails.responseText, bookmarkTitle, setTitle); },
      onerror: function(responseDetails) { gcalStatusDIV.innerHTML = "Error Loading Agenda"; }
   });
}

function parseCalendarXML(feedXML, bookmarkTitle, setTitle){
   var responseDOM = new XMLDoc(feedXML, function(error){if(XmlErrorOccurred == false) XmlErrorOccurred = true;});

   if ( responseDOM == null || responseDOM.docNode == null){
      gcalStatusDIV.innerHTML =   "<FONT style='color:red;font-size:.8em;'>Calendar response data contained invalid XML.</FONT>";
      return;
   }

   var calTitle = bookmarkTitle;
   var calColour = null;
   if ( /#[0-8A-Fa-f]{6}$/.test(calTitle) )
   {
   ...
   if (typeof(feedAuthor.length) != 'undefined' && feedAuthor.length != 0) {
   ...
   for (var eventNode = 0; eventNode < eventNodes.length; ++eventNode){
      ...
      evFTitle = eventNodes[eventNode].getElements("title")[0].getText();
      evTitle = shrink(evFTitle,deflen,"...");
      evURL = eventNodes[eventNode].getElements("link")[0].getAttribute('href');

      var summaryNode = eventNodes[eventNode].getElements('summary')[0];
      var recurrenceNode = eventNodes[eventNode].getElements('gd:recurrence')[0];
      ...
      if (typeof(evAuthor.length) != 'undefined' && evAuthor.length != 0){
      ...
      for (var whoI = 0; whoI < whoNodes.length; ++whoI){
         ...
         if (whoNodes[whoI].getElements("gd:attendeeStatus").length!=0
			&& whoNodes[whoI].getElements("gd:attendeeStatus")[0].getAttribute('value').match(/.declined/)
      ...
      if(!evCancelled && !aParent[evId]){
         ...
         for (var aWheni = 0; aWheni < aEvent.length; aWheni++){
            if (new Date(aEvent[aWheni]['evEndTS']).getTime() != aEvent[aWheni]['evEndTS']) aEvent[aWheni]['evEndTS'] = aEvent[aWheni]['evstartTS'];
               addEvent(new CalendarEvent(evId,evFTitle,aEvent[aWheni]['evStartTS'],aEvent[aWheni]['evEndTS'],evURL,evLocation,evRepeatTS,evRepeatType,evRepeatInt,evRepeatByDay, calColour));
         }
      }
   ...
   var evHTML = renderCalendar(EventList, false);
   GM_setValue("evHTML", evHTML);
   var date = new Date();
...
function renderCalendar(eventA, cached){
   ...
   if ( cached == true && typeof(eventA) == 'string' )
   {
	   	evHTML = eventA;
   }
   else
   {
	   ...
	   for (var eI = 0; eI < eCount; eI++){
	      ...
	      if (std['d'] != etd['d']){
	         ...
	         //summary = event.title;
	         var summary = shrink(event.title, deflen, "...");
	         var loc = shrink(event.location, deflen, "...");
	         evHTML += "<TR><TD colspan=" + colspan + "><A class='gcalItem' style='color:" + event.color + "; font-weight: bold;' href='" + event.URL   + "' target='_blank' title='" + event.title + "'>" + summary + "</A></TD></TR>";
	         if (Dlocation && event.location != '') evHTML += "<TR><TD " +   style + "style='color:" + event.colour + ";'" +" colspan=" + colspan + " title='" + event.location + "'>&nbsp;" + loc + "</TD></TR>";
	         ...
	         var summary = shrink(event.title, deflen, "...");
	         var loc = shrink(event.location, deflen, "...");
	         evHTML += "<TR><TD colspan=" + colspan + "><A class='gcalItem' style='color:" + event.colour +"; font-weight: bold;' href='" + event.URL   + "' target='_blank' title='" + event.title + "'>" + summary + "</A></TD></TR>";
	         if (Dlocation && event.location != '') evHTML += "<TR><TD " +   style + "style='color:" + event.colour + ";'" +" colspan=" + colspan + " title='" + event.location + "'>&nbsp;" + loc + "</TD></TR>";
