// ==UserScript==
// @name           Tumblr Hide Post English
// @namespace      http://cokomik.tumblr.com/
// @description    Hides Posts from your Tumblr Dashboard. Somewhat translated by teenager.tumblr.com
// @include        http://www.tumblr.com/*
// ==/UserScript==

/*

(C) 2011 cokomik & inciwetrust
http://cokomik.tumblr.com
http://inciwetrust.tumblr.com
...
function delMute(id, post)
{
	...
	$('tunmute-' + post).fade();
	$('tmuted-' + post).toggle();*/
	$('tunmute-' + post).innerHTML = 'You sure punk?? <a style="cursor: pointer;" onclick="window.location.reload(false);">Yes</a><a href="http://cokomik.tumblr.com">.</a> ';
}
...
function tmute(post, id, newID)
{
	...
	for (var i = 0; i < hidePost.length; i++)
	{
		...
		mutePost.innerHTML = '<span id="tmuted-' + post + '" style="display:none">' + mutePost.innerHTML + '</span><span id="tunmute-' + post + '"> Post Hidden. <a onclick="delMute('+ id + ', ' + post + ')" style="cursor: pointer;">Restore</a></span>';
		//mutePost.fade();
		if (newID)
			addMute(id);
	}
