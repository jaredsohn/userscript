...
var Options = {
	...
	pbFoodAlert  : false,
    alertConfig  : {aChat:true, aPrefix:'******* I Am Under Attack..!! *******', scouting:false, wilds:false, defend:true, minTroops:1, spamLimit:10, lastAttack:0, barbautoswitch:false,},
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>Search: </td><TD>\
      <SELECT id="srcType">\
        <OPTION value=0>Tcamp</option>\
        <OPTION value=1>Wilds</option>\
    ...
    m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Cords: &nbsp; X: </td><TD class=xtab>\
      <INPUT id=srchX type=text\> &nbsp; Y: <INPUT id=srchY type=text\> &nbsp;';
	 m += '<span><select id="ptprovinceXY"><option>--Sector--</option>';
...
clickedPlayerDetail : function (span, uid){
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; Cities: '
          + u.cities +' &nbsp; Population: '+ u.population +'</td></tr><TR><TD></td><TD>Province: ';
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:black">Last Seen: '+lastLogin+'</span>';
    } else {
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    m = '<CENTER><B>Search '+ typeName +'<BR>\
        Cords: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distance: '+ t.opt.maxDistance +'<BR></center>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min.lvl:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max.lvl:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Wild Type:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Grass/Lake', 3:'Oil Field', 4:'Hill', 5:'Mountian', 6:'Plain', 0:'All', 20:'Special'}, Options.wildType );
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      ...
      m += '</select></td></tr>\
            <TR><TD class=xtab align=right>Only Cords:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
	} else {
	 m += '<TR><TD class=xtab align=right>City Type:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'All', 2:'Alliance', 3:'Friendly', 4:'Hostile', 5:'Neutral', 6:'No Alliance', 7:'Misted'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>Sort by:</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Power</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
           </select></td></tr>\
		   <TR><TD class=xtab align=right>Min.Power:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>Show cords Only:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
...
if (DEBUG_TRACE) actionLog (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['TCamp', 'Grass', 'Lake', 'Oil Field', 'Hill', 'Mountian', 'Plain', 'Special' ];
    var t = Tabs.Search;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        ...
        if (t.opt.searchType == 2) {
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD width="10%"><b>Location</b></td><TD  width="5%"><b>Distance</b></td><TD width="15%"><b>City</b></td><TD width="15%"><b>Player</b></td><TD width="12%"><b>Power</b></td><td width="15%"><b>Alliance</b></td><TD width="28%" style="font-size:9px;"><b>Information</b></td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			if(Options.unownedOnly || t.opt.searchType == 0){
			  m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD width="20%"><b>Location</b></td><TD width="10%"><b>Distance</b></td><TD width="10%"><b>Level</b></td><TD width=60%><b>Type</b></td></tr>';
			} else {
			  m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD align="center" width="10%">Location</td><TD width="5%">Distance</td><TD align="center" width="5%">Level</td><TD align="center" width="10%">Content</td><TD align="center" width="15%">Owner</td><TD width="12%">Power</td><TD align="center" width="15%">Alliance</td><TD width=28% align="center" style="font-size:9px;">more Information</td></tr>';
			}
		}
	  }
      ...
      if (numRows > 500 && t.searchRunning){
        numRows = 500;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>CAUTION: This is the menu of up to 500\'den '+ dat.length +' \'results can be displayed up to..!</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					<DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A class=loc2>Leaders</a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][7] +')"><A class=loc2>Online Status</a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A class=loc2>Last Seen</a></div>';
			  ...
			  if(!dat[i][5] || t.opt.searchType == 0){
			    ...
			    m += '<TD align="right" valign="top" class=search1>'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD valign="top" class=search1 >'+ dat[i][4] +'</td><TD valign="top" class=search1 >'+ tileNames[dat[i][3]]+'</td><TD valign="top" class=search1 >'+ dat[i][9] +'</td><TD valign="top" class=search1 >'+dat[i][10]+'</td><TD valign="top" class=search1 >'+dat[i][11]+'</td><td class=search1 >';
				  if (dat[i][6]) {
					m += '<DIV onclick="PTscout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A style="font-size:9px;" >Message</a></div>';
				  } else {
					m += '<DIV onclick="PTpd(this, '+ dat[i][8] +')"><A class=loc2 >Details</a></div>\
					<DIV style="" onclick="PTpd2(this, '+ dat[i][8] +')"><A class=loc2>Leadersr</a></div>\
					<DIV style="" onclick="PCpo2(this, '+ dat[i][8] +')"><A class=loc2>Online Status</a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][8] +')"><A class=loc2>Last Seen</a></div>';
				  }
				m += '</td></tr>';
			  }
			}
...
if (DEBUG_TRACE) actionLog (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Searching: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('statStatus').innerHTML = 'Searching .. '+ x +','+ y;
...
Tabs.transport = {
  ...
  check:false,
    ...
    init: function(div){
      ...
      var m = '<DIV id=pbTowrtDivF class=pbStat>Auto-Transport</div><TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center">';
      ...
      m += '</tr></table></div>';
      m += '<DIV id=pbTraderDivDRoute class=pbStat>Transport Options</div>';
      m += '<TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center"><TR align="left">';
	    m += '<TD colspan=4>Time to check between transports: <INPUT id=pbtransportinterval type=text size=2 value="'+Options.transportinterval+'"\> In Minutes..</td></tr></table>';
      m += '<TD colspan=4>Min Number of transports needed:<INPUT id=pbminwagons type=text size=2 value="'+Options.minwagons+'"\> avoids useless transports ..</td></tr></table>';
      m += '<DIV style="margin-top:10px;margin-bottom:5px;">Now in English</div></table>';
    
      
      m += '<DIV id=pbTraderDivDRoute class=pbStat>Transport</div>';
      m += '<TABLE id=pbaddtraderoute width=95% height=0% class=pbTab><TR align="left">';
      m += '<TR align="left"><TD>From:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptrescity></span></div></td></tr>';

      m += '<TR align="left">';
      m += '<TD>To:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptcityTo></span></div></td>';
      m += '<TD>Or Cordinates</td>';
      ...
      m += '<TD width=75px>Transport Type:</td><TD width=150px><SELECT id="TransportTroop">';
      ...
      m+='</select></td><TD width=75px>Available amount:&nbsp;</td><TD id=TroopAmount align=left width=75px></td>';
      m+='<TD width=75px>Max Load:&nbsp;</td><TD id=CarryAmount align=left width=75px></td>';
      m += '<TR><TD >Assigned: </td><TD><INPUT id=TroopsToSend type=text size=6 maxlength=6 value="0">&nbsp;&nbsp;<INPUT id=MaxTroops type=submit value="MAX."></td>';
      ...
      m += '<TD width=180px  align=left>Keep in Town: <INPUT id=pbtargetamountFood type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountFood type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountWood type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountWood type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountStone type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountStone type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountOre type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountOre type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountGraph type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountGraph type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountTita type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountTita type=text size=11 maxlength=11 value="0"\></td>';
      ...
      m += '<TD width=180px align=left>Keep in Town: <INPUT id=pbtargetamountGold type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade Amount: <INPUT id=pbtradeamountGold type=text size=11 maxlength=11 value="0"\></td>';
      m += '<TD width=50px><INPUT id=MaxGold type=submit value="MAX"></td></tr>';

      m += '</table>';

      m += '<DIV style="text-align:center; margin-top:15px"><INPUT id=pbSaveRoute type=submit value="Save Route"><INPUT id=pbManualSend type=submit value="Send Manual"></div>';
...
document.getElementById('pbtargetamountTita').addEventListener('keyup', function(){
    ...
    updateTroops : function (city){
    	...
    	document.getElementById('Calc').innerHTML = 'Material: ' +  addCommas(t.Food + t.Wood + t.Stone + t.Ore + t.Graph + t.Tita + t.Gold) + ' / ' + addCommas(t.MaxLoad) + '&nbsp;&nbsp;(Transports Needed: <FONT color='+fontcolor+'>' + addCommas(t.TroopsNeeded) + '</font> )' ;
	...
	paintTradeRoutes: function(){
			...
			for (var i=0;i<(r.length);i++) {
			  citynameTo = null;
				...
				else var status = '<FONT color=red>Off</font>';
        ...
        m += '<TR><TD></td><TD>Transport:&nbsp;&nbsp'+unsafeWindow.arStrings.unitName[unit]+'</td></tr>';
        if (r[i].ship_Food) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + FOOD_IMAGE +'></td><TD>Keep: '+ addCommas(r[i].target_Food) +'</td><TD>Trade: '+ addCommas(r[i].trade_Food)+'</td>';
		if (r[i].ship_Wood) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + OIL_IMAGE +'></td><TD>Keep: '+ addCommas(r[i].target_Wood) +'</td><TD>Trade: '+ addCommas(r[i].trade_Wood)+'</td>';
		...
		if (r[i].ship_Ore) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + STEEL_IMAGE +'></td><TD>Keep: '+ addCommas(r[i].target_Ore) +'</td><TD>Trade: '+ addCommas(r[i].trade_Ore)+'</td>';
		...
		if (r[i].ship_Tita) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + TITA_IMAGE +'></td><TD>Keep: '+ addCommas(r[i].target_Tita) +'</td><TD>Trade: '+ addCommas(r[i].trade_Tita)+'</td>';
		if (r[i].ship_Gold) m += '<TR><TD></td><TD align=center><img height=35px width=35px src=' + GOLD_IMAGE +'></td><TD>Keep: '+ addCommas(r[i].target_Gold) +'</td><TD>Trade: '+ addCommas(r[i].trade_Gold)+'</td>';
       }
	     m +='</table>';
	 ...
	 editQueueElement: function(queueId){
       ...
       var queueId = parseInt(queueId);
	     var cityname;
	     var citynameTo = null;
	     var Types = ['Food','Oil','Stone','Steel','Graphene','Titanium','Gold'];
	     for (var y=0; y< Seed.cities.length;y++) {
...
for (y in unsafeWindow.arStrings.unitName) if(y == "u2"){continue} else if(y == "u14") {continue} else { n+='<option value="'+findoptvalue(y)+'">'+unsafeWindow.arStrings.unitName[y]+'</option>' };
       n+='</select></td></table><BR><TABLE  id=editRoutes class=pbTab>';
         ...
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountfood type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountfood type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + OIL_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipwood type=checkbox></td>';
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountwood type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountwood type=text size=11 maxlength=11 value="0"\></td></tr>';
         ...
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountstone type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountstone type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + STEEL_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipore type=checkbox></td>';
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountore type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountore type=text size=11 maxlength=11 value="0"\></td></tr>';
         ...
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountgraph type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountgraph type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + TITA_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshiptita type=checkbox></td>';
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamounttita type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamounttita type=text size=11 maxlength=11 value="0"\></td></tr>';
         n += '<TR><TD width=50px align=center><img src=' + GOLD_IMAGE + '></td>';
         n += '<TD width=50px align=center><INPUT id=pbbshipgold type=checkbox></td>';
         n += '<TD width=125px>Keep in Town: <INPUT id=pbbtargetamountgold type=text size=11 maxlength=11 value="0"></td>';
         n += '<TD width=125px>Trade Amount: <INPUT id=pbbtradeamountgold type=text size=11 maxlength=11 value="0"\></td></tr>'
    ...
    var t = Tabs.transport;
	  ...
	  case 'unt19': params.u19 = wagons_needed;break;
         ...
         new AjaxRequest(unsafeWindow.g_ajaxpath + "march.php" + unsafeWindow.g_ajaxsuffix, {
                  ...
                  if (rslt.ok) {
                  actionLog('Trade Material From: ' + cityname + "   To: " + xcoord + ',' + ycoord + "    \'Adetindeki Birimlerle: " + wagons_needed);
                 ...
                 actionLog('Failed to Transport: ' + cityname + ' -> ' + rslt.msg);
                  }
                  },
...
/****************************  Alarm Sekmesi Fonksiyonu  ******************************/
	...
	var uu4 = 'Orbital Ion Cannon';
	var uu5 = 'Infantry';
	var uu6 = 'Sniper';
	var uu7 = 'Mobil SAM';
	var uu8 = 'Tank';
	var uu9 = 'Supply Chopper';
	var uu10 = 'Fighter';
	var uu11 = 'Gunship';
	var uu12 = 'Bomber';
	var uu13 = 'Stealth Bomber';
	var uu15 = 'Tactical Nuke';
	var uu16 = 'Hellfire Tank';
	var uu17 = 'Anti-Tank Infantry';
	var uu18 = 'Special Forces';
	var uu19 = 'Cargo Transport';
        var uu20 = 'Predator Drone';
Tabs.tower = {
  tabOrder: 22,
  tabLabel: 'ALARM',
  myDiv: null,
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>ALARM MENU</div><TABLE class=pbTab><TR align=center>';
	...
	m += '</tr><TR align=center>';
	  for (var cityId in Cities.byID)
	   m += '<TD><CENTER><INPUT id=pbattackqueue_' + cityId + ' type=submit value="DETAILS"></center></td>';
    m += '</tr></table><BR><DIV><CENTER><INPUT id=pbSoundStop type=submit value="Mute Alarm"></center></div><DIV id=pbSwfPlayer></div>';
    m += '<BR><DIV class=pbStat>SETUP</div><TABLE class=pbTab>\
    <tr><td align=left><INPUT id=pbcellenable type=checkbox '+ (Options.celltext.atext?'CHECKED ':'') +'/></td>\
    <td align=left>SMS Number: <INPUT id=pbnum1 type=text size=4 maxlength=4 value="'+ Options.celltext.num1 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum2 type=text size=3 maxlength=3 value="'+ Options.celltext.num2 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\>\
&nbsp;<INPUT id=pbnum3 type=text size=4 maxlength=4 value="'+ Options.celltext.num3 +'"  '+(Options.celltext.provider==0?'DISABLED':'')+'\> <span style="color:#800; font-weight:bold">(Standard SMS rates apply)</span></td></tr><tr><td></td>\
 ...
 if(t.Providers[i].country == t.Providers[Options.celltext.provider].country)
    ...
    m += '</select></td></tr>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Report attack to allaince chat..!</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            ...
            <TR><TD align=right>Warn on Scout: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Warn on Wilds: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Defense Status: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Troop Status: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
            </table></td></tr>\
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Play Alarm Sound.!!</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>Get the Flash Player, or SWF Player..</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Audio file: &nbsp; </td><TD><INPUT id=pbsoundFile type=text size=55 maxlength=160 value="'+ Options.alertSound.soundUrl +'" \>\
             &nbsp; </td><TD><INPUT id=pbSoundLoad type=submit value=Upload><INPUT id=pbSoundDefault type=submit value=Default></td></tr>\
            ...
            <TR><TD></td><TD>Play:<INPUT id=pbSoundLength type=text size=3 maxlength=5 value="'+ Options.alertSound.playLength +'"> Seconds</td></tr>\
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
 ...
 e_soundFileLoaded : function (chan, isError){ // called by SWF when sound file finishes loading
    ...
    if (isError)  
      ...
      document.getElementById('pbLoadStat').innerHTML = 'INSTALLED.!';
  },  
  ...
  postToCell : function (m){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        data.embassy = 'Currently my Embassy: '+ availSlots +'of'+ emb.maxLevel;
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
            data.stat = 'My Troops are Hiding ..';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            data.stat = 'My Troops are Defending.!!';
        }
      }
    }
  ...
  postToChat : function (m){
    ...
    var city = Cities.byID[m.toCityId];
    if ( city.tileId == m.toTileId ) {
      target = 'Cordinates: ('+ city.x +','+ city.y + ')';
	  }
    else {
      if (!Options.alertConfig.wilds)
        return;
    ...
    var msg = Options.alertConfig.aPrefix +' ';
		msg += 'My Cords '+ target +'My '+ atkType  +' Comes From: '+ who +' Time till troops arrive: '+ unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +'\'Troop Types Inbound: ';
    var totTroops = 0;
    for (k in m.unts){
      var uid = parseInt(k.substr (1));
	  if(k == 'u1') {
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' My Embassy has: '+ availSlots +' Out of: '+ emb.maxLevel +' \'Slots..';
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
...
//DebugTimer.start(); 
     ...
     // <TR align=left><TD><SPAN class=ptStatLight>Joined on:</span> '+ dt.toLocaleDateString() +'</td>
	  ...
	  str += '<TR valign=top align=right><TD></td><TD style=\'background: #ffc\'></td>';
		...
		if(Gate == 0)
		  ...
		  str += '<TD><SPAN class=boldRed><blink>Defend</b></blink></span></td>';
		}
      ...
      str += _row ('Ant-Tank Infantry', rows[4]);
      ...
      str += _row ('Cargo Plane', rows[19]);
      str += _row ('Hellfire Tank', rows[16]);
      str += _row ('Stealth Bomber', rows[13]);
      str += _row ('Nuclear Warhead', rows[15]);
	  str += _row ('Orbital Cannon', rows[20]);
      str += '<TR><TD colspan=12><BR></td></tr>';
      ...
      for (r=1; r<5; r++){
	    ...
	    str += _row ('Mine:', rows[2],true);
        str += _row ('Stingers:', rows[1],true);
	
        str += _row ('Artillery:', rows[3],true);
        str += _row ('Anti-Aircraft:', rows[4],true);
        str += _row ('Total Disposable:', rows[5],true);
        str += _row ('Total Stable:', rows[6],true);
        str += '<TR><TD colspan=12><BR></td></tr>';
    }
           row = [];
      row2 = [];
      ...
      for(i=0; i<Cities.numCities; i++) {
        ...
        row2[i] = rp[1];
			...
			str += _row ('Food Used:', row3, true);
      str += _row ('Food Produced:', row2, true);      
      ...
      str += _row ('Food Reserve:', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=ptoverOriginal'+ (Options.includeCity?' CHECKED':'') +'>Show Troops In Cities!!</td></tr>';
    ...
    //  str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptoverIncTrainingTotal'+ (Options.includeTrainingTotal?' CHECKED':'') +' DISABLED>Show troops training totals</td></tr>';
     str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Font Size: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12,15:15}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr>';
      str += "</table></div>";
	  str+= 'Version : ' + Version;
...
//        <TD align=right><SPAN class=ptStatLight>Domain:</span> ' + unsafeWindow.domainName +'</td></tr></table></div>';
      ...
      str += "<TD width=100><B>Cities:"
      for(i=0; i<Cities.numCities; i++) {
        str += "<TD width=100><B>"+ Cities.cities[i].name.substring(0,11) +'</b><BR>' +"</td>";
      }
      str += "</tr><TD width=81>Gold:<BR>Tax:<BR>Income:<BR>Expenses:<BR><BR><B>Resource(1%):<BR>Military(0.5%):<BR>Research(0.5%):<BR>Construction(0.5%):</B>";
      Gold=[];
      Tax=[];
      Revenue=[];
      for(i=0; i<Cities.numCities; i++) {
...
Tabs.build = {
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbBuildDivF class=ptStat>Auto-Build Menu</div><TABLE id=pbbuildfunctions width=100% height=0% class=ptTab><TR>';
		...
		m += '<TD>Build Type: <SELECT id="pbBuildType">\
				<OPTION value=build>Next level</option>\
				<OPTION value=max>Max Level</option>\
				<OPTION value=destruct>Destruct</option>\
				</select></td>';
		//m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>Require Help?</td>';
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=ptStat>Builds</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
        ...
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD>Que:</td><TD id=pbbuildcount_' + Cities.cities[i].id + '>' + t["bQ_" + Cities.cities[i].id].length + '</td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            ...
            m += '<TD>Que Time:</td><TD id=pbbuildtotal_' + Cities.cities[i].id + '>' + timestring + '</td>';
        }
    ...
    doOne : function (bQi){
		...
		if (Seed.buildings['city' + currentcityid]["pos" + citpos] != undefined && Seed.buildings['city' + currentcityid]["pos" + citpos][0] != undefined) {	
			...
			if (mode == 'build') {
				...
				if (invalid == false) {							
					...
					if (params.lv > 9){ //make sure that no level 10+ is built
						t.cancelQueueElement(0, currentcityid, time, false);
						actionLog("Constuction over Level 9 Requires Manual Consturction!!");
						return;
					}
					if (params.lv > 1) {
						params.bid = bid;
					...
					new MyAjaxRequest(unsafeWindow.g_ajaxpath + "construct.php" + unsafeWindow.g_ajaxsuffix, {
						...
						onSuccess: function(rslt){
							if (rslt.ok) {
								actionLog("Under Construction: " + unsafeWindow.buildingcost['bdg' + bdgid][0] + " level: " + params.lv + " in City: " + cityName, true);								
								...
								if (rslt.error_code == 103) { // building has already the target level => just  delete
									t.cancelQueueElement(0, currentcityid, time, false);
									actionLog("Construction Removed.! Continue..!");
								} else {
									t.requeueQueueElement(bQi);
									document.getElementById('pbbuildError').innerHTML = Cities.byID[currentcityid].name +': '+ errmsg + " Building added back to Que..!";
								}
								actionLog(errmsg);
							}
					},
						onFailure: function(){
							document.getElementById('pbbuildError').innerHTML = "Connection Error Try Again";
						}
					});
				} else {
			...
			t.cancelQueueElement(0, currentcityid, time, false);
			actionLog("Removed ordinary construction.! Item not available.!!");
		}
	},
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>Construction for: ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="Sort by time"></td>';
...
m = '<STYLE>.xtabR {padding-right: 5px; border:none; background:none; white-space:nowrap; text-align:right;}\
        ...
        <DIV style="overflow-y:auto; overflow-x:hidden"><DIV class=ptStat>GENERAL INFORMATION </div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR style="text-align:center"><TD class=xtab></td><TD class=xtabHL colspan=9><B>REQUIREMENTS </b></td><TD class=xtabHL colspan=4><B>STATS</b></td><TD class=xtabHL colspan=4><B>Upkeep</b></td></tr>\
        ...
        <TD class=xtabH>Titanium</td><TD class=xtabH>Graphene</td><TD class=xtabH>Diamonds</td><TD class=xtabH>Uranium</td><TD class=xtabH>Population</td><TD class=xtabHL>Life</td><TD class=xtabH>Attack</td><TD class=xtabH>Speed</td>\
...
//actionLog ("ajax/allianceGetMembersInfo.php:\n"+ inspect (rslt, 5, 1));      
  ...
  show : function (){
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          ...
          <TR><TD class=xtab>Or: </td><TD class=xtab> Search Alliance: &nbsp;</td>\
        ...
        <option value="0,300" > Ant-Tank Infantry </option>\
        ...
        <option value="1,85" > Cargo Plane </option>\
        <option value="1,90" > Hellfire Tank </option>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'Need at least 3 characters..!';
      return;
    }
  ...
  eventGotPlayerOnlineList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Showing Results for: <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE width=100% align=center class=ptTab cellspacing=5>\
	  <TR style="font-weight:bold">\
	  <TD width=25%>Name</td><TD align=right width=15%>Power</td><TD width=15%> &nbsp; Online</td><TD width=15%>Facebook Prfl.&nbsp;</td><TD width=75%>Info</td></tr>';
    var row=0;
    var cl='';
    for (k in t.playerList){
      ...
      if (++row % 2)
       ...
       cl = '';
          ...
          <TD>'+ (rslt.data[u.userId]?"&nbsp;<SPAN class=boldRed><blink><b>Online Now</b></blink></span>":"") +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A style="color:#1D0AF2">Details</a> &nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A style="color:#1D0AF2">Last Seen</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerDetail : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "Search Complete ...";
    t.fetchPlayerInfo (uid, function (r) {t.gotPlayerDetail(r, span)});
  },
...
clickedPlayerLeaderboard : function (span, uid){
 ...
 gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD>Alliance: '+ a +' &nbsp; Cities: '
          + u.cities +' &nbsp; Population: '+ u.population +'</td></tr><TR><TD>Province: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
      p.push(unsafeWindow.provincenames['p'+pids[i]]);
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'Requires atleast 3 characters..!';
      return;
    }
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
  ...
  showMyAlliance : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'in order to use the feature must be members of the Alliance..!';
    }
  },
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("Please login or register first A Alliance..")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
  ...
  fetchPlayerLastLogin : function (uid, notify){
    ...
    gotPlayerLastLogin : function (rslt, span){
        ...
        if (lastLogin) {
          m = '<span style="color:black">Last seen: '+lastLogin+'</span>';
           ...
           m = '<span style="color:red">Last login date found: '+lastLogin+'</span>';
        }
        span.innerHTML = m + '';
      },
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
            ...
            <TD id=clickCol9 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Online Status</a></div></td>\
            ...
            <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Cordinates.</a></div></td>\
		    ...
		    <TD class=clickable><A><DIV>Last Seen</a></div></td></tr></thead>\
    ...
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Distance calculated cordst: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; Or, Select a city: <span id=dmcoords></span></td></tr></table>';
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Allows you to move the menu..</td></tr>\
		<TR><TD><INPUT id=ptahelpenable type=checkbox /></td><TD>Auto Help Alliance Members..</td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Wide Screen: '+ htmlSelector({normal:'Normal', wide:'Wide', ultra:'Ultra'},GlobalOptions.pbWideScreenStyle,'id=selectScreenMode') +' </td>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>Move the chat window(Wide Screen must be enabled..)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>Expands Map. (Wide Screen must be enabled..)</td></tr>\
		<TR><TD><INPUT id=pbrmmotdEnable type=checkbox /></td><TD> Remove MOTD .. </td></tr>\
		<TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>Enables GW refresh: <INPUT id=pbeverymins type=text size=2 maxlength=3 \> In Minutes</td></tr>\
		<TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>Enable Low Food Alarm..! (Checks every 2 hours..)</td></tr>\
		<TR><TD><BR> </td></tr>\
		<TR><TD colspan=2><B>Auto COllection:</b></td></tr>\
		<TR><TD><INPUT id=ptgoldenable type=checkbox /></td><TD>Auto Collect Gold.. <INPUT id=ptgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
		...
		<TR><TD><INPUT id=ptMCenable type=checkbox /></td><TD>Auto Collect Vechicles</td></tr>\
		<TR><TD><INPUT id=ptWHenable type=checkbox /></td><TD>Auto Collect Warheads</td></tr>';
		m += '</table><BR><BR><HR>The little <b>Blue Smurf</b> its not just a name, its in the game..!';
      div.innerHTML = m;
	  t.cont.innerHTML = m;
...
Tabs.PodTools = {
    ...
    init : function (div){
        ...
        try {
            ...
            <TR>\
                <TD><INPUT id=PodAttack     type=checkbox /></td><TD>Active</td>\
                <td style="width:5em"></td>\
                <TD><INPUT id=AttackWild type=checkbox /></td><TD>Attack Players</td></tr>\
            <TR>\
                <TD><INPUT id=SupplyTruck   type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Supply Truck</td>\
                <td></td>\
                <TD><INPUT id=SupplyChopper type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Supply CHopper</td>\
            </tr>\
            <TR>\
            ...
            <TR>\
                <TD><INPUT id=AttackAntitank type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Anti-Tank Infantry</td>\
                <td></td>\
            ...
            <TR>\
                ...
                <td></td>\
                <TD><INPUT id=SupplyJet     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Cargo Plane</td>\
            </tr>\
            <TR>\
            ...
            <TR>\
                <TD><INPUT id=AttackPredator type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Predator Drone</td>\
                <td></td>\
                <TD><INPUT id=AttackNuke     type=text size=4 maxlength=6 value=0 /></td><TD> &nbsp; Nuclear Warhead</td>\
            </tr>\
            <TR><TD><BR> </td></tr>';
            m += '</table><BR><BR><HR>After entering above numbers, on our map mouse click the place you wish to attack (Repeat click for each attack).!!';
            t.cont.innerHTML = m;
        ...
        if(generals && t.Attacking){
            ...
            for (var maxenergy=4; maxenergy>0; maxenergy--) {
                for(kk=0;kk< list.length;kk++) {
                    ...
                    if(count == maxenergy){
                        actionLog("Your : General " + kk + " Is at " + count + " Max Energy=" + maxenergy,true);
            ...
            if(list.length==0) {
                var text = "ERROR no available Generals..!";
                actionLog(text,true);
                unsafeWindow.Modal.hideModalAll();
                unsafeWindow.Modal.showAlert(text);
                window.setTimeout('Modal.hideModal();', 3000);
            ...
            if(kk!==list.length+2) {
                var text = "ERROR: Attention: " + kk + " your General has no Energy.!.";
                actionLog(text,true);
                unsafeWindow.Modal.hideModalAll();
                unsafeWindow.Modal.showAlert(text);
                window.setTimeout('Modal.hideModal();', 3000);
...
var RefreshEvery  = {
  ...
  doit : function (){
      actionLog ('Refresh: ('+ Options.pbEveryMins +' Time..)');
    reloadKOC();
  },
  ...
  Paint : function(){
     ...
     if ( Left < 60) text += '<BR>&nbsp;&nbsp;&nbsp;&nbsp;<small><FONT size=1px color=white>Page Refresh Time: </font></small><small><FONT color=red><B>'+ timestr(Left) +'</b></font></small></div>';
     else text += '<BR>&nbsp;&nbsp;&nbsp;&nbsp;<small><FONT size=1px color=white>Refresh: <B>'+ timestr(Left) +'</b></font><small></div>';
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){  
    ...
    if (Options.pbFoodAlert && timeLeft<(6*3600)) {
                ...
                msg += ' !!!Is LOW on FOOD!!! Current Food Left: '+addCommasWhole(foodleft)+' ('+timestrShort(timeLeft)+') Food Used: '+addCommas(usage);
                sendChat ("/a " + msg);
          }
    }
      }
...
function dialogRetry (errMsg, seconds, onRetry, onCancel, errCode){
  ...
  pop.getTopDiv().innerHTML = '<CENTER>Ads By Scratch & Sniff &reg;</center>';
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>Sorry a Hamster has escaped the cage:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Your page will Automatically refresh in .. <SPAN id=paretrySeconds></b></span> Seconds ...<BR><BR><INPUT id=paretryCancel type=submit value="Cancel" \>';
...
var CollectWH = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectWH;
    if (rslt.ok)
      actionLog ('SUCCESS!:'+ rslt.gained.resource2 +' Warhead '+ t.colCityName, true);
    else
      actionLog ('ERROR in collecting Warhead.! '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectMC = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectMC;
    if (rslt.ok)
      actionLog ('SUCCESS '+ rslt.gained.resource2 +' Launch Vehicle '+ t.colCityName, true);
    else
...
var CollectGold = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectGold;
    if (rslt.ok)
      actionLog ('SUCCESS in getting'+ rslt.gained.gold +' GOLD '+ t.colCityName +' ( '+ t.colHappy +')', true);
    else
      actionLog ('ERROR collecting GOLD!: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectFood = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectFood;
    if (rslt.ok)
      actionLog ('SUCCESS '+ rslt.gained.resource1 +' Food! '+ t.colCityName, true);
    else
      actionLog ('ERROR collecting FOOD!:'+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
var CollectOil = {
  ...
  e_ajaxDone : function (rslt){
    var t = CollectOil;
    if (rslt.ok)
      actionLog ('SUCCESS '+ rslt.gained.resource2 +' OIL '+ t.colCityName, true);
    else
      actionLog ('ERROR collecting Oil!: '+ t.colCityName +': <SPAN class=boldRed>'+ unsafeWindow.ERROR_CODE[rslt.error_code] +'</span>', true);
  },
...
MarchTimer: [],
 ...
 SendIt: false,
    ...
    STRTIME: ["","Construct","Research","Train","Walls","March"],
    ...
    CreateInterface: function () {
        ...
        this.UpdateDiv("Script...Loaded",1);
...
opt.setAttribute('id', 'marchBM');
    ...
    Build: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('Construction: ' + AutoBuilder.DisplayTime(Building.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.BuildTimer[cty]);
    ...
    Resrch: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('Research Time: ' + AutoBuilder.DisplayTime(Research.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.RsrchTimer[cty]);
