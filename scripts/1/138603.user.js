...
{
   var box = document.getElementById('throneInventoryList');
   logit("inventory: " + inspect(box,3,1)); 
}
*/
...
{   
    ...
    unsafeWindow.execSalvage = Tabs.throneSalvage.doSalvage;
    var n = unsafeWindow.cm.FETemplates.Throne.mainThrone.replace(
       '<li id="throneStatTab" class="inactive"> Stats </li>',
       '<li id="throneStatTab" class="inactive"> Stats </li><li id="throneTest" class="inactive" onclick="hideShow()"> Upgrader </li><li id="trexecsalvage" class="inactive" onclick="execSalvage()"> Run Salvager </li>');
    
    unsafeWindow.cm.FETemplates.Throne.mainThrone = n;
}
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    for (var i=0; i<sorter.length; i++) {
      ...
      //m += '<TD align=center class=notSel id=trtc'+ sorter[i][1].name +' ><A><SPAN>'+ sorter[i][1].label +'</span></a></td>';
      if ((i+1)%9 == 0) m+='</tr><TR>';
    }
    m+='</tr></table>';  
...
Tabs.throneSalvage = {
    ...
    init : function (div) {
       ...
       t.myDiv = div;
       var m = '<Div><DIV id=trSalvage class=trStat>Auto Salvage</div>';
       m += "<div id='trInfoArea'>";
       m += '</div>';
       m += '<TABLE id=trupgrader width =100% height=0% class=trTab style="padding-left: 20px;">';
       if (salvageData.salvageActive == false) {
	       m += '<tr><TD><div><INPUT id=trSalvagerPower type=submit value="Auto-Salvage = OFF"/></div></td>';
	   } else {
	       m += '<tr><TD><div><INPUT id=trSalvagerPower type=submit value="Auto-Salvage = ON"/></div></td>';
	   }
       m += '<td><b> Ville où mettre la pierre d Ether: </b></td>';
       m += "<td><div id='trSalvageCity'></td><td></td></tr>";
       
       m += '<tr><td><div> Garder a partir de: <select id="trSalvageQuality">';
       m += '<option value="1">Commun</option>';
       m += '<option value="2">Inhabituel</option>';
       m += '<option value="3">Rare</option>';
       m += '<option value="4">Epique</option>';
       m += '<option value="5">Superbe</option>';
       m += '</select> et Plus haut</div></td>';
       
       m += '<td>Ne pas effacer les <INPUT id=trSaveNum type=text size=3 maxlength=3 value="' + salvageData.throneSaveNum+ '"/> premiers items</td>';
       m += '<td><div><INPUT id=trSalAnyCity type=checkbox '+ (salvageData.anyCity?' CHECKED':'') +'/> Si ville remplie, disperser sur les autres</div></td>';
       ...
       //  rules definition
 	   ...
 	   m += '<tr><td>  <b>Definir les items a garder </b> </td>';
 	   
 	   m += '<td alight="left"><div><span>Faction: <select id="trFactionType">';
 	   m += '  <option value="any">Toutes</option>';
 	   m += '  <option value="fey">Fey</option>';
 	   m += '  <option value="briton">Briton</option>';
 	   m += '  <option value="druid">Druid</option>';
       m += '</select></span></div></td>';
 	   m += '<td alight="left"><div><span>Type: <select id="trCardType">';
 	   m += '  <option value="any">Toutes</option>';
 	   m += '  <option value="chair">Chaise</option>';
 	   m += '  <option value="table">Table</option>';
 	   m += '  <option value="window">Window</option>';
 	   m += '  <option value="banner">Bannière</option>';
 	   m += '  <option value="advisor">Advisor</option>';
 	   m += '  <option value="trophy">Trophée</option>';
       m += '</select></span></div></td>';
       m += '<td align="right"><INPUT id=trAddRule type=submit value="Créer une règle"/></td>';
       m += '</tr></table>';
       m += '<TABLE  class=trTabPad width=100% id="trConditionTable"  style="padding-left: 5px;">';
       m += '<tr><td align=left colspan=1><INPUT id=trAddRow type=submit value="Ajouter Condition"/></td>';
       m += '<td></td><td></td><td></td><td></td><td></td></tr>';
       m += '</table>';
       m += '</div><hr/>';
       m += '<div id="trSalvStatus" style="text-align: center;" >Chargement ... </div><hr/>';
       m += '<div class=trRulePane>';
       m += '<div align=center> <b> L auto-salvage garde les items correspondant aux critères suivants </b></div><hr/>'; 
   ...
   createRow : function()
   {
       ...
       var h  = "<td> <select id='" + rowId + "sel1'> <option value='true'> AVEC </option> <option value='false'>Sans</option></select></td>";
           ...
           h += "  <option value='e'>les deux</option>";
   ...
   buildRuleDisplay : function ()
   {
	   ...
	   for (i =0; i < salvageData.ruleSet.length; i++)
	   {
	       ...
	       m += "</div></td>";
	       m += "<td width=20%><INPUT id=trDelRule" + i + " type=submit value='Effacer la règle' /></td>";
		   m += '</tr>';
	   }
...
Tabs.organizer = {   
	...
	init : function (div){
       ...
       // setup the tab
       var m = '<Div style="width: 720px; height:600px;"><DIV id=trOrganizer class=trStat style="width: 740px;">ORGANIZER</div>';
	   ...
	   //m += '</tr>';    	   
 	   ...
 	   // preset selector
 	   m += '<TABLE  width=90% class=trTabPad2><tr align="center"><td width=30%><div><span>Preset: <select id="PresetList">';
       ...
       document.getElementById ('testSalvage').addEventListener ('click', function() {t.testSalvage();}, false);
   ...
   testSalvage : function() {
	   ...
	   document.getElementById('trDelResults').innerHTML = "<div> " + toDelete.length + " items targeted for deletion</div>";
   ...
   paintTable : function() {
	   ...
	   {
		  //nodes[n].style.height  = (TABLE_SCALE * nodes[n].style.height) + "px";
		  //nodes[n].style.width   = (TABLE_SCALE * nodes[n].style.width) + "px";
		  var d2 = nodes[n].childNodes[0];
		  var h = d2.offsetHeight;
		  var w = d2.offsetWidth;
		  //var d3 = document.getElementById("idsection");
		  //d2.style.height = d2.offsetHeight;
		  //d2.style.width  = d2.offsetWidth;
...
Tabs.upgrader = {   
	...
	init : function (div){
       ...
       // header stuff
       var m = '<Div><DIV id=trUpgrader class=trStat>AUTOMATED UPGRADER</div><TABLE id=trupgrader width=100% height=0% class=trTab>';
       m   += '<TR>';
	   ...
	   // retry interval entry
 	   ...
 	   // upgrade selector
 	   m += '<TABLE  class=trTabPad2><tr><td><div><span>Upgrade: <select id="trUpgradeList">';
       ...
       // enhance selector
 	   //m += '<TABLE  class=trTabPad2><tr align="center"><td><div><span>Enhance: <select id="trEnhanceList">';
       ...
       m += '<tr><td colspan=3><hr></td></tr>';
       m += '<tr align="center"><td colspan=2><div id=trLastResult class=indent25> <br> </div></td></tr>';
...
function display_confirm(confirm_msg,ok_function,cancel_function){
    ...
    if(!container_div) {
        ...
        container_div.style.zIndex=300000;
...
// even though Scriptish provides its own update mechanism (GM_updatingEnbled == true), lets us this method.
