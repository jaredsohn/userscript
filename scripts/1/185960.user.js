...
lamp_img = document.body.appendChild(createElement('img', {id:'lamp', src:lamp_off_src, class:'classUserInterfaceee'}, 'click toggleControl false'));
...
function eventHandlerNodeInserted(evt)
{
	...
	if(!document.body.getAttribute('class')  ||  document.body.getAttribute('class').search(/darkcolors_bgcolor/) < 0 )
		document.body.setAttribute('class', (document.body.getAttribute('class')?document.body.getAttribute('class'):'')+' darkcolors_txtcolor darkcolors_txtsize darkcolors_bgimg darkcolors_bgcolor darkcolors_fontfamily');
	if(!html.getAttribute('class')  ||  html.getAttribute('class').search(/darkcolors_bgcolor/) < 0 )
		html.setAttribute('class', (html.getAttribute('class')?html.getAttribute('class'):'')+' darkcolors_txtcolor darkcolors_txtsize darkcolors_bgimg darkcolors_bgcolor darkcolors_fontfamily');
...
function appendClassesToElement(el)
{
	...
	if(getStyle(el,'background-color')!='transparent')//add bgcolor class
	{
	...
	else if(el.tagName=='IMG' && el!=lamp_img)//add img opacity class
	{
		try{clss = el.getAttribute('class');}catch(e){}
		if(!clss)
			clss = '';	
	...
	//add text color, size, font-family, line-height classes
	if(el.textContent!='' || el.nodeName=='INPUT')
	{
		...
		if(clss=='' || clss.search(/darkcolors_txtcolor/) < 0 ){
			el.setAttribute('class', clss + ' darkcolors_txtcolor darkcolors_txtsize darkcolors_fontfamily  darkcolors_lineheight');
...
function init(initCalledByAutorun)
{
	...
	html = document.getElementsByTagName('html')[0];
	document.body.setAttribute('class', (document.body.getAttribute('class')?document.body.getAttribute('class'):'')+' darkcolors_txtcolor darkcolors_txtsize darkcolors_bgimg darkcolors_bgcolor darkcolors_fontfamily darkcolors_lineheight  darkcolors_img');
	html.setAttribute('class', (html.getAttribute('class')?html.getAttribute('class'):'')+' darkcolors_txtcolor darkcolors_txtsize darkcolors_bgimg darkcolors_bgcolor darkcolors_fontfamily darkcolors_lineheight darkcolors_img');
...
function optionsHTML()
{
	...
	//copy settings from
	var contDomain, domain, settings;
...
function optionsApply()
{
	...
	if(getOption('enablefontfamily') && (getOption('fontfamily') != null)){
		getId('darkcolorscss_fontfamily').innerHTML = '*.darkcolors_fontfamily { font-family:'+getId("opt_select_fontfamily").value+' !important;}';
	}else{
		getId('darkcolorscss_fontfamily').innerHTML = '';
	}	
...
function optionsEvent(evt)
{
	...
	switch(id)
	{
		...
		case 'opt_select_loadsetting':
			optionsClearCSS()
			getStoredSettings(getId('opt_select_loadsetting').value)
			optionsInterfaceUpdateState()
			//optionsInterfaceInit()
		...
		case 'opt_select_fontfamily':
			if(getId('opt_checkb_enablefontfamily').checked){
				getId('darkcolorscss_fontfamily').innerHTML = '.darkcolors_fontfamily, .darkcolors_fontfamily *:not(.classUserInterfaceee){ font-family: '+getId('opt_select_fontfamily').value+' !important; }';
				getId('opt_select_fontfamily').removeAttribute('disabled');
			}else{
...
function sliderMoved(slider)
{
	...
	switch(slider.getAttribute('id'))
	{
		...
		case 'slider_bgcolor':
			...
			//show
			getId('darkcolorscss_bgcolor').innerHTML = '.darkcolors_bgcolor, .darkcolors_bgcolor *:not(.classUserInterfaceee) { background-color:#'+hexcolor+' !important; border-color:#'+bordercolor+' !important;}';
			break;
...
function initCss(){
	if(initStageZero){
		GM_addStyle(
			'#lamp { position:fixed !important; top:1px !important; right:1px !important; z-index:2147483647 !important; cursor:pointer !important; display:block !important; visibility:visible !important; width: 14px !important; height:15px !important;}'
		)
	}else{
		GM_addStyle(
