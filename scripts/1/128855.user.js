...
function check_lists(notesHtml){	
	var blackList = new Array(http://brittapperry.tumblr.com/post/12868703918);
...
function erase_notes() {
	...
	for (var i=last_check, notesLength=notes.length; i<notesLength; i++) {
		...
		//Single note
		if ($(notes[i]).hasClass('single_notification')) {
			if(savedfrom) {
				$(notes[i]).empty().remove();
			}
		}
		
		//First note
        else if ($(notes[i]).hasClass('first_notification')) {
			if (savedfrom) {
		...
		//Last note
        else if ($(notes[i]).hasClass('last_notification')) {                
			if (savedfrom) {
				$(notes[i]).empty().remove();
