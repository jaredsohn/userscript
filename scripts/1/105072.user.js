...
//	・デザイン変更、これは完全に自己満足。
//2011.06.19 by tonton; 同じレベルの設備がある場合、手持ち資源が少ない資源設備を優先する。
...
var OPT_CHKBOXLV = [15,15,15,15,20,10,10,10,15,15,15,15,15,15,15,10,10,10,10,10,20,20,20,10];
var OPT_MAX_LV = "2";
...
function get_area(){
  ...
  // added by tonton;
  // 手持ち資源の少ない資源設備を優先的に建設できるように、
  // 資源設備のlevelに0〜0.5の値を追加する。
  // 資源設備以外には、0.6を追加する。
  var RES_NOW = [];
  RES_NOW["伐採所"] = parseInt( $("wood").innerHTML, 10 );
  RES_NOW["石切り場"] = parseInt( $("stone").innerHTML, 10 );
  RES_NOW["製鉄所"] = parseInt( $("iron").innerHTML, 10 );
  RES_NOW["畑"] = parseInt( $("rice").innerHTML, 10 );

  var res_max;
  res_max = RES_NOW["伐採所"];
  if (res_max < RES_NOW["石切り場"]) {
    res_max = RES_NOW["石切り場"];
  }
  if (res_max < RES_NOW["製鉄所"]) {
    res_max = RES_NOW["製鉄所"];
  }
  if (res_max < RES_NOW["畑"]) {
    res_max = RES_NOW["畑"];
  }

  var RES_PNT = [];
  RES_PNT["伐採所"] = RES_NOW["伐採所"]/res_max/2;
  RES_PNT["石切り場"] = RES_NOW["石切り場"]/res_max/2;
  RES_PNT["製鉄所"] = RES_NOW["製鉄所"]/res_max/2;
  RES_PNT["畑"] = RES_NOW["畑"]/res_max/2;
  RES_PNT["拠点"] = 0.6;
  RES_PNT["倉庫"] = 0.6;
  RES_PNT["銅雀台"] = 0.6;
  RES_PNT["鍛冶場"] = 0.6;
  ...
  RES_PNT["見張り台"] = 0.6;
  RES_PNT["修行所"] = 0.6;
  //alert(RES_PNT["伐採所"] + " - " + RES_PNT["石切り場"] + " - " + RES_PNT["製鉄所"] + " - " + RES_PNT["畑"]);
  ...
  var level_pnt;
	...
	for(var i=0,n=0; i<results.snapshotLength; i++){
		if(results.snapshotItem(i).alt.match(/(.*?)\s.*?(\d+)/)){
		        level_pnt = Number(RegExp.$2) + RES_PNT[RegExp.$1]; // tonton;
			var strURL = results.snapshotItem(i).href;
			//area[n] = new lv_sort(RegExp.$1,RegExp.$2,getURLxy(strURL));
			area[n] = new lv_sort(RegExp.$1, level_pnt, getURLxy(strURL)); // tonton;
			//alert(area[n].xy);
			n++;
		}
	}
	...
	/* test
	for (var xxx=1; xxx<10; xxx++) {
	  alert(area[xxx].name + " / " + area[xxx].lv + " / " + Math.floor(area[xxx].lv) + " / " + area[xxx].xy);
	}
	*/
...
function addOpenLinkHtml() {
	...
	if ( ifmixi.snapshotLength ) {
	  //		openLink.innerHTML = "<nobr><img src='http://img.mixi.jp/img/basic/favicon.ico' title='Auto Bilder'><b>自動建築</b></nobr>";
		openLink.innerHTML = "<nobr><b>自動建築</b></nobr>";
	} else {
