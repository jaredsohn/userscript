...
(function () {
    ...
    function allInOneOpera() {
        ...
        var About = {
            Version: { Now: '10.0.5.3', New: '10.0.5.4' },
...
[84935, 45735, 71870, 58800, 115, 71]//20
        ...
        function XMLGetR(num) {
            ...
            var xmf = [];
            $.get(xpath('//table[@id = "overview"]/tbody/tr[' + num + ']/td[2]/div/a[contains(@href, "berichte.php?id=")]').snapshotItem(0).getAttribute('href'), function (ajax) {
                ...
                table.innerHTML = xmf[1].find('#report_surround').html();
                ID('content').appendChild(table);
                xpath('//table[@id = "overview"]/tbody/tr[' + num + ']/td/input').snapshotItem(0).setAttribute('checked', 'checked');
                xpath('//table[@id = "overview"]/tbody/tr[' + num + ']/td[3]/a/img').snapshotItem(0).setAttribute('src', msgC);
        ...
        function XMLGetM(num) {
            document.body.style.cursor = 'wait';
            var xmf = [];
            $.get(xpath('//table[@id = "overview"]/tbody/tr[' + num + ']/td[2]/div/a[contains(@href, "nachrichten.php?id=")]').snapshotItem(0).getAttribute('href'), function (ajax) {
                xmf[2] = $(ajax);
                ...
                xpath('//table[@id = "overview"]/tbody/tr[' + num + ']/td/input').snapshotItem(0).setAttribute('checked', 'checked');
                xpath('/html/body//table[@id = "overview"]/tbody/tr[' + num + ']/td[3]/a/img').snapshotItem(0).setAttribute('src', msgC);
                document.body.style.cursor = 'default';
            });
        };
        function sh() {
    ...
    setTimeout(cMapChecker, 666);*/
        ...
        function PHP_Village_Access() {
            ...
            var sAll = '';
            var cID = '';
            for (a = 0; a < ID('villageList').getElementsByTagName('li').length; a++) {
                cID = ID('villageList').getElementsByTagName('li')[a].getElementsByTagName('a')[0].href.split('id=')[1];
                vAccessA[a] = eval(GM_getValue('accessA' + a + '_id' + cID, '[]'));
                vAccessB[a] = eval(GM_getValue('accessB' + a + '_id' + cID, '[]'));
                var acc = [], vAccA = [], vAccB = [], rid = '';

                if (cID == $('div#villageList ul li a.active').eq(0).attr('href').split('id=')[1]) {
                    if (ID('movements') && exp(/dorf1/)) {
                        if (ID('movements').getElementsByTagName('img')[0]) {
                            ...
                            vAccA.push(mov);
                            GM_setValue('accessA' + a + '_id' + cID, uneval(vAccA));
                        };
                    };
                    if (!ID('movements') && exp(/dorf1/)) {
                        GM_setValue('accessA' + a + '_id' + cID, '[]');
                    };
                    if (ID('building_contract') && exp(/dorf1/)) {
                        ...
                        vAccB.push(ax);
                        GM_setValue('accessB' + a + '_id' + cID, uneval(vAccB));
                    };
                    if (!ID('building_contract') && (exp(/dorf1/) || exp(/dorf2/))) {
                        GM_setValue('accessB' + a + '_id' + cID, '[]');
                    };
                };
                if (vAccessA[a][0]) { vAccA = vAccessA[a][0]; } else { vAccA = '<font color="silver">-</font>'; };
                if (vAccessB[a][0]) { vAccB = vAccessB[a][0]; } else { vAccB = '<font color="silver">-</font>'; }; ;
                td_ = '<td style="padding:3px; text-align:center;">';
                rid = '<tr>' + td_ + $('div#villageList ul li a').eq(a).html() + '</td>' + td_ + '' + vAccA + '</td>' + td_ + '' + vAccB + '</td></tr>';
                sAll = sAll + rid;
            };
            var xhtml = '<div id="vlls_state" style="position:absolute;"><table cellspacing="0" style="z-index:10000; background-color: white;width:160px;"><thead><tr><td colspan="4">' + $("div#villageList div:eq(0) a:eq(0)").html() + '</td></tr></thead><tbody>' + sAll + '</tbody></table>';
...
'#t4tools { background-color: rgba(255, 255, 255, 0.8); border-radius: 5px 5px 5px 5px; box-shadow: 0px 1px 2px 0px black; position: absolute; left:: ' + (RTL == "ltr" ? "300" : "100") + 'px; top: 66px; z-index: 10500;} #t4tools img {padding: 5px;}' +
