...
var k2=document.createElement('li');k2.innerHTML='<a class="navSubmenu" onClick="dFAcore(true);">DownFbAlbum(Setup)</a>';
...
function output(){
	if(location.href.match("/.*facebook.com/")){
		...
		for(var i=0; i<photos.length;i++){
			var id = photos[i].href.match(/fbid=(.*)/)[1];
		...
		qS('.link-profile strong').textContent=g.Env.user.username;
		document.title=g.photodata.aName;
	}
...
function fbAutoLoad(elms){
	...
	if(!g.last_fbid){
		...
		if(g.ajaxRetry<2){l=elms[elms.length-2].href;l=l.slice(l.indexOf('=')+1,l.indexOf('&'));g.ajaxRetry++;}
	...
	var p=location.href+'&';var isAl=p.match(/media\/set|set=a/),aInfo={},isPS=p.match(/photos_stream/),isGp=p.match(/group/);
	if(isGp){
...
function instaAjax(){
	...
	xhr.onload = function() {
		...
		qS('.link-profile strong').textContent='Loaded '+g.photodata.photos.length+' of '+total+' photos.';
...
unsafeWindow.dFAcore = function(setup) {
	...
	if(location.href.match(/.*facebook.com/)){
		...
		try{var s=qSA("script"); for(var i=0,t; t=s[i].textContent, i<s.length; i++){
			if(t.match(/envFlush\({/)){
				g.Env=JSON.parse(t.slice(t.indexOf("envFlush({")+9,-2)); break;
			}
		...
		g.Env=JSON.parse(s.match(/",\[({".*})\]\],/)[1]).props;g.stored=[];
		g.total=g.mode!=2?+qS('.number-stat').textContent:g.Env.user.counts.media;
		console.log(g.Env);
...
switch(request.type){
	...
	case 'export':
		...
		tHTML=tHTML+'<header id="hd"><div class="logo" id="logo"><div class="wrapper"><h1><a id="aName" href='+c.aLink+'>'+c.aName+'</a> '+((c.aAuth)?'- '+c.aAuth:"")+' <button onClick="cleanup()">ReStyle</button>Developed by<a href="http://cartw3.blogspot.in/" target="_blank"> Cart w3</a></h1><h1 class="msg">Press Ctrl+S (with Complete option) to save all photos. [Photos are located in _files folder]</h1></div></div></header>';
		...
		if(g.newWin){
			tHTML = tHTML.replace(/\s{2,}/g, '').replace(/%/g, '%25').replace(/&/g, '%26').replace(/#/g, '%23').replace(/"/g, '%22').replace(/'/g, '%27');
			var w = window.open("data:text/html;charset=utf-8," + tHTML,"_blank"); w.focus();
		}else{document.write(tHTML); document.close();}
		break;
		}
};