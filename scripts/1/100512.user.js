// ==UserScript==
// @name           KoC Toolkit - Deutsch
// @namespace      PDX
// @homepage       http://userscripts.org/scripts/show/100512
// @description    KoC Toolkit - German version by PDX
// @version        4.1.4
// @include       *kingdomsofcamelot*
// @icon	  http://koc.god-like.info/pdx.jpg
// @require       http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js 
// @require			http://koc.god-like.info/update/auto-updater.php?id=100512
// ==/UserScript==
 ...
 */
 ...
 /*
 THIS VERSION IS HAS JUST BEEN UPDATED
 */
...
function DD_POC_GUI(){
    ...
    window.GUI.clearstorage = function(){
        if (confirm("KoC Toolkit - Deutsch: Daten werden gelöscht! Bist du dir Sicher?")) {
            tk_data = "";
            window.tk_data = new DD_POC_DATA();
            handler.saveObject(tk_data); 
            alert("KoC Toolkit - Deutsch: Daten geleert!");
			
            if (confirm("KoC Toolkit - Deutsch muss neu gestartet werden!")) {
                location.reload(true);
            }
            else {
                return;
            ...
            alert("KoC Toolkit - Deutsch: Daten konnten nicht geleert werden!");
        }
        ...
        if (storageContent == "") 
            storageContent = '<li class="empty">Kein Log zur Zeit vorhanden!</li>';
    ...
    window.GUI.modalmessage = function(message){
		var timeout = 10000;
        var content = "Fenster wird in 10 Sekunden geschlossen!<br><br>"
		content += message;
    ...
    window.GUI.build = function(){
        ...
        if (data == null) {
            alert('KoC Toolkit - Deutsch: Toolkit Daten werden erstellt!');
            window.tk_data = new DD_POC_DATA();
            
            var s = tk_data.states = new Object();
            
            //default values
            s.ScriptVersion = '4.1.3';
            s.details = false;
            s.tabselected = 'log';
            s.mode = 0;
        ...
        if (typeof tk_data.states.ScriptVersion == 'undefined' || typeof tk_data.states.details == 'undefined' || typeof tk_data.states.tabselected == 'undefined' || typeof tk_data.states.mode == 'undefined' || typeof tk_data.states.pocbuildaction == 'undefined' || typeof tk_data.states.isnotrunning == 'undefined' || typeof tk_data.states.ispaused == 'undefined' || typeof tk_data.states.alerterOn == 'undefined' || typeof tk_data.states.ask4help == 'undefined' || typeof tk_data.options.widemap == 'undefined' || typeof tk_data.options.onattack == 'undefined' || typeof tk_data.options.minattack == 'undefined' || typeof tk_data.options.onscouts == 'undefined' || typeof tk_data.options.minscouts == 'undefined' || typeof tk_data.options.usechat == 'undefined' || typeof tk_data.options.chatchannel == 'undefined') {
			...
			alert('KoC Toolkit - Deutsch: Download war Fehlerhaft oder die Version ist veraltet! Bitte neue Version Downloaden!');
			tk_data = "";
        ...
        GUI.poclog.add("<b>KoC Toolkit - Deutsch: Version 4.1.3</b><br><br>");
        /*
        ...
        */
        ...
        GUI.logattack.add('Suche Angriffe!');
        GUI.logscout.add('Suche Späh Angriffe!');
        GUI.logfake.add('Suche Fake Angriffe!');
    ...
    window.GUI.hidePopups = function(){
        ...
        if (document.getElementsByTagName('body')[0].innerHTML.indexOf('Send a Travelling Faire to a Friend') > 0) {
            GUI.poclog.add('Popup Blocker: Faire geblockt!');
            Modal.hideModal();
        }
        ...
        if (document.getElementsByTagName('body')[0].innerHTML.indexOf('has cordially invited you to visit their Court') > 0 || document.getElementsByTagName('body')[0].innerHTML.indexOf('herzlich eingeladen sie an ihren Hof zu besuchen') > 0) {
            GUI.poclog.add('Popup Blocker: Invite geblockt!');
            Modal.hideModal();
        }
    }
...
function DD_POC_FUNCTIONS(){
    ...
    if (GUI.currentBuildMode == "build") {  
        if( w >= 9) {  
            GUI.modalmessage('ACHTUNG du hast versucht über Level 9 zu Makieren\nLevel 10 und 11 solltest du selbst Bauen, war ja nicht grad billig ;)');  
            return;  
        }  
    }
        var e = Math.pow(2, w);
    ...
    window.GUI.pocbuild.destruct = function(c){
        ...
        new (Ajax.Request)(g_ajaxpath + ("ajax/destruct.php" + g_ajaxsuffix), {
            ...
            onFailure: function(){
				alert("Verbindungsfehler! Bitte später noch mal Versuchen!");
            }
        })
    };
    ...
    window.GUI.pocbuild.display = function(a){
        ...
        y.push('<a class="button20" id="queuecancel_' + a + '" onclick="GUI.pocbuild.remove(this);"><span>Abbrechen</span></a>');
        y.push('</td></tr></table>');
        return y.join('');
    };
    ...
    window.GUI.poccontrols['refresh'] = function(){
        if (confirm("KoC Toolkit Deutsch: benötige Refresh!")) {
            location.reload(true);
        }
    }
    ...
    window.GUI.options.display = function(a){
        ...
        y.push('<td width="100%">');
		y.push('<b><u>Anischt</u></b><BR>');
        y.push('WideMap');
        y.push('</td><td><td>')
        if (GUI.widemap == false) {
            y.push('<a class="gem_button" id="widemap" onclick="GUI.togglewidemap();"><span>AUS</span></a>');
        }
        else {
            y.push('<a class="button20" id="widemap" onclick="GUI.togglewidemap();"><span>AN</span></a>');
        }
        y.push('</td></tr>');
        
        y.push('<tr><td>');
        y.push('<b><u>Angriff Informationen</u></b>');
        y.push('</td></tr>');
        /*
         y.push('</tr>');
    ...
    window.GUI.pocgui.getdisplay['build'] = function(){
        ...
        if (a.length > 0) {
            ...
            ret.push('<div align="center"> <b> Keine Gebäude Makiert!</b></div>');
        }
        return ret.join('');
    }
    ...
    window.GUI.pocgui.getdisplay['log'] = function(){
        ...
        if (tk_data.log.length > 0) {
            ...
            ret.push('<div align="center"> <b>Keine Logs zur Zeit Vorhanden!</b></div>');
        }
        return ret.join('');
    };
    //ATTACK
    window.GUI.pocgui.getdisplay['attack'] = function(){
        ...
        if (tk_data.attack.length > 0) {
            ...
            ret.push('<div align="center"> <b>Keine Angriffe auf dich! :)</b></div>');
        }
        return ret.join('');
    };
    //SCOUT
    window.GUI.pocgui.getdisplay['scout'] = function(){
        ...
        if (tk_data.scout.length > 0) {
            ...
            ret.push('<div align="center"> <b>Keine Späh Angriffe auf dich! :)</b></div>');
        }
        return ret.join('');
    };
    //FAKE
    window.GUI.pocgui.getdisplay['fake'] = function(){
        ...
        if (tk_data.fake.length > 0) {
            ...
            ret.push('<div align="center"> <b>Keine Fake Angriffe zur Zeit!</b></div>');
        }
        return ret.join('');
    };
	...
	*/
    ...
    window.GUI.settogglenames = function(){
        if (document.getElementById('timer_control')) {
            if (GUI.ispaused) {
                $('timer_control').innerHTML = 'Auto Bau = AUS';
            }
            else {
                $('timer_control').innerHTML = 'Auto Bau = AN';
            }
        }
        
        if (document.getElementById('txt_stopalert')) {
            if (GUI.alerterOn) {
                $('txt_stopalert').innerHTML = 'Alarm = AN';
            }
            else {
                $('txt_stopalert').innerHTML = 'Alarm = AUS';
            }
        }
        
        if (document.getElementById('txt_ask4help')) {
            if (GUI.ask4help) {
                $('txt_ask4help').innerHTML = 'Hilfe = AN';
            }
            else {
                $('txt_ask4help').innerHTML = 'Hilfe = AUS';
            }
        }
    };
    ...
    window.GUI.togglewidemap = function(){
        ...
        if (GUI.widemap) {
            ...
            handler.saveObject(tk_data);
            document.getElementById('refresh_text').innerHTML = "KoC Toolkit Deutsch: Benötige Refresh!";
            GUI.pocupdatedisplay();
        }
        else {
...
function DD_POC_VARIABLES(){
    ...
    window.pochtmls.controls['scout'] = '<a id="con_clearscout" class="gem_button" onclick="GUI.pocgui.control.onclick(this)"><span>Leeren</span></a> \
										<a id="con_stopalert" class="gem_button" style="margin-left: 8px;" onclick="GUI.poctogglealerter()"> \
										<span id="txt_stopalert"></span> \
										 </a>';
    window.pochtmls.controls['fake'] = '<a id="con_clearfake" class="gem_button" onclick="GUI.pocgui.control.onclick(this)"><span>Leeren</span></a>';
    window.pochtmls.controls['log'] = '<a id="con_clearlog" class="gem_button" onclick="GUI.pocgui.control.onclick(this)"><span>Leeren</span></a>';
    window.pochtmls.controls['options'] = '<a id="con_refresh" class="gem_button" onclick="GUI.pocgui.control.onclick(this)"><span id="refresh_text">Benötige kein Refresh!</span></a>';
    window.pochtmls.controls['build'] = '<a class="gem_button" onclick="GUI.poctoggletimer()">';
    
    if (GUI.ispaused) {
        var part2 = '<span style="text-align: center" id="timer_control">Auto Bau = AUS</span></a>';
    }
    else {
        var part2 = '<span style="text-align: center" id="timer_control">Auto Bau = AN</span></a>';
    }
    var part3 = '<a class="gem_button" onclick="GUI.togglebuildmode()"><span style="text-align: center" id="txt_buildmode">Makieren = AUS</span></a>';
    
	if (GUI.ask4help) {
        var part4 = '<a class="gem_button" onclick="GUI.toggleask4help()"><span style="text-align: center" id="txt_ask4help">Hilfe = AN</span></a>';
    }
    else {
        var part4 = '<a class="gem_button" onclick="GUI.toggleask4help()"><span style="text-align: center" id="txt_ask4help">Hilfe = AUS</span></a>';
    }
    ...
    window.pochtmls.mainbody = '<table class="tabs_engagement" id="pochmain" style="position:fixed; top:0px; left:740px;" cellspacing="0" border="0" width="450"> \
								<tr style="color:#FFFFFF; padding:0; background: none repeat scroll 0 0 none;" height="20" width="450" border="0" id="koc_head"> \
								<td style="background-color:none; border-bottom:0px; width: 450px;" > \
									<div style="margin-left:0px;" widht="450" id="koc_menu"> \
										<a id="flag_attack" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_attack">Angriffe</span></a> \
										<a id="flag_scout" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_scout">Späher</span></a> \
										<a id="flag_fake" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_fake">Fake</span></a> \
										<a id="flag_build" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_build">Bau</span></a> \
										<a id="flag_options" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_options">Einstellung</span></a> \
										<a id="flag_log" class="tab" onclick="GUI.pocgui.tabs.onlick(this)"><span id="s_log">Logs</span></a> \
									</div> \
								</td> \
								<td style="background-color:none; border-bottom:0px;"> \
								</td> \
								</tr> \
								<tr height="60px" style="border: 1px solid #A56631;" id="koc_lower"> \
								<td style="background-color:none; border: 1px solid #FF0000;" colspan="2"><b><u>Einstellung</u></b>  \
									...
									<span>Log Leeren</span></a> \
									</div> \
								</td> \
								</tr> \
								<tr> \
								<td style="background-color:none; border-bottom:0px;" height="360px" style="" bgcolor="none" valign="top" colspan="3" id="koc_debugmain"> \
								...
								<td height="36px" style="border: 1px solid #A56631;" colspan="3" id="koc_bottom"> \
									<div style=" position:relative; margin-left:0px"> \
										<a class="button20" style="margin-left: 8px;" onclick="GUI.clearstorage();"> \
											<span style="text-align: center" id="clear">Lokalen Speicher Löschen</span> \
										</a> \
										<a class="button20" style="margin-left: 8px;" onclick="GUI.showLocalStorage();"> \
											<span style="text-align: center" id="clear">Lokalen Speicher Anzeigen</span> \
										</a> \
									</div> \
								</td> \
								</tr> \
...
function DD_KOC_FUNCTIONS(){
    ...
    window.KOC.checkImpendingAttack = function(){
        ...
        Object.keys(seed.queue_atkinc).length > 0) {
            ...
            document.getElementById('s_attack').innerHTML = "Angriffe";
            document.getElementById('s_scout').innerHTML = "Späher";
            document.getElementById('s_fake').innerHTML = "Fake";
            GUI.logattack.add('Keine Angriffe auf dich zur Zeit! :)');
            GUI.logscout.add('Keine Späh Angriffe auf dich zur Zeit! :)');
            GUI.logfake.add('Keine Fake Angriffe auf dich zur Zeit! :)');
            
        }
    }
    window.KOC.AlertOnImpendingAttack = function(){
        var soundSrc, playerSrc;
        soundSrc = "http://koc.god-like.info/alarm.mp3";
        playerSrc = "http://koc.god-like.info/mediaplayer.swf";
    ...
    window.KOC.attack_viewimpending_view = function(c){
        ...
        if (marchType == "Attack" || marchType == "Angriff") {
            if (is_real_attack == true) {
                a_count++;
                GUI.logattack.add("<b>" + marchType + " von " + attacker + " (" + alliance + ") => ANKUNFT: " + arrivalTime + " mit (" + armySize + ")</b>");
                GUI.logattack.add("Ziel: " + cityname + " (" + xCoord + "/" + yCoord + ") => Ritter (Schlacht/Politik): " + combatLevel + " / " + techLevel);
                GUI.logattack.add(troops2);
                document.getElementById('s_attack').innerHTML = "Angriff (" + a_count + ")";
                ...
                f_count++;
                GUI.logfake.add("<b>" + marchType + " von " + attacker + " (" + alliance + ") => ANKUNFT: " + arrivalTime + " mit (" + armySize + ")</b>");
                GUI.logfake.add("Ziel: " + cityname + " (" + xCoord + "/" + yCoord + ") => Ritter (Schlacht/Politik): " + combatLevel + " / " + techLevel);
        ...
        if (marchType == "Scout" || marchType == "Späher") {
            s_count++;
            GUI.logscout.add("<b>" + marchType + " von " + attacker + " (" + alliance + ") => ANKUNFT: " + arrivalTime + " mit (" + armySize + ")</b>");
            GUI.logscout.add("Ziel: " + cityname + " (" + xCoord + "/" + yCoord + ") => Ritter (Schlacht/Politik): " + combatLevel + " / " + techLevel);
            GUI.logscout.add(troops2);
            document.getElementById('s_scout').innerHTML = "Späh Angriffe (" + s_count + ")";
...
function DD_DATA_HANDLER(savename, varname){
    ...
    this.saveObject = function(object){
        ...
        if (typeof(localStorage) == 'undefined') {
            alert('Dein Browser Unterstütz kein HTML Store bitte Browser Updaten!');
        }
        else {
            try {
                ...
                if (e == QUOTA_EXCEEDED_ERR) {
                    alert('KoC Toolkit - Deutsch: kann daten nicht Speichern!'); //data wasn't successfully saved due to quota exceed so throw an error
                }
            }
        }
    }
