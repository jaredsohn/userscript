// ==UserScript==
// @name           Gladiatus Provinciarum Arena
// @namespace      janis187
// @description    auto attacker
// @version  	    1.1
// @include        http://s*.gladiatus.*/game/index.php?mod=arena&submod=serverArena&aType=2*
// @require         	http://code.jquery.com/jquery-latest.js
// ==/UserScript==

var getonoff = GM_getValue("gladiatus_prar_onoff");
...
$('#onoff').click(function(){
    ...
    if ( onoff == 'off') {
        GM_setValue('gladiatus_prar_onoff', 'on');
        $(this).val('on');
    } else if ( onoff == 'on' ) {
        GM_setValue('gladiatus_prar_onoff', 'off');
        $(this).val('off');
    }
...
function startArena(ID,server,country,name,hash)
{
    ...
    $.ajax({
        url: "ajax.php?mod=arena&submod=doCombat&aType=2&opponentId="+ID+"&serverId="+server+"&country="+country+"&opponentName="+name+"&sh="+hash,
        //url: "ajax.php?mod=arena&submod=doCombat&aType=2&opponentId=78102&serverId=5&country=en&opponentName=skilganon&sh="+hash,
        type: 'get',
        dataType: 'html',
        async: false
