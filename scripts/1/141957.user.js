      ...
      albuminfo = song.album;
          ...
          song._doubanuname = unsafeWindow.$("#fm-user").text();
					...
					sc.getInfo(song, function(p){
						...
						var lastfmCount = document.getElementById("lastfm-count");
                        var fmPlayer = document.getElementById("fm-player");
                        var newNode = document.createElement("div");
                        newNode.id = "lastfm-count";
                        newNode.innerHTML = "<span style='margin-left: 385px; color: #3868FF;'>在last.fm中记录 " + p.len + " 次</span>";
                        if (lastfmCount){
                            fmPlayer.replaceChild(newNode, lastfmCount);
                        }else{
                            fmPlayer.appendChild(newNode);
                        }
					});
					break;
				case cmds.end:
