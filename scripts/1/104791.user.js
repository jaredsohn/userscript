// ==UserScript==
// @name           Kapibados Time 2 Title
// @namespace      Dynasty
// @description    Zeigt an wenn was fertig ist
// @date           14.06.2011
// @version        1.1
// @include        http://s*.kapibados.de*
// ==/UserScript==
...
var loc=document.location.toString().match(/http:\/\/s(.*?)\.kapibados\.de\/(.*?)\.php/);
switch (loc[2]) {
...
function soundplay()
{
  if (!$("player"))
  {
    var player = createElement('div', {'id': 'player', 'style': 'text-align:left;border: 1px solid black; position:absolute; top:2px; right:2px; padding:2px; z-index:1; background-color:white; color:black;'}, document.body);
    player.innerHTML='\
<object height="20" width="84" data="http://emff.sourceforge.net/codegenerator/skins/emff_silk.swf" type="application/x-shockwave-flash">\
...
function do_garten_map()
{
  ...
  window.setInterval(function () {
    ...
    if (lasttime == 9999999999)
      lasttime=Now;
    if (lasttime-Now<=0)
    { soundplay(); parent.document.title="FERTIG - Kapibados"; }
    else
    { soundstop(); parent.document.title=time2str(lasttime-Now)+" - Kapibados"; }
  },500);
}