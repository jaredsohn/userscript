...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'** I\'m being attacked! **', aIncoming:'troops', aEmbName:'embassy', aSlots:'slots', aDef:'My troops are', aHide:'My troops are',  scouting:false, wilds:false, defend:true, minTroops:10000, spamLimit:10, lastAttack:0, barbautoswitch:false, raidautoswitch: {}, },
 ...
 if(t.Providers[i].country == t.Providers[Options.celltext.provider].country)
    ...
    m += '</select></td></tr>\
        ...
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=left>Message Prefix: &nbsp; <INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'"\></td><TD align=right> &nbsp &nbsp &nbsp &nbsp Alert on scouting: &nbsp </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=left>My city at ?,? is being attacked by ???. &nbsp &nbsp Incoming &nbsp; <INPUT id=pbalertIncoming type=text size=10 maxlength=120 value="'+ Options.alertConfig.aIncoming +'"> &nbsp (arriving in X) </td><TD align=right> &nbsp &nbsp &nbsp &nbsp Alert on wild attack: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=left>My &nbsp; <INPUT id=pbalertEmbName type=text size=10 maxlength=120 value="'+ Options.alertConfig.aEmbName +'"> &nbsp has X of X &nbsp <INPUT id=pbalertSlots type=text size=10 maxlength=120 value="'+ Options.alertConfig.aSlots +'"> &nbsp available. &nbsp;</td><TD align=right> &nbsp &nbsp &nbsp &nbsp Display defend status: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
        <TR><TD><span style="color:#800; font-weight:bold">&nbsp &nbsp &nbsp &nbsp &nbsp ^ embassy ^</td></tr>\
            <TR><TD><span style="color:#800; font-weight:bold">WARNING <span style="color:#800; font-weight:bold"><sup>Replacing the word embassy will prevent your alliance</td></tr>\
            <TR><TD><span style="color:#800; font-weight:bold"><sup>members from receiving a sound alert if you are being attacked.</td></tr>\
        <TR><TD><BR></td></tr>\
            <TR><TD align=right>Comment Defending: &nbsp; <INPUT id=pbalertDef type=text size=35 maxlength=120 value="'+ Options.alertConfig.aDef +'"\> &nbsp DEFENDING! </td><TD align=right>Minimum # of troops: &nbsp </td><TD><INPUT id=pbalertTroops type=text size=3 value="'+ Options.alertConfig.minTroops +'" \><span id=pbalerterr></span></td></tr>\
            <TR><TD align=right>Comment Hiding: &nbsp; <INPUT id=pbalertHide type=text size=35 maxlength=120 value="'+ Options.alertConfig.aHide +'"\> &nbsp HIDING! &nbsp &nbsp &nbsp &nbsp &nbsp </td><TD align=right> &nbsp &nbsp &nbsp &nbsp Stop raids on impending. &nbsp </td><TD><INPUT id=pbalertraid type=checkbox '+ (Options.alertConfig.raid?'CHECKED':'') +'/></td></tr>\
            <TR><TD><BR></td></tr>\
            <TR><TD align=left>To test the output of the Tower Alert send a fake attack from the Test tab.</td></tr>\
            <TR><TD align=left>Check  "Send alliance chat alert as whisper :"  to prevent spamming alliance chat .</td></tr>\
            </table></td></tr>\
        <TR><TD><BR></td></tr>\
...
//   t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
    ...
    document.getElementById('pbalertIncoming').addEventListener ('change', t.e_alertOptChanged, false);
    document.getElementById('pbalertEmbName').addEventListener ('change', t.e_alertOptChanged, false);
    document.getElementById('pbalertSlots').addEventListener ('change', t.e_alertOptChanged, false);
    document.getElementById('pbalertDef').addEventListener ('change', t.e_alertOptChanged, false);
 ...
 Crafting : function (currentcity) {
  ...
  e_alertOptChanged : function (){
    ...
    Options.alertConfig.aIncoming=document.getElementById('pbalertIncoming').value;    
    Options.alertConfig.aEmbName=document.getElementById('pbalertEmbName').value; 
    Options.alertConfig.aSlots=document.getElementById('pbalertSlots').value;       
    Options.alertConfig.aDef=document.getElementById('pbalertDef').value;     
    Options.alertConfig.aHide=document.getElementById('pbalertHide').value;     
  ...
  postToChat : function (m){
	...
	if(m.marchStatus == 9)
		...
		msg += 'My '+ target +' is being '+ atkType  +' by '+ who +'. '+ 'Incoming ' + Options.alertConfig.aIncoming +' (arriving in '+ unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +') : ';
    var totTroops = 0;
    for (k in m.unts){
      var uid = parseInt(k.substr (1));
    ...
    if ( city.tileId == m.toTileId ){
      var emb = getCityBuilding(m.toCityId, 8);
      if (emb.count == 0)
      msg += ' My ' + Options.alertConfig.aEmbName + ' has ' + 'not been constructed in this kingdom.  Do not attempt to reinforce.';
        ...
        msg += ' My ' + Options.alertConfig.aEmbName + ' has ' + availSlots +' of '+  emb.maxLevel + ' ' + Options.alertConfig.aSlots + ' available. ';
        if (t.defMode[m.toCityId] == 0 && Options.alertConfig.defend==true)
        {
            msg+= Options.alertConfig.aHide + ' HIDING!';
        }
        if (t.defMode[m.toCityId] == 1 && Options.alertConfig.defend==true)
        {
            msg+= Options.alertConfig.aDef + ' DEFENDING!';
        }
...
Tabs.Barb = {
  ...
  barbOptions: function(){
  	 ...
  	 if(t.barboptions == null)	
		t.barboptions = new pbPopup ('pbbarboptions', 0,0, 375,375, true);
     ...
     y +='<TR><TD>Knight\'s Attribute : '+htmlSelector({politics:'Politics', combat:'Combat', intelligence:'Intelligence', resourcefulness:'Resourcefulness'}, AttackOptions.knightsattselector, 'id=knightsatt')+' Under Construction</td></tr>';
     y +='<TR><TD>Knight priority : '+htmlSelector({0:'Lowest skill', 1:'Highest skill'}, AttackOptions.knightselector, 'id=barbknight')+'</td></tr>';
     y +='<TR><TD>Max Knight Level: <INPUT id=Knightmax type=text size=3 maxlength=3 value='+ AttackOptions.Knightmaxlvl +' \></td></tr>';
     y +='<TR><TD>Min Knight Level: <INPUT id=Knightmin type=text size=3 maxlength=3 value='+ AttackOptions.Knightminlvl +' \></td></tr>';
     y+='</table></td></tr></table>';
	...
	document.getElementById('barbknight').addEventListener('change', function(){
    	...
    	document.getElementById('Knightmax').addEventListener('change', function(){
		if(parseInt(document.getElementById('Knightmax').value) > 255)
			document.getElementById('Knightmax').value = 255;
		if(parseInt(document.getElementById('Knightmax').value) < 50)
			document.getElementById('Knightmax').value = 50;
		if(parseInt(document.getElementById('Knightmin').value) > document.getElementById('Knightmax').value)
			document.getElementById('Knightmax').value = document.getElementById('Knightmin').value;
		AttackOptions.Knightmaxlvl=parseInt(document.getElementById('Knightmax').value);
		saveAttackOptions();
	},false);
    	document.getElementById('Knightmin').addEventListener('change', function(){
		if(parseInt(document.getElementById('Knightmin').value) > 255)
			document.getElementById('Knightmin').value = 255;
		if(parseInt(document.getElementById('Knightmin').value) < 50)
			document.getElementById('Knightmin').value = 50;
		if(parseInt(document.getElementById('Knightmax').value) < document.getElementById('Knightmin').value)
			document.getElementById('Knightmin').value = document.getElementById('Knightmax').value;
		AttackOptions.Knightminlvl=parseInt(document.getElementById('Knightmin').value);
		saveAttackOptions();
	},false);
