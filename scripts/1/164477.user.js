//Complete credits to Xerotic for making this, I just changed it to Emylbus
// ==UserScript==
// @name			Emylbus Griefing section report.
// @namespace 		xerotic/hfreportingriefing
// @description 	Reporting threads and posts to Emylbus in the griefing section.
// @include  		*hackforums.net/showthread.php*
// @version  		1.1.1
// ==/UserScript==

var navBit = document.getElementsByClassName('navigation')[0];
if(navBit.innerHTML.indexOf('<a href="forumdisplay.php?fid=203">Griefing</a>') != -1) {
	...
	for(var i = 0; i < authBut.length; i++) {
		...
		var formto = '<input type="hidden" name="to" id="to" value="Emylbus, " />';
		var formsubject = '<input type="hidden" name="subject" value="Griefing Section Report" />';
		...
		var formsend = '<input type="submit" class="bitButton" name="submit" value="Report to Emylbus" tabindex="9" accesskey="s" onclick="return confirm(\'Are you sure you want to report this post or thread to Emylbus?\');"/>';
		var formmessage = '<input type="hidden" name="message" value="Here is the post I am reporting: http://www.hackforums.net/showthread.php?pid='+pid+'#pid'+pid+'" />';
