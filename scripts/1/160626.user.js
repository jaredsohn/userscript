...
// @description			Auto Like Facebook by UYAK OGEN. Auto Like status, wall and facebook comments system uses delay timer that allows you to control the speed of access and prevents blocking of the account.
...
// ==/UserScript==
// ======= Jangan Menghapus Credit ======= | Edit by UYAK OGEN
...
if (body != null) {
    ...
    unsafeWindow.AutoLike = function () {
        ...
        alert('Started Auto Like | Edit by UYAK OGEN');
        bouncer_like();
    };
    ...
    unsafeWindow.LikeComments = function () {
        ...
        var prepare = document.getElementsByTagName("a");
        var buttons = new Array();
        for (var i = 0; i < prepare.length; i++) if (prepare[i].getAttribute("data-ft") != null && (prepare[i].getAttribute("title") == "Me gusta este comentario" || prepare[i].getAttribute("title") == "Like this comment" || prepare[i].getAttribute("title") == "Suka komentar ini" || prepare[i].getAttribute("title") == "Nyenengi tanggapan iki" || prepare[i].getAttribute("title") == "" || prepare[i].getAttribute("title") == "このコメントはいいね！" || prepare[i].getAttribute("title") == "좋아요 취소" || prepare[i].getAttribute("title") == "說這則留言讚" || prepare[i].getAttribute("title") == "J’aime ce commentaire" || prepare[i].getAttribute("title") == "")) {
            buttons[Counter] = prepare[i];
            Counter++;
        }
    ...
    unsafeWindow.UpdateVersion = function () {
        alert('You are currently using Auto Like Facebook v.2.1.3 beta 4. Edit by UYAK OGEN');
    };
}