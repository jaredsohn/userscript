...
var SCRIPT = {
	name: 'Legacy Membership Script',
	namespace: 'crohllf',
	source: 'http://userscripts.org/scripts/show/152475',
	identifier: 'http://userscripts.org/scripts/source/152475.user.js',
	meta: 'http://userscripts.org/scripts/source/152475.meta.js',
	version: '1.0',
	date: '2012-11-14',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=5004946';
var postId = '46483893';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4422204&page=2';
// var postId = '39923245';
var forumAccessURL = 'http://goallineblitz.com/game/forum_private_access.pl?forum_id=1366';
...
var list = {
	...
	apprentice: {
		member: {},
		title: 'Former Legacy Members',
		style: 'color: White; background-color: pink; padding: 0px 3px 0px 3px;',
		agentTitle: function(id, name) { return 'Former Legacy Member'; },
		forumAccess: 0,
	},
	promoteme: {
		member: {},
	...
	hitlist: {
		...
		style: 'color: brown; background-color: white; padding: 0px 3px 0px 3px;',
		agentTitle: function(id, name) { return '!! Former Member !!'; },
		forumAccess: 0,
	},
};
var listName = {};
		...
		if (pref.indexHilite.val) {
			$('#content table.forums span.last_post_user').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html(parts[0] + '<br><b style="color: red; background-color: black; padding: 0px 3px 0px 3px;">' + parts[1] + '</b>');
				}
			});
			$('#threads span.thread_starter').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html('Started by: <b style="color: Red; background-color: black; padding: 0px 3px 0px 3px;">' + name + '</b>');
				}
			});
			$('#threads span.last_post_user').each(function() {
				...
				var id = nameToId[name];
				if (list.family.member[id]) {
					$(this).html('<b style="color: red; background-color: black; padding: 0px 3px 0px 3px;">' + name + '</b>');
				}
			});
		}
		break;
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			$.each(names, function(id, name) {
				if (!(id = parseInt(id))) return;
					...
					++nonMemberAddicts;
					$('#fam_list_family a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1366&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.apprentice.member[id]) {
					$('#fam_list_apprentice a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1366&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.waitlist.member[id]) {
					$('#fam_list_waitlist a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1366&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list[idToList[id]].forumAccess == 1) {
					$('#post_content_' + postId + ' a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1366&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				}
			});
			var prefix = ' (';
			...
			if (membersWithTitles) {
				statusMsg += prefix + '<b style="color: red; background-color: black; padding: 0px 3px 0px 3px;">' + membersWithTitles + ' with titles</b>';
				prefix = ', ';
			}
			if (membersFNG) {
				statusMsg += prefix + '<b style="color: green;">' + membersFNG + ' Fucking New Guys :)</b>';
				prefix = ', ';
			}
			if (membersPromoteMe) {
			...
			if (membersWaitList) {
				statusMsg += prefix + '<b style="color: pink;">' + membersWaitList + ' Former Legacy Member</b>';
				prefix = ', ';
			}
			if (membersHitList) {
				statusMsg += prefix + '<b style="color: white; background-color: black; padding: 0px 3px 0px 3px;">' + membersHitList + ' hitlisted</b>';
				prefix = ', ';
			}
			if (unknown.length) {
			...
			statusMsg += ')';
			if (nonMemberAddicts) {
				statusMsg += ', ' + nonMemberAddicts + ' Legacy member has left the building';
			}
