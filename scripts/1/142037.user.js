...
// @namespace      http://userscripts.org/scripts/show/142037
// @description    Позволяет создать список городов которые вы часто атакуете с полными данными.  Так же показывает все ваши укрытия и то сколько шпионов на задании, сколько дома и сколько требуется нанять чтоб заполнить укрытие.
Работает в версиях 0.5.х (десктопной) и 0.4.5 (мобильной).
...
// @version        0.47
//
// @history 0.47   Скрипт русифицирован.
    ...
    */
	...
	drawTargetList:function(div, inInitialMouseOverMode){
		if (FarmList.isVersion042) {
			...
			html.push('<table id="FarmList_targetList" width="100%"><thead><tr class="FarmList_tableTitleRow"><th colspan="12"><a class="FarmList_toggleListButton" id="FarmList_toggleTargetList" title="'+(showTargetList?'Hide':'Show')+'">'+(showTargetList?'[-]':'[+]')+'</a><h2>Список целей</h2></tr>');
			...
			if(showTargetList && !inInitialMouseOverMode){
				...
				html.push('<tr>');
				html.push('<th><img height="20" src="'+FarmList.icons.target+'" title="Цель" class="left"/></th>');
				html.push('<th><img height="20" src="/skin/img/city/building_townhall.png" title="Уровень городской стены"/></th>');
				html.push('<th><img height="20" src="/skin/layout/stadtmauer_icon.png" title="Уровень городской стены"/></th>');
				...
				html.push('<th><img height="20" src="'+FarmList.icons.time+'" title="Время пути"/></th>');
				html.push('<th><img height="20" title="Требуемые сухогрузы" src="/skin/characters/fleet/40x40/ship_transport_r_40x40.png"/></th>');
				html.push('<th><img height="20" src="'+FarmList.icons.combat+'" title="Успешные атаки за 24 часа"/></th>');
				html.push('<th><img height="20" src="/skin/layout/bulb-off.png" title="Игрок"/></th>');
				html.push('<th><img height="20" src="'+FarmList.icons.military+'" title="Количество генералов"/></th>');
				html.push('<th colspan="2"><img height="20" src="/skin/layout/icon-island.png" title="Остров"/></th>');
				html.push('<th class="right" width="205"><img id="FarmList_clearDataButton" height="20" class="button" tite="Очистить все данные" src="'+FarmList.icons.removeTarget+'"></th>');
				...
				for(var i in FarmList.targets.keys){		
					...
					html.push('<td>'+(target.occupied?'<img src="'+FarmList.icons.occupied+'" title="Оккупирован" height="20"/> ':'')+'<a href="/index.php?view=island&cityId='+target.id+'">'+target.name+'</a>'+'</td>');
					...
					html.push('<td class="FL_tooltipAnchor"><a title="Показать остров ('+island.name+')" href="/index.php?view=island&id='+island.id+'">'+island.x+':'+island.y+'</a></td>');
					...
					if (spycount > 0) {
						...
						+'<a class="button" title="Шпионаж склада" id="FarmList_checkResources'+target.id+'"><img src="'+FarmList.icons.warehouse+'" height="18"/></a>'
						+'<a class="button" title="Шпионаж гарнизона" id="FarmList_checkGarrison'+target.id+'"><img src="'+FarmList.icons.barracks+'" height="18"/></a>'
						+'<a class="button" title="Шпионаж онлайна" id="FarmList_checkOnline'+target.id+'" style="width:28px; min-width:28px;"><img src="'+FarmList.icons.checkonline+'" height="18"/></a>'
						+'<a class="button" title="Отозвать шпиона" id="FarmList_recallSpy'+target.id+'"><img src="'+FarmList.icons.recall+'" height="18"/></a>'
					...
					+'<a class="button" id="FarmList_top'+target.id+'" title="Первым в списке"><img src="'+FarmList.icons.top+'" width="13"/></a>'
					+'<a class="button" id="FarmList_up'+target.id+'" title="Передвинуть выше"><img src="'+FarmList.icons.up+'" width="13"/></a>'
					+'<a class="button" id="FarmList_down'+target.id+'" title="Передвинуть ниже"><img src="'+FarmList.icons.down+'" width="13"/></a>'
					+'<a class="button" id="FarmList_ignore'+target.id+'" title="В самый низ"><img src="'+FarmList.icons.ignore+'" width="13"/></a>'
					+'<a class="button" id="FarmList_remove'+target.id+'" title="Удалить"><img src="'+FarmList.icons.removeTarget+'" height="18"/></a>'
					+'<a class="button" href="/index.php?view=plunder&destinationCityId='+target.id+'" title="Набег"><img height="18" src="'+FarmList.icons.pillage+'"/></a>'
					+'<a class="button " href="/index.php?view=blockade&destinationCityId='+target.id+'" title="Блокада"><img height="18" src="'+FarmList.icons.blockade+'"/></a>'
					+'<span class="FL_tooltipAnchor"><a class="button" id="FarmList_sendSpy'+target.id+'" title="Отправить шпиона"><img height="18" src="'+(spycount>0?FarmList.icons.mission:FarmList.icons.espionage)+'"/></a>'+(s==''?'':FarmList.createTooltip(s))+'</span>'
					+'</td></tr>');
				}
				...
				if(ignored.length>0){
					...
					for(var i in ignored){
						...
						var player=FarmList.getPlayer(target.playerId);
						if(done++>0) {
						    html.push('<a id="FarmList_unignore'+target.id+'" title="Игнорировать '+target.name+'">'+target.name+(player?' ('+player.name+' {'+player.status.substr(0,1).toUpperCase()+'}'+')':'')+'</a>');
		...
		// This is still annoyingly slow (~1 seconds if you have 50 or so targets).  But it's hard to make faster.
			...
			$('#FarmList_clearDataButton').click(function(){
				var answer=confirm('Вы собитаетесь удалить все собранные данные.'+NEWLINE+NEWLINE+'Продолжить?');
				if(answer)
					FarmList.clearData();
			});
			...
			if(showTargetList && !inInitialMouseOverMode){
				...
				html.push('<th><img height="20" src="/skin/img/city/building_townhall.png" title="Уровень городской стены"/></th>');
				html.push('<th><img height="20" src="/skin/layout/stadtmauer_icon.png" title="Уровень городской стены"/></th>');
				html.push('<th width="70" title="Требуеться стенобитки"><img src="/skin/characters/military/120x100/ram_r_120x100.png" width="20" title="battering ram"/>/<img src="/skin/characters/military/120x100/catapult_r_120x100.png" width="20" title="catapult"/>/<img src="/skin/characters/military/120x100/mortar_r_120x100.png" width="20" title="mortar"/></th>');
				html.push('<th><img height="20" src="'+FarmList.icons.time+'" title="Время пути"/></th>');
				html.push('<th><img height="20" title="Требуемые сухогрузы" src="/skin/characters/fleet/40x40/ship_transport_r_40x40.png"/></th>');
				html.push('<th><img height="20" src="'+FarmList.icons.combat+'" title="Успшеные атаки за 24 часа"/></th>');
				html.push('<th><img height="20" src="/skin/layout/bulb-off.png" title="Игрок"/></th>');
				html.push('<th><img height="20" src="'+FarmList.icons.military+'" title="Количество генералов"/></th>');
				html.push('<th colspan="2"><img height="20" src="/skin/layout/icon-island.png" title="Island"/></th>');
				html.push('<th class="right" width="205"><img id="FarmList_clearDataButton" height="20" class="button" tite="Удалить все данные" src="'+FarmList.icons.removeTarget+'"></th>');
				html.push('</tr></thead><tbody>');
				...
				for(var i in FarmList.targets.keys){		
					...
					for(var x in target.spies){
						...
						s+='<div class="spy"><img src="'+FarmList.icons.spy+'" height="22"/>'
						+'<a class="button" title="Шпионаж склада" id="FarmList_checkResources'+spy.id+'"><img src="'+FarmList.icons.warehouse+'" height="18"/></a>'
						+'<a class="button" title="Шпионаж гарнизона" id="FarmList_checkGarrison'+spy.id+'"><img src="'+FarmList.icons.barracks+'" height="18"/></a>'
						+'<a class="button" title="Шпионаж онлайна" id="FarmList_checkOnline'+spy.id+'" style="width:28px; min-width:28px;"><img src="'+FarmList.icons.checkonline+'" height="18"/></a>'
						+'<a class="button" title="Отозвать шпиона" id="FarmList_recallSpy'+spy.id+'"><img src="'+FarmList.icons.recall+'" height="18"/></a>'
						+' '+city.name
			    +'</div>'
						;
					}
					...
					html.push('<td class="actions">'
					+'<a class="button" id="FarmList_top'+target.id+'" title="Перым в списке"><img src="'+FarmList.icons.top+'" width="13"/></a>'
					+'<a class="button" id="FarmList_up'+target.id+'" title="Передвинуть выше"><img src="'+FarmList.icons.up+'" width="13"/></a>'
					+'<a class="button" id="FarmList_down'+target.id+'" title="Передвинуть ниже"><img src="'+FarmList.icons.down+'" width="13"/></a>'
					+'<a class="button" id="FarmList_ignore'+target.id+'" title="В конец списка"><img src="'+FarmList.icons.ignore+'" width="13"/></a>'
					+'<a class="button" id="FarmList_remove'+target.id+'" title="Удалить"><img src="'+FarmList.icons.removeTarget+'" height="18"/></a>'
					+'<a class="button" href="/index.php?view=plunder&destinationCityId='+target.id+'" title="Набег"><img height="18" src="'+FarmList.icons.pillage+'"/></a>'
					+'<a class="button " href="/index.php?view=blockade&destinationCityId='+target.id+'" title="Блокада"><img height="18" src="'+FarmList.icons.blockade+'"/></a>'
					+'<span class="FL_tooltipAnchor"><a class="button" id="FarmList_sendSpy'+target.id+'" title="Отправить шпиона"><img height="18" src="'+(spyCount>0?FarmList.icons.mission:FarmList.icons.espionage)+'"/></a>'+(s==''?'':FarmList.createTooltip(s))+'</span>'
		...
		// This is still annoyingly slow (~1 seconds if you have 50 or so targets).  But it's hard to make faster.
			...
			$('#FarmList_clearDataButton').click(function(){
				var answer=confirm('ВЫ собираетесь удалить все собранные данные.'+NEWLINE+NEWLINE+'Продолжить?');
				if(answer)
					FarmList.clearData();
			});
			debugTime('Adding FarmList_clearDataButton (drawTargetsList)');
	...
	drawHideoutList:function(div, inInitialMouseOverMode){
		...
		html.push('<table id="FarmList_hideoutList" width="100%"><thead><tr class="FarmList_tableTitleRow"><th colspan="4"><a class="FarmList_toggleListButton" id="FarmList_toggleHideoutList" title="'+(showHideoutList?'Hide':'Show')+'">'+(showHideoutList?'[-]':'[+]')+'</a><h2>Список укрытий</h2></tr>');
		if(showHideoutList && !inInitialMouseOverMode){
		
			html.push('<tr>');
			html.push('<th class="left"><img height="20" src="'+FarmList.icons.city+'" title="Город"/></th>');
			html.push('<th><img height="20" src="'+FarmList.icons.hideout+'" title="Уровень укрытия (макс. шпионов)"/></th>');
			html.push('<th><img height="20" src="'+FarmList.icons.defending+'" title="% шпионов в защите"/><img height="20" src="'+FarmList.icons.spy+'" title="Шпионы в защите / На задании"/></th>');
			html.push('<th class="right"><img id="FarmList_scanHideouts" class="button" title="Просканировать укрытия" src="'+FarmList.icons.hideout+'" height="20"/></th>');
			html.push('</tr></thead><tbody>');
			...
			for(var i in cities){
				...
				+'<td><a title="Перейти к '+city.name+'" href="/index.php?cityId='+city.id+'" onclick="var sel=document.getElementById(\'citySelect\');for(var i in sel.options){var opt=sel.options[i];if(opt.value==\''+city.id+'\'){sel.selectedIndex=i;}}sel.form.submit();return false;">'+city.name+'</a></td>'
				);
				
				var hideout=IkaTools.cityGetBuildingByType('safehouse',city);
				if(hideout){
					html.push('<td class="center"><a title="Показать укрытие в городе '+city.name+'" href="/index.php?view=safehouse&id='+city.id+'&position='+hideout.position+'">'+hideout.level+'</a></td>');
					...
					if(typeof(city.spiesUntrained)=='undefined'){
						...
						html.push('<td class="right"><a class="button" id="FarmList_trainSpy'+city.id+'">Нанять шпиона</a></td>');
						trainSpyCityIds.push(city.id);
					}else{
						html.push('<td class="right">Все достыпные шпионы наняты!</td>');
					}
					html.push('</tr>');
				}else{
					html.push('<td class="center" colspan="3">Нет укрытия</td></tr>');
				}
				...
				FarmList.targetRefreshData(target,false,function(target){
					...
					IkaTools.getRemoteDocument(url,function(doc){
						...
						if(mission=='resources'){
						    ...
						    if (jqReports.length>0) {
						        success=true;
						        alert('Статус миссии: ' + $(jqReports[0]).text());
						    }
	...
	targetSendSpy_v042:function(targetId) {
		...
		if(IkaTools.getCurrentCity().type){
			alert('Перейдите в свой городе прежде чем отправлять шпиона.');
			return;
		}
			...
			if(answer){
				...
				IkaTools.getRemoteDocument(url,function(doc){
					...
					if($('#mainview #cities',doc).length>0)
						...
						alert('Невозможно отправить шпиона! '+NEWLINE+$('#mainview',doc).html());
						FarmList.setStatus('',status);
					}
				});
			}else{
		...
		if(jq.length>0){
			...
			if(jq.length>0){
				...
				if(jq.length>0)
					...
					alert('Невзможно перейти на остров по ID!  Пожалуйста перейдите на остров через укрытие.');
			}
		}
	...
	addActionButton:function(cityLocation){
		...
		if(!target){
			...
			a.innerHTML='Добавить цель';
			a.title='Добавить в список целей';
			$(a).click(function(){
			...
			a.innerHTML='Удалить цель';
			a.title='Удалить из списка целей';
			$(a).click(function(){
				var target=FarmList.getTarget(targetId);
				if(typeof(target)=='undefined')return;
				var answer=confirm('Удалить все данные для '+target.name+'?');
				if(answer){
					FarmList.removeTarget(target);
					FarmList.draw();
					FarmList.addActionButton(cityLocation);
	...
	targetGetPlayerName:function(target){
		var target=FarmList.getTarget(target);if(!target)return;
		var player=FarmList.getPlayer(target.playerId);if(!player)return;
		return '<a class="playerStatus '+player.status+'" title="Показать топ ('+player.name+' игрок '+(player.status=='отпуске'?'в ':'')+player.status+')" href="?view=highscore&searchUser='+player.name+'">'+player.name+'</a>'+(player.allianceId?'</span> [<a title="Показать альянс ('+player.alliance+')" href="?view=allyPage&allyId='+player.allianceId+'">'+player.alliance+'</a>]':'')
	},
