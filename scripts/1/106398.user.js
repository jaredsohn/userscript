// ==UserScript==
// @name             KOC-Attack Italiano
// @version          ★ By Falco ★
// @namespace        KOC-Attack Italiano
// @homepage         http://userscripts.org/scripts/show/106398
// @description      Versione italiana di KOC Attack extra features. 

// @include        *kingdomsofcamelot.com/*main_src.php*
// @include *apps.facebook.com/kingdomsofcamelot/* 

// @icon           http://img31.imageshack.us/img31/228/falcob.jpg
// @require        http://koc.god-like.info/update/auto-updater.php?id=106398
// ==/UserScript==


var KOCAversion = '<b><span class=boldRed>-Version:★ By Falco ★-</b>';
...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		var m = '<a id="ImportBoxCancel" class="button20"><span>Chiudi</span></a>';
		...
		citysel += '<option value=options>* Opzioni *</option>';		
		citysel += '</select>';
	...
	ShowViewAttacksDialog:function() {
		...
		for(var c=1; c<=11; c++) {
			levelsel += '<option value="'+c+'">Livello '+c+'</option>';
		}
		levelsel += '</select>';
		
		var typesel ='<select id=srcAttackType>';
		typesel += '<option value="All">Tutto</option>';
		...
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Cerca: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Tutto</option>\
		<OPTION value=Camp>Barbari</option>\
		<OPTION value=Wilderness>Terre</option>\
		<OPTION value=Grassland>Praterie</option>\
		<OPTION value=Lake>Laghi</option>\
		<OPTION value=Mountains>Montagne</option>\
		<OPTION value=Forest>Foreste</option>\
		...
		<td class=xtab width=100 align=center>Scegli Città: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Scegli Livello: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Scegli tipo: &nbsp; </td>\
		...
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Pulisci Lista</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Cancella selezione</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Città</td><td>Coordinate</td><td nowrap>Cosa</td>';
		  h += '<td>Tipo</td><td>Attacco Truppe</td><td>Suicidio Truppe/Risorse</td><td>Distanza</td><td>Tempo</td><td>Ignora</td></tr>';
		  var tableRows= '';
		  var count = 1
	  ...
	  tableRows += '<td>'+parseInt(distance)+'</td>';
          ...
          ById('KOCAttackViewAttacksDelete').addEventListener('click',function() {
			...
			window.alert(deletes+' coordinate da cancellare');
	...
	ShowOptionsDialog:function() {
		...
		var attackfirst = '';
		var arrData = [["Camp", "Barbari"],["City", "Città"],["Transport", "Trasporto"],["Wilderness", "Terre"],["None", "Nessuno"]];
		...
		div.innerHTML="<form><table>"+
			...
			"Ordine d'attacco: <select id='KOCAttackOrder'><option value='mostTroops'>Usa più truppe</option><option value='closest'>Bersagli più vicini</option></select><br />"+
			"Attacca: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbari "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Terre "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Città "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Trasporto<br> "+
			"Priorità attacco a : "+attackfirst+
			"<br><br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> secondi tra un attacco e l'altro<br />"+
			"Tempo trascorso tra un attacco e l'altro allo <u>stesso obiettivo</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Attacco barbari:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />ora/e<br />"+
			"Attacco terre:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />ora/e<br />"+
			" Attacco città:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />ora/e<br />"+
			" Trasporto:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />minuti<br />"+
			" Nel trasporto, lascia almeno <input id='KOCAttackTransportReserveAmount' value='"+this.options.transportResourcesReserveAmount+"' size='10' /> di ogni risorsa in ciascuna città.</font>"+
			"</div>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% di regolazione dei ritardi .<br />"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distanza massima per un attacco/trasporto.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Lancia attacchi solo dalla città dove sono stati lanciati la prima volta.<br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> Non selezionare il cavaliere se non sei in modalità attacco.<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Riprova la marcia in caso di errore (premi riaggiorna dopo aver cambiato questa opzione).<br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Ferma l'auto attacco se sei attaccato.<span style='color:red'> (L'attacco non ripartirà dopo l'allarme. Usare a proprio rischio)</span><br />"+			
			"Apri questo indirizzo web (es. file audio/video) in una scheda se sei attaccato:<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Rimozione automatica dei resoconti degli attacchi.<br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Rimozione automatica resoconti attacchi alle terre.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> numero di attacchi max da visualizzare nella finestra.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Disabilita visione resoconti, ciò disabiliterà anche la raccolta dei resoconti nella pagina degli attacchi.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% di popolazione inattiva prima di addestrare in automatico <input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' />% della popolazione massima.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Per abilitare l'addestramento automatico, cliccare il pulsante auto-addestramento sotto il tipo di truppa nella caserma.</span><br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Nota: Se addestri il 100% della popolazione massima, potresti incorrere in errori di cache a causa delle risorse.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gold_30.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% felicità prima di attivare l'Autoraccolta oro.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Per attivare l'oro automatico, cliccare sul pulsante Autoraccolta oro nel castello.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> Disabilita le finestre \"Invita Amici\" durante il gioco.<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Aiuto automatico degli alleati nelle costruzioni/ricerche.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Nascondi le richieste di aiuto degli alleati nella chat (se sopra è attivo, dopo averli aiutati).<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Pubblica in automatico l'aiuto nelle costruzioni nella bacheca di FB.<br />"+
			"Se sopra è attivo, che impostazione di privacy vuoi? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tutti</option><option value='50'>Amici di amici</option><option value='40'>Solo amici</option><option value='10'>Solo io</option></select><br />"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Riconnettiti in automatico al dominio se vieni disconnesso durante la manutenzionedel server.<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Abilita la memorizzazione dei file di log in Firefox (utile se vuoi monitorare un bug del gioco).<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> secondi di passaggio ad un'altra città.<br />"+
			"Ruota tra le città <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> volte e poi aspetta "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />sec prima di riaggiornare.<br />"+
			"Lanciare gli attacchi dalle città: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"Importa/Esporta: copia o incolla le impostazioni qui...<br /><textarea id='KOCAttackImport'></textarea>"+
			...
			"<a id='KOCAttackOptionsSave' class='button20'><span>Salva</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Cancella</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Resetta opzioni</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Resetta tutto!</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Cancella tutti gli attacchi</span></a>"+
			"<a id='KOCAttackDeleteTransports' class='button20'><span>Cancella i trasporti</span></a>"+
			//"<a id='KOCAttackListAttacks' class='button20'><span>Lista attacchi</span></a><a id='KOCAttackListTransports' class='button20'><span>Lista trasporti</span></a>"+
	...
	*/
	...
	SetAttackStatusMessage:function() {
		...
		if(autoAttack) {
			toggle.innerHTML='<span>* Auto Attacco - Sì *</span>';
		} else {
			toggle.innerHTML='<span> * Auto Attacco - No *</span>';
		}
	},
	...
	*/	
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"Copia e incolla qui le coordinate (es. 343,434) una per ogni riga...<br />Attento: aggiungerà le truppe inserite in questa finestra.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> <label for=\"KOCAttackBulkAddForce\">Sovrascrivi gli altri attacchi con le stesse coordinate</label><br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Bersaglio:<br />");
		var arrData = [["Camp", "Barbari"],["City", "Città"],["Transport", "Trasporti"],["Wilderness", "Terre"]];
		for (var i=0; i < arrData.length; i++){
		...
		//AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' /> Questo è un attacco suicida .<br />");
		...
		AddHtml(bulkAddSuicideSpan,'<label for="KOCAttackBulkAddSuicideWave">Questo è un attacco suicida .</label>');
		...
		bulkAdd.innerHTML='<span>Aggiungi coordinate</span>';
		...
		bulkAddAttackLink.innerHTML='<span>Aggiungi coordinate</span>';
	...
	OnCastleBoxAppear:function(box) {
		...
		function SetAutoGoldA() {
			var str=t.GetAutoGold()?'Autoraccolta oro- Sì':'Autoraccolta oro - No';
		...
		function SetAbandonWildsA() {
			var str=t.GetAbandonWilds()?'Abbandona luoghi selvaggi  - Sì':'Abbandona luoghi selvaggi - No';
			b.innerHTML='<span>'+str+'</span>';
		}
		b.addEventListener('click',function() {
			var abandonWilds=t.GetAbandonWilds();
			if(!abandonWilds){
				var abandonWildsConfirm = confirm("Sei sicuro di voler abbandonare automaticamente tutti i luoghi selvaggi?")
	...
	OnAttackBoxAppear:function(box) {
		...
		AddHtml(suicideSpan,'<label for="KOCAttackSuicideWaveCheckbox">Attacco suicida</label>');
	...
	*/
	...
	AddTrainTroopsLink:function() {
		...
		var setTrainTroopsA=function() {
			...
			a.innerHTML='<span>'+(trainTroops[t.GetCurrentCityId()]==type?'Auto-addestramento - Sì':'Auto-addestramento - No')+'</span>';
		}
		a.addEventListener('click',function() {
			var trainTroops=JSON2.parse(t.GetValue('TrainTroops','{}'));
