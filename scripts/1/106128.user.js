//
// ==UserScript==
// @name			Dirty Service Pack 2, voted
// @author			Stasik0, BearOff, crea7or, flashface, slavka123, dirty_user 
// @namespace		http://dirty.ru/
// @description		Dirty Service Pack 2.6.6
...
var _$ = {
	...
	$comments: function (){
		return _$.$c('comment', document, 'div');
	},
	
	$commentsHolder:  function() {
			return _$.$('js-commentsHolder') || _$.$('profile_comments');
	},
	
	CheckCommentVoted: function(post_id, id,key, callback){
		var data = 'id=' + id +'&type=0&post_id=' + post_id ;
		var url  = '/votesctl' ;	
		var user = _$.getUsername();
     	var realWindow  = _$.GetRealWindow();
        var crossBrowserJSObject = (realWindow.wrappedJSObject || realWindow);
        var $A = crossBrowserJSObject.$A;

			
		_$.ajaxLoadPost(url,data,function(ajaxobject,params){
			var dup_temp = ajaxobject.responseText;
			var result = 0;
			var response = jsonParse(dup_temp);
			...
			callback(result,key,id,post_id);
		});
...
if( typeof _$.settings.vote_in_comments == "undefined") { _$.settings.vote_in_comments = 1; settingsSave = true; }
...
function dsp_tooltip_init(){
	...
	add_checkbox_event('dsp_c_vote_in_comments','vote_in_comments');
...
function DSP_make_content_settings(){
	...
	if(_$.$('dsp_setting_button_0').innerHTML.length<10){
		...
		dsp_txt += '<tr><td width="25" valign="top"><input id="dsp_c_vote_in_comments" type="checkbox" '+((_$.settings.vote_in_comments=='1')?'checked="checked"':'')+'></td><td style=""><label for="dsp_c_vote_in_comments">VOTED: Голосовать со страницы комментариев</label></td></tr>';
	...
	if (_$.settings.ignorlist == '1')
	{
		...
		else
		{
			...
			if (location.pathname.indexOf('/comments/') > -1)
			{
				...
				var spaceSpan;
				var commentsHolder = _$.$comments();
				for (var indexOfComment = 0; indexOfComment < commentsHolder.length; indexOfComment++)
				{
						currentCommentId = commentsHolder[indexOfComment].getAttribute('id');
						currentCommentBody = commentsHolder[indexOfComment].querySelector('div.c_body');
						currentCommentHeader = commentsHolder[indexOfComment].querySelector('div.c_footer');
...
//make vote possible in comments


if( _$.settings.vote_in_comments == '1'){
	if(_$.location.indexOf('/comments/') >= 0 &&  _$.location.indexOf('/user/') >= 0) {
		var realWindow = _$.GetRealWindow();
		var user = _$.getUsername();

		//setup voting guid
        var crossBrowserJSObject = (realWindow.wrappedJSObject || realWindow);
		crossBrowserJSObject.voteHandler.wtf = crossBrowserJSObject.wtf_vote;
		
		//setup comments collection
		var commentsHolder = _$.$commentsHolder();
		var allBodies = _$.$c('c_body',commentsHolder,'div');
		var allUsers = 	_$.$c('c_user',commentsHolder,'a');
		var allVotes = 	_$.$c('c_vote',commentsHolder,'div');
		...
		//make cicle by votes
		for (var key = 0; key < allVotes.length; key++)
		{
			var href = allVotes[key].parentNode.getElementsByClassName('c_icon')[0].href.split(/\//);
			var atrs = href[href.length-1].split(/#/); //get post and comment ID
			var time = allUsers[key].nextSibling.nodeValue;
			var comUser = allUsers[key].innerHTML; 
			...
			if (re.exec(time)) {
				...
				if ((cd-td)<0  || atrs[0]<340000 || comUser==user) {
					allVotes[key].childNodes[1].style.color = '#CC0000';
				} else {
					var checked = _$.CheckCommentVoted(atrs[0],atrs[1],key, function(result,key,id,post_id)
					{
						// var commentsHolder = _$.$commentsHolder();
						// var allVotes = 	_$.$c('c_vote',commentsHolder,'div');
						...
						a.setAttribute('onclick', "voteHandler.vote(this, '"+id+"', '0', '1', '"+post_id+"'); return false;");
						a.setAttribute('href', '#');
						if(result>0)
							a.setAttribute('class', 'vote_button vote_button_plus vote_voted');
						else
						...
						s.setAttribute('onclick', "voteDetailsHandler.show({type:'comment', button:this, id:'"+id+"', post_id:'"+post_id+"'});");
						s.setAttribute('class', 'vote_result');
						t = document.createTextNode(allVotes[key].childNodes[1].firstChild.nodeValue);
						s.appendChild(t);
						d.appendChild(s);
						...
						a.setAttribute('href', '#');
						if(result<0)
							a.setAttribute('class', 'vote_button vote_button_minus vote_voted');
						else
						...
						allVotes[key].parentNode.replaceChild(d,allVotes[key]);
