...
(function () {
    ...
    function allInOneOpera() {
        ...
        var About = {
            Version: { Now: '10.0.5.2', New: '10.0.5.3' },
...
.appendTo($('#logoutContainer'));
        ...
        function SubLanguage(sLang, oType) {
            ...
            if (sLang == 'hu') subLang['hu'] = ['Beállítások', 'Jegyzetek', 'Épületlista megjelenítése', 'Épületlista elrejtése', 'Mentés', 'nyersanyag küldése ide: ', 'egységek küldése ide: ', 'Linkek', 'Hozzáadás', 'Link URL:', 'Link Név:', 'Link törlése:', 'hasznos linkek', 'nyersanyag fejlodés kalkulátor', 'Bovített harcszimulátor', 'távolság kalkulátor', 'Búzakereso', 'Elefántkeresp', 'Sugár', 'Start', 'Keresés', '-tól', '-ig', 'Százalék', "frissítés", "A legújabb verzióval rendelkezel!", "Lehet, hogy béta verzióval rendelkezel", "elérheto egy újabb frissítés", "telepíted?", 'üzenetküldés ikon megjelenítése', 'Támadás táblázatba kiíratása', 'Egység/nyersanyag küldés megjelenítése', 'Támadó', 'Védo', 'Veszteségek', "információ", "profit", "Veszteségek", 'Minden oázis', 'Tekintse meg, válassza az összes üzenetek és jelentések', 'Az összes kijelölése', 'toke', 'Összes Eroforrás', 'teljes', 'sebesség', 'Capture jelentés', 'Jelentés Link ...'];
            ...
            if (sLang == 'sk') subLang['sk'] = ['Nastavenie', 'Poznámky', 'Zobrazit zoznam prác', 'Skryt zoznam prác', 'Uložit', 'poslat suroviny do: ', 'poslat armádu do: ', 'Odkaz', 'Pridat', 'Odkaz URL:', 'Názov odkazu:', 'Zmazat odkaz:', 'Helpodkazy', 'Kalkulacka rozvoju surovín', 'Rozšírený bojový simulátor', 'Kalkulacka vzdialeností', 'Vyhladávac Crop', 'Vyhladávac Slonov', 'Rádius', 'Štart', 'Scan', 'Z', 'Do', 'Percent', "obnovit", "Máte poslednú verziu!", "Je možné mat betaverziu", "nová verzia dostupná", "chcete to nainštalovat..?", 'Zobrazit ikonu poslat správu', 'Zobrazit tabulku informácií oútoku', 'Zobrazit poslat armádu/suroviny', 'Útocník', 'Obranca', 'Straty', "informácie", "zisk", "Straty", "Každé oázy", 'Zobrazit vyberte všetky správy a hlásenia', 'Vybrat všetko', 'Hlavná dedina', 'Všetky suroviny', 'Total', 'Rýchlost', 'Zachytit Hlásenie', 'Odkaz Hlásenia...', 'Zobrazit center císlo', 'Zobrazit informácie obchodníka', 'Zobrazit vzdialenost acas pre obchodníkov na trhu', 'Zobrazit popisky pre hrácov/aliancie ', 'Zobrazit vojsk tipy', 'Zobrazit tabulku surovín', 'Tabulka surovín', 'Zobrazit tabulku pre upgrade surovinových polí'];
    ...
    '<table id="crop_fields" style="border: 0px solid; background-color: white; width: auto;"></table></div>';
        ...
        function setting() {
            function getLanguage(lng, lType) {
                ...
                Language_['hk'] = ['????????', '??????', '???????????', '??????', '??.??.??????????', '??', '????', '????', '?????', '????', '????', '????', '??', '?????', '??????', '????(????)', '????(????????)', '????????'];
                Language_['hu'] = ['építéshez szükséges nyersanyagok megtekintése', 'Legyen egy táblázat az épületekrol a falu alatt', 'Egység és nyersanyagküldés gomb megjelenítése a falulistában', ' jelentésmegnyitás gomb megjelenítése', 'Üzenetmegnyitás gomb megjelenítése', 'Ido és nyersanyag kiszámítása a Kaszárnyában, Istállóban és Muhelyben', 'Bezárás', 'Menü linkek megjelenítése', 'Nyersanyag táblázat megjelenítése', 'Nyersanyag megjelenítése százalékban', 'Falu termelésének megjelenítése', 'menti kapcsolatok', 'kijelzo', 'Szín', "Szín: frissítés elérheto", "Szín: max szint", "Szín: frissítés nem lehetséges (nincs elég szabad eroforrás) ", " Szín: frissítés nem lehetséges (nincs elég kapacitása magtár / raktár) ", " Szín: frissítés keresztül NPC "];
                ...
                Language_['ru'] = ['???????? ??????????? ??? ????????????? ????????', '???????? ??????? ????????????? ?????', '???????? ?????? ??????? ????????/????? ? ?????? ????????', ' ???????? ?????? ???????? ??????', '???????? ?????? ???????? ?????????', '???????????? ??????????? ??????? ? ????? ? ???????, ???????, ??????????', '???????', '?????????? ???? ??????', '?????????? ?????? ????????', '?????????? ??????? ????????', '???????? ???????????? ???????', '??????????? ??????', '????????', '????', "????: ?????????? ????????", "????: ???????????? ???????", "????: ?????????? ?????????? (?? ??????? ????????)", "????: ?????????? ?????????? (?? ??????? ???????? ????????????? / ???????)", "????: ?????????? ????? NPC "];
                Language_['sk'] = ['Zobrazit potrebné suroviny na stavanie', 'Zobrazit tabulku budov pod dedinou', 'Zobrazit ikonyposlat vojakov a suroviny v zozname dedín', ' Zobrazit ikonu otvorit hlásenie', 'Zobrazit ikonu otvorit správu', 'Vyrátatpocet surovín a casu v Kasárnach, Stajniach, Tovární arzenálu', 'Zatvorit', 'Zobrazit ponuky odkazov', 'Zobrazit surovinycasovac', 'Zobrazit suroviny v percentách', 'Ukázat produkciu dediny', 'Uložené Odkazy', 'Zobrazenie', 'Farby', 'Farba aktualizácia k dispozícii', 'Farba max úroven', 'Farba aktualizácie nie jemožné (nedostatok surovín)', 'Farba aktualizácie nie je možné (nestací kapacita sýpok/skladov)', 'Farba aktualizovat cezNPC'];
        ...
        '<tr><td style="direction: ltr;" colspan="3">Language: <select style="direction:ltr;" id="cLang" onchange="cLang();">' +
