...
【其它补充】
 这次改版，google针对不同浏览器类型，不同UA版本，不同访问地区（国内访问/使用VPN访问），不同语系（英语显示/中文显示）都可能有区分式的设计。
 有些版本间是细微的变化，有些版本相差非常大，google的做法客观上导致维护负担的剧增。个人修改仅限于（FF4.0.1+ 中文显示)的情况。
...
function doit(){
	...
	GM_addStyle('\
			   ...
			   /* 分栏区域左对齐 */\
					.mw #center_col {margin-left: 3px !important;}\
			   /* 固定google生成的地图浮窗 */\
...
//	var cites = matchNode('//cite[1][ancestor::ol/li[@class="g" or starts-with(@class,"g ")] and following-sibling::span[@class="vshid"]]',doc);
    var cites = matchNode('//cite[1][ancestor::ol/li[@class="g" or starts-with(@class,"g ")] and following-sibling::span[@class="gl"][descendant::a[contains(text(),"网页快照")]]]',doc);
...
try{
    var i, len;
	var ols = matchNode('//div[@id="ires"]/child::ol[child::li]', doc);
	var resContainer;
    ...
    //对google搜索自动生成的地图信息进行移动，维持分栏美观
    var rhsmap = matchNode('//div[@id="rhs_block"][descendant::div[@id="rhs_map"] or descendant::img[@id="lu_map"]]', doc).snapshotItem(0);
    var resDiv = matchNode('//div[@id="res"]', doc).snapshotItem(0);
    if(rhsmap){
        resDiv.parentNode.insertBefore(rhsmap, resDiv.parentNode.firstChild);
        rhsmap.setAttribute('style','padding-bottom: 1px;');
        }
}
catch(e){
	printErrorLog(arguments.callee.toString().match(/^function\s*[^\(]+?\(/ig).toString().replace(/function\s*/ig,'') + ')', e);
