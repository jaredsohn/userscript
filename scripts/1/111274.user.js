// City of Wonder Wall Manager G+
// version 0.1
// 08-25-2011
// Copyright (c) 2011, Jake Yde
...
// select "City of Wonder Wal Manager G+", and click Uninstall.
//
// --------------------------------------------------------------------
//
// ==UserScript==
// @name           	City of Wonder Wall Manager G+.
// @namespace       http://userscripts.org/scripts/show/111274
// @description     Collect Bonuses and Assist Build Marvels for the Google+ game City of Wonder
// @include         https://plus.google.com/games
// @include			https://plus.google.com/games/notifications
// @run-at          document-end
...
// Changelog
//
// 0.5 Back to inkering with DSN's version of the script. ID'ed the 
// application ID in G+, found the class for the +1 button. Working
// on deciphering the rest.
//
// 0.4 removed all traces of the aforementioned and uploaded a placeholder
//
// 0.3 - The earlier version of FVWm was also poorly documented so we are 
// back to square 1. No worries all great things take time.
//
// 0.2 - Using the previous CoW WM has been scrapped as an idea due to poor 
// documentation instead we're going to use the beta of Joe Simmons' FVWM 
// Joe's kinda got a little bitchy about derivative works these days, but 
// but luckily he's released erlier versions under this very license. So 
// we can use that lovely code as a base instead. Specifically he changed
// it on 11-17-2009 @ 21:04 so we can only use code previous to that time
// as our base, luckily it is still publicly available.
// 
// 0.1 - 8.25.2011 Just getting started! :D it should be noted that the bulk 
// of this script was created by DSN for City of Wonders on Facebook, and 
// large parts of it were also created by Joe Simmons all I am doing here
// is trying to adapt it to work effectively in Google+
//
...
// Script Variables
var applicationID = "907809777960";
...
var wonder = {
  core: {
    init: function () {
      ...
      try {
        wonder.core.uname = document.getElementsByClassName("a-f-j-Ja a-f-ud-sr a-ud-sr b-n-l")[0].text;
        wonder.core.uid = document.getElementsByClassName('a-f-j-Ja a-f-ud-sr a-ud-sr b-n-l')[0].href.split("=")[1];
      }
      catch (e) {
        loaded++;
        if (loaded >= 5) {
      ...
      var plusMarvel = wonder.opts.get("opts." + wonder.core.uid + ".plusmarvel")
      var plusBonus = wonder.opts.get("opts." + wonder.core.uid + ".plusbonus")
      var dimCompleted = wonder.opts.get("opts." + wonder.core.uid + ".dimcompleted");
      if (autoExpand === undefined) autoExpand = true;
      if (plusMarvel === undefined) plusMarvel = true;
      if (plusBonus === undefined) plusBonus = true;
      ...
      // Load Plus Marvel Assist Option
      if (plusMarvel === true) {
        wonder.opts.plusmarvel = true;
        wonder.opts.set("opts." + wonder.core.uid + ".plusmarvel", true);
      }
      else {
        wonder.opts.plusmarvel = false;
        wonder.opts.set("opts." + wonder.core.uid + ".plusmarvel", false);
      }
      // Load Plus Bonus Collection Option
      if (plusBonus === true) {
        wonder.opts.plusbonus = true;
        wonder.opts.set("opts." + wonder.core.uid + ".plusbonus", true);
      }
      else {
        wonder.opts.plusbonus = false;
        wonder.opts.set("opts." + wonder.core.uid + ".plusbonus", false);
      }
      ...
      wonder.utils.logger("Plus Bonuses Collected: " + wonder.opts.plusbonus, "debug");
      wonder.utils.logger("Plus Marvel Assissted: " + wonder.opts.plusmarvel, "debug");
  ...
  opts: {
    ...
    config: function () {
      ...
      // Check and Set Plus Marvels Option
      if (plusMarvel === undefined) {
        wonder.opts.plusmarvel = true;
        var plusChecked = "checked";
      }
      else if (plusMarvel === true) {
        wonder.opts.plusmarvel = true;
        var plusChecked = "checked";
      }
      else if (plusMarvel === false) {
        wonder.opts.plusmarvel = false;
        var plusChecked = "";
      }
      // Check and Set Plus Bonuses Option
      if (plusBonus === undefined) {
        wonder.opts.plusbonus = true;
        var bPlusChecked = "checked";
      }
      else if (plusBonus === true) {
        wonder.opts.plusbonus = true;
        var bPlusChecked = "checked";
      }
      else if (plusBonus === false) {
        wonder.opts.plusbonus = false;
        var bPlusChecked = "";
      }
      ...
      if (oPanel === undefined || oPanel === null) {
        ...
        div.innerHTML = "<h4>Wall Manager Options</h4>" + closeImg + "<br/><br/>" + "<label for='autoExpand' id='autoExpandLbl'>Auto Expand 'More Posts'</label>" + "<input type='checkbox' id='autoExpand' " + expandChecked + "/><br/>" + "<label for='plusBonus' id='plusBonusLbl'>'Plus' Bonuses Collected</label><input type='checkbox' id='plusBonus' " + bPlusChecked + "/><br/><label for='plusMarvel' id='plusMarvelLbl'>'Plus' Marvels Assisted</label><input type='checkbox' id='plusMarvel' " + plusChecked + "/><br/>" + "<label for='plusMarvel' id='dimCompletedLbl'>Hide Posts Already Processed</label><input type='checkbox' id='dimCollected' " + dimChecked + "/><br/>" + "<a href='javascript:void(0);' id='cowwm-resetdb'>Reset All Databases</a>" + indent + "<a href='javascript:void(0);' id='cowwm-reset-user-db'>Reset User Database</a>";
        ...
        $('plusMarvel').addEventListener('change', function () {
          wonder.opts.set('opts.' + wonder.core.uid + '.plusmarvel', document.getElementById('plusMarvel').checked);
        }, true);
        $('plusBonus').addEventListener('change', function () {
          wonder.opts.set('opts.' + wonder.core.uid + '.plusbonus', document.getElementById('plusBonus').checked);
        }, true);
  ...
  actions: {
    ...
    getStories: function () {
      ...
      var myName = document.getElementsByClassName('a-f-j-Ja a-f-ud-sr a-ud-sr b-n-l')[0].text;
      var uid = document.getElementsByClassName('a-f-j-Ja a-f-ud-sr a-ud-sr b-n-l')[0].href.split("=")[1];
      ...
      if (stories !== null && stories.length > 0) {
        ...
        for (var i = 0; i < stories.length; i++) {
          ...
          if (appid !== null && appid[2] === applicationID) {
            ...
            var plusLink = stories[i].getElementsByClassName('esw');
            if (plusLink[0] === undefined) {
              plusLink === null;
            }
        ...
        for (var z = 0; blinks.length > z; z++) {
          ...
          var plus = blinks[z].esw;

          wonder.utils.logger("Collecting Bonus from " + blinks[z].actor, "debug");
          wonder.bonus.collect(sid, bonusURL, plus, key);
        }
      ...
      wonder.utils.getExternalResource(url, function (resp, url) {
        ...
        wonder.utils.getExternalResource(nextUrl, function (doc) {
          ...
          if (msg.search("city to see it in all its glory") !== -1) {
            ...
            if (wonder.opts.plusmarvel === true) {
              if (plus !== undefined && plus.title == "Plus this item" && plus !== null) {
                wonder.utils.logger("Marking Marvel " + sid + " Assisted with 'Plus'", "debug");
                plus.click();
              }
            }
            // Dim on Complete
  ...
  bonus: {
    collect: function (sid, url, plus, key) {
      ...
      if (cur >= collectorMax) {
        ...
        setTimeout(function () {
          wonder.bonus.collect(sid, url, plus)
        }, 1000);
        return;
      }
