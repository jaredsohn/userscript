...
var autoaddmoney      = GM_getValue("autoaddmoney", false);
...
if (sanction)
	schecked = " checked";
menu.innerHTML = "\
<input type=\"checkbox\" id=\"autoaddmoney\" "+bchecked+" /> Autobanking<br>\
...
function checkOptions()
{
	...
	if (document.getElementById('autobank').checked)
		GM_setValue("autoaddmoney",true);
	else
		GM_setValue("autoaddmoney",false);
...
function addit()
{
	if (GM_getValue("autoaddmoney",false))
	{
		var cash = getCash();
		if (cash > 0)
			postwith("cashcurrent.php", {depositAmount:cash, action:'Deposit'});
	}
...
autoaddmoney, (1*1000000000000000);
