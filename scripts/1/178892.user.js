...
//Settings to edit:
var maxBid    = 100000; //max you are willing to bid
var minRef    = 222; //min refresh time (ms)
var maxRef    = 223; //max refresh time (ms)
...
} else if(document.body.innerHTML.indexOf("BID SUCCESSFUL") > -1) {
	history.go(-1);
} else if(document.body.innerHTML.indexOf("view the updated list") > -1) {
	history.go(-1);
