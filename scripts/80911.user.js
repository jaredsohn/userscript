...
// end of imperative code.  Only function definitions below.
	...
	if(unsafeWindow.top.playerName == undefined) {
		if(top.frames[1]&&top.frames[1].document&& (bolds=top.frames[1].document.getElementsByTagName('b'))) {
			if(bolds[0]) {
				...
				GM_get('/api.php?what=status&for=AutoChoice',parseName);
			}
		} else {
			GM_get('/api.php?what=status&for=AutoChoice',parseName);
		}
...
//		GM_log("api response = " + txt);
		var CPInfo = JSON.parse(txt);
		playerName = CPInfo["name"];
		if (playerName) playerName = playerName.toLowerCase();
		GM_log("player name parsed as " +playerName);
		unsafeWindow.top.playerName = playerName;
		if(typeof callback == "function") {
			callback();
		}
