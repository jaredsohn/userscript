...
(
    function()
    {
        ...
        var seconds = gSeconds + Math.floor(Math.random() *
gRandom) + 1;
...
gImageIndicator.firstChild.style.width=(200*perc) +'px';
                gImageIndicator.firstChild.innerHTML =
seconds+"s"; 
        ...
        function AddComment()
        {
           ...
           var comment =
document.getElementsByName('comment_body');
            var button =
document.getElementsByTagName('input');

            if(comment.item(0).value == "") {
                comment.item(0).value =
myComment[Math.floor(Math.random() * myComment.length)] +
credits;
            }
            var i = 0;
