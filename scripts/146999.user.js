// ==UserScript==
// @name           OGame Redesign SpioHelper (Spanish by Argone)
// @namespace      -
...
// @updateURL      http://userscripts.org/scripts/source/106772.meta.js
// @downloadURL    https://userscripts.org/scripts/source/106772.user.js
// ==/UserScript==
...
(function(){
	...
	// GM-fix
	if (!this.GM_getValue || (this.GM_getValue.toString && this.GM_getValue.toString().indexOf("no soportado") != -1)) {
	...
	oGameVersionCheck('OGame Redesign SpioHelper','5.0.1','http://userscripts.org/scripts/show/106772');
	...
	var defaults = {
		...
		NAME:		dataName,
		T1:			"tipo",
		T2:			"sumario de reportes de espionaje",
		T3:			"coordenadas",
		T4:			"hora",
		T5:			"jugador (<span style='color:#99CC00;font-weight:normal;'>activity</span>)",
		T6:			"saqueo",
		T7:			"escombros",
		T8:			"defensas",
		T9:			"saltar al reporte de espionaje",
		T10:		'max. $1 ' + ($.trim($('div#countColonies .textCenter').text()).split(' '))[1],
		T11:		 $('ul#menuTable a.menubutton[href$="page=galaxy"] .textlabel').text() + ': mostrar posicion $1',
		T12:		"contador: $1 | saqueos: $2 | escombros: $3",
		T13:		"sin datos",
		T14:		"acciones",
		T15:		"marcar/desmarcar todo",
		T16:		"recursos invertidos",
		T17:		"puntos",
		T18:		"marcar/desmarcar reporte",
		T19:		"abrir/cerrar configuración",
		T20:		"añadir",
		T21:		"guardar",
		...
		T24:		"selección preferida de flota",
		T25:		"abrir flota en una nueva pestaña/ventana",
		T26:		"botones de acción después de ...",
		T27:		"formato de números en la tabla",
		T28:		"maximizar/minimizar tabla",
	...
	if (data.DFPERCENT === undefined || data.DFPERCENT.toString().length == 0) {
		if (console) console.log('load: /api/serverData.xml');
		ready = false;
	...
	if (data.TECH === undefined || $.isEmptyObject(data.TECH)) {
		if (console) console.log('load: /game/index.php?page=globalTechtree');
		ready = false;
	...
	var parseBericht = function(bericht){
		...
		rc.LOOT = {
			...
			s:	Math.round ((rc.R1 + rc.R2 + rc.R3) * rc.FACTOR)
		};
		data.LOOT += rc.LOOT.s;
		...
		for (var x1 = 0; x1 < data.BEVORZUGT.length; x1++) {
			rc.SHIPS.push(Math.ceil(cap / data.TECH[data.BEVORZUGT[x1]][5]));
		}
		...
		rc.DEBRISFIELD = {
			...
			s: Math.round((rc.FLEET.m + rc.FLEET.k) / 100 * data.DFPERCENT)
		};
		data.DF += rc.DEBRISFIELD.s;
		...
		for (var x2 = 0; x2 < data.BEVORZUGT.length; x2++) {
			l += data.BEVORZUGT[x2] + rc.SHIPS[x2];
		}
		rc.ATTACK += '&' + l;
	...
	var Styles = {
		...
		init: function() {
			...
			Styles.set([
				...
				'#spioHelper .shipSelect, #spioHelper .actionsPositionSelect, #spioHelper .newTabFleet1Select, #spioHelper .numberFormatSelect { background-color: #0F141A; border: 1px solid; width: 200px }',
	...
	var ausgabe = function(berichte){
		...
		for (var i = 0; i < berichte.length; i++) {
			...
			for (var x4 = 0; x4 < data.BEVORZUGT.length; x4++) {
				temp += '<tr><td style="padding-right:10px;">' + data.TECH[data.BEVORZUGT[x4]][0] + ': </td><td class="textRight">' + formatNumber(bericht.SHIPS[x4]) + '</td></tr>';
			}
			...
			// attack
			var attack = $('<a/>').attr('href',bericht.ATTACK).attr('title',bericht.ATTACKTITLE).text('A').addClass('SHicon').addClass('tooltip').appendTo(t3a);
			if (data.NEWTABFLEET1) attack.attr('target','_blank');
		}
		
		var sum = data.T12.replace(/\$1/,berichte.length).replace(/\$2/,formatNumber(data.LOOT)).replace(/\$3/,formatNumber(data.DF));
		var summary = $('<div/>').width(360).text(sum).addClass('summary').appendTo(div);
	...
	var getShipSelect = function(){
		var select = $('<select/>').addClass('shipSelect');
	...
	var getActionsPositionSelect = function(){
		var select = $('<select/>').addClass('actionsPositionSelect');
	...
	var getNewTabFleet1Select = function(){
		var select = $('<select/>').addClass('newTabFleet1Select');
	...
	var getNumberFormatSelect = function(){
		var select = $('<select/>').addClass('numberFormatSelect');
				...
				if (settings.url == 'http://' + data.UNI + '/game/index.php?page=messages' && settings.type == 'POST') {
					...
					if (berichteDOM.size() > 0) {
						...
						berichteDOM.each(function(){
							berichte.push(parseBericht($(this)));
						});
