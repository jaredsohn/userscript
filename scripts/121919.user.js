...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'<option value="80">Q4</option><option value="100">Q5</option><option value="120">Q6</option>' +
            '<option value="200" selected="selected">Q7</option>' +
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;# of fights: <input id="dmgFights" name="dmgFights" value="140" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">go!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>140 fights: </strong>' +
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 200, 140, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 200, 140, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	$j('div#enemy_defeated').before(
		...
		'<big>MultiHIT:&nbsp;</big>' +
		'<input id="multihit_count" type="text" size="3" maxlength="3" value="50" /><button id="multihit_start">HIT!</button>' +
