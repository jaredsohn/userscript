// ==UserScript==
// @name			Facebook Chat Emoticons vmc
// @description	                Facebook chat emoticons plus plus
...
// @version			V.1.0
// @versionnumber		V.0.1
// @namespace			http://userscripts.org/scripts/show/139635
// ==/UserScript==
//
		...
		if(parseInt(getValue('LastUpdate', '0')) + 86400000 <= (new Date().getTime())) {
			try {
				xmlhttpRequest( { method: 'GET',
								  url: 'http://userscripts.org/scripts/source/139635.meta.js?' + new Date().getTime(),
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@version\s+(\d+\.\d+)/)[1] > version) {
			if(confirm(	"There's an update available for 'Facebook Chat Emoticons Bar'.\n" +
			   ...
			   ) openInTab('http://userscripts.org/scripts/source/139635.user.js');
		}
	}
