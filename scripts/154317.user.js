...
function replace_msg(x) {
  $('div.dialog_body').html('Lito ' + x + ' Trabajo terminado! visitanos en <a target="_blank" href="http://www.facebook.com/NSHACK2012">http://www.facebook.com/NSHACK2012</a> By: Zasudark');
}
set_timer();
		...
		$('div.fbProfileBrowserList ul li.fbProfileBrowserListItem.uiListItem').each(function(index) {
			...
			if (COR == '0') {
				...
				if (!$(this).find('input').hasClass('darktips_delete')) {
					...
					$(this).find('div.fsl').prepend('<input type="checkbox" checked="checked" class="NSHACK2012_delete" title="Tick to delete this user." id="' + profileid + '">');
				} else {
