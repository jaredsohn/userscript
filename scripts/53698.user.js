// ==UserScript==
// @name           Ikariam pentru membri inactivi
// @namespace      Pat3utz
...
// ==/UserScript==
/*
 * This script comes from an idea of Pat3utz.
 ...
 */
var lang={
  ro: {'newAlert': 'Membri noi', 'newTotal': 'Total membri noi', 'aband': 'Abandon', 'totAban': 'Total abandon', 'confirm': 'Esti sigur ca vrei sa resetezi inregistrarile ?'},
...
function showToolConfig(e)
{
  ...
  else
  {
    ...
    div.innerHTML = '<h3 class="header" style="padding-top: 8px; height: 20px; background-image: url(http://s3.ikariam.it/skin/layout/bg_sidebox_header.jpg)"><b>Configurare</b></h3>'; 
    ...
    if(tpl == 1) div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1" checked >&nbsp;Imagini</div>';
    else div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1">&nbsp;Imagini</div>';
...
for(var member_name in members)
{
  ...
  if(typeof(actualValues[member_name]) == 'undefined')
  {
    ...
    tdOut.innerHTML = "<b>" + member_name + "</b> Puncte: <b>" + members[member_name] + "</b>";
