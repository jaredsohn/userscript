// ==UserScript==
// @name          YouScrobbler Center
// @description   Scrobbles the currently watching YouTube video to last.fm, optimised for YT-Center
// @author        Daniel Jackel (dev@0x4a.net)
// @namespace     userscripts.org
// @developer     Lukas Ha (http://userscripts.org/scripts/show/119694, http://www.lukash.de/youscrobbler)
// @developer     xanadus (https://userscripts.org/scripts/show/71606)
// @uso:script    357491
// @version       1.2.1c
...
// ==/UserScript==
// @downloadURL   https://userscripts.org/scripts/source/357491.user.js
// @updateURL     https://userscripts.org/scripts/source/357491.meta.js
/**
...
var scriptDownloadUrl = "https://userscripts.org/scripts/source/357491.user.js";
...
function initPreferences () {
  //us_saveValue('us_boxpos', "");
  // position of scrobble-popup
  if (!us_getValue('us_boxpos')) {
    var boxposx = getOffset(document.getElementById('player-api')).left + (parseInt(document.getElementById("player-api").style.width)/2) - 5;         
    var boxposy = getOffset(document.getElementById('player-api')).top + (parseInt(document.getElementById("player-api").style.height)/2) - 50 - 64;   
    us_saveValue('us_boxpos', (boxposx+'px-'+boxposy+'px'));
  ...
  if (!us_getValue('us_automode')) {
    us_saveValue('us_automode', false);
  }
...
function us_movebox(e) {
  console.log(us_getValue('us_drag'));
...
function us_addButton() {
  ...
  var t2 = time.getUTCFullYear()+'%2d'+m+'%2d'+d+'%20'+time.getUTCHours()+'%3a'+time.getUTCMinutes()+'%3a'+time.getUTCSeconds();
    ...
    el.innerHTML =  '.us_box { border-radius: 5px; border: 5px solid #333; background: #fff;'+
          ...
          '.us_infobox { z-index:1000000; background-color: #2B2B2B; border-radius: 5px; padding: 8px; position: fixed; right: 16px; bottom: 9px; border: 1px solid #353535; font-size: 90%; }'+
          '.us_infobox div { color: #666666; margin: 1px 5px; float: left; text-shadow: 0 1px 0 #000000; font-size: 90%; line-height: 1.2;}'+ 
          '.us_infobox div img { float: right; opacity: 0.8; margin: 0 -4px -2px 4px; }'+
          '.us_infobox .sep { color:#AAA; }'+
          '.us_trackinfo { color: #FFFFFF; font-weight: bold; padding-right: 5px; line-height: 1.2; max-height: 2.4em; font-size: 120%; }'+
          ...
          '#us_start_scrobblebutton {padding-left:3px!important; float: left;}'+ //Father check
          '#us_start_scrobblebutton.yt-uix-button.yt-uix-sessionlink.start.yt-uix-button-default:hover {float: left; position: static;}'+ 
          '#us_icon_small { vertical-align: text-bottom; margin-top: 5px;}'+ 
          '#foundInDBIcon { }'+
          ...
          '#us_scrobble_statusbar {float: left; background-color: #559900; display: none; height: 1px; width: 0; maxWidth: 84px; margin: -2px 0px 0px 1px} '+
  ...
  button.innerHTML = '<img id="us_icon_small" style="margin-bottom: -3px;" src="'+us_icon()+'" alt="icon" /><input id="us_temp_info" type="hidden" us_secs="'+secs+'" us_playstart_s="'+t+'" us_playstart="'+t2+'"/><input id="us_resetCore" type="button" style="display:none"/><a class="start" id="us_start_scrobblebutton"> <span id="us_start_scrobblebutton_text">Scrobble</span></a><span class="masthead-link-separator">|</span>';//postxanadus
  ...
  if (document.getElementById("masthead-nav")) {
    ...
    button.innerHTML = '<input id="us_temp_info" type="hidden" us_secs="'+secs+'" us_playstart_s="'+t+'" us_playstart="'+t2+'"/><input id="us_resetCore" type="button" style="display:none"/><a style="border-radius:2px; 2px; 2px; 2px;padding-left:6px!important" class="yt-uix-button yt-uix-sessionlink start yt-uix-button-default" id="us_start_scrobblebutton"><img id="us_icon_small" src="'+us_icon()+'" alt="icon"/> <span id="us_start_scrobblebutton_text">Scrobble</span></a><div id="us_scrobble_statusbar"></div>';
    BFather.insertBefore(button, BFather.firstChild);
  } else if (document.getElementById("mh")){
    BFather = document.getElementById("mh");
    button.setAttribute("class", "ml");
    button.innerHTML = '<img id="us_icon_small" style="margin-bottom: -3px;" src="'+us_icon()+'" alt="icon" /><input id="us_temp_info" type="hidden" us_secs="'+secs+'" us_playstart_s="'+t+'" us_playstart="'+t2+'"/><input id="us_resetCore" type="button" style="display:none"/><a class="start" id="us_start_scrobblebutton">Scrobble</a>';
...
function us_buttonStatus() { 
    if (secs > 30) {
      ...
      document.getElementById('us_icon_small').addEventListener('click', function () {us_autoMode()}, true);
...
function us_autoMode() {
  var mode = us_getValue("us_autoMode");
  mode = !mode;
  var autobutton = '';
  
  if (mode) {
    // red icon, automode on    
    autobutton = document.getElementById('us_icon_small');
    autobutton.style.opacity = "80";
    opacity(autobutton.id, 0, 80, 500);
  }
  else {
    // darker icon, automode off
    autobutton = document.getElementById('us_icon_small');
    autobutton.style.opacity = "40";
    opacity(autobutton.id, 0, 40, 500);
  }
  
  us_saveValue("us_autoMode", mode);
...
function us_closebox() {
  var offset = 0-window.pageYOffset;
  ...
  document.getElementsByTagName("body")[0].style.top = offset + 'px';
  document.getElementById('us_loginbox').style.display = "none";
  window.setTimeout(function() {document.getElementsByTagName("body")[0].style.position = "static"; }, 100);
  window.setTimeout(function() {location.hash = "#page-container"; }, 250);
}

//closes the info-box with fadeout effect
function us_closeinfobox() {
  opacity('us_infobox', 100, 0, 700);
  window.setTimeout(function() { document.getElementById('us_infobox').style.display = "none";}, 750);
}
...
function us_settings(updated) {
    ...
    if (BETACHANNEL) var beta = "Beta"; else var beta = ""; 
    var cont =  '<div id="us_loginbox_form" style="text-align:left"><form name="us_settings_form" onSubmit="return false"><table style="table-layout:fixed"><tr><td class="us_settings_grp us_settings_grp_left">'+
          ...
          '<span>Version: '+VERSION+' '+ beta +' </span><br/><span id="us_manualupdate"><a href="#" id="us_manualupdate_link">Check for Update</a></span></td></tr></table> '+ notice +
    ...
    // unlock scrolling
    window.setTimeout(function() {document.getElementsByTagName("body")[0].style.position = "static"; }, 500);
    window.setTimeout(function() {location.hash = "#page-container"; }, 500);
...
function scrobble_statusbar(status) {
  if (status=="scrobble") {
    document.getElementById("us_scrobble_statusbar").style.backgroundColor = "#559900";
...
function us_scrobble(artist,track,album,mbid,retry,queued,auto) {
  ...
  var time_left_to_scrobble = us_getTempData("us_leftToPlay");
    ...
    if (retry==0) {
      if (auto==1) {
        // Icon "clock--arrow.png" by Yusuke Kamiyamane (http://p.yusukekamiyamane.com)
        // Licensed under CC BY 3.0 (http://creativecommons.org/licenses/by/3.0/deed)
        us_infoBox('<div><span class="us_trackinfo"><span id="us_artist_display">'+artist+ '</span> <span class="sep">-</span> <span id="us_track_display">' +track+'</span></span> scrobbling in '+time_left_to_scrobble+' seconds. <img src="data:image/png;base64," alt="queued" /></div>');
        
        window.setTimeout(function() { us_closeinfobox();}, 4000);
      } else {
        us_boxcontent('Queued...','<div class="us_done">This will be scrobbled in '+time_left_to_scrobble+' seconds. </div>');
        window.setTimeout(function() { us_closebox(); }, 4000);
      }
    }
  }
  else {
    ...
    GM_xmlhttpRequest({
      ...
      onerror: function() {GM_xmlhttpRequest({
        method: "GET",
...
/* function us_scrobble(artist,track,album,mbid,retry,queued,auto) {
  ...
  "&timestamp=" + us_getTempData("us_playstart_s") + "&track=" + encodeURIComponent(track) + "&album=" + encodeURIComponent(album) + "&mbid=" + encodeURIComponent(mbid);
    ...
    try { clearTimeout(TO1); } catch (err) {console.log(err);}
    TO1Helper = true;
    TO1 = setTimeout(function() {us_scrobble(artist,track,album,mbid,1,1,auto)}, ((time_left_to_scrobble + 1) * 1000));
    if (retry==0) {
      if (auto==1) {
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url: scrobbleSongUrl + args,
      onload: function(responseDetails) { console.log(responseDetails); scrobbleFeedback (responseDetails, artist, track, queued) },
      onerror:  
...
function getTrackInfo(){
  ...
  // here now playing transmission?
  //us_nowplaying(decodeURIComponent(us_getTempData("artist")), decodeURIComponent(us_getTempData("track")), decodeURIComponent(us_getTempData("album")), decodeURIComponent(us_getTempData("mbid")), 0);
  return feedback;
}
...
function getVideoSecs() {
  if (document.getElementById('movie_player-html5')) {
    ...
    var scriptsCopy = document.getElementById("movie_player");
      
    //for (var i = 0; i < scriptsCopy.length; i++) {
    
      var x = scriptsCopy.attributes[getAttributeLocation('flashvars',scriptsCopy)].value;//prexanadus WORKING
...
function us_ajax_scanner() {
  ...
  if (us_getTempData("video_is_playing") == 1 && us_getTempData("us_reset_now")!="1" && leftToPlay >= 1) {
...
*   Now Playing
*/
function us_nowplaying(artist,track,album,duration,mbid,retry) {
  var APISIG = "api_keyc04c3a0b2966c04c75c03435ce9b6526artist" + artist + "methodtrack.updateNowPlayingsk" + sk + "timestamp" + timestamp + "track" + track + "714b7c27d70b765f9d8f6d8f52f0a09d";
  var np_url = "https://ws.audioscrobbler.com/2.0/";
  var np_args = "?method=track.updateNowPlaying&artist=" + encodeURIComponent(artist) + "&track=" + encodeURIComponent(track) + "&timestamp=" + us_getTempData("us_playstart_s") + "&sk=" + us_getValue("us_sessionKey") + "&api_key=" + APIKEY + "&api_sig=" + APISIG;
  if (album != 0 && album != "") {
    album==false;
    args += "&album=" + encodeURIComponent(album);
  }
  if (duration != 0 && duration != "") {
    duration==false;
    args += "&duration=" + encodeURIComponent(duration);
  }
  if (mbid != 0 && mbid != "") {
    mbid==false;
    args +=  "&mbid=" + encodeURIComponent(mbid);
  }
  console.log(np_url + np_args);
          ...
          onload: function(resp)
          {
            ...
            if (remote_version > local_version)
            {
              ...
              var cont =  '<div id="us_loginbox_form"><span>YouScrobbler '+ String(remote_version) +' is available.</span><br/>'+
                    ...
                    '<br/>'+
                    '<span><span style="font-weight:bold">Google Chrome</span> users will unfortunately have to manually update if not using Tampermonkey - <a target="_blank" href="http://www.lukash.de/youscrobbler/">Instructions</a></span>'+
