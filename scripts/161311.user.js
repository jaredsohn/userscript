...
// Unload ship (and all but fuelAmount fuel)
...

{
  ...
  var fuelNumber = shipStock[16] + Math.max(0,Math.min(buy[16], fuelAmount - shipStock[16])) - Math.max(0,Math.min(sell[16], shipStock[16] - fuelAmount));

  buyCommodity(16, Math.min(buy[16], fuelAmount - shipStock[16]));
  
  sellCommodity(16, Math.min(sell[16], shipStock[16] - fuelAmount));
...

{
...

  {
    ...
    if (document.getElementById(item) && i != 1 && i != 3 && i != 16)
  ...
  var useSpace = shipStock[1] + shipStock[3] + freeShipSpace() - Math.max(0, fuelAmount - fuelNumber);

  var realSpace = useSpace - useSpace%2;

  var foodAmount = realSpace / 2 + useSpace%2;

  var waterAmount = realSpace / 2;

  sellCommodity(1, Math.min(sell[1], shipStock[1] - foodAmount));

  sellCommodity(3, Math.min(sell[3], shipStock[3] - waterAmount));

  buyCommodity(1, Math.min(buy[1], foodAmount - shipStock[1]));

  buyCommodity(3, Math.min(buy[3], waterAmount - shipStock[3]));
...

{
  ...
  var realSpace = useSpace - useSpace%5;

  var foodAmount = realSpace * 3/5 + useSpace%5;

  var waterAmount = realSpace * 2/5;
...
// load all water for Starbase stocking
...

{
...

  {
    ...
    if (document.getElementById(item) && i != 3 && i != 16)
  ...
  var useSpace = shipStock[3] + freeShipSpace() - Math.max(0, fuelAmount - fuelNumber);
...
// load all food for Starbase stocking
...

{
...

  {
    ...
    if (document.getElementById(item) && i != 1 && i != 16)
  ...
  var useSpace = shipStock[1] + freeShipSpace() - Math.max(0, fuelAmount - fuelNumber);
...

{
...

  {
    ...
    if (document.getElementById(item) && i != 2 && i != 16)
  ...
  var useSpace = shipStock[2] + freeShipSpace() - Math.max(0, fuelAmount - fuelNumber);

  buyCommodity(2, useSpace);
