...
// @name           BBCODE - androidmza
// @namespace      BBCODE - androidmza
// @description    BBCoder para T!
// @include        http://*taringa.net/*
// @exclude        http://br.taringa.net/*
// @version        25.0
// @copyright      Copyright (c) 2009, androidmza
// @creator        androidmza (Andrés) ® 2009
// ==/UserScript==

// **COPYRIGHT NOTICE**
//
//    "BBCoder en Comentarios para T! y P!" Copyright (C) 2008-2009 Andrés
//
		...
		$('#return_agregar_comentario').slideUp(1000, function(){
			...
			$('#return_agregar_comentario').html(HTML).slideDown(1000, function(){
				window.setTimeout(function(){
					GM_xmlhttpRequest({
						method: 'GET',
						url: 'http://'+Dom+'/preview.php?titulo=Preview&cuerpo=' + encodeURIComponent(txt.value),
						onload: function(responseDetails){
							...
							var TitPrev = '<img src="'+URL+'cerrar.gif" alt="[X]" title="'+tarLang["Preview"][3]+'" border="0" ';
							TitPrev += 'onclick="javascript:($(\'#return_agregar_comentario\').slideUp(1000))" ';
							TitPrev += 'onmouseover="this.style.cursor=\'pointer\'" /> ';
							TitPrev += tarLang["Preview"][4]+' - Sharkale®';
							$('.box_cuerpo').attr('class','citacuerpo');
							$('#preview_titulo').html(TitPrev);
							html = $('#post-centro').html();
							$('#return_agregar_comentario').slideUp(1000, function(){
								$('#return_agregar_comentario').html(html).slideDown(1000);
							});
							$('#preview_comentario').remove();
						},
...
Taringa_Set_Function("act_post_nfu", function(){
	...
	$.ajax({
		...
		url: 'http://'+Dom+'/',
		success: function(h){
			secc_nfu = $(h).find('#izquierda').html();
...
Taringa_Set_Function("administrar_post", function(key, numpost){
	...
	$.ajax({
		...
		success: function(h){
			...
			document.documentElement.scrollTop = 0;
			if($('SharkaleAdmPost').size() == 0 || $('#SharkaleAdmPost').is(':hidden'))$('#SharkaleAdmPost').dialog({title: 'Administrador de Post - Sharkale®',width: 900, modal: true, position:['center',30], resizable: false, draggable: false });
		},
		error: function(h){
...
Taringa_Set_Function("actualizar_comentarios_post", function(todos){
	...
	$.ajax({
		...
		success: function(h){
			...
			if(error.size() != 0){
				...
				var html = $(h).find('div[class="box_cuerpo"]:eq(3)').html();
				if(html == null){
					$('.msg_add_comment').hide('slow').html('Error al actualizar comentarios.').show('slow');
					$('.Sharkale_cargando').hide();
					return;
				}
	...
	partes = url.split("/");
	var numpost = (partes[5].search(/\w/)? partes[4]:partes[5]);
	$('.Sharkale_cargando').show();
	$.ajax({
		type: "GET",
		url: "http://"+Dom+"/rss/comentarios/"+numpost,
		dataType: "xml",
		success: function(xmlDoc){
			var x=xmlDoc.getElementsByTagName("item");
			if(x[0].getElementsByTagName("title")[0].childNodes[0].nodeValue == 'Post Privado'){
				$('.msg_add_comment').hide('slow').html('Error al obtener datos del post. Causa: Post Privado').show('slow');
				$('.Sharkale_cargando').hide();
			}else{
			$.ajax({
				type: 'GET',
				url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetPostComments/'+numpost,
				success: function(h){
					var comment = Procesar_Datos(h);
					if(typeof(comment) == 'string'){
						$('.msg_add_comment').hide('slow').html('Error al obtener datos del post. Causa: '+comment).show('slow');
						$('.Sharkale_cargando').hide();
					}else{
					try{
						var HTML = '';
						var i=-1;
						//ultcom = $('#Sharkale_comentarios').find('a[href="javascript:void(0)"]:not(:has(span))').length;
						ultcom = $('div[id*="div_cmnt_"]').size();
						if($('#return_agregar_comentario').is(':visible') && $('#return_agregar_comentario').html() != '') ultcom--;
						for(var j in comment){
							i++;
							if(i >= ultcom || todos){
								nick = x[i].getElementsByTagName("dc:creator")[0].childNodes[0].nodeValue;
								texto = x[i].getElementsByTagName("description")[0].childNodes[0].nodeValue.replace(/\\\&/g, '&').replace(/\[br\]/g,'<br>');
								bbcode = comment[j]["comment"].replace(/\[br\]/g,'\n');
								HTML += shark_print_cmmt(comment[j]["comment-id"],(i+1),nick,nick,bbcode,comment[j]["date"],texto);
							}
						}
						$('.msg_add_comment').hide('slow');
						$('#return_agregar_comentario').html('');
						if(todos)
							$('#Sharkale_comentarios').html(HTML+'<div id="return_agregar_comentario"></div>');
						else
							$('#return_agregar_comentario').before(HTML);
						$('.Sharkale_cargando').hide();
					}catch(e){
						GM_log(e);
						$('.msg_add_comment').hide('slow').html('Error al actualizar comentarios. Intente nuevamente.').show('slow');
						$('.Sharkale_cargando').hide();
					}
					$('.Sharkale_cargando').hide();
					}
...
// ==URL de Imagenes==
	...
	var MenuMensajesVacio = GM_getValue("MMV", "http://i33.tinypic.com/zu1gd5.png");
	var MenuMensajesLleno = GM_getValue("MML", "http://i36.tinypic.com/2w3b577.png");
	var MenuMonitor = GM_getValue("MM", "http://i35.tinypic.com/258uet4.png");
	var MenuFavoritos = GM_getValue("MF", "http://i33.tinypic.com/2hfl37o.png");
	var MenuCuenta = GM_getValue("MC", "http://i35.tinypic.com/rif887.png");
	var MenuCerrarSesion = GM_getValue("MCS", "http://i35.tinypic.com/30awajs.png");
	var MenuModeracion = GM_getValue("MMod", "http://i38.tinypic.com/2qats9l.png");
...
function Restablecer_Imagenes(){
	...
	$('#mmv-url').val("http://i33.tinypic.com/zu1gd5.png");
	$('#mml-url').val("http://i36.tinypic.com/2w3b577.png");
	$('#mm-url').val("http://i35.tinypic.com/258uet4.png");
	$('#mf-url').val("http://i33.tinypic.com/2hfl37o.png");
	$('#mc-url').val("http://i35.tinypic.com/rif887.png");
	$('#mcs-url').val("http://i35.tinypic.com/30awajs.png");
	$('#mmod-url').val("http://i38.tinypic.com/2qats9l.png");
...
function Sharkale_Opciones(){
	...
	nuevaHTML += '    ..::[PS2] · Mis 50 Subidas · [121 Gb\'s]::.. by EternamenteCiego - Querias juegos para PS2, entrá acá.<br/>';
	nuevaHTML += '    <a href="http://www.taringa.net/posts/juegos/3295578/" target="_blank"><img src="http://i29.tinypic.com/2yuyb00.jpg" alt="Descargas Sweetdani" width="740"/></a><hr>';
	nuevaHTML += '    ..::Descargas Sweetdani::.. - Peliculas, Juegos y Series en Rapidshare.<br/>';
	nuevaHTML += '    <a href="http://descargas-sweetdani.blogspot.com/" target="_blank"><img src="http://i27.tinypic.com/2u93k0x.jpg" alt="Descargas Sweetdani" width="740"/></a><hr>';
	...
	nuevaHTML += '    <input type="button" class="ui-state-default ui-corner-all" title="Mis agradecimientos a gonx_666" value="gonx_666">';
...
}else if($("a[class='iniciar_sesion']").size() == 0){
...
///////////////-----------------Agregar Opciones en Comentarios------------------////////////////////
		if(PunFav == true){
			if($('#span_opciones1').size() != 0 || $('#span_opciones2').size() != 0){
				...
				$('span[class="box_txt post_agregar_comment"]:first').html('<span style="float:left;text-align:left;">Agregar un nuevo comentario</span><span style="float:right;text-align:right;font-weight:normal;width: 560px;">'+stropc+'</span>');
			}
		}
///////////////-----------------Agregar Emoticones------------------////////////////////
	...
	if($('div[typeof="foaf:Person"]').find('a[href="/perfil/1378048"]').length != 0){
		...
		mod_sharkale.find('img[title="Ultra Mega User"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','androidmza® La envidia de los Mods...');
...
///////////////------------------Agregar BBCodes--------------------////////////////////
		...
		if(NovatosPrinc){	
			$.ajax({
				...
				success: function(h){
					...
					GM_addStyle("* html #izquierda {width: 300px;}");
					GM_addStyle(".box_txt.ultimos_posts {width: 200px!important;}");
					GM_addStyle("#derecha .box_txt, .box_txt.para_un_buen_post {width: 200px!important;}");
					GM_addStyle("#derecha {padding:2px;float:left;height:auto;width:315px;overflow:hidden;}");
					GM_addStyle(".categoriaPost {font-size:11px;padding:2px 3px 2px 28px;}");
					GM_addStyle("* html #derecha {width: 303px;}");
					GM_addStyle(".box_link {font-size:12px;padding-left:2px;}");
					secc_nov = $(h).find('#izquierda').html();
	...
	perfilde = $.trim($('div[class="box_txt_perfil_izq"]:first').html());
	if(perfilde == "androidmza"){
		var per_androidmza = $('div[class="perfil_izq"]:first');
		per_androidmza.html(per_androidmza.html().replace(/New Full User/,'Ultra Mega User').replace(/No Rankeado/,'1'));
		per_androidmza.find('li:contains(Puntos) span').append('4684');
	}
	GM_addStyle(".link {height:16px;overflow:hidden;padding:3px;width:480px;}");
	if(perfilde.search(user) != -1){
		$('div[class="box_title"]:eq(3)').remove();
		$('div[class="box_perfil_der"]:eq(0)').remove();
		for (var i = 0; i < $('a[href*="/posts/"]').size(); i++) {
			...
			var HTML = '<span><span class="categoriaPost '+categ+'"/>';
			HTML += '<a href="javascript:void(0)" onclick="if(confirm(\'Seguro que deseas borrar este post?\')) if(confirm(\'Te pregunto de nuevo...segur@ que deseas borrar este post?\')) location.href=\'../post.borrar.php?id='+numpost+'&key='+key+'\'" title="Eliminar Post"><img src="http://i29.tinypic.com/242va0i.jpg" align="absmiddle"/></a> ';
			HTML += '<a href="../edicion.form.php?id='+numpost+'" title="Editar Post"><img src="http://i27.tinypic.com/dy6blc.jpg" align="absmiddle"/></a> ';
			HTML += '<a href="javascript:void(0)" onclick="javascript:administrar_post(\''+key+'\',\''+numpost+'\')" title="Administrar Post"><img src="http://i31.tinypic.com/2j0xch2.jpg" align="absmiddle"/></a> </span>';
		...
		for (var i = 0; i < $('a[href*="/posts/"]').size(); i++) {
			var esteLink = $('a[href*="/posts/"]:eq('+i+')');
			var categ = esteLink.attr('href').split('/')[2];
			var HTML = '<span><span class="categoriaPost '+categ+'"/></span>';
			esteLink.before(HTML);
		}
	}
}else if(path == 'fotos'){
...
///////////////-----------------Agregar Emoticones------------------////////////////////
	...
	unsafeWindow.buscador.select('taringa');
}
...
function AceptarActualizacion(){
	if (window.confirm('Hay disponible una nueva versión del script "BBCoder para Taringa" .\nTu versión instalada es la ' + ultversion +
		' y la última disponible es la ' + Act.ver + '\n\nCambios de la última versión:\n' + Act.cambio + '\n\n¿Desea actualizar el script?\n')) {
		GM_openInTab(Act.url);
		GM_openInTab('http://userscripts.org/scripts/show/58290');
	}
}
...
function Guardar_Theme() {
	setup = {dark:$("#znerp").find(":input:eq(0)").val(),
			light:$("#znerp").find(":input:eq(1)").val(),
			top:$("#znerp").find(":input:eq(2)").val(),
			tophover:$("#znerp").find(":input:eq(3)").val()};
...
function Cancelar_Theme() {
	addStyle(eval(GM_getValue('setup','({dark:"#000000", light:"#480C0C", top:"#BBBBBB", tophover:"#CCCCCC"})')));
	window.clearInterval(inter);
	$("#znerp").remove();
}
...
function Cambiar_Theme() {
	document.documentElement.scrollTop = 0;
	var setup = eval(GM_getValue("setup",'({dark:"#000000", light:"#480C0C", top:"#BBBBBB", tophover:"#CCCCCC" })'));
	var html = '<div id="znerp" style="position: absolute; left: '+((window.innerWidth / 2) - 255)+'px; top: 100px; z-index: 1337; background: rgba(255, 255, 255, 0.8) !important;-moz-border-radius:7px;padding:7px !important;-moz-box-shadow:0px 0px 30px rgba(0, 0, 0, 0.9);width: 577px">';
	html += '<center><h2><b>Cambiar Theme de Taringa</b><h2></center>';
	html += '<table><tr style="font-weight:bold;text-align:center"><td>Fondo fuera del borde</td><td>Borde</td></tr>';
	html += '<tr><td><div id="dark"><input value="'+setup.dark+'" class="color" type="text"></div></td>';
	html += '<td><div id="light"><input value="'+setup.light+'" class="color" type="text"></div></td></tr>';
	...
	html += '<tr><td><div id="top"><input value="'+setup.top+'" class="color" type="text"></div></td>';
	html += '<td><div id="tophover"><input value="'+setup.tophover+'" class="color" type="text"></div></td></tr>';
	...
	inter = window.setInterval(function (){
		var darkcolor = $("#znerp").find(":input:eq(0)").val();
		var lightcolour = $("#znerp").find(":input:eq(1)").val();
		var top = $("#znerp").find(":input:eq(2)").val();
		var tophover = $("#znerp").find(":input:eq(3)").val();
  ...
  satValImg.src = 'http://i28.tinypic.com/4hwrcp.jpg';
  ...
  function makeColorSelector(inputBox) {
    ...
    var newSatValImg = satValImg.cloneNode(false);
    ...
    var huePos = huePositionImg.cloneNode(false);
  ...
  $("#dark").find(":input:eq(0)").parent().before(makeColorSelector(document.getElementById("dark").getElementsByTagName("input")[0]));
  $("#light").find(":input:eq(0)").parent().before(makeColorSelector(document.getElementById("light").getElementsByTagName("input")[0]));
  $("#top").find(":input:eq(0)").parent().before(makeColorSelector(document.getElementById("top").getElementsByTagName("input")[0]));
  $("#tophover").find(":input:eq(0)").parent().before(makeColorSelector(document.getElementById("tophover").getElementsByTagName("input")[0]));
}

function addStyle(setup) {
		var css = "a{color:"+setup.dark+";}" +
				"body{background:"+setup.dark+";padding-top:10px;border-color:"+setup.light+";border-bottom:10px solid "+setup.dark+";}" +
				".box_cuerpo{background:#E7E7E7;-moz-border-radius-bottomleft:6px;-moz-border-radius-bottomright:6px;}";
				if(path == 'post'){
				css += ".box_cuerpo{background:"+setup.tophover+";}" +
				".moderacion_del_post {background:"+setup.tophover+" none repeat scroll 0 0;}" +
				".citacuerpo{background:"+setup.tophover+" none repeat scroll 0 0;border:1px solid #AAAAAA;}" +
				"hr{background:#AAAAAA}" +
				".divider{border-bottom:1px solid #AAAAAA;}" +
				"#cuerpo1 {background:"+setup.tophover+" !important}";
				}
				css += "#cuerpocontainer {background:"+setup.top+";}" +
				...
				"#maincontainer{-moz-border-radius:10px;padding:12px;background:url(http://i30.tinypic.com/2zojkea.png)no-repeat;}" +
				"#maincontainer{background-color:"+setup.light+"}" +
				"input.login, .button{background-color:"+setup.light+" !important}";
		GM_addStyle(css);
...
if(CambiarTheme) addStyle(eval(GM_getValue("setup",'({dark:"#000000", light:"#480C0C", top:"", tophover:"" })')));
