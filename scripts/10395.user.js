// ==UserScript==
// @name              WLR for Opera Sans Last Read function
// @namespace     http://forums.whirlpool.net.au
// @version            0.1
// @description     A port Opera of Meat Sack's WLR script. Note: this script has no "last read" function
...
if(document.URL.toLowerCase().indexOf('wlr') > -1){
	...
	$('#replies').prepend(''+
	'<div class="wlrsettings"><h4>Last Read Tracker</h4><input type="checkbox" name="LastReadTracker" disabled="disabled" />'+
	'<span class="infoclick">Info</span><div class="iverunouttanames"><p>Enables/Disables the Last Read Tracker</p>'+
	...
	'<div class="wlrsettings"><h4>Add Last Post Link (#bottom)</h4><input type="checkbox" name="lrTrackBottom" disabled="disabled" />'+
	...
	'<div class="wlrsettings"><h4>Higlight To Right Of Read Post</h4><input type="checkbox" name="highlightRight" disabled="disabled" />'+
	...
	'<div class="wlrsettings"><h4>Hide User\'s Posts </h4><input type="checkbox" name="HidePosts" disabled="disabled" />'+
	...
	$('.wlrsettings input:checkbox').each(function(){
		...
		if(($.browser.msie) && ($(this).attr("disabled") == 'disabled')){
...
if(document.cookie.indexOf('wlrcookie') > -1){


	var docCookArray = document.cookie.split('; ');

	var wlrCookArray = [];
		
	for(ca=0;ca<docCookArray.length;ca++){
	
		if(docCookArray[ca].indexOf('wlrcookie') > -1){
		
			var cookieSplit = docCookArray[ca].split('wlrcookie=')[1];

			wlrCookArray.unshift(cookieSplit); //so oldest cookie is at end
		}
	}

	var wlrCookArrayString = wlrCookArray.toString().replace(/,/g,"");

	var arr;
	
	var regexCheat = wlrCookArrayString+"t%3D";		
...
if((document.URL.indexOf('threads') > -1) || (document.URL.indexOf('user') > -1)&&(document.cookie.substr(document.cookie.indexOf('LastReadTracker:')+16,4) == 'true')){
	...
	$('td[@width=23] a').each(function(){
		...
		if((wlrCookArrayString !== undefined) && (wlrCookArrayString.indexOf(cutHref) > -1)){
		
			var getLastRead = regexCheat.slice(regexCheat.indexOf(cutHref), regexCheat.indexOf('t%3D', regexCheat.indexOf(cutHref)));
			
			var getCount;
			
			(document.URL.indexOf('user') > -1) ? getCount = bottomLinkParent.prev().prev().text()  :  getCount = bottomLinkParent.prev().prev().prev().prev().text();

			if( (Number(getCount) +1) > getLastRead.split('%23r')[1]){ //new posts
					...
					this.href = this.href.replace("&p=-1#bottom","").replace(""+cutHref+"", ""+decodeURIComponent(getLastRead)+"");
...
if (document.URL.indexOf('replies') > -1) {
	...
	var threadNoCheck = $('a').contains('First post')[0].href.replace("&p=-1","");
	var threadNumberWc;
	...
	function writeToCookie(lrl){


		function putStringArrayBackToCookie(lrl ,arr) {

				var w = 0;
				var wlrCookArrayFinal = [];		
		
				for(i=0;i<arr.length;i++) {

					if(wlrCookArrayFinal[w] === undefined) {
						
						wlrCookArrayFinal[w] = "t%3D"+arr[i];


					}
					else if(wlrCookArrayFinal[w].length < 4000){

						wlrCookArrayFinal[w] = "t%3D"+arr[i]+wlrCookArrayFinal[w];
				...
				for (wca=0;wca<wlrCookArray.length;wca++){
					var cookName = wca+'wlrcookie';

					document.cookie = cookName+'='+lrl+wlrCookArrayFinal[wca]+';expires=Fri, 17 Dec 2009 10:00:00 GMT';
				}	
		}
		
		if(document.cookie.indexOf('wlrcookie') < 0){
		
			document.cookie = '0wlrcookie='+lrl+';expires=Fri, 17 Dec 2009 10:00:00 GMT';
		}

		//gotta test this bit
		else if((wlrCookArray[0].length >= 3600) && ( document.cookie.split('; ').length < 19)){ //if current cookie's length is greater than or equal to 3600 (about 4K in size) & there are less than 19 cookies, create a new cookie
			var x = Number(document.cookie.charAt(document.cookie.lastIndexOf('wlrcookie')-1));	//the last index of wlrcookie should be the most recent cookie
			x++;
			document.cookie = x+'wlrcookie='+lrl+';expires=Fri, 17 Dec 2009 10:00:00 GMT';
			
		}
				
		else if(wlrCookArrayString.indexOf(threadNumberWc) < 0){ //if the thread number is not in any cookie, 

			if( document.cookie.split('; ').length < 19){
				var arrSplit = wlrCookArrayString.split( "t%3D" );
				arrSplit.shift();
				arr = arrSplit.reverse(); //split it then reverse it so when allocate back to cookies, the first item(which will be the oldest anchor) will be allocated to the oldest cookie first - which will then be at the end of the cookie's value

				putStringArrayBackToCookie(lrl, arr);						
		...
		else if((wlrCookArrayString.indexOf(threadNumberWc) > -1) && (Number(regexCheat.slice(regexCheat.indexOf('%23r', regexCheat.indexOf(threadNumberWc))+4, regexCheat.indexOf('t%3D', regexCheat.indexOf(threadNumberWc)))) < Number(lrl.split('%23r')[1]))){  //if they move down an already tracked page - should save new anchor (if it's higher) and delete old one - 

			var toSearchFor = regexCheat.slice(regexCheat.indexOf(threadNumberWc)-4, regexCheat.indexOf('t%3D', regexCheat.indexOf(threadNumberWc))); //search & remove

			if( document.cookie.split('; ').length < 19){
				

				var repTosear = wlrCookArrayString.replace(""+toSearchFor+"", "");
				if(repTosear.length === 0){
					document.cookie = '0wlrcookie='+lrl+';expires=Fri, 17 Dec 2009 10:00:00 GMT';
				}
				else{

					var arrSplit = repTosear.split( "t%3D" );

					arrSplit.shift(); //remove first empty split on left
				...
				if(wlrCookArray[wlrCookArray.length-1].slice(wlrCookArray[wlrCookArray.length-1].lastIndexOf("t%3D")).indexOf(threadNumberWc) > -1){ 
					var arrSplit = wlrCookArrayString.split("t%3D");
					arrSplit.shift();	//remove first empty split on left	
					arr.pop();	//remove anchor from the end beacuse it contains the anchor we want to replace anyway				
					arr.pop();	//remove anchor from the end so enough room to put another one in cookie	 
					arr = arrSplit.reverse(); //split it then reverse it so when allocate back to cookies, the first item(which will be the oldest anchor) will be allocated to the oldest cookie first - which will then be at the end of the cookie's value

					putStringArrayBackToCookie(lrl, arr);
				}
				else{
					wlrCookArrayString = wlrCookArrayString.replace(""+toSearchFor+"", "");
	...
	function readFromCookieOnThread(){
		...
		if(document.cookie.indexOf('wlrcookie') > -1){
			...
			if(wlrCookArrayString.indexOf(threadNumberWc) > -1) {

				var anchorfromCookie = regexCheat.slice(regexCheat.indexOf('%23r', regexCheat.indexOf(threadNumberWc))+4, regexCheat.indexOf('t%3D', regexCheat.indexOf(threadNumberWc)));
	...
	function findMin(numArrayLength, tempAnchorAray){ 
		...
		for(j=0;j<numArrayLength;j++){
			...
			if(tempAnchorAray[0] >= 0){
				...
				lrl = encodeURIComponent(hrefArray[j].substr(hrefArray[j].indexOf('?')+1));				
	...
	function saveTheCookieSaveTheWorld(){
		...
		if(document.URL.indexOf('bottom') > -1){
			...
			lrl = encodeURIComponent(hrefArray[hrefArray.length-1].substr(hrefArray[hrefArray.length-1].indexOf('?')+1));
	...
	if((document.URL.indexOf('&ux') < 0) && (document.cookie.substr(document.cookie.indexOf('LastReadTracker:')+16,4) == 'true')){
		...
		if($.browser.opera) { 
			...
			var largestScrollY = document.documentElement.scrollTop + window.innerHeight - 25;
			
			var lastScrollEvent = new Date().getTime();
			
			function operaSucksTehBigOne(){
			
				var eventTime = new Date().getTime();

				if ( ((eventTime - lastScrollEvent) > 500) && ((document.documentElement.scrollTop + window.innerHeight - 25) > largestScrollY) ) {
				
					lastScrollEvent = eventTime;
					
					largestScrollY = (document.documentElement.scrollTop + window.innerHeight - 25);

					saveTheCookieSaveTheWorld();
					
				}
	
			}
			
			window.onscroll = operaSucksTehBigOne;
	...
	if((document.title.indexOf('Coding & Web - Whirlpool Broadband Forums') > -1) && (document.cookie.substr(document.cookie.indexOf('CodeHighlight:')+14,4) == 'true')){
		...
		if($.browser.opera) { 
		    ...
		    el2.type = 'text/css';
		    el2.innerHTML = ".str { color: #080; }.kwd { color: #008; }.com { color: #800; }.typ { color: #606; }.lit { color: #066; }"+
			".pun { color: #660; }.pln { color: #000; }.tag { color: #008; }.atn { color: #606; }.atv { color: #080; }.dec { color: #606; }"+
...
if($.browser.msie) { 
	...
	if(document.cookie.substr(document.cookie.indexOf('Quicknav:')+9,4) == 'true'){
		...
		el3.type = 'text/css';
		el3.innerHTML = "#theHalf_nav { position: fixed; left:146px; top: 24px; font-size:11px;font-family:Arial,sans-serif;"+
		"background: transparent;width: 16px; height:16px;}#theHalf_nav ul, #theHalf_nav li { padding: 0; margin: 0; list-style:"+
		"none; }#theHalf_nav:hover { width:165px; }#theHalf_dropdown { width: 154px; position: absolute; left: -3000px; top: 0;"+
		"background: transparent; }#theHalf_dropdown div { width: 143px; position: absolute; left: -3000px; top: 0;background: "+
		"transparent; }#theHalf_dropdown ul { width: 132px; margin: 11px;border: 1px solid #444;border-width: 0 0 1px; }"+
		"#theHalf_dropdown div ul { margin: 30px 30px 30px 0; }#theHalf_nav a,#theHalf_nav span { display: block; text-decoration: none;}"+
		"#theHalf_dropdown a,#theHalf_dropdown span { padding: 2px; }#theHalf_dropdown span { cursor:default; }#theHalf_nav ul ul li "+
		"{ border: 1px solid #444; border-width:1px 1px 0;}#theHalf_dropdown li { position: relative; }#theHalf_button:hover "+
		"#theHalf_dropdown { left:-12px; top:-33px;}#theHalf_dropdown li:hover div { top:-31px; left:130px;}#theHalf_nav #theHalf_mini-zone "+
		"{ padding: 0; width: 15px; height: 15px; font-size: 1px; left: -3000px;bottom: -15px; top: auto; background: transparent; }"+
		"#theHalf_nav #theHalf_dropdown li:hover #theHalf_mini-zone { left: auto; right: -4px; top: auto; }#theHalf_parent {z-index: 1000;}"+
		"#theHalf_dropdown li { background: rgb(58, 67, 123); }#theHalf_dropdown li:hover { background: orange;color:#fff; }#theHalf_dropdown "+
		"ul ul li { background: rgb(97, 108, 163); }#theHalf_dropdown ul ul li:hover { background: orange;color:#fff; }#theHalf_dropdown li "+
		"a,#theHalf_dropdown li span { color: #fff; }#theHalf_dropdown ul ul li a,#theHalf_dropdown ul ul li a { color: #fff; }#theHalf_dropdown"+
		"ul { border-color:#000000;}#theHalf_nav ul ul li { border-color:#000000; }.theHalf_sep { height:4px; background-color:#000000; }"+
		".theHalf_section:before { content: '- '; }.theHalf_hidden { visibility:hidden; }";
		head3.appendChild(el3);
		
	 
		//copy & paste FTW!! ( © Bravo Worldwide Holdings Inc. )
		$("body").prepend('<div id="theHalf_nav"><ul><li id="theHalf_button"><div id="theHalf_parent"><a href=""><img style="border-style: none; background-color: transparent; width: 16px; height: 16px;" src="data:image/png;base64,"></a><div id="theHalf_dropdown"><ul><li style="z-index: 999;" class="theHalf_drop theHalf_left theHalf_hidden"><span class="theHalf_hidden">Hidden</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="">Hidden</a></li></ul></div></li><li style="z-index: 998;" class="theHalf_drop theHalf_left"><span style="cursor: pointer;" class="theHalf_site">WP User</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-user.cfm?id='+userNumber.split(' ')[1]+'">Contributed Threads</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-user-online.cfm">People Online</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/whim.cfm?show=inbox">Inbox</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/whim.cfm?show=outbox">Outbox</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/whim-contacts.cfm">Contacts</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-subs.cfm">Subscriptions</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-search.cfm">Thread Search</a></li><li class="theHalf_drop theHalf_right"><a href="http://whirlpool.net.au/user.cfm">Your Settings</a></li><li class="theHalf_drop theHalf_right"><a href="http://whirlpool.net.au/login.cfm?logout='+userNumber.split(' ')[1]+'">Log out</a></li></ul></div></li><li style="z-index: 997;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Technology</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=100">Broadband General</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=82">DSL Hardware</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=9">Networking</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=107">Voice over IP</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=63">Coding &amp; Web</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=116">Web Hosting</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=80">IT &amp; Telco</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=87">Peer to peer</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=91">On the internet</a></li></ul></div></li><li style="z-index: 996;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Mobile</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=114">Mobile Carriers</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=123">Mobile Phones</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=124">Mobile Data</a></li></ul></div></li><li style="z-index: 995;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Computers</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=7">PC Hardware</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=10">Windows</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=38">Apple</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=39">Linux/BSD</a></li></ul></div></li><li style="z-index: 994;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Lounges</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=112">Music Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=8">Gaming Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=83">Gadget Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=58">Movie Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=106">TV Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=71">Lifestyle Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=118">Sports Lounge</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=85">In the News</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=117">The Pool Room</a></li></ul></div></li><li style="z-index: 993;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Whirlpool</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=35">Forum feedback</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=92">Choosing an ISP</a></li></ul></div></li><li style="z-index: 992;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Companies</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=14">BigPond</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=15">OptusNet</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=68">Internode</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=72">iiNet</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=69">Netspace</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=94">Westnet</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=90">TPG</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=95">People Telecom</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=102">aaNet</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=105">Exetel</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=109">Adam</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=115">Wild I&amp;T</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=119">AAPT</a></li></ul></div></li><li style="z-index: 991;" class="theHalf_drop theHalf_left"><span class="theHalf_section">Connections</span><div id="theHalf_mini-zone"></div><div><ul><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=67">Regional, Satellite</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=31">Other broadband</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=18">Wireless ISPs</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=99">ISDN, DoV</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=5">Dial-up internet</a></li><li class="theHalf_drop theHalf_right"><a href="http://forums.whirlpool.net.au/forum-threads.cfm?f=97">Peering</a></li></ul></div></li></ul></div></div></li></ul></div>');
