...
// @description    Version 1.0.5 | Deff-Anfragen im Browsergame "Die Stämme" für Mails und das interne Forum mit BB-Codes formatieren.
...
var vers_ist = "DS - Deff-Anfrage 1.0.5";
...
if(url.match(/screen=overview$/)) {
	...
	var deff_truppen = document.getElementsByClassName("vis")[1].getElementsByTagName("tr").length-1;
	...
	if(deff_truppen >= 1) {
		...
		for(i=1; i<=deff_truppen; i++) {
			if(document.getElementsByClassName("vis")[1].getElementsByTagName("tr")[i].getElementsByTagName("td")[0].innerHTML.match(/knight/)) {
				einheit[i] = "Paladin";
				menge[i] = 1;
			}
			else {
				var einh = document.getElementsByClassName("vis")[1].getElementsByTagName("tr")[i].getElementsByTagName("td")[0].innerHTML.split("</strong> ");
				einheit[i] = einh[1];
				menge[i] = document.getElementsByClassName("vis")[1].getElementsByTagName("tr")[i].getElementsByTagName("td")[0].getElementsByTagName("strong")[0].innerHTML.replace("\n", "");
			}
	...
	for(i=0; i<gebaeude; i++) {
		if(gebaeude_tag > gebaeude_nacht) {
			...
			if(test.match(/Wall/)) {
				var wall = document.getElementsByClassName("label")[i].getElementsByTagName("a")[0].innerHTML.split("> ");
			}
		}
		else {
			var test = document.getElementsByClassName("label_night")[i].innerHTML;
			if(test.match(/Wall/)) {
				var wall = document.getElementsByClassName("label_night")[i].getElementsByTagName("a")[0].innerHTML.split("> ");
			}
		}
	}

	// Stufe des Walls als GM-Value speichern:
	GM_setValue("Deff-Wall-" + DorfID, wall[1]);
...
if(url.match(/screen=main/)) {
	...
	var wall = document.getElementsByClassName("vis")[tabelle].getElementsByTagName("tr")[zeile].getElementsByTagName("td")[0].getElementsByTagName("span")[0].innerHTML.replace("\n\t\t\t\t(Stufe ", "").replace(")", "");
...
if(url.match(/screen=place&mode=units/)) {
	...
	for(i=1; i<tabellen; i++) {
		var zeilen = document.getElementsByClassName("main")[0].getElementsByClassName("vis")[i].getElementsByTagName("tr").length;

		if(i == 1) {
			var zeilen = document.getElementsByClassName("main")[0].getElementsByClassName("vis")[i].getElementsByTagName("tr").length-1;
			var zellen = document.getElementsByClassName("main")[0].getElementsByClassName("vis")[i].getElementsByTagName("tr")[0].getElementsByTagName("th").length;
			
			for(n=1; n<zellen; n++) {
				einheit_name[n] = document.getElementsByClassName("main")[0].getElementsByClassName("vis")[i].getElementsByTagName("tr")[0].getElementsByTagName("th")[n].innerHTML.split(" title=\"");
				einheit_ist[n] = document.getElementsByClassName("main")[0].getElementsByClassName("vis")[i].getElementsByTagName("tr")[zeilen].getElementsByTagName("th")[n].innerHTML;
				
				// Daten als GM-Cookie speichern:
				GM_setValue("Deff-" + einheit_name[n][1].replace("\" alt=\"\">", "") + "-" + DorfID, einheit_ist[n]);
				if (GM_getValue("Deff-" + einheit_name[n][1].replace("\" alt=\"\">", "") + "-" + DorfID) == undefined) {
					GM_setValue("Deff-" + einheit_name[n][1].replace("\" alt=\"\">", "") + "-" + DorfID, einheit_ist[n]);
				}
			}
		}
