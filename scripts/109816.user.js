...
function toOperationsPage() {
    do_ajax('inner_page', 'remote/' + MW.getIntURL('socialmission', 'operations_view'), 1, 1);
    return false;
}
/**
 * Hide social module
...
function OperationsCenter() {
    ...
    var Events = {
        refresh_click: function() {
            fullLoad('remote/'+MW.getIntURL('socialmission','view'), 'initiator', function() {
                fullLoad('remote/'+MW.getIntURL('socialmission','view'), 'helper', function() {
                    loadingOverlay('hide');
                    updateOperations();
                });
            });
    ...
    function loadPage(nPage, type, callback) {
        httpAjaxRequest({
            'url': 'remote/'+MW.getIntURL('socialmission','view'),
            'liteLoad': 1,
            'data': { 
                'type': type,
                'page': nPage 
...
function FreeGiftsCenter() 
{
    ...
    function StartCollector(gifts) {
        ...
        var mwRegex      = /facebook.mafiawars.zynga|apps.facebook.zynga.com/;
        var limitRegex   = new RegExp(options.get('excludedPattern'), 'i');
        var nSuccess     = 0;
