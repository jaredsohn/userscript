...
// @version			0.19
// @versionnumber		0.19
// @license			http://www.facebook.com/profile.php?id=1548335484
// @namespace			http://userscripts.org/scripts/show/127098
// ==/UserScript==
//
		...
		if(parseInt(getValue('LastUpdate', '0')) + 86400000 <= (new Date().getTime())) {
			try {
				xmlhttpRequest( { method: 'GET',
								  url: 'http://userscripts.org/scripts/source/127098.meta.js?' + new Date().getTime(),
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@version\s+(\d+\.\d+)/)[1] > version) {
			if(confirm(	"There's an update available for 'Facebook Chat Emoticons Bar'.\n" +
			   ...
			   ) openInTab('http://userscripts.org/scripts/source/127098.user.js');
		}
	}
