...
GM_addStyle('.well.sidebar-nav > .nav { display: none; }'); //hide unused categories
GM_addStyle(".row-fluid .span3 {width: 300px;}"); // left pane width

jQuery('.reader .body-fixed-top').css({ // right pane width
	'width': jQuery(document).width() - 330,
	'left': 300,
	'margin-left': 20
});
...
GM_addStyle(".post.unread {border-left: 4px solid #30A67C}"); // unread item left border color
...
GM_addStyle("html body.reader div.container-fluid div.row-fluid div#main.nano div.slide {top: 55px !important;}");
