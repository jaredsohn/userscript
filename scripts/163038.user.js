...
// @namespace   http://userscripts.org:8080/scripts/review/163038
// @icon        http://s3.amazonaws.com/uso_ss/icon/163038/large.png?1365299642
// @updateURL   https://monkeyguts.com/89.meta.js?c
// @downloadURL https://monkeyguts.com/89.user.js?c
// @homepageURL https://monkeyguts.com/code.php?id=89
...
var scammers = [
        ...
        518237,509037,519871,518070,520930,591847,591751,524199,574330,538223,581220,585801,549749,586082,592206,
        531176,521408,597800,593446,550088,597500,598285,597362,586753,594826,597016,546856,596495,594023,597981,
        597961,580317,594566,594327,531400,593296,597878,597892,538620
];
...
var whitelist = [
        501553,19916,297645,86416,103626,169575,386991,36967,110547,83150,86208,534352
];
...
var thisScriptURL = location.protocol + '//userscripts.org:8080/scripts/show/163038'; 
 
// Tell auto-updater this script's meta data URL, for checking the script's latest version number
var thisScriptMetaURL = location.protocol + '//userscripts.org:8080/scripts/source/163038.meta.js'; 
...
if ((location.pathname.search('/show') > -1) || (location.pathname.search('/review') > -1) || (location.pathname.search('/fans') > -1) || (location.pathname.search('/issues') > -1) || (location.pathname.search('/discuss') > -1)){
        ...
        // Display tweak -- remove excess whitespace
        $('.script_summary:first').siblings('br:lt(2)').remove();
        ...
        // If the author wasn't found in any list, retrieve the script code for scanning
                ...
                $.ajax({
                        url: location.protocol + '//userscripts.org:8080/scripts/source/' + id + '.user.js', 
                        dataType: 'text', 
                        cache: false,
...
if (!$.cookie('_uso_session')) {
        $.ajax({
                url: location.protocol + '//userscripts.org:8080/login', 
                dataType: 'text', 
                async: false
        });
}
...
$('tr[id^="scripts-"]:not(.checked)').each(function(i,val){
        ...
        $.ajax({
                url: location.protocol + '//userscripts.org:8080/scripts/fans/' + suspects[i], 
                dataType: 'text', 
        ...
        // Otherwise, retrieve the script code for scanning
                ...
                getMeta(id).success(function(data){
                        ...
                        if (cache != 0){
                                ...
                                var cacheFilterVersion = cache.split(',')[2];
                        }
                        ...
                        if ( (cacheVersion != usoVersion) || (cache == 0) || (cacheFilterVersion != GM_info.script.version) ){
...
function getMeta(id){
        ...
        return $.ajax({
                url: location.protocol + '//userscripts.org:8080/scripts/source/' + id + '.meta.js', 
                dataType: 'text', 
                cache: false
        });
}
...
function checkScript(data,isList,id,usoVersion,authorText,authorID){
        //Skip scripts that don't contain at least one instance of the word "facebook", "ultoo", "ask.fm", "smarturl"
        if ( (data.search(/facebook/i) > -1) || (data.search(/ultoo/i) > -1) || 
        (data.search(/ask\.fm/i) > -1) || (data.search(/smarturl/i) > -1) || 
        (data.search(/downloadsuppplier/i) > -1) || (data.search(/reliabledownloads/i) > -1) ||
        (data.search(/reliablefiles/i) > -1) || (data.search(/goo\.gl/i) > -1) ||
        (data.search(/bit\.ly/i) > -1) ){
                ...
                if (data.search(/&action\=subscribe/i) > -1){
                        ...
                        if (isList){var reason='ask.fm spam';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/smarturl\.it/i) > -1){
                        if (isList){var reason='smarturl';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/downloadsuppplier\.org/i) > -1){
                        if (isList){var reason='downloadsuppplier';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/reliabledownloads\.org/i) > -1){
                        if (isList){var reason='reliabledownloads';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/reliablefiles\.com/i) > -1){
                        if (isList){var reason='reliablefiles';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/goo\.gl/i) > -1){
                        if (isList){var reason='goo.gl';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                } else if (data.search(/bit\.ly/i) > -1){
                        if (isList){var reason='bit.ly';tagScam(reason, id, authorText, authorID)} else{tagScamPage()}
                }
                else {  
                        var reason = 1;
                }
        ...
        GM_setValue(id,usoVersion + ',' + reason + ',' + GM_info.script.version);
}
...
function tagScam(reason,id,authorText,authorID){
        ...
        } else if ( (reason == 'smarturl') || (reason == 'goo.gl') || (reason == 'bit.ly') ) { 
                var reason = 'Contains URL hider.';
        } else if ( (reason == 'downloadsuppplier') || (reason == 'reliabledownloads') || (reason == 'reliablefiles') || (reason == 'bit.ly') ){ 
                var reason = 'Download link spam.';
        }
        ...
        $(row + 'span.authorWarn').html('<br /><span style="line-height:20px;font-weight:bold;color:darkred;"> ' + designation + ' • ' + 
                '<span style="color:darkblue">"<a style="color:darkblue;text-decoration:none;" href="' + location.protocol + '//userscripts.org:8080/users/' + authorID + '">' + authorText + 
                '</a>"</span> • ID: <a style="color:darkred;text-decoration:none;" href="' + location.protocol + '//userscripts.org:8080/users/' + authorID + '">' + authorID + '</a></span>');
