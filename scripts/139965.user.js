// ==UserScript==
// @name           SRDotDX - IQDragon
// @namespace      tag://kongregate
// @description    Easier Kongregate's Dawn of the Dragons
// @author         SReject, chairmansteve, JHunz, IQDragon
// @version        0.1.3
// @date           08.16.2012
...
// if any1 needs a simple way to calculate damage: atk*4+def+legionpower*5
	...
	window.SRDotDX = {
		version: {major: "0.1.3", minor: "IQDragon"},
		currentUser: active_user.username(),
		...
		config: (function(){
			...
			tmp.markMyRaidsVisited = (typeof tmp.markMyRaidsVisited == 'boolean'?tmp.markMyRaidsVisited:false);
			...
			tmp.emoteTextFormat = (typeof tmp.emoteTextFormat == 'string'?tmp.emoteTextFormat:"*<currentUser> <text>*");
			tmp.emoteTextFormat = tmp.emoteTextFormat.replace(/&#91;/g,"[").replace(/&#93;/g,"]").replace(/&#123;/g,"{").replace(/&#125;/g,"}")
			...
			tmp.addRaid = function(hash,id,boss,diff,seen,visited,user) {
				...
				if (typeof SRDotDX.config.getRaid(id) != 'object') {
					SRDotDX.config.raidList[id] = {
						...
						diff: diff,
						seen: seen,
						visited: (visited || (user == SRDotDX.currentUser && SRDotDX.config.markMyRaidsVisited)),
						user: user,
						lastUser: user,
			...
			if (m) {
				...
				if (typeof raid != 'undefined' && typeof raid != 'null') {
					...
					if (user != ''){
						...
						SRDotDX.config.raidList[raid.id].lastUser = user;
						
						// Do not add this raid to the last raid if the raid isnt displayed.
						if(!SRDotDX.config.hideRaidLinks && (!SRDotDX.config.hideSeenRaids || !raid.seen) && 
							(!SRDotDX.config.hideVisitedRaids || !raid.visited))
						{
							SRDotDX.lastRaidId = raid.id;
						}
					}
		...
		gui: {
					...
					if (typeof a != 'undefined' && a) { 
						...
						if(FPXshoIconLink){
							rh=SRDotDX.gui.cHTML('div').set({class: 'raid_list_item_head'}).html(' \
								...
								<a href="'+url+'" class="FPXlink">raid</a> \
			...
			FPXspamRaids: function () {
				...
				try
				{					
					document.FPXRaidSpamForm.Submit.disable();
					...
					setTimeout(function() {						
							...
							document.FPXRaidSpamForm.Submit.enable();
							SRDotDX.echo("Raid sharing finished");
			...
			ImportRaids: function () {
				console.log("[SRDotDX]::{FPX}:: Import STARTED...");
				try
				{					
					document.FPXRaidSpamForm.Import.disable();
					document.FPXRaidSpamForm.Import.value="Importing...";

					// Retrieve the current number of raids.
					var currentRaids = document.getElementById('raid_list').childNodes.length;
					var importListCount = 0;
					var linklist=document.FPXRaidSpamForm.FPXRaidSpamInput.value;
					if(linklist.length>10)
					{
						document.FPXRaidSpamForm.FPXRaidSpamInput.value = "";
						...
						while(link = patt.exec(linklist))
						{
							SRDotDX.getRaidDetails(link[0], 'Imported');
							// Update the import list count.
							importListCount++;
						}
					}
					
					var imported = document.getElementById('raid_list').childNodes.length - currentRaids;
					
					document.FPXRaidSpamForm.FPXRaidSpamInput.value = "Imported " +  imported + " raids, ignored " + (importListCount - imported) + " duplicates.";
					
					document.FPXRaidSpamForm.Import.value="Import raids";
					document.FPXRaidSpamForm.Import.enable();
					
					console.log("[SRDotDX]::{FPX}:: Import ENDED...");
				}catch(error)
				{
					console.log("[SRDotDX]::{FPX}::ERROR:: "+error);
				}
			...
			load: function () {
				if (typeof holodeck == 'object' && typeof holodeck._tabs == 'object' && typeof holodeck._tabs.addTab == 'function') {
					...
					SRDotDX.gui.cHTML('style').set({type: "text/css",id: 'SRDotDX_markMyRaidsVisitedClass'}).text('.SRDotDX_markMyRaidsVisited{display: '+(SRDotDX.config.markMyRaidsVisited == true?'none !important':'block')+'}').attach('to',document.head);
					...
					SRDotDX.gui.cHTML('style').set({type: "text/css"}).text(" \
						...
						#kong_game_ui div#lots_tab_pane ul li.tab.active div.tab_pane #help_list {overflow-y: scroll; font-family: Verdana, Arial, sans-serif; font-size: 11px;height:100%;} \
						...
						#kong_game_ui div#lots_tab_pane ul li.tab.active div.tab_pane #raid_list .raid_list_item .raid_list_item_head .FPXlink {display:block;width:20px;height:10px; padding-right: 5px;float: left;} \
					...
					var pane = SRDotDX.gui.cHTML('div').set({id: 'lots_tab_pane'}).html(' \
						...
						<ul id="SRDotDX_tabpane_tabs"> \
							...
							<div class="tab_head">Options</div> \
									...
									\
									<hr> \
									<input type="checkbox" id="FPX_options_displayLinkImg"> Display raid link beside Raid Name (requires refresh) \
									(<a href="#" onclick="return false;" onmouseout="FPX.tooltip.hide();" onmouseover="FPX.tooltip.show(\'Show a raid link next to the boss name inside the raid list.<br><strong>This may cause script to become excessively laggy as the list becomes larger.</strong>\');">?</a>)<br><hr> \
									\
									...
									<input type="checkbox" id="SRDotDX_options_hideVisitedRaidsInRaidList"> Hide (in the raid list) raids I\'ve already visited <br> \
									<input type="checkbox" id="SRDotDX_options_markMyRaidsVisited"> Mark raids that I post as visited <br><br><br> \
									/me text format.<br> \
									<textarea id="SRDotDX_options_emoteFormat"></textarea><br> \
							...
							<li class="tab"> \
								...
								<div id="FPXRaidSpamDiv"> \
										<FORM name="FPXRaidSpamForm" onSubmit="SRDotDX.gui.FPXspamRaids();return false"> \
											<input name="Submit"  type="submit" tabIndex="-1" value="Post Links to Chat" onClick="SRDotDX.gui.FPXspamRaids();return false;"/> <input name="Import" type="submit" tabIndex="-1" value="Import raids" title="Import raids from the share textarea." onClick="SRDotDX.gui.ImportRaids();return false;"/><br> \
							...
							<li class="tab"> \
							 ...
							 <li class="tab"> \
								...
								<div class="tab_pane"> \
									<div id="help_list" tabIndex="-1"> \
									<b>Command</b>: <code>/clear</code><br /> \
									Clears the currently active chat window<br /><i>Example: /clear</i> \
									\
									<br /><br /><b>Command</b>: <code>/version</code><br /> \
									Displays current script version information<br /><i>Example: /version</i> \
									\
									<br /><br /><b>Command</b>: <code>/update</code><br /> \
									Download and Installs latest version of script<br /><i>Example: /update</i> \
									\
									<br /><br /><b>Command</b>: <code>/reload</code><br /> \
									Reloads the game without refreshing the page<br /><i>Example: /reload</i> \
									\
									<br /><br /><b>Command</b>: <code>/lr [raidurl]</code><br /><b>Command</b>: <code>/loadraid [raidurl]</code><br /> \
									Loads the specified raid url without refreshing the page<br /><i>Example: /lr <wbr><a href="http://www.kongregate.com/games/5thPlanetGames/dawn-of-the-dragons?kv_action_type=raidhelp&amp;kv_difficulty=4&amp;kv_hash=rY98Mv05p5&amp;kv_raid_boss=batman&amp;kv_raid_id=5839">http://www.kongregate.com/games/<wbr>5thPlanetGames/...</a></i> \
									\
									<br /><br /><b>Command</b>: <code>/toggle</code><br /> \
									Displays if seen and visited raids are shown/hidden<br /><i>Example: /toggle</i> \
									\
									<br /><br /><b>Command</b>: <code>/toggle [seen|visited]</code><br /> \
									Displays if seen/visited raids are shown/hidden<br /><i>Example: /toggle seen</i> \
									\
									<br /><br /><b>Command</b>: <code>/toggle [seen|visited] [show|hide]</code><br /> \
									Sets seen/visited raids to shown or hidden<br /><i>Example: /toggle seen show</i> \
									\
									<br /><br /><b>Command</b>: <code>/raid [raidname]</code><br /> \
									Displays information about the specified raid<br /><code>[raidname]</code> can be a partial name of a raid. Information will be displayed for each matching raid<br /><i>Example: /raid tele</i> \
									\
									<br /><br /><b>Command</b>: <code>/raid [raidname] [difficulty]</code><br /> \
									...
									\
									<br /><br /><b>Command</b>: <code>/raidformat [on|off]</code><br /> \
									Toggles raid link formatting on or off<br /><i>Example: <code>/raidformat on</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/spit [raid id]</code><br /> \
									Generates a message showing your disdain for the cruddy raid someone has posted.<br /><i>Example:<code>/spit 608926</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/raidformat [format]</code><br /> \
									Changes your current raid format to the one you specify<br /><i>Example: <code>/raidformat &lt;visited:(v) &gt;&lt;name&gt; - &lt;diff&gt; &lt;fs&gt;/&lt;os&gt;</code></i><br /> \
									\
									<br /><br /> \
									<b>New commands</b> \
									<br /><br /><b>Command</b>: <code>/spitlast</code><br /> \
									Generates a message showing your disdain for the last cruddy raid someone has posted.<br /><i>Example:<code>/spitlast</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/hug [raid id]</code><br /> \
									Generates a message showing your love for the wonderful raid someone has posted.<br /><i>Example:<code>/hug 608926</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/huglast</code><br /> \
									Generates a message showing your love for the last wonderful raid someone has posted.<br /><i>Example:<code>/huglast</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/beg [raid name] [raid difficulty]</code><br /> \
									Generates a message showing your begging for the specific raid. <code>[raidname]</code> can be a partial name of a raid, if multiple raids matches only the first raid will be begged for.<br /><i>Example:<code>/beg erebus the black 4</code></i> \
									\
									<br /><br /><b>Autoracle features (&copy;&reg; Oracle_Hunter)</b> \
									<br /><br /><b>Command</b>: <code>/n00b</code> or <code>/noob</code><br /> \
									Posts advice for beginning players. This posts 10 messages to the current Kong chat.<br /><i>Example:<code>/n00b</code></i> \
									\
									<br /><br /><b>Command</b>: <code>/l0w</code> or <code>/low</code><br /> \
									Posts advice for low level players. This posts 11 messages to the current Kong chat.<br /><i>Example:<code>/l0w</code></i> \
									\
									<table><tr><th> \
									</div> \
								</div> \
...
This is probably only useful if you have a clipboard listener like my 'DotD raid link spammer' which may (or may not) be linked in the forums.";
					...
					// Listen to paste events and add a , just before the second link is pasted
					FPXimpSpam.ele().addEventListener("paste",function() {
						if (this.value == FPXSpamText) {
							this.value = "";
						} else if (this.value != "") {
					...
					var optsEmoteFormat = SRDotDX.gui.cHTML('#SRDotDX_options_emoteFormat');
					...
					optsEmoteFormat.ele().value = SRDotDX.config.emoteTextFormat;
					optsEmoteFormat.ele().style.width = e[0].offsetWidth - 12 + "px";
					...
					if (SRDotDX.config.markMyRaidsVisited) { optsMarkMyRaidsVisited.ele().checked = 'checked'}
					...
					optsEmoteFormat.ele().addEventListener("blur",function() {
						if (this.value != "") {
							SRDotDX.config.emoteTextFormat = this.value;
						}
						else {
							this.value = SRDotDX.config.emoteTextFormat;
						}
					});
					...
					optsMarkMyRaidsVisited.ele().addEventListener("click",function() {
						SRDotDX.config.markMyRaidsVisited = this.checked;
						SRDotDX.gui.toggleCSS({id: "SRDotDX_markMyRaidsVisitedClass", cls:".SRDotDX_markMyRaidsVisited{display: "+(this.checked == true?'none !important':'block')+"}"})
					},true);
					// Filtering tab					
					for (var i in SRDotDX.raids) {
						if (SRDotDX.raids.hasOwnProperty(i)) {
		...
		load: function (fails) {
			if (typeof holodeck == 'object' && typeof ChatDialogue == 'function' && typeof activateGame == 'function' && typeof document.getElementById('kong_game_ui') != 'null') {
				...
				holodeck.addChatCommand("spit",function(deck,text) {
					...
					if (p = /^\/spit ([0-9]*).*$/i.exec(text)) {
						...
						if (typeof r == 'object') {
							...
							SRDotDX.gui.FPXdoWork(text);
						}
					}
					return false;

				});
				holodeck.addChatCommand("spitlast",function(deck,text) {
					var p;
					if (p = /^\/spitlast*$/i.exec(text)) {
						var r = SRDotDX.config.getRaid(SRDotDX.lastRaidId);
						var adjectives = ['stupid','worthless','terrible','awful','despicable','horrible','disgusting','ridiculous','horrendous','abominable','hideous'];
						var spitSounds = ['*ptoo*','*ptooie*', '*ptui*', '*pthu*'];
						var random1=Math.floor(Math.random()*11);
						var random2=Math.floor(Math.random()*4);
				...
				holodeck.addChatCommand("hug",function(deck,text) {
					var p;
					if (p = /^\/hug ([0-9]*).*$/i.exec(text)) {
						var r = SRDotDX.config.getRaid(p[1]);
						var adjectives = ['adorable','beautiful','elegant','fancy','glamorous','handsome','sparkling','jolly','nice','delightful','delicious'];
						var spitSounds = ['*squeals*','*purrs*', '*wheeks*', '*chirps*'];
						var random1=Math.floor(Math.random()*11);
						var random2=Math.floor(Math.random()*4);
						if (typeof r == 'object') {
							var text = r.lastUser + ", I hug your " + adjectives[random1] + " " + SRDotDX.raids[r.boss].name + "! " + spitSounds[random2] + "!";
							SRDotDX.gui.FPXdoWork(text);
						}
					}
					return false;
				...
				holodeck.addChatCommand("huglast",function(deck,text) {
					var p;
					if (p = /^\/huglast$/i.exec(text)) {
						var r = SRDotDX.config.getRaid(SRDotDX.lastRaidId);
						var adjectives = ['adorable','beautiful','elegant','fancy','glamorous','handsome','sparkling','jolly','nice','delightful','delicious'];
						var spitSounds = ['*squeals*','*purrs*', '*wheeks*', '*chirps*'];
						var random1=Math.floor(Math.random()*11);
				...
				holodeck.addChatCommand("beg", function(deck,text){
					var p;
					if (p = /^\/beg (.*?)(?: ([1-4]))?$/i.exec(text)) {
						var msg = "";
						...
						for (var i in SRDotDX.raids) {
							if (SRDotDX.raids.hasOwnProperty(i)) {
								...
								if (raid.name.toLowerCase().indexOf(find) > -1) {
									var difficulty = "";
									if (start > 0) {
										difficulty = ["Normal ","Hard ","Legendary ","Nightmare ","Insane ","Hell "][start];
									}
								
									msg = "Can someone please give me some " + difficulty + raid.name + " raids?" ;
									SRDotDX.gui.FPXdoWork(msg);
									break;
								} else {
									//SRDotDX.echo('<b>/beg</b>: Raid ' + find + ' ' + start + ' not found');
								}
							}
						}
					}
					else {
						SRDotDX.echo('<b>/beg</b>: Invalid parameters specified (<a href="#" onclick="SRDotDX.gui.help(\'beg\')">help</a>)');
					}
					return false;
				});	
				holodeck.addChatCommand("me", function(deck,text){
					var p;
					if (p = /^\/me (.*?)$/i.exec(text)) {
						var txt = SRDotDX.config.emoteFormat;
						txt = txt.replace(/<currentUser>/gi,SRDotDX.currentUser);
						txt = txt.replace(/<text>/gi, p[1]);
						SRDotDX.gui.FPXdoWork(txt);
						return false;
					}
				});
				holodeck.addChatCommand("n00b", function(deck,text){
					var p;
					if ( (p = /^\/n00b$/i.exec(text) ) || (p = /^\/noob$/i.exec(text))) {
						
						var oracleAdvice = [
							'(1) Use Chrome and download the Raid Script',
							'The script can be found at http://userscripts.org/scripts/show/139965',
							'(2) Hit every Z9 Dragon Raid and Z8 Raid you see once for 1 stam', 
							'Z9 Dragons = Mesyra, Phaedra, Nimrod, Tenebra and Valanazes', 
							'Z8 Raids = Grimsley, Rift, Sisters, Mardachus',
							'(3) Complete every sector on every Zone on N ONLY',
							'(4) Every level add +1 EN and +2 Stam. If you get any extra SP in a level, put it in Stam',
							'Do this until you get LSI 7',
							'(5) Once you get to 100 Base Stamina you can start looking for Legions and Generals, and do a little farming.',
							'©® Oracle_Hunter'
							];
						
						var timer=500,ttw=1600,i=1;
						for (var i = 0; i < oracleAdvice.length; i++) {
							(function(param1) {return setTimeout(function() {SRDotDX.gui.FPXdoWork(param1);},timer); })(oracleAdvice[i]);
							timer += ttw;
						}
					}
					else {
				...
				holodeck.addChatCommand("l0w", function(deck,text){
					var p;
					if ( (p = /^\/l0w$/i.exec(text) ) || (p = /^\/low$/i.exec(text))) {
						
						var oracleAdvice = [
							'(1) Join a Guild and use your Honor to 1/2 FS as many NM raids as you can.',
							'Spend remaining Honor on Celeanos and Grobulars for Flags.',
							'(2) If you don\'t have a PC Legion, get either the Bucket Brigade 1 ("BB1") Legion or Boar Snout 2 ("BS2) Legion ASAP', 
							'BB1 = Common drop from Rhalmarius, a Guild Raid', 
							'BS2 = Epic drop from Z9 Dragons; do 1/2 FS on every one you see',
							'(3) Start tagging any Z7 or Z3 Raids you see.',
							'Z7 Raids = Erakka-Sak, Guilbert, Bellarius, Wexxa',
							'Z3 Raids = Scylla, Gravlock, Erebus, Tainted',
							'(4) Any N Erebus you have, try to do 1/2 FS damage. If you can do that easily, go to FS.',
							'Once you can easily FS N Erebuses, start 1/2 FSing N Bellariuses and then FSing them.',
							'©® Oracle_Hunter'
							];
				...
				holodeck.addChatCommand("totalstats", function(deck,text){
					var p;
					if ( p = /^\/totalstats$/i.exec(text) ) {
						var dictGlobal = { visited:0,visited_1:0,visited_2:0,visited_3:0,visited_4:0,
										   total:0,total_1:0,total_2:0,total_3:0,total_4:0};
						
						for (var id in SRDotDX.config.raidList) {
							if (SRDotDX.config.raidList.hasOwnProperty(id)) {
								var raid = SRDotDX.config.raidList[id];
								if (raid && raid.diff)
								{
									if (raid.visited) {
										SRDotDX.upDictionaryCount(dictGlobal, "visited");
										SRDotDX.upDictionaryCount(dictGlobal, "visited_" + raid.diff);
									}
									SRDotDX.upDictionaryCount(dictGlobal, "total");
									SRDotDX.upDictionaryCount(dictGlobal, "total_" + raid.diff);
								}
							}
						}
						
						var msg = "Total raids visited " + dictGlobal["visited"] + " out of " + dictGlobal["total"] + " raids.<br>";
						msg +=	"Total N raids visited " + dictGlobal["visited_1"] + " out of " + dictGlobal["total_1"] + " raids.<br>";
						msg +=	"Total H raids visited " + dictGlobal["visited_2"] + " out of " + dictGlobal["total_2"] + " raids.<br>";
						msg +=	"Total L raids visited " + dictGlobal["visited_3"] + " out of " + dictGlobal["total_3"] + " raids.<br>";
						msg +=	"Total NM raids visited " + dictGlobal["visited_4"] + " out of " + dictGlobal["total_4"] + " raids.<br>";
				...
				holodeck.addChatCommand("stats", function(deck,text){
					var p;
					if ( p = /^\/stats (.*?)$/i.exec(text) ) {
						var msg = "";
						var start = (!isNaN(p[2]))?p[2]-1:0;
						var find = p[1].toLowerCase();
						for (var i in SRDotDX.raids) {
							if (SRDotDX.raids.hasOwnProperty(i)) {
								var raidDef = SRDotDX.raids[i];
								if (raidDef.name.toLowerCase().indexOf(find) > -1) {
									...
									for (var id in SRDotDX.config.raidList) {
										if (SRDotDX.config.raidList.hasOwnProperty(id)) {
											var raid = SRDotDX.config.raidList[id];
											if (raid && raid.diff && raidDef.id == raid.boss)
											{
												if (raid.visited) {
													SRDotDX.upDictionaryCount(dictGlobal, "visited");
													SRDotDX.upDictionaryCount(dictGlobal, "visited_" + raid.diff);
									...
									if (msg != "") msg += "<br>"
									msg += "Total " + raidDef.name + " raids visited " + dictGlobal["visited"] + " out of " + dictGlobal["total"] + " raids.<br>";
									msg +=	"Total N " + raidDef.name + " raids visited " + dictGlobal["visited_1"] + " out of " + dictGlobal["total_1"] + " raids.<br>";
									msg +=	"Total H " + raidDef.name + " raids visited " + dictGlobal["visited_2"] + " out of " + dictGlobal["total_2"] + " raids.<br>";
									msg +=	"Total L " + raidDef.name + " raids visited " + dictGlobal["visited_3"] + " out of " + dictGlobal["total_3"] + " raids.<br>";
									msg +=	"Total NM " + raidDef.name + " raids visited " + dictGlobal["visited_4"] + " out of " + dictGlobal["total_4"] + " raids.<br>";
								}
							}
						}
						SRDotDX.echo(msg);
				...
				holodeck.addChatCommand("version", function(deck,text) {
					...
					d += '<a href="http://userscripts.org/scripts/show/139965" target="_blank">Go to script page</a>';
					SRDotDX.echo(d);
					return false;
				});
				holodeck.addChatCommand("update", function(deck,text) {
					window.open("http://userscripts.org/scripts/source/139965.user.js");
				...
				if (typeof (i = SRDotDX.getRaidDetails(document.location.href)) == 'object'){
					if (SRDotDX.config.getRaid(i.id)) {
						...
						SRDotDX.echo("addRaid without user(" + i.hash + "," + i.id + "," + i.boss + "," + i.diff + ",true,true,false,'')");
		...
		upDictionaryCount: function (dict, key) {
			var count = 0;
			if (dict[key] != null) {
				count = dict[key];
			}
