// ==UserScript==
// @name           Mejora del escondite
// @namespace      Original por: ikariamLibrary
// @description    Agrega el calculo de los barcos necesarios para robar todos los recursos
// @include        http://*.ikariam.*/*
// @exclude        http://board.ikariam.*/*
// ==/UserScript==

// Version 1.02 (29/03/2009)
...
function showTotal()
{
		...
		td1 = document.createElement("td");
		td1.setAttribute("class","job");
		td1.innerHTML = 'Nivel del Deposito:';

		td2 = document.createElement("td");
		 var wareDiv = <>
								<input type="text" style="text-align: right;" size="2" maxlength="2" id="warehouseLevelId" value="0"/>  (0-31)
							</>;
		...
		td1 = document.createElement("td");
		td1.setAttribute("class","job");
		td1.innerHTML = 'Barcos Necesarios:';
		...
		td2.innerHTML = <>
								<a
									href={backURL}
									title="Volver al Escondite"> <img
									src="skin/buildings/y100/safehouse.gif"
									width="160" height="100" /> <br/> <span class="textLabel">&lt;&lt; Volver al Escondite (La tabla del escondite)</span>
								</a>
								</>;
...
function validateInput()
{
	//validate input
		...
		{
			if(parseInt(level.value) > 31 || parseInt(level.value) < 0)
			{
...
function calculateShip()
{
		...
		//find safe amt
			var level = document.getElementById('warehouseLevelId');
			if (typeof(data['warehouse'][level.value]) != "undefined") {

					var b = data['warehouse'][level.value].b;
					var w = data['warehouse'][level.value].w;
					var m = data['warehouse'][level.value].m;
					var c = data['warehouse'][level.value].c;
					var s = data['warehouse'][level.value].s;
