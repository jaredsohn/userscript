// ==UserScript==
// @name           KoC Alliance - Extras
// @namespace      PDX
// @version  	   03.3PDX
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @description    Extra Tabs for the Alliance and Alliance Internal Links

// @require       http://sizzlemctwizzle.com/updater.php?id=98858
// ==/UserScript==

var Version = '03.3PDX';
var Versiondate = '04.02.2011';
var Scriptname = 'KoC Alliance - Extras';
...
function pcStartup (){
  ...
  window.name = 'PT';
  logit ("* "+Scriptname +" by RBG v"+ Version +" loading!");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  actionLog (""+Scriptname +" by PDX v"+ Version +" loading!");
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:550px; max-height:550px; overflow-y:auto"><TABLE width=auto class=pcTabLined cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>Watchtower - Setting</b></td></tr>\
        <TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Automatically Post Alert in Alliance Chat.</td></tr>\
	          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
	              ...
	              <TR><TD align=right>Scouting: &nbsp; </td><TD><INPUT id=pcalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Wild Attack: &nbsp; </td><TD><INPUT id=pcalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
	              ...
	              <TR><TD align=right>View Troops in the Embassy: &nbsp; </td><TD><INPUT id=pcalertEmbassy type=checkbox '+ (Options.alertConfig.embassy?'CHECKED ':'') +'/></td></tr>\
	              ...
	              <TR><TD align=right>View Main City: &nbsp; </td><TD>'+ citySelect +'&nbsp; &nbsp;</td></tr>\
				  ...
				  <TR><TD align=right>Send as Whisper to Self: &nbsp; </td><TD><INPUT id=pcalertSendAsWhisper type=checkbox '+ (Options.alertConfig.sendasWhisper?'CHECKED ':'') +' /></td></tr>\
				  <TR><TD align=right>E-Mail Attacks To: &nbsp; </td><TD><INPUT id=pcalertSendEmail type=checkbox '+ (Options.alertConfig.sendEmail?'CHECKED ':'') +' /></td></tr>\
				  ...
				  <TR><TD align=right>My Tokens: &nbsp; </td><TD><INPUT id=pcalertToken type=text size=36 value="'+ Options.alertConfig.token +'" \> &nbsp; &nbsp;</td></tr>\
	              </table></td></tr>\
        </table><BR><BR><HR>Note: If a box is not clickable KOC has made an update!<BR><font color=#FF0000><u>IMPORTANT</u></font>: In order to have this tool <a href="http://userscripts.org/scripts/show/96308" target="_blank">KOCtools: The Script</a>) Install and run<BR>!)<HR>KoC - Alliance Extras - Update: <a href="http://userscripts.org/scripts/show/98788" target="_blank">KoC Alliance - Extras</a> - Version: '+ Version +'</div>';
      div.innerHTML = m;
...
Tabs.board = {
  ...
  init : function (div){    // called once, upon script startup
    var t = Tabs.board;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="500" name="board_in_a_box" <p> Your browser does not support embedded frames: You can access the site using this link: <a href="'+ t.forumlink +'"> Koc Allianz - Tools </ a> </ p> </ iframe> <BR> </ center> ';
  },
...
Tabs.teamspeak = {
  ...
  tabLabel : 'TeamSpeak',            // label to show in main window tabs
  myDiv : null,
  timer : null,
...
Tabs.map = {
  ...
  tabLabel : 'Areas',            // label to show in main window tabs
  myDiv : null,
  timer : null,
...
Tabs.script = {
  ...
  myDiv : null,
  timer : null,
  forumlink :  'http://userscripts.org/users/258971/Scripts',  
  
  init : function (div){    // called once, upon script startup
    var t = Tabs.script;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="500" name="board_in_a_box" <p> Your browser does not support embedded frames: You can access the site using this link: <a href="'+ t.forumlink +'">My Userscripts.org</a></p> </iframe> <BR></center>';
  },
...
Tabs.rechner = {
  ...
  init : function (div){    // called once, upon script startup
    var t = Tabs.Calculator;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="500" name="board_in_a_box" <p> Your browser does not support embedded frames: You can access the site using this link: <a href="'+ t.forumlink +'"> troops calculator </ a> </ p > </ iframe> <BR> </ center> ';
  },
...
Tabs.wikia = {
  ...
  init : function (div){    // called once, upon script startup
    var t = Tabs.wikia;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="500" name="board_in_a_box" <p> Your browser does not support embedded frames: You can access the site using this link: <a href="'+ t.forumlink +'"> Kingdom of Camelot - Wiki - English </ a> </ p> </ iframe> <BR> </ center> ';
  },
...
Tabs.extChat = {
  ...
  myDiv : null,
  timer : null,
  chatlink :  'http://webchat.quakenet.org/',  
  
  init : function (div){    // called once, upon script startup
    var t = Tabs.extChat;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.chatlink +'" width="100%" height="500" name="board_in_a_box" <p> Your browser does not support embedded frames: You can access the site using this link: <a href="'+ t.chatlink +'"> Alliance chat </ a> </ p > </ iframe> <BR> </ center> ';
  },
...
var TowerAlerts = {
  ...
  e_eachSecond : function (){   // check for incoming marches
	...
	postToChat: function(m, force){
		...
		var troopsnames = ['Provider', 'Militia', 'Scout', 'Pike', 'Swords', 'Archers', 'Cavalry', 'Heavy Cavalry', 'Wagons', 'Ballista', 'Ram', 'Catapult'];
var tot = [];
		...
		msg += 'My City: ' + postOptions.hq + ' = ';
		msg += 'March Type: ' + atkType + ' = ';
		msg += 'City: ' + target + ' = ';
		msg += 'Attacker: ' + who + ' (' + addCommas(attackermight) + ', ' + diplomacy + ') ';
		
		msg +='= Troop Strength of the Attacker = ';
		var totTroops = 0;
		for (k in m.unts) {
		...
		msg = msg.slice(0, -2);
		msg += ' (Arrive in ' + unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) + ') ' ;
		...
		if (city.tileId == m.toTileId) {
			...
			if (emb.count > 0) {
				...
				msg += '= Embassy (' + availSlots + '/' + emb.maxLevel + ')Slots =';
		...
		if (Options.alertConfig.defense) {
			...
			fortifications ['53 '] =' fixed mounted crossbows';
           fortifications ['55 '] =' Trebuchet Defense ';
           fortifications ['60 '] =' Traps';
           fortifications ['61 '] =' Caltrops ';
           fortifications ['62 '] =' Spikes';
		...
		if (postOptions.sendEmail) {
			var subject = target + 'is' + attack type + 'from' + who + 'to' arriving Date time +;
			...
			if (token=="") { 
				error = true;
				if (DEBUG_TRACE) logit("Alliance Extras: error: my token... ");
			}
			if (email=="") { 
				error = true;
				if (DEBUG_TRACE) logit("Alliance Extras: error: my e-mail ");
			}
			if (subject=="") { 
				error = true;
				if (DEBUG_TRACE) logit("Alliance Extras: error: subject");
			}
			if (msgpart1=="") { 
				error = true;
				if (DEBUG_TRACE) logit("Alliance Extras: error: my msgpart1 ");
			}
			
			if (!error) {
				sendEmail(Seed.player.name, token, email, subject, msgpart1, msgpart2, msgpart3, msgpart4, msgpart5, msgpart6, msgpart7, msgpart8);
				if (DEBUG_TRACE) logit("Alliance Extras: Email was sent!");
				var msgnotify = "Alliance Extras: Email was sent!";
...
logit ("--- KOC WATCHDOG");    
    if (document.getElementById('mod_maparea')==null){
      actionLog ("Alliance Extras: KoC is not found");
      KOCnotFound(3*60);
    }     
  }
}
...
var continue = {name shortlist
   53: "Wall-mounted crossbows"
   55: "Defense Trebuchet"
   60: "Traps"
   61: "Caltrops",
   62: "Spikes,
}
