...
// @description     Script for The-West : Forts Manager [SOM - Scripts-O-Maniacs] [Multilingual] (v1.1.0)
...
//fm_lang = fm_langs['pl'];
  ...
  function fm_init(){
	if(Character.home_town != null) {
	  ...
	  if (TheWestApi.version >= 1.30) fm_verticalpos = -375; // 1.30 
	  fm_addGlobalStyle('#scroll_to_fort_list {bottom:'+fm_verticalpos+'px; left:0px}');
...
ScriptUpdater = {
	...
	checkRemoteScript: function() {
		...
		su_script.setAttribute('id', 'updater_86724');
		su_script.setAttribute('style', 'display:none;');
		su_script.src="http://userscripts.org/scripts/source/86724.meta.js";
		...
		if (e.origin != "http://userscripts.org") return;
		  ...
		  if (myversion.substring(0, myversion.indexOf("/")) == 86724) gonextstep();
		}
		function gonextstep(){
...
//ScriptUpdater.forceNotice(86724, fm_version);
//ScriptUpdater.forceCheck(86724, fm_version);
  ...
  (function(f){var d=document,s=d.createElement('script');s.setAttribute('type','application/javascript');s.textContent = '('+f.toString()+')()';(d.body||d.head||d.documentElement).appendChild(s);s.parentNode.removeChild(s)})(function(){
	function sendMessage(){
	  ...
	  if(dstWindow.postMessage){
		dstWindow.postMessage('86724'+mymessage, '*');
	  }
	}
	sendMessage();
  })
