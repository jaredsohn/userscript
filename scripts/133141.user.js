// ==UserScript==============
// @name           iPokemonscriptZ® Dutch Packed
// @namespace      iTycho And Imarco
...
function init() {
	...
	unsafeWindow.disableSubmitButton = function(form) {
		for (i = 0; i < form.elements.length; i++) {
			if (form.elements[i].type == 'submit') {
				form.elements[i].value = 'Wacht... Of Klik - iPokemonscriptZ® ';
			}
		}
		return true;
	} 
	...
	createToggler(container, 'Zoek andere pokemon', 'findRare');
	createToggler(container, 'Zoek legends', 'findLegendary');
