// ==UserScript==
// @name	Travian Mod Kit T3.5 working : version 4
// @namespace	Travian
...
///step 1 shof resources = rewritten
...
//SS1 -> dorpen url (op basis van vlist?)? case sensitive
//SS2 -> logout url?
//X1 -> tmp to get User -> username/ID  //for now in url format? -> http://s3.travian.nl/spieler.php?uid=xxxx
//X2 -> active village ID // you can find wich village is active with the dot -> sel
// SS3 voor het bekomen van X3 (villageID), Village links and village names.
// SS4 voor het bekomen van X4 en X -> X and Y coordinates //Deleted? -> direct
		...
		if (GM_getValue("InActive")){
			...
			if (!(/travian/.exec(document.domain))) {
				...
				var SS1 = TM_searchDOM('//table[@id="vlist"]'); 					//dorpenlijst??
				var SS2 = TM_searchDOM('//a[@href="logout.php"]');					//alert('ok');
															//alert(SS1.snapshotItem(0));									
															//alert(SS2.snapshotItem(0));	
			
				if (SS1.snapshotItem(0) && SS2.snapshotItem(0)){
			
					var X1 = TM_searchDOM('//p/a[3]');
					var X1b = X1.snapshotItem(0).toString();			
					var X1PlaceEqual = X1b.indexOf("=");

					User  = X1b.substring(X1PlaceEqual+1,X1b.length); 				//alert('Active User ID'+ User);

					//var X2 = /newdid=(\d+)/.exec(SS1.snapshotItem(0).getAttribute("href"));
					//var X2 = TM_searchDOM('following-sibling:://tr/td[@class="dot hl"]/a[1]');
					var X2 = TM_searchDOM("//td[@class='dot hl']/parent::tr/td[@class='link']/a[1]");//alert(X2.snapshotItem(0));
					var X2b = X2.snapshotItem(0).toString();			
					var X2PlaceEqual = X2b.indexOf("=");						
//alert('ok');
					ActiveVillage = X2b.substring(X2PlaceEqual+1,X2b.length); 			//alert('ok');					
															
//alert('Active Village ID'+ ActiveVillage );
					var tmpLocation = ActiveVillage.indexOf("&");	
					if (tmpLocation>2) {								//alert('cut village number voor:' + ActiveVillage);
						ActiveVillage = ActiveVillage.substring(0,tmpLocation);			//alert('cut village number na  :' + ActiveVillage); 
					}										//alert('ok');	
...
//alert(ActiveVillage);
					...
					var SS3 = TM_searchDOM('//table[@id="vlist"]/tbody/tr/td[@class="link"]/a');	
//alert('Villagecount= ' + SS3.snapshotLength);   				
					VillageNumber = SS3.snapshotLength; 						
//alert('VillageNumbercount'+VillageNumber );
			
					var tmpLocation;

					for ( var i=0 ; i < SS3.snapshotLength; i++ ) {
						var X3 = SS3.snapshotItem(i).getAttribute("href");			//alert('villageID' + X3);   				
						var X3b = X3.toString();			
						var X3PlaceEqual = X3b.indexOf("=");
															//alert('ok');	
						var X4 = TM_searchDOM('//table[@id="vlist"]/tbody/tr/td[@class="aligned_coords"]/div[@class="cox"]');
						//var X4 = TM_searchDOM('//table[@class="vlist"]/tbody/tr/td[@class="x"]');
															
//alert('X4 = '+X4.snapshotItem(i).innerHTML);
						//var X5 = TM_searchDOM('//table[@class="vlist"]/tbody/tr/td[@class="y"]');
						var X5 = TM_searchDOM('//table[@id="vlist"]/tbody/tr/td[@class="aligned_coords"]/div[@class="coy"]');
															//alert('X5' + X5);
						VillageIDs[i] = X3b.substring(X3PlaceEqual,X3b.length);					
															
//alert('villageID' + X3); 
 
						VillageIDs[i] = VillageIDs[i].substring(1,VillageIDs[i].length); 
						var tmpLocation = VillageIDs[i].indexOf("&");	
						if (tmpLocation>2) {
							VillageIDs[i] = VillageIDs[i].substring(0,tmpLocation); 
						}									
//alert('Active Village ID'+ ActiveVillage );
															
//alert('villageID i = ' + VillageIDs[i]);

						VillageLinks[i] = SS3.snapshotItem(i).getAttribute("href");
						VillageNames[i] = SS3.snapshotItem(i).innerHTML;
						VillageCoords[i] = X4.snapshotItem(i).innerHTML + '|' +  X5.snapshotItem(i).innerHTML;
					}
...
//alert('func ok -> gif???')
	TM_GetResourceInfo();
//alert('func ok -> TM_GetResourceInfo')
	TM_ShowTotalProduction();
//alert('func ok -> TM_ShowTotalProduction()')
	TM_GetBuildInfo(); //returns cookie :39/g16/20/Marktplaats,19/gx.gif/18/Pakhuis,20/gx.gif/15/Graansilo,21/gx.gif/5/Barakken,22/gx.gif/7/Handelskantoor,23/gx.gif/5/Ambassade,24/gx.gif/5/Steenbakkerij,25/gx.gif/20/Hoofdgebouw,26/g00/0/NULL,27/gx.gif/10/Heldenhof,28/gx.gif/5/Zaagmolen,29/gx.gif/5/IJzersmederij,30/gx.gif/7/Academie,31/gx.gif/10/Residentie,32/gx.gif/3/Wapensmid,33/gx.gif/3/Uitrustingssmederij,34/gx.gif/5/Korenmolen,35/g00/0/NULL,36/gx.gif/5/Bakkerij,37/gx.gif/10/Stal,38/gx.gif/1/Verzamelplaats,40/g0/8/Stadsmuur 
//alert('nok -> gif???')
...
function TM_ShowTotalProduction(){        // ›› Show total Village resource production, overlayed over left menu bar.
...
//alert('VillageIDs.length' + VillageIDs.length);					
	for (var i = 0; i < VillageIDs.length; i++) {
														
//alert(UniqueID + VillageIDs[i]);	
														
//alert(GM_getValue(UniqueID + VillageIDs[i] + '.Resources'));	
		if (GM_getValue(UniqueID + VillageIDs[i] + '.Resources')){
			ResourceValues1 = GM_getValue(UniqueID + VillageIDs[i] + '.Resources').split(":");	
//alert('ResourceValues1'+ResourceValues1);
				ResourceValues2 = ResourceValues1[0].split("/");				
//alert('ResourceValues2[0]'+ResourceValues2[1]);
...
//alert('ok after resources, before table')
	...
	var HTML =  '<table width="100%" cellpadding="4" cellspacing="0" border="0">'
			...
			//HTML += '<tr><td colspan=2 align="center">UniqueID=<br>' +UniqueID+ '<br>++ Production</td></tr>' +		
			HTML += '<tr><td colspan=2 align="center"><br><br>++ Production</td></tr>' + 
...
//	DIV = addDiv('TMleftmenu','TMleftmenu',HTML,'sleft');
	DIV = addDiv('TMleftmenu','TMleftmenu',HTML,'side_navi');
//alert('ok after table 01')
	//DIV.addEventListener('click', function() {this.parentNode.removeChild(this);}, true);
														
//alert('ok after table')
...
function TM_ShowEvents(){
	...
	var IMG = new Array();
	//var SS1 = TM_searchDOM('//div[@id="sright"]/table[@id="vlist"]/tbody/tr')  // village list
	var SS1 = TM_searchDOM('//div[@id="side_info"]/table[@id="vlist"]/tbody/tr')  // village list

//alert('SS1 '+SS1.snapshotLength);
	...
	//var SS2 = TM_searchDOM('//div[@id="sright"]/table[@id="vlist"]/tbody/tr/td[@class="link"]');	
	var SS2 = TM_searchDOM('//div[@id="side_info"]/table[@id="vlist"]/tbody/tr/td[@class="link"]');	
				
	var Colspan = (SS2.snapshotLength/VillageNumber) + L;if (Prefs[0]=='M'){Colspan--;}
//alert('SS2.snapshotLength '+SS2.snapshotLength);
//alert('VillageNumber '+VillageNumber);
//alert('(SS2.snapshotLength/VillageNumber)'+(SS2.snapshotLength/VillageNumber));
...
//alert('xTR'+xTR);
		...
		//xTD.removeChild(xTD.firstChild);
//alert('ok?'+xTR);
		//xTD.removeChild(xTD.firstChild); // remove the span with the small indicator dot
//alert('ok2?'+xTR);
...
/////////////////////////////
//TODO!!!	
//put here a function to see if needs update!!!!
/////////////////////////////
...
//alert('ok getres')
	...
	var SS1 =""; 										// village stock / village stock size
	var SS2 = TM_searchDOM('//table[@id="production"]//tr/td[@class="num"]'); 		// village production 
...
//alert('in loop'+i);
		SS1 = TM_searchDOM('//div[@id="res"]//tr/td'); 						
//type like wood and clay?? or needs the totals?
													
//alert('SS1 = '+SS1);
													
//alert('SS1.snapshotItem(i) = ' +SS1.snapshotItem(i));
		//production
		var SS2b = SS2.snapshotItem(i).innerHTML;
													
//alert('SS2b = '+SS2b);
		SS2b = parseFloat(SS2b)	
													
//alert('SS2b = '+SS2b);								
		VillageResources[i] =  SS2b + "/" + SS1.snapshotItem((i*2)+1).innerHTML ;
//alert('VillageResources[i]'+VillageResources[i]);
...
//alert(UniqueID + ActiveVillage + '.Resources')
...
function TM_GetBuildInfo(){ 		// ›› Get building info dorf1 and dorf2.
...
//alert('SS1.snapshotItem(0)'+SS1.snapshotItem(0));
	if (SS1.snapshotItem(0)) {
...
//alert('GM_setValue = '+UniqueID + ActiveVillage + '.Dorf1' + 'VillageLayout.join(:)');
	}



	var VillageLayout = new Array();
	var SS1 = TM_searchDOM('//map[@name="map2"]//area')

	if (SS1.snapshotItem(0)) {

//alert('SS1.snapshotItem(0) = '+SS1.snapshotItem(0));

		//oldline -> var SS2 = TM_searchDOM('//img[@class="d1"]/self::* | //img[@class="d1"]/following-sibling::*')	
		var SS2 = TM_searchDOM('//div[@class="d1"]/self::* | //div[@class="d1"]/following-sibling::*')	
//alert('SS2.snapshotItem(0) = '+SS2.snapshotItem(0));
//alert('SS1.snapshotLength = '+SS1.snapshotLength);
...
//alert('BuildingName = '+BuildingName);
//var tmp = SS2.snapshotItem(1);
//alert('tmp  = '+tmp );
				switch(i){
					//case 0:	VillageLayout[i] = '39/g16/' + BuildingLevel[1] + '/' + escape(BuildingName[1]);	alert('breake 0');break;
					//case 21:	VillageLayout[i] = '40/g0/' + BuildingLevel[1] + '/' + escape(BuildingName[1]);	alert('breake 21');break;
					default:	//var BuildingGIF = /(\d+)/.exec(SS2.snapshotItem(i-1).getAttribute("src"));
						// old line VillageLayout[i] = (i+18) + '/g' + BuildingGIF[1] + '/' + BuildingLevel[1] + '/' + escape(BuildingName[1]);}
						VillageLayout[i] = (i+18) + '/g' + 'x.gif' + '/' + BuildingLevel[1] + '/' + escape(BuildingName[1]);}
...
//alert('SETvALUE'+UniqueID + ActiveVillage + '.Dorf2 ->'+VillageLayout);
