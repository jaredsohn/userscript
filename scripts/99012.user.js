// ==UserScript==
// @name           sha512new2
// @namespace      sha512new2
// @description    sha512new2 finish2
// ==/UserScript==
...
if(key == null) {
	...
	var headText = document.getElementById("next").innerHTML;
	headText = headText.substring(headText.indexOf(":")+1,headText.lastIndexOf(":"));
	engine.setInClearText(headText);
	var keyBaru = hex_sha512(key);
	engine.decrypt();
	var decryptedHead = engine.getCryptedText();

	if(keyBaru == decryptedHead)
	{		
window.location.href = destination+"jump.php?username="+usernm+keyBaru+"";
                
	}else{
		alert('Kunci Anda Salah');
		window.location.href = destination+"index.php";
	}
}
