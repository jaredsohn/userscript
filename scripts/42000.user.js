...
// @namespace T3
// @version 3.6.2.5
// @description  (v 3.6.2.5 Travian v3 addons originally by Victor Garcia (aka Croc); - updated by Szabka pl51-7.June 2008; - updated by ms99-05.February.2009
// @source http://userscripts.org/scripts/show/28129
// @identifier http://userscripts.org/scripts/show/28129.user.js
// @exclude http://*.travian.org/*.php*
...
function functionMain(e) {
	...
	function switchLanguage(detectedLanguage) {
		switch (detectedLanguage) {
			case "it":
				// Por IcEye, rosfe y Danielle, Lello, Zippo, Nux, ns65, Acr111
			...
			case "de":
				...
				xLang['SHOWBIGICONMISC']		= "Palast/Resdenz/Akademie/Schatzkammer";
				xLang['PALACE']					= "Palast";
			...
			case "hk":
				...
				xLang['EDIT'] = '修改';
				xLang['SHOWMESOPENLINKS'] = '顯示pop-up IGM連結';
				xLang['NPCOPTIONS'] = 'NPC交易選項';
				xLang['NPCASSISTANT'] = '顯示NPC交易的連結和計算';
				xLang['NEWVILLAGEAV'] = '日期';
				xLang['TIMEUNTIL'] = '需要等待的時間';
				xLang['SHOWMAPTABLE'] = '在"karte.php"顯示 玩家/村莊/綠洲 表單';
				xLang['SHOWREPDELTABLE'] = '在報告頁面顯示 "全刪除"的表單';
				xLang['SHOWIGMLINKFORME'] = '顯示 發IGM給自己的圖示';
				xLang['CENTERMAP'] = '此村莊在地圖置中';
				xLang['SHOWCENTERMAPICON'] = '在村莊旁 顯示 "地圖置中"的圖示';
				xLang['INETGPOPTION'] = '網絡圖像包';
				xLang['ALLOWINETGP'] = '容許使用網絡圖像包';
			...
			case "sk":
				...
				xLang['RECURSO1'] = 'Driev';
				xLang['RECURSO2'] = 'Hliny';
				xLang['RECURSO3'] = 'Železa';
				xLang['RECURSO4'] = 'Obilia';
				xLang['TIEMPO'] = 'Čas';
			...
			case "tr":
				...
				xLang['RECURSO1']		= 'Odun';
				xLang['RECURSO2']		= 'Tuğla';
				xLang['RECURSO3']		= 'Demir';
				xLang['RECURSO4']		= 'Tahıl';
				xLang['TIEMPO']			= 'Süre';
	...
	var academyCost = [
		...
		[3325, 2420, 1360, 605,36,37],
		[4255, 3095, 1740, 775,43,40],
		[5445, 3960, 2230, 990,51,43],
		[6970, 5070, 2850, 1270,62,46],
		[8925, 6490, 3650, 1625,74,50],
		[11425, 8310, 4675, 2075,89,54],
		[14620, 10635, 5980, 2660,106,58],
		[18715, 13610, 7655, 3405,128,62],
		[23955, 17420, 9800, 4355,153,66] 				// Level 20
	];
	...
	//Image for the ally (Thank you, CuPliz13 !)
	...
	//Image for the market big icon (Thank you, CuPliz13 !)
	...
	//Image for the first military big icon (Thank you, CuPliz13 !)
	...
	//Image for the big icon bar - military 2 - blacksmith/armoury/townhall/hero's mansion (Thank you, CuPliz13 !)
	...
	//Image for the big icon - miscellaneous (Thank you, CuPliz13 !)
	...
	//big icons
	var cssBigIconStyle = "";
	cssBigIconStyle += "#n6, #n7, #n8, #n9, #n10, #n11 {width:70px; height:100px; background-repeat:no-repeat;}";
	...
	cssBigIconStyle += '#n11 {background-image: url(' + image["misc"] + ');}';
	...
	function getIdVillageV3(){
		...
		if (a) {
			try {
				...
				//villageID = getIdVillageFromCookie();
				//return villageID;
				return getIdVillageFromCookie();
			}
		} else {
	...
	*/
	...
	function calculateResourceTime(need){
		...
		for (var i = 0; i < 4; i++){
			...
			if (restante > 0) {
				...
				if (tiempo < 0 || capacity[i]-need[i]<0) {
                    maxTime = 'Infinity';
					var aRow = newRow("");
					var aCell = newCell('<img src="' + img("r/" + (i+1) + ".gif") + '" width="14" height="12" border="0" title="' + T('RECURSO' + (i+1)) +'"');
					var bCell = newCell(' ' + restante +' ', [['style', cRTstyle], ['align', docDir[1]], ['id', "timeout" + i]]);
					var cCell = newCell(' ' + T('NEVER') + ' ', [['width', '220'], ['style', cRTstyle], ['align', docDir[1]]]);
				   ...
				   var aCell = newCell('<img src="' + img("r/" + (i+1) + ".gif") + '" width="14" height="12" border="0" title="' + T('RECURSO' + (i+1)) + '"');
				   var bCell = newCell(' ' + restante +' ', [['style', cRTstyle], ['align', docDir[1]], ['id', "timeout" + i]]);
				   var cCell = newCell(' ' + tiempo + ' ', [['width', '60'], ['style', cRTstyle], ['align', docDir[1]], ['id', 'timeout']]);
				   aRow.appendChild(aCell);
				   aRow.appendChild(bCell);
				   aRow.appendChild(cCell);
				   resTable.appendChild(aRow);
		...
		if (maxTime == 'Infinity'){
			var xRow = newRow("");
			var aCell = newCell(T('LISTO'), [['style', cRTstyle], ['colspan' ,"2"]]);
			var bCell = newCell(T('NEVER'), [['style', cRTstyle]]);
			...
			var xRow = newRow("");
			var aCell = newCell(T('LISTO'), [['style', 'color:#000000;font-size:8pt;'], ['colspan', '2'], ['align', 'center']]);
			var bCell = newCell(computeTextTime(aDate), [['style', 'color:#000000;font-size:8pt;'], ['colspan', "1"], ['align', docDir[1]]]);
			xRow.appendChild(aCell);
			xRow.appendChild(bCell);
			resTable.appendChild(xRow);
			boolTb = true;
	...
	function calculateFillTime(){
		...
		for (var i = 0; i < 4; i++){
			...
			var aCell = newCell('(' + productionFormat + ', '+ displayRemainingTime +')', [["style","font-size:9px; color:#707070; text-align:" + docDir[0] + "; padding-" + docDir[0] + ":25px;"], ["colspan","2"], ["valign","top"]]);
	...
	*/
	...
	function getGeneralData(){
		...
		if ((strFirst.indexOf("speed") != -1) && (strLast == "se" || strLast == "ae")) {
			// for swedish and arabic speed servers
	...
	function bigIconsBar(){
		...
		var aPlus = find("//div[@id='ltop5']//a[contains(@href, 'plus.php')]", XPFirst);
		if (!aPlus) {
			var aPlus = find("//div[@id='ltop1']//a[contains(@href, 'plus.php')]", XPFirst);
		}
		if (aPlus) {
			aPlus.href += "?id=3";
		...
		if (boolShowBigIconMisc == "1") {
			// Associate the misc map with the image created (Thank you, CuPliz13 !)
			var miscLink = newLink("<img usemap='#misc' id='n11' src='" + img('a/x.gif') + "'>");
			bigIconBar.appendChild(miscLink);
			intAdditionalIcons += 1;
		}

		if (boolShowBigIconAlliance == "1") {
			//Associate the ally with the image create (Thank you, CuPliz13 !)
		...
		if (boolShowBigIconMarket == "1") {
			...
			iHTML += '<map name="market"><area shape="rect" coords="0,0,70,54" href="build.php?gid=17" title="' + T('ENVIAR') + '"><area shape="rect" coords="0,50,35,100" href="build.php?gid=17&t=1" title="' + T('COMPRAR') + '"><area shape="rect" coords="35,50,70,100" href="build.php?gid=17&t=2" title="' + T('VENDER') + '"></map>';
		}

		if (boolShowBigIconMilitary == "1") {
			// Map for the military big icon
			iHTML += '<map name="militar"><area shape="rect" coords="0,0,35,54" href="build.php?gid=16&j&k" title="' + T('RALLYPOINT') + '"><area shape="rect" coords="35,0,70,50" href="build.php?gid=19" title="' + T('BARRACKS') + '"><area shape="rect" coords="0,50,35,100" href="build.php?gid=20" title="' + T('STABLE') + '"><area shape="rect" coords="35,50,70,100" href="build.php?gid=21" title="' + T('WORKSHOP') + '"></map>';
		}
		
		if (boolShowBigIconMilitary2 == "1") {
			// Map for the military big icon
			iHTML += '<map name="militar2"><area shape="rect" coords="0,0,35,54" href="build.php?gid=24" title="' + T('TOWNHALL') + '"><area shape="rect" coords="35,0,70,50" href="build.php?gid=37" title="' + T('HEROSMANSION') + '"><area shape="rect" coords="0,50,35,100" href="build.php?gid=12" title="' + T('BLACKSMITH') + '"><area shape="rect" coords="35,50,70,100" href="build.php?gid=13" title="' + T('ARMOURY') + '"></map>';
		}
		
		if (boolShowBigIconMisc == "1") {
			// Map for the miscelaneous big icon
			iHTML += '<map name="misc"><area shape="rect" coords="0,0,35,54" href="build.php?gid=26" title="' + T('PALACE') + '"><area shape="rect" coords="35,0,70,50" href="build.php?gid=25" title="' + T('RESIDENCE') + '"><area shape="rect" coords="0,50,35,100" href="build.php?gid=22" title="' + T('ACADEMY') + '"><area shape="rect" coords="35,50,70,100" href="build.php?gid=27" title="' + T('TREASURY') + '"></map>';
		}
	...
	function addReadMesRepInPopup(aNode) {
		...
		if (aNode.parentNode) {
			...
			if (aNode.parentNode.innerHTML.indexOf(imgPrefix) == -1) {
				var aButton = newLink("&nbsp;&nbsp;", [['href', jsVoid], ['style', 'height:0px; width:20px; position:relative; float:' + docDir[1] + '; top:-15px']]);
				...
				//if (location.href.indexOf("?s=3") == -1) {
	...
	function insertAttSendResLinks(aNode, newdid) {
		...
		if (aNodeParent) {
			...
			if (imgIsHere == -1) {
				...
				var srlink = elem('a',"&nbsp;<img src='" + img("r/4.gif") + "' style='margin:3px 0px 1px 3px; display:inline;' height='12' title='" + T('ENVIAR') + "' alt='" + T('ENVIAR') + "' border='0'>");
				...
				var atklink = elem('a',"&nbsp;<img src='" + img("a/" + arrAction[0] + ".gif") + "' id='att_link_" + newdid + "' style='margin:3px 0px 1px 3px; display:inline;' height='12' title='" + arrAction[1] + "' alt='" + arrAction[1] + "' border='0'>");
	...
	function battleReport(aFrom){
		...
		for (var i = 0; i < 4; i++) {
			imgRes[i] = '<img src="' + img("r/" + (i+1) + ".gif") + '" width="18" height="12" border="0" title="' + T('RECURSO' + (i+1)) + '">';
		}
		var stBooty = [0, 0, 0, 0];
		...
		for (var g = 0; g < t.snapshotLength; g++){
			...
			for(var j = 1; j < 11; j++){
				var troopIndex = tTable.rows[1].cells[j].getElementsByTagName('img')[0].src.replace(/.*\/.*\//,'').replace(/\..*/,'');
			...
			if (g == 0) {
				...
				attdefCell.innerHTML = attdefPower[0].toLocaleString() + " " + "<img src='" + img("a/att_all.gif") + "' width='10px' height='10px'>";
			} else {
				//the defense power of the defender (per table)
				attdefCell.setAttribute('style', 'font-size:8pt; color:green');
				attdefCell.innerHTML = attdefPower[1].toLocaleString() + " " + "<img src='" + img("a/def_i.gif") + "' width='10px' height='10px'><br>" + attdefPower[2].toLocaleString() + " " + "<img src='" + img("a/def_c.gif") + "' width='10px' height='10px'>";
			}
		...
		var sTcell1 = newCell(T('STATISTICS'));
		...
		//attack power row
		atkRow = newRow("");
		var atkIconCell = newCell("<img src='" + img("a/att_all.gif") + "'>", [['style', 'border-top:1px solid #C2C2C2;']]);
		atkRow.appendChild(atkIconCell);
		var atkAPower = newCell(tadPower[0][0].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		atkRow.appendChild(atkAPower);
		var atkDPower = newCell(tadPower[1][0].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		atkRow.appendChild(atkDPower);
		sTable.appendChild(atkRow);
		
		//def power rows
		defiRow = newRow("");
		var defiIconCell = newCell("<img src='" + img("a/def_i.gif") + "'>", [['style', 'border-top:1px solid #C2C2C2;']]);
		defiRow.appendChild(defiIconCell);
		var defiAPower = newCell(tadPower[0][1].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		defiRow.appendChild(defiAPower);
		var defiDPower = newCell(tadPower[1][1].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		defiRow.appendChild(defiDPower);
		sTable.appendChild(defiRow);
		
		defcRow = newRow("");
		var defcIconCell = newCell("<img src='" + img("a/def_c.gif") + "'>", [['style', 'border-top:1px solid #C2C2C2;']]);
		defcRow.appendChild(defcIconCell);
		var defcAPower = newCell(tadPower[0][2].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		defcRow.appendChild(defcAPower);
		var defcDPower = newCell(tadPower[1][2].toLocaleString(), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		...
		var rewATotal = newCell(strRewATotal, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
		
		var rewRow1 = newRow("");
		var intDetailRowSpan = 1 + parseInt(boolShowBRStatDetails);
		log(3, "intDetailRowSpan = " + intDetailRowSpan);
		var rewLabelCell = newCell(labelReward, [['style', 'border-top:1px solid #C2C2C2;'], ['rowspan', intDetailRowSpan]]);
		...
		if (boolShowBRStatDetails == '1') {
			...
			var rewADetail = newCell(rewA, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
			rewRow1.appendChild(rewADetail);
		} else {
			rewRow1.appendChild(rewATotal);
		}
		
		var rewDDetail = newCell('-', [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;'], ['rowspan', intDetailRowSpan]]);
		rewRow1.appendChild(rewDDetail);
		sTable.appendChild(rewRow1);
		
		if (boolShowBRStatDetails == '1') {
			var rewRow2 = newRow("");
			var rewATotal = newCell(booty.toLocaleString() + " " + T('TOTAL'), [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
		...
		var lossATotal = newCell(strLossATotal, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
		if (tadPower[0][3] > 0) lossATotal.setAttribute('style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold; color:red');
		
		var strLossDTotal = (tadPower[1][3] + booty).toLocaleString() + (boolShowBRStatDetails == '1' ? " " + T('TOTAL') : '');
		lossDTotal = newCell(strLossDTotal, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
		if (tadPower[1][3] + booty > 0) lossDTotal.setAttribute('style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold; color:red');
		
		var lossRow1 = newRow("");
		var lossLabelCell = newCell(T('LOSS'), [['style', 'border-top:1px solid #C2C2C2;'], ['rowspan', intDetailRowSpan]]);
		...
		if (boolShowBRStatDetails == '1') {
			...
			var lossADetail = newCell(iLossA, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
			if (tadPower[0][3] > 0) lossADetail.setAttribute('style', 'border-top:1px solid #C2C2C2; font-size:8pt; color:red');
			lossRow1.appendChild(lossADetail);
			var lossDDetail = newCell(iLossD, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
			if (tadPower[1][3] + booty > 0) lossDDetail.setAttribute('style', 'border-top:1px solid #C2C2C2; font-size:8pt; color:red;');
		...
		var ccRow = newRow("");
		var ccRowLabelCell = newCell("<img src='" + img("r/5.gif") + "'>", [['style', 'border-top:1px solid #C2C2C2;']]);
		ccRow.appendChild(ccRowLabelCell);
		ccACell = newCell(tadPower[0][10] + " (-" + tadPower[0][8] + ")", [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		ccDCell = newCell(tadPower[1][10] + " (-" + tadPower[1][8] + ")", [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt;']]);
		...
		//hero row
		var heroRow = newRow("");
		var heroLabelCell = newCell("<img src='" + img("u/hero.gif") + "'>", [['style', 'border-top:1px solid #C2C2C2;']]);
		heroRow.appendChild(heroLabelCell);
		var acc = 0;
		if (tadPower[0][9] > 0) acc = tadPower[1][8];
		heroACell = newCell(acc, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
		if (tadPower[1][9] > 0) acc = Math.floor(tadPower[0][8] / tadPower[1][9]); else acc = 0;
		heroDCell = newCell(acc, [['align', docDir[1]], ['style', 'border-top:1px solid #C2C2C2; font-size:8pt; font-weight:bold;']]);
	...
	function preCompute1() {
		...
		if (boolShowResUpgradeTable == '1') {
			...
			var noOfRow = 0;
			for (var i = 0; i < 4; i++) {
				var td1 = newCell('<img src="' + img("r/" + (i+1) + ".gif") + '" width="18" height="12" border="0" title="' + T('RECURSO' + (i+1)) + '">', [['width','25%']]);
				...
				for (var j = 0; j < 25; j++){
					if ((boolIsThisTheCapital) || (!boolIsThisTheCapital && j < 10)){
						if (grid[i][j] > 0 && buildingCost[i][j+1] != null){
							...
							if (j > 0) imagen += '<img src="' + img("g/s/s" + j + ".gif") + '" style="position:relative; bottom:30px; ' + docDir[0] + ':12px;" border="0">';
							imagen += '</div></a>';
							var td = newCell(imagen);
							...
							//********************
							if (restante != null) {
								restante.appendChild(getCpRow(cpB));
								restante.appendChild(getCcRow(ccB));
								...
								var aTable = newTable("");
								var aRow = newRow("");
								var aCell = newCell('<a href="/build.php?id=' + grid[i][j] + '">' + T('EXTAVAILABLE') + '</a>', [['style', 'font-size:8pt; font-weight:bold;']]);
								aRow.appendChild(aCell);
								aTable.appendChild(aRow);
								aTable.appendChild(getCpRow(cpB));
								aTable.appendChild(getCcRow(ccB));
	...
	function getCpRow(cpPerDay) {
		...
		var cpCell = newCell(T('CPPERDAY') + ": " + cpPerDay[0] + " " + dAr + " " + cpPerDay[1], [['colspan', '3'], ['style', 'font-size:8pt; color:blue']]);
	...
	function getCcRow(ccPerDay) {
		...
		var dAr = '→';
		if (docDir[0] == 'right') dAr = '←';
		var ccCell = newCell('<img src="' + img("r/5.gif") + '">' + " : " + ccPerDay[0] + " " + dAr + " " + ccPerDay[1], [['colspan', '3'], ['style', 'font-size:8pt; color:red']]);
	...
	function createCNDiv(levelX, strNewLevel) {
		var aDIV = elem("DIV", "");
		var iHTML = levelX;
    ...
    function ShowCenterNumbers(arrBiP){
        ...
        if (boolShowCenterNumbers == "1" && (location.href.indexOf('dorf2') != -1 || location.href.indexOf('build.php?newdid=') != -1)) {
            ...
            if (map1Element){
                ...
                var lblBuilding, lvlBuilding, aDIV, coords;
                ...
                for (var i = 0; i < maxCount; i++) {
					var strNewLevel = null;
					lblBuilding = areaElements[i].getAttribute("title");
					lvlBuilding = /(\d+)/.exec(areaElements[i].getAttribute("title"));
					
					//log(3, "lblBuilding = " + lblBuilding);
                    ...
                    if (lvlBuilding){
						...
						if (arrBiP != null) {
							arrLevel = lblBuilding.split(" ");
							var bName = arrLevel.pop();
							bName = arrLevel.pop();
							bName = arrLevel;
							bName = arrLevel.join(" ");
							strNewLevel = getNewUpgradeLevel(arrBiP, bName, lvlBuilding[0]);
							//log(3, "bName = " + bName + "; lvlBuilding[0] = " + lvlBuilding[0] + "; strNewLevel[0] = " + strNewLevel[0]);
							lvlBuilding[0] = strNewLevel[0];
							//log(3, "bName = " + bName + "; lvlBuilding[0] = " + lvlBuilding[0]);
						}
						...
						if (!isNaN(gid)) {
							var aDIV = createCNDiv(lvlBuilding[0], strNewLevel);
							aDIV.style.top = parseInt(coords[1]) + 60 + 'px';
							...
							if (boolShowBuildColorCodes != "1") {
								...
								var colorCode = colorLvl(lvlBuilding[0], parseInt(gid), currentTotalRes);
								if (colorCode == 0) {
									aDIV.style.backgroundColor = CN_COLOR_NO_UPGRADE;
								} else if (colorCode == 2) {
									aDIV.style.backgroundColor = CN_COLOR_UPGRADABLE_VIA_NPC;
	...
	function preCompute2(){
		...
		ShowCenterNumbers(arrBiP);

		// get all the building titles
		var aXP = find('//map[@name="map1"]/area/@title', XPIterate);
		while ((bDesc[bDesc.length] = aXP.iterateNext())) {}

		// get alle the building links
		aXP = find('//map[@name="map1"]/area/@href', XPIterate);
		while ((bLink[bLink.length] = aXP.iterateNext())) {}

		// get all the building images
		var aXP = find('//div[@id="lmid2"]/img/@src', XPList);
		bImg[0] = document.createTextNode(img('g/g16.gif'));
		for (var i = 0; i < aXP.snapshotLength; i++) {
			if (aXP.snapshotItem(i).nodeValue.indexOf("special") == -1) {
				bImg[bImg.length] = aXP.snapshotItem(i);
			}
		}
		//while ((bImg[bImg.length] = aXP.iterateNext())) {}
		...
		if (a) {
			...
			if (b) bImg[bDesc.length - 4] = document.createTextNode(img(b));
		}
		//log(3, "before first round");
		for(var i = 0; i < bDesc.length - 3; i++) {
			if (bImg[i]) {
			if (bDesc[i] != null && basename(bImg[i].nodeValue) != 'iso.gif') {
				// Get current building level
				var bLevel = bDesc[i].nodeValue.split(" ");
				bLevel = parseInt(bLevel[bLevel.length - 1]);
				
				var gid = bImg[i].nodeValue.split("/");
				...
				switch (gid) {
					...
					case 19: {setGMcookie('barracks', 19, true); boolIsAvailableBarracks = true; getRace();} break;
			...
			for (var i = 0; i < bDesc.length - 3; i++) {
				if (bImg[i]) {
					if (bDesc[i] != null && basename(bImg[i].nodeValue) != 'iso.gif') {
						...
						var arrLevel = bDesc[i].nodeValue.split(" ");
						...
						if (buildingCost[gid] != null && buildingCost[gid][bLevel + 1] != null) {
							...
							switch (gid) {
								...
								case 31: bImg[i].nodeValue = image["empalizada"]; break;
								case 32: bImg[i].nodeValue = image["muralla"]; break;
								case 33: bImg[i].nodeValue = image["terraplen"]; break;
								case 16: bImg[i].nodeValue = image["rallypoint"]; break;
							}

							var td = newCell("", [['width', Math.floor(100/intNoCellsPerRow) + '%'], ['valign','bottom']]);
							aRow.appendChild(td);
							...
							var nametd = newCell('<a href="' + bLink[i].nodeValue + '">' + bDesc[i].nodeValue + strNewLevel[1] + '</a>', [['colspan',"2"], ['class', 'f10']]);
							nametr.appendChild(nametd);

							var bRow = elem("TR", "");
							table2.appendChild(bRow);

							var td2 = newCell('<a href="' + bLink[i].nodeValue + '"><img src="' + bImg[i].nodeValue + '" width="90%" border="0"></a>', [['class', 'f10']]);
							//var td2 = newCell('<a href="' + bLink[i].nodeValue + '"><img src="' + bImg[i].nodeValue + '" border="0"></a>', [['class', 'f10']]);
							bRow.appendChild(td2);
							...
							if (restante != null) {
								...
								var xTable = newTable("");
								var xRow = newRow("");
								var xCell = newCell('<a href="' + bLink[i].nodeValue + '">' + T('EXTAVAILABLE') + '</a>', [['style', 'font-size:8pt; font-weight:bold;']]);
								xRow.appendChild(xCell);
								xTable.appendChild(xRow);
								xTable.appendChild(getCpRow(cpB));
								xTable.appendChild(getCcRow(ccB));
	...
	*/
	...
	function getMerchantMultiplier() {
		...
		if (location.href.indexOf('speed') != -1) intMultiplierM = 3;
	...
	function getTroopsDetails(qDist, crtUserRace, boolIgnoreTS) {
		...
		if (location.href.indexOf('speed') != -1) arX[4] = 2;
		return arX;
	}
	...
	function marketResources() {
		...
		if (boolMarketResources) {
			...
			for (var i = 0; i < 4; i++){
				...
				xLink.addEventListener('click', createEventMarketResources(i, parseInt(currentResUnits[i]), max_transport, maxNoOfMerchants, maxCapacity), false);
				var aCell = newCell("", [['align', 'center']]);
				aCell.appendChild(xLink);
				aRow.appendChild(aCell);
			...
			var aCell = newCell("<img src='" + img("r/1.gif") + "'>" + "<img src='" + img("r/2.gif") + "'>" + "<img src='" + img("r/3.gif") + "'>" + "<img src='" + img("r/4.gif") + "'>", [['colspan', '2']]);
			...
			var uCell1 = elem("TD", "<img src='" + img("r/1.gif") + "' title='" + T('RECURSO1') + "' alt='" + T('RECURSO1') + "'>");
			...
			var uRow2 = elem("TR", "");
			var uCell4 = elem("TD", "<img src='" + img("r/2.gif") + "' title='" + T('RECURSO2') + "' alt='" + T('RECURSO2') + "'>");
			...
			var uRow3 = elem("TR", "");
			var uCell7 = elem("TD", "<img src='" + img("r/3.gif") + "' title='" + T('RECURSO3') + "' alt='" + T('RECURSO3') + "'>");
			...
			var uRow4 = elem("TR", "");
			var uCell10 = elem("TD", "<img src='" + img("r/4.gif") + "' title='" + T('RECURSO4') + "' alt='" + T('RECURSO4') + "'>");
		...
		function MerchantArrivingTitles(maxCapacity) {
			...
			var availableAndTotalMercsString = xpathResultEvaluate('//div[@id="lmid2"]/form/table/tbody/tr/td/table[@class="f10"]/tbody/tr/td/span[@class="f135 b"]/../../../tr[1]/td');
		...
		function addCumulativeArrivals(maxCapacity) {
            ...
            for(var i = 0; i < sendReceive.snapshotLength; i++) {
                ...
                if (sendReceive.snapshotItem(i).nodeName == "P") {
                    ...
                    var cCell = elem("TD", "<img src='" + img("a/clock.gif") + "'>");
                    ...
                    for (var xi = 1; xi < 6; xi++) {
                        if (xi < 5) {
                            var iCell = elem("TD", "<img src='" + img("r/" + (xi) + ".gif") + "'>");
            ...
            for (var zi = 0; zi < 4; zi++) {
				...
				var totalAtArrival = parseInt(currentResUnits[zi]);
				...
				if (productionPerSecond < 0) {
					timeToFill = Math.abs(parseInt(currentResUnits[zi]) / productionPerSecond);
	...
	function culturePoints(){
		...
		var cpHeader1Cell2 = newCell(arrValues[1], [['colspan', 2]]);
		cpHeader1.appendChild(cpHeader1Cell2);
		var cpHeader1Cell3 = newCell("<img src='" + img("a/clock.gif") + "' border='0'>", [['colspan', 2]]);
				...
				for (yi = 0; yi < aTable.rows.length; yi++) {
					...
					if (boolIsTownHall) {
						...
						if (yi == 0 && aTable.rows) {
							...
							var lvlArBlObj = find("//div[@id='lmid2']//h1", XPFirst);
							var lvlArBl = '';
	...
	function userBookmarks(){
		...
		if (boolShowBookmarks == "1") {
			...
			for (var i = 0; i < marcadores.length; i++){
				...
				var aCell = elem("TD", "");
				if (boolLockBookmarks != "1") {
					var delImg = newImage([['src', img("a/del.gif")], ['width', '12'], ['height', '12'], ['border', '0'], ['style', 'cursor:pointer'], ['title', T('ELIMINAR')]]);
    ...
    function villageList() {
		...
		for (var i = 0; i < cities.childNodes.length; i++){
			...
			//if (boolShowInOutIcons == "1") {
				//get the village name and the newdid ID for the inside/outside/center map links
				var newdidxi = cityLink.indexOf('?newdid=');
				cityLink = cityLink.substring(newdidxi + 7);
				newdidxi = cityLink.substring('?');
				if (newdidxi > 0) {
					cityLink = cityLink.substring(0, newdidxi);
				}
			...
			city.appendChild(elem("TD", "<a href='a2b.php?z=" + vID + "'><img src='" + img("a/def1.gif") + "' width='12' border='0' title='" + T('RALLYPOINT') + "' alt='" + T('RALLYPOINT') + "'></a>"));
			city.appendChild(elem("TD", "<a href='build.php?z=" + vID + "&gid=17'><img src='" + img("r/4.gif") + "' height='12' border='0' title='" + T('ENVIAR') + "' alt='" + T('ENVIAR') + "'></a>"));
			if (location.href.indexOf('karte') != -1) {
				if (boolShowDistTimes == "1" && xCoord != "" && yCoord != "") {
	...
	function selectAllTroops() {
        ...
        if ((troopTable) && !(tags7)) {			
			...
			if (parX) {
	      		...
	      		var aCell = newCell("<img src='" + img("a/att_all.gif") + "'> *", [['id', "troopsattpower"], ['width', "33,3%"]]);
	      		var bCell = newCell("<img src='" + img("a/def_i.gif") + "'> *", [['id', "troopsdefipower"], ['colspan', "2"], ['width', "33,3%"]]);
	      		var cCell = newCell("<img src='" + img("a/def_c.gif") + "'> *", [['id', "troopsdefcpower"], ['width', "33,3%"]]);
	      		...
	      		var eCell = newCell("<img  src='" + img("r/5.gif") + "'>", [['id', 'troopscropconsumption'], ['align', docDir[0]], ['colspan', "2"], ['width', '50%']]);
			...
			for (var j = 0; j < 5; j++) {
				...
				if (aCell) {
					...
					switch (j) {
						case 0:
						case 1:
						case 2: imgPath = img("a/" + totals[j][1] + ".gif"); break;
						case 3: imgPath = image["capacity"]; strX = ""; break;
						case 4: imgPath = img("r/5.gif"); strX = ""; break;
					}
					aCell.innerHTML = "<img src='" + imgPath + "'>" + strX + " " + totals[j][3].toLocaleString();
				}
			}
			return;
		}
	...
	function getOasisTroopsInfoTable(tNTroops, boolFromMap) {
		...
		for (var i = 0; i < tNTroops.childNodes.length; i++) {
			//toolTipHTML = toolTipHTML + "<tr>";
			var aRow = tNTroops.childNodes[i];
			if (aRow.childNodes[0].firstChild.src) {
				var ind = aRow.childNodes[0].firstChild.src.match(/^(.*)img\/un\/u\/(\d+)\.gif$/);
				if (ind.length < 3) return;
				var index = ind.pop();
				if (boolFromMap) {
					toolTipHTML += "<td><img src=" + aRow.childNodes[0].firstChild.src + "></td><td align='" + docDir[1] + "'>" + aRow.childNodes[1].textContent + "</td></tr>";
				}
		...
		if (boolFoundInfo == true) {
			if (tNinfo[0] != 0 + tNinfo[1] + tNinfo[2] > 0) {
				...
				toolTipHTML += "<tr><td><img src='" + img("a/def_i.gif") + "' width='16' height='16' border='0'></td><td align='" + docDir[1] +"'>&nbsp;" + tNinfo[0] + "</td></tr>";
				toolTipHTML += "<tr><td><img src='" + img("a/def_c.gif") + "' width='16' height='16' border='0'></td><td align='" + docDir[1] +"'>&nbsp;" + tNinfo[1] + "</td></tr>";
				toolTipHTML += "<tr><td><img src='" + img("r/5.gif") + "' width='18' height='12' border='0'></td><td align='" + docDir[1] +"'>&nbsp;" + tNinfo[2] + "</td></tr>";
			}
		}
		return toolTipHTML;
	}
	...
	function addTroopTimes() {
		...
		if (xCoord != "" && yCoord != "" && boolShowDistTimes == "1") {
			...
			if (lastRowActions) {
				createTimeTroopTable(lastRowActions, xCoord, yCoord, true);
				if (get("resfeld")) {
					//we are probably inside an oasis
					var tNTroops = find("//table[@class='f10']", XPFirst);
					if (tNTroops) {
						tNTroopsS = tNTroops.childNodes[0];
						var tNInfo = getOasisTroopsInfoTable(tNTroopsS, false);
						if (tNInfo != '') {
							tNInfo = "<table class='f10 b' cellpadding='0' cellspacing='0' border='0'>" + tNInfo + "</table>";
							var aPar = elem("P");
    ...
    function TravianBeyondSetup(){
		...
		var topCell = newCell(T('TBSETUPLINK') + " - " + versionText, [["width", "70%"]]);
		...
		var bottomCell = newCell(T('TBSETUPLINK') + " - " + versionText, [["colspan", "1"], ["width", "70%"]]);
	...
	function getTroopMerchantTooltipHTML(newdid, aColor, addCoords, addMTime, addTTime, boolAllRaces) {
		...
		if (qDist != 0) {
			...
			if (addCoords != undefined) {
				if (addCoords == true) {
					strDist = "&nbsp;&nbsp;(" + xActive + "|" + yActive + ") " + "<img src= '" + image["dist" + docDir[0].substr(0,1)] + "'>" + " (" + xyCoord[0] + "|" + xyCoord[1] + ")"; 
				}
			}
			if (strDist != "") {
		...
		if (crtUserRace != "false" && qDist != 0) {
			...
			var clockCell = "<td " + aColspan + aAlign + "><img src='" + img("a/clock.gif") + "'</td> ";
			if (addTTime == true) {
				//add the clock row
			...
			if (addTTime == true) {
				...
				for (iTroopType = 0; iTroopType < 10; iTroopType++) {
					...
					//log(4, strDist);
					iHTML += "<tr><td><img src =" + imgName + "></td><td align='" + properAlign +"'>" + "&nbsp;" + formatTime(aTime) + " h</td>" + strDist;
					if (boolAllRaces) {
						var imgNo = iTroopType + arY[3];
						var imgName = img("u/" + imgNo) + ".gif";
						var aTime = getTTime(iTroopType, arrRaces[0], arY);
						iHTML += "<td><img src =" + imgName + "></td><td align='" + properAlign +"'>" + "&nbsp;" + formatTime(aTime) + " h</td><td>&nbsp;&nbsp;&nbsp</td>";
						
						var imgNo = iTroopType + arZ[3];
						var imgName = img("u/" + imgNo) + ".gif";
						var aTime = getTTime(iTroopType, arrRaces[1], arZ);
						//log(4, strDist);
						iHTML += "<td><img src =" + imgName + "></td><td align='" + properAlign +"'>" + "&nbsp;" + formatTime(aTime) + " h</td>";
					}
					iHTML += "</tr>";
				}
			}
	...
	function showCellInfo(pos, aType) {
		var itext=['','(9)','<img src="' + img("r/3.gif") + '" alt="' + T('RECURSO4') + '">','(6)','<img src="' + img("r/2.gif") + '" alt="' + T('RECURSO3') + '">','<img src="' + img("r/1.gif") + '" alt="' + T('RECURSO2') + '">','(15)'];
	...
	function showOasisInfo(pos, animalsTable, newdid) {
		...
		if (toolTipHTML != '') {
			toolTipHTML = "<table class='f8' cellpadding='0' cellspacing='0' border='0'>" + toolTipHTML + "</table>";
		}
		//toolTipHTML += "</table>";
	...
	function showFieldTypeInTooltip(fieldtype, newdid) {
		...
		if (fieldtype != undefined) {
			...
			var dist = [[3, 3, 3, 9], [3, 4, 5, 6], [4, 4, 4, 6], [4, 5, 3, 6], [5, 3, 4, 6], [1, 1, 1, 15], [3, 4, 4, 7], [3, 5, 4, 6], [4, 3, 4, 7], [4, 3, 5, 6], [4, 4, 3, 7], [5, 4, 3, 6]];
			var info = dist[fieldtype-1];
			toolTipHTML += "<tr><td colspan='2'>";
			for (var i = 1; i < 5; i++) toolTipHTML += '<img src="' + img("r/" + (i) + ".gif") + '" width="18" height="12" border="0" title="' + T('RECURSO' + i) + '" alt="' + T('RECURSO' + i) + '">' + info[i-1] + ' ';
			toolTipHTML += "</td></tr>";
			if (boolShowDistTimes == "1") {
				toolTipHTML += "<tr><td>&nbsp;</td></tr>";
			}
	...
	*/
	...
	function infoResources() {
		...
		var aTimeOut = getRandomTimeRange(800);
		//genMapTable();
		//desplazarMapa();
		setTimeout(genMapTable, aTimeOut);
		setTimeout(desplazarMapa, aTimeOut);
		...
		*/
			...
			for(var i = 0; i < 7; i++)
				for(var j = 0; j < 7; j++) {
					...
					if (cellinfo && cellinfo.name !=null ) {
						...
						inforow.appendChild(elem('TD', '<a href="' + aHref.replace("karte.php?d", "a2b.php?z") + "><img src='" + img("a/" + arrAction[0] + ".gif") + "'width='12' title='" + arrAction[1] +"'></a>  <a href=" + aHref.replace("karte.php?d", "build.php?z") + "&gid=17><img src='" + img("r/4.gif") + "' height='12' title='" + T('ENVIAR') + "'></a>"));
		...
		function addLargeMapLink() {
			...
			if (aMap) {
				...
				lmLink.href = jsVoid;
				lmLink.innerHTML = "<img src = '" + img("m/max.gif") + "' title = '" + T('LARGEMAP') + "' alt = '" + T('LARGEMAP') + "'>";
	...
	function showOffers(){
		...
		if (ventas.length > 0){
			...
			for (var i = 0; i < ventas.length; i++){
				var tr = elem("TR", "");
				var td = elem("TD", '<img src="' + img("r/" + (ventas[i][2]) + ".gif") + '" width="18" height="12" border="0" title="' + T('RECURSO' + (ventas[i][2])) + '"> ' + ventas[i][0]);
				tr.appendChild(td);
	...
	function processVillage11(t){
		...
		find("//img[@id='aldea" + newdid + "_boton']", XPFirst).src = img("a/b2.gif");
	...
	function processVillage44(t) {
		...
		if (villageID != 0) {
			var vTroopTables = ansdoc.evaluate('//div[@id="lmid2"]/table/tbody/tr/td[1]/a[contains(@href, ' + villageID + ')]/../../../..|//div[@id="lmid2"]/p[@class="b f16"]', ans, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
			if (vTroopTables) {
	...
	function createEventRefreshVillageV2(newdid, i, xi){
		...
		return function(){
			find("//img[@id='aldea" + newdid[i] + "_boton']", XPFirst).src = img("a/b3.gif");
			if (xi == 1) {
				...
				ajaxRequest("dorf1.php?newdid=" + newdid[i], "GET", null, processVillage11,
					function(){ find("//img[@id='aldea" + newdid[i] + "_boton']", XPFirst).src = img("a/b4.gif"); });
				...
				if (boolUpdTroopsTraining) {
					...
					if (isAvailableBarracks != false) {
						...
						ajaxRequest(pgAjaxRequest, "GET", null, processVillage119, function(){ find("//img[@id='aldea" + newdid[i] + "_boton']", XPFirst).src = img("a/b4.gif"); });
					}
				...
				if (cpbuilding != false) {
					...
					find("//img[@id='aldea" + newdid[i] + "_boton']", XPFirst).src = img("a/b5.gif");
				...
				if (boolupdParty == true) {
					...
					if (isAvailableTownhall != "false") {
						...
						ajaxRequest(pgAjaxRequestParty, "GET", null, processVillage43, function(){ find("//img[@id='aldea" + newdid[i] + "_boton']", XPFirst).src = img("a/b4.gif"); });
					}
				}
	...
	function createDorf35Table(newdid, villageName, newPar, topRowText, secRowText) {
		...
		var tdTopHero = elem("TD","<img src=" + img("u/hero.gif") + ">");
	...
	function createDorf33Table(newdid, villageName, newPar, topRowText, secRowText) {
		...
		for (xi = 0; xi < 7; xi++){
			switch (xi) {
				...
				case 4: var tdTop2 = elem("TD", "<img src='" + img("a/clock.gif") + "' border='0'>"); break;
				case 5: var tdTop2 = elem("TD", "<img src='" + image['img4'] + "' border='0'>"); break;
				case 6: var tdTop2 = elem("TD", "<img src='" + img("a/clock.gif") + "' border='0'>"); break;
			}
	...
	function createDorf32Table(newdid, villageName, newPar, topRowText, secRowText, merchant) {
		...
		for (xi = 0; xi < 7; xi++){
			switch (xi) {
				...
				case 5: var tdTop2 = elem("TD", "<img src='" + image['img' + 4] + "' border='0'>/<img src='" + img("a/clock.gif") + "' border='0'>"); break;
	...
	function rowsDorf3(nodeToAppendTo, newdid, villageName, maxTD, tabNo, merchant) {
		for (var i = 0; i < newdid.length; i++){
			...
			var aLink = elem("A", "<img src='" + img("a/b5.gif") + "' border='0' title='" + T('ACTUALIZAR') + "' id='aldea" + newdid[i] + "_boton'>");
			aLink.href = jsVoid;
	...
	function NPCAssistant(typeNPC, xpNeeded, multipliers) {
		...
		for (var i = 0; i < xpNeeded.snapshotLength; i++) {
			...
			if (multipliers) {
				...
				var aLink = newLink(max, [['href', jsVoid]]);
				aLink.addEventListener('click', clickOnNPCAssistant(i, max), false);
		...
		function clickOnNPCAssistant(i, max) {
			return function() {
	...
	function tableTotalVillageTroopsV3() {
		...
		if (villageID != "0") {
			...
			if (vTroopTables.snapshotLength > 0) {
				...
				tTableConsumptionRow.cells[1].innerHTML = (aConsumption) + " <img src='" + img("r/4.gif") + "'>";
	...
	function isThisTheCapital() {
		...
		if (capitalCoordsString != "false") {
			...
			if (villageInfo[1] == undefined || villageInfo[1] == null || villageInfo[1] == "" || villageInfo == "0") {
				villageInfo = getIdVillageV3();
			}
			if (parseInt(capitalID) == parseInt(villageInfo[1])) {
				return true;
			} else {
				return false;
			}
	...
	function processTroopsTrainingTable() {
		...
		if (origTable) {
			...
			var hCell = newCell("<img src='" + img("a/clock.gif") + "'>" + "&" + "<img src='" + img("r/5.gif") + "'>", [['class', 'cbg1']]);
		...
		return function() {
			...
			if (trNoInput) {
				...
				if (!isNaN(intNo)) {
					...
					var aDiv = elem("DIV", "");
					var aTable = newTable();
					//var iHTML1 = "<img src='" + img("a/clock.gif") + "'><br>" + timeTotal + " h";
					var iHTML1 = '';
					var iHTML2 = '';
					if (totalSeconds > 0) {
						iHTML1 = timeTotal + " h";
					...
					if (totalConsumption > 0) {
						var iHTML2 = "<img src='" + img("r/5.gif") + "'> " + totalConsumption;
					}
					if (!totalTimeCell) {
						var aRow = elem("TR", "");
					...
					if (origTable) {
						...
						if (btCell0 && btCell1 && btCell3) {
							...
							if (intTTotal > 0) {
								btCell3.innerHTML = formatTime(intTTotal) + " h <br>" + "<img src='" + img("r/5.gif") + "'> " + intCTotal;
							} else {
								btCell3.innerHTML = '';
							}
						}
		...
		if (arrCoords) {
			for (var j = 0; j < arrCoords.length; j++) {
				...
				if (xyCoord.indexOf("(") == 0 && xyCoord.indexOf(")") != -1  && xyCoord.indexOf("|") != -1) {
					...
					"&nbsp;<a href='a2b.php?z=" + idVillage + "'><img border='0' height='12' src='" + img("a/" + arrAction[0] + ".gif") + "' title='" + arrAction[1] + "' alt='" + arrAction[1] + "'/></a>" +
					"&nbsp;<a href='build.php?z=" + idVillage + "&gid=17'><img border='0' height='12' src='" + img("r/4.gif") + "' title='" + T('ENVIAR') + "' alt='" + T('ENVIAR') + "'/></a>";
	...
	function showTroopInfoTooltip(ucIndex, troopName) {
        return function() {
            ...
            //attack power row
			var aCell2 = newCell("<img src='" + img("a/att_all.gif") + "'>", [['style', 'border-bottom:1px grey solid;']]);
			...
			var aCell4 = newCell("<img src='" + img("a/def_i.gif") + "'>", [['style', 'border-bottom:1px grey solid;']]);
			...
			var aCell6 = newCell("<img src='" + img("a/def_c.gif") + "'>", [['style', 'border-bottom:1px grey solid;']]);
            ...
            //crop consumption
			var aCell12 = newCell("<img src='" + img("r/5.gif") + "'>", [['style', 'font-size:8pt']]);
	...
	function getMerchantsInformation() {
		...
		//log(3, "enter getMerchantsInformation");
		//remove the old cookies that are not used anymore due to function change
		//deleteGMcookie("merchantsCapacity", false);
		//deleteGMcookie("merchantsCapacityV2", false);
