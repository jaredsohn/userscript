...
if((decodeURIComponent((new RegExp('[?|&]' + "upload" + '=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) == "1") && (location.pathname == "/boards/user.php")) {
    ...
    $("html.js body.wf-active div.wrapper div#mantle_skin div#content.container div.main_content div.span8 div.body table.board tbody tr td").eq(0).append(" ").text();
    var upload_user = $("html.js body.wf-active div.wrapper div#mantle_skin div#content.container div.main_content div.span8 div.body table.board tbody tr td").eq(0).text();
    ...
    if( user ) {
        $("tbody").append("<div style='float:left;width:100px;height:250px;'><img class='avatar' src='http://dl.dropboxusercontent.com/u/252632325/GameFAQs-Avatars/" + user + ".png' alt='' ></div>" );
        $("tbody").append("<div style='float:left;padding-left:10px'><ul id=settings class=\"paginate user\" style=\"margin:0;padding:0;\"><li>Global Avatar Settings:</li><li><a href=\"\" id=\"av_left\">Avatars to the Left</a></li><li><a href=\"\" id=\"av_right\">Avatars to the Right</a></li><li><a href=\"\" id=\"av_no\">No Avatars</a></li></ul> \
		...
		$("tbody").append("<div style='float:left;padding-left:10px;padding-top:10px;'>GameFAQs Avatars created by <a href='http://www.gamefaqs.com/users/Judgmenl/boards'>Secret Dragoon</a> - 2014.</div>");
	...
	if( cookie == "left" ) {
		...
		if($('span.author_data:nth-child(2)').length != 0) {
			for( var i = 0; i < 50; i++) {
				$('span.author_data:nth-child(2)').eq(i).after("<img src=http://dl.dropboxusercontent.com/u/252632325/GameFAQs-Avatars/" + $(".name").eq(i).text().split(' ').join('%20') + ".png alt= >" );
			}
		} else {
			/* Please note this is for users who have Message Poster Display "Above Message" */
			for( var i = 0; i < 50; i++) {
				$("td.msg").eq(i).prepend("<div style=\"position:absolute;\"><img class=avatar src=http://dl.dropboxusercontent.com/u/252632325/GameFAQs-Avatars/" + $(".name").eq(i).text().split(' ').join('%20') + ".png alt= ></div>" );
		...
		for( var i = 0; i < 50; i++) {
			...
			$("td.msg").eq(i).prepend("<div style=\"position:absolute; right:8px;\"><img class=avatar src=http://dl.dropboxusercontent.com/u/252632325/GameFAQs-Avatars/" + $(".name").eq(i).text().split(' ').join('%20') + ".png alt= ></div>" );
