// ==UserScript==
// @name           Addat.hu & Data.hu Links Checker
...
About this script
 ...
 Supports :  rapidshare.de netload.in easy-share.com addat.hu megashares.com 
             depositfiles.com vip-file.com yourfilehost.com sendspace.com filebase.to 
             link-protector.com bitroad.net vip-file.com filenavi.com data.hu 
...
var addat_hu= new Array(6)                         //http://addat.hu/78de5323/XP-Logo.gif.html
 addat_hu[0]="addat.hu/\\?";                  //name and search string in link to get description
 addat_hu[1]='free_download';                      // file_is_alive
 addat_hu[2]='Hiba!';                   // file_is_dead
 addat_hu[3]='dsdlhkhsgdsgdhskjhgd';                  // no download slots or temporarily unavailable or servererror                  
 addat_hu[4]="//a[contains(@href,'www.addat.hu/?')]"; 
 addat_hu[5]='dsdlhkhsgdsgdhskjhgd';                  // tos violation
...
var http_file_hosts=[netload_in,easy_share_com,addat_hu,megashares_com,rapidshare_de,depositfiles_com,vip_file_com,yourfilehost_com,sendspace_com,filebase_to,link_protector_com,bitroad_net,filenavi_com,data_hu,letitbit_net,zshare_net];
...
var lianks = document.evaluate(netload_in[4]+'|'+easy_share_com[4]+'|'+addat_hu[4]+'|'+megashares_com[4]+'|'+depositfiles_com[4]+'|'+vip_file_com[4]+'|'+sendspace_com[4]+'|'+yourfilehost_com[4]+'|'+rapidshare_de[4]+'|'+filebase_to[4]+'|'+link_protector_com[4]+'|'+bitroad_net[4]+'|'+filenavi_com[4]+'|'+data_hu[4]+'|'+letitbit_net[4]+'|'+zshare_net[4] ,  document,	null,	XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,	null);
...
//var regex = /http:\/\/(netload\.in\/date|w\d\d\.easy-share\.com|depositfiles|(www\.|)yourfilehost\.com|vip-file\.com|rapidshare\.de|(www\.|)sendspace\.com|(www\.|)megaupload\.com|(www\.|)addat\.hu|(www\.|d\d\d\.|)megashares\.com)[\w\-.+$!*\/(),~%?:@#&=\\]*/gi;
//var regex = /http:\/\/(netload\.in\/date|w\d\d\.easy-share\.com|depositfiles|(www\.|)yourfilehost\.com|vip-file\.com|rapidshare\.de|(www\.|)sendspace\.com|(www\.|)addat\.hu|(www\.|d\d\d\.|)megashares\.com)[\w\-.+$!*\/(),~%?:@#&=\\]*/gi;
var regex = /http:\/\/(netload\.in\/date|w\d\d\.easy-share\.com|filebase\.to\/files|link-protector\.com\/|depositfiles\.com\/files\/|bitroad\.net\/download\/|vip-file\.com\/download\/|www.filenavi\.com\/direct\/|(www\.|)yourfilehost\.com|vip-file\.com|rapidshare\.de|(www\.|)sendspace\.com|data\.hu\/get\/|www\.zshare\.net\/|(www\.|)addat\.hu|(www\.|d\d\d\.|)megashares\.com)[\w\-.+$!*\/(),~%?:@#&=\\]*/gi;
