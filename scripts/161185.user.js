...
var THUMB_RESOLVERS = {
	...
	twipple: {
		...
		regexThumb: /^http:\/\/p\.twipple\.jp\/show\/thumb\/\w+/,
		resolv: function(matched, complete) {
			complete('http://p.twipple.jp/show/thumb/' + matched[1]);
		}
	},
	photozou: {
	...
	pixiv: {
		...
		resolv: function(matched, complete) {
			var imgId = matched[1];
			callWebApi(EXT_SERV_ENDS.pixiv, { id: imgId }, function(data) {
				var imgUrl = data && 'http://img' + data.serverId + '.pixiv.net/img/' + data.userId + '/mobile/' + imgId + '_87ms.jpg';
				complete(imgUrl, true);
			});
		}
	},
	...
	facebook: {
		...
		resolv: function(matched, complete) {
			expandShortUrl('https://www.facebook.com/twitter/photo/?' + matched[1], function(url) {
				var imgUrl = null;
				if (url) {
		...
		ul.append(
			$E('li', { 'class':'ithumb-li' }).append(
				$E('a', { 'class':'ithumb-a', 'target':'_blank', 'href':urlEntry.url, 'rel':'url' }).append(
					$E('img', { 'src':urlEntry.thumbUrl, 'class':'ithumb-img' })
						.error(function() {
							var img = $(this);
...
// initial apply
setTimeout(function() { applyElements(document); }, 1000);
