// ==UserScript==
// @name           StBx
// @include        *stealthbux.com/ads.php*
// @include        *stealthbux.com/cks.php*
// ==/UserScript==
...
BUX = {
    ...
    ClickRedDot: function() {
        ...
        for (var a = links.length-1; a >= 0; a--) {
			...
			if (!currentLink.href || currentLink.href.indexOf('cks.php') < 0 || this.clicked[currentLink.href]) {
				...
				window.setTimeout(function() {
					window.history.go(0);
				}, Math.floor(40 + Math.random() * 5) * 1000);
            break;
			} 
    ...
    ClickLinks: function() {
        ...
        for (var a = links.length-1; a >= 0; a--){
            ...
            while (tr && tr.tagName != "BODY") {
                if (tr.tagName == "TR") {
                    if (tr.innerHTML.indexOf('plus_32.png') >= 0) {
                        trOk = true;
                    }
                }
                tr = tr.parentNode;
