...
// @include        http://www.schuelerprofile/index,home,VpNavi.htm
// ==/UserScript==
...
function getName(response,meno){
	var response2=response.split("http://www.schuelerprofile.de/Poke,deleteAll,ViewHome.htm",2);
	var meno2=meno.split('&amp;answerPoke=1',2);
	var response3=response2[1].split('http://www.schuelerprofile.de/Profile,profileMemberTo,ViewHome,1.htm?'+ meno2[0] +'">',2);
...
function addName(){
	...
	names.push(name);
	pokes.push(1);
	hrefs.push("http://www.schuelerprofile.de//Profile,profileMemberTo,ViewHome,1.htm?"+ htmlentities(id_meno2[0]));
	return;	
}
...
function pokus(){
	if (tukanie==1){
		...
		ajaxRequest('http://www.schuelerprofile.de//index,home,VpNavi.htm','GET',null,handleServerResponse,function(){alert("Nepodarilo sa nacitat stranku zo serveru.");});
	}
}
