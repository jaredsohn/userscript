...
function autobuild()
{
	if (page == "investment")
	{
		...
		if (def || inc)
		{
			...
			document.getElementById('wwfixernextbuilding').innerHTML = '<font color="#00ff00"><small><b>Next building: '+names[minIndex]+', need <img src="http://static.storm8.com/rl/images/money.png?v=140">'+fixNum(need)+'</b> - '+Math.ceil(need / rate)+'h</small></font>';
...
{
	// check health
	if (parseInt(document.getElementById('healthCurrent').innerHTML) <= 1650)
		return;
	// check ammo
	...
	if (!found)
		...
		//setTimeout("document.location = 'http://rl.storm8.com/hitlist.php'", Math.floor(Math.random() * 40)*90);
		//setTimeout("document.location = 'http://rl.storm8.com/hitlist.php'", 2000);
