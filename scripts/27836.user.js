           ...
           necesario[i]=necesario[i]-0;
			...
			if (restante > 0){
				...
				if (tiempo < 0 || total[i]-necesario[i]<0) {
                    tiempo_max = 'Infinity';
                    res_table += '<tr><td><img src="' + img('r/' + (i+1) + '.gif') + '" width="18" height="12" border="0" title="' + T('RECURSO' + (i+1)) + '"></td><td align="right" id="timeout' + i + '">' + restante + "</td><td><span style='background-color: #FF0000'>" + T('NUNCA') + '</span></td></tr>';
                } else {
...
//       log(1,a);
	...
	function calcularTextoTiempo(fecha){
		...
		var tiempo_restante='';
		if (horas < 24) tiempo_restante = "<span style='background-color: #00FF00'>" + T('HOY') + " " + T('A_LAS') + " " + '</span>';
		else if (horas < 48) tiempo_restante = "<span style='background-color: #FFFF00'>" + T('MANYANA') + " " + T('A_LAS') + " " + '</span>';
		else if (horas < 72) tiempo_restante = "<span style='background-color: #FF00FF'>" + T('PAS_MANYANA') + " " + T('A_LAS') + " " + '</span>';
		else tiempo_restante = T('EL') + " " + LZ(fecha.getDate()) + "/" + LZ((fecha.getMonth()+1));

		return tiempo_restante + '<b>' + LZ(fecha.getHours()) + ":" + LZ(fecha.getMinutes()) + '</b>';
	}
...
//				[T('UPGRADET'), "http://userscripts.org/scripts/source/15943.user.js", "_blank"],
		];
