...
Menu.init = function() {
  ...
  menuCode.push('<tr><td>Mean job payout</td><td id="jobincome_stat" style="color: green; text-align: right;"></td></tr>');
  menuCode.push('<tr><td>Mean bank loss</td><td id="bankloss_stat" style="color: red; text-align: right;"></td></tr>');
  menuCode.push('<tr><td>Upkeep</td><td id="upkeep_stat" style="color: red; text-align: right;"></td></tr>');
  menuCode.push('<tr><td colspan="2"><hr/></td></tr>');
  menuCode.push('<tr><td>Total</td><td id="total2_stat" style="color: green; text-align: right;"></td></tr>');
  menuCode.push('<tr><td colspan="2">&nbsp;</td></tr>');
  /* */
  menuCode.push('<tr><td>Attack Strength</td><td id="attackStrength_stat" style="color: green; text-align: right;"></td></tr>');
  menuCode.push('<tr><td>Defense Strength</td><td id="defenseStrength_stat" style="color: green; text-align: right;"></td></tr></table>');
  menuCode.push('<br/>');
...
Menu.update = function() {
  ...
  el = document.getElementById('bankloss_stat');
  if (boss.preferences.bank_active)
    el.innerHTML = menu_intToDollars(Math.round((boss.total_income + boss.job_income - boss.total_upkeep) * .1));
  else
    el.innerHTML = menu_intToDollars(0);
  ...
  if (boss.preferences.bank_active)
    total = total - Math.round(total * .1);
  el.innerHTML = '';
  if (total < 0) {
    el.style.color = 'red';
    el.innerHTML = '-';
  ...
  el = document.getElementById('attackStrength_stat');
  el.innerHTML = attack;
  el = document.getElementById('defenseStrength_stat');
  el.innerHTML = defense;
}
/* vim:set tw=80 sts=2 et ft=javascript: */
    ...
    for (var i = 0; i < divs.length; i++) {
      ...
      switch(Page.c_page) {
        case 'stockpile':
          ...
          if (item_id == 'item_57' || item_id == 'item_58' || item_id == 'item_59') item.stocktype = 'weapon'; 
        ...
        case 'city':
          ...
          break;
        case 'jobs':
          if (Utils.getElementsByXPath('.//a[contains(@href,"item")]/following-sibling::span', div)[0].innerHTML.match(/\%/))
            continue;
          else
            item = new Prepitem();
          break;
      ...
      switch (Page.c_page) {
        case 'jobs':
          div = divs[i].parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;
          str = div.innerHTML.replace(/\n/g,'');
          result = str.match(/Energy:&nbsp;(\d+).*\(\+(\d+)\)/);
          if (result != null)
            item.energy_per_unit = result[1] / result[2];
          else
            item.energy_per_unit = 15;
          GM_log("Item: " + item.name + " Energy: " + item.energy_per_unit);
          break;
        case 'city':
...
/* Modules */
var modules = ['inventory', 'datadisplay', 'bank', 'hospital', 'jobs', 'skill', 'autoCityBuy'];
//var modules = ['hitlist', 'inventory', 'datadisplay', 'bank', 'hospital', 'jobs'];
//#include "hitlist.module.js"
...
function inventory_exec() {
  ...
  if (pages.indexOf(Page.c_page) != -1) {
    ...
    for (var i = 0; i < divs.length; i++) {
      ...
      if (result) GM_log("You have " + inventory[divs[i].name] + " of " + divs[i].name);
    }
...
Joblist.prototype.updateData = function() {
  if (Page.c_page == 'jobs') {
    ...
    for (var i = 0; i < divs.length; i++) {
      ...
      this[tmp.value] = job;
      GM_log("Job added: " + job.name + jobItems);
    }
    this.save();
  }
}
...
function jobs_canDoJob(job) {
  ...
  /* Check mobsters */
  if (job.mobsters > boss.mobsters + 1) {
...
function skill_exec(){
  ...
  if (boss.preferences.skill == "recovery_max" && Page.c_page == 'boss'){
    ...
    var result = announcement.match(/You have <b>?\$?([0-9]+)/);
    var points = 0;
    if (result)
...
function skill_preferencesInterface(prefs){
  ...
  var div = document.createElement('div');
  div.innerHTML = '<label for="skillList">Skill to automatically level up: </label>';
  var select = document.createElement('select');
  var option;
  select.name = 'skillList';
  option = '<option value="none"';
  if (boss.preferences.skill == 'none') option += ' selected="selected"';
  option += '>None</option><option value="attack"';
  if (boss.preferences.skill == 'attack') option += ' selected="selected"';
  option += '>Attack Strength</option><option value="defense"';
  if (boss.preferences.skill == 'defense') option += ' selected="selected"';
  option += '>Defense Strength</option><option value="energy_max"';
  if (boss.preferences.skill == 'energy_max') option += ' selected="selected"';
  option += '>Max Energy</option><option value="health_max"';
  if (boss.preferences.skill == 'health_max') option += ' selected="selected"';
  option += '>Max Health</option><option value="recovery_max"';
  if (boss.preferences.skill == 'recovery_max') option += ' selected="selected"';
...
function bank_withdraw(amount, timer){
  if (document.getElementsByName("withdraw_amount")[0] != null){
    var submit = Utils.getElementsByXPath('//input[@value="Withdraw"]')[0];
    document.getElementsByName("withdraw_amount")[0].value = amount;
    Timer.start(submit, 'Withdrawing $' + amount + ' from the bank...', 5);
  }
}
  ...
  switch (Page.c_page) {
    ...
    case 'fight':
      ...
      if (Page.c_page == 'stockpile') {
        ...
        data.push('<br />');
        data.push('Current equipment usage: ' + datadisplay_getDisplayFightItems());
      } else {
...
function datadisplay_getDisplayFightItems() {
  var items;
  var returnVal = "<br><table>";
  ...
  for (var type = 0; type < types.length; type++) {
    ...
    while (mobsters && items.toSource() != '({})') {
      ...
      returnVal += "<tr><td width='10px'/><td>" + qty + " " + itemlist[best].name
      if (qty > 1)
        returnVal += "s";
      returnVal += "</td></tr>";
      mobsters -= qty;
...
function autoCityBuy_exec(){
  if (!boss.preferences.autoCityBuy) return;
  if (Page.c_page != 'city') return;
}

function autoCityBuy_buyProperty(){
  if (Page.c_page == 'city'){
    for (var item in itemlist){
      if (itemlist[item].type != 'city') continue;
      var td = Utils.getElementsByXPath('//a[contains(@name,"item")]')[0];
    }
  }
}

function autoCityBuy_preferencesInterface(prefs){
  if (boss.preferences.autoCityBuy == undefined) boss.preferences.autoCityBuy = false;
  var div = document.createElement('div');
  div.innerHTML = '<label for="autoCityBuy">Automatically buy city with best ROR: </label>';
  var input = document.createElement('INPUT');
  input.name = 'autoCityBuyChk';
  input.type = 'checkbox';
  if (boss.preferences.autoCityBuy) input.checked = true;
  div.appendChild(input);
  div.innerHTML += '<label for="autoCityBuy"> (bought in groups of 10)</label>';
  prefs.form.insertBefore(div, prefs.button);
}

function autoCityBuy_preferencesHandler(form, boss) {
  var input = form.elements.namedItem('autoCityBuyChk');
  if (boss.preferences.autoCityBuy != input.checked) {
    boss.preferences.autoCityBuy = input.checked;
    return true;
  }
  return false;
}

function hitListUser(url, timer){
  if (window.location.href != url){
    var action = new Object();
    action.message = "Hitlisting user...";
    action.page = 'hitlist';
    action.func = 'hitListUser';
    action.params = url;
    action.time = 0;
    boss.actions.hitlistUser = action;
    boss.save();
    window.location.href = url;
  }
  else{
    var cost = document.getElementsByName("bounty")[0].value;
    if (boss.cash < cost) hitListCurrentUser();
    else {
      Utils.getElementsByXPath('//form[contains(@action,"do.php")]')[0].submit();
    }
  }
}
...
function hitListCurrentUser(){
  ...
  if (boss.actions.hitlistUser) delete boss.actions.hitlistUser;
  var cost = document.getElementsByName("bounty")[0].value;
  if (boss.cash < cost){
    if ((cost - boss.cash) > boss.bank_cash) return;
    var action = new Object();
    action.message = "Going to bank...";
    action.page = 'bank';
    action.func = 'bank_withdraw';
    action.params = cost - boss.cash;
    action.time = 0;
    boss.actions.bank_withdraw = action;
    var action2 = new Object();
    action2.message = "Hitlisting user...";
    action2.page = 'bank';
    action2.func = 'hitListUser';
    action2.params = window.location.href;
    action2.time = 1;
    boss.actions.hitlistUserNav = action2;
    boss.save();
    Timer.start("http://apps.facebook.com/mobwars/bank/", "Going to bank...", 3);
  } else hitlistUser(window.location.href, null);
}
...
if (exception) {
  ...
  GM_log(error.innerHTML);
  Timer.start(window.location.href, 'Trying to recover...', 5);
}
...
if (Page.c_page == 'hitlist'){
  if (window.location.href.indexOf('add', 0) > 0){
    var form = Utils.getElementsByXPath('//form[contains(@action,"do.php")]')[0];
    var link = document.createElement('a');
    link.addEventListener('click', hitListCurrentUser, false);
    link.innerHTML = 'Withdraw money and hitlist this user.';
