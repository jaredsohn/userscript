// ==UserScript==
// @name           4chan filter + repetition filtering
// @namespace      4chandotnet
...
<div>\
  ...
  <span>Repeated</span> <input><br>\
  <labe>No Pic Only Posts<input type="checkbox"></label><br>\
</div>\
...
function applyF() {
  ...
  files = fetch('File');
  repeat_num = parseInt(fetch_num('Repeated'));
  ...
  function fetch_num(el) {
	...
	var tempgbl = GM_getValue('global' + el);
...
function spam(el) {
  ...
  if (files) {
	...
	var post_lines = bqIH.split("<br>");
	var last_line = "DEADBEEF";
	var repeat_count = 0;
	
	if(repeat_num > 0)
		for (j in post_lines)
		{
			if(last_line == post_lines[j])
				repeat_count++;
			else
			{
				last_line = post_lines[j]
				repeat_count = 0;
			}
	...
	if(GM_getValue('No Pic Only Posts'))
	{
		temp = x(filex, el);
		if (temp && (bqTC.length == 0))
			return true;
	}
}
