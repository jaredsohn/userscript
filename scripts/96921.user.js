...
VisitFriendsList:function() {
	...
	document.location.href="http://www.facebook.com/social_graph.php?node_id="+appid+"&class=AppUserManager&edge_type=mutual&start=0"; //change suggested by Lox Myth - see http://userscripts.org/topics/34630#posts-166598
	return true;
},
	...
	this.IterateButtons(function(info) {
		...
		if(( 
			...
			if(openType=="" || friendMenu) {
				// no need to open up in new tab/frame, just click quickly
				t.ClickAutoAcceptButton(info.obj,true);
			} else {
...
AddAutoAccept:function() {
	var event=document.getElementById('mainContainer');
	if(event) {
	...
	*/
	...
	mDiv.innerHTML=
		...
		"<select id='BulkRequestsNewTab'><option value=''>Just click normally (faster)</option><option value='tab'>Open up a new tab</option><option value='iframe'>Open up in iframe (at the bottom of page)</option><option value='hidden' selected>Hidden</option></select> when clicking on something (Some applications need this to confirm that you've accepted)<br />"+
		"Only click <input size='3' id='BulkRequestsMaxClicks' /> buttons at a time, <input size='3' id='BulkRequestsDelay' />secs inbetween each click.<br />"+
...
//			var addLink=nHtml.FindByXPath(document,"	.//a[contains(@href,'/addfriend.php')]");
