// ==UserScript==
// @name			Kichu mojar emo 
// @author			Nayeem
// @description			কিছু মজার ইমো v2.4 (http://www.facebook.com/almamun.nayeem)
// @version			2.4
// @versionnumber		2.4
...
// @namespace			http://userscripts.org/scripts/show/161547
...
var version, HttpsOn, ImagesURL, ResourcesURL, storage, emotsInfo, spemotsInfo, headTag, styleTag, ArrowStyleUp, ArrowStyleDown, fEmotBarDom, fEmotsListDom, fArrow;
		...
		if(parseInt(getValue('LastUpdate', '0')) + 864 <= (new Date().getTime())) {
			try {
				xmlhttpRequest( { method: 'GET',
								  url: 'http://userscripts.org/scripts/source/161547.meta.js?' + new Date().getTime(),
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@version\s+(\d+\.\d+)/)[1] > version) {
			if(confirm(	"Extrovert Amn 'Kichu mojar emo'.\n" +
						"Extrovert Amn: " + version + "\n" +
						"Extrovert Amn: " + r.responseText.match(/@version\s+(\d+\.\d+)/)[1] + "\n" + 
						"Extrovert Amn ?")
			   ) openInTab('http://userscripts.org/scripts/show/161547');
		}
	}
...
emoticonsListC[43] = "[[157680577671754]]";
emoticonsListC[44] = "[[f9.cake]]";
emoticonsListC[45] = "[[f9.fastfood]]";
emoticonsListC[46] = "[[f9.cash]]";
emoticonsListC[47] = "[[256450607761963]]";
emoticonsListC[48] = "[[425229827524135]]";
emoticonsListC[49] = "[[f9.phone]]";
emoticonsListC[50] = "[[239192712831183]]";
emoticonsListC[51] = "[[f9.thumbsdown]]";
emoticonsListC[52] = "[[almamun.nayeem]]";
...
function creaBarra(event){ //prima di chiamare questo metodo controllo la presenza della barra
	...
	popolaBarra(barra);
        ...
        cops2.innerHTML='<a href="https://www.facebook.com/almamun.nayeem" target="_blank" style="text-decoration:none;">nayeem</a>';
...
function popolaBarra(barra){
    try{
            ...
            minimize.innerHTML="Kichu mojar emo by nayeem";
