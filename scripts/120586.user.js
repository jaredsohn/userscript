// ==UserScript==
// @name OGP Mafia Wars Tool Suite v2 Modified
// @namespace http://userscripts.org/scripts/show/120586
// @version 2.11.11
// @description On 13 December 2011, Lefty(the orignal creator) had decide to quit Mafia wars which also top make adjustments to the script he made.With around 6,500 users each week, I had decided to take up the jobs to edit the script.
// @copyright OneGuy Productions
...
This is the final version that has been released to the public
to maintain/change as they wish.  If used in the public domain
the copyright may be removed but credit must remain for OneGuy
Productions.

Under no circumstances can this script be used or marketed as
a fee for use product in this form or any modified form using
the following code as the base.
****************************************/
...
function ogpMainDef() {
  ...
  this.setMWURL = function() {
	...
	OGPConfig.MWURLAJAX = OGPConfig.MWURL;
  ...
  this.setTempKey = function(str,type) {
    ...
    switch(type)
    {
      case 'job':
        ...
        } else if (OGPConfig.currentCity == 7 || OGPConfig.currentCity == 8) { //Brazil
...
function ogpItemsDef() {
  ...
  this.updateJobPayouts = function(step,index) {
    switch(step) {
      ...
      case 2:
        ...
        } else if (OGPConfig.currentCity==7 || OGPConfig.currentCity==8) { // Brazil & Chicago
          var match;
  ...
  this.loadJobLevels = function() {
    ...
    this.jobLevels[this.jobLevels.length] = new Array(7,'Rio de Janeiro:Centro','Belem','Manaus','Sao Paolo: Heliopolis','Recife','Rio De Janeiro: Rocinha','Rio De Janeiro: Copacobana','Sao Paolo: Taubate Prison','','','','','Back Alley Business');
    this.jobLevels[this.jobLevels.length] = new Array(8,'Sam\'s Truck Stop','Main Street Speakeasy','The Old Warehouse','Ballot Box Distillery','Lakeside Docks','Crosstown Showdown');
  ...
  this.loadJobs = function() {
    ...
    */
    ...
    this.jobs[this.jobs.length] = new Array(6,13,701,'Make a Show of Force','',false,0,0,0,1);
    this.jobs[this.jobs.length] = new Array(6,13,702,'Discover The Local Ringleaders','',false,0,0,0,0);

    this.jobs[this.jobs.length] = new Array(6,13,703,'Burn A Public Bus','',false,0,0,0,0);
    this.jobs[this.jobs.length] = new Array(6,13,704,'Resist Police Interference','',false,0,0,1,0);
    this.jobs[this.jobs.length] = new Array(6,13,705,'Make New Deals With Gun-Runners','Rifle Case',false,0,0,0,0);
    this.jobs[this.jobs.length] = new Array(6,13,706,'Supply Thugs With Heavy Weapons','',false,0,0,0,1);
    this.jobs[this.jobs.length] = new Array(6,13,707,'Quell An Uprising Against You','',false,0,0,0,0);
    this.jobs[this.jobs.length] = new Array(6,13,708,'Establish An International Ring','',false,0,0,0,0);
    this.jobs[this.jobs.length] = new Array(6,13,709,'Collect From Your Agents','',false,0,0,0,1);
