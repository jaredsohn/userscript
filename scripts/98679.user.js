// ==UserScript==
// @name           YouTube Auto HD & Remove Ads (PRO)
// @description    Buffers the video without autoplaying, removes in-video ads, and puts it in hd if the option is on.
...
autoBuffer : {label:"Auto[buffer/play]", type:"select", section : ["Main Options"], options : {
																	"buffer" : "Autobuffer",
																	"play" : "Autoplay",
																	"none" : "Both off"
																  }, "default" : "buffer"},
autoHD : {label:"Auto HD", type:"select", options : {
													"240p" : "240p",
													"360p" : "360p (normal)",
													"480p" : "480p",
													"720p" : "720p (HD)",
...
if(GM_config.get("autoBuffer") == "buffer") {
	...
	function pauseOnStart() {
		//document.title = document.getElementById("movie_player").getPlayerState();
		if(document.getElementById("movie_player").getPlayerState() == 1) {
			...
			window.setTimeout(function(mp) {
				...
				//window.setTimeout(function(mp) {
					mp.pauseVideo();
					//window.setTimeout(function(mp) {
						mp.setVolume(document.getElementById("mp_volume").value);
					//}, 100, mp);
				//}, 100, mp);
			}, 2500, mp);
		}
