// ==UserScript==
// @name             dackalılar atak
// @version          0.7.6
// @namespace        dackalılar atak
// @homepage         http://userscripts.org/users/284741/scripts
// @description      Yeni Atak Stili Türçeye Çevrilmişdir.
...
var KOCAttack={
	...
	ShowViewAttacksDialog:function() {
		...
		citysel += '<option value=All>Bütün Şehirler</option>';
		...
		levelsel += '<option value="All">Bütün Seviyeler</option>';
		for(var c=1; c<=10; c++) {
		...
		typesel += '<option value="All">Hepsi</option>';
		typesel += '<option value="normal">Atak</option>';
		typesel += '<option value="transport">Taşıma</option>';
		typesel += '</select>';
		
		div.style.display='block';
		div.innerHTML='';
		var m = '<DIV id="srcAttackOpts" style="height:30px">\
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Arama: </td><TD align=left>\
		<SELECT id=srcAttack>\
		...
		<OPTION value=Mountains>Dag</option>\
		<OPTION value=Forest>Keresteler</option>\
		<OPTION value=Hills>Tepeler</option>\
		<OPTION value=Plain>Ova</option>\
		<OPTION value=City>Şehir</option>\
		</select></td>\
		<td class=xtab width=100 align=center>Şehir Seç: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Seviye Seç: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Tür Seç: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>İptal</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Atak Listesi</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Net Listesi</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Seçileni Sil</span></a>\
		<a id="KOCAttackViewAttacksIgnore" class="button20"><span>Gör/Görme</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Şehir</td><td>Koord.</td><td nowrap>Ne</td>';
		  h += '<td>Tür</td><td>Saldırı Asker</td><td>Intihar Asker/Kaynaklar</td><td>Saat</td><td>Gör</td></tr>';
		  var tableRows= '';
		  var count = 1
		  for(var a=0; a<t.attacks.length; a++) {
		    if (t.attacks[a].currenttattackwavetype == 'bulkadded') continue;
			var levelInfo=t.GetLevelInfo(t.attacks[a]);
			if (levelInfo==undefined) continue;
			var type;
			if (levelInfo.type){
			  type = levelInfo.type;
			}
      ...
      tableRows += '</td>';
			...
			tableRows += '<td><input type=checkbox name=chkIgnore id=ignore'+count+' '+ignChecked+'></td>';
		  ...
		  for (var i=1; i<count; i++){
		    var ignore = 'ignore'+i;
		    ById(ignore).addEventListener('click',t.ChangeIgnore,false);
		  }	
		  
		  
//              ById('KOCAttackViewAttacksIgnore').addEventListener('click',function() {
//		    GM_log('Count: '+count);
//			for (var i=1; i<count; i++){
//			  var ignore = 'ignore'+i;
//			  if (ById(ignore).checked == true){
//			    ById(row).checked == false;
//			    var c = ById('coords'+i).innerHTML;
//			    var xy = c.split(",");
//			    t.attacks[i].ignore = true;
//			    //WinLog.write(inspect(t.attacks[i],10));
//			    GM_log('X,Y '+xy[0]+','+xy[1]);
//			    t.SetAttack(xy[0],xy[1],t.attacks[i]);
//			    var listBtn=ById('KOCAttackViewAttacksList');
//			    nHtml.Click(listBtn);
//			  }
//			}
//		  },false);		  
	...
	ChangeIgnore:function(){
	  GM_log('Clicked'+this.checked);
	},	
	
	GetCityName:function(cityid) {  
	  var cityName;
	  for(var a=0; a<Seed.cities.length; a++) {
	    if(Seed.cities[a][0] == cityid){
		  var cityName = Seed.cities[a][1];
		}
	  }
	  return cityName;
	},	
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Saldırı Sıralama: <select id='KOCAttackOrder'><option value='mostTroops'>İlk çoğu asker kullanın</option><option value='closest'>En Yakın İlk Hedefler</option></select><br />"+
			"Atak Tipi: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbar "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Bozkır "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Şehir "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Taşıma<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> Saniyede Bir Atak Gönder<br />"+
			"Aynı Hedefe Gönderme </u>Zaman Ayarı<br />"+
			"<div style='margin-left: 40px'>"+
			"Barbar Atak:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />Saat<br />"+
			"Bozkır Atak:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />Saat<br />"+
			" Şehir Atak:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />Saat<br />"+
			" Taşıma:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />Dakika"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> Uzak Şehirden Maksimum Mesafede Saldırı.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Sadece onlar ilk gelen başlatıldı kenti saldırılar.<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Bilinmeyen / Aşırı Trafik Hatası (Bu Seçeneği Değiştirdikten Sonra Varsa Dene Yürüyüşü Yeniden Basın).<br />"+
			"Biz Saldırı Altındayken  Bir Sekmede Bu Url Konumunu Aç ...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Otomatik Barbar Ve Bozkır Raporlarını SiL.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Saldırı Raporları, Saldırıda İletişimi Maksimum Tutmak İçin.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Devre dışı bırakmak için bu da devre dışı saldırı sayfa için rapor toplama olacak, raporlar görüntüleme.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% Mutluluk oLunca Otomatik Egitim Yapmak<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% Mutluluk oLunca Otomatik Altın Toplamak<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> Rahatsız Edici \"Arkadaşlarını davet et \" Penceresi Devre Dışı  .<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Otomatik Olarak İttifak Üyelerine Bina / Araştırma iLe Yardım Eder.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" />Sohbetde İttifak Yardım İstekleri ve Raporları Gizlemek  (Yukardakini İşaretlerdsen Önce Yardım Eder Sonra Kaparnır.).<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Otomatik Olarak Facebook Duvar Oyun İsteklerini Yayınlayın. (Yardım Talepleri Gibi) <br />"+
			"Yukarıda işaretlerseniz, ne gizlilik ayarını biz kullanmalıyım? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Herkes</option><option value='50'>Arkadaşlar ve Arkadaşları</option><option value='40'>Sadece Arkadaşlarım</option><option value='10'>Sadece Ben</option></select><br />"+
			"</td></tr>"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Otomatik olarak etki alanına oturum halinde kesilir bakım veya aşağı-zaman sunucusuna bağlı.<br />"+
			"<input id='KOCAttackUseAlternateReload' type='checkbox' "+(this.options.useAlternateReloadMethod?'checked':'')+" /> Alternatif bir yöntem yeniden kullanın (Eger Hata Mesajı Alıyosan Yeniden Başlatılması Gerekli.).<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Etkinleştirmek tanı Firefox hata (eğer bir hata raporu ile birlikte bilgi gönderiyorsunuz veya bir sorun hata ayıklama için çalışıyor eğer yararlı)<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> Şehirleri Seç<br />"+
			"Tüm şehirler arasında geçiş <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> kez ve daha sonra bekleyin "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />saniye önce Yenile.<br />"+
			"Saldırı Yapılcak Şehirler: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% Tüm gecikmelerden için rasgele ayarı (to look more human).<br />"+
			"İthalat / İhracat: Yapıştır veya burada ayarlarını kopyalamak ...<br /><textarea id='KOCAttackImport'></textarea>"+
			"<a class='button20' id='KOCAttackImportButton'><span>İthalat</span></a> <a class='button20' id='KOCAttackExportButton'><span>Ihracat</span></a><br />"+
			"</td></tr>"+

			"</table>"+
			
			"<a id='KOCAttackOptionsSave' class='button20'><span>Kaydet</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>İptal</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Ayarları Sıfırla</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Hepsini Sıfırla</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Bütün Saldırıları Sil</span></a><a id='KOCAttackDeleteTransports' class='button20'><span>Taşımaları Sil</span></a><a id='KOCAttackListAttacks' class='button20'><span>Atak Listesi</span></a><a id='KOCAttackListTransports' class='button20'><span> Liste Taşı</span></a></form>";
		var t=this;
	...
	AddViewAttacksLink:function() {
		...
		a=this.AddTabLink('Ataklara Bak');
		if(!a) {
			window.setTimeout(function() {
	...
	*/			
	...
	SetAttackStatusMessage:function() {
		...
		if(!toggle) {
			var t=this;
			toggle=this.AddTabLink('Oto Atak');
			if(!toggle) {
				window.setTimeout(function() {
					t.SetAttackStatusMessage();
				},t.GetRandTime(250));
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>Oto Atak - Açık</span>';
		} else {
			toggle.innerHTML='<span>Oto Atak - Kapalı</span>';
		}
	},
	...
	*/	
	...
	ClearOptions:function() {
		this.SetValue('Ayarlar',JSON.stringify({}));
	},
	GetOptions:function() {
		var json=this.GetValue('Ayarlar','{}');
		if(json=='') json='{}';
	...
	SetOptions:function(v) {
		this.SetValue('Ayarlar',JSON2.stringify(v));
	},
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"Buraya kopyala yapıştır Coords (yani 343.434) her satırda bir ...<br />Not: Yalnızca bu ekranda birliklerinin mevcut numarasını kullanarak hedef ekleyeceğiz.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Üzerine mevcut saldırı biri zaten varsa<br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Yerlerde Türü (Tüm bu tip eşleşmelidir koordinatları):<br />");
		var arrData = [["Camp", "Barbar"],["Wilderness", "Bozkır"]];
		...
		AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Bu bir Bozkır üzerinde tuzakları temizlemek için bir ilk intihar dalgadır.<br />");
		...
		bulkAdd.innerHTML='<span>Hepsini Ekle</span>';
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert('|DiJiTaL| '+added+' Adet Koordinat Eklendi.!');
		},false);
		AddHtml(bulkAddDiv,"<br />");

		var bulkAddAttackLink=document.createElement('a');
		bulkAddAttackLink.className='buttonDown20';
		bulkAddAttackLink.innerHTML='<span>Toplu Kord. Ekle</span>';
	...
	HideAttackEfforts:function() {
		...
		var a=document.createElement('a');
		a.innerHTML=' Saldırı Çablarını Göster/Gizle';
		//a.href='javascript:;';
	...
	OnCastleBoxAppear:function(box) {
		...
		function SetAutoGoldA() {
			var str=t.GetAutoGold()?'Otomatik - Açık':'Otomatik - Kapalı';
		...
		function SetAbandonWildsA() {
			var str=t.GetAbandonWilds()?'Bozkırları Terket - Açık':'Bozkırları Terket - Kapalı';
	...
	OnAttackBoxAppear:function(box) {
		...
		AddText(div2,'Atak Listesine Ekleme');
	...
	*/
		...
		for(var s=0; s<ss.snapshotLength; s++) {
			...
			if(onclick) {
				...
				if(onclickM && onclickM[6]!='"null"' && onclickM[12]!='"city"') {
					...
					owner=" "+onclickM[6].StripQuotes()+'<br />Kudret:'+might;
				}
			}
	...
	DrawClosestFarms:function() {
		...
		if(!div) {
			...
			expandA.innerHTML='B';
			expandA.title='Bilgiyi Genişlet';
	...
	GetResourcesSize:function() {  
		...
		var cityID = this.GetCurrentCityId();
 		resources[0] = parseInt(Seed.citystats['city'+cityID].gold[0]);
 		resources[1] = parseInt(Seed.resources['city'+cityID]['rec'+1][0] / 3600);
 		resources[2] = parseInt(Seed.resources['city'+cityID]['rec'+2][0] / 3600); 		
		resources[3] = parseInt(Seed.resources['city'+cityID]['rec'+3][0] / 3600);
 		resources[4] = parseInt(Seed.resources['city'+cityID]['rec'+4][0] / 3600);
	...
	*/
	...
	ListAttacks:function() {
		...
		DetailAttacks:function() {
		  ...
		  if(currentServerId<0){
			currentServerId = GM_getValue("KOCAttackLastKnownServerID",-1);
		  }
		  ...
		  for(var n=0; n<names.length; n++) {
			...
			//WinLog.write (inspect(attacks,10));
			//WinLog.write ("Hello");
		  }
		},
	...
	SetupMenu:function() {
		var t=this;
		GM_registerMenuCommand('KOCAttack - Ayarlar',function() {
			t.ShowOptionsDialog();
		});
		GM_registerMenuCommand('KOCAttack - Ataklara Bak',function() {
			t.ShowViewAttacksDialog();
		});		
		GM_registerMenuCommand('KOCAttack - Otomatik Atak',function() {
			t.ToggleAutoAttack();
		});
		GM_registerMenuCommand('KOCAttack - Fabrika Reset!',function() {
			t.FactoryReset();
		});
	},
