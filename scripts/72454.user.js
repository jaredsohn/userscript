...
function InsertCheaterTable() {
	...
	GM_xmlhttpRequest({method: 'GET', url: DOWNFIGHTDE_CHEATERPAGE_URL, onload: function(responseDetails) {
		...
		if (responseDetails.status == 200) {
			...
			if (TOWNEXTENSION == "B") {
				var currentdiv = 0;
			// sonst: Es wird in Hamburg oder München gespielt
			} else {
				var currentdiv = 1;
			}
			...
			var nrofcheaters = dftrs.length - 1;
			// Stadtnamen ermitteln
			switch (TOWNEXTENSION) {
				case "B": var townname = "Berlin"; break;
				case "HH": var townname = "Hamburg"; break;
				case "M": var townname = "München"; break;
			}
			...
			for (var i = 1; i <= dftrs.length - 1; i++) {
				// UserID und Username ermitteln
				var userid = GetUserIDFromDFDE(dftrs[i].getElementsByTagName("td")[5].innerHTML);
				...
				newtr.getElementsByTagName("td")[1].innerHTML = GetIconInsertHTML(userid, username, "");
				// NAME
				newtr.getElementsByTagName("td")[2].innerHTML = GetProfileLink(username, userid);
				// BANDE
			...
			for (var x = 1; x <= newtable.getElementsByTagName("tr").length - 1; x++) {
				...
				// UserID des aktuellen Benutzers ermitteln
				var userid = GetUserIDFromRow(newtable.getElementsByTagName("tr")[x]);
				...
				GM_xmlhttpRequest({method: 'GET', url: API_URL + userid + ".xml", onload: function(responseDetails) {
					...
					var currenttr = document.getElementById("cheaterid" + userid);
