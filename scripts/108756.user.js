...
// @name           FWZ: Streak Marker

// @description    Streakmarker : FIXED BECAUSE QUANG ISN'T GOING TO
...
function processStreak(text, dom) {
	...
	/* GOD DAMN IT QUANG YOU DO NOT DO A FOR EACH FOR NON OBJECT TYPE JAVASCRIPT ITERABLES YOU DUBMASS */
	...
	for (var table = 0; table < tables.length; table++) {
		...
		for (var forum = 0; forum < forums.length; forum++) {
			...
			if (streak){
				...
				//forums[forum].prototype.onclick = function(){ return confirm('This forum is already in your streak! Do you still want to pwn it?');};
			}
...
function append(forum, text, color) {
	...
	var element = document.createElement("span");
        element.style = "margin: 0 0 0 1em; color: " + color;
        element.innerHTML = text;

	if (forum.tagName.toLowerCase() == "a") {
		forum.parentNode.insertBefore(element, forum.nextSibling);
	}else{

		forum.insertBefore(element, forum.nextSibling);
	}
