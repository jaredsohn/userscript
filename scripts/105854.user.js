// ==UserScript==
// @name           Spiral Knights Forum Improvements
// @namespace      http://estragib.googlepages.com/custom
// @url            http://userscripts.org/scripts/source/105854.user.js
// @description    CSS and JavaScript improvements for the Spiral Knights Forum. Also adds an Ignore List.
...
// XPATHS
const ID_CONTENT_WRAPPER =    'drupalcontent-area';
const XPATH_TABLE_BODY =      'id("' + ID_LIST + '")/table/tbody';
const XPATH_TOPICS =          'id("' + ID_CONTENT_WRAPPER + '")//td[@class="created"]';
const XPATH_COMMENTERS =      'id("' + ID_CONTENT_WRAPPER + '")//div[contains(@class, "comment")]//div[contains(@class, "author-name")]';
...
// env
var context = /\/(forum|node)\/\d*/i.exec(window.location.href);

// main
if(context) {
	var mode = context[1].toLowerCase(); // either 's' or 't' for section or topic page
	
	fixGoogleChromeGreaseMonkeyAPI();
	insertMissingStyles();
	executeIgnore();
}

// Greasemonkey for Google Chrome doesn't support GM_getValue and GM_setValue - route these to localStorage isntead.
// From http://devign.me/greasemonkey-gm_getvaluegm_setvalue-functions-for-google-chrome/
function fixGoogleChromeGreaseMonkeyAPI() {
...
// Fix CSS on the site
function insertMissingStyles() {
	var styles = [];

    // The forum posts no longer contain footer links. Going to leave this here in case I need to add custom styles in the future.
	// styles.push('div.forum-post-footer a, div.forum-post-footer a:hover { color: #000C3D !important; }');
...
// Based on OkC Forum Ignore List from http://userscripts.org/scripts/show/33443
...
function insertStyles() {
	...
	styles.push('.' + CLASS_BLIND + ' { text-align: center; font-size: smaller; font-style: italic; opacity: .7; }');
	...
	if(mode == 'forum') {
		...
		styles.push('tr[' + ATTR_DIMMED + '] { opacity: ' + DIM_OPACITY + ';  }');
		styles.push('tr[' + ATTR_DIMMED + '] td { border-color: #192a38; }');
		styles.push('tr[' + ATTR_DIMMED + ']:hover { opacity: ' + DIM_OPACITY_HOVER + '; }');
		styles.push('tr[' + ATTR_DIMMED + ']:hover td { border-color: #466288; }');

	    styles.push('td.' + CLASS_BLIND + ' { display: table-cell !important; }');
	}
	else if(mode == 'node') {
	    styles.push('div[' + ATTR_HIDDEN + '="' + ATTR_VAL_HIDDEN_HARD + '"] { display: none !important; }');
	    styles.push('div[' + ATTR_HIDDEN + '="' + ATTR_VAL_HIDDEN_SOFT + '"] { border: none; }');
	    styles.push('div[' + ATTR_HIDDEN + '="' + ATTR_VAL_HIDDEN_SOFT + '"]>div { display: none; }');
	    styles.push('div[' + ATTR_HIDDEN + '="' + ATTR_VAL_HIDDEN_SOFT + '"] div.' + CLASS_BLIND + ' { display: block; }');

		styles.push('div[' + ATTR_DIMMED + '] { opacity: ' + DIM_OPACITY + '; }');
		styles.push('div[' + ATTR_DIMMED + ']:hover { opacity: ' + DIM_OPACITY_HOVER + '; }');
	}
...
function check() {
	...
	if(mode == 'forum')      userLinks = $x(XPATH_TOPICS);
	else if(mode == 'node') userLinks = $x(XPATH_COMMENTERS);
	...
	for (var j = 0; j < userLinks.length; j++) {
	    ...
	    for (var k = 0; k < ignoreList_.length; k++) {
	        var entry = ignoreList_[k];
			var opacity = 1;
		    ...
		    var entryText = entry.trim().toLowerCase();
			
        	if(linkText == entryText || linkText.indexOf("by " + entryText) === 0) hide(entryText, link, opacity);
		}
	}
}
...
function getRowFromLink(link) {
    if(mode == 'forum') {
        ...
        return link.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;
    }
}
...
// 	blind: opacity is 1
function hide(username, userLink, opacity) {
	var row = getRowFromLink(userLink);
	...
	if(opacity == 0) {
		...
		if(opacity != 1)
		    ...
		    // Row ids are not unique in this forum software, create our own.
            var rel = username + '_' + Math.floor(Math.random() * 100000).toString();
        	...
        	if(mode == 'forum') {
			    ...
			    if(!blind) {
    			    ...
    			    blind.colSpan = '6'; // 6 cells per row in sections
    			...
    			var blind = $x('div[@class="' + CLASS_BLIND + '"]', row)[0];
			    if(!blind) {
...
function restore(row) {
	if(row.hasAttribute(ATTR_HIDDEN)) {
		if(row.getAttribute(ATTR_HIDDEN) == ATTR_VAL_HIDDEN_SOFT) {
			var blind;
			if(mode == 'forum') {
			    blind = $x('td[@class="' + CLASS_BLIND + '"]', row)[0];
			} else {
			    blind = $x('div[@class="' + CLASS_BLIND + '"]', row)[0];
			}
...
function restoreAll() {
	...
	for (var j = 0; j < userLinks.length; j++) {
		...
		var row = userLink.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode; // a -> p/div -> td -> tr
...
function refreshIgnoreList() {
	...
	if(storedEntries.length) ignoreList_ = storedEntries.split(LIST_SEPARATOR);
	else                     ignoreList_ = [];
}
...
// aux functions
function addInputCell(row, type, cssClass, name, value) {
		cell = document.createElement('td');
