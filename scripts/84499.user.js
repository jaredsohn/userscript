...
// @author        Athy, edited by NT to fit 20048x1152 resolution (widescreen)
...
//click and drag handles
  	...
  	var infoGrow 		= 550;		//pixels to increase the info window starting height by (negative shrinks)
  	var infoWidth 		= 631;		//width of the info box in pixels (486 is default)
  	var infoHpos 		= 4;			//horizontal offset of the info from it's original position
  	var infoVpos 		= 35;			//vertical offset of the info from it's original position

	var tradeVpos		= 190;		//trade box vertical
	var tradeHpos		= 690;		//trade box horizontal

  	var chatGrow		= 754;		//pixels to increase the chat starting height by (negative shrinks)
  	var chatWidth		 = 775;		//width of the whole chat in pixels (including toon list)
  	var toonWidth	= 165;		//width of the toon list (i.e: text area = chatWidth - toonWidth)
  	var chatHpos 		= 661;		//horizontal offset of the chat from it's original position
  	var chatVpos 		= 4;			//vertical offset of the chat from it's original position

  	var invGrow 		= 375;		//pixels to increase the inventory starting height by (negative shrinks)
	var invWidth 		= 625;		//width of the inv box in pixels (486 is default)
  	var muleGrow 	= 375;		//pixels to increase the mule starting height by (negative shrinks)
  	var muleWidth 	= 625;		//width of the mule box in pixels (486 is default)
  	var storageGrow = 375;		//pixels to increase the storage starting height by (negative shrinks)
  	var storageWidth = 625;		//width of the storage box in pixels (486 is default)
...
//	document.getElementById('imgToggleInv').style.display="none";	document.getElementById('imgToggleMule').style.display="none";	document.getElementById('imgToggleStorage').style.display="none";
	...
	//i think this removes the game links and referal links
...
function changeUI(){
	...
	chatToggle("Main");// main off cuz it starts on by default
...
function loadT(){		location.href = "javascript:ShowChat('PublicTrade');";	setTimeout(loadToggles,1000);	}
...
function loadM(){	location.href = "javascript:ShowChat('Main');";			setTimeout(loadL, 1000);	}

setTimeout( loadM, 2000);
