...
{
    // Remove the top adbar and the ads on right side
    ...
    for (i=0;i<oTd.length;i++)
    {
        ...
        {            
			if (oTd[i].width == 164)
            {
	            oTd[i].parentNode.removeChild(oTd[i]);
            }
        }
    }
	...
	// Remove tweetme button
	if(window.location.href.substring(0,46) == 'http://www.thinkdigit.com/forum/showthread.php')
	{
		for (z=0;z<allIframe.length;z++) 
		{        
			if(allIframe[z].getAttribute("src").substring(0,24)=='http://api.tweetmeme.com')
			{
				allIframe[z].parentNode.removeChild(allIframe[z]);
			}
		}
	}
}
...
{	
	...
	searchTD.innerHTML='<form method="get" action="http://www.google.com/search"><input id="CustomSearchBox" onfocus="if (this.value == \'NeatDigit Ver 2.65\') {this.value=\'\';}" onblur="if (this.value == \'\') { this.value=\'NeatDigit Ver 2.65\'; }" type="text" style="color:#A4A4A4;" name="q" size="20" maxlength="255" value="NeatDigit Ver 2.65" />&nbsp;<input type="submit" style="background-color:#F2F2F2;border-color:#BDBDBD;border-style:solid;border-width:1px;" value="Search" />&nbsp;<input type="radio" style="display:none" name="sitesearch" value="thinkdigit.com/forum" checked /></form>';	
