...
function processProfilePage() {
  ...
  // 	    elmMore = '.uiMorePagerPrimary',
	...
	var elmStream = '#profile_minifeed',
      ...
      elmClBtn = '.uiSelectorChevronOnly',
	    elmPosts = './/*[@class="uiStreamSource"]',
	    elmMore = './/div[@class="pam uiBoxLightblue uiMorePagerPrimary"]',
	...
	function postDate(post) {
		if (typeof post === 'string') {
			...
			if (post === 'first') { 
				post = elms.snapshotItem(0);
				debugMsg('***first!**** \n'); 
				}
			if (post === 'last') { 
				post = elms.snapshotItem(elms.snapshotLength - 1);
				debugMsg('***last!**** \n');
				}
		}
		if ($(elmTime, post).snapshotItem(0) != null) { 
			return $(elmTime, post).snapshotItem(0).value * 1000; // TEST
		}
		else {
			debugMsg('**** NULL!!! ****\n');
			return -1;
	...
	function postsFirstOld(elms) {
		...
		for (ii=0; ii<elms_cnt; ii++) { // TESTES

			debugMsg('***Getting story time!***\n');
			tStory.setTime(postDate(elms.snapshotItem(ii)));
			if (postDate(elms.snapshotItem(ii)) != -1) {
	...
	function postsRemove(elms, from) {
      ...
      if (!(clbtn.getAttribute && clbtn.getAttribute('ajaxify') && clbtn.getAttribute('ajaxify').match(/story_type=(\d+)/i))){
        debugMsg("Skipping Element " +elms.snapshotItem(ii).innerHTML);
        continue;
      }
      debugMsg(clbtn.getAttribute('ajaxify'));
			...
			if (do_remove) {
				...
				xhr(window.location.protocol + '//www.facebook.com/ajax/minifeed.php?__a=1', '',
...
function aprProcess(autorun) {
	...
	var user_profile_new = $('//a[contains(@class,\'fbTimelineViewingSelf\')]').snapshotLength;
  if (!user_profile_new) user_profile_new = $('//a[contains(@class,\'edit_profilepicture\')]').snapshotLength;

	if (user_profile_new) {
		debugMsg('current page is user profile (new)');
		window.setTimeout(processProfilePage,5000);
		return;
	}
