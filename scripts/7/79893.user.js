...
var appConf = {
    fullServerName:"www.travian.com",
    gServer:"www_com",
    lang:'en',
    autoLogin:true,
    scriptPause:false,
...
function getCrtServer() {crtPage.search(/http:\/\/(.*)\//); appConf.fullServerName = RegExp.$1; appConf.gServer = appConf.fullServerName.replace(/\.travian\./,'_'); return;};
...
function doLogin() {
    ...
    if(loginBtn != null) {	
        ...
        if (refreshPage=='dorf1.php') {GM_setValue(appConf.gServer+'_RefreshPage','none'); refreshTime = 1000; }
        else if (refreshPage=='dorf2.php') {GM_setValue(appConf.gServer+'_RefreshPage','dorf1.php'); refreshTime = 1000; }
	    ...
	    if (appConf.userInfo.capVid != '') {
		    ...
		    if (villages[actV] === undefined)
		        villages[actV] = {Name:appConf.userInfo.capName,Newdid:appConf.userInfo.capNewdid,x:xy[0],y:xy[1],
		                          Link:'dorf1.php?newdid=' + appConf.userInfo.capNewdid,rType:2,isCap:true,BiP:[],lastUpBId:"",
		                          Ress:{curr:[0,0,0,0],capa:[0,0,0,0],pphr:[0,0,0,0],cost:0,corp:0,upTime:0}};
            else {
                ...
                villages[actV].Link = 'dorf1.php?newdid=' + appConf.userInfo.capNewdid;
...
function getSingleTownNewdid() {
	ajaxRequest("/dorf3.php", 'GET', null, function(ajaxResp) {
		if (ajaxResp) {
		    var ad = ajax2DIV(ajaxResp);
		    var aLnk = $xf("//a[contains(@href,'dorf1.php?newdid=') and text()='"+appConf.userInfo.capName+"']",'f',ad);
			aLnk.href.search(/dorf1.php\?newdid=(\d+)/);
...
function getBuildUrlOrSetTime(vId,tType,bId) {
	...
	if (gbUrl.readyState == 4) {
		if (gbUrl.status == 200) {
			...
			if (isLoginPage(ad)) location.href = "/dorf1.php";
...
function processAutoUpgrade(v) {
    ...
    var url = "/dorf1.php?newdid="+villages[v].Newdid;
...
//                    log(0,"Res Ind:"+bId+",Res Id:"+gId+",to Lvl:"+(tResLevel+1)+",Corp First:"+corpFirst);
			...
			if (targetTask!=null) {
			    ...
			    if (allRess.snapshotLength > 0) {
    				...
    				//location.href = 'dorf1.php?newdid='+villages[v].Newdid;
	    		}
			}
            setValue("tasks",tasks);
            ad = null;
	...
	doingTask = false;
	if (v!=actV) {
        var url = "/dorf1.php?newdid="+villages[actV].Newdid;
	...
	if (bUrl) {
		...
		GM_xmlhttpRequest({
			...
			onload: function(){
				...
				setValue("villages",villages);
				if (appConf.refreshAfterDone) location.href = 'dorf1.php?newdid='+villages[vId].Newdid;
...
function startProcess() {
    ...
    // Create page link
	if (crtPage.match(/dorf1\.php/)) processUrlDorf1();
