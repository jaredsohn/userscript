// ==UserScript==
// @name 	THE Mexico Kid: Download Youtube Videos, Dailymotion, Megavideo, Metacafe, Google, Yahoo, Spike, Myspace, Facebook, Veoh, Break, Current, Redtube videos and more!
// @namespace 	http://userscripts.org/users/159568
...
function checkForUpdate(a) {
	...
	if (a || !lastCheck || lastCheck != today) {
		GM_xmlhttpRequest({
			method: "GET",
			url: 'http://userscripts.org/scripts/source/159568.user.js',
			onload: function(results) {
			var global_version = results.responseText.match(/version[ ]*([0-9.]+)/i)[1];
				if (global_version.length && global_version != local_version) {
					if (confirm('[ Greasemonkey ] Save Tube : Version '+ global_version +' is now available. Update?')) {
						GM_openInTab('http://userscripts.org/scripts/show/159568');
					}
				}
				else if (a) {
...
function addDownloadLink(){
	var link =' [ <a href="' + dl_url + '" title="Download with Save Tube!"><img src="' + saveTubeIcon + '" alt="" valign="middle"/></a> ]';
...
function addDownloadBox() {
	var styles = [
		'#saveTubeBox {position: fixed; right: 5px; bottom: 5px; z-index: 1000;opacity: 0.8;}',
		'#saveTubeBox a {font-size:11px;font-family:Verdana;font-weight:bold;color:#008C00 !important;text-align:center;outline:none;background-color: #DFF1FD;border:1px solid #B6D9EE;padding:4px;display:block;text-decoration:none;}',
		'#saveTubeBox a:hover {border:1px solid #AE150E;background-color:#CE1A10;color:#FFFFFF !important;text-decoration:none;}',
		'#saveTubeBox img, #keepTubeBox a:hover img {background:none;margin:0px;padding:0px;border:none;vertical-align:middle}'
	];
	...
	downloadBox.id = 'saveTubeBox';
	downloadBox.innerHTML = '<a title="Download" target="_blank" href="' + dl_url + '"><img src="' + saveTubeIcon + '" width="16" height="16" /> Download</a>';
}