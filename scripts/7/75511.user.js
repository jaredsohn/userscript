// ==UserScript==
// @name         Entrainements+
// @namespace    das_bazie modifié LeonBuzz
// @description  Affichage des entrainements en permanence pour berlin, hamburg, paris, marseille et l'espagne
// @version		 1.5
...
addGlobalStyle('div#main_container { position:absolute; top:100px; left:50%; margin-left:15px; width:225px;}')
// textkopf
...
addGlobalStyle('.inhalt_container { padding-bottom:5px; padding-left:6px; background: url(http://i45.tinypic.com/3136tg4.png); background-position:-10px -5px; -moz-border-radius:10px; color:#000000; font-size:12px; text-align:left; } ')
	...
	url: ""+gamelink+"/skills/",
		onload: function(response) {
			var content = response.responseText;
				try
				{
				var niveau2 = content.split(/<\/span> \[(Niveau |Stufe )/)[2].split(']')[0];
				var niveau1 = content.split(/<\/span> \[(Niveau |Stufe )/)[4].split(']')[0];
				...
				var date_wait = content.split('<span class="style_skill">')[4].split('</span></div>')[0].replace(', ',' à ').replace(/([0-9]{2} )/,'20$1');
				
				var container_2_zeile_1 = "<!--<p class=\"Head\" style=\"padding-bottom:5px;\">Formation en attente:</p>-->";
				var container_2_zeile_2 = "<p style=\"padding-top:5px; color:#00004f;\">◘ [en attente] <b>"+w_end_wait+" ("+niveau1+")</b></p>";
				...
				var weiterbildung_0_2 = "<div class=\"prozentbalken_bg_1\"><div class=\"prozentbalken_1\" id=\"active_prozent_1\" style=\"width:"+width_balken0+"px;>"+percger0+"&#37;</div></div>";
					...
					var weiterbildung_0_2 = "<div class=\"prozentbalken_bg_1\"><div class=\"prozentbalken_1\" id=\"active_prozent_1\" style=\"width:"+width_balken+"px;>"+percger+"&#37;</div></div>";
						...
						var weiterbildung_0_1 = "<span style=\"color:#4f0000;\">entraines toi...</span>";
						var weiterbildung_0_2 = "<span style=\"color:#4f0000;\">Du Penner... "+smilie+"</span>";
						}
						}
...
GM_xmlhttpRequest({
  ...
  url: ""+gamelink+"/overview/",
      ...
      var content = response.responseText;
		...
		// menue bilden
		Weiterbildung.innerHTML = "<div id=\"main_container\" >"+inhalt_container_2+"<div class=\"inhalt_container\"><!--<p class=\"Head\" style=\"padding-bottom:8px;\">Formation en cours :</p>--><p style=\"padding-top:5px;\"><span style=\"color:#004f00;\"><b>► "+w_type1+" ("+niveau2+")</b></span></p>"+weiterbildung_0_1+"<br/>"+weiterbildung_0_2+"</div></div>";
