// ==UserScript==
// @name           kov liberator
// @namespace      http://userscripts.org/scripts/show/76866
// @description    Fjern kov's irriterende indhold.
// @include        http://*.komogvind.dk/*
// @include        http://komogvind.dk/*
...
GM_config.init('Configurable Options Script', {
	'removeAds': {
		'section': ['Reklamer', 'Fjern irriterende reklamer'],
		'label': 'Fjern reklamer',
		'type': 'checkbox',
		'default': true
	},
	'removeRedundant': {
		'section': ['Fyld', 'Fjern ligegyldige elementer'],
		'label': 'Fjern ligegyldige elementer',
		'type': 'checkbox',
		'default': true
	},
	'removeVipGames': {
		'section': ['VIP', 'Fjern VIP elementer'],
		'label': 'Fjern VIP spil fra lister',
		'type': 'checkbox',
		'default': true
	},
	'removeGift': {
		'section': ['"Gave"', 'Fjern "gave" før spilstart'],
		'label': 'Fjern "gave" fra KOV før spilstart',
		'type': 'checkbox',
		'default': true
	},
});

GM_registerMenuCommand("KOV adfree configuration", function(){GM_config.open();});

// "gave"
if (GM_config.get('removeGift')) {
	if(document.getElementById('surveyFrame')) {
		document.location.reload();
	}
}

// ligegyldige elementer
if (GM_config.get('removeRedundant')) {
	removeElements(new Array('chat', 'topBar', 'jackpotContainer'));
	if (document.getElementById('playbuttonWrapper')) {
		document.getElementById('playbuttonWrapper').childNodes[2].href = document.getElementById('playbuttonWrapper').childNodes[2].href.replace(/(.*),([0-9]{3,4}),([0-9]{3,4})\);$/g, "$1,($2-262),($3-95));");
	}
}

// reklamer
if (GM_config.get('removeAds')) {
	removeElements(new Array('createVip', 'alphaDiv', 'campaigns', 'notification2Lines', 'skyscraber', 'add_top', 'advertisements', 'becomeVip')); 
}

if (GM_config.get('removeVipGames')) {
	if (document.getElementById('ThemedBox4ContentWrapper')) {
		var games = document.getElementById('ThemedBox4ContentWrapper').getElementsByTagName('div');

		for (i in games) {	
			if (games[i].className == "gameRow glassbarTrigger") {
				if (games[i].childNodes[1].childNodes[5].childNodes.length > 0) {
	...
	if (document.getElementById('gameIndexPage')) {
		var games = document.getElementById('gameIndexPage').getElementsByTagName('td');

		for (i in games) {	
			if (games[i].className == "gameImage") {
				if (games[i].childNodes[1].childNodes.length > 1) {
