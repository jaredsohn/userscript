...
var cuba_collection ='<option disabled style="color:rgb(255, 217, 39);"><< CUBAN COLLECTIONS >>'+
			...
			'<option value="Collection" style="color:#33FF00;"><< Lotto Collection >>'+
			'<option value=221106>Bingo Card'+
			'<option value=221107>Deck OF cards'+
			'<option value=221108>Dice'+
			'<option value=221109>Wheel'+
			'<option value=221110>Slot'+
			'<option value=221111>Craps'+
			'<option value=221112>Shoe'+
...
var ny_loot='<option style="color:#33FF00;" DISABLED>-- CHOP SHOP SET --'+
		...
		'<option style="color:#33FF00;" DISABLED>-- WEAPONS DEPOT SET --'+
...
*/
    ...
    '<tr>'+
     ...
     '<table width="750" class="sexy_table1">'+
      ...
      '<tr>'+
        '<td width="70%" height="45" style="font-size:20px; padding-left:15px">Chucker v3.0</td>'+
...
//				eval(/groups_levels\s=\s([^;]+)/.exec(qty_response)[0]+';');
				...
				if(first_time==true){
					receiver_user_id=/groups_levels\s=\s{(.*)};/.exec(qty_response)[1];
                    receiver_user_id = receiver_user_id.replace(';','');
                    receiver_user_id = receiver_user_id.split(',');
...
function update_sw_sig(){
	...
	try{
		...
		var sig_url='http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=item&xw_action=view&xw_city=1&xw_client_id=8&cb='+cb[1]+'&ajax=1&liteload=1&sf_xw_user_id='+sender_profile_id+'&sf_xw_sig='+local_xw_sig;
...
try{
//                document.getElementById('popup_fodder').innerHTML = /"message_body">(.*)<\/td>/.exec(response);
//                alert(/You gave (some|an|a)(.+) to (.+)(script|td)/i.exec(response));
//                $('#inner_page').html(response);
				gift_text=/You gave (some|an|a)(.+) to (.+).(<script>|<\/td>)/i.exec(response);
//				alert(gift_text);
    			hangup_flag='false';
    			hangup_tries=0;
...
//				document.getElementById('poop_up').innerHTML='<div class="sexy_destination1">An excellent source for all the mafia wars tips and tricks that you\'ll need to know to play the game. Contains various guides and troubleshooting tips too.</div>';
			...
			case "Updates":
				...
				'<tr><td style="color:white; height:25px"><li>New Easter Egg items added</li></td></tr>'+
				'<tr><td style="color:white; height:25px"><li>New Chop shop Cars and items added</li></td></tr>'+
				'<tr><td style="color:white; height:25px"><li>Added Weapons Depot Items</li></td></tr>'+
				'</table></div>';
				break;
//			case "Donors":
