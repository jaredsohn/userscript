...
// @version      1.2.5 Anpassungen f2xuür München
...
function CheckPastFights(userid, username) {
	...
	GM_xmlhttpRequest({method: 'GET', url: FIGHTSEARCH_URL + username, onload: function(responseDetails) {
		...
		if (responseDetails.status == 200) {
			...
			// **********************************************************************************
			function GetFightStatistic(infoimg, userid, username, i, content, page, nrofpastfights, loss, wins, draws, evades) {
				...
				var Seite = content.indexOf("<strong>Seite:</strong>");
				if (Seite != -1)
					Seite = content.split("<strong>Seite:</strong>")[1].indexOf("fight/fightlog/"+(page+1)+"/");
				if (Seite != -1) {
					// **********************************************************************************
					// Beziehen der Fightsuchseite
					// **********************************************************************************
					GM_xmlhttpRequest({method: 'GET', url: FIGHLOG_URL + (page+1) + "/?q=" + username, onload: function(responseDetails) {
						//	alert(responseDetails.responseText.substr(responseDetails.responseText.length-4000,2000));
						// Wenn die Seite erfolgreich abgerufen werden konnte
						if (responseDetails.status == 200) {
							var content = responseDetails.responseText;
							GetFightStatistic(infoimg, userid, username, i, content, page+1, nrofpastfights, loss, wins, draws, evades);
							}
					}
					});
				}
				else
				{
					...
					var fightstring = GetFightstring(nrofpastfights) + ((nrofpastfights > 0) ? " (S/U/N/A: " + wins + "/" + draws + "/" + loss + "/" + evades + ")" : "");
			...
			for (var i = nrofids; i >= 1; i--) {
				...
				if (infoimg != null) {
					...
					var draws = 0;
					var evades = 0;
					GetFightStatistic(infoimg, userid, username, i, content, 1, nrofpastfights, loss, wins, draws, evades);
				}
			}
		}
	}
		...
		if (responseDetails.status == 200) {
			...
			function GetUserIDFromDFDE(content) {
				if (content.indexOf('user.getname.xml') != -1)
					return content.split('pennergame.de/dev/api/user.getname.xml?name=')[1].split('" target')[0];
			...
			for (var i = 1; i <= dftrs.length - 1; i++) {
				if (dftrs[i].getElementsByTagName("td")[5].innerHTML.indexOf('pennergame.de/') == -1)
					continue;
				if (dftrs[i].getElementsByTagName("td")[5].innerHTML.split('_blank">')[1].split('</a>')[0] != PZTownCode)
					continue;	
				nrofcheaters++;
				// Username ermitteln
