...
// pagetitle.innerHTML += '<span>&nbsp;<input type="button" class="okButton" id="monstroButton" value="Mise Ã  jour Syndicat"/></span>';
pagetitle.innerHTML += '<a type="button" class="okButton" id="monstroButton">Contrat V4</a>';
	
var monstroButton = document.getElementById('monstroButton');
if (!monstroButton) return;
	//Ajout de l'Ã©vÃ¨nement
...
function traitement() 
{
	...
	//RecupÃ©ration des infos sur les statistiques
	//--------------------------------------------
					...
					{																																																					
						...
						for(var k=0;k<=ee.length;k++)
						{
							if (ee[k].className == "contractMonster")
							{
								...
								ee[k].setAttribute('onMouseOver',"javascript:Tip.show(null,'Point bob : "+point_bob[jj]+"  Point tene : "+ point_tene[villeid[jj]] +"' ,null,event);");
								ee[k].setAttribute('onMouseOut','javascript:Tip.hide();');
								
								if (parseInt(niveau) > parseInt(tenebrae[villeid[jj]]))
								{
									ee[k].innerHTML = "&nbsp;&nbsp;&nbsp;<FONT COLOR='#CC0000'><b>BOB</b> : " + niveau + " <b>TENE</b> : " + tenebrae[villeid[jj]] + "</FONT>";
								}
								else
								{
									ee[k].innerHTML = "&nbsp;&nbsp;&nbsp;<FONT COLOR='#00CC00'><b>BOB</b> : " + niveau + " <b>TENE</b> : " + tenebrae[villeid[jj]] + "</FONT>";
								}
								break;
							}
						}
