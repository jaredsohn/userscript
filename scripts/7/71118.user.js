// ==UserScript==
// @name           Grepolis: Punten (NL)
// @version        1.00
// @date           15-09-2010
// @description    Bij Grepolis (browser spel) voegt de punten bij de nieuwe/huidige gebouwen bij de senaat toe.
// @include        http://*.grepolis.*
// @license 	   Open Source
// @copyright	   © Nimix Nederland
// @author         NimixWare
// @source         http://userscripts.org/scripts/show/71118
// ==/UserScript==
	...
	var grepoPoints = (function () {
		...
		var getUpgradeBuildingHTMLPart_new = function(building) {
			...
			if (typeof level === 'number' && typeof levelQueue === 'number') {
				...
				ret += 'Huidige ' + points + ' punten ';
				if(level !== levelQueue){
					for (var i = level; i < levelQueue && typeof buildingsData[name][i] === 'number'; i += 1) {
						points += buildingsData[name][i];
					}
					ret += '(' + points + ' punten na uitbreiding) ';
				}
				for (var i = levelQueue; typeof buildingsData[name][i] === 'number'; i += 1) {
					points += buildingsData[name][i];
				}
				ret += 'Totaal maximaal ' + points + ' punten. <br>';
				
				ret += 'Niveau ' + level; 
				if(level !== levelQueue){
					ret += ' (Volgende Niveau ' + levelQueue + ')';
				}
				ret += ' Maximaal ' + buildingsData[name].length + '.';
