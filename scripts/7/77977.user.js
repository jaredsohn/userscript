// ==UserScript==
// @name         2b´s Fightinfo TR
// @namespace    http://www.serserionline.com
// @author       sageo[http://berlin.pennergame.de/profil/id:1146285/]fixed by tooBIG
// @description  Stellt zahlreiche kleine und große Hilfen rund um das Thema Kämpfen zur Verfügung (serserionline)
// @include      http://*.serserionline.com/fight/
// @include      http://*.serserionline.com/fight/#*
// @include      http://*.serserionline.com/fight/?to=*
// @include      http://*.serserionline.com/fight/fightlog/*
// @include      http://*.serserionline.com/fight/overview/*
// @include      http://*.serserionline.com/fight/?status*
// @include      http://*.serserionline.com/profil/*
// @include      http://*.pennerzone.e/highscore/?page=*
// @version      1.2.9 serserionline.com
...
var THISSCRIPTINSTALL_URL = 'http://userscripts.org/scripts/show/77977';           // URL für Hauptseite bei userscripts.org
var THISSCRIPTSOURCE_URL = 'http://userscripts.org/scripts/source/77977.user.js';  // Skript-URL bei userscripts.org
...
// URLs für Icons
var INFO_ICON = 'http://www.downfight.de/grafiken/status.gif';                         // Icon für Pennerzone-Link
var ICON_LASTFIGHT_NOCOMMENT = 'http://www.downfight.de/grafiken/krankenhaus.png';       // Icon für letzte Kämpfe (noch kein Kampf) ohne Kommentar
...
var ICON_LASTFIGHT_NOCOMMENT_FIGHTED = 'http://img8.abload.de/img/diagramm99cx.png';   // Icon für letzte Kämpfe (schon mindestens ein Kampf) ohne Kommentar
...
var ICON_0_0 = 'http://static.pennergame.de/img/pv4/dots/0_0.gif';
var ICON_0_1 = 'http://static.pennergame.de/img/pv4/dots/0_1.gif';
var ICON_1_0 = 'http://static.pennergame.de/img/pv4/dots/1_0.gif';
var ICON_1_1 = 'http://static.pennergame.de/img/pv4/dots/1_1.gif';
var ICON_2_0 = 'http://static.pennergame.de/img/pv4/dots/2_0.gif';
var ICON_2_1 = 'http://static.pennergame.de/img/pv4/dots/2_1.gif';
var ICON_EVADE = 'http://static.pennergame.de/img/pv4/dots/evade.gif';
...
} else if (location.toString().indexOf("www.serserionline.com") != -1 || location.toString().indexOf("hamburg.pennerzone.de") != -1) {
	var TOWNBASE_URL = 'http://www.serserionline.com/';
...
function ReadFightData(table, IncomingFights) {
	...
	for (var x = 1; x <= trs.length - 1; x++) {	
		...
		if (tds.length > 1) {
			...
			IncomingFights[x - 1][0] = tds[1].innerHTML;  // Zaman
			IncomingFights[x - 1][1] = tds[3].innerHTML;  // Oyuncu
			IncomingFights[x - 1][2] = tds[4].innerHTML;  // Çete
			IncomingFights[x - 1][3] = tds[5].innerHTML;  // Çekil
		}
	}
}
...
function WriteFightData(table, IncomingFights) {
	...
	for (var x = 0; x < IncomingFights.length; x++) {	
		...
		tds[1].innerHTML = IncomingFights[x][0]; // Zaman
		tds[3].innerHTML = IncomingFights[x][1]; // Oyuncu
		...
		tds[4].innerHTML = IncomingFights[x][2];                    // Çete
		tds[5].innerHTML = IncomingFights[x][3];                    // Çekil
...
//	alert("hinzugefügt!");
	...
	currentimg.addEventListener("click", function(event) { 
		...
		if (sbpostmode == SBPOSTMODE_ACTIVE) {
			...
			var posttext = '[b]Angriff läuft bereits auf  [/b]' + ConvertUserToBB(boxcontent.split('Saldırı şu anda devam etmekte')[1].split('&nbsp;')[0]);
					...
					if (document.getElementById(idprefix + x).checked) {
						...
						if (tds.length == 8 || sbpostmode == SBPOSTMODE_INCOMING) {
							...
							if (tds[4].getElementsByTagName("a").length > 0) {
								...
								var gangid = tds[4].innerHTML.split('serserionline.com/profil/bande:')[1].split('/">')[0]
							} else {
								// Auslesen von Bandenname
								var gangname = "";
							}
						...
						if (sbpostmode == SBPOSTMODE_DONE) {
							...
							poststring = poststring + fighticon + " " + fighttime + " " + fightuser + fightgang + " [color=#2a2a2a]_[/color][color=" + colorstring + "]" + fightmoney + "[/color][color=#2a2a2a]_[/color][color=" + colorstring + "]"  + fightpoints + ' Puan[/color]\n';
	...
	GM_xmlhttpRequest({method: 'GET', url: FIGHTSEARCH_URL + username, onload: function(responseDetails) {
		...
		if (responseDetails.status == 200) {
			...
			function GetStatisticstring(content) {
				...
				return " (W=" + wins + "/L=" + loss + "/U=" + draws + "/A=" + evades + ")";
			}
...
function WriteNrOfIncomingFights(content, table) {
	...
	if (NrOfIncomingFights > 0) {
		...
		for (var i = 0; i < trs.length; i++) {
			// Wenn in der aktuellen Tabellenzeile "Gerçekleşen Kavgalar" steht
	...
	if (LogPageNr <= Number(document.getElementById("seitebis").value)) {
		...
		GM_xmlhttpRequest({method: 'GET', url: FIGHLOG_URL + LogPageNr + "/",onload: function(logresponseDetails) {
			...
			for (var i = 2; i <= fighttrs.length - 2; i++) {
				...
				Fights[i - 2][1] = trimString(fighttds[2].split("</td>")[0]);  // Tarih:
				Fights[i - 2][2] = trimString(fighttds[3].split("</td>")[0]);  // Çete1
				Fights[i - 2][3] = trimString(fighttds[4].split("</td>")[0]);  // Çete
				Fights[i - 2][4] = trimString(fighttds[5].split("</td>")[0]);  // Para
				Fights[i - 2][5] = trimString(fighttds[6].split("</td>")[0]);  // Puan
			}
...
// ********************************************************************************************************************
// ********************************************************************************************************************
// START PROGRAMM * START PROGRAMM * START PROGRAMM * tooBIG * START PROGRAMM *START PROGRAMM *START PROGRAMM * 
// ********************************************************************************************************************
// ********************************************************************************************************************
// ********************************************************************************************************************
	...
	if (!bl()) {
		...
		// ***********************************************************************************************
		if ((location.toString().indexOf(".serserionline.com/profil/") != -1) && (location.toString().indexOf("/bande:") == -1)) {
			// Info-Icons auf dem Profil einfügen
			HandleProfile();
		// ***********************************************************************************************
		// Wenn die aktuelle Seite eine Kampfprofil-Seite ist
		// ***********************************************************************************************
		} else if (location.toString().indexOf(".serserionline.com/fight/viewfight/") != -1) {
//			ap();
		// ***********************************************************************************************
		// Wenn die aktuelle Seite eine Bandenprofil-Seite ist
		// ***********************************************************************************************
		} else if (location.toString().indexOf("serserionline.com/profil/bande:") != -1) {
		...
		// ***********************************************************************************************
		} else if (location.toString().indexOf(".pennerzone.0e/highscore/") != -1) {
			HandlePennerzone();
		// ***********************************************************************************************
