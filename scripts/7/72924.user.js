// ==UserScript==
// @name           IGM MH Assistant
// @namespace      IGM MH Assistant
// @description    IGM MH Assistant
...
GM_config.init('הגדרות מערכת המשיבון',{
  Name:  { label: 'זיהוי:', type: 'text', cols:50, default: 'SuppX-X' },
  Welcome:  { label: 'ברכה:', type: 'text', cols:50, default: 'שלום' },  
  MidMsg:  { label: 'הודעת אמצע:', type: 'text', cols:50, default: 'אין צורך להשיב למכתב זה.' },  
  Byebye:  { label: 'סיומת:', type: 'text', cols:50, default: 'המשך משחק מהנה,' }   
  });
GM_registerMenuCommand('הגדרות משיבון',GM_config.open);

var support = document.forms[0].elements[2].value;
var IDName = GM_config.get('Name', 'נא להגדיר אות זיהוי');
var welmsg = GM_config.get('Welcome', 'נא להגדיר ברכה');
var byemsg = GM_config.get('Byebye', 'נא להגדיר ברכה');
var middlemsg = GM_config.get('MidMsg', 'נא להגדיר ברכה');

var sig = middlemsg + "\n" + byemsg + "\n" + IDName;
var igm = document.getElementById('message');
if (support<0) { 
if (igm) {
var idx = 0;
var str = welmsg + " " + document.getElementById('receiver').value + ",\n\n\n";
