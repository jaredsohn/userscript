// ==UserScript==
// @name           Stalk Awesomolocity's Tank
// @description    Checks if Awesomolocity's Tank is Glowing on Gaia.
// @include        *
// ==/UserScript==
			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' ('+tankId+') [Awesomolocity] has been glowing for '+seconds2HMS(gaiaT-glowT)+'.');
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/56811057/');
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/54193689/');
									GM_openInTab('http://absol.site88.net/gtools/glowTime.php?id=4423249');
								}
							}
							else{
								//alert('over');
							...
							alert(aquaN+' ('+tankId+') [Awesomolocity] will glow in '+seconds2HMS(glowT-gaiaT));
							if(open===true){
