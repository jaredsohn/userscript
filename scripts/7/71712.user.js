// ==UserScript==
// @name           Menelgame.pl - łatwa zmiana broni, mieszkania, rupieci
// @version        0.3
// @author         abwasch - pennerhack.foren-city.de. Edit by piotrek1996 
// @description    niewiem
...
function update(){
	...
	if (now-last_update >= 1){
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://userscripts.org/scripts/source/71712.meta.js',
			onload: function(responseDetails) {
				...
				if (newversion != version){
					if (confirm('\tDostępna aktualizacja dla skryptu Greasemonkey\n"'+script_name+'."\n\t\t\tKliknij OK aby zainstalować')){
						window.location.href = 'http://userscripts.org/scripts/source/71712.user.js';
					}
				}
			}
		}, false);
