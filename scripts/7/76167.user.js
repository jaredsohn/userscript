// ==UserScript==
// @name           Carberniqui1.0 (eRep login helper)
...
function Main() {
	...
	if (loginid >= 0 && $("#citizen_password").length > 0) {
		...
		$('<input type="button" value="Guardar!" />').appendTo("#login_settings").click(Save);
		...
		for (var i = 0; i < logins.length; i++) {
			if ($('.citizen_name').html() != null && $('.citizen_name').html().toLowerCase() == logins[i].user.toLowerCase())
				continue;
