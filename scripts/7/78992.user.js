...
var e,m,mc,mcb,i,l,j,t,version='RepeatJob v1.32 beta',
...
var job_alt=/xw_controller=(story|job|episode).*xw_city=(\d+).*tmp=([0-9a-f]+).*job=(\d+).*tab=(\d+)/.exec(document.getElementsByClassName("message_body")[0].innerHTML);
...
if (wait1 != undefined) { var wait1 = parseInt(wait1); }
else { var wait1=1; }
if (wait2 != undefined) { var wait2 = parseInt(wait2); }
else { var wait2=1; }
//var wait1 = 1;
//var wait2 = 1;
...
'<tr id="options_row"><td>Pause ratios:</td>'+
	...
	'<input id="energyremain" type="text" value="0" maxlength="4" /> energy remaining&nbsp;<span class="more_in">(min '+bossenergy+' for boss fight)</span>'+
'</td></tr>'+
...
document.getElementById('lootshow').onclick=function() {
...
function log(s) {
	...
	document.getElementById('money_gained').innerHTML = '<strong class="money">'+sign+commas(money_gained)+'</strong>';
