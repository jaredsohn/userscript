// ==UserScript==
// @name           4chan AntiSpam Lutes Version
// @namespace      Anonymous
// @description    Removes ALL current spam from the board (Lutes Updated version)
// @include        *4chan.org*
// ==/UserScript==
        	 ...
        	 (tCell[j].innerHTML.indexOf("what's playing over at LOLWUT TV") > 0) ||
  	         ...
  	         (tCell[j].innerHTML.indexOf("Untold hundreds of 4chan bans") > 0) ||
   	         ...
   	         (tCell[j].innerHTML.indexOf("Declaring WAR against /b/") > 0) ||
                 ...
                 (tCell[j].innerHTML.indexOf("R.I.P Justin 21/3/2010") > 0) ||
                 (tCell[j].innerHTML.indexOf("http://h1.ripway.com/taser/") > 0) ||
                 (tCell[j].innerHTML.indexOf("MEGAN FOX SEX TAPE") > 0) ||
                 (tCell[j].innerHTML.indexOf("http://megansextapefree.info/") > 0) ||
                 (tCell[j].innerHTML.indexOf("Chris brown VS Rihanna") > 0) ||
                 (tCell[j].innerHTML.indexOf("voucherbounty.com") > 0) ||
                 (tCell[j].innerHTML.indexOf("70-120 dollars") > 0) ||
                 (tCell[j].innerHTML.indexOf("SquareStyle.net") > 0) ||
                 (tCell[j].innerHTML.indexOf("Make $1000 a week or more") > 0) ||
                 (tCell[j].innerHTML.indexOf("$120-$200 a day per account") > 0) ||
                 (tCell[j].innerHTML.indexOf("Style.net") > 0) ||
                 (tCell[j].innerHTML.indexOf("The.surgeon is back") > 0)
