...
// @namespace      Lez
// @version        1.6
// @description    Muestra la informacion de todas tus ciudades y ademas permite la contruccion en cola
// @include        http://s*.ikariam.*/index.php*
// @homepage       http://userscripts.org/scripts/show/79090
...
var troopDB = {
  301: { score:0.8   , weight:0  }, //Slinger
  302: { score:1.28  , weight:3  }, // Swordsman
  303: { score:1.96  , weight:5  }, // Hoplite/Phalanx
  304: { score:3.92  , weight:5  }, // Gunsman/Marksman
  305: { score:21.06 , weight:30 }, // Mortar
  306: { score:9.12  , weight:0  }, // Catapult
  307: { score:4.2   , weight:0  }, // ram
  308: { score:5.71  , weight:15 }, // sg
  309: { score:19.2  , weight:30 }, // bomb
  310: { score:9.06  , weight:20 }, // Cook
  311: { score:14.12 , weight:10 }, // Doctor
  312: { score:5.12  , weight:0  }, // Gyrocopter
  313: { score:2.62  , weight:0  }, // Archer
  314: { score:0     , weight:0  }, // Wall
  315: { score:0     , weight:3  }, // Spearman
};
...
function addLoadTimesToPage(){
  ...
  $('.setMin,.sliderbg,.setMax,.textfield').click(adjustLoadingTime);
  $('.textfield').change(adjustLoadingTime);
  
  // Add loading time to duration time
  var html = $$('missionSummary').childNodes[1].childNodes[3].innerHTML.split("</span>");
  $$('missionSummary').childNodes[1].childNodes[3].innerHTML = html[0] + "</span>" + durationHMS(0) + ' + ' + html[1] + " = " + html[1];
}
function adjustLoadingTime(){
  ...
  $('.textfield').each(function(){ total_res += parseInt(this.value,10);});
  //debug(total_res,current.buildings.port,current.buildings.port.level);
  var divTime = $$('missionSummary').childNodes[1].childNodes[3].innerHTML;
  ...
  $X('//div[@id="missionSummary"]/div[1]/div[2]/text()').nodeValue = durationHMS(loadTime) + " + " + travelTimeText + " = " + durationHMS(loadTime + travelTime);
}
...
// collect data { mill, [ req, don, ], own, ts }
    ...
    case 'resource' :
      ...
      output = '<table><tr><th>Capacity</th><td>'+fmtNumber(guess) + ' / ' + fmtNumber(temp_resources.maxCapacity[param]) + '</td></tr>';
      output += '<tr><th>Free</th><td>' + fmtNumber(free) + '</td></tr>';
      if (production) {
        ...
        if (production) {
          if (production > 0) output += '<tr><th>Full In</th><td>' + durationHMS(free / Math.abs(production),3) + '</td></tr>';
          else output += '<tr><th>Depleted In</th><td>' + durationHMS(guess / Math.abs(production),3) + '</td></tr>';
        }
      } else if (temp_resources.wineConsumption) {
...
function buildPopUp(){
  ...
  function printResources(){
    ...
    cityOrder.forEach(function(id,i) {
      ...
      for (var resource in ICON) {
        var barText = outText = '&nbsp;';
        percent[resource] = Math.ceil(number(temp_resources.resources[resource])/number(temp_resources.maxCapacity[resource])*100);
        if (percent[resource] < 70)  outText = (fmtNumber(temp_resources.maxCapacity[resource]) || '-');
        else                            barText = (fmtNumber(temp_resources.resources[resource] - temp_resources.maxCapacity[resource]) || '-');
