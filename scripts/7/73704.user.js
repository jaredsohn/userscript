...
window.addEventListener("load", function() {
	...
	// test GM_xmlhttpRequest().abort() with a bogus url
	if (typeof GM_xmlhttpRequest != "undefined" && (GM_xmlhttpRequest({method: "HEAD", url: "ftp://ed67d7eb5cfe4be6.8c1d7629815a91ec"}).abort))
		checkWatchedLinks();
}, false);
...
executeOnPage(function(checkUpdates, updateNeeded) {
	...
	window.ytd = {formats: {
			0:  { fmt: 0 , quality: { level: 0, recdl: false, description: "Niedrige Qualität (rename to video.flv)" }, format: "FLV" , mres: { width: 320, height: 240 }, wmres: { width: 400, height: 226 }, aformat: "22kHz, mono"  , acodec: "MP3" , vcodec: "SVQ"    },
			5:  { fmt: 5 , quality: { level: 1, recdl: false, description: "Niedrige Qualität"                       }, format: "FLV" , mres: { width: 320, height: 240 }, wmres: { width: 400, height: 226 }, aformat: "22kHz, mono"  , acodec: "MP3" , vcodec: "SVQ"    },
			6:  { fmt: 6 , quality: { level: 2, recdl: false, description: "Hohe Qualität"                      }, format: "FLV" , mres: { width: 480, height: 360 }, wmres: { width: 480, height: 270 }                          , acodec: "MP3" , vcodec: "SVQ"    },
			13: { fmt: 13, quality: { level: 1, recdl: false, description: "Niedrige Qualität H.263"   }, mobile: true, format: "3GPP", mres: { width: 176, height: 144 }, wmres: { width: 176, height: 144 }, aformat: "8kHz, mono"   , acodec: "SAMR", vcodec: "H.263"  },
			15: { fmt: 15, quality: { level: -1, recdl: true, description: "Original Upload Format"            }, format: "Unknown"                                                                                                                                 },
			17: { fmt: 17, quality: { level: 2, recdl: false, description: "Niedrige Qualität MPEG-4"  }, mobile: true, format: "3GPP", mres: { width: 176, height: 144 }, wmres: { width: 176, height: 144 }, aformat: "44kHz, mono"  , acodec: "AAC" , vcodec: "MPEG-4" },
			18: { fmt: 18, quality: { level: 4, recdl: true , description: "iPod kompatibel, Hohe Qualität"     }, format: "MP4" , mres: { width: 480, height: 360 }, wmres: { width: 480, height: 270 }, aformat: "44kHz, stereo", acodec: "AAC" , vcodec: "H.264"  },
			22: { fmt: 22, quality: { level: 6, recdl: true , description: "HD, 720p"             }, format: "MP4" , vres: 720                                                            , aformat: "44kHz, stereo", acodec: "AAC" , vcodec: "H.264"  },
			34: { fmt: 34, quality: { level: 3, recdl: false, description: "Niedrige Qualität, 360p"              }, format: "FLV" , vres: 360                                                            , aformat: "44kHz, stereo", acodec: "AAC" , vcodec: "H.264"  },
			35: { fmt: 35, quality: { level: 5, recdl: true , description: "Normale Qualität, 480p"         }, format: "FLV" , vres: 480                                                            , aformat: "44kHz, stereo", acodec: "AAC" , vcodec: "H.264"  },
			36: { fmt: 36, quality: { level: 3, recdl: true , description: "Hohe Qualität MPEG-4" }, mobile: true, format: "3GPP", mres: { width: 320, height: 240 }, wmres: { width: 320, height: 240 }, aformat: "44kHz, mono"  , acodec: "AAC" , vcodec: "MPEG-4" },
			37: { fmt: 37, quality: { level: 7, recdl: true , description: "Full-HD, 1080p"       }, format: "MP4" , vres: 1080                                                           , aformat: "44kHz, stereo", acodec: "AAC" , vcodec: "H.264"  },
		},
	...
	dlButton.setAttribute("title", "Video auf Festplatte speichern");
	...
	else
	{
		...
		{
			if (inputs[i].getAttribute("name") == "video_id")
	...
	if (!fmtStreamMap)
	{
		if (videoPlayer)
		{
			var formats5 = videoPlayer.fa;
			var lastQLevel = 99;
			for (var i = 0; i < formats5.length; i ++)
			{
				var args = formats5[i].wa.substring(formats5[i].wa.indexOf("?") + 1).split("&");
				var fmt;
				...
				if (ytd.formats[fmt])
				{
					...
					ytd.formats[fmt].url = formats5[i].wa + "&title=" + (title ?  title : "video");
					ytd.formats[fmt].guessed = false;
				}
				else
					ytd.formats[fmt] = { fmt: fmt, quality: { level: lastQLevel - 0.5, description: formats5[i].o }, url: formats5[i].wa + "&title=" + (title ?  title : "video") };
			}
		}
		else
			compatError();
	}
	else if (fmtStreamMap[0] == "")
	{
		if (!videoId || !token)
			return compatError();
	...
	if (formatList.length)
		formatSpan.appendChild(document.createTextNode("Wähle ein Format für das Video:"));
	else
	...
	if (formatListMobile.length)
	{
		...
		mFormatSpan.appendChild(document.createTextNode("Oder wähle ein Mobiltelefonkompatibles Format:"));
