...
// 24/03/2010 Add to Wishlist button + Send all but one checkbox added, Version 3.3
// 24/03/2010 Some more grouping added (Consumables), Version 3.3
// 08/05/2010 Removed Blank mafia names
//            Top 6 loot set added
// 12/05/2010 Boosts Categorized and sets added
// 19/05/2010 New BK items added, Top 6 Updated
_________________
...
//	Already broken ? Add the scrollbar and stop the damn window from shrinking like a dried up prune
	...
	var delay=2, gifts=1, gift_Id, gift_Cat, gift_Category, url, gifts_sent=0, stop=true, gift_key=readCookie('gkey');
    ...
    var temp_giftCat, temp_receiverId, temp_receiverName, temp_giftId, temp_gifts;
	var count=0,i=0;
	var profile_link;
	...
	var first_time=true;
//	var fav_user_list=readCookie('Chucker_FavList');
	...
	var content=document.getElementById('popup_fodder');
...
var ny_loot='<option style="color:#33FF00;" value="TopLoot">-- TOP 6 LOOT --'+
		'<Option value="634">Hunter \'Spy\' XS'+
		'<Option value="633">Mugati Sport'+
		'<Option value="1545">Ninja'+
		'<Option value="666">Railgun'+
		'<Option value="1546">Royal Thai Marine'+
		'<Option value="1543">Titanium Katar'+
		'<option style="color:#33FF00;" value="ChopShop">-- CHOP SHOP SET --'+
        '<Option value="535">Acetylene Torches</Option>'+
		...
		'<option style="color:#33FF00;" value="NYConsumables">-- NEW YORK CONSUMABLES --'+
		...
		'<option style="color:#33FF00;" value="MoscowConsumables">-- MOSCOW CONSUMABLES --'+
		...
		'<option style="color:#33FF00;" value="BKConsumables">-- BANGKOK CONSUMABLES --'+
		...
		'<Option value="1548">Lamang Motorcycle'+
...
//		'<Option value="1514">Monk\'s Robe'+
		...
		'<option value="1542">Nak Kah Shotgun'+		
		...
		'<Option value="1547">Raed Armored Sedan'+
		...
		'<option value="1544">Titanium Mesh Jacket'+
...
var boosts='<option style="color:#33FF00;" value="Boosts"><< ALL BOOSTS >>'+
		'<option style="color:#33FF00;" value="EnergyBoosts">-- Energy Boosts --'+
        ...
        '<option style="color:#33FF00;" value="AttackBoosts">-- Attack Boosts --'+
		...
		'<option style="color:#33FF00;" value="DefenseBoosts">-- Defense Boosts --'+
		...
		'<option style="color:#33FF00;" value="RobbingBoosts">-- Robbing Defense Boosts --'+
		...
		'<option style="color:#33FF00;" value="MoneyBoosts">-- Money Boosts --'+		
		...
		'.sexy_input{background-color:black; color:#D0D0D0; width:70%; font-size:15px; font-weight:bold; border: 1px solid #666666; padding-left:0.2em}'+
    ...
    '<tr>'+
     ...
     '<table width="750" class="sexy_table1">'+
      ...
      '<tr>'+
        '<td width="70%" height="45" style="font-size:20px; padding-left:15px">Chucker v3.4.x</td>'+
        '<td width="27%" style="font-size:13px;"><a id="Website" href="http://arun.keen-computing.co.uk/" target="_blank">Arun\'s MW Helpers (updated)</a></td>'+
        '<td width="3%"><a href="#" id="close"><img alt="X" src="http://mwfb.static.zynga.com/mwfb/graphics/icon_remove_16x16_01.gif"></a></td>'+
      '</tr>'+
    '</table>'+
    ...
    '<tr class="sexy_menu1">'+
		...
		'<td width="165" style="text-align: center; border:1px solid #666666"><a id="BugsSuggestions" href="http://www.facebook.com/pages/Aruns-Mafia-Helpers/336498788027" target="_blank">Fan Page</a></td>'+
		'<td width="115" style="text-align: center; border:1px solid #666666"><a id="Pimp" href="http://mafia-taskforce.info/viewforum.php?f=6" target="_blank">Mafia Add</a></td>'+
		'<td width="115" style="text-align: center; border:1px solid #666666"><a id="Tips" href="http://mafia-taskforce.info/viewtopic.php?f=12&t=142" target="_blank">Tips/Tricks</a></td>'+
    ...
    '<table width="750" class="sexy_table1">'+
      ...
      '<tr height="25">'+
        ...
        '<td width="27%"><input type="text" id="quantity" value="1" class="sexy_input" onkeydown="return field_validate(event.keyCode);"></input>&nbsp;<input id="send_all" type="checkbox"></input>&nbsp;<input id="send_all_but_one" type="checkbox"></input></td>'+
      ...
      '<tr height="25">'+
        ...
        '<td align=right style="padding-left:5px"><a id="add_to_wishlist" class="sexy_button_new"><span><span style="width:160px">ADD TO WISHLIST</span></span></a></td>'+
      '</tr>'+
      '<tr height="25">'+
        '<td colspan="4"><a id="begin" class="sexy_button_new"><span><span style="width:115px">START</span></span></a>&nbsp;&nbsp;'+
        '<a id="end" class="sexy_button_new"><span><span style="width:115px">STOP</span></span></a>&nbsp;&nbsp;'+
        '<a id="queue" class="sexy_button_new"><span><span style="width:115px">ADD TO QUEUE</span></span></a>&nbsp;&nbsp;'+
        '<a id="queue_clear" class="sexy_button_new"><span><span style="width:115px">CLEAR QUEUE</span></span></a>&nbsp;&nbsp;'+
        '<a id="send_all_items" class="sexy_button_new"><span><span style="width:115px">SEND ALL</span></span></a></td>'+
      '</tr>'+
     '</table>'+
      
     '<table width="750" class="sexy_table1">'+
      '<tr>'+
        '<td width="8%" valign="top">Log :</td>'+
        '<td width="5%" valign="top"><input type="text" id="log_size" value="10" class="sexy_input" style="width:25px" onkeydown="return field_validate(event.keyCode);"></input></td>'+
        '<td id="logger">&nbsp;</td>'+
      '</tr>'+
     ...
     '<table width="750" class="sexy_table1">'+
      '<tr>'+
        '<td width="8%" valign="top">Queue :</td>'+
        '<td width="5%" valign="top"><input type="text" id="queue_size" value="10" class="sexy_input" style="width:25px" onkeydown="return field_validate(event.keyCode);"></input></td>'+
        '<td id="gift_queue">&nbsp;</td>'+
      '</tr>'+
...
//Setting all handlers for the various components
	...
	document.forms.something.send_all_but_one.onmousemove=function(){popupballoon_display("SendallButOne")};
...
//	document.getElementById('Donors').onmouseout=popupballoon_hide;
    ...
    try{
            ...
            try{
    			...
    			temp_receiverId = split_profile_link[split_profile_link.length - 1].replace('#','');
    //			receiver_profile_id = split_profile_link[split_profile_link.length - 1];
            ...
            catch(err){alert('Error adding to queue from Profile page - ' + err);}
    		  ...
    		  case document.forms.something.user_loc_list.checked:
                ...
                catch(err){alert('Error adding to queue from User list - ' + err);}
               break;
    		}
    ...
    //    	alert(item_name);
        ...
        switch(temp_giftCat){
            case 0:
                if (qty_item_id=="Collection" || qty_item_id=="NYCityCollection" || qty_item_id=="BangkokCityCollection" || qty_item_id=="MoscowCityCollection" || qty_item_id=="CubaCityCollection"){
                    add_set_to_queue();
                }
                else{
            		send_queue[send_queue.length] = temp_giftCat+":"+qty_item_id+":"+item_name+":"+temp_receiverId+":"+temp_receiverName+":"+temp_Gifts;
                }
                break;
            case 1:
                if (qty_item_id=="ChopShop" || qty_item_id=="TopLoot" || qty_item_id=="NYConsumables" || qty_item_id=="MoscowConsumables" || qty_item_id=="BKConsumables" || qty_item_id=="WeaponsDepot" || qty_item_id=="DepotParts" || qty_item_id=="CarParts"){
                    add_set_to_queue();
                }
                else{
                    send_queue[send_queue.length] = temp_giftCat+":"+qty_item_id+":"+item_name+":"+temp_receiverId+":"+temp_receiverName+":"+temp_Gifts;
                ...
                break;
            case 2:
                if (qty_item_id=="EnergyBoosts" || qty_item_id=="AttackBoosts" || qty_item_id=="DefenseBoosts" || qty_item_id=="RobbingBoosts" || qty_item_id=="MoneyBoosts" || qty_item_id=="Boosts"){
                    add_set_to_queue();
                }
                else{
                    send_queue[send_queue.length] = temp_giftCat+":"+qty_item_id+":"+item_name+":"+temp_receiverId+":"+temp_receiverName+":"+temp_Gifts;
    ...
    function removeItem(){
        var num = (this.id);
        num = /del([0-9]+)/.exec(num)[1];
        send_queue.splice(num,1);
        queue_display();
        return false;
    }
        ...
        switch(true){
    		  case document.forms.something.user_loc_profile.checked:
                try{
                    ...
                    profile_link=link_elements[i].href;
                    split_profile_link = profile_link.split("=");
    		      	receiver_profile_id = split_profile_link[split_profile_link.length - 1].replace('#','');
    //			receiver_profile_id = split_profile_link[split_profile_link.length - 1];
        ...
        var temp_giftCat, temp_giftId;
    
        for(temp_giftCat in item_amounts){
            for(temp_giftId in item_amounts[temp_giftCat]){
                if(parseInt(item_amounts[temp_giftCat][temp_giftId]) <= 0){
                    continue;
                }
                send_queue[send_queue.length] = temp_giftCat+":"+temp_giftId+":"+item_names[temp_giftCat][temp_giftId]+":"+receiver_profile_id+":"+receiver_name+":"+item_amounts[temp_giftCat][temp_giftId];
            }
        }
        queue_display();
        return false;
    ...
    //    alert_user('Size - '+size);
        ...
        try{
            for(i=0; i<size; i++){
                document.getElementById('gift_queue').innerHTML+=send_queue[i].split(":")[5]+'(x) '+send_queue[i].split(":")[2]+' to '+send_queue[i].split(":")[4]+' <a href=\'#\' id=\'del'+i+'\'><img alt="X" src="http://mwfb.static.zynga.com/mwfb/graphics/icon_remove_16x16_01.gif"></a><br>';
            }
        }
        ...
        catch(err){alert_user('Error assigning event - '+err);}
    }
    ...
    function send_all_qty(){
    	...
    	if(document.forms.something.send_all.checked){
    		...
    		if(isNaN(item_amounts[gift_Cat][qty_item_id])){
    			...
    			document.forms.something.quantity.value=item_amounts[gift_Cat][qty_item_id] - 1;
    		}
...
//    try{
    ...
    //				eval(/groups_levels\s=\s([^;]+)/.exec(qty_response)[0]+';');
                    ...
                    catch(err){alert('Error area 1 - '+err);}
                    if(gift_key==null){
        			...
        			if(first_time==true){
    				    eval(/groups_levels\s=\s(.+)};/.exec(qty_response)[0]+';');
    //				    alert(groups_levels);
    //				    receiver_user_id=groups_levels;
    					options_html='';
    					options_html += '<Option value=\'none\'>None</option>';
//    					if(fav_user_list!=null){
//                            options_html += '<option style="color:#33FF00;" disabled>-- Fav List --';
//                            fav_user_list = fav_user_list.split('|');
//                            options_html='';
//                            for(k=0;k<fav_user_list.length;k++){
//                                options_html+='<Option value='+fav_user_list[k].split(':')[0]+'>'+fav_user_list[k].split(':')[1]+'</option>'
//                            }
//    					}
    					for(receiver_id in groups_levels){
    					   if(groups_levels[receiver_id] == undefined || groups_levels[receiver_id]==' '){
    					       continue;
    				       }
    					   receiver_user_id[receiver_user_id.length] = [];
    					   receiver_user_id[receiver_user_id.length-1][0] = groups_levels[receiver_id];
    					   receiver_user_id[receiver_user_id.length-1][1] = receiver_id;
    					}
    					receiver_user_id.sort();
    					for(i=0; i<receiver_user_id.length; i++){
                            options_html+='<Option value='+receiver_user_id[i][1]+'>'+receiver_user_id[i][0]+'</option>';
    //                        options_html+='<Option value='+receiver_id+'>'+groups_levels[receiver_id]+'</option>';
                        }
    ...
    function update_sw_sig(){
    	...
    	try{
    		...
    		var sig_url='http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=item&xw_action=view&xw_city=1&xw_client_id=8&cb='+c[1]+'&ajax=1&liteload=1&sf_xw_user_id='+sender_profile_id+'&sf_xw_sig='+local_xw_sig;
...
//The Start sending button's handler code, mostly self explainatory
    ...
    /// Start button specific code here
        if(send_queue.length==0){
            ...
            if(qty_item_id=="Collection" || qty_item_id=="TopLoot" || qty_item_id=="NYConsumables" || qty_item_id=="MoscowConsumables" || qty_item_id=="BKConsumables" || qty_item_id=="NYCityCollection" || qty_item_id=="BangkokCityCollection" || qty_item_id=="MoscowCityCollection" || qty_item_id=="CubaCityCollection" || qty_item_id=="ChopShop" || qty_item_id=="WeaponsDepot" || qty_item_id=="DepotParts" || qty_item_id=="CarParts" || qty_item_id=="EnergyBoosts" || qty_item_id=="AttackBoosts" || qty_item_id=="DefenseBoosts" || qty_item_id=="RobbingBoosts" || qty_item_id=="MoneyBoosts" || qty_item_id=="Boosts"){
    ...
    //            }
    	...
    	url='http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=gift&xw_action=send&cb='+cb[1]+'&sf_xw_user_id='+sender_profile_id+'&sf_xw_sig='+local_xw_sig+'&gift_category='+gift_Category+'&gift_id='+gift_Id+'&gift_key='+gift_key+'&recipients[0]='+receiver_profile_id;
    //    alert(url);
    	...
    	catch(err){alert('Error in item amounts - '+err);}
    //	document.getElementById('err_logger').innerHTML='<br style="padding-left:5px">Before Hang Qty = '+before_hangqty;
    
        if(sender_profile_id == '684833130' || sender_profile_id == '100000605027552' || sender_profile_id == '1570761949'){
    ...
    //	min = new Date().getMinutes();
    	...
    	catch(err){alert('Error in timer - '+send_hangup_timer);}
    //	hr= (hr<10?'0'+hr:hr);
    ...
    //    	alert_user('\nURL - '+url);
    		...
    		send_hangup_timer = setTimeout(function(){hangup_flag='true'; xmlHTTP.abort(); hangup_check();},60000);
    		xmlHTTP.send(null);
    	}
    	...
    	if(xmlHTTP.readyState==4){
    		if(xmlHTTP.status==200){
    		  try{
    			...
    			catch(err){alert('Error in status 200 - '+err);}
    //			return;
    			try{
    			if(/You gave/.test(response)) {
    //                alert_user('Gifts sent - '+gifts_sent+' Gifts - '+gifts);
    try{
    				gift_text=/You gave (some|an|a)(.+) to (.+)\./i.exec(response);
...
//                    alert(gift_text);
    				...
    				gifts_sent++;
    //				alert(receiver_profile_id);
    //				alert('Sent <font color=#33FF00>'+gifts_sent+'(x)</font> '+gift_text[2]+'(s) to <a href="http://apps.facebook.com/inthemafia/profile.php?id=%7B%22user%22%3A%22'+receiver_profile_id+'%22%7D" target="_blank">'+gift_text[3]+'</a>');
    				logmsg('Sent <font color=#33FF00>'+gifts_sent+'(x)</font> of <font color=#33FF00>'+gifts+'</font> '+gift_text[2]+'(s) to <a href="http://apps.facebook.com/inthemafia/profile.php?id=%7B%22user%22%3A%22'+receiver_profile_id+'%22%7D" target="_blank">'+gift_text[3]+'</a>','false');
    ...
    catch(err){alert('Some error in received response - '+err);}
    //                post_request();
    //				post_request();
    			}
    ...
    //				alert_user('Stopped...');
    			...
    			catch(err){alert('Error in state_change - '+err);}
    //			else{
    //				document.getElementById('err_logger').innerHTML='Some thing unknown.. weird..';
    //			}
    
    		}
    	}
        }
        catch(err){alert('Error in state change function - '+err);}		
	...
	try{
		...
		document.getElementById('poop_up').style.left= (tempX - 300)+'px';
...
//                document.getElementById('poop_up').innerHTML='<div class="sexy_destination1">Currently Down</div>';
				break;
			case "Tips":
...
//                document.getElementById('poop_up').innerHTML='<div class="sexy_destination1">Currently Down</div>';
			...
			case "Updates":
				...
				'<tr><td style="color:white; height:25px"><li>Added new Bangkok loot, updated top 6</li></td></tr>'+
				'<tr><td style="color:white; height:25px"><li>Boosts categorized and sets added</li></td></tr>'+
				'<tr><td style="color:white; height:25px"><li>Add to Wishlist option</li></td></tr>'+
				'</table></div>';
				break;
//			case "Donors":
...
//        }
    ...
    function add_set_to_queue(){
    	...
    	if(qty_item_id=="Collection"){
    		for(i=1;i<8;i++){
    			collection_id_list[collection_id_list.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i].value;
    			collection_list_name[collection_list_name.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i].text;
    //			alert_user(collection_id_list);
    		}
            for(i=0;i<collection_id_list.length;i++){
                if(item_amounts[temp_giftCat][collection_id_list[i]] > 0 && !(isNaN(item_amounts[temp_giftCat][collection_id_list[i]]))){
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
                    send_queue[send_queue.length] = temp_giftCat+":"+collection_id_list[i]+":"+collection_list_name[i]+":"+temp_receiverId+":"+temp_receiverName+":"+temp_Gifts;
                }
            }
    	}
    	
        if(qty_item_id=="ChopShop" || qty_item_id=="WeaponsDepot" || qty_item_id=="NYConsumables"){
    		for(i=1;i<6;i++){
    			collection_id_list[collection_id_list.length] = document.forms.something.loot_gift_id.options[document.forms.something.loot_gift_id.selectedIndex+i].value;
    			collection_list_name[collection_list_name.length] = document.forms.something.loot_gift_id.options[document.forms.something.loot_gift_id.selectedIndex+i].text;
    //			alert_user(collection_id_list);
    		}
            for(i=0;i<collection_id_list.length;i++){
                if(item_amounts[temp_giftCat][collection_id_list[i]] > 0 && !(isNaN(item_amounts[temp_giftCat][collection_id_list[i]]))){
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
        ...
        if(qty_item_id=="MoscowConsumables" || qty_item_id=="TopLoot"){
    		for(i=1;i<7;i++){
    			collection_id_list[collection_id_list.length] = document.forms.something.loot_gift_id.options[document.forms.something.loot_gift_id.selectedIndex+i].value;
    			collection_list_name[collection_list_name.length] = document.forms.something.loot_gift_id.options[document.forms.something.loot_gift_id.selectedIndex+i].text;
    //			alert_user(collection_id_list);
    ...
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
    	...
    	if(qty_item_id=="NYCityCollection"){
            for(i=1;i<129;i+=8){
                for(j=1;j<8;j++){
                    collection_id_list[collection_id_list.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].value;
                    collection_list_name[collection_list_name.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].text;
                }
    //			alert_user(collection_id_list);
    		}
            for(i=0;i<collection_id_list.length;i++){
    ...
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
        ...
        if(qty_item_id=="CubaCityCollection"){
            for(i=1;i<41;i+=8){
                for(j=1;j<8;j++){
                    collection_id_list[collection_id_list.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].value;
                    collection_list_name[collection_list_name.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].text;
    ...
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
        ...
        if(qty_item_id=="MoscowCityCollection"){
            for(i=1;i<49;i+=8){
                for(j=1;j<8;j++){
                    collection_id_list[collection_id_list.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].value;
                    collection_list_name[collection_list_name.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].text;
    ...
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
        ...
        if(qty_item_id=="BangkokCityCollection"){
            for(i=1;i<41;i+=8){
                for(j=1;j<8;j++){
                    collection_id_list[collection_id_list.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].value;
                    collection_list_name[collection_list_name.length] = document.forms.something.collection_gift_id.options[document.forms.something.collection_gift_id.selectedIndex+i+j].text;
    ...
    //                        alert(item_amounts[gift_Cat][collection_id_list[i]]);
        ...
        if(qty_item_id=="Boosts"){

    		for(i=1;i<59;i++){
                var val = document.forms.something.boost_gift_id.options[document.forms.something.boost_gift_id.selectedIndex+i].value;    		
                if(val=="EnergyBoosts" || val=="AttackBoosts" || val=="DefenseBoosts" || val=="RobbingBoosts" || val=="MoneyBoosts" ){
                    continue;
                }
    			collection_id_list[collection_id_list.length] = document.forms.something.boost_gift_id.options[document.forms.something.boost_gift_id.selectedIndex+i].value;
    			collection_list_name[collection_list_name.length] = document.forms.something.boost_gift_id.options[document.forms.something.boost_gift_id.selectedIndex+i].text;
    		}
            for(i=0;i<collection_id_list.length;i++){
                if(item_amounts[temp_giftCat][collection_id_list[i]] > 0 && !(isNaN(item_amounts[temp_giftCat][collection_id_list[i]]))){
                    send_queue[send_queue.length] = temp_giftCat+":"+collection_id_list[i]+":"+collection_list_name[i]+":"+temp_receiverId+":"+temp_receiverName+":"+temp_Gifts;
    ...
    function Add_to_Wishlist(){
        do_ajax('inner_page', 'remote/html_server.php?xw_controller=collection&xw_action=wishlist_add&xw_city=1&cb='+sender_profile_id+UnixTS()+'&selected_city=1&gift_category='+gift_Cat+'&gift_id='+qty_item_id, 1, 1, 0, 0);
    }
...
function alert_user(message){
    ...
    minute = (minute<10?'0'+minute:minute);
try{
    document.getElementById('errormsg').innerHTML+='<tr style="padding-left:5px"><td>['+hr+':'+minute+'] '+message+'</td></tr>';
}
catch(err){}
}
