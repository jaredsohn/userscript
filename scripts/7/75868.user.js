...
(function() {
	...
	if(c[0].innerHTML.indexOf('Сс:') >= 0) // anti-Safari из-за глючного onload
		return;
	...
	var outPlace = tr.cells[0];
	var outPlaceHtml = outPlace.innerHTML;
	var tmp = /(\<a href=\"\/statlist.php\?r=)(\w*)([\S\s]*)/i.exec(outPlaceHtml);
	outPlace.innerHTML = outPlaceHtml + " " + tmp[1] + tmp[2] + "&type=i>предмет</a>";;
	...
	var income = out * out_price;
	
	var ss = 0;
	for (var i = 0, l = a_in.length; i < l; i++) 
		ss += a_in[i] * a_in_price[i];
	...
	c_new.innerHTML = '<b> Сс: <font color = "red">$' + (Math.round((ss / out)*100) / 100).toFixed(2) 
		+ '</font> (ЗП: <font color = "red">$' + (Math.round((salary * hours / out)*100)/100).toFixed(2)
		+ '</font>), прибыль: с единицы <font color = "red">$' + (Math.round((income / out)*100) / 100).toFixed(2) 
		+ '</font>, с клетки в час <font color = "red">$' + Math.round(income / hours) 
		+ '</font> ( x' + square + ' = <font color = "red">$' + Math.round(income*square / hours) 
		+ '</font>)</b><hr>'; 
