// ==UserScript==
// @name          PassThePopcorn Uploaded since last visit
// @namespace      http://death2y.uuuq.com/
// @description    Shows upload, download, and ratio change since last visit on passthepopcorn.org
// @author         death2y (minor changes for passthepopcorn - dark_skeleton)
...
//Menu Items
GM_registerMenuCommand("PassThePopcorn Stat Tracker: Show when no difference?", function() {
    var x = confirm("Would you like to show stat differences when there has been no change?");
    GM_setValue("showNoDifference",x);
});
GM_registerMenuCommand("PassThePopcorn Stat Tracker: Alert on change?", function() {
    var x=confirm("Would you like to be alerted when one of your stats have changed?");
    GM_setValue("alertOnChange",x);
});
GM_registerMenuCommand("PassThePopcorn Stat Tracker: Show only in profile?", function() {
...
if (showOnlyProfile == false) {
    ...
    var statsContainer = document.getElementsByTagName("ul")[6];
    var stats = statsContainer.getElementsByTagName("li");
    
    var uploaded   = toBytes(stats[2].innerHTML.split(': ')[1]);
    var downloaded = toBytes(stats[3].innerHTML.split(': ')[1]);
    var ratio      = stats[4].getElementsByTagName("span")[0].innerHTML;
}
