...
IKARIAM_ALL_IN_ONE_VERSION = '1.15';
...
IkariamAllInOne.enhanceIkaTools = function() {
    ...
    // Improved version of getMovementsUpdate that passes through the return document text
    IkaTools.getMovementsUpdate = function(callbackFunction) {
    ...
    // Improved version of getRemoteDocument where both the parsed document and the 
    // response text are passed to the callback (this is needed to parse some 
    // military mission times).
    IkaTools.getRemoteDocument = function(url, callback, method, data) {
        ...
        GM_xmlhttpRequest ({
            ...
            onload: function (response){
                ...
                callback(domResponse, response.responseText);
            }
        });
    };
            ...
            if(true) { // Incoming trade fleets on non-alternate rows have no class
                ...
                //for(var i = 0; i < tds.length; i++) {
                    try {
                        ...
                        if(tds[1].id.toString().match(/fleetRow/)) {
                            movement.id = parseInt(tds[1].id.toString().match(/\d+$/).toString());
                            movement.timeString = tds[1].innerHTML.toString();
                        ...
                        // get mission & status
                        if($('img', tds[5]).size() == 1 && $('img', tds[5]).attr('src').toString().match(/mission_/)) {
                            movement.mission =     $('img', tds[5]).attr('src').toString().match(/mission_[^\.]+/).toString().replace(/^mission_/, '');
                            if (tds[5].title.match(/\([^\)]+\)/)) {
                                movement.status = tds[5].title.match(/\([^\)]+\)/).toString().replace(/(\(|\))/g, '');
                            } else {
                                movement.status = tds[5].title;
                            }
                            movement.description = tds[5].title;
                            debug('Movement: ' + movement.mission + ', ' + movement.description);
                        }
                        // get abort href
                        if($('a', tds[8]).size() == 1 && $('a', tds[8]).attr('href').toString().match(/abortFleetOperation/)) {
                            movement.abortHref = $('a', tds[8]).attr('href').toString();
                        }
                        // get units
