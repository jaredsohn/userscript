...
if(FirstRun == true) {
var refresh = window.content.location.href.toString();
...
function getURL(qual) {
	downloadURL = "http://"+document.domain+"/get_video?video_id="+unsafeWindow.yt.getConfig('SWF_ARGS').video_id+"&t="+unsafeWindow.yt.getConfig('SWF_ARGS').t
	switch(qual) {
...
function isHD1080avail() {
  ...
  var fmt_map_check = unescape(s['fmt_map']);
    if (fmt_map_check.indexOf('37/4000000/9/0/115') > -1) {
		return true;
	}
	else {
		return false;
...
function isHD720avail() {
  ...
  var s = unsafeWindow.yt.getConfig('SWF_ARGS');
  var fmt_map_check = unescape(s['fmt_map']);
    if (fmt_map_check.indexOf('22/2000000/9/0/115') > -1) {
		return true;
	}
	else {
		return false;
...
function checkvideo(){
	...
	if (fmt_map_check.indexOf('35/640000/9/0/115') != -1) {
		return true;
	}
	if (fmt_map_check.indexOf('34/0/9/0/115') != -1) {
		return true;
	}
	if (fmt_map_check.indexOf('18/512000/9/0/115') != -1) {
		return true;
	}
