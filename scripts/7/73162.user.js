...
if(lang!='fr' && lang!='de' && lang!='es' && lang!='en' && lang!='pl' && lang!='hu' && lang!='pt' && lang!='ro' && lang!='cz' &&lang!='it' && lang!='gr'){lang='en';}
...
$(document).ready(function(){
	...
	//Grepostats Links
		...
		$('#gt_gs_container').append('<div id="gt_gs" style="float:left;width:130px;height:70px;background:url("></div>');
		...
		$('#gt_gs').append('<span style="font-size:14px;text-decoration:underline;">Grepostats</span>');
		$('#gt_gs').append('<div style="text-align:left;"><a href="'+gt_gs_player+'" target="_blank">Your Stats</a></div>');
		$('#gt_gs').append('<div style="text-align:left;"><a href="'+gt_gs_ally+'" target="_blank">Your Ally\'s Stats</a></div>');
		$('#gt_gs a').css({'font-size':'12px'});
	//BBCodes
	...
	//Navigation
		//General Stuff
			...
			$('#link_report').next().append('<span style="color:#FFCC66;"> | </span><a href="/game/alliance?action=forum&amp;town_id=7828&amp;h=hzH0TZ%2BO">Forum').next().remove();
		...
		//Help
			...
			$('#gt_help').append('<span style="font-size:10px;">Thanks for the translations: r0nin, Bogesz, RUISJESUS, TTF, davchy, Freezer2k, teobest1</span><br /><br />');
	...
	//Widgets
		//Language Widget
			...
			$('#widget_lang').append('Linguagem: '+language[lang]);
			$('#widget_lang').appendTo('body').draggable();
			$('#widget_lang').css({'position':'absolute','top':'10px','left':'10px'});
			var h1=$('#widget_lang').height()+20;
			h1=h1+'px';
		//Links Widget
			$('#cache').append('<div id="widget_links" class="gtwidget" style="width:200px;background-color:#3B3B3B;color:#ffcc66;z-index:1000;opacity:0.8;border:1px solid;padding:2px;text-align:left;font-size:10px;"></div>');
			$('#widget_links').append('<div style="text-align:center;font-size:12px;margin-bottom:4px;">Atalhos</div>');
			//Links
				$('#widget_links').append('<a href="building_place?action=simulator&town_id='+town+'">'+trad.sim+'</a><br />');
				$('#widget_links').append('<a href="building_main?town_id='+town+'">'+trad.main+'</a> - <a href="building_academy?town_id='+town+'">'+trad.academy+'</a><br />');
				$('#widget_links').append('<a href="building_barracks?town_id='+town+'">'+trad.barracks+'</a> - <a href="building_docks?town_id='+town+'">'+trad.docks+'</a><br />');
				$('#widget_links').append('<a href="building_storage?town_id='+town+'">'+trad.storage+'</a> - <a href="building_lumber?town_id='+town+'">'+trad.lumber+'</a> - <a href="building_stoner?town_id='+town+'">'+trad.stone+'</a> - <a href="building_ironer?town_id='+town+'">'+trad.iron+'</a><br />');
				$('#widget_links').append('<a href="building_temple?town_id='+town+'">'+trad.temple+'</a> - <a href="building_market?town_id='+town+'">'+trad.market+'</a><br />');
				$('#widget_links').append('<a href="building_wall?town_id='+town+'">'+trad.wall+'</a> - <a href="building_place?town_id='+town+'">'+trad.place+'</a><br />');
			$('#widget_links').appendTo('body').draggable();
			$('#widget_links').css({'position':'absolute','top':h1,'left':'10px','color':'#ffcc66'});
			$('#widget_links a').css({'color':'#ffcc66'});
			var h2=$('#widget_lang').height()+$('#widget_links').height()+30;
			h2=h2+'px';
		//Storage Widget
			...
			var h3=$('#widget_lang').height()+$('#widget_links').height()+$('#widget_storage').height()+56;
			h3=h3+'px';
		//Troop Movements
			...
			$('#widget_troops').append('<div style="text-align:center;font-size:12px;margin-bottom:4px;">Movimentos Tropa</div>');
			...
			if(location.href.match('game/index')){
				for(x in uW.UnitOverview.unit_movements){
					...
					if(gt_tm.type=='ask_farm_for_resources'){
						$('#gt_troops_'+x).append('Recursos: '+gt_tm.town.name_short);
			...
			var h3=$('#widget_lang').height()+$('#widget_links').height()+$('#widget_storage').height()+$('#widget_troops').height()+66;
			h3=h3+'px';
		//About Grepolis Tools Widget
			...
			$('#widget_about').append('<div style="text-align:center;font-size:12px;margin-bottom:4px;">Sobre Grepolis Tools</div>');
			$('#widget_about').append('Versão: '+gt_version+'<br />');
			$('#widget_about').append('Linguas: '+gt_langs+'<br />');
			$('#widget_about').append('<div style="text-align:center;"><a href="http://userscripts.org/scripts/show/71132" target="_blank">Verificar nova versão.</a></div>');
