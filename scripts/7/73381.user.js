...
function evalBaseElements(dom, serdom) {
	...
	var baseElements = '<h3 class="header">';
	    ...
	    baseElements += '</h3><div id="chMessagesListBox" style="width:240px;height:255px; padding:1px 0;font-size:12px; margin-left: 6px; display:'+showStatus+'">';
	
	    baseElements += '<div id="chMessagesList" style="height:250px;overflow:auto;margin-right: 2em;border: 1px solid #CB9B6B; font-size:8pt; background-color:'+color+'; text-align:left;"></div></div>';
...
function findFirstChildAfter( afterid, contid ){
	var cel = document.getElementById(contid);
	var ael = document.getElementById(afterid);
	if( (cel==null) || ( ael==null ) ) return null;
	return ael.nextSibling;

	//var id = '';
	//var count = cel.childNodes.length;
	//for( i=0; i < count; i++ ){
		//id = cecel.childNodes[i].id
	//}
	
	return null;
}

var ikaChatContainer = true;

// TODO: to be completed :)
function whereToShow() {
	//Added by Mellonedain:
	if( document.getElementById('militaryAdvisorDetailedReportView') == null )
	{
		if( document.getElementById('ikachatContainer') == null )
		{
			var par = document.getElementById('container2');
			//FIXME: Add specify value for Detailed page //militaryAdvisorDetailedReportView (too less space)
			//Add chat on top of the column
			var mar = findFirstChildAfter( 'breadcrumbs', 'container2' );
			if(mar == null){
				//alert('Use dafault position: End of column');
				mar = document.getElementById('mainview');
			}
			if( (par != null) && ( mar != null ) )
			{
				var chatCont = document.createElement('div');
				chatCont.setAttribute('id','ikachatContainer');
				chatCont.setAttribute('class','dynamic');
				//par.appendChild( chatCont );
				//mainview
				mar.parentNode.insertBefore( chatCont, mar );
				return 'ikachatContainer';
			}
			else
			{
				//alert( 'This not working on this page!' );
				ikaChatContainer = false;
			}
		}
		else
		{
			if( ikaChatContainer ) return 'ikachatContainer';
		}
	}
	//End if new method not working
