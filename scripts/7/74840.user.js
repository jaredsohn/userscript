...
// @description    Manages Treasure Isle wall posts
...
var wC;
var version = "1.3"; // cant use cdata+regexp to grab version dynamically because of webkit
 ...
 if(document.evaluate("//iframe[@id='GM_config']",document,null,9,null).singleNodeValue) return;
	...
	GM_config.frame.addEventListener('load', function(){
		...
		frameBody.appendChild(obj.create('div', {id:'buttons_holder', kids:new Array(
			...
			obj.create('button',{id:'cancelBtn', textContent:'Close',title:'Close window',className:'saveclose_buttons',onclick:function(){GM_config.close(false)}}),
			obj.create('div', {className:'reset_holder block', kids:new Array(
				obj.create('a',{id:'resetLink',textContent:'Restore setting to Default',href:'#',title:'Restore settings to default configuration',className:'reset',onclick:obj.reset})
		)}))}));
...
whichRegex : /(gem|bonus|save|fruit|unknown|send|treasure)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
...
accText : {
			...
			unknown : "Animal Adopted!",
			fruit : "Fruit Taken!",
			send : "Sent!",
			treasure : "Treasure Taken!"
		},
...
var statusText = !main.boxFull ? (!main.paused?"["+main.gameAcronym+"] "+main.currReqs+" Current Requests.":(!main.pauseClick?("["+main.pauseCount+"] "):"")+"[PAUSE] Press this window ripartire") : "[STOP] Gift boxes full - Reload the page to Start. ripartire";
...
case false: if(!$(key)) {
	...
	case "send": if(opts["send"] && wC!="wish") open(item.href, key, w); break; // open request in iframe
...
// pre-load the config
GM_config.init("<center><table><tr><td><img src='"+imgs.logo+"' width=50% height=50%></td><td>Treasure Isle Wall Manager "+version+" by Sisko V1.3 Translated by Darkshad</td></tr></table></center>", {
	bonus : {
		section : ["Options"],
		label : "Accept bonus?",
		type: "checkbox",
	...
	send : {
		label : "Send items (no wishlist)?",
		type: "checkbox",
		"default" : true,
	},
	sloth : {
		section : ["Specify the animals to be taken"],
		label : "Sloth",
		type : "checkbox",
		"default" : false,
	},
	...
	red : {
		section : ["Specify gems to collect"],
		label : "Red Gem",
		type : "checkbox",
		"default" : false,
	},
	...
	inputtimeoutenable : {
		label : "Autopause when writing a Comment",
		type : "checkbox",
		"default" : false,
		kids : {
			inputtimeout : {
...
try {
GM_registerMenuCommand("Treasure Isle Wall Manager Options "+version, main.config);
} catch(e) {}
...
main.create("a", {className:"item", href:"javascript:void(0);", onclick:main.config}, new Array(
	...
	main.create("span", {textContent:"Treasure Isle Wall Manager"+version})
))
)));
}
