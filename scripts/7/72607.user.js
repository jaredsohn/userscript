// ==UserScript==
// @name           Plemiona - Rekrutacja PL
// @description    Usprawniona masowa rekrutacja
...
// -----------------------------------------------------------------------------
//   Modyfikacje i rozpowszechnianie tego skryptu wyłącznie  
//                                     za zgodą autora.
// -----------------------------------------------------------------------------
...
(function(){
	...
	var updateWorkers = function() {
		...
		el.textContent = '(Wymagane miejsca w zagrodzie: '+neededworkers+')';
	}
	...
	var h3 = ce('h3');
	h3.textContent = 'Ustawienia grupy »'+selectedgroup+'«';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Ilość jednostek:';
	th.title = 'Oczekiwana liczba jednostek we wsi (nie ilość, która ma zostać dobudowana).';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Kontynuować rekrutacje:';
	th.title = 'Zaznacz oddziały, które będą rekrutowane po osiągnięciu ustalonych limitów';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	for(var i = 0; i < units.length; i++) {
		var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Wolne miejsca w zagrodzie:';
	th.title = 'Ile miejsc w zagrodzie zostawić wolnych (np. 100 dla szlachcica)';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Zachowaj surowce:';
	th.title = 'Ile surowców zostawić w rezerwie (Np. paczka na szlachte - 28k, 30k, 25k)';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	td.setAttribute('colspan', units.length);
	...
	for(var i = 0; i < 3; i++) {
		...
		if(i == 0) {
			...
			img.title = img.alt = 'Drewno';
		} else if(i == 1) {
			img.src = 'graphic/lehm.png';
			img.title = img.alt = 'Glina';
		} else if(i == 2) {
			img.src = 'graphic/eisen.png';
			img.title = img.alt = 'Żelazo';
		}
	...
	var btn = ce('button');
	btn.setAttribute('style', 'font-size:8pt;');
	btn.textContent = 'Wypełnij';
	btn.addEventListener('click', function(evt){
		// calculate and fill
		evt.target.disabled = true;
		evt.target.textContent = 'przeliczam...';
		var currentsettings = getCurrentSettings(selectedgroup);
		...
		document.getElementById('massrecruitbuilt').textContent = 'Uzupełnione: ' + builtstr + ' w ' + builtvilnum + ' wioskach';
		evt.target.textContent = 'Zrobione';
