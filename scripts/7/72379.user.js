...
function addCompletionTimes()
{
  ...
  var x = xpath("//td[starts-with(@id, 'time')]");
  for (var i = 0; i < x.length; i++) {
    var td = x[i];
...
function enhanceProductionPage()
{
  ...
  for (var row = 1; row <= rowCount; row++) {
    ...
    html += "<tr><td align='center'>"+plusLink(row, 1000)+"&nbsp;";
    html += plusLink(row, 10000)+"&nbsp;";
    html += plusLink(row, 0)+"</td></tr></table>";
    html += "</td></tr></table>";
  ...
  {
    seconds = Number(seconds);
    document.getElementById("time"+row).title = seconds;
  ...
  function setQuantities()
  {
    ...
    for (var row = 1; row <= rowCount; row++) {
      ...
      var timeCell = document.getElementById("time"+row);
