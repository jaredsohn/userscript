...
// @description   F\'fcgt ein Auswahlfeld im Internen Forum hinzu, damit man Smilies und Icons ausw\'e4hlen kann, au\'dferdem die BB-Codes f\'fcr Berichte und Code. Seit Version 2 k\'f6nnen Texte gespeicherter werden.\
// @namespace     c1b1.de\
...
\
F\'fcr Opera wird folgende Scriptversion empfohlen:\
http://userscripts.org/scripts/version/39879/112353.user.js\
\
F\'fcgt ein Auswahlfeld im Internen Forum hinzu, damit man Smilies und Icons ausw\'e4hlen kann, au\'dferdem die BB-Codes f\'fcr Berichte und Code.\
Seit Version 2 k\'f6nnen Texte gespeicherter werden.\
\
\
Screenshot:\
...
\
  ...
  if(gm && (forum || memo || mail || ally))\
    \{\
    mainDiv.addButton('Pers\'f6nliche Texte einf\'fcgen',icon_usertexts,function() \{\
      // User Texts' Box\
...
\
    ...
    th.appendChild(document.createTextNode('Pers\'f6nliche Texte'));\
    tr.appendChild(th);\
    table.appendChild(tr);\
\
    var texts = getTexts();\
...
\
    if(len == 0)\
      ...
      td.appendChild(document.createTextNode('Bisher keine Eintr\'e4ge'));\
      tr.appendChild(td);\
      table.appendChild(tr);\
      \}\
\
...
\
    ...
    th.appendChild(document.createTextNode('Pers\'f6nliche Texte bearbeiten'));\
    tr.appendChild(th);\
    table.appendChild(tr);\
\
    var texts = getTexts();\
...
\
      var td = document.createElement('td');\
      td.setAttribute('class','tdbutton');\
      td.setAttribute('title','L\'f6schen');\
      td.appendChild(document.createTextNode('X'));\
      td.addEventListener('click',function(i) \{ return function() \{\
       var c = confirm('Soll dieser Eintrag wirklich gel\'f6scht werden?');\
       if(c)\
         \{\
         texts.remove(i);\
         setTexts(texts);\
...
\
    ...
    td.setAttribute('class','tdbutton');\
    td.appendChild(document.createTextNode('Schlie\'dfen'));\
    td.addEventListener('click',function() \{\
      toggle('user_texts_edit');\
      return false;\
    \},false);\
...
\
  ...
  function workWithEntry(texts,n)\
    ...
    if(typeof(n) != 'undefined')\
      \{\
      var header = 'Pers\'f6nlichen Text bearbeiten';\
      var name = texts[n].name;\
      var text = texts[n].value;\
      \}\
    else\
      \{\
      var header = 'Neuer pers\'f6nlicher Text';\
      var name = '';\
      var text = '';\
      \}\
\
