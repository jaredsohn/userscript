...
// @date           2010-03-11
// @version        0.1
// @GM_version     0.8.20100211.5
//
// This script is a modified copy of gaillard's script
// ==/UserScript==
...
GM_config.init('Delicious Search on Google',{
    name:        { label: 'Username:',    type: 'text', cols:50, default: '' },
    privatekey:  { label: 'Private Key:', type: 'text', cols:50, default: '' }
  },
  configStyle,
  {
    open: function() {
      GM_config.addBorder(); // add a fancy border
      GM_config.resizeFrame('240px','240px'); // resize the config window
  },
  save: function() { location.reload(); }
}
);

GM_registerMenuCommand('Delicious Search on Google: Settings',GM_config.open);

if (GM_config.read().name == "") {
	GM_config.open();
}
...
GM_DUR = {
	...
	init : function()
	{
		...
		if( GM_DUR.un != "" )
		{
			...
			GM_xmlhttpRequest({
				method:"GET",
				url:"http://feeds.delicious.com/v2/json/"+GM_config.read().name+"/"+GM_DUR.un+"?private="+GM_config.read().privatekey,
				headers:{
					"User-Agent":"Mozilla/5.0",
					"Accept":"text/json"
				},
				onload:GM_DUR.handleUser
			});
		}
	},
	...
	handleUser : function(response)
	{
		...
		if( results.length > 0 )
		{
			...
			var h3 = h.appendChild(document.createElement("h3"));
			h3.className = "r";
			h3.innerHTML = "<a href='http://delicious.com/search?p=" + GM_DUR.un +"'>User Delicious results for <em>"+ query +"</em></a>";
			var t = h.appendChild(document.createElement("table"));
			t.className = "ts";
			var tb = t.appendChild(document.createElement("tbody"));
			var row = tb.appendChild(document.createElement("tr"));
