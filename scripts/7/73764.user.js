 ...
 * Gracias Isoroku por reportar errores
*****************************************************************/
...
function esconderCol(col) {
	...
	if (esconder[col]) {
		...
		eval("mifila_texto"+(col-1)+".innerHTML = '[→]'");
...
function produccion(id, lvl) {
	...
	var factor = 1;
	var factor2 = 1;
	if (top.location.href.indexOf('perbverso') != -1) { 
		factor = 100;
		factor2 = factor/3;
	}
	switch(id)
	{
	case 1:
		return parseInt(factor * 150 * lvl *  Math.pow(1.1, lvl));
		break;
	case 2:
		return parseInt(factor * 100 * lvl *  Math.pow(1.1, lvl));
		break;
	case 3:
		return parseInt(factor * 50 * lvl *  Math.pow(1.1, lvl) * (-0.002 * tmax + 1.28));
		break;
	case 4:
	...
	case 22: case 23: case 24:
		return parseInt(factor2 * 150000 *  Math.pow(1.5, lvl));
		break;
	default:
		return null;
	}
...
function sumaTiempos() {
	...
	for (var i = 0; i < time.length; i++)
	{
		if (input2[i].innerHTML.indexOf('input') != -1) {
				input[i] = parseInt(input2[i].getElementsByTagName('input')[0].value);
...
*********************************/
var opciones = new Array('visor virtual', 'construcciones','produccion','hangar','puerto estelar','proteccion planeta', 'UgaMail');
var opcionesid = new Array('vvirtual', 'construcciones', 'produccion','hangar','puerto','defensa','mensajes');
 ...
 * Generales
*********************************/
if (top.location.href.indexOf("?mode=game") != -1 || top.location.href.indexOf('?option') != -1 || top.location.href.indexOf('?mode=fleet') != -1 || top.location.href.indexOf('?mode=messages') != -1) { 
	//ID que se repiten
...
if (top.location.href.indexOf("?mode=game-hangar") != -1 && configuracion[3] == 1) {
	...
	for (var i=1;i<=th.length;i += 2)
	{
		...
		texto.innerHTML = '<br><b> Tiempo total: </b>' + '<t title="Tiempo que tardaran en hacerse las naves seleccionadas" id="'+id[k]+'"> 0s </t>';
		//Añadimos el evento al formulario
		if (td[i].innerHTML.indexOf('input') != -1) {
			td[i].getElementsByTagName('input')[0].addEventListener('keyup',function(){sumaTiempos();},true);
			td[i].getElementsByTagName('a')[0].addEventListener('click',function(){sumaTiempos();},true);
			td[i].getElementsByTagName('a')[1].addEventListener('click',function(){sumaTiempos();},true);
			td[i].getElementsByTagName('a')[2].addEventListener('click',function(){sumaTiempos();},true);
			td[i].getElementsByTagName('a')[3].addEventListener('click',function(){sumaTiempos();},true);
		}
		input2[k] = td[i];
		k++
	}
...
if (top.location.href.indexOf("?mode=game-defense") != -1 && configuracion[5] == 1) {
	...
	for (var i=1;i<=th.length;i += 2)
	{
		...
		var texto = document.createElement('span');
		td[i+1].insertBefore(texto, td[i+1].lastChild);
		texto.innerHTML = '<br><b> Tiempo total: </b>' + '<t title="Tiempo que tardarán en construirse las naves seleccionadas" id="'+id[k]+'"> 0s </t>';
 ...
 * MENSAJES
*********************************/
if (top.location.href.indexOf("?mode=messages") != -1 && configuracion[6] == 1) {
	//Tabla principal
	...
	for (var i = 8; i < tr.length -3; i++) {
		var temp = '';
		if (tr[i].innerHTML.indexOf('<th><input') != -1) { //Cabecera
			th = tr[i].getElementsByTagName('th');
			if (th[3].innerHTML.indexOf('Informe de espionaje') != -1) { //Informe
				temp = tr[i+1].getElementsByTagName('td')[2].getElementsByTagName('a')[0].getAttribute('href');
				temp = temp.replace('game-galaxy','fleet');
				temp += '&t=3&m=8'; //Indicamos a escombros y reciclar
				var div = document.createElement('div');
				div.setAttribute('id','enlaces');
				div.setAttribute('align','right');
				var a = document.createElement('a');
				a.setAttribute('href',temp);
				a.innerHTML = '<t title="Ingresar las coordenadas en puerto estelar" ><font color ="lightblue"> Reciclar </font></t>';
				div.appendChild(a);
				tr[i+1].getElementsByTagName('td')[2].setAttribute('width',500);
				tr[i+1].getElementsByTagName('td')[2].innerHTML = tr[i+1].getElementsByTagName('td')[2].innerHTML.replace('<br>','');
				tr[i+1].getElementsByTagName('td')[2].insertBefore(div, tr[i+1].getElementsByTagName('td')[2].firstChild); //añadimos el enlace		
				temp = temp.replace('&t=3&m=8','&t=1&m=6'); //ponemos a planeta y Espiar
				var a2 = document.createElement('a');
				a2.setAttribute('href',temp);
				a2.innerHTML = '<t title="Ingresar las coordenadas en puerto estelar" class="yellow"> Espiar </t>';
				tr[i+1].getElementsByTagName('div')[0].appendChild(a2);
				tr[i+1].getElementsByTagName('div')[0].innerHTML = tr[i+1].getElementsByTagName('div')[0].innerHTML.replace('</a><a', '</a> | <a'); 
				temp = temp.replace('&m=6','&m=1'); //ponemos Atacar
				var a3 = document.createElement('a');
				a3.setAttribute('href',temp);
				a3.innerHTML = '<t title="Ingresar las coordenadas en puerto estelar" class="red"> Atacar </t>';
				tr[i+1].getElementsByTagName('div')[0].appendChild(a3);
				tr[i+1].getElementsByTagName('div')[0].innerHTML = tr[i+1].getElementsByTagName('div')[0].innerHTML.replace('</a><a', '</a> | <a'); 
