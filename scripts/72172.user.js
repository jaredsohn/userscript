...
// ==UserScript==
// @name            SLhunterSL
// @namespace       Sorority Life
// @description     Vote for all members in your Sorority Life house
// @copyright       2010, SLhunterSL
// @license         None licensed and open to public
// @version         1.0.0
// @require         http://userscripts.org/scripts/version/49700/156182.user.js
// @require         http://userscripts.org/scripts/version/50018/109807.user.js
...
var userscriptUpdater = function() {
    var css = "div.greasemonkey_updater { font-size: 12px; background: #111111; padding: 10px 15px; border-width: 1px 0; border-style: solid; border-color: black; margin: 0 0 30px; } " +
              ...
              "div.greasemonkey_updater .greasemonkey_updater_link_to_hide a { color: #white; } " +
              "div.greasemonkey_updater p { margin: 0 0 15px 0; font-size: 12px !important; line-height: 140%; color: hotpink; }"; // css 
...
// global private SLhunterSL script variables
var fadeMainPage = false;  // in case of GM_config
...
var hmi = {
	...
	GM_addStyle(
        ...
        GM_addStyle(
            ".aficithumb {font-style: normal; font-size: x-small; text-align: left; color: white;  padding-left:4px;}"
        ); // GM_addStyle
    }, // addStyle
    ...
    initialize: function() {
        ...
        if (!this.initialized) {
	        ...
	        var aficionadoBabbleDiv = aficionadoYacketyYakDiv.appendChild(
                ...
                aficionadoBabbleDiv.setAttribute('style', 'display: table;');

	        var aficionadoBabbleImg = aficionadoBabbleDiv.appendChild(
	            document.createElement('img'));
...
greeting: function() {
        ...
        slife.api.users_getInfo(uids, fields, function (result, e) {
            if (!e) {
                ...
                hmi.yacketyYak(pic_url, 'Hello ' + first_name + '!', 'SLhunterSL');
            }
        }); // api.users_getInfo
    }, // greeting
...
UserConfirmation: function (reason) {
        ...
        var messageStr = "The SLhunterSL script seems to be running already!";
        ...
        messageStr += "Running SLhunterSL in two (or more) tabs/windows in parallel ";
...
CheckPage: function (count) {
        ...
        var className = "notificationsButton clearfix";
        var q = document.evaluate(".//div[@class='" + className + "']",
            document.body, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
        if(q && q.singleNodeValue) {
            var notificationElt = q.singleNodeValue;
            notificationElt.style.top   = "2px";
            notificationElt.style.right = "2px";
        }
	...
	var nm = GM_getValue(my_user_id+"-nullMembers", 0);
        ...
        if (!votingPaused) {
            var comment = notice;
            comment = comment.replace("%N", "SLhunterSL");
            comment = comment.replace("%N", "SLhunteSL");
            comment = comment.replace("%N", "SLhunterSL");
            comment = comment.replace("%D", dayname);
            comment = comment.replace("%D", dayname);
            comment = comment.replace("%D", dayname);
        ...
        // greet the user
        window.setTimeout(function () { slife.greeting(); }, 0);
