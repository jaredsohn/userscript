// ==UserScript==
// @name          Watch SVTPlay and Oppetarkiv without Flash
// @description   Autoplay videos on SVTPlay.se and Oppetarkiv.se, with your browsers default video plugin.
// @namespace     http://elundmark.se/code/
// @license       CC0 1.0 Universal; http://creativecommons.org/publicdomain/zero/1.0/
// @version       1.1
// @date          2013-08-13
// @author        elundmark
// @contact       mail@elundmark.se
// @homepage      https://userscripts.org/scripts/show/175624
// @updateURL     https://userscripts.org/scripts/source/175624.meta.js
// @downloadURL   https://userscripts.org/scripts/source/175624.user.js
// @include       /https?://(www.)?(svtplay|oppetarkiv)\.se/.+/
// @match         http://www.svtplay.se/*
// @match         http://svtplay.se/*
// @match         http://oppetarkiv.se/*
// @match         http://www.oppetarkiv.se/*
// @match         https://www.svtplay.se/*
// @match         https://svtplay.se/*
// @match         https://oppetarkiv.se/*
// @match         https://www.oppetarkiv.se/*
// @require       https://cdnjs.cloudflare.com/ajax/libs/jquery/2.0.2/jquery.min.js
// @grant         GM_log
// @grant         unsafeWindow
// ==/UserScript==

(function (w, $) {
  "use strict";
  if ( !w || !$ ) return;
  // hopefully some more sites in the future
  if ( (~w.document.location.host.indexOf("svtplay")
    || ~w.document.location.host.indexOf("oppetarkiv"))
    && w.document.location.pathname.indexOf("/kanaler") !== 0 ) {
    var log = function (s) {
      if ( typeof w.console === "function" ) {
        w.console.log(s);
      } else if ( typeof GM_log === "function" ) {
        GM_log(s);
      } else if ( typeof TM_log === "function" ) {
        TM_log(s);
      } else {
        throw new Error(s);
      }
    };
    try {
      var loadingGif = "data:image/gif;base64,"
        ,i
        // try for 30 seconds, then give up
        ,tryForSeconds = 30
        ,checkDelay    = 500
        ,checkedTimes  = Math.round((tryForSeconds*1000)/checkDelay)
        ,playerSize    = {}
        ,isTM          = typeof TM_log === "function"
        ,isGM          = typeof GM_log === "function"
        ,checkInterval
        ,iosM3u
        ,embedUrl
        ,getM3uJson    = function (url) {
          var jqxhr = $.ajax(url);
          jqxhr.always(function (text, status) {
            var svtJSON
              ,svtJSONstr
              ,$svtJSONel
              ,$playerObject
              ,validM3u
            ;
            if ( text && status === "success") {
              $playerObject = $( $.parseHTML(text, false) )
                .find("#player object").has("param[name='flashvars']");
              $svtJSONel = $playerObject && $playerObject.length
                ? $playerObject.find("param[name='flashvars']") : null;
              svtJSONstr = $svtJSONel && $svtJSONel.length
                ? $svtJSONel.attr("value") : "";
              svtJSON = svtJSONstr ? JSON.parse( decodeURIComponent( svtJSONstr )
                .replace(/^[^{]+/,"") ) : {};
              if ( svtJSON && svtJSON.video && svtJSON.video.videoReferences ) {
                for ( i = 0; i < svtJSON.video.videoReferences.length; i++ ) {
                  if ( svtJSON.video.videoReferences[i].playerType
                    && svtJSON.video.videoReferences[i].playerType === "ios" ) {
                    validM3u = svtJSON.video.videoReferences[i].url;
                    break;
                  }
                }
                if ( validM3u ) {
                  iosM3u = validM3u;
                } else {
                  throw new Error("Error! Couldn't find the m3u file.");
                }
              }
            }
          });
        }
        ,embedPluginPlayer = function (dims, url, $target) {
          var embedHtml
            ,fixedHeight = dims.height + 18
            // last I checked there was a bug in the VLC plugin for Chrome on Linux
            ,pluginType    = isTM && !isGM ? "generic" : "vlc"
          ;
          // this is the best compromise I could do since setting a certain
          // plugin doesn't meen that one will be choosen by the (OS) broswer
          if ( pluginType === "vlc" ) {
            embedHtml = ["<embed type='application/x-vlc-plugin' pluginspage='http://www.videolan.org'"
              ," class='us_plugin_embeds' style='margin:0 auto; padding: 0; border: 0;' "
              ,"autoplay='yes' loop='no' width='" + dims.width + "' height='" + fixedHeight + "' "
              ,"target='" + url + "' toolbar='true' controls='true' bgcolor='#000000'/>"
              ,"<object classid='clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921' "
              ,"style='margin:0 auto; border: 0; padding: 0; width: 0; height: 0;' "
              ,"codebase='http://download.videolan.org/pub/videolan/vlc/last/win32/axvlc.cab'></object>"
            ];
          } else if ( pluginType === "generic" ) {
            embedHtml = ["<embed style='margin:0 auto; padding: 0; border: 0;' "
              ,"type='application/vnd.apple.mpegurl' name='plugin' class='us_plugin_embeds' "
              ,"autoplay='yes' loop='no' width='" + dims.width + "' height='" + fixedHeight
              ,"' src='" + url + "' bgcolor='#000000' toolbar='true' controls='true'/>"
            ];
          }
          // set the container to matching height, hide the anchor, and then embed
          $target.parents(".playVideoBox:eq(0)").css("height", fixedHeight + "px")
            .end().after(embedHtml.join("")).hide(0);
        },
        $playerLink = $("#player[data-json-href]")
      ;

      if ( $playerLink.length ) {
        playerSize.width = Math.round($playerLink.width());
        playerSize.height = Math.round($playerLink.height());
        $playerLink.find("img, span, div").css("display", "none").end().css({
          "background-image"     : "url(" + loadingGif + ")"
          ,"background-position" : "center center"
          ,"background-color"    : "#000000"
          ,"background-repeat"   : "no-repeat"
          ,"height"              : playerSize.height + "px"
        });
        checkInterval = setInterval(function () {
          w.console.log(embedUrl);
          if ( checkedTimes-- === 0 ) {
            w.alert("Failed to load video, try again or disable this plugin and try it with Flash");
            $playerLink.add($("#player img, #player span, #player div")).show(0);
            clearInterval(checkInterval);
          }
          $playerLink.find("img, span, div").css("display", "none");
          embedUrl = !embedUrl && $playerLink.length ? $playerLink.data("href") : "";
          embedUrl && (getM3uJson(embedUrl));
          if ( $playerLink.length && iosM3u ) {
            clearInterval(checkInterval);
            embedPluginPlayer(playerSize, iosM3u, $playerLink);
          }
        }, checkDelay);
      }

    } catch (error) {
      log(error);
    }
  }
}((unsafeWindow||window), jQuery));
