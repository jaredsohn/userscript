...
// @include        http://*sylt.pennergame.de/profil/bande*
...
// @version        4.3.7 Test auf Urlaub verbessert
// ==/UserScript==

// @version        4.3.6 Urlaubs- und Haustieranzeige (Begleiter) wieder aktiviert
// @version        4.3.5 kleine Erweiterungen für Fightinfo
// @version        4.3.4 Erkennung von gelöschten Pennern verbessert
// @version        4.3.3 Erkennung von Urlaubern verbessert
// @version        4.3.2 Fehler bei Vorhandensein von Banden-Designs behoben
// @version        4.3.1 erweitert für Sylt
// @version        4.3.0 Version für FF 17
// @version        4.2.5 Stadtteil wurde falsch ermittelt
// @version        4.2.4 Urlaubsbild jpg durch png ersetzt
// @version        4.2.3 noch einmal kleine Korrekturen
// @version        4.2.2 kleine Korrekturen; Urlaubsanzeige
// @version        4.2.1 Tiere Köln; div. Korrekturen; Tiererkennung neu programmiert
// @version        4.2.0 kleinere Korrekturen; läuft auch unter Chrome
// @version        4.1.6 Update-Verfahren verbessert; Abstimmung mit Fightinfo-Skript
// @version        4.1.5 an neues Bandenprofil angepasst; Include-Adressen angepasst
// @version        4.1.4 Grafik getauscht; Banden-Awards verschoben
// @version        4.1.3 Diverse Korrekturen
// @version        4.1.2 Grafiken auf anderen Server verlegt
// @version        4.1.1 farbige Schwerter werden wieder angezeigt
// @version        4.1.0 Skript nach Patch wieder um laufen gebracht
// @version        3.8.7 PG Hamburg reloaded eingebaut
// @version        3.8.6 small Bugfix gesch.
...
// @version        3.7.7 Elefant in Muenchen gefixt und noch Titel eingebaut
...
// @version        3.7.4 Tiger in Muenchen gefixt
// @version        3.7.3 Kleiner Fix... Mouseover jetzt ganz weg.
// @version        3.7.2 Platzierung und Beitritsdatum in die Bande jetzt auch dabei und es wird wieder jede 2. Zeile in einer anderen Farbe ausgegeben ohne das es vom Mouseover gekillt wird.
// @version        3.7.1 Bei den Tieren werden jetzt beim Mouseover die Werte mit angezeigt, Änderung bei gelöschten Spielern und noch ein Paar kleinere Änderungen.
...
var THISSCRIPTSOURCE_URL = 'http://userscripts.org/scripts/source/'+THISSCRIPTNUMMER; // Skript-URL bei userscripts.org
...
var linkbase = 'http://i1106.photobucket.com/albums/h362/we1hnachtsmannB/';
...
var lang = '';
if (url.indexOf('http://www.pennergame')>=0 || url.indexOf('http://pennergame')>=0) {
var town = 'HH';
...
var town = 'HW';
var link = "http://www.halloween.pennergame.de";
}
...
if (lang != '')
    var sig = 'http://inodes.pennergame.de/'+lang+'/signaturen/';
else
    var sig = '';

if (town == 'HH' || town == 'HR') {
    var tiere = new Array(new Array('00001', 'Kakerlake', 0, 0, 0),
                          new Array('68930', 'Goldfisch', 1, 1, 1),
                          new Array('11836', 'Maus', 2, 3, 7),
                          new Array('73308', 'Hamster', 5, 4, 12),
                          new Array('52483', 'Wellensittich', 7, 5, 16),
                          new Array('31451', 'Taube', 8, 3, 1),
                          new Array('73684', 'Ratte', 10, 5, 0),
                          new Array('77310', 'Hase', 13, 10, 17),
                          new Array('21903', 'Frettchen', 18, 15, 19),
                          new Array('73735', 'Katze', 25, 20, 32),
                          new Array('89386', 'Falke', 27, 25, 22),
                          new Array('61402', 'Schlange', 44, 38, 10),
                          new Array('62474', 'Hausziege', 36, 40, 21),
                          new Array('12758', 'Pudel', 28, 29, 62),
                          new Array('73933', 'Dressierte Maus', 43, 37, 253),
                          new Array('48263', 'Adler', 39, 41, 38),
                          new Array('09051', 'Sch&auml;ferhund', 55, 45, 43),
                          new Array('15240', 'Pitbull', 65, 59, 1),
                          new Array('62456', 'Cockerspaniel', 59, 40, 56),
                          new Array('32563', 'Chihuahua', 32, 28, 133),
                          new Array('90385', 'Pferd', 62, 69, 80),
                          new Array('98962', 'Giraffe', 71, 82, 98),
                          new Array('64220', 'Nilpferd', 74, 89, 34),
                          new Array('73953', 'Krokodil', 95, 75, 30),
                          new Array('43703', 'Tiger', 136, 46, 69),
                          new Array('12536', '&Auml;ffchen', 52, 43, 230),
                          new Array('25834', 'Nashorn', 100, 95, 39),
                          new Array('94826', 'Elefant', 110, 101, 42),
                          new Array('14896', 'Eisb&auml;r', 138, 87, 64),
                          new Array('mammut_xmas', 'Mammut', 138, 87, 67),
                          new Array('maskottchen', 'Maskottchen', 49, 40, 299));
}
else if (town == 'BL') {
    var tiere = new Array(new Array('48264', 'Silberfisch', 0, 0, 0),
                          new Array('75284', 'Grasfrosch', 1, 1, 1),
                          new Array('92653', 'Rotkehlchen', 2, 3, 7),
                          new Array('02634', 'Clownfisch', 5, 4, 12),
                          new Array('01743', 'Erdm&auml;nnchen', 7, 5, 16),
                          new Array('11542', 'M&ouml;we', 8, 3, 1),
                          new Array('66294', 'Opossum', 10, 5, 0),
                          new Array('11634', 'Streifenh&ouml;rnchen', 13, 10, 17),
                          new Array('11743', 'Igel', 18, 15, 19),
                          new Array('47838', 'Hausschwein', 25, 20, 32),
                          new Array('94652', 'Schneeeule', 27, 25, 22),
                          new Array('65384', 'Bisamratte', 136, 46, 69),
                          new Array('18540', 'Moorschnucke', 36, 40, 21),
                          new Array('76538', 'Yorkshire Terrier', 28, 29, 62),
                          new Array('73933', 'Dressierte Maus', 43, 37, 253),
                          new Array('64133', 'Habicht', 39, 41, 38),
                          new Array('48256', 'Border Collie', 55, 45, 43),
                          new Array('98641', 'Dogge', 65, 59, 1),
                          new Array('28463', 'Golden Retriever', 59, 40, 56),
                          new Array('32563', 'Mops', 32, 28, 133),
                          new Array('96242', 'Elch', 62, 69, 80),
                          new Array('85242', 'Zebra', 71, 82, 98),
                          new Array('99624', 'Kamel', 74, 89, 34),
                          new Array('13323', 'Schnappschildkr&ouml;te', 95, 75, 30),
                          new Array('83290', 'Leopard', 136, 46, 69),
                          new Array('37551', 'Waschb&auml;r', 52, 43, 230),
                          new Array('16342', 'Berliner-B&auml;r', 100, 95, 39),
                          new Array('88643', 'Tapir', 110, 101, 42),
                          new Array('73526', 'T-Rex', 138, 87, 64),
                          new Array('31337', 'Ph&ouml;nix', 146, 107, 95),
                          new Array('mammut_xmas', 'Mammut', 146, 107, 99));
}
else if (town == 'MU') {
    var tiere = new Array(new Array('00001', 'L&auml;use', 0, 0, 0),
                          new Array('68930', 'Schabe', 1, 1, 1),
                          new Array('11836', 'Hendl', 2, 3, 7),
                          new Array('73308', 'Ratte', 5, 4, 12),
                          new Array('52483', 'Zebramangusten', 7, 5, 16),
                          new Array('31451', 'Taube', 8, 3, 1),
                          new Array('73684', 'Emsen', 10, 5, 0),
                          new Array('77310', 'Schneehase', 13, 10, 17),
                          new Array('21903', 'Erdhenne', 18, 15, 19),
                          new Array('73735', 'Katze', 25, 20, 32),
                          new Array('89386', 'Steinadler', 27, 25, 22),
                          new Array('61402', 'Wolpertinger', 44, 38, 10),
                          new Array('62474', 'Mufflon', 36, 40, 21),
                          new Array('12758', 'Zamperl', 28, 29, 62),
                          new Array('73933', 'Dressierte Maus', 43, 37, 253),
                          new Array('48263', 'Oachkatzl', 39, 41, 38),
                          new Array('09051', 'G&auml;mse', 55, 44, 43),
                          new Array('15240', 'Facke', 65, 59, 1),
                          new Array('62456', 'Bisamratte', 59, 40, 56),
                          new Array('32563', 'Gartenschl&auml;fer', 33, 28, 133),
                          new Array('90385', 'Elwetritsch', 62, 69, 80),
                          new Array('98962', 'Giraffe', 71, 82, 98),
                          new Array('64220', 'Seel&ouml;we', 74, 89, 34),
                          new Array('73953', 'Krokodil', 95, 75, 30),
                          new Array('43703', 'Tiger', 136, 46, 69),
                          new Array('12536', 'Affe', 52, 43, 230),
                          new Array('25834', 'Nashorn', 100, 95, 39),
                          new Array('94826', 'Elefant', 110, 101, 42),
                          new Array('14896', 'Eisb&auml;r', 145, 98, 51),
                          new Array('mammut_xmas', 'Mammut', 145, 98, 55),
                          new Array('s1', 'Premium-Spatz', 3, 3, 6));
}
else if (town == 'KL') {
    var tiere = new Array(new Array('ameise', 'Ameise', 0, 0, 0),
                          new Array('regenwurm', 'Wurm', 1, 1, 1),
                          new Array('kroete', 'Kr&ouml;te', 2, 3, 7),
                          new Array('tarantel', 'Tarantula', 5, 4, 12),
                          new Array('Fledermaus', 'Fledermaus', 7, 5, 16),
                          new Array('kaninchen', 'Kaninchen', 8, 3, 1),
                          new Array('73684', 'Ratte', 10, 5, 0),
                          new Array('ente', 'Ente', 13, 10, 17),
                          new Array('kakadu', 'Kakadu', 18, 15, 19),
                          new Array('eichhoernchen', 'Eichh&ouml;rnchen', 25, 20, 32),
                          new Array('aal', 'Aal', 27, 25, 22),
                          new Array('malteser', 'Malteser', 44, 38, 10),
                          new Array('biber', 'Biber', 36, 40, 21),
                          new Array('dachs', 'Dachs', 28, 29, 62),
                          new Array('73933', 'Dressierte Maus', 43, 37, 253),
                          new Array('waschbaer', 'Waschb&auml;r', 39, 41, 38),
                          new Array('schnauzer', 'Schnauzer', 55, 45, 43),
                          new Array('pinguin', 'Pinguin', 65, 59, 1),
                          new Array('wildkatze', 'Wildkatze', 59, 40, 56),
                          new Array('luchs', 'Luchs', 32, 28, 133),
                          new Array('wolf', 'Wolf', 62, 69, 80),
                          new Array('kondor', 'Kondor', 71, 82, 98),
                          new Array('orang_utan', 'Orang-Utan', 74, 89, 34),
                          new Array('geissbock', 'Gei&szlig;bock', 95, 75, 30),
                          new Array('kaiman', 'Kaiman', 136, 46, 69),
                          new Array('jaguar', 'Jaguar', 52, 43, 230),
                          new Array('25834', 'Nashorn', 100, 95, 39),
                          new Array('94826', 'Elefant', 110, 101, 42),
                          new Array('14896', 'Eisb&auml;r', 138, 87, 64));
}
...
for (x = tables.length-1; x > 0; x--) {
   if (document.getElementsByTagName("table")[x].innerHTML.indexOf("profil/id") != -1)
      break;
}
var table = document.getElementsByTagName("table")[x-1];
...
if (tr[0].getElementsByTagName("td")[0].getElementsByTagName("table").length > 1) {
    var td = tr[0].getElementsByTagName("td")[0].removeChild(tr[0].getElementsByTagName("td")[0].getElementsByTagName("table")[1]);
    var newtr = document.createElement("tr");
    newtr.appendChild(td);
    table.appendChild(newtr);
}
table = document.getElementsByTagName("table")[x];
...
GM_xmlhttpRequest({
  ...
  onload: function(responseDetails) {
      ...
      var myid = dom.getElementsByTagName('uid')[0].getAttribute('value');
      ...
      GM_xmlhttpRequest({
          ...
          onload: function(responseDetails) {
            ...
            var minPts = pts - pts * 0.2;
            var maxPts = pts + pts * 0.5;
            var xx = 0;
            var exhist = false;
            for (var x = 1; x < tr.length; x++) {
                if (tr[x].innerHTML.indexOf("profil/id") == -1)
                   continue;
                if (!tr[x].id)
                   continue;
                if (xx == 0)
                    exhist = tr[x].innerHTML.indexOf('id="hist') != -1;
                xx++;
                tr[x].setAttribute('name', 'tr'+x);
                if (xx%2 == 0){
                  tr[x].setAttribute('style', 'background-image: url('+linkbase+'bgeven.png);');
                }
                else {
                  tr[x].setAttribute('style', 'background-image: url('+linkbase+'bgodd.png);');
                }
                var text1 = tr[x].getElementsByTagName("td")[1].innerHTML.split('/profil/id:');
                var id = text1[1].split('/"');

                if (tr[x].getElementsByTagName('td').length > 3 || exhist) {
                    var tds = tr[x].getElementsByTagName('td')[1].getElementsByTagName('td').length + 2;
                    tr[x].getElementsByTagName('td')[tds].setAttribute('style', 'display:none');
                }

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'rang'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'reg'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'joined'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'stadt'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'geld'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'promille'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'tier'+xx);
                newtd.setAttribute('style', 'padding-top:5px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'onlinestatus'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'schlecht'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:center;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'angreifen'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'nachricht'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'freundesch'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'poebel'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:right;');

                var newtd = tr[x].appendChild(document.createElement('td'));
                newtd.setAttribute('id', 'urlaub'+xx);
                newtd.setAttribute('style', 'padding-top:6px; text-align:right;');

                Geldladen(myid, pts, minPts, maxPts, id[0],xx);
            }

            var newtr = document.createElement('tr');
            newtr.setAttribute('style', 'background-image: url('+linkbase+'bghead.png);');
            ...
            newtd = newtr.appendChild(document.createElement('td'));
            newtd.setAttribute('style', 'width:50px;font-weight:bold;text-align:right;');
            newtd.innerHTML = "<u>Platz</u>";
            newtd = newtr.appendChild(document.createElement('td'));
            ...
            newtd.setAttribute('style', 'width:25px;font-weight:bold;text-align:center;');
            newtd.innerHTML = "&nbsp;";
            table.getElementsByTagName('tbody')[0].insertBefore(newtr, tr[1]);
          }
      });
  }
});

function Geldladen(myid, pts, minPts, maxPts, id, x) {
    GM_xmlhttpRequest({
        method: 'GET',
        ...
        onload: function(responseDetails) {
            var parser = new DOMParser();
            var dom = parser.parseFromString(responseDetails.responseText, "application/xml");
            if (document.getElementById("points"+x))
                var points = parseInt(document.getElementById('points'+x).innerHTML.replace(/[\.,]/g, ''));
            else
            ...
            document.getElementById('rang'+x).innerHTML ='<div style="width:50px">'+rang+'</div>';
            ...
            }else if ((minPts <= points) && (points <= maxPts) && (points - points * 0.2 > pts)) {
            ...
            }else if ((points - points * 0.2 <= pts) && (pts <= points + points * 0.5)){
    ...
    onload: function(responseDetails) {
        ...
        //----------------------------------//
        try {
            if (content.indexOf('margin-top:12px;">') != -1)
                var hausi5 = content.split('margin-top:12px;">')[1];
            else
                var hausi5 = content.split('class="petimg')[1];
            var hausi3 = hausi5.split('</div>')[0];

            if (hausi3.indexOf('<img src="') != -1) {
                var hausi4 = hausi3.split('<img src="')[1];
                var hausi2 = hausi4.split('"')[0];
            }
            else {
                var hausi4 = hausi3.split('url(')[1];
                var hausi2 = hausi4.split(')')[0];
            }

            var petname = "";
            for (var i = 0; i < tiere.length; i++)
                if (hausi2 == statics+'shop/'+lang+'/tiere/'+tiere[i][0]+'.jpg') {
                    petname =       tiere[i][1];
                    var att = ' ' + tiere[i][2] + ' ';
                    var def = ' ' + tiere[i][3] + ' ';
                    var dex = ' ' + tiere[i][4] + ' ';
            ...
            if (petname != "")
                var tiertooltip = '<font style=\"color:yellow; font-size:140%;\"><b>'+petname+':</b></font><br>'+
                                  '<img src="'+hausi2+'" height="100" width="89"></img><br>'+
                                  '<u><b>Werte:</b></u><br>'+ATT+att+DEF+def+DEX+dex;
            else
                var tiertooltip = 'N/A';
            var tierzusammenbauen = '<a class="tooltip"><font style=\"color:yellow; font-size:100%;\">'+petname+'</font>'+
                                    '<span style= "width: 140px;"><center>'+tiertooltip+'</center></span></a>';


            //----------------------------------------------------//
            // Selbsterstelltes Haustier ermitteln                //
            //----------------------------------------------------//
    ...
    //              tr[x].setAttribute('style', 'background:red;');
                    document.getElementsByName('tr'+x)[0].setAttribute('style', 'background-image: url('+linkbase+'bgred.png);');
            }
            else {
                var location1 = content.split('class="row1"')[3];
                location1 = location1.split('bgcolor="#232323">')[2];
                var location3 = location1.split('</td>')[0];
            }
        }
        catch(e) {
            var location3 = '<div><font style=\"font-size:100%;\"><b>Premium Profil</b></font></div>';
        }
        ...
        if (suche != -1)
            suche = (content.indexOf("messages/write/?to=" + id, suche)!=-1 || content.indexOf('class="profil_button mail')!=-1)?-1:1;
        try{
            if (suche != -1) {
    ...
    //            tr[x].setAttribute('style', 'background:red;');
        ...
        //----------------------------------------------------------------------------------------------------//
        // Geschlecht oder angelegter Plunder der sich auf Profil auswirkt ermitteln                          //
        //----------------------------------------------------------------------------------------------------//

        try {

            suche = content.split('class="sex"');
            geschlecht = suche[0].substr(-100).split('src="')[1].split('"')[0];

            if(geschlecht.indexOf('icons/female.jpg') != -1) {var geschpic = linkbase+"female.png";}
            else if(geschlecht.indexOf('icons/male.jpg') != -1) {var geschpic = linkbase+"male.png";}
            else if(geschlecht.indexOf('plunder/nikolaus.png') != -1) {var geschpic = ""+statics+"plunder/nikolaus.png";}
            else if(geschlecht.indexOf('plunder_new/old/valday.png') != -1) {var geschpic = ""+statics+"plunder_new/old/valday.png";}
            else if(geschlecht.indexOf('plunder/cake.png') != -1) {var geschpic = ""+statics+"plunder/cake.png";}
            else if(geschlecht.indexOf('premiummedia/img/js/swfobject.js') != -1) {var geschpic = linkbase+"premium.png";}
            else if(geschlecht.indexOf('events/easter/profil_wiesendeko.png') != -1) {var geschpic = linkbase+"premium.png";}

            if(geschpic == linkbase+'premium.png'){var geschpictooltip = "Premium Profil";}
            else if(geschpic == ''+statics+'plunder/nikolaus.png'){var geschpictooltip = "Rote Schleife";}
            else if(geschpic == ''+statics+'plunder_new/old/valday.png'){var geschpictooltip = "Be my Valentine!";}
            else if(geschpic == ''+statics+'plunder/cake.png'){var geschpictooltip = "Happy Birthday!";}
            else if(geschpic == linkbase+'female.png'){var geschpictooltip = "Frau";}
            else if(geschpic == linkbase+'male.png'){var geschpictooltip = "Mann";}
            ...
            if (content.indexOf("buch_s.png") != -1) {var geschpic = statics+"events/buch_s.png";var geschpictooltip = "Goldenes Buch";}
            if (geschpic == "") {
                var geschlecht_image = '<font style=\"color:green; font-size:15px;\"><b>P</b></font>';
                var geschpic = linkbase+"premium.png";
            }
            else
                var geschlecht_image = '<div style="display:inline-block;"><img src="'+geschpic+'" height="15" width="15" title="'+geschpictooltip+'"></img></div>';
            }
        ...
        try{
            if (suche != -1) {
                ...
                var online2a =linkbase+"offline.png";
        ...
        //---------------------------------//
        // Urlaubsstatus ermitteln          //
        //---------------------------------//

        var suche = content.lastIndexOf('class="profil_button mail');
        if (suche == -1)
            suche = content.lastIndexOf("messages/write/?to=" + id);
	else
            suche = content.substr(0, suche).lastIndexOf("<table"); 
        if (suche != -1)
            suche = content.substr(suche).search("befindet sich im Urlaub");
        try{
            if (suche != -1) {
                var urlaub ="Spieler ist im Urlaub"
                document.getElementById('urlaub'+x).innerHTML ="<img src=\""+linkbase+"urlaub.png\" style=\"width:20px; height:20px\" title=\""+urlaub+"\"></img>";
            }
            else {
                var urlaub ="&nbsp;"
                document.getElementById('urlaub'+x).innerHTML ='<div style="width:25px">'+urlaub+'</div>';
            }
    }
        catch(e){
        }
