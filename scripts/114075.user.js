...
// @include        http://armonik.sozlukspot.com/*
// @include        http://www.armoniksozluk.com/*
// ==/UserScript==
var mevcutVersion = "0.2.2";
if(localStorage.getItem("armonikReserved")==null) localStorage.setItem("armonikReserved", "|capua genclik|");	//ilk kurulum ayari
if(localStorage.getItem("armonikSakat")==null) localStorage.setItem("armonikSakat", "|behcet oturucum acti |");		//ilk kurulum ayari

if(localStorage.getItem("armonikReserved").substr(0, 1)!="|" && localStorage.getItem("armonikReserved").length>0)
localStorage.setItem("armonikReserved", "|" + localStorage.getItem("armonikReserved"));

if(localStorage.getItem("armonikReserved").substr(localStorage.getItem("armonikReserved").length-1, 1)!="|" && localStorage.getItem("armonikReserved").length>0)
localStorage.setItem("armonikReserved", localStorage.getItem("armonikReserved") + "|");

var tarih = new Date();
var bugun = tarih.getTime();
var miniPopup = 0;
sayfaList = localStorage.getItem("armonikReserved");
...
// eğer buraya girildiyse orta frame yüklenmiş demektir
	baslikAdi = "";
	baslikAdi = document.title.replace("armonik sozluk - ", "");
	document.title = baslikAdi + " - armonik sozluk";
...
//	if(gelenAdres!="http://armonik.sozlukspot.com/" && gelenAdres!="http://www.armoniksozluk.com")	funCerceveGetir();
...
if(typeof(unsafeWindow)=='undefined')
{
	...
	{
		var sakatListesi = localStorage.getItem("armonikSakat");
		...
		for(var i=0; i <= kacTaneSakatEklenecek; i++)
		{
			if(eklenecekSakatListesi[i].length > 3)
			{
				...
				if(sakatListesi.indexOf("|" + siradakiSakat + "|")==-1)
				{
					...
					localStorage.setItem("armonikSakat", sakatListesi);
					sakatListesiGetir();
				}
			}
		}
	...
	function reserveBosalt()
	{
		...
		{
			localStorage.setItem("armonikReserved", "");
	...
	function sakatBosalt()
	{
		...
		{
			localStorage.setItem("armonikSakat", "")
			document.body.innerHTML = "bu liste bomboş.";
		}
	}	
	...
	function etGoster(gosterilecekEt, hangiBaslik)
	{
		...
		kacarmonikSayfa = ((gosterilecekEt - (gosterilecekEt % 25)) / 25) + 1;
                if(gosterilecekEt % 25==0) kacarmonikSayfa--;
		adres = "http://" + location.hostname + "/w/" + hangiBaslik.replace(/ /g, "-") + "/";
		adres += kacarmonikSayfa + "/";
	...
	function funSakatSil(kim)
	{
		...
		{
			var eskiSakatList = localStorage.getItem("armonikSakat");
			var yeniSakatList = eskiSakatList.replace(kim + "|", "");
			if(yeniSakatList.substr(-1)!= "|") yeniSakatList += "|";
	
			localStorage.setItem("armonikSakat", yeniSakatList);
	...
	function funSakatYap(kim)
	{
		...
		{
			if(localStorage.getItem("armonikSakat").length > 0)
			{
				var eskiSakatList = localStorage.getItem("armonikSakat");
				localStorage.setItem("armonikSakat", eskiSakatList + kim + "|");
				parent.frames[2].location.reload();
			}else{
				localStorage.setItem("armonikSakat", "|" + kim + "|");
				parent.frames[2].location.reload();
			}
		}
	}
	...
	function reservedSayfalariEkle(sayfaList)
	{
		...
		var sayfaBolunmus = localStorage.getItem("armonikReserved").split("|");
		var temaKonumu = parent.frames[0].document.head.innerHTML.indexOf("<link rel=\"stylesheet\"");
		var iconKonumu = parent.frames[0].document.head.innerHTML.indexOf('<script type=\"text/javascript\"');
		...
		for(var i=0; i<=localStorage.getItem("armonikReserved").split("|").length-1; i++)
		{
	...
	function toggleReserve(baslik, x)
	{
		...
		if(localStorage.getItem("armonikReserved").length != 0)
		{
			if(sayfaList.indexOf(baslik + "|")==-1)
			{
				sayfaList += baslik + "|";
				localStorage.setItem("armonikReserved", sayfaList);
...
{	
	var sayfaList = localStorage.getItem("armonikReserved");
...
function funResimGom()
{
	...
	for(i=0;i<linkSayisi;i++)
	{
		...
		if((mevcutLink.href.indexOf(".png")>-1 || mevcutLink.href.indexOf(".jpg")>-1 || mevcutLink.href.indexOf(".jpeg")>-1 || mevcutLink.href.indexOf(".gif")>-1) && (linkListesi[i-1].id!=rastgele && mevcutLink.href.substr(-1)!="/") && mevcutLink.href.indexOf("armonikcaps.in")==-1)
		{
...
{
	sakatList = localStorage.getItem("armonikSakat");
...
//	if(bugun - localStorage.getItem("armonikUpdate") > 86400000 || localStorage.getItem("armonikUpdate")==null)	// 86400000 = 24 * 60 * 60 * 1000ms = 1 gun
//	{
	var tarayici = detectBrowser();
	if(tarayici!="opera")									// chrome veya firefox ise
	{
		localStorage.setItem("armonikUpdate", bugun);
		GM_xmlhttpRequest({
			method: 'GET',
...
function funEtGetir()
{
	...
	var sayfaBaslik  = document.title.replace("armonik sozluk - ", "").replace(" - armonik sozluk", "");
