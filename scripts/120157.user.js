// ==UserScript==
// @namespace       heroeswm
// @name            HWM Object Work
// @description     Удобное устройство на работу.
// @version         5.5.3p
...
if (window === top) {
    ...
    var desc = {
        'details':
              '<ul>'
            + '<li>Позволяет запомнить код устройства на работу на объекте с малой зарплатой чтобы успеть устроиться на объект с большой зарплатой.</li>'
            + '<li>Позволяет запомнить код устройства на работу чтобы не вводить его каждый раз заново при неудачной попытке устроиться (когда быстро кончается золото или рабочие места на объекте).</li>'
            + '<li>Позволяет отображать фактическую зарплату на объекте (с учетом уровня в гильдии рабочих).</li>'
            +'</ul>',
    ...
    var conf = {
        ...
        //'autoMode': {'type': 'bool', 'title': 'Автоматический режим', 'def': false, 'desc': 'Работа в автоматическом режиме.'},
        'replaceMode': {'type': 'bool', 'title': 'Режим замены', 'def': false, 'desc': 'Режим замены оригинальной формы для устройства на работу.'},
        'showTimers': {'type': 'bool', 'title': 'Отображение таймеров', 'def': true, 'desc': 'Отображать всплывающее окно с таймером до возможности следующего устройства на работу.'},
        'bothPayment': {'type': 'bool', 'title': 'Обе зарплаты', 'def': false, 'desc': 'Отображать номинальную и фактическую зарплаты одновременно.'},
        'payment': {'title': 'Зарплата', 'desc': 'Минимальная зарплата для устройства на работу.'},
        'code': {'title': 'Код', 'desc': 'Код для устойства на работу.'}
    }
...
if (!disabled) {
    ...
    // Если мы уже работаем
    if (window !== top) {
        if (GM_getValue('work', false)) {
            var popup = tgui.getPopup(['Work']);
            if (popup.length > 0) {
                var status = GM_getValue('status', PageState.getPageTitle() + '.');
                top.document.title = status.substr(0, status.length - 1);
                tgui.destroyPopup(['Work']);
            }
        }
    }
    ...
    // Если мы отправляли код (для устройства на работу)
    if (url.attr('path') == '/object_do.php') {
        
        var send = GM_getValue('send', false);
        var objectId = url.param('id');
        var objectPayment = GM_getValue('payment_' + objectId);

        // А мы устроились на работу?
        var text = document.body.textContent;
        
        // Возможные сообщения
        var messages = [
            'Вы устроены на работу.',
            'Введен неправильный код.',
            'На объекте недостаточно золота.',
            'Нет рабочих мест.'
        ];
        
        var status = null;
        for (var i in messages) {
            var re = new RegExp(messages[i]);
            if (re.test(text)) {
                status = messages[i];
                break;
            }
        ...
        // Если мы устроились на работу
        if (status === messages[0]) {
            GM_setValue('send', false);
            GM_setValue('work', true);
            GM_setValue('time', Time.toNumber());
        }
        
        var unexpected = false;
        if (status === null) {
            status = 'Непредвиденный вывод.';
            unexpected = true;
        }
        
        GM_log(Time.toString() + ' ' + 'Объект №' + objectId + ' (зарплата ' + objectPayment + '). ' + status);
        if (unexpected)
            GM_log(String.trim(text).replace(/\s\s+/g, ' '));
        
        // Если мы устроены на работу или нет рабочих мест - подчищаем код
        if (i < 2)
            GM_setValue('code', '');
            
        // Если мы устроены на работу - удаляем каптчу
        if (i < 1)
            GM_setValue('captcha', '');
    ...
    // Если мы заходили на объект (посмотреть места)
    if (url.attr('path') == '/object-info.php') {
        
        
        // Код и минимальная зарплата
        var code = GM_getValue('code', '');
        var payment = GM_getValue('payment', 0);
        var autoMode = GM_getValue('autoMode', false);
        var replaceMode = GM_getValue('replaceMode', false);
        
        var captcha = GM_getValue('captcha', '');
        var playerId = GM_getValue('playerId', '');
        var objectId = GM_getValue('objectId', '');
        var codeId = GM_getValue('codeId', '');
        ...
        */
        function calcRealPayment(payment, guildLevel) {
            ...
            guildLevel = parseInt(guildLevel);
            var mod = null;
            switch (guildLevel) {
                case 0: mod = 1.0;
                case 1: mod = 1.1;
            ...
            return Math.floor(payment * mod);
        }
        
        
        // Добавляем информацию о фактической зарплате
        var guildLevel = GM_getValue('guildLevel', 0);
        if (parseInt(guildLevel) > 0) {
            
            var nodePayment = XPath.findFirst("//td[text()='Зарплата: ']");
            var valPayment = parseInt(nodePayment.nextSibling.textContent.replace(',', ''));
            var realPayment = calcRealPayment(valPayment, guildLevel);
            
            var bothPayment = GM_getValue('bothPayment', false);
            if (bothPayment) {
                nodePayment.parentNode.innerHTML += 
                    '<td>&nbsp;&nbsp;&nbsp;</td>' + 
                    '<td>(</td><td>' + Auction.htmlPrice(realPayment, undefined, true) + '</td><td>)</td>';
            } else {
                nodePayment.nextSibling.innerHTML = Auction.htmlPrice(realPayment, undefined, true);
            }
            
        }

        // Парсим тип объекта
        var re = new RegExp('Тип:\\s+([^<]+)<br>', 'ig');
        var res = re.exec(document.body.innerHTML);
        objectType = (res != null) ? res[1] : null;
        
        // Парсим зарплату
        var re = new RegExp('Зарплата:\\s+(\\d+)');
        var res = re.exec(document.body.textContent);
        objectPayment = (res != null) ? parseInt(res[1]) : null;
        if (objectPayment !== null)
            GM_setValue('payment_' + url.param('id'), objectPayment);
        //GM_log(Time.toString() + ' ' + 'На объекте №' + url.param('id') + ' ' + objectType + ' определена зарплата: ' + objectPayment + ' золота.');
              
        
        // Ищем форму
        // <form method="POST" action="object_do.php" name="working">
        //var form = $('form[name="working"][action="object_do.php"][method="POST"]');
        var form = null;
        for (var i = 0; i < document.forms.length; i++) {
            if (document.forms[i].name == 'working') {
        ...
        */
        function sendWork() {
            ...
            // Запоминаем что мы хотели работать
            GM_setValue('send', true);
            
            // http://www.heroeswm.ru/object_do.php?id=286&code=UNGKHU&code_id=c7184d0f8c904a99769e82ec018f708a&pl_id=3947350&rand1=0.23384&rand2=157.6670
            // Отправляем код (для устройства на работу)
            GM_log(Time.toString() + ' ' + 'Устраиваемся на работу на объект №' + objectId + '.');
            document.location.replace(
                url.attr('protocol') + '://' + url.attr('host') + '/object_do.php?id=' + 
                objectId + '&code=' + code + '&code_id=' + codeId + '&pl_id' + playerId
            );
        ...
        * @param Element input
        * @param sendNow Устроиться сейчас
        */
        function onButtonClick(input, sendNow) {
            
            var value = input.value;
            GM_log(Time.toString() + ' ' + 'Установен код: \'' + value + '\'');
            GM_setValue('code', value);
        ...
        * @param sendNow Устроиться сейчас
        */
        function onKeyPress(e, input, sendNow) {
            e = e || top.event;
            var key = e.which || e.keyCode;
            switch (key) {
                // Клавиша Enter
                case 13:
                    onButtonClick(input, sendNow);
                    break;
            }
        }
        ...
        */
        function onKeyUp(input) {
            var ru = new Array('й','ц','у','к','е','н','г','ш','щ','з','ф','ы','в','а','п','р','о','л','д','я','ч','с','м','и','т','ь');
            var en = new Array('q','w','e','r','t','y','u','i','o','p','a','s','d','f','g','h','j','k','l','z','x','c','v','b','n','m');
            var s1 = input.value;
            var s2 = '';
            for (var i = 0; i < s1.length; i++) {
                var c = s1[i].toLowerCase();
                for (var j = 0; j < ru.length; j++)
                    if (c == ru[j])
                        c = en[j];
                s2 += c;
            }
            input.value = s2.toUpperCase();
        }
        
        
        /**
        */
        function createTable(goodWork) {
            ...
            img.style.border = 'solid 1px #сссссс';
            img.src = captcha;
            var button = top.document.createElement('input');
            button.type = 'button';
            button.value = (goodWork ? 'Устроиться' : 'Запомнить');
            button.style.border = 'solid 1px #999999';
            ...
            button.onclick = function() {onButtonClick(input, goodWork);};
            input.onkeypress = function(e) {onKeyPress(e, input, goodWork);};
            input.onkeyup = function() {onKeyUp(input);};
            
            table.rows[0].cells[0].appendChild(img);
            table.rows[1].cells[0].appendChild(input);
            table.rows[1].cells[0].appendChild(button);
            table.rows[2].cells[0].innerHTML = 
                '<br/>' + 
                '<a href="object-info.php?id=' + objectId + '" style="text-decoration:none;">' + 
                objectType + ' (' + objectPayment + ' золотых)' + 
                '</a>';
        ...
        if (form != null) {
            ...
            GM_setValue('work', false);
            
            
            // <img width="250" height="60" border="0" name="imgcode" src="work_codes/15610-79/3951336--374643.jpeg">
            //var captcha = form.('img[name="imgcode"]').attr('src');
            var captcha = form.getElementsByTagName('img')[0].src;
            //GM_log(Time.toString() + ' ' + 'На объекте №' + url.param('id') + ' Найдена каптча: ' + '\n' + captcha);
            GM_setValue('captcha', captcha);
            
            
            // <param value="params=2942734|15|0374d6858e39f1cd9fd6252ac8c915f3|" name="FlashVars">
            //var params = form.('param[name="FlashVars"]').attr('value');
            var params = form.getElementsByTagName('param');
            for (var i = 0; i < params.length; i++)
                if (params[i].name == 'FlashVars')
                    break;
            
            var re = /^params=(\d+)\|(\d+)\|([^\|]+)\|$/ig;
            var matches = re.exec(params[i].value);
            var playerId = matches[1];
            var objectId = matches[2];
            var codeId = matches[3];
            GM_setValue('playerId', playerId);
            GM_setValue('objectId', objectId);
            GM_setValue('codeId', codeId);
            
            
            var goodWork = (
                (parseInt(objectPayment) >= parseInt(payment))
            );
            ...
            if (window != top) {
                ...
                if (popup.length === 0) {
                    ...
                    top.document.title = 'Работа';
                    var table = createTable(false);
                    var popup = tgui.createPopup(['Work'], 'Работа');
                    $('.Body', popup).append(table);
                    
                } else {
                
                    // Обновляем картинку капчи
                    $('img', popup).attr('src', captcha);
                    $('input[type="text"]', popup).attr('value', code);
                ...
                if (replaceMode) {
                    ...
                    var table = createTable(goodWork);
                    ...
                    var b = top.document.createElement('b');
                    b.textContent = 'Устройство на работу';
            ...
            // Если автоматический режим включен, код введен и зарплата устраивает
            if (autoMode)
                if ((goodWork) && (code.length > 0))
                    sendWork();
            ...
            if (window === top) {
                ...
                if (captcha.length > 0) {
                    ...
                    if (replaceMode) {
                        ...
                        try {
                            var xpath = "/html/body/center/table/tbody/tr/td/table/tbody/tr/td/div";
                            var node = XPath.findFirst(xpath, top.document);
    ...
    if (window !== top) {
        ...
        var showTimers = GM_getValue('showTimers', true);
        var timeWork = GM_getValue('time', null);
        
        if ((showTimers) && (timeWork !== null)) {
            timeWork = Time.fromNumber(timeWork);
            timeNow = new Date();
            
            var delay = timeWork.getTime() + 1000 * 60 * 60 - timeNow.getTime();
            var sign = (delay < 0) ? false : true;
            var text = Time.delayString(Math.abs(delay));
            
            if (sign) {
                text = 'вы сможете устроиться на работу через ' + text + '.';
            } else {
                text = 'вы нигде не работаете уже ' + text + '.';
            }
            
            tgui.setPopupTimer('ObjectWork', '<b>Гильдия рабочих</b>: ' + text);
        }
