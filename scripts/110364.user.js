...
function incrementDay() {
    ...
    if (getAge() >= extended_plateau_age) {
	    if ((getLevel()+getBoosts()) < 77) {
	    	// plateau players can boost 3 times
	    	var addedBoosts = Math.min(77-getBoosts()-getLevel(), 3);
...
function enhanceTraining() {
	...
	for (var a=0; a<attributeTrainingOptions.length; a++) {
		...
		if (current<50) {
			promptMsg += '\n'+att+"\t"+(att!="confidence"?"\t":"")+current+"% > "+(current+10)+"%\tCost:"+ (current+12)/2 +" BT";
		} else {
...
function multiTraining() {
	...
	var cost = 6 + getListOfMultiTrainAttributes().length*5;
...
// before changing, multipliers were 1.68,1.92,2.08 or 1.05,1.2,1.3
...
function multiTrain() {
	...
	if (cost > getTP()) {
    	...
    	// before changing back to 1.6 values are 1.68,1.92,2.08 or 1.05,1.2,1.3 previous
