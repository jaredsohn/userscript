...
if(location.pathname != "/scratch.php") { //Check jail or other stupid page
	...
	if(document.body.innerHTML.search('/static/images/userbadges/donateplus.png')!=-1) { //clicklimit
		...
		if(old && document.body.innerHTML.indexOf('color="red"')!=-1) {
			...
			if(num != 10) { setTimeout(click, 1500); } else { setTimeout(click, 60000); }
			...
			if(num < 10) {
				...
				for(i=0; i<input.length; i++){
					if(input[i].getAttribute('value') == 'Raspar!') {
		...
		box.innerHTML += "<br><br><b>Stats All-time:</b><br><br>Cards scratched: " + number_format(cards, 0, '.', '.') + " ($ " + (number_format(cards*5000, 0, '.', '.')) + " spent)

<br>Wins: " + number_format(wins, 0, '.', '.') + "<br>Money won: $ " + number_format(money, 0, '.', '.') + " <br>Bullets won: " + number_format(bullets, 0, '.', '.') + " <br>Win ratio: " + number_format

(((wins/cards)*100), 2, ',', ',') + "%<br>Money / card: $ " + (Math.round(money/cards)) + "<br>Bullets / card: " + (Math.round(bullets/cards)) + " <br>Profit: $ " + number_format((money + 

(bullets*750) - (cards*5000)), 0, '.', '.') + " (750 p/b) <br>Price / bullet: $ " + number_format(calc / bullets, 0, '.', '.');
		box.innerHTML += "<br><br><b>Stats Today:</b><br><br>Cards scratched: " + number_format(dCards, 0, '.', '.') + " ($ " + (number_format(dCards*5000, 0, '.', '.')) + " 

spent)<br>Wins: " + number_format(dWins, 0, '.', '.') + "<br>Money won: $ " + number_format(dMoney, 0, '.', '.') + " <br>Bullets won: " + number_format(dBullets, 0, '.', '.') + " <br>Win ratio: " + 

number_format(((dWins/dCards)*100), 2, ',', ',') + "%<br>Money / card: $ " + (Math.round(dMoney/dCards)) + "<br>Bullets / card: " + (Math.round(dBullets/dCards)) +" <br>Profit: $ " + 

number_format((dMoney + (dBullets*750) - (dCards*5000)), 0, '.', '.') + " (750 p/b) <br>Price / bullet: $ " + number_format(calc2 / dBullets, 0, '.', '.') + " <br>";
