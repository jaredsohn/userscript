// ==UserScript==
// @name           WEAPON V20120114
// @namespace      WEAPON
// @version        WEAPON V20120114
...
'U17', 'Predator drones'
'U9': 'Heli-loads',
'U11', 'combat helicopter'
'U10', 'Fighter'
'U12', 'Bomber',
'U19': 'freight transport',
'U16', 'Hellfire armor'
'U13', 'Stealth Bomber'
/ / 'U15': 'tactical nuke'
/ / 'U20', 'Orb. Ion Cannon '
...
out.push ('Automatic <b> troop training and automatic construction of the rampart of defense </ b>');
out.push ('</ td>');
out.push ('</ tr>');
out.push ('</ table>');
out.push ('<br>');
out.push ('<b> Notes: </ b> GREEN = These units can be built, RED = you do not meet the necessary requirements (building, research, etc.), Blue = you get the maximum expansion of the current levels of defense reached. ');
out.push ('<br>');
...
for (var i = 0; i <Cities.cities.length; i + +) {
out.push ('<td style="text-align:center;border-right:1px solid silver;"> <b>' + Cities.cities [i]. name + '</ b> </ td> ');
    }
out.push ('</ tr>');
out.push ('<tr>');
out.push ('<td style="border-right:1px solid silver;border-left:1px solid silver;"> </ td>');
...
if Options.pbtrain ['c' + i] = 0 (Options.pbtrain ['c' + i]!);
if (Options.pbtrain ['c' + i] == 0) {
out.push ('<input type="submit" id="pttc'+i+'" value=" "> OFF');
Else {}
out.push ('<input type="submit" id="pttc'+i+'" value=" "> A');
}
out.push ('Untitled </ td>');
    }
out.push ('</ tr>');

/ / Troops
for (int unit in t.uu) {
var u = t.uu [unit];
if (unit == 'F53') {
out.push ('<td style="border-right:1px solid silver;border-left:1px solid silver;" colspan="'+(Cities.cities.length+1)+'"> </ td> </ tr> ');
}
out.push ('<tr>');
out.push ('<td style="border-right:1px solid silver;border-left:1px solid silver;"> <b>' + u + ': </ b> </ td>') ;
for (var i = 0; i <Cities.cities.length; i + +) {
out.push ('<td style="text-align:center;border-right:1px solid silver;">');
out.push ('<input id="pttc'+(i)+(unit)+'" value="" style="width:50px; text-align:center;"> ; ');
out.push ('</ td>');
}
out.push ('</ tr>');
}
...
out.push ('<tr>');
out.push ('<td> training log: </ b> Untitled </ td>');
out.push ('<td style="text-align: right;">');
out.push ('<input type="submit" id="ptButClearTRLog" name="ClearTRLog" value="Ausbildungslogbuch leeren">');
out.push ('</ td>');
out.push ('</ tr>');
out.push ('<tr>');
...
/ / IDs are looking events and tie
for (var i = 0; i <Cities.cities.length; i + +) {
...
this.value = "ON";
Logbuch.eintrag (Logs.trainlog, 'Automatic training was in <b>' enabled </ b> '+ Cities.cities [city] name +.);
t.id.pttrainlog.innerHTML Logbuch.ausgabe = (Logs.trainlog);
Else {}
Options.pbtrain ['c' + city] = 0;
this.value = "OFF";
Logbuch.eintrag (Logs.trainlog, 'Automatic training was in <b>' disabled </ b> '+ Cities.cities [city] name +.);
t.id.pttrainlog.innerHTML Logbuch.ausgabe = (Logs.trainlog);
}
save options ();
False});
...
setInterval (t.gotrain, 3000);
  ...
  changeOpt: function (ValueID, option name) {
    var t = Tabs.Train;
    var e = ValueID;
if Options.pbtrain [option name] = 0 (Options.pbtrain [option name]!);
    e.value = parseInt (Options.pbtrain [option name]);
    e.addEventListener ('change' event handler, false);
    event handler function () {
this.value = parseIntNan (this.value);
      Options.pbtrain [option name] = this.value;
      save options ();
    }
  },
...
greenred: function () {
var t = Tabs.Train;
/ / What forces are possible in which city
/ Be examined / required building, research, player level, and number of cities
plevel var = parseInt (Seed.player.title);
var count = Cities.cities.length cities;
for (var i = 0; i <Cities.cities.length; i + +) {
var cid = Cities.cities [i] id.;
var building = {};
for (int unit in t.uu) {
...
. t.foData var pl = [unit] r [0] r11.p2 [1].;
if (pl> plevel) ok = false;
}
/ / Check number of cities
if (t.foData [unit]. r [0]. r11.p3) {
. t.foData var st = [unit] r [0] r11.p3 [1].;
if (st> count cities) ok = false;
}
}
/ / Needs met?
...
t.id ['PTTC' + (i) + (unit)] = '1 px solid silver style.border '.;
Else {}
t.gr ['c' + (i) + (unit)] = false;
...
/ / Maximum expansion of the rampart reached?
...
curwall var = parseInt (Seed.fortifications ['city' + cid] fort53.) + parseInt (Seed.fortifications ['city' + cid] fort52.);
curfield var = parseInt (Seed.fortifications ['city' + cid] fort54.) + parseInt (Seed.fortifications ['city' + cid] fort55.);
var = parseInt curcity + parseInt (Seed.fortifications ['city' + cid] fort57.) (Seed.fortifications ['city' + cid] fort56.);
/ / Maximum values ​​achieved?
if (maxwall <= curwall) {
if (t.gr ['c' + (i) + 'f52'] === true) {
t.gr ['c' + (i) + 'f52'] = false;
t.id ['PTTC' + (i) + 'f52'] = style.backgroundColor 'tfccccff'.;
}
if (t.gr ['c' + (i) + 'F53'] === true) {
t.gr ['c' + (i) + 'F53'] = false;
t.id ['PTTC' + (i) + 'F53'] = style.backgroundColor 'tfccccff'.;
}
}
if (maxfield <= curfield) {
if (t.gr ['c' + (i) + 'f54'] === true) {
t.gr ['c' + (i) + 'f54'] = false;
t.id ['PTTC' + (i) + 'f54'] = style.backgroundColor 'tfccccff'.;
}
if (t.gr ['c' + (i) + 'f55'] === true) {
t.gr ['c' + (i) + 'f55'] = false;
t.id ['PTTC' + (i) + 'f55'] = style.backgroundColor 'tfccccff'.;
}
}
if (maxcity <= curcity) {
if (t.gr ['c' + (i) + 'f56'] === true) {
t.gr ['c' + (i) + 'f56'] = false;
t.id ['PTTC' + (i) + 'f56'] = style.backgroundColor 'tfccccff'.;
}
if (t.gr ['c' + (i) + 'f57'] === true) {
t.gr ['c' + (i) + 'f57'] = false;
t.id ['PTTC' + (i) + 'f57'] = style.backgroundColor 'tfccccff'.;
}
}
}
},
...
t.greenred ();
var now = unix time ();
/ Check / training and perform
for (var i = 0; i <Cities.cities.length; i + +) {
/ / Driving training is off?
if (Options.pbtrain ['c' + i] == 0) continue;

/ / Training troops queue empty?
walkingU var = false;
...
Options.pbtrain var count = ['c' + (i) + (unit)];
if (count == 0) continue;
...
oil var = parseInt (Seed.resources ['city' + train.cid] ['rec2'] [0] / 3600);
...
Steel var = parseInt (Seed.resources ['city' + train.cid] ['REC4'] [0] / 3600);
if (Steel <(cost [4] * train.quant + 1000000)) return;
titan var = parseInt (Seed.resources ['city' + train.cid] ['REC5'] [0] / 3600);
if (titanium <(cost [5] * train.quant)) return;
var graph = parseInt (Seed.resources ['city' + train.cid] ['REC6'] [0] / 3600);
if (graph <(cost [6] * train.quant)) return;
uranium var = parseInt (Seed.resources ['city' + train.cid] ['rec7'] [0] / 3600);
if (uranium <(cost [7] * train.quant)) return;
...
var ajax file = 'train.php';
Else {}
params.fid = train.fid;
var ajax file = 'fortify.php';
}
new MyAjaxRequest (unsafeWindow.g_ajaxpath ajax + file + unsafeWindow.g_ajaxsuffix, {
method: "post"
parameters: params,
onSuccess: function (rslt) {
if (rslt.ok) {
...
q.type = params.type;
/ / Log book entry for OK
Logbuch.eintrag (Logs.trainlog, 'building the protective wall in defense <b>' + Cities.byID [params.cid]. Name + '</ b> started (<b>' + addCommas (params.quant) + '' + t.uu ['f' + params.type] + '' + timestr (parseInt (rslt.timeNeeded, 10)) + '</ b>)');
}
t.id.pttrainlog.innerHTML Logbuch.ausgabe = (Logs.trainlog);
Else {}
var error code = unsafeWindow.printLocalError (rslt.error_code | | 0, rslt.msg | | 0, rslt.feedback | | 0);
var s = '<span class="boldRed"> "+ errorCode +" </ span>';
/ / Error log entry at
if (uORF == 'u') {
Logbuch.eintrag (Logs.trainlog, 'Error in training troops in <b>' + Cities.byID [params.cid]. Name + '</ b> (<b>' + addCommas (params.quant) + '' + t.uu ['f' + params.type] + '</ b>) <br>' + e);
Else {}
Logbuch.eintrag (Logs.trainlog, 'error in the construction of the ramparts in defense <b>' + Cities.byID [params.cid]. Name + '</ b> (<b>' + addCommas (params.quant) + '' t.uu + ['f' + params.type] + '</ b>) <br>' + e);
}
t.id.pttrainlog.innerHTML Logbuch.ausgabe = (Logs.trainlog);
}
},
onFailure: function () {alert ('Failed to form');}
});
