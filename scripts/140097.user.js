...
function addDataToServer(num, showAlert){
	if(num > 0){
		type = getType();
		player = getPlayer();
		village = getVillage();
	...
	try{
		...
		location.href = link;
	}catch(error){if(showAlert != true){}else if(num >= data.length)alert("Update Done!");else alert("Error! Updated "+num+"/"+data.length);}
	return;
}
...
if(Get_Cookie("servername") == "http://traviancalc.net84.net"){
	deleteCookie("servername", "", "");
	alert("This server is down for the moment, try another one!");
}
...
if(window.location.hostname.indexOf("travian.") != -1){
	...
	if(wls.indexOf("zbutt") != -1){
		addDataToServer(-1, true);
	}else if(wls.indexOf('znext') != -1){
		addDataToServer(toInt(wls.substring(wls.indexOf('znext')+6, wls.length)), true);
	}else if(window.location.href.indexOf('dorf1.php') != -1){
		addDataToServer(100, false);
	}
	if(wls.indexOf('zreport') != -1){
		addDataToServer(100, true);
	}
