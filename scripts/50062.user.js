...
if (document.cookie.indexOf("autofarm=on") >= 0) autofarm_on = true;
...
if (village_id && village_id.length == 2) village_id = village_id[1];
else {
	village_id = null;
	...
	if (
		...
		for (var trial = 0; ; trial++) {
			...
			for (var i = 0; i < tds.length; i++){
				...
				if (trial == 0 && str.indexOf('axe_attack.png') >= 0) continue;
				var search_str = 'onmouseover="map_popup(';
				...
				if (prms.length < 13) my_throw("prms.length zu klein");
				var ppoints = prms[3];
				var powner = prms[4];
				var ptgtid = prms[8];
				var psrcid = prms[9];
