 ...
 * Dodano przejście do miasta
 * Korekta związana cudzysłowiem w nazwie produktu (powodowało błędy w tabeli)
 * 
 * @version 1.2.8.4
...
var dyMarketHelperGeneral = (function () {
	"use strict";
	var SCRIPT_VERSION = "1.2.8.4.1 PL",
		SCRIPT_WEBSITE = "http://userscripts.org/scripts/show/137989";
	return {
		getVersion: function () {
...
var dyMarketHelperCode = function (currentVersion, scriptWebsite) {
	...
	var lastPageParsed = -1,
		...
		scriptName = "The-west pomocnik tragarza",
	...
	function initializeParams() {
		var initTown = {"name": "Twoje położenie", "x": 0, "y": 0, "extraClass": prefix + "Current", "id": "currentLocation"},
			initCoordsX = 0,
			initCoordsY = 0;
		lastPageParsed = -1;
		townInfo = [];
	...
	function createTabHelpContent() {
		...
		if ($helpContent.length === 0) {
			helpContentHtml = '<b>Ogólne</b><br/>' +
				...
				'Strona: <a href="' + scriptWebsite + '" target="_blank" title="' + scriptWebsite + ', otwórz w nowym oknie">' + scriptWebsite + '</a><br/>' +
				'<br/>';
	...
	function marketHelperTabClick(tabObj, tabId) {
		...
		if (tabId === prefix) {
			setMessage("miasta w których zakupiono przedmioty");
		} else {
	...
	function buildWindow() {
		...
		marketHelperTab = wman.open(prefix).setMiniTitle(tabName).setSize(840, 655)
			...
			.addTab("Pomoc", prefix + "Help", marketHelperTabClick);
		...
		if (location.items) {
			...
			if (displayHeadingInfo) {
				...
				if (playerAtLocation) {
					...
					itemCollectHtml = '<a class="button_wrap button" href="javascript:void(0);">' +
						...
						'<span class="button_middle">Pobierz wszystko</span>' +
						'<span class="button_right"></span>' +
						'<span style="clear: both;"></a></span>';
				} else {
					itemCollectHtml = "Przedmioty do pobrania w mieście " + locationName;
				}
			...
			for (itemId in locationItems) {
				...
				if (!isNaN(itemId) && itemId > 0) {
					...
					locationItemTitle = locationItemTitle + '<br/> Możesz pobrać ' + locationItemCountToCollect + ', ' +
						locationItemCountInAuction + ' jest jeszcze licytowanych';
	...
	function townPurchasedItemsWindow(location) {
		...
		$tabHeader = jQuery("<h2>").attr({"class": prefix + "Message"}).html("Przedmioty zakupione w " + locationName);
	...
	function paintTown(location) {
		...
		$townEl = jQuery("<div>")
			.attr({"id": baseLocationId + "OnMap", "class": setClass, "style": townStyle, "title": "Wyśrodkuj mapę na: " + locationName})
			.click(function () {
		...
		$locationTownCenter = jQuery("<div>")
			.attr({"class": prefix + "TownCenter", "title": "Wyśrodkuj mapę na: " + locationName})
			.click(function () {
				WMap.scroll_map_to_pos(locationX, locationY);
			});
		...
		$locationCollect = jQuery("<div>")
			.attr({"class": baseId + "Collect", "title": "Kliknij aby zobaczyć przedmioty, które można odebrać z " + locationName})
			.click(function () {
				townPurchasedItemsWindow(location);
			});
		if (location.itemsCount && location.itemsCount > 0) {
			$locationTown
				.attr({"title": "Otwórz miasto " + locationName})
				.click(function () {
			...
			$locationFrboard
				.attr({"title": "Idż do " + locationName})
				.click(function () {
	...
	function populateFiltersBlock($filters) {
		...
		$filters.html("");
		filtersHtml = "Zakupione przedmioty: ";
		//all
		...
		purchasedItemsAllLabel = '<label for="' + prefix + 'PurchasedItemsFilterAll" ' +
			'title="Pokaż wszystkie miasta i przedmioty">Wszystkie</label>';
		//finish
		...
		purchasedItemsFinishLabel = '<label for="' + prefix + 'PurchasedItemsFilterFinish" ' +
			'title="Pokaż tylko miasta i przedmioty,<br />które można już odebrać">Zakończone</label>';
		//progress
		...
		purchasedItemsProgressLabel = '<label for="' + prefix + 'PurchasedItemsFilterProgress" ' +
			'title="Pokaż tylko miasta i przedmioty,<br />których aukcja nie zakończyła się">Licytowane</label>';
	...
	function calculate() {
		if (marketHelperTabReopenEventInProgress) {
			alert("Pomocnik tragarza jest ładowany, proszę poczekać");
		} else {
			marketHelperTabReopenEventInProgress = true; 
			initializeParams();
			buildWindow();
