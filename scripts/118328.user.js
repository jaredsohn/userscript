...
for(var i=1,j=Y_Coords.length-1; i<j; i++) {
	if (P_Coords[i] != 1) {
	if (i > 5) {
		rtemp = Math.sqrt(Math.pow(Math.abs(parseFloat(Y_Coords[i]) - parseFloat(Y_Coords[i-5])), 2) + Math.pow(Math.abs(parseFloat(X_Coords[i]) - parseFloat(X_Coords[i-5])), 2))/5
		rmaxV = Math.max(rtemp,rmaxV)}
...
document.getElementById('maxrspeed').innerHTML = (rmaxV*10).toFixed(2)
