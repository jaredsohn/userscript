...
// @require        https://firebfplite.duapp.com/BFP/JS/hmac-sha1.js
// @require    https://firebfplite.duapp.com/BFP/JS/enc-base64-min.js
...
var aid, date, cid, vid, restxt, O_Player, mode, selector, spid,
	page,
	urltype = "download",
...
var pos = isVideo ? "video" : (isSP ? "sp" : (isHome ? "home" : (isBangumi1 ? "bangumi1" : (isBangumi2 ? "bangumi2" : isLetv ? "letv" : "other"))));
...
var DT = {
	...
	isBili: function() {
		...
		try {
			return player.src.indexOf("bilibili");
		} catch (e) {
			return -1;
		}
	},
...
var UT = {
	...
	getURL: function(name) {
		...
		switch ((name || "home")) {
			...
			case "video":
				VT.setAid(isVideo[1]);
				url = 'http://api.bilibili.tv/view?type=json&id=' + VT.getAid() + '&appkey=0a99fa1d87fdd38c&page=' + (VT.getPage() || 1); //video
				if (localStorage.getItem("isYQL") == "true") {
					url = extra1 + encodeURIComponent(url) + extra2;
				}
				break;
			case "sp":
				url = 'http://api.bilibili.tv/view?type=json&id=' + VT.getAid() + '&appkey=0a99fa1d87fdd38c&page=' + (VT.getPage() || 1); //sp
				if (localStorage.getItem("isYQL") == "true") {
					url = extra1 + encodeURIComponent(url) + extra2;
				}
				break;
			case "spinfo":
				...
				break;
			case "videoPlay1":
				url = "https://secure.bilibili.tv/secure," + VT.getPath(); //player1
				break;
			case "videoPlay2":
				url = "https://static-s.bilibili.tv/play.swf?" + VT.getPath(); //player2
				break;
			case "videoPlay3":
				url = "https://ssl.bilibili.tv/secure," + VT.getPath(); //player3
				break;
			case "lebili":
		...
		case "home":
			var func = function() {
				...
				if (restxt.indexOf("qwertqwert") > -1) {
					...
					if (V != version) {
						var notice = "<br/><li style='color:red'>新版本号为:" + "<div style='color:green'>" + V + "</div>" + " 更新内容:" + "<div style='color:blue'>" + str[3] + "</div>" + " 需要更新请点击<a id='updateBFP' href='http://userscripts.org/scripts/source/165424.user.js' target='_blank'>这里</a>" + "</li>";
...
var VT = {
	...
	setCid: function(newcid) {
		localStorage.setItem("BFP_CID." + VT.getAid(), newcid);
	},
	getAid: function() {
		return aid;
	},
	getCid: function() {
		var BFP_CID = localStorage.getItem("BFP_CID." + VT.getAid());
		if (BFP_CID) {
	...
	getCloudURL: function(method) {
		// var url = localStorage.getItem("BFP." + object);
		// if (url == null) {
		var object = VT.getAid();
		if (mode == "full") {
		...
		// } else {
		// 	console.log(object);
		// 	notify("BFP锁定目标 光谱分析开始", "成功获得缓存中的云端地址 解析中");
		// 	return url;
		// }
	},
	...
	checkValidity: function() {
		if (mode == "full") {
			...
			if (result.indexOf("视频不允许在您当前所在地区播放") > -1) {
				...
				AT.setResTxt(1);
				notify("BFP充能失败", "B站没有本地视频源哦 爱奇艺番过几天就好拉 \r\n顺便问一句[无广告无弹幕的爱奇艺你们愿意看吗www 可以把答案告诉我哦]", 10000);
				setTimeout(function() {
					VT.autoSelect();
				}, 5000);
				try {
...
function notify(title, body) {
	...
	var grant = Notification.permission && Notification.permission || (webkitNotifications.checkPermission && webkitNotifications.checkPermission().toString() || "console");
