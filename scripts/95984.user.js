// ==UserScript==
// @name             HAKAN-MM- OTO SALDIRI
// @version          HAKAN.MM.1
// @namespace        HAKAN_MM_ OTO SALDIRI
// @homepage         http://userscripts.org/scripts/show/89473
// @description      OTO Barbar Saldırısı Yapar.Komple Türkçe
...
function inspect(obj, maxLevels, level){
  var str = '', type, msg;
	...
	if(maxLevels < 1)     
		return '<font color="red">HATA: Girilen Değer Büyük Olmalıdır > 0 dan..</font>';

	// We start with a non null object
	if(obj == null)
	return '<font color="red">HATA: NESNE <b>NULL</b></font>';
...
var KOCAttack={
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Saldırı TÜRÜ: <select id='KOCAttackOrder'><option value='mostTroops'>Asker Sayısına Göre</option><option value='closest'>Önce Yakındakiler</option></select><br />"+
			"Saldırı Yerleri: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbar "+
			...
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> Saniyede Bir Saldırı YAP..!<br />"+
			"OTO Saldırı Zamanlama<u>SEÇENEKLERİ.</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Barbar Saldırı Zamanı:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />Saat<br />"+
			"Bozkır Saldırı Zamanı:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />Saat<br />"+
			"Şehir Saldırı Zamanı:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />Saat<br />"+
			"Taşıma Zamanı:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />Dakika"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' />Mesafeye Kadar Saldırı Yapabilsin...<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" />Girilen Koordinatları Yanlızca Girildiği Şehirden Saldırıyı Gerçeklertirmek İçin İŞATERLEYİNİZ..!!<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Burası,Oyun da Aşırı Trafikden Doğan Hatalar İçindir...<br />"+
			"Girilen url adresi yeni bir sekmede açılır...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Gerçekleşen Saldırılar Sonunda Oluşan Raporları Otomatik Olarak SİLER..<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Sıra Raporlar Görünsün...<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Saldırı Raporlarının Görüntülenmesini Devre Dışı Bırakmak İstendiğinde...<br />"+

			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% ye Geldiğinde Mutluluğu Arttır..<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de Mutluluk Olduğunda ALTINI Arttır...<br />"+
			"</td></tr>"+

			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" />Açılır Pencereler Engellensin mi..?<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" />Arkadaşlarınızın İnşaat/Araştırma YArdımlarına Otomatik Yardım Edilsin mi..?<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" />Chatbox da Çıkan YArdım İsteklerini YArdım Ettikden Sonra Gizler...<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" />Yapılan İnşaat/Araştırmalarınızı Otomatik Yayınlasın mı..?.<br />"+
			"Kimlerin YArdım İSteklerine Onay Verilsin..? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Herkes</option><option value='50'>Ark.mın Arkadaşları</option><option value='40'>Yanlızca Arkadaşlar</option><option value='10'>Yanlızca Ben</option></select><br />"+
			"</td></tr>"+

			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" />Sayfanızda Gerçekleşecek BAKIM/HATA Seçenekleri...<br />"+
			"<input id='KOCAttackUseAlternateReload' type='checkbox' "+(this.options.useAlternateReloadMethod?'checked':'')+" />Alternatif Metod..(ÖNERİLMEZ)<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" />Alınan Hataları Raporlayarak Firefox Genel Birimlerine Gönderir..(ÖNERİLMEZ..)<br />"+
			"</td></tr>"+

			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> Saniyede Şehirleri Gez..<br />"+
			"Bu Şehirlerden <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> beklenir ve "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />secs before refreshing.<br />"+
			"Saldırı Yapılacak Şehirler: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% Rastgele Gecikme Ayarları..<br />"+
			"Taşıma/Aktarma: Lütfen Koordinatları ve Komutları Buraya Yapıştırın...<br /><textarea id='KOCAttackImport'></textarea>"+
			...
			"<a id='KOCAttackOptionsSave' class='button20'><span>KAYDET</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>ÇIK</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Ayarları Sıfırla..!</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Fabrika Ayarlarına DÖN..!!</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Kayıtlı Tüm Saldırıları SİL..!!</span></a></form>";
		var t=this;
	...
	*/			
	...
	SetAttackStatusMessage:function() {
		...
		if(!toggle) {
			var t=this;
			toggle=this.AddTabLink('OTO-Saldırı');
			if(!toggle) {
				window.setTimeout(function() {
					t.SetAttackStatusMessage();
				},t.GetRandTime(250));
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>OTO-Saldırı=AÇIK</span>';
		} else {
			toggle.innerHTML='<span>OTO-Saldırı=KAPALI</span>';
		}
	},
	...
	*/	
	...
	GetOptions:function() {
		...
		var defOptions={"attackDelay":8,
			...
			"randomPercent":20,
			"keepReports":15,
			"attackMaxDistance":6000,
			"lockAttackFromCity":true,
			"waitAfterCitiesDone":30,
			"autoAttackCitiesDoneMax":2,
			"changeCitySecs":5,
			"retryMarch":true,
			"noViewReports":false,
			"chromeKeepReports":2,
			"percentOfPopToTrain":85,
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"<br /><input id='KOCAttackBulkAddForce' type='checkbox' />Yanlış Koor. Hatasını Önlemek İçin İşaretleyiniz..!<br />");
		...
		bulkAdd.innerHTML='<span>Hafızaya AL..!</span>';
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert('** EvrenSerdar ** EKLENEN KOORDİNAT SAYISI '+added+' ADETTİR..!');
		},false);
		AddHtml(bulkAddDiv,"<br />");

		var bulkAddAttackLink=document.createElement('a');
		bulkAddAttackLink.className='buttonDown20';
		bulkAddAttackLink.innerHTML='<span>Toplu Koordinat Ekleyin..!</span>';
	...
	HideAttackEfforts:function() {
		...
		var a=document.createElement('a');
		a.innerHTML='GİZLE/GÖSTER Saldırı Efektlerini';
		//a.href='javascript:;';
	...
	OnAttackBoxAppear:function(box) {
		...
		AddText(div2,'Yapılan Saldırınızın HAFIZAYA Alınmasını İSTEMİYORSANIZ İşaretleyiniz..!');
	...
	*/
	...
	DeleteAllStoredAttacks:function() {
		...
		window.alert("KAYITLI BÜTÜN SALDIRILAR SİLİNECEKTİR..ONAYLIYORMUSUNUZ..??");
	},
	...
	SetupMenu:function() {
		var t=this;
		GM_registerMenuCommand('Evren_Serdar OTO-Saldırı Seçenekleri',function() {
			t.ShowOptionsDialog();
		});
		GM_registerMenuCommand('Evren_Serdar OTO-Saldırı',function() {
			t.ToggleAutoAttack();
		});
		GM_registerMenuCommand('Evren_Serdar OTO-Saldırı Fabrika Ayarları..!!',function() {
			t.FactoryReset();
		});
	},
