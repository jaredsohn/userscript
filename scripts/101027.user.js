// ==UserScript==
// @name           YouTube Lyricss
// @namespace      http://userscripts.org/users/28612
// @version        3.1   //version mejorada para el nuevo Youtube (anterior version 3.04.00) añadida en lineas 56, 269 y 278
// @changes        Fixed the parsing of the video category,Added the "movies" video category y modificado el lugar del panel ahora esta arriba(linea 235) ahora tambien esta modificado para que se pueda ver con el nuevo diseño de Youtube ademas de que se encoge y amplia el panel
...
var SU=null;
var DU=null;
var grande=1;  //flag para controlar el tamaño del panel de lyrics
init();
...
function init()
{
	...
	}else{ if (CM.config.addUserScriptCommandToShowLyricsPanel){ $rmc("MenuShow",[ScriptInfos.name],initLyricsPanel);initLyricsPanel();}};
	//Añadido para minimizar el panel de las letras por defecto
	if (grande==1){
	   var minus = document.getElementById("YLLyricsContentDiv");	
	   minus.setAttribute('style', "height:0px;");
	   grande=0;
    }
...
function addLyricsPanel(reset)
{
	...
	if (div1==null)
	{
		...
		//var div2=$id("more-from-panel") || $id("watch-channel-videos-panel");   antes estaba puesto por debajo del panel  de info
		//var div2=$id("more-from-panel") || $id("watch-next-list");  //ahora se pone por encima(manera antigua fallaba en algunos casos)
		//div2.parentNode.insertBefore(div1,div2);
		//var padre=$id("watch-other-vids");   //nueva manera localizamos el padre de la columna lateral	
		//var lat = document.getElementById("watch-sidebar");


		//var padre=$id("watch-sidebar");
		var padre = document.getElementById("watch-sidebar");   //nueva barra lateral del nuevo diseño de Youtube
		padre.insertBefore(div1, padre.firstChild)  //e insertamos el panel de lyrics en el primer hijo (ahora sirve incluso para VEVO)
		...
		$idp("PanelSwitch").addEventListener("click",function(e){if (!e.target.onlyOnce) {e.target.onlyOnce=true; amplia(); if(grande==1)searchForLyrics(); }},false); //añadida la funcion amplia
...
//Añadida funcion para ampliar o reducir el panel de lyrics
function amplia()
{
    var panel = document.getElementById("YLLyricsContentDiv");	
	if(grande==0){	 //ampliamos el panel
	   panel.setAttribute('style', "height:200px;");
	   grande=1;
	}else{ //minimizamos el panel
