...
ResCalc = 
{
	...
	init : function()
	{
		...
		else
		{
			...
			{
				ResCalc.rclog( "No unit definition found - using defaults from grepo v.1.07");
			}
		}

		// Add Pop-up texts we use here (will be bound to html elements later by "setPopup" calls)
		uW.PopupFactory.addTexts({
					rcwood_lost  : "<h4>Wood</h4>Base wood cost for lost Units.</br>Researches like <i>'Trainer'</i> are not taken into consideration!",
					rcstone_lost : "<h4>Stone</h4>Base stone cost for lost Units.</br>Researches like <i>'Trainer'</i> are not taken into consideration!",
					rciron_lost  : "<h4>Silver</h4>Base silver cost for lost Units.</br>Researches like <i>'Trainer'</i> are not taken into consideration!",
					rcfavor_lost : "<h4>Favor</h4>Base stone cost for lost Units.</br>Researches like <i>'Trainer'</i> are not taken into consideration.",
					rcbh_lost    : "<h4>Population</h4>Population freed up due to lost units.",
					rcatt_lost   : "Cost for Attacker",
					rcdef_lost   : "Cost for Defender",
					openResBox   : "Open Resource Window",
					closeResBox  : "Close Resource Window",
		...
		if ( ResCalc.UnitData )
		{
			...
			else if ( uW.document.URL.indexOf( "action=simulator") > 0 ) 
			{
				...
				ResCalc.createResPopup('fcSim', 'Rohstoffe');
			...
			else if ( uW.document.URL.indexOf( "game/building_wall?") > 0 ) 
			{
				...
				// #0 - Title row   ("Besiegt..." / "Verluste...")
				// #1 - Subtile row ("...als Angreifer"  )
				// #2 - unit row
				// #3 - Subtile row ("...als Verteidiger" )
				// #4 - unit row
				...
				ResCalc.createResPopup( 'fcWall', 'Total Resources' );
				...
				if (rv)
				{
					...
					if ( attCostsTable.length | defCostsTable.length )
					{
						var sumNode = $("<div style='text-align:left;font-size:9px;'>"
								+"<center><span style='text-decoration:underline;'><b>Kosten f&uuml;r Verluste</b></span>"
								+"<table cellpadding='0px' cellspacing='0px'>"
								+"<tr><td>"+attCostsTable+"</td>"
								
								+ "<td><b><table cellpadding='0px' cellspacing='0px'>"
								+ "<tr><td align=center>Wood  </td></tr>"
								+ "<tr><td align=center>Stone </td></tr>"
								+ "<tr><td align=center>&nbsp;&nbsp;Silver&nbsp;&nbsp;</td></tr>"
								+ "<tr><td align=center>Favor</td></tr>"
								+ "<tr><td align=center>&nbsp;&nbsp;Pop&nbsp;&nbsp;</td></tr></table></b></td>"
								
								+"<td>"+defCostsTable+"</td></tr></table></center></div>");
						sumNode.appendTo('#resources');
					}
					else
						ResCalc.rclog("No units found in report.");
				}
				else
