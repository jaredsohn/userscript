...
//											log('[notifications]','hide');
						...
						onSelectorLoad('#rightCol',function(o) {
							...
							*/
							...
							// For update checking
							if (SCRIPT_TYPE!="chrome_gallery" && options.get('check_for_updates9')) {
								var t = time();
								var update_check_interval = (options.get('update_check_interval9') * hours) || (8*hours);
								...
								// If it's past the time to check, then check!
									...
									get_content('update',true,update_check_interval,function(update) {
										...
										if (update && update.version && update.version>version_ack && update.version>version) {
											...
											var close_options = {'close_pref':'check_for_updates9', 'close_msg':'Are you sure you want to close this message and NOT get notifications of updates? This is NOT recommended! You may end up using an old version of Social Fixer that stops working correctly as Facebook changes!'};
