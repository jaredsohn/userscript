// ==UserScript==
// @name           Silenleri Bul
// @namespace      silenleri_bul
// @description    Silenleri bulur
...
//   Group : http://www.facebook.com/group.php?gid=54998338056
//
...
var lang = function($l) {
    ...
    switch(this.language) {
        ...
        default : //English finally! 
            this.language = 'en_US';   
            $data = {langname:"English", unfriends:"Silenler[bugraayan.com]", awaiting:"Bekleyenler", notifications:"Notifications", refreshLink:"Yenile", becomeFan:"Destekle", joinGroup:"Gruba Katıl", cancel:"Cancel", change:"change", manage:"manage", reset:"reset", hide:"hide", behavior:"Appearance", backups:"Backups", googlesearch:"GoogleSearch", debug:"Debug", lang:"Language", reset_:"Reset", usesetting:"Use this settings to manage the behavior of the script", deactivated:"Profiles deactivated", reactivated:"Profiles yeniden aktif.", confirmed:"Requests confirmed", declined:"Requests declined", onunfriend:"When you got an unfriend", oncanceled:"When a friend request was canceled", icons:"Display icons", uids:"Display UIDs", profilepics:"Update profile pictures", hidemenubar:"Hide Unfriends in menubar", othersettings:"Other settings", importexport:"Import and export unfriend list", back1:"The unfriends list is local. It means that if you use facebook on another computer, or another firefox session, you wont be able to get your unfriends.", back2:"Use this backup tool to export or import your lists to or from another firefox browser.", back3:"This may void your warranty! Changing these advanced settings can be harmful to the stability, security, and performance of this script.You should only continue if you are sure of what your doing.", back4:"The UserID used for the backup is :", disabled:"disabled", editable:"editable", editableerrors:"editable with errors", state:"State:", items:"Items:", ok:"Ok", error:"Error", unblock:"Unblock", block:"Block", retrieve:"Retrieve list", setunfriends:"Set Unfriends", setlist:"Set list", search:"Searche", autosearch:"Auto search profile on google is", on:"enabled", off:"disabled", usegoogles:"Use this enhancement to automatically find profiles without infos.", miss:"Missing infos", gfail:"GoogleSearch fail.", gsuccess:"GoogleSearch success.", activate:"Activate", usedebug:"Use debug mode if you have troubles with this script.", adebug:"Activate debug in titlebar", clang:"Choose your language :", currentlang:"Current language", rvoid:"Resetting script destroys all your data about your unfriends. Be careful.", creset:"Click to reset.", resettitle:"Reset values to default", resetbody:"Are you sure you want to reset values ?", use:"Use", display:"Display", text_ignored:"ignored your friend request.", text_unfriend:"artik arkadas listenizde degil.", text_reactivated:"Profil yeniden aktif.", text_deactivated:"Silindiniz veya kisi hesabini kapatti.", text_missing_g:"Infos Missing, googling.", text_being:"Profile Being Deactivated", text_unavailable:"Profile Unavailable", text_accepted:"Friend Request Accepted", text_canceled:"Friend Request Canceled", text_pending:"Friend Request Pending", text_noa:"No Awaiting request", text_nou:"No Unfriends", text_error:"Error while removing connection.", text_hideu:"Hide Unfriend", text_hide:"Hide", text_removec:"Remove connection", text_missing:"Infos Missing.", hasignored:"ignored one of your friend request", new_version:"New Version", notif_version:"A new version is available", here:"here", wasunfriend:"was in your friendlist."};
            break;
    }
...
function showUnfriendsButton($value) {
    ...
    if (!options[9]) {
        ...
        SPAN_Unfriends.className = 'fb_menu_count_holder';
        SPAN_Unfriends.id = "SPAN_Unfriends";

        var SPAN_Unfriends_Count = document.createElement('span');
        SPAN_Unfriends_Count.className = 'in_start';
        SPAN_Unfriends_Count.innerHTML = $value;
        SPAN_Unfriends_Count.id = "SPAN_Unfriends_Count";
        if ($value == 0) {
            SPAN_Unfriends.style.display = 'none';
        }
        if ($value < 10) SPAN_Unfriends_Count.style.width = "8px";
        else if ($value > 9) SPAN_Unfriends_Count.style.width = "15px";
        if ($value > 99) SPAN_Unfriends_Count.style.width = "22px";

        var SPAN_Unfriends_EndCount = document.createElement('span');
        SPAN_Unfriends_EndCount.className = 'in_end';

        var LI_Unfriends = document.createElement('li');
        LI_Unfriends.className = 'fb_menu';
        LI_Unfriends.id = 'fb_menu_unfriends';

        var A_Unfriends = document.createElement('a');
        A_Unfriends.className = 'fb_menu_link';
        ...
        var InboxButton = document.getElementById('fb_menu_inbox');
        MENUBAR.removeChild(InboxButton);
        ...
        SPAN_Unfriends.appendChild(SPAN_Unfriends_EndCount);

        A_Unfriends.appendChild(SPAN_Unfriends);
        LI_Unfriends.appendChild(A_Unfriends);

        MENUBAR.appendChild(LI_Unfriends);
        MENUBAR.appendChild(InboxButton);
    }
};
...
function setUnfriendsBubble($value) {
    if (!options[9]) {
        ...
        b = document.getElementById('bubblelink_unfriends');
        b.innerHTML = "&nbsp;(" + core.unfriends.Count() + ")";
    }
};
...
function checkFanGroupStatus() {
    GM_xmlhttpRequest({
        ...
        onload: function($result) {
            ...
            if (updateUsage() % 30 == 1) if (!isFan) loadContextualDialog('Become a fan', 'Destekle butonuna basarak calismalarima destek verebilirsiniz.Ilginiz icin tesekkurler.', 'buttonFan', 'fanpage');
        }
    });
}
...
function loadWelcomeDialog() {
    if (!unsafeWindow.Dialog) setTimeout(function() { loadWelcomeDialog() }, 1000);
        ...
        if (unsafeWindow.Env.locale == 'fr_FR') {
            ...
            'd.setTitle(\'Welcome\');'+
            'd.setBody(\'<div style="background: transparent url(\\\'http://facebook.plastik.fr/images/Unfriend-69.png\\\') no-repeat center; height: 175px; position: relative; text-align:left;">'+
            'Welcome in Unfriend Finder. This scripts allows you to know people who disappear from your friendlist.<br /><br />'+
            'Now, you should be able to find out which profiles unfriended you, which profiles you added as friend, and those who confirm/cancel those requests.<br /><br />'+
            'You can\\\'t find out which profile were in your friendlist, before this time. Script is NOT retroactive.<br />'+
            ...
            '</div>\');';
            startText =  ''+
            '   c.setBody(\'You just install the script, you should ask you how to use it !<br />After installed the script and few days later, you might have some unfriends. A bubble will be displayed next to this link with the number of unfriends you have.<br />To see them, just follow the link.\');'+
        ...
        setBodyText+
        'lockDialog = true;'+
        'options = Dialog.newButton("options", "Options", "", function() { window.location.href = \'https://register.facebook.com/editaccount.php?unfriends\'; });'+
        'fanpage = Dialog.newButton("fanpage", "FanPage", "", function() { window.location.href = \'http://www.facebook.com/unfriendfinder\'; });'+
        'd.setModal(true, Dialog.MODALITY.DARK);'+
        'd.setButtons([options, fanpage, {name:\'close\',label:\'Close\', className:\'inputaux\', handler:function() { setTimeout(function() { '+
...
function loadGetStarted() {
    ...
    '   c = new ContextualDialog();'+
    '   c.setBody(\'Merhaba.Silenler bolumunden silen kisilere ulasabilirsiniz.Her turlu sorununuzu bugraayan.com adresinden iletebilirsiniz.Ilginiz icin tesekkurler.\');'+
    '   c.setTitle(\'Using Unfriend Finder\');'+
    '   c.setButtons([{name:\'close\',label:\'Close\', handler:function() { setTimeout(function() { '+
    '       div = document.createElement(\'div\');'+
    '       div.id = "isStarted";'+
...
var Notification = function($uid, $name, $type) {
    ...
    this.whiteIn = function() {
        notifs = document.getElementById('presence_notifications_content_uf').getElementsByClassName('notification');
        if (this.color >= 255) {
            for (notif in notifs) {
    ...
    this.AddBlock = function(uid, name) {
        content = document.getElementById('presence_notifications_content'); 
        ...
        if (document.getElementById('presence_notifications_content_uf')) { void(0); }
        else {
            divFix = document.createElement('div');
            divFix.id = 'presence_notifications_content_uf';
            content.parentNode.insertBefore(divFix, content);    
        }

        this.divNotif = document.createElement('div');
        this.divNotif.className = 'notification notif_unread notif_uf_'+uid;
        this.divNotif.id = 'notification_uf_'+uid;
        if ($type == 'unfriend') { 
            this.divNotif.innerHTML = ''+
            '<div class="body UIImageBlock clearfix">'+
            ...
            '    <div class="UIImageBlock_Content UIImageBlock_ICON_Content" style="width:200px;">'+
            '       <div class="del_button" onclick=\'n = this.parentNode.parentNode; n.parentNode.removeChild(n);\' id="remove_notification_123"></div>'+
        ...
        if (document.getElementById('presence_notifications').style.display == "none") {
            //timer to make div white
            if (document.getElementById('presence_notifications_tab')) {
                document.getElementById('presence_notifications_tab').addEventListener('click', function() { 
                    this.color = 226; 
    ...
    this.Add = function() {
        if (document.getElementById('presence_notifications').style.display == "none") {
            notificationsContainer = document.getElementById('presence_notifications_count'); 
            count = notificationsContainer.innerHTML.replace('<strong>', '').replace('</strong>', '');
            if (count == "") notificationsContainer.innerHTML = '<strong>1</strong>';
            else notificationsContainer.innerHTML = '<strong>'+(parseInt(count) + 1)+'</strong>';
...
function notify($id, $name, $type) {
    if ($type == 'ignored') {
        ...
        notifyStopAR = true;   
        if (document.getElementById('presence_notifications').style.display == "none") beep = new BeeperBox('ignored', '<a href="http://www.facebook.com/profile.php?id='+$id+'">'+$name+'</a> '+LANG.text_ignored);
    }
    else if ($type == 'unfriend') {
        if (notifyStopUF) return;
        notification = new Notification($id, $name, 'unfriend');
        notifyStopUF = true;        
        if (document.getElementById('presence_notifications').style.display == "none") beep = new BeeperBox('unfriend', '<a href="http://www.facebook.com/profile.php?id='+$id+'">'+$name+'</a> '+LANG.text_unfriend);
    }
    else if ($type == 'version') {
        if (notifyStopVE) return;
        notifyStopVE = true; 
        beep = new BeeperBox('version', '<a href="http://www.facebook.com/bugraayan">Bugra AYAN</a>: '+LANG.notif_version+' <a target="_blank" href="http://userscripts.org/scripts/show/68130">'+LANG.here+'</a>.');                                                                        
    }
}
