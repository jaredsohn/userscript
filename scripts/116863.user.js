...
ChangeLog
--------
2010-12-3 by agunchan
searchDefs:	add opbbs
show():  	change caption and fit width
handleKey():wsad to replace with +-LeftRight
moveSlide():fix currentImagePosition bug
*****************************/

// ==UserScript==
// @name		Greased Lightbox Mod
// @namespace	http://shiftingpixel.com
// @description	Enhances browsing on websites that link to images. Use A and D keys to cycle through images on page.
// @include		http://bbs.opbbs.net/*
// ==/UserScript==
...
var greasedLightbox = {
	...
	*/
	searchDefs : [		
		...
		{
			name				: 'newsmth',
			includeRegExp		: /^https?:\/\/(.*?\.)?newsmth\.net\/bbst?con\.php/i,
			linkRegExp			: /^http:\/\/att\.newsmth\.net\/att\.php.*\.(jpe?g|gif|png)$/i,
			showFunction		: function(event) { greasedLightbox.showFrom(event, 'newsmth'); return false; }
		},
		
		// opbbs
		{
			name				: 'opbbs',
			includeRegExp		: /^https?:\/\/(.*?\.)?opbbs\.net\/forum\.php\?mod=(viewthread|redirect)&tid=/i,
			linkRegExp			: /^http:\/\/.*\.(jpe?g|gif|png)/i,
			showFunction		: function(event) { greasedLightbox.showFrom(event, 'opbbs'); return false; }
		}
	...
	showFrom : function(event, showName) {
		...
		greasedLightbox.show(event, img, address, showName);
	}, // showFromLink()
	...
	getAddress : function(linkObj) {
		...
		if (!address) {
			address = linkObj.getAttribute('src');
		}
	...
	center : function(objToCenter, arrayPageScroll, arrayPageSize) {
		...
		if (arrayPageSize[3] - 35 < objToCenter.offsetHeight) {
			var objTop					= arrayPageScroll[1] + 35;
		} else {
	...
	show : function(event, img, context, showName) {
		...
		capt = showName + "(" + (greasedLightbox.currentImagePosition+1) + "/" + greasedLightbox.allImageLinks.length + ")";
...
//				}
	...
	moveSlide : function(event, moveByAmount) {
		if (greasedLightbox.allImageLinks.length > 1) {
			greasedLightbox.currentImagePosition += moveByAmount;
			if (greasedLightbox.currentImagePosition < 0) 
				greasedLightbox.currentImagePosition = greasedLightbox.allImageLinks.length - 1;
			if (greasedLightbox.currentImagePosition >= greasedLightbox.allImageLinks.length)
				greasedLightbox.currentImagePosition = 0;
			
			var slideToLoad						= greasedLightbox.allImageLinks[greasedLightbox.currentImagePosition];
	...
	// checkLink()
	// get image link and add click listener
	checkLink : function(searchDefsToUse, thisLink, href) {
		var lightboxedLinksTotal = this.allImageLinks.length;
	...
	init : function() {
		...
		for(var i = 0; i < greasedLightbox.searchDefs.length; i++) {
			if(greasedLightbox.searchDefs[i]['includeRegExp'].test(currentURL)) {
				searchDefsToUse.push(greasedLightbox.searchDefs[i]);
				break;
			}
		}
		...
		if (searchDefsToUse[0]['name'] == "opbbs") {
			links = document.evaluate('//div[@class="pcb"]//img[@src]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		
			var isSerialized = false;
			var h1 = document.evaluate('//h1[@class="ts"]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
			if (h1.snapshotLength > 0) {
				h1 = h1.snapshotItem(0).innerHTML;
				if (h1.indexOf("\u8FDE\u8F7D") != -1 && h1.indexOf("\u718A\u732B\u6C49\u5316") != -1) {
					isSerialized = true;
				}
			}
			...
			for (var i = 0; i < links.snapshotLength; i++) {
				...
				href		= unescape(thisLink.getAttribute('src'));
				if (isSerialized) {
					if (i == 0) {
						if (thisLink.width > 250)
							thisLink.width = 250;
					} else {
				...
				greasedLightbox.checkLink(searchDefsToUse, thisLink, href);
			}
		} else {
			...
			for(var i = 0; i < links.snapshotLength; i++) {
				...
				greasedLightbox.checkLink(searchDefsToUse, thisLink, href);
				if (greasedLightbox.allImageLinks.length > 1 && greasedLightbox.allImageLinks[greasedLightbox.allImageLinks.length-1]["link"] == href) {
					thisLink.parentNode.removeChild(thisLink);
				}
			}
		}
		
		if(!this.allImageLinks.length) return;
	...
	en : [
		{
			...
			next			: 'Next image (d key)',
			previous		: 'Previous image (a key)',
			magnify			: 'Magnify image (w key)',
			shrink			: 'Shrink image (s key)',
