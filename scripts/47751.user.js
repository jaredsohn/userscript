// ==UserScript==
// @name            Transporte entre ciudades para Ikariam
// @namespace       kkhweb.com
// @author          empty_soul (kevinhill)
// @description     Añade destinos de barcos en la barra izquierda para facilitar el transporte de bienes entre las colonias. Es una traducción de Intercity Trading (http://userscripts.org/scripts/show/54649).
// @version         4.2.2
...
// @exclude         http://*.ikariam.*/*view=militaryAdvisorDetailedReportView*
// @exclude         http://*.ikariam.*/*view=highscore*
// @exclude         http://*.ikariam.*/*view=version*
...
//NOTA: Aún no está aprobado este script, así que úsalo bajo tu propio riesgo.
...
//Member Data
    ...
    CFG:                    Config,
    icons:                  {},
    loadingSpeeds:          [0,30,60,93,129,169,213,261,315,373,437,508,586, 

672,766,869,983,1108,1246,1398,1565,1748,1950,2172,2416,2685,2980,3305, 

3663,4056,4488,4964,5488,6697,7394,8161,9004,9932,10952,12074],
//Member Functions
    decTimeConvert: function(time){var hours=parseInt(time/60);time-=(hours*60);var minutes=parseInt(time);time-

=minutes;time*=60;var seconds=parseInt(time);return{h:hours,m:minutes,s:seconds};},
    removeIndex: function(array, from, to){var rest=array.slice((to||from)+1||array.length);array.length=from<0?

array.length+from:from;return array.push.apply(array,rest);},
    run: function(debug) {
        if(debug === true) {
    ...
    scriptOptionsConfig : function(){
        ...
        this.CFG.init({
            'General':{
                html:'<p>Personaliza el <i>script</i> de '+this.scriptName+' con estas opciones.<br/>Presiona la 

tecla "escape" para salir sin guardar los cambios.</p>',
                fields:{
                    'position' : {
                        type : 'select',
                        value : 'top',
                    ...
                    'simpleView' : {
                        type : 'checkbox',
                        value : true,
                        label : 'Vistas simples',
                        text : 'Embarque simple + tiempo de carga'
                    },
                    'hideOccupied' : {
                        type : 'checkbox',
                        value : true,
                        label : 'Comercio local (beta)',
                        text : 'Ocultar ciudades ocupadas o con guarnición'
                    },
                    'lastShipment' : {
                        type : 'checkbox',
                        value : false,
                        label : 'Embarque previo (beta)',
                        text : 'Muestra el último embarque que hiciste'
                    }
                }
            },
            "Acerca de" : {
                html : '<p><a href="http://userscripts.org/scripts/show/' +this.usoNum + '" target="_blank" 

style="font-weight:bold !important;">' +
                    this.CFG.scriptName +
                    '</a> de <a href="http://userscripts.org/users/emptysoul12" target="_blank">Empty_Soul</a></p>' 

+
                    '<p>¡Este <i>script</i> facilta el transporte!<br>Traslada recursos, tropas o flotas entre ciudades 

(propias, ocupadas o con permiso de guarnición).</p>'
            }
        });
    ...
    renderMainBlock : function() {
        var title = '<span title="' +this.language.title +' v' +this.curVer +'" id="ictHeader" class="header"><p 
...
id="shade">';
    ...
    loadCSS : function(){
        GM.css(
            ...
            '.ictIcon {background-position:top left; background-repeat:no-repeat; opacity:0.5; filter:alpha

(opacity=50);top:-3px; left:-5px; height:12px; width:20px; z-index:0;}' +
    ...
    insertLastShipment : function(cargo) {
        ...
        var route = '<p id="ictLastShipmentReceiver">';
            route += '<div style="margin-left:10px;"><span style="font-weight:bold;">' + cargo.from + 

'<br/></span>';
            route += '<img src="' + GM_getResourceURL('arrow') + '" 
        ...
        if(enabled === true) {
            var shipTroops = function() {
                ...
                var route = '<span class="ictShipH">' + from + '</span>';
                    ...
                    route += '<span class="ictShipT" id="ictTravel">' + $('.journeyTime').text().split(':')[1] + 

'</span>';

                $('#selectArmy div:first p:first').css('text-align','center').html(route);
                $('.costs').css('width','300px');
                $('#missionSummary .transporters').css({'margin-top':'-110px','margin-left':'120px','margin-
...
bottom':'80px'});
            ...
            var shipGoods = function(){
                InterTrade.attachLoadCalculator(50);
                    ...
                    $('#missionSummary .transporters').css({'margin-top':'-55px','margin-left':'120px','margin-
