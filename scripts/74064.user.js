...
// @description    Many small tweaks to improve GPX; Patching the Tab Closer again
...
function modifyUserList()
{
	...
	if (sortOnlineList)
	{
		...
		function sigmaValuer()
		{
			return function(row)
			{
				var interWith = parseInt(trim(row.cells[2].innerHTML.replace(/,/g, "")));
				var interFrom = parseInt(trim(row.cells[3].innerHTML.replace(/,/g, "")));
				return interFrom - interWith + (interWith + interFrom) / 1000;
			}
		}
		
		function sort(table, valuer, order)
		{
			var rows = new Array();
			for (var i = start; i < table.rows.length; i += 2)
			{
				rows.push({row: table.rows[i], statusRow: table.rows[i+1], value: valuer(table.rows[i])});
			}
			rows.sort
			(
				function(a, b)
			...
			for (var i = 0; i < rows.length; i++)
			{
				...
				table.appendChild(rows[i].statusRow);
			}
		}
		...
		{
			case 1: // Int. With Asc
				sort(table, columnValuer(2, true), false);
				break;
			case 2: // Int. With Desc
				sort(table, columnValuer(2, true), true);
				break;
			case 3: // Int. From Asc
				sort(table, columnValuer(3, true), false);
				break;
			case 4: // Int. From Desc
				sort(table, columnValuer(3, true), true);
				break;
			case 5: // Sigma's Mix
				sort(table, sigmaValuer(), true);
				break;
			default:
...
function closeTabs()
{
	...
	if (document.referrer.indexOf("/pc") > -1)
		return;
	if (document.getElementsByTagName("form").length <= 2 && useTabCloser)
...
function modifyPoketch()
{
	...
	function modifySettings()
	{
		if (!chrome)
		{
			var settings = document.getElementById("SaveSettingsTD").parentNode.parentNode.parentNode.parentNode;
			...
			if (showMaturityLeft)
			{
				...
				for (var i = 0; i < xps.length; i++)
				{
					...
					if (xp.nodeValue)
					{
						...
						xp.nodeValue = addCommas(max - min) + " (" + percent + ")";
					}
				}
	...
	var hatching = 0;
	function hatchEgg(fname, fhash)
	{
		unsafeWindow.$.ajax
		({
			url: "http://gpxplus.net/AJAX/OptionDialog", 
			data: {option: "hatch", fname: fname, hash: fhash, confirm: true}, 
			cache: false, 
			dataType: "json", 
			type: "POST", 
			success: function (r)
	...
	function getHash(fname)
	{
		unsafeWindow.$.ajax
		({
			url: "http://gpxplus.net/AJAX/OptionDialog", 
			data: {option: "hatch", fname: fname}, 
			cache: false, 
			dataType: "json", 
			type: "POST", 
			success: function (r)
			{
				var hash = substring(r.html, "hash: '", "'");
				if (hash)
				{
					hatchEgg(fname, hash.text);
				}
				else
				{
					window.alert("There was a problem hatching one of your eggs!");
		...
		if (party)
		{
			...
			if (link)
			{
				if (!link.getAttribute("modified"))
				{
					...
					hatchLink.id = "hatchAllLink";
					hatchLink.innerHTML = "Hatch All";
					party = party.firstElementChild.children[1];
					var start = "Login name: ";
					var end = "&nbsp;";
					parent.appendChild(hatchLink);
					party.innerHTML = party.innerHTML.replace(start + substring(party.innerHTML, start, end).text, parent.innerHTML);
					hatchLink = document.getElementById("hatchAllLink")
