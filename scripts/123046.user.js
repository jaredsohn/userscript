	...
	$.ajax(
		{
			...
			success : function() {
				if( iaskSearchResult.sp.m > 0 ) {
					...
					var	regex = /([A-Z\u0391-\uffe5])/g;
					//好吧，我只会写最普通的循环
					for( var resultIndex = 0,resultTotal = iaskSearchResult.sp.result.length; resultIndex < resultTotal; resultIndex++) {
						var result = iaskSearchResult.sp.result[resultIndex];
						title = result.title;
						//这里是在控制title的长度？
						//我调试的时候，replace这里报错
