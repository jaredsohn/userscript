// ==UserScript==
// @name DS Farmmanager2
// @description Version 2.0.6.Zeigt erweiterte Dorfinfos auf der Karte, sowie im VP 
// @author Hypix
...
if( lang == "des" || (lang == "ch" && welt < 4) || lang == "chs" )
  lang = "de";
...
var texts = 
{
  ...
  { 
    ...
    gui:
    {
      title: "DS-Farmmanager",
      redir_villageinfo: "Dorfinfos",
      redir_place: "Truppen schicken",
      ...
      popupShowBuildingChange: "Gebäudestufenänderungen anzeigen",
      ...
      placeShowBuildings: ["Gebäude anzeigen", "Nein", "Nur den Wall", "Alle"],
      placeShowBuildingChange: "Gebäudestufenänderungen anzeigen",
      placeShowCatas: "Benötigte Katapulte um Gebäude zu zerstören anzeigen",
      ...
      farmlistSettings: ["Farmliste","Farmen mit den meissten Rohstoffen im Umkreis von","Feldern","Umkreis erhöhen, wenn nicht alle Top-Farmen min. ", " Resourcen haben ."],
      ...
      farmUnitsConfig: "Konfiguration der Farmeinheiten",
      priority: "Priorität",
      ...
      placeSendRam: ["Rammen nur mitschicken, wenn","Wall min. auf Stufe", "ist","genug Rammen vorhanden sind, um den Wall zu zerstören"],
      sendram: "Rammen mitschicken",
      unitGroups: "Einheitengruppen",
      delGroup: "Gruppe löschen",
      newGroup: "Neue Gruppe",
      minUnits: "Mindestmenge der Einheiten (einer Geschwindigkeit)",
  ...
  if( e )
  {
    ...
    if( settings.useHotKeys )
    {
      ...
      if( typeof(unsafeWindow) != "undefined" )
        hotKeys.push( { key: 65, func: function() { var a = document.getElementById("selectAllUnits"); var cmd = ""; if( typeof(unsafeWindow) != "undefined" ) cmd += "unsafeWindow."; cmd += a.href.replace("javascript:",""); debug.log(cmd); eval(cmd); } } );
      else
...
function setGlobalHotkeys()
{
  ...
  if( settings.useHotKeys )
  {
    ...
    {
      if( a[j].accessKey == "a" )
        hotKeys.push( { key: 109, href: a[j].href } );
      else if( a[j].accessKey == "d" )
        hotKeys.push( { key: 107, href: a[j].href } );
    }
  }
}
...
{ 
  ...
  if( container )
  {
    ...
    for( var i = 0; i < tds.length; i++ )
    {
      ...
      {
        a = a[0];
        var report = a.getAttribute("dsfm_village").split(",");
        var apar = parseParams(a.href);
        var vid = apar.village;
        var container = document.getElementById("mapCoords");
        var off = (mapSize.count-1)/2;
...
function showSettings()
{
  ...
  p.innerHTML = '<form action="https://www.paypal.com/cgi-bin/webscr" method="post"><table><tr><td><input type="hidden" name="cmd" value="_s-xclick"><input type="hidden" name="hosted_button_id" value="DTQ463G5SFB28"><input type="image" src="https://www.paypal.com/de_DE/DE/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="Jetzt einfach, schnell und sicher online bezahlen – mit PayPal."><img alt="" border="0" src="https://www.paypal.com/de_DE/i/scr/pixel.gif" width="1" height="1"></td><td>Entwicklung des Farmmanagers unterstützen</td></tr></table></form>';
...
function mapData()
{
  var data = { title: mapData.arguments[0], 
           bonus_image: mapData.arguments[1], 
           bonus_text: mapData.arguments[2], 
           points: mapData.arguments[3], 
           owner: mapData.arguments[4], 
           ally: mapData.arguments[5], 
           reserved_by: mapData.arguments[6], 
           reserved_till: mapData.arguments[7], 
           village_groups: mapData.arguments[8], 
           lastatt: mapData.arguments[12] ? texts[lang].locale.date2MS(mapData.arguments[12].match(new RegExp(texts[lang].regex.reportDate)))/1000 : 0,
         };
  return data;
}
