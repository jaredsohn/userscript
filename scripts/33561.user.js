...
try
{
	...
	{
		var offset = 0;
		// If you look at a deposit, that isn't from the city selected, you miss an element and have to correct for this
		if (document.getElementById('mainview').childNodes.length <= 5)
			offset = 2;
		
		var donationList = document.getElementById('mainview').childNodes[5-offset].childNodes[1];
		...
		for (var j = 1; j < donationList.rows.length; j+=cities)
		{
			...
			else // Chadoios
			{
				...
				var lvl = document.body.childNodes[4].childNodes[1].childNodes[9-offset].childNodes[3].childNodes[3].innerHTML.split("</span>")[1];

				if (lvl == undefined) // Deposit is upgrading
					lvl = document.body.childNodes[4].childNodes[1].childNodes[9-offset].childNodes[3].childNodes[5].innerHTML.split("</span>")[1];
			...
			if (thisElement.elements[0].value == 'Options')
			{
				...
				div.innerHTML = 
					"<div id='leecher_checker'>"+
						"<h3>Leecher Checker v1.2.3 options</h3>"+
