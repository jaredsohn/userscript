// ==UserScript==
// @name                Google Reader + Twitter [MOD]
// @namespace           http://userscripts.org/users/91271
// @description       	Adds reader posts to twitter when clicked, shorten url by bit.ly, based on terababy's Google Reader + Twitter
// @version             1.0.1
// @author              cataerogong
...
function postBookmark(event){
	...
	urlinput.value = url;
	titleinput.value='[GR]' + title;
	btnSend = document.getElementById("btnSend");
	btnBitly = document.getElementById("btnBitly");
	btnCount = document.getElementById("btnCount");
	btnCancel = document.getElementById("btnCancel");
	btnSend.addEventListener('click',saveBookmark,false);
	btnBitly.addEventListener('click',getShortenAndInsert,false);
...
function getShortenAndInsert(event) {
	var thelongurl = urlinput.value;
	urlinput.value = "shorten url...";
	var login  = 'cataerogong';
	var apiKey = 'R_8da5f7690a9ebbb81e72062dcb048cda';
	var url = 'http://api.bit.ly/shorten?callback=?&version=2.0.1&longUrl=' + thelongurl + '&login=' + login + '&apiKey=' + apiKey;
	GM_xmlhttpRequest({
		method: 'GET',
		url: url,
		onload: function(res) {
			...
			if(data.errorCode == 0)
			{
				result = data.results[thelongurl].shortUrl;
			}
			else
			{
