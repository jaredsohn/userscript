...
// @namespace      blognomic
// @description    Original by Sparrow, vote-counting added by alethiophile
...
function mainPage() {
  $("<button type=\"button\">Sparrowscript settings</button>").prependTo("#sidebar2").click(settings_query);
...
//  GM_log(quorum);
  $("div[class='comment']").
	...
	$(cbody).find("img").each(
	  function(j) {
	    var cb = this;
	    votes[name] = cb.getAttribute('alt');
	    if (settings['debug']) {
  ...
  var veto = false;
  var sk = false;
  if (votes[proposer] == undefined && isProposal)
    votes[proposer] = 'for';
  if (settings['debug']) {
