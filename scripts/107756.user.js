...
// @translated to english 2011 uwillywanakadoit
// ==/UserScript==
...
unsafeWindow.AutoBuilder = {
    ...
    STRTIME: ["","Constr","Invest","Enter","Defens"],
    ...
    CreateInterface: function () {
        ...
        this.UpdateDiv("IGNITION....CORRECT",1);
    ...
    Collect: function (cty) {
        ...
        setTimeout("Modal.hideModalAll();",4000);
        this.UpdateDiv("Money Cashed...",cty);
    },
    CollectInt: function () {
        for(var i = 1; i <= this.NumCity; i++)
        {
    ...
    Build: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('Construction even in: ' + AutoBuilder.DisplayTime(Building.Queue.first().timeRemaining)," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.BuildTimer[cty]);
            ...
            else
            {
                ...
                {
                    clearTimeout(this.BuildTimer[cty]);
                    this.UpdateDiv("Construction Deactivated.",cty);
                    return;
                }
    ...
    Train: function(cty) {
        ...
        else
        {
            ...
            {
                setTimeout("AutoBuilder.UpdateDiv('training event in: ' + AutoBuilder.DisplayTime(Barracks.Queue.timeRemaining())," + cty + "); Modal.hideModalAll();",2000);
                clearTimeout(this.TrainTimer[cty]);
    ...
    Stop: function (go, bra) {
        ...
        {
            this.UpdateDiv("Ignition " + this.STRTIME[bra] + "....",cty);
            switch (bra)
            {
                case 1:
