// ==UserScript==
// @name           GOR Toolbox Utils
// @version        0.1.65
// @namespace      GOR Toolbox Utils
// @description    GoR Toolbox Utils
...
// @require 	   	 http://sizzlemctwizzle.com/updater.php?id=113864
// ==/UserScript==

var Version = '0.1.65';
var crightmsg="English Version "+Version +" by Mark 2011 Based on a script by BeWorld"
      ...
      for(i=0; i < Cities.numCities; i++) {
    	...
    	var timeLeft = parseInt(Cities.cities[i].c.resources[1].count)  / usage * 3600;
          ...
          if (timeLeft<0){
                ...
                msg += ' is running short of food. Current Stock : '+addCommasWhole(foodleft).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ')+' ('+timestrShort(timeLeft)+') Upkeep : '+addCommas(usage).replace(',',' ').replace(',',' ').replace(',',' ').replace(',',' ')+'/h';
                sendChat ("/a " + msg);
                //alert(msg);
          }
      }  
   ...
   clickREACitySourceSelect : function (city){
    ...
    for (r=1; r<13; r++){
      ...
      ById("REApdestunit"+r).addEventListener ('click', function() {
        ...
        ById(nomcha2).value=0; // on met ÃƒÆ’  0
        //for(i=0; i<Cities.numCities; i++) {
    	 	...
    	 	var entTotal=0;
    	  	for(i=0; i<Cities.numCities; i++) {
    	  	       ...
    	  	       if ( parseInt(Cities.cities[i].c.resources[1].hourlyTotalRate()) < parseInt(Cities.cities[i].c.upkeep()) ) {
    	  	        // entretien supÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬ ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©rieur ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬ ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢  la production
...
my.Crests = {
  ...
  show : function (){ 
	...
	if (unsafeWindow.items[1107].count>0){dominatan=unsafeWindow.items[1107].count}else{domitan=0};
...
<TABLE CLASS="MYTABLE" CELLPADDING=3 CELLSPACING=0 align=center>\
    ...
    <TR CLASS="MYTABLE">  \
              ...
              <br>'+domitan+' / 2 '+unsafeWindow.arStrings.itemName["i1107"]+'\
              </TD>\
      </TR>\
		</TBODY>\
  </TABLE>';
