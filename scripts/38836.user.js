...
// @description     Adds link on ScT to send torrent to uT webui v0.3
...
function scanLinks() {
  ...
  if (!host || !port) {
    alert('You need to configure the "ScT to uT" user script with your uTorrent WebUI parameters before using it. Please go to your profile page, /my.php');
...
function match(url) {
    return url.match(/https?:\/\/.*scenetorrents\.org\/download\.php/);
}
...
function addPreferences() {
  var pmRow = document.getElementsByClassName("g_my_maintable")[0]
      .getElementsByClassName("my_col1")[1].parentNode;
  
  var row = document.createElement("tr");
  
  var labelCell = document.createElement("td");
  labelCell.className = "my_col1";
  ...
  settingsCell.className = "my_col2";
