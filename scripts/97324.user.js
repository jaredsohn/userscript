...
// @description    This is a Beast script. Beware. You have been WARNED
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('Beast Kit', eventHideShow, mouseMainTab);
}
...
var KOCAttack={
	...
	*/			
	...
	SetAttackStatusMessage:function() {
		...
		if(!toggle) {
			var t=this;
			toggle=this.AddTabLink('Beast Attack');
			if(!toggle) {
				window.setTimeout(function() {
					t.SetAttackStatusMessage();
				},t.GetRandTime(250));
	...
	//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
		if(autoAttack) {
			toggle.innerHTML='<span>Beast Attack - Own</span>';
		} else {
			toggle.innerHTML='<span>Beast Attack - Off</span>';
		}
	},
