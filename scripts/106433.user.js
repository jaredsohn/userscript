// ==UserScript==
// @name           Simplified Readable juridat.be
// @version	       1.1.2
// @namespace      rj
...
// Largely based on the script Readable juridat.be & ejustice.just.fgov.be v1.1 (2008-05-10)
// by Naja Melan (http://userscripts.org/scripts/show/11725)
...
text      : "texte",
info      : "Contenu et d&eacute;lai de consolidation",
infoLink  : "http://www.ejustice.just.fgov.be/loi/contenu2.htm"
}

// the main page with the search frame -- COMMENTED BY RJ
/*
...
function NxtHit(){}\
  ...
  //If we loaded a list with results
    ...
    else if( hidFrame.loi_l1 )
    {
      ...
      for( var i = formNum-1; i >= 0; --i ) {
        allForms[i].setAttribute( "target", "_blank" );
        allForms[i].setAttribute( "method", "get" );
        allForms[i].setAttribute( "action", "/cgi_loi/loi_a1.pl" );
      }
...
//maintables
preambule : "Pr&eacute;ambule",
modif     : "Modification(s)",
content   : "Table des mati&egrave;res",
text      : "Texte",
...
//textTable
showOrig  : "Montrez l&rsquo;original",
hideOrig  : "Cachez l&rsquo;original"
...
if( topTable = $xPath1( '//a[@name="top"]/following-sibling::table[1]' ) )
{
  ...
  if( raadHref && raadHref.match( /mbid=([^&]+)/ ) ) indexNum = RegExp.$1;
...
// New code to correct a bug in Firefox 5 (script on the page were not recognized)

XMLDoc.getElementsByTagName("head")[0].innerHTML = newHead.innerHTML;
XMLDoc.getElementsByTagName("body")[0].innerHTML = newBody.innerHTML;

var s = document.createElement("script");
s.innerHTML = document.getElementById("scriptTag").innerHTML;
document.getElementsByTagName("head")[0].removeChild(document.getElementById("scriptTag"));
...
//Check if it starts a new paragraph § 1. § 2.
  reExpr = /(^"?|^"?.+?:) ?((?:\(.*?\))? ?(?:<span class="startDateAnnotation">.*?<\/span>)?)*? *?(\(?§ *?\d{1,3}(?:erbis|er|bis)?(?:\.| - | (?![a-z]))\)?)([^,].*)?/,
  reFirst = /^"?\(?§ *?1(?!bis)([^\d]|$)/;
...
//Check if it starts with a summation token like 1§ 2§
  reExpr = /(^"?|^"?.+?: ?)(<span class="startDateAnnotation">.*?<\/span>)*? *?(\(?\d{1,2}§(?:bis)?\)? ?)(.*)/;
  reFirst = /^"?\(?1§/;
...
if( document.location.href.match( /http:\/\/www\.ejustice\.just\.fgov\.be\/(wet\/wet|loi\/loi)\.htm/ ) ) {
	document.querySelectorAll("frame")[1].addEventListener("load", ProcessDocumentLinks, false);
}

if( document.location.href.match( /http:\/\/www\.ejustice\.just\.fgov\.be\/cgi_loi\/loi_a1.pl/ ) ) {
	GM_addStyle(myCss);
}
