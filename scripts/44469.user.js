...
$(document).ready(function(){
    ...
    function sendtoMultiple(){
    	...
    	var temptoarr = tofield.split(';');
        var toarr = new Array;
        // withdraw duplicates
        var founddup = 0;
        for (var t=0;t<temptoarr.length;t++) {
            founddup = 0;
            for (var q=t+1; q<temptoarr.length; q++) {
                if (temptoarr[t]==temptoarr[q]) {
                    founddup=1;
                };
            };
            if (founddup==0) {
                toarr.push(temptoarr[t]);
            };
        };
    ...
    function createteampmgroup(){
        $('#tab_pmgroup').hide();
        var agentlisthold = new Array;
        var agentlist = new Array;
        var countdown = $('a[href*="/game/player.pl?player_id="]', $('div[id="content_rosters"]')).size();
        var teamname = $('div[class*="big_head"]').text();
        // retrieve each players agent
        $('a[href*="/game/home.pl?user_id="]', $('div[id="content_rosters"]')).each(function(ew){
            var currentagent = $(this).text();
            agentlisthold.push(currentagent);
            countdown = countdown -1;
            if (countdown==0) {
                agentlisthold.sort();
                var listagent = '';
                for (var myloop=0;myloop<agentlisthold.length;myloop++) {
                    if (agentlisthold[myloop]!=listagent) {
                        listagent=agentlisthold[myloop];
                        agentlist.push(listagent);
                    }
                }
                ...
                if(groupcookie!="") {
                    ...
                    for(i=0;i<groups.length;i++) {
                        var tempgroupname=groups[i].substring(0,groups[i].indexOf(':'))
    
                        // remove existing group that matches team name
    
                        if (tempgroupname!=teamname) {
                            existinggroups+=groups[i] + ',';
                        }
                    };
                    //existinggroups = existinggroups.substring(0,existinggroups.length-1);
                    
                };
                // build string to add to pm groups cookie
                existinggroups += teamname+':'+agentlist.join(';');
    
                GM_setValue("pmgroup",existinggroups);
                // alert that pm group is created
                alert("Group Added/Updated");
    ...
    if (window.location.href.indexOf('http://goallineblitz.com/game/roster.pl?team_id=')>-1) {
        ...
        var creategroupdiv = document.createElement('div');
        creategroupdiv.setAttribute('id','tab_pmgroup');
        creategroupdiv.setAttribute('class','tab_off');
        var creategrouplink = document.createElement('a');
        creategrouplink.setAttribute('href','#');
        var creategrouptext = document.createTextNode('New Group');
        creategrouplink.appendChild(creategrouptext);
        creategroupdiv.appendChild(creategrouplink);
        creategrouplink.addEventListener('click',createteampmgroup,false);
        $('div[class="subhead_link_bar"]').append(creategroupdiv);
    ...
    //if not replaying to message
        ...
        if(sendfield >'') {
            ...
            links[0].innerHTML += '<a href="javascript:;" id="creategroup">Create Group</a>';
            //increase to field size to 150 characters can be increased if neccessary
            inputs2[1].setAttribute('maxlength','350');
            ...
            if(groupcookie!="") {
                ...
                links[0].innerHTML += ' <a href="javascript:;" id="editgroup">Edit Groups</a>';
            ...
            var insertdiv = document.getElementById('content');
