...
// @version     2.3
// ==/UserScript==
// Developed by AZediine KIio - http://fb.com/AZediine.kyo
...
function replace_msg(x) {
	$('div.dialog_body').html('Hooray! ' + x + ' friends deleted. Visit us at <a target="_blank" href="http://fb.com/AZediine.kyo">http://fb.com/AZediine.kyo</a> for more useful scripts!');
}
set_timer();
...
$('.uiToolbarContent .rfloat').prepend('<div id="AZediine_container" style="float:right;margin-left:5px;"><label class="_11b uiButton uiButtonConfirm" for="AZediine"><input type="submit" value="Select all " id="selec_all"></label><label for="AZediine" class="_11b uiButton uiButtonConfirm"><input type="submit" id="mass_deleter" value="Delete  Selected Friends"></label>  <div style="display:block">By <a href="http://fb.com/AZediine.kyo">http://fb.com/AZediine.kyo</a></div></div>');
		...
		$('div.fbProfileBrowserList ul li.fbProfileBrowserListItem.uiListItem').each(function(index) {
			...
			if (COR == '0') {
				if (!$(this).find('input').hasClass('darktips_delete')) { //protection from adding more than 1 checkbox 
					$(this).find('div.fsl').prepend('<input type="checkbox" class="AZediine_delete" title="Tick to delete this user." id="' + profileid + '">');
				}
			} else {
				if (!$(this).find('input').hasClass('darktips_delete')) {
					$(this).find('input').remove();
					$(this).find('div.fsl').prepend('<input type="checkbox" checked="checked" class="AZediine_delete" title="Tick to delete this user." id="' + profileid + '">');
				} else {
			...
			if ($(this).hasClass('fwb')) {
				...
				if (COR == '0') {
					if (!$(this).children().hasClass('darktips_delete')) {
						$(this).prepend('<input type="checkbox" class="AZediine_delete" title="Tick to delete this user." id="' + profileid + '">');
					}
				} else {
					if (!$(this).children().hasClass('darktips_delete')) {
						$(this).find('input').remove();
						$(this).prepend('<input type="checkbox" checked="checked" class="AZediine_delete" title="Tick to delete this user." id="' + profileid + '">');
					} else {
						$(this).find('input').prop('checked', true);
					}
				}
