...
// @description    more features for the dAmn browser client
...
//           alert("notify");
  ...
  notifyNewVersion = function () {
    ...
    upd.setAttribute("style","z-index:999;position:fixed;top:2px;right:2px;width:300;height:140;opacity:0.8;padding:5px;text-align:center;background-color:#98A39D;border-color:#78837D;border-style:solid;border-width:1px;-moz-border-radius: 1em;");
    upd.innerHTML='<div style="opacity:1" id="upd1">      A newer version of ' + this.SCRIPT_NAME + ' is available.<br/>\n'+
      '<a style="display:block;" href="' + this.SCRIPT_URL + '"><b>Click here and select "Install This User Script" to update,<br>\nthen reload this page</b></a></div>';
...
for(i=0;i<document.getElementsByTagName("script").length;i++)
 if(document.getElementsByTagName("script")[i].src=="http://chat.deviantart.com/chat07/dAmn.js?06.3.12"||document.getElementsByTagName("script")[i].src=="http://chat.deviantart.com/chat07/dAmnChat.js?06.3.12")
  supported++;
if(!dAmn_Client_Agent)
 {
 }
else
if( (supported>=2&&/dAmn WebClient 0\.7\.pre\-1/.test(dAmn_Client_Agent))||window.confirm("dAx has detected that the current version number of dAmn does not comply with the version number that this script was coded for (an update and I didn't notice it yet)\nThere's a good chance it will still work, but I will give no guarantee that it will work without problems. If you notice problems, please make screenshots and send them to me.")) { // guarantee only for this client
...
editTimestamps=function(firsttime)
 {
 useTimeStamps=window.confirm((firsttime==true?"Welcome to dAx "+VERSION+", now offering Timestamps (you may disable/uninstall Timestampify)\n\n":"")+"Do you want to use Timestamps in dAmn? (this setting will be saved by a cookie)");
 ...
 dAmn_Join(room);
    ...
    if(unsafeWindow.addem.length)
     for(var e in unsafeWindow.addem)
      {
      if((code=/^(\:[a-zA-Z0-9][a-zA-Z0-9]+\:)|(\:[A-Z])$/.exec(unsafeWindow.addem[e].code)) && !addemotes[code[0]] && (thumb=/\:thumb[0-9][0-9]+\:/.exec(unsafeWindow.addem[e].thumb)) && (creator=/[a-zA-Z0-9\-]+/.exec(unsafeWindow.addem[e].creator)))
...
//               while(args.charAt(args.length-1)==" ") args=args.substring(0,args.length-1);
               switch(args)
                ...
                case 'chat': makeResultBox(this.channel,'',0,'<b>/chat</b><br>opens a private chatroom with another online user on dAmn. <b>This user has to enter this room manually, by entering <i>/chat yourname</i> as well.</b>'); break;

                default: makeResultBox(this.channel,'', 0,'<b><a href="http://www.deviantart.com/view/16684830/">dAx '+VERSION+'</a></b><br><i>Features:</i><ul><li><i>:faq##:</i> support and adds <i>/faqsearch</i> command<li><i>/gettitle</i> and <i>/gettopic</i> commands added &gt;&gt; /about gettitle or /about gettopic<li><i>/setaway</i> starts an away mode &gt;&gt; /about setaway<li><i>/chat</i> creates a private chatroom with another user of dAmn &gt;&gt; /about chat<li>Additional emoticons added. For a list, use the <i>/emotes</i> command<li>Timestamps (originally created by `<a href="http://doofsmack.deviantart.com/">doofsmack</a>) are a part of dAx now <sub>(included them already since FF 1.5 broke them for some people)</sub></ul>'); break;
                }
               didsmth=true;
               break;
            case 'ignore':
                    ...
                    if( file.match(/\.gif$/i) ){
                        ...
                        else
                        {
                            ...
                            if( tmp && tmp.length > 1 ){
                                tmp = tmp[0].split('.')
                                if( tmp && tmp.length > 2 )
                                    path = file.replace(/^fs(\d+)\.deviantart\.com/,"ic1.deviantart.com/fs$1").replace(/^images2\.deviantart\.com/,"ic3.deviantart.com/images2");
