...
// @description    Crawling TRT EPG data
// @include        http://www.trt.net.tr/TV/TvAkis*
// ==/UserScript==

var akis = document.getElementById("ctl00_tvIcerikAralik").childNodes[1].childNodes[6];
var gun = document.getElementById("ctl00_tvIcerikAralik").childNodes[1].childNodes[3];
...
function getEPGDate(){
	var now = new Date();
	var topBar = gun.childNodes[0].childNodes[0].childNodes;
...
"</td></tr>";
	...
	for (var i=1; i<akis.rows.length; i++) {
		hour = akis.rows[i].cells[0].innerHTML;
		program = (akis.rows[i].cells[1].childNodes.length > 1) ? akis.rows[i].cells[1].childNodes[0].innerHTML : akis.rows[i].cells	[1].innerHTML;
		url = (akis.rows[i].cells[1].childNodes.length > 1) ? akis.rows[i].cells[1].childNodes[0].href : "";
