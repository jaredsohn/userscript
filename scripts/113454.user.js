// ==UserScript==
// @name           Fkkchacebook Fixed Header (Always-At-Top)
// @namespace      http://skossfsfhy.com
// @creator        Stefan Ksfsfoshy
// @description    Keeps Facsffsebook's header consistently visible (fixed header). Also adds some other features.
...
if (user !== false) {
    addStyle (
        '#fbFixedOptions {display: none; z-index: 10000; position: fixed; padding: 25px; background-color: rgba(255, 255, 255, .9); -webkit-border-radius: 

10px; -moz-border-radius: 10px; border-radius: 10px; border: 10px solid rgb(230, 230, 230); width: 700px; margin-left: -370px; height: 400px; margin-top: -

220px; left: 50%; top: 50%;}' +
	...
	GM_xmlhttpRequest({
		...
		onload: function(response) {
			...
			if (newVersion != currentVersion) {
				...
				node.innerHTML = '<a target="_blank" href="http://userscripts.org/scripts/show/'+scriptId+'">Update Available ('+newVersion
...
function stripText(firstString, secondString, subject) {
	...
	if (firstPos != -1) {
		if (secondString != '')
			return subject.substring(subject.indexOf(firstString)+firstString.length, subject.indexOf(secondString, subject.indexOf
...
function changeOptions() {
	...
	for (var i = 0; i < document.getElementsByClassName('fbFixedProfPicInput').length; i++) {
		if (parseInt(document.getElementById('fbFixedForm').elements.namedItem('profPic'+i).value).toString() == document.getElementById

('fbFixedForm').elements.namedItem('profPic'+i).value)
...
function applyOptions() {
	...
	if (options.disableFindFriendsLink == 't')
		document.getElementById('headNav').getElementsByClassName('rfloat')[0].firstChild.children[document.getElementById

('headNav').getElementsByClassName('rfloat')[0].firstChild.children.length-2].style.display = 'none';
	else
		document.getElementById('headNav').getElementsByClassName('rfloat')[0].firstChild.children[document.getElementById

('headNav').getElementsByClassName('rfloat')[0].firstChild.children.length-2].style.display = 'block';
}

function createProfPics() {
    tempNode = document.createElement('div');
    tempNode.id = 'fbFixedProfPicSection';
    tempNode.innerHTML = '<li class="fbFixedProfPics"><a href=""><img src="" height="100%"/><span class="fbFixedTooltips uiTooltipWrap bottom left 

leftbottom"><span class="fbFixedTooltipsText uiTooltipText uiTooltipNoWrap"></span></span></a></li><li class="fbFixedProfPics"><a href=""><img src="" 

height="100%"/><span class="fbFixedTooltips uiTooltipWrap bottom left leftbottom"><span class="fbFixedTooltipsText uiTooltipText 

uiTooltipNoWrap"></span></span></a></li><li class="fbFixedProfPics"><a href=""><img src="" height="100%"/><span class="fbFixedTooltips uiTooltipWrap bottom 

left leftbottom"><span class="fbFixedTooltipsText uiTooltipText uiTooltipNoWrap"></span></span></a></li><li class="fbFixedProfPics"><a href=""><img src="" 
...
left leftbottom"><span class="fbFixedTooltipsText uiTooltipText uiTooltipNoWrap"></span></span></a></li><li id="fbFixedProfPicSelf" 

class="fbFixedProfPics"><a href="http://www.facebook.com/profile.php?id='+user+'"><img src="http://graph.facebook.com/'+user+'/picture?type=square" 

height="100%"/><span class="fbFixedTooltips uiTooltipWrap bottom left leftbottom"><span class="uiTooltipText uiTooltipNoWrap">Your 
    ...
    for (var i = 0; i < profPicNode.children.length-1; i++) {
        if (options['profPic'+i] != 'none') {
			...
			if (options['profPic'+i+'Name'] == 't')
				profPicNode.children[i].firstChild.href = 'http://www.facebook.com/'+options['profPic'+i];
			else
				profPicNode.children[i].firstChild.href = 'http://www.facebook.com/profile.php?id='+options['profPic'+i];
            profPicNode.children[i].firstChild.firstChild.src = 'http://graph.facebook.com/'+options['profPic'+i]+'/picture?type=square';
        } else {
...
function createOptions() {
    ...
    tempNode.innerHTML = '<div id="fbFixedOptionsMain"><div id="fbFixedOptionsHeader"><h1>Facebook Fixed Header Options</h1><h2></h2></div><form 

id="fbFixedForm" name="fbFixedForm"></div><div id="fbFixedOptionsSubmitPanel"><input style="float: left;" id="fbFixedOptionsSubmit" onclick="return false;" 

type="submit" value="Submit" /><input style="float: left;" type="submit" value="Cancel" onclick="return false;" id="fbFixedOptionsCancel"><p style="float: 

right; margin: 6px 0 0;">Created by <a target="_blank" href="http://skoshy.com">Stefan Koshy</a> | Current Version: <b>'+currentVersion+'</b> | <a 

href="http://userscripts.org/scripts/show/103328" target="_blank">Check here for updates</a></p></div>';
...
function showOptions() {
	...
	if (!otherUserID) 
		document.getElementById('fbFixedOptionsHeader').getElementsByTagName('h2')[0].innerHTML = '<span style="color: maroon;">Not viewing a 

profile: go to a Profile/Page to get its ID.</span>';
	else
		document.getElementById('fbFixedOptionsHeader').getElementsByTagName('h2')[0].innerHTML = '<img src="http://graph.facebook.com/'+otherUserID

+'/picture?type=square"> <b>'+document.getElementsByClassName('profileName')[0].innerHTML+'</b>\'s ID -- <textarea style="font-weight: bold; padding: 0; 

margin: 0; width: 150px; height: 14px; border: 1px solid black; background: transparent; text-align: center; font-size: 12px; color: red;" readonly="true">' 

+ otherUserID + '</textarea>';
}
...
function updateOptions() {
    ...
    if (options.profPicForSelf == 't')
        text += 'checked ';
    text += 'value="profPicForSelf" /> Enable a profile picture link instead of the "Profile" link</p></td></tr><tr><td style="width: 50%;"><p><input 

type="checkbox" name="enableFixedHeader" ';
	if (options.enableFixedHeader == 't')
        text += 'checked ';
...
';
	if (options.disableFindFriendsLink == 't')
        text += 'checked ';
	text += 'value="disableFindFriendsLink" /> Disable the <a href="http://i.imgur.com/ssFJ5.png" target="_blank">"Find Friends" 

link</a></p></td></tr></table><div style="display: table;"><table id="fbFixedOptionsProfPicTable" style="display: table-cell;"><tr><th>#</th><th>Profile 

Username/ID</th></tr><tr><td>1</td><td><input type="text" name="profPic0" class="fbFixedProfPicInput" 

value="'+options.profPic0+'"/></td></tr><tr><td>2</td><td><input type="text" name="profPic1" class="fbFixedProfPicInput" 

value="'+options.profPic1+'"/></td></tr><tr><td>3</td><td><input type="text" name="profPic2" class="fbFixedProfPicInput" 

value="'+options.profPic2+'"/></td></tr><tr><td>4</td><td><input type="text" name="profPic3" class="fbFixedProfPicInput" 

value="'+options.profPic3+'"/></td></tr><tr><td>5</td><td><input type="text" name="profPic4" class="fbFixedProfPicInput" 

value="'+options.profPic4+'"/></td></tr><tr><td>6</td><td><input type="text" name="profPic5" class="fbFixedProfPicInput" 

value="'+options.profPic5+'"/></td></tr><tr><td>7</td><td><input type="text" name="profPic6" class="fbFixedProfPicInput" 

value="'+options.profPic6+'"/></td></tr></table><p style="vertical-align: middle; background-color: rgba(255, 255, 100, 0.3); border: 1px solid #FFFF5A; 

padding: 10px; margin-top: 0; display: table-cell;"><b>Profile Picture Bookmarks Instructions</b><br/><br/>To modify/delete/add bookmarks to the Facebook 

header, change the values in the table below.<br/>A <b>username</b> is like http://facebook.com/<b>john.doe</b>. A <b>user ID</b> is like 

http://facebook.com/profile.php?id=<b>100438743</b>.<br/><br/>To get a friend\'s/Page\'s user ID easily, go to their profile and open up this options panel. 
