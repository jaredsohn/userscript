...
// @require        	http://xavier.hinfray.free.fr/ikariam/87680.js
// @require       	http://xavier.hinfray.free.fr/ikariam/57756.js 
// @require           http://xavier.hinfray.free.fr/ikariam/58203.js
// @require           http://xavier.hinfray.free.fr/ikariam/62718.js
// @version	0.98
// 
...
Config.settings = {
	"General":{
		fields:{
			externalArmyHelper:{
				type:'checkbox',
				label:'<a href="http://127.0.0.1/show/59908" target="_blank" title="v0.23 by GrAndAG" style="font-weight:bold !important;">Army Helper</a>',
	...
	"Events":{
		...
		fields:{
			...
			eventsEmailHtml:{
				type:'html',
				value:'  <p style="border-top:1px solid #ccc; padding-top:1em !important;"><strong>Email Settings</strong> - see "Email Settings" on <a href="http://127.0.0.1/show/80545" target="_blank">this page</a> for more information.</p>',
			},
			emailNoticeUrl:{
				type:'text',
				label:'Server',
	...
	"About":{
		html:'	<p><a href="http://127.0.0.1/show/' + ScriptUpdater.scriptId + '" target="_blank" style="font-weight:bold !important;">Ikariam ExMachina v' + ScriptUpdater.scriptCurrentVersion +
...
/*
			...
			label.innerHTML = '<a href="http://127.0.0.1/show/58191" style="font-weight:bold;">Library Tools v' + ScriptUpdater.scriptCurrentVersion + '</a> by <a href="http://userscripts.org/users/PhasmaExMachina">PhasmaExMachina</a> - [ <a href="javascript:void(0)" id="libraryImprovementsUpdates>check for updates</a> ]';
...
IAB = {
	...
	views:{
		...
		options:function() {	
			...
			content.innerHTML = '<a name="autoBuild"></a><h3><a href="http://127.0.0.1/show/80545" target="_blank">Ikariam Auto Build' + 
	...
	text:{
		...
		el:{	// Greek
			addToQueue:'???s?ese st?? ????',
			autoBuild:'??t?µat? ?t?s?µ?', // name of script
			demolishConfirm:'???s???! ????e?ta? ?a ?atedaf?se?? ???????O???? a?t? t? ?t????. ??sa? s???????;',
			options:{
				by:'by', // used in "Auto Build by PhasmaExMachina"
				descriptions:{
				smartChecking:'???s?µ?p??se t?? ???p?? ??e???',
				autoDisable:'?pe?e???p???se t? script a? e??a? a?e?e??? ??a p??? ??a',
				highlightRows:'highlight alternate rows in Auto Build info box',
				},
				help:{
					smartChecking:'? ???p??? ??e???? ?a tse???e? a?t?µata ???e 30 de?te???epta a? ??t? st?? ???? ? st?? ??sta a?t?µat?? ?t?s?µat?? µp??e? ?a ?t?ste?. ??t? ???eta? ????? ep????????a µe t??? server t?? pa????d???.',
					autoDisable:'??t? ? ep????? ?a ape?e???p??e? t? a?t?µat? ?t?s?µ? st?? pe??pt?s? p?? t? script de? ??e? t???e? ??a a??et? ?????, p???µa t? ?p??? µp??e? ?a e??a? ???s?µ? a? pa??e?? se p?????? d?af??et????? ?p?????st??. G?a pa??de??µa a? pa??e?? se ??a? ?p?????st? µ??a ?a? µet? pa? se ??a? ???? ? se??? p?? e??e? af?se? st?? p??t? µp??e? ?a µ? se e??p??ete? p?a, ?a? ?a µ? ???e?? ?a s??e??se? ?a ?s??e?.',
				},
				hour:'??a',
				hours:'??e?',
				labels:{
					show:'?µf???se',
					reCheckEvery:'?pa???e??e ??a a?t?µat? ?t?s?µ? ???e',
					smartChecking:'???p??? ??e????',
					autoDisable:'??t?µat? ape?e???p???s?',
					highlightRows:'Highlight Rows',
				},
				minute:'?ept?',
				minutes:'?ept?',
				show:{
					allCities:'??a ta ?t???a se ??e? t?? p??e??',
					currentCity:'???? ta ?t???a se a?t? t?? p???',
				},
				updatesAndHistory:'Script Updates & History',
				v:'v', // used to label version number (e.g. "v0.36")
			},
			queue:{
				autoUpgrade:'??t?µat? a?aß??µ?s?',
				change:'a??a??',
				clearAll:'d?a??af? ????',
				clearAllDesc:'?fa??ese ??a ta ?t?s?µata ap? t?? a?t?µat? ??sta ?t?s?µat??',
				downInQueue:'up in queue', // used in "?eta????se [building name] µ?a ??s? ??t? st? ??sta"
				emptyNotice:'? ??sta a?t?µat?? ?t?s?µat?? e??a? ?de?a.',
				enableAutoBuild:'??e???p???se t? a?t?µat? ?t?s?µ?',
				enableDisableAutoUpgrades:'??e???p???se / ?pe?e???p???se t? a?t?µat? ?t?s?µ? ??a ??e? t?? p??e??',
				expandOrCollapse:{
					autoBuild:'?e??st?p???se ? e?a??st?p??se t? ??sta a?t?µat?? ?t?s?µat??',
					autoUpgrades:'?e??st?p???se ? e?a??st?p??se t?? a?t?µate? a?aßa?µ?se??',
				},
				fromAutoUpgrades:'ap? t?? a?t?µate? a?aßa?µ?se??', // used in "?fa??ese [building name] ap? t?? a?t?µate? a?aßa?µ?se??"
				fromQueue:'ap? t? ??sta', // used in "?fa??ese [building name] ap? t?? ??sta"
				move:'?eta????se', // used in "?eta????se [building name] p???/??t? st? ??sta"
				queue:'???e st? ??sta',
				showing:{
					allCities:'??e? ?? p??e??',
					currentCity:'??????sa p???',
				},
				upInQueue:'???? st? ??sta', // used in "?eta????se [building name] µ?a ??s? p??? st? ??sta"
				remove:'?fa??ese', // used in "?fa??ese [building name] ap? ..."
				timeUntilNextDesc:'?p????p?µe??? ?????? µ???? t?? ep?µe?? ??e??? ??a a?t?µat? ?t?s?µ? (???e ???? ??a ?a epa?e????e?? t??a)',
			},			
		},
	},
}
IkaNotify = {
	...
	views:{
		options:function() {
			...
			var html = '<a name="ikaNotify"></a><h3><a href="http://127.0.0.1/show/80545" target="_blank" title="More information about this script">Ikariam Notifier</a> \
...
var ExternalScripts = {}
//----------------------------- Ikariam Army Helper v0.23 - http://127.0.0.1/show/59908 
ExternalScripts.ArmyHelper = function () {
	const languages = {
		...
		"greek": {
			...
			air: "??a???e? ????de?",
			line1: "????a G?aµµ? ?????",
			line2: "G?aµµ? ????? ?e????? ?µß??e?a?",
			flankLeft: "???ste?? ??e???",
			flankRight: "?e??? ??e???",
			submarine: "?p?ß????e? ????de?",
			reserve: "?p??eµa",
			phalanx: "?p??t??",
			steamgiant: "G??a?ta? ?tµ??",
			spearman: "??t??e?t?? ???at??",
			swordsman: "??f?µ????",
			slinger: "??t??e?t??",
			archer: "????t??",
			...
			medic: "G?at???",
			ship_ram: "S??f??(-?) ?µß???",
			ship_flamethrower: "F????ß???(-a)",
			ship_steamboat: "S??f??(-?) ???p?-??da-?????",
			ship_ballista: "S??f??(-?) ?a???st??",
			ship_catapult: "S??f??(-?) ?atape?t??",
			ship_mortar: "S??f??(-?) ?????µat??",
			ship_submarine: "????a(-e?) ?at?d?s??",
			addSlot: "Add units to one slot",
			removeSlot: "Remove units from one slot",
			fillRow: "Fill out one row with units",
			selectUnits: "Select units",
		...
		"hungarian": {
			...
			ship_steamboat: "Evezokerék hajó",
		...
		"latvian": {
			garrison: "Garnizons",
			advisor: "Konsultants",
			cityLevel: "Pilsetas limenis",
			support: "Palidziba",
			artillery: "Artilerija",
			air: "Gaisa speki",
			line1: "Priekšeja linija",
			line2: "Talas kaujas linija",
			...
			cook: "Pavars",
			medic: "Arsts",
			ship_ram: "Tarana kugis",
			ship_flamethrower: "Ugunsmetejs",
			ship_steamboat: "Dzenrata kugis",
			ship_ballista: "Ballistiskais kugis",
			ship_catapult: "Katapultas kugis",
			ship_mortar: "Minmeteja kugis",
			ship_submarine: "Zemudene",
			addSlot: "Pievienot vienibas uz vienu laucinu",
			removeSlot: "Nonemt vienibas no viena laucina",
			fillRow: "Aizpildit vienu kaujas rindu ar vienibam",
			selectUnits: "Izvelies vienibas",
			assignNone: "Nevienu",
			assignAll: "Sutit visu",
			assignField: "Aizpildit kaujas lauku",
			presetsTitle: "Iepriekšplanots sastavs",
			presetsAdd: "Pievienot jaunu sastavu",
			presetsRemove: "Dzest sastavu",
			presetsNewName: "Noradiet nosaukumu kaujas sastavam lai to saglabatu",
			optShowTitles: "Radit kaujas rindu virsrakstus"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		//  "lithuanian":   { },
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		"polish": {
			...
			air: "Lotnictwo",
			line1: "Linia frontu",
			line2: "Linia walki na odleglosc",
			flankLeft: "Lewa flanka",
			flankRight: "Prawa flanka",
			submarine: "Okrety podwodne",
			reserve: "Rezerwa",
			phalanx: "Hoplita",
			steamgiant: "Gigant",
			spearman: "Wlócznik",
			...
			cook: "Kucharz",
			medic: "Medyk",
			ship_ram: "Okret z taranem",
			ship_flamethrower: "Okret z miotaczem ognia",
			ship_steamboat: "Okret parowy z taranem",
			ship_ballista: "Balista",
			ship_catapult: "Okret z katapulta",
			ship_mortar: "Okret z mozdzierzem",
			ship_submarine: "Okret podwodny",
			addSlot: "Wypelnij jeden slot",
			removeSlot: "Opróznij jeden slot",
			fillRow: "Wypelnij cala linie",
			selectUnits: "Wybierz jednostki",
			assignNone: "Nic",
			assignAll: "Wszystko",
			assignField: "Wypelnij pole bitwy",
			presetsTitle: "Ustawienia",
			presetsAdd: "Dodaj ustawienie",
			presetsRemove: "Usun ustawienie",
			presetsNewName: "Podaj nazwe ustawienia",
			optShowTitles: "Show battlefield row titles"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		"portuguese": {
		...
		"turkish": {
			garrison: "Garnizon",
			advisor: "Danisman",
			cityLevel: "Sehir Boyutu",
			support: "Destek",
			artillery: "Topçu",
			air: "Hava",
			line1: "Ön Cephe",
			line2: "Uzun Menzilli Savas Hatti",
			flankLeft: "Sol Kanat",
			flankRight: "Sag Kanat",
			submarine: "Submarine",
			reserve: "Rezerv",
			phalanx: "Phalanx",
	...
	function drawAttackHelper(type, draw){
		...
		if (draw) {
			...
			html += '<div style="margin: -18px 10px 8px 5px; text-align: right;">By <a target="_blank" href="http://127.0.0.1/show/59908">Ikariam Army Helper</a>.</div>\n';
	...
	function drawOptions(){
		var div = document.createElement('div');
		div.setAttribute('class', "content");
		var html = '<h3><a href="http://127.0.0.1/show/59908" target="_blank">Ikariam Army Helper </a> <span style="font-weight: normal;"> by <a href="http://userscripts.org/users/grandag" target="_blank">GrAndAG</a></span></h3>'
