...
(function () {
  ...
  var DOWNLOAD_LINK_MESSAGE='Pobierz';
  var DOWNLOAD_TOOLTIP_MESSAGE='Pobierz ten film wideo';
  ...
  var DEFAULT_UI=1, PANDA_UI=2;
   
  var videoId, videoTicket, videoFormats, videoTitle='';
  var interfaceVersion=DEFAULT_UI;
  run();
...
function run(){
  ...
  if (headerTitle!=null){
    videoTitle=headerTitle.textContent || headerTitle.innerText || ''; // innerText for IE<9
  }
  if (videoTitle==''){
  ...
  buttonElement.setAttribute('class', 'yt-uix-button yt-uix-tooltip' + ((interfaceVersion==DEFAULT_UI)?' yt-uix-tooltip-reverse':''));  
