...
//
// ==UserScript==
// @name          Link Accoutrements
// @namespace     http://aurgasm.us
// @description   Append a small icon to mailto, PDF, doc, xls, & ppt links
// @include       http://*
// @include       https://*
// @include       file://*
// ==/UserScript==
...
for (var i = 0; i < links.snapshotLength; i++) {
   ...
   if (a.lastChild.nodeName == "IMG") continue;
   
   icon = document.createElement('img');  // I'm creating elements for every link, but never putting them in the document. Is this bad? Is there a way delete them?
   icon.height = 16; icon.width  = 16; icon.style.margin = 0; icon.style.padding = 0; icon.style.paddingLeft = '1px'; icon.style.border = 0;   icon.style.verticalAlign = "text-bottom";   
   ...
   if(a.href.match(re_mail)) {
      if (a.text.match(re_mailat) || a.text.match(re_maile))  continue; // if the link text is an address or says email, dont place an icon.
