// ==UserScript==
// @name           Calculadora de Chatarra
// @namespace      http://ego-shooters.foren-city.de/
// @description    Puedes calcular Cuanto dinero obtendr�s tras la venta, o cuanta chatarra vender para obtener una cantidad deseada
...
// @include        http://www.mendigogame.es/stock/bottle/
// @include        http://mendigogame.es/stock/bottle/sell/
// @include        http://www.mendigogame.es/stock/bottle/sell/
...
tr[1].innerHTML = '<td><img src="http://media.pennergame.de/img/cash.png" alt="summe"></td><td>Obtenido de la venta</td><td><input id="summe" type="text" size="3" name="summe" value="' + my_kurs_2 + '" onKeyup="document.getElementById(\'menge_verkauf\').value = Math.ceil(document.getElementById(\'summe\').value / ' + my_kurs_2 + '); document.getElementById(\'all\').value = Math.round((Number(document.getElementById(\'summe\').value) + '+ cash +')*100)/100; if(document.getElementById(\'all\').value.length > 6){document.getElementById(\'all\').size++;document.getElementById(\'summe\').size++;document.getElementById(\'menge_verkauf\').size++;};"/>&nbsp;&euro;</td>';


tr[2].innerHTML = '<td><img src="http://media.pennergame.de/img/cash.png" alt="summe2"></td><td>Dinero total tras la venta</td><td><input id="all" type="text" size="3" name="all" value="' + (Math.round((Number(cash) + Number(my_kurs_2))*100)/100) + '" onKeyup="document.getElementById(\'menge_verkauf\').value = Math.ceil((document.getElementById(\'all\').value - '+ cash +') / ' + my_kurs_2 + '); document.getElementById(\'summe\').value = Math.round((document.getElementById(\'all\').value - '+ cash +')*100)/100; if(document.getElementById(\'all\').value.length > 6){document.getElementById(\'all\').size++;document.getElementById(\'summe\').size++;document.getElementById(\'menge_verkauf\').size++;};"/>&nbsp;&euro;</td>';

tr[3].innerHTML = '<td colspan="3">Maximo rendimiento:</td>';
tr[4].innerHTML = '<td colspan="3">Al precio actual '+ my_kurs_2*100 +' Cent = '+ Math.round(maximal*my_kurs_2*100)/100 +' &euro;</td>';
tr[5].innerHTML = '<td colspan="3">Venta  16 Cent =  '+ Math.round(maximal*16)/100 +' &euro;</td>';
tr[6].innerHTML = '<td colspan="3">Venta  17 Cent =  '+ Math.round(maximal*17)/100 +' &euro;</td>';
tr[7].innerHTML = '<td colspan="3">Venta  18 Cent =  '+ Math.round(maximal*18)/100 +' &euro;</td>';
