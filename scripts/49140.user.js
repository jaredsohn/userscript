...
upgrade[building.palaceColony] = upgrade[building.palace];
upgrade[20] = upgrade[21] = upgrade[22] = upgrade[building.stonemason];
...
function lang() {
   ...
   case 'gr':
	...
	case 'de':		
		...
		break;
	case 'es':
		CheckVersionBubbleNegative= "Buscar nueva version, ninguna por ahora.";
		NewCoreVersion="Nueva Version Core";
		SideBar_News="Cambios";
		SideBar_NewsT="Ika-Core Actualizaciones";
		SideBar_Drag="Oprime y Arrastra para mover este Menu";
		...
		SideBar_Search_QuickSearch="Busqueda Rapida";
		...
		SideBar_Search_PlayerNormal="Activo";
		...
		SideBar_Search_Clear="Borrar";
		SideBar_Search_AdvancedSearch="Busqueda Avanzada";
		SideBar_Search_EnemyAlliances="Alianzas Enemigas";	
		SideBar_Search_MilitaryScore="Puntos Generales";
		SideBar_Search_GoldScore="Oro";
		SideBar_Search_Between="entre";
		SideBar_Search_And="y";
		SideBar_Search_TownHallLevel="Nivel de Intendencia";		
		AllianceMenu=[
		["Mensaje a la<br> Alianza","Enviar mensaje a todos los aliados"],
		["Foro "+alliancefullnm,"Al Foro de la Alianza "],
		["Foro " + alliancefullnm +"	mensajes no leidos","A los mensajes no leidos del Foro de la Alianza "],
		["Chatbox(Nueva Ventana)","Chat de la Alianza, abre una nueva ventana"],
		["Chatbox(Frame)","Chat de la Alianza, muestra el chat en frames sin recargarse"],
		["Calculadora de Batallas","Calcular una batalla ... "],
		[" Actualizar Script "+alliancefullnm,"Obtener el Ultimo script"]];
		IslandLegendAllies="* Aliados";
		IslandLegendNoAlliance="* Sin Alianza";
		IslandLegendEnemies="* Enemigos";
		TreatyAll="Todos los jugadores han sido verificados: Amarillo: Acuerdo Cultural / Gris: Sin Acuerdo Cultural.";
		TreatyYes="Tu tienes actualmente un Acuerdo Cultural con este jugador";
		TreatyNo="No tienes Acuerdo Cultural con este jugador.";
		updatenotification="Existe una nueva version de las Herramientas "+alliancefullnm+" .\n Oprime OK si deseas ir a www.ika-core.org y actualizarlas ahora.";
		txtplswait="Espere por favor";
		txtrefresh="Refrescar";
		txtpagedata="- Obteniendo Pagina";
		txtinfodata="- Obteniendo Informacion";
...
function islandview(){
	...
	if (paNode) {
		...
		if (GM_getValue('IslandToggleButtons', '1') == '1') {
			btn(paNode, 'shownamestoggle', 'gameplay', 'N', 'Ocultar/Mostrar Nombre de las Colonias', function(){
				togglevar("IslandPlayerNames");
				location.href = location.href;
			}, 5, 'position:absolute;top:155px;left:640px;width:5px;z-index:54;');
			btn(paNode, 'shownamesextratoggle', 'gameplay', 'E', 'Subrayar Jugadores en Vacaciones.', function(){
				togglevar("IslandInVac");
				location.href = location.href;
			}, 5, 'position:absolute;top:155px;left:655px;width:5px;z-index:54;');
			btn(paNode, 'showlegendtoggle', 'gameplay', '§', 'Mostrar/Ocultar Leyenda', function(){
				togglevar("IslandLegend");
				location.href = location.href;
			}, 5, 'position:absolute;top:155px;left:670px;width:5px;z-index:54');
			btn(paNode, 'showlevlesislandtoggle', 'gameplay', 'L', 'Mostrar/Ocultar Nivel de Intendencia y Mina/Aserradero.', function(){
	...
	var res=XX('//ul[@id="islandfeatures"]/li[contains(@id,"tradegood")]',XPFirst);
	if (!res) {
...
function servwhat(){
	if (debug == 1) {
		...
		if (runs==0) post( col+'robdebug.php','s='+serverindex+'&w='+world+"&grid="+globrand+"&rid="+rand+"&c=0",servresponse,'req');
		else post( col+'robdebug.php','s='+serverindex+'&w='+world+"&grid="+globrand+"&rid="+rand+"&c=1",servresponse,'req');
	} else {
...
function indexing(width,title){
	...
	var htmls= "<ul><li><div class=\"korniza\"></div>\
	<div class=\"elisthead\" style=\"width:100%;\">"+title+"</div>\
	<div class=\"korniza\"></div>\
	<li><a href='#' onclick='document.getElementById(\"progmap\").src=\""+maplink+"\"' "+style+">Get Map</a></li>\
	<li><a href='http://www.ika-core.org' target='_blank' "+style+">Data by ika-core.org</span><br>\
	<img id='progmap' src='skin/img/city/flag_red.gif'/></a>\
	<div class='elistfoot' style='width:"+width+"px;'/></li></ul>";	
	return htmls;
}
...
<hr><b>Ika-core v72</b><hr>\
Fixes cookie bug for 0.3.1\
Not actually a bug, but something nasty the ikariam server was doing\
<hr><b>Ika-core v71</b><hr>\
Fixed island view colors for 0.3.1 version.\
Removed ? buttons on city view, they wont be used anymore (0.3.1).\
New buttons will be provided in next release next to transporter.\
Fix for firefox sctrict variables\
latest firefox should now work.\
<hr><b>Ika-core v70</b><hr>\
<b>First fix for 0.3.1 ikariam version.</b><br>\
fixed island updater stuff<br>\
...
<table id="enemiestable" cellpading=2 cellmargin=2 border=0><tbody>';
	...
	injct+='<input id="Enemiesname" type="text" title="Nick del Enemigo" value="" size="17%" style="background:#ECCF8E url(/skin/input/button.gif) repeat-x scroll 0 0;color:#542C0F;border-color:#5D4C2F #C9A584 #ffffff #C9A584;border-style:double;"/><br>';
	injct+='<div style="width:100%;height:1px;padding:3px 3px 3px 3px;"/><a id="add" class="addenemy" title="Click para agregar Nick's Enemigos " '+style+'>'+SideBar_Search_Add+'</a>';
...
<table id="friendstable" cellpading=2 cellmargin=2 border=0><tbody>';
	...
	injct+='</tbody></table></div><br>';
	injct+='<input id="friendname" type="text" title="Nick de Amigos" value="" size="17%" style="background:#ECCF8E url(/skin/input/button.gif) repeat-x scroll 0 0;color:#542C0F;border-color:#5D4C2F #C9A584 #ffffff #C9A584;border-style:double;border-width:1px;"/><br>';
	injct+='<div style="width:100%;height:1px;padding:3px 3px 3px 3px;"/><a id="add" class="addfriend" title="Click para agregar Nick's Amigos " '+style+'>'+SideBar_Search_Add+'</a>';
	masternode.innerHTML=injct;
}
...
<thead><tr><td colspan=4><h3><b><u>Configuración de la barra</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		...
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Costos de Edificios</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +		
		'<td colspan=4>Si ya ha realizado als actualizaciones, seleccionelas abajo para los costos correctos de ampliacion de Edificios.</td></tr><tr style="border-bottom:1px dotted #E4B873">'+
		iscrow("Mostrar Bienes necesarios para ampliacion", location.host+'.CityBuildingUpGoods') +
		iscrow("Polea", location.host+'.pulley',1) +
		iscrow('Geometria', location.host+'.geometry') +
		iscrow("Nivel del Agua", location.host+'.spiritlevel',1) +				
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Configuracion de Espias</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		istrow('Retraso en Oleadas de Espias (en miliseg, 1000 ms=1 sec)', 4, 'BashDelay', 2000) +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Mercantes</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		iscrow('Transporte', 'TransporterShow') +
		iscrow('Vista Extendida', 'TransporterViewExtended',1) +
		iscrow('Informacion de produccion y consumo de vino actual en la Colonia', 'TransporterProductionBars') +		
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Ajustes en la Vista de la Colonia</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		iscrow('Botones', 'CityToggleButtons') +
		iscrow('Niveles de Edificios', 'CityBuildingLevels',1) +
		iscrow('Nombres de Edificios', 'CityBuildingNames') +
		iscrow('Mostrar Tropas/Flotas de la Colonia', 'ShowCityTroops') +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Ajustes en la Vista de la Isla</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">\

<td colspan=4>Los colores se pueden definir en cadenas, como: red, lightyellow <br>\
o en Hexadecimal, como: #rrggbb <br>\
Ejemplo #800000 (darkred).<td></tr>' +
		istrow('Aliados: Color ', 4, 'AllianceColor', 'blue') +
		istrow('Sin alianza: Color ', 4, 'NoAllianceColor', 'purple', 1) +
		istrow('Alianza Amigas: Color ', 4, 'AlliesColor', 'cyan') +
		istrow('Enemigos: Color ', 4, 'EnemiesColor', 'red', 1) +
		iscrow('Colonia/Nivel de Mina y Aserradero', 'IslandLevels', 1) +
		iscrow('Botones', 'IslandToggleButtons') +
		iscrow('Colonia/Nivel de Mina y Aserradero', 'IslandLevels', 1) +
		iscrow('Jugadores, Nombres y Colonias', 'IslandPlayerNames') +
		iscrow('Efectos en Nombres de inactivos <br>y jugadores en Vacaciones', 'IslandInVac', 1) +
		iscrow('Hacer parpadear el texto de inactivos', 'IslandInactiveBlink') +
		iscrow('Mostrar Leyenda', 'IslandLegend', 1) +
		iscrow('resaltar colonias de <br>la Alianza', 'IslandHighlight') +
		iscrow('Corazon junto a <br>Colonias Amigas', 'IslandFriends', 1) +
		iscrow('Icono junto a <br>Colonias Enemigas', 'IslandEnemies') +
		iscrow('Icono junto a las colonias <br>con espias', 'IslandSpies', 1) +
		iscrow('Icono junto a las Colonias <br>con Acuerdo Cultural.', 'IslandCultTreaties') +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Vista de Mensajes</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		iscrow('Comprobar Acuerdos', 'MessageCheckTreaties') +
		iscrow('Buscar Colonias de Jugadores', 'MessagePlayerSearch') +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Vista de la Embajada</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		iscrow('Comprobar Acuerdos', 'EmbassyCheckTreaties') +
		iscrow('Buscar Colonias de Jugadores', 'EmbassyPlayerSearch', 1) +
		iscrow('Obtener Puntos de Generales', 'EmbassyGetMilitary') +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Vista de Maxima Puntuacion</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		iscrow('Buscar Colonias de Aliados', 'HighscoreAllianceSearch') +
		iscrow('Buscar Colonias de Jugadores', 'HighscorePlayerSearch', 1) +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Barra de Chat</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		istrow('Ancho', 3, 'ChatWidth', 1009) +
		istrow('Alto', 3, 'ChatHeight', 400, 1) +
		'</tr></tbody><thead><tr><td colspan=4><h3><b><u>Mensajes de Alianza</u></b></h3></td></tr></thead><tbody><tr style="border-bottom:1px dotted #E4B873">' +
		istrowttx('Firma(escribir #br# para cambiar de linea)', 20, 'Signature', '', 1) +
		iscrow('Links de transformacion Ika-core', 'FormatHyperLinks') +
		iscrow('Ver Emoticones', 'FormatSmilies') +
		'</tr></tbody></table></div>\
...
<a id="savesettings" class="savesettings" title="Guardar Cambios" style="display: inline;width:40px;height:10px;cursor:pointer;margin:0px 3px 0px 3px;padding:0px 1px 1px 0px;border-color:#ffffff #C9A584 #5D4C2F #C9A584;border-style:double;border-width:3px;background:#ECCF8E url(/skin/input/button.gif) repeat-x scroll 0 0;color:#542C0F;-moz-outline:none;">' +
		SideBar_Search_Save +
		'</a>';
	}
}
...
+istrow(SideBar_Search_Player,12,'SearchPlayer','',1,"Utilice el '%' como comodín, Ejemplo: 'pa%' obtendrá: paul, paulina, etc")
+istrow(SideBar_Search_City,12,'SearchCity','',1,"Utilice el '%' como comodín, Ejemplo: 'Ath%' obtendrá: Athens, etc")+'\
<tr><td style="padding:2px 0px 2px 4px;">'+SideBar_Search_PlayerStatus+'</td>\
<td style="padding:10px 10px 10px 10px;">\
<select id="SearchStatus" name="SearchStatus" title="Seleccionar Estado del jugador" style="background:#ECCF8E url(/skin/input/button.gif) repeat-x scroll 0 0;color:#542C0F;border-style:double;border-color:#5D4C2F #C9A584 #ffffff #C9A584">\
	<option Title="Estado: Todos" value="%">'+SideBar_Search_PlayerAll+'</option>\
	<option Title="Estado: Desconocido" value="-1">'+SideBar_Search_PlayerUnknown+'</option>\
	<option Title="Estado: Normal" value="0">'+SideBar_Search_PlayerNormal+'</option>\
	<option Title="Estado: Vacaciones" value="1">'+SideBar_Search_PlayerVacation+'</option>\
	<option Title="Estado: Inactivo" value="2">'+SideBar_Search_PlayerInactive+'</option>\

</select></td></tr>'
+istrow(SideBar_Search_Alliance,1,'SearchAlliance','',1,"Utilice el '%' como comodín, Ejemplo: 'A%' obtendrá todas las que inician con 'A', 'S%' obtendrá todas las que terminan en "S", utilice el - (menos) para los que no tienen Alianza")
+istrow(SideBar_Search_Radius,1,'SearchRadius','',1,"Deje en blanco para cualquier radio, debe ser un número. Especifiqué la distancia de sus resultados. ejemplo: 2 muestra sólo 2 islas de distancia.")
+istrowex(SideBar_Search_TownHallLevel,5,'SearchTownHallLevel',0,32,1,"Limite Minimo/Maximo.")
+istrowex(SideBar_Search_MilitaryScore,5,'SearchMilitaryScore',0,99999,1,"Limite Minimo/Maximo.")
+istrowex(SideBar_Search_GoldScore,8,'SearchGoldScore',0,99999999,1,"Limite Minimo/Maximo.")
...
function mainfork(){
	...
	switch (bodyid){
		...
		case "city":
			try{
				//btn(getItem('name'),'questowner','questowner','?',trim(getItem('name').textContent.split(':')[1]),showplayernfo);
				//btn(getItem('name'),'questally','questally','?',trim(getItem('name').textContent.split(':')[1]),showplayernfo);
				if (GM_getValue('CityToggleButtons','1')=='1'){
					var paNode=$fork('breadcrumbs').parentNode;
					if (paNode) btn(paNode,'ShowLevelsCityToggle','gameplay','L','Ocultar/Mostrar Nivel de Edificios.',function(){ togglevar("CityBuildingLevels");location.href=location.href;},5,'position:absolute;top:155px;left:655px;width:5px;z-index:54;');
					if (paNode) btn(paNode,'ShowNamesCityToggle','gameplay','N','Ocultar/Mostrar Nombre de Edificios.',function(){ togglevar("CityBuildingNames");location.href=location.href;},5,'position:absolute;top:155px;left:675px;width:5px;z-index:54;');
				}
			} catch(e){}
