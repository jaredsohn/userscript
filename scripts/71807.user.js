...
var run_once=0,sarbazan=[''],nejad,anva = [' پشتیبانی ',' حمله ',' غارت '],suffixGlobal = location.hostname;
function level(){
...
(function(){
	...
	if(document.URL.search('.{2,}travian.{3,}/berichte.php.{1,}id')!=-1){
		...
		if (pb == null)return;
		anc.href='#'; anc.innerHTML='<B> تنظیمات غارت سریع </b>';
		anc.addEventListener('click',function(){
			...
			sc+= '<B>تنظیمات غارت سریع</b><BR>';
			popDiv.innerHTML=sc;
			_addNew.type='button';
			_addNew.value = ' اضافه کردن تعداد جدید ';
			popDiv.appendChild(_addNew);
			
			_addNew.addEventListener('click',function(){
				var new_tedad = parseInt(prompt("تعداد نیروی جدید را وارد کنید"));
