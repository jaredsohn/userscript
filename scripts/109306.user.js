...
// @version 0.9.1
// Chrome的最后一次单独更新，今后将只更新FF版本(Chrome通用)
// ==/UserScript==
    ...
    if (pageFiles.length>0)
    {
            for(i=0;i<pageFiles.length;i++)
        {
            ...
            if(mcAddr!=null)
            {
                ...
                mcAtom.checkboxs.push({'lastname':lastname,'checkbox':checkbox,'id':mcAtom.lastAddedCheckbox});
...
fetchChecked:function()
{
    ...
    $.each(checkedBox,function(i,n){
        ...
        if(className=='mcCheck')
        {
            ...
            mcAtom.mcQueue.push({'fileName':fileName,'Addr':furl});
            }
        }
    });
    ...
    else if(process=='fetch')
    {
        ...
        $.each(urls,function(i,url)
        {
            try
            {
                ...
                if(flag.length==32)
                {
                    mcAtom.mcQueue.push({'Addr':url,'fileName':null});
                }
            }
            catch(err)
            {
...
processByInput:function()
{
    ...
    var inputBox=$("<textarea id='fids'></textarea>").css({'width':'490px','height':'200px'});
...
showWindow:function(node,title)
{
    ...
    var naviWin=$('<div id="navWin"></div>')
    .append('<div id="boxHeader"></div>')
    .append('<div id="boxContent"></div>');
    naviWin.find('#boxHeader')
        .append('<span>'+title+'</span>')
        .append('<a style="float:right" id="close_btn" href="javascript:mcAtom.closeBox();">关闭</a>');    
    naviWin.find('#boxContent').append(node);
    mcAtom.$.blockUI({message:naviWin,css:{width:"520px",height:"300px"}});
},
    ...
    mcAtom.renewList=[];
        ...
        $.each(checkedBox,function(i,n){
            ...
            if(className=='mcCheck')
            {
                ...
                mcAtom.renewList.push({'filename':fileName,'fid':mcAddr[1]});
    //            mcAtom.requestMC(mcAtom.mcQueue);
                }
            }
        });
...
showAddress:function()
{
    ...
    mcAtom.addrBox=$('<div id="batchPublish"></div>')
                    ...
                    .append('<div id="30_link" class="mcTab"><textarea></textarea></div>')
                    .append('<div id="60_link" class="mcTab"><textarea></textarea></div>')
                    .append('<div id="instant_link" class="mcTab"><textarea></textarea></div>')
                    .append('<div id="unavailable_link" class="mcTab"><textarea></textarea></div>');
    
    mcAtom.addrBox.find('.mcTab').css('display','none');
    
    mcAtom.addrBox.find('#batchHeader')
    .append('<a style="float:left" id="output_txt" href="javascript:mcAtom.saveToDownList();">输出到文件</a>')
    .append('<a style="float:left;margin-left:10px" id="info_txt" href="javascript:mcAtom.saveInfoFile();">输出信息列表</a>')
    .append('<a style="float:left;margin-left:10px" id="linkpage" href="javascript:mcAtom.showLinkPage();">链接页面</a>')    
    .append('<span>MCATOM LIST</span>')
    .append('<a style="float:right" id="close_btn" href="javascript:mcAtom.closeBox();">关闭</a>');
    
    batchContent=mcAtom.addrBox.find('#batchContent');    
    
    
    mcAtom.addrBox.find('#batchBar ul')
    .append('<li><a href="#30_link" id="30_archor">30秒后可下载</a></li>')
    .append('<li><a href="#60_link" id="60_archor">60秒后可下载</a></li>')
    .append('<li><a href="#instant_link" id="instant_archor">立刻可下载</a></li>')
    .append('<li><a href="#unavailable_link" id="unavailable_archor">暂不可下载</a></li>')
    .find('li').css('list-style','none').css('float','left').css('margin-left','30px');
...
initButton:function()
{
    ...
    var fetchChecked=mcAtom.createButton($('<a href="javascript:mcAtom.fetchChecked();">提取所选</a>'));
    var selectAll=mcAtom.createButton($('<a href="javascript:mcAtom.checkAll();">全选/取消全选</a>'));
    var showAddr=mcAtom.createButton($('<a href="javascript:mcAtom.showAddress();">提取结果</a>'));
    var renewSelect=mcAtom.createButton($('<a href="javascript:mcAtom.renewChecked();">祝福所选</a>'));
    var manualbox=mcAtom.createButton($('<a href="javascript:mcAtom.processByInput();">手动提取/祝福</a>'));
    var checkLoc=mcAtom.createButton($('<a href="javascript:mcAtom.showCheckedLoc();">提取链接</a>'));
