...
(function () {
  ...
  if ( !scriptUrl )
  {
    ...
    for (var i=0; i<scripts.length; i++) {
      ...
      if (src && src.search(/togetherjs.js(\?.*)?$/) !== -1) {
      	scriptUrl=src.replace( /\/*togetherjs.js(\?.*)?$/,"" );
      	console.warn( "Detected scriptUrl as",scriptUrl );
        break;
      } else if (src && src.search(/togetherjs-min.js(\?.*)?$/) !== -1) {
      	scriptUrl=src.replace( /\/*togetherjs-min.js(\?.*)?$/,"" );
      	console.warn( "Detected scriptUrl as",scriptUrl );
        break;
      }
    }
  ...
  function addScript(url) {
    ...
    script.src = scriptUrl + url + "?bust=" + cacheBust;
