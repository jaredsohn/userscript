// ==UserScript==
// @name           Another Testy - Don't Install. D:<.
			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' ('+tankId+') Prongs has been glowing for '+seconds2HMS(gaiaT-glowT)+'.');
								if(open===true){
									...
									GM_openInTab('http://absol.site88.net/gtools/glowTime.php?id=3610281');
							...
							alert(aquaN+' ('+tankId+') Prongs will glow in '+seconds2HMS(glowT-gaiaT));
							if(open===true){
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(3435829,false);//Prongs
	}
}
checkTanks();
