...
var KOCAversion = 'Niknah 20110110, Fred-20110402a';
...
unsafeWindow.kocaListValues = function (notify){
...
unsafeWindow.kocaGetAllAttacks = function (notify){
  var notifyFunc = notify;
  setTimeout (function (){
      var ret = {};
      var atkPrefix = 'attack_'+ KOCAttack.GetServerId();
      var names = KOCAttack.browser_listValues();
      for (var i=0; i<names.length; i++){
        if (names[i].substr(0, atkPrefix.length)==atkPrefix)
          ret[names[i]] = JSON2.parse(KOCAttack.browser_getValue(names[i], null));
      }
      notifyFunc(ret);
    }, 1);
}

unsafeWindow.kocaGetValues = function (list, notify){
  var notifyFunc = notify;
  setTimeout (function (){
      var ret = {};
      for (var i=0; i<list.length; i++)
        ret[list[i]] = JSON2.parse(KOCAttack.browser_getValue(list[i], null));
      notifyFunc(ret);
    }, 1);
}

unsafeWindow.kocaSetValues = function (obj, notify){
  var notifyFunc = notify;
  setTimeout (function (){
      for (var k in obj){
        if (obj[k] == null)
          KOCAttack.browser_setValue(k, null);
        else
          KOCAttack.browser_setValue(k, JSON2.stringify(obj[k]));
      }
      notifyFunc();
    }, 1);
}
