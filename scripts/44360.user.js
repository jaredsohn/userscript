// ==UserScript==
// @name           Vkontakte Udaff Mod
// @namespace      http://aero.3dn.ru
// @description    Расширенные функции ВКонтакте.ру
...
var checkVKUpdateNow = GET['checkMUVUpdateNow']?true:false;
// проверка обновления vkUdaff Mod
if (checkUpdates || checkVKUpdateNow) {
	...
	if (checkVKUpdateNow)
		showDialog('Обновление Udaff Mod','<center><h1><font color="#444444"><b>Проверяю обновления...</b></font></h1></center>','','Спрятать','',hideFunc());
	
	if (nowDate.getTime() - lastDate.getTime() < 0) {
		GM_setValue('lastUpdatesCheck',nowDate.toUTCString());
	}

	if (lastDate.getTime() < nowDate.getTime() - updatePer*60*60*1000 || checkMUVUpdateNow) {
		GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://aero.3dn.ru/new_version',
				headers: {
					'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
					'Accept': 'application/atom+xml,application/xml,text/xml',
					},
...
function Update(responseDetails) {
	if (responseDetails.status == 200) {
	    	...
	    	if (version > MUVVersion && (version > skipVersion || checkMUVUpdateNow)) {
	    		ShowNewVersionDialog(version,changelog)
	    	} else if (checkMUVUpdateNow) {
	    		ShowNoNewVersion(MUVVersion);
	    	}
   	}
}
...
function ShowNewVersionDialog(version,changelog) {
	var title = "Обновление Udaff Mod";
	var text = '<h1><font color="#444444"><b>Новая версия Udaff Mod! '+version+'</b></font></h1><br><br>Изменения в новой версии:<ul>';
	...
	showDialog("Обновление Udaff Mod",text,"Обновить","Отмена",hideFunc()+"location.href='В будущем появиться ресурс'",hideFunc());	
}
function ShowNoNewVersion(version) {
	var title = "Обновление Udaff Mod";
	var text = '<center><h1><font color="#444444"><b>Твоя версия Udaff Mod '+version+' пока единственная и самая последняя=)</b></font></h1></center>';
	showDialog("Обновление Udaff Mod",text,'',"Ок",'',hideFunc());
}
...
function showReplyBox() {
	if (xmlhttp.readyState == 4) {
		...
		showDialog('Сообщение','<table>'+content+'</table>','Закрыть','Отправить',"document.getElementById('replyBoxOpened').value = 'false';"+hideFunc(),"this.disabled=true; if (ge('reply_to_mess').value.length > 1) {ge('postMessage_2').submit();} else {alert('А где текст? =).'); return false;};"+hideFunc(),485);
	}
}
...
if (location.pathname == '/settings.php') {
	...
	if (GET['act'] != 'Udaff Mod') {
		// Добавляем нашу вкладку
		menuElements.innerHTML += '<li>'+settInActiveLink('Udaff Mod','?act=Udaff Mod','4.8em')+'</li>';
		...
		// Добавляем нашу вкладку
		menuElements.innerHTML += '<li class="activeLink">'+settActiveLink('Udaff Mod','?act=Udaff Mod','4.8em')+'</li>';
		
		editorPanelElements.innerHTML = "     <div id=\"cname\" class=\"settingsPanel\">"+
		"<h4>Настройки Udaff Mod<small><font color='#98A7B5'>версия "+MUVVersion+"</font></small></h4><div align='right'><small><a href='?checkVKUpdateNow=1&act=Udaff Mod'>Проверить обновление</a></small></div>"+
		"<form method=\"GET\" id=\"vkpatchSettings\" name=\"vkpatchSettings\" action=\"\"><input type=\"hidden\" name=\"act\" value='Udaff Mod'><input type=\"hidden\" name=\"Udaff Mod\">"+
		...
		"<tbody>"+
			...
			settCheckbox('downAudio',			'Скачать mp3',			'Специально для тебя Удаффчик =*')+
			...
			settLine('Обновление')+
			settCheckbox('checkUpdates',		'Обновление',			'Проверять наличие новых версий vkPatch.')+		
...
if (document.getElementById('vkontakte')) {
	...
	link.innerHTML = '<a href=\"http://vkontakte.ru/id4794900" target=\"_blank\">Страница Удаффа =)</a>';
