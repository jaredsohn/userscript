...
function init () {
	...
	with( uW )
	{
		if ( WMap && WMap.mapData )
		{
			...
			$('head').append(
			"<style type='text/css'>"+
			".mFInfo { color:#ffc446; font-size:0.7em; border:0px; margin-top:10px;margin-left:20px;text-shadow:0.1em 0.1em 0.2em black;}"+
			".mFTrade { color:#40E040;  }"+
			".mFOK { color:#11caec;  }"+
			".mFRisk { color:#ffeb46; }"+
			".mFDanger { color:#ff4646;}"+
			"</style>" );
			...
			for (var townNr in towns )
			{
				...
				if ( town.mood )
				{
					...
					if (tel && tel.length > 0)
					{
						...
						+ ((town.mood < 80 ) ? (( town.mood < 60 ) ? (( town.mood < 40 ) ? "mFDanger" : "mFRisk" ) : "mFOK" ) : "mFTrade") +'">'
