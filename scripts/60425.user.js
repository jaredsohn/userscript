// ==UserScript==
// @name          Amazon ACLA Library Linky
// @namespace     http://fatknowledge.blogspot.com
// @description	  v1.3.2 Search the Allegheny County Public Library System Catalog from Amazon book listings.  
// @include       http://*.amazon.*
// ==/UserScript==
...
(function(){
//	GM_log('Amazon ACLA Library Linky');	

	var libraryName = 'Allegheny County Libraries';
	var libraryIsbnUrlPattern = 'http://catalog.einetwork.net/search/i?SEARCH='
	var libraryTitleUrlPattern = 'http://catalog.einetwork.net/search/t?SEARCH='
...
//other changes in getBookStatus() might be needed as well (for example to handle states not listed below)
	...
	var libraryAvailable = /AVAILABLE/;
	var libraryOnOrder = /copy ordered/;
	var libraryInProcessing = /IN PROCESSING/;
...
//					"#ffffac"  //light yellow
			...
			else if ( libraryAvailable.test(page) )
				{
				setLibraryHTML(
					libraryUrlPattern, isbn,
					"On the shelf now!",
...
//					"#ffffac"  //light yellow
			...
			else if ( libraryInProcessing.test(page) )
				{
				setLibraryHTML(
					libraryUrlPattern, isbn,
					"In processing!",
					"In processing (available soon) at "+ libraryName,
					"#AA7700"  // dark yellow
//					"#ffffac"  //light yellow
					);
			   foundCount++;
...
//					"#ffffac"  //light yellow
			   ...
			   foundCount++;
				...
				setLibraryHTML(
					libraryUrlPattern, isbn,
					"On the Reference Shelf now!",
					"Noncirculating version available now at "+ libraryName,
					"green" 
//					"#2bff81" //light green
					);
			   foundCount++;
