...
// @description    Greasemonkey script to simplify life in WOS
...
(
	function (){
		...
		const cDomain1 = "s1.worldofshinobi.eu";
		const cDomain2 = "www.worldofshinobi.eu";
		const cPasswd  = "pwd";
		const cLogin   = "login";
		...
		// =======================================================================================
		// Fonctions generiques pour positionner le mode Connexion automatique
		// =======================================================================================
		
		var fFlag_Cnx = function  (){
			var sIsActivated = fGetLogin();
			if ( sIsActivated == "" ){
				var mLogin = document.getElementsByName(cLogin);
				var mPasse = document.getElementsByName(cPasswd);
				var sLogin = mLogin[0].value;
				var sPasse = mPasse[0].value;
				...
				if (( sLogin != "" ) && ( sPasse != "" )){
					...
					alert("Mot de passe ou Login nom d\351fini");
				}
			} else {
				if ( confirm(" D\351sactiver la reconnexion automatique ? ") ){
					fSetLogin("");
					fSetPass("");
				}
			}
		...
		var fGetLogin = function (){ return fGetStr("CNX_LOGIN"); };
		var fGetPass  = function (){ return fGetStr("CNX_PASS"); };
		
		var fSetLogin = function (value){ fSetStr("CNX_LOGIN", value); };
		var fSetPass  = function (value){ fSetStr("CNX_PASS",  value); };
		
		var fPage_CNX = function (){
			if ( fGetLogin() != "" ){
				var mLogin = document.getElementsByName(cLogin);
				var mPasse = document.getElementsByName(cPasswd);
				mLogin[0].value = fGetLogin();
				mPasse[0].value = fGetPass();
				var form = document.forms[0];
				form.submit();
			}
		};
