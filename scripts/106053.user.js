...
// @namespace		HellHound
// @description		KoC Power Bot - German Version by PDX edit: HellHound
...
var Version = '1.0.5H';
var UpdateVersion ='20110613a-german';
...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'!!! ACHTUNG = ANGRIFF !!!', scouting:false, wilds:false,  defend:false, minTroops:1, spamLimit:10, lastAttack:0, barbautoswitch:false },
 ...
 m += '</select></td></tr>\
	  ...
	  <TR><TD align=right>Def Status Anzeigen: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
		<TR><TD align=right>Min. Truppenstärke: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
		 <TR><TD align=right>Bot Angriff deaktivieren bei Angriffen:  &nbsp; </td><TD><INPUT id=pbalertbarb type=checkbox '+ (Options.alertConfig.barb?'CHECKED':'') +'/></td></tr>\
<TR><TD align=right>Bot Angriff nach dem Angriff Aktevieren:  &nbsp; </td><TD><INPUT id=pbalertbarbres type=checkbox '+ (Options.alertConfig.barbresume?'CHECKED':'') +'/></td></tr>\
			</table></td></tr>\
		<TR><TD><BR></td></tr>\
...
/*********************************  Cresting Tab ***********************************/
 Tabs.Crest = {
  ...
  init : function (div){
    ...
    m += '<TABLE class=ptTab><TR><TD>Welle 1 -> </td><TD><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.png></td><TD><INPUT id=R1MM type=text size=5 maxlength=5 value="'+CrestOptions.R1MM+'" (Wenn keine Truppen mehr in der Stadt sind wird es aufhören die erste Welle zu schicken)</td>';
...
m += '<TR><TD>Welle 2 -> </td><TD><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.png></td><TD><INPUT id=R2MM type=text size=5 maxlength=5 value="'+CrestOptions.R2MM+'"</td>';
...
************/
 ...
 Tabs.Raid = {
  ...
  init : function (div){
	...
	AddSubTabLink('Raids fortsetzen', t.ResumeAllRaids, 'pbraidtabRes');
	AddSubTabLink('Raids löschen', t.DeleteAllRaids, 'pbraidtabDel');
	
	
	var m = '<DIV class=pbStat>RAID FUNKTION</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
	    m += '<TD><INPUT id=pbRaidStart type=submit value="Raid = '+ (Options.RaidRunning?'AN':'AUS') +'" ></td>';
	    m += '</tr></table></div>';
	    ...
	    m += '<DIV class=pbStat>GESPEICHERTEN RAIDS</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
  ...
  lookup : function (){
      	...
      	for (c=0; c< Seed.cities.length;c++) {
	   	...
	   	if (t.resuming == false && t.stopping == false && t.deleting == false && t.activecount != 0) document.getElementById('pbraidtab').innerHTML = '<span style="color: #ff6">Raids Stoppen ('+ t.activecount + ')</span>';
	   	else if (t.resuming == false && t.stopping == false && t.deleting == false) document.getElementById('pbraidtab').innerHTML = '<span style="color: #CCC">Raids Stoppen ('+ t.activecount + ')</span>';
   		if (t.resuming == false && t.resuming == false && t.deleting == false && t.stopcount !=0) document.getElementById('pbraidtabRes').innerHTML = '<span style="color: #ff6">Raids fortsetzen ('+ t.stopcount + ')</span>';
   		else if (t.resuming == false && t.stopping == false && t.deleting == false) document.getElementById('pbraidtabRes').innerHTML = '<span style="color: #CCC">Raids fortsetzen ('+ t.stopcount + ')</span>';
   		if (t.resuming == false && t.stopping == false && t.deleting == false && t.stopcount !=0) document.getElementById('pbraidtabDel').innerHTML = '<span style="color: #ff6">Raids löschen ('+ t.stopcount + ')</span>';
   		else if (t.resuming == false && t.stopping == false && t.deleting == false) document.getElementById('pbraidtabDel').innerHTML = '<span style="color: #CCC">Raids löschen ('+ t.stopcount + ')</span>';
  },
  ...
  paint : function ()	{
  	...
  	var botStat = {0:'Undefined',
  						...
  						7:'Timed out',
  						8:'Resting'};
  	var o = '<FONT size=2px><B><size=18><center><font size="+2">Schlachtzugstimer: <u><font color=#ff0000>'+ timestr( 86400 - ( unixTime() - t.rslt.settings.lastUpdated )) +'</font></font></u></center></size></b></font>';
  	document.getElementById('ptRaidTimer').innerHTML = o;
  	
  	var z ='<TABLE class=pbTab><TR><TD width=60px align=center><A onclick="pbStopAll()">Stoppen</a></td><TD width=70px>Zeit</td><TD width=85px>Koords</td><TD width=50px>Level</td><TD width=50px></td><TD width=50px><A onclick="pbDeleteAll()">LÖSCHEN</a></td></TR>';
  	if (t.rslt['queue'] != ""){
  	...
  	z+='</table>';
  	if (t.rslt['queue'] == "") z ='<TABLE class=pbTab><TR><TD>Keine Raids in der Stadt!</td></TR>';
  	document.getElementById('PaintRaids').innerHTML = z;
  	
  	var check = true;
  		if (t.save != ""){
  			var a ='<TABLE class=pbTab><TR><TD width=60px></td><TD width=70px></td><TD width=85px>Koords</td><TD width=50px>Level</td><TD width=50px></td><TD width=50px></td></tr>';
  			for (y in t.save){
  	...
  	if (check) a ='<TABLE class=pbTab><TR><TD>Keine gespeicherten Raids!</td></TR>';
  					 ...
  					 onSuccess: function(transport){
  						var rslt = eval("(" + transport.responseText + ")");
  		                  if (rslt.ok) {
  		                  		...
  		                  		actionLog('Stoppen: '+ Seed.cities[i][1]);
  		                  		updateraidbutton('Stoppen: '+ t.stopprogress.toFixed(0) + '%', 'pbraidtab');
  					 ...
  					 onSuccess: function(transport){
  						var rslt = eval("(" + transport.responseText + ")");
  		                  if (rslt.ok) {
  		                  		...
  		                  		if (t.stopprogress.toFixed(0) == 100) {
  		                  			 t.stopprogress = 0;
  		                  			 setTimeout(function(){updateraidbutton('Raids fortsetzen ('+ t.stopcount + ')', 'pbraidtabRes');t.resuming = false;}, (5000));
  		                  		}		
  						  }
  						  else {
  ...
  checkRaids : function (){
	...
	t.raidtimer = setTimeout(t.checkRaids, 10000);
  },
  ...
  toggleRaidState : function (){
  	...
  	if(Options.RaidRunning){
  		...
  		t.raidtimer = null;
  		document.getElementById('pbRaidStart').value = 'Raid = AUS';
  	} else {
  		Options.RaidRunning = true;
  		t.raidtimer = setTimeout(t.checkRaids, 5000);
  		document.getElementById('pbRaidStart').value = 'Raid = AN';
  	}
  },
...
Tabs.AutoTrain = {
  ...
  myDiv: null,
      ...
      var m = '<DIV id=pbAT class=pbStat>AUTO AUSBILDEN</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR align="center">';
      
      var m = '<DIV id=pbAT class=pbStat>AUTO AUSBILDEN</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR align="center">';
      if (TrainOptions.Running == false) {
          m += '<TD><INPUT id=pbAutoTrainState type=submit value="Auto Ausbilden = AUS"></td>';
      }else {
          m += '<TD><INPUT id=pbAutoTrainState   type=submit value="Auto Ausbilden = AN"></td>';
      }
      m += '<TD><INPUT id=pbShowTrainHelp type=submit value="Hilfe"></td>';
      m += '</tr></table></div>';
      
      
      
      
      m += '<DIV id=pbAT class=pbStat>AUSBILDUNGS EINSTELLUNGEN</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR align="center">';
      ...
      for (i=0;i<Seed.cities.length;i++){
	      ...
	      m += '<TD width=20px><SELECT id="TroopsCity'+city+'"><option value="Select">- = Wähle = -</options>';
  ...
  helpPop : function (){
    var helpText = '<BR>Automatisch Ausbilden:<UL><LI>Wähle ein Truppentyp pro Stadt aus dem Dropdown Menü.\
        <LI>Gebe die minnimum Rohstoffe ein, die in der Stadt bleiben sollen (0 bedeutet alles benutzen).\
        <LI>Mache in der Spalte der Stadt ein Häckchen (Die Nummer in den Spalten ist die Stadt angabe).\
        <LI>Drücke den Ausbilden Button damit er auf "AN" steht.</ul>\
        <BR><B>!!WICHTIG!!: Das minnimum der Anzahl der Truppen, die ausgebildet werden, ist 500! (Das wird im nächsten release eine freie variable).</b>';
  ...
  toggleAutoTrainState: function(obj){
        ...
        if (TrainOptions.Running == true) {
            ...
            obj.value = "Auto Ausbilden = AUS";
        }
        else {
            TrainOptions.Running = true;
            obj.value = "Auto Ausbilden = AN";
        }
        saveTrainOptions();
    },
