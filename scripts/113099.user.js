...
var hint={
	...
	create:function(){
		...
		var left=hint.getOffsetLeft(a,hint.needEl)+Math.round(a.clientWidth/2); var top=hint.getOffsetTop(a,hint.needEl)+Math.round(a.clientHeight/2);
...
function setUserInfo(text,href){
	...
	var name=utf8_decode(xmlDoc.evaluate('//name',xmlDoc,nsResolver,XPathResult.ANY_TYPE,null).iterateNext().textContent);
		...
		if(xmlDoc.evaluate('//taunt',xmlDoc,nsResolver,XPathResult.ANY_TYPE,null).iterateNext()){
			var taunt=xmlDoc.evaluate('//taunt',xmlDoc,nsResolver,XPathResult.ANY_TYPE,null).iterateNext().textContent;
			html+=printRow('&nbsp;','<div style="color:#777;font-size:0.9em;line-height:10px !important">'+taunt+'</div>');
		}
...
function getBreadName(number){
	switch(number){
		...
		default: if(number>=5 && number<=60) return 'Чушкарь ('+number+')'; else return 'ХЗ о_О ('+number+')';
	}
}
