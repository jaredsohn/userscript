...
function showOptions(){
  ...
  var element, id;
  var opts  = '<h3>Reorder Towns v?? (by <a href="http://userscripts.org/users/53907">Overkill</a>)</h3><i>(will automatically save, no need to hit the "Save settings!" button)</i><br />'
...
function writeTransportBoxHTML(){
  ...
  switch (transporterContainer) {
    ...
    case 'Overkill Bar' :
      ...
      overkillBar_add(barTab,'<a href="">Version: ??</a>');
...
if ($('servertime') && unsafeWindow.IKARIAM){
  ...
  var lst_val = GM_getValue( 'cities_' + server, "[]" );
  if( lst_val === undefined )
      var myCityList = [];
  else
      var myCityList = eval( lst_val );    // load saved order
