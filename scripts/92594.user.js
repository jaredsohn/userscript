...
// @name           astTools
// @namespace      ikatips
// @description    cambiar la fuente / calcular nivel de defensa
// @include        http://*ikariam.*/index.php*
// @author         Verx traducido por juan iglesias
// ==/UserScript==
...
'{display: block;}';
//esta característica es casi estándar, utilizada en muchos script greasemonkey
...
+ ' <li><h2>Tools</h2>'
+ '   <ul>'
+ '     <li><a href="#" title="Cambiar la fuente" onclick="showTools(1)">Fuente <input style="margin-left:10px; visibility:hidden;" id="c'+astToolsIds[0]+'" type=CheckBox CHECKED DISABLED /></a></li>'
+ '     <li><a href="#" title="Calcular la resistencia de la polis" onclick="showTools(2)">Defensa <input style="margin-left:10px; visibility:hidden;" id="c'+astToolsIds[2]+'" type=CheckBox CHECKED DISABLED/></a></li>'
+ '     <li><a href="#" title="Buscar un objetivo" onclick="showTools(3)">Escanear <input style="margin-left:10px; visibility:hidden;" id="c'+astToolsIds[3]+'" type=CheckBox CHECKED DISABLED/></a></li>'
+ '   </ul>'
+ ' </li>'
+'</ul>';
+'</DIV>';
...
var percdef = null;
if  (isNaN(parseInt(wl)) || isNaN(parseInt(cl))) {alerta ('Introduzca los valores numéricos!');document.getElementById('percDef').innerHTML =''}
...
+'<select id="Color">'
 ...
 +'<option>Red</option>'
+'</select>'
+'<input type="button" class="button" style="margin-top:-25px;margin-left:370px;" value="Cambiar" onclick="setFontIka()"/><input type="button" class="button" style="margin-top:-37px;margin-left:530px;" value="Restablecer" onclick="clearFont()"/></DIV></TD></TR></TABLE>';
var changeFontNode;
...
function initDef(){
var defBoxHTML =
'<DIV style="vertical-align: middle; text-align:center; margin-bottom: 10px; height: 30px; background: RGB(228,184,115);"><B>Cálculo de los muros de defensa</B></DIV>'+
'<TABLE><TR><TD align="left" valign="middle"><SPAN>Nivel Muros :</SPAN></TD><TD align="left" valign="middle"><input id="walllvl" type="text" size="20" maxlength="10" value=1 /></TD></TR><TR><TD align="left" valign="middle"><SPAN>Nivel Intendencia :</SPAN></TD><TD align="left" valign="middle"><input id="citylvl" type="text" size="20" maxlength="10" value=1 /></TD></TR><TR><TD colspan="2"  valign="middle" align="center"><input type="button" class="button" onclicK="calculateDefLvl()" value="Calcular % de la defensa" /></TD></TR><TR><TD colspan="2" valign="medio" align="centro"><DIV font-style: Arial; font-size: 14pt; text-align: center; vertical-align:center; id="percDef" style="width:95px; height:30px; border: 1px solid RGB(228,184,115);"></DIV></TD></TR></TABLE>';
...
addAstraToolsMenu();
//aparece en la parte superior izquierda abrir un enlace, al hacer clic muestra la seleccion para cambiar la fuente
initFontSelects();
//establece la fuente de la cookie (por defecto los datos se almacenan 2 días)
initFont();
initDef();