...
// @description    Collect stone,Claim mystery gift,Claim collectible gift,Adopt animal,Get buildable item,Get topiary,Help animal,Heal animal,Feed animal,Find animal,Lend hand
...
// @require        http://sizzlemctwizzle.com/updater.php?id=75837
// ==/UserScript==

(function() { // use an anonymous function wrapper

var version = '1.1.14'; // cant use cdata+regexp to grab version dynamically because of webkit

/*
The changelog is now on the main script page
http://userscripts.org/scripts/show/75837
*/
...
accTextRegex : /(you opened the mystery gift box|you saved|hurray|found|congratulations|you received|you helped|thank you)/,
...
case "humpback": w="whalehumpback"; break;
}
...
removeDone : function() {
//if(main.currReqs > 0) return;
var done = $g(".//a[contains(@href,'"+main.gameURLpart+"') or starts-with(@id,'item_done_') or starts-with(@id,'item_failed_') or starts-with(@id,'item_skip_') or contains(.,'"+main.xpQueryTextExcludes.join("') or contains(.,'")+"'))]/ancestor::li[starts-with(@id,'stream_story_') and not(contains(@id, '_collapsed'))]", {node:main.stream});
for(var i=0,len=done.snapshotLength; i<len; i++) if(!$g(".//*[contains(@id,'_story_') and contains(@id,'_collapsed')]", {type:9, node:done.snapshotItem(i)})) main.remove(done.snapshotItem(i).id);
},
...
expand : function() {
var posts=$g("count(.//*[(starts-with(@id,'stream_story_') or starts-with(@id,'div_story_') or starts-with(@id,'li_story_')) and contains(@class,'"+main.gameID+"')])", {node:main.stream, type:1}),
...
if(text) text=$g(".//div[@class='zooDialog_subtitleAndBody']/h1", {node:text,doc:doc,type:9}) || text || doc.body;
...
var btn = doc.getElementById('zooButton').src = "http://cdn.rockyou.com/apps/zoo/dialogs/btn*.png";
if(gotItem==true && btn) {
...
switch(main.opts["autolike"]==true && (loadgame==true || failedItem==true || gotItem==true)) {case true: main.like(key); break;}
...
// remove the iframe
switch(loadgame==true || failedItem==true || (failedItem==false && gotItem==false && loadgame==false) || doc.URL.find("/zoo/home.php?") || doc.URL.find("/zoo/visitZoo.php?") || doc.URL.find("/zoo/giftAnimals.php?") || doc.URL.find("/zoo/race.php?")) {case true: main.remove(key); break;}
},
...
window.setTimeout(main.remove, main.reqTO, key);
},
...
switch(main.opts["colorcode"]==true) {
case true:
var div = $g("//a[@id='"+item.getAttribute("id")+"']/ancestor::*[starts-with(@id,'stream_story_') and not(contains(@id,'_collapsed'))]", {type:9});
...
case false: if(failed[w].inArray(key)) {
	...
	case "find" : if(opts["find"]==true && (opts[wA]==true)) open(item.href, key, w);
					else item.setAttribute("id", "item_skip_"+key); break;
	case "heal" : if(opts["heal"]==true && (opts[wA]==true)) open(item.href, key, w);
					else item.setAttribute("id", "item_skip_"+key); break;
...
GM_config.init("<img src='"+imgs.logo+"'> v"+version, {
	adopt : {
	    ...
	    kids : {
		    ...
		    help : {
		        label : "Help the beached Humpback?",
				type: "checkbox",
				"default" : false
		    },
		    feedhelp : {
		        label : "Help Feed Animals?",
				type: "checkbox",
				"default" : false
		    },
		    claim : {
		        label : "Claim the Mystery Gift?",
				type: "checkbox",
				"default" : false
		    },
		    claimbonus : {
    ...
    collect : {
	    label : "Collect Speed Stone?",
		type: "checkbox",
		"default" : false,
		kids : {
		    topiarybabyanimal : {
			label : "Get Baby Animal Topiary?",
			type: "checkbox",
			"default" : false
			},
			materialsgreenhouse : {
			label : "Get greenhouse materials?",
			type: "checkbox",
			"default" : false
			},
			lend : {
	...
	"default" : false,
	    kids : {
		    highlandsheepscottish : {
		    label : "Scottish Highland Sheep?",
		    type: "checkbox",
		    "default" : false
		    },
		    sloth : {
	...
	maxposts : {
        ...
        removedone : {
	label : "Hide done/failed/skipped items from feed?",
	type : "checkbox",
	"default" : false,
...
main.create("a", {className:"item", href:"javascript:void(0);", onclick:main.config}, new Array(
	main.create("span", {className:"imgWrap"}, new Array(main.create("img", {src:imgs.icon}))),
	main.create("span", {textContent:main.gameAcronym+" "+version+" Options"})))));
if(f && !$("navItem_test_1")) f.parentNode.appendChild(link1);
if(a && !$("navItem_test_2")) a.appendChild(link2);
