...
/* TO DO 
 - Eagle Nest tab => Battlegrounds + implants + My + strongholds + implants self-collection
 - Send Training Troops to Catacombs after trained
 - Info tab: Sort by training groups
 - Info tab: Button for sending troops to the catacombs / sanctuary
 - display scrolls
 - Tab jobs: Construction, Training, Lost Arts, Skills
*/
(function() {
var scriptVersion	= '2013.0723';
var scriptId		= '162539';
var client_ver		= '403';
...
if ( !window ||
	 !( (/rulesofwargame\.com\/stormfall\.fb/.test(window.location.href)) ||
		(/apps\.facebook\.com\/stormfall/.test(window.location.href)))
	) return;
...
/* Error messages */
var kFatalSeedTitle	= 'ERROR IN THE DATA RECOVERY SERVER';
var kFatalSeedMsg	= 'Thank you disable the script and see if you can play manually. If c\'possible, then reactivate the script and try again. If the\'error persists, thank you contact the\'author. If you can not wait then manually play, the game is probably being updated.';
var kFatalSWF	= '"<B>Error \'initialization :</b> <BR><BR>Could not find\'élément SWF"';
...
var unit_groups = [ {id:0, desc:'infantry'},
...
//					if (swf.id == 'flashContent') swf.style.width = maxWidth+'px';
	...
	try {
		...
		progressBar.start({ steps:8, delay:startupDelay, title:'Initializing ...', stepText:'Loading data' });
		...
		function stepStarting (current_step) {
			...
			if (retry <= 10){
				switch ( current_step ) {
					case 1: /* FB data */
						progress_title = 'Retrieving FB info ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'FB player data ...' });
						Facebook.init(function (res) {
							if (res.ok) {
								onSuccess ('Facebook recovered data', wait_time, current_step + 1);
							} else {
						...
						break;
					case 2: /* FB friends */
						progress_title = 'Retrieving list of FB friends ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'List FB friends ...' });
						Facebook.initFriends(function (res) {
							if (res.ok) {
								onSuccess ('Friends List FB recovered', wait_time, current_step + 1);
							} else {
								onError (res.status, res.errmsg, 'FB friends', wait_time, current_step);
							}
						});
						break;
					case 3: /* SignIn */
						progress_title = 'Retrieving player data ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'Server : '+C.attrs.defaultServer+', segment '+C.attrs.defaultSegment });
						Seed.init(function (res) {
							if (res.ok) {
								onSuccess ('Player initialized data', wait_time, (redo_SignIn ? current_step : current_step + 1));
							} else {
					...
					case 4: /* Client static data */
						progress_title = 'Retrieving static data ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'Static customer data' });
						Manifest.init(function (res) {
							if (res.ok) {
								var next = 1;
								if (!Seed.gameData.alliance || Seed.gameData.alliance.id <= 0) next++;
								onSuccess ('Static initialized data', wait_time, (redo_SignIn ? current_step : current_step + next));
							} else {
								onError (res.status, res.errmsg, 'Manifest', wait_time, current_step);
							}
						});
						break;
					case 5: /* Getting alliance data */
						progress_title = 'Data recovery alliance ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'Alliance data ...' });
						Seed.initAlliance(function (res) {
							if (res.ok) {
								onSuccess ('Alliance initialized data', wait_time, current_step + 1);
							} else {
					...
					case 6: /* Getting map data */
						progress_title = 'Retrieving data map ...';
						progressBar.update ({ step:current_step, title:progress_title, stepText:'Retrieving data map ...' });
						Map.init(function (res) {
							if (res.ok) {
								onSuccess ('map initialized data', wait_time, current_step + 1);
							} else {
								onError (res.status, res.errmsg, 'map', wait_time, current_step);
							}
						});
						break;
		...
		actionLog('<B>' + scriptVersion + ' Loading ...</B>');
		consoleLog('<B>' + scriptVersion + ' Loading ...</B>');
		stepStarting( 1 );

		function startScript() {
			if (updaterPop)	setTimeout (function(){updaterPop.destroy()}, 100);
			progressBar.update ({ step:8, title:'Finalization...', stepText:'Initializing tabs ...' });
...
//			Data.setDefaultValues ('options');
			...
			actionLog('<B>' + scriptVersion + ' loaded</B>');
			consoleLog('<B>' + scriptVersion + ' loaded</B>');
...
var Cyborgs = {
	create : function (t, user_id, notify){
		...
		if (!found || !user) {
			...
			dial.getContentDiv().innerHTML = 'Can not create spectrum ...\nWhether missing data in this player list d\'amis';
		...
		dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
		dial.getContentDiv().innerHTML = 'Création d\'un spectre pour '+user.fullname+'...';
		Cyborgs.createCmd (user_id, callback);
		function callback (res){
			if (res.done){
				actionLog('Spectre pour <B>'+user.fullname+'</B> créé');
				dial.getContentDiv().innerHTML = 'Spectre pour <B>'+user.fullname+'</B> créé';
				setTimeout (function(){dial.destroy(); if (notify) notify(true);}, 2000);
				return;
			} else {
				dial.getContentDiv().innerHTML = '<B>Oops, an error s\'is produced during creation of the spectrum.</B>\n'+(res.errmsg || '');
	...
	createCmd : function (user_id, callback){
		...
		new MyAjaxRequest (C.attrs.defaultServer, C.attrs.defaultSegment, 'CreateCyborg', JSON.stringify(data), function (rslt) {
			if (rslt.ok && rslt.dat) {
				...
				verboseLog('Cyborgs.create Command failed with l\' error : ' + rslt.errmsg);
	...
	createForUserCmd : function (user_id, callback){
		...
		new MyAjaxRequest (C.attrs.defaultServer, C.attrs.defaultSegment, 'CreateCyborgForUser', JSON.stringify(data), function (rslt) {
			if (rslt.ok && rslt.dat) {
				...
				verboseLog('For Cyborgs.created User Command failed with l\' error : ' + rslt.errmsg);
				if (callback) callback({done:false, errmsg:rslt.errmsg});
				return;
			}
		}, true);
...
var Data = {
		...
		try {
			...
			for ( var item_name in obj ) {
				...
				if (stored_object != null && typeof (stored_object) == 'object') {
					verboseLog ('Cleaning deleted variables : [ ' + name + ' ]');
	...
	setObject : function (key, value) {
		...
		try {
			...
			if ( e === QUOTA_EXCEEDED_ERR || (e.code === 22 && e.name === 'QUOTA_EXCEEDED_ERR') )	{
				verboseLog('Error saving in the local storage , '+name+'='+value+', error='+inspectObj(e,8,1));
				logit ('LocalStorage : Quota exceeded! Thank you clear the cache and continuing browser data');
			}
		}
	},
	onUnload : function () {
	...
	save : function () {
		verboseLog('Saving data in the local Storage');
...
var Facebook = {
	...
	init : function (callback) {
		...
		t.signInFB (function (rslt) {
			if (rslt.ok)
				 verboseLog('Request of Facebook data sent successfully');
			else verboseLog('In Facebook.sign returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
	...
	initFriends : function (callback) {
		...
		t.getFBfriends (function (rslt) {
			if (rslt.ok)
				 verboseLog('Request the list of Facebook friends sent successfully');
			else verboseLog('Facebook.get friends has returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
			if (callback) callback(rslt);
		});
	},
...
var Manifest = {
	...
	init : function (callback) {
		...
		t.getStaticData (function (rslt) {
			if (rslt.ok)
				 verboseLog('Application of static data sent successfully');
			else verboseLog('Manifest.get Static Data returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
			if (callback) callback(rslt);
		});
	},
		...
		new MyAjaxRequest (Seed.defaultServer, Seed.defaultSegment, 'Client.GetStaticData', JSON.stringify(Seed.player.s), function (rslt) {
			if (rslt.ok) {
				...
				try {
					...
					rslt.ok = false;
					rslt.errmsg = 'Manifest.updated Manifest returned l\'erreur ' + e.toString();
				}
				if (rslt.ok) {
					try {
						...
						rslt.ok = false;
						rslt.errmsg = 'Seed.updated Resources returned l\'error ' + e.toString();
					}
				}
			}
			if ( callback )	callback (rslt);
	...
	updateManifest : function () {
		...
		for (var trp in Seed.reqs) {
			if (Seed.reqs[trp].stats && Seed.reqs[trp].stats.type == 'troop'){
				...
				/* 1-16 : normal | 17-32 : impérial | 33 : spectre | 34-36 : immortels (grp 8) | 37-40 : nains | 41-44 : nains impériaux */
				var offset = 0;
				if (this_grp%2!=0){
					if (trp<32) offset = 16;
...
var Map = {
	...
	init : function (callback) {
		...
		t.fetchMap (function (rslt) {
			if (rslt.ok)
				 verboseLog('Query data card sent successfully to the server');
			else verboseLog('Map.fetch Map returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
			if (callback) callback(rslt);
			...
			if (rslt.ok) {
				if (rslt.dat.n) {
					try {
						...
						rslt.ok = false;
						rslt.errmsg = 'Seed.fetchMap.user Notes returned l\'error ' + e.toString();
					}
				}
...
//			t.callback ({done:false});
				...
				VisitUsers.visitCmd (user_id.toString(), segment_id, function (res){
					...
					if (res.done){
						...
						if (t.percent != t.old_percent) {
							progressBar.update ({ step:t.percent, stepText:'Please wait ...<br>Steps '+(t.step+1)+'/'+t.steps+' : player '+t.users_count+'/'+t.total_users });
							t.old_percent = t.percent;
						}
					} else {
						var msg = 'Error visit <b>'+user.fullname+'</B><br>'+(res.errmsg || '');
		...
		if (user) {
			...
			VisitUsers.visitCmd (user_id.toString(), segment_id, function (res){
				if (res.done){
					...
					verboseLog('VisitUsers.visit Command failed with l\' error : ' + rslt.errmsg);
					if (callback) callback({done:false, errmsg:rslt.errmsg});
					return;
				}
			});
	...
	checkOneUser : function (t, user_id, notify){
		...
		if (!user) {
			var dial = new ModalDialog (t.container, 300, 150, '', false);
			dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
			dial.getContentDiv().innerHTML = 'Player can not refresh ...\nMissing player';
			setTimeout (function(){dial.destroy(); if (notify) notify (true);}, 2000);
		}
		var dial = new ModalDialog (t.container, 300, 165, '', false, null);
		dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
		dial.getContentDiv().innerHTML = 'Missing player... '+user.fullname+'...';
		Map.getOneDetail (user_id, callback);
		function callback (res){
			if (res.done){
				actionLog('data <B>'+user.fullname+'</B> refreshed');
				dial.getContentDiv().innerHTML = 'data <B>'+user.fullname+'</B> refreshed';
				setTimeout (function(){dial.destroy(); if (notify) notify(true);}, 2000);
			} else {
				dial.getContentDiv().innerHTML = '<B>Oopps, an error s\'occured.</B>\n'+(res.errmsg || '');
				dial.allowClose (true);
				if (notify) notify(false);
				return;
			}
...
var Seed = {
	...
	init : function (callback) {
		...
		t.signIn (function (rslt) {
			if (rslt.ok)
				 verboseLog('Request data player successfully sent to the server');
			else verboseLog('Seed.signIn Returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
			if (callback) callback(rslt);
		...
		new MyAjaxRequest (C.attrs.defaultServer, C.attrs.defaultSegment, 'SignIn', options.params, function (rslt) {
			if (rslt.ok) {
				...
				if (rslt.dat.r){
					...
					redo_SignIn = true;
					verboseLog('Redirect server '+C.attrs.defaultServer+', Segment '+C.attrs.defaultSegment);
				} else {
					redo_SignIn = false;
					...
					try {
						...
						rslt.ok = false;
						rslt.errmsg = 'Seed.gameData Returned l\'error ' + e.toString();
					}
					try {
						...
						rslt.ok = false;
						rslt.errmsg = 'Seed.gameData.userNotes Returned l\'error ' + e.toString();
					}
					try {
						Seed.gameData.locationNotes = cloneProps(jsonDto.getLocationNotes(Seed.player.l));
					} catch (e) {
						rslt.ok = false;
						rslt.errmsg = 'Seed.gameData.locationNotes Returned l\'error ' + e.toString();
					}
					try {
						Seed.gameData.alliances = cloneProps(jsonDto.getAllianceNotes(Seed.player.y));
					} catch (e) {
						rslt.ok = false;
						rslt.errmsg = 'Seed.gameData.alliances Returned l\'error ' + e.toString();
					}
				}
			}
			if ( callback )	callback (rslt);
	...
	initAlliance : function (callback) {
		...
		t.getAlliance (function (rslt) {
			if (rslt.ok)
				 verboseLog('Request data alliance successfully sent to the server');
			else verboseLog('Seed.getAlliance Returned l\'error ' + rslt.ok + ' - ' + rslt.errmsg);
			if (callback) callback(rslt);
...
var Treasures = {
	collectAll : function (t, user_id, notify){
		...
		if (boxes <= 0) {
			var dial = new ModalDialog (t.container, 300, 150, '', false);
			dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
			dial.getContentDiv().innerHTML = 'No tribute to collect';
		...
		var dial = new ModalDialog (t.container, 300, 165, '', false, null);
		dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
		dial.getContentDiv().innerHTML = 'Collection of tributes ...';
		Treasures.getTreasures (user_id, callback);
		function callback (res){
			if (res.done){
				actionLog('<B>Collection of tributes</B>: completed');
				dial.getContentDiv().innerHTML = '<B>Collection of tributes</B>: completed ('+res.count+' collected)';
				setTimeout (function(){dial.destroy(); }, 2000);
				if (notify) notify(true);
				return;
			} else {
				dial.getContentDiv().innerHTML = '<B>Oopps, error collecting tributes.</B>';
				dial.allowClose (true);
				if (notify) notify(false);
				return;
			}
...
var VisitUsers = {
	...
	thisUser : function (t, user_id, notify){
		...
		if (!found || !user) {
			var dial = new ModalDialog (t.container, 300, 150, '', false);
			dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
			dial.getContentDiv().innerHTML = 'Visit impossible...\nWhether or not missing data in this player list of\'friends';
			setTimeout (function(){dial.destroy(); if (notify) notify (true);}, 2000);
		}
		for (var r in production_icons)
		...
		logit('======> Visit '+user.fullname+ (collect ? ' and collection of tributes' : ''));
		var dial = new ModalDialog (t.container, 300, 165, '', false, null);
		dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
		dial.getContentDiv().innerHTML = 'Visit the Castle'+ user.fullname + (collect ? '\net collection of tributes' : '') +'...';
	...
	allUsers : function (div, user_ids, notify){
		...
		progressBar.start({ steps:t.steps, delay:1, byCount:true, title:'Visits to selected friends ...', stepText:'Please wait ...' });
	...
	visitNext : function (){
		...
		t.step++;
		t.percent = toNum(t.step * 100 / t.steps);
		var steptext = 'Visit the Castle <b>'+ user.fullname +'</b>'+ (collect ? '\net collection of tributes' : '') +'...';
	...
	startVisit : function (user_id, to_collect, callback){
		...
		var msg = 'Visit the castle <b>'+ user.fullname +'</b> : ';
		function deal_step (current_step) {
			switch ( current_step ) {
				case 1:
					var segment_id = ((user.segment_id == undefined || user.segment_id == null) ? Seed.dfltSegmentId : user.segment_id);
					VisitUsers.visitCmd (user_id.toString(), segment_id, function (res){
						if (res.done){
							...
							if (callback) callback({done:false,
													msg:'Error visiting <b>'+user.fullname+'</B><br>'+(res.errmsg || ''),
					...
					if (VisitUsers.current.gameData && VisitUsers.current.gameData.city && VisitUsers.current.gameData.city.buildings) {
						...
						if (labo_lvl > 0 && !already_created) {	//&& !is_own_cyborg 
							Cyborgs.createForUserCmd (user_id, function (cyb_res){
								if (cyb_res.done) {
									cyborg_created = true;
									msg += '<b>spectrum created</b>. ';
								} else 
									msg += 'Error creating the spectrum.<br>'+(cyb_res.errmsg || '');
								deal_step(current_step + 1);
							});
						} else {
							var sep = '';
							msg += 'Spectrum not created (';
							if (labo_lvl <= 0) { msg += 'no crypt'; sep = ', '; }
							if (is_own_cyborg) { msg += sep+'is already in my spectra'; sep = ', '; }
							if (already_created) { msg += sep+'already created\'Today'; sep = ', '; }
							msg += ') ';
							deal_step(current_step + 1);
						}
					} else deal_step(current_step + 1);
				...
				case 3:
					if (to_collect) {
						Treasures.getTreasures (user_id, function (treas_res){
							if (treas_res.done)
								msg += '<b>'+treas_res.count+'</b> tributes collected';
							else msg += 'Error collecting tributes.<br>'+(treas_res.errmsg || '');
							deal_step(current_step + 1);
						});
					} else  deal_step(current_step + 1);
					break;
		...
		new MyAjaxRequest (Seed.segments[segment_id].server, Seed.segments[segment_id].segment, 'VisitUser2', this_id, function (rslt) {
			if (rslt.ok && rslt.dat) {
				...
				try {
					...
					verboseLog('VisitUsers.visitCmd.gameData returned l\' error : ' + rslt.errmsg + ' generated ' + jsonDto.step);
					if (callback) callback({done:false, errmsg:rslt.errmsg});
					return;
				}
				if (callback) callback({done:true});
				...
				verboseLog('VisitUsers.visitCmd failed with l\' error : ' + rslt.errmsg);
				if (callback) callback({done:false, errmsg:rslt.errmsg});
				return;
			}
		}, true);
	...
	LoadBinary : function (url, index, degraded) {
		...
		function mycb(rslt) {
			if (rslt.ok) {
				...
				if ((/URL_/.test(type)) && Data.options.sound[type] != url) { Data.options.sound[type] = url; logit ('New url for Data.options.sound.'+type+' = '+url); }
			} else {
...
Tabs.Info = {
	...
	init : function (div){
		...
		+'	<tr align=center><td width="100px"><a id=tabInfo_ScriptUpdate style="color:#FFFFFF;text-decoration:none;">Update Script</a></td>'
		...
		+'</tr></table></div>'
		+'<table width=100%><tr>'
		+'	<td width=25%><input id=tabInfo_Refresh type=button value="Refresh Data"></input></td>'
		+'	<td width=25% align=center><input id=tabInfo_Toggle type=button value="Refresh Page"></input></td>'
		+'	<td width=25% align=center>&nbsp;</td>'
		+'	<td width=25% align=center><input id=tabInfo_Collect type=button value="Collect tributes"></input></td>'
		+'</tr></table>'
		+'<ul class=tabs>'
		+'	<li class="tab first"><a id=tabInfoOverview>Overview</a></li>'
		+'	<li class="tab"><a id=tabInfoMarches>Marches</a></li>'
		+'</ul>'
	...
	tabInfoOverview : function (){
		...
		if (my_alli.id > 0) {
			my_alli_text = '<font color=yellow>' + my_alli.name + '</font><br>'
						  +'<font color=white>Position '+my_alli.rating_pos+', '+my_alli.membership.members_count+' members of '+my_alli.membership.members_limit+'</font>';
		} else my_alli_text = '&nbsp;';
		m += '<div class=bsx_subtitle><table class=bsx_table>'
			+'<tr valign=top>'
			+'	<td align=center width=39px>'+ vignette(Facebook.SND.id) +'</td>'
			+'	<td align=left width=35%><font color=white>'+ Facebook.SND.name + '<br>level '+ player.level + my_xp_text +'</font></td>'
		...
		m += '<table width=100%>'
			+'<tr valign=middle>'
			+'	<td align=center width=40px title="Attack Points">'+ vignette2('ui/icons/legend_attack_stats.png',25) +'</td>'
			+'	<td align=left width=70px title="Attack Points"><div id=attacker_rate></div></td>'
			+'	<td align=center width=40px title="Defense Points">'+ vignette2('ui/icons/legend_defence_stats.png',25) +'</td>'
			+'	<td align=left width=70px title="Defense Points"><div id=defender_rate></div></td>'
			+'	<td align=center width=40px title="Raid Points">'+ vignette2('ui/icons/legend_robbery_stats.png',25) +'</td>'
			+'	<td align=left width=70px title="Raid Points"><div id=robber_rate></div></td>'
			+'	<td align=center width=40px title="Siege Points">'+ vignette2('ui/icons/legend_capture_stats.png',25) +'</td>'
			+'	<td align=left width=70px title="Siege Points"><div id=raids_rate></div></td>'
			+'	<td align=center width=40px title="Saphires">'+ vignette2('ui/quests/icons/saphires.png',25) +'</td>'
			+'	<td align=left width=70px title="Saphires"><div id=saphir_stck></div></td>'
			+'</tr></table>'
			+'<table width=100%>'
			+'<tr valign=middle>';
		for (var r in production_icons) {
			m += '	<td align=center width=38px title="Tributes '+production_icons[r].desc+'">'+ vignette2(production_icons[r].treasure,25) +'</td>'
				+'	<td align=left width=50px title="Tributes '+production_icons[r].desc+' + those with friends"><div id='+r+'_treas></div></td>';
		}
		m += '	<td align=center width=38px title="Runes skull">'+ vignette_swf('ui/skillTree/iconNanopodBig.swf',25) +'</td>'
			+'	<td align=left width=50px title="Runes skull"><div id=runes_stck></div></td>'
			+'	<td align=center width=40px title="Raids available">'+ vignette2('ui/skin_parts/buttons/buttonsMapMenu/global_robbery.png',25) +'</td>' //occupation.png
			+'	<td align=left width=30% title="Raids available"><div id=raids_avail></div></td>'
			+'</tr></table>'
			+'<table style="margin-top:3px" width=100% class=row_style>'
			+'	<tr class=row_headers align=center>'
			+'		<td width=35px>Type</td>'
			+'		<td width=17% colspan=2>per Hour</td>'
			+'		<td width=17% colspan=2>Production</td>'
			+'		<td width=17% colspan=2>consumption</td>'
			+'		<td width=17% colspan=2>Stocks</td>'
			+'		<td width=32% colspan=2>capacity</td>'
			+'	</tr>';
		for (var r in production_icons) {
		...
		m += '</table>'
			...
			+'		<td width=17% align=left><b>Lost Arts :</b></td>'
			...
			+'		<td width=17% align=left><b>Skills :</b></td>'
		...
		if (catacombes_found) {
			m += '<table style="margin-top:3px" width=100%>'
				...
				+'		<td width=13% align=left><input type=submit class="btn_red small" value="Send March" id=tabInfo_SendCatacombes /></td>'
				+'		<td width=70%>&nbsp;</td>'
				+'	</tr>'
				+'</table>'
		}
		m += '<table style="margin-top:3px" width=100% class=row_style>'
			...
			+'		<td width=34px>Unit</td>'
			+'		<td width=11% title="In the castle (red) / In the catacombs"><span class=bold_red>In castle</span><br>hidden</td>'
			+'		<td width=11%>Besieged</td>'
			+'		<td width=11%>Marching</td>'
...
*/
		...
		function refreshView () {
			...
			for (var r in production_icons) {
				...
				if (conso != 0) {
					var txt = 'Units : '+ numf(toNum(Seed.gameData.account.troop_consumption[r]),' ') +' /h\n'
			...
			var tj = '<table style="margin-top:3px" width=100%>', title = 'Training :';
			if (jobs && jobs.length > 0) {
				...
				tj += '	<tr valign=middle>'
					...
					+'		<td align=center width=240px><span class=bold_red>NONE</span></td>'
			...
			function dispJob (type) {
				...
				if (job && job.end_at > serverTimeMs()) {
					...
					setTitle(type+'_in_progress', 'NONE');
					setText(type+'_img', '&nbsp;' );
					setText(type+'_progress', '<span class=bold_red>NONE</span>');
					setText(type+'_time', '&nbsp;');
				}
			}
	...
	tabInfoMarches : function (){
		...
		var m = '<div id=tabMarches_divtable>'
			+'	<div class=bsx_title>Overview</div>'
			+'	<table class=bsx_table style="margin-top:3px" width=100%>'
			+'		<tr valign=top align=left>'
			+'			<td width=25%><input id=tabMarches_CB_myraids type=checkbox '+ (Data.options.info.myraids?'CHECKED ':'') +' /> My raids</td>'
			+'			<td width=25%><input id=tabMarches_CB_mysieges type=checkbox '+ (Data.options.info.mysieges?'CHECKED ':'') +' /> My sieges</td>'
			+'			<td width=25%><input id=tabMarches_CB_myspies type=checkbox '+ (Data.options.info.myspies?'CHECKED ':'') +' /> My Spies</td>'
			+'			<td width=25%><input id=tabMarches_CB_myreinf type=checkbox '+ (Data.options.info.myreinf?'CHECKED ':'') +' /> My reinfs</td>'
			+'		</tr><tr valign=top align=left>'
			+'			<td width=25%><input id=tabMarches_CB_otherraids type=checkbox '+ (Data.options.info.otherraids?'CHECKED ':'') +' /> Raids on enemy</td>'
			+'			<td width=25%><input id=tabMarches_CB_othersieges type=checkbox '+ (Data.options.info.othersieges?'CHECKED ':'') +' /> Sieges on enemy</td>'
			+'			<td width=25%><input id=tabMarches_CB_otherreinf type=checkbox '+ (Data.options.info.otherreinf?'CHECKED ':'') +' /> Reinfs to allies</td>'
			+'			<td width=25%><input id=tabMarches_CB_garnisons type=checkbox '+ (Data.options.info.garnisons?'CHECKED ':'') +' /> garrisons</td>'
			+'		<tr valign=top align=left>'
			+'			<td width=25%><input id=tabMarches_CB_mycaravans type=checkbox '+ (Data.options.info.mycaravans?'CHECKED ':'') +' /> My caravans</td>'
			+'			<td width=50% colspan=2><input id=tabMarches_CB_othercaravans type=checkbox '+ (Data.options.info.othercaravans?'CHECKED ':'') +' /> Caravans approaching</td>'
			+'		</tr>'
			+'	</table><br>'
		...
		function display_marches () {
			...
			for (var id in t.marches) {
				...
				m += '<tr valign=middle ref='+id+' style="border-top: 1px solid #898989; background-color:'+ row_style +'" title="'+ order_desc[march.type]+march.name +'">'
					...
					+(cancellable ? '<input type=submit class="btn_red small" title="Cancel the march" value="&nbsp;X&nbsp;" style="width:20px;" id="march_cancel_'+id+'" ref="'+id+'" />' : '')
					+(recallable ? '<input type=submit class="btn_red small" title="Recall units" value="&nbsp;&lt&lt&nbsp;" style="width:20px;" id="march_recall_'+id+'" ref="'+id+'" />' : '')+'</td>'
					+'</tr>';
		...
		function refresh_marches () {
			...
			for (var id in t.marches) {
				...
				if (!isNaN(time_left) && time_left > 0 && !march.cancelled) {
					...
					var status = '', type1 = '', type2 = '', status2 = ' (pending update)';
					switch (march.target_type) {
						case 'raid'		 : type1 = ' The battlefield'; break;
						case 'beacon'	 : type1 = ' The Beacon'; type2 = 'your beacon '; break;
						case 'location'	 : type1 = ' The\'Location'; type2 = 'your location '; break;
						default : type1 = ' The castle'; type2 = 'Your Castle '; break;
					}
					if (march.own_march) {
						switch (march.type) {
							case order_ids.Intelligence	 : status = 'Your spies enter'; break;
							case order_ids.Robbery		 : status = 'Your forces enter'; break;
							case order_ids.Occupation	 : status = 'Your forces occupy'; status2 = ''; break;
							default : status = 'DEFEND Thy forces'; status2 = ''; break;
						}
						status += type1+'...'+status2;
						document.getElementById('march_progress_'+id).parentNode.style.textAlign = 'right';
					} else {
						switch (march.type) {
							case order_ids.Robbery		 : status = 'undergoes a raid'; break;
							case order_ids.Occupation	 : status = 'is besieged'; status2 = ''; break;
							default : status = 'defended'; status2 = ''; break;
						}
						status = type2+status+'...'+status2;
						document.getElementById('march_progress_'+id).parentNode.style.textAlign = 'left';
					}
					if (march.own_march && march.type == order_ids.Bunker && march.owner_id == march.target_id) status = 'In the catacombs';
					if (march.cancelled) status = 'March canceled';
 ...
 // logit('Seed.gameData.world_data.units = '+JSON.stringify(Seed.gameData.world_data.units));
		...
		function onClickCancel (event){
			...
			var dial = new ModalDialog (t.container, 300, 165, '', false, null);
			dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
			dial.getContentDiv().innerHTML = 'Cancellation of the march ...';
		...
		function cancelMarch (march_id, callback) {
			...
			new MyAjaxRequest (C.attrs.defaultServer, C.attrs.defaultSegment, 'CancelUnit', JSON.stringify(data), function (rslt) {
				if (rslt.ok && rslt.dat) {
					...
					if (callback) callback({done:true, msg:'March canceled'});
				} else {
					if (callback) callback({done:false, msg:'Error of the\'cancellation<br>'+(rslt.errmsg || '')});
				}
				return;
			}, true);
		}
...
Tabs.Contacts = {
	...
	init : function (div){
		...
		var m = '<div class=bsx_title>Contacts</div>'
			+'<ul class=tabs>'
			+'	<li class="tab first"><a id=tabContactsList>Crypt</a></li>'
			+'	<li class="tab"><a id=tabTargetList>Targets</a></li>'
			+'</ul>'
	...
	tabContactsList : function (){
		...
		var m = '<div id=tabContacts_Results style="height:640px">'
			+'	<div class=bsx_title>List of contacts</div>'
			+'	<table class=bsx_table style="margin-top:3px" width=100%>'
			+'		<tr valign=top align=left>'
			+'			<td width=16% align=left><label>My Wraiths : </label></td>'
			+'			<td width=10% align=left><span id=tabContacts_myCyborgs></span></td>'
			+'			<td width=17% align=left><label>souls available : </label></td>'
			+'			<td width=10% align=left><span id=tabContacts_daily></span></td>'
			+'			<td width=19% align=left><label>Wraith with friends : </label></td>'
		...
		m += '</tr></table></td>'
			...
			+'		<tr valign=top align=left>'
			+'			<td width=28% colspan=2><input id=tabContacts_CB_View type=checkbox '+ (Data.options.contacts.detailled_view?'CHECKED ':'') +' /> Detailed view</td>'
			+'			<td colspan=4><input id=tabContacts_CB_Done type=checkbox '+ (Data.options.contacts.hide_created?'CHECKED ':'') +' /> Hide wraiths created</td>'
			+'			<td width=20%><input id=tabContacts_CB_Treas type=checkbox '+ (Data.options.contacts.collect_treas?'CHECKED ':'') +' /> Collect tribute</td>'
			+'		</tr><tr valign=top align=left>'
			+'			<td width=1%><input type=radio name="tabContacts_Filter" value="0" /></td>'
			+'			<td width=19% align=left><label>All my friends</label></td>'
			+'			<td width=1%><input type=radio name="tabContacts_Filter" value="1" /></td>'
			+'			<td width=19% align=left><label>Friends of my league</label></td>'
			+'			<td width=1%><input type=radio name="tabContacts_Filter" value="2" /></td>'
			+'			<td width=19% align=left><label>Friends outside league</label></td>'
		...
		function display_detailled (){
			...
			+'		<td id=tabContacts_tal_0 width="100px" style="overflow-x:auto"><A><span class=bold_white>Name</span></A></td>'
			+'		<td id=tabContacts_tal_1 width="50px"><A><span class=bold_white>Level</span></A></td>'
			...
			for (var f=0; f<t.contact_list.length; f++) {
				...
				m += '	<tr valign=top>'
					...
					+'		<td align=center width=15%><input type=submit class="btn_green small" value="summon" style="width:auto;" id="tabContacts_Invo_'+f+'" ref="'+t.contact_list[f].id+'" /></td>'
					+'		<td align=center width=15%><input type=submit class="btn_green small" value="Visit" style="width:auto;" id="tabContacts_Visi_'+f+'" ref="'+t.contact_list[f].id+'" /></td>'
					+'	</tr>';
			}
			m += '</table>';
			return m;
	...
	tabTargetList : function (){
		...
		var m = '<div id=tabTargets_Results style="height:640px">'
			...
			+'			<td width=19% align=left><label>All</label></td>'
			+'			<td width=1%><input type=radio name="tabTargets_Filter" value="1" /></td>'
			+'			<td width=19% align=left><label>except friends</label></td>'
			...
			+'	<table class=bsx_table width=100%>'
			+'		<tr valign=top align=left>'
			+'			<td width=30%><label>min level : </label>' + levelSelect('Min', nvl(Data.options.contacts.min_level,9) ) + '</td>'
			+'			<td width=30%><label>max level : </label>' + levelSelect('Max', nvl(Data.options.contacts.max_level,100) ) + '</td>'
			+'			<td width=40%><label>Max Distance : </label><select id=tabTargets_MaxRadius>';
		...
		m += '			</select></td>'
			...
			+'				<td width=40%><label><input id=tabTargets_Refresh type=button value="Refresh selection" /></label></td>'
			+'		</tr>'
			+'	</table><br>'
		...
		function display_detailled (){
			...
			+'	<tr class=row_headers>'
			+'		<td width="40px">Avatar</td>'
			+'		<td id=tabTargets_tal_0 width="150px" style="overflow-x:auto"><A><span class=bold_white>Name</span></A></td>'
			+'		<td id=tabTargets_tal_1 width="45px"><A><span class=bold_white>Level</span></A></td>'
			...
			for (var f=0; f<t.target_list.length; f++) {
				...
				m += '			</tr></table>'
					...
					+'		<td align=right colspan=2 title="Last raid"><div id='+t.target_list[f].id+'_lastraid></td>'
					+'	</tr>';
			}
			m += '</table>';
			return m;
		...
		function onClickAction(event){
			...
			if (id[0] && id[1]) {
				...
				if (tot <= 0) {
					var dial = new ModalDialog (t.container, 300, 165, '', false, null);
					dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
					dial.getContentDiv().innerHTML = 'No units available';
					setTimeout (function(){dial.destroy(); }, 2000);
					...
					var dial = new ModalDialog (t.container, 300, 165, '', false, null);
					dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
					dial.getContentDiv().innerHTML = 'No raid available'+next;
					setTimeout (function(){dial.destroy(); }, 2000);
		...
		function scanMap (notify){
			...
			progressBar.start({ steps:100, delay:1, byCount:true, title:'Refreshing the selection ...', stepText:'Please wait ...' });
			Map.scanMap ({x:x, y:y, radius:radius, min:min, max:max}, callback);
			function callback (res){
				if (res.done){
					actionLog('<B>Scan the map</B>: Finished');
					var now = serverTimeMs();
					//Data.options.contacts.last_update = new Date(now).myString();
					if (notify) notify (true);
					progressBarPop.destroy();
				} else {
					progressBarPop.getContentDiv().innerHTML = '<B>Oops, error while scanning the map ...</B>';
...
Tabs.Radar = {
	...
	init : function (div){
		var t = Tabs.Radar;
		t.container = div;
		var m = '<div class=bsx_title>Nest\'eagle</div>'
			+'<ul class=tabs>'
			+'	<li class="tab first"><a id=tabRadarBattle>Battlefield List</a></li>'
			+'	<li class="tab"><a id=tabRadarMines>Mines</a></li>'
			+'	<li class="tab"><a id=tabRadarMyMines>My Mines</a></li>'
			+'</ul>'
	...
	tabRadarBattle : function (){
		...
		var m = '<div id=tabRadar_0_Results style="height:640px">'
			+'	<div class=bsx_title>List of battlefields</div>'
		...
		function display (){
			...
			var m = '<table class=row_style>'
				+'	<tr class=row_headers>'
				+'		<td width="40px">&nbsp;</td>'
				+'		<td id=tabRadarBattle_tal_0 width="140px" style="overflow-x:auto"><A><span class=bold_white>Name</span></A></td>'
				+'		<td id=tabRadarBattle_tal_1 width="50px"><A><span class=bold_white>Level</span></A></td>'
				+'		<td id=tabRadarBattle_tal_2 width="50px"><A><span class=bold_white>Defense</span></A></td>'
			...
			for (var f=0; f<t.target_list.length; f++) {
				...
				m += '	<tr valign=middle>'
					...
					+'		<td align=right>'+t.target_list[f].dist+'Km</td>'
					+'		<td align=center>'+t.target_list[f].x+' / '+t.target_list[f].y+'</td>'
					+'		<td align=center width=35px><div style="color:blue; text-decoration:underline; cursor:pointer; margin-bottom:2px; margin-right:1px; float:left;" title="Attack">'+ img +'</div></td>'
					+'	</tr>';
			}
			m += '</table>';
			return m;
		...
		function onClickAction(event){
			...
			if (id[0] && id[1]) {
				...
				if (tot <= 0) {
					var dial = new ModalDialog (t.container, 300, 165, '', false, null);
					dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
					dial.getContentDiv().innerHTML = 'List of battlefields...';
					setTimeout (function(){dial.destroy(); }, 2000);
 ...
 Tabs.Options = {
	...
	init : function (div) {
		...
		try {
			...
			+'	<tr valign=top>'
			+'		<td><b>Game Options :</b></td>'
			+'	</tr>'
			+'</table>'
			+'<br>'
			+'<table class=bsx_table>'
			+'	<tr valign=top>'
			+'		<td><b>Script options :</b></td>'
			+'	</tr><tr valign=top>'
			+'		<td>'
			+'		<label><input id=tabOptions_CB_Verbose type=checkbox '+(Data.options.verboseLog.enabled?' CHECKED ':'')+' /> Enable detailed logging</label>'
			+'		</td>'
			+'	</tr>'
			+'</table>'
			+'<br>'
			...
			+'					<td><input id=tabOptions_BPlay type=submit value=Play>'
			+'						<input id=tabOptions_BStop type=submit value=Stop>'
			+'						<input id=tabOptions_BDefault type=submit value=Defaut></td>'
			+'				</tr>'
			+'				<tr valign=top>'
			+'					<td><label>Training</label></td>'
			+'					<td><input id=tabOptions_TFile size=40 maxlength=150 type=text value="' + Data.options.sound.URL_units + '" />&nbsp</td>'
			+'					<td><input id=tabOptions_TPlay type=submit value=Play>'
			+'						<input id=tabOptions_TStop type=submit value=Stop>'
			+'						<input id=tabOptions_TDefault type=submit value=Defaut></td>'
			+'				</tr>'
			+'				<tr valign=top>'
			+'					<td><label>Lost Arts</label></td>'
			+'					<td><input id=tabOptions_RFile size=40 maxlength=150 type=text value="' + Data.options.sound.URL_research + '" />&nbsp</td>'
			+'					<td><input id=tabOptions_RPlay type=submit value=Play>'
			+'						<input id=tabOptions_RStop type=submit value=Stop>'
			+'						<input id=tabOptions_RDefault type=submit value=Defaut></td>'
			+'				</tr>'
			+'			</table>'
			+'		</td>'
			+'	</tr>'
			...
			+'	<tr valign=top>'
			+'		<td>'
			+'		<label><input id=tabOptions_btnRefresh type=button value="Refresh" /></label>'
			+'		</td>'
			+'	</tr>'
			+'</table>'
			+'<br>'
			+'<table class=bsx_table width=100%>'
			+'	<tr valign=top>'
			+'		<td width=40%><b>Tab Options :</b></td>'
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabContacts type=checkbox '+ (Data.options.disable_contacts?'CHECKED ':'') +' /> deactivate the...<B>Contacts Tab</B></label></td>'
			+'		<td><label><input id=tabOptions_TabRadar type=checkbox '+ (Data.options.disable_radar?'CHECKED ':'') +' /> deactivate the...<B>Eagles Nest Tab</B></label></td>'
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabAlliance type=checkbox '+ (Data.options.disable_alliance?'CHECKED ':'') +' /> deactivate the...<B>Alliance Tab</B></label></td>'
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabLog type=checkbox '+ (Data.options.disable_log?'CHECKED ':'') +' /> deactivate the...<B>Log Tab</B></label></td>'
			+'		<td>&nbsp;</td>'
			+'	</tr>'
			+'</table><br>'
			+'<div class=bsx_title style="margin-bottom:10px">Local data storage</div>'
			+'<table class=bsx_table width=100%>'
			+'	<tr valign=top>'
			+'		<td width=30%><label><input id=tabOptions_btnSave type=button value="Save" title="Force save the current settings of the script" /></label></td>'
			+'		<td width=30%><label><input id=tabOptions_btnClean type=button value="Clear Data" title="Clear data from local storage (internet cache)" /></label></td>'
			+'		<td width=40%><label><input id=tabOptions_btnInspect type=button value="Inspect Data" title="Inspect the cached data" /></label></td>'
			+'	</tr><tr valign=top><td style="font-size:2px">&nbsp</td>'
			+'	</tr><tr valign=top>'
			+'		<td width=30%><label><input id=tabOptions_btnParams type=button value="Params session" title="Inspect the session parameters" /></label></td>'
			+'	</tr>'
			+'</table><br>'
			+'</div>';
			t.container.innerHTML = m;
	...
	onClickSaveSetting : function () {
		var t = Tabs.Options;
		dialogConfirm('Do you want to save data in permanent the current setting of the script?',
			function(){
				try {
					Data.save();
					verboseLog('Saving data in the local Storage');
	...
	onClickClearStorage : function () {
		var t = Tabs.Options;
			dialogConfirm('Are you sure want to delete all permanent data?',
                function(){
					try {
...
function dialogCopyPaste (params){
	...
	var layout = '<br>'
				...
				+'<table width=100%>'
				+'<tr align=center>'
				+'<td><input id=btn_ok type=button class=confirm_button value="Close" /></td>'
				+'</tr></table>';
...
function updaterConfirm (msg, onContinue, onCancel, two_buttons){
	...
	if (two_buttons) {
		layout += '	&nbsp; &nbsp;'
				+'<input id=btn_cancel type=button class=confirm_button value="Cancel" />';
	}
	layout += '</td></tr></table>';
...
function ModalDialog (curtainDiv, width, height, styleName, allowClose, notifyClose){
	...
	this.div.innerHTML = ''
		...
		+'			<input id=ModalDialog_btnClose type=button value="Close" style="display:none" />'
		+'		</center></div>'
		+'		</td>'
		+'	</tr>'
		+'</table>';
...
function UnitSendDialog (curtainDiv, order_id, owner_id, target_id, troops, allowClose, notifyClose){
	...
	if (troops)
		h += '	<td align=center width=12% id=UnitSend_sel_all><A><span class=ref_gold>All</span></A></td>'
			+'	<td align=center width=12% id=UnitSend_sel_att><A><span class=ref_gold>Attack</span></A></td>'
			+'	<td align=center width=12% id=UnitSend_sel_def><A><span class=ref_gold>Defense</span></A></td>'
			+'	<td align=center width=12% id=UnitSend_sel_spy><A><span class=ref_gold>Spy</span></A></td>'
			+'	<td align=center width=12% id=UnitSend_sel_none><A><span class=ref_gold>None</span></A></td>';
	else
	...
	this.div.innerHTML = ''
		...
		+'			<td width=10%>&nbsp;</td>'
		+'			<td width=45%><input id=UnitSend_btnClose type=button value="Close" style="display:none" /></td>'
		+'		</center></div>'
		+'		</td>'
		+'	</tr>'
		+'</table>';
	...
	if (document.getElementById('UnitSend_btnSubmit'))
		document.getElementById('UnitSend_btnSubmit').addEventListener('click', function (){
			setText('UnitSend_Body', '<div height=100% width=100% align=center valign=middle>Launching the march ...</div>');
	...
	function sendMarch (this_order_id, this_owner_id, this_target_id, troops_to_send, callback) {
		...
		new MyAjaxRequest (C.attrs.defaultServer, C.attrs.defaultSegment, 'SendUnit', JSON.stringify(data), function (rslt) {
			if (rslt.ok && rslt.dat) {
				...
				if (callback) callback({done:true, msg:'march sent'});
			} else {
				if (callback) callback({done:false, msg:'Error of the\'March<br>'+(rslt.errmsg || '')});
			}
			return;
		}, true);
	}
...
var tabManager = {
	...
	init : function (mainDiv){
		...
		+'	<tr align=center><td width=45% align=left>'+ scriptName +' by '+ mainAuthor  +' - v'+ scriptVersion +'</td>'
		+'		<td width=33% align=center><SPAN id=tabManager_Alert></span></td>'
...
function refreshGameData (container, notify){
	...
	dial.getTopDiv().innerHTML = '<div class=bsx_title><center><b>'+scriptName+' : Message</b></center></div>';
	dial.getContentDiv().innerHTML = 'Updating player data ...';
	...
	AutoRefresh.autoRefreshCmd(function (rslt) {
		if (rslt.done) {
			actionLog('Player data updates');
			dial.getContentDiv().innerHTML = 'Player data updates';
			setTimeout (function(){dial.destroy(); if (notify) notify(true);}, 2000);
			return;
		}
	});
...
setMainStyles();
var initialDelay = Math.randRange(8000, 11000);
progressBar.init (800+Math.randRange(1,50), 100, 450, 150, 'initialization', 300, false);
progressBar.start({ steps:1, delay:initialDelay, title:scriptName +' - v'+ scriptVersion + ' : Starting ...', stepText:'Delay before initial startup script' });
