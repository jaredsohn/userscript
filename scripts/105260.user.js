// ==UserScript==
// @name           OGame Redesign: AutoExpoFleet for Raider with Light Fighter
// @namespace      8arlock
// @description    Auto calc expofleet for Raider with Light Fighter: 142LC+5LF+1Bmb+1EP/183LC+24LF+1Dr+1EP
// @version        1.1.6
...
(function() 
{
 ...
 //http://uni.ogame.ru/game/index.php?page=highscoreContent&category=1&type=0&site=1
/*
Точнее, скрипт симил этих флотов:

1) 183 БТ, 24 ЛИ, 1 ШЗ, 1 Уник - этим я летаю сейчась.
2) 190 БТ, 11 ЛИ, 1 ШЗ, 1 Бобр - один из оптимальных флотов.
3) 193 БТ, 2 ЛИ, 1 ШЗ, 1 Бобр - другой оптимальный флот.
4) 189 БТ, 6 ЛИ, 1 ШЗ, 1 Уник - другой оптимальный флот.
5) 188 БТ, 9 ЛИ, 1 ШЗ 1 Уник - другой оптимальный флот.
6) 192 БТ, 5 ЛИ, 1 ШЗ, 1 Бобр - другой оптимальный флот.

Эксполимит 9.000: 142 БТ, 5 ЛИ, 1 ШЗ, 1 Бобр.
Эксполимит 6.000: 1 МТ, 92 БТ, 4 ЛИ, 1 ШЗ, 1 Бобр.
Эксполимит 2.500: 35 БТ, 1 ЛИ, 1 ШЗ, 1 Бобр.
*/
    ...
    function calcExp()
    {
        ...
        calcUseShip(210,   1);
		calcUseShip(203, (2500 >= evaluemax ? 35 : 6E3 >= evaluemax ? 92 : 9E3 >= evaluemax ? 142 : 183));
		calcUseShip(202, (2500 >= evaluemax ? -1 : 6E3 >= evaluemax ?  1 : 9E3 >= evaluemax ?  -1 :  -1));
		calcUseShip(204, (2500 >= evaluemax ?  1 : 6E3 >= evaluemax ?  4 : 9E3 >= evaluemax ?   5 :  24));
		calcUseShip(211, (2500 >= evaluemax ?  1 : 6E3 >= evaluemax ?  1 : 9E3 >= evaluemax ?   1 :  -1));
		calcUseShip(213, (2500 >= evaluemax ? -1 : 6E3 >= evaluemax ? -1 : 9E3 >= evaluemax ?  -1 :   1));
        calcUseShip(203, null); 
        calcUseShip(202, null); 
