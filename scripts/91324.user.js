// ==UserScript==
// @name                 إظهـار نقاط الهجوم في التقارير + نقاط الدفاع
// @version              1.3.8
// @author               Samuel Essig ( الـدمـار :تــم التعريب عن طريق )
// @namespace            c1b1.de
// @homepage             http://c1b1.de
// @copyright            2008 - 2010, Samuel Essig (http://c1b1.de)
// @license              No Distribution!
// @description          نقطاط الهجوم التي اكتسبتها انت , نقاط الدفــاع التي إكتسبها عدوك 
...
if(defender_table)
  ...
  td.appendChild(document.createTextNode('نـقـاط الدفاع: '+stringInt(defender_points)));
  tr.appendChild(td);
...
if($('#attack_info_def').$('th')[1].firstChild.nodeValue != '---') // If village not empty
  ...
  var td = document.createElement('td');
  td.setAttribute('colspan','2');
  td.appendChild(document.createTextNode('نقـاط الهجوم: '+stringInt(attackers_points)));
  tr.appendChild(td);
