...
if (test!=null) { // pa and map available ?
	...
	if (mapoptions.length > 0) {
		...
		var html = '<h4>erweiterte Optionen</h4><input id="belief_movement" type="checkbox"';
		
		var opt1 = getCookie('ds_bel_mov');
		if (opt1==1) html += ' checked="checked"';
		html += ' name="belief_movement" onclick="if (this.checked) setCookie(\'ds_bel_mov\', \'1\', 30); else setCookie(\'ds_bel_mov\', \'0\');window.location.reload();" /><label for="belief_movement">Truppenbewegungen anzeigen</label>';
		html += '<br/><input id="belief_buildinginfo2" type="checkbox"';

		var opt1b = getCookie('ds_bel_bui2');
		if (opt1b==1) html += ' checked="checked"';
		html += ' name="belief_buildinginfo2" onclick="if (this.checked) setCookie(\'ds_bel_bui2\', \'1\', 30); else setCookie(\'ds_bel_bui2\', \'0\');window.location.reload();" /><label for="belief_buildinginfo2">&nbsp;&nbsp;nur anzeigen wenn auf Karte</label>';

		html += '<br/><input id="belief_buildinginfo" type="checkbox"';

		var opt2 = getCookie('ds_bel_bui');
		if (opt2==1) html += ' checked="checked"';
		html += ' name="belief_buildinginfo" onclick="if (this.checked) setCookie(\'ds_bel_bui\', \'1\', 30); else setCookie(\'ds_bel_bui\', \'0\');window.location.reload();" /><label for="belief_buildinginfo">Gebaeudeinformationen anzeigen</label>';


		var dorfid = document.location.href.split("village=")[1].split("&")[0];

		html += '<br/><br/><a href="/game.php?village=' + dorfid + '&screen=overview_villages&mode=commands&group=0" target="_blank" title="Oeffnet ein neues Fenster mit der Befehlsuebersicht.">Befehlsuebersicht</a><br/><span style="font-size: 10px;"> Achtung: Gruppe -alle- wird ausgewaehlt.</span>';
		html += '<br/><br/><h4>Legende</h4><div style="width: 10px;height: 10px;background-color: #D00;"></div>Angriff<br/><div style="width: 10px;height: 10px;background-color: #00D;"></div>Spaeh<br/><div style="width: 10px;height: 10px;background-color: #0D0;"></div>AG';
