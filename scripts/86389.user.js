...
// @description live script testing
// @include     http://apps.facebook.com/inthemafia*
// @exclude     http://facebook.mafiawars.com/mwfb/remote/html_server.php?*
// @version     1.07
// ==/UserScript==
...
javascript:(function(){
	...
	var version = 'RepeatJob Vegas v1.06 experimental', jobstodo = 0, job = 0, onevent = 'Pause', wait1 = 1, wait2 = 3, combinedloot='';
	var bossenergy = parseFloat(document.getElementById('user_max_energy').innerHTML*0.2).toFixed(0);
	var exp_gained = 0, energy_spent = 0, stamina_spent = 0, lootcount = 0, run = 1;
	var money_gained = 0, money_banked = 0, bankamount = 0, sign = '$';
	var master = 0, wheel = 0, bag = 0;
	var content = document.getElementById('content_row');
	var log_size = 10, log_keep=/(mastered|completed|secret)/, timestamping = true;
	...
	function get_selected_jobinfo() {
		...
		if (thejob.snapshotItem(0) === wrong|| thejob.snapshotItem(0) === blank ) {
		...
		if (button.snapshotItem(0) !== wrong) {
	...
	function read_settings() {
		...
		if (parameters.length < 4) {
			settings = wait1+'|'+wait2+'|'+onevent+'|'+log_size;
			createCookie('rjv_settings',settings);
...
//	var bankingrow = '';
	...
	// '<tr>'+
		// '<td colspan="3">'+
			// 'QuickLinks: '+
			// '<a id="current_job">Original Job</a> - '+
			// '<a id="hot_tip">Hot Tip</a> - '+
			// '<a id="alarm_code">Alarm Code</a> - '+
			// '<a id="key_card">Hotel Security Key Card</a>'+
		// '</td>'+
	// '<tr>'+
	'<tr>'+
		'<td>Jobs done:</td><td><span id="jobs">0</span> of <input id="jobstodo" name="jobstodo" type="text" value="'+jobstodo+'" maxlength="4" /> (0 = Unlimited)'+bankingrow+'</td>'+
	...
	'<tr>'+
		'<td>Stats:</td>'+
		'<td>Exp: <span id="exp_gained" class="good"></span> <span id="exp_ratio" class="more_in"></span>&nbsp; Money: <span id="money_gained"></span>&nbsp;<span class="more_in" id="money_banked"></span></td>'+
	...
	'<tr><td>Strength:</td><td colspan="2"><img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_mafia_attack_22x16_01.gif" width="22" height="16" ><span id="userattack"></span> <img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_mafia_defense_22x16_01.gif" width="22" height="16" ><span id="userdefense"></span></td></tr>'+
	'<tr>'+
		'<td valign="top"><a href="#" id="lootshow">Hiding</a> loot:</td>'+
	...
	function log(s) {
		...
		document.getElementById('money_gained').innerHTML = '<strong class="cur_cash">'+sign+commas(money_gained)+'</strong>';
			...
			$.ajax({
				...
				success: function (bankresponse){
					try {
						...
						if (/(cannot deposit)/.test(bankdata['success_message'])) {
							bankamount = 0;
							document.getElementById('bankamount').value = 0;
							log(timestamp()+' <span class="bad">Banking not done because</span>: '+bankdata['success_message']);
						}
					}
					catch (ignorebank) { 
					...
					setTimeout(function(){ repeat_job(jobid,tabid,oid,jobsig); },wait);
				}
			});
		}
	}
	...
	//console.log(jid+' '+tid+' '+oid+' '+jsig);
		...
		// }
			...
			if (p(document.getElementById(cashid).innerHTML) > bankamount && bankamount > 0 && run != 0) {
				banker(p(document.getElementById(cashid).innerHTML),5);
			}
			else {
				do_job('http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=map&xw_action=dojob&xw_city=4&job='+jid+'&tab='+tid+'&opponent_id='+oid+'&job_sig='+jsig);
			}
		}
		else {
			msg('Pausing...');
	...
	function pick_weakest(obj) {
		...
		// if (obj[0]['group_size'] < obj[1]['group_size'] < obj[2]['group_size']) {
			// var oid = obj[0]['id']
			// var jobsig = obj[0]['job_sig'];
			//console.log('Picked opponent nr 0 level: '+obj[0]['level']+' mafia: '+obj[0]['group_size']);
		// }
		// if (obj[1]['group_size'] < obj[0]['group_size'] < obj[2]['group_size']) {
			// var oid = obj[1]['id']
			// var jobsig = obj[1]['job_sig'];
			//console.log('Picked opponent nr 1 level: '+obj[1]['level']+' mafia: '+obj[1]['group_size']);
		// }
		// if (obj[2]['group_size'] < obj[0]['group_size'] < obj[1]['group_size']) {
			// var oid = obj[2]['id']
			// var jobsig = obj[2]['job_sig'];
			//console.log('Picked opponent nr 2 level: '+obj[2]['level']+' mafia: '+obj[2]['group_size']);
		// }
	...
	function do_job(url) {
		...
		$.ajax({
			...
			success: function (response){
				...
				if ((object['popup'] !== undefined) && debug) {
					console.log(object['popup_id']);
					console.log(object['popup'].replace(/\\"/g,''));
				}
				//http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=job&xw_action=share_loot&xw_city=4&tmp=c8302f57e20227cc41ad085840df8942&cb=71ed4c40c0bf11dfb1006fd49e25e5df&xw_person=1838839&liteload=1&xw_client_id=8&loot=1
				...
				if(objectdata['job_results']['success'] == '1') {
					...
					if (objectdata['job_results']['boss_consumable'] !== undefined) {
						parse_loot(objectdata['job_results']['boss_consumable']);
					}
					if (objectdata['job_results']['chop_shop_loot'] !== undefined) {
						parse_loot(objectdata['job_results']['chop_shop_loot']);
					}
					if (objectdata['job_results']['casino_loot'] !== undefined) {
						parse_loot(objectdata['job_results']['casino_loot']);
					}
					if (objectdata['job_results']['collection'] !== undefined) {
						parse_loot(objectdata['job_results']['collection']);
					}
					var pre = '';
					...
					if (/You lost the fight/.test(objectdata['job_results']['title'])) {
						...
						log(timestamp()+pre+' Gained <span class="good">'+exp+' exp</span> and <span class="cur_cash"><strong>V$'+commas(money)+'</strong></span>'+mastery+loot.slice(0,loot.length-2));
					}
					function f () {
						repeat_job(jobid,tabid,oid,jobsig)
					}
					...
					missing_items = true;
					var missing = '';
					if (objectdata['job_results']['missing_items'].length === undefined) {
						missing_item = objectdata.job_results.missing_items.name[0];
						missing = objectdata.job_results.missing_items.name[0]+' (x'+objectdata.job_results.missing_items.amount[0]+')';
					}
					else {
	...
	function parse_loot(lootobject) {
		...
		if (lootobject.length !== undefined) {
			...
			add_loot(lootobject['name'].replace(/^[A|a]n?\s/,''),lootobject['quantity']);
	...
	if (jobs_that_cost.indexOf(jobid) != -1) {
		log(timestamp()+'Job costs money, not depositing any cash.');
		bankamount = 0;
		document.getElementById('bankamount').value = 0;
	}
	
	repeat_job(jobid,tabid,opponent,jobsig);
	log('This is a experimental release, expect it to blow up :)');
