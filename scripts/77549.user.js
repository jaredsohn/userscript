// ==UserScript==
// @name           1000Manga
// @namespace      Knace & Ikaraus
// @description    A 1000manga script: Loads all images in the entire chapter vertically. You only need to scroll down to continue reading which keeps the flow of the manga. Also makes saving the entire chapter simple and easy! It's just a simple modification of OneManga script, since both are the same shit.
// @version          1.2.1
...
var url_directory = 'http://www.1000manga.com/'+url[1];
var url_chapter = url[2];
var page = '01';
	//if(target_load) { page = url[3]; }


var skip_summary = GM_getValue('skip_summary', true);
...
if( window.location==url[0]+'//'+url[2]+'/'+url[3]+'/'+url[4]+'/' ) {
	...
	{ //Clear and Create Menu
		...
		var element = document.createElement('div');
			...
			element.innerHTML = '<div style="background:#FFFFFF; border:solid #000000 medium; padding:3px"><a href="http://'+window.location.host+'">1000manga</a> : <a href="'+url_directory+'">Manga Info</a> : [ <a href="'+chapter_prev+'">&lt;&lt;</a> <select id="chapters" onchange="window.location.href=\''+url_directory+'/\'+this.value">'+ch_selecter+'</select> <a href="'+chapter_next+'">&gt;&gt;</a> ] ('+total_images+' Images) <select id="page" onchange="var e=document.getElementById(\'\'+this.value);if(e){e.style.display=\'\'}window.location=\'#p\'+this.value">'+pg_selecter+'</select></div>';
