...
// ==UserScript==
// @author      Risi fixed by bluelovers modified by Serekos
// @namespace	http://userscripts.org/
// @name		Travian Task Queuer
// @description	Schedule delayed constructions, upgrades and attacks. fixed by bluelovers
// @include     http://s*.travian.*/*
// @include     http://s*.travian3.*/*
// @include     http://welt*.travian.*/*
// @include     http://news*.travian.*/*.php*
...
switch(sLang) {
	...
	case "cn": //by Jacky-Q
		aLangBuildings = ["", "伐木场", "黏土矿", "铁矿场", "农场", "木材厂", "砖块厂", "铸造厂", "磨坊", "面包房", "仓库", "粮仓", "铁匠铺", "军械库", "竞技场", "村中心大楼", "集结点", "市场", "大使馆", "兵营", "马厩", "工场", "研发所", "山洞", "市政厅", "行宫", "皇宮", "宝库", "交易所", "大兵营", "大马厩", "罗马城墙", "日尔曼城墙", "高卢城墙", "石匠铺", "酿酒厂", "陷阱机", "英雄园", "大仓库", "大粮仓", "世界奇观"];
	...
	case "tw":  //by bluelovers 2008.01.31.03.57
		aLangBuildings = ["", "伐木場", "泥坑", "鐵礦場", "農田", "鋸木廠", "磚廠", "鋼鐵鑄造廠", "麵粉廠", "麵包店", "倉庫", "穀倉", "鐵匠", "盔甲廠", "競技場", "村莊大樓", "集結點", "市場", "大使館", "兵營", "馬廄", "工場", "研究院", "山洞", "村會堂", "行宮", "皇宮", "寶物庫", "交易所", "大兵營", "大馬棚", "城牆", "土牆", "木牆", "石匠鋪", "釀酒廠", "陷阱", "英雄宅", "大倉庫", "大穀倉", "世界奇觀"];
		aLangTasks = ["建築", "升級", "攻擊", "研發", "訓練", "運輸", "平倉", "拆除"];
		aLangStrings = ["預定建築", "預定升級", "預定攻擊", "預定研發",
		"將此事項預定稍後執行.", "建築開始了 ", " 已嘗試但結果不明.",
		"等級", " 不能建築.", " 不能升級.", "此事項已預定稍後執行.",
		"目前生產:", "我們暫時不能預定稍後執行.", "不能預定稍後執行!", "已預定稍後執行項目",
		"删除", "稍後送出", "訓練不能預定執行因為沒有選擇軍隊.","你的軍隊已送去", "你的軍隊不能送去",
		"支援", "攻擊", "搶奪", "投石車會瞄準",	"隨機",
		"於", "或之後",	"秒", "分", "時", "日",
		"偵察資源和軍隊", "偵察防禦設施和軍隊","不在",
		"攻擊無法預定執行,因為沒有指定目的地.", "at site no.",
		"分類以:", "類型 ", "時間 ", "目標 ", "選項", "村莊 ",
		"Task History", "flush history",
		"研發開始了 ", " 不能研發.",
		"稍後訓練", "偵察", "稍後訓練", "軍隊", "訓練", "訓練開始了 ", " 不能訓練.",
		"攻擊不能預定執行因為沒有選擇軍隊.","或者","重覆執行","間隔時間","間隔縮減量(累計)","連續點擊出兵"
		];

//		var aLangStringsMaster = ["Build later", "Upgrade later", "Attack later", "Research later"
...
//		, "The attack cannot be scheduled because no troops were selected."
//		];
		...
		aLangTransModel= ["混合模式", "只運糧食", "只運資源", "固定模式"];
		aLangStringt   = ["新兵開發", "自動運輸", "預定平倉", "商人", "從", "到", "運輸方式", "來回用時", "分", "秒", "運送次數:", "次", "下次運輸時間:", "已經設置完成!", "已經開始了！", "沒有足夠的商人！！", "本次運輸失敗,資源不足或商人未回。", "不必平倉,重新計畫延時", "糧食資源：", "自動平倉成功！！", "伺服器返回失敗資訊！", "平倉目標：", "木頭", "泥土", "鐵塊", "糧食", "木泥鐵平均分配", "分配方案：" , "注意：不要修改運輸出發村。過度使用小心封號。", "保持為0表示平均分配，期望分配資源應該比實際的少。", "你正在使用伺服器時間.", "你正在使用電腦本機時間.","預定時間強制平倉(無延時檢測)", "自動拆除"];
	...
	case "ae":
		aLangBuildings = ["", "الحطاب", "حفرة الطين", "منجم حديد", "حقل القمح", "معمل النشارة", "اعمال البناء", "حديد مسبك", "المطاحن", "مخبز", "مخزن", "مخزن الحبوب", "حداد", "مستوع الاسلحة", "ساحة البطولة", "المبنى الرئيسي", "نقطة التجمع", "السوق", "السفارة", "ثكنه", "الاسطبل", "المصانع الحربية", "الاكاديمية", "المخبأ", "البلدية", "السكن", "قصر", "غرفة الكنز", "المكتب التجاري", "ثكنة كبيرة", "اسطبل كبير", "سور القريه", "سور القرية", "الحاجز", "الحجار", "Brewery", "الصياد", "قصر الأبطال", "مخزن كبير للمواد الخام", "ذاكرة حبوب كبيرة", "معجزه العالم"];
	...
	case "ua": //by Evgeny Lukyanenko
		aLangBuildings = ["", "Лісоповал", "Глиняний кар'єр", "Залізна копальня", "Ферма", "Деревообробний завод", "Цегляний завод", "Чавуноливарний завод", "Млин", "Пекарня", "Склад", "Зернова комора", "Кузня зброї", "Кузня обладунків", "Арена", "Головна будівля", "Пункт збору", "Ринок", "Посольство", "Казарма", "Стайня", "Майстерня", "Академія", "Схованка", "Ратуша", "Резиденція", "Палац", "Скарбниця", "Торгова палата", "Большая казарма", "Большая конюшня", "Мур", "Земляной вал", "Изгородь", "Каменетес", "Пивоварня", "Капканщик ", "Таверна", "Большой склад", "Большой амбар", "Чудо"];
		aLangTasks = ["Побудувати", "поліпшити", "Атаковать", "Досліджувати", "Обучить"];
		aLangStrings = ["побудувати пізнше", "розвити пізнше", "атаковати пізнше", "обучити пізнше", "Запланувати задачу.", "Мы начали строительство ", " мы попробовали, но результат не известен.", "рівень", " не может быть построено.", " не может быть развито.", "Задача запланирована.", "Текущее производство:", "Мы не можем планировать этого сейчас.", "Запланированной задачи не существует!", "Заплановані задачи", "Удалити", "відправити пізнше", "Атака не может быть запланирована, поскольку войска не выбраны.", "Ваши войска были отправленны", "Ваши войска не могут быть отправлены", "Підкріплення", "Звичайний", "Розбійницький набіг", "Катапульты нацелены на", "Случайно", "в", "или по истечении", "секунд", "минут", "часов", "дней", "Разведка ресурсов и войск", "Разведка войск и оборонительных сооружений", "Отсутствует", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Сортувати по:", "type ", "time ", "target ", "options ", "village "];
		aLangTroops[0] = ["Легіонер", "Преторіанець", "Імперіанець", "Кінний розвідник", "Кіннота імператора", "Кіннота Цезаря", "Таран", "Вогняна катапульта", "Сенатор", "Поселенець", "Герой"]; //Римляни
		aLangTroops[1] = ["Дубинник", "Списник", "Сокирщик", "Скаут", "Паладин", "Тевтонський вершник", "Стінобитне знаряддя", "Катапульта", "Ватажок", "Поселенець", "Герой"]; //Тевтонці
		aLangTroops[2] = ["Фаланга", "Мечник", "Слідопит", "Тевтацький грім", "Друїд-вершник", "Едуйська кіннота", "Таран", "Катапульта", "Лідер", "Поселенець", "Герой"]; //Галли
		break;
...
//		break;
	...
	default: // default is english
		aLangBuildings = ["", "Woodcutter", "Clay Pit", "Iron Mine", "Wheat Field", "Sawmill", "Brickworks", "Iron Foundry", "Flour Mill", "Bakery", "Warehouse", "Granary", "Blacksmith", "Armory", "Tournament Square", "Main Building", "Rally Point", "Marketplace", "Embassy", "Barracks", "Stable", "Siege Workshop", "Academy", "Cranny", "Town Hall", "Residence", "Palace", "Treasury", "Trade Office", "Great Barracks", "Great Stable", "City Wall", "Earth Wall", "Palisade", "Stonemason", "Brewery", "Trapper", "Hero's Mansion", "Great Warehouse", "Great Granary", "Wonder"];
		aLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train", "Transport", "Trade"];
		aLangStrings = ["Build later", "Upgrade later", "Attack later", "Research later", "Schedule this task for later.", "We started builing ", " was attempted with unknown result.", "level", " cannot be built.", " cannot be upgraded.", "The task was scheduled.", "Current production:", "We can't schedule this task right now.", "Task scheduling is not available!", "Scheduled Tasks", "Delete", "Send later", "No troops were selected.", "Your troops were sent to", "Your troops could not be sent to", "Support", "Attack", "Raid", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "away", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ", "Task History", "flush history", "We started researching ", " cannot be researched."];
		aLangTroops[0] = ["Legionnaire", "Praetorian", "Imperian", "Equites Legati", "Equites Imperatoris", "Equites Caesaris", "Battering Ram", "Fire Catapult", "Senator", "Settler", "Hero"];  //Romans
		aLangTroops[1] = ["Clubswinger", "Spearman", "Axeman", "Scout", "Paladin", "Teutonic Knight", "Ram", "Catapult", "Chief", "Settler", "Hero"];  //Teutons
...
///////////////////////////resource auto transportor/////////////////
	if (aThisTask[0]!=5 && (isNaN(aThisTask[5])) || (aThisTask[5] <= 0)){ //modify or delete the transport task at last step in addToHistory()
		aTasks.splice(i, 1);  //delete this task
		bTaskDeleted = true;
	} else if (aThisTask[5] > 0) { 
		
		var iTaskTime = parseInt(oDate.getTime()/1000) + 60*(2*(.5-Math.random())+parseInt(aThisTask[5]));
		//printMsg(aThisTask[1]+"->"+iTaskTime,false);
			aThisTask[1] = iTaskTime;
		...
		for (var i=0;i<aTasks.length;i++){
			...
			if (aTask.join(",")==aThisTask){
				if (aThisTask[0] == 5){//Silly market attempt thingy.
...
//	oTaskList.appendChild(oDeleteAllTask);
	...
	for(var i = 0; i < aTasks.length; i++) {
		...
		if(bUseServerTime) {
			...
			//var sTime = oDate.toLocaleString();
			var sTime = formatDate(oDate.getUTCFullYear(), (oDate.getUTCMonth() + 1), oDate.getUTCDate(), oDate.getUTCHours(), oDate.getUTCMinutes(), oDate.getUTCSeconds());
		...
		if (parseInt(aThisTask[5]) == 0){
			...
			var sTime = oDate.getYear()+","+ monthNames[oDate.getUTCMonth()]+","+ oDate.getUTCDate()+","+ oDate.getUTCHours()+":"+ oDate.getUTCMinutes()+":"+ oDate.getUTCSeconds();			
            var sTime = "<span style='color:black; cursor:pointer;' id='ttq_tasktime_" +i+ "' title='This is your local time.' ttq_taskid='" +i+ "' >" + sTime + "</span>";}
        else{            var sTime = "<span style='color:blue; cursor:pointer;' id='ttq_tasktime_" +i+ "' title='This event repeats every "+parseInt(aThisTask[5])+" minutes.' ttq_taskid='" +i+ "' >" + oDate.toLocaleString() + "</span>";
        }
}
...
///////////////////////////resource auto transportor/////////////////end
		...
		oTaskRow.innerHTML = "<span class='ttq_time_village_wrapper' style='display:inline !important;'>" +sTime + "<span class='ttq_village_name'>" + sVillageName+ "</span>" + ":</span> <span title='" +sTaskMoreInfo+ "' size = 5 style='cursor:help;' >" +sTask+ " " +sTaskSubject+ " </span></span>";
...
/**
  ...
  */
function setTask(iTask, iWhen, target, options, iVillageId, infLoop) {
		_log(3, "-> setTask()");
...
//	}
	...
	if(iVillageId) {
		newValue += ',' + iVillageId+','+infLoop;
	} else {
		newValue += ',' + 'null'+','+infLoop;
	}
...
/*
function abbrevDate(dateObj){
		var D = date.getDay(),
			m = date.getMonth(),
			H = date.getHours(),
			M = date.getMinutes(),
			s = date.getSeconds(),
	return dateObj.getDay()+", "+dateObj.getMonth()+", ";
}*/
...
///////////////////////////resource auto transportor/////////////////
	...
	if (aTask[0]==5){ //append it again if transport times > 0
		...
		bLocked = false;
	} //else if (aTask[5]>0 && (!isNaN(aThisTask[5])) ){ //append it again if transport times > 0
	//	bLocked = true;
	//	data = readCookie("TTQ_TASKS");
	//	aTasks = data.split("|");
	//	for (var i=0;i<aTasks.length;i++){
	...
	//				var oDate = new Date();  // current GMT date. TODO: server time
	//				var iTaskTime = parseInt(oDate.getTime()/1000);
	//				aThisTask[1] = parseInt(oDate.getTime()/1000) + 60*(2*(.5-Math.random())+parseInt(aThisTask[5])); 
	//				if (aThisTask[1] < iTaskTime) aThisTask[1] = iTaskTime + getRandom(iDelayMIN,iDelayMAX);
					//aThisTask[3] = options.join("_");
	//				aTasks[i] = aThisTask.join(",");
	//			}
...
//			var iCoordY = coordZToY(target);
			...
			sWhat = targetCords.length > 1 ? targetCordValue : '<span size = "2" id="ttq_placename_' +target+ '">' + getPlaceName(target)+ '</span>';

			sMoreInfo = (iVillageId ? getVillageLists2(iVillageId) : '') +'<br>' +tt('To') + '<input name="cords" size = "2" value="'+targetCordValue+'" style="width:80%"><br />' + sMoreInfo;
...
//				sTaskExMode += options.innerHTML;
				...
				for (var i=19; i <= 40; i++) {
					sTaskExMode += '<option size = "8" value="' + i + '" '+ (i == options ? 'selected' : '') +'> ' + i + '. </option>';
				}
			}
...
///////////////////////////resource auto transportor/////////////////end
	...
	if (iTask != 5) {
		sTaskRepeated = ' '+t("or") + ' '
			+ t("EspacedTime")+ ' <input name="espacedTime"  size="3" type="text" id="espacedTime" value ="" />'
			...
			+'</select> '
			+ t('repeat')+ ' <input size="4" width="10" name="nbOfRepeat" type="text" id="nbOfRepeat" value = "1" /> '
			+ t('ForeachAddTime')+ ' <input size="3" width="10" name="nbOfRepeat_add" type="text" id="nbOfRepeat_add" value = "0" /> '
			+'<select name="timeUnit_nbOfRepeat_add">'
				...
				+'<option value="86400">' + t("days") + '</option>'
			+'</select> ';
		sTaskInf=t("Loop every ")+'<input name="loop_indef" type="text" id="loop_indef" value="0" size=3 />'+t(' minutes, until stopped.')+'<br/>'
	}
...
function handleTimerForm(oForm) {
	...
	var infLoop=oForm.loop_indef.value
	if(at == '') {
...
//		_log(3, "<- handleTimerForm()");
	...
	if (oForm.nbOfRepeat!=null && oForm.espacedTime != null && oForm.espacedTime.value && oForm.nbOfRepeat.value > 1){
		...
		setTask(oForm.timerTask.value, iTaskTime, oForm.timerTarget.value, oForm.timerOptions.value, iVillageId, infLoop);
			_log(3, "<- handleTimerForm()");
	}
}
...
function getVillageLists(opt){
  ...
  ctx += tt("From");
  ctx += ("<select size = '7' id='t_villageid'>"+options1+"</select>&nbsp;"+tt("To"));
  ctx += ("<select id='t_toxy' size = '6' onchange='var aaa = this.value.split(\"|\");document.getElementById(\"t_x\").value=aaa[0];document.getElementById(\"t_y\").value=aaa[1];'>"+options2+"</select>&nbsp;");
