...
002 Add Map Begin Level and End Level Option save
003 Add Map Level Begin to show Map Info data
...
ShowOptionsDialog:function() {
	...
	div.innerHTML="<form>"+
		...
		"<input id='KOCAttackMapLevelBegin' value='"+this.options.mapLevelBegin+"' size='3' /> Map Level Begin.<br />"+
		"<input id='KOCAttackMapLevelEnd' value='"+this.options.mapLevelEnd+"' size='3' /> Map Level End.<br />"+
		"<hr />"+
	...
	ById('KOCAttackOptionsSave').addEventListener('click',function() {
		...
		t.options.mapLevelBegin=parseFloat(ById('KOCAttackMapLevelBegin').value);
		t.options.mapLevelEnd=parseFloat(ById('KOCAttackMapLevelEnd').value);
		
		t.options.attackMaxDistance=parseInt(ById('KOCAttackMaxDistance').value);
...
GetOptions:function() {
	...
	var defOptions={"attackDelay":15,"attackType":"Camp","attackOrder":"closest","autoRemoveReports":true,"attackSecsSinceLastCity":60*60*12,"attackSecsSinceLastCamp":3600,"randomPercent":20,"mapLevelBegin":1,"mapLevelEnd":10,"keepReports":10,"attackMaxDistance":60,"waitAfterCitiesDone":15,"autoAttackCitiesDoneMax":3,"changeCitySecs":15,"retryMarch":true,"noViewReports":false,"chromeKeepReports":2,"percentOfPopToTrain":100,"autoGoldHappiness":99};
	...
	if(autoAttack && autoAttack.x==xy[0] && autoAttack.y==xy[1] && !ignore.checked) {
		...
		&& btnMarch.style.opacity!=0.5
			...
			this.Log('auto attack, not enough troops/knights: '+xy[0]+','+xy[1]+
				", knights avail:"+(knightSelect.options.length-1)+", Not enough troops:"+notFull+', needed:'+totalTroops);
...
DrawLevelIcons:function() {
	...
	for(var s=0; s<ss.snapshotLength; s++) {
		...
		if( parseInt(m[1]) >= this.options.mapLevelBegin  && parseInt(m[1]) <= this.options.mapLevelEnd ){
