...
var PGStartTime=new Date();
var PGtoolversion="V0.1";
var PGtoolDBversion="V0.1";
var chlgsplit;
...
//the real code
addPGheader = function addPGheader() {
...
var PGEndTime=new Date();
var PGExecutionTime=PGEndTime-PGStartTime;

tds[1].innerHTML = "Pre *Game CP " + PGtoolversion + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + tds[1].innerHTML;
...
{
if (name.match("Photo Gallery")=="Photo Gallery") return true;
}

PreGameClicked = function PreGameClicked()
{

if (MY_getValue("PG.auto")=="true") automode=true
  else automode=false;
...
if (automode)
  {
  automode=false;
  MY_setValue("PG.auto","false");
  automodetxt="off";
  }
  else
    {
    automode=true;
    MY_setValue("PG.auto","true");
    automodetxt="on";
    }

link=document.getElementById("PreGame");
link.innerHTML="PreGame " + automodetxt; // +" (" + playername +")" ;
...
if ((newchlgstatus=="none")&&(value=="CLOSED")) return "Excluded";
...
if (chlgstatus=="DAILY CHAT") return "Orange";
if (chlgstatus=="Daily Chat") return "Orange";
if (chlgstatus=="Photo Gallery") return "Orange";
...
if (chlgstatus=="CHAT") return "Important Information for the group.";
if (chlgstatus=="Chat") return "Important Information for the group.";
if (chlgstatus=="INFO") return "Important Information for the group.";
if (chlgstatus=="GAME") return "Extra games to play.";
if (chlgstatus=="Photo Gallery") return "Photo Gallery.";
...
if (newchlgstatus=="Photo Gallery") anchor.innerHTML= '[SHOW_ROOM]';
...
if (newchlgstatus=="ErrExclPlay")  //ErrExclPlay
 {
 wanchor=thisdocument.getElementById("PreGameStatusDiv");
 wanchor.style.display='block';
 wanchor=thisdocument.getElementById("PreGameStatus");
 ...
 */            }
		   ...
		   if (chlgname.match("VOTE")=="VOTE")
             ...
             //if (!inbrackets)
               //{
               if (((photosposted==2)&&(chlgnum=='15'))&&(lastvote.match('5')=="5"))
                 newchlgstatus="Finished";
               if (((photosposted==2)&&(chlgnum=='14'))&&(lastvote.match('5')=="5"))
                 newchlgstatus="Finished";
               if (((photosposted==2)&&(chlgnum=='13'))&&(lastvote.match('5')=="5"))
                 newchlgstatus="Finished";
	   ...
	   if ((newchlgstatus=="none")&&(photosposted==20)&&(chlgnum=='XXX')&&(chlgname.match("OPEN")=="OPEN")) newchlgstatus="Filled";
           ...
           var updanchor=thisdocument.getElementById("PG."+thread);
           
           var sdata=newchlgstatus+"-!**!-"+commentcounter+"-!**!-"+thread+"-!**!-"+chlgstatuscode;
           //GM_log(sdata);
           MY_setValue("PG."+chlgnum,sdata);
...
for (i=1;i<trs.length;i++) {
  ...
  if (nonchallenge3(chlgname))  chlgstatus="Photo Gallery";
   
  mncommentcounter=tds[2].innerHTML;

  // add statusses
  myanchor=thisdocument.createElement('a');
  myanchor.id="PG."+thread;
  ...
  if (chlgstatus=="UPDATING") 
    {
     ...
     lfullstatus=MY_getValue('PG.'+chlgnum);
	 
	   if (MY_getValue('PG.'+chlgnum)==undefined) loadneeded=true;
	     else 
	     {
...
if (env=="none") 
  {
  alert('PreGame tool is only supported under Greasemonkey or IE7PRO');
  return
  }
...
if (MY_getValue("PG.auto")==undefined) MY_setValue("PG.auto","true");
if (MY_getValue("PG.lastloadtime")==undefined) MY_setValue("PG.lastloadtime",PGStartTime.getTime().toString());
...
lastloadtime=MY_getValue("PG.lastloadtime");
elapstime=PGStartTime.getTime()-lastloadtime;
if (elapstime > 1000*60*3) //more then 3 minutes force reload 
  {
  forcereload=true;
  MY_setValue("PG.lastloadtime",PGStartTime.getTime().toString());
  }  
...
myanchor=thisdocument.createElement('a');
myanchor.innerHTML="PreGame "+ automodetxt; // +" (" + playername +")" ;
myanchor.href=location;
myanchor.id="PreGame";
myEventListener(myanchor,'click',PreGameClicked,false);
Docp[0].appendChild(myanchor);

mydiv=thisdocument.createElement('div');
mydiv.style.display='none';
mydiv.id="PreGameStatusDiv";
Doctds[1].appendChild(mydiv);

myanchor=thisdocument.createElement('p');
myanchor.id="PreGameStatus";
myanchor.innerHTML="PreGame statusfield";
