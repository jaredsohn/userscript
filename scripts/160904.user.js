...
var SCRIPT = {
	name: 'The Brethren Krewe 1.2',
	namespace: 'DavyJonesLocker',
	source: 'http://userscripts.org/scripts/show/160904',
	identifier: 'http://userscripts.org/scripts/source/160904.user.js',
	meta: 'http://userscripts.org/scripts/source/160904.meta.js',
	version: '1.2.0',
	date: '2013-03-06',
};
...
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=5046466&page=1';
// var postId = '46941133';
...
var listVersion = 0;
var listNames = ['family', 'admiral', 'captain', 'quartermaster', 'cockswain', 'pirate', 'landlubber', 'walkedtheplank'];
var list = {
	family: {
		member: {},
	...
	walkedtheplank: {
		...
		style: 'color: black; background-color: #a03c19; padding: 0px 3px 0px 3px;',
		...
		if (pref.indexHilite.val) {
			$('#content table.forums span.last_post_user').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html(parts[0] + '<br><b style="color: red;">' + parts[1] + '</b>');
				}
			});
			$('#threads span.thread_starter').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html('Started by: <b style="color: red;">' + name + '</b>');
				}
			});
			$('#threads span.last_post_user').each(function() {
				...
				var id = nameToId[name];
				if (list.family.member[id]) {
					$(this).html('<b style="color: red;">' + name + '</b>');
				}
			});
		}
		break;
	...
	(function() {
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			$('#content div.content_container tbody:eq(0) tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				++members;
				if (list.walktheplank.member[id]) {
					++memberWalked The Plank;
					console.warn('PlankWalker agent found on forum access list: ' + id + ' ' + realName + ' (listed as ' + names[id] + ')');
					delete names[id];
					names.num--;
				} else if (list.landlubber.member[id] || list.pirate.member[id] || list.cockswain.member[id] || list.quartermaster.member[id] || list.captain.member[id] || list.admiral.member[id] || list.krewe.member[id]) {
					if (list.family.member[id]) {
						++membersWithTitles;
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			var nonMemberKrewe = 0;
			$.each(names, function(id, name) {
				if (!(id = parseInt(id))) return;
				else if (list.krewe.member[id]) {
					++nonMemberKrewe;
					$('#fam_list_krewe a[href$="/new_message.pl?to=' + id + '"]').after(') (<a href="/game/forum_private_access.pl?forum_id=1107&action=Invite&user_name=' + escape(name) + '"><b style="color: #008787;">invite</b></a>');
				} else if (list.admiral.member[id]) {
			...
			if (membersWithTitles) {
				statusMsg += prefix + '<b style="color: black;">' + membersWithTitles + ' with titles</b>';
				prefix = ', ';
			}
			if (membersAdmiral) {
				statusMsg += prefix + '<b style="color: blue;">' + membersAdmiral + ' admirals</b>';
				prefix = ', ';
			}
			if (membersCaptain) {
				statusMsg += prefix + '<b style="color: blue;">' + membersCaptain + ' captains</b>';
				prefix = ', ';
			}
			if (membersQuartermaster) {
				statusMsg += prefix + '<b style="color: blue;">' + membersQuartermaster + ' quartermasters</b>';
				prefix = ', ';
			}
			if (membersCockswain) {
				statusMsg += prefix + '<b style="color: blue;">' + membersCockswain + ' cockswains</b>';
				prefix = ', ';
			}
			if (membersPirate) {
				statusMsg += prefix + '<b style="color: red;">' + membersPirate + ' pirates</b>';
				prefix = ', ';
			}
			if (membersLandlubber) {
				statusMsg += prefix + '<b style="color: green;">' + membersLandlubber + ' landlubbers</b>';
				prefix = ', ';
			}
			if (membersWalked The Plank) {
				statusMsg += prefix + '<b style="color: pink; background-color: #a03c19; padding: 0px 3px 0px 3px;">' + membersWalked The Plank + ' plankwalkers</b>';
				prefix = ', ';
			}
			if (unknown.length) {
				statusMsg += prefix + ' <b style="color: yellow;">' + unknown.length + ' unknown</b>';
				prefix = ', ';
			}
			statusMsg += ')';
			if (nonMemberKrewe) {
				statusMsg += ', ' + nonMemberKrewe + ' non-member Krewe';
			}
...
function setupPrefs() {
	...
	function prefMakeDivAll() {
		var out = '<div id="krewe_prefs_' + numPrefDivs + '" style="color: black; float: right; height: 22px; margin: 6px 1px 6px 0px;">'; // class: page_selectors
...
//		setupPrefs();
	...
	$('div[id^="krewe_prefs_"] input').change(prefSet);
}
