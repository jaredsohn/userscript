...
function mainPage() {
  ...
  $("a[title='Comments']").each(function(i) {
	...
	if (num_new_comments > 0) {
	  $('#header').append('<a href="' + link.href + '" style="color: ' + settings['colorNew'] + '">' + (num_comments - old_comments) + '</a> ')
	} else {
	  $('#header').append('0 ')
	}
      }
...
//  GM_log(quorum);
  $("div[class='comment']").
	...
	$(cbody).find("img").each(
	  function(j) {
	    var icon = this.getAttribute('alt')
	    if(icon == 'for' || icon == 'against' || icon == 'imperial' || (emperor == name && icon == 'veto')) {
	      votes[name] = icon;
	      if (settings['debug']) {
	         GM_log('In vote-counter: vote #' + j + ': ' + votes[name]);
		   }
	    }
	  });
      });
