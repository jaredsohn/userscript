// ==UserScript==
// @name		Scripts_SpacesWars_Corrigé
// @namespace	none
//
...
var nbUnis			= infos_version.nbUnis;
	...
	if (page == "leftmenu")
	{
		...
		var gm_icon = build_node(	"div", ["class", "style"], ["lm_lang", "float:right; margin-right:5px;"],
									"<a href='"+configLink+"?lang="+lang+"&uni="+get_infos_from_page().universe+"' target='Hauptframe' title='Scripts_SpacesWars_Corrigé'>"
	...
	if (can_load_in_page("RConverter") && infos_scripts.RConverter )
	{
		...
		if(rapport[rapport.length-3].getElementsByClassName('space0')[0]!=null){
			resultat_combat+="  "+rapport[rapport.length-3].getElementsByClassName('space0')[0].innerHTML.replace(/<font color=\"#7BE654\">/g,'[b][size=120][color=#C03000]').replace(/<\/font>/g,'[/b][/size][/color]')
		}
		var resultat_CDR = rapport[rapport.length-2].getElementsByClassName('space0')[2].innerHTML.replace(/<font color=\"#7BE654\">/g,'[b][size=120][color=#7BE654]').replace(/<\/font>/g,'[/b][/size][/color]');
		var renta_attaquant = rapport[rapport.length-2].getElementsByClassName('space0')[3].innerHTML.replace(/<font color=\"#7BE654\">/g,'[b][size=120][color=#7BE654]').replace(/<\/font>/g,'[/b][/size][/color]').replace(/<font color=\"#DB5656\">/g,'[b][size=120][color=#DB5656]').replace(/<\/font>/g,'[/b][/size][/color]').replace(/<br>/g,'\n');
		var renta_defenseur = rapport[rapport.length-2].getElementsByClassName('space0')[4].innerHTML.replace(/<font color=\"#7BE654\">/g,'[b][size=120][color=#7BE654]').replace(/<\/font>/g,'[/b][/size][/color]').replace(/<font color=\"#DB5656\">/g,'[b][size=120][color=#DB5656]').replace(/<\/font>/g,'[/b][/size][/color]').replace(/<br>/g,'\n');;
	...
	if (can_load_in_page("AllinDeut") && infos_scripts.AllinDeut )
	{
		...
		for (var i=0; i<doms.length; i++)
		{
			...
			doms[i].appendChild(build_node("div", [], [], "<font color='lime'>AllinDeut</font> : "+get_slashed_nb(""+parseFloat(inDeut))));
		}
