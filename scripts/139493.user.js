...
// @author 		  Bigpetroman
// @include       http://www.neobux.com/forum/?/1/245999/*
// @version       1
// ==/UserScript==
		...
		for(var j=0; j<EBP_SubMensaje.length; j++)
		{
			...
			switch(j)
			{
				...
				case 2: //obtenemos la membresia y la fecha
					...
					ebp_Fecha = ebp_Fecha.replace(ebp_Text_alas, '');
				...
				case 6: //obtenemos el número del post y los datos reportados por los usuarios
					...
					ebp_FechaAdPrize = EBP_SubMensaje_Indiv[EBP_SubMensaje_Indiv.length-1];
					ebp_FechaAdPrize = ebp_FechaAdPrize.replace(/'/g, '');
					ebp_FechaAdPrize = ebp_FechaAdPrize.replace(/]/g, '');
					if(ebp_FechaAdPrize != 0)
					{
						ebp_TipoAct = 1;
					}else{
						ebp_TipoAct = 3;
...
function EBP_Obtiene_Datos(MyTexto)
{
	...
	for(var i=0; i<MyTextDivide.length; i++)
	{
		...
		if((sTextLinea.indexOf('neopoints canjeados:') == 0 ) && (b_ebp_AdPrize == false))
		{
			...
			if((sTextLinea.indexOf('neopoints:') == 0) && (b_ebp_FechaAdPrize == false))
			{
				b_ebp_FechaAdPrize = true;
