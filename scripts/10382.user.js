...
function addButtonToMenu()
{
	if(document.getElementById('button_bar'))
	{
		...
		divbut = buttonBar.appendChild(document.createElement('a'));
		divbut.setAttribute('style','height:24px;');
		divbut.href = "#reply";
		commentedButton = divbut.appendChild(document.createElement('img'));
...
function setCommented(commented)
{
	if(commented)
	{
		commentedButton.src = commentedButtonImg;
		commentedButton.alt = "Yay! You've commented on this photo!"
	}
	else
	{
		commentedButton.src = notCommentedButtonImg;
...
function getPhotoComments()
{
	...
	for(var i = 0; i < list.length && user_name == ""; i++)
	{
		if(list[i].href == "http://" + window.location.hostname + "/account")
		{
			user_name = list[i].innerHTML;
		}
	}
	...
	if(user_name != "")
	{
		...
		GM_xmlhttpRequest({
		method:"GET",
		url: "http://api.flickr.com/services/rest/?method=flickr.photos.comments.getList&api_key=c493f3861b5354fb7a953e02fedf6a21&photo_id=" + photo_id,
		onload:function(details) {
			if(details.responseText.indexOf("authorname=\"" + user_name + "\"") != -1)
			{
				setCommented(true);
				var permalink = details.responseText.substring(details.responseText.indexOf("permalink=\"",details.responseText.indexOf("authorname=\"" + user_name + "\""))+11,details.responseText.indexOf("\"",details.responseText.indexOf("permalink=\"",details.responseText.indexOf("authorname=\"" + user_name + "\""))+11));
				divbut.href = permalink;
			}
			else
				setCommented(false);
