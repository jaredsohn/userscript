// External Script. H 1327797794 1327796936
/*****************************************************************************\
...
(function () {
					...
					if (motherBloc) {
						...
						for (i = 0; i < motherBloc.length; i += 1) {
							if (motherBloc[i].name === name) {
								motherBloc[i].checked = true;
								targets.push(motherBloc[i]);
							}
						}
					} else {
						targets = document.getElementsByName(name);
						if (!confirm("Êtes vous sur de vouloir " + command + " tous les sujets sélectionnés ?")) {
							targets = [];
						}
					}
					if (targets.length > 0) {
	...
	shar.kick_user.init = function init(mother, loc) {
		...
		if (!this.mother) {
			this.mother = mother;
			this.checkLoc(loc);
			this.mother.setSecondOccurAt(2 * getTimerLength());
		} else {
	...
	shar.forums[0].autoReload = function autoReload(sujet, loc) {
		...
		if (loc.url.search.hasOwnProperty("reload")) {
			this.noFollow(sujet.parentNode);
			this.noFollow(document.getElementById("col2"));
	...
	shar.forums[0].noFollow = function noFollow(bloc) {
		var a, i;
		if (bloc) {
			a = bloc.getElementsByTagName("a");
	...
	shar.forums[1].checkLoc = function checkLoc(loc) {
		var pass, key, list, command, ancre, motif, bloc, ul, data, url, tracker, pseudo, date, go, delay;
		key = loc.url.search[this.mother.name("key")];
		...
		if (key && command) {
			...
			if (ul) {
				...
				list = command.split("/");
				delay = (option("kick_delay") < 0 || date < option("kick_delay"));
				if (pass) {
					while (list.length > 0) {
						command = list.pop();
						go = false;
...
//			child.setAttribute("onclick", "");
		...
		if (ico !== null && date !== null && nb !== null) {
			...
			if (ico === "topic_marque_on.gif" || ico === "topic_marque_off.gif") {
				...
				if (target !== null) {
					...
					this.mother.system.save.unload(key);
	...
	shar.sujets.react = function react(warn, data) {
		var groupe, list, action, shooter, script, shoot, pseudo, motif, date;
		pseudo = this.get("pseudo", data);
		groupe = this.get("groupe", data);
		date = new Date().getTime() - this.mother.messages.get("time", data);
			...
			if (action) {
				...
				while (list.length > 0) {
					...
					if (action === "supprimer") {
						...
						if (shooter) {
							if (parseInt(data.id, 10) !== data.loc.topic) {
