...
// 2008-09-19: Return to work on the script. Added event listener due to report of Opera incompatibility from operauser, resulting in all initialization stuff being moved to an init function at the top of the script.
// 2009-01-23: Improvement for Image Expansion. New Discussion Map feature. Quick Sage's form clearing bug with Firefox investigated, but not resolved. Liked 420chan's more compact options panel, so shrunk down its padding. Tooltips for Options Panel. Basic Discussion Map function confirmed functional on all boards of (personal) interest.
...
/* Options Section
   Note to self:
   Make functions or object to automate creating new options.
*/
			...
			var partialImageExpansionLimitOptionText2 = document.createTextNode(" Pixels Wide");
			...
			var partialImageExpansionHeightLimitOptionText1 = document.createTextNode("Expand To ");
			var partialImageExpansionHeightLimitOptionField = document.createElement('input');
			var partialImageExpansionHeightLimitOptionText2 = document.createTextNode(" Pixels High");
			...
			var threadNavOptionText = document.createTextNode(" Enable Quick Thread Navigation");
			...
			var discussionMapOptionText = document.createTextNode(" Enable Discussion Map");
	...
	{ 
		...
		if (img.getAttribute("class") != "thumb expanded") 		// If we are expanding...
		{
			...
			else
			{
				...
				if (grabbedThumbnailMsgs.snapshotLength > 0)
				{
					grabbedThumbnailMsgs.snapshotItem(grabbedThumbnailMsgs.snapshotLength - 1).textContent = "Full image shown.";
				}
			}
...
//			img.style.opacity = ".5";
			...
			if (grabbedThumbnailMsgs.snapshotLength > 0)
			{
				grabbedThumbnailMsgs.snapshotItem(grabbedThumbnailMsgs.snapshotLength - 1).textContent = "Thumbnail displayed, click image for full size.";
			}
		}
	 }
...
function to64Bit(input)
{
	var keyBase64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
	var output = "";
	while (input > 0)
	{
		output = keyBase64.charAt(input & 0x3F) + output;
		input = input >> 6;
	}
	return output;
}
...
function addThreadHiding(threadDiv)
{
	...
	var allElementsFollowing = document.evaluate("following-sibling::"+ ((window.location.href.indexOf('nanoha.chupatz.com/') != -1 || window.location.href.indexOf('iichan.ru/') != -1 || window.location.href.indexOf('deltaxxiv.org/') != -1) ? '' : ns) + "node()",newLink,nsResolver,XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,null);
	...
	var threadMatch = new RegExp (','+to64Bit(threadNum).replace(/\x2B/g, "\\x2B")+',');
	...
	if (threadMatch.test(wkExtHiddenThreads))
...
function toggleThreadDisplay(threadNumber, toggleLink, calledByCookie)
{
	...
	if (toggleLink.getAttribute('class') == 'hidethreadlink') // If the thread was not hidden (and will now be toggled hidden)...
	{
		...
		if (!calledByCookie)
		{
			...
			var wkExtHiddenThreadsKey = 'wkExtHiddenThreads_'+boardPath;
			var wkExtHiddenThreadsOld = get_cookie(wkExtHiddenThreadsKey);
			var wkExtHiddenThreadsNew = to64Bit(threadNumber)+','+wkExtHiddenThreadsOld;
			set_cookie(wkExtHiddenThreadsKey,wkExtHiddenThreadsNew.substr(0, wkExtHiddenThreadsOld.length),365); // fail safe
			set_cookie(wkExtHiddenThreadsKey,wkExtHiddenThreadsNew,365);
		}
	}
	else // If the thread was already hidden...
	{
		...
		var wkExtHiddenThreads = ','+get_cookie('wkExtHiddenThreads_'+boardPath);
		var threadMatch = new RegExp (','+to64Bit(threadNumber).replace(/\x2B/g, "\\x2B")+',', "g");
		wkExtHiddenThreads = wkExtHiddenThreads.replace(threadMatch,',');
		set_cookie('wkExtHiddenThreads_'+boardPath,wkExtHiddenThreads.substr(1),365); //
	}
}
...
function addReplyNotes()
{
	...
	function addChildrenView(currentReply, currentBox)
	{
		...
		replyNotice.textContent = " "+threadMap[currentReply].length+((threadMap[currentReply].length > 1) ? " references" : " reference (>>"+threadMap[currentReply][0]+")")+" \u2192";
	...
	function togglePostChildrenView(parentNumber, postsContainer)
	{
		...
		else						// Otherwise, set up the container and add in each reply post.
		{
			...
			var headerText = document.createTextNode("References to No."+parentNumber);
