...
// @include        http://www.gaiaonline.com/marketplace/mystore/sell/?item_id=*&item_param=*&item_param2=*
// ==/UserScript==
...
function warn(ele,LMP,sellVal){
	if (!isNaN(sellVal) && LMP>0){
		if(sellVal<LMP-LMP*.25){
			if (sellVal<=LMP/2){
				ele.textContent=addCommas(sellVal)+" gold is way too low!";
			}
			else{
				ele.textContent=addCommas(sellVal)+" gold is too low!";
			}
		}
		else if(sellVal>LMP+LMP*.25){
			if (sellVal>=LMP*2){
				ele.textContent=addCommas(sellVal)+" gold is way too high!";
			}
			else{
				ele.textContent=addCommas(sellVal)+" gold is too high!";
			}
		}
		else{
