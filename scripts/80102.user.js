...
// @description   Убирает рекламу и ненужные разделы (настраивается), позволяет загружать и изменять размер видео, отображает все комментарии на странице видео, расширяет описание, может предотвращать автозапуск и автозагрузку видео, добавляет выбор HD (High Definition) видео и т.д.
...
/*
	Author: Mindeye
	Translated into Russian: Areopagit (http://userscripts.org/users/155252)
...
var YTF_DOWNLOAD_TOOLTIP_DATA_STRING = "Видео: %v%?{%3|true| 3D|} %wx%h, Аудио: %a %z КГц";
...
var playerThemesArray = ["Темная", "Светлая"];
...
// Adds support for standard Greasemonkey functions/objects if they aren't defined

// Defines unsafeWindow (the unwrapped version of window) as a reference of window
if ((typeof unsafeWindow !== "object") || (unsafeWindow === null)) {
	var unsafeWindow = window;
}

// Defines GM_registerMenuCommand as a function that doesn't do anything
...
if (typeof GM_addStyle !== "function") {
	GM_addStyle = function(cssString) {
		var styleEl = createNode("style", {type: "text/css"}, {textContent: cssString});
		document.getElementsByTagName("head")[0].appendChild(styleEl);
	};
}

// Tries to log the passed string to the error console
if (typeof GM_log !== "function") {
	GM_log = function(logString) {
		try {
...
// Returns the stored value of the passed config key (with a "ytf" prefix), or the passed default value if it doesn't exist
// If localStorage or native JSON aren't available, the default value is returned
if (typeof GM_getValue !== "function") {
	GM_getValue = function(configKey, configDefaultValue) {
		if ((window.localStorage) && (window.JSON)) {
			var configValue = localStorage.getItem("ytf" + configKey);
...
// Stores the passed config key (with a "ytf" prefix) and value
// If localStorage or native JSON aren't available, it does nothing
if (typeof GM_setValue !== "function") {
	GM_setValue = function(configKey, configValue) {
		if ((window.localStorage) && (window.JSON)) {
			localStorage.setItem("ytf" + configKey, JSON.stringify(configValue));
		}
	};
}

// Adds support for some array generics if they aren't available (Javascript 1.6 additions)
["indexOf", "lastIndexOf", "filter", "forEach", "every", "map", "some", "slice"].forEach(function(funcName) {
	if (typeof Array[funcName] !== "function") {
		Array[funcName] = function(arraylikeObj) {
...
function scriptConfiguration(evt) {
	...
	function createDialog() {
		...
		confMask = createNode("div", {id: "ytf-conf-mask", title: "Нажмите здесь, чтобы вернуться на страницу"}, null, [["click", destroyDialog, false]]);
		confDialog = createNode("div", {id: "ytf-conf-dialog"},
			{innerHTML: "<div id='ytf-conf-title'>Настройка YousableTubeFix</div>" +
				"<div id='ytf-conf-tabbar'>" +
				"<input id='ytf-conf-tab-watch' type='button' value='Просмотр' data-ytf-conf-tab-index='0'>" +
				"<input id='ytf-conf-tab-channel' type='button' value='Страница каналов' data-ytf-conf-tab-index='1'>" +
				"<input id='ytf-conf-tab-search' type='button' value='Страница поиска' data-ytf-conf-tab-index='2'>" +
				"<input id='ytf-conf-tab-all' type='button' value='Все страницы' data-ytf-conf-tab-index='3'>" +
				"</div>" +
				"<div id='ytf-conf-content'>" +
				"<div id='ytf-conf-tab-watch-content' data-ytf-conf-tab-index='0'>" +
				"<ul>" +
				"<li>" +
				"<span>Размер видео:</span>" +
				...
				"</select>" +
				"<input type='text' id='ytf-conf-size-custom' value='" + ((isNaN(resizeNumericValue)) ? "' disabled='disabled'" : resizeNumericValue + "'") + " title='Целое или десятичное число (множитель размера). Например, введите 2, чтобы удвоить размер видео'>" +
				"</li>" +
				"<li>" +
				"<span>Шаблон для имен загруженных файлов:</span>" +
				"<input type='text' id='ytf-conf-filename-save-pattern' value='" + scriptConfig.filenameSavePattern + "' title='/t - название видео, /u - имя пользователя видео, /i - id видео, /f - формат видео, /w - горизонтальный и /h - вертикальный размер видео (в пикселях)'>" +
				"</li>" +
				"<li><span>Удалить (используйте Ctrl+клик / Shift+клик для выбора нескольких):</span></li>" +
				"<select id='ytf-conf-remove-disable-sel' multiple='multiple' size='5'>" +
				"<option id='ytf-conf-remove-video-information'" + ((scriptConfig.removeVideoInformation) ? " selected='selected'" : "") + ">- Информация о видео</option>" +
				"<option id='ytf-conf-remove-actions'" + ((scriptConfig.removeActions) ? " selected='selected'" : "") + ">- Раздел действий</option>" +
				"<option id='ytf-conf-remove-related-videos'" + ((scriptConfig.removeRelatedVideos) ? " selected='selected'" : "") + ">- Раздел связанных видео</option>" +
				"<option id='ytf-conf-remove-comments'" + ((scriptConfig.removeComments) ? " selected='selected'" : "") + ">- Раздел комментариев</option>" +
				"<option id='ytf-conf-remove-video-responses'" + ((scriptConfig.removeVideoResponses) ? " selected='selected'" : "") + ">- Раздел ответов на видео</option>" +
				"<option id='ytf-conf-remove-header'" + ((scriptConfig.removeHeader) ? " selected='selected'" : "") + ">- Раздел заголовка</option>" +
				"<option id='ytf-conf-remove-footer'" + ((scriptConfig.removeFooter) ? " selected='selected'" : "") + ">- Раздел нижнего колонтитула</option>" +
				"<option id='ytf-conf-remove-subscription-reminder'" + ((scriptConfig.removeSubscriptionReminder) ? " selected='selected'" : "") + ">- Всплывающее напоминание о подписке</option>" +
				"</select>" +
				"<li><span>Автоматически расширять:</span></li>" +
				"<input type='checkbox' id='ytf-conf-expand-video'" + ((scriptConfig.expandVideo) ? " checked='checked'" : "") + "><label for='ytf-conf-expand-video'>Окно видео</label><br>" +
				"<input type='checkbox' id='ytf-conf-expand-info'" + ((scriptConfig.expandInfo) ? " checked='checked'" : "") + "><label for='ytf-conf-expand-info'>Информация о видео</label><br>" +
				"<li><span>Автозапуск и автозагрузка видео:</span></li>" +
				"<input type='radio' id='ytf-conf-autoplay-prevent-neither' name='ytf-conf-autoplay-choice' value='0'" + ((scriptConfig.autoplayMode === 0) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-prevent-neither'>Не предотвращать автозапуск и автозагрузку</label><br>" +
				"<input type='radio' id='ytf-conf-autoplay-prevent-autoplay' name='ytf-conf-autoplay-choice' value='1'" + ((scriptConfig.autoplayMode === 1) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-prevent-autoplay'>Предотвращать только автозапуск</label><br>" +
				"<input type='radio' id='ytf-conf-autoplay-prevent-both' name='ytf-conf-autoplay-choice' value='2'" + ((scriptConfig.autoplayMode === 2) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-prevent-both'>Предотвращать автозапуск и автозагрузку</label><br>" +
				"<input type='checkbox' id='ytf-conf-override-autoplay-playlists'" + ((scriptConfig.overrideAutoplayPlaylists) ? " checked='checked'" : "") + "><label for='ytf-conf-override-autoplay-playlists'>Отменить при воспроизведении плейлиста</label><br>" +
				"<input type='checkbox' id='ytf-conf-override-autoplay-fg-tab'" + ((scriptConfig.overrideAutoplayFgTab) ? " checked='checked'" : "") + ((!isVisibilitySupported) ? " disabled='disabled' title='Ваш браузер не поддерживает данную функцию'" : "") + "><label for='ytf-conf-override-autoplay-fg-tab'" + ((!isVisibilitySupported) ? " title='Ваш браузер не поддерживает данную функцию'" : "") + ">Отменить в активной вкладке</label><br>" +
				"<li>" +
				"<input type='checkbox' id='ytf-conf-scroll-to-video'" + ((scriptConfig.scrollToVideo) ? " checked='checked'" : "") + "><label for='ytf-conf-scroll-to-video'>Прокручивать страницу до окна видео</label>" +
				"</li>" +
				"<li>" +
				"<span>Качество видео по умолчанию:</span>" +
				...
				"<ul>" +
				"<li><span>Автозапуск и автозагрузка видео:</span></li>" +
				"<input type='radio' id='ytf-conf-autoplay-channel-prevent-neither' name='ytf-conf-autoplay-channel-choice' value='0'" + ((scriptConfig.channelAutoplayMode === 0) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-channel-prevent-neither'>Не предотвращать автозапуск и автозагрузку</label><br>" +
				"<input type='radio' id='ytf-conf-autoplay-channel-prevent-autoplay' name='ytf-conf-autoplay-channel-choice' value='1'" + ((scriptConfig.channelAutoplayMode === 1) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-channel-prevent-autoplay'>Предотвращать только автозапуск</label><br>" +
				"<input type='radio' id='ytf-conf-autoplay-channel-prevent-both' name='ytf-conf-autoplay-channel-choice' value='2'" + ((scriptConfig.channelAutoplayMode === 2) ? " checked='checked'" : "") + "><label for='ytf-conf-autoplay-channel-prevent-both'>Предотвращать автозапуск и автозагрузку</label><br>" +
				"<input type='checkbox' id='ytf-conf-channel-override-autoplay-fg-tab'" + ((scriptConfig.channelOverrideAutoplayFgTab) ? " checked='checked'" : "") + ((!isVisibilitySupported) ? " disabled='disabled' title='Ваш браузер не поддерживает данную функцию'" : "") + "><label for='ytf-conf-channel-override-autoplay-fg-tab'" + ((!isVisibilitySupported) ? " title='Ваш браузер не поддерживает данную функцию'" : "") + ">Отменить в активной вкладке</label><br>" +
				"<li>" +
				"<span>Качество видео по умолчанию:</span>" +
				...
				"<ul>" +
				"<li>" +
				"<input type='checkbox' id='ytf-conf-download-from-search'" + ((scriptConfig.downloadFromSearch) ? " checked='checked'" : "") + "><label for='ytf-conf-download-from-search'>Добавлять прямые ссылки для скачивания</label>" +
				"</li>" +
				"</ul>" +
				"</div>" +
				"<div id='ytf-conf-tab-all-content' data-ytf-conf-tab-index='3'>" +
				"<ul>" +
				"<li><span>Удалить или отключить:</span></li>" +
				"<input type='checkbox' id='ytf-conf-remove-ads'" + ((scriptConfig.removeAds) ? " checked='checked'" : "") + "><label for='ytf-conf-remove-ads'>Рекламные объявления</label><br>" +
				"<input type='checkbox' id='ytf-conf-remove-default-language-box'" + ((scriptConfig.removeDefaultLanguageBox) ? " checked='checked'" : "") + "><label for='ytf-conf-remove-default-language-box'>Блок языка по умолчанию</label><br>" +
				"<input type='checkbox' id='ytf-conf-remove-watermark'" + ((scriptConfig.removeWatermark) ? " checked='checked'" : "") + "><label for='ytf-conf-remove-watermark'>Водяной знак на видео</label><br>" +
				"<input type='checkbox' id='ytf-conf-disable-annotations'" + ((scriptConfig.disableAnnotations) ? " checked='checked'" : "") + "><label for='ytf-conf-disable-annotations'>Аннотации видео (отключить)</label><br>" +
				"<li><span>Скрыть ссылки для скачивания:</span></li>" +
				"<input type='checkbox' id='ytf-conf-formats-hide-3d'" + ((scriptConfig.formatsHide3D) ? " checked='checked'" : "") + "><label for='ytf-conf-formats-hide-3d'>Ссылки на 3D видео</label><br>" +
				"<span>Ниже этого качества:</span>" +
				...
				"</select><br>" +
				"<input type='checkbox' id='ytf-conf-formats-only-best'" + ((scriptConfig.formatsOnlyBest) ? " checked='checked'" : "") + "><label for='ytf-conf-formats-only-best'>Все, кроме самого высокого качества</label><br>" +
				"<li>" +
				"<span>Скрывать панели плеера:</span>" +
				...
				"<option id='ytf-conf-controls-autohide-mode-opt-neither' value='0'" + ((scriptConfig.controlsAutohideMode === 0) ? " selected='selected'" : "") + ">Никакую из панелей</option>" +
				"<option id='ytf-conf-controls-autohide-mode-opt-both' value='1'" + ((scriptConfig.controlsAutohideMode === 1) ? " selected='selected'" : "") + ">Обе панели</option>" +
				"<option id='ytf-conf-controls-autohide-mode-opt-progress' value='2'" + ((scriptConfig.controlsAutohideMode === 2) ? " selected='selected'" : "") + ">Только панель прогресса</option>" +
				"</select>" +
				"</li>" +
				"<li>" +
				"<span>Обложка плеера:</span>" +
				...
				(function() { //'
					return playerThemesArray.map(function(t) {
						return "<option id='ytf-conf-player-theme-opt-" + t + "' value='" + t + "'" + ((scriptConfig.playerTheme === t) ? " selected='selected'" : "") + ">" + t.capitalizeFirstChar() + " тема</option>";
					}).join("");
				})() +
				"</select>" +
				"</li>" +
				"<li>" +
				"<span>Качество Flash Player:</span>" +
				...
				"<input type='button' id='ytf-conf-buttons-ok' value='OK' title='Сохранить текущую конфигурацию'>" +
				"<input type='button' id='ytf-conf-buttons-cancel' value='Отмена' title='Вернуться на страницу без сохранения'>" +
				"</div>"});
	...
	function saveConfiguration() {
		...
		if (vsValValue === null) { // Doesn't continue if the chosen video size is invalid
			window.alert("Неверный размер видео по умолчанию");
			switchToTab(0);
			vsCustomInput.focus();
			return;
		}
	...
	if (!scriptShowMessage) {
		...
		if (messageDiv) {
			...
			messageDiv = createNode("div", {id: "ytf-script-version-message", title: "A new YousableTubeFix version is available"},
				{innerHTML: "Доступна новая версия YousableTubeFix (" + scriptNewVersionDate + ")<br><br>" +
					"<a href='" + scriptHomepageURL + "' target='_blank' title='Перейти на домашнюю страницу YousableTubeFix'>Перейти на страницу</a>" +
					"<a id='ytf-script-version-message-hide' href='javascript:void(null)' title='Скрыть уведомления для этой сессии'>Скрыть</a>"});
...
// Checks if there is a new script version according to the version information provided by a remote version check script inserted into the page
...
function scriptCheckVersion() {
	...
	// Creates a hidden div with a listener for a custom event and appends it to the document body
	var versionCheckDiv = createNode("div", {class: "ytf-hidden", id: "ytf-version-check-div"}, null,
		[["ytfremoteVersionCheck", scriptVersionTest, false]]);
	document.body.appendChild(versionCheckDiv);

	// Inserts the version check script in a script tag. This script will put the remote version in the hidden div and fire the custom event
	var versionCheckScript = createNode("script", {src: scriptVersionCheckJsURL, type: "text/javascript"});
	document.getElementsByTagName("head")[0].appendChild(versionCheckScript);

	// Gets the remote version and checks if the script can be updated
	// It is called by the versionCheckDiv event listener
	function scriptVersionTest(evt) {

		// Removes the listener for the custom event
		versionCheckDiv.removeEventListener("ytfremoteVersionCheck", arguments.callee, false);

		// Parses the remote version from the hidden div's text content, making sure it is a number
		var remoteVersion = parseInt(versionCheckDiv.textContent, 10);
		...
		// Removes the hidden div and the version check script, as they are no longer necessary
		delNode(versionCheckDiv);
		delNode(versionCheckScript);
...
var scriptHomepageURL = "http://userscripts.org/scripts/show/36453";
var scriptVersionCheckJsURL = "http://dl.dropbox.com/u/3311359/YousableTubeFix/ytfOperaVersionCheck.js";
	...
	function scriptWatchAPI() {
		...
		if (!meSt.apiAlreadyCalled) {
			...
			if (contentPanel) {
				...
				var mainResizeButton = createDropdownYTButton("ytf-main-resize-button", "Изменить размер", "Изменить размер видео-плеера", "ytf-main-resize-button-content", "ytf-main-resize-menu");
				...
				if (watchVA.videoFormats.length !== 0) { // Adds the links to the menu if there is at least one VideoFormat in the VideoAdapter
					...
					if (formatsArray.length !== watchVA.videoFormats.length) {
						mainDownloadMenu.appendChild(createNode("span", {class: YT_MENU_ITEM_CLASS + " ytf-main-download-info", title: "Некоторые ссылки были скрыты в настройках"}, {textContent: "Дополнительные ссылки: " + (watchVA.videoFormats.length - formatsArray.length)}));
					}
				}
				else { // Only adds a warning to the menu if there isn't any VideoFormats in the VideoAdapter
					mainDownloadMenu.appendChild(createNode("span", {class: YT_MENU_ITEM_CLASS + " ytf-main-download-warning", title: "Не найдены ссылки для скачивания этого видео"}, {textContent: "Ссылки для скачивание недоступны"}));
				}
				mainDiv.appendChild(mainDownloadMenu);

				// Creates the download button and appends it to the main div
				var mainDownloadButton = createDropdownYTButton("ytf-main-download-button", "Скачать", "Скачать это видео", "ytf-main-download-button-content", "ytf-main-download-menu");
				mainDiv.appendChild(mainDownloadButton);

				// Creates the configuration button and appends it to the main div
				var mainConfigureButton = createNode("button", {class: YT_BUTTON_CLASSES + " " + YT_BUTTON_TOOLTIP_CLASS + " " + YT_BUTTON_TOOLTIP_REVERSE_CLASS, id: "ytf-main-configure-button", role: "button", title: "Настроить YousableTubeFix", type: "button"}, null,
					[["click", scriptConfiguration, false]]);
				mainConfigureButton.appendChild(createNode("span", {class: YT_BUTTON_CONTENT_CLASS, id: "yt-main-configure-button-content"}, {textContent: "Настроить"}));
		...
		if (watchVA.VAType === "Flash") {
			...
			setTimeout(function() {resizePlayer(scriptConfig.videoSize, scriptConfig.scrollToVideo);}, 250); // Uses a 250 miliseconds delay to workaround a Opera/YouTube player bug (the size isn't correctly updated if it is resized inmediately after a reload)			
