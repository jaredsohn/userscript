...
// @description    Highscore para munich
// @include        http://*muenchen.pennergame.de/highscore/*
// ==/UserScript==
...
var siglink = "http://img.muenchen.pennergame.de/cache/signaturen/";
...
function Geldladen(id,x) {
	GM_xmlhttpRequest({
    	method: 'GET',
   	url: 'http://www.muenchen.pennergame.de/dev/api/user.' + id + '.xml',
        ...
        onload: function(responseDetails) {
			...
			GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://www.muenchen.pennergame.de/profil/id:'+ id +'/',
				onload: function(responseDetails) {
					...
					var mendigoOnlineVar = profil.search('title="Online"');
					 if (mendigoOnlineVar != -1) {
						newtd.innerHTML = ' <img src="http://media.pennergame.de/img/on.png" width="8" height="8"> ' + newtd.innerHTML
					...
					try {
						var side_split_3 = side_split_2[0].split('<img style="margin-top:3px" src="http://www.muenchen.pennergame.de/headline/');
					...
					try {
					   ...
					   // var haustier = ' <img style="margin-top:3px" src="http://www.muenchen.pennergame.de/headline/' + tier_ueberschrift +'/?size=28" /><br />'+ haustier_bild + '<br />' + tier_beschreib + tier_tip;
...
else if (cash > min4b && cash <= min5) {
   newtd.style.color = "#FF0000";
   newtd.style.fontWeight = "bolder";
   newtd.style.fontSize = "16px";
}
else if (cash > min5 && cash <= min5b) {
   newtd.style.color = "#FF0000";
   newtd.style.fontWeight = "bolder";
   newtd.style.fontSize = "18px";
   alert('MENDIGO CON PASTA !!');

}
else if (cash > min5b) {
   newtd.style.color = "#FF0000";
   newtd.style.fontWeight = "bolder";
   newtd.style.fontSize = "20px";
   alert('ASPIRANTE A PALACIO !!');
