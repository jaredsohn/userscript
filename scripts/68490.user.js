...
// ==/UserScript==
/*
 * This script is just a translation of the ikariam all in one embassy tool made by Luca Saba ( downloaded at 
 * http://userscripts.org/scripts/source/35189.user.js ) in arabic by yazmat (wich is me :D)
*/
...
var lang={
  ...
  ae: {'newAlert': 'أعضاء جدد', 'newTotal': 'مجموع الأعضاء الجدد', 'aband': 'ترك التحالف', 'totAban': 'مجموع تاركي التحالف', 'confirm': 'هل أنت متأكد من أنك تريد محي النقاط ؟'},
}

var local = 'ae'; //For i18n
...
function showToolConfig(e)
{
  ...
  else
  {
    ...
    div.innerHTML = '<h3 class="header" style="padding-top: 8px; height: 20px; background-image: url(http://s3.ikariam.it/skin/layout/bg_sidebox_header.jpg)"><b>خصائص</b></h3>'; 
    div.innerHTML += '<div style="margin-left: 80px; margin-top: 20px; text-align: left">';
    
    if(tpl == 0) div.innerHTML += '<input id="eToolCfText" style="position: absolute;left: 60px;" type="radio" name="garba" value="0" checked >&nbsp;كتابة&nbsp;<br/>';
    else div.innerHTML += '<input id="eToolCfText" style="position: absolute;left: 60px;" type="radio" name="garba" value="0">&nbsp;كتابة&nbsp;<br/>';
    
    if(tpl == 1) div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1" checked >&nbsp;صور&nbsp;</div><br/>';
    else div.innerHTML += '<input id="eToolCfImages" style="position: absolute;left: 60px;" type="radio" name="garba" value="1">&nbsp;صور&nbsp;</div><br/>';
    
    //GM_log(div.innerHTML);
    div.innerHTML += '<input id="eToolConfButton" type="button" class="button" value="تسجيل">';
...
function getActualData()
{
  ...
  for(i=1; i < membersTab.rows.length - 1; i++)
  {
    ...
    if(typeof(recorded_points[pName]) === 'undefined')
	  {
	    membersTab.rows[i].cells[points_cell_idx].innerHTML += " (<font color='#F00'>جديد</font>)";
  ...
  if(status == 'online')
  {
    switch(tpl)
    {
      case 1:
        ...
        break;
      default:
        rowElement.cells[0].innerHTML="<B><font color='#008800'>متصل</FONT></B>";        
        break;
    }
  }
  else if(status == 'inactive')
  {
    switch(tpl)
    {
      case 1:
        ...
        break;
      default:
        rowElement.cells[0].innerHTML = "<B><font color='#708090' SIZE='1'>خامل ("+lastOnline + ")</FONT></B>";        
        break;
    }
  }
  else if(status == 'offline')
  {
    switch(tpl)
    {
      case 1:
        ...
        break;
      default:
        rowElement.cells[0].innerHTML = "<font color='#F00' SIZE='1'>غير متصل ("+lastOnline+")</FONT>";
        break;
    }
  }
