...
function workoutPage() {
  ...
  setTimeout(handleExporter,timeout);
}
...
function handleExporter() {
  ...
  if (gpxAnch.snapshotLength == 0) {
    ...
    setTimeout(closeExport,timeout)
...
function getAnchorElements() {
  ...
  if (itemInList == snapLinks.snapshotLength) {
    ...
    setTimeout(getAnchorElements,timeout);
  }                 
...
function historyPage() {
  var startTm = document.evaluate("//input[@name='timePeriodControl:startTime']", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
  var endTm = document.evaluate("//input[@name='timePeriodControl:endTime']", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
  GM_log("startTm: " + startTm);
  GM_log("endTm: " + endTm);
  startTm.value = GM_getValue("GM_fromDate",'');
  endTm.value = GM_getValue("GM_toDate",'');
  startTm.onkeyup();
  endTm.onkeyup();
  setTimeout(advanceToStartPage,timeout);
}
...
function handleRetries(callingFunction) {
  ...
  if (retryCounter > 5) {
    ...
    setTimeout(eval(callingFunction),timeout);
  }
}
...
function initScript() {
  ...
  if (status == "init") {
    ...
    if(currentMode == "history") {
      showMessage("Export script running: historyPage");
      setTimeout(historyPage,timeout);
    } else if (currentMode == "workout") {
      t = new Date().getTime();
      t = String(t);
      GM_setValue("GM_lastTimeUsed",t);
      showMessage("Export script running: workoutPage");
      setTimeout(workoutPage,timeout);
    }
  } else {
	resetGMVars();
	alert ("The script is in an unknown state.\nSaved values have been reset.\n\nYou should CLOSE THIS WINDOW and then go to the Endomondo page again.\nSorry for the hassle....");
	return (false);
  }
}
