...
// script link: http://userscripts.org/scripts/show/127574
// original script link: http://userscripts.org/scripts/show/36799
// ver 1.0 @ 2008-4-15
//  Initialize release
// ver 1.1 @ 2008-4-16
...
//     with last version
// ver 2.0.9 @ 2012-3-4 by Marvel.Liu <marvelliu@Gmail.com>
//   * Fixed the bug, work in fx 10
//



//var e = document.createElement("script");
//e.src = 'file:///r:/36799.user.js';
...
var TianyaConfig = function () {
    this.defaultopt = {floatbar   :1,             //Enable Floatbar
                       ...
                       autoclose  :3,             //Auto close the helpbar in 5 secs
...
TianyaHelper.prototype = {
	...
	process: function() {
        ...
        try {
            ...
            var newHTML = newFirstAuthor + $X('id("pContentDiv")[last()]')[0].innerHTML.replace(/<div class="newcontent"[^>]*>/g, '');
            newHTML = newHTML.replace(/(<table[^>]*>)/g, '</div><div class="newcontent">\$1');

            $X('id("pContentDiv")[last()]')[0].innerHTML = newHTML;
 			//$X('id("pContentDiv")[last()]/div[@class="newcontent" and descendant::a[contains(@href, "vwriter")]]').forEach(function (node) {
            $X('id("pContentDiv")[last()]/div[@class="newcontent"]').forEach(function (node) {
            ...
            try {
    			...
    			$X('(id("pContentDiv")|id("AllDelResponse"))/div[@class="item"]/div[@class="vcard"]').forEach(function (node) {

                    var divNode = document.createElement('div');
                    
                    var writer = "";
                    var ws = node.getElementsByTagName('a');
                    
                    for(var i in ws)
                    {
                        if(!ws[i] || !ws[i].href)
                            continue;
                        if(ws[i].href.indexOf("my.tianya")>=0)
                            writer = ws[i].textContent;
                    }
                    ...
                    GM_log("writer:"+writer);
                    divNode.className = 'allwriters writer' + writerId + ' newcontent';
    				...
    				while ( copyNode && copyNode.nodeName != "BR" ) {
                        if (copyNode.nodeName == "DIV") {
                            copyNode.className = 'folder post';
                            if (option.linkpost) {
                    ...
                    //GM_log("parentNode:"+node.parentNode.innerHTML);
                    ...
                    var colNodes = divNode.firstChild.getElementsByTagName('span');
                    if ( option.slideshow ) {
                         colNodes[0].align = "left";
                         colNodes[0].innerHTML = '<a class="updown" href="javascript:void(0)" title="缩进回复" onclick="TYH_folderClickHandle(event);"/>';
                    }
	...
	init: function() {
		if (this.running) {
            ...
            setGlobalStyle('a.updown:link, a.updown:visited { display:block; width:16px; height:16px; outline:none; background:url(data:image/gif;base64,' + gif_up +') no-repeat center center;}' +
                           ...
                           '.vcard {width:100%;display:inline;}' +
...
.mainbox {clear:both;margin-top:-1px;border-top:1px solid #A4CDF2;height:210px;width:160px; background:#EBF0FF;}\
#tabmain div{display:none;color:#1E7ABA;}\
#tabmain div.block{display:block;}\
#tabmain {padding:0px;margin:4px;}\
#tabmain table{padding:0px;margin:0px;border-collapse:collapse;height:600px;width:100%;color:#1E7ABA;}\
#tabmain select,#tabmain textarea {padding:1px;margin:1px;border:none;}\
#tabmain textarea{border:1px solid #A4CDF2;background:transparent;overflow-x:hidden;overflow-y:auto;width:98%;height:140px;font-family:"Lucida Grande","Microsoft Yahei",Verdana,Geneva,Lucida,Helvetica,sans-serif;font-size:12px;color:#1E7ACE;}\
...
#tabmain #tabdiv5 {height:200px; width:100%; overflow-x:hidden; overflow-y:auto; border:none;}\
	...
	init: function () {
        ...
        $X('id("tianya_helpbar")/div/ul/li[last()]')[0].addEventListener('click', function(event) {
            ...
            for (var i in optname) {
                GM_log('.//input[@id="'+optname[i]+'"]:'+Tyconfig.option[optname[i]]);
