...
// @namespace      asgblm_grepoFarmer
// @require http://sizzlemctwizzle.com/updater.php?id=71277
// @include        http://*.grepolis.*/game/map*
// @description    Helps with the farming by showing several additional data. like the increased strength of the current attack, Max loot of the current attack etc, it also
// helps with calculating the necessary troop amount for a given action, this is a modified version, the original script can be found at http://userscripts.org/scripts/show/65877
// the English version can be found at http://userscripts.org/scripts/show/70651, some modifications came from discussions on the original script that can be found at http://userscripts.org/topics/45550
// ==/UserScript==

/* CHANGE-LOG
* 0.12b 
 * - Fixed bug in layout not showing duration counter with new game version 1.13 (revolt icon and towns with breakthrough tech researched icons overlapping the duration counter)
 * - Fixed spelling in description and change-log.
 *
* 0.12
 * - Fixed bug in layout not showing correctly when attacking players
 *
 * 0.11
 * - Changed auto update support to make script shorter.
 * - Changed default command action from Looting to Strengthen as it is used more often.
 ...
 *   
 * 0.10
 * - Added extra information to loot capacity and max loot given the current strength, thanks to Kedor (http://userscripts.org/users/131203)
 * - When clicked on the troops image it will no longer fill the input with the maximum troop number, but instead if it its a strengthening action it will.
		...
		var bindDurationCounter_new = function () {
			if (TownInfo.type === 'farm_town_info') {
				...
				var html =	'<div style="position: absolute; width: auto; right: 107px; top: 235px;">'+
							'<div class="tilx_infos" style="background-image:url(' + images.booty + ');border-width:1px 0 1px 1px;padding-right:5px;width:auto;" id="tilx_res_cont">' +
								'<span id="tilx_res">0</span> ' + 
								'<label><input type="checkbox" id="tilx_booty"> With <em>Booty</em></label>' + 
							'</div>' + 
							'<div class="tilx_infos" style="background-image:url(' + images.mood_drop + ');float:left;width:auto;border-width:0 1px 1px;padding-right:10px;" id="tilx_mood_drop">0</div>' +
							'<div class="tilx_infos" style="background-image:url(' + images.strength_drop + ');border-width:0 0 1px;width:auto;" id="tilx_strength_drop">0</div>' +
							'</div>'+
							'<div style="position: absolute; top: 172px; left: 373px;">'+
							'<div class="tilx_infos" style="background-image:url(' + images.strength + ');margin-top:3px;border-width:1px 1px 0;width:35px;" id="tilx_strength">0</div>' +
