// ==UserScript==
// @name           YouTube: KeepVid redirect button
// @namespace      Aspi
// @description    Adds a button that redirects to KeepVid's YouTube download service for the video you are currently watching.
...
// ==ChangeLog==
// @history        1.08 Updated to be compatible with new YouTube layout.
...
(function () {
	...
	var keepvidURL = 'http://www.keepvid.com/?url=' + location.href, container = document.getElementById('watch7-main'), nextToElement = document.getElementById('watch-dislike');
	
	if ((nextToElement || container) && !document.getElementById('watch-download')) {
		...
		if (nextToElement) {
			...
			btn.className = nextToElement.className;
			
			// Inject download button after last child.
			nextToElement.parentNode.insertBefore(btn, nextToElement.nextSibling);
			
			// Create space between last child and download button.
			nextToElement.style.marginRight = '1em';
		} else {
		...
		if (btn.offsetTop > nextToElement.offsetTop) {
