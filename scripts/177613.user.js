...
// @edit author  eCitizen Earentir
// @link       http://userscripts.org/scripts/show/170806
...
function letsJQuery() {
  ...
  content.prepend('<div id="myStatBoxL" style="z-index:1; position: absolute; top: 300px; left: 10px; width: 160px; opacity: 0.8;"></div>' + '<div id="myOverBox" style="z-index:1; position: absolute; top: 240px; left: 285px; width: 200px; opacity: 0.8;"></div>');
  content.append('<div id="myStatBoxR" style="z-index:100; position: absolute; top: 300px; left: 590px; width: 160px; opacity: 0.8;"></div>');

    $j('div#enemy_defeated').before('<div id="MHPSet" style="position:relative;width:600px;top:-220px;left:70px;clear:both;padding:3px;margin:5px 0;font-weight:bold;color:#000;text-align:center">' + '&nbsp;&nbsp;<input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food">&nbsp;Eat food</label>' + '&nbsp;&nbsp;<input type="checkbox" id="multihit_bazooka" name="multihit_bazooka" checked="checked"><label for="multihit_bazooka">&nbsp;No Bazooka</label>' + '&nbsp;&nbsp;<input type="checkbox" id="multihit_energy" name="multihit_energy"><label for="multihit_energy">&nbsp;Energy Bars</label>' + '<div id="multihit_message" style="padding:2px;color:#ff0000"></div></div>');
    //+  '<button id="heros_update">Update</button>&nbsp;&nbsp;Hero Lists' + '<div id="multihit_message" style="padding:2px;color:#ff0000"></div></div>');    
  
    $j('div#MHPSet').before('<div id="MHP" style="position:relative;top:-380px;width:236px;left:250px;clear:both;padding:3px;margin:5px 0;font-weight:bold;color:#000;text-align:center;z-index:999">' + 'Energy:&nbsp;' + '<input id="multihit_count" style="text-align:right;" type="text" size="3" maxlength="4" value="1000" />' + '&nbsp;X 10&nbsp;' + '<button id="multihit_start">HIT!</button><br />' );
  ...
  unsafeWindow.battleFX.hit = function () {
    ...
    if (multiHitRunning) {
      ...
      $j('div#multihit_message').html('Consumed: ' + multiHitDone * 10 + '&nbsp;&nbsp;Kills: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? 'Finished!' : 'Last Enemy alive!') + ')');
...
console.log ( '1');
      if ( unsafeWindow.globalStop ) {
      	$j('button#multihit_start').html('WAIT');  
        multiHitLoopId = setTimeout("jQuery.fn.multiHIT()", 1*60*1000);
     	
      }
      console.log ( '2');
      console.log ( multiHitCount+' '+multiHitDone); 
    if ( multiHitCount <= multiHitDone) {
...
//    }
      ...
      for (var i = 0; i < 4; i++) {
        ...
        if (typeof bh['stats']['current'][zone][division][att] != 'undefined') {
          ...
          for (var j = 0; j < bh['stats']['current'][zone][division][att].length; j++) {
            ...
            top5ABH[i] += '<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/' + bh.fightersData[bh['stats']['current'][zone][division][att][j].citizen_id].id + '">' + bh.fightersData[bh['stats']['current'][zone][division][att][j].citizen_id].name + '</a></td><td>' + bh['stats']['current'][zone][division][att][j].kills + '</td><td><strong>' + numberWithCommas(bh['stats']['current'][zone][division][att][j].damage) + '</strong></td></tr>';
            aBHTotal += Number(bh['stats']['current'][zone][division][att][j].damage);
          }

          top5ABH[i] += '<tr><td>Total Damage</td><td>--</td><td><strong>' + numberWithCommas(aBHTotal) + '</strong></td></tr>';
        }
        ...
        if (typeof bh['stats']['current'][zone][division][def] != 'undefined') {
          ...
          for (var j = 0; j < bh['stats']['current'][zone][division][def].length; j++) {
            ...
            top5DBH[i] += '<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/' + bh.fightersData[bh['stats']['current'][zone][division][def][j].citizen_id].id + '">' + bh.fightersData[bh['stats']['current'][zone][division][def][j].citizen_id].name + '</a></td><td>' + bh['stats']['current'][zone][division][def][j].kills + '</td><td><strong>' + numberWithCommas(bh['stats']['current'][zone][division][def][j].damage) + '</strong></td></tr>';
            dBHTotal += Number(bh['stats']['current'][zone][division][def][j].damage);
          }

          top5DBH[i] += '<tr><td>Total Damage</td><td>--</td><td><strong>' + numberWithCommas(dBHTotal) + '</strong></td></tr>';
        }
      }
  ...
  console.log ( 'test');
   multiHitCount = 1000;
    //unsafeWindow.jQuery.fn.multiHIT();
	unsafeWindow.$j('button#multihit_start').click();
    //jQuery.fn.multiHIT();
