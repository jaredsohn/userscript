...
var SCRIPT = {
 url: 'http://userscripts.org/scripts/source/67010.user.js',
  version: '1.0.14',
  build: '52',
  name: 'inthemafia',
  appID: 'app10979261223',
  ajaxPage: 'inner2',
  presentationurl: 'http://userscripts.org/scripts/show/67010',
...
var cashMoscowElt; 
var cashBankokElt;              // Moscow cash DOM element
...
if (!initialized) {
  ...
  var cities = new Array(
    ...
    ['BANGKOK', undefined, 140, cashBangkokIcon, 'cashBangkok Icon', 'autoBankBangkok', 'bankConfigBangkok', 'autoSellCratesBangkok', 'B$']
  );
...
function upgradeFightRobTab() {
  ...
  if (isChecked('fightLocationCUBA')) {
    ...
    else if (isChecked('fightLocationBANKOK')) {
    loc = BANKOK;
}
...
function saveDefaultSettings() {
  ...
  GM_setValue('bankConfigBANKOK', '50000');
...
*/
  ...
  var autoBankBangkokOn  = (document.getElementById('autoBankBankok').checked === true);
  ...
  var bankConfigBangkok     = document.getElementById('bankConfigBankok').value;
...
if (autoBankBangkokOn && (isNaN(bankConfigBangkokInt) || bankConfigBangkokInt < 10)) {
  ...
  saveCheckBoxElementArray(['autoClick','autoLog','logPlayerUpdates','hideAttacks',
                            'autoMission','autoBank','autoBankMoscow','autoBankBangkok','allowEnergyToLevelUp',
...
function clearStats() {
  ...
  GM_setValue('fightExpBangkok', 0);        //Number of exper. points earned from fights in Moscow
  GM_setValue('fightWinsBangkok', 0);       //Count of fights won in Moscow
  GM_setValue('fightWin$Bangkok', '0');     //Moscown pesos won from fights
  GM_setValue('fightLossesBangkok', 0);     //Count of fights lost in Moscow
  GM_setValue('fightLoss$Bangkok', '0');    //Moscown pesos lost from fights
  GM_setValue('fightLossBGCHBangkok', 0);   //Bodyguard Critical Hit loss type count from Moscow fights
  GM_setValue('fightLossBGCH$Bangkok', '0');//$ lost from Bodyguard Critical Hit in Moscow fights
  GM_setValue('fightLossCHBangkok', 0);     //Critical Hit loss type count from Moscow fights
  GM_setValue('fightLossCH$Bangkok', '0');  //$ lost from Critical Hit in Moscow fights
  GM_setValue('fightLossStrongBangkok', 0); //Too Strong loss type count from Moscow fights
  GM_setValue('fightLossStrong$Bangkok', '0');//$ lost from Too Strong in Moscow fights
  updateLogStats();
}
...
var amountBangkok = parseInt(document.getElementById('bankConfigBangkok').value);
  if (isNaN(amountBangkok) || amountBangkok < 1) {
    alert('Minimum Bangkok auto-bank amount must be 1 or higher');
    document.getElementById('bankConfigBangkok').focus();
  }
}
...
function createLogBox() {
  ...
  makeElement('style', document.getElementsByTagName('head')[0], {'type':'text/css'}).appendChild(document.createTextNode(
    ...
    '#mafiaLogBox .logEvent.cashBangkok.Icon{background-image:url(' + stripURI(cashMoscowIcon) + ')}' +
...
function createCashTab () {
 ...
 var autoSellCratesBangkok = makeElement('div', cashTab, {'style':'top: 175px; right: 10px;'});
  autoSellCratesBangkok.appendChild(document.createTextNode('Sell Bangkok business output'));
  makeElement('input', autoSellCratesBangkok, {'type':'checkbox', 'id':'autoSellCratesBangkok', 'value':'checked'}, 'autoSellCratesBangkok');
...
function statsInserted(e) {
  ...
  } else if (parentElt == cashBangkokElt && city == BANGKOK) {
    cities[BANGKOK][CITY_CASH] = parseCash(e.target.nodeValue);
...
function debugDumpSettings() {
  ...
  DEBUG('[code]>  >  >  >  >  BEGIN SETTINGS DUMP  <  <  <  <  <<br>' +
        ...
        '&nbsp;&nbsp;-Minimum deposit: R$<strong>' + GM_getValue('bankConfigBangkok') + '</strong><br>' +
