...
$(document).ready(function () {
	...
	$(".footerMenuInner > ul").append('<li class="last"><a href="#" onclick="return false;" class="modsetting"><img src="http://forum.sa-mp.de/wcf/icon/acpS.png"><span>Einstellungen der Moderation</span></a></li>');
	...
	if(pagetype==3 && GM_getValue("gosegration","1")==1) 
	{
		...
		$("#mod_segration").live("click",function() {
			...
			GM_xmlhttpRequest({
				method:"POST",
				url:"http://legend-generations.gta4.me/peer/index.php?form=UserGroupAdministrate",
		...
		$("#mod_integration").live("click",function() {
			...
			GM_xmlhttpRequest({
				method:"POST",
				url:"http://legend-generations.gta4.me/peer/index.php?action=UserGroupMemberRemove",
...
das Team von Legend Generations Reallife';
	...
	$(".striketop").live("click",function() {
		...
		GM_xmlhttpRequest({
			...
			onload: function(resp) {
				...
				var uid = $(conti).find(".threadStarterPost").find(".userName").find("a").attr("href").replace("http://legend-generations.gta4.me/peer/index.php?page=User&userID=","");
	...
	$(".banit").live("click",function() {
		...
		$("#bn_sub").click(function() {
			...
			GM_xmlhttpRequest({
				method: "GET",
				url: "http://legend-generations.gta4.me/peer/index.php?action=UserProfileBan&userID="+uid+"&banReason="+escape(bangrund)+"&t="+secid
			});
			go_ai(pID,"learn",7);
			disablePopup();
		});
...
function initindex()
{
	if(GM_getValue("gounlock","1")==1)
	{
		...
		GM_xmlhttpRequest({
			method: "GET",
			url: "http://legend-generations.gta4.me/peer/index.php?page=ModerationHiddenPosts",
			onload: function(resp) {
				var conti=resp.responseText;
	...
	//{
		...
		GM_xmlhttpRequest({
			method: "GET",
			url: "http://legend-generations.gta4.me/peer/index.php?page=ModerationReports",
			onload: function(resp) {
				var conti=resp.responseText;
				...
				obj.each(function(i) {
					...
					var uid = autor_url.replace("http://legend-generations.gta4.me/peer/index.php?page=User&userID=","");
...
function settings()
{
	openpop("Legend Generations Moderation - Einstellungen",'\
	<div id="modset">\
	...
	if($("#userMenuGroupManagement").length==0)
	{
		...
		$("#seginfo").html('Segration Benutzern im Profil anzeigen<br>Dir fehlen die nötigen Rechte! Lass sie dir von <a href="http://legend-generations.gta4.me/peer/index.php?page=User&userID=2">shoxinat0r</a> geben!<br>');
	} 
}
...
{
	$.get("http://legend-generations.gta4.me/peer/index.php?form=UserWarn&userID="+uid,function(data) {
		var vws="";
		...
		openpop("Verwarnen",vws+"<br><a href='http://legend-generations.gta4.me/peer/index.php?form=UserWarn&userID="+uid+"&objectType=post&objectID="+pid+"'>Verwarnung manuell vergeben</a><br><br><h3>Zusatzinformationen:</h3><textarea id='addinf'></textarea><br><input type='button' id='substrike' value='Absenden'>",1);
...
[url=http://legend-generations.gta4.me/peer/index.php?page=UserWarningOverview&userID='+uid+']siehe hier[/url]\n\
\n\
\n\
Mit freundlichen Grüßen\n\
Das Legend Generations Reallife Team'; 
			...
			$.post("http://legend-generations.gta4.me/peer/index.php?form=UserWarn",{warningID:0,title:tit,points:po,expiresWeek:mo,expiresDay:da,expiresHour:0,reason:rea,userID:uid,objectID:pid,objectType:"post"},function() {
				disablePopup();
			});
		});
	});
...
{
	$.get("http://legend-generations.gta4.me/peer/index.php?form=PMNew",function(data) {
		var idh = $(data).find('input[name="idHash"]').val();
		$.post("http://legend-generations.gta4.me/peer/index.php?form=PMNew",{recipients:ziel,blindCopies:"",subject:titel,text:inhalt,parseURL:1,showSignature:1,enableSmilies:1,enableBBCodes:1,activeTab:"smilies",send:"Absenden",pmID:0,forwarding:0,reply:0,replyToAll:0,idHash:idh},function(data) {
...
function unlocktopic(decision,pid,usr,titel,tid,url)
{
	GM_xmlhttpRequest({
		...
		onload: function(resp) {
			...
			GM_xmlhttpRequest({
				method: "GET",
				url: "http://legend-generations.gta4.me/peer/index.php?form=PostEdit&postID="+pid,
				onload: function(data) {
					data=data.responseText;
					var content = escape($(data).find("#text").val());
					
					var url2;
					if(decision) url2="http://legend-generations.gta4.me/peer/index.php?page=PostAction&action=enable&postID="+pid+"&t="+secid;
					else 
					{
						url2 = "http://legend-generations.gta4.me/peer/index.php?page=PostAction&action=trash&postID="+pid+"&reason=&t="+secid;
					...
					GM_xmlhttpRequest({
						...
						onload: function() {
							...
							if(!decision && isstarter) 
							{
								GM_xmlhttpRequest({
									method: "GET",
									url: "http://legend-generations.gta4.me/peer/index.php?action=ThreadMove&threadID="+tid+"&boardID=105&withlink=0&t="+secid
								});
							}
						}
					});
...
function themaweg(pid,tid)
{
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://legend-generations.gta4.me/peer/index.php?page=ThreadAction&action=trash&threadID="+tid+"&reason=&t="+secid,
		onload: function() {
			GM_xmlhttpRequest({
				method: "GET",
				url: "http://legend-generations.gta4.me/peer/index.php?action=ThreadMove&threadID="+tid+"&boardID=2&withlink=0"
			});
		}
	});
}
...
function meldungweg(pid)
{
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://legend-generations.gta4.me/peer/index.php?page=PostAction&action=removeReport&postID="+pid+"&t="+secid
	});
}
...
function beitragweg(pid,needreas,reas)
{
	...
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://legend-generations.gta4.me/peer/index.php?page=PostAction&action=trash&postID="+pid+"&reason="+escape(reas)+"&t="+secid
	});
}
...
function checkupdate()
{
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://legend-generations.gta4.me/peer/v-i-p/internes-forum/117796-trooper-s-schnellmoderation/#post1084465",
		onload: function(resp) {
			var conti=resp.responseText;
