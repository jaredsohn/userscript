// ==UserScript==
// @name           Path Of Exile Forum Enhancer
// @namespace      ignarsoll-poe-forum-enhancer
...
require(["main"], function() {
	...
	if (parts.length > 1 ) {
    var section = parts[1];
    var subsection = parts[2];
	}

	if( section == "view-thread"){
	...
	else if( section == "view-profile") {
			
		if ( $(document).find('.loggedInStatus').find('.profile-link').find('a[class!="twitch"]')[0].textContent == subsection ) {
			$('.profile').append(createOptionsMenu());
