...
function hhb_init() {
    ...
    hhb_d.innerHTML += '<form style="cursor:default;">' + '<input type="checkbox" id="hhb_mehlist_button" name="MEH-list" checked="checked" /> <span style="color:gold;">MEH-list</span> ' + '<input id="hhb_wootlist_button" type="checkbox" name="WOOT!-list" checked="checked" /> <span style="color:gold;">WOOT!-list</span><br/>' + '<input type="checkbox" id="hhb_curatelist_button" name="Curate-list" checked="checked" /> <span style="color:gold;">Curate-</span> ' + '<input id="hhb_undecidedlist_button" type="checkbox" name="Undecided-list" /><span style="color:gold;">Undecided-list</span>' + '</form>';
  ...
  document.getElementById('hhb_curatelist_button').addEventListener('change', function () {
        hhb_curatelist_on = !hhb_curatelist_on;
        hhb_meanlist();
    });
...
function hhb_meanlist() {
    ...
    for (i in a) { //thanks to... (forgot)someone for the pointers
        ...
        //a[i].superuser=false;
        //if(typeof(a[i].ambassador)==="undefined')
        //a[i].ambassador=false;
        //if(typeof(a[i].owner)==='undefined')
        //a[i].owner=false;
        ///}
        if (typeof (a[i].superuser) !== 'undefined' && a[i].superuser == true) {
            str += 'chat-from-super ';
       } else if (typeof (a[i].moderator) !== 'undefined' && a[i].ambassador == true) {
    ...
    curatelist = ' ' + totalCURATEs.toString() + curatelist;
   ...
   if (hhb_curatelist_on) {
        str += '<div id="hhb_curatelist_div" style="border:1px solid rgb(0,255,255); "><span style="font-weight:bold">Curate-list:</span>';
        str += curatelist;
        str += '</div>';
    }
