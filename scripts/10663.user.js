// ==UserScript==
// @name Accept All Friend Requests (Ajaxified version)
// @author Seal | Translated by Mr Nobody
// @version 1.0
...
var sc = function sc(){//////////////* AQUI ï¿½ O COMEï¿½O *//////////////
	if(document.forms[1] && location.hostname.indexOf('orkut.com') != -1 && location.href.toLowerCase().indexOf("home.asp") >-1){
		...
		//Faz uma fila das requisiï¿½ï¿½es
		function ajaxFila(){
			if(uids.length>0)ajaxfight(uids.shift());
			else {
				document.body.removeChild(divLoad);
				alert("All Firends Have been Added.Thanks for using Accept All friend Requests Script From Orkut Plus!");
			}
		}
		//executa as requisiï¿½oes
		function ajaxfight(uid) {
			...
			ajax.onreadystatechange=function(){
				if (ajax.readyState==4 ) {
					...
					ajaxFila();//vai buscar outra requisiï¿½ï¿½o na fila
				}
			}
			ajax.send(params);
		}
