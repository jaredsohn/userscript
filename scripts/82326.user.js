...
var SUC_script_num = 82326; // Change this to the number given to the script by userscripts.org (check the address bar)
...
var GBD = {
	...
	startListerning : function() {
		...
		if (pagesNode.length) {
			...
			//pagesNode.bind('DOMNodeInserted', GBD.processPage);
	...
	addDownloadPage : function(target, src)
	{
		...
		$(parent).append(
			$('<div class="gbd_downloadpage"><a href="'+ src +'" target="_blank" title="Download this page"><img src="'+ GBD.icons.downloadPage +'" /><p>'+ pageName +'</p></a></div>')
		);
	...
	prepareDownload : function() {
		...
		$('#gbdPanel').html('<h1 id="gbdHeader">Google Book Downloader '+ GBD.version +'</h1>' + 							'<div id="gbdResult">Getting page list, please wait ...</div>' +
	...
	showLink : function(pid, src) {
		...
		$('<div><a target="_blank" href="'+ src + '&w=' + GBD.pageWidth + '">' + pid + '.png</a></div>').appendTo('#gbdResultList');
