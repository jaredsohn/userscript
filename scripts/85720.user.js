// ==UserScript==
// @name           Handball Player Notes
// @namespace      TeamHB
// @description    TeamHB Player Notes
...
// 0.0.0.8 highlight maxed out stars in training details of player page
//
...
//         after clicking "Stored" there is now an acknowledge message that tells the user "stored successfully"
...
if (url.indexOf('pldetails') > 0) {
	...
	for (var i = 0; i < allPlayers.snapshotLength; i++) {
	    ...
	    if (thisPlayer.innerHTML.indexOf('progress_maybe.png')>=0){
	    	thisPlayer.style.background="orange";
	    }
  ...
  if ((url.indexOf('players') > 0) || (url.indexOf('training') > 0)) {
	...
	for (var i = 0; i < allPlayers.snapshotLength; i++) {
		...
		if (thisNotes != null) {
			var mynote="";
			if(thisNotes.length > 0) {
				mynote=thisNotes.substring(0,13);
			}
			var shorty="";
			if(thisNotes.length > 13) {
				shorty="...";
			}
...
function saveNotes(){
       var style2='font-size: 12px; font-family: verdana, arial, sans-serif;';
       var notesbox = document.getElementById('txtNotes');
       GM_setValue(plyrid  + "_notes",notesbox.value);
       document.getElementById('stored').innerHTML='<font color=red style="'+style2+'">KLART!</font>';
}
