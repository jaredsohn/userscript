// ==UserScript==
// @name           Enhanced Google Analytics
// @namespace      Analytics
...
if(window.location.href.match('reporting/referring_sources?'))
{
...
else if(window.location.href.match('reporting/keywords?'))
{
    //max table display limit
    // display_limit = 100
    // how much growth is necessary before the element is considered to have had enough of a traffic increase
...
//Display the button
var tab_element;
tab_element = window.document.createElement("DIV");
button_element = window.document.createElement("TD");
button_html = '<a href="#" onclick="return false;" class="button" id="adv_vis_button_link"> <b><b><b>Who sent me unusual traffic?</b></b></b> </a>';
loading_html = '<a href="#" onclick="return false;" class="button" id="adv_vis_button_link"> <b><b><b>Loading...</b></b></b> </a>';
final_html = '<br><br><div><div><div></div></div></div><br>'
// tab_element.innerHTML = tab_html;
button_element.innerHTML = button_html;
button_element.addEventListener("click", dostuff, true);

//report_tools
document.getElementById('ReportContent').addEventListener("DOMNodeInserted", insert_button, true);

// html_insert_it(document.getElementById('TabView'),tab_element);
// html_insert_after(document.getElementById('email_button_item'),button_element);
html_insert_after(document.getElementById('Narrative'),tab_element);
insert_button();
...
function replacer(nncountnn,nnnamenn,nnnum1nn,nnnum2nn,nnnum3nn,nnnum4nn,nnnum5nn) {
    
    return '<tbody><tr class="highlight"><td class="count">' + nncountnn + 
                    '.</td><td class="text" colspan="1"><div title="' + nnnamenn + 
                    '" class="text_wrapper"><div class="text_wrapper">' + site_link_ref.replace(/nnnamenn/g, nnnamenn) + 
                    '</div></div></td><td id="f_cell_0_0" class="sort">' + nnnum1nn + 
                    '</td><td id="f_cell_0_1" class="">' + nnnum2nn + 
                    '</td><td id="f_cell_0_2" class="">' + nnnum3nn + 
                    '</td><td id="f_cell_0_3" class="">' + nnnum4nn + 
                    '%</td><td id="f_cell_0_4" class="">' + nnnum5nn + 
                    '%</td></tr>'
    
}

//record index to value conversions for the dropdown
var index_converter = {'10': 0,
                       '25': 1,
                       '50': 2,
                       '100': 3,
                       '250': 4,
...
var tail_string = (<r><![CDATA[	     <td colspan="7" class="table_controls"> <div class="box"> <form action="" onsubmit='return false;'> <div class="pagination_controls"> <span class="button_label">Show rows:</span> <select id="nnnndropdown" onchange="return true;"><option value="10" selected="selected">10</option> <option value="25">25</option> <option value="50">50</option> <option value="100">100</option> <option value="250">250</option> <option value="500">500</option></select> </div> </form> </div> </td> </div>
]]></r>).toString();
...
function change_visible_rows() {
    if (GM_getValue('gm_display',-1) !=this.options[this.selectedIndex].value)
    {
        GM_setValue('gm_display', this.options[this.selectedIndex].value);
        dostuff();
    }
}
...
function dostuff()
{
    document.getElementById('ReportContent').removeEventListener("DOMNodeInserted", insert_button, true);
    ...
    //clean the tables out first
    tab_element.innerHTML = "";
    
    var startdate, enddate, urlstartdate, urlenddate, olddate, newdata;
    button_element.innerHTML = loading_html;    
    button_element.removeEventListener("click", dostuff, true);
    ...
    if (lentime < look_back)
    {startdate = increment_date(middate, -1 * look_back);
    lentime = count_days(middate, startdate);}
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: (window.location.href.split("#")[0].replace(site_ref + '?', 'export?fmt=2&')).replace(urlenddate, middate).replace(urlstartdate, startdate) + '&limit=' + limit.toString() + '&&rpt=' + download_report_ref,
            headers: {
            ...
            onload: function(responseDetails) {
                ...
                olddata = responseDetails.responseText;
                    GM_xmlhttpRequest({
                    method: 'GET',
                    url: (window.location.href.split("#")[0].replace(site_ref + '?', 'export?fmt=2&')).replace(urlenddate,enddate).replace(urlstartdate,startdate) + '&limit=' + limit.toString() + '&&rpt=' + download_report_ref,
                    headers: {
                        'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
                        'Accept': 'application/atom+xml,application/xml,text/xml',
                    },
                    onload: function(responseDetails) {
                        ...
                        var new_linkers = new Array();
                        var big_growers = new Array();
                        var big_shrinkers = new Array();
                        ...
                        for (i=stopper;1;i++)
                        {
                            ...
                            var match = fulllist[i].split(',');
                            new_values[match[0]] = match;
                        ...
                        for (i=stopper;1;i++)
                        {
                            ...
                            var match = oldlist[i].split(',');
                            old_values[match[0]] = match;
                        }
                        for (el in new_values)
                        {
                            if (old_values[el])
                            {   
                                if ((new_values[el][1]/old_values[el][1] > 1 + (look_back*(1 + growth_tolerance)/lentime))
                                    &&
                                    (new_values[el][1] > growth_per_day)
                                   )
                                {
                                    //element has grown by at least growth_tolerance since last time
                                    big_growers[el] = new_values[el];
                                }
                            }
                            else
                            {
                                //value not in the old list
                                new_linkers[el] = new_values[el];
                            }
                        }
                        
                        for (el in old_values)
                        {
                            if ((new_values[el]) 
                                &&
                                (new_values[el][1]/old_values[el][1] < 1 + (look_back*(1 - growth_tolerance)/lentime))
                                &&
                                (old_values[el][1] > growth_per_day)
                                )
                            {           
                                //element has grown by at least growth_tolerance since last time
                                big_shrinkers[el] = new_values[el];
                            }
                        }
                        ...
                        a = 0;
                        for (el in big_growers)
                        {
                            if (a>=parseInt(GM_getValue('gm_display', 10))) {break;}
                            a += 1;
                            arr = convert_array(big_growers[el]);
                            if (arr[1]<minimum_number_elements) {break;}
                            body += replacer(a.toString(), arr[0], arr[1],arr[2],arr[3],arr[4],arr[5]);
                        }
                        new_tab_html = '<h3>' + label_ref + ' With ' + (growth_tolerance * 100).toString() + '% Higher Traffic Over the Last ' + look_back.toString() + ' Days:</b></h3><br>' + head_string + body + tail_string.replace('nnnn','a') + '<br>';
                        new_tab_element.innerHTML = new_tab_html;
                        tab_element.appendChild(new_tab_element);
                        ...
                        body = ""
                        a = 0;
                        for (el in big_shrinkers)
                        {
                            if (a>=parseInt(GM_getValue('gm_display', 10))) {break;}
                            a += 1;
                            arr = convert_array(big_shrinkers[el]);
                            if (arr[1]<minimum_number_elements) {break;}
                            body += replacer(a.toString(), arr[0], arr[1],arr[2],arr[3],arr[4],arr[5]);
                        }
                        new_tab_html = '<br><h3>' + label_ref + ' With ' + (growth_tolerance * 100).toString() + '% Lower Traffic Over the Last ' + look_back.toString() + ' Days:</b></h3><br>' + head_string + body + tail_string.replace('nnnn','b') + '<br>';
                        new_tab_element.innerHTML = new_tab_html;
                        tab_element.appendChild(new_tab_element);
                        
                        body = ""
                        //only display new linkers if the page is not the keywords page
                        if(!window.location.href.match('reporting/keywords??'))
                        {
                            a = 0;
                            for (el in new_linkers)
                            {
                                if (a>=parseInt(GM_getValue('gm_display', 10))) {break;}
                                a += 1;
                                arr = convert_array(new_linkers[el]);
                                if (arr[1]<minimum_number_elements) {break;}
                                body += replacer(a.toString(), arr[0], arr[1],arr[2],arr[3],arr[4],arr[5]);
                            }

                            new_tab_element = window.document.createElement ("SPAN");
                            new_tab_html = '<br><h3>New ' + label_ref + ' Appearing in Last ' + look_back.toString() + ' Days:</h3><br>' + head_string + body + tail_string.replace('nnnn','c') + '<br>';
                            new_tab_element.innerHTML = new_tab_html;
                            tab_element.appendChild(new_tab_element);
                        }
                        
                        
                        final_element = window.document.createElement ("SPAN");
                        final_element.innerHTML = final_html;
                        tab_element.appendChild(final_element);
                        button_element.innerHTML = button_html;
                        unsafeWindow.document.getElementById('adropdown').onclick = change_visible_rows;
                        unsafeWindow.document.getElementById('adropdown').selectedIndex = index_converter[GM_getValue('gm_display', '10').toString()];
                        unsafeWindow.document.getElementById('bdropdown').onclick = change_visible_rows;
                        unsafeWindow.document.getElementById('bdropdown').selectedIndex = index_converter[GM_getValue('gm_display', '10').toString()];
                        unsafeWindow.document.getElementById('cdropdown').onclick = change_visible_rows;
                        unsafeWindow.document.getElementById('cdropdown').selectedIndex = index_converter[GM_getValue('gm_display', '10').toString()];
                        button_element.addEventListener("click", dostuff, true);
                        document.getElementById('ReportContent').addEventListener("DOMNodeInserted", insert_button, true);
                    }
                });
