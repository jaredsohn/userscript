...
var locationSpans = xpath("//span[@class='PropDetailsSummarySpecText']");
...
if (moreButton && propertyId.length > 0 && GM_xmlhttpRequest) {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://www.mls.ca/PropertyPhotos.aspx?propertyID='+propertyId,
		onload: function(response) {
			if (response.status == 200) {
				// Remove more button
				moreButton.parentNode.removeChild(moreButton);
				var html = response.responseText;
				var imgs = html.match(/http:\/\/images\.mls\.ca\/listings\/[^']+/g) ;
