...
var url=document.URL;//on rΓ©cupΓ©re l'url du site.

if(/fr/.test(url))//en fonction de l'url on prend les regex(dΓ©pend de la langue du site).
	var regex=new Array(
...
//En fonction du language du naviguateur on va utiliser un langage associΓ©.
switch(myLanguage){
	case 'fr':
	var lang=new Array(
		' Fini Γ  ',
...
break; 
	/*
        ...
        ' termina a las ',
        'Cerrar',
        'Actualizar mΓ‘s tarde.',
        'Lista de construcciΓ³n',
        'AΓ±adir edificio.',
        'Construir en',
        'horas',
        'minutos y',
...
function urlParse(url,param)//on rΓ©cupΓ©re une des valeurs get d'une url(son nom est le param.
{
	var chaine=url;//On rΓ©cupΓ©re l'url du site.
...
function createDiv(id,classN,html)// On ajoute un div 
{
	...
	div = document.createElement('div');//on crΓ©e le div
...
{
	var lien=document.createElement('a');//crΓ©ation d'un lien
...
function createInput(name,type,size,value,id)//fonction de crΓ©ation input
{
...
function createBr()//fonction de crΓ©ation saut de ligne
{
...
/*--------------------------------------------------------
CrΓ©ation des fonctions de temps.
---------------------------------------------------------*/
...
function setTextTime(temps)// CrΓ©e le temps de fin.
{
	var Temps = new Date();
...
function constructionTempsFin()//Ajoute le temps de fin de construction des bΓ’timents.
{
...
function idIleRecup()
{
	//rΓ©cupΓ©ration de l'id de la ville :
...
function promptLanguage()
{
	...
	while(newLanguage!='fr' && newLanguage!='en' && newLanguage!='pt' && newLanguage!='da' && newLanguage!='gr')
	{
	var newLanguage = prompt("Ikariam Kronos Tools :\n What language do you speak?\n(fr:French; en:English; pt: Portuguese) \nLanguage: " +myLanguage);
		if(newLanguage=='fr' || newLanguage=='en' || newLanguage=='pt' || newLanguage=='da'|| newLanguage=='gr')
		{
