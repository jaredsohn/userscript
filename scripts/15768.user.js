...
function swapIcons(ctx){
	...
	$x('//td[@class="thumb vcard author"]/a', ctx).forEach(function(link){
		...
		if(!src){
			...
			src = cache[name] = 'http://chart.apis.google.com/chart?' + queryString({
				cht  : 'p',
				chs  : '55x55',
				chd  : toDatas(values),
				chco : toColors(name, values),
			});
		}
...
function toDatas(values){
	...
	for (var i=0; i<values.length; i++)
		datas.push(CHARS.charAt(Math.round((CHARS.length-1) * values[i])));
