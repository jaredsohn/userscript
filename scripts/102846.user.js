...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      ...
      <TR><TD align=right>benötige mehr mehr: </td><td colspan=2><font color=#FF0000><B> <span id=ptttLimiter></span> </b></font></TD></TR>\
...
Tabs.msg = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptstat>Suche in Ein/Ausgangs Box oder Allianz Berichte</div>';
    m += '<DIV class=ptentry style="height:30px"><table>';
    m += '<TABLE width=95% class=ptTab><TD>Suche in: <select id="searchSelect"><option value="Reports">Reports</options>';
    m += '<option value="Eing.box">Inbox</option>';
    m += '<option value="Ausg.box">Outbox</option></select></td>';
    ...
    m += '<TD>Suche in: <select disabled=true id="searchForSelect"><option value="Subject">Fach Zweig 
</options>';
    m += '<option value="User">Anwender</option></select></td>';
    
    m += '<TD>Suche nach:<INPUT id=searchString disabled=true type=text size=10 maxlength=25 value=""</td>';
    m += '<TD><INPUT id=StartSearch type=submit value="Starte Suche"></td>';
    ...
    document.getElementById('StartSearch').addEventListener ('click', function(){ 
        	 if (document.getElementById('searchSelect').value == "Reports") {
        	 		t.totalPages = 0;
        	 		t.content = '<center><table><thead><th>anklicken</th><th>Page#</th><th>Datum</th><th>Angreifer</th><th>von</th><th>Allianz</th><th>Aktion</th><th>Ziel</th><th>Type</th><th>auf</th></thead><tbody>';
        	 		t.searchReports("",1); 
        	 }
        	 else {
        	 	t.totalPages = 0;
        	 	t.content = '<center><table><thead><th>Click</th><th>Page#</th><th>Datum</th><th>From</th><th>Subject</th></thead><tbody>';
        	 	t.searchMail("",1); 
        	 }
        }, false);
...
//    t.show ();
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('Einstellung Zurücksetzten', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
    for (var c=0; c<Cities.numCities; c++){
  ...
  /***   MARCHES SUBTAB  ***/
    showMarches : function (){
     ...
     for (var c=0; c< Seed.cities.length;c++) {
		...
		m += '</table>';
	m += '<span style=\"font-size:10px; color:#555; line-height:18px; \"><font color=#600000><B><u>Hinweis</u></b></font>: Um die Truppen zurück zu rufen klick auf die Marschbilder!!</span>';
