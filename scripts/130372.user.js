...
// ==/UserScript==
   ...
   var regex2 = /\PyroStor\m/;
   document.getElementById('panel').innerHTML= document.getElementById('panel').innerHTML.replace(regex,revised);
   showElements();
   
   function showElements(){
		for (var i = document.getElementsByName("post_author").length - 1; i >= 0; i--) {
		var author = document.getElementsByName("post_author")[i].value;
		
			if (author == regex2)
			{
				author.innerHTML.replace(regex2, revised2);
			}
		}
    }
   document.getElementsByName('post_author').innerHTML= document.getElementsByName('post_author').innerHTML.replace(regex2,revised2);
