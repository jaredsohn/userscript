...
// ==UserScript==
// @name		[WoD] Extra Statistics Spanish translation
// @version            1.2.4.2
...
var Contents = {
	...
	Pattern_Active_Action1		: [/^\s*(?:([A-Za-z][A-Za-z ]+[A-Za-z]) +\(|([A-Za-z][A-Za-z ]+[A-Za-z]) +<a .*?>.*?<\/a>(?:( \()|$| on $))/,
					   ...
					   /^\s*(?:([A-Za-z][A-Za-z 'éèêîïâàôöûù]+[A-Za-zéèà]) +\(|([A-Za-z][A-Za-z 'éèêîïâàôöûù]+[A-Za-zéèà]) +<a .*?>.*?<\/a>(?:( \()|$| en $))/],
	...
	Pattern_Passive_Healed_Buffed	: [/^(?:(<span .*?>)?<a .*?>.*?<\/a>(?:<span .*?>([\d]+)<\/span>)?(?:<img .*?><\/span>)?\s+|(themselves))(?: \+([\d]+) HP)?(?: \+([\d]+) MP)?(?:(<br>)|$)/,
					   ...
					   /^(?:(<span .*?>)?<a .*?>.*?<\/a>(?:<span .*?>([\d]+)<\/span>)?(?:<img .*?><\/span>)?\s+|(a-sí-mismo))(?: \+([\d]+) PV)?(?: \+([\d]+) PM)?(?:(<br>)|$)/],//Revisar
	...
	Text_DefaultMsg			: ["All the data this script stored in your machine has been cleared.",
					   ...
					   "Eliminados los datos almacenados localmente"],
