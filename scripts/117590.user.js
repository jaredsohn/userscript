// ==UserScript==
// @name           eRepublik battle master Traditional Chinese
// @version        1.1.1
// @author         eCitizen Scyld
// @namespace      eCitizenScyld
// @translator	   eCitizen kiletw  
...
function bhStats(att, def) {
	...
	$j('div#myStatBoxL').html(
		'<div id="BHTableL">' +
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>公民</th><th>擊殺數</th><th>影響力</th></tr>' + att + '</table></div>' +
		'</div>'
		);
	$j('#BHTableR').remove();
	$j('div#myStatBoxR').html(
		'<div id="BHTableR">' +
		'<div class="BHTable" style="float:right"><table class="BHTable"><tr><th>公民</th><th>擊殺數</th><th>影響力</th></tr>' + def + '</table></div>' +
		'</div>'
	);
...
function histStats(hist) {
	...
	$j('div#myOverBox').html(
		'<div id="OOTable">' +
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>公民</th><th>擊殺數</th><th>影響力</th></tr>' + hist + '</table></div>' +
		'</div>'
	);
...
function getRegionInfo(id)
{
	...
	$j('button#hospitals_get').html('請等待...').attr('disabled', 'true');
	...
	GM_xmlhttpRequest({
			...
			if (regionsData && regionsData.length > 0) {
				...
				for (var a = 0; a < regionsData.length; a++) {
					...
					if (regionsData[a].name == regionName && regionsData[a].buildings.hospitals.length > 0) {
						...
						$j('button#hospitals_get').html('無醫院 - 也許等下有').removeAttr('disabled');
		...
		onreadystatechange:	function(response) {
			...
			$j('button#hospitals_get').html('請等待...');

		},
		onerror: function(response) {

			$j('button#hospitals_get').html('錯誤! 重試中...');
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'</select>    # 輸出次數: <input id="dmgFights" name="dmgFights" value="94" size="4" maxlength="4" />' +
			...
			'<div class="stat"><small>含仇敵加成: <strong><span style="font-size:12px;margin-right:15px">' +
	...
	$j('div#enemy_defeated').before(
		...
		'<big>輸出次數: </big>' +
		'<input id="multihit_count" type="text" size="3" maxlength="3" value="1" /><button id="multihit_start">攻擊!</button>' +
		'  <input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food"> 自動吃食物回體</label>' +
		'  <input type="checkbox" id="multihit_bazooka" name="multihit_bazooka" checked="checked"><label for="multihit_bazooka"> 不使用火箭筒</label><br />' +
		'<input type="checkbox" id="multihit_energy" name="multihit_energy"><label for="multihit_energy"> 自動在沒體力額度時使用棒棒糖</label>' +
		'  <input type="checkbox" id="multihit_hk" name="multihit_hk"><label for="multihit_hk"> 自動在沒體力額度時使用血包</label><br />' +
                ' <input id="hittime" type="text" size="3" maxlength="4" value="1010" /><label for="hit_time"> 輸出時間間隔(毫秒)</label><br /> ' +
		'<button id="heros_update">更新</button>  英雄列表將會自動的更新,但是你依然可以點這按鈕更新它' +
		...
		'<button id="hospitals_get">使用醫院</button></div>' +
		'</div><br />' +
		'<fieldset id="BLOG"><legend>v1.1.1 BATTLE LOG:  <button id="battlelog_clear">清除</button></legend><div id="battlelog"></div></fieldset>'
	);
	...
	unsafeWindow.battleFX.hit = function() {
		...
		if (multiHitRunning) {
			...
			bLog('攻擊完成!');
			$j('div#multihit_message').html('已攻擊: ' + multiHitDone + '  殺敵數: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? '最後的敵人已擊殺!' : '最後的敵人存活中!') + ')');
			clearTimeout(multiHitLoopId);
			multiHitLoopId = setTimeout("jQuery.fn.multiHIT()", hittime);
	  ...
	  bLog('Change Weapon');
		...
		$j.post(url, {_token: unsafeWindow.SERVER_DATA.csrfToken, battleId: unsafeWindow.SERVER_DATA.battleId}, function (response) {
			...
			if ($j(".listing span img").eq(-1).attr("src").indexOf("q10") !== -1) {
				...
				$j('button#multihit_start').html('攻擊!');
				return;
	...
	$j('button#multihit_start').click(function() {
		...
		if (multiHitRunning) {
			...
			multiHitCount = $j('input#multihit_count').val();
				...
				$j('button#multihit_start').html('<strong>停止!</strong>');
				bLog('攻擊中...');
	...
	$j('button#hospitals_get').click(function() {
		...
		if (unsafeWindow.SERVER_DATA.onlySpectator != 1 && unsafeWindow.SERVER_DATA.isResistance != 1 && id != unsafeWindow.SERVER_DATA.countryId) {
			...
			$j('button#hospitals_get').html('攻擊方無法使用醫院!').attr('disabled', 'true');
			return;
