// ==UserScript==
// @name           Pokemon Vortex - Arceus (final)
// @namespace      vortexrising.tk
// @include        http://omega.pokemonvortex.org/map.php?map=14
// @include        http://sigma.pokemonvortex.org/map.php?map=14
...
var settings = {
	'keys': ['autoBattle', 'findRare', 'findArceus','catchPok','findNewpoke'],
	'data': {},
	'save': function() {
...
function moveAround() {
	...
	var xpos = parseInt(Math.sin(movPos * (Math.PI / 18))*10);
	var ypos = parseInt(Math.cos(movPos * (Math.PI / 18))*10);
	unsafeWindow.PlayRequest(xpos + 4, ypos + 4, parseInt(Math.random()*4)+1);
...
function init() {
	...
	createToggler(container, 'Find Arceus', 'findArceus');
