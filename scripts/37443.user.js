...
// @include        http://apps.facebook.com/crusades/battle
// @include        http://apps.facebook.com/crusades/invite
// @include        http://apps.new.facebook.com/crusades/battle
// @include        http://apps.new.facebook.com/crusades/invite
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/crusades/invite';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/crusades/inventory')]");
	//First link is always yourself from the 'inventory' link at the top
	members.shift();
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/crusades/invite.php?action=create',
