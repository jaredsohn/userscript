// ==UserScript==
// @name           Nettbys profildesignere 0.0.1
// @namespace      Nettbys profildesignere 0.0.1
...
akt.innerHTML = akt.innerHTML + 'http://blizzu.com/sp_ny/img/SPB-Logo-nettby.png">';
...
function poeng(e){try{e.innerHTML=e.innerHTML.replace(/Poeng i dag/,"Poeng jeg har fått i dag").replace(/(\d*)\/20\)/,"--0--)").replace(/Poeng totalt/,"Jeg har totalt").replace(/\(\d*\/2000\)/,"(2001/2000)").replace(/Bling-O-Meter:/,"Profildesign BLING!:");
...
for(i=0;i<links.length;i++) {
 if(links[i].innerHTML=="mujaff") {
  links[i].innerHTML="ProfildesignerLederen";
