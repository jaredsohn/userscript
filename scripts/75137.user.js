// ==UserScript==
// @name           Radio Hunter Moderacao
// @namespace      by Joseph | uid=15068755456499681157
// @description    Script para facilitar o trabalho da moderacao da Radio Hunter
// @dgohcodes      orkut.com.br/Community?cmm=54966690
// @include        *.orkut.*/Main#CommMsgs?cmm=54966690*
// @include        *.orkut.*/CommMsgs?cmm=54966690*
// @include        *.orkut.*/Main#CommMemberManage?cmm=54966690*
// @include        *.orkut.*/CommMemberManage?cmm=54966690*
// @include        *.orkut.*/Community?cmm=54966690*
// @include        *.orkut.*/Main#Community?cmm=54966690*
// ==/UserScript==
...
function captchaBAN(env)
	{
		...
		'			ajax.open("POST","CommMsgPost?cmm=54966690&tid=5523851941824052433", false);' + 
...
function captchaONOFF(env, status)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?cmm=54966690&tid=5523871587004464337", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function online()
	{
		...
		var texto = String("[b][green]ON![/green][/b]<br>").concat("\n\n[silver]",String(Math.random()),"[/silver]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
...
function offline()
	{
		...
		var texto = String("[b][red]OFF![/red][/b]<br>").concat("\n\n[silver]",String(Math.random()),"[/silver]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
				"subjectText" : encodeURIComponent(""),
