// ==UserScript==
// @name           Quick Quote (with BBcode for Chrome) BETA!
// @author         amareus, patapper, darkip, filefly, tobbez and captZEEbo, Mordred
// @namespace      http://what.cd
// @description    Quick Quote (with BBcode for Chrome)
...
function copySel() {
	...
	if (selText != '' && selText != null) {
		if (document.URL.match(/viewconv/)) {
			...
			load_page_then(ssl+"what.cd/forums.php?action=get_post&post=" + quoteID, function(response) {
				...
				var spaces = [''].concat(selText.trim().replace(/[\S ]* wrote:/g, "").replace(/\s+/g, "").replace(/^#/gm, "").replace(/'/g,"&#39;").split("")).concat('');
				...
				if (match) {
					var repl = match[0];
					inputBox.value += "[quote=" + userName + "]" + repl.replace("&#39;","'") + "[/quote]";
					...
					inputBox.value += "[quote=" + userName + "]" + originalBBCode.replace("&#39;","'") + "[/quote]";
				}
	...
	//	window.Quote(quoteID,userName);
		location.assign("javascript:Quote('" + quoteID + "', '" + userName + "');void 0");
	}
...
// amareus, patapper, darkip, filefly, tobbez and captZEEbo, Mordred
