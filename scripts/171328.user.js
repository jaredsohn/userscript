// ==UserScript==
// @name           bro1_favoritetrade
// @namespace      http://chaki.s27.xrea.com/br3/
// @include        http://*.1kibaku.jp/*
// @require	       http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js
// @description    ブラウザ一騎当千 トレード関連ツール詰め合わせ by きの。一騎当千化デシ+明木
// @author         kino.
// @version        2.41
// ==/UserScript==
...
var AH_list = ["慈愛舞踏","寵愛舞踏","神医舞踏","丹田功","施設補強"];
...
// @namespace      http://chaki.s27.xrea.com/br3/
// @description    ブラウザ一騎当千 お気に入りトレード一覧表示 by きの。
...
function favorite_trade(){
	...
	function set_favorite(){
		...
		html += "　　・1kibakujpzourijp_cardnolink<br>"
		...
		html += "　<input type=checkbox id=cb_flg14>【トレード】10TP即落札検索<br>"
		html += "　<input type=checkbox id=cb_flg3>【トレード/デッキ】トレードリンク追加<br>"
		html += "　<input type=checkbox id=cb_flg6>【トレード/トーシダス/武将図鑑】トレード価格調査・出品補助<br>"
		html += "　<input type=checkbox id=cb_flg7>【トーシダス】トーシダス結果画面に「このカードを破棄」ボタン追加<br>"
		html += "　<input type=checkbox id=cb_flg9>【トーシダス】トーシダスライトで特定のカードを自動破棄<br>"
		html += "　　　┃<input type=radio name=bushodasu value=0>破棄リストにないカードを取得したら止める（半自動トーシダス）<br>"
		html += "　　　┃<input type=radio name=bushodasu value=1>破棄リストにないカードを取得しても引くのを止めない（全自動トーシダス）<br>"
		...
		html += "<br>※例外：慈愛劉備(No1007)、施設補強阿斗(No1048)、慈愛于禁(No2036)<br />\n"
		...
		html += "<br>※ここにカードNoを入力したカードをトーシダスライトで引くと自動的に破棄してもう一枚引きます。<br />\n"
		...
		function chg_select(a){
			if( a == "C"){
				auto_delete_cno = "1001,1003,1005,1006,1015,1016,1018,1040,1045,1049,1060,1066,1081,,,,,,,,2001,2003,2006,2008,2010,2012,2015,2017,2020,2023,2026,2034,2035,2037,2039,2041,2048,2054,,,3001,3002,3004,3005,3007,3010,3012,3016,3019,3021,3023,3036,3038,3047,,,,,,,4001,4003,4005,4006,4008,4010,4012,4014,4015,4016,4019,4021,4036,4037,4038,4045,4065,4071,4077,4079,4084,,,,,,,,,,,";
			}
			if( a == "2"){
				auto_delete_cno = "1001,1002,1005,1006,1015,1016,1017,1018,1028,1040,1044,1045,1049,1060,1066,1081,,,,2001,2002,2006,2007,2008,2009,2010,2023,2034,2037,2039,2041,2047,2048,2054,,,,,3005,3006,3012,3013,3014,3015,3016,3021,3022,3023,3024,3037,3038,,,,,,,,4001,4002,4003,4004,4005,4006,4008,4010,4011,4012,4019,4020,4021,4025,4026,4027,4036,4037,4038,4045,4065,4071,4077,4083,4084,,";
			}
			if( a == "2C"){
				auto_delete_cno = "1001,1002,1003,1005,1006,1015,1016,1017,1018,1028,1040,1044,1045,1049,1060,1066,1081,,,2001,2002,2003,2006,2007,2008,2009,2010,2012,2015,2017,2020,2023,2026,2034,2035,2037,2039,2041,2047,2048,2054,,,3001,3002,3004,3005,3006,3007,3010,3012,3013,3014,3015,3016,3019,3021,3022,3023,3024,3036,3037,3038,3047,,,4001,4002,4003,4004,4005,4006,4008,4010,4011,4012,4014,4015,4016,4019,4020,4021,4025,4026,4027,4036,4037,4038,4045,4065,4071,4077,4079,4083,4084";
			}
			var cno = auto_delete_cno.split(",");
			for(var i=0;i<l_length2;i++){
				if( i < cno.length ){
...
// @description    ブラウザ一騎当千：トレード画面でカードＩＤを表示させる
...
// @name           1kibakujpzourijp_cardnolink
// @namespace      http://1kibakujp.zouri.jp/cardnolink
...
// @description    ブラウザ一騎当千　スコア単価計算　（修行合成対応改造）
...
// @namespace      http://chaki.s27.xrea.com/br3/
// @description    mixi版ブラウザ一騎当千　トーシダスに「破棄」メニュー追加
...
function TradingSupport(){
	...
	for (var i = 0; i < cards.snapshotLength; i++){
		...
		var cardno = document.evaluate('//*[@class=\"cardno_new\"]',document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotItem(i).innerHTML;
...
function ExhibitSupport(){
	...
	function ExhibitCard(n,i) {
		return function() {
			...
			var ssid = jQuery.cookie('SSID');

			var data = "exhibit_cid="+n+"&exhibit_price="+txt.value+"&exhibit_btn=出品する&ssid="+ssid;
			GM_xmlhttpRequest({
				method:"POST", 
...
// @name           ----
// @namespace      http://chaki.s27.xrea.com/br3/
// @description    トーシダスライトで特定のカードを自動破棄　ｂｙきの。

function AutoDelete(){
	var type = document.getElementsByTagName("h3").item(0).innerHTML;
	if( type.indexOf("トーシダスライト") != -1 ){
...
function DelMessage(x){
	...
	var sender = '\n<span class="notice">ブラウザ一騎当千運営チーム</span>\n';
			...
			if( b_list.item(i).innerHTML.indexOf("セット先") != -1 ){
				...
				for( var j = 3; j < 6; j++ ){
					...
					for(var k = 0; k < AH_list.length; k++){
						if(sk_name.snapshotItem(i).innerHTML.indexOf( AH_list[k] ) != -1 && sk_flg.snapshotItem(i).className != "used"){
...
if(auto_heal_flg == "2"){unsafeWindow.setTimeout(AH_STEP2,200);};
if(auto_heal_flg == "3"){unsafeWindow.setTimeout(AH_STEP3,200);};
if(auto_heal_flg == "4"){unsafeWindow.setTimeout(AH_STEP4,200);};
if(auto_heal_flg == "5"){unsafeWindow.setTimeout(AH_STEP5,200);};
if(auto_heal_flg == "6"){unsafeWindow.setTimeout(AH_STEP6,200);};
if(auto_heal_flg == "7"){unsafeWindow.setTimeout(AH_STEP7,200);};
if(auto_heal_flg == "8"){unsafeWindow.setTimeout(AH_STEP8,200);};
...
if(auto_heal_flg >= 2 && path.indexOf("/card/")!=-1){
	...
	newElm.innerHTML = "　自動回復スキル動作中　　STEP " + auto_heal_flg + " / 8";
...
function AH_STEP3(){
	if( path.indexOf("domestic_setting.php") != -1){
		if(document.body.innerHTML.indexOf("設定されている闘士を解除する")!=-1){
			auto_heal_flg = 8;
			GM_setValue(BRO3_AUTO_HEAL_FLG, auto_heal_flg);
...
function AH_STEP8(){
	auto_heal_flg = 1;
	document.getElementById("ah_text").innerHTML = "　自動回復：　<b>内政闘士がセットされていたため動作を停止しました</b>"
...
function search_trade2(n){
	...
	if( n == 0){
		url = "http://"+host+"/card/trade.php?s=price&o=a&t=name&k=&tl=1&r_ur=0&r_sr=1&r_r=1&r_uc=1&r_c=1";
	}else if( n == 1){
		url = "http://"+host+"/card/trade.php?s=price&o=a&t=name&k=&tl=1&r_ur=1&r_sr=0&r_r=1&r_uc=1&r_c=1";
	}else if( n == 2){
		url = "http://"+host+"/card/trade.php?s=price&o=a&t=name&k=&tl=1&r_ur=1&r_sr=1&r_r=0&r_uc=1&r_c=1";
	}else if( n == 3){
	...
	loop_j:for(var j=0;j<tr.snapshotLength;j++){
		if(tr.snapshotItem(j).innerHTML.indexOf("---") != -1){
			...
			regexp.match(/<strong>(\S*)<\/strong>[\s\S]*id=([0-9]*)/);
			TP = RegExp.$1;
			t_id[cnt+1] = RegExp.$2;
			TP = TP.replace(/,/,"");
