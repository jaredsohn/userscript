// ==UserScript==
// @name        Sindhi Editors
// @copyright   2011, Aamirzia Charan (http://www.sindhsalamat.com)
// @namespace   http://www.sindhsalamat.com
// @version     0.5 Beta
// @description Convert all edit areas on a webpage into Sindhi editors.
// @license     BSDL; http://www.opensource.org/licenses/bsd-license.php
// @include     http://www.blogger.*
// ==/UserScript==
...
WebPad.ProcessKeypress = function(e) {
    ...
    var editorID = WebPad.CurrEdit.getAttribute("SindhiEditorId");
    if (!WebPad.Editors[editorID].SindhiMode) return;
...
WebPad.ToggleLanguage = function(el) {
    var editorID = el.getAttribute("SindhiEditorId");
    isSindhi = this.Editors[editorID].SindhiMode;
    if (isSindhi) {
        this.setEnglish(el);
    }
    else {
...
WebPad.setSindhi = function(el) {
    var editorID = el.getAttribute("SindhiEditorId");
    this.Editors[editorID].SindhiMode = 1;
    el.focus(1);
...
WebPad.setEnglish = function(el) {
    var editorID = el.getAttribute("SindhiEditorId");
    this.Editors[editorID].SindhiMode = 0;
    el.setAttribute("SindhiMode", false);
    el.focus(1);
...
WebPad.setAttributes = function(el) {
    if (el.tagName.toUpperCase() == "IFRAME") {
        ...
        el.setAttribute("SindhiEditorId", editorID);
        this.Editors[editorID] = { SindhiMode: 1, Editor: el };
        WebPad.CurrEdit = el; 
    ...
    var editorID = this.getID();
    el.setAttribute("SindhiEditorId", editorID);
    this.Editors[editorID] = { SindhiMode: 1, Editor: el, OldFont: saveFont, OldBackground: saveBkcolor };
    WebPad.CurrEdit = el;
...
WebPad.getID = function() {
    this.EditorId++;
    return "SindhiEditor_" + this.EditorId;
}
...
WebPad.makeNormalEditor = function(el) {
    ...
    el.setAttribute("SindhiEditorId", null);
}

WebPad.makeSindhiEditor = function(el) {
...
WebPad.makeSindhiEditorById = function(idx) {
    var el = document.getElementById(idx);
    WebPad.makeSindhiEditor(el);
}
...
WebPad.init = function() {
    ...
    WebPad.showStatus(WebPad.STATUS_ID, 'Sindhi Editor Loaded', 5000);
    window[WebPad.NAME] = WebPad.toggleSindhiEditor;
    WebPad.toggleSindhiEditor();
};

WebPad.toggleSindhiEditor = function(ex) {

    var editorID = ex.getAttribute("SindhiEditorId");
    if (editorID && editorID != "null") {
        WebPad.makeNormalEditor(ex);
    }
    else {
        WebPad.makeSindhiEditor(ex);        
    }
};
    ...
    for (var i = 0; i < iframes.length; i++) try {
		...
		for (var x = 0, elem; elem = inputElems[x++];)
		  for (var y = 0, ex; ex = elem[y++];)
			...
			{  
				WebPad.addEvent(ex, 'dblclick', function() { WebPad.toggleSindhiEditor(this); });
			}
        
        if ((iframeDocument.designMode.toUpperCase() == "ON" || iframeDocument.body.contentEditable.toUpperCase() == "TRUE") )
		{
			WebPad(iframe.contentWindow.document, 'dblclick', function() { WebPad.toggleSindhiEditor(iframe); });
		}
...
function initEditAreas() {
 ...
 function CheckElement(e)
 {
	...
	{
		WebPad.toggleSindhiEditor(targ);
	}
 }
  
WebPad.addEvent(window.document, 'click', function(e){CheckElement(e);});
WebPad.addEvent(window, 'load', function(e){
	...
	for (i = 0; i < allTextAreas.length; i++) 
	{
		...
		WebPad.toggleSindhiEditor(thisTextArea);
	}
	...
	for (var i = 0; i < allInputs.length; i++) 
	{
		...
		if(thisInput.type == 'text')
		{
			...
			WebPad.toggleSindhiEditor(thisInput);			
		}
	}
});
