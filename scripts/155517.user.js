...
/* ?????????????? version 3(GPL version 3 of the License),
   ????????????????? ???????????????????,
   ????????????????????????????
 */ 
...
$(document).ready(function(){
	var LangAR={}, CHINESE={
		'Save':'??','Clear':'??','Donate':'??','Strength':'??','Influence':'???','Political Party':'??','Military Unit':'??',
		'Auto-Memorize Quantity':'??????','Auto-Memorize Money Amount':'??????','Toggle to open Donate button':'??????????',
		'No donate links under this post!':'??????????!','Do you want to open all donate links?':'???????????????'	};
	...
	'tw'==lang?LangAR=CHINESE:$.each(CHINESE,function(k){LangAR[k]=k});
	if (homeRE.test(path)) {
		...
		$('.comment_head ul.tabs').append('<a href="javascript:;"><img class="eDonate" src="'+opeImg+'" style="padding-top:10px;" title="switch Donate button"></a><div style="display:inline; color:#B2B2B2;">' +LangAR['Toggle to open Donate button']+ '</div>');
		$('.comment_head ul.tabs img.eDonate').bind('click', function(){ cComment(true) }); return;
...
document.body.appendChild(script);
			...
			for (i=0; i<$('#member_listing tbody tr').length; i++) {
				...
				if (id!=myid&&null!=id) {
					e.find('td.avatar').prepend(
						...
						'<a class="cGolLink_' +id+ '" href="javascript:;"><img style="margin:1px;" src="' +golImg+ '" title="Donate Money"></a>' +
						'<a target="_blank" href="'+window.location.protocol+'//www.erepublik.com/' +lang+ '/economy/donate-items/' +id+ '"><img style="margin:1px;" src="' +donImg+ '" title="Donate"></a></div></div>');
				}
			}
	...
	function cComment(list) {
		if (list) {
			if (0!=$('.articlecomments .eDH').length) {
				...
				var n=($('.pager.comments li a.on').length==1) ? ($('.pager.comments li a.on').text()-1)*100 : 0;
			...
			$('.comment_head ul.tabs img.eDonate').attr('src',offImg).unbind('click').bind('click', function(){ cComment(false) });
		} else {
			0!=$('.articlecomments .eDH').length&&($('.articlecomments .eDH').css('display','none'));
			$('.comment_head ul.tabs img.eDonate').attr('src',opeImg).unbind('click').bind('click', function(){ cComment(true) });
		}
	}
	...
	function cStorage(storage_item) {
		...
		if ('UI'==storage_item) {
			...
			$('#dhmsg').text('The quantity has been recorded!').fadeIn(400).fadeOut(1600);
		} else if('Clear'==storage_item) {
			...
			$('#dhmsg').text('The quantity has been cleared!').fadeIn(400).fadeOut(1600);
		}
	}
	function cMoney(filling_item) {
		if ('UI'==filling_item) {
			...
			$('#dhmsg').text('Saved!').fadeIn(400).fadeOut(1600);
		} else if ('Clear'==filling_item) {
			...
			$('#dhmsg').text('Cleared!').fadeIn(400).fadeOut(1600);
		}
	}
