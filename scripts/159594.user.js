// ==UserScript==
// @name       Banana Battle Master
// @version    1.2
// @ori author   eCitizen Scyld
// @edit author  eCitizen DiM
...
function letsJQuery() {
  ...
  content.prepend('<div id="myStatBoxL" style="z-index:1; position: absolute; top: 430px; left: 18px; width: 200px; opacity: 0.9;"></div>' + '<div id="myOverBox" style="z-index:1; position: absolute; top: 268px; left: 285px; width: 200px; opacity: 0.9;"></div>');
  content.append('<div id="myStatBoxR" style="z-index:1; position: absolute; top: 405px; left: 540px; width: 200px; opacity: 0.9;"></div>');

  $j('div#enemy_defeated').before('<div id="MHP" style="position:relative;width:836px;float:left;clear:both;padding:3px;margin:5px 0;font-weight:bold;color:#000;text-align:center">' + '<big>Expect Energy Consumed:&nbsp;</big>' + '<input id="multihit_count" style="text-align:right;" type="text" size="3" maxlength="4" value="1" />' + '&nbsp;X 10 Energy&nbsp;' + '<button id="multihit_start">^BaNaNa^</button><br />' + '&nbsp;&nbsp;<input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food">&nbsp;Eat food when needed</label>' + '&nbsp;&nbsp;<input type="checkbox" id="multihit_bazooka" name="multihit_bazooka" checked="checked"><label for="multihit_bazooka">&nbsp;NOT use Bazooka</label><br />' + '<input type="checkbox" id="multihit_energy" name="multihit_energy"><label for="multihit_energy">&nbsp;USE Energy Bars if no more food</label>' + '<div id="multihit_message" style="padding:2px;color:#ff0000"></div></div>');
    ...
    if (opt.url.indexOf('/battle-stats/') > -1 && unsafeWindow.SERVER_DATA.onlySpectator != 1) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        if (typeof bh['stats']['current'][zone][division][att] != 'undefined') {
          ...
          top5ABH[i] += '<tr style="color:red;"><td>Total Damage</td><td>-*-</td><td><strong>' + aBHTotal + '</strong></td></tr>';
        }
        ...
        if (typeof bh['stats']['current'][zone][division][def] != 'undefined') {
          ...
          top5DBH[i] += '<tr style="color:red;"><td>Total Damage</td><td>-*-</td><td><strong>' + dBHTotal + '</strong></td></tr>';
        }
      }
