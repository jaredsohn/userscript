// ==UserScript==
// @name           Ikariam Research Times
// @version        0.6
// @author         HomerOfXi & PhasmaExMachina
// @description    Script to show research times for Ikariam 0.3.2+
// @homepage       http://userscripts.org/scripts/show/57209
...
// @history        0.6 Added research to the academy view
// @history        0.6 Added options to turn on/off each mod
// @history        0.6 Added auto update ability
// @history        0.5 Added progress bar with % of gathered research needed for each research item. 
// @history        0.5 Removed greasemonkey log messages
// @history        0.4 Fixed bug that occurred if you had exactly 24hrs left
// @history        0.4 Changed script to only run on necessary pages
// @history        0.4 Optimized some code for faster performance
// @history        0.3 Some more fixes for research points over 1,000
// @history        0.3 Added message logging to the error console
// @history        0.2 Added fix that enabled displaying times for researches over 1,000 points
// @history        0.1 Fixed typo that prevented research times needing 1day+ to not display
// @history        0.0 Initial Release

// ==/UserScript==

ScriptUpdater.check(57209, '0.6');
		...
		for(var i=0; i < researches.length; i++) {
			...
			if(hoursNeeded > 0) {	
				...
				if(showTime) { //only build string if option is true or does not exist
				  strTime = '<img src="/skin/resources/icon_time.gif" style="display:block; float:left;"/>' + '    ~ ' + ResearchTimes.getRemainingTime(hoursNeeded);
				}
				if(showPercent) { //only build string if option is true or does not exist
				  ...
				  strPercentage = percentage+"%<table width='75%' style='border:1px solid #775726;' ><tr><td width='"+percentage+"%' style='background-color:#775726'></td><td width='"+emptyPct+"%'></td></tr></table>";
				}
	...
	views:{
      ...
      var showTime = IkaTools.getVal('showTime');   
		  //create the settings panel
			...
			settingsPanel.innerHTML = '<h3 class="header"><span class="textLabel">Ikariam Research Times</span></h3><div class="content"><form action="index.php" method="GET"><input type=hidden name=view value="options"><table cellpadding="0" cellspacing="0">'
			    + '<tr><th>Show Research in Academy</th><td>' 
			    + '<input id="showAcademyResearch" type="checkbox" name="sar" ' + (showAcademyResearch ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Show Percent Complete</th><td>' 
			    + '<input id="showPercent" type="checkbox" name="sp" ' + (showPercent ? 'checked' : '')
					+ '></td></tr>'
			    + '<tr><th>Show Time Remaining</th><td>' 
