// ==UserScript==
// @name           Wall Manager Sidekick fl)
// @description    Assists Wall Manager with football life posts
...
// @version        001
// @require        http://sizzlemctwizzle.com/updater.php?id=<your_userscript_script_id_here>&days=1
...
(function() { 
	...
	function dock(){
		...
		var doorMark=$('wmDoor_app113382708698448');
		if (doorMark) return; //already posted to door

		var attachment={
			appID:'113382708698448',
			alias:'fl',
			hrefKey:'a',
			name:'footballlife',
			thumbsSource:'',
			...
			accText: {
				...
				*/
				flhelp : "Help your friend", flask : "Ask friends for materials", flcheck : "Check out acheivement",
			},

			tests: [
				{link:"go play",ret:"exclude"},
				{url:'ref=nf',ret:'flhelp'},
				{url:'ref=nf',ret:'flask'},
				{url:'ref=nf',ret:'flcheck'},
			...
			menu: {
			flsep2:{type:'separator',label:'Basic',section:["Football Life Manager Options"],kids:{
					
				}},
					flupdateSidekick:{type:'link',label:'Update Sidekick',href:'http://userscripts.org/scripts/source/108737.user.js'},
					flupdateIframeBlock:{type:'link',label:'Install Annoying Iframe Blocker',href:'http://userscripts.org/scripts/source/99045.user.js'},
				flsep:{type:'separator',label:'Gift',section:["Football Life Manager Options"],kids:{
					
				}},
				flhelp:{type:'checkbox',label:'Help your friend',kids:{
					flask:{type:'checkbox',label:'Ask friends for materials'},
					flcheck:{type:'checkbox',label:'Check out acheivement'},
				}},
		...
		door.appendChild(createElement('div',{id:'wmDoor_app113382708698448','data-ft':attString}));
	...
	//********************************************************************************
		...
		run : function (){
			...
			try{
				...
				//<object type="application/x-shockwave-flash" data="https://s3-eu-west-1.amazonaws.com/flclient/FLClient.swf?version=43.5.0.0" width="760" height="700" id="FLClient" style="visibility: visible; "><param name="wmode" value="transparent"><param name="bgcolor" value="#FFFFFF"><param name="allowScriptAccess" value="always"><param name="flashvars" value="svc=ft&amp;fb_user=100002610050483&amp;fb_access_token=113382708698448|2.AQD09Ep6v9hgQgAh.3600.1311951600.1-100002610050483|7JsBYu5tBH7An7xS8bTZgFGcHGA&amp;addr=112.175.228.84&amp;port=18987&amp;appid=113382708698448"></object>
				/*var gameLoaded=unsafeWindow.location.href.startsWith('http://apps.facebook.com/footballlife/?ref=bookmarks&count=0') && html.contains('version=43.5.0.0');
				*/
			} catch(e){window.setTimeout(main.run,500);return;}
		...
		*/
		//*************************************************************************************
			...
			if (html.find('style="visibility: visible; "'))statusCode=-6; //already claimed
