...
// --------------------------------------------------------------------
// ==UserScript==
// @name           TribalWars Enhancer v3 para Trampa
// @version        3a
// @namespace     جهراوي- jahrauuy@hotmail.com
// @description    Ads Remover. 15x15 Map. Shortcuts Bar. TW-Tools quick-access. Multi-Language and more!
// @include        http://ae*.guerrastribales.ae/*
		...
		'enhance_game' : function() { // Add the premium-like toolbar
 		//make sure twe.curVillage is initalized
			...
			setFunc("tool_info", function(type){
				...
				if (type == 'player') {
					window.open("http://" + twe.World + ".twplus.org/globalsearch/?q=" + escape(tribe_name) + "&globalsearch_submit=Buscar");
				} else {
					window.open("http://" + twe.World + ".twplus.org/globalsearch/?q=" + escape(ally_name) + "&globalsearch_submit=Buscar");                                        
				}
			});
...
*/
 ...
 _generate_report : function() {
	 ...
	 var time;
	 if(/tribalwars.ae/.test(location.host)) {
