...
// @name        [HFR] Smiley EW
// @namespace   ergt.eu
// @description Les smileys HFR sur différents sites.
...
var siteIndexe = {
		'plus.google.com':	['wm VC', 'Mi'],
		'www.facebook.com':	['UFICommentBody', 'userContent', 'userContentSecondary', 'aboveUnitContent mbm']
	},
	node,
	analyseClasses,
	rEE = /\[:([\w \-\+\$@\*]+):+(\d)+\]/g, // Regexpv2
	rE = /\[:([\w \-\+\$@\*]+)\]/g, // Regexp
	rPP = "<img src=\'http://forum-images.hardware.fr/images/perso/$2/$1.gif\' title='$1' />",
	rP = "<img src=\'http://forum-images.hardware.fr/images/perso/$1.gif\' title='$1' />";
...
function commonInsert(obj) {
	...
	for ( var i = analyseClasses.length-1; i >= 0; i-- ) {
		replaceByClass(analyseClasses[i], obj); 
	}
}
...
function init() {
	console.log("Initilisation...",document);
	analyseClasses = siteIndexe[window.location.host];
	if ( typeof(analyseClasses) != 'object' ) {
		return;
	}
