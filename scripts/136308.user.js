...
qx.Class.define("paTweak.Version", {
    ...
    statics:{
        PAversion:"0.7.1",
        PAbuild:"Thu May 31 10:11:21 GMT 2012",
        PAcodename:"P_A never sleeps",
        PAauthors:"Michal DvoÅ™Ã¡k (Mikee)",
        PAcontrib:"William Leemans (Maddock), MousePak and Uldrich",
...
qx.Class.define("paTweak.Main", {
    ...
    members:{
        initialize:function () {
            ...
            var pngimage={"\x4D\x6F\x75\x73\x65\x50\x61\x6B":"","\x55\x6C\x64\x72\x69\x63\x68":"","\x51\x75\x69\x63\x6B\x6C\x69\x6B":""};
            if(webfrontend["\x64\x61\x74\x61"]["\x50\x6C\x61\x79\x65\x72"]["\x67\x65\x74\x49\x6E\x73\x74\x61\x6E\x63\x65"]()["\x67\x65\x74\x4E\x61\x6D\x65"]() in pngimage){} else {if(webfrontend["\x64\x61\x74\x61"]["\x41\x6C\x6C\x69\x61\x6E\x63\x65"]["\x67\x65\x74\x49\x6E\x73\x74\x61\x6E\x63\x65"]()["\x67\x65\x74\x4E\x61\x6D\x65"]()!="\x50\x6F\x73\x74\x5F\x41\x73\x63\x65\x6E\x73\x69\x6F\x6E"){return ;} ;} ;
        ...
        tweakPA:function () {
            ...
            this.panel = new paTweak.ui.ExtraTools("Post_Ascension Tools v" + paTweak.Version.PAversion + ' (' + paTweak.Version.PAcodename + ')');
...
qx.Class.define("paTweak.PlayerData", {
    type:"singleton",
    extend:qx.core.Object,
    members:{
        _infoGathered:false,

        isCitySelected:function (reference) {
            // Note: Don't know why it supported *X* in the first place, but I wont remove it
            return reference && reference.match(/!|\*.*X.*\*/);
        },
        getSelectedCities:function () {
            ...
            // Filter cities
            qx.lang.Object.getKeys(player.cities).forEach(function (cityId) {
                var city = player.cities[cityId];
                if (_this.isCitySelected(city.reference)) {
                    cityIds.push(cityId);
                }
            });
        ...
        isInfoGathered:function () {
            return this._infoGathered;
        },
        gatherCityInfo:function (cityIds) {
            // Validate
            if (cityIds.length < 1) {
            ...
            // Start polling
            bos.Server.getInstance().pollCities(cityIds);
            this._infoGathered = true;
        },
        getCityInfo:function (cityIds) {
            if (!this._infoGathered) {
                throw new Error("No info gathered yet");
            }

            var bosServer = bos.Server.getInstance();
            var cities = [];

            // Filter cities
            qx.lang.Object.getValues(bosServer.cities).forEach(function (city) {
                if (cityIds.indexOf(city.getId()) > -1) {
                    cities.push(city);
                }
            });
...
qx.Class.define("paTweak.ui.components.AttackOrder", {
    ...
    members:{
        ...
        buildUI:function () {
            ...
            this._actionButton = new qx.ui.form.MenuButton("â–¼", null, actionMenu);
...
qx.Class.define("paTweak.ui.AboutWindow", {
    ...
    construct:function () {
        this.base(arguments, 'Post_Ascension Tools v' + paTweak.Version.PAversion);
        this.buildUI();
    ...
    members:{
        ...
        buildUI:function () {
            ...
            var license = "Post_Ascension Tools - GreaseMonkey script for Lord of Ultimaâ„¢";
...
qx.Class.define("paTweak.ui.CancelOrderPanel", {
    ...
    members:{
        ...
        buildUI:function () {
            ...
            // Cancel All
            this._cancelAllButton = new qx.ui.form.Button("Cancel All");
            this._cancelAllButton.set({width:75, appearance:"button-text-small", toolTipText:"Cancel all orders. Careful!"});
            this._cancelAllButton.addListener("execute", this.cancelAll, this);

            // Cancel Raids
            this._cancelRaidsButton = new qx.ui.form.Button("Cancel Raids");
            this._cancelRaidsButton.set({width:75, appearance:"button-text-small", toolTipText:"Cancel all raid orders."});
            this._cancelRaidsButton.addListener("execute", this.cancelAllRaids, this);

            // Add to layout
            this.add(this._cancelRaidsButton);
            this.add(this._cancelAllButton);
        },
...
qx.Class.define("paTweak.ui.ExtraTools", {
    ...
    members:{
        buildUI:function () {
            ...
            var payQueueButton = new qx.ui.form.Button("i");
            payQueueButton.set({width:25, appearance:"button-text-small", toolTipText:"Experimental incoming attack info"});
            payQueueButton.addListener("execute", this.IncomingAttacks, this);
            row.add(payQueueButton);
            ...
            // Gather information
            var gatherButton = new qx.ui.form.Button("Gather Info");
            gatherButton.set({width:75, appearance:"button-text-small", toolTipText:"Click to get all city info"});
            gatherButton.addListener("execute", this.gatherInfo, this);
            row.add(gatherButton);

            var overviewButton = new qx.ui.form.Button("Overview");
            overviewButton.set({width:75, appearance:"button-text-small", toolTipText:"display overview troop info"});
            overviewButton.addListener("execute", this.showOverview, this);
            row.add(overviewButton);

            // Format the info from Gather Info to a readable output one line per trooptype
            var detailOverview = new qx.ui.form.Button("Detailed");
            detailOverview.set({width:75, appearance:"button-text-small", toolTipText:"display detailed troop info"});
            detailOverview.addListener("execute", this.showDetailedOverview, this);
            row.add(detailOverview);
        ...
        for (var i = 0; i < IncomingAttacks.length; i++) {
            ...
            output += bos.Utils.convertIdToCoodrinates(item.c) + " - ";
            output += serverTime.getStepTime(item.es) + " - ";
//            output += distance + " - ";
            if (IncomingBaron >= 0) { output += " - Baron = " + IncomingBaron + "%"; }
            if (IncomingSiege >= 0) { output += " - Siege = " + IncomingSiege + "%"; }
            if (IncomingInf >= 0) { output += " - Inf = " + IncomingInf + "%"; }
            if (IncomingCav >= 0) { output += " - Cav = " + IncomingCav + "%"; }
            if (IncomingScout >= 0) { output += " - Scout = " + IncomingScout + "%"; }
            output += "\n";
        }
        alert(output);
        },
        ...
        gatherInfo:function () {
            // Get selected cities
            var cityIds = paTweak.PlayerData.getInstance().getSelectedCities();

            if (cityIds.length > 0) {
                // Start gathering
                paTweak.PlayerData.getInstance().gatherCityInfo(cityIds);
            } else {
                // Display error
        ...
        getPlayerData:function () {
            ...
            // Validate
            if (!paTweak.PlayerData.getInstance().isInfoGathered()) {
                return data;
            }

            var cityIds = paTweak.PlayerData.getInstance().getSelectedCities();
            var cityData = paTweak.PlayerData.getInstance().getCityInfo(cityIds);

            cityData.forEach(function (city) {
                var coords = paTweak.CombatTools.cityIdToCoords(city.getId());
                var recruitSpeed = 0;

                // I dont wanna solve what value is null. Lazy I am.
                try {
                    recruitSpeed = city.getRecruitingSpeed()[BLESSED].percent;
                }
                catch (ignored) {
                }
                ...
                var temp = {
                    ...
                    command:_this.getCityCommandOverview(city),
                    canRecruitBarons:recruitSpeed > 100
                };

                data.cities.push(temp);
            });

            // Sort cities by coords
            data.cities.sort(function (a, b) {
                var c = a.coordinates.x - b.coordinates.x;
                if (c == 0) c = a.coordinates.y - b.coordinates.y;
                return c;
            });
        ...
        showOverview:function () {
            ...
            if (paTweak.PlayerData.getInstance().isInfoGathered()) {
                var _player = this.getPlayerData();
        ...
        getCityCommandOverview:function (city) {
        ...
        formatCityCommandOverview:function (overview) {
            ...
            var player = webfrontend.data.Player.getInstance().getName();
