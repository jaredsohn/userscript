...
(function(){
    ...
    setTimeout(function(){
        ...
        var el = $('<a href="#">Căutare</a>').click(mainworld);
    ...
    function init(){
        ...
        $("#ally").autocomplete(M.alliances, {
            ...
            autoFill: false,
            formatItem: function(row, i, max){
                return row.name + ' - ' + row.members + ' membri';
            },
            formatMatch: function(row, i, max){
                return row.name;
            },
        ...
        $("#player").autocomplete(M.players, {
            ...
            autoFill: false,
            formatItem: function(row, i, max){
                return row.name + ' ' + row.points + ' puncte';
            },
            formatMatch: function(row, i, max){
                return row.name;
            },
    ...
    function filter(cfg){
        ...
        _log('Filter using:');
        _log(cfg);
        $('#grw_info').html('S-au găsit ' + res.length + ' polisuri ');
        if (cfg.filter === 'ally') {
        ...
        function sortByTowns(r){
            ...
            $.each(r, function(i, o){
                ...
                //html += '<a href="#' + o.town.id + '">' + o.town.name +' ('+o.town.points+ 'puncte)</a>';
                if (o.player) {
                    html += ' [' + o.player.name + ']';
                }
                if (o.ally) {
                    html += ' din alianţa ' + o.ally.name;
                }
                if (o.player) {
        ...
        function filterMain(){
            ...
            if (cfg.ally) {
                ...
                if (myalliance) {
                    ...
                    console.error('Alianţa nu a fost găsită: ' + cfg.ally);
                }
            }
            _log(cfg);
            var res = [];
    ...
    function formatAlly(cfg, a){
        return a.name + ' ' + a.members + ' members, ' + a.points + ' puncte, rang ' + a.rank + ', ' + a.villages + ' polisuri';
    }
    ...
    function formatPlayer(cfg, p, withAlly, withName){
		...
		t+=p.points + ' puncte, rang ' + p.rank + ', ' + p.towns + ' polisuri';
        if (withAlly && p.ally) {
    ...
    function renderHtml(cb){
        ...
        '<p><label>Alianţă</label><input type="text" id="ally" value=""/><input type="hidden" id="hally" value=""/></p>' +
        '<p><label>Jucător</label><input type="text" id="player" value=""/><input type="hidden" id="hplayer" value=""/></p>' +
        '<p><label>Marea</label><input type="text" id="sea" value=""/></p>' +
        '<p><label>Puncte</label><input type="text" id="points" value=""/></p>' +
        '<p><label>Distanţă</label><input type="text" id="distance" value=""/></p>' +
        '<p><label>_de la</label><input type="text" id="town" value=""/><input type="hidden" id="htown" value=""/></p>' +
        '<p><label>Fantomă</label><input type="checkbox" id="ghost"/></p>' +
        '<p><label>BB-Code</label><input type="checkbox" id="bbcode"/></p>' +
        '<p><label>Filtru</label><select id="filter"><option value="town">După polisuri</option><option value="player">După jucător</option><option value="ally" selected="selected">După alianţă/polisuri</option></select></p>' +
        '<input id="search" type="button" value="Căutare"></input><br/>' +
        '<a href="/data/alliances.txt">alianţe</a> - <a href="/data/players.txt">jucători</a> - <a href="/data/towns.txt">polisuri</a><br/>' +
        '</div>' +
