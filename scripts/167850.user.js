// ==UserScript==
// @name                豆藤·改
// @namespace           http://userscripts.org/scripts/show/167850
// @description         默认不勾选赞助豆藤，增加仅作用于豆瓣
// @include             http://douban.com/*
// @include             http://*.douban.com/*
// @include             http://douban.fm/*
// @include             http://*.douban.fm/*
// @version             2013.0519.1846
...
function init(){
	...
	function sw_set(ch){
		...
		if (ch){
			...
			db_read('auto_hide',true);//自动隐藏工具条
			db_read('tidybar', false);//读取紧凑工具条样式
			db_read('requote', false);//允许引用、回复某人发言
			db_read('settler', true);//读取浮动定位工具选项
			db_read('hide_grpinfo',false);//自动隐藏小组介绍
			db_read('friendsTool',false);//友邻广播下方增加快捷工具条
			// db_read('multiSearch',true);//读取全站搜索选项
			db_read('slideBar',true);//开启顶部滑动导航条
			db_read('autoElevator',false);//开启自动电梯
			db_read('exBoard',true);//读取留言板增强选项
			db_read('musicLink',true);//读取音乐下载链接选项
			db_read('sortGroup',false);//开启小组分类
			db_read('alb_big',true);//中键相册大图
			db_read('highslide',false);//相册浏览模式
			db_read('loadAllPic',true);//一键显示全部大图
			db_read('down_photo',true);//批量下载大图
			db_read('supportHelper',false);//通过在卓越、淘宝商城上买书支持豆藤
			db_read('addPageLoader',true);//加载下一页功能
			db_read('autoLoadPage',false);//自动加载下一页
	...
	*/
	...
	var Douban = /douban\.(com|fm)/i.test(location.href), doubanHelper = {name: '豆藤·改',id: '167850',version: '2013.0519.1846'};
