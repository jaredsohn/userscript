// ==UserScript==
// @name       Cap Online - Quest B2B Suggested Sell Price
// @namespace  http://use.i.E.your.homepage/
// @version    0.5 - add updateURL
// @description  Adds a suggested price for adding a request to the 
// @match      http://*.capitalism-online.com/eos/market-requests.php*
// @match      http://*.ratjoy.com/eos/market-requests.php*
// @copyright  2013+, WNZ
// @updateURL  https://userscripts.org/scripts/source/292096.user.js
// ==/UserScript==
...
function parsePrice(price) {
    price = price.replace("$", "");
    var suffixes = ["","k","M","B","T","Q","E","Z"];
    
    var priceParts = price.split(" ");
    var priceNumber = priceParts[0];
    ...
    {
        var priceSuffix = priceParts[1];
        for (var i = 0; i < suffixes.length; i++)
        {
            if (priceSuffix == suffixes[i])
            {
                return parseFloat(priceNumber) * Math.pow(1e3, i);
            }
        }
    }
}

function colorizeB2BListings() {
    $("#b2b_table tr:gt(0)").each(function() {
        var row = $(this);
        var productName = row.find("td.market_prod_name").html().split('<br>')[0];
        var productQuality = parseFloat(row.find("td:nth-child(3)").text());
        var productPriceCell = row.find("td:nth-child(5)");
        var productPrice = parsePrice(productPriceCell.text());
        var productBaseValue = productDb[productName] / 100;
        var suggestedPrice = (productBaseValue * (1 + productQuality / 50)).toFixed(2);
        var priceRatio = productPrice / suggestedPrice;

        if (!productPriceCell.attr('priceRatioPercentSet'))
        {
            productPriceCell.append("&nbsp;(" + (priceRatio*100).toFixed(0) + "%)");
            productPriceCell.css("white-space", "nowrap");
            productPriceCell.attr('priceRatioPercentSet', '1');
        }

        var hue = getHue(priceRatio);
        var saturation = "75%";
        var lightness = "75%";
        
        row.css("background-color", "hsl(" + hue + ", " + saturation + ", " + lightness + ");");
    });
}

function getHue(priceRatio) {
    if (priceRatio < .75)
    {
        //priceRatio = .75;
        priceRatio = 1.5;
    ...
    // .75 - 1.5 => 0 - 1
    priceRatio = priceRatio - .75;

    // 0 - 1 => 120 - 0
    return 120 - (priceRatio * 120);
...
GM_addStyle(".default_table td{padding: 4px !important}");
$(document).ajaxStop(colorizeB2BListings);
