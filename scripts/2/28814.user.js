...
// @author         QP
// @namespace      QP
// @description    Simplifies lots of Travian tasks - v3.5.0
...
/** Presentation info
----- Small description -----
<i>Quick</i> Scout/Fake/Hero/Farm<br>Delayed Send Troops<br><i>Plus</i> features - Map, ..<br>IGM Autolinks<br><i>Enhanced</i> Rally Point/Market/Residence/Palace/HM<br>Field/Building Levels<br>Quick Delete Reports<br>..<br>All langs & servers
...
<ul>
	...
	<li>Residence / Palace Culture Points
		<ul>
			<li>on the script, configure the variable about the type of server - default is speed server</li>
			<li>this will adjust the CPs needed for each level</li>
		</ul>
	</li>
...
<ul>
	...
	<li>Features to be added
		<ul>
			...
			<li>Plus 13x13 Map with Colors - only last learnt diplomacy in server - if you sit someone, check their alliance overview after logging into their account, instead retrieve the user from one of the MAP pages that is retrieved!!!!</li>
...
//	<meta http-equiv="content-type" content="text/html; charset=UTF-8">
	...
	function createElemKarte2Image(parentNode, src, indexX, indexY, onClickEvents, onMouseOverEvents, onMouseOutEvents) {
//		debug(DBG_HIGHEST, "[createElemKarte2Image] indexX " + indexX + " indexY " + indexY);

		var img = createElementAppend('img', parentNode);
		img.src = src;
		...
		img.setAttribute("onclick", onClickEvents);
		img.setAttribute("onmouseover", onMouseOverEvents);
		img.setAttribute("mouseout", onMouseOutEvents);
...
//				);
	...
	ajaxDocument(baseKarteUrl + coordZMoveXY(middleZ, -3, -3), "", "", ajaxMapForKarte2, [0, 0, div5]);
	ajaxDocument(baseKarteUrl + coordZMoveXY(middleZ, -3,  3), "", "", ajaxMapForKarte2, [0, 6, div5]);
	ajaxDocument(baseKarteUrl + coordZMoveXY(middleZ,  3,  3), "", "", ajaxMapForKarte2, [6, 6, div5]);
	ajaxDocument(baseKarteUrl + coordZMoveXY(middleZ,  3, -3), "", "", ajaxMapForKarte2, [6, 0, div5]);
...
*/
	...
	*/
	function ajaxMapForKarte2(doc, params) {
		debug(DBG_HIGHEST, "[ajaxMapForKarte2] ");
...
//			debug(DBG_HIGHEST, "[ajaxMapForKarte2] currImg.src " + currImg.src);
...
//			debug(DBG_HIGHEST, "[ajaxMapForKarte2] imgCoords " + imgCoords);
			createElemKarte2Image(params[2], currImg.src,
									...
									"map('"+playerInfo.dname+"','"+playerInfo.name+"','"+playerInfo.ew+"','"+playerInfo.ally+"','"+playerInfo.x+"','"+playerInfo.y+"')",
									"map('','','','','000','000')"
									);
		}
	}
	...
	createElemKarte2AreaArrow(map6, coordZMoveY(middleZ,  7), "762,115,30", mapSavedLang[3]);	// north
	createElemKarte2AreaArrow(map6, coordZMoveX(middleZ,  7), "770,430,30", mapSavedLang[4]);	// east
	createElemKarte2AreaArrow(map6, coordZMoveY(middleZ, -7), "210,430,30", mapSavedLang[5]);	// south
	createElemKarte2AreaArrow(map6, coordZMoveX(middleZ, -7), "200,115,30", mapSavedLang[6]);	// west
...
//	allianceTag = allianceTag.split(" - ")[0];
	...
	for(var i=0; i<allianceColumn.snapshotLength; i++) {
		var currAlliancesCell = allianceColumn.snapshotItem(i);
		var arrCurrAlliances = currAlliancesCell.textContent.split(" - ");
...
/**
* setWarningColorForDiplomacy
*/
function setWarningColorForDiplomacy(node, diplomacy, otherAlliance, ownAlliance) {
	return setConfigColorForDiplomacy(node, diplomacy, otherAlliance, ownAlliance,
		DEF_COLOR_DIPLOMACY_WAR, DEF_COLOR_DIPLOMACY_NAP, DEF_COLOR_DIPLOMACY_ALLY);
}
...
function setColorForDiplomacy(node, diplomacy, otherAlliance, ownAlliance) {
	return setConfigColorForDiplomacy(node, diplomacy, otherAlliance, ownAlliance,
		DEF_COLOR_DIPLOMACY_ALLY, DEF_COLOR_DIPLOMACY_NAP, DEF_COLOR_DIPLOMACY_WAR);
}


/**
* setConfigColorForDiplomacy
*/
function setConfigColorForDiplomacy(node, diplomacy, otherAlliance, ownAlliance, colorAlly, colorNap, colorWar) {
	...
	if (diplomacyGroup > -1) {
		switch (diplomacyGroup) {
			case DEF_DIPLOMACY_ALLY: node.style.backgroundColor = colorAlly; break;
			case DEF_DIPLOMACY_NAP:  node.style.backgroundColor = colorNap;  break;
			case DEF_DIPLOMACY_WAR:  node.style.backgroundColor = colorWar;  break;
			default: dbg("[setColorForDiplomacy] Invalid diplomacy group " + diplomacyGroup);  break;
		}
	}
	return ((diplomacyGroup == DEF_DIPLOMACY_ALLY) || (diplomacyGroup == DEF_DIPLOMACY_NAP));
}
...
function transformPageWWStatistics_addCoordsForWWVillages() {
	...
	xpEvalFirst('//div[@id="lmid2"]/table/tbody/tr[2]/td[2]').width = "";
...
*/
function retrieveDorf2Loyalty() {
	var loyalty = xpEvalFirst('//div[@id = "lmid2"]/div[@class="dname"]/h1/div[@class="f7 c5"]');
	if (loyalty) {	// loyalty not at 100%
		return parseInt(loyalty.innerHTML.split(" ")[1]);
	}
	return 100;
}
...
function transformPageDorf2_addBuildingLevels() {
	...
	debug(DBG_HIGHEST, "[transformPageDorf2_addBuildingLevels] "
	+ "\n d2info " + d2info
	);
...
function retrievePageDorf1Or2_langLevel() {
	...
	debug(DBG_HIGHEST, "[retrievePageDorf1Or2_langLevel] constructionType.snapshotLength " + constructionType.snapshotLength);
	if (constructionType.snapshotLength>0) {
		debug(DBG_HIGHEST, "[retrievePageDorf1Or2_langLevel] constructionType.snapshotItem(0).innerHTML " + constructionType.snapshotItem(0).innerHTML);
...
//		var buildingType = /g(\d+)b?.gif$/.exec(buildingImgs.snapshotItem(i).src);
		var buildingType = /g(\d+)(b|_\d)?.gif$/.exec(buildingImgs.snapshotItem(i).src);
		buildingType = ((buildingType==null) ? -1 : buildingType[1]);

		var buildingTypePlace = 8;
		if (buildingType != 40) {
			buildingTypePlace = buildingImgs.snapshotItem(i).className.substr(1);
...
debug(DBG_NORMAL, "[retrievePageDorf2_Info] i " + i
	...
	// join the 2 arrays
	for(var i=0; i<22; i++) {
//		d2info += "," + ((buildType[i]==undefined)?-1:buildType[i]) + "_" + ((buildLevel[i]==undefined)?-1:buildLevel[i]);
...
function retrievePageDorf1_Info() {
	...
	debug(DBG_NORMAL, "[retrievePageDorf1_Info] dorf1Info " + dorf1Info);
}
...
function createSendQuickFarmSendTroopsLink(pname, nodeA, linkText, tribe) {
	...
	newButton.addEventListener('mouseover', function() {
			var inputNode = xpEvalFirst('//input[@name="' + pname + '"]');
...
function transformPageIGMsList_addSelectAllCheckbox() {
	...
	var buttonsRow = xpEvalFirst('//div[@id="lmid2"]/form/table/tbody/tr[last()]');
...
function actionPageReportsTradeList_deleteAllReportsOfGivenType() {
	...
	if (checkboxes.snapshotLength == 0) {
		gmReset_ReportsAction();
		var allLink = xpEvalFirst('//div[@id="lmid2"]/p[@class="txt_menue"]/a[not(contains(@href,"&t="))]');
		document.location.href = allLink.href;
		return;
	}

	document.getElementsByName("s10")[0].click();
	var deleteButton = xpEvalFirst('//div[@id="lmid2"]/form/table/tbody/tr/td/input[@class="std"][1]');
...
function transformPageReportList_addDeleteByReportTypeButtons() {
	var deleteButton = xpEvalFirst('//div[@id="lmid2"]/form/table/tbody/tr/td/input[@class="std"][1]');
	var deleteLang = deleteButton.value;
	deleteLang = deleteLang.substr(0, 1).toUpperCase() + deleteLang.substr(1);

	var form = xpEvalFirst('//div[@id="lmid2"]/form');
...
function transformPageResidenceOrPalaceCulturePoints_addCPsForVillages() {
	var culturePointsLink = xpEvalFirst('//div[@id="lmid2"]/p[@class="txt_menue"]/a[contains(@href, "build.php?id=")][contains(@href, "&s=2")]');
	var culturePointsLang = culturePointsLink.innerHTML;

	var divLmid2 = document.getElementById("lmid2");

	var currentTotalCPs = xpEvalFirst('//div[@id="lmid2"]/p[not(@class)][2]/b[1]').innerHTML;
 	var currentTotalCPsPerDay = xpEvalFirst('//div[@id="lmid2"]/table/tbody/tr[2]/td/b').innerHTML;
	var villagesLang = xpEvalFirst('//div[@id="lright1"]/a[1]/span').innerHTML.replace(":","");
...
* transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops
*/
function transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops() {
	debug(DBG_HIGHEST, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] ");
	var incomingTroops = xpathEvaluate('//div[@id="lmid2"]/p[@class="b"]|//div[@id="lmid2"]/table[@class="tbg"]');
	var incomingTables = false;
	var activeVillageCoordZ = getActiveVillageCoordZ();
	for(var i=0; i<incomingTroops.snapshotLength; i++) {
		var currentNode = incomingTroops.snapshotItem(i);
		// PARAGRAPHs
		if (currentNode.nodeName == "P") {
			debug(DBG_NORMAL, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] i " + i +
					" P - currentNode.textContent " + currentNode.textContent);
			if (/\((\d+)\)/.test(currentNode.textContent)) {
				incomingTables = true;
			} else {
				incomingTables = false;
			}
			continue;
		}
		debug(DBG_NORMAL, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] i " + i +
				" incomingTables " + incomingTables + " currentNode.nodeName " + currentNode.nodeName + " currentNode.textContent " + currentNode.textContent);
		// TABLES
		if (incomingTables) {
			debug(DBG_NORMAL, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] incomingTables "
								+ incomingTables + " currentNode.innerHTML " + currentNode.innerHTML);

			var troopImagesRow = xpEvalContextFirst(currentNode, 'tbody/tr/td/img/../..');
			var tribe = getTribeFromDocumentAndTroopPosition(troopImagesRow);
			var timeToArrive = xpEvalContextFirst(currentNode, 'tbody/tr/td/table/tbody/tr/td/span[@id="timer1"]');
			var arrivalTime = xpEvalContextFirst(currentNode, 'tbody/tr/td/table/tbody/tr/td/span[not(@id)]/..');
			var timeToArriveInSeconds = timeColonSeparatedToValue(timeToArrive.innerHTML);
			var incomingCoordZ = getParamFromUrl(xpEvalContextFirst(currentNode, 'tbody/tr/td[2]/a[contains(@href, "karte.php?d=")]').href, "d");
			var distance = coordDistZtoZ(incomingCoordZ, activeVillageCoordZ);
			var unitIndex = getSlowestUnit(timeToArriveInSeconds, distance, tribe, CONFIG_TRAVIAN_SERVER_TYPE==DEF_TRAVIAN_SERVER_TYPE_SPEED, 0)
			debug(DBG_HIGHEST, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] "
						+ "\n tribe " + tribe
						+ "\n timeToArrive " + timeToArrive.innerHTML
						+ "\n arrivalTime " + arrivalTime.innerHTML
						+ "\n timeToArriveInSeconds " + timeToArriveInSeconds
						+ "\n incomingCoordZ " + incomingCoordZ
						+ "\n distance " + distance
						+ "\n unitIndex " + unitIndex
						);

			var titleCell = xpEvalContextFirst(currentNode, 'tbody/tr/td[2]');
			titleCell.appendChild(createUnitImage(tribe, unitIndex));
		}
	}
}
...
* createUnitImage
*/
function createUnitImage(tribe, index) {
	var unitGifNumber = index+1;
	switch (tribe) {
		case TRIBE_ROMAN:	unitGifNumber += 0; break;
		case TRIBE_TEUTON:	unitGifNumber += 10; break;
		case TRIBE_GAUL:	unitGifNumber += 20; break;
		case TRIBE_NATURE:	unitGifNumber += 30; break;
		case TRIBE_NATAR:	unitGifNumber += 40; break;
		case TRIBE_UNDISCLOSED:	unitGifNumber += 50; break;
		default:
			break;
	}
	var img = document.createElement("img");
	img.src = "img/un/u/" + ((index == -1) ? "geb" : unitGifNumber) + ".gif";
	return img;
}

/**
* getTribeFromDocumentAndTroopPosition
*/
function getTribeFromDocumentAndTroopPosition(rowWithImages) {
	
	var settlerImageSrc = xpEvalContextFirst(rowWithImages, 'td/img[contains(@src, "img/un/u/")][contains(@src, "0.gif")]').src;
	var imgNumber = parseInt(settlerImageSrc.substr(settlerImageSrc.indexOf("0.gif") - 1, 2));
...
/**
* getSlowestUnit
* Calculate which of the troops is the slowest incoming unit in an attack given the distance, time to arrive,
* tribe, server type and tournament square.
*/
function getSlowestUnit(timeInSeconds, distance, tribe, isSpeedServer, tournamentSquareLevel) {
	var speedMultiplier = (isSpeedServer ? 2 : 1);
	var longDistanceSpeedMultiplier = ((100 + (10 * tournamentSquareLevel)) / 100);
	var closeDistance = (distance < 30) ? distance : 30;
	var longDistance = (distance < 30) ? 0 : (distance - 30);
	var times = new Array();
	for(var i=0; i<DEF_TROOPS[tribe].length; i++) {
		var speed = 1000;
		if (i != scoutTroopIndex[tribe]-1) {
			speed = DEF_TROOPS[tribe][i][DEF_TROOPS_STATS_INDEX_SPEED] * speedMultiplier;
		}
		times[i] = 3600 * (closeDistance / speed);
		times[i] += 3600 * (longDistance / (speed * longDistanceSpeedMultiplier));
	}
	var minIndex = -1;
	var min = -1;
	for(var i=0; i<times.length; i++) {
		if (times[i] < timeInSeconds) {
			times[i] = 0;
		} else {
			if ((min<0)||(min>times[i])) {
				min = times[i];
				minIndex = i;
			}
		}
	}
	debug(DBG_HIGHEST, "[getSlowestUnit] "
				+ "\n timeInSeconds " + timeInSeconds
				+ "\n distance " + distance
				+ "\n tribe " + tribe
				+ "\n isSpeedServer " + isSpeedServer
				+ "\n tournamentSquareLevel " + tournamentSquareLevel
				+ "\n speedMultiplier " + speedMultiplier
				+ "\n longDistanceSpeedMultiplier " + longDistanceSpeedMultiplier
...
function transformPageRallyPoint_addCoordsForOwnVillageReinfsAway() {
	...
	for(var i=0; i<ownReinfsLinks.snapshotLength; i++) {
		...
		var distance = coordDistZtoZ(coordZ, activeVillageCoordZ);
		distance = Math.floor(distance*100)/100;
		currentLink.firstChild.innerHTML += " &nbsp; " + "<span class='QPcoords2'>" + distance + "</span>";
	}
}
...
function actionPageSendTroops_universalTroopsMove() {
	...
	xpEvalFirst('//input[@name="c"][@value="' + utm[DEF_UTM_ATTACKTYPEINDEX] + '"]').checked = true;
...
//		}, true);
		...
		randomDelay(237, 741, function() {okButton.click()});
		return true;
	}
}
...
function retrieve_SendTroopsPage_coordZ() {
	var coordX = xpEvalFirst('//input[@name="x"]').value;
	var coordY = xpEvalFirst('//input[@name="y"]').value;
...
/** Is the page of any field */
function isThisPageField() { return (isPageField(document.location.href)); }
function isPageField(url) {
	if (isThisPageAnyBuildingPage()) {
		var idParam = getParamFromUrl(url, "id");
		if ((idParam) && (idParam < 19)) {
			return true;
		}
	}
	return false;
...
function isThisPageTreasuryPage() {
	if (isThisPageAnyBuildingPage()) {
		var textBelowBuildingNameAndLevel = xpEvalFirst('//div[@id="lmid2"]/p[@class="f10"]');
		if (textBelowBuildingNameAndLevel) {
			if (textBelowBuildingNameAndLevel.textContent.indexOf("10") >= 0) {
				return true;
			}
		}
	}
...
/** Is the page of the market place to trade with the NPC merchant */
function isThisPageMarketNpc() {
	if (isThisPageAnyMarketPage()) {
		return (getParamFromUrl(document.location.href, "t") == 3);
	}
	return false;
}
...
function isPageReportAttackScout(url) {
	if (isPageReportAttack(url)) {
		var tribe = getTribeBySettlerTroopsInAnyPage(0);
		var attackUnitsQtty = xpathEvaluate('//table[@class = "tbg"][position() = 1]/tbody/tr[@class = "cbg1"]/../tr[position() = 3]/td');
...
//===========================================================================================================
//===========================================================================================================
//==========================  Checks for newer script version @ userscripts.org  ============================
//===========================================================================================================
//===========================================================================================================
...
//				if (uploadedVersion != DEF_SCRIPT_VERSION) {
				if (isVersionGreater(uploadedVersion, DEF_SCRIPT_VERSION)) {
...
* isVersionGreater
*/
function isVersionGreater(version, baseVersion) {
	var arrVersion = version.split(".");
	var arrBaseVersion = baseVersion.split(".");
	debug(DBG_NORMAL, "[isVersionGreater] "
			+ "\n arrVersion " + arrVersion
			+ "\n arrBaseVersion " + arrBaseVersion
			);
	for(var i=0; i<arrVersion.length && i<arrBaseVersion.length; i++) {
		if (arrVersion[i] < arrBaseVersion[i]) {
			return false;
		} else if (arrVersion[i] > arrBaseVersion[i]) {
			return true;
		}
	}
	debug(DBG_NORMAL, "[isVersionGreater] "
			+ "\n arrVersion.length " + arrVersion.length
			+ " < arrBaseVersion.length " + arrBaseVersion.length
			);
	return (arrVersion.length > arrBaseVersion.length);
}
...
var CONFIG_FEATURE_RESOURCES_INFO = 3;	// 0 - disable; 1 - production only; 2 - timer only; 3 - both
