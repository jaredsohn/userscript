...
// @include        http://baseball.fantasysports.yahoo.com/b1/22414*
// @include        http://baseball.fantasysports.yahoo.com/b1/2948*
...
// $LastChangedDate: 2013-09-30 13:53:04 -0500 (Mon, 30 Sep 2013) $
// ==/UserScript==
/*
	Updates:
	...
	30-Sep-2013 - Updated to handle getting career stats with Yahoo's new player page format.
*/
/*
Usage:
				...
				getCareerStatsButton.addEventListener( 'click', function(e)
				{
					...
					for ( var iPlayer = 0; iPlayer < playerElements.snapshotLength; iPlayer++ )
					{
						...
						if ( !isNaN( parseInt( owner.players[ iPlayer ].id, 10 ) ) )
						{
							...
							GM_xmlhttpRequest({
								method: 'GET',
								url: 'http://sports.yahoo.com/mlb/players/' + owner.players[ iPlayer ].id,
	...
	for ( var iTable = 0; iTable < tables.length; iTable++ )
	{
		...
		if ( matches )
		{
			...
			for ( var iRow = 2; iRow < table.rows.length; iRow++ )
			{
				...
				if ( /totals/i.test( row.cells[ 0 ].innerHTML ) )
				{
