// ==UserScript==
// @name          Gallery Viewer for Broadband
// @namespace     http://joemck.cjb.net/
// @description	Loads full-size pic instead of reduced one in image/discussion page in some galleries.
...
(function() {
  ...
  for (i=0; i < pics.length; i++) {
    var thisPic = pics[i];
    if (/*(thisPic.src.indexOf("/albums/") == -1) ||*/ (thisPic.src.indexOf("normal_") == -1)) {
    ...
    var idx = thisPic.src.indexOf("normal_");
    if (thisPic.src.indexOf("/",idx) != -1)
      ...
      continue;
    thisPic.src = thisPic.src.substr(0,idx)+thisPic.src.substr(idx+7);
