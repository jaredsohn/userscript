// ==UserScript==
// @name           Travian: Battle Analyse PL
// @version        4.4.1
// @author         pamoramix
// @email          wojo1562gmail.com
// @namespace      Travian
// @description    Battle Analyse 4.4.1 PL
...
switch(lang){
	case ".pl":
		langfile=["Suma ataku","Stracone surowce","Suma ochrony przeciw piechocie",
							"Suma ochrona przeciw kawalerii","Statystyki generalne","Roznica strat","Konsumpcja wojska",
							"Doswiadzczenie bohatera","Nosnosc","Stracone surowce","Ilosc zboza jedzonego przez wojsko",
							"Nie bylo bohatera","Bohater","Ilosc zabranych surowcow","Maksymalna ilosc zrabowanych surowcow",
							"Atak","Obrona"];
		break;
}
...
function generatereport(){
	...
	if (warsim != 1 && worksave == 1) {
		...
		button1.setAttribute("value", "zapisz raport");
		...
		travilogForm.appendChild(elem("span", "", "ukryj walczacych"));
		...
		travilogIFrame.innerHTML = "Czekaj";
...
function oazicinfo(){
	...
	if(tdl.snapshotItem(0).firstChild.src.match(/u\/(\d+)/)==undefined){
		id('tba_imgcopy').style.display='none';
		return;
	};
