// ==UserScript==
// @name            Yuwie Commentator
// @description     Provides an easier way to send comments to all of your friends and referrals at Yuwie.com. Recommend also using snaplinks extension for firefox.
// @namespace       http://r.yuwie.com/addictedgamer
// @include         *.yuwie.com/members/friends/*
// @include         *.yuwie.com/members/referrals/*
// @exclude          *.yuwie.com/members/friends/main.asp
function do_platypus_script() {
do_modify_url_it(window.document,'/HTML[1]',/\.yuwie\.com\/members\/friends\/details\.asp/gi,'.yuwie\.com/members/comments/send\.asp',true);
do_modify_url_it(window.document,'/HTML[1]',/\.yuwie\.com\/profile\//gi,'.yuwie\.com/members/comments/send\.asp',true,'&w=p');
...
function modify_single_url(doc, match_re, replace_string, node, crap) {
    if (node.href) {
node.href = node.href.replace(match_re, replace_string);
    if (crap) {
      if(node.href.indexOf('&w=p') != -1) {
        node.href = node.href + crap;
      }
    }
    };
};
function do_modify_url_it(doc, node, match_re, replace_string, global_flag, crap) {
    match_re = new RegExp(match_re);
    if (global_flag) {
var allurls = doc.getElementsByTagName('A');
for(var i = 0, url; url = allurls[i]; i++)
  modify_single_url(doc, match_re, replace_string, url, crap);
    } else {
modify_single_url(doc, match_re, replace_string, node, crap);
    };
};
