...
(function($){
  ...
  var getPlaylistDetails = function(playlist_id, playlist_title, playlist_owner, cb) {
    GM_xmlhttpRequest({ 
      ...
      onload:  function (res) {
        ...
        var video_id;     
        //previewElem.attr('data-spr').replace(/\/([0-9]+):jpeg_preview_sprite.jpg/, function(_m, _vid) { video_id = _vid*1; });
  ...
  waitFor('.pl_video_tabs ul.mo_tabs', function(){
    ...
    tabs.prepend('<li id="tab_myplaylists" class="pull-start mrg-end-md"><a id="tab_myplaylists" href="" class="js-tipsy foreground2 video_title_on_hover">Included By</a></li>');
    ...
    //Delete css rule so newly added tab fits on the page
    //var aCssRule = getCSSRule('.mrg-end-md');
    //aCssRule.style.marginRight='0px';
