...
// This puts a link into a search on LCLS library system from the movie page.
...
// select "IMDB - Lycoming County Library System Search 1.0", and click Uninstall.

//
// ==UserScript==
// @name          IMDB - Lycoming County Library System Search 1.0
// @namespace     http://userscripts.org/users/51482
// @description	  While viewing a title on IMDB, it will place a link to a search for said title in the Lycoming County Library System (Pennsylvania).
...
(function() {
	...
	//alert("ok, we've looked for the spot at amazon.com to drop content");
		...
		if (isbn)  {
			...
			GM_xmlhttpRequest( { 
				method:"POST", 
				url:'http://lyco.sirsi.net/uhtbin/cgisirsi/X/0/0/123?searchdata1="' + isbn + '"',
				data:'',
					...
					for (var i=0; i < arrContent.length; i++) {
						if ( arrContent[i].match(/Place Hold/)) {
							...
							temp = temp.split('"')
							temp = temp[0]
							placeHold = "http://lyco.sirsi.net" + temp;
					...
					if(!flagME){
						...
						itemType = "DVD";
						var mediaUrl = 'http://lyco.sirsi.net/uhtbin/cgisirsi/X/0/0/60/30/X';
						url = mediaUrl
						insertItemSearchText(itemAvailabilityDiv, "Lycoming County Library System Search Results", url, itemType);
					}
					else{
						itemType = "DVD";
						var mediaUrl = 'http://lyco.sirsi.net/uhtbin/cgisirsi/X/0/0/123?searchdata1="';
						url = mediaUrl + isbn + '"';
						insertItemSearchText(itemAvailabilityDiv, "Lycoming County Library System Search Results", url, itemType);
						/*title found*/
					}		
...
if (!GM_xmlhttpRequest || !GM_log) {
    alert('The LCLS imdb Lookup script requires Greasemonkey 0.5.3 or higher.  Please upgrade to the latest version of Greasemonkey.');
    return;
}