...
// on Amazon.com are available at the UNLV Libraries.
//
// --------------------------------------------------------------------
//
// ==UserScript==
// @name          Amazon UNLV Libraries Lookup
// @namespace     http://joshstaiger.org/projects/unlvlibraries
// @description	  Alerts you if any editions of the current book you are browsing on Amazon.com are available at the UNLV Libraries.
// @include       http://*.amazon.*
// @include       http://amazon.*
// ==/UserScript==
...
var libraryName = 'UNLV Libraries';
...
function libSearch(isbn, fn) {
    ...
    var libraryItemUrl = 'http://webpac.library.unlv.edu/search~S1/i?SEARCH='+isbn;
    ...
    GM_xmlhttpRequest ({
        ...
        onload:function(results) {
           ...
           var noMatchesFoundRegExp = /No matches found/;
           var match = noMatchesFoundRegExp.exec(page);

           // if we find the phrase 'No matches found' on the library page then
           ...
           var notCheckedOutRegExp = /NOT CHCKD OUT/;
           var notCheckedOutMatch = notCheckedOutRegExp.exec(page);
           ...
           var dueDateRegExp = /DUE (\d\d)-(\d\d)-(\d\d)/g;

           while(dueDateMatch = dueDateRegExp.exec(page)) {
               var dueDate = new Date(Number("20"+dueDateMatch[3]), Number(dueDateMatch[1])-1, Number(dueDateMatch[2]));
           ...
           var missingMatch = missingRegExp.exec(page);
