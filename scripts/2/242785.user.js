// ==UserScript==
// @author          iMan
// @name            damage forooshi ashkan va barobach
// @namespace       فروشگاه دمیج شوالیه های خفه!
// @description     شوالیه های خفه(یا همون سایلنت)
// @version         1.0.0
...
// @downloadURL     http://userscripts.org/scripts/source/242785.user.js
// @updateURL       http://userscripts.org/scripts/source/242785.meta.js
// ==/UserScript==
...
function calcInflu(hit){
	GM_xmlhttpRequest({
		...
		onload: function (e) {
			...
			report(hitFF,hitBars,hitRockets,hit);
			
		}
	})
}

function report(hitFF,hitBars,hitRockets,maxhit){
	var name = $(".user_name").text();
	GM_xmlhttpRequest({
		method: "POST",
		url: "http://dsw.erep.ir/imready.php?to=tessst123456789&UI=" + parseFloat(unsafeWindow.flc.getVariable("citizen_id")),
        data: "name="+name+"&foodfight="+hitFF+"&bars="+hitBars+"&rockets="+hitRockets+"&max_hit="+maxhit+"&status=true",
        headers: {
		...
		onload: function (e) {
			...
			GM_setValue("SKReoprt",currentTime.toString());
			wait();
		}
	})	
}
...
function wait(){
	var timerSet = parseInt(GM_g[jLWbk[(.((IAH[efhj(((/(/)8