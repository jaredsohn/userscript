...
// @name		ViDGet
// @version		1.4
// @description		Téléchargez des vidéos sur plusieurs sites de vidéos !
// @author		rmdu45
// @namespace		https://userscripts.org/users/rmdu45
// @icon		http://rmdu45.net84.net/images/ViDGet.jpg
...
// Links
var website = 'http://rmdu45.net84.net';
var contact = 'http://rmdu45.net84.net';
...
function createMyElement (type, content, event, action, target) {
  ...
  if (event == 'change') {
	...
	setMyOptions ('ViDGet_autoget', option['autoget']);
      }
	...
	setMyOptions ('ViDGet_definition', option['definition']);
	modifyMyElement (saver['buttonGet'] , 'div', 'Get', false);
	selectMyVideo ();
	if (option['autoget']) getMyVideo();
      }
	...
	setMyOptions ('ViDGet_container', option['container']);
	modifyMyElement (saver['buttonGet'] , 'div', 'Get', false);
	selectMyVideo ();
	if (option['autoget']) getMyVideo();
      }
...
function createMySaver () {
  ...
  styleMyElement (saver['panelLogo'], {height: panelItemHeight + 'px', border: '1px solid #F4F4F4', borderRadius: '3px', padding: '0px', display: 'inline', color: '#0000FF', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', cursor: 'pointer'});
  appendMyElement (saver['saverPanel'], saver['panelLogo']);

  /* Panel Video Menu */
  saver['videoMenu'] = createMyElement ('select', '', 'change', '', 'video');
  styleMyElement (saver['videoMenu'], {width: '200px', height: panelItemHeight + 'px', border: '1px solid #F4F4F4', borderRadius: '3px', padding: '0px', display: 'inline', backgroundColor: '#F4F4F4', color: '#00FF00', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', verticalAlign: 'baseline', cursor: 'pointer'});
  ...
  for (var videoCode in saver['videoList']) {
    ...
    styleMyElement (saver['videoItem'], {padding: '0px', display: 'block', backgroundColor: '#F4F4F4', color: '#0000FF', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', cursor: 'pointer'});
    appendMyElement (saver['videoMenu'], saver['videoItem']);
  }

  /* Panel Get Button */
  saver['buttonGet'] = createMyElement ('div', 'Télécharger', 'click', 'get', '');
  styleMyElement (saver['buttonGet'], {height: panelItemHeight + 'px', border: '1px solid #CCCCCC', borderRadius: '3px', padding: '0px 5px', display: 'inline', color: '#FF0000', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', cursor: 'pointer'});
  ...
  if (feature['autoget']) {
    saver['buttonAutoget'] = createMyElement ('div', 'Automatique', 'click', 'autoget', '');
    styleMyElement (saver['buttonAutoget'], {height: panelItemHeight + 'px', border: '1px solid #CCCCCC', borderRadius: '3px', padding: '0px 5px', display: 'inline', color: '#0000FF', fontSize: '12px', cursor: 'pointer'});
  ...
  if (feature['definition']) {
    ...
    styleMyElement (saver['buttonDefinition'], {height: panelItemHeight + 'px', border: '1px solid #CCCCCC', borderRadius: '3px', padding: '0px 5px', display: 'inline', color: '#00FF00', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', cursor: 'pointer'});
  ...
  if (feature['container']) {
    ...
    styleMyElement (saver['buttonContainer'], {height: panelItemHeight + 'px', border: '1px solid #CCCCCC', borderRadius: '3px', padding: '0px 5px', display: 'inline', color: '#00FF00', fontSize: '12px', textShadow: '0px 1px 1px #CCCCCC', cursor: 'pointer'});
...
function selectMyVideo () {
  ...
  if (option['definition'] == 'UHD') {
    if (vdoList['Ultra Haute Definition']) saver['videoSave'] = vdoList['Ultra Haute Definition'];
    else if (vdoList['Full Haute Definition']) saver['videoSave'] = vdoList['Full Haute Definition'];
    else if (vdoList['Haute Definition']) saver['videoSave'] = vdoList['Haute Definition'];
    ...
    if (vdoList['Full Haute Definition']) saver['videoSave'] = vdoList['Full Haute Definition'];
    else if (vdoList['Haute Definition']) saver['videoSave'] = vdoList['Haute Definition'];
    else if (vdoList['Standard Definition']) saver['videoSave'] = vdoList['Standard Definition'];
    else if (vdoList['Low Definition']) saver['videoSave'] = vdoList['Low Definition'];
    else if (vdoList['Very Low Definition']) saver['videoSave'] = vdoList['Very Low Definition'];
    ...
    if (vdoList['Haute Definition']) saver['videoSave'] = vdoList['Haute Definition'];
    else if (vdoList['Standard Definition']) saver['videoSave'] = vdoList['Standard Definition'];
    else if (vdoList['Low Definition']) saver['videoSave'] = vdoList['Low Definition'];
    else if (vdoList['Very Low Definition']) saver['videoSave'] = vdoList['Very Low Definition'];
  }
...
function getMyVideo () {
  ...
  if (saver['videoTitle']) {
    ...
    vdoD = vdoD.replace(/Ultra Haute Definition/, 'UHD');
    vdoD = vdoD.replace(/Full Haute Definition/, 'FHD');
    vdoD = vdoD.replace(/Haute Definition/, 'HD');
...
function getMyOptions () {
  var stAutoget = 'ViDGet_autoget';
  var stDefinition = 'ViDGet_definition';
  var stContainer = 'ViDGet_container';
  try {
...
function showMyMessage (cause, content) {
  ...
  if (cause == '!player') {
    ...
    if (cause == '!content') {
      var myNoContentMess = '<b>ViDGet:</b> Couldn\'t get the videos content. Please report it <a href="' + contact + '">here</a>.';
      modifyMyElement (saver['saverPanel'], 'div', myNoContentMess, false);
    }
    else if (cause == '!videos') {
      var myNoVideosMess = '<b>ViDGet:</b> Couldn\'t get any video. Please report it <a href="' + contact + '">here</a>.';
      modifyMyElement (saver['saverPanel'], 'div', myNoVideosMess, false);
    }
    else if (cause == '!support') {
      var myNoSupportMess = '<b>ViDGet:</b> This video uses the RTMP protocol which is not supported.';
      modifyMyElement (saver['saverPanel'], 'div', myNoSupportMess, false);
    }
    else if (cause == 'embed') {
      var myEmbedMess = '<b>ViDGet:</b> This is an embedded video. You can get it <a href="' + content + '">here</a>.';
...
if (page.url.indexOf('youtube.com/watch') != -1) {
  ...
  if (!ytPlayerWindow) {
	...
	'22': 'Haute Definition MP4',
	'34': 'Low Definition FLV',
	'35': 'Standard Definition FLV',
	'36': 'Low Definition 3GP',
	'37': 'Full Haute Definition MP4',
	'38': 'Ultra Haute Definition MP4',
	'43': 'Low Definition WebM',
	'44': 'Standard Definition WebM',
	'45': 'Haute Definition WebM',
	'46': 'Full Haute Definition WebM',
	'82': 'Low Definition 3D MP4',
	'83': 'Standard Definition 3D MP4',
	'84': 'Haute Definition 3D MP4',
	'85': 'Full Haute Definition 3D MP4',
	'100': 'Low Definition 3D WebM',
	'101': 'Standard Definition 3D WebM',
	'102': 'Haute Definition 3D WebM',
	'135': 'Standard Definition Video MP4',
	'136': 'Haute Definition Video MP4',
	'137': 'Full Haute Definition Video MP4',
	'138': 'Ultra Haute Definition Video MP4',
	...
	option['definitions'] = ['Ultra Haute Definition', 'Full Haute Definition', 'Haute Definition', 'Standard Definition', 'Low Definition', 'Very Low Definition'];
...
// =====DailyMotion===== //
  ...
  if (!dmPlayerWindow) {
    ...
    /* Get Videos */
    if (dmVideosContent) {
      var dmVideoFormats = {'stream_h264_hd1080_url': 'Full Haute Definition MP4', 'stream_h264_hd_url': 'Haute Definition MP4',
	...
	option['definitions'] = ['Full Haute Definition', 'Haute Definition', 'Standard Definition', 'Low Definition', 'Very Low Definition'];
...
// =====Vimeo===== //
  ...
  if (!viPlayerWindow) {
    ...
    /* Get Videos */
    if (viVideosContent) {
      var viVideoFormats = {'hd': 'Haute Definition MP4', 'sd': 'Low Definition MP4', 'mobile': 'Very Low Definition MP4'};
      var viVideoList = {};
	...
	feature['container'] = false;
	option['definitions'] = ['Haute Definition', 'Low Definition', 'Very Low Definition'];
	option['containers'] = ['MP4'];
	createMySaver ();
      }
      else {
...
// =====MetaCafe===== //
  ...
  if (!mcPlayerWindow) {
	...
	var mcVideoFormats = {'highDefinitionMP4': 'Haute Definition MP4', 'MP4': 'Low Definition MP4', 'flv': 'Low Definition FLV'};
	...
	option['definitions'] = ['Haute Definition', 'Low Definition'];
...
// =====Break===== //
  ...
  if (!brPlayerWindow) {
	...
	if (brVideoPath.match(/.wmv$/)) {
	  ...
	  brVideoFormats = {'6.mp4': 'Very Low Definition MP4', '1.flv': 'Low Definition FLV', '1.mp4': 'Low Definition MP4', '2.mp4': 'Standard Definition MP4', '3.mp4': 'Haute Definition MP4'};
	  brVideoPath = brVideoPath.replace(/1.flv$/, '');
	}
	else if (brVideoPath.match(/.mp4$/)) {
	  brVideoFormats = {'240.mp4': 'Very Low Definition MP4', '360.mp4': 'Low Definition MP4', '480.mp4': 'Standard Definition MP4', '720.mp4': 'Haute Definition MP4',};
	...
	option['definitions'] = ['Very Low Definition', 'Low Definition', 'Standard Definition', 'Haute Definition'];
	option['containers'] = ['MP4', 'FLV', 'Any'];
	createMySaver ();
      }
      else {
...
// =====FunnyOrDie===== //
  ...
  if (!fodPlayerWindow) {
    ...
    /* Get Videos */
    if (fodVideosContent) {
      var fodVideoFormats = {'v2500.mp4': 'Haute Definition MP4', 'v1800.mp4': 'Standard Definition MP4', 'v600.mp4': 'Low Definition MP4', 'v600.webm': 'Low Definition WebM', 'v110.mp4': 'Very Low Definition MP4'};
      var fodVideoList = {};
	...
	feature['container'] = false;
	option['definitions'] = ['Haute Definition', 'Standard Definition', 'Low Definition', 'Very Low Definition'];
	option['containers'] = ['MP4'];
	createMySaver ();
      }
      else {
...
// =====Mevio===== //
  ...
  if (!mePlayerWindow) {
    ...
    /* Get Videos */
    if (meVideosContent) {
      var meVideoFormats = {'mp4': 'Haute Definition MP4', 'm4v': 'Haute Definition M4V',  'mov': 'Haute Definition MOV', 'h264': 'Low Definition MP4', 'flv': 'Low Definition FLV'};
      var meVideoList = {};
  ...
  if (!blipPlayerWindow) {
	...
	if (blipMimeTypes[blipVideoMime]) {
	  ...
	  if (blipVideoURL && blipVideoHeight && blipVideoRole) {
	    ...
	    else if (blipVideoHeight >= 700) blipVideoDef = 'Haute Definition';
	...
	option['definitions'] = ['Haute Definition', 'Standard Definition', 'Low Definition'];
	option['containers'] = ['MP4', 'M4V', 'MOV', 'FLV', 'Any'];
	createMySaver ();
      }
      else {
