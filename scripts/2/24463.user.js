...
// @name		Travian: Resource++ v4 PL
// @namespace	http://userscripts.org/
// @description	Travian: Resource++ v4 PL v0.5  
...
function onLoad(){ // runs the differnt functions and procedures, 
	...
	switch(lang){ //load only the lang you need into memory
		...
		break;
		case '.pl':
		langfile[lang]=['Surowce potrzebne do rozbudowy:','nie zmieści się za', 'Brakuje','Przegląd','Surowce', 
		"",'Potrzebne surowce będą dziś o ','Potrzebne surowce będą jutro o ', 'Potrzebne surowce będą za: ', 
		' dni ', ' Koszt: ' , 'W magazynie/spichlerzu więcej o: ','Potrzebne surowce będą za: ','Mniej o','Więcej o', ' Wszystkie potrzebne surowce będą za: ','Balanced', 
		'Surplus depleted in: ','Upgrade ',' godzinna produkcja.','Neutral balance.' , 'Extend Warehouse' , 'Extend Granary' ];						
		break;
		case '.uk':
...
function resize(e){
	...
	if(!answer) {
		if(e.id=='ResBlock') {
			...
			resWidth=305; //305 default
		} else if(e.id=='NavLinks'){
			GM_setValue('NavLinksPx', 150);
			navWidth=180; // 180 default
		} else if(e.id=='ToolsBlock'){
			GM_setValue('ToolsBlockPx', 254);
			toolsWidth=305; // 305 default
		}
		return;
	}
	...
	if(e.id=='ResBlock') {
		...
		resWidth=answer; //305 default
	} else if(e.id=='NavLinks'){
		GM_setValue('NavLinksPx', answer);
		navWidth=answer; // 180 default
	} else if(e.id=='ToolsBlock'){
		GM_setValue('ToolsBlockPx', answer);
		toolsWidth=answer; // 305 default
	}
...
function toolMenu(vis){
		...
		con.style.height='15px';
		con.style.cssFloat='left';
		con.appendChild( document.createTextNode(  'Tool Bar' ) );
...
var button=document.createElement( 'input' );
	...
	formpost.appendChild( document.createTextNode( 'Współrzędne: ')  );
...
var button=document.createElement( 'input' );
	...
	button.name='s1';
	button.style.marginLeft='10px';
	formpost.appendChild( document.createTextNode( 'Gracz: ')  );
...
function quickLinks(vis){
	...
	var con=document.createElement('div');
		...
		con.style.height='15px';
		con.style.cssFloat='left';
		con.appendChild( document.createTextNode(  'Nawigacja' ) );
	...
	var onSelect=document.createElement('select');
		...
		onSelect.appendChild(makeOption('Zarządzanie jednostkami','#'));
		onSelect.appendChild(makeOption('Miejsce zbiórki','build.php?id=39'));
		onSelect.appendChild(makeOption('Wyślij jednostki','a2b.php?s=4'));
		onSelect.appendChild(makeOption('Symulator walk','warsim.php'));
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change',function(e){jump(e);},true);
		onSelect.appendChild(makeOption('Rynek','#'));
		onSelect.appendChild(makeOption('Wyślij surowce','build.php?gid=17'));
		onSelect.appendChild(makeOption('Kup surowce','build.php?gid=17&t=1'));
		onSelect.appendChild(makeOption('Sprzedaj surowce','build.php?gid=17&t=2'));
		onSelect.appendChild(makeOption('Handlarz NPC','build.php?gid=17&t=3'));
		cell.appendChild(onSelect);
		row.appendChild(cell);
		mymenu.appendChild(row);
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Raporty','#' )      );
		onSelect.appendChild( makeOption('Wszystkie','berichte.php' ) );
		onSelect.appendChild( makeOption('Pomoc','berichte.php?t=1' ) );
		onSelect.appendChild( makeOption('Handel','berichte.php?t=2' ) );
		onSelect.appendChild( makeOption('Atak','berichte.php?t=3' ) );
		onSelect.appendChild( makeOption('Pozostałe','berichte.php?t=4' ) );
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Wiadomości','#' )      );
		onSelect.appendChild( makeOption('Skrzynka odbiorcza','nachrichten.php' ) );
		onSelect.appendChild( makeOption('Napisz','nachrichten.php?t=1' ) );
		onSelect.appendChild( makeOption('Skrzynka nadawcza','nachrichten.php?t=2' ) );
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Statystyki','#' )      );
		onSelect.appendChild( makeOption('Gracze','statistiken.php' ) );
		onSelect.appendChild( makeOption('Osady','statistiken.php?id=2' ) );
		onSelect.appendChild( makeOption('Sojusze','statistiken.php?id=4' ) );
		onSelect.appendChild( makeOption('OFF','statistiken.php?id=31' ) );
		onSelect.appendChild( makeOption('DEF','statistiken.php?id=32' ) );
		onSelect.appendChild( makeOption('Ogólne','statistiken.php?id=0' ) );
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Sojusz','#' )      );
		onSelect.appendChild( makeOption('Podgląd','allianz.php' ) );
		onSelect.appendChild( makeOption('Forum','allianz.php?s=2' ) );
		onSelect.appendChild( makeOption('Chat','allianz.php?s=6' ) );
		onSelect.appendChild( makeOption('Ataki','allianz.php?s=3' ) );
		onSelect.appendChild( makeOption('Nowości','allianz.php?s=4' ) );
		onSelect.appendChild( makeOption('Opcje','allianz.php?s=5' ) );
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Gospodarka','#' )      );
		onSelect.appendChild( makeOption('Pole zboża','build.php?gid=4' ) );
		onSelect.appendChild( makeOption('Las','build.php?gid=1' ) );
		onSelect.appendChild( makeOption('Kopalnia gliny','build.php?gid=2' ) );
		onSelect.appendChild( makeOption('Kopalnia żelaza','build.php?gid=3' ) );
		onSelect.appendChild( makeOption('Magazyn surowców','build.php?gid=10' ) );
		onSelect.appendChild( makeOption('Spichlerz','build.php?gid=11' ) );
		onSelect.appendChild( makeOption('Kryjówka','build.php?gid=23' ) );
		onSelect.appendChild( makeOption('Rynek','build.php?gid=17' ) );
		onSelect.appendChild( makeOption('Młyn','build.php?gid=8' ) );
		onSelect.appendChild( makeOption('Huta stali','build.php?gid=7' ) );
		onSelect.appendChild( makeOption('Cegielnia','build.php?gid=6' ) );
		onSelect.appendChild( makeOption('Tartak','build.php?gid=5' ) );
		onSelect.appendChild( makeOption('Piekarnia','build.php?gid=9' ) );
		onSelect.appendChild( makeOption('Browar','build.php?gid=35' ) );	
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);	
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Militaria','#' )      );
		onSelect.appendChild( makeOption('Palisada','build.php?gid=33' ) );
		onSelect.appendChild( makeOption('Mur miejski','build.php?gid=31' ) );
		onSelect.appendChild( makeOption('Wały','build.php?gid=32' ) );
		onSelect.appendChild( makeOption('Koszary','build.php?gid=19' ) );
		onSelect.appendChild( makeOption('Traper','build.php?gid=36' ) );
		onSelect.appendChild( makeOption('Akademia','build.php?gid=22' ) );
		onSelect.appendChild( makeOption('Kuźnia','build.php?gid=13' ) );
		onSelect.appendChild( makeOption('Stajnia','build.php?gid=20' ) );
		onSelect.appendChild( makeOption('Warsztat','build.php?gid=21' ) );
		onSelect.appendChild( makeOption('Zbrojownia','build.php?gid=12' ) );
		onSelect.appendChild( makeOption('Duże kosary','build.php?gid=29' ) );
		onSelect.appendChild( makeOption('Duża stajnia','build.php?gid=30' ) );
		onSelect.appendChild( makeOption('Dwór bohaterów','build.php?gid=37' ) );
		cell.appendChild(  onSelect   );
		row.appendChild( cell );
		mymenu.appendChild(row);	
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Infrastruktura','#' )  );
		onSelect.appendChild( makeOption('Główny budynek','build.php?gid=15' ) );
		onSelect.appendChild( makeOption('Miejsce zbiórki','build.php?gid=16' ) );
		onSelect.appendChild( makeOption('Rezydencja','build.php?gid=25' ) );
		onSelect.appendChild( makeOption('Ambasada','build.php?gid=18' ) );
		onSelect.appendChild( makeOption('Pałac','build.php?gid=26' ) );
		onSelect.appendChild( makeOption('Ratusz','build.php?gid=24' ) );
		onSelect.appendChild( makeOption('Kamieniarz','build.php?gid=34' ) );
		onSelect.appendChild( makeOption('Targ','build.php?gid=28' ) );
		onSelect.appendChild( makeOption('Skarbiec','build.php?gid=27' ) );
		onSelect.appendChild( makeOption('Plac Turniejowy','build.php?gid=14' ) );
		cell.appendChild(  onSelect );
		row.appendChild( cell );
		mymenu.appendChild(row);	
	
	var listCoords = getPos(mymenu.id, '719px_180px').split('_');
		mymenu.style.top = listCoords[0];
		mymenu.style.left = listCoords[1];
		mymenu.style.position = 'absolute';
		mymenu.style.zIndex =100;
