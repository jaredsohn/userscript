// ==UserScript==
// @name	   Idle Dungeon Fight
// @namespace  http://www.zeerd.com/
// @version	0.2.2
// @description  auto fight
// @copyright  2013+, zeerd (原作者)  stingx(添加遍历副本)
...
function create_pd(){
    ...
    html += "<input type='checkbox' id='ipd_all' ><span title='遍历之前请先确保已经用非遍历模式把所有副本配置完成.' class='color2 f12 mr_10'>遍历全副本</span>";	
	html += "<br/>";
	...
	html += "<span class='color2 f12 mr_10'>间隔<input type=\"text\" id=\"ipd_interval\" name=\"ipd_interval\" value=\"4000\">毫秒战斗一次，</span>";
	html += "<span class='color2 f12 mr_10'>重复战斗<input type=\"text\" id=\"ipd_r_cnt\" name=\"ipd_r_cnt\" value=\"5\">次</span>";
 

	html += "<span style=\"display: block;\" id=\"ipd_btn\"><a href=\"javascript:_ipdStartPD();\" class=\"color4\">开始自动打副本</a></span>";
...
if (document.location.href.indexOf('marrla.com/Map.aspx') != -1){
	...
	function _ipdStartPD(){\n\
		if(document.getElementById(\'ipd_all\').checked)\n\
			ipdContinueAll(19);\n\
		else\n\
			ipdStartPD();\n\
	}\n\
	function ipdContinueAll(did){\n\
		//alert(did);\n\
		if (did==0){\n\
		document.getElementById(\'ipd_count\').innerHTML = "完成"; \n\
		window.open("QuitFight.aspx?t=4", "ipd_frame");\n\
		return;\n\
		}\n\
		var i = did;\n\
		if (i==6||i==8) \n\
			ipdContinueAll(--i);\n\
		var setting = window.localStorage.getItem("ipd_setting_"+i);\n\
        if (setting ==null)\n\
			ipdContinueAll(--i);\n\
		var key = setting.split("|");\n\
		var url_t = (key[0]=="true")?"t=a&":"";\n\
		var url_ext= (key[1]=="true")?"ext=true&":"ext=false&";\n\
		var url_did = "did=-"+i+"&";\n\
		var url_mg ="mg="+key[10]+"&";\n\
		var url_cgid = "cgid=";\n\
		if(i == "9" || i == "10"){\n\
			url_cgid += key[2] + "_" + key[3];\n\
		}\n\
		else if(i == "17" || i == "18" || i == "19"){\n\
			url_cgid += key[2] + "_" + key[3] + ",";\n\
			url_cgid += key[4] + "_" + key[5] + ",";\n\
			url_cgid += key[6] + "_" + key[7];\n\
		}\n\
		else{\n\
			url_cgid += key[2] + "_" + key[3] + ",";\n\
			url_cgid += key[4] + "_" + key[5];\n\
		...
		setTimeout(function(){\n\
			//alert("checked");\n\
			if	(window.frames["ipd_frame"].location.pathname=="/Map.aspx"){\n\
				//alert(window.frames["ipd_frame"].location.pathname!="/Map.aspx");\n\
				ipdContinueAll(--i);\n\
			}\n\
			else\n\
				ipdContinueAll(i);\n\
