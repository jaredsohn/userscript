// Amazon DVD Library Lookup
// Modified Sean LeBlanc's code which 
// Modified Josh Staiger's (excellent) code
// to adapt to LION search.

// TODO: Add in "other editions" search?
// TODO: General clean-up of unused code.

// Currently, this script will search for the book in the Colorado Jefferson County library.
// If not found there, it will search on Prospector, then Worldcat.
...
// select "Amazon - JeffCo Lookup 1.2.5", and click Uninstall.

//
// ==UserScript==
// @name          Amazon - DVD Lookup 1.2.5
// @namespace     http://sean-leblanc.blogspot.com
// @description	  While viewing a book on Amazon, this script will search for the book in the Colorado Jefferson County library. If not found there, it will search on Prospector. If not found there, it will try to search Worldcat. Also, it will try to detect when viewing an item that is a CD, DVD or VHS, and insert a search link into JeffCo Library to use on that page. Based on Josh Staiger's Wake County Library Lookup.
// @include       http://*.amazon.*
// @include       http://amazon.*
// @version 	  1.2.4
// ==/UserScript==
...
// 1.2.4 - Updated due to Amazon changes.

// 1.2.5 - Altered DVD/VHS detection.


var dbg = false;
var jeffcoLibraryName = 'LION Library';
var shortLibName = "LION";
...
function prospectorSearch(isbn, newDiv) {
	var xmlUrlProspector = "http://catalog.lioning.org/search/i" + isbn + "&searchtype=i";
	// Add another div at this point:
	var secondDiv = document.createElement('div');
	secondDiv.setAttribute('class', 'stub');
	newDiv.parentNode.insertBefore(secondDiv, newDiv.nextSibling);
...
function libSearch(isbn, newDiv) {
	var xmlUrlJeffco = "http://catalog.lioning.org/search/?searchtype=i&searcharg=" + isbn + "&SORT=&searchscope=9";
    var successText = "/request.gif";
	//var found = false;
...
var mediaUrl = "http://catalog.lioninc.org/search/X?searchtype=t&searcharg=";
var cdTypeUrlAppend = "&searchscope=28";
var dvdTypeUrlAppend = "&searchscope=29";
var vhsTypeUrlAppend = "&searchscope=26";
