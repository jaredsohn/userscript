...
// @author		Ace
// @author		Ray (Falcon)
// @author        The Ey3 (Tony) <---- Yes thats true
// ==/UserScript==
 ...
 * Revision 4.0.6  2007/07/14 18:25:00  Ace
 * Added buttons next to players name on the affair page for one click voting.
 * Fixed bug on overview page that didn't show Quick Links bar when there were no defenses
 *
 * Revision 4.0.7 2007/09/07
 * Added Storm links to top bar
 *
 * Revision 4.0.5c  2007/06/12 05:37:00  Ace
 * Added quick forum links to the top bar
 *
 * Revision 4.0.5b  2007/06/11 23:53:00  Ace
 * Added max attack needed for land grabs on universe page and alliance members page
 * Added target lookup to fleets page (c/o Gavry)
 * Added a rough guide to the attack and defense incoming on the planet status screen
 *
 * Revision 4.0.5  2007/03/21 20:55:50  TMX
 * Merged with Nevo+
 *
...
const scriptversion = '- S.K Special ';
const scriptversionID = 'DeVo+  ' + scriptversion;
const scriptTag = 'Top Secret edition';
...
var pCoords = new Array(4); // players planet co-ords
...
// score required for land capture
var attackPerLand = 286000;
var maxLandGrabPercent = .25;

// attack/defense values
var APPROXATTACK = 12721;
var APPROXDEFENSE = 8948;
var avgAPPROXATTACK = 7206;
var avgAPPROXDEFENSE = 4947;
...
function evoAffairs()  {
	var table = document.evaluate(".//table[@class='t_little'][preceding-sibling::div[@class='separator title']]", contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	if( table != null ) {
...
function evoAllianceMembers() {
	...
	for( var i = 0; memberTable = memberTables.snapshotItem(i); i++ ) {
		...
		memberTable.rows[0].cells[1].style.width = '210px';
		memberTable.rows[0].cells[1].textContent = 'Land (Att for max land)';
		...
		for(var j = 1; j < memberTable.rows.length; j++) {
			...
			//
			// start of add max att required
			//
			var totalland = evoString2Number(memberTable.rows[j].cells[1].textContent); // Get land data
			var afterTick1 = totalland * (1 - maxLandGrabPercent);
			var afterTick2 = afterTick1 * (1 - maxLandGrabPercent);
			var afterTick3 = afterTick2 * (1 - maxLandGrabPercent);
			var maxAttTick1 = (totalland - afterTick1) * attackPerLand;
			var maxAttTick2 = (afterTick1 - afterTick2) * attackPerLand;
			var maxAttTick3 = (afterTick2 - afterTick3) * attackPerLand;
			var avgAttTick1and3 = (maxAttTick1 + maxAttTick3) / 2;
			var reqForMaxLandGrab = (maxAttTick2 + avgAttTick1and3) / 2;
			memberTable.rows[j].cells[1].innerHTML = '&nbsp;' + totalland + '&nbsp;&nbsp;(<b>' + addCommas(Math.round(reqForMaxLandGrab)) + '</b>)';
			//
			// end of add max att required
			//
		}
	}
...
function evoUniverse() {
	...
	if( table != null ) {
		...
		table.rows[0].cells[5].textContent = 'Max att required'; // New heading
		...
		for(var i=2; i < table.rows.length; i++) {
			...
			var totalland = evoString2Number(cell2.textContent); // Get land data
			var afterTick1 = totalland * (1 - maxLandGrabPercent);
			var afterTick2 = afterTick1 * (1 - maxLandGrabPercent);
			var afterTick3 = afterTick2 * (1 - maxLandGrabPercent);
			var maxAttTick1 = (totalland - afterTick1) * attackPerLand;
			...
			var avgAttTick1and3 = (maxAttTick1 + maxAttTick3) / 2;
			var reqForMaxLandGrab = (maxAttTick2 + avgAttTick1and3) / 2;
			table.rows[i].cells[5].innerHTML = '&nbsp;' + addCommas(Math.round(reqForMaxLandGrab));
			//
			// end of add max att required
			//
...
// Planetary Status
//
regPageHandler(/^\/planet\/home$/i,  evoPlanetaryStatus);
function evoPlanetaryStatus() {
	var attackers = document.evaluate(".//table[tbody/tr/td[text()='Attackers']]", contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	var defenders = document.evaluate(".//table[tbody/tr/td[text()='Defenders']]", contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	var i = 0;
	var j = 0;
	var creatures = 0;
	var fleets = 0;
	...
	if (attackers != null) {
		for (i = 0; node = attackers.snapshotItem(i); i++) {
			creatures = 0;
			fleets = 0;
			
			while (tmp = /(\d+) creatures (outgoing|inbound|attacking) (to|from)/g.exec(node.rows[2].cells[0].textContent)) {
				creatures += Number(tmp[1]);
				fleets++;
			}
			
			if (creatures > 0) {
				node.rows[0].cells[0].innerHTML = "Attackers: " + creatures + " creature" + (creatures > 1 ? "s " : " ") + "<span title='Approx Att^2 in millions | Def^2 in millions' style='cursor:help;'><i>~(" + ((creatures * avgAPPROXATTACK) / 1000000).toFixed(2) +  "M to " + ((creatures * APPROXATTACK) / 1000000).toFixed(2) + "M att | " + ((creatures * avgAPPROXDEFENSE) / 1000000).toFixed(2) + "M to " + ((creatures * APPROXDEFENSE) / 1000000).toFixed(2) + "M def)</i></span> from " + fleets + " fleet" + (fleets > 1 ? "s" : "") + ".";
			}
		}
	} //avgAPPROXATTACK
	...
	if (defenders != null) {
		for (i = 0; node = defenders.snapshotItem(i); i++) {
			creatures = 0;
			fleets = 0;
			
			while (tmp = /(\d+) creatures (outgoing|inbound|defending) (to|from)/g.exec(node.rows[2].cells[0].textContent)) {
				creatures += Number(tmp[1]);
				fleets++;
			}
			
			if (creatures > 0) {
				node.rows[0].cells[0].innerHTML = "Defenders: " + creatures + " creature" + (creatures > 1 ? "s " : " ") + "<span title='Approx Att^2 in millions | Def^2 in millions' style='cursor:help;'><i>~(" + ((creatures * avgAPPROXATTACK) / 1000000).toFixed(2) +  "M to " + ((creatures * APPROXATTACK) / 1000000).toFixed(2) + "M att | " + ((creatures * avgAPPROXDEFENSE) / 1000000).toFixed(2) + "M to " + ((creatures * APPROXDEFENSE) / 1000000).toFixed(2) + "M def)</i></span> from " + fleets + " fleet" + (fleets > 1 ? "s" : "") + ".";
			}
		}
	}
}
...
(function () {
	...
	addItemToContent(node, 'Forum Links: ', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/_/416/view/unread\">Discussion</a>", true);
      addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/_/905-Random-Stuff/view/unread\">Random Stuff</a>", true);
      addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/_/109273-Alliances/view/unread\">Alliances</a>", true);
      addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/_/109280-Serpants-King-Alliance-Forum/view/unread\">S.K Alliance Forum</a>", true);
      addItemToContent(node, '', 't_normal b', "<br/>", false);
	addItemToContent(node, 'General Use: ', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/overview\">Overview Page</a>", true);
	addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/news\">News Page</a>", true);
      addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5-dev.neondragon.net/fleets\">Fleets Page</a>", true);
	addItemToContent(node, '', 't_normal b', "<a href=\"http://ev5.neondragon.net/overview\">Goto Regular Evo</a>", true);
...
//
// ***************************************************************************
// ** Format a number by adding commas (returns a string)
// ***************************************************************************
...
function evoUnitsInitialize() {
	...
	if(! dEvo ) {
		units['monkey']           = new evoUnit('Monkey'           ,UT_NATURAL ,3   ,450    ,225    ,4   ,4   ,3   ,0  ,40 ,true);
		...
		units['elephant']         = new evoUnit('Elephant'         ,UT_NATURAL ,19  ,36000  ,18000  ,72  ,75  ,22  ,4  ,40 ,true);

		units['horse']            = new evoUnit('Horse'            ,UT_NATURAL ,5   ,1800   ,900    ,13  ,13  ,4   ,1  ,40 ,true);
		units['fox']              = new evoUnit('Fox'              ,UT_NATURAL ,9   ,6480   ,2880   ,24  ,25  ,8   ,0  ,40 ,true);
		units['puma']             = new evoUnit('Puma'             ,UT_NATURAL ,13  ,9900   ,4500   ,32  ,32  ,5   ,1  ,40 ,true);
		...
		units['cheetah']          = new evoUnit('Cheetah'          ,UT_NATURAL ,16  ,12600  ,6300   ,41  ,29  ,5   ,1  ,40 ,true);
		units['panther']          = new evoUnit('Panther'          ,UT_NATURAL ,16  ,14000  ,8000   ,32  ,41  ,6   ,1  ,40 ,true);
		units['tiger']            = new evoUnit('Tiger'            ,UT_NATURAL ,20  ,16200  ,8100   ,44  ,43  ,11  ,2  ,40 ,true);
		units['rhino']            = new evoUnit('Rhino'            ,UT_NATURAL ,24  ,25200  ,15300  ,66  ,51  ,3   ,3  ,40 ,true);
		...
		units['wyvern']           = new evoUnit('Wyvern'           ,UT_ENG     ,15  ,25000  ,17500  ,64  ,43  ,7   ,2  ,40 ,true);
		units['demon']            = new evoUnit('Demon'            ,UT_ENG     ,20  ,36000  ,18000  ,93  ,58  ,5   ,3  ,40 ,true);
