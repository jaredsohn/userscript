// ==UserScript==
// @name           EitangoMemo
// @namespace      http://blog.nicetrip.org/
// @include        *
// ==/UserScript==
...
(function() {
  ...
  this.wsseHeader = function(Username, Password) {
      var w = wsse(Password);
...
//
  var config = {
    username: "",
    password: "",
    loadingElemID: "studyEnglishElem"
  };

  (function() {
    var elem = document.createElement("div");
    elem.id = config["loadingElemID"];
    elem.innerHTML = "Now Bookmarking";
    with(elem.style){
      position = "fixed";
      top = "4px";
      right = "4px";
  ...
  window.addEventListener("keypress", function(keypressEvent) {
    ...
    if (keypressEvent.charCode != 98) return;
    ...
    // alc
    var alcUrl = 'http://eow.alc.co.jp/'+selection+'/';
  
    // hatena bookmark
    var username = config["username"];
    var password = config["password"];
    var wsse = new Wsse;
    var endPoint = 'http://b.hatena.ne.jp/atom/post';
    var requestBody = '<entry xmlns="http://purl.org/atom/ns#"> <title>dummy</title> <link rel="related" type="text/html" href="'+alcUrl+'" /> <summary type="text/plain">'+tag+loc+'</summary> </entry>';
    var headersWithWsse = {
      'Accept': 'application/x.atom+xml, application/xml, text/xml, */*',
      'X-WSSE': wsse.wsseHeader(username, password)
    };
    var onload = function(){ 
      document.getElementById(config["loadingElemID"]).style.display = 'none';
      //console.log("OK!"); 
    }
    ...
    var hatenaOpt = {
      ...
      url: endPoint,
      data: requestBody,
      headers: headersWithWsse,
      onload: onload,
    };
  
    document.getElementById(config["loadingElemID"]).style.display = 'inline';
    GM_xmlhttpRequest(hatenaOpt);
  }, false);
