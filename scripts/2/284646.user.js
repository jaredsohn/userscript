...
// @name           QLRanks.com Display Colorized
// @version        1.78
        ...
        onSuccess: function() {
          ...
          QLRD.waitFor(names, gt, function(error, players, gt) {
            ...
            for (var i = 0, out = [], len = players.length; i < len; ++i) {
                ...
                var  name_color = 7;
                if (players[i].name== quakelive.username) name_color = 3;

                var elo_color = 7;
                if (players[i].elo>1000){ elo_color = 6;}
                if (players[i].elo>1200){ elo_color = 5;}
                if (players[i].elo>1400){ elo_color = 4;}
                if (players[i].elo>1600){ elo_color = 3;}
                if (players[i].elo>1800){ elo_color = 2;}
                if (players[i].elo>2000){ elo_color = 1;}

                out.push("^" + name_color + players[i].name + " ^7(^"+elo_color + players[i].elo + "^7)");
