// Bloglines Content Retrieve
// version 0.2
// 2007-07-11
//-----------
// Usage
// When you use bloglines and reading stories from hk yahoo! news or mysinablog,
...
//----------
// change log
// 0.2 - add hompy, wretch.cc support
// 0.1 - port from google reader scripts 
//---------------
// ==UserScript==
// @name          Bloglines Content Retrieve
// @namespace     http://www.bloglines.com/greasemonkey/content
// @description   Retrieve content in Bloglines.
	...
	function LinkKey(event) {
		...
		if (k == "g"){
			...
			for(var i=0; i<tds.length; i++){
				if(tds[i].className && tds[i].className == "selected"){
					articleTd = tds[i];
					break;
				}
			}
			var articleLink = articleTd.getElementsByTagName("h3")[0].getElementsByTagName("A")[0];
			var currentContent = articleTd.getElementsByTagName("td")[0];
				...
				if(key){
					...
					GM_xmlhttpRequest({
						...
						onload: function(response){
							...
							currentContent.innerHTML = "<div>"+resultHtml+"</div>";
