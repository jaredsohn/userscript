// ==UserScript==
// @name = "Mousehunt Edited for private use"
...
// only firefox browser require the code below	
if (browserDetection() == "firefox")
{
	// Code below are copy paste from Content Global Reader written by arantius
	// Source code of Content Global Reader are taken from http://userscripts.org/scripts/review/85398
	...
	( 
		function() 
		{
			...
			{
				var msg_data = {'type': 'read_content_global', 'callback_id': id, 'name': name, 'data': value, };
			...
			function receive_global(event) 
			{
				try 
				{
					...
					if ('read_content_global' != result.type) 
						return;
					if (!callbacks[result.callback_id]) 
						return;
			...
			read_content_global = function(name, callback) 
			{
				...
				location.href = 'javascript:dispatch_global(' + id + ', "' + name.replace(/\"/g, '\\"') + '", ' + name + ');void(0);';
			}
		}
	)();
	// End of Content Global Reader
}
...
var optionElement;
var travelElement;
var baitQuantityStartIndex;
var trinketQuantity = -1;
var trinketQuantityStartIndex;
...
function retrieveDataFirst()
{
	...
	if (scriptElementList)
	{
		...
		for (i = 0; i < scriptElementList.length; ++i)
		{
			...
			// get cheese quantity
			baitQuantityStartIndex = scriptString.indexOf("bait_quantity");
			...
			trinketQuantityStartIndex = scriptString.indexOf("trinket_quantity");
			if (trinketQuantityStartIndex >= 0)
			{	
				trinketQuantityStartIndex += 18;
				var trinketQuantityEndIndex = scriptString.indexOf(",", trinketQuantityStartIndex);
				var trinketQuantityString = scriptString.substring(trinketQuantityStartIndex, trinketQuantityEndIndex);
				trinketQuantity = parseInt(trinketQuantityString);
				
				trinketQuantityStartIndex = undefined;
				trinketQuantityEndIndex = undefined;
				trinketQuantityString = undefined;
			}
			...
			var nameStartIndex;
			var nameEndIndex;
			nameStartIndex = scriptString.indexOf("username");
			if (nameStartIndex >= 0)
			{
				nameStartIndex += 10;
				nameEndIndex = scriptString.indexOf(",", nameStartIndex);
				var nameString = scriptString.substring(nameStartIndex, nameEndIndex);
				username = nameString;
				
				nameStartIndex = undefined;
...
function retrieveData()
{
	...
	if (browser == "firefox")
	{
		read_content_global('user.next_activeturn_seconds', getPageVariable);
		read_content_global('user.has_puzzle', getPageVariable);
		read_content_global('user.user.bait_quantity', getPageVariable);
		read_content_global('user.user.trinket_quantity', getPageVariable);
		read_content_global('user.user.location', getPageVariable);
		read_content_global('user.user.username', getPageVariable);
	}
	else if (browser == "opera")
	{
		...
		trinketQuantity = user.trinket_quantity;
	...
	else if (browser == "chrome")
	{
		...
		trinketQuantity = parseInt(getPageVariableForChrome("user.trinket_quantity"));
		username = getPageVariableForChrome("user.username");
	}
	else
	{
...
function getPageVariable(name, value)
{
	...
	else if (name == "user.trinket_quantity")
	{
		trinketQuantity = parseInt(value);
	}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if (!aggressiveMode)
				{
					displayTimer("Username: "+ username + " | Location: "+ currentLocation + " | Charm: " + trinketQuantity + " | Cheese: "+baitQuantity + " | Horn: " + timeformat(hornTime), 
						timeformat(hornTime) + "  <i>(included extra " + timeformat(hornTimeDelay) + " delay & +/- 5 seconds different from MouseHunt timer)</i>", 
						"-");
					
					// check if user manaually sounded the horn
					var soundedHorn = getStorage("soundedHorn");
					if (soundedHorn != undefined && soundedHorn != null && soundedHorn == "true")
					{
						soundedHorn = undefined;
...
function embedTimer(targetPage)
{
	if (showTimerInPage)
	{
		...
		if (headerElement)
		{
			...
			if (showPerference == undefined || showPerference == null)
			{
			...
			var showPerferenceSpan = document.createElement('span');
			var showPreferenceLinkStr = '<a id="showPerferenceLink" name="showPerferenceLink" onclick="if (document.getElementById(\'showPerferenceLink\').innerHTML == \'<b>[Hide Perference]</b>\') { document.getElementById(\'preferenceDiv\').style.display=\'none\';  document.getElementById(\'showPerferenceLink\').innerHTML=\'<b>[Show Perference]</b>\'; } else { document.getElementById(\'preferenceDiv\').style.display=\'block\'; document.getElementById(\'showPerferenceLink\').innerHTML=\'<b>[Hide Perference]</b>\'; }">';
			if (showPerference == true)
				showPreferenceLinkStr += '<b>[Hide Perference]</b>';
			else
				showPreferenceLinkStr += '<b>[Show Perference]</b>';
			showPreferenceLinkStr += '</a>';
			showPreferenceLinkStr += '&nbsp;&nbsp;&nbsp;';
			showPerferenceSpan.innerHTML = showPreferenceLinkStr;
			showPerferenceLinkDiv.appendChild(showPerferenceSpan);
			showPreferenceLinkStr = undefined;
			showPerferenceSpan = undefined;
			showPerferenceLinkDiv = undefined;
			
			var hr2Element = document.createElement('hr');
			timerDivElement.appendChild(hr2Element);
			hr2Element = undefined;
			
			var perferenceHTMLStr = '<table border="0" width="100%">';
			if (aggressiveMode)
			{
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Bot aggressively by ignore all safety measure such as check horn image visible before sounding it">';
				perferenceHTMLStr += '<b>Aggressive Mode</b>';
				perferenceHTMLStr += '</a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="AggressiveModeInputTrue" name="AggressiveModeInput" value="true" onchange="if (document.getElementById(\'AggressiveModeInputTrue\').checked == true) { document.getElementById(\'HornTimeDelayMinInput\').disabled=\'disabled\'; document.getElementById(\'HornTimeDelayMaxInput\').disabled=\'disabled\';}" checked="checked"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="AggressiveModeInputFalse" name="AggressiveModeInput" value="false" onchange="if (document.getElementById(\'AggressiveModeInputFalse\').checked == true) { document.getElementById(\'HornTimeDelayMinInput\').disabled=\'\'; document.getElementById(\'HornTimeDelayMaxInput\').disabled=\'\';}"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Extra delay time before sounding the horn (in seconds)">';
				perferenceHTMLStr += '<b>Horn Time Delay</b>';
				perferenceHTMLStr += '</a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="HornTimeDelayMinInput" name="HornTimeDelayMinInput" disabled="disabled" value="' + hornTimeDelayMin.toString() + '"/> seconds';
				perferenceHTMLStr += ' ~ ';
				perferenceHTMLStr += '<input type="text" id="HornTimeDelayMaxInput" name="HornTimeDelayMaxInput" disabled="disabled" value="' + hornTimeDelayMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			else
			{
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="HornTimeDelayMinInput" name="HornTimeDelayMinInput" value="' + hornTimeDelayMin.toString() + '"/> seconds';
				perferenceHTMLStr += ' ~ ';
				perferenceHTMLStr += '<input type="text" id="HornTimeDelayMaxInput" name="HornTimeDelayMaxInput" value="' + hornTimeDelayMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			if (enableTrapCheck)
			{
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Enable trap check once an hour"><b>Trap Check</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="TrapCheckInputTrue" name="TrapCheckInput" value="true" onchange="if (document.getElementById(\'TrapCheckInputTrue\').checked == true) { document.getElementById(\'TrapCheckTimeOffsetInput\').disabled=\'\'; document.getElementById(\'TrapCheckTimeDelayMinInput\').disabled=\'\'; document.getElementById(\'TrapCheckTimeDelayMaxInput\').disabled=\'\';}" checked="checked"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="TrapCheckInputFalse" name="TrapCheckInput" value="false" onchange="if (document.getElementById(\'TrapCheckInputFalse\').checked == true) { document.getElementById(\'TrapCheckTimeOffsetInput\').disabled=\'disabled\'; document.getElementById(\'TrapCheckTimeDelayMinInput\').disabled=\'disabled\'; document.getElementById(\'TrapCheckTimeDelayMaxInput\').disabled=\'disabled\';}"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Trap check time different value (00 minutes - 45 minutes)"><b>Trap Check Time Offset</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeOffsetInput" name="TrapCheckTimeOffsetInput" value="' + trapCheckTimeDiff.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Extra delay time to trap check (in seconds)"><b>Trap Check Time Delay</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeDelayMinInput" name="TrapCheckTimeDelayMinInput" value="' + checkTimeDelayMin.toString() + '"/> seconds';
				perferenceHTMLStr += ' ~ ';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeDelayMaxInput" name="TrapCheckTimeDelayMaxInput" value="' + checkTimeDelayMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			else
			{
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeOffsetInput" name="TrapCheckTimeOffsetInput" disabled="disabled" value="' + trapCheckTimeDiff.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeDelayMinInput" name="TrapCheckTimeDelayMinInput" disabled="disabled" value="' + checkTimeDelayMin.toString() + '"/> seconds';
				perferenceHTMLStr += ' ~ ';
				perferenceHTMLStr += '<input type="text" id="TrapCheckTimeDelayMaxInput" name="TrapCheckTimeDelayMaxInput" disabled="disabled" value="' + checkTimeDelayMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			if (isKingWarningSound)
			{
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Play sound when encounter king\'s reward"><b>Play King Reward Sound</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="PlayKingRewardSoundInputTrue" name="PlayKingRewardSoundInput" value="true" checked="checked"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="PlayKingRewardSoundInputFalse" name="PlayKingRewardSoundInput" value="false" /> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			else
			{
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="PlayKingRewardSoundInputTrue" name="PlayKingRewardSoundInput" value="true" /> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="PlayKingRewardSoundInputFalse" name="PlayKingRewardSoundInput" value="false" checked="checked"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			if (reloadKingReward)
			{
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Reload the the page according to King Reward Resume Time when encount King Reward"><b>King Reward Resume</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="KingRewardResumeInputTrue" name="KingRewardResumeInput" value="true" onchange="if (document.getElementById(\'KingRewardResumeInputTrue\').checked == true) { document.getElementById(\'KingRewardResumeTimeInput\').disabled=\'\'; }" checked="checked"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="KingRewardResumeInputFalse" name="KingRewardResumeInput" value="false" onchange="if (document.getElementById(\'KingRewardResumeInputFalse\').checked == true) { document.getElementById(\'KingRewardResumeTimeInput\').disabled=\'disabled\'; }"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="Duration of pausing the script before reload the King\'s Reward page (in seconds)"><b>King Reward Resume Time</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="KingRewardResumeTimeInput" name="KingRewardResumeTimeInput" value="' + kingPauseTimeMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			else
			{
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="KingRewardResumeInputTrue" name="KingRewardResumeInput" value="true" onchange="if (document.getElementById(\'KingRewardResumeInputTrue\').checked == true) { document.getElementById(\'KingRewardResumeTimeInput\').disabled=\'\'; }"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="KingRewardResumeInputFalse" name="KingRewardResumeInput" value="false" onchange="if (document.getElementById(\'KingRewardResumeInputFalse\').checked == true) { document.getElementById(\'KingRewardResumeTimeInput\').disabled=\'disabled\'; }" checked="checked"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="text" id="KingRewardResumeTimeInput" name="KingRewardResumeTimeInput" disabled="disabled" value="' + kingPauseTimeMax.toString() + '"/> seconds';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			if (pauseAtInvalidLocation)
			{
				perferenceHTMLStr += '<tr>';
				perferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				perferenceHTMLStr += '<a title="The script will pause if player at different location that hunt location set before"><b>Remember Location</b></a>';
				perferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="PauseLocationInputTrue" name="PauseLocationInput" value="true" checked="checked"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="PauseLocationInputFalse" name="PauseLocationInput" value="false" /> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			else
			{
				...
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '<td style="height:24px">';
				perferenceHTMLStr += '<input type="radio" id="PauseLocationInputTrue" name="PauseLocationInput" value="true"/> True';
				perferenceHTMLStr += '   ';
				perferenceHTMLStr += '<input type="radio" id="PauseLocationInputFalse" name="PauseLocationInput" value="false" checked="checked"/> False';
				perferenceHTMLStr += '</td>';
				perferenceHTMLStr += '</tr>';
			}
			
			perferenceHTMLStr += '<tr>';
			perferenceHTMLStr += '<td style="height:24px; text-align:right;" colspan="2">';
			perferenceHTMLStr += '(Changes only take place after user save the preference) ';
			perferenceHTMLStr += '<input type="button" id="PerferenceSaveInput" value="Save" onclick="	\
			...
			{
				if (secureConnection)
					perferenceHTMLStr += 'window.location.href=\'https://www.mousehuntgame.com/canvas/\';"/>';
				else
					perferenceHTMLStr += 'window.location.href=\'http://www.mousehuntgame.com/canvas/\';"/>';
			}
			else if (hiFivePlatform)
			{
				if (secureConnection)
					perferenceHTMLStr += 'window.location.href=\'https://mousehunt.hi5.hitgrab.com/\';"/>';
				else
					perferenceHTMLStr += 'window.location.href=\'http://mousehunt.hi5.hitgrab.com/\';"/>';
			}
			else if (mhPlatform)
			{
				if (secureConnection)
					perferenceHTMLStr += 'window.location.href=\'https://mousehuntgame.com/\';"/>';
				else
					perferenceHTMLStr += 'window.location.href=\'http://mousehuntgame.com/\';"/>';
			}
			perferenceHTMLStr += '&nbsp;&nbsp;&nbsp;</td>';
			perferenceHTMLStr += '</tr>';
...
function embedScript()
{
    ...
    headNodes[0].appendChild(scriptNode);
