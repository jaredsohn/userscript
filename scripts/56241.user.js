...
if (document.location.href.match("user.php")) {
  function displayBuildQueue() {
    $(visibleTownDisplay).style.width="330px";
        ...
        for(var coord in villages) {
          ...
          if (villages[coord].childNodes[0].innerHTML.match(cordTxt)) {
            ...
            try {
              ...
              var defensetxt= "<span style='color:#4747FF;font-size:1.6em'>"+comma(t["Military"]["Defensive"])+"</span>"
...
//         // 1 offensive (girl)
//         if (txt.match(/Arquero|Poncho|Soldado/)) off=off+amt;
//         // 2 defensive
//         if (txt.match(/Fusilero|Pistolero|Granjero/)) def=def+amt;
//         // 3 offensive
//         if (txt.match(/Luchador tomahawk|El Mariachi|Francotirador/)) off=off+amt;
//         // 4 info don't add for this
//         // 5 defensive
//         if (txt.match(/Arquero montado|Caballería|Cowboy/)) def=def+amt;
//         // 6 offensive
//         if (txt.match(/Trabuco|Conquistador|Fanfarrón/)) off=off+amt;
//         // 7 offensive
//         if (txt.match(/Demonio de fuego|Bombolero|Blaster/)) off=off+amt;
//         // 8 offensive
//         if (txt.match(/Flecha ardiente|Cañón|Cañón/)) off=off+amt;
//         // 9 offensive
  ...
  for(var i=0;i<list.length;i++) {
    try {
      ...
      if (content.match(/[0-9]+\|[0-9]+/)) {
        ...
        newlink.innerHTML="abre el mapa en "+content+" (hay " + dis + " cuadros aproximadamente)"
...
if (document.location.href.match("action=map")) {
 ...
 b.innerHTML = "Fija estas coordenadas como tu casa"
 b.href="javascript:// Fija estas coordenadas como tu casa para cálcular la distancia"
