 ...
 * v1.02 - fixed a few bugs that caused the tasks not to display properly (eg next next action lists with no 
 * 		   items, and/or without the "@"
 *       - added new configuration settings for inbox, and tickler font size when they have items in them - 
 *         ie you can make it really obvious when you have items in these buckets that need attention before 
 *         anything else if you want - I've set them pretty large, but you can tweak to your taste.
 *
 * 
 ...
 *
 * This script is based on an implementation of David Allen's Getting Things Done (GTD).  If you are unfamiliar 
 * with its workings you should read up on it on the web or by the book.  What follows will only make sense to 
 * those with some familiarity with GTD concepts.
 * 
 * There are many ways to implement a GTD system within Remember The Milk.  I've implemented it in a way I believe 
 * works well and this script is based on that method.  If your own implementation works very differently this 
 * script probably won't be applicable, but if the only real differences for you are some list names, then the 
 * script is easy to modify and future versions may even let you set the names for the different "bucket" lists.
 *
 * Ok, so here's how to implement this GTD system:
 *
 * Lists:
 *
 * Hardscaped (non-smartlist) Lists:  Other than the already existant Inbox, the other hardscape list is "Lone Action" 
 * for non-project-related tasks.  There is also a variable number of hardcaped lists for different projects you might 
 * have.  For the list name, I prefix it with "p." for programmatic reasons.  So "p.Annual Report" etc.  This will be 
 * displayed without the prefix within the app.
 *
 * Smartlists: 
 * "!Misstagged" : "(not(list:inbox) and not(tag:project) and not(tag:tickler) and not(tag:calendar) and not(tag:wait) 
 * 					and not(tag:project) and not(tag:nextaction) and not(tag:someday)) or (not(list:inbox) and 
 * 					not(tag:project) and not(tag:someday) and not(tag:wait) and not(timeEstimate:"> 0")) or 
 * 					(not(list:inbox) and not(tag:project) and not(tag:someday) and not(tag:wait) and not(tagContains:"@@")) 
 * 					or ((tag:tickler or tag:calendar) and due:never) or (not(tag:tickler or tag:calendar) and not(due:never)) 
 * 					or (tag:nextaction and not(tag:@call or tag:@web or tag:@errand or tag:@home or tag:@work))"
 * 	Comment: the misstagged smartlist is a list that becomes visible in a noticeable way (red with white letters) when a task 
 * 			is not tagged and have other fields filled in appropriately.  Please see below for how these tags need to be filled in.
 * 
 ...
 *
 * Adding Tasks:
 * The workflow I've created is that if I don't have the time to enter all the info about a task, but just want to do a 
 * quick brain dump, I add the task into the Inbox list.  When doing this, the Inbox display becomes larger to call attention 
 * to itself, since generally you want to try to keep your inbox empty.
 *
 *  If you want to enter all the info about a task that's already in your Inbox (or you are entering it for the first time and 
 *  have the time to add all the info, you first need to add it to one of the hardscaped lists.  If it is part of a project, 
 *  add it to that project's list.  If it doesn't have to do with a project, add it to your Lone Actions list.  
 * 
 * Next, you need to specify a "bucket" to put the task into.  You do this by tagging it with one (and only one) of the 
 * following: nextaction, tickler, calendar, wait, or someday.  if you are putting it into the calendar or tickler bucket, you 
 * need to add a due date, however, that due date will mean different things for each.  For the Calendar, the due date is when 
 * the event, etc. happens, and the item will always show up in your calendar list.  For Tickler's, the due date really takes 
 * on the meaning of "show date" - ie the task will not even be visible in the list until the date that you specify in the due 
 * date field.  For the next action bucket, there are two additional pieces of information that needs to be filled in.  One is 
 * a time estimate, and the other is "energy level." Both of these are part of what David Allen specifies as ways that can help 
 * you determine what your next action next action will be.  Time estimate has a normal field in RTM but energy level doesn't so 
 * what I do is specify it with a tag.  The tags I use for energy level (that is the mental and/or physical energy level necessary 
 * for completing the task) are @@high, @@medium, and @@low.
 *
 * I think this basically lays out how the implementation works, but if there's something that's not clear, please post any 
 * questions you have at the script's page on userscripts.org and I'll try to answer them and add to this description.
 *
 * Here are some basic settings for fonts and background colors.  Eventually I will try to increase the number of these to add 
 * more flexibility:
 */
...
varEmptyInboxFontSize = "11px"; 					// font size of Inbox list item when it  doesn't contain any tasks
varEmptyInboxFontWeight = "normal"; 				// font "weight" (ie boldness) of Inbox list item when it  doesn't contain 
													//	any tasks - possible values are "normal", "bold", "bolder", "lighter", 
													//	"100", "200", "300", "400", "500", "600", "700", "800", "900"
varNonEmptyInboxFontSize = "11px"; 					// font size of Inbox list item when it has some tasks in it - by default 
													//	doubled in size to call attention to itself
varNonEmptyInboxFontWeight = "bold"; 				// font "weight" (ie boldness) of Inbox list item when it has some tasks 
													// 	in it - - possible values are "normal", "bold", "bolder", "lighter", 
													//	"100", "200", "300", "400", "500", "600", "700", "800", "900"
varMisstaggedFontSize = "12px"; 					// font size of Misstagged list
varMisstaggedFontWeight = "bold"; 					// font "weight" (ie boldness) of Misstagged list - possible values are 
													//	"normal", "bold", "bolder", "lighter", "100", "200", "300", "400", "500", 
													//	"600", "700", "800", "900"
varLoneActionsFontSize = "12px"; 					// font size of Lone Actions list - made small by default since this generally 
													//	won't be a list that you look at often or at all.
varLoneActionsFontWeight = "normal"; 				// font "weight" (ie boldness) of Lone Actions list - - possible values are 
													//	"normal", "bold", "bolder", "lighter", "100", "200", "300", "400", "500", 
													//	"600", "700", "800", "900"
varNonEmptySentFontSize = "12px"; 					// font size of Sent list that has some items in it (empty sent list doesn't 
													//	display at all) - by default this font is small because (at least for me) 
													//	this list is almost never used 
varNonEmptySentFontWeight = "normal"; 				// font "weight" (ie boldness) of Sent list that has some items in it (empty 
													//	sent list doesn't display at all) - - possible values are "normal", "bold", 
													//	"bolder", "lighter", "100", "200", "300", "400", "500", "600", "700", "800", "900"
varEmptyProjectListFontSize = "12px"; 				// font size for a project list item that has no tasks in it - this is a way to 
													//	call attention to project lists that have nothing in them.
varEmptyProjectListFontWeight = "bold"; 			// font "weight" (ie boldness) for a project list item that has no tasks in it - - 
													//	possible values are "normal", "bold", "bolder", "lighter", "100", "200", "300", 
													//	"400", "500", "600", "700", "800", "900"
varOtherBucketsFontSize = "12px"; 					// font size for list items within the "Other Buckets" section (aside from the 
													//	section header & tickler).
varOtherBucketsFontWeight = "normal"; 				// font "weight" (ie boldness) for list items within the "Other Buckets" section 
													//	(aside from the section header & tickler) - possible values are "normal", "bold", 
													//	"bolder", "lighter", "100", "200", "300", "400", "500", "600", "700", "800", "900"
varNextActionFontSize = "12px"; 					// font size for list items within the "Next Actions" section (aside from the 
													//	section header).
varNextActionFontWeight = "normal"; 				// font "weight" (ie boldness) for list items within the "Next Actions" section 
													//	(aside from the section header).- possible values are "normal", "bold", "bolder", 
													//	"lighter", "100", "200", "300", "400", "500", "600", "700", "800", "900"
varTicklerFontSize = "12px"; 						// font size for Tickler File link - make larger to make a more obvious "alert"
varTicklerFontWeight = "bold"; 						// font "weight" (ie boldness) for Tickler File link - possible values are "normal", 
													//	"bold", "bolder", "lighter", "100", "200", "300", "400", "500", "600", "700", 
													//	"800", "900"
// my variables
varLeftColumnWidth = "240px";
varListFontSize = "11px";

// one final note:  I've changed some of the attributes in the createleftcolumn function below in order to make this compatible with 
// a wider formatting of RTM.  If this makes your page too wide, you might need to play a little with these settings.
...
var createLeftColumn = function() {
	...
	leftColumn.style.width = varLeftColumnWidth;
...
var moveListTabs = function() {
	...
	var listTabsClone = listTabs.cloneNode(true);
	...
	if (window.wrappedJSObject.listTabsClone) {
		var listItems = window.wrappedJSObject.listTabsClone.div.getElementsByTagName("li");

		for ( var i = 0; window.wrappedJSObject.listTabsClone.data && window.wrappedJSObject.listTabsClone.data[i]; ++i) {
			if (listItems[i].firstChild.innerHTML.indexOf("ps-") == 0) {
				listItems[i].style.display = "none";
			} else if (listItems[i].firstChild.innerHTML.indexOf("wk-") == 0) {
				listItems[i].style.display = "none";
			} else if (listItems[i].firstChild.innerHTML.indexOf("ch-") == 0) {
				listItems[i].style.display = "none";
			} else if (listItems[i].firstChild.innerHTML.indexOf("All Tasks") == 0) {
				listItems[i].style.display = "none";
			} else if (listItems[i].firstChild.innerHTML.indexOf("Inbox") == 0) {
				listItems[i].style.display = "none";
			}
...
var moveTabsToTheLeft = function() {
	...
	if (content && window.wrappedJSObject.view) {
		if (window.wrappedJSObject.view.getSelected() == "Tasks") {
			// Using this width for tasks page
			content.style.width = "1240px";
			showLeftColumn();
		} else {
			// Overview page
	...
	if (window.wrappedJSObject.listTabs) {
		...
		for ( var i = 0; window.wrappedJSObject.listTabs.data && window.wrappedJSObject.listTabs.data[i]; ++i) {
			...
			if (listItems[i].firstChild.innerHTML == "Inbox") {
				...
				listItems[i].style.fontSize = varListFontSize;
				listItems[i].firstChild.innerHTML = listItems[i].firstChild.innerHTML + " (" + tasksCount + ")";
			} else {
				// lists without items
				listItems[i].style.fontSize = varListFontSize;
...
var subscribeToFilterEditFinished = function() {
	if (window.wrappedJSObject.messageBus)
		window.wrappedJSObject.messageBus.subscribe(
				window.wrappedJSObject.listTabs.blitDiv,
				window.wrappedJSObject.listList.mbn + "setFilterSuccess");
}
