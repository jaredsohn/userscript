...
// @version        001.10.02.27
// @author         werner.vinck@me.com
// @namespace      werner.vinck@me.com
	...
	var Berekenen = function(){
		...
		var Invullenscherm = function(titel, Ltijd, Atijd){
			...
			for (var i=1; i<trs.length ; i++){
				if (trs[i].getElementsByTagName('td')[0].textContent == titel){
					trs[i].getElementsByTagName('td')[1].replaceChild(document.createTextNode(Ltijd+ttxt+Atijd),trs[i].getElementsByTagName('td')[1].firstChild);
					OK=1;
				};
			};
			if (OK==null){
		...
		var testvb = -1;
		//Nakijken of de Alias beschikbaar is
		try {
			var vb = document.getElementsByTagName('th')[0].getElementsByClassName('lkn_aliasbtn')[0].textContent;
			//De alias is beschikbaar, nakijken of het een tijdsaanduiding is
			var re=/:/g;
			var pos = vb.search(re);
			var punt = 0;
			if (pos>0){
				//Da Alias is een tijdsaanduiding, nakijken hoeveel millisec er opgegeven zijn
		...
		var Markt = OmzettenTijd(tMarkt, 0, punt);
		...
		if (testvb == 0 || pos<1){
			...
			Invullenscherm('Eenheid', 'Vertrektijd (nl'+wereld+')', ' ');
			for (var i=12; i>0 ; i--){
