...
// @description      Traduction de KOC Attack Extra en FR par BeWorld avec ameliorations diverses
...
"Priorit&eacute; des attaques : "+attackfirst+
			"<br><br />"+			
		"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> secondes entre deux attaques<br />"+
			...
			"</div>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% d\'ajustement pour tout les delais (pour avoir l\'air plus naturel).<br />"+
		        "<input id='KOCAttackRally' value='"+this.options.rallyKeep+"' size='3' /> du point de ralliement a garder<br />"+
			"<br />"+
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Supprimer automatique des rapports barbares/TS.<br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Supprimer automatique des rapports farm.<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Nombre maximun de rapports a conserver dans la messagerie.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> D&eacute;activer l\'affichage des rapports, la suppression auto ci dessus sera annul&eacute;e &eacutegalement.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de la population inactive disponible avant d'initier la formation automatique et utiliser <input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' />% de disponible.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Pour activer la formation automatique, activer le bouton de formation automatique d\'une unit&eacute; dans une caserne.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gold_30.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de bonheur avant d\'initialiser la collecte automatique de l\'or.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Pour activer la collecte auto de l\'or, activer le bouton AUTO GOLD sur chaque chateau.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableMapDraw' type='checkbox' "+(this.options.disableDrawMapIcons?'checked':'')+" /> Deactive l\'amelioration de la carte.<br />"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> D&eacuteactiver les message \"Invite Friends\" dans le jeu. (Attention, cela peut supprimer les tr&eacute;sors)<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Aider automatiquement les membres de l\'alliance sur les constructions/recherches.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Masquer les demandes d\'aides sur le tchat.<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Publier automatiquement sur votre mur Facebook.<br />"+
			"Si coch&eacute; ci dessus, quelle niveau de confidentialit&eacute voulez-vous ? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tout le monde</option><option value='50'>Amis et leurs Amis</option><option value='40'>Amis seulement</option><option value='10'>Moi uniquement</option></select><br />"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Reconnecter automatique si le serveur est en maintenance.<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Activer les erreurs (log) dans la console Firefox.<br />"+
			"</td></tr>"+
			...
			"Import/Export: Coller ou copier les parametres ici...<br /><textarea id='KOCAttackImport'></textarea>"+
	...
	*/	
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Type d'attaque (Les coordonnees doivent correspondres a ce type) :<br />");
		var arrData = [["Camp", "Camps"],["City", "Ville"],["Transport", "Transport"],["Wilderness", "Wildernesses"]];
		for (var i=0; i < arrData.length; i++){
		...
		AddHtml(bulkAddSuicideSpan,'<label for="KOCAttackBulkAddSuicideWave">Ceci est la vague de suicide pour &eacute;liminer les pieges des TS.</label>');
	...
	HandleDomainLogin:function() {
		...
		if (t.currentPage == "domain_selection" && t.options.autoLogBackIn && !t.domainLoginActionTaken) {
			...
			if(playButtons && playButtons.snapshotLength>0){
				...
				statusDiv.innerHTML='<center>KoCAttack Extra: Retour sur KOC dans '+timeDifference+' secondes...</center>';
				if(timeDifference==0){
					...
					statusDiv.innerHTML='<center>KoCAttack Extra: Retour sur KOC immediatement...</center>';
					var functionCall = {
						'action':'load_url',
				...
				statusDiv.innerHTML='<center>KoCAttack Extra: rechargement de la page dans '+timeDifference+' secondes...</center>';
				if(timeDifference==0){
					statusDiv.innerHTML='<center>KoCAttack Extra: Rechargement automatique de la page immediatement...</center>';
					t.ReloadWindow();
					t.domainLoginActionTaken=true;
				}
			}
	...
	OnAttackBoxAppear:function(box) {
		...
		AddHtml(suicideSpan,'<label for="KOCAttackSuicideWaveCheckbox">C\'est la vague suicide</label>');
	...
	*/
	...
	DeleteRenfortIn:function() {
		...
		this.IterateMsgReports(function(a,tr,desc,x,y) {
			...
			var m=this.onclickReportRe.exec(onclick);
			//GM_log('Recherche : ' + desc + ' Résultat : ' + desc.indexOf('Renforc'));
			if(m && desc.indexOf('Renforc')==-1) {
				return true; //Don't select 
			}
	 ...
	 return deletes;
        ...
        DeleteAttacksIn:function(param) {
			var deletes=0;
			this.IterateMsgReports(function(a,tr,desc,x,y) {
				var onclick=a.getAttribute('onclick');
				if(!onclick) return false;
			...
			//	return true; //Don't select if attacker is not yourself
			    ...
			    return true; // dont select
			} else if(m && desc.indexOf('Eclaireur')!=-1) {
			   return true; // dont select
			} else if(m && desc.indexOf('Transport')!=-1) {
			   return true; // dont select
			}
	...
	AddCheckBarbarians:function() {
		...
		a.innerHTML='<span>TS/Barb/Transp</span>';
		...
		// Suppression rapports de renforts
				var a=document.createElement('a');
						var t=this;
						a.addEventListener('click',function() {
							t.DeleteRenfortIn();
						...
						a.className='inlineButton20Red';
						//a.style.paddingLeft='30px';
						a.innerHTML='<span title="Supprime les rapports de renforts recu">Renf.</span>';
				  	    if(msgBody.nextSibling) {
							msgBody.nextSibling.insertBefore(a,msgBody.nextSibling.childNodes[0]);
							//msgBody.nextSibling.appendChild(a);
						} else {
		...
		// Suppression rapports d'attaques entrantes
		...
		var a=document.createElement('a');
				...
				a.className='inlineButton20Red';
				//a.style.paddingLeft='30px';
				a.innerHTML='<span title="Supprime les rapports d attaques entrantes seulement">Att. Ent.</span>';
		  	    if(msgBody.nextSibling) {
					msgBody.nextSibling.insertBefore(a,msgBody.nextSibling.childNodes[0]);
					//msgBody.nextSibling.appendChild(a);
				} else {
		...
		// Suppression rapports d'attaques sortantes
		var a=document.createElement('a');
						var t=this;
						a.addEventListener('click',function() {
							t.DeleteAttacksIn(0);
						...
						a.className='inlineButton20Red';
						//a.style.paddingLeft='30px';
						a.innerHTML='<span title="Supprime les rapports d attaques sortantes seulement">Att. Sor.</span>';
				  	    if(msgBody.nextSibling) {
							msgBody.nextSibling.insertBefore(a,msgBody.nextSibling.childNodes[0]);
							//msgBody.nextSibling.appendChild(a);
						} else {
	...
	AddTrainTroopsLink:function() {
		...
		var setTrainTroopsA=function() {
			...
			a.innerHTML='<span>'+(trainTroops[t.GetCurrentCityId()]==type?'Former Auto - On':'Former Auto - Off')+'</span>';
		}
		a.addEventListener('click',function() {
			var trainTroops=JSON2.parse(t.GetValue('TrainTroops','{}'));
