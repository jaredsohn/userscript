	...
	init: function(){
		...
		$('#list li img').one('load', function() {
			...
			setTimeout(function(){
				...
				$(that).parent().find('.process').fadeOut('fast');
			}, 100);		
		})
			.each(function() {
...
if (document.title.indexOf('制服诱惑区') != -1 || $('#main div')[0].innerHTML.indexOf('達蓋爾的旗幟') != -1 ){
	UI.init();
}

$('img[src^="images/wind_green/file/headtopic"]').parent().parent().hide();