// ==UserScript==
// @name			Facebook Auto Like  V 0.0.1 By_Delyo
// @namespace			FALV5
// @version			0.5
// @copyright			Delyo
// @description			Auto Like And Confirm (mass) | You can mass like and confirm
// @author			Ardik Didianto (http://userscripts.org/users/delyo)
...
if(body != null) {
	...
	div.innerHTML = "<div style='background-color: #2E5392; color: #FFFFFF; border: 1px solid #333333;'><center><a style='color: #FFFFFF;' <a href='https://www.facebook.com/fwcombiedotus' title='Click to See Delyo Profile on Facebook'> Delyo Profile </a></div>"
	...
	div2.innerHTML = "<div style='background-color: #2E5392; color: #FFFFFF; border: 1px solid #333333;'><a style='color: #FFFFFF;' onclick='spoiler()' title='Click to Hidden Autolike'>&laquo;</a>&nbsp;<a href='https://facebook.com/fwcombiedotus/' target='_blank' title='Autolike Version 0.0.1 by Delyo | Copyright 2012 | All Right Reserved' style='color: #FFFFFF;'>Autolike Version 0.0.1</a> | <a style='color: #FFFFFF;' onclick='thanks()'>about</a></div>"
	
	body.appendChild(div);
	body.appendChild(div2);
	
	unsafeWindow.thanks = function() {
	alert("Thanks for installing this autolike By_Delyo :)\n\nDelyo\nCopyright (c) 2012");
	}
	...
	for(i=1;i<=20;i++) {
		...
		div2.style.width = "125px";
		div2.innerHTML ="<div style='background-color: #2E5392; color: #FFFFFF; border: 1px solid #333333;'><a style='color: #FFFFFF;' onclick='spoiler()' title='Click to Hidden Autolike'>&laquo;</a>&nbsp;<a style='color: #FFFFFF;' href='https://facebook.com/fwcombiedotus/' target='_blank' title='Autolike Version 0.0.1 by Delyo| Copyright 2012 | All Right Reserved'>Autolike Version 0.0.1</a> | <a style='color: #FFFFFF;' onclick='thanks()'>about</a></div>"
		}
		else {
...
body.appendChild(div);
	unsafeWindow.LikeComments = function() {
		...
		for (var i = 0; i < prepare.length; i++)
			if(prepare[i].getAttribute("data-ft")!=null&&(prepare[i].getAttribute("title")=="Me gusta este comentario"||prepare[i].getAttribute("title")=="Like this comment"||prepare[i].getAttribute("title")=="Suka komentar ini"||prepare[i].getAttribute("title")=="Dolanan Auto Like Seger Yaa..."||prepare[i].getAttribute("title")=="?????????????? ????????????????"||prepare[i].getAttribute("title")=="J???aime ce commentaire"||prepare[i].getAttribute("title")=="Bu delyo be??en")) {
				buttons[Counter] = prepare[i];
				Counter++;
			}
		function check_link(linknumber) {
