// ==UserScript==
// @name        Ultoo Script for sms.ultoo.com ServerBy Vikram Singh Bais
...
if(url.search(pattern)==0)
{
	...
	{
		document.getElementsByName('PollUserName')[0].value="Vikram Singh Bais";
		document.getElementsByName('PollUserQuestion')[0].value="How many Time U Watching TV in a Year..? & Me only "+Math.floor((Math.random() * 100) + 1)+".";
...
if(url.search(pattern)==0)
{
	window.location.href ="http:\/\/sms.ultoo.com\/home.php/g";
}
...
if(url.search(pattern)==0)
{
	...
	{	
		...
		document.getElementById('Message_').value="Hi Frnds , Jai Shri Krishna Gud Mrng Friends...! '"+Math.floor((Math.random() * 1000000000) + 1)+"'";
