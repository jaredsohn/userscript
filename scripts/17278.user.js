...
//
// ==UserScript==
// @name          Format Amazon Wishlist
// @namespace     AmazonWishlist
// @description   Output 
// @include       http://www.amazon.com/*wishlist*
// @include       http://www.amazon.co.uk/*wishlist*
// @include       https://www.amazon.com/*wishlist*
// @include       https://www.amazon.co.uk/*wishlist*
// ==/UserScript==
...
if (location.hostname.indexOf('uk') > 0 ||location.hostname.indexOf('ca') > 0) {
	...
	var buttonLocationString="//span[@id='listTitleTxt']/..";
	var wishlistLocationString="//span[@id='listTitleTxt']";
}
	...
	function processObject(dObj) {
		...
		if (typeof dObj == 'object') {
			allTitles = document.evaluate(
				"//span[contains(@class,'small productTitle')][strong]",
				dObj,
				null,
				XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
				null);
			...
			// use string handling
			var pos=0;
			var titleStart='<span class="small productTitle"><strong>';
			var titleEnd='</strong></span>';
			var authorStart='<span class="tiny">';
			var authorEnd='</td>';
			...
			while ( (titleStartPos=dObj.indexOf(titleStart,pos)) && titleStartPos >= 0 ) {
				...
				authorStartPos=dObj.indexOf(authorStart,titleEndPos);
				if (authorStartPos) {
