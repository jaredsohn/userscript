/*
 *  This file is part of OGame Plus, based on OGame# by Spitfire
 *  Copyright (C) 2007 Trucas
...
// @name			OGame Plus
// @author			Trucas
// @description			Ogame melhorado
// @include			http://uni*.ogame.com.pt/game/*
...
const scriptAuthor = 'Mordillo';
const scriptName = 'OGame Plus';
const scriptVersion = '0.1.95';
...
const c_emptyingStore = '¡O almacén se está vaciando!';
...
if (leftmenu) {
	...
	var nodo = locateFirst("//font[a='Alianças']/../../..");
	if (nodo) {
			...
			unsafeWindow.actualizaMesg = function () {
				...
				GM_xmlhttpRequest({
					...
					onload:function(details) {
						...
						try {
							var numMens = parseInt(texto.match(/Tens (\d+)/)[1], 10);
...
//						if (ataque!='') alert('Nos atacan!')
		...
		select.lastChild.innerHTML="<option value='1' title='O destino é um planeta'>Planeta</option><option value='3' "+(data[3]=='1'?'selected':'')+" title='O destino é uma lua'>Lua</option>";
	...
	if (getScriptConf('LMSearch')) {
		var nodo = locateFirst("//table/tbody/tr/td/div/font[a='Procurar']/..");
		if (nodo) {
			nodo.innerHTML='';
...
//		cad += newTableRowCk('conf', 'transportModeEnabled', c_transportModeEnabled);
	...
	config.parentNode.insertBefore(initTab(['Configuração OGame', 'Configuração '+scriptName],['ogame', 'script']), config);
...
//				if (blockedTech.lastChild.innerHTML != ' - ')
				blockedTech.lastChild.innerHTML = "<font color='red'>Máximo nivel útil alcançado</font>";
			}
		...
		for (var i in techsSaved) {
			try {
				tech = response.match(new RegExp("gid="+techsSavedIds[i]+".{0,50}Nível (\\d+ )\\)"))[1];
			} catch(e) {}
			if (tech!=0) {
		...
		if (coordsTable) {
			...
			title.innerHTML = "<th>Naves</th><th>Disponiveis</th><th>-</th><th>-</th>";
			...
			nodo.innerHTML = "<th colspan='2'><a href=\"javascript:void();\" onclick=\"" + ninguna + "\">Nenhuma nave</a></th><th colspan='2'><a href=\"javascript:void();\" onclick=\"" + todas + "\">Todas as naves</a></th>";
		}
		else if (timeLeft) {
...
//					document.getElementById('max').id='max_'+inputs.snapshotItem(i).name.match(/\[(.*)\]/)[1];
		...
		if (buildingsSection && building && getScriptConf('buildingList')) {
			...
			for (var i=0; i<gidNames.snapshotLength; i++) {
				setGidTag(gidNames.snapshotItem(i).href.match(/gid=(\d+)/)[1], gidNames.snapshotItem(i).innerHTML);
			}
			...
			}catch(e){GM_log('Error in building stack: '+e)}
			loading = false;
		}
...
// 		var string='document.getElementById';
...
else if (contentSection=='resources' && getScriptConf("improvedResources")) {
	var T_Recursos = locateSnapshot("//font[@color]");
	...
	try {
		...
		individualEnergy.title = 'Energia produzida por cada satélite';
	...
	if (ResForm) {
		...
		CuentaRec.innerHTML = '<br /><table width="550">'+
			...
			'<td width=26% class="c" align=center>Deutério</td>'+
			'</tr><tr>' +

				
			'<td class="c" colspan="4">Produção</td>'+
			'</tr><tr>'+
			...
			'</tr><tr>'+
				'<td class="c" title="Dias e horas de produção" align=center onkeyup="' +
				'var dias = 0;' +
			...
			'<td class="c" colspan="4">Armazenamento</td>'+
			'</tr><tr>' +
			'<td class="c" align=center title="Capacidade dos Armazéns">Estado</td>'+
			...
			'</tr><tr>' +
			'<td class="c" align=center title="Tempo restante para esgotar cada armazém">Esgotado em:</td>'+
...
//		document.getElementsByTagName('tbody')[4].setAttribute('onchange', 'alert(1)');
	...
	if (getScriptConf('proInputs')) {
		var cells = locateSnapshot("/html/body/center[2]/form/table/tbody/tr/th[a='máx']");
	...
	unsafeWindow.whenResponse = function whenResponse(){
		...
		switch(retVals[0]) {
			...
			case "602":
				...
				break;
			case "603":
				unsafeWindow.addToTable("O jogador tem protecção de jogador fraco", "error");
				break;
			case "604":
			...
			case "605":
				...
				break;
			case "610":
				unsafeWindow.addToTable("Só há "+retVals[1]+" sondas disponiveis, enviando", "notice");
				break;
			case "611":
				unsafeWindow.addToTable("Não há sondas de espionagem disponiveis", "error");
				break;
			case "612":
				unsafeWindow.addToTable("Não podes controlar mais frotas", "error");
				break;
			case "613":
				unsafeWindow.addToTable("Não tens deutério suficiente", "error");
				break;
			case "614":
				unsafeWindow.addToTable("Não existe planeta", "error");
				break;
			case "615":
				unsafeWindow.addToTable("Não há combustivel suficiente", "error");
				break;
			case "616":
...
// 			options+="<option style='color:"+subTags[1]+"'>"+subTags[0]+"</option>";
		...
		for (var i=0; i<allys.snapshotLength; i++) {
			...
			if (ally) { // coloreamos
				...
				//allys.snapshotItem(i).innerHTML=allys.snapshotItem(i).innerHTML.replace(new RegExp('(Alianza |   )('+ally+')( en la| $)', 'i'), '$1<font color='+(color2!=''?color2:color)+'>$2</font> '+etiquetas+'$3');
...
//		var content=document.getElementsByTagName("table")[5].innerHTML;
	...
	if ((document.URL.search('a=4')>-1) && getScriptConf('galaxyTool')) {
		...
		var nodo = locateFirst('/html/body/center/table/tbody/tr[th="Página principal"]/th[2]/a');
		if (nodo) {
			setScriptConf('foroAlly', nodo.innerHTML.replace(/:/g, ';'));
			document.getElementById('foroAlly').href=nodo.innerHTML;
		}
		nodo = locateFirst('/html/body/center/table/tbody/tr[th="TAG"]/th[2]');
		if (nodo) {
		...
		nodo = locateFirst('/html/body/center/table/tbody/tr[th="Email colectivo"]');
		if (nodo) {
	...
	// defensas
	parser['Lanzamisiles'] = new Array("&numunits[1][0][ra]=", "&d14=");
	parser['Láser pequeño'] = new Array("&numunits[1][0][l_l]=", "&d15=");
	parser['Láser grande'] = new Array("&numunits[1][0][s_l]=", "&d16=");
	parser['Cañón Gauss'] = new Array("&numunits[1][0][g]=", "&d17=");
	parser['Cañón iónico'] = new Array("&numunits[1][0][i]=", "&d18=");
	parser['Cañón de plasma'] = new Array("&numunits[1][0][p]=", "&d19=");
	parser['Cúpula pequeña de protección'] = new Array("&numunits[1][0][k_s]=", "&d20=");
	parser['Cúpula grande de protección'] = new Array("&numunits[1][0][g_s]=", "&d21=");
	// flota
	parser['Nave pequeña de carga'] = new Array("&numunits[1][0][k_t]=", "&d0=");
	parser['Nave grande de carga'] = new Array("&numunits[1][0][g_t]=", "&d1=");
	parser['Cazador ligero'] = new Array("&numunits[1][0][l_j]=", "&d2=");
	parser['Cazador pesado'] = new Array("&numunits[1][0][s_j]=", "&d3=");
	parser['Crucero'] = new Array("&numunits[1][0][kr]=", "&d4=");
	parser['Nave de batalla'] = new Array("&numunits[1][0][sc]=", "&d5=");
	parser['Colonizador'] = new Array("&numunits[1][0][ko]=", "&d6=");
	parser['Reciclador'] = new Array("&numunits[1][0][re]=", "&d7=");
	parser['Sonda de espionaje'] = new Array("&numunits[1][0][sp]=", "&d8=");
	parser['Bombardero'] = new Array("&numunits[1][0][bo]=", "&d9=");
	parser['Satélite solar'] = new Array("&numunits[1][0][so]=", "&d10=");
	parser['Destructor'] = new Array("&numunits[1][0][z]=", "&d11=");
	parser['Estrella de la muerte'] = new Array("&numunits[1][0][t]=", "&d12=");
	parser['Acorazado'] = new Array("&numunits[1][0][sk]=", "&d13=");
	// tecnologias
	parser['Tecnología militar'] = new Array("&techs[1][0][w_t]=", "&dattack=");
	parser['Tecnología de defensa'] = new Array("&techs[1][0][s_t]=", "&dshield=");
	parser['Tecnología de blindaje'] = new Array("&techs[1][0][r_p]=", "&darmory=");
	...
	if (coordLinker || colorMens) {
		...
		for (var i=0; i<texto.snapshotLength; i++) {
			...
			if (colorMens) {
				...
				texto2=texto2.replace(/jogador ([\s\S]*) envía/, "jugador <font color="+getScriptConf('colorPriv')+">$1</font> envía"); // nombre de jugador en entregas
...
//const strings
	...
	const c_ogameSkin		= ' Fondo de skin: ';
	const c_forum_type		= 'Compactar para foro de tipo: ';
	const c_forum_color		= ' Fondo del foro: ';
	const c_light		= 'Claro';
	const c_dark		= 'Oscuro';
	const c_old_favour		= ' Ver el reporte "clásico"';
	const c_old_slooses		= 'Perdidas del ';
	const c_oll_plooses		= 'Perdidas de los ';
	...
	const c_link1 = 'Compactado con el ';
	const c_link2 = 'Compactador automático de batallas';
