// ==UserScript==
// @name           favela - money_sort_attack_list
// @version	   0.3
// @author	   abwasch - pennerhack.foren-city.de
// @description	   sort victim by money, points or registration date
// @include        http://sampa.faveladogame.com.br/highscore/user/*
// ==/UserScript==

var link = 'http://sampa.faveladogame.com.br';
...
var points = document.getElementsByClassName('ttip')[6].firstChild.data;
...
document.getElementById('auswahl').innerHTML = '\nscore range:&emsp;&emsp;min <input style="text-align:left\" type=\"text\" size=\"12\" maxlength=\"12\" id=\"min_punkte\" value=\"'+min+'\">\n'
					+'&emsp;&emsp;max <input style="text-align:left\" type=\"text\" size=\"12\" maxlength=\"12\" id=\"max_punkte\" value=\"'+max+'\">\n'
					+'&emsp;&emsp;search victim: <input style="text-align:right\" type=\"text\" size=\"2\" maxlength=\"5\" id=\"anzahl_gegner\" value=\"'+anzahl_gegner+'\">\n'
					+'&emsp;<input style=\"cursor:pointer; font-weight:bold; color:#e4e4e4; background-color:#901010\" type=\"submit\" id=\"start\" value=\"GO!\">\n'
...
document.getElementById('start').addEventListener('click', function(){
	...
	document.getElementById('anzeige').innerHTML = '<span id=\"anzahl\">0<\/span> pages found! read';
	min = document.getElementById('min_punkte').value;
	max = document.getElementById('max_punkte').value;
...
function userabfrage(){
	document.getElementById('anzeige').innerHTML = '<span id=\"anzahl\">0<\/span>/'+gegner_id.length+' user found! read API';
	zaehler = 0;
...
function anzeige(){
	...
	var table = '<style type=\"text/css\">.a-liste:hover {background-color:#505050}\n</style>\n'
		+'<span style=\"font-size:12px; font-weight:bold\">attack list <\/span><i>(can sort by money, points and reg-date)</i><br><br>'
		+'<table style=\"position:absolute; left:20px\"><tr height=\"28\">\n'
		+'<th width=\"220px\">Bum<\/th>'
		+'<th width=\"100px\" align=\"center\"><span id=\"points\" style=\"cursor:pointer\">Points<\/span><\/th>'
		+'<th width=\"220px\">Gang<\/th>'
		+'<th width=\"100px\" align=\"center\"><span id=\"reg\" style=\"cursor:pointer\">Reg-Date<\/span><\/th>'
		+'<th width=\"80px\" align=\"right\"><span id=\"money\" style=\"cursor:pointer\">Money<\/span><\/th>'
