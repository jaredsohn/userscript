...
*
*	YouScrobbler is build on the code of youscrobble (http://userscripts.org/scripts/show/71606) but goes its own way
*	Authentication-Function is taken and edited from ScrobbleSmurf (http://daan.hostei.com/lastfm/)
*
*/
...
function initPreferences () {
	...
	if (!us_getValue('us_compatibility-mode')) {
		us_saveValue('us_compatibility-mode',false);
	}
...
function us_addButton() {
    ...
    el.innerHTML =	'.us_box { border-radius: 5px; border: 5px solid #333; background: #fff;'+
					...
					'.us_settings_grp_left { width:145px}'+
					'.us_settings_grp_right { width:115px}'+
					...
					'.us_box input[type=submit]:hover {padding: 1px 3px 2px 5px; background-image: url(data:image/gif;base64,);}'+
	...
	button.innerHTML = '<img id="us_icon_small" style="margin: 0px -8px -3px 0px;" src="'+us_icon()+'" alt="icon" /><input id="us_temp_info" type="hidden" us_secs="'+secs+'" us_playstart_s="'+t+'" us_playstart="'+t2+'"/><a class="start" id="us_start_scrobblebutton">Scrobble</a><span class="masthead-link-separator">|</span>';//postxanadus
...
function us_showBox(loggedIn, message, auto) {
	...
	if (!isLoggedIn()) {
		...
		'<br /></div><div class="us_submitbuttons"><input id="us_submit" value="Login" type="submit" /></div>';
		us_boxcontent('Login to last.fm',cont);
		
		document.getElementById('us_submit').addEventListener('click', us_authenticate, false);
	} else {
		us_scrobbleform(loggedIn, message, auto);
	}
}
...
*/
function us_scrobbleform(loggedIn, message, auto) {
	...
	if (TO1Helper) {
		...
		scrobbleStatus = '<div id="scrobbleStatus_parent"> scrobble in <span id="scrobbleStatus" style="font-weight:bold">'+restTime+'</span> sec</div>';
	} 
	...
	if (auto==1) {
		if (message == "autoScrobbleFailed") {
			messageText += '<div class="us_error">AutoScrobble failed. Please scrobble manually.</div>';
		}
		if (!isMusicCategory()) {
			messageText += '<div class="us_error">Video is not in the Category <span style="font-style:italic">Music</span></div>';
		}
		if (feedback == "bad") {
    ...
    var cont = '<div id="us_loginbox_form">'+messageText+'<form name="us_scrobbleform" onSubmit="return'+
                         ...
                         '</form></div><div class="us_submitbuttons"><div class="us_submitbuttons_box_left" title="Activate automatic scrobbleing?"><input id="us_autoscrobble" name="us_autoscrobble" type="checkbox"'+checkedText+'><label for="us_autoscrobble" style="vertical-align:middle;">Auto</label></div>'+scrobbleStatus+'<input id="us_submit" value="Scrobble" type="submit" />'+
                         '</div>';
...
function us_settings(updated) {
		...
		var cmodeStatus = "";
		if (us_getValue("us_compatibility-mode", false) == true) {
			cmodeStatus = " checked";
		} 
		var cont =  '<div id="us_loginbox_form" style="text-align:left"><form name="us_settings_form" onSubmit="return false"><table style="table-layout:fixed"><tr><td class="us_settings_grp us_settings_grp_left">'+
					...
					'<div style="border-top:1px dotted #CCCCCC;margin-top:4px;padding-top:2px"><input type="checkbox" id="us_settings_cmode"'+cmodeStatus+'/><label for="us_settings_cmode">Compatibility-Mode</label> <a href="#" id="us_cmode_info-toggle">?</a><br/><span id="us_cmode_info" style="font-size:60%;color:gray;display:none">Deactivates visual effects and is useful, if having problems with other scripts.</div></td>'+
					'<td class="us_settings_grp us_settings_grp_right"><div class="us_settings_grp_heading">Database</div><span>Entries: '+us_getValue("database.id").split(" ").length+'</span>'+
		...
		document.getElementById('us_cmode_info-toggle').addEventListener('click', function(){document.getElementById("us_cmode_info").style.display="block";document.getElementById("us_cmode_info-toggle").style.display="none"}, false);
		...
		function us_savesettings() {
			...
			if (document.getElementById("us_settings_cmode").checked)  {
				us_saveValue("us_compatibility-mode", true);
			} else {
				us_saveValue("us_compatibility-mode", false);
			}
...
function tryAutoScrobble () {
	if (us_getValue("us_autoscrobble_active", 0) == 1) {
		...
		var musicVideo = isMusicCategory();
		if ((isLoggedIn())&&(response=="found")&&(musicVideo)) {
			us_scrobble(decodeURIComponent(us_getTempData("artist")),decodeURIComponent(us_getTempData("track")),"","",0,0,1);
		} else if ((response=="bad")||(!musicVideo)) {
			us_showBox(false, "", 1);
		} else {
			us_showBox(false, "autoScrobbleFailed", 1);
		}
	}
}
...
function us_scrobble(artist,track,album,mbid,retry,queued,auto) {
	...
	document.getElementById("us_start_scrobblebutton").innerHTML = 'Scrobble <span id="us_autoscrobble_on" style="font-weight:bold; color: #66CC00;" title="scrobbling active">On</span>';
}
...
function us_resetlogin(error) {
	...
	cont = cont+'<div id="us_loginbox_form">'+
		resetInfo+
		'<span>Click Login below to authenticate your account</span><br/><br/>'+
		'<span style="font-style:italic;">Note: You will leave this site and be redirected here after having logged in to Last.FM </span><br/><br /></div><div class="us_submitbuttons"><input id="us_submit" value="Login" type="submit" /></div>';
	us_boxcontent('Login to last.fm',cont);
	document.getElementById('us_submit').addEventListener('click', us_authenticate, false);
}
	...
	if ((us_getTempData("artist")!=0) || (us_getTempData("track")!=0)) {
		...
		if (getDatabaseData()==true) {
			...
			if (location.href.indexOf("youtube.com/user/") != -1) {
				...
				var titleContent = document.getElementById("eow-title").textContent;
			}
			...
			titleContent = titleContent.replace(/"/gi, '');
			titleContent = titleContent.replace(/'/gi, '');
			...
			if ((musicInfo.length == 1)||(musicInfo[0] == false) || (musicInfo[1] == false)) {
				...
				var i = 1;
				while (i==1) {
					if (musicInfo[0].charAt(0) == false) {
						musicInfo[0] = musicInfo[0].substring(1, musicInfo[0].length);
					} else {
						i = 0;
					}
				}
				i = 1;
				while (i==1) {
					if (musicInfo[1].charAt(0) == false) {
						musicInfo[1] = musicInfo[1].substring(1, musicInfo[1].length);
					} else {
						i = 0;
					}
				}
				if (musicInfo.length == 2) {
					i = 1;
					while (i==1) {
						if (musicInfo[1].charAt(musicInfo[1].length-1) == false) {
							musicInfo[1] = musicInfo[1].substring(0, musicInfo[1].length-1);
						} else {
							i = 0;
						}
					}
...
function saveDatabaseData(id, artist, track) {
	...
	//New entry
		var ids = us_getValue("database.id", 0).split(" ");
		//Already 500 entries -> delete oldest and insert new
		if (ids.length>=500) {
			var artists = us_getValue("database.artist", 0).split(" ");
			var tracks = us_getValue("database.track", 0).split(" ");
...
function getVideoSecs () {
	if (document.getElementById('movie_player-html5')) {
		...
		if (us_getValue("us_compatibility-mode", false) == false) {
...
function actuallyStatus () {
	...
	if ((leftToPlay-1) > 0) {
		if (TO2 == 0) {
			TO2 = 1;
			TO2 = window.setInterval(function () {actuallyStatus();}, 1000);
		}
	} else {
...
function checkFirstRun () {
	...
	if (localVersion == 0) {
		...
		'<span>Join the <a target="_blank" href="http://www.last.fm/group/YouScrobbler">Last.fm Group</a> to stay tuned.</span><br/><br/>'+
					...
					onload: function(resp)
					{
						...
						if (remote_version > local_version)
						{
							...
							var cont =  '<div id="us_loginbox_form"><span>YouScrobbler '+ String(remote_version) +' is available.</span><br/>'+
										'<br/>'+
										'<span>Reload the Page after insatlling the update for the changes to take effect.</span><br/>'+
										'<br/>'+
							...
							document.getElementById("us_manualupdate").firstChild.innerHTML = "No Update avaiilable";
						}
					},
					onerror: function () {
						if ((forced)) {
