// ==UserScript==
// @name 		Travian AutoTask for SE (Swedish)
// @author 		congxz6688 (edited by Arias)
// @version 	1.2.4.2.0
// @description	Swedish translation of the original script
...
// @exclude 	*.css
// @exclude 	*.js
// @require 	http://sizzlemctwizzle.com/updater.php?id=56716
// ==/UserScript==


var crtVersion="1.2.4.2.0";

/** - Start of Version History - **

<strong><pre> - VERSION HISTORY:</pre></strong>

1.2.4.2.0 - 
* Add improve function at Blacksmith and Armory	// From original script update
* Debug for auto_Resource_upgrade			// From original script update
* Other debug						// From original script update
* Change to a much easier autoupdate
* Fixed some other bugs
* Added exclude option, so if you add a "&" after the url
	to a report you will get it clean without any script

1.2.4.1.8 -
* Removed arabic support, you can find it in other script
* Some translation changes (to make it look better)
* Schedule "horse soldiers" seems to work now
* Changed some text for the update popup
* Fixed the bug for Barack, now you can schedule training

1.2.4.1.7 - 
* Fixed some more translation
* Fixed some bugs (see "current bugs" list)
* Added support for Arabic

1.2.4.1.6 -
* Schemaläggning av residens fixat, även fester verkar följa med.
* Diverse översättningar
* Kör med version history i scriptet

  ** - End of Version History - **/

/** - Start of current bugs - **

<strong><pre> - CURRENT BUGS:</pre></strong>

- You cant schedule upgrade of the residens building 									[ X]  (seems to work after 1.2.4.1.6)
- You cant schedule traing of footsoldiers (dont see any buttom at all)							[ X] (fixed 1.2.4.1.8)
- You cant schedule traing of riding soldiers (you can see the buttom, but nothing happends if you push it)	[ X] (fixed 1.2.4.1.8)
- You cant schedule to build NEW buildings (dont see any buttom at all)						[ X] (fixed 1.2.4.1.7)
- Need some more right translation to errorText										[ X] (think I got it all now 1.2.4.1.7)
- Auto transport from market dont work											[X] (fixed 1.2.4.1.7)
...
switch (Xlang) {
	...
	case "se"://thanks to Arias
		...
		addTaskText = ["Lägg till uppgift", "Stil", "Aktiv by", "Task target", "Till", "Läge", "Kontruktions stöd", "Råvaro koncentration", "Flytta upp", "Flytta ner", "Radera", "Uppgifter", "Prio. ", "Radera alla uppgifterna"];
		...
		taskoftext = ["Schemalägg uppgradering", "Schemalägg ny byggnad", "Uppgradering: Råvaro fält", "Av", "På", "På", "Av", "The resource fields distribution of this village is ", "Autotransport", "Autotransport is not opened", "Opened", "Transport lyckades", "-  Händelser", "Trans_In_Limit", "Stand.", "Ändra ", "Trä/Lera/Järn", "Vete", "Schemalägg demolition", "Schemalägg attack", "Attack type", "Res tid", "antal upprepningar", "interval time","00:00:00","Catapult target","Random", "Okänd", "times", "Månad", "Dag", "Trupper skickade", "Schemalägg Träning","Tränings plats","Träningen klar","Anpassad transport","Sätt intervallen för omladdning av sidan","Detta är intevallen för omladdning av sida,\n standard är 20 minuter, vänligen ange ny intervall:\n\n","Trans_Ut_Kvar","Schemalägg fest","Liten fest","Stor fest","Sätt intervall av råvarukoncentration","minuter","Pausad","Igång","Starta","pause","Schedule Improve","Improve Attack","Improve defence","Uppgradera fält"];
		errorText = ["För få råvaror", "Dina arbetare är redan ute på jobb", "Byggnad klar", "Påbörjar byggnad", "Under utveckling", "Ditt magasin är för litet. Vänligen uppgradera ditt magasin för att fortsätta ditt byggnadsarbete.", "Din silo är för liten. Vänligen uppgradera din silo för att fortsätta ditt byggnadsarbete.", "Tillräckligt med resurser", "Brist på mat: utöka vetefälten först!", "En fest pågår redan.","There is already research going on"];
...
if (inputs.snapshotLength == 1 && 0 < inputs.snapshotItem(0).value.length) {
	...
	function createAutoResLink(){
		...
		autoResDiv.innerHTML = taskoftext[51].bold() + ":&#160;&#160;";		// Changed this one to make it look better at dorf1 ! -Arias
	...
	function createTaskListTable(){
		...
		if (taskss != "false") {
			...
			aTd5.setAttribute("colspan", "4");
				...
				aTd7.align="right"
				aTd7.innerHTML="&#160;&#160;"+"<a href='http://userscripts.org/scripts/show/56716' id='verdisp'  target='_blank'>"+crtVersion+"</a>"
