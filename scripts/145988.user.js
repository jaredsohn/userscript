...
(function($){
    ...
    function dlimg(a,b,c){
        ...
        GM_xmlhttpRequest({
            ...
            onload:function(f) {
                d=f.response;
                var j=0,t,n=new Uint8Array(d.length)//f.responseHeaders.match("Content-Length: (.+)")[1]/1);
    ...
    if(ph=="/member_illust.php"||(ph=="/bookmark.php"&&/[&?]id=\d+/.test(url)||url.indexOf("type=user")>0)){
        ...
        if(ph=="/bookmark.php"&&url.indexOf("type=user")>0&&url.indexOf("id=")>0)
            window.onfocus=(function(){
                ...
                function a(){
                    ...
                    for(var i in a)$(document.querySelector("a[href='member.php?id="+a[i]+"']")).parent().parent().hide(1200,function(){$(this).remove()});
                    $(document.querySelectorAll(".users a")).attr("target","_blank");
                    //console.log("[onfocus]:已从画师列表中移去忽略部分");
                }
                return function(e){if((f+1e3)<(f=e.timeStamp))a()};
            })();
        if(url.indexOf("type=user")<0||url.indexOf("id=")<0){
            if(ph=="/member_illust.php"&&url.indexOf("mode")<0){
                ...
                var a=window.localStorage["bkm"].split(","),c=location.search.match(/[&?]id=(\d+)/)[1],hs=a.indexOf(c);
                $(".pages:first").html("")
