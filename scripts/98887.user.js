...
//炒客论坛屏蔽某些人
if (lodex("www.chaoke.so") != -1) {
	var someone = ["哈哈哈哈", "CTMD"];
	for (j = 0; j < someone.length; j++) {
		$('tbody').each(function () {
			if ($(this).find('.author>cite>a').text().trim() == someone[j]) {
				$(this).detach();
			}
		});
		$('tr').each(function () {
			if ($(this).find('.postinfo>a').text().trim() == someone[j]) {
				$(this).detach();
			}
		});
	}
...
//youtube屏蔽袁腾飞
if (lodex("www.youtube.com") != -1) {
	$('li').each(function () {
		if ($(this).find('.title').text().indexOf("袁腾飞") != -1 || $(this).find('.title').text().indexOf("袁老师") != -1) {
			$(this).detach();
			GM_log("ok! 干掉了一个他妈的袁腾飞！");
		}
	});
}

//百度贴吧屏蔽某些人的帖子
if (lodex("tieba.baidu.com") != -1) {
	GM_addStyle(".l_thread_title{background-color:#6495ED !important;padding:0px 5px 0px 5px !important; width:906px !important; margin:-7px 0px 5px -16px !important; } .tb-editor-editarea{color:black !important;max-width:560px !important; padding-bottom:8px !important; padding-right:8px !important; background-color:#FFFFFF !important;}");
	$('tr').each(function () {
		if (($(this).find('td:eq(2)').text().indexOf("虎") != -1 && $(this).find('td:eq(2)').text().indexOf("狮") != -1) || ($(this).find('td:eq(2)').text().indexOf("藏獒") != -1)) {
			$(this).detach();
		}
	});
	$('li').each(function () {
		if (($(this).find('.th_lz').text().indexOf("虎") != -1 && $(this).find('.th_lz').text().indexOf("狮") != -1) || ($(this).find('.th_lz').text().indexOf("藏獒") != -1)) {
			$(this).detach();
		}
	});
	var someone = [
	...
	for (j = 0; j < someone.length; j++) {
		$('tr,li').each(function () {
			if ($(this).find('td:eq(3),.th_author').text().trim() == someone[j]) {
				$(this).detach();
			}
		});
		$('.l_post').each(function () {
			if ($(this).find('.d_author_anonym,.d_name>a').text().trim() == someone[j]) {
				$(this).detach();
			}
		});
	}
