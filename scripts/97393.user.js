...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y'); 
 ...
 Options.alertConfig.raidautoswitch[m.toCityId] = true; 
  ...
  postToChat : function (m){
    ...
    msg += 'La mia '+ target +' e\' '+ atkType  +' da '+ who +'. Truppe in arrivo (arrivano tra '+
...
check:false, 
 ...
 m += '</tr></table></div>';
 m += '<DIV id=pbTraderDivDRoute class=pbStat>AGGIUNGI PERCORSO</div>'; 

 m += '<TABLE id=pbaddtraderoute width=95% height=0% class=pbTab><TR align="left">';
 m += '<TD>Da:</td> <TD width=310px><DIV style="margin-bottom:10px;"><span id=ptrescity></span></div></td></tr>'; 
 ...
 m += '<DIV style="margin-top:10px;margin-bottom:5px;">Se l\'ammontare su "invia" e\' 0 allora mandera\' l\'ammontare che si trova su "tieni". L\'oro solo se c\'e\' spazio...</div>';
 ...
 m += '<DIV style="text-align:center; margin-top:15px"><INPUT id=pbSaveRoute type=submit value="Aggiungi percorso"></div>';
...
params.queue[0].cityMarches.knightId = parseInt(document.getElementById ('AddKnights').value);
 ...
 if (rslt.ok) {
 t.stopprogress = t.stopprogress + (100/Seed.cities.length);
 actionLog('Riprendo: '+ Seed.cities[i][1]);
 updateraidbutton('Riprndo: '+ t.stopprogress.toFixed(0) + '%', 'pbraidtabRes');
 if (t.stopprogress.toFixed(0) == 100) {
 t.stopprogress = 0;
 setTimeout(function(){updateraidbutton('Ripendo Raids ('+ t.stopcount + ')', 'pbraidtabRes');t.resuming = false;}, (5000));
 }
 }
 else {
...
new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/_dispatch.php" + unsafeWindow.g_ajaxsuffix, {
 ...
 updateraidbutton('Cancello: '+ t.stopprogress.toFixed(0) + '%', 'pbraidtabDel');
 if (t.stopprogress.toFixed(0) == 100) {
 t.stopprogress = 0;
 t.GetRaids(cityId);
 setTimeout(function(){updateraidbutton('Cncella Raids ('+ t.stopcount + ')', 'pbraidtabDel');t.deleting = false;}, (5000));
 } 
...
checkRaids : function (){
 ...
 params.subject = "Resoconto Raids";
 var message = 'Statistiche:' + '%0A';
 message += '%0A'+'%0A' + 'Cibo raccolto (in 1 oa di babari)' +'%0A';
 ...
 message+= Seed.cities[q-1][1] + ': Iniziale: ' + addCommas(AttackOptions.Foodstatus[q]) + ' Finale :' + addCommas(parseInt(Seed.resources[cityID]['rec1'][0] / 3600)) + ' Raccolto: ';
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=PubReq type=checkbox '+ (GlobalOptions.autoPublishGamePopups?'CHECKED ':'') +'/></td><TD>Pubblica in automatico le richieste nella bacheca a '+ htmlSelector({80:'Tutti', 50:'Amici degli amici', 40:'Solo amici', 10:'Solo io'},GlobalOptions.autoPublishPrivacySetting,'id=selectprivacymode') +' (Tutti i domini)</td>\
...
Tabs.AutoTrain = {
  ...
  myDiv: null,
      ...
      init: function(div){
       ...
       var m = '<DIV id=pbAT class=pbStat>ADDESTRAMENTO AUTOMATICO</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR>';
 ...
 m += '<DIV id=pbAT class=pbStat>OPZIONI</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR align="center">';
...
var FairieKiller  = {
  ...
  setEnable : function (tf){
    if (tf)
      unsafeWindow.Modal.showModalUEP = eval ('function (a,b,c) {actionLog ("Popup delle fiere bloccato");}');
    else
