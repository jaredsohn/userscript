...
function openCommentButtons(comment) {
    var title = xpathNode("(./div[@class='sict_open'] | ./div[@class='sict_open']/div)/div[@class='commentTop']/div[@class='title']/h4", comment);
    if (!title)
        return;
...
function insertOpen(comments) {
    ...
    for (var i = 0; i < openComments.snapshotLength; i++) {
        ...
        var cid;
        try {
            cid = xpathString("(./div | ./div/div)[@class='commentTop']/div[@class='details']/span[@class='otherdetails']/a[last()]/@href", comment).match(/(cid=|#)(\d+)/)[2];
        } catch (e) {
            continue;
        }
...
function nextOpenComment(i, single) {
    ...
    var commentTop = xpathNode("(./div | ./div/div)[@class='commentTop']", comment);
    if (!commentTop)
        continue;
