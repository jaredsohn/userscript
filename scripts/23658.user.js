// ==UserScript==
// @name          Kolay Kullanım Turkce v3.0.2.9
// @namespace     http://none
// @description   Bursat tarafından türkçeleştirilmiştir.
// @include       http://*.klansavaslari.net/*
// @version       3.0.2.9 Turkce
// ==/UserScript==
...
// nombre del world más el idioma, ej. w3es
var baseURL = getParam('GETURL');	// Url del world al que estamos conectados
var language = baseURL.match(/(es)|(de)|(pl)|(it)|(fr)|(ro)|(nl)|(net)/g);  // Idioma seleccionado
	if(language == null) {language = "es";}
	
var wID = baseURL.match(/\b[ds|tw|pl|w]\d+\b/g) + language;  // Mundo en el que nos encontramos
var wNum = baseURL.match(/[0-9]+/); // Numero que identifica el mundo

var wVersion = GM_getValue(wID+"_version", "0");
...
var twMenuLine = new Array();
var villagesBuildingsLevel = new Array(0);  // Listado de los niveles de cada edificio
var infoPlayer = new Array();
var UnitNum = 0;

var extImgsPath = "http://s184.photobucket.com/albums/x195/twextension/"; // Ruta de las imágenes
var twImgsPath = "http://www.tribalwars.es/graphic/"; 										//  Ruta para las imágenes originales de tw
// http://i184.photobucket.com/albums/x195/twextension/
var twUnitsNamesArray = new Array();
var twCostsByUnit = new Array();
var twFarmVillagersByUnit = new Array();
var twSpeedByUnit = new Array();

var twBuildingsNamesArray = new Array();
...
// Method: resetValue
// Summary: reset  world value
// Params: nothing
// Returns: nothing
function initWorld() {

if (wVersion != version){
	GM_setValue (wID+"_version", version);
	GM_setValue (wID+"_UnitsArmy", 0);
	GM_setValue (wID+"_WorldBuildings",0);
	return false;
	}
	else
	{
	var InitValue = twInitWorldArmy() & twInitWorldBuildings();
	return (InitValue);
	}
}
//__________________________________________________________________________________________________________________//
...
{
//return true;
/*	var adsFrame = window.parent.document.getElementById('main');
	if (window.parent.document.title =="Tribal Wars"){
	if (adsFrame){
		window.parent.location.href = window.parent.location.href.replace("&top","")+"&sky";
		return false;
	}
	else{
		var elemF = window.parent.document.getElementsByTagName('iframe');
		elemF[1].parentNode.style.width = "0px";
		elemF[0].parentNode.style.right = "0px";
	
		return true;
	}
	}
*/
/*	var adsFrame = window.parent.document.getElementsByName('ad');
	if(adsFrame[0] == null){
		url = document.location.href 
		alert (url);
		//window.parent.location.href = window.parent.location.href.replace("&top","")+"&sky";
		//window.parent.location.href = url+"&sky";
		return false;
	}
	else{
...
*/
	var adsFrame = window.parent.document.getElementsByTagName('frameset')[0].cols;
	if(adsFrame == ""){
		url = document.location.href; 
		window.parent.location.href = window.parent.location.href.replace("&top","")+"&sky";
		return false;
...
//Summary:
// Params: string
// Returns:Indice in array
 
Array.prototype.ArrayIndice=function(valeur){
var place=-1
...
function twLoadingData(mode)
{
	...
	{ 
		...
		newHTML.innerHTML = '<div style="width:300px;margin-top:150px;margin-bottom:auto;background-color:#F8F4E8;border:2px solid #804000;padding:30px 30px 30px 40px;margin-left:auto; margin-right:auto;"><img src="http://i184.photobucket.com/albums/x195/twextension/loading.gif" border="0" hspace=20 align="middle"><b>'+twLanguage['general'][0]+'</b></div>';
...
// Method: 
// Summary: Evita el molesto mensaje y la redirección obligatoria a la zona premium.
// Params: nothing
// Returns: nothing
/*
function(){
...
*/	
	...
	if(getParam('screen') == 'map'){
		...
		if(pID == 0 || pID != pIDold){
		twGetpID(); 
		while ((pID != pIDold) || (pID == 0)){pID = GM_getValue(wID+"_pID", 0);}
		//return;
		}
		}
	...
	twMenuLine["twMainMenuArray"] = new Array();
	twLanguage["mainMenu"] = new Array();
	twMenuLabel["mainMenu"] = new Array();

//	var patt = /screen=info_player/;
	var targetPlayer = window.document.body.getElementsByTagName("a");
	var tarNumb = targetPlayer.length;
		for (var a=0 ; a < tarNumb; a++){
		var pmatch = targetPlayer[a].toString().match("screen=info_player");
			if (pmatch) {
			var newinfoPlayer = document.createElement("div");
			newinfoPlayer.setAttribute('id','btnPlayerPopUp'+a);
			newinfoPlayer.setAttribute('class','btnPlayerPopUp');
			targetPlayer[a].parentNode.insertBefore(newinfoPlayer,targetPlayer[a]);
			newinfoPlayer.appendChild(targetPlayer[a]);
			}
		}
	// Eventos
		for (var a=0 ; a < tarNumb; a++){
		if($('btnPlayerPopUp'+a)){
			$('btnPlayerPopUp'+a).addEventListener('mouseover', function(Event) {
					twPlayerPopUp(Event)
			}, true );		
		}
		}
...
*/
	// Enlaces: MiniImágenes de los Recursos, granja y otros
	...
	twLinks["twUtils"][0] = "http://www.compactador.com.ar/TW/index.php";
	twLinks["twUtils"][1] = "http://www.tribalpacker.mx.gs";
	twLinks["twUtils"][2] = "http://www.prosur.com/tribalwars/inicio.php";
	twLinks["twUtils"][3] = "http://tribalwars.servegame.org";
	twLinks["twUtils"][4] = "http://www.guscreations.com/item.php?key=buscador_tribalwars";
	twLinks["twUtils"][5] = "http://www.savefile.com/files/489797";
	twLinks["twUtils"][6] = "http://granjerodepro.es.kz";
	twLinks["twUtils"][7] = "http://www.tw-tools.com/";
	twLinks["twUtils"][8] = "http://w1.dsplus.de/index.php?tool=units";
	twLinks["twUtils"][9] = "http://w1.dsplus.de/index.php?tool=buildings";	
	twLinks["twUtils"][10] = "http://tribal.texplico.com/tribal/index.php";
	
	// Datos sobre los Recursos:
	// 0->Madera Almacenada, 1->Barro Almacenado, 2->Hierro Almacenado, 3->Capacidad Almacén
	var resourcesPath = "/html/body/table[2]/tbody/tr/td[2]/table/tbody/tr/td/table/tbody";	
	twGlobalInfo["resources"] = new Array();
	twLanguage["resources"] = new Array ();
	// wood
	twLanguage["resources"][0]= document.evaluate(resourcesPath+"/tr/td[1]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML.toString().match("(title=\")(.*(?=\" alt))")[2];
	twGlobalInfo["resources"][0] = document.evaluate(resourcesPath+"/tr/td[2]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML;
	//stone
	twLanguage["resources"][1]= document.evaluate(resourcesPath+"/tr/td[3]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML.toString().match("(title=\")(.*(?=\" alt))")[2];
	twGlobalInfo["resources"][1] = document.evaluate(resourcesPath+"/tr/td[4]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML;
	//iron
	twLanguage["resources"][2]= document.evaluate(resourcesPath+"/tr/td[5]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML.toString().match("(title=\")(.*(?=\" alt))")[2];
	twGlobalInfo["resources"][2] = document.evaluate(resourcesPath+"/tr/td[6]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML;
	//storage capacity
	twLanguage["resources"][3]= document.evaluate(resourcesPath+"/tr/td[7]",document, null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML.toString().match("(title=\")(.*(?=\" alt))")[2];
...
//		twGlobalInfo["InterfaceItems"][4] = extImgsPath+"overview.png";
		twGlobalInfo["InterfaceItems"][4] = "http://s184.photobucket.com/albums/x195/twextension/overview.png";
...
// Method: twLanguageData
// Summary: Inicializa Tribal eXtension Languages 
// Params: nothing
// Returns: nothing
function twLanguageData()
{	

if (language =="net"){	
	twLanguage["mainMenuTitle"] = new Array("Ordular", "Depo", "Popilasyon", "Köy");
//	twLanguage["resources"] = new Array ("Odun", "Kil", "Demir", "Capacit&eacute;&nbsp;de&nbsp;stockage");
	twLanguage["VillageNav"] = new Array("Yenile", "Merkez Binası","Harita","Köy Listesi","Ordu Listesi");
	twLanguage["reportKeys"] = new Array("Bot&iacute;n:", "Da&ntilde;o con arietes", "Da&ntilde;o causado con catapultas", "Aprobaci&oacute;n disminuida", "Edificios", "unidades fuera", "Materias Primas espionados");
	twLanguage["report"] = new Array("Bot&iacute;n", "Da&ntilde;o con arietes", "Da&ntilde;o causado con catapultas", "Aprobaci&oacute;n disminuida", "Edificios", "unidades fuera", "Materias Primas espionados");
  	twLanguage["farm"] = new Array("Kullanılan_Popilasyon", "Toplam_Popilasyon", "Kalan_Popilasyon");
	twLanguage["VillageInfo"] = new Array();
		twLanguage["VillageInfo"]["Info"] ="Oyuncu Bilgileri"; 
		twLanguage["VillageInfo"]["Coords"] ="Koordinat"; 
		twLanguage["VillageInfo"]["Points"] ="Puan"; 
		twLanguage["VillageInfo"]["Player"] ="Oyuncu"; 
		twLanguage["VillageInfo"]["Tribe"] ="Klan"; 
		twLanguage["VillageInfo"]["Action"] ="Komutlar"; 
		twLanguage["VillageInfo"]["Map"] ="Haritada Ortala"; 
		twLanguage["VillageInfo"]["Units"] ="Üniteler"; 
		twLanguage["VillageInfo"]["Time"] ="Ulaşım Süresi"; 
		twLanguage["VillageInfo"]["Details"] ="Bilgi"; 
		twLanguage["VillageInfo"]["PlayerHistory"] ="Oyuncu Bilgisi"; 
		twLanguage["VillageInfo"]["TribeHistory"] ="Klan Bilgisi";
	twLanguage["general"]= new Array ("Data Yükleniyor ...");
...
function twInitNewCSS()
{
	...
	newCSS += '.btnPlayerPopUp, .btnPlayerPopUp div {display: inline;opacity:1;background-color:#F1EBDD;}';
	newCSS += '.btnPlayerPopUp div ul, .btnPlayerPopUp div ul ul, .btnPlayerPopUp div ul ul ul {display:none; }';	
	newCSS += '.btnPlayerPopUp ul {position:absolute; display:none; padding:0; list-style: none; border:2px solid #DED3B9; background-color:#F8F4E8;}';
	newCSS += '.btnPlayerPopUp li {position:relative; float:left; line-height:1.25em; width:98px; margin-bottom:-1px;  border-bottom:1px solid #DED3B9; padding:2px; text-align:center; background:transparent}';
	newCSS += '.btnPlayerPopUp li a {display : block;	font-weight : bold;	text-decoration : none;background:transparent}';
	newCSS += '.btnPlayerPopUp li a:hover {color:#0082BE; background:#FFF;opacity:10;}';
	newCSS += '.btnPlayerPopUp:hover div {display:inline; list-style: none;}';
	newCSS += '.btnPlayerPopUp:hover div ul {display:inline; width:100px;margin-top:0em; opacity:0.3;}';
	newCSS += '.btnPlayerPopUp:hover div:hover ul {opacity:1;}';
	newCSS += '.btnPlayerPopUp:hover div ul div,.btnPlayerPopUp:hover div ul ul, .btnPlayerPopUp:hover div ul ul ul {display:none;}';	
	...
	newCSS += '#twResourcesInfo {position:absolute;top:131px;margin-left:100px;z-index:1000;}';
	...
	// Css Menú desplegable de los pueblos
	...
	newCSS += '#VillageInfoResources {position:absolute;top:43px;left:0;margin-left:160px;z-index:1000;}';
	...
	// Css Para posicionar la Barra de Navegación superior
	newCSS += '#twNavBar {width:850px;height:150px;margin-top:40px;z-index:1010}';
	
	// Css para posicionar el popup de información de cada jugador
...
function twInitInterface()
{
	...
	newHTML += '</div>';
/*
	newHTML += '<div style="position:absolute; left:220px; top:16px; width:15px; height:28px;">';
	newHTML += '	<img src="'+twGlobalInfo["InterfaceBackgroundParts"][19]+'" width="15" height="28" alt="">';
	newHTML += '</div>';
	newHTML += '<div style="position:absolute; left:240px; top:45px; width:15px; height:29px;">';
	newHTML += '	<img src="'+twGlobalInfo["InterfaceBackgroundParts"][20]+'" width="15" height="29" alt="">';
	newHTML += '</div>';
	newHTML += '<div style="position:absolute; left:710px; top:45px; width:15px; height:29px;">';
	newHTML += '	<img src="'+twGlobalInfo["InterfaceBackgroundParts"][20]+'" width="15" height="29" alt="">';
	newHTML += '</div>';
*/
...
// Method: twInitWorldArmy
// Summary: 
// Params:
// Returns: nothing
function twInitWorldArmy()
{
	var WorldUnits = GM_getValue(wID+"_UnitsArmy" ,0);
	if(WorldUnits == 0) {
	return false;
	}
	else
	{
	twUnitsNamesArray = new Array();
	twCostsByUnit = new Array();
	twFarmVillagersByUnit = new Array();
	twSpeedByUnit = new Array();
	twLinks["twUnitsImg"] = new Array();
	twLanguage["units"] = new Array();
	
	var UnitParam = new Array();
	var UnitsRaw = WorldUnits.split("UnitRaw=");
	UnitNum = UnitsRaw.length-1;
	for (var i=1; i<=UnitNum;i++){
		//alert (UnitsRaw[i]);
		UnitParam = UnitsRaw[i].split(";;");
		//var ParamNum = UnitParam.length;
		twUnitsNamesArray[i-1] = UnitParam[0];
		twCostsByUnit[i-1] = new Array(UnitParam[3],UnitParam[4],UnitParam[5]);
		twFarmVillagersByUnit[i-1] = UnitParam[6];
		twSpeedByUnit[i-1] = UnitParam[11];
		twLanguage["units"][i-1] = UnitParam[1];
		// Enlaces: MiniImágenes de las Unidades
		twLinks["twUnitsImg"][i-1] = UnitParam[2];
		}
	return true;
	}
}
...
// Method: twGetWorldUnits
// Summary:  Names of units
// Params: 
// Returns: nothing
function twGetWorldArmy()
{
	GM_xmlhttpRequest({ 
		method: 'GET',
		url: baseURL+'/help.php?mode=units',
		headers: { 'User-agent': 'Mozilla/4.0', 'Accept': 'text/xml',},
		onload: function(responseDetails) 
		{
			var newHTML = document.createElement("div");
			newHTML.setAttribute('id', 'WorldUnits');
			newHTML.innerHTML = responseDetails.responseText;
			document.body.insertBefore(newHTML, document.body.childNodes[0]);
			var path = 'html/body/div/table/tbody/tr/td/table[2]/tbody/tr/td[2]/table/tbody';		
			var subHTML = document.evaluate(path,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue ;
			var rawsArmy = subHTML.getElementsByTagName("tr").length;
			var UnitsCollection= "";
			var UnitNode ="";
			var i=0;
			var j=0;
			for( i = 2; i <= rawsArmy; i++){
			UnitsCollection += "UnitRaw=";
			UnitNode = document.evaluate(path + '/tr['+i+']',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
			
//Nom natif;; Nom ;;icone path;; Bois ;; Argile ;; Fer ;; Villageois ;; Force offensive ;; Défense générale ;; Défense de la cavalerie ;; Défense archers ;; Vitesse ;; Butin;;
			UnitsCollection += UnitNode.childNodes[1].innerHTML.toString().match("(unit=)(.*(?=',))")[2]+";;";
			UnitsCollection += UnitNode.childNodes[1].childNodes[0].text.match("( )(.*)")[2]+";;";
			UnitsCollection += UnitNode.getElementsByTagName("img")[0].src+";;";
			
			for ( j = 1 ; j < 11 ; j++){UnitsCollection += UnitNode.getElementsByTagName("td")[j].textContent+";;" ;}
			}
			
			GM_setValue(wID+"_UnitsArmy", UnitsCollection);			
...
// Method: twInitWorldBuildings
// Summary: 
// Params:
// Returns: nothing
function twInitWorldBuildings()
{
	var BuildingsList = GM_getValue(wID+"_WorldBuildings" ,0);
	if(BuildingsList == 0) {
	return false;
	}
	else
	{
	twBuildingsNamesArray = new Array();
	twLinks["twBuildingsImg"] = new Array();
	twLanguage["buildings"] = new Array();
	
	var BuildingParam = new Array();
	var BuildingsRaw = BuildingsList.split("BuildingRaw=");
	var BuildingsNum = BuildingsRaw.length-1;
	for (var i=1; i<=BuildingsNum;i++){
		BuildingParam = BuildingsRaw[i].split(";;");
		twBuildingsNamesArray[i-1] = BuildingParam[0];
		twLanguage["buildings"][i-1] = BuildingParam[1];
		twLinks["twBuildingsImg"][i-1] = BuildingParam[2];
		}
	return true;
	}
}
...
// Method: twGetWorldBuildings
// Summary:  Names of buildings
// Params: 
// Returns: nothing
function twGetWorldBuildings()
{
	GM_xmlhttpRequest({ 
		method: 'GET',
		url: baseURL+'/help.php?mode=buildings',
		headers: { 'User-agent': 'Mozilla/4.0', 'Accept': 'text/xml',},
		onload: function(responseDetails) 
		{
			var newHTML = document.createElement("div");
			newHTML.setAttribute('id', 'WorldBuildings');
			newHTML.innerHTML = responseDetails.responseText;
			document.body.insertBefore(newHTML, document.body.childNodes[0]);
			var path = '/html/body/div/table/tbody/tr/td/table[2]/tbody/tr/td[2]/table/tbody';		
			var subHTML = document.evaluate(path,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue ;
			var rawsBuilding = subHTML.getElementsByTagName("tr").length;
			var BuildingsCollection= "";
			var i=0;
			for(i = 1; i <= rawsBuilding; i++)
			{
			BuildingsCollection += "BuildingRaw=";
			var BuildingNode = (document.evaluate(path + '/tr['+i+']',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue);
			
//Nom natif;; Nom ;;icone path;; 
			BuildingsCollection += BuildingNode.childNodes[1].innerHTML.toString().match("(building=)(.*(?=',))")[2]+";;";
			BuildingsCollection += BuildingNode.childNodes[1].childNodes[1].text.match("( )(.*)")[2]+";;";
			BuildingsCollection += BuildingNode.getElementsByTagName("img")[0].src+";;";
				
			}
			GM_setValue(wID+"_WorldBuildings", BuildingsCollection);	
			newHTML.parentNode.removeChild(newHTML);
		}
	})
return true;
...
function twGetpID()
{
	...
	{	
		...
		GM_xmlhttpRequest({
			...
			headers: {'User-agent': 'Mozilla/4.0','Accept': 'text/html'},
			onload: function(responseDetails) {
	...
	GM_xmlhttpRequest({ 
		...
		onload: function(responseDetails) 
		{
			...
			if(txt.test(/action=cancel/) && txt.test(/mode=destroy/)){
				path = '/html/body/div/table[3]/tbody/tr/td/table[3]/tbody/tr[2]/td/table['+(txt.split(/screen=premium/).length -1) +']/tbody/';}
			if(!txt.test(/action=cancel/) && txt.test(/mode=destroy/)){
				path = "/html/body/div/table[3]/tbody/tr/td/table[2]/tbody/tr[2]/td/table/tbody/";}
			if(txt.test(/action=cancel/) && !txt.test(/mode=destroy/)){
				path = "/html/body/div/table[3]/tbody/tr/td/table["+ (txt.split(/screen=premium/).length + 1) +"]/tbody/";
				}
		/*	if(txt.test(/action=cancel/) && txt.test(/mode=destroy/)) path = "/html/body/div/table[3]/tbody/tr/td/table[3]/tbody/tr[2]/td/table[2]/tbody/";
			if(!txt.test(/action=cancel/) && txt.test(/mode=destroy/)) path = "/html/body/div/table[3]/tbody/tr/td/table[2]/tbody/tr[2]/td/table/tbody/";
			if(txt.test(/action=cancel/) && !txt.test(/mode=destroy/)) path = "/html/body/div/table[3]/tbody/tr/td/table[3]/tbody/"
		*/		
			do
			{
				str = ""; 		
...
{
	//twLanguage["resources"] = new Array ("Madera", "Barro", "Hierro", "Almac&eacute;n");
	var newDiv = "";
	newDiv += "<div id='VillageInfoResources'>";
	newDiv += "<table>";
	newDiv += "		<tbody>";
	newDiv += "			<tr>";
	newDiv += "				<th style='color:#F1EBDD;background:url("+twGlobalInfo["InterfaceBackgroundParts"][20]+") #804000 right no-repeat; width:auto;height:29px;padding:2px 10px 2px 2px;margin-right:5px'>"+twLanguage["mainMenuTitle"][1]+': </th>';
	newDiv += "				<td title=\""+twLanguage['resources'][0]+"\" style='padding:0 0 0 15px'><img src='"+twLinks["twOtherImg"][0]+"' /></td><td>"+twGlobalInfo["resources"][0]+"</td>";
	newDiv += "				<td title=\""+twLanguage['resources'][1]+"\"><img src='"+twLinks["twOtherImg"][1]+"' /></td><td>"+twGlobalInfo["resources"][1]+"</td>";
	newDiv += "				<td title=\""+twLanguage['resources'][2]+"\"><img src='"+twLinks["twOtherImg"][2]+"' /></td><td>"+twGlobalInfo["resources"][2]+"</td>";
	newDiv += "				<td title=\""+twLanguage['resources'][3]+"\"><img src='"+twLinks["twOtherImg"][3]+"' /></td><td id='storage'>"+twGlobalInfo["resources"][3]+"</td>";
	newDiv += "			</tr>";
	newDiv += "		</tbody>";
	newDiv += "	</table>";
	newDiv += "</div>";
	// Añadimos los recursos a la Barra de navegación
	if($('VillageInfoResources')) $('VillageInfoResources').parentNode.removeChild($('VillageInfoResources'));
	$('twVillageNavBar').innerHTML += newDiv;


	var bois = document.getElementById('wood');
	var pierre = document.getElementById('stone');
	var fer = document.getElementById('iron');
	var store = parseInt(document.getElementById('storage').innerHTML);

	var secFull,fullStr;

	secFull = (3600*(store-parseInt(bois.innerHTML))/parseInt(bois.title));
	fullStr = t_format2(Math.ceil(Math.max(0,secFull))).slice(0,-3);
	if (parseInt(bois.title)==store)
		fullStr = "<b>" + fullStr + "</b>";
	bois.innerHTML=bois.innerHTML+"<br />"+fullStr;
	bois.style.fontSize="8pt";

	secFull = (3600*(store-parseInt(pierre.innerHTML))/parseInt(pierre.title));
	fullStr = t_format2(Math.ceil(Math.max(0,secFull))).slice(0,-3);
	if (parseInt(pierre.title)==store)
		fullStr = "<b>" + fullStr + "</b>";
	pierre.innerHTML=pierre.innerHTML+"<br />"+fullStr;
	pierre.style.fontSize="8pt";
	
	secFull = (3600*(store-parseInt(fer.innerHTML))/parseInt(fer.title));
	fullStr = t_format2(Math.ceil(Math.max(0,secFull))).slice(0,-3);
	if (parseInt(fer.title)==store)
		fullStr = "<b>" + fullStr + "</b>";
	fer.innerHTML=fer.innerHTML+"<br />"+fullStr;
	fer.style.fontSize="8pt";
...
function twShowInfoFarm()
{
	...
	newDiv += "	<tbody>";
	newDiv += "		<tr>";
	newDiv += "			<th style='background:url("+twGlobalInfo["InterfaceBackgroundParts"][20]+") #804000 right no-repeat;color:#F1EBDD;width:auto;height:29px;padding:2px 10px 2px 8px;'>"+twLanguage["mainMenuTitle"][2]+": </th>";
	newDiv += "			<td title="+twLanguage["farm"][0]+" style='padding:0 0 0 12px'><img src='"+twLinks["twOtherImg"][4]+"' /></td><td>"+twGlobalInfo["farm"][0] +"</td>";
	newDiv += "			<td title="+twLanguage["farm"][1]+"><img src='"+twLinks["twOtherImg"][4]+"' /></td><td>"+twGlobalInfo["farm"][1]+"</td>";
	newDiv += "			<td title="+twLanguage["farm"][2]+"><img src='"+twLinks["twOtherImg"][4]+"' /></td><td style='color:green;'><b>"+twGlobalInfo["farm"][2]+"</b></td>";
	newDiv += "		</tr>";
	newDiv += "	</tbody>";
	newDiv += "</table>";
	newDiv += "</div>";
	// Añadimos los datos de la granja a la Barra de Navegación
...
function twShowInfoArmy(village_id)
{
	...
	newDiv += "	<tbody>";
	newDiv += "		<tr>";
	newDiv += "			<th style='background: url("+twGlobalInfo["InterfaceBackgroundParts"][19]+") #804000 right no-repeat;color:#F1EBDD;width:auto;height:28px;padding:2px 10px 2px 2px;'>"+twLanguage["mainMenuTitle"][0]+":";
//	newDiv += '<img src="'+twGlobalInfo["InterfaceBackgroundParts"][20]+'" width="15px" height="28" alt=""></th>';
	for(var i=0;i<UnitNum;i++){
		if(Army[village][twUnitsNamesArray[i]] > 0 && Army[village][twUnitsNamesArray[i]] != null) newDiv += "		<td title=\""+twLanguage["units"][i]+"\" style='padding:0 5px 0 15px;'><img src='"+twLinks["twUnitsImg"][i]+"'></td><td>"+Army[village][twUnitsNamesArray[i]]+"</td>";
	}
	newDiv += "		</tr>";
	newDiv += "	</tbody>";
	newDiv += "</table>";
	...
	if($('VillageInfoArmy')) 
		...
		$('twVillageNavBar').innerHTML += "<div id='VillageInfoArmy' style='border:0;padding:0'>" +newDiv+"</div>";
...
function twGlobalResumeArmy()
{
		...
		var newHTML ="";
		newHTML += "<h2>"+twLanguage["VillageNav"][4]+"</h2>";
/*		newHTML += "<table id='GlobalArmyInfo' width=100% style='text-align:left;'>"
		newHTML += "	<tr style='text-align:center'><th rowspan=2>Nombre</th><th rowspan=2>Puntos</th><th colspan=9 width:80% align='center'>Tropas</th></tr>";
		newHTML += "	<tr>";
		
		for(i=0;i<twUnitsNamesArray.length;i++)
	...
	*/
		newHTML += "<table id='GlobalArmyInfo' width=100% style='text-align:left;'>"
		newHTML += "	<tr style='text-align:center'><th rowspan=2>"+twLanguage["mainMenuTitle"][3]+"</th><th rowspan=2>"+twLanguage["VillageInfo"]["Points"]+"</th><th colspan="+UnitNum+" style='width:70%; text-align:center'>"+twLanguage["mainMenuTitle"][0]+"</th></tr>";
		newHTML += "	<tr>";
		
		for(i=0;i<UnitNum;i++)
			newHTML += "<th title=\""+twLanguage["units"][i]+"\"><center><img src='"+twLinks["twUnitsImg"][i]+"'></center></th>";
		...
		//document.createTextNode("<tr><th>Nº</th><th>Pueblo</th><th colspan='9'>Unidades</th");
...
function twGlobalResumeArmy_AddVillageInfo(village)
{
	...
	newTD = document.createElement("td")
	newTD.setAttribute("style", "padding:0 5px 0 5px; text-align:right;")
	...
	for(var i=0;i<UnitNum;i++){
		newTD = document.createElement("td")
		newTD.setAttribute("title", twLanguage["units"][i])
		newTD.setAttribute("style", "padding:0 5px 0 5px; text-align:right;")
...
function twGetVillagesList()
{
	GM_xmlhttpRequest({
			...
			onload: function(responseDetails) {
					...
					var j = 0;
					nVillages = 0;					  
//  /html/body/div/table/tbody/tr/td/table[2]/tbody/tr/td/table[2]/tbody/tr[2]/td/a
// /html/body/table[3]/tbody/tr/td/table/tbody/tr[2]/td[1]/span[@id='label_36717']/a/span[@id='label_text_36717']

					while(document.evaluate('/html/body/div/table/tbody/tr/td/table[2]/tbody/tr/td/table[2]/tbody/tr['+i+']/td',document,null,XPathResult.STRING_TYPE,null).stringValue != "")
					{
...
// Summary: Dada una id de un pueblo nos devuelve la posición de éste dentro del array de pueblos.
// Params: village
// Returns: id
function twGetVillageNum(id)
{
	...
	var k = 0;
	// Obtiene la posición del pueblo que buscamos en la matriz de pueblos
	while( i > 0){
		if(Villages[k]["id"] == id)
			village = k;
		i--;
...
{	
	try{
		...
		// Comprueba si nos están atacando en ese pueblo, en caso afirmativo muestra un icono al lado del nombre del pueblo
		...
		newDiv += '		<tbody>';
		newDiv += '			<tr>';
		newDiv += '				<td style="border-right:1px solid #804000"><a id="btnDataRefresh" href="javascript:void(0)"><img src="'+twGlobalInfo["InterfaceItems"][7]+'" title="'+twLanguage["VillageNav"][0]+'" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["overview"]+'"><img src="'+       twGlobalInfo["InterfaceItems"][4]+'" title="'+twLanguage["VillageNav"][1]+'" />&nbsp;</a></td>';
/*'+twGlobalInfo["InterfaceItems"][4]+'"*/		
//		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["overview"]+'"><img src="'+twGlobalInfo["InterfaceItems"][4]+'" title='+twLanguage["VillageNav"][1]+' />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["map"]+'"><img src="'+twGlobalInfo["InterfaceItems"][5]+'" title="'+twLanguage["VillageNav"][2]+'" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["villages"]+'"><img src="'+twGlobalInfo["InterfaceItems"][8]+'" title="'+twLanguage["VillageNav"][3]+'" />&nbsp;</a></td>';
		newDiv += '				<td><a id="btnShowAllArmy" href="javascript:void(0)"><img src="'+twGlobalInfo["InterfaceItems"][6]+'" title="'+twLanguage["VillageNav"][4]+'" />&nbsp;</a></td>'
		newDiv += '			</tr>'
		newDiv += '		</tbody>';
		newDiv += '	</table>';
		newDiv += '</div>';

		// Añadimos el Menú desplegable de pueblos a la barra de Navegación
	...
	// Eventos del Menú Desplegable de Pueblos
	if($('btnVillagesList'))
	{
		var vDropDown = new Fx.Slide('twVillagesList', {duration: 200}); // Desplegable
...
// Eventos para el menú de la barra de navegación
	if($('btnShowAllArmy')){
...
function twCalculatorTimeByUnit(oVillage, dVillage)
{
	...
	for(var i=0;i<UnitNum;i++)
	{
		...
		// Añadido a la Matriz de distancias
...
// Summary: Devuelve los costes en recursos de la cantidad de tropas que recibe al ser llamada la función.
...
{
	twShowBuildingsLevel();
	var myNewSize = 14; // 1280
	if (screen.width == 1152) myNewSize = 12;
	if (screen.width == 1024) myNewSize = 10;
	if (screen.width == 800) myNewSize = 8;
	
	var oMap = document.getElementById('map');
	// Redefinición de la medidas del mapa
	oMap.style.width= 53 * myNewSize +"px";
	oMap.style.height= 38 * myNewSize + "px";
	...
	var mapCoordsNode = "//table[@id='mapCoords']/tbody";
	var yDirection = document.evaluate(mapCoordsNode+"/tr[2]/td[1]" ,document,null,XPathResult.STRING_TYPE,null).stringValue - document.evaluate(mapCoordsNode+"/tr[1]/td[1]" ,document,null,XPathResult.STRING_TYPE,null).stringValue;

	for (var k=0; k < InputCount; k++){
		if (document.getElementsByTagName("input")[k].name == 'x'){
		vCoords['x'] = document.getElementsByTagName("input")[k].value - (myNewSize/2);
		};
		if (document.getElementsByTagName("input")[k].name == 'y'){
		vCoords['y'] =  eval(document.getElementsByTagName("input")[k].value)-(myNewSize/2);
		};
	}

	var oMapNew = document.getElementById('mapNew');

// InnerHTML
	var ihtml = "";
	ihtml += '<tr>';
	ihtml += '<td height="38">' + (vCoords['y'] + (1- yDirection) * myNewSize/2 + (1 + yDirection)/2 - 1)  + '</td>';
	ihtml += '<td colspan="' + myNewSize + '" rowspan="' + myNewSize + '">';
	ihtml += '<div style="background-image:url(graphic/map/gras4.png); position:relative; width:' + (53 * myNewSize) + 'px; height:' + (38 * myNewSize) +'px; overflow:hidden" id="map">';
	ihtml += '<div id="mapOld" style="position:absolute; left:0px; top:0px">';
	ihtml += '<div style="color:white; margin:10px">'+oMapNew.innerHTML+'</div>';
	ihtml += '</div>';
	ihtml += '<div id="mapNew" style="position:absolute; left:0px; top:0px"></div>';
	ihtml += '</div>';
	ihtml += '</td>';
	ihtml += '</tr>';
	for(jj=1; jj<myNewSize; jj++){
		ihtml += '<tr><td width="20" height="38">' + (vCoords['y']  + (1- yDirection) * myNewSize/2 + (1 + yDirection)/2 + yDirection*jj -1) + '</td></tr>';
	}
	ihtml += '<tr id="map_x_axis">';
	ihtml += '<td height="20"></td>';
	for(jj=0; jj<myNewSize; jj++){
		ihtml += '<td align="center" width="53">' + (vCoords['x'] + jj) + '</td>';
	}
	ihtml += '</tr>';
	document.getElementById("mapCoords").innerHTML = ihtml;
	
	//Topo
	var tRect = document.getElementById('topoRect');
	tRect.style.width = 5 * myNewSize + 'px';
	tRect.style.height = 5 * myNewSize + 'px';
	
	var tRectTop = tRect.style.top;
	tRectTop = tRectTop.substring(0, tRectTop.length - 2 );
	tRectTop = parseInt(tRectTop) - (2.5 * myNewSize - 19) ;
	
	var tRectLeft = tRect.style.left;
	tRectLeft = tRectLeft.substring(0, tRectLeft.length -2);
	tRectLeft = parseInt(tRectLeft) - (2.5 * myNewSize - 15) ;
	...
	oMiniMap.singleNodeValue.innerHTML  = '<div style="position:relative; left:0px; top:'+(parseInt (oMap.style.height) /2)+'px ; Background-color:#DED3B9;color:#804000">'+oMiniMap.singleNodeValue.innerHTML +'</div>';
	

	GM_xmlhttpRequest({
			method: 'GET',
			url: twLinks["twMainMenuArray"]["map"] +'&xml&start_x='+(parseInt(vCoords['x']))+'&start_y='+(parseInt(vCoords['y']))+'&size_x='+myNewSize+'&size_y='+myNewSize,
			headers: {
			...
			onload: function(responseDetails) {		
					document.getElementById("mapNew").innerHTML = responseDetails.responseText;
	...
	newJS  = 'ajaxMapInit('+(vCoords['x'])+', '+(vCoords['y'])+','+myNewSize+', "'+twLinks["twMainMenuArray"]["map"]+'&xml",'+ yDirection+', 1);';
	addGlobalJS(newJS);

// ???
	/*newJS  = 'ajaxMapInit('+(3)+', '+(1)+', 14, "'+twLinks["Mapa"]+'&xml", -1, 1);';
  addGlobalJS(newJS);
  document.getElementById('mapOld').innerHTML*/
}
//__________________________________________________________________________________________________________________//
...
{
		var i = 2;
		var path = '/html/body/table[3]/tbody/tr/td/table/tbody/tr/td[2]/table/tbody/tr/td/table[1]/tbody/';
		//tr[2]/td[1]
		// /html/body/table[3]/tbody/tr/td/table/tbody/tr/td[2]/table/tbody/tr/td/table[1]/tbody   /tr[3]/td/table[3]/tbody/tr[1]/th[1]
		// /html/body/table[3]/tbody/tr/td/table/tbody/tr/td[2]/table/tbody/tr/td/table[1]/tbody   /tr[3]/td/table[4]/tbody/tr[1]/th[1]
		// Comprobación: Informe reenviado (init = 5) o Informe propio (init = 3)
		var init = 3;
		...
		// Pérdidas del Atacante en la Batalla
		...
		// Pérdidas del Defensor en la Batalla
		...
		// Información de Botín, espionaje y daños.
		var k = 0;
		var i = 1;
		var j = 0;
		...
		while(str != ""){
			...
			while(j < twLanguage["reportKeys"].length){
				if(str.search(/twLanguage["reportKeys"][j]/i) != -1){
					if(i == 1)
...
function twReportGenerator(reportInfo)
{
		...
		newReport[0] = '[b]Batalla del d&iacute;a '+reportInfo['date'].substring(0,8)+' a las '+reportInfo['date'].substring(14,8)+'[/b]\n';
		// Vencedor de la Batalla
		if(attackerLossesUnits == attackerTotalUnits)
			newReport[0] +='La batalla ha sido ganada por el defensor ('+reportInfo['defender']['player']+').';
		else
			newReport[0] +='La batalla ha sido ganada por el atacante ('+reportInfo['attacker']['player']+').';

		
		newReport[1] = '\n\n[i]Suerte:[color=red] '+reportInfo['luck']+' [/color][/i]\n';
		newReport[2] = '[i]Moral:[color=blue] '+reportInfo['morale']+' [/color][/i]\n\n';
		

		// Datos del Atacante
		newReport[3] = '[b]Atacante: [color=purple][size=15] '+reportInfo['attacker']['player']+' [/size][/color][/b]\n'; 
		newReport[4] = '[b]Pueblo: [color=navy][size=13] '+reportInfo['attacker']['village']['name']+' ['+reportInfo['attacker']['village']['x']+'|'+reportInfo['attacker']['village']['y']+'] [/size][/color][/b]\n';
	
		// Tropas del Atacante
		newReport[5] = '';
		for(i=0;i<9;i++){
			if(reportInfo['attacker']['army'][twUnitsNamesArray[i]]['total'] > 0	)
					newReport[5] += '[img]'+twLinks["twUnitsImg"][i]+'[/img]'+twLanguage['units'][i]+'[b] '+reportInfo['attacker']['army'][twUnitsNamesArray[i]]['total']+' [/b].Perdi&oacute;: [color=darkred] '+reportInfo['attacker']['army'][twUnitsNamesArray[i]]['losses']+' [/color]([color=green]'+parseInt( parseInt(reportInfo['attacker']['army'][twUnitsNamesArray[i]]['total'])-parseInt(reportInfo['attacker']['army'][twUnitsNamesArray[i]]['losses']))+'[/color])\n';
		}
		newReport[5] += '__________________________________________________________________________'
		newReport[5] += '\n[i][b]P&eacute;rdidas del Atacante:[/b]'; 
		newReport[5] += '\n&nbsp; '+twLanguage["mainMenuTitle"][0]+': [color=darkred]'+attackerLossesUnits+'[/color]/[b]'+attackerTotalUnits+'[/b] ([color=green]'+(attackerTotalUnits-attackerLossesUnits)+'[/color]) Unidad(es) ([b][color=blue]'+(Math.round((attackerLossesUnits*100/attackerTotalUnits)*10)/10)+'%[/color][/b]) Unidad(es)'
		newReport[5] += '\n&nbsp; '+twLanguage["mainMenuTitle"][1]+': [img]'+twLinks["twOtherImg"][0]+'[/img]'+reportInfo['attacker']['lossesResources']['wood']+'[img]'+twLinks["twOtherImg"][1]+'[/img]'+reportInfo['attacker']['lossesResources']['stone']+'[img]'+twLinks["twOtherImg"][2]+'[/img]'+reportInfo['attacker']['lossesResources']['iron'];
		newReport[5] += '&nbsp;(Total: [img]'+twLinks["twOtherImg"][3]+'[/img][b]'+(parseInt(reportInfo['attacker']['lossesResources']['wood'])+parseInt(reportInfo['attacker']['lossesResources']['stone'])+parseInt(reportInfo['attacker']['lossesResources']['iron']))+'[/b] recursos)[/i]\n';
		
		
		newReport[6] = '\n[b]Defensor: [color=purple][size=15] '+reportInfo['defender']['player']+' [/size][/color][/b]\n'; 
		newReport[7] = '[b]Pueblo: [color=navy][size=13] '+reportInfo['defender']['village']['name']+' ['+reportInfo['defender']['village']['x']+'|'+reportInfo['defender']['village']['y']+'] [/size][/color][/b]\n';
		...
		if(parseInt(defenderTotalUnits) != 0)
		{
			...
			for(i=0;i<9;i++){
				if(reportInfo['defender']['army'][twUnitsNamesArray[i]]['total'] > 0	)
						newReport[8] += '[img]'+twLinks["twUnitsImg"][i]+'[/img]'+twLanguage['units'][i]+'[b] '+reportInfo['defender']['army'][twUnitsNamesArray[i]]['total']+' [/b].Perdi&oacute;: [color=darkred] '+reportInfo['defender']['army'][twUnitsNamesArray[i]]['losses']+' [/color]([color=green]'+(parseInt(reportInfo['defender']['army'][twUnitsNamesArray[i]]['total'])-parseInt(reportInfo['defender']['army'][twUnitsNamesArray[i]]['losses']))+'[/color])\n';
			}
			newReport[8] += '__________________________________________________________________________'
			newReport[8] += '\n[i][b]P&eacute;rdidas del Defensor:[/b]';
			newReport[8] += '\n&nbsp;'+twLanguage["mainMenuTitle"][0]+': [color=darkred]'+defenderLossesUnits+'[/color]/[b]'+defenderTotalUnits+'[/b] ([color=green]'+(defenderTotalUnits-defenderLossesUnits)+'[/color]) Unidad(es) ([b][color=blue]'+(Math.round((defenderLossesUnits*100/defenderTotalUnits)*10)/10)+'%[/color][/b])';
			newReport[8] += '\n&nbsp;'+twLanguage["mainMenuTitle"][1]+': [img]'+twLinks["twOtherImg"][0]+'[/img]'+reportInfo['defender']['lossesResources']['wood']+'[img]'+twLinks["twOtherImg"][1]+'[/img]'+reportInfo['defender']['lossesResources']['stone']+'[img]'+twLinks["twOtherImg"][2]+'[/img]'+reportInfo['defender']['lossesResources']['iron'];
			newReport[8] += '&nbsp;(Total: [img]'+twLinks["twOtherImg"][3]+'[/img][b]'+(parseInt(reportInfo['defender']['lossesResources']['wood'])+parseInt(reportInfo['defender']['lossesResources']['stone'])+parseInt(reportInfo['defender']['lossesResources']['iron']))+'[/b]).[/i]\n';
			newReport[8] += '\n';
		}
		else
		{
			...
			{
				newReport[8] = '[i]Las tropas permanecen Ocultas, no has logrado verlas.[/i]';
				newReport[8] +='\n\n[i][b]Unidades Totales Defensor:[/b] [b]Perdi&oacute;[/b] [color=darkred]????[/color]/[b]????[/b] Unidad(es) ([b][color=blue]??%[/color][/b]). [b]Resto[/b]: [color=green]????[/color] Unidad(es)[/i].\n\n';
			}
			else
			{
				newReport[8] = '[i]Sin Tropas. El Pueblo est&aacute; vac&iacute;o[/i]';
				newReport[8] +='\n\n[i][b]Unidades Totales Defensor:[/b] [b]Perdi&oacute;[/b] [color=darkred]0[/color]/[b]0[/b] Unidad(es) ([b][color=blue]100%[/color][/b]). [b]Resto[/b]: [color=green]0[/color] Unidad(es) [/i].\n\n';
			}
		}
		...
		newReport[9] ='P&eacute;rdidas Totales:';
		...
		newReport[9] += ' (Total: [b]'+(parseInt(reportInfo['attacker']['lossesResources']['wood'])+parseInt(reportInfo['defender']['lossesResources']['wood'])+parseInt(reportInfo['attacker']['lossesResources']['stone'])+parseInt(reportInfo['defender']['lossesResources']['stone'])+parseInt(reportInfo['attacker']['lossesResources']['iron'])+parseInt(reportInfo['defender']['lossesResources']['iron']))+'[/b] recursos).';
...
// Summary: Genera la vista prévia del informe compactado
// Params: report[]
...
function twPreviewReportGenerator(reportWithForumTags)
{
	...
	var reportPreview = "";
	var ForumTags = new Array('[b]', '[/b]', '[i]', '[/i]', '[u]', '[/u]', '\n', '[color=darkred]', '[color=sienna]','[color=purple]','[color=navy]', '[color=green]', '[color=red]', '[color=blue]', '[/color]', '[size=13]', '[size=15]', '[/size]','[img]http://www.tribalwars.es/graphic/unit/unit_spear.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_sword.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_axe.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_spy.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_light.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_heavy.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_ram.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_catapult.png[/img]', '[img]http://www.tribalwars.es/graphic/unit/unit_snob.png[/img]','[img]http://www.tribalwars.es/graphic/holz.png[/img]', '[img]http://www.tribalwars.es/graphic/lehm.png[/img]', '[img]http://www.tribalwars.es/graphic/eisen.png[/img]', '[img]http://www.tribalwars.es/graphic/res.png[/img]');
	var HtmlTags = new Array('<b>', '</b>', '<i>', '</i>','<u>', '</u>', '<br>', '<span style="color:#C40000;">', '<span style="color: sienna;">','<span style="color: purple;">','<span style="color: navy;">','<span style="color: green;">', '<span style="color: red;">', '<span style="color: blue;">', '</span>', '<span style="font-size: 13px;">','<span style="font-size: 15px;">', '</span>','<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_spear.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_sword.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_axe.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_spy.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_light.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_heavy.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_ram.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/unit/unit_catapult.png"/>', '<img border="0" class="resizeImage" alt="" src="http://tribalwars.es/graphic/unit/unit_snob.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/holz.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/lehm.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/eisen.png"/>', '<img border="0" class="resizeImage" alt="" src="http://www.tribalwars.es/graphic/res.png"/>');
...
function twReportInterface()
{
		...
		newHTML += '<dl id="reportEffect"><dl>'
			newHTML += "<h2><b>Compactador</b></h2>"
			newHTML += "<div><a href='"+twUrlScreen+"village="+vID+presentURL+"&view="+getParam("view")+"' >Atr&aacute;s</div>"
			// opciones
			newHTML += '<dt id="twReportSettings"><a href="javascript:void(0)">Opciones</a></dt>';
			newHTML += '<dd>';
			newHTML += '		<input type="checkbox" id="coords" checked >Mostrar/Ocultar Coordenadas<br>';
			newHTML += '		<input type="checkbox" id="nombreAtc" checked >Mostrar/Ocultar Nombre Atacante<br>';
			newHTML += '		<input type="checkbox" id="nombreDef" checked >Mostrar/Ocultar Nombre Defensor<br>';
			newHTML += '		<input type="checkbox" id="ArmyAtc" checked >Mostrar/Ocultar Tropas Atacante<br>';
			newHTML += '		<input type="checkbox" id="ArmyDef" checked >Mostrar/Ocultar tropas Defensor<br><br>';
			newHTML += '		<input type="button" id="aplicaCambios" value="Aplicar Cambios" disabled ><br>';
			newHTML += '</dd>';
			
			// Informe Compactado
			newHTML += '<dt><a href="javascript:void(0)">Informe</a></dt>';
			newHTML += '<dd><textarea id="twReport"  name="twReport" cols=94 rows=15>'+reportWithForumTags+'</textarea></dd>';

			// Vista Prévia del informe
			newHTML += '<dt id="twReportPreview"><a href="javascript:void(0)">Vista pr&eacute;via</a></dt>';
...
// Summary: Barra con información sobre la tropa, recursos,granja, menú desplegable de pueblos y los enlaces
//					rápidos a la visión del pueblo y el mapa.
// Params: nothing
// Returns: nothing
...
function twSubMainMenuBar(AnchorT)
{
	var menuCount =twMenuLabel["mainMenu"].length;
		for (var i = menuCount-1; i > 0 ; i--) {
		MenuLabel = twMenuLabel["mainMenu"][i];
		if (AnchorT.target.id.toString().test(MenuLabel)) {
			var MenuPath = twLinks["twMainMenuArray"][MenuLabel];
			GM_xmlhttpRequest({ 
			method: 'GET',
			url: MenuPath ,
			headers: { 'User-agent': 'Mozilla/4.0', 'Accept': 'text/xml',},
			onload: function(responseDetails) {
				var newHTML = document.createElement("div");
				newHTML.setAttribute('id', 'GetSubMenu');
				newHTML.innerHTML = responseDetails.responseText;
				document.body.insertBefore(newHTML, document.body.childNodes[1]);	
				var subMenu = newHTML.getElementsByTagName("a");
				var subMenu2 = '';
				for (var a=0 ; a < subMenu.length; a++){
					var pmatch = subMenu[a].toString().match("(screen=(?!&)"+MenuLabel+"&mode=(?!forum)|mode=server_info)");
					if (pmatch) {
					subMenu2 = subMenu[a].parentNode.parentNode.parentNode.getElementsByTagName("td");
						if (subMenu2.length > 1){
							if (MenuLabel != "help") AnchorT.target.removeAttribute('target');
							var subMenuBar ='<ul style= "text-transform: none;">';
							for (var m = 0 ; m < subMenu2.length ; m++) {
							subMenuBar += '<li>'+subMenu2[m].innerHTML+'</li>';
							};
							subMenuBar += '</ul>';
							var div = document.createElement("div");
							div.setAttribute('id','subMenu');
							div.innerHTML = subMenuBar;
							var sp = AnchorT.target.parentNode;
							sp.appendChild(div);
						}
					break;
					}
					else {
					if (MenuLabel != "logout") AnchorT.target.setAttribute('target','_blank');
					}
...
// Summary: Permite acceder de forma fácil y rápida a todas las opciones del menú principal.
// Params: nothing
// Returns: nothing
...
var codeNavBar = '';
	var str="";
	str = document.evaluate('/html/body/table/tbody/tr/td',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
		
var subMenuHTML = document.evaluate('/html/body/table[1]/tbody/tr',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
var acount = subMenuHTML.getElementsByTagName("a").length;
var menuIndex = 0;
var subHTML ="";
for (var i=0; i < acount; i++){
subHTML = subMenuHTML.getElementsByTagName("a")[i];
	var MenuLabel = "";
	if (subHTML.toString().test(/screen=/)) {
	MenuLabel = subHTML.toString().match("(screen=)([a-z0-9]*)")[2];
	}
	if (subHTML.toString().test(/logout/)) MenuLabel = "logout" ;
	if (subHTML.toString().test(/index/)) MenuLabel = "index" ;
	if (subHTML.toString().test(/chat/)) MenuLabel = "chat" ;
	if (subHTML.toString().test(/help/)) MenuLabel = "help" ;
	if (twMenuLabel["mainMenu"].indexOf(MenuLabel) == -1) {
	twMenuLabel["mainMenu"][menuIndex] = MenuLabel;
	twMenuLine["twMainMenuArray"][MenuLabel] = '<a id="MenuLabel_'+MenuLabel+'" href="'+subHTML+'"';
	twMenuLine["twMainMenuArray"][MenuLabel] += '>'+subHTML.innerHTML+'</a>' ;
	
	}	else  {
	menuIndex-- ;
	twMenuLine["twMainMenuArray"][MenuLabel] = twMenuLine["twMainMenuArray"][MenuLabel].replace("</a>","&nbsp;" + subHTML.innerHTML+"</a>");
	}
	twLinks["twMainMenuArray"][MenuLabel] = subHTML.toString();
	twLanguage["mainMenu"][MenuLabel] = subHTML.innerHTML;

	menuIndex++ ;
 }
	
	// Barra de Menú Principal
	codeNavBar += 	'<ul class="twMenu" style="width:95px;background-image: url(\''+twGlobalInfo['InterfaceBackgroundParts'][25]+'\'); background-repeat: repeat-y; text-transform: capitalize;">';
	var menuCount = twMenuLabel["mainMenu"].length;

	for(var i = menuCount-1; i > -1 ; i--) {
	var MenuLabel = twMenuLabel["mainMenu"][i];
	codeNavBar += '<li>';
	codeNavBar += twMenuLine["twMainMenuArray"][MenuLabel] ;
	codeNavBar += '</li>';
	}
	codeNavBar += '<li><a href="#">Bursat</a>';
	codeNavBar += '<ul>';
	codeNavBar += '<li><a href="http://www.klanstats.com/tribe_details.php?World=w12&TribeID=30614" target="_blank" ><span style="top:60px;"><img src="'+twGlobalInfo["InterfaceIcons"][0]+'" />&nbsp;Burşat</span></a></li>';
	codeNavBar += '</ul>';
	codeNavBar += '</li>';
	...
	Anchors[j].addEventListener('mouseover', function(AnchorT) {
		twSubMainMenuBar(AnchorT);
		},
		true);
	}	
        if($('LoadingData')) twLoadingData(1);
...
function twRankingBar()
{
	...
	str = document.evaluate('/html/body/table[2]/tbody/tr/td',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML;
	...
	newHTML += '<div style="position:absolute;top:115px;left:205px;width:700px;">';
		...
		newHTML += '	<div style="float:left;width:141px;height:22px;margin-top:3px;background-image:url(\''+twGlobalInfo["InterfaceItems"][10]+'\');background-repeat:repeat-x"><div style="margin:3px;color:#804000;font-weight:bold">'+PosYPuntos;
		newHTML += '<span style="margin-left:15px;color:green;font-weight:bold">'+nVillages+'&nbsp;'+twLanguage["mainMenuTitle"][3]+'</span></div></div>';
		...
		newHTML += '	<div style="float:left;"><img  src="'+twGlobalInfo["InterfaceItems"][9]+'" width="30" height="28" border="0" alt=""  /></div>';
		newHTML += '	<div style="float:left;width:141px;height:22px;margin-top:3px;background-image:url(\''+twGlobalInfo["InterfaceItems"][10]+'\');background-repeat:repeat-x"><div style="margin:3px;color:#804000;font-weight:bold">. . .</div></div>';
		newHTML += '	<div style="float:left;margin-top:3px"><img  src="'+twGlobalInfo["InterfaceItems"][11]+'" width="9" height="22" border="0" alt="" /></div>';
		newHTML += '</div>';
...
function twBuildingsBar()
{
	...
	//	Enlaces
	//newHTML += '<div id="mMain" class="iconBuilding" style="width:120px;top:155px;margin-left:840px;"><a href="'+twLinks["twMainScreens"][0]+'">Edificio Principal</a></div>';
	//newHTML += '<div id="mBarracks" class="iconBuilding" style="width:55px;top:191px;margin-left:905px;" ><a href="'+twLinks["twMainScreens"][1]+'">Cuartel</a></div>';
	//newHTML += '<div id="mStable" class="iconBuilding" style="width:55px;top:229px;margin-left:905px;" id="mStable"><a href="'+twLinks["twMainScreens"][2]+'">Cuadra</a></div>';
	//newHTML += '<div id="mGarage" class="iconBuilding" style="width:50px;top:265px;margin-left:910px;"><a href="'+twLinks["twMainScreens"][3]+'">Taller</a></div>';
	...
	//newHTML += '<div id="mSmith" class="iconBuilding" style="width:65px;top:375px;margin-left:895px;"><a href="'+twLinks["twMainScreens"][5]+'">Herrer&iacute;a</a></div>';
	//newHTML += '<div id="mMarket" class="iconBuilding" style="width:130px;top:411px;margin-left:828px;"><a href="'+twLinks["twMainScreens"][7]+'">Plaza del Mercado</a></div>';
//	twLanguage["buildings"] []= new Array ("0 Edificio Principal", "1 Cuartel", "2 Cuadra", "3 Taller", "4 Corte Aristocr&aacute;tica", "5 Herrer&iacute;a", "6 Plaza de Reuniones", "7 Plaza de Mercado", "8 Los Le&ntilde;adores", "9 Barrera", "10 Mina de Hierro", "11 La Granja", "12 Almac&eacute;n", "13 El Escondrijo", "14 Muralla");
	...
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("market");
	newHTML += '	<area id="MarketIcon" title="'+twLanguage["buildings"][BuildingNumber]+'" shape="poly" alt='+twLanguage["buildings"] [BuildingNumber]+' coords="0,287, 9,281, 29,281, 33,277, 40,277, 44,281, 45,288, 41,294, 36,295, 36,303, 29,310, 6,311, 0,304" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("smith");
	newHTML += '	<area id="SmithIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,251, 9,245, 29,245, 33,241, 40,241, 44,245, 45,252, 41,258, 36,259, 36,267, 29,274, 6,275, 0,268" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("place");
	newHTML += '	<area id="PlaceIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,214, 9,207, 27,207, 33,202, 41,204, 44,208, 45,214, 42,219, 36,222, 36,230, 29,236, 8,236, 0,231" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("snob");
	newHTML += '	<area id="SnobIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,176, 9,170, 29,170, 33,166, 40,166, 44,170, 45,177, 41,183, 36,184, 36,192, 29,199, 6,200, 0,193" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("garage");
	newHTML += '	<area id="GarageIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,142, 7,135, 28,136, 32,132, 37,130, 42,133, 45,140, 42,147, 36,150, 36,158, 29,165, 7,165, 0,159" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("stable");
	newHTML += '	<area id="StableIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,105, 7,98, 29,98, 33,93, 40,93, 44,99, 45,106, 41,112, 36,113, 36,121, 29,128, 7,127, 0,122" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("barracks");
	newHTML += '	<area id="BarracksIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,67, 9,61, 29,61, 33,57, 40,57, 44,61, 45,68, 41,74, 36,75, 36,83, 29,90, 6,91, 0,84" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	BuildingNumber = twBuildingsNamesArray.ArrayIndice("main");
	newHTML += '	<area id="MainIcon" title="'+twLanguage["buildings"] [BuildingNumber]+'" shape="poly" alt="" coords="0,30, 9,24, 29,24, 33,20, 40,20, 44,24, 45,31, 41,37, 36,38, 36,46, 29,53, 6,54, 0,47" href="'+twLinks["twMainScreens"][BuildingNumber]+'" />';
	newHTML += '</map>';
...
function twSetReport()
{
	...
	// Comprobación: Informe reenviado (init = 5) o Informe propio (init = 3)
		...
		newHTML += '	<tr>';
		newHTML += '		<th>Opciones</th>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
		newHTML += '		<td><a id="twLinkReporter" href="javascript:void(0)" >&raquo; Compactar</a></td>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
		newHTML += '		<td><a id="twLinkSimu" href="'+baseURL+'/staemme.php?village='+vID+'&screen=place&mode=sim'+url+'" >&raquo; Simular</a></td>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
		newHTML += '		<td><a id="TwSaveReport" href="javascript:alert(\'Deshabilitado.\');" >&raquo; Guardar</a></td>';
		newHTML += '	</tr>';

		/*
		newHTML += '	<tr>';
		newHTML += '		<td><a href="javascript:void(0);">Atacar: </a></td>';
		newHTML += '	</tr>';
		newHTML += '	<tr>';
...
function TwSetVillageInfo()
{
	try{
		...
		infoPlayer["Jugador"] = document.evaluate('/html/body/table[3]/tbody/tr/td/table/tbody/tr[4]/td[2]/div',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML; 
		...
		infoPlayer["JugadorID"] = infoPlayer["Jugador"].substring(infoPlayer["Jugador"].indexOf("&amp;id=")+8, infoPlayer["Jugador"].indexOf("\">"));
		...
		//  /html/body/table[2]/tbody/tr/td[1]/table/tbody/tr[@id='menu_row2']/td[4]/b
//		var CoordOrigen = document.evaluate('/html/body/table[2]/tbody/tr/td/b',document,null,XPathResult.STRING_TYPE,null).stringValue; 
		var CoordOrigen = document.evaluate('/html/body/table[2]/tbody/tr/td[1]/table/tbody/tr/td[4]/b',document,null,XPathResult.STRING_TYPE,null).stringValue; 
		...
		newinfo_village += '			<tr><th>'+twLanguage["VillageInfo"]["Info"]+'</th></tr>';
		newinfo_village += '			<tr><td>'+twLanguage["VillageInfo"]["Coords"]+': <b>'+ CoordDestino +'</b></td></tr>';
		newinfo_village += '			<tr><td>'+twLanguage["VillageInfo"]["Points"]+': <b>'+ infoPlayer["Puntos"] +'</b</td></tr>';
//
		newinfo_village += '			<tr><td>'+twLanguage["VillageInfo"]["Player"]+':<div class="btnPlayerPopUp"><a id="newbtnPlayerPopUp" href="'+twUrlScreen+'village='+vID+'&screen=info_player&id='+infoPlayer["JugadorID"]+'" >'+ infoPlayer["NombreJugador"] +'</a></div></td></tr>';
// id="btnPlayerPopUp0" class="btnPlayerPopUp" 
		// 		newinfo_village += '			<tr><td>'+twLanguage["VillageInfo"]["Player"]+': <ul  class="twMenu"><li><a id="btnPlayerPopUp" href="javascript:void(0);">'+ infoPlayer["NombreJugador"] +'</a></li></ul></td></tr>';

		newinfo_village += '			<tr><td>'+twLanguage["VillageInfo"]["Tribe"]+': '+ infoPlayer["Tribu"] +'</td></tr>';
		newinfo_village += '			<tr><th>'+twLanguage["VillageInfo"]["Action"]+'</th></tr>';
		newinfo_village += '			<tr><td><a 	href= "'+twLinks["twMainMenuArray"]["map"]+'&x='+infoPlayer["CoordDestino"][0]+'&y='+infoPlayer["CoordDestino"][1]+'" >&raquo; '+twLanguage["VillageInfo"]["Map"]+'</a></td></tr>';
		...
		newinfo_village += '		<tbody>';
		newinfo_village += '			<tr><th>'+twLanguage["VillageInfo"]["Units"]+'</th><th>'+twLanguage["VillageInfo"]["Time"]+'</th></tr>';
		
		for(var i=0;i<UnitNum;i++)
			newinfo_village += '			<tr><td><a href="javascript:popup(\'popup_unit.php?unit='+twUnitsNamesArray[i]+'\', 520, 520)">'+twLanguage["units"][i]+'</a></td><td align=right>'+tiempoUnidad[i]+'&nbsp;</td></tr>';
		
		newinfo_village += '		</tbody>';
		newinfo_village += '	</table';
		newinfo_village += '	<table class="vis" style="float:left;width:350px">';
		newinfo_village += '		<tbody>';
		newinfo_village += '			<tr><th>'+twLanguage["VillageInfo"]["Details"]+'</th></tr>';
		newinfo_village += '			<tr><td><a href="javascript:alert(\'Deshabilitado.\');">&raquo; '+twLanguage["VillageInfo"]["PlayerHistory"]+'</a></td></tr>';
		newinfo_village += '			<tr><td><a href="javascript:alert(\'Deshabilitado.\');">&raquo; '+twLanguage["VillageInfo"]["TribeHistory"]+'</a></td></tr>';
		newinfo_village += '		</tbody>';
		newinfo_village += '	</table>';	
		newinfo_village += '</div>'; 
...
// Eventos
		if($('newbtnPlayerPopUp')){
			$('newbtnPlayerPopUp').addEventListener('mouseover', function(Event) {
					twPlayerPopUp(Event)
			}, true );		
		}
...
// Params: nothing
// Returns: nothing
function twPlayerPopUp(PopUpEvent)
{
	var sp = PopUpEvent.target;
	var indexID = sp.parentNode.innerHTML.toString().indexOf("&amp;id=")+8;
	var PlayerID = sp.parentNode.innerHTML.toString().substring(indexID, sp.parentNode.innerHTML.toString().indexOf('\"',indexID));
	//alert (sp.parentNode.innerHTML.toString()+"\n"+PlayerID);
	
	//if($('playerPopUp')) return;
	//alert (PopUpEvent.target.parentNode.innerHTML);
	var newInfo_village ="";
//	newInfo_village += '<ul style="width:auto; height:auto; background-color:#F8F4E8; border:1px solid #804000;padding:3px;">';
	newInfo_village += '<ul>';
//	newInfo_village += '	<table>';
//	newInfo_village += '		<tbody>';
//	newInfo_village += '			<tr id="btnPlayerPopUpClose" href="javascript:void(0);"><th>Informaci&oacute;n del Jugador</th><td><a  >X</a></td></tr>';
//	newInfo_village += '			<tr><td colspan=2 >
	newInfo_village +='<li><a href="#">'+twLanguage["VillageInfo"]["Player"]+'</a>';
//	newInfo_village +='<li><a href="'+twUrlScreen+'village='+vID+'&screen=info_player&id='+PlayerID+'">'+twLanguage["VillageInfo"]["Player"]+'</a>';
	newInfo_village +='</li>';	
//	</td></tr>';
//	newInfo_village += '			<tr><td colspan=2 >
//	newInfo_village +='<li><a id="btnPlayerToolbox" href="javascript:void(0)">Günlüğü</a>'
	newInfo_village +='<li><a id="btnPlayerToolbox" href="http://www.klanstats.com/tribe_details.php?World='+wNum+'&TribeID='+PlayerID+'\">Günlüğü</a>'
//	newInfo_village +='<li><a id="btnPlayerToolbox" href="javascript:void(0)">Günlüğü</a>'
	newInfo_village +='</li>';	
//	</td></tr>';
//	newInfo_village += '			<tr><td colspan=2 >
//http://www.klanstats.com/tribe_details.php?World=
	newInfo_village +='<li id="btnPlayerTool4TW"><a href="#" onclick="GM_openInTab(\'http://www.klanstats.com/tribe_details.php?World='+wNum+'&TribeID='+PlayerID+'\');" >Mesaj Yaz</a>';
//	newInfo_village +='<li><a id="btnPlayerTool4TW" href="http://www.klanstats.com/tribe_details.php?World='+wNum+'&TribeID='+PlayerID+'" target="_self">Mesaj Yaz</a>';
//	newInfo_village +='<li><a id="btnPlayerTool4TW" href="javascript:void(0)">Mesaj Yaz</a>';
	newInfo_village +='</li>';	
//	newInfo_village +='<li><a id="btnPlayerAllies" href="javascript:void(0)">Aliados m&aacute;s cercanos</a>'
//	newInfo_village +='</li>';	
//	</td></tr>';
//	newInfo_village += '		</tbody>';
//	newInfo_village += '	</table>';
	...
	var newHTML = document.createElement("div");
	//newHTML.setAttribute('class', 'vis');
	//newHTML.setAttribute('class', 'twMagicPopUp');
	newHTML.innerHTML = newInfo_village;
	//	document.body.insertBefore(newHTML, document.body.childNodes[1]);

	sp.appendChild(newHTML);
...
/*	if($('playerPopUp')){
		$('playerPopUp').addEvent('mouseout', function() {
				$('playerPopUp').parentNode.removeChild($('playerPopUp'));
		});		
	}
	*/
	if($('btnPlayerToolbox')){
		$('btnPlayerToolbox').addEvent('click', function() {
				//twPlayerCurriculum();
				GM_openInTab('http://www.klanstats.com/tribe_details.php?World=w'+wNum+'&TribeID='+PlayerID+'.html'); 
		});		
	}
	
	if($('btnPlayerTool4TW')){
		$('btnPlayerTool4TW').addEvent('click', function() {
				//twPlayerCurriculum();
				GM_openInTab('http://s'+wNum+'.klansavaslari.net/game.php?village=8131&screen=mail&mode=new&player='+PlayerID+'.html'); 
		});		
	}
	...
	if($('btnPlayerAllies')){
		$('btnPlayerAllies').addEvent('click', function() {
				//twPlayerInfoAllies();
				GM_openInTab('http://fr'+wNum+'.tw-tools.com/index.php?object=stat&method=ally_file&lng='+language+'&type=html&id='+PlayerID);
		});		
	}
}
//__________________________________________________________________________________________________________________//
...
function twPlayerCurriculum()
{
	...
	newInfo_village += '	<table>';
	newInfo_village += '		<tbody>';
	newInfo_village += '			<tr><th width=100%>Curr&iacute;culum del Jugador en todos los mundos</th><td><a id="btnPlayerCurriculumClose" href="javascript:void(0);" >X</a></td></tr>';
	newInfo_village += '			<tr><td><div id="twPLayerCurriculum"><div id="loadingCurriculum">Loading...</div></div></td></tr>';	
	newInfo_village += '		</tbody>';
	newInfo_village += '	</table>';
	newInfo_village += '</div>'
	...
	if(DebugMode == 1) GM_log("Loading Player Curriculum...");
	GM_xmlhttpRequest({ 
		method: 'GET',
...
function twPlayerInfoAllies()
{
	...
	newInfo_village += '	<table>';
	newInfo_village += '		<tbody>';
	newInfo_village += '			<tr><th width=100%>Aliados m&aacute;s cercanos </th><td><a id="btnPlayerCurriculumClose" href="javascript:void(0);" >X</a></td></tr>';
	newInfo_village += '			<tr><td><div id="twPLayerCurriculum"><div id="loadingCurriculum">Loading...</div></div></td></tr>';	
	newInfo_village += '		</tbody>';
	newInfo_village += '	</table>';
	newInfo_village += '</div>'
	...
	if(DebugMode == 1) GM_log("Loading Info Allies Curriculum...");
	GM_xmlhttpRequest({ 
		method: 'GET',
		...
		onload: function(responseDetails) {
			...
			var menu = '';
			menu += '<fieldset><legend>Opciones</legend>';
			...
			menu += '		<input type="button" name="getInfo" id="btnGetInfo" value="Buscar" align=right/>'
			menu += '</fieldset>'
...
{
	try{
	// Las tecnologías del defensor por defecto serán de nivel 3
	// Las tecnologías del atacante por defecto serán de nivel 1
		for(var i=0;i<UnitNum;i++){
	...
	// Se pone la moral con la que atacas según la del informe
		if(getParam('moral') != '') document.getElementById('moral').value = getParam('moral');
		
	// Si ha habido espionaje, se pone el nivel de la muralla. Por defecto será nivel 20.
	...
	//var nodePath= '/html/body/table/tbody/tr/td/table[2]/tbody/tr/td[2]/table';
	var nodePath = "/html/body/table[3]/tbody/tr/td/table/tbody/tr/td[1]/table/tbody";
	
		if(document.evaluate(nodePath,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.innerHTML != "")
		{
	...
	// Mostrar recursos perdidos		
			var NewElem;
			NewElem = document.evaluate(nodePath,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null);
...
function saveNotes() 
	{
	GM_setValue("vID_"+vID+"_notes",document.getElementById('note').value);
	document.getElementById('bn').value = "Notununuz bu köyünüzde kayıt edildi";
	return false;
	}

function addNoteHandler() {
		document.getElementById('bn').addEventListener('click',function() {saveNotes();}, true);
		}
...
function afficheNotes(){
	...
	var noteHTML=document.createElement("div");
		noteHTML.innerHTML="<b>Bu köyünüz hakkında notlarınız:</b><br /><textarea class='fm' style='width:100%;height:110px;' id='note'></textarea><br><input type='button' id='bn' value='Notunuzu kayıt etmek istiyorsanız tıklayınız.'  class=fm style='width:100%;height:25px;margin-left:auto;margin-right:auto;background:#eee;'>";
		noteHTML.style.marginLeft = "auto";
		noteHTML.style.marginRight = "auto";
		document.body.childNodes[7].childNodes[1].childNodes[0].childNodes[1].appendChild(noteHTML);
		addNoteHandler();
		document.getElementById('note').value = GM_getValue("vID_"+vID+"_notes","");
	}
}

function calculer_distances()
{
	...
	document.getElementById('temps').innerHTML='Süre : '+j+' gün, '+h+' sa,'+m+' dk,'+s+' sn';
	document.getElementById('distance').innerHTML='Uzaklık : '+(Math.round(distance*100)/100)+' alan uzaklıkta';
}
...
function distances()
{
	if(getParam('screen')=="map")
	{
		...
		d.innerHTML="<br /><table class='main' width='840' style='margin-left:96px;'><tr><td>Saldırı Köyü: </td><td>x <input type='text' size='3' id='x1' /></td><td>y <input type='text' size='3' id='y1' /></td><td rowspan='2'><select name='units' id='units0'><option value='12'>Mızrakçı</option><option value='15'>Kılıç Ustası</option><option value='12'>Baltacı</option><option value='6'>Casus</option><option value='6.5'>Hafif Atlı</option><option value='7.5'>Ağır Atlı</option><option value='20'>Şahmerdan</option><option value='24'>Mancınık</option><option value='30'>Misyoner</option><option value='6'>Tüccarlar</option></select></td><td rowspan='2' style='margin-left:50px;'><input type='button' value='Hesapla' id='button_distance' /></td><td style='width:400px' id='distance'></td></tr>"+
		"<tr><td>Hedef Köy : </td><td>x <input type='text' size='3' id='x2' /></td><td>y <input type='text' size='3' id='y2' /></td><td id='temps'aaa></td></tr></table>";
		document.body.appendChild(d);
		document.getElementById('button_distance').addEventListener("click",calculer_distances, true);
	}
}
	...
	try{
		twLanguageData();
		 if (twHidesFrame()) {
			...
			if(getParam('screen') == 'place'){
			twGetWorldArmy();
			if(getParam('screen') == 'place' && getParam('mode') == 'sim'){twSetSim();}
			}
			if(getParam('screen') == 'main'){twGetWorldBuildings();}
			...
			if(twInitWorldArmy()==false) {
			twGetWorldArmy();
			}
	
			if(twInitWorldBuildings()==false) {
			twGetWorldBuildings();
			}
			twLoadingData(0);
			setTimeout("location.reload(true)",(!twInitWorldArmy()*5000+!twInitWorldBuildings()*5000));
			}
		}
