// ==UserScript==
// @name           Pokemon Chumma Computer
// @description    Sunny Boy.
// @author         GiveMeSomeSunshine
// @include        http://tpkrpg.net/*
// @version        1
...
if(document.body.innerHTML.search(/Submit/) == -1){
  if(document.body.innerHTML.search(/You have entered the correct code/) != -1) {
    window.location = "/battle.php?Battle=Trainer&BattleID=450"
  
    }
    inputs = document.getElementsByTagName("input")
    for(i = 0; i < inputs.length; i++) {
      if(inputs[i].value == "PoisonPowder") { inputs[i].click() }
    }
    inputs = document.getElementsByTagName("input")
    for(i = 0; i < inputs.length; i++) {
      if(inputs[i].value == "Continue Battle") { inputs[i].click() }
