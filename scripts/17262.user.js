...
// ==UserScript==
// @name          BitHUmen IMDb tools
// @namespace     http://userscripts.org/users/40035/scripts
// @description   IMDb Tools for bH, nCore, DS v0.4.3, by pzs
...
// @include       http://darkside.dyn-o-saur.com:1024/torrents.php*
// @include       http://darkside.dyn-o-saur.com:1024/torrentsearch.php*
// ==/UserScript==


unsafeWindow.imdbTools = {

	version: 'v0.4.3',

	x: null,
	i: 0,
	genre_betoltve: false,
	a1: 0, // összes imdb oldal
	a2: 0, // betöltött imdb oldal
	a3: 0, // összes betöltött műfaj
	oldal: '',
	t: [], // torrentek, amiknek be kell tölteni az imdb pontszámát, t[i]: imdbID
	genre_link_style:'',
	...
	opt: {		bh: {			menu_left: '24px',
		...
		evmax: 0,
		evtorol: 0,
		savefeltetel: true // lapozásnál feltételek megjegyzése
	},
	...
	init: function(){		x=unsafeWindow.imdbTools;
		...
		if(window.location.href.match(/bithumen/)) x.oldal='bh';
		if(window.location.href.match(/ncore/)) x.oldal='ncore';
		if(window.location.href.match(/darkside/)) x.oldal='ds';

		switch(x.oldal){			case 'bh': break;
			case 'ncore': x.genre_link_style='class="infolink"'; break;
			case 'ds': break;
		}
		...
		setTimeout(function(){
    	x.opt.evmin=GM_getValue('evmin', x.opt.evmin);
    	x.opt.evmax=GM_getValue('evmax', x.opt.evmax);
    	x.opt.evtorol=GM_getValue('evtorol', x.opt.evtorol);
    	x.opt.savefeltetel=GM_getValue('savefeltetel', x.opt.savefeltetel);
    }, 0);

    // location.hash betöltése
    var h=location.hash;
    if(h.match(/#imdbtools\|([^\|]*)\|([^\|]*)\|?([^\|]*)/)){    	var e=RegExp.$1;
    	var m=RegExp.$2;
    	var is=RegExp.$3;
    	if(e!=""){    		x.szukites.evmin=e.match(/^([0-9]+),([0-9]+),(true|false)$/)[1];
    		x.szukites.evmax=e.match(/^([0-9]+),([0-9]+),(true|false)$/)[2];
    		x.szukites.evtorol=e.match(/^([0-9]+),([0-9]+),(true|false)$/)[3]=="true";
    		x.szukit_ev(true);
    	}
    	if(m!=""){    		x.szukites.mufaj=m.split(',');
    	}
    	if(is=="imdbszerint") x.szukites.imdbszerint=true;
    	x.setLink();
    }

    if(x.szukites.imdbszerint) x.rendez();

		x.getValue('getgenre_autostart_'+x.oldal, 'igen', x.imdb_getgenre_autostart, {});
	},

	imdb_getgenre_autostart: function(obj){
		if(obj.value=='igen' || x.szukites.mufaj.length!=0){
			x.imdb_getgenre();
		}
	},

	fejlec: function(){		var $a=$('body');
		switch(x.oldal){			case 'bh': $a=$('#torrenttable tr:first td:eq(1)'); break;
			case 'ncore': $a=$('div.fejlecbox_szoveg:contains(Böngész)'); $a.parent().parent().attr('id', 'imdbT_torrenttable'); break;
			case 'ds': $a=$('#torrenttable tr:first th:eq(2)'); $a.find('a:first').text('Név'); break;
		}

		$a.append(' <a href="#" class="iwiwtools_menu" onclick="return false;">[IMDb Tools]</a>');
		$a.append('<div class="iwiwtools_menu" style="font-weight:normal;display:none;position:absolute;border:1px dotted #000;padding:6px;background:'+x.opt[x.oldal].menu_bg+';color:'+x.opt[x.oldal].menu_color+';z-index:10;margin-left:'+x.opt[x.oldal].menu_left+'">'
		  +'<a style="font-weight:bold" href="#" onclick="this.blur();imdbTools.rendez();return false;" title="Az oldalon lévő torrentek rendezése IMDb pontszám alapján">&raquo; IMDb szerint rendez</a><br/><br/>'
		  +'<a style="font-weight:bold" href="#" onclick="this.blur();imdbTools.evrendez();return false;" title="Az oldalon lévő torrentek rendezése évszám szerint">&raquo; Évszám szerint rendez</a><br/><br/>'
			+'<a style="font-weight:bold" href="#" onclick="this.blur();imdbTools.szukit_ev();return false;">&raquo; Szűrés évszámra:</a><br/>'
			+'min.: <input name="it_evmin" type="text" value="" style="width:40px;"/> - max.: <input name="it_evmax" type="text" value="" style="width:40px;"/><br/>'
			+'<small>(ha 0-át adsz meg, nincs korlát)</small><br/>'
			+'<label><input name="it_evtorol" type="checkbox" value="ON"/> évszám nélküli torrentek törlése</label><br/><br/>'
			+'<a style="font-weight:bold" href="#" title="Összes torrent megjelenítése" onclick="this.blur();imdbTools.szukit_torol();return false;">&raquo; Szűrés törlése</a><br/><br/>'
		  +'<a style="font-weight:bold" href="#" onclick="this.blur();imdbTools.imdb_getgenre_setautostart();return false;" title="Műfajok automatikus betöltésének be/kikapcsolása ezen az oldalon">&raquo; Automatikus műfajbetöltés be/ki</a><br/><br/>'
		  +'<a style="font-weight:bold" href="#" onclick="this.blur();imdbTools.set_savefeltetel();return false;" title="Lapozásnál a feltételek (évszám, műfaj szűkítés és imdb szerinti rendezés) megjegyzése">&raquo; Feltételek megjegyzése lapozásnál be/ki</a><br/>'
			+'</div>');
		$a.append(' <a id="imdbTools_getg" href="#" onclick="this.blur();imdbTools.imdb_getgenre();return false;" title="Minden filmnél betölti az IMDb.com-ról a műfajt (ez akár 1 percig is eltarthat!)">[műfajok betöltése]</a>');
		$a.append(' <a href="http://userscripts.org/scripts/show/17262" target="_blank" title="IMDb Tools for bH, nCore, DS '+x.version+'">[?]</a>');
		$('.iwiwtools_menu').hover(function(){			$('div.iwiwtools_menu').stop().css('opacity', '1').show();		}, function(){			$('div.iwiwtools_menu').fadeOut('fast');		});
		setTimeout(function(){			$('.iwiwtools_menu input[name=it_evmin]').val( x.opt.evmin );
			$('.iwiwtools_menu input[name=it_evmax]').val( x.opt.evmax );
			if(x.opt.evtorol==1) $('.iwiwtools_menu input[name=it_evtorol]').attr('checked', 'checked');		}, 100);

	},

	// IMDB_GETGENRE
	imdb_getgenre: function(){
		if(x.genre_betoltve){			alert('A műfajok már betöltve, vagy betöltés alatt vannak!');
			return;
		}

		$('#imdbTools_getg').html('[műfajok betöltése...]');
		var b=false; // van e torrent, amit be kell e tölteni		x.genre_betoltve=true;
		x.a1=0;
		x.a2=0;
		x.a3=0;
		var j=0;

		// kikeressük az imdb linkeket és készítünk a műfajoknak egy SPAN-t (id: imdbTools_g_j....)
		switch(x.oldal){			case 'bh':
				$('#torrenttable tr:gt(0) a[href*=imdb.com]').each(function(){
					if(!$(this).attr('href').match(/\/(tt[0-9]+)/)) return;					var s=$(this).attr('href').match(/\/(tt[0-9]+)/)[1];
					$(this).after(' <span id="imdbTools_g_'+j+'"></span>');
					x.t[j++]=s;
				});
				break;

			case 'ncore':
				$('#imdbT_torrenttable div.box_nagy_alap a[href*=imdb.com]').each(function(){
					$(this).parent().css({left:'10px', position:'absolute', overflow:'hidden', width:'560px', height:'18px'});
					if(!$(this).attr('href').match(/\/(tt[0-9]+)/)) return;
					var s=$(this).attr('href').match(/\/(tt[0-9]+)/)[1];
					$(this).after(' <span id="imdbTools_g_'+j+'"></span>');
					x.t[j++]=s;
			...
			case 'ds':
				$('#torrenttable tr:gt(0) a[href*=imdb.com]').each(function(){					if(!$(this).attr('href').match(/\/(tt[0-9]+)/)) return;
					var s=$(this).attr('href').match(/\/(tt[0-9]+)/)[1];
					$(this).parent().parent().find('p.small').append(' <span id="imdbTools_g_'+j+'"></span>');
					x.t[j++]=s;
				});
				break;
		}

		if(x.t.length==0){			$('#imdbTools_getg').html('[műfajok betöltve]');
			return;
		}

		for(var j=0; j<x.t.length; j++){			x.getValue('imdb_'+x.t[j], '', x.imdb_getgenre_s2, {a: j} );
		}

	},

  // IMDB_GETGENRE_S2
	imdb_getgenre_s2: function(obj){
		if(obj.value!=''){ // ha megvannak a műfajok, kiírjuk
			var t=obj.value.split('|');
			for(var j=0;j<t.length;j++)
				t[j]='<a href="#" onclick="imdbTools.szukit(\''+t[j]+'\');return false;" '+x.genre_link_style+' title="Csak azokat a torrenteket mutasd, melynek műfaja \''+t[j]+'\'">'+t[j]+'</a>';
			$('#imdbTools_g_'+obj.a).html('('+t.join(', ')+')');
			x.a3++;
		}else{ // ha nincsenek meg a műfajok, letöltjük
			x.a1++;
			x.xmlhttpRequest({
				method: 'GET',
				url: 'http://www.imdb.com/title/'+ x.t[obj.a] +'/',
		 		onload: function(data) {	x.imdboldal_feldolgoz(data.responseText, obj.a);	}
			});
		}
		x.getgenre_showstat();
	},
	...
	// IMDBOLDAL_FELDOLGOZ
	imdboldal_feldolgoz: function(s, a){		x.a2++;
		x.getgenre_showstat();
		var k=s.indexOf('<h5>Genre:</h5>');
		if(k==-1){			x.a3++;
			x.getgenre_showstat();
			return false;
		}
		s=s.substr(k + 15);
		s=s.substr(0, s.indexOf('</div>')).split('</a>');
		var t=[];
		for(var i=0;i<s.length;i++){			var m=s[i].match(/href="[^"]*Genres\/([^\/]+)/);
			if(!m) continue;
			t.push(m[1]);
		}
		var ki=t.join('|');
		// mentés böngészőbe:
		x.setValue('imdb_'+x.t[a], ki);
		if(ki!="") x.imdb_getgenre_s2({value: ki, a: a});
	},

	// GETGENRE_SHOWSTAT
	getgenre_showstat: function(){		if(x.a1==0 || x.a1==x.a2) $('#imdbTools_getg').html('[műfajok betöltve]'); else $('#imdbTools_getg').html('[ '+Math.round( x.a2/x.a1*100 )+'% ]');
		if(x.t.length==x.a3){			$(x.szukites.mufaj).each(function(){x.szukit(this, true);});
		}
	},

  // SZÜKÍTÉS
	szukit: function(s, onload){		//alert(s);
		if(onload!=true){
			x.szukites.mufaj.push(s);
			x.setLink();
		}
		switch(x.oldal){			case 'bh':
				$('#torrenttable tr:gt(0)').each(function(){
					var s0=$(this).find('span[id^=imdbTools_g_]').text();
					if(s0.indexOf(s)==-1) $(this).hide();
				});
				break;

			case 'ncore':
				$('#imdbT_torrenttable div.box_nagy_alap').each(function(){					if($(this).find('span[id^=imdbTools_g_]').text().indexOf(s)==-1){						$(this).prev().hide();
						$(this).next().hide();
						$(this).hide();
					}				});
				break;
	...
	// SZÜKITÉS ÉVSZÁMRA
	szukit_ev: function(onload){		if(onload==true){			var evmin=x.szukites.evmin;
			var evmax=x.szukites.evmax;
			var evtorol=x.szukites.evtorol;
		}else{
			x.opt.evmin=parseInt($('.iwiwtools_menu input[name=it_evmin]').val());
			x.opt.evmax=parseInt($('.iwiwtools_menu input[name=it_evmax]').val());
			if($('.iwiwtools_menu input[name=it_evtorol]').get(0).checked) x.opt.evtorol=1; else x.opt.evtorol=0;
	    setTimeout(function(){	    	GM_setValue('evmin', x.opt.evmin);
	    	GM_setValue('evmax', x.opt.evmax);
	    	GM_setValue('evtorol', x.opt.evtorol);	    }, 0);
			var evmin=x.opt.evmin;
			var evmax=x.opt.evmax;
			var evtorol=(x.opt.evtorol==1);
			x.szukites.evmin=evmin;
			x.szukites.evmax=evmax;
			x.szukites.evtorol=evtorol;
			x.setLink();
		}
		switch(x.oldal){
			case 'bh':
				$('#torrenttable tr:gt(0)').each(function(){
					var s0=$(this).find('a[href^=details.php]').text();
					if(s0.match(/[^0-9]([0-9]{4})[^0-9]/)){						var ev=parseInt(RegExp.$1);
						if(evmin!=0&&ev<evmin || evmax!=0&&ev>evmax) $(this).hide();
					}else if(evtorol) $(this).hide();
				});
				break;

			case 'ncore':
				$('#imdbT_torrenttable div.box_nagy_alap').each(function(){
					var s0=$(this).find('a[href^="javascript:torrent"], a[href*="torrent_adatok.php"]').attr('title');
					if(s0.match(/[^0-9]([0-9]{4})[^0-9]/)){
						var ev=parseInt(RegExp.$1);
						if(evmin!=0&&ev<evmin || evmax!=0&&ev>evmax) $(this).hide().prev().hide().next().next().hide();
					}else if(evtorol) $(this).hide().prev().hide().next().next().hide();
			...
			case 'ds':
				...
				break;
		}		$('div.iwiwtools_menu').hide();
	},

	// SZÜKÍTÉS TÖRLÉSE
	szukit_torol: function(s){		x.szukites.evmin=null;
		x.szukites.mufaj=[];
		x.szukites.imdbszerint=false;
		x.setLink();
		switch(x.oldal){
			case 'bh':
				$('#torrenttable tr:gt(0)').each(function(){
	...
	// linkeket állítja be x.szukites alapján
	setLink: function(){
		if(!x.opt.savefeltetel) return;		// #imdbtools|evmin,evmax,evtorol|mufaj1,mufaj2|imdbszerint
		// #imdbtools||mufaj1,mufaj2
		// #imdbtools|evmin,evmax,evtorol|
		// #imdbtools|||imdbszerint
		var s="#imdbtools|";
		if(x.szukites.evmin!=null){			s+=x.szukites.evmin+','+x.szukites.evmax+','+x.szukites.evtorol;
		}
		s+='|';
		if(x.szukites.mufaj.length!=0){			s+=x.szukites.mufaj.join(',');
		}
		s+='|';
		if(x.szukites.imdbszerint) s+='imdbszerint';
		location.hash=s;
		switch(x.oldal){
			case 'bh':
				$links=$('a[href*=browse.php][href*=page]');
				break;
			case 'ncore':
				$links=$('a[href*=letoltes.php][href*=oldal]');
				break;
			case 'ds':
				$links=$('a[href*=torrents.php][href*=page], a[href*=torrentsearch.php][href*=page]');
				break;
		}
		$links.each(function(){
				if($(this).attr('href').indexOf('#')>=0){
					this.href=this.href.match(/^(.*)#/)[1] + s;
				}else{
					this.href+=s;
				}
		});
	...
	rendez: function(){		x.szukites.imdbszerint=true;
		x.setLink();
  	switch(x.oldal){  		case 'bh': x.bh_rendez(); break;
  		case 'ncore': x.ncore_rendez(); break;
  		case 'ds': x.ds_rendez(); break;
  	}
  	$('div.iwiwtools_menu').hide();
	},
	...
	bh_rendez: function(){		var t1=[]; // sor
		...
		$('#torrenttable tr:gt(0)').each(function(){
			t1[i]=$(this);
			t2[i]=0;
			t2[i]=parseFloat($(this).find('a[href*=imdb.com]').text().substr(7,3));
			if(isNaN(t2[i])) t2[i]=-0.5;
			i++;
		});
		var max=0;
		var maxi=0;
		while(max!=-1){
			max=-2;
			for(i=0;i<t2.length;i++) if(t2[i]>max) {maxi=i; max=t2[maxi];}
			if(max!=-1) t1[maxi].appendTo('#torrenttable');
			t2[maxi]=-1;
		}
	},

	ncore_rendez: function(){
		var t1a=[]; // sor
		var t1b=[]; // sor
		var t1c=[]; // sor
		var t2=[]; // imdb pont
		...
		while(max!=-1){
			max=-2;
			for(i=0;i<t2.length;i++) if(t2[i]>max) {maxi=i; max=t2[maxi];}
			if(max!=-1){				t1a[maxi].appendTo('#imdbT_torrenttable > div:last');
				t1b[maxi].appendTo('#imdbT_torrenttable > div:last');
				t1c[maxi].appendTo('#imdbT_torrenttable > div:last');
			}
			t2[maxi]=-1;
		}
	},
	...
	ds_rendez: function(){
		...
		$('#torrenttable tbody > tr').each(function(){
			t1[i]=$(this);
			t2[i]=0;
			t2[i]=parseFloat($(this).find('a[href*=imdb.com]').text().substr(6,3));
			if(isNaN(t2[i])) t2[i]=-0.5;
			i++;
		});
		var max=0;
	...
	evrendez: function(){
  	switch(x.oldal){
  		case 'bh': x.bh_evrendez(); break;
  		case 'ncore': x.ncore_evrendez(); break;
  		case 'ds': x.ds_evrendez(); break;
  	}
  	$('div.iwiwtools_menu').hide();
	},

	bh_evrendez: function(){
		var t1=[]; // sor
		var t2=[]; // imdb pont
		var i=0;
		$('#torrenttable tr:gt(0)').each(function(){
			t1[i]=$(this);
			t2[i]=0;
			if( $(this).find('a[href^=details.php]').text().match(/[^0-9]([0-9]{4})[^0-9]/) ){				t2[i]=parseInt(RegExp.$1);
			}else{
				t2[i]=0;
			}
			i++;
	...
	ncore_evrendez: function(){
		...
		$('#imdbT_torrenttable div.box_nagy_alap').each(function(){
			...
			t1c[i]=$(this).next();
			t2[i]=0;
			if( $(this).find('a[href^="javascript:torrent"], a[href*="torrent_adatok.php"]').attr('title').match(/[^0-9]([0-9]{4})[^0-9]/) ){
				t2[i]=parseInt(RegExp.$1);
			}else{
				t2[i]=0;
			}
	...
	// AUTOMATIKUS BETÖLTŐDÉS BE/KIKAPCSOLÁSA
	imdb_getgenre_setautostart: function(){		if(confirm('Be akarod kapcsolni az automatikus műfajbetöltést?')){			x.setValue('getgenre_autostart_'+x.oldal, 'igen');
			alert('Automatikus műfajbetöltés bekapcsolva.');
		}else{			x.setValue('getgenre_autostart_'+x.oldal, 'nem');
			alert('Automatikus műfajbetöltés kikapcsolva.');
		}
	},

  set_savefeltetel: function(){		if(confirm('Lapozásnál feltételeket jegyezze meg a szkript (évszám, műfaj szűkítés)?')){
			x.setValue('savefeltetel', true);
			x.opt.savefeltetel=true;
			alert('Feltételek megjegyzése bekapcsolva.');
		}else{
			x.szukites.evmin=null;
			x.szukites.mufaj=[];
			x.setLink();
			x.opt.savefeltetel=false;
			x.setValue('savefeltetel', false);
			alert('Feltételek megjegyzése kikapcsolva.');
		}
  },

	// müködéshez szükséges függvények:
	setValue: function(nev, ertek){		setTimeout(function() {
			GM_setValue(nev, ertek);
		}, 0);
	},
	...
	getValue: function(nev, ertek, fgv, obj){
		setTimeout(function() {
			obj.value=GM_getValue(nev, ertek);
			fgv(obj);
		}, 0);
	},
	log: function(s){		window.setTimeout(function(){			GM_log(s);
