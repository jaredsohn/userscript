// ==UserScript==
// @name           Quick Quote (Chrome)
// @namespace      http://userscript.org
// @description    Quick Quote (Chrome)
...
if (document.URL.match(/viewconv/)) {
	...
	for (var e = 0; e < links.length; e++) {
		if (links[e].innerHTML == '[Quote]') {
			links[e].setAttribute('quoteID', links[e].parentNode.getElementsByClassName('post_id')[0].innerHTML.substr(1));
			links[e].setAttribute('userName', links[e].parentNode.getElementsByTagName('a')[1].innerHTML);
			links[e].addEventListener('click', copySel, false);
			links[e].innerHTML = '[Quick Quote]';
			links[e].removeAttribute('onclick');
			links[e].className = 'newQuote';
		}
	}
}