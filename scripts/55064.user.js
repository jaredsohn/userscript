// ==UserScript==
	...
	// @download       http://userscripts.org/scripts/source/55064.user.js
	// @updateURL      http://userscripts.org/scripts/source/55064.meta.js
	// @include        http://*megaupload.com/
	// @include        http://*megaupload.com/?
	// @include        http://*megaupload.com/?d=*
	// @include        http://*cum.com/
	// @include        http://*cum.com/?
...
if (getElementsByClassName('start_flash_block') != '') document.getElementsByClassName('start_flash_block')[0].style.display = 'none';
...
//	var MUP_nb_dl = 0;
//	var	MUP_url_nb_dl = window.location.href.split("?")[0]+'mgr_bdr.php';
	if (document.getElementById('dlbutton'))
	{
		...
		if (getElementsByClassName('download_banners_bl') != '')
			document.getElementsByClassName('download_banners_bl')[0].style.display = 'none';
		if (getElementsByClassName('download_top_text') != '')
			document.getElementsByClassName('download_top_text')[0].style.display = 'none';
		if (getElementsByClassName('download_table') != '')
			document.getElementsByClassName('download_table')[0].style.display = 'none';
		if (getElementsByClassName('main_shadow3') != '')
			document.getElementsByClassName('main_shadow3')[0].style.display = 'none';
		if (getElementsByClassName('bottom_bl') != '')
			document.getElementsByClassName('bottom_bl')[0].style.display = 'none';
		MUP_downloadlink = true;

		//alert(parseInt(document.getElementById('countertime').innerHTML.split(':')[2],10));
		if (document.getElementById('countertime'))
			Counter_MUP = parseInt(document.getElementById('countertime').innerHTML.split(':')[2],10)+1;

		document.getElementById('dlbutton').removeAttribute('onclick');
		MUP_url_movie = document.getElementById('dlbutton').href;
...
//		MUP_url_movie = window.location.href.split("?")[0]+'mgr_dl.php?'+window.location.href.split("?")[1];
//		GM_xmlhttpRequest({
//			method: 'POST',
//			url: MUP_url_nb_dl,
...
//				try
//				{
//					MUP_nb_dl = parseInt(responseDetails.responseText[0]);
//				}
//				catch(Err) {}
//			}
//		});
...
//		}
	...
	else if (document.getElementById('downloadlink'))
	{
		MUP_downloadlink = true;
		if (Counter_MUP > 0)
			Counter_MUP = parseInt(document.getElementById('countdown').innerHTML)+2;
	...
	var MUP_load = false;
	if (extension!='' &&  (extension == 'avi' || extension == 'divx' || extension == 'mp4' || extension == 'mkv'))
	{
		MUP_stream = true;
	...
	var nb_balise_strong = document.getElementsByTagName('strong').length;
	...
	if (MUP_stream)
	{
		if (window.location.href.match('megaupload'))
			MUP_taille_fic = document.getElementsByClassName('download_file_size')[0].innerHTML;
		else
		{
			for (i=0; i<nb_balise_strong ; i++)
			{
				if (document.getElementsByTagName('strong')[i].innerHTML=='Taille du fichier:' || document.getElementsByTagName('strong')[i].innerHTML=='File size:')
				{
					MUP_taille_fic = document.getElementsByTagName('strong')[i].nextSibling.nodeValue;
					break;
				}
			}
		}
	...
	if (LireCookie('l') == 'en')									MUP_LANG = 'en';
	else if (MUP_LANG!='fr' && MUP_LANG!='en')						MUP_LANG = 'en';
	...
	switch ( MUP_LANG )
	{
		...
		MUP_LNG =
		{
			...
			wait_sec:			'secondes',
			//nb_dl:				MUP_nb_dl+(MUP_nb_dl > 1 ? ' téléchargements restants':' téléchargement restant')+(MUP_nb_dl == 0 ? ' 1-3 liens directs par IP toutes les 24 heures':''),
		...
		MUP_LNG =
		{
			...
			wait_sec:			'seconds',
			//nb_dl:				MUP_nb_dl+' remaining download'+(MUP_nb_dl > 1 ? 's':'')+(MUP_nb_dl == 0 ? ' 1-3 free direct links per IP every 24 hour':''),
	...
	if (megavideo_url != '')
	{
		megavideo_fic = "\n"+'<div style="color:'+GM_getValue('MUP_color_text')+';font-variant:small-caps;text-align:center;margin:auto;float:left;width:33%">';
			megavideo_fic += "\n"+'<a style="color:'+GM_getValue('MUP_color_href')+';" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" href="'+megavideo_url+'" target="_blank" style="font-size:12px; color:#000000;">'+MUP_LNG.link_mv_01+'</a>';
			megavideo_fic += "\n"+'<br /><img style="vertical-align:center;width:150px;height:20px" src="http://wwwstatic.megavideo.com/images/logo.gif" alt="" /><br />';
			megavideo_fic += "\n"+'<a style="color:'+GM_getValue('MUP_color_href')+';" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" href="#" onclick="alert(\''+MUP_LNG.error_alert+'\');" style="font-size:12px; color:#000000;">'+MUP_LNG.link_mv_02+'</a>';
		megavideo_fic += "\n"+'</div>';
	}
	...
	megaupload_footer = "\n"+'<div style="z-index:1000;color:'+GM_getValue('MUP_color_text')+';background-color:'+GM_getValue('MUP_color_bg')+';font-variant:small-caps;text-align:center;width:'+(MUP_Width-20)+'px;min-width:650px;margin:auto;border:4px groove;padding:5px">';
		megaupload_footer += megavideo_fic;
		megaupload_footer += "\n"+'<div id="donate" style="height:48px;float:left;width:'+(megavideo_url!=''?'33%':'50%')+';">';
//			megaupload_footer += "\n"+'<a style="color:'+GM_getValue('MUP_color_href')+'" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=discussions%40hotmail%2efr&lc=FR&item_name=MUP%20userscripts&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_SM%2egif%3aNonHosted" title="'+MUP_LNG.donate_title+'" target="_blank">';
			megaupload_footer += "\n"+'<form style="color:'+GM_getValue('MUP_color_href')+'" title="'+MUP_LNG.donate_title+'" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" target="_blank" action="https://www.paypal.com/cgi-bin/webscr" method="post">';
...
//				megaupload_footer += "\n"+'<br /><img style="border:none;max-width:150px;height:20px" src="'+MUP_LNG.donate_src_button+'" alt="PayPal !" /><br />';
				megaupload_footer += "\n"+'<input style="border:none;color:'+GM_getValue('MUP_color_text')+';max-width:150px;height:20px;" type="image" name="submit" src="'+MUP_LNG.donate_src_button+'" alt="PayPal!" />';
				megaupload_footer += "\n"+'<br />';
//				megaupload_footer += "\n"+'<br />'+MUP_LNG.donate_02;
			megaupload_footer += "\n"+'</form>';
//			megaupload_footer += "\n"+'</a>';
		megaupload_footer += "\n"+'</div>';
		megaupload_footer += "\n"+'<div id="img_MU" style="float:left;width:'+(megavideo_url!=''?'33%':'50%')+';">';
			megaupload_footer += "\n"+'<span style="color:'+GM_getValue('MUP_color_href')+';cursor:pointer;" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" onclick="document.getElementById(\'old_body\').style.display=\'none\';document.getElementById(\'new_body\').style.display=\'block\';">'+MUP_LNG.link_mu_01+'</span>';
			megaupload_footer += "\n"+'<br /><img style="vertical-align:center;width:150px;height:20px" src="http://wwwstatic.megaupload.com/images/logo.gif" alt="" /><br />';
	...
	new_body_page = "\n"+'<div id="gen_body" style="background-color:'+GM_getValue('MUP_color_bg')+';padding-top:15px;padding-bottom:'+(window.innerHeight/3)+'px;">';
		new_body_page += "\n"+'<div id="old_body" style="display:'+(MUP_load?'none':'block')+';background-color:#FFF;width:100%;'+(MUP_downloadlink?'height:'+(MUP_Height+50)+'px;':'')+'min-height:550px;margin:auto;text-align:center;">'+body_ori+'</div>';
		new_body_page += "\n"+'<div id="new_body" style="display:'+(MUP_load?'block':'none')+';">';
			new_body_page += "\n"+'<div id="url_dl" style="font-size:16px; font-weight:bold;margin-bottom:8px">';
				new_body_page += "\n"+'<a id="url_link" style="font-size: 16px;color:'+GM_getValue('MUP_color_href')+';" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" href="'+MUP_url_movie+'" title="'+MUP_LNG.link_mu_dl+' '+MUP_name_fic+'">'+MUP_name_fic+'</a>';
				new_body_page += "\n"+'<span id="url_size" style="color:'+GM_getValue('MUP_color_text')+';font-size:smaller;margin-left:25px">'+MUP_taille_fic+'</span>';
				new_body_page += "\n"+'<span id="url_plus" style="color:'+GM_getValue('MUP_color_href')+';margin-left:25px;" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;">&nbsp;</span>';
			new_body_page += "\n"+'</div>';
			new_body_page += "\n"+'<div style="color:'+GM_getValue('MUP_color_text')+';text-align:center;width:'+(MUP_Width-10)+'px;min-width:660px;margin:auto;border:4px groove;padding:0px">';
				new_body_page += Choix_view;
			new_body_page += "\n"+'</div>';
			new_body_page += "\n"+'<div id="frame_allocine" style="margin:auto;width:1050px;display:none"><iframe style="width:100%;height:'+MUP_Height+'px;border:none" src="http://www.allocine.fr/film/"></iframe></div>';
			new_body_page += "\n"+'<div id="frame_imdb" style="margin:auto;width:1050px;display:none"><iframe style="width:100%;height:'+MUP_Height+'px;border:none" src="http://'+MUP_LNG.link_imbd+'/"></iframe></div>';
			new_body_page += "\n"+'<div id="Movie_load" style="margin:auto;width:630px;height:'+(MUP_Height/2)+'px;display:'+(MUP_stream?'none':'block')+';text-align:center;padding-top:'+(MUP_Height/2)+'px">';
				new_body_page += "\n"+'<input id="Movie_url" type="text" size="100" value="" />';
			new_body_page += "\n"+'</div>';
			new_body_page += "\n"+'<div id="Div_Counter_MUP" style="margin:auto;width:630px;height:'+(MUP_Height/2)+'px;display:'+((MUP_stream && Counter_MUP > 0)?'block':'none')+';text-align:center;padding-top:'+(MUP_Height/2)+'px;font-size:24px;font-weight:bold;">';
				new_body_page += MUP_LNG.wait_thx+' <span id="Time_Counter_MUP">'+Counter_MUP+'</span> '+MUP_LNG.wait_sec;
			new_body_page += "\n"+'</div>';
			new_body_page += "\n"+'<object id="lecteur_divx" style="display:'+((Counter_MUP == 0)?'block':'none')+';margin:auto;width:'+MUP_Width+'px;height:'+MUP_Height+'px" onmouseover="document.getElementById(\'MUP_MUP\').style.visibility=\'visible\'" type="video/divx" '+(MUP_stream?'data="'+MUP_url_movie+'"':'')+' codebase="http://download.divx.com/player/DivXBrowserPlugin.cab">';
			if (MUP_stream)
			{
				new_body_page += "\n"+'<param name="src" value="'+MUP_url_movie+'" />';
			}
			new_body_page += "\n"+'<param name="minVersion" value="2.0" />';
			if (Prev_img_Divx!='') new_body_page += "\n"+'<param id="previewImage" name="previewImage" value="'+Prev_img_Divx+'" />';
			new_body_page += "\n"+'<param name="custommode" value="'+GM_getValue('MUP_skin')+'" />';
			new_body_page += "\n"+'<param name="autoPlay" value="'+GM_getValue('MUP_autoplay')+'" />';
			new_body_page += "\n"+'<param name="AutoStart" value="'+GM_getValue('MUP_autoplay')+'" />';
			new_body_page += "\n"+'<param name="wmode" value="transparent" />';
			new_body_page += "\n"+'<param name="loop" value="false" />';
			new_body_page += "\n"+'<div style="padding-top:50px">'+MUP_LNG.error_l1+MUP_LNG.error_l2+'<span style="color:'+GM_getValue('MUP_color_href')+';cursor:pointer" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" onclick="location.reload();">'+MUP_LNG.error_l3+'</span>'+MUP_LNG.error_l4+'</div>';
			new_body_page += "\n"+'</object>';
//			new_body_page += '<a style="color:'+GM_getValue('MUP_color_href')+';" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" href="'+MUP_url_nb_dl+'" target="_blank">'+MUP_LNG.nb_dl+'</a>';
...
function options_MUP()
{
	...
	html_MUP_options += '<div style="font-weight:bold;text-align:center;color:#000;margin:auto;">'+MUP_LNG.opt_title+' '+MUP_LNG.name+'</div>\n';
		...
		html_MUP_options += '<div class="MUP_option_cols">\n';
			...
			html_MUP_options += '</div>\n';
			html_MUP_options += '<div class="MUP_option_valeur">\n';
				html_MUP_options += '<input class="InputOption" name="MUP_format" type="radio" '+(GM_getValue('MUP_ratio')=='1.78'?'checked="checked"':'')+'" value="1.78" /><sup>16</sup>/<sub>9</sub>\n';
				html_MUP_options += '<input class="InputOption" name="MUP_format" type="radio" '+(GM_getValue('MUP_ratio')=='1.60'?'checked="checked"':'')+'" value="1.60" /><sup>16</sup>/<sub>10</sub>\n';
				html_MUP_options += '<input class="InputOption" name="MUP_format" type="radio" '+(GM_getValue('MUP_ratio')=='1.33'?'checked="checked"':'')+'" value="1.33" /><sup>4</sup>/<sub>3</sub>\n';
			html_MUP_options += '</div>\n';
			...
			html_MUP_options += '</div>\n';
			html_MUP_options += '<div class="MUP_option_valeur">\n';
				html_MUP_options += '<input class="InputOption" id="MUP_AP" type="checkbox" '+(GM_getValue('MUP_autoplay')=='true'?'checked="checked"':'')+' />\n';
			html_MUP_options += '</div>\n';

			html_MUP_options += '<br style="clear:both;"/>\n';

			html_MUP_options += '<div class="MUP_option_texte">\n';
				html_MUP_options += '<label for="MUP_skin">Skin:</label>\n';
			html_MUP_options += '</div>\n';
			html_MUP_options += '<div class="MUP_option_valeur">\n';
				html_MUP_options += '<select id="MUP_skin">\n';
					html_MUP_options += '<option value="DivX8" '+(GM_getValue('MUP_skin')=='DivX8'?'selected="selected"':'')+'>DivX8</option>\n';
					html_MUP_options += '<option value="Stage6" '+(GM_getValue('MUP_skin')=='Stage6'?'selected="selected"':'')+'>Stage6</option>\n';
					html_MUP_options += '<option value="StageVuGold" '+(GM_getValue('MUP_skin')=='StageVuGold'?'selected="selected"':'')+'>StageVuGold</option>\n';
				html_MUP_options += '</select>\n';
			html_MUP_options += '</div>\n';
		...
		html_MUP_options += '<div style="text-align:center">\n';
			...
			html_MUP_options += '<input class="InputOption" id="MUP_load" type="checkbox" '+(GM_getValue('MUP_autoload')!='false'?'checked="checked"':'')+'" />\n';
		html_MUP_options += '</div>\n';
...
function save_options_MUP(evt)
{
	...
	GM_setValue('MUP_skin',document.getElementById('MUP_skin').value);
...
function options_bg_MUP(evt)
{
	...
	{
		document.getElementById('MUP_div_check_bg').appendChild(createElement('input',{id:"MUP_check_bg", class:"InputOption", type:"checkbox", checked:"checked"}, 'click save_options_bg_MUP false'));
		document.getElementById('MUP_div_check_bg').appendChild(createElement('span',{id:"MUP_check_valid", style:"font-style:italic"}, '', ' On'));
	}
	else
	{
		document.getElementById('MUP_div_check_bg').appendChild(createElement('input',{id:"MUP_check_bg", class:"InputOption", type:"checkbox"}, 'click save_options_bg_MUP false'));
...
function getCSS()
{
	...
	html_css += '#div_MUP_options {font-variant:small-caps;position:fixed; margin:auto; width:600px; bottom:10px; left:'+(((window.innerWidth-600)/2)-26)+'px; z-index:9999; background-color:#FFCC66; padding:5px 8px 5px 8px; -moz-border-radius:5px; font-size:small;}';
	html_css += '#div_MUP_update {text-align:center;font-size:22px;font-variant:small-caps;position:fixed; margin:auto;width:100%;height:100%;top:2px;z-index:9999;background-color:#000;color:#FFCC66;opacity: 0.95;padding-top:'+((window.innerHeight-150)/2)+'px;}';
//	html_css += '#div_MUP_update {text-align:center;font-variant:small-caps;position:fixed; margin:auto; width:600px; top:2px; left:'+(((window.innerWidth-600)/2)-26)+'px; z-index:9999; background-color:#FFCC66; padding:5px 8px 5px 8px; -moz-border-radius:5px; font-size:small;}';
	html_css += '#div_MUP_options_bg {font-variant:small-caps;position:fixed; margin:auto; width:600px; top:50px; left:'+(((window.innerWidth-600)/2)-26)+'px; z-index:9999; background-color:#FFCC66; padding:5px 8px 5px 8px; -moz-border-radius:5px; font-size:small;}';
	...
	html_css += '.MUP_Link {cursor:pointer;font-weight:bold;text-decoration:underline;}';
	html_css += '.BR {margin:10px;}';
	html_css += '.InputOption {opacity:100 !important;}';
...
function new_MUP_version(evt)
{
	...
	html_MUP_update = MUP_LNG.title_update_new+' '+MUP_LNG.name+'<br class="BR" /><br class="BR" />';
	...
	document.getElementById('div_MUP_update').appendChild(createElement('div', {onclick:"this.parentNode.parentNode.removeChild(document.getElementById(\'div_MUP_update\'))", class:"MUP_Link"}, '', '<br class="BR" />'+MUP_LNG.option_close+''));
}
function update_MUP()
{
