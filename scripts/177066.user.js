...
(function(){
	...
	function init(){
		$(".pt_submit .subbtn_bg").before("BDUSS：<input type='text' id='txtBDUSS' value=''/><select id='os'></select><input id='win8' class='subbtn_bg' value='客户端' type='button'/>");	
		var win8=document.querySelector("#win8");	
		var os=document.querySelector("#os");
		win8.addEventListener("click",tbs,false);
		$("#os").css("float","left");
    	var osSel =[
    		"iPhone",
    		"Android",
    	...
    	for(var i=0;i<osSel.length;i++){
    		var osOpt = new Option(osSel[i], i);
    		os.options[os.options.length] = osOpt;			
    	}	
	}
	...
	function getTbs(data){
		...
		GM_xmlhttpRequest({
			...
			onload:function(response) {
			    var tbs=response.responseText;
			    tbsJSON=eval('('+tbs+')');
				tbsJSON=tbsJSON.anti.tbs;
			    post(tbsJSON,client_type);
			}
		});	
	}
	function tbs(){
	...
	function post(tbs,client_type){
		var content=$(".tb-editor-editarea").html();
