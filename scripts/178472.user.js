...
// @include     *://*/gptc.php?v=entry*
// @include     *://*/gptc.php?v=read*
// @include     *://*/view.php?id=*
	...
	setTimeout(function(){
		...
		$.each(clickSrc,function(k,v){
			...
			if(thisItem.attr('href')){
				if( ( thisItem.attr('href').indexOf('gptc.php?v=entry&type=ptc') >= 0 || thisItem.attr('href').indexOf('gptc.php?v=read') >= 0 ) && thisItem.text().indexOf('Cheat Check') == -1){
					var obj = {
			...
			$.each($('a,div'),function(k,v){
				...
				if(thisItem.attr('href')){
					...
					if( (thisItem.attr('onclick').toString().indexOf('entry&type=ptc') >= 0  ) && thisItem.text().indexOf('Cheat Check') == -1){
						var obj = {
							href 	: 'gptc.php?v='+thisItem.attr('onclick').toString().split(',')[1].match(/'[^]+'/)[0].replace(/'/g,''),
							jObj	: thisItem
						}
						arr.push(obj); 
					}
...
if(wparent.location.href.indexOf('gptc.php?v=read') != -1 && self == top){
	var div = $('<div>');
...
if(wparent.location.href.indexOf('gptc.php?v=entry') != -1 && 
self == top){
	...
	if($('frame:nth(1)').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('idw-silverclix')!=-1)cheatCheck = [8,14,3];
		...
		if(wparent.location.host.indexOf('javaklix')!=-1)cheatCheck=[42,35,38];
		...
		if(wparent.location.host.indexOf('cashtravel')!=-1)cheatCheck=[];
		if(wparent.location.host.indexOf('67.222.148.150')!=-1)cheatCheck=[31,26];
		...
		var inters = setInterval(function(){
			...
			if($('#timer',doc).text().indexOf('Click') >= 0 || $('#timer img:visible',doc).size() || fulltimer==-1){
				...
				function ajaxRec(btnKey){
					clearInterval(inters);
					$.ajax({
						url : 'gptc.php?v=verify&buttonClicked='+btnKey+'&id='+win.id+'&type='+win.type+'&pretime='+win.pretime+'&'+win.url_variables
