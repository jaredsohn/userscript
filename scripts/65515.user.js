// ==UserScript==
// @name           TW Massenrekrutierung
// @description    Erweiterte Massenrekrutierung
...
(function(){
	...
	var lang = {
		...
		'de': {
			...
			'settoobig': 'Die Einstellungen für die Massenrekrutierung sind zu gross um in einem Cookie gespeichert zu werden. Es kann daher zu Problemen kommen. Das Löschen einiger Gruppen kann das Problem lösen.'
		}
	};
	...
	var set_cookie = function(arr) {
		if(!/[&?]t=\d+/.test(location.href)) {
			var value = 'massrecruiting=' + escape(_JSON.stringify(arr)) + ';';
			if(value.length > 4096) {
				alert(lang[curlang]['settoobig']);
			}
			document.cookie = value + ' expires=' + (new Date(2036, 1, 1)).toGMTString() + ';';
		}
	};
	...
	var h3 = ce('h3');
	h3.textContent = lang[curlang]['settings'] + ' Â»'+selectedgroup+'Â«';
