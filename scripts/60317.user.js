// ==UserScript==
// @name          Interface-aware version of Book Burro
// @namespace     http://overstimulate.com/userscripts/
// @description   Book prices from various book stores
...
var handlers = [
  ...
  { name: 'Ad Libris', id: 'alse', hostname: /\badlibris\.se$/i,
    ...
    ajaxPrice: function( html, http )
    {
      if( html.match( 'Ingen titel med detta ISBN finns hos AdLibris.' ) )
	return '';
      return SEK( html.match( 'class="price">([^<]*)<' )[1] );                       //FIXEDDDDD
    } },/*
  { name: 'Akademibokhandeln', id: 'abse', hostname: /\bakademibokhandeln\.se$/i,   //CHECKEARRR
    getISBN: nextSameTagAfter( 'td', '^ISBN' ),
    updateURL: firstLinkTo( 'cc_artikel.visa_artikelkort', 'http://www.akademibokhandeln.se/db/caweb/' ),
    bookURL: 'http://www.akademibokhandeln.se/db/caweb/sok.avanc_artiklar?mtitel=exact&cisbn=978%s',
    ajaxPrice: /<span class="pris">\s*([^<]*)</i, priceFix: SEK },*/
  { name: 'Alibris', id: 'alibris', hostname: /\balibris\.com$/i,                  //FIXEDDDDD
    getISBN: nextTagAfter( 'b', 'ISBN:' ),
    bookURL: 'http://www.alibris.com/booksearch', ajaxData: 'S=R&qisbn=%s',
    updateURL: firstLinkTo( '/booksearch.detail?', 'http://www.alibris.com' ),
    ajaxPrice: /<strong>(&euro;[^<]*)</i },/*        
  { name: 'AllDirect.com', id:'alldirect', hostname: /\balldirect\.com$/i,
    getISBN: nextTagAfter( 'b', 'ISBN:' ),
    bookURL: 'http://www.alldirect.com/book.asp?&isbn=%s',
    ajaxPrice: 'All Direct\x27s[^P]*Price[^\$]*([^<]*)<' },*/
  { name: 'Amazon', id: 'amazon', hostname: /\bamazon\.com$/i,                           //FIXEDDDDD                   
    getISBN: function() {
      if( location.href.match( 'rate-this' ) ) return;
      return location.href.match( /\/([0-9X]{10})(\/|\?|$)/i )[1];
    },
    bookURL: 'http://www.amazon.com/exec/obidos/ASIN/%s/'+ amazon_associate_code,
    ajaxURL: 'http://webservices.amazon.com/onca/xml?Service=AWSECommerceService&AWSAccessKeyId=' + amazon_dev_key +'&Operation=ItemLookup&IdType=ISBN&SearchIndex=Books&ResponseGroup=OfferSummary&ItemId=%s',    
    ajaxPrice: function( html, http )
    {    	
      var xml = str2xml( html );
      var our = xml.getElementsByTagName( 'LowestNewPrice' );
      var used = xml.getElementsByTagName( 'LowestUsedPrice' );
      return { amazon: our.length && our[0].childNodes[2].textContent || '',
	  amazon_used: used.length && used[0].childNodes[2].textContent || '' };
    } },/*
  { name: 'Amazon (used)', id: 'amazon_used', ajaxPrice: false,
    bookURL: 'http://www.amazon.com/exec/obidos/redirect?tag='+ amazon_associate_code +'&path=tg/stores/offering/list/-/%s/all/' },  //BUT WORKS */
  { name: 'Barnes & Noble', id: 'bn', hostname: /\bbarnesandnoble\.com$/i,                              //FIXEDDDDD
    bookURL: 'http://search.barnesandnoble.com/booksearch/isbninquiry.asp?isbn=%s',
    ajaxURL: 'http://search.barnesandnoble.com/booksearch/isbninquiry.asp?isbn=%s',
    ajaxPrice: /<strong>(\$[^<]*)</ },
  { name: 'Biblio.com', id: 'biblio', hostname: /\bbiblio\.com$/i,                                 //FIXEDDDDD   
    bookURL: 'http://www.biblio.com/isbn/%s.html', ajaxMethod: 'GET',
    ajaxPrice: /((?:&#036;|$)\S+)\s*</i },
  { name: 'BiggerBooks.com', id: 'biggerbooks', hostname: /\bbiggerbooks\.com$/i,                      //FIXEDDDDD  
    bookURL: 'http://www.dpbolvw.net/click-'+ half_associate_code +'-9467039?ISBN=%s',
    ajaxURL: 'http://www.biggerbooks.com/bk_detail.asp?ISBN=%s', ajaxPrice: /<span class="price">([^<]+)/ },
  { name: 'Bokus.com', id: 'bokus', hostname: /\bbokus\.com$/i,                                          //FIXEDDDDD  
    getISBN:/(?:\/|FAST_VALUE=ISBN&FAST=|ISBN=)([0-9X]{10})(\.html)?(\?|&|$)/i,
    bookURL: 'http://www.bokus.com/b/%s.html', 
    ajaxURL: 'http://www.bokus.com/cgi-bin/book_search.cgi?FAST_VALUE=ISBN&FAST=%s',
    ajaxPrice: />([^:]*):-</i, ajaxMethod: 'GET', priceFix: SEK },
  { name: 'Bookbyte', id: 'bookbyte_new', hostname: /\bbookbyte\.com$/i,                              //FIXEDDDDD 
    getISBN: function(){ return document.getElementById('lbIsbn').textContent; },
    bookURL: 'http://www.bookbyte.com/product.aspx?isbn=%s',
    ajaxURL: 'http://www.bookbyte.com/product.aspx?isbn=%s', ajaxPrice: function( html, http ) {
      var prices = { bookbyte_new: html.match( /new:\s(\$\d{1,3}\.\d{2})/ ),
		     bookbyte_used: html.match( /used:\s(\$\d{1,3}\.\d{2})/ ),
		     bookbyte_bazaar: html.match( />(\$\d{1,3}\.\d{2})</ ) }, i;
      for( i in prices )
	prices[i] = prices[i] ? unHTML( prices[i][1] ).replace( /[^$.0-9]/g, '' ) : '';
      return prices;
    } },
  { name: 'Bookbyte (used)', id: 'bookbyte_used', ajaxPrice: false, bookURL: 'http://www.bookbyte.com/product.aspx?isbn=%s' },     //FIXEDDDDD 
  { name: 'Bookbyte (bazaar)', id: 'bookbyte_bazaar', ajaxPrice: false, bookURL: 'http://www.bookbyte.com/product.aspx?isbn=%s' }, //FIXEDDDDD 
  /*{ name: 'Bookpool.com', id: 'bookpool', hostname: /\bbookpool.com$/i,
    getISBN: /(?:qs|is|sm)[\/=]([0-9X]{10})(&|\?|\.|$)/i,
    bookURL: 'http://www.bookpool.com/sm/%s',
    ajaxPrice: /Our\s+Price:?(?:<[^>]*>\s*)*([^<]+)/i },*/
  { name: 'Booksamillion.com', id: 'bamm', hostname: /\bbooksamillion\.com$/i,                                            //FIXEDDDDD 
    bookURL: 'http://www.booksamillion.com/ncom/books?isbn=%s', 
    ajaxURL: 'http://www.booksamillion.com/ncom/books?isbn=%s', ajaxMethod: 'GET',
    ajaxPrice: /Online\sPrice:\s(\$\d{1,3}\.\d{2,2})/i }, //function( html, http ){ return '$' + html.match( /Online\sPrice:\s(\$\d{1,2}\.\d{2,2})/i ); } },
  { name: 'Buy.com', id: 'buy', hostname: /\bbuy\.com$/i,                                                               //FIXEDDDDD
    getISBN: function(){ return document.title.match( /ISBN ([0-9X]{10})/i )[1]; },
    bookURL: 'http://www.buy.com/retail/GlobalSearchAction.asp?qu=%s',
    ajaxPrice: /class="adPrice"[^>]*><b>(\$\d{1,3}\.\d{2,2})<\/b><\// },
  { name: 'CDON.com', id: 'cdon', hostname: /\bcdon\.se$/i,                                                             //FIXEDDDDD
    getISBN: nextSameTagAfter( 'td', 'ISBN:' ), updateURL: function( html, http ) {
      return unHTML(html.match( /<td class="title"><a href="([^"]*)/ )[1]);
    }, bookURL: 'http://cdon.se/böcker/advanced-search?isbn=%s',
    ajaxPrice: /<td class="price">((\d)*) kr<\/td>/i, ajaxMethod: 'GET', priceFix: SEK },
  { name: 'eCampus.com', id: 'ecampus', hostname: /\becampus.com$/i,
    bookURL: 'http://www.anrdoezrs.net/click-'+ half_associate_code +'-5029466?ISBN=%s',
    ajaxURL: 'http://www.ecampus.com/bk_detail.asp?ISBN=%s', ajaxPrice: /Our\s+Price\s*<font[^>]*>([^<]+)/i },/*
  { name: 'Ex Libris', id: 'exlibris', hostname: /\bexlibris\.se$/i,
  ...
  { name: 'Half.com', id: 'half', hostname: /\bhalf\.ebay\.com$/i,                                              //FIXEDDDDD
    getISBN: nextTagAfter( 'b', 'ISBN:' ),
    bookURL: 'http://books.half.ebay.com/ws/web/HalfISBNSearch?isbn=%s',
    ajaxURL: 'http://books.half.ebay.com/ws/web/HalfISBNSearch?isbn=%s',
    ajaxPrice: 'Best[^P]*Price[^$]*([^<]*)<' },
  { name: 'Bokia', id: 'bokia', hostname: /\bbokia\.se$/i,                                                     //FIXEDDDDD
    getISBN: /(?:\/bok|&s_search=)([0-9Xx]{10})(\.html)?(\?|$)/, 
    bookURL: 'http://www.bokia.se/results.html?all_search=%s',
    ajaxURL: 'http://www.bokia.se/results.html?all_search=%s',
    ajaxMethod: 'GET', ajaxPrice: /<span class="pricetips price">((\d)*):-</, priceFix: SEK },/*
  { name: 'Libris Media', id: 'librismedia', hostname: /\blibris\.se$/i,
    getISBN: function(){ return document.body.innerHTML.match( /<span class=[^>]*>\s*ISBN ([0-9X]{10})/mi )[1]; },
    updateURL: function( html, http ) {
    	GM_log( html ) ;
      return unHTML(html.match( /<li class="libris-grid-startrow">[\n\r\s]*<a href="([^"]*)"/ ));
    },
    bookURL: 'http://www.libris.se/catalogsearch/result/?q=%s',
    ajaxURL: 'http://www.libris.se/catalogsearch/result/?q=%s',    
    ajaxMethod: 'GET',
    ajaxPrice: [/<span class="normal2">Pris:.*?<span[^>]*>(\d+)(,\d\d)? *kr</mi,
		/<b>\s*Libris l\345gpris (\d+)(,\d\d)? *kr/mi,
		/<span class="fakta">\s*(\d+)(,\d\d)? *kr/mi], priceFix: SEK },*//*
...
function makeAjaxCallback( handler, isbn )
{
  ...
  switch( typeof callback )
  {
	...
	if(!regexps.length) regexps=new Array(regexps); //FIXEDDDDDDD
...
function burro( location, isbn )
{
  ...
  addEventHandler( close, 'click', function(){ root.parentNode.removeChild( root ); } );//document.body.removeChild( root ); } );  //Changed
...
function init(loadBookOcc)
{
  ...
  //{
	...
	//if( (isbn = checkISBN( isbn )) )
	  //burro( h.id, isbn );
	  var bookBurroPanel = burro( "Barnes", isbn );

          var addBookChildOcc = window.doc.createEvent("ProcessingEvents");     
          addBookChildOcc.initProcessingEvent("appendChildBook",book,bookBurroPanel,{});
          window.doc.dispatchEvent(addBookChildOcc); 
	//else if( debug )
...
window.doc.addEventListener("loadBook",init,true);