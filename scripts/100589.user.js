// ==UserScript==
// @name			IMDB - share vote on Facebook, Twitter, Tumblr - Turkish
// @description		Adds links to IMDB title pages that let you share your vote details on Facebook and/or Twitter turkish version
// @version			1.6
...
//shortenTwitterUrl = false;
shortenTwitterUrl = true; // use the bit.ly API to create a short URL like http://imdb.to/jI6bw

bitlyLogin = 'shorter'; // customize for your own bit.ly API login details
bitlyKey = 'R_2546703adeade4dc850d14825242f183';

// ==/User-Defined Variables==

findPattern = "//span[@class='rating-rating rating-your']";
...
if (voteResults.snapshotItem(0) != null && voteResults.snapshotItem(0).innerHTML != "") {
	...
	var imdbUrl = "http://imdb.com/title/tt"+getIMDBid();

	findPattern = "//td[@id='img_primary']/a/img";
	imageResults = document.evaluate( findPattern, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );
	// make sure there's an image if we're going to share the image too
	if (imageResults.snapshotItem(0) != null) {
		var movieImage = imageResults.snapshotItem(0).src;
		shareFacebookUrl = 'http://www.facebook.com/connect/prompt_feed.php?attachment={"name"%3A"'+movieName+'"%2C"caption"%3A"{*actor*} '+movieName+' adlı filme IMDb de 10 üzerinden '+voteValue+' verdi."%2C"href"%3A"'+imdbUrl+'"%2C"media"%3A[{"type"%3A"image"%2C%0A     "src"%3A"'+escape(movieImage)+'"%2C"href"%3A"'+escape(imdbUrl)+'"}]}';
	}
	else {
		shareFacebookUrl = 'http://www.facebook.com/connect/prompt_feed.php?attachment={"name"%3A"'+movieName+'"%2C"caption"%3A"{*actor*} gave the film '+movieName+' a rating of '+voteValue+'/10 on IMDb."%2C"href"%3A"'+imdbUrl+'"}';
	}

	var shareTumblrUrl = 'http://www.tumblr.com/share?v=3&amp;u=' +escape(imdbUrl)+ '&t=&s=I%20gave%20the%20film%20' +movieName+ '%20a%20rating%20of%20' +voteValue+ '/10%20on%20IMDb.';
	var shareTwitterUrl = 'http://twitter.com/home?status=gave the film '+movieName+' a rating of '+voteValue+'/10 on %23IMDb '+imdbUrl;

	addedDivFBLink = document.createElement('div');
	addedDivFBLink.innerHTML = '<span><strong>Share</strong> your vote:</span><a target="_blank" title="Share on Facebook" href=\''+shareFacebookUrl+'\'><img src="'+facebookImage+'" alt="Share on Facebook"></a> <a target="_blank" id="GM_twitter_link" title="Share on Twitter" href=\''+shareTwitterUrl+'\'><img src="'+twitterImage+'" alt="Share on Twitter"></a> <a target="_blank" title="Share on Tumblr" href=\''+shareTumblrUrl+'\'><img src="'+tumblrImage+'" alt="Share on Tumblr"></a>';
	addedDivFBLink.setAttribute('id','GM_ShareVoteOnFacebook');

	findPattern = "//div[@class='social_networking']";
	...
	if (shortenTwitterUrl) {
	
		var bitlyAPIFull = 'http://api.bit.ly/shorten?version=2.0.1&login='+bitlyLogin+'&apiKey='+bitlyKey+'&format=xml&longUrl=' + imdbUrl;
		...
		GM_xmlhttpRequest({
			...
			onload: function(results) {
				...
				var shortLink = dom.getElementsByTagName('shortUrl')[0].textContent;
				var twitterLink = document.getElementById("GM_twitter_link");
				twitterLink.href = shareTwitterUrl.replace(imdbUrl, shortLink);
			}
