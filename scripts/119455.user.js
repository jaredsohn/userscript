...
// @description    允许用户隐藏，调整大小或修改特定的页面元素
...
if(document.evaluate("//iframe[@id='GM_config']",document,null,9,null).singleNodeValue) return;
   ...
   GM_config.frame.addEventListener('load', function(){
       ...
       frameBody.appendChild(obj.create('div', {id:'buttons_holder', kids:new Array(
           obj.create('button',{id:'saveBtn',textContent:'保存',title:'保存选项并关闭窗口',className:'saveclose_buttons',onclick:function(){GM_config.close(true)}}),
           obj.create('button',{id:'cancelBtn', textContent:'取消',title:'关闭窗口',className:'saveclose_buttons',onclick:function(){GM_config.close(false)}}),
           obj.create('div', {className:'reset_holder block', kids:new Array(
               obj.create('a',{id:'resetLink',textContent:'还原为默认值',href:'#',title:'还原设置到默认配置',className:'reset',onclick:obj.reset})
       )}))}));
...
function AddCustomizeMenu() {
    if (!document.getElementById("cust-menu-item-inner")) {
        ...
        customizeMenuItemInner.innerHTML = "个性化...";
