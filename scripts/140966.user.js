// ==UserScript==
// @name           Tribalwars Premium Hack Czech/Slovak Version made by MajkyTM
// @namespace      www.pointdownloads.net
// @description    Tento script ti pridá všetky funkcie prémium, a to všetko zadarmo.
// @autor          FernandoXLR
...
if (game_data.player.premium == false) {
	...
	if (language == 'pt') {
		...
		if (language == 'nl'){        
			...
			if (language == 'ae'){
				...
				lang_messages [3] = 'Allow HTML in messages';
	    ...
	    lang_knight = 'Paladin';
	    lang_snob = 'Šlachtic';
            lang_map[0] = 'Vyhledať dediny:';
	    lang_map[1] = 'Dediny barbarú:';
	    lang_map[2] = 'min:';
	    lang_map[3] = 'Dediny hráčov:';
	    lang_map[4] = 'max:';
	    lang_map[5] = 'Dosah:';
	    lang_map[6] = '» Vyhladať';	
	    lang_map[7] = 'Mini mapa:';
	    lang_map[8] = 'Mapa:';
	    lang_map[9] = 'Zmeniť veľkosť mapy';	   
	...
	if ((/.*?screen=report.*?mode=.*?view=.*/).test(location.href) == true) {
		if (language != 'ae') {
	...
	*/  }
    ...
    if ((/https?:\/\/.*?\/game.php\?.+screen=mail&mode=view.*/).test(location.href) == true) {                
