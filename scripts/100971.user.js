...
// @description    Bulfleet bot
// @include        http://*.bulfleet.com/*
// @include        http://un2.bulfleet.com:82/*
// ==/UserScript==
...
var botcheckSound    	= sOldHornLong;
...
function oaPlaySound() {
	...
	switch ( theSound ) {
		case botcheckSound:	emb.setAttribute("volume", volAttacks);
							break;
...
function botcheck_alert() {
	theSound = botcheckSound;
	oaPlaySound();
	return;
}


function attack_alert()
{
...
{
	attack_presence = findString("Информация за враждебни флоти");
	if (attack_presence) attack_alert();
	
	message_presence = findString("нови съобщения");
	if (message_presence) new_message_alert();	
...
unsafeWindow.bulfleet = new Object();

unsafeWindow.bulfleet.init = function() {
...
function hack_main_autoRefresh() {
	if(conf['autoRefreshMainPHP']) {
		if(window.location!="http://un2.bulfleet.com:82/botcheck.php") {
			...
			if(wl.indexOf("main.php") != -1) {
				...
				setTimeout('bulfleet.event_hax_do_af5();',after);
				s=""+(parseInt(after)/1000)+" sec.";
				unsafeWindow.bulfleet.af5_after=s;
				window.status=s;
				check_status();
			}
		}
...
/* panel button af5 */
unsafeWindow.bulfleet.event_panel_af5 = function(obj) {
		conf['autoRefreshMainPHP'] = (conf['autoRefreshMainPHP'] == true ? false : true);
		if(!obj) obj=this;
		obj.innerHTML = unsafeWindow.bulfleet.event_panel_af5_getValue(true)
}


unsafeWindow.bulfleet.event_panel_af5_getValue = function(inner) {
		str = 'Refresher: ';
		if(conf['autoRefreshMainPHP'] === true) {
			str += 'On ('+parseInt(unsafeWindow.bulfleet.af5_after)+') sec.';
//			str += 'On ('+lbl_nanocredits+') sec.';
		}
		else { str += 'Off'; }
		return str;
}
...
unsafeWindow.bulfleet.event_hax_do_af5 = function(inner) {
		if(conf['autoRefreshMainPHP'] === true) {
			window.location='http://un2.bulfleet.com:82/main.php';
		}
}
/* krai af5 */
...
/* panel Attacks */
unsafeWindow.bulfleet.event_attack = function(obj) {
		conf['Attack'] = (conf['Attack'] == true ? false : true);
		if(!obj) obj=this;
		obj.innerHTML = unsafeWindow.bulfleet.event_attack_getValue(true)
}
unsafeWindow.bulfleet.event_attack_getValue = function(inner) {
		str = 'Radar: ';
		if(conf['Attack'] === true) {
			str += 'On';
		}
...
unsafeWindow.bulfleet.init_panelControls = function() {
//		unsafeWindow.bulfleet.event_panel_af5();
}

function Add_OptionPanel() {
	obj = document.getElementById("secondary-navigation");
	if(!obj) return false;
	htmlSrc = '<div class="nav-block">\n'+
	'<h3 class="first">Доп. настройки</h3>\n'+
	'<ul>\n'+
	'<li><a href="javascript:void(0);" id="Autorefresh" onclick="bulfleet.event_panel_af5(this);" title="Ще опресни до '+(unsafeWindow.bulfleet.af5_after ? '('+unsafeWindow.bulfleet.af5_after+')' : '')+'">'+unsafeWindow.bulfleet.event_panel_af5_getValue()+'</a></li>\n'+

	'<li><a href="javascript:void(0);" id="Radar" onclick="bulfleet.event_attack(this);" title="Включва/изключва търсенето на атака">'+unsafeWindow.bulfleet.event_attack_getValue()+'</a></li>\n'+

	'<li><a class="target 1" href="http://un2.bulfleet.com:82/galaxy.php?c1=4&c2=237" title="User0" >User0' + ' <img class="last_login" width="130" height="15" src="last_login_pic.php?p_id=155906"> </a></li>\n'+
	
	'<li><a class="target 2" href="http://un2.bulfleet.com:82/galaxy.php?c1=4&c2=226" title="User1" >User1' + ' <img class="last_login" width="130" height="15" src="last_login_pic.php?p_id=156882"> </a></li>\n'+
	
	'<li><a class="target 2" href="http://un2.bulfleet.com:82/galaxy.php?c1=4&c2=226" title="User2" >User2' + ' <img class="last_login" width="130" height="15" src="last_login_pic.php?p_id=156730"> </a></li>\n'+
	
	'<li><a class="target 2" href="http://un2.bulfleet.com:82/galaxy.php?c1=4&c2=190" title="User3" >User3' + ' <img class="last_login" width="130" height="15" src="last_login_pic.php?p_id=156816"> </a></li>\n'+
	
	'<li><a class="target 2" href="http://un2.bulfleet.com:82/galaxy.php?c1=4&c2=199" title="User4" >User4' + ' <img class="last_login" width="130" height="15" src="last_login_pic.php?p_id=151716"> </a></li>\n'+
		
	'</ul>\n'+
	'</div>';
	obj.innerHTML = htmlSrc + obj.innerHTML;
	setTimeout("bulfleet.init_panelControls()",500);

}

window.addEventListener("load", function(e) {
  unsafeWindow.bulfleet.init();
}, false);