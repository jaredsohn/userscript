// ==UserScript==
// @name           TribalWars v7
// @namespace      my.scripts.tw
// @include        http://gr13.fyletikesmaxes.gr/*
// @exclude 	   http://gr1.fyletikesmaxes.gr/*
// @exclude 	   http://gr2.fyletikesmaxes.gr/*
// @exclude 	   http://gr3.fyletikesmaxes.gr/*
// @exclude 	   http://gr4.fyletikesmaxes.gr/*
// @exclude 	   http://gr5.fyletikesmaxes.gr/*
// @exclude 	   http://gr6.fyletikesmaxes.gr/*
// @exclude 	   http://gr7.fyletikesmaxes.gr/*
// @exclude 	   http://gr8.fyletikesmaxes.gr/*
// @exclude 	   http://gr9.fyletikesmaxes.gr/*
// @exclude 	   http://gr10.fyletikesmaxes.gr/*
// @exclude 	   http://gr11.fyletikesmaxes.gr/*
// @exclude 	   http://gr12.fyletikesmaxes.gr/*
...
var buildingNames=['Επιτελείο','Στρατώνας','Σταύλος','Εργαστήριο','Ακαδημία','Μέρος συγκέντρωσης','Αγορά'];
var buildingLinks=['main','barracks','stable','garage','snob','place','market'];

var otherLinks = ['Στρ/γία Αμυντικό', 'Στρ/γία Επιθετικό','cavalry','Store resources','Attack richest village','Scout for farms',''];
var otherFuncs = ['recruit.defensive', 'recruit.offensive','calculateArmy','storeVillageResources','selectVillageWithMoreReseources','scoutForFarms',''];

var cssStyle = 	'.shortcutBarLeft {background: url("/graphic/index/mainborder-left-top.png") no-repeat scroll left top transparent; width: 25px;}' +
								'.shortcutBarRight {background: url("/graphic/index/mainborder-right-top.png") no-repeat scroll right top transparent;width: 25px;}';
...
function recruitGeneral(units, lowBound, hiBound, distribution) {
    ...
    /*if (popMax == 24000) {
       ...
       availResources = [wood * (distribution/100), stone * (distribution/100), iron * (distribution/100)];
       total_costs=[0,0,0];
    } else {*/
       //consider the population cost
       availResources = [wood * (distribution/100), stone * (distribution/100), iron * (distribution/100), (popMax - popCurrent) * (distribution/100)];
       total_costs=[0,0,0,0];
    //}
...
function getQueue() {
    ...
    queueTable = $xf("//table[@class='vis'][2]");
    if (queueTable != null && queueTable != undefined) {
	   rows = queueTable.getElementsByTagName('tr');
	   for (i = 0; i < rows.length; i++) {
		   unit = rows[i].cells[0].textContent;
		   if (unit != 'Εκπαίδευση') {
			   unit = unit.split(' ');
			   switch(unit[1]) {
				   case 'Δορατοφόρος':
					   ret[0] += parseInt(unit[0]);
					   break;
				   case 'Δορατοφόροι':
					   ret[0] += parseInt(unit[0]);
					   break;
				   case 'Ξιφομάχος':
					   ret[1] += parseInt(unit[0]);
					   break;
				   case 'Ξιφομάχοι':
					   ret[1] += parseInt(unit[0]);
					   break;
				   case 'Τσεκουρομάχος':
					   ret[2] += parseInt(unit[0]);
					   break;
				   case 'Τσεκουρομάχοι':
					   ret[2] += parseInt(unit[0]);
					   break;
				   case 'Τοξότης':
					   ret[3] += parseInt(unit[0]);
					   break;
				   case 'Τοξότες':
					   ret[3] += parseInt(unit[0]);
					   break;
				   ...
				   case 'Ανιχνευτής':
					   ret[4] += parseInt(unit[0]);
					   break;
				   case 'Ανιχνευτές':
					   ret[4] += parseInt(unit[0]);
					   break;
				   case 'Ελαφρύ':
					   ret[5] += parseInt(unit[0]);
					   break;
				   case 'Έφιππος':
					   ret[6] += parseInt(unit[0]);
					   break;
				   case 'Έφιπποι':
					   ret[6] += parseInt(unit[0]);
					   break;
				   case 'Βαρύ':
					   ret[7] += parseInt(unit[0]);
					   break;
					   
				   case 'Πολιορκητικός':
					   ret[8] += parseInt(unit[0]);
					   break;
				   case 'Πολιορκητικοί':
					   ret[8] += parseInt(unit[0]);
					   break;
				   case 'Καταπέλτης':
					   ret[9] += parseInt(unit[0]);
					   break;
				   case 'Καταπέλτες':
					   ret[9] += parseInt(unit[0]);
					   break;
...
function getResourcesFromReport() {
	xpathResult=doc.evaluate("//table[@id='attack_spy']//tr[1]//td[1]",doc,null,XPathResult.STRING_TYPE,null);
...
function calculateResources(sumResources, el) {
	if (sumResources > -1) {
			...
			if (el != undefined && el != null && el != '' ) {
				var html = cav+' light cavalry needed, '+hcav+' heavy cavalry needed, '+spear+' spears needed, '+sword+' swords needed, '+axe+' axes needed, '+archers+' archers needed';
				var exactUnits = calculateExactUnitsToSend(sumResources);
				html += "<br/><strong>Exact units to send ["+exactUnits+"]</strong>";
				$(el).html(html);
				return;
			}
...
function calculateExactUnitsToSend(sumResources) {
	...
	for(var i in unitArr) {
		var unit = unitArr[i];
		unitElem = $xf("//form[@id='units_form']//table//tr//td//table[@class='vis']//input[@name='"+unit+"']/following-sibling::a");//, "r").snapshotItem(0);
...
var selectVillageWithMoreReseources = function () {
	setTimeout(function(){
		...
		if (targetVillageId > 0 ) {
			...
			link='game.php?village='+village+'&screen=place&target=' + targetVillageId + '&resources=' + maxResources;
	...
	if(url.indexOf('screen=place')==-1) {
		...
		setTimeout(function() {
			...
			if (farmCoordinates != undefined) {
				...
				alert('No coordinates are set. Please set the coordinates at the Settings screen.');
			}
		});
	} 
}
...
function settings() {
	setTimeout(function() {
		...
		if (url.indexOf('screen=settings')>-1) {
			...
			var settingsTable = '<table border="0"><tr><th colspan="2" align="center"><h2>Tribal Wars v7 settings<h2></th></tr>';
			settingsTable += '<tr><td>Χωριά φάρμες (comma separated e.g xxx|yyy,kkk|ttt)</td><td><input type="text" id="farmCoords" name="farmCoords" size="100" value="' + farmCoordinates + '" /></td></tr>';
			settingsTable += '<tr><td colspan="2" align="center"><input type="button" id="saveFarmsBtn" value="Αποθήκευση" /></td></tr>';
			settingsTable += '</table>';
...
function main() {
	setTimeout(function() {
		...
		village = getCurrentVillage();    
		if (village!=null) {
			var shortcutBar = '<table id="shortcutBar" align="center" border="1" cellpadding="3" style="margin-top:7px;"><tr>';
			for(var i=0;i<buildingNames.length;i++) {
				shortcutBar += '<td><a href="/game.php?village='+village+'&screen='+buildingLinks[i]+'">' + buildingNames[i] + '</a></td>';
			}
			shortcutBar += '</tr><tr>';
			// create 2nd row
			for(i=0;i<otherLinks.length;i++) {
				shortcutBar += '<td><a href="' + otherFuncs[i] + '" name="extraFunctions">' + otherLinks[i] + '</a></td>';
			}
			shortcutBar += '</tr></table>';
			var wrappingHtml = '<tr class="shadedBG"><td class="bg_left"><div id="shortcutLeftDiv" class="shortcutBarLeft"> </div></td><td valign="top">' + shortcutBar + '</td><td class="bg_right"><div id="shortcutRightDiv" class="shortcutBarRight"> </div></td></tr>';
			...
			var armyForFarming = '<div id="armyForFarming" style="text-align: center"></div>';
			...
			// set the height equal to the height of the links table
			// needs to be set in this place in order to take into account
			// the lines that print the number of troops to be sent for farming
			var divHeight = $('#shortcutLeftDiv').parent().next().height();
			$('#shortcutLeftDiv').css({
				'height': divHeight
			});
			$('#shortcutRightDiv').css({
				'height': divHeight
			});
			...
			for (var i=0;i<villageLinks.length;i++) {
				villageLinks[i] = villageLinks[i].replace(/screen=(.*)"?/, "screen=" + screens + '"');
				villageDropList += '<li stlyle="display:block;height:26px;">' + villageLinks[i] + '</li>';
			}
			villageDropList += '</ul></div>';
			$(villageNameElem).after(villageDropList);
			$('#villagesList').css({
				'top': $('#menu_row2_village').offset().top + $('#menu_row2_village').height(),
