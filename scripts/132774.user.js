...
// @include        http://*.ika-world.com/search.php?view=player_details&*
// @include        http://*.ika-world.com/search.php?view=alliance_details&*
...
// old opaque map : http://img80.xooimage.com/files/0/8/e/map-33a07a6.png
//
...
GM_addStyle(
	...
	'#isoMapClose { background-image: url("/skin/interface/window_control_sprite.gif"); width:18px; height:18px; cursor:pointer; display:block; position:absolute; top:0px; right:-1px }\n' +
...
var IsoMap = new function() {
	...
	this.init = function() {
		...
		var matches = window.location.host.match(/s(\d+)\.([a-z]+)\.ikariam.com/);
		if(matches) {
			...
			$('div#GF_toolbar li').each(function() {
				if($(this).css('position') == 'absolute')		// This tries to fix a bug in other people's scripts.
					$(this).css('position', 'relative');		// As if I didn't have enough to do with my own bugs  :-(
			});

			// Ajout du bouton "IsoMap"
			$('div#GF_toolbar ul').append('<li><a id="isoMapButton" class="worldbg">IsoMap</a></li>');
			$("a#isoMapButton").click(function() {
				// Lazy map creation
				if ( IsoMap.islandData == null )
					IsoMap.createIsoMap();
				$("div#isoMapContainer").fadeToggle();
			});
		...
		// Ika-world : Language, server (loads 4 times on each page...)
		var matches = window.location.href.match(/http:\/\/([a-z]+)\.ika-world\.com\/search\.php\?view=player_details&land=([a-z]+)&welt=(\d+)/);
		if(matches) {
			IsoMap.setServer(matches[1], matches[3]);
			IsoMap.addPlayerFromIkaWorld();
			return;
		}
		var matches = window.location.href.match(/http:\/\/([a-z]+)\.ika-world\.com\/search\.php\?view=alliance_details&land=([a-z]+)&welt=(\d+)&/);
		if(matches && (window.location.href.indexOf('&show=towns') > 0)) {
			IsoMap.setServer(matches[1], matches[3]);
			IsoMap.addAllianceFromIkaWorld();
			return;
		}
		// ikascore
	...
	this.addPlayerFromIkascore = function() {
		...
		titletd.closest('tr').siblings().each(function(i){
			...
			var island = $(this).children('td:eq(2)').text();
	...
	this.addPlayerFromIkaWorld = function() {
		var playerDiv = $('div[id^="spieler_"]');
		if(playerDiv.length == 0)
			return;
		var playerId = playerDiv.attr('id').substr(8);
		var playerName = playerDiv.find('a:eq(0)').text();
		var playerAlliance = $('a[title="Ally details"]').text();
		var playerCities = [];
		var lines = $('span.townhall').closest('tbody').children('tr');
		lines.each(function(i){
			if((i == 0) || (i == lines.length-1))
				return;
			var city = {
				name:$(this).children('td:eq(0)').text(),
				x:parseInt($(this).children('td:eq(2)').text()),
				y:parseInt($(this).children('td:eq(3)').text()),
				id:''
			};
			// city id en option
			var cityLink = $(this).find('a:eq(0)').attr('href');
			if(cityLink) {
				var cityHref = cityLink.match(/&id=(\d+)&selectCity=(\d+)/);
				if(cityHref) {
					// http://s18.fr.ikariam.com/index.php?view=island&islandId=81&selectCity=991
					city.id = cityHref[2];
				}
			}
			playerCities.push(city);
		});
		if(IsoMap.storePlayerData(playerId, playerName, playerAlliance, (new Date()).getTime(), playerCities))
			$('span.townhall').closest('table').before('<div id="isoMapIkaworldButton" style="font-size:14pt;font-weight:bold;">IsoMap ✓</div>');
	};

	this.addAllianceFromIkaWorld = function() {
		var table = $('div#main table:eq(0)');
		if(table.length == 0) return;
		var td = table.find('td:eq(1)');
		var page = td.children('input').attr('value');
		var pageinfo = td.html().replace(/\(.*?\)/, '');	// enlève les parenthèses
		var pageinfo2 = pageinfo.replace(/<.*?>/, '');		// enlève le tag input
		var pages = parseInt(pageinfo2.match(/\d+/)[0]);
		var alliance = table.find('tr:eq(2) td:eq(1)').text();
		var time = (new Date()).getTime();
		var tr = table.find('tr');
		var rows = tr.length;	// 50 maximum par page
		var needsSaving = false;
		tr.each(function(i){
			if((i < 2) || (i == rows-1)) return;	// on passe l'en-tête et le pied
			var player = $(this).find('td:eq(0) a');
			var playerId = player.attr('href').match(/\d+/)[0];
			var playerName = player.text();
			var vacation = ($(this).find('span[title="Vacation"]').length > 0);
			if(vacation)
				IsoMap.removePlayerFromDB(playerId, playerName);
			else {
				var cityLink = $(this).find('td:eq(3) a');
				var city = {
					id:cityLink.attr('href').match(/&selectCity=(\d+)/)[1],
					name:cityLink.text(),
					x:$(this).find('td:eq(6)').text(),
					y:$(this).find('td:eq(7)').text()
				};
				needsSaving |= IsoMap.updatePlayerData(playerId, playerName, alliance, time, city);
			}
		});
		if(needsSaving) {
		...
		html += needsSaving ? '✓' : '✘';
		if(page < pages)
			html += '<br />'+pageinfo.replace(/<.*? value="(\d+)".*?>/, '$1')+' <input type="submit" value="-&gt;" name="seite_change" class="submit">';
		html += '</div>';
		table.before(html);
	};
	...
	this.removePlayerData = function() {
		...
		var inputid = $(this).closest('tr').find('input').attr('id');
		...
		GM_setValue(IsoMap.serverId, JSON.stringify(IsoMap.serverData));
		$('#enemyList').html(IsoMap.enemyList());
	...
	this.createIsoMap = function() {
		...
		if(typeof GM_info != 'undefined')			// FF / greasemonkey
		...
		$('body').append(html);
		var left = ($('body').width() - $('#isoMapContainer').width())/2;
		$('#isoMapContainer').css('left', left+'px');
		IsoMapSVGView.init();
		$("#isoMapClose").click(function() { $('#isoMapContainer').fadeOut('fast') });
		$("#updateAllyDataButton").click( IsoMap.updateAllySequence );
		IsoMap.setPlayerEvents();
		IsoMap.plotIslands();
		$('#isoMapContainer').draggable({ cursor:'move', handle:'#isoMap>h3'});
		...
		IsoMap.clickNoDrag($('.isoMapDropContainer h3'), IsoMap.toggleDropList );
		$('.isoMapDropContainer').draggable({cursor:'move', handle:'h3, .dragHandle' });
		$('#icon2D').click(IsoMapSVGView.switchTo2D);
		$('#iconIso').click(function(){IsoMapSVGView.switchTo3D(1)});
		$('#iconPersp').click(function(){IsoMapSVGView.switchTo3D(2)});
	}

	this.setPlayerEvents = function() {
		// playerForm : set events
		$('.isoMapCheckAll').click( function(){IsoMap.checkAll(this, true); return false;} );
		$('.isoMapCheckNone').click(function(){IsoMap.checkAll(this, false); return false;} );
		$('.isoMapCheckAll').mouseover( IsoMap.highlightPlayer );
		$('.isoMapCheckNone').mouseover( IsoMap.highlightPlayer );
		$('#isoMap input[type="checkbox"]').each(function(i) { $(this).click( IsoMap.togglePlayer ); });
		$('#isoMap label').each(function(i) { $(this).mouseover( IsoMap.highlightPlayer ); });
		$('#enemyDiv tr td:last-child').click( IsoMap.removePlayerData );
	}
	...
	this.checkAll = function(element, on) {
		...
		div.find('input').each(function(i){
			if($(this).is(':checked') != on) {
				...
				var playerName = $(this).parent().text().substr(1);		// parent() : le label. On enlève l'espace insécable avant le nom
				if(on)
	...
	this.clickNoDrag = function(elt, handler) {
		elt.mousedown( function() { $(this).data('pos', $(this).offset()) } );
		elt.mouseup( handler );
	}

	this.toggleDropList = function() {
		// $(this) est l'élément h3
		...
		if(arrow == '▼') {
			$(this).siblings('.isoMapDrop').slideDown('fast');
			$(this).text('▲'+title+'▲');
		}
		else {
			$(this).siblings('.isoMapDrop').slideUp('fast');
			$(this).text('▼'+title+'▼');
		}
	}
	...
	this.enemyList = function() {
		...
		html += IsoMap.langStrings.importFrom+' <a href="http://'+IsoMap.lang+'.ika-world.com/search.php?view=player_details&land='+IsoMap.lang+'&welt='+IsoMap.server+'" target="_blank">ika-world</a><br/>';
		return html;
	}
	...
	this.updateAllianceData = function() {
		...
		if(!unsafeWindow.ikariam.model.hasAlly || installation) {
			...
			var allyName = $('table.ally_info_table td:eq(1)').attr('title');
			if(allyName) {
			...
			var allianceDiv = $('#tab_diplomacyAlly, #embassy');
			if(!allianceDiv.length)
				return;
			var memberList = $('table#allyMemberList', allianceDiv);
...
//			var columns = $('table#memberList tbody tr:eq(0) td').length;		// should be 6
			var nameColumn = $('th:not([class]):eq(0)', memberList).prevAll().length;		// =0 without other scripts running.
			$('tbody:eq(0) tr', memberList).each(	// tbody pour ne pas prendre le thead, :eq(0) parce que d'autres scripts ajoutent une table dans la table.
				function(index) {
					if(index == 0) return;			// On passe la ligne de légende.
					var playerName = jQuery.trim( $(this).children().filter('td:eq('+nameColumn+')').text() );
					var playerLink = $(this).find('a.message').attr('href');			//	...index.php?view=sendIKMessage&receiverId=1480
			...
			// Save alliance data for other screens.
		}
		...
		if(updatingSequence) {
			var nextPage = $('span.page_bar ~ a', allianceDiv).attr('href');
	...
	/** $(this) désigne l'une des checkboxes correspondant aux joueurs.
	 */
	this.togglePlayer = function() {
		var playerName = $(this).parent().text().substr(1);		// parent() : le label. On enlève l'espace insécable avant le nom
		var isAlly = ($(this).closest('div').attr('id').substr(0, 4) == 'ally');
	 ...
	 *  sinon on va le chercher avec $(this), qui peut être l'un des label de joueurs ou un lien [aucun] ou [tous].
	 */
	this.highlightPlayer = function(playerName) {
		if(typeof playerName != 'string') {
			if($(this).get(0).tagName == 'LABEL') {
				playerName = $(this).text().substr(1);		// on enlève l'espace insécable avant le nom
			}
			else
...
//				centerY = offset.top + container.height()/2;
		...
		$('#isoMap').mousewheel(function(event, delta, deltaX, deltaY) {
