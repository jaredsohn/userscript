// ==UserScript==
// @name           Wall Manager Sidekick (Township)
// @description    Assists Wall Manager with Township posts
// @include        /^http(s)?:\/\/(.*?)\.social\.playrix\.com/(.*?)\.php\?\*/
// @include        https://*.social.playrix.com/*.php?*
// @include        http://*.social.playrix.com/*.php?*
// @include        /^http(s)?:\/\/apps\.facebook\.com\/township\/\*/
// @include        https://apps.facebook.com/township/*
// @include        http://apps.facebook.com/township/*
// @include        http://xsell.6waves.com/*
// @include        http://trk.6waves.com/*
// @include        http://cdn.social.playrix.com/township-6w/*
// @include        http://township-6w.social.playrix.com/resources.php?*
// @include        http://www.facebook.com/pages/FB-Wall-Manager/*
// @license        http://creativecommons.org/licenses/by-nc-nd/3.0/us/
// @require        http://userscripts.org/scripts/source/123889.user.js
// @version        3.0.3.2
// @copyright      Robert Bücheler
// ==/UserScript==
...
(function() {
	...
	var version = "3.0.3.2"; //use something like 1.2.3.4 where 1 is the version, 2 is the major revision, 3 is the minor revision, and 4 is a bug fix
	var appID="128190763881372"; //find your appID on facebook or use google
	var scriptID="159393"; //the id of your script on userscripts.org
	var appName="Township"; //the text name of the app/game you are collecting for
	var appIcon="http://photos-a.ak.fbcdn.net/photos-ak-snc7/v43/192/128190763881372/app_2_128190763881372_1911.gif"; //example:"http://photos-d.ak.fbcdn.net/photos-ak-snc1/v85006/71/100333333405439/app_2_100333333405439_3507.gif"
	...
	function dock(){
		...
		var materials = [
			...
			{name:"coins", id:"share_coins", event:"get"},
			{name:"materials", id:"res_ask", event:"send"},
		...
		var sendWords = ["needs","send","benötigt","besoin","bisogno","could sure use some","want to get","You'll get a","envíale","envoie-leur"];
			...
			section_main:{type:"section",label:appName+" ("+version+")",kids:{
				...
				sep_GET:{type:'separator',label:'Get Items',kids:{
					coins:{type:"checkbox",label:"Coins"},
 					materials:{type:'checkbox',label:'Materials'},
				...
				sep_SEND:{type:"separator",label:"Send Items",kids:{
 					sendmaterials:{type:'checkbox',label:'Materials'},
	...
	function read(){
		...
		*************************************************** */
		...
		//already claimed
		if (text.match(/((Y|y)ou('ve)? already (responded|claimed|used))/)) statusCode=-6; 
		...
		//generic fails
		else if (text.match(/(can't claim this reward|Only friends can help each other)/))statusCode=-1;
		...
		//accepts
		else if (text.match(/(Gift Claimed Successfully|You just got|You got some materials|successfully sent)/)) statusCode=1;
		else if (text.match(/(Great! You received a bonus|You got some materials|successfully sent)/)) statusCode=1;
