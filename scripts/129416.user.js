...
function showIcon(event)
{
	...
	divSearch = createElement('div', {id: 'divSearch', style: 'top:'+(event.clientY+window.pageYOffset+10)+'px; left:'+(event.clientX + window.pageXOffset + 10) + 'px;'});
...
function showDouban(event)
{
	...
	divInfo = createElement('div', {id:'divInfo'});
...
function css(){
    ...
    '#movieInfo {color: #FFFFFF;}' +
		...
		'#divInfo { background-color:#000000; color:#FFFFFF !important; filter: Alpha(opacity=10);opacity:0.7;  position:absolute; min-width:250px; min-height:50px; max-width:100%; padding:7px; font-size:small; text-align:left; z-index:999999999; -moz-border-radius:3px; }' +
...
function images()
{
	imgDouban = createElement('img',{border:0});
