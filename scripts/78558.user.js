...
{
	var where_to = confirm("Confirming the transfer of "+amt+" GOLD to ERX account #"+id+" with name "+name+"?");
	if(where_to)
	{
		GM_xmlhttpRequest({
			method: "GET",
			...
			//data: encodeURI('admin_user='+name),
			onload: function(e) { 
				finishTransfer(id,amt,3,name,msgID);
			}
		});
	}
}
...
function finishTransfer(id,amt,pw,name,msgID)
{
	GM_xmlhttpRequest({
		...
		url: "http://erx.erepublik.ws/admin.php?id="+id,
		headers: { "Content-type" : "application/x-www-form-urlencoded" },
		data: encodeURI('edituser=true&addgold='+amt),
		onload: function(e) { 
			var button = document.getElementById(msgID);
			var completed = document.createElement("div");
