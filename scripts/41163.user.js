// ==UserScript==
// @name          Travian: Attack builder .RS add
// @description   Attack builder - m4rtini (m4rtini89@gmail.com) updated by by bluelovers
// @author        updated by by bluelovers 
...
switch(sLang)
{
	...
	case "ae": ///by Dream1 & منصور العتيبي
...
case "rs":
default:
		text = ["Ипрашити гузе =)","Неважећа врста напада!","Нема координата!","нема довољно трупа! (Trупа #","није унесена трупа","Почиње","Вероватно лоше координате","Немате извиђача","Завршено","Raspored таласа :","Додај нови талас","Ресет","Распоред напада:","Врста напада:","Нормалан","Појачање","Пљачка","Извиђач Рес/Трупе","Извиђач Одб/Трупе","Координате","Испраши гузе! =)","Време доласка","Долазак у:","Намести временски интервал","Број напада тог специфичног таласа","помоћу '|' нпр: 0,0|1,1","Грешка:","Не могу до активног села. Претпостављам налог са једним селом","Време доласка намештено у:","Време између таласа у мс: (стартно 500) "];

		cataText = ["Означи мету =)","Насумично","Дрвосеча","Рудник глине","Рудник гвожђа","Њива","Пилана","Циглана","Ливница","Млин","Пекара","Складиште","Силос","Ковачница оружја","Ковачница оклопа","Витешка арена","Главна зграда","Место окупљања","Пијаца","Амбасада","Касарна","Штала","Радионица","Академија","Општина","Резиденција","Палата","Трговачки центар","Велика касарна","Велика штала","Дворац хероја","Велико складиште","Велики силос","Светско чудо","Ризница"];
		break;

	case "hk":
	case "tw": //credit to chihsun for the translation.
			text = ["兵力調動中","無效的攻擊類型!","未指定攻擊目標!","沒有足夠的軍隊! (軍隊#","沒有設定攻擊軍隊","正在派遣軍隊","錯誤的攻擊目標","沒有偵察軍種","完成派兵","攻擊設定","新增攻擊","還原設定","攻擊設定:","攻擊類型:","正常攻擊","增援","搶奪攻擊","偵察資源/軍隊","偵察防禦/軍隊","攻擊目標","立即派遣軍隊","軍隊到達時間","到達目標於:","設定攻擊到達時間","攻擊的波數","用 <span style='color: red; font-weight: bold'>" + targetSplit + "</span> 分隔多村、 <span style='color: red; font-weight: bold'>" + cordsSplit + "</span> 分隔坐標，如 0" + cordsSplit + "0" + targetSplit + "1" + cordsSplit + "1","錯誤:","無法取得目標村莊的資訊","軍隊到達時間:","波次延遲(毫秒):"];
...
function fixcords(c, r) {

	c = c.replace(')', '),');

	var ca1 = c.split(targetSplit);
	var ca2 = new Array();
	...
	for (var i=0; i < ca1.length; i++) {
		ca2[i] = splitCords(ca1[i]);

		if (!isNaN(ca2[i][0]) && !isNaN(ca2[i][1])) {
			ca1[i] = ca2[i][0] + cordsSplit + ca2[i][1];
		}
	}

	return (r ? ca1.join(targetSplit) : ca1);
}

function splitCords(c) {
	c.match(/(\-?[0-9]{1,3})\s?\|\s?(\-?[0-9]{1,3})/ig);
...
function getconfcords() {
	var c = document.getElementById('cords').value;
	c = fixcords(c);

	document.getElementById('cords').value = c.join(targetSplit);
...
//
//		cords = fixcords(cords);
//
//		document.getElementById('cords').value = cords.join(targetSplit);
...
//		var postvar = 'b=1&t1='+ troops[1][wavesSent] +'&t4='+ troops[4][wavesSent] +'&t7='+ troops[7][wavesSent] +'&t9='+ troops[9][wavesSent] +'&t2='+ troops[2][wavesSent] +'&t5='+ troops[5][wavesSent] +'&t8='+ troops[8][wavesSent] + '&t10=0' +'&t11='+ troops[10][wavesSent] +'&t3='+ troops[3][wavesSent] +'&t6='+ troops[6][wavesSent] +'&c='+ c +'&dname=&x='+xcord+'&y='+ycord+'&s1=ok';
		...
		var postvar = "id=39&c=" +c+ "&kid=" +coordsXYToZ(xcord, ycord);
...
function post3 (url, data) {
	GM_xmlhttpRequest({
		...
		url: url,
		headers:{'Content-type':'application/x-www-form-urlencoded'},
		data:encodeURI(data + "&a=" + getRandom(10000, 99999)),
...
attackInterface.innerHTML = '<fieldset><legend>'+text[12]+'</legend><div id=start>' +
	...
	'</tr><tr><td colspan="3">'+
		'<textarea name="cords" id="cords" class="fm" style="width: 98%">' + getCords() + '</textarea>' +
...
myimbabutton.addEventListener("click", function () {
...
function abort()
{
	setTimeout(function(){realAbort()},500 + timebetweenwaves);
}
...
//	var tempPostvar = 'b=1&t1=' + troops[1][tempWaveNumber] + '&t4=' + troops[4][tempWaveNumber] + '&t7='+ troops[7][tempWaveNumber] +'&t9='+ troops[9][tempWaveNumber] +'&t2='+ troops[2][tempWaveNumber] +'&t5='+ troops[5][tempWaveNumber] +'&t8='+ troops[8][tempWaveNumber] +'&t10='+ troops[11][tempWaveNumber] +'&t3='+ troops[3][tempWaveNumber] +'&t6='+ troops[6][tempWaveNumber] +'&c='+ 3 +'&dname=&x='+Xcord+'&y='+Ycord+'&s1=ok';
	...
	var tempPostvar = 'b=1&t1=' + troops[1][tempWaveNumber] + '&t4=' + troops[4][tempWaveNumber] + '&t7='+ troops[7][tempWaveNumber] +'&t9='+ troops[9][tempWaveNumber] +'&t2='+ troops[2][tempWaveNumber] +'&t5='+ troops[5][tempWaveNumber] +'&t8='+ troops[8][tempWaveNumber] + '&t10=0&t11=' + troops[10][tempWaveNumber] +'&t3='+ troops[3][tempWaveNumber] +'&t6='+ troops[6][tempWaveNumber] +'&c='+ 3 +'&dname=&x='+Xcord+'&y='+Ycord+'&s1=ok';
...
//	debug('http://' + document.domain + "/a2b.php?"+did, 1);
...
function debug (msg, br) {
	...
	countDiv = document.getElementById('count');
	countDiv.innerHTML = countDiv.innerHTML + (countDiv.innerHTML && br ? '<br>' : '') + '<span style="">' + msg + '</span>';
}
