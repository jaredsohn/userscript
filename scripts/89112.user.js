...
(function(){
	...
	var langDB={
		...
		"de" : {
			...
			"wall":"Wall",  
			"spy":"Spionage",  
			"popinfo":"Bevölkerungs-Information",  
			"farmhas":"Max. Plätze",  
			"bldinfo":"Benutzt für Gebäude",  
			"trpinfo":"Benötigt für gescoutete Truppen",  
			"farmfree":"Kann mom. maximal für Tuppen verwendet werden",  
			"resinfo":"Ressourcen-Information",  
			"hidefull":"Produktionszeit:",  
			"storagefull":"Gesamtspeicherplatz (xxx) - versteckt (yyy) Produktionszeit:",  
			"lastactive":"Zuletzt aktiv: ",  
			"now":"Aktiv",  
			"justleft":"Vor Sekunden gefarmt",  
			"unknown":"Unbekannt; Speicher voll",  
			"time":" Std zuvor/nicht komplett gefarmt",  
			"nextrun":"Gleiche Truppen, max. Kapazität in: ~ xxx Std.",  
			"overkill":" Kapazitäts-Overkill",  
			"lootprtime":"Momentane Beute entspricht ~ xxx Std. Produktion",  
			"perhour":"pro Std.",  
			"hour":"Std.",  
			"loot":"Beute:",  
	...
	var canCarry;
	table = getNextElement(table,'table');
	if(table.getElementsByTagName('tr')[0] != null) {
		if(table.getElementsByTagName('th')[0] != null) {
			if(grabText(table.getElementsByTagName('th')[0],1) == langDB[lang]["loot"]){
				canCarry = grabText(table.getElementsByTagName('td')[1],2).split("/");
				for(var i = 0 ; i < 2 ; i++) {	
					if(canCarry[i].indexOf(".") > 0) {
