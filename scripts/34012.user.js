// ==UserScript==
// @name          youscrobble
// @namespace     http://crosbow.peterpedia.org/lastfm/
// @description   scrobbles the currently watching youtube video to last.fm
// @include       http://*.youtube.tld/watch?v=*
// @include       http://youtube.tld/watch?v=*
// @require       http://usocheckup.dune.net/34012.js?method=update
// ==/UserScript==
...
function us_addButton() {
    ...
    var Ausdruck = /'SWF_ARGS':.+ "length_seconds": "(.+?)",/g;
    var scriptsCopy = document.getElementsByTagName('script');
    for (var i = 0; i < scriptsCopy.length; i++) {
	var x = scriptsCopy[i].innerHTML;
    	if (Ausdruck.exec(x)) {
       		secs = RegExp.$1;
		break;
    	}   
    ...
    var i = document.getElementById('masthead-nav-user');
    var button = createIdElement("span","us_scrobblebutton");
    var classatr = document.createAttribute("class");

    button.innerHTML = '<img id="us_icon_small" style="margin: 0px -10px -3px 0px;" src="'+us_icon()+'" alt="audioscrobbler.com" /><a>Scrobble</a>';

    i.insertBefore(button, document.getElementById("masthead-nav-user").firstChild);
...
function us_help() {
         ...
         'alt="last.fm" /></a><br /><br />CTRL+Click the little icon to switch its colour!<br /><br />Visit <a href="http://userscripts.org/scripts/show/34012" title="userscripts page">this page</a> for some more information.</p>';
