// ==UserScript==
// @author         balázs9876
// @name           Ikariam Shortcuts v1.4(HUN)
// @namespace      Ikariam
// @description    Ikariam Keyboard Shortcuts Script v1.4
// @version        v1.4
  ...
  if (!e.altKey && !e.shiftKey) {
	if (isNaN(key)) {
      switch (String.fromCharCode(e.keyCode)) {
        ...
        case 'V': return location.href = getBuildingUrl("townHall");
        case 'K': return location.href = getBuildingUrl("port");
        ...
        case 'P': return location.href = getBuildingUrl("palace");
        case 'F': return location.href = getBuildingUrl("wall");
        case 'H': return location.href = getBuildingUrl("palaceColony");
        case 'R': return location.href = getBuildingUrl("warehouse");
        case 'E': return location.href = getResourceLink('resource');
        case 'L': return location.href = getResourceLink('tradegood');
        case 'U': return location.href = getAdvisorUrl('advDiplomacy');
      }
    } else {
	  var n = (key || 14) - 1;
	  var id = cityIDs()[n];
...
function getResourceLink(resType)	{
	var str = $X('id("changeCityForm")//ul/li[@class="viewIsland"]/a') + '';
	str = str.replace(/view=island/, 'view=' + resType).replace(/&id=/, '&type=' + resType + '&id=');		
	return str;
}
...
function getAdvisorUrl(n)	{
	return $X('id("advisors")/ul/li[@id="' + n + '"]/a');
}
