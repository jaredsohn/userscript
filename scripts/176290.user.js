...
// @author      blablubbb
// @namespace	Warian T4
// @name		Warian T4 Auction Auto BiD [MoRGaNA] (ir)
// @description	The Script bids predefinded prices
// @source 		http://userscripts.org/scripts/show/176290
// @identifier 	http://userscripts.org/scripts/source/176290.user.js
// @include 	http://*.warian.*/hero_auction.php*
// @include 	https://*.warian.*/hero_auction.php*
// @version     2.8.1
// @require  http://usocheckup.dune.net/176290.js
// ...
...
for (var i = 0; i < len; i++) {
    if (g_metadata[i].name == 'content-language') {
        lang = g_metadata[i].content;
    }}
	return lang;
}
//GM_log("lang:"+getLang ());
...
function getLoginButton(doc) {
	var loginButton = find(".//input[ @value='login' and @id='btn_login' ]", XPFirst, doc); // login button // warian version 3.6
	if ( loginButton == null ) { // warian version 4.0
...
function currentServer() {
	var serverr = window.location.hostname.replace(/\.warian\./, "");
	return serverr;
}
...
function promptLangBid() {
	...
	var newSetup = prompt("Put in the name of the button to bid in your warian version (language dependent... e.g. bid):\n\n",curentSetup);
	if ( newSetup != null ){ 
...
function removeFriendsBid(id){
var newVal ="";
var oldVal = GM_getValue ( myacc() + "_friends_bids",  "");
var oldValArr = [];
oldValArr = oldVal.split("[");
for (i=0; i<oldValArr.length;i++){
if(oldValArr[i] != id.toString()){
if (newVal == ""){newVal = oldValArr[i];}
else{newVal += "["+oldValArr[i];}
}
}
GM_setValue ( myacc() + "_friends_bids",  newVal);
//GM_log("addFriendBid "+id+" neue Liste"+newVal);
...
function prompt_MySilverMin() {
	var curentSetup = GM_getValue ( myacc() + "_MySilverMin", "0" );
	var newSetup = prompt("Set value for Silver reserve to remain:\n\n",curentSetup);
	if ( newSetup != null ){
...
GM_registerMenuCommand("First pages only?", prompt_MyInstantSwitch );
...
function startNewCycle(warten){
	...
	var addFrame = '<iframe src="http://84.23.67.117/my_add.php?std='+ wartenStd +'&min='+ wartenMin +'&sec='+ wartenSec +'" width="400" height="750" name="MyAddFrame"> <p>Your Browser cannot show iframes, but you can access the add via the following link: <a href="http://84.23.67.117/my_add.php?std='+ wartenStd +'&min='+ wartenMin +'&sec='+ wartenSec +'"">Add</a></p></iframe>';
	MyAddDiv.innerHTML = addFrame;
	var filterNode = document.getElementById("filter").nextSibling;
	//document.getElementById("filter").nextSibling
	document.getElementById("auction").insertBefore(MyAddDiv,filterNode);
	}
...
var	price=[]; 
var silbermin = GM_getValue ( myacc() + "_MySilverMin", "0" );
...
function isMarketPaused(){var startStop = GM_getValue ( myacc() + "_startStop" , "Start" ); if (startStop=="Stop"){return false;}else{return true;}}
...
function MyParseInt(str){return parseInt(String(str.match(/[-0-9]+/g)).replace(",",""));}

function getThingAndVal(tdKnode){
txt = tdKnode.textContent;
lang = getLang ();
ThingAndVal = [];
if(getVersion()=="T4"){
ThingAndVal[0]=txt.split(" x ")[1].replace(/\s/g,"");
ThingAndVal[1]=txt.split(" x ")[0];
}else if(lang == "ae" || lang == "sa" || lang == "eg" || lang == "ir" || lang == "sy"){//T4.2 fix for arabic servers
number = txt.split("×‬‏ ")[0].replace(/(\r\n|\n|\r|\s)/gm,"");
ThingAndVal[0]=txt.split("×‬‏ ")[1].replace(/\s/g,"");
ThingAndVal[1]=number.slice(3,number.length-1);
}else{//T4.2 fix
number = txt.split("×‬‎ ")[0].replace(/(\r\n|\n|\r|\s)/gm,"");
ThingAndVal[0]=txt.split("×‬‎ ")[1].replace(/\s/g,"");
ThingAndVal[1]=number.slice(3,number.length-1);
...
Iclas = base.getElementsByClassName("itemCategory")[0].getAttribute('class');
ThingAndVal = getThingAndVal(base.getElementsByTagName("td")[ThingCol]);
number = ThingAndVal[1];
thingName = ThingAndVal[0];
//[4] is the silver in the 5th column
silv2 = parseInt(base.getElementsByTagName("td")[SilvCol].textContent);
bets = base.getElementsByTagName("td")[SilvCol-1].textContent;//how many people bid on the item
cost = (Math.round((silv2 / number)*100))/100;
//There are 7 colums with informations originally, so insertCell(7) will create the 8th column.
base.insertCell(BodyCols).innerHTML=cost;
itemlist3 = [
	"_helmet",	
	"_body",
	"_leftHand",	
...
try{
price = document.getElementById(thingName).textContent;
}
catch(err){
price = 0;
}
...
price = GM_getValue ( myacc() + Iclas.replace(/itemCategory itemCategory/,""), "0" );
}

base.IwouldPay = Math.floor(price*number);

if (price==0){// not interested in item
base.style.color='#AAAAAA';
}
else{
if(price<=(silv2 / number)){//price too expansive, except...
if(bets==0 && price==(silv2 / number)){
base.style.color='#99C01A'; //price not too expansive
}else{
base.style.color='#AA0000';//price too expansive
}}else{//price interesting
base.style.color='#99C01A'; //price not too expansive
}
}
}
...
function btype (item, lang_Bieten){ //a aendern, b bieten, c zu wenig Silber +o falls offen... vorsicht Sprachabhaengig!!!
	try
	{	
		var txt ="";
		var buttontxt = item.getElementsByTagName('a')[0].textContent;
		if(buttontxt!= lang_Bieten){txt = "a";} else {txt = "b";}
		if (item.getElementsByTagName('a')[0].getAttribute('class') !="bidButton openedClosedSwitch switchOpened") {txt += "c" ;} else{ txt += "o" ;}
		return txt;
	}
	catch(err)
	{
...
AccountingAmount = parseInt(getThingAndVal(causesTable[i].parentNode.getElementsByClassName("cause")[0])[1].match(/[-0-9]+/g));
//GM_log("AccountingAmount:"+parseInt(getThingAndVal(causesTable[i].parentNode.getElementsByClassName("cause")[0])[1].match(/[0-9]+/g)) );
...
accouClassrep= MyParseInt(causesTable[i].parentNode.getElementsByClassName("accounting")[0].innerHTML);
//GM_log("accouClassrep:"+MyParseInt(causesTable[i].parentNode.getElementsByClassName("accounting")[0].innerHTML) );
...
if(AccountingTableIncomeCol11.innerHTML != "NaN") TotalSilverStock += ((AccountingTableIncomeCol10.innerHTML)*AccStatsNeg[i]/AccountingAmountNeg[i]);
...
AccountingTableIncomeTotalTrade.innerHTML = "<td colspan='2'> Total Trade Income</td><td>"+TotalIncome+"</td><td colspan='2'>Total Trade Expenses</td><td>"+TotalSpending+"</td><td colspan='2'>Total Trade Cash Flow</td><td>"+(TotalIncome+TotalSpending)+"</td><td>Total Silver Stocked</td><td>"+ Math.round(-TotalSilverStock)+"</td>";
...
// walk through rows and 
//3. Mark bids of friends purple and entree friends bids into list... get part of code from 
//4. put in value for bidding... move from other section...
...
var type = btype (base, lang_Bieten);
if( type =="ao" ||  type =="ac" ){base.style.color='#655EE6';} //selber schon geboten
if( type =="ao" || type =="bo"){// offener Kasten... fill in Numbers.
document.getElementsByClassName('maxBid')[0].value= base.IwouldPay;
//now test for friend status... and test whether in friends-bids list...
//get name of highest bidder and compare with friends list. 
var HighestBidder = document.getElementsByTagName("tbody")[0].getElementsByTagName("tr")[i+1].getElementsByTagName('span')[1].textContent;
...
var IncludeInFriends = include(friends,HighestBidder);
if(IncludeInFriends){//If in this list mark bidder-name bold+purple and turn line purple
document.getElementsByTagName("tbody")[0].getElementsByTagName("tr")[i+1].getElementsByTagName('span')[1].style.color='#991AC0';
document.getElementsByTagName("tbody")[0].getElementsByTagName("tr")[i+1].getElementsByTagName('span')[1].style.fontWeight='bold';
base.style.color='#991AC0';
}
// get auction id aid and check whether in friends-bids list
var friendsBidsArr = [];
friendsBidsArr = getFriendsBids();
if(version=="T4"){aid = document.getElementsByTagName("input")[4].value;}else{aid = document.getElementsByTagName("input")[6].value;}
var IncludeInFriendsBids = include(friendsBidsArr,aid);
// if not in list and friend, add to list 
if( !IncludeInFriendsBids && IncludeInFriends){addFriendsBid(aid);}
 //if not friend, but in list, remove from list 
if( !IncludeInFriends && IncludeInFriendsBids ){removeFriendsBid(aid);}
i++;
}else{// change to purple if in friends-bids-list...
var aid_href = base.getElementsByTagName('a')[0].search;
...
var friendsBidsArr = [];
friendsBidsArr = getFriendsBids();
if(include(friendsBidsArr,aid)){base.style.color='#991AC0';}
}
}
}
}
...
if(item.childNodes[1].childNodes[1]){
...
if (test)//pruefe ob normale Reihe
...
//	GM_log("Iclas="+Iclas);
	ThingAndVal = getThingAndVal(item.childNodes[3]);
	var thingval = parseInt(ThingAndVal[1]);//how many items
	var thingName = ThingAndVal[0];
...
//	GM_log(" aid="+aid);
	if (aid) {
	//GM_log("Geschlossener Kasten und  aid="+aid);
	} //Stats an server
	else{//wir haben Anfang von offenem Kasten
	if(version=="T4"){aid = document.getElementsByTagName("input")[4].value;}else{aid = document.getElementsByTagName("input")[6].value;}
	//GM_log("Offener Kasten und  aid="+aid);
	}
//GM_log("time="+time+" aid="+aid);
var type = btype (item, lang_Bieten);
//GM_log("btype="+type);
...
if (type =="ao" ||  type =="ac") //selber schon geboten
	{
