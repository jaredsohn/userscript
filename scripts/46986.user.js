...
function refreshTaskList(aTasks) {
  ...
  for(var i = 0; i < aTasks.length; i++) {
    ...
    if(bUseServerTime) {
      ...
      sTime = "<span style='color:#973C05; cursor:pointer;' id='ttq_tasktime_" +i+ "' title='حال میکنی تراوین رو مسخره کن ' ttq_taskid='" +i+ "' >" + sTime + "</span>";
    } else {  //local time
      var oDate = new Date( parseInt(aThisTask[1]) * 1000 );
      var sTime = "<span style='color:black; cursor:pointer;' id='ttq_tasktime_" +i+ "' title='حال میکنی تراوین رو مسخره کن' ttq_taskid='" +i+ "' >" + oDate.toLocaleString() + "</span>";
    }
