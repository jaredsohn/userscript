...
// @namespace	  http://userscripts.org/scripts/show/10577
// @description	On SensibleErection.com, redisplays comments in a more easily interpreted tree, allows expanding / collapsing of threads, and adds inline comment moderation and replies
// @downloadURL https://sites.google.com/site/sensiblefacial/sensiblefacial.user.js
// @match http://sensibleerection.com/*
// @match http://*.sensibleerection.com/*
...
var scriptVersion = '3.5.4.1';
var X11 = navigator.userAgent.indexOf('X11') > -1;
var scriptLoc = 'http://sensiblefacial.googlepages.com/sensiblefacial.user.js';
var arrowRight = "&#9654;";
var arrowDown = "&#9660;";
...
(function () {
	...
	if (location.href.indexOf('sensibleerection.com') < 0) return false;
	...
	with (queueing) {
		className = 'text_12px';
		style.cssText = 'position: fixed; bottom: 0px; left: 0px; padding: 5px; display: none; '
...
//		options.push(['boxShadows','Allow shadows and curves in themes',true]);
	...
	, modReplies = getNodes("/html/body/div[@id='Layer1']/table[@cellpadding!='5']/tbody/tr/td[2]/span[@class='entry_details_text']/a[2]")
	, authors = getNodes("/html/body/div[@id='Layer1']/table[@cellpadding!='5']/tbody/tr/td[2]/span[@class='entry_details_text']/a[1]")
	...
	function showScore(e) {
		...
		onload: function(oXML) {
			var mods = oXML.responseText.substring(oXML.responseText.indexOf('<div class="date_header_text">Comment Moderation</div>'), oXML.responseText.indexOf('<div class="logo_box">'))
	...
	if (settings.scores()) {
		var scores = getNodes("//span[@class='entry_details_text']//*[text()[starts-with(.,'Score:')] and name(.)='STRONG' or name(.)='FONT']");
		if (scores) {
			var tar = scores[0].parentNode.getElementsByTagName('a');
			if (!tar || !tar[1] || !/comment\.php/.test(tar[1].href)) scores.shift();
