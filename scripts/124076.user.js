...
(function() {
	...
	String.prototype.addWord= function(word){
		...
		if (!words.inArray(word))return this + " " + word;
		return this;
	}
		
	String.prototype.removeWord= function(word){
		return this.split(" ").removeByValue(word).join(" ");
	}

	String.prototype.containsWord= function(word){
		return this.split(" ").inArray(word);
	}

	String.prototype.replaceWord= function(word,word2){
		return this.split(" ").replace(word,word2).join(" ");
	}

	String.prototype.toggleWord= function(word){
		var words = this.split(" ");
 		...
 		addToFrame : function(field, i, k, prevSibling) {
			...
			switch(field.type) {
				...
				case 'checkbox':
					...
					), className: 'config_var '+(field.format||'block')+(field.newitem?' newopt'+((field.newitem===true)?"":field.newitem):'')});
			}
			if(field.kids) {
	...
	var fb = {
		...
		get newsFeed(){ return $("home_stream") || $("app_stories") || $("pagelet_wall") || $("pagelet_home_stream"); }, //news feed node
		...
		//searches for ID where only a user's ID will exist, not accepting places where an alias might exist
		getUserId: function(){
			var ret = selectSingleNode(".//form[@class='attachmentSelectForm']/ul/input[@name='targetid']");
			if (!ret) ret = selectSingleNode(".//form[@class='attachmentForm']/input[@name='xhpc_targetid']");

			return (ret)?ret.value:"";
		},

		//searches for user's alias but will accept ID if available, returns the value of getUserId if all else fails
		getUserAlias: function(){
			var ret = selectSingleNode(".//a[@class='headerTinymanName']"); 
			if (!ret) ret = selectSingleNode(".//a[@class='fbxWelcomeBoxName']");
			if (!ret) ret = selectSingleNode(".//img[contains(@class,'headerTinymanPhoto')]/ancestor::a");
			
			if (ret) ret=ret.href.split('/').last();

			//get the user ID in place of an alias since no alias exists
			if (!ret) ret = fb.getUserId();

			if (ret) {if (ret.find("profile.php?")) ret = ret.getUrlParam("id"); else ret = ret.split('?')[0];}
			return (ret)?ret:"";
		},
		...
		getStoryAppIdFromActionLinks: function(storyNode){
			...
			if (!ret) ret = selectSingleNode(".//span[contains(@data-ft,'{\"type\":26}') or contains(@data-ft,'{\"type\":\"26\"}')]/a[contains(@data-hovercard,'page.php?id=')]",{node:storyNode});
			if (ret) ret = ret.getAttribute("data-hovercard").getUrlParam("id") || ret.href.getUrlParam("id");
			return ret;
		},
		...
		getStoryIsMyPost: function(storyNode){
			...
			var thisAlias = fb.getUserAlias();

			var ret;
			if (thisUser) ret = (storyOwner.find(thisUser) || actorAlias.find(thisUser));
			if (!ret && thisAlias) ret = (storyOwner.find(thisAlias) || actorAlias.find(thisAlias));
			return ret;
		},
		...
		getStoryTargetsMe: function(storyNode) {
			...
			var storyTarget = fb.getStoryTargetActorId(storyNode);
			var storyActor = fb.getStoryActorId(storyNode);

			//first return true if the story only targets the parent actor because that means it targets everybody
			if (storyTarget=== storyActor) return true;

			//then return true if the story targets this user
			var ret;
			var thisId = fb.getUserId();
			var thisAlias = fb.getUserAlias();
			if (thisId) ret = storyTarget.find(thisId);
			if (!ret && thisAlias) ret = storyTarget.find(thisAlias);
			return ret;
		},
		...
		isLiked: function(storyNode){
			var unlike = selectSingleNode(".//button[contains(@data-ft,'\"type\":23') or contains(@data-ft,'\"type\":\"23\"') or (@name='unlike')]",{node:storyNode});
			return (unlike!=null);
		},
		...
		getStoryMessage: function(storyNode){//3="media"
			var ret =selectSingleNode(".//span[contains(@data-ft,'\"type\":3') or contains(@data-ft,'\"type\":\"3\"')]",{node:storyNode});
			if (ret) return ret.textContent;
			return "";
		},

		getStoriesByAppId: function(appId,omitTitleProcessed) {
			var stories = selectNodes(".//*[contains(@data-ft,'\"type\":26') or contains(@data-ft,'\"type\":\"26\"')]/a[contains(@href,'application.php?id="+appId+"')]//ancestor::*[starts-with(@id,'stream_story_')"+(omitTitleProcessed?" and (not(@title='processed')) and (not(@title='working'))":"")+"]",{type:7,node:fb.newsFeed});
			if (!stories || !stories.snapshotLength) stories = selectNodes(".//*[contains(@data-ft,'\"type\":26') or contains(@data-ft,'\"type\":\"26\"')]/a[contains(@data-hovercard,'page.php?id="+appId+"')]//ancestor::*[starts-with(@id,'stream_story_')"+(omitTitleProcessed?" and (not(@title='processed')) and (not(@title='working'))":"")+"]",{type:7,node:fb.newsFeed});
			return stories;
			...
			if(newNotes.snapshotLength && newNotes.snapshotItem) for(var i=0;i<newNotes.snapshotLength;i++){
				...
				if (newNote){
					...
					if (val) {
						...
						//add an entry to the enable dynamics block
						GM_config.settings["section_dynamic"]["kids"]["enableDynamic"]["kids"]["dynamic"+appID]=checkBox(newset["name"]+" ("+appID+")",true);
						...
						//add dontsteal for this game
						GM_config.settings["section_filters"]["kids"]["dontstealBlock"]["kids"][appID+"dontsteal"]=checkBox(newset["name"],false);						
	...
	var main = {
		...
		doWhichTestTree : function(post, tests, custom) {
			debug.print("doWhichTestTree()",{level:0});

			var appID = fb.getStoryAppId(post);

			var linkText=fb.getStoryActionLinkText(post).toLowerCase(); 
			var bodyText=fb.getStoryBodyText(post).toLowerCase();
			var linkHref=fb.getStoryActionLink(post).href.toLowerCase();
			var msgText=fb.getStoryMessage(post).toLowerCase();
			...
			function mText(t){if(t!=null) return msgText.find(t.toLowerCase()); else return false;}
			...
			for (var i=0,test;((test=tests[i]) && (w===null));i++) {
				...
				if (ret!="dynamic" && ret!="none" && ret!="exclude") ret=appID+ret; //add appID except to magic words
				test["ret"]=ret;
			    }

			    //part to make dynamic collection tests work only if they are the correct appID
			    if ((custom && (appID==test["appID"] || !test["appID"])) || !custom){
				...
				if (subTests) {
					...
					for (var i2=0,subTest,found=false;((subTest=subTests[i2]) && (!found));i2++) {
						var testX = test[testType].replace('{%1}',subTest);
						...
						found=(test["url"])?(lHref(testX)?true:false):
				  		  ...
				  		  (test["msg"])?(mText(testX)?true:false):
					...
					for (var i2=start,found=false;((!found) && i2<=end);i2++)  {
						...
						//return a found value, replacing %1 with a number
					...
					var testX = test[testType].toLowerCase();

					w=(test["url"])?(lHref(testX)?test["ret"]:w):
					  (test["body"])?(bText(testX)?test["ret"]:w):
					  (test["html"])?(bCode(testX)?test["ret"]:w):
					  (test["link"])?(lText(testX)?test["ret"]:w):
				  	  (test["msg"])?(mText(testX)?test["ret"]:w):
					  (test["either"])?(lText(testX)?test["ret"]:(bText(testX)?test["ret"]:w)):
					w;	
					testX=null;
				...
				w = ((kids && w)?main.doWhichTestTree(post, kids, custom):w) || w; //if kids return null, default to key found above
		...
		which : function(post,appID) {
			...
			var dynamicTests = main.dynamicTests;

			//check user built dynamic tests first if enabled and told to run first
			if (main.opts["dynamic"+appID] && main.opts["dynamicFirst"] && dynamicTests) {
				debug.print("trying dynamicFirst");
				w=main.doWhichTestTree(post,dynamicTests,true)||"none";
			}

			//process this game's tests if dynamic didn't already get one
			if (w=="none" || !w || w=="") {
				debug.print("trying normal mode");
		...
		setAsAccepted : function(key,w,comment,status) {
			...
			var text=(w=="dynamic")?"Dynamic Grabbed":(((w.find("send")?"Sent ":w.find("wishlist")?"":"Got ") + main.gameData[appID]["accText"][w]) || main.accDefaultText);
		...
		onFrameLoad : function(key,timeout,appID,short){
			...
			try{
				...
				gotItem=((hashStatus>0) || (hashStatus==-6) || (hashStatus==-4) || (hashStatus==-3) || responseLess || (hashStatus==-15 && main.opts["accepton15"]));
			...
			if(main.gameData[appID]["flags"]["requiresTwo"]) {
				if (nopopLink) {
					...
					GM_xmlhttpRequest({
						...
						onerror: function(response) {
							debug.print("onFrameLoad: final stage: error returned",{level:3});
					...
					debug.print("onFrameLoad: nopopLink is null and a string was expected",{level:3});
					main.setAsFailed(key, w, null,-16);
					main.oTab.location.href="about:blank";
					if (main.requestsOpen>0) main.requestsOpen--;
					return;
				}
		...
		open2 : function(key,timeout,appID,expectedHref){
			...
			try {
				...
				if (main.oTab.document.title==="Problem loading page"){
					...
					window.setTimeout(function(e){main.open2(key,timeout,appID);},1000);
					return;
				}
				...
				window.setTimeout(function(e){main.onFrameLoad(key,timeStamp()+main.reqTO,appID);},1000);
		...
		open : function(key){
			...
			debug.print("open: targetHref="+targetHref,{level:0});
		...
		doPreOpFilters : function(){
			...
			if (main.opts["hidemyposts"]||main.opts["olderdepth"]) {
				var userID = fb.getUserId();
				var userAlias = fb.getUserAlias();

				if (userID) deleteNodes(".//*[contains(@class,'actorName')]/a[contains(@data-hovercard,'id="+userID+"') or contains(@href,'id="+userID+"')]//ancestor::*[starts-with(@id,'stream_story_')]", {node:fb.newsFeed});
				if (userAlias) deleteNodes(".//*[contains(@class,'actorName')]/a[contains(@href,'/"+userAlias+"')]//ancestor::*[starts-with(@id,'stream_story_')]", {node:fb.newsFeed});
			}
		...
		identifyPost : function(post,appID){
			...
			//stylize links
			if (main.opts["floatRight"+appID]) link.className=link.className.addWord('float_right');
			...
			if (isMyPost) {
				link.setAttribute("id", "item_skip_"+key);
				link.className=link.className.addWord("excluded");
				post.setAttribute("title","processed");
				debug.print("identifyPost: post created by user",{level:0});
				if (main.opts["hidemyposts"]) main.remove(post);
				return;
			}
			...
			if (isStale && main.opts["skipStale"]) {
				link.setAttribute("id", "item_skip_"+key);
				link.className=link.className.addWord("excluded");
				post.setAttribute("title","processed");
				debug.print("identifyPost: post is stale",{level:0});
				return;
			}
			...
			//avoid liked posts
			var isLiked = fb.isLiked(post);
			if(debugThis) debug.print("is Liked:"+isLiked);
			if (isLiked && main.opts["skipLiked"]) {
				debug.print("identifyPost: post liked by user",{level:0});
				if (main.opts["markLiked"]){
					main.colorCode(link, "done");
					link.textContent = (w=="dynamic")?"Dynamic Grabbed":(((w.find("send")?"Sent ":w.find("wishlist")?"":"Got ") + main.gameData[appID]["accText"][w]) || main.accDefaultText);
				}
				link.className=link.className.addWord("excluded");
 				link.setAttribute("id", "item_skip_"+key);
				post.setAttribute("title","processed");
			...
			if (w=="none" || w=="exclude") {
				post.setAttribute("title","processed");
				link.setAttribute("id", "item_skip_"+key);
				if(w=="exclude")link.className=link.className.addWord("excluded");
				debug.print("identifyPost: post returned 'none' or 'exclude'",{level:0});
				return;
			}
			...
			if (!alreadyRead){
				...
				link.className=link.className.addWord("identified");
			}
				
			//debug recognition
			if (main.opts["debugrecog"]) link.textContent = (w=="dynamic")?"Dynamic Grab":(w.find("send")?"Send ":w.find("wishlist")?"":"Get ") + main.gameData[appID]["accText"][w];
			...
			if (isW2W && !isPostForMe && main.opts[appID+"dontsteal"]) {
				link.className=link.className.addWord("excluded");
				link.setAttribute("id", "item_skip_"+key);
				post.setAttribute("title","processed");
				debug.print("identifyPost: post protected by dontSteal flag",{level:0});
				return;
			}
			...
			if (!main.acc[w].inArray(key)) {
				...
				if(main.failed[w].inArray(key)) {
					...
					if ((w=="dynamic") || main.opts[w] || (w.startsWith(appID+"send") && main.opts[appID+"sendall"]) ) {
		...
		processPosts : function(){ //return;
			...
			if (main.opts['displaymode']=="1") {
				window.setTimeout(function(e){
					...
					} else debug.print("processPosts: post is null",{level:1});
				},1000);
			}
			else {
				window.setTimeout(function(e){
	...
	if($(main.navIDnf) || $("pagelet_navigation") || $("sideNav")) { 
		...
		GM_config.init("<img src='"+imgs.logo+"'> v"+version, {
			section_main:section("Wall Manager Info",{
				MainMessageCenter:separator("Documentation - Messages - Help",null,{
					...
					//MainFBGroup:anchor("View/Join FB Support Group","http://www.facebook.com/groups/192230914189236/"),
				},true),
			}),
			...
			section_filters:section("Feed Filter Options",{			
				ignorefeeds:optionBlock("Don't Process On",{			
					...
					ignorefriendslist:checkBox("Friends Lists (sk=fl or lists.php)"),
				...
				hidefeedimages:optionBlock("Remove Feed Parts",{
					dropfeedimages:checkBox("Bonus Images (not suggested for some games)"),
					dropprofimages:checkBox("User Thumbs"),
					removeBody:checkBox("Feed Body"),
					filterComments:checkBox("Comments (may cause auto-like to fail)"),
				},true),

				filters:optionBlock("Hide By Type",{
					hidemyposts:checkBox("My Posts"),
					filterSkipped:checkBox("Unwanted Items"),
					removedone:checkBox("Finished Items"),
					filterUnsupported:checkBox("Unsupported Games (buggy)"),
					filterWishlists:checkBox("Don't Hide Known Wish Lists"),
				}),
				...
				dontstealBlock:optionBlock("Don't take W2W posts not for me",{},false),


				likedOpts:optionBlock("'Like' Related Options",{
					skipLiked:checkBox("Skip Liked"),
					markLiked:checkBox("Mark Liked As Accepted (must check Skip Liked)"),
				}),

				skipStale:checkBox("Skip Posts Older Than 24 Hours"),
				blockads:checkBox("Block Ads? (not updated in a long time)"),
			}),
			...
			section_basicopts:section("Basic Tech Options",{						
				...
				gameSpecific:optionBlock("Game-specific",{
				...
				styling:optionBlock("Styling",{
					...
					accepton15:checkBox("Mark posts accepted when 'Unrecognized Response' is returned?"),
				}),

				autoLikeBlock:optionBlock("Auto-Like",{
					autolike:checkBox("Auto \"like\" Accepted Posts?"),
					likeonlysend:checkBox("But \"like\" only \"send\" posts"),
				}),

				
				interruptions:optionBlock("Interruptions",{
					inputtimeoutenable:checkBox("Auto-pause When Typing"),
					inputtimeout:{label:"Pause Length (seconds)", type:"float", "default":30},
				}),

				timeouts:optionBlock("Time-outs",{
					reqtimeout:inputBox("Item Acceptance Page Timeout (seconds)",45),
					failontimeout:checkBox("Mark Timeout as Failure (default: retry indefinitely)"),
				}),

				History:optionBlock("History",{
				...
				debugOpts:optionBlock("Messaging and Debugging",{

					newuserhints:checkBox("Enable Message (important messages)",true),
					status:checkBox("Enable Status (processed counter)",true),
					debug:checkBox("Enable Debug"),
					debugLevel:{
					...
					debugrecog:checkBox("Enable Recognition (link text changes)",true),
				}),
			...
			section_dynamic:section("Dynamic Collection",{			
				enableDynamic:optionBlock("Enable by Game",{},true),			
				dynamicFirst:checkBox("Run Dynamic Tests BEFORE Sidekick Tests (unchecked: Sidekicks test first)",true),
				dynamicInput:{
					label:"Custom Tests",
					title:"Build your own custom tests for your favorite games. Be sure to check out the tutorial via the link below.",
					type:"textarea",
					cols:80,
					rows:10,
					"default":"{\"appID\":\"266989143414\",\"link\":\"play pioneer trail\",\"ret\":\"exclude\",\"note\":\"this is an example\"}"
				},

				dynamicTutorial:anchor("Tutorial","http://fbwm.wikia.com/wiki/Dynamic_Tutorial"),
		...
		feedInt = window.setInterval(function(e){
			if (fb.newsFeed && !($("status_WM"))){
				fb.newsFeed.parentNode.insertBefore(main.create("div", {id:"status_WM",textContent:"", onclick:function(e){
	...
	window.addEventListener("beforeunload", function(e) {
		...
		if (refto)window.clearTimeout(refto);
		if (feedInt)window.clearInterval(feedInt);
	...
	/*var newGroup = main.getValue("offerFBGroup",true);
	if(newGroup != false) {
		main.message("A new FB group has been created to assist with support.<br><br>"+
			"<a href=\"http://www.facebook.com/groups/192230914189236/\" target=\"_blank\">Click here</a> to ask to join this hidden group.<br><br>"+
			"Get all of your questions about WM answered by the people who make WM and its sidekicks, and by people who have used WM since its beginning."
		);
		main.setValue("offerFBGroup",false);
	}*/
	...
	var dynInput = main.opts["dynamicInput"],dynOutput;
	if (dynInput=="{'appID':'266989143414','link':'play pioneer trail','ret':'exclude','note':'this is an example'}") {
		main.opts["dynamicInput"]="";
		GM_config.set("dynamicInput", "");
	} else {
		dynInput = "["+dynInput+"]";
		debug.print("dynamic input: "+ dynInput);
		if (main.opts["dynamicInput"]) try{
			main.dynamicTests = JSON.parse(dynInput);
			dynOutput = JSON.stringify(main.dynamicTests);
			debug.print("dynamic output: "+ dynOutput);
		} catch(e){debug.print(''+e,{level:3})}
		if (!dynOutput && main.opts["dynamicInput"]) main.message("Your Dynamic Grabber console contains code that will not compile. Your code is: \n"+dynInput);
	}
	...
	//check user Id
	debug.print("user id: "+fb.getUserId());
	debug.print("user alias: "+fb.getUserAlias());
