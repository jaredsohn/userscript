...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('KOC Tools', eventHideShow, mouseMainTab);
  kocWatchdog ();
  WideScreen.init ();
...
Tabs.Search = {
  ...
  getUserLogin : function (uid){
     var t = Tabs.Search;
     var s = Tabs.AllianceList;
	 s.fetchPlayerLastLogin (uid, function (r) {t.GetUserLogin2(r)});
	 return t.userLastLogin;
   },
		
  GetUserLogin2 : function (rslt){
     var t = Tabs.Search;
	 t.userLastLogin = ""
	 if (!rslt.ok){
       t.userLastLogin = rslt.errorMsg;
       return;
     }
     ...
     if (lastLogin) {
       t.userLastLogin = lastLogin;
     } else {
       t.userLastLogin = 'No login data found:';
     }
   },
...
Tabs.Marchesa = {
  ...
  init : function (div){
	...
	unsafeWindow.DelT = t.cancelQueueElement;
	t.traderState = {
       running: false,
        };
    t.readTraderState();
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubA type=submit value=Transport></td>\
          <TD><INPUT class=pbSubtab ID=ptmrchSubM type=submit value=Reassign></td>\
 ...
 //***   Transport SubTab  ***//
  ...
  e_tradeRoutes: function(){
	  ...
	  setTimeout(function(){ t.e_tradeRoutes();}, Options.transportinterval*60);
  ...
  paintTradeRoutes: function(){
			...
			for (var i = (r.length-1); i>=0; i--) {
				...
				var queueId = i;
				t._addTabTR(queueId,cityname, r[i].target_x, r[i].target_y, r[i].ship_Food, r[i].target_Food, r[i].trade_Food,r[i].ship_Wood, r[i].target_Wood, r[i].trade_Wood,r[i].ship_Stone, r[i].target_Stone, r[i].trade_Stone,r[i].ship_Ore, r[i].target_Ore, r[i].trade_Ore,r[i].ship_Gold, r[i].target_Gold, r[i].trade_Gold);
	        }
	    },

  _addTabTR: function(queueId,cityname, cityX,cityY,ship_Food, target_Food, trade_Food,ship_Wood, target_Wood, trade_Wood,ship_Stone, target_Stone, trade_Stone,ship_Ore, target_Ore, trade_Ore,ship_Gold, target_Gold, trade_Gold){
	 	var t = Tabs.Marchesa;
		 ...
		 document.getElementById('tradecancel_' + queueId).addEventListener('click', function(){
	        t.cancelQueueElementTR(queueId);
	     }, false);
	 },

  _addTabHeaderTR: function() {
	 var t = Tabs.Marchesa;
	     var row = document.getElementById('pbRoutesQueue').insertRow(0);
	     row.vAlign = 'top';
  ...
  cancelQueueElementTR: function(queueId){
		 var t = Tabs.Marchesa;
	     var queueId = parseInt(queueId);
  ...
  e_reassignRoutes: function(){
      ...
      setTimeout(function(){ t.e_reassignRoutes();}, Options.reassigninterval*60);
  ...
  showReassignRoutes: function () {
		...
		//t.popReassignRoutes = new CPopup('pbShowReassign', 0, 0, 1100, 500, true, function() {clearTimeout (1000);});
  ...
  _addTabRE: function(queueId,cityname,target_x,target_y,SendSupplyTroop,SupplyTroop,SendMilitiaman,Militiaman,SendScout,Scout,SendPikeman,Pikeman,SendSwordsman,Swordsman,SendArchers,Archers,SendCavalry,Cavalry,SendHeavyCavalry,HeavyCavalry,SendSupplyWagons,SupplyWagons,SendBallista,Ballista,SendBatteringRam,BatteringRam,SendCatapult,Catapult){
		 ...
		 document.getElementById('tradecancel_' + queueId).addEventListener('click', function(){
	        t.cancelQueueElementRE(queueId);
	     }, false);
	 },

  _addTabHeaderRE: function() {
	 var t = Tabs.Marchesa;
	     var row = document.getElementById('pbRoutesQueue').insertRow(0);
	     row.vAlign = 'top';
	     row.insertCell(0).innerHTML = "ID";
  ...
  cancelQueueElementRE: function(queueId){
	     var t = Tabs.Marchesa;
	     var queueId = parseInt(queueId);
 ...
 /***   Incoming SUBTAB  ***/
      showIncoming : function (){
        ...
        //z += '<TD><INPUT id=TEST2 type=submit value="TEST2"></td>';
         var z = '<TABLE id=pdincoming cellSpacing=10 width=100% height=0% class=pbTab><div class=ptstat>ANGRIFFE AUF DIE BURGE</div>';
  ...
  /***   MARCHES SUBTAB  ***/
    showMarches : function (){
     ...
     for (var c=0; c< Seed.cities.length;c++) {
    		...
    		if (Seed.queue_atkp[cityID].length !=0) m+= '<TR><TD colspan=10 ><div class=ptstat><B>' + cityname +' </b></div></td></tr>';
  		    for (k in Seed.queue_atkp[cityID]){
  				 if (Seed.queue_atkp[cityID].length !=0) {
  				   ...
  				   if (status=="Lagernd")
  				    ...
  				    if (Seed.queue_atkp[cityID][k]["unit1Count"] > 0 && marchstatus == "going") m += '<TD>Versorger: '+ addCommas(Seed.queue_atkp[cityID][k]["unit1Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit1Return"] > 0 && marchstatus == "returning") m += '<TD>Versorger: '+ addCommas(Seed.queue_atkp[cityID][k]["unit1Return"]) +'</td>';
  				    
  				    if (Seed.queue_atkp[cityID][k]["unit2Count"] > 0 && marchstatus == "going") m += '<TD>Miliz: '+ addCommas(Seed.queue_atkp[cityID][k]["unit2Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit2Return"] > 0 && marchstatus == "returning") m += '<TD>Miliz: '+ addCommas(Seed.queue_atkp[cityID][k]["unit2Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit3Count"] > 0 && marchstatus =="going") m += '<TD>Späher: '+ addCommas(Seed.queue_atkp[cityID][k]["unit3Count"])+'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit3Return"] > 0 && marchstatus == "returning") m += '<TD>Späher: '+ addCommas(Seed.queue_atkp[cityID][k]["unit3Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit4Count"] > 0 && marchstatus =="going") m += '<TD>Lanzen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit4Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit4Return"] > 0 && marchstatus == "returning") m += '<TD>Lanzen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit4Return"])+'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit5Count"] > 0 && marchstatus =="going") m += '<TD>Schwerter: '+ addCommas(Seed.queue_atkp[cityID][k]["unit5Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit5Return"] > 0 && marchstatus == "returning") m += '<TD>Schwerter: '+ addCommas(Seed.queue_atkp[cityID][k]["unit5Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit6Count"] > 0 && marchstatus =="going") m += '<TD>Bogen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit6Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit6Return"] > 0 && marchstatus == "returning") m += '<TD>Bogen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit6Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit7Count"] > 0 && marchstatus =="going") m += '<TD>Kav: '+ addCommas(Seed.queue_atkp[cityID][k]["unit7Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit7Return"] > 0 && marchstatus == "returning") m += '<TD>Kav: '+ addCommas(Seed.queue_atkp[cityID][k]["unit7Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit8Count"] > 0 && marchstatus =="going") m += '<TD>S-Kav: '+ addCommas(Seed.queue_atkp[cityID][k]["unit8Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit8Return"] > 0 && marchstatus == "returning") m += '<TD>S-Kav: '+ addCommas(Seed.queue_atkp[cityID][k]["unit8Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit9Count"] > 0 && marchstatus =="going") m += '<TD>Wagen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit9Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit9Return"] > 0 && marchstatus == "returning") m += '<TD>Wagen: '+ addCommas(Seed.queue_atkp[cityID][k]["unit9Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit10Count"] > 0 && marchstatus =="going") m += '<TD>Ballis: '+ addCommas(Seed.queue_atkp[cityID][k]["unit10Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit10Return"] > 0 && marchstatus == "returning") m += '<TD>Ballis: '+ addCommas(Seed.queue_atkp[cityID][k]["unit10Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit11Count"] > 0 && marchstatus =="going") m += '<TD>Ram: '+ addCommas(Seed.queue_atkp[cityID][k]["unit11Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit11Return"] > 0 && marchstatus == "returning") m += '<TD>Ram: '+ addCommas(Seed.queue_atkp[cityID][k]["unit11Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit12Count"] > 0 && marchstatus =="going") m += '<TD>Katas: '+ addCommas(Seed.queue_atkp[cityID][k]["unit12Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit12Return"] > 0 && marchstatus == "returning") m += '<TD>Katas: '+ addCommas(Seed.queue_atkp[cityID][k]["unit12Return"]) +'</td></tr>';
   			  }
  		    }
...
//logit ("enc: "+ inspect (enc, 6, 1));    
    ...
    if (numSlots == 0){
      s += '<BR><CENTER><B>Bei dir sind keine verstärkungs truppen ein Marschiert... :/</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
