// ==UserScript==
//
// @name			kinox.to Movie Iframe
// @namespace		http://userscripts.org/users/spongshga 
// @description		Watch every movie on kinox.to in an iframe. the iframe contains almost only the player Also bypasses some host-side "captcha-like" checks like timers or plugin availability where possible. tested in ff 6.0 and chrome 13.0. Autor to version 9.8: http://userscripts.org/users/181110  
// @version			0.9.9
// @include			http://kinox.to/*
// @include			http://www.kinox.to/*
// @include			http://alpha.kinox.to/* 
// @include			http://beta.kinox.to/* 
...
var hoster = {
	...
	all: function () {
		...
		// cleanup the onbeforeunload() of kinox.to
		this.contentEval(
			"if (window.parent) window.parent.postMessage('rdy=1', 'http://kinox.to');" +
			"if (window.parent) window.parent.postMessage('rdy=1', 'http://alpha.kinox.to');" +
			"if (window.parent) window.parent.postMessage('rdy=1', 'http://beta.kinox.to');"
		);
	},
