...
(function() {
  ...
  var convertText = function(site, text, callback) {
    if(!callback) {
      callback = function() {};
    }
    ...
    GM_xmlhttpRequest({
      method : "GET",
      url : site.url.replace(/%s/, (charsets[site.charset][1])(encodeURIComponent(text))),
      ...
      onload : function(response) {
        ...
        callback(actions[site.action ? site.action : 'replace'](text, result));
      }
    });
  };
  ...
  siteInfo.get(SITEINFO_URL, function(entries) {
    $x('//textarea').forEach(function(textarea) {
      ...
      entries.forEach(function(entry) {
        html.option += <option>{entry.name}</option>;
      });

      p.innerHTML = html.toXMLString();
      ...
      select.addEventListener('change', function(e) {
        ...
        var entry = siteInfo.entries[select.selectedIndex - 1];
        var site = entry.data;

        convertText(site, textarea.value, function(text) {
          textarea.value = text;
        });
      }, false);
    });
  ...
  GM_registerMenuCommand('Textarea Text Converter - clear cache', function() {
