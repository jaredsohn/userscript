...
unsafeWindow.qbrenameentry = function(renameme) {
	var renameto = prompt("Переименуйте Вашу закладку:","");//Rename your Quickmark
	if((renameto == '') || (renameto == '^') || (renameto == ';'))
	{
		alert("Неверное название места! Название не может содержать символов ^ и ;!");//Invalid name! Cannot have ^ or ;!
		return;
	}
...
unsafeWindow.qbaddentry = function() {
	...
	var addname = prompt("Как вы желаете назвать это место?","");//What would you like to name this location
	if((addname.search(/\|/) != -1) || (addname.search(/;/) != -1))
	{
		alert('Неверное название места! Название не может содержать символов | и ;!');//Invalid location name! No | or ;!
		exit;
	}
...
unsafeWindow.qbexport = function() {
	if(unsafeWindow.places != "0"){
		prompt("Скопируйте эти данные! Позже Вы сможете импортировать их!", unsafeWindow.places);//Copy this! You can import it later!
	}
	else {
		alert("У Вас нет закладок для экспорта!");//You have no Quickmarks to export
	}
	unsafeWindow.qbbuildinterior();
}
...
unsafeWindow.qbimport = function() {
	var imported = prompt("Вставьте Ваши закладки сюда. Следите за тем что бы они имели правильный формат! (nameofquickmark|http://pagelocationgoeshere.com;)","");//Paste you Quickmark(s) here. Make sure they follow the correct format
	if(unsafeWindow.places == "0"){
...
unsafeWindow.qbdeleteall = function() {
	if(confirm("Вы абсолютно уверены в том, что хотите удалить все закладки?"))
	{
		unsafeWindow.places = "0";
		window.setTimeout(GM_setValue, 0, "places", "0");		
	}
	unsafeWindow.qbbuildinterior();//Are you absolutely sure you want to delete all your quickmarks
}
...
unsafeWindow.qbmovedown = function(moveitdown) {
	...
	for(i = 0; i < splitplaces.length-1; i++){
		if(splitplaces[i] == moveitdown){
			...
			{
				alert("Эта закладка уже последняя в списке.");//This quickmark is already the last in the list
				break;
			}
		}
	}
...
unsafeWindow.qbbuildinterior = function()
{
	...
	if(unsafeWindow.places != "0")
	{
		...
		for(i = 0; i < splitplaces.length-1; i++){
			...
			if(i>0){
			qbHTML += '<a title="Переместить закладку выше" href="javascript:qbmoveup(\'' + placepiece[0] + '|' + placepiece[1] + ';\');"><img src="http://www.imgboot.com/images/enigmabrand/moveup_1.png" border="0"></a>';//Move Quickmark Up
			}
			else {
			...
			qbHTML += '<a title="Переместить закладку ниже" href="javascript:qbmovedown(\'' + placepiece[0] + '|' + placepiece[1] + ';\');"><img src="http://www.imgboot.com/images/enigmabrand/movedown_1.png" border="0"></a>';//Move Quickmark Down
			}
			else {
			qbHTML += '<img src="http://www.imgboot.com/images/enigmabrand/movetrans.png" height="10" width="10" border="0">';
			}
			qbHTML += '<a title="Удалить закладку" href="javascript:qbdeleteentry(\'' + placepiece[0] + '|' + placepiece[1] + ';\');"><img src="http://imgboot.com/images/enigmabrand/redx.png" border="0"></a>';//Delete Quickmark
			qbHTML += ' <a title="Переименовать закладку" href="javascript:qbrenameentry(\'' + placepiece[0] + '|' + placepiece[1] + ';\');"><img src="http://imgboot.com/images/enigmabrand/rename.png" border="0"></a> - </td>';//Rename Quickmark
	...
	{
	qbHTML += '<div style="border-bottom:1px #F3DDB5 solid; width:376px; height:315px; overflow-y:auto; overflow-x:hidden;"><table style="width:375px; border: none;"><tr><td align="center">Нет сохраненных закладок</td></tr></table></div>';//No places saved
	}

	qbHTML += '<a href="javascript:qbaddentry();">Добавить текущую страницу</a><br>';//Quickmark current page
	qbHTML += '<a href="javascript:qbexport();" title="Экспортировать список"><img src="http://www.imgboot.com/images/enigmabrand/export.gif" border="0"></a>';//Export list
	qbHTML += ' <a href="javascript:qbimport();" title="Импортировать список (Добавить)"><img src="http://www.imgboot.com/images/enigmabrand/import.gif" border="0"></a>';
	qbHTML += ' <a href="javascript:qbdeleteall();" title="Удалить все закладки"><img src="http://www.imgboot.com/images/enigmabrand/redx2_1.png" border="0"></a>';//Delete all Quickmarks
	qbHTML += '</div>';
...
// Add the style
GM_addStyle("#quickbar { width:410px; position:absolute; left:-412px; height:400px; top:15px; z-index: 100; background:url(http://www.imgboot.com/images/enigmabrand/bgnotebarmid.gif); background-repeat:repeat-y; border:1px black solid;}");
