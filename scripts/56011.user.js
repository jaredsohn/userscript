...
// @namespace     http://none
// @description   Conjunto de Utilidades para TribalWars v3.0.1 by JonnyD
// @include       http://br*.tribalwars.*/game.php*
...
function twInitialize()
{
	...
	twLanguage["es"]["units"] = new Array("Pikinier", "Miecznik", "Topornik", "Zwiadowca", "Lekka kawaleria", "Ciężka kawaleria", "Taran", "Katapulta", "Szlachcic");
	...
	twLanguage["es"]["resources"] = new Array ("Drewno", "Glina", "Żelazo", "Pojemność spichlerza");
...
function twInitInterface()
{
	...
	newHTML += '	<img src="http://members.lycos.co.uk/crazymooses/Interface6_07.png" width="67" height="59" alt="">';
	newHTML += '</div>';
	newHTML += '<div style="position:absolute; left:67px; top:59px; width:66px; height:59px;">';
	newHTML += '	<img src="http://members.lycos.co.uk/crazymooses/Interface6_08.png" width="66" height="59" alt="">';
	newHTML += '</div>';
...
{
		var newHTML ="";
		newHTML += "<h2>Wszystkie jednostki wojska</h2>";
		newHTML += "<table id='GlobalArmyInfo' width=100% style='text-align:left;'>"
		newHTML += "	<tr style='text-align:center'><th rowspan=2>Wioski</th><th rowspan=2>Punkty</th><th colspan=9 width:80% align='center'>Wojsko</th></tr>";
		newHTML += "	<tr>";
...
{	
	try{
		...
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["overview"]+'"><img src="'+twGlobalInfo["InterfaceItems"][4]+'" title="Zobacz wioskę" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["map"]+'"><img src="'+twGlobalInfo["InterfaceItems"][5]+'" title="Zobacz mapę" />&nbsp;</a></td>';
		newDiv += '				<td><a href="'+twLinks["twMainMenuArray"]["villages"]+'"><img src="'+twGlobalInfo["InterfaceItems"][8]+'" title="Zobacz wszystkie swoje wioski" />&nbsp;</a></td>';
		newDiv += '				<td><a id="btnShowAllArmy" href="javascript:void(0)"><img src="'+twGlobalInfo["InterfaceItems"][6]+'" title="Zobacz swoje całe wojsko"/>&nbsp;</a></td>'
		newDiv += '			</tr>'
		newDiv += '		</tbody>';
		newDiv += '	</table>';
		newDiv += '</div>';
...
function twReportGenerator(reportInfo)
{
		...
		newReport[5] += '\n&nbsp; Wojsko: [color=darkred]'+attackerLossesUnits+'[/color]/[b]'+attackerTotalUnits+'[/b] ([color=green]'+(attackerTotalUnits-attackerLossesUnits)+'[/color]) Unidad(es) ([b][color=blue]'+(Math.round((attackerLossesUnits*100/attackerTotalUnits)*10)/10)+'%[/color][/b]) Unidad(es)'
		...
		if(parseInt(defenderTotalUnits) != 0)
		{
			...
			newReport[8] += '\n&nbsp;Wojsko: [color=darkred]'+defenderLossesUnits+'[/color]/[b]'+defenderTotalUnits+'[/b] ([color=green]'+(defenderTotalUnits-defenderLossesUnits)+'[/color]) Unidad(es) ([b][color=blue]'+(Math.round((defenderLossesUnits*100/defenderTotalUnits)*10)/10)+'%[/color][/b])';
...
function twMainMenuNavigationBar()
{
	...
	codeNavBar +=				'<ul>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][3]+'">Nowa wiadomość</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][0]+'">Wiadomości otrzymane</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][1]+'">Buz&oacute;n Salida</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][2]+'">Archiwum</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["mails"][4]+'">Zablokuj nadawcę</a></li>';
	codeNavBar +=				'</ul>';
	...
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][0]+'">Graczy</a></li>';
	codeNavBar +=					'<li><a href="'+twLinks["twMainMenuArray"]["ranking"][1]+'">Plemion</a></li>';
	codeNavBar +=				'</ul>';
	codeNavBar +=			'</li>';
	...
	codeNavBar +=							'<li><a href="http://www.tribalwars.pl/" target="_blank">Plemiona Polskie (pl)</a></li>';
...
function twRankingBar()
{
	...
	newHTML += '<div style="position:absolute;top:115px;left:205px;width:700px;">';
		...
		newHTML += '	<div style="float:left;"><img  src="'+twGlobalInfo["InterfaceItems"][9]+'" width="30" height="28" border="0" alt=""  /></div>';
		newHTML += '	<div style="float:left;width:141px;height:22px;margin-top:3px;background-image:url(\''+twGlobalInfo["InterfaceItems"][10]+'\');background-repeat:repeat-x"><div style="margin:3px;color:#804000;font-weight:bold"><a href="http://pl8.plemiona.pl/staemme.php?village=54634&screen=buddies">Przyjaciele</a></div></div>';
		newHTML += '	<div style="float:left;margin-top:3px"><img  src="'+twGlobalInfo["InterfaceItems"][11]+'" width="9" height="22" border="0" alt="" /></div>';
		newHTML += '</div>';
		...
		newHTML += '<div style="float:left;width:185px;margin-left:50px">';
		newHTML += '	<div style="float:left;"><img  src="'+twGlobalInfo["InterfaceItems"][9]+'" width="30" height="28" border="0" alt=""  /></div>';
		newHTML += '	<div style="float:left;width:141px;height:22px;margin-top:3px;background-image:url(\''+twGlobalInfo["InterfaceItems"][10]+'\');background-repeat:repeat-x"><div style="margin:3px;color:#804000;font-weight:bold">edited by Zemloshi</div></div>';
		newHTML += '	<div style="float:left;margin-top:3px"><img  src="'+twGlobalInfo["InterfaceItems"][11]+'" width="9" height="22" border="0" alt="" /></div>';
		newHTML += '</div>';
...
function twBuildingsBar()
{
	...
	newHTML += '	<area id="SmithIcon" title="Kuźnia" shape="poly" alt="" coords="0,251, 9,245, 29,245, 33,241, 40,241, 44,245, 45,252, 41,258, 36,259, 36,267, 29,274, 6,275, 0,268" href="'+twLinks["twMainScreens"][5]+'" />';
...
function TwSetVillageInfo()
{
	try{
		...
		newinfo_village += '			<tr><td>Położenie: <b>'+ CoordDestino +'</b></td></tr>';
		newinfo_village += '			<tr><td>Punkty: <b>'+ infoPlayer["Puntos"] +'</b</td></tr>';
		newinfo_village += '			<tr><td>Nick: <a id="btnPlayerPopUp" href="javascript:void(0);">'+ infoPlayer["NombreJugador"] +'</a></td></tr>';
		newinfo_village += '			<tr><td>Plemię: '+ infoPlayer["Tribu"] +'</td></tr>';
		newinfo_village += '			<tr><th>Akcja</th></tr>';
		newinfo_village += '			<tr><td><a 	href= "'+twLinks["twMainMenuArray"]["map"]+'&x='+infoPlayer["CoordDestino"][0]+'&y='+infoPlayer["CoordDestino"][1]+'" >&raquo; Centruj mapę</a></td></tr>';
