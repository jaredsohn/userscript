...
var WoDTool = {
	...
	getAttr : function ( context ) {
		...
		WoDTool.walker( 'tr', function ( tr ) {
			if ( tr.className.indexOf( 'content_table_row_' ) != -1 ) {
				if ( tr.childNodes[1] ) {
					var attr = [];
		...
		WoDTool.walker( 'table.content_table td', function ( td ) {
			if ( td.innerHTML.sindexOf( ['英雄的等级', '体力', '生命值再生', '法力', '法力值再生', '每回合的行动次数', '先攻权'] ) != -1 ) {
				var attr = [];
				if ( td.childNodes[1] ) {
				...
				if ( attr[0].indexOf( '值再生' ) == -1 ) {
					attr[1] = td.parentNode.childNodes[3].innerHTML.txt();
				} else {
					attr[1] = td.innerHTML.txt().substr( td.innerHTML.txt().indexOf( '值再生' ) + 4 );
				}
				attrs.push( attr );
			}
		}, context );
	...
	getSkill : function ( context ) {
		...
		WoDTool.walker( 'tr.content_table_header th.content_table', function (th) {
			if ( th.innerHTML == '训练费' ) {
