			...
			try{
				...
				if(glowT){
					if(json[1][2][tankId]['game_info'][1]["instance_id"]!=GM_getValue('instance_id~'+tankId,0)){
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								alert(aquaN+' has been glowing for '+seconds2HMS(gaiaT-glowT)+'. The TankId is '+tankId+'.');
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/t.57983783/');
								}
							}
							else{
								//alert('over');
							...
							alert(aquaN+' will glow in '+seconds2HMS(glowT-gaiaT)'. The TankId is '+tankId+'.');
							if(open===true){
								GM_openInTab('http://www.gaiaonline.com/landing/flashaquarium/?userid='+json[1][2][tankId]['user_id']);
							}
							else{
								GM_openInTab('http://www.gaiaonline.com/forum/compose/topic/new/?f=393');
								GM_openInTab('http://www.gaiaonline.com/forum/gaia-aquarium/gl-wing-b-ty-4-tanks-rand-m-gifts/t.57983783/');
							}
						}
					}
				}
...
function checkTanks(){
	...
	checkTank(5081629, false);//sachara
	checkTank(5074505, false);//Miko Tadita
	checkTank(5916899, false);//Penelope Kitty
	checkTank(5661737, false);//arrowMaker
	checkTank(5319161, false);//jellykans
	checkTank(3790013, false);//Merlaine
	}
}
checkTanks();
