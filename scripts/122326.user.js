// ==UserScript==
// @name           Gerenciador de comunidade
// @namespace      orkutudo
// @description    Script para auxiliar dono e moderadores de comunidades no orkut
// @include        http://orkutudo.com/*
// @version	       2.0
// ==/UserScript==
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=73595741&tid=5694192874848778042&start=1", false);
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var texto = "Tópico deletado: ".concat("[b]",titulo, "[/b]", breakLine, "Criador: [link]http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, breakLine, "Motivo: ","[b][red]", motivo,"[/b][/red]  ", breakLine, "Horário: ", pegaData(), breakLine, "[silver]", rnd, "[/silver]");
		var envia = {
	...
	function postaMembroBanido(uid, motivo) {
		...
		var breakLine = '\n';
		var rnd = String(Math.random());
		var texto = "Membro banido: ".concat("[link]http://www.orkut.com.br/Main#Profile?", uid, "[/link]", breakLine, "Motivo: [b][red]", motivo, "[/b][/red]", breakLine, "Horário: ", pegaData(), breakLine, breakLine, breakLine,  "[silver]", rnd, "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"bodyText": encodeURIComponent(texto),
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tópico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Digite o motivo de estar deletando este topico:','Tópico de spam.'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
