...
// @namespace      5zen
// @description    書簡を７人以上の君主に送ることができるツール
// @include        http://*.3gokushi.jp/message/new.php*
// @licence        "blank"さんが作って5zenが手直し
...
(function(){
  ...
  // At user page
  if ( ( location.href.match(/.*\/message\/new\.php.*/) ) && (document.body.outerHTML.indexOf("送信内容に間違いがないか確認してください。")==-1) ) {
    process();
  }
})();
...
function process() {
  ...
  // 従来のボタンの消去
  var send_btn = xpath('//input[@id="btn_send"]', document);
  if (send_btn.snapshotLength) {
    send_btn.snapshotItem(0).style.display = "none";
  }

  var send_btn = xpath('//input[@id="btn_back"]', document);
  if (send_btn.snapshotLength) {
    send_btn.snapshotItem(0).style.display = "none";
  }
  ...
  div.innerHTML = '<a name="analysis"></a><input '
    +' value="&nbsp;複数宛に今すぐ送信&nbsp;"'
    +' type="button" id="mass_send"><span id="mass_send_help"> （←７人以上の君主に送るときはこちらをクリック！）</span>';
  ...
  document.getElementById("mass_send").addEventListener("click",
    function() {
      ...
      setTimeout(function(){
        ...
        sendMessages(tos,subject_obj.value, "この書簡は" + to_obj.value + " （敬称略）に同時に送信されています。\r\n\r\n" + body_obj.value,ssid);
...
function sendMessages(tos,subject_text,body_text,ssid) {
 for ( var i=0; i<tos.length; i++ ) {
   send_message(tos[i],subject_text,body_text,ssid);
 }
}

function send_message(to,subject_text,body_text,ssid) {
  var path = "/message/new.php";
  var param = {"to":to,"subject":subject_text,"body":body_text,"reply_flg":"","ssid":ssid,"reply_message_id":"","btn_send":"送信"};
console.log(param);
  var request = new Ajax.Request( path, {
        method: "post",
        asynchronous: true,
        parameters: param, 
        onSunccess: function(request) {}
    } );
}
