// ==UserScript==
// @name	Kingsage Enhancement Suite 1.1.4 TR
// @version	1.1.4
// @author	grafilicious
// @turkish	Zihnefendet a.k.a Right Kan
...
// TODO: test in chrome, opera, update-reminder,  show cost for 1 level, save everything apart from user settings in another obj?
	...
	/**/
		...
		css += '.kes-width2 {width: 62px; margin: -2 0 0 9px; display:  vertical-align: text-top;}';
		...
		var kesForUtil = {
			...
			updateCheck: function(version) {
				...
				$.ajax({
					type: 'get',
					dataType: 'jsonp',
					url: 'http://egoserv.com/',
					success: function(data) {
						// check data.version against version show in notification
					}
				});
			},
			//######## UTILITIES ########//
			...
			getGroups: function(villageId) {
				var url = 'http://' + host + '/popup.php?s=groups&m=village&village_id=' + villageId + av;
				var groups = [];
				$.ajax({
					type: 'POST',
					async: false,
			...
			getSession: function(url, success) { // TODO: replace the other session functions
				var session;
				$.ajax({
					type: 'get',
					async: false,
			...
			genericBBCode: function(creator, data) {
				$('body').append('<div id="kes_box"><textarea style="width: 99%; max-height: 90% !important; resize: none;" id="kes_genericBBCode"></textarea></div><div id="kes_overlay" class="kes-backl"></div>');
	...
	function displayAttacksWithSeconds(element) { // TODO: fix for multipage attacks
		...
		$('table.borderlist').eq(element).find('tr').each(function () {
			...
			if (cur.find('td:last > span').length > 0) {
				var incomming = $(this).find('td:last > span').html();
				incomming = incomming.match(/[0-9]{1,2}:[0-9]{2}:[0-9]{2}/, '')[0].split(':');
				var inc = createDate(incomming);
				var correct = new Date();
				var servertime = createDate($('#servertime').text().split(':'));
				correct.setHours(servertime.getHours() + inc.getHours());
				correct.setMinutes(servertime.getMinutes() + inc.getMinutes());
				correct.setSeconds(servertime.getSeconds() + inc.getSeconds());

				var replacable = cur.find('td:nth-child(4)');
				var repl = replacable.text();
				repl = repl.replace(/[0-9]{2}:[0-9]{2}/, correct.getHours() + ':' + $.kes('paddedNumber', correct.getMinutes()) + ':' + $.kes('paddedNumber', correct.getSeconds()));
				replacable.text(repl);
			}
		});
	}
	...
	var draw = {
		helper: {
			...
			returnUnitsForTrooplinks: function (no) {
				...
				out += '<li><span class="kes-width2"><font size="1">Şablon Adı</font></span>&nbsp<input type="text" id="kes_trooplinks_' + no + '_name"size="5" value="' + troops.name + '" max-length="7"></li>';
				// TODO unit runtime
				for(var unit in unit_runtime) {
					if (unit_runtime.hasOwnProperty(unit)) {
						out += '<li><span class="kes-width"><img src="http://' + host + '/img/units/unit_' + unit + '.png"></span><input id="kes_trooplinks_' + no + '_' + unit + '" type="text" value="' + troops[unit] + '" size="5"></li>';
					}
				}
				out += '</ul>';
				return out;
		...
		trooplinks: function () {
			...
			for(var i = 1; i <=3; i++) {
				...
				o += '<h2> ' + l.troops + ' #' + i + '</h2><div class="kes_list">' + this.helper.returnUnitsForTrooplinks(number[i]) + '</div></div>';
			}
			return o;
		},
		highlightgroups: function (highlightgroups) {
			...
			for(var i in highlightgroups) {
				if (highlightgroups.hasOwnProperty(i)) {
					...
					o += '<h2> ' + l.group + ' #' + no + '</h2><ul id="kes_highlightgroups_' + i + '_group"><li><span class="kes-width">' + l.name + ':</span><input id="kes_highlightgroups_' + i + '_name" size="5" maxlength="4" type="text" value="' + highlightgroups[i].name + '"></li>';
	...
	function putSettings(start_tab) {
		...
		var html = '';
			...
			html += '				<span class="kes-heading">Kingsage Enhancement Suite<a href="http://kes.egoserv.com/whykes" target="_blank"><span class="kes-icons kes-icon-home"></span></a></span><span class="kes-subheading">version ' + version + '<br><img src="http://i1186.photobucket.com/albums/z379/araniaexumai/Turkey-icon_zpse51655ae.png"  height="20" width="20"><font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000"> Türkçe çeviri : Okan Kırtay </font></span>';
			html += '			<div class="kes-menu">';
			html += '				<div class="kes-separator"></div>';
			html += '				<a href="#" id="kes_reset_settings"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.resetSettings + '</font></a>';
			html += '				<div class="kes-separator"></div>';
			html += '				<a href="#" rel="content_modules"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.enableDisableModules + '</font></a>';
			html += '				<a href="#" rel="content_trebuchet"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.buildingTrebuchet + '</font></a>';
			html += '				<a href="#" rel="content_spy"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.spy + '</font></a>';
			html += '				<a href="#" rel="content_market"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.marketSettings + '</font></a>';
			html += '				<a href="#" rel="content_troops"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.highlighttroops + '</font></a>';
			html += '				<a href="#" rel="content_groups"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.highlightgroups + '</font></a>';
			html += '				<a href="#" rel="content_trooplinks"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.trooplinks + '</font></a>';
			html += '				<div class="kes-separator"></div>';
			html += '			</div>';
			html += '		</div>';
			...
			html += '			</div>';
			html += '		</div>';
			html += '		<div id="content_modules" class="kes-content-wrapper"><div class="kes-content-title"><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000"><h1>' + l.enableDisableModules + '</h1></font></div><div class="kes-content"><ul>' + draw.modul(presets.modul, k.modul) + '</ul></div></div>';
			html += '		<div id="content_trebuchet" class="kes-content-wrapper">';
			html += '			<div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.buildingTrebuchet + '</font></h1></div>';
			...
			html += '			<div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.spy + '</font></h1></div>';
			...
			html += '			<div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.marketSettings + '</font></h1></div>';
			html += '			<div class="kes-content">';
			...
			html += '			</div>';
			html += '		</div>';
			html += '		<div id="content_troops" class="kes-content-wrapper"><div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.contentHighlighttroops.heading + '</font></h1></div><div class="kes-content">' + draw.highlighttroops(k.units.modul, units) + '</div></div>';
			html += '		<div id="content_groups" class="kes-content-wrapper"><div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.contentHighlightgroups.heading + '</font></h1></div><div class="kes-content">' + draw.highlightgroups(k.highlightgroups) +'<br style="clear: left;"></div></div>';
			html += '		<div id="content_trooplinks" class="kes-content-wrapper"><div class="kes-content-title"><h1><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.contentTrooplinks + '</font></h1></div><div class="kes-content">' + draw.trooplinks() + '</div></div>';
			html += '	</div>';
			html += '</div>';
		$('body').append(html);
	...
	if (page.match('s=overview') && !page.match('s=overview_villages')) {
		...
		function formGenerator(groups) {
			...
			for(var g in groups) {
				...
				o += '<span><input name="vg[]" value="' + groups[g].id + '" type="checkbox" ' + checked + '> ' + groups[g].name + ' </span>';
				if(groups[g].checked) {
		...
		if(tableCount.length == (6 - isModernView)) {
			...
			if(destroyList.length > 0) {
				...
				buildList.before('<span class="click" id="kes_cancelAllDestroy"> ' + l.cancelAllDestroy + '</span><br />'); // LANG
		...
		if(k.modul.massbuild && premium) {
			...
			$('a[href*="&s=build_main&a=buildBuilding"]').each(function() {
				...
				var replacement  = '<a ' + modernStyling + 'href="' + current.attr('href') + '"><span><font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.buildOne + '</font></span></a> ';
					...
					replacement += '<span ' + modernStyling + 'class="click kes_massbuild" data-url="' + current.attr('href') + '"><span style="background-image:url(http://i1186.photobucket.com/albums/z379/araniaexumai/sparkle_zps5a995b94.gif);"><font size="1" style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + l.buildMax + '</font></span></span>';
		...
		if (identifyActiveTab('&m=command')) {
			...
			if (page.match('sub=send')) {
				...
				function displayTrooplinks() {
					var o = '<div style="padding: 5px;"><span style="font-weight: bold;"> ' + l.trooplinks + '</span>';
			...
			$(document).ready(function () {
				...
				$('input[id*="fillOutButton"]').replaceWith('<input id="kes_disband" type="submit" value=" ' + l.fillIn + '">');
		...
		$(document).ready(function () {
			...
			if (k.modul.showAttacksOnMap) {
				...
				function showAttackInformation(villageId) {
					...
					$('#kes_show_attack_info').find('td[id*="n"]').html('<b>' + n + '</b>')
						.end().find('td[id*="p"]').html('<a href="http://' + host + '/game.php?s=info_player&id=' + attack_single[1] + av +'" target="_blank">' + attack_single[0] + '</a>')
							.end().find('td[id*="v"]').html('<a href="http://' + host + '/game.php?s=info_village&id=' + attack_single[3] + av +'" target="_blank">' + attack_single[2] + '</a>')
								.end().find('td[id*="a"]').html('<a href="http://' + host + '/game.php?s=info_ally&id=' + attack_single[5] + av +'" target="_blank">' + attack_single[4] + '</a>')
					...
					function showNextTwenty() {
						...
						if (len == 0) {
							...
							for (var i = 1; i <= size; i++) {
								...
								o += '<tr><td><a href="http://' + host + '/game.php?s=info_player&id=' + attack_info[1] + av +'" target="_blank">' + attack_info[0] + '</a></td>';
								o += '<td><a href="http://' + host + '/game.php?s=info_village&id=' + attack_info[3] + av +'" target="_blank">' + attack_info[2] + '</a></td><td><b>' + attack_info[7] + '</b></td><td>' + y + '</td></tr>';
							}
							o += '</table>';
							return o;
						}
			...
			if (k.modul.targetExport) {
				$('#mapContainer').parent().before('<form action="javascript:void(0);">' + l.player + ': <input type="text" id="kes_target">&nbsp&nbsp<input type="submit" value=" ' + l.targetExport + '" id="kes_targetExportSubmit"><br><input type="checkbox" id="kes_onlyAbandoned"> ' + l.onlyAbandoned + '<input type="checkbox" id="kes_showAsMessage"> ' +l.asBBCode + ' <span id="kes_targetExport_error" style="color: crimson; font-weight: bold; display: none;"></span></form><br />');
				//* add handler
			...
			if (k.modul.insertIntoRuntimeCalc) {
				$('table.borderlist').eq(2).find('tr').each(function () {
					...
					if (cur.find('td[class*="list"]')) {
						...
						children.eq(0).find('img').replaceWith('<a target="_blank" href="http://' + host + '/game.php?&s=tools&m=runtime_calculator' + data + av + '"><img src="http://' + host + '/img/command/attack.png"></a>');
					}
				});
			}
		}
		...
		function createQueue(pages) {
			var pages	= pages/50 + 1,
				step	= 50,
				url 	= 'http://' + host + '/game.php?s=ally&m=attacks&start=' + av,
				queue 	= [];
		...
		function createOptions(market) {
			...
			for(var opt in market) {
				if (market.hasOwnProperty(opt) && opt != 'd3fault') {
					o += ' <span id="kes_market_' + opt + '" opt="' + opt + '" class="click kes_mark"><i><b><font style="text-shadow:#009a00 5px 5px 5px" color="#009a00">' + arrow + ' ' + market[opt].name + '</font></b></i></span>';
				}
			}
			return o;
		};

		$('table[class*="borderlist"]').eq(3).append('<tr><td colspan="3"><span id="kes_market_all"  opt="0" class="click kes_mark"><i><b><font  style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + arrow + ' ' + l.all + '</font></b></i></span>' + createOptions(k.market) +'</tr>');
		...
		if (k.modul.filterOverview && identifyActiveTab('s=overview_villages&m=1')) {
			...
			var ui =  '<table id="filterTroops_table" class="borderlist" style="width: 820px;">';
				...
				ui +=	'<tr><td><input id="kes_filterTroops"  value="' + l.filter + '" type="submit"></td><td><input type="text" size="4" maxlength="5"></td>';
				ui += 		'<td><input type="text" size="4" maxlength="5"></td><td><input type="text" size="4" maxlength="5"></td>';
				ui += 		'<td><input type="text" size="4" maxlength="5"></td><td><input type="text" size="4" maxlength="5"></td>';
				ui +=		'<td><input type="text" size="4" maxlength="5"></td><td><input type="text" size="4" maxlength="5"></td>';
				ui +=		'<td><input type="text" size="4" maxlength="5"></td><td><input type="text" size="4" maxlength="5"></td>';
				ui += 		'<td><input type="text" size="4" maxlength="5"></td><td><input type="text" size="4" maxlength="5"></td></tr>';
				ui += 	'<tr><td>Hedef: <input id="kes_filterTroops_target" type="text" size="7"></td>';
			...
			function getWorldRuntime() {
				$.ajax({
					type: 'post',
					url: 'http://' + host + '/help.php?m=worldinfo',
					success: function (data) {
						var runtime = 1;
			...
			function getResearchSession(villageId) {
				...
				$.ajax({
					type: 'get',
					async: false,
					url: 'http://' + host +'/game.php?village=' + villageId + '&s=build_barracks&m=research' + av,
					success: function (data) {
					...
					current.find('td:gt(0):lt(10)').each(function (i) {
						if ($(this).find('img[src*="grey.png"]').length) {
							if (last.find('img[src*="unit_' + research_units[i] + '"]').length === 0) {
								...
								var researchLink = 'http://' + host + '/game.php?village=' + villageId + '&s=build_barracks&m=research&a=research&p=' + session + '&unit=' + research_units[i] + av;
						...
						$.ajax({
							...
							success: function() {
								if(queue.length > 0) {
									...
									$.ajax({
										type: 'post',
										url: 'http://' + host + '/game.php?s=overview_villages&m=8' + page + av,
										success: function (data) {
		...
		if (self.match(player) && player != "") {
			...
			$('a[href*="edit_player_colors"]').after('<br /><span><a href="http://' + host + '/game.php?village=' + id + '&s=build_barracks' + av + '">' + arrow + ' ' + l.goToBarracks + '</a></span><br />\
				<span><a href="http://' + host + '/game.php?village=' + id + '&s=build_main' + av + '">' + arrow + ' ' + l.goToMain + '</a></span>');
		} else {
		...
		if (k.modul.bbCodeExport) {
			...
			$('table.borderlist').eq(3).prepend('<tr><th colspan="3"><span class="click" id="kes_enablebbCodeExport"><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + arrow +'  ' + l.enableBBCodeExport + '</span></th></tr>');
			$('#kes_enablebbCodeExport').bind('click', function () {
				...
				if ($.kes('isKey', 'kes_thread')) {
					...
					$('#kes_threadId_avail').html('<span style="color: green; font-weight: bold;">' + ' "' + threadName + '" ' + l.thread + l.selected + '.</span>');
				} else {
						...
						function confirmPost(setts, playerName, threadName, threadId, settsPerPosts) {
								...
								<div class="kes-backlight" style="position: fixed; top: 25px; left: 25px; z-index:200;">\
									...
									<div id="kes_showSelectedSetts">\
										<div style="line-height: 1.1; text-align: left; width: 50%; margin: 0; float: left; border-right: 1px solid #DDD;">\
											<h3>' + l.summary + ':</h3>\
											' + l.postIn + ' "' + threadName +'" (<a href="http://' + host + '/forum.php?s=forum_thread&thread_id=' + threadId + av + '" target="_blank">' + threadId + '</a>)<br /> ' + l.player + ': ' + playerName + '<br />\
							...
							$('#confirm_post_submit').bind('click', function () {
								...
								function makePost(threadId) {
									...
									$.ajax({
										type: 'post',
										url: 'http://' + host + '/forum.php?s=forum_thread&thread_id=' + threadId + '&a=forumReplyThread' + av,
										data: 'text=' + single,
										complete: function () {
											if (data.length > 0) {
												makePost(threadId);
		...
		$('table.borderlist').eq(2).append('<tr><td><span style="background-image:url(http://i1186.photobucket.com/albums/z379/araniaexumai/sparkle_zps5a995b94.gif);"><font style="text-shadow:#de0000 5px 5px 5px" color="#de0000">' + l.trooppoints + ':</span></td><td><span style="background-image:url(http://i1186.photobucket.com/albums/z379/araniaexumai/sparkle_zps5a995b94.gif);"><font style="text-shadow:#0000de 5px 5px 5px" color="#0000de">' + $.kes('prettyNumber', troopScore[0]) + ' (' + ((troopScore[0]/troopScore[1])*100).toFixed(2) + '%)</font></span></td></tr>');
		...
		$('iframe[src*="forum.php"]').load(function () {
			...
			if (iframeURL.match('s=forum_thread' + av + '&thread_id=')) {
				...
				$(frame).find('div.smallButton:first').before('<div id="kes_save_thread" class="smallButton"><span style="cursor: pointer;"> ' + l.saveThread + '</span></div>');
		...
		if (k.modul.massforward) {
			...
			$('table.borderlist').eq(5).append('<tr><td colspan="3"><span style="font-weight:bold;">' + l.recipient + ': </span><input type="text" id="kes_massforward_nick"> <input type="submit" id="kes_massforward" value=" ' + l.forward + '"> <span style="display: none; font-weight: bold;" id="kes_massforward_text"></span></td></tr>');
...
var de = {
	...
	filter: 'Filtre',
	attacker: 'Saldıran',
	trooppoints: 'Asker Puanı',
	troops: 'Birlikler',
	settings: 'Ayarlar',
	resetSettings: 'Ayarları Sıfırla',
	adoptSettings: 'Ayarları kayıt edilemedi ,ve değişiklikleriniz sıfırlandı.',
	group: 'Grup',
	unit: 'Birim',
	amount: 'Miktar',
	color: 'Renk',
	none: 'Yok',
	validColor: 'Seçtiğini renk geçerlidir',
	invalidColor: 'Seçtiğiniz renk geçerli değildir',
	enableDisableModules: 'Modül Yönetimi',
	buildingTrebuchet: 'Mancınık Hedef Ayarı',
	marketSettings: 'Market Ayarları',
	spy: 'Hızlı Casusluk Ayarı',
	highlighttroops: 'Birlik Vurgulama Ayarı',
	highlightgroups: 'Grup Vurgulama Ayarı',
	trooplinks: 'Hızlı Asker Linkleri',
	contentFirstUsage: [
		'Wilkommen bei KES',
	...
	contentTrebuchet: 'Mancınık saldırıları için varsayılan hedef  ',
	contentSpy: 'Varsayılan casus miktarı %s ',
	contentMarket: {
		default_option: 'Otomatik seçim',
		stone: 'Taş',
		wood: 'Odun',
		iron: 'Demir'
	},
	contentHighlighttroops: {
		heading: 'Birlik vurgulama ayarı',
		abbr: 'Simge',
		off: 'Saldırı',
		def: 'Savunma',
		count: 'Kont',
	...
	contentHighlightgroups: {
		heading: 'Grup vurgulama ayarı',
		replacement: 'In Abwesenheitsvertretung nicht verfügbar'
	},
	contentTrooplinks: 'Varsayılan Birlik Ayarı',
	...
	calcTrooppoints: 'İttifakın asker puanını hesapla',
	loading: 'Yükleniyor',
	bbCode: 'BB-Code olarak görüntüle',
	...
	forward: 'İlet',
	forwardSuccess: 'Seçilmiş mesaj/lar ve/veya rapor/lar alıcıya iletilmiştir',
	forwardError: 'Seçilmiş bir ya da birkaç rapor iletilmedi',
	saveThread: '<font size="1" style="text-shadow:#ef0000 5px 5px 5px" color="#ef0000">Konuyu seç',
	enableBBCodeExport: 'BB-Code Aktarıcısı',
	exportBBCode: '<font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">Aşağıda seçilmiş olan köyler ,<br>forumda seçilmiş olan konuya BB-Code ile eklenir</font>',
	sortContinent: '<font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">Kıtaya Göre Sırala</font>',
	masscoinFixActive: 'Mengenprägen',
	sortCoords: '<font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">Koordinata Göre Sırala</font>',
	sortPoints: '<font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">Puana Göre Sırala</font>',
	sortName: '<font size="1" style="text-shadow:#de0000 5px 5px 5px" color="#de0000">İsme Göre Sırala</font>',
	bbCode: 'BB-Code',
	deselectAll: 'Seçimi Sil',
	thread: 'Adlı Konu ',
	selected: 'Seçildi',
	goChooseThread: '<font size=1.2>Aktarım yapabilmek için önce forumda bir konu seçmeniz gerekli</font>',
	deselect: 'Seçme',
	close: 'Zum Schliessen auf den abgedunkelten Bereich klicken',
	summary: 'Seçim Özeti',
	postIn: 'Seçimin yazılacağı konu :',
	amountOfSetts: 'Seçilmiş köy sayısı ',
	postsToBeCreated: 'Seçim için oluşturulacak mesaj sayısı : %s ',
	formatting: 'Mesaj formatı',
	header: 'Başlık',
	settlementDisplay: 'Mesaj Düzeni',
	footer: 'Alt Bilgi',
	confirm: 'Tamam',
	preview: 'Önizleme',
	chooseThreadFirst: 'Aktarım yapabilmek için önce forumda bir konu seçmeniz gerekli',
	goToBarracks: 'Kışlaya Gir',
	goToMain: 'Kaleye Gir',
	attackWithSpies: '%s Casus ile saldır',
	selectedGroups: 'Seçilen Gruplar',
	markSelection: 'Auswahl markieren',
	deleteSelection: 'Auswahl löschen',
	retrieve: 'Zurückrufen',
	researchMissingTroops: 'Tamamlanmamış birlikleri araştır',
	beingProcessed: 'Çalışıyor',
	researchStarted: 'Araştırmalar başladı',
	noMissingResearch: 'Araştırılmamış birlik yok',
	troopFilter: 'Asker Filtresi',
	target: 'Hedef',
	toa: 'Süre',
	days: 'Gün',
	hours: 'Saat',
	minutes: 'Dakika',
	seconds: 'Saniye',
	withoutRuntime: 'Süreyi Dahil Etme',
	save: 'Kayıt Et',
	arrival: 'Süre',
	noMatch: 'Bu seçime uyan eşleşme yok ',
	runtimecalc : 'Varış',
	loadAllAttacks: 'Tüm saldırıları yükle',
	loadingFinished: 'Yükleme tamamlandı',
	defender: 'Savunmacı',
	nameCoords: 'Oyuncu adı veya koordinat yazın',
	reset: 'Sıfırla',
	attack: 'Saldırı',
	attacks: 'Saldırı',
	saveAttacks: 'Saldırıları Kaydet',
	attacksSaved: 'Saldırılar Kayıt Edildi',
	resetSavedAttacks: 'Kayıdı Sıfırla',
	highlighttroopsError: 'Birlik vurgulama fonksiyonun çalışabilmesi için "Kendi askerlerimi haritada göster." özelliğinin araştırılmış olması gerekli',
	highlighttroopsActivate: 'Birlik vurgulama fonksiyonun çalışabilmesi için "Birlikleri göster" özelliğinin açık olması gerekli',
	close: 'Kapat',
	nick: 'Oyuncu',
	ally: 'İttifak',
	arrival: 'Süre',
	moreAttacks: 'Bu köye gelen diğer saldırılar',
	nextAttacks: 'Diğer %s saldırı',
	targetExport: 'Koordinatları listele',
	asBBCode: '<font size=1.2>BB-Code ekle</font>',
	onlyAbandoned: '<font size=1.2>Yalnızca terkedilmiş köyleri listele</font>',
	entries: 'Adet köy var',
	noMatch: 'Eşleşme bulunamadı',
	fillIn: 'Ausfüllen',
	trooplinks: 'Varsayılan Birlikler',
	buildOne: '1 Seviye Yükselt',
	buildMax: 'Seviye Yükselt',
	modul: {
		marketOptions: '<font size="2">Market Kısayolları</font><br><i><font size=1>Markette daha hızlı nakliye yapabilmek için şablonlar ekleyebilirsiniz</font></i>',
		troopsOnMap: '<font size="2">Asker Vurgulama</font><br><i><font size=1>Bu özellik ile köylerinizi içerisindeki askerlere göre haritada vurgulayabilirsiniz</font></i>',
		showAttacksOnMap: '<font size="2">İttifak Üyesine Gelen Saldırıyı Haritada Vurgula</font><br><i><font size=1>İttifak saldırılar listesi ile ittifak üyelerinin saldırı alan köylerini haritada vurgulayabilirsiniz</font></i>',
		insertIntoRuntimeCalc: '<font size="2">İttifaka Gelen Saldırıyı Süre Hesaplayıcısına Ekle</font><br><i><font size=1>İttifak saldırı listesindeki saldırılar süre hesaplayıcısına ekleyip hangi birliklerden oluştuğunu görebilirsiniz</font></i>',
		highlightgroups: '<font size="2">Grup Vurgulama</font><br><i><font size=1>Köylerini gruplarına göre haritada vurgulayabilirsiniz</font></i>',
		massdisband: '<font size="2">Ayarlı Terhis</font><br><i><font size=1>...</font></i>',
		simulator: '<font size="2">Klasik Simülatör</font><br><i><font size=1>Klasik simülatörü tekrar kullanabilirsiniz</font></i>',
		filterOverview: '<font size="2">Koordinata Göre Birlik Filtresi</font><br><i><font size=1>Bu özellik ile bir köye herhangi bir birlik ile köyleriniz arasında istediğiniz sürede ulaşan orduları görebilirsiniz</font></i>',
		bbCodeExport: '<font size="2">Koordinatlara BB-Code Ekleme</font><br><i><font size=1>Oyuncu profilinde seçilen köylere bb-code ekleyebilirisiniz</font></i>',
		massforward: '<font size="2">Toplu Yönlendirme</font><br><i><font size=1>Mesaj kutunuzda seçtiğiniz mesaj ve raporlar toplu olarak yönlendirebilirsiniz</font></i>',
		trooplinks: '<font size="2">Kışla İçin Hazır Birlik Şablonları</font><br><i><font size=1>Kışlada kullanmak için hazır asker şablonları ekleyebilirsiniz</font></i>',
		targetExport: '<font size="2">Haritadan Koordinat Listeleme</font><br><i><font size=1>Harita ekranında görünen köylerin koordinatlarını listeleyebilirsiniz</font></i>',
		massbuild: '<font size="2">Hızlı Bina İnşaları</font><br><i><font size=1>Bina inşalarınızı daha hızlı yapabilirsiniz</font></i>'
	},
	units: {
		militia: 'Köy Korucusu',
		sword: 'Muhafız',
		spear: 'Silahtar',
		axe: 'Baltacı',
		bow: 'Okçu',
