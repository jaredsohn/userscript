// ==UserScript==
// @name		Facebook Switch Accounts [Kaum Ndaloe 2014]
// @namespace	FSA
// @auther		http://kaumndaloe.blogspot.com
...
(function($){
	...
	'en_us':{
			...
			'BTN_CLS_ACCOUNT'		:'Hapus Semua Akun',
			'BTN_EXPORT_ACCOUNT'	:'Export Account',
			'BTN_IMPORT_ACCOUNT'	:'Import Account',
			'BTN_SETTING'			:'Pengaturan',
			'BTN_OK'				:'OK',
			'BTN_CANCEL'			:'Cancel',
			'ALT_NAMENULL'			:'Masukan Username Dan Password Kamu!',
			'ALT_DELACCOUNT'		:'Hapus Akun Ini',
			'ALT_LOGIN'				:'Login, Silahkan Menunggu!',
	...
	if( document.getElementsByTagName('body')[0].className.indexOf('Locale') != -1 ){
		...
		FSA.Methods = {
			...
			About:function(){
				...
				'<div style="width:420px;" id="version">'+$T('LAB_NEW_VERSION', fsa_json.new_version ) +'<br />'+ $T('LAB_NEW_UPLOAD',fsa_json.time)+'</div>'+
				'<div style="width:420px;">Script Page：<a href="http://userscripts.org/scripts/show/63761">http://userscripts.org/scripts/show/187320</a><br/>'+
				'<p>Create By Kaum Ndaloe <a href="http://kaumndaloe.blogspot.com" traget="_blank">http://kaumndaloe.blogspot.com</a></p></div>';
				...
				if ((forced) || (parseInt(getValue('SUC_last_update', '0')) + 86400000 <= (new Date().getTime()))){
					try{
						GM_xmlhttpRequest({method: 'GET',url: 'http://userscripts.org/scripts/source/'+SUC_script_num+'.meta.js?'+new Date().getTime(),headers: {'Cache-Control': 'no-cache'},onload:function(resp){
							...
							fsa_json.new_version = remote_version;
