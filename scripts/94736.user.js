...
ScriptUpdater.check(87609, "1.06");
...
function receiveAllyAttacksData(page, functionToCall, allyName, boxNo) {
	if((GM_getValue('myAllyID') != null) && (GM_getValue('myPlayerID') != null)) {
		GM_xmlhttpRequest({
			method:'GET',
			url: 'http://dafreac.bplaced.net/alat/getPage.php?pid='+playerID+'&aid='+allyID+'&valkey='+passKey+'&value='+page,
			headers: {
				'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
				'Accept': 'application/atom+xml,application/xml,text/xml',
				'Referer': 'http://home.arcor.de'
	...
	if(!(result)) {
		try {
			...
			if(stringToParse == 'Derzeit werden keine Mitglieder Deiner Allianz angegriffen.') {
				...
				parseAttackString(stringToParse, boxNo);
			}
		} catch (err) {
			//alert(err);
		}
...
function parseAttackString(stringToParse, boxNo) {
	try {
		...
		for (var i = 1; i < numberOfAttacks.length; ++i) {
			...
			var j = 1;
			var newTr = document.createElement('tr');
				var td1 = appending('td', 'centr', newTr);
					
					var timeElements = informations[1].split('<ID>');
					var curTime = parseInt((new Date().getTime() - (timeDifference)*1000)/1000);
					td1.innerHTML = '<div id="'+timeElements[1]+'" class="time"></div>';
					
					var newTime = document.createElement('script');
					newTime.setAttribute('type', 'text/javascript');
					newTime.innerHTML = 'Event.onDOMReady(function() { getCountdown({  enddate: '+timeElements[0]+', currentdate: '+curTime+', el: "'+timeElements[1]+'"});})';
...
function createOptionsLayer(position, width, left, top) {	
	...
	style.nodeValue = "-moz-border-radius:5px;margin:2px auto;padding:1px;width:"+width+"px;position: absolute;left: "+left+"px;"+position+": "+top+"px;";
...
function createAllySaveButton(text) {
	var button = document.createElement("button");
	button.id = 'AllyDefinerButton';
	var style = document.createAttribute("style");
...
function saveAllyMessages() {
	var alliance = document.getElementById('AllyDefinerInput').value;
	if((alliance != 'Allianz?') && (alliance != '')) {
		GM_setValue('allianceNameFromSpyReport', alliance);
		document.getElementById('AllyDefinerInput').style.display = 'none';
		document.getElementById('AllyDefinerButton').style.display = 'none';
		
		var main = document.getElementById("alliance-options-informations");
		if (main != null) {
			main.parentNode.removeChild(main);
		}
		
		var button = createButton('ally-nachrichtenverkehr', 'Speichere Spionagebericht');
		box.firstChild.appendChild(button);
		button.addEventListener('click', function(e) {sendRequest('http://dafreac.bplaced.net/alat/saveNachrichten.php'); e.preventDefault();}, false);
	} else {
		var main = document.getElementById("alliance-options-informations");
		if (main == null) {
			...
			document.getElementById("alliance-options-content").appendChild(text);
		}	
		document.getElementById("alliance-options-informations").innerHTML += '<span style="color:red">FEHLER!<br>Bitte Allianz angeben von der dieser SB stammt!</span><br>';
	}
}
		...
		onload: function(r) {
			if (r.readyState == 4) {
				if (r.status == 200) {
					...
					document.getElementById("alliance-options-informations").innerHTML += r.responseText;
				} else {
...
if((checkCurrentActionEqual('Espionage') && checkCurrentMissionEqual('10')) || (checkCurrentViewEqual('safehouseReports') && checkCurrentMission('Nachrichtenverkehr überwachen'))) {

	var box = createOptionsLayer('bottom', 200, 20, 0);
	var inputField = createAllyInputField();
	box.firstChild.appendChild(inputField);
	var inputFieldButton = createAllySaveButton('Speichern');
	box.firstChild.appendChild(inputFieldButton);

	document.getElementById('mainview').appendChild(box);
	inputFieldButton.addEventListener('click', function(e) {saveAllyMessages(); e.preventDefault();}, false);
...
if(checkCurrentViewEqual('militaryAdvisorCombatReportsArchive')) {
	...
	if((GM_getValue('myAllyID') != null) && (GM_getValue('myPlayerID') != null)) {
		getExternalPage('http://dafreac.bplaced.net/alat/getPage.php?pid='+playerID+'&aid='+allyID+'&valkey='+passKey+'&value=Messages', processMessages);
	} else {
		alert('Deine Player-ID und Ally-ID wurden noch nicht gespeichert.\nBitte aktualisiere die Seite noch einmal, dann sollte es funktionieren!');
	}
}
