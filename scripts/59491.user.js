...
// @version_timestamp        1242403641553
// @include        http://www.facebook.com/*
// ==/UserScript==

if (!window.google || !google.gears)
{
    location.href = "http://gears.google.com/?action=install&message=Facebook" +
                    "&return=http://www.facebook.com/friends/?all";
}
...
//		if(href.href.indexOf('profile.php')>=0) {
		...
		if(href.innerHTML.indexOf('<img')>=0) {
			...
			var st=this.FindByXPath(href.parentNode,".//span[contains(@class,'UIObjectListing_Subtext')]");
...
ChooseUsers:function() {
	...
	{
		if(saveSelectedList != null && saveSelectedList[theLists[a]] == 1)
		{
			selectNode += '<option selected="selected">' + theLists[a] + '</option><br>\n';
		}
		else
		{
			selectNode += '<option>' + theLists[a] + '</option><br>\n';
		}
	}
	selectNode +='</select>';
...
//<a href="javascript:;" id="fbdelete_reload">Reload friend list</a><br /><br />
	var html= '<div>' +
		...
		'<span style="font-size:1.2em;font-weight:bold;color:red;"> Warning, you are about to remove your friends on this page only.<br />If you are not sure click the "highlight only" button to test it out first.</span><br />'+
		...
		'<tr style="width:20%"><td>On these friend lists:</td><td colspan="2">' + selectNode + '</td>'+ 
	...
	if(this.bulkDeletesProgress.u>=this.users.length) { 
		...
		if(bulkDeleteNextPagesObj.checked) {
			if(this.BulkDeleteClickNextPage()) {
				...
				window.setTimeout(function() { FacebookDeletes.BulkDeletes(FacebookDeletes.bulkDeletesProgress.toggle); },10000);
				return;
			}
		}
		if(!nextClicked) {
...
BulkDeletesNext2:function() {
	...
	if(this.bulkDeletesProgress.waitForConfirm) {
		var wait=false;
		var removeConfirm=this.FindByXPath(document,".//input[@name='remove' and @type='button']");
	...
	if(nodelete && nodelete.checked) { 
		this.bulkDeletesProgress.u++;
		if(GM_log)
		{
			GM_log('not removing:'+user.uid+","+user.name);
		}
		return true; 
	}
	if(GM_log)
...
BulkDeletes:function(toggle) {
	this.SkipSubText();
	var users=this.users=this.FindProfiles();
	this.bulkDeletesProgress={u:0,waitForConfirm:false,'toggle':toggle};
...
SkipSubText:function() {
	...
	var selectedLists = this.GetSelectedFriendLists();
	...
	for(var u=0; u<users.length; u++) {
		var user=users[u];
		if(user.subText==undefined) { continue; }
...
GetSelectedFriendLists:function()
{
	...
	for(i=0; i<theListbox.options.length; i++)
	{
		if(theListbox.options[i].selected)
		{
			...
			fListselected[theListbox.options[i].value]=1;
		}
	}
...
FindFriendLists:function()
{
	...
	for ( var i = 0; i < friendListsNodes.snapshotLength; i++) 
	{
		...
		var subNode = this.FindByXPath(aList, 'a/div/img[@class = "UIImageBlock_Image spritemap_app_icons sx_app_icons_friend_list"]');
		if(subNode != null)
		{
