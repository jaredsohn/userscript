...
1.8
Updated by lj:conn
+username could be '*' now, for every user
+community page included in filtering
+Expression style fixed
...
1.6.1
Updated by lj:dimrub 
+keyword filter
+more styles supported

1.5
Original code by lj:slobin
...
//div - return any <div> found

./div - return relatively first <div> found inside current node

.//div[@class="comment_wrapper"] - return <div> with a 'class' property equal to 'comment_wrapper'

.//p[@class='poster']/span[last()]//b/text() - return text encapsulated by <b> 
found in last <span> of <p> with a 'class' property equal to 'poster'
...
*/
  ...
  for (var i = 0; i < styles.length; i++) {
	...
	var user = userNode.singleNodeValue.nodeValue;
      ...
      var userFilter = filter[user];
	if (!userFilter) { userFilter = filter['*']; } // conn: if no user rule found, check common rule
      if (userFilter) {
