...
javascript:(function(){
    ...
    var power_attack = true, user_power_attack = false;
    ...
    var power_attack_enable = true,user_power_attack_enable = false;
    var heal_thres = 21;
	var temp_key;
	var cb_value;
	var xw_person_key;
    ...
    temp_key = /&tmp=(.+?)&/.exec(document.body.innerHTML)[1];
    cb_value = /&cb=(.+?)&/.exec(document.body.innerHTML)[1];

	getLocal_xw_sig_And_TempKey(document.body.innerHTML);
...
//            '<td width="25%"><input type="radio" id="Spammers" name="fight_choice">Add Me Spammers</input></td>'+
            ...
            '<tr><td width="10%" valign="top"><a href="#" id="log_show">Log</a> &nbsp;&nbsp;<input type="text" id="log_size" value="100" class="sexy_input" style="width:20px"></input></td>'+
            '<td width="1%" valign="top">:</td>'+
...
//    }
    ...
    function loadfightlist(){
        ...
        for(i=1; i<fighttable.snapshotLength; i+=2){
			var elt = document.evaluate('.//a[@style=""]', fighttable.snapshotItem(i).getElementsByTagName('td')[0], null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
			if(elt){
			  fightlist_isAlive[fightlist_isAlive.length] = true;
			}else{
				fightlist_isAlive[fightlist_isAlive.length] = false;
			}
...
//        alert(fightlist_attack[i].replace(/&tmp=([^&]+)/.exec(fightlist_attack[i])[0],''));
        ...
        req = $.ajax({type: "POST", url: link, data: params, timeout: 10000,
            success: function (msg){
                document.getElementById('fights').innerHTML = actual_count;
                process_user_attack(msg);
            },
			error: function(){setTimeout(pre_check_user,1000);}
        });
...
//            },30000);
        ...
        req = $.ajax({type: "POST",url: link, timeout: 10000, data: params,
            success: function (msg){
...
//            document.getElementById('ice_post'+(iced_posts.length-1)).onclick = new Function(this.id,publish_ice);
        ...
        if(check_city != fight_city){
            ...
            $.ajax({type: "POST", data: params, timeout: 10000,
...
//                return;
    ...
    function process_user_attack(attack_results){
		...
		getLocal_xw_sig_And_TempKey(attack_results);
        ...
        if(!((/You win/.test(attack_results)) || (/You lose/.test(attack_results)))){
            ...
            var profile_link = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&xw_city=1&tmp='+ temp_key+'&cb='+userid+UnixTS()+ '&xw_person=' + xw_person_key +'&user='+user_list[j]+'&ref=fight_list&xw_client_id=8';
            $.ajax({type: "POST",url: profile_link, data: params, timeout: 10000,
              success: function (msg){
...
//                        logmsg('Reached last member, reloading..','attack_log');
				 ...
				 getLocal_xw_sig_And_TempKey(msg);
...
//                    logmsg('Reached last member, reloading..','attack_log');
                ...
                if(user_list.length == 1){
					setTimeout(pre_check_user,10*1000);
                }else{
                    setTimeout(pre_check_user,random_delay*1000);
				}
            }
            return;
        }
...
//                    logmsg('Reached last member, reloading..','attack_log');
	...
	function pre_check_user(){
        try{
            var target_id = user_list[j];
            var temp;
            logmsg('Running Ice Check on '+user_names[j],'status');
            if (target_id.indexOf("p")!=-1){
				target_id = target_id.substring(2);
                temp='target_pid';
            }else{
                temp='target_id';
            }
			...
			var hitlist_url = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hitlist&xw_action=set&xw_city='+fight_city+ '&tmp=' + temp_key + '&cb='+ cb_value + '&xw_person=' + xw_person_key + '&'+ temp +'='+target_id+'&xw_client_id=8';

            $.ajax({type: "POST",url: hitlist_url, timeout: 10000, data: params,
                success: function (msg){
					getLocal_xw_sig_And_TempKey(msg);
                    if (/You can't add/.test(msg)){
						...
						if(user_list.length > 1){
                            setTimeout(pre_check_user,1000);
                        }else{
							setTimeout(pre_check_user,15*1000);
						}

                        return;
                    }
                    else if (/Hitlist Bounty Amount/.test(msg)){
                        logmsg(user_names[j]+' is alive...','status');
                        logmsg(user_names[j]+' is alive...','attack_log');
                        setTimeout(attack_user,1000);
                        return;
                    }
                },
                error: function(req,status,err){
        ...
        catch(err){alert(err);alert(j);alert(user_list[j]);}
    }

	function getLocal_xw_sig_And_TempKey(attack_results){
         var i1, i2;

		 // try{
		 // temp_key = /tryBuy.*?tmp=([a-f0-9]+)/.exec(attack_results)[1];
		 // }catch(err){
			// try{
			  // temp_key = /&tmp=(.+?)&/.exec(attack_results)[1];
			// }
			// catch(err){
				// logmsg('Error getting temp key','attack_log');
			// }
		 // }

		 if (attack_results.indexOf('local_xw_sig') != -1) {
		    i1 = attack_results.indexOf('local_xw_sig')
            i2 = attack_results.indexOf("';",i1)

            local_xw_sig      = attack_results.slice(i1+16,i2);
		 }
		 try{
			  xw_person_key = /&xw_person=(.+?)&/.exec(attack_results)[1];
			}
			catch(err){
				logmsg('Error getting xw_person_key','attack_log');
			}
	}

	function pauseAndStart(){
		//setTimeout(start_attack,1000);
	}
	...
	function goFightNav() {
	  ...
	  if (!elt) {
		...
		if (!elt) {
		  logmsg('Can\'t find fight nav link to click.', 'attack_log');
		  return;
		}
	  }
	  clickElement(elt);
	...
	function clickElement(elt) {
	  if (!elt) {
		logmsg('BUG DETECTED: Null element passed to clickElement().', 'attack_log');
		return;
	  }
    ...
    function pre_check(){
		...
		if(fightlist_isAlive[i]==false){
		  skip_reason = 'Initially iced';
          next();
		}
		...
		if( fightlist_isAlive[i] == false ){
			logmsg(fightlist_names[i]+' is iced in color','attack_log');
			skip_reason = 'Iced';
            next();
			return;
		}
        ...
        if(document.getElementById('ice_check').checked){
			...
			var hitlist_url = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hitlist&xw_action=set&xw_city='+fight_city+ '&tmp=' + temp_key + '&cb='+ cb_value +'&xw_person=' + xw_person_key +'&target_pid='+target_id+'&xw_client_id=8';
            $.ajax({type: "POST",url: hitlist_url, timeout: 10000, data: params,
                success: function (msg){
                    if (/You can't add/.test(msg)){
...
//        document.getElementById('popup_permanence').innerHTML += 'Heal travel value - '+heal_travel+'<br>';
        ...
        if(fight_city==heal_city){
            ...
            $.ajax({type: "POST",data: params, timeout: 10000, dataType: "json",url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+"&xw_city=" + heal_city + "&xw_person=" + xw_person_key +"&xw_client_id=8",
                success: function (msg){
            ...
            $.ajax({type: "POST",data: params, timeout: 10000, dataType: "json",
				url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=hospital&xw_action=heal&cb="+cb+ "&xw_city=" + heal_city + "&xw_person=" + xw_person_key + "&xw_client_id=8",
                success: function (msg){
                    if(msg.hospital_message == null){
                        logmsg('Error during healing, retrying in 2 seconds..','status');
                        setTimeout(heal,2000);
                    ...
                    $.ajax({type: "POST", data: params, timeout: 10000, url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+heal_city+"&cb="+cb+"&destination="+fight_city+"&xw_person=" + xw_person_key + "&from=fight&nextParams=&xw_client_id=8",
                    success: function (msg){
            ...
            $.ajax({type: "POST", data: params,  timeout: 10000, 
			url: "http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=travel&xw_action=travel&xw_city="+fight_city+"&cb="+cb+"&xw_person=" + xw_person_key + "&destination="+heal_city+"&from=fight&nextParams=&xw_client_id=8",
            success: function (msg){
                heal_travel=true;
