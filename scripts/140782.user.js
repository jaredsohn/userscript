...
<table width="100%" cellspacing="1"><tr><td>\
	<li><a href="#" width="12%" id="show_TON_ui_0">Building</a></li>\
...
function realm_user_aa_context(type, aa_enabled) {
    this.type = type;
    
    this.realm = realmId;
    this.userId = userId;
    this.aa_enabled = aa_enabled;
    
    register_context_handler(this.type, this);
    
    function context_equals(a, b) {
        if ((typeof a == 'undefined') && (typeof b == 'undefined')) return true;
        if ((typeof a == 'undefined') || (typeof b == 'undefined')) return false;
        if ((a == null) && (b == null)) return true;
        if ((a == null) || (b == null)) return false;
        
        if ((a.realm == b.realm) && (a.userId == b.userId)) return true;
        return false;
    }
    function within_current_context(obj) {
        if (!is_defined(obj) || !obj) { return null; }
        
        if (obj.type != this.type) { return null; }
        if (!obj.context) {
            obj.context = {realm:this.realm, userId:this.userId};
            return obj;
        } 
        if (!context_equals(obj.context, {realm:this.realm, userId:this.userId})) {
            return null;
        }
        return obj;
    }
    function operation_permitted(id, obj, op) {
        if (!aa_enabled) return obj;
        
        if (op == REMOVE_OP) {
            var general = get_general_of_troops(obj.unit);
            if (!general) return obj;

            setTimeout(function() {
                if ((obj.status == RUNNING) || (obj.status == COMPLETE)  || (obj.status == TO_SCHEDULE)) {
                    alert('Server processing the job, cannot delete now!');
                    return;
                }
                if (obj.status == ERROR_IN_REQUEST) {
                    game_setting[GENERALS_LOCATIONS].value[general] = IN_CITY;
                    update_object(game_setting_ids[GENERALS_LOCATIONS], game_setting[GENERALS_LOCATIONS]);

                    return;
                }
                if (game_setting[GENERALS_LOCATIONS].value[general] != IN_CITY) {
                    var reset_status = confirm("By removing attack, do you want to reset the General's status?");
                    if (reset_status) {
                        game_setting[GENERALS_LOCATIONS].value[general] = IN_CITY;
                        update_object(game_setting_ids[GENERALS_LOCATIONS], game_setting[GENERALS_LOCATIONS]);
                    }
    ...
    this.within_current_context = within_current_context;
    this.operation_permitted = operation_permitted;
    return this;
}
...
var job_states = {};
job_states[TO_SCHEDULE] = "Waiting for Server";
