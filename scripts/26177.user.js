...
for( var i = 0; i < tables.length; i++ )
{
  ...
  if( bg && bg.match( /i\/kukla/ ) )
  {
    ...
    a.appendChild( document.createTextNode( 'Save current' ) ) ;
    td.appendChild( a ) ;
    ...
    a.appendChild( document.createTextNode( 'Remove All' ) ) ;
    td.appendChild( a ) ;
    tr.appendChild( td ) ;
    o1.appendChild( tr ) ;
    ...
    invent_names = GM_getValue( "lwm_inventory_name" ) ;
    if( invent_names )
    {
      ...
      {
        inv_set_str = GM_getValue( "lwm_inventory_set_" + invent_name[i1] ) ;
...
function clear_set()
{
  invent_name = GM_getValue( "lwm_inventory_name" ).split( '|' ) ;
  invent_count = invent_name.length ;
  if( invent_count && invent_count > 0 )
  {
    for( var i1 = 0; i1 <= invent_count; i1++ )
    {
      GM_setValue( "lwm_inventory_set_" + invent_name[i1] , null ) ;
      GM_setValue( "lwm_inventory_name_" + invent_name[i1] , null ) ;
      a = document.getElementById( 'id_inv_tr_' + invent_name[i] ) ;
      a.parentNode.removeChild( a ) ;
    }
  }
  GM_setValue( "lwm_inventory_name" , '' ) ;
  return ;
}
...
function del_invent_set()
{
  ...
  for( var i = 0; i < invent_count; i++ )
  {
    ...
    {
      GM_setValue( "lwm_inventory_set_" + invent_name[i] , '' ) ;
      GM_setValue( "lwm_inventory_name_" + invent_name[i] , '' ) ;
  ...
  GM_setValue( "lwm_inventory_name" , dress_id ) ;
  return;
}
...
function save_cur_set()
{
  ...
  for( var i = 0; i < anchors.length; i++ )
  {
    ...
    if( el.href.match(/http:\/\/www\.lordswm\.com\/inventory.php\?pull_off=/) )
    {
  ...
  if( dress_id == '' )
  {
    alert( 'You are undressed' )
    return ;
  }

  var str = prompt( 'Enter name of the set:' , '' );
  if( str == null ) return
  if( str == '' )
  {
  ...
  invent_name = GM_getValue( "lwm_inventory_name" ) ;
  if( !invent_name )
  {
    invent_name = '' ;
  }
  ...
  GM_setValue( "lwm_inventory_name" , invent_name ) ;
  GM_setValue( "lwm_inventory_set_" + time_cur , dress_id ) ;
  GM_setValue( "lwm_inventory_name_" + time_cur , encodeURIComponent( str ) ) ;
...
function save_invent_set()
{
  ...
  tr = this.parentNode.parentNode
  set = tr.getAttribute( 'sets' )
  GM_setValue( "lwm_inventory_set_" + set , dress_id ) ;
...
function name_invent_set()
{
  ...
  var str = prompt( 'Enter name of the set' , decodeURIComponent( GM_getValue( "lwm_inventory_name_" + set ) ) );
  if( str == null ) return
  ...
  GM_setValue( "lwm_inventory_name_" + set , encodeURIComponent( str ) ) ;
  tr = this.parentNode.parentNode
  tr.parentNode.insertBefore( get_set( set ) , tr.nextSibling )
  tr.parentNode.removeChild( tr ) ;
}
...
{
  set = this.parentNode.parentNode.getAttribute( 'sets' )
  inv_set = GM_getValue( "lwm_inventory_set_" + set ).split('|')
  ...
  function f_inv_off_all()
  {
    ...
    ({
      method: "GET",
      url: url+"inventory.php?all_off=100" ,
      onload: function(res)
      {
        f_inv_on( 0 ) ;
        return ;
  ...
  if( inv_off.length == 0 )
  {
    ...
    {
      alert( 'You are already dressed in it' )
      return;
    }
    f_inv_on( 0 )
...
function get_set( t )
{
  ...
  a = document.createElement( 'a' );
  a.href = 'javascript:void(0);' ;
  a.title = 'Put on this set' ;
  a.addEventListener( "click", pull_on_off , false );
  name = GM_getValue( "lwm_inventory_name_" + t ) ;
  if( name && name != '' )
  {
    a.appendChild( document.createTextNode( decodeURIComponent( name ) ) ) ;
  } else
  {
    a.appendChild( document.createTextNode( 'Set ' + t ) ) ;
  }
  td0.appendChild( a ) ;
  d.appendChild( td0 ) ;

  td0 = document.createElement( 'td' );
  inv_set = GM_getValue( "lwm_inventory_set_" + t ).split('|');
  ...
  a = document.createElement( 'a' );
  a.href = 'javascript:void(0);' ;
  a.title = 'Save the current set instead'
