...
function initialize() {
	...
	var userBarContainert = document.getElementById("yt-masthead");
	if(userBarContainert != null) {
		try {
...
function scrobbleFeedback(response) {
	...
	if(jsonObject.error != null) {
		...
		var album = jsonObject.scrobbles.scrobble.album.text;
		if(album) {
...
function addStyle() {
	...
	style.innerHTML = "#masthead-lastfm { border-top: 1px solid #CCC; border-bottom: 1px solid #CCC; min-width: 1016px; }\n" +
	...
	".lastfm-text-input { margin-top: 2px; }";	
}
/**
...
function addLastFmButton() {
	...
	var htmlCode = "<div id=\"lastfm-button\">" + 
		"	<button role=\"button\" data-orientation=\"vertical\" class=\"yt-masthead-user-icon yt-uix-button yt-uix-button-hh-default\" type=\"button\">" +
...
function addLastFmOptions() {
	...
	if(bodyContainer != null && accountContainer != null)
	{
		var mockDiv = document.createElement("div");
		mockDiv.innerHTML = htmlCode;
		
		bodyContainer.insertBefore(mockDiv.firstChild, accountContainer.nextSibling);
	}
...
function toggleLastFmForm() {
	...
	var buttonClass = buttonArrow.getAttribute("class");
	if(buttonClass == null || countOccurencesInString(buttonClass, "reversed") < 1) {
		buttonArrow.className = "reversed";
...
function authenticate() {
	//Remove any token parameter that could have been added before.
	var tokenRegex = /(\?|%3F|&|%26)token=[^&\?]*/gi;
	var currentURL = document.URL.replace(tokenRegex, "");
	//Replace any parameters added with '&' a '?' if no question tag is in the URL.
	if(!stringContainsChar(currentURL, "?")) {
		currentURL = currentURL.replace("&", "?");
	}
	//Replace any '?' in the redirect to '%3F' and '&' to '%26'.
...
function tryGetArtistMeta() {
	var xSongArtistPath = "//ul[@id='watch-description-extra-info']/li[1]/span[@class='metadata-info']/a";
	try { 
...
function tryGetTitleMeta() {
	...
	var xSongTitlePath = "//ul[@id='watch-description-extra-info']/li[0]/span[@class='metadata-info']/span['metadata-info-title']";
	try {
