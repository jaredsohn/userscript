	...
	showNotice:function() {
		if(ScriptUpdater.meta.name && ScriptUpdater.meta.version) {	
			...
			noticeWrapper.id = "ScriptUpdater" + ScriptUpdater.scriptId + "BodyWrapper";
				...
				html.push('" title="Закрыть"/><img src="');
				...
				if(!ScriptUpdater.forceNoticeEnabled) {
					html.push('<p>Новая версия скрипта <strong><a href="http://userscripts.org/scripts/show/');
					...
					html.push('</a> </strong> доступна для установки.</p>');
				} else {
				...
				if(ScriptUpdater.scriptCurrentVersion) {
					html.push('<p>У вас установлена <strong>');
					html.push(ScriptUpdater.scriptCurrentVersion)
					html.push('</strong> версия скрипта. Самая последняя версия - <strong>');
					html.push(ScriptUpdater.meta.version);
					html.push('</strong></p>');
				}
				
				if(ScriptUpdater.meta.history) {
					html.push('<h2>Исторя версий:</h2><div id="ScriptUpdater' + ScriptUpdater.scriptId + 'History">');
				...
				*/
				...
				html.push('<input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'CloseButton" value="Закрыть" style="background-image:url(');
				html.push(ScriptUpdater.icons.close);
				html.push(')"/><input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'BodyInstall');
				html.push(ScriptUpdater.scriptId);
				html.push('" value="Установить" style="background-image:url(');
				html.push(ScriptUpdater.icons.install);
				html.push(');"/>');
				html.push('Проверять этот скрипт на обновления ');

				html.push('<select id="ScriptUpdater' + ScriptUpdater.scriptId + 'Interval"> \
								<option value="3600000">каждый час </option>\
								<option value="5">каждый час </option>\
