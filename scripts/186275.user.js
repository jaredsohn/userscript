// ==UserScript==
// @name        MyFreeFarm Portuguese
// @namespace   http://userscripts.org/scripts/show/186275
...
try{
	...
	// Important constants
	const COUNTRY="PT"; // The country ISO-code (2 digits)
	const LANGUAGE="pt"; // The language ISO-code (2 digits)	
	...
	texte["msgMarketsaleContent"]="(.*) comprou a voc"+e_circumflex+" \\s*(\\d+)x (.*?) por\\s*<br>\\s*(.*?) pD\\."; // The text where the information is stated. The information has to be replaced by "(.*?)".
	// For the following you have to take a message sent if you sell something via contract
	texte["msgContractsaleContent"]="(.*) assinou um contrato com voc"+e_circumflex+"!<br><br>\\s*Foram vendidos os seguintes produtos:<br>([\\S\\s]*)\\s*<br>\\s*Foram creditados (.*?) pD na sua conta\\."; // The text where the general information is stated. The information has to be replaced by "(.*?)".
	...
	texte["msgFriend"]="(.+) gostaria de adicionar voc"+e_circumflex+" como"; // The subject. The person has to be replaced by "(.+)"
