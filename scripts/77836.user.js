...
Credits:
	...
	Pete Lundrigan Mastermind for profile linked running .
	...
	Mohamed Hafiz for contributing code.
	http://www.facebook.com/profile.php?id=543666686
*/


javascript: (function() {
    var version = 'Mg9Hs Bagman Mugger - PP_mod v1.06 beta';
    try {
	...
	fb_url = 'http://www.facebook.com/profile.php?id=', cuba_gained = 0, ny_gained = 0, moscow_gained = 0, bangkok_gained = 0, exp_gained = 0, friends_helped = 0;
        ...
        var log_size = 10;
        var log_keep = /(Limit|friends|Facebook|Starting|Invalid|Link)/;
        ...
        var URLs = [];
        var successful = 0, failed = 0, total = 0, old = 0, invalid = 0, leech = false;
        var limit = 5000;


        //Yesterday date in unix

        var yesterday = 24 * 60 * 60;
        var timelimit = 999999;
	...
	'<form name="spockform">' +
		'<table class="messages">' +
			'<tr><td>Configuration</td><td colspan="2" align="right" style="text-align:rigth;font-size:0.8em;">' + version + ' - <a href="http://www.spockholm.com/mafia/bookmarklets.php" target="_top">Spockholm Mafia Tools</a> - <a href="http://www.spockholm.com/mafia/donate.php?MysteryBagger" alt="Buy Spockholm a beer" target="_blank">PintWare</a> (Donate) - <a href="#" id="close"><img src="http://www.spockholm.com/mafia/stop.gif" title="Close" width="16" height="16"></a></td></tr>' +
			'<tr><td>Limit:</td><td colspan="2"><input type="text" maxlength="5" size="5" id="thelimit" value="' + limit + '"> Number of friends to check, lower this if you are having problems. <span class="energy_highlight"><a id="loadfriendslnk" href="#">Then Click Here.</a></span></td></tr>' +
			'<tr><td>Time In hours:</td><td colspan="2"><input type="text" maxlength="2" size="2" id="timelimit" value="' + timelimit + '"> Limit checks to links younger than this.</td></tr>' +
			'<tr id="manual_list">' +
				...
				'</textarea></td>' +
			'</tr>' +
			'<tr><td valign="top">Add your link to <a href="http://www.facebook.com/editprofile.php?sk=basic" target="_blank">Favorite Quotes</a>.</td><td colspan="3">Your MB Link: <a href="#" id="gift_url">Loading...</a></td></tr>' +
			'<tr><td></td><td colspan="2"><span class="energy_highlight">Copy and paste the link above into the quotes section of your profile page at least DAILY so that your mafia friends can reap the same great rewards that you will when you click start below.</span></td></tr>' +
	...
	'<tr>' +
		...
		'<td><span id="spock_progress"></span></td><td></td>' +
		'</tr>' +
	'<tr>' +
		'<td width="100">Stats:</td>' +
		'<td><span id="spock_stats"></span></td><td></td>' +
	'</tr>' +
		'<tr>' +
		'<td>Status:</td>' +
	...
	'</table>';
        ...
        function getnewgiftlink() {
            ...
            $.ajax({ type: "GET", url: baglink,
                success: function(msg) {
                    document.getElementById('gift_url').href = (/<fb:req-choice url='([^']+)'/.exec(msg)[1]);
                    document.getElementById('gift_url').innerHTML = 'Right click here and copy link location.';
                }
            });
        }
        create_div();
        ...
        function start() {
            ...
            yesterday = (new Date().getTime()).toString().substring(0, 10) - (timelimit * 60 * 60);
			...
			//document.getElementById('spock_stats').innerHTML = "Successful: <span id='successful'>0</span>&nbsp; Failed: <span id='failed'>0</span>&nbsp; Old: <span id='old'>0</span>&nbsp; Invalid: <span id='invalid'>0</span>";
            document.getElementById('spock_stats').innerHTML = 'Successful: <span id="successful" class="good">0</span>&nbsp; Failed: <span id="failed" class="bad">0</span>&nbsp;<span class="more_in">Old: <span id="old">0</span>&nbsp; Invalid: <span id="invalid">0</span></span>';
            document.getElementById('spock_progress').innerHTML = (successful + failed) + ' of ' + total + ' &nbsp; <span class="more_in">(' + ((successful + failed) / total * 100).toFixed(1) + '%)</span>';
            ...
            function actionDo(url, pattern, callback, callback_param, invoke, time) {
                if (!invoke) {
                    ...
                    $.ajax({
                        ...
                        success: function(msg) {
                            ...
                            if (pattern == 'by sending a free' && isOk) {
                                ...
                                // alert(text);
                                add_loot(text);
                                u = /from_user=p%7C(\d+)/.exec(url);
                                if (u != null) { from_user = u[1]; }
                                log(timestamp() + 'Collected Mystery Bag from ' + from + '. ');
            ...
            function actionOpenBag(from_user, invoke, time) {
                msg('Checking Mystery Bag from ' + from_user + ', posted on ' + time);
            ...
            function actionGetBag(from_user, time_id, gift_cat, gift_id, gkey, invoke) {
                actionDo('xw_controller=interstitial&xw_action=accept_gift&from_user=' + from_user + '&time_id=' + time_id + '&item_cat=' + gift_cat + '&item_id=' + gift_id + '&gkey=' + gkey, '', actionOpenBag, from_user, invoke, time_id);
            };
            function actionSequentialGetBag(dummy, invoked) {
                ...
                var str = unescape(currentURL);
                if (debug) log("unescaped url " + str);
                if (str.indexOf('next_params=') != -1 && str.indexOf('next_params={') == -1) {
                    str = decode64(currentURL.substr(isCorrect + 'next_params='.length));
                    if (debug) log("decoded url " + str);
                }
                ...
                if (_from_user == null) {
                    str = unescape(currentURL);
                    _from_user = /"from_user":"([|0-9]+)"/.exec(str); if (_from_user != null) _from_user = _from_user[1];
                    _time_id = /"time_id":"([0-9]+)"/.exec(str); if (_time_id != null) _time_id = _time_id[1];
                    _gift_cat = /"item_cat":"([0-9]+)"/.exec(str); if (_gift_cat != null) _gift_cat = _gift_cat[1];
                    _gift_id = /"item_id":"([0-9]+)"/.exec(str); if (_gift_id != null) _gift_id = _gift_id[1];
                    _gkey = /"gkey":"([0-9a-fA-F]+)"/.exec(str); if (_gkey != null) _gkey = _gkey[1];
                ...
                //alert(_time_id + ' - ' + yesterday);

                if (_time_id < yesterday) {

                    var date = new Date(_time_id * 1000);

                    // will display time in 5/6 10:30:23 format
                    var formattedTime = date.toLocaleDateString();  //date.getDay() + "/" + date.getMonth() + " - " + date.getHours() + ':' + date.getMinutes(); ;

                    old++;
                    document.getElementById('old').innerHTML = old + ' (' + parseFloat(old / total * 100).toFixed(0) + '%)';
                    if (debug) log(_time_id);
                    if (debug) log(_from_user);
					var user_output = 'undefined';
					if(/p/.test(_from_user)) {
						user_output = '[<a href="http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&user='+_from_user+'">MW profile</a>]';
					}
					else {
						user_output = '[<a href="http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&user='+_from_user+'">MW profile</a>][<a href="http://www.facebook.com/profile.php?id='+_from_user+'">FB</a>]';
						//user_output = '[<a href="http://www.facebook.com/profile.php?id='+_from_user+'">FB</a>]'
					}
                    log(timestamp() + 'Link older than ' + timelimit + ' hour(s) ' +user_output+ ' posted on ' + formattedTime);
                    //if(/p/.test(_from_user)) {
                    //if (debug) log(_from_user);
                    actionGetBag(_from_user, _time_id, _gift_cat, _gift_id, _gkey, false);
                    //}
                    //else {				
                    //log(timestamp()+'Invalid link from '+_from_user+'. '+fblink(_from_user));
                    //actionGetBag(_from_user, _time_id, _gift_cat, _gift_id, _gkey, false);
                    //invalid++;
                    //document.getElementById('invalid').innerHTML = invalid + ' (' + parseFloat(invalid / total * 100).toFixed(0) + '%)';
                    //}
                }

                else {
                    //add_loot("Old bag, skipped");
                    //log(timestamp()+'Old link from '+from_user[1]+'. '+fblink(from_user[1]));
                    /*
                    if(/p/.test(_from_user)) {
                    log(timestamp()+'Link older than 24 hour(s) '+_from_user);
                    }
                    else {
                    log(timestamp()+'Invalid link from '+_from_user+'. '+fblink(_from_user));
                    invalid++;
