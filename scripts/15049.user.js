// ==UserScript==
// @name           LDRize Mibuffer flickr.share command
// @namespace      http://ido.nu/kuma/
// @description    add flickr.share command to Minibuffer.
...
var SCRIPT_VERSION  = '2007.11.30';
var SCRIPT_URL      = "http://userscripts.org/scripts/show/15049";
...
//
// replacing XMLHttpRequest with GM_xmlhttpRequest.
// slightly modified.
//
// (c) Julien Couvreur
...
/*+*/		for (var i in _data ) {
/*+*/			pairs.push(
/*+*/				[i, _data[i]].map( encodeURIComponent ).join("=")
/*+*/			);
/*+*/		}
/*+*/		_data = pairs.join("&");
/*+*/	}
...
console.log(responseDetails, self, responseDetails.status);
...
var Tumblr = {
	...
	read : function(user, type, count, handler){
		...
		addErrback(function(e){
			if(e.message!=StopProcess) throw e;
		}).
	...
	reblog : function(id){
		var url = Tumblr.TUMBLR_URL+'reblog/'+id;
		return doXHR(url).addCallback(function(res){
			// [FIXME] chop dash
			...
			case 'photo':
			case 'quote':
				fields['post[two]'] = fields['post[two]'].replace(/ (\u2014|&mdash;) <a.*?\/a>/gi, '').trim();
			}
	...
	post : function(params){
		if(params.type == 'reblog')
			return Tumblr.reblog(params.id)
		
		return doXHR(Tumblr.TUMBLR_URL+'share', {
			method : 'POST',
			referrer : Tumblr.TUMBLR_URL,
			...
			// [FIXME] use regular form
			switch(res.channel.URI.asciiSpec.replace(/\?.*/,'')){
			case Tumblr.TUMBLR_URL+'share_confirmation':
				return;
			case Tumblr.TUMBLR_URL+'login':
				throw 'Not loggedin.';
			case Tumblr.TUMBLR_URL+'share':
				if(res.responseText.match(/You can upload more tomorrow./))
	...
	share : function(ctx, ext){
		return withDocument(ctx.document, function(){
			return maybeDeferred(ext.extract(ctx)).addCallback(function(params){
				...
				var ds = {};
				var filter = getPref('posterFilter');
...
/*+*/			return ress;
	...
	extracters : {
		'Photo - Flickr' : {
			...
			check : function(ctx){
				if(ctx.onImage){
					...
					return img.src.match(this.RE);
				}
			},
			extract : function(ctx){
				var id = ctx.target.src.match(this.RE)[1];
		...
		'Quote - LDR' : {
			check : function(ctx){
				return ctx.hostname == 'reader.livedoor.com' && ctx.selection;
			},
			extract : function(ctx){
		...
		'ReBlog - LDR' : {
			check : function(ctx){
				if(ctx.hostname == 'reader.livedoor.com'){
					var item = Tombloo.Service.extracters.LDR.getItem(ctx, true);
					return item && item.href.match('^http://.*?\\.tumblr\\.com/') ||  (ctx.onImage && ctx.target.src.match('^http://data\.tumblr\.com/'));
				}
			},
			extract : function(ctx){
		...
		'Photo - LDR(FFFFOUND!)' : {
			check : function(ctx){
				if(ctx.hostname == 'reader.livedoor.com' && tagName(ctx.target)=='img'){
					var item = Tombloo.Service.extracters.LDR.getItem(ctx, true);
					return item && item.href.match('^http://ffffound\\.com/');
				}
			},
			extract : function(ctx){
		...
		'Photo - LDR' : {
			check : function(ctx){
				return ctx.hostname == 'reader.livedoor.com' && tagName(ctx.target)=='img';
			},
			extract : function(ctx){
				var exts = Tombloo.Service.extracters;
		...
		'Link - LDR' : {
			check : function(ctx){
				return ctx.hostname == 'reader.livedoor.com';
			},
			extract : function(ctx){
				with(Tombloo.Service.extracters){
					LDR.getItem(ctx);
		...
		'ReBlog - Tumblr' : {
			check : function(ctx){
				return $x('//iframe[starts-with(@src, "http://www.tumblr.com/dashboard/iframe")]') && this.getLink(ctx);
			},
			extract : function(ctx){
			...
			getLink : function(ctx){
				return $x(
					'(ancestor::div[@class="post"]//a[starts-with(@href,"http://'+ctx.host+'/post/")])[1]/@href', 
					ctx.target);
			},
		},
		...
		'Photo - FFFFOUND!' : {
			...
			extract : function(ctx){
				...
				var title = $x('//div[@class="title"]/a');
				
				return {
					type   : 'photo',
					source : ctx.target.src.replace(/_m(\..{3})$/, '$1'),
					body   : title.textContent.link(title.href) + ' (via ' + author.textContent.link(author.href) + ')',
				}
			},
		},
		...
		'Photo' : {
			...
			extract : function(ctx){
				...
				return {
					type   : 'photo',
					source : tagName(target)=='object'? target.data : target.src,
					body   : ctx.title.link(ctx.href),
				}
			},
		},
		...
		return Tombloo.Service.Photo.getByFileExists(user, size, false).
			addCallback(function(photos){
				...
				return deferredForEach(photos, function(photo){
					...
					return Tumblr.Photo.download(photo.getFile(size)).addCallback(wait, 0.5);
				});
			}).
...
var FFFFOUND = {
	...
	post : function(ps){
		// The image you choose is already in your found.
...
//
// LDRize Mibuffer flickr.share command implementation code
// (c)2007 KUMAGAI Kentaro ku0522a*gmail.com
// http://ido.nu/kuma/
//
...
var Imageinfo = {
	IMAGEINFO: [
		{
			url: 'http://([\w\-]+\.)?flickr\.com/photos/.+',
			image:		'./p[@class="Photo"]/a/img',
			permalink:	'./p[@class="Photo"]/a',
			owner:		'//h1',
			title:		'./p[@class="Photo"]/a/img/@alt',
			".filters": {
				image: [
					['replace', /_\w\.(jpg|gif|png)$/i, '_o_d\.$1']
				],
				owner: [
					['replace', /'s photos\s*$/, ""]
				]
	...
	lookup: function (url) {
		var rule = null;
		this.IMAGEINFO.some( function (candidate) {
			rule = candidate;
			var re = new RegExp(rule.url);
			return url.match( re );
		} );
		return rule;
	},
	getLargestImage: function (paragraph) {
		var images = $x(".//img", paragraph);
		var largest = null;
		var maxValue = 0;
	...
	extract: function (paragraph, rule) {
		var result = {};
		for (var attribute in rule) {
			if ( attribute.charAt(0) == "." || attribute == "url")
				continue;

			var xpath = rule[attribute];
			var resultSet = $x(xpath, paragraph);
			trace(resultSet,xpath)
				if ( !resultSet || resultSet.length <= 0 )
					continue;

			var node =    resultSet.pop()
				var value;

			var tagname = ( node.nodeName || node.localName );
			if ( tagname ) {
				tagname = tagname.toLowerCase();

				// its annoying to make absolute URI from relative path
				// set in src/href attributes.
				if ( tagname == 'img' ) {
					value = node.src;
					result.imageElement = node;
				} else if (tagname == 'a' ) {
					value = node.href;
				} else {
					value = node.textContent.replace(/^\s*/,"").replace(/\s*$/,"") ;
				}
			} else {
				value = node.nodeValue
			}

			if ( rule[".filters"] && rule[".filters"][attribute] ) {
				rule[".filters"][attribute].forEach( function (list) {
					var method = list[0];

					// to prevent evaluating anything except string or RegExp.
					var args = list.slice(1).filter( function (arg) {
						return (
							( arg instanceof RegExp ) ||
							( typeof arg == 'string' )
						)
					} );
					trace ('method', method, value, value[method] );
...
// obsolete method.
	toImageURL: function (paragraph, rule) {
		var image = null;
		if ( rule && rule.image ) {
			var images = $x(rule.image, paragraph);
			if ( images ) {
				image = images.pop();
			}
		} else {
			image = this.getLargestImage(paragraph);
		}
		...
		var u = null;
		// image is instanceof XPCNativeWrapper. not HTMLImageElement.
		var tagname = image.nodeName || image.localName;
		if (tagname && tagname.toLowerCase() == 'img'  ) {
			u = image.getAttribute("src");
		} else {
			trace(image, image.localName, image.nodeName);
			u = image;
		}

		if ( rule && rule.filter ) {
			var re = rule.filter[0];
			var replacement = rule.filter[1];
			if (
				( re instanceof RegExp ) &&
				( typeof replacement == 'string' )
			) {
				// apply does not work on native methods?
			trace("rule", u, re.toString());
				u = u.replace(re, replacement);
			}
		}
		trace("filt", u);
		return u;
...
var targetServiceID = 'Tumblr';
var MinibufferFlickrShareOnTumblr = {
	pinnedNodes: 0,
	reblogged: 0,
	posting_period: 750,
	message_duration: 1500,
	share: function (paragraph, rule, stdin) {
/*
		var info = Imageinfo.extract(paragraph, rule);

		trace("extracted imageinfo", info);
		//return {
		//	'post[type]'  : 'photo',
		//	t             : m.title,
		//	u             : m.href,
		...
		var params = {
			type: 'photo',
			title: info.title + " on Flickr",
			href: info.permalink,
			source: info.image,
			body: info.title + " (via " + info.owner.link( info.image ) + ")",
		};
*/
		// getLargestImage works good enough at every situation.
		var targetImage = Imageinfo.getLargestImage(paragraph);

		trace("targetImage", targetImage);
		...
		var res = Tombloo.Service.share(context,
			Tombloo.Service.extracters[ 'Photo - Flickr' ]
		);

		res.addCallback( function (res) {
			if ( res[targetServiceID][0] ) {
				window.Minibuffer.execute("toggle-pin", [paragraph]);

				var self = MinibufferFlickrShareOnTumblr;
				window.Minibuffer.message(
					Math.floor( ++self.reblogged / self.pinnedNodes * 100 )
						+ "% " + 'posted',
	...
	minibufferCommand: function (stdin) {
		var rule = Imageinfo.lookup(document.location.href);
		trace("minibufferCommand", stdin, this, window.Minibuffer, window.LDRize);
		var self = MinibufferFlickrShareOnTumblr;
		self.pinnedNodes = stdin.length;
		self.reblogged = 0;

		if ( self.pinnedNodes > 0 ) {
			var msg = "Posting " + self.pinnedNodes +
				("photo" + ( (self.pinnedNodes > 1) ? 's' : '') );
			window.Minibuffer.message( msg, self.message_duration);
		}
		return stdin.map( function ( paragraph, n ) {
			return setTimeout( function () {
				self.share.apply(self, [paragraph, rule] );
			}, self.posting_period * n );
		} );
	}
}
...
if ( typeof GM_log != 'undefined' ) {
	window.Minibuffer.addCommand( {
		'tumblr.share': MinibufferFlickrShareOnTumblr.minibufferCommand
	} );
	trace("MinibufferFlickrShareOnTumblr added", GM_xmlhttpRequest.toSource());
}
...
MochiKit.Async.getXMLHttpRequest= function () {
	...
	xhr.channel = {
		URI: {
			asciiSpec: Tumblr.TUMBLR_URL+'share_confirmation'
		}
	};
	return xhr;
}
