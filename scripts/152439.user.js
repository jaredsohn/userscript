...
/* -------------------------------------------- 设置开始 ------------------------------------------*/
	...
	var BLACK_LIST_KEYWORD = ['mp3', 'music', 'xiami.com'];
...
/* -------------------------------------------- 设置结束 ------------------------------------------*/
	...
	function restore_iframe_attr_name(){
		if(topSavedObj != null){ // in the top window
			...
			window.parent.postMessage(JSON.stringify(fv_data), '*');
	...
	function show_popup_icon(obj){
		...
		if(check_need_reload() == true){
			...
			window.name = Math.random().toString();
			var fv_data = {name: 'fv_popup_player', msg: 'MSG_ID_SAVE_RANDOM_NAME_2', from: window.name};
			window.parent.postMessage(JSON.stringify(fv_data), '*');
		}
	...
	function del_popup_icon(){
		...
		if(obj){
			...
			var fv_data = {name: 'fv_popup_player', msg: 'MSG_ID_DELETE_POPUP_ICON'};
			windowChild(randomName2).postMessage(JSON.stringify(fv_data), '*');
			restore_iframe_attr_name();
			randomName2 = 'none';
		}
	}
		...
		if(fv_data.name && fv_data.name == 'fv_popup_player'){
			...
			case 'MSG_ID_CHECK_IF_SHOW_POPUP_ICON_FOR_IFRAME':
				...
				if(obj && obj.length == 1){
					...
					if(objSize.w == bodySize.w && objSize.h == bodySize.h){
						show_popup_icon(obj);
						ret = true;
					}
				}
	...
	function mouseover_hdlr(e){
		...
		if(tag == 'embed' || tag == 'object'){
			...
			windowChild(obj.name).postMessage(JSON.stringify(fv_data), '*');
		}
	...
	if(document.doctype && document.doctype.name != "wml"){ // added by iZML
		_addStyle(styleText);
	}
