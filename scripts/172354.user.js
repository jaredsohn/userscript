...
wparent.location.href.indexOf('?view=ads') != -1)&& self == top){
	...
	div2.append(
		$('<h3 class="titlePTCJungle">').text('PTC - JUNGLE  ')		
	)	
        ...
        function rec(ctr){
            ...
            if(arr[ctr]){
                if(typeof adsWind == 'undefined')
					adsWind = wparent.open(arr[ctr].href,"PTCJungle","width=100,height=50,top=1,left=1");
				else	
				...
				if(adsWind == null){
					...
					$(adsWind).ready(function(){adsWind.resizeTo(940,200)})	
				}
            }
            else{	
