// ==UserScript==
// @name           Asian_Sensation owner label
// @namespace      Asian_Sensation owner label
...
function findName(test) {
    if (test.innerHTML.indexOf('/game/home.pl?user_id=162456', 0)>=0) return 1;
  return 0;
}
...
for(var i=0,j=els.length; i<j; i++) {
	if (findName(els[i])){
		...
		els[i].innerHTML = els[i].innerHTML + '<b>Owner of D.A.D.D.Y.</b><br/>'
