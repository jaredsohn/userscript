...
// Removed because new FB code won't over-write this dummy method
	...
	try {
		...
		this.render = function(tab_id) {
			...
			<div id="bfb_option_container">
				...
				<div id="bfb_options_content" class="content">
					...
					<div class="option tab_popular">
						...
						<div class="desc">
							%auto_switch_to_recent_stories% Force the news feed to switch to "Recent Stories First" if available.
						</div>
					</div>
					<div class="option tab_popular">
						...
						<div class="desc">
							%chronological% Reorder posts to be chronological and hide "Top Stories"
						</div>
					</div>
					...
					<div class="option tab_layout tab_layout">
						...
						Add quick links to left navigation: 
							...
							%show_nav_edit_friends% Edit Friends 
					...
					<div class="option tab_layout">
						Hide :
							...
							%hide_chat_panel% Friends on Chat
					...
					<div class="option tab_layout">
						...
						<div>
							...
							<div class="bfb_sub_option bfb_option_advanced">Refresh every %friend_tracker_interval% hour(s)</div>
						</div>
						<div>
					...
					<div class="option tab_posts">%expand_see_more% Auto-expand "see more" links in posts to show the full post</div>
					<div class="option tab_posts">
			...
			-->
					...
					<div class="option tab_display">%show_char_counter% Add a character count down in status update boxes with a max of %char_counter_max2% characters</div>
			...
			<!--						<div class="option tab_advanced">Hide Friend Activity if it matches pattern: %friend_activity_hide_text%</div> -->
					...
					<div class="option tab_css">Custom post action "sprite" url: %sprite_url%</div>
					...
					<div class="option tab_advanced">%debug% Enable Debug Mode</div>
					<div class="option tab_advanced">%performance_monitor% Enable Performance Monitoring: %performance_monitor_auto_scroll% Auto-Scroll %performance_monitor_dom_insertions% Show DOM Inserts %performance_monitor_show_messages% Show Messages</div>
	...
	onIdLoad('pageNav',function(nav) {
		if ($('bfb_options_button')==null) {
			...
			if (options.get('disabled')) {
				...
				add_option_item('Social Fixer Options',null,function() { options.displayOptions(); hide_bfb_menu(); return false;  },'top');
				add_option_item(el('li','menuDivider'),null,null,'top');

				add_option_item('Frequently Asked Questions',null,function() { window.open('http://BetterFacebook.net/faq.php'); hide_bfb_menu(); return false;  },'top');
				add_option_item('Support Page',null,function() { window.open('http://facebook.com/174424289341'); hide_bfb_menu(); return false;  },'top');
				add_option_item('About Social Fixer',null,function() { better_fb_options('tab_about'); hide_bfb_menu(); return false;  },'top');
				//add_option_item('Donate to Support Matt',null,function() { window.open('http://SocialFixer.com/donate.php'); hide_bfb_menu(); return false;  },'top');
...
/*
	...
	.fbDockChatBuddyListNub .fbNubFlyout { min-height:0 !important; }
*/
	...
	html.lock_header div#blueBar.fixed_elem { position: fixed !important; }
			...
			getValue([content_key,ttl_key],[null,null],function(vals) {
				...
				if (use_cache && vals[ttl_key] && time()<vals[ttl_key]) {
					...
					ajax({'method':'GET','headers':{},'url':url,'onload':function(res) { 
						if (res && res.status && res.status==200 && typeof res.responseText!="undefined") {
...
//	make_hideable('#pagelet_rhc_ticker .fbFeedTicker','Live Activity Ticker','hide_happening_now');
				...
				onSelectorLoad('ul.fbChatOrderedList',function(list){ 
					...
					var render_chat_item = function(id) {
						...
						if (uinfo) {
							...
							var h = '<a style="'+style+'" class="clearfix" href="#" onclick="Chat.openTab('+id+',\''+uinfo.name+'\',\'friend\');return false;" rel="ignore"><img class="pic" src="'+uinfo.thumbSrc+'"/><span class="name">'+uinfo.name+'</span><i class="status img sp_3xd8gf sx_94f6be"></i></a>';
	...
	if (options.get('auto_switch_to_recent_stories')) {
		onSelectorLoad('.uiStreamHeaderChronologicalForm a.itemAnchor[aria-checked="false"][href="/?sk=h_chr"]',function(a) {
			if (!chrono_switched) {
				chrono_switched = true;
				quickmessage('Social Fixer is automatically switching you to Chronological view... Please wait... ');
...
//	onSelectorLoad('.uiUfiViewPrevious',function(o) {
//		clickLink(QS(o,'input'));
//	},false);
