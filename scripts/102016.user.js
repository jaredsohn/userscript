// ==UserScript==
// @name          MMHK-VicesEtRales
// @namespace     http://mightandmagicheroeskingdoms.ubi.com/play
// @description   Script pour MMHK par l'alliance Vices et Râles (v.81)
...
//  repeat:function(i, m) { for ( var o = []; m > 0; o[--m] = i) {} return o.join(''); },
 ...
 localStorage:
 {
  ...
  // TODO : et pourquoi elle s'appelle pas juste get() elle lÃ  ?
 ...
 // cette fonction (API.dump), empruntÃ©e au script Legends, permet d'afficher dans la "console" locale le contenu d'une variable
 // elle n'est utilisÃ©e (sauf oubli de ma part) que lors de mes tests.
 // c'est pour cette raison qu'elle est en commentaire.
 // mais par simplicitÃ© pour moi je la laisse en commentaire au lieu de la supprimer
...
//         else
//         {
//          if ( H.ArrayUtils.isArray(v) ) { li.appendChild(cE('SPAN', {className:'VRlog_array', innerHTML:'Array (Ã©lÃ©ments : ' + v.length + ')'})); }
...
var
 MAINMENU, CACHE, SENTINELLE, NOTES, CONFIG, UBI_SERVER, BACKUP, AUTOSCAN, JACTARI, SIEGES, CHAT, STOCKS, REPERES, MAGICAL_GUILD, MY_CITIES, MY_ALLY, MAILER, HEROMOVE, PANNEAU, MISCTOOLS, RAPPORTS, AUTOCOMPLETE, CUSTOMLINK,
...
// stopEvent = API.EVT.stop,
 ...
 l10n =
 {
  ...
  autocompleteInProgress:{FR:'Recherche en cours', EN:'Search in progress'},
  btnCfg:{FR:'Configurer', EN:'Configurate'},
  btnDeplacement:{FR:'Afficher/Masquer les trajets dÃ©ja parcourus', EN:'Show/Hide hero moves already travelled'},
  btnGuildes:{FR:'Sorts disponibles', EN:'Spells available'},
  btnMisc:{FR:'Outils divers', EN:'Misc tools'},
  btnNotes:{FR:'Notes', EN:'Notes'},
  btnPanneaux:{FR:'Afficher/Masquer les panneaux', EN:'Show/Hide alerts'},
  btnReperes:{FR:'RepÃ¨res', EN:'Shortcuts'},
  btnSentinelle:{FR:'Sentinelle', EN:'Sentinel'},
  btnSieges:{FR:'SiÃ¨ges dÃ©tectÃ©s', EN:'Seats found'},
  btnStock:{FR:'Production', EN:'Production'},
  cfgMarketLegend:{FR:'Place du marchÃ©', EN:'Market Place'},
  cfgPrixUnitaire_buy:{FR:"Prix unitaire d'achat", EN:'Unit purchase price'},
  cfgPrixUnitaire_sell:{FR:'Prix unitaire de vente', EN:'Unit sell price'},
  checkCommandUpdate:{FR:'Vices et RÃ¢les (v%d) - VÃ©rifier la mise Ã  jour', EN:'Vices et RÃ¢les (v%d) - Check for update'},
  checkForUpdate:{FR:'VÃ©rifier la mise Ã  jour', EN:'Check for update'},
  clearFilters:{FR:'Effacer les filtres', EN:'Clear filters'},
  configGenerique:{FR:'GÃ©nÃ©rique', EN:'Main'},
  configMAJ:{FR:'Mises Ã  jour', EN:'Updates'},
  configOpenOnStart:{FR:'Ouverts au dÃ©marrage', EN:'Opened at start'},
  configSentinelle:{FR:'Sentinelle', EN:'Sentinel'},
  configProfilLocal:{FR:'Profil local', EN:'Local profil'},
  configProfilServer:{FR:'Profil BDD', EN:'DB profil'},
  coordsInvalides:{FR:'CoordonnÃ©es invalides', EN:'Invalid coordinates'},
  customLink:{FR:'Ajouter une url personnalisÃ©e', EN:'Add a custom url'},
  deliveryCaravan:{FR:new RegExp('La caravane de ([\u0000-\uFFFF]+) livre la citÃ© de ([\u0000-\uFFFF]+)', 'i'),
                   EN:new RegExp("([\u0000-\uFFFF]+)'s caravan is delivering ([\u0000-\uFFFF]+) city", 'i')},
  deliveryRelay:{FR:new RegExp('Le relais de ([\u0000-\uFFFF]+) livre la citÃ© de ([\u0000-\uFFFF]+)', 'i'),
                 EN:new RegExp("([\u0000-\uFFFF]+)'s relay is delivering ([\u0000-\uFFFF]+) city", 'i')},
  detailDistance:{FR:'%1$s est Ã  %2$s rÃ©gions au %3$s de %4$s (%5$s,%6$s)',
                  EN:'%1$s is %2$s areas away at %3$s from %4$s (%5$s,%6$s)'},
  directionFull_E:{FR:'Est', EN:'East'},
  directionFull_N:{FR:'Nord', EN:'North'},
  directionFull_NE:{FR:'Nord-est', EN:'North-east'},
  directionFull_NW:{FR:'Nord-ouest', EN:'North-west'},
  directionFull_S:{FR:'Sud', EN:'South'},
  directionFull_SE:{FR:'Sud-est', EN:'South-east'},
  directionFull_SW:{FR:'Sud-ouest', EN:'South-west'},
  directionFull_W:{FR:'Ouest', EN:'West'},
  disponibilites:{FR:'DisponibilitÃ©s :', EN:'Availability :'},  MGuildSelectASpell:{FR:'SÃ©lectionnez un sort dans la liste ci-contre', EN:'Please select a spell in the list'},
  distanceCalculer:{FR:'Calculer la distance en nombre de rÃ©gions traversÃ©es', EN:'Calculate the distance in number of areas crossed'},
  distanceFrom:{FR:'de', EN:'from'},
  distanceTo:{FR:'vers', EN:'to'},
  errStructure:{FR:"La structure du jeu est diffÃ©rente. Cette version n'est plus compatible.",
                EN:'The structure of the game is different. This version is not compatible anymore.'},
  errorLocalStorage:{FR:"Vices et RÃ¢les\nlocalStorage n'est pas activÃ©.\nDans la barre d'url tapez,  about:config\npuis placez Ã  'true' la clÃ© 'dom.storage.enabled'",
                     EN:"Vices et RÃ¢les\nlocalStorage is not active.\nIn the url bar type,  about:config\nthen set to 'true' the key 'dom.storage.enabled'"},
  errorWhileCheckingVersion:{FR:'Une erreur est apparue pendant la vÃ©rification de mise Ã  jour :\n%s', EN:'An error occurred while checking for an update:\n%s'},
  FindCity:{FR:'Trouver une ville par son nom', EN:'Find a town from its name'},
  FindCityGoTo:{FR:'Se rendre sur la ville', EN:'Goto town'},
  focusToXY:{FR:'Centrer sur la position (x,y)', EN:'Focus on (x,y)'},
  haltEnd:{FR:'Vers : ', EN:'To : '},
  haltStart:{FR:'De : ', EN:'From : '},
  HerosEligibles:{FR:'HÃ©ros Ã©ligibles', EN:'Heroes eligible'},
  HerosEligiblesNone:{FR:'Aucun hÃ©ro Ã©ligible', EN:'No hero eligible'},
  HeroMoveHalt:{FR:'Fait une halte', EN:'Halt'},
  HeroMoveToHalt:{FR:'En mouvement vers une halte', EN:'Moving to halt'},
  HeroMoveToTarget:{FR:'En mouvement vers la cible', EN:'Moving to target'},
  HeroMoveUnknown:{FR:'Type de mouvement %s inconnu', EN:'Movement type %s unknown'},
  hideFilters:{FR:'Masquer les filtres', EN:'Hide filters'},
  immobilisation:{FR:'Immobilisation', EN:'Downtime'},
  liste_diffusion:{FR:'Liste de diffusion', EN:'Mailing liste'},
  liste_diffusion_destinataires:{FR:'Destinataires', EN:'Destinataires'},
  liste_diffusion_explain:{FR:"Ajoutez un ou plusieurs destinataires hors de l'alliance Ã  votre liste en les sÃ©parant par des points virgule. Exemple : Joueur1;un autre joueur;unTroisiÃ¨me",
  ...
  marketSource:{FR:'Source', EN:'From'},
  marketCible:{FR:'Cible', EN:'To'},
  marketLivraison:{FR:'Livraison', EN:'Delivery'},
  marketPaidPrice:{FR:'Prix rÃ©el', EN:'Real price'},
  marketRetour:{FR:'Retour', EN:'Back'},
  marketShowPrice:{FR:'Prix affichÃ©', EN:'Shown price'},
  marketYouBuySell_raise_OFFER:{FR:'Achat aux enchÃ¨res', EN:'Buying at auction'},
  marketYouBuySell_raise_REQUEST:{FR:'Vente aux enchÃ¨res', EN:'Selling at auction'},
  marketYouBuySell_instant_OFFER:{FR:'Achat direct', EN:'Instant buy'},
  marketYouBuySell_instant_REQUEST:{FR:'Vente directe', EN:'Instant sell'},
  MGuildSwitchMode:{FR:'Changer le mode de restitution', EN:'Switch render mode'},
  newVersionConfirmInstall:{FR:"La version %s du script ViscÃ©ral est disponible.\nPensez a recharger le jeu une fois l'installation terminÃ©e.\nVoulez-vous l'installer maintenant ?",
                            EN:"ViscÃ©ral tool version %d is available.\nDon't forget to reload the game once the installation is complete.\nWould you like to install it now ?"},
  nextBidPrice:{FR:'Prochaine enchÃ¨re', EN:'Next bid'},
  noCaravanMoving:{FR:'Aucune caravane en mouvement', EN:'No caravan on the move'},
  noShortcutCreated:{FR:'Aucun repÃ¨re crÃ©Ã©', EN:'No shortcut created'},
  noSpellKnown:{FR:'Aucun sort connu', EN:'No spell known'},
  noUpdateAvailable:{FR:'Pas de mise Ã  jour disponible.', EN:'No update available.'},
  openOnStartMisctools:{FR:'Outils', EN:'Tools'},
  openOnStartNotes:{FR:'Notes', EN:'Notes'},
  openOnStartReperes:{FR:'RepÃ¨res', EN:'Shortcuts'},
  openOnStartStocks:{FR:'Stocks', EN:'Stocks'},
  openOnStartMisc:{FR:'Outils', EN:'Tools'},
  playerId:{FR:'Joueur #%d', EN:'Player #%d'},
  profilEndommage:{FR:'Profil endommagÃ©', EN:'Profil broken'},
  profilLoad:{FR:'Charger', EN:'Load'},
  profilLoadComplete:{FR:'Chargement du profil terminÃ©.', EN:'Profil load complete.'},
  profilLoadExplanation:{FR:'Collez votre profil dans la zone de texte ci-dessous puis appuyez sur le bouton "OK". Attention, votre profil actuel sera Ã©crasÃ©.',
                         EN:'Paste your profil in the area below and press the "OK" button. Warning, your actual profil will be erased.'},
  profilLoadLocalTitle:{FR:'Restaurer votre profil depuis une sauvegarde locale', EN:'Restore your profil from a local save'},
  profilLoadNotFound:{FR:"Votre profil n'a pas Ã©tÃ© trouvÃ© dans la base de donnÃ©es", EN:'Your profil can not been found in the database'},
  profilLoadServerTitle:{FR:'Restaurer votre profil depuis la base de donnÃ©es.', EN:'Restore your profil from the database'},
  profilSave:{FR:'Sauver', EN:'Save'},
  profilSaveComplete:{FR:'Enregistrement du profil terminÃ©.', EN:'Profil save complete.'},
  profilSaveEmpty:{FR:'Le profil soumis est vide, rien Ã  enregistrer.', EN:'The profil is empty, nothing to save.'},
  profilSaveLocalTitle:{FR:'Enregistrer votre profil sur votre disque dur', EN:'Save your profil to your hard drive'},
  profilSaveServerTitle:{FR:'Enregistrer votre profil dans la base de donnÃ©es.', EN:'Save your profil to the database'},
  remoteTimeDelta:{FR:'Vous avez un dÃ©calage de %s avec le serveur', EN:'You have a lag of %s with the server'},
  repereAdd:{FR:'Ajouter un repÃ¨re', EN:'Add a shortcut'},
  repereDelete:{FR:'Supprimer ce repÃ¨re', EN:'Remove shortcut'},
  repereFocusOn:{FR:'Centrer sur (%s,%s)', EN:'Focus on (%s,%s)'},
  resize:{FR:'Redimensionner', EN:'Resize'},
  sentFiltre_ally:{FR:'Alliances', EN:'Allies'},
  sentFiltre_ally_title:{FR:'Le ou les noms des alliances Ã  filtrer, sÃ©parÃ©s par des points virgule.', EN:"Alliances's names to be filtered, separated by semicolons."},
  sentFiltre_coords:{FR:'CoordonnÃ©es', EN:'Coordinates'},
  sentFiltre_coords_title:{FR:'La ou les coordonnÃ©es Ã  filtrer, sÃ©parÃ©es par des points virgul. 1,2 ; 3,4 ; 5,6', EN:'Coordinates, separated by semicolons 1,2 ; 3,4 ; 5,6'},
  sentFiltre_hero:{FR:'Position du hÃ©ro', EN:'Hero position'},
  sentFiltre_hero_title:{FR:'La distance sÃ©parant le point actuellement centrÃ© sur la carte et la position du hÃ©ro', EN:'The distance between the point now centered on the map and the hero position'},
  sentFiltre_myally:{FR:'Mon alliance', EN:'My ally'},
  sentFiltre_myally_title:{FR:"Filtrer les dÃ©placement de l'alliance", EN:'Filter ally moves'},
  sentFiltre_myself:{FR:'Mes dÃ©placements', EN:'My moves'},
  sentFiltre_myself_title:{FR:'Filtrer mes dÃ©placements', EN:'Filter my moves'},
  sentFiltre_player:{FR:'Joueurs', EN:'Players'},
  sentFiltre_player_title:{FR:'Le ou les noms des joueurs Ã  filtrer, sÃ©parÃ©s par des points virgule.', EN:"Players's names to be filtered, separated by semicolons."},
  sentFiltre_town:{FR:'Villes', EN:'Cities'},
  sentFiltre_townDest:{FR:'Ville de destination', EN:'Destination city'},
  sentFiltre_townDest_title:{FR:'La distance sÃ©parant le point actuellement centrÃ© sur la carte et la ville de destination', EN:'The distance between the point now centered on the card and the destination city'},
  sentFiltre_townImpact:{FR:'Distance Ã  parcourir', EN:'Remaining distance'},
  sentFiltre_townImpact_title:{FR:'La distance sÃ©parant la position du hÃ©ro Ã  sa ville de destination', EN:'The distance between the position of hero and his destination city'},
  sentFiltre_town_title:{FR:'Le ou les noms des villes Ã  filtrer, sÃ©parÃ©s par des points virgule.', EN:"Cities's names to be filtered, separated by semicolons."},
  sentinelleCloseOnClick:{FR:'Fermer sur la sÃ©lection', EN:'Close on click'},
  sentinelleDistance:{FR:'%s rÃ©gions', EN:'%s areas'},
  sentinelleHistory:{FR:'RÃ©gions historisÃ©es : ', EN:'Areas archives : '},
  sentinelleNoRefresh:{FR:'0 pour aucun', EN:'0 for none'},
  sentinelleRefresh:{FR:'Rafraichissement', EN:'Refresh'},
  sentinelle_forceServerDelay:{FR:"DÃ©lai d'appel au serveur : ", EN:'Server call delay : '},
  sentinelle_forceServerDelay_warning:{FR:'Plus le dÃ©lai est court plus le risque de ban est grand', EN:'The shorter the delay, the higher the risk'},
  sentinelle_forceServerRead:{FR:'Forcer la lecture complÃ¨te des joueurs', EN:'Force full extraction of players'},
  sentinelle_forceServerRead_notrecommended:{FR:"Non recommandÃ©, bien qu'un systÃ¨me de cache et de pool soit mis en place, cela reste des appels non autorisÃ©s au serveur",
                                             EN:'Not recommended, although a cache and pool system is established, it remains unauthorized calls to the server'},
  sentinelleHeroMoveTo:{FR:'%1$s est Ã  %2$s regions de son objectif et se dÃ©place en direction du %3$s de %4$s (%5$s,%6$s) vers %7$s (%8$s,%9$s)',
                        EN:'%1$s is %2$ areas away from his target, moving %3$s from %4$s (%5$s,%6$s) to %7$s (%8$s,%9$s)'},
  sentinelle_posCur:{FR:'en', EN:'at'},
  sentinelle_posFrom:{FR:'de', EN:'from'},
  sentinelle_posTo:{FR:'vers', EN:'to'},
  showFilters:{FR:'Afficher les filtres', EN:'Show filters'},
  siege_assiegeurs:{FR:'AssiÃ©geants', EN:'Besiegers'},
  siege_end:{FR:'Fin', EN:'End'},
  siege_nothingFound:{FR:'Aucun siÃ¨ge dÃ©tectÃ©', EN:'No siege found'},
  siege_sieges:{FR:'AssiÃ©gÃ©s', EN:'Besiegeds'},
  siege_title:{FR:'SiÃ¨ges', EN:'Seats'},
  siege_villes:{FR:'Villes', EN:'Towns'},
  simulateFight:{FR:'Simuler ce combat',EN:'Simulate this fight'},
  menuFormat:{FR:'Format du menu', EN:'Menu format'},
  menuFormat_dyn:{FR:'Dynamique', EN:'Dynamic'},
  menuFormat_horiz:{FR:'Horizontal', EN:'Vertical'},
  menuFormat_vert:{FR:'Vertical', EN:'Horizontal'},
  titleBlocNote:{FR:'Bloc-notes', EN:'Notes'},
  titleConfig:{FR:'ViscÃ©ral - Configuration', EN:'ViscÃ©ral - Configuration'},
  titleMagicalGuild:{FR:'Sorts disponibles', EN:'Spells available'},
  titleMiscTools:{FR:'Outils divers', EN:'Misc tools'},
  titleProfil:{FR:'Restaurer un profil', EN:'Restore profil'},
  titleReperes:{FR:'RepÃ¨res', EN:'Shortcuts'},
  titleSentinelle:{FR:'Sentinelle', EN:'Sentinel'},
  titleStocks:{FR:'Productions et stocks', EN:'Productions and stocks'},
  toggleSentinelle_off:{FR:"Stopper l'actualisation automatique", EN:'Stop automatic reload'},
  toggleSentinelle_on:{FR:"DÃ©marrer l'actualisation automatique", EN:'Start automatic reload'},
  TooltipIncomeHourly:{FR:'Revenu horaire', EN:'Hourly income'},
  TooltipIncomeIsEmpty:{FR:'VIDE', EN:'EMPTY'},
  TooltipIncomeUntilEmpty:{FR:'Vide dans', EN:'Empty in'},
  TooltipIncomeUntilFull:{FR:'Plein dans', EN:'Full in'},
  undefinedVariable:{FR:'La variable %s est indÃ©finie.\nAbandon.', EN:'Variable %d is undefined.\nAborting.'},
  UpdateAvailable:{FR:'Mise Ã  jour v.%s disponible', EN:'Update v.%s available'},
  undefinedDatas:{FR:'DonnÃ©es indÃ©finies.', EN:'Undefined datas.'},
  unitaryPrice:{FR:'Prix unitaire', EN:'Unitary price'},
  versionInvalide:{FR:'Cette version du script ViscÃ©ral est trop ancienne pour pouvoir se synchroniser Ã  la base de donnÃ©es.',
                   EN:'This version of the ViscÃ©ral script is too old to be allowed to synchronize with the database.'},
  zoomRepere13:{FR:'13x13 rÃ©gions', EN:'13x13 areas'},
  zoomRepere35:{FR:'35x35 rÃ©gions', EN:'35x35 areas'},
  zoomRepere35Warning:{FR:"Le format 35x35 ne dÃ©clenchera aucun recentrage si la cible (x,y) est dÃ©jÃ  affichÃ©e sur l'interface (bug non rÃ©solu pour le moment)",
                       EN:'The format 35x35 will not trigger any focus to the (x,y) target if the area is already visible on the interface'},
  zoomRepereSize:{FR:'Format des repÃ¨res', EN:'Shortcuts format'},
...
function updateCheck(forced)
{
 ...
 // VÃ©rif une fois par jour (24 h * 60 m * 60 s * 1000 ms)
...
Window.MANAGER = (function()
{
 ...
 {
  // mon dieu que c'est laid tout Ã§a
...
// mon alliance
//**************************
// TODO : trouver comment gÃ©rer les wings
MY_ALLY = (function(){
 ...
 function drawByTowns(spells)
 {
  ...
  for ( i = 0; i < towns.length; i++ )
  {
   ...
   if ( ecoles.length == 2 )
   {
    ...
    if ( ecoleLvl5 ) { nodeEcoles.appendChild(cTN(' spÃ©cialisation : ' + ecoleLvl5)); }
   }
  }
  if ( !found ) { nodeListe.appendChild(cTN(i18n('noSpellKnown'))); }
 }
  ...
  function drawCaravans(N/*ode*/, all)
  {
   ...
   if ( caravans.length > 0 )
   {
    ...
    for ( i = 0; i < caravans.length; i++ )
    {
     ...
     cell = row.insertCell(-1); cell.innerHTML = c.livraison ? dateH(c.livraison) : 'livrÃ©';
...
function open()
{
 function oncreate()
 {
  ...
  // ajout du bloc profil local Ã  profil
  tmp.appendChild(div);
  ...
  // ajout du bloc profil server Ã  profil
  tmp.appendChild(div);
...
//  x = cVX; y = cVY;
  ...
  cell = row.insertCell(-1); cell.style.textAlign = 'right'; cell.innerHTML = H.DateUtils.timestampToString(moves[i].endDate, H.DATEUTILS_TIME_FORMAT_HOURS_MINUTES) + ' (#' + moves[i].HeroMoveId + ')';
  cell.addEventListener('click', BLINK.on, true);
  ...
  cell = row.insertCell(-1); cell.className = 'clickable '+moves[i].isOwnSource; cell.innerHTML = ' ' + moves[i].nameSource; cell.title = '('+moves[i].xS+','+moves[i].yS+')'; cell.addEventListener('click', openXY(moves[i].xS,moves[i].yS), true);
 ...
 if ( moves.length <= 0 )
 {
  ...
  cell = row.insertCell(-1);
  cell.colSpan = 15;
  cell.innerHTML = 'Aucun mouvement dÃ©tectÃ©';
 }
...
function createWin()
{
 ...
 function createFilters()
 {
  ...
  for ( i = 0; i < opts.length; i++ )
  {
   ...
   {
    case 'myself':
     node = cE('SELECT', {id:'VRSF_myself', title:title, childs:[new Option('',''), new Option('Que mes dÃ©placements','only'), new Option('Sauf mes dÃ©placements','except')]});
     node.addEventListener('change', updateFiltres, true);
     cell(1, cE('LABEL', {htmlFor:'VRSF_myself', title:title, innerHTML:lbl}));
     cell(3, node);
    break;
    case 'myally':
     node = cE('SELECT', {id:'VRSF_myally', title:title, childs:[new Option('',''), new Option('Que mon alliance','only'), new Option('Sauf mon alliance','except')]});
     node.addEventListener('change', updateFiltres, true);
...
function cbAddToRecipient(R)
{
 ...
 {
  n.value = v+';'+n.value;
  // TODO : pas la bonne de mÃ©thode de uncheck le .recipientListCheckBox
...
function cbRemoveContentSpam(R)
{
 ...
 {
  map = [/>([0-9]{1,2}\/[0-9]{1,2} [0-9]{1,2}:[0-9]{1,2} De :[\u0000-\uFFFF]+?)(<br>Ã€ : [\u0000-\uFFFF]+?<br>Sujet :[\u0000-\uFFFF]+?)<br>/g,
...
{
 // TODO : trouver une maniÃ¨re moins aggressive pour les tooltips, doit bien y avoir un prototype ou un callback disponible
 function cb(retour)
 {
...
function enhanceMarketPlace()
{
 function cbAuction(retour)
 {
  ...
  callbackFunction = function(E) { if ( this.playerId ) { H.openPlayerProfileFrame(this.playerId); } }.bind(c);
  n.addEventListener('click', callbackFunction, false);
  if (n.setEvent) {
    n.setEvent('click', callbackFunction);
  } else {
    n.addEventListener('click', callbackFunction, false);
  }
  ...
  {
  n.style.cursor = 'pointer';
  callbackFunction = function(E) { if ( this.bestPlayerId ) { H.openPlayerProfileFrame(this.bestPlayerId); } }.bind(c);
  if (n.setEvent) {
    n.setEvent('click', callbackFunction);
  } else {
    n.addEventListener('click', callbackFunction, false);
...
function init()
{
 function set(p)
 {
  ...
  if ( p && p.imageElement && (m=p.content) && (m=m.message) )
  {
   ...
   else if ( /si[Ã¨|e]ge/mi.test(m) ) { cN = 'SIEGE'; }
...
function UBIfixes()
{
 ...
 // Ã©vite les messages Ã©tranges "HOMMK.getElement error : No #189531 Region found!"
...
function displayTooltip(retour)
{
 var x, y, n, i, tmp, match,
     distChilds = [], eId = 'WorldMap' + worldId + 'MapArea',
 ...
 function getDistanceTowns(m,k)
 {
  ...
  if ( x != c.x || y != c.y )
  {
   distChilds.push({name:c.cityName,x:c.x,y:c.y,dist:getDistance(x,y,c.x,c.y,1), dir:getDirectionTexte(x,y,c.x,c.y)});
  }
 }
 ...
 if ( this.element && this.element.id == eId && ( n = this.hommkTooltipElement ) )//  && n.nodeType === document.ELEMENT_NODE && n.tagName == 'DIV' )
 {
  ...
  if ( x && y )
  {
   ...
   if ( distChilds.length > 0 )
   {
    ...
    {
     tmp = distChilds[i];
     nodeDist.appendChild(cE('DIV', {className:'VR_dir', childs:[ cE('SPAN', {className:'distance', innerHTML:tmp.dist}), cE('DIV', {className:'dir'+tmp.dir}), cE('SPAN', {className:'town', innerHTML:tmp.name}) ]}));
    }
...
//**************************
// Lien vers JACTARI d'aprÃ¨s le code de jeyries
//**************************
JACTARI = (function(){
var
...
function nouveau_combat()
{
 var donnees = {
    ...
    d:{
        ...
        lieu:0, // 1:citÃ©/2:rÃ©gion/3:halte
        fortification:0, // 1:fortin/2:citadelle/3:chÃ¢teau
        forts:0,
        fort_principal:0,
        dolmens:0,
        ralliement:0,
...
function encode_donnees_combat(donnees)
{
 ...
 triplets[8] |= (donnees.d.fortification & 3) << 20; // [ff] dÃ©fense de citÃ©
 triplets[8] |= (donnees.d.dolmens & 15) << 16;
 triplets[8] |= (donnees.d.forts & 7) << 13; // [FFF] dÃ©fense de rÃ©gion
...
function launch_jactari(frame)
{
 ...
 try {
  ...
  for ( k = 0; k < spells.length; k++ )
  {
   ...
   // TODO : Ã§a sert Ã  rien le convert_spell. Parce que (Ã  vÃ©rifier)  convert_spell["ICE_BOLT"] === "ICE_BOLT"
...
function init()
{
 function cb(retour)
 {
  var n, type, frame = this, c = frame.getChildElement('Defender'), cE = API.DOM.createElement,
   ...
   // pillage 10 minutes de dÃ©lai, attaque 0 minutes de dÃ©lai
...
// // API.log('dÃ©lay halte : ' + delay_halte_nuit);
...
// taille d'une coordonnÃ©e X ou Y
'.VR_XY { width:25px }',
...
// boutons OUI/NON d'une fenÃªtre
...
// todo : lolo rendre les noms des clÃ©s plus cohÃ©rentes vrcfg vr_config
...
// les panneaux affichÃ©s/masquÃ©s
...
// todo : trouver l'Ã©lÃ©ment cachÃ© qui fait que quand le hÃ©ro principal fait une attaque, il n'est plus identifiÃ© comme tel.
...
function KO(R)
{
 ...
 container.appendChild(cE('LI', {innerHTML:'Erreur de la rÃ©ponse'}));
...
function frmDistanceCompute(E)
{
 ...
 if ( n && s && c )
 {
  ...
  if ( x1 && y1 && x2 && y2 )
  {
   ...
   if ( dist > 0.1 ) { n.innerHTML = i18n('sentinelleDistance', dist); }
   else { n.innerHTML = ''; }
  }
...
{
 var n = gBI('VRcurTime'), v = H.DateUtils.timestampToString(H.worldMap.getRemoteTimestamp(), H.DATEUTILS_TIME_FORMAT_LOCALE_TIME_STRING);
 if ( n ) { n.textContent = v; }
 return v;
}
function stopTimer() { if ( toid ) { toid = clearInterval(toid); } }
function startTimer() { stopTimer(); toid = setInterval(updateTimer, 1000); }
...
function init()
{
 ...
 try { locale = H.locale != 'fr_FR' ? 'EN' : 'FR'; } catch(ex) {}
 errStructure = i18n('errStructure');
 URL.jactari = URL.jactari + ( locale == 'EN' ? 'fight' : 'combat' );
// try {
