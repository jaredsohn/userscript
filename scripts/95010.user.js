...
// @author			mindfox
// @original_author			PhasmaExMachina
...
// @require        	http://userscripts.org/scripts/source/95008.user.js
// @require       	http://userscripts.org/scripts/source/95007.user.js
// @require         http://userscripts.org/scripts/source/95006.user.js
// @require         http://userscripts.org/scripts/source/95009.user.js

// @version		0.99a
// 
// @history 	0.99a Added Dump building type
// @history 	0.99 Fixed several bugs and removed/replaced malicious required scripts
...
// @history 	0.98 Fixed plug link showing up on advisors with anti-plus enabled // @history		0.97 Added option to hide friends bar
...
ScriptUpdater.check(95010, "0.99a");
...
Config.settings = {
	...
	"Events":{
		...
		fields:{
			...
			eventsEmailHtml:{
				type:'html',
				value:'  <p style="border-top:1px solid #ccc; padding-top:1em !important;"><strong>Email Settings</strong> - see "Email Settings" on <a href="http://userscripts.org/scripts/show/95010" target="_blank">this page</a> for more information.</p>',
			},
			emailNoticeUrl:{
				type:'text',
				label:'Server',
	...
	"About":{
		html:'	<p><a href="http://userscripts.org/scripts/show/' + ScriptUpdater.scriptId + '" target="_blank" style="font-weight:bold !important;">Ikariam ExMachina v' + ScriptUpdater.scriptCurrentVersion +
				...
				'<p>This is a collection of Ikariam hacks by PhasmaExMachina</p>', 		fields:{
			debugMode:{
				type:'checkbox',
				label:'Debug Mode',
			...
			function resourceDetails() {
				...
				$('#cityResources ul.resources li').each(function(i) {
					if (i > 1) {
						...
						var onHand = qty.text().match(/((\d|,|\.)+k?).*$/i)[1];
						var onHandThousands = onHand.match(/k$/g);
						onHand = parseInt(onHand.replace(/[^\d]/g, ''));
						onHand = onHandThousands ? onHand * 1000 : onHand;
						...
						var qtyProtected = (warehouseLevels * 480) + 100; //(numWarehouses * 80);
	...
	parseMessageList:function() {
		...
		function resetNextListener() {
			$('#mainview a[href*="&start="]').click(function(event) {
				event.preventDefault();
			for (var xxyy=0; xxyy < 4; xxyy=xxyy+1) {
		...
		if(Config.get('ajaxNextMessages')) {
				resetNextListener();
		}
	},
...
IAB = {
	...
	init:function() {
		...
		if ($('#buildingUpgrade').size() == 1) {
			...
			else if ($('a.cancelUpgrade').size() == 1)
				var position = $('a.cancelUpgrade').attr('href').match(/position=(\d+)/)[1];
			else 
				var position = $('form#buildForm input[name=position]').attr('value');
...
//GM_log(position);
	...
	views:{
		...
		options:function() {	
			...
			content.innerHTML = '<a name="autoBuild"></a><h3><a href="http://userscripts.org/scripts/show/95010" target="_blank">Ikariam Auto Build' + 
...
IkaNotify = {
	...
	views:{
		options:function() {
			...
			var html = '<a name="ikaNotify"></a><h3><a href="http://userscripts.org/scripts/show/95010" target="_blank" title="More information about this script">Ikariam Notifier</a> \
