...
function getPlayer(qual) {
	url = getURL(qual);
	player =  "<center style='z-index:999'><EMBED type=\"application/x-ms-wmp\" id=\"YTPlayer\" stretchToFit=\"true\" zoom=\"200%\" zoomabsolute=\"2000\" pluginspage=\"http://port25.technet.com/pages/windows-media-player-firefox-plugin-download.aspx\"";
	player += "	width=\""+conf["width"]+"\" height=\""+conf["height"]+"\"";
	player += "	src=\""+url+"\" autostart=\""+conf["autostart"]+"\" loop=\""+conf["loop"]+"\" volume=\""+conf["volume"]+"\"";
	player +=" AllowChangeDisplaySize=\"true\">";
	player += "</EMBED></table>";
...
function getURL(qual) {
	...
	switch(qual) {
		case "hq": return downloadURL + "&fmt=18&asv=2"; break;
		case "hd720": return downloadURL + "&fmt=22&asv=2"; break;
		case "hd1080": return downloadURL + "&fmt=37&asv=2"; break;
		default: return "";
	}
...
if (qual == "flash"){
	playerDiv.innerHTML = keepFlash + "<center>[<a id=\"wmp\">Enable YouTube Windows Media Player</a>]<center>";
		...
		unsafeWindow.yt.www.watch.player.onPlayerSizeClicked("wide");
}
