...
function removeActivity(doc, remove) {
  var settings = GM_config.read('recentSettings');
  forEach($x('.//div[contains(concat(" ", @class, " "), "UIRecentActivity_Stream")]//a[contains(concat(" ", @class, " "), "uiButtonSuppressed")]', doc), 
          function(recent) {
            ...
            if ((typeof settings[story_type] == "boolean" ? !settings[story_type] : GM_config.get('recentHide'))) {
              fbAjax("http://www.facebook.com/ajax/minifeed.php?__a=1", "profile_fbid="+$('profile_id').value+"&ministory_key="+url.match(/story_key=(\d+)&/i)[1]+"&story_type="+story_type+"&revoke_permission=");
              if (remove)
...
if (self.location == top.location)
    var checker=setInterval(function(){
		...
		if(GM_config.get("filteronly") && main.realURL.find("filter=app_"+main.gameID)) {
