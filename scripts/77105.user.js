	...
	for(var i=0;i<listtown.length;i++){
		...
		if(stown){
			...
			if (itime==1) {
				...
				if (linkPoz == 0)	
					stown.parentNode.firstChild.innerHTML = 'т�';
				else {
					if (linkPoz > 3) {
					...
					if (stown.parentNode.firstChild.innerHTML.indexOf('href') == -1) {
						stown.parentNode.firstChild.innerHTML += '<a href="'+ linkOut + '"><b>т�</b></a>';
					}
				}
			}else{
...
function genhint(town,z){
	...
	if (linkPoz == 0)	
		currTown.parentNode.firstChild.innerHTML = 'т�';
	else {
		if (linkPoz > 3) {
			linkOut = 'build.php' + newdID + Links[linkPoz] ;
		} else {
		...
		if (currTown.parentNode.firstChild.innerHTML.indexOf('href') == -1) {
			currTown.parentNode.firstChild.innerHTML += '<a href="'+ linkOut + '"><b>т�</b></a>';
		}
	}
