// ==UserScript==
// @name           Sohma_Kitten Check for glowing tanks.
// @description    Checks tanks Sohma_Kitten wanted for if they are glowing to help her stalk her tank.
// @include        *
// ==/UserScript==
						...
						if(glowT<=gaiaT){
							if(glowT+720>gaiaT){
								...
								if(open===true){
									...
									GM_openInTab('http://www.gaiaonline.com/forum/compose/entry/new/59177783/');
									GM_openInTab('http://absol.site88.net/gtools/glowTime.php?id=458951');
								}
							}
							else{
								//alert('over');
...
function checkTanks(){
	...
	if(time>=GM_getValue('date',time-20)+20){
		...
		checkTank(458951,false);//Sohma_Kitten
	}
}
checkTanks();
