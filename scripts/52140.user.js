// ==UserScript==
// @name           Farmscript for Travian 3.5 (No-skip version by Akdr)
// @author         pixelgeek (update from Akdr)
// @namespace	   FM
// @description    Updated Version from the Version by Akdr
...
// @email          norrlandspuben.se
// @version        1.2.0
// ==/UserScript==

var SCRIPT = {
	url : 'http://userscripts.org/scripts/source/48557.user.js',
	version : '1.2.0' //same value as @version
};
...
//Skipping villages
var skipVillages = false;		//if false, no skipping of farms
var maxSkipCount = -1;			//Max. Number for skipped farms before changing villages, make it -1 to try the whole list
//Map
var farmMarkingFlag = true;		//if false then no farm marking in Map
var farmImage = "http://imgtrav.ifrance.com/img/m/d130.gif"; //d01 d02 d03 d04 is also possible. but might conflict later when you get allies and stuff like that.
//Logging
...
function main() {
	...
	if (document.URL.indexOf("berichte.php") == -1 && GM_getValue("Active_" + suffixGlobal, -1) > -1  && GM_getValue("ReadRep_" + suffixGlobal, -1)==1 )
	{
	...
	*/
	...
	if (url.indexOf('karte.php')>-1 && url.indexOf('karte.php?d=')==-1 && farmMarkingFlag)
	{
				//get center village
	...
	if (notFoundHtml.indexOf(searchStr) > -1) { //village not found or player bannded
		...
		if(arr[3]=="pixelgeek") {
			...
			if (formNode.indexOf("kid") > -1) { //confimation page
				...
				// If random, remove it from queue, else move it to the last position
				if(GM_getValue("RandomFarming_" + suffixGlobal, 0)==1) {
					...
					for (i = 0; i < troopsCount.length; i++) {
						if (parseInt(availableTroops[i]) < parseInt(troopsCount[i])) {
							...
							{						
								...
								setTimeout("window.location.replace('" + rootPath + "a2b.php')",Random());
								if(skipVillages) { //only display message, when it's really skipping to next farm
...
function checkreports() { //analyse new reports
	...
	if ((url.indexOf("berichte.php?id=") != -1 || url.indexOf("berichte.php?newdid=") != -1) ) {
		...
		window.location.replace(rootPath + lastRepPage);
	}
...
function attacktable(){ //analyse report table
	...
	if (rescell != null){
		if(tm!=null){
			...
			FM_log(4,"resText="+rescell.textContent);
			...
			//alert(atstemp[1]);
			//alert(efficiency+"/"+gain+"/"+tribedefender);
...
function stopEngine() {
	//alert("stopping Engine");
	var vList = GM_getValue("FarmerVillages_" + suffixGlobal, "").split(",");
	//alert("length="+vList.length+"/splitvalue="+vList[0].split("~")[1]);
...
function getT35version() {
	...
	FM_log(3,"Travian Version is: "+vT35);
	return;
}
...
function getPlayerId() { //retrieve Player's ID
	var user = dom.xs('//*[@id="sleft"]/p/a[starts-with(@href, "spieler.php?uid=")]');
	var playerID = (user) ? getParamFromUrl(user.href, 'uid') : '';
...
function getActiveVillageXY() { //retrieve coordinates of active Village from Villagelist
	...
	if(vT35 == 1) {
		...
		FM_log(2,"unknown Travian Version");
	}
	...
	if (villageNode.snapshotLength > 0) {
		...
		FM_log(4,"villageTable not existing - Single Village Account");
		xy.push("");
		xy.push("");
	}
	//alert(xy);
...
function insertEditFarmBox() {
	...
	dom.get('FMcancelButton').addEventListener('click', hidePopupFarmEditBox,
			true);
	FM_log(3,"FarmEditBox Inserton done");
