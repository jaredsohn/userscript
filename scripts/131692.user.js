  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
		...
		<tr><td>&nbsp;&nbsp;&nbsp-</td><td>Save inactive Range items: '+ htmlSelector({0:'----', 1:'All slots', 3:'slot 4 or slot 5', 5:'slot 5 only', 8:'slot 4 and slot 5', 9:'slot 3,4 and 5'},Options.RangeSaveModeSetting,'id=selectRangeSaveMode') + '<span style="color:#800; font-weight:bold"><sup> &nbsp *Saves range items</sup></span></td></tr>\
...
function saveUpgradeData (){
  var serverID = getServerId();
  setTimeout (function (){GM_setValue ('UpgradeData_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(upgradeData));}, 0);
}
...
function saveOptions (){
  var serverID = getServerId();
  setTimeout (function (){GM_setValue ('Options_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(Options));}, 0);
}

function saveChatOptions (){
  var serverID = getServerId();
  setTimeout (function (){GM_setValue ('ChatOptions_' + Seed.player['name'] + '_' +serverID, JSON2.stringify(ChatOptions));}, 0);
}
...
function readUpgradeData (){
  var serverID = getServerId();
  s = GM_getValue ('UpgradeData_' + Seed.player['name'] + '_' +serverID);
  if (s != null){
    opts = JSON2.parse (s);
    for (k in opts){
...
function readOptions (){
  var serverID = getServerId();
  s = GM_getValue ('Options_' + Seed.player['name'] + '_' +serverID);
  if (s != null){
    opts = JSON2.parse (s);
    for (k in opts){
      if (matTypeof(opts[k]) == 'object')
...
function readChatOptions (){
  var serverID = getServerId();
  s = GM_getValue ('ChatOptions_' + Seed.player['name'] + '_' +serverID, '[]');
  if (s != null){
    opts = JSON2.parse (s);
    for (k in opts){
      if (matTypeof(opts[k]) == 'object')
...
var DeleteThrone = {
    ...
    startdeletethrone : function(){
		var t = DeleteThrone;
        var rangeNum = [5, 91, 21, 58, 63, 77]; 	// 81 = crafting speed
        var throneSaveNum;
        var countItem = 0;
