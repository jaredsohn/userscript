...
var iCheckEvery = 1000;  // How often do we check for tasks to trigger in miliseconds. 
...
var iPreloadTime = 30;  // How many seconds ahead is the code for building and upgrading prefetched. 
...
if (init) {
    ...
    //Styles
    var cssStyle = "";
    cssStyle += "#ttq_tasklist, #ttq_history {position:absolute; background-color:#90DD43; border:1px solid #C0C0C0; color:#000000; padding:5px 10px; z-index:100; -moz-border-radius:0px; background: #ECECEC; left: 682px; top: 605px;}";
    ...
    cssStyle += ".ttq_time_village_wrapper {font-style:none; font-size:90%; display:block;}";
    ...
    if(GM_getValue("ttq_moveable")==true){
      cssStyle += ".handle {cursor: move;}";
    }
    cssStyle += "a.ttq_sortlink, a#ttq_flush_history {color:#C0C0C0;} a.ttq_sortlink:hover, a#ttq_flush_history:hover {color:#F64809} a.ttq_sortlink_active {color:black}";
    cssStyle += ".ttq_sort_header {}";
...
function toggleMove() {
  if(GM_getValue("ttq_moveable")==true){
    GM_setValue("ttq_moveable", false);
  }
  else{
    GM_setValue("ttq_moveable", true);
  }
  document.location.reload();
	return GM_getValue("ttq_moveable");
}
function toggleHeader() {
  if(GM_getValue("ttq_header")==true){
    GM_setValue("ttq_header", false);
  }
  else{
    GM_setValue("ttq_header", true);
  }
  return GM_getValue("ttq_header");
}
/**************************************************************************
...
function refreshTaskList(aTasks) {
	...
	if(GM_getValue("ttq_moveable")==true){
    oTaskList.innerHTML = "<div id='ttq_draghandle' class='handle ttq_draghandle'>Travian Task Queue - Movable mode</div>";
  }
	
	//Sort links
	var currentSort = getOption("TASKLIST_SORT", 1, "integer");
	var sortLinkWrapper = document.createElement("div");
	sortLinkWrapper.innerHTML += "<span class='ttq_sort_header'>" +aLangStrings[36]+ "</span> ";
...
function onLoad() {
	...
	GM_registerMenuCommand("Travian: Task Queue - Moveable", toggleMove);
