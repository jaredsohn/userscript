...
// @description    TüRkçeDir. EmRe ArsLan ('AnaSoLuasLanI')
// ==/UserScript==
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('DiJiBoT', eventHideShow, mouseMainTab);
  kocWatchdog ();
  WideScreen.init ();
...
Tabs.tower = {
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>KuLE ALARIMLARI</div><TABLE class=pbTab><TR align=center>';
    ...
    m += '</tr></table><BR><DIV><CENTER><INPUT id=pbSoundStop type=submit value="Ses Alarimini Kapat"></center></div><DIV id=pbSwfPlayer></div>';
    m += '<BR><DIV class=pbStat>UYGULAMALAR</div><TABLE class=pbTab>\
        <TR><TD><INPUT id=pbalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Otomatik OLarak Ittifak Sohbetine Gonder.</td></tr>\
        <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
            <TR><TD align=right>Mesaj BasLigi: &nbsp; </td><TD><INPUT id=pbalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
            <TR><TD align=right>Gozcu Uyarisi: &nbsp; </td><TD><INPUT id=pbalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>Bozkir Uyarisi: &nbsp; </td><TD><INPUT id=pbalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
            <TR><TD align=right>En Az Asker Sayisi: &nbsp; </td><TD><INPUT id=pbalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pbalerterr></span></td></tr>\
            </table></td></tr>\
        <TR><TD><BR></td></tr>\
        <TR><TD><INPUT id=pbSoundEnable type=checkbox '+ (Options.alertSound.enabled?'CHECKED ':'') +'/></td><TD>Atak ve GozcuLerde Sesi Oynat</td></tr>\
        <TR><TD></td><TD><DIV id=pbLoadingSwf>SWF Player Yukleniyor</div><DIV style="display:none" id=pbSoundOpts><TABLE cellpadding=0 cellspacing=0>\
            ...
            <TR><TD align=right>Ses: &nbsp; </td><TD><TABLE cellpadding=0 cellspacing=0 class=pbTab><TR valign=middle><TD><SPAN id=pbVolSlider></span></td><TD width=15></td><TD align=right id=pbVolOut>0</td></td></table></td><TD align=center><SPAN id=pbLoadStat>xx</span></td></tr>\
            <TR><TD align=right><INPUT id=pbSoundRepeat type=checkbox '+ (Options.alertSound.repeat?'CHECKED ':'') +'/></td><TD> Repeat every <INPUT id=pbSoundEvery type=text size=2 maxlength=5 value="'+ Options.alertSound.repeatDelay +'"> Dakikalar</td></tr>\
            <TR><TD></td><TD>Oynatılacak <INPUT id=pbSoundLength type=text size=3 maxlength=5 value="'+ Options.alertSound.playLength +'"> Saniyeler</td></tr>\
            <TR><TD></td><TD><INPUT type=submit value="Oynat" id=pbPlayNow></td></tr></table></div></td></tr>\
        </table><BR>';
  	t.myDiv.innerHTML = m;
...
//logit ("NOW="+ now + ' alarmActive='+ Options.alertSound.alarmActive + ' expireTime='+ Options.alertSound.expireTime);
  ...
  e_soundFileLoaded : function (chan, isError){ // called by SWF when sound file finishes loading
    ...
    if (isError)  
      ...
      document.getElementById('pbLoadStat').innerHTML = 'Yuklendi';
  },  
...
Tabs.build = {
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbBuildDivF class=pbStat>INSAA FONKSIYONLARI</div><TABLE id=pbbuildfunctions width=100% height=0% class=pbTab><TR>';
        if (t.buildStates.running == false) {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Otomatik Insaa = Kapali"></td>';
        }
        else {
            m += '<TD><INPUT id=pbBuildRunning type=submit value="Otomatik Insaa = Acik"></td>';
        }
		m += '<TD><INPUT id=pbBuildMode type=submit value="Insa Modu = Acik"></td>';
		m += '<TD>Insaa Turu: <SELECT id="pbBuildType">\
				<OPTION value=build>Yukselt</option>\
				<OPTION value=max>mak. Seviye</option>\
				<OPTION value=destruct>Yoket</option>\
				</select></td>';
		m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>Yardım?</td>';
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=pbStat>INSAA SURELERI</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><B>' + Cities.cities[i].name + '</b></center></td>';
        }
		m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
            m += '<TD colspan=2><CENTER><INPUT id=pbbuild_' + Cities.cities[i].id + ' type=submit value="Goster"></center></td>';
        }
        m += '</tr><TR>';
        for (var i = 0; i < Cities.cities.length; i++) {
    ...
    bot_buildslot: function(c, a){
		...
		if (DEBUG_TRACE) logit("Pos: " + buildingPos + " Tur: " + buildingType + " Seviye: " + buildingLevel + " Id: " + buildingId);
    ...
    _addTab: function(queueId, cityId, buildingType, buildingTime, buildingLevel, buildingAttempts, buildingMode){
        ...
        row.insertCell(6).innerHTML = '<a class="button20" id="queuecancel_' + queueId + '"><span>Iptal</span></a>';
    ...
    toggleStateRunning: function(obj){
        ...
        if (t.buildStates.running == true) {
            ...
            t.saveBuildStates();
            obj.value = "Otomatik Insaa =  Kapali";
        }
        else {
            t.buildStates.running = true;
            t.saveBuildStates();
            obj.value = "Otomatik Insaa =  Acik";
        }
    },
    toggleStateMode: function(obj){
		var t = Tabs.build;
        if (obj.value == 'Insaa Modu = Kapali') {
			unsafeWindow.buildslot = t.bot_buildslot; // overwrite original koc function
            obj.value = "Insaa Modu = Acik";
        }
        else {
			unsafeWindow.buildslot = t.koc_buildslot; // restore original koc function
			obj.value = "Insaa Modu = Kapali";
        }
    },
...
//				13:{'name':"-------",'x':375,'y':375},
    ...
    m = '<DIV class=ptentry><TABLE width=100% class=pbTab><TR><TD class=pbDetLeft>Aranacak: </td><TD width=99%>';
    m += htmlSelector ({0:"Barbar Kamplari", 1:"Bozkirlar", 2:"Sehirler"}, null, 'id=pasrcType'); 
    m += '</td></tr><TR><TD class=pbDetLeft>Koord: </td><TD class=xtab>X=<INPUT id=pasrchX type=text\> &nbsp;Y=<INPUT id=pasrchY type=text\>\
      &nbsp; Uzaklik: <INPUT id=pasrcDist size=3 value=10 /> &nbsp; <SPAN id=paspInXY></span>\
      <TR><TD class=pbDetLeft>Veya:</td><TD>Vilayete Gore Ara: <select id="provinceXY"><option>--Vilayetler--</option>';
    for (var i in Provinces)
    	m += '<option value="'+i+'">'+Provinces[i].name+'</option>';
    m += '</select></td></tr>';
    m += '<TR><TD colspan=2 align=center><INPUT id=pasrcStart type=submit value="Aramaya Basla"/></td></tr>';
    m += '</table></div>\
  ...
  clickedSearch : function (){
    ...
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Gosterilecek Azami Seviye:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Gosterilecek Asgari Seviye:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Bozkir Turu:</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'GoL/Cayir', 3:'Kersteler', 4:'Tepeler', 5:'Dag', 6:'oVa', 0:'Hepsi'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>Sahipsiz:</td><TD class=xtab><INPUT id=pafilUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    }
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Sort By:</td><TD class=xtab><SELECT id=pafilSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
			...
			<TR><TD class=xtab align=right>Sadece Koord.:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
			</table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
    } else {
		m+= '</select></td></tr><TR><TD class=xtab align=right>Sadece Sis:</td><TD class=xtab><INPUT id=pafilMisted type=CHECKBOX '+ (Options.mistedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Sadece Dusman:</td><TD class=xtab><INPUT id=pafilHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Aranacak:</td><TD class=xtab><SELECT id=pafilSortBy>\
  ...
  hideShowClicked : function (){
    ...
    if (div.style.display == 'none'){
      ...
      document.getElementById('spanHideShow').innerHTML = 'K U C U L T';
    } else {
      div.style.display = 'none';
      document.getElementById('spanHideShow').innerHTML = 'B U Y U L T';
    }
  },
  
  dispMapTable : function (){
    var tileNames = ['Barbar Kampi', 'Gol', 'Cayir', 'Keresteler', 'Tepeler', 'Dag', 'Ova' ];
    var t = Tabs.Search;
    ...
    if (dat.length == 0){
      ...
      if (coordsOnly)
        m = '<TABLE align=center id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Konum</td></tr>';
      else {
      if (t.opt.searchType == 2) {
			 m = '<TABLE id=pasrcOutTab class=pbSrchResults cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Loc</td><TD align=right>UZaklik</td><TD>Oyuncu</td><TD align=right>Guc</td><TD>Ittifak</td><TD></td></tr>';
		} else { 
			m = '<TABLE id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Konum</td><TD style="padding-left: 10px">Uzaklik</td><TD style="padding-left: 10px;">Seviye</td><TD width=80%> &nbsp; Tip</td><TD style=""></td></tr>';
		}
	}
      ...
      for (i=0; i<numRows; i++){
        ...
        if (coordsOnly) {
          ...
          if (t.opt.searchType == 2) { // city search
            ...
            if (dat[i][5])
              m += '<TD colspan=4>* MISTED * &nbsp; &nbsp; <SPAN onclick="pbSearchScout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A>Gozcu</a></span></td></tr>';
            else{
              var allStyle = '';
              if (dat[i][12]=='f')
  ...
  stopSearch : function (msg){
    ...
    if (t.opt.searchType==0 && document.getElementById('KOCAttackToggle')!=null){    
      document.getElementById ('pbSrcExp').innerHTML = '<CENTER>'+ strButton20('ToPlu Atak', 'id=pbSrcDoExp') +'</center>'; 
...
// TODO: save memory, remove city name ?   			
    ...
    document.getElementById('pastatSearched').innerHTML = 'Aranan: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('pastatStatus').innerHTML = 'Aranıyor '+ x +','+ y;
  ...
  clickedLookup : function (pid){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>Oyuncu Lookup</b></center>';
    pop.getMainDiv().innerHTML = '<DIV class=pbStat>Sayi Tahtasi Bilgisi</div><SPAN id=pblupLB>Sayi Tahtasi...</span>\
      <BR><DIV class=pbStat>Ittifak</div><SPAN id=pblupAI>Ittifak Bilgisi...</span>';
    pop.show (true);
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Siralama:</b> Bulunamadi! (misted?)<BR><BR>';
      return;
    }
    ...
    var m = '<CENTER><SPAN class=boldRed>NOTE: Liste 24 Saatde Bir Guncelleniyor</span></center><TABLE class=pbTabSome>';
    m += '<TR><TD class=pbDetLeft>Oyuncu Ad:</td><TD>'+ name +'</td></tr>\
      <TR><TD class=pbDetLeft>Guc:</td><TD>'+ p.might +' (rank #'+ p.rank +')</td></tr>\
      <TR><TD class=pbDetLeft>Ittifak:</td><TD>'+ aName +'</td></tr>\
      <TR valign=top><TD class=pbDetLeft>Sehirler:</td><TD><TABLE class=pbTabSome><TR style="font-weight:bold"><TD>Sehir Adi</td><TD>Koord.</td><TD>Level</td><TD>Istatik</td><TD>Kurulum</td></tr>';
  ...
  gotPlayerInfo : function (rslt, span){
    ...
    m += '<TR><TD class=pbDetLeft>Oyuncu Ad:</td><TD>'+ p.genderAndName +'</td></tr>\
      <TR><TD class=pbDetLeft>Might:</td><TD>'+ p.might +'</td></tr>\
      <TR><TD class=pbDetLeft>Facebook Profili:</td><TD><A target="_tab" href="http://www.facebook.com/profile.php?id='+ p.fbuid +'">Yeni Tabda Ac</a></td></tr>\
      <TR><TD class=pbDetLeft>Ittifak:</td><TD>'+ p.allianceName +'</td></tr>\
      <TR valign=top><TD class=pbDetLeft>Vilayetler:</td><TD style="white-space:normal">'+ prov.join(', ') +'</td></tr>';
...
var exportToKOCattack = {
  ...
  doExport : function (coordList, city){
    ...
    var m = '<DIV class=pbStat>KOC Attack Data</div><BR><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabPadNW>\
      <TR style="font-weight:bold; background-color:white"><TD>Hedef Türü</td><TD style="padding:1px" align=center>#<BR>Hedefler</td><TD width=15></td>';
    ...
    if (isKOCattack){
      m += '<BR><CENTER>'+ strButton20('KoC Ataga TopLu Koord. Ekle', 'id=pbSrcDoBA') +'</center>';
    } else {
...
// TODO: WAIT FOR City select ?
    ...
    function doBulkAdd (){
      for (var b=1; b<11; b++){
        ...
        if (tot<1 && cList['lvl'+ b].length>0){
          document.getElementById('ptETerr_'+ b).innerHTML = 'Asker Secilmedi';
          return; 
        } else if (tot>100000) {
          document.getElementById('ptETerr_'+ b).innerHTML = 'Too many troops';
          return; 
...
// WinManager.hideall();      <=== TODO!  
    ...
    function e_attackDialog (tf){
      ...
      document.getElementById('pbSrcExpResult').innerHTML += 'Eklendi '+ list.length +' targets for '+ city.name +'<BR>';
...
Tabs.Attack = {
  ...
  init : function (div){
    var t = Tabs.Attack;
    t.myDiv = div;
    t.myDiv.innerHTML = '<TABLE width=100% height=100% class=pbTab><TR><TD><INPUT id=pbBarbShow type=submit value="Tüm hedefleri Goster" \> <BR>\
  ...
  e_clickedShow : function (){    // show all current attack data
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      for (i=0; i<atks.length; i++)
        m += '<TR><TD>Barbar Kampi '+ atks[i].seviye +'</td><TD>'+ atks[i].x +','+ atks[i].y +'</td><TD> &nbsp; Dist='+ atks[i].dist.toFixed(2) +'</td></tr>';
    }    
...
Tabs.Test = {
  ...
  init : function (div){
    var t = Tabs.Test;
    t.myDiv = div;
    var m = '<TABLE><TR><TD align=right>Gozcu: </td><TD><INPUT type=checkbox id=pbfakeIsScout></td></tr>\
        <TR><TD align=right>Bozkır: </td><TD><INPUT type=checkbox id=pbfakeIsWild></td></tr>\
        <TR><TD align=right>Sahte Rapor: </td><TD><INPUT type=checkbox id=pbfakeFalse></td></tr>\
        <TR><TD align=right>Saniyeler: </td><TD><INPUT type=text size=4 value=300 id=pbfakeSeconds></td></tr>\
        <TR><TD align=right>Milis: </td><TD><INPUT type=text size=6 value=5000 id=pbfakeMilitia></td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=pbtestSendMarch type=submit value="Sahte Atak" \></td></tr></table>\
        <INPUT id=pbReloadKOC type=submit value="KoC YeniLe" \>\
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.getMainDiv().innerHTML = '<CENTER><BR><FONT COLOR=#550000><B>An error has ocurred:</b></font><BR><BR><DIV id=paretryErrMsg></div>\
      <BR><BR><B>Automatically retrying in <SPAN id=paretrySeconds></b></span> Saniyeler ...<BR><BR><INPUT id=paretryCancel type=submit value="IPTAL " \>';
...
Tabs.Gifts = {
  ...
  init : function (div){
    var t = Tabs.Gifts;
    t.myDiv = div;    
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD width=200></td><TD align=center><INPUT id="pasubGifts" type=submit value="Hediyeleri Ara" \></td><TD width=200 align=right><INPUT id=paGiftHelp type=submit value=HELP></td></tr></table><HR>\
  ...
  helpPop : function (){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>Power Bot Help</b>: Hediyeleri Kabul Et</center>';
    pop.show (true);
  },
  ...
  e_clickedGifts : function  (){     // (also cancel accepting)
    var t = Tabs.Gifts;
    if (t.accepting){
      document.getElementById('pasubGifts').value = 'Hediyeleri Ara';
      document.getElementById('giftDiv').innerHTML+= '<BR><SPAN class=boldRed>Iptal Edildi.</span>';
      t.accepting = false;
      return; 
    }
    document.getElementById('giftDiv').innerHTML = 'Facebook Hediyeler Sayfasina Baglaniyor ...';
...
//logit (inspect (t.gifts, 8, 1));     
      ...
      if (t.gifts.length<1){
        document.getElementById('giftDiv').innerHTML = m + '<BR><BR><CENTER>Hediye Bulunamadi!</center>';
        return;
      }
      m += '<TABLE class=pbTab align=center><TR><TD align=right>Kabul Edilecek Alan: </td><TD>'
        + htmlSelector (Options.giftDomains.list, getServerId(), 'id=pbGiftServers') +'</td></tr>\
          <TR><TD align=right>Hediyeler Kabul edildikden Sonra Silinsinmi</td><TD>'
        + htmlSelector ({y:'Her Zaman', e:'Hatali İse', n:'Asla'}, Options.giftDelete, 'id=pbGiftDel')
        + '</td></tr><TR><TD>Select gifts you want to accept and hit: </td><TD width=250><INPUT type=submit id=pbGiftDo value="Hediyeleri Kabul Et">\
        &nbsp; <SPAN id=pbGiftNone class=boldRed></span></td></tr></table><HR><TABLE class=pbTab><TR valign=top><TD>\
        <INPUT id=pbGiftButAll type=submit value="Hepsi" style="width:100%; margin-bottom:5px"><BR><INPUT id=pbGiftButNone type=submit value="Yok"></td>\
        <TD width=10></td><TD><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabLined>\
        <TBODY id=pbGiftTbody>\
        <TR style="font-weight:bold; background:white"><TD>Hediye</td><TD>Tarih</td><TD>Gonderen(aLan)</td><TD width=20></td></tr>';
  ...
  getErDone : function (){ 
    ...
    document.getElementById('giftDiv').innerHTML = '<DIV id=acpDiv style="height:400px; max-height:400px; overflow-y:auto"><B>Kabul ediliyor '+ t.doList.length +' hediye:</b><BR></div>';    
    t.acceptNext ();
  },
  ...
  acceptNext : function (){
    ...
    if (gift == null){
      t.allDone ('Hediyeler Kabul Edildi'); 
      return;
    }
