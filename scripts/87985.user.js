...
// @description It automatically blow the horn for you.
...
function action()
{
	...
	{
		// update timer
		displayTimer("No more cheese!", "No cheese...", "No cheese...", "-");
...
function embedTimer(targetPage)
{
	if (showTimerInPage)
	{
		...
		if (headerElement)
		{
			...
			{
				titleElement.innerHTML = "<b>MouseHunt AutoHorn (version " + scriptVersion + ")</b> - <font color='red'>Aggressive Mode</font>";
			}
			else
			{
				titleElement.innerHTML = "<b>MouseHunt AutoHorn (version " + scriptVersion + ")</b>";
			}
			...
			if (targetPage)
			{
				...
				if (pauseAtInvalidLocation)
				{
					...
					travelElement.innerHTML = "<b>Hunt Location:</b> Please Wait...";
			...
			else
			{
				...
				helpTextElement.innerHTML = "<b>Note:</b> MouseHunt AutoHorn will only function at <a href='http://apps.facebook.com/mousehunt/'>Hunter Camp</a>. This is to prevent the bot from interfering user's activity.";
...
function displayTimer(title, nextHornTime, checkTime, kingTime)
{
	...
	if (showTimerInPage)
	{
		nextHornTimeElement.innerHTML = "<b>Time left to sound horn:</b> " + nextHornTime;
		checkTimeElement.innerHTML = "<b>Next Trap Check Time:</b> " + checkTime;
		kingTimeElement.innerHTML = "<b>Appx. Next King's Reward Time:</b> " + kingTime;
	}
...
{
	// update timer
	displayTimer("Blow The Horn...", "Blow The Horn...", "Blow The Horn...", "Blow The Horn...");
	...
	if (!aggressiveMode)
	{
		...
		if (headerElement)
		{
			...
			else
			{
				...
				// update timer
				displayTimer("Fetching Data...", "Hunter horn not found. Fetching data...", "Hunter horn not found.Fetching data...", "Hunter horn not found. Fetching data...");
...
function afterSoundingHorn()
{
	...
	if (headerElement)
	{
		...
		if (headerStatus.indexOf("hornready") != -1)
		{
			// Problem in sounding the horn
			
			// update timer
			displayTimer("Retrying...", "Retrying...", "Retrying...", "Retrying...");
		...
		else
		{
			...
			// update timer
			displayTimer("Horn sounded. Fetching Data...", "Horn sounded. Fetching data...", "Horn sounded. Fetching data...", "Horn sounded. Fetching Data...");
