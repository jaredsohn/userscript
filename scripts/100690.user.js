// ==UserScript==
// @name          Youtube Link an XBMC senden
// @namespace     http://oyox.de
// @description   Fügt auf Youtube ein Menü für XBMC mit Abspielen, Pause und Stopp hinzu
// @date          2012-04-04
// @creator       Marco H.
// @version       2.3
// @include       *youtube.*/*
...
// Basiert auf dem Script von Erik T. http://userscripts.org/users/278527/scripts

// Für oYoX.de:
// Unterstützung für Chrome hinzugefügt
// Einstellungen-Knopf hinzugefügt
//   inkl. Speicherung der Einstellung
// Sprache übersetzt
// Div. Fehler beseitigt

// Changelog: 04.04.2012 - ID parsing geändert
// Changelog: 11.04.2012 - automatisches Verstecken
...
function modify_xbmc_address() {
	xbmc_address = window.prompt('Die Adresse zum XBMC Web-Interface eingeben:\n(username:passwort@adresse:port)', xbmc_address);
	if (xbmc_address != null){
		GM_setValue("XBMC_ADDRESS", xbmc_address);
	}
	xbmc_address = GM_getValue('XBMC_ADDRESS');
}
		...
		setTimeout(
				function() {
					GM_xmlhttpRequest( {
						...
						data : '{"jsonrpc": "2.0", "method": "XBMC.Play", "params":{ "file" : "plugin://plugin.video.youtube/?action=play_video&videoid=' + video_url.replace('v=', '') + '" }, "id" : 1}'
					})
				}, 250);
	}
}
	...
	setTimeout(
			function() {
				GM_xmlhttpRequest( {
					...
					url : 'http://' + xbmc_address + '/jsonrpc',
					headers : 'Content-type: application/json',
					data : '{"jsonrpc": "2.0", "method": "VideoPlayer.PlayPause", "id" : 1}'
				})
			}, 250);
}
...
function stop_movie() {
	setTimeout(function() {
		GM_xmlhttpRequest( {
			...
			url : 'http://' + xbmc_address + '/jsonrpc',
			headers : 'Content-type: application/json',
			data : '{"jsonrpc": "2.0", "method": "VideoPlayer.Stop", "id" : 1}'
		})
	}, 250);
}
...
function hide_xbmc() {
    document.getElementById('xbmc').style.right='-80px';
}
...
if (clip != undefined) {
	...
	xbmc_play.setAttribute('title', 'Starte Wiedergabe');
	...
	xbmc_pause.setAttribute('title', 'Wiedergabe pausieren');
	...
	xbmc_stop.setAttribute('title', 'Wiedergabe stoppen');

	var xbmc_conf = document.createElement('span');
	xbmc_conf.addEventListener('click', modify_xbmc_address, false);
	xbmc_conf.setAttribute('id', 'btConf');
	xbmc_conf.setAttribute('title', 'Einstellungen');
	...
	xbmc_playback_control.appendChild(xbmc_conf);
...
GM_addStyle('#xbmc { opacity:0.3; width:120px; position:fixed; z-index:100; bottom:30px; right:-80px; display:block; background:#080808; -moz-border-radius-topleft: 20px; -moz-border-radius-bottomleft:20px; -webkit-border-top-left-radius:20px;  -webkit-border-bottom-left-radius:20px; } ')
