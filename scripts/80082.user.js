...
// --------------------------------------------------------------------
// ==UserScript==
// @name           El Bruto Manager
// @version        0.5g.2
// @namespace      http://www.gamertoolz.com
// @description    A suite of extra options for playing El Bruto
...
// @include        http://*gamertoolz.com/mybrute/mybrutemanager/elconfig.php*
...
// - Add: Language Notes and variables to make porting over to meinbrutalo.de and labrute.fr easier
//
// ==/ChangeLog==
...
var brutesite='meinbrutalo.de'; //EspaÃ±ola meinbrutalo.de 
//var brutesite='labrute.fr'; //FranÃ§aise labrute.fr
//var configpage='configure'; //English mybrute.com 
var configpage='elconfig'; //EspaÃ±ola meinbrutalo.de 
...
if(site==brutesite)	{
	...
	if (n[0]){
		...
		//EspaÃ±ola meinbrutalo.de  'Ha ocurrido un error... Â¡brutal!'    'Mantenimiento en curso...'
		//FranÃ§aise labrute.fr                                     'Maintenance technique en cours...'
		if(n[0].innerHTML=='Ha ocurrido un error... Â¡brutal!' || n[0].innerHTML=='Mantenimiento en curso...'){
			pagesuccess=false;
			if(n[0].innerHTML=='Ha ocurrido un error... Â¡brutal!'){
				brutalerror=true;
			}
		}
	}
...
if(page=='cellule' && self==top && pagesuccess){
	...
	if(managed || arched || command=='?add' || command=='?ead'){
		...
		if(n) { var thisname=n[0].innerHTML;
			...
			//EspaÃ±ola meinbrutalo.de <h1>Celda de xxxx</h1>
			...
			//EspaÃ±ola meinbrutalo.de   'Ha ocurrido un error... Â¡brutal!'   'Mantenimiento en curso...'
			//FranÃ§aise labrute.fr                                     Maintenance technique en cours...
			if(thisname=='Ha ocurrido un error... Â¡brutal!' || thisname=='Mantenimiento en curso...')
				thisname=' ';
		...
		//EspaÃ±ola meinbrutalo.de  10,1
		//FranÃ§aise labrute.fr 12,1
		var cell_f =document.getElementsByClassName('areneInfos');
		if(cell_f[0]) cell_fights=parseInt(cell_f[0].innerHTML.substr(10,1));
		...
		if(active_just_battled==1 && parseInt(active_next_battle)>0){
			if(parseInt(active_next_battle)!=cell_fights) {
				...
				if(ld[0]){
					...
					//EspaÃ±ola meinbrutalo.de  'puntos'
					//FranÃ§aise labrute.fr 'point'
					battlegain=news.substring(news.indexOf('puntos')-1,1);
...
if(config_quick_nav==1 && readytofight && cell_fights==0 && brutenum<numbrutes-1){
	...
	if(managed && config_show_quick_fight==1 && cell_fights>0){
		...
		if(quick_opp!='' && quick_opp.length>1){
			if(readytofight){
				...
				//EspaÃ±ola meinbrutalo.de Atacar a
				//FranÃ§aise labrute.fr Attaque
				if(config_name_quick_fight==1) oppname='Atacar a '+quick_opp+'('+quick_level+')<br />';
				...
				//EspaÃ±ola meinbrutalo.de 'Tomando '+quick_opp+'('+quick_level+') ...'
				//FranÃ§aise labrute.fr ''+quick_opp+'('+quick_level+') de saisie ...'
				battlebutton='Tomando '+quick_opp+'('+quick_level+') ...';
			}
		}
		else{
			//English mybrute.com  Complicated Grammar, will address later
			//EspaÃ±ola meinbrutalo.de Complicated Grammar, will address later
	...
	if(config_arch_enemies==1 && numenemies>0){
		...
		//EspaÃ±ola meinbrutalo.de Buscando a
		//FranÃ§aise labrute.fr Recherche de
		enemyaction='var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Buscando a \'+enemy+\'...\'; var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/vs/\'+enemy+\'\';';
		//English mybrute.com Your Enemies
		//EspaÃ±ola meinbrutalo.de Mis Enemigos
		//FranÃ§aise labrute.fr Vos Ennemis
		enemydropdown='<form id="enemyform" name="enemyform" method="post" action="">	<select name="enemychooser" style="width: 125px;" onchange="javascript:var enemy=0; enemy=document.enemyform.enemychooser.options[document.enemyform.enemychooser.selectedIndex].value; if(enemy!=0){ '+enemyaction+' }">    <option value="0">Mis Enemigos</option>';
		for(i=0;i<numenemies;i++){
	...
	controller=controller+'</map>';
		...
		//EspaÃ±ola meinbrutalo.de Volver a la celda de '+GM_getValue('active_current_brute','')+'
		//FranÃ§aise labrute.fr De nouveau Ã  la cellule de '+GM_getValue('active_current_brute','')+'
		controller=controller+'<a href="http://'+GM_getValue('active_current_brute','')+'.'+brutesite+'/cellule" style="font-size:small;">Volver a la celda de '+GM_getValue('active_current_brute','')+'</a>';
	}
			...
			if(config_arena_bonus_preview==1){
			   var hoveraction="";
				...
				if(config_arch_enemies==1){
					...
					//EspaÃ±ola meinbrutalo.de "AÃ±adir '+opp+' a Mis Enemigos"
					//FranÃ§aise labrute.fr "Ajoutez '+opp+' Ã  votre liste d\'ennemis"
					enemybutton='<div id="mbmnmearena'+(i+1)+'" style="width:25px;height:15px;background-color:#FF0000;text-align:center;vertical-align:middle;float:right;overflow:hidden;font-size:xx-small;color:#000000;" onclick="window.open(\'http://'+opp+'.'+brutesite+'/cellule/?ead\',\''+config_arena_enemy_target+'\'); return false;" title="AÃ±adir '+opp+' a Mis Enemigos">ARCH</div>';
               	}
	...
	if(numbrutes>0){
		//English mybrute.com Your Brutes
		//EspaÃ±ola meinbrutalo.de Mis Brutos
		//FranÃ§aise labrute.fr Vos Brutes
		dropdown='<form id="bruteform" name="bruteform" method="post" action="">	<select name="brutechooser" style="width: 125px;" onchange="javascript:var brute=0; brute=document.bruteform.brutechooser.options[document.bruteform.brutechooser.selectedIndex].value; if(brute!=0){ location.href=\'http://\'+brute+\'.'+brutesite+'/'+page+'/\'; }">    <option value="0">Mis Brutos</option>';
		var alias=' ';
		for(i=0;i<numbrutes;i++){
			...
			if(config_arch_enemies==1){
				for(j=1;j<=6;j++){
					if(enemies[i]==arenaopps[j]){
						...
						//EspaÃ±ola meinbrutalo.de "Quitar "+opp+" de mi Mis Enemigos"
						//FranÃ§aise labrute.fr "Enlevez "+opp+" de votre liste d\'ennemis"
                		thebutton.setAttribute("title","Quitar "+opp+" de mi Mis Enemigos");
		...
		if(!pagesuccess){
			...
			if(active_quick_fight_retry>0){
				...
				if(ql[0]){
					...
					//EspaÃ±ola meinbrutalo.de  'No se pudo encontrar a nadie<br />probando de nuevo...'
					//FranÃ§aise labrute.fr 'Incapable de trouver un adversaire<br />Essai encore...'
					ql[0].innerHTML='No se pudo encontrar a nadie<br />probando de nuevo...';
				}
			}
			else if(ql[0]){
				//English mybrute.com
	...
	if(pagesuccess){
		...
		if(sf[0] && ql[0]){
			...
			if(button[0]){
				...
				//EspaÃ±ola meinbrutalo.de Atacar a 
				//FranÃ§aise labrute.fr Attaque
				if(config_name_quick_fight==1) oppname='Atacar a '+opp+'('+opplevel+')<br />';
				GM_setValue('active_quick_fight_loaded',1);
				//English mybrute.com
	...
	if(!pagesuccess && ql[0]){
			...
			if(active_quick_fight_retry>0){
				...
				//EspaÃ±ola meinbrutalo.de  'Incapaz de encontrar a un opositor<br />El intentar otra vez...'
				//FranÃ§aise labrute.fr 'Incapable de trouver un adversaire<br />Essai encore...'
				ql[0].innerHTML='No se pudo encontrar a nadie<br />probando de nuevo...';
			}
			else{
	...
	if(pagesuccess){
		...
		//EspaÃ±ola meinbrutalo.de  Bonus de '+thisname+' Victorias
		//FranÃ§aise labrute.fr Bonifications de '+thisname+' Victoires
		fullpage.innerHTML='<div id="bonuspreview" style="font-size:large;position: absolute; top: 0px; left: 0px;background-color:#FAF8C3;width:217px;height:420px; border:0px; margin: 0px; padding: 0px; overflow:hidden;">Bonus de '+thisname+'<br />'+thisvictory+' Victorias<br /><embed type="application/x-shockwave-flash" src="http://data.'+brutesite+'/swf/uc.swf?v=15" id="inventory" name="inventory" bgcolor="#FAF8C3" quality="high" menu="false" allowscriptaccess="always" flashvars="'+invid+'" height="400" sc wmode="opaque"></div>';
		}
	}
	else{
