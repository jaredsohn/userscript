// ==UserScript==
// @name           Re-Sign Roster 2
// @namespace      goallineblitz.com
// @description    Send Max Salary with 10K Bonus to your roster with No trades.
...
function sendOffers(offers){	
	...
	for (var i=0; i < players; i++){
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://goallineblitz.com/game/make_offer.pl?player_id=' + 
...
response.responseText;
				var maximum = parseInt(document.getElementById

('maximum_salary').innerHTML);
				playerId = document.getElementById

('note_container').nextSibling.nextSibling.value;				
				var offerData = 'action=Send Offer' +
						'&bonus='+10,000 +
						'&contract_type=season' +
						'&daily_salary=' + maximum +
