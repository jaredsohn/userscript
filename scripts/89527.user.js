...
// @description    Publishing Engine helpscript by Robert Buzink
...
if (vknavform) {
	...
	vkresetnav.innerHTML = "<a href=# onClick=\"document.getElementById('navigationItems2.priority').value = '6';document.getElementById

('navigationItems2664.priority').value = '6'\">Reset</a>";
}
...
if (ondertekst) {
    ...
    newElement2.innerHTML = "<p><em><a target='_blank' href='http://twitter.com/volkskrant'>Volg de Volkskrant op Twitter</a></em></p><p><em><a 

target='_blank' href='http://facebook.com/volkskrant'>Word vriend van de Volkskrant op Facebook</a></em></p><p><em><a target='_blank' 

href='http://www.volkskrant.nl/vk-online/'>Lees meer in de Volkskrant</a></em></p>";
...
if (vkauteur) {
    ...
    sourcelinks.innerHTML = "<a href=\"#\" onClick=\"javascript:document.getElementById('author').value = \'ANP\';\">ANP</a> <a href=\"#\" onClick=

\"javascript:document.getElementById('author').value = \'Novum\';\">Novum</a> <a href=\"#\" onClick=\"javascript:document.getElementById('author').value 

= \'AP\';\">AP</a> <a href=\"#\" onClick=\"javascript:document.getElementById('author').value = \'Novum/AP\';\">Novum/AP</a> <a href=\"#\" onClick=

\"javascript:document.getElementById('author').value = \'ANP/Reuters\';\">ANP/Reuters</a> <a href=\"#\" onClick=\"javascript:document.getElementById

('author').value = \'Van onze verslaggever \' + document.getElementById('author').value;\">Ver</a> <a href=\"#\" onClick=

\"javascript:document.getElementById('author').value = \'Van onze correspondent \' + document.getElementById('author').value;\">Cor</a> <a href=\"#\" 

onClick=\"javascript:document.getElementById('author').value = \'Van onze redactie \';\">Red</a>";
}
...
//		newElement.innerHTML = "ANP";	
	        ...
	        vkbody.value = vkbody.value.replace(/\'\'/gi, '\'');
                ...
                //Myanmar en al-qaeda etc
                vkbody.value = vkbody.value.replace(/Myanmar/gi, 'Birma');
                vkbody.value = vkbody.value.replace(/al-qaeda/gi, 'Al Qaida');
                vkbody.value = vkbody.value.replace(/Myanmar/gi, 'Birma');
                vkbody.value = vkbody.value.replace(/al-qaeda/gi, 'Al Qaida');
                vkbody.value = vkbody.value.replace(/Kaddafi/gi, 'Kadhafi');
                vkbody.value = vkbody.value.replace(/Gaddafi/gi, 'Kadhafi');
                vkbody.value = vkbody.value.replace(/Gadhafi/gi, 'Kadhafi');
                vkbody.value = vkbody.value.replace(/Muammar/gi, 'Moammar');
...
function copyToClipboard(s)
			{
				...
				{
					// You have to sign the code to enable this or allow the action in about:config by changing user_pref
...
var instrumentTextarea = function(textarea) {
 var centerTextarea = function() {
    ...
    var textareaKeydown = function(e) {
		...
		//cleans up the mess before text of ANP
		//var regpl1 = /\n[ ]?[A-Z][A-Z]+[ ]\(/g
		//var regpl1 = /[A-Z][A-Z ]+[ ]\(/g
		var regpl1 = /[A-Z][A-Z ]+[ ]\([A-Z\/]+\) (-|—) /g
		if (vkintro.value.match(regpl1)) {
			var plmatch = vkintro.value.match(regpl1);
			var plsplit = vkintro.value.split(plmatch);
			vkintro.value = plsplit[1];
			//vkintro.value = plmatch + plsplit[1];
		}

		//cleans up the mess before text of AP
		var regpl2 = /.*— /g
		if (vkintro.value.match(regpl2)) {
			var plmatch2 = vkintro.value.match(regpl2);
			var plsplit2 = vkintro.value.split(plmatch2);
			vkintro.value = plsplit2[1];
		}
