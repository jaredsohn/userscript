// ==UserScript==
// @name           Ikariam General Helper (Tristan mod)
// @version        0.36.8.1
// @namespace      krwq_ikariam
// @description    Ikariam General Helper (Tristan mod) - some formatting additions to the formatted message including headers for the columns and red text. Users should more readily recognize this as incoming attacks because it follows a similar presentation to the Military Advisor when incoming attacks are displayed.

// @include        http://s*.ikariam.*/index.php?view=embassyGeneralAttacksToAlly*
// @include        http://s*.ikariam.*/?view=embassyGeneralAttacksToAlly*

// @history        0.36.8.1 (Tristan) removed "sum of all troops" in favour of actual attack records and image types with (open) listed in front of ongoing attacks...also consolidated list of attacks into shorter output to stay within ikariam message character restriction of 8000 chars
// @history        0.36.8 (Tristan) added red text format and column headings to message output
...
const currversion = 14;

// rzeczywista wersja gry
ScriptUpdater.check(66604, '0.36.7');
  ...
  "co.il": // Dannidin
   {
    ...
    /* Report */
    report:            "דוח",
    sumofenemiesunits: "מספר אוייבים' יחידות",
...
If you want to help in translating, please write a topic <a href="http://userscripts.org/scripts/discuss/66604">here</a>.<br>\
...
/*************************************************************************************************/
var sheet,sheetdebug,sheetreport;
function onJQLoaded() { /* część główna skryptu */

// możliwość rozwijania/zwijania opcji
// zrobić funkcję pokaż/ukryj obiekt
sheet = document.createElement('style');
sheet.innerHTML = "tr.igh_embassy_options { display: none }";
...
sheetreport = document.createElement('style');
sheetreport.innerHTML = "tr.igh_embassy_report { display: none }";
document.body.appendChild(sheetreport);

/*************************************************************************************************/

// jeśli rozwinięte to rozwiń
if (expanded_options)
  sheet.innerHTML = "tr.igh_embassy_options { display: table-row }";
  
if (expanded_report)
  sheetreport.innerHTML = "tr.igh_embassy_report { display: table-row }";
...
AddContent(content);
document.getElementById("igh_report_textarea").innerHTML=AttacksToReport(attacks);
...
function AttacksToReport(att)
{
  ...
  if (att.length==0)
   ...
   {
    t+='<MessageFormatter><table><tr><td><font color=black><b>Type</b></font></td><td><font color=black><b>Attacker</b> (units)</font></td><td><font color=black><b>Defender</b> (townid)</font></td><td></td><td><font color=black><b>ETA / Next Round</b></font></td></tr><tr><td colspan="5"><hr></td></tr>';
    for (var i=0; i<att.length; i++)
     {
      if (att[i].active)
       {
...
//        t+='<td><a href="'+LINK_CITY+att[i].cityid+'"><b>'+att[i].player+' ('+att[i].cityid+')</b></a></td></tr>';
	  ...
	  t+='<tr><td>';
          if (att[i].attacks[j].type==LANG.blockharbour)
            t+=IMG_BLOCKADE+"(open)"; else
          if (att[i].attacks[j].type==LANG.pillage)
            t+=IMG_PLUNDER+"(open)"; else
          if (att[i].attacks[j].type==LANG.occupytown)
            t+=IMG_OCCUPY+"(open)"; else
          t+="</td>";
          t+='<td><a href="'+LINK_CITY+att[i].attacks[j].cityid+'"><font color=red><b>'+att[i].attacks[j].player+'</b></font></a> <font color=red>('+att[i].attacks[j].units+')</font></td>';
          t+='<td><a href="'+LINK_CITY+att[i].cityid+'"><font color=red><b>'+att[i].player+'</b> ('+att[i].cityid+')</font></a></td>';
          t+="<td>"+IMG_TIME+"</td>";
          t+="<td><font color=red>"+att[i].attacks[j].time+"</font></td>";
          t+="</tr>";
         }
       } else
        for (var j=0; j<att[i].attacks.length; j++)
...
function toggleexpanded_options()
{
  ...
  if (expanded_options)
    sheet.innerHTML = "tr.igh_embassy_options { display: table-row }"; else
    sheet.innerHTML = "tr.igh_embassy_options { display: none }";
}
...
function toggleexpanded_report()
{
  ...
  if (expanded_report)
    sheetreport.innerHTML = "tr.igh_embassy_report { display: table-row }"; else
    sheetreport.innerHTML = "tr.igh_embassy_report { display: none }";
}
...
function AddContent(content)
{
  ...
  //$("#igh_embassy").show("slow");
  $("#igh_embassy").fadeIn();

  // miejsce wstawienia ramki - stare
  /*if (igh_options[o_positiontop][op_value]) // nad atakami
   {
    var el = document.getElementsByClassName("contentBox01h")[0];
    var p = el.innerHTML.indexOf("<br>")+4;
    el.innerHTML = el.innerHTML.substr(0,p) + t + el.innerHTML.substr(p);
   } else
   { // pod atakami
    var el = document.getElementsByClassName("contentBox01h")[0];
    var p = el.innerHTML.indexOf("</div>",el.innerHTML.indexOf("</table>"))+6;
    el.innerHTML = el.innerHTML.substr(0,p) + t + el.innerHTML.substr(p);
   }*/
...
{
  var p = posB(location.host,".ikariam.",0);
  if (p==-1)
   {
    // error
    return "";
