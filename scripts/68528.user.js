...
var server = serverLang + serverType;
...
function run()
{
	...
	if(comString.innerHTML == "new user")
	{
		...
		var userName = "";
		var userPass = "";
		if(userName = prompt('Add meg a felhasználó nevét',''))
		{
			if( userPass = prompt('Add meg a felhasználó jelszavát',''))
			{
				...
				alert(userName + " sikeresen felvéve!");
	...
	{
		comString.innerHTML = "semmi";
		if(confirm("Az összes felvett felhasználó elveszik!\nFolytatja?"))
		{
			GM_setValue(server+"num","0");
			window.location = window.location.toString();
		}
	...
	else if(comString.innerHTML.substring(0,5) == "login")
	{
		...
		window.location = "http://"+serverType+".travian."+serverLang+"/login.php";
	}
}
...
function createMenu()
{
	...
	userMenu.innerHTML = "<hr>Users:<br>";
	...
	js += "else ";
	js += "alert('Másik folyamat zajlik');";
	userMenu.innerHTML += "<a href=\"javascript: void(0);\" onclick=\"" + js + "\">New User</a>\n";
	if(GM_getValue(server+"num","0") != "0")
	{
		...
		js += "else ";
		js += "alert('Másik folyamat zajlik');";
		userMenu.innerHTML += "<a href=\"javascript: void(0);\" onclick=\"" + js + "\">Delete all</a>\n";
	}
}
