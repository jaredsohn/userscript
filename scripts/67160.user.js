...
// @namespace   Travian
// @author 	congxz6688,deFox,Tuga, v_kir
// @version 	1.2.4.7.9
...
var crtVersion="Ver 1.2.4.7.9";

/**
 * Date:
 *   30 Jun 2009 - 30 Jan 2010
 * Authors:
 ...
 *   Earlier changes log: http://userscripts.org/scripts/show/67160
 */
...
//					var maxx = aDoc.evaluate('//*[@class="max"]', aElem, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
					var texts= aDoc.getElementsByClassName("text");
//					var texts= aDoc.evaluate('//*[@class="text"]', aElem, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);

					for(i in texts){
//					for(var i = 0; i < texts.snapshotLength; i++){
						maxNum = maxx[i].innerHTML.split("(")[1].split(")")[0];
//						maxNum = maxx.snapshotItem(i].innerHTML.split("(")[1].split(")")[0];
...
*/					var input = abDoc.evaluate("//input", abElem, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
					...
					for(var i = 0; i < input.snapshotLength; i++){
						...
						if (/id|a|sz|kid|r[1-4]/.test(paramName)) {
	...
	function createTaskListTable(){
		...
		if (taskss != "false") {
			...
			aTd5.setAttribute("colspan", "4");
				...
				aTd7.align="right"
				aTd7.innerHTML="&#160;&#160;"+"<a href='http://userscripts.org/scripts/show/67160' id='verdisp'  target='_blank'>"+crtVersion+"</a>"
