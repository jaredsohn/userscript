...
function test_warner(){
	if(document.getElementById('warnbild')){
		...
		document.body.removeChild(document.getElementById('warndiv'));
		document.getElementById('test_warner').value = 'Testbild';
	}else{
		testbild = true;
		warner_date[0] = document.getElementById("ab_top").value;
		warner_date[1] = document.getElementById("ab_left").value;
		warner_date[2] = document.getElementById("bild_h").value;
		warner_date[3] = document.getElementById("bild_b").value;
		warner_bild();
		int_bewegung();
...
function warner(){
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://malle.pennergame.de/fight/overview/',
		onload: function(responseDetails) {
			var content = responseDetails.responseText;
			//http://berlin.pennergame.de/profil/id:2286149/
			if(content.match(/warning/)){
				if(one_sound){
					var content_split = content.split('Zeitpunkt')[1].split('eintreffende Kämpfe')[0];
...
function warner_bild(){
	...
	warndiv.setAttribute('style', 'position:absolute; top:'+warner_date[0]+'px; left:'+warner_date[1]+'px; z-index:100;height:'+warner_date[2]+'px;width:'+warner_date[3]+'px;');
	if(!testbild){
		warndiv.setAttribute('onclick', 'window.location.href=\'/fight/overview/#form1\'');
		warndiv.style.cursor = 'pointer';
	}else
		warndiv.style.cursor = 'move';
	document.body.appendChild(warndiv);
}
function int_bewegung(){
	document.getElementById("warndiv").addEventListener('mousedown', dragstart, false);
	document.addEventListener('mousemove', drag, false);
	document.addEventListener('mouseup', dragstop, false);
	//document.getElementById("warnbild").onmousedown = dragstart(this);
	//document.getElementById("warnbild").onmousemove = drag;
	//document.getElementById("warnbild").onmouseup = dragstop;	
}
function dragstart() {
   //Wird aufgerufen, wenn ein Objekt bewegt werden soll.
	element = document.getElementById("warndiv");
  dragobjekt = element;
...
function drag(ereignis) {
	...
	if(dragobjekt != null) {
		warner_date[1] = posx - dragx;//left
		warner_date[0] = posy - dragy;//top
		dragobjekt.style.left = warner_date[1] + "px";
		dragobjekt.style.top = warner_date[0] + "px";
		document.getElementById("warnbild").style.left = warner_date[1] + "px";
		document.getElementById("warnbild").style.top = warner_date[0] + "px";
		GM_setValue("warnerdaten1", warner_date[1]);
		GM_setValue("warnerdaten0", warner_date[0]);
		document.getElementById("ab_left").value = warner_date[1];
		document.getElementById("ab_top").value = warner_date[0];	
	}
}
