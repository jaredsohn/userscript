...
(function () {
	...
	langData['fr'] = {
		...
		menuOgame_decolorerMarchand: 'Décolorer Marchand & Officiers', // remove color of trader / officer
		// page recherche
	...
	var authorsData = {
		// outil d'acc�l�ration messagerie
	...
	function showInsideMenu(posElement, menu) {
		...
		if (menu == "optionsRCDetaille") {
			insideMenuGlobalDiv.appendChild(insideMenuTitle("Options des Rapports de Combats D&eacute;taill&eacute;s"));
		}
		...
		if (menu == "optionsBatiments") {
			insideMenuGlobalDiv.appendChild(insideMenuTitle("Options des B&acirc;timents"));
	...
	if(document.getElementById('metal_box')) {
	...
	// On affiche le MonkeyTool's que dans les pages o� le premier ul contient logout
	...
	if (monkeyLoad('oMO_Decolor_Link') && menuOfficiers) {
		menuOfficiers[0].className = 'menubutton';
		menuOfficiers[1].className = 'menubutton';
	}

	if (monkeyLoad('oMO_Remove_Marchand') && liOfficiers) {
		liOfficiers[0].parentNode.removeChild(liOfficiers[0]);
	}

	if (monkeyLoad('oMO_Remove_Officiers') && liOfficiers) {
		liOfficiers[1].parentNode.removeChild(liOfficiers[1]);
	}
	...
	if (monkeyLoad('oPO_Accelerer_Messagerie')) {
		(function(){
			...
			if(document.getElementsByTagName('link')[5]) {
				document.styleSheets[0].deleteRule(5);
				document.styleSheets[0].insertRule(":focus"+"{"+"outline:none;"+"}", 5);
			}
		})()
	}
	...
	if (monkeyLoad('oPF_Points_Expe') && globalURLAction == 'fleet1')
	{
		(function(){
			...
			var expeIcon = document.createElement('img');
				...
				expeIcon.style.marginTop = '16px';
				
			
			var newInput = document.createElement('div');
				newInput.style.position = 'relative';
				newInput.style.marginTop = '20px';
				newInput.style.marginLeft = '18px';
				newInput.style.width = '42px';
			...
			// eventListeners de modification des forms
			for (i = 0,j = 0; ogameInputs[i]; i++) {
				...
				if (ogameInputs[i].className) {
					...
					ogameFleetInputs[j].addEventListener("mouseup", countExpePointsWait, false);
					j++;
				}
			}
			for (i = 0,j = 0; ogameAnchors[i]; i++) {
				if (ogameAnchors[i].className == 'max tips') {
					...
					ogameMaxAnchors[j].addEventListener("mouseup", countExpePointsWait, false);
					j++;
				}
			}
			document.getElementById('sendall').addEventListener("mouseup", countExpePointsWait, false);
			// et un petit setInterval pour quand les actions ça passe pas bien :P
			window.setInterval(countExpePoints, 500);
