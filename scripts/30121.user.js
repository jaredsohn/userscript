// ==UserScript==
// @name           Extra citations for oper.ru' comments, v.0.0.2
// @namespace      oper.ru
...
function mkAuthorLink(ref, label)
{
    ...
    if(label == 'Goblin' || label == 'FVL') newElt.style.color = '#ff2020';
    return newElt;
}
...
{
    var newElt = document.createElement('a');
    var pageIdx = Math.round((label - 1) / 100 - 0.5);
    if(pageIdx > 0)
        newElt.href = ref + '&page=' + pageIdx + '#' + label;
    else
        newElt.href = ref + '#' + label;
...
function makeCitations()
{
    ...
    */ 
    ...
    for(var j in a) {
        ...
        if(el2.innerHTML.indexOf('FVL') >= 0 || 
                el2.innerHTML.indexOf(lblFVL) >=0 ||
                el2.innerHTML.indexOf('viklequick') >=0 ||
                el2.innerHTML.indexOf('Goblin') >=0 ) {
     ...
     */
        ...
        links.sort();
        for(var z in links) {
            xdiv.appendChild(mkRefLink(baseref, links[z], j));
            xdiv.appendChild(document.createTextNode(' '));
        }
        if(links.length > 0) xdiv.appendChild(mkLink('6,' + j, lblHide));
        xdiv.appendChild(document.createElement('br'));
    ...
    if(event.target.href) {
        ...
        if( j >=0 ) {
            var kind = event.target.href.substring(j + 17, j + 18);
                ...
                var k = event.target.href.indexOf(')', j + 18);
                kind = event.target.href.substring(j + 19, k);
                hideThread(kind);
            }
        }
    }
