...
// @namespace      noobs
// @version        20111211
// @description    Powertools for Global Warfare (search limitation updated)
...
//beginning test script addition
  ...
  clickedSearch : function (){
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	 ...
	 m += '<TR><TD class=xtab align=right>Sort By:</td><TD class=xtab><SELECT id=filSortBy>\
			...
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
                        <OPTION value="level" '+ (Options.srcSortBy=='Alliance'?'SELECTED':'')  +'>Alliance</option>\
           </select></td></tr>\
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      ...
      if (numRows > 64000 && t.searchRunning){
        numRows = 64000;
        document.getElementById('srchSizeWarn').innerHTML = '<FONT COLOR=#600000>NOTE: Table only shows 64000 of '+ dat.length +' results until search is complete.</font>';
      }
      for (i=0; i<numRows; i++){
	    m += '<TR valign="top"';
