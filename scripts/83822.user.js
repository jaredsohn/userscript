...
function resolveBettorType(name){
	...
	for(i=0;i<LATE PAYMENT.length;i++){
		if(name == LATE PAYMENT[i]){
			return 'LATE PAYMENT';
		}
	}
	
	for(i=0;i<LOAN WELCHER.length;i++){
		if(name == LOAN WELCHER[i]){
			return 'LOAN WELCHER';
		}
	}
...
div.innerHTML += '<br/><b><a href="http://forums.d2jsp.org/forum.php?f=104"' + 
	...
	switch(resolveBettorType(user)){
		case 'LATE PAYMENT':
			div.innerHTML += '<font color="chartreuse"><b>LATE PAYMENT</b></font>';
			break;
		case 'LOAN WELCHER':
			div.innerHTML += '<font color="black"><b>LOAN WELCHER</b></font>';
			break;
		case 'unknown':
...
function getNames(){
	...
	{
	    method: 'GET',
	    url: 'http://loanlist.angelfire.com/',
	    headers: 
		{
