...
// @author         jonls (I just adapted it to PtP - dark_skeleton)
// @include        http://*passthepopcorn.org/*
// @include        https://*passthepopcorn.org/*
...
(function() {
	...
	GM_registerMenuCommand('PassThePopcorn Advanced Buffer Calculator: '+(display_in_header ? 'Hide buffer in header' : 'Show buffer in header'), function() {
	...
	GM_registerMenuCommand('PassThePopcorn Advanced Buffer Calculator: '+(use_ratio_coloring ? 'Use simple ratio coloring' : 'Use advanced ratio coloring'), function() {
	...
	if (/passthepopcorn\.org\/user\.php\?id=/.test(document.URL)) {
		/* Find stats box */
		...
		else if (user_up < 1*TiB) append_transition_date(1*TiB, 'TorrentMaster');
	}
