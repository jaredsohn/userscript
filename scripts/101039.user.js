// ==UserScript==
// @name           KoC Advanced PowerBot
// @namespace      KoC Advanced PowerBot
// @homepage         http://userscripts.org/scripts/show/101039
...
// @require          http://tomchapin.me/auto-updater.php?id=101039
// ==/UserScript==


var Version = '20110418';
...
function pbStartup (){
  ...
  window.name = 'PT';
  logit ("* KoC Advanced PowerBot v"+ Version +" Loaded");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  actionLog ("KoC Advanced PowerBot v"+ Version +" Loaded  (KofC version: "+ anticd.getKOCversion() +")");
  ...
  AddMainTabLink('PowerBot', eventHideShow, mouseMainTab);
  kocWatchdog ();
  WideScreen.init ();
...
Tabs.trader = {
  ...
  tradeRoutes: [],
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbTowrtDivF class=ptstat>Currently in progress...</div><TABLE id=pbtraderfunctions width=100% height=0% class=pbTab><TR align="center">';
        if (t.traderState.running == false) {
            m += '<TD><INPUT id=pbTraderState type=submit value="Transports = OFF"></td>';
       }
        else {
            m += '<TD><INPUT id=pbTraderState type=submit value="Transports = ON"></td>';
        }
		m += '<TD><INPUT id=pbShowRoutes type=submit value="Show Routes"></td>';
        m += '</tr></table></div>';
		m += '<DIV id=pbTraderDivD class=ptstat>Add Transport</div>';
...
function KOCnotFound(secs){
  ...
  div.innerHTML = '<DIV style="font-size:18px; background-color:#a00; color:#fff"><CENTER><BR>KoC Advanced PowerBot has detected that KOC is not loaded<BR>\
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  pop.centerMe(mainPop.getMainDiv());
  pop.getTopDiv().innerHTML = '<CENTER>KoC Advanced PowerBot</center>';
