// JavaScript Document
// ==UserScript==
// @name           Acuerdos culturales para Ikariam
...
// @description    Permite encontrar acuerdos culturales disponibles entre los usuarios que tienen el script instalado.
...
GoodsHelper = {
	...
	init:function() {
		...
		$.post('http://www.fioleta.com/check_update.html', pr, function(data) {
		  if (data.update=="yes") {
			  $('li.notes').html('<a href="http://www.fioleta.com/" title="Cultural Goods finder" target="_blank"><span class="textLabel" id="fioleta_CG">AcCG finder</span></a>');
		  } else {
				...
				$('li.notes').html('<a href="javascript: void(0);" title="Cultural Goods finder"><span class="textLabel" id="fioleta_CG">Acuerdos culturales (<span id="free_CG">0</span>)</span></a>');
	...
	GoodsDisplay: function() {
		$.get("http://www.fioleta.com/getCGlist.html?userID="+ GoodsHelper.config.userID +"&server=" + GoodsHelper.config.SERVER, function(data2){
			var text='<div class="yui-skin-sam" style="z-index: 9999999999; position: absolute;"><div id="examplecontainer" ><div id="resizablepanel" style="height: 500px;"><div class="hd"><div class="div1"><div class="div2"><p>Bienes culturales                <span><a href="javascript:goodsFinder()"><img src="skin/layout/notes_close.gif"/></a></span></p>                </div></div></div>                <div class="bd"><div class="rightborder"><div id="messageBox"><span id="message">' + data2  + '</span></div> </div></div> <div class="ft"><div class="bottom"><span class="chars" id="chars"></span> script by <a href="http://www.fioleta.com" target="_blank">Fioleta</a>\'2010  <span id="send_data">¡Espera, por favor! No cierres las ventanas.</span></div> </div> <center><div class="button2"><a href="javascript:goodsFinder()" class="button notice">Aceptar</a></div></center> </div> </div> </div>';
	...
	getUserInfo: function () {
		...
		$.get("http://" + GoodsHelper.config.SERVER + ".ikariam.com/index.php?view=options", function(data){
			...
			if (userID>0)
			{
				...
				alert("¡No se puede ejecutar el script!");
				return false;
			}
		}); 
	},
			...
			$.get("http://" + GoodsHelper.config.SERVER + ".ikariam.com/index.php?view=city&id="+d.value, function(data){
				...
				if (position!=null)
				{
					$.get("http://" + GoodsHelper.config.SERVER + ".ikariam.com/index.php?view=museum&id="+d.value+"&position="+position[1], function(data2){
						...
						$("#send_data").text("Enviando "+temp_count);
						if ( countCities==temp_count )
						{
							GoodsHelper.update_CG();
							$("#send_data").text("Enviando los datos.");
						}
					}); 
				} else {
					temp_count++;
	...
	update_CG: function () {
		...
		$.post('http://www.fioleta.com/updateCG.html', pr, function(data) {
		  if (data.issave!="yes") {
			  alert("¡Error en el guardado de tu estado!");
			  return;
		  } 
		  $("#send_data").text("Tus datos se han almacenado.");
		  GoodsHelper.config.freeGoods=0;
		}, "json");
	},
	getfreeCG: function () {
