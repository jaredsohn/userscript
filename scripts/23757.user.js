...
if (ogs_init != true)
{
    ...
    var neueseite_user = '';    //enthält den namen wer die seite zurzeit benützt z.b. 'farmliste' und *sperrt* somit den zugriff für andere
                                //nur frei wenn '' leer.
    ...
    // Es muesen alle Seiten einen page Eintrag bzw. bis auf "renameplanet" eine SID haben, "ainfo" und "bericht" werden damit ausgeschlossen
...
function ogameskript()
{
		...
		for (var i = 0; i < Links.length; i++) // Ueber alle max-Links
		{
			...
			if (ID == 202 || ID == 203 || ID == 209) // fuer kleine und grosse Transporter
			{
				...
				if (Vorhanden < BenSchiffe) // evntl. Warnung einfuegen und Link roeten
				{
					...
					//}
					var farbe = 'color: red;';
                    tooltiptext += 'erforderlich: ' + BenSchiffe + '<br />vorhanden: ' + Vorhanden + '<br />fehlend: <font style=\\\''+farbe+'\\\'>'+(BenSchiffe - Vorhanden)+'</font>';
				}
				else
                {
				...
				Links[i].parentNode.appendChild(NeuesElement('a', 'ben', 'href', url, 'onmouseover', 'return overlib(\''+tooltiptext+'\');', 'onmouseout', 'return nd();', 'style', farbe)); // Link einfuegen
			}
		}
	...
	if (Datei == 'overview')
	{
		...
		if (ogs_load('bewegteress') == true)
		{
			...
			code += '<tr><td class="c" colspan="4">Ressources en transit</td></tr>';
			code += '<tr><th><b>Ressources</b></th><th><b>';
			code += "<font color='"+vColorRess[0]+"'>Métal</font></b></th><th><b>";
			code += "<font color='"+vColorRess[1]+"'>Cristal</font></b></th><th><b>";
			code += "<font color='"+vColorRess[2]+"'>Deutérium</font></b></th></tr>";
			...
			code += '<tr><th><span style="color:' + TextCol + ';">Totaux:</span></th><th><span style="color:' + vColorRess[0] + ';">' + TausenderZahl(met) + '</span></th><th><span style="color:' + vColorRess[1] + ';">' + TausenderZahl(kris) + '</span></th><th><span style="color:' + vColorRess[2] + ';">' + TausenderZahl(deut) + '</span></th></tr>';
			code += '</table></center>';
		...
		if (ogs_load('handelsrechner') == true)
		{
			function handelsrechner()
			{
				...
				handelstext = 'J\'aurais à proposer ' + TausenderZahl(menge) + ' ' + hertauschen + '  contre ' + handel + ' haben (Taux: ' + kursmet.replace(/\./g, ',') + ':' + kurskris.replace(/\./g, ',') + ':' + kursdeut.replace(/\./g, ',') + ')';
			...
			code2 += '<tr><th colspan="5" style="text-align:center; padding:10px;" id="handelstext">J\'aurais à proposer 0 Deuterium contre 0 Métal et 0 Cristal (Taux: ' + ogs_load('kursmet').replace(/\./g, ',') + ':' + ogs_load('kurskris').replace(/\./g, ',') + ':' + ogs_load('kursdeut').replace(/\./g, ',') + ')</th></tr>';
	...
	if (Datei == 'messages')
	{
	    ...
	    function oldspiostuff(thisthing)
	    {
			...
			// Ressourcen
			if (a1.indexOf('Métal:') + 1)
			{
			    var met = SpioWert('Métal:');
			...
			code += '<br>M&ouml;gliche Beute: <span style="color:' + TextCol + ';">' + TausenderZahl(Math.round(met / 2)) + ' Métal</span>, <span style="color:' + TextCol + ';">' + TausenderZahl(Math.round(kris / 2)) + ' Cristal</span>, <span style="color:' + TextCol + ';">' + TausenderZahl(Math.round(deut / 2)) + ' Deutérium</span>';
...
function ogs_setup_start()
{
    ...
    function ogs_setup_save()
    {
	    ...
	    // Speichern fertig
	    window.alert('Einstellungen wurden erfolgreich gespeichert! Bitte beachte, dass die Einstellungen nur f'+ue+'r den aktuellen Account und den aktuellen Computer gelten und erst mit dem n'+ae+'chsten Seitenaufruf angezeigt werden.');
	    ogs_setup_close();
    }
	...
	// Allgemein Config
	createHeader('Général', 'Sur cette partie, tu peux modifier les option général.');
    createColTxt('skriptcolor','Couleur de script','Couleur de texte pour les extensions de script insérées');
	createEdit('max_tab_breite','Largeur de tableaux','Largeur maximal des tableaux en Pixel (entre 520 et 1.000)',5,4);
	createCheck('left_menu_fix', 'Menu gauche fixe', 'Le menu gauche fixe (si de manière activée, des parties du menu sous le bord inférieur ne peuvent plus être accessibles, vous pouvez peut être le réglé par Skin)');
	createCheck('endzeiten','Fins du monde','Des fins du monde indiquent (les Bâtissant, l\'Arrivée de flottes, etc.)');
	createColTxt('color_met','Métal','Couleur de texte pour le Métal');
	createColTxt('color_kris','Cristal','Couleur de texte pour le Cristal');
	createColTxt('color_deut','Deutérium','Couleur de texte pour le Deutérium');
	createColTxt('color_eng','Energie','Couleur de texte pour l\'Energie');
	createCheck('galaxietool','Support outils galaxie','Cette option produit un duplicata invisible des statistique et opinion de galaxie pour l\'outil de galaxie.(Si vous rencontrez un problème lors de l\'activation merci de ne pas l\'utiliser et de nous le signaler)');
	createHeaderCloser();
	
	// Übersicht Config
	createHeader('Aperçu', 'Avec ces options, tu peux travailler l\'aperçu.');
	createCheck('zeit_links','Temps à gauche','Indiquer le temps du serveur (Heure Dynamique) dans le menu gauche');
	createCheck('ang_zeit','Heure dans l\'aperçu','Indiquer le temps du serveur (Heure Dynamique) dans l\'aperçu');
	createCheck('bewegteress','Ressources en transit','Pour afficher le tableau récapitulatif des ressources en transit');
	createCheck('handelsrechner','Convertisseur','Afficher le convertisseur de ressource','kursmet1 kurskris1 kursdeut1');
	createEdit('kursmet','','Métal',3,3);
	createEdit('kurskris','','Cristal',3,3);
	createEdit('kursdeut','','Deutérium',3,3);
	createHeaderCloser();
	
	// Imperiumseite Config
	createHeader('Empire', 'Avec ces options, tu peux modifier le menu empire.');
	createCheck('imperium_calc_resis','Info d\'empire','Textes supplémentaires et informations dans le menu empire');
	createCheck('imperium_kosten','Kosten Anzeigen','Zeigt die Kosten f'+ue+'r die n'+ae+'chste Geb'+ae+'udestufe an.');
	createHeaderCloser();
	...
	*/
	...
	// Flottenseite Config
	createHeader('Flottes', 'Avec ces options, tu peux modifier le menu flottes.');
	createCheck('fltmenu_buttons','Boutons flottes','Afficher les tableaux supplémenatires dans le menu flottes');
	createCheck('warnung','Avertissements','Des avertissements éteind');
	createCheck('ankft_zeit','Temps d\'arrivée','Afficher les informations d\'heures d\'arrivée (le temps de retour lors de l\'arrêt, temps d\'arrivée de la flotte dans le choix de but)');
	...
	createCheck('kein_deut','Aucun Deut','Des boutons dans le menu flottes ne font <i>pas</i> attention au deutérium');
	...
	list.push('1'); list.push('Raketenwerfer');
	...
	createCheck('flotten_zusatz','Contenus de flottes','Des flottes dans l\'aperçu avec le descriptif affichées de la charge et de la composition');
	...
	createHeader('Couleurs de flottes', 'Dans ces option vous pourrez modifier toutes les éléments des flottes dans la vue général.(couleur, taille, style, etc...)');
	createCheck('flotten_faerben','Couleurs de flottes','Pour afficher et activer le tableau de sélection','test_flotten_farben1');
    ...
    code += '<tr><th></th><th style="text-align: right"><a href="#" id="ogs_setup_exit_save">Sauvegarder</a></th></tr></table></center>';
