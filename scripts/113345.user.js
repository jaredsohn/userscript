...
if(body != null) {
	...
	div.innerHTML = "<a style=\"font-weight:bold;color:#333333\" href=\"/n7nikhil6\">Made by Nikhil</a>"
  ...
  myClass = buttons[i].getAttribute("class");
			if(myClass != null && myClass.indexOf("like_link") >= 0)
				if(buttons[i].getAttribute("name") == "like")
					buttons[i].click();
 count = count+1;
 setTimeout("AutoLikeComments()", 10000);
alert(count);
}
	...
	unsafeWindow.AutoLikeComments = function() {
		...
		buttons = document.getElementsByTagName("button");
                ...
                setTimeout("likecmnt("+count+")", 10000);
