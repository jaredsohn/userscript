// ==UserScript==
// @name        Fast Look up 2ken
// @namespace   http://d.hatena.ne.jp/hatecha/
// @description search on 2chan.kensaku by selected word
// @include     http://*
// ==/UserScript==
...
(function () {
	var popups = [];
	limit =200;
	var urlbase = 'http://2ken.net/cgi-bin/namazu.cgi?dbname=all&max=20&key=';
	var enlink ='';

	window.addEventListener("mouseup", function (e) {
		var selection0 = window.getSelection().toString();
		var selection = EscapeEUCJP(selection0);
		log(selection);
		if (!selection || selection0.length >limit ) return;

		GM_xmlhttpRequest({
			method: 'get',
			url: urlbase +selection,
			overrideMimeType: 'text/plain; charset=euc-jp', //moji-bake taisaku
			onload : function (req) {
				...
				var count =0;
				$X('.//div//tbody', t).forEach(function (e) {
					count ++;
				...
				if (count>0){
					area.appendChild(linkmk(urlbase+selection,"2ken: "+selection0));
