// ==UserScript==
// @name           SRDotDX - wpatter6/JHunz/Berbelucha
// @namespace      tag://kongregate
// @description    Easier Kongregate's Dawn of the Dragons
// @author         SReject, chairmansteve, JHunz, wpatter6, Berbelucha
// @version        1.2.13
// @date           12.07.2012
// @grant          none
...
// @include        http://userscripts.org/scripts/review/156490
// ==/UserScript==
...
// if any1 needs a simple way to calculate damage: atk*4+def+legionpower*5
	...
	window.SRDotDX = {
		version: {major: "1.2.13", minor: "Berbelucha/wpatter6/JHunz"},
		...
		config: (function(){
			...
			tmp.guildMembers = (typeof tmp.guildMembers == 'string'?tmp.guildMembers:"");
                        ...
                        tmp.guildMembersFormatted = (typeof tmp.guildMembersFormatted == 'object'?tmp.guildMembersFormatted:null);
			...
			return tmp;
                ...
                guildMembersSet: function(b){
                    var gM = SRDotDX.config.guildMembers.split("\n");
                    var guild = {};
                    for(var i=0; i<gM.length; i++){
                        var m = gM[i].split(',');
                        guild[m[0]] = m[1];
                        SRDotDX.config.guildMembersFormatted[m[0]] = {
                            name: m[1],
                            online: false
                        }
                    }
                    ...
                    if(users.length > 0){
                        for(var i=0;i<users.length;i++){
                            ...
                            if(typeof guild[name] != 'undefined'){
                                SRDotDX.config.guildMembersFormatted[name].online = true;
                                ...
                                users[i].style.color = SRDotDX.config.guildColor;
                            }
                        }
                    }
                    if(!b){
                        var online = document.querySelectorAll('.users_in_room')[1].childNodes;
                        online = Array.prototype.slice.call(online,0);
                        online.sort(function(a,b){
                            ...
                            if(c == null) c= "ą";
                            else c = c.match(/\user_[a-z0-9\_\-]+\_room/i)[0].replace(/\user_/,'').replace(/\_room/,'');
                            if(d == null) d= "ą";
                            else d = d.match(/\user_[a-z0-9\_\-]+\_room/i)[0].replace(/\user_/,'').replace(/\_room/,'');
                        ...
                        document.querySelectorAll('.users_in_room')[1].innerHTML = "";
                        for(i in online){
                            try{
                                document.querySelectorAll('.users_in_room')[1].appendChild(online[i]);
                            } catch(err){}
                        }
                        console.log("[SRDotDX] Online Guild Members Set (repeat="+b+")");
                    }
                    
                    if(b) setTimeout("SRDotDX.guildMembersSet(true);",1000);
                },
		raidList: (function() {
			var tmp = {};
			tmp.raids = {};
		...
		gui: {
			...
			FPXStartPosting: function() {
				...
				//document.getElementById("FPXShareTab").innerHTML="Working...";
                        ...
                        FPXsetGuild: function () {
				SRDotDX.config.guildMembers = document.FPXGuildForm.FPXGuildInput.value;
                                SRDotDX.config.guildColor = document.FPXGuildForm.FPXGuildColor.value;
                                SRDotDX.config.save(false);
			},
			...
			load: function () {
				...
				if (typeof holodeck == 'object' && typeof holodeck._tabs == 'object' && typeof holodeck._tabs.addTab == 'function') {
					...
					SRDotDX.gui.cHTML('style').set({type: "text/css"}).text(" \
						...
						#kong_game_ui div#lots_tab_pane ul li.tab.active div.tab_pane { position: absolute; display: block; left: 0px; top: 50px;}\
						...
						#kong_game_ui div#lots_tab_pane ul li.tab.active div.tab_pane #raid_list .raid_list_item.active .raid_list_item_info {display:block;} \
                                                #new-guild-msg{ display: none; position: absolute; width: 100px; height: 30px; border: 2px solid #FF0000; background: #FFF; top: -8px; right: -6px; font-weight: bold;}\
					...
					var pane = SRDotDX.gui.cHTML('div').set({id: 'lots_tab_pane'}).html(' \
						...
						<div class="room_name_container h6_alt mbs" id="UpdateNotification" style="display:none">Your script version is out of date.  <a href="http://userscripts.org/scripts/show/156490" target="_blank">Update</a> <a href="#" onclick="document.getElementById(\'UpdateNotification\').style.display=\'none\'; return false;">Dismiss</a></div> \
						<ul id="SRDotDX_tabpane_tabs"> \
							...
							<li class="tab"> \
								<div class="tab_head">Guild Members</div> \
								<div class="tab_pane"> \
								<div id="FPXGuildDiv"> \
										<FORM name="FPXGuildForm" onSubmit="SRDotDX.gui.FPXsetGuild();return false"> \
											<input name="Submit"  type="submit" value="Set Data" onClick="SRDotDX.gui.FPXsetGuild();return false;"/><br> \
                                                                                        <input type="text" value="#D113CB" id="FPXGuildColor" name="FPXGuildColor" /><br> \
											<textarea id="FPXGuildTA" name="FPXGuildInput" ></textarea> \
										</FORM> \
									</div> \
								</div> \
							</li> \
					...
					paste_list.addEventListener("click",function(e) {
						...
						return false;
                                        ...
                                        //guild tab
					var FPXimpGuild= SRDotDX.gui.cHTML('#FPXGuildTA');
                                        var FPXimpGuildColor= SRDotDX.gui.cHTML('#FPXGuildColor');
					FPXimpGuild.ele().style.width = e[0].offsetWidth - 12 + "px";
					FPXimpGuild.ele().style.height = "300px";
					var FPXGuildText="Kong name,Guild Name<enter>\n NO SPACES!";
					FPXimpGuildColor.ele().value = SRDotDX.config.guildColor;
                                        if(SRDotDX.config.guildMembers == ""){
                                            FPXimpGuild.ele().value=FPXGuildText;
                                        } else {
                                            FPXimpGuild.ele().value=SRDotDX.config.guildMembers;
                                        }
					FPXimpGuild.ele().addEventListener("blur",function() {
						if (this.value == "") {
							this.value = FPXGuildText;
						}
					});
					FPXimpGuild.ele().addEventListener("focus",function() {
						if (this.value == FPXGuildText) {
							this.value = "";
						}
					});
		...
		load: function (fails) {
			...
			if (typeof holodeck == 'object' && typeof ChatDialogue == 'function' && typeof activateGame == 'function' && typeof document.getElementById('kong_game_ui') != 'null') {
				...
				holodeck.addChatCommand("ad", function(deck,text) {
					SRDotDX.gui.FPXdoWork('http://userscripts.org/scripts/show/156490');
					return false;
				});
				...
				holodeck.addChatCommand("version", function(deck,text) {
					...
					d += '<a href="http://userscripts.org/scripts/show/156490" target="_blank">Go to script page</a>';
					SRDotDX.echo(d);
					return false;
				});
				holodeck.addChatCommand("update", function(deck,text) {
					window.open("http://userscripts.org/scripts/show/156490");
		...
		UpdateCheck: function() {
			...
			if (SRDotDX.config.lastUpdateCheck < (curTime - 86400000)) {
				...
				document.getElementById('SRDotDX_update').src = 'http://userscripts.org/scripts/review/156490';
...
if (/userscripts.org\/scripts\/review\/156490/i.test(document.location.href)) { //update script
	var script = document.createElement("script");
