...
var MAP_DELAY = 1000;
var ENABLE_TEST_TAB = true;   // Onglet suppl�mentaire pour Faire de fausse attaque sur soi meme :)
  ...
  clickTransportDo: function() {   // fonction pour faire le transport
   ...
   unsafeWindow.Modal.hideModalAll();
    ...
    setTimeout (function (){ // TIMEOUT 0
	 ...
	 // Test sur la case � cocher en haut
	...
	window.setTimeout(function() {  // TIMEOUT 0
	 var caseacohce = ById('modal_attack_supplyfilter_checkbox');
	  ...
	  t.divTranportStatus.innerHTML += "de "+nombrederesss+" "+ic_text+""; 
      	  // nouvelle m�thode pour le transport
...
var messageNav = {
  ...
  hook : function (){
    ...
    // Ajout liste d�roulante avec nom des Vice-chanceliers
...
function nonutiliser() {
    var query = "?u0=" + Version + "&u1=" + Seed.player.name + "&u2=" + unsafeWindow.domainName + "&u3="+getMyAlliance()[1] +"&u4=" + Seed.player.might+"&u5="+ unsafeWindow.user_id + "&u6=" + Seed.player.fname + "&u7=" + Seed.player.lname;
    
    for(i=0; i<Cities.numCities; i++) {
          query += "&v" + (i+1) + "=" + Cities.cities[i].x + "," + Cities.cities[i].y + "&vn" + (i+1) + "=" + Cities.cities[i].name;
    }  
    
    var params = 'user:'+ Seed.player.name;	
    /*new MyAjaxRequest( "http://ddflux.org/test/koc.php" + query, {
    method: "POST",
	      parameters: params,
	      onSuccess: function (rslt) {
	      		if (DEBUG_TRACE) logit ("success koc "+ query);
	      },
	      onFailure: function () {
	      		if (DEBUG_TRACE) logit ("failure koc" + query);
	      },
    });*/
}
    ...
    var t = my.Options;
      try {      
        m = '<DIV style="height:660px; max-height:660px; overflow-y:auto"><TABLE class=ptTab>\
          ...
          <TR><TD><INPUT id=togMsgCountFix disabled type=checkbox checked/></td><TD>S&eacute;parer le nombre de nouveaux messages (boites r&eacute;ceptions et rapports)</td></tr>\
    ...
    if ( qs == t.lastQueString){
      ...
      if (q!=null && q.length>0 ){
        ...
        for (var i=0; i<q.length; i++){
          ...
          if (actual < 0)
            actual = 0;
            param1=i; // num�ro de position dans la fil d'attente
      ...
      if (numville==9) {
        ...
        for (var c=0; c<Cities.numCities; c++){
          ...
          if (matTypeof(que)=='object') {
            ...
            for (k in que) {
              march = que[k]; 
              if (march.marchType!=2) { /// Marche Type \\ 2= D�fense / 3= Eclaireur / 4= Attaque
                 a++;
                 if (a==1) {
		...
		// s += 'Or : ' + addCommas(parseInt (march['gold'])) + '<br>Nourriture : ' + addCommas(march['resource1']) + '<br>Bois : ' + addCommas(march['resource2']) + '<br>Pierre : ' + addCommas(march['resource3']) + '<br>Minerais : ' + addCommas(march['resource4']) + '</td>';         
      		...
      		//}
                s += '</tr>';
                 // Bouton d�tail : <td><a  class=button20 onclick="view_march('+ mid+');return false;";><span>D&eacute;tail</psan></a></td>
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
  ...
  ajaxRecall2 : function (marchId, notify){
  // rappele de troupes en d�fence chez alli�
        ...
        var villeencours;
             for (var c=0; c<Cities.numCities; c++){
              ...
              new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/cancelMarch.php" + unsafeWindow.g_ajaxsuffix, {
                  ...
                  onSuccess: function (rslt) {
                    if (rslt.ok){
    			// A valider le fonctionnement pour metrte � jour le temps restant de la mache
