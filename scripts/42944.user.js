...
function addStyle() {
	GM_addStyle('\
		...
		.tube_box a.download_link:hover {\
...
function getinlinevars(onload) {
	...
	function extract(str) {
		...
		vars.ID = str.match(/flashvars\.v\s+= ["'](.*)["'];\n/)[1];
	}
	onload();
}
function getxmlvars(onload) {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://www.megavideo.com/xml/videolink2.php?v='+vars.ID,
		headers: {
		...
		onload: function(xml) {
			...
			vars.title = urldecode(attr.getAttribute("title"));
			vars.added = urldecode(attr.getAttribute("added"));
			vars.added = vars.added.replace(/\s,/, ",");
			vars.embed = urldecode(attr.getAttribute("embed"));
...
function load_player() {
	$("playerdiv").innerHTML = '<embed src="http://www.megavideo.com/v/'+vars.embed+'" type="application/x-shockwave-flash" width="800" height="450" allowscriptaccess="never" allowfullscreen="true" wmode="transparent"></embed>';
}
