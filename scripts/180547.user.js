...
function LL_carga() {
    ...
    var imageHeader = 'http://i.imgur.com/AVpbU.jpg';
...
function infoprio(i) {
    ...
    default: x = 'Öncelik ' + i + ' no definida en el complemento';
    }    
	return x;
}
function battles() {
	var x = division();
    $.get('https://docs.google.com/document/d/1o8nJEaFhu1Cq9XkVF0mg0FPjHLFpUdwoNAvI8-YzUEE/preview', function(data) {
        ...
        var shout = prios[0].split(';');
     	if ( shout[1] != "" && shout[1].length > 0 ) {
          var shoutdiv = '<div class="rest">-----------------</br><b>Dikkat!!</b> - Mensaje del MoD :  </br></br>'+ shout[1].replace(/\\n/g,"<br>") +'</br>-----------------</br></div>';
        ...
        for (var i = 1; i < battles.length; i++) {
            if (battles[i] != "" && battles[i].length > 0) {
                ...
                var _htmlBattles = '<h4>Öncelik : ' + nameprio2(i) + '</h4>' + ico + '<ul id="prio_' + nameprio(i) + '" class="prio_' + nameprio(i) + '"></ul>';
