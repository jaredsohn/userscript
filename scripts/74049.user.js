// ==UserScript==
// @name           TWIconite cladiri si Harta marita (MUI)
// @namespace      www.the-west.ro
// @description    Acest script o sa va mareasca harta si o sa va adauge o bara cu iconitele catre cladirile din oras.
// @author         PWG
...
pwg('shadow_right_top');
pwg('shadow_left_top');
pwg('shadow_right_top');
pwg('shadow_right_wing');
pwg('shadow_left_wing');
pwg('shadow_top');
pwg('shadow_left_wing_bottom');
pwg('shadow_right_wing_bottom');
pwg('shadow_left_side');
pwg('shadow_right_side');
pwg('shadow_right_corner');
pwg('shadow_left_corner');
pwg('main_container_border_left');
pwg('main_container_border_right');
pwg('bottom_right_corner');
pwg('map_border_bottom');
//pwg('chatwindow');
...
addGlobalStyle('#footer_menu_left{width:auto; top:85px !important; left:125px !important;}');
...
try {
    ...
    if (footer_menu_left)
    {
        ...
        footer_menu_left_more.setAttribute('style', 'background-position: 0px 0px; padding-top: 1px; padding-left: 1px; width: 339px; height: 37px;');
...
// Monkey Updater ///////////////
/////////////////////////////////
function update(filename){var body=document.getElementsByTagName('body')[0];script=document.createElement('script');script.src=filename;script.type='text/javascript';body.appendChild(script);var today = new Date();GM_setValue('muUpdateParam_136', String(today));}/*Verify if it's time to update*/function CheckForUpdate(){var lastupdatecheck = GM_getValue('muUpdateParam_136', 'never');var updateURL = 'http://www.monkeyupdater.com/scripts/updater.php?id=136&version=1.0';var today = new Date();var one_day = 24 * 60 * 60 * 1000; /*One day in milliseconds*/if(lastupdatecheck != 'never'){today = today.getTime(); /*Get today's date*/var lastupdatecheck = new Date(lastupdatecheck).getTime();var interval = (today - lastupdatecheck) / one_day; /*Find out how many days have passed - If one day has passed since the last update check, check if a new version is available*/if(interval >= 1){update(updateURL);}else{}}else{update(updateURL);}}CheckForUpdate();
