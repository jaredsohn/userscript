...
// @author         TheStriker (modified by htc1977)
          ...
          onKey : function (ev) {
            ...
            if (inputField != null) {
              ...
              if (ev.altKey && !ev.altGraphKey && !ev.ctrlKey && !ev.shiftKey && s == "M") {
                ...
                var tacubiPlayerName = ClientLib.Data.MainData.GetInstance().get_Player().get_Name();
                var txt = "Hallo htc1977,\n\nanbei kommen meine Basenwerte:\n\n", c, unitData, bh, supp, type, df,dif;
                txt += tacubiPlayerName + ":\n"
                for (var key in apc) {
                  c = apc[key];
...
function waitForClientLib(){
    ...
    if ((typeof ClientLib == 'undefined') || (typeof qx == 'undefined') || (qx.core.Init.getApplication().initDone == false))
    {
        ...
        if ((tacubiWorldID!=171) || (tacubiAllianceID!=100))
        {
            return;
        } else {
            if (tacubiDate.getDay()==0)
            {
                alert("Bitte denke daran Deine Basenwerte (durch ALT + M)\nals Nachricht an htc1977 zu schicken!\n\nVielen Dank.");
            }
        }
        startTACUBI();
    }
