...
var urlscript = 'http://www.geer26_pesadilla.com.ar/descargas/';
...
switch(pathArray[1]){
	...
	case 'api':
		return;
		break;
	case 'geer26_pesadilla':
	case 'geer26_pesadillaBBC':
	case 'geer26_pesadillabbc':
		path = 'geer26_pesadillaBBC';
		break;
}
...
if(Tiny){
	...
	$('h1:contains(Cargar)').html('Geer® 2010');
...
var theme = GM_getValue('geer26_pesadilla-theme','http://ajax.googleapis.com/ajax/libs/jqueryui/1.7.2/themes/dark-hive/jquery-ui.css');
...
$.fn.themeswitcher = function(settings){
	...
	function updateCSS(locStr){var cssLink = $('<link href="'+locStr+'" type="text/css" rel="Stylesheet" class="ui-theme" />');$("head").append(cssLink);if( $("link.ui-theme").size() > 3){$("link.ui-theme:first").remove();}GM_setValue('geer26_pesadilla-theme', locStr);}	
	/* Inline CSS 
	---------------------------------------------------------------------*/
...
var Actualizar = GM_getValue("Actualizar", ultversion);
	...
	GM_registerMenuCommand("BBCoder para T! y P! - Versión Instalada: " + ultversion, function() {
		unsafeWindow.mydialog.alert("BBCoder para T! y P! - geer®","Tu versión del script instalada actualmente es: " + ultversion);
	},null,null,"V");
...
function Cargar_Emoticones(){
	...
	for (var i = 0; i < emot.length; i=i+2){
		nroemot++;
		html += '<span id="shark_emot_'+nroemot+'" style="position: relative; left: 5px; top: 4px;"><img src="'+emot[i+1]+'" alt="" "[img='+emot[i+1]+']" style="height:60px;width:60px;"><a href="javascript:void(0)" onclick="$(\'#shark_emot_'+nroemot+'\').remove()"><img src="http://i26.tinypic.com/211rk2t.gif" alt="Eliminar" title="Eliminar" height="12" width="12"></a><input type="hidden" name="geer26_pesadilla_emot_'+nroemot+'" value="'+emot[i]+'"><input type="hidden" name="geer26_pesadilla_emot_'+nroemot+'" value="'+emot[i+1]+'"></span>';
	}
	return html;
}
		...
		if(urlbarra.search(/^https?:\/\//gi) != -1 && urlbarra.search(/,/) == -1){
			...
			if(urlemot != '' && urlemot != null){
				if(urlemot.search(/^https?:\/\//gi) != -1 && urlemot.search(/,/) == -1){
					nuevosemot++;
					html = '<span id="shark_emot_nueva_'+nuevosemot+'" style="position: relative; left: 5px; top: 4px;"><img src="'+urlemot+'" alt="" title="[img='+urlemot+']" style="height:60px;width:60px;"><a href="javascript:void(0)" onclick="$(\'#shark_emot_nueva_'+nuevosemot+'\').remove()"><img src="http://i26.tinypic.com/211rk2t.gif" alt="Eliminar" title="Eliminar" height="12" width="12"></a><input type="hidden" name="geer26_pesadilla_emot_nueva_'+nuevosemot+'" value="[img='+urlemot+']"><input type="hidden" name="geer26_pesadilla_emot_nueva_'+nuevosemot+'" value="'+urlbarra+'"></span>';
...
function Guardar_Emoticones(){
	if(confirm('¿Está seguro que desea guardar los cambios en los emoticones?\n\nEste cambio es permanente.')){
		...
		for(var i=0; i<input.length; i++){
			var o = input.item(i);
			if(o.type=='hidden' && o.name.search(/geer26_pesadilla_emot/gi) != -1){
				var emot = o.value;
...
Taringa_Set_Function("eliminar_imagen", function(id){
	...
	$('#imagen_' + id).val('');
	$('#geer26_pesadilla_imagen_' + id).remove();
...
Taringa_Set_Function("agregar_imagen", function(id){
	...
	var imagen_nueva_div = '<div id="geer26_pesadilla_imagen_nueva_'+imagenes_nuevas+'"><a id="shark_a_nueva_'+imagenes_nuevas+'" href="javascript:void(0)" target="_blank" title="Abrir en nueva ventana"><img id="shark_img_nueva_'+imagenes_nuevas+'" src="http://i25.tinypic.com/i69yrm.jpg" style="max-width: 300px; max-height: 300px;" border="0"></a></div>';
...
function Abrir_Chat(){
	GM_openInTab("http://www.everywherechat.com/everywherechat.swf?defaultRoom=geer26_pesadilla_radio&fontSize=12&roomList=false&theme=night");
}
...
function Sharkale_Opciones(){
	...
	nuevaHTML += '    <a href="http://radio.geer26_pesadilla.co.cc/" target="_blank"><img src="http://i32.tinypic.com/zkg95l.jpg" alt="..::Radio Sharkale®::.." width="740"/></a>';
...
//----Monitor----//
    ...
    menu += '		<li><a href="/geer26_pesadillaBBC" target="_blank">Activar receptor de alertas de nuevos comentarios</a></li>';
    menu += '	</ul>';
	menu += '	</li>';
//----Favoritos----//
...
//----Opciones----//
	...
	menu += '	<li><a href="http://www.geer26_pesadilla.com.ar" title="Visiten la Radio de Sharkale® =P" target="_blank"><img src="http://i29.tinypic.com/3029onc.jpg" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle"/></a></li>';
//----Actualizar----//
...
///////////////-----------------Agregar FullScreenMode------------------////////////////////
	modificar_posts();
	if($('span[class="given-name"]').html() == "geer26_pesadilla"){
		var mod_geer26_pesadilla = $('span[class="given-name"]').parent().parent();
		mod_geer26_pesadilla.html(mod_geer26_pesadilla.html().replace(/New Full User/gi,'Administrador'));
		mod_geer26_pesadilla.find("div[class=metadata-usuario] span:eq(0)").append('4684');
		$('img[title="Administrador"]').attr('style','position: absolute; top: -703px; clip: rect(703px, 16px, 720px, 0px);').removeAttr('alt');
		mod_geer26_pesadilla = $('img[title="Administrador"]').parent().attr('id','icon_shark').clone();
		mod_geer26_pesadilla.find('img[title="Administrador"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','geer26_pesadilla® La envidia de los Mods...');
		$('#icon_shark').after(mod_geer26_pesadilla);
	}
...
///////////////------------------Agregar BBCodes--------------------////////////////////
	...
	if(perfilde == "geer26_pesadilla"){
		$('li[class="rango"]').css('width','195px').find('span').html('Administrador');
	...
	for (var i = 0; i < fotos.snapshotLength; i++) {
		...
		div.setAttribute('id','geer26_pesadilla_'+agregarFoto.id);
		agregarFoto.size = 90;
...
}else if(path == 'geer26_pesadillaBBC'){
