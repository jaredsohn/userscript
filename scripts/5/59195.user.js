...
// @version        v1.3
// @author         bodhiBrute
// @description    Gruppezueghörigkeit in der Dorfübersicht bearbeite
// @include        http://ch*.staemme.ch/*screen=overview
// @include        http://ch*.staemme.ch/*screen=overview&*
// @include        http://ch*.staemme.ch/*screen=info_village*
// @include        http://ch*.staemme.ch/groups.php*
// ==/UserScript==

//Bestimmt ob die Gruppezueghörigkeitstabelle auch angezeigt werden soll
tabellezeigen=false;

//Bestimmt ob die aktuelle Gruppezueghörigkeit hervorgehoben werden soll
highlight=true;

//Bestimmt ob in der Dorfansicht die Tabelle noch vor den letzen Berichten angezeigt werden soll
vorBerichtenAnzeigen=false;

//Bestimmt ob eine leere Tabelle zum Gruppensetzen in der Dorfansicht angezeigt werden soll
inDorfansichtAnzeigen=false;
...
function starten() {
	try {
		...
		if(doc.URL.match("screen=overview")){
			...
			if(GM_getValue("groups||"+server+"||"+uvid+"||0", "0||default").split("||")[0]!=0){
				...
				for(var i=0;i<tabellen.length;i++){
					if((ths=tabellen[i].getElementsByTagName('th'))[0]!=null&&ths[0].innerHTML.match('Gruppezuegh.rigkeit')){
						...
						editth.innerHTML="Gruppezuegh&ouml;rigkeit";
						if(tabellezeigen){editth.innerHTML+=" bearbeite";}
						...
						if(tabellezeigen){
							...
							edit.action+=	"if((ths=tabellen[i].getElementsByTagName('th'))[0]!=null&&ths[0].innerHTML.match('Gruppezuegh.rigkeit')){";
		...
		if(inDorfansichtAnzeigen&&doc.URL.match("screen=info_village")){
			var link1=null;
			var link2=null;
			...
			for(var i=0;i<(links=vistabellen[0].getElementsByTagName("a")).length;i++){
				...
				if(links[i].innerHTML.match("Gruppezuegh.rigkeit.bearbeite")){
					link2=links[i].href;
				}
			}
