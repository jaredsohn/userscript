...
User = $$('.nameholder')[0].innerHTML;
UserID = User.substring(User.indexOf('profile/')+8, User.indexOf('">'));
User = User.substring(User.indexOf('">')+2, User.indexOf('</a>'));
...
defaultInventory = {food:0, gift:0, weapon:0, movingtickets:0, grain:0, diamonds:0,
...
LangStrings = {
	en: {
		...
		oFail:				'The option you wrote was incorrect',
		oSelectOption:		'Select one of this options writing its number:\n\n0: Exit\n1: Add New Module\n2: Move a Module\n3: Delete a Module\n ',
		oSelectModule:		'Select one of the available modules writing its exact name (case-sensitive):\n\n0: Exit\n',
		oGetParam:			'This module requests you the following parameter:\n',
		oModuleAdded:		'The following module has been added:\n',
		oSelectForDel:		'Select one of your modules writing its number for its deltion:\n\n0: Exit\n',
		oSureDel:			'Are you sure that you want to delete the following module?\n',
		oModuleDeleted:		'The following module have been deleted successfully:\n',
		oSelectForMove:		'Select one of your modules writing its number to move it:\n\n0: Exit\n',
		oSelectForMove2:	'Select the position to which you want to move it:\n\n0: Exit\n',
		oEndMove:			'Final Position',
	...
	es: {
		...
		mSkillShow:		{
			prompt: 'Elige una opción de las siguientes:\n0: Mostras habilidad activa\n1: Manufactura\n2: Recursos Naturales\n3: Construcciones\n '
		},
		mWarMod:	{
			fought: ['Hoy no has luchado', 'Hoy has luchado ', ' vez', ' veces'], healed: ['Hoy a&uacute;n no te has curado', 'Hoy ya te has curado']
		},
		
		oFail:				'La opción introducida no es correcta',
		oSelectOption:		'Elige un opción escribiendo su número:\n\n0: Salir\n1: Añadir Módulo\n2: Mover un Módulo\n3: Borrar un Módulo\n ',
		oSelectModule:		'Elige uno de los módulos disponibles escribiendo su nombre exacto (sensible a mayúsculas):\n\n0: Salir\n',
		oGetParam:			'Éste módulo solicita el siguiente parámetro:\n',
		oModuleAdded:		'The following module has been added:\n',
		oSelectForDel:		'Selecciona un módulo escribiendo su número para borrarlo:\n\n0: Salir\n',
		oSureDel:			'¿Seguro que quieres borrar este módulo?\n',
		oModuleDeleted:		'El módulo siguiente se ha eliminado correctamente:\n',
		oSelectForMove:		'Elige un módulo escribiendo su número para moverlo:\n\n0: Exit\n',
		oSelectForMove2:	'Elige la posición a la que quieres moverlo:\n\n0: Exit\n',
		oEndMove:			'Posición Final',
...
ModuleHandler = {
	...
	ExpShow: function () {
		...
		coremenu.insertBefore(holder, coremenu.firstChild.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling);
		...
		var inCell = (new Element('div')).setStyle({width: expPerc+'%', height:'15px', padding:0, margin:0, background: '#BCDE7F'});
		var cell = (new Element('div')).addClassName('item').update(inCell).setStyle({
		...
		setTimeout(function(cell, inCell){
			var offset = inCell.cumulativeOffset();
			
			var textHold = (new Element('div')).update(expPerc+' %');
			textHold.absolutize().setStyle({
				left: offset[0]+'px', top: (offset[1])+'px', color: '#617D07', fontWeight: 'bold', width: '70px',
				textAlign: 'center', fontSize: '10px', fontFamily: 'Tahoma', paddingTop: '1px'
			}).wrap(cell);
			
			var tooltipHolder = (new Element('div')).absolutize().setStyle({
				left: (offset[0]+72)+'px', top: offset[1]+'px', padding: 0, width: 'auto'
			});
			tooltipHolder.wrap(cell);
			...
			cell.observe('mouseover', function(Ev){tooltipHolder.show()});
	...
	GoldenMarket: function () {
		/*var table = $$('table.offers')[0];
		var rows = table.select('tr');
		
		(new Element('th')).update('Gold').setStyle().wrap(rows[0]);
		
		for (var i=1; i<rows.length; i++) {
			var current = rows[i].select('td')[3];
			var curPrice = parseFloat(current.select('.special')[0].innerHTML + current.select('sup')[0].innerHTML);
		
			var currency = current.select('.currency')[0].innerHTML;
		
			var td = new Element('td');
			td.update(curPrice).wrap(rows[i]);
			td.id = '_goldValueRow'+i;
		}
		
		GM_xmlhttpRequest({
			url: 'http://api.erepublik.com/v1/feeds/exchange/'+currency+'/GOLD.json',
			method: 'GET',
			onload: function (h) {
				var goldValue = JSON.parse(h.responseText)[0].offer['for'];
				for (i=1; i<21; i++) {
					if (cell = $('_goldValueRow'+i)) {
						var goldPrice = Math.round(cell.innerHTML * goldValue * 100)/100;
						var goldSplit = goldPrice.toString().split('.');
						
						cell.update(
							'<span class="special">' + goldSplit[0] + '</span><sup>.' + goldSplit[1] + '</sup>'
						);
					}
				}
			}
	...
	SkillShow: function () {
		...
		setTimeout(function(strdisp){
			var offset = strdisp.cumulativeOffset();
			
	
			var tooltipDamHolder = (new Element('div')).absolutize().setStyle({
				left: (offset[0]+72)+'px', top: (offset[1]+4)+'px', padding: 0, width: 'auto'
			});
			...
			strdisp.observe('mouseover', function(Ev){tooltipDamHolder.show()});
	...
	WarMod: function () {
		setTimeout(function(){
			...
			var offset = wn.cumulativeOffset();
			
			var tooltip = (new Element('div')).absolutize().setStyle({
				left: (offset[0]+71)+'px', top: (offset[1]-3)+'px',
				width: 'auto', height: '29px',
				textAlign: 'left', fontFamily: 'Tahoma', fontSize: '11px', color: '#3298C7',
			}).hide();
			tooltip.wrap(wn);
			tooltip.id = 'WarModTooltip';
			
			var textHold = (new Element('div')).setStyle({
				cssFloat: 'left', background: 'url(' + ImageWarModTooltip + ') no-repeat', paddingLeft: '8px', paddingTop: '1px', height: '28px'
			});
			...
			(new Element('div')).setStyle({
				cssFloat: 'left', background: 'url(' + ImageWarModTooltip + ') right no-repeat', width: '4px', height: '29px'
			}).wrap(tooltip);
			...
			if (!Cookie.misc.healedToday && Cookie.misc.foughtToday > 0) {
				...
				var imgLink = ((new Element('img')).writeAttribute('src', ImageHeal)).wrap(new Element('a')).setStyle({
					...
					Cookie.misc.healedToday = true;
					setCookie(User, Cookie);
					location.pathname = '/'+CurrentLang+'/region/'+Cookie.data.region;
				});
				
				imgLink.wrap($('wellnessvalue'));
			}
			
			wn.observe('mouseover', function(Ev){$('WarModTooltip').show()});
...
OptionsLink.observe('click', optSelect);
...
function setCookie (name, value) {
	setTimeout(GM_setValue, 0, 'eRT_'+name, JSON.stringify(value));
}
...
function optSelect () {
	switch (prompt(Lang.oSelectOption)) {
		case '0': case '': case null:	break;
		case '1':	optNewModule();		break;
		case '2':	optMoveModule();	break;
		case '3':	optDeleteModule();	break;
		default:	alert(Lang.oFail); optSelect();	break;
	}
}

function optNewModule () {
	var message = Lang.oSelectModule;
	for (i in Modules) {
		if (!isOr || Modules[i].Or) {
			message += i + ': ' + Modules[i].name + ' ( ' + Modules[i].params.length + ' )\n';
		}
	}
	moduleName = prompt(message+' ');
	if (moduleName=='0' || moduleName=='' || moduleName==null) {
		return;
	} else if (typeof Modules[moduleName] == 'undefined') {
		alert(Lang.oFail);
		optNewModule();
		return;
	} else {
		params = [];
		for (i=0; i<Modules[moduleName].params.length; i++) {
			param = prompt(Lang.oGetParam + '#' + (i+1) + ': ' + Modules[moduleName].params[i] + '\n');
			if (param==null) {
				return;
			}
			params.push(param);
	...
	alert(Lang.oModuleAdded + moduleName + ' ( ' + params.join(', ') + ' ) ');
	
	setCookie(User, Cookie);
	optSelect();
}

function optMoveModule () {
	message = Lang.oSelectForMove;
	for (i=0; i<Cookie.modules.length; i++) {
		message += (i+1) + ': ' + Cookie.modules[i].name + ' ( ' + Cookie.modules[i].params.join(', ') + ' )\n';
	}
	toMove = prompt(message+' ');
	if (toMove=='0' || toMove=='' || toMove==null) {
		return;
	} else if (toMove<1 || toMove > Cookie.modules.length) {
		alert(Lang.oFail);
		optMoveModule();
		return;
	} else {
		message = Lang.oSelectForMove2;
		for (i=0; i<Cookie.modules.length; i++) {
			message += (i+1==toMove ? ' - ' : '') + (i+1) + ': ' + Cookie.modules[i].name + ' ( ' + Cookie.modules[i].params.join(', ') + ' )\n';
		}
		toMove2 = prompt(message + (i+1) + ': <' + Lang.oEndMove + '>\n ');
		
		if (toMove2=='0' || toMove2=='' || toMove2==null) {
			return;
		} else if (toMove2<1 || toMove2 > Cookie.modules.length+1) {
			alert(Lang.oFail);
			optMoveModule();
			return;
		} else {
			Cookie.modules.splice(toMove2-1, 0, Cookie.modules[toMove-1]);
			Cookie.modules.splice(toMove-(toMove2<=toMove ? 0 : 1), 1);
		}
	}
...
function optDeleteModule () {
	message = Lang.oSelectForDel;
	for (i=0; i<Cookie.modules.length; i++) {
		message += (i+1) + ': ' + Cookie.modules[i].name + ' ( ' + Cookie.modules[i].params.join(', ') + ' )\n';
	}
	toDel = prompt(message+' ');
	if (toDel=='0' || toDel=='' || toDel==null) {
		return;
	} else if (toDel<1 || toDel > Cookie.modules.length) {
		alert(Lang.oFail);
		optDeleteModule();
		return;
	} else {
		if (confirm(Lang.oSureDel + Cookie.modules[toDel-1].name + ' ( ' + Cookie.modules[toDel-1].params.join(', ') + ' )\n ')) {
			alert(Lang.oModuleDeleted + Cookie.modules[toDel-1].name + ' ( ' + Cookie.modules[toDel-1].params.join(', ') + ' )\n ');
			Cookie.modules.splice(toDel-1, 1);
		}
	}
