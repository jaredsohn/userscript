...
var item= document.evaluate('//b[. = "Blue Draik Egg"]',document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);if (item.snapshotLength > 0){item = item.snapshotItem(0);selectedlink=item.previousSibling.previousSibling;window.location = selectedlink}return;}
	...
	var item = document.evaluate('//b[. = "Pirate Draik Egg"]', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	...
	var item = document.evaluate('//b[. = "Lost Desert Draik Egg"]', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	if (item.snapshotLength > 0) {
		item = item.snapshotItem(0);
		selectedlink = item.previousSibling.previousSibling;
		window.location = selectedlink
