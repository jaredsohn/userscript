// ==UserScript==
// @name           Mod mass by Olo
// @description    Mod by Oloprincepolo
// @author         Oloprincepolo
// @namespace      http://google.pl
// @include        *game.php?*screen=train&mode=mass*
// ==/UserScript==

// -----------------------------------------------------------------------------
//      Modifikationen und Weiterverbreitung dieses Scripts benÄ�Ĺ�tigen die 
...
(function(){
	...
	var updateWorkers = function() {
		...
		el.textContent = '(Te jednostki zajma '+neededworkers+' miejsc w zagrodzie)';
	}
	...
	var h3 = ce('h3');
	h3.textContent = 'Masowa rekrutacja dla grupy '+selectedgroup+'';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Ilosc jednostek:';
	th.title = 'Die gewÄ�Ĺşnschte Menge fÄ�Ĺşr das fertige Dorf (nicht die Menge, die pro Durchgang gebaut werden soll).';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Kontrolowac?:';
	th.title = 'Wenn die gewÄ�Ĺşnschte TruppenstÄ�Â¤rke erreicht wurde, wird weiter gebaut, bis der BH voll ist.';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	for(var i = 0; i < units.length; i++) {
		var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Ilosc wolnego miejsca:';
	th.title = 'Wie viel BH-PlÄ�Â¤tze sollen frei gelassen werden (z.B. 100 fÄ�Ĺşr ein AG).';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'Ile ma zostac surki w wioskach?:';
	th.title = 'Wie viel Rohstoffe sollen mindestens Ä�Ĺşbrig bleiben.';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	td.setAttribute('colspan', units.length);
	...
	var btn = ce('button');
	btn.setAttribute('style', 'font-size:8pt;');
	btn.textContent = 'Wprowadz';
	btn.addEventListener('click', function(evt){
		// calculate and fill
		evt.target.disabled = true;
		evt.target.textContent = 'lÄ�Â¤uft...';
		var currentsettings = getCurrentSettings(selectedgroup);
		...
		document.getElementById('massrecruitbuilt').textContent = 'Zostanie zrobione: ' + builtstr + ' w ' + builtvilnum + ' wioskach';
		evt.target.textContent = 'Wprowadz';
