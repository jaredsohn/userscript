...
//
// Modified by Steve Kinney (http://stevekinney.net) to display retweets in
// the style of the Tweetie client for Mac and iPhone.
//
// http://www.sean-o.com
// Modified by phill84
//
//
// @name           Tweetie Style Retweet
	...
	if (isHome) {
		...
		$('.actions a:odd').each(function(){
			...
			if(origAuthor != me_name) {
				...
				// add RT element
				var retweet = origTweet + ' (via @' + origAuthor + ')';
				if (isHome) {
					var newEl = $('<a>')
						.text('RT')
						.attr({
