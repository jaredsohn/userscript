...
// @include		http://*warshireonline.com/sandbox/Human/*
...
regPageHandler(/^\/sandbox\/Human\/(build.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(razebuild.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(explore.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(caravan.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(market.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(senate.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(hsubunits.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(military.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(blackops.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(smelter.php)?$/i,  userInputParser);
regPageHandler(/^\/sandbox\/Human\/(kmanage.php)?$/i,  userInputParser);
...
//Monster Ranch Sandbox
regPageHandler(/^\/sandbox\/Human\/(monsterranch.php)?$/i,  wsMonsterranch);
function wsMonsterranch() {
	var nBs = document.getElementsByTagName("b");
	if ((nBs.length == 8) && (!xpath('//input[@size="3"]',content,true))) {
		addAutofill(nBs[4],nBs[4].textContent,xpath('//input[@type="text"]',content,true));
...
// magi guild Sandbox
regPageHandler(/^\/sandbox\/Human\/(magiguild.php)?$/i,  wsMagiguild);
function wsMagiguild() {
	var nBs = content.getElementsByTagName("b");
	if ((nBs) && (nBs[4])) {
		addAutofill(nBs[4],nBs[4].textContent,xpath('//input[@size="10"]',content,true));
...
//Sandbox Structures
regPageHandler(/^\/sandbox\/Human\/(structures.php)?$/i,  wsStructures);
function wsStructures() {
	//Grab resources
	var pResources = [];
	var tmpStats = content.getElementsByTagName('table')[2].getElementsByTagName("td");
...
//Raze Structures Sandbox
regPageHandler(/^\/sandbox\/Human\/(razebuild.php)?$/i,  wsRazebuild);
function wsRazebuild() {
	//onClick; onBlur: Refreshed max cells
	var razeTables = content.getElementsByTagName('table');
	function refreshMaxCells(content) {
...
regPageHandler(/^\/sandbox\/Human\/(vkingdom.php)?$/i,  wsVkingdom);
function wsVkingdom() {
	//Target the right table.
	var kingdomsTable = content.getElementsByTagName('table')[1];
	var cPower, myRow, startRow;
...
//Blackops (Theives Guild) Sandbox
regPageHandler(/^\/sandbox\/Human\/(blackops.php)?$/i,  wsBlackops);
function wsBlackops() {
	//Attach autofill.
	var thieves = xpath("//b[3]",content,true);
	if (thieves) {
...
//Explore Sandbox
regPageHandler(/^\/sandbox\/Human\/(explore.php)?$/i,  wsExplore);
function wsExplore() {
	//Get right table.
	var explorationTable = content.getElementsByTagName('table')[1];
	//Insert new cell to hold max values.
...
//Hero Sandbox
regPageHandler(/^\/sandbox\/Human\/(hero.php)?$/i,  wsHero);
function wsHero() {
	var tables = content.getElementsByTagName('table');
	var hLink, heroID, heroesTable;
	//If player has no hero..
...
//Subunits Sandbox
regPageHandler(/^\/sandbox\/Human\/(hsubunits.php)?$/i,  wsHsubunits);
function wsHsubunits() {
	var theTable = xpath("//center/table",content,true);
	var submit = xpath('//input[@type="submit"]',content,true);
	var cellNr = 2;
...
regPageHandler(/^\/sandbox\/Human\/(build.php)?$/i,  wsBuild);
function wsBuild() {
	function refreshMaxCells(content) {
		var tmpGold = pGold;
		var tmpLumber = content.getElementsByTagName('table')[1].rows[0].cells[1].innerHTML.replace(/,/g,'');
...
regPageHandler(/^\/sandbox\/Human\/(military.php)?$/i,  wsMilitary);
function wsMilitary() {
	var unitsTable = content.getElementsByTagName('table')[2];
	//onClick; onBlur: Refreshed max cells
	function refreshMaxCells(unitsTable) {
