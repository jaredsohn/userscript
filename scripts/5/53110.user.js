...
// @namespace      http://userscripts.org/scripts/show/53110
...
function init() {
	...
	profil_id = document.evaluate('//ul[@id="Grid-Navigation-Main"]/li/a[text()="Meine Seite"]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.toString();
...
function parseMeineFreunde(d) {
	...
	for (var i = 0; i < friends.length; i++) {
		if (friends[i].id != "") continue;
		var url = friends[i].childNodes[1].childNodes[1].href;
		var id = url.substring(url.indexOf('Profile/')+8);
		//var id = url.substring(url.length - 16, url.length); // DELETE ME
