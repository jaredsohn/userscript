...
//
// v0.5b - May 12th, 2009
// - Fix: Changed the code marking Quick Fights for slightly better response and to try to deal with multiple elbruto.es tabs/windows
...
var brutesite='elbruto.es'; //English elbruto.es 
//var brutesite='elbruto.es'; //Española elbruto.es 
//var brutesite='labrute.fr'; //Française labrute.fr
var configpage='configure'; //English elbruto.es 
//var configpage='elconfig'; //Española elbruto.es 
//var configpage='configurez'; //Française labrute.fr
...
if(init05g!=1){
	...
	//Española elbruto.es 
	//alert('¡Versión 0.5g del El Bruto Manager instalada!\r\n\r\nHan cargado al encargado con los ajustes de defecto para cualquier nueva opción.\r\nLos defectos pueden o no pueden caber sus necesidades individuales.\r\n\r\nA la derecha superior del encargado es un acoplamiento llamado "Config & Resumen Bruto"\r\nChasque ese acoplamiento para ver cuál es nuevo y para modificar El Bruto Manager para requisitos particulares para caber sus necesidades.\r\n\r\n¡Disfrute del El Bruto Manager!');
	//Française labrute.fr
	//alert('La version 0.5g d\'La Brute Manager a installé !\r\n\r\nLe directeur a été chargé avec les arrangements de défaut pour toutes les nouvelles options.\r\nLes défauts peuvent ou peuvent ne pas adapter vos différents besoins.\r\n\r\nVers la droite supérieure du directeur est un lien appelé le "Config & Résumé Brute"\r\nCliquez sur ce lien pour voir ce qu\'il y a de neuf et pour adapter l\'La Brute Manager aux besoins du client pour adapter vos besoins.\r\n\r\nAppréciez l\'La Brute Manager !');
}
...
if(site==brutesite)	{
	...
	if (n[0]){
		//English elbruto.es 'A brutal error has been encountered!' 'Technical maintenance'
		//Española elbruto.es                                      'Mantenimiento en curso...'
		//Française labrute.fr                                     'Maintenance technique en cours...'
		if(n[0].innerHTML=='A brutal error has been encountered!' || n[0].innerHTML=='Technical maintenance'){
			pagesuccess=false;
			if(n[0].innerHTML=='A brutal error has been encountered!'){
				brutalerror=true;
			}
		}
...
if(page=='cellule' && self==top && pagesuccess){
	...
	if(managed || arched || command=='?add' || command=='?ead'){
		...
		if(n) { var thisname=n[0].innerHTML;
			...
			//Española elbruto.es                                       'Mantenimiento en curso...'
			//Française labrute.fr                                     Maintenance technique en cours...
			if(thisname=='A brutal error has been encountered!' || thisname=='Technical maintenance')
				thisname=' ';
		...
		//English elbruto.es  9,1
		//Española elbruto.es  10,1
		//Française labrute.fr 12,1
		...
		if(active_just_battled==1 && parseInt(active_next_battle)>0){
			if(parseInt(active_next_battle)!=cell_fights) {
				...
				if(ld[0]){
					news=ld[0].innerHTML;
					//English elbruto.es  'experience'
					//Española elbruto.es  'puntos'
					//Française labrute.fr 'point'
					battlegain=news.substring(news.indexOf('experience')-1,1);
...
if(config_quick_nav==1 && readytofight && cell_fights==0 && brutenum<numbrutes-1){
	...
	//English elbruto.es  My Brute Manager - Config & Brute Summary
	//Española elbruto.es El Bruto Manager - Config & Resumen Bruto
	//Française labrute.fr La Brute Manager - Config & Résumé Brute
	var topbar='<table width="250" border="0" cellspacing="0" cellpadding="0"><tr><td height="13" colspan="2"><div id="mbmtopbar" style="font-size:x-small; background-color:#996633; width:250px; height:13px; color:#FFFFFF"><a href="http://www.gamertoolz.com/mybrute/mybrutemanager/" target="gamertoolz" style="text-decoration:none;font-size:x-small;color:#FFFFFF;">My Brute Manager v'+version+'</a> | <a href="http://www.gamertoolz.com/mybrute/mybrutemanager/'+configpage+'.php" style="text-decoration:none;font-size:x-small;color:#FFFFFF;">Config & Brute Summary</a></div></td></tr>';
	var menu='<td width="110" align="right" valign="top"><div id="mbmmenu">';
	//English elbruto.es  "Remove '+thisbrute+' from your brute list"
	//Española elbruto.es "Quite '+thisbrute+' de su lista bruto"
	//Française labrute.fr "Enlevez '+thisbrute+' de votre liste brute"
	var menudel='<div id="mbmdel" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/log_lose.gif);background-position:-5px -5px;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?del\',\'_self\')" title="Remove '+thisbrute+' from your brute list"></div>';
	//English elbruto.es  "Add '+thisbrute+' to your brute list"
	//Española elbruto.es "Agregue '+thisbrute+' a su lista bruto"
	//Française labrute.fr "Ajoutez '+thisbrute+' à votre liste brute"
	var menuadd='<div id="mbmadd" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/log_win.gif);background-position:-5px -5px;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?add\',\'_self\')" title="Add '+thisbrute+' to your brute list"></div>';
	//English elbruto.es  "Attack '+thisbrute+' with '+GM_getValue('active_current_brute','')+'"
	//Española elbruto.es "Ataque '+thisbrute+' con el '+GM_getValue('active_current_brute','')+'"
	//Française labrute.fr "Attaque '+thisbrute+' avec l\''+GM_getValue('active_current_brute','')+'"
	var menuattack='<div id="mbmatk" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/lvl_3.gif);background-position:-1px -1px;background-repeat:no-repeat;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+active_current_brute+'.'+brutesite+'/vs/'+thisbrute+'\',\'_self\')" title="Attack '+thisbrute+' with '+GM_getValue('active_current_brute','')+'"></div>';
	//English elbruto.es  "Add '+thisbrute+' to your list of enemies"
	//Española elbruto.es "Agregue '+thisbrute+' a su lista de enemigos"
	//Française labrute.fr "Ajoutez '+thisbrute+' à votre liste d\'ennemis"
	var menuenemyadd='<div id="mbmnmeadd" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/lvl_2.gif);background-position:-1px -1px;background-repeat:no-repeat;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?ead\',\'_self\')" title="Add '+thisbrute+' to your list of enemies"></div>';
	//English elbruto.es  "Remove '+thisbrute+' from your list of enemies"
	//Española elbruto.es "Quite '+thisbrute+' de su lista de enemigos"
	//Française labrute.fr "Enlevez '+thisbrute+' de votre liste d\'ennemis"
	var menuenemydel='<div id="mbmnmeadd" style="width:35px;height:35px;background-image:url(http://data.'+brutesite+'/img/log_child.gif);background-position:-5px -5px;background-repeat:no-repeat;text-align:center;vertical-align:bottom;float:right;cursor:pointer;" onclick="window.open(\'http://'+thisbrute+'.'+brutesite+'/cellule/?ede\',\'_self\')" title="Remove '+thisbrute+' from your list of enemies"></div>';
	...
	if(managed && config_show_quick_fight==1 && cell_fights>0){
		...
		if(quick_opp!='' && quick_opp.length>1){
			if(readytofight){
				//English elbruto.es  Attack
				//Española elbruto.es Ataque
				//Française labrute.fr Attaque
				if(config_name_quick_fight==1) oppname='Attack '+quick_opp+'('+quick_level+')<br />';
				GM_setValue('active_quick_fight_loaded',1);
				//English elbruto.es  v=14
				...
				//English elbruto.es  'Grabbing '+quick_opp+'('+quick_level+') ...'
				//Española elbruto.es ''+quick_opp+'('+quick_level+') que ase ...'
				//Française labrute.fr ''+quick_opp+'('+quick_level+') de saisie ...'
				battlebutton='Grabbing '+quick_opp+'('+quick_level+') ...';
			}
		}
		else{
			//English elbruto.es  Complicated Grammar, will address later
			//Española elbruto.es Complicated Grammar, will address later
			//Française labrute.fr Complicated Grammar, will address later
			var arenalookupbrute=thisbrute;
			if(active_helper_brute!='') {
				arenalookupbrute=active_helper_brute;
				quick_level='starting at Level '+active_helper_level;
			}
			...
			if(qfcount>0){
				...
				battlebutton='Searching for '+cell_fights+' Random opponent(s) '+quick_level+'';
			}
		}
	...
	if(config_arch_enemies==1 && numenemies>0){
		...
		//English elbruto.es Looking for
		//Española elbruto.es Buscar 
		//Française labrute.fr Recherche de
		enemyaction='var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Looking for \'+enemy+\'...\'; var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/vs/\'+enemy+\'\';';
		//English elbruto.es Your Enemies
		//Española elbruto.es Sus Enemigos
		//Française labrute.fr Vos Ennemis
		enemydropdown='<form id="enemyform" name="enemyform" method="post" action="">	<select name="enemychooser" style="width: 125px;" onchange="javascript:var enemy=0; enemy=document.enemyform.enemychooser.options[document.enemyform.enemychooser.selectedIndex].value; if(enemy!=0){ '+enemyaction+' }">    <option value="0">Your Enemies</option>';
		for(i=0;i<numenemies;i++){
	...
	controller=controller+'</map>';
		...
		//English elbruto.es Back to '+GM_getValue('active_current_brute','')+'\'s Cell
		//Española elbruto.es De nuevo a la célula de '+GM_getValue('active_current_brute','')+'
		//Française labrute.fr De nouveau à la cellule de '+GM_getValue('active_current_brute','')+'
		controller=controller+'<a href="http://'+GM_getValue('active_current_brute','')+'.'+brutesite+'/cellule" style="font-size:small;">Back to '+GM_getValue('active_current_brute','')+'\'s Cell</a>';
	}
	...
	//Española elbruto.es 
	//rightcol[0].innerHTML='<div>¡Bienvenido a la Arena!</div><div>Aquí tienes una selección de adversarios de tu nivel. Elige uno, ¡y que empiece el combate!</div>';
	//Française labrute.fr 
	//rightcol[0].innerHTML='<div>Bienvenue dans l\'arène !</div><div>J\'ai sélectionné quelques adversaire à ta taille. Choisis-en un, et on commence le combat !</div>';
			...
			if(config_arena_bonus_preview==1){
			   var hoveraction="";
				...
				if(config_arch_enemies==1){
					//English elbruto.es  "Add '+opp+' to your list of enemies"
					//Española elbruto.es "Agregue '+opp+' a su lista de enemigos"
					//Française labrute.fr "Ajoutez '+opp+' à votre liste d\'ennemis"
					enemybutton='<div id="mbmnmearena'+(i+1)+'" style="width:25px;height:15px;background-color:#FF0000;text-align:center;vertical-align:middle;float:right;overflow:hidden;font-size:xx-small;color:#000000;" onclick="window.open(\'http://'+opp+'.'+brutesite+'/cellule/?ead\',\''+config_arena_enemy_target+'\'); return false;" title="Add '+opp+' to your list of enemies">ARCH</div>';
               	}
			...
			if(config_arch_enemies==1){
				for(j=1;j<=6;j++){
					if(enemies[i]==arenaopps[j]){
						...
						//English elbruto.es  "Remove "+opp+" from your list of enemies"
						//Española elbruto.es "Quite "+opp+" de su lista de enemigos"
						//Française labrute.fr "Enlevez "+opp+" de votre liste d\'ennemis"
                		thebutton.setAttribute("title","Remove "+opp+" from your list of enemies");
	...
	var ql=false;
		...
		if(!pagesuccess){
			...
			if(active_quick_fight_retry>0){
				...
				if(ql[0]){
					//English elbruto.es  'Unable to Find An Opponent<br />Trying again...'
					//Española elbruto.es  'Incapaz de encontrar a un opositor<br />El intentar otra vez...'
					//Française labrute.fr 'Incapable de trouver un adversaire<br />Essai encore...'
					ql[0].innerHTML='Unable to Find An Opponent<br />Trying again...';
				}
			}
			else if(ql[0]){
				//English elbruto.es
				...
				//Española elbruto.es 
				//ql[0].innerHTML='Incapaz de encontrar a un opositor<br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/arene\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Buscar a un opositor al azar...\';">Intente a otro bruto al azar</a>';
				//Française labrute.fr 
				//ql[0].innerHTML='Incapable de trouver un adversaire<br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/arene\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Recherche d\'un adversaire aléatoire...\';">Jugez une autre brute aléatoire</a>';
			}
			else {
	...
	if(pagesuccess){
		...
		if(sf[0] && ql[0]){
			...
			if(button[0]){
				...
				//Española elbruto.es Ataque
				//Française labrute.fr Attaque
				if(config_name_quick_fight==1) oppname='Attack '+opp+'('+opplevel+')<br />';
				GM_setValue('active_quick_fight_loaded',1);
				//English elbruto.es
	...
	if(!pagesuccess && ql[0]){
			...
			if(active_quick_fight_retry>0){
				...
				//Española elbruto.es 
				//ql[0].innerHTML='Incapaz de encontrar a un opositor<br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/arene\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Buscar a un opositor al azar...\';">Intente a otro bruto al azar</a><br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/vs/'+opp+'\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Buscar '+opp+'...\';">Intento '+opp+' otra vez</a>';
				//Française labrute.fr 
				//ql[0].innerHTML='Incapable de trouver un adversaire<br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/arene\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Recherche d\'un adversaire aléatoire...\';">Jugez une autre brute aléatoire</a><br /><a style="font-size:small;" onclick="javascript:var battleframe=document.getElementById(\'battleframe\'); battleframe.src=\'http://'+thisbrute+'.'+brutesite+'/vs/'+opp+'\'; var battlediv=document.getElementsByClassName(\'vsLaunch\'); battlediv[0].innerHTML=\'Recherche de '+opp+'...\';">Essai '+opp+' encore</a>';
			}
	}
}
	...
	if(pagesuccess){
		...
		if(bonuses[1]){
			...
			if(n) { var thisname=n[0].innerHTML;
				...
				//if(thisname.substr(0,9)=='Celda de')
				//	thisname=thisname.substr(9,thisname.length-9);
							//Française labrute.fr <h1>Cellule de ugobos</h1>
				//if(thisname.substr(0,11)=='Cellule de ')
				//	thisname=thisname.substr(11,thisname.length-11);

				//English elbruto.es  'A brutal error has been encountered!' || thisname=='Technical maintenance'
				//Española elbruto.es 
				//Française labrute.fr                                Maintenance technique en cours...
				if(thisname=='A brutal error has been encountered!' || thisname=='Technical maintenance')
					thisname=' ';
		...
		//English elbruto.es  '+thisname+'\'s Bonuses Victories
		//Española elbruto.es  Primas de '+thisname+' Victorias
		//Française labrute.fr Bonifications de '+thisname+' Victoires
		fullpage.innerHTML='<div id="bonuspreview" style="font-size:large;position: absolute; top: 0px; left: 0px;background-color:#FAF8C3;width:217px;height:420px; border:0px; margin: 0px; padding: 0px; overflow:hidden;">'+thisname+'\'s Bonuses<br />'+thisvictory+' Victories<br /><embed type="application/x-shockwave-flash" src="http://data.'+brutesite+'/swf/uc.swf?v=15" id="inventory" name="inventory" bgcolor="#FAF8C3" quality="high" menu="false" allowscriptaccess="always" flashvars="'+invid+'" height="400" sc wmode="opaque"></div>';
		}
	}
	else{
