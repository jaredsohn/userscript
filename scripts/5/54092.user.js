...
items[299] = {item_id:299, nshort:'band_grey', name:'Серая бандана', type:'head', level:1, price:8, image:'images/items/head/band_grey.png', image_mini:'images/items/head/mini/band_grey.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{trade:1}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};\
\
\
...
items[729] = {item_id:729, nshort:'stolen_goods', name:'Контрабандный товар', type:'yield', level:null, price:110, image:'images/items/yield/stolen_goods.png', image_mini:'images/items/yield/stolen_goods.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};\
...
items[776] = {item_id:776, nshort:'kates_ring', name:'Кольцо Кейт', type:'yield', level:null, price:1000, image:'images/items/yield/kates_ring.png', image_mini:'images/items/yield/kates_ring.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};\
...
items[1709] = {item_id:1709, nshort:'gold', name:'Сокровища индейцев', type:'yield', level:null, price:26000, image:'images/items/yield/gold.png', image_mini:'images/items/yield/gold.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};\
items[1711] = {item_id:1711, nshort:'key2', name:'Второй ключ', type:'yield', level:null, price:46, image:'images/items/yield/key2.png', image_mini:'images/items/yield/key2.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'shop'};\
...
raboty[72] = {rus_name:'Преследование бандитов', name:'bandits', malus:384, navyki:{tough:1,endurance:1,hide:1,leadership:1,tactic:1}, resultaty:{dengi:28, opyt:75, vezenie:85, boom:83, produkty:{0:5,774:2}}};\
...
raboty[78] = {rus_name:'Кража со взломом', name:'burglary', malus:517, navyki:{endurance:1,hide:2,tactic:1,trade:1}, resultaty:{dengi:80, opyt:34, vezenie:81, boom:26, produkty:{0:12}}};\
...
raboty[83] = {rus_name:'Контрабанда', name:'smuggle', malus:410, navyki:{hide:1,swim:1,shot:1,trade:1,appearance:1}, resultaty:{dengi:62, opyt:45, vezenie:83, boom:56, produkty:{0:22}}};\
...
\
raboty[105] = {rus_name:'Форт. Атака', name:'attack', malus:0, navyki:{aim:1,dodge:1,endurance:1,leadership:1}, resultaty:{dengi:0, opyt:0, vezenie:0, boom:100, produkty:{}}};\
raboty[106] = {rus_name:'Форт. Атака (мет. и увёрт. по 0,5)', name:'attack', malus:0, navyki:{aim:0.5,dodge:0.5,endurance:1,leadership:1}, resultaty:{dengi:0, opyt:0, vezenie:0, boom:100, produkty:{}}};\
raboty[107] = {rus_name:'Форт. Защита', name:'defend', malus:0, navyki:{aim:1,dodge:1,hide:1,leadership:1}, resultaty:{dengi:0, opyt:0, vezenie:0, boom:100, produkty:{}}};\
raboty[108] = {rus_name:'Форт. Защита (мет. и увёрт. по 0,5)', name:'defend', malus:0, navyki:{aim:0.5,dodge:0.5,hide:1,leadership:1}, resultaty:{dengi:0, opyt:0, vezenie:0, boom:100, produkty:{}}};\
\
\
	...
	for(vv in bagazh.items){
		...
		if (!unsafeWindow.bi2_uchet[tid]){
	    	...
	    	if (unsafeWindow.items[tid]){
				...
				if (!cres.h)	{
					unsafeWindow.einfo+='Часть данных о предмете неверна:\n';
					unsafeWindow.assign_citem(tid, cobj);
					unsafeWindow.einfo+=unsafeWindow.print_citem(tid)+'\n';
				}
				else if(!cres.s){
					unsafeWindow.winfo+='Неправильное название у предмета:\n';
					unsafeWindow.assign_citem(tid, cobj);
					unsafeWindow.winfo+=unsafeWindow.print_citem(tid)+'\n';
				}
			}
			else{
				unsafeWindow.items[tid] = {item_id:tid, nshort:'nothing', name:'Заглушка', type:'yield', level:0, price:0, image:'images/items/right_arm/clayjug.png', image_mini:'images/items/right_arm/mini/clayjug.png', characterClass:null, characterSex:null, speed:null, bonus:{skills:{}, attributes:{}}, set:{key:null, name:null}, shop:'nil'};
				unsafeWindow.assign_citem(tid, cobj);
				unsafeWindow.einfo+='Отсутствует описание предмета:\n';
				unsafeWindow.einfo+=unsafeWindow.print_citem(tid)+'\n';
			}
		}
    }
    for(vv in unsafeWindow.Wear.wear){
		...
		if (!unsafeWindow.bi2_uchet[tid]){
	    	...
	    	if (unsafeWindow.items[tid]){
				...
				cres=unsafeWindow.compare_citem(tid, cobj);
				if (!cres.h)	{
					unsafeWindow.einfo+='Часть данных у предмента неверна:\n';
					unsafeWindow.assign_citem(tid, cobj);
					unsafeWindow.einfo+=unsafeWindow.print_citem(tid)+'\n';
				}
				else if(!cres.s){
...
unsafeWindow.bi2_res2html = function (){
    ...
    while (count_rab < 255){
        ...
        if (count_rab!=111){
	    	...
	    	ihtm += '<span class="skill_box_value green_text" style="text-align:center;">х'+unsafeWindow.resultaty[count_rab].ton+'</span></span>';
...
unsafeWindow.bi2_sortir = function (tip, minto){
    ...
    for (irab in unsafeWindow.bi2_htmlrab){
        ...
        case 'boom':
            ...
            break;
        case 'name':
            unsafeWindow.bi2_sortrab[ind_arr].ves= (irab > 100) ? 'я ' : '';
...
unsafeWindow.bi2_vesch_polezna = function(value){
    for (kh in unsafeWindow.bi2_khochuka)
        unsafeWindow.bi2_khochuka[kh] = false;
    if (value > 0)
        unsafeWindow.bi2_khochuka[value] = true;
    unsafeWindow.bi2_hideraboty(value);
}

unsafeWindow.bi2_reporter2 = function(){
    grgr = '';
    unsafeWindow.bi2_process=false;
    //new unsafeWindow.HumanMessage('Начинаем вывод данных', {type: 'success'});
    ...
    if (unsafeWindow.bi2_khochuka.length > 0){
        ...
        if (unsafeWindow.bi2_khochuka.length > 1){
            ...
            for (kh in unsafeWindow.bi2_khochuka){
                grgr += '<option value=\"'+kh+'\">'+unsafeWindow.items[kh].name+'</option>';
            }
            grgr += '</select>';
        }
        for (kh in unsafeWindow.bi2_khochuka){
        if (unsafeWindow.bi2_khochuka[kh]){
            grgr += '<table><tr><td>';
            ...
            grgr +='Всего доступных  работ:' + unsafeWindow.chislo_rabot_to + '; &laquo;'+vesch.name+rere+ unsafeWindow.khoroshi_to[kh] +' раз.<br />';
            grgr +='Всего работ:' + unsafeWindow.chislo_rabot + '; &laquo;'+vesch.name+rere+ unsafeWindow.khoroshi[kh] +' раз.';
            grgr += '</td></tr></table>';
        }}
        grgr += '<hr>';
        grgr += '</td></tr>';
...
unsafeWindow.bi2_vreporter = function () {
    unsafeWindow.bi2_show_window();
    vrvr = '<table>';
    for (count_rab in unsafeWindow.porabotaj){
        if (!unsafeWindow.prosto_veschi[count_rab])
            continue;
        vrvr += '<tr>';
        rabota = unsafeWindow.raboty[count_rab];
        vrvr += '<td rowspan=\"2\">';
        ...
        for (ri in rres.produkty){
            ...
            vrvr+='title=\"'+unsafeWindow.items[ri].name+'\" alt=\"'+unsafeWindow.items[ri].name+'\" ';
            vrvr+='src=\"'+unsafeWindow.items[ri].image_mini+'\" /></div><div class=\"jy_bi2\">'+rres.produkty[ri]+'%</div>';
            vrvr+='</div>';
        }
        vrvr += '</td></tr>';
        vrvr += '<tr><td>';
        ...
        if (count_rab!=111){
		    ...
		    vrvr += (unsafeWindow.resultaty[count_rab].to+unsafeWindow.raboty[count_rab].malus-unsafeWindow.resultaty[count_rab].ton)+'</span></span>';
            vrvr += '<span title=\"Очки от комплектов\" class="calculation_visualisation img_plus" style=\"margin-left:0px;\">';
	    	vrvr += '<span class="skill_box_value green_text" style="text-align:center;">'+unsafeWindow.resultaty[count_rab].ton+'</span></span>';
		    vrvr += '<span title=\"Сложность работы\" class="calculation_visualisation img_minus"  style=\"margin-left:0px;\">';
    		vrvr += '<span class="skill_box_value" style="text-align:center;">'+unsafeWindow.raboty[count_rab].malus+'</span></span>';
    		...
    		vrvr += (unsafeWindow.resultaty[count_rab].to > 0) ? 'rgb(0,255,0)' : 'rgb(255,0,0)';
	    	vrvr += '">'+unsafeWindow.resultaty[count_rab].to+'</span></span>';
	    }
        vrvr += '</td><td>';
        ...
        for (jj in unsafeWindow.rabnavyki[count_rab]){
            for (aa = unsafeWindow.rabnavyki[count_rab][jj].mul; aa > 0; --aa){
                ...
                vrvr += '<td><a class=\"tt3\" href=\"#\" ><span>'+unsafeWindow.pers.skill_titles[jj]+':'+unsafeWindow.rabnavyki[count_rab][jj].znach+'</span>';
                vrvr += '<div class=\"skill_box\" style=\"padding: 35px 28px 6px 2px; background: transparent url(/images/skill/skills_'+unsafeWindow.bi2_s2a[jj];
                vrvr += '.png) repeat scroll '+unsafeWindow.bi2_s2px[jj]+'px 0px; color: rgb(255, 255, 255); ';
                ...
                vrvr += unsafeWindow.rabnavyki[count_rab][jj].znach+'</div>';
                vrvr += '</a></td>';
            }
        }
        vrvr += '</tr></tbody></table>';
        ...
        for (ee = 0; ee < unsafeWindow.bi2_types.length; ++ee){
            ctype = unsafeWindow.bi2_types[ee];
            vrvr += '<tr><td>';
            for (vv = unsafeWindow.prosto_veschi[count_rab][ctype].length-1; vv >= 0;  --vv){
                sid = unsafeWindow.prosto_veschi[count_rab][ctype][vv].tid;
                vrvr+='<div style=\"display:inline; float:left;\">';
                vesch = unsafeWindow.items[sid];
                vrvr+='<a class=\"tt2\" href=\"#\" ><span><b>'+vesch.name+':</b>&nbsp;'+unsafeWindow.prosto_veschi[count_rab][ctype][vv].bon;
                if (vesch.set.key){
                    vrvr += '<br /><em>'+vesch.set.name+'</em>';
                }
                for (ind in vesch.bonus.attributes){
                    vrvr += unsafeWindow.bi2_a_print(ind, vesch.bonus.attributes[ind]);
                }
                for (ind in vesch.bonus.skills){
                    vrvr += unsafeWindow.bi2_s_print(ind, vesch.bonus.skills[ind]);
                }
                vrvr += '</span>'
                vrvr+='<div class=\"bag_item\" ><img src=\"'+vesch.image_mini+'\" />';
                vrvr+='<div class="bag_item_count"><p style="text-align:center;">'+unsafeWindow.prosto_veschi[count_rab][ctype][vv].bon+'</p></div></div>'
                vrvr+='</a>'
                if (unsafeWindow.prosto_veschi[count_rab][ctype][vv].price > 0){
                    vrvr+='<br />';
                    vrvr +='<span style=\"text-align:center;\">'+unsafeWindow.prosto_veschi[count_rab][ctype][vv].price+'&nbsp;$</span>';
                }
                vrvr +='</div>';
            }
            vrvr += '</td></tr>'
...
//    new unsafeWindow.HumanMessage('Начинаем вывод полученных данных', {type: 'success'});
    grgr='';
    ...
    if (unsafeWindow.bi2_khochuka.length > 0){
        unsafeWindow.chislo_rabot = 0;
        unsafeWindow.chislo_rabot_to = 0;
        unsafeWindow.khoroshi = [];
        unsafeWindow.khoroshi_to = [];
        for (kh in unsafeWindow.bi2_khochuka){
            unsafeWindow.khoroshi[kh] = 0;
            unsafeWindow.khoroshi_to[kh] = 0;
            unsafeWindow.bi2_khochuka[kh]=false;
        }
        unsafeWindow.bi2_khochuka[kh]=true; // last item;
        for (rr in unsafeWindow.porabotaj){
            if(unsafeWindow.resultaty[rr]){
                ++unsafeWindow.chislo_rabot;
                if (unsafeWindow.resultaty[rr].to > 0) ++unsafeWindow.chislo_rabot_to;
                for (ii = unsafeWindow.bi2_types.length-1; ii >= 0; --ii){
                    for (kh in unsafeWindow.bi2_khochuka){
                        if (unsafeWindow.resultaty[rr].items[ii].tid == kh){
                            unsafeWindow.khoroshi[kh] += 1;
                            if (unsafeWindow.resultaty[rr].to > 0) unsafeWindow.khoroshi_to[kh] += 1;
                        }
                    }
                }
            }
            if(unsafeWindow.resultaty_z[rr]){
                ++unsafeWindow.chislo_rabot;
                ++unsafeWindow.chislo_rabot_to;
                for (ii = unsafeWindow.bi2_types.length-1; ii >= 0; --ii){
                    for (kh in unsafeWindow.bi2_khochuka){
                        if (unsafeWindow.resultaty_z[rr].items[ii].tid == kh){
                            unsafeWindow.khoroshi[kh] += 1;
                            unsafeWindow.khoroshi_to[kh] += 1;
                        }
                    }
                }
            }
...
unsafeWindow.bi2_s_print = function(nav, val){
    ...
    result+='\" >'+unsafeWindow.pers.skill_titles[nav]+':'+val+'</div>';
    */
...
bi2_vybvesch = function () {\
	for (irabota in porabotaj) {\
		...
		resultaty[irabota] = {};\
		resultaty[irabota].to = summa_ochkov2(pers.skills, rabota.navyki);\
...
unsafeWindow.bi2_worker = function(schet){
	...
	if (!unsafeWindow.bi2_inv_imported){
	    ...
	    if (!unsafeWindow.bi2_inv_imported){
	        new unsafeWindow.HumanMessage('Предварительно необходимо импортировать багаж. Откройте, и дождитесь загрузки.<br />После полной загрузки багажа, его можно свернуть или закрыть.');
	        unsafeWindow.bi2_process=false;
	        return;
	    }
	}
	...
	if (schet){
        ...
        unsafeWindow.bi2_vybvesch();
    }
}


unsafeWindow.bi2_simselect='<select class=\"bi2_sel\" id=\"bi2_rabota\" size=\"1\" onchange=\"javascript:$(\'bi2_skol_rabot_o\').checked=true;\">';
unsafeWindow.bi2_mulselect='<select title=\"Выбор нескольких работ &mdash; с нажатой клавишей Ctrl\" class=\"bi2_sel\" multiple=\"multiple\" id=\"bi2_rabota\" size=\"6\" onchange=\"javascript:$(\'bi2_skol_rabot_n\').checked=true;\">';
unsafeWindow.bi2_conselect='\
<option value=\"0\">	Выбор работы:	</option>\
...
<option value=\"105\">	Атака форта	</option>\
<option value=\"106\">	Атака форта (метк. и увёрт. по 1/2)	</option>\
<option value=\"107\">	Защита форта	</option>\
<option value=\"108\">	Защита форта (метк. и увёрт. по 1/2)	</option>\
...
unsafeWindow.bi2_show_shmot = function(irab){
    ...
    if (!bi2_shmot){
		...
		html2 += '<span>';
		html2 += '<a href=\"javascript:bi2_close_shmot();\"' + unsafeWindow.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html2 += '</span>';
...
unsafeWindow.bi2_show_window = function(){
    ...
    if (!bi2_window){
		...
		html1 += '<span>';
		html1 += '<a href=\"javascript:bi2_minimize_window();\"' + unsafeWindow.bi2_tlink + ' title=\"Свернуть\">&nbsp;_&nbsp;</a>&nbsp;';
		html1 += '<a href=\"javascript:bi2_stretch_window();\"' + unsafeWindow.bi2_tlink + ' title=\"-\">&nbsp;^&nbsp;</a>&nbsp;';
		html1 += '<a href=\"javascript:bi2_close_window();\"' + unsafeWindow.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html1 += '</span>';
...
unsafeWindow.bi2_show_panel = function(){
	...
	if (!bi2_title) {
		...
		html0 += '<span>';
		html0 += '<a href=\"javascript:bi2_minimize_title();\"' + unsafeWindow.bi2_tlink + ' title=\"Свернуть\">&nbsp;_&nbsp;</a>&nbsp;';
		html0 += '<a href=\"javascript:bi2_stretch_title();\"' + unsafeWindow.bi2_tlink + ' title=\"Развернуть\">&nbsp;^&nbsp;</a>&nbsp;';
		html0 += '<a href=\"javascript:bi2_close_title();\"' + unsafeWindow.bi2_tlink + ' title=\"Закрыть\">&nbsp;*&nbsp;</a>&nbsp;';
		html0 += '</span>';
...
<form id=\"bi2_form\">\
	...
	<input type= \"button\" title=\"Просто показывает не больше семи предметов каждого типа (из всех, с выбранными параметрами отсечки) с наибольшими ТО, без учёта комплектов.\" value=\"Предметы с ТО\" style=\"float:right;\" onclick=\"javascript:bi2_worker(false)\"/>\
	<input id=\"bi2_skol_rabot_v\" name=\"bi2_skol_rabot\" type=\"radio\" value=\"v\" style\"margin:auto 5px;\" />Все&nbsp;работы<br />\
	<input id=\"bi2_skol_rabot_n\" name=\"bi2_skol_rabot\" type=\"radio\" value=\"n\" style\"margin:auto 5px;\" onchange=\"javascript:bi2_vselect(true);void(0)\" />Нескольно&nbsp;работ (на&nbsp;выбор)<br />\
	<input id=\"bi2_skol_rabot_o\" name=\"bi2_skol_rabot\" type=\"radio\" value=\"o\" checked=\"checked\" style\"margin:auto 5px;\" onchange=\"javascript:bi2_vselect(false);void(0)\" />Одна&nbsp;работа\
	</div>\
    ...
    for (vv = 1; vv < 900; ++vv){
        ...
        if (unsafeWindow.items[vv]){
            html0 +='<option value=\"'+vv+'\">	'+unsafeWindow.items[vv].name+'	</option>';
        }
    }
    html0 += '</optgroup><optgroup title=\"Остатки комплектов\" label=\"Остатки комплектов\">';
    // добавим сетовых ручками
    html0 += '<option value=\"792\">	'+unsafeWindow.items[792].name+'	</option>';
    html0 += '<option value=\"794\">	'+unsafeWindow.items[794].name+'	</option>';
    html0 += '<option value=\"797\">	'+unsafeWindow.items[797].name+'	</option>';
    html0 += '<option value=\"768\">	'+unsafeWindow.items[768].name+'	</option>';
    html0 += '<option value=\"723\">	'+unsafeWindow.items[723].name+'	</option>';
    html0 += '<option value=\"1715\">	'+unsafeWindow.items[1715].name+'	</option>';
    html0 += '<option value=\"854\">	'+unsafeWindow.items[854].name+'	</option>';		
    html0 += '</optgroup>';
html0 += '</select></div></div>\
	...
	<div' +
		unsafeWindow.bi2_vblock +
		'>\
		<span title=\"Здесь можно выбрать слоты (части тела) которые будут оставаться пустыми, то есть скрипт не будет подбирать предметы, для не отмеченных частей\"><input id=\"bi2_sloty\" value=\"bi2_sloty\" type=\"checkbox\" onchange=\"javascript:if (checked) {$(\'bi2_sloty_content\').style.display=\'block\'}else{$(\'bi2_sloty_content\').style.display=\'none\'};void(0)\" style=\"margin:auto 23px auto 23px;\" />Не&nbsp;учитываем <strong>не&nbsp;выбранные</strong>&nbsp;слоты:<br /></span>\
	...
	<div' +
		...
		<input id=\"bi2_rigth_arm\" type=\"checkbox\" checked=\"1\" style=\"margin:auto 23px auto 21px;\" />Дуэльное оружие<br />\
