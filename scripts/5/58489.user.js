...
if (!initialized) {
  ...
  var autoStatDescrips  = ['target value', 'ratio against Level', 'ratio against Attack',
                           'ratio against Defense', 'ratio against Health', 'ratio against Energy',
                           'ratio against Stamina'];
  var autoStatModes     = ['autoStatAttackMode', 'autoStatDefenseMode', 'autoStatHealthMode',
                           'autoStatEnergyMode', 'autoStatStaminaMode'];
  var autoStatValues    = ['autoStatAttack', 'autoStatDefense', 'autoStatHealth',
                           'autoStatEnergy', 'autoStatStamina'];
  ...
  // job description, unadjusted energy cost, job number, tab number, unadjusted exp payout
...
function doAutoPlay () {
  ...
  if (staminaFlag && skipStaminaSpend) {
...
function autoAccept() {
  ...
  var elt = xpathFirst('.//li[contains(@class, "tab_on")]//a[contains(@onclick, "controller=recruit")]', innerPageElt);
  if (!elt) {
  ...
  elt = xpathFirst('.//a[contains(@onclick, "action=accept") and contains(@onclick, "user_id=all")]', innerPageElt);
  if (!elt) {
...
function autoStat() {
  ...
  var statPts = new Array(5);
  var statFallbacks = new Array(5);
  var curStats = [curAttack,curDefense,maxHealth,maxEnergy,maxStamina];
  var modeStats = [1,level,curAttack,curDefense,maxHealth,maxEnergy,maxStamina];

  for (var i = 0; i < curStats.length; i++) {
    // Calculate the Points needed to reach target stat     
    statPts[i] = (modeStats[GM_getValue(autoStatModes[i])] * GM_getValue(autoStatValues[i])) - curStats[i];

    // Account for priority
    statPts[i] = statPts[i] * (Math.pow(10,4) / Math.pow(10, GM_getValue(autoStatPrios[i])));    
    
    // Fallback method
    statFallbacks[i] = GM_getValue(autoStatFallbacks[i]) == 'checked' ? i : 'E';
  }
  ...
  if (maxPts <= 0) {
    ...
    if (statFallbacks.join("") != "EEEEE") {            
      ...
      while (statFallbacks.indexOf(nextStat) == -1)
        nextStat = (nextStat + 1) % 5;

      DEBUG('Next stat to increment: ' + nextStat);        
  ...
  switch (statIndex) {
    ...
    default           : 
      addToLog('warning Icon', 'BUG DETECTED: Invalid calculated maxPtsToTarget value "' + maxPtsToTarget + '".');
...
function canMission() {
  ...
  if (GM_getValue('multipleJobs') == 'checked' &&
    ...
    for (var i=0; i < multiple_jobs_list.length; i++) {
      var mission = missions[multiple_jobs_list[i]];
      ...
      mission[7] = job_ratio;
      mission[8] = cost;
      multiple_jobs_ratio_sorted.push(multiple_jobs_list[i]);
      
      if (mission[8] <= energy) {
        enoughEnergy = true;
        if (mission[6] >= expLeft) {
          singleJobLevelUp.push(multiple_jobs_list[i]);
        }
      }
    }
...
function clearOldCheckBoxStatuValues() {
  ...
  if (isNaN (parseInt (GM_getValue('autoStatAttack', 0))))
    GM_setValue('autoStatAttack', 0);
  if (isNaN (parseInt (GM_getValue('autoStatDefense', 0))))
    GM_setValue('autoStatDefense', 0);
  if (isNaN (parseInt (GM_getValue('autoStatHealth', 0))))
    GM_setValue('autoStatHealth', 0);
  if (isNaN (parseInt (GM_getValue('autoStatEnergy', 0))))
    GM_setValue('autoStatEnergy', 0);
  if (isNaN (parseInt (GM_getValue('autoStatStamina', 0))))
...
function saveDefaultSettings() {
  ...
  GM_setValue('autoStatAttackMode',  0);
  GM_setValue('autoStatDefenseMode', 0);
  GM_setValue('autoStatHealthMode',  0);
  GM_setValue('autoStatEnergyMode',  0);
  GM_setValue('autoStatStaminaMode', 0);  
  GM_setValue('autoStatAttackPrio',  0);
  GM_setValue('autoStatDefensePrio', 0);
  GM_setValue('autoStatHealthPrio',  0);
  GM_setValue('autoStatEnergyPrio',  0);
  GM_setValue('autoStatStaminaPrio', 0);  
  GM_setValue('autoStatAttackFallback',  0);
  GM_setValue('autoStatDefenseFallback', 0);
  GM_setValue('autoStatHealthFallback',  0);
  GM_setValue('autoStatEnergyFallback',  0);
  GM_setValue('autoStatStaminaFallback', 0);      
  GM_setValue('autoStatAttack',  0);
  GM_setValue('autoStatDefense', 0);
  GM_setValue('autoStatHealth',  0);
  GM_setValue('autoStatEnergy',  0);
...
*/
  ...
  var autoStatAttack  = document.getElementById('autoStatAttack').value;
  var autoStatDefense = document.getElementById('autoStatDefense').value;
  var autoStatHealth  = document.getElementById('autoStatHealth').value;
  var autoStatEnergy  = document.getElementById('autoStatEnergy').value;
  var autoStatStamina = document.getElementById('autoStatStamina').value;
  if (autoStatOn &&
      (isNaN(autoStatAttack)  ||
       isNaN(autoStatDefense) ||
       isNaN(autoStatHealth)  ||
       isNaN(autoStatEnergy)  ||
       isNaN(autoStatStamina))) {
    alert('Please enter valid numbers for auto-stat (Misc tab).');
    return;
  }
  ...
  //                           
  GM_setValue('restAutoStat', 0);
  GM_setValue('autoStatAttack',  autoStatAttack);
  GM_setValue('autoStatDefense', autoStatDefense);
  GM_setValue('autoStatHealth',  autoStatHealth);
  GM_setValue('autoStatEnergy',  autoStatEnergy);
  GM_setValue('autoStatStamina', autoStatStamina);  
  GM_setValue('autoStatAttackMode',  document.getElementById('autoStatAttackMode').selectedIndex);
  GM_setValue('autoStatDefenseMode', document.getElementById('autoStatDefenseMode').selectedIndex);
  GM_setValue('autoStatHealthMode',  document.getElementById('autoStatHealthMode').selectedIndex);
  GM_setValue('autoStatEnergyMode',  document.getElementById('autoStatEnergyMode').selectedIndex);
  GM_setValue('autoStatStaminaMode', document.getElementById('autoStatStaminaMode').selectedIndex);
  GM_setValue('autoStatAttackPrio',  document.getElementById('autoStatAttackPrio').selectedIndex);
  GM_setValue('autoStatDefensePrio', document.getElementById('autoStatDefensePrio').selectedIndex);
  GM_setValue('autoStatHealthPrio',  document.getElementById('autoStatHealthPrio').selectedIndex);
  GM_setValue('autoStatEnergyPrio',  document.getElementById('autoStatEnergyPrio').selectedIndex);
  GM_setValue('autoStatStaminaPrio', document.getElementById('autoStatStaminaPrio').selectedIndex);
  ...
  saveCheckBoxElementArray(['autoClick','autoLog','logPlayerUpdates','hideAttacks',
                            ...
                            'autoStatStaminaFallback','hourlyStatsOpt','autoGiftSkipOpt',
  ...
  if (saveCheckBoxElement('multipleJobs')) {
    ...
    for (var i = 0; i < document.getElementById('selectMission').options.length; i++) {
      if(document.getElementById('selectMission').options[i].selected == true) {
...
function isLoggable(line) {
  ...
  for (var i = 0; i < logPatternPass.length; i++) {
    if (logPatternPass[i] && line.indexOf(logPatternPass[i]) != -1) {
      return true;
    }
  }

  if (logPatternFail.length == 0) {
    return false;
  }      
   
  // Do not log if line contains any pattern from list
  for (var i = 0; i < logPatternFail.length; i++) {
    if (logPatternFail[i] && line.indexOf(logPatternFail[i]) != -1) {
      return false;
    }
  }   
...
function createMiscTab() {
  ...
  id = 'autoStatAttackPrio';
  var autoStatAttackPrio = makeElement('select', miscTab, {'id':id, 'title':title,'style':' top: ' + xTop + 'px; left: ' + (yLeft + 10) + 'px;'});  
  
  id = 'autoStatDefensePrio';
  var autoStatDefensePrio = makeElement('select', miscTab, {'id':id, 'title':title,'style':' top: ' + (xTop + 25) + 'px; left: ' + (yLeft + 10) + 'px;'});  
  
  id = 'autoStatHealthPrio';
  var autoStatHealthPrio = makeElement('select', miscTab, {'id':id, 'title':title,'style':' top: ' + (xTop + 50) + 'px; left: ' + (yLeft + 10) + 'px;'});  
  
  id = 'autoStatEnergyPrio';
  var autoStatEnergyPrio = makeElement('select', miscTab, {'id':id, 'title':title,'style':' top: ' + (xTop + 75) + 'px; left: ' + (yLeft + 10) + 'px;'});  
  
  id = 'autoStatStaminaPrio';
  var autoStatStaminaPrio = makeElement('select', miscTab, {'id':id, 'title':title,'style':' top: ' + (xTop + 100) + 'px; left: ' + (yLeft + 10) + 'px;'});  
  
  for (var i = 0; i < 5; i++) {
    var choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode('Priority ' + (i + 1)));
    autoStatAttackPrio.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode('Priority ' + (i + 1)));
    autoStatDefensePrio.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode('Priority ' + (i + 1)));
    autoStatHealthPrio.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode('Priority ' + (i + 1)));
    autoStatEnergyPrio.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode('Priority ' + (i + 1)));
    autoStatStaminaPrio.appendChild(choice);
  }  
  autoStatAttackPrio.selectedIndex = GM_getValue('autoStatAttackPrio', 0);
  autoStatDefensePrio.selectedIndex = GM_getValue('autoStatDefensePrio', 0);
  autoStatHealthPrio.selectedIndex = GM_getValue('autoStatHealthPrio', 0);
  autoStatEnergyPrio.selectedIndex = GM_getValue('autoStatEnergyPrio', 0);
  autoStatStaminaPrio.selectedIndex = GM_getValue('autoStatStaminaPrio', 0);    
    
  // Status Allocation Mode Settings
  title = 'Please select where to base Attack status';
  id = 'autoStatAttackMode';
  var autoStatAttackMode = makeElement('select', miscTab, {'id':id, 'title':title, 'style':'width:180px; top: ' + xTop + 'px; left: ' + (yLeft + 150) + 'px;'});  
  
  title = 'Please select where to base Defense stat.';
  id = 'autoStatDefenseMode';
  var autoStatDefenseMode = makeElement('select', miscTab, {'id':id, 'title':title, 'style':'width:180px; top: ' + (xTop + 25)+ 'px; left: ' + (yLeft + 150) + 'px;'});   
  
  title = 'Please select where to base Health stat.';
  id = 'autoStatHealthMode';
  var autoStatHealthMode = makeElement('select', miscTab, {'id':id, 'title':title, 'style':'width:180px; top: ' + (xTop + 50)+ 'px; left: ' + (yLeft + 150) + 'px;'});  
  
  title = 'Please select where to base Energy stat.';
  id = 'autoStatEnergyMode';
  var autoStatEnergyMode = makeElement('select', miscTab, {'id':id, 'title':title, 'style':'width:180px; top: ' + (xTop + 75)+ 'px; left: ' + (yLeft + 150) + 'px;'});    
  
  title = 'Please select where to base Stamina stat.';
  id = 'autoStatStaminaMode';
  var autoStatStaminaMode = makeElement('select', miscTab, {'id':id, 'title':title, 'style':'width:180px; top: ' + (xTop + 100)+ 'px; left: ' + (yLeft + 150) + 'px;'});    
  
  for (var i = 0; i < autoStatDescrips.length; i++) {
    var choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode("Attack " + autoStatDescrips[i]));
    autoStatAttackMode.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode("Defense " + autoStatDescrips[i]));
    autoStatDefenseMode.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode("Health " + autoStatDescrips[i]));
    autoStatHealthMode.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode("Energy " + autoStatDescrips[i]));
    autoStatEnergyMode.appendChild(choice);
    
    choice = document.createElement('option');
    choice.value = i;
    choice.appendChild(document.createTextNode("Stamina " + autoStatDescrips[i]));
    autoStatStaminaMode.appendChild(choice);
  }  
  autoStatAttackMode.selectedIndex = GM_getValue('autoStatAttackMode', 0);
  autoStatDefenseMode.selectedIndex = GM_getValue('autoStatDefenseMode', 0);
  autoStatHealthMode.selectedIndex = GM_getValue('autoStatHealthMode', 0);
  autoStatEnergyMode.selectedIndex = GM_getValue('autoStatEnergyMode', 0);
  autoStatStaminaMode.selectedIndex = GM_getValue('autoStatStaminaMode', 0);  
    
  // Status values
  var autoStatAttack = makeElement('div', miscTab, {'style':'top:' + xTop + 'px; left:' + (yLeft + 100) + 'px;'});
  makeElement('input', autoStatAttack, {'type':'text', 'style':'width: 40px;', 'value':GM_getValue('autoStatAttack', 0), 'id':'autoStatAttack', 'size':'1'});    
  
  var autoStatDefense = makeElement('div', miscTab, {'style':'top: ' + (xTop + 25)+ 'px; left:' + (yLeft + 100) + 'px;'});
  makeElement('input', autoStatDefense, {'type':'text', 'style':'width: 40px;', 'value':GM_getValue('autoStatDefense', 0), 'id':'autoStatDefense', 'size':'1'});        
  
  var autoStatHealth = makeElement('div', miscTab, {'style':'top: ' + (xTop + 50)+ 'px; left:' + (yLeft + 100) + 'px;'});
  makeElement('input', autoStatHealth, {'type':'text', 'style':'width: 40px;', 'value':GM_getValue('autoStatHealth', 0), 'id':'autoStatHealth', 'size':'1'});    
  
  var autoStatEnergy = makeElement('div', miscTab, {'style':'top: ' + (xTop + 75)+ 'px; left:' + (yLeft + 100) + 'px;'});
  makeElement('input', autoStatEnergy, {'type':'text', 'style':'width: 40px;', 'value':GM_getValue('autoStatEnergy', 0), 'id':'autoStatEnergy', 'size':'1'});    
  
  var autoStatStamina = makeElement('div', miscTab, {'style':'top: ' + (xTop + 100)+ 'px; left:' + (yLeft + 100) + 'px;'});
  makeElement('input', autoStatStamina, {'type':'text', 'style':'width: 40px;', 'value':GM_getValue('autoStatStamina', 0), 'id':'autoStatStamina', 'size':'1'});    
  
  // Left-over points  
  title = 'Check this to distribute points to Attack when goals are reached';
  id = 'autoStatAttackFallback';
  var divAttackFB = makeElement('div', miscTab, {'style':'top: ' + xTop + 'px; left:' + (yLeft + 340) + 'px; '});
  elt = makeElement('input', divAttackFB, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, 'autoStatAttackFallback');
  label = makeElement('label', divAttackFB, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Use Attack as fallback'));
  
  title = 'Check this to distribute points to Defense when goals are reached';
  id = 'autoStatDefenseFallback';
  var divDefenseFB = makeElement('div', miscTab, {'style':'top: ' + (xTop + 25) + 'px; left:' + (yLeft + 340) + 'px; '});
  elt = makeElement('input', divDefenseFB, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, 'autoStatDefenseFallback');
  label = makeElement('label', divDefenseFB, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Use Defense as fallback')); 
  
  title = 'Check this to distribute points to Health when goals are reached';
  id = 'autoStatHealthFallback';
  var divHealthFB = makeElement('div', miscTab, {'style':'top: ' + (xTop + 50) + 'px; left:' + (yLeft + 340) + 'px; '});
  makeElement('input', divHealthFB, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, 'autoStatHealthFallback');
  label = makeElement('label', divHealthFB, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Use Health as fallback'));
  
  title = 'Check this to distribute points to Energy when goals are reached';
  id = 'autoStatEnergyFallback';
  var divEnergyFB = makeElement('div', miscTab, {'style':'top: ' + (xTop + 75) + 'px; left:' + (yLeft + 340) + 'px; '});
  makeElement('input', divEnergyFB, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, 'autoStatEnergyFallback');
  label = makeElement('label', divEnergyFB, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Use Energy as fallback'));
  
  title = 'Check this to distribute points to Stamina when goals are reached';
  id = 'autoStatStaminaFallback';
  var divStaminaFB = makeElement('div', miscTab, {'style':'top: ' + (xTop + 100) + 'px; left:' + (yLeft + 340) + 'px; '});
  makeElement('input', divStaminaFB, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, 'autoStatStaminaFallback');
  label = makeElement('label', divStaminaFB, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' Use Stamina as fallback')); 
   
  title = 'Check this to enable log-filtering';
  id = 'filterLog';
  var filterLog = makeElement('div', miscTab, {'style':'text-align: right; top: ' + (xTop + 130) + 'px; right: 430px;'});
  ...
  var filterPass = makeElement('div', miscTab, {'style':'text-align: left; top: ' + (xTop + 155) + 'px; left: 20px;'});
  var filterFail = makeElement('div', miscTab, {'style':'text-align: left; top: ' + (xTop + 260) + 'px; left: 20px;'});
  ...
  makeElement('textarea', filterPass, {'style':'position: static; width: 15em; height: 7em;', 'id':'logFilterPass', 'title':'Enter each pattern on a separate line.'}).appendChild(document.createTextNode(GM_getValue('logFilterPass', defaultLogPass.join('\n'))));
  ...
  makeElement('textarea', filterFail, {'style':'position: static; width: 15em; height: 7em;', 'id':'logFilterFail', 'title':'Enter each pattern on a separate line.'}).appendChild(document.createTextNode(GM_getValue('logFilterFail', defaultLogFail.join('\n'))));
  makeElement('br', filterFail);
...
function refreshSettings() {
  ...
  staminaCeiling = (maxStamina * (1 - parseInt(GM_getValue('selectStaminaUse', 0)) * .10)).toFixed(0);  
...
function customizeMasthead() {
  ...
  var mwapTitle = 'Mafia Wars Autoplayer ' + SCRIPT.version + ' (Build ' + SCRIPT.build + ')';
  makeElement('div', mastheadElt, {'style':'position: absolute; top: 24px; left: 500px; right: 17px; text-align: left; font-size: 11px; font-weight: bold; color: #FFFFFF'}).appendChild(document.createTextNode(mwapTitle));
  var menuElt = makeElement('div', mastheadElt, {'id':'ap_menu', 'style':'position: absolute; top: 38px; left: 500px; right: 17px; text-align: left;'});
...
function customizeNames() {
  ...
  for (var i = 0; i < elts.length; i++) {
    if (!elts[i].innerHTML.replace(/\xAD/g, '').trim()) {
...
function getPlayerStats() {
  ...
  if (isNaN (curDefense) || isNaN(curAttack)) {
    ...
    for (var i=0; i < pJavaScripts.snapshotLength; i++ ) {    
...
function customizeProfile() {
  ...
  if (statsTable) {
    var statsDiv = statsTable.parentNode.parentNode.parentNode.previousElementSibling; //Yeah, I know it's sort of hackish, but seems the cleanest way to get the div.
  ...
  if (GM_getValue('multipleJobs') == 'checked') {
    ...
    for (var i in cycle_jobs) {
      if(cycle_jobs[i].length > 1) {
        ...
        for (var n = 0; n < cycle_jobs[i].length; n++) {
  ...
  if (currentJobMastered > 0) {
    ...
    if (!jobList.length) {
      ...
      if (tierPercent == 100) {
        ...
        //       then by tier.
        var nextTierJob;
        for (var i = selectMission + 1; i < missions.length; i++) {
...
function parsePlayerUpdates(messagebox) {
  ...
  if (messageTextNoTags.indexOf('attacked by') != -1) {
    ...
    result += ' by ' + user;
    if (messageTextNoTags.match(/you won.*you gained .*?(\d+) experience points?.*?(C?\$[\d,]*\d)/i)) {
      // The fight was won.
      var cost = RegExp.$2;
...
function onRobTab() {
  ...
  if (xpathFirst('.//li[contains(@class, "tab_on")]//a[contains(@onclick, "controller=robbing")]', innerPageElt)) {
    return true;
  }
...
function goLocation(toCity) {
  ...
  if (!elt) {
    elt = xpathFirst('//div[@id="' + SCRIPT.appID + '_travel_menu"]//a[contains(@onclick, "destination=' + (toCity + 1) + '")]');
  }
  if (elt) {
    clickElement(elt);
  ...
  switch (action) {
    ...
    case 'rob':
      ...
      if (innerNoTags.indexOf('You successfully robbed') != -1) {
        ...
        innerNoTags.match(/taking\s.*?(\d+\s+damage).*dealing\s.*?([\d.]+%\s+damage).*?(C?\$[\d,]*\d).*?(\d+\s+experience)/i);
