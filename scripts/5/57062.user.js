...
function helpFriends() {
	...
	ifrm.setAttribute("src",' http://apps.facebook.com/inthemafia/track.php?next_controller=job&next_action=give_help&zy_track=newsfeed&from='+MWmafia[tmp]   +'&skip_interstitial=1');
...
function helpFriends() {
	var MWmafia = getList('MWmafia');
	var ifrm = document.getElementById("addFrame") 
	ifrm.setAttribute("src",'http://apps.facebook.com/inthemafia/track.php?zy_track=newsfeed&sendkey=de7ec089abc031e82f1be27b24d62df6&ztrack_category=level_up_brag&ztrack_subcategory=109_2&next_controller=index&next_action=levelup_boost_claim&next_params=%7B%22friend_id='+MWmafia[tmp]+'&skip_interstitial%22%3A%221%22%7D&ref=nf');
	top.document.title = 'Friend:'+tmp;
	tmp++;
	if (tmp<MWmafia.length) window.setTimeout(helpFriends,1000); else 
	{
