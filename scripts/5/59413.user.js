// ==UserScript==
// @name OGame Redesign : Perfect Plunder Neater Edition
// @namespace http://userscripts.org/scripts/show/59413
// @description OGame : I edited the Perfect Plunder script slightly so it automatically collapses all those reports that show planets without enough resources, while you define what's enough by editing the "large_cargo_limit" value that can be found on the line 13.
// @date 2009-10-11
// @creator Black Cat (mkey edit)
// @include http://*.ogame.*/game/index.php?page=showmessage*
// @include http://*.ogame.*/game/index.php?page=messages*
// @exclude
// ==/UserScript==

// !!!!!!!!!!!!!! Change this one to appropriate value !!!!!!!!!!!!!!
const large_cargo_limit = 5;
	...
	function showPlunder () {
		...
		for (var i=0; i<tables.length; i++) {
			if (tables[i].className == "material spy") {
				...
				if (large_cargos<large_cargo_limit){
					tables[i].parentNode.parentNode.parentNode.setAttribute("style", "display: none;");
				}
