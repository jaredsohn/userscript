...
\*-------------*/
    ...
    //if ((Destyling_google_cache == 'yes') && (location.href.search(/q=cache.*?(rapidshare|megaupload|megarotic|filefactory)/) != -1)) {
        ...
        for (var i = allmu.length - 1; i >= 0; i--) { // can check 10000 Bites of links so all.join('\n').length <= 10000
            ...
            window.setTimeout(function() {
                GM_xmlhttpRequest({
                    ...
                    onload: function(result) {
                        res = result.responseText;
                        res = res.replace(/\d=www.megaupload.com&\d=www.megarotic.com/, '');
...
function check_configuration(){
  ...
  if (!(GM_getValue("script_configurated"))){
		GM_setValue("script_configurated",true);
		GM_setValue("Send_the_urls_of_the_pages_you_are_checking_in_the_open_database", false);
		GM_setValue("Show_black_backround_in_dead_links", false);
		GM_setValue("Show_line_through_in_dead_links", false);
		GM_setValue("Remove_html_from_rapidshare_urls", false);
		GM_setValue("No_live_links", false);
		GM_setValue("Display_Page_Stats", false);
		GM_setValue("Destyling_google_cache", true);

  }else{
		var Send_the_urls_of_the_pages_you_are_checking_in_the_open_database = GM_getValue("Send_the_urls_of_the_pages_you_are_checking_in_the_open_database");
		var Show_black_backround_in_dead_links = GM_getValue("Show_black_backround_in_dead_links");
		var Show_line_through_in_dead_links = GM_getValue("Show_line_through_in_dead_links");
		var Remove_html_from_rapidshare_urls = GM_getValue("Remove_html_from_rapidshare_urls");
		var No_live_links = GM_getValue("No_live_links");
		var Display_Page_Stats = GM_getValue("Display_Page_Stats");
		var Destyling_google_cache = GM_getValue("Destyling_google_cache");		
  }

  check(Send_the_urls_of_the_pages_you_are_checking_in_the_open_database,Show_black_backround_in_dead_links,Show_line_through_in_dead_links,Remove_html_from_rapidshare_urls,No_live_links,Display_Page_Stats,Destyling_google_cache);	
}
...
GM_registerMenuCommand("[Rapidshare Links Checker] Check The Links In This Page", Check_The_Links_In_This_Page, "w", "shift alt", "q");
...
var  configurationinnerHTML = <><![CDATA[
  ...
  <div class="popup_block">
	...
	<h3 id='h3hideshowtitle'>Rapidshare Links Checker Userscript Configuration</h3>
 ...
 informations are sent to the database, and you are able to erase the data you send.</p>
	...
	function changeConfiguration(e){
		if(GM_getValue(e.target.id)){
			GM_setValue(e.target.id, false);
			document.getElementById(e.target.id).firstChild.checked = GM_getValue(e.target.id);
			var result= 'disabled';
			}else{
	...
	for (var i = existingobject.length - 1; i >= 0; i--) {
		if (GM_getValue(existingobject[i].id)){
	...
	GM_registerMenuCommand("[Rapidshare Links Checker] Configuration", configuration, null, null, "R");
	...
	GM_registerMenuCommand("[Rapidshare Links Checker] Make The Links To Full Urls", makeTheCheckedLinksToFullURl, "w", "control alt", "w");
