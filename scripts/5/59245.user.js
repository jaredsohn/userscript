// ==UserScript==
// @name           Pfandrechner fuer pennergame 4.0 by basti1012
...
var kurs11 = kurs1.innerHTML.split('value="')[1];
...
text = new Array('Restflaschen','Gew&uuml;nschter Betrag','Gesamtbetrag');
...
kurs_s = data.kurs;
newtr[4].innerHTML = '<td><img src="'+pic[3]+'" /></td><td>Kurs(cent)</td><td><input type="text" size="5" id="field4" value="'+kurs_s+'"/></td>';
...
function calc() {
	...
	if(this.id=="menge_verkauf") {
		...
		if(field[3].value>1000000) {
			alert('Sie sind im Begriff, mehr als 1.000.000EUR durch den Verkauf von Pfandflaschen zu erreichen.');
		}
}
}});