...
// @description Replaces the images generated by LaTeX2HTML with MathML code
// @include     *
// ==/UserScript==

// example page: http://www.dlsi.ua.es/~mlf/nnafmc/pbook/node22.html

(
	function()
	{
		if(document.documentElement.innerHTML.indexOf("Converted with LaTeX2HTML")==-1)
			return;
...
/* here starts my code :) */
		...
		window.addEventListener ( "load", function () {
			var xpath = "//img[starts-with(@src,'img')]";
			var res = document.evaluate(xpath, document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null); 
			
			var i;
			var imgNode;
			for(i=0;imgNode = res.snapshotItem(i);i++)
			{
				var latex = imgNode.alt;
				
				latex = latex.replace(/\\begin\{displaymath\}/, "$");
				latex = latex.replace(/\\end\{displaymath\}/, "$");

				var latexNode = document.createTextNode(latex);
				imgNode.parentNode.insertBefore(latexNode, imgNode);
