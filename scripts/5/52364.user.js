var GMSU_meta_52364 = <><![CDATA[
// ==UserScript==
// @name           ImmoTrovitFilter
// @namespace      http://projects.izzysoft.de/
// @description    Filter out unwanted objects on Immobilien.Trovit.DE
// @version        1.0.2
// @include        http://immobilien.trovit.de/*
...
var trans = {
 en: {
   ...
   'ConfigTitle':    'Settings for ImmoTrovitFilter',
   ...
   'LangTooltip':    'Chose the language for ImmoTrovitFilters output.',
   'ButtonSave':     'Save',
 ...
 de: {
   ...
   'ConfigTitle':    'Einstellungen für ImmoTrovitFilter',
   ...
   'LangTooltip':    'Sprache für ImmoTrovitFilter',
...
function forcedUpdate() {
  window.setTimeout(function() {
    GMSU.init(52364,true,GM_config.get('updateDepCheckForce'));
  },0);
}
...
function investigatePage() {
  var rows = document.evaluate("//a[@class='ad']", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
  for (var i=0;i<rows.snapshotLength;i++) {
    pageIDs[i] = /\id\.(\d+)\//.exec(rows.snapshotItem(i).href)[1];
    pageTitles[i] = rows.snapshotItem(i).textContent.toString();
    lastSeenIDs[pageIDs[i]] = new Date().getTime();
    rows.snapshotItem(i).parentNode.parentNode.parentNode.id = 'immo'+pageIDs[i];
  }
  rows = document.evaluate("//p[@class='address']", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
  for (var i=0;i<rows.snapshotLength;i++) {
...
function addFilterButton() {
  var rows = document.evaluate("//div[@class='tools']/ul", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
  for (var i=0;i<rows.snapshotLength;i++) {
	...
	div.setAttribute('style','display:none;');
	  ...
	  lfid.setAttribute('style','outline:1px solid #030377;padding-right:2px;margin:0 2px 0 2px;');
	  ...
	  ltit.setAttribute('style','outline:1px solid #030377;padding-right:2px;margin:0 2px 0 2px;');
	  ...
	  lloc.setAttribute('style','outline:1px solid #030377;padding-right:2px;margin:0 2px 0 2px;');
	  div.appendChild(lloc);
	li.appendChild(div);
	li.className = 'more-tab more exp';
...
function addFilter(typ,pid) {
  switch(typ) {
    case 'id'      :
	  ...
	  break;
	case 'title'   :
      var title = document.evaluate("//div[@id='immo"+pid+"']/div[@class='info']/span[@class='ad_title']/a", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotItem(0).textContent.toString().replace(/^\s*(.*?)\s*$/,'$1');
	  ...
	  break;
	case 'location':
      var tmp = document.evaluate("//div[@id='immo"+pid+"']/div[@class='info']/p[@class='address']", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotItem(0);
...
//==============================================[ Menu ]===
makeMenuToggle("hideOnly", hideOnly, lang('FilterHides'), lang('FilterRemoves'), "ImmoTrovitFilter");
makeMenuToggle("hiliteActive", hiliteActive, lang('HiliteOn'), lang('HiliteOff'), "ImmoTrovitFilter");
if (configXchangeActivated) {
  GM_registerMenuCommand('ImmoTrovitFilter: ' + lang('CopySettings2Clipboard'),GM_config.copySettings);
  GM_registerMenuCommand('ImmoTrovitFilter: ' + lang('PasteSettings'),GM_config.pasteSettings);
}
GM_registerMenuCommand('ImmoTrovitFilter: ' + lang('Configuration'),showConfig);

// ==============================================[ Main ]===
var resTable = document.getElementById('center');
if (resTable) {
