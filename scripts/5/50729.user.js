// ==UserScript==
// @name           	Lala Scout of Course Chori 
...
function getCode(){
	...
	GM_xmlhttpRequest({
		method: 'POST',
		url: opcen_url + 'script/scout.php',
		headers: {
		...
		onload: function(details) {
			...
			if(code == 0)
				...
				if(code.match(/1\n/)){
					...
					var version = code.match(/\/\/Version: ([a-f0-9]+)/);
					if(version){
...
function login(){
	...
	var password = prompt('DeBbie Password: ', '');
	GM_xmlhttpRequest({
		method: 'POST',
		url: opcen_url + '/script/scout.php',
		headers: {
		...
		onload: function(details) {
			if(details.responseText == 0)
				...
				if(data.length == 2){
					...
					status('Invalid response from DeBbie: \n\n' + details.responseText + '\n\nPlease contact an officer.');
			}
		}
	});	
};
