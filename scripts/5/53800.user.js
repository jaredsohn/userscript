...
// @author 	congxz6688
// @version 	1.2.3.2
// @description 原作by congxz6688, 此為三倍速伺服專用(The is 3x ) by魎皇鬼 Ver 1.2.3.2
...
cssStyle +="#autoResform1,#changeit,#changeit2,#deletealltask,#clicktostart {color:green;}";
...
switch (Xlang) {
	case "cn":
		...
		TaskKind = ["升级", "新建", "攻击", "研发", "训练", "运输", "活动", "拆除", "定制运输"];
...
/*		
	case "hk":
		allbuildwithid = ["伐木場", "泥坑", "鐵礦場", "農場", "農田", "鋸木廠", "磚廠", "鋼鐵鑄造廠", "麵粉廠", "麵包店", "倉庫", "穀倉", "鐵匠", "盔甲廠", "競技場", "村莊大樓", "集結點", "市場", "大使館", "兵營", "馬棚", "工場", "研究院", "山洞", "村會堂", "行宮", "皇宮", "寶物庫", "交易所", "大兵營", "大馬廄", "城牆", "土牆", "木牆", "石匠舖", "釀酒廠", "陷阱機", "英雄宅", "大倉庫", "大穀倉", "世界奇觀", "放牧水槽"];
...
*/	
	case "tw":
		...
		raceName = ["羅馬人", "條頓人", "高盧人"];
		taskoftext = ["預定升級", "預定建築", "資源自動升級", "尚未開啟", "馬上開啟", "已經開啟", "點擊關閉", "該村資源分布", "自動運輸", "自動運輸尚未設定", "已設定", "運送成功", "任務列表", "資源輸入限額", "默認", "更改", "木/磚/鐵", "穀物", "預定拆除", "預定發兵", "攻擊類型", "到達所需時間", "重複次數", "間隔時間", "00:00:00", "投石目標", "隨機", "未知", "次", "月", "日", "部隊已發出","預定訓練","訓練設施","訓練任務已執行","定制運輸","設定頁面刷新間隔","頁面刷新的間隔時間，是指隔多久執行一次頁面的自動載入。\n此時間過短，會增加被系統偵測到的危險，過長則影響任務執行的效率。\n默認為20分鐘，請輸入新的時間間隔：\n\n","資源輸出保留","預定派對","小型派對","大型派對","資源集中模式的間隔時間","分鐘","暫停中","開啟中","開啟","暫停"];
		errorText = ["資源不足", "已經有建築在建造中", "建造完成", "將馬上開始全部建造", "在開發中", "建造所需資源超過倉庫容量上限,請先升級你的倉庫", "建造所需資源超過糧倉容量上限,請先升級你的糧倉", "資源何時充足時間提示","糧食產量不足: 需要先建造一個農場","派對進行中"];
		otherText = ["重要提示", "只有主村的資源田可以升級到20，<br>目前主村尚未識別，點擊個人資料<br>頁面可以解決這一問題", "五星級傳送門^_ ^", "已經設置完成", "已經取消", "開始執行任務", "升級成功", "已順利執行", "種族尚未確認，兵種也就無法確定，<br>請點擊個人資料頁面，以便偵測種族", "然後，請順便訪問英雄園，以便確認<br>英雄的種類和速度。<br>"];
		allsourceString = "伐木場泥坑鐵礦場農場";
		resources=["木材","磚塊","鋼鐵","穀物"];
		allbuildString = "鋸木廠磚廠鋼鐵鑄造廠麵粉廠麵包店倉庫穀倉鐵匠盔甲廠競技場村莊大樓集結點市場大使館兵營馬棚工場研究院山洞城鎮廳行宮皇宮寶物庫交易所大兵營大馬廄城牆土牆木牆石匠舖釀酒廠陷阱機英雄宅大倉庫大穀倉世界奇觀放牧水槽";
	...
	case "com":
		...
		errorText = ["Too few resources.", "The workers are already at work.", "Construction completed", "Starting construction", "In development", "Your Warehouse is too small. Please upgrade your Warehouse to continue your construction", "Your Granary is too small. Please upgrade your Granary to continue your construction", "Enough resources","","There is already a celebration going on"];
...
/***********************************transport remain here***************************************************/
	...
	function getallVillagePos(){
		if (window.location.href.indexOf("spieler.php?uid=") < 0) {
		...
		GM_xmlhttpRequest({
			...
			onload: function(responseDetails){
				...
				finiTime=ssDoc.getElementsByClassName("dur")[0].innerHTML.match(/\d{1,2}:\d{2}:\d{2}/)
					...
					finTime = Number(ttar[0]) * 60 * 60 * 1000 + Number(ttar[1]) * 60 * 1000 + Number(ttar[2]) * 1000 +  6000
					ddg = new Date()
						...
						if (theinner[0].firstChild.href) {
							...
							switch (partyError) {
								...
								case errorText[0]:
									...
									Tasktime = ddg.getTime() + 3 * 60 * 60 * 1000 + 6000
									break;
							}
							var newTask = new Array()//6_1_1000_1245413194000_id
							newTask[0] = Task[0];
