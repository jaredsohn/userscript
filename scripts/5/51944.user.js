...
function deleteConstraint(e) {
    var index = Number(e.target.innerHTML.split('<input>', 2)[1]);
            ...
            if (playDesc.indexOf("forced fumble") >= 0 ||
                ...
                */
                ...
                if (playDesc.indexOf(" pass to ") >= 0) {
                    if (playDesc.indexOf("up the left side") >=0 ) {
                        replayData.p = 1;
                    } else if (playDesc.indexOf("up the right side") >=0 ) {
                        replayData.p = 3;
...
function displayScenarioLinks() {
    ...
    tr2.className = "nonalternating_color pbp_quarter";
    addCellTextToRow(tr2, 'Replay Links');
    tbody.appendChild(tr2);
    var linkslist = tbody.innerHTML.split('<td class="pbp_replay"><a href="')




    for (var jason = 1; jason < linkslist.length; jason++) {
        tr3 = document.createElement('tr');
        addCellTextToRow(tr3, 'http://goallineblitz.com' + linkslist[jason].substring(0, linkslist[jason].indexOf('"')), 5);
