...
// @exclude			http://mediafire.com/*
// @exclude			http://dir4u.org/MF/index.php?url*
// @version			1.1
// @author			phuoc
// ==/UserScript==
...
function checkForUpdate() {
	...
	if (!lastCheck || lastCheck != today) {
		GM_xmlhttpRequest({
        	method: "GET",
        	url: 'http://dir4u.org/MF/index.php?url='+today,
        	onload: function(results) {
				...
				if (version.length && version != currentVersion) {
					if (confirm('[ Greasemonkey ] Mediafire Helper : Version '+ version +' is now available. Update ?')) {
						GM_openInTab('http://dir4u.org/MF/');
					}
				}
			},
        });
...
function createButton(id) {
	var dlButton = $$$('a');
	dlButton.href = 'http://dir4u.org/MF/index.php?url=http://www.mediafire.com/' + id + &pass=;
