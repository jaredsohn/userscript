...
// @version        5.5.7
// @author         off99555
// @email          off9955555@hotmail.com
// @namespace      Travian
// @description    Battle Analyse 5.5.7 (add Thai language) เพิ่มภาษาไทย - ผู้จัดทำ off99555
...
var ScriptAuthor='off99555';
var ScriptVersion='5.5.7';
var ScriptLink='http://userscripts.org/scripts/show/57673';
var ScriptInstall='http://userscripts.org/scripts/source/57673.user.js';
var AuthorLink='http://www.google.com/profiles/off99555';
...
switch(lang){
	case ".ir":
		langfile=["قدرت حمله کل","مقدار خسارت وارده","قدرت دفاع در مقابل پیاده نظام",
			"قدرت دفاع در مقابل سواره نظام","آمار کلی","میزان سود / زیان","میزان مصرف گندم ارتش",
			"تجربه قهرمان","گنجایش حمل","میزان گندم از دست رفته","میزان مصرف گندم ارتش",
			"قهرمان در حمله حضور نداشت","قهرمان","میزان منابع غارت شده","بیشترین میزان قابل حمل توسط نیروهای باقی مانده",
			"حمله کننده","دفاع کننده","سود","بازده حمله"];
		break;
	case ".co.il":
	...
	case ".ru":
		...
		break;
	case ".asia":
		langfile=["รวมค่าการโจมตีทั้งหมด","ราคาทหารที่ตาย","ป้องกันกองทหารราบ",
							"ป้องกันกองทหารม้า","สถิติทั่วไป","ทรัพยากร","การบริโภค",
							"ค่าประสบการณ์ของฮีโร่","ทรัพยากรที่ได้รับ","จำนวนทรัพยากร","จำนวนข้าวที่ทหารบริโภค",
							"ไม่มีฮีโร่","ฮีโร่","ทรัพยากรที่เราขโมยได้","ทรัพยากรสูงสุดที่สามารถขโมยได้",
							"ผู้โจมตี","ผู้ป้องกัน","ผลกำไร","ทหารปล้นได้"];
		break;
	default:
...
function showHelp(ev) {
	...
	if (tm != null) {
		var str =   "<div><img src=" + imgatt_all + "><span>*" + tm[unit][0] + "</span></div>"
		str = str + "<div><img src=" + imgdef_i + "><span>*" + tm[unit][1] + "</span></div>"
		str = str + "<div><img src=" + imgdef_c + "><span>*" + tm[unit][2] + "</span></div>"
		str = str + "<div><img src=" + imgu14 + "><span>*" + tm[unit][8] + "</span></div>"
...
function main(){
	...
	if(GM_getValue('tba_tooltips', '1') == '1') {
	...
	if (window.location.href.match(/karte.php\?/)){
		nn=document.getElementById('troop_info');
		if(nn==null)return;
	...
	// End Warsim
		...
		var tabreport = document.getElementById('report_surround');
		if(!tabreport) return;
//		alert('z');
		
/*		if(tab.snapshotLength<3)tab=find("//table[contains(@class,'std')]",XPList);
		if(tab.snapshotItem(1)==undefined)return;
		if(tab.snapshotItem(1).id=='MeXaon_ver_table')return;
		if(tab.snapshotItem(1).getElementsByTagName("td").length < 24)return;*/
...
//	alert(tab.snapshotItem(1-warsim).tagName);

	if(warsim==1){
		table=find("//table[@class='results attacker']", XPFirst).getElementsByTagName("td");
	} else {
		table=document.getElementById('attacker').getElementsByTagName("td");
	}
	attacktable();
...
//			GM_log("table[tda+i].textContent="+table[tda+i].textContent);
//			GM_log("table[tdl+i].textContent="+table[tdl+i].textContent);
...
//			alert(res);
			atstemp[2]=parseInt(res[1])+parseInt(res[2])+parseInt(res[3])+parseInt(res[4]);
		...
		attHTML += '<img title="'+langfile[0]+'" src="'+imgatt_all+'" align="left" height="15" width="15" style="padding-left: 3px">&nbsp;<font class="f8"><i>'+atstemp[0]+'</i></font>';
		if (warsim != 1) {
			table[2].innerHTML=attHTML;
		} else {
			table[0].innerHTML=attHTML;
		}	
		rowi=document.createElement("tr");
		cell1=document.createElement("td");
		cell1.innerHTML=langfile[1];
		cell2=document.createElement("td");
		cell2.setAttribute("align","left");
		cell2.setAttribute("colspan",10+statushero);
		cell2.style.textAlign = "left";
		cell2.innerHTML='<font class="f8"><i><img class="r1" src="img/x.gif">'+lostres[0]+'&nbsp;<img class="r2" src="img/x.gif">'+lostres[1]+'&nbsp;<img class="r3" src="img/x.gif">'+lostres[2]+'&nbsp;<img class="r4" src="img/x.gif">'+lostres[3]+'&nbsp; <b>(-'+atstemp[1]+')</b></i></font>';
		...
		document.getElementById("tba_info").appendChild(rowProfit);
		document.getElementById("tba_info").appendChild(rowEfficiency);
...
//	alert(troops);
	...
	table[0].innerHTML='<img title="'+langfile[2]+'" src="'+imgdef_i+'" align="absmiddle" height="15" width="15">&nbsp;<font class="f8"><i>'+dtstemp[0]+'</i></font><br><img title="'+langfile[3]+'" src="'+imgdef_c+'" align="absmiddle" height="15" width="15">&nbsp;<font class="f8"><i>'+dtstemp[1]+'</i></font>';
	rowi=document.createElement("tr");
	cell1=document.createElement("td");
	cell1.innerHTML=langfile[1];
	cell2=document.createElement("td");
	...
	cell2.setAttribute("colspan",10+statushero);
	cell2.style.textAlign = "left";
	cell2.innerHTML='<font class="f8"><i><img class="r1" src="img/x.gif">'+lostres[0]+'&nbsp;<img class="r2" src="img/x.gif">'+lostres[1]+'&nbsp;<img class="r3" src="img/x.gif">'+lostres[2]+'&nbsp;<img class="r4" src="img/x.gif">'+lostres[3]+'&nbsp; <b>(-'+dtstemp[2]+')</b></i></font>';
	rowi.appendChild(cell1);
	rowi.appendChild(cell2);
	tbody=document.createElement("tbody");
	tbody.className="goods";
...
function generatereport(){
	...
	cell2.innerHTML=lostA+'*<img src="'+imgr4+'" title="'+langfile[9]+'">';
	cell3=document.createElement("td");
	cell3.setAttribute("align","right");
	cell3.innerHTML=ats[4]+'*<img src="'+imgr5+'" title="'+langfile[10]+'">';
	cell4=document.createElement("td");
	cell4.setAttribute("align","right");
	if(ats[5]==0){
			cell4.innerHTML='0*<img src="'+imgdel+'" title="'+langfile[11]+'">';
	}else cell4.innerHTML=dts[5]+'*<img src="'+imghero+'" title="'+langfile[12]+'">';
	cell5=document.createElement("td");
	...
	cell2=document.createElement("td");
	cell2.setAttribute("align","right");
	cell2.innerHTML=(-lostB)+'*<img src="'+imgr4+'" title="'+langfile[9]+'">';
	cell3=document.createElement("td");
	cell3.setAttribute("align","right");
	cell3.innerHTML=dts[3]+'*<img src="'+imgr5+'" title="'+langfile[10]+'">';
	cell4=document.createElement("td");
	cell4.setAttribute("align","right");
	if(dts[4]==0){
			cell4.innerHTML='0*<img src="'+imgdel+'" title="'+langfile[11]+'">';
	}else cell4.innerHTML=ats[6]+'*<img src="'+imghero+'" title="'+langfile[12]+'">';
	...
	if (warsim != 1 && worksave == 1) {
		...
		button1.setAttribute("value", "บันทึกรายงาน");
		...
		travilogForm.appendChild(elem("span", "", "ไม่ต้องระบุชื่อ"));
		...
		travilogIFrame.innerHTML = "กำลังโหลด";
		
		var divLoading = document.createElement('div');
		divLoading.id = 'tba_travilogIframeRowDiv'; 
		divLoading.innerHTML = "<b>--= กำลังโหลด =--</b>";
...
//		cell1.setAttribute('align', 'left');
	...
	if(warsim==0){
		...
		document.getElementById('content').appendChild(trep);
	}else{
		var dist = find("//form[@action='warsim.php']/p", XPFirst);
//		var dist = document.getElementById('');
		dist.parentNode.insertBefore(elem('p','',''), dist);
		dist.parentNode.insertBefore(trep, dist);
...
function genreporta2b(){
	...
	cell1.innerHTML='<span id="ats0">'+ats[0]+'</span>*<img src="'+imgatt_all+'" title="'+langfile[0]+'">';
	cell2=document.createElement("td");
	cell2.setAttribute("align","right");
	cell2.innerHTML='<span id="ats1">'+ats[1]+'</span>*<img src="'+imgdef_i+'" title="'+langfile[2]+'">';
	cell3=document.createElement("td");
	cell3.setAttribute("align","right");
	cell3.innerHTML='<span id="ats2">'+ats[2]+'</span>*<img src="'+imgdef_c+'" title="'+langfile[3]+'">';
	cell4=document.createElement("td");
	cell4.setAttribute("align","right");
	cell4.innerHTML='<span id="ats4">'+ats[4]+'</span>*<img src="'+imgr5+'" title="'+langfile[10]+'">';
	cell5=document.createElement("td");
	cell5.setAttribute("align","right");
//	cell5.setAttribute("rowspan","2");
...
function oazicinfo(){
	...
	var race=parseInt(tdl.snapshotItem(0).firstChild.className.match(/u(\d+)/).pop()/10)*10;
	for(var i=0;i<tdl.snapshotLength;i+=3){
		ut=parseInt(tdl.snapshotItem(i).firstChild.className.match(/u(\d+)/).pop())-race-1;
	...
	cell1=document.createElement("td");
	cell1.setAttribute("align","right");
	cell1.innerHTML=+m[0]+'*<img src="'+imgatt_all+'" title="'+langfile[0]+'">';
	row1.appendChild(cell1);
	
	cell1=document.createElement("td");
	cell1.setAttribute("align","right");
	cell1.innerHTML='<span id="ats1">'+m[1]+'</span>*<img src="'+imgdef_i+'" title="'+langfile[2]+'">';
	row2.appendChild(cell1);

	cell1=document.createElement("td");
	cell1.setAttribute("align","right");
	cell1.innerHTML='<span id="ats2">'+m[2]+'</span>*<img src="'+imgdef_c+'" title="'+langfile[3]+'">';
	row3.appendChild(cell1);

	cell1=document.createElement("td");
	cell1.setAttribute("align","right");
	cell1.innerHTML='<span id="ats4">'+m[3]+'</span>*<img src="'+imgr5+'" title="'+langfile[10]+'">';
	row4.appendChild(cell1);
	
	trep.setAttribute("style","float: left; width:180px; margin-left: 20px");
	trep.className="tbg";
	trep.appendChild(row1);
	trep.appendChild(row2);
	trep.appendChild(row3);	
	...
	document.getElementById('troop_info').parentNode.insertBefore(trep, document.getElementById('troop_info').nextSibling);
}

function kartec(ev){
	var str='';
	var tdl=find('//table[@id="troop_info"]/tbody/tr/td',XPList);
	var race=parseInt(tdl.snapshotItem(0).firstChild.className.match(/u(\d+)/).pop()/10);
	for(var i=0;i<tdl.snapshotLength;i+=3){
		ut=parseInt(tdl.snapshotItem(i).firstChild.className.match(/u(\d+)/).pop());
		uc=parseInt(tdl.snapshotItem(i+1).innerHTML.match(/(\d+)/).pop());
		str=str+ut+','+uc+',';
	}
...
GM_registerMenuCommand(ScriptName +' v' + ScriptVersion + " > Install latest version (ติดตั้งเวอร์ชั่นล่าสุด)", function(){ GM_openInTab(ScriptInstall)});
GM_registerMenuCommand(ScriptName +' > Script Page (หน้าสคริปต์)', function(){ GM_openInTab(ScriptLink)});
GM_registerMenuCommand(ScriptName +' > Contact Me (ติดต่อผม)', function(){ GM_openInTab(AuthorLink)});
