// ==UserScript==
// @author         rikuo
// @name           changed to the bird
// @namespace      http://d.hatena.ne.jp/rikuo/
// @description    All icons of twitter are changed to the bird.
	...
	if(users.snapshotLength){
		for(var i=0,ul = users.snapshotLength; i < ul; ++i){
			...
			if(enable){
				...
				img.src = 'http://s.twimg.com/a/1253141863/images/default_profile_'+num+'_normal.png';
			}else{
...
if(l.match(/^http:\/\/twitter.com\//i)){
	...
	query[1] = 'descendant::span[contains(concat(" ",@class," ")," author ")]/a/img[not(contains(@src,"http://s.twimg.com/a/1253141863/images/default_profile_"))]';
	query[0] = 'descendant::span[contains(concat(" ",@class," ")," author ")]/a/img[contains(@src,"http://s.twimg.com/a/1253141863/images/default_profile_")]';
	...
	query[1] = 'descendant::img[contains(concat(" ",@class," ")," user_icon ") or contains(concat(" ",@class," ")," fav_icon ") and not(contains(@src,"http://s.twimg.com/a/1253141863/images/default_profile_"))]';
	query[0] = 'descendant::img[contains(concat(" ",@class," ")," user_icon ") or contains(concat(" ",@class," ")," fav_icon ") and contains(@src,"http://s.twimg.com/a/1253141863/images/default_profile_")]';
	...
	if(window.AutoPagerize) {
		...
		window.addEventListener('GM_AutoPagerizeLoaded', changeBirdIcon ,false);
	}
...
function changeBirdIcon(){
	window.AutoPagerize.addFilter(function(docs){
		if(enable)docs.forEach(changeBird);
	});
}
