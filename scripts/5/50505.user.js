// ==UserScript==
// @name          TW(NL) PA Overzichten Sorteren
// @namespace     c1b1.de
...
if(url.indexOf('mode=tech') != -1)
  ...
  dom.addEvent(img,'click',function() {
    ...
    a.appendChild(dom.text('Continent'));
    div.appendChild(a);
    div.appendChild(dom.n('br'));
    ...
    a.appendChild(dom.text('Coördinaten'));
    div.appendChild(a);
    div.appendChild(dom.n('br'));
    ...
    dom.addEvent(a,'click',function()
      ...
      sortE(elist,0,false,'search:'+prompt('Geef de te filteren tekst op',''));
     });
...
function sortE(elist,tdN,direction,special)
  ...
  this.f = function(a,b)
    ...
    if(this.n == 0 && this.special == 'ccords')
      ...
      {
      var a = parseInt(dom.trim(dom.trim(grabText(a.getElementsByTagName('td')[0],4)).split('C').pop()));
      var b = parseInt(dom.trim(dom.trim(grabText(b.getElementsByTagName('td')[0],4)).split('C').pop()));
      }
    else if(this.n == 1)
      {
