// ==UserScript==
// @name           Shartak Inventory Counter - Revised
// @namespace      http://userscripts.org/scripts/show/
// @description    Counts used inventory spaces and displays remaining free spaces.
...
var total=70, used=0, quantity=0, output='', inv=document.getElementsByTagName("li"), invLen=inv.length;
if (invLen>0) { // If the user has any items (i.e., isn't dead or resting from a resurrection)
	for (i=0; i<invLen; i++) { // For each <li></li> element
		if (inv[i].innerHTML.search(/value="backpack/)>0) { // If a backpack is in inventory
			total=100; //Change the inventory space
		} 
