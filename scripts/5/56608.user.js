// ==UserScript==
// @author		Original Author: usr8472@gmail.com, ported to T3.5 by ftbhrygvn@yahoo.com.hk
// @name		Travian: Resource++ v4 for T3.5
...
function onLoad(){ // runs the differnt functions and procedures, 
	...
	switch(lang){ //load only the lang you need into memory
	   ...
	   break;
	    ...
	    break; 
		case '.hk':
	    langfile[lang]=['升級需要:','泛濫', '用盡','資源計','資源棒', 
		'時','足夠資源 今天 在','足夠資源 明天 在', '足夠資源 ', 
		'天後 在', ' 最大等候時間後: ' , '存量: ','等候時間: ','不足','剩餘', ' 最大等候時間: ','平衡', 
		'用盡: ','升級 ',' 資源生產','自然平衡' , '倉庫需要升級' , '穀倉需要升級' ];
	    break; 
	default:
		langfile[lang]=['Needed for upgrading:','Overflow', 'Depleted','Resource Meters','Resource Bars', 
		" o'clock",'Enough resources today at ','Enough resources tomorrow at ', 'Enough resources in ', 
	...
	if(side){
		...
		side.appendChild(document.createElement('p'));
		addElementArray(side.lastChild,opt);
		pop();
	} else {
		return;
	}
...
/************************ Drag n drop*******************************/
/*** from JoeSimmons of http://userscripts.org/ **/
function dragStart(e){
	dragObj.elNode = e.target.parentNode.parentNode.parentNode;
	if (e.target.nodeType == 3) dragObj.elNode = dragObj.elNode.parentNode;
...
function dragStop(e) {
	...
	setOption(dragObj.elNode.id, dragObj.elNode.style.top +'_'+ dragObj.elNode.style.left);
}
...
function quickLinks(vis){
	...
	var con=document.createElement('div');
		...
		con.style.height='15px';
		con.style.cssFloat='left';
		con.appendChild( document.createTextNode(  'Navigator' ) );
	...
	var onSelect=document.createElement('select');
		...
		onSelect.appendChild(makeOption('Armies','#'));
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change',function(e){jump(e);},true);
		onSelect.appendChild(makeOption('Marketplace','#'));
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Reports','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Messages','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Statistics','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Alliance','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Resources','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Military','#' )      );
		...
		onSelect.style.width=w;
		onSelect.addEventListener('change', function(e) {jump(e);} , true);
		onSelect.appendChild( makeOption('Goverment','#' )  );
...
function resourcePerBuild(){ // search the page for resource needed to build, and calculates the surplus, deficit or other information
	for(var i=0; i<1; i++){
		...
		if(!returnObjById('contract')) return;
		var current=returnObjById('contract').textContent.match(/(\d+)\s+\|/g);
		...
		document.getElementById('contract').appendChild(holdtable);
	}
...
function pop(){
	...
	fulltable.appendChild(  drow );
	drow.style.cursor='move';	
