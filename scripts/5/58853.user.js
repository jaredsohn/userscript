...
// @authorS		m4rtini (original), j000 (T3.5), Tweaks Derrin, Tweaks Crucial 
// @namespace	T3.5
// @version		V3.5.3
...
//time in minutes between checks. USE RANDOM! It makes it doesn't look like a script.
//10 + Math.floor(Math.random()*5) will check for attacks between 10 and 15 minutes
var timetocheck = 2 + Math.floor(Math.random()*5); 
var soundUrl = "http://simplythebest.net/sounds/WAV/WAV_files/cartoon_WAV_files/bunny_troubles.wav";
//var soundUrl = "file:///D:/temp/Travian/bunny_troubles.wav";
//Replace above with below for alternative sound files
//same as above BUT sounds/WAV/events_WAV/event_WAV_files/somethingwrong.wav AND temp/Travian/somethingwrong.wav"; 
	...
	if (active) {
		...
		send(url, function(responseDetails) {
				...
				if (checkImg(pulled)) {
					...
					infobar.innerHTML ='<span style="color:#0000FF">All clear ...</span>';
				} else {
					var msg ='<span style="color:#FF0000" title="'+fullInfo+'">' + villageNameA + ' Under attack!</span>';
					if (sounds_enabled)
						msg += "<embed src='"+soundUrl+"' hidden=true autostart=true loop=false>";
		...
		FreezeScreen('', false);
		if (!attacked) {
			infobar.innerHTML ='<span style="color:#00CC00">All Clear ...</span>';
		} else {
			var msg ='<span style="color:#FF0000" title="'+fullInfo+'">' + villageNameA + ' Under attack!</span>';
			if (sounds_enabled)
				msg += "<embed src='"+soundUrl+"' hidden=true autostart=true loop=false>";
...
function underAttackCheck (forced) {
	...
	if (exptime < now || forced == true) {
		...
		var msg = 'Checking:<br>Please Wait..';
