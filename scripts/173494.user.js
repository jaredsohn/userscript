...
function GetStarResult(equipData, baseData, multiplier, offset) {
	function CompareWithBaseData(equipData, baseData, multiplier, offset) {
		...
		if (resultStr == '')	resultStr='GODLIKE!!!'
		return resultStr;
	}
	resultStr = '';
	if (!equipData)resultStr = '※※※';
		...
		for (var y in equipData) {
			resultStr += CompareWithBaseData(equipData[y], baseData, multiplier);
		}
	}
	return resultStr;
}
...
Cruncher = {
  ...
  scale: function(source) {
	...
	compareBaseData: function(source,result){
		...
		if (weaponT=source.Info.Name.match(/Axe|Club|Estoc|Mace|Scythe|Katana/)){//双手武器以及斧子棍棒
			...
			damageData ={	'Axe':		[59.66,64.00,66.60,71.77,	39.17,41.79,43.55,47.01],
							...
							'Katana':	[61.37,65.71,68.31,74.34,	40.88,43.50,45.25,49.57],
			...
			accuracyData ={	'Axe':		[9.63,10.28,10.92,11.56,	9.63,10.28,10.92,11.56],
							...
							'Mace':		[18.13,19.38,20.02,21.88,		6.59,7.25,7.28,7.92],
							'Katana':	[39.97,42.45,44.9,48.58,	21.16,22.42,23.66,25.52],
			...
			critData ={		'Axe':		[4.29,4.51,4.72,5.15,			4.29,4.51,4.72,5.15],
							...
							'Katana':	[10.17,10.7,11.23,12.29,		6.29,6.61,6.93,7.56],
		...
		else if (weaponT=source.Info.Name.match(/Buckler|Kite|Tower/)){//盾牌
			/Barrier/.test(source.Info.Name)?weaponD=0:weaponD=4
			blockData ={	'Buckler':	[23.82,25.89,26.94,28.99,		18.83,19.9,20.95,23],
							'Kite':		[28.81,31.88,32.92,34.97,	23.82,25.89,26.94,28.99],
			...
			pabData ={	'Phase Cap': 	{'Intelligence':[5.66,5.98,6.3,6.91], 'Wisdom':[5.66,5.98,6.3,6.91], 'Agility':[3.86, 4.18, 4.5, 4.81]},
						'Phase Robe':	{'Intelligence':[6.86,7.48,7.8,8.41], 'Wisdom':[6.86,7.48,7.8,8.41], 'Agility':[4.46, 4.78, 5.1, 5.41]},
						'Phase Gloves':	{'Intelligence':[5.06,5.38,5.7,6.31], 'Wisdom':[5.06,5.38,5.7,6.31], 'Agility':[3.56, 3.88, 4.2, 4.51]},
						'Phase Pants':	{'Intelligence':[6.26,6.58,6.9,7.51], 'Wisdom':[6.26,6.58,6.9,7.51], 'Agility':[4.16, 4.48, 4.8, 5.11]},
						'Phase Shoes':	{'Intelligence':[4.46,4.78,5.1,5.41], 'Wisdom':[4.46,4.78,5.1,5.41], 'Agility':[3.26, 3.58, 3.6, 3.91]},
			}
			...
			mdamageData ={	'Katalox':	[25.61,27.33,28.20,30.74,	41.37,43.92,45.62,49.82],
							'Oak':		[21.46,23.18,24.05,25.76],
							'Redwood':	[23.95,25.67,26.54,29.08,	36.39,38.95,40.65,44.01],
							'Willow':	[21.46,23.18,24.05,25.76,	36.39,38.95,40.65,44.01]
			}
			...
			pmiData ={	'Helmet':	[ 9.00, 9.65,10.08,10.93,	6.48,6.92,7.14,7.78],
						'Armor':	[10.68,11.33,11.97,13.03,	7.74,8.18,8.61,9.46],
						'Gauntlets':[ 8.16, 8.81, 9.24,10.09,	5.85,6.29,6.51,7.15],
						'Leggings':	[ 9.84,10.49,11.13,11.98,	7.11,7.55,7.98,8.62],
						'Boots':	[ 7.32, 7.97, 8.19, 8.83,	5.22,5.66,5.88,6.31]
			}
			...
			pabData ={	'Helmet':		{'Strength':[3.26,3.58,3.6,3.91],'Agility':[4.16,4.48,4.8,5.11]},
						'Breastplate':	{'Strength':[3.86,4.18,4.5,4.81],'Agility':[5.06,5.38,5.7,6.31]},
						'Gauntlets':	{'Strength':[2.96,3.28,3.3,3.61],'Agility':[3.86,4.18,4.5,4.81]},
						'Leggings':		{'Strength':[3.56,3.88,4.2,4.51],'Agility':[4.46,4.78,5.1,5.41]},
...
Wiki = {
    ...
    if (temp = equip.match(/(Cotton|Gossamer|Phase|Leather|Kevlar|Shade|Plate|Shield (?!of)|Power)/)) { // armor
      target = Wiki._evaluate(
        './/h3[./span[text()="' + temp[1].trim() + '"]]/following-sibling::h4' +
          '[./span[text()="' +temp[1].trim()+ Controller._extractSlot(equip,'').replace(/Helmet/,'Head') + '"]]/following-sibling::table'
      );
    } else { // other
      var type = equip.replace(/Crude|Fair|Average|Superior|Exquisite|Magnificent|Legendary|Ethereal/g,'');
      type = type.replace(/Shocking|Arctic|Tempestuous|Fiery|Astral|Quintessential|Demonic|Hallowed/g,'');
      //alert(type)
      type = type.match(/([^\s]+)/)[1].trim();
	 // alert('.//h3[./span[starts-with(text(),"' + type + '")]]/following-sibling::table')
  ...
  _highlightTable: function(target,equip) {
    ...
    if (/collapsibleTable/.test(target.id)) {
      if (unsafeWindow && unsafeWindow.collapseTable) unsafeWindow.collapseTable(abc=target.id.match(/(\d+)$/)[1]); // firefox
