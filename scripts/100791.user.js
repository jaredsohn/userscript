// ==UserScript==
// @name           KOC Power Tools italiano
// @namespace      Tiestoale[ITA] & Giucas & maciu[ITA]
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @description    Miglioramenti e correzioni per Kingdoms of Camelot
// @require        http://tomchapin.me/auto-updater.php?id=103087
// @version        20110623_ITA
// ==/UserScript==



var Version = '20110623_ITA';

var Title = 'KOC Power Tools - Official KoC Scripters Edition Italiano!';
...
Added limitation cause for troops.
***/
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('TOOLS ITALIANO', eventHideShow, mouseMainTab);
TestSomething.init ();  
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      m += '<TR><TD colspan=20><DIV class=ptstat><TABLE class=ptNoPad width=100%><TR><TD width=100></td><TD width=90% align=center>'+ city.name 
        +' &nbsp; ('+ city.x +','+ city.y +')</td><TD width=100 align=right>Terre: '+ totw +' su '+ castle +' &nbsp; </TD></tr></table></div></td></tr>';
      var row = 0;  
      var sortem = [];

      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Tipo di Terra</td><TD></td><TD align=left>Coordinate</td><TD>Trappole</td><TD align=left>Mercenari</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' CAMBIA/COSTRUISCI DIFESE ') +'</td></tr>';
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Costo: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("COSTRUSCI DIFESE", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Oro: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
Tabs.Knights = {
 tabLabel : 'Cavalieri',
  ...
  show : function (){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=13><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          ...
          <TD width=26>Int</td><TD width=26>Ric</td><TD width=90 align=center colspan=5>--- Non Assegnato ---</td><TD width=40 align=right> Salario </td><TD width=35>Livello</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
  ...
  clickedAssignPoints : function (e, cid, kid, rid){
    ...
    div.innerHTML = 'Sto Assegnando '+ unassigned +' punti abilita\'Â  per '+ knightRoles[rid][1] +' ... ';
...
var messageNav = {
  ...
  hook : function (){
    ...
    var button = makeButton20('Invia a tutti gli UFFICIALI');
  ...
  eventForward : function (){
    document.getElementById('modal_msg_write_subj').value = "inoltrato: " + document.getElementById('modal_msg_view_subj').innerHTML.toString().stripTags();
    ...
    document.getElementById('modal_msg_write_txt').value = '[messaggio originale inviato da '+ from +' inoltrato a te:]\n'+ body;
...
//t.fetchTEST();    
    ...
    if (getMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER>Devi fare parte di un\'alleanza per usare questa funzione.</center>';
      t.state = 1;
      return;
    }
    var m = '<div ><DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Inserisci tutto il nome o in parte di un GIOCATORE: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="Trova Giocatore" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab>OPPURE: </td><TD class=xtab>Inserisci tutto il nome o in parte dell\'ALLEANZA: &nbsp;</td>\
          <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Trova Alleanza" /> &nbsp; <INPUT id=myAllSubmit type=submit value="'+ getMyAlliance()[1] +'" /></td>\
...
*/
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          <TR><TD class=xtab align=right></td><TD class=xtab>Inserisci tutto il nome o in parte di un GIOCATORE: &nbsp;</td>\
            ...
            <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
          <TR><TD class=xtab>OPPURE: </td><TD class=xtab>Inserisci tutto il nome o in parte dell\'ALLEANZA: &nbsp;</td>\
            <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="Trova Alleanza" /></td>\
            <TD class="xtab ptErrText"><SPAN id=ptallErr></span></td></tr>\
           <TR><TD class=xtab></td><TD class=xtab> &nbsp;\
            </td>\
            <TD class=xtab><INPUT align=right id=idMyAllSubmit type=submit value="'+ getMyAlliance()[1] +'"/>\
             <TD class=xtab><span align=right <b>Simula ATTACCO/RINFORZO con: </b></span></td>\
             <TD class=xtab ><div><select id="idFindETASelect">\
        <option value="0,250" > -- Seleziona -- </option>\
        <option value="0,180" > Truppe rif. </option>\
        <option value="0,200" > Soldati </option>\
        <option value="0,3000" > Spie </option>\
        <option value="0,300" > Lance </option>\
        <option value="0,275" > Spade </option>\
        <option value="0,250" > Archi </option>\
        <option value="1,1000" > Cavalli </option>\
        <option value="1,750" > CavPes </option>\
        <option value="1,150" > Carri </option>\
        <option value="1,100" > Balestre </option>\
  ...
  eventGotPlayerOnlineList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Mostro i nomi che contengono <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Potere</td><TD> &nbsp; Online</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Dettagli </td></tr>';
    var row=0;
    var cl='';
    for (k in t.playerList){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>dettagli</a> &nbsp; <BR></span><SPAN onclick="PTpl2(this, \''+ u.userId +'\')"><A>Classifica</a><BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>ultimo accesso</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "recupero info classifica ...";
  ...
  clickedPlayerGetLastLogin : function (span, uid){
     var t = Tabs.AllianceList;
     span.onclick = '';
     span.innerHTML = "recupero data di accesso ...";
  ...
  gotPlayerLeaderboard2 : function (rslt,span,uid){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Classifica:</b> Non trovato! (nebbie?)';
      return;
    }
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    for (var i=0; i<p.cities.length; i++){
      ...
      if (c.dateCreated && c.dateCreated.substr(0,2)=='20')
        created = ' &nbsp; creato: ' + c.dateCreated.substr(0,10);
  ...
  eventListSubmit : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'Devi essere in un\'alleanza per usare questa opzione.';
    }
  },
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Mostro i nomi che contengono <B>"'+ t.aName +'"</b></div>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Vedi Membri</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 ) {
      document.getElementById('allListOut').innerHTML = 'Elenco le Alleanze.';
      return;
    }
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Vedi Membri</a></td></tr>';
    }
...
return 0;        
    ...
    for (var i=0; i<t.dat.length; i++){
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ addCommasInt(t.dat[i][1]) 
         ...
         </td><TD  nowrap class=xxtab>'+ (t.dat[i][10]?'<SPAN>'+ (t.dat[i][10]>0?timestr(t.dat[i][10],1):'NA') +'</span>':'<SPAN>NA</span>') +'<td class=xxtab><SPAN onclick="PCplo(this, \''+ t.dat[i][11] +'\')"><A>Ultimo Accesso</a></span><td></tr>';
    }
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat ><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>La distanza calcolata e\' da <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' giocatori trovati &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Giocatore</div></a></td>\
        ...
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Citta\'Â </a></div></td>\
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Ruolo</a></div></td>\
        <TD id=clickCol9 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Online</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Nome Citta\'Â </a></div></td>\
        ...
        <TD id=clickCol10 onclick="PTalClickSort(this)" class=clickable><A><DIV>Arrivo in</a></div></td>\
    ...
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Mostra distanza da: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; Oppure, Scegli citta\'Â : <span id=dmcoords></span></td></tr></table>';
  ...
  displayPlayer : function (allName,rslt){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
            ...
            <DIV class=ptstat ><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab>Alliance: '+ allName +'</td>\
              <TD class=xtab width=80% align=center>Ultimo login: <SPAN id=lastlogin>'+  rslt.playerInfo.lastLogin+'</span></td><TD class=xtab align=right></td></tr></table></div>\
      <div style="max-height:470px; height:470px; overflow-y:auto;"><TABLE id=tabAllMembers align=center cellpadding=0 cellspacing=0><THEAD style="overflow-y:hidden;">\
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Giocatore</div></a></td>\
        <TD id=clickCol1 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Potere</a></div></td>\
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Citta\'</a></div></td>\
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Posizione</a></div></td>\
        <TD id=clickCol9 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Online</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Nome citta\'</a></div></td>\
        <TD id=clickCol4 onclick="PTalClickSort(this)" class=clickable><A><DIV>Liv</a></div></td>\
    ...
    document.getElementById('allListOut').innerHTML = m;  //style="top:670px; left:0px; position:absolute;
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Show distance from: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; O, scegli citta\': <span id=dmcoords></span></td></tr></table>';
    document.getElementById('clickCol'+t.sortColNum).className = 'clickable clickableSel';
    ...
    gotPlayerLastLogin : function (rslt, span){
        ...
        if (lastLogin) {
          m = '<span style="color:black">Ultimo accesso: '+lastLogin+'</span>';
           ...
           m = '<span style="color:red">Ultimo accesso non trovato: '+lastLogin+'</span>';
        }
        span.innerHTML = m + '';
      },
...
var crestreq = { 3:{1101:4, 1102:2, 1103:1},
 ...
 m += '<DIV class=ptstat>ELMI/SIGILLI RICHIESTI</div><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR CLASS="xtabH">\
        <TD class=xtab></TH>\
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Abilita trascinamento finestra (tieni premuto sulla barra del titolo)</td></tr>\
        <TR><TD><INPUT id=ptHideOnGoto type=checkbox /></td><TD>Nascondi la finestra se clicchi sulle coordinate nella mappa. <SPAN class=boldRed>&nbsp;(NOVITA\')</span></td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Mostra l\'autonomia in rosso se il cibo scende sotto le \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> ore, non influisce con l\'allarme del cibo!</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodTower type=checkbox /></td><TD>Abilita l\'allarme del cibo. (Avviso impostato a 6 ore, con controlli ogni 30min.)<SPAN class=boldRed>&nbsp;(GRANDE)</span></td></tr>\
	<TR><TD colspan=2><P><B>Impostazioni della chat:</b></td></tr>\
	<TR><TD><INPUT id=togChatStuff type=checkbox /></td><TD>Abilita miglioramenti (coord. cliccabili, click su icona per sussurro, colori). <SPAN class=boldRed>&nbsp;(NOVITA\')</span></td></tr>\
        <TR><TD><INPUT id=togChatGlobal type=checkbox /></td><TD>Colora sfondo nella chat globale. <SPAN class=boldRed>&nbsp;(GRANDE)</span></td></tr>\
	<TR><TD><INPUT id=togChatWhisper type=checkbox /></td><TD>Carattere dei sussurri in ROSSO.<SPAN class=boldRed>&nbsp;(GRANDE)</span></td></tr>\
	<TR><TD><INPUT id=togChatBold type=checkbox /></td><TD>Carattere chat in grassetto.<SPAN class=boldRed>&nbsp;(GRANDE)</span></td></tr>\
	<TR><TD colspan=2><P><B>Caratteristiche KofC:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Mostra il numero dei messaggi, separato da quello dei reports.</td></tr>';
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Abilita i miglioramenti nei reports dell\'Alleanza.</td></tr>\
        ...
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Abilita i pulsanti INOLTRA e A TUTTI GLI UFFICIALI).</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>Migliora la lettura di messaggi e reports.</td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>Avvisa se avvio una marcia alle coord 0,0.</td></tr>\
        <TR><TD><INPUT id=togGmtClock type=checkbox /></td><TD>Abilita orario GMT accanto a quello di "Camelot Time" <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togAttackPicker type=checkbox /></td><TD>Abilita le funzioni rinforza, riassegna e trasporta nella finestra di attacco <SPAN class=boldRed>&nbsp;(NUOVA opzione)</span></td></tr>\
        <TR><TD><INPUT id=togBatRounds type=checkbox /></td><TD>Display # of rounds in battle reports <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togAtkDelete type=checkbox /></td><TD>Abilita pulsante cancella nella visione dei reports <SPAN class=boldRed>&nbsp;(NOVITA\')</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>Correzione errori KofC:</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Sistema la torre per mostrare l\'obiettivo esatto (citta\', terra o invalido)</td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Sistema la mappa per mostrare le distanze fra le citta\' selezionate invece della prima citta\'.</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Sistema gli allarmi creati da falsi attacchi degli esploratori.</td></tr>\
        <TR><TD><INPUT id=togKnightSelect type=checkbox /></td><TD>Non selezionare il cavaliere se cambi un tipo di marcia con un\'altra <SPAN class=boldRed>(NOVITA\')</span></td></tr>\
        <TR><TD><INPUT id=togCoordBox type=checkbox /></td><TD>Tieni la finestra delle coord e bookmarks sopra l\'attivita\' truppe <SPAN class=boldRed>(NOVITA\')</span></td></tr>\
        </table><BR><BR><HR>ATTENZIONE: se un quadratino e\' grigio, vuol dire che KofC ha modificato il codice rendendolo inutilizzabile.';
      t.cont.innerHTML = m;
...
Tabs.Train = {
  ...
  init : function (div){
    ...
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Addestra le truppe e costruisci le difese della citta\'</div><DIV style='height:5px'></div><DIV class=ptentry>\
      <DIV style='text-align:center; margin-bottom:5px;'>Scegli Citta\': &nbsp; <span id=ptspeedcity></span></div>\
      <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
      <TABLE align=center><TR><TD align=right>Tipo truppa: </td><TD colspan=2>\
      <SELECT id=ptttType>\
        <option value='1'>Truppe rif.</option>\
        <option value='2'>Soldati</option>\
        ...
        <option value='8'>Cavalleria pes.</option>\
        <option value='9'>Carri trasp.</option>\
      ...
      <TR><TD align=right># di slots: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
        <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
		<TR><TD align=right>Limitato da: </td><td colspan=2><span id=ptttLimiter></span></TD></TR>\
      <TR><TD align=right valign=top>Metti i lavoratori in pausa: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Ti permette di addestrare piu\' truppe ma la popolazione potrebbe andare in negativo.</span></td></tr>\
      ...
      <TR><TD align=right># di slots: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
...
changeTroopSelect : function (){
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (t.running){
      t.stopTraining('<SPAN class=boldRed>Addestramento cancellato dal giocatore</span>');
      return; 
    }    
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Il numero di truppe deve essere maggiore di 0.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Non posso addestrarle (il max e\' '+ t.stats.MaxTrain +' truppe)</font>';
      return;
    }
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    t.TDspSpace.innerHTML = 'Livello mura: <B>'+ t.stats.wallLevel +'</b><BR>Spazi mura: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        Spazi campi: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (t.running){
      t.stopTraining('<SPAN class=boldRed>Costruzione cancellata dal giocatore</span>');
      return; 
    }    
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Il numero di truppe deve essere maggiore di 0.</font>';
    ...
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Non posso addestrarle (il max e\' '+ t.stats.MaxDefTrain +' total)</font>';
      return;
    }
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.stopTraining ('<B>Difese in coda.</b>');
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Costruisco '+ cmd[2] +' '+  fortNamesShort[cmd[1]] +' a '+ Cities.byID[cityId].name +' ('+ que.length +' slots vuoti)<BR>');
...
m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=8%><B>TrRif:</b></td><TD width=8%><B>Soldati:</b></td><TD width=8%><B>Spie:</b></td>\
        <TD width=8%><B>Lance:</b></td><TD width=8%><B>Spade:</b></td><TD width=8%><B>Archi:</b></td>\
        <TD width=8%><B>Cav:</b></td><TD width=8%><B>Cav pes:</b></td><TD width=8%><B>Carri:</b></td>\
        <TD width=8%><B>Baliste:</b></td><TD width=8%><B>Arieti:</b></td><TD width=8%><B>Catap:</b></td><tr>';
 ...
 '<TD width=8%>'+Seed.units['city'+cityId]['unt11']+'</td><TD width=8%>'+Seed.units['city'+cityId]['unt12']+'</td><tr></table>';
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        ...
        <TD width=16%><SPAN id=ptttr_ore><B>Miner.:</b><BR>'+ addCommasInt(t.stats.ore) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_gold><B>Oro:</b><BR>'+ addCommasInt(t.stats.gold) +'</span></td>\
        <TD width=16%><SPAN id=ptttr_pop><B>Popol.:</b><BR>'+ addCommasInt(t.stats.idlePop) +'</span></td></tr></table><BR>';
...
// defense queue ....
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.stopTraining ('<B>Truppe in coda.</b>');
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Addestro '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +' a '+ Cities.byID[cityId].name +' ('+ que.length +' slots vuoti)<BR>');
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Iniziato:</span> '+ dt.toLocaleDateString() +'</td>\
      ...
      str += "<DIV id=overMainDiv style='font-size:"+ Options.overviewFontSize +"px'><TABLE class=ptTabOverview cellpadding=0 cellspacing=0><TR valign=top align=right><TD width=65></td><TD width=88 style='background: #ffc'><B>TOTALE</b></td>";
      for(i=0; i<Cities.numCities; i++) {
...
str += '<TR valign=top align=right><TD></td><TD style=\'background: #ffc\'></td>';
    ...
    if(Gate == 0)
      ...
      str += '<TD><SPAN class=boldRed><blink>Difesa</blink></span></td>';
    }
      ...
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Includi truppe/risorse in marcia</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptoverIncTraining'+ (Options.includeTraining?' CHECKED':'') +'>Includi truppe in addestramento</td></tr>';
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><INPUT type=CHECKBOX id=ptOverOver'+ (Options.overviewAllowOverflow?' CHECKED':'') +'>Cambia misura carattere \
         &nbsp; &nbsp; FONT SIZE: '+ htmlSelector ({9:9, 10:10, 11:11, 12:12}, Options.overviewFontSize, 'id=ptoverfont') +'</td></tr><BR>';
      str += "</table></div>";
      str+= '<CENTER><B>Versione Koc Power Tools in Italiano Creata da Tiestoale[ITA] & Giucas</B></center>';
...
Tabs.msg = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptstat>Cerca nella casella di posta o nei report dell\'alleanza</div>';
    m += '<DIV class=ptentry style="height:30px"><table>';
    m += '<TABLE width=95% class=ptTab><TD>Cerca in: <select id="searchSelect"><option value="Reports">Report</options>';
    m += '<option value="inbox">Posta in Arrivo</option>';
    m += '<option value="outbox">Posta in Uscita</option></select></td>';
    
    m += '<TD>Pagina: <select id="pagesSelect">';
    ...
    m += '<option value=101> TUTTO </option></select></td>'
    
    m += '<TD>Cerca in: <select disabled=true id="searchForSelect"><option value="Subject">Oggetto</options>';
    m += '<option value="User">Utente</option></select></td>';
    
    m += '<TD>Ricerca:<INPUT id=searchString disabled=true type=text size=10 maxlength=25 value=""</td>';
    m += '<TD><INPUT id=StartSearch type=submit value="Inizia RICERCA"></td>';
    ...
    document.getElementById('StartSearch').addEventListener ('click', function(){ 
        	 if (document.getElementById('searchSelect').value == "Reports") {t.data=[];t.handleArcSearch();}
        	 	...
        	 	t.data=[];
        	 	t.content = '<center><table><thead><th>Azione</th><th>Pagina#</th><th>Data</th><th>Da</th><th>Oggetto</th></thead><tbody>';
        	 	t.searchMail("",1); 
        	 }
        }, false);
  ...
  searchMail : function() {
    ...
    results.innerHTML = '<center><b>...Sto Cercando '+t.maxPages+' pages...</b></center>';
    t.data=[];
  ...
  DisplayReports : function (){
    ...
    var results=document.getElementById("ReportResults");
    if(!t.data.length) {
       results.innerHTML = '<center>Nessun attacco trovato alla vostra alleanza</center>';
       return;
    }
    var m = '<center><table><thead><th>Pagina#</th><th>Data</th><th>Attaccante</th><th>Da</th><th>Alleanza</th><th>Azione</th><th>Bersaglio</th><th>A</th></thead>';
    m += '<tbody>';
  ...
  handleArcSearchCB : function(rslt, page) {
    ...
    if (rslt) {
       ...
       if (rslt.arReports && page) {
         ...
         for (var i = 0; i < rptkeys.length; i++) {
              ...
              if (rpt.side0CityId > 0)
                ...
                rpt.side0CityName = 'nessuno';
              if (rpt.side1CityId > 0)
                rpt.side1CityName = rslt.arCityNames['c'+rpt.side1CityId];
              else
                rpt.side1CityName = 'nessuno';
              if (rpt.marchType == 1)
                  rpt.marchName = 'Trasporto';
              else if (rpt.marchType == 3)
                  rpt.marchName = '<SPAN class=sco> Esplorazione</span>';
              else if (rpt.marchType == 2)
                  rpt.marchName = 'Reinf';
              else if (rpt.marchType == 4)
                  rpt.marchName = '<SPAN class=atk> Attacco</span>';
       ...
       if (parseInt(page)+1 <= t.maxPages) {
          var results=document.getElementById("ReportResults");
          results.innerHTML = '<center><b>...Sto Cercando '+(parseInt(page)+1)+'...</b></center>';
  ...
  handleArcSearch : function() {
    ...
    if ( t.maxPages==99999)
       t.maxPages=t.totalPages;
    results.innerHTML = '<center><b>...Sto Cercando nelle '+t.maxPages+' pagine...</b></center>';
    t.data=[];
    t.getAllianceReports(1);
  },
  ...
  searchMail : function(rslt, pageNum) {
    ...
    if (parseInt(pageNum) <= t.maxPages) {
          ...
          document.getElementById('searchStatus').innerHTML = "FATTO";
       } 
  },
  ...
  showBody: function (messageBody) {
  	...
  	t.popMsg.getTopDiv().innerHTML = '<TD><B>MESSAGGI</td>';
  	t.popMsg.show(true)	;
   },
  ...
  searchInMail: function(rslt,pageNum){
	   ...
	   for (k in myarray) {
	      ...
	      if (lookup.search(what, "i") != -1){
	        t.content += '<TR><TD><A><SPAN onclick="getmsg('+ myarray[k]['messageId'] +')">APRI</span></a></td>'
...
Tabs.Marches = {
  ...
  init : function (div){
    ...
    t.cont = div;
    t.cont.innerHTML = '<TABLE class=ptTab align=center><TR><TD><INPUT class=pbSubtab ID=ptmrchSubA type=submit value=Movimenti></td>\
          ...
          if(Seed.queue_atkinc.length !=0){
       		//if  (parseInt(Seed.queue_atkinc[k]["marchType"]) ==3 || parseInt(Seed.queue_atkinc[k]["marchType"])==4){
               	...
               	if (Seed.queue_atkinc[k]["resource1"] > 0) z += '<TD>Cibo: '+ addCommas(Seed.queue_atkinc[k]["resource1"]) +'</td>';
        		    ...
        		    if (Seed.queue_atkinc[k]["resource3"] > 0) z += '<TD>Pietre: '+ addCommas(Seed.queue_atkinc[k]["resource3"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["resource4"] > 0) z += '<TD>Miner: '+ addCommas(Seed.queue_atkinc[k]["resource4"]) +'</td>';
        		    ...
        		    if (Seed.queue_atkinc[k]["unit1Count"] > 0 && marchstatus == "going") z += '<TD>Truppe rif.: '+ addCommas(Seed.queue_atkinc[k]["unit1Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit1Return"] > 0 && marchstatus == "returning") z += '<TD>Truppe rif.: '+ addCommas(Seed.queue_atkinc[k]["unit1Return"]) +'</td>';
        		    
        		    if (Seed.queue_atkinc[k]["unit2Count"] > 0 && marchstatus == "going") z += '<TD>Soldati: '+ addCommas(Seed.queue_atkinc[k]["unit2Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit2Return"] > 0 && marchstatus == "returning") z += '<TD>Soldati: '+ addCommas(Seed.queue_atkinc[k]["unit2Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit3Count"] > 0 && marchstatus =="going") z += '<TD>Spie: '+ addCommas(Seed.queue_atkinc[k]["unit3Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit3Return"] > 0 && marchstatus == "returning") z += '<TD>Spie: '+ addCommas(Seed.queue_atkinc[k]["unit3Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit4Count"] > 0 && marchstatus =="going") z += '<TD>Spie: '+ addCommas(Seed.queue_atkinc[k]["unit4Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit4Return"] > 0 && marchstatus == "returning") z += '<TD>Lance: '+ addCommas(Seed.queue_atkinc[k]["unit4Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit5Count"] > 0 && marchstatus =="going") z += '<TD>Lance: '+ addCommas(Seed.queue_atkinc[k]["unit5Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit5Return"] > 0 && marchstatus == "returning") z += '<TD>Spade: '+ addCommas(Seed.queue_atkinc[k]["unit5Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit6Count"] > 0 && marchstatus =="going") z += '<TD>Archi: '+ addCommas(Seed.queue_atkinc[k]["unit6Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit6Return"] > 0 && marchstatus == "returning") z += '<TD>Archi: '+ addCommas(Seed.queue_atkinc[k]["unit6Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit7Count"] > 0 && marchstatus =="going") z += '<TD>Cavalli: '+ addCommas(Seed.queue_atkinc[k]["unit7Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit7Return"] > 0 && marchstatus == "returning") z += '<TD>Cavalli: '+ addCommas(Seed.queue_atkinc[k]["unit7Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit8Count"] > 0 && marchstatus =="going") z += '<TD>Cavalli pes: '+ addCommas(Seed.queue_atkinc[k]["unit8Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit8Return"] > 0 && marchstatus == "returning") z += '<TD>Cavalli pes: '+ addCommas(Seed.queue_atkinc[k]["unit8Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit9Count"] > 0 && marchstatus =="going") z += '<TD>Carri: '+ addCommas(Seed.queue_atkinc[k]["unit9Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit9Return"] > 0 && marchstatus == "returning") z += '<TD>Carri: '+ addCommas(Seed.queue_atkinc[k]["unit9Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit10Count"] > 0 && marchstatus =="going") z += '<TD>Baliste: '+ addCommas(Seed.queue_atkinc[k]["unit10Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit10Return"] > 0 && marchstatus == "returning") z += '<TD>Baliste: '+ addCommas(Seed.queue_atkinc[k]["unit10Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit11Count"] > 0 && marchstatus =="going") z += '<TD>Arieti: '+ addCommas(Seed.queue_atkinc[k]["unit11Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit11Return"] > 0 && marchstatus == "returning") z += '<TD>Arieti: '+ addCommas(Seed.queue_atkinc[k]["unit11Return"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit12Count"] > 0 && marchstatus =="going") z += '<TD>Catap: '+ addCommas(Seed.queue_atkinc[k]["unit12Count"]) +'</td>';
        		    if (Seed.queue_atkinc[k]["unit12Return"] > 0 && marchstatus == "returning") z += '<TD>Catap: '+ addCommas(Seed.queue_atkinc[k]["unit12Return"]) +'</td>';
        		    
        		    if (marchType == 3 && Seed.queue_atkinc[k]["unts"]["u3"] > 0) z += '<TD>Spie: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u3"]) +'</td>';
        		    if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u1"] > 0) z += '<TD>Truppe rif.: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u1"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u2"] > 0) z += '<TD>Soldati: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u2"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u3"] > 0) z += '<TD>Spie: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u3"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u4"] > 0) z += '<TD>Lancieri: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u4"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u5"] > 0) z += '<TD>Spadaccini: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u5"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u6"] > 0) z += '<TD>Arcieri: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u6"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u7"] > 0) z += '<TD>Cavalli: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u7"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u8"] > 0) z += '<TD>Cavalli pes.: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u8"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u9"] > 0) z += '<TD>Carri: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u9"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u10"] > 0) z += '<TD>Baliste: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u10"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u11"] > 0) z += '<TD>Arieti: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u11"]) +'</td>';
               		if (marchType == 4 && Seed.queue_atkinc[k]["unts"]["u12"] > 0) z += '<TD>Catapulte: '+ addCommas(Seed.queue_atkinc[k]["unts"]["u12"]) +'</td>';
               		z += '</tr>';
              } 
        // } 
     }
    		...
    		if (Seed.queue_atkp[cityID].length !=0) m+= '<TR><TD style=\'background: #99CCFF;\' align=center><B>' + cityname +': </b></td></tr>';
  		    for (k in Seed.queue_atkp[cityID]){
  				 if (Seed.queue_atkp[cityID].length !=0) {
  				    ...
  				    else marchstatus = "andata";
  				   
  				    if (marchstatus =="ritorno" && marchType == 2 && Seed.queue_atkp[cityID][k]["destinationUnixTime"] < (now) && marchStatus != 2) marchtime = timestr(returnUnixTime, true);
  				    if (marchstatus =="ritorno" && marchType == 4 && Seed.queue_atkp[cityID][k]["destinationUnixTime"] < (now) && marchStatus == 2) marchtime = timestr(returnUnixTime, true);
  				    ...
  				    if (marchstatus =="ritorno" && marchType != 2) marchType = 100;
  				    if (marchstatus =="ritorno" && marchType == 2 && marchStatus == 2) marchType = 102;
  				    
  				    if (marchstatus =="ritorno" && marchType == 2 && Seed.queue_atkp[cityID][k]["destinationUnixTime"] < (now) && marchStatus != 2) marchType = 100;
  				    ...
  				    switch (marchType) {
  					    case 1: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/transporting.jpg";status="Trasporto";break;
  					    case 2: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/reinforce.jpg";status="Rinforzo";break;
  					    case 3: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/scouting.jpg";status="Esplor.";break;
  					    case 4: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/attacking.jpg";status="Attacco";break;
  					    case 5: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/transporting.jpg";status="Riassegna";break;
  					    case 100: icon="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/returning.jpg";status="Ritorno";break;
  				    ...
  				    if (knight != null)  m += '<TD>Caval.:'+ knight +'</td>';
  				    
  				    if (Seed.queue_atkp[cityID][k]["toTileType"] == 11) m+='<TD>Lago Liv: ' + Seed.queue_atkp[cityID][k]["toTileLevel"] + '</td>';
  				    else if (Seed.queue_atkp[cityID][k]["toTileType"] == 20) m+='<TD>Prateria Liv: ' + Seed.queue_atkp[cityID][k]["toTileLevel"] + '</td>';
  				    else if (Seed.queue_atkp[cityID][k]["toTileType"] == 30) m+='<TD>Collina Liv: ' + Seed.queue_atkp[cityID][k]["toTileLevel"] + '</td>';
  				    else if (Seed.queue_atkp[cityID][k]["toTileType"] == 40) m+='<TD>Montagna Liv: ' + Seed.queue_atkp[cityID][k]["toTileLevel"] + '</td>';
  				    else if (Seed.queue_atkp[cityID][k]["toTileType"] == 50) m+='<TD>Pianura Liv: ' + Seed.queue_atkp[cityID][k]["toTileLevel"] + '</td>';
  					else if (Seed.queue_atkp[cityID][k]["toCityId"] ==0) m +='<TD>Barb Liv: '+Seed.queue_atkp[cityID][k]["toTileLevel"]+'</td>';
  				    ...
  				    if (Seed.queue_atkp[cityID][k]["resource3"] > 0) m += '<TD>Pietre: '+ addCommas(Seed.queue_atkp[cityID][k]["resource3"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["resource4"] > 0) m += '<TD>Miner.: '+ addCommas(Seed.queue_atkp[cityID][k]["resource4"]) +'</td>';
  				    ...
  				    if (Seed.queue_atkp[cityID][k]["unit1Count"] > 0 && marchstatus == "going") m += '<TD>Truppe rif: '+ addCommas(Seed.queue_atkp[cityID][k]["unit1Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit1Return"] > 0 && marchstatus == "returning") m += '<TD>Truppe rif: '+ addCommas(Seed.queue_atkp[cityID][k]["unit1Return"]) +'</td>';
  				    
  				    if (Seed.queue_atkp[cityID][k]["unit2Count"] > 0 && marchstatus == "going") m += '<TD>Soldati: '+ addCommas(Seed.queue_atkp[cityID][k]["unit2Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit2Return"] > 0 && marchstatus == "returning") m += '<TD>Soldati: '+ addCommas(Seed.queue_atkp[cityID][k]["unit2Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit3Count"] > 0 && marchstatus =="going") m += '<TD>Spie: '+ addCommas(Seed.queue_atkp[cityID][k]["unit3Count"])+'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit3Return"] > 0 && marchstatus == "returning") m += '<TD>Spie: '+ addCommas(Seed.queue_atkp[cityID][k]["unit3Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit4Count"] > 0 && marchstatus =="going") m += '<TD>Lance: '+ addCommas(Seed.queue_atkp[cityID][k]["unit4Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit4Return"] > 0 && marchstatus == "returning") m += '<TD>Lance: '+ addCommas(Seed.queue_atkp[cityID][k]["unit4Return"])+'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit5Count"] > 0 && marchstatus =="going") m += '<TD>Spade: '+ addCommas(Seed.queue_atkp[cityID][k]["unit5Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit5Return"] > 0 && marchstatus == "returning") m += '<TD>Spade: '+ addCommas(Seed.queue_atkp[cityID][k]["unit5Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit6Count"] > 0 && marchstatus =="going") m += '<TD>Archi: '+ addCommas(Seed.queue_atkp[cityID][k]["unit6Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit6Return"] > 0 && marchstatus == "returning") m += '<TD>Archi: '+ addCommas(Seed.queue_atkp[cityID][k]["unit6Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit7Count"] > 0 && marchstatus =="going") m += '<TD>Cavalli: '+ addCommas(Seed.queue_atkp[cityID][k]["unit7Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit7Return"] > 0 && marchstatus == "returning") m += '<TD>Cavalli: '+ addCommas(Seed.queue_atkp[cityID][k]["unit7Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit8Count"] > 0 && marchstatus =="going") m += '<TD>Cavalli pes: '+ addCommas(Seed.queue_atkp[cityID][k]["unit8Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit8Return"] > 0 && marchstatus == "returning") m += '<TD>Cavalli pes: '+ addCommas(Seed.queue_atkp[cityID][k]["unit8Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit9Count"] > 0 && marchstatus =="going") m += '<TD>Carri: '+ addCommas(Seed.queue_atkp[cityID][k]["unit9Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit9Return"] > 0 && marchstatus == "returning") m += '<TD>Carri: '+ addCommas(Seed.queue_atkp[cityID][k]["unit9Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit10Count"] > 0 && marchstatus =="going") m += '<TD>Baliste: '+ addCommas(Seed.queue_atkp[cityID][k]["unit10Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit10Return"] > 0 && marchstatus == "returning") m += '<TD>Baliste: '+ addCommas(Seed.queue_atkp[cityID][k]["unit10Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit11Count"] > 0 && marchstatus =="going") m += '<TD>Arieti: '+ addCommas(Seed.queue_atkp[cityID][k]["unit11Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit11Return"] > 0 && marchstatus == "returning") m += '<TD>Arieti: '+ addCommas(Seed.queue_atkp[cityID][k]["unit11Return"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit12Count"] > 0 && marchstatus =="going") m += '<TD>Catap: '+ addCommas(Seed.queue_atkp[cityID][k]["unit12Count"]) +'</td>';
  				    if (Seed.queue_atkp[cityID][k]["unit12Return"] > 0 && marchstatus == "returning") m += '<TD>Catap	: '+ addCommas(Seed.queue_atkp[cityID][k]["unit12Return"]) +'</td></tr>';
   			  }
  		    }
  ...
  showReinforcements : function (){
    var rownum = 0;
    var names = ['TrRif', 'Sol', 'Spie', 'Lance', 'Spade', 'Archi', 'Cav', 'CavPes', 'Carri', 'Bali', 'Arieti', 'Cat'];
    var t = Tabs.Marches;
    clearTimeout (t.displayTimer);
...
//logit ("enc: "+ inspect (enc, 6, 1));    
    s = '<div class=ptstat>Mostra le truppe accampate nelle tue ambasciate.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Nessuna truppa accampata.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Giocatore (caval.)</b></td>';
...
// TODO: Only allow 'send home' if troops are here now  (marchStatus = ?)              
    ...
    s += '<BR><BR><DIV style="font-size: 10px">ATTENZIONE: Devi riaggiornare per vedere nuovi rinforzi o le truppe rimaste dopo una battaglia.</div>';
