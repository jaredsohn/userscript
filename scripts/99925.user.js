// ==UserScript==
// @name           Hachi Tools Stand Alone II
// @namespace      Hachi
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @include	   http://corpr8.co.uk/newalert/html
// @description    Enhancements and bug fixes for Kingdoms of Camelot
// @require        http://sizzlemctwizzle.com/updater.php?id=99884&days=1

// ==/UserScript==

var Version = '11.0.0';

var Title = 'Hachi Tools Stand Alone II';
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('Falcon Tools ' + Version, eventHideShow, mouseMainTab);
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
		...
		var kN = knight.knightName.length > 25 ? knight.knightName.substr(0,24) + '...' : knight.knightName;
        m += kN + '</td><TD>'+ skills[0] +'</td><TD>'+ skills[1] +'</td><TD>'+ skills[2] +'</td><TD>' + skills[3]
...
var FoodAlerts = {
  ...
  e_eachMinute : function (){   
    ...
    for(i=0; i < Cities.numCities; i++) {
      ...
      if (Options.enableFoodAlert && timeLeft >= 0 && timeLeft < Options.FoodAlertHours * 3600) {
	    var msg = '';
...
//DebugTimer.start(); 
    try {
      ...
      for(i=0; i<Cities.numCities; i++) {
        str += "<TD width=81><B>"+ Cities.cities[i].name.substring(0,8) +'</b><BR>'+coordLink(Cities.cities[i].x,Cities.cities[i].y )+'</td>';
...
//**********************************
	  ...
	  if (Options.includeTraining) {
        ...
        str += _rowShort ('</b>St Tr<b>', t_rows[6]);
...
function addCommasShort(nStr){
  ...
  if (x1.length <= 2) {
    ...
    x2 = (x1.length == 12 + neg_ ? '' : '.' + x1.substr(x1.length - 9 + neg_, 12 - x1.length + neg_)) + ' G';
    rgx = /(\d+)(\d{9})/;
