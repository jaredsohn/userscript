// ==UserScript==
// @name			Trchan X - Hover Version
// @version			v1.04 - 02 Jan. 2012
// @namespace		        http://freedollchan.org/scripts
// @author			IMC, Sthephan Shinkufag @ FreeDollChan
// @copyright		        ©2012, IMC
// @description		        Trchan X 
// @include			*trchan.org*
// @icon                        http://www.trchan.org//favicon.ico
// ==/UserScript==
...
function removeFavorities(el) {
	...
	$del($up(el, 2));
	if(arr.length == 0)
		$1($id('DESU_favor')).innerHTML = '<b>Favori Başlıklar...</b>';
	setStored('DESU_Favorities', arr.join('|'));
}
...
function addControls() {
	...
	$before(el, [
		$New('div', [
			$btn('Ayarlar', function() {
				$delCh($id('DESU_hidden'));
				$delCh($id('DESU_favor'));
				$disp($id('DESU_controls'));
			}),
			$btn('Saklı', hiddenPostsPreview),
			$btn('Favoriler', favorThrdsPreview),
			$new('input', {
				'type': 'button',
				'id': 'refresh_btn',
				'value': 'Yenile'}, {
			...
			$if(isMain && pr.on, $btn('Yeni Başlık', function() {$disp(pr.area)})),
			$if(!isMain, $new('span', {
				'html': '[<a href="http://' + location.hostname + '/' + brd + '/">Geri Dön</a>]',
				'style': 'float:right'
			}))
			], {
			'id': 'DESU_panel',
	...
	$append($id('DESU_controls'), [
		$new('div', {
			'text': 'Trchan kullanıcı script ayarları',
			'style': 'text-align:center; font-weight:bold; font-size:14px; margin-bottom:3px'
		}),
		$New('div', [
			chBox(1, 'Filtreler <öneri: hepsini pasifleştir> '),
			$new('span', {
				'html': '[<a>&gt;</a>]',
		...
		$New('div', [
			divBox(2, 'Sıraları yinele'),
			divBox(3, 'Kelimeleri yinele'),
			divBox(4, 'Özel karakterler'),
			divBox(5, 'Uzun sütunlar'),
			divBox(6, 'Uzun kelimeler'),
			divBox(7, 'Numaralar'),
			divBox(8, 'Kayıtlar')
			], {
			'id': 'antiwipe_cfg',
		...
		chBox(14, 'Şu sayıdan fazla karakterli ', toggleMaxtext, 'maxtext_hider'),
		$new('input', {'type': 'text', 'id': 'maxtext_field', 'value': Cfg[15], 'size': 4}),
		$txt(' yazıları sakla'),
		$new('br'),
		$if(hasSage, chBox(9, 'Sage Tikli ', toggleSage, 'sage_hider')),
		$if(pr.subj, chBox(10, 'Konu Tikli ', toggleTitle)),
		chBox(11, 'Yazı Yok Tikli ', toggleNotext, 'notext_hider'),
		chBox(12, 'Resim Yok Tikli ', toggleNoimage, 'noimage_hider'),
		$new('br'),
		chBox(13, 'Rehber? ( Ruscada, bana bir çevirmen bul? ): ', toggleRegexp, 'regexp_hider'),
		$new('span', {
			'html': '[<a>?</a>]',
			'style': 'cursor:pointer'}, {
			'click': function() {$alert('В тексте/теме поста:\nололо\nОП хуй\n...\n\nРегулярные Expressions: $exp выраж.\n$exp /[bб].[tт]+[hх].[rр][tт]/ig\n$exp /кукл[оа]([её]б|бляд|быдл)/ig\n\nКартинки: $img [<,>,=][вес][@ширxвыс]\n$img <35@640x480\n$img >@640x480\n$img =35\n\nИмя/трипкод: $name [имя][!трипкод][!!трипкод]\n$name Sthephan!ihLBsDA91M\n$name !!PCb++jGu\nЛюбой трипкод: $alltrip\n\nAutoзамена (после перезагр.): $rep искомое заменяемое\n$rep /\:cf:/ig <img src="http://1chan.ru/img/coolface.gif" />\n$rep /(ху[йияеё])/ig <font color="red">beep</font>')}
		}),
		$attr($btn('Uygula', function() {applyRegExp()}), {'style': 'float:right'}),
		$new('br', {'clear': 'both'}),
		$new('textarea', {
			'id': 'regexp_field',
		...
		$new('br'),
		optSel('prochidden_sel', ['Değiştirme', 'Birleştir', 'Kaldır'], 16, function() {
			processHidden(this.selectedIndex, Cfg[16]);
		}),
		$txt(' Saklı Pozisyonlar'),
		divBox(19, 'Başlığı görürken ayarları uygula.'),
		divBox(18, 'Ekstra menü tuşlarını gizle.'),
		divBox(17, 'Gizli maddeleri görmek için fareyle üzerinde gezdir.'),
		$new('hr', {'style': 'margin:5px 0'}),
		optSel('upload_sel', ['Kapalı.', 'Otomatik', 'Account-click', 'Tıklayarak'], 20),
		$txt(' Postları yükle, Her şu dakikada yeni cevaplar için = '),
		optSel('upload_int', [0.5, 1, 1.5, 2, 5, 15, 30], 21),
		$txt(' Dakika(lar)'),
		$new('br'),
		optSel('refprv_sel', ['Kapalı.', 'Önizlemesiz', 'Önizlemeli'], 24),
		$txt(' >>linklerde gezinme, '),
		chBox(25, 'Gecikme'),
		$New('div', [
			optSel('imgexp_sel', ['Yapma', 'Varsayılan', 'Ön İzlemeli'], 26),
			$txt(' Resimleri Genişlet.')
		]),
		$New('div', [
			$txt('Şu linkleri göm:'),
			chBox(30, 'YouTube Linkleri'),
			chBox(31, 'MP3 Linkleri'),
			chBox(32, 'Resim Linkleri')
		]),
		$if(!(ch._4ch && !(sav.GM || sav.script)),
			divBox(27, 'Yenilemeksizin post at (post kontrol)')),
		divBox(28, 'Hareketli Uyarı.'),
		divBox(29, 'Düz yazı tuşu.'),
		$if(!ch.dc, divBox(33, 'Saklı postları göster.')),
		$new('span', {'style': 'float:right; padding: 0 100px 115px 0; '
			+ 'background:url(http://img842.imageshack.us/img842/244/132543806049.png) no-repeat'}),
		divBox(34, 'Link sokmak için tıkla.'),
		divBox(35, 'Postlardaki isimleri sakla.', function() {toggleCfg(35); scriptStyles()}),
		$if(ch._2ch, divBox(36, 'Postlarda kaydırma yok.', function() {toggleCfg(36); scriptStyles()})),
		divBox(37, 'Tüm spoilerlar varsayılan olarak açık.', function() {toggleCfg(37); scriptStyles()}),
		$if(hasSage, divBox(38, 'E-posta bölümü yerine Sage tıkı.')),
		$if(pr.on, divBox(40, 'Yanıt kutusu aşağıda olsun.')),
		optSel('caplang_sel', ['Kapalı.', 'Tr', 'Rus'], 41),
		$txt(' '),
		$if(pr.on, $New('div', [
			optSel('txtbtn_sel', ['Kapalı.', 'Grafiksel.', 'Yazı.', 'Varsayılan.'], 22, function() {
				saveCfg(22, this.selectedIndex);
				$Del('.//div[@id="txt_btns"]');
				if(Cfg[22] != 0) {textFormatPanel(pr); if(qr.on) textFormatPanel(qr)}
			}), 
			$txt(' BBcode tuşları ( Yoksa, bilinçli kır ) '),
			chBox(23, 'aşağı ', function() {
				if(Cfg[22] != 0) {textFormatPanel(pr); if(qr.on) textFormatPanel(qr)}
			})
		])),
		$if(pr.name, $New('div', [
			$new('input', {'type': 'text', 'id': 'usrname_field', 'value': Cfg[44], 'size': 20}),
			chBox(43, ' Varsayılan isim', function() {
		...
		$if(pr.passw, $New('div', [
			...
			chBox(45, ' Varsayılan şifre', function () {
		...
		$if(pr.on, $txt('Gösterme: ')),
		$if(pr.rules, chBox(47, 'kurallar ',
			function() {$disp(pr.rules); if(qr.on) $disp(qr.rules)})),
		$if(pr.gothr, chBox(48, 'goto field ',
			function() {$disp(pr.gothr); if(qr.on) $disp(qr.gothr)})),
		$if(pr.passw, chBox(49, 'şifre ',
...
function hiddenPostsPreview() {
	...
	forAll(function(post) {if(post.Vis == 0) {
		...
		$append(table, [
			$if(((!pp && tcnt++ == 0) || (pp && pcnt++ == 0)), $new('tr', {
				'html': '<th><b>Saklı ' + (pp ? 'postlar' : 'Başlıklar') + ':</b></th>'
			})),
			$New('tr', [
				cln,
	...
	if(!table.hasChildNodes()) {
		table.innerHTML = '<tr><th>Saklı Başlıklar...</th></tr>';
		return;
	}
	$append(table.insertRow(-1), [
		$new('hr'),
		$btn('Hepsini Genişlet', function() {
			if(/все/.test(this.value)) {
				this.value = 'Undo';
				for(var cln, i = 0; cln = clones[i++];)
					setPostVisib(cln.pst, 1);
			} else {
				this.value = 'Hepsini Genişlet';
				for(var cln, i = 0; cln = clones[i++];)
...
function favorThrdsPreview() {
	...
	var table = $x('.//tbody', div);
	var data = getStored('DESU_Favorities');
	if(!data) table.innerHTML = '<tr><th>Favori Başlıklar...</th></tr>';
	else {
		table.appendChild($new('tr', {'html': '<th><b>Favori Başlıklar:</b></th>'}));
		var arr = data.split('|');
		for(var i = 0; i < arr.length/4; i++) {
			var host = arr[i*4];
			var b = arr[i*4 + 1];
...
function selectPostHider(post) {
	...
	var a = addSelMenu('phide_' + post.Num, 0, Cfg[29] == 0 ? 14 : 19,
		['Tüm başlığı saklamak için tuşa tıkla', 'Resmi saklamak için tıkla', 'Benzer yazıları saklamak için tıkla']);
...
function iframeLoad(e) {
	...
	if(!ch._4ch) try {
		...
		if(ks) {
			...
			$alert('Iframe error. Try <a href="' + lh + '">' + lh + '</a>', null, true);
...
function sageBtnFunc(obj) {
	...
	var mail = $prev($attr($x('.//span[@id="sage_btn"]', obj.form), {'html': c
		? '&nbsp;<span class="sage_icn"></span><b style="color:red">SAGE AKTİF</b>'
		: '<i>(sage&nbsp;yapmak&nbsp;için&nbsp;tıkla)</i>'}));
...
function formSubmit(obj) {
	$event($attr(obj.subm, {'value': 'Submit'}), {'click': function() {
		if(Cfg[27] == 1) {
			if(ch._4ch && (sav.GM || sav.script)) setStored('DESU_4chan_cache', 're');
			$alert('Post atılıyor...', 'wait');
		}
...
function textFormatPanel(obj) {
	...
	var btns = $New('div', [
		$if(Cfg[22] == 2, $txt('[ ')),
		tfBtn('Bold', '**', 'b', 'B', '', pre + pre2 +'WTYBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIazjIYbmEQII95E3JZD530ZzyajtwbUJHYjzekhPLc8LRE5/NZa+azXCTqdWDet1W46sQc20NhIRbhQ2HhXQOiIleiFSIdAuOioaQhQs9lZF5TI6bDJ2Ff02ODaKkqKyanK2whKqxsJsjKLi4Kgq8vb6/viIhADs=', tx),
		tfBtn('Cursive', '*', 'i', 'i', 'font-style:italic', pre + pre2 +'V5YBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIa0TI4XcsKpk9ZBHKcCSuWKwym3X0rFztIXz1VskJJQRtBofV7G9jTp8r6/g2nn7fz80Lfmp+cws9gXt9hIYMiHiKfoyOhIuHlJeSl5SGIyienioKoqOkpaQiIQA7', tx),
		$if(!ch.dc && !ch._410 && !ch.iich, tfBtn('Underlined', '__', 'u', 'U', 'text-decoration:underline', pre +'6CgoGhoaP' + pre3 +'PoCQtZGmepjg+bOu+7iIxD2LfeI4/DK3/Op4PSEQIazjIIbnMHXNKZrCHvEWtzV3Pkeh2IwdvAizuOrZlslctPjO4YvY4XHbD1/Rv3mtv+P1gEH9gf399hWARigeMhX5uC44NYIwQSpILPZGSnI6ZDJudop+hDYynqI1/pKKtrK2dmSMotLQqCri5uru6IiEAOw==', tx)),
		tfBtn('Strike-through', !ch._410 ? '' : '^^', 's', 'S', 'text-decoration:line-through', pre +'2hoaE1NTf' + pre3 +'NoCQtZGmepjg+bOu+7iIxD2LfeI4/DK3/Op4PSEQIazrIYQn5HXXL6KGZe+KUkIQWW+05tOAlWCseO7zjBDbNPjO+aog8Kq/XtW54en5g470NgYKDWIOBeYNLhoqGbguEU4KFhgs9j4lSBxGGgZUMl5BMnJ2Wo6aDnqCno6mrp5UjKLKyKgq2t7i5uCIhADs=', tx),
		tfBtn('Spoiler', '%%', 'spoiler', '%', '', pre + pre2 +'V7YBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIa0Xg0XZoOp9Q2xIBqVqvWGnPkUhgv9euY9sFm8Vkr/mLZnDV63Bi7G404lg73WGH+p96PQt2hIWGhguCh4uHiQyDjJENjpCSi5SWjJiZjQwjKKCgKgqkpaanpiIhADs=', tx),
		tfBtn('Code', "`", 'code', 'C', '', pre + pre2 +'WGYBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIa0Xg8Qc5OCGQYA+Jazqv0V3Pkeh2rd4ENJxwbMlNsrp8DjvXZDOD6z7Aw3JHY7938v+AeYBNgIUNcguDfnxQgAs9iYpXT46QhlYHjZUMkYaee4+cn6OhnaOFjyMoq6sqCq+wsbKxIiEAOw==', tx),
		tfBtn('Quote', '', '', '&gt;', '', pre + pre2 +'WEYBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIa7jDoWg75iAQZdGpg0p/Qkdiy+VaD92to6cNh7/dMaNsPke5anabq4TAyY28ft+oQ/ZxfHt+gmoLgn0HUIgNCz2Hg4p/jI2PfIuUeY4MkJmIm52efKCinwwjKKmpKgqtrq+wryIhADs=', tx),
		$if(Cfg[22] == 2, $txt(' ]'))
		], {
		'id': 'txt_btns',
		'html': '&nbsp;',
...
function insertYouTube(link, el) {
	if($x('.//embed[@src="' + link + '"]', el)) $delCh($1(el));
		...
		'" type="application/x-shockwave-flash" wmode="transparent" width="485" height="390" />');
}
...
function allImgExpander() {
	...
	var txt = '[<a>Resimleri Genişlet</a>]&nbsp;';
	$id('DESU_panel').appendChild($new('span', {
		...
		'click': function() {
			...
			btn.innerHTML = /Раhiddenь/.test(btn.innerHTML) ? '[<a>Close image</a>]&nbsp;' : txt;
		}}));
}
...
function showRefMap(post, rNum, isUpd, arr, tNum, b) {
	...
	if(!el) {
		...
		el = $new('div', {
			'class': 'rfmap',
			'id': 'rfmap_' + rNum,
			'html': '<br><FONT SIZE=2>   Cevaplar: ' + ref
		});
		doRefPreview(el);
		$after(msg, [el]);
	} else {
...
function showPostPreview(e) {
	...
	cln.innerHTML = '<span class="wait_icn">&nbsp;</span><span>&nbsp;Yükleniyor...</span>';
	if(post) {
		...
		functor(cln, err || ajaxPosts[tNum][pNum] || 'Post not found', true);
	});
	$del($id(cln.id));
...
function parseJSONdata(x) {
	...
	for(var i = 0, tLen = thrds.length; i < tLen; i++) {
		...
		for(var j = 0, pLen = posts.length; j < pLen; j++) {
			...
			ajaxPosts[tNum][pNum] = '<label><a class="delete icon"><img src="/images/blank.png"></a>' + (x.sage ? '<img src="/images/sage-carbon.png" alt="Сажа" title="Сажа">' : '') + (x.subject ? '<span class="replytitle">' + x.subject + '</span>' : '') + '<span class="postername">' + x.name + '</span> ' + x.date + ' </label><span class="reflink"><a href="/' + brd + '/' + res + tNum + '.xhtml#i' + pNum + '">No.' + pNum + '</a></span>' + (j == 0 ? '<span class="cpanel">[<a href="/' + brd + '/' + res + tNum + '.xhtml">Открыть Thread</a>]</span>' : '') + '<br>' + (x.files.length > 0 ? farr.join('') + (x.files.length > 1 ? '<br style="clear: both">' : '') : '') + '<div class="postbody"><div class="message">' + txt.join('<br>').replace(/(^|<br>)(%%<br>)(.+?)(<br>%%.*?)(<br>|$)/g, '<div class="spoiler">$3</div>').replace(/(^|<br>)(``<br>)(.+?)(<br>``.*?)(<br>|$)/g, '<pre>$3</pre>').replace(/quote><br>/g, 'quote>') + '</div></div>' + (x.op == true ? '<div class="abbrev">' + 'Всего ' + thrds[i].posts_count + ' Posts, из них ' + thrds[i].files_count + ' с файлами</div>' : '');
	...
	AJAX(true, brd, tNum, function(err) {
		if(err) {
			...
			if(last > 5 || last == 1) thr.appendChild($new('span', {
				'html': '[<a style="cursor:pointer">Сollapse Thread</a>]'}, {
...
function infoNewPosts(err, del) {
	if(Cfg[20] == 3) return;
	if(err) {
		$alert('Thread №' + oPosts[0].Num + ' not available:\n' + err);
...
function initNewPosts() {
	...
	if(Cfg[20] == 2 || Cfg[20] == 3)
		$after($x('.//div[@class="thread"]'), [$new('span', {
			'id': 'newpst_btn',
			'html': '[<a style="cursor:pointer">Получить новые posts</a>]'}, {
...
function hideThread(post, note) {
	...
	var x = $new('span', {
		...
		'html': 'Başlık <a style="cursor:pointer">№' + post.Num + '</a> saklı <i>('
			+ (!note ? getTitle(post).substring(0, 50) : 'otomatik gizli: ' + note) + ')' + '</i>'
	});
	$event($x('.//a', x), {
...
function mergeHidden(post) {
		...
		$before(post, [$new('span', {
			...
			'click': function() {
				...
				$prev(hDiv).innerHTML = 
					...
					+ '[<i><a>hiddenо:</a> ' + hDiv.childNodes.length + '</i>]';
				$disp(hDiv);
			}}
		), el]);
	}
	...
	if(!next || getVisib(next.id.match(/\d+/)) == 1)
		$prev(el).innerHTML =
			unescape('%u25B2') + '[<i><a>hiddenо:</a> ' + el.childNodes.length + '</i>]';
}
...
function applyRegExp(txt) {
	...
	if(wrong) {$alert('Error в ' + wrong); return}
	fld.value = val;
...
function findSameText(post, origPost, origVis, origWords) {
	...
	$del($id('note_' + post.Num));
	if(origVis != 0) hidePost(post, ' bununla aynı yazı >>' + origPost.Num);
...
function doScript() {
	...
	timeLog += '\n\nВсего: ' + endTime + 'ms';
	$id('process_time').textContent = 'Processing time: ' + endTime + 'ms';
}
...
function prepareLinks(parent) {
	var list = document.evaluate('.//a[img or span/img]', parent, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
function activateLink(a) {
	return hosts.some(function(host) {
		...
		a.setAttribute('mpiv-cfg', [pos > 0 ? url.substr(pos) : url, host.q, host.xhr].join('\t'));
		a.addEventListener('mousemove', placePreview, false);
		a.addEventListener('mouseout',  cleanUp, false);
		a.addEventListener('mousedown', cleanUp, false);
		return true;
	});
}

function onLinkMouseOver(e) {
	if(!e.relatedTarget || document.getElementById('mpiv-preview') || !activateLink(this)) return;
	var img = getPreview(this);
	var cfg = this.getAttribute('mpiv-cfg').split('\t');
	var url = cfg[0], q = cfg[1], xhr = cfg[2];
	setStatus('loading');
	if(q) {
		downloadPage(rel2abs(url, window.location.href), q, xhr, img.getAttribute('mpiv-t'));
	} else {
		if(xhr) {
			downloadImage(url, url, img.getAttribute('mpiv-t'));
		} else {
			setPreview(url);
			checkProgress(true);
		}
	...
	document.addEventListener('DOMMouseScroll', cleanUp, false);
	document.addEventListener('mousewheel', cleanUp, false);
	this.appendChild(img);
}

function cleanUp(e) {
	var rt = e.relatedTarget, t = e.target;
	if(rt && t && rt.parentNode && t.parentNode) {
		var rtp = rt.parentNode, tp = t.parentNode;
		if(rtp.tagName == 'A' && (rtp == tp.parentNode || rtp == tp) || tp.tagName == 'A' && tp == rtp.parentNode) return;
	}
	if(this.tagName == 'A') {
		this.removeEventListener('mousemove', placePreview, false);
		this.removeEventListener('mouseout',  cleanUp, false);
		this.removeEventListener('mousedown', cleanUp, false);
		this.removeAttribute('mpiv-cfg');
	}
	document.removeEventListener('keyup', onKeyUp, false);
	document.removeEventListener('DOMMouseScroll', cleanUp, false);
	document.removeEventListener('mousewheel', cleanUp, false);
	setStatus(false);
	setPreview(false);
}
...
function onKeyUp(e) {
	if(e.keyCode == 16) {
		var img = getPreview();
		if(!img) return;
		if(img.getAttribute('mpiv-full') || img.naturalWidth <= img.getAttribute('mpiv-cw') && img.naturalHeight <= img.getAttribute('mpiv-ch')) {
			img.removeAttribute('mpiv-full');
		} else {
			img.setAttribute('mpiv-full', true)
		}
		placePreview();
	}
}
...
function onNodeInserted(e) {
	var t = e.target;
	if(t.tagName == 'TABLE' || t.tagName == 'DIV')
		window.setTimeout(prepareLinks, 0, t);
	else if(t.tagName == 'A')
		window.setTimeout(prepareLink, 0, t);
}
...
function onUnload() {
	try {
		...
		var list = document.querySelectorAll('a.mpiv-ready');
		for(var i = list.length; i--;) {
			list[i].removeEventListener('mouseover', onLinkMouseOver, false);
		}
		hosts = null;
	} catch(ex) {
		GM_log('Error in onUnload(): ' + ex);
	}
}
	...
	GM_xmlhttpRequest({
		...
		onload: function(req) {
			try {
				...
				var img = getPreview();
				if(!img || img.getAttribute('mpiv-t') != t) return;
				if(xhr) {
...
function checkProgress(start) {
	...
	var img = document.getElementById('mpiv-preview');
	if(!img) {
		window.clearInterval(checkProgress.interval);
		return;
	}
...
function placePreview(e) {
	...
	var cx, y;
	if(e) {
		img.setAttribute('mpiv-cx', cx = e.clientX);
		img.setAttribute('mpiv-cy', cy = e.clientY);
	} else {
		cx = parseInt(img.getAttribute('mpiv-cx'));
		cy = parseInt(img.getAttribute('mpiv-cy'));
	}
	var cw = img.getAttribute('mpiv-cw');
	var ch = img.getAttribute('mpiv-ch');
	if(img.getAttribute('mpiv-full')) {
		img.style.maxWidth = null;
		img.style.maxHeight = null;
		img.style.left = -1 * Math.min(1, Math.max(0, 5/3*(cx/cw-0.2))) * (img.naturalWidth - cw) + 'px';
		img.style.top  = -1 * Math.min(1, Math.max(0, 5/3*(cy/ch-0.2))) * (img.naturalHeight - ch) + 'px';
	} else {
...
function showError(ex, t) {
	var img = getPreview();
	if(!img || t && img.getAttribute('mpiv-t') != t) return;
	setStatus('error');
	GM_log(ex);
}

function getStatus() {
	var img = document.getElementById('mpiv-status');
	if(!img) {
...
function setStatus(status) {
	var img = getStatus();
	if(status == 'loading') {
		img.src = 'data:image/gif;base64,';
	} else if(status == 'error') {
		img.src = 'data:image/gif;base64,';
	}
	if(status !== false) {
		window.addEventListener('mousemove', onStatusMouseMove, false);
		img.style.display = '';
	} else {
		window.removeEventListener('mousemove', onStatusMouseMove, false);
		img.parentNode.removeChild(img);
	}
}

function onStatusMouseMove(e) {
	var img = getStatus();
	img.style.left = e.clientX + 30 + 'px';
	img.style.top  = e.clientY + 30 + 'px';
}

function getPreview(parent) {
	var img = document.getElementById('mpiv-preview');
	if(!img && parent) {
		img = document.createElement('img');
		img.id = 'mpiv-preview';
		img.setAttribute('mpiv-t', Date.now());
		var de = document.documentElement;
		img.setAttribute('mpiv-cw', Math.min(window.innerWidth,  de.clientWidth)  - 2);
		img.setAttribute('mpiv-ch', Math.min(window.innerHeight, de.clientHeight) - 2);
		...
		img.addEventListener('mousedown', cleanUp, false);
...
function setPreview(src, parent) {
	var img = getPreview(parent);
	if(!img) return;
	if(src) {
		img.src = src;
		img.style.display = 'none';
	} else {
		img.removeEventListener('mousedown', cleanUp, false);
