...
var SCRIPT = {
	name: 'DWO',
	namespace: 'crohllf',
	source: 'http://userscripts.org/scripts/show/92404',
	identifier: 'http://userscripts.org/scripts/source/92404.user.js',
	meta: 'http://userscripts.org/scripts/source/92404.meta.js',
	version: '1.9.0',
	date: '2010-12-15',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=10050';
var postId = '39838957';
// Debug location, top of page 2:
// var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4415664&page=2';
// var postId = '39860346';
...
var listVersion = 0;
var listNames = ['family', 'apprentice', 'promoteme', 'waitlist', 'shitlist'];
var list = {
	family: {
		member: {},
		title: 'DWO Members',
	...
	(function() {
		...
		$.get(forumAccessURL, function(forumAccessData) {
			...
			$('#content div.content_container tbody:eq(0) tr:not(.nonalternating_color)', forumAccessData).find('td:eq(0) a').each(function() {
				...
				++members;
				if (list.shitlist.member[id]) {
					++membersShitList;
					console.warn('Shitlisted agent found on forum access list: ' + id + ' ' + realName + ' (listed as ' + names[id] + ')');
					delete names[id];
					names.num--;
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			if (membersShitList) {
				statusMsg += prefix + '<b style="color: white; background-color: #a03c19; padding: 0px 3px 0px 3px;">' + membersShitList + ' Shitlisted</b>';
				prefix = ', ';
			}
			if (unknown.length) {
