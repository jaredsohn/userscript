...
// @require        http://sizzlemctwizzle.com/updater.php?id=96447
// ==/UserScript==
...
-----------------------     
ThankYou a TODO (BOmmel): Add TransportTab
ThankYou a TODO (Hiro): Add WhisperAlertTab
Todo (Demons) : Add Edit
-----------------------     
History:

2011-02-04 (Version 0.2.0) Demons
   - Initial 
   - change structure to get "Templiers Tools" as Tab working with "Power Tools" and "Power Bot" 
   - add food info and fortification info to Advanced TowerAlert
2011-02-08 (Version 0.2.7) Demons
   - add board tab
   - change tab theme colors to ally colors
 
  - fix "Templiers Tools" tab to fit to tab menue 
2011-02-08 (Version 0.2.10) 
   - add adv.players tab (+oninestatus, +loginDate)   
**********************/

var Version = '0.2.11';
var Versiondate = '09.02.2011';
var Scriptname = 'KOC TempliersTools';
var ScriptTabName = 'Templiers Tools';
...
function pcStartup (){
  ...
  window.name = 'PT';
  logit ("* "+Scriptname +" v"+ Version +" Loaded");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  actionLog (""+Scriptname +" v"+ Version +" Loaded");
...
Tabs.board = {
  ...
  tabLabel : 'Forum',            // label to show in main window tabs
  myDiv : null,
  timer : null,
  forumlink :  'http://lestenpliers.xooit.fr',  
  
  init : function (div){    // called once, upon script startup
    var t = Tabs.board;
    t.myDiv = div;
    div.innerHTML = '<CENTER><iframe src="'+ t.forumlink +'" width="100%" height="550" name="board_in_a_box" <p>Ihr Browser kann leider keine eingebetteten Frames anzeigen: Sie k&ouml;nnen die eingebettete Seite &uuml;ber den folgenden Verweis aufrufen: <a href="'+ t.forumlink +'">Forum Des Templiers</a></p> </iframe> <BR></center>';
  },
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=pcentry><TABLE width=100% cellpadding=0>\
        ...
        </table><span style="vertical-align:middle;" id=pcaltInput></span></div>\
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class=xxtab>'+ t.dat[i][3] +'</td>\
            ...
            <TD align=center class=xxtab><DIV onclick="pcGotoMap('+ t.dat[i][5] +','+ t.dat[i][6] +')"><A>'+ t.dat[i][5] +','+ t.dat[i][6] +'</a></div></td>\
            <TD align=right class=xxtab style="padding-right:20px;">'+ t.dat[i][8].toFixed(2) +'</td><td><SPAN onclick="PCpo(this, \''+t.dat[i][9] +'\')"><A>onlinestatus</a></span></td><td><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>last Login</a></span><td></tr>';
    }
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=pcstat><TABLE id=tabAllMembers cellpadding=10  cellspacing=2 width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
      ...
      <TABLE id=tabAllMembers align=center cellpadding=2 cellspacing=2 width=100%><THEAD>\
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PCalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
        ...
        <TD><A><DIV>Status</a></div></td><TD><A><DIV>Last Login</a></div></td></tr></thead>\
...
function sendEmail (username, token, email, subject, msgpart1, msgpart2, msgpart3, msgpart4, msgpart5, msgpart6, msgpart7, msgpart8){  
    ...
    new MyAjaxRequest( "http://lestenpliers.xooit.fr" + query, {
	      method: "POST",
	      parameters: params,
	      onSuccess: function (rslt) {
