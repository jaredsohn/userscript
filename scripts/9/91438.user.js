// ==UserScript==
// @name TW Pro (hun)
// @description Beépül a felszerelések menüpontba és segít felöltöztetni a karaktered a kívánt munkához, közvetlen link a TW-DB.info weboldalhoz, Kereskedő button
// @author NEXTON
...
// Kiegészítések 2010.11.27. (xside):
// kereskedő button
// tw-db.info button
// Kiegészítés 2010.12.01. (dani0808):
// Kereskedő és tw-db button felcserélése


function init() {
  /* Load Itemtrader to refresh offering */
  AjaxWindow.show('item_trader');
...
function addButton() {
  var menuElem = new Element('div',{'id':'menu_item_trader', styles:{background:'url(http://noob.hu/2010/11/27/kereskedo.jpg) -0px 0px'}});
  menuElem.innerHTML = '<a href="#" onclick="AjaxWindow.show(\'item_trader\');">' +
...
// tw-db.info button start

function getItemTWdbVersion() {
	return "1.0";
}
...
function addButton() {
  var menuElem = new Element('div',{'id':'tw-db.info', styles:{background:'url(http://xside.co.cc/script/TheWest/images/twdb_left.jpg) 0px 0px'}});
  menuElem.innerHTML = '<a href="http://tw-db.info/index.php?lang=hu" target="_blank">' +
                       '  <img src=\"images/main/menu_highlight.png\" style=\"filter:alpha(opacity=50);-moz-opacity:0.5;-khtml-opacity:0.5;opacity:0.5;\"/>' +
                       '  <span>TW-db.info</span>' + '</a>';
  menuElem.injectAfter($('menu_settings'));
  var marginTop =  parseInt($('workbar_right').getStyle('margin-top')) + 27;
  $('workbar_right').setStyle('margin-top', marginTop + 'px');
}
var ItemTWdbFunctions = ['init', 'addButton']; 
var ItemTWdbScript = document.createElement('script');
ItemTWdbScript.type='text/javascript';
ItemTWdbScript.text =  'if(window.ItemTWdb == undefined) {\n';
ItemTWdbScript.text += '  window.ItemTWdb = new Object();\n';

for (var i = 0; i< ItemTWdbFunctions.length; i++) {
  var itFunction = ItemTWdbFunctions[i];
  ItemTWdbScript.text += '  ItemTWdb.' + itFunction + ' = ' + eval(itFunction.toString()) + '\n';
};
ItemTWdbScript.text += '  ItemTWdb.init();\n';
ItemTWdbScript.text += '}';
document.body.appendChild(ItemTWdbScript);

// tw-db.info button end
...
(function(){
	var twpro_lang = {
	info: ['OFFLINE verzió +ikonokkal (xSide)', 'http://xside.co.cc/script/TheWest/tw_pro_hun_offline.user.js'],
	WEBSITE: 'Weboldal',
	AUTHOR: 'Szerző',
	TRANSLATOR: 'Kiegészítés',
	TWPRO_DISABLED: 'szkript kikapcsolva',
	SORTBYNAME: 'Rendezés <b>név</b> alapján',
	SORTBYXP: 'Rendezés <b>tapasztalat</b> alapján',
	SORTBYWAGES: 'Rendezés <b>munkabér</b> alapján',
	...
	FILTERJOBS: 'A nem végezhető munkák elrejtése',
	FILTERCLOTHING: 'Csak a legjobb ruhákat mutassa a választott munkához',
	CHOOSEJOB: 'Válassz munkát',
	CALCJOB: 'Kis türelmet...',
	INVENTSTATS: 'Felszerelés statisztika',
	SELLVALUE: 'Eladási árak',
	EQUIPMENT: 'Felvett ruhák',
	BAG: 'Táska tartalma',
	OBJECTS: 'Tárgyak:',
	PRODUCTS: 'Termékek',
	TOTAL: 'Összesen',
	QUANTITIES: 'Darabszámok',
	HATS: 'Fejfedők:',
		NECKLACES: 'Nyakpántok',
		WEAPONS: 'Fegyverek',
		CLOTHING: 'Ruházatok',
		SHOES: 'Cipők',
		ANIMAL: 'Nyerges állatok',
		NONYIELDS: 'Egyebek',
		YIELDS: 'Termékek',
	LOADING: 'Töltés...',
	LABORP: 'MP',
	CONSTRUCTION: 'Építés',
	HPTOTAL: 'Teljes életerő',
	STARTCALC: 'Kattints ide...',
	CONVERT: 'Konvertálás',
	MULTISEL: 'Kombinált termékeladás',
	SELL: 'Kiválasztottak eladása',
	CONFIRMSELL: 'Valóban el szeretnél adni %1 darab terméket?',
	SELLING: 'Eladás...',
	SALEDONE: 'A kiválaszott termékeket sikeresen eladtad',
	NONESELECTED: 'Legalább egy dolgot ki kell választanod!',
	JOBRANKSETTINGS: 'Munkarangsor beállítása',
	SEARCHINVENTORY: 'Keresés...',
	NOSEARCHRESULT: 'A(z) %1 keresése nem hozott eredményt.%2Összes felszerelés mutatása%3',
	SEARCHHELP: 'Keresés a felszerelések között.\n A behelyettesítő karakterek használata megengedett\n ( a "*" tetszőleges számú karaktert helyettesít be,\n a "?" egyetlen karaktert helyettesít be)',
	APPLY: 'Elfogad',
	HEALTH: 'Életerő pont',
		SKILLFINGER: 'Maximum Fine motor skills',
	DUELSHOOTINGATT: 'Párbaj-lőfegyver (támadó)',
	DUELSHOOTINGDEF: 'Párbaj-lőfegyver (védekező)',
	DUELVIGOR: 'Párbaj-ütőfegyver',
	FORTATTACK: 'Erődharc (támadó)',
	FORTDEFEND: 'Erődharc (védekező)',
	SKILLRIDE: 'Lovaglás'
	};
	var twpro_langs = {};
	twpro_langs.de = {
	...
	*/
		...
		function twpro_insertList(twpro_data) {
			...
			var twpro_xhtml = '<table id="twpro_jobDisplay" style="display: inline;position: absolute; visibility:hidden; margin-left:-2px;"><tr><td>';
