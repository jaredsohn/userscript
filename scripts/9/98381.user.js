	...
	function createTweak() {
		...
		LT_strings = {
			...
			"es": {
				...
				"chat_btn_city": "Город",
				"up_count_tt": "Доступно зданий для улучшения:",
				"chat_btn_player": "Игрок",
				"chat_btn_alliance": "Альянс",
				"chat_btn_url": "Ссылка",
				"copy_coords_btn": "Координаты в чат",
				"up_cout_tt": "Здания, доступные для повышения уровня:",
...
/* farm */				"50": {"w": [ 50, 200, 400, 1400, 3500, 6000, 10000, 16000, 25000, 38000 ],"s": [ 0, 0, 200, 600, 1500, 3000, 5000, 8000, 13000, 20000 ]},
			...
			members: {
				...
				initialize: function() {
					...
					if (!/en|de|es|pl|pt/.test(this.lang)) this.lang = "en";
		...
		qx.Class.define("louTweak.layoutWindow", {
			...
			construct: function() {
				...
				this.win.setWidth(450);
				this.win.setHeight(275);
				...
				// Page 1
				this.ssTa1 = new qx.ui.form.TextArea("").set({height: 65});
				this.ssTa1.addListener("click", function(){this.selectAllText();});
				page1.add(this.ssTa1);
				this.ssTa2 = new qx.ui.form.TextArea("").set({height: 65});
				this.ssTa2.addListener("click", function(){this.selectAllText();});
				page1.add(this.ssTa2);
				...
				createExpImpWindow: function(t) {
					...
					btn.addListener("click", function() {
						...
						if (id == 1) {
							...
							if (typeof obj == "object" && obj != null) {
								LT.main.layoutWindow.cityLayouts = qx.util.Json.parse(txt);
								localStorage.setItem("LT_cityLayouts", txt);
								this.close();
							} else {
								alert(L("opts").import_invalid);
							}
				...
				updateContinent: function() {
					...
					if (LT.a.visMain.mapmode == "r") {
						...
						if (qx.lang.Object.isEmpty(v)) {
							...
							dp_object = region_object.DP;
							if (dp_object == null || dp_object == undefined) return;
							d_object = dp_object.d;
							if (d_object == null || d_object == undefined) return;

							for (var n in d_object) {
								data_table = d_object[n].EIB;
