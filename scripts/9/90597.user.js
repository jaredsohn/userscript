// ==UserScript==
// @name            The West - TW Pro FR 1.29 [SOM]
// @namespace       http://userscripts.org/scripts/show/90597
// @description     Script for The-West: TW Pro FR 1.29 [SOM - Scripts-O-Maniacs] [Multilingual] (v1.002a)
...
// @version         1.002a
//
// @history         1.002a Vérification de la version de TW incluse dans le script lui-même (TW Pro FR 1.29 & 1.30 peuvent à présent être installés en même temps sur tous les serveurs, il ne sera donc pas nécessaire de jouer avec les autorisations des scripts pendant la période de transition)
...
var myScript = "function twpro_version_old(){return '"+twpro_version()+"';}";
...
twpro_langs.fr = {
	info: ['Zyphir Randrott', 'mailto:zyphir.randrott@gmail.com', 4145, '.1.6.'],
	//WEBSITE: 'Website',
	AUTHOR: 'Auteur',
...
*/
	...
	function twpro_injectScript() {
		...
		twpro_supportLink.href = 'http://userscripts.org/scripts/show/90597';
		twpro_supportLink.target = '_blank';
		twpro_supportLink.appendChild(document.createTextNode('v'+twpro_version_old()));
		...
		$(twpro_supportAuthor.id).addMousePopup(new MousePopup('Nexton<br>Lekensteyn<br>Sandevil<br>Pedro Ramirez<br>Zyphir Randrott'));
	...
	function twpro_insertList(twpro_data) {
		...
		twpro_xhtml += '<div style="position:absolute;margin-top:25px;left:-100px"><a href="javascript:" onclick="TWPro.twpro_showsleepy()"><img id="twpro_sleepyonly" alt="" src="images/transparent.png" width="20" height="17" style="background-image:url(data:image/png;base64,'+sortSchlafImg+');background-position: 0px -17px" onmouseover="this.style.backgroundPosition=\'-0px 0\'" onmouseout="this.style.backgroundPosition=\'-0px -17px\'"/></a></div>';
		...
		twpro_xhtml += ' <input id="twpro_jobsort_filter" type="checkbox"' + (TWPro.twpro_preference('Hide_unjobs')?' checked="checked"':'') + '  onclick="TWPro.twpro_preference(\'Hide_unjobs\',this.checked);TWPro.twpro_clickfilterList()"/> <input id="twpro_clothingfilter" type="checkbox"' + (TWPro.twpro_preference('dispJobClothingOnly')?' checked="checked"':'') + '  onclick="TWPro.twpro_preference(\'dispJobClothingOnly\',this.checked);TWPro.twpro_bagVis()" />';
		...
		// settings for job rankings
					document.getElementById('window_inventory_title').innerHTML=document.getElementById('window_inventory_title').innerHTML+'<div style="margin-left:450px; margin-top:10px; font-family:Arial, Verdana, sans-serif; font-size: 13px; font-weight: normal;">'+TWPro.lang.JOBRANKSETTINGS+':</div>';

		// twpro_xhtml = '<div style="left:20px;position:absolute;margin-top:0px;">'+TWPro.lang.JOBRANKSETTINGS+': ';
		twpro_xhtml = '<div style="left:367px;position:absolute;margin-top:0px;">';
		...
		twpro_xhtml += '<div style="position:absolute;margin-top:390px;left:150px"><label style="position:absolute;top:4px;left:21px;font-style:italic;color:#636;cursor:pointer" for="twpro_searchinventory">'+TWPro.lang.SEARCHINVENTORY+'</label><input type="text" style="width:150px;background:#fff url(data:image/png;base64,'+sortButtonImg2+') no-repeat scroll 0 0;padding:0 2px 0 19px" id="twpro_searchinventory" onfocus="previousSibling.style.display=\'none\'" onblur="if(this.value==\'\')previousSibling.style.display=\'block\'" onkeyup="TWPro.twpro_searchInventory(event.keyCode==13)" /><span id="twpro_search_help" style="font-weight:bold;color:#191970;cursor:help;background:#D4C7B0;width:20px;height:17px">?</span></div>';
//		twpro_xhtml += '<input id="twpro_jobsort_filter" type="checkbox"' + (TWPro.twpro_preference('Hide_unjobs')?' checked="checked"':'') + '  onclick="TWPro.twpro_preference(\'Hide_unjobs\',this.checked);TWPro.twpro_clickfilterList()" style="position:absolute;margin-top:53px;left:450px"/>';
...
ScriptUpdater.check(90597, twpro_version());