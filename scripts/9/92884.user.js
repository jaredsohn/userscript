...
// @description Available for all Language "(v9.9.6)"
// @version     9.9.6
...
var version = '9.9.6';
var scriptP = 'http://userscripts.org/scripts/show/63218';
...
switch (lang) {case '.com.tr:
setupFile = ['Büyük Ikonlar',
             'Pazar Filtresi',
             'Ne Kadar Hammade Lazim',
             'Linkler',
             'NPC Saticisi',
             'Koy Liste Ikonlari',
             'Arama çubugu',
...
SEVFile =   ['Kaydet', 'Cikis', 'Versiyon'];
Searchfile= ['Oyuncular', 'Köyler', 'Birlikler', 'Saldiran', 'Savunan', 'Kahraman', 'Ara'];
Linksfile=  ['Links', 'Link URL', 'Link adi', 'Link Ekle', 'Hepsini Sil'];
Allyfile =  ['Birlik', 'Login'];
Notefile =  ['Notalar', '- Kaydet -'];
bigIfile =  ['Pazar Yeri', 'Kisla', 'Askeri Üs', 'Akademi', 'Ahir, 'Tamirhane'];
Buildfile=  ['Insaat bu kadar sure icinde bitecek'];
Spilerfile= ['Toplam:'];
battlefile= ['Kayip:', 'Saldiran:', 'Savunan:', 'Tam cömertlik'];
var StyleX= ['text-align: left;', ' 5px 25px 25px 5px;']
};


switch (lang) {case '.com.sa':case '.com.eg':case '.ae':
setupFile = ['عرض: قائمة المباني',
             'عرض: خصائص السوق',
             'عرض: الموارد المطلوبة',
             'عرض: قائمة الروابط',
             'عرض: خصائص تاجر المبادلة',
             'عرض: خصائص القرى',
             'عرض شريط البحث',
             'اظهار مجموع الموارد و الأنتاج'];
             
SEVFile =  ['حفظ', 'خروج', 'الأصدار'];
...
setupFile = ['Показывать: БОЛЬШИЕ иконки',
             'Показывать: Фильтр на рынке',  
             'Показывать: Необходимые ресурсы',  
             'Показывать: Ссылки',  
             'Показывать: NPC Торговца',  
             'Показывать: Иконки в списке деревень',  
             'Показывать: Панель поиска',  
             'Показывать: Всего ресурсов'];  

SEVFile = ['Сохранить', 'Выход', 'Версия'];
Searchfile= ['Игроки', 'Деревни', 'Альянсы', 'Атакующие', 'Обороняющиеся', 'Герои', 'Поиск'];
Linksfile= ['Ссылки', 'URL ссылки', 'Название ссылки', 'добавить ссылку', 'удалить всё'];
Allyfile = ['Альянс', 'Логин'];
Notefile = ['Заметки', '- сохранены -'];
bigIfile = ['Рынок', 'Казарма', 'Пункт сбора', 'Академия', 'Конюшня', 'Мастерская'];
Buildfile= ['строительство закончится в:'];
Spilerfile= ['Всего:'];
battlefile= ['Потери:', 'Атакующий:', 'Обороняющийся:', 'Всего награблено:'];
var StyleX= ['text-align: left;', ' 5px 25px 25px 5px;']
} //Russian (Ukranian)
...
ID("ltimeWrap").innerHTML+=''+
                                                ...
                                                '&nbsp;<img style="cursor:pointer;" id="NoteBook" src="http://img828.imageshack.us/img828/3112/notepadicon.gif" />'+
...
if (Vlist !=null) {
  ...
  '<tbody><tr><td><button id="X" style="text-align: center;">'+SEVFile[1]+'</button></td></tr></tbody></table>'; GM_addStyle(GM);
  for (var X=0;X<JS;X++){
  ...
  var tempA = document.createElement("span"); tempA.innerHTML = testA;  
...
var icon = '<table cellspacing="1" style="border: 1px solid silver; width: auto; -moz-border-radius: 0px 0px 5px 5px;"><tbody>'+
               ...
               '<img title="'+bigIfile[0]+'" src="http://img412.imageshack.us/img412/1706/g17.gif"></a></td>'+
               '<td><a href="build.php?gid=20">'+
               '<img title="'+bigIfile[4]+'" src="http://img713.imageshack.us/img713/5306/g20.gif"></a></td>'+
               '</tr>'+
               '<tr>'+
               ...
               '<img title="'+bigIfile[1]+'" src="http://img26.imageshack.us/img26/6854/gid19.png"></a></td>'+
               '<td><a href="build.php?gid=21">'+
               '<img title="'+bigIfile[5]+'" src="http://img835.imageshack.us/img835/2360/g21.png"></a></td>'+
...
var newImg = CELM('IMG',[['src',"http://img227.imageshack.us/img227/4307/messk.png"],['style',"cursor:pointer"]]);
...
for (vn in villages) {
  ...
  villages[vn].innerHTML += " <td><a href='a2b.php?z=" + myVid + "'><img src='http://img835.imageshack.us/img835/672/mret.gif'/></a></td> ";
