...
function organizefreegiftlinks(){
         ...
         for (var i=0, il=linkname.length; i<il; i++) {
               ...
               bagstats[currentGift].link != ' ' ? bagstats[currentGift].link += bagstats[currentGift].count+' : '+freegiftlink2(currentGift,linkurl[0]) + '<br>' : bagstats[currentGift].link = '';
...
function acceptAllLink(loot) {
      return '<a href="#" tag="acAll" loot="'+loot+'" >[EOD Accept All]</a>';
   }
function freegiftlink2(tag,link) {
      return '<a tag="'+tag+'"href="'+link+'" target="_blank" onclick="this.innerHTML=\'Visited Link\';">[EOD Getz Link]</a>';
   }
