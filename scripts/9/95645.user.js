// ==UserScript==
// @name           TW Massa-recruiter
// @description    TW Massa-recruiter
// @author         Michael Richter, Vertaler: burnacid
// ==/UserScript==

// -----------------------------------------------------------------------------
// 20100918: fixed for Account Sitting
// 20110128: attempt to fix for V7.0
(function(){
	...
	var updateWorkers = function() {
		...
		el.textContent = '(تستطيع تدريب '+neededworkers+'في هذه القرية )';
	}
	
	// get available groups
	var groups = [];
	var selectedgroup = '';
	var grps = $x('//td/a[contains(@href,"&screen=train&mode=mass") and contains(@href,"&group=") and not(contains(@href,"&page=")) and starts-with(.," [")]');
	if(grps.length > 0) {
	...
	var recform = $x('//form[contains(@action,"screen=train&mode=mass&") and contains(@action,"action=train_mass&h=")]');
	if(recform.length == 0)
		return;
	...
	var h3 = ce('h3');
	h3.textContent = 'تدريب مجموعة : »'+selectedgroup+'«';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'أدخل القوات';
	th.title = 'De gewenste troepen voor het voltooide dorp.';
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'تدريب المزيد من :';
	th.title = 'Wanneer het gewenste troepenaantal bereikt word, zal er verder gerekruteerd worden, tot de boerderij vol is.';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	for(var i = 0; i < units.length; i++) {
		var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'حفظ مكان في المزارع :';
	th.title = 'Hoeveel bevolking er niet gebruikt moet worden.';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	...
	var tr = ce('tr');
	var th = ce('th');
	th.textContent = 'حفظ موارد :';
	th.title = 'Hoeveel grondstoffen er bewaard blijven.';
	th.style.textAlign = 'right';
	tr.appendChild(th);
	var td = ce('td');
	td.setAttribute('colspan', units.length);
	...
	for(var i = 0; i < 3; i++) {
		...
		if(i == 0) {
			...
			img.title = img.alt = 'Hout';
		} else if(i == 1) {
			img.src = 'graphic/lehm.png';
			img.title = img.alt = 'Leem';
		} else if(i == 2) {
			img.src = 'graphic/eisen.png';
			img.title = img.alt = 'Ijzer';
		}
	...
	var btn = ce('button');
	btn.setAttribute('style', 'font-size:8pt;');
	btn.textContent = 'إدخال القوات';
	btn.addEventListener('click', function(evt){
		// calculate and fill
		evt.target.disabled = true;
		evt.target.textContent = 'Bezig...';
		var currentsettings = getCurrentSettings(selectedgroup);
		var rows = $x('//form[contains(@action,"screen=train&mode=mass&") and contains(@action,"action=train_mass&h=")]/table[1]/tbody/tr/td/parent::tr');
		var builtsum = [];
		for(var i = 0; i < units.length; i++) {
			builtsum[i] = 0;
		}
		...
		document.getElementById('massrecruitbuilt').textContent = 'عدد القوات ' + builtsum.join(', ') + ' في  ' + builtvilnum + ' قرية';
		evt.target.textContent = 'جاري';
