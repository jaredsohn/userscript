// ==UserScript==
// @name           Blip.fm Download Music YT
// @description    Add to Blip.fm an easy DOWNLOAD button next to the NEXT button on page bottom. Enjoy!
...
function isPlaying(action, args) {
	...
	var url = args[0].url;
	if (args.length && url.indexOf('http://') == 0) {
		showDownloadButton(args[0].id, url);
	} else {
		showDownloadButton(args[0].id, "http://www.youtube-mp3.org/get?video_id=" + url);
	}
}
...
function showDownloadButton(musicID, musicURL) {
	...
	var newHTML = controls.innerHTML + '<span id="dDownload"> | <a target="_blank" href="'+musicURL+'">download</a></span>';
