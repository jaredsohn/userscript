...
// @namespace      editado por Jean | uid=9672668406679258824
...
|			                                              |
|	
|     este script foi criado por joseph,e editado por Jean Mendes     |
|						                      |
+---------------------------------------------------------------------+
*/
...
var colocaNaPagina = function() {
	...
	function online() {
		...
		var texto = String("[b]Che [Blue]Gay![/Blue][/b]").concat("\n\n[silver]", String(Math.random()), "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function offline() {
		...
		var texto = String("[b]To Vazando [red]Xaus![/red][/b]").concat("\n\n[silver]", String(Math.random()), "[/silver]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent(""),
