// ==UserScript==
// @name           HAKAN.-MM- OTO-Saldırı 1
// @namespace      HAKAN.-MM- OTO-Saldırı 1
// @description    HAKAN.-MM- OTO-Saldırı 1 Full Türkçe Scriptidir
...
ShowOptionsDialog:function() {
	...
	div.innerHTML="<form><table>"+
		...
		"Attack Order: <select id='KOCAttackOrder'><option value='mostTroops'>Önce Yakındakilere</option><option value='closest'>Asker Sayısına Göre</option></select><br />"+
		"Attack Type: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbar "+
		...
		"ABarbar Saldırı Zamanı:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />Saat<br />"+
		...
		"</div>"+
		"<br />"+
		"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' />esafeye Kadar Saldırı Yapabilsin...<br />"+
		...
		"Bu Şehirler de<input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> beklenir ve "+
		"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' />Saniye sonra sayfa yenilenir...<br />"+
		...
		"<a id='KOCAttackOptionsSave' class='button20'><span>KAYDET</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>ÇIK</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Seçenekleri SIFIRLA..</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Fabrika Ayarlarına DÖN..!</span></a></form>";
	var t=this;
...
//	if(autoAttack && autoAttack.cities && autoAttack.cities[t.GetCurrentCityId()]) {
	if(autoAttack) {
		toggle.innerHTML='<span>OTO-AÇIK</span>';
	} else {
		toggle.innerHTML='<span>OTO-KAPALI</span>';
	}
},
...
BulkAddAttackLink:function(box) {
	...
	AddHtml(bulkAddDiv,"Lord Evren_Serdar.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Yanlış Koord. Hatalarına Karşı Mutlaka İŞARETLENMELİDİR.!<br />");
	...
	bulkAdd.innerHTML='<span>HAFIZAYA AL.!</span>';
...
HideAttackEfforts:function() {
	...
	var a=document.createElement('a');
	a.innerHTML='GZLE/GÖSTER -Saldırı Efektlerini..';
	//a.href='javascript:;';
...
OnAttackBoxAppear:function(box) {
	...
	AddText(div2,'Yapacağınız Saldırının HAFIZAYA eklenmesini İstemiyorsanız İŞARETLEYİNİZ..!!');
