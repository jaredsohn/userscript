// ==UserScript==
// @name		Facebook Palit Accounts
// @namespace	FSA
// @auther		http://facebook.com/dars.bernabe
...
FB_addStyle(
		'#fsa-account { position:relative; display:list-item; }'+
		'#fsa-account:hover ul, #fsa-account:hover li { display:block; }'+
		'#fsa-account ul a, #fsa-account ul a:focus { background:white; }'+
		'#fsa-account-list { overflow:auto; }'+
		'#fsa-account img { background:url(/rsrc.php/z8S5R/hash/ez3x5cuc.png) no-repeat -137px 0; height:4px; left:5px; position:relative; top:-2px; width:7px; }'+
		...
		'#fsa-account ul a {color:#3a579a;display:block;font-weight:normal;height:auto;_margin-right:-25px;padding:4px 10px 5px;white-space:nowrap;*white-space:auto;_white-space:nowrap}'+
		'#fsa-account ul a:hover {background:#6d84b4;border-bottom:1px solid #3b5998;border-top:1px solid #3b5998;color:#fff;padding:3px 10px 4px}'+
		'#fsa-account li {display:block;float:none}'+
...
FB_addScriptUrl('http://fb-hataw-palit-accounts.googlecode.com/files/json2.js');
FB_addScriptUrl('http://fb-hataw-palit-accounts.googlecode.com/files/fsa_setting.js');

//FB_addScriptUrl('http://127.0.0.1/fsa_setting.js');
//============================================================================================================================
// Start Defind
//============================================================================================================================
var IMG = {
...
var set_default = {"sel_language":"en_us","chk_update":false,"chk_login_home":false,"chk_login_auto":false,"ipt_lock_pass":"39bc1c02846bc8749a79ee2b65d51e5f","ipt_custom_title":"Switch","chk_use_custom_title":false,"chk_show_user_name":true}
...
var TIME = {},$FSA = {},FSA = new Object(), fuid_uarr = new Array(),language_select,noLogin = false,fsa_json = {"language_select":{"en_us":"English"}},fblocation = getValue('fblocation', WURL);
var face_lang = {
	en_us :	{
		...
		'ALT_PASS_DIFF'			:'Inconsistent %s, please enter agein!',
		'ALT_ERROR'				:'%s Wrong！',
...
if( document.getElementsByTagName('body')[0].className.indexOf('Locale') != -1 ){
	if( $('#pageNav').size() > 0) {
		...
		var $LOGIN = $('#login_form').find('input[type="submit"]');
		var p = $LOGIN.position();
	...
	$FSA['fb_pn'].append('<li id="fsa-account"><a href="#" onclick="return false;" id="fsa-account-link" style="padding-right:16px;"><span>'+ $T('TITLE') +'</span><img src="http://static.ak.fbcdn.net/rsrc.php/z12E0/hash/8q2anwu7.gif" class="img" style="background:url(\'/rsrc.php/z8S5R/hash/ez3x5cuc.png\') no-repeat scroll -137px 0 transparent; height:4px; left:5px; position:relative; top:-2px; width:7px;"></a>'+
	...
	FSA.Methods = {
		...
		LoginFacebook:function(id){
			...
			var Content = '<form method="POST" action="https://www.facebook.com/login.php?login_attempt=1" id="switch_login_form">'+
						...
						'<input type="hidden" autocomplete="off" id="non_com_login" name="non_com_login" /><input type="submit" class="inputsubmit" id="login" name="login" value="Login" />'+
						'</div></form>';
			...
			window.setTimeout(function(){ $ById('switch_login_form').submit() },1000);
		},
		...
		DialogBox:function(title,bodyContent,okFunction,cancel){
			...
			var Msg = '<div class="generic_dialog_popup" style="top: 125px;">'+
				...
				'</div>'+
			'</div>';
			$("body").append('<div id="div_Box" class="generic_dialog pop_dialog">'+Msg+'</div>');
		...
		About:function(){
			...
			'<div style="width:420px;" id="version">'+$T('LAB_NEW_VERSION',fsa_json.version) +'<br />'+ $T('LAB_NEW_UPLOAD',fsa_json.time)+'</div>'+
			'<div style="width:420px;">Script Page：<a href="http://userscripts.org/scripts/show/98186">http://userscripts.org/scripts/show/98186</a><br/>'+
			'<p>'+$('#data_FSA_ABout').html()+'</p></div>';
    ...
    TIME['load_timeout'] = 0;
	TIME['load'] = window.setInterval(function(){
		if( $('#data_fsa_json').size() > 0 || TIME['load_timeout'] > 10){
			fsa_json =  $.parseJSON( $('#data_fsa_json').html() );
			face_lang = $.parseJSON( $('#data_face_lang').html() );
			FSA.LanguageChange(language)
			timeout = 0;
			$.each( fsa_json['language_select'] ,function(lan,lanf){
				language_select += '<option value="'+lan+'">'+lanf+'</option>';
			})
			clearInterval( TIME['load'] );
    ...
    $("#FSA_BTN_ABOUT",$FSA['fb_pn']).live('click',function(){ FSA.About(); },false);
	...
	$("#FSA_BTN_EDIT_ACCOUNT",$FSA['fb_pn']).live('click',function(){
		var Content = '<table width="100%" id="account_edit"><thead>'+
	...
	$("#FSA_BTN_SETTING",$FSA['fb_pn']).live('click',function(){
		...
		FSA.DialogBox( $T('BTN_SETTING') ,Content,function(){
			if(! $("#FSA_CHK_USE_CUSTOM_TITLE").attr("checked")  ){
			...
			$.each(set_var, function(i,x){
				...
				switch (key.substr(0,3)){
					...
					case "CHK":	setting[x] = $("#FSA_"+key ).attr("checked");break;
		...
		$.each(setting, function(key,val){
			...
			switch (key.substr(0,3)){
				...
				case "CHK":	$("#FSA_"+key ).attr("checked",val);break;
...
function checkUpdate(){
	if(setting.chk_update && parseFloat(version)*10 < parseFloat(fsa_json.version)*10 ){
		var Content = $T('LAB_NEW_UPDATA')+'<br><a href="http://userscripts.org/scripts/show/98186">http://userscripts.org/scripts/show/98186</a>';
		FSA.DialogBox( $T('LAB_NEW_VERSION', $T('LAB_VERSION',fsa_json.version)) ,Content,function(){
			window.top.location.href = 'http://userscripts.org/scripts/show/98186';
		})
	}
}