...
// @description    Enables direct streaming of videos from 2shared as well as from megaupload and megaporn via www.icedivx.com. Plus, some browsers get the added feature of bypassing the mega captcha. This script works closely with the high quality video links database www.icefilms.info
...
}else if(location.host.match('dankfile.com')){
    var pagecontent = document.body.innerHTML;
	var bad =pagecontent.search("The file you were looking for could not be found, sorry for any inconvenience.");
;
    if (bad > 0 && !document.getElementById('iceVerify')){
    iceVerify();
	}
    function iceDankeEmbed(){
         if (document.getElementById('np_vid')){
        downloadlink = document.getElementById('np_vid').src;
        }
        
	    iceEmbed();
        }

        setTimeout(iceDankeEmbed() ,1000);
