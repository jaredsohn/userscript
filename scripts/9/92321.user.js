...
// @description    Script publicado por Coolbum67 traducido al español por Pedro Vico
// @require        http://sizzlemctwizzle.com/updater.php?id=92321
// ==/UserScript==
...
var Options = {
  ...
  enableFoodWarn : true,
  foodWarnHours : 24,
  alertConfig  : {aChat:false, aPrefix:'** Me Atacan!!!! **', scouting:false, wilds:false, minTroops:1, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  GM_addStyle ('.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    .hostile td { background:#F73F6A; }.friendly td{background:#1949B7; }.ally td{background:#8D3BCB; }\
    .neutral td { background:#74CEEF; }.unaligned td { background:#85C150; }\
  ...
  try {
    ...
    pop.getTopDiv().innerHTML = '<CENTER>ERROR Power Tools !</center>';
    pop.getMainDiv ().innerHTML = '<DIV style="font-size:18px; font-weight:bold;"><CENTER><p>La dependencia "prototype.js" no fue encontrada. <BR> Por favor, vuelva a instalar Power Tools Español <A HREF="http://userscripts.org/scripts/show/92321"> aquí.</a></div>';
    pop.setZindex(111115);
    pop.show(true);
  }
    
  window.name = 'PT';
  logit ("* KOC Power Tools Español v"+ Version +" Cargado");
  readOptions();
...
//FoodAlerts.init();
  ...
  AddMainTabLink('MENÚ', eventHideShow, mouseMainTab);
}
...
var knightRoles = [
  ['Capataz', 'politics', 'Pol'],
  ['Mariscal', 'combat', 'Com'],
  ['Alquimista', 'intelligence', 'Int'],
  ['Administrador', 'resourcefulness', 'Res'],
];
...
my.Wilds = {
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('RESTAURAR', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
...
//Seed.wilderness['city'+Cities.cities[3].id] = [];      
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Tipo Tierras</td><TD></td><TD align=left>Cord</td><TD>Trampas</td><TD align=left>Mercenarios</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' CAMBIAR DEFENSAS ') +'</td></tr>';
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Más Trampas:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i  
        ...
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Coste: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("APLICAR", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Oro: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
  ...
  e_butWildSet : function (c){
    ...
    popDiv.innerHTML = '<TABLE class=ptTab width=100% height=100%><TR><TD>\
          <DIV class=ptstat>Configurado Defensas en Tierras</div>\
          <DIV id=ptWildBuildDiv class=ptDiv style="padding:10px; height:225px; max-height:225px; overflow-y:auto"></div>\
          </td></tr><TR><TD align=center>'+ strButton20('CANCELAR', 'id=ptWildCancel') +'</td></tr></table>';
  ...
  processQue : function (errMsg){
    ...
    if (what[0] == 'T'){
      div.innerHTML += 'Contruyendo '+ what[2] +' trampas desde '+ Cities.byID[t.buildList.cityId].name +'\'para la tierra '+ wild.xCoord +','+ wild.yCoord +' ... ';
      t.postBuyTraps (t.buildList.cityId, what[1], what[2], t.processQue);
    } else {
      div.innerHTML += 'Actualizando Mercenarios a '+ mercNames[what[2]] +' desde '+ Cities.byID[t.buildList.cityId].name +'\ para la tierra '+ wild.xCoord +','+ wild.yCoord +' ... ';
...
//logit ("UPDATE GOLD TIMER");    
  ...
  postHireMercs : function (cid, tid, newLevel, oldLevel, notify){
    if (DISABLE_POST_DEFENSES){
//      setTimeout (function (){notify(null)}, 1500);
      setTimeout (function (){notify('Bien, esto no es un error real, sólo George probando como se ve un mensaje de error. Si es largo, cómo se forma? Está bien? Estás seguro? Nuri! sacame de esta cosaaaa!!!')}, 1500);
      return;
    }
    var params = Object.clone(unsafeWindow.g_ajaxparams);
    params.cid = cid;
...
my.Knights = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        ...
        if (knight.knightStatus == 10){
          ass = '<TD class=ptentry align=left colspan=2>Marchando</td>';
        } else {  
          if (unpoints > 0){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=11><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          <TR class=ptwpad style="font-weight:bold" align=right><TD width=80>Rol</td><TD width=160 align=center>Nombre</td><TD width=26>Pol</td><TD width=26>Com</td>\
          <TD width=26>Int</td><TD width=26>Ing</td><TD width=70 align=left colspan=2>Sin asignar</td><TD width=30 align=right>Estado </td><TD width=20>Salario </td><TD width=20>Nivel</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
      ...
      m += '<TR align=right><TD colspan=9><B>Total Salario:</b></td><TD>'+ addCommas(totSalary) +'</td></tr>';        
    }
  ...
  clickedAssignPoints : function (e, cid, kid, rid){
    ...
    if (knight.knightStatus == 10){
      ...
      row.childNodes[7].innerHTML = 'Marchando';
      return; 
    }
    sk = [];
    ...
    div.innerHTML = 'Asignando '+ unassigned +' puntos de habilidad en '+ knightRoles[rid][1] +' ... ';
  ...
  postDone : function (rslt, div){
    ...
    clearTimeout (t.displayTimer);
    if (rslt.ok){
      div.innerHTML += '<B>ASIGNADO.</b>';
      t.displayTimer = setTimeout (t.show, 400);
    } else {
...
var messageNav = {
  ...
  hook : function (){
    ...
    var button = makeButton20('Alto Mando');
...
//                msg += 'Mi ciudad ' + Cities.cities[i].name.substring(0,10) + ' (' +
//                       Cities.cities[i].x +','+ Cities.cities[i].y + ')';
//                msg += ' está falta de comida. Tengo: '+addCommasWhole(foodleft)+' ' y durará:'//('+timestrShort(timeLeft)+') '+addCommas(usage);
//                sendChat ("/a " + msg);
//          }
//      }  
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    if ( city.tileId == m.toTileId )
      target = 'ciudad en '+ city.x +','+ city.y;
    else {
    ...
    msg += 'Mi '+ target +' es '+ atkType  +' X '+ who +'. (llegarán en '+
        unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) +') Tropas en camino: ';
    var totTroops = 0;
    for (k in m.unts){
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' Mi embajada tiene ' + availSlots +' plazas libres y está actualizada a nivel '+ emb.maxLevel +' :)';
      }
    }
...
my.AllianceList = {
  ...
  show : function (){
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          <TR><TD class=xtab align=right></td><TD class=xtab>Escribe todo o parte del nombre de un jugador: &nbsp;</td>\
          ...
          <TR><TD class=xtab> </td><TD class=xtab>Escribe todo o parte del nombre de una Alianza:&nbsp;</td>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'Mínimo 3 caracteres';
      return;
    }
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Jugadores encontrados que coinciden con <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE width=95% align=center class=ptTab cellspacing=0><TR style="font-weight:bold"><TD>Nombre</td>\
      <TD align=right>Poderío</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Búsqueda </td></tr>';
    var row=0;
    var cl='';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>detalles</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>Clasificación</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerDetail : function (span, uid){
    var t = my.AllianceList;
    span.onclick = '';
    span.innerHTML = "buscando detalles ...";
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = my.AllianceList;
    span.onclick = '';
    span.innerHTML = "buscando  información de casificación ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Clasificación:</b> No encontrado! (niebla?)';
      return;
    }
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Clasi: </b></td><TD colspan=6> Poderío: '+ p.might  +' &nbsp; Alianza: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      var created = '';
      if (c.dateCreated && c.dateCreated.substr(0,2)=='20')
        created = ' &nbsp; &nbsp; creada: ' + c.dateCreated.substr(0,10);
      m += '<TR><TD align=right><B>-'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName 
      +' (<a onclick="PTgotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; nivel: '
        + c.tileLevel +' &nbsp; &nbsp; estado: '+ cityStatusString (c.cityStatus) + created +'</td></tr>';
    }  
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Detalle:</b> &nbsp; </td><TD>Alianza: '+ a +' &nbsp; Ciudades: '
          + u.cities +' &nbsp; Población: '+ u.population +'</td></tr><TR><TD></td><TD>Provincias: ';
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'Mínimo 3 caracteres';
      return;
    }
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Alianzas encontradas que coinciden con <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Alianza</td><TD class=xtab>Posición</td><TD class=xtab>Miembros</td>\
        <TD align=right class=xtab>Poderío</td><TD class=xtab>Diplomacia</td><TD class=xtab></td></tr>';
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Ver Miembros</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center>Distancia desde <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span></td><TD class=xtab align=right>'+ numPlayers +' encontrados &nbsp; </td></tr></table></div>\
      <TABLE id=tabAllMembers align=center cellpadding=0 cellspacing=0><THEAD>\
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Jugador</div></a></td>\
        <TD id=clickCol1 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Poderío</a></div></td>\
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Ciudades</a></div></td>\
        <TD id=clickCol2 onclick="PTalClickSort(this)" class=clickable align=center><A><DIV>Estado</a></div></td>\
        <TD id=clickCol7 onclick="PTalClickSort(this)" class=clickable><A><DIV>Nombre Ciudad</a></div></td>\
        <TD id=clickCol4 onclick="PTalClickSort(this)" class=clickable><A><DIV>Nivel</a></div></td>\
        <TD id=clickCol5 onclick="PTalClickSort(this)" class=clickable><A><DIV>Coord.</a></div></td>\
        <TD id=clickCol8 onclick="PTalClickSort(this)" class=clickable><A><DIV>Distancia</a></div></td></tr></thead>\
      ...
      <DIV style="top:670px; left:0px; position:absolute; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Los datos son de la clasificación y pueden ser de hace 24 horas!</td>\
          <TD class=xtab align=right>Clic en encabezados para ordenar</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Mostrar distancia desde: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; o elige ciudad: <span id=dmcoords></span></td></tr></table>';
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("Ninguna alianza encontrada bajo")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
...
my.Test = {
  ...
  show : function (){
    function xyNotify(city, x, y){
      var m = '[ Notificado: '+ (city?city.name:'null') +', x='+ x +', y='+ y +' ]';
      document.getElementById('testNotify').innerHTML = m;
    }
    var t = my.Test;
    if (t.state == null){
      var m = '<TABLE><TR><TD align=right>Exploción: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
          <TR><TD align=right>Tierra: </td><TD><INPUT type=checkbox id=fakeIsWild></td></tr>\
          <TR><TD align=right>Informe Falso: </td><TD><INPUT type=checkbox id=fakeFalse></td></tr>\
          <TR><TD align=right>Segundos: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
          <TR><TD align=right># de Milicos: </td><TD><INPUT type=text size=6 value=5000 id=fakeMilitia></td></tr>\
          <TR><TD colspan=2 align=center><INPUT id=testSendMarch type=submit value="Falso Ataque" \></td></tr></table>\
          <BR><DIV id=testDiv style="background-color:#227694; maxwidth:675; max-height:430px; height:430px; overflow-y:auto;"></div>';
      t.cont.innerHTML = m;
  ...
  createFakeAttack : function (cityNum, isScout, isWild, isFalse, secs, numMilitia){
    ...
    march.players.u1234567.n = 'Homer Simpson';
  ...
  show : function (){
    ...
    if (t.state == null){
      m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
          ...
          <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>VALORES TROPAS</div><TABLE align=center cellpadding=1 cellspacing=0>\
          <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>COSTE</b></td><TD class=xtabHL colspan=7><B>ESTADÍSTICAS</b></td><TD class=xtabHL><B>MANTE</b></td></tr>\
          <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Ali</td><TD class=xtabH>Mad</td><TD class=xtabH>Pie</td>\
          <TD class=xtabH>Min</td><TD class=xtabH>Pob</td><TD class=xtabHL>Pod</td><TD class=xtabH>Vid</td><TD class=xtabH>Ata</td><TD class=xtabH>Def</td><TD class=xtabH>Vel</td><TD class=xtabH>Alc</td><TD class=xtabH>Car</td>\
          <TD class=xtabHL>cons/h</td></tr>\
      ...
      m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';
      m += '</table>';
      m += '<BR><DIV class=ptstat>CALCULADOR DE DISTANCIA</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR><TD class=xtab align=right><B>1ª Situacion: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> o elige ciudad: <SPAN id=ptloc1></span></td></tr>\
        <TR><TD class=xtab><B>2ª Situacion: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> o elige ciudad: <SPAN id=ptloc2></span></td></tr></table>\
        ...
        <BR><BR><DIV class=ptstat>MAPA PROVINCIAS</div><DIV id=ptProvMap><IMG width=680 height=654 SRC="'+ URL_PROVINCE_MAP +'"></div></center></div>';
      t.cont.innerHTML = m;
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = 'La distancias desde '+ x1 +','+ y1 +' a '+ x2 +','+ y2 +' es: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
    ...
    if (t.state != 1){
      try {      
        m = '<TABLE class=ptTab>\
          ...
          <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Habilitar el arrastre de la ventana (mover la ventana  arrastrando la barra superior con el ratón)</td></tr>\
          <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Mostrar \alimentos negativos\ en rojo si el alimento se agotara en menos de \
              <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> horas</td></tr>\
          <TR><TD colspan=2><P><B>Caracteristicas de KOC:</b></td></tr>\
          <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Mostrar el número de nuevos mensajes separados del número de informes  sobre el icono de mensajes.</td></tr>\
          <TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Publicar automaticamente los ataques entrantes en el chat de la alianza.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
              <TR><TD align=right>Mensaje Prefijado(editable): &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
              <TR><TD align=right>Alertar si te exploran: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
              <TR><TD align=right>Alertar si atacan tierras: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
              <TR><TD align=right>Cuando vengan un mínimo de: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
              </table></td></tr>\
          <TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Activar informes mejorados de la Alianza.</td></tr>\
          <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Permitir que otros scripts cambien el formato de los Informes de la Alianza.</td></tr>\
          <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Habilitar mensajería mejorada  (botones "Reenviar" y "Alto Mando").</td></tr>\
          <TR><TD colspan=2><BR><BR><B>Correccion de errores de Kingdoms of Camelot:</b></td></tr>\
          <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Fijar informe de la torre para mostrar el destino exacto (de la ciudad, tierra o no válido)</td></tr>\
          <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Fijar el mapa para mostrar distancias de la ciudad seleccionada, en lugar de estar siempre en la primera ciudad.</td></tr>\
          <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Fijar falsas alertas de ataque creadas a partir de misiones propias de exploración.</td></tr>\
          </table><BR><BR><HR>NOTA: Tener en cuenta que si una casilla está de color gris probablemente haya habido un cambio de codigo de KofC y la opción dejará de funcionar.';
        t.cont.innerHTML = m;
  
        document.getElementById('alertEnable').addEventListener ('change', e_alertOptChanged, true);
        document.getElementById('alertPrefix').addEventListener ('change', e_alertOptChanged, true);
        document.getElementById('alertScout').addEventListener ('change', e_alertOptChanged, true);
        document.getElementById('alertWild').addEventListener ('change', e_alertOptChanged, true);
        document.getElementById('alertTroops').addEventListener ('change', e_alertOptChanged, true);
...
my.Search = {
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = my.AllianceList;
    span.onclick = '';
    span.innerHTML = "buscando info en clasificación ...";
    t.fetchLeaderboard (uid, function (r) {t.gotPlayerLeaderboard(r, span)});
  },
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Clasificación:</b> No encontrada! (Niebla?)';
      return;
    }
    var p = rslt.results[0];
    var an = p.allianceName;
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ officerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Clasificación: </b></td><TD colspan=2> Poderío: '+ p.might  +' &nbsp; Alianza: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      m += '<TR><TD align=right><B>Ciudad '+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName 
      +' (<a onclick="PTgotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; nivel: '
        + c.tileLevel +' &nbsp; &nbsp; estado: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; creada: ' + c.dateCreated.substr(0,10) +'</td></tr>';
    }  
    span.innerHTML = m + '</table>';
  },
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Detalles:</b> &nbsp; </td><TD>Alianza: '+ a +' &nbsp; Ciudades: '
          + u.cities +' &nbsp; Población: '+ u.population +'</td></tr><TR><TD></td><TD>Provincias: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
  ...
  show : function (cont){
    ...
    if (t.state == null){
      this.cont.innerHTML = '\
        <DIV class=ptentry><table><tr><td><TABLE><TR valign=top><TD class=xtab width=100 align=right>Búsqueda para: </td><TD>\
        <SELECT id="srcType">\
          <OPTION value=0>Campamentos Bárbaros</option>\
          <OPTION value=1>Tierras Vírgenes</option>\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Desde: &nbsp; X: </td><TD class=xtab>\
        <INPUT id=srchX type=text\> &nbsp; Y: <INPUT id=srchY type=text\> &nbsp; <SPAN id=spInXY></span>\
        </td></tr><TR><TD class=xtab align=right>Max. Distancia: </td><TD class=xtab><INPUT id=srcDist size=4 value=10 /></td></tr>';
  ...
  clickedSearch : function (){
    ...
    if (t.searchRunning){
      t.stopSearch ('BÚSQUEDA CANCELADA!');
      return;
    }
    ...
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>75)
      errMsg += "Max distancia debe estar entr 1 y 75<BR>";
    if (errMsg != ''){
    ...
    document.getElementById ('srcStart').value = 'Parar Búsqueda';
    ...
    m = '<CENTER><B>Exploración de '+ typeName +'<BR>\
        Desde: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distancia: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>OPCIONES DE LISTADO:</b><BR></td></tr>';
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     m += '<TR><TD class=xtab align=right>Min. nivel a mostrar:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max. nivel a mostrar:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Tipo de tierra:</td><TD class=xtab align=right>\
            Bosques<INPUT id=woodWild type=CHECKBOX'+ (Options.woodWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Praderas/Lagos<INPUT  id=foodWild type=CHECKBOX '+ (Options.foodWild?' CHECKED':'') +'></td>\
	       <TD class=xtab align=right>Montañas<INPUT id=mtnWild type=CHECKBOX '+ (Options.mtnWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Planicies<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD class=xtab align=right>Colinas<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '</select></td></tr><TR><TD class=xtab align=right>Solo Libres:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>Ordenar por:</td><TD class=xtab><SELECT id=filSortBy>\
          <OPTION value="level" '+ (Options.srcSortBy=='level'?'SELECTED':'')  +'>Poderío</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
            </select></td></tr>\
			<TR><TD class=xtab align=right>Ver solo Coordenadas:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
    } else {
		m+= '</select></td></tr><TR><TD class=xtab align=right>Solo en Niebla:</td><TD class=xtab><INPUT id=filMisted type=CHECKBOX '+ (Options.mistedOnly?' CHECKED':'') +'\><td></tr>';
                m+= '<TR><TD class=xtab align=right><font color=#8D3BCB>Solo Aliados:</td><TD class=xtab><INPUT id=filAlly type=CHECKBOX '+ (Options.allyOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right><font color=#1949B7>Solo Amigables:</td><TD class=xtab><INPUT id=filFriendly type=CHECKBOX '+ (Options.friendlyOnly?' CHECKED':'') +'\><td></tr>';
                m+= '<TR><TD class=xtab align=right><font color=#F73F6A>Solo Hostiles:</td><TD class=xtab><INPUT id=filHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right><font color=#74CEEF>Solo Neutrales:</td><TD class=xtab><INPUT id=filNeutral type=CHECKBOX '+ (Options.neutralOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right><font color=#85C150>Solo sin Alianza:</td><TD class=xtab><INPUT id=filUnaligned type=CHECKBOX '+ (Options.unalignedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>Ordenar Por:</td><TD class=xtab><SELECT id=filSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Nivel</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distancia</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right>Solo Coordenadas:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
...
if (DEBUG_TRACE) logit (" 0 clickedSearch()... fuera de tiempo ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
    setTimeout (function(){Map.request (xxx, yyy, 15, t.mapCallback)}, MAP_DELAY);
  },

  dispMapTable : function (){
    var tileNames = ['Campamento Bárbaros', 'Pradera', 'Lago', 'Bosque', 'Colina', 'Montaña', 'Planicie', 'Ciudad' ];
    var t = my.Search;
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    if (dat.length == 0){
      m = '<BR><CENTER>No encontrados</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
		...
		if (t.opt.searchType == 2) {
			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Pos</td><TD >Dist</td><TD>Ciudad</td><TD>Dueño</td><TD>Poderío</td><td>Alianza               </td><TD width=80% style="font-size:9px;">Más info</td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			m = '<TABLE id=srcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Posición</td><TD style="padding-left: 10px">Distancia</td><TD style="padding-left: 10px;">Nvl</td><TD width=80%> &nbsp; Tipo</td><TD style=""></td></tr>';
		}
	  }
        ...
        if (coordsOnly)
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					m += '<DIV onclick="PTscout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A style="font-size:9px;" >Explorar</a></div>';
				} else 
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A style="font-size:9px;" >Detalles</a></div> <DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Clasificación</a></div>';
            m+= '</td><TD  valign="top">'+ (dat[i][5]?' Nublado':'') +'</td></tr>';
			} else { 
          m += '<TD align=right  valign="top">'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD align=right>'+ dat[i][4] +'</td><TD> &nbsp; '+ tileNames[dat[i][3]]
            +'</td><TD  valign="top">'+ (dat[i][5]?' DUEÑO':'') +'</td></tr>';
			}
		}
       }
      m += '</table>';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    var t = my.Search;
    document.getElementById ('statStatus').innerHTML = '<FONT color=#58FA58>'+ msg +'</font>';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Explorados: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    if (t.curX > t.lastX){
      ...
      t.curY += 15;
      if (t.curY > t.lastY){
        t.stopSearch ('FIN DE EXPLORACIÓN!');
        return;
      }
    }
    ...
    document.getElementById ('statStatus').innerHTML = 'Explorando '+ x +','+ y;
  ...
  show : function (){
    ...
    if (t.state == null){
      s = "<DIV id=trainTopSelect>\
        <DIV class=ptstat>Entrenar tropas y contruir en muro/defensas de campo</div><DIV style='height:10px'></div><DIV class=ptentry>\
        <DIV style='text-align:center; margin-bottom:10px;'>Selecciona Ciudad: &nbsp; <span id=ptspeedcity></span></div>\
        <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
        <TABLE align=center><TR><TD align=right>Tipo de Tropa: </td><TD colspan=2>\
        <SELECT id=ptttType>\
          <option value='1'>Tropas de Suministro</option>\
          <option value='2'>Milicianos</option>\
          <option value='3'>Exploradores</option>\
          <option value='4'>Piqueros</option>\
          <option value='5'>Espadachines</option>\
          <option value='6'>Arqueros</option>\
          <option value='7'>Caballería</option>\
          <option value='8'>Caballería Pesada</option>\
          <option value='9'>Carretas de Suministro</option>\
          <option value='10'>Ballistas</option>\
          <option value='11'>Arietes</option>\
          <option value='12'>Catapultas</option>\
        </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
        <TR><TD align=right>por espacio: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
          <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
        <TR><TD align=right>espacios: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
          <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
        <TR><TD align=right valign=top>Trabajadores al ralentí: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
          <SPAN style='white-space:normal;'>Le permite entrenar más soldados. Dejando la población inactiva en negativo temporalmente.</span></td></tr>\
        <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='Entrenar Tropas'\></td></tr>\
        </table></td><TD width=20></td><TD style='border-left:solid 2px;' width=50% align=center>\
        <TABLE align=center><TR><TD align=right>Tipo Defensa: </td><TD colspan=2>\
        <SELECT id=pttdType>\
          <option value='53'>Ballestas</option>\
          <option value='55'>Fundíbulos</option>\
          <option value='60'>Trampas</option>\
          <option value='61'>Abrojos</option>\
          <option value='62'>Barreras con picos</option>\
        </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
        <TR><TD align=right> por espacio: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
          <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
        <TR><TD align=right> espacios: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
        ...
        <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Hacer Defensas'\></td></tr></table>\
        </td></tr></table></div></div>\
        ...
        <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Cola Tropas &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
          <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Cola Defensas &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
        </div>";
      t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>El número de tropas por espacio debe ser mayor de 0.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>No se pueden entrenar tantas tropas (max es '+ t.stats.MaxTrain +' total)</font>';
      return;
    }
    if (numSlots<1 || numSlots>t.stats.barracks - t.stats.queued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Numero de espacios invalido.</font>';
      return;
    }
    t.setBusy(true);
    var que = [];
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; const:'+ t.stats.defOwned;   
    ...
    t.TDspSpace.innerHTML = 'Nivel del muro: <B>'+ t.stats.wallLevel +'</b><BR>Espacios en muro: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        Espacio en Campos: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>El número de unidades por espacio debe ser mayor que 0.</font>';
      return;
    }
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>No se pueden entrenar tantas unidades (max es '+ t.stats.MaxDefTrain +' total)</font>';
      return;
    }
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      t.displayCityStats();
      if (errMsg){
        t.dispTrainStatus ('<font color=#5ECA5A><B>ERROR: '+ errMsg +'</b></font><BR>');
        t.setBusy(false);
        return;
      }
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Cola de defensas terminadas.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Entrenando '+ cmd[2] +' '+  fortNamesShort[cmd[1]] +'<BR>');
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><B>Alimento:</b></td><TD width=16%><B>Madera:</b></td><TD width=16%><B>Piedra:</b></td>\
        <TD width=16%><B>Mineral:</b></td><TD width=16%><B>Oro:</b></td><TD width=16%><B>Pob Inact:</b></td></tr>\
...
// troop queue .... 
    ...
    m = t.stats.barracks +' cuarteles';
    if (t.stats.queued > 0)
      m += ', '+ t.stats.queued +' espacios en uso';
    if (totTime > 0)
...
// defense queue ....
    ...
    m = t.stats.Dqueued +' espacios en uso';
    if (totTime > 0)
      m += ', '+ unsafeWindow.timestr(totTime);
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<font color=#54AE51><B>Cola de tropas terminada.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Entrenando '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +'<BR>');
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Empezó el:</span> '+ dt.toLocaleDateString() +'</td>\
        <TD><SPAN class=ptStatLight>Poderio:</span> ' + addCommas(Seed.player.might) +'</td>\
      ...
      if (Options.includeMarching)
        str += '<TD width=81><B>Marchando</b></td>';
      str += "</tr>";
      ...
      str += _row ('Oro', rows[0]);
      str += _row ('Alimento', rows[1]);
      str += _row ('Madera', rows[2]);
      str += _row ('Piedra', rows[3]);
      str += _row ('Mineral', rows[4]);
      str += '<TR><TD><BR></td></tr>';
      for (r=1; r<13; r++){
        rows[r] = [];
        for(i=0; i<Cities.numCities; i++) {
      ...
      rownum = 0;
      str += _row ('T.Sumin.', rows[1]);
      str += _row ('Milician.', rows[2]);
      str += _row ('Exploras', rows[3]);
      str += _row ('Piqueros', rows[4]);
      str += _row ('Espadas', rows[5]);
      str += _row ('Arqueros', rows[6]);
      str += _row ('Caballer.', rows[7]);
      str += _row ('Cb.Pesada', rows[8]);
      str += _row ('Carretas', rows[9]);
      str += _row ('Ballistas', rows[10]);
      str += _row ('Arietes', rows[11]);
      str += _row ('Catapultas', rows[12]);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('Cola Muro', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Incluir Tropas en Marcha/Recursos</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
...
my.Encamped = {
  ...
  show : function (cont){
    var rownum = 0;
    var names = ['Sum', 'Mil', 'Exp', 'Piq', 'Esp', 'Arq', 'Cab', 'Pes', 'Car', 'Bal', 'Ari', 'Cat'];
    var t = my.Encamped;
    ...
    s = '<div class=ptstat>Tropas acampadas en cada una de tus embajadas.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>No hay tropas acampadas.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Jugador (nivcab)</b></td>';
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Mostrar origen tropas';
    s += '<BR><BR><DIV style="font-size: 10px">NOTA: Tendras que volver a refrescar KoC para mostrar nuevos acampados o las tropas que quedan después de una batalla.</div>';
    t.cont.innerHTML = s;
    checkBox = document.getElementById('idCheck2');
    ...
    function CboxHandler (t){
      ...
      function eventChange (){
        ...
        if (isNaN(x) || x<0 || x>750){
          that.coordBoxX.style.backgroundColor = '#EF5BFA';
          return;
        }
        if (isNaN(y) || y<0 || y>750){
          that.coordBoxY.style.backgroundColor = '#EF5BFA';
          return;
        }
        that.coordBoxX.style.backgroundColor = '#ffffff';
        that.coordBoxY.style.backgroundColor = '#ffffff';
...
function DialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  if (!rdiv){
    ...
    rdiv.innerHTML = '<DIV style="background: #A0E2D9"><CENTER>KOC Power Tools Español 2.9 </div>\
      <CENTER><BR><FONT COLOR=#550000><B>An error has ocurred:</b></font><BR><BR><DIV id=retryErrMsg></div>\
      <BR><BR><B>Reintentar automaticamente en <SPAN id=retrySeconds></b></span> segundos ...<BR><BR><INPUT id=retryCancel type=submit value="CANCELAR Reintentar" \>';
