...
// @require         http://userscripts.org/scripts/source/127908.user.js
// @require        http://www.betawarriors.com/bin/gm/62718user.js
// @version			0.34
//
// @history			0.34 This script has been replaced by http://userscripts.org/scripts/show/80545
// @history			0.33 Trading post now shows overview of trades available 
// @history			0.32 Added display of time until full to town hall view (based on overkill's code)
// @history			0.31 Fixed incorrect display of building levels when looking at someone else's city view
// @history			0.31 Building levels in someone else's city view are marked in red
// @history			0.31 Misc fixes and better support for Google Chrome
...
ScriptUpdater.check(57197, "0.34");
...
IkaEmpire = {
					...
					if(!cities[i].type || cities[i].type.match(/tradegood/)) {
						...
						tr.style.borderTop = "1px dotted #9D836A";
								...
								resourceTds[type] = document.createElement('td');
									...
									switch(type) {
										case 'population':
											...
											resourceTds[type].innerHTML += '<a style="'+full+' display:inline; padding:0; width:auto; height:auto; background:none;" href="/index.php?view=townHall&position=0&id=' + cities[i].id + '" name="' + cities[i].id +'">' + text + '</a>';	
											break;
										case 'gold':
											resourceTds[type].innerHTML += '<a style="display:inline; padding:0; width:auto; height:auto; background:none;" href="/index.php?view=finances" name="' + cities[i].id +'">' + 
										...
										case 'research':
											if(IkaTools.cityGetBuildingByType(cities[i], 'academy')) {
												resourceTds[type].innerHTML += '<a style="display:inline; padding:0; width:auto; height:auto; background:none;" href="/index.php?view=academy&position=' + IkaTools.cityGetBuildingByType(cities[i], 'academy').position + '&id=' + cities[i].id + '" name="' + cities[i].id +'">+' + change;
											}
											break;
										case 'wood':
											resourceTds[type].innerHTML += '<a style="display:inline; padding:0; width:auto; height:auto; background:none;" href="/index.php?view=resource&type=resource&id=' + IkaTools.cityGetIslandId(cities[i]) + '" name="' + cities[i].id +'">' + text + '</a>';	
											break;
										default:
											resourceTds[type].innerHTML += type.toString() != IkaTools.cityGetTradegoodType(cities[i]).toString() ? text : '<a style="display:inline; padding:0; width:auto; height:auto; background:none;" href="/index.php?view=tradegood&type=tradegood&id=' + IkaTools.cityGetIslandId(cities[i]) + '" name="' + cities[i].id +'">' + text + '</a>';	;
									}
									$(resourceTds[type]).click(function() {
							...
							if(IkaTools.cityGetIslandId(cities[i]) > 0) {
								$(td2).append('<img src="/skin/layout/icon-island.gif" style="cursor:pointer; height:16px; margin-right:2px; float:right;" name="index.php?view=island&cityId=' + cities[i].id + '&islandId=' + cities[i].islandId + '" title="View city on island"/>');
	...
	views:{
		branchOffice:function() {
			GM_addStyle("#tradeOverview .paginator, #tradeOverview .unicode { display:none !important; } #resourceSale0, #resourceBuy0 { padding-top:0 !important; } #tradeOverview td, #tradeOverview th { font-size:.8em; } #tradeOverview img { height:12px; }\
			...
			$('div.contentBox01h:eq(2) h3').html('<span style="width:50%; float:left;" class="textLabel">Sell Offers</span><span class="textLabel">Buy Offers');
			var target = $('div.contentBox01h:eq(2) div.content');
			target.html('<table id="tradeOverview" cellpadding="0" cellspacing="0" border="0" style="width:100%;"><tr><td id="resourceSale0"></td><td id="resourceBuy0"></td></tr><tr><td id="resourceSale1"></td><td id="resourceBuy1"></td></tr><tr><td id="resourceSale2"></td><td id="resourceBuy2"></td></tr><tr><td id="resourceSale3"></td><td id="resourceBuy3"></td></tr><tr><td id="resourceSale4"></td><td id="resourceBuy4"></td></tr>');
			var html = '';
			var range = $('select[name="range"]').attr('value');
			for(var i = 0; i < 5; i++) {
				IkaTools.getRemoteDocument(document.location.toString() + '&range=' + range + '&orderBy=&searchResource=' + (i == 0 ? '' : i) + 'resource&type=444', function(doc) {
					...
					if(html.match(/resource=/)) {
						...
						$('#resourceSale' + resourceNum).html(html.replace(/\/resources\/icon_\./g, '/resources/icon_' + type + '.'));
					}
				});
			}
			for(var i = 0; i < 5; i++) {
				IkaTools.getRemoteDocument(document.location.toString() + '&range=' + range + '&orderBy=sellDesc&searchResource=' + (i == 0 ? '' : i) + 'resource&type=333', function(doc) {
					html = $('div.contentBox01h:eq(2) div.content', doc).html();
					var type = 'wood';	
					if(html.match(/resource=/)) {
						var matches = html.match(/resource=(\d)/);
						...
						$('#resourceBuy' + buyNum).html(html.replace(/\/resources\/icon_\./g, '/resources/icon_' + type + '.'));
					}
				});
			}
			// &range=4&orderBy=sellDesc&searchResource=resource&type=444
		},
		townHall:function() {
		...
		city:function() {
			try { var isMyCity = IkaTools.getCityById(document.location.toString().match(/id=(\d+)/)[1]); } catch(e) { var isMyCity = false; }
				...
				$('#mainview ul#locations li').each(function(i) {
					...
					if(this.id.match(/\d+$/)) {
						...
						if(type) {
							...
							if(!isMyCity) dot.style.background = "#880000"
...
/*
					...
					if(building) {
						...
						dot.className = "ikaEmpireBuildingLevel";
						if(building.level < 10 || building.level.toString() == 'NaN') dot.style.padding = "2px 6px";						
						dot.innerHTML = building.level.toString() == 'NaN' ? 0 : building.level + building.type;
						if(IkaTools.buildingGetResourceMissingTotal(building) == 0 && building.level != IkaTools.maxBuildingLevel) dot.style.background = "#008800";
						$('div.buildingimg', this).append(dot);
						$('div.constructionSite', this).append(dot);
						switch(building.type) {
