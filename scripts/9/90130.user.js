...
////// @homepage       http://userscripts.org/scripts/show/74562
// @include        http://s*.ikariam.*/*
// @resource wooddeal wooddealer.gif
// @resource winedeal winedealer.gif
// @resource marbledeal marbledealer.gif
// @resource crystaldeal crystaldealer.gif
// @resource sulfurdeal sulfurdealer.gif
// @version        3.7
// @history        3.7   Include Script Updater Check //killed, does not work with userscripts.org @resources 
// @history        3.6   Include Maximum for distribute.
// @history        3.5   Fixed cities problem [Now display only own cities].
// @history        3.4   Include Default Deal "Wine Dealer".
// @history        3.3   Include Menu Interface Control Dealer to show and hide options. Include Priority Control from Deals [Up and Down].
// @history        3.2   Fixed use script in more than one world. Fixed problem get crystal resource in transports.
// @history        3.1   Fixed problem with Empire Board.
// @history        3.0   Include material from ships to calculate the distribution. Include the capacity of warehouse in each city to calculate the distribution. Include Minimum for deposit. Include Interface Control Dealer [with many new options]. Include Save Data in Cookies. Include Wood Dealer.   
// @history        2.0   All Resources. Distribution with each page refresh [Configure with min_ini]. Repeat Distribution [Configure with min_rep]. Include inform. Include configure show_image and show_inform. Include configure minimum for each resource. Include configure city control for each resource.   
// @history        1.0   Only Wine Dealer. Distribution Wine to login in ikariam.
// ==/UserScript==
...
var url_img_wood_dealer = GM_getResourceURL("wooddeal");
var url_img_wine_dealer = GM_getResourceURL("winedeal");
var url_img_marble_dealer = GM_getResourceURL("marbledeal");
var url_img_crystal_dealer = GM_getResourceURL("crystaldeal");
var url_img_sulfur_dealer = GM_getResourceURL("sulfurdeal");
...
setVar("Max_ship",getVar("Max_ship",10));
...
h.id = "menu_dealer";
//  heading  
h.setAttribute('style','padding:7px 0px 7px 0px; position:relative;clear:both;float:left;border-color:#C9A584 #5D4C2F #5D4C2F #C9A584;border-style:double;border-width:3px;background-image: url(skin/input/button.gif) ;text-decoration:none;width:100%;color:#612d04;font:bold 12px Arial, Helvetica, sans-serif;text-align:left;');
//h.innerHTML = '<table align="left" width="100%"><tr><td id="cel_menu_1" align="center">Ikariam Resource Dealer </td><td id="cel_menu_2" align="center" width="18%"></td><td id="cel_menu_3" align="center" width="18%"></td><td id="cel_menu_4" align="center" width="18%"></td><td id="cel_menu_5" align="center" width="18%"></td></tr></table>';
h.innerHTML = '<table align="left" width="100%"><tr><td "GLon" id="GLon_off" align="center" width="10%"></td><td id="cel_menu_1" align="center">Ikariam Resource Dealer 3.0 MOD 4</td><td id="cel_menu_2" align="center" width="10%"></td><td id="cel_menu_3" align="center" width="10%"></td><td id="cel_menu_4" align="center" width="10%"></td><td id="cel_menu_5" align="center" width="10%"></td><td align="right">Ships</td> <td "Ships" id="ships" align="center" width="10%"></td></tr></table>';
p.appendChild(h);
...
h.id = "form_dealer";
// newdeal  
h.setAttribute('style','position:relative;display:none;clear:both;float:left;border-color:#C9A584 #5D4C2F #5D4C2F #C9A584;border-style:double;border-width:3px;background-image: url(skin/input/button.gif);text-decoration:none;width:100%;color:#612d04;font:bold 12px Arial, Helvetica, sans-serif;text-align:center;');
p.appendChild(h);

var tabla  = '<form name="form_dealer_add" id="form_dealer_add" ><table align="left" border="1" bordercolor="#c69262" width="100%">';
tabla += '<tr height="22"><td>Name</td><td>Res.</td><td>Min. Deal</td><td>Max. Deal</td><td>Min. Stock</td><td title="(Minutes)">Rpt</td>';
...
//{
//GM_log (cityRec[i]);
cityRec[i] = cityRec[i].toLowerCase();
//GM_log (cityRec[i]);
//}
...
h = document.createElement('div');
h.id = "dealers";
h.setAttribute('style','position:relative;clear:both;float:left;border-color:#C9A584 #5D4C2F #5D4C2F #C9A584;border-style:double;border-width:3px;background-image: url(skin/input/button.gif) ;text-decoration:none;width:100%;color:#612d04;font:bold 12px Arial, Helvetica, sans-serif;text-align:center;');
...
function cargar_dealers(){
	...
	if(dealer_name.length-1>0){
		...
		tabla += '<tr height="30"><td></td><td></td><td></td><td>Name</td><td>Resource</td><td>Min. Deal</td><td>Max. Deal</td><td>Min. Stock</td><td title="(Minutes)">Repeat</td>';
		for (var i = 0; i < cityId.length; i++) {
			tabla += '<td title="'+cityNombre[i]+'">C'+(i+1)+'<img height="14" src="skin/resources/icon_'+cityRec[i]+'.gif"/></td>';
		}
		...
		for (var i = 0; i < dealer_name.length-1; i++) {
			...
			var rem = i%2;
				...
				if(getVar(nombre+"_on")==1){
					LineStyle = 'style="font-weight:bold;background: #fdf7dd"'
					}else{
				...
				{
					if(getVar(nombre+"_on")==1){
					LineStyle = 'style="font-weight:bold;background: #FBEED1"'
					}else{
...
//			tabla += '<tr height="20"><td style="font-weight:normal;background: #eee0c0">'+(i+1)+'</td>';
			tabla += '<tr height="40">';
			tabla += '<td '+ LineStyle +">"+(i+1)+'</td>';
			tabla += '<td '+ LineStyle +(i+1)+' id="UP_'+i+'" ></td>';
			tabla += '<td '+ LineStyle +(i+1)+' id="DW_'+i+'" ></td>';
			...
			tabla += '<td '+ LineStyle +">"+Fecha(getVar(nombre+"_fecha")*1000*60)+'</td><td '+ LineStyle +' id="Mod_'+i+'"></td><td '+ LineStyle +' id="Del_'+i+'"></td><td '+ LineStyle +' id="On_Off_'+i+'"></td><td align="left" '+ LineStyle +">"+getVar(nombre+"_estado")+'</td></tr>';
			}
...
h.id = "dealer_inform";
//  inform paneel 
h.setAttribute('style','text-align:left;position:relative;clear:both;float:left;border-color:#C9A584 #5D4C2F #5D4C2F #C9A584;border-style:double;border-width:3px;background: #FBEED1 ;text-decoration:none;width:100%;color:#612d04;font:normal 12px Arial, Helvetica, sans-serif;');
//h.setAttribute('style','text-align:left;position:relative;clear:both;float:left;border-color:#C9A584 #5D4C2F #5D4C2F #C9A584;border-style:double;border-width:3px;text-decoration:none;width:100%;color:#612d04;font:normal 12px Arial, Helvetica, sans-serif;');
...
h.id = "b_GL_on";
h.innerHTML = 'All Off';
h.addEventListener("click",function(){f_GL_on();},false);
p.appendChild(h);
// The switch
function f_GL_on()
	{
	var dealer_names = getVar("dealer_names","");
	var dealer_name = dealer_names.split('|');
	for (var i = 0; i < dealer_name.length-1; i++) //all deals
		{
		var nombre=dealer_name[i];
		if(getVar(nombre+"_on")==1) // if on..
			{
			setVar(nombre+"_on",0);  // switch to off
			}
		}
...
h.setAttribute('class','button');
//h.id = b_ships
h.addEventListener("click",function(){Maxships()},false);
h.innerHTML = Max_ship;
p.appendChild(h);


function Maxships(){
Max_ship = prompt("Enter number of ships to load");
	if(Max_ship < 1){
	Max_ship = 1;
	}

setVar("Max_ship", Max_ship);
////////  need refresh here
...
function proc_accion(text){
	...
	if(rec_accion<=ind_accion){
		...
		switch(accion[rec_accion-1]){
			...
			if (cant[rec_accion-1]>Max_ship) {
				cant[rec_accion-1]=Max_ship;
				}
...
function salida(msg){
	...
	var rand_time = 0;
////  random 33% or  if>30 minutes+10 min max
	var rand_time = Math.random()-0.5;
	if (repeat>30){
		rand_time = rand_time*10;
	}else{	
	rand_time = rand_time*repeat/3;
	}
	
	var fecha = parseInt(momentoActual/(1000*60)+repeat+rand_time);	
...
function obt_resource(text){
	...
	ini2 = ini2 + 1;
	var fin = text.indexOf('<',ini2); 
	var temp = text.substring(ini2,fin).replace(",",""); 
// if there is a k, remove it and multipicate by 1000 
	if (temp.indexOf('k',0) > 0)	{
	temp = temp.replace("k","");
	temp = parseInt(temp) * 1000;
	return temp;
	}else {
	return parseInt((text.substring(ini2,fin).replace(",","")));
	}
}
...
function obt_barcos2(text){
	var ini1 =  text.indexOf('</span><span>',0);
	var ini2 = ini1 + 13;
	var fin = text.indexOf(' ',ini2);
	return parseInt(text.substring(ini2,fin));
}
...
function obt_b1(text){
	...
	while (ini!=-1){
		...
		if(ini2<aux){
			...
			ret[ind] = parseInt((text.substring(ini3,fin).replace(",","")));
		}else{
			ret[ind] = 0;
		}
		ind++;
