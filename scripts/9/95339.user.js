// ==UserScript==
// @name             KOCAttack - Extra Features Turkce !
// @version          0.5.3
// @namespace        KOCAttack-Extra
// @homepage         http://bookmypage.com/koc
// @description      Niknah'in KOCAttack script (as of Oct. 24, 2010)inin Turkceye cevrilmis versiyonudur. Selcuk Bayraktar
...
// OTO SALDIRI			
			...
			"OTO SALDIRI <br>Saldiri Emri: <select id='KOCAttackOrder'><option value='mostTroops'>once En cok asker olani kullan</option><option value='closest'>once En yakin hedefi kullan</option></select><br />"+
			"Saldirilacak alan tipleri: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbar "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Bozkir "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Sehir "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Tasima<br> "+
			"<br />"+
			"Her saldiri arasindaki bekleme suresi<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> sn. <br />"+
			"<u>Ayni hedefe</u> saldiri yapmak icin bekleme sureleri<br />"+
			"<div style='margin-left: 40px'>"+
			"Barbar saldirisi  :<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60))+"' size='3' />saat<br />"+
			"Bozkir saldirisi   :<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />saat<br />"+
			" Somurge saldirisi:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />saat<br />"+
			" Tasima      :<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />dakika"+
			"</div>"+
			"<br />"+
			"Saldiri yapilacak olan yerin sehire maximum mesafesi <input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> <br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Saldiri ilk hangi sehirden yapildiysa bundan sonraki saldirilarida o sehirden yap .<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Eger (unknown/excess traffic error) hatasi olustuysa yuruyusu tekrar dene... (Bu secenegi degistirdiyseniz sayfayi yenileyin).<br />"+
			"Eger saldiri aliyorsam yeni bir sekmede bu web sayfasi acilsin ... <br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br /> <br> Aciklama: Bu program yuklendikten sonra sistem butun saldirilari aklinda tutar ve sende bunu harita sayfasinda solda liste seklinde gorebilirsin. Oto saldiriyi actiktan sonra belirledigin ayarlar dogrultusunda oyun ayni saldiriyi tekrarlayacaktir. Eger saldiriyi iptal etmek istersen; ilgili yeri sec, saldiri tusuna bas ve sol ustte cikan saldiriyi sil butonuna bas <br> Tavsiyem: Oto saldiriyi kapat tum saldirilarini 1 saat boyunca duzenli olarak yap ve oto saldiriyi calistir."+
...
// MESAJ AYARLARI
			...
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Barbar ve Bozkir raporlarini otomatik olarak sil.<br />"+
			"En fazla <input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> tane saldiri raporunu sakla.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Otomatik Rapor goruntulemeyi kapat , Bunu isaretlerseniz  Oto rapor silici calismaz.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"Oto Egitim yapmadan once nufusun % <input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />unu kullanilabilir duruma getir.<br />"+
			"Oto Altin tusuna basmadan once mutluluk seviyesini % <input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' /> yap.<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Ittifak uyelreinin bina ve arastirmalar icin yardim isteklerine otomatik olarak cevap ver .<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Birlik uyelerinin yardim isteklerini ve raporlari chat te gosterme (eger ustteki isaretliyse, yardim ettikten sonra gizler).<br />"+
			"<input id='KOCAttackAutoPublishHelpRequests' type='checkbox' "+(this.options.autoPublishHelpRequests?'checked':'')+" /> Insaat ve Arastirma hizlandirma isteklerimi otomatik olarak duvarimda yayinla.<br />"+
			"Eger ustteki isaretliyse, Hangi guvenlik ayarini kullanalim? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Herkes</option><option value='50'>Arkadaslarimin arkadaslari</option><option value='40'>sadece arkadaslarim</option><option value='10'>Sadece ben</option>< /select><br />"+
			
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> saniyede bir sehirler arasinda gecis yap.<br />"+
			"Tum sehirler arasinda <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> kere gecis yap ve sayfayi yenilemeden once  "+
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /> saniye bekle.<br />"+
			"Atak yapilacak sehirlerini sec: "+okCitiesHtml+'<br /> <br>Oyunda o anda hangi sehir seciliyse o sehirden oto saldiri varsa yapar bu yuzden sehirler arasi gecisi ayarlamalisin yoksa tum sehirlerinde oto saldiri yapamazsin. Ayrica belli bir donguden sonra sayfayi otomatik olarak yenilemen belli takilmalari engelleyecektir'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% random adjustment for all delays (to look more human)(Ne ise yaradigini henuz bende cozemedim).<br /><br>"+
			"Ayarlari Yedekleme: Mevcut kullandiginiz ayarlari yedeklemek icin Yedek al tusuna basin kutucugun icine yazilar gelecek, yazilara tiklayin, tumunu secin, kopyalayin ve kopyaladiginiz yaziyi bir yere yapistirarak saklayin. Tum ayarlariniz yedeklendi<br> Daha once yaptiginiz bir ayari kaydetmek icin ayarlarinizi bulundugu yerden kopyalayip kutucuga yapistirin ve Ayarla tusuna basin.<br /><textarea id='KOCAttackImport'></textarea>"+
			"<a class='button20' id='KOCAttackImportButton'><span>Ayarla</span></a> <a class='button20' id='KOCAttackExportButton'><span>Yedek Al</span></a><br />"+
			"</td></tr>"+

			"</table>"+
			
			"<a id='KOCAttackOptionsSave' class='button20'><span>Kaydet</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Iptal</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Ayarlari Sifirla</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Herseyi Sifirla</span></a> <a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Hafizadaki tum saldirilari sil</span></a></form>";
		var t=this;
	...
	*/	
	...
	BulkAddAttackLink:function(box) {
		...
		AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Bu saldiri; Bozkirlardaki tuzaklari temizlemek icin intihar saldirisidir<br />");
	...
	HideAttackEfforts:function() {
		...
		var a=document.createElement('a');
		a.innerHTML='Saldiri cabalarini Gizle/Goster';
		//a.href='javascript:;';
	...
	OnAttackBoxAppear:function(box) {
		...
		AddText(div2,'Saldiri listesinde gormezden gel');
		...
		if(attack) {
			...
			if(attack.time) {
				AddHtml(div,'Son saldiri: '+SecsToStr(nowSecs-attack.time)+' once<br />');
			}
			if(attack.comment){
	...
	//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
			...
			deleteBtn.innerHTML='<span>Saldiriyi sil</span>';
	...
	OnToolTipAppear:function(box) {
		...
		if(troops && troops.length>0) {
			...
			var nowSecs=new Date().getTime()/1000;
			var lastAttackStr='Son saldiri: '+SecsToStr(nowSecs-lastAttack)+' once'+(attack.ignore?' <b>(ignore)</b> ':'');
