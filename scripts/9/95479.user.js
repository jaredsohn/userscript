...
// @name		IkArt
// @version		1.6.0
// @namespace		http://ika-art.teknopop.com
// @description		IkArt : Ikariam avec des pУЉriodes de transitions aube, jour, crУЉpuscule et nuit selon l'heure de votre ordinateur, avec un thУЈme hivernal et amУЉlioration des graphismes...
// @author		Create and develop by Typhon with Quenquen407, Matheod, HiSt, Destructor & TeKnoPolis ...
...
// ---- !!! VOUS N'AVEZ PAS LE DROIT DE MODIFIER OU COPIER CE SCRIPT !!! ProtУЉgУЉ par : http://www.iddn.org ----

// ---- Last Edit : 15/12/2010
// ---- This script is developped for Ikariam version 0.3.2 to 0.4.1----

// ---- To contact the Creator, Graphik Creator and Script Creator ; minigrapheur@live.fr ----
// ---- To contact a Script Creator ; matheodikart@gmail.com ----
...
// -------------------------------------------------------
// ---------- DУЉfinition des variables de bases ----------
// -------------------------------------------------------

// On dУЉfinis le chemin du site
...
var reportInboxLeftSign = GM_getValue('logo', dirPath+'/logo/ikart.png');
			...
			onload	: function(responseDetails) {
				...
				if(nbMessage > lastMessageLoad && messagePerPage  > lastMessageLoad) {
					...
					if(nbMessage > lastMessageShow) {
							...
							for(i = (newNbTableTr - 4); i >= (messagePerPage * 3 + 1); i--) {
...
if(document.getElementById('island') && document.getElementById('cities')) {
	...
	for(i = 0; i <= 15; i++) {
		...
		if(city.getElementsByTagName('ul')[0]) {
			...
			var cityName = cityInfo.getElementsByTagName('li')[0].innerHTML;
			var cityLevel = cityInfo.getElementsByTagName('li')[1].innerHTML;
			cityName = cityName.substr((cityName.indexOf('</span>') + 7));
			cityLevel = cityLevel.substr((cityLevel.indexOf('</span>') + 7));
			if(cityLevel > 27) {cityLevel = 27;}
...
if(document.getElementById('options')) {
	...
	document.getElementById('ikartOnglet').innerHTML = '<a title="IkArt" href="?view=options&page=ikart"><em>IkArt</em></a>'
	...
	if(param == 'ikart') {
		...
		if(langage == 'fr') {
			...
			titleOptions = 'Options IkArt';
			optionSite = 'Site Officiel IkArt';
			siteAccess = 'Acc&eacute;der';
			optionVersion = 'Version du script';
			optionLangage = 'Langage';
			titleTheme = 'Th&egrave;me';
			optionTheme = 'Versions';
			optionLogo = 'URL logo (203*85px)';
			logoGallery = 'Galerie';
			galleryCity = 'City';
			galleryAdvisors = 'Conseillers';
			galleryDefault = 'Défaut';
			optionStretchView = 'Allonger la vue de la ville';
			optionReverseWine = 'Inverser cave et pressoir';
			optionActiveAdvisors = 'Activer les conseillers spéciaux';
			optionActiveIcons = 'Changer les icônes ressources';
			titlePeriods = 'Configuration des p&eacute;riodes';
			optionHelp = 'Aide';
			optionDefaultConfiguration = 'Configurations par défaut';
			configurationAll = 'Configuration: Aube + Jour + Cr&eacute;puscule + Nuit';
			configurationDayNight = 'Configuration: Jour + Nuit';
			configurationDay = 'Configuration: Jour';
			optionDawn = 'Aube';
			optionDay = 'Jour';
			optionDusk = 'Cr&eacute;puscule';
			optionNight = 'Nuit';
			periodStart =  '00h &agrave;';
			periodEnd =  '59h';
			titleAddOption = 'Options supplémentaires';
			optionMessagePerPage = 'Nombre de message par page';
			buttonSave = 'Sauver les param&egrave;tres !';
		}
		else if(langage == 'org') {
		...
		unsafeWindow.ikartsvg = function (allongerIsCheck, invertionIsCheck, activeAdvisorIsCheck, activeIconIsCheck, nuit1, nuit2, aube1, aube2, jour1, jour2, crepuscule1, crepuscule2, logo, iRep, iLang, messagePerPage) {
...
// -------- Layout --------
STYLE('#container #header {		background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/layout/bg_header.jpg); }');
...
STYLE('#extraDiv1 {		background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/layout/bg_sky.jpg); }');
STYLE('#extraDiv2 {		background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/layout/bg_ocean.jpg); }');

// -------- Conseillés --------
if(activeSpecialAdvisor == true) { if(repertoire == 'snow' || repertoire == 'halloween') {
...
STYLE('#island #container #mainview #cities .buildplace .claim {	background-image:url('+dirPath+'/'+repertoire+'/'+periode+'/flag_yellow.gif);	width: 29px; height: 40px; display: block; position: absolute; left: 26px; bottom: 20px; }');
...
STYLE('#island #mainview #cities .level16 div.cityimg, #island #mainview #cities .level17 div.cityimg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_7_red.gif); }');
STYLE('#island #mainview #cities .level18 div.cityimg, #island #mainview #cities .level19 div.cityimg, #island #mainview #cities .level20 div.cityimg, #island #mainview #cities .level21 div.cityimg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_red.gif); }');
STYLE('#island #mainview #cities .level22 div.cityimg, #island #mainview #cities .level23 div.cityimg, #island #mainview #cities .level24 div.cityimg, #island #mainview #cities .level25 div.cityimg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_red.gif); }');
STYLE('#island #mainview #cities .level26 div.cityimg, #island #mainview #cities .level27 div.cityimg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_red.gif); }');
...
STYLE('#island #mainview #cities .level16 div.ownCityImg, #island #mainview #cities .level17 div.ownCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_7_blue.gif); }');
STYLE('#island #mainview #cities .level18 div.ownCityImg, #island #mainview #cities .level19 div.ownCityImg, #island #mainview #cities .level20 div.ownCityImg, #island #mainview #cities .level21 div.ownCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_blue.gif); }');
STYLE('#island #mainview #cities .level22 div.ownCityImg, #island #mainview #cities .level23 div.ownCityImg, #island #mainview #cities .level24 div.ownCityImg, #island #mainview #cities .level25 div.ownCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_blue.gif); }');
STYLE('#island #mainview #cities .level26 div.ownCityImg, #island #mainview #cities .level27 div.ownCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_blue.gif); }');
...
STYLE('#island #mainview #cities .level16 div.allyCityImg, #island #mainview #cities .level17 div.allyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_7_green.gif); }');
STYLE('#island #mainview #cities .level18 div.allyCityImg, #island #mainview #cities .level19 div.allyCityImg, #island #mainview #cities .level20 div.allyCityImg, #island #mainview #cities .level21 div.allyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_green.gif) ; }');
STYLE('#island #mainview #cities .level22 div.allyCityImg, #island #mainview #cities .level23 div.allyCityImg, #island #mainview #cities .level24 div.allyCityImg, #island #mainview #cities .level25 div.allyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_green.gif) ; }');
STYLE('#island #mainview #cities .level26 div.allyCityImg, #island #mainview #cities .level27 div.allyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_green.gif) ; }');
...
STYLE('#island #mainview #cities .level16 div.treatyCityImg, #island #mainview #cities .level17 div.treatyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_7_yellow.gif); }');
STYLE('#island #mainview #cities .level18 div.treatyCityImg, #island #mainview #cities .level19 div.treatyCityImg, #island #mainview #cities .level20 div.treatyCityImg,#island #mainview #cities .level21 div.treatyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_yellow.gif); }');
STYLE('#island #mainview #cities .level22 div.treatyCityImg, #island #mainview #cities .level23 div.treatyCityImg, #island #mainview #cities .level24 div.treatyCityImg,#island #mainview #cities .level25 div.treatyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_yellow.gif); }');
STYLE('#island #mainview #cities .level26 div.treatyCityImg, #island #mainview #cities .level27 div.treatyCityImg {	background-image: url('+dirPath+'/'+repertoire+'/'+periode+'/ile/city/city_8_yellow.gif); }');
