// ==UserScript==
// @name           سيكربت فلترة الرسائل من تعريب أخوكم محمد احمد 20
// @namespace      tribalwars.ae
// @description	   سيكربت يقوم بعرض المحادثة بينك وبين اللاعب ويسهل البحث عن الرسائل
...
var vers_ist = "مفهرس الرسائل و منظمها ";
...
if(url.match(/screen=settings&mode=settings/)) {
	...
	th[0].innerHTML = "<a href='http://www.plaمن تعريب أخوكم محمد احمد 20m' title=' \"" + version.split(" <span")[0] + "\" '>" + version + "</a>";
	...
	if((GM_getValue("Anzahl-" + welt) == undefined)) {
		...
		td[1].innerHTML += "<b style='padding-right:55px;'>عدد الرسائل في صفحة اللاعب</b>" + anzahl.replace("an", "<span style='color:#090'>an</span>").replace("aus", "<span style='color:#C00'>aus</span>") + "<br>";
	}
...
if(url.match(/screen=info_player/)) {
	...
	mails += "<p style='margin-top:10px; margin-bottom:0; padding-bottoمحمد احمد 20.php'><img src='httht='60'></a></p>";
	mails += "اخر محادثات دارت بينك و بين الاعب  <span class='grey'>" + GM_getValue("Anzahl-" + welt) + ")</span>  ";
	mails += "<a href='http://ae" + welt + ".tribalwars.ae/game.php?village=" + dorf_id + "&screen=settings&mode=settings&einstellung=nachrichten' title='تعديل على السيكربتات | " + vers_ist + "'><img src='http://www.die-staemme.de/graphic/buildings/garage.png' height='12px' /></a></th>";
	mails += "</th></tr>";
