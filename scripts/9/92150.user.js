// ==UserScript==
// @name           CityVille Wall Manager
// @namespace      http://userscripts.org/users/260789
// @description    Manages CityVille wall posts
// @include        http://*.facebook.com/home.php?filter=app_291549705119*
// @include        https://*.facebook.com/home.php?filter=app_291549705119*
...
var imgs = {
bg : "http://i55.tinypic.com/b4t2qg.jpg",
...
gameID : "291549705119", // game id
gameURLpart : "cityville", // game url folder for apps.facebook.com/HERE/ (only some games have this)
whitelistWhenPaused : "", // categories separated by commas to be accepted even if gift box is full
gameName : "CityVille",
gameAcronym : "CityVWM",
gameKeyUrlKeyword : "sendKey=", // used in the regex and xpath to look for "key=" or "sk=" or whatever it may be
xpQueryTextExcludes : ["Help friend out", "Share", "Play Fishville now", "found some", "is trying to", "just earned the", "has been rewarded", "needs medicine in", "found a", "is giving away", "has a stack of", "could use a little bit of love", "has given Love to"],

// empty options object for later modification
opts : {},

// all regexps are stored here
whichRegex : /(bonus|oyster|adopt|medicine|mine|hatch|ticket|rare fish eggs|love|reward)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
colorRegex : /(white|black)/,
creatureRegex : /(manta ray|sea ?turtle|sea ?horse|baby ?snake)/,
...
accText : {
			...
			oyster : "Got this oyster!",
			adopt : "Adopted this creature!",
			hatch : "Helped hatch this egg!",
			unknown : "Adopted this unknown creature!",
			medicine : "Got this medicine!",
			eggitem : "Got this bonus egg item!",
			gemstoneitem : "Got this gemstone bonus item!",
			gemstones : "Got these gemstones!",
			fantaseaitem : "Got this fantasea bonus item!",
			ticket : "Got this scratch ticket!"
		},
...
switch(w) {
case "love": w="bonus"; break;
case "mine": if(text.find("Easter Egg")) w="eggitem";
				else if(text.find("Gemstones")) w="gemstoneitem";
				else w="fantaseaitem"; break;
case "reward": if(text.find("Gemstones")) w="gemstones";
					else w="bonus"; break;
case "rarefisheggs": w="hatch"; break;
}
return w;
},
...
var w=e.textContent.toLowerCase().match(main.creatureRegex), color=main.whichColor(e.textContent.toLowerCase()), text=e.parentNode.parentNode.parentNode.textContent;
...
if($("GM_config") || main.currReqs >= main.opts["maxrequests"]) return;
		...
		var opts=main.opts, open=main.open, accText=main.accText, getKey=main.getKey,
			which=main.which, whichCreature=main.whichCreature, whichEgg=main.whichEgg,
...
case false: if(!$(key)) {
	switch(w) {
	case "adopt": if(opts["adopt"] == true && opts[wA] == true) open(item.href, key, w);
					else item.setAttribute("id", "item_skip_"+key); break;
	case "ticket": if(opts["ticket"] == true) {
						newTab(item.href, key);
...
// pre-load the config
GM_config.init("FishVille Wall Manager v"+version, {
	bonus : {
	...
	adopt : {
		label : "Adopt Creatures? (master option)",
		type: "checkbox",
		"default" : true,
		kids : {
			unknown : {
	...
	eggitem : {
		label : "Grab Spring Bonus Items?",
		type: "checkbox",
		"default" : true,
	},
	gemstoneitem : {
		label : "Grab Gemstone Bonus Items?",
		type: "checkbox",
		"default" : true,
	},
	gemstones : {
		label : "Get extra gemstones?",
		type: "checkbox",
		"default" : true,
	},
	fantaseaitem : {
		label : "Grab Fantasea Bonus Items?",
		type: "checkbox",
		"default" : true,
	},
	ticket : {
		label : "Open scratch tickets in new tabs?",
		type: "checkbox",
		"default" : false,
	},
	mantaray : {
		section : ["Specific Creature Adoption"],
		label : "Manta Ray",
		type : "checkbox",
		"default" : false,
	},
