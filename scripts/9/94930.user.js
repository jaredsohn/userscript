...
var oaVersion = "v2.4";
// ** Email Timeout. This is how long it will wait until it tries to close the window (tab) that the emailer opened. Default is 5 seconds = 5000
var EmailWindowTimeout = 5000;
...
var MIN = 300;  // Minmum number of seconds to wait until reload overview. Default = 14 minutes = 840 seconds
var MAX = 600; // Maximum number of seconds to wait until reload overview. Default = 30 minutes = 1800 seconds
...
function oaSendMail(from,subject,content) {
	...
	tElement1.setAttribute("value",subject);  //Used here to show amount of time until the attack hits. 
...
function checkForAlarm(tData, emailOK, attackOK, espionageOK){
	...
	var hrs  = 0;
	var days = 0;
	var theContent = "Incoming Attack in ";
	...
	for ( var i = 0 ; i < tEvents.length ; ++i ) {
		...
		if ( espionageOK && (theSound != attackSound) && (tMission[0].firstChild.src.indexOf("spionage") != -1) ) {
			...
			theSound = probeSound;
		} else if ( (emailOK || attackOK) && (tMission[0].firstChild.src.indexOf("angriff") != -1) ) {
			if ( emailOK ) {
				secs = theTimes[parseInt(tPlayerType[0].id.split("-")[1])];
				if ( isNaN(secs) ) {
					theAttackTime = "Unknown Attack Time";
					theContent = "You have Incoming attack, fleet details not available";
				} else {
					mins = 0;
					hrs  = 0;
					days = 0;
					...
					theContent +=  theAttackTime + "!\nArrival Time: " + tEvents[i].getElementsByClassName("arrivalTime")[0].innerHTML + "\nShips: " + parseInt(oaTrim(tEvents[i].getElementsByClassName("detailsFleet")[0].innerHTML)) + "\nFrom: " + tEvents[i].getElementsByClassName("originFleet")[0].innerHTML + " " + tEvents[i].getElementsByClassName("coordsOrigin")[0].firstChild.innerHTML + "\nTo: " + tEvents[i].getElementsByClassName("destFleet")[0].innerHTML + " " + tEvents[i].getElementsByClassName("destCoords")[0].firstChild.innerHTML;
				}	
			}
	...
	if ( theAttackTime != "none" ) oaSendMail("Attack@" + theAttackTime + "." + theServer,  "OGame Incoming Attack", theContent);
}

function oaAlarmCheck (tOnOffs){
	var sUrl = document.getElementById("eventdetails").getElementsByClassName("tipsStandard")[0].href.split("/");
	sUrl = sUrl[sUrl.length-1];
	bLog("Reloader = " + tOnOffs[0] + ", Emailer = " + tOnOffs[1] + ", Attack = " + tOnOffs[2] + ", Espionage = " + tOnOffs[3] + ", Messages = " + tOnOffs[4]);
	theSound = 0;
	var i;
	if ( tOnOffs[4] && (parseInt(oaTrim(document.getElementById("message-wrapper").getElementsByTagName("span")[0].innerHTML)) > 0) ) theSound = messageSound;
	var emailOK		= tOnOffs[1];
	var attackOK	= tOnOffs[2];
	var espionageOK	= tOnOffs[3];
	if ( emailOK || attackOK || espionageOK ) {
		var tEventFrame = document.getElementById("iframeEventBox");
		if ( tEventFrame ) checkForAlarm(tEventFrame.contentWindow.document.body.innerHTML, emailOK, attackOK, espionageOK);
		else {
...
function oaSetServer () {
	theServer = location.hostname.replace(/\.|ogame/g,"");
	theLang = document.getElementById("playerName");
...
function setVal( vName, vValue ) {
    GM_setValue((theServerString+vName), vValue);
    return true;
}

function getVal( vName, vValue ) {
    return GM_getValue((theServerString+vName), vValue);
}

function setOnOff(key, value) {
    var options = getVal('oaOnOffs', '42');
	if (options == '42') options = [true,false,true,true,true];
	else options = options.split(",");
    options[key] = value;
	options = options.join(",");
    setVal('oaOnOffs', options);
	return options;
}
...
function oaLoad () {
	...
	for ( i = 0 ; i < 5 ; ++i ) tOnOffs[i] = (tOnOffs[i] == "true") ? true : false;
	oaAlarmCheck(tOnOffs);
	var leftMenu = document.getElementById("menuTable");
	if ( leftMenu == null ) {
		...
		for ( var i = 0 ; i < 5 ; ++i ) {
			...
			tLI.innerHTML = '<span class="menu_icon" style="cursor:pointer"><img id="'+i+'oaOnOff" src="' + (tOnOffs[i] ? onPic : offPic) + '"></span><a class="menubutton" href="javascript:" accesskey="" target=""><span class="textlabel">'+aLang[i]+'</span></a>';
...
// Uncomment the below to send a test email on each load.

//  oaSendMail("OGame@Alarm.Cheese","This is a test email","One Cheesy Test");
