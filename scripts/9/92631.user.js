...
/*
	...
	2010-04-13	v0.99	First release of Brutus Helper, a combination of Collect Boosts and Job Helper.
	2010-04-18	v1.00	Updated release, fixed a few typos here and there.
	2010-04-19	v1.01	Rewrote ajax calls to use jQuery syntax instead of do_ajax.
	2010-07-22  v1.05	Added Las Vegas social help
	2010-08-01  v1.06	Fix for latest Zynga changes
	2010-08-23  v1.07	Fix for latest Zynga changes with exp ui
	2010-09-17	v1.08	Added Armory
	2010-09-19	v1.09	Added Loyalty Points
	2010-10-17	v1.10	Fix for exp_to_next_level and added Italy
*/

javascript:(function (){
var version='Brutus Helper v1.10 beta';
try {
  if (document.getElementById('app10979261223_iframe_canvas')) {
	...
	fb_url='http://www.facebook.com/profile.php?id=',cuba_gained=0,ny_gained=0,moscow_gained=0,bangkok_gained=0,vegas_gained=0,italy_gained=0,exp_gained=0,friends_helped=0;
	...
	redo="";
	selectable = ['','New_York','Level','Cuba','Iced','Moscow','Money_Laundry','Moscow_Boss','Chop','Bangkok_Boss','Vegas','Italy','Weapons','Armory','zoo','Robbery','Loyalty','Achievement'];
	...
	'<form name="spockform">'+
		'<table class="messages">'+
			...
			for(var i=1;i<selectable.length;i++){
				config_html += '<td '+(i%2==0&&i>0?'colspan="2"':'')+'><input type="checkbox" name="'+selectable[i]+'" id="'+selectable[i]+'" '+(selectable[i]=='Achievement'||selectable[i]=='Loyalty'||selectable[i]=='Money_Laundry'||selectable[i]=='Vegas'?'':'checked')+'/>&nbsp;'+selectable[i]+' '+(selectable[i]=='Money_Laundry'?'<span class="bad">Not working</span>':'')+'</td>';
	...
	function start(){
		...
		if (document.spockform.Italy.checked) { cities.push('Italy'); }
		...
		if (document.spockform.Armory.checked) { cities.push('Armory'); }
		if (document.spockform.zoo.checked) { cities.push('zoo'); }
		if (document.spockform.Loyalty.checked) { cities.push('Loyalty'); }
	...
	function log(s) {
		...
		if (italy_gained > 0) { money_gained += ' &nbsp; <span class="good">L$'+commas(italy_gained)+'</span>  '; }
	...
	function joblink() {
		...
		if (oldcity == "Achievement") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=index&xw_action=ach_celeb&sharer='+friends[0]+'&aid='+eval(ach-1)+'&skip_interstitial=1&skip_req_frame=1">Achievement</a>]</span>'; }
		if (oldcity == "Stash") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=job&xw_action=collect_loot&sharer='+friends[0]+'&skip_interstitial=1&skip_req_frame=1">Secret Stash</a>]</span>'; }
		...
		if (oldcity == "Armory") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=propertyV2&xw_action=cs_help_final&target='+friends[0]+'&building_type=13&skip_interstitial=1&skip_req_frame=1">Armory</a>]</span>'; }
		if (oldcity == "zoo") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=propertyV2&xw_action=cs_help_final&target='+friends[0]+'&building_type=14&skip_interstitial=1&skip_req_frame=1">zoo</a>]</span>'; }
		...
		if (oldcity == "Loyalty") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=index&xw_action=crm_levelup_claim&friend_id='+friends[0]+'&skip_interstitial=1&skip_req_frame=1">Loyalty</a>]</span>'; }
		if (oldcity == "Italy") { return '<span class="more_in">[<a href="'+MWURL+'xw_controller=story&xw_action=give_help_social&target_id='+friends[0]+'&job_city=6">Italy</a>]</span>'; }
	}
	...
	function help_url(c) {
		...
		if (c == "Italy") { return 'html_server.php?xw_controller=story&xw_action=give_help_social&target_id='+friends[0]+'&job_city=6'; }
		...
		if (c == "Armory") { return 'html_server.php?xw_controller=propertyV2&xw_action=cs_help_final&target='+friends[0]+'&building_type=13'; }
		if (c == "zoo") { return 'html_server.php?xw_controller=propertyV2&xw_action=cs_help_final&target='+friends[0]+'&building_type=14'; }
		if (c == "Robbery") { return 'html_server.php?xw_controller=robbing&xw_action=mastery_bonus&target='+friends[0]; }
		if (c == "Loyalty") { return 'html_server.php?xw_controller=index&xw_action=crm_levelup_claim&friend_id='+friends[0]+'&friend='+friends[0]; }
	}	
	...
	function stats_ratios(s) {
		...
		document.getElementById('user_cash_italy').innerHTML = /user_cash_italy.*?"(.*?)"/.exec(s)[1];
		//expnow = parseInt(/user_experience.*?(\d+)/.exec(s)[1]);
		//expnext = parseInt(/exp_for_next_level.*?(\d+)/.exec(s)[1]);
		//expneed = eval(expnext-expnow);
		expneed = parseInt(/exp_to_next_level.*?(\d+)/.exec(s)[1]);
		...
		try {
			local_xw_sig = /local_xw_sig = '([a-f0-9]+)'/.exec(s)[1]; 
				if (/loot_confirmed=yes.*?incnt_choice/.test(s)) {
					...
					else if (m=/You got a bonus in celebration.*money">(R?C?V?L?)(\$[\d,]+)/.exec(s)) {
						log('Received '+m[1]+m[2]+' from '+friends[0]+' achievement '+ach);
						//if (m[3]) { addloot(m[3]); }
					}
					else if (m=/You got a bonus in celebration.*money">(R?C?V?L?)(\$[\d,]+).*>(\d+) ?Experience<\/div>/.exec(s)) {
					...
					else if (m=/You received (\d+) Loyalty Points/.exec(s)) {
						log(timestamp()+'<span class="good">Received '+m[1]+' loyalty points!'+joblink());
					}
					...
					if (m=/your cut: ([$\d,]+)/.exec(s)) {
						...
						if ((jobsdone[oldcity] == 25) && (oldcity == 'Italy')) {
							validcities.splice(validcities.indexOf(oldcity),1);
							log(timestamp()+'<span class="bad">'+oldcity+' limit reached</span>, not checking for more jobs there. '+joblink());
						}
					}
					...
					else if (/You have already received your free loyalty points for the day./.test(s)) {
						validcities.splice(validcities.indexOf(oldcity),1);
