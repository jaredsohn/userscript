...
// @description   Adds a link to play videos from Vimeo in XBMC
// @date          2011-01-17
// @creator       Erik T.
// @include       *vimeo.com/*
// ==/UserScript==
// Simple script to send Vimeo movies to XBMC, uses the Vimeo plugin so it should be installed to make it work
...
function play_movie() {
 ...
 var video_url;
 video_url = document.getElementById('clip_id')['value']

 if(video_url != undefined)
 {
  GM_log('Playing video :' + video_url);
  setTimeout( function() {
   GM_xmlhttpRequest({
    method: 'POST',
    url: 'http://' + xbmc_address + '/jsonrpc',
    headers: 'Content-type: application/json',
    data: '{"jsonrpc": "2.0", "method": "XBMC.Play", "params":{ "file" : "plugin://plugin.video.vimeo/?action=play_video&videoid=' + video_url + '" }, "id" : 1}'
   })
  }, 250);
 }
}
...
try {
 var clip = document.getElementById('clip_id')['value'];
} catch(Exception) {}
