...
//				13:{'name':"-------",'x':375,'y':375},
  ...
  clickedSearch : function (){
   ...
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
		...
		m+= '<TR><TD class=xtab align=right>Friendly Only:</td><TD class=xtab><INPUT id=pafilFriendly type=CHECKBOX '+ (Options.friendlyOnly?' CHECKED':'') +'\><td></tr>';
  ...
  dispMapTable : function (){
    ...
    for (i=0; i<t.mapDat.length; i++){
      ...
      if (t.opt.searchType==2 && type==7 ) {
        if (!Options.friendlyOnly || t.mapDat[i][12] == 'f') {
