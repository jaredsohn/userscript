...
var cpos = [ 8, 37 ];
var map = {
	url: "http://images3.wikia.nocookie.net/__cb20110127194405/deadfrontier/images/6/6c/Krnellmap.png",
	sq : 50,
	offsetX: 51,
	offsetY: 1,
	width: 44,
...
genbutt("Up",function() { if(cpos[1] == map.height) { return; } cpos[1]++; redraw(); });
genbutt("Down",function() { if(cpos[1] == 1) { return; } cpos[1]--; redraw(); });
genbutt("Left",function() { if(cpos[0] == 1) { return; } cpos[0]--; redraw(); });
genbutt("Right",function() { if(cpos[0] == map.width) { return; }  cpos[0]++; redraw(); });
nbut++;
genbutt("Checked",function() { setMapMark(cpos,"cleared"); });
genbutt("Clear",function() { clearMapMarks(); });
nbut++;
genbutt("Refit",function() { refit(); redraw(); });
