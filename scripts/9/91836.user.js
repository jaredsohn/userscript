...
var MAP_DELAY = 200;
var CANCEL_TRAINING_DELAY = 200;   // without a delay, some (recoverable) 'something has gone wrong' errors occur. The server is probably delaying SQL updates on cancels.
  ...
  eventGotPlayerList : function (rslt){
    ...
    for (k in rslt.matchedUsers){
      ...
      m += '<TR valign=top><TD>'+ u.genderAndName +'</td><TD></td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><DIV onclick="PTpd(this, '+ u.userId +')"><A>details</a></div></td></tr>';
    }
    m += '</table></div>';
