// ==UserScript==
// @name           Listeni Davet Et (Tek Tuşla)
// @namespace      MsNKoPaTCoCuK
// @description    Bu Script ile Tek Tuşla davet Edebiliceksiniz :)
...
function addbutton(){
	var slist=document.getElementById('_view_all')?document.getElementById('_view_all').parentNode:undefined;
		if(slist!==undefined){
			...
			link.setAttribute('onclick','txtc=this.firstChild.firstChild.firstChild.firstChild;if(txtc.textContent=="Tümünü Seç"){txtc.textContent="Seçilenleri Sil";}else{txtc.textContent="Tümünü Seç";}var friends=document.getElementById("all_friends").getElementsByTagName("li");for(x=0;x<friends.length;x++){if(typeof friends[x]==="object"&&friends[x].className!="disabled"){fs.click(friends[x]);}}return false;');
			...
			var txt=document.createTextNode('Tümünü Seç');
