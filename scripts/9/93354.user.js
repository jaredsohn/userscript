...
var BetterGSInsert = function($, window, undefined) {
			BetterGS = {};
			BetterGS.version = '1.481'; 
			BetterGS.init = function() {
				var styles = 
					'<style>' +
						'#BetterGS-options-caller { background-position: 48px -155px; background-repeat: no-repeat; color: #eee; float: right; font-weight: bold; margin: 6px; padding-right: 20px; } ' +
					'</style>';
					
				$('head').append(styles);
				
				$(window).load(function() {
					$('#header_userOptions').before('<a id="BetterGS-options-caller" href="#" class="header_user_button main_asset">BetterGS</a>');
					$('#BetterGS-options-caller').bind('click', function(ev) {
						ev.preventDefault();
				...
				if(store.get('BetterGS').disablezoomwarn) {
					BetterGS.disableZoomWarn.init();
				}
			 ...
			 */
			BetterGS.options = {};
			BetterGS.options.defaults = {'lyrics' : 1, 'autosearch' : 0, 'tabtitle' : 1, 'adremoval' : 1, 'disablezoomwarn': 1};
			...
			BetterGS.options.init = function() {
				var conf =  !!store.get('BetterGS') ? store.get('BetterGS') : store.set('BetterGS', BetterGS.options.defaults) || BetterGS.options.defaults,
					...
					'<div id="BetterGS-modal"><div id="BetterGS-optionspanel">' +
						...
						'<h2><a href="http://userscripts.org/scripts/show/93354">Better Grooveshark v'+ BetterGS.version +'</a></h2>' +
						...
						'</div>' +
						'<div class="BetterGS-option">' +
							'<h3>Disable zoom warning</h3>' +
							'<div class="BetterGS-option-row">' +
								'<input id="BetterGS-options-disablezoomwarn" type="checkbox" '+ (conf.disablezoomwarn ? 'checked' : '') +' /><label for="BetterGS-options-disablezoomwarn">Disables the "out of focus" warning when zooming the page</label>' +
							'</div>' +
						'</div>' +
			 ...
			 */
			BetterGS.hideAds = {};
			BetterGS.hideAds.version = '1.1';
			BetterGS.hideAds.init = function() {
				var styles = '<style> ' +
				'#application { margin-right: 0px !important; } ' +
				'#page_content_pane { width: auto !important; } ' +
				'.noResults_pane { width: 100% !important; } '+
				'#capital, .capital, .capitalWrapper_728, #page_content_pane + .page_column_capital, .page_column + .page_column_capital, #capitalBelowFoldHelper_300, #capitalAboveFoldHelper_300, #searchCapitalWrapper_728, #searchCapitalWrapper_300, #playlistCapitalWrapper_300, .capitalAboveFoldHelper_300 { display: none !important; } ' +
				'</style>';
				$('head').append($(styles));
			};
			/**
					...
					'<div id="BetterGS-content">' +
						'<div id="BetterGS-interact">' +
							'<div class="BetterGS-row BetterGS-head">' +
								...
								'<h3 id="BetterGS-head"><a target="_blank" href="http://userscripts.org/scripts/show/93354">Lyrics</a></h3>' +
							'</div>' +
			...
			BetterGS.lyrics.getLyricsQuery = function(href) {
				return 'SELECT * FROM html WHERE url="' + href + '" AND xpath="'+ BetterGS.lyrics.getCurrentSiteDefs().xpath +'"';
			};		
			...
			BetterGS.lyrics.handleClicks = function() {
				$('#BetterGS-wrap').bind('click', function(ev) {
					...
					if($(ev.target).attr('href') == 'http://userscripts.org/scripts/show/93354') {
						return true;
					}
			...
			BetterGS.disableZoomWarn = {};
			BetterGS.disableZoomWarn.init = function() {
				$(window).load(function() {
					GS.Controllers.PlayerController.prototype.onZoomChange = function() {};
				});
			};
			BetterGS.init();
		};
