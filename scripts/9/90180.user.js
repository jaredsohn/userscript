...
//
// ==UserScript==
// @name			Fluent Super Next Page
// @author			antiufo
// @version			0.2.3.3
// @date				2010-11-10
// @id					fluent-super-next-page
// @homepageURL 		http://at-my-window.blogspot.com/?page=fluent-next-page
// @namespace	antiufo
// @description    Seamlessly switches to the next page (with prefetch) when you scroll to the bottom of a page
// @description 	Based on Super Next Page
// @scriptsource 		http://userscripts.org/scripts/show/90180
// @noframes
// @include          http://*
// @include          https://*
...
//==================================================================
	//参数设置
	var URL = 'http://userscripts.org/scripts/show/90180';
	var VERSION = '0.2.3';
 ...
 */
	...
	IconHelper.prototype.initHelp = function() {
	    ...
	    versionDiv.innerHTML = '<a href="' + URL +
	        '">Fluent Super Next Page</a> ' + VERSION
...
var historyManage = (function(){
	...
	var capturedKeys=[38, 40, 33, 34, 35, 36];
	var lastScrollEventAlreadyHandled=false;
	var previousEventWasAtBottom=false;
	var remainingDummyTicksBeforeEnd=0;
	var remainingDummyTicksAfterBegin=0;
	window.addEventListener('DOMMouseScroll', onMouseWheelTick, true);
	window.addEventListener('scroll', onScroll, true);
	window.addEventListener('keydown', function(event){
		if(event.ctrlKey || event.shiftKey || event.metaKey || delay || capturedKeys.indexOf(event.keyCode)==-1) return;

		GM_log("keydown")
		onPageUpDown(event);
	 }, true);

	function isAtBottom(bias){
		if(!bias) bias=0;
		if(window.scrollMaxY === 0) return false;
		return window.pageYOffset+bias+2 >= window.scrollMaxY;
	}
	...
	function onScroll(){
		if(lastScrollEventAlreadyHandled){
			lastScrollEventAlreadyHandled=false;
			return;
		}
		GM_log("Scroll CONTINUOUS: "+isAtBottom())
		if(isAtBottom()){
			if(!nextPageTimer){
				GM_log("Reached bottom (continuous)"+" "+scrollMaxY+" "+pageYOffset)
				openNextPage();
			//	nextPageTimer = window.setTimeout(openNextPage, 1000)
			}
			previousEventWasAtBottom = true;
		}else{
		/*	if(nextPageTimer){
				window.clearTimeout(nextPageTimer);
				nextPageTimer=0;
			}*/
			previousEventWasAtBottom = false;
		}
	}
	
	function onPageUpDown(e){
		var bias;
		if(e.keyCode==38) bias=-19;
		if(e.keyCode==40) bias=+19;
		if(e.keyCode==33) bias=-(window.innerHeight - 38);
		if(e.keyCode==34) bias=+(window.innerHeight - 38);
		if(e.keyCode==36) bias=-Infinity;
		if(e.keyCode==35) bias=+Infinity;
		remainingDummyTicksAfterBegin=0;
		if(onScrollSteps(bias, false)) e.preventDefault();
	}
	
	function onMouseWheelTick(e){
		if(onScrollSteps(e.detail * 152 / 8, true)) e.preventDefault();
	}
	
	
	function onScrollSteps(bias, usingTicks){
		lastScrollEventAlreadyHandled=true;
		GM_log("Scroll STEPS "+isAtBottom(bias)+" "+scrollMaxY+" "+pageYOffset+"+"+bias+"="+(pageYOffset+bias))
		
		if(remainingDummyTicksAfterBegin > 0){
			GM_log("Ignored after begin: "+remainingDummyTicksAfterBegin)
			remainingDummyTicksAfterBegin--;
			return true;
		}
		...
		if(isAtBottom(bias)){
			...
			if(!usingTicks) remainingDummyTicksBeforeEnd=0;
		
			if(previousEventWasAtBottom && remainingDummyTicksBeforeEnd==0){
				previousEventWasAtBottom=false;
				remainingDummyTicksAfterBegin=2;
				GM_log("Reached bottom (steps)")
				openNextPage();
				return true;
			}else{
			
				if(remainingDummyTicksBeforeEnd>0) remainingDummyTicksBeforeEnd--;
				
				previousEventWasAtBottom = true;
			}
		}else{
			previousEventWasAtBottom = false;
			remainingDummyTicksBeforeEnd = 2;
		}
		return false;
	}
