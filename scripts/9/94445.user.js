...
contentEval(function() {
	window.addEvent('domready', function() {
		...
		/**
		 * returns item which maximizes provided skill and is of provided type
		 *
		 * params:
		 * 	type: 
		 * 		"body" | "yield" | "neck" | "foot" | 
		 * 		"head" | "right_arm" | "left_arm" | "animal"
		 * 	skill: 
		...
		/*
		var getBestItem = function(type, skill) {
			//check params if they are valid
			if (type == undefined || skill == undefined) { 
				throw new Error("You must provide both params type and skill");
			}
			var skill_exists = false;
			for (var attribute in Character.skill_names) {
				if (Character.skill_names[attribute].indexOf(skill) != -1) { skill_exists = true; break; }	
			}
			if (!skill_exists) { 
				throw new Error("Unknown skill");
			}
			if (typeof(type) != "string" || (typeof(type) == "string" && !["body","yield","neck","foot","head","right_arm","left_arm","animal"].contains(type))) {
				throw new Error("Unknown wear type");
			}
			var bestItem = null;	
			var map = getItemsMap();
			map[type].each(function(item) {
				var obj = item.item.obj;
				var bonus = item.getSkillBonus(skill);
				if ((bestItem == null) || (bestItem != null && bestItem.getSkillBonus(skill) < bonus)) { 
					bestItem = item; 
				}
			});
			return bestItem;
		...
		var getBestJobItems = function(requiredSkills, jobId) {
			...
			if (!isObjectEmpty(map.sets)) {
				...
				for (var setName in map.sets) {
					...
					if (set.length <= 1) {
						if (debug) appendToLog("\tSkipping set '" + setName + "' because you have only 1 item from it (and therefore no bonus)");
						continue;
					}

					if (debug) appendToLog("Processing set '" + setName + "' ...");
			...
			if (debug) {
				...
				appendToLog('Total bonus is: +' + (!isObjectEmpty(bestCombination) ? bestCombinationBonus : totalBonusUsingWearTypes));
			}
			return bestItemsMap;
		};
...
//			}
		 ...
		 * 	</a>
		 */
		var createCalculationButton = function(taskSkills, jobId, title_suffix) {
			var button = createButton(lang.getPhrase("button_best_items") + (title_suffix ? (" - " + title_suffix) : ""));
			var click_handler = function(event) {
				//button.removeEvent("click", click_handler);
		...
		var processJobWindow = function(url) {
			...
			switch (params["window"]) {
				...
				case "cityhall_build":
					...
					appendElement(createCalculationButton(['build','build','build','repair','leadership']), $("window_cityhall_build_" + params["building"] + "_content").getElements('p')[0]);
					break;
				case "headquarter_build":
					...
					appendElement(createCalculationButton(['build','build','build','repair','leadership']), $("window_headquarter_build_" + params["fortbuilding"] + "_content").getFirst().getFirst());
					break;
				case "duel":
					if (debug) {
