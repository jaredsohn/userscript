// ==UserScript==
// @name             KOCAttack FR - Extra Features!
// @version          0.9.5.2
// @namespace        KOCAttack-Extra
// @description      Traduction de KOC Attack Extra en FR par BeWorld
// @homepage         http://userscripts.org/scripts/show/99495
...
// @require          http://tomchapin.me/auto-updater.php?id=99495
// ==/UserScript==
...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		var m = '<a id="ImportBoxCancel" class="button20"><span>Annuler</span></a>';
		m += '<a id="ImportData" class="button20"><span>Importer</span></a>';
		m += '<a id="ExportData" class="button20"><span>Exporter</span></a>&nbsp;';
	...
	ShowViewAttacksDialog:function() {
		...
		//WinLog.write(inspect(cities));
		var citysel ='<select id=srcCity>';
		citysel += '<option value=All>Toutes les villes</option>';
		for(var c=0; c<cities.length; c++) {
		...
		levelsel += '<option value="All">Tout les niveaux</option>';
		for(var c=1; c<=11; c++) {
			levelsel += '<option value="'+c+'">Niv '+c+'</option>';
		}
		levelsel += '</select>';
		
		var typesel ='<select id=srcAttackType>';
		typesel += '<option value="All">Tout</option>';
		typesel += '<option value="normal">Attaques</option>';
		...
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Rechercher : </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Tout</option>\
		<OPTION value=Camp>Barbares</option>\
		<OPTION value=Wilderness>T.S.</option>\
		<OPTION value=Grassland>Prairie</option>\
		<OPTION value=Lake>Lac</option>\
		<OPTION value=Mountains>Montagne</option>\
		<OPTION value=Forest>Bois</option>\
		...
		<td class=xtab width=100 align=center>Selectionner une ville : &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Selectionner Niveau: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Selectionner Type: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>Annuler</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Liste des Attaques</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Nettoyer la Liste</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Supprimer la selection</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>Ville</td><td>Coords</td><td nowrap>Cible</td>';
		  h += '<td>Type</td><td>Unit&eacute; Attaque</td><td>Unit&eacute; Suicide/Ressources</td><td>Dist<td>Temps</td><td>Ignorer</td></tr>';
		  var tableRows= '';
		  var count = 1
	  ...
	  tableRows += '<td>'+parseInt(distance)+'</td>';
          ...
          ById('KOCAttackViewAttacksDelete').addEventListener('click',function() {
			...
			if (deletes>0)window.alert(deletes+' des coordonnees ont ete effacees');
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Ordre d'attaque : <select id='KOCAttackOrder'><option value='mostTroops'>Utiliser le plus de troupes en premier</option><option value='closest'>Cibles les plus proches en premier</option></select><br />"+
			"Type d'attaque : <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Barbares "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Terres Sauvages  "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Villes "+
			"<input type='checkbox' "+(this.options.attackTypeTransport?'checked':'')+" id='KOCAttackTypeTransport'>Transport<br> "+
			"<br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> secondes entre deux attaques<br />"+
			"Temps avant de re-attaquer la <u>m&ecirc;me cible</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Attaque sur Barbares toute les <input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' /> heures<br />"+
			"Attaque sur TS toute les <input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' /> heures<br />"+
			"Attaque sur Villes toute les <input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' /> heures<br />"+
			"Transport tout les <input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' /> minutes"+
			" Si transport, essayer de garder un minimun <input id='KOCAttackTransportReserveAmount' value='"+this.options.transportResourcesReserveAmount+"' size='10' /> de chaque ressources sur chaque ville <font color=#FF0000>(NOUVEAU)</font>"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distance max d\'attaque depuis la ville.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Lancer les attaques depuis la ville ou elles ont &eacute;t&eacute; lanc&eacute;es en premiers.<br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> Enlever le chevalier quand c\'est pas une attaque.<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> R&eacute;essayer si une erreur inconnue ou un trafic excessif survient (Rafraichir apres avoir chang&eacute; cette option).<br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Arreter les attaques aitomatiques sur une alerte d\'attaque entrante.<span style='color:red'> (Cela ne relancera pas l\'attaque automatique !!)</span><br />"+			
			"Ouvrir cette adresse dans un onglet quand je suis attaqu&eacute;...<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Supprimer automatiquement les rapports d\'attaques sur camps barbares / terres sauvages / transport.<br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Supprimer automatiquement les rapports d\'attaque farm<br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Nombre maximum de rapports d\'attaque &agrave; conserver dans la boite de messages.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> D&eacute;sactiver l'affichage des rapports, la suppression des rapports d'attaques sera &eacute;galement d&eacute;activ&eacute;e.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de la population inactive disponible avant d\'initier la formation automatique.<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de bonheur avant d\'initialiser la collecte d\'or automatique<br />"+
			"</td></tr>"+
			
		    "<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackDisableInviteFriends' type='checkbox' "+(this.options.disableInviteFriends?'checked':'')+" /> D&eacute;activer les messages \"Invite Friends\" du jeu.<br />"+
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Aider automatiquement les membres de l\'alliance avec la construction/la recherche...<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Masquer les demandes d\'aide dans le tchat (si la case ci-dessus est coch&eacute;e, le faire apres les avoir aid&eacute;s).<br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Publier automatiquement sur votre mur de facebook.<br />"+
			"Niveau de confidantialit&eacute; voulez-vous ? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tout le monde</option><option value='50'>Amis et leur amis</option><option value='40'>Amis seulement</option><option value='10'>Moi uniquement</option></select><br />"+
			"</td></tr>"+
			...
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> Activer les erreurs (log) dans la console Firefox (useful if trying to debug a problem or if you are submitting details along with a bug report).<br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> secondes entre le changement de villes.<br />"+
			...
			"<input id='KOCAttackCitiesDelay' value='"+this.options.waitAfterCitiesDone+"' size='3' /> secondes avant de rafraichir.<br />"+
			"Lancer les attaques depuis les villes : "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' /> % d\'ajustement al&eacute;atoire des retards (pour avoir l\'air plus naturel).<br />"+
			"Import/Export: Coller ou copier les parametres ici...<br /><textarea id='KOCAttackImport' row=1></textarea>"+
			"<a class='button20' id='KOCAttackImportButton'><span>Importer</span></a> <a class='button20' id='KOCAttackExportButton'><span>Exporter</span></a>"+
			"</td></tr>"+

			"</table>"+
			
		"<TABLE width=100%><TR><TD align=center>"+
			"<a id='KOCAttackOptionsSave' class='button20'><span>Enregistrer</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Annuler et Fermer</span></a></td></tr><tr><td><a id='KOCAttackOptionsReset' class='button20'><span>Reinitialiser les options</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>TOUT Reinitialiser !</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Supprimer Toutes les attaques</span></a>"+
 		    "<a id='KOCAttackDeleteTransports' class='button20'><span>Effacer les transports</span></a>"+
		    //"<a id='KOCAttackListAttacks' class='button20'><span>Liste attaques</span></a><a id='KOCAttackListTransports' class='button20'><span>Liste des transports</span></a>"+
            "</td><TD align=right>"+ KOCAversion +"</td></tr></table></form>";
		var t=this;
	...
	AddViewAttacksLink:function() {
		...
		a=this.AddTabLink('Voir Attaques');
		if(!a) {
			window.setTimeout(function() {
	...
	*/
	...
	SetAttackStatusMessage:function() {
		...
		if(autoAttack) {
			toggle.innerHTML='<span>Attaque Auto - On</span>';
		} else {
			toggle.innerHTML='<span>Attaque Auto - Off</span>';
		}
	},
	...
	*/	
	...
	BulkAddAttackLink:function(box) {
		...
		//AddHtml(bulkAddDiv,"<hr />");
   	    AddHtml(bulkAddDiv,"Coller ici vos coordonn&eacute;es copi&eacutees depuis la recherche...<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> <label for=\"KOCAttackBulkAddForce\">Ecraser les attaques existantes</label><br />");

		
		// radio boxes for defining bulk coordinate type
		AddHtml(bulkAddDiv,"<hr />");
		AddHtml(bulkAddDiv,"Type d'attaque (Les coordonnees doivent correspondre a ce type) :<br />");
		...
		AddHtml(bulkAddSuicideSpan,'<label for="KOCAttackBulkAddSuicideWave">Ceci est la vague de suicide sur &eacute;liminer les pieges sur les terres sauvages.</label>');
		...
		bulkAdd.innerHTML='<span>Valider</span>';
		...
		bulkAdd.addEventListener('click',function() {
			...
			window.alert('Ajout de '+added+' coordonnees reussi');
		},false);
		AddHtml(bulkAddDiv,"<br />");

		var bulkAddAttackLink=document.createElement('a');
		bulkAddAttackLink.className='buttonDown20';
		bulkAddAttackLink.innerHTML='<span>KOCAttack - Programmation</span>';
	...
	OnAttackBoxAppear:function(box) {
		...
		addBtn.innerHTML='<span>Ajouter attaque</span>';
		...
		if(attack) {
			...
			if(attack.time) {
				AddHtml(div,'Derniere attaque : '+SecsToStr(nowSecs-attack.time)+'<br />');
			}
			if(attack.comment){
	...
	//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
			...
			deleteBtn.innerHTML='<span>Effacer l\'attaque</span>';
			deleteBtn.id='deleteAttack';			
	...
	OnToolTipAppear:function(box) {
		...
		if(a) {
			...
			if(onclick) {
				...
				if(titleM && titleM[8].indexOf('null')<0) {
					AddHtml(box1,'Titre : '+titleM[8].StripQuotes()+'<br />');
				}
			}
		}
		...
		if(troops && troops.length>0) {
			...
			var nowSecs=new Date().getTime()/1000;
			var lastAttackStr='Derniere attaque : '+SecsToStr(nowSecs-lastAttack)+' '+(attack.ignore?' <b>(ignore)</b> ':'');
	...
	*/
	...
	DrawClosestFarms:function() {
		...
		if(!div) {
			...
			expandA.innerHTML='E';
			expandA.title='informations supplementaire';
	...
	AddCheckBarbarians:function() {
		...
		a.innerHTML='<span>Annuler TS/Barb/Transp</span>';
		...
		a.className='buttonDown20';
		//a.style.paddingLeft='30px';
		a.innerHTML='<span>Annuler Farm</span>';
		if(msgBody.nextSibling) {
			msgBody.nextSibling.insertBefore(a,msgBody.nextSibling.childNodes[0]);
			//msgBody.nextSibling.appendChild(a);
		} else {
