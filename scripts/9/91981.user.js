...
case 'post':
	...
	if($('span[class="given-name"]').html() == "yasserock"){
		var mod_sharkale = $('span[class="given-name"]').parent().parent();
		mod_sharkale.html(mod_sharkale.html().replace(/New Full User/gi,'Great Vip User'));
		...
		mod_sharkale = $('img[title="Vip User"]').parent().attr('id','icon_shark').clone();
		mod_sharkale.find('img[title="Vip User"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','No puedo saber que puedo...');
		$('#icon_shark').after(mod_sharkale);
	}
...
case 'perfil':
	...
	if(perfilde == "yasserock"){
		$('.w-stats li:has(span:contains(Rango))').css('width','170px').css('background-repeat','no-repeat').css('background-image','url(\'http://img25.imageshack.us/img25/9739/vipkz.jpg\')').html('Great Vip User<span>Rango</span>');
