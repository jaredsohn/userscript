// ==UserScript==
// @name           KOC Power Tools - TÜRKÇE
// @description    MRFQ tarafıdan Türkçe'ye çevrilmiştir.
// @namespace      MRFQ
...
var Options = {
  ...
  enableFoodWarn : true,
  foodWarnHours : 24,
  alertConfig  : {aChat:false, aPrefix:'** BEN SALDIRI ALIYORUM...YARDIM EDİN! **', scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
};
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  GM_addStyle ('.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    .düşman td { background:crimson; }.Dost td{background:lightblue; }.Müttefik td{background:royalblue; }\
    .neutral td { background:lightgreen; }.unaligned td { background:gold; }\
    .Normal td { background:lightgreen; }.unaligned td { background:gold; }\
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('MENÜ', eventHideShow, mouseMainTab);
}
...
var knightRoles = [
  ['Ustabaşı', 'politics', 'Pol'],
  ['Mareşal', 'combat', 'Com'],
  ['Simya', 'intelligence', 'Int'],
...
my.Wilds = {
  ...
  show : function (){
    ...
    m = '<CENTER>'+ strButton20('YENİLE', 'id=ptwref') +'</center><TABLE cellspacing=0 cellpadding=0 class=ptTabPad align=center>';
...
//Seed.wilderness['city'+Cities.cities[3].id] = [];      
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Bozkır</td><TD></td><TD align=left>Koordinat</td><TD>Tuzaklar</td><TD align=left>Paralı Asker</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' SAVUNMA BELİRLE ') +'</td></tr>';
        ...
        for (i=0; i<sortem.length; i++){
          ...
          m += '<TR align=right'+ (row++%2?'':' class=ptOddrow') +'><TD align=left>'+ wildNames[wild.tileType] +'</td>\
            ...
            <TD></td><TD align=left class=ptentry><B>Adet Belirle:</b> <INPUT onchange="ptInpWildTraps(this)" id=ptwt_'+ c +'_'+ i 
          ...
          m += '<TD class=ptentry> &nbsp; &nbsp; <B>Paralı Asker:</b> ' + htmlSelector(mercNames, wildDef.mercLevel, 'id=ptwm_'+ c +'_'+ i) +' &nbsp; &nbsp; </td></tr>';
        }
        m += '<TR><TD colspan=6></td><TD class=ptentry align=center colspan=3><TABLE><TR><TD width=40% align=left>Maliyet: <SPAN id=ptwgc_'+ c +'>0</span></td>\
            <TD width=10%>'+ strButton20("SAVUNMAYA BAŞLA", 'onclick="ptButWildSet('+ c +')"') +'<TD width=40% align=right>Altın: <SPAN id=ptwgt_'+ c +'>0</span></td></td></tr></table></td></tr>';
      } else {
...
my.Knights = {
  ...
  show : function (){
    ...
    function _dispKnight (roleId, knight){
      ...
      if (knight == null) {
        ...
        if (knight.knightStatus == 10){
          ass = '<TD class=ptentry align=left colspan=2>Yürüyüş</td>';
        } else {  
          if (unpoints > 0){
    ...
    for (var c=0; c<Cities.numCities; c++) {
      ...
      m += '<TR><TD colspan=11><DIV class=ptstat>'+ Cities.cities[c].name +'</div></td></tr>\
          <TR class=ptwpad style="font-weight:bold" align=right><TD width=80>Görevi</td><TD width=160 align=center>İsim</td><TD width=26>Pol</td><TD width=26>Com</td>\
          <TD width=26>Int</td><TD width=26>Res</td><TD width=90 align=center colspan=3>- Atanmamış -</td><TD width=40 align=right> Maaş </td><TD width=35>Seviye</td></tr>';
      totSalary = 0;
      var did = {}; 
      var row = 0;
      ...
      m += '<TR align=right><TD colspan=9><B>Toplam Maaşlar:</b></td><TD>'+ addCommas(totSalary) +'</td></tr>';        
    }
  ...
  clickedAssignPoints : function (e, cid, kid, rid){
    ...
    if (knight.knightStatus == 10){
      ...
      row.childNodes[7].innerHTML = 'Yürüyüş';
      return; 
    }
    sk = [];
...
var messageNav = {
  ...
  hook : function (){
    ...
    var button = makeButton20('Bütün Subaylar');
    document.getElementById('ptfwdbut').appendChild (button);
    button.addEventListener ('click', function (){document.getElementById("modal_msg_write_to").value = "<subaylar>"}, false);
  },
  ...
  msgSendHook : function (){
    ...
    if (to.toLowerCase() != '<subaylar>' || getMyAlliance()[0]==0)
      return false;
...
my.AllianceList = {
  ...
  show : function (){
    ...
    if (t.state == null){
      ...
      var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
          <TR><TD class=xtab align=right></td><TD class=xtab>Oyuncu adını giriniz: &nbsp;</td>\
          ...
          <TR><TD class=xtab>veya: </td><TD class=xtab> İttifak adını giriniz: &nbsp;</td>\
           ...
           <TR><TD class=xtab>OR: </td><TD class=xtab> &nbsp;\
            <INPUT align=left id=allListSubmit type=submit value="İttifak Listesi" /></td>\
            <TD class=xtab><INPUT align=right id=allGotoPage type=submit value="Sayfa" />\
             <INPUT align=right id=idPageNum type="text" value='+t.curPage+' size=4 />\
             <INPUT align=left id=idMyAllSubmit type=submit value="Benim Müttefikler"/>\
             </td>\
             <TD class=xtab ></td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'En az 3 karekter giriniz.';
      return;
    }
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>Arıyor ...</center>';
  ...
  eventGotPlayerList : function (rslt){
    ...
    var m = '<DIV class=ptstat>Oyuncu Eşleşmeleri <B>"'+ t.pName +'"</b></div>\
      ...
      <TD align=right>Kudret</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Daha fazla bilgi </td></tr>';
    var row=0;
    var cl='';
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>Ayrıntılar</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>Sınıflandırma</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = my.AllianceList;
    span.onclick = '';
    span.innerHTML = "sınıflandırma bilgisi alınıyor ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Leaderboard:</b> Bulunamadı! (sisli?)';
      return;
    }
    ...
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Sınıflandırma: </b></td><TD colspan=2> Kudret: '+ p.might  +' &nbsp; İttifak: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      var created = '';
      if (c.dateCreated && c.dateCreated.substr(0,2)=='20')
        created = ' &nbsp; &nbsp; kurma: ' + c.dateCreated.substr(0,10);
      m += '<TR><TD align=right><B>Şehir #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName 
      +' (<a onclick="PTgotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; seviye: '
        + c.tileLevel +' &nbsp; &nbsp; durumu: '+ cityStatusString (c.cityStatus) + created +'</td></tr>';
    }  
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Ayrıntılar:</b> &nbsp; </td><TD>İttifak: '+ a +' &nbsp; Şehirler: '
          + u.cities +' &nbsp; Nüfus: '+ u.population +'</td></tr><TR><TD></td><TD>Vilayetler: ';
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'En az 3 karekter giriniz.';
      return;
    }
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=ptstat>İttifak eşleşmeleri <B>"'+ t.aName +'"</b></div>\
    <TABLE><TR style="font-weight:bold"><TD class=xtab>İttifak Adı</td><TD class=xtab>Puan</td><TD class=xtab>Üye</td>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ all.allianceId +')">Üyeleri Gör</a></td></tr>';
    }
    document.getElementById('allListOut').innerHTML = m;
  },
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    var m = '<div style="overflow:auto; height:556px;width:564px;"><TABLE><thead><TR style="font-weight:bold"> \
        <th class=xtab>İttifak Adı</th><th class=xtab>Sıra</th><th class=xtab>Üye</th>\
        <th align=right class=xtab>Kudret</th><th class=xtab>Diplomasi</th><th class=xtab></th></tr></thead><tbody>';
    document.getElementById('allListOut').innerHTML = m;
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PTgetMembers('+ alliance.allianceId +')">Üyeleri Gör</a></td></tr>';
    }
    m += '</tbody></TABLE><div style="font-weight:bold"; height:20px;width:560px; ><span> <a onclick="PTalClickPrev(-1)">   (Başla)   </a><a onclick="PTalClickPrev(10)">   (-100)   </a><a onclick="PTalClickPrev(5)">   (-50)   </a><a onclick="PTalClickPrev(1)"> (Önceki) </a> \
          <a onclick="PTalClickNext(1)"> (Sonraki) </a><a onclick="PTalClickNext(5)"> (+50) </a><a onclick="PTalClickNext(10)"> (+100) </a><a onclick="PTalClickNext(9999)"> (Son) </a> </span></div>';
    m += '</div>';
    document.getElementById('allListOut').innerHTML = m;
 },
  ...
  displayMembers : function (allName, numPlayers){
    ...
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=ptstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
        <TD class=xtab width=80% align=center> <SPAN id=distFrom>'+ Cities.cities[0].name +' ('+ Cities.cities[0].x +','+ Cities.cities[0].y +')</span>  şehrinden mesafe</td><TD class=xtab align=right>'+ numPlayers +' oyuncu bulundu &nbsp; </td></tr></table></div>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PTalClickSort(this)" class=clickable><A><DIV>Oyuncu</div></a></td>\
        ...
        <TD id=clickCol3 onclick="PTalClickSort(this)" class=clickable><A><DIV>Şehir</a></div></td>\
        ...
        <TD id=clickCol8 onclick="PTalClickSort(this)" class=clickable><A><DIV>Uzaklık</a></div></td></tr></thead>\
      ...
      <DIV style="top:670px; left:0px; position:absolute; width:100%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
        <TABLE width=100%><TR><TD class=xtab>Veriler 24 saate kadar eski olabilir!</td>\
          <TD class=xtab align=right>Sıralamak için sütun başlıklarına tıklayınız</td></tr></table></div>';
    document.getElementById('allListOut').innerHTML = m;
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Koordinat yaz: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; veya, şehir seç: <span id=dmcoords></span></td></tr></table>';
  ...
  show : function (){
    ...
    if (t.state == null){
      m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
          ...
          <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>Birim Bilgileri</div><TABLE align=center cellpadding=1 cellspacing=0>\
          <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>MALİYET</b></td><TD class=xtabHL colspan=7><B>İSTATİSTİKLER</b></td><TD class=xtabHL><B>BAKIM</b></td></tr>\
          <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Yiyecek</td><TD class=xtabH>Kereste</td><TD class=xtabH>Taş</td>\
          <TD class=xtabH>Cevher</td><TD class=xtabH>Nüfus</td><TD class=xtabHL>Kudret</td><TD class=xtabH>Hayat</td><TD class=xtabH>Saldırı</td><TD class=xtabH>Sav.</td><TD class=xtabH>Hız</td><TD class=xtabH>Menzil</td><TD class=xtabH>Bakım</td>\
      ...
      m += '<TR class=xtabLine><TD colspan=14 class=xtabLine></td></tr>';
      m += '</table>';
      m += '<BR><DIV class=ptstat>Mesafe Hesaplama</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR><TD class=xtab align=right><B>İlk Şehir: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> veya, şehir seçin: <SPAN id=ptloc1></span></td></tr>\
        <TR><TD class=xtab><B>İkinci Şehir: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> veya, şehir seçin: <SPAN id=ptloc2></span></td></tr></table>\
        ...
        <BR><BR><DIV class=ptstat>İl Haritası</div><DIV id=ptProvMap><IMG width=680 height=654 SRC="'+ URL_PROVINCE_MAP +'"></div></center></div>';
      t.cont.innerHTML = m;
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = ''+ x1 +','+ y1 +' ve '+ x2 +','+ y2 +' arası: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
...
try {    
  ...
  show : function (){
    ...
    if (t.state != 1){
      try {      
        m = '<TABLE class=ptTab>\
          <TR><TD colspan=2><B>Yapılandırma:</b></td></tr>\
          <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Etkinleştirme penceresini sürükle (mouse ile üst çubuğunu sürükleyerek hareket ettir.)</td></tr>\
          <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>Yiyeceğin bitimine<INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> saatten az süre kalınca KIRMIZI ile göster</td></tr>\
          <TR><TD colspan=2><P><B>KofC Özellikleri:</b></td></tr>\
          <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>Mesajların ve Raporların sayılarını ayrı simge  ile göster.</td></tr>\
          <TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Otomaik olarak ittifak chat sayfasında gelen saldırıları yayınla.</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
              <TR><TD align=right>İleti Öneki: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
              <TR><TD align=right>Haberci hakkında uyarı: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
              <TR><TD align=right>Saldırı sırasında uyarı: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
              <TR><TD align=right>En az # Asker: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
              </table></td></tr>\
          <TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>Gelişmiş ittifak rapolarını etkinleştir.</td></tr>\
          <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Diğer scriptlerin ittifak raporlarını değiştirmesine izin ver.</td></tr>\
          <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>Gelişmiş mesajlaşmayı etkinleştir. ("ilet" ve "bütün subaylar" buttonları).</td></tr>\
          <TR><TD colspan=2><BR><BR><B>KofC Hata Düzeltmeleri:</b></td></tr>\
          <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Hatalı kule raporlarını düzelt.</td></tr>\
          <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Haritada herzaman ilk şehir yerine,seçili şehir mesafesini göstermek için düzeltme.</td></tr>\
          <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>Hatalı saldırı raporlarını düzelt.</td></tr>\
...
my.Search = {
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = my.AllianceList;
    span.onclick = '';
    span.innerHTML = "sınıfladırma bilgisi alınıyor ...";
    t.fetchLeaderboard (uid, function (r) {t.gotPlayerLeaderboard(r, span)});
  },
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    for (var i=0; i<p.cities.length; i++){
      ...
      m += '<TR><TD align=right><B>Şehir #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName 
      +' (<a onclick="PTgotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; seviye: '
        + c.tileLevel +' &nbsp; &nbsp; durumu: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; kurma: ' + c.dateCreated.substr(0,10) +'</td></tr>';
    }  
    span.innerHTML = m + '</table>';
  },
  ...
  show : function (cont){
    ...
    if (t.state == null){
      this.cont.innerHTML = '\
        <DIV class=ptentry><table><tr><td><TABLE><TR valign=top><TD class=xtab width=100 align=right>Arama için: </td><TD>\
        <SELECT id="srcType">\
          <OPTION value=0>Barbar Kampı</option>\
          <OPTION value=1>Bozkırlar</option>\
		  <OPTION value=2>Şehirler</option>\
        </select></td></td> \
        <td> Vilayet:\
        ...
        <tr><td width=100  align=right >Bölge: </td><td><select id="idQuadSelect">\
        <option value="188,188"> Kuzeybatı </option>\
        <option value="563,188"> Kuzeydoğu </option>\
        <option value="188,563"> Güneybatı </option>\
        <option value="563,563"> Güneydoğu </option>\
      ...
      m = '<TABLE><TR valign=middle><TD class=xtab width=100 align=right>Koordinat: &nbsp; X: </td><TD class=xtab>\
        <INPUT id=srchX type=text > &nbsp; Y: <INPUT id=srchY type=text > &nbsp; <SPAN id=spInXYold></span>\
        </td></tr><TR><TD class=xtab align=right>Max. Mesafe: </td><TD style="width: 200px;" class=xtab><INPUT id=srcDist size=3 value=10 /></td>\
        <td></td>\
        </tr>';
  ...
  srchTypeHandler : function (){
    ...
    if (t.opt.searchType == 0)
      typeName = 'Barbar Kampları';
    else if (t.opt.searchType == 1)
      typeName = 'Bozkırlar';
	else 
	  typeName = 'Şehirler';

    var m = '<CENTER><B>Aranan '+ typeName +'<BR>\
        Koordinat: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Mesafe: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>Liste Seçenekleri:</b><BR></td></tr>';
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
     m += '<TR><TD class=xtab align=right>Min.Seviye:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
    ...
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>Bozkır Türü:</td><TD class=xtab align=right>\
            Orman<INPUT id=woodWild type=CHECKBOX'+ (Options.woodWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Çayır/Göl<INPUT  id=foodWild type=CHECKBOX '+ (Options.foodWild?' CHECKED':'') +'></td>\
	       <TD class=xtab align=right>Cevher<INPUT id=mtnWild type=CHECKBOX '+ (Options.mtnWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Ova<INPUT id=plnWild type=CHECKBOX '+ (Options.plnWild?' CHECKED':'') +'></td>\
           <TD class=xtab align=right>Taş<INPUT id=hillWild type=CHECKBOX'+ (Options.hillWild?' CHECKED':'') +'></td></tr>';
      m += '<TR><TD class=xtab align=right>Sadece Sahipsiz:</td><TD class=xtab><INPUT id=filUnowned type=CHECKBOX '+ (Options.unownedOnly?' CHECKED':'') +'\><td></tr>';
    } 
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        ...
        m+= '<TR><TD class=xtab align=right >Göster:</td><TD class=xtab align=left ><SELECT style="width: 135px" id=idSrchFilter>\
             <OPTION value=0>Bütün Şehirler</option>\
             <OPTION value=1>Düşman Şehirleri</option>\
	     <OPTION value=2>Sisteki Şehirler</option>\
	     <OPTION value=3>Müttefik Şehirleri</option>\
	     <OPTION value=4>Dost Şehirler</option>\
	     <OPTION value=5>Normal Şehirler</option>\
	     <OPTION value=6>İttifaksız Şehirler </option>\
             </select></td></tr>';

		m+= '<TR><TD class=xtab align=right >Sırala:</td><TD class=xtab><SELECT style="width: 135px" id=filSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>Kudret</option>\
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Mesafe</option>\
        </select></td></tr>\
        <TR><TD class=xtab align=right >Sadece Koordinatlar:</td><TD class=xtab align=left ><INPUT type=checkbox id=coordsOnly ></td></tr>\
        </table></div><BR><SPAN id=srchSizeWarn></span>';
  ...
  clickedSearch : function (){
    ...
    document.getElementById ('srcStart').value = 'Aramayı Bitir';
    ...
    document.getElementById ('statStatus').innerHTML = 'Aramaya başlıyor... '+ xxx +','+ yyy;
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Barbar Kampı', 'Çayır', 'Göl', 'Orman', 'Tepe', 'Cevher', 'Ova', 'Şehir' ];
    var t = my.Search;
...
if (DEBUG_TRACE) DebugTimer.display('if (t.opt.searchType == 2 && type == 7 )  document.getElementById( idSrchFilter ).value = ' + document.getElementById('idSrchFilter').value + ' ' + t.mapDat[i][12]);
              switch(parseInt (document.getElementById('idSrchFilter').value)) {
                case 0:
                 ...
                 break;
                case 1:
                   if (t.mapDat[i][12] == 'Düşman') dat.push(t.mapDat[i]);
                   break;
                case 2:
                   if (t.mapDat[i][5]===true) dat.push(t.mapDat[i]);
                   break;
                case 3:
                   if (t.mapDat[i][12] == 'Müttefik') dat.push(t.mapDat[i]);
                   break;
                case 4:
                   if (t.mapDat[i][12] == 'Dost') dat.push(t.mapDat[i]);
                   break;
                case 5:
                   if (t.mapDat[i][12] == 'Normal') dat.push(t.mapDat[i]);
                   break;
                 case 6:
                   if (t.mapDat[i][12] == 'Belirsiz') dat.push(t.mapDat[i]);
                   break;
             }
	  } else {
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    if (dat.length == 0){
      m = '<BR><CENTER>Bulunamadı.</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=srcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Konum</td></tr>';
      else {
		if (t.opt.searchType == 2) {
			 m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>Konum</td><TD >Mesafe</td><TD>Şehir</td><TD>Sahibi</td><TD>Kudret</td><td>İttifak               </td><TD width=80% style="font-size:9px;">Daha fazla bilgi</td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			m = '<TABLE id=srcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Konum</td><TD style="padding-left: 10px">Mesafe</td><TD style="padding-left: 10px;">Lvl</td><TD width=80%> &nbsp; Türü</td><TD style=""></td></tr>';
		}
	  }
        ...
        if (coordsOnly)
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					m += '<DIV onclick="PTscout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A style="font-size:9px;" >Haberci yolla</a></div>';
				} else 
					m += '<DIV onclick="PTpd(this, '+ dat[i][7] +')"><A style="font-size:9px;" >Ayrıntılar</a></div> <DIV style="" onclick="PTpd2(this, '+ dat[i][7] +')"><A style="font-size:9px;">Sınıflandırma</a></div>';
          ...
          m += '<TD align=right  valign="top">'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD align=right>'+ dat[i][4] +'</td><TD> &nbsp; '+ tileNames[dat[i][3]]
            +'</td><TD  valign="top">'+ (dat[i][5]?' SAHİPLİ':'') +'</td></tr>';
			}
		}
       }
      m += '</table>';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'Arama: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('statStatus').innerHTML = 'Arıyor... '+ x +','+ y;
  ...
  show : function (){
    ...
    if (t.state == null){
      s = "<DIV id=trainTopSelect>\
        <DIV class=ptstat>Sıralı Asker ve Alan savunması yapmak için</div><DIV style='height:10px'></div><DIV class=ptentry>\
        <DIV style='text-align:center; margin-bottom:10px;'>Şehir Seç: &nbsp; <span id=ptspeedcity></span></div>\
        <TABLE class=ptTab width=100%><TR valign=top><TD width=50%>\
        <TABLE align=center><TR><TD align=right>Birlik Türü: </td><TD colspan=2>\
        <SELECT id=ptttType>\
          <option value='1'>İkmal Birliği</option>\
          <option value='2'>Milis</option>\
          <option value='3'>Haberci</option>\
          <option value='4'>Mızraklı Asker</option>\
          <option value='5'>Silahşörler</option>\
          <option value='6'>Okçu</option>\
          <option value='7'>Süvariler</option>\
          <option value='8'>Ağır Süvari</option>\
          <option value='9'>Malzeme Arabası</option>\
          <option value='10'>Balista</option>\
          <option value='11'>Koçbaşı</option>\
          <option value='12'>Katapult</option>\
        </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
        <TR><TD align=right># slot başına: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
          <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
        <TR><TD align=right># slotlar: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
          <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
        <TR><TD align=right valign=top>Boştaki işcileri dahil et.: </td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
          <SPAN style='white-space:normal;'>Daha fazla asker üretmeni sağlar.Geçiçi olarak nüfusunu eksiye düşürür.</span></td></tr>\
        ...
        <TABLE align=center><TR><TD align=right>Defens Türü: </td><TD colspan=2>\
        <SELECT id=pttdType>\
          <option value='53'>Tatar Yayları</option>\
          <option value='55'>Savunma Mancınığı</option>\
          <option value='60'>Tuzak</option>\
          <option value='61'>Bıtırık</option>\
          <option value='62'>Çivili Bariyer</option>\
        </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
        <TR><TD align=right># slot başına: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
          <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
        <TR><TD align=right># slotlar: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
        ...
        <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Defans Kur'\></td></tr></table>\
        </td></tr></table></div></div>\
        ...
        <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Asker Üretim Kuyruğu &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
          <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>Defans Kuyruğu &nbsp; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
        </div>";
      t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    if (perSlot*numSlots > t.stats.MaxTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Üretim için gerekli koşullar uygun değil. (maksimum '+ t.stats.MaxTrain +' tane üretebilirsin.)</font>';
      return;
    }
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    if (isNaN(slots) || slots<0)
      slots = 0;
    t.TDspMax.innerHTML = 'max:'+ t.stats.MaxDefTrain +'&nbsp; sahipsin:'+ t.stats.defOwned;   
  ...
  clickDefDo : function (){
    ...
    if (perSlot*numSlots > t.stats.MaxDefTrain){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Üretim için gerekli koşullar uygun değil. (maksimum '+ t.stats.MaxDefTrain +' tane üretebilirsin.)</font>';
      return;
    }
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Yapılan savunma sırası.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Eğitim '+ cmd[2] +' '+  fortNamesShort[cmd[1]] +'<BR>');
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><B>Yiyecek:</b></td><TD width=16%><B>Kereste:</b></td><TD width=16%><B>Taş:</b></td>\
        <TD width=16%><B>Cevher:</b></td><TD width=16%><B>Altın:</b></td><TD width=16%><B>Boş Nüfus:</b></td></tr>\
...
// troop queue .... 
    ...
    m = t.stats.barracks +' kışlalar';
    if (t.stats.queued > 0)
      m += ', '+ t.stats.queued +' slotlar';
    if (totTime > 0)
...
// defense queue ....
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>Yapılan asker sırası.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        t.dispTrainStatus ('Eğitim '+ cmd[2] +' '+  unsafeWindow.unitcost['unt'+cmd[1]][0] +'<BR>');
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Üyelik:</span> '+ dt.toLocaleDateString() +'</td>\
      ...
      str += "<TABLE class=ptTabLined cellspacing=0><TR valign=top align=right><TD width=95></td><TD width=88 style='background: #ffc'><B>TOTAL</b></td>";
      ...
      if (Options.includeMarching)
        str += '<TD width=81><B>Yürüyüş</b></td>';
      str += "</tr>";
      ...
      rownum = 0;
      str += _row ('İkmal B.', rows[1]);
      str += _row ('Milis', rows[2]);
      str += _row ('Haberci', rows[3]);
      str += _row ('Mızraklı', rows[4]);
      str += _row ('Silahşör', rows[5]);
      str += _row ('Okcu', rows[6]);
      str += _row ('Süvari', rows[7]);
      str += _row ('A.Süvari', rows[8]);
      str += _row ('Araba', rows[9]);
      str += _row ('Balista', rows[10]);
      str += _row ('Koçbaşı', rows[11]);
      ...
      str += _row ('Sur', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Asker Yürüyüşlerini ve Kaynakları Göster. </td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
        str += '<BR><BR><INPUT id=subSeed type=submit name="SEED" value="TAMİR">';
...
my.Encamped = {
  ...
  show : function (cont){
    var rownum = 0;
    var names = ['İkmal', 'Mil', 'Hab', 'Mız', 'Sİl', 'Okcu', 'Süv', 'A.Süv', 'Araba', 'Balli', 'Koç', 'Kat'];
    var t = my.Encamped;
    ...
    s = '<div class=ptstat>Bütün büyükelçiliklerin herbirinde kamp kurmuş askerleri gösterir.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Hiçbir Asker Kamp Kurmamış.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>Oyuncu (Şövalye)</b></td>';
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> Orjinal Askeri Göster.';
    s += '<BR><BR><DIV style="font-size: 10px">NOT: Eğer bir savaştan sonra yeni bilgileri veya kalan askerleri görmek için sayfayı yenilemeniz gerekecektir..</div>';
    t.cont.innerHTML = s;
    checkBox = document.getElementById('idCheck2');
...
function DialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  if (!rdiv){
    ...
    rdiv.innerHTML = '<DIV style="background: #cccccc"><CENTER>KOC Power Tools - TÜRKÇE</div>\
      <CENTER><BR><FONT COLOR=#550000><B>Bir hata oluştu:</b></font><BR><BR><DIV id=retryErrMsg></div>\
      <BR><BR><B>Otomatik olarak yeniden denemeye <SPAN id=retrySeconds></b></span> saniye ...<BR><BR><INPUT id=retryCancel type=submit value="İPTAL" \>';
...
var debugWin = {
  ...
  doit : function (){ 
    ...
    if (t.popDebug == null){  
      ...
      t.popDebug.getTopDiv().innerHTML = 'TAMİR';
      t.popDebug.getMainDiv().innerHTML = '<DIV><INPUT type=submit id=dbsuball value=Hepsi> &nbsp; <INPUT type=submit id=dbsubnone value=Hiçbiri> &nbsp; \
        <INPUT type=submit id=dbdefaults value=Varsayılan> &nbsp; <INPUT type=submit id=dbsubdo value=Göster></div>\
...
function saveOptions (){
  var serverID = GetServerId();
  GM_setValue ('ptOptions_'+serverID, JSON2.stringify(Options));
}

function readOptions (){
  var serverID = GetServerId();
  s = GM_getValue ('ptOptions_'+serverID);
  if (s != null){
    opts = JSON2.parse (s);
    for (k in opts)
      Options[k] = opts[k];
...
function CPopup (prefix, x, y, width, height, enableDrag, onClose) {
  ...
  this.div.style.zIndex = "11111100";        // KOC modal is 100210 ?
