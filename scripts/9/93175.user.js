...
function getChildElementById(parent,tag,ID){
	var result=null;
...
if (document.location.href.search("bg.molehillempire.com")!=-1) {
	var lng = "bg";
	var gamepage = ".bg.molehillempire.com";
	var reg = /http:\/\/s(\d+)\.bg\.molehillempire\.com\/(.*?)\.php/i;
	var reg2 = /http:\/\/(.*)bg\.molehillempire\.com\/(.*)/i;
	var delimThou = ".";
	var regDelimThou = /\./g;
	var delimDeci = ",";
	var regDelimDeci = /,/;
...
function do_main () {
	...
	if(unsafeWindow.zeigeStadtMain){
		...
		parent.Event.add(document.getElementById("bus"),"click",function(){ 
			...
			document.location.href="http://s29.wurzelimperium.de/stadt/index.php?karte=2";
		...
		for (var i=1;i<205;i++){
			if ((unsafeWindow.garten_kategorie[i]!="u") && (unsafeWindow.garten_kategorie[i]!="z")){
				...
				if (unsafeWindow.top.global_x=="2"){ 
					i++;
					if (i%17==16){ i+=17*(parseInt(unsafeWindow.top.global_y,10)-1)+1; }
					else if (i%17==0){ i+=17*(parseInt(unsafeWindow.top.global_y,10)-1); }
				}
			}
		}
	},true); 
...
function do_login(){
	...
	if (loc[2].search("logout")!=-1) {
		...
		var loginButton = getChildElementById($("form_login"),"input","submitlogin");
		...
		function submit_login(accNr){
			if(logindata[accNr][0]==lng){
				...
				getChildElementById($("form_login"),"input","login_user").value=logindata[accNr][2];
				getChildElementById($("form_login"),"input","login_pass").value=logindata[accNr][3];
				click(loginButton);	
			} else {
