// ==UserScript==
// @name           Menelgame - łatwa zmiana broni, mieszkania, rupieci
// @description    Skrypt dodaje obok awatara 3 ikonki - bron, mieszkanie, rupieć i menu do ich zmiany http://opuszczone.c0.pl
// @version        1.1
// @author         http://opuszczone.c0.pl & mikskape
// @include        http://*menelgame.pl/*
// @exclude	   http://opuszczone.c0.pl
// ==/UserScript==
...
GM_xmlhttpRequest(
{
	...
	onload: function(responseDetails) {
		...
		if (wersja != s_wersja) {
		alert('Za chwilę zostanie pobrana nowa wersja skryptu "Menelgame - łatwa zmiana broni, mieszkania, rupieci". \nProszę potwierdzić instalację.')
	...
	GM_xmlhttpRequest({		//abfrage im netz
		...
		onload: function (responseDetails){
			...
			for (var i = 0, z = 1; i < table.length; i++){
				...
				if(table[i].search('value=\"Użyj\"') != -1){		//arrays mit dem zeug was benutzbar ist
	...
	GM_xmlhttpRequest({
		...
		onload: function (responseDetails){
			...
			for (var i = 0, z = 1; i < table.length; i++){
				...
				if(table[i].search('value=\"Zamieszkany\"') != -1){
...
function plunder(){
	...
	GM_xmlhttpRequest({
		...
		onload: function (responseDetails){
			...
			var angelegt = web.split('<h3>Założony</h3>')[1];
			...
			var werte = new Array("ATT: ", "DEF: ", "zręczność: ", "Efekt: ");
