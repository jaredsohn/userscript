...
// @description Version 1.0.1 Formatiert Berichte Forumtauglich (Ingame)
// @author Hypix
...
var urlType = 1;          // 0 = Keine Links, 1 = DS-BB-Codes, 2 = [URL=], 3 = [URL=""]
...
var showSpyBeute = true;  // true = ErspÃ¤hte Rohstoffe anzeigen, false = verbergen
var showSpyBuild = true;  // true = ErspÃ¤hte GebÃ¤ude anzeigen, false = verbergen
var showSpyExtern = true; // true = Truppen ausserhalb anzeigen, false = verbergen

var res = location.href.match( /http:\/\/([^\/]+)\/game\.php\?village=(\d+)&screen=info\_command/ );
if( res )
{
  ...
  createBBForm(cell, 200, createBBCode, false);
  createBBCode();
}
...
if( res )
{
  ...
  cell.rowSpan=tab.rows.length;
  createBBForm(cell,180,createBBCode,true);
  createBBCode();
}
    ...
    switch( node.nodeName )
    {
      case "TABLE":
        if( /rabe(_grau)?\.png/.test(node.innerHTML) )
          return ret;
        else if( node.rows[0] && !/<table[^>]*>/.test(node.rows[0].innerHTML) && /\/unit\/unit\_[^.]+\.png/.test(node.rows[0].innerHTML) )
      ...
      case "A":
        ...
        {
          res = node.href.match( /screen=info\_village&id=(\d+)/ );
          if( res )
          {
            cp = node.innerHTML.lastIndexOf("(");
            coords = node.innerHTML.substring(cp).match( /(\d+)\|(\d+)/ );
            lnk = removeWhiteSpaces(node.innerHTML);
            switch( urlType )
            {
              case "0":
                ret += lnk;
              ...
              case "1":
                ...
                break;
              default:
                ret += getBBUrl(url+"page.php?page=inbound&screen=info_village&id="+res[1],lnk);
            }
          }
        }
        skipChildren = true;
...
function parseUnits(node)
{
  ...
  {
    if( title ) ret += "[b]"+removeWhiteSpaces(node.rows[r].cells[0].innerHTML)+"[/b]\n";
...
function createBBForm(cell,size,createBBCode,report)
{
  ...
  tab.insertRow(tab.rows.length).appendChild(document.createElement("th")).innerHTML = "DS-Berichteformatierer-Optionen";
  tab.style.width = "100%";
  ...
  cell.innerHTML = "Allgemein";
  cell.colSpan=2;
  tabAllg.insertRow(tabAllg.rows.length).insertCell(0).innerHTML = "Spieler- und Dorflinks:";
  ...
  ctrl.options.add(option);  
  tabAllg.insertRow(tabAllg.rows.length).insertCell(0).innerHTML = "Bilder verwenden:";
  cell = tabAllg.rows[tabAllg.rows.length-1].insertCell(1);
