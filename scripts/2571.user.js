...
// @description  v0.2 - Another Google Reader subscriber. This one was designed to use as little screen size as possible. It only adds a feed icon in the top right corner. On mouse over the subscribe links are shown.
// ==/UserScript==
...
if (feeds.length > 0) {
	...
	for (var i = 0, feed; feed = feeds[i]; i++) {
		JGRSfeeds.innerHTML += '<a href=http://www.google.com/reader/view/feed/'+ encodeURIComponent(feed.href) +'>'+ feed.title +'</a>';
	}
}
