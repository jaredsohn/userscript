// ==UserScript==
// @name           Mel_Bipson_Bank AutoRefresher
// @description    Based on code from Bipson_Bank
// @namespace      http://gdriv.es/mel_bipson/
// @author         melbipson@gmail.com 
// @version        .4
...
if (document.URL.indexOf("channel=mel_bipson_bank") > -1){
	quit()
}
...
var GM_config = {
 ...
 if(document.evaluate("//iframe[@id='GM_config']",document,null,9,null).singleNodeValue) return;
	// Create frame
	document.body.appendChild((GM_config.frame=GM_config.create('iframe',{id:'GM_config', style:'position:fixed; top:0; left:0; opacity:0; display:none; z-index:999; width:300px; height:300px; max-height:95%; max-width:95%; border:1px solid #000000; overflow:auto;'})));
	...
	GM_config.frame.addEventListener('load', function(){
		...
		for (var i in settings) {
			...
			if (field.section) {
				...
				anch = frameBody.appendChild(create('div', {className:'section_header_holder', id:'section_'+secNo, kids:new Array(
				  create('a', {className:'section_header center', href:"javascript:void(0);", id:'c_section_kids_'+secNo, textContent:"Main Options", onclick:function(){GM_config.toggle(this.id.substring(2));}}),
				  create('div', {id:'section_kids_'+secNo, className:'section_kids', style:obj.getValue('section_kids_'+secNo, "")=="none"?"display: none;":""})
				  )}));
				secNo++;
			}
...
// START HERE ///////////////////////
GM_config.init('Bipson Bank Settings', {
		'refresh': {
			'label': 'Refresh Timer (refresh page to update)',
			'type': 'int',
			'default': 60
		},
		'hide': {
...
GM_registerMenuCommand('Bipson Settings',optWin); 


r = GM_config.get('refresh') * 1000;
if (r < 10000) {
	r = 10000;
}
...
bE.id = 'b';
bE.innerHTML = '&nbsp;';
bE.style.overflow = "auto"
bE.style.height = "18px";

bipsonChat = document.createElement("div");
bipsonChat.setAttribute("id","bipsonChat");
bipsonChat.style.display = 'none'
bipsonChat.innerHTML = '<iframe frameborder="0" scrolling="no" id="bipson_chat_embed" src="http://twitch.tv/chat/embed?channel=mel_bipson_bank&amp;popout_chat=true" height="432" width="335"></iframe>'

e = document.getElementById('controls');

if (e != null){
	$('#controls').css( "padding-top", "-=10px" )
	$('#controls').css( "padding-bottom", "-=10px" )
	$('#control_buttons').css( "padding-top", "-=8px" )
    
    // check if chat or /old is in url
    if (document.URL.indexOf("chat") > -1 || document.URL.indexOf("/old") > -1) {
        e.parentNode.insertBefore(bE, e); 
    } else {
        e.insertBefore(bE, e.firstChild);
    }
	
	document.body.appendChild(bipsonChat)
}
...
function l() {
	if (e == null){return;}
	//if (toggle == 2) {return;}
	$('#b').load('https://googledrive.com/host/0Bx5UkV86XCCgRWN6YTEzQlFGa2s/bank.html #' + unsafeWindow.PP['login'].toLowerCase());
...
function refresh(){
	if (i > 1800000 || GM_config.get('hide') === true){
		bE.innerHTML = '&nbsp;';
...
$('#chat_text_input').keyup(function (e) {
   if (e.keyCode == 13 && e.shiftKey) {		
		exec(function() {
			el = document.getElementById("chat_text_input")
			CurrentChat.ircSystem.join('#mel_bipson_bank')
			CurrentChat.ircSystem.send_channel_message(el.value)
...
function exec(fn) {
    ...
    s.textContent = '(' + fn + ')();';
    document.body.appendChild(s); // run the script
...
function getLast(){
	iframe = document.getElementById('bipson_chat_embed')
	innerDoc = (iframe.contentDocument) ? iframe.contentDocument : iframe.contentWindow.document;
//	GM_log("innerDoc: " + JSON.stringify(innerDoc.getElementsByTagName('*')));
//	GM_log("viewers: " + JSON.stringify(innerDoc.getElementById('viewers').length));
	ul = innerDoc.getElementById('chat_line_list')
//	GM_log("chat_line_list : " + JSON.stringify(ul.length));
	items = ul.querySelectorAll(".chat_from_mel_bipson_bank .chat_line");
	last = items[items.length -1]
	text = last.innerHTML.replace('» ', '')
	return text
}

var t
var status = "Nothing happened yet"

function getText(){
	if (GM_config.get('hide') == true) {return;}
	t = getLast()
	if (t.indexOf("MrDestructoid Bets") > -1){
		reg = /!betRED for (.*?)(?:\s|$) ----- !betBLUE for (.*?)(?:\s|$)</g;
//		GM_log("Kreygasm");
		m = reg.exec(t)
		s = '<div>BETS OPEN! <font color="FF0000">'+m[1]+'</font> vs <font color="0000FF">'+m[2]+'</font></div>'	
	} else if (t.indexOf("Total bets: |RED|") > -1) {
		reg = /Total bets: \|RED\| (.+) \*vs\* \|BLUE\| (.+)/g;
		m = reg.exec(t)
		s = '<div>Total: <font color="FF0000">'+m[1]+'</font> vs <font color="0000FF">'+m[2]+'</font></div>'
	} else if (t.indexOf("Wins!  Proportionate") > -1) {
		reg = /(.+) Wins!  Proportionate/g;
		m = reg.exec(t)
		s = '<div>'+m[1]+' won the last match.</div>'
	} else if (t.indexOf("!!!All Bets CANCELED!!!") >-1) {
		s = '<div>Bets have been canceled!</div>'
	} else if (t.indexOf("!msg") >-1) {
		text = t.substring(5)
		s = '<div>'+text+'</div>'	
	}
	...
	if (s != status) {
		...
		refreshTimer = setInterval(refresh, r)
		status = s
		bE.innerHTML = status
...
var getTextTimer = setInterval(getText, 1000)
var refreshTimer = setInterval(refresh, r)
