// ==UserScript==
// @name           Tumblr Follow asanusta Cost 
// @namespace      @asanusta
...
{
    var listdiv = menus[m].getElementsByClassName("user_menu_list")[0];
    var href = menus[m].parentNode.getElementsByClassName("post_avatar")[0].href;
    var alt = menus[m].parentNode.getElementsByClassName("post_avatar")[0].title;
    litem = document.createElement("a");
    litem.rel = href;
    litem.title = alt;
    litem.innerHTML = '<div class="user_menu_list_item"><span class="user_menu_icon">?</span>Stats</div>';
    litem.addEventListener("click", function(evt) {unsafeWindow.getInfo(this);}, false);
    listdiv.appendChild(litem);
}