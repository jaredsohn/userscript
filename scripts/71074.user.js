// ==UserScript==
// @name           Ahrens858 4 mod
// @namespace      Ahrens858 4 mod
...
function findName(test) {
    if (test.innerHTML.indexOf('/game/home.pl?user_id=12186', 0)>=0) return 1;
  return 0;
}
...
for(var i=0,j=els.length; i<j; i++) {
	if (findName(els[i])){
		...
		els[i].innerHTML = els[i].innerHTML + '<b>Ahrens4MOD</b><br/>'
