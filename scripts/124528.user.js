...
function main() {
	...
	***********************************************/
    ...
    // 1080p is pooped
	if ( use1080p && (  fmt_list.search("46%2F") != -1 || fmt_list.search("37%2F") != -1 || flashvars.search("size=1920x1080") != -1) ) {
		setFlashvar( "vq", "hd1080" );
	}
	else if ( use720p && ( fmt_list.search("45%2F") != -1 || fmt_list.search("22%2F") != -1 )) {
		setFlashvar( "vq", "hd720" );
		//awful fix to get 1080p to work
		if (use1080p) setFlashvar( "vq", "hd1080" );
	}
...
//force the player to stay one size... constantly... no goingbackses
...
// timer for checking if the page has changed in relation to clicking a related video and YouTube loading the whole page via Ajax -- currently not working
function checkChanged(video_id) {
    
    var playerDetails = fetchPlayer();
    var flashvars = playerDetails[2];
    
	var start = flashvars.search("video_id=") + 9;
	var end = flashvars.indexOf("&", start);
	var len = end - start;
	var new_id = flashvars.substr(start, len);

	//if ( debug ) GM_log( "video_id: " + video_id );
	//if ( debug ) GM_log( "new_id: " + new_id );
	
    if (typeof video_id != "undefined" && new_id != video_id) {
        main();
    }
    else {
        var video_id = new_id;
        var check=window.setInterval(checkChanged(video_id), 1000); 
    }
}
