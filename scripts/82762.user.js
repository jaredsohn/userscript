...
javascript:(function(){
    ...
    var lower_mafia = 1, upper_mafia = 500;
    ...
    var table_html='<form id="something">'+
			...
			'<th width="48%" style="font-size:12px; text-align:right"> <a id="Website" align="right" href="http://arun.keen-computing.co.uk" target="_blank">Don Shqiptar Brawler</a> - <a id="Donate" href="http://arun.keen-computing.co.uk/?page_id=31" target="_blank">Donate</a></th>'+
            ...
            '<td width="3%">Of</td>'+
            '<td width="75%"><input type=text id="attackcount" value="0" class="sexy_input" style="width:30px" onkeydown="return field_validate(event.keyCode);"> / Exp to Level : <input type=text id="exp_to_level" value="0" class="sexy_input" style="width:30px" onkeydown="return field_validate(event.keyCode);"></td>'+
...
//            '<td width="1%">:</td>'+
//            '<td width="89%">Restart in <input type=text id="restart_min" value="1" class="sexy_input" style="width:25px" onkeydown="return field_validate(event.keyCode);"> minutes, <input type="checkbox" id="finish_heal_enable">Heal, Travel to </td>'+
//            '</tr></table>'+
//			'</tr>'+
//			
			'<tr>'+
...
//            '<td width="25%"><input type="radio" id="Spammers" name="fight_choice">Add Me Spammers</input></td>'+
            ...
            '<tr><td width="10%" valign="top"><a href="#" id="log_show">Log</a> &nbsp;&nbsp;<input type="text" id="log_size" value="3" class="sexy_input" style="width:20px"></input></td>'+
...
//    }
    ...
    function UnixTS(){
        return (Math.round(new Date().getTime() / 0));
    }
    		...
    		if(pageLoading==0){
                switch(true){
                    case document.forms.something.Fightlist.checked:
                        setTimeout(loadfightlist,0);
                    break;
                    case document.forms.something.Users.checked:
                        if(attack_or_not){
                            setTimeout(loaduserlist,0)
                        }
                    break;
                }
...
//                alert(fightlist_faction[fightlist_faction.length - 1]);
        ...
        if(fightlist_match >= 3){
            ...
            fightlist_match=0;
            setTimeout(function(){attack_or_not=true;loadfightpage();},3000);
            return;
        }
...
//            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=attack&xw_city=1&tmp='+tmp_key+'&cb='+cb+'&opponent_id='+user_list[m]+'&xw_client_id=8&ajax=1&liteload=1&sf_xw_user_id='+userid+'&sf_xw_sig='+local_xw_sig;
            ...
            user_urls[user_urls.length]= 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=fight&xw_action=power_attack&xw_city=1&cb='+cb+'&opponent_id='+user_list[m]+'&origin=profile&xw_client_id=8';
...
//                document.getElementById('stash_post'+(stash_posts.length-1)).onclick = new Function(this.id,publish_stash);
        ...
        if(/iced_pop_text/.test(attack_results)){
            ...
            document.getElementById('ice_posts_log').innerHTML += iced_posts[iced_posts.length-1][0]+' - <a href="#" id="ice_post'+(iced_posts.length-1)+'" onclick="return false;">Post</a><br>';
...
//                return;
    ...
    function process_user_attack(attack_results){
        ...
        if(!((/You win/.test(attack_results)) || (/You lose/.test(attack_results)))){
            ...
            var profile_link = 'http://facebook.mafiawars.com/mwfb/remote/html_server.php?xw_controller=stats&xw_action=view&xw_city=1&cb='+userid+UnixTS()+'&user='+user_list[j]+'&xw_client_id=8';
...
//            else{
            exp_gained += parseInt(/\+(\d*) Experience/.exec(attack_results)[1]);
            logtext += '<img src="http://mwfb.static.zynga.com/mwfb/graphics/icon_experience_16x16_01.gif" alt="Exp"> <span class="good">' + parseInt(/fightres_experience good.+?\n(.*)\+(\d*) Experience/.exec(attack_results)[2])+'</span>';
//            }
...
//                    logmsg('Reached last member, reloading..','attack_log');
                ...
                setTimeout(attack_user,random_delay*0);
            }
            return;
        }
        else{
            ...
            $.ajax({type: "POST",url: hitlist_url, timeout: 30000, data: params,
                success: function (msg){
                    if (/You can't add/.test(msg)){
                        ...
                        setTimeout(attack,random_delay*0);
                        return;
                    }
                },
                error: function(req,status,err){
