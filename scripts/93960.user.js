// ==UserScript==
// @name           Combat Report Auto-Converter
// @autor          WTF
// @icon           http://img268.imageshack.us/img268/9123/36135250.png
...
IkariamLanguageDetection.getLanguage = function( langCode ) { return IkariamLanguageDetection.langToLanguageMap[ langCode ] || 'spanish'; }
IkariamLanguageDetection.lang = unsafeWindow.LocalizationStrings.language || 'es';
...
language_resources = {
	...
	spanish:	{ population:"Población", actions:"Puntos de acción", wood:"Madera", wine:"Vino", marble:"Mármol", glass:"Cristal", sulfur:"Azufre", gold:"Oro"}
};
...
document.getElementById('extraDiv4').style.visibility  = 'hidden';
document.getElementById('extraDiv4').style.position  = 'absolute';
document.getElementById('extraDiv4').style.top  = '0px';
document.getElementById('extraDiv4').style.left  = '0px';
document.getElementById('extraDiv5').style.visibility  = 'hidden';
document.getElementById('extraDiv5').style.position  = 'absolute';
document.getElementById('extraDiv5').style.top  = '0px';
document.getElementById('extraDiv5').style.left  = '0px';
document.getElementById('extraDiv6').style.visibility  = 'hidden';
document.getElementById('extraDiv6').style.position  = 'absolute';
document.getElementById('extraDiv6').style.top  = '0px';
document.getElementById('extraDiv6').style.left  = '0px';
        ...
        onload: function(response) { 
            ...
            if(new_version.length>6) {
				...
				if(VERSION!=new_version) { 
					if(confirm('Su script Combat Report Auto-Converter está obsoleto. ¿Desea actualizar a la última versión? (' + new_version + ')')) {  
...
function postToIkariam() { 
	forum_adress_array = {
		ar: ['0','38','44','86','136','161','180','189'],
		cl: ['0','32','65'],
		co: ['0','35','52','108','115'],
		es: ['0','107','108','109','110','111','112','113','121','134','152','195','216','234','260','327','342','365','445','461'],
...
function BuildStandardReport() {
	...
	var full_whodefend5 = document.getElementById('mainview').innerHTML.split('<label>Defensor:</label>');
	...
	var full_whoattack5 = document.getElementById('mainview').innerHTML.split('<label>Atacante:</label>');
	...
	if(GM_getValue('CRAC_ally')==2) { 
		...
		GM_xmlhttpRequest({
 			...
 			data: "view=highscore&searchUser=" + atackers_ally_member,
  			headers: {    "Content-Type": "application/x-www-form-urlencoded"  },
 			onload: function(response) {
				if (/class="allyLink" href="/.test(response.responseText)){ 
					atackers_ally_alliance4 = response.responseText.split('class="allyLink" href=');
	...
	if(GM_getValue('CRAC_ally')==2) { 
		...
		GM_xmlhttpRequest({
  			method: "POST",
  			url: search_player_url,
 			data: "view=highscore&searchUser=" + defenders_ally_member,
 			headers: { "Content-Type": "application/x-www-form-urlencoded" },
 			onload: function(response) {
	 			if (/class="allyLink" href="/.test(response.responseText)){ 	
					defenders_ally_alliance4 = response.responseText.split('class="allyLink" href=');
	...
	var winner2 = CRAC_REPORT.battle.victors.replace(/Ganadores: /g,'');
	var winner1 = winner2.replace(/Ganadores: /g,'');
	var winner3 = winner1.split(',');
	var winner = winner3[0];
	var loser2 = CRAC_REPORT.battle.losers.replace(/Perdedores: /g,'');
	...
	if(winner_attacker==1) { 
		...
		if(still_process.test(document.getElementById('mainview').innerHTML)) { 
	...
	results += '<br/>';
	results += '<br/>[b][i]Compactado con: [url=http://board.ikariam.es/index.php?page=Thread&threadID=129508]Combat Report Auto-Converter[/url] v2.6.8 © by [url=http://board.ikariam.es/index.php?page=User&userID=47984]WTF[/url][/i][/b]';
...
function Toplain(text) {
		text = text.replace('[img]http://'+server+'/skin/resources/icon_wood.gif[/img]','Madera:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_wine.gif[/img]','Vino:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_glass.gif[/img]','Cristal:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_sulfur.gif[/img]','Azufre:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_citizen.gif[/img]','Burgueses:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_wood.gif[/img]','Madera:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_wine.gif[/img]','Vino:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_glass.gif[/img]','Cristal:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_sulfur.gif[/img]','Azufre:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_citizen.gif[/img]','Burgueses:');
		text = text.replace('[b][i]Compactado con: [url=http://board.ikariam.es/index.php?page=Thread&threadID=129508]Combat Report Auto-Converter[/url] v2.6.8 © by [url=http://board.ikariam.es/index.php?page=User&userID=47984]WTF[/url][/i][/b]','Compactado con: Combat Report Auto-Converter v2.6.8 © by WTF');
...
function BuildReport() {
	...
	var new_mainview =  '<div class="footer"></div></div><div class="contentBox01h"><h3 class="header"><a href="http://board.ikariam.es/index.php?page=Thread&threadID=129508" target="_blank">Combat Report Auto-Converter</a> v2.6.8 © by <a href="http://board.ikariam.es/index.php?page=User&userID=47984" target="_blank">WTF</a></b></h3><div class="content"><table style="border:none;"><tr><td style="border:none; height:8px;"></td></tr><tr><td style="vertical-align:middle; text-align:center; border:none;">Haz click sobre Seleccionar la parte que quieras, pulsa CTRL+C y luego pégala en el foro pulsando CTRL+V';
	...
	new_mainview += '</td></tr><tr style="border:none;"><td style="border:none; width:674px; text-align:center;"><button class="button" style="margin-right:40px;" id="CRAC_selectallsubject">Seleccionar asunto</button><button style="margin-right:40px;" class="button" id="CRAC_selectall">Seleccionar contenido</button>';
	...
	new_mainview += '</td></tr><tr><td style="text-align:center; border:none; height:1px; width:674px;"><textarea id="CRAC_toselectsubject" style="width:0px; height:0px; border:none;">' + final_subject + '</textarea><textarea id="CRAC_toselect" style="width:0px;  height:0px; border:none;">' + final_text + '</textarea><div style="display:hidden;" id="CRAC_intro"></div></td></tr></table></div><div class="footer"></div></div>';
	...
	if(GM_getValue('CRAC_iscollapsed')==2) {
		new_mainview += 'style="display:none;"';
	}
	new_mainview += '><table><tr><td style="border:none; text-align:center;"><form action="#" method="POST" name="CRAC_options" id="CRAC_options"><table><tr><td rowspan="2" style="border:none; vertical-align:middle; text-align:center; width:115px;"><b>Tipo de foro:</b></td><td style="border:none; text-align:left; width:559px;"><span style="margin-right:100px;"><input type="radio" id="forumclass1" name="forumclass" value="Burning"> Burning Board (Foro Ikariam)</span><span style="margin-right:100px;"><input type="radio" id="forumclass2" name="forumclass" value="phpBB3"> phpBB3</span><input type="radio" id="forumclass3" name="forumclass" value="phpBB2"> phpBB2</td></tr><tr><td style="border:none; text-align:left;"><span style="margin-right:74px;"><input type="radio" id="forumclass4" name="forumclass" value="SMF">SMF</span><span style="margin-right:74px;"><input type="radio" id="forumclass5" name="forumclass" value="vBulletin"> vBulletin</span><span style="margin-right:74px;"><input type="radio" id="forumclass6" name="forumclass" value="internal"> Interno Alianza</span><input type="radio" id="forumclass7" name="forumclass" value="plain"> Sin formato</td></tr><tr><td style="height:10px; border:none;"></td></tr><tr><td style="border:none; text-align:center;"><b>Color de fondo:</b></td><td style="border:none; text-align:left;"><span style="margin-right:80px;"><input type="radio" id="background1" name="background" value="Light"> Claro (Foro Ikariam) </span><input type="radio" id="background2" name="background" value="Dark"> Oscuro</td></tr><tr><td style="height:10px; border:none;"></td></tr><tr><td style="border:none; vertical-align:middle; text-align:center;"><b>Introducción:</b></td><td style="border:none; text-align:left;"><span style="border:none; margin-right:20px;"><input type="checkbox" id="hour" name="hour"> Hora</span><span style="border:none; margin-right:20px;"><input type="checkbox" id="city" name="city"> Nombre de la ciudad</span><span style="border:none; margin-right:20px;"><input type="checkbox" id="CRAC_bfsize" name="CRAC_bfsize"> Tamaño del campo de batalla</span><input type="checkbox" id="CRAC_showrounds" name="CRAC_showrounds"> Número de rondas</td></tr><tr><td style="height:10px; border:none;"></td></tr><tr><td style="border:none; vertical-align:middle; text-align:center;"><b>Resumen tropas:</b></td><td style="border:none; text-align:left;"><span style="border:none; margin-right:32px;"><input type="checkbox" id="CRAC_wordunits" name="CRAC_wordunits"> Palabra "unidades" </span><span style="border:none; margin-right:32px;"><input type="checkbox" id="CRAC_troops_percent" name="CRAC_troops_percent"> Porcentajes perdidos de cada unidad </span><input type="checkbox" id="CRAC_showwall" name="CRAC_showwall"> Nivel de la muralla</td></tr><tr><td style="height:10px; border:none;"></td></tr><tr><td rowspan="2" style="border:none; vertical-align:middle; text-align:center;"><b>Informes y estadísticas:</b></td><td style="border:none; text-align:left;"><span style="border:none; margin-right:22px;"><input type="checkbox" id="CRAC_ratio" name="CRAC_ratio"> Relación de pérdidas</span><span style="border:none; margin-right:22px;"><input type="checkbox" id="CRAC_troops" name="CRAC_troops"> Número de unidades perdidas</span><input type="checkbox" id="CRAC_off_points" name="CRAC_off_points"> Puntos ofensivos y defensivos</td></tr><tr><td style="border:none; text-align:left;"><span style="border:none; margin-right:24px;"><input type="checkbox" id="CRAC_percent" name="CRAC_percent"> Porcentajes en los resúmenes</span><span style="border:none; margin-right:24px;"><input type="checkbox" id="CRAC_loot" name="CRAC_loot"> Botín de guerra</span><input type="checkbox" id="CRAC_ally" name="CRAC_ally"> Alianza en asunto (sólo cuenta una)</td></tr></table></form><button class="button" id="CRAC_save_settings">Click aquí si has modificado algo de la configuración</button></td></tr></table></div><div class="footer">';
	...
	if(GM_getValue('CRAC_ally')==2) { 
		...
		mainview = mainview.replace(cadena_CRAC_ally,cadena_CRAC_ally_checked);
	}
	...
	var addonclick4 = document.getElementById('CRAC_selectall');
	addonclick4.addEventListener("click", SelectAll, false);

	var addonclick5 = document.getElementById('CRAC_selectallsubject');
	addonclick5.addEventListener("click", SelectAllSubject, false);
