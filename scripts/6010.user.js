...
/*
 * $Id: PHI+.user.js,v 1.2 2006/11/02 09:06:23 Pipes6969 Exp $
 *
 * $Log: PHI+.user.js,v 1.1 $
 * Revision 1.2  2006/11/02 09:06:23 Pipes6969
 * Go to Tools->User Script Commands->  Added some cool stuff.
 * 
...
//.//div[@id='main']/div[@id='content']/table[@class='t_little' and position()=1]/tbody/tr[4]/td


//var AddPer = document.evaluate(".//div[@id='main']/div[@id='content']/table[@class='t_little' and position()=1]/tbody/tr[1]/td[2]", XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;

//var Mystuff = document.createTextNode(' Add to buddies');

//AddPer.appendChild(document.createTextNode(' Add to buddies'));



//AddPer.parentNode.AppendChild(+Mystuff+);
...
//
	...
	var cdeftable = document.evaluate(".//div[@id='cdef']/div[@class='alt2']/table", contents, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null).singleNodeValue;
	var totDef = 0, totAtt = 0;
	
	for (i = 0; i < cdeftable.rows.length; i++){
	var thedef = cdeftable.rows[i].cells[0].textContent.toLowerCase();
		unit = units[thedef];
		attack = unit.getAttackScore(cdeftable.rows[i].cells[1].textContent);
		defence = unit.getDefenseScore(cdeftable.rows[i].cells[1].textContent);
	totAtt += Math.round(attack);
	totDef += Math.round(defence * defendersBoost);
	}

	var thecell = document.createElement('DIV');
	thecell.className = 'separator title';
	thecell.innerHTML = "Static Att / Def: " + evoNumber2String(totAtt) + " / " + evoNumber2String(totDef);
	cdeftable.parentNode.appendChild(thecell);
...
var memberTables = document.evaluate(".//div[@id='alliancememberlist']/form/div/table", contents, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
	...
	for (var i = 0; memberTable = memberTables.snapshotItem(i); i++) {
		...
		for (var j = 1; j < memberTable.rows.length; j++) {
			...
			else if( target > tmax ) {score.style.color ='dodgerblue'; if (isPrimary == true) { nbr++; }else{ snbr++;} }
			else{score.style.color = 'lime'; if (isPrimary == true) { nbr++; }else{ snbr++;} }
...
var totalscanable = Math.round((curAmps-(curLand * boosts[UT_RATIO]))/boosts[UT_RATIO] - 1);


scantable.rows[3].cells[2].innerHTML = scantable.rows[3].cells[2].innerHTML + '<br /><span style="text-size: 0.8em;color: greenyellow">' +ratio+ '</span>/<span style="text-size: 0.8em;color: greenyellow">'+totalscanable+'</span>';
}
...
function evoScans() {
	...
	if (scan.indexOf("Sector Scan") != -1) {
	    ...
	    for( var i=6; i<=9; i++) {
	        if( rows[i].cells[1].innerHTML.length > 0 ) rows[i].cells[1].innerHTML = evoNumber2String(parseInt(rows[i].cells[1].innerHTML));
	        if( rows[i].cells[3].innerHTML.length>0 ) rows[i].cells[3].innerHTML = evoNumber2String(parseInt(rows[i].cells[3].innerHTML));
	    }
...
// Add the function to add numbers to the correct box when clicking
	var AddByClick80 = function(e) {
                 	fleetsTable.rows[this.parentNode.rowIndex].cells[this.cellIndex+1].getElementsByTagName('INPUT')[0].value = (this.textContent * boosts[UT_FLEET]).toFixed(0);
	        };
	for( var i = 1; i < (fleetsTable.rows.length); i++ ) {
		for( var j = 1; j <= 7; j+=2 ) {
		if (fleetsTable.rows[i].cells.length != 9) continue;
		...
		row.addEventListener("click", AddByClick80, false);
...
//myBody = document.className("alt1")[0];


//myBodyElements = myBody.className("p");



//myTextNode = document.createTextNode("80 %");
...
function evoNews() {
	...
	//var evoExpTerm = ".//h3[@class='cnewstitle' and child::span[starts-with(.,'Battle Report')]]/following-sibling::table";
	var evoExpTerm = "//TD[contains(text(), 'Battle')]/../following-sibling::TR/TD/table";
	var RegExpTerm = (dEvo ? dEvoExpTerm : evoExpTerm);
	...
	for (var i = 0; battleReport = battleReports.snapshotItem(i); i++) {
		...
		for (j = 3; j < rows.length; j++) {
		  var unitName = rows[j].cells[0].innerHTML.toLowerCase();
		  if (unitName == "therizinosaurus") unitName = "therizinosaurus";
			unit = units[unitName];
			if (!unit) break;
...
function evoConfigBoosts() {
	...
	//if( (n = prompt("Percent of Fleet to be selected? (e.g. 0.8 = 80%, 1 = 100%)", boosts[UT_FLEET]*1)) != null && !isNaN(n = Number(n)) )
	//	GM_setValue('boostFleet', String(n/100));
	alert("Please reload the page for the changes to take effect.");
}

GM_registerMenuCommand( "Configure Load Fleet by ...", evoConfigPercent, null, null, '%' );
function evoConfigPercent() {
	var n;
		if( (n = prompt("Percent of Fleet to be selected? (e.g. 0.8 = 80%, 1 = 100%)", boosts[UT_FLEET]*1)) != null && !isNaN(n = Number(n)) )
		GM_setValue('boostFleet', String(n));
		alert("Please reload the page for the changes to take effect.");
		//alert("SHIT "+boostFleet);

}



GM_registerMenuCommand( "Configure Amp Ratio ...", evoConfigRatio, null, null, '/' );
function evoConfigRatio() {
	var n;
		if( (n = prompt("This is for your Land Scan:Amp Ratio.  Enter how many Amps you need for 1 Land Scan (e.g. 1:4 = 4, 1:9.8 = 9.8 )", boosts[UT_RATIO]*1)) != null && !isNaN(n = Number(n)) )
		GM_setValue('boostRatio', String(n));
		alert("Please reload the page for the changes to take effect.");
		//alert("SHIT "+boostFleet);
...
function evoUnitsInitialize() {
	...
	if(! dEvo ) {
		...
		units['therizinsaurus']   = new evoUnit('Therizinsaurus'   ,UT_ENG     ,6   ,5300   ,2650   ,26  ,22  ,3   ,1  ,40 ,true);
