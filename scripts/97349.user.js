...
// @description    Script para facilitar o trabalho da moderacao da UserBars - UB
// @dgohcodes      orkut.com.br/Community?cmm=111097390*
// @include        *.orkut.*/Main#CommMsgs?cmm=111097390*
// @include        *.orkut.*/CommMsgs?cmm=111097390*
// @include        *.orkut.*/Main#CommMemberManage?cmm=111097390*
// @include        *.orkut.*/CommMemberManage?cmm=111097390*
// @include        *.orkut.*/Community?cmm=111097390*
// @include        *.orkut.*/Main#Community?cmm=111097390*
// ==/UserScript==

/*
+---------------------------------------------------------------------+
| 		Visite a comunidade UserBars - UB      |
|		http://www.orkut.com.br/Main#CommMsgs?cmm=111097390&tid=5575296570804868584&start=1     |
|		 				      |
|						      |
+---------------------------------------------------------------------+
*/
...
eval("5575297760510809576");

var colocaNaPagina = function(){

tpc="CommMsgPost?cmm=111097390&tid=5575297760510809576";
...
function captchaONOFF(env, status)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?cmm=111097390&tid=5575296570804868584", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function postaTopicoDeletado(titulo, uid, motivo)
	{
		...
		var texto = "[b]<font color=#00008B'>Titulo:</font>[/b] ".concat(titulo,breakLine,
			    "[b]<font color='#0000CD'>Criador:</font>[/b] ",uid,breakLine, 
			    "[b]<font color='#0000EE'>Motivo:</font>[/b] ",motivo,breakLine,
			    "[b]<font color=#0000FF'>Horario:</font>[/b] ", pegaData(),breakLine,breakLine,
			    "[silver]",rnd,"[/silver]");
		var envia = {
