// ==UserScript==
// @name           Assassin-a-Nator Ice Tester
...
javascript:(function(){
	...
	//initial variables
	var version = 'Assassin-a-Nator v0.62 beta',
	userid = User.id,
	...
	'<div id="'+spocklet+'_main">'+
		...
		'<tr><td colspan="3">'+
			...
			'<div style="float:right;"><a href="#" id="'+spocklet+'_mode" class="sexy_button_new short black" title="Click to change attack mode"><span><span>Fightlist Mode</span></span></a></div>'+
		'</td></tr>'+
...
//				'<input id="'+spocklet+'_exp_level" type="text" value="0" maxlength="4" name="'+tagname+'" /> exp to level&nbsp;&nbsp;'+
			...
			'</td>'+
		'</tr>'+
		'<tr id="'+spocklet+'_runtime_row" style="display:none;"><td>Runtime:</td><td colspan="2"><span id="'+spocklet+'_runtime"></span><br />Error 500: <span class="bad" id="'+spocklet+'_err500" /> <span class=more_in>(~<span id="'+spocklet+'_err500_perc" />%)</span></td></tr>'+
	...
	function parse_attack(response) {
		// if (/Sniper?Shot/i.test(response)) {
			// log('Sniper Shot');
			// alert(response);
			// run = false;
			// return;
		...
		if (/index_controller/.test(response)) {
			...
			pausing(2,'Game responded with Error 500, multiple action blocked. Trying again in ',function(){ repeat_attack(); });
			stats.err500++;
	...
	// $('#'+spocklet+'_log_toggle, #'+spocklet+'_loot_toggle, #'+spocklet+'_popup_toggle').bind('click', function () {
		// if ($('#'+this.id.replace(/_toggle/,'')).css('display') != 'none') {
			// $(this).addClass('green').removeClass('black');
			// $('#'+this.id.replace(/_toggle/,'')).show();
		// }
