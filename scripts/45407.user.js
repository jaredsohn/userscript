// ==UserScript==
// @name           Facebook MobWars Helper 1.1.4 Beta
// @namespace      http://userscripts.org/users/84113
// @description    Helps and improves the playing experience of the Facebook application MobWars with automatic functions.
// @source         http://userscripts.org/scripts/show/45402
// @identifier     http://userscripts.org/scripts/source/45402.user.js
// @version        1.1.4
// @date           2009-03-29
...
Menu.init = function() {
  ...
  menuCode.push('<div><input type="checkbox" id="quickHeal"' + (GM_getValue("MWHelper_Quickheal", false)? ' checked="checked"' : '') +'>Enable Quickheal</input></div>');
  ...
  document.getElementById("quickHeal").addEventListener('click', function (){GM_setValue("MWHelper_Quickheal", this.checked); if (this.checked) quickHeal();}, true);
...
function quickHeal(){
	try{
		var health = parseInt(document.getElementById("app8743457343_cur_health").innerHTML);
		if(health < (boss.max_health * .6)){
			GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://' + location.host + '/mobwars/hospital/',
				headers: {
					'User-agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.0.7) Gecko/2009021910 Firefox/3.0.7 (.NET CLR 3.5.30729)',
					'Host': location.host,
					'Referer': 'http://' + location.host + '/mobwars/hospital/',
					'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
					'Cookie': document.cookie
				},
				onload: function(responseDetails) {
					var start = responseDetails.responseText.indexOf("<form method=\"post\" action=\"do.php\"");
					var end = responseDetails.responseText.indexOf("</form>", start) + 7;
					var form = loadXMLString(responseDetails.responseText.substring(start,end));
					var req = "http://" + location.host + "/mobwars/hospital/do.php";
					for (i in form.getElementsByTagName("input")){
						if (form.getElementsByTagName("input")[i].attributes.getNamedItem("type").value != "submit"){
							req += ((i == 0)? "?" : "&") + form.getElementsByTagName("input")[i].attributes.getNamedItem("name").value
							req += "=" + form.getElementsByTagName("input")[i].attributes.getNamedItem("value").value
						}
					}
					if (req != "http://" + location.host + "/mobwars/hospital/do.php"){
						GM_xmlhttpRequest({
							method: 'GET',
							url: req,
							headers: {
							...
							onload: function(responseDetails) {
							  var regex = new RegExp('(app8743457343_cur_health"[a-z 0-9\="]+>)([0-9]+)', 'gi');
							  regex.test(responseDetails.responseText);
							  var health = RegExp.lastParen;
							  document.getElementById("app8743457343_cur_health").innerHTML = health;
							  if(parseInt(health) < (boss.max_health * .6)) quickHeal();
							},
	...
	} catch(e){GM_log("Error in Mobwars Helper healMe:" + e.message);}
}
function Boss() {
  this.name = 'Anonymous';
  this.cash = 0;
...
if (GM_getValue("MWHelper_Quickheal", false)) quickHeal();
boss.new_level = 0;
boss.save();
} ) ();
/* vim:set tw=80 sts=2 et ft=javascript: */