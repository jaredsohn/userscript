...
wparent.location.href.indexOf('index.php?view=diegoflavio') != -1||
	...
	function rec(ctr){
		...
		if(arr[ctr]){
			if(typeof adsWind == 'undefined')
				...
				adsWind.location.href = arr[ctr].href;
			$(adsWind).ready(function(){adsWind.resizeTo(600,600)})	
			div.text('So far we have clicked : '+clickNum+' / '+ arr.length);
		}
		else{	
			var timeReload=120000;
			var inters = setInterval(function(){	
				timeReload-=1000;
				div.text("Reloading in :"+timeReload);
				if(timeReload == 0){
					clearInterval(inters)
					window.location.reload()
				}
