...
var infoneueversion = "Neu in dieser Version vom Pennermenue:\n- Unterstuetzung von PG Muenchen\n- Bug beim Auslesen der User-ID behoben\n- Einige weitere kleine Bugfixes\n\nViel Spass!";
...
var stadtbuchstabe;
if(document.location.href.indexOf("berlin")>=0) {
				stadtbuchstabe = "b";
} else {
				stadtbuchstabe = (document.location.href.indexOf("muenchen")>=0) ?  "m" : "hh";
}

var id = document.getElementsByTagName("body")[0].innerHTML.split('<h2><a href="/profil/id:')[1].split('/" title="Dein User Profil anzeigen">')[0];
...
var nachrichticon_url = "http://file1.npage.de/001730/84/bilder/nachricht.png";
...
button[8] = new buttonserst("nachricht", "Nachrichten direkt schreiben", "<td><img src='"+nachrichticon_url+"' style='cursor:pointer' title='Eine Nachricht direkt an einen Spieler schreiben' id='nachricht'></td>", false);
...
function einzahlenfkt(posx,posy) {
				if(document.location.href.indexOf("/gang/credit/")>=0) {
								alert("Du bist bereits auf der Einzahlseite, bitte die Funktion dort benutzen!")
				} else {
	              ...
	              if(!kassenlayer) {
	                      ...
	                      document.getElementById("einzahlbutton").addEventListener("click", function() {
	                              document.getElementById("f_money").value = parseInt(document.getElementById("betrag_kasse").value);
...
function weiterbildenfkt(posx,posy,trinkenfertig) {
				...
				function wbplanlegen() {
								if (plunderbody.indexOf("> "+wbplunder+"<")>=0) {
												alert("Der Weiterbildungsplunder ist bereits angelegt, es wird direkt zur Weiterbildungsseite gewechselt. Wenn die Promille zu niedrig sind, bitte den Weiterbildungsplunder wieder ablegen und die Funktion erneut versuchen!");
...
function nachrichtfkt() {
				var nachrichtlayer = document.getElementById("nachrichtlayer");
				if(!nachrichtlayer) {
								var nachrichtdiv = document.createElement("div");
								nachrichtdiv.setAttribute("style","display:block;width:500px;position:absolute;top:"+(((screen.height-200)/2)-100)+"px;left:"+(screen.width-500)/2+"px;background-color:black;color:white;font-size:10pt;border:solid 2px black;z-index:200;padding:10px;");
								nachrichtdiv.innerHTML='<div><form name="form2" id="nachrichtform" method="post" action="/messages/write/send/"></form></div>';
								nachrichtdiv.setAttribute("id","nachrichtlayer");
								nachrichtdiv.setAttribute("align","left");
								document.getElementsByTagName("body")[0].appendChild(nachrichtdiv);
								document.getElementById("nachrichtform").innerHTML+='Empf&auml;nger:<br><input type="text" style="border:0;background:#646260;color:black;" name="f_toname" id="f_toname"><br><br>Betreff:<br><input type="text" style="border:0;background:#646260;color:black;" name="f_subject" id="f_subject"><br><br><textarea id="f_text" name="f_text" rows="5" style="background-color:#54514d;border:0;width:100%"</textarea><input type="hidden" value="" name="f_did" id="f_did"><br><br><input type="submit" value="Abschicken">'+
								'&nbsp;<input type="button" id="versteckenachricht" value="Abbrechen">';
								nachrichtlayer = document.getElementById("nachrichtlayer");

								document.getElementById("versteckenachricht").addEventListener("click",ausblenden,false);
				} else {
								einblenden();
				}

				function einblenden() {
								nachrichtlayer.style.display="block";
				}
				function ausblenden() {
												nachrichtlayer.style.display="none";
				}
}
//###################################################################################################################################################
//###################################################################################################################################################
