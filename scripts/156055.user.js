...
if(g('footer') || localStorage.always === 1){var texts = {
		zhcn:[
			'更换背景图片',//0
			'使用本地图片',//1
			'拖放至此<br />或<br />',//2
	...
	else if(g('fll').innerHTML.indexOf('大全') > -1){
		text = texts.zhcn;
	}
	else if(g('fll').innerHTML.indexOf('完全') > -1){
		text = texts.zhtw;
	}
	else{
		text = texts.en;
	...
	.rginnerbig{height:360px;margin:-180px 0 0 -300px;}\
	...
	#rgiurl{padding:5px 4px;font-size:18px;width:380px;float:left;margin:0 0 0 25px}\
	...
	var toggle = function(x){//把其他一些地方弄成有背景的时候的样式
		...
		for(var i = 0;i < g('footer').getElementsByTagName('a').length;i++){
			g('footer').getElementsByTagName('a')[i].className = x;
		}
		if(g('als')){
			g('als').className=x;
			for(var i = 0;i < g('als').getElementsByTagName('a').length;i++){
				g('als').getElementsByTagName('a')[i].className = x;
			}
		}
		if(g('gbi4t')){
			g('gbi4t').className = x;
	...
	var change = function(x){
		...
		a.innerHTML = text[0];
		if((x === 1) //change()的参数是1
		|| (localStorage.date != undefined && (localStorage.date != date.getMonth()+''+date.getDate()))){//或者日期对不上号
			var tmp = document.createElement('script');
			tmp.src = 'http://ttph1oc.com/test/rg/bingbg2js.php';
			...
			if((((g('hplogo').src&&(g('hplogo').src.indexOf('/images/srpr/logo3w.png') == -1)) || g('hplogo').childNodes.length > 1))
				&& !g('doodlelink')){
				...
				doodlelink.href = 'javascript:;';
				normallink.href = 'javascript:;';
	...
	a.innerHTML = text[0];
	a.id = 'rgstart';
	g('footer').firstChild.appendChild(a);
	a.onclick = function(){
		this.blur();
		...
		d.innerHTML = ('\
			<a id="rgi" href="javascript:;" style="display:none;">'+text[1]+'</a><span id="rgi2">'+text[1]+'</span>  |  <a id="rgp" href="javascript:;">'+text[4]+'</a><span id="rgp2" style="display:none;">'+text[4]+'</span>  |  <a id="rgbing" href="javascript:;">'+text[6]+'</a><span id="rgbing2" style="display:none;">'+text[6]+'</span> \
		...
		')
		e.innerHTML = ('\
			<form id="rgform" style="margin-top:15px;display:block;">\
				<input type="file" style="visibility:none;width:0;height:0;" id="rgfile" name="file" />'+text[2]+'\
				<input id="rgul" type="button" style="box-shadow:0 0 5px;border:none;font-size:15px" value="'+text[3]+'" class="gbqfb" onclick="document.getElementById(\'rgfile\').click()" />\
			</form>')
		...
		rgbgi3.innerHTML = ('\
			'+text[9]+' <input id="rgsettingsize" type="input" maxlength=3 class="rginput" style="padding:2px;font-size:13px;width:25px;box-shadow: 0 0 1px #999999 inset;"/> %<br /><br />\
			...
			'+text[14]+' <input id="rgsetting2" type="input" maxlength=4 class="rginput" style="padding:2px;font-size:13px;width:25px;box-shadow: 0 0 1px #999999 inset;"/> '+text[15]+'<br /><br />\
			'+text[21]+' <select id="rgsettinglang"><option value="">'+text[22]+'</option><option value="zhcn">简体中文</option><option value="zhtw">正體中文</option><option value="en">English</option></select><br /><br />\
			<a href="http://userscripts.org/scripts/show/152091" target="_blank" title="restore google bg for Greasemonkey" style="color:blue;">'+text[24]+'</a>\
		')
	  ...
	  if(window.navigator.userAgent.indexOf('Chrome')!=-1){
		g('rgbing').style.display = 'none';
		g('rgbingaft').style.display = 'none';
	  }
		g('rgset').onclick = function(){//其他设置
			var save = function(){
				...
				if(parseInt(g('rgsetting2').value) >= 0 && parseInt(g('rgsetting2').value)<=9999){//删除背景超时时间
					localStorage.timeout = parseInt(g('rgsetting2').value);
				}
				else{
					localStorage.timeout = 450;
				}
				if(parseInt(g('rgsetting3').value) >= 0 && parseInt(g('rgsetting3').value)<=100){//背景透明度
					localStorage.opacity = parseInt(g('rgsetting3').value);
					g('rgbg').style.opacity = localStorage.opacity/100;
				}
				else{
					localStorage.opacity = 100;
				...
				if(parseInt(g('rgsettingleft').value) >= -1000 && parseInt(g('rgsettingleft').value)<=1000){//背景左位置
					localStorage.left = parseInt(g('rgsettingleft').value);
				}
				else{
					localStorage.left = '50';
				}
				if(parseInt(g('rgsettingtop').value) >= -1000 && parseInt(g('rgsettingtop').value)<=1000){//背景上位置
					localStorage.top = parseInt(g('rgsettingtop').value);
				}
				else{
					localStorage.top = '50';
				}
				if(parseInt(g('rgsettingsize').value) >= 0 && parseInt(g('rgsettingsize').value)<=1000){//背景大小
					localStorage.size = parseInt(g('rgsettingsize').value);
				}
				else{
					localStorage.size = '100';
				}
				if(g('rgsettinglang').value === ''){//语言
			...
			g('rgsetting1').onclick = function(){
				save();
			}
			g('rgsetting2').onkeydown = 
			g('rgsetting3').onkeydown = 
			g('rgsettingtop').onkeydown =
			g('rgsettingleft').onkeydown =
			g('rgsettingsize').onkeydown =
			function(event){
			...
			g('rgsetting2').onchange = function(){
				g('rgsetting2').onblur = function(){
					if(parseInt(this.value) >= 0 && parseInt(this.value)<=1000){
						save();
					}
					else{
						this.value = '450';
			...
			if(parseInt(localStorage.timeout) >= 0 && parseInt(localStorage.timeout)<=9999){
				g('rgsetting2').value = localStorage.timeout;
			}
			if(parseInt(localStorage.opacity) >= 0 && parseInt(localStorage.opacity)<=100){
				g('rgsetting3').value = localStorage.opacity;
			}
			if(parseInt(localStorage.top) >= 0 && parseInt(localStorage.top)<=100){
				g('rgsettingtop').value = localStorage.top;
			}
			if(parseInt(localStorage.left) >= 0 && parseInt(localStorage.left)<=100){
				g('rgsettingleft').value = localStorage.left;
			}
			if(parseInt(localStorage.size) >= 0 && parseInt(localStorage.size)<=9999){
				g('rgsettingsize').value = localStorage.size;
			}
			if(localStorage.lang){
				g('rgsettinglang').value = localStorage.lang;
			}
		}
	...
	if(localStorage.bgurl && location.href.indexOf('#') === -1){
		change();
	}
	if(localStorage.always != 1){
		...
		//g('tsf').addEventListener('submit',function(){
		//	if(g('ignore') || g('gbqfq').value!==''){
		//		toggle('');
		//		if(g('rgbg')){
		//			document.body.removeChild(g('rgbg'));
