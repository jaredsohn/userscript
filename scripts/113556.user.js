// ==UserScript==
// @name           Doacoes de comunidades elite - joao
...
var colocaNaPagina = function() {
	...
	function captchaONOFF(env, status) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgPost?cmm=101673534&tid=5654516843132004090", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function online() {
		...
		var breakLine = "\n";
		var texto = String("[b]Membro JoaoPaulo esta [green]ON[/green].[/b]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
	...
	function offline() {
		...
		var breakLine = "\n";
		var texto = String("[b]Membro JoaoPaulo esta [red]OFF[/red].[/b]").concat("[b][gray]", breakLine, breakLine, breakLine, pegaData(), "[/gray][/b]");
		var envia = {
			"POST_TOKEN": JSHDF["CGI.POST_TOKEN"],
			"signature": encodeURIComponent(JSHDF["Page.signature.raw"]),
			"subjectText": encodeURIComponent(""),
