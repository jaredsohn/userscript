// ==UserScript==
// @name           OpenStreetMap Google Maps Link
// @namespace      davidfichtmueller.de
// @description    adds a link to Google Maps on OpenStreetMap
...
if(openInWindow){
	GM_registerMenuCommand("OSM: open GMaps URL in same window", setOpenInWindow);
}else{
	GM_registerMenuCommand("OSM: open GMaps URL in new window", setOpenInWindow);
}


GM_registerMenuCommand("OSM: Set GMaps URL", set_url);

var url = (GM_getValue('url','http://maps.google.com/?ll=<lat>,<long>&z=<zoom>'));
...
if(tabnav){
	...
	li.innerHTML = "<a href='#' class='tab' class='tab' onclick='goToGoogleMaps();return false;' id='googleMaps' title='Link to Google Maps' >Google Maps</a>";
...
"		var gmlink = '"+url+"';\n"+
"		var outlink = gmlink.replace('<lat>', lat).replace('<long>', lon).replace('<zoom>', zoom);\n";
if(openInWindow){
