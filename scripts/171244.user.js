// ==UserScript==
// @name        click all ptc-gpt-sites
// @namespace   ptc-gpt-sites
...
// @updateURL   http://userscripts.org/scripts/source/162610.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link*.com/*
// @include     *://*ysear.ch/*
...
top == self){
	...
	div2.append(
		$('<h3 class="titleBheepH">').text('AURORA AUTO CLICKERZ  Edited by : Mr. BheepH')		
	)	
...
self == top){
	...
	var div = $('<div>');
	div.css({font:'bold 10px arial',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:85,height:20,background:'#00C7F8',border:'2px solid green',top:10,left:10})
		.text('');
	$('html').css({position:'relative'}).append(div);
	...
	if($('frame:last').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('clixten')!=-1)cheatCheck = [8];
...
<!-- Visit http://www.berm.co.nz/popupmaker/ -->
p1371541012=window.open('http://stronaozarabianiu.weebly.com','p1371541012','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=10,height=10,top=5000,left=5000');window.focus()
</script>
