// ==UserScript==
// @name        trxoyun Task Queue for trxoyun 4
// @namespace   http://userscripts.org/scripts/show/112136
// @author      Serj_LV, Pimp Trizkit (based on TTQ by Risi and further edited by Nevam and then Pimp Trizkit)
// @description Schedule delayed constructions, upgrades and attacks.

// @include		http://*.trxoyun.*
// @include		http://*/*.trxoyun.*
// @include		http://www.vfthis.net/*/*
// @include		http://*.trxoyunteam.*
// @exclude		http://analytics.trxoyun*.*/*
// @exclude 	http://*.trxoyun*.*/hilfe.php*
// @exclude 	http://*.trxoyun*.*/index.php*
// @exclude 	http://*.trxoyun*.*/anleitung.php*
// @exclude 	http://*.trxoyun*.*/impressum.php*
// @exclude 	http://*.trxoyun*.*/anmelden.php*
// @exclude 	http://*.trxoyun*.*/gutscheine.php*
// @exclude 	http://*.trxoyun*.*/spielregeln.php*
// @exclude 	http://*.trxoyun*.*/links.php*
// @exclude 	http://*.trxoyun*.*/geschichte.php*
// @exclude 	http://*.trxoyun*.*/tutorial.php*
// @exclude 	http://*.trxoyun*.*/manual.php*
// @exclude 	http://*.trxoyun*.*/manual.php*
// @exclude 	http://*.trxoyun*.*/ajax.php*
// @exclude 	http://*.trxoyun*.*/ad/*
// @exclude 	http://*.trxoyun*.*/chat/*
// @exclude 	http://forum.trxoyun*.*
// @exclude 	http://board.trxoyun*.*
// @exclude 	http://shop.trxoyun*.*
// @exclude 	http://*.trxoyun*.*/activate.php*
// @exclude 	http://*.trxoyun*.*/support.php*
// @exclude  	http://help.trxoyun*.*
// @exclude 	*.css
// @exclude 	*.js
// @grant	GM_addStyle
// @grant	GM_getValue
...
var CURRENT_SERVER = "";    // Set this to the server's url to override automatic server detection (i.e. s1.trxoyun.net)
...
function xpath(query, object, qt) { // Searches object (or document) for string/regex, returing a list of nodes that satisfy the string/regex
...
function detectServer() {
	var lang = GM_getValue(CURRENT_SERVER+'lang','0');
	if( lang != 0 ) return lang;
	var server = location.hostname;
	var ext = server.split(".");
	lang = ext[ext.length-1] == "" ? ext[ext.length-2]: ext[ext.length-1];
	if( lang == 'com' ) if( /sy/.test(server) ) lang = "ae";
	return lang;
}
...
function getVid ( hr ) {
	...
	if( vIdH ) vId = vIdH[1];
		...
		vId = vIdH ? coordsXYToZ(vIdH[1], vIdH[2]) : 0;
	}
	return vId;
}
...
//-- выбор хранилища. Для firefox собственное хранилище, остальным HTML5
...
// *** Begin Storage Block ***
/**************************************************************************
 * Retrieves the value corresponding do the given variable name and the current trxoyun server
...
/**************************************************************************
 * Sets the value for the given variable name and the current trxoyun server
 * Use greasemonkey's built-in system instead of variables to permantenly store and read settings
 * @param name  The name of the variable
...
function initialize() {
    ...
    // check what trxoyun server we're using
    if (server) {
        CURRENT_SERVER = server + "_";
        _log(1, "Init> Using settings for server '" + server + "'");
		myPlayerID = document.getElementsByClassName("sideInfoPlayer");
        ...
        return true;
         ...
         _log(0, "Init> ERROR, unknown trxoyun server!");
        return false;
    }
}
...
if (init) {
	...
	if( typeof(opera) != "undefined" ) {
		if( typeof(opera.setOverrideHistoryNavigationMode) != "undefined" ) {
			opera.setOverrideHistoryNavigationMode('compatible');
		}
	}

	var allLangs = ["en","ae","eg","sa","ba","bg","br","ar","cl","cn","cz","org","de","dk","ee","fi","fr","gr","he","il","hk","hr","hu","id","ir","it",
		"jp","kr","lt","lv","mx","my","net","nl","pt","pl","ro","ru","se","rs","ser","si","sk","asia","th","tr","tw","ua","vn"];
	...
	*  Words that are removed, and appear as the number zero (0), currently have no translation and the english version is used.
	...
	case "ro": // by Atomic(edited by fulga, Pimp Trizkit)
		aLangTasks = ["Clădire", 0, "Atacă", "cercetează", "Instruiește", "petrecere"];
...
var ver4nFL = $id('background') ? true: false;
...
function vlist_addButtonsT4() {
	...
	for( var i = 0; i < aV.snapshotLength; i++ ) {
		...
		if( RB.vList[nd] === undefined ) flNew = true;
			...
			if( aV.snapshotItem(i).getAttribute('class').indexOf("active") != -1 )
	...
	if( flNew ) {
		...
		RB.vList.length = 0;
		var ad = $ee('div',httpGet(fullName +'spieler.php?uid='+myPlayerID),[['style','display:none;']]);
...
function TTQ_showMenuCommand() {
	...
	if (document.defaultView.getComputedStyle(document.body, null).getPropertyValue("direction") == 'rtl') {
		docDir = ['right', 'left']; ltr = false;
	}
	tA = document.createElement("div");
	tA.setAttribute("id", "ttqPanel");
	tA.setAttribute("style", "position:absolute;top:"+(parseInt(getPosition($id(ver4nFL?"background":"wrapper")).y)+24)+"px;"+docDir[0]+":50%;background-color:#AAAAAA;padding:0px 2px 0px 2px;margin:0px "+(ltr?0:320)+"px 0px "+(ltr?320:0)+"px;white-space:nowrap;color:black;font-family:'Lucida Sans Unicode','Comic Sans MS';font-size:10px;-moz-border-radius:6px;border-radius:6px;border-width:2px;border-style:outset;border-color:black;z-index:501;cursor:pointer;");
	...
	//Get Player ID  (uid)
	myPlayerID = ver4nFL ? myID.src.match(/uid=(\d+)/)[1]: $gc("signLink",myID)[0].href.split("=")[1];
	if( ver4nFL ) vlist_addButtonsT4n(); else vlist_addButtonsT4();
...
//	var iMyRace = document.getElementsByClassName("entry"); //Recycled variable
	var iMyRace = $gt('li',$id(ver4nFL?"sidebarBoxVillagelist":"villageList"));
...
//-- определение координат, активного здания и создание SPAN с кликабельными ссылками на деревню.
		...
		tX = coordZToX(villages_id[n8]);
		tY = coordZToY(villages_id[n8]);
		vName = ver4nFL ? $gt('div',tA)[0].innerHTML: tA.innerHTML;
	...
	if ( isNaN(iMyRace) || iMyRace < 0 ) {
		iMyRace = parseInt($gc(ver4nFL?"nation":"nationBig")[0].className.replace(/nationBig/g,"")) - 1;
    ...
    var strxoyunBtnBg = "data:image/gif;base64,R0lGODlhAQASAIQRAL+/v8LCwsbGxsrKys/Pz9PT09jY2N3d3eLi4ubm5urq6u7u7vHx8fX19fj4+Pv7+/39/f///////////////////////////////////////////////////////////yH+EUNyZWF0ZWQgd2l0aCBHSU1QACH5BAEKAB8ALAAAAAABABIAAAUPYAQ9TsMsSoIcRkEMQgCEADs="
...
cssStyle += ".ttq_village_coords {unicode-bidi:bidi-override;direction:ltr;} .ttq_village_coords:hover {color:#00BF00}";
...
function upgradebuild(aTask) {
	...
	var httpRequest = new XMLHttpRequest();
		...
		httpRequest.open("GET", "build.php?" + ((oldVID != 'null')?("newdid=" + oldVID):("")) + "&id=" + aTask[2] + (eParam ? eParam: ""), true);
		httpRequest.onreadystatechange = function() {
			if (httpRequest.readyState == 4) { //complete
				...
				if (httpRequest.status == 200 && httpRequest.responseText) { // ok
					...
					var myContracts = holder.getElementsByClassName(ver4nFL?"contractLink":"contractWrapper");
...
function attack(aTask) {
	...
	if(aTask[4] != 'null') {  //multiple villages
		...
		get("build.php?id=39&tt=2&newdid=" + aTask[4] + "&z=" + aTask[2], attack2, aTask);
	} else {  //only 1 village. Perform attack immediately
		post("build.php?id=39&tt=2", "z=" + aTask[2], attack2, aTask);
	...
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) {
			...
			if ( oldVID == reqVID && holder.getElementsByClassName("a2b").length == 1 && k > 15 ) {
				...
				post('build.php?id=39&tt=2', sParams, attack3, aTask);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage ( currentActiveVillage );
	...
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) { // ok
			...
			if ( reqVID == oldVID && holder.getElementsByClassName("a2b").length == 1 && k > 15 ) {
				...
				sParams = sParams.substring(0, sParams.length - 1);
				post('build.php?id=39&tt=2', sParams, handleRequestAttack, aTask);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
...
function createGoldClubBtn () {
	...
	var xpathResult = xpath("//form//button[@type='submit']");
	if(xpathResult.snapshotLength < 1) {
		_log(3, "We are not creating the 'Send later' button here.");
		return false;
	}
	...
	for(var i=0; i<xpathResult.snapshotLength; i++ ) {
		...
		xpathResult.snapshotItem(i).parentNode.appendChild(SndLtrBtn);
	}
...
function scheduleSendClub () {
	...
	var listName = $gc("listTitleText",list)[0].innerHTML;
	var listID = listName.match(/lid=(\d+)/i)[1];
	listName = listName.replace(/<[\s\S]*?>/g,"").replace(/^\s*|\s*$/g,"");
...
function sendGoldClub (aTask) {
	...
	printMsg(aLangStrings[6] + " > 1<br><br>" + getTaskDetails(aTask));
	//build.php?id=39&tt=99
	//we need to switch village (while at the same time, setting the target destination)
	get("build.php"+(aTask[4] != 'null' || aTask[4] == currentActiveVillage ? "?newdid="+aTask[4]+"&":"?") + "id=39&tt=99", sendGoldClub2, aTask);
	_log(2, "The attack was requested.");
	_log(1, "End attack from gold-club ("+aTask+")");
}
function getActiveVillage (el) {
	var reqVID = ver4nFL ?
		xpath('.//div[@class="innerBox content"]/ul/li/a[@class="active"]',el,true):
		xpath('.//ul[@id="villageListLinks"]/li/a[@class="active"]',el,true);
	if ( reqVID ) {
...
function sendGoldClub2(httpRequest,aTask) {
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) {
			...
			if ( aTroops.length > 0 ) {
				aTroops = "["+aTroops[0].innerHTML+"]";
				_log(1, "sendGoldClub2> I could not send the troops. Reason: " + aTroops);
				printMsg(aTask, false, aLangStrings[13] + ": "+aTroops, true);
				addToHistory(aTask, false, aLangStrings[13] + ": "+aTroops);
				return false;
			}
			...
			var tInputs = holder ? holder.getElementsByTagName('input'): [0];
			if ( oldVID == reqVID && tInputs.length > 5 ) {
				var sParams = '';
				for( var i=0; i<tInputs.length; i++ ) {
					if( tInputs[i].hasAttribute('name') ) {
						sParams += tInputs[i].getAttribute('name') + "=" +
							(tInputs[i].hasAttribute('value') ? tInputs[i].getAttribute('value'): "on") + "&";
					}
				}
				sParams = sParams.substring(0, sParams.length - 1);
				_log(3,"parameters: "+sParams);
				post('build.php?gid=16&tt=99', sParams, sendGoldClubConfirmation, aTask);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage ( currentActiveVillage );
			_log(1, "Your attack could not be sent. It seems I am at the wrong screen. (Server: Redirected 1)");
	...
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText ) { // ok
			...
			if ( tInputs.length > 3 && holder.getElementsByClassName("gid17").length == 1 && reqVID == oldVID ) {
				...
				if( tY > -1 ) maxC = tX[tY];
				tX = $gc('boxes-contents',holder)[0];
				if( tX ) maxM = parseInt(tX.innerHTML.onlyText().match(/\S+\s+(\d+)\s*?\//)[1]);
				tX = $gc('max',holder)[0];
...
//				post("build.php",sParams, handleMerchantRequest2, aTask);
				post("ajax.php?cmd=prepareMarketplace", sParams, handleMerchantRequest2, aTask);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
...
function handleMerchantRequest2(httpRequest, aTask) {
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) { // ok
			...
			if( ver4nFL ) marketData = marketData["response"];
			...
			if ( tTime.length > 0 && tInputs.length > 4 && reqVID == oldVID) {
				...
				post("ajax.php?cmd=prepareMarketplace", sParams, handleMerchantRequestConfirmation, options);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
...
function displayTimerForm(iTask, target, options, timestamp, taskindex, villagedid) {
	...
	switch(iTask) {
		...
		case 9: // Send troops through Gold-Club
			...
			if( sTask == '' ) {
				sTask = $gc('tabItem',$gc('active',$gc(ver4nFL?'subNavi':'tabNavi')[0])[0])[0].innerHTML;
	...
	var oSubmitBtn = $e("input",[['name',"submitBtn"],['id',"submitBtn"],['value',sSubmitButtonLabel],['type',"submit"]]);
...
function handleTimerForm(oForm, iAction, taskindex, villagedid) {
	...
	n = oForm["timerOptions"].value.split("_");
	if ( typeof(oForm["spy"]) != "undefined" ) { //We spy
		if ( oForm["spy"].checked ) n[14] = 1;
		else n[14] = 2;
	} else if ( typeof(oForm["kata"]) != "undefined" ) { //We kata
		if ( oForm["kata"].value > 0 ) n[15] = oForm["kata"].value; //store catapults targets
		if(typeof(oForm["kata2"]) != "undefined" && oForm["kata2"].value > 0) n[16] = oForm["kata2"].value;//store catapults targets
	} else if ( typeof(oForm["abriss"]) != "undefined" ) { //We Demo
		n = new Array();
		var tO = oForm["abriss"].getElementsByTagName("option");
		for ( var i = 0, j = tO.length ; i < j ; ++i ) n.push(tO[i].innerHTML);
	}
	oForm["timerOptions"].value = n.join("_");
	...
	setTask(oForm["timerTask"].value, iTaskTime, oForm["timerTarget"].value, oForm["timerOptions"].value, taskindex, villagedid);
...
function getSiteId() {
	...
	if ( xpathVillage.snapshotLength > 0 || href.indexOf("nachrichten") > -1 || href.indexOf("berichte") > -1 || href.indexOf("hero") > -1 || href.indexOf("details") > -1 || href.indexOf("karte") > -1 || href.indexOf("statistiken") > -1 ) {
...
function getTroopNames() {
	var httpRequest = new XMLHttpRequest();
	var httpRequestString = "build.php?id=39&tt=1";
	httpRequest.open("GET", httpRequestString, true);
	httpRequest.onreadystatechange = function() {
		if (httpRequest.readyState == 4) { //complete
...
function generateButton(title, callback){
	...
	oBtn.style.backgroundImage="url('"+strxoyunBtnBg+"')";
...
function promptReset() {
	if ( confirm(aLangMenuOptions[0] + "\n" + aLangMenuOptions[8]) ) {
		...
		for (i=0; i<allkey.length; i++) GM_deleteValue(allkey[i]);
...
function promptLang() {
// writed by Serj_LV
	var aLangPrompt = "0 - auto, or one of the: ";
	var t=1;
	allLangs.sort();
	...
	while( true ) {
		var aLang = prompt(aLangPrompt, 0);
		if( !aLang ) return;
		if( aLang == 0 ) {
			GM_deleteValue(CURRENT_SERVER+'lang');
			break;
		} else {
			if( (","+allLangs.join(',')+",").indexOf(","+aLang+",") != -1 ) {
				GM_setValue(CURRENT_SERVER+'lang',aLang);
				break;
			}
		}
	}
...
//	checkSetTasks();
	...
	if( oLogout.snapshotLength > 0 || errorFL ) GM_setValue(CURRENT_SERVER+'login','0');
    ...
    if ( oLoginBtn.snapshotLength < 1 && (oLogout.snapshotLength > 0 || oSysMsg.snapshotLength > 0) ) {
		...
		// writed by Serj_LV
		var logPas = GM_getValue(CURRENT_SERVER+'login','0');
		if( logPas != '0' ) {
			logPas = logPas.split('/');
			...
			oLoginBtn.snapshotItem(0).addEventListener('click',function() {
				if ( oLogin.value.length > 0 && oPassword.value.length > 0 )
					GM_setValue(CURRENT_SERVER+'login',oLogin.value+'/'+oPassword.value);
			}, false);
		}
		...
		_log(0, "Initializacion failed, Auto-login failed. trxoyun Task Queue is not running");
	}
}
