...
// @version      5.0.7
// @homepageURL  https://userscripts.org/scripts/show/176859
// @downloadURL  https://userscripts.org/scripts/source/176859.user.js
// @updateURL    https://userscripts.org/scripts/source/176859.meta.js
// @grant        GM_addStyle
// @grant        GM_getValue
// @grant        GM_setValue
// @include      http://*
...
(function() {
    ...
    // postMessage 在一些 Firefox 的配置中会跟 ABP 冲突从而造成卡住
    // 测试结果：在用的配置卡住，另2个配置没有问题

    var browser = (function(){  // 浏览器检测
    ...
    function run() {
        ...
        init(browser, window, document, db);
    }

    var cplink;  // 翻上来的最近的页面的url;
    ...
    function init(browser, window, document, db) {
        ...
        //引用console对象的部分函数.
        var xbug = GM_getValue("debug") || false;
        ...
        function floatWindow() {
                    ...
                    <div id="sp-fw-main">\
                        ...
                        <div id="sp-fw-foot">\
                         <input type="checkbox" id="sp-fw-enable" title="总开关,启用js,否则禁用." name="sp-fw-enable"/>启用\
                          <span id="sp-fw-debug-button" class="sp-fw-spanbutton" title="' + xbug + '">debug</span>\
            ...
            // debug 按钮
            var debugButton = $('sp-fw-debug-button');
            debugButton.addEventListener('click', function(e){
                xbug = !xbug;
                GM_setValue("debug", xbug);
                debugButton.setAttribute("title", xbug);
            ...
            if (browser.opera) {
                ...
                gss();
                window.addEventListener('scroll', function gs() {
        ...
        function autopager(SSS, floatWO) {
            ...
            if (insertPoint) {
                ...
                if (xbug) C.error('验证是否能找到插入位置节点:失败', (SSS.a_HT_insert ? SSS.a_HT_insert[0] : ''), 'JS执行终止');
            ...
            if (pageElement.length > 0) {
                ...
                if (xbug) C.error('验证是否能找到主要元素:失败,', SSS.a_pageElement, 'JS执行终止');
                floatWO.updateColor('Astop');
                return;
            }
        ...
        if (xbug) C.log('搜索高级规则和自动匹配过程总耗时:', new Date() - startTime, '毫秒');
    ...
    function buildElement(type, atArr, inner, action, listen) {
    ...
    function createDocumentByString(str) {
        if (!str) {
            if (xbug) C.error('没有找到要转成DOM的字符串');
            return;
        }
