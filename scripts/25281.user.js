// ==UserScript==
// @name           Army Script CZ česky
// @author         Micruss
// @email          davidek.s@email.cz
// @namespace      Army
// @description    Ikariam Games Tools
// @include        http://*.ikariam.*/*
// @exclude        http://board.ikariam.*/
// ==/UserScript==

// ===========================================================================
// Include en "IkariamFriendList v.04 by Elnaira
// Include en "ScoreLinker 0.5.7" by ImmortalNights (c)
// Include en "Kronos Utils v.04 by Kronos" mods Johan Sundström
// ===========================================================================
...
'#flHeader { height: 26px; width: 150px; background-image: url(http://s10.ikariam.org/skin/layout/bg_sidebox_header.jpg); background-repeat: no-repeat; font-weight: bold; font-size: 13px; text-align: center; padding-top: 5px; cursor: pointer; }' + 
'#flHeader2 { height: 60px; width: 26px; background-image: url(http://s10.ikariam.org/skin/characters/60h/citizen_r.gif); background-repeat: no-repeat; background-position: right; font-weight: bold; font-size: 13px; text-align: right; cursor: pointer; }' + 
'#flContent { height: 395px; background-image: url(http://s10.ikariam.org/skin/layout/bg_stone.jpg); margin-top: -5px; padding: 7px; overflow: auto; display: none; font-family: Arial; font-size: 12px; }' + 
'#flFooter { background-image: url(http://s10.ikariam.org/skin/layout/bg_sidebox_footer.gif); height: 5px; display: none;  }' +
...
var langs = {
  "cz": ["Czech", " konec ", "Zavřít", "Udělat později.",
         "Seznam budov", "Přidat budovu.", "Hotova za",
         "hodin", "minut a", "sekund",
         "Nastavení", "Jazyk", "Zbývajicí čas",
         "No building in waiting.", "Dřevo", "Luxe",
         "Znovu hledat", "Ukázal", "Skrýtý", "Plný: ",
         "JanFebMarAprMayJunJulAugSepOctNovDec"],
  "fr": ["Français", " Fini a ", "Fermer", "Upgrader plus tard.",
         ...
         "JanFévMarAvrMaiJunJuiAouSepOctNovDéc"],
  "en": ["Czech", " konec ", "Zavřít", "Udělat později.",
         "Seznam budov", "Přidat budovu.", "Hotova za",
         "hodin", "minut a", "sekund",
         "Nastavení", "Jazyk", "Zbývajicí čas",
         "No building in waiting.", "Dřevo", "Luxe",
         "Znovu hledat", "Ukázal", "Skrýtý", "Plný: ",
  // By Tico:
  "pt": ["Portugues", " acaba as ", "Fechar", "Evoluir mais tarde.",
         "Lista de construçao", "Adicionar edificio.", "Construir em",
         "horas", "minutos e", "segundos",
         "confirmar", "Lingua usada", "Tempo de execuçao",
         "Nenhum Edificio em espera.", "Madeira", "Luxo"],
  "da": ["Dansk", " Fardig kl. ", "Luk", "Opgrader senere.",
         "Bygnings liste", "Tilfoj bygning.", "Byg kl.",
         "timer", "minutter og", "sekunder",
         "bekraft", "Sprog brugt", "Udforelsestid",
         "Ingen bygning venter.", "Tra", "Luxe"],
  // By A.Rosemary:
  "sp": ["Espagnol", " termina a las ", "Cerrar", "Actualizar más tarde.",
         "Lista de construcción", "Anadir edificio.", "Construir en",
...
var name = "Přehled surovin a armády";
var version = " 0.2";
/*-------------------------------------
...
function luxuryType(type) {
  ...
  switch (type) {
    ...
    case "crystal": return what.replace("crytal", "glass");
...
function title(detail) {
  ...
  if (server) {
    host = " "+ server[2];
    server = "?ß?????????µ????????????".charAt(parseInt(server[1], 10)-1);
  }
  if (!detail)
    detail = $X('id("breadcrumbs")/*[last()]').textContent;
  document.title = "Vítej" + " : " +(server ? server + " " : "") + ": " + host + " : " + detail ;
}
