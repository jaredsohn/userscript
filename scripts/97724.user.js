// ==UserScript==
// @name           Travian Super Map by Tatiana
// @namespace      Modified by Tatiana
// @description    Travian Super Map
// @include        http://*.travian.*/karte*
//
// history:        Originally imported from Big Map MultiLung for Travian by Stardust_gr (http://userscripts.org/scripts/show/93119)
...
switch (mlang) {
	...
	case "tw":
        ...
        case "my":titlestring = ["Peta Besar", "Kampung", "Pemain", "Populasi", "Persekutuan"];	
                break;
        case "ro":titlestring = ["Harta mare", "Sat", "Juc?tor", "Popula?ie", "Alian??"];
		break;
		...
		break;
        case "cz":titlestring = ["Velká mapa", "Vesnice", "Hrá?", "Populace", "Aliance"];
		break;	
		...
		break;
        case "lt":titlestring = ["Didelis žem?lapis", "Gyvenviet?s pavadinimas", "Žaid?jas", "Populiacija", "Aljansas"];
		break;
        case "rs":titlestring = ["?????? ????", "????", "?????", "??????????", "?????"];
		break;
        case "kr":titlestring = ["? ??", "??", "????", "??", "??"];
		break;
        case "lv":titlestring = ["Liel? karte", "Ciems", "Sp?l?t?js", "Popul?cija", "Alianse"];
		break;
	case "jp":titlestring = ["?????", "??", "?????", "??", "??"];
		break;
	case "il":titlestring = ["??? ?????", "???", "????", "?????????", "????"];
	...
	case "cn":titlestring = ["????", "???", "??", "???", "??"];
        ...
        case "eg":
                ...
                break;
case "ba":
titlestring = ["Velika Karta", "Naziv Sela", "Igra?", "Stanovnici", "Alijansa"];
break;
case "hr":
titlestring = ["Velika Mapa", "Selo", "Igra?", "Stanovnici", "Savez"];
break;
case "pl":
titlestring = ["Du?a Mapa", "Osada", "Gracz", "Ludno??", "Sojusz"];
break;
case "vn":
titlestring = ["B?n ?? l?n", "Làng", "Ng??i Ch?i", "Dân S?", "Liên Minh"];
break;
        case "gr":
                titlestring =["??????? ??????", "?????", "???????", "?????????", "????????"];
                break;
...
var Board = {
	...
	Link : function(given_size){
		...
		//link.innerHTML = ".....  (?????) ???? " + given_size + "X" + given_size;
	...
	Init : function(size){
		...
		//Modified by Tatiana
		lim_x = this.X - Math.floor((size - 1) * 3.5);
		lim_y = this.Y - Math.floor((size - 1) * 3.5);
		lim_px = -4;
		lim_py = -50 + size * 140;
		//End of Modification by Tatiana
