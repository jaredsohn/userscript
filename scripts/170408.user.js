// ==UserScript==
// @name           Travian QP Targets Pro + Last Update
...
Array.prototype.printMultiArrays = function () {
	debug(DBG_HIGHEST, "[printMultiArrays] this.length " + this.length);
	for(var i=0; i<this.length; i++) {
		debug(DBG_HIGHEST, "[printMultiArrays] this["+i+"] " + this[i]);
...
/**
* multiJoin
* Creates a string by joining multi level arrays and using multiple seperators.
*
* @param {Array} joinStrings Array with the strings to join this Array.
*/
Array.prototype.multiJoin = function (joinStrings) {
	if (joinStrings == null) { return this; }
	dbg("[multiJoin] joinStrings.length " + joinStrings.length + " joinStrings " + joinStrings);
	if (joinStrings.length == 0) { return this; }
	dbg("[multiJoin] this.length " + this.length + " this " + this);

	var strResult = "";
	var s1 = joinStrings.shift();
	// while there are multiple dimensions in the array, recurse into the dimensions until depth is only 1
	while (this[0] instanceof Array) {
		for(var i=0; i<this.length; i++) {
			this[i] = this[i].multiJoin(joinStrings);
			dbg("[multiJoin] this["+i+"] "+ this[i]);
		}
	}

	// join the last depth array with current separator string
	strResult += this.join(s1);
	dbg("[multiJoin] strResult " + strResult);

	joinStrings.unshift(s1);
	return strResult;
}
...
* @param {Array} joinStrings Array with the strings to join this Array.
*/
Array.prototype.multiJoinOLD = function (joinStrings) {
	if (joinStrings == null) { return this; }
	dbg("[multiJoin] joinStrings.length " + joinStrings.length + " joinStrings " + joinStrings);
	if (joinStrings.length == 0) { return this; }
	dbg("[multiJoin] this.length " + this.length + " this " + this);
	...
	var strResult = "";
	var s1 = joinStrings.shift();
	var arrThisLevelJoin = new Array();
	if (this[0] instanceof Array) {
		for(var i=0; i<this.length; i++) {
			strResult += this[i].multiJoin(joinStrings);
			dbg("[multiJoin] this["+i+"] "+ this[i] +"\n strResult " + strResult);
		}
	} else {
//		arrThisLevelJoin.push(this.join(s1));
		strResult += this.join(s1);
		dbg("[multiJoin] strResult " + strResult);
	}
...
//	var arrThisLevelJoin = new Array();
//	for(var i=0; i<this.length; i++) {
//		debug(DBG_HIGHEST, "[multiJoin] arrThisLevelJoin["+i+"] " + arrThisLevelJoin[i]);
/*		if (this[i] instanceof Array) {
			strResult += this[i].multiJoin(joinStrings);
//			this[i] = new Array();
		} else {
			strResult += this[i].join(s1);
		}
*/
/*		if (this[i] instanceof Array) {
			arrThisLevelJoin.push(this[i].multiJoin(joinStrings));
		} else {
			arrThisLevelJoin.push(this[i].join(s1));
		}
*/
//		arrResult.printMultiArrays();
//	}
//	arrThisLevelJoin = this.join(s1);
//	strResult += arrThisLevelJoin;
//	dbg("[multiJoin] strResult " + strResult);
...
//		debug(DBG_HIGHEST, "[transformPageGeneric_addBuildTime] resources " + resources);
		...
		if (quantityInput) {
			...
			function updateBuildTimeSpan(resourcesToBuild, parentTable, quantityInput, eventType, eventTriggerNode) {
				eventTriggerNode.addEventListener(eventType, function () {
//					debug(DBG_HIGHEST, "[transformPageGeneric_addBuildTime] update TTB ");
					createBuildTimeSpan(resourcesToBuild, parentTable, quantityInput);
//					debug(DBG_HIGHEST, "[transformPageGeneric_addBuildTime] update TTB end ");
				}, true);
			}
			updateBuildTimeSpan(resources, currentClockCell.parentNode.parentNode, quantityInput, "keyup", quantityInput);

			var maxNumberLink = xpEvalContextFirst(quantityInput, '../../td/div[@class="f75"]/a');
			updateBuildTimeSpan(resources, currentClockCell.parentNode.parentNode, quantityInput, "click", maxNumberLink);
		}
...
//			debug(DBG_HIGHEST, "[createBuildTimeSpan] FOR i " + i);
	...
	var granaryNeeded = 0;
	var totalResourcesProduction = parseInt(0);
	...
	for(var j=0; j<g_res_now.length; j++) {
		...
		totalResourcesProduction += parseInt(g_res_prod[j]);
	...
	// Warehouse and Granary available; resources needed

		debug(DBG_HIGHEST, "[createBuildTimeSpan] Warehouse and Granary available; resources needed \n"
			+ "\n needsMoreResources " + needsMoreResources
			+ "\n totalResourcesActual " + totalResourcesActual
			+ "\n totalResourcesNeeded " + totalResourcesNeeded
			+ "\n neededResources " + neededResources
			+ "\n maxTimeToReady " + maxTimeToReady
			+ "\n g_res_prod[3] " + g_res_prod[3]
			+ "\n totalResourcesProduction " + totalResourcesProduction
		);


	var timeToBuild = "---";
	var timeToBuildStyle = "";

	// Needed crop and negative crop production -> infinite time
	if ((neededResources[3] >= 0) && (g_res_prod[3] < 0)) {
		timeToBuild = DEF_CHAR_INFINITY;
		timeToBuildStyle = "style=font-weight:bold;color:" + DEF_COLOR_RESOURCE_TIMERS_INFINITY;
	} else {
		timeToBuild = timeInSecondsToColonSeparatedTxt(maxTimeToReady*3600);
	...
	var res = '<span class="c" style="white-space:nowrap;">'
			...
			+ '<span id="QPtimer" ' + timeToBuildStyle + '>' + timeToBuild + '</span>'
			+ ' '
			+ '</span> ';
	...
	if (totalResourceDiff > 0) {
		...
		var timeToBuildWithGold = "---";
		var timeToBuildStyleWithGold = "";

		// Needed total resources and negative total production -> infinite time
		if ((neededResources[3] >= 0) && (totalResourcesProduction < 0)) {
			timeToBuildWithGold = DEF_CHAR_INFINITY;
			timeToBuildStyleWithGold = "style=font-weight:bold;color:" + DEF_COLOR_RESOURCE_TIMERS_INFINITY;
		} else {
			timeToBuildWithGold = timeInSecondsToColonSeparatedTxt(timeInSeconds);
		...
		goldSpan.innerHTML = ''
				...
				+ '<img class="res" src="img/un/a/clock.gif" />'
				+ '<span id="QPtimer" '+timeToBuildStyleWithGold+'>'
				+ timeToBuildWithGold
				+ '</span>';

		debug(DBG_NORMAL, "[createBuildTimeSpan] timeInSeconds*1000 " + timeInSeconds*1000);
	}
}
...
function transformPageTreasury_addCoordsForConstructionPlanHolderVillages() {
	var constructionPlanHolderVillagesLinks = xpathEvaluate('//div[@id="lmid2"]/table[@class="tbg"]/tbody/tr/td/a[contains(@href, "allianz.php?aid=")]/../../td/a[contains(@href, "karte.php?d=")]');
...
* retrievePageHeroMansion_heroTroopType
*/
function retrievePageHeroMansion_heroTroopType() {
	var heroTroopType = xpEvalFirst('//div[@id="lmid2"]/table[@class="tbg"]/tbody/tr[@class="rbg"]/td/span[@class="t"]').textContent;
	heroTroopType = heroTroopType.replace(/\(|\)/g, "");
	gmSave_HeroType(heroTroopType);
}



/** save, reset, load, createKey, exists - HeroType - <server>_heroTroopType */
function gmSave_HeroType(heroTroopType) { gmSave_Escape(gmKey_HeroType(), heroTroopType); }
function gmReset_HeroType() { gmSave_HeroType(""); }
function gmLoad_HeroType() { return gmLoad_Unescape_UndefinedIsEmptyString(gmKey_HeroType()); }
function gmKey_HeroType() { return DEF_PKEY_SU_PREFIX + DEF_PKEY_SU_HEROTROOPTYPE; }
function gmExists_HeroType() { return (gmLoad_Unescape_UndefinedIsEmptyString(gmKey_HeroType()) != ""); }
...
function transformPageHeroMansion_addHeroLevelInfo() {
	...
	levelTxt = levelTxt.trim();
	levelTxt = levelTxt.substr(0, levelTxt.indexOf(" "));
...
function transformGeneric_addOverflowDepleteTimes() {
	...
	for(var i=0; i<4; i++) {
		...
		var resColor = ( (g_res_prod[i] <= 0) ? ( (g_res_prod[i] < 0) ? ';color:red' : ';color:'+DEF_COLOR_RESOURCE_TIMERS_INFINITY+';font-size:larger;' ) : '');
...
function retrieveMyProfile_Capital() {
	var capitalCoordsCell = xpathEvaluate('//div[@id="lmid2"]/table/tbody/tr/td[1]/span/../../td[3]')
...
* retrieveAndSavePageResidenceOrPalaceCulturePoints_serverType
*/
function retrieveAndSavePageResidenceOrPalaceCulturePoints_serverType() {

	var neededCPsForNextVillage = parseInt(xpEvalFirst('//div[@id="lmid2"]/p/b[2]').innerHTML);

	var trav2 = DEF_CP_TRAVIAN_2.indexOf(neededCPsForNextVillage);
	var trav3 = DEF_CP_TRAVIAN_3.indexOf(neededCPsForNextVillage);
	var travSpeed = DEF_CP_TRAVIAN_SPEED.indexOf(neededCPsForNextVillage);

	var serverType = DEF_TRAVIAN_SERVER_TYPE_UNKNOWN;

	if (travSpeed > -1) {
		serverType = DEF_TRAVIAN_SERVER_TYPE_SPEED;
	} else {
		if ((trav2 > -1) && (trav3 > -1)) {
			serverType = DEF_TRAVIAN_SERVER_TYPE_NORMAL_OR_CLASSIC;
		} else {
			if (trav2 > -1) {
				serverType = DEF_TRAVIAN_SERVER_TYPE_2;
			} else {
				serverType = DEF_TRAVIAN_SERVER_TYPE_3;
			}
		}
	}
	debug(DBG_HIGHEST, "[retrieveAndSavePageResidenceOrPalaceCulturePoints_serverType] "
			+ "\n trav2 " + trav2
			+ "\n trav3 " + trav3
			+ "\n travSpeed " + travSpeed
	...
	gmSave_ServerType(serverType);
}


/** save, reset, load, createKey, exists - serverType - <server>_serverType */
function gmSave_ServerType(serverType) { gmSave(gmKey_ServerType(), ""+serverType); }
function gmReset_ServerType() { gmSave_ServerType(""); }
function gmLoad_ServerType() { return gmLoadNumber(gmKey_ServerType()); }
function gmKey_ServerType() { return DEF_PKEY_S_PREFIX + DEF_PKEY_S_SERVERTYPE; }
function gmExists_ServerType() { return (gmLoad_ServerType != -1); }

function gmSave(key, value) { GM_setValue(key, value); }
function gmLoad(key) { return GM_getValue(key, ""); }

function gmLoadNumber(key) { var r = gmLoad(key); r = (r=="") ? -1 : r; return parseInt(r); }
...
function createElemTravianCPTable(tableParent, villagesText, cpText, currentTotalCPs, currentTotalCPsPerDay) {
	...
	var cpArray;
	var travServerType = gmLoad_ServerType();
	debug(DBG_HIGHEST, "[createElemTravianCPTable] travServerType " + travServerType);
	switch (travServerType) {
		...
		case DEF_TRAVIAN_SERVER_TYPE_NORMAL_OR_CLASSIC:
							if (CONFIG_TRAVIAN_SERVER_TYPE_NORMAL_SPEED_DEFAULT_IS_NORMAL) {
								cpArray = DEF_CP_TRAVIAN_3;
							} else {
								cpArray = DEF_CP_TRAVIAN_2;
							}
							break;
		case DEF_TRAVIAN_SERVER_TYPE_UNKNOWN:
		default: debug(DBG_HIGHEST, "[createElemTravianCPTable] ERROR: wrong travServerType " + travServerType); break;
	}
...
/**
* transformPageSendTroopsBack_addRealReturnDuration
* Shows the real duration of sending troops back.
* When more than 1 type of troop is available to return home, the show time in the original page is of the
* slowest unit, if you don't want to send that type of troop back, then that time is wrong.
*/
function transformPageSendTroopsBack_addRealReturnDuration() {
	debug(DBG_HIGHEST, "[transformPageSendTroopsBack_addRealReturnDuration] ");
	var returningTroops = xpEval('//div[@id="lmid2"]/form/table[@class="tbg"]/tbody/tr/td/input[@class="fm"]');
	// only if 2 or more types of troops are available for returning is this needed
	if (returningTroops.snapshotLength > 1) {


		debug(DBG_NORMAL, "[transformPageSendTroopsBack_addRealReturnDuration] 2 or more");

		var arrivalLang = xpEvalFirst('//div[@id="lmid2"]/form/table[@class="tbg"]/tbody/tr[last()]/td[1]').textContent;
		var tbody = createElemTravianTable(arrivalLang, xpEvalFirst('//div[@id="lmid2"]'), true);
		var tr = createElementAppend("tr", tbody);
		var tdUnit = createElemAppendAndSetInner("td", tr, "-");
		var tdDuration = createElemAppendAndSetInner("td", tr, "-");
		var tdArrival = createElemAppendAndSetInner("td", tr, "-");

		var troopsRow = xpEvalFirst('//div[@id="lmid2"]/form/table[@class="tbg"]/tbody/tr[2]');
		var tribe = getTribeFromDocumentAndTroopPosition(troopsRow);

		var durationTime = xpEvalFirst('//div[@id="lmid2"]/form/table[@class="tbg"]/tbody/tr/td/table/tbody/tr/td[1]').textContent;
		var tdArrivalTime = xpEvalFirst('//div[@id="lmid2"]/form/table[@class="tbg"]/tbody/tr/td/table/tbody/tr/td/span/..');

		var tdDurationPrefix = /(.*\s)\d+:\d+:\d+/.exec(durationTime);
		tdDurationPrefix = (tdDurationPrefix.length > 0) ? tdDurationPrefix[1] : "";
		var tdDurationSuffix = /\d+:\d+:\d+(\s.*)/.exec(durationTime);
		tdDurationSuffix = (tdDurationSuffix.length > 0) ? tdDurationSuffix[1] : "";
		var slowestDurationInSeconds = /\d+:\d+:\d+/.exec(durationTime)[0];
		slowestDurationInSeconds = timeColonSeparatedToValue(slowestDurationInSeconds);
		var spanArrival = xpEvalContextFirst(tdArrivalTime, 'span[@id="tp2"]');
		var actualTime = timeColonSeparatedToValue(spanArrival.textContent) - slowestDurationInSeconds;
		actualTime = (actualTime < 0) ? (actualTime + 86400) : actualTime;

		/** Inherited vars: troopsRow */
		function getHeroTroopIndex() {
			if (gmExists_HeroType()) {
				var heroTroopType = gmLoad_HeroType();
				var troopImages = xpEvalContext(troopsRow, 'td/img');
				for(var i=0; i<troopImages.snapshotLength; i++) {
					var currImageTitle = troopImages.snapshotItem(i).title;
					if (currImageTitle == heroTroopType) {
		dbg("[getHeroSpeed] "
				+ "\n i " + i
				);
						return i;
		...
		/** Inherited vars: tribe, isHero */
		function getSlowestAvailableUnitSpeed(returningTroops) {
			// get slowest unit speed
			var slowestAvailableUnitPos = 999999;
			for(var i=0, len=returningTroops.snapshotLength; i<len; i++) {
				var currentReturningTroops = returningTroops.snapshotItem(i);
				if ((currentReturningTroops.value != "") && (currentReturningTroops.value > 0)) {
					var unitPos = currentReturningTroops.name;
					unitPos = parseInt(unitPos.substring(2, unitPos.length-1)) - 1;
					var isEvaluatingHero = false;
					if (unitPos == 10) {
						unitPos = getHeroTroopIndex();
						isEvaluatingHero = true;
					}
					var currUnitSpeed = DEF_TROOPS[tribe][unitPos][DEF_TROOPS_STATS_INDEX_SPEED];

					if ((slowestAvailableUnitPos == 999999) ||
						(DEF_TROOPS[tribe][slowestAvailableUnitPos][DEF_TROOPS_STATS_INDEX_SPEED] > currUnitSpeed)) {
						slowestAvailableUnitPos = unitPos;
						isHero = isEvaluatingHero;
					}
				}
			}
			return (slowestAvailableUnitPos == 999999) ? -1 : slowestAvailableUnitPos;
		}

		var slowestUnitForAllAvailableBeforeChanges = getSlowestAvailableUnitSpeed(returningTroops);
		debug(DBG_NORMAL, "[transformPageSendTroopsBack_addRealReturnDuration] "
				+ "\n slowestUnitForAllAvailableBeforeChanges " + slowestUnitForAllAvailableBeforeChanges
				+ "\n returningTroops.snapshotLength " + returningTroops.snapshotLength
				);

		for(var i=0, len=returningTroops.snapshotLength; i<len; i++) {
			var currentReturningTroops = returningTroops.snapshotItem(i);

			currentReturningTroops.addEventListener("keyup", function() {
				// get slowest unit speed
				var slowestAvailableUnitSpeed = getSlowestAvailableUnitSpeed(returningTroops);
				debug(DBG_NORMAL, "[transformPageSendTroopsBack_addRealReturnDuration] slowestAvailableUnitSpeed " + slowestAvailableUnitSpeed);

				if (slowestAvailableUnitSpeed >= 0) {
					// speed ratio of the slowest to the current changed
					var slowestSpeedRatio = DEF_TROOPS[tribe][slowestAvailableUnitSpeed][DEF_TROOPS_STATS_INDEX_SPEED] /
											DEF_TROOPS[tribe][slowestUnitForAllAvailableBeforeChanges][DEF_TROOPS_STATS_INDEX_SPEED];

					var slowestAvailableUnitImageIndex = isHero ? 11 : slowestAvailableUnitSpeed+1;
				debug(DBG_HIGHEST, "[transformPageSendTroopsBack_addRealReturnDuration] "
						+ "\n slowestAvailableUnitSpeed " + slowestAvailableUnitSpeed
						+ "\n isHero " + isHero
						+ "\n slowestAvailableUnitImageIndex " + slowestAvailableUnitImageIndex
						);
					tdUnit.innerHTML = troopsRow.cells[slowestAvailableUnitImageIndex].innerHTML;
					var newDurationInSeconds = (slowestDurationInSeconds / slowestSpeedRatio);
					var newDuration = timeInSecondsToColonSeparatedTxt(newDurationInSeconds);
					tdDuration.innerHTML = tdDurationPrefix + newDuration + tdDurationSuffix;

					var newArrivalTime = actualTime + newDurationInSeconds;
					newArrivalTime = (newArrivalTime >= 86400) ? (newArrivalTime - 86400) : newArrivalTime;

					// resetting the real arrival cell
					tdArrival.innerHTML = "";
					for(var i=0; i<tdArrivalTime.childNodes.length; i++) {
						tdArrival.appendChild(tdArrivalTime.childNodes[i].cloneNode(true));
					}

					var spanArrivalClone = xpEvalContextFirst(tdArrival, 'span[@id="tp2"]');
					spanArrivalClone.innerHTML = timeInSecondsToColonSeparatedTxt(newArrivalTime);
		...
		if (slowestUnitForAllAvailableBeforeChanges >= 0) {
			var slowestAvailableUnitImageIndex = isHero ? 11 : slowestUnitForAllAvailableBeforeChanges+1;
			debug(DBG_HIGHEST, "[transformPageSendTroopsBack_addRealReturnDuration] "
				+ "\n slowestUnitForAllAvailableBeforeChanges " + slowestUnitForAllAvailableBeforeChanges
				+ "\n isHero " + isHero
				+ "\n slowestAvailableUnitImageIndex " + slowestAvailableUnitImageIndex
			...
			tdUnit.innerHTML = troopsRow.cells[slowestAvailableUnitImageIndex].innerHTML;
			tdDuration.innerHTML = durationTime;
			tdArrival.innerHTML = tdArrivalTime.innerHTML;
		}
	}
}
...
function transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops() {
	...
	var activeVillageCoordZ = getActiveVillageCoordZ();
	// incoming and outgoing troops - THE 2 TOP CELLS WITH VILLAGES LINKS - THE SOURCE AND DESTINATION
	for(var i=0; i<incomingTroops.snapshotLength; i++) {
		...
		// TABLES
		if (incomingTables) {
			debug(DBG_NORMAL, "[transformPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] "
						+ " incomingTables " + incomingTables
						+ " currentNode.innerHTML " + currentNode.innerHTML);
			transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops(currentNode, activeVillageCoordZ);
		}
	}
}

/**
* transformPageRallyPoint_incomingTroopTable_addSlowestIncomingUnitOnIncomingTroops
* @param {HtmlNode} tableNode
*/
function transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops(tableNode, activeVillageCoordZ) {
	debug(DBG_NORMAL, "[transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] ");

	var titleRow = xpEvalContextFirst(tableNode, 'tbody/tr[1]');

	var troopImagesRow = xpEvalContextFirst(tableNode, 'tbody/tr/td/img/../..');
	var tribe = getTribeFromDocumentAndTroopPosition(troopImagesRow);
	var timeToArrive = xpEvalContextFirst(tableNode, 'tbody/tr/td/table/tbody/tr/td/span[contains(@id, "timer")]');
	var arrivalTime = xpEvalContextFirst(tableNode, 'tbody/tr/td/table/tbody/tr/td/span[not(@id)]/..');
	var timeToArriveInSeconds = timeColonSeparatedToValue(timeToArrive.innerHTML);
	var incomingCoordZ = getParamFromUrl(xpEvalContextFirst(titleRow, 'td[1]/a[contains(@href, "karte.php?d=")]').href, "d");
	var distance = coordDistZtoZ(incomingCoordZ, activeVillageCoordZ);

	debug(DBG_HIGHEST, "[transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] "
				+ "\n incomingCoordZ " + incomingCoordZ
				+ "\n activeVillageCoordZ " + activeVillageCoordZ
				+ "\n distance " + distance
				);

	if (distance == 0) {
		// troops returning from a reinf / attack / raid - no need to show anything
		debug(DBG_HIGHEST, "[transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] returning troops - nothing to show ");
		return;
	}
	if (!isNaN(xpEvalContextFirst(tableNode, 'tbody/tr[3]/td[2]').textContent)) {
		debug(DBG_HIGHEST, "[transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] troop numbers available - nothing to show ");
		return;
	}

	var travServerType = gmLoad_ServerType();
	var isSpeedServer;
	if (travServerType == DEF_TRAVIAN_SERVER_TYPE_UNKNOWN) {
		isSpeedServer = (CONFIG_TRAVIAN_SERVER_TYPE_SPEED_DEFAULT_IS_SPEED) ? true : false;
	} else {
		isSpeedServer = (travServerType==DEF_TRAVIAN_SERVER_TYPE_SPEED) ? true : false;
	}


	var unitIndex = getSlowestUnit(timeToArriveInSeconds, distance, tribe, isSpeedServer, 0)
	unitIndex += 2;	// 1 to shift for 0..n to 1..n; another 1 for the 1st cell being empty
	debug(DBG_HIGHEST, "[transformTableIncomingTroopsPageRallyPoint_addSlowestIncomingUnitOnIncomingTroops] "
				+ "\n tribe " + tribe
	...
	// change the span of the title
	titleRow.cells[1].colSpan = parseInt(titleRow.cells[1].colSpan) - 2;

	// add the lowest unit cell - image and drop down list
	var slowestIncomingUnitCell = createElementAppend('td', titleRow);
	var slowestTroopImage = xpEvalContextFirst(troopImagesRow, 'td['+unitIndex+']/img').cloneNode(true);
	slowestIncomingUnitCell.colSpan = "2";
	slowestIncomingUnitCell.appendChild(slowestTroopImage);
	if (distance > DEF_TOURNAMENT_SQUARE_EFFECT_DISTANCE) {
		var tournSquareLevelSelect = createElemAppendAndSetInner('select', slowestIncomingUnitCell, dropDownListCreateNumericOptions(0, 21));

		function updateSlowestUnit(eventType, tournSquareLevelSelect, troopImagesRow, slowestIncomingUnitCell, timeToArriveInSeconds, distance, tribe) {
			tournSquareLevelSelect.addEventListener(eventType,	function() {
				debug(DBG_HIGHEST, "[updateSlowestUnit] selecting");
				var unitIndex = getSlowestUnit(timeToArriveInSeconds, distance, tribe, isSpeedServer, tournSquareLevelSelect.selectedIndex);
				unitIndex += 2;	// 1 to shift for 0..n to 1..n; another 1 for the 1st cell being empty
				debug(DBG_HIGHEST, "[updateSlowestUnit] "
//						+ "\n tableNode " + tableNode.innerHTML
						+ "\n timeToArriveInSeconds " + timeToArriveInSeconds
						+ "\n distance " + distance
						+ "\n tribe " + tribe
						+ "\n unitIndex " + unitIndex
						+ "\n tournSquareLevelSelect.selectedIndex " + tournSquareLevelSelect.selectedIndex
						);
				var slowestTroopImage = xpEvalContextFirst(troopImagesRow, 'td['+unitIndex+']/img').cloneNode(true);
				debug(DBG_HIGHEST, "[updateSlowestUnit] "
						+ "\n unitIndex " + unitIndex
						+ "\n slowestTroopImage.src " + slowestTroopImage.src
						);
				slowestIncomingUnitCell.removeChild(xpEvalContextFirst(slowestIncomingUnitCell, 'img'));
				slowestIncomingUnitCell.insertBefore(slowestTroopImage, tournSquareLevelSelect);
				debug(DBG_HIGHEST, "[updateSlowestUnit] /selecting");
			}, true);
		}
		updateSlowestUnit("change", tournSquareLevelSelect, troopImagesRow, slowestIncomingUnitCell, timeToArriveInSeconds, distance, tribe);
		updateSlowestUnit("keyup", tournSquareLevelSelect, troopImagesRow, slowestIncomingUnitCell, timeToArriveInSeconds, distance, tribe);
	}
...
function getSlowestUnit(timeInSeconds, distance, tribe, isSpeedServer, tournamentSquareLevel) {
	...
	var closeDistance = (distance < DEF_TOURNAMENT_SQUARE_EFFECT_DISTANCE) ? distance : DEF_TOURNAMENT_SQUARE_EFFECT_DISTANCE;
	var longDistance = (distance < DEF_TOURNAMENT_SQUARE_EFFECT_DISTANCE) ? 0 : (distance - DEF_TOURNAMENT_SQUARE_EFFECT_DISTANCE);
	var times = new Array();
...
//dbg("[transformGeneric_replaceAllCoordsByLink] txtNode.innerHTML " + txtNode.innerHTML);
	var res = txtNode.innerHTML;
	if (CONFIG_MSGCOORDS_EXTENDEDSEPARATORS) {
		// JS has no lookbehind => use look ahead and match if after there is no: </a> OR "> (which is the end of the href and start A tag
		res = res.replace(/-?\d+[\|\/,\\;]-?\d+(?!(\w+<\/a>)|(\w+">))/g, replaceCoordsByLink);
	} else {
		res = res.replace(/-?\d+\|-?\d+(?!(\w+<\/a>)|(\w+">))/g, replaceCoordsByLink);
	}
	txtNode.innerHTML = res;
	...
	if (isThisPageAnyBuildingPage()) {
		...
		if (textBelowBuildingNameAndLevel) {
			if (textBelowBuildingNameAndLevel.textContent.indexOf("10") >= 0) {
				// at least the HM also has the number 10 in the text below the title/name of the building
				if (xpEvalFirst('//div[@id="lmid2"]/table[@class="tbg"]/tbody/tr/td[@class="rbg"][@colspan="5"]/a[@name="h2"]')) {
					return true;
				}
			}
		}
...
/** Is the page of Hero's Mansion with an active Hero */
// ???????? - only when there is a hero created;
// MISSING: no troops available; create/ressuscitate; creating/ressuscitating
function isThisPageHeroMansionWithActiveHeroPage() {
	if (isThisPageAnyBuildingPage()) {
...
/** Is the page of Oasis at the Hero's Mansion */
function isThisPageHeroMansionOasisPage() {
	if (isThisPageAnyBuildingPage()) {
		return (document.location.pathname.indexOf("&land") >= 0);
	}
	return false;
}
...
/** Is any page of the market place with a completed action (url has no params: <server>/build.php ) */
function isThisPageAnyMarketPageWithCompletedAction() {
	if (isThisPageAnyMarketPage()) {
dbg("[isThisPageAnyMarketPageWithCompletedAction] document.location.href <" + document.location.pathname + ">");
dbg("[isThisPageAnyMarketPageWithCompletedAction] /build.php                 <" + "/build.php" + ">");
		return (document.location.href.endsWith("/build.php"));
	}
	return false;
}
...
String.prototype.equals = function (str) {
	if ((this == null) && (str == null)) { return true; }	// both are null		=> they are equal
	if ((this == null) || (str == null)) { return false; }	// only one is null		=> different
	if (this.length != str.length) { return false; }		// different lengths	=> different

	// here the lengths are already guaranteed to be the same
	return (this.indexOf(str) == 0);	// if same one contains other				=> equal
}
...
String.prototype.endsWith = function (str) {
	...
	if (pos >= 0) {
		var posLastCharOfStr = pos + str.length;
		return (this.length == posLastCharOfStr);
	}
	return false;
}
...
//html > body > div #lmidall > div #lmidlc > div #lmid1 > div #lmid2 > form > p .b c3
//html > body > div #lmidall > div #lmidlc > div #lmid1 > div #lmid2 > form > p .b c5
...
/** Is the page of the market place to offer resources */
function isThisPageMarketOffer() {
	if (isThisPageAnyMarketPage()) {
		return (getParamFromUrl(document.location.href, "t") == 2);
	}
	return false;
}


/** Is the page of the market place to makes offers after making an offer which failed */
function isThisPageMarketOfferFailed() {
	if (isThisPageAnyMarketPageWithCompletedAction()) {
		var selectNodes = xpEval('//div[@id="lmid2"]/form/table[@class="f10"]/tbody/tr/td/select[@class="fm"]');
		if (selectNodes.snapshotLength == 2) {
			return (xpEvalFirst('//div[@id="lmid2"]/form/p[@class="e"]'));
		}
	}
	return false;
}
...
/** Is the page of the market place to makes offers after making an offer */
function isThisPageMarketOfferDone() {
	if (isThisPageAnyMarketPageWithCompletedAction()) {
		var selectNodes = xpEval('//div[@id="lmid2"]/form/table[@class="f10"]/tbody/tr/td/select[@class="fm"]');
		return (selectNodes.snapshotLength == 2);
	}
	return false;
}
...
/** Is the page any send troops page (send without target, send with target, confirm send) */
function isThisPageSendTroopsBackPage() { return (isPageSendTroopsBackPage(document.location.href)); }
function isPageSendTroopsBackPage(url) {
	if (isPageAnySendTroopsPage(url)) {
		if ((getParamFromUrl(url, "d")) && (getParamFromUrl(url, "c"))) {
			return true;
		}
	}
	return false;
...
function randomDelay(min, max, callMethod) {
	...
	debug(DBG_LOWEST, "[randomDelay] delaying: " + rnd);
...
var CONFIG_FAKE_ATTACK = __CONFIG_FAKE_ATTACK_LEGG_PHALX_CLUB;
...
// ##########   InGame Message Coordinates normal separator  |   extended separators  \ / , ;   ##########
var CONFIG_MSGCOORDS_EXTENDEDSEPARATORS = true;

// ##########   Server Type Default between normal speed types: true = Normal; false = Classic   ##########
// Only used until the server type is learnt: normal and classic is only distinguished after having 3 towns
var CONFIG_TRAVIAN_SERVER_TYPE_NORMAL_SPEED_DEFAULT_IS_NORMAL = true;

// ##########   Server Type Default speed types: true = Speed; false = NotSpeed   ##########
// Only used until the server type is learnt
var CONFIG_TRAVIAN_SERVER_TYPE_SPEED_DEFAULT_IS_SPEED = true;
