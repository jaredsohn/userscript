...
var currURL		= location.href;			// http://www.erepublik.com/en/citizen/profile/5878406
...
function bhStats(att, def) {
	...
	$j('div#myStatBoxL').html(
		...
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>Giocatore</th><th>Uccisioni</th><th>Danni</th></tr>' + att + '</table></div>' +
		'</div>'
		);
	$j('#BHTableR').remove();
	$j('div#myStatBoxR').html(
		'<div id="BHTableR" style="margin-top:80px;">' +
		'<div class="BHTable" style="float:right"><table class="BHTable"><tr><th>Giocatore</th><th>Uccisioni</th><th>Danni</th></tr>' + def + '</table></div>' +
		'</div>'
	);
...
function histStats(hist) {
	...
	$j('div#myOverBox').html(
		'<div id="OOTable">' +
		'<div class="BHTable" style="float:left"><table class="BHTable"><tr><th>Giocatore</th><th>Uccisioni</th><th>Danni</th></tr>' + hist + '</table></div>' +
		'</div>'
	);
...
function getRegionInfo(id)
{
	...
	$j('button#hospitals_get').html('Attendere...').attr('disabled', 'true');
	...
	GM_xmlhttpRequest({
		...
		onreadystatechange:	function(response) {
			...
			$j('button#hospitals_get').html('Attendere...');

		},
		onerror: function(response) {

			$j('button#hospitals_get').html('Errore!');
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'</small></div></div>' +
			'<h3>Calcolatore danni' +
			'<span style="float:right">Armi: <select id="dmgWeapon" size="1">' +
			...
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;numero di colpi: <input id="dmgFights" name="dmgFights" value="0" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">Calcola!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>0 Colpi: </strong>' +
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 120, 0, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 120, 0, 1.1) + '</span></strong></small></div></div></div>'
		);
		...
		$j('button#dmgCalc').click(function() {
			...
			var fights  = $j('input#dmgFights').val();
				...
				$j('div#dmgResults').html(
					'<strong>' + fights + ' Colpi: </strong>' +
					'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, $j('select#dmgWeapon').val(), fights, 1) + '</h4>' +
					'<div class="stat"><small>Con NE: <strong><span style="font-size:12px;margin-right:15px">' +
	...
	var content = $j('div#pvp_battle_area');
         ...
         */
        var current_damage = $j('#total_damage strong').text();
        var strength = $j('#fighter_skill').text().replace(",","");
        var rankString = $j('#rank_icon').attr('title').split(':')[1].trim();
        var militaryRank = rank[rankString];
        
         var max_hitQ6 = dmgCalc(militaryRank, strength, 120, 1, 1);
        ...
        '<tr><td><div>'+
        '<div><b >&nbsp;</b><small>Q6 hit / done:</small>'+
        '<p id="pCurrentHitQ6" class="pCurrentHit">'+ max_hitQ6 +' / ' +  Math.round(current_damage/max_hitQ6) +'</p></div></td></tr>');
	...
	content.prepend(
		'<div id="myStatBoxL" style="position: absolute; top: 380px; left: 18px; width: 200px; opacity: 0.8;"></div>' +
	...
	$j('div#enemy_defeated').before(
		...
		'<input id="multihit_count" type="text" size="3" maxlength="3" value="1" /><button id="multihit_start">Spara!</button>' +
		'&nbsp;&nbsp;<input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food">&nbsp;Usa il cibo se necessario</label>' +
		'&nbsp;&nbsp;<input type="checkbox" id="multihit_bazooka" name="multihit_bazooka" checked="checked"><label for="multihit_bazooka">&nbsp;Non usare bazooka</label><br />' +
		'<input type="checkbox" id="multihit_energy" name="multihit_energy"><label for="multihit_energy">&nbsp;Usa le barrette se finisce il cibo</label>' +
		'&nbsp;&nbsp;<input type="checkbox" id="multihit_hk" name="multihit_hk"><label for="multihit_hk">&nbsp;Usa HK se finisce il cibo</label><br /><br />' +
		'<button id="heros_update">Aggiorna</button>&nbsp;&nbsp;La lista si aggiorna automaticamente, oppure puoi aggiornarla tu.' +
...
//		'<button id="hospitals_get">get hospitals</button></div>' +
		'<fieldset id="BLOG"><legend>v1.3.7 Azioni:&nbsp;&nbsp;<button id="battlelog_clear">Cancella</button></legend><div id="battlelog"></div></fieldset>'
	);
...
//	}, 250);
	...
	unsafeWindow.battleFX.hit = function() {
		...
		if (multiHitRunning) {
			...
			bLog('Colpito!');
			$j('div#multihit_message').html('Hits: ' + multiHitDone + '&nbsp;&nbsp;Kills: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? 'Ultimo nemico ucciso!' : 'Ultimo nemico vivo!') + ')');
	...
	unsafeWindow.jQuery.fn.getWell = function() {
		...
		if ($j('#heal_btn small').html() != '0' && !h.hasClass('disabled') && unsafeWindow.SERVER_DATA.onlySpectator == 0) {
			...
			bLog('Recupero la vita con il cibo...');
			$j('#heal_btn small').hide();
			h.removeClass('hospital_btn');
			h.attr('title', 'Uso il Cibo');
	  ...
	  bLog('Cambio arma');
		...
		$j.post(url, {_token: unsafeWindow.SERVER_DATA.csrfToken, battleId: unsafeWindow.SERVER_DATA.battleId}, function (response) {
			...
			if ($j(".listing span img").eq(-1).attr("src").indexOf("q10") !== -1) {
				...
				bLog('Non posso recuperare la vita. Stop.');
				multiHitRunning = false;
				$j('button#multihit_start').html('Spara!');
				return;
	...
	$j('button#multihit_start').click(function() {
		...
		if (multiHitRunning) {
			...
			if (multiHitCount > 0) {
				...
				$j('button#multihit_start').html('<strong>Fermo!</strong>');
				bLog('AutoFighters-ITA Versione 1.3.7 By MC46 donate per sostenere lo sviluppo dello script: http://www.erepublik.com/en/citizen/profile/5878406');
                                bLog('Comincio...');
	...
	$j('button#hospitals_get').click(function() {
		...
		if (unsafeWindow.SERVER_DATA.onlySpectator != 1 && unsafeWindow.SERVER_DATA.isResistance != 1 && id != unsafeWindow.SERVER_DATA.countryId) {
			...
			$j('button#hospitals_get').html('Gli attaccanti non posso usare gli ospedali!').attr('disabled', 'true');
			return;
		...
		if (opt.url.indexOf('/battle-stats/') > -1 && unsafeWindow.SERVER_DATA.onlySpectator != 1) {
			...
			for ( var i = 0; i < history.length; i++ ) {
				...
				top5HIST = top5HIST+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[history[i].citizen_id].id+'">'+bh.fightersData[history[i].citizen_id].name+'</a></td><td>'+history[i].kills+'</td><td><strong>'+history[i].damage+'</strong></td></tr>';
                        ...
                        top5HIST = top5HIST+'<tr style="color:yellow;"><td>Danni totali Top5:</td><td>'+top5CHLeftSideKills+'</td><td>' + top5CHLeftSide + '</td></tr>';
		  histStats(top5HIST);
			...
			if (typeof bh['stats']['current'][zone][division][att] != 'undefined') {
				...
				for ( var i = 0; i < bh['stats']['current'][zone][division][att].length; i++ ) {
					...
					top5ABH = top5ABH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['stats']['current'][zone][division][att][i].citizen_id].id+'">'+bh.fightersData[bh['stats']['current'][zone][division][att][i].citizen_id].name+'</a></td><td>'+bh['stats']['current'][zone][division][att][i].kills+'</td><td><strong>'+bh['stats']['current'][zone][division][att][i].damage+'</strong></td></tr>';
                        ...
                        top5ABH = top5ABH+'<tr style="color:yellow;"><td>Danni totali Top5:</td><td>'+top5LeftSideKills+'</td><td>' + top5LeftSide + '</td></tr>';
		  if (typeof bh['stats']['current'][zone][division][def] != 'undefined') {
				...
				for ( i = 0; i < bh['stats']['current'][zone][division][def].length; i++ ) {
					...
					top5DBH = top5DBH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['stats']['current'][zone][division][def][i].citizen_id].id+'">'+bh.fightersData[bh['stats']['current'][zone][division][def][i].citizen_id].name+'</a></td><td>'+bh['stats']['current'][zone][division][def][i].kills+'</td><td><strong>'+bh['stats']['current'][zone][division][def][i].damage+'</strong></td></tr>';
                        ...
                        top5DBH = top5DBH+'<tr style="color:yellow;"><td>Danni totali Top5:</td><td>'+top5RightSideKills+'</td><td>' + top5RightSide + '</td></tr>';
			setLink2BH(aBH, dBH);
