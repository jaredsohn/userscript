// ==UserScript==
// @name           YuForum Links Checker
// @namespace      http://userscripts.org/scripts/show/96078
// @include		http://*
//
...
// ==/UserScript==
// version                     3.25.02 2 Feb 2011

//GM_log("Start()");

var local_version = new Number(3.25);
...
var all_rapidshare_regex = /(http\:|^.*?http:|^.*?http%3A)\/\/rapidshare\.com/gi;
...
function filehosts_update(){
	...
	//GM_log("date_filehosts_loaded: from filehosts_update: " + GM_getValue("date_filehosts_loaded"));
	get_hosts();	
...
function get_hosts(){
	...
	//GM_log("date_filehosts_loaded: " + upd);
	if(ys > upd){
...
function get_hosts_process(){
	var hosts_data = GM_getValue("filehosts", 0).split(ret + ret);
...
function hf_link_check(hf_links){
...
function mu_link_check(mu_links){
	//GM_log("MU start")
				...
				onload: function(result) {
					...
					if (tempanav){
						...
						if (tempanavlinklinks) {
							DiplayTheDeletedLinks(tempanavlinklinks);
						}
					}
					if (livelink){
						var livelinklinks = new Array();
...
function rs_link_check(rs_links){
	...
	if (GMHosts.indexOf("rapidshare.com|") != -1){
		...
		var rsNumberCombined = rsNumber.join(",");
		var rsStringCombined = rsString.join(",");
		//alert(rsNumber);
		//alert(rsString);
	    ...
	    //                   114153692,thisisabad.link.rar,0,0,0,0,0\n");
	          ...
	          GM_xmlhttpRequest({
	            method: "GET",
	            url: 'https://api.rapidshare.com/cgi-bin/rsapi.cgi?sub=checkfiles_v1&files=' + rsNumberCombined + "&filenames=" + rsStringCombined + "&cbf=RSAPIDispatcher&cbid=1",
	            headers:{'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey', 'Content-type':'application/x-www-form-urlencoded'},
	            onload:function(result) {
	                var res = result.responseText;
...
function linkify(){
    try{
        ...
        var regex_exclude_html_trunc = /http:\/\/uploading\.com|http:\/\/letitbit.net|http:\/\/www\.gshare\.com|http:\/\/rapidshare\.com|http:\/\/netload\.in/gi;
