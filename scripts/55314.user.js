...
function render() {
	...
	if (currentFrame > play_data.length-1) {
		...
		var p = document.getElementById("play_description");
...
//console.log("fixFrames");
    ...
    for (var i=0; i<unsafeWindow.play_data.length; i++) {
        ...
        for (var j=0; j<unsafeWindow.play_data[i].length; j++) {
            ...
            if (unsafeWindow.play_data[i][j].p != null) d.p = unsafeWindow.play_data[i][j].p;
            a.push(d);
        }
        uwpd.push(a);
    }
...
function changeInfo(page) {
    ...
    var desc = document.getElementById("play_description");
...
function handleFieldGoal(play) {
    ...
    console.log(desc.childNodes.length);//firstChild.innerHTML);
    console.log(p.play);
...
function fixFrames2() {
console.log("fixFrames");
var temp = unsafeWindow.play_data.slice();
console.log(unsafeWindow.play_data[0][0].x+","+unsafeWindow.play_data[0][0].y);
    var pd = new Array();
