...
// @description    Cross search for requests and snatches on What.CD and Waffles.fm.
// @author         jonls
...
(function() {
	...
	function search_whatcd(url_base, artist, torrent, callback) {
		var search_url = url_base+'/torrents.php?action=advanced&torrentname='+encodeURIComponent(torrent);
	...
	if (/what\.cd\/torrents\.php/.test(document.URL)) {
		...
		function requests_search_waffles() {
			$('#content table > tbody > tr.rowa, #content table > tbody > tr.rowb').each(function(i) {
				...
				search_waffles(waffles_uid, waffles_passkey, artist, request, function(search_url, results) {
					if (results.length > 0) {
						result_span.html('<a href="'+results[0].url+'">'+results[0].text+' ('+binary_size_string(results[0].size)+')</a> (<i><a href="'+search_url+'">Other results</a></i>)');
					} else {
						result_span.html('Nothing found (<i><a href="'+search_url+'">Refine search</a></i>)');
						result_div.slideUp(500);
		...
		GM_registerMenuCommand('Cross Site Torrent Search: Search What.CD', function() {
			...
			$('#requesttable > tbody > tr').each(function(i) {
				...
				if (m) {
					...
					var result_span = $('<span>Loading...</span>');
					var result_div = $('<div><span style="font-weight: bold; padding-left: 18px; height: 16px; background: transparent url('+WHATCD_ICON_URI+') no-repeat scroll left center;">What.CD</span>: </div>');
					result_div.append(result_span);
					row.children('td').eq(0).append(result_div);

					search_whatcd(whatcd_url_base, artist, request, function(search_url, results) {
						if (results.length > 0) {
							result_span.html('<a href="'+whatcd_url_base+'/'+results[0].url+'">'+results[0].artist+' - '+results[0].release+'</a> (<i><a href="'+search_url+'">Other results</a></i>)');
						} else {
							result_span.html('Nothing found (<i><a href="'+search_url+'">Refine search</a></i>)');
							result_div.slideUp(500);
							row.children('td').eq(0).click(function(event) { result_div.slideToggle(500); });
