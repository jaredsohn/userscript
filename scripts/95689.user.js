...
//
// @history        2.0.2 Minor changes
// @history        2.0.1 Updater added
// @history        2.0.0 Initial release (last update of PwrTools)
// ==/UserScript==
...
/***************
WindowManager: Cooperate with Power Bot
TabManager: restructured
Fixed message counts
Warn on 0,0 march
Knights: assign to any skill
enhanced report/list page navigation and 'back' button 
fix for new KofC reports change
***************/
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  try {
    ...
    pop.getTopDiv().innerHTML = '<CENTER>PwrToolz ERROR!</center>';
    pop.getMainDiv ().innerHTML = '<DIV style="font-size:18px; font-weight:bold;"><CENTER><p>The dependency "prototype.js" was not found.<BR> Please reinstall PwrToolz <A HREF="http://userscripts.org/scripts/show/89356">here.</a></div>';
    pop.setLayer(5);
    pop.show(true);
  }
    
  window.name = 'PT';
  logit ("* KOCPwrToolz v"+ Version +" Loaded");
  readOptions();
  setCities();
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('PWRTOOLZ', eventHideShow, mouseMainTab);
}
...
var TowerAlerts = {
  ...
  postToChat : function (m, force){
    ...
    var target, atkType, who, alliance;
  ...
  var attackermight = parseInt(m.players['u' + m.pid].m);
	if(Options.includemight)
	  who += '-'+ addCommas(attackermight);
    if (m.fromXCoord)
      who += ' at '+ m.fromXCoord +','+ m.fromYCoord ;
	if (Options.includealliance)
	  alliance = m.players['u' + m.pid].a

    var msg = '';
    if (!force)
      msg = t.postToChatOptions.aPrefix +' ';
    msg += 'My '+ target +' is being '+ atkType  +' by '+ who +' of '+ alliance +'. Incoming Troops (arriving in '+
...
Tabs.AllianceList = {
  ...
  eventGotAllianceList : function (rslt){
    ...
    <TABLE><TR style="font-weight:bold"><TD class=xtab>Alliance Name</td><TD class=xtab>AllianceID</td><TD class=xtab>Rank</td><TD class=xtab>Members</td>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      if (all.relation && all.relation==1)
        ...
        dip = 'Hostile';
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD class=xtab>'+all.allianceId+'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
...
//        <TR><TD colspan=2><BR><BR><B>Updates:</b></td></tr>\
//		<TR><TD><INPUT id=checkforupd type=button value="Check for updates" OnClick="ScriptUpdater.forceNotice(scriptId, currentVersion);" /></td></tr>\
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      ...
      for (i=0; i<numRows; i++){
        ...
        m += ' ><TD><DIV onclick="ptGotoMap('+ dat[i][0] +','+ dat[i][1] +')"><A>'+ dat[i][0] +','+ dat[i][1] +'</a></div></td>';
        if (coordsOnly)
          m += '</tr>';
        else
			if (t.opt.searchType == 2) { 
				...
				if (dat[i][9]) { // Is Misted?
					m += '</td><TD align=left> &nbsp; (Misted)</td></tr>';
				} else {
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      if (cmd[0] == 'T'){
        ...
        doDefTrain (cityId, cmd[1], cmd[2], function(errMsg){Tabs.Train.doDefQueue(cityId, que, errMsg);} );
      }
    } catch (err) {
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      if (cmd[0] == 'T'){
        ...
        doTrain (cityId, cmd[1], cmd[2], function(errMsg){Tabs.Train.doQueue(cityId, que, errMsg);} );
      }
    } catch (err) {
      logit (inspect (err, 8, 1));
      t.dispTrainStatus ('<font color=#550000>PROGRAM ERROR: '+ err.message +'</font><BR>');
    ...
    try {
      ...
      for(i=0; i<Cities.numCities; i++) {
        str += "<TD width=81><B>"+ Cities.cities[i].name.substring(0,10) +'</b><BR>\
				(<a onclick="ptGotoMap('+Cities.cities[i].x +','+ Cities.cities[i].y+')">'+ Cities.cities[i].x +','+ Cities.cities[i].y  +"</a>)</td>";
      }
      if (Options.includeMarching)
	  ...
	  for(i=0; i<Cities.numCities; i++){
		...
		if(Gate == 0)
			...
			str += '<TD>Defending</td>';
			// str += '<TD><SELECT id="Gate_'+cityID+'" >';
			// str += htmlOptions( {0:'Hiding', 1:'Defending'}, Gate);
			// str += '<span id=err'+cityID+'></span>';
			// str += '</select></td>';
	  }
	  if (Options.includeMarching)
        str += '<TD width=81></td>';
	  ...
	  // for(i=0; i<Cities.numCities; i++){
	    ...
	    // document.getElementById ('Gate_'+city).addEventListener ('change', function (){
	// var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
		// params.cid = cityID;
		// params.state = parseInt(document.getElementById('Gate_'+city).value);
	// new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/gate.php" + unsafeWindow.g_ajaxsuffix, {
      // method: "post",
      // parameters: params,
      // onSuccess: function (rslt) {
		// document.getElementById('err'+city).innerHTML = "Successful";
      // },
      // onFailure: function (rslt) {
		// document.getElementById('err'+city).innerHTML = "Failed";
      // }
    // });
		// });
	  // }
  ...
  Cpager : function (a, b){
    ...
    function getHtmlElement (){
      ...
      div.innerHTML = '<STYLE>table.ptPageNav tr td  {border:none; text-align:center; padding:0px 1px;}\
        ...
        if (document.getElementById('modal_msg_list'))
          document.getElementById('modal_msg_list').style.minHeight = '320px';
      return div;
    }
