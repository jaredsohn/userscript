...
// @version		0.109
// 
// @history             0.108 Update icons.
// @history             0.108 Made one tiny tweak to support mobile worlds m123.xyz.ikariam.  See http://userscripts.org/scripts/show/132189 to login to the mobile version.
// @history             0.107 Added "buy as default" trade option.
// @history             0.106 Hide new GameForge bar option.  Better icons for alerts (desktop notification).  Better building completed alert.
// @history             0.105 Show training ships in addition to training units.
// @history             0.103 Added option to automatically accept daily bonus.
// @history             0.102 Show garrisons on island view.
// @history             0.101 Removed Army Helper display (now in Ikariam itself).  Fixed score display for 4.3.  Few small presentation issues fixed.
// @history             0.100 Various fixes for changes in v0.4.2.4 Ikariam HTML changes.
// @history             0.99 Small fix for v0.4.2.4.
// @history             0.98 Updated to no longer use PhasmaExMachina scripts (because of delete-city hack done to his/her scripts).  Removed Growl support because I don't have a copy of that script handy or a way to test it for a while.
...
ScriptUpdater.check(88157, "0.109");
...
Config.settings = {
	...
	"Overviews":{
		fields:{
			...
			tradeOverviewTypeDefaultToBuy:{
				type:'checkbox',
				label:'Buy by default',
				text:'check to make default trade type buy instead of sell',
				value:false,
			},
			worldOverview:{
				type:'checkbox',
			...
			enhancedMilitaryAdvisor:{
				type:'checkbox',
				label:'Enhanced Military Advisor',
				text:'Allow incoming attacks to be ignored',
				value: false,
			}
		}
	},
...
ExMachina = {
	...
	init:function() {
		if($('#loginForm').size() == 0 && document.body.id != 'errorLoggedOut' && $('#mainview').size() == 1) {	// in game
			...
			if(Config.get('hideGFBar')) {
				GM_addStyle(
					'body #GF_toolbar { top: 2px; } ' +
			    		'body #container { top: 0px; } ' +
					'body #extraDiv1 { top: 0px; } ' +
					'body #extraDiv2 { top: 147px; } ' +
					'#mmonetbar { display: none; visibility: hidden; }'
				);
			}
			if(Config.get('autoAcceptDailyBonus')) {
			...
			function resourceDetails() {
				...
				$('#cityResources ul.resources li').each(function(i) {
					if (i > 1) {
						...
						qty.css('top', '-3px');
						var onHand = IkaTools.cityGetResource(this.className);
						var tooltip = $('.tooltip', this);
						// get capacity
						var capacity = IkaTools.cityGetResourceMax(this.className);
			...
			function enhancedMilitaryAdvisor() {
				var advMilitaryNode = $($('#advMilitary a')[0]);
				if (advMilitaryNode.hasClass('normalalert')) {
					var ignoredMovements = IkaTools.getVal('ignoredMovements');
					if (ignoredMovements) {
						GM_addStyle('#advisors #advMilitary a.questionalert { background-image: url(http://www.clker.com/cliparts/7/0/9/b/11949856321026402567tasto_9_architetto_franc_01.svg); background-size: 90px 90px; background-repeat:no-repeat;}');
						advMilitaryNode.removeClass('normalalert');
						advMilitaryNode.addClass('questionalert');
						
						IkaTools.getMovementsUpdate(function(movements) {
							for(var i = 0; i < movements.length; i++) {
								if (movements[i].type == 'hostile') {
									if (!(movements[i].id in ignoredMovements && 
										ignoredMovements[movements[i].id])) {
										advMilitaryNode.removeClass('questionalert');
										advMilitaryNode.addClass('normalalert');
										return;
									}
								}
			...
			if(Config.get('enhancedMilitaryAdvisor')) enhancedMilitaryAdvisor();
	...
	updateMilitaryOverview:function() {
		...
		for(var i = 0; i < cities.length; i++) {
			for(var x = 0; x < IkaTools.unitTypes.length; x++) {
				...
				if(i == 0)	// output header styles once (only during first city)
					GM_addStyle('#empireMilitaryOverview th.' + IkaTools.unitTypes[x] + ' { background-image:url(/skin/characters/military/x40_y40/y40_' + IkaTools.unitTypes[x] + '_faceright.png) !important; background-repeat:no-repeat; background-position:center ' + (typeof(top[IkaTools.unitTypes[x]]) != 'undefined' ? top[IkaTools.unitTypes[x]] : '-2px') + '; }');	
			}
			for(var x = 0; x < IkaTools.shipTypes.length; x++) {
				...
				if(i == 0)	// output header styles once (only during first city)
					GM_addStyle('#empireMilitaryOverview th.ship_' + IkaTools.shipTypes[x] + ' { background-image:url(/skin/characters/fleet/40x40/ship_' + IkaTools.shipTypes[x] + '_r_40x40.png) !important; background-repeat:no-repeat; background-position:center ' + (typeof(top[IkaTools.shipTypes[x]]) != 'undefined' ? top[IkaTools.shipTypes[x]] : '-2px') + '; }');	
			}
		}
		...
		html += '<th><span style="float: right"><span id="ikaEmpireMilitaryRefreshDetails"></span><img id="ikaEmpireMilitaryRefresh" src="' + ExMachina.icons.refreshSmall + '"margin-right:2px; cursor:pointer;" title="Reload all military information"/></span></th></tr>';
		for(var i = 0; i < cities.length; i++) {
			...
			for(var x = 0; x < IkaTools.shipTypes.length; x++) {
				if(typeof(totalShips[IkaTools.shipTypes[x]]) != 'undefined' && totalShips[IkaTools.shipTypes[x]] > 0) {
					var inTraining = IkaTools.cityGetShipInTraining(IkaTools.shipTypes[x], cities[i])
					...
					html += '<td align="center">' + (qty > 0 ? qty : ((inTransit || inTraining.qty > 0) ? '' : '-')) + 
						(inTransit ? ' <span title="' + inTransit + ' in transit towards ' + IkaTools.cityGetSimpleName(cities[i]) + '" style="font-size:.8em !important;">(+' + inTransit + ')</span>' : '') +
						(inTraining.qty > 0 ? ' <span title="' + inTraining.qty + ' training in shipyard" style="font-size:.8em !important; cursor:help;">(+' + inTraining.qty + ')</span>' : '') +
						'</td>';
				}
			}
			html += '<td>';
			if(cities[i].id != IkaTools.getCurrentCityId()) {
				html += '<a href="http://' + document.domain + '/index.php?view=deployment&deploymentType=fleet&destinationCityId=' +  cities[i].id + '" title="Station fleets"><img src="http://' + document.domain + '/skin/actions/move_fleet.png" style="display:inline; float:right; height:14px; margin-left:5px;"/></a>\
						<a href="http://' + document.domain + '/index.php?view=deployment&deploymentType=army&destinationCityId=' +  cities[i].id + '" title="Deploy troops"><img src="http://' + document.domain + '/skin/actions/move_army.png" style="display:inline; float:right; height:14px;"/></a>';
			} else {
				html += '<img src="http://' + document.domain + '/skin/actions/move_fleet.png" style="display:inline; float:right; height:14px; margin-left:5px; opacity:.3;"/>\
						<img src="http://' + document.domain + '/skin/actions/move_army.png" style="display:inline; float:right; height:14px; opacity:.3;"/>';
			}
			// island link
			if(IkaTools.cityGetIslandId(cities[i]) > 0) {
				html += '<img src="http://' + document.domain + '/skin/layout/icon-island.png" style="cursor:pointer; height:16px; margin-right:2px; float:right;" name="index.php?view=island&cityId=' + cities[i].id + '&islandId=' + cities[i].islandId + '" title="View city on island"/>';
			}
			...
			if(shipyard)
				html += '<a title="Go to shipyard (level ' + shipyard.level + ')" href="javascript:ikaEmpireGoTo(\'/index.php?view=shipyard&position=' + shipyard.position + '&id=' + shipyard.cityId + '\', ' + shipyard.cityId + ');"/><img src="http://' + document.domain + '/skin/img/city/building_shipyard.png" style="float:right; margin-right:5px; height:14px;"/></a>';
			if(barracks)
				html += '<a title="Go to barracks (level ' + barracks.level + ')" href="javascript:ikaEmpireGoTo(\'/index.php?view=barracks&position=' + barracks.position + '&id=' + barracks.cityId + '\', ' + barracks.cityId + ');"/><img src="http://' + document.domain + '/skin/img/city/building_barracks.png" style="float:right; margin-right:5px; height:14px;"/></a>';
			html += '</td></tr>';
		}			
		// draw totals
	...
	reloadMilitaryOverview:function(show) {
		...
		IkaTools.reloadAllMilitary(function() {
			...
			$('#ikaEmpireMilitaryRefreshDetails').text("");
			$('#ikaEmpireMilitaryRefresh').attr('src', ExMachina.icons.checkmark);
		}, function(done, total) {
			$('#ikaEmpireMilitaryRefreshDetails').text(done + "/" + total);
		});
	},
	drawBuildingOverview:function() {
		// check for owned building types
		var ownedBuildings = {};
		var preferredBuildingOrder = [
		    'townHall', 'tavern', 'museum', 'academy',
		    'temple', 'branchOffice', 'workshop', 
		    'barracks', 'shipyard', 
		    'safehouse', 'embassy', 
		    'palace', 'palaceColony',
		    'forester', 'stonemason', 'winegrower',  'glassblowing', 'alchemist',
		    'carpentering', 'architect', 'vineyard', 'optician', 'fireworker',
		    'port', 'wall', 
		    'dump', 'warehouse', 
		    ];
		var cities = IkaTools.getCities();
		for (var x = 0; x < IkaTools.buildingTypes.length; x++) {
			ownedBuildings[IkaTools.buildingTypes[x]] = 0;
		}
		for(var i = 0; i < cities.length; i++) {
			for (var x = 0; x < IkaTools.buildingTypes.length; x++) {
				var type = IkaTools.buildingTypes[x];
				var count = IkaTools.cityGetBuildingsByType(type, cities[i]).length;
				ownedBuildings[type] = Math.max(ownedBuildings[type], count);
			}
		}
		if(!IkaTools.buildingsOverview) {
			...
			var html = [];
			html.push('<tr valign="middle" style="background-image:url(/skin/input/button.png);"><td><div style="width:80px; padding-left:1em;"></div></td>');
			for(var i = 0; i < preferredBuildingOrder.length; i++) {
				for (var j = 0; j < ownedBuildings[preferredBuildingOrder[i]]; j++) {
					html.push('<td style="text-align:center; padding:0 .05em;" title="');
					html.push(IkaTools.getText('buildings.' + preferredBuildingOrder[i]));
					html.push('"><img src="http://' + document.domain + '/skin/img/city/building_');
					html.push(preferredBuildingOrder[i] == 'townHall' ? 'townhall' : preferredBuildingOrder[i]);
					html.push('.png" style="max-height:21px; max-width:25px; margin-top:5px;"/></td>\</td>');
				}
			}
			html.push('</tr>');
			...
			for(var i = 0; i < cities.length; i++) {
				...
				if(!cities[i].type || cities[i].type.match(/tradegood/)) {
					...
					html.push(')</a></td>');
					// buildings
					for(var x = 0; x < preferredBuildingOrder.length; x++) {
						var buildings = IkaTools.cityGetBuildingsByType(preferredBuildingOrder[x], cities[i]);
						for (var j = 0; j < ownedBuildings[preferredBuildingOrder[x]]; j++) {
							if (j < buildings.length) {
								...
								html.push('<td style="text-align:center; border-left:1px dotted #9D836A;"></td>');
							}
						}
					}
					html.push('</tr>');
					...
					if(building && !IkaTools.buildingIsMaxed(building)) {
						...
						for(var b = 0; b < resourceNames.length; b++) {
							if(hasResources) {
								...
								if (required > 0)
									txt += '<nobr><img src="http://' + document.domain + '/skin/resources/icon_' + resourceNames[b] + '.png" style="height:12px; margin-left:5px; vertical-align:middle;"/> <span style="font-size:.8em;">' + IkaTools.addCommas(required) + '</span></nobr> ';
							} else {
								var missing = IkaTools.buildingGetResourceMissing(resourceNames[b], building);
								if (missing > 0)
									txt += '<nobr><img src="http://' + document.domain + '/skin/resources/icon_' + resourceNames[b] + '.png" style="height:12px; margin-left:5px; vertical-align:middle;"/> <span style="font-size:.8em;">' + IkaTools.addCommas(missing) + '</span></nobr> ';
							}										 													 
						}	
						txt += '<nobr><img src="http://' + document.domain + '/skin/resources/icon_time.png" style="height:14px; margin-left:5px; margin-top:-2px; vertical-align:middle;"/> <span style="font-size:.8em;">' + building.resources.time + '</span></nobr> ';
	...
	drawEmpireOverview:function() {
		if(!ExMachina.worldOverview) {
			...
			ExMachina.worldOverview.setAttribute('style', 'display:none; background-color:#FEE8C8; border:1px solid #9D836A; text-align:left; width:950px; margin-left:-320px; position:relative;'); 
				...
				table.style.width = "100%";
				table.innerHTML = '<tr valign="bottom" style="background-image:url(/skin/input/button.png)"><td><div style="width:80px; padding-left:1em;"></div></td>\
								  <td style="text-align:center; padding:0 .05em;"><img src="http://' + document.domain + '/skin/img/city/constructionSite.png" style="height:12px; margin-top:5px;" title="Building under construction"/></td>\
						...
						tr.style.borderTop = "1px dotted #9D836A";
							...
							if(IkaTools.cityGetIslandId(cities[i]) > 0 && IkaTools.cityGetTradegoodType(cities[i])) {
								var tradegood = document.createElement('img');
									tradegood.src = "/skin/resources/icon_" + IkaTools.cityGetTradegoodType(cities[i]) + ".png";
							...
							// island link
							if(IkaTools.cityGetIslandId(cities[i]) > 0) {
								$(td2).append('<img src="http://' + document.domain + '/skin/layout/icon-island.png" style="cursor:pointer; height:16px; margin-right:2px; float:right;" name="index.php?view=island&cityId=' + cities[i].id + '&islandId=' + cities[i].islandId + '" title="View city on island"/>');
		...
		if(city.buildings) {
			...
			for(var i = 0; i < buildings.length; i++) {
				if (buildings[i].position) {
					...
					a.innerHTML = '<img src="http://' + document.domain + '/skin/img/city/building_' + (buildings[i].position == 0 ? 'townhall' : buildings[i].type) + '.png" align="absmiddle" style="margin-right:.5em;  width:24px;"/>' +
					parsedName +
					' (' +
					buildings[i].level +
					...
					for (var x = 0; x < resourceNames.length; x++) {
						...
						if (missing > 0) {
							a.innerHTML += '<img src="http://' + document.domain + '/skin/resources/icon_' + resourceNames[x] + '.png" style="height:12px; margin-left:5px;"/> <span style="font-size:.8em;">' + IkaTools.addCommas(missing) + '</span>';
						}
					}
	...
	drawMovements:function(movements) {
		...
		var movementsHtml = "";
		var advancedView = Config.get('enhancedMilitaryAdvisor');
		var ignoredMovements = IkaTools.getVal('ignoredMovements') || {};
		var newIgnoredMovements = {};
		for(var i = 0; i < movements.length; i++) {				
			...
			var ignored = false;
			if (movements[i].id in ignoredMovements) {
				newIgnoredMovements[movements[i].id] = ignoredMovements[movements[i].id];
				ignored = newIgnoredMovements[movements[i].id];
			}
			movementsHtml += '<tr style="' + (i != movements.length - 1 ? 'border-bottom: 1px dotted rgb(157, 131, 106);' : '') + '">'+
				...
				'<td>' + (movements[i].direction == 'left' ? '<img src="http://' + document.domain + '/skin/interface/arrow_left_' + arrowColor + '.png" style="height:10px;"/>' : ' ') + '</td>' +
				'<td><img src="' + (movements[i].mission == 'occupy' ? 'skin/interface/mission_occupy.jpg' : 'skin/interface/mission_' + movements[i].mission + '.png') + '" title="' + movements[i].description + '"/></td>'+
				'<td>' + (movements[i].direction == 'right' ? '<img src="http://' + document.domain + '/skin/interface/arrow_right_' + arrowColor + '.png" style="height:10px;"/>' : ' ') + '</td>' +
				...
				'<td>' + ((movements[i].direction != 'left' && typeof(movements[i].abortHref) != 'undefined')? '<a href="http://' + document.domain + movements[i].abortHref + '" title="Abort Mission" style="padding:none; height:auto; width:auto;"><img src="' + ( '/skin/interface/btn_abort.png') + '"/></a>' : ' ') + '</td>' +
				(advancedView ? '<td><input type="checkbox" class="advancedMovementsIgnore" id="advancedMovementsIgnore_' + movements[i].id + '" ' + (ignored ? 'checked="checked"' : '') + '"></input></td>' : '') +
				'</tr>';
		}
		IkaTools.setVal('ignoredMovements', newIgnoredMovements);
		transportDiv.innerHTML = '<table id="ikaEmpireTradeMovements" cellpadding="0" cellspacing="0" border="0" style="width:100%;">'+	
									'<tr style="background-image:url(/skin/input/button.png); border-bottom: 1px dotted rgb(157, 131, 106); vertical-align:middle;"><th>Time</th><th>Units</th><th>Origin</th><th> </th><th>Mission</th><th> </th><th>Destination</th><th>Action</th>' + (advancedView ? '<th>Ignore</th>' : '' ) + '</tr>' + 
									movementsHtml + '</table>';
		...
		$('.advancedMovementsIgnore', transportDiv).click(function(data) {
			var id = parseInt($(this).attr('id').match(/\d+/)[0]);
			var ignoredMovements = IkaTools.getVal('ignoredMovements') || {};
			ignoredMovements[id] = $(this).attr('checked');
			IkaTools.setVal('ignoredMovements', ignoredMovements);
		});
	},
			...
			if (!this.className.match(/exMachinaParsed/)) {
				...
				if (Config.get('messagePreviews')) {
					...
					if (subject.match(/(circular\smessage)/i)) 
						icon = '<img src="/skin/img/city/building_embassy.png" style="height:16px; vertical-align:middle; display:inline;" title="Circular alliance message"/> ';
					else 
						if (subject.match(/(cultural\sgoods\streaty)|(cultural\streaty)/i)) 
							icon = '<img src="/skin/museum/icon32_culturalgood.png" style="height:16px; vertical-align:middle; display:inline;" title="Cultural treaty message"/> ';
						else 
							icon = '<img src="/skin/board/schriftrolle_offen2.png" style="height:14px; vertical-align:middle; display:inline;" title="Personal message"/> ';
					$('.subject', this).html('<div style="height:1.5em; overflow:hidden;">' + icon + $('.msgText', $(this).next()).text().substring(0, 150) + '</div>');
				}
				//--------------------------------- Reply as Circular ------------------
				...
				if(IkaTools.getView() == 'diplomacyAdvisor' && allianceId != 0)
					$('td.reply', $(this).next().next()).each(function() {
						$('span:eq(0)', this).each(function() {
							...
							if (link.length > 0) {
								var url = link.attr('href').replace(/receiverId=[0-9]*/, 'allyId=' + allianceId) + '&msgType=51';
	...
	getCityListForInfoBox:function(cities) {
		...
		');
		var html = '<table cellspacing="0", cellpadding="0" border="0" width="100%" class="playerCityListForInfoBox">';
		for (var i = 0; i < cities.length; i++) {
			 var city = cities[i];
			html += '<tr valign="middle">';
			html += '<td style="font-size:11px;">' +
						'<nobr><a href="http://' + document.domain + '/index.php?view=island&id=' + city.islandId + '&selectCity=' + city.id + '" title="View ' + city.name + ' on its island">' +
						'[' + city.islandX + ':' + city.islandY + '] ' + (city.name.length > 10 ? city.name.match(/^.{10}/)[0] + '' : city.name) + ' (' + city.level + ')</a></nobr>' +
					'</td><td>' +
...
ExMachina.views.branchOffice = function() {
	...
	if (Config.get('tradeOverviewTypeDefaultToBuy')) {
		if ($('#resource').val() == '0') {
			$('#resourceTradeType').val(333);
		}
		if ($('#tradegood1').val() == '0') {
			$('#tradegood1TradeType').val(333);
		}
		if ($('#tradegood2').val() == '0') {
			$('#tradegood2TradeType').val(333);
		}
		if ($('#tradegood3').val() == '0') {
			$('#tradegood3TradeType').val(333);
		}
		if ($('#tradegood4').val() == '0') {
			$('#tradegood4TradeType').val(333);
		}
	}
		...
		for(var i = 0; i < 5; i++) {
			IkaTools.getRemoteDocument(loc + '&range=' + range + '&orderBy=&searchResource=' + (i == 0 ? '' : i) + 'resource&type=444', function(doc) {
				...
				if(html.match(/resource=/)) {
					...
					$('#resourceSale' + resourceNum).html(html.replace(/\/resources\/icon_\./g, '/resources/icon_' + type + '.').replace(/<img\s+src=".+?icon_gold\.png"[^<]+</g, '<img src="skin/resources/icon_gold.png" align="absmiddle"/><'));
				}
			});
		}
		for(var i = 0; i < 5; i++) {
			IkaTools.getRemoteDocument(loc + '&range=' + range + '&orderBy=sellDesc&searchResource=' + (i == 0 ? '' : i) + 'resource&type=333', function(doc) {
				...
				if(html.match(/resource=/)) {
					...
					$('#resourceBuy' + buyNum).html(html.replace(/\/resources\/icon_\./g, '/resources/icon_' + type + '.').replace(/<img\s+src=".+?icon_gold\.png"[^<]+</g, '<img src="skin/resources/icon_gold.png" align="absmiddle"/><'));
				}
			});
		}
	}
...
ExMachina.views.island = function(){
	...
	function handleCityInfoLoad(id) {
		...
		if(Config.get('playerCitiesInIslandView')) {
			if ($('#playerCityListWrapper').size() == 0) {
				if ($('.cityinfo').is('table')) {
					$('#information .cityinfo tbody').append('<tr><td id="playerCityListWrapper" style="margin:1em 10px;" colspan="4"> </td></tr>');
				} else {
			...
			function drawScore(label, type) {
				var cityInfoNode = $('#information .cityinfo');
				if($('#exMachinaInlineScore').size() == 0) {
					if (!cityInfoNode.is('table')) {
						$('#information .cityinfo .ally').after('<li id="exMachinaInlineScore" style="height:.5em; overflow:hidden;"> </li>');
					}
				}
				if($('#exMachinaInlineScore_' + type).size() == 0) {
					if (cityInfoNode.is('table')) {
						$('.score:last', cityInfoNode).after('<tr class="score sub" id="exMachinaInlineScore_' + type + '"><td colspan="2"><span>' + label + '</span></td><td class="scoreValue" colspan="2">loading ...</td></tr>');
					} else {
				...
				IkaTools.getScoreForPlayer(type, player, function(score){
					var node = $('#exMachinaInlineScore_' + type);
					if (node.is('tr')) {
						$('td:last;', node).html(IkaTools.addCommas(score.points) + " (" + IkaTools.addCommas(score.rank) + ")");
					} else {
	...
	var tradegood = $('#islandfeatures #tradegood a').css('background-image').match(/\/([^\/]+)\.png/)[1];
	switch(tradegood) {
	...
	$('ul#cities li.cityLocation').each(function(i) {
		...
		if(target) {
			target.islandId = document.location.toString().match(/island&id=(\d+)/)[1];
			target.cityLevel = /[(](\d+)/.exec($('.name', this).text())[1];
			target.inactive = $('.inactivity', this)[0] ? true : false;
			// Nested tags are a pain
			var tagStrippedOwner = $('.owner', this).html().replace(/(<[^d].*?\/>|<[^d].*?<\/[^\d]+?>)/g, '');
			target.playerName = tagStrippedOwner.match(/^\s*([^\s].+)(\s*$|\s*<)/)[1];
			target.tradegood = tradegood;
			/*
			// fetch player score
			...
			*/
			...
			var html = '<div class="pillageHelperInfoWrapper"><p style="margin-top:1.5em;"><strong>Pillage Helper:</strong>\
							<span style="font-size:8px; position:relative; top:-2px;">\
								<span title="Wall level"><img src="http://' + document.domain + '/skin/layout/stadtmauer_icon.png" style="height:16px; display:inline; margin-left:5px; vertical-align:middle"/> ' + Pillage.targetGetWallLevel(target) + '</span>\
								<span title="Mortars needed"><img src="http://' + document.domain + '/skin/characters/military/120x100/mortar_r_120x100.png" style="height:16px; margin-left:7px; display:inline; vertical-align:middle"/> ' + Pillage.targetGetMortarsNeeded(target) + '</span>\
								<span title="Port level"><img src="http://' + document.domain + '/skin/img/city/building_port.png" style="height:16px; margin-left:5px; vertical-align:middle; display:inline;"/> ' + Pillage.targetGetPortLevel(target) + '</span>\
							</span>\
						</p>';
			...
			if(Pillage.targetGetResourcesLastUpdatedString(target) != '?') {
				...
				if(totalAvailable > 0) {
					for(var i = 0; i < Pillage.resourceTypes.length; i++) {
						...
						if(available > 0)
							html += ' <img src="http://' + document.domain + '/skin/resources/icon_' + type + '.png" style="display:inline; height:12px; vertical-align:middle; margin-right:2px;"/>' + IkaTools.addCommas(available);
					}
					var shipsNeeded = Math.ceil(totalAvailable / 500);
					html += ' <img src="skin/characters/fleet/40x40/ship_transport_r_40x40.png" style="height:14px; vertical-align:middle; display:inline;"/> ' + shipsNeeded;
				} else
					html += ' none ';
				html += '</p></div>';
			}
	...
	if(Config.get('searchIconIslandPlayer')) {
		$('.owner').each(function() {
			if ($(this).is('li')) {
				var playerName = $(this).contents()[1].nodeValue.toString().trim();
				...
				var playerName = $('td:eq(1)', this).text().trim();
				var html = '<img class="ikaSearchIcon icon" title="Show ' + playerName + '\'s cities on map" src="'+IkaSearch.icons.magnifier+'" style="cursor:pointer; display:inline; margin:0 .2em;"/>';
				$('td:last', this).append(html);
			}
			
		});
		$('.ally').each(function(i) {
			if ($(this).is('li')) {
				var spanNode = $('span:first-child + a', this);
				var alliance = $(spanNode).text();
				...
				var alliance = $('td:eq(1)', this).text().trim();
				var html = '<img name="' + alliance + '" class="ikaSearchIcon icon" title="Show ' + alliance + '\'s cities on map" src="' + IkaSearch.icons.magnifier + '" style="cursor:pointer; display:inline; margin:0 .2em;"/>';
				$('td:last', this).append(html);
	...
	if (Config.get('expandCargo')) {
		...
		$('#fleetMovements tr td .tooltip2').each(function(){
			...
			if ($('.unitBox', this).size() > 0) {
				...
				units += '<td class="icon"><img src="skin/characters/fleet/40x40/ship_transport_r_40x40.png"></td>'
				if (!Config.get('antiPlus')) {
					tooltip = "<div class='tooltip2' style-'z-index: 2000; display: none;'>" + $(this).html() + "</div>";
				}
			...
			units += '</tr>'
			quantities += '</tr>'
			html += units + quantities + '</table>' + tooltip;
			...
			IkaSearch.getPlayerCities(playerName, function(cities) {
				if(cities.length > 0) {
					...
					if(cities[0].vacation)
						$(_this).prepend('<img src="http://' + document.domain + '/skin/layout/icon-palm.png" style="vertical-align:middle; height:14px;" title="This player is in vacation mode"/> ');
...
ExMachina.views.townHall = function() {
	...
	if(true) {
	    ...
	    if (curPopulation > 0 && maxPopulationSpace > 0) {
	      ...
	      $('ul.stats .happiness').html(_(status) + durationHMS(timeLeftEx*3600,2) + msg_extra);
			}
		}
	}
...
Treaty = {
	...
	icons:{
		ct:'<img src="skin/museum/icon32_culturalgood.png" width="16" height="16" align="absmiddle" title="You have a cultural treaty with this player" style="cursor:help;" />',
		ctPending:'<img src="skin/museum/icon32_culturalgood.png" width="16" height="16" align="absmiddle" style="opacity:.4; cursor:help;" title="You have requested a cultural treaty with this player" />',
	},
	...
	views:{
		...
		museum:function(root) {
			...
			// read CTs
			Treaty.ctPlayerIds = {};	
			$('tr', $('.contentBox01h:last', root)).each(function(i) {
				if(i > 0) {
					var playerId = this.innerHTML.match(/receiverId=(\d+)/)[1];
...
Pillage = {
	...
	drawTargetsOverview:function() {
		...
		var html = '<table cellpadding="0" cellspacing="0" border="0" width="100%">\
						<tr valign="middle" style="background-image:url(/skin/input/button.png); height:14px;">\
							<th>Player</th>\
							<th style="text-align:center"><img src="http://' + document.domain + '//skin/unitdesc/unit_attack.png" style="height:16px" title="Player\'s military score"/></th>\
							<th>City</th>\
							<th style="text-align:center"><img src="http://' + document.domain + '/skin/img/city/building_townhall.png" style="height:16px" title="City Level"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/img/city/building_barracks.png" style="height:16px;" title="Garrison information"/></th>\
							<th style="text-align:center"><img src="http://' + document.domain + '/skin/layout/stadtmauer_icon.png" style="height:16px" title="Wall Level"/></th>\
							<th style="text-align:center"><img src="http://' + document.domain + '/skin/characters/military/120x100/mortar_r_120x100.png" style="height:16px" title="Port Level"/></th>\
							<th style="text-align:center"><img src="http://' + document.domain + '/skin/img/city/building_port.png" style="height:16px" title="Port Level"/></th>\
							<th style="text-align:center"><img src="http://' + document.domain + '/skin/characters/military/120x100/spy_120x100.png" style="height:16px" title="Spies in target"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/resources/icon_wood.png" style="height:14px;" title="Wood available for pillaging"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/resources/icon_wine.png" style="height:14px;" title="Wine available for pillaging"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/resources/icon_marble.png" style="height:14px;" title="Marble available for pillaging"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/resources/icon_crystal.png" style="height:14px;" title="Crystal available for pillaging"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/resources/icon_sulfur.png" style="height:16px;" title="Sulphur available for pillagin"/></th>\
							<th style="text-align:center;"><img src="http://' + document.domain + '/skin/characters/fleet/40x40/ship_transport_r_40x40.png" style="height:16px;" title="Trade ships required to carry lootg"/></th>\
		...
		for(var q = 0; q < sorted.length; q++) {
			...
			html += '	<td>' + (Pillage.targetGetTradegoodType(target) ? '<img src="http://' + document.domain + '/skin/resources/icon_' + Pillage.targetGetTradegoodType(target) + '.png" style="height:14px; vertical-align:middle; margin-right:4px;"/> ' : '');
			...
			html += '	</td>\
						<td style="text-align:right;">'
							html += Pillage.targetGetIslandId(target) ?
									'	<a href="http://' + document.domain + '/index.php?view=island&cityId=' + target.id + '&islandId=' + Pillage.targetGetIslandId(target) + '"><img src="http://' + document.domain + '/skin/layout/icon-island.png" style="height:16px;" title="View ' + target.cityName + ' on its island"/></a>' :
									'	<img src="http://' + document.domain + '/skin/layout/icon-island.png" style="height:16px;" class="disabled" title="Unable to view island because its ID is not yet known"/>'
			// missions
							...
							html += '</div>';
			// pillage
							html += ' <a href="http://' + document.domain + '/index.php?view=plunder&destinationCityId=' + target.id + '&islandId=' + Pillage.targetGetIslandId(target) + '" title="Pillage ' + target.cityName + '"><img src="http://' + document.domain + '/skin/actions/plunder.png" style="height:14px;"/></a>';
			// blockade
							html += ' <a href="http://' + document.domain + '/index.php?view=blockade&destinationCityId=' + target.id + '&islandId=' + Pillage.targetGetIslandId(target) + '" title="Blockade ' + target.cityName + '"><img src="http://' + document.domain + '/skin/actions/blockade.png" style="height:14px;"/></a>';
			// send spy
							html += Pillage.targetGetIslandId(target) ?
										' <a href="http://' + document.domain + '/index.php?view=sendSpy&destinationCityId=' + target.id + '&islandId=' + Pillage.targetGetIslandId(target) + '" title="Send spy to ' + target.cityName + '"><img src="http://' + document.domain + '/skin/actions/espionage.png" style="height:14px;"/></a>' :
										' <img src="http://' + document.domain + '/skin/actions/espionage.png" style="height:14px;" class="disabled" title="Unable to send spy because island ID is not yet known"/>';											
	...
	safehouseDrawTargetResources:function(target) {
		...
		if(Pillage.targetGetResourcesLastUpdatedString(target) != '?') {
			if(totalAvailable > 0) {
				...
				var shipsNeeded = Math.ceil(totalAvailable / 500);
				html += '</ul> <img src="skin/characters/fleet/40x40/ship_transport_r_40x40.png" style="height:24px; vertical-align:middle"/> ' + shipsNeeded;
			} else
				html += ' none ';
				...
				$('#mainview div.spyinfo').each(function(i) {
					...
					if(targetId) {
						...
						if(Pillage.targetIsOccupied(target)) {
							html += '<img src="http://' + document.domain + '/skin/img/island/besatzung_rot_cursor.png" style="margin-left:10px; height:30px; vertical-align:middle" title="The target city is under military occupation"/> '
						}
						html += '</p>';
						var totalAvailable = Pillage.targetGetResourceAvailableTotal(target);
						if(Pillage.targetGetResourcesLastUpdatedString(target) != '?') {
		...
		if($('#buildingUpgrade').size() == 1 && upgradeBox && !IkaTools.buildingIsMaxed(building)) {		
			...
			if (building) {
				...
				$('#buildingUpgrade ul.resources li').each(function() {
					...
					if(cname != 'time') {
						var missing = IkaTools.buildingGetResourceRequired(cname, building) - IkaTools.cityGetResource(cname, IkaTools.getCityById(building.cityId));
						if(missing > 0) {
							resourcesHtml += ' <strong style="color: red;">-' + IkaTools.addCommas(missing) + '</strong>';
						} else {
							resourcesHtml += ' <span style="color: green;">+' + IkaTools.addCommas(-missing) + '</span>';
						}
		...
		for(var i = 0; i < buildings.length; i++) {
			if(IkaTools.getVal('autoBuildFilterByCity').toString() != 'yes' || buildings[i].cityId == IkaTools.getCurrentCityId()) {
				...
				for(var x = 0; x < resourceNames.length; x++) {
					...
					if(missing > 0) {
						html.push('<div style="float:left; margin-right:.7em;"><img src="http://' + document.domain + '/skin/resources/icon_' + resourceNames[x] + '.png" class="autoBuildResourceIcon" align="absmiddle"/>');
...
IkaNotify = {
	...
	notices:{
		...
		audio:{
			...
			checkedByDefault: false,
			test: function() {
				$('body').append('<audio autoplay="autoplay" loop="loop"><source src="http://soundjax.com/reddo/67560%5Ealarma.mp3" type="audio/mp3" /></audio>');
			},
			send:function(eventType, message) {
				$('body').append('<audio autoplay="autoplay" loop="loop"><source src="http://soundjax.com/reddo/67560%5Ealarma.mp3" type="audio/mp3" /></audio>');
			}
			...
			test: function() {
				if (window.webkitNotifications.checkPermission() == 0) { // 0 is PERMISSION_ALLOWED
					 window.webkitNotifications.createNotification(
        					'http://' + document.domain + '/skin/layout/advisors/general_alert.png', 'Test notification', 'Desktop notification is working').show();
				} else {
					window.webkitNotifications.requestPermission();
				}
			},
			send:function(eventType, message, icon) {
				window.webkitNotifications.createNotification(icon, 'Ikariam alert: ' + eventType, message).show();
			},
		},
	},
	init:function() {
		...
		if (Growler && Growler.addNoticeType) {
			// register with growl
		...
		IkaTools.addMovementHandler(function(movement) {
			if (movement.direction && !movement.notified) {
				...
				IkaNotify.sendNotice('movements', movement.mission + action + destination, 'http://' + document.domain + '/' + (movement.mission == 'occupy' ? 'skin/interface/mission_occupy.jpg' : 'skin/interface/mission_' + movement.mission + '.png'));
			}
		});
	},
	checkForEvents:function() {
		...
		if(d.getTime() >= IkaNotify.getProcessTime()) {
			// check for attacks
			IkaTools.getMovementsUpdate(IkaNotify.checkForAttacksResult);
			IkaNotify.setNextProcessTime();
		}
	},
	checkForAttacksResult:function(movements, doc, documentText) {
		// check town advisor
		IkaNotify.checkAdvisers(doc);
		for (var i = 0; i < movements.length; i++) {
			if (movements[i].type == 'hostile') {
				if (!IkaNotify.attackAlreadyStored(movements[i])) {
					IkaNotify.storeAttack(movements[i]);
					IkaNotify.sendNotice('attack', movements[i].targetCityName + " is being attacked by " + movements[i].originCityName + " (" + movements[i].originPlayerName + ")!", 'http://' + document.domain + '/skin/layout/advisors/general_alert.png');
					// send email
					if(IkaNotify.isReadyToEmail()) {
						var url = IkaNotify.backendUrl + (IkaNotify.backendUrl.match(/\?/) ? '&' : '?') +  'ikaNotify=attack&email=' + IkaNotify.email + 
									...
									'&originId=' + movements[i].originId +
									'&originCityName=' + movements[i].originCityName +
									'&originPlayerName=' + movements[i].originPlayerName +
									'&targetId=' + movements[i].targetId +
									'&targetCityName=' + movements[i].targetCityName +
									'&time=' + movements[i].time +
									'&units=' + movements[i].units +
	...
	checkAdvisers:function(root) {
		...
		if($('#advCities', root).size() == 1) {
			IkaNotify.checkAdviser('advCities', root, '/skin/layout/advisors/mayor_active.png');
			IkaNotify.checkAdviser('advMilitary', root, '/skin/layout/advisors/general_active.png');
			IkaNotify.checkAdviser('advResearch', root, '/skin/layout/advisors/scientist_active.png');
			IkaNotify.checkAdviser('advDiplomacy', root, '/skin/layout/advisors/diplomat_active.png');
		}
	},
	checkAdviser:function(type, root, icon) {
		if(!$('#' + type + ' a', root).hasClass('normal')) {
			if(typeof(IkaTools.getVal(type + '_isActive')) != 'undefined' && IkaTools.getVal(type + '_isActive').toString() != "yes") {
				IkaNotify.sendNotice(type, "The " + IkaNotify.events[type] + " has something to say!", 'http://' + document.domain + icon);
		...
		if(delay > 0) {
			...
			for(var i = 0; i < IkaNotify.constructions.length; i++) {
				if(d.getTime() >= IkaNotify.constructions[i].endTime) {
					...
					var city = IkaTools.getCityById(building.cityId);
					IkaNotify.sendNotice('construction', 'Your ' + building.name + ' in ' + city.name + ' is now level ' + (parseInt(building.level) + 1), 'http://' + document.domain + '/skin/img/city/building_' + building.type + '.png');
	...
	getAttacks:function() {
		...
		var newAttacks = {};
		for(x in attacks) {
			if(parseInt(attacks[x].arrivalTime) > d.getTime()) {
	...
	attackAlreadyStored: function(attack) {
		var attacks = IkaNotify.getAttacks();
		return typeof(attacks[attack.id.toString()]) == 'undefined' ? false : true;
	},
	views:{
		options:function() {
			var content = document.createElement('div');
			var delayOptions = '<option value="30000">30 seconds<options><option value="180000">3 minutes<options>\
								<option value="300000">5 minutes<options>\
								<option value="420000">7 minutes<options>\
			...
			var html = '<a name="ikaNotify"></a><h3><a href="http://userscripts.org/scripts/show/80545" target="_blank" title="More information about this script">Ikariam Notifier</a> \
							...
							<table cellspacing="0" cellpadding="0"><tbody>\
								<tr><th>Check for events every</th>\
								  <td><select id="ikaNotifyMinDelay"><option value="10000">10 seconds</option>' + delayOptions + '</select>\
				...
				html += '<tr><th> </th><td>\
				<input type="button" class="button" id="ikaNotifyResetNotifications" value="Reset Notifications"/>\
			...
			$('#ikaNotifyResetNotifications').click(function() {
				IkaNotify.saveAttacks({});
			});
			//scroll down
		...
		militaryAdvisorMilitaryMovements:function() {
			...
			IkaNotify.checkForAttacksResult(IkaTools.getMovements());
		},
	},
	...
	sendNotice:function(eventType, message, icon) {
		for(var notice in IkaNotify.notices) {
			if(IkaNotify.showCheck(notice, eventType)) {
				IkaNotify.notices[notice].send(eventType, message, icon);
			}
		}
	},
	saveOptions:function() {
	...
	getProcessTime:function() {
		var value = (IkaTools.getVal('processTime') || '').toString().match(/^\d+$/) ? IkaTools.getVal('processTime') : 0;
		return value;
	},
	setNextProcessTime:function() {
		var d = new Date();
		console.log("Next delay: " + IkaNotify.getMinDelay() + ", " + IkaNotify.getMaxDelay());
...
var IkaSearch = {
	...
	getCitiesFromDom:function(r, player) {
		var cities = [];
		if (player) {
		...
		$('#main table:eq(1) tr', r).each(function(i){
			if (i > 1) {
				...
				if(matches) {
				    var foundPlayer = $('td:eq(0)', this).text();
				    if (!player || player == foundPlayer) {
					    cities.push({
						    id: matches[2],
						    islandId: matches[1],
	...
	getSearchUrlBase:function(player, alliance, action) {
		...
		var matches = document.location.toString().match(/[sm](\d+)\.([^\.]+)\.ikariam\./);
		if (matches)
			var land = matches[2];
		else {
			var matches = document.location.toString().match(/[sm](\d+)\.ikariam\./);
	...
	getPlayerCities:function(player, callback) {
		IkaTools.getRemoteDocument(IkaSearch.getSearchUrlBase(player, ''), function(r) {
			callback(IkaSearch.getCitiesFromDom(r, player));
		}, "POST", "");
	},	
		...
		IkaSearch.showWorld(function() {
			...
			function getNextResultPage(pageNum) {
				...
				IkaTools.getRemoteDocument(url, function(r) {
					var cities = IkaSearch.getCitiesFromDom(r, player);
					showCitiesOnMap(cities);
				}, 'POST', data);
			}
			if(alliance != '' || player != '')
				IkaTools.getRemoteDocument(url, function(r) {
					handleSearchResults(IkaSearch.getCitiesFromDom(r, player));
...
ExternalScripts.ArmyHelper = function () {
	const languages = {
		...
		"bosnian": {
			...
			reserve: "Rezerva",
			phalanx: "Kopljanik",
			steamgiant: "eljezni div",
			spearman: "Baca? koplja",
			swordsman: "Ma?evalac",
			slinger: "Pra?kar",
			archer: "Strijelac",
			marksman: "Sumforni Muketar",
			ram: "Ovan",
			catapult: "Katapult",
			mortar: "Minobaca?",
		...
		"french": {
			...
			line2: "Ligne de bataille de longue porte",
			flankLeft: "Flanc gauche",
			flankRight: "Flanc droit",
			submarine: "Submarine",
			reserve: "Rserve",
			phalanx: "Phalange",
			steamgiant: "Gant  vapeur",
			spearman: "Lancier",
			swordsman: "piste",
			slinger: "Lance pierre",
			archer: "Archer",
			marksman: "Tireur d'lite",
			ram: "Blier",
			catapult: "Catapulte",
			mortar: "Mortier",
			gyrocopter: "Gyrocoptre",
			bombardier: "Bombardier",
			cook: "Cuisiner",
			medic: "Mdecin",
			ship_ram: "Ram-Ship",
		...
		"german": {
			...
			archer: "Bogenschtze",
			marksman: "Schwefelbchsen-Schtze",
			ram: "Ramme",
			catapult: "Katapult",
			mortar: "Mrser",
		...
		"hungarian": {
			garrison: "Hely?rsg",
			advisor: "Tancsad",
			cityLevel: "Vros mret",
			support: "Tmogats",
			artillery: "Tzrsgi",
			air: "Lgi",
			line1: "Frontvonalban",
			line2: "Lvsz vonalban",
			flankLeft: "Bal szrnyon",
			flankRight: "Jobb szrnyon",
			submarine: "Bvrhaj",
			reserve: "Foglalt",
			phalanx: "Hoplita",
			steamgiant: "G?zris",
		...
		"italian": {
			...
			addSlot: "Aggiungi unit ad uno slot",
			removeSlot: "Rimuovi unit ad uno slot",
			fillRow: "Riempi una linea con le unit",
			selectUnits: "Seleziona unit",
			assignNone: "Nessuna",
			assignAll: "Tutte",
		...
		"latvian": {
			garrison: "Garnizons",
			advisor: "Konsultants",
			cityLevel: "Pils?tas l?menis",
			support: "Pal?dz?ba",
			artillery: "Artil?rija",
			air: "Gaisa sp?ki",
			line1: "Priek?j? l?nija",
			line2: "T?l?s kaujas l?nija",
			...
			cook: "Pav?rs",
			medic: "?rsts",
			ship_ram: "Tar?na ku?is",
			ship_flamethrower: "Ugunsmet?js",
			ship_steamboat: "Dzenrata ku?is",
			ship_ballista: "Ballistiskais ku?is",
			ship_catapult: "Katapultas ku?is",
			ship_mortar: "M?nmet?ja ku?is",
			ship_submarine: "Zem?dene",
			addSlot: "Pievienot vien?bas uz vienu lauci?u",
			removeSlot: "No?emt vien?bas no viena lauci?a",
			fillRow: "Aizpild?t vienu kaujas rindu ar vien?b?m",
			selectUnits: "Izv?lies vien?bas",
			assignNone: "Nevienu",
			assignAll: "S?t?t visu",
			assignField: "Aizpild?t kaujas lauku",
			presetsTitle: "Iepriekpl?nots sast?vs",
			presetsAdd: "Pievienot jaunu sast?vu",
			presetsRemove: "Dz?st sast?vu",
			presetsNewName: "Nor?diet nosaukumu kaujas sast?vam lai to saglab?tu",
			optShowTitles: "R?d?t kaujas rindu virsrakstus"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		//  "lithuanian":   { },
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		"polish": {
			...
			air: "Lotnictwo",
			line1: "Linia frontu",
			line2: "Linia walki na odleg?o??",
			flankLeft: "Lewa flanka",
			flankRight: "Prawa flanka",
			submarine: "Okr?ty podwodne",
			reserve: "Rezerwa",
			phalanx: "Hoplita",
			steamgiant: "Gigant",
			spearman: "W?cznik",
			...
			catapult: "Katapulta",
			mortar: "Mo?dzierz",
			gyrocopter: "?yrokopter",
			bombardier: "Bombardier",
			cook: "Kucharz",
			medic: "Medyk",
			ship_ram: "Okr?t z taranem",
			ship_flamethrower: "Okr?t z miotaczem ognia",
			ship_steamboat: "Okr?t parowy z taranem",
			ship_ballista: "Balista",
			ship_catapult: "Okr?t z katapult?",
			ship_mortar: "Okr?t z mo?dzierzem",
			ship_submarine: "Okr?t podwodny",
			addSlot: "Wype?nij jeden slot",
			removeSlot: "Opr?nij jeden slot",
			fillRow: "Wype?nij ca?? lini?",
			selectUnits: "Wybierz jednostki",
			assignNone: "Nic",
			assignAll: "Wszystko",
			assignField: "Wype?nij pole bitwy",
			presetsTitle: "Ustawienia",
			presetsAdd: "Dodaj ustawienie",
			presetsRemove: "Usu? ustawienie",
			presetsNewName: "Podaj nazw? ustawienia",
			optShowTitles: "Show battlefield row titles"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		"portuguese": {
		...
		"spanish": {
			...
			air: "Aire",
			line1: "Lnea de combate principal",
			line2: "Lnea de combate a larga distancia",
		...
		"swedish": {
			...
			line1: "Frontlinje",
			line2: "Distanslinje",
			flankLeft: "Vnster flank",
			flankRight: "Hger flank",
			submarine: "Ubt",
			reserve: "Reserv",
			phalanx: "Hoplit",
			steamgiant: "ng-jtte",
		...
		"turkish": {
			garrison: "Garnizon",
			advisor: "Dan??man",
			cityLevel: "?ehir Boyutu",
			support: "Destek",
			artillery: "Topu",
			air: "Hava",
			line1: "n Cephe",
			line2: "Uzun Menzilli Sava? Hatt?",
			flankLeft: "Sol Kanat",
			flankRight: "Sa? Kanat",
			submarine: "Submarine",
			reserve: "Rezerv",
	...
	function storeTargetCity(){
		$("ul#cities li.cityLocation.city").each(function(){
			...
			var node = $('.cityinfo .name', this);
			var level = 0;
			if (node.is('li')) {
				level = parseInt(/[(](\d+)/.exec(node.text())[1]);
			} else {
				level = parseInt(/[(](\d+)/.exec($('td:eq(1)', node).text())[1]);
			}
			if (isNaN(level)) {
				return;
			}
	...
	//-----------------------------------------------------------------------------
	function makeGarrisonTable(city){
		var html = "<table>\n";	
			for (row in unitsList.army) {
				html += '<tr align="center">\n';
				for (i in unitsList.army[row]) {
					var qty = IkaTools.cityGetUnitQty(unitsList.army[row][i], city);
					html += '<td style="padding: 3px;"><div style="padding: 3px; background-color: #FAEAC6;' + ((qty == 0) ? ' opacity:0.3;' : '') + '"><img class="advisorUnitIcon" src="' + images[unitsList.army[row][i]] + '" title="' + language[unitsList.army[row][i]] + '"><br><span>' + qty + '</span></div></td>\n'
				}
				html += '</tr>\n';
			}
			...
			for (row in unitsList.fleet) {
				for (i in unitsList.fleet[row]) {
					var qty = IkaTools.cityGetShipQty(unitsList.fleet[row][i].substring(5), city);
					html += '<td style="padding: 3px;"><div style="padding: 3px; background-color: #FAEAC6;' + ((qty == 0) ? ' opacity:0.3;' : '') + '"><img class="advisorUnitIcon" src="' + images[unitsList.fleet[row][i]] + '" title="' + language[unitsList.fleet[row][i]] + '"><br><span>' + qty + '</span></div></td>\n'
				}
			}
			html += '</tr>\n';
	...
	function drawUnitButtons(type, targetBattlefieldProperties, draw){
		GM_addStyle(
			'#container .assignUnits .button.fillRow { left: 562px; width:8px; } ' +
			'#container .assignUnits .textfield { left: 600px !important; }' +
			'#container .assignUnits .button.minus { left: 470px !important; }' +
			'#container .assignUnits .button.plus { left: 516px !important; }' +
			'div.assignRightBlock { height: 45px; text-align: right; margin: -15px 24px -15px 5px;"}');
		$("ul.assignUnits > li > a.plus").each(function(){
			...
			if (draw) {
				place.append('<a class="button fillRow" title="' + language.fillRow + '">#</a>');
			}
			
			var unit = $(this).parents("li:last").attr("class").replace(/^\s*(.*?)\s*$/, "$1").replace(/\s+.*$/, "");
			var id = $("input.textfield", $(this).parent()).attr("id");
			...
			if (draw) {
				$("a.fillRow", place)[0].addEventListener('click', function(){
					this.blur();
	  ...
	  <input type="button" value="' +
		...
		var shipButtons = '<a class="button" id="ArmyHelper_assignAllShips" style="position:absolute; top:0px; left:70px;">' + language.assignAll + '</a>';
		$("#plusminus a:last").after(shipButtons);
		$("#ArmyHelper_assignAllShips").click(function() {
			var shipsAvailable = $("#globalResources .transporters span:eq(1)").text().match(/\d+/)[0];
			$("#extraTransporter").val(shipsAvailable);
		});
	}
	...
	//-----------------------------------------------------------------------------
	function drawIsland(){
		GM_addStyle('.AH_Garrison{float:left; position: relative; }');
		GM_addStyle('.AH_GarrisonTable{display:none; z-index: 9999; position: relative; top: 0; left: 0; background-color: rgb(252, 244, 222); width: 362px; height: 203px; line-height:12px; padding: 1px 5px 5px;}');
		GM_addStyle('.AH_Garrison:hover >.AH_GarrisonTable{display:block;}');
	
		var showGarrisons = Config.get('islandShowGarrisons');
		var garrisonCities = {};
		$('select#citySelect option').each(function(i) {
			garrisonCities[$(this).attr('value')] = null;
		});
		$('ul#cities li.cityLocation').each(function(i) {
			try { var cityId = this.innerHTML.match(/city_(\d+)/)[1]; } catch(e) { var cityId = false; }
			if(showGarrisons && cityId) {
				if (cityId in garrisonCities) {
					//var html = '<span><img src="http://' + document.domain + '/skin/characters/military/120x100/phalanx_r_120x100.png" height="16" /><span>';
					var html = '<span class="AH_Garrison"><img class="AW_GarrisonImage" src="' + images.phalanx + '" height="16" width="16" align="absmiddle"><div class="AH_GarrisonTable">';
					html += makeGarrisonTable(IkaTools.getCityById(cityId)) 
					html += "<div style='position: absolute; top: -10px; left: -5px; width: 384px; height: 5px;'><img width='100%' src='" + images.hintTop + "'/></div>" +
						"<div style='1000; position: absolute; top: 2px; left: -5px; width: 12px; height: 100%; background: transparent url(" +
						images.hintLeft +
						") repeat-y scroll 0 0;'></div><div style='position: absolute; top: 2px; right: -5px; width: 12px; height: 100%; background: transparent url(" +
						...
						"'/></div>";
				html += '</div></span><clear>';
					$('span.before', this).after(html);
				}
			}
		});
		$('.AH_Garrison').mouseover(function() {
			$('#friends').hide();
			$('.cityLocation').hide();
			$('#islandfeatures').hide();
			$(this).parent().parent().parent().show();
		});
		$('.AH_Garrison').mouseout(function() {
			$('#friends').show();
			$('#islandfeatures').show();
			$('.cityLocation').show();
		});;
	...
	views:{
		...
		island:function() {
			...
			$('ul#cities li').each(function() {
				try { var allyId = this.innerHTML.match(/allyId=(\d+)/)[1]; } catch(e) { var allyId = false; }
				if(allyId) {
					AllianceColor.addAllianceColorSelector($('.cityinfo .ally a', this).eq(0));
				}
			});
			AllianceColor.updateColors();
			AllianceColor.startChangeListener();
