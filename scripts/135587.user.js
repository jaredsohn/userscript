...
// @version        1.0.8
// @author         eCitizen Scyld - mods by hesar, update by Stileth, mod by gronie
// @namespace      eCitizenScyld
// @description    v1.0.8 added option to count by kills against hits
// @description    updated with Q6 support - by hesar
// @description    v1.0.7 added ranks till God of War*** and Q6 hit break line in profile
// @description    v1.0.8 added option to count by kills against hits
...
function letsJQuery() {
	...
	var content = $j('div#pvp_battle_area');
        ...
        var strength = $j('#fighter_skill').text();
        ...
        '<div><b >&nbsp;</b><small>Q6 / done:</small>'+
        '<p id="pCurrentHit">'+ max_hit +' / ' + current_damage/max_hit +'</p></div></td></tr>');
        $j('#total_damage').css('margin-top','-30px');
        /*
	...
	$j('div#enemy_defeated').before(
		...
		'&nbsp;&nbsp;<input type="checkbox" id="multihit_kills" name="multihit_kills"><label for="multihit_kills">&nbsp;Count kills</label>' +
	...
	unsafeWindow.jQuery.fn.multiHIT = function() {
                ...
                var hitsOrKills = multiHitDone;
                if ($j('input#multihit_kills').is(':checked'))
                    {
                        unsafeWindow.console.log(multiHitEnemyKill, ' ', multiHitDone, ' ', multiHitCount);
                        hitsOrKills =  multiHitEnemyKill;
                    }
		if (unsafeWindow.globalStop || multiHitCount == hitsOrKills) {
                        unsafeWindow.console.log('stop by ',hitsOrKills , ' or ',unsafeWindow.globalStop);
			multiHitRunning = false;
			$j('button#multihit_start').html('HIT!');
			bLog('All done.');
			return;
			...
			if (typeof bh['current'][zone][att] != 'undefined') {
				...
				for ( var i = 0; i < bh['current'][zone][att].length; i++ ) {
					...
					top5ABH = top5ABH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['current'][zone][att][i].citizen_id].id+'">'+bh.fightersData[bh['current'][zone][att][i].citizen_id].name+'</a></td><td>'+bh['current'][zone][att][i].kills+'</td><td><strong>'+bh['current'][zone][att][i].damage+'</strong></td></tr>';
                                        top5LeftSide += parseInt(bh['current'][zone][att][i].damage,10);
                                        top5LeftSideKills += parseInt(bh['current'][zone][att][i].kills,10);
		  ...
		  if (typeof bh['current'][zone][def] != 'undefined') {
				...
				for ( var i = 0; i < bh['current'][zone][def].length; i++ ) {
					...
					top5DBH = top5DBH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['current'][zone][def][i].citizen_id].id+'">'+bh.fightersData[bh['current'][zone][def][i].citizen_id].name+'</a></td><td>'+bh['current'][zone][def][i].kills+'</td><td><strong>'+bh['current'][zone][def][i].damage+'</strong></td></tr>';
                                        top5RightSide +=  parseInt(bh['current'][zone][def][i].damage,10);
                                        top5RightSideKills +=  parseInt(bh['current'][zone][def][i].kills,10);
	     	}
