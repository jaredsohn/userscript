// ==UserScript==
// @name           Pokec.sk - Elite Mini Chat
// @namespace      Elite Minichat - JS-jQuery (by Merlin)/AS-FLASH (by Weroro)/PHP-mySQL (by Marvin-Hog)
// @description    Chat klient pre Azet Admin Network (AAN) pre komunikáciu správcov miestností
// @include        http://pokec-sklo.azet.sk/miestnost/*
// @include        http://www-pokec.azet.sk/miestnost/*
// @date           2012-01-03
// @author         This script made by Merlin & Weroro
// @version        1.3.4
// ==/UserScript==
...
*   master function 
*   include chat on
*   Azet glass page source code
*   and set Query functions to define object
*/
  ...
  if(navigator.userAgent.indexOf("Chrome")!=-1)
  {
    window.unsafeWindow||(
      ...
      {
        var elite_a=document.createElement("p");
        elite_a.setAttribute("onclick","return window;");
        return elite_a.onclick()
      }
      ()
    )
  }

  var elite_url = document.URL,
      elite_oldRoom = 'http://www-pokec.azet.sk/',
      elite_verify;
  
  var elite_cssStare =  "<style type=\"text/css\" id=\"cssStare\">"+
                  "#pokecRadio{visibility:hidden !important;}"+
                  "#mozaikaAzet,#topBanner .topBanner iframe{visibility:hidden !important;}"+
                  "#frameBnr{visibility:hidden!important;}"+
                  "</style>";
   
  var elite_cssNove =	"<style type=\"text/css\" id=\"cssNove\">"+
                "#pokecRadio{display:none !important;}"+
                "#ucho,#reklama_parapet,#mozaikaAzet,#reklama,#frameBnr,#topBanner .topBanner iframe{display:none !important;}"+
                ".css_reklama160x600 .css_reklamavpravo, #c_fotoalbumy_uvod .css_reklama300x300 .css_reklamavpravo,.css_reklama300x300 .css_reklamaomne, .css_reklamavlavo300, .c_brandbb{ display:none !important;}"+
                ".mrgTop2,.c_reklama .c_objekt,#advert{display:none !important;}"+
                ".css_lista s100,#js_idLista,.loparik nadol{box-shadow: none !important; -moz-box-shadow: none !important;}"+
                "#sklo{height:96% !important;}"+
                "</style>";      
				
		
				
  // Close topBanner window
  if(elite_url.search(/^http:\/\/www-pokec.azet.sk\//)!=-1)
  {
 ...
 /* function removeElement(id)
  ...
  // Close topBanner window END
  if (elite_url.substr(0,25) == elite_oldRoom)  // if old glass
  { 
    elite_verify = unsafeWindow.myNick;    // load user nickname
    elite_userid = unsafeWindow.curUID;
    $("body").append(elite_cssStare);       // hide adds and radio
  }
  else                              // else new glass
  {
    elite_verify = unsafeWindow.nick;
    elite_userid = unsafeWindow.i9;
    
     //removeElement('reklama'); 
    $("body").append(elite_cssNove);
                     
  }

    $(function()
		{
		  var elite_w = 550,	// iframe window width
			elite_h = 200,	// iframe window height
			elite_borderW = 8,	// iframe border width on pixels
			elite_borderR = 8,	// iframe border round definition on pixels
			elite_borderColor = "0,0,0,0.35",	// border color define with rgba(R,G,B,A)
			elite_wrapW = elite_w+(elite_borderW*2),		// wraper width
			elite_wrapH = elite_h+40+(elite_borderW*2),	// wraper height
			elite_chatURL = "http://elite.weroro.sk/",
			elite_flashTag = '<iframe src="'+elite_chatURL+'?azet='+elite_verify+'&token='+elite_userid+'" width="'+elite_w+'" height="'+elite_h+'" scrolling="no" frameborder="0"></iframe>',
			elite_buttonOpacity = "0.5",	// visibility (0.00 - 1.0), 1 = 100%
			elite_button = "data:image/png;base64,",
			elite_miniStyle = "<style type=\"text/css\">#chatWrap{width:66px;height:36px;overflow:hidden;z-index:9996;position:absolute;bottom:20px;margin:0 auto}#chatBigDiv{position:absolute;bottom:0}#chatBigDiv a{position:relative;float:left;bottom:-130px;width:"+elite_w+"px}#toggler{display:block;width:30px;height:30px;opacity:"+elite_buttonOpacity+";position:relative;left:36px;margin-bottom:-82px}#toggler img{width:30px;height:30px}#frame{width:"+elite_w+"px;height:"+elite_h+"px;display:block;position:relative;top:86px;border:"+elite_borderW+"px solid rgba("+elite_borderColor+");border-radius:"+elite_borderR+"px}</style>",
			elite_minichat = '<div id="chatWrap"><div id="chatBigDiv"><a><div id="toggler"><img src="'+elite_button+'" id="chatToggler"></div><div id="frame">'+elite_flashTag+'</div></a></div></div>';
			  
			$("body").append(elite_miniStyle).append(elite_minichat);			// include cascading styles and AANC in page source
				
			$("#toggler").click(function()		// chat switch
					{
						var elite_chat = $('a', $("#chatBigDiv"));
						if (elite_chat.css("bottom") == "-130px") 
						{
							$('#chatWrap').animate({top: '-=220px', height: '+=220px', width: '+=500', left: '-=500'}, 200);
							elite_chat.animate({bottom: ["86px", "swing"]}, 500);		// if chat is hidden set visible
							$(this).animate({left: '+=500'}, 1000);
							$(this).fadeTo("fast", 1);
						}
						else
						{ 
							elite_chat.animate({bottom: ["-130px", "swing"]}, 200);	// if chat is visible set hide
							$(this).animate({left: '-=500'}, 1000);
							$('#chatWrap').animate({top: '+=220px', height: '-=220px', width: '-=500', left: '+=500'}, 1000);
							$(this).fadeTo("fast", elite_buttonOpacity);
						}
					});
							
			if ($.cookie('mini_cookie'))
			{	// if cookie exist
				var elite_coords = $.cookie('mini_cookie').split(',');	// load iframe window position from cookie
			}
			else
			{
				var elite_coords = [600,550]; // default value
			}
					
			$('#chatWrap').css({top: parseFloat(elite_coords[0]), left: parseFloat(elite_coords[1])});	// set iframe window position from cookie
					
			$("#chatWrap").draggable({stop: function(e, ui)		// include dragable mode on this object
				{
					$.cookie('mini_cookie',$('#chatWrap').css('top') + ',' + $('#chatWrap').css('left')); // save iframe window position
				}});					
			});
