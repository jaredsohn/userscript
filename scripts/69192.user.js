...
// Credit goes to Tim Wilson & JK for his NFL script
...
if( sport != '' ) {
	...
	// Add the search icon to the Recent Transactions area.
	addSearchIcon("recenttransactions");

	// And add it to the Transactions page.
...
// TRW: Added for changing player link.
var addPlayerLinkGraphic = "data:image/gif;base64,"+
	"R0lGODlhEgALAPcAAJlnA%2F%2F%2F%2F%2BGdOOaJAPzu2c57APO6ZseJLP"+
	"feuahxIP78%2BOGND75yAPbOku2bIeF5AO%2BqQ%2FXo1fTEfOmTEueeL%2B"+
	"6iMbNrAOySC%2F7z5OmUFu%2BmOt6TJPG1WNd%2BAKtmAPTBdPPIieN9AK18"+
	"NPrkw%2BeOCsR0AOimQf%2F%2F%2F%2B2eKeyUEvft3v3v2%2ByXF9iBAqxp"+
	"AOuNA9yEAPfRmffFc%2FCvS%2F337eSGAO%2BoP%2F%2F%2F%2FwAAAAAAAA"+
	...
	"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"+
	"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"+
	"AAAAAAAAAAACH5BAEAAAEALAAAAAASAAsAQAiEAAMIDPABggYNKFA4YMHCxM"+
	"CHGTIMmEgRRgELHh4KrKDgw4ULA07ESHGiQAmNAWzYcPCx5YUaLVqgdNCRos"+
	"gJHRhYQBlAQIsCJnViFMHTwIyDCRemSLFgA0oNJ6Ke%2BCC1ZAGUKBRE%2FR"+
	"BSagkGKFlMtRkDp06UFBbA6NDBZIkSLgAkGBgQADs%3D";
// The above graphic was replaced by Yahoo's plus sign graphic.  Credit goes
// to LooGie (http://userscripts.org/users/15252) for the idea.
	...
	for( var i = 0; i < tds.length; i++ ) {
		if( tds[i].getAttribute('class') == 'player' ) {
			thisElement = tds[i].getElementsByTagName('a')[0];

			playerName = thisElement.text.replace(/^.* /,'');
...
//			addPlayerLink.innerHTML = "<img border='0' src='" + addPlayerLinkGraphic +"'>" ;
			addPlayerLink.innerHTML = "<img border='0' src='http://us.i1.yimg.com/us.yimg.com/i/us/sp/fn/default/full/add.gif'>" ;

			// 10/22/2007: Changed from insertBefore with 3 nextSiblings to appendChild with 2 in case there aren't 3 nextSiblings.
			thisElement.parentNode.appendChild(addPlayerLink, thisElement.nextSibling.nextSibling);
		}
	}
}
	...
	for( var i = 0; i < tds.length; i++ ) {
		if( tds[i].innerHTML == 'BN'
			...
			for( var j = 0; j < thisRowTds.length; j++ ) {
				if( tds[i].innerHTML == 'BN' ) {
					thisRowTds[j].style.backgroundColor = '#fec';
				} else {
