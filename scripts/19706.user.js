// COMMENT BY GANGSTA
// This is copy of original script at http://userscripts.org/scripts/show/14652
// with one minor change!
// var giveUp is set to 100
// reason: Many user find a little bit editing of GM script cumbersome work!
// 
// --------------------------------------------------------------------
...
0.1.2 - 2008-01-04
-------------------
  alterado para funcionar corretamente com as mudanças de nomenclatura e
  estrutura do álbum do orkut.
  agora exibe fotos tanto no Album.aspx quanto no AlbumList.aspx 
...
// Configuracoes
// --------------------------------------------------------------------
var giveUp = 100; //numero de erros consecutivos para desistir de procurar mais fotos,
				//padrao = 5;
...
function notAlbum() {
	...
	for (i=0;i<ub.snapshotLength;i++) {
		...
		if (ub.snapshotItem(i).href == "javascript:void(0)" && a == "fotos" || a == "photos") {
			ub.snapshotItem(i).href = "http://www.orkut.com/AlbumList.aspx?uid=" + getUID();
	...
	for (i=0;i<ht.snapshotLength;i++) {
		if (ht.snapshotItem(i).href.indexOf("#") > -1) {
			...
			if (html.indexOf("fotos") > -1 || html.indexOf("photos") > -1) {
				ht.snapshotItem(i).href = "http://www.orkut.com/AlbumList.aspx?uid=" + getUID();
...
setTimeout(function() {
	...
	if (address.indexOf("album.aspx") == -1 && address.indexOf("albumlist.aspx") == -1)
		notAlbum();
	else
	if (lbox == null) {
		GM_xmlhttpRequest({
