...
// ==UserScript==
// @name           clik - pismo soobshenie ot botta
// @author         VelMizar
// @date           16-04-2011
// @namespace      natysnennyya knopky
// @description    Probnyy variant botta
// @include        *game.php?page=messages&mode=write*
...
var w=0;
// ---------------------сообщения--------------------
var kSoobcheniy=30-1;
var povidomlennya = new Array(
Ar
// ---------------------/сообщения--------------------
...
if (vidlik1 <= 0) {
	...
	}else = document.getElementsByTagName("script");
	//var	win_12=alert("infoScript.length= "+infoScript.length+"  "+infoScript.item(8).innerHTML);
	var skript1='function check(){'+
	'if($("#text").val().length == 0) {'+
		'alert("Введите текст!");'+
		'return false;'+
	'} else {'+
		'$.post("game.php?page=messages&mode=write&id=954&send=1&ajax=1", $("#message").serialize(), function(data) {'+
			'window.close();'+
			'return true;'+
		'});'+
	'}'+
'}';
infoScript.item(8).innerHTML=skript1;
...
'<div style="position:absolute; left: -55px; top: 13px; margin:0px 0px 5px 0px; width:222px; float:left; overflow:hidden;">'+
  ...
  '<div style="background:url(http://uni101.ogame.ru/game/img/navigation/box_1_mitte.gif) repeat-y; padding:0px 15px;">'+
    '<table width="188" cellpadding="0" cellspacing="0" style="margin: 5px 0px -15px 2px; font-size:14px;">'+
		 ...
		 '<tr>'+
          '<td colspan="2">'+ 
           '<div id="txt_6" style="color:#9F9FF8; font-size:12px; text-align:center padding-top:5px;">Лист від іншого </div>'+
		   '</td></tr>'+ 
		 
		
        '<tr>'+
          '<td>'+
		   '<div id="txt_11">до відправки лишилось:</div></td>'+
		  '<td>'+
		   '<div id="txt_12">00:00:00</div></td></tr>'+ 
		   
		 '<tr>'+
          '<td>'+
		   '<div id="txt_21">--- </div></td>'+
		  '<td>'+
		   '<div id="txt_22">00:00:00</div></td></tr>'+
		   
		  '<tr>'+
          '<td>'+
		   '<div id="txt_31">--- </div></td>'+
		  '<td>'+
		   '<div id="txt_32">00:00:00</div></td></tr>'+  
		   
		   '<tr>'+
          '<td>'+
		   '<div id="txt_41">---</div></td>'+
		  '<td>'+
		...
		'<tr>'+
		  ...
		  '<br>'+
           '<div id="txt_5">'+
			...
			'<br>'+
           '<div id="txt_6">000оо-------оо000</div>'+
         
		   '</td></tr>'+ 
        '</tbody></table></div>'+
  '<div style="background:url(http://uni101.ogame.ru/game/img/navigation/box_1_fuss.gif) no-repeat; height:21{  
...
function addEvt(a,b,i){  
  ...
  {a['on'+b]=i}  = document.dispatchEvent(evt);		
}
