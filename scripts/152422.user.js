// ==UserScript==
// @namespace       heroeswm
// @name            HWM Hero Preview
// @description     Предпросмотр страницы героя.
// @version         3.7.28
// @grant           GM_setValue
// @grant           GM_getValue
...
// @exclude         http://www.heroeswm.ru/pl_info.php*
// @exclude         http://www.heroeswm.ru/object-info.php*
// @exclude         http://www.heroeswm.ru/clan_info.php*
// @exclude         http://www.heroeswm.ru/auction.php*
// @exclude         http://www.heroeswm.ru/tavern.php*
// @exclude         http://www.heroeswm.ru/*event*.php*
// @exclude         http://www.heroeswm.ru/*quest*.php*
// ==/UserScript==
...
if (window === top) {
    ...
    var desc = {
        'details':
              '<ul>'
            + '<li>Предоставляет возможность просматривать параметры героев, их армию и артефакты перед битвой не уходя со страницы.</li>'
            + '<li>Полностью совместим со скриптом <a href="http://hwmforum.ru/forum_messages.pl?id=437">Добавление в инфу перса инфы об ОА, ПП и ХП армии</a> от <a href="http://www.heroeswm.ru/pl_info.php?nick=alex_kocharin">alex_kocharin</a>.</li>'
            +'</ul>',
    ...
    var conf = {
        ...
        'modeMouseOver': {'type': 'bool', 'title': 'При наведении', 'def': false, 'desc': 'Режим срабатывания при наведении на ник героя.'},
        'showTalents': {'type': 'bool', 'title': 'Показывать навыки', 'def': true, 'desc': 'Отображение навыков в предпросмотре.'},
        'showSkills': {'type': 'bool', 'title': 'Показывать умения', 'def': true, 'desc': 'Отображение умений в предпросмотре.'},
        'autoclose': {'type': 'bool', 'title': 'Одно окно', 'def': true, 'desc': 'В режиме срабатывания по клику отобржать всегда только одно окно предпросмотра.'},
        'supMode': {'type': 'bool', 'title': 'Иконка степенью', 'def': false, 'desc': 'Отображение иконки предпросмотра степенью (бывает что мешает нормальному отображению страницы).'},
    }
...
if (!disabled) {
    ...
    // Работаем только в основном окне
    if (window === top) {
        
        var modeMouseOver = GM_getValue('modeMouseOver', false);
        ...
        var supMode = GM_getValue('supMode', false);
        
        // Перебираем все ссылки
        var refs = document.getElementsByTagName('a');
        for (var i = 0; i < refs.length; i++) {
            // На каждую ссылку пытаемся применить предпросмотр
            UIHeroPreview(refs[i], modeMouseOver, showTalents, showSkills, autoclose, supMode);
        }
