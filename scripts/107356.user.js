// ==UserScript==
// @name           Dot World Order v3.2 (dynamic)
// @namespace      crohllf
// @version        3.2
// @date           2011-07-20
...
var SCRIPT = {
	name: 'Dot World Order v3.2 (dynamic)',
	namespace: 'crohllf',
	source: 'http://userscripts.org/scripts/show/99493',
	identifier: 'http://userscripts.org/scripts/source/99493.user.js',
	meta: 'http://userscripts.org/scripts/source/99493.meta.js',
	version: '3.2',
	date: '2011-07-20',
};

var threadURL = 'http://goallineblitz.com/game/forum_thread.pl?thread_id=4554883';
var postId = '41385592';
...
var list = {
	family: {
		member: {},
		title: 'DWO Members',
		style: 'color: Gold; background-color: black; padding: 0px 3px 0px 3px;',
	...
	apprentice: {
		...
		title: 'FNG',
		style: 'color: green;',
		agentTitle: function(id, name) { return 'FNG'; },
		forumAccess: 1,
	},
	promoteme: {
		member: {},
		...
		if (pref.indexHilite.val) {
			$('#content table.forums span.last_post_user').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html(parts[0] + '<br><b style="color: Gold; background-color: black; padding: 0px 3px 0px 3px;">' + parts[1] + '</b>');
				}
			});
			$('#threads span.thread_starter').each(function() {
				...
				if (list.family.member[id]) {
					$(this).html('Started by: <b style="color: Gold; background-color: black; padding: 0px 3px 0px 3px;">' + name + '</b>');
				}
			});
			$('#threads span.last_post_user').each(function() {
				...
				var id = nameToId[name];
				if (list.family.member[id]) {
					$(this).html('<b style="color: Gold; background-color: black; padding: 0px 3px 0px 3px;">' + name + '</b>');
				}
			});
		}
		break;
...
//						data.html(data.html().replace(n + '</a>', n + '</a> (real name: ' + realName.replace('(banned)', '<b><i>(banned)</i></b>') + ')'));
			...
			if (membersWithTitles) {
				statusMsg += prefix + '<b style="color: Gold; background-color: black; padding: 0px 3px 0px 3px;">' + membersWithTitles + ' with titles</b>';
				prefix = ', ';
			}
			if (membersFNG) {
