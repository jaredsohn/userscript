// ==UserScript==
// @name           Dosser_User_Profil_von_NewMan V1.1.1
// @namespace      http://ego-shooters.foren-city.de/
// @description    Shows the users cash and alcohol status in the user profile
...
function user(dom){
	...
	try {
		...
		if(cash.length >= 9){
			cash_show = "&pound;" + cash.substring(0,cash.length-8) + "." + cash.substring(cash.length-8,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length);
		} else if (cash.length>=6){
			cash_show = "&pound;" + cash.substring(0,cash.length-5) + "." + cash.substring(cash.length-5, cash.length-2) + "," + cash.substring(cash.length-2, cash.length);
		} else if(cash.length>2){
			cash_show = "&pound;" + cash.substring(0,cash.length-2) + "," + cash.substring(cash.length-2, cash.length);
		} else if(cash.length==2){
			cash_show = "&pound;0," + cash;
		}
		else {
			cash_show = "&pound;0,0" + cash;
		}
