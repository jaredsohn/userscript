// ==UserScript==
// @name         Guangues que dominam Bairros
// @namespace    http://www.faveladogame.com.br
// @author       Edited by James Maxwel
// @description  Mostra as Gangues que domina os Bairros
...
} else if (location.toString().indexOf("http://sampa.faveladogame.com.br") != -1) {
...
function PrepareTable(table, debug) {	
	...
	for(var i = 0; i < trs.length; i++) {
	 ...
	 trs[0].getElementsByTagName("td")[3].style.width = '60px'; 
 	 trs[0].getElementsByTagName("td")[3].innerHTML = '<span class="city_name">Latas</span>'; 
...
function AddHeader(table, AddMultiFlag) {
	...
	// Text einfügen
	newtr.getElementsByTagName("td")[0].innerHTML = '<span class="city_name">' + GetNrOfOpenDistricts(table.innerHTML) + ' de ' + GetNrOfDistricts(table.innerHTML) + ' bairros podem ser dominados.</span>';
			...
			if (responseDetails.status == 200) {
				...
				for(i = 1; i < trs.length; i++) {
					...
					// sonst: Dieser Stadtteil ist derzeit von keiner Bande besetzt
						...
						// Link zur vorherrschenden Bande in die erste Zelle hinzufügen
						currenttr.getElementsByTagName("td")[0].innerHTML = '<span class="city_name"><font color="' + color + '">' + icon + Districtname + '</font></span><br />&nbsp;Nenhuma Gangue domina este bairro';
					}
				}
