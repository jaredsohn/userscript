...
// @author         Peety (thx Heinzel) Aenderung DS V6.0 by Hypix
// @version        1.99.20  
...
function check_angriff() {
	...
	try {								// auf Angriffssymbol pruefen
        awert = parseInt(Body.match(/graphic\/unit\/att\.png[\?*][0-9*]"[^>]*> \((\d+)\)/)[1], 10);
	// nur bis DS V5.x:  awert = parseInt(Body.match(/graphic\/unit\/att\.png[?*][0-9*]" alt=""> \((\d+)\)/)[1], 10);
