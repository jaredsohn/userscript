...
function showPreferencesMenu(){
	...
	// names last updated
	var timeAgo = Math.round(((new Date().getTime()) - (GM_getValue('DSExpireTime') - CACHE_TIME)) / 60000);
	if(GM_getValue('DSExpireTime') == '0'){
		...
		for(var j=0;j<LIST.sports.length;j++){
			...
			//if(!(GM_getValue('BSHideSpt' + sport.title,false))){
				if(sport.names[nameList[i].toUpperCase()]){
					...
					resultsArray.push({sport:sport.title,status:'Not Duck',id:j});
					if(results.rank < 0){
						results = {rank:0,status:'Not Duck'};
					}
				}
			//}
		}
			...
			// }else{
				for(var j=0;j<resultsArray.length;j++){
					function createLinkAll(){
						...
						var appendStr = '<div class="sportStatusHolder"><a href="javascript:void(0);">Duck Status</a>: ' + colorTheStatus(resultsArray[j].status) + '</div>';
...
function showFlushLink(){
	var timeAgo = Math.round(((new Date().getTime()) - (GM_getValue('DSExpireTime') - CACHE_TIME)) / 60000);
	$('BODY DIV.crt.links').append(' | Duck Status version: ' + VERSION.toFixed(2) + ', <span id="PSBottomSpan">cached for ' + timeAgo + ' minutes <a href="javascript:void(0);" id="PSBottomFlush">flush</a></span>');
...
function gatherSport(sequence){
	if(sequence + 1 > LIST.sports.length){
		//save to cache
		GM_setValue('DSExpireTime',(new Date().getTime() + CACHE_TIME) + '');
		GM_setValue('DSListInfo',JSON.stringify(LIST));
...
function retrieveCache(){//return false;
	...
	var expTime = GM_getValue('DSExpireTime');
	...
	LIST = JSON.parse(GM_getValue('DSListInfo'));
	parsePage();
	return true;
};
...
function retrieveHeader(){
	GM_xmlhttpRequest({
		...
		onerror:function(err){
			...
			if(GM_getValue('DSListInfo')){// we have old lists, we will use that for now
				LIST = JSON.parse(GM_getValue('DSListInfo'));
				parsePage();
			}
		}
