...
// @updateURL http://userscripts.org/scripts/source/115000.meta.js
// ==/UserScript==
...
(function(){if(''!=document.title){h=document.getElementsByTagName('head')[0];if(location.href.match(/(:\/\/)(acid3\.acidtests\.org\/)/)){;
  ...
  if(OFF){s=document.createElement('script');s.src=user+'78184.user.js';h.appendChild(s);}
}if((!location.href.match(/(:\/\/)(\w*\.?youtube\.com\/)(my_|embed\/)/))&&(location.href.match(/(:\/\/)(\w*\.?youtube\.com\/)/))){
  if(OFF){s=document.createElement('script');s.src=user+'39167.user.js';h.appendChild(s);}
}if(!location.href.match(/(:\/\/)(\w*\.?twitter\.com\/|\w*\.?facebook\.com\/|i-mobile\.co\.jp\/|[\w\-]*\.?nicovi|[\w\-]*\.?google\.[\w\-\.]+\/|translate|ad)/)){
  if(OFF){s=document.createElement('script');s.src=user+'110178.user.js';h.appendChild(s);}
}}})();
