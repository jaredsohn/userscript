...
// @version	 	 1.0
// ==/UserScript==
//Italian translation by Kurtz Waiser. Special thanks to Daemonfool!
...
menuLinkReplacements = {
	...
	"myMilitaryUnit.html"	:["Military unit","UnitÃ Â  Militari"],
	...
	"militaryUnitStatistics.html"	:["Military unit stats","UnitÃ Â  Militari"],
...
hpTabsReplacements = {
	...
	"#friendsShouts":["Military unit","UnitÃ Â  Militare"],
...
function doHP() {
	...
	for (kk in hpContentEventsReplacements) {
		...
		allElements.innerHTML=allElements.innerHTML.replace(/(was attacked by )([^<]*)(<[^>]+>)/g,"Ã¨ stata attaccata da $2$3");
		...
		allElements.innerHTML=allElements.innerHTML.replace(/(was accepted)/g,"Ã¨Â¨ stata approvata dal Congresso");
		allElements.innerHTML=allElements.innerHTML.replace(/(was proposed in congress )/g,"Ã¨Â¨ stata proposta al Congresso");
	...
	allElements.innerHTML=allElements.innerHTML.replace(/(\- Country)([^\-]*)(\-  Military Unit)([^\-]*)(\- Friends)/," Nazione | $2  UnitÃ Â  Militari | $4 Amici");
...
function doWork() {
	...
	if (allElements.innerHTML.match(/You have no job^\./)) {
		...
		if (allElements.innerHTML.match("stay in a country")) {
			...
			tmp.innerHTML=tmp.innerHTML.replace(/You need to stay in a country where company is located to work/,"Per lavorare devi essere nello Stato dove Ã¨Â¨ situata l'azienda");
			idxoffset = 2;
		}
		...
		if (allElements.innerHTML.match("You have not worked today")) {
			...
			tmp.innerHTML=tmp.innerHTML.replace(/(Your base productivity)/,"ProduttivitÃ¨Â  base");
			tmp.innerHTML=tmp.innerHTML.replace(/(Productivity modifiers)/,"Modificatori della ProduttivitÃ¨Â ");
			tmp.innerHTML=tmp.innerHTML.replace(/(Total productivity)/,"ProduttivitÃ¨Â  finale");
			tmp.innerHTML=tmp.innerHTML.replace(/(Units produced)/,"UnitÃ Â  prodotte");
			tmp.innerHTML=tmp.innerHTML.replace(/(Raw company quality)/,"QualitÃ Â  dell'azienda raw");
		}
	}
}
...
function doProductMarkets() {
	...
	tmp.innerHTML=tmp.innerHTML.replace("Country:","Nazione:");
	tmp.innerHTML=tmp.innerHTML.replace("Quality:","QualitÃ Â :");
	...
	tmp.innerHTML=tmp.innerHTML.replace(/(Stock)/,"QuantitÃ Â ");
...
function docompaniesForSale() {
	...
	tmp = allElements;
	tmp.innerHTML=tmp.innerHTML.replace(/(Company)/,"Azienda");
	tmp.innerHTML=tmp.innerHTML.replace(/(Product)/,"Prodotto"); // PERCHÃ¨ non va?
...
function doTrain() {
	...
	//already trained
		...
		tmp.innerHTML=tmp.innerHTML.replace(/(You have already trained today)/,"Oggi ti sei giÃ Â  allenato. Torna domani!");
		rowoffset = 2;
	}
	
	tmp = allElements.children[1+rowoffset];
	tmp.innerHTML=tmp.innerHTML.replace(/(You have already trained today. Please come back tomorrow.)/,"Oggi ti sei giÃ Â  allenato. Torna domani!");
...
function doBattle() {
	...
	if (allElements.innerHTML.match("occupant country to")) {
		...
		allElements.innerHTML=allElements.innerHTML.replace(/(This round was won by:)/g,"Questo round Ã¨Â¨ stato vinto da:");
	} else {
		tmp = allElements.children[1];
...
function doProfile() {
	...
	tmp.innerHTML=tmp.innerHTML.replace(/Military unit:/,"UnitÃ Â  Militare:");
	...
	//no
	tmp.innerHTML=tmp.innerHTML.replace(/No military unit/,"Nessuna UnitÃ Â  Militare");
...
function doTravel() {
	...
	tmp = allElements.children[8];
	tmp.innerHTML=tmp.innerHTML.replace(/Ticket Quality:/,"QualitÃ Â  biglietto:");
...
function replaceContractComm(obj, kbn) {
	...
	obj.innerHTML=obj.innerHTML.replace(/ will instantly donate the following products/," donerÃ¨Â  subito i seguenti Prodotti");
	obj.innerHTML=obj.innerHTML.replace(/ will instantly donate the following sum of money/," donerÃ¨Â  subito le seguenti somme di Denaro");
	obj.innerHTML=obj.innerHTML.replace(/ will be charged with following debt/," si farÃ¨Â  carico del seguente Debito");
...
function doContract() {
	...
	if (isDummy) {
		...
		tmp = allElements.children[0];
		tmp.innerHTML=tmp.innerHTML.replace(/Product quantity:/,"QuantitÃ¨Â :");
		tmp = allElements.children[2];
		tmp.innerHTML=tmp.innerHTML.replace(/Product type:/,"Tipo di Prodotto:");
		tmp = allElements.children[5];
		tmp.innerHTML=tmp.innerHTML.replace(/Product quality:/,"QualitÃ Â  del Prodotto:");
...
function doMonetaryMarket() {
    ...
    tmp = allElements.children[0]
    tmp.innerHTML = tmp.innerHTML.replace(/Seller/,"Venditore");
    tmp.innerHTML = tmp.innerHTML.replace(/Amount/,"QuantitÃ¨Â ");
...
function doCitizenMarketOffers() {
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/Country/,"Nazione")
    tmp.innerHTML = tmp.innerHTML.replace(/Product/,"Prodotto")
    tmp.innerHTML = tmp.innerHTML.replace(/Quantity/,"QuantitÃ¨Â ")
    tmp.innerHTML = tmp.innerHTML.replace(/Price/,"Prezzo")
    tmp.innerHTML = tmp.innerHTML.replace(/Post new offer/,"Aggiungi nuova offerta")
...
function doCompany() {
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/The company will be created in your current location - region /,"La Azienda verrÃ¨Â  creata nella regione dove ti trovi adesso: ")
...
function doInviteFriends() {
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/Additionally you get/,"In piÃ¨Â¹ tu guadagni");
    ...
    tmp.innerHTML = tmp.innerHTML.replace(/Note, that your friends will also receive an additional/,"Nota che anche il tuo amico guadagnerÃ¨Â ");
    tmp.innerHTML = tmp.innerHTML.replace(/Gold for registering through your game invitation link when they reach level 7/,"Gold in piÃ¨Â¹ del normale quando ragigungerÃ¨Â  livello 7 per essersi registrato col tuo referrer link");
    tmp = allElements.children[4]
    ...
    tmp = allElements.children[17]
    tmp.innerHTML = tmp.innerHTML.replace(/Show more clicks/,"Mostra piÃ¨Â¹ click");
...
function doPremium() {
	...
	tmp.innerHTML="<h2 style='text-align: center'>Account Premium</h2>"
		+ "<i>E-sim</i> Ã¨Â¨ un gioco gratuito e tutti possono giocare senza pagare niente. Tuttavia se sei un fan di E-sim potresti sostenerci trasformando il tuo account in un account Premium<br/><br/>"
		+ "L'<u>Account Premium</u> ti darÃ¨Â  accesso ad alcune informazioni interessanti e funzionalitÃ¨Â  utili come dettagli aggiuntivi nelle battaglie e messaggi di gruppo alla MU o al Partito. In aggiunta gli altri giocatori vedranno una <i>prestigiosa Stella</i> nel tuo profilo<br>"
...
function doDonations() {
	...
	tmp.innerHTML = tmp.innerHTML.replace(/Your donation will be visible to everyone on this page/,"La tua donazione sarÃ¨Â  visibile a tutti in questa pagina")
...
{
var aggiornamento = document.getElementsByTagName("td");
aggiornamento[18].innerHTML += "<center><iframe id='frame' src='http://www.youandy.altervista.org/E-sim/index.php?v=1.1' width='200px'></iframe></center>";
}