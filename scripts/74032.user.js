// ==UserScript==
// @name          dR Chatbox
// @description   Adds new features to dR Chatbox
// @include       http://www.desirulez.net/
// @include       http://www.desirulez.net/index.php
// @version       2.6.1
// @author        Jaanoo
// @date          2010-04-10
// ==/UserScript==
...
var chatbox = $('dbtech_shoutbox_frame');
if (chatbox) {
	chatbox.style.height = '100%';
}
//change the following value to false to disable automatic text box color change to black
chatcolor = true;

if (chatcolor) {
	var chattxt = $('dbtech_shoutbox_editor');
	if (chattxt) {
		chattxt.style.color = 'Black';
	}
}
...
if (!(myDate<today)&&!(myDate>today))
  {
  alert("Today is Jaanoo aka Hassan's Birthday. Wish him. HAHA this was fun :D");
  }
	...
	versionInfo: {
		...
		compareVersions: function (updatedVersion) {
			...
			if (SVC.userRequestCheck) {
				...
				if (reply) GM_openInTab("http://userscripts.org/scripts/show/74032" + SVC.scriptNum);
