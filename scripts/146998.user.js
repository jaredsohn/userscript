// ==UserScript==
// @name           OGameRediseño Compactador Batallas DEMONS
// @namespace      http://userscripts.org/scripts/show/146998
// @id				Demons-Compactador
// @namespace		Demons-Compactador
// @description		Demons Compactador de Batallas y Espionajes. HoChiChaos modified by CaOs & Argone, modified by Demons
// @version        1.0.1
// @author         HoChiChaos
// @updateURL      http://userscripts.org/scripts/show/146998.meta.js
// @downloadURL    http://userscripts.org/scripts/show/146998.user.js
...
var SCRIPT = {
  name: "Compactador automático de batallas DEMONS"
  ,url: "http://userscripts.org/scripts/show/146998"
  ,version: "1.0.1"
  ,funciona_ok: "5.0.99.99"
};
...
function codificar(patron, tipo, version) {
    ...
    if(tipo == "HTML") {
        ...
        marcas = [
	    [/{DEMONS}/gi, '<img src="http://i.imgur.com/FR5nF.png" border=0 alt="Demons Fornax"></a>'],
	    [/{DEMONS_I}/gi, '<a href="http://DeAThCuLTArMaGeDDon.com" target="_CaOs" title="Demons Fornax"><img src="http://DeAThCuLTArMaGeDDon.com/images/DCA/CaOs.png" border=0 alt="Demons Fornax"></a>'],
	    [/{DEMONS_R}/gi, '<img src="http://i.imgur.com/Blbse.png" border=0 alt="Reciclado 100% por Demons Fornax"></a>'],
	    [/{DEMONS_S}/gi, '<img src="http://i.imgur.com/bGFpr.png" border=0 alt="Demons Fornax">'],
            [/{B}/gi, '<b>'],
            [/{\/B}/gi, '</b>'],
            [/{I}/gi, '<i>'],
            [/{\/I}/gi, '</i>'],
    ...
    if(tipo == "OGame") {
        ...
        marcas = [
            [/{DEMONS}/gi, '[[img]http://i.imgur.com/FR5nF.png[/img]'],
	    [/{DEMONS_I}/gi, '[url=http://DeAThCuLTArMaGeDDon.com][img]http://DeAThCuLTArMaGeDDon.com/images/DCA/CaOs.png[/img][/url]'],
	    [/{DEMONS_R}/gi, '[img]http://i.imgur.com/Blbse.png[/img]'],
	    [/{DEMONS_S}/gi, '[img]http://i.imgur.com/bGFpr.png[/img]'],
            [/{B}/gi, '[B]'],
            [/{\/B}/gi, '[/B]'],
            [/{I}/gi, '[I]'],
            [/{\/I}/gi, '[/I]'],
...
function generarPatron()  {
	...
	patron += '{DEMONS_S}{NL}{NL}';

        if(getNumRondas() == 0) {
          patron += '{SIZE_PEQ}La batalla duró 1 ronda el día ' + getFecha() + '{/SIZE}{NL}';
        }
        else {
          patron += '{SIZE_PEQ}La batalla duró ' + (getNumRondas()+1) + ' rondas el día ' + getFecha() + '{/SIZE}{NL}';
        }
...
function temporizador() {
  ...
  var ya_esta_compactado = document.getElementById('compactado');
  var hay_batalla_para_compactar = getElementsByClass("combatreport");

  if(hay_batalla_para_compactar != null) {

    if(ya_esta_compactado == null) {

            generarPatron();
            
            master = hay_batalla_para_compactar[0];
...
if( location.href.indexOf('page=messages') != -1 ||
     location.href.indexOf('page=combatreport') != -1 ) {
    var int=setInterval (temporizador, 1100);
}
