	...
	(function (f) {
		...
		var dump = function (obj, depth) {
			...
			init_TWmapTBR = function () {
				...
				TWMap.TBR = [];
				TWMap.urls.TBRmap = config.map + 'tribe=' + game_data.player.ally_id + '&player=' + game_data.player.id + '&world=' + game_data.world + '&v=' + config.version + '&json';
					...
					loadSectors: function (sectors) {
						...
						for (var i = 0; i < wait_sectors.length; i++) {
							...
							//if(wait_data.data!==null&&wait_data.tiles!==null&&wait_data.pmap!==null){
								...
								TWMap.TBR.handler.loadSector_TBRResult('TBRmap_' + gd.player.id+ '_' + wait_data.id,this.loadSector_TBRResult,wait_data,this);
							//}
						...
						if(this._sector_request_queue.length>0){
							...
							for(var i=0;i<this._sector_request_queue.length;i++){
								...
								$.ajax({type:'GET',url:query_string,dataType:'json',success:function(data){
									loadSector_TBRResult('TBRmap_' + gd.player.id+ '_' + wait_data.id, this.loadSector_TBRResult, wait_data, this);
...
/* report: 	t#1{th{Onderwerp} td{datum} {t#2}} 
			...
			if (tr = doc.getElementById('attack_info_att')) {
				...
				if (verdediger == player_id) {
				 ...
				 var type_of_units = 'attack_info_att_units';
				 var my_units;
				} else {
				 var type_of_units = 'attack_info_def_units';
				}
				
				// ** UPDATE: Sake/Warre ** //

				if (tr = doc.getElementById(type_of_units)) {
					tr = tr.rows;
					for (var i = 0, len = tr.length; i < len; i++) {
						c = tr[i].cells[0].textContent;
						if (c == txtDB["troops"]) {
						...
						} else if (type_of_units == 'attack_info_att_units') {
							my_units = "";
							var val = "";
							for (a = 1, clen = tr[i].cells.length; a < clen; a++) {
								if (n = tr[i].cells[a].innerHTML) {
				...
				// ** UPDATE: Sake/Warre ** //
				if (type_of_units == 'attack_info_att_units') {
				 addState("TBRmap+: Notitie opslaan", true);
				 
				 var host = config['info'];
				 host += 'tribe=' + tribe_id;
				 ...
				 $.ajax({
					...
					success: function (data, status, req) {
						...
						dt[1] = dt[1].split(':');
						var d = new Date();
						if (dt[0][2].length == 2) { dt[0][2] = 2000+(dt[0][2]*1); }
						d.setFullYear(dt[0][2],(dt[0][1]*1)-1,dt[0][0]);
						d.setHours(dt[1][0]);
						d.setMinutes(dt[1][1]);
						d.setSeconds(dt[1][2]);
						d.setMilliseconds(0)
						
						if ((typeof data.note != "undefined") && (typeof data.notehtml != "undefined") && (data.note != '')) {
						 if (data.note.match('#TBRMAP\\+#')) {
						  var ndate = data.note.split('#')[2];
						  if (isNaN(ndate)) { ndate = 0; }
						  var nd = new Date();
						  nd.setTime(ndate);
						  if (d.getTime() < ndate) {
						   addState("TBRmap+: Er is een recentere aanval geweest vanuit dit dorp", true);
						   return;
						  } else if (d.getTime() == ndate) {
						   addState("TBRmap+: Notitie is al bekend", true);
						   return;
						  }
						 }
						}
						// Post TBRnote through ajax, then submit for tribalwars note
						var host = config.note + 'world=' + world + '&village=' + village + '&tribe=' + tribe_id + '&player=' + player_id + '&owner=' + player_id + '&hash=' + data.note_hash + '&v=' + config.version + '&json';

						var my_note = '#TBRMAP+#'+d.getTime()+'#';
						my_note += '\n\n[b]Aanval ontvangen uit dit dorp[/b]';
						my_note += '\nDatum: ' + datum;
						...
						if (my_units) {
							var z = 0;
							my_note += '\n';
							my_units = my_units.split('|');
							var my_aantal = aantal.split('|');
							var my_verliezen = verliezen.split('|');
							for (z=1;z<my_units.length;z++) {
								if (my_aantal[z] > 0) {my_note += '\n[img]' + my_units[z] + '[/img]:' +my_aantal[z]};
								if (my_verliezen[z] > 0) { my_note += ' (-'+my_verliezen[z]+')'; }
							};
						}
						...
						$.ajax({
							...
							success: function (data, status, req) {
								addState("TBRmap+: Notitie opgeslagen!", true);
							}
						});
					}
				 });
