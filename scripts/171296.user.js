...
// @exclude        http://filecloud.io*
// @exclude        http://*putlocker.io*
// @exclude        http://putlocker.com*
// @usoscript      9467
// ==/UserScript==
 
fileMETA["update"] = 'fixed hotfile.com, filefactory.com, crocko.com ';
 ...
 return;
	...
	if ( location.href.match(/https:\/\/torrentproject.com\/userscripts\/rapidshare_links_checker_new\/search\//))
	{
		...
		function GetAndShowMeTheRapidshare(googlesearchurl,where,rightpositionprs,TheQueryIsCommingFrom)
		{
			...
			GM_xmlhttpRequest(
			{
				...
				onload:function(responseDetails)
				{
					...
					//googleresponse = googleresponse.replace(/\/interstitial\?url=/g,'');	
		...
		function wheretosearch()
		{
			...
			"all" 						 : '"rapidshare.com/files"||"letitbit.net"||"badongo.com/file/"||"depositfiles.com/"||"vip-file.com/download"||"filefactory.com/file"||"storage.to/get"||"hotfile.com/dl"||"netload.in/date"',
			"all2" 						 : '"rapidshare.com/files"||"letitbit.net"||"badongo.com/file/"||"depositfiles.com/"||"vip-file.com/download"||"filefactory.com/file"||"storage.to/get"||"hotfile.com/dl"||"netload.in/date"',
...
//		Check_ifile_dot_it_links = GM_getValue("Check_ifile_dot_it_links", false);
	 ...
	 firsttime)
	{
		...
		if (((numberofrslinks > 0) || (numberofhflinks > 0) || (numberofwulinks > 0) || (numberofustationlinks > 0)|| (numberofuplinks > 0) || (numberoffflinks > 0) || (numberoffjlinks > 0) || (numberoffglinks > 0) || (numberofnllinks > 0) || (numberoffslinks > 0) || (numberofmulinks > 0)) && !(document.getElementsByTagName('RSLC')[0])) {
			add_RSLC_style();
		}
		...
		function getfflinkschecked(all)
		{
			for (var i = all.length - 1; i >= 0; i--) {
				GM_xmlhttpRequest(
				{
					...
					//url: 'http://www.filefactory.com/tool/links.php',
					headers: {
						'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
						'Content-type': 'application/x-www-form-urlencoded'
					},
	 ...
	 function getfglinkschecked(all)
		{
			...
			console.log(all[i]);
			console.log(encodeURIComponent(all[i]));
			//alltobeids = all[i].replace(/\n/g,'');
			//console.log(alltobeids);
				GM_xmlhttpRequest(
				{
					method: "POST",
					...
					//http://filegag.com/?op=checkfiles
					//http://filegag.com/?op=checkfiles&list=http%3A%2F%2Ffilegag.com%2Fx9ivqhjkzw5z%2Fdeprived-wrathof_ans-xvid.avi.html%0D%0Ahttp%3A%2F%2Ffilegag.com%2Fx9ivqhjkz95z%2Fdeprived-wrathof_ans-xvid.avi.html%0D%0Ahttp%3A%2F%2Ffilegag.com%2F09ivqhjkzw5z%2Fdeprived-wrathof_ans-xvid.avi.html%0D%0Ahttp%3A%2F%2Ffilegag.com%2Fx9ivqhjkzw5z%2Fdeprived-wrathof_ans-xvid.avi.html%0D%0Ahttp%3A%2F%2Ffilegag.com%2Fzxv7mnk9vkei%2FBattle.of.the.Pacific.2011.DVDRip.XviD.4PlayHD.avi.html%0D%0A&op=checkfiles&process=Check%20URLs
 ...
 function DiplayTheDeletedLinks(fotfoundlinks)
	 ...
	 function DiplayTheTempAvailableLinks(tempanavlinklinks)
		...
		var numberoflinks=0;
		numberoflinks = numberofrslinks + numberoffflinks + numberoffjlinks + numberofnllinks + numberoffglinks + numberoffslinks + numberofuplinks + numberofhflinks + numberofwulinks + numberofustationlinks + numberofmulinks;
 ...
 document.getElementById('Keyboard_functions').innerHTML +='<br/>'+first_key_keycodename+'+'+String.fromCharCode(configuration_keycode)+'=Configuration <br/>'+first_key_keycodename+'+'+String.fromCharCode(makeTheCheckedLinksToFullURl_keycode)+'=Convert links to full address links <br/>'+first_key_keycodename+'+'+String.fromCharCode(check_all_links_keycode)+'=Check all the links (for ajax sites)';
	...
	function start()
	{
		...
		var mediafire_com= new Array(6) 
		 ...
		 mediafire_com[2]='(?:no longer stored)|(?:Invalid or Deleted File)|(?:Free Sign Up)'; 
		...
		var depositfiles_com= new Array(6) 
		 ...
		 depositfiles_com[2]='does not exist'; 
		...
		var uploaded_net= new Array(6)									 
		 ...
		 uploaded_net[2]='(?:"box_red")|(?:Page not found)'; 
		 uploaded_net[3]='optional--'; 
		 uploaded_net[4]="//a[contains(@href,'uploaded.net/') or contains(@href,'ul.to/')]"; 
		...
		var filecloud_io= new Array(6) 
		 filecloud_io[0]='filecloud\.io\/'; 
		 filecloud_io[1]='(request download ticket)|(in order to download)'; 
		 filecloud_io[2]='(file removed)|(file expired)|(no such file)|(page not found)'; 
		 filecloud_io[3]='optional--'; 
		 filecloud_io[4]="//a[contains(@href,'filecloud.io')]"; 
		 filecloud_io[5]='optional--'; 
		 filecloud_io[6]='&nbsp;\\n\\t\\t(\\d.*?)	</span>';
		 http_file_hosts.push(filecloud_io);
		}
		if(Check_putlocker_dot_com_links){
		var putlocker_com= new Array(6) 
		 putlocker_com[0]='putlocker\.com\/'; 
 ...
 if (Show_info_in_tooltip){
		...
		var http_file_hosts_all = [imdb_com,netload_in,crocko_com,mediafire_com,megashares_com,narod_ru,rapidshare_de,veehd_com,vip_file_com,yourfilehost_com,sendspace_com,gigasize_com,filepost_com,duckload_com,link_protector_com,bitshare_com,cramit_in,bitroad_net,uploadbox_com,keepfile_com,letitbit_net,x7_to,zshare_net,uploaded_net,filesend_net,filecloud_io,putlocker_com,solidfile_com,shareflare_net,sharejunky_com,storage_to,megaftp_com,co_cc_vault,sharingmatrix_com,rapidshare_com,megaupload_com,filefactory_com];
		var totalxpath='';
		var totalourls='';
 ...
 if(muligtLink.match(regex)){	
					...
					for(myArray = null; myArray = regex.exec(muligtLink); )
					{
						...
						if ( (prefix.toLowerCase() != 'http://adf.ly/3966902/http://') && (prefix.toLowerCase() != 'http://adf.ly/3966902/https:/') )
						{
							href = 'http://adf.ly/3966902/http://' + href;
						}			
						var a = document.createElement('a');
