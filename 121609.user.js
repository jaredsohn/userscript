...
function doTravel() {
	...
	replaceLinkByHref({
		"http://wiki.e-sim.org/en/Traveling":["Traveling tutorial on wiki","Déplacement tutoriel sur le Wiki"]
	});
}
...
function doNewspaper() {
	...
	if (allElements) {
		...
		tmp.innerHTML=tmp.innerHTML.replace(/Please save a copy of your article on a hard disc before submitting it/,"SVP , faites une sauvegarde de votre articles sur votre disque dur avant de l'envoyer!!!!");
	}
}
...
function doContract() {
	...
	if (isDummy) {
		...
		tmp.nodeValue=tmp.nodeValue.replace(/you can only propose contract to your friend/,"vous ne pouvez proposer des contrats que à vos ami");
	}
	
	replaceLinkByHref({
		"profile.html?id=0":["Dummy citizen","Citoyen par défaut"],
		"contracts.html":["Go back to contract list","Retour à la liste des contrat"]
	});
	replaceInputByValue({
		"Delete":["Delete","Supprimer"],
