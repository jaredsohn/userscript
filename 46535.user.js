...
// === Changelog
// 5.04.01
// Изменены регулярки для добавления ссылок на разделы пользователей /[Dr]Wolf/
// ----------
// 5.04
// Исправлена ошибка: Скачиваение музыки с результатов поиска /Марьясин Семён/
...
if (checkUpdates || checkVKUpdateNow) {
	...
	if (checkVKUpdateNow)
		showDialog('Обновление vkPatch','<center><h1><font color="#444444"><b>Проверка обновления…</b></font></h1></center>','','Скрыть','',hideFunc());
...
if (location.pathname == '/settings.php') {
	...
	if (GET['act'] != 'vkPatch') {
		...
		"<h4>vkPatch Настройки <small><font color='#98A7B5'>версия "+vkpathVersion+"</font></small></h4><div align='right'><small><a href='?checkVKUpdateNow=1&act=vkPatch'>Проверить обновление</a></small></div>"+
...
if (document.getElementById('vkontakte')) {
	...
	link.innerHTML = '<a href=\"http://klinifini.livejournal.com/tag/vkPatch\" target=\"_blank\">vkPatch</a>';
