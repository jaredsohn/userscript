...
// @version        0.12
//
// @history        0.12 Added RTL support
// @history        0.11 Removed footer buttons width to make it autosize for different languages
// @history        0.10 Added labels to option tabs
// @history        0.09 Expanded size
// @history        0.09 Improved positioning
// @history		     0.09 Added multi-language support
...
Config = {
	...
	callback:null,
	tempOptions:{},
	footerHtml:'<span style="font-size:.9em;">'+IkaTools.getText('scriptOptions','footer','Note: You may need to refresh the page to see changes.')+'</span>',
	reloadOnSave:false,
	...
	show:function(callback) {
		...
		if(typeof(Config.styleDrawn) == 'undefined') {				// apply styling
		  try {var left = ($('#container2').offset().left+50) + 'px';}catch(e){var left = '10%';}
			GM_addStyle('\
					#ConfigMask { position:absolute; width:100%; top:0; left:0; height:100%; background-color:#000; opacity:.7; z-index:9000; } \
					#ConfigBody * { border:none; font-size:12px; color:#333; font-weight:normal !important; margin:0 !important; padding:0 !important; background:none; text-decoration:none; font-family:Helvetica Neue,Arial,Helvetica,sans-serif; line-height:1.2em; text-align:'+IkaRTL.txtLeft+';} \
					#ConfigBody { width:900px; margin:auto !important; top:30px; position:fixed; left:'+left+'; text-align:'+IkaRTL.txtLeft+'; background:#f9f9f9; border:1px outset #333; padding:0 !important; font-family:Arial; font-size:14px; -moz-border-radius:5px; cursor:default; z-index:9010; color:#333; padding-bottom:1em !important; } \
					...
					#ConfigContentBox td.fieldLabel { text-align:'+IkaRTL.txtRight+' !important; padding-'+IkaRTL.txtRight+':.5em !important; font-weight:bold !important; }\
					...
					#ConfigHistory ul { margin-'+IkaRTL.txtLeft+':2em !important; } \
					#ConfigClose { float:'+IkaRTL.txtRight+'; cursor:pointer; height:14px; opacity:.5; } \
					#ConfigClose:hover { opacity:.9; } \
					#ConfigFooter { padding:1.5em 1em 0 !important; } \
					#ConfigFooter input { border:1px outset #666; padding:3px 5px 5px 20px !important; background:no-repeat 4px center #eee; -moz-border-radius:3px; cursor:pointer; float:'+IkaRTL.txtRight+'; margin-'+IkaRTL.txtLeft+':.5em !important; } \
					#ConfigFooter input:hover { background-color:#f9f9f9; } \
					#ConfigFooter select { border:1px inset #666; }\
					#ConfigContentBox #ConfigFieldTable { width:auto !important; margin-'+IkaRTL.txtLeft+':2em !important; }\
		...
		var noticeWrapper = document.createElement('div');
			...
			noticeWrapper.id = "ConfigBodyWrapper";
				...
				var html = '<h1>\
								...
								' + (typeof(Config.scriptName) == 'string' ? Config.scriptName + ' - ' : '') + ' ' + IkaTools.getText('scriptOptions', 'settings', "Settings") + '\
							</h1>\
				...
				var i = 0;
				var firstTabId = "";
				for(var item in Config.settings) {
					var id = 'configTab' + item.replace(/\s/g, '_');
					html += '<span id="' + id + '">' + ((Config.settings[item].label) ? Config.settings[item].label : item) + '</span>';
				...
				html += '<div id="ConfigFooter">\
							<input type="button" id="ConfigCancelButton" value="' + IkaTools.getText('scriptOptions','cancel','Cancel') + '" style="background-image:url(' + Config.icons.close + ')"/>\
							<input type="button" id="ConfigCloseButton" value="' + IkaTools.getText('scriptOptions','save','Save') + '" style="background-image:url(' + Config.icons.save + ')"/>\
							' + Config.footerHtml + '\
						</div>';
	...
	activateTab:function(id) {
		...
		if(id == "configTabAbout" && typeof(ScriptUpdater) == 'object' && typeof(ScriptUpdater.scriptId) != 'undefined') {
			html += '<p><br/><a href="javascript:void(0)" id="ConfigCheckUpdatesLink">'+IkaTools.getText('scriptOptions','CheckUpdates','Check for updates')+'</a></p>';
		}
