// ==UserScript==
// @name           Description-of-the-villages-tribalwars-plapl.com
// @namespace      none
// @description    KDNتدوين وصف صغير في القرى / to take short village notes / 
// @version        1.03 - 200907310615
// @author         Peety, Simon Hilz (map-routine)
// @include        http://*.*tribal*.*/game.php?*screen=map*
// @include        http://*.*tribal*.*/game.php?*screen=info_village&id=*
// ==/UserScript==
...
function options(pos, id) {					// Optionen anzeigen
	...
	// Textcolor
		...
		td1text = document.createTextNode("لون الخط:");
	...
	// Icons
		...
		td4.innerHTML = "<style=\"vertical-align:middle;\">أيقونة: <img src="+bild[ein[2]]+" alt=\"\">";
		...
		for (i = 1; i < bild.length; i++) {
			...
			icon[i].addEventListener("click",function() {
				ein[2] = this.value; 
				set_info(id, ein[0], ein[1], ein[2], ein[3]);
				td4.innerHTML = "<style=\"vertical-align:middle;\">أيقونة: ";
				if (ein[2] > 1) {
...
function del_notizen(server) {				// alle gespeicherten Notizen einer Welt löschen
	...
	liste = GM_listValues();
	var sicher;
	sicher = window.confirm("في اختيارك لي هدا الخيار ,سا يتم حدف جميع الملاحضات في هدا العالم ,لي جميع القرى ,هل انته متاكد من هدا الفعل؟\n\n\n www.plapl.com");
	if (sicher) {
