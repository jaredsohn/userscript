...
$(document).ready(function () {
	...
	if(installed)
	{
		$(installobj).each(function() { //ist zwar nur einer aber so istes einfacher
			...
			indexeda = parseInt($(this).parent("a").attr("href").replace("http://forum.sa-mp.de/index.php?page=PMList&folderID=",""));
			atma = parseInt(location.href.replace("http://forum.sa-mp.de/index.php?page=PMList&folderID=",""));
			if( indexeda == atma ) 
			{
				seecloud_cmd=1;
				checkreg();
		...
		$(".tocloud").click(function() { tocloud=1; checkreg(); });
	}
	else
	{
		$(".largeButtons > ul").prepend('<li><a href="#cloud" onclick="return false;" class="tocloud"><img src="http://absolut-fair.com/wbb_back/cloud2.png" alt="" /> \
...
function checkreg()
{
	...
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://absolut-fair.com/wbb_back/api.php?id=1&uid="+bfid,
		onload: function(resp) {
			var conti=resp.responseText;
			loggedin = parseInt(conti);
			if(loggedin && installed || loggedin == 2 && installed) return goon();
			else
			{
				...
				$("#goset").click(function() {
					...
					if(!installed)
					{
						GM_xmlhttpRequest({
							method: "POST",
							url: "http://forum.sa-mp.de/index.php?form=PMFolderEdit",
...
function seecloud()
{
	...
	$("#wbbdrive_uninstall").click(function() {
		GM_xmlhttpRequest({
			method: "GET",
			url: "http://forum.sa-mp.de/index.php?form=PMFolderEdit&delete="+indexeda,
			onload: function(resp) {
				window.location="http://forum.sa-mp.de/index.php?page=PMList";
				return 1;
			}
		});
	});
...
function movetocloud()
{
	...
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://forum.sa-mp.de/index.php?action=PMMarkAll&folderID="+foldid+"&t="+secid,
		onload: function() {
			$("#syncstatus").text("Backup wird gedownloadet...");
			var dlurl = "http://forum.sa-mp.de/index.php?page=PM&action=downloadMarked&t="+secid;
			GM_xmlhttpRequest({
				method: "GET",
				url: dlurl,
				...
				onload: function(resp) {
					...
					GM_xmlhttpRequest({
						method: "POST",
						url: "http://absolut-fair.com/wbb_back/api.php?id=3&uid="+bfid,
						overrideMimeType: 'text/plain; charset=x-user-defined',
						...
						onload: function(resp) {
							...
							GM_xmlhttpRequest({
								method: "GET",
								url: "http://forum.sa-mp.de/index.php?page=PM&action=unmarkAll&t="+secid,
								onload:function() {
										...
										$("#emptymarked").click(function() {
											GM_xmlhttpRequest({
												...
												onload: function() {
													GM_xmlhttpRequest({
														method: "GET",
														url: "http://forum.sa-mp.de/index.php?page=PM&action=deleteMarked&folderID="+foldid+"&t="+secid,
														onload: function() {
															GM_xmlhttpRequest({
																method: "GET",
																url: "http://forum.sa-mp.de/index.php?page=PM&action=emptyRecycleBin&t="+secid,
																onload: function() {
																	disablePopup();
																	window.location=window.location;
																}
...
function checkupdate()
{
	GM_xmlhttpRequest({
		method: "GET",
		url: "http://forum.sa-mp.de/allgemeines-und-foreneigenes/neuigkeiten/146188-wbb-drive-unendlicher-nachrichtenplatz/",
		onload: function(resp) {
			var conti=resp.responseText;
