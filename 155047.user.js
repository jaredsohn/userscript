// ==UserScript==
// @name        Boxz Price History Image修改版
...
function google_chart(response) {
    ...
    if (img_node === null) {
        ...
        image_node.target = '_blank';
        image_node.appendChild(img_node);
    }
    return image_node;
}
function create_xitie_history_image_node(response) {
    var temp_document, img_node, img_node_src, image_node;
    temp_document = document.createElement('html');
    temp_document.innerHTML = response.responseText;
    img_node = temp_document.querySelector("#bk img");
    if (img_node === null) {
        image_node = document.createElement('p');
        image_node.innerHTML = '这个产品貌似没有历史价格数据，<a href="' + response.finalUrl + '">查看链接</a>。';
    } else {
        img_node.src = img_node.src.replace("item.taobao.com", "www.xitie.com");
        img_node.src = img_node.src.replace("detail.tmall.com", "www.xitie.com");
        img_node.style.marginTop = "10px";
        img_node.style.marginBottom = "10px";
        // 加上链接
        image_node = document.createElement('a');
...
function create_xitie_history_url(prefix, product_uid) {
    var detail_url = "http://www.xitie.com/" + prefix + ".php?no=" + product_uid;
    var chart_url = null;
...
var sites = [{
    ...
    get_history_url: function() {
        var reg, product_uid, history_url;
        reg = new RegExp("http://item.taobao.com/item.htm\\?\(.*&\)?id=\(\\d+\)");
        product_uid = url.match(reg)[2];
        history_url = create_xitie_history_url('tmall', product_uid);
        return history_url;
    },
    request_callback: function(response) {
        var image_node, place_node;
        image_node = create_xitie_history_image_node(response);
        place_node = document.getElementById('detail');
        place_node.parentNode.insertBefore(image_node, place_node.nextElementSibling);
    }
}, {
    domain : 'tmall.com',
    get_history_url: function() {
        var reg, product_uid, history_url;
        reg = new RegExp("http://detail\\.tmall\\.com/item.htm?.*?id=(\\d+)");
        product_uid = url.match(reg)[1];
        history_url = create_xitie_history_url('tmall', product_uid);
        return history_url;
    },
