...
// @name        cashons
// @namespace   cashons
// @include     *://*cashons.com/index.php?view=account&ac=earn&sid=*
// @include     *://*cashons.com/gpt.php?v=entry*
// @include     *://*cashons.com/gpt.php?v=rewardzone*
// @require     http://userscripts.org/scripts/source/159898.user.js?ver=1
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link.com/*
// @include     *://*oc.us/*
...
if(wparent.location.href.indexOf('index.php?view=click') != -1 || wparent.location.href.indexOf('index.php?view=account') != -1 && top == self){
	var arr = [],ctr = 0;
	var div = $('<div>');
	var clickNum = ctr + 1 ;
	var loading = 0;
	if(wparent.location.href.indexOf('www.cashons.com') >= 0 && wparent.name == 'janbee'){
	...
	$.each($('table *'),function(k,v){
		...
		if(thisItem.attr('href')){
			...
			if(thisItem.attr('href').indexOf('gpt.php?v=entry') >= 0){
	...
	function rec(ctr){
		loading = 0;
		if(arr[ctr]){
			var adsWind = wparent.open(arr[ctr].href,"janbee","width=100,height=50,top=1000,left=20000");	
...
if(wparent.location.href.indexOf('gpt.php?v=entry') != -1 || wparent.location.href.indexOf('gpt.php?v=rewardzone') != -1 && self == top){
window.addEventListener('beforeunload', function(e){ return false; }, true);
	
	
	if(wparent.location.href.indexOf('gpt.php?v=rewardzone') >= 0){
		
		window.claimReward(Math.floor((Math.random()*4)));
	...
	if($('frame:last').attr('src').indexOf('www.cashons.com') >= 0 && wparent.name == 'janbee'){
		wparent.opener.success('');
		wparent.close();
	}
	if($('frame:last').attr('src').indexOf('cheat') >= 0 ){
		var cheatCheck = [41,45,35,48];//1;
