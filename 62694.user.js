...
if (!initialized) {
  ...
  staminaSpendChoices[STAMINA_HOW_FIGHT_RANDOM] = '攻擊隨機的對手';
  staminaSpendChoices[STAMINA_HOW_FIGHT_LIST]   = '攻擊指定的對手';
  staminaSpendChoices[STAMINA_HOW_HITMAN]       = '暗殺懸賞名單上的對手';
  ...
  bountySelectionChoices[BOUNTY_SHORTEST_TIME]  = '最短的時間';
  bountySelectionChoices[BOUNTY_LONGEST_TIME]   = '最長的時間';
  bountySelectionChoices[BOUNTY_HIGHEST_BOUNTY] = '最高獎金';
  bountySelectionChoices[BOUNTY_RANDOM]         = '無偏好（隨機）';
  ...
  var autoStatDescrips  = ['等級', '攻擊', '防禦', '血量', '能量', '耐力', '勢力'];
  ...
  var cities = new Array(
    ...
    ['Cuba', undefined, 35, cashCubaIcon, 'cashCuba Icon', 'autoBankCuba', 'bankConfigCuba', 'autoSellCrates'],
    ['Moscow', undefined, 70, cashMoscowIcon, 'cashMoscow Icon', 'autoBankMoscow', 'bankConfigMoscow', 'autoSellCratesMoscow']
  )
  ...
  var missions = new Array(
    ...
    ['Use The Guard\'s Keys To Access the Bank Armor',105,35,3,MOSCOW,140],
    ...
    ['Hunt Down A Ural Liberation Front Contact',216,63,5,MOSCOW,231],
    ...
    ['Start An Avalanche Above The Terrorist Camp',146,72,5,MOSCOW,205],
    ...
    ['Gun Down Some Russian Muscle',239,76,6,MOSCOW,258],
    ['Attack A Mafiya Business',137,77,6,MOSCOW,188],
    ['Hijack A Mafiya Cargo',135,78,6,MOSCOW,179],
    ['Threaten A Mafiya Moneyman\'s Family',128,79,6,MOSCOW,176],
    ['Burn Down A Vory Safehouse',137,80,6,MOSCOW,188],
    ['Hit A Vory Nightclub',127,81,6,MOSCOW,171],
    ...
    ['Assault The Mansion Walls',148,85,6,MOSCOW,212]
  );
...
Animate.prototype.clearTimeout = function() {
  if (this.TOUT) {
    DEBUG('結算 ' + this.desc + ' 計時器 ' + this.TOUT + '.');
...
Animate.prototype.setTimeout = function(fx, delay) {
  ...
  DEBUG('開始 ' + this.desc + ' 計時器 ' + this.TOUT +
        ', 延遲=' + delay/1000 + ' 秒。');
}
...
Animate.prototype.start = function() {
  if (running && settingsOpen === false) {
    ...
    DEBUG('Autoplayer暫停。沒有開始 ' + this.desc + ' 計時器.');
  }
}
...
function doAutoPlay () {
  ...
  if (running &&
      ...
      health < maxHealth &&
      (health > 19 || (autoStamBurnif && stamina > 0) || canForceHeal())) {
    autoHeal();
    return;
  }
  ...
  if (GM_getValue('burnFirst') == 'checked' && !maxed && (autoMissionif || autoStaminaSpendif)) {
  ...
  // Racketing
  if (running && !maxed && GM_getValue('racketCollect') == 'checked' && ((new Date() >= new Date(GM_getValue('nextRacket', 0))) || GM_getValue('nextRacket', 0) == 0)) {
    if (racketCollect()) return;
  }
  ...
  if (running && !maxed) {
    for (var i = 0, iLength = cities.length; i < iLength; ++i) {
      if (level >= cities[i][CITY_LEVEL] &&
          GM_getValue(cities[i][CITY_SELLCRATES]) == 'checked' &&
  ...
  // Auto-bank
  if (running &&
      GM_getValue(cities[city][CITY_AUTOBANK]) == 'checked' &&
  ...
  // Auto-giftwaiting
  if (running && GM_getValue('autoGiftWaiting')) {
  ...
  // Auto-war
  if (running && ((GM_getValue('autoWar') == 'checked') || (GM_getValue('autoWarResponse') == 'checked') || (GM_getValue('autoWarReward') == 'checked'))) {
    if (autoWar()) return;
  }
  ...
  if (running && idle && GM_getValue('idleInCity') == 'checked' && city != GM_getValue('idleLocation', NY)) {
...
function autoSendEnergyPack() {
  ...
  if (sendPackButton) {
    ...
    addToLog('info Icon','你發送能量包給你的Mafia。');
    return true;
  } else {
...
function racketCollect() {
  ...
  if (!onPropertyNav()) {
    if (onRacketNav()) {
      ...
      //<a style="float: none;" class="sexy_button"><span class="sexy_influence">Shake Down Again</span></a></div>
      if(GM_getValue('racketReshakedown')=='checked') {
        ...
        if(elt) {
          ...
          if(mastered && GM_getValue('racketPermanentShakedown')!='checked') {
...
function autoStat() {
  ...
  if (maxPtDiff <= 0 && GM_getValue('autoStatDisable') == 'checked') {
  ...
  switch (statIndex) {
    ...
    case STAMINA_STAT   : link += '&upgrade_key=max_stamina';    break;
    default             :
...
function canMission() {
  ...
  if (GM_getValue('multipleJobs') == 'checked' &&
    ...
    if (singleJobLevelUp.length > 0) {
      ...
      if (GM_getValue('endLevelOptimize', 0) == 'checked') {
...
function autoFight(how) {
  ...
  if (autoFight.profileSearch && onProfileNav()) {
    ...
    opponent.attack = xpathFirst('.//a[contains(@href, "xw_controller=fight")]', innerPageElt);
    // FIXME: Should update this player in its list so we don't have to visit
    //        the profile again next time.
  ...
  if (!opponent.attack) {
    ...
    if (!onProfileNav()) {
      ...
      Autoplay.fx = goProfileNav(opponent.id);
      Autoplay.start();
      return true;
    }
  }
  ...
  // Attack!
  if (!opponent.attack) {
    DEBUG('No way to attack opponent (' + opponent.id + ').');
    return false;
  }
  Autoplay.fx = function() {
    clickAction = 'fight';
...
function getDisplayedOpponents(element, forceRefresh) {
  ...
  DEBUG(getDisplayedOpponents.opponents.length + ' 對手列出。');
...
function findFightOpponent(element) {
  ...
  var a = [];
  a = a.concat(newOpponents, fightListAvoid.get());
  if (GM_getValue('fightStealth') == 'checked' || newOpponents.length) {
    DEBUG(fightListInactive.debug());
    DEBUG(fightListActive.debug());
    a = a.concat(fightListInactive.get(), fightListActive.get());
  }
  var blacklist = new PlayerList();
  blacklist.set(a);
  ...
  for(var i = 0, iLength=opponents.length; i < iLength; ++i) {
    ...
    if (!opponent.id) continue;
    if (blacklist.indexOf(opponent) != -1) {
      blacklistCount++;
      continue;
    }

    // This opponent is acceptable.
    DEBUG('Found new fight opponent: name=' + opponent.name +
          ', id=' + opponent.id + ', level=' + opponent.level +
          ', mafia=' + opponent.mafia);
    fightListNew.add(opponent);
  }
...
function handleVersionChange() {
  ...
  if (GM_getValue('hideInHospital') == undefined) {
    ...
    if (GM_getValue('forceHealOpt2') == 'checked')
...
*/
  ...
  saveCheckBoxElementArray(['autoClick','autoLog','logPlayerUpdates','hideAttacks',
                            'autoMission','autoBank','waitForFull', 'autoBankMoscow',
                            ...
                            'autoBuy','autoSellCrates','autoEnergyPack',
                            ...
                            'autoWar','autoWarResponse','autoWarReward','autoWarPublish',
                            'autoWarResponsePublish','autoWarRewardPublish','autoGiftWaiting',
                            'burnFirst','autoLottoBonus']);
  ...
  GM_setValue('autoWarTarget', document.getElementById('autoWarTarget').value);
...
function updateMastheadMenu() {
  ...
  if (running) {
    ...
    elt.appendChild(document.createTextNode('暫停執行'));
    ...
    elt.appendChild(document.createTextNode('重新啟動'));
    menuElt.insertBefore(elt, menuElt.firstChild);
...
function pause() {
  ...
  addToLog('pause Icon', 'Autoplayer已暫停。日誌與統計手冊不跟踪活動。');
  updateMastheadMenu();
}
...
function unPause() {
  ...
  addToLog('play Icon', 'Autoplayer恢復 ...');
  updateMastheadMenu();
...
function isLoggable(line) {
  ...
  if (!line || debug || GM_getValue('filterLog') != 'checked' || isNaN(filterOpt))
    return true;
...
function debugOnOff() {
  ...
  if (GM_getValue('enableDebug') == 'checked') {
    addToLog('info Icon', '調試日誌記錄禁用。');
    GM_setValue('enableDebug', 0);
    debug = false;
    if (GM_getValue('logOpen') != 'open') {
      alert('Debug logging disabled.');
    } else if (debugElt) {
      debugElt.style.color = 'rgb(100, 0, 0)';
    }
  } else {
    GM_setValue('enableDebug', 'checked');
    debug = true;
    showMafiaLogBox();
    addToLog('info Icon', '啟用調試日誌記錄。');
    if (debugElt) {
      debugElt.style.color = 'rgb(255, 0, 0)';
    }
    debugDumpSettings();
    checkLanguage();
  }
...
function createLogBox() {
  ...
  var logClrButton = makeElement('div', mafiaLogBox, {'class':'mouseunderline', 'style':'position: absolute; left: 5px; top: 0px; font-weight: 600; cursor: pointer; color: rgb(255, 217, 39);'});
    logClrButton.appendChild(document.createTextNode('清除記錄'));
  ...
  var logClrStatsButton = makeElement('div', mafiaLogBox, {'class':'mouseunderline', 'style':'position: absolute; left: 85px; top: 0px; font-weight: 600; cursor: pointer; color: rgb(255, 217, 39);'});
    logClrStatsButton.appendChild(document.createTextNode('明確統計'));
  ...
  var closeLogButton = makeElement('div', mafiaLogBox, {'class':'mouseunderline', 'style':'position: absolute; right: 5px; top: 0px; font-weight: 600; cursor: pointer; color: rgb(255, 217, 39);'});
    closeLogButton.appendChild(document.createTextNode('關閉'));
    closeLogButton.addEventListener('click', hideMafiaLogBox, false);

  var title = 'Click to toggle debug log';
  var debugElt = makeElement('div', mafiaLogBox, {'class':'mouseunderline', 'title':title,'id':'ap_debug_log', 'style':'position: absolute; right: 80px; top: 0px; font-weight: 600; cursor: pointer; color: rgb(100, 0, 0);'});
    debugElt.appendChild(document.createTextNode('調試'));
    debugElt.addEventListener('click', debugOnOff, false);

  if (debug) debugElt.style.color = 'rgb(255, 0, 0)';
...
function createSettingsBox() {
  ...
  var tabNav = makeElement('div', elt, {'id':'tabNav', 'style':'position: static; display: inline-block; background: transparent url(' + stripURI(redBgImage) + ') repeat-x scroll 0 0; border: 1px solid #FFFFFF; fontsize: 13px; line-height: 28px; height: 30px;'});
    var generalTabLink = makeElement('div', tabNav, {'class':'selected'});
      makeElement('a', generalTabLink, {'href':'#', 'rel':'generalTab'}).appendChild(document.createTextNode('一般選項'));
    var displayTabLink = makeElement('div', tabNav, {'class':'selected'});
      makeElement('a', displayTabLink, {'href':'#', 'rel':'displayTab'}).appendChild(document.createTextNode('顯示'));
    var socialTabLink = makeElement('div', tabNav, {'class':'selected'});
      makeElement('a', socialTabLink, {'href':'#', 'rel':'socialTab'}).appendChild(document.createTextNode('社交活動'));
    var skillsTabLink = makeElement('div', tabNav);
      makeElement('a', skillsTabLink, {'href':'#', 'rel':'skillsTab'}).appendChild(document.createTextNode('技能'));
    var energyTabLink = makeElement('div', tabNav);
      makeElement('a', energyTabLink, {'href':'#', 'rel':'energyTab'}).appendChild(document.createTextNode('能量'));
    var staminaTabLink = makeElement('div', tabNav);
      makeElement('a', staminaTabLink, {'href':'#', 'rel':'staminaTab'}).appendChild(document.createTextNode('耐力'));
    var investmentTabLink = makeElement('div', tabNav);
      makeElement('a', investmentTabLink, {'href':'#', 'rel':'investmentTab'}).appendChild(document.createTextNode('投資'));
    var aboutTabLink = makeElement('div', tabNav);
      makeElement('a', aboutTabLink, {'href':'#', 'rel':'aboutTab'}).appendChild(document.createTextNode('關於'));
  ...
  makeElement('button', saveButton).appendChild(document.createTextNode('儲存設定'));
  saveButton.addEventListener('click', saveSettings, false);

  // Create Help button
  var helpButton = makeElement('span', settingsBox, {'class':'sexy_button', 'style':'left: 260px; bottom: 10px;'});
  makeElement('button', helpButton).appendChild(document.createTextNode('幫助'));
  ...
  makeElement('button', updateButton).appendChild(document.createTextNode('檢查更新'));
...
function createGeneralTab() {
  ...
  makeElement('label', lhs, {'for':id}).appendChild(document.createTextNode(' 啟用自動暫停:'));
  ...
  makeElement('br', item, {'class':'hide'});
  id = 'autoPauseExp';
  lhs.appendChild(document.createTextNode('升級經驗剩餘多少暫停:'));
  ...
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 治療在:'));
  id = 'healLocation';
  ...
  makeElement('label', rhs, {'title':title}).appendChild(document.createTextNode(' 當低於血量 '));
  ...
  makeElement('label', rhs, {'title':title}).appendChild(document.createTextNode(' 點'));
  ...
  title = hideInHosp.checked ? ' Hide in hospital but...' : ' 隱藏在醫院';
  ...
  makeElement('label', lhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 空閒時, 飛往:'));
  ...
  id = 'burnFirst';
  makeElement('input', lhs, {'style':'vertical-align: middle;','type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 使用全部:'));
  ...
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 優先,如果兩者都沒達到最大值'));
...
function createDisplayTab() {
  ...
  title = '確認啟用日誌記錄';
  makeElement('input', lhs, {'style':'vertical-align: middle;','type':'checkbox', 'id':id, 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 啟用日誌記錄:'));
  id = 'autoLogLength';
  makeElement('input', rhs, {'type':'text', 'id':id, 'value':GM_getValue(id, '300'), 'size':'2', 'style':'text-align: left'});
  rhs.appendChild(document.createTextNode(' (最大)信息於日誌'));
  ...
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 日誌播放更新:'));
  id = 'logPlayerUpdatesMax';
  makeElement('input', rhs, {'type':'text', 'id':id, 'value':GM_getValue(id, '25'), 'size':'2', 'style':'text-align: left'});
  rhs.appendChild(document.createTextNode(' (最大)的更新'));
  ...
  makeElement('label', lhs, {'for':id}).appendChild(document.createTextNode(' 啟用日誌過濾:'));

  id = 'filterOpt';
  var filterOpt = makeElement('select', rhs, {'id':id});
  var filterOptions = ['接受模式','拒絕模式'];
  var choice;
  ...
  makeElement('br', item, {'class':'hide'});
  id = 'filterPatterns';
  var filterText = makeElement('textarea', rhs, {'style':'position: static; width: 15em; height: 8em;', 'id':id, 'title':'每個模式鍵入一個單獨的一行。'});
  filterText.appendChild(document.createTextNode(''));
  makeElement('br', rhs);
  makeElement('font', rhs, {'style':'font-size: small;'}).appendChild(document.createTextNode('輸入每個名稱模式在單獨的一行。'));
  ...
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 隱藏廣告'));
  ...
  title = '移動電子郵件選項至底部';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 移動電子郵件選項至底部'));
  ...
  title = '對齊遊戲到左邊';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 對齊遊戲到左邊'));
  ...
  title = '只顯示攻擊摘要';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id,'title':title}).appendChild(document.createTextNode(' 總結攻擊至播放器更新信息'));
...
function createSocialTab() {
  ...
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 要求任務協助在:'));
  title = '鍵入最低經驗要求的任務求助，或設0無最低.';
  id = 'autoAskJobHelpMinExp';
  makeElement('input', rhs, {'type':'text', 'value':GM_getValue(id, '0'), 'title':title, 'id':id, 'size':'2'});
  rhs.appendChild(document.createTextNode(' 最低經驗值'));

  var item = makeElement('div', list);
  var lhs = makeElement('div', item, {'class':'lhs'});
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '輸入的信息和任務求助一起發佈到你的塗鴉牆。如果你不要在任務求助內留信息就空白。';
  id = 'autoAskJobHelpMessage';
  lhs.appendChild(document.createTextNode('信息發佈:'));
  ...
  label.appendChild(document.createTextNode(' 接受 mafia 邀請'));
  ...
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 自動幫助任務'));
  ...
  makeElement('label', rhs, {'for':id}).appendChild(document.createTextNode(' 跳過塗鴉牆發佈的禮物'));
  ...
  id = 'autoGiftWaiting';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 自動點擊等待選擇的禮物'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  notificationStopTitle = '提醒其他用戶處理彈出窗口撤消通告。';
  notificationID = 'notificationHandle';
  var notificationLabel = makeElement('label', lhs);
  notificationLabel.appendChild(document.createTextNode('撤消的通告:'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '自動求救，如果有人對你發動戰爭';
  id = 'autoWarResponse';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 呼叫戰爭後援:'));

  // War - Publish call for back up
  title = '自動發佈求救給你的Mafia';
  id = 'autoWarResponsePublish';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 發佈呼救'));
  ...
  id = 'autoWar';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 自動宣戰: '));
  // War - autowar published
  title = '這場戰爭自動發布到您的Mafia這樣他們就可以幫助';
  id = 'autoWarPublish';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 發佈發動的戰爭 '));
  // War - autowar target
  id = 'autoWarTarget';
  title = '鍵入對手 facebook ID碼';
  makeElement('input', rhs, {'type':'text', 'style':'width: 80px; vertical-align: middle;', 'title':title, 'value':GM_getValue(id, '0'), 'id':id, 'size':'5'});

  // War - Auto-reward friends in war
  var item = makeElement('div', list);
  var lhs = makeElement('div', item, {'class':'lhs'});
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '自動獎勵那些幫助你贏得戰爭的人';
  id = 'autoWarReward';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align: middle', 'value':'checked'}, id);
  makeElement('label', lhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 在戰鬥中獎勵朋友: '));
  // War - reward published
  title = '自動發佈獎勵幫助你的Mafia';
  id = 'autoWarRewardPublish';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'value':'checked'}, id);
  makeElement('label', rhs, {'for':id, 'title':title}).appendChild(document.createTextNode(' 發佈獎勵'));
...
function createSkillsTab() {
  ...
  makeElement('label', autoStats, {'for':id, 'title':title, 'style':'vertical-align: middle;'}).appendChild(document.createTextNode(' 啟用自動加點'));
  ...
  for (var i = 0, iLength=autoStatModes.length; i < iLength; ++i ) {
    var title = '請選擇以哪項為基礎在 ' + autoStatDescrips[i + 1] + ' 加點';
  ...
  for (var i = 0, iLength=autoStatFallbacks.length; i < iLength; ++i ) {
    var title = '確認分發點數到 ' + autoStatDescrips[i + 1] + ' 當目標達成後';
    ...
    label.appendChild(document.createTextNode(' 使用 ' + autoStatDescrips[i+1] + ' 作為後備'));
    xTopCur += 25;
  }
  ...
  for (var i = 0, iLength=autoStatPrios.length; i < iLength; ++i ) {
    ...
    for (var j = 0, jLength=autoStatRatios.length; j < jLength; ++j) {
      var choice = document.createElement('option');
      choice.value = j;
      choice.appendChild(document.createTextNode('優先 ' + (j + 1)));
      sel.appendChild(choice);
    }
...
function createEnergyTab() {
  ...
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 使用能量:'));
  title = ' 每次做一項主要任務';
  id = 'masterAllJobs';
  label = makeElement('label', rhs, {'for':id, 'title':title});
  ...
  label.appendChild(document.createTextNode(title + ' '));
  var unChkAll = makeElement('input', label, {'id':'unCheckAll','style':'font-size: smaller; display : ' + ((GM_getValue('multipleJobs') == 'checked') ? '':'none'),'type':'button', 'value':'Uncheck All'});
  ...
  var handler = function() {
    if (multipleJobs.checked) {
      ...
      var labelText = '下一項主要的任務:';
      var title = '選擇下一份要掌握的任務。一旦掌握了，會自動挑選另一份任務。';
      selectMissionLabel.firstChild.nodeValue = labelText;
      selectMissionLabel.title = title;
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 當能量全滿時執行任務'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '估計你將要執行任務的平均經驗值到能量的比率。例如,一個任務它的經驗值10而需要5能量,那比率就是2。鍵入0,如果你喜歡不管浪費的使用能量。';
  id = 'estimateJobRatio';
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode('估計任務率:'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '如果你的人物類型是Maniac請點選 (如果不是Fearless(為啥google翻譯出來是霍元甲...) 或 Mogul).';
  id = 'isManiac';
  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align:middle', 'value':'checked'}, 'isManiac', 'checked');
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 人物類型是Maniac(瘋子!!@@")'));
  ...
  title = '確認這個道具你已經擁有',
  label = makeElement('label', lhs, {'title':title});
  label.appendChild(document.createTextNode('任務需使用的物品:'));
  title = '確認你已經擁有Helicopter,它來自掌握全部 Consigliere(顧問) 的任務。';
  id = 'hasHelicopter';
  ...
  title = '確認你已經擁有Private Island,它來自掌握全部 Underboss(小頭目) 的任務。';
  id = 'hasPrivateIsland';
  ...
  title = '確認你已經擁有Golden Throne,它來自掌握全部 Boss(頭目) 的任務。';
  id = 'hasGoldenThrone';
...
function createStaminaTab() {
  ...
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 使用耐力:'));
  ...
  id = 'selectStaminaUse';
  item.appendChild(document.createTextNode('使用耐力後當剩餘 '));
  ...
  item.appendChild(document.createTextNode(' 耐力給手動時使用。'));

  // Level up
  var item = makeElement('div', list, {'class':'single'});
  title = '忽略耐力最低設置,如果你即將要升級。(小月猜測應該是升級後這選項就會取消)';
  ...
  label.appendChild(document.createTextNode(' 在達到下個等級前不保留耐力'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  lhs.appendChild(document.createTextNode('戰鬥於:'));
  id = 'fightRandomLoc';
  ...
  id = 'fightLevelMax';
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode('最高等級:'));
  ...
  title = '讓最高的等級限制加上自己的等級。例如，如果您的等級為10，最高等級設置為 5，高於15的對手將可避免。';
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 加上自己等級'));
  ...
  makeElement('br', item, {'class':'hide'});
  id = 'fightMafiaMax';
  title = '避免攻擊到Mafia大於這個數的對手。',
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode('最大Mafia數:'));
  ...
  title = '讓最高的Mafia限制加上你自己的Mafia出場人數。例如，如果你有300個Mafia，最大Mafia設置為 50，Mafia超過 350的對手將可避免。';
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 加上自己 mafia 出場人數'));
  ...
  makeElement('br', item, {'class':'hide'});
  id = 'fightMafiaMin';
  title = '避免攻擊到Mafia小於這個數的對手。',
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode('最小Mafia數:'));
  ...
  // Maximum mafia relative?
  title = '根據自己的Mafia出場人數減去設定的人數。例如，如果你有300個Mafia，最低Mafia設置為 50，Mafia數少於250名的對手將可避免。';
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 用自己Mafia出場人數減去'));
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 避免攻擊Top Mafia保鏢'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  title = '避免攻擊名子包含特定的模式的隨機對手';
  id = 'fightAvoidNames';
  makeElement('input', lhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align:middle', 'value':'checked'}, 'fightAvoidNames', 0);
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 避開Mafia團體:'));
  makeElement('textarea', rhs, {'style':'position: static; width: 15em; height: 8em;', 'id':'fightClanName', 'title':'輸入每個模式（如Mafia家族名稱）在單獨一行。'}).appendChild(document.createTextNode(GM_getValue('clanName', defaultClans.join('\n'))));
  makeElement('br', rhs);
  makeElement('font', rhs, {'style':'font-size: small;'}).appendChild(document.createTextNode('輸入每個名稱模式在單獨的一行。'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  lhs.appendChild(document.createTextNode('攻擊這些對手'));
  makeElement('textarea', rhs, {'style':'position: static; width: 180px; height: 105px;', 'id':'fightList', 'title':'輸入每個對手的ID碼（不是他們的名字）在單獨一行。'}).appendChild(document.createTextNode(GM_getValue('fightList', '')));
  makeElement('br', rhs);
  makeElement('font', rhs, {'style':'font-size: small;'}).appendChild(document.createTextNode('輸入每個 Facebook的ID碼在單獨一行。'));
  ...
  label = makeElement('label', rhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 刪除強大的對手'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  lhs.appendChild(document.createTextNode('暗殺懸賞在:'));
  id = 'hitmanLoc';
  ...
  title = '忽略低於這個金額的懸賞目標。',
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode('最低獎金:'));
  ...
  var rhs = makeElement('div', item, {'class':'rhs'});
  makeElement('br', item, {'class':'hide'});
  lhs.appendChild(document.createTextNode('選擇對目標的偏好:'));
  id = 'bountySelection';
  ...
  label = makeElement('label', lhs, {'for':id, 'title':title});
  label.appendChild(document.createTextNode(' 避開Mafia團體:'));
  makeElement('textarea', rhs, {'style':'position: static; width: 15em; height: 8em;', 'id':'hitmanClanName', 'title':'輸入每個模式（如Mafia家族名稱）在單獨一行。'}).appendChild(document.createTextNode(GM_getValue('clanName', defaultClans.join('\n'))));;
  makeElement('br', rhs);
  makeElement('font', rhs, {'style':'font-size: small;'}).appendChild(document.createTextNode('輸入每個名稱模式在單獨的一行。'));
...
function createInvestmentTab () {
  ...
  autoBuy.appendChild(document.createTextNode('啟用自動購買'));
  ...
  selectPropertiesTitle.appendChild(document.createTextNode('選擇你想要購買的物件:'));
  ...
  selectPropertiesNote.appendChild(document.createTextNode('* 為不會被搶劫的屬性'));

  title = '不會花費低於這個數額的現金(小月猜測只要低於這個金額就不會被花掉)';
  var buyMinAmount = makeElement('div', investmentTab, {'style':'top: 50px; right: 10px;'});
  buyMinAmount.appendChild(document.createTextNode('最低現金: '));
  ...
  autoSellCrates.appendChild(document.createTextNode('售出Cuba的事業產出'));
  ...
  autoSellCratesMoscow.appendChild(document.createTextNode('售出Moscow事業產出'));
  ...
  collectNYTake.appendChild(document.createTextNode('自動收取紐約3小時的產出'));
  ...
  racketCollect.appendChild(document.createTextNode('自動收取 racket'));
  ...
  racketReshakedown.appendChild(document.createTextNode('再一次Shake down'));
  ...
  racketPermanentShakedown.appendChild(document.createTextNode('永久再一次Shake down'));
  ...
  for (var i = 0, iLength = cities.length; i < iLength; ++i) {
    ...
    title = '存入 ' + cities[i][CITY_NAME] + ' 銀行 ';
...
function createAboutTab () {
  ...
  devs.appendChild(document.createTextNode('貢獻者:'));
  ...
  var devNames = ['StevenD', 'CharlesD', 'Eric Ortego', 'Jeremy', 'Liquidor', 'AK17710N', 'Fragger',
                 ...
                 'Cam', 'ChrisMSB', 'vmzildjian', 'Scrotal', 'Bushdaka','中文化 : 飛小月'];
...
function handleModificationTimer() {
  ...
  if (!xpathFirst('./div[@id="ap_inner"]', innerPageElt)) {
    ...
    setListenContent(true);
    DEBUG('新設計的內頁內容: ' + innerPageElt.id);
    pageChanged = true;
...
function handlePublishNotificationsInternal(e) {
  ...
  if (running && popup && popup.id == 'pop_content') {
  ...
  if (running && popup.parentNode &&
    ...
    if (skipPost) {
      ...
      // Gift post
      var giftCheck = xpathFirst('.//div[@class="UIStoryAttachment_Title" and contains(text(), "sent")]/a[contains(@href, "sendgiftshort")]', popup);
      ...
      // Job Help
      var jobHelpCheck = xpathFirst('.//div[@class="UIStoryAttachment_Title" and contains(text(),"requested help")]', popup);
      ...
      // War Reward
      var warRewardCheck = xpathFirst('.//div[@class="UIStoryAttachment_Title" and contains(text(),"rewarded their friends with")]', popup);
      if (warRewardCheck && GM_getValue('autoWarReward') == 'checked') {
        var msgElt = document.getElementById("feedform_user_message");
        var publishPost = xpathFirst('.//input[@value="Publish"]', popup);
        if (msgElt && publishPost && (GM_getValue('autoWarRewardPublish')=='checked')) {
          clickElement(publishPost);
      ...
      // War back up request
      var warHelpCheck = xpathFirst('.//div[@class="UIStoryAttachment_Title" and contains(text(),"needs help to win their War")]', popup);
      if (warHelpCheck && GM_getValue('autoWarResponse') == 'checked') {
        var msgElt = document.getElementById("feedform_user_message");
        var publishPost = xpathFirst('.//input[@value="Publish"]', popup);
        if (msgElt && publishPost && (GM_getValue('autoWarResponsePublish')=='checked')) {
          clickElement(publishPost);
      ...
      // War Declaration
      var warDeclareCheck = xpathFirst('.//div[@class="UIStoryAttachment_Title" and contains(text(),"needs your help to win a major battle")]', popup);
      if (warDeclareCheck && GM_getValue('autoWar') == 'checked') {
        var msgElt = document.getElementById("feedform_user_message");
        var publishPost = xpathFirst('.//input[@value="Publish"]', popup);
        if (msgElt && publishPost && (GM_getValue('autoWarPublish')=='checked')) {
          clickElement(publishPost);
...
function refreshSettings() {
  ...
  if (staminaFloor >= maxStamina) {
    // Subtract one or else fight/hit will never run.
...
function checkLanguage() {
  ...
  DEBUG('語言是 "' + document.documentElement.lang + '".');
  addToLog('warning Icon', '不幸的是，只有英文版本的遊戲完全支持。如果您遇到問題，請將您的Facebook的語言為英語，然後再試一次。');
}
...
function customizeMasthead() {
  ...
  lobjAutoPlay.appendChild(document.createTextNode('設定'));
  lobjAutoPlay.addEventListener('click', toggleSettings, false);

  // View log button.
  menuElt.appendChild(document.createTextNode(' | '));
  var lobjViewLogButton = makeElement('span', menuElt);
  lobjViewLogButton.appendChild(document.createTextNode('日誌'));
...
function customizeProfile() {
  ...
  if (statsDiv) {
    ...
    if (tabElt && tabElt.getAttribute('onclick').match(/user=(\d+)/)) {
      ...
      // Promote
      // FIXME: Does this still work? And if so, does it work on enemies?
      statsDiv.appendChild(document.createTextNode(' | '));
...
function jobLoot(element) {
  ...
  DEBUG('得到 ' + lootbag.length + ' 裝備在這個任務。');
...
function debugDumpSettings() {
  ...
  DEBUG('>  >  >  >  >  BEGIN SETTINGS DUMP  <  <  <  <  <<br>' +
        ...
        'Auto War Response Publish: <strong>' + showIfUnchecked(GM_getValue('autoWarResponsePublish'))  + '</strong><br>' +
        'Auto War Reward Publish: <strong>' + showIfUnchecked(GM_getValue('autoWarRewardPublish'))  + '</strong><br>' +
        'Auto War Publish: <strong>' + showIfUnchecked(GM_getValue('autoWarPublish'))  + '</strong><br>' +
        'Auto War Response: <strong>' + showIfUnchecked(GM_getValue('autoWarResponse'))  + '</strong><br>' +
        'Auto War Reward: <strong>' + showIfUnchecked(GM_getValue('autoWarReward'))  + '</strong><br>' +
        'Auto War: <strong>' + showIfUnchecked(GM_getValue('autoWar'))  + '</strong><br>' +
        'Auto War Target: <strong>' + GM_getValue('autoWarTarget', 0) + '</strong><br>' +
        '-------------------Skills Tab--------------------<br>' +
...
function autoLotto() {
  ...
  if (GM_getValue('autoLottoBonus', 0) == 'checked') {
...
function autoWar() {
  ...
  if (warRewardButton && (GM_getValue('autoWarReward') == 'checked')) {
    Autoplay.fx = function() {
      clickElement(warRewardButton);
      DEBUG('Clicked to reward my mafia.');
      addToLog('good Icon', '<span style="font-weight:bold;color:rgb(255,217,39);">War</span>: Rewarded my mafia!');
    };
    Autoplay.start();
    return true;
  }

  // Grab the button off the main page if it exists!
  // If we're supposed to call for help this is the only way we get to the page
  var warCheckButton = xpathFirst('.//table//tbody//tr//td[3]//a//span[contains(text(), "Check your war")]', innerPageElt);
  if (warCheckButton) {
    Autoplay.fx = function() {
      clickElement(warCheckButton);
      DEBUG('Clicked to check my war.');
    };
    Autoplay.start();
    return true;
  }
  ...
  DEBUG('Processing war page');

  // Check for a war that may already be under way!
  var warStatus = xpath('//*[@id="app10979261223_war_timer"]', innerPageElt);
  if (warStatus.snapshotItem(0)!=null) {
    warStatus = warStatus.snapshotItem(0).parentNode.innerHTML.untag().trim();
    if ((warStatus.indexOf("A new war can start in:") >= 0) || (warStatus.indexOf("War Ends in:") >= 0)) {
      // This is the time remaining for the running war.
      // We wont even come back to this routine until the war is over
      if (warStatus.indexOf("A new war can start in:") >= 0) {
        warStatus = warStatus.substr(warStatus.indexOf("A new war can start in:")+24,20);
        var warRunning = 0;
      } else {
        warStatus = warStatus.substr(warStatus.indexOf("War Ends in:")+13,20);
        var warRunning = 1;
      }
      var warStatusTmp = warStatus.split(' ');
      var warTimerTmp = warStatusTmp[0].split(':');
      // Ok, so MW doesn't keep the hours or minutes as they tick off. hh:mm:ss -> mm:ss -> ss
      var warMinuteAdd = 0;
      switch (warTimerTmp.length-1) {
        // ss
        case 0 :
                 // Only seconds left, don't add anything
                 break;
        // mm:ss
        case 1 :
                 // Minutes are first in the list
                 warMinuteAdd += parseInt(warTimerTmp[0]);
                 break;
        // hh:mm:ss
        case 2 :
                 // Hours are first in the list
                 warMinuteAdd += parseInt(warTimerTmp[0])*60+parseInt(warTimerTmp[1]);
                 break;
      }
      warMinuteAdd += 5;

      // Limit timer to 2 hours.
      if (warMinuteAdd > 120) setGMTime('warTimer', '2 hours');
      else setGMTime('warTimer', warMinuteAdd + ' minutes');

      // If the war is running, come back in 3 minutes
      if (warRunning) setGMTime('warTimer', '3 minutes');

      // We're on the page, does it have an 'Attack and Win' button?
      // This generates a popup for publishing, that's handled over in the PublishNotifications routines
      if (GM_getValue('autoWar') == 'checked') {
        var warAttackButton = xpathFirst('.//div//a[@class="sexy_button"]//span[contains(.,"Attack and Win")]', innerPageElt);
        if (warAttackButton) {
          Autoplay.fx = function() {
            clickElement(warAttackButton);
            DEBUG('Clicked the war attack button.');
            addToLog('good Icon', '<span style="font-weight:bold;color:rgb(255,217,39);">War</span>: Attacked for the Win!');
          };
          Autoplay.start();
      ...
      // This generates a popup for publishing, that's handled over in the PublishNotifications routines
      if (GM_getValue('autoWarReward') == 'checked') {
      ...
      // This generates a popup for publishing, that's handled over in the PublishNotifications routines
      if (GM_getValue('autoWarResponse') == 'checked') {
        ...
        if (warHelpMeButton) {
          Autoplay.fx = function() {
            ...
            addToLog('good Icon', '<span style="font-weight:bold;color:rgb(255,217,39);">War</span>: Getting help with my war!');
          };
          Autoplay.start();
          setGMTime('warTimer', '00:00');
          return true;
      ...
      // If we are here, then there's a war active, but there's nothing for us to do
      // Just come back when warTimer is up
      return false;
    } else {
      // If we get here, then the timer exists but we don't have the right verbage for it
      addToLog('warning Icon','<span style="font-weight:bold;color:rgb(255,0,0);">Error</span>: Could not get the war timer!');
      return false;
    }
  } else {
    // At this point we either start a war or return false
    DEBUG('No war found, processing options');

    // Check to see if we tried to start a war with an invalid target
    var warInvalidTarget = xpathFirst('.//table//tbody//tr//td[contains(text(), "Target user does not exist")]', innerPageElt);
    // /html/body/div[4]/div/div/div/div[2]/div/div/div[6]/div/div/table/tbody/tr[7]/td[2]/div/table/tbody/tr/td
    if (warInvalidTarget) {
      DEBUG('Invalid war target!');
      addToLog('warning Icon','<span style="font-weight:bold;color:rgb(255,0,0);">Error</span>: Invalid war target: ' + GM_getValue('autoWarTarget'));
      GM_setValue('autoWar',0);
      DEBUG('autoWar turned off due to invalid target!');
      return false;
    };

    // Are we supposed to start a war?
    if (GM_getValue('autoWar') == 'checked') {
      if (GM_getValue('autoWarTarget',0) == 0) {
        // If we're here, then the target is not set
        GM_setValue('autoWar',0);
        addToLog('warning Icon','<span style="font-weight:bold;color:rgb(255,0,0);">Error</span>: Invalid war target: ' + GM_getValue('autoWarTarget',0));
        DEBUG('autoWar turned off due to invalid autoWarTarget');
        return false;
      }

      // Ok, no war is going on right now, so let's start one
      ...
      // This generates a popup for publishing, that's handled over in the PublishNotifications routines
      // Since we specified a target we need to use a generated link
      document.location = 'http://apps.facebook.com/' + SCRIPT.name +
                        SCRIPT.controller + 'war' +
                        SCRIPT.action + 'declare_war' +
                        SCRIPT.city + (city + 1) +
                        '&target_id=' + GM_getValue('autoWarTarget');
      addToLog('good Icon', '<span style="font-weight:bold;color:rgb(255,217,39);">War</span>: Started war with: ' + GM_getValue('autoWarTarget'));
      // supposed to click an opponent to attack
    }

    // Nothing to do, come back after 3 minutes
    DEBUG('No war actions found.');
    setGMTime('warTimer', '3 minutes');
    return false;
  }
}
...
function goLocation(toCity) {
  ...
  var elt = xpathFirst('//div[@id="' + SCRIPT.appID + '_travel_menu"]//a[contains(., "' + cities[toCity][CITY_NAME] + '")]');
