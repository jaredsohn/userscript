...
// @author       Benoit485
// @version      0.4
// @date         2013-09-21 18H40
// @include      http://www.nucleagame.fr/uni*/game.php?page=buildings*
// @include      http://www.nucleagame.fr/uni*/game.php?page=research*
...
eval(ansa.initScript(
{
	...
	version: '0.4',
	url: 'http://userscripts.org/scripts/source/166902.user.js', 
	options: {}
}) );
...
if(ansa.url.get.match(/page=buildings/) || ansa.url.get.match(/page=research/))
{
	...
	var newTable = '';
	var newTableOk = '<tr><td colspan="6" style="background:none;border:0px;"><br><br></td></tr>';
	...
	for(i=0,j=trsPage.snapshotLength; i<j; i+=3)
	{
		...
		var newTrBat = '<tr><td><img onclick="'+onclickShowInfos+';" src="'+img+'" width="30px"></td><td>'+formUp+'</td>'
		              + '<td onclick="'+onclickShowInfos+';">'+nom+' (Lvl '+niveau+')</td><td>';
		if(tit!='0') newTrBat += '['+tit+']';
		if(carb!='0') newTrBat += (tit!='0' ? ' ' : '')+'['+carb+']';
		if(trit!='0') newTrBat += ((tit!='0' || carb!='0') ? ' ' : '')+'['+trit+']';
		if(energie!='0') newTrBat += ((tit!='0' || carb!='0' || trit!='0') ? ' ' : '')+'['+energie+']';
		newTrBat += '</td><td>Temps :<br>'+temps+'</td>';
		if(ansa.url.get.match(/page=buildings/) ) newTrBat += '<td>'+lienDetruire+'</td><tr>';
		
		if( notMaxlevel(nom, niveau) ) newTable += newTrBat;
		else newTableOk += newTrBat;
	}
	

	var tableBats = evalPath("id('content')/table"+(getId('infobox') ? '[2]':'')+"", document).snapshotItem(0);
	tableBats.innerHTML = newTable+newTableOk;
...
function notMaxlevel(nom, level)
{
	...
	var data =
	{
		'Mine de Métal' : 100,
		'Mine de Cristal' : 100,
		'Synthétiseur de deutérium' : 100,
		'Centrale électrique solaire' : 25,
		...
		'Réservoir de deutérium' : 12,
		'Laboratoire de recherche' : 15,
		'Terraformeur' : 10,
		'Dépôt de ravitaillement' : 10,
		'Silo de missiles' : 10
	}
