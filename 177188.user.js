...
// @description    Cross search for requests and snatches on What.CD and Waffles.fm. Small changes made by jondb: updated the what.cd search url, added date to search results, added what.cd button in waffles request page. Small changes made by andy0: a butcher fix for waffles searching what
// @author         jonls
		...
		whatcd_proxy.get(search_url, 'text/xml',
			function(response) {
				...
				if (m) {
					...
					var results = [];
					doc.find('group').each(function(i) {
						var result = {
					...
					*/
					...
					// andy0 butcher
					callback(search_url, true);
				} else {
					//callback(search_url, []);
					
					// andy0 butcher
					callback(search_url, false);
				}
			}
		);
	}
				...
				if (m) {
					...
					search_whatcd(whatcd_url_base, artist, request, function(search_url, results) {
						...
						if (results) {
							...
							result_span.html('Found something for (<i><a href="'+search_url+'">'+artist+' - '+request+'</a></i>)');
						} else {
							result_span.html('Nothing found (<i><a href="'+search_url+'">Refine search</a></i>)');
							result_div.slideUp(500);
							row.children('td').eq(0).click(function(event) { result_div.slideToggle(500); });
