...
// if any1 needs a simple way to calculate damage: atk*4+def+legionpower*5
	...
	window.SRDotDX = {
		...
		gui: {
			...
			ImportTabDelimitedRaids: function() {
				var linklist=document.FPXRaidImportExportForm.FPXImportExportRaidInput.value;
				var patt = new RegExp("http...www.kongregate.com.games.5thPlanetGames.dawn.of.the.dragons.[\\w\\d_=&]+\\s?", "ig");
				var link;

				while(match = patt.exec(linklist)) {
					if (match != null) {
						var link = match[0];
						(function(param1) { SRDotDX.getRaidDetails(param1,'');})(link);
					}
					
				}

				document.FPXRaidImportExportForm.FPXImportExportRaidInput.value="";
				...
				if (typeof holodeck == 'object' && typeof holodeck._tabs == 'object' && typeof holodeck._tabs.addTab == 'function') {
					...
					var pane = SRDotDX.gui.cHTML('div').set({id: 'lots_tab_pane'}).html(' \
						...
						<ul id="SRDotDX_tabpane_tabs"> \
							...
							<li class="tab"> \
								<div class="tab_head">Excel</div> \
								<div class="tab_pane"> \
									<div id="FPXRaidImportExportDiv"> \
										<FORM name="FPXRaidImportExportForm" onSubmit="return false;"> \
											<input name="Submit"  type="submit" tabIndex="-1" value="Import Raids" onClick="SRDotDX.gui.ImportTabDelimitedRaids();return false;"/><br> \
											<textarea id="FPXImportExportRaidTA" name="FPXImportExportRaidInput" cols="32" rows="30" ></textarea> \
										</FORM> \
									</div> \
								</div> \
							</li> \
