...
(function() {
GM_registerMenuCommand("Change XBMC IP, Apple Trailers",set_XBMC_IP);
...
function insertDownloadLink(movie_url, res) {
    ...
    if (res.indexOf('p') != -1) {
		...
		smilie = ""+res + "x"+(res*0.425);
    }
    
    inner_html = ''+
		'<a title="'+title+'" href="http://'+XBMC_IP+'/xbmcCmds/xbmcHttp?command=playfile&parameter='+movie_url+'" ' +
		'style="color:#333;font-weight:900;font-size:8pt;'+
		'text-decoration:none;" onclick="return false;"\'>' +
		smilie + '</a>';
    var li = document.createElement("li");
    li.innerHTML = inner_html;
	li.addEventListener('click',function(){playURL('http://'+XBMC_IP+'/xbmcCmds/xbmcHttp?command=playfile&parameter='+movie_url)},false);
...
function getDownloadLink() {
    ...
    // new default trailer page: 
	var nobjects = xpath(document, "//a[@class='large' or @class='small' or @class='medium' or @class='hd']");
    ...
    // pass 1:
    nobjects = xpath(document, "//object/embed[@height > 100]");
        ...
        onload: function(responseDetails) {
            ...
            if (path_default.indexOf(".mov") != (path_default.length-4)) {
                ...
                if (this.url.indexOf("-h.ref.mov") == -1) {
                    ...
                    res = path_default.replace(/.*([0-9]{4}.?)\.mov/, "$1");
                    if (res == path_default) {
                        res = path_default.replace(/.*([0-9]{3}.?).*/, "$1");
                    }
