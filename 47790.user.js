// ==UserScript==
// @name          YouTube+
// @description   Adds control panel, light effect, downloading and more stufffor youtube.
// @include       http://*.youtube.com/watch*
// @include       http://youtube.com/watch*
// @include	  http://*youtube.com/*v=*
// @copyright     Didrik Hansen
// @version       4.27
// @license       Creative Commons Attribution-Noncommercial 7.3 Norway License
// ==/UserScript==
...
my_banner.innerHTML = '<div style="border-bottom: 1px solid #CCCCCC; margin-bottom: 10px; font-size: small; background-color: #FF0000; color: #FFFFFF;">' +
    ...
    '<a style="color:#FFFFFF; font-weight:bold; font-size:10px;">"Powered by : Didrik Hansen"</a>' +
    '</p></div>';
...
save_as.setAttribute('style', 'color: #FF0000;');
save_as.textContent = "  ";
...
function addLightsButton()
{
	...
	imgLightsButton.setAttribute('title', 'Lights Out');
}
...
function lightsOut()
{
	...
	if(!document.getElementById('lightsOut')) {
		...
		imgLightsOut.setAttribute('style', 'position:absolute;top:0;left:0;width:100%;height:' + document.height + 'px;z-index:100;');
...
function uncheckAll() {
var ops = xp("//input[@type='checkbox']");
if(ops) {
for(var i=0; i<ops.snapshotLength; i++) {
ops.snapshotItem(i).checked = false;
}
return true;
}
else { return false; }
...
if(ops) {
for(var i=0; i<ops.snapshotLength; i++) {
ops.snapshotItem(i).checked = true;
}
return true;
}
else { return false; }
...
n = t == 'object' ? id_node : document.getElementById(id_node);
...
function uncheck(unchecked_name)  {
if(unchecked_name) {
id(unchecked_name).checked = false;
return true;
}
else { return false; }
}

function checked(checked_name) {
if(id(checked_name)) { return id(checked_name).checked; }
else { return false; }
}
...
if(id(hide_id)) {
if(parent) {
if(parent==1) { id(hide_id).parentNode.style.display = "none"; }
if(parent==2) { id(hide_id).parentNode.parentNode.style.display = "none"; }
if(parent==3) { id(hide_id).parentNode.parentNode.parentNode.style.display = "none"; }
}
else {
id(hide_id).style.display = "none";
}
return true;
}
else { return false; }
...
if(id(show_id)) {
if(parent==1) { id(show_id).parentNode.style.display = ""; }
else if(parent==2) { id(show_id).parentNode.parentNode.style.display = ""; }
else if(parent==3) { id(show_id).parentNode.parentNode.parentNode.style.display = ""; }
else { id(show_id).style.display = ""; }
return true;
}
else { return false; }
}
...
function customize() {
var hide_ads = GM_getValue("ads", false);
var hide_embed_form = GM_getValue("embed_form", false);
var hide_url_form = GM_getValue("url_form", false);
var hide_more_from = GM_getValue("more_from", false);
var hide_related_videos = GM_getValue("related_videos", false);
var hide_promoted_videos = GM_getValue("promoted_videos", false);
var hide_comments = GM_getValue("comments", false);
var hide_tags = GM_getValue("tags", false);
var hide_video_details = GM_getValue("video_details", false);
var hide_ratings_and_views = GM_getValue("ratings_and_views", false);
var hide_search_box = GM_getValue("search_box", false);
var hide_navigation_tabs = GM_getValue("navigation_tabs", false);
var hide_also_watching= GM_getValue("also_watching", false);
var hide_views = GM_getValue("views", false);
var hide_watch_main = GM_getValue("watch_main", false);
var hide_share_options = GM_getValue("share_options", false);
...
if(hide_url_form) {
hide("watch-url-div");
if(id("url_form")) { check("url_form"); }
} else { show("watch-url-div"); }

if(hide_embed_form) {
hide("watch-embed-div");
if(id("embed_form")) { check("embed_form"); }
} else { show("watch-embed-div"); }

if(hide_more_from) {
hide("watch-channel-videos-panel");
if(id("more_from")) { check("more_from"); }
} else { show("watch-channel-videos-panel"); }

if(hide_related_videos) {
hide("watch-related-videos-panel");
if(id("related_videos")) { check("related_videos"); }
} else { show("watch-related-videos-panel"); }

if(hide_promoted_videos) {
hide("watch-promoted-container", 1);
if(id("promoted_videos")) { check("promoted_videos"); }
} else { show("watch-promoted-container"); }

if(hide_comments) {
hide("watch-comments-stats");
if(id("comments")) { check("comments"); }
} else { show("watch-comments-stats"); }


if(hide_tags) {
hide("watch-video-tags-div");
if(id("tags")) { check("tags"); }
} else { show("watch-video-tags-div"); }

if(hide_video_details) {
hide("watch-channel-vids-div");
if(id("video_details")) { check("video_details"); }
} else { show("watch-channel-vids-div"); }

if(hide_ratings_and_views) {
hide("watch-ratings-views");
if(id("ratings_and_views")) { check("ratings_and_views"); }
} else { show("watch-ratings-views"); }

if(hide_search_box) {
hide("masthead-search-term", 3);
if(id("search_box")) { check("search_box"); }
} else { show("masthead-search-term", 3); }

if(hide_navigation_tabs) {
hide("tab-wrapper");
if(id("navigation_tabs")) { check("navigation_tabs"); }
} else { show("tab-wrapper"); }

if(hide_also_watching) {
hide("watch-active-sharing");
if(id("also_watching")) { check("also_watching"); }
} else { show("watch-active-sharing"); }

if(hide_views) {
hide("watch-views-div");
if(id("views")) { check("views"); }
} else { show("watch-views-div"); }

if(hide_watch_main) {
hide("watch-main-area");
if(id("watch_main")) { check("watch_main"); }
} else { show("watch-main-area"); }

if(hide_share_options) {
hide("aggregationServicesDiv");
if(id("share_options")) { check("share_options"); }
...
GM_setValue("navigation_tabs", checked("navigation_tabs"));
...
if(!id("yt_c_options")) {
// Add options link
var langs = id("region-and-language-picker-links-wrapper");
// Create the span element
var span = document.createElement("span");
span.setAttribute("class", "util-item");
// Create the link element
var a = document.createElement("a");
a.href = "#";
a.addEventListener("click", options_box, false);
a.setAttribute("class", "uiLanguagePickerLink eLink");
a.appendChild(document.createTextNode("YouTube Customization"));
// Put the link inside the span
span.appendChild(a);
// Add the span with link inside it to the other language links
langs.appendChild(span);

// Create the options box (invisible until options link is clicked)
var box = document.createElement("div");
box.setAttribute("style", "text-align:left; background-color:#eee; position:fixed; top:50px; right:20px; padding:10px; font-size:12px; color:#000; z-index:99; border: 2px ridge #000; display:none;");
box.setAttribute("id", "yt_c_options");
// Create the script options
box.innerHTML  = "<table><th colspan=\"2\">Hide:</th><tbody><tr><td>Ads</td><td><input type=\"checkbox\" id=\"ads\"/></td></tr><tr><td>URL Form</td><td><input type=\"checkbox\" id=\"url_form\" /></td></tr><tr><td>Embed Form</td><td><input type=\"checkbox\" id=\"embed_form\" /></td></tr><tr><td>More From Panel</td><td><input type=\"checkbox\" id=\"more_from\" /></td></tr><tr><td>Related Videos</td><td><input type=\"checkbox\" id=\"related_videos\"/></td></tr><tr><td>Promoted Videos</td><td><input type=\"checkbox\" id=\"promoted_videos\" /></td></tr><tr><td>Comments</td><td><input type=\"checkbox\" id=\"comments\"/></td></tr><tr><td>Video Tags</td><td><input type=\"checkbox\" id=\"tags\"/></td></tr><tr><td>Video Details</td><td><input type=\"checkbox\" id=\"video_details\"/></td></tr><tr><td>Ratings & Views</td><td><input type=\"checkbox\" id=\"ratings_and_views\"/><tr><td>Seach Box</td><td><input type=\"checkbox\" id=\"search_box\"/></td></tr><tr><td>Navigation Tabs</td><td><input type=\"checkbox\" id=\"navigation_tabs\"/></td></tr><tr><td>Also Watching</td><td><input type=\"checkbox\" id=\"also_watching\"/></td></tr><tr><td>Views</td><td><input type=\"checkbox\" id=\"views\"/></td></tr><tr><td>Watch Main Area</td><td><input type=\"checkbox\" id=\"watch_main\"/></td></tr><tr><td>Share Options</td><td><input type=\"checkbox\" id=\"share_options\"/></tr></tbody></table><br>";
var uca = document.createElement("a");
uca.href = "#";
uca.appendChild(document.createTextNode("Un-Check All"));
uca.addEventListener("click", uncheckAll, false);
box.appendChild(uca);
box.appendChild(document.createElement("br"));
box.appendChild(document.createElement("br"));
var ca = document.createElement("a");
ca.href = "#";
ca.appendChild(document.createTextNode("Check All"));
ca.addEventListener("click", checkAll, false);
box.appendChild(ca);
box.appendChild(document.createElement("br"));
box.appendChild(document.createElement("br"));
...
close.addEventListener("click", options_box, false);
box.appendChild(close);
// Add the box to the page but as invisible
...
ffghj.setAttribute('href', 'http://s.ytimg.com/yt/favicon-vfl86270.ico');
