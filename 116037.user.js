...
(function () {
   ...
   flickreasyphotopost.prototype = {
	   ...
	   for(i=0;i<MEMORY;i++) {
		 ...
		 if(id != undefined && id != '') {
		   ...
		   a.addEventListener('click',function(evt) {
								...
								var photoPage = GM_getValue(index+".photopage");
								if (!photoPage) {
									photoPage = 'http://www.flickr.com/'+GM_getValue(index+".ownerurl")+'/'+GM_getValue(index+".id")+'/';
								}

								var mesg = "&quot;"+GM_getValue(index+".title")+"&quot; by "+name+' <a href="http://6v8.gamboni.org/Flickr-Easy-Photo-Post.html">[?]</a>'+
								  "\n"+'<a href="'+ photoPage +'" title="Go to the photo page"><img src="http://static.flickr.com/'+GM_getValue(index+".server")+'/'+GM_getValue(index+".id")+'_'+GM_getValue(index+".secret")+size+'.jpg" alt="'+GM_getValue(index+".title")+'" alt="'+GM_getValue(index+".title")+' by '+name.replace(/<\/?b>/g,'')+'"/></a>';
	 ...
	 getJSVariable: function (regex) {
        // Thanks to Vispillo for this compact code
        	var retval;
        	$x('//script', document).forEach( function (script, idx) {
            		if (retval != undefined) {
                		return;
            		}
	 ...
	 getApiKey: function () {
        	...
        	var retval =  this.getJSVariable(reMatch);
        	if (retval == undefined) {
                	retval = this.getJSVariable(/\"?api_key\"?[ :]+[\'\"]([^\'\"]+)[\'\"]/);
        	}
        	return retval;
	 },
	 getAuthHash: function () {
	 	return this.getJSVariable(/\"?auth_hash\"?[ :]+[\'\"]([^\'\"]+)[\'\"]/);
	 },
	 getSecret: function () {
	 	return this.getJSVariable(/\"?secret\"?[ :]+[\'\"]([^\'\"]+)[\'\"]/);
	 },

	 getPhotoData: function(photoId, current_index, callback) {
	 	var api_key = this.getApiKey();
		var auth_hash = this.getAuthHash();
		var secret = this.getSecret();
	 	GM_xmlhttpRequest({
			...
			onload: function (response) {
				try {
					var photoInfo = JSON.parse(response.responseText);
				} catch (e) {
					GM_log("ERROR parsing API data: " + e);
					return;
				}
				if (photoInfo.stat == "ok") {
					var photo = {
						id: photoInfo.photo.id,
						ownersUrl: 'photos/' + photoInfo.photo.owner.username,
						photoPage: photoInfo.photo.urls.url[0]._content,
						ownername: photoInfo.photo.owner.realname,
						username: photoInfo.photo.owner.username,
						nsid: photoInfo.photo.nsid,
						server: photoInfo.photo.server,
						secret: photoInfo.photo.secret,
						title: photoInfo.photo.title._content
					};
					callback(photo, current_index);
				} else {
					GM_log("ERROR retrieving API data: " + photoInfo.code + " - " + photoInfo.message);
				}
			},
			onerror: function (response) {
				GM_log("AJAX call failed");
			}
		});
	 },

	 doPhotoPageStuff: function (photo, current_index) {
		var id = photo.id;
		 var i = 0;
		 var exist = false;
		 for(i=0;i<MEMORY;i++) {
		 ...
		 if(!exist) {
		   ...
		   GM_setValue(new_index+".photopage",photo.photoPage);
		   GM_setValue(new_index+".ownername",photo.ownername || photo.username || photo.nsid);
		   GM_setValue(new_index+".server",photo.server);
		   GM_setValue(new_index+".secret",photo.secret);
		   GM_setValue(new_index+".title",photo.title);
		 }

		 //insert the commenter stuff.
		 var arse = flickreasyphotopost.prototype.createCommenter();
	 ...
	 init: function() {
	   if(document.location.pathname.indexOf('editcomment') >= 0) {
		 ...
		 var photo;
		 var id = /.*flickr.com\/photos\/[^\/]+\/(\d+)/.exec(document.location.href)[1];
		 this.getPhotoData(id, current_index, this.doPhotoPageStuff);
	   }
	 }
   ...
   var clear_history = function() {
	 for(var i=0;i<MEMORY;i++) {
	   ...
	   GM_setValue(i+".id",'');
	   GM_deleteValue(i+".ownerurl");
	   GM_setValue(i+".photopage","");
