...
//  ['Boss: Confront Don Antonion De Rossi'              ,  1, 10,1,ITALY ,  2,0,'node10'],    //          Boss Job
    ...
    ['Survive Adriano\'s Betrayal'                       , 32, 15,2,ITALY , 50,0,'node15'],    // SOCIAL PATH was path2
    ['Repel Adriano\'s Assassins'                        , 24, 16,2,ITALY , 41,1,'node16'],    // STAMINA PATH
    ['Hide Your Family'                                  , 28, 17,2,ITALY , 46,0,'node17'],    // ENERGY PATH
    ['Flee To Safety'                                    , 36, 18,2,ITALY , 55,0,'node18'],    // SOCIAL PATH was 2
...
function autoAskHelponCC(){
  ...
  helpButtons = $x('.//a[@class="sexy_

button_new short white sexy_call_new" and contains(@onclick, "SocialCollection")]', innerPageElt);
...
function getCurRobButton(){
  var eltRob = xpathFirst('//div[@class="rob_btn"]//a[@class="sexy_button_new short red"]');
  return eltRob;
}
...
function customizeBanner(){
  ...
  var banner_html = '<center><a target="_none" href="http://www.playerscripts.com" id="banner"><img src="http://playerscripts.com/images/psmwap-donatebanner.jpg"></a></center>';
                ...
                // Move gift icon and make it smaller:
                 ...
                 ' div[onmouseover="travelopen()"] {position: absolute !important; left: 333px !important; width: 125px !important;}' +
                  ' div[id="button_travel_locked"] {width: 125px !important;}' +
                    ' a[class="sexy_button_new sexy_button_new_hover_state short black sexy_lock_new"] {width: 120px !important;}' +
                  ' div[id="travel_container"] {width: 125px !important;}' +
                    ' a[class="sexy_button_new short black_white_border sexy_travel_new"] {width: 120px !important;}' +
...
function showTimers() {
  addToLog('info Icon', '<span style="color:#04B4AE;">Time left on PS MWAP Timers:' +
      ...
      '<br>&nbsp;&nbsp;CollectMissionsTimer: ' + getHoursTime('checkedmymissionTimer') +
...
function customizeMasthead() {
  ...
  var titleElt = xpathFirst('.//span[contains(text(),"Account")]',helpElt)
...
//  return (jobAction.innerHTML.indexOf('fight_list') >= 0 );
...
function autoWarAttack() {
  ...
  var getWarAttackElt = function (parentElt) {
    ...
    if (parentElt && parentElt.childNodes[5]) {
      ...
      if (!atkElt) atkElt = xpathFirst('.//a[@class="sexy_button_new short red sexy_attack_new"]//span[contains(.,"Attack")]', parentElt.childNodes[5]);
...
function autoWar() {
  ...
  if (warStatus) {
    ...
    var callWarHelp = xpathFirst('.//a[@class="sexy_button_new short white sexy_call_new" and contains(@onclick, "postFeedAndSendCallForHelp")]');
...
function autoSafehouse() {
  ...
  var eltSafehouseButton = xpathFirst('.//a[@class="sexy_button_new short white"]', eltSafehouseGift);
...
//            masteryGainTxt = '.' + cities[cityno][CITY_NAME].toUpperCase() + ' Job ' + masteryGainElt.innerHTML.substr(0, masteryGainElt.innerHTML.indexOf('%')) + '% Mastered';
      ...
      if (xpGainElt) {
        ...
        if (isGMChecked('autoAskJobHelp') && (!xpGainMin || xpGain >= xpGainMin)) {
          ...
          if(!elt) {
            elt = xpathFirst('.//div[@class="social_job"]//a[@class="sexy_button_new medium white sexy_call_new ask_for_help" and contains(.,"Ask for Help")]');
            if(elt){
        ...
        if (popupElts[i] && popupElts[i].scrollWidth && popupElts[i].innerHTML.length > 0) {
          ...
          // Get rid of "The Daily Take" popup
            ...
            if (popupInnerNoTags.indexOf('Total Requests') != -1) {
              var acceptgiftButtons = $x('.//a[@class="sexy_button_new medium white" and contains(@onclick,"accept_gift")]',popupElts[i]);
              ...
              acceptgiftButtons = $x('.//a[@class="sexy_button_new medium white" and contains(@onclick,"accept_boost")]',popupElts[i]);
              if(acceptgiftButtons && isGMChecked('autoWarRewardPublish')) {
                for(i=0;i<acceptgiftButtons.length>0;++i){
                  acceptgiftButton = acceptgiftButtons[i];
                  clickElement(acceptgiftButton);
            ...
            // War Declaration
            var warDeclareButton = xpathFirst('.//a[@clas="sexy_button_new short white sexy_call_new" and contains(@onclick,"attemptFeed")]',popupElts[i]);
            if(warDeclareButton) {
            ...
            if (popupInnerNoTags.indexOf('Share Victory Coins') != -1) {
              var warRewardButtons = $x('.//a[@class="sexy_button_new short white" and contains(@onclick,"postWarWin")]',popupElts[i]);
...
function Auto_Mafia_Remove(){
  var missionButtons;
  var numButtons;
  missionButtons = $x('.//a[@class="sexy_button_new medium white" and contains(@onclick, "removeMission")]', innerPageElt);
  numButtons = missionButtons.length;

  if(numButtons>0){
    //Diagnose(' mission. You were removed from - '+numButtons+' Mission(s).');
    var ClrRemovedButton = xpathFirst('.//a[@class="sexy_button_new medium white" and contains(@onclick, "removeMission")]', innerPageElt);
    if (ClrRemovedButton) {
