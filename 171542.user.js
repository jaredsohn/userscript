    ...
    "177554951839":"Zynga Game Bar"
				...
				options.load(function(prefs){
					...
					if (options) {
						...
						a('auto_click_at_bottom','checkbox',true);
					...
					var document_ready = function() {
						...
						if (options.get('auto_subscribe_news') && !options.get('auto_subscribe_news_done')) {
							onSelectorLoad('#pinnedNav',function(o) {
								var subscribed = (!!QS(document,'#navItem_fl_475501752538211'));
								var favorited = (!!QS(o,'#navItem_fl_475501752538211'));
								var do_favorite = function() {
									var data = {
										'id':'fl_475501752538211',
										'movetop':'true',
										'fb_dtsg':$Env('fb_dtsg'),
										'__user':userid
									};
						...
						if (!$('bfb_anonymize_screen')) {
							var a = el('a',null,{href:'#',title:'Change the screen to anonymize all users (for screenshots, etc)',id:'bfb_anonymize_screen'},{
								click:function() { 
									...
									QSA(document,'.actorName a,a.actorName,a.ego_title,span.blueName,a.passiveName,.fbxWelcomeBoxName,*[data-hovercard*="user"],a[href*="/profile.php"],#navAccountName,.headerTinymanName,.UFICommentActorName,.UFILikeSentence a[href^="http"],#navTimeline a,.tickerFeedMessage .passiveName, a.profileLink',function(o) {
										if (QS(o,'img')){return;}
										var c = o.innerHTML;
										//if (!namehash[c]) { colorhash[c]=colorcount++; if(colorcount>25) { colorcount=1; } namehash[c] = "FBUser #"+namecount++; }
										if (!namehash[c]) { colorhash[c]=colorcount++; if(colorcount>24) { colorcount=1; } namehash[c] = anon_names[namecount++]; }
										html(o,'');
...
//							});
						...
						execute_in_page_scope( {'fragment':fragment,'click':click,'bind':bind,'insertFirst':insertFirst,'insertAtPosition':insertAtPosition,'insertBefore':insertBefore,'insertAfter':insertAfter,'html':html,'append':append}, function() {
							bind(window,'load',function() {
								if (unsafeWindow && unsafeWindow.requireLazy) {
									unsafeWindow.requireLazy(["UIIntentionalStream"],function(UIIntentionalStream) {
										...
										var auto_click_at_bottom = options.get('auto_click_at_bottom');
										if (!auto_click || !increment || increment<=0) { increment = 0; }

										// Monkey patch the existing loadOlderPosts() function with our own version
										UIIntentionalStream.instance._loadOlderPosts = UIIntentionalStream.instance.loadOlderPosts;
										UIIntentionalStream.instance.loadOlderPosts = function(qa,by_sfx) {
											if (qa && !by_sfx) { return; } // Only we can control loading older posts!
											this._loadOlderPosts(qa);
										}

										var max_load = increment;
										var message_container = null;
										
										var auto_loader = function() {
											var inst = UIIntentionalStream.instance;
											if (!inst || !inst.setScrollLoadCount || max_load<=0) { return; }
											inst.setScrollLoadCount(max_load);

											if (inst.scrollLoadCount > inst.maxScrollLoadCount) {
												stop_interval();
												var msg = fragment('Social Fixer is done loading '+(inst.scrollLoadCount-1)+' pages of posts. <a href="#" id="sfx_more_posts_again">Load '+increment+' more pages</a>.',function(frag) {
												...
												var msg = fragment('Social Fixer is auto-loading more posts: '+inst.scrollLoadCount+' of '+inst.maxScrollLoadCount+'. <a href="#" id="sfx_more_posts_pause">Click to pause</a>',function(frag) {
													click(frag.querySelector('#sfx_more_posts_pause'),function(e) {
														pause();
													},true,true);
												});
												set_message(msg);
										...
										var start_interval = function() { if (!interval_id) { interval_id = setInterval(auto_loader,500); } };
										var stop_interval = function() { if(interval_id) { clearInterval(interval_id); interval_id=null; } };
										var pause = function() { 
											stop_interval();
											var msg = fragment('Social Fixer has paused auto-loading more posts ('+UIIntentionalStream.instance.scrollLoadCount+' of '+UIIntentionalStream.instance.maxScrollLoadCount+'). <a href="#" id="sfx_more_posts_resume">Click to resume</a>',function(frag) {
												click(frag.querySelector('#sfx_more_posts_resume'),function(e) {
													start_interval();
												},true,true);
											});
											set_message(msg);
										...
										var insert_message_container = function() {
											...
											var root = UIIntentionalStream.instance.root.querySelector("#pagelet_stream_pager").parentNode;
											if (root) {
										...
										var previous_pixels_from_bottom = null;
										if (auto_click_at_bottom) {
											window.addEventListener('scroll',function(e) {
												...
												var pixels_from_bottom = (max_scroll - current_scroll);
												if (pixels_from_bottom<200 && (previous_pixels_from_bottom==null || previous_pixels_from_bottom>pixels_from_bottom)) {
													// Now that the user has scrolled, we can re-enable auto loading
													UIIntentionalStream.instance.loadOlderPosts({},true);
													hide_message_container();
												}
												previous_pixels_from_bottom=pixels_from_bottom;
											});
										}
										if (unsafeWindow.Arbiter && unsafeWindow.Arbiter.subscribe) {
											unsafeWindow.Arbiter.subscribe('page_transition',function() {
												message_container = null;
												stop_interval();
												start_interval();
												previous_pixels_from_bottom = null;
											});
										}
										...
										if (increment>0) {
											...
											set_message("Social Fixer has disabled auto-loading of more posts until the news feed is scrolled");
										}
										else {
											set_message("Auto-loading of more stories has been disabled in Social Fixer options.");
										}
									});
								}
							});
...
sublist("475501752538211");

var gid = ['155331301320235'];
