...
//2011.05.04 フェイスブック版ブラウザ三国志用に、中国語（繁体字）に対応。
...
function setVillageFacility() {
	...
	for (var i = 0; i < actionsElem.snapshotLength; i++) {
		...
		if (buildStatusElem.snapshotLength > 0) {
			...
			for(var j=0; j<buildStatusElem.snapshotLength;j++){
				if(buildStatusElem.snapshotItem(j).parentNode.innerHTML.match(RegExp("(建造中|建築準備中)"))) {
					//施設建設数
					cnt++;
				}
			}
	...
	if(OPT_KATEMURA == 1) {
		...
		for(var i=0;i < area_all.length;i++){
			...
			else if(area_all[i].name.match(/田\s.*?(\d+)/)){hatake++;if(parseInt(RegExp.$1)>=5){suzume_Flag=1;}}
...
//			if(tmp-hatake > 0){ //それでも平地が余っていれば
			if(tmp > 0){ //それでも平地が余っていれば
				if(Chek_Sigen(new lv_sort("田",0,"")) != 1){ //資源チェック
...
//	var popupTop = 150;
	...
	ccreateCheckBox(td1, "OPT_CHKBOX3", OPT_CHKBOX[3], "製鐵場","自動でLv上げをする建築物にチェックをしてください。",0);
		...
		GM_xmlhttpRequest({
			...
			onload:function(x){
				...
				for(i=1;i<skillElem.snapshotLength;i++){
					...
					if((skillTag.indexOf("知識",0) > 1 )||(skillTag.indexOf("技術",0) > 1 )||(skillTag.indexOf("吳的治世",0) > 1 )||(skillTag.indexOf("王佐",0) > 1 ) ){
