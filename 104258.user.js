...
var Util = {
    ...
    /**
     * Search for a javascript code that apply html code to popup_fodder element, return a new element with that code.
    ...
    parsePopup: function(htmlText) {
        try {
            ...
            if ( sHtml ) {
		        return h$(String(sHtml).replace(/\\"/g,'"'));
            }
        }
        catch(err) {
...
function FreeGiftsCenter() 
{
    ...
    var CSCollector = function() {
        ...
        this.addLinks = function(text) {
            ...
            var bUseFilter = $('#freegifts_usefilter')[0].checked;
    ...
    function startCollector(gifts) {
            ...
            function parseAndOpen(longUrl) {
                try {
                    ...
                    if(options.get('ignoreLimits') !== true && excluded.is(link.gift_id)) {
                        addMessageLog('Skipped. You\'ve reached the limits for '+current.name+'.');
                        doNextAction(next_gift);
                        return;
                    }
                    ...
                    collectGift(link, nSendBack, function(data) {
                        ...
                        if (limitRegex.test(data.message) && !excluded.is(link.gift_id)) {
                            excluded.add(link.gift_id);
                            addMessageLog('Limit reached. "'+current.name+'" added to exclude list.');
                        }
                        doNextAction(next_link);
                    });
                }
        ...
        function addMessageLog(message, icon, user_id) {
            ...
            .append(c$('p').html(userProfile + message)).prependTo(msgLog);
        }
    ...
    function genMainDom() {
        ...
        c$('div').css({'float':'left','width':'49%','height':600,'text-align':'center'}).append(
            ...
            .append(c$('label','for:select_fbgroups').text('Scan In:').css('color','#F4C506'))
            .append(c$('select','id:select_fbgroups,class:black_box').width('80%').change(Events.selectGroup_change))
            .append('<br />')
...
function HomeFeedCenter() 
{
    ...
    var CSStream = function(feed) {
        ...
        switch (sGrpName) {
            case 'doGiveSocialHelp'      : cFilter = new CSFilter('socialHelpFilter');   break;
            case 'doCollectLoot'         : cFilter = new CSFilter('collectLootFilter');  break;
            case 'doPropertyHelp'        : cFilter = new CSFilter('propertyHelpFilter'); break;
            case 'doClaimBonusAndReward' : cFilter = new CSFilter('rewardFilter');       break;
        }
        ...
        httpAjaxRequest({
            ...
            success: function(htmlText) 
            {
                ...
                if (action.popup && (r = Util.parsePopup(htmlText))) {
                    if ((r2 = r.find(action.popup)).length > 0) {
                        callback(r2.text());
                        return;
                    }
                    else if ( /txt_past_war_results.png/.test(r.html()) ) {
    ...
    function filterWar(htmlText) {
        ...
        try {
            ...
            if ((new CSFilter('warFilter'))(text) !== false) {
    ...
    function doOperation(htmlText, callback) 
    {
        ...
        try {
            ...
            if ((new CSFilter('missionFilter'))(sName) !== false) {
    ...
    function autoMode() {
        ...
        function closeAndNext(message, delay) {
            ...
            if (feedStream.length() < 1) {
                var refreshDelay = options.get('refreshDelay')*60;
        ...
        function refreshList() {
            ...
            facebook.queryFeed(options.get('feedsLimit'), function(fs) {
                ...
                if ((feedStream = new CSStreamCollection(fs, true)).length() < 1) {
                    messageTimer.start('No new posts, refresh again in %N%...', 60, refreshList);
                    return;
                }
                $('#total_feeds', '#feed_center_header').html(feedStream.length());
                doNewHelp();
    ...
    function genMainDom() {
        ...
        popupElt.applyOptions({
            ...
            'feedstream_refreshdelay'      : {0.25:'0.25', 1:'1', 3:'3', 5:'5', 15:'15', 30:'30', 60:'60'}
        });
    }
...
function Battlefield(_sHtmlText, _sList, _callback) 
{
     ...
     */
    var CSQueryResult = function(data) {
        var Qry  = h$(data);
     ...
     * @return {CSOpponent}
     */
    var CSOpponent = function(id, title, name, level, mafia, profile, url, iced, faction) {
        
        this.id       = id ? Util.parseNum(id) : 0;
        this.title    = title;
        this.name     = name;
        this.level    = Util.isNumber(level) ? parseInt(level) : 0;
        this.mafia    = Util.isNumber(mafia) ? parseInt(mafia) : 0;
        this.clanName = null;
        this.clanId   = null;
        this.iced     = (iced === true);
    ...
    function getOpponentFromProfile(data) {
        ...
        return new CSOpponent(id, '', name, level, 0, MW.getProfileLink(id),  url);
    }
    ...
    function parseOpponentTable(htmlText) {
        ...
        jQry.find('.fight_table tr:has(a)').each(function(index, element) 
        {
            ...
            try {
                ...
                // clan
                elem = $('td:eq(0) a[selector]:regex(href,controller=clan)', element);
                opp.clanId   = Util.uSplit(elem.attr('href')).id;
                opp.clanName = $.trim(elem.text());
                //
                opp.name     = $.trim($('td:eq(0) a[selector]:regex(href,controller=stats)', element).text());
    ...
    function parseAttackResponse(htmlText, autoMode) {
        ...
        obj = new CSQueryResult(htmlText);
