// ==UserScript==
// @name           Create Gang wars invites
// @namespace      Gang wars friends.
// @description    Automatically invites people to your clan that you find on the fight page. -modified by xreti from-->Frisno altered the  the original code  from Andy Calderbanks  "Create dragon wars invites" http://userscripts.org/scripts/show/31912
// @include        http://apps.facebook.com/gangwarsgame/fight.php
// @include        http://apps.facebook.com/gangwarsgame/recruit.php
// @include        http://apps.new.facebook.com/gangwarsgame/fight.php
// @include        http://apps.new.facebook.com/gangwarsgame/recruit.php
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/gangwarsgame/recruit.php';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/gangwarsgame/stats.php?user=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/gangwarsgame/recruit.php?action=create',
