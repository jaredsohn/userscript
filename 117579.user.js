...
// @version        4.2
// @history        4.2   multiple sources speedup the loading (improved but way to go)
// @history        4.1   Bugfix: resource >1M
...
function calcular(text){
	...
	*/
	...
	var nsrc = 0;
	for (var i = 0; i < xresource.length; i++) {
		if(cresource[i]==1 && xresource[i]>0) {
			tot_rep +=xresource[i];
			nsrc += 1;
		}
	}
	...
	if(tot_rep>max_cap){
		var diff = tot_rep - max_cap;
		for (var i = 0; i < xresource.length; i++) {
			if(cresource[i]==1 && xresource[i]>0) {
				var adjust = parseInt(diff*xresource[i]/tot_rep);
				if(xresource[i] < adjust) adjust = xresource[i] - 1; // at least 1 ship  
				xresource[i] -= adjust;
				diff -= adjust;
				tot_rep -= adjust;
				nsrc -= 1;
