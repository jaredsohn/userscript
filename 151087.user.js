// ==UserScript==
// @name           LoU Najazdy (oryginalnie LoU Suite)
// @description    Latwe grabienie lochow oraz atakowanie potworow. Przetlumaczona wersja LoU Suite (LGG + Philokei Edition)
// @namespace      https://sites.google.com/site/lordofultimapl/
// @license        Creative Commons Attribution-ShareAlike 3.0 Unported License - http://creativecommons.org/licenses/by-sa/3.0/
// @icon		   https://sites.google.com/site/lordofultimapl/favicon.ico?attredirects=0&d=1
		...
		function createSuite()
		{
			...
			qx.Class.define("louSuite.main", {
				...
				members:{
					...
					startScript     :function ()
					{
						...
						btn.set({width:30, appearance:"button-text-small", toolTipText:"LoU Najazdy"});
			...
			qx.Class.define("lou_suite.gui.DungeonRaider", {
				...
				construct:function ()
				{
					...
					this.setLabel("Najazd lochow");
					this.setLayout(new qx.ui.layout.VBox(10));
					this.add(this._createToolBar());
					this._tableModel = new qx.ui.table.model.Simple();
					var columnNames = [ "Id", "Row Info", "Rodzaj", "Poziom", "Postep", "Kordy", "Miasto", "Dystans", "Jednostki", "Akcja"];
				...
				members  :{
					...
					getUnitsPerDungeon             :function ()
					{
						...
						var units = {
							Gory:{
								...
								3 :[23, 147, 687, 2324, 4500, 9750, 21000, 30000, 54000, 90000], // Luczik
								//4: [7, 32, 146, 375, 1250, 2834, 6250, 10417, 16667, 25000], // Gwardzista
								7 :[30, 150, 700, 1800, 6000, 13600, 30000, 50000, 80000, 120000], // Mag
								11:[11, 53, 245, 630, 2100, 4760, 10500, 17500, 28000, 42000] // Rycerz

							},
							wzgorza    :{
								6:[15, 65, 250, 850, 2500, 5000, 12000, 20000, 35000, 50000], // Berserker
								3:[23, 147, 687, 2324, 4500, 9750, 21000, 30000, 54000, 90000], // Lucznik
								//4: [5, 20, 75, 255, 750, 1500, 3600, 6000, 10500, 15000], // Gwardzista
								7:[30, 130, 500, 1700, 5000, 10000, 24000, 40000, 70000, 100000], // Mag

								11:[11, 46, 175, 595, 1750, 3500 , 8400, 14000, 24500, 35000], // Rycerz

								12:[15, 65, 250, 850, 2500, 5000, 12000, 20000, 35000, 50000] // Czarownik
							},
							Las  :{
								6:[15, 65, 250, 850, 2500, 5000, 12000, 20000, 35000, 50000], // Berserker
								3:[23, 147, 687, 2324, 4500, 9750, 21000, 30000, 54000, 90000], // Lucznik
								...
								11:[10, 44, 167, 567, 1667, 3334, 8000, 13334, 23334, 33334], // Rycerz
								10:[8, 33, 125, 425, 1250, 2500, 6000, 10000, 17500, 25000] // Paladyn
							},
							Morze     :{
								16:[2, 9, 57, 110, 277, 417, 567, 834, 1250, 1667], // Slup
								15:[1, 4, 23, 44, 111, 167, 227, 334, 500, 667], // Fregata
								17:[1, 1, 6, 11, 28, 42, 57, 84, 125, 167] // Galeon
							}
						};
					...
					onTroopsSent                   :function (event, v)
					{
						...
						switch (code.r1)
						{
							...
							case 4: // Not enough units
								message = "Za malo wojska, sprobuj z innego miasta";
								break;
							case 64: // No more command slots
								message = "Za duzo polecen, sprobuj z innego miasta";
								break;
							default:
					...
					createCitiesContinentsSelectBox:function ()
					{
						...
						sb.setToolTipText("Sortuj: <b>kontynenty</b>");
					...
					_createToolBar                 :function ()
					{
						...
						this.sbLevel.setToolTipText("Sortuj: <b>min. poziom</b>");
						...
						for (var i = 1; i < 10; i++)
						{
							var item = new qx.ui.form.ListItem("Poziom " + i, null, i);
						...
						var btnViewRegion = new qx.ui.form.Button("widok regionu");
						...
						this.btnUpdateView = new qx.ui.form.Button("Skanuj");
						...
						var btnRecount = new qx.ui.form.Button("Przelicz wojsko");
						...
						this.sbField.setToolTipText("max. odleglosc <b>od miasta</b>");

						for (var i = 1; i <= 20; i++)
						{
							var itemField = new qx.ui.form.ListItem("Pola " + i, null, i);
			...
			qx.Class.define("lou_suite.gui.BossRaider", {
				...
				construct:function ()
				{
					...
					bos.gui.SummaryPage.call(this);
					this.setLabel("Najazd na potwory");
					this.setLayout(new qx.ui.layout.VBox(10));
					this.add(this._createToolBar());
					this._tableModel = new qx.ui.table.model.Simple();
					var columnNames = [ "Id", "Row Info", "Rodzaj", "Poziom", "Kordy", "Miasto", "Dystans", "Jednostki", "Akcja"];
					...
					getUnitsPerBoss                :function ()
					{
						var units = {
							Smok :{
								...
								3 :[84, 500, 3334, 6667, 16667, 25000, 33334, 50000, 75000, 100000], // Lucznik
								5 :[100, 600, 4000, 8000, 20000, 30000, 40000, 60000, 90000, 120000], // Templariusz
								9 :[42, 250, 1700, 3300, 8300, 12500, 17000, 25000, 37500, 50000], // Kusznik
								11:[19, 112, 756, 1467, 3689, 5556, 7556, 11112, 16667, 22223], // Rycerz
								7 :[36, 215, 1429, 2858, 7143, 10715, 14286, 21429, 32143, 42858], // Mag
								12:[21, 125, 834, 1667, 4167, 6250, 8334, 12500, 18750, 250000], // Czarownik
								10:[28, 167, 1134, 2200, 5534, 8334, 11334, 16667, 25000, 33334] // Paladyn
							},
							Moloch :{
								6 :[50, 300, 2000, 4000, 10000, 15000, 20000, 30000, 45000, 60000], // Berserker
								3 :[84, 500, 3334, 6667, 16667, 25000, 33334, 50000, 75000, 100000], // Lucznik
								5 :[100, 600, 4000, 8000, 20000, 30000, 40000, 60000, 90000, 120000], // Templariusz
								9 :[63, 375, 2500, 5000, 12500, 18750, 25000, 37500, 56250, 75000], // Kusznik
								11:[28, 167, 1112, 2223, 5556, 8334, 11112, 16667, 25000, 33334], // Rycerz
								7 :[24, 143, 972, 1886, 4743, 7143, 9715, 14286, 21429, 28572], // Mag
								12:[14, 84, 567, 1100, 2767, 4167, 5667, 8664, 12500, 16667], // Czarownik
								10:[42, 250, 1667, 3334, 8334, 12500, 16667, 25000, 37500, 50000] // Paladyn
							},
							Hydra  :{
								6 :[34, 200, 1360, 2640, 6640, 10000, 13600, 20000, 30000, 40000], // Berserker
								3 :[56, 334, 2267, 4400, 11067, 16667, 22667, 33334, 50000, 66667], // Lucznik
								5 :[68, 400, 2720, 5280, 13280, 20000, 27200, 40000, 60000, 80000], // Templariusz
								9 :[63, 375, 2500, 5000, 12500, 18750, 25000, 37500, 56250, 75000], // Kusznik
								11:[28, 167, 1112, 2223, 5556, 8334, 11112, 16667, 25000, 33334], // Rycerz
								7 :[36, 215, 1429, 2858, 7143, 10715, 14286, 21429, 32143, 42858], // Mag
								12:[21, 125, 834, 1667, 4167, 6250, 8334, 12500, 18750, 250000], // Czarownik
					...
					onTroopsSent                   :function (event, v)
					{
						...
						switch (code.r1)
						{
							...
							case 33554436: // Already ordered
								message = "Juz atakowany.";
								break;
							case 33554432: // Already ordered from a different castle
								message = "Juz atakowany z innego miasta";
								break;
							case 16781312: // Dungeon is closed
								message = "Juz atakowany";
								break;
							case 4: // Not enough units
								message = "Za malo jednostek przelicz wojsko.";
								removeBoss = false;
								break;
							case 64: // No more command slots
								message = "Limit polecen wyczerpany, sprobuj z innego miasta";
								removeBoss = false;
								break;
							default:
								message = "Unknown Error: " + code;
