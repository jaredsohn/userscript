// ==UserScript==
// @name PROP UNLOCKERaa
// @description PROP UNLOCKERaa
...
function ogpDisplayDef() {
  ...
  this.updateTitle = function(step,index) {
    ...
    switch(step) {
      ...
      case 1:
        var r = OGPAjax.ajax[index].response;
        var regexstring = /<div class="title">\s+.*"(.*)"\s+level/;
  ...
  this.buildMenu = function() {
    ...
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(51);">Italy</a>&nbsp;';
    ...
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(52);">Italy</a>&nbsp;';
    ...
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(53);">Italy</a>&nbsp;';
    ...
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(54);">Auto-Build Shop Item</a><br />';
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(55);">Collect Stream Links</a><br />';
    //txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(46);">Collect Mystery Bags</a><br />';
    txt +='</td>';
    txt +='<td>';
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(30);">Smart Drone Runner</a><font color=#ff0000> UPDATED!</font><br />';
    ...
    //txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(58);">Rob Christmas Trees</a><font color=#ff0000> NEW!</font><br />';
    txt +='</td>';
    txt +='<td>';
    ...
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(57);">My Inventory</a><br />';
    txt +='<a href="javascript:;" onclick="OGPDisplay.menuClick(56);">My Mafia Family</a><br />';
    txt +='</td>';
    txt +='<td>';
    txt +='<a href="javascript:top.location.href = \'http://apps.facebook.com/inthemafia/?zy_link=appage&ref=bookmarks\';">Reset To FB</a><br />';
        ...
        for (var i=0; i < 9; i++) {
          if (parseInt(OGPFight.arRobSpots[i][3]) <= cs && parseInt(OGPFight.arRobSpots[i][3]) != 0) {
            if (OGPFight.arRobSpots[i][2]=='') {
              ...
              setTimeout("OGPAjax.buildAjax('" + url + "','OGPFight.autoRobbing','4,%ix%');",i*0);
            } 
          } else {
            if (OGPFight.arRobSpots[i][2]=='') {
...
function ogpDroneRunnerDef() {
  ...
  this.droneSetup = function() {
    ...
    txt += '<td>Fighting: Max Lvl: <input size="5" type="text" name="txtFLevel" id="txtFLevel" value="' + this.fightLevel + '"> - Max Mafia: <input size="3" type="text" name="txtFMafia" id="txtFMafia" value="' + this.fightMafia + '"></td>';
    txt += '<td colspan="2">Rob/Fight in ';
    ...
    switch(step) {
      case 0:
        ...
        if (parseInt(OGPConfig.curStamina) > 0) {
          ...
          if (OGPConfig.currentCity != OGPDrone.stamRobCity)
            OGPTravel.goCity(OGPDrone.stamRobCity,"OGPDrone.spendStamina(3,0)");
          else
      ...
      case 1:
        ...
        if (OGPConfig.currentCity != OGPDrone.stamHealCity) {
      ...
      case 4:
        ...
        var i = OGPParser.getAttackTarget(OGPDrone.fightLevel,OGPDrone.fightMafia);
        if (i==-1) {
      ...
      case 5:
        ...
        if (r.indexOf('You Won!') > 0) {
          ...
          if (OGPConfig.curStamina <= 0 || OGPConfig.curExpNeeded-OGPConfig.curExp < 25) {
            OGPDisplay.setHTML('divDroneStatus','Low stamina, stopping before level up');
            OGPDrone.isburningstamina = false;
            return;
          }
          // Check health
        ...
        for (var i=0; i < 9; i++) {
          if (parseInt(OGPDrone.arRobSpots[i][3]) <= cs && parseInt(OGPDrone.arRobSpots[i][3]) != 0) {
            if (OGPDrone.arRobSpots[i][2]=='') {
              ...
              setTimeout("OGPAjax.buildAjax('" + url + "','OGPDrone.spendStamina','12,%ix%');",i*500);
            } 
          } else {
            if (OGPDrone.arRobSpots[i][2]=='') {
  ...
  this.myInventory = function(step,index) {
    switch(step) {
      ...
      case 1:
        ...
        var e = r.indexOf('var Qualities',s);
        r = r.substr(s,e-s);
        // Get the items
        var regexitem = /\{[^}]+\}/g;
  ...
  this.myAccount = function(step,index) {
    switch(step) {
      ...
      case 35:
        ...
        //e$('divOGPDebug').innerHTML = OGPAccount.shid + ' - ' + OGPAccount.ehid + '<br><br>' + r + '<textarea>' + r + '</textarea>';
        //break;
        var s = r.indexOf('<strong>');
        while ( s > 0) {
          var tmp = OGPAccount.getInvItem(r,s);
  ...
  this.sendLoot = function(step,index) {
    switch(step) {
      case 0: // See if the user is on a profile page
        ...
        var tmp = OGPParser.getValueInTags(document.body.innerHTML,'class="title"');
        tmp = tmp.split('"');
  ...
  this.sendCollections = function(step,index) {
    ...
    switch(step) {
      case 0:
        ...
        txt += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:OGPSend.colSendCity=' + OGPConfig.currentCity + ';OGPSend.sendCollections(1,4);">';
        txt += 'Crew</a>';
        txt += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:OGPSend.colSendCity=' + OGPConfig.currentCity + ';OGPSend.sendCollections(1,256873);">';
        txt += 'Special</a>';
        txt += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:OGPSend.colSendCity=' + OGPConfig.currentCity + ';OGPSend.sendCollections(1,9);">';
        txt += 'Mission</a>';
...
function ogpParserDef() {
  ...
  this.parseJobResults = function(r) {
    //e$('divOGPDebug').innerHTML = '<textarea cols=100 rows=8>' + r + '</textarea>';
    // Clear the res array
    res['timeout'] = false;
  ...
  this.getFightList = function(res) {
    ...
    while (s > 0) {
      e = res.indexOf('</tr>',s);
      e = res.indexOf('</tr>',e+1);
      var txt = res.substr(s,e-s);
      txt = txt.replace(/\t/g,'');
      txt = txt.replace(/\r\n/g,'');
      txt = txt.replace(/\n/g,'');
      var regexstr = /<span\s+>(.*?)<.*?user=(.*?)&.*?>(.*?)<.*Level\s([0-9]+).*?([0-9]+).*?\/table.*?xw_city=(.*?)'/;
      var match = regexstr.exec(txt);
      if (match) {
        var uid = OGPParser.decodeBase64(unescape(match[2]));
        var iced = 0;
        if (res.indexOf('iced.png') > 0) iced=1;
        OGPConfig.curFightList[OGPConfig.curFightList.length] = new Array(match[1],uid,match[3],match[4],match[5],match[6],iced,1);
      }
      s = res.indexOf('<tr >',e);
    }
  };
  
  this.getAttackTarget = function(lvl,mafia) {
    var found = -1;
    for (var i=0; i < OGPConfig.curFightList.length; i++) {
      if (found == -1 && OGPConfig.curFightList[i][6] == 1 && OGPConfig.curFightList[i][7]==1) {
        if (parseInt(OGPConfig.curFightList[i][3]) <= parseInt(lvl) && parseInt(OGPConfig.curFightList[i][4]) <= parseInt(mafia))
          found = i;
      }
    }
    return found;
    ...
    switch(step) {
      ...
      case 1: // Traveled (if necessary to property city) - Set up and collect from the properties
        ...
        if (OGPProperty.collectFlag) {
          ...
          for (var i=0; i < OGPItems.cityProperties.length; i++) {
            if (OGPItems.cityProperties[i][0] == OGPProperty.currentCity) {
              if (OGPProperty.currentCity >= 1 && OGPProperty.currentCity <= 6) { // Collect all for speed
...
function ogpItemsDef() {
  ...
  this.buildShopItems = function(step,index,prop) {
    switch(step) {
      case 0:
        ...
        txt += '</select></td>';
        txt += '<tr><td colspan="4"><a onclick="OGPItems.buildShopItems(1,0,0)">Build Items</a></td></tr>';
        txt += '</table>';
        OGPDisplay.setHTML('divOGPSetup',txt);
        break;
      ...
      case 2:
        ...
        var txt = '';
        txt += '<table id="tblBuildShop" name="tblBuildShop">';
        txt += '<tr><th colspan="2">Build Shop Items Results</th></tr><tr><td style="width:120px;">Chop Shop:</td><td><div id="divItem1" name="divItem1"></div></td></tr><tr><td>Weapons Depot:</td><td><div id="divItem2" name="divItem2"></div></td></tr><tr><td>Armory:</td><td><div id="divItem3" name="divItem3"></div></td></tr><tr><td>Zoo:</td><td><div id="divItem4" name="divItem4"></div></td></tr>';
        txt += '</table>';
        OGPDisplay.setHTML('divOGPResults',txt);
  ...
  this.loadCities = function() {
    ...
    this.cities[this.cities.length] = new Array('Italy','mw_city6','story',6,'propertyV2','story_tab','L$','italy');
  ...
  this.loadProperties = function() {
    ...
    this.cityProperties[this.cityProperties.length] = new Array(1,13,'Armory');
    */
  ...
  this.loadJobLevels = function() {
    ...
    this.jobLevels[4] = new Array(5,'District 1 - North Las Vegas','District 2 - Paradise City','District 3 - The Lower Strip','District 4 - Shogun Casino','District 5 - Mojave Desert','District 6 - The Upper Strip','District 7 - Area 51','District 8 - Hoover Dam');
    this.jobLevels[5] = new Array(6,'Region 1 - Roma','Region 2 - Palermo','Region 3 - Venezia','Region 4 - Milano','Region 5 - Napoli');
