...
// @author       Fidelis
// @version      3.6.1 Bugfix beim Login
// @description  Vereinfachte Killabgabe für Downfight
...
// @version      3.6.1 Bugfix beim Login
// @version      3.6 Assassinen hinzugefügt +++ Kompatibilisiert für Zepterträgertabelle +++ Bug bei Accountübernahme gefixt
// @version      3.5 Titelberücksichtigung umgangen +++ Minor Bugfix
...
//jQuery
GM_xmlhttpRequest({method:'GET',url:unescape('http://code.jquery.com/jquery-latest.min.js'),onload:function(jquery){start(jquery.responseText)}});
...
//Script
var server = 'http://downfight.eu/'; //Gespeicherte Grafiken 
	...
	if(l.search(/^https?:\/\/([a-z]{3,8}\.)?pennergame\.de\/fight\/(\#[a-z0-9]{1,20}|(fightlog\/(\d{1,5}\/|\?q=.*?)?)|overview\/|\?status=[a-z0-9]{1,20}|\?to=[A-Za-z0-9_\-+\s]{1,30})?$/g) != -1){
		...
		if(typeof city != 'undefined' && city != false){
			...
			var myhtml = '<tr id="fightids">';
				...
				myhtml += '<a style="cursor:pointer" class="tooltip">[?]';
				myhtml += '<span>';
				...
				myhtml += '<strong>Killabgabe:</strong><br />Es wird zun&auml;chst gepr&uuml;ft, ob du Soldat, Legion&auml;r, M&ouml;nch oder Gesindel bist. Danach werden an der entsprechenden Stelle deine Kills gegen Legion&auml;re/Soldaten, Pilger und Cheater abgegeben.<br /><br />';
				...
				myhtml += '</span>';
				myhtml += '</a></span><span style="float:right"><input type="checkbox" name="fightid_setting_showid" id="fightid_showid"/><label for="fightid_showid">&nbsp;&nbsp;Textfeld mit FightID anzeigen</label></span><br /><hr size="1">';
				myhtml += '<div id="fightid_status"></div>';	

				myhtml += '<span id="fightid_setting_container"><strong>Abgabeart:</strong>&nbsp;&nbsp;&nbsp;<span id="fightid_settings"><input value="kill" type="radio" id="fightid_setting_kill" name="fightid_setting"><label for="fightid_setting_kill">&nbsp;Killabgabe</label>&nbsp;&nbsp;<input value="geb" type="radio" id="fightid_setting_geb" name="fightid_setting"><label for="fightid_setting_geb">&nbsp;Geb&auml;udeabgabe</label></span><br />';
				myhtml += '<span id="fightid_geb_settings"><strong>Downfighter:</strong>&nbsp;&nbsp;&nbsp;<span id="fightid_downfightstring">-/-</span><br />';
				myhtml += '<strong>Hochfighter:</strong>&nbsp;&nbsp;&nbsp;<span id="fightid_hochfightstring">-/-</span></span></span>';
				myhtml += '<hr size="1">';
				myhtml += '</td>';
				myhtml += '</tr>';
			...
			var df_assassin = '<input value="df_assassin" type="radio" id="fightid_df_assassin" name="fightid_df"><label for="fightid_df_assassin">&nbsp;Abu Khabab Camp</label>';
			...
			var hf_assassin = '<input value="hf_assassin" type="radio" id="fightid_hf_assassin" name="fightid_hf"><label for="fightid_hf_assassin">&nbsp;Abu Khabab Camp</label>';
			...
			var fighter,groupnr,sperrstufe;
			//Multi-Login für DownFight initialisieren
			var subdomain = new Array('', '1.', '2.', '3.', '4.', '5.', '');
			var subdomainfree = subdomain.slice(0);
			var s = 0;
					...
					GM_xmlhttpRequest({
						...
						onload:function(){
							...
							GM_xmlhttpRequest({
								...
								url:unescape(opiurl),
								onload:function(opi){
									set_error('Lade personalisiere Daten von '+subdomain[start]+'downfight.de ...');
									var content = opi.responseText;							
									var login = $(content).find('loggedin').html();
									if(login == 0){
										set_error('<a href="http://'+subdomainfree[0]+'downfight.de/?seite=login" target="_blank">Bitte logge dich zuerst bei Downfight ein!</a>');
										s++;
										df_login(s);
									}
									else if(login == 1){
										set_error('<a href="http://'+subdomainfree[0]+'downfight.de/?seite=login" target="_blank">Bitte logge dich zuerst mit <b>deinem</b> Account bei Downfight ein!</a>');
										subdomainfree.splice(s, 1).filter(function(n){return n});
										s++;
										df_login(s);
									}
									else if(login == 2){
										...
										//Pilger
										if(sperrstufe == 7){
											var kl = (groupnr == 2) ? df_kloster+'&nbsp;&nbsp;&nbsp;'+df_casino+'&nbsp;&nbsp;&nbsp;' : '';
											var kl = (groupnr == 1) ? legi_ges+'&nbsp;&nbsp;&nbsp;'+df_casino+'&nbsp;&nbsp;&nbsp;' : kl;
											var kl = (groupnr == 5) ? ges_legi+'&nbsp;&nbsp;&nbsp;'+df_casino+'&nbsp;&nbsp;&nbsp;' : kl;
											var kl = (groupnr == 6) ? df_assassin+'&nbsp;&nbsp;&nbsp;' : kl;
											
											var downfightstring = kl+df_pub;
											var hochfightstring = '-/-';
										...
										//Assassinen
											...
											var downfightstring = df_assassin+'&nbsp;&nbsp;&nbsp;'+df_pub;	
											var hochfightstring = hf_assassin+'&nbsp;&nbsp;&nbsp;'+hf_puff;
											fighter = 'Assassine';
										}
										...
										$('input#fightid_df_kloster').prop('checked', true)/*.prop('disabled', true)*/;
										$('input#fightid_hf_kloster').prop('checked', true)/*.prop('disabled', true)*/;
										$('input#fightid_df_assassin').prop('checked', true);
										$('input#fightid_hf_assassin').prop('checked', true);
										...
										else if(groupnr == 1 || groupnr == 2 || groupnr == 5 || groupnr == 6){
										...
										if(content.indexOf("<Downfight></Downfight>") != -1)
											set_error('Du (<b>'+me+'</b>) bist in dieser Stadt noch nicht verifiziert oder freigeschaltet.<br />Falls du schon freigeschaltet bist, versuche, dich auf Downfight in eine Liste <a target="_blank" href="http://downfight.de/?seite=voreintragen">einzutragen</a>!<br /><br />Falls <b>'+me+'</b> NICHT dein Account ist, <a id="username_reset" href="#" title="Deinen Usernamen neu laden"><b>KLICKE HIER!</b></a>');
										else
											set_error('Downfight ist gerade &uuml;berlastet. Versuche es in ein paar Minuten noch einmal!');
										$("#username_reset").on('click', function(){
											GM_deleteValue('me'+c_str);
											location.reload();
										});
									}
								},
			...
			function create_buttons(){
				if(buttons == true){
					...
					$('tr > td a[href*="/fight/viewfight/"]').closest('td').next('td').next('td').each(function(){
						if($(this).find('img.fightid_button').length < 1){
							...
							if(GM_getValue('abgabe_settings_'+c_str) == 'kill')
								set = true;
							if(src.match(/.*?\/(1_0|1389738561)(kopie)?\.gif$/) || (src.match(/.*?\/(1_1|1389738584)(kopie)?\.gif$/) && (groupnr == 1 || groupnr == 5 || groupnr == 6)))
								type = 'df';
							else if(src.match(/.*?\/(0_0|1389738495)(kopie)?\.gif$/) && set == false && sperrstufe != 7)
								type = 'hf';
							else if(src.match(/.*?\/(0_0|1389738495|0_1|1389738532|1_1|1389738584)(kopie)?\.gif$/) && set == true)
								type = 'kill';
							if(type != false){
								...
								$('img.fightid_button', this).on('click', function(){
									switch($('input[name=fightid_df]:checked').val()){
										...
										case 'df_assassin':
											var df_gebaeude = '246';
											var df_gebaeudestring = 'im Abu Khabab Camp';
											break;
										default:
											var df_gebaeude = '226';
											var df_gebaeudestring = 'im Casino';
									...
									switch($('input[name=fightid_hf]:checked').val()){
										...
										case 'hf_assassin':
											var hf_gebaeude = '246';
											var hf_gebaeudestring = 'im Abu Khabab Camp';
											break;
										default:
											var hf_gebaeude = '55';
											var hf_gebaeudestring = 'im Puff';
