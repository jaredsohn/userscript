...
const VERSIONfunctionFile = "1.0.17";
...
function GM_setValue2(a,b){
	// for developing / finding security errors
	GM_setValue(a,b);
	if(GM_getValue(a)!=b){
		GM_log("GM_setValue2 ERROR :"+a+":"+b+"!="+GM_getValue(a));
...
function getTimeStr(time,hideSeconds){ // was int2timestr
	...
	time=time/60;
	help = time%24;
	str = str.replace("hour",Math.floor(help));
	time=time/24;
...
function getDateText(time,yearformat){ // was datumDay
	...
	if (time2 < today){
		return getDateStr(time,yearformat);
	} else if (time2 < (today+(1*24*60*60))){
		return texte["day0"];
	} else if (time2 < (today+(2*24*60*60))){
		return texte["day1"];
...
function explode(str){
	...
	if (typeof str != "number" && typeof str != "string") throw ("Explode error Argument not a String or Number");

	try{
		return eval('(' + str + ')');
	} catch(err){
		GM_log("ERROR Explode\n" + err + " : " + str);
		throw ("ERROR Explode\n" + err);
	}
}
...
function print_r(arr,line,showType){
	var str = new String();
	if (!line) line="";
		...
		for (var i in arr ){
			try{
				...
				if (typeof arr[i] == "string" || typeof arr[i] == "number" || typeof arr[i] == "boolean") {
					str += line + (type?"[":"{") + i + (type?"]":"}") + " = " + arr[i] + ((showType)? " " + typeof(arr[i]):"") + "<br/>";
				} else if(typeof arr[i] == "undefined"){
					str += line + (type?"[":"{") + i + (type?"]":"}") + " = " + "<br/>";
				} else if(typeof arr[i] != "function"){
					str += print_r(arr[i],line +(type?"[":"{") + i + (type?"]":"}"),showType);
				}
			}catch(err){
...
function produktPic(racktype,product,append){
// racktype: 0 = rack, 1 = forrest
	var prodNum = isNaN(parseInt(product,10))?prodId[product]:parseInt(product,10);
	var newdiv = createElement("div",{"racktype":racktype,"prod":prodNum,"style":"display:inline-block;margin-right:3px;border:none;vertical-align:bottom;"});
	if (racktype==0){
		if (prodNum>0){
			...
			newdiv.setAttribute("class","f_m_symbol"+prodNum);
	} else {
		GM_log("unknown arguments in function-call of produktPic :racktype="+racktype+",product="+product);
	}
	if (append){ append.appendChild(newdiv); }
	return newdiv;
}
function produktMillPic(prodNum,append){
	var newdiv = createElement("div",{style:"float:left;margin-right:3px;border:none;"}); //TODO float needed?
	newdiv.setAttribute("class","fmm"+prodNum);
	if (append) append.appendChild(newdiv);
	return newdiv;
}
...
function alert2(text,yes,no,yesFkt,noFkt){
	...
	newdiv = createElement("div",{"style":"display:block;position:fixed;width:300px;top:50%;left:50%;margin-left:-180px;padding:30px;background-color:yellow;border:3px solid black;-moz-border-radius:10px;color:black;font-weight:bold;"},newdiv);
...
function showInLogBubble(text,timer,color){
	...
	var newdiv = createElement("div",{"class":"blackbox","viewendtime":(now+timer),"style":"background-color:"+color+";border:2px solid black;-moz-border-radius:10px;padding:5px;margin-top:5px;"},LOG_BUBBLE_BOX,getDaytimeStr(now)+"&nbsp;"+text);
}
