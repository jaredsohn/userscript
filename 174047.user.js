...
// @name           e-sim HR WIP
// @namespace      e-sim HR
// @description    e-Sim CRO WIP translating from persian
// @version        1.0
// @include        http://*.e-sim.org*
// @icon 		   http://e-sim.home.pl/testura/img/favicon.png
// @grant          none
// ==/UserScript==
//Hrvatski prijevod napraio cortex18
//Credits for Sahand
//

menuLinkReplacements = {
	"work.html"				:["Work","Posao"],
	"train.html"			:["Train","Trening"],
	"equipment.html"		:["Equipment","Oprema"],
	"companies.html"		:["Companies","Firme"],
	"newspaper.html"		:["Newspaper","Novine"],
	"myParty.html"			:["Party","Stranka"],
	"contracts.html"		:["Contracts","Ugovori"],
	"myShares.html"		    :["Shares","Dionice"],
	"myAuctions.html"		:["Auctions","Aukcije"],
        "inviteFriends.html"	:["Invite friends","Pozovi prijatelje"],
	"myMilitaryUnit.html"	:["Military unit","Vojska"],
	"subscription.html"     :["Premium account","Premium račun"],
	
	"productMarket.html"	:["Product market","Trgovina"],
	"jobMarket.html"		:["Job market","Burza"],
	"monetaryMarket.html"	:["Monetary market","Mjenjačnica"],

	"auctions.html"		    :["Auctions","Aukcije"],
	"stockMarket.html"	    :["Stock market","Stock Market"],
	"companiesForSale.html"	:["Companies for sale","Firme na prodaji"],
	"specialItems.html"		:["Special items","Specijalne stvari"],
	
	"countryStatistics.html"		:["Country statistics","Statistike država"],
	"partyStatistics.html"			:["Party statistics","Statatistike stranaka"],
	"newspaperStatistics.html"		:["Newspaper statistics","Statistike novina"],
	"citizenStatistics.html"		:["Citizen statistics","Statistike građana"],
	"militaryUnitStatistics.html"	:["Military unit stats","Statistike vojski"],
        "stockCompanyStatistics.html"	:["stock market stats",""],
	"donations.html"                :["Donations","Donacijeا"],
	
"news.html\?newsType=TOP_ARTICLES"		:["Top articles","Najbolji članci"],
	"news.html\?newsType=LATEST_ARTICLES"	:["Latest articles","Najnoviji članci"],
	"news.html\?newsType=MILITARY_EVENTS"	:["Military events","Vojna događanja"],
	"news.html\?newsType=POLITICAL_EVENTS"	:["Political events","Politička događanja"],
	
	"battles.html"							:["Battles","Borbe"],
	"countryPoliticalStatistics.html"		:["War and politics","Ratovi i politika"],
	"countryEconomyStatistics.html"			:["Economy","Ekonomija"],
	"countryLaws.html"						:["Laws","Zakoni"],
	"partyElections.html"					:["Party elections","Stranački izbori"],
	"congressElections.html"				:["Congress elections","Kongresni izbori"],
	"presidentElections.html"				:["Presidential elections","Predsjednički izbori"],
	"pendingCitizenshipApplications.html"	:["Citizenship","Državljanstvo"],
	"googleMap.html"						:["Map","Karta"],

};

menuTextReplacements = {
	"myPlacesButton":["My places","Moja mjesta"],
	"marketButton":["Market","Tržnica"],
	"statisticsButton":["Statistics","Statistike"],
	"newsButton":["News","Novosti"],
	"electionsButton":["Country","Država"]
};

hpTitleReplacements = {
	"News":["News","Novosti"],
	"Shouts":["Shouts","Statusi"],
	"Battles":["Battles","Borbe"],
	"Events":["Events","Događanja"]
};

hpTabsReplacements = {
	"#topArticles":["Global","Global"],
	"#latestArticles":["Latest","Najnoviji"],
	"#localArticles":["Local","Lokalni"],
	
	"#countryShouts":["Country","Država"],
	"#friendsShouts":["Military unit","Vojska"],
	"#myShouts":["Friends","Prijatelji"],
	
	"#localBattles":["Country","Državne borbe"],
	"#substidedBattles":["Subsidized","Plaćene borbe"],
	"#hotBattles":["Important","Važne borbe"],

	"#localEvents":["Military","Vojni"],
	"#globalEvents":["Military","Vojni"],
	"#politicalEvents":["Political","Politički"]
};
...
MonetaryMarketRemplacement = {
	"http://wiki.e-sim.org/en/MonetaryMarket"	:["Monetary market tutorial","Tutorijal za mjenjačnicu"],
};
...
function replaceAlliesList(k) {
	...
	for (var i = 0; i < results.snapshotLength; i++) {
		obj = results.snapshotItem(i);
		obj.innerHTML = obj.innerHTML.replace(/(\d*)( allies)/,"$1 saveznika");
	}
}
...
function replaceNoAlliesAndOtherInfo() {
	...
	for (var i = 0; i < results.snapshotLength; i++) {
		...
		if (tmp.innerHTML.match("no allies")) {
			...
			tmp.nodeValue=tmp.nodeValue.replace(/no allies/,"nema saveznika");
		}
		...
		tmp.innerHTML=tmp.innerHTML.replace(/Resistance war/,"Pokret otpora");
		tmp=results.snapshotItem(i).children[4];
		tmp.innerHTML=tmp.innerHTML.replace(/Subsidies/,"Poticaj");
		tmp.innerHTML=tmp.innerHTML.replace(/none/g,"nema");
