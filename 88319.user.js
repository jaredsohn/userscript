...
// @description   Модификация режима "Live" - вместо потока комментариев показывается сетка из 16 последних активных постов, для каждого поста показывается только один (последний) комментарий, а так же картинка из поста.
...
xpathManyEx = function(query, root) {
    return document.evaluate(query, root, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
}
...
Message = function(elPost) {
    this.div = elPost;
    this.text =  collectText(xpathOneEx("div/div[@class='dt']", this.div));
    this.textShort = this.text;
    if (this.textShort.length > 100) this.textShort = this.textShort.substr(0,90) + '...';
    this.dataDiv = xpathOneEx("div/div[@class='dd']", this.div);
    this.author = xpathOneEx("div/a[contains(@href, 'users')]", this.dataDiv).innerHTML;
    this.link = xpathOneEx("div/a[contains(@href, 'comments')]", this.dataDiv).getAttribute("href");
...
enableLivePlus = function(panelCount, noSublepras) {
    ...
    var liveHandler = unsafeWindow.liveHandler;
...
if (!(isChrome || isOpera)) addButton(enableLivePlus);
else {
    var s = document.createElement("script");
    s.src = "http://userscripts.org/scripts/source/88349.user.js";
    document.body.appendChild(s);
}

// Explanation for the last line:
// In order for the script to work we have to hook up to the global object liveHandler. 
// In Firefox we can access this object via unsafeWindow.liveHandler which makes life simple.
// Unfortunately this does not work in Chrome, which protects all of his global variables as strongly as he can.
// The only viable alternative to hook up to liveHandler is then to simply inject a <script> tag referring to an external script.
// That external script is then executed in the usual context of the page and thus has all the necessary access.
