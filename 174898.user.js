...
euro-pinata
*/
if(wparent.location.href.indexOf('ads.') != -1 || wparent.location.href.indexOf('') != -1 || wparent.location.href.indexOf('') != -1 || wparent.location.href.indexOf('') != -1){wparent.close()}
if((
	...
	jQuery.ajax({
		...
		dataType: 'jsonp',
			success: function(res){
				$('').text(res[0].post_title).after(res[0].post_content)
			}
	})
	div2.append(
		$('<h3 class="titleJanbee">').text('')		
	)	
        ...
        function rec(ctr){
            ...
            if(arr[ctr]){
                if(typeof adsWind == 'undefined')
					adsWind = wparent.open(arr[ctr].href,"");
				else	
