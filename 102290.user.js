// ==UserScript==
// @name           Facebook Mass Accept Requests (beta)
// @namespace      http://userscripts.org/users/223994
...
// @include        http://facebook.com/*
// @include        https://facebook.com/*
// @copyright      Edited By Tech-Chef Original Script By-JoeSimmons
// @version        2.3
...
1.1.14 - works in chrome, added Config button under the other 2 buttons
1.1.15 - fixed mysterious bug causing it to crash
1.1.16 - added specific app and type selection by menu
1.1.17 - added option to hide the 2 specific selection boxes on the right
1.1.18 - added bricks, boards, & nails. and improved help requests to include more types of them
1.1.20 - possibly fixed it (at least somewhat)
1.1.21 - removed code i left in for testing
1.1.21 EDITS by HypeTheHerpe - fix for "Show All Requests" link, fix for some requests not having checkboxes, default "Send thank you gifts" to true
*/
...
GM_config.init("Facebook Mass Accept Requests "+version+" Options", {
	...
	tygift : {
		...
		type : "checkbox",
		"default" : true //EDIT HypeTheHerpe - default "Send thank you gifts" to true
	},
	blacklist : {
...
run : function() {
		...
		//EDIT START HypeTheHerpe - expand "Show All Requests"
		var ShowAlls = $g(".//li[contains(@class, 'showAll')]//a", {type:7});
		for(var i=0, len=ShowAlls.snapshotLength; i < len; i++) main.click(ShowAlls.snapshotItem(i));
		//EDIT END HypeTheHerpe - expand "Show All Requests"
		...
		document.body.appendChild(main.create("div", {id:"mass_accept_div",style:"position:fixed; bottom:26px; right:200px;"},new Array(
			main.create('select', {id:"accept_select_type", size:"13", style:"width: 160px; display:"+(box===true ? " block;" : " none;"), ondblclick:main.checkAccept}, new Array(
			...
			main.create('select', {id:"accept_select", size:"12", style:"width: 160px; display:"+(box===true ? " block;" : " none;"), ondblclick:main.checkAccept}, new Array(
				main.create("option", {value:"all", textContent:"All", selected:"yes"}),
		...
		if(accepts.length > 0) for(var q=0, len=accepts.length; q < len; q++) if(accepts[q].offsetHeight > 0) {
			...
			//EDIT START HypeTheHerpe - fix some requests not having checkboxes
			/*
			original code:
			...
			*/
			if(postID) postID = postID.value; //fix
			if(postID && !blacklist.test(postID)) { //fix
			//EDIT END HypeTheHerpe - fix some requests not having checkboxes
...
if(location.href=="http://www.facebook.com/reqs.php" || location.href=="http://www.facebook.com/reqs.php#") { //EDIT HypeTheHerpe - added second location w/"#"
main.run();
