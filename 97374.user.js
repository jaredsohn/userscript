// ==UserScript==
// @name             KOCAttack FRANCAIS
// @version          0.8.8 FR
// @namespace        KOCAttack-Extra
// @description      KOC Attack 0.8.8 traduis par mimi28700
...
Traduction par BrunoLeGrand !
Le 18/02/2011 - + implementation de la nouvelle version pour correction bug 404
Mise � jour via le script 94200 avec encore plus d'options :) lol
*/
...
var KOCAttack={
	...
	ShowImportExportBox:function() {
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  h += '<td>Type</td><td>Unit&eacute; Attaque</td><td>Unit&eacute; Suicide/Ressources</td><td>Heure</td><td>Ignorer</td></tr>';
		  var tableRows= '';
		  var count = 1
	...
	ChangeIgnore:function(e){
	  ...
	  if(this.currentServerId<0){
		ServerId = GM_getValue("KOCAttackLastKnownServerID",-1);
	  }
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"Temps avant de re-attaquer la <u>meme cible</u>...<br />"+
			"<div style='margin-left: 40px'>"+
			"Attaque sur Barbares :<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />hrs<br />"+
			"Attaque sur TS :<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />hrs<br />"+
			" Attaque sur Villes :<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />hrs<br />"+
			" Transport :<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />mins"+
			"</div>"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> distance maximum de la ville la plus loin a attaquer.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Lancer les attaques seulement de la ville d'ou elles ont ete lancees en premier.<br />"+
			"<br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> R&eacute;essayer si une erreur inconnu ou un trafic exesif survient (Rafr&eacute;chir apr&egrave;s avoir chang&eacute; cette option).<br />"+
			...
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Nombres maximum de rapports d\'attaque &agrave; conserver dans la boite de messages.<br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> D&eacute;sactiver l'affichage des rapports, serons aussi d&eacute;sactiver la collecte des rapports d'attaque.<br />"+
			
			"</td></tr>"+
			"<tr><td valign='top' align='center'><img src='img/happiness.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% de la population inactive disponible avant d\'inissier la formation automatique.<br />"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% de bonheur avant d\'inissialiser la collecte d\'or automatique<br />"+
			"</td></tr>"+
			...
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Masquer les demandes d\'aide dans le tchat (ci-dessus est coch&eactue;e, le faire apres les avoir aid&eactue;).<br />"+
			...
			"If above is checked, what privacy setting should we use? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Tout le monde</option><option value='50'>Amis et leur amis</option><option value='40'>Amis seulement</option><option value='10'>Moi uniquement</option></select><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gems.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Reconnecter automatiquement au bon domaine s'il y a une maintenance sur le jeu.<br />"+
		...
		"<TABLE width=100%><TR><TD align=center>"+
			...
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Supprimer Toutes les attaques</span></a><a id='KOCAttackDeleteTransports' class='button20'><span>Effacer les transports</span></a><a id='KOCAttackListAttacks' class='button20'><span>Liste attaques</span></a>"+
			"<a id='KOCAttackListTransports' class='button20'><span>Liste des transports</span></a></td><TD align=right>"+ KOCAversion +"</td></tr></table></form>";
	...
	var t=this;
		...
		for(var c=1; c<=8; c++) {
			...
			var SetCity=function(target,set) {
				target.style.border=set?'2px solid #0F0':'';
...
bulkAddTable.insertRow(-1).insertCell(-1).appendChild(bulkAddDiv);
		...
		AddHtml(bulkAddDiv,"Copier et coller vos coords ici (ex. 343,434)<br />Note: it will only add the target using the current number of troops on this screen.<br /><input id='KOCAttackBulkAddForce' type='checkbox' /> Overwrite existing attack if one already exists<br />");
		...
		var arrData = [["Camp", "Barbares"],["Wilderness", "TS"]];
		...
		AddHtml(bulkAddDiv,"<input id='KOCAttackBulkAddSuicideWave' type='checkbox' onClick='javascript:document.getElementById(\"KOCAttackBulkAddLocationType_Wilderness\").checked=true;' /> Ceci est la premiere vague -suicide- sur une TS.<br />");
		...
		bulkAdd.innerHTML='<span>Ajouter Coords</span>';
		...
		bulkAddAttackLink.innerHTML='<span>Ajouter Coords</span>';
    ...
    return;
		...
		var a=document.createElement('a');
		a.innerHTML="Montrer/Cacher les objets d'effort d'Attaque";
		//a.href='javascript:;';
	...
	OnAttackBoxAppear:function(box) {
		...
		AddText(div2,'Ignorer dans la liste des attaques');
