...
// @namespace       smth
// @description     Circle of Trust military coordination script
// @version         1.0.5
...
function announce_init_blocks() {
    if ($('#orderContainer').length > 0) {
        ...
        GM_xmlhttpRequest({
            method  : 'Post',
            url : 'http://198.211.119.68:3838/cot',
            data : "id=" + parseFloat(unsafeWindow.flc.getVariable("citizen_id")),
                ...
                if (data.responseText) {
                    try {
                        ...
                        if (responce) {
                            ...
                            $('.announce-blocks-marquee').html(responce["text"]);
                        } else {
...
function announce_blocks() {
    ...
    $('#announce-blocks-panel').append('<div class="announce-blocks-announces"></div>');
        ...
        $('#announce-blocks-panel').find('div.announce-blocks-announces').append('<div class="announce-annouces-content"></div>');
            $('#announce-blocks-panel').find('div.announce-annouces-content').append('<h1>CoT HQ:</h1>');
