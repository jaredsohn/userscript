...
const MAX_CAPTCHAS_TO_LOAD = 250;
var requestTimer = {};
var captchaQueue = [];
...
function updateServerStatus() {
  ...
  if (averageRequestTime < 300) {
    ...
    serverStatusBox.textContent = 'lento';
  } else if (averageRequestTime < 2000) {
    serverStatusBox.textContent = 'super lento';
  } else {
    serverStatusBox.textContent = 'super duper lento';
  }
}
...
function updateStats(response) {
  try {
    ...
    if (moraleText) {
     ...
     var goldText = extractText(response, "Gold:", '</font>');
    if (goldText) {
      goldStatusBox.innerHTML = goldText.replace(/\s/g, '')+"</font>";
    }
...
function initializeRecruiter() {
  ...
  document.body.innerHTML =
      '<div id="linkBar">' +
      '  <span style="color:#fff">YAR v ' + VERSION +
      '  </span> &#xbb; ' +
      ...
      '    Cargados: <span id="imageStatusBox">0</span><br>' +
      '    Links en cola: <span id="linkListLength">0</span><br>' +
      '    Moral: <span id="moraleStatusBox">-</span><br>' +
      '    Oro: <span id="goldStatusBox">-</span><br>' +
...
// document.getElementById('Open1').addEventListener(
  ...
  goldStatusBox = document.getElementById('goldStatusBox');
