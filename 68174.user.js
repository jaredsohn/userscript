// ==UserScript==
// @name           			TV Calendar Reloaded for NZB
// @namespace     			localhost
...
// @homepage				http://userscripts.org/scripts/show/68174
...
autoUpdate (68174, "2.28");

var SEARCH_LINK_TITLE = "ezRSS Search Link";
var SEARCH_LINK_VALUE = "ezRSS";
var SEARCH_NZBS_LINK_TITLE = "NZBS Search Link";
var SEARCH_NZBS_LINK_VALUE = "NZBS";
var SUBTITLE_LINK_TITLE = "TVSubtitles.net Search Link";
var SUBTITLE_LINK_VALUE = "TVSubtitles";
...
var SHOW_FILENAME = getSetGMVariable("SHOW_FILENAME");		//true will display the filename
//End of Properties
...
var NZBS_SEARCH_URL = "http://nzbs.org/index.php?action=search"        
              + "&q=" + SHOW_NAME_TEXT
              + "&catid=t1&age=";
              
var SUBTITLE_SEARCH_URL = "http://www.tvsubtitles.net/search.php"
              + "?q=" + SHOW_NAME_TEXT
...
function runThroughDays(days, i)
{
	var shows = days[i].getElementsByClassName('ep');
		...
		for (var y = 0; y < shows.length; y++)
		{
				...
				var episodeName = shows[y].getElementsByTagName('a')[0].innerHTML;
				
				console.log("episodeName = " + episodeName);
				
				//Retrieves the shows season and episode numbers
				var episodeElement = shows[y].getElementsByTagName('a')[1];//shows[y].parentNode.getElementsByClassName('seasep')[0].childNodes[1];
				...
				{
					episodeElement.innerHTML = episodeElement.innerHTML + createSearchLink(episodeName, seasonNumber, episodeNumber);
				}
				episodeElement.innerHTML = episodeElement.innerHTML + "<br/>Subtitle: " + createSubtitleLink(episodeName, seasonNumber, episodeNumber);
...
function createSearchLink(showName, seasonNumber, episodeNumber)
{
  ...
  //Inserts the show details into the torrent search string
	url = url.replace(SHOW_NAME_TEXT, escape(showName));
  searchLink += ', <a href="' + url + '" target="_blank" title="' + SEARCH_NZBS_LINK_TITLE + '" style="color: white; font-weight: normal">' + SEARCH_NZBS_LINK_VALUE + '</a>';
	
	return searchLink;
}

function createSubtitleLink(showName, seasonNumber, episodeNumber)
{
	var url = SUBTITLE_SEARCH_URL;
	...
	var searchLink = '<a href="' + url + '" target="_blank" title="' + SUBTITLE_LINK_TITLE + '" style="color: white; font-weight: normal">' + SUBTITLE_LINK_VALUE + '</a>';
