// ==UserScript==
// @name           torrentz.com_More_Comments fix
...
//original script by hosts: http://userscripts.org/scripts/show/20894

var lianks = document.evaluate("//div[@class='download']/dl/dt/a", document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
...
var thepiratebayse = new Array(6)
thepiratebayse[0] = "thepiratebay.se"; //name and search string in link to get description
thepiratebayse[1] = '<div class="nfo">.*?<\/div>'; //descriptionxpath
thepiratebayse[2] = 'same';
thepiratebayse[3] = '';
thepiratebayse[4] = '';
thepiratebayse[5] = '<div id="comments">.*?<div class="ads" '; //commentsxpath
...
var torrents = [thepiratebay, thepiratebayse];
...
function AddDescription(name, description) {
    ...
    newdescriptions.innerHTML = '<h2>Description from ' + name + '</h2><div class="usdescription">' + '<p style="padding: 2px; float: right; font-size: 0.9em;"><span style="color:#588FC7;"><strong>' + name + '</strong> description</span></p>' + description + '</div>';
...
function AddComments(name, comments) {
    if (comments != null) {
        ...
        newcomments.innerHTML = '<h2>Comments from ' + name + '</h2><div class="uscomment">' + '<p style="padding: 2px; float: right; font-size: 0.9em;"><span style="color:#336699;"><strong>' + name + '</strong> comments</span></p>' + comments + '</div>';
...
GM_addStyle('div.uscomments {background:#336699 url(/img/ctr.gif) no-repeat scroll right top;} ');
...
//GM_addStyle('div.usdescriptions {background:#588FC7 url(/img/ctr.gif) no-repeat scroll right top;margin:20px 245px 20px 0pt;clear: right;} ');
GM_addStyle('div.usdescriptions {background:#588FC7 url(/img/ctr.gif) no-repeat scroll right top;clear: right;} ');
...
GM_addStyle('pre {white-space: pre-wrap; word-wrap: break-word; font-family:"Courier New",Courier,monospace;font-size:11px; }');
