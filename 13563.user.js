// Barra de formatação para FireFox + Greasemonkey
// As configurações são feitas na própria página do orkut
// Se achar bugs, ou quiser dar sugestões só me deixar um scrap
// Esse script é open-source, você pode editar como quiser
// Entre na comunidade para saber as auterações que foram feitas
// http://www.orkut.com/Community.aspx?cmm=32166398
//
// Agradecimentos:
// Amey =D
//
// ==UserScript==
// @name           Amey's toolbar
// @author         Link <http://www.orkut.com/Profile.aspx?uid=10912831631714212098>
...
L=[];
Lang="Spreading|Divulgação|Add Signature|Adicionar Assinatura|Edit|Editar|Delete|Deletar|Change to Default|Tornar Padrão|Close|Fechar|C|P|This post was edited with the|Este post foi editado com a|This signature was already added, do you want add it again?|Essa assinatura ja foi adicionada, deseja adicionar novamente?|Do you really want to delete|Deseja mesmo deletar|Title:|Título:|Signature:|Assinatura:|Cancel|Cancelar|Bold|Negrito|Italic|Itálico|Underline|Sublinhado|Menu Color|Menu Cor|with Title|com Título|Image|Imagem|Signature , Right Click to Edit|Assinatura , Clique com Botão Direito para Editar|Search Friends|Buscar Amigos|Search Communities|Buscar Comunidades|Recover Lost Text|Recuperar Texto Perdido|Settings|Configurações|Profile|Perfil|Edit Profile|Editar Perfil|Album|Álbum|Marks|Listas|Scrapbook|Recados|Testimonials|Depoimentos|Fans|Fãs|Messages|Mensagens|Friends|Amigos|Communities|Comunidades|Authors Profile|Perfil do Autor|Oficial Community|Comunidade Oficial|Troca a Língua para o Português|Change the language to English|Ctr+Click open in a new tab , Shift+Click open in a new page|Ctr+Clique abre em uma nova aba , Shift+Clique abre em uma nova página|Loading...|Carregando...|There is not community|Não há comunidade|Type the address|Digite o endereço|Type the name of the link|Digite o nome do link|Type the name of your friend|Digite o nome do seu amigo|Type the name of the Community|Digite o nome da comunidade|Nothing was selected, type something that you want translate|Nada foi selecionado, digite algo que queira traduzir|Translation|Tradução|Preview|Pré Visualização|Automatic Signature in the Scraps|Assinatura Automática nos Scraps|Automatic Signature in the Posts|Assinatura Automática nos Posts|Color:|Cor:";
Lang=Lang.split('|');
y=0;
...
var ver="0904"; /* Versão deste escript 0904 */
var whoid=String(document.cookie.match(/id=\d+/i)).replace(/ID=/,''); // Aqui pega o seu UID, não tem nada a ver com roubo de auth por cookies, ok? XD
var sbutton="style=\"text-decoration: none; color: black\""; //estilo dos botões
...
if(VC('bf.'+whoid)==undefined){SC('bf.'+whoid,'-1')}; // se não tiver nada gravado no cookie grava o valor -1
...
sainsp=contsainfull.split(','); // da split
	...
	contmenusain+="<tr><td width=\"80\"><a href=\"javascript://\" onclick=\"addsaintxt("+i+")\" "+sbutton+"><div>"+sbold+namesain[i]+"</b></div></a></td>"+ // Título
	"<td><a href=\"javascript://\" onclick=\"editassinatura("+i+")\" title=\""+L[2]+" &#34;"+namesain[i]+"&#34;\" "+sbutton+"><div>[E]</div></a></td>"+ // Botão Editar
	"<td><a href=\"javascript://\" onclick=\"deleteassinatura("+i+")\" title=\""+L[3]+" &#34;"+namesain[i]+"&#34;\" "+sbutton+"><div>[D]</div></a></td>"+ // Botão Deletar
	"<td><a href=\"javascript://\" onclick=\"assinaturapadrao("+i+")\" title=\""+L[4]+" &#34;"+namesain[i]+"&#34;\" "+sbutton+"><div>["+L[6]+"]</div></a></td></tr>"; // Botão Tornar Padrão
	};
if(contsainfull.substring(0,2)=="-1"){sbold2="<b>"}else{sbold2=""}; // se o n da assinatura padrao for -1 a assinatura será a divulgacao
}
loadmenuassinatura();
...
bmenuf();
	...
	wih=wih-10;
	gebi('bf.divassinatura').scrollTop=1; // verifica se há barra
...
if(n==-1){saindivulgacao();return;} // se o numero for -1 adicionará a assinatura de divulgacao
	if(txta(0).value.indexOf(contsain[n])>-1) // se a assinatura ja foi adicionada, pedirá confirmacao
	{
		if(confirm(L[8])==true)
		{
		txta(0).value+="\n\n"+contsain[n];
...
{
saind="[b]"+L[7]+" [link=http://www.orkut.com/Community.aspx?cmm=8515367][Blue]Barra de Formatação[/Blue][/link] [Red]v."+ver+"[/Red] by [link=http://www.orkut.com/Profile.aspx?uid=15998896938353128878][Green]Link[/Green][/link][/b]";
...
"<a href=\"javascript://\" onclick=\"addassinatura()\" "+sbutton+">[OK]</a> "+ // botão ok
"<a href=\"javascript://\" onclick=\"gebi('bf.assinatura').innerHTML=''\" "+sbutton+">["+L[12]+"]</a>"+ // botão cancelar
bmenuf();
}
...
contsainfull+=","+editsn+","+editsc; // adiciona a assinatura no fim da variável
...
//--------------------------------------------
// Adicionar a assinatura padrão
//--------------------------------------------
function addassinaturapadrao()
{
sainsp=contsainfull.split(','); // pega o no. da assinatura padrao
addsaintxt(sainsp[0]); // chama a função addsaintxt() pra jogar pro txtarea
}
//--------------------------------------------
// Função principal usada em algumas funções como Insert(),smilie();
//--------------------------------------------
function mainInsert()
{
...
{
	if(!txta(0)){return;} // se nao tiver uma txtarea nao vai
	mainInsert(); // chama a função principal
...
xml.send(null);
	...
	// FUNÇÃO PARA SUBIR E DESCER O MENU COMUNIDADES
function mcup()
{
...
{
if(!esrc.match(/^http:/)){esrc="data:image/gif;base64,"+esrc}; // se não tiver o "http://" ele usa base64 pra add a figura
...
try{txta(0).value=btoa(txta(0).value)}catch (e){alert('Não foi possível a codificação');}return false;
}
...
//--------------------------------------------
// Janela tradução e previsualização
//--------------------------------------------
function tjan(T)
{
