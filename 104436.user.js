// ==UserScript==
// @name           Tumblr Follow
// @description    See who you follow that doesn't follow you back and who follows you that you don't follow back.
// @namespace      http://userscripts.org/users/344103
// @creator        lostnaked
// ==/UserScript==
...
var fbcss = '#facebox{position:absolute;top:0;left:0;z-index:100;text-align:left;} #facebox .fbpopup{position:relative;border:3px solid rgba(0,0,0,0);-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px;-webkit-box-shadow:0 0 18px rgba(0,0,0,0.4);-moz-box-shadow:0 0 18px rgba(0,0,0,0.4);box-shadow:0 0 18px rgba(0,0,0,0.4);} #facebox .fbcontent{display:table;width:370px;padding:10px;background:#fff;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;} #facebox .followwhobox { width:550px !important; } #facebox .fbcontent > p:first-child{margin-top:0;} #facebox .fbcontent > p:last-child{margin-bottom:0;} #facebox .fbclose{position:absolute;top:5px;right:5px;padding:2px;background:#fff;} #facebox .fbclose img{opacity:0.3;} #facebox .fbclose:hover img{opacity:1.0;} #facebox .fbloading{text-align:center;} #facebox .fbimage{text-align:center;} #facebox img{border:0;margin:0;} #facebox_overlay{position:fixed;top:0px;left:0px;height:100%;width:100%;} .facebox_hide{z-index:-100;} .facebox_overlayBG{background-color:#000;z-index:99;} .facebox p{margin:10px 0;}';

var fwcss = '#followwhotable { width:100%; border-collapse:separate; } #followwhotable th { background-color:#444444 !important;color:#FFFFFF !important; } #followwhotable a { text-decoration:none; } #followwhotable a:hover { font-style:italic !important; } #followwhotable th { width:50%; } #followwhotable td { font-size:80%; width:50%; } #followwhotable .fw-greyrow { background-color:#DDDDDD; } #followwhotable td.fw-last { border-bottom:1px solid #444444 !important; }';
...
function tfc_init() {
   jQ("body").append('<div id="113977_followwhodisplay" style="display:none;"><div style="font:bold 24px Georgia,serif;color:#1f354c;">followcheckr.</div><div class="followwholist" style="height:' + ((getPageHeight()/10)*7) + 'px;overflow-y:auto;text-align:center;margin-top:10px;"></div></div>');

   jQ("head").append('<style type="text/css">' + "\n" + fbcss + "\n" + fwcss + "\n" + '</style>'); 
   var fl = jQ('#right_column').find('a[href$="/followers"]');

   var uf = jQ("#113977_unfollowdelta");
   var notintxt = '<a id="113977_followwhonotin" class="tracked_tag_control" onclick="return false;" href="#">&rho;</a>';
   ...
   jQ('#113977_followwhonotin').click(function() {
      followers = jQ(this).parent().children("a:first").html().match(/([0-9][0-9,\.]*)/);
      followees = jQ('#dashboard_nav_following').children("a:first").html().match(/[\s>]([0-9][0-9,\.]*)/);
   ...
   jQ(document).ajaxError(function(e, xhr, settings) {
      if (!failed && (/\/followers\/page\/[0-9]*/.test(settings.url) ||
                      /\/following\/page\/[0-9]*/.test(settings.url))) {
         failed = true;
         jQ('#113977_followwhodisplay .followwholist').html('<p><em>There was an error, please try again later.</em></p>');
         jQ.facebox({ div: '#113977_followwhodisplay' }, 'followwhobox');
      }
   });
}
...
function doFWGet(followers, followees, show) {
   ...
   if (show) {
      jQ('#113977_followwhodisplay .followwholist').html('<p><img src="' + limg + '" /></p>');
      jQ.facebox({ div: '#113977_followwhodisplay' }, 'followwhobox');
   }
   ...
   for (i=0; i<followerpages; i++) {
      jQ.get('/followers/page/'+(i+1), function(data) {
         if (/id="dashboard_followers"/.test(data) == false) {
            if (!failed) {
               failed = true;
               jQ('#113977_followwhodisplay .followwholist').html('<p><em>There was an error getting the list of tumblrs that follow you, please try again later.</em></p>');
               jQ.facebox({ div: '#113977_followwhodisplay' }, 'followwhobox');
            }
            return true;
         }
         ...
         var j = 0;
         var pg = data.match(/\/followers\/page\/([0-9]*)[^>]*">&larr;/m);
         if (pg == undefined || pg == null || pg.length < 2)
            j = 0;
         else
            j = Number(pg[1]);
   ...
   for (i=0; i<followeepages; i++) {
      jQ.get('/following/page/'+(i+1), function(data) {
         if (/id="dashboard_following"/.test(data) == false) {
            if (!failed) {
               failed = true;
               jQ('#113977_followwhodisplay .followwholist').html('<p><em>There was an error getting the list of tumblrs you follow, please try again later.</em></p>');
               jQ.facebox({ div: '#113977_followwhodisplay' }, 'followwhobox');
            }
            return true;
         }
         ...
         var j = 0;
         var pg = data.match(/\/following\/page\/([0-9]*)[^>]*">&larr;/m);
         if (pg == undefined || pg == null || pg.length < 2)
            j = 0;
         else
            j = Number(pg[1]);
...
function doFWDisplay(followerstart,followeestart,show) {
   //unsafeWindow.console.log(followerstart + " " + followeestart);
   var fin = true;
   var i,j;
   i=0;
   j=0;
   ...
   if (fin) {
      followerdone = [];
      followeedone = [];
      //unsafeWindow.console.log("followers");
      ...
      //unsafeWindow.console.log("followees");
...
function doFWFinish(followers, followees, show) {
   ...
   txt += '</tr></tbody></table>';
   jQ('#113977_followwhodisplay .followwholist').html(txt);

   followyou = [];
   youfollow = [];
   jQ.facebox({ div: '#113977_followwhodisplay' }, 'followwhobox');
   jQ('#113977_followwhodisplay .followwholist').empty();
}
