// ==UserScript==
// @name             KoC Advanced AutoTools
// @version          0.9.5.4
// @namespace        KoC Advanced AutoTools
// @homepage         http://userscripts.org/scripts/show/101009
// @description      Automation tools for attacking, training, etc.
...
// @require          http://tomchapin.me/auto-updater.php?id=101009
// ==/UserScript==
...
var KOCAttack={
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% of idle population available before we auto train <input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' /># of troops.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">The number of troops that are queued will be the lesser between the number above, and the max available.</span><br />"+
	...
	*/	
	...
	GetOptions:function() {
		...
		var defOptions={"attackDelay":5,
			...
			"attackSecsSinceLastCity":60*60*8,
			"attackSecsSinceLastCamp":3600,
			"attackSecsSinceLastWild":3600,
			"attackSecsSinceLastTransport":5,
			...
			"noViewReports":false,
			"chromeKeepReports":2,
			"percentOfPopToTrain":10,
			"percentToTrainOfMax":100,
			"autoGoldHappiness":100,
	...
	*/
	...
	//GM_log('idleTotal:'+idleTotal+', labourTotal:'+labourTotal+', popneeded:'+popNeeded);	
		if(popAvail>0 && popAvail>=popNeeded) {
            ...
            if(numInp && maxObj) {
                numInp.value=this.options.percentToTrainOfMax; //Math.floor(parseInt(maxObj.textContent)*(parseInt(this.options.percentToTrainOfMax)/100));
                //
                if (parseInt(numInp.value)>parseInt(maxObj.textContent)) {
