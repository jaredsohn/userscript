// ==UserScript==
// @name LWM Progress Bar
// @author Silentio
// @namespace LWM
// @version 0.8 20081025
...
function main(e){      
    ...
    //Enchanters' Guild branches (9 levels)
    ...
    function showExpBar(){
            ...
            for (var i = 25; i < tabelle.snapshotLength; i++){
                ...
                if (player_faction == "") {
                    if (tabelle.snapshotItem(i).childNodes[0].childNodes[0]) {
                        if (tabelle.snapshotItem(i).childNodes[0].childNodes[0].childNodes[0]) {
                            if (tabelle.snapshotItem(i).childNodes[0].childNodes[0].childNodes[0].textContent.search(/\[\d+\] \- (.*)/) > 0) {
                                player_faction = RegExp.$1;
                                if (factions.indexOf(player_faction) < 0) player_faction="";
                            }
                        }
        ...
        if (GM_getValue("SHOW_MERCENARIES_PROGRESS_BAR", true)) {
            ...
            skill_info.innerHTML = skill_info.innerHTML.replace("<br>&nbsp;&nbsp;"+ T('Commanders\' guild'), progress_bar_html +"&nbsp;&nbsp;"+ T('Commanders\' guild'));
        }
	
	//========== Commanders' guild
        if (GM_getValue("SHOW_COMMANDERS_PROGRESS_BAR", true)) {
            lvl_info = skills[12];
            lvl_info.search(/\((\d*.?\d*)\)/);
            exp_attuale = RegExp.$1;
    
            progress_bar_html = makeProgressBar(exp_attuale, "", commander_guild_lvl);
