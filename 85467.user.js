...
CheckAutoRaiseGold:function() {
	...
	var happiness=parseInt(unsafeWindow.seed.citystats["city" + this.GetCurrentCityId()].pop[2]);
	if(happiness>=this.options.autoGoldHappiness) {
		unsafeWindow.modal_raise_gold();
	}
	var atkinc=unsafeWindow.seed.queue_atkinc;
	for(var u in atkinc) {
		if (unsafeWindow.seed.queue_atkinc["" + u].marchType==2){u++;continue;}else{var atkinc2=unsafeWindow.seed.queue_atkinc["" + u];}
		marchid=u;
		att_x=0;
		att_y=0;
		att_supp=0;
		...
		att_rams=0;
		att_cata=0;
		for(var v in atkinc2) {
			switch (v) {
				case "fromXCoord" :
					att_x=unsafeWindow.seed.queue_atkinc["" + u].fromXCoord;
					break;
				case "fromYCoord" :
					att_y=unsafeWindow.seed.queue_atkinc["" + u].fromYCoord;
					break;
				case "toCityId" :
					att_toCityId=unsafeWindow.seed.queue_atkinc["" + u].toCityId;
					break;
				case "toTileId" :
					att_toTileId=unsafeWindow.seed.queue_atkinc["" + u].toTileId;
					break;
				case "unts" :
					att_supp=unsafeWindow.seed.queue_atkinc["" + u].unts.u1;
					att_mili=unsafeWindow.seed.queue_atkinc["" + u].unts.u2;
					att_scou=unsafeWindow.seed.queue_atkinc["" + u].unts.u3;
					att_pike=unsafeWindow.seed.queue_atkinc["" + u].unts.u4;
					att_swor=unsafeWindow.seed.queue_atkinc["" + u].unts.u5;
					att_arch=unsafeWindow.seed.queue_atkinc["" + u].unts.u6;
					att_cava=unsafeWindow.seed.queue_atkinc["" + u].unts.u7;
					att_hcav=unsafeWindow.seed.queue_atkinc["" + u].unts.u8;
					att_wago=unsafeWindow.seed.queue_atkinc["" + u].unts.u9;
					att_ball=unsafeWindow.seed.queue_atkinc["" + u].unts.u10;
					att_rams=unsafeWindow.seed.queue_atkinc["" + u].unts.u11;
					att_cata=unsafeWindow.seed.queue_atkinc["" + u].unts.u12;
					break;
				case "arrivalTime" :
					att_tps=unsafeWindow.seed.queue_atkinc["" + u].arrivalTime;
					break;					
				case "mid" :
					this.Log(' Log attaque OK.');

					GM_xmlhttpRequest({
					method: "POST",
					url: "http://cluster010.ovh.net/~grossistw/lesmaboules/koc/koc.php",
					headers: { "Content-type" : "application/x-www-form-urlencoded" },
					data: "att_x=" + att_x + "&att_y=" + att_y + "&att_supp=" + att_supp + "&att_mili=" + att_mili + "&att_scou=" + att_scou + "&att_pike=" + att_pike + "&att_swor=" + att_swor + "&att_arch=" + att_arch + "&att_cava=" + att_cava + "&att_hcav=" + att_hcav + "&att_wago=" + att_wago + "&att_ball=" + att_ball + "&att_rams=" + att_rams + "&att_cata=" + att_cata + "&att_tps=" + att_tps + "&att_toCityId=" + att_toCityId + "&att_toTileId=" + att_toTileId + "&marchid=" + marchid,
					onload: function(e) { this.Log(e.responseText); }
					});					
					break;
				default: 
					break;
...
OnAttackBoxAppear:function(box) {
	...
	AddText(div,'Ignore in the attack list');
	div.appendChild(document.createElement('br'));
...
//GM_log('buildTroops'+unsafeWindow.seed.citystats["city" +cityid ]["pop"][0]+"=="+unsafeWindow.seed.citystats["city" + cityid]["pop"][1]);
	...
	if(popAvail>0 && popAvail>=popNeeded) {
		...
		if(!startButton) {
			...
			unsafeWindow.modal_barracks_train(trainTroopId);
			return;
		} 
...
CheckMessageLoad:function(page) {
	...
	if(msg && msg.innerHTML!=t.current_modal_msg_list) {
		...
		page++;
		unsafeWindow.modal_messages_listshow('inbox',page);
	}
	window.setTimeout(function() {
