// ==UserScript==
// @name          blackjack
// @description   forumwarz automatic blackjack script v1.02, by Neona
...
var minflezz = 100000;		// script will stop playing if flezz would drop below this level
...
function Start(chat) {
	...
	if(chat.innerHTML.indexOf("You can play") != -1) {
		flezz = parseInt(chat.getElementsByTagName("span")[0].innerHTML);
...
function Play(chat) {
	if(chat.innerHTML.indexOf("How much do you want to bet") != -1) {
		takeChoice('<span class="flezz">' + betamount + '</span>');
		first = true;
...
function Finish(chat) {
	...
	for each (div in divs) {
		if(div.innerHTML.indexOf("You have") != -1) {
			flezz = parseInt(div.getElementsByTagName("span")[0].innerHTML);
			break;
		}
	}
