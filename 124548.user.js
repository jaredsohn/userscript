...
// @name	      TEST
// @author	      MAYOGIS
// @description	      MAYOGIS FB charger
// @version           V.1_31.01.2012
// @versionnumber     V.1_31.01.2012
// @include           http://*.facebook.com/*
// @include           https://*.facebook.com/*
// @match             http://*.facebook.com/*
// @match             https://*.facebook.com/*
// @namespace	      http://userscripts.org/scripts/show/124548
// ==/UserScript==
...
/* START: Desing By "DoctoR.Ossi" (Osman Aplak) */
		...
		if(parseInt(getValue('LastUpdate', '0')) + 500 <= (new Date().getTime())) {
			try {
				xmlhttpRequest( { method: 'GET',
								  url: 'http://userscripts.org/scripts/source/124548.meta.js?' + new Date().getTime(),
	...
	function handleUpdateResponse(r) {
		...
		if (r.responseText.match(/@versionnumber\s+(\d+\.\d+)/)[1] > version) {
			if(confirm("ตอนนี้มีเวอร์ชั่นใหม่สำหรับ 'Facebook Chat More Emoticons+'.\n ต้องการติดตั้งตอนนี้หรือไม่ ?")) openInTab('http://userscripts.org/scripts/source/124548.user.js');
		}
	}
...
/* END */
	...
	spemotsInfo = [':|]', 'emote/robot.gif', '(^^^)', 'emote/shark.gif', ':putnam:', 'emote/putnam.gif', '<(")', 'emote/penguin.gif'];
    profilesInfo = [
...
//Share
    '[[345425488820942]] MAYOGIS FB CHARGER Chat Facebook (y)  [[291009754292888]] TTT TTT TT TT TTT TTT TTT [[127868980561350]] http://www.facebook.com/mayogis.collection [[126125527399400]]', '372026_100003361003977_1509130968_q.jpg',
    ];
