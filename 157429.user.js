...
// ==UserScript==
// @name           CyJAX Firefox Dialer
// @namespace      http://cyber-cottage.co.uk
// @description    Finds and links phone numbers on the given page, and allows them to be seamlessly called from your Asterisk server, without navigating away from the page.
// @include        *
// @version       0.1
// ==/UserScript==
...
(function () {
        ...
        const scriptURL = 'http://<serveraddress>/click2call.php';
        ...
        function doCallBack(numb,e) {
                e.preventDefault();
                      GM_xmlhttpRequest({
                      method:"POST",
                      url: scriptURL + '?EXTEN=' + 'SIP/' + username + '&date=' + minuteV + '&hgiy=' + scriptPWD + '&page=' + parts +  '&number=' + String(numb).replace(/[\-\s\/\(\)\.]/g, "")
                });
        }
