// ==UserScript==
// @name           Create Heroes vs Villians game invites.
// @namespace      Heroes vs Villians friends.
// @description    Automatically invites people to your clan that you find on the fight page. Frisno altered the  the original code  from Andy Calderbanks  "Create dragon wars invites" http://userscripts.org/scripts/show/31912; Altered from Frisno's Vampire invite script.
// @include        http://apps.facebook.com/heroesvillains/fight.php
// @include        http://apps.facebook.com/heroesvillains/recruit.php
// @include        http://apps.new.facebook.com/heroesvillains/fight.php
// @include        http://apps.new.facebook.com/heroesvillains/recruit.php
// ==/UserScript==
...
if ((!reqID || since > 86400) && document.location.href.indexOf('fight') != -1) {
	document.location.href = 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/heroesvillains/recruit.php';
	return;
}
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/heroesvillains/stats.php?user=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/heroesvillains/recruit.php?action=create',
