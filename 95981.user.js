...
// @version          0.9.7.1.4
// @namespace        -MM-
// @homepage         http://userscripts.org/scripts/show/95981
// @description      KoC Attack - Extra Feature! Version by -MM-.
...
var KOCAttack={
	...
	ShowViewAttacksDialog:function() {
		...
		//WinLog.write(inspect(cities));
		var citysel ='<select id=srcCity>';
		citysel += '<option value=All>Alle Städte</option>';
		for(var c=0; c<cities.length; c++) {
		...
		levelsel += '<option value="All">Alle Levels</option>';
		for(var c=1; c<=11; c++) {
		...
		typesel += '<option value="bulkadded">Hinzugefügte Koordinaten</option>';
		typesel += '</select>';
		
		div.style.display='block';
		div.innerHTML='';
		var m = '<DIV id="srcAttackOpts" style="height:30px">\
		<TABLE><TR valign=bottom><TD class=xtab width=100 align=center>Suche nach: </td><TD align=left>\
		<SELECT id=srcAttack>\
		<OPTION value=All>Alle</option>\
		<OPTION value=Camp>Barbern Lager</option>\
		<OPTION value=Wilderness>Wildnisse</option>\
		<OPTION value=Grassland>Grassland</option>\
		<OPTION value=Lake>See</option>\
		<OPTION value=Mountains>Berge</option>\
		<OPTION value=Forest>Wälder</option>\
		<OPTION value=Hills>Hügel</option>\
		<OPTION value=Plain>Ebene</option>\
		<OPTION value=City>Städte</option>\
		<OPTION value=Transport>Transport</option>\
		<OPTION value=Unknown>Unbekannt </option>\
		</select></td>\
		<td class=xtab width=100 align=center>Burg: &nbsp; </td>\
		 <td align=left><span id=ptattackcity></span></td>\
		 <td class=xtab width=100 align=center>Markiere Level: &nbsp; </td>\
		 <td align=left><span id=ptattacklevel></span></td>\
		 <td class=xtab width=100 align=center>Markiere Type: &nbsp; </td>\
		 <td align=left><span id=ptattacktype></span></td></tr>\
		</table></div>\
		<a id="KOCAttackViewAttacksCancel" class="button20"><span>Schließen</span></a>\
		<a id="KOCAttackViewAttacksList" class="button20"><span>Angriffs Liste</span></a>\
		<a id="KOCAttackViewAttacksClearList" class="button20"><span>Leere Liste</span></a>\
		<a id="KOCAttackViewAttacksDelete" class="button20"><span>Makierte Löschen</span></a>\
		...
		ById('KOCAttackViewAttacksList').addEventListener('click',function() {
		  ...
		  var h = '<table>';
		  h += '<tr><td><input type=checkbox id=selAllAttacks></td><td>&nbsp;</td><td>City</td><td>Koordinaten</td><td nowrap>What</td>';
		  h += '<td>Art</td><td>Angriffs Truppen</td><td>Selbs Mort Truppen/Resources</td><td>Dist</td><td>Zeit</td><td>Ignorieren</td></tr>';
		  var tableRows= '';
		  var count = 1
	...
	ShowOptionsDialog:function() {
		...
		var attackfirst = '';
		var arrData = [["Barbaren", "Barbaren"],["Städte", "Städte"],["Transporte", "Transporte"],["Wildnisse", "Wildnisse"],["Keine", "Keine"]];
		...
		div.innerHTML="<form><table>"+
			...
			"Agriffs Order: <select id='KOCAttackOrder'><option value='mostTroops'>Die meisten Truppen zuerst</option><option value='closest'>Nächste Ziele zuerst</option></select><br />"+
			"Attack Type: <input type='checkbox' "+(this.options.attackTypeCamp?'checked':'')+" id='KOCAttackTypeCamp'>Babaren Lager "+
			"<input type='checkbox' "+(this.options.attackTypeWild?'checked':'')+" id='KOCAttackTypeWild'>Wildnisse "+
			"<input type='checkbox' "+(this.options.attackTypeCity?'checked':'')+" id='KOCAttackTypeCity'>Städte "+
			...
			"<br><br />"+
			"<input id='KOCAttackDelay' value='"+this.options.attackDelay+"' size='3' /> Sekunden in der die Angriffe ausgesendet werden!<br />"+
			"Die Zeit in der die <u>die Selben</u>Ziele Angegriffen werden!<br />"+
			"<div style='margin-left: 40px'>"+
			"Angriff auf Babaren Lager:<input id='KOCAttackHoursSinceLastCamp' value='"+(this.options.attackSecsSinceLastCamp/(60*60))+"' size='3' />Stunden<br />"+
			"Angriff auf Wildnisse:<input id='KOCAttackHoursSinceLastWild' value='"+(this.options.attackSecsSinceLastWild/(60*60))+"' size='3' />Stunden<br />"+
			" Angriff auf Städte:<input id='KOCAttackHoursSinceLastCity' value='"+(this.options.attackSecsSinceLastCity/(60*60))+"' size='3' />Stunden<br />"+
			" Transporte:<input id='KOCAttackMinsSinceLastTransport' value='"+(this.options.attackSecsSinceLastTransport/(60))+"' size='3' />Minuten<br />"+
			" Beim Transport darauf achten das min. <input id='KOCAttackTransportReserveAmount' value='"+this.options.transportResourcesReserveAmount+"' size='10' /> Resource in der Stadt einbehalten wird! <font color=#FF0000>(NEU)</font>"+
			"</div>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% zufällige Anpassung für alle Verzögerungen (um menschlicher zu schauen).<br />"+
			"<br />"+
			"<input id='KOCAttackMaxDistance' value='"+(this.options.attackMaxDistance)+"' size='3' /> max. Entfernung der Angriffe./Transporte.<br />"+
			"<input id='KOCAttackLockAttackFromCity' type='checkbox' "+(this.options.lockAttackFromCity?'checked':'')+" /> Greift nur aus der Stadt an,in der Stadt zuerst Angriffs Koordienaten eingegeben wurde..(gleiche Koordienaten in mehrere Städten)<br />"+
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> Ritter nur im Angriff einsetzten!<font color=#600000>(setzt bei Verstärkung  keinen Ritter automatisch ein!)</font> <br />"+
			"<input id='KOCAttackRetryMarch' type='checkbox' "+(this.options.retryMarch?'checked':'')+" /> Marsch Abbrechen wenn: unknown/excess traffic error <font color=#600000>(Refresh nötig!)</font><br />"+
			"<input id='KOCAttackImpendingStopAttack' type='checkbox' "+(this.options.impendingStopAttack?'checked':'')+" /> Auto Attack bei angegriffen auf dich Ausschalten.<span style='color:red'> (Auto Attack wird nach dem Angriff NICHT automatisch eingeschaltet! auf eigene Risiko!)</span><br />"+			
			"Öffne diese URL (Link für sound/video Balken) wenn du unter Angriff stehst:<br /><input id='KOCAttackImpendingAttackUrl' size='60' value='"+(this.options.impendingAttackUrl)+"' /><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chrome_message_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackRemoveReports' type='checkbox' "+(this.options.autoRemoveReports?'checked':'')+" /> Automatisches Löschen der Babaren/Widlniss Berichten.<font color=#600000>(Refresh nötig!)</font><br />"+
			"<input id='KOCAttackRemoveFarmReports' type='checkbox' "+(this.options.autoRemoveFarmReports?'checked':'')+" /> Automatisch Löschen der Farm Angriffs Berichte.<font color=#600000>(Städte Angriffe!Refresh nötig!)</font><br />"+
			"<input id='KOCAttackKeepReports' value='"+this.options.keepReports+"' size='3' /> Angriffs Berichte die auf der Angriffs Seite Angezeigt werden. <font color=#600000>(Fenster,aussenden der Truppen)</font><br />"+
			"<input id='KOCAttackNoViewReports' type='checkbox' "+(this.options.noViewReports?'checked':'')+" /> Automatisch Ausschalten der Berichte aufruf. <font color=#600000>(auf der Angriffs Seite sind Berichte deaktiviert)</font><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/chome_alliance_up.png' /></td><td valign='top'>"+
			"<input id='KOCAttackPercentOfPopToTrain' value='"+this.options.percentOfPopToTrain+"' size='3' />% der müßigen verfügbaren Bevölkerung wird Atomatisch Ausgebildet!<input id='KOCAttackPercentToTrainOfMax' value='"+this.options.percentToTrainOfMax+"' size='3' />% bevor max. erreicht ist.<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Ativieren Sie automatisch Truppen Ausbielden in der kaserne.</span><br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Info: geben Sie nie 100% müßigen verfügbaren Bevölkerung ein! Da dieses nie erreicht werden kann somit auch nicht atomatisch Ausgebildet werden kann!.</span><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/gold_30.png' /></td><td valign='top'>"+
			"<input id='KOCAttackAutoGoldHappiness' value='"+this.options.autoGoldHappiness+"' size='3' />% Glücklischkeit wird Gold eingetrieben<br />"+
			"<span style=\"font-size:10px; color:#555; line-height:18px; \">Aktivieren Sie in Ihrem Schloß gold eintreiben Buton!.</span><br />"+
			"</td></tr>"+
			...
			"<input id='KOCAttackAutoHelpAlliance' type='checkbox' "+(this.options.autoHelpAlliance?'checked':'')+" /> Automatisch bei Forschung und Bau Alianz mit Spielern Helfen.<br />"+
			"<input id='KOCAttackHideAllianceHelpRequests' type='checkbox' "+(this.options.hideAllianceHelpRequests?'checked':'')+" /> Forschung und Bauhilfen im Chat nicht Anzeigen! <br />"+
			"<input id='KOCAttackAutoPublishGamePopups' type='checkbox' "+(this.options.autoPublishGamePopups?'checked':'')+" /> Automatisch Veröffentlichen.(sucht hilfe) auf der facebook Start Seite.<br />"+
			"Wer darf die Anfrage sehen ? <select id='KOCAttackAutoPublishPrivacy'><option value='80'>Jeder</option><option value='50'>Freunde von Freunde</option><option value='40'>Friends Only</option><option value='10'>Only Me</option></select><br />"+
			"<input id='KOCAttackAutoLogBackIn' type='checkbox' "+(this.options.autoLogBackIn?'checked':'')+" /> Automatisch neu Laden wenn Facebook oder KoC abstürzt..<br />"+
			"<input id='KOCAttackEnableLogging' type='checkbox' "+(this.options.enableLogging?'checked':'')+" /> in der Firefox fehler console Diagnostic logging einschalten. <font color=#600000>(Bug Reports!)</font><br />"+
			"</td></tr>"+
			
			"<tr><td valign='top' align='center'><img src='img/buildings/castle_lvl10.png' /></td><td valign='top'>"+
			"<input id='KOCAttackChangeCitySecs' value='"+(this.options.changeCitySecs)+"' size='3' /> Sekunden in der die Städte <u>besucht</u> werden.<br />"+
			"Durch alle Städte <input id='KOCAttackCitiesDoneMax' value='"+(this.options.autoAttackCitiesDoneMax)+"' size='3' /> <u>mal</u> besuchen, <u>dann</u> "+
			...
			"Aus diesen Städten soll Angegriffen werden: "+okCitiesHtml+'<br />'+

			"<tr><td valign='top' align='center'></td><td valign='top'>"+
			"<input id='KOCAttackRandom' value='"+this.options.randomPercent+"' size='3' />% zufällige anordnung <font color=#600000>(das es realiestischer ist!)</font><br />"+
			...
			"<a id='KOCAttackOptionsSave' class='button20'><span>Speichern</span></a> <a id='KOCAttackOptionsCancel' class='button20'><span>Schließen</span></a> <a id='KOCAttackOptionsReset' class='button20'><span>Zürücksetzen</span></a> <a id='KOCAttackOptionsResetAll' class='button20'><span>Alles Zürücksetzen!</span></a>"+
			"<a id='KOCAttackDeleteAllStoredAttacks' class='button20'><span>Löschen der Koordinaten</span></a>"+
			"<a id='KOCAttackDeleteTransports' class='button20'><span>Löschen der Transporte</span></a>"+
			//"<a id='KOCAttackListAttacks' class='button20'><span>Angriffs Liste</span></a><a id='KOCAttackListTransports' class='button20'><span>Transporte</span></a>"+
			"</td><TD align=right><a href='http://userscripts.org/scripts/show/95981' target='_blank'>KoC Attack </a><BR>Deutche Version by  <a href='http://userscripts.org/users/287110/scripts' target='_blank'>-MM-</a><BR>Version:<span style='color:red'> "+ KOCAversion +" </span></td></tr></form>";
		var t=this;
