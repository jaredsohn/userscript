...
var VERSION_NAME = "ブラウザ一騎当千Beyond";
var IMG_DIR = "/extend_project/ikki/img/";
var hostname = location.hostname;
var server = hostname.substring(0,2);
...
var isMixi = location.host.match(/^[s|y]\d+\./i) ? true : false;
...
if ( isMixi && OPT_VILLAGE_LIST_BOX )  disp_villageListBox();
...
if(!isMixi && OPT_SMALLBTN)   disp_SmallButton();
...
//イメージ初期化
//////////////////////
var img_mura, img_naisei, img_map, img_ken, img_hanma, img_user, img_mail;
function initImages() {
    if( !OPT_LARGEICON ) {
        ...
        img_ally = 'data:image/gif;base64,'+
                'R0lGODlhCgAKAPcAAAAAAIACAoADA4oXF5sCApsDA5sEBJsMDJAjI5QqKpYvL5k1NZs4OKdPT6lK'+
                'Srlzc+QBAeQNDeQaGv4AAP4CAv8DA/8FBf8JCf4KCv8NDf8PD/8YGOQrK/8lJf4sLP8uLv86Ov9Z'+
                'Wf9tbf9ubv52dv96esKGhu2env+QkAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                ...
                'AAhPAP89MMDhn8GD/xoUiOABxAiEDAhAoFAhAwgUBhkUgDChY4UOJUwoOCABQ0cKFyYgGODgBIkP'+
                'Gixs+CcgAUIRGUL8C7AAIUKePg8CDfovIAA7';
    } else {
        img_mura = 'data:image/gif;base64,'+
        ...
        img_ally = 'data:image/gif;base64,'+
                'R0lGODlhDgAOAPcAAAAAAH8AAIADA4IGBoMICIUMDIYODocPD40dHZQAAJQBAZQLC5cxMZo2Np0y'+
                'MqhSUqlUVKpXV7Zvb7dwcLt4eNMAANMBAdMCAtMHB9MNDdMmJvsAAPsCAvsHB/8AAP8BAf8CAv8H'+
                'B/8ICP8MDP8ODv8PD/8QEP8UFP8XF/8YGPohIf8jI/8sLPoyMv83N9d9ff9DQ/9ERPtiYv9hYf9k'+
                'ZPtxcf9wcP9xcf90dP9/fwAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'+
                ...
                'AAiCAP/9O5AggwyBCBMOTIChBQ0bOBT+o0DQQgcSJU7EUDiBYIUNHjx8KLHCBcKOCT6GDPnBxA2B'+
                'Eg4ouMBhZcgSISVAKLBAg4oQNnF6aIDAgIMXNVikGAHCA85/AgYwkIAwBwwTIlBEDEDggcQZIQVy'+
                'jSBRoAexBMiWRTh2Ldu0bgUGBAA7';
     }
}
//////////////////////
//スタイル初期化
...
function initResources()
{
    ...
    var spns = $a('//span[@class="increase" or @class="resource"]');
    for(var i=0 ; i<spns.length ; i++) {
        var str = spns[i].previousSibling.nodeValue.match(/(学力|体力|武力|糧)\s+(-?[0-9]+)/);
        if( str ) {
            switch( str[1] ) {
            case "学力":
...
//  OPT_REPORT_NP = cloadData( "OPT_REPORT_NP", 1 );
    ...
    OPT_EN_ALERT = cloadData( "OPT_EN_ALERT", 0 );
...
//  OPT_REPORT_NP = cgetCheckBoxValue("OPT_REPORT_NP");
    ...
    OPT_EN_ALERT = cgetCheckBoxValue("OPT_EN_ALERT");
...
//  csaveData( "OPT_REPORT_NP", OPT_REPORT_NP );
    ...
    csaveData( "OPT_EN_ALERT", OPT_EN_ALERT );
...
function addOptionsHtml() {
    ...
    //設定項目
    ccreateCheckBox(td1, "OPT_VILLAGE", OPT_VILLAGE, "建築表示機能","会場の表示で資源不足で建築不可能な場合、LV表示を黄色にします",0);
    ccreateCheckBox(td1, "OPT_BASELINK", OPT_BASELINK, "拠点リンク機能","本拠地や会場のリンクにマップ・内政などのリンクを追加します",0);
    ccreateCheckBox(td1, "OPT_MAPLINK", OPT_MAPLINK, "領地リンク機能","領地や敵地・NPC会場等へのリンク",0);
...
//  ccreateCheckBox(td1, "OPT_DECK", OPT_DECK, "デッキ画面改善","デッキ画面の改善を行います",0);
    ...
    createDistanceBox(td2, OPT_TTDISTANCE_ITEMS, 10);
...
//  ccreateCheckBox(td3, "OPT_REPORT_NP", OPT_REPORT_NP, "同盟ログ前後表示","同盟ログの詳細に、[前のログ]・[次のログ]を追加します（限定的）",0);
    ...
    ccreateCheckBox(td3, "OPT_DELMSG", OPT_DELMSG, "メール/レポート削除機能","メールやレポートの内容ページに、削除ボタンを付けます",0);
    ccreateCheckBox(td3, "OPT_TSENDTIME", OPT_TSENDTIME, "出発時刻計算","出陣画面で、到着希望時間から出発時刻を計算します",0);
    if (!isMixi) {
        ccreateCheckBox(td3, "OPT_SMALLBTN", OPT_SMALLBTN, "小さいボタン","右上の4つのボタンを小さくします。状況も1行にします。",0);
    }

    ccreateCheckBox(td3, "OPT_ATTACKMAP", OPT_ATTACKMAP, "出陣先表示","マップ上に、現在出陣中の目印を付けます",0);
    ccreateCheckBox(td3, "OPT_CARD_CMB", OPT_CARD_CMB, "合成支援ボタン","カード合成後に、同一カードで合成を続けるボタンを追加します",0);
    ...
    ccreateCheckBox(fs, "OPT_PIKA_BLINKBLD", OPT_PIKA_BLINKBLD, "建設中施設の点滅表示","建設中を点滅させます",0);
    ...
    var fs = d.createElement("fieldset");
    var lg = d.createElement("legend");
    lg.appendChild(d.createTextNode("　Presented by ご隠居　") );
    fs.appendChild(lg);
    td3.appendChild(fs);
    ccreateCheckBox(fs, "OPT_EN_ALERT", OPT_EN_ALERT, "アラート抑制","拠点、領地に関するアラートを抑制します（Auto_Bilder等への対策）",0);
    ...
    fs.appendChild(lg);
    td4.appendChild(fs);
    ccreateCheckBox(fs, "OPT_CASTLE_AID", OPT_CASTLE_AID, "拠点援軍ボタン機能","城や分校のリンクの横に援軍ボタンを表示します",0);
...
function disp_baseLink() {
    var elm = $x('id("lodgment")/div[@class="floatInner"] | //div[@class="floatInner clearfix"] | //div[@class="sideBoxInner basename"]');
    if( !elm ) return;
...
function disp_mapLink() {
    ...
    if( location.pathname == "/land.php" ) {
        ...
        var table = $x("//table[@class=\"tables\"]");
        if( table ) {
            var lnk = d.createElement("a");
            lnk.href = "javascript:void(0);";
...
function saveMapLink( ) {
    if( location.pathname == "/land.php" ) {
        ...
        var tmp = d.body.innerHTML.match(/<td>頭首<\/td>[^<]*?<td>(.+)<\/td>/);
        if( !tmp ) return;
        var user_name = tmp[1];
        ...
        tmpHTML = tmpHTML.replace(reg,
            function() {
                var txt = "<table class='tables'><tr><th class='solClass'>　</th>" +
                            ...
                            "</tr><tr><th class='solClass'>下級闘士</th>";
                for(var i = 2 ; i<= 13 ; i++) {
...
//                      cdeleteUserXY(aid);
    ...
    function getDatafromUserHTML(html){
        ...
        tmp = html.match(/\<td\>総生徒数\<\/td\>[\s\S]+?<td\>([0-9]+)\<\/td\>/);
        if( !tmp ) return null;
...
function disp_ResourcesTotal(){
    ...
    var spn = $x('//span[@class="increase" or @class="resource"]');
    if( !spn ) return;
    var box = isMixi ? spn.parentNode : spn.parentNode.parentNode;
...
function disp_RemoveList() {
    ...
    if( location.pathname == "/land.php" && URL_PARAM.x && URL_PARAM.y ) {
        var rmtime = d.body.innerHTML.match(/(現在領地を破棄中です|現在会場を建設中です|現在分校を建設中です).*\n{1,3}(\d+-\d+-\d+ \d+:\d+:\d+)/ );
        if( rmtime ) {
           if( rmtime[1] == "現在領地を破棄中です" ) {
                addList(rmtime[2], 0, URL_PARAM.x, URL_PARAM.y );
            } else if( rmtime[1] == "現在会場を建設中です" ) {
    ...
    if( location.pathname == "/facility/castle.php" ) {
        ...
        var rmtime = d.body.innerHTML.match(/(会場を削除中です。|分校を削除中です。)[^\d]*(\d+-\d+-\d+ \d+:\d+:\d+)に完了します。/);
        if( rmtime ) {
...
//      else if( $x("//div[@id=\"changemapscale\"]/ul/li[@class=\"sort20 now\"]") ) type=3;
    ...
    for(var i=0 ; i<lists.length ; i++) {
        ...
        switch(lists[i].kind){
            ...
            case 1: title = "会場破棄"; break;
            case 2: title = "分校破棄"; break;
            case 3: title = "会場作成"; break;
            case 4: title = "分校作成"; break;
            case 5: title = "レベルアップ"; break;
        }
        var icon = "";
        if( lists[i].kind == 1 || lists[i].kind == 3 ) {        //会場破棄or会場作成
            icon = IMG_DIR + "panel/village_b_l.png";
        } else if( lists[i].kind == 2 || lists[i].kind == 4 ) {  //分校破棄or分校作成
        ...
        var sizestyle = "";
        if( lists[i].kind == 0 || lists[i].kind == 1 || lists[i].kind == 2 ) {      //領地破棄or会場破棄or分校破棄
        ...
        addHtml += '<a href="'+caddSessionId('/map.php?x=' + lists[i].x + '&y=' + lists[i].y +'')+'" title="マップ(' + lists[i].x + ',' + lists[i].y + ')"' + sizestyle + '>';
    ...
    function addList(tim, kind, x, y) { //kind=0:領地破棄 1:会場破棄 2:分校破棄 3:会場作成 4:分校作成 5:領地LvUp
        var lists = cloadData("RemoveList", "[]", true, true);
            ...
            if( kind == "xy" ) {
                ...
                for(var i=0; i<trs.length ; i++) {
                    ...
                    if(td.style.color=="red") {
                        strs.push({"node":trs[i], "value":"★★★★★★★★★★" + td.innerHTML});//NPC分校だけ特別に
                    } else {
                        strs.push({"node":trs[i], "value":td.innerHTML});
                    }
                }
...
function disp_ToolTipsDistance()
{
    ...
    function showToolTips( evt, distance, trainingLevel , eTrainingLevel)
    {
        ...
        tw.style.top = (evt.pageY -200 ) + "px";

        var dv = d.createElement("div");
//コード修正
        dv.innerHTML = "時間の目安　　陸上トラックLv."+eTrainingLevel+" (+"+(eTrainingLevel*0.1*distance).toFixed(3)+"%)<br>距離 [" + distance.toFixed(2) + "]　運動場Lv." + trainingLevel + " (+" + (TL*5) + "%)" ;
...
//訓練所→運動場
//遠征訓練所→陸上トラック
        var map1 = $a('//area[contains(@alt, "運動場")]');;
        if(map1.length!=0) {
			var lv1 = map1[0].alt.match(/運動場 LV.([0-9]+)/);
			level = parseInt(lv1[1]);
        }
        var map2 = $a('//area[contains(@alt, "陸上トラック")]');;
        if(map2.length!=0) {
			var lv2 = map2[0].alt.match(/陸上トラック LV.([0-9]+)/);
			elevel = parseInt(lv2[1]);
        }
//ここまで
        var lists = cloadData("TrainingLevels", "[]", true, true);
...
function disp_AttackMap()
{
    ...
    if( location.pathname == "/facility/unit_status.php" ) {
        var tds = $a("//table[@summary=\"出撃中の下級闘士\" or @summary=\"移動中の下級闘士\"]/tbody/tr[position()>1]/td[1]");
...
//            GM_log(tds[i+0].textContent);
            ...
            if( !tim ) continue;
            var kind = 0;
            if( tds[i].parentNode.parentNode.parentNode.getAttribute("summary") == "移動中の下級闘士" )
                kind = 1;
...
function disp_CardCombine()
{
    ...
    var a = d.createElement("a");
    a.href = "expup.php?cid=" + URL_PARAM.cid;
    a.title = "修行合成を行う";
    a.className = "trainingUnion";
    a.appendChild(d.createTextNode("修行合成を行う"));
    div2.appendChild(a);
...
{
    var trs = $a("//table[@class=\"commonTables\"]//tr[position()<13]");
    for(var i=0 ; i<trs.length ; i++) {
        trs[i].childNodes[trs[i].childNodes.length-2].colSpan ++;
    }

    var tr = $x("//table[@class=\"commonTables\"]//tr[13]");
    var th = d.createElement("th");
    th.className = "ttl4";
    th.id = id + "_title";
    ...
    trs = $a("//table[@class=\"commonTables\"]//tr[position()>13]");
    for(var i=0 ; i<trs.length ; i++) {
        ...
        .addEventListener('click',
            function() {
                ...
                if (index == 0 || index == 3) {
                    ...
                    } else if ($('k').value.toString().match(/玉|陣|鬼|闘|防|守|突|魏|呉|蜀|王|神|皇|帝|格闘|槍闘|弓闘|強襲|攻城|戦陣|奮陣|烈陣|閃陣|号令|疾駆|舞踏|知識|技術|増強|援護|丹田功|富国|豊穣|韋駄天|精鋭進撃|英雄|霧散消沈|天衣無縫|天下無双|高材疾足|神機妙算|沈思黙考|龍魂胎動/)) {
...
function disp_timerLinkDepot() {
    ...
    setTimeout(function(){
        ...
        var box = ccreateSideBox('beyond_sideBox_timerOpenLink',timerIcon,"タイマーボタン");
...
function disp_villageListBox() {
    ...
    var conf = cloadData( "beyond_sideBox_villageListBoxconf" , "null", true, true );
    if (conf == null && location.pathname == "/map.php") {
        csaveData("beyond_sideBox_villageListBoxconf",{"float":true, "open":true, "x":616, "y":134, "pos":99 });
    }

    var box = ccreateSideBox('beyond_sideBox_villageListBox',IMG_DIR+'common/sidebar/icon_base.gif',"拠点");
    ...
    for (var key in VILLAGES_INFO) {
        ...
        if (village.vid == cvid) {
            ...
            item.href = '/village_change.php?village_id='+village.vid+'&from=menu&page=' + encodeURIComponent(location.pathname + location.search);
        }
...
function initVillages() {
    ...
    function _villageInit() {
        var xpath = isMixi ? 'id("lodgment")/div[@class="floatInner"]//li/*[@title]' : '//div[@class="sideBoxInner basename"]//li/*[@title]';
        var villages = $a(xpath);
        ...
        for (var i=0;i < villages.length;i++) {
            ...
            if ((matches = village.title.match(reg))) {
                ...
                addObj.x = parseInt(matches[2]);
                addObj.y = parseInt(matches[3]);
            }

            if (typeof village.href == 'string' && (matches = village.href.match(/village_id=(\d+)/))) {
                addObj.vid = parseInt(matches[1]);
            }
            else {
                var viDoc = $x('//input[@name="village_id"]');
                if (viDoc) {
                    addObj.vid = parseInt(viDoc.value);
                }
                else {
                    noVids.push(addObj);
                }
            ...
            cajaxRequest("/user/" , "GET", "", function(req) {
                ...
                for (var i = 0;i < noVids.length;i++) {
                    ...
                    if ((matches = link.href.match(/village_id=(\d+)/))) {
                        VILLAGES_INFO[(noVids[i].x+"_"+noVids[i].y).replace(/-/g,"m")].vid = parseInt(matches[1]);
                    }
                }
    ...
    function _profileInit() {
        var compList = new Object();
        var matches = null;
        var trs = $a('//table[@class="commonTables"]//tr[13 < position()]');
        for (var i = 0;i < 10;i++) {
            var tr = trs[i];
            var pop = $x('.//td[3]',tr);
            if (!pop.innerHTML.match(/\d+/)) {
                break;
            }

            var addObj = new Object();
            var loc = cgetElementSibling(pop,1);
            if ((matches = loc.innerHTML.match(/(-?\d+),(-?\d+)/))) {
                addObj.x = parseInt(matches[1]);
                addObj.y = parseInt(matches[2]);
            }
            ...
            addObj.basename = link.innerHTML.replace(/^\s*|\s*$/g,"");
            compList[(addObj.x+"_"+addObj.y).replace(/-/g,"m")] = addObj;
        }
