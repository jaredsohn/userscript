// ==UserScript==
// @name          WarezienS Ignore Script
// @namespace     Stink Loadz
// @description   Anti Noob.
// @include       http://www.wareziens.info/*
// ==/UserScript==
...
function toggleignore(event) {
    ...
    var changed = false;
    if(test > 0) {
        r = confirm("Supprimer cette personne de votre blocklist ?");
        if(true == r) {
            removeignoreid(postdiv);
            buttonpressed.textContent = "Bloquer ?";
            changed = true;
        }
    }
    else {
        r = confirm("Ajouter cette personne à votre blocklist ?");
        if(true == r) {
            ignore_list.push(getposterid(postdiv));
            buttonpressed.textContent = "*BLOQUEE*";
            changed = true;
        }
    }
...
function setupignores() {
    ...
    for (i = 0 ; i < divs.length ; i++ ) {
        if (divs[i].className.match(/blockpost/)) {
            ...
            var ignoredstatusstring = "Bloquer ?";
            if(isignored) {
                ignoredstatusstring = "*BLOQUEE*";
            }
            ...
            showbutton.textContent = "Voir ce message";
