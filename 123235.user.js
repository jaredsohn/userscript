...
window.addEventListener("load", function(e) {
  setTimeout(function(){
  var totalScore = Number(document.getElementsByClassName('borderlist')[2].getElementsByTagName("tr")[2].getElementsByTagName('td')[1].innerHTML.split('.').join(""));
  var allTDs = document.getElementsByClassName('borderlist')[3].getElementsByTagName("td")
  var cityScores = []
  for( var i=0; i<allTDs.length; i++ ){
    if( i%3 == 2 )
      cityScores.push( Number(allTDs[i].innerHTML.split('.').join("")) )
  }
