// ==UserScript==
// @name DS Assistent2
// @description Version 2.0.1 DS-Assistent
// @author Hypix
...
win.ScriptAPI.register( 'DS Assistent', 7.4, 'Hypix', 'ds_scripte@hypix.de' );
...
//      ovlMoral: "Moral",
      ...
      reset: "Zurücksetzen",
      fake: "Fake",
      newVersion: ["Neue Version installiert", "Aktuelle Version:", "Forenthread", "Wiki", "Nicht mehr an diese Version erinnern"],
...
*/
  ...
  showFarmList : function(event) {
    ...
    sorter.addSortColumn(7,compareNumericCell,1);
...
//    $("info").style.zIndex = 100;
  ...
  watchMap : function() {
    ...
    if( !Map.twmap.scrolling && (Map.mopX != x || Map.mopY != y) ) {
      ...
      Map.mopX = x;
      Map.mopY = y;
      if( Settings.settings.map.rememberPos && (Map.twmap.pos[0] != curVillage.map.x || Map.twmap.pos[1] != curVillage.map.y) ) {
...
var RunTimes = {
  ...
  doIt : function() {
    ...
    if( lib.params.screen=="market" )
      ...
      RunTimes.modCommand();
    else if( lib.params.screen == "place" && lib.params.mode == "call" ) {
      RunTimes.modCall();
    }
  },
  ...
  modCall : function() {
    var tab = $("village_troup_list");
    var sorter = new TableSorter(tab);
    sorter.addSortColumn(0,compareStringCell);
    sorter.addSortColumn(1,compareOrderCell,1);
    for( var i = 2; i < tab.rows[0].cells.length-1; i++ ) 
      sorter.addSortColumn(i,compareNumericCell);
    for( var i = 1; i < tab.rows.length; i++ ) {
      var coords = tab.rows[i].cells[0].textContent.match(texts[lib.lang].regex.villageLink);
      if( coords ) {
        tab.rows[i].cells[1].setAttribute("coords",coords[1]+"_"+coords[2]);
        tab.rows[i].cells[1].setAttribute("dsfm_order",i);
        RunTimes.createToolTip(tab.rows[i].cells[1]);
      }
    }
  },
}
...
var PrivateVillageName = {
  ...
  modAll : function() {
    ...
    var lnk = document.evaluate("//a[(contains(@href,'screen=overview') and not(contains(@href,'_villages')) or contains(@href,'info_village')) and not(contains(@class,'village_switch_link'))]",document,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);
    for( var i = 0; i < lnk.snapshotLength; i++ ) {
      var a = lnk.snapshotItem(i);
      var res = a.href.match(/village=(\d+)&screen=overview$|info_village&id=(\d+)$/);
      if( res ) {
...
function run() {
  ...
  SLSwitcher.doIt();
  NavBarSwitcher.doIt();
  if( lib.params.screen != "place" && version != lib.storage.getValue("version","") )
    lib.checkVersion();
}
function showTime() {
  var e = $('serverTime');
...
function HypixDSLib(prefix,forceGM,useIdx) {
  ...
  this.checkVersion = function() {
    if(typeof(GM_xmlhttpRequest) != 'undefined') {
      GM_xmlhttpRequest({
        method: "GET",
        url: 	"http://forum.die-staemme.de/showpost.php?p=2810431",
        headers: {},
        onload: function(response) {
          ...
          if( res ) {
            ...
            if( curVer == version ) {
              var pos = [];
              pos[0] = text.indexOf("<b>Letzte Änderungen</b>");
              pos[1] = text.indexOf("</div>",pos[0]);
              var changes = "";
              if( pos[0] != -1 && pos[1] != -1 ) {
                changes = text.substring(pos[0],pos[1]);
                var popup = new lib.Popup("dsfm_versioncheck",texts[lib.lang].gui.title + " - " + texts[lib.lang].gui.newVersion[0],true,400,300);
                var html = texts[lib.lang].gui.newVersion[1]+'<b>'+version+'</b></br>';
                if( changes != "" ) 
                  html += '<div style="text-align: left; border: 1px solid #804000; width: 400px; height: 200px; overflow:auto; background-color:#D2C09E;">'+changes+'</div>';
                html += '<a href="http://forum.die-staemme.de/showpost.php?p=2810431">' + texts[lib.lang].gui.newVersion[2] + '</a> - ';
                html += '<a href="http://hypix.de/staemme/wiki/doku.php?id=ds-assistent">' + texts[lib.lang].gui.newVersion[3] + '</a><br/>';
                popup.content.innerHTML = html;
                popup.content.appendChild(ce("br"));
                var a = popup.content.appendChild(ce("a"));
                a.href = "http://hypix.de/staemme/spenden.html";
                a.innerHTML = texts[lib.lang].gui.donate;
                popup.content.style.textAlign = "center";
                popup.center();
                popup.show();
                lib.storage.setValue("version",version);
              }
            }
          }
          else {
