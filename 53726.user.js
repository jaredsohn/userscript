...
// @description   Adds image thumbnails under image anchors in kecy.roumen.cz & maso.roumen.cz
// @author        Yanni_SVK
// @version       2.4
// @include       http://halbot.haluze.sk/images/*
// @include       http://www.dusky.sk/pics/*
// @include       http://kecy.roumen.cz/*
// @include       http://maso.roumen.cz/*
// ==/UserScript==
	...
	for (var i = 0; i < odkazy.length; i+=3){
		...
		if(prip){
			if(prip==".gif" || prip==".jpg" || prip==".jpeg" || prip==".png" ){
				...
				if(document.location.host=="kecy.roumen.cz"){
					odkazy[i].href="http://kecy.roumen.cz/"+odkazy[i].href.substr(parseInt(odkazy[i].href.indexOf("?file="))+6)
				}
				if(document.location.host=="maso.roumen.cz"){
					odkazy[i].href="http://maso.roumen.cz/"+odkazy[i].href.substr(parseInt(odkazy[i].href.indexOf("?file="))+6)
				}
		...
		if(prip==".gif" || prip==".jpg" || prip==".jpeg" || prip==".png" ){
			...
			if(document.location.host=="kecy.roumen.cz"){
				if(document.location.pathname!="/roumingArchive.php"){
					novy.setAttribute("src","http://kecy.roumen.cz/"+this.href.substr(parseInt(this.href.indexOf("?file="))+6))
				}else{
					novy.setAttribute("src","http://archiv.roumen.cz/archive/"+this.href.substr(parseInt(this.href.indexOf("?file="))+6))

				}
			}else if(document.location.host=="maso.roumen.cz"){
				if(document.location.pathname!="/masoArchive.php"){
					novy.setAttribute("src","http://maso.roumen.cz/"+this.href.substr(parseInt(this.href.indexOf("?file="))+6))
				}else{
					novy.setAttribute("src","http://maso.roumen.cz/archived/"+this.href.substr(parseInt(this.href.indexOf("?file="))+6))
