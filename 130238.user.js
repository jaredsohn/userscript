...
function getNewChatLines(chat,lastTime) {
    var lines = chat.split(/<br>|<br|<br\/>|<br \/>/);
    var newlines = "";
    for ( var i = lines.length-2; i >= 0; i-- ) {
        var currentLine = lines[i];
        var res = /(\d+)-(\d+)-(\d+)\s(\d+):(\d+):(\d+)/.exec(currentLine);
        var date = new Date(res[1], res[2], res[3], res[4], res[5], res[6]);
        var time = date.getTime();
        if ( time <= lastTime ) {
            break;
        }
        newlines = currentLine + "<br>" + newlines;
    }
    return newlines;
}
...
function updateCurrentGameChat() {
    var gameNumber = getGameNumber(window.location.href); 
    var currentChat = document.getElementById("chat");
    var chatLineCount = countChatLines(currentChat.innerHTML);
    ...
    gameRequest.onreadystatechange = function() {
        if(gameRequest.readyState == 4) {
            var page = $(gameRequest.responseText);
            var chat = page.find("#chat");
            var chatContent = chat.html();
            
            try {
                var newChatLineCount = countChatLines(chatContent);
                if ( newChatLineCount > chatLineCount ) {
                    console.log("New chat found - refreshing chat");
                    //currentChat.innerHTML += getNewChatLines(chatContent,chatLineCount);
                    //adjustChatScroll();
                    refreshChatContent();
                }
            }
...
function refreshChatContent() {
    execPageJavascript(function() {
        sendRequest();
    });
}

function execPageJavascript(fn) {
...
function initialize() {
    ...
    if ( isGamePage(window.location.href) ) {
        setInterval(updateCurrentGameChat,5000);
        //updateCurrentGameChat();
    }
}
