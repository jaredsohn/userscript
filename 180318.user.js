...
(function(){
    ...
    function gettails(){
		try{
			if(!$.parseJSON(localStorage.ge_usetail)) return '';
			var l=localStorage.ge_tailindex,t=$.parseJSON(localStorage.ge_tails);if(!t) return '';
			if(l==0) l=Math.floor(Math.random()*t.length);else if(l>t.length)l=t.length-1; else l--;
			t=t[l];var d=t.data;
			if(t.type=='j') d=eval(d);
			else if(t.type=='H') {d=d.split('\n');d=d[Math.floor(Math.random()*d.length)];}
			return '<br><br>'+d;
		}catch(err){return '<br><br>尾巴生'+err+'病了@_@'}
	}
	...
	function imgFormat(){
        $('#ueditor_replace').append('<p>'+gettails()+'</p>');
	...
	function lzlPost(floor_num,quote_id){
        $(".lzl_simple_wrapper .tb-editor-editarea").append(gettails());
