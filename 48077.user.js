// ==UserScript==
// @name           Allegro thumbnail revealer
// @namespace      http://www.allegro.pl
// @include        *allegro.pl/*
// @exclude        https://*
// @exclude        *.allegro.pl/item*
// @exclude        *.allegro.pl/new_item*
// @exclude        *.allegro.pl/enter_login*
// ==/UserScript==
...
function cache_set(key, val) {
	cache[key] = new Array(new Date().getTime(), val);
	if (cnt == 0) writeCache();
}
...
function writeCache() {
	var toc = '';
	for (var i in cache) {
		toc += '|'+cache[i][0]+'|'+i+'|'+cache[i][1];
	}
	if (toc == '') toc = '|';
	window.setTimeout(GM_setValue,0,'cache', toc);
}

cnt = $('a img[src$="foto.gif"],img[width=64][height=1][src$="blank.gif"]').length;

loadCache();

$('a img[src$="foto.gif"],img[width=64][height=1][src$="blank.gif"]').css({
	width: '64px',
    height: '48px',
    margin: '0px'		
});
...
$('a img[src$="foto.gif"]').each(function() {
	var thimg = this;
	var href = $(this).parent().attr('href');
	var src = cache_get(href);
	$(this).css({
	...
	if (src == null) {
		ajaxManager.add({
			...
			success: function(data) {
				var p1 = data.indexOf('<a href="#gallery">');
				var p2 = data.indexOf('width=', p1);
				src = data.substring(p1+28, p2-1);
				$(thimg).attr('src', src);
				cnt--
				cache_set(href, src);
			}
...
$('img[width=64][height=48][src$="blank.gif"]').each(function() {
	var thimg = this;
	...
	if (src == null) {
		ajaxManager.add({
			url: href, 
			success: function(data) {
				var imgs = $('#user_field img', $(data)).filter(function(idx) {
					return !$(this).is('img[src*=allegro.pl]') && !$(this).is('img[src*=/site_images/]');
				})
				var num = Math.ceil(imgs.length / 2.);
				src = $(imgs.get(num-1)).attr('src');
				cnt--
				cache_set(href, src);
				if(src != 'undefined') $(thimg).attr('src', src).wrap('<a href="'+href+'"></a>');
			}
		});
	} else {
		if (src != 'undefined') $(this).attr('src', src).wrap('<a href="'+href+'"></a>');
		cnt--
		cache_set(href, src);
	}
});

$('body').append('<div id="img128" style="border: 1px solid black; width: 128px; height: 96px; position: absolute; display: none;"><img width="128" height="96" src="http://static.allegro.pl/site_images/1/0/blank.gif"/></div>');
$('img[width=64][height=48]').hover(function() {
 var p = $(this).position();
 var img = $(this).attr('src').replace('/64x48/', '/128x96/');
 $('#img128 img').attr('src', img);
 $('#img128').css({
 'left': (p.left-32)+'px',
 'top': (p.top-24)+'px'
 }).show();
