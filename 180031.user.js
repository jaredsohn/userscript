...
// @grant        none
// @include      http://www.waterkant.net/suriname/forum/viewtopic.php*
// @include      http://www.waterkant.net/suriname/forum/post.php*
// @include      http://www.waterkant.net/suriname/forum/edit.php*
// @include      http://www.waterkant.net/suriname/forum/search.php?action=show_new
// @include      http://www.waterkant.net/suriname/forum/search.php?action=show_new&p=*
// @run-at       document-end
// ==/UserScript==
(function(){"use strict";
	var userjs = "kaboela535455";
/*ignore some forums on new posts page*/
	if (self.location.pathname == "/search.php" && self.location.search.match(/show_new/)) {
		...
		var forums = document.querySelectorAll("div.forum-forums li.info-forum > a[href^='http://www.waterkant.net/suriname/forum/viewforum.php?id=']");
		for (var f=0; f<forums.length; f++) {
			...
			if (id) {
				...
				if (hide.indexOf(id) > -1) {
					...
					forums[f].parentNode.appendChild(createTag("a", {"a":{"title":"click to hide new posts from forum #"+id},"e":{"click":function(e){
						hide.push(id);
