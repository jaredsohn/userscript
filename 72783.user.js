// ==UserScript==
// @name           Japanese Popular SBM Count With Google
// @namespace      http://web-marketing.zako.org/greasemonkey/
// @description    Displays japanese popular SBM count in web search results of Google.
// @author         http://userjs.0fk.org
// @include        http://*.google.*/search*
// @include        http://*.google.*/custom*
// @include        https://*.google.*/search*
// @include        https://*.google.*/custom*
...
var sbmfilter = function (context) {
	var links = document.evaluate('.//a[@onmousedown][parent::*[not(child::span[@class="sbm"])]]', context, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	for (var i = 0, j = links.snapshotLength; i < j; ++i) {
		...
		a.parentNode.appendChild($N('span', {
			...
			$N('a', {
				...
				'style': 'margin:0px 0px 0px 20px;border:0;vertical-align:middle;'
			})]),
				//Buzzuri
			$N('a', {
				'href': 'http://buzzurl.jp/entry/' + url
			}, [$N('img', {
				'src': 'http://api.buzzurl.jp/api/counter/v1/image?url=' + url,
				'style': 'margin:0px 0px 0px 5px;border:0;vertical-align:middle;'
			})]),
			//Yahoo
			$N('a', {
				'href': 'http://bookmarks.yahoo.co.jp/url/' + url
			}, [$N('img', {
				'src': 'http://num.bookmarks.yahoo.co.jp/image/small/' + url,
				'style': 'margin:0px 0px 0px 5px;border:0;vertical-align:middle;'
			})]),
			// del.icio.us
			$N('a', {
				...
				'src': 'http://labs.creazy.net/sbm/delicious/textimg/' + url ,
				'style': 'margin:0px 0px 0px 5px;border:0;vertical-align:middle;'
			})]),
						// facebook
			$N('img', {
				'src': 'http://remix-remix.rash.jp/facebookapi/getiine.php?URL=' + url ,
				'style': 'margin:0px 0px 0px 5px;border:0;vertical-align:middle;'
					
			}),
				// tweetbuzz
			$N('a', {
				'href': 'http://tweetbuzz.jp/redirect?url=' + url
			}, [$N('img', {
				'src': 'http://tools.tweetbuzz.jp/imgcount?url=' + url ,
				'style': 'margin:0px 0px 0px 5px;border:0;vertical-align:middle;'
			})]),
