...
function findDefender(players, play_data) {
	...
	for (j = 0; j < play_data.length; j++) {
		...
		if ((balldata.x == wrdata.x) && (balldata.y == wrdata.y)) {
			break;
		}
	}
...
function findBlocker(players,play_data) {
	...
	for (j = 0; j < play_data.length; j++) {
		...
		for (var i=(k*11)+1; i<(k*11)+12; i++) {
			...
			var offydist = Math.abs(off.y - unsafeWindow.play_data[j][rusheridx].y);
			var offxdist = Math.abs(off.x - unsafeWindow.play_data[j][rusheridx].x);
...
function addDistance(d) {
	...
	for (var i=0; i < plays.length; i++) {
		if (plays[i].innerHTML.indexOf(' pass to ')!=-1){			
			...
			var str = plays[i].innerHTML;
			var idx = str.indexOf(" pass to ");
			plays[i].innerHTML = str.slice(0,idx)+s+str.slice(idx);
			break;
		}
	}
	*/
...
function addBlocker(d) {
	...
	if (idx != 10) {
		...
		var idx = playText.indexOf(" hurried by ")+12;
		idx2 = playText.slice(idx).indexOf(", ");
	}
