...
// @description    BBCoder para T! (Crear post, comentarios, menÃº)
// @include        http://*taringa.net/*
// @exclude        http://br.taringa.net/*
// @version        25.0
// @copyright      Copyright (c) 2009, Sharkale
// @creator        Sharkale (Alejandro Barreiro) Â® 2009
// ==/UserScript==
...
// EspaÃ±ol
tarLang = new Array();
tarLang["BarraBBC"] = new Array ("AlineaciÃ³n Izquierda", "AlineaciÃ³n Centrada", "AlineaciÃ³n Derecha", "Letra Negrita", "Letra Cursiva", "Letra Subrayada","Fuente", "Insertar Video de YouTube", "Insertar Video de Google", "Insertar Video de MegaVideo", "Insertar canciÃ³n de GoEar", "Insertar canciÃ³n de eSnips", "Insertar archivo SWF", "Insertar ImÃ¡gen", "Insertar ImÃ¡gen clickeable", "Insertar URL", "Agregar BBCode a todas las URL escritas", "Insertar una Cita", "Ver Preview del Comentario");
tarLang["BarraBBC"]["color"] = new Array ("Color", "Bordo", "Rojo", "Naranja", "MarrÃ³n", "Amarillo", "Verde", "Oliva", "Cyan", "Azul", "Azul Oscuro", "Indigo", "Violeta", "Negro");
tarLang["Preview"] = new Array ("Debes escribir algo para realizar la previsualizaciÃ³n", "CARGANDO...", "Cargando preview", "Cerrar PrevisualizaciÃ³n", "PrevisualizaciÃ³n", "Hubo un error al previsualizar, Por favor intente nuevamente.","Cargando preview del post. Por favor espere.");
tarLang["Promt"] = new Array ("Ingrese la direcciÃ³n del video de YouTube:", "DirecciÃ³n del video invÃ¡lida", "Si el video posee alta deficiÃ³n acepte esta ventana.\n\nDe lo contrario cancelela o no se verÃ¡ el video.", "Ingrese la direcciÃ³n del video de Google:", "Ingrese el cÃ³digo entero que provee la pÃ¡gina a travÃ©s del link \"Video Incrustado\" en cada video de MegaVideo:", "Ingrese la direcciÃ³n URL Ã³ el cÃ³digo HTML Ã³ la ID de la canciÃ³n deseada de GoEar:", "DirecciÃ³n de la mÃºsica invÃ¡lida", "Ingrese la direcciÃ³n \"URL\" de la canciÃ³n proporcionada por eSnips:\n\nEjemplo:\nhttp://www.esnips.com/doc/01f6cd69-e218-4faa-8f1c-1e5cc0cc4111/Rodrigo - La Mano De Dios", "Ingrese la direcciÃ³n URL del archivo SWF", "Ingrese el texto a citar", "Ingrese la direcciÃ³n URL completa de la imÃ¡gen", "Ingrese la direcciÃ³n URL completa del link", "Ingrese la URL que desea postear", "Â¿Desea agregarle una etiqueta a la URL?");
// ==/Variables de Lenguaje==
...
$.fn.themeswitcher = function(settings){
	...
	switcherpane.hover(
		function(){cerrar = false;},
		function(){
			cerrar = true;
			var switcher = this;
			$.timer(500, function(timer){
				if(switcherpane.is(':visible') && cerrar){$(switcher).spHide();}
				timer.stop();
			});
		}
	);
	...
	function updateCSS(locStr){
		...
		GM_setValue('sharkale-theme', locStr);
	}	
...
var CambiarTheme = GM_getValue("CambiarTheme",true);
	...
	if(CambiarTheme) GM_registerMenuCommand("BBCoder para Taringa - Cambiar el theme de la pÃ¡gina", Cambiar_Theme);
	...
	GM_registerMenuCommand("BBCoder para Taringa - VersiÃ³n Instalada: " + ultversion, function() {
		alert("Tu versiÃ³n instalada actualmente es: " + ultversion);
	},null,null,"V");
...
function Procesar_Datos(datos){
	if(datos == 'Disculpas! Hubo un error al procesar lo solicitado. Por favor, int&eacute;ntalo nuevamente en unos minutos.'){
		...
		switch(datos["error"]["code"]){
			...
			case 11: return 'LÃ de pedidos por minuto superado.';break;
...
function Sharkale_TopPost(alcargar,t,p){
	/** Que raro que Taringa! desactivÃ³ la api paralostop post.
		...
		$.ajax({
			...
			success: function(h){
				...
				if(typeof(categorias) == 'string'){
					$('#cattop').html('<option value="-1">Todas</option>');
				}else{
					var html = '<option value="-1">Todas</option>';
					for(var i in categorias){
						html += '			<option value="'+categorias[i]["id"]+'">'+categorias[i]["name"]+'</option>';
					}
					$('#cattop').html(html);
				}
			},
	...
	$.ajax({
		...
		success: function(h){
			...
			if(typeof(maspunt) == 'string'){
				$('#maspunt').html('<b>Error al obtener datos.<br><br>Causa: </b>'+maspunt);
			}else{
				var html = '';
				for(var i in maspunt){
					html += '			<b>'+maspunt[i]["position"]+'</b> - <a title="'+maspunt[i]["title"]+' - '+maspunt[i]["date"]+'" href="'+maspunt[i]["location"]+'">'+(maspunt[i]["title"].length > 35? maspunt[i]["title"].substring(0,35)+'...':maspunt[i]["title"])+'</a> ('+maspunt[i]["score"]+' pnts)<br/>';
				}
				$('#maspunt').html(html);
			}
		},
		error: function(h){
			$('#maspunt').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
				//////////////////////////////////////////
	...
	$.ajax({
		type: 'GET',
		url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetTopPosts/'+tiempo+'/2/'+posts,
		success: function(h){
			mascoment = Procesar_Datos(h);
			if(typeof(mascoment) == 'string'){
				$('#mascoment').html('<b>Error al obtener datos.<br><br>Causa: </b>'+mascoment);
			}else{
				var html = '';
				for(var i in mascoment){
					html += '			<b>'+mascoment[i]["position"]+'</b> - <a title="'+mascoment[i]["title"]+' - '+mascoment[i]["date"]+'" href="'+mascoment[i]["location"]+'">'+(mascoment[i]["title"].length > 35? mascoment[i]["title"].substring(0,35)+'...':mascoment[i]["title"])+'</a> ('+mascoment[i]["score"]+' com)<br/>';
				}
				$('#mascoment').html(html);
			}
		},
		error: function(h){
			$('#mascoment').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
			//////////////////////////////////////////
	...
	$.ajax({
		type: 'GET',
		url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetTopPosts/'+tiempo+'/3/'+posts,
		success: function(h){
			masfav = Procesar_Datos(h);
			if(typeof(masfav) == 'string'){
				$('#masfav').html('<b>Error al obtener datos.<br><br>Causa: </b>'+masfav);
			}else{
				var html = '';
				for(var i in masfav){
					html += '			<b>'+masfav[i]["position"]+'</b> - <a title="'+masfav[i]["title"]+' - '+masfav[i]["date"]+'" href="'+masfav[i]["location"]+'">'+(masfav[i]["title"].length > 35? masfav[i]["title"].substring(0,35)+'...':masfav[i]["title"])+'</a> ('+masfav[i]["score"]+' fav)<br/>';
				}
				$('#masfav').html(html);
			}
		},
		error: function(h){
			$('#masfav').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
			//////////////////////////////////////////
	...
	$.ajax({
		type: 'GET',
		url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetTopPosts/'+tiempo+'/4/'+posts,
		success: function(h){
			masvisit = Procesar_Datos(h);
			if(typeof(masvisit) == 'string'){
				$('#masvisit').html('<b>Error al obtener datos.<br><br>Causa: </b>'+masvisit);
			}else{
				var html = '';
				for(var i in masvisit){
					html += '			<b>'+masvisit[i]["position"]+'</b> - <a title="'+masvisit[i]["title"]+' - '+masvisit[i]["date"]+'" href="'+masvisit[i]["location"]+'">'+(masvisit[i]["title"].length > 30? masvisit[i]["title"].substring(0,30)+'...':masvisit[i]["title"])+'</a> ('+masvisit[i]["score"]+' vis)<br/>';
				}
				$('#masvisit').html(html);
			}
		},
		error: function(h){
			$('#masvisit').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
			//////////////////////////////////////////
	...
	$.ajax({
		type: 'GET',
		url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetTopUsers/'+tiempo+'/1/'+posts,
		success: function(h){
			masposts = Procesar_Datos(h);
			if(typeof(masposts) == 'string'){
				$('#masposts').html('<b>Error al obtener datos.<br><br>Causa: </b>'+masposts);
			}else{
				var html = '';
				for(var i in masposts){
					html += '			<b>'+masposts[i]["position"]+'</b> - <a href="http://www.taringa.net/perfil/'+masposts[i]["nick"]+'">'+masposts[i]["nick"]+'</a> ('+masposts[i]["score"]+' posts)<br/>';
				}
				$('#masposts').html(html);
			}
		},
		error: function(h){
			$('#masposts').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
			//////////////////////////////////////////
	...
	$.ajax({
		type: 'GET',
		url: 'http://'+Dom+'/api/shark32455Dfa/json/Posts-GetTopUsers/'+tiempo+'/2/'+posts,
		success: function(h){
			maspuntos = Procesar_Datos(h);
			if(typeof(maspuntos) == 'string'){
				$('#maspuntos').html('<b>Error al obtener datos.<br><br>Causa: </b>'+maspuntos);
			}else{
				var html = '';
				for(var i in maspuntos){
					html += '			<b>'+maspuntos[i]["position"]+'</b> - <a href="http://www.taringa.net/perfil/'+maspuntos[i]["nick"]+'">'+maspuntos[i]["nick"]+'</a> ('+maspuntos[i]["score"]+' puntos)<br/>';
				}
				$('#maspuntos').html(html);
			}
		},
		error: function(h){
			$('#maspuntos').html('<b>Error al obtener datos.<br><br>Causa: </b>Taringa no responde. =)');
		}
	});
	});
			//////////////////////////////////////////
	...
	var html = '';
	if(alcargar){
	html += '	<div align="center" class="box_cuerpo">';
	html += '		<b>TOPs de </b>';
	html += '		<select id="fechatop">';
	html += '			<option selected="1" value="-1">Todos los tiempos</option>';
	html += '			<option value="1">Hoy</option>';
	html += '			<option value="2">Ayer</option>';
	html += '			<option value="3">De la semana</option>';
	html += '			<option value="4">Del mes</option>';
	html += '			<option value="5">Mes Anterior</option>';
	html += '		</select>';
	html += '		<b>de la Categoria:</b>';
	html += '		<select id="cattop">';
	html += '			<option value="-1">Cargando...</option>';
	html += '		</select>';
	html += '		<input id="Btn_Filtrar_Tops" type="button" title="Filtrar Top Posts" value="Filtrar - SharkaleÂ®" style="font-size: 12px; width: 200px;" class="login"/><br/>';
	html += '		<b>La visualizaciÃ³n de los top puede tardar unos minutos... TambiÃ©n ten en cuanta que solo puedes hacer 2 consultas por minuto.</b>';
	html += '	</div><br/>';
	html += '	<div id="actualizar_busqueda">';
	}
	html += '	<div style="width: 300px;float:left;height:auto;overflow:hidden;padding:2px 6px;" id="top_izquierda">';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 200px;" class="box_txt">Top post con mÃ¡s puntos</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="maspunt" style="width: 285px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div><br/>';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 200px;" class="box_txt">Top post mÃ¡s comentado</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="mascoment" style="width: 285px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div>';
	html += '	</div>';
	html += '	<div style="width: 300px;float:left;height:auto;overflow:hidden;padding:2px 6px;" id="top_centro">';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 268px;" class="box_txt">Top post mÃ¡s favorito</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="masfav" style="width: 284px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div><br/>';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 268px;" class="box_txt">Top post mÃ¡s visitado</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="masvisit" style="width: 284px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div>';
	html += '	</div>';
	html += '	<div style="width: 300px;float:left;height:auto;overflow:hidden;padding:2px 6px;" id="top_derecha">';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 268px;" class="box_txt">Top usuario mÃ¡s posteador</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="masposts" style="width: 284px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div><br/>';
	html += '		<div style="width: 300px;" class="box_title">';
	html += '			<div style="width: 268px;" class="box_txt">Top usuario con mÃ¡s puntos</div>';
	html += '			<div style="background-image:url(http://i.t.net.ar/images/cor-der.gif);background-position:right top;background-repeat:no-repeat;float:right;padding-right:8px;padding-top:4px;"></div>';
	html += '		</div>';
	html += '		<div id="maspuntos" style="width: 284px;" class="box_cuerpo">';
	html += '			<div align="center"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></div>';
	html += '		</div>';
	html += '	</div>';
	if(alcargar){
...
function shark_print_cmmt(id,nro,userid,nick,txtcita,date,txt){
	...
	dr('<a name="cmnt_'+id+'"></a>');
	dr('<div id="div_cmnt_'+id+'">');
	if(autor)
		dr('<input type="checkbox" name="cmnt_'+id+'" />');
	dr('<a onclick="citar_comment('+id+')" href="javascript:void(0)">');
	dr('#'+nro);
	dr('</a>');
	dr(' - ');
	dr('<a href="/perfil/'+userid+'"><b');
	dr(' id="autor_cmnt_'+id+'" user_comment="'+nick+'" text_comment="'+txtcita+'"');
	dr('>'+nick+'</b></a> ');
	dr('<a onclick="citar_comment('+id+')" href="javascript:void(0)"><span style="position:relative;"><img border=0 src="http://i.t.net.ar/images/big3v1.gif" style="position:absolute; top:-160px; clip:rect(160px 16px 176px 0px);" alt="Citar Comentario" title="Citar Comentario" /><img border=0 src="http://i.t.net.ar/images/space.gif" style="width:16px;height:16px" align="absmiddle" /></span></a> ');
	dr('<a href="/mensajes/a/'+nick+'"><span style="position:relative;"><img border=0 src="http://i.t.net.ar/images/big1v8.gif" style="position:absolute; top:-462px; clip:rect(462px 16px 478px 0px);" alt="Mandale un mensaje" title="Mandale un mensaje" /><img border=0 src="http://i.t.net.ar/images/space.gif" style="width:16px;height:16px" align="absmiddle" /></span></a> ');
	dr('| <span class="size10">'+date+'</span> dijo:<br />');
	dr(txt+'</div>');
	dr('<hr />');
	return HTML;
}
...
function CrearBarraEmoticones(){
	...
	var URLtaringa = 'http://i.t.net.ar/images/smiles/';
	var nuevaHTML = '<div id="Sharkale_TaringaEmoticones" align="center" class="SharkaleEmoticones" style="display:'+(EmotVis == true || path == "mensajes"? 'block':'none')+'">';
...
function Reestablecer_Emoticones(){
	if(confirm('Â¿EstÃ¡ seguro que desea volver a los emoticones originales?\n\nEste cambio es permanente.')){
...
Taringa_Set_Function("Ocultar_Emoticones", function(elemento){
	if(elemento.value == "Mostrar Emoticones"){
		$('#Sharkale_TaringaEmoticones').slideDown();
		elemento.value = "Ocultar Emoticones";
	}else{
		$('#Sharkale_TaringaEmoticones').slideUp();
		elemento.value = "Mostrar Emoticones";
	}
});
			...
			$('#return_agregar_comentario').html(HTML).slideDown(1000, function(){
				$.ajax({
					...
					success: function(h){
						...
						var HTML = '<div class="cita" style="color:#464646;text-shadow:0 1px 0 #CCCCCC;"><img src="'+URL+'cerrar.gif" alt="[X]" title="'+tarLang["Preview"][3]+'" border="0" onclick="javascript:($(\'#return_agregar_comentario\').slideUp(1000))" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />'+tarLang["Preview"][4]+' - SharkaleÂ®</div>';
						HTML += '<div class="citacuerpo">'+respuesta+'</div>';
						$('#return_agregar_comentario').slideUp(1000, function(){
	...
	$.ajax({
		...
		success: function(h){
			...
			$('#izquierda').fadeOut('slow',function(){$('#izquierda').html(secc_nfu);}).fadeIn('slow',function(){
				en_nfu = false;
				$("div[class='box_txt ultimos_posts']:first").html('Ãšltimos posts <a href="javascript:void(0)" onclick="javascript:act_post_nfu()" title="Actualizar Post" > <img src="http://i28.tinypic.com/wrx1uc.jpg" align="top" height="16" width="16" /></a>');
				secc_nfu = null;
			});
		},
		error: function(h){
	...
	$.ajax({
		...
		success: function(h){
			...
			for(var i in comment){
				HTML = '<hr><div id="Sharkale_div_cmnt_'+comment[i]["comment-id"]+'"><input type="checkbox" name="'+comment[i]["comment-id"]+'"> <span id="Sharkale_cmnt_'+comment[i]["comment-id"]+'"><strong>'+comment[i]["nick"]+'</strong></span> | '+comment[i]["date"]+' dijo:<br />'+comment[i]["comment"]+'</div>'+HTML;
			}
...
Taringa_Set_Function("actualizar_comentarios_post", function(todos){
	$('.Sharkale_cargando').show();
	$.ajax({
		type: 'GET',
		url: window.location.href,
		success: function(h){
		GM_log(h);
			if(h.search('container400') != -1){
				var error = $(h).find('div[class="container400"]:first').find('div[class="box_cuerpo"]:first');
				$('.Sharkale_cargando').hide();
				error.find('br,:input').remove();
				$('.msg_add_comment').hide('slow').html(error.html()).show('slow');
				error = null;
			}else{
				var html = h.replace(/^[\w\W]*\<\!\-\- Comentarios \-\-\>/i,'<html><body>').replace(/\<div[^>]*>/,'');
				html = html.replace(/\<div id="return_agregar_comentario[\w\W]*$/,'');
				html += '<div id="return_agregar_comentario" style="display:none">&nbsp;</div>';
				$('.Sharkale_cargando').hide();
				$('#Sharkale_comentarios').html(html);
				modificar_posts();
				html = null;
			}
		},
		error: function(h){
			$('.msg_add_comment').hide('slow').html('Como de costumbre Taringa no responde. =)').show('slow');
			$('.Sharkale_cargando').hide();
		}
	});
});
		
Taringa_Set_Function("add_comment", function(ult, nro){
	if($('#cuerpo_comment').val() == ''){
		$('#cuerpo_comment').focus();
		return;
	}
	$('#button_add_comment').attr('disabled', 'true');
	$('#button_add_comment').addClass('ui-state-disabled');
	$('.Sharkale_cargando').show();
	$('#gif_cargando_add_comment').show();
	$.ajax({
		type: 'POST',
		url: '/comentario2.php',
		data: 'postid=' + datospag.postid + '&comentario=' + encodeURIComponent($('#cuerpo_comment').val()) + '&ult='+ult+'&key=' + datospag.user_key + '&nro=' + nro,
		success: function(h){
			if(h.charAt(0)==0){ //Error
				$('.msg_add_comment').hide('slow').html(h.substring(3)).show('slow');
			}else if(h.charAt(0)==1){ //OK
				if($('#sin_comentarios'))
					$('#sin_comentarios').slideUp('slow');
				$('#return_agregar_comentario').html(h.substring(3));
				$('#return_agregar_comentario').slideDown('slow');
				$('#cuerpo_comment').val('');
				$('.msg_add_comment').hide('slow');
			}
			$('.Sharkale_cargando').hide();
			$('#gif_cargando_add_comment').hide();
			$('#button_add_comment').removeAttr('disabled');
			$('#button_add_comment').removeClass('ui-state-disabled');
		},
		error: function(h){
			$('.msg_add_comment').hide('slow').html('Como de costumbre Taringa no responde. =)').show('slow');
			$('.Sharkale_cargando').hide();
...
function Sharkale_Opciones(){
	...
	div.setAttribute('id', 'SharkaleOpciones');
	div.setAttribute('style', 'display:none');
	var nuevaHTML = '<div id="tabs">';
	nuevaHTML += '<ul>';
	...
	nuevaHTML += '	<label><input type="checkbox" id="shark_vYT" '+(vYT? 'checked':'')+'> Reproductor de Videos de YouTube en formato HQ.</label><br/>';
	nuevaHTML += '	<label><input type="checkbox" id="shark_PermHD" '+(PermHD? 'checked':'')+'> Preguntar por calidad HQ al crear nuevo video.</label><br/>';
	...
	nuevaHTML += '	<label><input type="checkbox" id="shark_LinkCheck" '+(LinkCheck? 'checked':'')+'> Activar LinkChecker (Verificador de links).</label><br/>';
	...
	nuevaHTML += '	<label><input type="button" class="ui-state-default ui-corner-all"  id="Btn_CambiarTheme" value="Cambiar Theme de Taringa!"><br/><br/>';
	...
	nuevaHTML += '	Historial de ModeraciÃ³n <img id="previe-mmod" src="'+MenuModeracion+'" width="25" height="25" align="absmiddle"/>';
	...
	nuevaHTML += '	<input type="button" value="Restablecer Cambios" id="Btn_Restablecer_Imagenes" title="Restablecer a los imÃ¡genes originales del script" class="ui-state-default ui-corner-all"/><hr>';
	...
	nuevaHTML += '	<div id="tagboard" style="width:800px;">Deja tu opiniÃ³n aquÃ ya que no voy a crear nuevos post debido a que me lo cierran por pavadas:';
	...
	nuevaHTML += '		<br><input type="text" id="tb_msg" name="miarrobatag207890_msg" title="Escribe aquÃel mensaje que aparecerÃ¡ en el Tagboard" value="Tu mensaje aquÃ maxlength="250" class="ui-state-default ui-corner-all" style="width:760px;">';
	...
	nuevaHTML += '		<br><input type="submit" id="miarrobatag207890_submit" title="Enviar mensaje al Tagboard" name="miarrobatag207890_submit" value="EnvÃ Mensaje" class="ui-state-default ui-corner-all" style="width:800px;">';
	nuevaHTML += '	</form></div>';
	nuevaHTML += '</div>';
	...
	nuevaHTML += '    ..::Descargas Sweetdani::.. - Peliculas, Juegos, Series y MÃºsica.<br/>';
	...
	nuevaHTML += '    <a href="http://radio.sharkale.com.ar/" target="_blank"><img src="http://i32.tinypic.com/zkg95l.jpg" alt="..::Radio SharkaleÂ®::.." width="740"/></a>';
	...
	nuevaHTML += '    Â¿Alguna vez te han molestado todos esos anuncios e imÃ¡genes en internet que, normalmente, hacen que tarde en cargar mÃ¡s el resto de la pÃ¡gina? InstalaAdblock Plus ahora y olvÃ de ello.<br/>';
	nuevaHTML += '    <a href="https://addons.mozilla.org/es-ES/firefox/addon/1865" title="AÃ±adir Adblock Plus a Firefox" target="_blank"><img src="http://i40.tinypic.com/2ywy8sg.jpg" align="absmiddle" /><img src="http://2.bp.blogspot.com/_aoEJ4wYL5ww/SJuBhTkrJvI/AAAAAAAAAHQ/GfY6UdrA4Vs/s320/firefox-adblock-plus.png" align="absmiddle" height="150"/></a><hr>';
	nuevaHTML += '    Web of Trust le mantiene seguro mientras navega y compra en Internet. Los sÃ con cÃ³digo de color de WOT le ayudan a evitar timos en lÃ, robo de identidad, sitios de compra no fiables y amenazas de seguridad antes de que haga clic. Navegue mÃ¡s seguro y aÃ±ada WOT a Firefox ahora mismo.<br>';
	nuevaHTML += '    <a href="https://addons.mozilla.org/es-ES/firefox/addon/3456" title="AÃ±adir WOT a Firefox" target="_blank"><img src="http://i40.tinypic.com/2ywy8sg.jpg" align="absmiddle" /><img src="https://addons.mozilla.org/en-US/firefox/images/t/31153/1239092219" align="absmiddle" /><img src="https://addons.mozilla.org/en-US/firefox/images/t/24581/1218474941" align="absmiddle" /><img src="https://addons.mozilla.org/en-US/firefox/images/t/24569/1218460666" align="absmiddle" /></a>';
	nuevaHTML += '</b></div>';
	...
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 24.6:</span></u></b><br>* Arreglado perfil y Puntos en Negrita.<br>* Arreglado el actualizador de comentarios.<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 24.5:</span></u></b><br>* BBCode en comunidades.<br>* Preview en Comunidades.<br>* [Fixed] Bugs BBCode en comunidades.<br>* Avatares en comunidades modificados.<br>* Nueva cÃ³digo para previsualizar mÃ¡s eficÃ¡z.<br>* Tops desactivados. (Taringa! desactivÃ³ la api para los tops. Parece que manolo y su ram de mierda de 64MB no la soporta)<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 24.0:</span></u></b><br>* Nuevo logo de Taringa.<br>* ActualizaciÃ³n general del cambiador de theme a jQuery. (Chupala Gonx lame chota!!!)<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 23.6:</span></u></b><br>* Nuevas Opciones en el Cambiador de Theme.<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 23.5:</span></u></b><br>* Arreglos Varios.<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 23.4:</span></u></b><br>* Color de Fondo Configurable.<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 23.3:</span></u></b><br>* Actualizador de Comentarios (incluido para post privados).<br>';
	nuevaHTML += '    <b><u><span style="color: red;">Changelog versiÃ³n 23.2:</span></u></b><br>* Bugs, Bugs, Bugs y mÃ¡s Bugs.<br>';
	...
	nuevaHTML += '    <span style="font-size:30px;font-family:Arial Black">SharkaleÂ® 2009</span><br/><br/><br/>';
	...
	nuevaHTML += '    <hr><form action="https://www.paypal.com/cgi-bin/webscr" method="post"><input type="hidden" name="cmd" value="_s-xclick"><input type="hidden" name="hosted_button_id" value="7853233">Capaz alguien me dona 10ctv que le quedan ahÃtiraditos en la cuenta de PayPal ^.^ <input type="image" src="https://www.paypal.com/es_ES/ES/i/btn/btn_donate_LG.gif" border="0" name="submit" align="absmiddle" alt="PayPal. La forma rÃ¡pida y segura de pagar en Internet." class="ui-state-default ui-corner-all"><img alt="" border="0" src="https://www.paypal.com/es_XC/i/scr/pixel.gif" width="1" height="1"></form><hr>';
	...
	nuevaHTML += '</div>';
	//nuevaHTML += '    ';
	div.innerHTML = nuevaHTML;
	$('body').before(div);
	$('#ajaxdiv').remove();
	try{$('#SharkaleOpciones').dialog({title: 'Opciones Avanzadas del Script - SharkaleÂ®',width: 900, modal: true, position:['center',30], resizable: false, draggable: false });}catch(e){};
...
var newCSS  = '.menu_izq {-moz-background-clip:border;-moz-background-inline-policy:continuous;-moz-background-origin:padding;background:transparent url(../menu_left_corner.gif) no-repeat scroll left top;float:left;height:30px;margin:auto;padding-left:10px;position:relative;text-align:left;vertical-align:middle;width:700px!important;}';
	...
	menu += '		<li><b>Busqueda RÃ¡pida</b><form method="get" action="http://www.taringa.net/buscador-taringa.php"><input name="q" title="Buscador RÃ¡pido" style="border: 0px none; font-size: 10px; width:160px;padding-bottom:2px;" type="text"></form></li>';
...
//----Cuenta----//
	...
	menu += '			<ul>';
	menu += '				<li><a href="/cuenta/perfil/editar/paso1">FormaciÃ³n y trabajo</a>';
	menu += '				<li><a href="/cuenta/perfil/editar/paso2">MÃ¡s sobre mi</a>';
...
//----Mi Perfil----//
    ...
    menu += '		<li><a href="/comentarios/'+user+'">Mis Ãºltimos comentarios</a></li>';
    menu += '	</ul>';
	menu += '	</li>';
	menu += '</ul><span class="separador">&nbsp;</span>';
//----Logout----//
...
//----Opciones----//
	menu += '<ul class="tMenu">';
	menu += '	<li><a href="javascript:void(0)" id="Sharkale_Opciones" title="Opciones Avanzadas del Script"><img src="'+MenuOpciones+'" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle"/></a></li>';
	menu += '</ul><span class="separador">&nbsp;</span>';
	menu += '<ul class="tMenu">';
	menu += '	<li><a href="http://radio.sharkale.com.ar" title="Visiten la Radio de SharkaleÂ® =P" target="_blank"><img src="http://i29.tinypic.com/3029onc.jpg" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle"/></a></li>';
	menu += '</ul><span class="separador">&nbsp;&nbsp;&nbsp;&nbsp;</span>';
//----Actualizar----//
	menu += '<ul id="MenuAct" class="tMenu" style="display:none;">';
	menu += '	<li><a href="javascript:void(0)" id="Sharkale_Actualizar" title="Hay una nueva versiÃ³n disponible del Script" style="text-decoration:blink;"><img src="'+MenuActualizacion+'" style="width: '+MenuSize+'px; height: '+MenuSize+'px;" align="absmiddle" /> Nueva VersiÃ³n</a></li>';
	menu += '</ul>';
	menu += '</span>';
...
$('#Sharkale_Opciones').click(function() {
	var HTML = '<div id="ajaxdiv" style="border: 1px solid black; padding: 2px; background: rgb(233, 233, 255) none repeat scroll 0% 0%; width: 100px; -moz-background-clip: border; -moz-background-origin: padding; -moz-background-inline-policy: continuous; position: fixed; visibility: visible; opacity: 1; right: 45%; top: 40%;text-align:center;">';
	HTML += '<br /><br /><img src="'+URL+'cargando.gif" alt="Cargando" title="Cargando Opciones" border="0" /><br /><br />';
	HTML += '<span style="font-weight: bold">Cargando Opciones del Script</span><br /><br />';
	HTML += '</div>';
	$('body').append(HTML);
	$.timer(500, function (timer) {
		timer.stop();
...
if(path == 'post'){
	var eliminarbr = $('div[id="cuerpo1"] br');
	eliminarbr.slice(eliminarbr.length-4).remove();
	GM_addStyle('#gif_cargando_add_comment {margin-left:650px;margin-top:30px;position:absolute !important;}');
	document.imagen = new Image;
	document.imagen.src = URL+'cargando.gif';
	$('div[class*="ads"]').parent().remove();
///////////////------------------Agregar Boton Ir Abajo--------------------////////////////////
	if($('#titulo1')){
		$('a[href*="cielo"]').attr('id','pie-a').removeClass('irCielo').attr('style','font-weight:bold;font-size:13px;').attr('href','#maincontainer');
		...
		$('#titulo1').html(Tierra + '<span style="vertical-align: middle;padding-top: 1px;">' + $('#titulo1').html()+ '</span>');
		$('div[class="comentarios_container"]').find('div[class="box_rss"]:first').prepend('<a href="#pie-a" class="size13"><strong>Ir a la Tierra</strong></a>');
	}
///////////////-----------------Agregar FullScreenMode------------------////////////////////
	modificar_posts();
///////////////------------------Modificar Agregar Comentario--------------------////////////////////
	if($("#cuerpo_comment").length != 0){
///////////////-------------------------Agregar BBCodes--------------------------////////////////////
...
///////////////-----------------Agregar Opciones en Comentarios------------------////////////////////
		if(PunFav == true){
			if($('#span_opciones1').size() != 0 || $('#span_opciones2').size() != 0){
				var stropc = '';
				if($('#span_opciones1').size() == 1){
					var opc1 = $('#span_opciones1').clone().html();
					if(opc1.match(/solidarios[\w\W]*span_opciones2/))
						opc1 = '<b>Dar puntos:</b> Los posts solidarios no se pueden calificar';
					else if(opc1.match(/para dar[\w\W]*span_opciones2/))
						opc1 = '<b>Dar puntos:</b> No hay puntos para dar';
					stropc = opc1;
				}else stropc = '<b>Dar puntos:</b> Usuarios Novatos no pueden calificar';
				if($('#span_opciones2').size() == 1){
					var opc2 = $('#span_opciones2').clone();
					opc2.html(opc2.find('a').slice(0,3).html(''));
					opc2 = opc2.html();
					if(stropc != '') 
						stropc = opc2 + ' | ' + stropc;
					else
				...
				$('span[class="box_txt post_agregar_comment"]:first').html('<span style="float:left;text-align:left;">Agregar un nuevo comentario</span><span style="float:right;text-align:right;font-weight:normal;width: 550px;">'+stropc+'</span>');
			}
		}
///////////////-----------------Agregar Emoticones------------------////////////////////
		$('div[class="box_cuerpo agregar_comm_izq"]:first').removeAttr('class');
		$('div[class="box_cuerpo agregar_comm_der"]:first').removeAttr('class').attr('style','text-align: center');
		txt.setAttribute('style', 'min-height: 280px; width: 660px;');
		BotonOcultar = '<input class="button" value="'+(EmotVis == true? "Ocultar Emoticones":"Mostrar Emoticones")+'" onclick="Ocultar_Emoticones(this);return false;" id="ocultar_emot" type="button" title="Mostrar/Ocultar la barra de emoticones rÃ¡pidos">';
		$('#MapSmiles').remove();
		$('a[href="javascript:openpopup()"]').parent().remove();
		$('#smiles').replaceWith('<span style="float:left;">'+BotonOcultar+'</span><span id="seccion_enviar" style="float:right;"></span><br><br>'+CrearBarraEmoticones());
		$('#button_add_comment').attr('id','sharkborrar').clone(true).attr('id','button_add_comment').prependTo('#seccion_enviar');
		$('#sharkborrar').remove();
		$('#button_add_comment').removeAttr('disabled');
		autor = $(':button[value="Eliminar"]').is(':visible');
		GM_addStyle('.Sharkale_cargando {display:none;}');
		HTML = '<div class="citacuerpo" style="font-size: 11px;"><input type="button" value="Actualizar Comentarios - SharkaleÂ®" class="button" title="Actualizar todos los comentarios. Si hay demasiados puede trabarse un momento." onclick="actualizar_comentarios_post()">&nbsp;&nbsp;<span class="Sharkale_cargando"><img src="http://i27.tinypic.com/2wp8bdg.gif" align="absmiddle"/></span></div>';
		$('#return_agregar_comentario').parent().after(HTML);
		...
		$('div[class="box_cuerpo"][style*="font-size"]').attr('id','Sharkale_comentarios');
	}
	if($('div[typeof="foaf:Person"]').find('a[href="/perfil/Brhada"]').length != 0){
		var mod_sharkale = $('div[typeof="foaf:Person"]');
		mod_sharkale.html(mod_sharkale.html().replace(/New Full User/gi,'New Full user').replace(/No Rankeado/,'T! Rank: 1').replace(/ Puntos/,'4684 Puntos'));
		$('img[title="Moderador"]').attr('style','position: absolute; top: -703px; clip: rect(703px, 16px, 720px, 0px);').removeAttr('alt');
		mod_sharkale = $('img[title="Moderador"]').parent().attr('id','icon_shark').clone();
...
///////////////------------------Agregar BBCodes--------------------////////////////////
	...
	if(perfilde == "Brhada"){
		$('li[class="rango"]').css('width','195px').find('span').html('Moderador');
		...
		$('a[href*="bloquear"]:first').attr('href','javascript:alert("Â¿CÃ³mo vas a ser tan sorete de bloquear al creador del script que estas usando? ^^")');
	}
...
function AceptarActualizacion(){
	...
	unsafeWindow.mydialog.title('Actualizador del "BBCoder para Taringa"');
	unsafeWindow.mydialog.body('<h2>Hay disponible una nueva versi&oacute;n del script "BBCoder para Taringa"</h2>Tu versiÃ³n instalada es la <b>' + ultversion +'</b> y la Ãºltima disponible es la <b>' + Act.ver + '</b><h3><u>Cambios de la Ãºltima versiÃ³n:</u></h3><ul type="disc">' + Act.cambio + '</ul><h3>&iquest;Desea actualizar el script?</h3>');
...
function Cambiar_Theme() {
	...
	var html = '<div id="Sharkale_Theme" style="position: fixed; left: '+((window.innerWidth / 2) - 288)+'px; top: '+((window.innerHeight / 2) - 320)+'px; z-index: 1337; background: rgba(255, 255, 255, 0.8) !important;-moz-border-radius:7px;padding:7px !important;-moz-box-shadow:0px 0px 30px rgba(0, 0, 0, 0.9);width: 577px">';
...
if(CambiarTheme) addStyle(eval(GM_getValue('theme','({fondofuera:"#000000", borde:"#480C0C", fondodentro:"#BBBBBB", fondopost:"#CCCCCC" })')));
...
hosts.push (["gigasize.com",
			 ...
			 /Download error|Error en la descarga|Erreur lors du tÃ©lÃ©chargement|Download-Fehler/gi,
			 /[\d.]+[KM]B/
			 ]);
...
hosts.push (["megaupload.com",
			 ...
			 /Enlace no vÃ¡lido|archivo ha sido borrado/,
			 /[\d.]+ [KM]B/
			 ]);
hosts.push (["megavideo.com",
			 /MEGAVIDEO\.COM\/\?/gi,
			 /id=\"playertd\"/gi,
			 /Este video no estÃ¡ disponible/gi,
			 ""
			 ]);
...
hosts.push (["rghost.ru",
			 /RGHOST\.RU\/[\d]+/gi,
			 /Downloaded/,
			 /Ð¾Ð±Ð¼ÐµÐ½ Ñ„Ð°Ð¹Ð»Ð°Ð¼Ð¸|ÑÑ‚Ñ€Ð°Ð½Ð¸Ñ†Ñ‹ Ð½Ðµ ÑÑƒÑ‰ÐµÑÑ‚Ð²ÑƒÐµÑ‚/,
			 /[\d.]+ [KM]B/
			 ]);
...
hosts.push (["yabadaba.ru",
			 ...
			 /ÐŸÐ¾Ð»ÑƒÑ‡Ð¸Ñ‚ÑŒ ÑÑÑ‹Ð»ÐºÑƒ Ð´Ð»Ñ ÑÐºÐ°Ñ‡Ð¸Ð²Ð°Ð½Ð¸Ñ!/gi,
			 ""
			 ]);
...
GM_addStyle("#link_muerto {background:transparent url("+link_muerto_png+") no-repeat scroll 100% 50%;padding-right:17px;}");
GM_addStyle("#link_inaxesible {background:transparent url("+link_nodisponible_png+") no-repeat scroll 100% 50%;padding-right:17px;}");
GM_addStyle("#link_redirector {background:transparent url("+link_redirector_png+") no-repeat scroll 100% 50%;padding-right:65px;}");
