// coding: utf-8
// ==UserScript==
// @name		Ikariam Triumphal Arch
// @namespace	triumphal-arch-11x.ikariam
// @version	149
// @author		oliezekat
// @description	Keep history of your combat reports of won or lost (sea) battles with counter to prevent Bash rule, and plundering daily summary. Ability to support several servers and languages...
// @icon http://s3.amazonaws.com/uso_ss/icon/47093/thumb.png
...
TriumphalArch.Init = function()
	...
	this.Log.Add('Ended after '+((this.EndTime - this.StartTime)/1000)+'s');
	};
...
TriumphalArch.Remove_Old_CitiesStats = function()
	...
	var timeWithoutBattles = new Date();
	timeWithoutBattles.setTime(this.Ikariam.serverTime() - (1000*60*60*24*30*6));
	...
	for (StatsId in this.DB.Stats)
		...
		if (this.DB.Stats[StatsId].deletedcity == true)
			{
			stDelCities++;
			// Need keep them for last combat reports
			}
			
		if ((this.DB.Stats[StatsId].lastTime == undefined) || (this.DB.Stats[StatsId].lastTime <= timeWithoutBattles.getTime()))
			{
			stOldCities++;
			if (delete this.DB.Stats[StatsId]) stDelStats++;
			}
		}
	...
	this.Log.Add(''+stSaved+' cities stats in DB: '+stDelCities+' deleted cities and '+stOldCities+' cities without battles since 6 months. '+stDelStats+' cities stats removed.');
	};
...
TriumphalArch.Merge_Old_CombatReports = function()
	...
	crTimeLimit.setTime(this.Ikariam.serverTime() + this.Ikariam.serverTimeZoneOffset() - this.Ikariam.CombatReport_MaxTime());
...
TriumphalArch.ViewIsCombatReports = function()
	...
	if (this.DB.Get_Text('DIR') == 'rtl')
		{
		var finishedReports = document.getElementById("finishedReports");
		this.DOM.Add_ClassName(finishedReports, 'taRTL');
		}
...
TriumphalArch.ViewIsIsland = function()
	...
	if (this.DB.Get_Text('DIR') == 'rtl')
		...
		this.DOM.Add_ClassName(infocontainer, 'taRTL');
		}
	this.Set_Common_Styles();
...
TriumphalArch.Insert_Island_CitiesInfos = function()
	...
	var CitiesInfos = this.DOM.Get_Nodes("//div[@id='mainview']/ul[@id='cities']/li/*[contains(@class,'cityinfo')]");
	if (CitiesInfos != null)
		{
		...
		*/
		bashTimeLimit.setTime(this.Ikariam.serverTime() + this.Ikariam.serverTimeZoneOffset() - this.Ikariam.Bash_Delay());
		...
		*/
		...
		function FetchCityLevel(sContent)
			{
			var CityLevel = 0;
			var resReg = /[\(]([0-9]+)[\)]/i.exec(sContent);
			if (resReg != null)
				{
				CityLevel = parseInt(resReg[1]);
				}
			return CityLevel;
			}
		...
		for (var i = 0; i < CitiesInfos.snapshotLength; i++)
			{
			var cityInfos = CitiesInfos.snapshotItem(i);
			var container = cityInfos;
			var cityLocation = cityInfos.parentNode;
			var cityInfoType = cityInfos.tagName;
			this.Log.Add('cityInfoType='+cityInfoType);
			
			var cityId = 0;
			var cityName = '';
			var cityLevel = 0;	// Todo: fetch city level
			var cityBattlefieldWall = 0;
			var isBarbarianVillage = false; // Todo
			
			if (cityInfoType == 'UL')
				{
				var lis = cityInfos.getElementsByTagName("li");
				cityName = this.Str.Trim(this.Str.Trim_Brackets(lis[0].childNodes[1].textContent));
				cityLevel = FetchCityLevel(lis[0].childNodes[1].textContent);
			...
			else if (cityInfoType == 'TABLE')
				{
				var lis = cityInfos.getElementsByTagName("tr");
				var tds = lis[0].getElementsByTagName("td");
				cityName = this.Str.Trim(this.Str.Trim_Brackets(tds[1].textContent));
				cityLevel = FetchCityLevel(tds[1].textContent);

				var ulActions = cityLocation.getElementsByTagName("ul")[0];
				var lisActions = ulActions.getElementsByTagName("li");
				for (var k = 0; k < lisActions.length; k++)
					{
					var liAction = lisActions[k];
				...
				// Create OldSchool list
				var tr = document.createElement('tr');
				tr.setAttribute("class", "taCityInfo");
				...
				container.setAttribute("class", "cityinfo");
				td.appendChild(container);
				
				cityInfos.appendChild(tr);
				cityInfoType = 'UL';
				}
				
			// Fix for Army Helper script
			if (cityLevel > 0)
				{
				container.innerHTML = container.innerHTML+'<li class="citylevel taCityLevel"><span class="textLabel">Level: </span>'+cityLevel+'</li>';
				}
			...
			if (cityLevel > 0)
				...
				if (cityLevel < 5)
					{
					cityBattlefieldWall = 3;
					}
				else if (cityLevel < 10)
					{
					cityBattlefieldWall = 5;
					}
				else
					{
					cityBattlefieldWall = 7;
					}
				}
			if (cityBattlefieldWall > 0)
				{
				var WallIcon = '<img src="skin/layout/icon-wall.gif"/>';
						
				var crBattlefield = '';
				for (var w=1; w <= cityBattlefieldWall; w++)
					{
					crBattlefield += WallIcon;
					}
				
				container.innerHTML = container.innerHTML+'<li class="name taBattlefield"><span class="textLabel"><nobr>'+
			...
			for (var j = 0; j < CRs.length; j++)
				...
				if ((cityId != 0) && (this.DB.CombatReports[CombatId].tcityid != undefined) && (this.DB.CombatReports[CombatId].tcityid != 0) && (this.DB.CombatReports[CombatId].tcityid != cityId))
				...
				else if ((cityName == this.DB.CombatReports[CombatId].tcityname) || (this.DB.CombatReports[CombatId].tcityid == cityId))
					{
					var CombatTime			 = new Date(this.DB.CombatReports[CombatId].time);
					var CombatLocalTime		 = new Date(this.DB.CombatReports[CombatId].time-this.Ikariam.serverTimeZoneOffset());
					var BattleClass = '';
					...
					if ((this.DB.CombatReports[CombatId].own == true) || (this.DB.CombatReports[CombatId].isAttacked == true))
						...
						{
						var ResType;
						var IconsNum = 0; // Todo: hide more than 13 icons
						for (ResType in this.DB.CombatReports[CombatId].loot)
							...
							crLoot += ResIcon;
							IconsNum++;
							if (ResValue > 2000) { crLoot += ResIcon; IconsNum++; }
							if (ResValue > 5000) { crLoot += ResIcon; IconsNum++; }
							if (ResValue > 10000) { crLoot += ResIcon; IconsNum++; }
							if (ResValue > 20000) { crLoot += ResIcon; IconsNum++; }
							}
						}
			...
			if (LastBattles != '')
				...
				var CombatLocalTime	 = new Date(CityStats.lastTime-this.Ikariam.serverTimeZoneOffset());
			...
			if (cityInfoType == 'UL')	
				...
				container.innerHTML = container.innerHTML +
					'<li class="name alt taStats" title="'+this.DB.Get_Text('BattlesWLB')+'">'+
...
TriumphalArch.Insert_Next_CombatReports = function(numMax)
	...
	if ((Subjects != null) && (Subjects.snapshotLength > 0))
		...
		bashTimeLimitUTC.setTime(this.Ikariam.serverTime() - this.Ikariam.Bash_Delay());
		...
		for (var i=0; i < Subjects.snapshotLength; i++)
			...
			if ((this.DB.CombatReports[CombatId].own == true) || (this.DB.CombatReports[CombatId].isAttacked == true))
				...
				for (ResType in this.DB.CombatReports[CombatId].loot)
					...
					if (ResValue > 20000) crLoot += ResIcon;
			...
			if (this.DOM.Has_ClassName(tr, 'taStats') == false)
				...
				if (((this.DB.CombatReports[CombatId].own == true) || (this.DB.CombatReports[CombatId].isAttacked == true)) && (this.DB.CombatReports[CombatId].attackers != undefined))
					{
					tds[3].innerHTML += '<div class="taAttackers" dir="ltr">vs '+this.DB.CombatReports[CombatId].attackers+'</div>';
					}
...
TriumphalArch.Set_Island_Styles = function()
	...
	// define CSS 
	var default_style = <><![CDATA[
	#information .cityinfo tr.taCityInfo td { padding:0 !important; }
	#information .cityinfo tr.taCityInfo td ul.cityinfo { width:100% !important; }
	#information .cityinfo tr.taCityInfo td ul.cityinfo li { margin: 2px 0; }
	#information .cityinfo tr.taCityInfo td ul.cityinfo li .textLabel { display:inline-block; min-width:80px; }
	
	li.taCityLevel { display: none; }
	
	li.taBattlefield {clear: both}
	.taBattlefield img {max-height: 12px; margin-right: 1px !important; display: inline !important;}
	...
	li.taLastBattles ul li {
		...
		background-image: url(skin/characters/military/x40_y40/y40_swordsman_faceright.gif);
		background-position: -3px -4px;
		background-repeat: no-repeat;
		}
	.taRTL li.taLastBattles ul li {
		padding-left: 10px !important;
		padding-right: 33px !important;
		background-position: 165px -4px;
		}
	
	li.taLastBattles ul li.running {
		background-image: url(skin/advisors/military/bang_soldier.gif);
		background-position: -1px -4px;
		}
	.taRTL li.taLastBattles ul li.running {
		background-position: 165px -4px;
		}
	
	li.taLastBattles ul li.navy {
		background-image: url(skin/characters/fleet/40x40/ship_ballista_r_40x40.gif);
		background-position: -3px -7px;
		}
	.taRTL li.taLastBattles ul li.navy {
		background-position: 165px -4px;
		}
	...
	li.taLastBattles ul li img {
		max-height: 11px;
		margin: 0px !important;
		margin-right: -8px !important;
		display: inline !important;
		}
	.taRTL li.taLastBattles ul li img {
		margin: 0px !important;
		margin-left: -5px !important;
		}
	
	li.taLastBattles ul li .taUnConfirmed img { margin-right: 0px !important; margin-left: 2px !important; }
	.taRTL li.taLastBattles ul li .taUnConfirmed img { margin-right: 2px !important; margin-left: 0px !important; }
	]]></>.toXMLString();
	GM_addStyle(default_style);
	};
...
TriumphalArch.Set_CombatReports_Styles = function()
	...
	.taRTL table.operations thead tr th.subject {text-align: right;}
	
	table.operations thead tr th.taIslands img { border: none; max-height: 15px;}
	
	table.operations thead tr th.taLoots {text-align: center; border-left: 1px solid #FDF1D4;}
	.taRTL table.operations thead tr th.taLoots {border-left: none; border-right: 1px solid #FDF1D4;}
	
	table.operations tbody tr.taStats {background-color: #FDF7DD;}
	table.operations tbody tr.alt {background-color: #FDF1D4;}
	
	table.operations tbody tr.taStats td.date { min-width: 105px; }
	
	table.operations tbody tr.taStats td.subject { 
		background-image: url(skin/characters/military/x40_y40/y40_swordsman_faceright.gif);
		background-position: 0px -4px;
		background-repeat: no-repeat;
		padding-left: 42px !important;
		border-bottom: 1px solid #FDF1D4;
		width: auto !important;
		}
	.taRTL table.operations tbody tr.taStats td.subject {
		padding-left: 0px !important;
		padding-right: 42px !important;
		background-position: right -4px;
		}
		
	table.operations tbody tr.taNavy td.subject {
		background-image: url(skin/characters/fleet/40x40/ship_ballista_r_40x40.gif);
		background-position: 0px -6px;
		}
	.taRTL table.operations tbody tr.taNavy td.subject {
		background-position: right -6px;
		}
		
	table.operations tbody tr.taStats td.running { 
		background-image: url(skin/advisors/military/bang_soldier.gif);
		background-position: 0px -4px;
		}
	.taRTL table.operations tbody tr.taStats td.running {
		background-position: right -4px;
		}
		
	table.operations tbody tr.taNavy td.running { 
		background-image: url(skin/advisors/military/bang_ship.gif);
		background-position: 0px -1px;
		}
	.taRTL table.operations tbody tr.taNavy td.running {
		background-position: right -1px;
		}
		
	table.operations tbody tr td.subject .taUnConfirmed img { margin-right: 0px !important; margin-left: 2px !important; }
	.taRTL table.operations tbody tr td.subject .taUnConfirmed img { margin-right: 2px !important; margin-left: 0px !important; }
	
	table.operations tbody tr.taStats td.subject .taAttackers {
		font-size: 10px;
		color: #CB9B6A;
		direction: ltr;
		}
	...
	table.operations tbody tr.taStats td.taBash { border-left-style: solid; border-right-style: solid;}
	
	table.operations tbody tr.taStats td.taLoot {text-align: left; }
	.taRTL table.operations tbody tr.taStats td.taLoot {text-align: right; }
	
	table.operations tbody tr.taStats td.taLoot img {max-height: 13px; margin-right: -8px;}
	.taRTL table.operations tbody tr.taStats td.taLoot img { margin-right: 0px;margin-left: -8px;}
	...
	.taRTL #taSumLoots { padding-right: 24px !important; }
	
	#taSumLoots img { width: 13px; max-height: 9px; margin-right: -16px;}
	]]></>.toXMLString();
	GM_addStyle(default_style);
...
TriumphalArch.DB.Load_Texts = function(lang)
	...
	this._Parent.Log.Add('Set language code: '+lang);
	...
	*/
	...
	Texts['ae'].UnConfirmedCR	 = "<span dir='ltr'>(%1)</span> combat report not confirmed.";
	Texts['ae'].EnhancedBy		 = "Enhanced by %1 <span dir='ltr'>(v. %2)</span>.";
	...
	Texts['il']					 = {};
	Texts['il'].DIR				 = 'rtl';
	Texts['il'].WonLostBash		 = "bash / lost / won"; 
	Texts['il'].BattlesWLB		 = "Battles: Bash / Lost / Won";
	Texts['il'].WLB				 = "Bash/L/W"; 
	Texts['il'].UnConfirmedCR	 = "<span dir='ltr'>(%1)</span> combat report not confirmed.";
	Texts['il'].wood			 = "עצים";
	Texts['il'].wine			 = "יין";
	Texts['il'].marble			 = "שיש";
	Texts['il'].glass			 = "גביש קריסטל";
	Texts['il'].sulfur			 = "גופרית";
	Texts['il'].EnhancedBy		 = "Enhanced by %1 <span dir='ltr'>(v. %2)</span>.";
	...
	Texts['pk']					 = {};
	Texts['pk'].DIR				 = 'rtl';
	Texts['pk'].BattleFor		 = "%1 کی لڑائ";
	Texts['pk'].SeaBattleNear	 = "%1 شہر کے سامنے سمندر مین لڑائ";
	Texts['pk'].WonLostBash		 = "bash / lost / won"; 
	Texts['pk'].BattlesWLB		 = "Battles: Bash / Lost / Won";
	Texts['pk'].WLB				 = "Bash/L/W"; 
	Texts['pk'].UnConfirmedCR	 = "<span dir='ltr'>(%1)</span> combat report not confirmed.";
	Texts['pk'].wood			 = "عمارتی سامان";
	Texts['pk'].wine			 = "جوس";
	Texts['pk'].marble			 = "مر مر کا پتھر";
	Texts['pk'].glass			 = "شیشہ گلاس";
	Texts['pk'].sulfur			 = "سلفر";
	Texts['pk'].EnhancedBy		 = "Enhanced by %1 <span dir='ltr'>(v. %2)</span>.";
	...
	// ikariam.si by damac
	Texts['si']					 = {};
	Texts['si'].BattleFor		 = "Bitka za %1";
	Texts['si'].SeaBattleNear	 = "Morska bitka blizu %1";
	Texts['si'].Battles			 = "Bitke";
	Texts['si'].LastBattle		 = "Zadnja bitka";
	Texts['si'].BattlesXXXhrs	 = "Bitke v zadnjih %1 urah";
	Texts['si'].WonLostBash		 = "Z / P / B";
	Texts['si'].NoMoreCR		 = "Ni več nedavnih poročil napadov";
	Texts['si'].NextXXXCR		 = "naslednjih %1 nedavnih poročil napadov";
	Texts['si'].BattlesWLB		 = "Bitke: Zmage / Porazi / Bash";
	Texts['si'].WLB				 = "Z / P / B";
	Texts['si'].UnConfirmedCR	 = "(%1) Poročilo je nepotrjeno.";
	Texts['si'].Loots			 = "Plen";
	Texts['si'].wood			 = "les";
	Texts['si'].wine			 = "vino";
	Texts['si'].marble			 = "marmor";
	Texts['si'].glass			 = "kristal";
	Texts['si'].sulfur			 = "žveplo";
	Texts['si'].DoUpgrade		 = "Želite naložiti \"%1\" v.%2 ?";
	Texts['si'].EnhancedBy		 = "Izboljšano z %1 (v.%2).";
	Texts['si'].NewVersion		 = "NOVA VERZIJA v.%1 JE NA VOLJO !";
	Texts['si'].BattleField		 = "Bojišče";
	Texts['si'].SinceX			 = "od %1";
	
	// sk.ikariam.com by Michal Diablo
	Texts['sk']                  = {};  
	Texts['sk'].DIR              = '';
	Texts['sk'].BattleFor        = "Boj o %1";
	Texts['sk'].SeaBattleNear    = "Námorný boj pri %1";
	Texts['sk'].Battles          = "Battles";  
	Texts['sk'].LastBattle       = "Posledné boje";  
	Texts['sk'].BattlesXXXhrs    = "Boje za posledných %1hrs"; 
	Texts['sk'].WonLostBash      = "Víťazstvá / Porážky / Bash"; 
	Texts['sk'].NoMoreCR         = "Žiadne daľšie správy.";  
	Texts['sk'].NextXXXCR        = "ďaľšie %1 správy z boja...";
	Texts['sk'].BattlesWLB       = "Boje: Víťazstvá / Porážky / Bash";  
	Texts['sk'].WLB              = "V. / P. / Bash";
	Texts['sk'].UnConfirmedCR    = "(%1) správa z boja nebola potvrdená.";
	Texts['sk'].Loots            = "Korisť";  
	Texts['sk'].wood             = "Stavebný materiál";  
	Texts['sk'].wine             = "Víno";  
	Texts['sk'].marble           = "Mramor";  
	Texts['sk'].glass            = "Kryštálové sklo";  
	Texts['sk'].sulfur           = "Síra";  
	Texts['sk'].DoUpgrade        = "Chceš nainštalovať \"%1\" v. %2 ?";
	Texts['sk'].EnhancedBy       = "Enhanced by %1 (v. %2)."; 
	Texts['sk'].NewVersion       = "NOVÁ VERZIA V. %1 DOSTUPNÁ !";
	Texts['sk'].BattleField      = "Bojisko"; 
	Texts['sk'].SinceX           = "Since %1";
...
TriumphalArch.DB.Load_Filters = function(lang)
	...
	/* 
	IMPORTANT:
	Don't translate english filters. You may create filters defined with texts as displayed into the game for your world server.
	*/
	...
	// ikariam.il
	Filters['il']					 = {};
	Filters['il'].DefaultCityName	 = "פוליס";
	Filters['il'].BarbarianVillage	 = "כפר הברברים";
	Filters['il'].Barbarians		 = "גודל העיר";
	...
	// pk.ikariam.com (todo)
	Filters['pk']					 = {};
	Filters['pk'].DefaultCityName	 = "لاہور";
	Filters['pk'].BattleFor			 = "(.*) کی لڑائ";
	Filters['pk'].SeaBattleNear		 = "(.*) شہر کے سامنے سمندر مین لڑائ";
	Filters['pk'].PlayerFromCity	 = "(.+)\\s+کے(.*)\\s+سے";
	Filters['pk'].BarbarianVillage	 = "بیر بیرین";
	Filters['pk'].Barbarians		 = "وحشی";
	...
	// ikariam.si
	Filters['si']					 = {};
	Filters['si'].BattleFor			 = "^Bitka\\s+za\\s+mesto\\s+(.*)$";
	Filters['si'].SeaBattleNear		 = "^Bitka\\s+na\\s+morju\\s+blizu\\s+mesta\\s+(.*)$";
	Filters['si'].PlayerFromCity	 = "^(.+)\\s+iz\\s+(.*)$";
	Filters['si'].BarbarianVillage	 = "Barbarsko naselje";
	Filters['si'].Barbarians		 = "Barbari";
	Filters['si'].DeletedPlayer		 = "Izbrisan igralec";
	
	// sk.ikariam.com by Michal Diablo
	Filters['sk']                    = {};  
	Filters['sk'].BattleFor          = "^Boj\\s+v\\s+(.*)$";
	Filters['sk'].SeaBattleNear      = "^Námorný\\s+boj\\s+pri\\s+(.*)$"; 
	Filters['sk'].PlayerFromCity     = "^(.+)\\s+z\\s+(.*)$"; 
	Filters['sk'].CityOfPlayer       = ""; // 
	Filters['sk'].BarbarianVillage   = "Barbarská dedina"; 
	Filters['sk'].Barbarians         = "Barbari"; 
	Filters['sk'].DeletedPlayer      = "Vymazaný hráč"; 
...
TriumphalArch.DB.Get_Text = function(key, s1, s2, s3, s4, s5, s6)
	...
	var Output = '';
	var Values = [];
	if (s1 != undefined) Values[0] = s1;
	if (s2 != undefined) Values[1] = s2;
	if (s3 != undefined) Values[2] = s3;
	if (s4 != undefined) Values[3] = s4;
	if (s5 != undefined) Values[4] = s5;
	if (s6 != undefined) Values[5] = s6;
	
	if ((this.Texts != undefined) && (this.Texts != null) && (this.Texts[key] != undefined))
		{
		Output = this.Texts[key];
		
		Output = this._Parent.Str.ValuesReplace(Output,Values);
		}
	else
		{
		Output = key;
...
TriumphalArch.DB.Insert_CombatReport_Stats = function(StatsId, CombatReport)
	...
	if (CombatReport.time > 0)
		...
		if ((this.Stats[StatsId].own != true) && (CombatReport.own != true) && (CombatReport.isAttacked != true) && (CombatReport.tBarbarianVillage != true) && (CombatReport.isEmpty != true))
			{
			var bashTimeLimit = new Date();
			// GameForge servers are germany time-zoned for CRs
			bashTimeLimit.setTime(this._Parent.Ikariam.serverTime() + this._Parent.Ikariam.serverTimeZoneOffset() - this._Parent.Ikariam.Bash_Delay());
...
TriumphalArch.Ikariam.phpSet = function(valueName)
	{
...
TriumphalArch.Ikariam.Grab_JSCompleteScript_Version = function(defaultVersion)
	{
	// Requires: DOM
	var Version = '';
...
TriumphalArch.Ikariam.serverTime = function()
	...
	if (this._serverTime == null)
		{
		if (this.phpSet('serverTime') != undefined)
			{
			this._serverTime = parseInt(this.phpSet('serverTime'))*1000;
...
// Diff with UTC time for combat reports because GameForge servers are germany time-zoned .
TriumphalArch.Ikariam.serverTimeZoneOffset = function()
	{
	if (this._serverTimeZoneOffset == null)
		{
		var serverTimeZoneOffset = (2*60*60*1000); // Summer time +2
		
		if ((unsafeWindow != undefined) && (unsafeWindow.startServerTime != undefined))
			{
			var localTime = new Date();
			var startServerTime = unsafeWindow.startServerTime;
			
			serverTimeZoneOffset = localTime.getTime() - localTime.getTimezoneOffset()*60*1000 - startServerTime;
			
			serverTimeZoneOffset = Math.round(serverTimeZoneOffset/(60*1000))*60*1000;
			}
		
		this._Parent.Log.Add('serverTimeZoneOffset: '+serverTimeZoneOffset+'ms');
...
// Diff with user local time
TriumphalArch.Ikariam.serverTimeDiff = function()
	{
	if (this._serverTimeDiff == null)
		{
		var serverTimeDiff = (parseInt(this.phpSet('serverTime'))*1000-(new Date()).getTime());
		
		serverTimeDiff = Math.round(serverTimeDiff/(60*1000))*60*1000;
		
		this._serverTimeDiff = serverTimeDiff;
		this._Parent.Log.Add('serverTimeDiff: '+this._serverTimeDiff+'ms');
		}
	
	return this._serverTimeDiff;
	};
	
TriumphalArch.Ikariam.LocalizationStrings = function(valueName,sectionName,subSectionName)
	{
...
TriumphalArch.Ikariam.CombatReport_MaxTime = function()
	{
	return (1000*60*60*24*7*1+this.serverTimeZoneOffset());
	};
...
TriumphalArch.Ikariam.Is_Version_0424 = function()
	{
	// Requires: Str
...
TriumphalArch.Ikariam._Fetch_CombatReport_v032 = function(database)
	...
	if (DefendersDIV != null)
		...
		for (i = 0; i < resDefenders.length; i++)
			...
			var playerid = 0;
			var resReg = /[\?&;]{1}receiverId=([0-9]+)\"\>(.*)\<\/\a.*\<\/\a/i.exec(DefenderHTML);
			if (resReg != null)
				{
...
TriumphalArch.DOM.Get_Nodes = function(path, root)
	{
...
TriumphalArch.DOM.Get_First_Node = function(path, root)
	{
...
TriumphalArch.DOM.Get_First_Node_Value = function(path, defaultValue, root)
	{
...
TriumphalArch.DOM.Get_First_Node_TextContent = function(path, defaultValue, root)
	{
	var value = this.Get_First_Node(path, root);
	if (value != null)
		{
...
TriumphalArch.Str.ValuesReplace = function(text, values)
	{
...
TriumphalArch.Str.FormatBigNumber = function(num, alwaysShowSign, thousandSeparator)
	{
