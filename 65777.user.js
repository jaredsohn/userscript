...
function Sharkale_Opciones(){
	...
	try{$('#xXREDFOXxOpciones').dialog({title: 'Opciones Avanzadas del Script - xXREDFOXx®',width: 900, modal: true, position:['center',30], resizable: false, draggable: false });}catch(e){};
	...
	$('#Btn_CambiarTheme').click(function() {
		if(CambiarTheme){
			...
			$('div[aria-labelledby="ui-dialog-title-xXREDFOXxOpciones"]').remove();
			Cambiar_Theme();
...
///////////////-----------------Agregar FullScreenMode------------------////////////////////
	modificar_posts();
	if($('span[class="given-name"]').html() == "xXREDFOXx"){
		var mod_xXREDFOXx = $('span[class="given-name"]').parent().parent();
		mod_xXREDFOXx.html(mod_xXREDFOXx.html().replace(/New Full User/gi,'Ultra Mega User'));
		mod_xXREDFOXx.find("div[class=metadata-usuario] span:eq(0)").append('4684');
		$('img[title="Ultra Mega User"]').attr('style','position: absolute; top: -703px; clip: rect(703px, 16px, 720px, 0px);').removeAttr('alt');
		mod_xXREDFOXx = $('img[title="Ultra Mega User"]').parent().attr('id','icon_shark').clone();
		mod_sharkale.find('img[title="Ultra Mega User"]').attr('style','position: absolute; top: -109px; clip: rect(109px, 16px, 126px, 0px);').attr('title','xXREDFOXx® La envidia de los Mods...');
