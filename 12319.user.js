// ==UserScript==
// @name           ImmobilienScout24Helper
// @namespace      http://www.314bits.com
// @description    Lets you write comments on properties and hide non-interesting apartments
...
// All your GM code must be inside this function
    function letsJQuery() {
			...
			// we look for H4 tags that include a link, with the text obIdControl
			// and we execute this function for each H4
			$('H4 A[@href*="obIdControl"]').each(function(i){
				// grab the ID
				...
				$('H4[A[@href*="obIdControl=' + id +'"]]').append(
					...
					'" class="abe_ta" style="border:1px solid #EEE; padding:3px; height:30px; width:450px;">' + 
					comment + '</textarea>');
				
				// add a new row (TR) containing a show button for hidden properties
				$('H4[A[@href*="obIdControl=' + id +'"]]').parent().parent().next().next().
					after('<tr style="background-color:#EEE;"><td/><td/><td colspan="4"><input id="abe_show_' + id + '" type="button" class="abe_show" value="show" style="border:1px solid #BBB; margin-right:10px;"></td></tr>');				
				...
				if (visible == "hidden") {
					$('H4[A[@href*="obIdControl=' + id +'"]]').parent().parent().hide().next().hide().next().hide().next().show();				
				} else {
					$('H4[A[@href*="obIdControl=' + id +'"]]').parent().parent().show().next().show().next().show().next().hide();									
				}
