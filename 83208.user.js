...
// @namespace      http://userscripts.org/users/180390
...
var version = "1.3.18050"; // cant use cdata+regexp to grab version dynamically because of webkit
...
whichRegex : /(part|sample|seeds|give item to|gnome|bushel|catch a|reward|an egg|eggs|decoration|bonus|hatch|adopt|bouquet|perfect bunch|present|fuel|help|collectible|flag|get materials|cow|valentine|claim|some|gold|bamboo|reed thatch|bottle|blanket|smoker|beeswax|brick|nail|wooden board|horseshoe|harness|horse|baby animal|floral bracket|green beam|glass sheet|white trellis|irrigation pipe|honeybee|queen bee)/,
ampRegex : /&amp;/g,
spaceRegex : /\s+/g,
colorRegex : /(stallion|cream mini|miniature|white-tailed|morgan|red brown|simmental|breton|party|belted|haflinger|cream draft|groovy|holstein|mouflon|mustang|tuscan|longhorn|appaloosa|neapolitan|boer|chocolate|clydesdale|percheron|pinto|silver|(black|blue|purple|pink) pony|pink patch|kelly green|red|purple|pony|green|pink|brown|black|baby|white|gr[ae]y)(?=.+)/,
calfRegex : /found a (simmental|fan|neapolitan|holstein|groovy|longhorn|belted|chocolate) calf/,
animalRegex : /(mini f|buck|pig|llama|goat|cow|turtle|turkey|sheep|kitten|kitty|duckling|duck|rabbit|bull|stallion|penguin|horse|calf?|foal?|pony(?! foal?))/,
...
case "claim": if(text.find("collectible")) w="collectible";
				else if(lText.find("pig")) w="adopt";
...
case "getmaterials": if(text.find("Garage")) w="materialsgarage";
...
var lText=e.textContent.toLowerCase(), w=lText.match(main.colorRegex), text=e.parentNode.parentNode.parentNode.textContent;
...
if(lText.find("calf") && main.calfRegex.test(text.toLowerCase())) w = text.toLowerCase().match(main.calfRegex)[1];
return w;
},
...
if(gotItem==true && (yes || no)) {
	...
	switch(w) {
		...
		case "collectiblegemstone": whichCollectibleOffered = main.checkCollectible(doc, text, main.whichGemstoneRegex); break;
	}
...
GM_config.init("<img src='"+imgs.logo+"'> v"+version, {
	bonus : {
		...
		kids : {
			...
			gnome : {
				label : "Get Gnomes? (bonus as a fallback if there's no gnome)",
				type : "checkbox",
				"default" : false
			}
		}
	...
	giveitemto : {
		label : "Send gifts on give item links?",
		type : "checkbox",
		"default" : true
	},
	hatchwhite : {
	...
	foalbrown : {
		...
		kids : {
			...
			foalcreammini : {
				label : "Cream Mini Foal",
				type : "checkbox",
				"default" : false
			},
			foalminiature : {
				label : "Miniature Foal",
				type : "checkbox",
				"default" : false
			},
			minifoalstallion : {
				label : "Stallion Mini Foal",
				type : "checkbox",
				"default" : false
			}
		}
