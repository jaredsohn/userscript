...
// @description    KOC PwrTools traduit par mimi28700
// ==/UserScript==
...
var Options = {
  ...
  foodWarnHours : 24,
  lastVersion : null,
  alertConfig  : {aChat:false, aPrefix:'** je me fais attaquer ! **', scouting:false, wilds:false, minTroops:10000, spamLimit:10 },
};
...
//logit ( 'Options.ptWinPos: '+ inspect (Options.ptWinPos, 5, 1)); 
  ...
  AddMainTabLink('POWER fr', eventHideShow, mouseMainTab);
}
...
var knightRoles = [
  ['Contremaitre', 'politique', 'Pol'],
  ['Maréchal', 'combat', 'Com'],
  ['Alchimiste', 'intelligence', 'Int'],
  ['Régisseur', 'resource', 'Res'],
];
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      if (matTypeof(cWilds) != 'array') {
        m += '<TR style="background-color:white; font-weight:bold;" align=right><TD align=left>Wild Type</td><TD></td><TD align=left>Coords</td><TD>Traps</td><TD align=left>Mercenaries</td>\
         <TD width=15></td><TD colspan=3 class=entry>'+ htmlTitleLine(' Ajouter les Defences ') +'</td></tr>';
...
// TODO: Seed.wildDef is null if just aquired     
  ...
  processQue : function (errMsg){
    ...
    if (div.innerHTML != '')
      div.innerHTML += 'terminer.<BR>';
    var wild = Seed.wilderness['city'+ t.buildList.cityId]['t'+what[1]];
    if (what[0] == 'T'){
      div.innerHTML += 'Building '+ what[2] +' traps for '+ Cities.byID[t.buildList.cityId].name +'\'s TS a '+ wild.xCoord +','+ wild.yCoord +' ... ';
      t.postBuyTraps (t.buildList.cityId, what[1], what[2], t.processQue);
    } else {
      div.innerHTML += 'mettre Mercenaire '+ mercNames[what[2]] +' pour '+ Cities.byID[t.buildList.cityId].name +'\'s wilderness at '+ wild.xCoord +','+ wild.yCoord +' ... ';
...
var messageNav = {
  ...
  msgSendHook : function (){
    ...
    params.subject = document.getElementById("modal_msg_write_subj").value +' [envoyer a tt les leaders]';
...
var TowerAlerts = {
  ...
  viewImpending_hook : function (atkinc){    
    ...
    if (t.fixTargetEnabled){ 
      ...
      if (!city || (atkinc.marchType!=3 && atkinc.marchType!=4)){  
        target = '<B>FAUX RAPORT!</b>';
        isFalse = true;
      } else if (city.tileId == atkinc.toTileId){
        target = city.name + ' ('+ city.x + ','+ city.y +')';
      } else {
        wilds = Seed.wilderness['Ville'+atkinc.toCityId];
        m = '';
        for (k in wilds){
  ...
  postToChat : function (m, force){
    ...
    if (DEBUG_TRACE) logit ("checkTower(): imminent a "+ unixTime()  +": \n"+ inspect (m, 8, 1));
    if (m.marchType == null)      // bogus march (returning scouts)
      return;
    if (ENABLE_TEST_TAB) Tabs.Test.addDiv ("imminent !<BR><PRE style='margin:0px;'>" + inspect (m, 8, 1) +'</pre>');
    if (m.marchType == 3){
    ...
    if ( city.tileId == m.toTileId )
      ...
      for (k in Seed.wilderness['city'+m.toCityId]){
        if (Seed.wilderness['city'+m.toCityId][k].tileId == m.toTileId){
          target += ' a '+ Seed.wilderness['city'+m.toCityId][k].xCoord +','+ Seed.wilderness['city'+m.toCityId][k].yCoord;
          break;
        }
      }
    }
	...
	if (t.postToChatOptions.includealliance)
	  var alliance = 'de '+Seed.allianceNames[allianceId]+'('+diplomacy+')';
	 else
		alliance = '';
    ...
    msg += 'Ma '+ target +' est '+ atkType  +' par '+ who +' ' +alliance+'. troupe a l aproche (arrivent dans '+
    ...
    if ( city.tileId == m.toTileId ){
      ...
      if (emb.count > 0){
        ...
        msg += ' mon ambasade a '+ availSlots +' sur '+ emb.maxLevel +' place disponible.';
      }
    }
...
Tabs.AllianceList = {
  ...
  init : function (div){
    ...
    var m = '<DIV class=ptentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Saisissez tout ou une partie d un nom de joueur:&nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=allPlayName size=20 type=text /> &nbsp; <INPUT id=playSubmit type=submit value="recherche joueurs" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptplayErr></span></td></tr>\
        <TR><TD class=xtab>OU: </td><TD class=xtab> Saisissez tout ou une partie d un nom d aliance &nbsp;</td>\
          <TD class=xtab><INPUT id=allAllName type=text /> &nbsp; <INPUT id=allSubmit type=submit value="recherche aliance" /></td>\
          <TD class="xtab ptErrText"><SPAN id=ptallErr></span></td></tr>\
		<TR><TD class=xtab>OU: </td><TD class=xtab align=right> liste de aliance: &nbsp;</td>\
		    <TD class=xtab><INPUT align=left id=idMyAllSubmit type=submit value="mon aliance"/>\
            <INPUT align=left id=allListSubmit type=submit value="liste toute les aliances" /></td></tr>\
  ...
  eventPlayerSubmit : function (){
    ...
    t.pName = name;
    if (name.length < 3){
      document.getElementById('ptplayErr').innerHTML = 'entrer au moin 3 caractere';
      return;
    }
  ...
  eventGotPlayerList : function (rslt){
    ...
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PTpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="PTpl(this, \''+ u.userId +'\')"><A>leaderboard</a></span> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>statut en ligne</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>derniere connection</a></span></td></tr>';
    }
    m += '</table></div>';
  ...
  clickedPlayerDetail : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "recherche detail ...";
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "recherche info du leaderboard ...";
  ...
  clickedPlayerCheckOnline : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "recherche du statut en ligne ...";
  ...
  clickedPlayerGetLastLogin : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "recherche des detail de connection ...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>Leaderboard:</b> non disponible! (sous brume?)';
      return;
    }
  ...
  gotPlayerLastLogin : function (rslt, span){
    ...
    if (lastLogin) {
      m = '<span style="color:blue">derniere connection: '+lastLogin+'</span>';
    } else {
  ...
  eventSubmit : function (){
    ...
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'entrer au moin 3 caractere';
      return;
    }
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
    document.getElementById('allListOut').innerHTML = '<BR><BR><CENTER>recherche...</center>';
  ...
  eventListSubmit : function (){
    ...
    if (myA[0]!=0  ) {
       ...
       document.getElementById('allListOut').innerHTML = 'vous devez etre dans une aliance pour utiliser cet partie.';
    }
  },
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      ...
      m += '<TR style="max-height:30px"><TD class='+ tabclass +'>'+ t.dat[i][0] +'</td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
			...
			<td class='+ tabclass +'><SPAN onclick="PCpo(this, \''+t.dat[i][9] +'\')"><A>statut en ligne</a></span></td>\
			<td class='+ tabclass +'><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>derniere connection</a></span><td> </tr>';
    }
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    document.getElementById('altInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
        <TD class=xtab>Indiquer la distance parcourue à partir de: &nbsp; X: <INPUT size=2 type=text id=plyrX /> Y: <INPUT size=2 type=text id=plyrY /> &nbsp; ou, choisi une ville <span id=dmcoords></span></td></tr></table>';
  ...
  fetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    ...
    function combineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
        if (rsltA.msg.indexOf("aucune aliance disponible")!=0 || !rsltM.ok){
          notify (rsltA);
          return;
        }
        rsltA.ok = true;
...
Tabs.Test = {
  ...
  init : function (div){
	...
	var citySelect = '   <SELECT id=fakeCity>';
	    ...
	    citySelect += '</select>';
    var m = '<TABLE><TR><TD align=right>eclairage: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        <TR><TD align=right>ts: </td><TD><INPUT type=checkbox id=fakeIsWild></td></tr>\
        <TR><TD align=right>faux raport: </td><TD><INPUT type=checkbox disabled id=fakeFalse></td></tr>\
        <TR><TD align=right>Seconds: </td><TD><INPUT type=text size=4 value=300 id=fakeSeconds></td></tr>\
        <TR><TD align=right># d unite de ravitaillement: </td><TD><INPUT type=text size=6 value=0 id=fakeSupply></td></tr>\
		<TR><TD align=right># de milicien: </td><TD><INPUT type=text size=6 value=5000 id=fakeMilitia></td></tr>\
		<TR><TD align=right># de eclaireur: </td><TD><INPUT type=text size=6 value=0 id=fakeScout></td></tr>\
		  <TR><TD align=right># de piquier: </td><TD><INPUT type=text size=6 value=0 id=fakePike></td></tr>\
		  <TR><TD align=right># de paladin: </td><TD><INPUT type=text size=6 value=0 id=fakeSword></td></tr>\
		  <TR><TD align=right># d archer: </td><TD><INPUT type=text size=6 value=0 id=fakeArcher></td></tr>\
		  <TR><TD align=right># de cavalier: </td><TD><INPUT type=text size=6 value=0 id=fakeCav></td></tr>\
		  <TR><TD align=right># de cavalier lourd: </td><TD><INPUT type=text size=6 value=0 id=fakeHeavy></td></tr>\
		  <TR><TD align=right># de Wagons de ravitaillement: </td><TD><INPUT type=text size=6 value=0 id=fakeWagon></td></tr>\
		  <TR><TD align=right># of Baliste: </td><TD><INPUT type=text size=6 value=0 id=fakeBallista></td></tr>\
		  <TR><TD align=right># of Belier: </td><TD><INPUT type=text size=6 value=0 id=fakeRam></td></tr>\
		  <TR><TD align=right># of Catapulte: </td><TD><INPUT type=text size=6 value=0 id=fakeCat></td></tr>\
		  <TR><TD align=right>fau nom a utiliser: </td><TD><INPUT type=text size=13 value=oftheNOOBS id=fakeName></td></tr>\
		  <TR><TD align=right>ville ciblé: </td><TD>'+citySelect+'</td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=testSendMarch type=submit value="lancer fausse attaque" \></td></tr></table>\
...
Tabs.Info = {
  ...
  init : function (div){
    ...
    m = '<STYLE>.xtabH {background:#ffffe8; border:none; padding-right: 5px; padding-left: 5px; margin-left:10px; }\
        ...
        <DIV style="height:650px; max-height:650px; overflow-y:auto; overflow-x:hidden"><DIV class=ptstat>INFORMATION DES UNITE</div><TABLE align=center cellpadding=1 cellspacing=0>\
        <TR align=center><TD class=xtab></td><TD class=xtabHL colspan=5><B>prix pour construire</b></td><TD class=xtabHL colspan=7><B>STATISTIQUE</b></td><TD class=xtabHL><B>Upkeep</b></td></tr>\
        <TR valign=bottom align=right><TD class=xtab></td><TD class=xtabHL>Nouriture</td><TD class=xtabH>Bois</td><TD class=xtabH>Pierre</td>\
        <TD class=xtabH>Minerai</td><TD class=xtabH>Pop</td><TD class=xtabHL>Might</td><TD class=xtabH>Life</td><TD class=xtabH>Atack</td><TD class=xtabH>Defence</td><TD class=xtabH>Vitesse</td><TD class=xtabH>Range</td><TD class=xtabH>Load</td>\
        <TD class=xtabHL>Nouriture</td></tr>\
    ...
    for (ui=1; ui<13; ui++){
      ...
      cost = unsafeWindow.unitcost['unt'+ui];     //  NOM, Nouriture, Bois, Pierre, Minerai, ?, IdlePop, Temps
      stats = unsafeWindow.unitstats['unt'+ui];   //  Vie, Attaque, Defense, Vitesse, Range, Load
    ...
    m += '<BR><DIV class=ptstat>CALCULATEUR DE DISTANCE</div><DIV class=ptentry><TABLE align=center cellpadding=1 cellspacing=0>\
      <TR><TD class=xtab align=right><B>premiere coord: </b></td><TD  class=xtab> X: <INPUT id=calcX type=text\> Y: <INPUT id=calcY type=text\> Or, choose city: <SPAN id=ptloc1></span></td></tr>\
      <TR><TD class=xtab><B>deuxieme coord: </b></td><TD class=xtab> X: <INPUT id=calcX2 type=text\> Y: <INPUT id=calcY2 type=text\> ou, choisi une ville <SPAN id=ptloc2></span></td></tr></table>\
...
//    ce.style.top = (off.top+y+provMapCoords.topMargin-eMap.offsetHeight) +'px';      
  ...
  eventLocChanged : function (city, x, y){
    ...
    var m = 'la distance entre '+ x1 +','+ y1 +' et '+ x2 +','+ y2 +' est: &nbsp;<B>'+ distance (x1, y1, x2, y2).toFixed(2) +'</b>';
  ...
  init : function (div){
    ...
    try {      
      m = '<TABLE class=ptTab>\
        ...
        <TR><TD><INPUT id=ptAllowWinMove type=checkbox /></td><TD>Déplacement de la fenêtre Activer ( déplacer la fenêtre en faisant glisser la barre supérieure avec la souris)</td></tr>\
        <TR><TD><INPUT id=ptEnableFoodWarn type=checkbox /></td><TD>metre \'food left\' en rouge ci l autonomie de nouriture et de \
            <INPUT id=optFoodHours type=text size=3 value="'+ Options.foodWarnHours +'"> hours</td></tr>\
        <TR><TD colspan=2><P><B>Caractéristiques KOC:</b></td></tr>\
        <TR><TD><INPUT id=togMsgCountFix type=checkbox /></td><TD>afficher les mp separer des raport </td></tr>';
		if (ENABLE_ALERT_TO_CHAT){
	 m += '<TR><TD><INPUT id=alertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>publier un mesg automatiquement a l arvé d une attaque</td></tr>\
          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
          <TR><TD align=right>message a pblier sur le chat: &nbsp; </td><TD><INPUT id=alertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
          <TR><TD align=right>Alerte en cas d eclairage: &nbsp; </td><TD><INPUT id=alertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Alerte en cas d attaque d une TS: &nbsp; </td><TD><INPUT id=alertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Poster l aliance de l attaquand: &nbsp; </td><TD><INPUT id=alertAlliance type=checkbox '+ (Options.alertConfig.includealliance?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Poster la puissance de l attaquand: &nbsp; </td><TD><INPUT id=alertMight type=checkbox '+ (Options.alertConfig.includemight?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>alerte sonore: &nbsp; </td><TD><INPUT id=alertSound type=checkbox '+ (Options.alertConfig.sound?'CHECKED ':'') +'/></td></tr>\
          <TR><TD align=right>Minimum de troupe pour declencer le msg: &nbsp; </td><TD><INPUT id=alertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=alerterr></span></td></tr>\
          </table></td></tr>';
		  }
      m += '<TR><TD><INPUT id=togAllRpts type=checkbox /></td><TD>modifier les raport aliance.</td></tr>\
        <TR><TD><INPUT id=togAllowAlter type=checkbox /></td><TD>Autoriser d autres scripts pour modifier le format des rapports de l Alliance.</td></tr>\
        <TR><TD><INPUT id=togEnhanceMsging type=checkbox /></td><TD>afficher les bouton("forward" et "all officers" ) dans les msg .</td></tr>\
        <TR><TD><INPUT id=togPageNav type=checkbox /></td><TD>ameloirer la navigation dans les pages des mesg et raport. <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD><INPUT id=togWarnZero type=checkbox /></td><TD>etre alerter en ces de marche vers 0,0. <SPAN class=boldRed>&nbsp;(NEW)</span></td></tr>\
        <TR><TD colspan=2><BR><BR><B>correction des bug KOC :</b></td></tr>\
        <TR><TD><INPUT id=togTowerFix type=checkbox /></td><TD>Rapport de la tour pour voir la cible exacte (city, wild or invalid)</td></tr>\
        <TR><TD><INPUT id=togMapDistFix type=checkbox /></td><TD>Fixer la carte pour voir les distances de la ville actuellement sélectionné, au lieu de toujours la première ville.</td></tr>\
        <TR><TD><INPUT id=togTowerFix2 type=checkbox /></td><TD>fix les fausse attaque creer par un eclairage.</td></tr>\
        </table><BR><BR><HR>Notez que si une case à cocher est grisée il ya probablement eu un changement de KOC, ce qui rend l option inutilisable.';
      t.cont.innerHTML = m;
...
Tabs.Recherche = {
  ...
  init : function (div){
    ...
    this.cont = div;
    this.cont.innerHTML = '\
      <DIV class=ptentry><table><tr><td><TABLE><TR valign=bottom><TD class=xtab width=100 align=right>chercher pour: </td><TD>\
      <SELECT id="srcType">\
    ...
    m += '<TR><TD class=xtab></td><TD class=xtab><INPUT id=srcStart type=submit value="Comencer la recherche"/></td></tr>';
    m += '</table>';
...
clickedPlayerDetail : function (span, uid){
  ...
  clickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "Recherche d'info du leaderboard ...";
    t.fetchLeaderboard (uid, function (r) {t.gotPlayerLeaderboard(r, span)});
  },
  
  clickedPlayerCheckOnline : function (span, uid){
    var t = Tabs.AllianceList;
	var s = Tabs.Recherche;
    span.onclick = '';
    span.innerHTML = "recherche statut en ligne ...";
  ...
  clickedPlayerGetLastLogin : function (span, uid){
	...
	var s = Tabs.Recherche;
    span.onclick = '';
    span.innerHTML = "recherche da la date de connection...";
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ officerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Puissance: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      var c = p.cities[i];
      m += '<TR><TD align=right><B>City #'+ (i+1) +':</b></td><TD> &nbsp; '+ c.cityName +' '+coordLink (c.xCoord, c.yCoord)+'</td><TD width=75%> &nbsp; level: '
        + c.tileLevel +' &nbsp; &nbsp; status: '+ cityStatusString (c.cityStatus) +' &nbsp; &nbsp; crée: ' + c.dateCreated.substr(0,10) +'</td></tr>';
    }  
    span.innerHTML = m + '</table>';
  },
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    if (u.allianceName)
      a = u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=ptTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; ville: '
          + u.cities +' &nbsp; Population: '+ u.population +'</td></tr><TR><TD></td><TD>Provinces: ';
    var pids = u.provinceIds.split (',');
    var p = [];
    for (var i=0; i<pids.length; i++)
  ...
  gotPlayerStatus : function (rslt, span,uid){
    ...
    if (p[uid] == true) {
      m = '<span style="color:green"><b>en ligne!</b></span>';
       ...
       m = '<span style="color:red"><b>n est pas en ligne</b></span>';
    }  
    span.innerHTML = m + '';
  },
  ...
  eventSubmit : function (){
    ...
    t.aName = document.getElementById('allAllName').value;
    if (t.aName.length < 3){
      document.getElementById('ptallErr').innerHTML = 'Entrez au moins 3 caractères';
      return;
    }
    var myA = getMyAlliance ();
    document.getElementById('altInput').innerHTML = '';
...
//End edit city search
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X doit etre entre 0 et 749<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y doit etre entre 0 et 749<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>100)
      errMsg += "la distance max doit etre entre 1 et 100<BR>";
    if (errMsg != ''){
    ...
    if (t.opt.searchType == 0)
      typeName = 'Camps Babares';
    else if (t.opt.searchType == 1)
      typeName = 'TS';
	else 
	  typeName = 'ville';

    m = '<CENTER><B>Chercher pour  '+ typeName +'<BR>\
        Centre: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; Distance: '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>LISTE DES OPTIONS:</b><BR></td></tr>';
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>lvl minimum a afficher:</td><TD class=xtab> <INPUT id=filMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>lvl max a afficher:</td><TD class=xtab> <INPUT id=filMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
	}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>type de TS:</td><TD class=xtab><SELECT id=filWildType>';
      m += htmlOptions ( {1:'Prairie/Lakc', 3:'Bois', 4:'coline', 5:'montagne', 6:'Plaine', 0:'TOUT'}, Options.wildType );
      m += '</select></td></tr><TR><TD class=xtab align=right>sans aliance seulement:</td>\
	...
	if (t.opt.searchType == 1 || t.opt.searchType == 0) {
	  m += '<TR><TD class=xtab align=right>trier par :</td><TD class=xtab><SELECT id=filSortBy>\
            ...
            <TR><TD class=xtab align=right>afficher les coordoné uniquement:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
            </table></div><BR><SPAN id=srchSizeWarn></span>';
	} else {
	 m += '<TR><TD class=xtab align=right>type de ville:</td><TD class=xtab><SELECT id=filCities>';
     m += htmlOptions ( {1:'TOUT', 2:'Alliée', 3:'Amies', 4:'Hostile', 5:'Neutre', 6:'Sans Aliance', 7:'Sous Brume'}, Options.cityType );
     m += '</select></td></tr>';		
	 m += '<TR><TD class=xtab align=right>trier par</td><TD class=xtab><SELECT id=filSortBy>\
			<OPTION value="puissance" '+ (Options.srcSortBy=='puissance'?'SELECTED':'')  +'>puissance</option>\
			<OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>Distance</option>\
           </select></td></tr>\
		   <TR><TD class=xtab align=right>puissance minimum a afficher:</td><TD class=xtab><INPUT type=text id=minMight value='+Options.MightSrc+' size=3 \></td></tr>\
           <TR><TD class=xtab align=right>afficher les coordoné uniquement:</td><TD class=xtab><INPUT type=checkbox id=coordsOnly \></td></tr>\
           </table></div><BR><SPAN id=srchSizeWarn></span>';
	}
    ...
    document.getElementById ('statStatus').innerHTML = 'recherche a  '+ xxx +','+ yyy;
if (DEBUG_TRACE) logit (" 0 clickedSearch()... setTimeout ..:" + xxx +' , '+ yyy +' , '+ t.mapCallback.name);
  ...
  dispMapTable : function (){
    var tileNames = ['Camps barbares', 'Prairie', 'Lac', 'Bois', 'Coline', 'Montagne', 'Plaine', 'Ville' ];
    var t = Tabs.Recherche;
    var coordsOnly = document.getElementById('coordsOnly').checked;
if (DEBUG_TRACE) DebugTimer.start();
    function mySort(a, b){
      if (Options.srcSortBy == 'niveau'){
        if ((x = a[4] - b[4]) != 0)
          return x;
      }
	  if (Options.srcSortBy == 'puissance'){
        if ((x = b[10] - a[10]) != 0)
          return x;
      }
      return a[2] - b[2];
    ...
    for (i=0; i<t.mapDat.length; i++){
	  ...
	  if (t.opt.searchType == 2 && type == 7 ) {
			if (!(Options.cityType == 2) || t.mapDat[i][12] == 'mon aliance')
			if (!(Options.cityType == 3) || t.mapDat[i][12] == 'aliée') 
			if (!(Options.cityType == 4) || t.mapDat[i][12] == 'hostile')
			if (!(Options.cityType == 5) || t.mapDat[i][12] == 'neutre') 
			if (!(Options.cityType == 6) || t.mapDat[i][12] == 'sans aliance') 
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: FILTER');
    ...
    document.getElementById('statFound').innerHTML = 'Trouvé: '+ dat.length;
    if (dat.length == 0){
      m = '<BR><CENTER>aucun trouvé</center>';
    } else {
      dat.sort(mySort);
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: SORT');
      if (coordsOnly)
        m = '<TABLE align=center id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>coord</td></tr>';
      else {
        if (t.opt.searchType == 2) {
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>coord</td><TD >Dist</td><TD>City</td><TD>Owner</td><TD>Might</td><td>Alliance               </td><TD width=80% style="font-size:9px;">More info</td><TD style="padding-left: 10px;"></td></tr>';
		} else { 
			m = '<TABLE id=srcOutTab cellpadding=2 cellspacing=2><TR style="font-weight: bold"><TD>coord</td><TD style="padding-left: 10px">Distance</td><TD style="padding-left: 10px;">Lvl</td><TD width=80%> &nbsp; Type</td><TD style=""></td></tr>';
		}
	  }
          ...
          m += '</tr>';
			...
			if (t.opt.searchType == 2) { 
				 ...
				 if (dat[i][5]) {
					...
					<DIV style="" onclick="PCpo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">statut en ligne</a></div>\
					<DIV style="" onclick="PCplo2(this, '+ dat[i][7] +')"><A style="font-size:9px;">derniere connection</a></div>';
...
if (DEBUG_TRACE) DebugTimer.display('SEACHdraw: DRAW');
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('srcStart').value = 'demarer recherche';
...
if (DEBUG_TRACE) logit (" 3 mapCallback()");
    ...
    document.getElementById('statSearched').innerHTML = 'recherche: '+ t.tilesSearched;
    t.dispMapTable();
    ...
    document.getElementById ('statStatus').innerHTML = 'recherche a '+ x +','+ y;
...
Tabs.Entrainement = {
  ...
  init : function (div){
    var t = Tabs.Entrainement;
    t.cont = div;
    s = "<DIV id=trainTopSelect>\
      <DIV class=ptstat>Entrainer des troupes et des defences de rempars</div><DIV style='height:10px'></div><DIV class=ptentry>\
      ...
      <SELECT id=ptttType>\
        <option value='1'>unité de ravitaillement</option>\
        <option value='2'>Milicien</option>\
        <option value='3'>Eclaireur</option>\
        <option value='4'>Piquier</option>\
        <option value='5'>Paladin</option>\
        <option value='6'>Archer</option>\
        <option value='7'>Cavalier</option>\
        <option value='8'>Cavalier Lourd</option>\
        <option value='9'>wagon de ravitaillement</option>\
        <option value='10'>Baliste</option>\
        <option value='11'>Belier</option>\
        <option value='12'>Catapulte</option>\
      </select> &nbsp; (max <span id=ptttSpMax></span>)</td></tr>\
      <TR><TD align=right># par partie: </td><TD><INPUT id='ptttInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='ptttButMaxPS' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># de partie: </td><TD><INPUT id='ptttInpSlots' size=2 type='text' value='1'\></td>\
        <TD width=75%><INPUT id='ptttButMaxSlots' type=submit value='max'\> &nbsp; (max <span id=ptttSpMaxSlots>1</span>)</td></tr>\
      <TR><TD align=right valign=top>Ensemble des travailleurs au repos:</td><TD colspan=2><INPUT type=CHECKBOX id=chkPop"+ (Options.maxIdlePop?' CHECKED ':'') +"> \
        <SPAN style='white-space:normal;'>Permet de former davantage de troupes. Met temporairement la population en négatif.</span></td></tr>\
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='ptttButDo' type=submit value='Entrainer troupe'\></td></tr>\
      </table></td><TD width=20></td><TD style='border-left:solid 2px;' width=50% align=center>\
      <TABLE align=center><TR><TD align=right>type de defence : </td><TD colspan=2>\
      <SELECT id=pttdType>\
        <option value='53'>Arbalete</option>\
        <option value='55'>Trebuchet</option>\
        <option value='60'>Piège</option>\
        <option value='61'>chausse trape</option>\
        <option value='62'>Palisade a pointe</option>\
      </select> &nbsp; (<span id=pttdSpMax></span>)</td></tr>\
      <TR><TD align=right># par packet: </td><TD><INPUT id='pttdInpPS' size=5 type='text' value='0'\></td>\
        <TD><INPUT id='pttdButMaxPS' type=submit value='max'\> &nbsp; (max <span id=pttdSpMaxPS>0</span>)</td></tr>\
      <TR><TD align=right># de packet: </td><TD><INPUT id='pttdInpSlots' size=2 type='text' value='1'\></td>\
      ...
      <TR><TD colspan=3 align=center><DIV style='height:10px'></div><INPUT id='pttdButDo' type=submit value='Construire defence'\></td></tr></table>\
      </td></tr></table></div></div>\
      ...
      <TR><TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>liste d atente troupe &nbsp; (<SPAN id=statTTtot></span>)</b><BR><HR></div><DIV id=divSTleft style='overflow-y: auto; height:210px; max-height:210px'></div></td>\
        <TD width=50% style='padding-left:15px; padding-right:15px'><DIV style='text-align:center'><B>liste d atente defence; (<SPAN id=statDTtot></span>)</b><BR><HR></div><DIV id=divSTright style='overflow-y: auto; height:210px; max-height:210px'></div></td></tr>\
      </div>";
    t.cont.innerHTML = s;
...
/*******  TROOPS  ******/  
  ...
  clickTroopDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Nombre de troupes par logement doit être supérieure à 0.</font>';
      return;
    }
...
/*******  DEF  ******/  
  ...
  updateTopDef : function (){
    ...
    t.TDspSpace.innerHTML = 'niveau des rempars <B>'+ t.stats.wallLevel +'</b><BR>espace sur les rempars: '+ (t.stats.wallSpaceUsed+t.stats.wallSpaceQueued)  +'/<B>'+ t.stats.wallSpace +'</b><BR>\
        espace sur le terrain: '+ (t.stats.fieldSpaceUsed+t.stats.fieldSpaceQueued) +'/<B>'+ t.stats.fieldSpace +'</b>';
  },
  ...
  clickDefDo : function (){
    ...
    t.displayCityStats ();
    if (perSlot<1){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>Nombre d unités par logement doit être supérieure à 0.</font>';
      return;
    }
    ...
    if (numSlots<1 || numSlots > t.stats.wallLevel-t.stats.Dqueued){
      t.divTrainStatus.innerHTML = '<FONT COLOR=#550000>nombre de logement invalide.</font>';
      return;
    }
    t.setBusy(true);
    var que = [];
  ...
  doDefQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>liste d atente enregistre.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        ...
        doDefTrain ( cityId, cmd[1], cmd[2], 
          function(errMsg){
            setTimeout(function (){Tabs.Entrainement.doDefQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500);
          } );
      }
    } catch (err) {
  ...
  displayCityStats : function (){
    ...
    m += '<TABLE class=ptTab width=100%><TR align=center>\
        <TD width=18%><B>Nouriture:</b></td><TD width=16%><B>Bois:</b></td><TD width=16%><B>Pierre:</b></td>\
        <TD width=16%><B>Minerai:</b></td><TD width=16%><B>Or:</b></td><TD width=16%><B>Population:</b></td></tr>\
...
// defense queue ....
  ...
  doQueue : function (cityId, que, errMsg){
    ...
    try {
      ...
      var cmd = que.shift();
      if (!cmd){
        t.dispTrainStatus ('<B>entrainemant enregistrer.</b><BR>');
        t.setBusy(false);
        return;
      }
      if (cmd[0] == 'T'){
        ...
        doTrain (cityId, cmd[1], cmd[2], 
          function(errMsg){
           setTimeout(function (){Tabs.Entrainement.doQueue(cityId, que, errMsg);}, (Math.random()*3500)+1500 );
          }
        );
      }
    } catch (err) {
...
//DebugTimer.start(); 
    try {
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;"><TABLE cellspacing=0 cellpadding=0 class=ptTab width=97% align=center>\
        <TR align=left><TD><SPAN class=ptStatLight>Date :</span> '+ dt.toLocaleDateString() +'</td>\
        ...
        <TD align=right><SPAN class=ptStatLight>Domaine:</span> ' + unsafeWindow.domainName +'</td></tr></table></div>';
      ...
      str += _row ('Or', rows[0]);
      str += _row ('Nouriture', rows[1]);
      str += _row ('Bois', rows[2]);
      str += _row ('Pierre', rows[3]);
      str += _row ('Minerai', rows[4]);
      str += '<TR><TD><BR></td></tr>';
      for (r=1; r<13; r++){
        rows[r] = [];
        for(i=0; i<Cities.numCities; i++) {
      ...
      rownum = 0;
      str += _row ('unité de transport', rows[1]);
      str += _row ('milicien', rows[2]);
      str += _row ('éclaireur', rows[3]);
      str += _row ('Piquier', rows[4]);
      str += _row ('Paladin', rows[5]);
      str += _row ('Archer', rows[6]);
      str += _row ('Cavalier', rows[7]);
      str += _row ('Cava. lourd', rows[8]);
      str += _row ('Wagon', rows[9]);
      str += _row ('Baliste', rows[10]);
      str += _row ('Belier', rows[11]);
      ...
      str += _row ('autonomie nouriture', row, true);
      str += '<TR><TD><BR></td></tr>';
      ...
      str += _row ('nbre de chevalier', row, true);
      ...
      str += _row ('liste d atente troupe', row, true);
      ...
      str += _row ('liste d atende defence', row, true);
      str += '<TR><TD class=xtab></td><TD class=xtab colspan=4><BR><INPUT type=CHECKBOX id=idCheck'+ (Options.includeMarching?' CHECKED':'') +'>Inclure les troupes/resources en deplacement</td></tr>';
      str += "</table>";
      if (DEBUG_BUTTON)
...
Tabs.Renfort = {
  ...
  hide : function (){
    clearTimeout (Tabs.Renfort.displayTimer);
  },

  show : function (cont){
    var rownum = 0;
    var names = ['ravitailleur', 'Millicien', 'eclaireu', 'Piquier', 'Paladin', 'Archer', 'Cavalier', 'Cav. lourd', 'Wagon', 'Balliste', 'Belier', 'Catapulte'];
    var t = Tabs.Renfort;
    ...
    s = '<div class=ptstat>Affichage de toutes les troupes campées à chacune de vos ambassades.</div><BR>';
    if (numSlots == 0){
      s += '<BR><CENTER><B>Aucune Troupe Ne Campe.</b></center>';
    } else {
      s += '<STYLE> .tot{background:#f0e0f8;} .city{background:#ffffaa;}</style>';
      s += '<TABLE cellspacing=0 width=100%><TR align=right><TD align=left width=16%><B>joueurs</b></td>';
    ...
    s += '<BR><BR><INPUT type=CHECKBOX id=idCheck2 '+ (Options.encRemaining?'':' CHECKED ') +'> afficher les troupe original';
    s += '<BR><BR><DIV style="font-size: 10px">NOTE: Vous aurez besoin d actualiser Koc pour voir de nouveaux campements ou des troupes restantes après une bataille.</div>';
    t.cont.innerHTML = s;
    checkBox = document.getElementById('idCheck2');
