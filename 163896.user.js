...
// ==/UserScript==
var openingMessage = GM_getValue("openingMessage", "Hi stranger!"); //default opening message
...
if (document.title) { //checking if we're on the main page. 
    ...
    function checkForStatus() {
        ...
        if (thisScriptEnabled()) { //if autoconnect has been enabled
            var divNewChat = document.getElementsByClassName("newchatbtnwrapper");
            var btnNewButton = document.getElementsByClassName("disconnectbtn");

            for (var i = 0; i < divNewChat.length; i++) {
		...
		// Only "few" messages, and none of them are my own: write opening message
        if (document.getElementsByClassName("logitem").length > 0 && document.getElementsByClassName("logitem").length <= 3) {        	
        	if (document.getElementsByClassName("youmsg").length == 0) {
        		if (openingMessageEnabled()) { //if opening message has been enabled
        			if (document.getElementsByClassName("logitem")[0].textContent.search(". Say hi!")) {
    ...
    function addMenu() {
        ...
        divReconnect.innerHTML = "New Chat (F2)";
        ...
        divAutoConnect.innerHTML = "<input id=oac-autoconnect type=checkbox checked><label for=oac-autoconnect>Auto-Connect (F4)</label>";
    ...
    $('#oac-autoconnect').click(function () { //if autoconnect is enabled, F2 because a key for a new chat. if not, F2 just exits a chat
        if (this.checked) {
            $('#newStranger').html("New Chat (F2)"); //with autoconnect
        } else {
            $('#newStranger').text("End Chat (F2)"); //without autoconnect
        }
    });
