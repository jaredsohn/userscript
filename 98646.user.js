...
// @require     http://userscripts.org/scripts/source/98645.user.js
// @require     http://userscripts.org/scripts/source/98729.user.js
// @require     http://userscripts.org/scripts/source/98728.user.js
// @require     http://userscripts.org/scripts/source/98726.user.js
// @version		0.99a

// @history		0.99a Added Ikariam: Handling by Phate
// @history		0.99 Added dump
...
ScriptUpdater.check(98646, "0.99a");
...
Config.settings = {
	"General":{
		fields:{
			externalArmyHelper:{
				type:'checkbox',
				label:'<a href="http://" target="_blank" title="v0.23 by GrAndAG" style="font-weight:bold !important;">Army Helper</a>',
	...
	"Events":{
		...
		fields:{
			...
			eventsEmailHtml:{
				type:'html',
				value:'  <p style="border-top:1px solid #ccc; padding-top:1em !important;"><strong>Email Settings</strong> - see "Email Settings" on <a href="http://" target="_blank">this page</a> for more information.</p>',
			},
			emailNoticeUrl:{
				type:'text',
				label:'Server',
	...
	"About":{
		html:'	<p><a href="http://userscripts.org/scripts/show/' + ScriptUpdater.scriptId + '" target="_blank" style="font-weight:bold !important;">Ikariam ExMachina v' + ScriptUpdater.scriptCurrentVersion +
				'</a> by <a href="http://userscripts.org/users/Pitmm" target="_blank">PhasmaExMachina</a>' +
...
/*
			...
			label.innerHTML = '<a href="http:" style="font-weight:bold;">Library Tools v' + ScriptUpdater.scriptCurrentVersion + '</a> by <a href="http://userscripts.org/users/Pitmm">Pitmm</a> - [ <a href="javascript:void(0)" id="libraryImprovementsUpdates>check for updates</a> ]';
...
IAB = {
	...
	views:{
		...
		options:function() {	
			...
			content.innerHTML = '<a name="autoBuild"></a><h3><a href="http://" target="_blank">Ikariam Auto Build' + 
								' </a> <span style="font-weight:normal;"> ' + IAB.getText('options.by') + ' <a href="http://" target="_blank">PhasmaExMachina</a></span></h3>' + 
	...
	text:{
		...
		el:{	// Greek
			addToQueue:'ΠρϦσθεσε στην ουρά',
			autoBuild:'ΑυτϦματο χτίσαμο', // name of script
			demolishConfirm:'Προσοχή! ΠρϦκεαταα να κατεδαφίσεας μΛμΚΛΗΡΩΤΙΚΑ αυτϦ το κτίραο. Είσαα σίγουρος;',
			options:{
				by:'by', // used in "Auto Build by PhasmaExMachina"
				descriptions:{
				smartChecking:'Χρησαμοποίσε τον έξυπνο έλεγχο',
				autoDisable:'Απενεργοποίησε το script αν είναα ανενεργϦ γαα πολύ ώρα',
				highlightRows:'highlight alternate rows in Auto Build info box',
				},
				help:{
					smartChecking:'μ έξυπνος έλεγχος θα τσεκάρεα αυτϦματα κάθε 30 δευτερϦλεπτα αν κάτα στην ουρά ή στην λίστα αυτϦματου χτασίματος μπορεί να χταστεί. ΑυτϦ γίνεταα χωρίς επακοανωνία με τους server του πααχναδαού.',
					autoDisable:'Αυτή η επαλογή θα απενεργοποαεί το αυτϦματο χτίσαμο στην περίπτωση που το script δεν έχεα τρέξεα γαα αρκετϦ χρϦνο, πράγμα το οποίο μπορεί να είναα χρήσαμο αν παίζεας σε πολλούς δααφορετακούς υπολογαστές. Γαα παράδεαγμα αν παίζεας σε έναν υπολογαστή μέρα καα μετά πας σε έναν άλλο η σεαρά που είχες αφήσεα στον πρώτο μπορεί να μη σε εξυπηρετεί παα, καα να μη θέλεας να συνεχίσεα να ασχύεα.',
				},
				hour:'άρα',
				hours:'άρες',
				labels:{
					show:'Εμφάνασε',
					reCheckEvery:'Επανέλεγξε γαα αυτϦματο χτίσαμο κάθε',
					smartChecking:'Έξυπνος έλεγχος',
					autoDisable:'ΑυτϦματη απενεργοποίηση',
					highlightRows:'Highlight Rows',
				},
				minute:'ΛεπτϦ',
				minutes:'Λεπτά',
				show:{
					allCities:'Φλα τα κτίραα σε Ϧλες τας πϦλεας',
					currentCity:'ζϦνο τα κτίραα σε αυτή την πϦλη',
				},
				updatesAndHistory:'Script Updates & History',
				v:'v', // used to label version number (e.g. "v0.36")
			},
			queue:{
				autoUpgrade:'ΑυτϦματη αναβάθμαση',
				change:'αλλαγή',
				clearAll:'δααγραφή Ϧλων',
				clearAllDesc:'Αφαίρεσε Ϧλα τα χτασίματα απϦ την αυτϦματη λίστα χτασίματος',
				downInQueue:'up in queue', // used in "ζετακίνησε [building name] μαα θέση κάτω στη λίστα"
				emptyNotice:'Η λίστα αυτϦματου χτασίματος είναα άδεαα.',
				enableAutoBuild:'Ενεργοποίησε το αυτϦματο χτίσαμο',
				enableDisableAutoUpgrades:'Ενεργοποίησε / Απενεργοποίησε το αυτϦματο χτίσαμο γαα Ϧλες τας πϦλεας',
				expandOrCollapse:{
					autoBuild:'ζεγαστοποίησε ή ελαχαστοποίσε τη λίστα αυτϦματου χτασίματος',
					autoUpgrades:'ζεγαστοποίησε ή ελαχαστοποίσε τας αυτϦματες αναβαθμίσεας',
				},
				fromAutoUpgrades:'απϦ τας αυτϦματες αναβαθμίσεας', // used in "Αφαίρεσε [building name] απϦ τας αυτϦματες αναβαθμίσεας"
				fromQueue:'απϦ τη λίστα', // used in "Αφαίρεσε [building name] απϦ την λίστα"
				move:'ζετακίνησε', // used in "ζετακίνησε [building name] πάνω/κάτω στη λίστα"
				queue:'Βάλε στη λίστα',
				showing:{
					allCities:'Φλες οα πϦλεας',
					currentCity:'Τρέχουσα πϦλη',
				},
				upInQueue:'Πάνω στη λίστα', // used in "ζετακίνησε [building name] μαα θέση πάνω στη λίστα"
				remove:'Αφαίρεσε', // used in "Αφαίρεσε [building name] απϦ ..."
				timeUntilNextDesc:'ΡπολοαπϦμενος χρϦνος μέχρα τον επϦμενο έλεγχο γαα αυτϦματο χτίσαμο (Κάνε κλακ γαα να επανελέγξεας τώρα)',
			},			
		},
	},
}
IkaNotify = {
	...
	views:{
		options:function() {
			...
			var html = '<a name="ikaNotify"></a><h3><a href="http:" target="_blank" title="More information about this script">Ikariam Notifier</a> \
							<span style="font-weight:normal">by <a href="http://" target="_blank">PhasmaExMachina</a></span></h3>\
...
ExternalScripts.ArmyHelper = function () {
	const languages = {
		...
		"arabic": {
			...
			cook: "طباخ",
			medic: "طبيب",
			ship_ram: "سفينة مزودة بقوة دفر",
			ship_flamethrower: "قاذف اللهب",
			ship_steamboat: "قوة دفر الطارة",
		...
		"greek": {
			garrison: "Φρουρά",
			advisor: "Σύμβουλος",
			cityLevel: "Επίπεδο ΠϦλης",
			support: "ζονάδες Ρποστήραξης",
			artillery: "ζονάδες Πυροβολακού",
			air: "Εναέραες ζονάδες",
			line1: "Κύραα Γραμμή ζάχης",
			line2: "Γραμμή ζάχης ζεγάλης Εμβέλεαας",
			flankLeft: "Αραστερή Πλευρά",
			flankRight: "Δεξαά Πλευρά",
			submarine: "Ρποβρύχαες ζονάδες",
			reserve: "ΑπϦθεμα",
			phalanx: "μπλίτης",
			steamgiant: "Γίγαντας Ατμού",
			spearman: "Εκτοξευτής ΔϦρατος",
			swordsman: "Ξαφομάχος",
			slinger: "Εκτοξευτής",
			archer: "ΤοξϦτης",
			...
			catapult: "Καταπέλτης",
			mortar: "Κονίαμα",
			gyrocopter: "ΓυροκϦπτερο",
			bombardier: "ΒομβαρδαστακϦ",
			cook: "ζάγεαρας",
			medic: "ΓαατρϦς",
			ship_ram: "Σκάφος(-η) Έμβολο",
			ship_flamethrower: "ΦλογοβϦλο(-α)",
			ship_steamboat: "Σκάφος(-η) Κουπί-ΡϦδα-ΚραϦς",
			ship_ballista: "Σκάφος(-η) Βαλλαστών",
			ship_catapult: "Σκάφος(-η) Καταπελτών",
			ship_mortar: "Σκάφος(-η) Κοναάματος",
		...
		"russian": {
			garrison: "Гарнизон",
			advisor: "момощник",
			cityLevel: "УровенѦ города",
			support: "моддержка",
			artillery: "АртиллериѬ",
			air: "АвиациѬ",
			line1: "ЛиниѬ фронта",
			line2: "Стрелковые подразделениѬ",
			flankLeft: "Левыб фланг",
			flankRight: "мравыб фланг",
			submarine: "модводные лодки",
			reserve: "Резерв",
			phalanx: "Гоплит",
			steamgiant: "маровоб гигант",
			spearman: "Копебщик",
			swordsman: "жечник",
			slinger: "мращник",
			archer: "Лучник",
			...
			cook: "мовар",
			medic: "Доктор",
			ship_ram: "КораблѦ с тараном",
			ship_flamethrower: "Огнемётныб кораблѦ",
			ship_steamboat: "мароход с тараном",
			ship_ballista: "КораблѦ с баллистоб",
			ship_catapult: "КораблѦ с катапулѦтоб",
			ship_mortar: "КораблѦ с мортироб",
			ship_submarine: "модводнаѬ лодка",
			addSlot: "ДобавитѦ вобска в одну клетку",
			removeSlot: "УбратѦ вобска из одноб клетки",
			fillRow: "ЗаполнитѦ вобсками один рѬд",
			selectUnits: "ВыбратѦ вобска",
			assignNone: "Ничего",
			assignAll: "Все",
			assignField: "ЗаполнитѦ поле боѬ",
			presetsTitle: "Закладки",
			presetsAdd: "ДобавитѦ новую закладку",
			presetsRemove: "УдалитѦ закладку",
			presetsNewName: "Укажите имѬ новоб закладки длѬ сохранениѬ",
			optShowTitles: "ОтображатѦ названиѬ линиб фронта"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		//  "serbian":      { },
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		"turkish": {
			...
			air: "Hava",
			line1: "Ön Cephe",
			line2: "Uzun Menzilli Savaż Hattı",
			flankLeft: "Sol Kanat",
			flankRight: "Saļ Kanat",
			submarine: "Submarine",
			reserve: "Rezerv",
			phalanx: "Phalanx",
	...
	function drawAttackHelper(type, draw){
		...
		if (draw) {
			...
			html += '<div style="margin: -18px 10px 8px 5px; text-align: right;">By <a target="_blank" href="http://">Ikariam Army Helper</a>.</div>\n';
...
{
	var html ='';
	$('select#citySelect option').each(function(num)
	{
		var nick = nameCity($(this).text(),true);
		var idPly = $(this).attr('value');
		html += $(this).attr('selected')?	"<tr><td class='hand_namecity' title='"+ nick[1] +"'>"+ nick[0] +"</td><td colspan='3' id='handlingtime'></td></tr>":
				"<tr>"+
					"<td class='hand_namecity' title='"+ nick[1] +"'><a class='hand_namecity' value='"+ num +"' href='#'>"+ nick[0] +"</a></td>"+
...
function getOpt(reset)	// get option parameter
{
	...
	if (!Hand_opt || reset) 	// Init parameter
	{
		Hand_opt = {};
...
try
{
	...
	{
		window.open("http://userscripts.org/scripts/show/98646");
	}
}
catch(er1) 				
	{alert(er1);}
