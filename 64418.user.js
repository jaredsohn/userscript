...
(function() {
	...
	var whatcd_url_base = document.URL.match(/^(https:\/\/ssl\.sharetheremote\.org|http:\/\/sharetheremote\.org)/)[1];
	...
	/* Create status box */
	var status = StatusBox('ShareTheRemote Snatched');
	...
	/* Reset command */
	GM_registerMenuCommand('ShareTheRemote Snatched: Reset', function() { snatch_cache.clear(); GM_setValue('last_update', '0'); location.reload(); });

	/* Register menu command to enter custom style */
	var custom_style = GM_getValue('custom_style', DEFAULT_STYLE);
	GM_registerMenuCommand('ShareTheRemote Snatched: Enter custom style...', function() {
	...
	function mark_snatched_links() {
		...
		if (/sharetheremote\.org\/torrents\.php/.test(document.URL)) {
			/* Parse search */
			var search = {};
