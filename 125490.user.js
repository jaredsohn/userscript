// ==UserScript==
// @name           CarSales.com.au Enlarge thumbnail pictures in used cars search results
// @namespace      Seifer - http://userscripts.org/users/33118
// @include        http://www.carsales.com.au/*
// ==/UserScript==
	...
	scriptName='CarSales.com.au Enlarge thumbnail pictures in used cars search results';
	scriptId='125490';
	scriptVersion=1;
...
if(images) {
	for(var i = 0; i < images.length; i++){
		children = images[i].children;
		for(var ii = 0; ii < children.length; ii++){
			img = children[ii].firstElementChild.firstElementChild;
			img.setAttribute('class','hoverEnlarge');
			imgsrc = img.getAttribute('src');
			imgsrcf = imgsrc.replace("?size=Thumb","");
			img.setAttribute('data-full',imgsrcf);
			img.setAttribute('data-thumb',imgsrc);
			img.addEventListener('mouseover',function(event){ 
	this.setAttribute('src',this.getAttribute('data-full'));
	this.setAttribute('style','position:absolute;width:auto !important;z-index:9999');
	 },false);
			img.addEventListener('mouseout',function(event){ 
	this.setAttribute('src',this.getAttribute('data-thumb'));
