...
unsafeWindow._getFrontiervilleGift = safeWrap(_getFrontiervilleGift);
...
_appSettings = {
	...
	'app_201278444497_0':{
		'name':'Frontierville',
		'functionName': 'unsafeWindow._getFrontiervilleGift(\'app_201278444497_0\')'
...
function _getFrontiervilleGift(_str){ 
	var all		=	count(_str);
	var temp	=	0;
	if ($("#giftFrontiervilleMain").html() == null) $("#imgList").append("<div id='giftFrontiervilleMain'>Accepting Frontierville request...<div id='giftFrontiervilleBody' style='padding-left:10px'></div>Accepted <span id='giftFrontiervilleSum'>0</span> gift and/or <span id='neibFrontiervilleSum'>0</span> neighbor.</div>");
	for (var i in _request){
		if(_request[i].appName == _str) {
			var url_array = getArgs(_request[i].link.replace(/actions\[/,'').replace(/]/,''));
			var _link = _request[i].link.replace(/actions\[/,'').replace(/]/,'');
			...
			if(_isGift == 1){
				GM_xmlhttpRequest({ url: _link, method:'get', data: i, 
					onload: function(resp){
						temp++;
						if($('.giftConfirm_name > span',resp.responseText).html()){ 
							$("#giftFrontiervilleBody").append("<div> Accept: <b>" + $('.giftConfirm_name > span',resp.responseText).html() + "</b></div>");
							$("#giftFrontiervilleSum").html(parseInt($("#giftFrontiervilleSum").html(),10)+1);
							_accept(this.data);
						}
						if(temp == all){
							$('#'+_str+'_loading').remove();
