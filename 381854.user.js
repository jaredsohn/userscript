...
top == self){
	...
	div2.append(
		$('<h3 class="titleJanbee">').text('PTC Clicker')		
	)	
	...
	setTimeout(function(){
		...
		$.each(clickSrc,function(k,v){
			...
			if(thisItem.attr('href')){
				if( ( thisItem.attr('href').indexOf('fxgpt.php?v=entry&type=fxptc') >= 0 || thisItem.attr('href').indexOf('fxgpt.php?v=read') >= 0 ) && thisItem.text().indexOf('Cheat Check') == -1){
					var obj = {
		...
		if(!arr.length){
			$.each($('a,div'),function(k,v){
				...
				if(thisItem.attr('href')){
					...
					if( (thisItem.attr('onclick').toString().indexOf('entry&type=fxptc') >= 0  ) && thisItem.text().indexOf('Cheat Check') == -1){
						var obj = {
							href 	: 'fxgpt.php?v='+thisItem.attr('onclick').toString().split(',')[1].match(/'[^]+'/)[0].replace(/'/g,''),
							jObj	: thisItem
						}
						arr.push(obj); 
					}
...
if(wparent.location.href.indexOf('fxgpt.php?v=read') != -1 && self == top){
	var div = $('<div>');
...
if(wparent.location.href.indexOf('fxgpt.php?v=entry') != -1 && 
self == top){
	...
	if($('frame:nth(1)').attr('src').indexOf('cheat') >= 0 ){
		...
		if(wparent.location.host.indexOf('x-click')!=-1)cheatCheck = [64,85,125,126,123,56,55];
		if(wparent.location.host.indexOf('clickbux')!=-1)cheatCheck = [45,60,71,75];
		if(wparent.location.host.indexOf('ceoclix')!=-1)cheatCheck = [38,25];
		if(wparent.location.host.indexOf('idw-silverclix')!=-1)cheatCheck = [27,34,14,3,57,20,15,64,32,50,8,40];
		if(wparent.location.host.indexOf('juicybux')!=-1)cheatCheck = [30,2,25];
		if(wparent.location.host.indexOf('happyclix')!=-1)cheatCheck = [1];
		if(wparent.location.host.indexOf('cashtravel')!=-1)cheatCheck = [22];
		if(wparent.location.host.indexOf('votre-part-degains-ptc')!=-1)cheatCheck = [8,1,7];
		if(wparent.location.host.indexOf('arrowptc')!=-1)cheatCheck = [10,7];
		if(wparent.location.host.indexOf('pybux')!=-1)cheatCheck = [];
		if(wparent.location.host.indexOf('luckyptc')!=-1)cheatCheck = [6,3];
		if(wparent.location.host.indexOf('moderob')!=-1)cheatCheck = [6];
		...
		var inters = setInterval(function(){
			...
			if($('#timer',doc).text().indexOf('Click') >= 0 || $('#timer img:visible',doc).size() || fulltimer==-1){
				...
				function ajaxRec(btnKey){
					clearInterval(inters);
					$.ajax({
						url : 'fxgpt.php?v=verify&buttonClicked='+btnKey+'&id='+win.id+'&type='+win.type+'&pretime='+win.pretime+'&'+win.url_variables
