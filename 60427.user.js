...
var sLang = "es";
var sGetParameters = "";
var sCurrentServer = "s2.travian.net";    // Set this to the server's url to override automatic server detection
                            // (i.e. s1.travian.net)
...
switch(sLang) {
	case "sk":
		aLangStrings = ["Vlastné jednotky", "Návrat z", "Podpora pre", "Jednotky od", "Útok na", "Lúpež proti", "Špehovanie v", "Preskúmať opustené údolie", "Prebehol pokus o aktualizáciu.", "Aktualizácia úspešná", "Aktualizácia zlyhala!", "Nastaviť Troop Tool Updater", "Aktualizovať Troop Tool", "Troop tool Updater:", "Skopíruj sem autentifikačný kľúč zo svojho profilu v Troop Tool-e"];
		break;
	case "en":
	case "com":
	case "uk":
	case "es":
	default:
		aLangStrings = ["Tropas propias", "Regreso de", "Refuerzo para", "Tropas de", "Ataque", "Atraco", "Ataque batidores", "Preskúmať opustené údolie", "Se desconoce el resultado de la actualización.", "Tus tropas se han actualizado correctamente.", "¡Actualización fallida!", "Clave de actualización", "Actualizar tropas", "Actualizador de tropas:", "Introduce la clave de actualización de tu perfil de www.tropasrisk.tk"];
		break;
}

// Do not change the array below!
var aLangStringsMaster = ["user_own_troops", "user_return_from", "user_troops_for", "user_troops_of", "user_attack_to", "user_raid_to", "user_spy_at", "user_explore_oasis", "Update attempted with unknown result.", "Your troops were successfully updated.", "Update failed!", "Troop Tool Updater setup", "Update Troop Tool", "Troop tool Updater:", "Enter the authentication key from your Troop tool profile"];
...
function createLink() {
	...
	//var xpathRes = xpath("id('lmid2')//a[@href='warsim.php']");  // Travian < 3.5
	var xpathRes = xpath("id('textmenu')//a[@href='warsim.php']");
...
function parse() {
	...
	//var tables = xpath("id('lmid2')/table"); // Travian < 3.5
	var tables = xpath("//table[@class='troop_details']");
	...
	for(var i=0; i < tables.snapshotLength; i++) {
		...
		if(pos > -1) {
			for(var thisMatch in matches) {
				...
				aTroops[pos + 1 + thisMatch] = parseInt(aTroops[pos + 1 + thisMatch]) + parseInt(thisNum[0]);
			}
