// ==UserScript==
// @name           Combat Report Auto-Converter Movil
// @autor          Reaper modificado por Alex Corsario
// @icon           http://img196.imageshack.us/img196/8994/50187963.jpg
// @description    Script para Ikariam. Editado del Script de Alex Corsario para -NO-.
...
var url_split3 = url_now.split('http://m');
...
if(today2>last_check) {
	check_version('http://bit.ly/cracversion'); 
}
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) { 
			...
			if(testtext.test(xhr.responseText)) { 
				...
				if(VERSION!=version4) { 
					if(confirm('Su script Combat Report Auto-Converter Argentino está obsoleto. ¿Desea actualizar a la última versión?')) {  
						openInNewTab('http://userscripts.org/scripts/show/106532','Actualizar Script'); 
					}
				} else { 
					updateVersionCheck();
				}
...
var adress = new Array('0','38','44','86','136','161','180','189','121','134','152','195','216','234','260','327','342');
...
function BuildStandardReport() {
	...
	var full_whoattack = CRAC_REPORT.info.attackers;
	var full_whodeffend = CRAC_REPORT.info.defenders;
	...
	for(i=0;i<full_whodeffend2.length;i++) {
		...
		if(full_whodeffend3.length>1) {
			...
			if(ffnew==1) {
				defenders[i] = full_whodeffend4.trim();
			} else {
				full_whodeffend4 = full_whodeffend4.replace(/^\s*|\s*$/g,"");
				defenders[i] = full_whodeffend4;
			}
		}
	}
	attackers_string = '';
	...
	results += '<br/>';
	results += '<br/>[b][i]Compactado con: [url=http://board.ar.ikariam.com/board113-discusi%C3%B3n-global/board120-scripts-y-add-ons/53587-combat-report-auto-converter-argentino/]Combat Report Auto-Converter Argentino[/url] v0.4.5b [url=http://board.es.ikariam.com/index.php?page=Thread&threadID=129508]©[/url] Editado por [url=http://board.ar.ikariam.com/user/6694-/]Reaper[/url][/i][/b]';
...
function Toplain(text) {
		...
		text = text.replace('[img]http://'+server+'/skin/resources/icon_sulfur.gif[/img]','Azufre:');
		text = text.replace('[img]http://'+server+'/skin/resources/icon_citizen.gif[/img]','Burgueses:');
		text = text.replace('[b][i]Compactado con: [url=http://board.ar.ikariam.com/board113-discusi%C3%B3n-global/board120-scripts-y-add-ons/53587-combat-report-auto-converter-argentino/]Combat Report Auto-Converter Argentino[/url] v0.4.5b [url=http://board.es.ikariam.com/index.php?page=Thread&threadID=129508]©[/url] Editado por [url=http://board.ar.ikariam.com/user/6694-/]Reaper[/url][/i][/b]','Compactado con: Combat Report Auto-Converter Argentino v0.4.5b © Editado por Reaper');
...
function BuildReport() {
	...
	var new_mainview =  '<div class="footer"></div></div><div class="contentBox01h"><h3 class="header"><a href="http://board.ar.ikariam.com/board113-discusi%C3%B3n-global/board120-scripts-y-add-ons/53587-combat-report-auto-converter-argentino/" target="_blank">Combat Report Auto-Converter Argentino</a> v0.4.5b <a href="http://board.es.ikariam.com/index.php?page=Thread&threadID=129508" target="_blank">©</a> Editado por <a href="http://board.ar.ikariam.com/user/6694-/" target="_blank">Reaper</a></b></h3> <div class="content"><table style="border:none;"><tr><td style="border:none; height:8px;"></td></tr><tr><td style="vertical-align:middle; text-align:center; border:none;">Hacé click en Seleccionar Título o Seleccionar Batalla, pulsa CTRL+C y luego pegala en el foro pulsando CTRL+V';
