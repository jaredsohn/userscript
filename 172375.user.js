	...
	var xhr = $.ajaxSetup({
		complete: function(a,b,c,d){
			...
			function rec(ctr){
				loading = 0;
				if(arr[ctr]){
					winder = wparent.open(arr[ctr].href,"","width=600,height=200,top=1,left=1");	
					...
					setTimeout(function(){window.open("about:blank","","width=100,height=100,top=1,left=1") ;},12000000);
				}	
			}
			rec(ctr);
