...
//}
//{ Parsing des paramètres situé après le "?" dans l'url, pour info.args
(function(){
...
//}
//{ Classe gérant le stockage de données
function Stockage(namespace) {
	this.storageKeyName = 'RaidFacile ' + info.univers + ' ' + info.playerId + ' ' + namespace;
	// Le mapping sert à ce que ça prenne moins de place dans le stockage, mais qu'on ait toujours un texte compréhensible dans le code
	// Il sert aussi à définir la valeur par défaut
	this.mapping = {
...
var stockageData = new Stockage('données');
stockageOption.load();
stockageData.load();
//}
...
Intercom.prototype = {
	...
	*/
	...
	received: function(event){
		...
		case 'loaded':
			// l'autre intercom est chargé on peut traiter les messages en attente
			this.loaded = true;
...
/************ Variables de langues ************/{
	...
	if((location.href.indexOf('fr.ogame',0)) >=0) {
		localization = {
			missions: {
				'1': 'Attaquer',
				'2': 'Attaque groupée',
...
/************************* PAGE DE TABLEAU *************************/{
	...
	function showAttaque(data) {
		...
		var missions = {};
		var html = $($.trim(data));
		// On va chercher tous les événements (allers + retours)
		...
		for (var destination in missions) {
			...
			if (missions[destination][9]) { // Détruire
				couleur = missions[destination][9][0] ? col_dest : col_dest_r;
			} else if (missions[destination][2]) { // Attaque groupée
...
/** partie mise à jour **///{
	function mise_a_jour(version) {
		...
		var popup = $('<div title="'+info.nom+' - Mise à jour">'
	...
	function checkUpdate() {
		...
		if((info.startTime - parseInt(date_mise_ajours)) > 1000*60*60*6) { // vérification toutes les 6h
...
if (document.getElementById("eventboxFilled") && info.args.page !== 'galaxy') {
	window.setTimeout(function() {
		// récupération de l'url de la page flotte
		var url_1 = url_1 = document.getElementById("menuTable").getElementsByClassName('menubutton')[7].href;
		// test de la présence d'une flotte à soi -> dans ce cas récupèration de l'url de la page mouvement
		...
		// test si le lien n'est pas là avant de le rajouter
...
/////////////////// TABLEAU ///////////////////
	...
	for (var i = 0; i < xpathResult.snapshotLength; i++) {
		//apparement ne sait pas accéder à la page movement avec &raidefacil=scriptOptions ni d'autre attribut
...
// Modification Deberron
						...
						//recupere vaisseau et defense
							...
							var acronyme_def = 'Défense<div class="splitLine"></div>';
							var nb_totaldef = 0;
...
/* 							var indexof_inactif = inactif_normal.indexOf(nom_joueur);
	...
	/*************** Affichage des MaJ et news ***************///{
		if (!info.chrome || info.tampermonkey) {
			// Sous chrome on installe le script comme une extension, donc les mises à jour sont automatiques
			checkUpdate();
		}
		/*news désactivées
