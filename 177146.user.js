// ==UserScript==
// @name           Kaskus Replay
// @namespace      kaskus.Replay.Verandaddict
// @version        1.3
// @description    Quick reply for kaskus forum
// @icon           http://3.bp.blogspot.com/-c3kYtrAVUGw/T9VpOjBZfdI/AAAAAAAADPE/h6ccxgvAGQw/s1600/kaskus-logo-yellow.jpg
// @homepage       http://kask.us/g91WZ
...
// @author         NakalAngga (Kaskus UID:2757777)
// @co-author      Adamantoi
// ==/UserScript==
...
//
// For information cek my twitter @kaskuserAngga
...
$(document).ready(function(){
    ...
    qr.innerHTML=''+
        ...
        '<img id="gbrCapcay" src="http://img812.imageshack.us/img812/5676/jessicaveranda.jpg"><div class="clear_both"></div>'+
        '</div>'+
        ...
        '<li>Klik Jessica Veranda JKT48 jika capcay tidak muncul</li><li>Save siggy dahulu sebelum melakukan preview</li><li>Buka semua spoiler hotkey CTRL+G/SHIFT+X</li>'+
    ...
    $settingContainer=''+
        ...
        '<h1>Thanks To:</h1>'+
        '<ul><li><b> </b> Muhammad Angga a.k.a NakalAngga </li><li><b> </b> Mimin Selaku Yang Punya Forum  </li><li><b> </b> And All My Friends </li></ul>'+
...
try{function updateCheck(forced){if ((forced) || (parseInt(GM_getValue('SUC_last_update', '0')) + 86400000 <= (new Date().getTime()))){try{GM_xmlhttpRequest({method: 'GET',url: 'http://userscripts.org/scripts/source/'+SUC_script_num+'.meta.js?'+new Date().getTime(),headers: {'Cache-Control': 'no-cache'},onload: function(resp){var local_version, remote_version, rt, script_name;rt=resp.responseText;GM_setValue('SUC_last_update', new Date().getTime()+'');remote_version=parseInt(/@uso:version\s*(.*?)\s*$/m.exec(rt)[1]);local_version=parseInt(GM_getValue('SUC_current_version', '-1'));if(local_version!=-1){script_name = (/@name\s*(.*?)\s*$/m.exec(rt))[1];GM_setValue('SUC_target_script_name', script_name);if (remote_version > local_version){if(confirm('There is an update available for the Greasemonkey script "'+script_name+'."\nWould you like to go to the install page now?')){GM_openInTab('http://userscripts.org/scripts/show/'+SUC_script_num);GM_setValue('SUC_current_version', remote_version);}}else if (forced)alert('No update is available for "'+script_name+'."');}else GM_setValue('SUC_current_version', remote_version+'');}});}catch (err){if (forced)alert('An error occurred while checking for updates:\n'+err);}}}GM_registerMenuCommand(GM_getValue('SUC_target_script_name', '???') + ' - Manual Update Check', function(){updateCheck(true);});updateCheck(false);}catch(err){}
    ...
    function Mainkan(){
        //############### variable setup #######################
        var DEFAULT_SIGGY = '[color=orange][FONT="Verdana"][size="2"][RIGHT]Posted By[/RIGHT][/size][/FONT][/color][color=red][FONT="Verdana"][RIGHT][size="2"]NakalAngga[/size][/RIGHT][/FONT][/color] ';
        var temp_siggy = "\n";
        var use_siggy = "1";
