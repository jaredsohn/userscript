// ==UserScript==
// @name           cRPG Character Page Enhancements
// @namespace      crpg
...
{
  var code = document.createTextNode(global.containerUnknown.innerHTML);

  var warning = document.createElement('div');
  warning.innerHTML = 
    '<span style="font-weight:bold">'+
	  'WARNING: unknown or modified page elements were found (see below). It is recommended to disable the extension script. '+
	  '<a href="javascript:" onclick="document.getElementById(\'cunknown\').style.display = \'none\';">Ignore the warning and continue.</a>'+
	  '<br>Maybe a new version of script is already available <a href="http://userscripts.org/scripts/show/86900">here</a>.'+
	'</span><hr>';

  global.containerUnknown.insertBefore(warning, global.containerUnknown.firstChild);
  global.containerUnknown.appendChild(document.createElement('hr'));
  global.containerUnknown.appendChild(code);
...
function createHidden(name, value, parent)
{
...
{
  var version = document.createTextNode('v7.6 2011-03-17 - ');
...
function parsePageElements()
{
  ...
  for (var i = 0; i < all.length; i++)
  {
	...
	else if (node.nodeName == 'A' && node.textContent.indexOf('Credits for additional content from other modders') > -1 )
	{
	...
	else if (node.className != null && node.className.indexOf('donatorlist') > -1)
	{
	  move.push({node:node, container:global.containerDonation});
	}
...
function parseShopElements()
{
  ...
  if(paramItemType != '')
  {
	createHidden( "shop[" + paramItemType + "]", "true", form);
	form.action = getBaseUrlPlusParameterAndHash('itemtype', paramItemType, '#shop');
  }
...
function createShopItem(row, repairs)
{
  ...
  var difficultyMatch = desc.match(/requirement *(\d*)/);
