...
---
Initial version
Fixed overlay issue when new report graphics are enabled

1.1
---
Fixed issues with wrapper script
Switched to JQuery for communication
Script should now be compatible with Opera

1.2
---
Fixed escape issues for URL-schema characters in GET-request (#, ?, :)


1.3
---
Rewritten and adapted to a changed ajax.php-file

*/

var api = typeof unsafeWindow != 'undefined' ? unsafeWindow.ScriptAPI : window.ScriptAPI;
api.register( 'DSsendReportsToOBST', [8.5, 8.6, 8.7, 8.8, 8.9], 'Torridity', 'nospam@dsworkbench.de' );
...
function checkRequestResult(responseDetails) {
	if (responseDetails.error) {
		...
		var cur_obst_rid = responseDetails.reportID;
		var url = load('url') + "/index.php?page=reports&action=view&id=" + cur_obst_rid;

		// Erfolg melden
		error('<a href="'+ url +'" target="_blank">Bericht</a> erfolgreich eingelesen', 'green');
...
function sendReport() {
	...
	var reportText = window.text;//.replace(/\?/g, "%3F").replace(/#/g, "%23").replace(/:/g, "%3A");
	var data = {
		report: reportText,
		user: user,
		pass: pwhash,
...
function init(){
	...
	if (rids['ds_'+ cur_ds_rid]) {
		...
		if (url == '') {
			...
			status.innerHTML = '<a href="'+ rep_url +'" target="_blank">Bericht</a> bereits eingelesen';
		}
	} else {
