// ==UserScript==
// @name           Earth Empires Add-on v1.3
...
if(pagesub == "/purchases"){
	...
	for(var i=0; i<allTD.length; ++i){
		...
		if(allTD[i].innerHTML.substr(8,6) == "Troops"){	// find td object with keyword in it
		...
		else if(allTD[i].innerHTML.substr(8,4) == "Jets"){
			td_index = i+4;
			var jet_idx = td_index;
			addListener(jet_idx, 1, 2);		
		}

		else if(allTD[i].innerHTML.substr(8,7) == "Turrets"){
			td_index = i+4;
			var turret_idx = td_index;
			addListener(turret_idx, 1, 3);
		}

		else if(allTD[i].innerHTML.substr(8,5) == "Tanks"){
			td_index = i+4;
			var tank_idx = td_index;
			addListener(tank_idx, 1, 4);
		}

		else if(allTD[i].innerHTML.substr(8,8) == "Bushels<"){
			td_index = i+4;
