...
window.addEventListener('load', function(event) {
   ...
   dojo.addOnLoad(
        function() {
            ...
            attributeName = "";
            if (confirm("Reroll for Speed?")) {
                attributeName= "Speed";
            } else if (confirm("Reroll for Strength?")) {
                attributeName = "Strength";
            } else if (confirm("Reroll for Agility?")) {
                attributeName = "Agility";
            } else if (confirm("Reroll for Kicking?")) {
                attributeName = "Kicking";
            } else if (confirm("Reroll for Punting?")) {
                attributeName = "Punting";
            } else if (confirm("Reroll for Throwing?")) {
                attributeName = "Throwing";
            } else if (confirm("Reroll for Catching?")) {
                attributeName = "Catching";
            } else if (confirm("Reroll for Jumping?")) {
                attributeName = "Jumping";
            }
            if (attributeName != "") {
...
function parseReloadedPage(response, ioArgs) {
    ...
    if (s >= minimumAttribute && confirm(attributeName+' = '+s+'\n\nYou wanted the attribute to be at least '+minimumAttribute+'\n\nKeep this? Hit cancel to keep rerolling\n\nTotal number of rerolls:'+rerollcount)) {
        ...
        rerollcount++;
	document.getElementById("contexMenuContainer").innerHTML = ('rolling' + rerollcount);
