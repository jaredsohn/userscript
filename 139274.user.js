...
// @description    http://userscripts.org/scripts/show/139274
// @description    updated with Q7 support by J - mod by S, added used Q6 & Q7 weapon in battle window calculated with max current damage (only finished kills without NE) - edited by J, BH & CH top 5 fixed
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 200, 94, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 200, 94, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	var content = $j('div#pvp_battle_area');
        ...
        var max_hit_6 = dmgCalc(militaryRank, strength, 120, 1, 1);
        $j('#total_damage').prepend('<tr><td><div>'+
        '<div><b >&nbsp;</b><small>Hit / Q6:</small>'+
        '<p id="pCurrentHit">'+ max_hit_6 +' / ' + (current_damage/(max_hit_6*6)).toFixed(2) +'</p></div></td></tr>');
        $j('#total_damage').css('margin-top','-100px');
    
        var max_hit_7 = dmgCalc(militaryRank, strength, 200, 1, 1);
        $j('#total_damage').prepend('<tr><td><div>'+
        '<div><b >&nbsp;</b><small>Hit / Q7:</small>'+
        '<p id="pCurrentHit">'+ max_hit_7 +' / ' + (current_damage/(max_hit_7*10)).toFixed(2) +'</p></div></td></tr>');
        $j('#total_damage').css('margin-top','-100px'); 
