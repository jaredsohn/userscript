...
// pabst fixed something 3/4/2012
...
function parsePlayerPage(address, page) {
    ...
    getInetPage("http://glb.warriorgeneral.com/game/home.pl", parseCurrentDay);
}
...
function parseCurrentDay(address, page) {
    ...
    setDay(parseInt(d));
    getInetPage("http://glb.warriorgeneral.com/game/bonus_tokens.pl?player_id="+playerId, loadTrainingUpgrades);
}
...
function loadTrainingUpgrades(address, page) {
    ...
    getInetPage("http://glb.warriorgeneral.com/game/boost_player.pl?player_id="+playerId, loadAvailableBoosts);
    getInetPage("http://glb.warriorgeneral.com/game/xp_history.pl?player_id="+playerId, loadBoostCount);
}
...
function incrementDay() {
    ...
    if (getAge() >= extended_plateau_age) {
        if ((getLevel()+getBoosts()) < 79) {
            ...
            var addedBoosts = Math.min(79-getBoosts()-getLevel(), 6);
...
// move this to after the age has incremented, to use an if command incase the number of days is equal to 281
...
function calcMaxPossiblemulti4BTGain() {
    ...
    return Math.floor((trainingDaysLeft-1)/4)*12;
}
