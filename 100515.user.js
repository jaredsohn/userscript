...
// ===============================================================================
// Licencja i zrzeczenie sie odpowiedzialnosci
// ===============================================================================
// Ten program udostepniam na zasadzie : podoba Ci sie - podeslij gold autorowi :D
// Ilosc golda jest Twoja prywatna sprawa.
// Konto autora: http://www.erepublik.com/pl/citizen/profile/1283469
// Program dostarczam bez zadnych gwarancji oraz odpowiedzialnosci za negatywne
// dzialanie - korzystasz na wlasna odpowiedzialnosc.
...
var currURL		= 
location.href;																// http://www.erepublik.com/pl/citizen/profile/2622385
var arrURL		= 
...
function letsJQuery() {
    ...
    if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
        ...
        $j('div.citizen_military:last').after(
            '<h3>istatisçikçi keldi hanım</h3>' +
            ...
            '</small></div></div>' +
            '<h3>kaç cm?' +
            '<span style="float:right">dabanca: <select id="dmgWeapon" size="1">' +
            '<option value="0">yok</option><option value="20">tekkırma</option><option value="40">daramalı</option><option value="60">füze</option>' +
            '<option value="80">helikobder</option><option value="100" selected="selected">tank buv buv</option>' +
            '</select>&nbsp;&nbsp;&nbsp;&nbsp;# kaç posta: <input id="dmgFights" name="dmgFights" value="25" size="4" maxlength="4" />' +
            '&nbsp;<button id="dmgCalc">40 yabar</button>' +
        ...
        $j('button#dmgCalc').click(function() {
            ...
            $j('div#dmgResults').html(
                ...
                '<div class="stat"><small>NE bonusuynan: <strong><span style="font-size:12px;margin-right:15px">' +
	...
	unsafeWindow.battleFX.hit = function() {
		...
		if (multiHitRunning) {
			...
			bLog('ve goll!!');
			$j('div#multihit_message').html('Hits: ' + multiHitDone + '&nbsp;&nbsp;Kills: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? 'Last Enemy killed!' : 'ılık göt hayatta!') + ')');
	...
	unsafeWindow.battleFX.blow = function() {
		...
		if (multiHitRunning) {
			...
			bLog('ENEMY KILLED!');
			$j('div#multihit_message').html('Hits: ' + multiHitDone + '&nbsp;&nbsp;Kills: ' + multiHitEnemyKill + ' (' + (multiHitLastKilled ? 'son incinen!' : 'Last Enemy ALIVE!') + ')');
		}
...
//	});
	...
	$j('div#pvp_header h2').prepend('<div style="float:left;font-size:x-small">yenile: <select id="monitor_refresh" size="1">' +
...
'<input type="checkbox" id="multihit_food" name="multihit_food" checked="checked"><label for="multihit_food">&nbsp;lazım oldukca eppek yi</label>' +
...
'<fieldset id="BLOG"><legend>savaş kayıtların:&nbsp;&nbsp;<button id="battlelog_clear">clear</button></legend><div id="battlelog"></div></fieldset>'
);
