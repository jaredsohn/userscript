// ==UserScript==
// @name           Reddit Comment Bookmarks For Opera
...
(function() {
	...
	/**
	  * Adapted from emulate GM functions by TarquinWJ
	  * version 1.3.1
	...
	if(typeof(GM_xmlhttpRequest) == undefined){
		function GM_xmlhttpRequest(options){
			//We'll simply make jsonp requests because all XHRs that are made through this API are to the appengine.
			//Obviously its not a good emulator
			var url = options['url'] + ((options['method'] == 'POST') ? ('?' + options['data']) : '') + '&jsonp=?';
			...
			onload: function(c){
				...
				if((offset > 0) || (len == 10)){
					...
					if(offset > 0){
						...
						nextPrev.appendChild(document.createTextNode(' ' + ((len == 10) ? '| ' : '')));
					}
					if(len == 10){
						var lnk = document.createElement('a');
						lnk.setAttribute('href', 'javascript:void();');
