...
// @exclude		*://plus.google.com/u/*/_/streamwidgets*
...
var StraightGoogle = (function ($$d) {
	...
	// for Google Image Search and Web Search
	...
	function img_search_clean() {
		...
		// mark light box
		var thumbnail = $$d.querySelector("#rg_hl");
		// do lightbox
		$K.lightbox.bind(thumbnail);
	}
	...
	function _start() {
		...
		if (/:\/\/plus\.google\.com\/.*$/.test($K.url)) {
			...
			// for both plus homepage and notification
			$K.window.onReady("AF_initDataChunkQueue",
				function ($w, $var) {
		...
		// Google Image Search
			...
			// inject the google image processing function
			var __inject_image_checker_timer = null,
				__setHrefParam = function () {};
			// stop google adding some additional parameters to URL links
			function __inject_image_window() {
				$K.window.get("google").isr.Hover.setHrefParam = function (a) {
					if (/^https?:\/\/\w+\.google\.com(\.\w+)*\//.test(''+a.href))
						return __setHrefParam.apply(__setHrefParam, Array.prototype.slice.apply(arguments));
				}
			}
			// re-inject the clean function if the google.isr.Hover is reloaded
			function __inject_image_checker() {
				var Gi = $K.window.get("google").isr;
				if (Gi.Hover.__inited)
					return;
				// mark google.isr as loaded
				Gi.Hover.__inited = true;
				// clean function
				$K.listen(
					"#rg_s",
					"DOMNodeInserted",
				...
				// google reload script after window is resized / url is changed
				__inject_image_window();
				img_url_clean();
			}
			// text css style
			$K.css.set('.rg_hr span#rg_hr a{ color: #093; white-space: nowrap; text-decoration: none; }');
			
			$K.window.onReady(
				'google',
				function ($w) {
					var Gi = $w.google.isr;
					__setHrefParam = Gi.Hover.setHrefParam;
					$K.url.onHashChange(__inject_image_checker, true);
					// google will reset Gi.Hover after the script is loaded
					Gi.Hover.__inited = true;
					$K.listen(
						$$d,
						"mousemove",
						__inject_image_checker
					);
				},
				{
					test: function (w) {
						// if Google Image script is initialized
						return w['google'] && w['google'].isr && w['google'].isr.Hover;
					}
				}
			);
		}
