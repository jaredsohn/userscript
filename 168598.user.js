...
function showLookupIcon(evt)
{
	...
	//div container 
	divLookup = createElement('div', {id:'divLookup', style:'background:transparent; color:#000000; position:absolute; padding:1px; z-index:10000; -moz-border-radius:2px; top:'+(evt.clientY+window.pageYOffset+10)+'px; left:'+(evt.clientX+window.pageXOffset+10)+'px; '});
...
function lookup(evt)
{
	...
	// 修改获取翻译的方式，从抠取网页中的翻译文本改为获取Google Translate API提供的翻译文本
...
function quickLookup()
{
	...
	currentURL = "http://translate.google.cn/translate_t?text=" + txtSel + "&langpair=" + getId('optSelLangFrom').value + "|" + getId('optSelLangTo').value;
	GM_xmlhttpRequest({
		method: 'GET',
		url: currentURL,
		onload: function(resp) {
...
function extractResult(translation)
{
	...
	tl = GM_getValue('to') ? GM_getValue('to') : "auto";
	lang = sl + "|" + tl;
	currentURL = "http://translate.google.cn/translate_t?text=" + encodeURIComponent(txtSel) + "&langpair=" + lang;
...
function options(evt)
{
	...
	{
		divOptions = createElement('div', {id:'divOpt', style:'border-left:2px solid #5A91D8; position:relative; padding:5px; display:inline; float:right;'});
