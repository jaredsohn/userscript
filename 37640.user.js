...
function parseStatsList(statslist, ret) {
  ...
  var els = statslist.getElementsByTagName("*");
  for (var i=0; i<els.length; i++) {
    if (els[i].className == 'stat_head') ret[count++] = els[i].innerHTML;
    if (els[i].className == 'stat_value' || els[i].className == 'stat_value_boosted') {
       var val = els[i].innerHTML;
       var pos=val.indexOf('<');
       if (pos >= 0) val = val.substring(0, pos);
       ret[count++] = parseFloat(val);
    }
  }
...
window.setTimeout( function() {
     ...
     var player_name = document.getElementById('player_name'), pos = "", strength=0, speed, agility, jumping;
     ...
     var ptr1 = player_name.innerHTML.indexOf("(", 0);
     if (ptr1 >= 0) {
       var ptr2 = player_name.innerHTML.indexOf(")", ptr1);
       if (ptr2 >= ptr1) pos=player_name.innerHTML.substring(ptr1+1, ptr2);
     }

     var player_stats = document.getElementById('player_stats');
     if (player_stats == null) return;

     var stats_list = getElementsByClassName('alternating_color1', player_stats);
     for (var i=0; i<stats_list.length; i++)
       arr = parseStatsList(stats_list[i], arr);
     stats_list = getElementsByClassName('alternating_color2', player_stats);
     for (var i=0; i<stats_list.length; i++)
       arr = parseStatsList(stats_list[i], arr);
