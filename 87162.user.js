...
// @description    A couple of Neogame functions for Ogame Redesign - v1.26.6
// @version	1.26.6
...
/*
03.12.2010 v1.26.4
- Bug with event list (sometimes fleets disappeared)

16.11.2010 v1.26.3
* Fix for Jumpgate in 1.4.3

17.10.2010 v1.26.2
* Workaround for compatibility with other scripts
- Bug with phalanx/IPM range

10.09.2010 v1.26.1
- A trouble when showing player name in Event list

09.08.2010 v1.26.0
+ Options: Show fleet composition and cargo in EventList/Phalanx
+ Option: Autocopy coordinates globally (from any page opened in the browser)
+ Option: Scroll time to the next event in the window title
+ Show phalanx and IPM range on station/defense pages
+ Always set mission to 'Colonization' when it is enabled
* Several style fixes
*/
...
var AntigameFunc = function()
{
	...
	var metal_multiplier = 1.0;
	var crystal_multiplier = 1.2;
	var deuterium_multiplier = 2.2;
	var multipliertext = metal_multiplier + ":" + crystal_multiplier + ":" + deuterium_multiplier;
	var supertotal_threshold = 40000;
	...
	AntiGame_lang.InterfaceEN =
	{
		...
		lbl_debris: 'Debris',
		lbl_total: 'Total',
		lbl_total_w: 'Total weighted',
		lbl_loot: 'Loot',
		lbl_loot_w: 'Loot weighted',
		lbl_metal: 'Metal',
		lbl_crystal: 'Crystal',
		
		lbl_shipSSCargoAlt: 'SS',
	...
	AntiGame_lang.InterfaceRU =
	{
		...
		opt_uni_reDesigned: 'Старая вселенная с введенным редизайном',
		...
		opt_evt_showReduced: 'Сжатый список событий',
		opt_evt_TimeMode: 'Время / Обратный отсчет по умолчанию в сжатом списке событий',
			...
			try {
				function sendRequest(url, li) {
					...
					xhr.onreadystatechange = function()
					{
						if (xhr.readyState == 4)
							if (xhr.status == 200)
								EventList.showFleetDetails(li, xhr.responseText);
					}; 
					xhr.send(null); 
				}
	...
	var Jumpgate = {
		Show: function(e)
		{
			if(!e || !e.target || e.target.id != "jumpgate") return;
...
';
		...
		{
			this.insertCSSRules();
			document.body.addEventListener("DOMNodeInserted", Jumpgate.Show, false);
		}
	}
	...
	var SpyReport = 
	{
		...
		{
			var total_debris = this.debris_metal + this.debris_crystal;
			var w_total_debris = this.debris_metal * metal_multiplier + this.debris_crystal * crystal_multiplier;
			var total = this.metal + this.crystal + this.deuterium;
			var w_total = this.metal * metal_multiplier + this.crystal * crystal_multiplier + this.deuterium * deuterium_multiplier;
			
			w_total      = Math.ceil(w_total);
			w_loot		 = Math.ceil(w_total/2);
			w_supertotal = Math.ceil(w_loot + w_total_debris);
			...
			var ss_cargos    = Math.ceil(capacity_needed/1500);
			var small_cargos = Math.ceil(capacity_needed/5000);
			var large_cargos = Math.ceil(capacity_needed/25000);

			SimpleTable.init(Options.Interface.lbl_resources + " - " + multipliertext);
			SimpleTable.addCell(Options.Interface.lbl_total, total);
			SimpleTable.addCell(Options.Interface.lbl_shipSSCargoAlt, ss_cargos);

			SimpleTable.addCell(Options.Interface.lbl_loot_w, w_loot);
			SimpleTable.addCell(Options.Interface.lbl_shipLCargoAlt, large_cargos);
			
			if (total_debris < supertotal_threshold) {
				SimpleTable.addCell("", "");
			}
			else {
				SimpleTable.addCell("Supertotal weighted", w_supertotal);
			}
...
/*
	...
	var Buildings = 
	{
		...
		Show: function(e)
		{
			...
			var id = document.getElementsByName('type')[0];
			if (id) id = id = parseInt(id.value,10);
	...
	try	{
		...
		if ( Utils.isCurrentPage('station-moon') )
			Jumpgate.Run();
			
		if ( Options.showResources )
			Resources.Resources_Run();
			
		if (Options.misc_scrollTitle)
			Misc.scrollTitle()
