...
var friendClans = "|TemplarS|Czeka Nas Chwała|Hypnose|Liberated|";
var enemyClans = "|NighTmarE|Friends Team|Air-Traffic Control|Aes Sedai|Sprzedawcy Marzeń|Hortus Gloria|Enchanted Elementium|";
var enemyPlayers = "|czesio gra|blondi|czesiczek|FurianS|Brash|Astenna|dielpiero|Wesoły Topek|";
var enemyPlayersE2 = "|czesio gra|FurianS|Cortezon|czesiczek|Brash|Astenna|dielpiero|Wesoły Topek|";
...
["Demonis Pan Nicości",	"Heros",	"210",	"Demon",	"/obrazki/npc/hum/sekta_demon_cz_s.gif"],
["Tepeyollotl",			"Heros",	"260",	"Kot",		"/obrazki/npc/pot/tepeyollotl.gif"]];
	...
	for (var w=0 in items)
	{
		...
		if (Opcje.autoStrzaly && items[w].cl==21 && items[w].st==7 && items[w].loc=="g")
		{
			if (items[w].stat.indexOf("ammo=") >= 0) 
			{
				...
				if (parseInt(s[1]) <= 50 && !m.autoEquipArrows(items[w].name)) actionStop("Strzaly");
			}
		}
	}
...
questmap[quest_iter++] = [qcmdTalk2,3,79,"40.5"]; //do eder
	...
	{	    
        ...
        questmap[quest_iter++] = [qcmdKill,19,26]; //zabij weza na drodze
        questmap[quest_iter++] = [qcmdEnter,95,17]; //Słoneczna Wyżyna
        questmap[quest_iter++] = [qcmdEnter,63,19]; //Zasłoniete Jezioro
        questmap[quest_iter++] = [qcmdEnter,63,34]; //Spokojne Przejście
        questmap[quest_iter++] = [qcmdEnter,63,50]; //Eder
    ...
    questlist[questno++] = [ID_MAHOPTEKAN, quest_iter, "QD:Mahopt.qII"];
    {
        ...
        questmap[quest_iter++] = [qcmdUseItem, "teleportacji na Kwieciste"];  // uzyj
        questmap[quest_iter++] = [qcmdKill,48,32]; 	// zabij bandyte na moscie
        questmap[quest_iter++] = [qcmdEnter,50, 0]; // -> Lazurowe WzgA3rze
        questmap[quest_iter++] = [qcmdKill,19,26]; 	// zabij weza na drodze
        questmap[quest_iter++] = [qcmdEnter,95,17]; // -> SA?oneczna WyAYyna
        questmap[quest_iter++] = [qcmdEnter,63,19]; // -> ZasA?oniÄ?te Jezioro
        questmap[quest_iter++] = [qcmdEnter,63,34]; // -> Spokojne PrzejA?cie
        questmap[quest_iter++] = [qcmdEnter,63,50];	// -> Eder
        questmap[quest_iter++] = [qcmdTalk2,26,40,"20.1|71.2"]; //do torneg
        ...
        questmap[quest_iter++] = [qcmdEnter,50, 0]; // -> Lazurowe WzgA3rze
        questmap[quest_iter++] = [qcmdKill,19,26]; 	// zabij weza na drodze
        questmap[quest_iter++] = [qcmdEnter,35,0]; // -> SA?oneczna WyAYyna
        questmap[quest_iter++] = [qcmdEnter,48,0]; // -> Thuzal
...
var mActive = new moduleActivity();
	...
	{	
		if (mMargo.nhero>0 && !heronotify)
		{
			...
			if (xmlhttp)
			{
				var params = "op=hero&nick="+mMargo.heroname+"&mapid="+unsafeWindow._map.id+""+mMargo.heroposx+"&y="+mMargo.heroposy+"&world="+window.location.href.split("margo+")[0];
				trace(params);
