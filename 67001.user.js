// Die Stamme Erweiterung PL
// tlumaczenie niedokonczone jeszcze
//
// --------------------------------------------------------------------
...
// --------------------------------------------------------------------
// ==UserScript==
// @name           Die Staemme Features PL
// @namespace      http://userscripts.org/scripts/edit_src/67001
// @description    Staemme Features PL
...
//Neuen Link (inkl. Bild) fur das Menu erstellen
...
function expandMarketLink(market_link)
{
	...
	var eigene_angeb = createLinkNode("game.php?"+getUrlParam("village")+"&screen=market&mode=own_offer","","Własne propozycje","");
	var fremde_angeb = createLinkNode("game.php?"+getUrlParam("village")+"&screen=market&mode=other_offer","","Obce oferty","");
	var status = createLinkNode("game.php?"+getUrlParam("village")+"&screen=market&mode=traders","","Status kupcy","");
...
{
	var submenu = document.createElement("ul");
	var truppen = createLinkNode("game.php?"+getUrlParam("village")+"&screen=place&mode=units","","Wojska","");
	var sim = createLinkNode("game.php?"+getUrlParam("village")+"&screen=place&mode=sim","","Symulator","");
...
function expandBerichtLink(bericht_link)
{
	var submenu = document.createElement("ul");
	arr = new Array();
	arr[0] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=report&mode=attack","","Atak","");
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=report&mode=defense","","Obrona","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=report&mode=support","","Pomoc","");
	arr[3] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=report&mode=trade","","Handel","");
	arr[4] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=report&mode=other","","Inne","");
...
function expandSettingsLink(setting_link)
{
	...
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=email","","Adres E-Mail","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=settings","","Ustawienia","");
	arr[3] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=move","","Zacznij od nowa","");
	arr[4] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=delete","","Usuń konto","");
	arr[5] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=share","","Dzielenie łącza","");
	arr[6] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=vacation","","Zastępstwo","");
	arr[7] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=logins","","Logowania","");
	arr[8] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=change_passwd","","Zmień hasło","");
	arr[9] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=settings&mode=poll","","Ankiety","");
	for (var i=0; i<arr.length; i++)
	{
		submenu.appendChild(arr[i]);
	}
...
function expandPremiumLink(prem_link)
{
	var submenu = document.createElement("ul");
	var arr = new Array();
	arr[0] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=premium&mode=help","","Przywileje","");
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=premium&mode=premium","","Zakup","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=premium&mode=points","","Uaktywnij","");
	arr[3] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=premium&mode=log","","Protokół","");
	for (var i=0; i<arr.length; i++)
	{
		submenu.appendChild(arr[i]);
	}
...
function expandRangliste(rangliste_link)
{
	var submenu = document.createElement("ul");
	var arr = new Array();
	arr[0] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=ally","","Plemiona","");
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=player","","Gracz","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=con_ally","","Plemiona na kontynencie","");
	arr[3] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=con_player","","Gracze na kontynencie","");
	arr[4] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=kill_player","","Pokonani przeciwnicy","");
	arr[5] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ranking&mode=kill_ally","","Pokonani przeciwnicy (plemiona)","");
	for (var i=0; i<arr.length; i++)
	{
		submenu.appendChild(arr[i]);
	}
...
function expandStamm(stamm_link)
{
	...
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ally&mode=members","","Członkowie","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=ally&mode=contracts","","Dyplomacja","");
...
function expandNachrichten(nachrichten_link)
{
	var submenu = document.createElement("ul");
	var arr = new Array();
	arr[0] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=mail&mode=mass_out","","Wiadomość grupowa","");
	arr[1] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=mail&mode=new","","Napisz wiadomość","");
	arr[2] = createLinkNode("game.php?"+getUrlParam("village")+"&screen=mail&mode=block","","Zablokuj nadawcę","");
	for (var i=0; i<arr.length; i++)
	{
		submenu.appendChild(arr[i]);
	}
...
{	
	if (getUrlParam("screen") == "screen=overview")
	{
		...
		/*Falls in der Dorfliste etwas ausgewahlt wurde, muss eine Uberprufung stattfinden
		weil die counter nicht gesetzt wurden, das Dorf aber verandert wurde */
	...
	if (getUrlParam("screen") == "screen=place") //Versammlungsplatz wird aufgerufen
	{
		if ((getUrlParam("mode") == "") || (getUrlParam("mode") == "mode=command"))
		{
			...
			document.addEventListener('click', function(event)
			{
				...
				{
					Check = confirm(unescape("Wollen Sie den Favorit wirklich loschen?"));
					if (Check == true)
					{
		...
		if (getUrlParam("mode") == "mode=sim") //Links in den Simulator einfugen
		{
			sim_insertAttModeLinks();
		}
	}
	
	//Marktplatz - Favoriten einfugen
	...
	{
		//Wird ein spezieller Bericht wird angeschaut, muss gepruft werden ob das kein Bericht ist, bei dem Unterstutzung in einem anderen Dorf angegriffen wurde
		if ((getUrlParam("view") != "") && (document.body.innerHTML.indexOf("Deine Unterstutzung") < 0))
		{
	...
	//Schnellleiste erstellen und einfugen
	createMenubar();
	
	//Gesamtes Menu ersetzen
	expandMenu();
	
	//Pfeil-Navigation einfugen
...
function ajax_createVillageList()
{
	...
	GM_xmlhttpRequest({
		...
		onload:function(details) {
				...
				location.reload();
				alert(unescape("Dorferliste aktualisiert! Ab jetzt kann mit Pfeilen navigiert werden!"));
		}
	});
}

/* Fugt die Links-Rechts Pfeile hinzu, mit denen man schnell durch die Dorfer switchen kann */
...
//Fugt in der Ubersicht einen Button hinzu, bei dessen click ein Fenster mit den vorhandenen Dorfern erscheint
function insertWhenStorageIsFull()
{
	...
	if (document.body.innerHTML.indexOf("att.png") >= 0) //Angriff lauft -> weitere Table wurde eingefugt durch Stamme-Script -> nachste Table verwenden
	{
	...
	catch (e) //tritt ein Fehler auf, ist die klassische Dorfubersicht aktiv -> table [8]
	{
...
function sim_insertAttModeLinks()
{
	...
	if (table.getElementsByTagName("tr")[0].getElementsByTagName("th")[0].innerHTML.indexOf("img") >= 0) //Simulation ausgefuhrt
	{
		sim_insertLostRessources(table);
		
		if (help.innerHTML.indexOf("Schaden durch") >= 0) //Rammbocke/Katapultas wurden mitsimuliert -> nachste Table
		{
	...
	else //keine Simulation ausgefuhrt
	{
...
function saveSpiedTroups()
{
	...
	GM_setValue("spy_Pikinier",parseInt(data[1].innerHTML)-parseInt(lost[1].innerHTML));
	GM_setValue("spy_Miecznik",parseInt(data[2].innerHTML)-parseInt(lost[2].innerHTML));
	GM_setValue("spy_Topornik",parseInt(data[3].innerHTML)-parseInt(lost[3].innerHTML));
	GM_setValue("spy_Łucznik",parseInt(data[4].innerHTML)-parseInt(lost[4].innerHTML));
	GM_setValue("spy_Zwiadowca",parseInt(data[5].innerHTML)-parseInt(lost[5].innerHTML));
	GM_setValue("spy_Lekki kawalerzysta",parseInt(data[6].innerHTML)-parseInt(lost[6].innerHTML));
	GM_setValue("spy_bŁucznik",parseInt(data[7].innerHTML)-parseInt(lost[7].innerHTML));
	GM_setValue("spy_Ciężki kawalerzysta",parseInt(data[8].innerHTML)-parseInt(lost[8].innerHTML));
	GM_setValue("spy_Taran",parseInt(data[9].innerHTML)-parseInt(lost[9].innerHTML));
	GM_setValue("spy_Katapulta",parseInt(data[10].innerHTML)-parseInt(lost[10].innerHTML));
	GM_setValue("spy_Rycerz",parseInt(data[11].innerHTML)-parseInt(lost[11].innerHTML));
	...
	if (table && (table.getElementsByTagName("tr").length > 1)) //Wall auch erspaht
	{
		...
		{
			var wall = geb.innerHTML.split("Wall <b>"); //(Stufe X)</b><br /> bleibt ubrig
			
			if (wall.length > 1) //Wall wurde erspaht
			{
	...
	else //Wall wurde nicht erspaht
	{
		GM_setValue("spy_wall",0);
	}
	
	/*Wall wurde vielleicht nicht erspaht, aber beschadigt -> Gebaudestufe ersichtlich */
	if (document.body.innerHTML.indexOf("Schaden durch") >= 0)
	{
		...
		//Wurde der Wall zusatzlich durch Katapultapulte beschadigt?
...
{
	document.getElementsByName("def_spear")[0].value = GM_getValue("spy_Pikinier");
	document.getElementsByName("def_sword")[0].value = GM_getValue("spy_Miecznik");
	document.getElementsByName("def_axe")[0].value = GM_getValue("spy_Topornik");
	document.getElementsByName("def_archer")[0].value = GM_getValue("spy_Łucznik");
	document.getElementsByName("def_spy")[0].value = GM_getValue("spy_Zwiadowca");
	document.getElementsByName("def_light")[0].value = GM_getValue("spy_Lekki kawalerzysta");
	document.getElementsByName("def_marcher")[0].value = GM_getValue("spy_bŁucznik");
	document.getElementsByName("def_heavy")[0].value = GM_getValue("spy_Ciężki kawalerzysta");
	document.getElementsByName("def_ram")[0].value = GM_getValue("spy_Taran");
	document.getElementsByName("def_catapult")[0].value = GM_getValue("spy_Katapulta");
	document.getElementsByName("def_knight")[0].value = GM_getValue("spy_Rycerz");
...
function expandMenu()
{
	...
	//Uberprufung ob es neue Nachrichten im Stamm gibt (Image kommt vor, weil das Bild auch ein Link ist muss mit link-counter (erhohung um 1) gearbeitet werden)
	var stamm;
	var stamm_forum;
	if (link_array.length == 12) // -> Neue Beitrage im Stammesforum, Bild kommt vor
	{
...
{
	var main_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=main","graphic/buildings/main.png","Ratusz","");
	var kaserne_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=barracks","graphic/buildings/barracks.png","Koszary","");
	var stall_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=stable","graphic/buildings/stable.png","Stajnia","");
	var werkstatt_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=garage","graphic/buildings/garage.png","Warsztat","");
	var marktplatz_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=market","graphic/buildings/market.png","Rynek","");
	var vers_platz_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=place","graphic/buildings/place.png","Plac","");
	var adelshof_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=snob","graphic/buildings/snob.png","Pałac","");
	var schmiede_link = createLinkNode("game.php?"+getUrlParam("village")+"&screen=smith","graphic/buildings/smith.png","Kuźnia","");
	...
	//Menu-Div erstelle und Style setzen
	...
	//Schnellleiste im Dokument einfugen
