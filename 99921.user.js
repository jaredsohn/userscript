// ==UserScript==
// @name           Tower by Cook Francais
// @namespace      Cook
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @description    Tower Alert de l\'Alliance en francais
...
-----------------------     
TODO (BOmmel): Add TransportTab
TODO (Hiro): Add WhisperAlertTab
-----------------------     
History:

2011-02-04 (Version 0.2.0) Hiro
   - Initial 
   - change structure to get "Phoenix Tools" as Tab working with "Power Tools" and "Power Bot" 
   - add food info and fortification info to Advanced TowerAlert
2011-02-08 (Version 0.2.7) Hiro
   - add board tab
   - change tab theme colors to ally colors
   - fix "Phoenix Tool" tab to fit to tab menue 
2011-02-08 (Version 0.2.10) Hiro
   - add adv.players tab (+oninestatus, +loginDate)   
**********************/

var Version = 'eres_200311_fr';
var Versiondate = '20.03.2011';
var Scriptname = 'Tower Cook FR';
var ScriptTabName = 'Tower Cook';
...
fixTower     : true,
  ...
  alertConfig  : {
  	aChat:true, 
	aPrefix:'**** Au secour les Papys un connard veut me poutrer ****', 
	scouting:true, 
	wilds:true, 
	minTroops:1, 
	spamLimit:10, 
...
Tabs.Options = {
  tabOrder: 40,
  tabLabel : 'Configurez TowerAlert',
  myDiv : null,
  fixAvailable : {},
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pcTabLined cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Poster automatiquement les attaques sur le chat de l\'alliance.</td></tr>\
	          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
	              ...
	              <TR><TD align=right>Alerte sur eclaireur : &nbsp; </td><TD><INPUT id=pcalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Alerte sur attaques TS : &nbsp; </td><TD><INPUT id=pcalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Nombre minimum de troupe : &nbsp; </td><TD><INPUT id=pcalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pcalerterr></span></td></tr>\
	              <TR><TD align=right>Montrer troupes dans l\'ambassade : &nbsp; </td><TD><INPUT id=pcalertEmbassy type=checkbox '+ (Options.alertConfig.embassy?'CHECKED ':'') +'/></td></tr>\
	              ...
	              <TR><TD align=right>Montrer les pommes utilisees : &nbsp; </td><TD><INPUT id=pcalertFood type=checkbox '+ (Options.alertConfig.food?'CHECKED ':'') +' /></td></tr>\
	              <TR><TD align=right>Montrer mes defenses : &nbsp; </td><TD><INPUT id=pcalertDefense type=checkbox '+ (Options.alertConfig.defense?'CHECKED ':'') +' /></td></tr>\
	              <TR><TD align=right>Ma ville principale "precisez le nom et coord" : &nbsp; </td><TD>'+ citySelect +'&nbsp; &nbsp;</td></tr>\
				  <TR><TD align=right>Ma ville est en defense "Precisez cette option": &nbsp; </td><TD><INPUT id=pcalertactivedefense type=checkbox '+ (Options.alertConfig.activedefense?'CHECKED ':'') +' /></td></tr>\
	              <TR><TD align=right>Envoyer l\'alerte sur le canal de chat : &nbsp; </td><TD><INPUT id=pcalertSendToAlly type=checkbox '+ (Options.alertConfig.sendtoAlly?'CHECKED ':'') +' /></td></tr>\
				  <TR><TD align=right>M\'envoyer un chuchotement : &nbsp; </td><TD><INPUT id=pcalertSendAsWhisper type=checkbox '+ (Options.alertConfig.sendasWhisper?'CHECKED ':'') +' /></td></tr>\
				  <TR><TD align=right>Envoyer une alerte par courriel : &nbsp; </td><TD><INPUT id=pcalertSendEmail type=checkbox '+ (Options.alertConfig.sendEmail?'CHECKED ':'') +' /></td></tr>\
		          <TR><TD align=right>Mon adresse courriel : &nbsp; </td><TD><INPUT id=pcalertEmailAddress type=text size=36 value="'+ Options.alertConfig.emailAddress +'" \> &nbsp; &nbsp;</td></tr>\
				  <TR><TD align=right>Mon token : &nbsp; </td><TD><INPUT id=pcalertToken type=text size=36 value="'+ Options.alertConfig.token +'" \> &nbsp; &nbsp;</td></tr>\
	              </table></td></tr>\
      ...
      document.getElementById('pcalertTroops').addEventListener ('change', t.e_alertOptChanged, false);
		...
		document.getElementById('pcalertactivedefense').addEventListener ('change', t.e_alertOptChanged, false);
  ...
  e_alertOptChanged : function (){
	...
	Options.alertConfig.activedefense=document.getElementById('pcalertactivedefense').checked;
...
Tabs.board = {
  ...
  tabLabel : 'MAP Joueurs ou Alliances',            // label to show in main window tabs
  myDiv : null,
  timer : null,
  forumlink :  'http://koc.dunno.com/index.sjs?f=KocMapViewer&server_id=241',  
...
var TowerAlerts = {
  ...
  e_eachSecond : function (){   // check for incoming marches
	...
	postToChat: function(m, force){
		...
		msg += 'Ma ville principale est : ' + postOptions.hq + '. ';
		if (postOptions.activedefense){ 
			msg += 'Ma ville est ***en defense***. ';
			} else {
			msg += 'Ma ville est ***en sanctuaire***. ';			
			}	
		msg += 'Le type d\'assaut est : ' + atkType + '. ';
		msg += 'La cible est : ' + target + '. ';
		msg += 'L\'attaquant est : ' + who + ' (' + addCommas(attackermight) + ', ' + diplomacy + ').';
		...
		if (city.tileId == m.toTileId) {
			...
			if (emb.count > 0) {
				...
				msg += 'Mon embassade (' + availSlots + '/' + emb.maxLevel + ')';
		...
		if (Options.alertConfig.defense) {
			...
			fortifications['53'] = 'Arbaletes';
          	fortifications['55'] = 'Trebuchets';
          	fortifications['60'] = 'Pieges';
		...
		if (postOptions.sendEmail) {
			var subject = target + ' est attaqu� en ' + atkType + ' par ' + who + ' dans '+ arrivingDatetime;
