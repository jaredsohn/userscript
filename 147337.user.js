// ==UserScript==
// @name          Kaskus Quick Reply.co.id, Kaskus Spoiler Alert and Scroll on top
// @namespace     http://userscripts.org/scripts/show/147337
// @include       http://www.kaskus.co.id/showthread.php?*
// @version       Edit
// @dtversion     110521318
// @timestamp     1305932549494
// @description   Qr, Spoiler Alert for Kaskus.co.id-old and scroll on top
// @author        idx; arifhn; kucingapes
    ...
    var parse_ajax_post = function(html){
        ...
        if(html.indexOf('POSTERROR')!=-1){ // there's some error          
            ...
            if(cucok = ErMsg.match(/Please\s*try\s*again\s*in\s*([^.(]+)/i)){
                ...
                r.msg = (/did\snot\smatch/i.test(ErMsg) ? 'recaptcha not match, please try again..': ErMsg );
            }
            ...
            r.msg = '"Kaskus overload", post may has been posted. Try to reload first and conform the new post.';
        
        }else{
        
            r.msg = 'error, kaskus is crowded or your account may switch to kaskus beta';
        }
...
var QRdp = {
  // QR delay post 
  key: KS+"QR_LASTPOST"
 ,defDelay: 61 // seconds; assumed 32 sec; fact is 30 sec
...
var SML_LDR = {
 ...
 ,custom:{
    initCustom: function(){
      ...
      el = createEl('a',{id:'manage_help',href:'javascript:;','class':'twbtn lilbutton', style:'padding:1px 5px;margin-left:20px;display:none;',title:'HOME'}, '[ ? ]');
      Dom.add(el,cont);
      on('click',el,function(e){
      alert( ''
...
var rSRC = {
 ...
 ,getSCRIPT: function(){
  return (''
    ...
    +    '{theme:"blackglass",lang:"en"'
    +     ',custom_translations:{refresh_btn:"Reload reCaptcha :: ['+(gvar.isOpera ? 'Ctrl+Alt+R':'Alt+R')+']",instructions_visual:"insert reCaptcha:"}'
    +    '}'
    +  ');'
    +'};'    
...
"LightSteelbBlue": "LightSteelbBlue",
...
"Gold": "Gold"
    ...
    break;
    case "font":
     return ["Arial","Calibri","Arial Black","Arial Narrow","Book Antiqua","Century Gothic","Comic Sans MS","Courier New","Fixedsys","Franklin Gothic Medium","Garamond","Georgia","Impact","Lucida Console","Lucida Sans Unicode","Microsoft Sans Serif","Palatino Linotype","System","Tahoma","Times New Roman","Trebuchet MS","Verdana","Cambria","Berlin Sans FB","Fixedsys","Constantia","Segoe UI"];
    break;
    case "size":
...
,'549': [H+'lebaran05.gif', ':ketupat', 'ketupat']
,'550': [H+'lebaran04.gif', ':maafaganwati', 'maafaganwati']
,'551': [H+'lebaran03.gif', ':salaman', 'salaman']
,'552': [H+'lebaran02.gif', ':bedug', 'bedug']
,'553': [H+'lebaran01.gif', ':maafagan', 'maafagan']
...
,'907': [H+'fd_2.gif', ':cd:', 'Cape deeehh']
 ...
 ,getBOT_greet: function(min, max){
    var c = [
     ,'150.000 hours spent daily typing in captchas'
     ,'reCAPTCHA - Stop Spam, Read Books. O Reilly Radar.'
     ,'The system needs to make sure you are a human'
     ,'The system needs to make sure you are not a robot'
     ,'Reading Book Everyday - Everytime'
 ...
 ,getTPL_prompt_reCAPTCHA: function(){
  return (''
     ...
     +  '<a href="http://'+'userscripts.org/scripts/show/147337" target="_blank" title="Info, Tips, Suggestion, Digitalize">HOME</a>&nbsp;&#8212;'
     +  '</span>'
     +   '</div>'
     +   '<div class="spacer" id="posting_notify"></div>'
     +   '<div id="recaptcha_container" style="text-align:center;">'
...
// scroll kebawah
var SCROLL_PERCENTAGE = 0.6;
var SCROLL_DISTANCE_THRESHOLD = 1700;
var SCROLL_PERCENTAGE_NEEDED = 0.75;
...
$(window).ready(function(){
    var css = {
        ...
        '#scroll-to-top-from-script.show:hover' : {
            'opacity' : '0.7',
            'cursor' : 'pointer !important',
        },
    };
    appendCSS(css);
    $('body').append("<div title='Klik untuk naik ke atas gan!' id='scroll-to-top-from-script'></div>");
    
    $('#scroll-to-top-from-script').bind('click',function(){
        $(document).unbind('scroll');
        hideButton();
        $('html, body').animate({scrollTop: 0}, 800, function(){
            $(document).unbind('scroll').bind('scroll', showOrHideButton);
        });
    });
    ...
    var showOrHideButton = function() {
        ...
        if(windowHeight / docHeight <= SCROLL_PERCENTAGE_NEEDED && 
           ($(document).scrollTop() + windowHeight >= docHeight * SCROLL_PERCENTAGE && docHeight > SCROLL_DISTANCE_THRESHOLD 
            || docHeight < SCROLL_DISTANCE_THRESHOLD  && $(document).scrollTop() + windowHeight >= SCROLL_DISTANCE_THRESHOLD * SCROLL_PERCENTAGE_NEEDED
           )
          ){
            showButton();
        } else {
...
function showButton() {
    $('#scroll-to-top-from-script').addClass('show');
}
function hideButton() {
    $('#scroll-to-top-from-script').removeClass('show');
}
