...
function previewShortcut()
{
  ...
  if (current_entry)
  {
    var index = calcEntryIndex(current_entry);
    if (index>-1)
    {
...
function handleKeypress(event)
{
  ...
  if (String.fromCharCode(event.which)=="f" && !event.ctrlKey) {
    previewShortcut();
    try {
...
function scrollTo(index)
{
    ...
    try
    {
      location.href = 'javascript:void(w.$=-1);';
      location.href = 'javascript:void(w.Xf('+index+'));';
    }
    catch(err) { }
}
...
function setFont()
{
	for(var i=0; i<document.styleSheets.length; i++)
	  for(var j=0; j<document.styleSheets[i].cssRules.length; j++)
		{
		  //if(document.styleSheets[i].cssRules[j].selectorText.indexOf('body') > -1)
		  if (document.styleSheets[i].cssRules[j].style == undefined) continue;
		  if(document.styleSheets[i].cssRules[j].style[0] == 'font-family')
		  {
			document.styleSheets[i].cssRules[j].style.cssText = "font-family: tahoma;"
			return;
	      }
		}
}
