...
function submitNow()
{
	var form = document.getElementById("right_column_tools_checkin_form");
	form.submit();
}
...
function displayCountdown()
{
	...
	area.innerHTML = "<br> Time Remining: <br/>";
...
{
	var first = document.getElementById("hero_shot_container");
	...
	mainLabel.setAttribute('style','display:block; font-weight:bold; color:#666666; font-size:14pt; margin:20px');
	mainLabel.innerHTML = "When do you want to check in?";
	dateSelect.appendChild(mainLabel);

	//The date portion.
	dateLabel = document.createElement("label");
	dateLabel.setAttribute('style','display:block; font-weight:bold; color:#666666; margin-left:20px');
	...
	monthInput.setAttribute('style','margin-left:20px');
	...
	hourInput.setAttribute('style','margin-left:20px');
	...
	timeLabel.setAttribute('style','display:block; font-weight:bold; color:#666666; margin-left:20px; margin-top:10px');
	...
	//The delay button.
	delayButton = document.createElement("img");
	delayButton.setAttribute('id','delay-button');
	delayButton.setAttribute('src','http://nburoojy.org/other/btn_delay.php');
	delayButton.setAttribute('style','margin-left:20px; margin-top:10px');
	delayButton.setAttribute('alt','Delay');
	delayButton.addEventListener("click", beginDelay, true);

	newnode.appendChild(delayButton);
	...
	countdownArea.setAttribute('style','display:block; font-weight:bold; color:#666666; font-size:14pt; margin:20px');
...
{
	var element = document.getElementsByClassName("right_column_tools_text_half_left")[0];

	// create the new submit button.
	newitem = "<img alt=\"Delay\" \
		    id=\"submitCheckInTimeout\" \
		    name=\"retrievePaxListTimeout\" \
		    src=\"http://nburoojy.org/other/btn_delay.php\" \
		    title=\"Check In Online\"></img>";
	...
	newbutton = document.getElementById("submitCheckInTimeout");
	// make the click call our wait function
	newbutton.addEventListener("click", displayDelayMenu, true);
...
else if(/southwest\.com\/($|\#|\?)/.test(document.location.href))
{
	homePageFormEdit();
}
