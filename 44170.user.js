// ==UserScript==
// @name           FarmingMachine - Teuton mod Upgrayedd FIX
// @author         Sowrov (inital contribution by FDisk, Fuji) Mod by M4rtini. Fixes by Upgrayedd
...
// ==/UserScript==
var SCRIPT = {
	url : 'http://userscripts.org/scripts/source/44170.user.js',
	version : '4.3.0.1' //same value as @version
};
...
var minWait = 5200; //Don't make it smaller then 5000ms!
...
function drawMessageBox() {
	...
	runningDiv
			.setAttribute(
					"style",
					"text-align:center; margin:auto; position:absolute; top:750px; left:700px; width:370px; height:200px; background-color: #c2d9ec; border: 3px solid yellow; -moz-border-radius: 10px;");
...
function sendtroops() 
{
  ...
  //alert(fque);
	...
	if (notFoundHtml.indexOf("<div class=\"f10 e b\">") > -1) 
	{ //village not found or player bannded
	...
	if (sIndex < farmList.length) 
	{
		...
		else 
		{
			...
			if (arr[7] == "true") 
			{
				GM_log('sendtroops x1');
				GM_log('sendtroops x1 arr'+arr);
				GM_log('sendtroops x1 arr[0]'+arr[0]);
				GM_log('sendtroops x1 arr[1]'+arr[1]);
				var xy = arr[0].split(",");
...
skipcounter = 0;
	...
	var random = GM_getValue("RandomFarming_"+suffixGlobal, 0);
	if (newList.length == 0) 
	{ //Village Change is not possible, Engine is pause for X sec		
		pause = Random(30*minWait, 48 * maxWait);		
...
'NO_FARM_YET' :"Nastav si nějakou farmu (přehled cizí vesnice po kliknutí na mapu)",
'FARM' :"Farma",
...
'SUCCESS_COUNT' :"Celkem úspěšně poslaných loupenží",
'HALT_FARMING' :"Zastavit farmení",
'MACHINE_RUNNING' :"Farming Machine je spuštěný",
...
'TRIBE_SELECT3' :"Nenastavil jsi správně číslo, Nastavuji si tě jako římana.",
'ROMAN' : "Říman",
'TEUTON' : "Germán",
'GAUL' : "Gal"
}; 
		...
		lang = {
            ...
            'UPDATE_NOW' :"Da li Å¾elite aÅ¾urirati sada",
            'CONFIRM' :"jeste li sigurni",
            'REMOVEING' :"Uklanjanje",
            'SWITCH_V' :"promjeni na drugo selo",
            'AFTER' :"nakon",
            'SEC' :"sekunda",
            'NO_VILLAGE' :"Nema drugih sela za otiÄ‡i!",
            'WAITING' :"Äekanje",
            'BEFORE_RE' :"Ponovna provjera prije nego.",
            'ADD_FARM' :"HoÄ‡ete dodat farmu",
            ...
            'NO_FARM_YET' :"Postavite bilo koje selo kao svoju farmu kad uzmete odreÄ‘eno selo",
            'FARM' :"Farma",
            'SORT_FARM' :"sortiraj farme po udaljenosti od vaÅ¡eg sela ",
            ...
            'START_M' :"Izaberite poÄetnu farmu,iz ovog stubca",
            'START_FARMING' :"PoÄmi farmat iz izabranog sela",
            ...
            'NEXT_FARM' :"Izaberite slijedeÄ‡u farmu",
            'SUCCESS_COUNT' :"Ukupno uspjesno poslan grof",
            'HALT_FARMING' :"OsnaÅ¾eno Poljodjelstvo",
            'MACHINE_RUNNING' :"Farming Machine pokrenut",
            'CHANGE_VILLAGE' :"PokuÅ¡avanje promijene na drugo selo",
...
function checkreports()
{
	...
	{
		if(document.getElementsByTagName('a')[i].href.indexOf('dorf3') != -1)
		{
		  dorf3index = i;
		}
	}
