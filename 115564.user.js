...
// @name           E-Sim Advanced
// @namespace      eSim
// @description    e-sim advanced function and gattling
// @version        1.3
...
function es_show_battle() {
    ...
    if (es_resistance) {
        es_battle_bar = es_battle_bar + ' Fight for: <select id="es_fightfor"><option value="defender">Defender</option><option value="attacker">Attacker</option></select>';
    }
    es_battle_bar = es_battle_bar + ' Weapon: <select id="es_weapon"></select>'+
        ' Hit: <input type="text" id="es_hit" val="1" style="width:40px" />'+
        ' Timeout: <select id="es_timeout"><option value="500">0.5-1</option><option value="1000">1-1.5</option><option value="1500">1.5-2</option><option value="2000">2-2.5</option></select>'+
...
function es_init_battle() {
    ...
    this.init = function() {
        $this.battle_round_id = jQuery('#battleRoundId').val();
    ...
    this.monitor_battle = function() {
        ...
        jQuery.get('/battle.html', {'id':$this.battle_round_id}, function(data){
    ...
    this.on_hit = function() {
        ...
        $this.hit_run = true;
        var param_hit = {'side':'side', 'value':$($('.fightButton')[0]).val()};
        if ( $this.current_health > 40 && (parseInt(jQuery('#es_hit').val())-5>=$this.hit_count)) {
            param_hit.value = 'Berserk! (5 hits)';
        }
        param_hit.battleRoundId = $this.battle_round_id;
        ...
        jQuery.post('fight.html',param_hit,function(data) {
            ...
            if (param_hit.value!='Berserk! (5 hits)') {
                $this.hit_count += 1;
            } else {
                $this.hit_count += 5;
            }
