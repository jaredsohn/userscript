// ==UserScript==
// @name			wTorrent Adder
// @namespace			http://codingrockets.com/
// @description			Let's you download torrents using wTorrent, an rTorrent interface, remotely from your browser. This for users who have wtorrent at "address.tld/wtorrent" this works with Gazelle and BTSource.
// @include			*
// ==/UserScript==

/*
	Some lady did a lot of stuff.. but im to lazy to site it.
*/
...
(function() {
	...
	// wTorrent FQDN or IP
	var host = GM_getValue("wTorrent_FQDN","HOST") ; 
	// wTorrent Port (if not 80)
	var port = GM_getValue("wTorrent_Port","");
	// wTorrent Username
	var username = GM_getValue("wTorrent_User","USERNAME"); 
	// wTorrent Password
	var password = GM_getValue("wTorrent_Pass","PASSWORD");
	
	// init variables in firefox about:config 
	
	if (GM_getValue("wTorrent_FQDN") == undefined) {
		GM_setValue("wTorrent_FQDN","HOST");
		GM_setValue("wTorrent_Port","");
		GM_setValue("wTorrent_User","USERNSME");
		GM_setValue("wTorrent_Pass","PASSWORD");
	...
	function wTorrentMenu() { GM_openInTab("http://"+host+"/wtorrent/"); }
	...
	function wTorrentAdd(wTorrentLinkId) {
		...
		post('http://'+host+'/wtorrent/index.php?cls=AddT', 'torrenturl='+encodeURIComponent(wTorrentLink.href)+'&download_dir=/home/'+username+'/files/&start_now=on', function(s) {
			...
			// check for login page, and if we find the login we attempt to login
				...
				post('http://'+host+'/wtorrent/','userf='+username+'&passwdf='+password+'&user_login=Login', function(l) {
				//post('http://'+host+'/wtorrent/?userf='+username+'&passwdf='+password+'&user_login=Login', function(l) {
