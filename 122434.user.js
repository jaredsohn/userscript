...
var ScrobblerIcon = function(fm) {
	...
	if (location.href.endsWith('/audio') || location.href.endsWith('/search')) {
        //log_("audio == 0 || search == 0");
	...
	if (location.href.endsWith('/audio')) {
        //log_("audio == 0");
		document.getElementById('audio_summary').appendChild(this.scrobblerDiv);
		document.getElementById('audio_summary').appendChild(this.link);
	} else if (location.href.endsWith('/search')) {
        //log_("search == 0");
		document.getElementById('summary').appendChild(this.scrobblerDiv);
		document.getElementById('summary').appendChild(this.link);
	} else {
        //log_("no audio no search");
...
var PlayingIcon = function(track, track_id) {
    ...
    var song = jQuery('div#audio'+this.track_id);
              
    var songTags;
    this.isSearch = !(location.href.endsWith('/audio') || (location.href.indexOf('/audio?audio_id=') != -1) || (location.href.indexOf('/audio?album_id=') != -1) || (location.href.indexOf('/audio?id=') != -1) || (location.href.indexOf('/audio?act=recommendations') != -1));

    if (this.isSearch)
        songTags = song.find('td.info');

    else
        songTags = song.find('div.title_wrap');

	this.songSection = songTags.get(0);
...
PlayingIcon.prototype = {
	...
	start: function() {
		...
		this.songSection.appendChild(this.playingDiv);
...
var InfoPanel = function(track, id, isSearch) {
	this.track = track;
	this.track_id = id;
	...
	if (!infoDiv) {
        ...
        if (isSearch)
            p = document.getElementById('sections_filter');

        else
            p = document.getElementById('album_filters');
        //log_(p);
		
        if (!p) return;
		infoDiv = p.parentNode.appendChild(document.createElement('div'));
		infoDiv.id = 'lastfm_trackInfo';
		infoDiv.setAttribute('style', 'background: #f7f7f7; display: none; padding: 10px; margin-top: 1em; position: static; width: 156px; margin: 0 -10px;');
	}
...
var scrobbler = {
	...
	start: function(rid) {
        ...
        var song = jQuery('div#audio'+rid);
                  
        var songTags;
        this.isSearch = !(location.href.endsWith('/audio') ||
                          (location.href.indexOf('/audio?audio_id=') != -1) ||
                          (location.href.indexOf('/audio?album_id=') != -1) ||
			  (location.href.indexOf('/audio?act=recommendations') != -1) ||
                          (location.href.endsWith('/audio?act=edit')) ||
                          (location.href.indexOf('/audio?id=') != -1));
	    ...
	    len = song.find('input#audio_info'+rid).attr('value').split(',')[1];
	    // edit song feature
	    if (songTags.find('[id=performer'+rid+']').length)
        artist = songTags.find('[id=performer'+rid+']').get(0).textContent;
      
      else
        artist = songTags.find('a[href]').get(0).textContent;
	      
        if (this.isSearch)
    		title = songTags.find('span#title'+rid).text();

        else
    		title = songTags.find('span.title').text();

        //log_(songTags);
        log_('artist='+artist+' title='+title);
		...
		if ((/*location.href.endsWith('=audio') && */(location.href.indexOf('/search?c') != -1)) ||
		    !this.isSearch) {
            //log_('add info panel');
		
			new InfoPanel(track, this.cid, this.isSearch);
	    }
		track.start();
	},
	stop: function() {
    ...
    else
    {
	    ...
	    var showPlayerHandler = function(id) {
		    ...
		    var r = arguments.callee.janeth_original ?
			   ...
			   if (location.href.endsWith('audio?act=edit') && scrobbler.cid == id)
          return r;
