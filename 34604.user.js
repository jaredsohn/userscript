/********************** LICENSE NOTES ****************************
بحث ترافيان السريع - the light weight aplication for the game Travian (C)
...
******************************************************************/
// ==UserScript==
// @name           بحث ترافيان السريع
// @namespace      http://booboo.cwi.hu
// @description    تعديل وتعريب Y3GOB (ترافيان العربي).
// @version 1.07.1
// @author	Booboo & Y3GOB
// @copyright Booboo& Y3GOB
...
// @news - Warns if the saved data doesn't fit in a message 
// @news - The searching co-ordinates filled automatically to the active village's co-ordinates
// @news - It has got a more accurate sorting engine
// @news - Bug that sometimes truncated the saved datas, fixed till version 1.07
// @news - The remaining time counter is totaly rewritten
// @news - Better language support on 'Right to Left' servers
// @news - More talkative updater function :-)
// ==/UserScript==
...
var LD = new Array;
LD['STARTCOOR'] = decodeURIComponent(GM_getValue('STARTCOOR', 'بحث بالاحداثيات'));
LD['RADIUS'] = decodeURIComponent(GM_getValue('RADIUS', 'بحث نصف القطر:'));
LD['RAD_TITLE'] = decodeURIComponent(GM_getValue('RAD_TITLE', '0 to 56. Zero searches in the actual 7x7.'));
LD['SEARCHFOR'] = decodeURIComponent(GM_getValue('SEARCHFOR', 'بحث القرى'));
LD['OASIS'] = decodeURIComponent(GM_getValue('OASIS', 'الواحات'));
LD['SEARCH'] = decodeURIComponent(GM_getValue('SEARCH', 'يبحث'));
LD['SEARCH_BTN'] = decodeURIComponent(GM_getValue('SEARCH_BTN', 'بحث'));
LD['CLOSE'] = decodeURIComponent(GM_getValue('CLOSE', 'اغلاق'));
LD['LOAD_BTN'] = decodeURIComponent(GM_getValue('LOAD_BTN', 'جلب'));
LD['CH_LANG'] = decodeURIComponent(GM_getValue('CH_LANG', 'اختر اللغه'));
LD['PLAYER'] = decodeURIComponent(GM_getValue('PLAYER', 'الاعب:'));
LD['ALLIANCE'] = decodeURIComponent(GM_getValue('ALLIANCE', 'التحالف:'));
LD['AVAIL_LANGS'] = decodeURIComponent(GM_getValue('AVAIL_LANGS', 'اللغات المتوفرة:'));
LD['SAVE'] = decodeURIComponent(GM_getValue('SAVE', 'حفظ'));
LD['CANCEL'] = decodeURIComponent(GM_getValue('CANCEL', 'الغاء'));
LD['SERVER'] = decodeURIComponent(GM_getValue('SERVER', 'سيرفر:'));
LD['DATE'] = decodeURIComponent(GM_getValue('DATE', 'التاريخ:'));
LD['CENTER'] = decodeURIComponent(GM_getValue('CENTER', 'مركز البحث:'));
LD['HITS'] = decodeURIComponent(GM_getValue('HITS', 'عدد القراءة:'));
LD['NEWVER'] = decodeURIComponent(GM_getValue('NEWVER', "هناك نسخه احدث متوفرة."));
LD['UPDATENOW'] = decodeURIComponent(GM_getValue('UPDATENOW', 'تحديث الان?'));
LD['TRAV_TIME'] = decodeURIComponent(GM_getValue('TRAV_TIME', 'المسافة:'));
LD['NOHITS'] = decodeURIComponent(GM_getValue('NOHITS', 'لا توجد نتيجة.'));
LD['LOAD_TITLE'] = decodeURIComponent(GM_getValue('LOAD_TITLE', 'نسخ/حفظ: للمحافظة على بياناتك'));
LD['SAVE_TITLE'] = decodeURIComponent(GM_getValue('SAVE_TITLE', 'يمكن حفظ النص واغلاق النافذة'));
LD['ERROR_DATA'] = decodeURIComponent(GM_getValue('ERROR_DATA', "هناك خطأ فالبيانات."));
LD['IGM'] = decodeURIComponent(GM_getValue('IGM', 'ارسال كرسالة'));
LD['LOAD_CONFIRM'] = decodeURIComponent(GM_getValue('LOAD_CONFIRM', 'تستطيع استخدام هذه الرساله الان?'));
LD['TIME_REMAINS'] = decodeURIComponent(GM_getValue('TIME_REMAINS', 'الوقت المتبقي:'));
LD['DB_FORMAT'] = decodeURIComponent(GM_getValue('DB_FORMAT', 'قاعدة البيانات'));
...
var mainDiv = addElem("div", '<h2 align="center"><u>بحث ترافيان السريع</u></h2>'+
...
function stat(index, max)
{
	...
	$('TMA_divStat').innerHTML = '<p style="font-size:large; text-align:center;"><b>' + LD['SEARCH'] + FJ[counter] + parseInt(index / (max / 100)) + '%</b></p><p style="text-align:center"><b>' + LD['TIME_REMAINS'] + '</b> <font color="white">' + timeToString((max-index)*idoKoz.count()) + '</font></p>';
}
