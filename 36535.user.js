...
// @description   Adds links to Scopus searches to allow exporting of citations to CiteULike. Please read the "Message about ScopeULike" on the comment board.
...
if (window.location.href.match(/results\.url\?/)){
	...
	for (i=0; i<oI.length; i++){
		if (oI[i].width=="49%") {
			...
			newA[i].innerHTML = "[ScopeULike]";
			newA[i].href="http://www.citeulike.org/posturl?url="
						+ "year=" + escape(year[i])
						+ (issue[i]?"&issue=" + escape(issue[i]):"")
						+ (volume[i]?"&volume=" + escape(volume[i]):"")
						+ (pages[i]?"&pages=" + escape(pages[i]):"")
						+ "&authors=" + escape(authors[i])
						+ "&jour=" + escape(journal[i])
