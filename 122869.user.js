// ==UserScript==
// @name           Spy++
// @description    by Darth Brunus, edited by __Zx_and Captain Ron
// @version        1.4
...
// @include        http://uni2.playstarfleet.com/buildings/research_lab*
// @include        http://playstarfleet.com/buildings/research_lab*
// @include        http://uni2.playstarfleetextreme.com/buildings/research_lab*
// @include        http://playstarfleetextreme.com/buildings/research_lab*
// ==/UserScript==
...
function run() {
	...
	if(fullURL.match(/(research_lab)/)){
		var locIDnum = document.getElementById("locations_table").innerHTML.match(/id="location_(\d*)"/g)[2].replace(/[^0-9]/g, "");
		var wepTechID = document.getElementById("location_" + locIDnum);
		var wepTech_amount = wepTechID.getElementsByClassName("amount")[0];
		var wepTech_level = wepTech_amount.innerHTML.trim();
		
		localStorage.setItem("statscript.research.Weapons Tech", wepTech_level);
	}
	
	if(fullURL.match(/(messages)/)){
...
function Defense() {
	...
	this.largeDecoy = 0;
	this.plasmaCannon = 0;
	this.missileBattNuke = 0;
	this.laserCannonNuke = 0;
	this.pulseCannonNuke = 0;
	this.particleCannonNuke = 0;
	this.decoyNuke = 0;
	this.gaussCannonNuke = 0;
	this.largeDecoyNuke = 0;
	this.plasmaCannonNuke = 0;
	this.totalNuke = 0;
...
function EspionageReport(msgObj) {
	...
	this.parse = function() {
		...
		var m = esp.match(/(Hephaestus Class Attack Platform|Planet|Moon|Encounter) ([^\[]+)\[(\d+):(\d+):(\d+)(\w?)\]/);
		...
		if(this.fleetsCount > 0) {
			...
			if(esp.match(/DEFENSES:/) != null) {
				...
				{
					var MyWeaponTech=(localStorage.getItem("statscript.research.Weapons Tech")==null) ? 14 : parseInt(localStorage.getItem("statscript.research.Weapons Tech"));  
					var armorBonus = 1 + (0.1 * this.armorTech);
					var nukeAttack = (1 + (0.1 * MyWeaponTech)) * 12000;
					this.defense.missileBattNuke = Math.ceil((this.defense.missileBatt * 200 * armorBonus) / nukeAttack);
					this.defense.laserCannonNuke = Math.ceil((this.defense.laserCannon * 200 * armorBonus) / nukeAttack);
					this.defense.pulseCannonNuke = Math.ceil((this.defense.pulseCannon * 800 * armorBonus) / nukeAttack);
					this.defense.particleCannonNuke = Math.ceil((this.defense.particleCannon * 800 * armorBonus) / nukeAttack);
					this.defense.decoyNuke = Math.ceil((this.defense.decoy * 2000 * armorBonus) / nukeAttack);
					this.defense.gaussCannonNuke = Math.ceil((this.defense.gaussCannon * 3500 * armorBonus) / nukeAttack);
					this.defense.largeDecoyNuke = Math.ceil((this.defense.largeDecoy * 10000 * armorBonus) / nukeAttack);
					this.defense.plasmaCannonNuke = Math.ceil((this.defense.plasmaCannon * 10000 * armorBonus) / nukeAttack);
					
					this.defense.totalNuke = Math.ceil( ( ((this.defense.missileBatt * 200) + (this.defense.laserCannon * 200) + 
					(this.defense.pulseCannon * 800) + (this.defense.particleCannon * 800) + (this.defense.decoy * 2000) + 
					(this.defense.gaussCannon * 3500) + (this.defense.largeDecoy * 10000) + (this.defense.plasmaCannon * 10000)) * armorBonus) / nukeAttack ) + this.defense.abm;
	...
	this.parseDefense = function(armorTech) {
	...
	this.getPlusReport = function() {
		...
		s += "<a href='" + this.msg.coordsLink + "'>[" + this.coords.galaxy + ":" + this.coords.system + ":" + this.coords.slot + (this.type == "Moon" ? "m" : (this.type == "Encounter" ? "e" : "" )) + "]</a><br>";
		...
		var boldRed = "<span style='color: #FF4040; font-weight: bold'>"
		var endBoldRed = "</span>"
...
//		if(this.hasDefenses && this.type != "Encounter") {
		if(this.hasDefenses) {
			s += "DEFENSES: (";
			s += "<span style='color: #FF4040; font-weight: bold'>";
			s += "&nbsp;Total Nukes: " + this.defense.totalNuke + "</span>&nbsp;):<br>";
			if (this.defense.missileBatt > 0 && this.defense.missileBatt > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.missileBattNuke + "</span>) ";				
			}
			if (this.defense.laserCannon > 0 && this.defense.laserCannon > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.laserCannonNuke + "</span>) ";
			}
			if (this.defense.pulseCannon > 0 && this.defense.pulseCannon > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.pulseCannonNuke + "</span>) ";
			}
			if (this.defense.particleCannon > 0 && this.defense.particleCannon > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.particleCannonNuke + "</span>) ";
			}
			if (this.defense.decoy > 0 && this.defense.decoy > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.decoyNuke + "</span>) ";
			}
			if (this.defense.gaussCannon > 0 && this.defense.gaussCannon > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.gaussCannonNuke + "</span>) ";
			}
			if (this.defense.largeDecoy > 0 && this.defense.largeDecoy > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.largeDecoyNuke + "</span>) ";
			}
			if (this.defense.plasmaCannon > 0 && this.defense.plasmaCannon > 0){
				...
				if (this.type != "Encounter")
					s += "(<span style='color: #FF4040; font-weight: bold'>" + this.defense.plasmaCannonNuke + "</span>) ";
			}
	...
	this.getPlusSender = function() {
		...
		s+= this.planetName + " [" + this.coords.galaxy + ":" + this.coords.system + ":" + this.coords.slot + (this.type == "Moon" ? "m" : (this.type == "Encounter" ? "e" : "" )) + "]";
		return s;
	}
};
