...
(function() {
	...
	for(var i=listePosts.length-1; i>=0; i--) {
		...
		if(!censure && (substr_count(listePosts[i].textContent, 'P')>20 || /(( ?<br( \/)?>( |\n)?){25})|([W ]{50,})/.test(listePosts[i].innerHTML))) {
			listePosts[i].style.display = 'none';
			listePosts[i].previousElementSibling.previousElementSibling.innerHTML += '<em>Campute</em>';
			censure = true;
		}
