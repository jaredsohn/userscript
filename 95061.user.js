// ==UserScript=='
// @name           IMDB - Add FilmWeb rating
// @namespace      http://userscripts.org/users/27225
// @description    Retreives filmweb rating for the given movie/tv series
// @version	  	   2.2
// @include        http://www.imdb.com/title/*
// @include        http://*imdb.com/title/*
// @require			https://ajax.googleapis.com/ajax/libs/jquery/1.7.0/jquery.min.js
// ==/UserScript==

if(unsafeWindow.console){
   var glog = unsafeWindow.console.log;
} 

if (window.top != window.self)  //don't run on frames or iframes
    return;

	
	
//WHICH IMDB VERSION IS IT?======================================================================
if (document.getElementById('tn15') != null) {
	oldImdbLayout = true;
}
else {
	oldImdbLayout = false;
}

findPattern = "//table[@class='probody']";
results = document.evaluate( findPattern, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null );

if (results.snapshotItem(0) != null)
{
	proImdbLayout = true;
}
else {
	proImdbLayout = false;
}
//==================================================================================================

var fwIcon = 'data:image/png;base64,';
var docTit = document.title
var title = escape(removeQuotes(docTit.substring(0, docTit.indexOf(' ('))));
//get the year from the title
var year;
if(docTit.search('/') != -1)
	year = docTit.substring(docTit.indexOf('(') + 1, docTit.indexOf('/'));
else
	year = docTit.substring(docTit.indexOf('(') + 1, docTit.indexOf(')'));

//is this a movie or a TV series?
if($(".tv-extra, .tv-series-smaller").length > 0)
	var type = 'serial';
else
	var type = 'film';

var fwSearchURL = 'http://www.filmweb.pl/search/'+type+'?q='+title+'&startYear='+(parseInt(year) - 1)+'&endYear='+year+'&startRate=&endRate=&startCount=&endCount=&sort=TEXT_SCORE&sortAscending=false';
addFWElement();
requestFWPage(fwSearchURL);

function addFWElement(){
	if (oldImdbLayout == true) {
		labelHtml = '<h5>Filmweb:</h5>';
		insertedDivClass = 'info';
		resultsPrefix = '<div class="info-content">';
		resultsSuffix = '</div>';
	}
	else {

		if (proImdbLayout == true) {
			labelHtml = '<h5 style="font-size:90%;">Filmweb:</h5>';
			insertedDivClass = 'info';
			resultsPrefix = '<div class="info-content">';
			resultsSuffix = '</div>';
		}
		else {
			labelHtml = 'Filmweb:';
			insertedDivClass = '';
			resultsPrefix = '';
			resultsSuffix = '';
		}

	}
	
	//create filmweb element
	var fwElement = document.createElement('div')
	fwElement.setAttribute('class',insertedDivClass);
	var loadingIcon = 'data:image/gif;base64,';
	var RTStyle = 'style=""';
	fwElement.innerHTML = '<div id="filmweb">'+labelHtml+resultsPrefix+' sprawdzam <img src="'+loadingIcon+'" alt ="checking">'+resultsSuffix+'</div>';
	
	//add it to the page
	var insertSelector = "div.star-box";

	if (oldImdbLayout == true) {
		insertSelector = "[class='info']";
	}

	if (proImdbLayout == true) {
		insertSelector = "table.probody";
	}
	$($(insertSelector).get(0)).append(fwElement);
}


function requestFWPage(the_url) {
	GM_xmlhttpRequest({
		url: the_url,
		method: 'GET',
		onload: function(response) {
			
			var html = response.responseText;
			var fwElement = document.getElementById('filmweb');
			
			if(html.search('goToLink') != -1){ //the add welcome screen is returned. Do the request again.
				requestFWPage(the_url);
				return;
			}
			var search_results = $(html).find('.sep-hr.resultsList');
			if(search_results.length > 0){
				var movieLink = 'http://www.filmweb.pl'+search_results.first().find("a.hdr.hdr-medium").first().attr('href');
				requestFWPage(movieLink);
			}
			else if($(html).find("strong[rel='v:rating']").length > 0){
				var rating = getRating(html,the_url);
				var votes = getVotes(html);
				var yourRating = getYourRating(html);
				var yourRating = ( yourRating != -1)?(" <strong>Twoja ocena:</strong> "+yourRating):"";
				fwElement.innerHTML = labelHtml+resultsPrefix+'<strong>Ocena:</strong> '+rating+' <strong>głosów:</strong> '+votes+yourRating+'&nbsp;&nbsp;<a target="_blank" href="'+the_url+'"><img width="16" height="16" src="'+fwIcon+'" /></a>'+resultsSuffix;
			}
			else
				fwElement.innerHTML = labelHtml+resultsPrefix+'Nie znaleziono filmu&nbsp;&nbsp;<a target="_blank" href="'+the_url+'"><img width="16" height="16" src="'+fwIcon+'" /></a>'+resultsSuffix;
			
			
			
		}
	});
}

function getRating(html,url){
	return $(html).find("strong[rel='v:rating']").text();
}

function getYourRating(html){
	var $currentVoteLabel = $(html).find(".gwt-currentVoteLabel");
	if($currentVoteLabel.length > 0)
	{
		if($currentVoteLabel.get(0).innerHTML.search('moja ocena to:') == -1)
			return $currentVoteLabel.get(0).innerHTML;
		else
			return -1;
	}
	else
		return -1;
	
}

function getVotes(html){
	return $(html).find("[property='v:votes']").get(0).innerHTML;
}

//function to remove double quotes from the beginning and end of a string
function removeQuotes(text){
	if(text.match("^"+'"')=='"')
		text = text.substring(1);
	if(text.match('"'+"$")=='"')
		text = text.substring(0,text.length - 1);
	return text;
}