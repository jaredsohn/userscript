...

{
				...
				btn.addListener("click", function(){
					...
					this.expImpWin.getUserData("ta").setValue("{\"LTOptions\":"+qx.util.Json.stringify(LT.options)+",\"TurboOptions\":"+localStorage.getItem("LoUTweakTurboData")+"}");
					this.expImpWin.open();
				}, this);
				cont.add(btn);
					...
					btn.addListener("click", function(){
						...
						if (id == 1) {
							...
							if (typeof obj == "object" && obj != null) {
								tmp=qx.util.Json.parse(txt);
								if (tmp.TurboOptions && tmp.LTOptions) {
									localStorage.setItem("LoUTweakTurboData", qx.util.Json.stringify(tmp.TurboOptions));
									LT.options = tmp.LTOptions;
									localStorage.setItem("LT_options", qx.util.Json.stringify(tmp.LTOptions));
								} else {
									LT.options = tmp;
