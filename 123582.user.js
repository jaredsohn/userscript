...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  mainPop = new CPopup ('pb', Options.pbWinPos.x, Options.pbWinPos.y, 1005,1005, Options.pbWinDrag, 
      function (){
        tabManager.hideTab();
...
Tabs.build = {
    ...
    doOne : function (bQi){ 
		...
		//mat/ Power : 49 @ 19:41:45.274: Pos: 6 Type: 13 Level: 8 Id: 1523749
...
//				13:{'name':"-------",'x':375,'y':375},
  ...
  dispMapTable : function (){
    ...
    if (dat.length == 0){
      ...
      for (i=0; i<numRows; i++){
        m += '<TR><TD><DIV onclick="ptGotoMapHide('+ dat[i][0] +','+ dat[i][1] +')"><A>'+ dat[i][0] +','+ dat[i][1] +'</a></div></td>';
        if (coordsOnly) {
          m += '</tr>';
        } else {
...
Tabs.sample = {
  tabOrder : 75,                    // order to place tab in top bar
...
Tabs.trader = {
  tabOrder: 32,
  tabLabel: 'Trader',
  tabDisabled : !ENABLE_TRADER_TAB,         // if true, tab will not be added or initialized
  myDiv: null,
  timer: null,
  traderState: [],
  lTR: [],
...
Tabs.Gifts = {
  ...
  init : function (div){
    ...
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD width=200></td><TD align=center><INPUT id="pasubGifts" type=submit value="Check for Gifts" \></td><TD width=200 align=right><INPUT id=paGiftHelp type=submit value=HELP></td></tr></table><HR>\
        <DIV id=giftDiv style="width:150%; min-height:300px; height:100%">';
