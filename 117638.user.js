...
var AppInfo = {
    ...
    meta     : 'http://userscripts.org/scripts/source/111889.meta.js'
};
...
var Tooltips = {
    ...
    _Resources: {
        'en_US': {
            ...
            description: 'English Tooltip v1.3<br><br>Created by <a href="http://userscripts.org/users/250944" target="_black">dakam</a>.',
            content: {
                // CONFIGURATION
                'config_popup': {
...
var MainMenu = {
    ...
    init: function() {
        ...
        .html('<fb:like href="http://www.facebook.com/pages/Legend-Arena-LIS-/147866151891838" '
...
function FanPage() {
unsafeWindow.open('http://www.facebook.com/pages/Legend-Arena-LIS-/147866151891838?sk=app_106878476015645');
}
function unframe() {
    unsafeWindow.open('http://facebook.mafiawars.zynga.com/mwfb/index.php?skip_req_frame=1&mwcom=1');
}
function EMI() {
unsafeWindow.open('http://www.facebook.com/pages/New-Recruitment-Centre-Legend-Indo-Spartan-LIS/159519067444309');
}
        ...
        function parseAllResponses(responseCache) {
            ...
            if ( attack_again ) {
                ...
                if ( lastGoodResult.success !== ERROR_SUCCESS ) {
                    Logger.debug('User '+PlayerList.current.name+' skipped. Reason: All attacks failed.')
                    attack_again = false;
                } else {
...
function FreeGiftsCenter()
{
    ...
    function getSendBackRequest(text) {
        var index, request = {};
        if (text && (index = text.indexOf('sendGiftbackRequest')) > -1) {
            try {
	            eval(text.substr(index));
            } catch (e) {
    ...
    function parseStatus(data, limit_time) {
        ...
        Util.each(data, function(index, status) {
...
function PageGift()
{
    ...
    var Events = {
        ...
        publishAll_click: function() {
            ...
            getLinks(false, function(c) {
                ...
                if (options.persLinks) {
                    getPersonalLinks(function(p) { 
                        if (options.persLinksInBody) {
                            message = '';
                            Util.each(p, function(i, g) {
                                message += g.name + ' => ' + g.short_url + '\n';
                            });
                        }
                        else {
                            c = p.concat(c);
    ...
    function postPublish(result) {
        if (result && !result.error) {
            ...
            showHelpPopup({
                ...
                +        '<br><br>Do you want to reset your links and try again?',
                buttons: [{
                    label: 'Yes',
                    addClass: 'short white',
                    onclick: function() {
                        resetLinks(true);
                        Events.publishAll_click();
                    }
                }, {label: 'Close'}]
            });
        }
    ...
    function publishLinks(gifts, target_id, message) {
        ...
        facebook.streamPublish(attachment, postPublish);
    }
    ...
    function Initialize() {
        ...
        .append(x$('gpopt_perslinks', 'Include my personal links'))
        .append(c$('a').text('(Open "My Links" to configure)').click(UserLinks).css('margin-left',5))
        .append(c$('div').css({'clear':'both','margin-top':3}))
        .append(x$('gpopt_perslinksinbody', 'Personal links in message body (facebook can block it).'))
        .append(c$('div').css({'clear':'both','margin-top':3}))
        .append(x$('gpopt_hidegroups', 'Don\'t group my links by categories.'))
        .append(c$('div').css({'clear':'both','margin-top':8}))
        .append(s$('myselections', 'My Selection', 200))
        .append(c$('span').text('|').css('margin','0px 10px 0px 10px'))
        .append(c$('a').text('Create').click(Events.saveSelection_click))
        .append(c$('span').text('|').css('margin','0px 10px 0px 10px'))
        .append(c$('a').text('Update').click(Events.updateSelection_click))
        .append(c$('span').text('|').css('margin','0px 10px 0px 10px'))
        .append(c$('a').text('Delete').click(Events.remSelection_click));

        $('#myselections').change(Events.selection_click);

        // Add buttons
        freegift_buttons = $('.freegift_buttons').first().empty().css('min-height',32);
        b$('Short Links','name:showShortLinks,class:medium white sexy_send_gift_new')
          .appendTo(freegift_buttons).click(Events.shortLinks_click).css('margin-right',10);
        b$('Publish All','name:publishLinks,class:medium white sexy_announce_gray')
