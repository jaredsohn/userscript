// ==UserScript==
// @name            NewCherrybelle (Stormfall Power Tools - by CF)
// @namespace       www.wrob.info/aow/js/aow_tool.user.js
// @icon            http://img66.xooimage.com/files/3/6/d/jaws64-2b5041c.png
// @description     Enhanced Power Tools for Stormfall, by CF
// @grant           GM_xmlhttpRequest
// @match           *https://apps.facebook.com/stormfall/*
// @match           *://apps.facebook.com/stormfall/*
// @include         *https://apps.facebook.com/stormfall/*
// @include         *://apps.facebook.com/stormfall/*
// @version         2013.0801
// @changeLog		Correction d'un bug + changement n° version client
// ==/UserScript==
...
var kFatalSWF	= '"<B>Error \'initialization :</b> <BR><BR>Impossible de toruver l\'élément SWF"';
...
var unit_groups = [ {id:0, desc:'Infantry'},
					{id:1, desc:'Imperial Infantry'},
					{id:4, desc:'Cavalry'},
					{id:5, desc:'Imperial Cavalry'},
					{id:2, desc:'Ocult Units'},
					{id:3, desc:'Imperial Ocult Units'},
					{id:6, desc:'Bestiary'},
					{id:7, desc:'Imperial Bestiary'},
					{id:8, desc:'Castle Guards'} ];
	...
	try {
		...
		Data.init({
			...
			options	: {
				...
				wikiUrl			: 'http://wiki.plarium.com/index.php?title=Stormfall%3A_Age_of_War_Wiki',
				contacts	: {
					current_tab		: 0,
					detailled_view	: true,
					hide_created	: false,
		...
		actionLog('<B>' + scriptVersion + ' (client v' + client_ver + ') Loading ...</B>');
		consoleLog('<B>' + scriptVersion + ' (client v' + client_ver + ') Loading ...</B>');
		stepStarting( 1 );
			...
			for (var id in t.marches) {
				...
				if (!isNaN(time_left) && time_left > 0 && !march.cancelled) {
					...
					switch (march.target_type) {
						...
						case 'beacon'	 : type1 = ' The Beacon'; type2 = 'Ton phare '; break;
						case 'location'	 : type1 = ' The\'Location'; type2 = 'Ton implantation '; break;
						default : type1 = ' le château'; type2 = 'Your Castle '; break;
					}
					if (march.own_march) {
						switch (march.type) {
							...
							default : status = 'DEFEND The forces'; status2 = ''; break;
						}
...
Tabs.Contacts = {
	...
	tabTargetList : function (){
		...
		var m = '<div id=tabTargets_Results style="height:640px">'
			+'	<div class=bsx_title>List of potential targets</div>'
			+'	<table class=bsx_table style="margin-top:3px" width=100%>'
			+'		<tr valign=top align=left>'
		...
		function display_detailled (){
			...
			+'		<td id=tabTargets_tal_3 width="130px"><A><span class=bold_white>Experience</span></A></td>'
			+'	</tr>';
...
Tabs.Radar = {
	...
	init : function (div){
		...
		var m = '<div class=bsx_title>Nest\'eagle</div>'
			...
			+'	<li class="tab"><a id=tabRadarMyMines>Mines</a></li>'
			+'</ul>'
...
Tabs.Options = {
	...
	init : function (div) {
		...
		try {
			...
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabContacts type=checkbox '+ (Data.options.disable_contacts?'CHECKED ':'') +' /> deactivate the...<B>Contacts</B></label></td>'
			...
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabAlliance type=checkbox '+ (Data.options.disable_alliance?'CHECKED ':'') +' /> deactivate the...<B>Alliance</B></label></td>'
			+'	</tr><tr valign=top>'
			+'		<td><label><input id=tabOptions_TabLog type=checkbox '+ (Data.options.disable_log?'CHECKED ':'') +' /> deactivate the...<B>Log</B></label></td>'
			+'		<td>&nbsp;</td>'
			+'	</tr>'
			+'</table><br>'
			...
			+'		<td width=30%><label><input id=tabOptions_btnClean type=button value="Remove" title="Clear data from local storage (internet cache)" /></label></td>'
			+'		<td width=40%><label><input id=tabOptions_btnInspect type=button value="Visualiser" title="Inspect the cached data" /></label></td>'
...
function UnitSendDialog (curtainDiv, order_id, owner_id, target_id, troops, allowClose, notifyClose){
	...
	if (this_id >= 0) {
		...
		if (!is_null(note)) {
			var dist = getDistance(Seed.gameData.account.x, Seed.gameData.account.y, note.x, note.y);
			var fbid = (note.social_id ? (note.social_id.indexOf("fb")>=0 ? note.social_id.substr(2) : note.social_id.fbid) : '');
			name = note.fullname+' (Level '+note.level+')';
			...
			if (!is_null(note)) {
				...
				var dist = getDistance(Seed.gameData.account.x, Seed.gameData.account.y, note.x, note.y);
				var req = Seed.statics.raid_types[note.type];
				name = req.name+' (Level '+note.level+')';
				coords = 'X:'+note.x+', Y:'+note.y+' (Dist '+dist+' Km)';
				img = vignette2('ui/globalMap/raids/rl'+(note.type < 10 ? '0' : '')+ note.type+'.jpg');
			}
		}
...
function dispError (msg,target){
	...
	dial.getTopDiv().innerHTML = '<div class=title><center><b>Cherrybelle : Error</b></center></div>';
...
var AutoUpdater = {
	days	: 1,
	name	: "Cherrybelle (Stormfall Power Tools - by CF)",
...
var progressBar = {
	...
	init : function (x, y, width, height, title, bar_width, modal, container){
		...
		progressBarPop.getTopDiv().innerHTML = '<div class=bsx_title><center><b>Cherrybelle : ' + title + '</b></center></div>';
		var layoutDiv = document.createElement('div');
		layoutDiv.className = 'bsx_container';
		layoutDiv.style.backgroundColor = 'rgb(245,245,228)';
 		layoutDiv.style.color = '#000';
...
progressBar.start({ steps:1, delay:initialDelay, title:scriptName +' - v'+ scriptVersion + ' (client v' + client_ver + ') : Starting ...', stepText:'Delay before initial startup script' });
