...
// @namespace       heroeswm
// @name            HWM Set
// @description     Позволяет оперировать наборами (навыков, армий, артефактов).
// @version         1.13.2
// @grant           GM_setValue
// @grant           GM_getValue
...
SkillWheel.parseDoc = function(doc) {
    ...
    try {
        ...
        for (var i = 0; i < arr1.length - 1; i++) {
            ...
            //if (skill.current)
            //    GreaseMonkey.log(SkillWheel.textSkill(skill));
...
SkillWheel.htmlSet = function(frac, skills, skillSet, name, width, height) {
    ...
    */
    ...
    for (var i = 0; i < skillSet.length; i++) {
        ...
        //GreaseMonkey.log(skill.name + ' -> ' + groupName);
...
if (window === top) {
    ...
    var desc = {
        'details':
              '<ul>'
            + '<li>Для управления наборами навыков следует находиться на странице задания навыков.</li>'
            + '<li>Для управления наборами существ следует находиться на странице набора существ.</li>'
            + '<li>Для управления наборами артефактов следует находиться на странице инвентаря.</li>'
            + '</ul>',
    ...
    var conf = {
        ...
        'popup': {'type': 'bool', 'title': 'Сообщения', 'def': true, 'desc': 'Отображение сообщений после смены набора.'},
        'parsePlayerInfo': {'type': 'bool', 'title': 'Парсинг страниц персонажей', 'def': false, 'desc': 'Извлечение со страниц описания персонажей информации о существах.'},
        'armySetImages': {'type': 'bool', 'title': 'Набор существ картинками', 'def': true, 'desc': 'Отображение наборов существ картинками.'},
        'skillSetImages': {'type': 'bool', 'title': 'Набор навыков картинками', 'def': true, 'desc': 'Отображение наборов навыков картинками.'},
        'skillImageSize': {'title': 'Размер картинки навыка', 'def': 60, 'desc': 'Высота картинки навыка (60 - высота картинки на странице информации о персонаже).'},
        'invSetImages': {'type': 'bool', 'title': 'Набор артефактов картинками', 'def': true, 'desc': 'Отображение наборов артефактов картинками.'},
        //'invImageSize': {'title': 'Размер картинки артефакта', 'def': 50, 'desc': 'Высота картинки артефакта (50 - высота картинки на странице информации о персонаже).'},
    }
...
if ((!disabled) && (window === top)) {
    ...
    if (url.attr('path') == '/home.php') {
        ...
        GM_setValue('fractionCode', home.playerFracId);
        
    }
    
    
    // На странице набора армии
    if (url.attr('path') == '/army.php') {
        
        var fractionCode = Number(GM_getValue('fractionCode', 0));
        //GreaseMonkey.log('fractionCode == ' + fractionCode);
        if (fractionCode > 0) {
        
            // Парсим документ
            var stacks = Army.parseDoc(document);
            //GreaseMonkey.log('stacks == ' + '\n' + String.fromObjects(stacks));
            if (stacks !== false) {
                
                // Формируем пользовательский интерфейс
                
                /** Обновление окна набора навыков
                */
                function updateWindowArmy() {
                
                    /** Возаращает название набора
                    * @param Element button
                    * @return String
                    */
                    function getSetName(button) {
                        var row = button.parentNode.parentNode;
                        var b = row.previousSibling.getElementsByTagName('b')[0];
                        if (b !== undefined)
                            return b.textContent;
                        return row.previousSibling.previousSibling.textContent;
                    }
                    
                    // Сохраненные наборы
                    var sets = Army.listSets(fractionCode);
                    
                    // Кнопки управления набором
                    var buttons =
                        "<tr class='buttons'>" +
                        "<td title='Применить данный набор для героя.'><input class='use' type='button' value='Применить'/></td>" + 
                        "<td title='Удалить данный набор.'><input class='del' type='button' value='Удалить'/></td>" + 
                        //"<td title='Сохранить текущий набор в данный.'><input class='save' type='button' value='Сохранить'/></td>" + 
                        "</tr>";                
                    
                    var armySetImages = GM_getValue('armySetImages', true);
                    if (armySetImages) {
                        
                        // Отображение наборов картинками
                        var table = '';
                        for (var i = 0; i < sets.length; i++)
                            table += 
                                "<tr><th colspan='3'>" + sets[i] + "</th></tr>" + 
                                "<tr><td colspan='3'>" + Army.htmlSet(fractionCode, stacks, undefined, sets[i], '100%', undefined) + "</td></tr>" + 
                                buttons;
                 
                    } else {
                        
                        // Отображение наборов текстом
                        var table = '';
                        for (var i = 0; i < sets.length; i++)
                            table += 
                                "<tr><td colspan='3'><b>" + sets[i] + "</b>: " + Army.textSet(fractionCode, stacks, undefined, sets[i]) + "</td></tr>" + 
                                buttons;
                        
                    }
                    
                    $('.Body', winShow).html('<table>' + table + '</table>');
                    $('.Body', winShow).css({'max-height': '480px', 'overflow': 'auto'});
                    $('.Body table', winShow).css({'font-size': '12px', 'color': '#000000', 'width': '100%'});
                    $('.Body table td', winShow).css({'color': '#000000'});
                    if (armySetImages) {
                        $('.Body table th', winShow).css({'text-align': 'center'});
                        $('.Body table td', winShow).css({'text-align': 'center'});
                    }
                    $('.Body table td.buttons', winShow).css({'text-align': 'right'});
                    $('.Body input', winShow).css({'width': '80px', 'border': 'solid 1px #999999', 'margin': '2px', 'padding': '1px', 'background': '#cccccc', 'font-size': '12px', 'cursor': 'pointer', 'margin-left': '10px'});
                    $('.Body input.use').click( function(){ Army.useSet(document, fractionCode, undefined, getSetName(this), function(){ document.location.reload(); }); } );
                    $('.Body input.del').click( function(){ Army.deleteSet(fractionCode, getSetName(this)); updateWindowArmy(); } );
                    //$('.Body input.save').click( function(){ Army.saveSet(fractionCode, getSetName(this), skillSet); updateWindowArmy(); } );
                    
                }
            
                /** Переключение окна набора
                */
                function toggleWindowArmy() {
                    updateWindowArmy();
                    gui.toggleWindow([gui.scriptName, 'ArmyShow']);
                }
                
                /** Парсинг формулы состава армии
                */
                function parseArmy(text) {
                    ...
                    for (var i = 0; i < arr.length; i++)
                        if (arr[i].length > 0)
                            set[set.length] = parseInt(arr[i]);
                    GreaseMonkey.log('Задан набор существ:' + '\n' + Army.textSet(fractionCode, stacks, set));
                    return set;
                }
                
                // Окно текущих наборов
                var winShow = gui.createWindow([gui.scriptName, 'ArmyShow'], 'Наборы существ');
                var bClose = this.createWindowButton([gui.scriptName, 'ArmyShow', 'Close'], 'Закрыть');
                bClose.click( function(){ toggleWindowArmy(); });
                
                // Окно нового набора
                var winSave = gui.createWindow([gui.scriptName, 'ArmySave'], 'Сохранить набор существ');
                $('.Body', winSave).html("<input type='text' name='name' value='Новый набор'/><br/><input type='text' name='value' value='0-0' title='Формула состава армии: количества существо по уровням от 1 до 7 (без пропусков, с любым разедлителем).'/>");
                $('.Body', winSave).css({'text-align': 'center'});
                $(".Body input", winSave).css({'width': '80%', 'border': 'solid 1px #999999', 'margin-top': '5px'});
                var bSave = this.createWindowButton([gui.scriptName, 'ArmySave', 'Save'], 'Сохранить');
                bSave.click( function(){ 
                    Army.saveSet(
                        fractionCode,
                        $(".Body input[name='name']", winSave).val(),
                        parseArmy($(".Body input[name='value']", winSave).val())
                    );
                    gui.toggleWindow([gui.scriptName, 'ArmySave']); 
                });
                $("input", winSave).keydown( function(e) {var code = (e.keyCode ? e.keyCode : e.which); if (code == 13) { bSave.click(); }} );
                var bClose = this.createWindowButton([gui.scriptName, 'ArmySave', 'Close'], 'Закрыть');
                bClose.click( function(){ gui.toggleWindow([gui.scriptName, 'ArmySave']); } );
                
                // Кнопки меню
                var bAccess = gui.createMenuButton(['Army'], 'Армия', 'Доступ к функциям наборов существ.');
                var bShow = gui.createMenuButton(['Army', 'ArmyShow'], 'Показать', 'Показать сохраненные наборы существ.');
                bShow.click(function() { toggleWindowArmy(); gui.toggleMenu(); });
                var bNew = gui.createMenuButton(['Army', 'ArmySave'], 'Новый', 'Сохранить новый набор существ.');
                bNew.click(function() { gui.toggleWindow([gui.scriptName, 'ArmySave']); gui.toggleMenu(); $(".Body input[name='name']", winSave).focus(); });
        ...
        if (fractionCode > 0) {
            ...
            /** Создание кнопки применения набора существ
            * @param String name
            */
            function createUseArmyButton(name) {
                var bArmy = gui.createMenuButton(['Army', 'Use' + i], name);
                bArmy.click(function() {
                    Army.useSet(document, fractionCode, undefined, name, 
                        function() {
                            GreaseMonkey.log('Установлена армия: ' + name + '.');
                            var popup = GM_getValue('popup', true);
                            if (popup) {tgui.setPopupSet('ArmySet', 'Установлена армия: <b>' + name + '</b>');}
                        }
                    );
                    gui.toggleMenu(); 
                });
            ...
            // Кнопки меню
            var bArmies = gui.createMenuButton(['Army'], 'Армия', 'Переключение наборов существ.');
            var sets = Army.listSets(fractionCode);
            for (var i = 0; i < sets.length; i++)
                createUseArmyButton(sets[i]);
    ...
    // На странице колеса навыков
    if (url.attr('path') == '/skillwheel.php') {
        
        // Парсим документ
        var skillsData = SkillWheel.parseDoc(document);
        if (skillsData !== false) {
            
            //GM_setValue('fractionCode', skillsData.fractionCode);
            var fractionCode = skillsData.fractionCode;
            var skills = skillsData.skills;
            var str = String.fromObjects(skills, [], false, ['desc', 'required']);
            //GreaseMonkey.log('Доступные навыки:' + '\n' + str);
            
            var skillSet = SkillWheel.skillSet(skills);
            var str = String.fromColumns([skillSet]);
            //GreaseMonkey.log('Текущие навыки:' + '\n' + str);
            
            // Формируем пользовательский интерфейс
            
            /** Обновление окна набора навыков
            */
            function updateWindowSkills() {
                ...
                */
                ...
                // Сохраненные наборы
                var sets = SkillWheel.listSets(fractionCode);
                ...
                var skillSetImages = GM_getValue('skillSetImages', true);
                if (skillSetImages) {
                    
                    var size = GM_getValue('skillImageSize', 60);
                    // Отображение наборов картинками
                    var table = '';
                    for (var i = 0; i < sets.length; i++)
                        table += 
                            "<tr><th colspan='3'>" + sets[i] + "</th></tr>" + 
                            "<tr><td colspan='3'>" + SkillWheel.htmlSet(fractionCode, skills, undefined, sets[i], undefined, size) + "</td></tr>" + 
                            buttons;
                    ...
                    for (var i = 0; i < sets.length; i++)
                        table += 
                            "<tr><td colspan='3'><b>" + sets[i] + "</b>: " + SkillWheel.textSet(fractionCode, skills, undefined, sets[i]) + "</td></tr>" + 
                            buttons;
                ...
                $('.Body table td.buttons', winShow).css({'text-align': 'right'});
                $('.Body input', winShow).css({'width': '80px', 'border': 'solid 1px #999999', 'margin': '2px', 'padding': '1px', 'background': '#cccccc', 'font-size': '12px', 'cursor': 'pointer', 'margin-left': '10px'});
                $('.Body input.use').click( function(){ SkillWheel.useSet(document, fractionCode, undefined, getSetName(this), function(){ document.location.reload(); }); } );
                $('.Body input.del').click( function(){ SkillWheel.deleteSet(fractionCode, getSetName(this)); updateWindowSkills(); } );
                $('.Body input.save').click( function(){ SkillWheel.saveSet(fractionCode, getSetName(this), skillSet); updateWindowSkills(); } );
                
            }
            
            /** Переключение окна набора навыков
            */
            function toggleWindowSkills() {
                updateWindowSkills();
                gui.toggleWindow([gui.scriptName, 'SkillsShow']);
            }
            
            // Окно текущих наборов
            var winShow = gui.createWindow([gui.scriptName, 'SkillsShow'], 'Наборы навыков');
            var bClose = this.createWindowButton([gui.scriptName, 'SkillsShow', 'Close'], 'Закрыть');
            bClose.click( function(){ toggleWindowSkills(); });
            
            // Окно нового набора
            var winSave = gui.createWindow([gui.scriptName, 'SkillsSave'], 'Сохранить набор навыков');
            $('.Body', winSave).html("<input type='text' name='name' value='Новый набор'/>");
            $('.Body', winSave).css({'text-align': 'center'});
            $("input[name='name']", winSave).css({'width': '80%', 'border': 'solid 1px #999999'});
            var bSave = this.createWindowButton([gui.scriptName, 'SkillsSave', 'Save'], 'Сохранить');
            bSave.click( function(){ 
                SkillWheel.saveSet(
                    fractionCode,
                    $(".Body input[name='name']", winSave).val(),
                ...
                gui.toggleWindow([gui.scriptName, 'SkillsSave']);
            });
            $("input", winSave).keydown( function(e) {var code = (e.keyCode ? e.keyCode : e.which); if (code == 13) { bSave.click(); }} );
            var bClose = this.createWindowButton([gui.scriptName, 'SkillsSave', 'Close'], 'Закрыть');
            bClose.click( function(){ gui.toggleWindow([gui.scriptName, 'SkillsSave']); } );
            
            // Кнопки меню
            var bAccess = gui.createMenuButton(['Skills'], 'Навыки', 'Доступ к функциям наборов навыков.');
            var bShow = gui.createMenuButton(['Skills', 'SkillsShow'], 'Показать', 'Показать сохраненные наборы навыков.');
            bShow.click(function() { toggleWindowSkills(); gui.toggleMenu(); });
            var bNew = gui.createMenuButton(['Skills', 'SkillsSave'], 'Сохранить', 'Сохранить текущий набор навыков.');
            bNew.click(function() { gui.toggleWindow([gui.scriptName, 'SkillsSave']); gui.toggleMenu(); $(".Body input[name='name']", winSave).focus(); });
        ...
        if (fractionCode > 0) {
            ...
            /** Создание кнопки применения набора навыков
            * @param String name
            */
            function createUseSkillButton(name) {
                var bSkill = gui.createMenuButton(['Skills', 'Use' + i], name);
                bSkill.click(function() {
                    SkillWheel.useSet(document, fractionCode, undefined, name, 
                        function() {
                            GreaseMonkey.log('Установлен набор навыков: ' + name + '.');
                            var popup = GM_getValue('popup', true);
                            var warnText = 'Внимание! Если в старом или новом наборе были навыки меняющие стостав армии героя, следует обновить армию героя. Если в старом или новом наборе были навыки "Образование", следует обновить параметры героя.';
                            if (popup) {tgui.setPopupSet('SkillsSet', 'Установлен набор навыков: <b>' + name + '</b><br><i style="color: red;">' + warnText + '</i>');}
                        }
                    );
                    gui.toggleMenu(); 
                });
            ...
            // Кнопки меню
            var bSkills = gui.createMenuButton(['Skills'], 'Навыки', 'Переключение наборов навыков.');
            var sets = SkillWheel.listSets(fractionCode);
            for (var i = 0; i < sets.length; i++)
                createUseSkillButton(sets[i]);
    ...
    // На странице колеса навыков
    if (url.attr('path') == '/inventory.php') {
        
        // Парсим документ
        var arts = Inventory.parse(document);
        
        if (arts !== false) {
            
            var str = String.fromObjects(arts, [], false, ['img', 'imgSrc', 'uid', 'placeBody', 'placeNom']);
            //GreaseMonkey.log('Доступные артефакты:' + '\n' + str);
            
            // Формируем пользовательский интерфейс
            
            /** Обновление окна набора артефактов
            */
            function updateWindowArts() {
                ...
                */
                ...
                // Текущий набор героя
                var invSet = Inventory.parseCurrentSet(document);
                var str = String.fromObjects(invSet);
                //GreaseMonkey.log('Одетые артефакты:' + '\n' + str);
                
                // Сохраненные наборы
                var sets = Inventory.listSets();
                ...
                var invSetImages = GM_getValue('invSetImages', true);
                if (invSetImages) {
                    
                    var size = GM_getValue('invImageSize', 50);
                    // Отображение наборов картинками
                    var table = '';
                    for (var i = 0; i < sets.length; i++)
                        table += 
                            "<tr><th colspan='3'>" + sets[i] + "</th></tr>" + 
                            "<tr><td colspan='3'>" + Inventory.htmlSet(arts, undefined, sets[i], size, size) + "</td></tr>" + 
                            buttons;
                    ...
                    for (var i = 0; i < sets.length; i++)
                        table += 
                            "<tr><td colspan='3'><b>" + sets[i] + "</b>: " + Inventory.textSet(arts, undefined, sets[i]) + "</td></tr>" + 
                            buttons;
                ...
                $('.Body', winShow).css({'max-height': '480px', 'overflow': 'auto'});
                $('.Body table', winShow).css({'font-size': '12px', 'color': '#000000', 'width': '100%'});
                $('.Body table table', winShow).css({'display': 'inline-block', 'width': 'auto'});
                $('.Body table td', winShow).css({'color': '#000000'});
                if (invSetImages) {
                    $('.Body table th', winShow).css({'text-align': 'center'});
                    $('.Body table td', winShow).css({'text-align': 'center'});
                ...
                $('.Body table td.buttons', winShow).css({'text-align': 'right'});
                $('.Body input', winShow).css({'width': '80px', 'border': 'solid 1px #999999', 'margin': '2px', 'padding': '1px', 'background': '#cccccc', 'font-size': '12px', 'cursor': 'pointer', 'margin-left': '10px'});
                $('.Body input.use').click( function(){ Inventory.useSet(undefined, getSetName(this), function(){ document.location.reload(); }); } );
                $('.Body input.del').click( function(){ Inventory.deleteSet(getSetName(this)); updateWindowArts(); } );
                $('.Body input.save').click( function(){ Inventory.saveSet(getSetName(this), invSet); updateWindowArts(); } );
                
            }
            
            /** Переключение окна набора навыков
            */
            function toggleWindowArts() {
                updateWindowArts();
                gui.toggleWindow([gui.scriptName, 'ArtsShow']);
            }
            
            // Окно текущих наборов
            var winShow = gui.createWindow([gui.scriptName, 'ArtsShow'], 'Наборы артефактов');
            var bClose = this.createWindowButton([gui.scriptName, 'ArtsShow', 'Close'], 'Закрыть');
            bClose.click( function(){ toggleWindowArts(); });
            
            // Окно нового набора
            var winSave = gui.createWindow([gui.scriptName, 'ArtsSave'], 'Сохранить набор артефактов');
            $('.Body', winSave).html("<input type='text' name='name' value='Новый набор'/>");
            $('.Body', winSave).css({'text-align': 'center'});
            $("input[name='name']", winSave).css({'width': '80%', 'border': 'solid 1px #999999'});
            var bSave = this.createWindowButton([gui.scriptName, 'ArtsSave', 'Save'], 'Сохранить');
            bSave.click( function(){
                var invSet = Inventory.parseCurrentSet(document);
                var str = String.fromObjects(invSet);
                GreaseMonkey.log('Одетые артефакты:' + '\n' + str);
                Inventory.saveSet($(".Body input[name='name']", winSave).val(), invSet);
                gui.toggleWindow([gui.scriptName, 'ArtsSave']);
            } );
            $("input", winSave).keydown( function(e) {var code = (e.keyCode ? e.keyCode : e.which); if (code == 13) { bSave.click(); }} );
            var bClose = this.createWindowButton([gui.scriptName, 'ArtsSave', 'Close'], 'Закрыть');
            bClose.click( function(){ gui.toggleWindow([gui.scriptName, 'ArtsSave']); } );
            
            // Кнопки меню
            var bAccess = gui.createMenuButton(['Arts'], 'Артефакты', 'Доступ к функциям наборов артефактов.');
            var bShow = gui.createMenuButton(['Arts', 'ArtsShow'], 'Показать', 'Показать сохраненные наборы артефактов.');
            bShow.click(function() { toggleWindowArts(); gui.toggleMenu(); });
            var bNew = gui.createMenuButton(['Arts', 'ArtsSave'], 'Сохранить', 'Сохранить текущий набор артефактов.');
            bNew.click(function() { gui.toggleWindow([gui.scriptName, 'ArtsSave']); gui.toggleMenu(); $(".Body input[name='name']", winSave).focus(); });
            
        }
            
    } else {
        
        /** Создание кнопки применения набора артефактов
        * @param String name
        */
        function createUseArtsButton(name) {
            var bArt = gui.createMenuButton(['Arts', 'Use' + i], name);
            bArt.click(function() { 
                Inventory.useSet(undefined, name, 
                    function() {
                        GreaseMonkey.log('Установлен набор артефактов: ' + name + '.');
                        var popup = GM_getValue('popup', true);
                        if (popup) {tgui.setPopupSet('InvSet', 'Установлен набор артефактов: <b>' + name + '</b>');}
                    }
                );
                gui.toggleMenu(); 
            });
        ...
        // Кнопки меню
        var bArts = gui.createMenuButton(['Arts'], 'Артефакты', 'Переключение наборов артефактов.');
        var sets = Inventory.listSets();
        for (var i = 0; i < sets.length; i++)
            createUseArtsButton(sets[i]);
        
    }
    
    // На странице информации о персонаже
    if (url.attr('path') == '/pl_info.php') {
        
        var parsePlayerInfo = GM_getValue('parsePlayerInfo', false);
        if (parsePlayerInfo) {
            
            // Ищем объект с армией героя
            var obj = document.getElementById('showarmy');
            var stacks = Army.parseArmyObject(obj);
            
            // Сохраняем из него информацию по названиям существ
            for (var i = 0; i < stacks.length; i++)
                Army.saveStack(stacks[i]);
