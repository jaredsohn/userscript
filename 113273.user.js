// ==UserScript==
// @name           Neopets: Money Tree [ Grabs Things! ]
// @namespace      http://userscripts.org/users/106594
// @description    Tries to pick Things when appear on Money Tree
...
{
var i,highest=0,am=new Array(),amounts = $x("//b[contains(text(), 'piece')]",7);
if(amounts.length==0) {window.location.reload(2000);}
else if(amounts.length==1) {location.href = amounts[0].parentNode.firstChild.href;}
else {
for(i=0; i<amounts.length; i++) {am[i] = parseInt(amounts[i].innerHTML);}
...
{
var i,highest=0,am=new Array(),amounts = $x("//b[contains(text(), 'morphing')]",7);
if(amounts.length==0) {window.location.reload(2000);}
else if(amounts.length==1) {location.href = amounts[0].parentNode.firstChild.href;}
else {
for(i=0; i<amounts.length; i++) {am[i] = parseInt(amounts[i].innerHTML);}
...
{
var i,highest=0,am=new Array(),amounts = $x("//b[contains(text(), 'dubloon')]",7);
if(amounts.length==0) {window.location.reload(2000);}
else if(amounts.length==1) {location.href = amounts[0].parentNode.firstChild.href;}
else {
for(i=0; i<amounts.length; i++) {am[i] = parseInt(amounts[i].innerHTML);}
