...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'**naber len ittifak.... **', scouting:false, wilds:false, defend:true, minTroops:10000, spamLimit:10, lastAttack:0, barbautoswitch:false, raidautoswitch: {}, },
  ...
  spamconfig   : {aspam:false, spamvert:'bize katılırmısın!!!!', spammins:'10', atime:2 , spamstate:'a'},
...
function facebookInstance (){
  function setWide (){
	...
	if(e){
		...
		if (GlobalOptions.pbWideScreenStyle=="geniş") e.parentNode.style.width = '1520px';
		if (GlobalOptions.pbWideScreenStyle=="epeygeniş") e.parentNode.style.width = '1900px';
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  AddMainTabLink('son samuray', eventHideShow, mouseMainTab);
  tabManager.init (mainPop.getMainDiv());
  actionLog ("samuray Bot v"+ Version +" yüklü  (KofC version: "+ anticd.getKOCversion() +")");
...
Tabs.build = {
    ...
    init: function(div){
        ...
        var m = '<DIV id=pbBuildDivF class=pbStat>inşaat fonksiyonları</div><TABLE id=pbbuildfunctions width=100% height=0% class=pbTab><TR>';
		...
		m += '<TD><INPUT id=pbBuildMode type=submit value="inşaat ekle = kapalı"></td>';
		m += '<TD>inşaat tipi: <SELECT id="pbBuildType">\
				<OPTION value=build>üst seviye</option>\
				<OPTION value=max>max seviye</option>\
				<OPTION value=destruct>yık</option>\
				</select></td>';
		m += '<TD><INPUT id=pbHelpRequest type=checkbox '+ (t.buildStates.help?' CHECKED':'') +'\></td><TD>yardım iste!!!</td>';
		m += '</tr></table></div>';
        m += '<DIV id=pbBuildDivQ class=pbStat>inşaat sırala</div><TABLE id=pbbuildqueues width=100% height=0% class=ptentry><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
			...
			if (mode == 'build') {
				...
				if (invalid == false) {							
					...
					new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/construct.php" + unsafeWindow.g_ajaxsuffix, {
						...
						onSuccess: function(rslt){
							if (rslt.ok) {
								actionLog("inşaat " + unsafeWindow.buildingcost['bdg' + bdgid][0] + " seviye " + params.lv + " oluyor " + cityName);								
    ...
    showBuildQueue: function(cityId, focus){
        ...
        t.popBuildQueue.getTopDiv().innerHTML = '<TD width="200px"><B>inşaat sırası ' + cityName + '</b></td><TD><INPUT id=pbOptimizeByTime type=submit value="zamanı düzenle"></td>';
    ...
    toggleStateMode: function(obj){
		var t = Tabs.build;
        if (obj.value == 'inşaat ekle = kapalı') {
...
Tabs.Search = {
  ...
  init : function (div){
    ...
    m = '<DIV class=ptentry><TABLE width=100% class=pbTab><TR><TD class=pbDetLeft>ne arayalım: </td><TD width=99%>';
    m += htmlSelector ({0:"barbar", 1:"bozkır", 2:"şehir"}, null, 'id=pasrcType');
	m += '&nbsp; &nbsp; &nbsp; <span class=pbDetLeft>arama şekli: &nbsp;';
	m += htmlSelector({square:"karesel", circle:"dairesel"}, Options.srcdisttype, 'id=pbsrcdist');
    m += '</span></td></tr><TR><TD class=pbDetLeft>At: </td><TD class=xtab>X=<INPUT id=pasrchX type=text\> &nbsp;Y=<INPUT id=pasrchY type=text\>\
      &nbsp; yarıçap: <INPUT id=pasrcDist size=3 value=10 /> &nbsp; <SPAN id=paspInXY></span></tr>\
      <TR><TD class=pbDetLeft>Or:</td><TD>alanların tamamı aranacak: <select id="provinceXY"><option>--alanlar--</option>';
    for (var i in Provinces)
    	m += '<option value="'+i+'">'+Provinces[i].name+'</option>';
    m += '</select></td></tr>';
    m += '<TR><TD colspan=2 align=center><INPUT id=pasrcStart type=submit value="aramayı başlat"/></td></tr>';
    m += '</table></div>\
  ...
  helpPop : function (){
       var helpText = 'Asker Sayıları (KOC WIKI den!!!):<BR>';
       helpText += '<A target="_tab" href="http://koc.wikia.com/wiki/Barbarian_Camps">Daha fazla bilgi için Koc Wikia</a>';
       helpText += '<TABLE><TR><TD>Seviye</td><TD>Asker Cinsi</td></tr>';
       helpText += '<TR><TD>1</td><TD>500 İkmal Birliği + 500 Okçu</td></tr>';
       helpText += '<TR><TD>2</td><TD>500 İkmal Birliği + 2500 Okçu</td></tr>';
       helpText += '<TR><TD>3</td><TD>500 İkmal Birliği + 5000 Okçu</td></tr>';
       helpText += '<TR><TD>4</td><TD>500 İkmal Birliği + 7500 Okçu</td></tr>';
       helpText += '<TR><TD>5</td><TD>15000 Okçu</td></tr>';
       helpText += '<TR><TD>5</td><TD>12000 Okçu - seviye 10 ok yapımı ve seviye 9 Hafifletici Toz</td></tr>';
       helpText += '<TR><TD>6</td><TD>25000 Okçu - seviye 9 ok yapım</td></tr>';
       helpText += '<TR><TD>6</td><TD>22000 Okçu - seviye 10 ok yapım</td></tr>';
       helpText += '<TR><TD>7</td><TD>45000 Okçu - seviye 10 ok yapım</td></tr>';
       helpText += '<TR><TD>7</td><TD>44000 Okçu - seviye 10 ok yapım ve şovalye 69+</td></tr>';
       helpText += '<TR><TD>7</td><TD>40000 Okçu - seviye 10 ok yapım ve şovalye 94+</td></tr>';
       helpText += '<TR><TD>8</td><TD>28000 Balista - Level 10 ok yapım ve şovalye 91+</td></tr>';
       helpText += '<TR><TD>9</td><TD>56000 Balista - Level 10 ok yapım ve şovalye 98+</td></tr>';
       helpText += '<TR><TD>10</td><TD>125000 Katapult (500 Katapult kayıp!)</td></tr></tr></table>';
  
       var pop = new CPopup ('giftHelp', 0, 0, 425, 375, true);
       pop.centerMe (mainPop.getMainDiv());  
       pop.getMainDiv().innerHTML = helpText;
       pop.getTopDiv().innerHTML = '<CENTER><B>Samuray Bot Yardım: Raids</b></center>';
       pop.show (true);
     },
  ...
  clickedSearch : function (){
    ...
    if (isNaN (t.opt.startX) ||t.opt.startX<0 || t.opt.startX>749)
      errMsg = "X arasında olmalıdır 0 ve 749<BR>";
    if (isNaN (t.opt.startY) ||t.opt.startY<0 || t.opt.startY>749)
      errMsg += "Y arasında olmalıdır 0 ve 749<BR>";
    if (isNaN (t.opt.maxDistance) ||t.opt.maxDistance<1 || t.opt.maxDistance>75)
      errMsg += "mesafe arasında olmalıdır 1 ve 75<BR>";
    if (errMsg != ''){
      document.getElementById('pasrcResults').innerHTML = '<FONT COLOR=#660000>İMKANSIZ:</font><BR><BR>'+ errMsg;
      return;
    }

    t.searchRunning = true;
    document.getElementById ('pasrcStart').value = 'aramayı bitir';
    m = '<DIV class=pbStat><TABLE width=100% cellspacing=0><TR><TD class=xtab width=125><DIV id=pastatSearched></div></td>\
        ...
        <TD class=xtab align=right width=125><DIV id=pastatFound></div></td></tr></table></div>\
          <TABLE width=100%><TR valign=top>\
            ...
            <TD align=center valign=middle><A id=pbAhideShow style="text-decoration:none; cursor:pointer;"><DIV style="width:1em; border:1px solid red; padding:10px 2px; background-color:#fee"><SPAN id=spanHideShow> G İ Z L E</span><BR><BR> L<BR>I<BR>S<BR>T<BR><BR> O<BR>P<BR>T<BR>I<BR>O<BR>N<BR>S </div></a></td>\
    ...
    if (t.opt.searchShape == 'square')
      ...
      var distName = 'yarıçap';
    m = '<CENTER><B>aranılan '+ typeName +'<BR>\
        merkez: '+ t.opt.startX +','+ t.opt.startY +'  &nbsp; '+ distName +': '+ t.opt.maxDistance +'<BR></center>\
        <DIV class=ptentry><TABLE cellspacing=0 width=100%><TR align=center><TD class=xtab colspan=10><B>LİSTE SEÇENEKLERİ:</b><BR></td></tr>';
        
    if (t.opt.searchType == 1 || t.opt.searchType == 0) {
      m += '<TR><TD class=xtab align=right>Min seviye göster:</td><TD class=xtab> <INPUT id=pafilMinLvl size=2 value='+ Options.srcMinLevel +' /></td></tr>\
        <TR><TD class=xtab align=right>Max seviye göster:</td><TD class=xtab> <INPUT id=pafilMaxLvl size=2 value='+ Options.srcMaxLevel +' /></td></tr>';
		}
    if (t.opt.searchType == 1){
      m += '<TR><TD class=xtab align=right>bozkır tipi:</td><TD class=xtab><SELECT id=pafilWildType>';
      m += htmlOptions ( {1:'çayır/göl', 3:'orman', 4:'tepe', 5:'dağ', 6:'ova', 0:'hepsi'}, Options.wildType );
	  m+= '</select></td></tr>';
	  m+= '<TR><TD class=xtab align=right>çayır/göl:</td><TD class=xtab><INPUT name=pbfil id=pafilGrass type=CHECKBOX '+ (Options.GrassOnly?' CHECKED':'') +'\><td></tr>';
	  m+= '<TR><TD class=xtab align=right>orman:</td><TD class=xtab><INPUT name=pbfil id=pafilWood type=CHECKBOX '+ (Options.WoodOnly?' CHECKED':'') +'\><td></tr>';
	  m+= '<TR><TD class=xtab align=right>tepe:</td><TD class=xtab><INPUT name=pbfil id=pafilHill type=CHECKBOX '+ (Options.HillOnly?' CHECKED':'') +'\><td></tr>';
	  m+= '<TR><TD class=xtab align=right>dağ:</td><TD class=xtab><INPUT name=pbfil id=pafilMount type=CHECKBOX '+ (Options.MountOnly?' CHECKED':'') +'\><td></tr>';
	  m+= '<TR><TD class=xtab align=right>ova:</td><TD class=xtab><INPUT name=pbfil id=pafilPlain type=CHECKBOX '+ (Options.PlainOnly?' CHECKED':'') +'\><td></tr>';
	  m+= '<TR><TD class=xtab align=right>hepsi:</td><TD class=xtab><INPUT name=pbfil id=pafilAll type=CHECKBOX '+ (Options.srcAll?' CHECKED':'') +'\><td></tr>';
   ...
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
        m+= '<TR><TD class=xtab align=right>sırala:</td><TD class=xtab><SELECT id=pafilSortBy>\
          ...
          <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>mesafe</option>\
			...
			<TR><TD class=xtab align=right>sadece koordinatlar:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
			</table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
    } else {
		m+= '</select></td></tr><TR><TD class=xtab align=right>sisde:</td><TD class=xtab><INPUT name=pbfil id=pafilMisted type=CHECKBOX '+ (Options.mistedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>düşman:</td><TD class=xtab><INPUT name=pbfil id=pafilHostile type=CHECKBOX '+ (Options.hostileOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>samimi:</td><TD class=xtab><INPUT name=pbfil id=pafilFriendly type=CHECKBOX '+ (Options.friendlyOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>müttefik:</td><TD class=xtab><INPUT name=pbfil id=pafilAllied type=CHECKBOX '+ (Options.alliedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>nötr:</td><TD class=xtab><INPUT name=pbfil id=pafilNeutral type=CHECKBOX '+ (Options.neutralOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>ittifaksız:</td><TD class=xtab><INPUT name=pbfil id=pafilunAllied type=CHECKBOX '+ (Options.unalliedOnly?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>hepsi:</td><TD class=xtab><INPUT name=pbfil id=pafilAll type=CHECKBOX '+ (Options.srcAll?' CHECKED':'') +'\><td></tr>';
		m+= '<TR><TD class=xtab align=right>sırala:</td><TD class=xtab><SELECT id=pafilSortBy>\
          <OPTION value="might" '+ (Options.srcSortBy=='might'?'SELECTED':'')  +'>kudret</option>\
             <OPTION value="dist" '+ (Options.srcSortBy=='dist'?'SELECTED':'')  +'>mesafe</option>\
        </select></td></tr>\
		<TR><TD class=xtab align=right>minumum kudret:</td><TD class=xtab><INPUT type=text id=paminmight size=6 value='+ Options.minmight +'>\
        <TR><TD class=xtab align=right>sadece koordinat:</td><TD class=xtab><INPUT type=checkbox id=pacoordsOnly \></td></tr>\
        </table></div><BR><SPAN id=pasrchSizeWarn></span><DIV id=pbSrcExp></div>';
    ...
    document.getElementById ('pastatStatus').innerHTML = 'aranıyor '+ xxx +','+ yyy;
  ...
  dispMapTable : function (){
    var tileNames = ['Barbar', 'çayır', 'göl', 'orman', 'tepe', 'dağ', 'ova' ];
    var t = Tabs.Search;
    ...
    document.getElementById('pastatFound').innerHTML = 'bulunan: '+ dat.length;
    if (dat.length == 0){
      ...
      if (coordsOnly)
        m = '<TABLE align=center id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>koordinat</td></tr>';
      else {
      if (t.opt.searchType == 2) {
			 m = '<TABLE id=pasrcOutTab class=pbSrchResults cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>Loc</td><TD align=right>Dist</td><TD>oyuncu</td><TD align=right>kudret</td><TD>ittifak</td><TD>Online</td><TD></td></tr>';
		} else {
			m = '<TABLE id=pasrcOutTab cellpadding=0 cellspacing=0><TR style="font-weight: bold"><TD>koordinat</td><TD style="padding-left: 10px">mesafe</td><TD style="padding-left: 10px;">seviye</td><TD width=100px> &nbsp; Tipi</td><TD></td><TD>barbar saldırısı</td></tr>';
		}
	}
      ...
      for (i=0; i<numRows; i++){
        ...
        if (coordsOnly) {
          ...
          if (t.opt.searchType == 2) { // city search
            ...
            if (dat[i][5])
              m += '<TD colspan=4>* siste * &nbsp; &nbsp; <SPAN onclick="pbSearchScout('+ dat[i][0] +','+ dat[i][1] +');return false;"><A>gözetle</a></span></td></tr>';
            else{
              var allStyle = '';
              if (dat[i][12]=='f')
              ...
              m += '<TD>'+ dat[i][9]+'</td><TD align=right>'+ dat[i][10] +'</td><TD><SPAN '+ allStyle +'>'+ dat[i][11]+'</span></td><TD>'+(dat[i][13]?'<SPAN class=boldDarkRed>ONLINE</span>':'')+'</td><TD><A onclick="pbSearchLookup('+ dat[i][7] +')">araştır</a></td></tr>';
            }
			} else {
          m += '<TD align=right  valign="top">'+ dat[i][2].toFixed(2) +' &nbsp; </td><TD align=right>'+ dat[i][4] +'</td><TD> &nbsp; '+ tileNames[dat[i][3]]
            +'</td><TD  valign="top">'+ (dat[i][5]?(dat[i][6]!=0?' <A onclick="pbSearchLookup('+dat[i][6]+')">sahibi</a>':'<A onclick="pbSearchScout('+ dat[i][0] +','+ dat[i][1] +');return false;">MISTED</a>'):'') +'</td>';
          if (t.opt.searchType == 0) m+= '<TD align=center  valign="top"><A onclick="pbExportToRaid('+ dat[i][0]+','+dat[i][1] +')">raid saldırısı</a></td>';
          m+='</tr>';
			}
		}
  ...
  stopSearch : function (msg){
    ...
    document.getElementById ('pasrcStart').value = 'aramaya başla';
	...
	if (t.opt.searchType==2){
	  document.getElementById ('pbSrcExp').innerHTML = '<CENTER>'+ strButton20('haberci listesi oluştur', 'id=pbSrcDoScout') +'</center>';
  ...
  ShowScoutList : function (coordlist, city){
	...
	var m = '<DIV class=pbStat>oto haberci seçenekleri</div>';
		m += '<DIV>gönderilecek haberci miktarı: <input id=pbsrcScoutAmt value="'+Options.srcScoutAmt+'" /></div><BR>';
		m += '<DIV>şehir seç: <span id=pbsrcScoutcitypick> </span></div><BR>';
		m += '<DIV class=pbStat>haberci nereden <span id=pbsrcScoutcity>'+city.name+'</span> <BR> toplam hedef '+coordlist.length+'</div>';
		m += '<DIV style="max-height:220px; overflow-y:auto;"><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabPadNW><TR style="font-weight:bold; background-color:white"><TD width=15><input type=checkbox id=pbsrcScout_All /></td><TD>hedef koordinatlar</td></tr>';
	  for(i=0; i<coordlist.length; i++){
	    ...
	    m += '</table></div>';
		m += '<BR><CENTER>'+ strButton20('gözcüyü gönder', 'id=pbSrcStartScout') +'</center>';
	...
	popScout.getTopDiv().innerHTML = '<CENTER><B>samuray haberci listesi</b></center>';
	popScout.show(true);
  ...
  doScout : function(list, city){
	...
	if(list.length < 1){
		document.getElementById('pbSrcScoutResult').innerHTML = '<SPAN class=boldRed>İMKANSIZ: koordinat seçilmedi</span>';
		t.clickedStartScout();
		return;
	}
  ...
  doScoutCount : function(list, city, total, count){
	...
	if(!t.scouting){
		...
		document.getElementById('pbSrcStartScout').innerHTML = '<SPAN>gözcüyü gönder</span>';
		return;
	}
	if(total <= (count)){
		document.getElementById('pbSrcScoutResult').innerHTML += 'bitti!<BR>';
		t.clickedStartScout();
		return;
	}
  ...
  getRallypoint: function(cityId){
	...
	var t = Tabs.Search;
		if(t.scouting == false){
			...
			document.getElementById('pbSrcStartScout').innerHTML = '<SPAN>durdur</span>';
		} else {
			t.scouting = false;
			document.getElementById('pbSrcStartScout').className = 'button20 ptButton20';
...
// TODO: save memory, remove city name ?   			
  ...
  clickedLookup : function (pid){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>araştırılan oyuncu</b></center>';
    pop.getMainDiv().innerHTML = '<DIV class=pbStat>oyuncu bilgileri</div><SPAN id=pblupLB>Looking up leaderboard...</span>\
      <BR><DIV class=pbStat>ittifak araştırması</div><SPAN id=pblupAI>Looking up alliance info...</span>';
    pop.show (true);
  ...
  ExportToRaid : function (X,Y){
    ...
    pop.getTopDiv().innerHTML = '<CENTER><B>raid saldırısı</b></center>';
	  ...
	  m += '<BR><CENTER>' +strButton20('yardım', 'id=pbHelp')+'<SELECT id=RaidKnights type=list></select></center>';
	  m+= '<BR><CENTER>'+ strButton20('saldır ve kaydet', 'id=pbRaidSave') +'</center>';
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (rslt.totalResults == 0){
      span.innerHTML = '<B>kişi bilgileri:</b> bulunamadı! (kayıp?)<BR><BR>';
      return;
    }
    ...
    var m = '<CENTER><SPAN class=boldRed>NOT: kişi bilgileri 24 saat öncesidir..</span></center><TABLE class=pbTabSome>';
    m += '<TR><TD class=pbDetLeft>oyuncu adı:</td><TD>'+ name +'</td></tr>\
      <TR><TD class=pbDetLeft>kudret:</td><TD>'+ p.might +' (rank #'+ p.rank +')</td></tr>\
      <TR><TD class=pbDetLeft>ittifak:</td><TD>'+ aName +' ('+ getDiplomacy(p.allianceId) +')</td></tr>\
      <TR valign=top><TD class=pbDetLeft>şehirleri:</td><TD><TABLE class=pbTabSome><TR style="font-weight:bold"><TD>şehir adı</td><TD>koordinatı</td><TD>seviye</td><TD>statü</td><TD>oluşturulan</td></tr>';
  ...
  gotPlayerInfo : function (rslt, span){
    ...
    m += '<TR><TD class=pbDetLeft>oyuncu adı:</td><TD>'+ p.genderAndName +'</td></tr>\
      <TR><TD class=pbDetLeft>kudret:</td><TD>'+ p.might +'</td></tr>\
      <TR><TD class=pbDetLeft>Facebook profili:</td><TD><A target="_tab" href="http://www.facebook.com/profile.php?id='+ p.fbuid +'">yeni sekmede aç </a></td></tr>\
      <TR><TD class=pbDetLeft>ittifak:</td><TD>'+ p.allianceName +' ('+ getDiplomacy(p.allianceId) +')</td></tr>\
      <TR valign=top><TD class=pbDetLeft>bulunduğu alan:</td><TD style="white-space:normal">'+ prov.join(', ') +'</td></tr>';
...
/*********************************  Cresting Tab ***********************************/
 Tabs.Crest = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    var selbut=0;
    var m = '<DIV id=pbTowrtDivF class=pbStat>oto arma arama fonksiyonu</div><TABLE id=pbcrestfunctions width=100% height=0% class=pbTab><TR align="center">';
     if (CrestOptions.Running == false) {
	       m += '<TD><INPUT id=Cresttoggle type=submit value="arma = kapalı"></td>';
	   } else {
	       m += '<TD><INPUT id=Cresttoggle type=submit value="arma = açık"></td>';
	   }
    m += '<TD><INPUT id=CrestHelp type=submit value="yardım"></td>';
    m += '<TD><INPUT id=pbsendreport type=checkbox '+ (Options.crestreport?' CHECKED':'') +'\> rapor gönder ';
    m += '<INPUT id=pbsendreportint value='+ Options.CrestMsgInterval +' type=text size=3 \> saatde </td></table>';
    
    m += '<DIV id=pbOpt class=pbStat>arma seçenekleri</div><TABLE id=pbcrestopt	 width=100% height=0% class=pbTab><TR align="center"></table>';
    m += '<DIV style="margin-bottom:10px;">arayacak şehir: <span id=crestcity></span></div>';
    
    m += '<TABLE class=ptTab><TR><TD>bozkır koord: X:<INPUT id=pbcrestx type=text size=3 maxlength=3 value="'+CrestOptions.X+'"</td>';
    m += '<TD>Y:<INPUT id=pbcresty type=text size=3 maxlength=3 value="'+CrestOptions.Y+'"</td></tr></table>';
   
    m += '<TABLE class=ptTab><TR><TD>atak <b>1</b>: </td><TD><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.png></td><TD><INPUT id=R1MM type=text size=5 maxlength=5 value="'+CrestOptions.R1MM+'" (When left 0 it will not send out a first wave, for whatever reason you want to do that...)</td>';
    ...
    m += '<TR><TD>atak <b>2</b>: </td><TD><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_30.png></td><TD><INPUT id=R2MM type=text size=5 maxlength=5 value="'+CrestOptions.R2MM+'"</td>';
  ...
  helpPop : function (){
    var helpText = '<BR>arma sekmesi tekrar tekrar saldırı için tasarlandı.<BR>';
    helpText += 'bu saldırılar 2 dalga halinda olur.<BR>';
    helpText += 'toplanma merkezinizden emin olun!<BR>';
    helpText += 'koordinat ve asker sayılarını gir ve "armayı başlat".<BR><BR>';
    helpText += 'asker sayıları (KOC WIKI den):<BR>';
    helpText += '<A target="_tab" href="http://koc.wikia.com/wiki/Wilderness">daha fazla bilg Koc Wikia de</a>';
...
************/
 ...
 Tabs.Raid = {
  ...
  init : function (div){
	...
	var m = '<DIV class=pbStat>barbar fonksiyon</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
	    m += '<TD><INPUT id=pbRaidStart type=submit value="sıfırla = '+ (Options.RaidRunning?'açık':'kapalı') +'" ></td>';
	    m += '<TD><INPUT id=pbsendreport type=checkbox '+ (Options.foodreport?' CHECKED':'') +'\> rapor gönder ';
	    m += '<INPUT id=pbsendreportint value='+ Options.MsgInterval +' type=text size=3 \> saatde </td>';
	    m += '</tr></table></div>';
	    m += '<DIV class=pbStat>çalışan barbar</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
	    ...
	    m += '<DIV class=pbStat>kayıtlı barbar</div><TABLE width=100% height=0% class=pbTab><TR align="center">';
  ...
  lookup : function (){
      	...
      	for (c=0; c< Seed.cities.length;c++) {
	   	...
	   	if (t.resuming == false && t.stopping == false && t.deleting == false && t.activecount != 0)
			document.getElementById('pbraidtab').innerHTML = '<span style="color: #ff6">barbar stop ('+ t.activecount + ')</span>'
	   	else if (t.resuming == false && t.stopping == false && t.deleting == false)
			document.getElementById('pbraidtab').innerHTML = '<span style="color: #CCC">barbar stop ('+ t.activecount + ')</span>'
   		if (t.resuming == false && t.resuming == false && t.deleting == false && t.stopcount !=0)
			document.getElementById('pbraidtabRes').innerHTML = '<span style="color: #ff6">barbar devam('+ t.stopcount + ')</span>'
   		else if (t.resuming == false && t.stopping == false && t.deleting == false)
			document.getElementById('pbraidtabRes').innerHTML = '<span style="color: #CCC">barbar devam ('+ t.stopcount + ')</span>'
   		if (t.resuming == false && t.stopping == false && t.deleting == false && t.stopcount !=0)
			document.getElementById('pbraidtabDel').innerHTML = '<span style="color: #ff6">barbarı sil ('+ t.stopcount + ')</span>'
   		else if (t.resuming == false && t.stopping == false && t.deleting == false)
			document.getElementById('pbraidtabDel').innerHTML = '<span style="color: #CCC">barbarı sil ('+ t.stopcount + ')</span>'
  },
  ...
  paint : function ()	{
  	...
  	var botStat = {0:'Undefined',
  						...
  						7:'Timed out',
  						8:'Resting'};
  	var o = '<FONT size=2px><B>barbar zamanı: '+ timestr( 86400 - ( unixTime() - t.rslt.settings.lastUpdated )) +'</b></font>';
  	document.getElementById('ptRaidTimer').innerHTML = o;
  	
  	var z ='<TABLE class=pbTab><TR><TD width=60px align=center><A onclick="pbStopAll('+t.cityId+')">durdur</a></td><TD width=70px>zaman</td><TD width=85px>koord</td><TD width=50px>seviye</td><TD width=50px></td><TD width=50px><A onclick="pbDeleteAll()">sil</a></td></TR>';
  	if (t.rslt['queue'] != ""){
  		for (y in t.rslt['queue']) {
  			if (t.rslt['queue'][y]['botMarches'] != undefined) {
  				...
  				if (botMarchStatus == 3) z+='<TD><A onclick="pbEditRaid('+ y +')">düzenle</a></td>';
	  				else z+='<TD><FONT COLOR= "CCCCCC">düzenle</font></td>';
  				if (botMarchStatus == 3) z+='<TD align=center><A onclick="pbDeleteRaid('+ t.rslt['queue'][y]['botMarches']['marchId']+')">sil</a></td>';
  				else z+='<TD align=center><FONT COLOR= "CCCCCC">sil</font></td>';
  				//z +='<TD width=25px></td><TD>Statü: '+ botMarchStat[botMarchStatus]+'</td>';
  				z +='<TD width=25px></td><TD>kalan zaman: '+ timestr(restPeriod) +'</td>';
  				z+='</tr>';
  			}
  		}
  	}
  	z+='</table>';
  	if (t.rslt['queue'] == "") z ='<TABLE class=pbTab><TR><TD>bu şehirde saldırı yok!</td></TR>';
  	document.getElementById('PaintRaids').innerHTML = z;
  	
  	var check = true;
  		if (t.save != ""){
  			var a ='<TABLE class=pbTab><TR><TD width=60px></td><TD width=70px></td><TD width=85px>Coords</td><TD width=50px>seviye</td><TD width=50px></td><TD width=50px></td></tr>';
  			for (y in t.save){
  				if (t.save[y] != undefined && t.cityId == t.save[y]['cityId']){
  					...
  					a +='<TD><A onclick="pbEditSavedRaid('+ y +')">düzenle</a></td>';
  					a +='<TD align=center><A onclick="pbAddRaid('+ t.save[y]['marchId']+')">ekle</a></td></tr>';
  					check = false;
  				}	
  			}
  			m+='</table>';
  	...
  	if (check) a ='<TABLE class=pbTab><TR><TD>şehirde kayıtlı saldırı yok!</td></TR>';
  ...
  EditSavedRaid : function (y){
	  ...
	  pop.getTopDiv().innerHTML = '<CENTER><B>saldırıyı düzenle</b></center>';
	  cityId =  t.save[y]['cityId'];
		  ...
		  m+='<TD width=25px></td><TD>gidiş-dönüş: '+ timestr((t.save[y]['returnUnixTime'] - t.save[y]['destinationUnixTime'])*2)+ '</td></tr></table>';
  ...
  EditRaid : function (y){
  	  ...
  	  pop.getTopDiv().innerHTML = '<CENTER><B>düzenle</b></center>';
    					...
    					var rslt = eval("(" + transport.responseText + ")");
    	                  if (rslt.ok) {
    					 	 ...
    					 	 /* var pop = new CPopup ('pbEditRaid', 0,0, 750,250, true);
				 		  	  ...
				 		  	  pop.getTopDiv().innerHTML = '<CENTER><B>imkansız</b></center>';
				 		  	  var m= '<TABLE id=pbRaidAdd width=100% height=0% class=pbTab><TR align="center">';
  ...
  getKnights : function(cityId){
         ...
         for (k in Seed.knights['city' + cityId]){
         		if ( Seed.leaders['city' + cityId]["resourcefulnessKnightId"] != Seed.knights['city' + cityId][k]["knightId"] && Seed.leaders['city' + cityId]["politicsKnightId"] != Seed.knights['city' + cityId][k]["knightId"] && Seed.leaders['city' + cityId]["combatKnightId"] != Seed.knights['city' + cityId][k]["knightId"] && Seed.leaders['city' + cityId]["intelligenceKnightId"] != Seed.knights['city' + cityId][k]["knightId"]){
         		   if (Seed.knights['city' + cityId][k]["knightStatus"] == 1 ) status = "boşta";
...
Tabs.ActionLog = {
  ...
  init : function (div){
    ...
    t.myDiv = div;
    t.myDiv.innerHTML = '<DIV class=pbStat>EYLEM GEÇMİŞİ - VERSİYON: '+ Version+'</div><DIV style="height:535px; max-height:535px; overflow-y:auto">\
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>Samuray Bot Yapılandırma:</b></td></tr>\
        <TR><TD><INPUT id=pballowWinMove type=checkbox /></td><TD>pencere sürüklemeyi etkinleştir</td></tr>\
        <TR><TD><INPUT id=pbTrackWinOpen type=checkbox /></td><TD>yenilemede pencereleri hatırla</td></tr>\
        <TR><TD><INPUT id=pbHideOnGoto type=checkbox /></td><TD>harita koordinatlarını tıklandığında pencereyi gizle</td></tr>\
        <TR><TD><INPUT id=pbWideOpt type=checkbox '+ (GlobalOptions.pbWideScreen?'CHECKED ':'') +'/></td><TD>Geniş ekran şekli etkinleştir: '+ htmlSelector({normal:'Normal', wide:'Geniş', ultra:'Ultra'},GlobalOptions.pbWideScreenStyle,'id=selectScreenMode') +' (yenileme gerektirmektedir)</td>\
        <TR><TD colspan=2><BR><B>Özellikler:</b></td></tr>\
        <TR><TD><INPUT id=pbFairie type=checkbox /></td><TD>açılır pencereleri devre dışı bırak</td></tr>\
        <TR><TD><INPUT id=pbWatchEnable type=checkbox '+ (GlobalOptions.pbWatchdog?'CHECKED ':'') +'/></td><TD>dakikada 1 yenile (yenileme gerektirir)</td></tr>\
        <TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>oyunu yenile <INPUT id=pbeverymins type=text size=2 maxlength=3 \> dakika</td></tr>\
        <TR><TD><INPUT id=pbChatREnable type=checkbox /></td><TD>sohbet sağ tarfta (geniş ekran gerektirir)</td></tr>\
		<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>büyük harita kullan (geniş ekran gerektirir)</td></tr>\
        <TR><TD><INPUT id=pbGoldEnable type=checkbox /></td><TD>mutluluk ulaştığında otomatik altın topla <INPUT id=pbgoldLimit type=text size=2 maxlength=3 \>%</td></tr>\
        <TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>yiyecek uyarısını etkinleştir (6 Saat saatte bir kontrol)</td></tr>\
        <TR><TD colspan=2><BR><B>Ekstra Özellikler:</b></td></tr>\
        <TR><TD><INPUT id=HelReq type=checkbox /></td><TD>ittifak yardım mesajları</td></tr>\
        <TR><TD><INPUT id=DelReq type=checkbox /></td><TD>sohbet ittifak isteklerini gizle</td></tr>\
        <TR><TD><INPUT id=PubReq type=checkbox '+ (GlobalOptions.autoPublishGamePopups?'CHECKED ':'') +'/></td><TD>Facebookda mesajları yayınla '+ htmlSelector({0:'----', 80:'herkes', 50:'arkilerimin arkileri', 40:'sadece arkilerim', 10:'sadece ben'},GlobalOptions.autoPublishPrivacySetting,'id=selectprivacymode') +' (tüm alanlarda)</td>\
        <TR><TD><INPUT id=MapExtra type=checkbox /></td><TD>oyuncu ve harita göster.</td></tr>\
        <TR><TD><INPUT id=deletetoggle type=checkbox /></td><TD> otomatik sil (barbar raporları)</td></tr>\
        <TR><TD><INPUT id=deletes0toggle type=checkbox /></td><TD> otomatik sil (nakliye raporları)</td></tr>\
        <TR><TD><INPUT id=deletes1toggle type=checkbox /></td><TD> otomatik sil (bozkır raporları)</td></tr>\
        </table><BR><BR><HR>not:onay kutusu gri renkte ise muhtemelen çalışmaz.</div>';
        m += strButton20('seçenekleri sıfırla', 'id=ResetALL');
      div.innerHTML = m;
...
Tabs.AutoTrain = {
  ...
  myDiv: null,
      ...
      var m = '<DIV id=pbAT class=pbStat>oto asker</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR>';
      ...
      m += '<DIV id=pbAT class=pbStat>asker seçenekleri</div><TABLE id=pbAT width=100% height=0% class=pbTab><TR align="center">';
      ...
      for (i=0;i<Seed.cities.length;i++){
	      ...
	      m += '<TD width=150px><SELECT id="TroopsCity'+city+'"><option value="Select">--seç--</options>';
	      for (y in troops) m+='<option value="'+y+'">'+troops[y]+'</option>';
	      m+='</select></td>';
	      m +='<TD width=100px>Min.: <INPUT id=treshold'+city+' type=text size=5 maxlength=5 value="'+ TrainOptions.Threshold[city]+'"\></td>';
	      m +='<TD width=130px><INPUT type=checkbox id="SelectMax'+city+'"> Max.: <INPUT id=max'+city+' type=text size=6 maxlength=6 value="'+ TrainOptions.Max[city]+'"\></td>';
	      m +='<TD>nüfus kullan: ';
        ...
        m += '<TD width=40px></td><TD width=5%><img src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/food_30.png"></td>';
	      ...
	      m += '<TD><SELECT id="Resource'+city+'"><option value="true">sakla</options>';
	      m+='<option value="false">kullan</option>';
	      m+='</select></td></tr>';	    
  ...
  helpPop : function (){
    var helpText = '<BR><DL><dt>oto asker:<dd><LI>şehrin önündeki kutuyu işaretle.</dd>\
    	<dd><LI>basacağınız askeri seçin.</dd>\
        <dd><LI>asker sayısını yazın(max. isteğe bağlı).</dd>\
        <dt>kaynakları gir:</dt>\
          <dd><LI>sakla: saklamak istediğin kaynak miktarı..</dd>\
          <dd><LI>kullan:kullanmak istediğin kaynak miktarı.</dd>\
        <dt>başlamak için: </dt>\
          <dd><LI>oto asker= açık butonuna bas.</dd></ul>';
    var pop = new CPopup ('giftHelp', 0, 0, 550, 250, true);
    pop.centerMe (mainPop.getMainDiv());  
    pop.getMainDiv().innerHTML = helpText;
    pop.getTopDiv().innerHTML = '<CENTER><B>Son Samuray Yardım</b>:  oto asker</center>';
    pop.show (true);
  },
...
Tabs.Spam = {
  ...
  tabLabel : 'mesaj',            // label to show in main window tabs
  myDiv : null,
  timer : null,  
  ...
  init : function (div){    // called once, upon script startup
    var t = Tabs.Spam;
    t.myDiv = div;
    var m = '<DIV class=pbStat>reklamlaaarrr</div><TABLE class=pbTab width=100% height=0% ><TR align="center">';

       if (Options.spamconfig.aspam == true) {
        m += '<TD><INPUT id=pbSpamEnable type=submit value="mesaj açık"></td>';
       }
       else {
        m += '<TD><INPUT id=pbSpamEnable type=submit value="mesaj kapalı"></td>';
       }

       if (Options.spamconfig.spamstate == 'a') {
        m += '<TD><INPUT id=pbSpamState type=submit value="ittiye yolla"></td>';
       }
       else {
        m += '<TD><INPUT id=pbSpamState type=submit value="globale yolla"></td>';
       }
        m += '</tr></table></div>';
       m += '<DIV class=pbStat>ayarlar</div><TABLE class=pbTab>';
        m += '<tr><td>otomatik mesaj <INPUT id=pbSpamMin type=text size=2 maxlength=3 value="'+ Options.spamconfig.spammins +'"  \> dakikada bir</td></tr><BR>\
              <tr><TD><TABLE cellpadding=0 cellspacing=0>\
              <TD align=left>mesajın: &nbsp; </td><TD><INPUT id=pbSpamAd type=text size=60 maxlength=500 value="'+ Options.spamconfig.spamvert +'" \></td></tr>\
              </table><BR>';
 ...
 togglespam: function(obj){
  ...
  if (Options.spamconfig.spamstate == 'a') {
   ...
   obj.value = "globale yolla ";
  }
  else {
   Options.spamconfig.spamstate = 'a';
   obj.value = "ittiye yolla";
  }
  saveOptions ();
 ...
 toggleon: function(obj){
  ...
  if (Options.spamconfig.aspam == true) {
   ...
   obj.value = "mesaj kapalı";
  }
  else {
   Options.spamconfig.aspam = true;
   obj.value = "mesaj açık";
   SpamEvery.init();
  }
  saveOptions ();
...
Tabs.Resources = {
  ...
  init : function (div){
    var t = Tabs.Resources;
		t.myDiv = div;    
    div.innerHTML = '<TABLE cellpadding=0 cellspacing=0 class=pbTab width=100%><TR><TD align=center><INPUT id="pballlist" type=submit value="kullanıcı listesi" \></td></tr></table><HR>\
  ...
  e_clickfetchlist : function  (){     // (also cancel accepting)
	...
	t.users = [];
    if (t.accepting){
      document.getElementById('pballlist').value = 'kullanıcı listesi';
      document.getElementById('resDiv').innerHTML+= '<BR><SPAN class=boldRed>Cancelled.</span>';
      t.accepting = false;
      return;
    }
    document.getElementById('resDiv').innerHTML = 'liste alınıyor ... <span id=pbResUserListCount></span>';
    ...
    function gotUserList(userList){
		...
		document.getElementById('resDiv').innerHTML += '<BR>kullanılabilir liste alınıyor ... <span id=pbResUserAvailCount></span>';
		t.checkDailyAction(userList, listGifts);
	}
    
    function listGifts (){
	  t.city = Cities.cities[0];
	  var m = '<DIV class=pbStat><CENTER>kullanıcı.... &nbsp; &nbsp; &nbsp; ('+ t.users.length +' bulundu)</center></div>';
      if (t.users.length<1){
        document.getElementById('resDiv').innerHTML = m + '<BR><BR><CENTER>kullanıcı bulunamadı!</center>';
        return;
      }
      m += '<TABLE class=pbTab align=center><TR><TD align=right>kaynak toplanacak şehir: </td><TD id=pbrescityselspan></td></tr>\
          <TR><TD align=right>kaynak seç</td><TD>'
        + htmlSelector (t.resource, Options.getResType, 'id=pbResColType')
        + '</td></tr><TR><TD>kullanıcılardan kaynak toplamak istiyorum: </td><TD width=250><INPUT type=submit id=pbResDo value="kaynak kabul">\
        &nbsp; <SPAN id=pbResNone class=boldRed></span></td></tr></table><HR><TABLE class=pbTab><TR valign=top><TD>\
        <INPUT id=pbResButAll type=submit value="hepsi" style="width:100%; margin-bottom:5px"><BR><INPUT id=pbResButNone type=submit value="hiçbiri"></td>\
        <TD width=10></td><TD><TABLE align=center cellpadding=0 cellspacing=0 class=pbTabLined>\
        <TBODY id=pbResTbody>\
        <TR style="font-weight:bold; background:white"><TD>adı</td><TD>kudret</td><TD width=20></td></tr>';
  ...
  getErDone : function (){
    ...
    if (t.doList.length==0){
      document.getElementById('pbResNone').innerHTML = 'seçilmedi!';
      return;
    }
    t.accepting = true;
    document.getElementById('pballlist').value = 'kabulu durdur';
    document.getElementById('resDiv').innerHTML = '<DIV id=rsltDiv style="height:400px; max-height:400px; overflow-y:auto"><B>kabul edildi '+ t.doList.length +' users:</b><BR></div>';    
    t.acceptNext ();
  },
...
//logit ("getErDone.gotGiftData ... \n"+ inspect (gift, 8, 1));
      if (!t.accepting)
        return;
      if (rslt.ok){
        var msg = rslt.gold +' gold and '+rslt.resource +' '+ t.resource[rslt.resourcetype]+'&nbsp; &nbsp; tamam.';
		actionLog ('kabul edildi '+gift.name+': '+ rslt.gold +' gold and '+ rslt.resource +' '+ t.resource[rslt.resourcetype]);
      ...
      if (rslt.msg)
        ...
        msg = '<SPAN class=boldRed>imkansız: '+ rslt.ajaxErr +'</span>';
     ...
     new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/train.php" + unsafeWindow.g_ajaxsuffix, {
       ...
       onSuccess: function(transport) {
         ...
         if (rslt.ok) {
           ...
           actionLog(logcity  + ' eğitimde ' + idle + ':  ' + troops[unitId] );
