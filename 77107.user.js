...
if (firstRun)
{
  myimbabutton.innerHTML = "Whooping some ass =)";
  cordN = 1;
 ...
 check = false;
  ...
  if ((totTroops[x] * cord.length)  > getTotalUnit('t'+tempX) ) {errorMsg("not enough troops! (Troop #" + x+ ")"); abort(); return;}
  if (totTroops[x] > 0) {check = true;}
  }
  if (!check)
  {
  errorMsg("No troop input");
  abort();
  return;
  }
...
attackInterface.innerHTML = '<fieldset><legend>Attack setup:</legend><table><tr><td><div id=start>' +
  '<table><tr><td>Attack type:</td></tr>'+
  '<tr><td>' +
 	'<select name=\"typeAttack\" id=\"typeAttack\">' +
 	'<option value=\"3\">Normal</option>' +
 	  '<option value=\"2\">Reinforcement</option>' +
    	'<option value=\"4\">Raid</option>' +
    	 	'<option value=\"5\">Scout Res/Troops</option>' +
   	      '<option value=\"6\">Scout Def/Troops</option>' +
 	'</select>' +
  '</td></tr><tr><td> Cords**</td>'+	
  '</tr><tr><td>'+
	'<input type=\"text\" value="' +getCords() +  '" name=\"cords\" id=\"cords\" />' +
	'</td><td>'+
	'</td></tr><tr><td>'+
  	'<button id=\"myimbabutton\" >Whoop some ass! =)</button>' +
  	'</td><td>'+
  	'<button id=\"arrivalTime\">Arrival time</button>'+
  	'</td><td>'+
  	'<div id=\"arrivalTimeDiv\"></div>'+
  	'</td></tr><tr><td>' +
  	'Arrive at:'+ 
    '</td></tr><tr><td>'+
  	'<input id=\"timedArrivalInput\" value=\"hh:mm:ss"><button id=\"timedArrivalButton">Set timed arrival</button>'+
	'</td></tr></table><p style="font-size: 75%">* - number of attacks of that specific wave</p><p style="font-size: 75%">** - seperate with \'|\' ex: 0,0|1,1</p>';

interfaceStart.appendChild(attackInterface);


//angrepsbУИlge interface 
function addNewWave()
{
...
if (tag.snapshotLength)
{
	...
	errorMsg("Unable to get active village. Assuming one village account");
  return "";
    }
}
...
if (tag.snapshotLength)
  {
	document.getElementById('arrivalTimeDiv').innerHTML = "Arrvial time:" + tag.snapshotItem(0).innerHTML;
...
if (diffTime >= 1)
{
 ...
 minutes = count[2];
 seconds = count[3];
 seconds = parseInt(seconds) + parseInt(diffTime);
 if (seconds >= 60)
 {
 minutes++;
 seconds = seconds - 60;
