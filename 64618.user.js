...
// @description    Avanturist.org forum PATCH (unpacked)
// @version        0.11.18
...
var scriptVersion = "0.11.18";
var scriptDate = "28.08.2011";
var app;
...
XPathHelper.divMain             = "./div[@class='main']";

XPathHelper.divMenuSite         = XPathHelper.divMain + "/div[ @class='topMenu' and div[position()=1 and @class='link'] ]";
XPathHelper.divMenuUser         = XPathHelper.divMenuSite + "/following-sibling::div[1]";
XPathHelper.tableMenuUser       = XPathHelper.divMenuUser + "/table[not(@class)]";
XPathHelper.tdCabinet           = "./tbody[1]/tr[1]/td[2]";
...
XPathHelper.divContainer        = XPathHelper.divMain + "/div[@class='container']";
XPathHelper.divContent          = XPathHelper.divContainer + "/div[@class='content-leftcenter']";

XPathHelper.tableMenuForum      = XPathHelper.divContent + "/table[1]";
...
XPathHelper.tableBoardTable     = XPathHelper.divContent + "/table[contains(@class, 'boardTable')]";
...
XPathHelper.trBoardContent      = "./th/table[@class='boardTable']/tbody[1]/tr[1]";
// Тег SMALL, содержащий список модераторов раздела или ветки
XPathHelper.smallModerators     = XPathHelper.trBoard + "/th[1]/table[@class='boardTable']/tbody[1]/tr/th/small | " +
...
XPathHelper.divBottom           = XPathHelper.divMain + "/div[@class='bottom']";
// BR после строки "SMF © 2006, Simple Machines LLC"
XPathHelper.brCopyright         = XPathHelper.divBottom + "/br[1]";

XPathHelper.divMessages         = XPathHelper.divContent + "/div[@class='textwoshadow']/div[@id='messagesDiv']";
XPathHelper.msgTable            = XPathHelper.divMessages + "/table[@class='table']";
XPathHelper.msgTableBody        = XPathHelper.msgTable + "/tbody[1]";
...
XPathHelper.tbodyMessageForm    = "./table[2]/tbody[1]/tr[2]/td[1]/table[1]/tbody[1]";
XPathHelper.textareaMessage     = XPathHelper.tbodyMessageForm + "/tr/td[2]/textarea[@name='message']";
...
function CBoard(boardTr, boardIdx) {
    ...
    for (var i = boardTr.rowIndex + 1; i < rows.length; ++i) {
        var topicTr = rows[i];
        if (topicTr.cells.length != 6 && topicTr.cells.length != 7) {
            break;
        }

        var topic = new CTopic(this, topicTr);
        if (topic.isRead || this.collapseAlways) {
...
function CTopic(board, topicTr) {
    ...
    this.board = board;
    this.topic = topicTr;
    this.isRead = topicTr.cells[1].innerHTML.indexOf("new_post.gif") == -1;
...
function main(e) {
    ...
    function CMenuControl() {
        ...
        // Верхнее меню форума с пунктами "Голосование", "Модерация", "Участники", "Поиск"
        ...
        this.menuUser = $x1(XPathHelper.divMenuUser);
...
//            }
    ...
    CPageView.prototype = {
        ...
        addEditButtons: function() {
            ...
            this.textArea  = $x1(XPathHelper.textareaMessage, $id("postmodify"));
            if (this.textArea == null) {
                log.error("the text area wasn't found");
                return;
            }

            var br = $x1(XPathHelper.brToolbar, $id("postmodify"));
            if (br == null) {
    ...
    app = {
        ...
        run: function() {
            ...
            var isTopic = false;
            if (/index\.php.+topic[=,]/.test(url) || /forum\/topic\/\d+/.test(url)) {
                isForumPage = true;
                isTopic = true;
                handleTopicPage();
            } else if (/index\.php.+board[=,]/.test(url)) {
...
if (/^https?:\/\/(www\.)?avanturist.org(\/|\?|$)/.test(url)) {
    if (isOpera) {
