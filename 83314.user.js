...
// @name MyTT
// @namespace MyTT
// @description  Show 10 most repeated words from your TL or any other twitter profile. With RaphaelJS pie-chart.
// @include        http://twitter.com/*
// ==/UserScript==

//Beta version... much to improve.


//Get the jQuery already in Twitter:
$ = unsafeWindow.$;
...
//Based on the Pie Chart example in RapahelJS.com
    ...
    var angle = 0,
        ...
        process = function (j) {
            var value = values[j],
                ...
                txt = paper.text(cx + (r + delta + 55) * Math.cos(-popangle * rad), cy + (r + delta + 25) * Math.sin(-popangle * rad), labels[j]).attr({fill: "#05a", stroke: "#fff", "stroke-width": .5, opacity: 0, "font-family": 'Fontin-Sans, Arial', "font-size": "20px", "font-weight " : "900" });
            p.mouseover(function () {
	   $("#myTTchart").css('overflow','visible');
                p.animate({scale: [1.1, 1.1, cx, cy]}, ms, "elastic");
                txt.animate({opacity: 1}, 200);
            }).mouseout(function () {
                p.animate({scale: [1, 1, cx, cy]}, ms, "elastic");
                txt.animate({opacity: 0}, ms);
	   $("#myTTchart").css('overflow','hidden');
            });
            angle += angleplus;
            chart.push(p);
            chart.push(txt);
...
/*
@Author: Rodolfo Lopez (@rodolfolopex on twitter)
@Requires: jQuery 1.1.3+
@Version: 1.0
@Description: Creates a bigger div to work with canvas/raphael. 
Dual licensed under the MIT (http://www.opensource.org/licenses/mit-license.php)
and GPL (http://www.opensource.org/licenses/gpl-license.php) licenses.
*/

function bigCanvas(el,more){

var ctop=Math.round($("#"+el).offset().top)-more;
var cleft=Math.round($("#"+el).offset().left)-more;
var cwidth = $("#"+el).width()+(more*2);
var cheight = $("#"+el).height()+(more*2);

//create actual bigger canvas div
var c = document.createElement('DIV');
c.id=el+"_bigcanvas";
c.style.position = "relative";
c.style.top = "-"+more+"px"; //ctop+"px";
c.style.left = "-"+more+"px"; //cleft+"px"
c.style.width = cwidth+"px";
c.style.height = cheight+"px";
//c.style.border = "1pt solid red";
c.style.zIndex = 1;

document.getElementById(el).appendChild(c);

return {id: c.id, width: cwidth, height: cheight}
}

var RapahelReady = false;
var paper;
var TTinit = true;

//hide twitter recomendations.
$("#recommended_users").hide();
$("#similar_to_followed").hide();

try{ if(Raphael!="") RapahelReady = true; }
catch(e){}

function MyTTStart() {

	// URL: Profile
    if($("#follow-button-on-profile-page")){
	unsafeWindow.MyTT = MyTT;

//"More" button on top: 
	$("#timeline_heading").after("<p><div><a href='#' id='more_top' class='round more'>More</a></div>"); 
	$("#more_top").live('click', function(){ $("#more").trigger('click'); });


	$("#more").live('click', function(){   
		setTimeout("MyTT();",2000);
	});
...
function MyTT(){
  ...
  if(spans[i].className == "entry-content")
	twitts[i]= i+ " : " + spans[i].textContent;
  }
//alert(twitts.length);
// - - - - - - --  -- - -  - - -  -- - - - - -    - - -  -   -   --    - -
var wordRegExp = /\w+(?:'\w{1,3})?/g; //old
//var wordRegExp = "(?<=#)\w*?(?=\W)";
var words = {};
var matches;
var aList;
while ((matches = wordRegExp.exec(twitts)) != null){
...
for (var word in words)
    if (words.hasOwnProperty(word) && word.length>3 && words[word] > 1)
...
var htmlTTList = []; 
var rtotal=0;
for(i = 0; i < ( wordList.length >= 10 ? 10 : wordList.length ); i++){ rtotal+= wordList[i][1]; }
for(i = 0; i < ( wordList.length >= 10 ? 10 : wordList.length ); i++){
    percent = Math.round(((wordList[i][1]*100)/rtotal));
    htmlTTList[i] = '<li class="link-title"><a href="http://twitter.com/search?q='+wordList[i][0]+'" class="search_link" rel="nofollow">'+wordList[i][0]+' \n'+percent+'% ('+wordList[i][1]+' of '+wordList.length+')</a> <input type="hidden" value="'+percent+ '%"></li>';
}
	//alert(htmlTTList.join("\n"));

var mtC = false;
try{ mtC = ($.cookie("menus").indexOf("mtC")>-1)?true:false; }catch(e){};
var chart_display = mtC?"display:none;":"";

	if(TTinit){
	        var myTTList = document.createElement('DIV');
		myTTList.id = "myTTList_div";
		myTTList.className = mtC?"collapsible collapsed":"collapsible";
		//document.getElementById("primary_nav").appendChild(document.createTextNode("MightyT"));
		document.getElementById("primary_nav").appendChild(myTTList);
		$("#myTTList_div").html('<h2 class="sidebar-title" id="mt_menu"><span>MyTT (Beta)</span></h2> <div id="myTTchart" style="width:200px;height:200px;overflow:hidden;'+chart_display+'"></div> <ul id="DTTList" class="sidebar-menu trends-links">'+ htmlTTList.join("\n") +  '</ul>');
	}
	else {
	$("#myTTList_div").html('<h2 class="sidebar-title" id="mt_menu"><span>MyTT (Beta)</span></h2> <div id="myTTchart" style="width:200px;height:200px;overflow:hidden;'+chart_display+'"></div> <ul id="DTTList" class="sidebar-menu trends-links">'+ htmlTTList.join("\n") +  '</ul>');
	}
		$("#myTTList_div").isCollapsibleMenu().click(function(){ if($(this).hasClass('collapsed')) $('#myTTList_div #myTTchart').slideDown(100); else  $('#myTTchart').slideUp(100); });


if(RapahelReady){
	values = [];
	labels = [];
	$("#myTTList_div li").each(function () {
		values.push(parseInt($("input", this).val(), 10));
		labels.push($("a", this).text());
	});

	acanvas = bigCanvas("myTTchart",200);
	paper = Raphael(acanvas["id"], acanvas["width"], acanvas["height"]);
	paper.pieChart(Math.round(acanvas["width"]/2), Math.round(acanvas["height"]/2), 85, values, labels, "#EEE");
		...
		$('#mt_menu').click(function(){  
			$('#myTTchart').toggle(); 
			$('#DTTList').toggle(); 
			//$('#myTTList_div').toggleClass('collapsed');
		//slideDown(100,function(){D.removeClass("collapsed");
		});
		*/
