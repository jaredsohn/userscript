// ==UserScript==
// @name           Удафф Вконтакте
// @namespace      http://aero.3dn.ru
// @description    Функционал вконтакте для Удаффов
// @include        http://vkontakte.ru/*
// ==/UserScript==
// Author: Артём Ласовский
// Version: 2.01
// Site: aero.3dn.ru
...
// === Changelog
// 2.01
// Исправлена ошибка: Скачиваение музыки с результатов поиска
// Исправлена ошибка: Редактирование заметок
// ----------
// 2
// Исправлена ошибка: сбивается поисковый запрос при переходе на другую страницу.
...
if (checkUpdates || checkVKUpdateNow) {
	...
	if (checkVKUpdateNow)
		showDialog('Обновление Удафф Вконтакте','<center><h1><font color="#444444"><b>Поиск Удаффьих обновлений</b></font></h1></center>','','Скрыть','',hideFunc());
...
function ShowNewVersionDialog(version,changelog) {
	var title = "Обновление Удафф Вконтакте";
	var text = '<h1><font color="#444444"><b>Доступна новая версия Удафф Вконтакте '+version+'</b></font></h1><br><br>Изменения в новой версии:<ul>';
	...
	showDialog("Обновление Удафф Вконтакте",text,"Обновить","Отмена",hideFunc()+"location.href='http://aero.3dn.ru/.....'",hideFunc());	
}
function ShowNoNewVersion(version) {
	var title = "Обновление Удафф Вконтакте";
	var text = '<center><h1><font color="#444444"><b>Твоя версия Удафф Вконтакте '+version+' является самой последней</b></font></h1></center>';
	showDialog("Обновление Удафф Вконтакте",text,'',"Ок",'',hideFunc());
}
...
if (/\/video[-0-9]/.test(location.pathname) && downVideo) {
	...
	document.getElementById("videoactions").innerHTML += "<a href='"+link+">Скачать Видео</a>";
}
...
if (location.pathname == '/profile.php' || location.pathname.substring(0,3) == '/id') {
	...
	if (document.getElementById("opinions") != null && confirmOpinion) {
		...
		element.innerHTML = element.innerHTML.replace(/<a href=\"javascript: postOpinion\(\)\">([^]+)<\/a>/,"<a href=\"javascript:if(confirm('Удафф действительно хочет отправить анонимное сообщение?)')){postOpinion();}\">$1<\/a>");
	}
...
function showReplyBox() {
	if (xmlhttp.readyState == 4) {
		...
		showDialog('Сообщение','<table>'+content+'</table>','Закрыть','Отправить',"document.getElementById('replyBoxOpened').value = 'false';"+hideFunc(),"this.disabled=true; if (ge('reply_to_mess').value.length > 1) {ge('postMessage_2').submit();} else {alert('Где текст Удафф?.'); return false;};"+hideFunc(),485);
	}
}
...
function showNotFound() {
	document.getElementById('bigResult').innerHTML += '<div class="fallBack">Нету ничего такого Удафф =(</div>';
}
...
if (location.pathname == '/settings.php') {
	...
	if (GET['act'] != 'Удафф Вконтакте') {
		// Добавляем нашу вкладку
		menuElements.innerHTML += '<li>'+settInActiveLink('Удафф Вконтакте','?act=vkPatch','4.8em')+'</li>';
		...
		// Добавляем нашу вкладку
		menuElements.innerHTML += '<li class="activeLink">'+settActiveLink('Удафф Вконтакте','?act=vkPatch','4.8em')+'</li>';
		
		editorPanelElements.innerHTML = "     <div id=\"cname\" class=\"settingsPanel\">"+
		"<h4>Удафф Вконтакте Настройки <small><font color='#98A7B5'>версия "+vkpathVersion+"</font></small></h4><div align='right'><small><a href='?checkVKUpdateNow=1&act=vkPatch'>Проверить обновление</a></small></div>"+
		"<form method=\"GET\" id=\"vkpatchSettings\" name=\"vkpatchSettings\" action=\"\"><input type=\"hidden\" name=\"act\" value='vkPatch'><input type=\"hidden\" name=\"vkpatch\">"+
		"<table class=\"editor\" style=\"margin-left: 0px; width: 420px;\" border=\"0\" cellspacing=\"0\">"+
		"<tbody>"+

			settCheckbox('useLinks',			'Ссылки',				'Ссылки на разедлы аккаунта с закрытйо страницей')+
			...
			settCheckbox('downAudio',			'Скачать mp3',			'Для тебя Удаффчик =*).')+
			...
			settLine('Обновление')+
			settCheckbox('checkUpdates',		'Обновление',			'Проверять наличие новых версий Удафф Вконтакте.')+		
...
function showMessageSettSaved() {
	...
	message = 'Настройки Удафф Вконтакте сохранены.';
...
function setParams(arr) {
	for (i in arr) {
		if (arr[i]['default'] === true || arr[i]['default'] === false) {
			...
			if (GET['Удафф Вконтакте'] != undefined) 
...
if (document.getElementById('vkontakte')) {
	...
	link.innerHTML = '<a href=\"http://aero.3dn.ru" target=\"_blank\">Удаффий сайт</a>';
