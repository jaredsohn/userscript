// ==UserScript==
// @name           Seedz!
// @namespace      modified by Sergey, originally by xbtc
...
(function()
{
	...
	var _protos={
		...
		"gazelle":
		{
			...
			userLink:			"/user.php",
			userIdRegexp:		/id=([0-9]+)/,
			snatchPage:			new RegExp("torrents.php\?.*type=seeding.*"),
	...
	var _defs={
		...
		"deepbassnine.com":			function(){return _extend(_protos["gazelle"],
									{
										siteName:	"DEEPBASSNiNE",
										svPrefix:	"db9"
									});},
		"lztr.us":					function(){return _extend(_protos["gazelle"],
									{
										siteName:	"LzTr",
										svPrefix:	"lz"
									});}
	...
	if(!defs)
	{
		GM_registerMenuCommand("Seedz! couldn't find defs for this site...", function(){alert("This means there's something curious going on. Contact the developer, maybe?")});
		return;
	}
	...
	var menuCmdPrefix="Seedz! ("+defs.siteName+"): #"+userId+" ("+userName+"): ";
	...
	if(snatchLink&&!onSnatchPage)
	{
		if(defs.canShowHelp)
		{
			...
			helpDiv.innerHTML="<b>CAN HAS SNATCHES?</b><br/><br/>Hey "+userName+", this is Seedz! talking. You might want to click <b>My Snatches</b> to make sure I know of all the things you've downloaded and keep track of them.<p/>Don't worry, this popup won't appear once I know what you've been downloading.";
	...
	if(validLinks.length)
	{
		...
		for(var i=0;i<validLinks.length;i++)
		{
			...
			if(res)
			{
				...
				{
					validLinks[i].innerHTML+=" <span style=\"color:white;font-weight:bold;background:green\" title=\"ID = "+res+"\">SEEDING</span>"; //  "+res+"
		...
		if(snatchMode)
		{
			...
			if(ibn)
			{
				...
				var text=
					...
					"This script knows of <b>"+known+"</b> torrents being seeded.";
				if(uploadsAreSnatches) text+="<br/><b>Your uploads are currently also counted as snatches.</b><br/>";
				text+="<br/>To get the script to recognize which torrents you are seeding, you must go through every page of your seeds page. If that count changes, the script will be out of date and you will have to repeat the process (yes that does suck). If you want the script to forget what all you are uploading, go to Tools > GreaseMonkey > Script Commands > Forget. This script is a modified version of Snitch from wfm.hostei.com.</div>";
				infoDiv.innerHTML=text;
				ibn.parentNode.insertBefore(infoDiv,ibn.nextSibling);
			}
			else
			{
				document.title+=" (Seedz!: "+out.length+")";
			}
