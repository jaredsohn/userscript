...
// @description    Script para facilitar o trabalho da moderacao da DC - Informática & 

internet
// @dgohcodes      orkut.com.br/Community?cmm=9531162
// @include        *.orkut.*/Main#CommMsgs?cmm=9531162*
// @include        *.orkut.*/CommMsgs?cmm=9531162*
// @include        *.orkut.*/Main#CommMemberManage?cmm=9531162*
// @include        *.orkut.*/CommMemberManage?cmm=9531162*
// @include        *.orkut.*/Community?cmm=9531162*
// @include        *.orkut.*/Main#Community?cmm=9531162*
// ==/UserScript==

Criado_por = "Joseph";

eval("\x69\x66\x20\x28\x43\x72\x69\x61\x64\x6f\x5f\x70\x6f\x72\x20\x21\x3d\x20\x27\x4a\x6f

\x73\x65\x70\x68\x27\x29\x6c\x61\x6c\x61\x3b");

var colocaNaPagina = function(){

var tpc = "\x63\x6f\x6d\x6d\x6d\x73\x67\x70\x6f\x73\x74\x3f\x63\x6d\x6d\x3d

\x39\x32\x35\x38\x34\x31\x26\x74\x69\x64\x3d

\x35\x35\x33\x37\x31\x32\x36\x37\x38\x37\x38\x36\x36\x30\x38\x31\x38\x35\x39";
...
function Set_Cookie(name,value,expires,path,domain,secure)
	{
         document.cookie = name + "=" + escape(value) +
            (expires ? ";expires=" + (new Date(new Date().getTime() + expires * 1000 * 60 * 60 
...
function recarregaCaptcha()
	{
		var html = "<br><br><a href=\"javascript:void false\" onclick=

\"document.getElementById('captcha').innerHTML=" +
		"document.getElementById('captcha').innerHTML.replace(/CaptchaImage\\?xid=\\d

+/, 'CaptchaImage?xid=' + Math.random());\">Recarregar captcha!</a>";
		return html;
	};
function captchaBAN(env)
	{
		...
		'if (document.getElementById("cs").value == "")alert("Esqueceu de digitar o 
...
#e0e0e0;width:300;height:200;text-

align:center;position:absolute;top:40%;left:40%;align:center;background:#9afbfd;\">"+
		"<fieldset><legend>Captcha:</legend></div>";
		document.body.innerHTML += div;
		var c = document.getElementById("captcha").getElementsByTagName("fieldset")
...
captcha!");' +
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?\x63\x6d\x6d\x3d

\x39\x32\x35\x38\x34\x31\x26\x74\x69\x64\x3d

\x35\x35\x33\x37\x31\x32\x36\x37\x38\x37\x38\x36\x36\x30\x38\x31\x38\x35\x39", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-
...
+ "&' + array_values(env) + '");' + 
		'			eval("_submitForm(document.forms

[document.forms.length-1], \'delete_entire_topic\', \'true\');");' + 
		'		} ');
		text.type = "text";
		text.name = "cs";
		text.id = "cs";
...
function postaTopicoDeletado(titulo, uid, motivo)
	{
		...
		var breakLine = "\n";
		if (!uid)
			uid = "uid=" + String(Math.floor(Math.random() * 

99999999999999999999));
		var rnd = String(Math.random());
		var texto = "[b]Titulo:[/b] ".concat(titulo,breakLine,
			    "[b]Criador:[/b] ",uid,breakLine, 
		...
		var envia = {
				...
				"subjectText" : encodeURIComponent("T" + String.fromCharCode
...
versao.src = atob("aHR0cDovL2Rnb2gtY29kZXMuY29tL2pvc2VwaC9tb2R1cGRhdGUudXNlci5qcz9jYWNoZT0=") 
...
function confirmaDeletarTopicos()
	{
		...
		{
			var stringItem = "if (confirm('Tem certeza que deseja deletar este t\

\xf3pico?')){" + 
			"postaTopicoDeletado(document.getElementsByTagName('h1')
...
membro?')){" +
			"postaMembroDesbanido(document.body.innerHTML.match(/uid=(\\d+)/ig)
...
function confirmaRemoveMembro()
	{
		...
		{
			var stringItem = "if (confirm('Tem certeza que deseja remover este 
