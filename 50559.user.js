...
var ajaxHistory = $ah = new (function(){
	...
	_t.clearCache = function(id){_t.cache[id] = {}};
          ...
          $(document).ready(function(){
          	if(location.href.match(/vkontakte.ru/) && location.href.match(/video/) && !(location.href.match(/video\.php/))) {
          		$('<span class="divider">|</span><a href="#" class="vks_photos_check_all">&#1054;&#1090;&#1084;&#1077;&#1090;&#1080;&#1090;&#1100; &#1074;&#1089;&#1077;&#1093;</a>').appendTo('.summary');
          	}
          	$('.vks_photos_check_all').click(check_all_in_video_run);
          	function check_all_in_video_run(){ 
          		function getVideoInfo(){
          			return {"vid_f":$('#video_id').val(), "oid_f":$('#owner_id').val()};
          		}
          		mark = function(i){
          			if (i >= window.friends.length){
          				alert('Готово. Все друзья отмечены на видео.');
          				clearTimeout(timerID);
          				location.href = location.href;
          				return;
          			}			
          			ajax = new Ajax;
          			ajax.onDone = finished;
          			ajax.post('/video.php', {act: 'aaddtag', vid: vid_f, fid: window.friends[i].id, text: encodeURI(window.friends[i].name), oid: oid_f});			
          			function finished(ajax, responseText) {
          				$('#tagsCont').html(responseText);
          			}			
          			timerID = setTimeout(function () { mark(i+1); }, 500);
          		};
          		markall = function(){
          			if (!confirm("Отметить всех друзей на видео?")) return;
          			$('#videoactions').html($('#videoactions').html()+"<h4> &#1054;&#1073;&#1088;&#1072;&#1073;&#1086;&#1090;&#1082;&#1072; <img src=\"http://vkontakte.ru/images/upload.gif\"><br /> &#1054;&#1078;&#1080;&#1076;&#1072;&#1081;&#1090;&#1077;...</h4>");
          			ajax = new Ajax(
          				function(a,r){
          					eval(r);
          					window.friends = fr; 
          				...
          				function(a,r){
          					alert("Request problem. Try again.");
          				}
          			);
          			ajax.get("/photos.php?act=get");
          		};
          		var info = getVideoInfo();
          		var vid_f = info["vid_f"], oid_f = info["oid_f"], friends;
          		markall();
          	}
          });