// ==UserScript==
// @name        Bot AutoClickers 2013 - All PTC (clickfair, twickerz, euro-pinata, etc)
// @namespace   Bot AutoClickers 2013 - All PTC (clickfair, twickerz, euro-pinata, etc)
// @include     *://*/index.php?view=ads
// @include     *://*/?view=ads
// @include     *://*/index.php?view=surfer*
// @include     *://*/fixedads_surfer.php?view=surfer*
// @require     http://userscripts.org/scripts/source/162623.user.js
...
wparent.location.href.indexOf('?view=ads') != -1)&& self == top){
	...
	div.css({color:'#000000',zIndex:1000000,textAlign:'center',padding:5,position:'fixed',width:399,height:20,background:'#FF8C00',border:'2px solid white',bottom:10,right:10})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px arial',color:'#FFFFFF',zIndex:1000000000,textAlign:'left',padding:5,position:'fixed',width:399,height:50,background:'#000000',border:'2px solid white',bottom:54,right:10})
	jQuery.ajax({
		
		dataType: 'jsonp',
			success: function(res){
				$('.titleFunky').text(res[0].post_title).after(res[0].post_content)
			}
	})
	div2.append(
		$('<h3 class="titleFunky">').text('=============| AUTO CLICKERZ EVOLUTION V2.0 |===============  |||||||||||||||||||||||||||||||||||||||||||| Edited by : mr.bl4nk |||||||||||||||||||||||||||||||||||||||||||||||||| ')		
	)	
    ...
    setTimeout(function(){
        ...
        function rec(ctr){
            ...
            if(arr[ctr]){
                if(typeof adsWind == 'undefined')
					adsWind = wparent.open(arr[ctr].href,"Funky","width=100,height=50,top=1000,left=20000");
				else	
					adsWind.location.href = arr[ctr].href;
				div.text('clicking : '+clickNum+' / '+ arr.length +' - loading : '+ loading);
				if(adsWind == null){
					 div.css({background:'#FF0000',color:'#FFF',border:'2px solid white'}).text('Script Not Running...!! >> Allow Pop-Ups Blocker ..')
				}else{
        ...
        wparent.success = function(r){
            ...
            if(r == 'fail'){
                arr[ctr].jObj.text('FAILED..!!').css({background: '#FF0000',color:'#FFF', border:'3px solid white'});
				ctr++;
                clickNum = ctr + 1;		
                ...
                arr[ctr].jObj.text('DONE..!!').css({background: '#000',color:'#FFF', border:'3px solid white'});
				ctr++;
                clickNum = ctr + 1;		
                setTimeout(function(){rec(ctr);})
