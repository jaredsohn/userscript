// ==UserScript==
// @name           13oNator v0.49
// @author         Spocklet Team
// @description    Edit version Assassin-a-Nator
// @credits        Spockholm
// ==/UserScript==
/*
	Mod by Raimi Syazwan
	For education and java learning
	Retrying to heal min 5 sec
	Reloading fight list 2 sec
	Reloading rivals 2 sec
	Reloading Family Battle targets 2 sec
...
javascript:(function(){
	...
	if (/m.mafiawars.zynga.com/.test(document.location)) {
		...
		//Credits to Toenailsin for this new unframe fix
	...
	'<div id="'+spocklet+'_main">'+
		...
		'<tr>'+
			...
			'<td align="right" style="text-align:right;">'+
				...
				'Burst: <input id="'+spocklet+'_burst" title="Zynga multi-click, 1-7 valid. In case of revolvers, lower the value" type="text" value="5" maxlength="1" name="'+tagname+'" />'+
			'</td>'+
		'</tr>'+
		//'<tr><td>Zombie Status:</td><td colspan="2"><span id="'+spocklet+'_zombie"></span></td></tr>'+
...
//				'<input id="'+spocklet+'_exp_level" type="text" value="0" maxlength="4" name="'+tagname+'" /> exp to level&nbsp;&nbsp;'+
		...
		'<tr>'+
			...
			'<tr id="'+spocklet+'_filters" style="display:none;"><td></td><td colspan="2">'+
				...
				// '<tr>'+
					...
					// '<td><span class="more_in">Humans only attack Zombies.<br />Zombies attack everyone when infestation is Critical.</span></td>'+
				// '</tr>'+
				'<tr>'+
	...
	function banker(cash,city) {
		if (cash < 10) {
			log(timetamp()+'<span class="bad">You need to deposit at least $10!</span>');
	...
	function logtrunc(message) {
		...
		var keep = /failsafe|bank can only hold|You cannot deposit any more|Turkey stolen/;
	...
	*/
	...
	function do_attack() {
		if (targets.length == 0 && fightlist == 'fightlist') {
			...
			targets = [];
			pausing(1,'Ran out of targets, reloading fight list in',function(){ load_fightlist(); });
			return;
		}
		...
		if (targets.length == 0 && fightlist == 'rivals') {
			targets = [];
			pausing(1,'Ran out of targets, reloading rivals in',function(){ load_rivals(); });
			return;
		}
		if (targets.length == 0 && fightlist == 'battle') {
			targets = [];
			pausing(2,'Ran out of targets, reloading Family Battle targets in',function(){ load_battle(); });
			return;
		}
		if (run) {
	...
	function parse_attack(response) {
		...
		if (fightResult) {
			...
			if (fightResult.cash_class == 'brazil' || fightResult.cash_class == 'chicago' ) {
				multiplier = 5;
			}
			...
			//Zombie Event
			if (Util.isset(fightResult.user_stats) && Util.isset(fightResult.user_stats.fight_mastery_div) && /YOU ARE (INFECTED|HUMAN)/i.test(fightResult.user_stats.fight_mastery_div)) {
				var img = $(fightResult.user_stats.fight_mastery_div).find('img[title="status"]').attr('src') || '';
				var points = $(fightResult.user_stats.fight_mastery_div).find('div.fightbar_skill:contains("Points:")').text() || '';
				var status = $(fightResult.user_stats.fight_mastery_div).find('.fightbar_text > span:last').text();
				var statuscolor=(status=="Low"?'green':(status=="Critical!"?"red":(status=="Medium"?"yellow":"orange")));
				$('#'+spocklet+'_zombie').html('<img src="'+img+'" width="18" height="18" />'+points+' Status: <span style="color:'+statuscolor+';font-weight:bold;">'+status+'</span>');
				if ($('#'+spocklet+'_autozombie').is(':checked')) {
					if (/YOU ARE HUMAN/i.test(fightResult.user_stats.fight_mastery_div)) {
						$('#'+spocklet+'_badge').val('101');
					}
					if (/YOU ARE INFECTED/i.test(fightResult.user_stats.fight_mastery_div)) {
						if (status == 'Critical!') {
	...
	function parse_hospital(hospital) {
		...
		if (/You cannot heal so fast/.test(object.hospital_message)) {
			pausing(5,'Retrying heal in ...',function(){ repeat_attack(); });
			return;
		}
		else {
