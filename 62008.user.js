// ==UserScript==
// @name           Favorites Spy Reports TR 1.0 by wipau
// @namespace      ogc20-modules/
// @description    Link hatırlatıcı & title Spy report Only for GreaseMonkey (cookie in Opera can be rewrite gamedata)
// @version        1.0beta
// @include        http://*.ogame.com.tr/game/index.php?page=messages&session=*
// @exclude        http://uni*.ogame.com.tr/*
// @copyright      2009, wipau
// ==/UserScript==
var Utils = {
...
var Favors = {
	...
	saveSpyReport: function(id) {
		for (i = 0; i < Favors.Favor.l.length; i++) 
			if (Favors.Favor.l[i] == id) {
				unsafeWindow.fadeBox('Bu rapor zaten ekli');
				return false;
			}
	...
	ShowSpyReports: function() {
		...
		if (Favors.Favor.l.length > 0) {
			var tbl = unsafeWindow.$('<table cellspacing="0" cellpadding="0" id="mailz" class="ogc20Ftable list"><tr class="first"><th class="from">Den</th><th class="subject">Konu</th><th class="date">Tarih</th><th class="action"/></tr></table>');
			for (i = 0; i < Favors.Favor.l.length; i++) {
				var id = Favors.Favor.l[i];
				var el = Favors.Favor['s' + id];
				text = '<tr id="' + el.id + 'TRF" class="entry trigger alt"><td class="from">Donanma Komutanlığı</td><td class="subject">' +
				...
				'<a title="|Kaldır Listeden" class="ogc20Fdel tips" rel="'+el.id+
			...
			unsafeWindow.fadeBox('Rapor Listeye Eklendi');
		}
		return false;
	},
