// ==UserScript==
// @name           	Vejidas Leecher Checker
// @namespace      http://hightekcafe.com/ikariam
// @description    	A script for Ikariam that colors leechers
// @include       	 http://s*.ikariam.*/index.php*view=resource*
// @include       	 http://s*.ikariam.*/index.php*view=tradegood*
...
// Chrome compatibility under Blank Canvas Script Handler by AubergineAnodyne
// css
...
var options_menu = document.getElementById('resourceUsers').childNodes[1];
	...
	addGlobalStyle('a.vlctooptip span {display:none; padding:2px 3px; margin-left:1px; width:150px;}');
	...
	addGlobalStyle('a.vlctooptip:hover span{display:inline; position:absolute; background:#ffffff; border:1px solid #cccccc; color:#6c6c6c;}');
	...
	{
		var isNone = function(playerName) {
			var ret = /^(\s|&nbsp;)*$/.test(playerName);
			return ret;
                }
		...
		mill_number = 44;
			...
			workplace[20] = new Array( 420,   154304,    744658);
			workplace[21] = new Array( 448,   205012,    949670);
			workplace[22] = new Array( 478,   270839,   1220509);
			workplace[23] = new Array( 506,   311541,   1532050);
			workplace[24] = new Array( 536,   411229,   1943279);
			workplace[25] = new Array( 566,   506475,   2449754);
			workplace[26] = new Array( 598,   665201,   3114955);
			workplace[27] = new Array( 628,   767723,   3882678);
			workplace[28] = new Array( 660,  1007959,   4890637);
			workplace[29] = new Array( 692,  1240496,   6131133);
			workplace[30] = new Array( 724,  1526516,   7657649);
			workplace[31] = new Array( 758,  1995717,   9653366);
			workplace[32] = new Array( 790,  2311042,  11964408);
			workplace[33] = new Array( 824,  3020994,  14985402);
			workplace[34] = new Array( 860,  3935195,  18920597);
			workplace[35] = new Array( 894,  4572136,  23492733);
			workplace[36] = new Array( 928,  5624478,  29117211);
			workplace[37] = new Array( 964,  7325850,  36443061);
			workplace[38] = new Array(1000,  9011590,  45454651);
			workplace[39] = new Array(1036, 11085051,  56539702);
			workplace[40] = new Array(1072, 13635591,  70175293);
			workplace[41] = new Array(1110, 17704143,  87879436);
			workplace[42] = new Array(1146, 20630781,  96210217);
			workplace[43] = new Array(1182, 26786470, 122996687);
			workplace[44] = new Array(1220, 32948197, 155944884);
		}
		else
		{
		...
		mill_number = 36;
			...
			workplace[23] = new Array( 660,  1108885,   4993800);
			...
			workplace[31] = new Array( 990,  8185063,  37199981);
			workplace[32] = new Array(1035, 10314552,  47514533);
			workplace[33] = new Array(1080, 13588513,  61103046);
			workplace[34] = new Array(1126, 17122961,  78226007);
			workplace[35] = new Array(1166, 21576366,  99802373);
			workplace[36] = new Array(1214, 27187657, 126990030);
		}
		...
		for (var jj = 1; jj < donationList.rows.length; jj++)
		{
			...
			towns = 1;
			if (isNone(playerName)) {the_minus = the_minus+1;}
			// 					    	       			      Player /		 	Num of towns / 	
			if (isNone(playerName)) {
		...
		for (var j = 1; j < donationList.rows.length; j+=cities)
		{
			...
			var cities = 1;
			j3++;
			while ( (j3 < donationList.rows.length) && isNone(donationList.rows[j3].cells[0].innerHTML) )
			{
			...
			var j2 = j;
			j2++;
				while ( (j2 < donationList.rows.length) && isNone(donationList.rows[j2].cells[0].innerHTML) )
				{
			...
			if (sort_all_mills==2) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"vlctooptip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>"+Comma(Math.round(ordered_towns[j][6]))+" wood has been donated per town.</span></a>";
			}
			if (sort_all_mills==1) {
			 donationList.rows[j].cells[4].innerHTML="<a class=\"vlctooptip\" href=\"javascript:void(0)\">"+ordered_towns[j][4]+"<span>"+Comma(Math.round(ordered_towns[j][7]))+" wood has been donated per town.</span></a>";
			}
			...
			donationList.rows[j].cells[4].innerHTML += "<a class=\"vlctooptip\" href=\"javascript:void(0)\">"+tothree(newpercent)+"%<span>"+woodneed100+""+woodneed110+"</span></a>";
		}
		alltheoptions();
		...
		//$("#alloptions").filter(".vlctooptip").click( function() { alert("clicked"); } );
