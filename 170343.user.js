// ==UserScript==
// @name	ICE Quick Stream Pro + Last Update
// @version	6.0
...
function bad_content(content){
	...
	content.match(RegExp("\.(001|002|003|rar|zip) ?[\'\"<]","i"));
}
...
// 2shared
	...
	// check for bad link
	var content = document.getElementsByTagName('head')[0].innerHTML + document.getElementById('overall').innerHTML;
...
// mediafire
	...
	{
		var match = document.body.innerHTML.match(RegExp("[\'\">](https?://[0-9\:\.]{5,30}/[0-9a-z]{8,16}/[0-9a-z]{8,16}/.+?\.(?:mkv|ogm|divx|avi|mp4|flv|webm|mov))[\'\"<]","i"));
		if (match){
