// ==UserScript==
// @name           PassThePopcorn Advanced Buffer Calculator
// @namespace      http://jonls.dk
// @downloadURL    https://userscripts.org/scripts/source/158171.user.js
// @description    Show buffer in page header and on user pages.
// @author         Original Author of buffer calculator script: pinguen 
// @author         Author of Advanced Buffer Calculator: jonls
// @author         Adapted to PtP: dark_skeleton
// @author         Fixed to work with current PtP site: Veszerin
...
// @date           2013-06-29
// ==/UserScript==
//Edited to fit PassThePopcorn tracker by dark_skeleton
//Edited 2013-02-01 to updates in PassThePopcorn's code by Veszerin
...
(function() {
	...
	var RATIO_COLORING_MODEL = function(ratio) {
		return ratio / 2;
	};
	...
	function parse_binary_size(s) {
		...
		if (unit == 'KiB') amount *= KiB;
		else if (unit == 'MiB') amount *= MiB;
		else if (unit == 'GiB') amount *= GiB;
		else if (unit == 'TiB') amount *= TiB;
		else if (unit == 'PiB') amount *= PiB;
		return amount;
	}
	...
	function ratio_color(ratio) {
		...
		function lookup_color(gradient, index) {
			...
			for ( var i = 0; i < gradient.length; i++) {
				if (index < offset + gradient[i].size) return interpolate(gradient[i].color, gradient[i + 1].color,
						(index - offset) / gradient[i].size);
	...
	GM_registerMenuCommand('PassThePopcorn Advanced Buffer Calculator: '
			+ (display_in_header ? 'Hide buffer in header' : 'Show buffer in header'), function() {
	...
	GM_registerMenuCommand('PassThePopcorn Advanced Buffer Calculator: '
			+ (use_ratio_coloring ? 'Use simple ratio coloring' : 'Use advanced ratio coloring'), function() {
	...
	function insert_buffer(parent, up, down, save_name) {
		...
		/* Append buffer stats */
		var buffer_item = $('<li id="stats_buffer">Buffer: </li>');
		...
		if (use_ratio_coloring) buffer_amount.css('color', ratio_color(up / down));
			...
			buffer_amount.addClass(ratio_class(calculate_buffer(up, down, 1.0)));
	...
	if (/passthepopcorn\.me\/user\.php\?id=/.test(document.URL)) {
		/* Find stats box */
		...
		function append_transition_date(amount, name) {
			...
			stats_list
					.append($('<li><span title="' + binary_size_string(amount) + '">' + name
							+ ' upload</span> met: <span title="' + date_string + '">In ' + human_date_string
							+ '</span></li>'));
		}

		if (user_up < 40 * GiB) append_transition_date(40 * GiB, 'Member');
		else if (user_up < 80 * GiB) append_transition_date(80 * GiB, 'Power User');
		else if (user_up < 500 * GiB) append_transition_date(500 * GiB, 'Elite');
		else if (user_up < 1 * TiB) append_transition_date(1 * TiB, 'Torrent Master');
		else if (user_up < 5 * TiB) append_transition_date(5 * TiB, 'Torrent King');
		else if (user_up < 10 * TiB) append_transition_date(10 * TiB, '"Custom Class"');
	}

	/* Insert projected ratio for downloads */
	$('.torrent_table .torrent, .torrent_table .group_torrent').children('td').each(function(i) {
		var field = $(this);
		if (/^[\d.]+ (B|KiB|MiB|GiB|TiB|PiB)$/.test(field.text())) {
