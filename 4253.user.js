// ==UserScript==
// @name          Atlanta-Fulton County Georgia Library System
// @namespace     
// @description   Is this Amazon, Barnes and Noble, Powell's, or Netflix item (book, CD, DVD) available from the Atlanta-Fulton County Georgia Library System?
...
GreaseFrame.prototype = {
    ...
    load: function(response) {
        ...
        with(iframe) {
            ...
            contentDocument.write(response.responseText.replace(/<script(.|\s)*?>(.|\s)*?<\/script>/gmi, '').replace(/src\s*=\s*"(.|\s)*?"/gmi, ''));
...
Library.prototype.triple = function() {
    while(true) {
        var session = new String(Math.floor(Math.random() * 1000));
...
Library.prototype.search = function(success, failure) {
    this.location = this.location.replace(new RegExp(this.placeholderSession, 'g'), this.triple());
...
Library.prototype.parse = function(document) {
    ...
    this.foundHoldingListSize = countFor(this.xpath['holdingList'], document) - 1;
...
function LibraryGAFulton() { Library.call(this); }
LibraryGAFulton.prototype = new Library();
LibraryGAFulton.prototype.placeholderISBN13           = 'PLACEHOLDERISBN13';
LibraryGAFulton.prototype.placeholderISBN10           = 'PLACEHOLDERISBN10';
LibraryGAFulton.prototype.placeholderTitle            = 'PLACEHOLDERTITLE';
LibraryGAFulton.prototype.placeholderAuthor           = 'PLACEHOLDERAUTHOR';
LibraryGAFulton.prototype.locationSearchByISBN10      = 'http://afcatalog.co.fulton.ga.us/uhtbin/cgisirsi/x/0/0/5?searchdata1=PLACEHOLDERISBN10';
LibraryGAFulton.prototype.locationSearchByISBN13      = 'http://afcatalog.co.fulton.ga.us/uhtbin/cgisirsi/x/0/0/5?searchdata1=PLACEHOLDERISBN13';
LibraryGAFulton.prototype.locationSearchByTitleAuthor = 'http://afcatalog.co.fulton.ga.us/uhtbin/cgisirsi/x/0/0/5?srchfield1=TI^TITLE^SERIES^Title+Processing^title&searchoper1=AND&srchfield2=AU^AUTHOR^AUTHORS^Author+Processing^author&searchoper2=AND&searchdata1=PLACEHOLDERTITLE&searchdata2=PLACEHOLDERAUTHOR';
LibraryGAFulton.prototype.locationSearchByTitle       = 'http://afcatalog.co.fulton.ga.us/uhtbin/cgisirsi/x/0/0/5?srchfield1=TI^TITLE^SERIES^Title+Processing^title&searchdata1=PLACEHOLDERTITLE';
LibraryGAFulton.prototype.xpath['holdingList']     = '//td[@class="searchsum"]/table';
LibraryGAFulton.prototype.xpath['holding']         = '//div[@id="panel1"]/table';     
LibraryGAFulton.prototype.xpath['holdingLibrary']  = '//div[@id="panel1"]/table/table/tbody/tr/td[position()=1][@class="holdingsheader"]';
LibraryGAFulton.prototype.xpath['holdingCopy']     = '//div[@id="panel1"]/table/table/tbody/tr/td[position()=2][@class="holdingslist"]';
LibraryGAFulton.prototype.xpath['holdingMaterial'] = '//div[@id="panel1"]/table/table/tbody/tr/td[position()=3][@class="holdingslist"]';
LibraryGAFulton.prototype.xpath['holdingLocation'] = '//div[@id="panel1"]/table/table/tbody/tr/td[position()=4][@class="holdingslist"]';
...
Store.prototype = {
    ...
    renderBefore: function() {
        ...
        if(this.position == 'l') {
            this.div.style.left            = (contextPosition[0] - context.offsetWidth  / 2) + 'px';
            this.div.style.top             = (contextPosition[1] + context.offsetHeight / 2 - 15) + 'px';
            this.div.style.width           = (context.offsetWidth - 12) + 'px';
        }
        else {
            this.div.style.left            = (contextPosition[0]) + 'px';
            this.div.style.top             = (contextPosition[1] - 20) + 'px';
            this.div.style.width           = (context.offsetWidth - 12) + 'px';
        }
...
Barnes.prototype.xpath['context']  = '//*[@id="product-image"]/div[@class="look-inside-pdp"]//img';
Barnes.prototype.xpath['title']    = '//div[@id="product-info"]/div/h2/text()';
Barnes.prototype.xpath['author']   = '//div[@id="product-info"]/div/h2/em/a/text()';
...
Netflix.prototype.xpath['context']  = '//img[@class="boxShotImg"]';
Netflix.prototype.xpath['title']    = 'substring-before(concat(//h2[@class="title"]/text(), ":"), ":")';
Netflix.prototype.xpath['author']   = undefined; 
Netflix.prototype.xpath['actor']    = '//a[contains(@href,"RoleDisplay")]/text()';
Netflix.prototype.xpath['director'] = undefined;
...
Netflix.prototype.position          = 't';

if(window.location.toString().match(/^http:\/\/www\.amazon\.com\//))           { new Amazon (new LibraryGAFulton()); }
if(window.location.toString().match(/^http:\/\/[^\.]*\.barnesandnoble.com\//)) { new Barnes (new LibraryGAFulton()); }
if(window.location.toString().match(/^http:\/\/www\.powells\.com\//))          { new Powells(new LibraryGAFulton()); }
if(window.location.toString().match(/^http:\/\/[^\.]*\.netflix\.com\//))       { new Netflix(new LibraryGAFulton()); }