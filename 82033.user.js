...
// @creator        Vincenzo  Visciano (http://www.flickr.com/people/liberodicrederci/)
// @contributor    Alesa Dam (http://flickr.com/alesadam/): make it run on the new photo page layout + support for Google Chrome, and Opera
// @date           07/22/2010
// @version        2.5
// @modified       Jun 12, 2013
//
...
//
// @run-at	   document-end
// @downloadURL	   https://userscripts.org/scripts/source/82033.user.js
// @updateURL      https://userscripts.org/scripts/source/82033.meta.js
//
// ==/UserScript==
//
...
if (Browser.Engine.webkit || // Chrome, Safari
    ...
    var keyPrefix = 'FlickrPublishNow.';

    GM_log = function (message) {
        if (Browser.Engine.webkit) {
            console.info("FPN: " + message);
        } else {
            opera.postError("FPN: " + message);
        }
    }
...
var api_key = 'dbddeadbeab5a7aac29a8d52813cc538'; // the app's own key

if (debug) GM_log("DEBUGGING");

function showUpdateNotification(onlineVersion) {
    var color = 'white';
    ...
    new Element('a', {
        html: 'Flickr Publish Now!: ' + onlineVersion + ' available',
        href: Browser.Engine.webkit ? 
        ...
        target: Browser.Engine.webkit ? '_blank' : '',
        title: Browser.Engine.webkit ? 'to the scripts install page (opens in new tab)' : 'click to install new version',
        styles: {
            'color': color,
        ...
        events: {
            click: function () {
                if (!Browser.Engine.webkit) { // Firefox: install directly
                    this.innerHTML = 
    ...
    new Element('a', {
        ...
        href: 'http://www.flickr.com/groups/flickrhacks/discuss/72157624450469772/',
        styles: {
            'text-decoration': 'none'
        },
        target: '_blank'
...
function checkVersion() {
  try {
    ...
    if (!lastVersionCheckTime || elapsedtime / 1000 > 60 * 60 * 12) { //more then 12h ago
		getReplyInfo({ threadId: '72157634086550096', replyId: '72157634081280081', callback: function(retval) {
    ...
    if ($chk(onlineVersion)) {
        ...
        var currentVersionParts = reVersionMatch.exec(FPNversion);
        ...
        if (updateAvailable) {
            showUpdateNotification(onlineVersion);
        }
    }
  } catch (e) {
...
function idem(domid, photo, visibilityItem) {
    var formValue = $('' + domid).checked;
    var photoValue = photo.visibility[visibilityItem];
    if (formValue && photoValue === 1) {
        return true;
    }
    if (!formValue && photoValue === 0) {
        return true;
    }
    return false;
}
...
var runningRequests = 2; // setPerms and setDates
function reloadPage() {
    runningRequests--;
    if (runningRequests == 0) {
        $$('div.button-container')[0].empty();
        $$('div.button-container').adopt(new Element('small', { html: "reloading page.." }));
        document.location.href = document.location.href;
    }
}

function updatePrivacy(photoId, publishedCallback) {
    new Request({
        url: 'http://www.flickr.com',
        onSuccess: function (responseText, responseXML) {
            try {
                ...
                if (result.stat === 'fail') {
                    $('FPNpublic-' + photoId).getParent().getElements('img').each(function (img) {
                        img.src = images.error;
                    });
                    $('FPNprivate-' + photoId).getParent().getElements('img').each(function (img) {
                        img.src = images.error;
                    });
                    $('FPNdialog-' + photoId).adopt(new Element('span', { html: result.message}));
                    return;
                }
                $('FPNpublic-' + photoId).getParent().getElements('img').each(function (img) {
                    img.src = images.ok;
                ...
                $('FPNdialog-' + photoId).adopt(new Element('span', { html: e }));
                return;
            }
        },
        onFailure: function (response) {
            ...
            $('FPNdialog-' + photoId).adopt(new Element('span', {
                html: "error getting photo info using id " + photoId + ": " + response.statusText
            }));
        }
    }).get('/services/rest', {
            api_key: GM_getMagisterLudi(),
            ...
            nojsoncallback: 1,
            photo_id: photoId,
            is_public: ($('FPNpublic-' + photoId).checked ? 1 : 0),
            is_friend: ($('FPNfriends-' + photoId).checked ? 1 : 0),
            is_family: ($('FPNfamily-' + photoId).checked ? 1 : 0)
    });
}

function updatePostedDate(photoId, publishedCallback) {
    new Request({
        url: 'http://www.flickr.com',
        onSuccess: function (responseText, responseXML) {
            try {
                ...
                if (result.stat === 'fail') {
                    $('FPNdate-' + photoId).getElement('img').src = images.error;
                    $('FPNdialog-' + photoId).adopt(new Element('span', { html: result.message}));
                    return;
                }
                $('FPNdate-' + photoId).getElements('img').each(function (img) {
                    img.src = images.ok;
                });
                publishedCallback(photoId);
            } catch (e) {
...
function publishNOW(photoId, publishedCallback) {
    var publicVisibility = $('FPNpublic-' + photoId).checked;
    var friends = $('FPNfriends-' + photoId).checked;
    var family = $('FPNfamily-' + photoId).checked;

    new Element('img', { src: images.updating }).inject(
        publicVisibility ? $('FPNpublic-' + photoId).getParent() : $('FPNprivate-' + photoId).getParent()
    );

    new Element('img', { src: images.updating }).inject($('FPNdate-' + photoId));

    // get the current privacy settings to compare with
    new Request({
        url: 'http://www.flickr.com',
        onSuccess: function (responseText, responseXML) {
            try {
                ...
                if (publicVisibility) {
                    if (photo.visibility.ispublic === 1) { // nothing to change
                        $('FPNpublic-' + photoId).getParent().getElement('img').src = images.ok;
                        publishedCallback(photoId);
                    } else { // make public
                        updatePrivacy(photoId, publishedCallback);
                    }
                } else {
                    if (photo.visibility.ispublic === 0) { // nothing changed?
                        if (idem('FPNfriends-' + photoId, photo, 'isfriends') && 
                            idem('FPNfamily-' + photoId, photo, 'isfamily')) { // nothing changed
                            $('FPNprivate-' + photoId).getParent().getElement('img').src = images.ok;
                            publishedCallback(photoId);
                        } else {
                            updatePrivacy(photoId, publishedCallback);
                        }
        ...
        onFailure: function (response) {
            ...
            api_key: api_key, // use the app's key for logging
            auth_hash: GM_getAuthHash(),
            auth_token: GM_getAuthToken(),
            format: 'json',
    ...
    updatePostedDate(photoId, publishedCallback);
}

function togglePublishDialog(photoId, publishedCallback) {
    var publishDialog = $('FPNdialog-' + photoId);
    if ($chk(publishDialog)) {
        publishDialog.destroy();
    } else {
        var layoutDiv = new Element('div', {
            id: 'FPNdialog-' + photoId,
            'class': 'edit-dialog privacy-edit bd',
            styles: {
                borderStyle: 'inset'
            }
        }).inject($('FPNlink-' + photoId), 'after');

        $('FPNdialog-'+photoId).adopt(
            new Element('ul').adopt(
                new Element('li').adopt(
                    new Element('input', {
                        ...
                        'class': 'force-left',
                        name: 'FPN_privacy',
                        id: 'FPNprivate-' + photoId,
                        value: 0,
                        checked: false
                    }),
                    new Element('label', {
                        html: 'Only you ',
                        'for': 'FPNprivate-' + photoId
                    }).adopt(new Element('small', { html: '(private)' }))
                ),
                new Element('ul').adopt(
                    new Element('li').adopt(
                        new Element('input', {
                            ...
                            value: 1,
                            name: 'FPN_privacy',
                            id: 'FPNfriends-' + photoId,
                            checked: false,
                            events: {
                                click: function (e) {
                                    if (this.checked) {
                                        $('FPNprivate-' + photoId).checked = true;
                                    }
                                }
                            }
                        }),
                ...
                new Element('li').adopt(
                    new Element('input', {
                        ...
                        'class': 'force-left',
                        name: 'FPN_privacy',
                        id: 'FPNpublic-' + photoId,
                        value: 1,
                        checked: true,
                        events: {
                            click: function () {
                                $('FPNfriends-' + photoId).set('checked', false);
                                $('FPNfamily-' + photoId).set('checked', false);
                            }
                        }
                    }),
                    new Element('label', {
                        html: 'Anyone ',
                        'for': 'FPNpublic-' + photoId
                    }).adopt(new Element('small', { html: '(public)' }))
                ),
                new Element('li').adopt(
                    new Element('hr')
                ),
                new Element('li', {
                    html: 'Posted date: NOW',
                    id: 'FPNdate-' + photoId
                })
            ),
            new Element('div', {
		    ...
		    id: 'FPN-save-' + photoId,
			    ...
			    var saveButton = $(e.target);
                            publishNOW(saveButton.id.replace('FPN-save-', ''), publishedCallback);
                        }
                    }
                }),
                new Element('input', {
                    'class': 'CancelButt owner-cancel',
		    id: 'FPN-cancel-' + photoId,
                    type: 'button',
                    value: 'Cancel',
                    events: {
                        click: function (e) {
			    var cancelButton = $(e.target);
                            togglePublishDialog(cancelButton.id.replace('FPN-cancel-', ''));
                        }
                    }
                })
            ),
	    new Element('div', {
		html: 'Please consider a donation: ',
		styles: {
			'font-style': 'italic',
			'text-align': 'center'
		}
...
GM_setValue("version", FPNversion);
...
if (document.location.href.match(/flickr\.com\/photos\/[^\/]+\/\d+/)) { // photo page
	var discussPhoto = document.getElementsByClassName('owner-edit');

	if (!$chk(discussPhoto) || discussPhoto.length == 0) {
		return;
	}
	try {
		var photoId = document.location.href.match(/flickr\.com\/photos\/[^\/]+\/(\d+)/)[1];
		$('photo-sidebar-additional-info').adopt(
		    new Element('div', {
			styles: {
			...
			new Element('a', {
				...
				id: 'FPN-publish-' + photoId,
				events: {
					click: function (e) {
						var anchor = $(e.target);
						var photoId = anchor.id.replace('FPN-publish-', '');
						togglePublishDialog(photoId, function () {
							reloadPage();
						});
					}
				}
		...
		GM_log("no owner sidebar found (looking at another's photos)");
	}
} else if (document.location.href.match(/flickr\.com\/photos\/[^\/]+\/(?:page\d+\/)?/) ||
           document.location.href.match(/flickr\.com\/photos\/[^\/]+\/with\/\d+/)) { // photostream pages
	if (!document.location.href.match(/flickr\.com\/photos\/[^\/]+\/(?:page\d+\/)?\?.*details=1/) &&
            !document.location.href.match(/flickr\.com\/photos\/[^\/]+\/with\/\d+\/\?details=1/)) { // photostream details pages
		GM_log("only active on details pages (add '?details=1' to the url); aborting");
		return;
	}
	if ($$('a.change-photo').length == 0) {
		GM_log("on another's photostream; aborting");
		return;
	}
	$$('div.photo-display-item').each(function (photoDisplayItem) {
		var photoId = photoDisplayItem.get('data-photo-id');
		var privacy = photoDisplayItem.getElement('p.Privacy');
		var publishNowDiv = new Element('div').inject(privacy, 'after');
		publishNowDiv.adopt(
			new Element('a', {
				html: 'Publish',
				href: 'javascript:void(0)',
				...
				events: {
					click: function (e) {
						var anchor = $(e.target);
						var photoId = anchor.id.replace('FPN-publish-', '');
						togglePublishDialog(photoId, function(photoId) {
						    try{
						    	var photoDiv = $('photo_' + photoId);
							var privacy = photoDiv.getElement('p.Privacy');
							privacy.setStyle('text-decoration', 'line-through');
							/*privacy.getChildren().each(function (el) {
								GM_log("striking through a " + el + " element");
								el.setStyle('text-decoration', 'line-through');
							});*/
						    } catch (e) { // called multiple times, but we can only move the element once
						    	//alert("error: " + e + " - photoId: " + photoId );
						    }
						});
					}
				}
