  ...
  var ujs_seob = {
    ...
    initFrameData : function()
    {
      ...
      with(ujs_seob.frame)
      {
        ...
        yandex.url = 'http://bar-navig.yandex.ru/u?ver=4.0.0&show=1&url=' + escape(url) + ujs_seob.urlToken + yandex.frameToken;
      ...
      with(ujs_seob.frame.dir)
      {
        ...
        rambler.getDataFromDocumentFunction = function () {
          var info = '';
          var rubrics = document.getElementsByTagName('div');          
          for(var i = 0; i < rubrics.length; i++)
          {
            if(rubrics[i].className && rubrics[i].className == 'rubrics')
            {
              var a = rubrics[i].getElementsByTagName('a');
              var sep = '', count = 0;
              for(var j = 0; j < a.length; j++)
              {
                if(a[j].href && a[j].href.search(/\/cgi-bin\/counter_search\?/i) != -1)
                {
                  count++;
                  if(count > ujs_seob.maxDirectoryResults)
                  {
                  ...
                  info += sep + '<a href="' + a[j].href + '">' + a[j].innerText + '</a>';
                  var next = a[j].nextSibling;
      ...
      with(ujs_seob.frame.index)
      {
        ...
        yandex.checkUrl = 'http://yandex.ru/yandsearch?surl=' + host + '&pag=u&numdoc=50&rd=0';
        yandex.url = yandex.checkUrl + ujs_seob.urlToken + yandex.frameToken;
        yandex.domain = 'yandex.ru';
        yandex.deleteFrame = ujs_seob.removeFrames;
        yandex.style = style;
        ...
        rambler.checkUrl = 'http://search.rambler.ru/cgi-bin/rambler_search?filter=' + host + '&limit=50&sort=0';
        rambler.url = rambler.checkUrl + ujs_seob.urlToken + rambler.frameToken;
        rambler.domain = 'search.rambler.ru';
        rambler.deleteFrame = ujs_seob.removeFrames;
        rambler.style = style;
        rambler.IsObjectLoaded = function() {
