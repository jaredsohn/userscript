// ==UserScript==
	// @name           MUP_CastReel
	// @fullname       MUP_CastReel
	// @author         http://userscripts.org/users/107391
	// @description    DivX Player for MegaUpload @ CastReel.com
	// @version        1.0.1
	// @date           18/03/2010
	// @include        http://www.megaupload.com/*
	// @include        http://castreel.com/videoplayer/muplayer.php*
// ==/UserScript==
...
*/
	...
	new_body_page = '<div id="gen_body" style="background-color:'+GM_getValue('MUP_color_bg')+';padding-top:15px;padding-bottom:'+(window.innerHeight/3)+'px;">';
		...
		new_body_page += '<div id="new_body" style="display:'+(MUP_load?'block':'none')+';">';
			new_body_page += '<div id="url_dl" style="font-size:11px; font-weight:bold;margin-bottom:8px">';
...
//			new_body_page += '</div>';
			new_body_page += '<div id="Movie_load" style="margin:auto;width:900px;height:'+(MUP_Height/2)+'px;display:'+(MUP_stream?'none':'block')+';text-align:center;padding-top:'+(MUP_Height/2)+'">';
				new_body_page += '<input id="Movie_url" type="text" size="100" value="" />';
			new_body_page += '</div>';
			new_body_page += '<div id="Div_Counter_MUP" style="margin:auto;width:900px;height:'+(MUP_Height/2)+'px;display:'+(MUP_stream?'block':'none')+';text-align:center;padding-top:'+(MUP_Height/2)+';font-size:24px;font-weight:bold;">';
				new_body_page += MUP_LNG.wait_thx+' <span id="Time_Counter_MUP">'+Counter_MUP+'</span> '+MUP_LNG.wait_sec;
			new_body_page += '</div>';
			new_body_page += '<object id="lecteur_divx" style="display:none;margin:auto;width:900px;height:484px" onmouseover="document.getElementById(\'MUP_MUP\').style.visibility=\'visible\'" type="video/divx" '+(MUP_stream?'data="'+MUP_url_movie+'"':'')+' codebase="http://download.divx.com/player/DivXBrowserPlugin.cab">';
			if (MUP_stream)
			{
			...
			new_body_page += '<param name="custommode" value="Stage6" />';
			...
			new_body_page += '<div style="padding-top:0px">'+MUP_LNG.error_l1+MUP_LNG.error_l2+'<span style="color:'+GM_getValue('MUP_color_href')+';cursor:pointer" onmouseover="this.style.color=\''+GM_getValue('MUP_color_hover')+'\';return true;" onmouseout="this.style.color=\''+GM_getValue('MUP_color_href')+'\';return true;" onclick="location.reload();">'+MUP_LNG.error_l3+'</span>'+MUP_LNG.error_l4+'</div>';
	...
	html_MUP_options_bg += '<div id="MUP_preview_img" style="text-align:center;display:none;">\n';
		...
		if (MUP_options_TabImg.length>1)
		{
			html_MUP_options_bg += '<div style="overflow:auto;'+(Math.ceil((MUP_options_TabImg.length-1)/7)>3?'height:150px;':'')+'">\n';
				...
				{
					html_MUP_options_bg += '<img onclick="document.getElementById(\'MUP_url_bg\').value=this.src" style="height:0px;max-width:80px;vertical-align:middle;cursor:pointer;" src="'+unescape(MUP_options_TabImg[i])+'" alt="'+MUP_LNG.option_error_find+'" title="'+(i<10?'0'+i:i)+' : '+unescape(MUP_options_TabImg[i])+'" />';
				}
			html_MUP_options_bg += '</div>\n';
		}
		else
