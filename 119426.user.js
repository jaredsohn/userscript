...
(function () {
    ...
    if (typeof unsafeWindow.jQuery == 'undefined') {
        ...
        js.src = 'http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js';
    ...
    function krowzerUserScript() {
        createCssRules();
        $("body").append($('<div id="divKrowzerContainer"><h>Krowzer:RU</h2></div>'));
        ...
        if (KHOMMK.player.content.allianceId) {
            buildPlayersOnline();
            $divKC.append($('<button id="btKrowzerPlayersOnlineFrame" type="button">Играют</button>').click(function () { closeFrames(); getPlayersOnline(); $("#divKrowzerPlayersOnlineFrame").show(); }));
        }
        $divKC.append($('<button id="btKrowzerResourceRequestListFrame" type="button">Заявки</button>').click(function () { closeFrames(); getResources('rrl'); }));
        $divKC.append($('<button id="btKrowzerCityResourcesFrame" type="button">Ресурсы в городах</button>').click(function () { closeFrames(); buildCityResourceTable(); }));
        $divKC.append($('<button id="btKrowzerCityTroopsFrame" type="button">Войска в городах</button>').click(function () { closeFrames(); buildCityTroopsTable(); }));
        if (KHOMMK.player.content.worldSeasonNumber >= 3) {
            $divKC.append($('<button id="btKrowzerFortressResourcesFrame" type="button">Ресурсы в Фортах</button>').click(function () { closeFrames(); getFortressResources(); }));
            $divKC.append($('<button id="btKrowzerFortressOverviewFrame" type="button">Обзор фортов</button>').click(function () { closeFrames(); getFortressList(); }));
    ...
    function buildPlayersOnline() {
        $("body").append($('<div id="divKrowzerPlayersOnlineFrame" class="divKrowzerFrame"><h2>Активные Игроки<span class="spanClose">X</span></h2><div class="divKrowzerScroll"><table><tfoot><tr><td><button class="btRefreshOnline" type="button" style="font-size:11px;">Refresh list</button></td></tr></tfoot><tbody></tbody></table></div></div>'));
    ...
    function buildCityResourceTable() {
        ...
        $("body").append($('<div id="divKrowzerCityResourceFrame" class="divKrowzerFrame"><h2>Ресурсы в городах<span class="spanClose">X</span></h2></div>'));
        $("#divKrowzerCityResourceFrame .spanClose").click(function () { closeFrames(); });
        var tbl = '<div class="divKrowzerScroll"><table><thead><tr><th class="thLeft">Город</th><th>Золото</th><th>Дерево</th><th>Руда</th><th>Ртуть</th><th>Кристаллы</th><th>Сера</th><th>Самоцветы</th></tr></thead>';
    ...
    function buildFortressResourceTable(json) {
        if (json) {
            ...
            if (list) {
                var l = list.length;
                var tbl = '<div class="divKrowzerScroll"><table><thead><tr><th class="thLeft">Форт</th><th>Отсчёт</th><th>Next Floor</th><th>Wood</th><th>Ore</th><th>M</th><th>C</th><th>S</th><th>G</th></tr></thead><tfoot><tr><td colspan="9"></td></tr></tfoot><tbody>';
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    if (fortress.upgradeComponentList) {
                        ...
                        for (var i3 = 0; i3 < fl; i3++) {
                            var floor = colfloors[i3];
                            if (floor.upgradeResourceList) {
                                var rl = floor.upgradeResourceList;
                                var no = floor.label.replace('Level ', '');
                                var fn = (i3 == 0 ? fortname : '');
    ...
    function buildFortressOverviewTable(json) {
        if (json) {
            ...
            $("body").append($('<div id="divKrowzerFortressOverviewFrame" class="divKrowzerFrame"><h2>Обзор фортов<span class="spanClose">X</span></h2></div>'));
            $("#divKrowzerFortressOverviewFrame .spanClose").click(function () { closeFrames(); });
            $('#divKrowzerFortressOverviewFrame').append('<div class="divKrowzerScroll"><table id="tblFortressList"><thead><tr><th class="thLeft">Форт</th><th>Владелец</th><th>Отсчёт</th><th>Общая сила</th><th>Ворота</th><th>Этаж 1</th><th>Этаж 2</th><th>Этаж 3</th><th>Этаж 4</th><th>Этаж 5</th></tr></thead><tfoot><tr><td colspan="10"></td></tr></tfoot><tbody></tbody></table></div>');
                        ...
                        if (datan) {
                            ...
                            if (isnotcontrolled) {
                                ...
                                if (datan.hero) {
                                    var hero = datan.hero;
                                    as0 = hero.armyPower;
                                    gate = _gate.format(hero.playerName, addCommas(hero.armyPower));
                                }
                                ...
                                totalstrength = addCommas(as0 + as1 + as2 + as3 + as4 + as5);
                                floor1 = _floor.format(dt1, pct1, addCommas(as1));
                                floor2 = _floor.format(dt2, pct2, addCommas(as2));
                                floor3 = _floor.format(dt3, pct3, addCommas(as3));
                                floor4 = _floor.format(dt4, pct4, addCommas(as4));
                                floor5 = _floor.format(dt5, pct5, addCommas(as5));
    ...
    function buildResourceRequestTable(json) {
        if (json) {
            $("#divKrowzerResourceRequestFrame").remove();
            $("body").append($('<div id="divKrowzerResourceRequestFrame" class="divKrowzerFrame"><h2>Заявки на ресурсы<span class="spanClose">X</span></h2></div>'));
            $("#divKrowzerResourceRequestFrame .spanClose").click(function () { closeFrames(); });
            var _requestrow = '<tr{0}><td class="tdLeft">{1}</td><td>{2}</td><td>{3}</td></tr>';
            var list = json.d["MarketPlaceFrame" + KHOMMK.sideBar.content.attachedRegionCityList[0].id].auctionRequestList;
            if (list) {
                var l = list.length;
                var tbl = '<div class="divKrowzerScroll"><table><thead><tr><th class="thLeft">Тип</th><th>Количество</th><th>Автор</th></tr></thead><tfoot><tr><td colspan="3"></td></tr></tfoot><tbody>';
                for (var i = 0; i < l; i++) {
                    var request = list[i];
                    tbl += _requestrow.format((i % 2 == 0) ? '' : ' class="trOdd"', request.ressourceEntityName, request.quantity, request.playerName);
                }
                tbl += '</tbody></table></div>';
                $('#divKrowzerResourceRequestFrame').append(tbl);
            }
        }
    }
