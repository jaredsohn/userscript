...
unsafeWindow.createGroup = function() {
	...
	if (groupname && Groups[groupname] === undefined) {
		...
		document.getElementById('group_menu').options[document.getElementById('group_menu').options.length] = new Option(groupname,groupname,false,true);
	}
}
...
unsafeWindow.updateGroupList = function(grp,plsel,grpsel) {
	...
	if ( grp == '' ) {grpsel.options.length = 1;return;}
	...
	if ( grp == 'new' ) {
		...
		group = Groups[grp];
		grpsel.options.length = 1;

		var j = 1;
		for (var i = 0;i<plsel.options.length;i++) {
			if (group.indexOf(plsel.options[i].value) != -1) {grpsel.options[j] = new Option(plsel.options[i].text,plsel.options[i].value);j++;}
		}
	}
}
...
unsafeWindow.updatePlayerList = function(frmsel,tosel) {
	var tslen = tosel.options.length;
	var frlen = frmsel.options.length;

	if (frmsel.name != 'group_select') tosel.options[tslen] = new Option(frmsel.options[frmsel.selectedIndex].text,frmsel.options[frmsel.selectedIndex].value);
	if (frmsel.name != 'players_select') frmsel.options[frmsel.selectedIndex] = null;
...
unsafeWindow.displayGroups = function(selection) {
	...
	// Group Management
		...
		group_menu.options[0] = new Option('Select Group or New','');
		group_menu.options[0].setAttribute('style','text-decoration: underline;');
		group_menu.options[1] = new Option('-New Group-','new');
		for (var i = 0, len = GroupKeys.length; i < len; i++) {
			group_menu.options[i+2] = new Option(GroupKeys[i]);
		}
		...
		for (var i=0,j=0;i<select.options.length;i++) {
			if (select.options[i].value == 'empire')  { // Do it here, because we may not always have a 'To Empire'
				players_select.options[j] = new Option('Empire','empire');
				j++;
			}
			else if (!isNaN(select.options[i].value)) {
				players_select.options[j] = new Option(select.options[i].text.match(/(.*) \[/)[1],select.options[i].value);
				j++;
			}
		}
		...
		group_select.options[0] = new Option('Members of this Group');
		group_select.options[0].setAttribute('style','text-decoration: underline;');
...
unsafeWindow.processMessage = function(form) {
    ...
    var groups = new Array();
    var checked = 0;
	if (!form.group.length) {groups.push(form.group.value);checked=1} // Only one group is present
	for(var i=0; i < form.group.length; i++) {
		if(form.group[i].checked) {
			groups.push(form.group[i].value);
			checked++;
		}
	}
		...
		for (var i=0; i<recipients.length; i++) {
			...
			for (var j=0; j<form.elements.length; j++) {
				...
				if (form.elements[j].name == 'message') {
					pars += 'message='+encodeURIComponent(form.message.value+'\n\n[Group Messaging: Message '+(i+1)+' of '+recipients.length+']')+'&';
				}
			}
