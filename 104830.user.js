...
// ================================================ ===============================
// License and Disclaimer (by google translator)
// ================================================ ===============================
// This program releases on the principle: do you like - upload gold author: D
// Number of Gold is your private matter.
// Account author: http://www.erepublik.com/pl/citizen/profile/1283469
// The program provides no guarantee and responsibility for the negative
// action - use at your own risk.

//updated by alienplayer,  http://www.erepublik.com/en/citizen/profile/2933752, on 6/15/2011
//1. fix the issue that max hit can not be calculated correctly
//2. change default hit times from 30 to 60
						...
						for (var b = 0; b < 15; b++) {
							...
							for (var c = 0; c < hospitals.length; c++) {
								...
								if (hospitals[c].zone_id == b + 1) {
									...
									row += '<small><img width="33" src="http://www.erepublik.com/images/icons/industry/4/q' + quality + '.png" />';
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
			...
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;# of fights: <input id="dmgFights" name="dmgFights" value="60" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">go!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>60 fights: </strong>' +
	...
	foodId = $j('div#large_sidebar input[type=hidden]').attr('id');
	...
	var styles =
		'<style type="text/css"> ' +
		'div.BHTable { width: 200px; height: 75px; border: 1px solid #777; border-radius: 5px; padding: 6px; background-color: #333; } ' +
	...
	content.prepend(
		'<div id="myStatBoxL" style="position: absolute; top: 400px; left: 22px; width: 250px; opacity: 0.8;"></div>' +
		'<div id="myOverBox" style="position: absolute; top: 300px; left: 315px; width: 250px; opacity: 0.8;"></div>'
	);
	content.append(
		'<div id="myStatBoxR" style="position: absolute; top: 400px; left: 560px; width: 250px; opacity: 0.8;"></div>'
	);
	...
	$j('div#enemy_defeated').before(
		...
		'<fieldset id="BLOG"><legend>v0.141 BATTLE LOG:&nbsp;&nbsp;<button id="battlelog_clear">clear</button></legend><div id="battlelog"></div></fieldset>'
	);
	...
	unsafeWindow.battleFX.countNextBattle = function(time) {
		...
		function checkTime(periods) {
			if ($j.countdown.periodsToSeconds(periods) == 0) {
				...
				setTimeout(function() { top.location.href = document.location.href; }, 3000);
			}
		}
