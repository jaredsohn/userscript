// ==UserScript==
// @version 1.0
// @name HD Youtube downloader
// @description Youtube HD Downloader
		...
		GM_xmlhttpRequest({
			...
			var global_version = results.responseText.match(/version[ ]*([0-9.]+)/i)[1];
				if (global_version.length && global_version != local_version) {
					if (confirm('[ Greasemonkey ] HD Tube : Version '+ global_version +' is now available. Update?')) {
...
function addDownloadLink(){
	var link =' [ <a href="' + dl_url + '" title="Download with HD tube!"><img src="' + keepTubeIcon + '" alt="" valign="middle"/></a> ]';
