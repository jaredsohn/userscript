...
var GegnerFerono   = "";  // w1.gondal.de
var GegnerThur     = "";  // www.gondal-de.de
var GegnerVenja    = "";  // w2.gondal.de
...
function EvalMKs(server,defAnz) {
  ...
  if(Anzahl=="unlimited" && defAnz!="unlimited") {
    GM_setValue(server + "_MKFight",defAnz);
    Anzahl = defAnz;
  }
  // Unlimited MK's
  if(defAnz=="unlimited") {
    GM_setValue(server + "_MKFight","unlimited");
    return true;
  }
  ...
  if(Anzahl==null) {
    ...
    if(defAnz>0) {
      ...
      if(defAnz>=MKAvail) {
        ...
        GM_setValue(server + "_MKFight",MKAvail-1);
        return true;
      }
      if(defAnz<MKAvail) {
        // Weniger MK's angegeben als verfuegbar
        GM_setValue(server + "_MKFight",defAnz-1);
        return true;
      }
    }
  }
  ...
  if(Anzahl>0) {
    GM_setValue(server + "_MKFight",Anzahl-1);
    return true;
  }
}
...
switch(pathname) {
        ...
        for(var x = 0;x<35;x++) {
          if(typeof(document.getElementById("search").getElementsByTagName("a")[x])!="undefined") {
            ...
            if(typeof(document.getElementById("search").getElementsByTagName("a")[x])!="undefined") {
              ...
              var Gegnerlink = document.getElementById("search").getElementsByTagName("a")[x].getAttribute('onclick');
              var posStart = Gegnerlink.indexOf("'/");
              var posEnd   = Gegnerlink.indexOf("';",posStart);
              var Gegnerlink  = Gegnerlink.substr(posStart+1, posEnd - posStart-1);           
            } else {
              notsearched = true;
            }
          }
  ...
  case "/fights/waitFight":
    switch(document.location.hostname) {            
        // Regulaere Welten Gondal
        case "www.gondal-de.de":
          if(MKThur==true && EvalMKs("Thur",AnzThur)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "w1.gondal.de":
          if(MKFerono==true && EvalMKs("Ferono",AnzFerono)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "w2.gondal.de":
          if(MKVenja==true && EvalMKs("Venja",AnzVenja)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "w3.gondal.de":
          if(MKSahscria==true && EvalMKs("Sahscria",AnzSahscria)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        // Speedwelten - Gondal
        case "s1.gondal.de":
          if(MKSpeed0==true && EvalMKs("Speed0",AnzSpeed0)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "s1-0.gondal.de":
          if(MKSpeed1==true && EvalMKs("Speed1",AnzSpeed1)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "s1-1.gondal.de":
          if(MKSpeed2==true && EvalMKs("Speed2",AnzSpeed2)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;
        case "s1-2.gondal.de":
          if(MKSpeed3==true && EvalMKs("Speed3",AnzSpeed3)==true) { SeitenRedirect("/fights/waitFight/buy"); }
          break;    
    }
