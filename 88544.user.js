// ==UserScript==
// @name            Script Updater (userscripts.org) UA [DO NOT INSTALL]
...
// @version         1.09
// @history        	1.09 Added extra params in check() [instead insertUpdateCheck]. added integration to GM menu
// @history        	1.08 Added insertUpdateCheck for create force update check link
...
ScriptUpdater = {
	version:"1.09",
	//------------------------------------------- "public" methods --------------------------------------
	// check() - check new version available for specified scriptId and currentVersion
	//    extra.menuCaption - add "force check" item to GM menu
	//    extra.beforeElement or extra.afterElement - add "force check" link to page
	//      extra.forceHTML - use as internal html for "force check" link
	check:function(scriptId, currentVersion, callback, extra) {	
		ScriptUpdater.initVars(scriptId, currentVersion, callback, true, false, extra);
		var d = new Date();
		...
		if (ScriptUpdater.beforeElement)
			ScriptUpdater.insertUpdateCheck(scriptId, currentVersion, ScriptUpdater.beforeElement, 1);
		if (ScriptUpdater.afterElement)
			ScriptUpdater.insertUpdateCheck(scriptId, currentVersion, ScriptUpdater.afterElement);
		if (ScriptUpdater.menuCaption)
			ScriptUpdater.insertUpdateMenu(scriptId, currentVersion);
	},
	...
	checkStored:function() {
		if(typeof(ScriptUpdater.scriptId) != 'undefined' && typeof(ScriptUpdater.scriptCurrentVersion) != 'undefined') {
			return (typeof(ScriptUpdater.getVal('versionAvailable')) != 'undefined' && ScriptUpdater.scriptCurrentVersion.toString() != ScriptUpdater.getVal('versionAvailable').toString());
		} else return false;
	},
//------------------------------------------- "private" methods --------------------------------------
	$:function(id) {
	...
	initVars:function(scriptId, currentVersion, callbackFunction, useNotice, forceNoticeEnabled, extra) {
		...
		ScriptUpdater.scriptName    = ScriptUpdater.getVal('scriptName');
		ScriptUpdater.beforeElement = false;
		ScriptUpdater.afterElement  = false;
		ScriptUpdater.forceHTML     = false;
		ScriptUpdater.forceHint     = false;
		ScriptUpdater.menuCaption   = false;
		if (typeof(extra) != 'undefined') {
			ScriptUpdater.beforeElement = (typeof(extra.beforeElement) != 'undefined') ? extra.beforeElement : false;
			ScriptUpdater.afterElement  = (typeof(extra.afterElement)  != 'undefined') ? extra.afterElement : false;
			ScriptUpdater.forceHTML     = (typeof(extra.forceHTML   )  != 'undefined') ? extra.forceHTML    : 'v.$(version)';
			ScriptUpdater.forceHint     = (typeof(extra.forceHint   )  != 'undefined') ? extra.forceHint    : 'Перевірити оновлення $(name) v.$(version)';
			ScriptUpdater.menuCaption   = (typeof(extra.menuCaption )  != 'undefined') ? extra.menuCaption  : false;
		}
	},
	checkRemoteScript:function() {
		...
		GM_xmlhttpRequest ({
			...
			onload: function (response) {
				...
				ScriptUpdater.setVal('versionAvailable', ScriptUpdater.meta.version);
				ScriptUpdater.setVal('scriptName', ScriptUpdater.meta.name);
	...
	showNotice:function() {
		if(ScriptUpdater.meta.name && ScriptUpdater.meta.version) {	
			...
			noticeWrapper.id = "ScriptUpdater" + ScriptUpdater.scriptId + "BodyWrapper";
				...
				html.push('" title="Закрити"/><img src="');
				html.push(ScriptUpdater.icons.uso);
				html.push('" align="absmiddle" style="margin-top:-2px;"/><a href="http://userscripts.org/scripts/show/57756" target="_blank" title="Про Userscripts.org Script Updater v');
				html.push(ScriptUpdater.meta.version);
				html.push('">Userscripts.org Updater</a></h1>');
				if(!ScriptUpdater.forceNoticeEnabled) {
					html.push('<p>Знайдено новішу версію скрипта <strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="На сторінку скрипта">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong> , яку можна встановити.</p>');
				} else {
					html.push('<p><strong><a href="http://userscripts.org/scripts/show/');
					html.push(ScriptUpdater.scriptId);
					html.push('" target="_blank" title="Перейти на сторінку скрипта" style="margin:0; padding:0;">');
					html.push(ScriptUpdater.meta.name);
					html.push('</a> </strong></p>');
				}				
				if(ScriptUpdater.scriptCurrentVersion) {
					html.push('<p>Зараз у Вас встановлена версія <strong>');
					html.push(ScriptUpdater.scriptCurrentVersion)
					html.push('</strong>. Актуальна версія <strong>');
					html.push(ScriptUpdater.meta.version);
					html.push('</strong></p>');
				}
				
				if(ScriptUpdater.meta.history) {
					html.push('<h2>Історія змін:</h2><div id="ScriptUpdater' + ScriptUpdater.scriptId + 'History">');
				...
				*/
				...
				html.push('<input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'CloseButton" value="Закрити" style="background-image:url(');
				html.push(ScriptUpdater.icons.close);
				html.push(')"/><input type="button" id="ScriptUpdater' + ScriptUpdater.scriptId + 'BodyInstall');
				html.push(ScriptUpdater.scriptId);
				html.push('" value="Встановити" style="width:100px; background-image:url(');
				html.push(ScriptUpdater.icons.install);
				html.push(');"/>');
				html.push('Шукати оновлення цього скрипта ');

				html.push('<select id="ScriptUpdater' + ScriptUpdater.scriptId + 'Interval"> \
								<option value="3600000">щогодини </option>\
								<option value="21600000">раз на 6 годин </option>\
								<option value="86400000">щодня </option>\
								<option value="604800000">щотижня </option>\
...
//			$('#ScriptUpdater" + ScriptUpdater.scriptId + "Body')[0].style.marginTop = (unsafeWindow.scrollY + 125) + 'px';
	...
	insertUpdateCheck:function(scriptId, currentVersion, element, insertBefore) {
		var html  = ScriptUpdater.forceHTML.replace('$(id)', scriptId).replace('$(version)', currentVersion).replace('$(name)', ScriptUpdater.getVal('scriptName'));
		var hint  = ScriptUpdater.forceHint.replace('$(id)', scriptId).replace('$(version)', currentVersion).replace('$(name)', ScriptUpdater.getVal('scriptName'));
		var a = document.createElement('a');
		a.id = 'updateCheck';
		a.setAttribute('title', 'Check update for ' + scriptId);
		a.setAttribute('style', 'margin:10px; cursor:pointer; font-size:smaller;');
		a.addEventListener("click", function() { ScriptUpdater.forceNotice(scriptId, currentVersion); }, true);
		a.innerHTML = html;
		if (insertBefore) {
	...
	insertUpdateMenu:function(scriptId, currentVersion, caption) {
		var caption = ScriptUpdater.menuCaption.replace('$(id)', scriptId).replace('$(version)', currentVersion).replace('$(name)', ScriptUpdater.getVal('scriptName'));
		GM_registerMenuCommand(caption, function() { ScriptUpdater.forceNotice(scriptId, currentVersion); })
	},
};
