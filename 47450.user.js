...
//
//
//  "That's a key, isn't it? It's not a good idea to walk around with a
//  key ring, because first of all, it looks so sloppy," the man said,
//  pointing to her skirt, where she kept pinned the small key to her
//  old school gym locker, a souvenir kept from a single experience long
//  ago.
//
//  "Most employees around here would never be seen with a key ring," he
//  continued, still extending his finger. "I've heard some departments
//  are lax in their rules and what not, but my thinking is, if you hold
//  a set of keys, why walk around with them for the whole world to see?
//  I don't like things seen," he concluded, glancing into the hall
//  behind her and bristling.
//
//  She tried to move through the doorway, but the man stopped her with
//  a large rubbery hand to her shoulder, and this angered her, but he
//  smiled so ingratiatingly that she somehow wound up giving him a
//  similar smile.
//
//                       an excerpt from Dra-- by Stacey Levine, p. 81
//
//
//
//
...
(function() {
  // to get around current 5xx errors for logged-in users and to
  // circumvent the 60 requests-per-min throttling, use coral.
  ...
  var get_issues = function(repo) {
    GM_xmlhttpRequest({
      method: 'GET', 
      url: GH + '/issues/list/' + $.user + '/' + repo.name + '/open',
  ...
  var get_commits = function(repo) {
    GM_xmlhttpRequest({
      method: 'GET', 
      url: GH + '/commits/list/' + $.user + '/' + repo.name + '/master',
      onload: function(resp) {
        eval("var data = " + resp.responseText + ";");
  ...
  var get_network = function(repo) {
    GM_xmlhttpRequest({
      method: 'GET', 
      url: GH + '/repos/show/' + $.user + '/' + repo.name + '/network',
      onload: function(resp) {
        eval("var data = " + resp.responseText + ";");
        repo.network = data.network.sort(function(a, b) {
