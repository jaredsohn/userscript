// ==UserScript==
// @name           Imperion Mods with Happy's addons
...
// @author         PhasmaExMachina & Warlock8 & MrHappy
...
ScriptUpdater.check(74036, '0.39.5');
	...
	"Harvest":{
		...
		fields:{
			...
			oneminuteTimer:{
				type:'checkbox',
				label:'Timer',
				text:'Activate one minute timer',
				value:false
			}
		}
	},
					...
					$(survey).click(function(){
						...
						if(systemId) {
							...
							if(slotsAvailable < planetsToSurvey.length) {
								alert("Are you mad? unable to perform Raid on Sadam.\n\nYou only have " + slotsAvailable + " of the " + planetsToSurvey.length + " fleet slots needed.")
								error = true;
							}			
							if(!error && confirm('Zardoz Are you sure you want to Attack ' + planetsToSurvey.length + ' empty planets?' +
								"\n\nYou must have " + planetsToSurvey.length + " fleet slots and Ships available."
							)) {
								...
								alert("Ships successfully sent to " + spiesSent + " planets. " + (remaining > 0 ? "\n\nUnable to Raid " + remaining + " planet" + (remaining > 1 ? 's' : '') + ".\n\nCheck fleet spot and Ship availability." : ''));
							}
							return false; // don't zoom the map back out
						}
					});
				}
				$('#phasmaSurveySystemButton').attr('title', 'Kill unoccupied planets in this system');
