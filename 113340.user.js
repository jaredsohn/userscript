...
var colocaNaPagina = function() {
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		var envia = {
			...
			"subjectText": encodeURIComponent("TÃƒÂ³pico Excluido:"),
...
function confirmaDeletarTopicos() {
	...
	if (new RegExp(/delete\_entire\_topic/i).test(tagBody.innerHTML)) {
		var stringItem = "if (confirm('Tem certeza que deseja deletar este tÃƒÂ³pico?')){postaTopicoDeletado(document.getElementsByTagName('h1')[0].innerHTML,document.body.innerHTML.match(/uid=(\\d+)/ig)[1],prompt('Qual o motivo de estar deletando este tÃƒÂ³pico:\\n','T\ÃƒÂ³pico de spam'));_submitForm(this, 'delete_entire_topic', 'true');; return false;}";
