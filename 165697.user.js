...
- 	removed the randomly timed auto-woot (detrimental feature)
~ 	changed it with an auto-woot that woots in the last possible seconds,
	this way it will not do any harm to a possible vote to skip the current song. 
+	Added a countdown visual to the auto-woot button, showing the seconds
	total remaining before woot.
	
v2.2.0.2: MODDED
+	Added a random timer to the Auto-WOOT feature. 
	When Auto-WOOT is enabled, the timer will check the current song's playtime
	and WOOT at a random time during the first half of the song. 
...
function hhb_init() {
    ...
    document.getElementById('hhb_autowoot_button').addEventListener('click', function (e) {
        ...
        //		doc =e.target;
        if (hhb_auto) {
            doc.innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(0,138,5);font-weight:900;\'>Enabled</span>.';
            hhb_autowoot();
        } else {
						if( auto_woot_timer.getStatus() ){
							auto_woot_timer.cancel();
						}
            doc.innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(233,6,6);font-weight:900;\'>Disabled</span>.';
        }
    });
...
var auto_woot_timer = {
  ...
  vote: function() {
		...
		// this shouldn't be here? Lets just leave it until we decide to expand the feature-set
		document.getElementById('hhb_autowoot_button').innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(0,138,5);font-weight:900;\'>Enabled</span>.';
		//
  },
  ...
  setup: function() {
    this.cancel();
    var self = this;
		//sloppy way to get the remaining time for currently playing media
		//regexp looks for any number (single or multiple), followed by a colon and ending with any number between 0 and 59, example: 312:59 (minutes, seconds)
		var time_remaining_formatted = document.getElementById( 'time-remaining-value' ).innerHTML;
		var time_regexp = /^(\d*):([0-5]\d)/g;
		var matcharray = time_regexp.exec( time_remaining_formatted );
		if( matcharray ){
			
			//random in first half of the song. Is detrimental, don't use. 
			//var seconds_left = ((+matcharray[1]*60)+(+matcharray[2])-10);
			//var wait_before_woot = (Math.round(Math.random()*seconds_left)+5) >> 1;  
			//
			
			//add up minutes and seconds as total in seconds,
			//then deduce 6 from the total, the woot will go off 6 seconds before the ending of the song
			var seconds_left = (+matcharray[1]*60)+(+matcharray[2]);
			var wait_before_woot = seconds_left-15;
			(wait_before_woot < 0)?wait_before_woot=0:""; //this really won't matter with wait_before_woot set at seconds_left-6, the last 5 seconds voting is disabled.
			this.timeleft = +wait_before_woot;
			
			// this shouldn't be here? Lets just leave it until we decide to expand the feature-set
			document.getElementById('hhb_autowoot_button').innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(0,138,5);font-weight:900;\'>In ' + this.timeleft + ' sec</span>.';
			//
			
			this.timeoutID = window.setTimeout(function(){self.vote()}, wait_before_woot*1000);
			this.intervalID = window.setInterval(function(){self.countdown()} ,1000);
  ...
  cancel: function() {
    if(typeof this.timeoutID == "number") {
      ...
      delete this.timeoutID;
			...
			// this shouldn't be here? Lets just leave it until we decide to expand the feature-set
			document.getElementById('hhb_autowoot_button').innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(233,6,6);font-weight:900;\'>Disabled</span>.';
			//
    }
  },
  ...
  countdown: function(){
	var doc = document.getElementById('hhb_autowoot_button');
		if(!this.timeleft)return;
			this.timeleft--;
			// Going to let it countdown no matter what, it's just a visual thing anyway. Stupid?
			//if(this.timeleft <= 0){
			//	this.timeleft = 0;
			//	doc.innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(0,138,5);font-weight:900;\'>Enabled</span>.';
			//}
		doc.innerHTML = 'Auto-WOOT!: <span style=\'color:rgb(0,138,5);font-weight:900;\'>In ' + this.timeleft + ' sec</span>.'
  }
};
