// ==UserScript==
// @name           Travian quick links
// @namespace      http://userscripts.org/users/72477
// @description    Shows quick links below villages. At the moment links are stored inside the script
...
var tblBody = document.createElement("tbody");
        ...
        for (var j = 0; j < links.length/2; j++) {
            ...
            for (var i = 0; i < 2; i++) {
				...
				alink.setAttribute("href", urlBase+ links[2*j+1]);
		...
		alink.appendChild(document.createTextNode("Links:"));
...
// GM_log("AddNewLink() :: called");
 ...
 loc =  location.href.substring(location.href.lastIndexOf('/',urlBase.length)+1, location.href.length);
// GM_log("links :" + links.length);

 new_link = prompt("New link url",loc);
 if(!new_link) { return}
 new_link_name = prompt("New link name","");
