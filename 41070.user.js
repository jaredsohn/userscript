...
// @license			GPLv3
// @require			http://static.bitcrunch.de/javascript/pg-sc-api-2-5-2.js
// ==/UserScript==
...
var require = new Object({
	stylesheet:'http://static.bitcrunch.de/css/pghe-2-2-1.css',
	javascript:'http://static.bitcrunch.de/javascript/pg-sc-api-2-5-2.js'
});
	...
	if( state == 'gang' ) {
		switch ( task ) {
		  ...
		  case "dashboard:complete":
			  statusMessage("Resultate der Bandensuch werden gerendert...");
			  renderGangSearchResultTable();
			  break;
		  case "showGangSearchResults:complete":
			  statusMessage("Es wird nach Updates gesucht...");
		...
		switch ( task ) {
		  ...
		  case "dashboard:complete":
			  if( GET('details') ) {
				  statusMessage("Banden-IDs werden gesammelt...");
				  loadGangInfo( GET('details'), collectGangIDs )
			  } else {
				  statusMessage("Spieler-IDs werden gesammelt...");
				  collectUserIDs();
			  }
			  break;
		  case "userids::collected":
			  statusMessage("Spielerinformationen werden geladen...");
			  loadUserInfo( user[ count ], userHandler );
			  break;
		  case "userinfo::complete":
			  if( GET('serverload') == 'middle' || GET('serverload') == 'low' ) {
				  statusMessage("Bandeninformationen werden geladen...");
		  ...
		  case "ganginfo::complete":
			  if( GET('serverload') == 'low' ) {
				  statusMessage("Erweiterte Spielerinformationen werden geladen...");
		  ...
		  case "userprofile::complete":
			  statusMessage("Highscoretabelle wird gerendert...");
			  insertHighscoreData();
			  break;
		  case "insert::complete":
			  statusMessage("Es wird nach neuen Updates gesucht...");
			  scriptUpdate( meta.id, scriptUpdateHandler );
			  break;
...
function createLayout() {
	...
	try {
		...
		var items = new Array(
				...
				['News', url.news ],
				['Blog', url.blog ],
				['BitCrunch.de', 'http://bitcrunch.de/pennergame/hamburg/cheatsheet' ]
		);
		...
		content.innerHTML  = "<br /><b>&nbsp;<a style='text-decoration:none;color:#808080;' href='" + url.highscore + "'>" + meta.name + "</a></b><br /><br />";
...
function insertDashboardData() {
	try {
		...
		if( profile.overview ) {
			...
			div.innerHTML += "Angriff: <b>"  + profile.letsfight.att + "</b><br />";
			div.innerHTML += "Verteidigung: <b>" + profile.letsfight.def + "</b><br />";
			div.innerHTML += "<br />";
			div.innerHTML += "Promille: " + highlightPermille(profile.overview.permille) + "<br />";
			div.innerHTML += "Geld: " + highlightCash(profile.overview.cash) + "<br />";
			div.innerHTML += "<br />";
			div.innerHTML += "Punkte: <b>" + profile.overview.points + "</b><br />";
			
			// by Antitheus
			// div.innerHTML += "RPunkte: <b>" + (profile.overview.points + profile.letsfight.winlospoints) + "</b><br />";

			div.innerHTML += "Platzierung: <b>" + profile.overview.position + "</b><br />";
			
			// by Antitheus
			div.innerHTML += "Serverzeit: <b id='Uhrzeit'></b><br />"; // ein kleiner Bugfix da sonst der Errorlog vollgeschmirt wird
			status.appendChild(div);
			...
			div.innerHTML += "Gewonnen: <b>" + tmp.win + "</b> (" + wins_percentage + "%)<br />";
			div.innerHTML += "Verloren: <b>" + tmp.loose + "</b> (" + loose_percentage + "%)<br />";
			div.innerHTML += "<br />";
			div.innerHTML += "Punktebereich deiner Opfer<br /><b>" + tmp.min_points + "</b> bis <b>" + tmp.max_points + "</b>";
			div.innerHTML += "<br /><br />";

			// by Antitheus START
			div.innerHTML += "Zeig mir meine Opfer:";
			div.innerHTML += "<br />";
			
			
			function getMyVictims(max_points, min_points, htmltext, target) {
				var a = createElement('a');
				a.setAttribute('href', 'http://pennergame.de/highscore/range/1/?max_points=' + max_points + '&min_points=' + min_points + ( GET('serverload') ? '&serverload=' + GET('serverload') : '' )   );
				if (target)
					a.target = target;
				else
					a.target = "_self";
			...
			if( tmp.outgoing_fight ) {
				...
				div.innerHTML += "Abgehender Kampf gegen:<b><br /><a target='_blank' href='" + str + "'>" + tmp.name.slice(0, 23) + "</a></b><br /><br />"
				div.innerHTML += "Die Schlacht endet am:<br /><b>" +  tmp.end + "</b><br />";
				div.innerHTML += "<br />";
				div.innerHTML += getStopFightForm('Angriff abbrechen');
			} else {
				div.innerHTML = "<i>Keine ausgehenden K&auml;mpfe</i>";
			}
			status.appendChild(div);
			...
			if( tmp.incomming_fight ) {
				...
				var str = url.user_profile.replace(/\{\$id\}/, tmp.id );
				div.innerHTML += "Eingehender Angriff von:<b><br /><a target='_blank' href='" + str + "'>" + tmp.name.slice(0, 23) + "</a></b><br /><br />"
				div.innerHTML += "Zeitpunkt des Einschlags:<br /><b>" +  tmp.end + "</b><br />";
			} else {
			...
			status.innerHTML += "<br /><br /><b>Daten konnten nicht geladen werden. Bitte die Seite <a href='javascript:location.reload();'>neu laden</a>, oder <a href='http://pennergame.de/login/'>einloggen</a></b>";
		}
	...
	try {
		if( result ) {
			if( result.version != meta.version ) {
				...
				update.innerHTML += "Klick <b><a title='" + result.source + "' href='" + result.source + "'>hier</a></b> um das Update zu installieren. <b>Information</b> zur neuen Version:<br /><br />" + result.notes;
				insertAfter($('dashboard'), update );
				statusMessage("<b><a style='color:green;' title='" + result.notes + "' href='" + result.source + "'>Updates verfügbar!</a></b>");
			} else {
...
function renderGangSearchResultTable() {
	...
	if( rawTable[ 1 ].innerHTML.match(/Es wurden keine Banden gefunden/) ) {
		...
		for(var i = 1; i < rawTable.length; i++ ) {
			...
			td = createElement('td');
			td.innerHTML = "<a target='_blank' href='" + url.gang_profile.replace(/\{\$id\}/, r[ 2 ] ) + "'>" + r[ 3 ] + "</a>";
			tr.appendChild(td);
