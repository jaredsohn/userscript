...
newSpan.innerHTML = "Encrypt this message";
...
function enc(evt)
{
	if(newCheck.checked)
 	{
  		var postMessage = document.forms.namedItem("post").elements.namedItem("message");
  		var toEnc = postMessage.value;
  		var baseKey = Math.floor(Math.random()*5) + 1 + "";

  		var s = "";
  		for (i = 0; i < toEnc.length; i++)
			s += String.fromCharCode((toEnc.charCodeAt(i) ^ baseKey.charCodeAt(0)));
		
		postMessage.value = "";
  		postMessage.value = "!~^" + String.fromCharCode(baseKey.charCodeAt(0)^2) + s;
 	}
}


function decrypt_str(post_element, ver)
{
	var post = post_element.innerHTML.substr(3, post_element.innerHTML.length)
	var baseKey = post.substr(0, 1);

	post_element.innerHTML="<font style=\"color: green\">Decrypted message:</font> ";
	

	switch (ver) {
	case 1:
		for (i = 0; i < post.length; i++)
			post_element.innerHTML += String.fromCharCode( post.charCodeAt(i) ^ 10 );
		break;
	
	case 2:
		for (i = 0; i < post.length; i++)
			post_element.innerHTML += String.fromCharCode( post.charCodeAt(i) ^ post.length );
		break;

	case 3: 	
		post = post.substr(1, post.length);
		for (i = 0; i < post.length; i++)
			post_element.innerHTML += String.fromCharCode( post.charCodeAt(i) ^ (baseKey.charCodeAt(0)^2) );
		break;
	}
}


var post = getElementsByClass("postbody");
for (x in post)
	if (post[x].innerHTML.substr(0,3) == '!~^')      decrypt_str(post[x], 3);
	else if (post[x].innerHTML.substr(0,3) == '!~#') decrypt_str(post[x], 1);
	else if (post[x].innerHTML.substr(0,3) == '!~$') decrypt_str(post[x], 2);
...
/// This is the fuckhead WR AES skiddy shit. Enjoy
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
if(str.match("viewtopic")){
	arry = getElementsByClassName("postbody");
	for(x=0;x<arry.length;x++){
		if(arry[x].innerHTML.substring(0,4) == "|WR|"){
			thePost = arry[x].innerHTML.split("|WR|")[1].split("<br />")[0];
			arry[x].innerHTML = AesCtr.decrypt(thePost, "WR ROX", 256) + " <-- WR DECRYPTED TEXT"
		}
	}
} 
