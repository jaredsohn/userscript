// ==UserScript==
// @name            Google Reader Starred Counter(chrome)
// @description     Google Reader Starred Counter for Chrome 
// @version         1.0.0
...
**Google Reader Starred Counter(repair)
***http://userscripts.org/scripts/show/48094


**Google Reader Unread Count (Angus)
***http://sites.google.com/site/angusdev/projects/google-reader-unread-count
...
function waitForReady() {
  if (starredEle = counterEle()) {
	...
	document.addEventListener("SiteScriptMessage", function (response) { 
		USER_ID = JSON.parse(response.data);
		requestStar(starredEle);
	}, false); 

	location.href = 'javascript:(' + String(function(){ 
		var SOMEDATA = _USER_ID; 
...
function requestStar() {
	...
	xhr.onload = function(){
		if(xhr.status === 200){
			...
			starredEle.innerHTML = ' ('+ entry.length +')';
