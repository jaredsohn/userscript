// ==UserScript==
// @name           Falcon Advanced Tools 
// @namespace      Falcon
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @include        http://apps.facebook.com/kingdomsofcamelot/*
// @description    Additional features for Kingdoms of Camelot
// @require        http://sizzlemctwizzle.com/updater.php?id=96556
// ==/UserScript==


/***********************

Falcon Advanced Tools
-----------------------    

- Features:
 
 * EmailNotify: Sends a Email, if someone attacks you
 * Advanced TowerAlert: adds some additional infos to the alert:
     1. Publish your HQ to Allychat
     2. Troops in embassy(with Knightlevel) 
     3. Own Troops in City
     4. Total Troops
     5. Food, foodusage and time left until runnig out of food
     6. Defense Fortifications in city
     7. Search Own Ally Members
     8. Koc Wiki 

     
-----------------------     
TODO : Add TransportTab
TODO : Add WhisperAlertTab
-----------------------     
History:
3-16-2011 (Version 0.3.01)
   - added Gift tab  credits to george jetson for gift script
   - Added WebSites url
   - Fixed layout added test tab, fixed adv.players online status enhanced 
   - fixed lag issue, food amount corrected, fixed incoming troop amount.
2-11-2011 (Version 0.2.18)
2-11-2011 (Version 0.2.12) 
   - adv.players tab: fixing CityPicker, beautify tableoutput
2-10-2011 (Version 0.2.15) 
   - towerAlert: fixing timeLeft, add Defense Status to Alert
   - adv.players tab: add link to ally from players search
2-09-2011 (Version 0.2.11) 
   - add adv.players tab (+oninestatus, +loginDate)
2-08-2011 (Version 0.2.7) 
   - add board tab
   - change tab theme colors to ally colors
   - fix "Phoenix Tool" tab to fit to tab menue
2-04-2011 (Version 0.2.0) 
   - Initial 
   - change structure to get "Advanced Tools" as Tab working with "Power Tools" and "Power Bot" 
   - add food info and fortification info to Advanced TowerAlert
       
**********************/

var Scriptversion = '0.3.01';
var Scriptversiondate = '3.16.2011';
var Scriptname = 'Advanced Tools';
var ScriptTabName = 'Advanced Tools';
...
function pcStartup (){
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    table.pcMainTab tr td.notSel {font-weight:bold; font-size:13px; border: 1px solid; border-style: solid solid none solid; background-color:#A09AC0; color:white; border-color:black;}\
    tr.pcPopTop td { background-color:#D7AD08; border:none; height: 21px;  padding:0px; }\
  ...
  window.name = 'PC';
  logit ("* "+Scriptname +" v"+ Scriptversion +" Loaded");
  readOptions();
  readGlobalOptions ();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  mainPop = new CPopup ('pc', Options.pcWinPos.x, Options.pcWinPos.y, 750,600, Options.pcWinDrag, 
      function (){
        tabManager.hideTab();
  ...
  actionLog (""+Scriptname +" v"+ Scriptversion +" Loaded");
...
Tabs.board = {
  ...
  myDiv : null,
  timer : null,
  forumlink :  'http://uniquehacks.net/',  
...
Tabs.sample = {
  ...
  init : function (div){    // called once, upon script startup
    ...
    div.innerHTML = '<CENTER><BR>This is a sample tab implementation<BR><BR>Showing food for '+ cityName +' : <SPAN id=pcSampleFood>0</span>\
  ...
  show : function (){         // called whenever this tab is shown
    ...
    document.getElementById('pcSampleFood').innerHTML = addCommas (food);
    clearTimeout (t.timer);
...
Tabs.Test = {
  ...
  init : function (div){
    var t = Tabs.Test;
    t.myDiv = div;
    var m = '<TABLE><TR><TD align=right>Scout: </td><TD><INPUT type=checkbox id=pcfakeIsScout></td></tr>\
        <TR><TD align=right>Wild: </td><TD><INPUT type=checkbox id=pcfakeIsWild></td></tr>\
        <TR><TD align=right>False Report: </td><TD><INPUT type=checkbox id=pcfakeFalse></td></tr>\
        <TR><TD align=right>Seconds: </td><TD><INPUT type=text size=4 value=300 id=pcfakeSeconds></td></tr>\
        <TR><TD align=right># of Militia: </td><TD><INPUT type=text size=6 value=5000 id=pcfakeMilitia></td></tr>\
        <TR><TD colspan=2 align=center><INPUT id=pctestSendMarch type=submit value="Fake Attack" \></td></tr></table>\
        <INPUT id=pcReloadKOC type=submit value="Reload KOC" \>\
        <BR>Force ajax errors : <INPUT type=checkbox id=pcajaxErr>\
...
Tabs.Options = {
  ...
  init : function (div){
    ...
    for (var c=0; c<Cities.numCities; c++) {
         ...
         citySelect += '<option value=\''+ aCity +'\' '+ (Options.alertConfig.hg==aCity?'SELECTED ':'') +'>'+aCity+'</option>';
    }
	citySelect += '</select>';
	    
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pcTabLined cellspacing=0 cellpadding=0>\
        <TR><TD colspan=2><B>TowerAlert Config:</b></td></tr>\
        <TR><TD><INPUT id=pcalertEnable type=checkbox '+ (Options.alertConfig.aChat?'CHECKED ':'') +'/></td><TD>Automatically post incoming attacks to alliance chat.</td></tr>\
	          <TR><TD></td><TD><TABLE cellpadding=0 cellspacing=0>\
	              <TR><TD align=right>Message Prefix: &nbsp; </td><TD><INPUT id=pcalertPrefix type=text size=60 maxlength=120 value="'+ Options.alertConfig.aPrefix +'" \></td></tr>\
	              <TR><TD align=right>Alert on scouting: &nbsp; </td><TD><INPUT id=pcalertScout type=checkbox '+ (Options.alertConfig.scouting?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Alert on wild attack: &nbsp; </td><TD><INPUT id=pcalertWild type=checkbox '+ (Options.alertConfig.wilds?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Minimum # of troops: &nbsp; </td><TD><INPUT id=pcalertTroops type=text size=7 value="'+ Options.alertConfig.minTroops +'" \> &nbsp; &nbsp; <span id=pcalerterr></span></td></tr>\
	              <TR><TD align=right>Show troops in embassy: &nbsp; </td><TD><INPUT id=pcalertEmbassy type=checkbox '+ (Options.alertConfig.embassy?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Show my troops: &nbsp; </td><TD><INPUT id=pcalertMytroops type=checkbox '+ (Options.alertConfig.mytroops?'CHECKED ':'') +'/></td></tr>\
	              <TR><TD align=right>Show food usage: &nbsp; </td><TD><INPUT id=pcalertFood type=checkbox '+ (Options.alertConfig.food?'CHECKED ':'') +' /></td></tr>\
	              <TR><TD align=right>Show my defense: &nbsp; </td><TD><INPUT id=pcalertDefense type=checkbox '+ (Options.alertConfig.defense?'CHECKED ':'') +' /></td></tr>\
	              <TR><TD align=right>My headquarter: &nbsp; </td><TD>'+ citySelect +'&nbsp; &nbsp;</td></tr>\
	              <TR><TD align=right>Send alert to ally chat: &nbsp; </td><TD><INPUT id=pcalertSendToAlly type=checkbox '+ (Options.alertConfig.sendtoAlly?'CHECKED ':'') +' /></td></tr>\
				  <TR><TD align=right>Send alert as whisper to myself: &nbsp; </td><TD><INPUT id=pcalertSendAsWhisper type=checkbox '+ (Options.alertConfig.sendasWhisper?'CHECKED ':'') +' /></td></tr>\
				  <TR><TD align=right>Send alert email: &nbsp; </td><TD><INPUT id=pcalertSendEmail type=checkbox '+ (Options.alertConfig.sendEmail?'CHECKED ':'') +' /></td></tr>\
		          <TR><TD align=right>My email address: &nbsp; </td><TD><INPUT id=pcalertEmailAddress type=text size=36 value="'+ Options.alertConfig.emailAddress +'" \> &nbsp; &nbsp;</td></tr>\
				  <TR><TD align=right>My token: &nbsp; </td><TD><INPUT id=pcalertToken type=text size=36 value="'+ Options.alertConfig.token +'" \> &nbsp; &nbsp;</td></tr>\
	              </table></td></tr>\
  ...
  e_alertOptChanged : function (){
	...
	Options.alertConfig.sendtoAlly=document.getElementById('pcalertSendToAlly');
...
//SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>onlinestatus</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>last Login</a></span>
...
Tabs.AllianceList = {
  tabOrder : 25,
  tabLabel : 'Adv.Players',
  myDiv : null, //change cont with myDiv
  dat : [],
  curPage : 0,
  ...
  init : function (div){
    ...
    t.myDiv = div;
    unsafeWindow.PCgetMembers = t.eventGetMembers;
    unsafeWindow.PCpd = t.clickedPlayerDetail;
    unsafeWindow.PCpl = t.clickedPlayerLeaderboard;
    ...
    var m = '<DIV class=pcentry><TABLE width=100% cellpadding=0>\
        <TR><TD class=xtab align=right></td><TD class=xtab>Enter all or part of a player name: &nbsp;</td>\
          <TD width=80% class=xtab><INPUT id=pcallPlayName size=20 type=text /> &nbsp; <INPUT id=pcplaySubmit type=submit value="Find Player" /></td>\
          <TD class="xtab pcErrText"><SPAN id=pcplayErr></span></td></tr>\
        <TR><TD class=xtab>OR: </td><TD class=xtab> Enter all or part of an alliance name: &nbsp;</td>\
          <TD class=xtab><INPUT id=pcallAllName type=text /> &nbsp; <INPUT id=pcallSubmit type=submit value="Find Alliance" /></td>\
          <TD class="xtab pcErrText"><SPAN id=pcallErr></span></td></tr>\
		<TR><TD class=xtab>OR: </td><TD class=xtab align=right> List alliances: &nbsp;</td>\
            ...
            <INPUT align=left id=allListSubmit type=submit value="List all" /></td></tr></table><span style="vertical-align:middle;" id=pcaltInput></span></div>\
        <SPAN id=pcallListOut> &nbsp; </span>';
    t.myDiv.innerHTML = m;
    document.getElementById('pcallSubmit').addEventListener ('click', t.eventSubmit, false);
    document.getElementById('pcplaySubmit').addEventListener ('click', t.eventPlayerSubmit, false);
    document.getElementById('pcallAllName').addEventListener ('focus', function (){document.getElementById('pcallErr').innerHTML='';}, false);
    document.getElementById('pcallPlayName').addEventListener ('focus', function (){document.getElementById('pcplayErr').innerHTML='';}, false);
  ...
  hide : function (){
  	    mainPop.div.style.width = 750 + 'px';
  	  	mainPop.div.style.height = 600 + 'px';
  	  	mainPop.div.style.maxHeight = 600 + 'px';  	  	  	  	
  },

  show : function (){
  	    mainPop.div.style.width = 1110 + 'px';
  	  	mainPop.div.style.height = 700 + 'px';
  	  	mainPop.div.style.maxHeight = 700 + 'px';  	  	
  },
  ...
  eventPlayerSubmit : function (){
    var t = Tabs.AllianceList;
    document.getElementById('pcplayErr').innerHTML='';
    var name = document.getElementById('pcallPlayName').value;
    t.pName = name;
    if (name.length < 3){
      document.getElementById('pcplayErr').innerHTML = 'Enter at least 3 characters';
      return;
    }
    document.getElementById('pcaltInput').innerHTML = '';
    document.getElementById('pcallListOut').innerHTML = '<BR><BR><CENTER>Searching ...</center>';
  ...
  eventGotPlayerList : function (rslt){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      document.getElementById('pcallListOut').innerHTML = rslt.errorMsg;
      return;
    }
    var m = '<DIV class=pcstat>Showing players matching <B>"'+ t.pName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE width=95% align=center class=pcTab cellspacing=0><TR style="font-weight:bold"><TD>Name</td>\
    ...
    for (k in rslt.matchedUsers){
      ...
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ addCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="PCpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="PCpl(this, \''+ u.userId +'\')"><A>leaderboard</a> &nbsp; <BR></span><SPAN onclick="PCpo(this, \''+ u.userId +'\')"><A>onlinestatus</a></span>&nbsp; <BR></span><SPAN onclick="PCplo(this, \''+ u.userId +'\')"><A>last Login</a></span></td></tr>';
    }
    m += '</table></div>';
    document.getElementById('pcallListOut').innerHTML = m;
  },
  ...
  gotPlayerLeaderboard : function (rslt, span){
    ...
    if (!an || an=='' || p.officerType==4)
      ...
      an += '<a onclick="PCgetMembers('+ p.allianceId +')">('+ officerId2String(p.officerType) +')</a>';
    m = '<TABLE cellspacing=0 class=pcTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Might: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>'; 
    for (var i=0; i<p.cities.length; i++){
      ...
      +' (<a onclick="pcGotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; level: '
  ...
  gotPlayerDetail : function (rslt, span){
    ...
    var a = 'None';
    if (u.allianceName)
      a = '<a onclick="PCgetMembers('+ u.allianceId +')">'+u.allianceName +' ('+ getDiplomacy(u.allianceId) + ')</a>';
    var m = '<TABLE cellspacing=0 class=pcTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; Cities: '
  ...
  eventSubmit : function (){
    var t = Tabs.AllianceList;
    document.getElementById('pcallErr').innerHTML='';
    t.aName = document.getElementById('pcallAllName').value;
    if (t.aName.length < 3){
      document.getElementById('pcallErr').innerHTML = 'Enter at least 3 characters';
      return;
    }
  ...
  eventGotAllianceList : function (rslt){
    ...
    var m = '<DIV class=pcstat>Showing alliances matching <B>"'+ t.aName +'"</b></div>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PCgetMembers('+ all.allianceId +')">View Members</a></td></tr>';
    }
    document.getElementById('pcallListOut').innerHTML = m;
  },
...
//New Allylist
  eventListSubmit : function (){
    var t = Tabs.AllianceList;
    var myA = getMyAlliance ();
    document.getElementById('pcallListOut').innerHTML = '<BR><BR><CENTER> ...</center>';
    if (myA[0]!=0  ) {
       t.curPage=1;
       t.fetchOtherAllianceInfo ( 1, t.eventGotOtherAlliancePage);
    }
    else {
       document.getElementById('pcallListOut').innerHTML = 'You must be an alliance member to use this feature.';
    }
  },
  ...
  gotoPage : function (){
    ...
    if (t.MaxPage < 0 ) {
      document.getElementById('pcallListOut').innerHTML = 'List Alliances first.';
      return;
    }
    if (t.MaxPage < 0 || val > t.MaxPage || val < 1) {
      document.getElementById('pcallListOut').innerHTML = 'Page number out of range';
      return;
    }
    t.curPage = val;
    document.getElementById('pcallListOut').innerHTML = '<BR><BR><CENTER> ...</center>';
  ...
  eventGotOtherAlliancePage : function (rslt){
    ...
    for (var i=0; i<rslt.otherAlliances.length; i++) {
      ...
      m += '<TR class="'+ dip + '"><TD class=xtab>' + alliance.name +'</td><TD align=right class=xtab>'+ alliance.ranking +'</td><TD align=right class=xtab>'+ alliance.membersCount +'</td>\
       ...
       <TD class=xtab><a onclick="PCgetMembers('+ alliance.allianceId +')">View Members</a></td></tr>';
    }
    ...
    m += ' &nbsp; page <INPUT align=right id=idPageNum type="text" size=4 value='+ t.curPage + ' /> of '+t.MaxPage+' <INPUT align=right id=allGotoPage type=submit value="Jump to page" />';
  ...
  reDisp : function (){
    ...
    for (var i=0; i<t.dat.length; i++){
      ...
      m += '<TR style="max-height:30px"><TD class='+ tabclass +'>'+ t.dat[i][0] +'</td><TD align=right class='+ tabclass +'>'+ addCommasInt(t.dat[i][1]) +'</td><TD align=center class='+ tabclass +'>'+ t.dat[i][3] +'</td>\
            ...
            <TD align=center class='+ tabclass +'><DIV onclick="pcGotoMap('+ t.dat[i][5] +','+ t.dat[i][6] +')"><A>'+ t.dat[i][5] +','+ t.dat[i][6] +'</a></div></td>\
            <TD align=right class='+ tabclass +' style="padding-right:20px;">'+ t.dat[i][8].toFixed(2) +'</td><td class='+ tabclass +'><SPAN onclick="PCpo(this, \''+t.dat[i][9] +'\')"><A>onlinestatus</a></span></td><td class='+ tabclass +'><SPAN onclick="PCplo(this, \''+ t.dat[i][9] +'\')"><A>last Login</a></span><td></tr>';
    }
    var tbody = document.getElementById('pcallBody');
...
//new CtableToText('tabAllMembers').toText();
  ...
  displayMembers : function (allName, numPlayers){
    ...
    unsafeWindow.PCalClickSort = alClickSort;
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
            ...
            .xxtab{background-color:none; padding-left:2px; padding-right:1px;}\
            .xxtab_even{background-color:#F6E3CE; padding-left:2px; padding-right:1px;} </style>\
      <DIV class=pcstat><TABLE id=tabAllMembers cellpadding=2  cellspacing=2 width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
      ...
      <TABLE id=tabAllMembers align=center cellpadding=2 cellspacing=2 width=90%><THEAD>\
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="PCalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
        <TD id=clickCol1 onclick="PCalClickSort(this)" class=clickable align=center><A><DIV>Might</a></div></td>\
        <TD id=clickCol3 onclick="PCalClickSort(this)" class=clickable><A><DIV>Cities</a></div></td>\
        <TD id=clickCol2 onclick="PCalClickSort(this)" class=clickable align=center><A><DIV>Rank</a></div></td>\
        <TD id=clickCol7 onclick="PCalClickSort(this)" class=clickable><A><DIV>City Name</a></div></td>\
        <TD id=clickCol4 onclick="PCalClickSort(this)" class=clickable><A><DIV>Lvl</a></div></td>\
        <TD id=clickCol5 onclick="PCalClickSort(this)" class=clickable><A><DIV>Coords</a></div></td>\
        <TD id=clickCol8 onclick="PCalClickSort(this)" class=clickable><A><DIV>Distance</a></div></td>\
        <TD><A><DIV>Status</a></div></td><TD><A><DIV>Last Login (for CET +9)</a></div></td></tr></thead>\
      <TBODY id=pcallBody style="background-color:#ffffff; overflow-y:auto; overflow-x:hidden;"></tbody></table>\
      <DIV style="top:670px; left:0px; position:absolute; width:90%;  background-color:#ffffff; border-top:1px solid; margin-top:8px; color:#700; font-weight:bold;">\
    ...
    document.getElementById('pcallListOut').innerHTML = m;
    document.getElementById('pcaltInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
...
var TowerAlerts = {
  ...
  e_eachSecond : function (){   // check for incoming marches
	...
	postToChat: function(m, force){
		...
		/** ** My Defense Status ** */
		var defstat = parseInt(Seed.citystats["city" + m.toCityId].gate);
		if (defstat == 1) {
			defstatString = 'ON';
		} else {
			defstatString = 'OFF';
		...
		msg += 'Typ: ' + atkType + '. ';
		msg += 'Target: ' + target + '(DEF: '+defstatString+') . ';
		...
		msg = msg.slice(0, -2);
		msg += ' (arriving in ' + unsafeWindow.timestr(parseInt(m.arrivalTime - unixTime())) + ' -> ' + arrivingDatetime +').' ;
		...
		if (city.tileId == m.toTileId) {
			...
			if (emb.count > 0) {
				...
				msg += 'My embassy (' + availSlots + '/' + emb.maxLevel + ')';
		...
		for (r = 1; r < 13; r++) {
			...
			if (defstat == 1) { 
				tot[r] += num; //Nur bei aktivierter Def zu DefTruppen mitzählen
			}
			if (num > 0) 
		...
		if (postOptions.food) {
			...
			msg += ' ** FOOD **:';
			msg += ' '+addCommas(food)+ ' (' +addCommas(foodIncome)+') -> time left: '+timeLeftShort+'. ';
		}
		var msgpart5 =msg;
		if (DEBUG_TRACE) {
		...
		if (postOptions.sendEmail) {
			var subject = target + ' is being ' + atkType + ' by ' + who + ' on '+ arrivingDatetime;
...
var CollectGold = {
  ...
  tick : function (){
    ...
    for (var c=0; c<Cities.numCities; c++){
      ...
      if (happy>=Options.pcGoldHappy && since>15*60){
...
var RefreshEvery  = {
  timer : null,
  init : function (){
    if (Options.pcEveryMins < 1)
      Options.pcEveryMins = 1;
    RefreshEvery.setEnable (Options.pcEveryEnable);
  },
  setEnable : function (tf){
    var t = RefreshEvery;
    clearTimeout (t.timer);
    if (tf)
      t.timer = setTimeout (t.doit, Options.pcEveryMins*60000);
  },
  doit : function (){
    actionLog ('Refreshing ('+ Options.pcEveryMins +' minutes expired)');
    reloadKOC();
  }
}
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    m += '<TD class=spacer width=90% align=right>'+ Scriptversion +' ('+ Scriptversiondate + ') &nbsp;</td></tr></table>';
