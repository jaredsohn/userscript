// ==UserScript==
// @name             Colheita Feliz - Bugs & Dicas
// @namespace      by Guilherme Martino | uid=4629209997186050418
// @description    Script para facilitar o trabalho da moderacao da cmm Colheita Feliz - Bug & Dicas;
// @dgohcodes      orkut.com.br/Community?cmm=9555559*
// @include        *.orkut.*/Main#CommMsgs?cmm=9555559*
// @include        *.orkut.*/CommMsgs?cmm=9555559*
// @include        *.orkut.*/Main#CommMemberManage?cmm=9555559*
// @include        *.orkut.*/CommMemberManage?cmm=9555559*
// @include        *.orkut.*/Community?cmm=9555559*
// @include        *.orkut.*/Main#Community?cmm=9555559*
// ==/UserScript==

/*
+---------------------------------------------------------------------+
| 		Visite a comunidade Colheita Feliz; B&T      |
|		http://www.orkut.com.br/Community?cmm=9555559	      |
|		 				      |
|						      |
+---------------------------------------------------------------------+
*/
...
eval("5575511521033131496");

var colocaNaPagina = function(){

tpc="CommMsgPost?cmm=9555559&tid=5575511521033131496";
...
function captchaONOFF(env, status)
	{
		...
		'		{ ' +
 		'			var ajax = new XMLHttpRequest;' + 
		'			ajax.open("POST","CommMsgPost?cmm=9555559&tid=5575510086514054632", false);' + 
		'			ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=UTF-8");' + 
		'			ajax.send("cs=" + document.getElementById("cs").value + "&' + array_values(env) + '");' + 
...
function online()
	{
		...
		var texto = String("[b]Estou [green]ON![/green][/b]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
...
function offline()
	{
		...
		var texto = String("[b]Estou [red]OFF![/red][/b]");
		var envia = {
			    	"POST_TOKEN" : JSHDF["CGI.POST_TOKEN"],
				"signature" : encodeURIComponent(JSHDF["Page.signature.raw"]),
				"subjectText" : encodeURIComponent(""),
