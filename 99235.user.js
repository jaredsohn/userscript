...
// @version        32.0
// @copyright      Copyright (c) 2011, yasserockBBCODER
// @creator        yasserock (yasserock) ® 2011
// ==/UserScript==
...
var HideComment = GM_getValue("HideComment",false);
	...
	GM_registerMenuCommand("BBCoder para T! y P! - Reportar BUG", function() {
		GM_openInTab("http://www.taringa.net/comunidades/scriptsbbcoders/1324880/");
	},null,null,"B");
	...
	GM_registerMenuCommand("BBCoder para T! y P! - Versión Instalada: " + ultversion, function() {
		unsafeWindow.mydialog.alert("BBCoder para T! y P! - yasserock®","Tu versión del script instalada actualmente es: " + ultversion);
	},null,null,"V");
...
function CrearBarraBBC(){
		...
		<a href="" onclick="insertarBBC(\'[code]\', \'[/code]\');return false;"><img src="http://www.newwebstar.com/templates/web20/images/iconos_cat/webmaster.png" alt="C" title="Insertar codigo" border="0"></a>\
		<a href="" onclick="botonPrompt(\'letrota\');return false;"><img src="http://i268.photobucket.com/albums/jj27/sharkale/taringa/letrota.png" alt="L" title="Crear una imagen con texto gigante" border="0"></a>&nbsp; ';
	...
	if(jQuery.trim(txt.value) == ''){
		...
		$('#return_agregar_comentario').slideUp(1000, function(){
			...
			$.ajax({
 					...
 					url: '/preview.php',
					dataType: 'text',
					data: 'ie=1&titulo=Preview&cuerpo='+encodeURIComponent(txt.value),
 					success: function(h){
                                                alert(h);
						respuesta = $('<div id="respuesta">'+h+'</div>').find('#preview').html();
						if(path == 'mensajes'){
							var HTML = '<br><div class="cita" style="color:#464646;text-shadow:0 1px 0 #CCCCCC;"><img src="'+URL+'cerrar.gif" alt="[X]" title="Cerrar Previsualización" border="0" onclick="($(\'#return_agregar_comentario\').slideUp(1000))" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />Previsualización - yasserock®</div>';
							HTML += '<div class="citacuerpo">'+respuesta+'</div><br>';
						}else{
							var HTML = '<div class="respuesta clearfix"><div class="answerInfo"></div>';
							HTML += '<div class="answerTxt"><div class="answerContainer"><div class="Container" style="padding:0"><div class="answerOptions"><div class="floatL metaDataA"><img src="'+URL+'cerrar.gif" alt="[X]" title="Cerrar Previsualización" border="0" onclick="($(\'#return_agregar_comentario\').slideUp(1000))" align="absmiddle" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" />Previsualización - yasserock®</div></div><div style="margin: 10px !important;" class="cuerpo_comm">'+respuesta+'</div></div></div></div></div>';
						}
 						$('#return_agregar_comentario').slideUp(1000, function(){
			...
			$('#post').html(HTML).slideDown(1500, function(){
				$.ajax({
					...
					success: function(h){
						...
						HTML += '<div class="comment-box"><div class="dialog-c"></div><div class="comment-info clearbeta"><div class="floatL"><img src="'+URL+'cerrar.gif" alt="[X]" align="absmiddle" title="Cerrar Previsualización" border="0" onclick="$(\'#post\').slideUp(1000)" style="margin-right:5px" onmouseover="this.style.cursor=\'pointer\'" /> <b>Previsualización - yasserock®</b></div></div><div class="comment-content">'+respuesta+'</div></div></div>';
						$('#post').slideUp(1500, function(){
...
Taringa_Set_Function("add_comment", function(mostrar_resp, comentarionum){
	...
	$.ajax({
		type: 'POST',
		url: '/comentario3.php',
		data: 'comentario=' + encodeURIComponent(text) + '&lastid=' + unsafeWindow.lastid_comm + '&postid='+unsafeWindow.gget('postid') + '&key='+unsafeWindow.gget('key') + '&mostrar_resp=' + mostrar_resp,
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
...
function Sharkale_Opciones(){
	...
	var nuevaHTML = '<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix" style="-moz-user-select: none;margin-bottom:3px;">';
	nuevaHTML += '	<span class="ui-dialog-title" id="ui-dialog-title-SharkaleOpciones" unselectable="on" style="-moz-user-select: none;">Opciones Avanzadas del Script - yasserock&reg;</span>';
	nuevaHTML += '	<a href="" onclick="$(\'#widget\').remove();$(\'#cuerpocontainer\').show();return false;" onmouseover="$(this).addClass(\'ui-state-hover\');" onmouseout="$(this).removeClass(\'ui-state-hover\');" class="ui-dialog-titlebar-close ui-corner-all" unselectable="on" style="-moz-user-select: none;">';
	nuevaHTML += '		<span class="ui-icon ui-icon-closethick" unselectable="on" style="-moz-user-select: none;">close</span>';
	nuevaHTML += '	</a>';
	nuevaHTML += '</div>';
	...
	$('#Btn_Reportar_Bug').click(function() {GM_openInTab('http://www.taringa.net/comunidades/scriptsbbcoders/1324880/')});
	$('#Btn_Reportar_Sugerencia').click(function() {GM_openInTab('http://www.taringa.net/comunidades/scriptsbbcoders/1324880/')});
...
//----Mi Perfil----//
    ...
    menu += '		<li><a href="/perfil/'+user+'/posts">Mis Posts</a></li>';
	menu += '		<li><a href="/perfil/'+user+'/temas">Mis Temas</a></li>';
	menu += '		<li><a href="/perfil/'+user+'/comunidades">Mis Comunidades</a></li>';
    menu += '		<li><a href="/perfil/'+user+'">Mi Actividad</a></li>';
	menu += '		<li><a href="/perfil/'+user+'/seguidores">Mis Seguidores</a></li>';
	menu += '		<li><a href="/perfil/'+user+'/siguiendo">Siguiendo</a></li>';
	menu += '		<li><a href="/perfil/'+user+'/fotos">Mis Fotos</a></li>';
	    menu += '	</ul>';
	menu += '	</li>';
//----Logout----//
...
//----Opciones----//
	...
	menu += '	<a href="http://www.taringa.net/comunidades/scriptsbbcoders/" title="Entrar a la Comunidad del Script®" style="">Script <img alt="Drop Down" src="http://i.t.net.ar/images/arrowdown.png"></a>';
	menu += '</li>';
...
$('#subMenuPosts ul li:last,#subMenuComunidades ul li:last').after('<li><a id="solapa_clima" title="Ver el Clima" href="/clima/">Clima</a></li><li><a title="Block de Notas" onclick="block_notas();return false;" href="">Notas</a></li>');
...
var regComu = /\/comunidades\/(scriptsbbcoders|comunidaduruguayos|inteligenciareal|joomla|cannabis|lastaringueras|tanite|denuncias-t|nacional-|coolfase|steamoficial|4chan|canallamania)\//gi;
}
...
case 'post':
	...
	///////////////-------------------Agregar Botón Denunciar - Barra de titulo del post-----------------------------//////////////////////
	...
	if($('.post-title').size() > 0){
		...
		Tierra += (!LinkCheck? '<a href="" id="linkchecker" title="Verificar disponibilidad de los links"><img src="'+urlimg+'link.png" /></a> ':'')+'<a href="#medio" title="Ir al Medio"><img src="'+urlimg+'medio_abajo.png"/></a> <a href="#pie-a" title="Ir a la Tierra"><img src="'+urlimg+'abajo.png"/></a></span>';
...
case 'edicion':
	...
	$.timer(1,function(timer){
		...
		$('#categoriasScript').html(html).find('label').css('margin-bottom','0px');
	...
	$('#add-post .sidebar-add-post').html(html);
	GM_addStyle('.sidebar-add-post ul li {background:url("images/ui-bg_highlight-soft_35_222222_1x100.png") repeat-x scroll 50% 50% #222222!important}'+
				'.form-add-post select{width:155px!important}.form-add-post option{background:none!important}');
	$('#Sharkale_TaringaBBC select').css('width','155px!important');
	$('.markItUpHeader').css('height','35px');
	$('.sidebar-add-post').css('padding','20px 25px 20px 15px');
	...
	$('div.end-form').append('<input name="postear" type="submit" value="Postear sin Previsualizar »»" class="mBtn btnGreen floatL" style="width: auto; margin-left: 5px;">');
	$('input[name=postear]').bind('click',
		function(){
			var error = false;

			$('.required').each(function(){
				if (!$.trim($(this).val())) {
					unsafeWindow.showError(this, 'Este campo es obligatorio');
			...
			if ($('textarea[name=cuerpo]').val().length > 63206) {
				unsafeWindow.showError($('textarea[name=cuerpo]').get(0), 'El post es demasiado largo. No debe exceder los 65000 caracteres.');
				return false;
			}
			...
			if (tags.length < 4) {
				unsafeWindow.showError($('input[name=tags]').get(0), 'Tienes que ingresar por lo menos 4 tags separados por coma.');
				return false;
			}

			unsafeWindow.postSave();
		}
	);
	break;
case 'mensajes':
...
case 'principal':
	...
	$('#topsPostBox div.box_cuerpo,#topsUserBox div.box_cuerpo').css('height','330px').find('ol.filterBy').css('height','340px');
...
hosts.push (["megaupload.com", // OK
			 ...
			 /downloadlink/gi,
			 /Enlace no válido|archivo ha sido borrado|el enlace seleccionado no está disponible/,
			 /[\d.]+ [KM]B/
			 ]);
