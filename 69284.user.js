// ==UserScript==
// @name           Hs Moderacao
// @namespace      editado por julhinho | uid=12422895694866262691
// @description    Script para facilitar o trabalho da moderacao PDO+
// @PDO+     www.orkut.com.br/Main#Community?cmm=95689255
// @include        *.orkut.*/Main#CommMsgs?cmm=95689255*
// @include        *.orkut.*/CommMsgs?cmm=95689255*
// @include        *.orkut.*/Main#CommMemberManage?cmm=95689255*
// @include        *.orkut.*/CommMemberManage?cmm=95689255*
// @include        *.orkut.*/Community?cmm=95689255*
// @include        *.orkut.*/Main#Community?cmm=95689255*
// ==/UserScript==
/*
+---------------------------------------------------------------------+
| 		Visite a comunidade Hacker street            |
|		http://www.orkut.com.br/Community?cmm=95689255	      |
|	
|     este script foi criado por josep,e editado por julhinho    |
|						                      |
+---------------------------------------------------------------------+
*/


Editado_por = "julhinho";
...
var colocaNaPagina = function() {
	...
	function captchaBAN(env) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				ajax.open("POST", "CommMsgPost?cmm=97399793&tid=5431035078708784113", false);
	...
	function captchaONOFF(env, status) {
		...
		button.onclick = function() {
			if (document.getElementById("cs").value == "") alert("Esqueceu de digitar o captcha!");
				...
				var ajax = new XMLHttpRequest;
				env.cs = document.getElementById("cs").value;
				ajax.open("POST", "CommMsgPost?cmm=97399793&tid=5431034855370484721", false);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
				ajax.send(array_values(env));
	...
	function postaTopicoDeletado(titulo, uid, motivo) {
		...
		ajax.open("POST", "CommMsgPost?cmm=95689255&tid=5439585598565913862", false);
	...
	function online() {
		...
		var ajax = new XMLHttpRequest;
		ajax.open("POST", "CommMsgPost?cmm=95689255&tid=5439585942163297542", false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
		ajax.send(array_values(envia));
	...
	function offline() {
		...
		var ajax = new XMLHttpRequest;
		ajax.open("POST", "CommMsgPost?cmm=95689255&tid=54395859421632975426", false);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
		ajax.send(array_values(envia));
