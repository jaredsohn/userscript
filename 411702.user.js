...
// @version    0.2
// @description  Scrobbling your music to Last.fm when using the web music player of xiami.com.
// @match      http://www.xiami.com/play*
// @copyright  2014+, WeirdFish
// ==/UserScript==
 ...
 */
var init = function(){
    console.log('init scrobbler...');
    scrobbler.setSongInfoFN(getSongInfo, {checktime: 2000});
};
...
var getSongInfo = function(){
    var song = {};
    var arr = document.getElementById('J_trackInfo').childNodes;
    song.title = document.getElementById('J_trackName').innerText;
    song.artist = arr[3].innerText;
    song.duration = timeParse(document.getElementById('J_durationTime').innerText);
    song.playTime = timeParse(document.getElementById('J_positionTime').innerText);
    song.album = document.getElementById('J_playerCover').childNodes[0].title.split('-')[0];
    return song;
};
...
//do not send scrobbles unless 90% of a song has been played
var playerDot = document.getElementById('J_playerDot');
if(playerDot){
    playerDot.addEventListener('click', function(e){
        var oldTime = getSongInfo().playTime;
        setTimeout(function(){
            var newTime = getSongInfo().playTime;
            offset = oldTime - newTime;
            scrobbler.seek(offset);
        }, 0);
    }, true);
}

//scrobbling times & love
scrobbler.on('nowplaying', function(){
    var loveElement = document.getElementById('J_trackFav');
    loveElement.addEventListener('click', function(){
        if(loveElement.title == '收藏'){
            scrobbler.love();
        }
        else if(loveElement.title = '取消收藏'){
            scrobbler.unlove();
        }
    });
    scrobbler.getInfo(scrobbler.song, function(info){
        //times
        var span = document.getElementById('lastfm-scrobbles');
        if(span){
            span.innerText = '在 last.fm 中记录了 ' + info.len + ' 次';
        }
        else{
            span = document.createElement('span');
            span.innerText = '在 last.fm 中记录了 ' + info.len + ' 次';
            span.style.cssText = 'display:inline-block; padding-left:20px;';
            span.id = 'lastfm-scrobbles';
            var parentDiv = document.getElementById('J_trackInfo');
            parentDiv.appendChild(span);
        }

        //love
        if(info.islove == '1' && loveElement.title == '收藏'){
            loveElement.click();
        }
        if(info.islove == '0' && loveElement.title == '取消收藏'){
            scrobbler.love();
        }
    });
});
