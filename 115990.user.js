...
var defaultCfg = [
	...
	0,		// 26	expand images (0=off, 1=simple, 2=+preview)
...
function removeFavorities(el) {
	...
	$del($up(el, 2));
	if(arr.length == 0)
		$1($id('DESU_favor')).innerHTML = '<b>Selected threads missing...</b>';
	setStored('DESU_Favorities', arr.join('|'));
}
...
function addControls() {
	...
	$before(el, [
		$New('div', [
			...
			$btn('Hidden posts preview', hiddenPostsPreview),
			$btn('Favorite threads preview', favorThrdsPreview),
			$new('input', {
				'type': 'button',
				'id': 'refresh_btn',
				'value': 'Refresh'}, {
			...
			$if(isMain && pr.on, $btn('Create a thread', function() {$disp(pr.area)})),
			$if(!isMain, $new('span', {
				'html': '[<a href="http://' + location.hostname + '/' + brd + '/">Back</a>]',
				'style': 'float:right'
			}))
			], {
			'id': 'DESU_panel',
	...
	$append($id('DESU_controls'), [
		...
		$New('div', [
			divBox(2, 'Repeat Rows'),
			divBox(3, 'Repeat Words'),
			divBox(4, 'Special Characters'),
			divBox(5, 'Long Columns'),
			divBox(6, 'Long Words'),
			divBox(7, 'numbers'),
			divBox(8, 'CBSA/Register')
			], {
			'id': 'antiwipe_cfg',
			'style': 'display:none; padding-left:15px'
		}),
		chBox(14, 'Hide the text more ', toggleMaxtext, 'maxtext_hider'),
		$new('input', {'type': 'text', 'id': 'maxtext_field', 'value': Cfg[15], 'size': 4}),
		$txt(' character'),
		$new('br'),
		$if(hasSage, chBox(9, 'With soot ', toggleSage, 'sage_hider')),
		$if(pr.subj, chBox(10, 'With the theme ', toggleTitle)),
		chBox(11, 'No text ', toggleNotext, 'notext_hider'),
		chBox(12, 'No image ', toggleNoimage, 'noimage_hider'),
		$new('br'),
		chBox(13, 'expressions: ', toggleRegexp, 'regexp_hider'),
		$new('span', {
			'html': '[<a>?</a>]',
			'style': 'cursor:pointer'}, {
			'click': function() {$alert('LOLWUT В тексте/теме поста:\nололо\nОП хуй\n...\n\nРегулярные выражения: $exp выраж.\n$exp /[bб].[tт]+[hх].[rр][tт]/ig\n$exp /кукл[оа]([её]б|бляд|быдл)/ig\n\nКартинки: $img [<,>,=][вес][@ширxвыс]\n$img <35@640x480\n$img >@640x480\n$img =35\n\nИмя/трипкод: $name [имя][!трипкод][!!трипкод]\n$name Sthephan!ihLBsDA91M\n$name !!PCb++jGu\nЛюбой трипкод: $alltrip\n\nАвтозамена (после перезагр.): $rep искомое заменяемое\n$rep /\:cf:/ig <img src="http://1chan.ru/img/coolface.gif" />\n$rep /(ху[йияеё])/ig <font color="red">beep</font>')}
		}),
		$attr($btn('Apply', function() {applyRegExp()}), {'style': 'float:right'}),
		$new('br', {'clear': 'both'}),
		$new('textarea', {
			'id': 'regexp_field',
		...
		$new('br'),
		optSel('prochidden_sel', ['Do not change', 'merge', 'delete'], 16, function() {
			processHidden(this.selectedIndex, Cfg[16]);
		}),
		$txt(' hidden posts'),
		divBox(19, 'Apply filters to the thread'),
		divBox(18, 'Additional menu on the button to hide'),
		divBox(17, 'View hidden by pointing to the number'),
		$new('hr', {'style': 'margin:5px 0'}),
		optSel('upload_sel', ['Off.', 'Auto', 'Score + click', 'by clicking'], 20),
		$txt(' loading of posts in the thread,*, T='),
		optSel('upload_int', [0.5, 1, 1.5, 2, 5, 15, 30], 21),
		$txt('min*'),
		$new('br'),
		optSel('refprv_sel', ['Off.', 'No cards', 'The card'], 24),
		$txt(' Navigation>> links*, '),
		chBox(25, 'delay'),
		$New('div', [
			optSel('imgexp_sel', ['not', 'usually', 'With preview'], 26),
			$txt(' open image')
		]),
		$New('div', [
			$txt('The argument:'),
			chBox(30, 'player YouTube*'),
			chBox(31, 'player mp3*'),
			chBox(32, 'Pictures*')
		]),
		$if(!(ch._4ch && !(sav.GM || sav.script)),
			divBox(27, 'Share it without rebooting (check response) *')),
		divBox(28, 'animate notice'),
		divBox(29, 'Buttons posts in the form of text *'),
		$if(!ch.dc, divBox(33, 'Disclose the reduction in posts*')),
		$new('span', {'style': 'float:right; padding: 0 100px 115px 0; '
			+ 'background:url(http://krautchan.net/banner) no-repeat'}),
		divBox(34, 'insert post number on click*'),
		divBox(35, 'Hide the names in posts', function() {toggleCfg(35); scriptStyles()}),
		$if(ch._2ch, divBox(36, 'No scrolling in posts', function() {toggleCfg(36); scriptStyles()})),
		divBox(37, 'reveal spoilers', function() {toggleCfg(37); scriptStyles()}),
		$if(hasSage, divBox(38, 'Sage instead of the field E-mail*')),
		$if(pr.on, divBox(40, 'Transfer form the answer down*')),
		optSel('caplang_sel', ['Откл.', 'Eng', 'Rus'], 41),
		$txt(' force captcha input'),
		$if(pr.on, $New('div', [
			optSel('txtbtn_sel', ['Off.', 'Graphical.', 'Uprosch.', 'standard.'], 22, function() {
				saveCfg(22, this.selectedIndex);
				$Del('.//div[@id="txt_btns"]');
				if(Cfg[22] != 0) {textFormatPanel(pr); if(qr.on) textFormatPanel(qr)}
			}), 
			$txt(' formatting buttons '),
			chBox(23, 'down ', function() {
				if(Cfg[22] != 0) {textFormatPanel(pr); if(qr.on) textFormatPanel(qr)}
			})
		])),
		$if(pr.name, $New('div', [
			$new('input', {'type': 'text', 'id': 'usrname_field', 'value': Cfg[44], 'size': 20}),
			chBox(43, ' Constant name', function() {
		...
		$if(pr.passw, $New('div', [
			...
			chBox(45, ' Permanent password', function () {
		...
		$if(pr.on, $txt('Do not show: ')),
		$if(pr.rules, chBox(47, 'regulations ',
			function() {$disp(pr.rules); if(qr.on) $disp(qr.rules)})),
		$if(pr.gothr, chBox(48, 'field goto ',
			function() {$disp(pr.gothr); if(qr.on) $disp(qr.gothr)})),
		$if(pr.passw, chBox(49, 'password ',
...
function hiddenPostsPreview() {
	...
	forAll(function(post) {if(post.Vis == 0) {
		...
		$append(table, [
			$if(((!pp && tcnt++ == 0) || (pp && pcnt++ == 0)), $new('tr', {
				'html': '<th><b>hidden ' + (pp ? 'positions' : 'Trades') + ':</b></th>'
			})),
			$New('tr', [
				cln,
	...
	if(!table.hasChildNodes()) {
		table.innerHTML = '<tr><th>Hidden missing...</th></tr>';
		return;
	}
...
function favorThrdsPreview() {
	...
	var table = $x('.//tbody', div);
	var data = getStored('DESU_Favorities');
	if(!data) table.innerHTML = '<tr><th>Selected Trades missing...</th></tr>';
	else {
		table.appendChild($new('tr', {'html': '<th><b>Selected Trades:</b></th>'}));
		var arr = data.split('|');
		for(var i = 0; i < arr.length/4; i++) {
			var host = arr[i*4];
			var b = arr[i*4 + 1];
...
function selectPostHider(post) {
	...
	var a = addSelMenu('phide_' + post.Num, 0, Cfg[29] == 0 ? 14 : 19,
		['hide selection', 'hide images', 'Hide similar text']);
...
function formSubmit(obj) {
	$event($attr(obj.subm, {'value': 'Send'}), {'click': function() {
		if(Cfg[27] == 1) {
...
function textFormatPanel(obj) {
	...
	var btns = $New('div', [
		...
		tfBtn('oblique', '*', 'i', 'i', 'font-style:italic', pre + pre2 +'V5YBQtZGmepjg+bOu+7hIxD2LfeI4/DK3/Op4PSEQIa0TI4XcsKpk9ZBHKcCSuWKwym3X0rFztIXz1VskJJQRtBofV7G9jTp8r6/g2nn7fz80Lfmp+cws9gXt9hIYMiHiKfoyOhIuHlJeSl5SGIyienioKoqOkpaQiIQA7', tx),
		...
		tfBtn('Strikethrough', !ch._410 ? '' : '^^', 's', 'S', 'text-decoration:line-through', pre +'2hoaE1NTf' + pre3 +'NoCQtZGmepjg+bOu+7iIxD2LfeI4/DK3/Op4PSEQIazrIYQn5HXXL6KGZe+KUkIQWW+05tOAlWCseO7zjBDbNPjO+aog8Kq/XtW54en5g470NgYKDWIOBeYNLhoqGbguEU4KFhgs9j4lSBxGGgZUMl5BMnJ2Wo6aDnqCno6mrp5UjKLKyKgq2t7i5uCIhADs=', tx),
...
function allImgExpander() {
	...
	$id('DESU_panel').appendChild($new('span', {
		...
		'click': function() {
			...
			btn.innerHTML = /Раскрыть/.test(btn.innerHTML) ? '[<a>Collapse images</a>]&nbsp;' : txt;
		}}));
}
...
function parseJSONdata(x) {
	...
	for(var i = 0, tLen = thrds.length; i < tLen; i++) {
		...
		for(var j = 0, pLen = posts.length; j < pLen; j++) {
			...
			ajaxPosts[tNum][pNum] = '<label><a class="delete icon"><img src="/images/blank.png"></a>' + (x.sage ? '<img src="/images/sage-carbon.png" alt="Сажа" title="Сажа">' : '') + (x.subject ? '<span class="replytitle">' + x.subject + '</span>' : '') + '<span class="postername">' + x.name + '</span> ' + x.date + ' </label><span class="reflink"><a href="/' + brd + '/' + res + tNum + '.xhtml#i' + pNum + '">No.' + pNum + '</a></span>' + (j == 0 ? '<span class="cpanel">[<a href="/' + brd + '/' + res + tNum + '.xhtml">Open thread</a>]</span>' : '') + '<br>' + (x.files.length > 0 ? farr.join('') + (x.files.length > 1 ? '<br style="clear: both">' : '') : '') + '<div class="postbody"><div class="message">' + txt.join('<br>').replace(/(^|<br>)(%%<br>)(.+?)(<br>%%.*?)(<br>|$)/g, '<div class="spoiler">$3</div>').replace(/(^|<br>)(``<br>)(.+?)(<br>``.*?)(<br>|$)/g, '<pre>$3</pre>').replace(/quote><br>/g, 'quote>') + '</div></div>' + (x.op == true ? '<div class="abbrev">' + 'Всего ' + thrds[i].posts_count + ' positions, including ' + thrds[i].files_count + ' file</div>' : '');
...
function hideThread(post, note) {
	...
	var x = $new('span', {
		...
		'html': 'Threads <a style="cursor:pointer">№' + post.Num + '</a> hidden <i>('
