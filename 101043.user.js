...
//
// ==UserScript==
// @name          HOMMK Lametastrophe
// @author        Julien Eyries (orig) much rebuilt by fooburger
// @namespace     http://www.jeyries.fr/hommk
// @description   mess with MMHK
// @include       http://mightandmagicheroeskingdoms.ubi.com/*
// @include       http://forums-mightandmagicheroeskingdoms.ubi.com/en/*
// @version       1.0.89
// @date          2011-04-20
// ==/UserScript==
...
if (location.href != 'http://mightandmagicheroeskingdoms.ubi.com/play'){
    if ( document.location.href.match(/pm.php/)) {
        console.log('storing cookie: ' + document.cookie);
        GM_setValue('messagepage','testing');
        GM_setValue('forum-cookie',document.cookie);
    }
    console.log("wrong location : " + location.href);
    return;
}
GM_setValue('mainpage','testing');
...
var version = '1.0.89'; // also, check meta-data field @version
...
function do_request_worldMapAlertAction( json, callback ){
    var xmlhttp= new XMLHttpRequest();
    xmlhttp.open("POST", 'http://mightandmagicheroeskingdoms.ubi.com/ajaxRequest/worldMapAlertAction',true);
    xmlhttp.onreadystatechange=function() {
        if (xmlhttp.readyState==4) {
            callback( JSON.parse(xmlhttp.responseText) );
        }
...
function build( panel ){
   ...
   // build request for player names 
    ...
    var match_regexp = new RegExp('[0-9]+,[0-9]+'); // does javascript handle \d?
    if (typeof(coords) !== 'undefined' && coords.match(match_regexp)) {
        var xxx_vals = coords.split(',');
        moves = moves.filter( function(a) { return ((a.masterHeroMove.x2 == xxx_vals[0]) && (a.masterHeroMove.y2 == xxx_vals[1])); } );
    }
    ...
    table.id = "hommk-vigie-main-table-foo";
    ...
    cell.innerHTML = "Dest";
    var cell = row.insertCell(-1);
    cell.innerHTML = "D/W/H Ranks";
    var cell = row.insertCell(-1);
    cell.innerHTML = "PlayerDom";
    var cell = row.insertCell(-1);
    cell.innerHTML = "Origin";
    var k = 0;

    moves.forEach( function(obj) {

	var playerId = obj.masterHeroMove.playerId;
	var playerTmp = "Looking Up Player: ("+obj.masterHeroMove.playerId+")";
        var row = table.insertRow(-1);
        ...
        var cell = row.insertCell(-1);          
        cell.innerHTML = "scores_"+playerId;          
        var cell = row.insertCell(-1);          
        cell.innerHTML = "domscore_"+playerId;          
        var cell = row.insertCell(-1);          
        $('<a href="#goto" class="ui-state-default">'+coord(move.x1, move.y1)+'</a>')
            .appendTo($(cell))
            .click(function() {

                console.log( "center "+coord(move.x1, move.y1));
        ...
        do_request(
            ...
            ,function(json) {
                for (keyVar in json.d) {
                    var data = json.d[keyVar];
                }

                var lookup_str = "Looking Up Player: ("+data.id+")";
                var score_lookup_str = "scores_"+data.id;
                var score_str = data.dominationPosition+"/"+data.wealthPosition+"/"+data.honorPosition;
                var iterator = document.evaluate('//td[text()=\''+lookup_str+'\']', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null );
                for (var i=0; i<iterator.snapshotLength;i++) {
                    iterator.snapshotItem(i).innerHTML = data.playerName;
                    if (document.getElementById("nonaligned_move").checked && data.allianceId == HOMMK.player.content.allianceId) {
                        var me = iterator.snapshotItem(i);
                        me.parentNode.parentNode.removeChild(me.parentNode);
                    }
                }
                var iterator = document.evaluate('//td[text()=\''+score_lookup_str+'\']', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null );
                for (var i=0; i<iterator.snapshotLength;i++) {
                    iterator.snapshotItem(i).innerHTML = score_str;
                }
                var domscore_lookup_str = "domscore_"+data.id;
                var iterator = document.evaluate('//td[text()=\''+domscore_lookup_str+'\']', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null );
                for (var i=0; i<iterator.snapshotLength;i++) {
                    iterator.snapshotItem(i).innerHTML = Math.round(data.dominationScore / 1000000) + "M";
                }
            }
        );
    });
...
alert('XFORM');
	...
	return dataString;
	//document.getElementById('foo').innerHTML=dataString;
}

	$(panel).append('Launching transform (test)');
	GM_log('Step 0');
	var url = 'http://fooburger.sage.unr.edu/~bchrisman/foo/xslt_test1.xsl';
	GM_log('Step 1');
	GM_xmlhttpRequest({
	//_GM_xmlhttpRequest_({
		method: "GET",
		...
		onload: function(response) {
			...
			$(panel).append(transformXML(xslt));
		},
		onerror: function(response) {
			console.log(response);
			alert('xsltrequest failed: '+response.responseText);
		}
	});
	GM_log('Step 2');
}
...
function toggle_identity(){
    ...
    $dialog = $("<div id='hommk-user-identity'></div>")
        .html(localize("XSLT Placeholder Heading"))
	    .dialog({
            zIndex: 90000,
            width: 427,
            minWidth: 427,
...
function check_frames(){
        ...
        // search for 'button-able' strings
        //    var iterator = document.evaluate('//*[contains(.,\'ClickableCoordinates-\')]', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null );
        //        var re = RegExp("ClickableCoordinates-(\d+)-(\d+)");
        //        for (var i=0; i<iterator.snapshotLength;i++) {
        //            iterator.snapshotItem(i).innerHTML.replace(re, "TestValue+$1+$2");
        //        }
...
function init() { 
    ...
    $('<button id="hommk-user-scribe-button" style="margin-top:5px; display:block;">Spy-Formatter</button>')
        .button({disabled:true})
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    //$('<div>Nonallied Move<input type="checkbox" id="nonaligned_move" style="margin-top:5px; display:block;"/></div>')
    $('<div><input type="checkbox" id="nonaligned_move"/>Nonallied Move</div>')
        .appendTo($panel_extended);

    $('<button style="margin-top:5px; display:block;">Army Movements</button>')
        .button()
        .appendTo($panel_extended)
        .click(function() {
			$(this).button('widget').removeClass( "ui-state-focus" );
    ...
    $('<input type="text" id="coordinates" size=7 style="margin-top:5px; display:block;" value=xxx,yyy />').appendTo($panel_extended);

    //$('<input type="text" id="search_radius_input" size=2 style="margin-top:4px; display:block;" value=1 />').appendTo($panel_extended);;
