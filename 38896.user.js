    ...
    if (gTimeOnly) {
        ...
        if (gBrakeFormat) {
            ...
            if (gParsiShortFormat) {
                p = "<span style='font-family:tahoma; font-size:9pt;' dir='rtl'>" +
                formatPersian(pa[2]) +
                " " +
...
function ToPersianDateShort(gdStr){
    var gd = new Date(gdStr);
    var pa = calcGregorian(gd.getFullYear(), gd.getMonth() + 1, gd.getDate());
    return "<span style='font-family:tahoma; font-size:9pt;' dir='rtl'>" +
    formatPersian(pa[2]) +
    " " +
    PERSIAN_MONTHS[pa[1] - 1] +
    " " +
...
/*
 Name: Persian Date for Blogger
 Developer : Vahid Nasiri
 Lastupdate: 19-12-2008
 */
function replaceFaEn(strIn){
    return strIn.replace("دسامبر", "December").replace("نوامبر", "November").replace("اکتبر", "October").replace("سپتامبر", "September").replace("آگوست", "August").replace("جولای", "July").replace("ژوئن", "June").replace("می", "May").replace("آوریل", "April").replace("مارس", "March").replace("فوریه", "February").replace("ژانویه", "January");
}
...
function YahooDate(data){
    ...
    if (dateArray.length > 2) {
        ...
        if (!isNaN(year)) {
            year += 2000;
            var month = stripAlphaChars(dateArray[0]);
            if (!isNaN(month)) {
                var day = dateArray[1];
                if (!isNaN(day)) {
                    retVal = ToPersianDateShort(new Date(year + "/" + month + "/" + day));
                }
            }
        }
    }
    else {
        data = data.replace("AM", "ق.ض");
        data = data.replace("PM", "ب.ض");
        retVal = data;
    }
...
function getBloggerPYear(data){
    if (!isNaN(data)) {
        ...
        modYear++;
        var pa = calcGregorian(gd.getFullYear(), gd.getMonth() + 1, gd.getDate());
        return formatPersian(pa[0]);
    }
    else 
        return data;
}

function jQuerySet(obj, longFormat){
    var postDate = obj.text().replace("،", ",").replace("،", ",").replace("TH,", ",").replace("th,", ",");
    if (isWordPress) 
        postDate = replaceFaEn(postDate);
    var tryCov = new Date(postDate);
    if (tryCov != "Invalid Date") {
        if (longFormat) 
            obj.html(ToPersianDateLong(tryCov));
        else 
            obj.html(ToPersianDate(tryCov));
    }
    else {
        postDate = postDate.replace("AM", "ق.ض");
        postDate = postDate.replace("PM", "ب.ض");
        obj.html(postDate);
    }
}
...
function letsJQuery(){
    ...
    $("meta").each(function(){
        var obj = $(this);
        var meta = obj.attr("content");
        if (meta == "blogger") 
            isBlogger = true;
        if (meta.toLowerCase().indexOf("wordpress") != -1) 
            isWordPress = true;
    });
    
    
    if (window.location.host.toLowerCase().indexOf("blogspot.com") != -1 ||
    window.location.host.toLowerCase().indexOf("blogger.com") != -1) 
        isBlogger = true;
    
    if (window.location.host.toLowerCase().indexOf("wordpress.com") != -1) 
        isWordPress = true;
    ...
    if (isBlogger) {
        ...
        $("p.comment-timestamp").each(function(){
            var obj = $(this);
            jQuerySet(obj, true);
        });
    }
    ...
    if (window.location.host.toLowerCase().indexOf("mail.yahoo.com") != -1) {
        $("td.sortcol").each(function(){
            var obj = $(this);
            obj.html(YahooDate(obj.text()));
        });
        
        $("nobr").each(function(){
            var obj = $(this);
            obj.html(ToPersianDateLong(obj.html()));
        });
        
        $("nobr.tableViewCellPaddingHoriz").each(function(){
            var obj = $(this);
            obj.html(ToPersianDateLong(obj.html()));
        });
    }
...
function loadJquery(){
    ...
    eval(function(p, a, c, k, e, r){
        ...
        return p
    }('(H(){J w=1b.4M,3m$=1b.$;J D=1b.4M=1b.$=H(a,b){I 2B D.17.5' +
    ...
    'a){I 7["5s"+b]()+25(7,"2C"+c+"4b")+25(7,"2C"+4f+"4b")+(a?25(7,"6S"+c)+25(7,"6S"+4f):0)}})})();', 62, 669, ('|||||||this|||||||||||||||||||||||||||||||||||if|function|return|var|length|data|true|else|type|each|false|for|document|elem' +
