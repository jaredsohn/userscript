...
function showLookupIcon(evt)
{
	...
	divLookup.addEventListener('mouseout', mout, false);
...
function lookup(evt)
{
	...
	var left;
	if (evt.clientX+window.pageXOffset>800)
		left = parseInt(divLookup.style.left)-200;
	else
		left= parseInt(divLookup.style.left)+19;
	...
	//div container
	divDic = createElement('div', {id:'divDic', style:'background-color:#D6E9F8 !important; border:1px solid #0066CC; color:#000000; position:absolute; top:'+top+'; left:'+left+'px; min-width:250px; min-height:50px; max-width:50%; padding:1px; font-size:small; text-align:left; z-index:10000;'});
...
function options(evt)
{
	...
	if(!divOptions)//show options
	{
		...
		//from
		formOp=createElement('select', {id:'optSelLangFrom'}, null, languagesGoogle);
		divOptions.appendChild(createElement('span', null, null,'From:'));
		divOptions.appendChild(formOp);
		...
		//to
		toOp=createElement('select', {id:'optSelLangTo'}, null, languagesGoogle);
		divOptions.appendChild(createElement('span', null, null,' To:'));
		divOptions.appendChild(toOp);
		getId('optSelLangTo').value = GM_getValue('to') ? GM_getValue('to') : "zh-CN";;
		getId('optSelLangTo').addEventListener('change', quickLookup, false);
		
		divOptions.appendChild(createElement('span', null, null,'&nbsp;'));
		divOptions.appendChild(createElement('a', {href:HREF_NO}, 'click swap false', 'Swap'));
