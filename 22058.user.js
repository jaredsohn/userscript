...
a.innerHTML="";
	...
	function preCalculate1(){
		...
		if (marcadores[f][5]==1 && Naldeia == marcadores[f][0]){
		making = 0;
	...
	//	if(marcadores1[i][1] == "http://" + server + "/build.php?newdid=" + id_aldea1 + "&gid=11" && marcadores1[i][3] == Naldeia){making = 1;}}
		...
		if (ww[i]==null && j<maxlvl) {ww[i]=recursos1; lvl[i]=j;}
					...
					if (temp < temp1 && marcadores) {
						...
						add='00:00:00';
						if (Campos == 1){
							add= Const.firstChild.childNodes[linha].childNodes[2].textContent.split(" ")[0];
...
//prompt(Alternativa);
		...
		var marcadores1 = obtenerValorCookie("AutoProd");
		cookies=marcadores.length + marcadores1.length;
		if (cookies>3){table.style.width = 686 + 'px';}
...
//automercadores
		...
		var tr = document.createElement("TR");
		var td1 = elem("TD", "<b>Enviar quantas vezes?  </b>" + '<input name="multi" value="1" size="1" maxlength="3" class="multi">' + "<b> Tempo? </b>" + '<input name="timex" value="" size="1" maxlength="5" class="timex">');
...
// test end
	...
	for (var i = 0; i < marcadores.length; i++){
			var tr = document.createElement("TR");
			var td = elem("TD", "<span>&#8226;</span>&nbsp; <a href='" + marcadores[i][1] + "'><font color='red'>" + marcadores[i][0] + "</font> " + marcadores[i][4] + "</a>");
			var enlace = elem("A", " <img src='" + img('a/del.gif') + "' width='12' height='12' border='0' title='" + T('ELIMINAR') + "'>");
			enlace.href = "javascript:void(0);";
		...
		if (marcadores.length > 0){
			...
			if (b < 0) {
				if (marcadores[i][1] == location.href) {
					...
					ssss = RegExp.$1;
					if (marcadores[i][6]==" " || !marcadores[i][6] || marcadores[i][6]=="undefined") var did= "dorf2.php?a="; 
					else {if (marcadores[i][6].length >2) did= "build.php?id=" + ssss + "&" + marcadores[i][6];
					else var did= "dorf2.php?a="+marcadores[i][6];}
					a = find("//a[contains(@href, '" + did + "')]", XPFirst);
					if (a) {var j = readCookie("AutoLinks" + "_" + uid + "_" + server);
						if (j != null){j = j.split("$$");j.splice(i, 1);createCookie("AutoLinks" + "_" + uid + "_" + server, j.join("$$"), 365);removeElement(find("//*[@id='" + "AutoLinks" + "']", XPFirst));}
						setTimeout(function() { document.location.href=a; } , 1000);}
...
//		a.innerHTML = 'lol';
	...
	if (find("//input[@type='text']", XPList).snapshotLength == 4) {
...
//automercadores
	if (location.href.match(/dorf3.php($|\?s=(\d+)$)/)){return;} else{
		var marcadores1 = obtenerValorCookie("AutoLinks");
		var f = calMinArray (marcadores1, marcadores1.length, 2);
		if (f != null) {var tempLinks = marcadores1[f][2];} else {var tempLinks = 32*86400;} 

		var marcadores = obtenerValorCookie("AutoProd");
		var i = calMinArray (marcadores, marcadores.length, 3);
		if (i != null) {tempProd = marcadores[i][3];} else {var tempProd = 32*86400;}
		
		var merc = obtenerValorCookie("Multimerc");
		var h = calMinArray (merc, merc.length, 9);
		if (h != null) {var mercmin = merc[h][9];} else {var mercmin = 32*86400;} 
...
//				prompt(tempos)
			...
			agregarElementoCookie("Multimerc", [rec[0], rec[1], rec[2], rec[3], merc[h][4], merc[h][5], count7, merc[h][7], tempos[0], tempos[1], merc[h][10]]);}
	 ...
	 */
		...
		// Parametros reconocidos
                ...
                var parametros = ["desp","pontus_cultura", "Ordem_" + hehe, "marcadores_" + hehe, "notas_" + hehe, "ventas_" + hehe, "capital_" + hehe, "AutoProd_" + hehe, "AutoLinks_" + hehe, "Multimerc_" + hehe];
...
//					td.setAttribute("align", "center");
	...
	//	var lol = find("//table[@class='tbg']", XPFirst);
		...
		var td = elem("TD", T('RESUMEN') + ' dos Armaz&eacute;ns ');
				var enlace = elem("A", "<img src='" + img('a/b5.gif') + "' border='0' title='" + T('ACTUALIZAR') + "' id='Todas'>");
				enlace.href = "javascript:void(0);";
...
//	if (location.href.match(/karte2.php($|\?z=)/)){		desplazarMapa(); return; }
	...
	setTimeout(function() { document.getElementsByTagName('form')[0].submit(); } , 20000);
	...
	if (zzz) {setTimeout(function() { document.location.href=zzz; } , 60000);}
	...
	if (location.href.match(/karte.php($|\?z=)/)){mostrarMarcadores();preCalculate3();desplazarMapa();infoRecursos();}
	...
	if (location.href.match(/nachrichten.php($|\?t=|\?s=|\?id=)/))		blocNotas(); else {	AutoProd();	Autolink();
	}
