// ==UserScript==
// @name new boss fighter
// @namespace Superchicken
// @description boss fighter is a script for Zynga's Mafia Wars game.
...
javascript:(function (){
	...
	function click_attack_button() {
		var button = $('#attackBtn > a:first','#bossv2_wrapper_terr') || null;
		var health_need = parseInt(jQuery.parseJSON($(button).attr('requirements')).health+10) || 30;
		if (User.health < health_need) {
			...
			var target='';
			var button = $('#attackBtn > a:first','#bossv2_wrapper_terr') || null;
			var stamina_need = parseInt(jQuery.parseJSON($(button).attr('requirements')).stamina) || 225;
		...
		$.ajax({
			...
			success: function (response) {
				...
				if (object.heal_success == 1) {
					...
					setTimeout(function() { $('#'+spocklet+'_heal').removeClass('red green').addClass('black'); }, 1500);
					log('Heal success');
					heal_time = timestamp();
				}
				else {
