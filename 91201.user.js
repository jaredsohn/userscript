...
// @name           Traditional RT (Mod)

// @namespace      

// @description    Old School RT Functionality for New Twitter, Allows retweeting with Comments (addition of ":)
...
(function() {
// click
var rt_click = function(screen_name,content) {
	return function (e) {
		if (unsafeWindow.twttr.widget.TweetDialog) {
                        unsafeWindow.showTweetBox(screen_name,content);
                        var RTBoxes = document.getElementsByClassName('twitter-anywhere-tweet-box-editor');
                        var RTBox = RTBoxes[RTBoxes.length - 1];
                        RTBox.focus();
                        RTBox.setSelectionRange(0,0);
		} else {
...
var make_all  = function() {
	...
	for (var i = 0; i < si_length ; i++) {
        ...
        myElmnt.innerHTML='function showTweetBox(s,c) { var RTBox = new twttr.widget.TweetDialog({modal: true,draggable: false,template: {title: _("ReTweet @"+s+": (The Real Retweet)")},defaultContent:"RT @"+s+": "+c,origin: "Traditional-RT"}); RTBox.open();}';
...
document.addEventListener('DOMNodeInserted', function(event) {
	...
	if (/stream-item/.test(elmt.getAttribute('class'))) {
        ...
        if (/tweet permalink-tweet/.test(elmt.getAttribute('class'))) {
		make_retweet_action(elmt);
	}
}, false);
