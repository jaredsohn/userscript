...
// @modified date   21 March 2010
// @modified by	    Wayne Child/Austrian Alex/John Vanderbeck
//
...
{
    pName="Unknown Name";
    if (env=="GM") pName=unsafeWindow.global_name;
    if (env=="PRO") pName=window.global_name;
    if (env=="CHROME")
    {
	// in chrome we can't simply grab the variable that Flickr sets
	// because it isn't visible until the page is loaded.  I THINK this has
	// to do with the whole Chrome Sandbox thing, but not sure.  Instead we
	// parse the webpage and grab the "Logged in as" information at the top right
	links = document.getElementsByTagName("a");
        for (i = 0; i < links.length; i++)
	{
		if (links[i].href == "http://www.flickr.com/account")
			pName = links[i].text;
	}
    }
    MY_log(pName);
...
if (env=="CHROME") return localStorage.setItem(a,b);
}
...
if (env=="CHROME") return localStorage.getItem(a);
}
...
if (env=="CHROME") element.addEventListener(a,b,c);
return element;
}
