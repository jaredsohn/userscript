...
// 28/01/2010
// ==UserScript==
// @name Vendetta Highscore
// @author UnDeaD[GER] & mitm[ITA]
// @descrizione: HS-Tool
		...
		if(celleth[9].innerHTML.search("value=\"s\" selected") != -1) {
			...
			if(conta) {
				...
				var valore = GM_getValue(serverID + 'delta')?GM_getValue(serverID + 'delta'):'0';
		                newDiv.innerHTML += '<hr><p align=left><font color=white style=Courier size=2>&nbsp;By <a href=mailto:tonyxxl@web.de><font color=green>UnDeaD</font><font color=white> & </font><font color=green>mitm</font></a></p><p align=left>&nbsp;Datum der ausgewählten HS : ' + riferimento + '&nbsp;&nbsp;<button type=button STYLE=\'font-size:8pt;font-family:Courier;\' id=DISATTIVA>Deaktivieren</button>&nbsp;&nbsp;<br>&nbsp;Punkte für Halb-Inaktiv&nbsp;&nbsp;<input type=text name=delta_box value=' + valore + ' maxlength=20 size=6>&nbsp;&nbsp;<button type=button STYLE=\'font-size:8pt;font-family:Courier;\' id=DELTA>Save</button></p><hr>'; 
		                for(i = 1; i<=conta; i++){
	        	                classifica = GM_getValue(serverID + 'conta' + i);
		                        newDiv.innerHTML += '<p align=left><font color=white size=2 style=Courier>&nbsp;Highscore vom ' + classifica + '</font>&nbsp;&nbsp;<button type=button STYLE=\'font-size:8pt;font-family:Courier;\' id=ATTIVA' + i + '>Aktivieren</button>&nbsp;&nbsp;<button type=button STYLE=\'font-size:8pt;font-family:Courier;\' id=CANCELLA' + i + '>Cancel</button></p>';
				...
				for(i = 1; i<=conta; i++){
					...
					if( confirm( 'Highscore vom ' + hs + ' wirklich löschen?' ) ) {
			...
			if (riferimento) { 
				...
				var pagina = document.getElementsByTagName("a");
						...
						if (pttot_s == pttot) {
							html += '<b><font color=orange STYLE=\'font-size:8pt;font-family:Verdana;cursor:pointer;\'>&nbsp;Inaktiv</font></b></span>';
							...
							if ((parseInt(pttot) - parseInt(pttot_s)) <= document.getElementsByName('delta_box').item(0).value) {
								html += '<b><font color=blue STYLE=\'font-size:8pt;font-family:Verdana;cursor:pointer;\'>&nbsp;Halb-Inaktiv</font></b></span>';
							} else {
								html += '<font color=#31e024 STYLE=\'font-size:8pt;font-family:Verdana;cursor:pointer;\'>&nbsp;Zuwachs</font></span>';
							}
						} else if (parseInt(pttot_s) > parseInt(pttot)) {
        	                                        html += '<font color=red STYLE=\'font-size:8pt;font-family:Verdana;cursor:pointer;\'>&nbsp;Verluste</font></span>';
						}
			...
			if (saved == data && !(riferimento)) {
				...
				var stato = 'Update der HS ' + saved + ' um ' + orario ;
				conta = GM_getValue(serverID + 'conta');
				salva = 1;
			...
			if(salva) {
				...
				var parentDiv = page.parentNode;
                                newDiv = document.createElement("div");
				newDiv.innerHTML = '<p align=left><font color=yellow size=2 style=Courier>&nbsp;<blink>Wer das liest ist Doof :)</blink><br>&nbsp;' + stato + '</font></p>';
...
function pl_det(pl_id, pl_name, conta, serverID){
	...
	html = '<html><head><title>Details Player ' + pl_name + '</title></head><body bgcolor=black><font color=#dfdbc9><H1>Details der Punkteentwicklung ' + pl_name + '</H1></font><table border=1 bordercolor=black cellpadding=3 cellspacing=0><tr><td height="7" bgcolor=#dfdbc9>Highscore</td><td height="7" bgcolor=#dfdbc9>Position</td><td height="7" bgcolor=#dfdbc9>Name</td><td height="7" bgcolor=#dfdbc9>Ausbildung</td><td height="7" bgcolor=#dfdbc9>Gebäude</td><td height="7" bgcolor=#dfdbc9>Einheiten</td><td height="7" bgcolor=#dfdbc9>Gesamt</td><td height="7" bgcolor=#dfdbc9>Gebäudeanzahl</td></tr>';
	...
	html += '</table><p><a href="javascript:void(null);" onClick="window.close();"><font color=gray>Schließen</font></a></p></body></html>';
	...
	if(id != conta){
		...
		for(var i = 0; i < valori.length; i++){
			if(valori[i].search(pattern) != -1) {
				contatore++;
				if(contatore % 200 == 0) alert('Fertig^^');
		...
		alert('Löschen der aktuellen Highscore ist nicht möglich...');
	}
	//alert('Finito!');
