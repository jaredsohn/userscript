...
// @include     *://*/view.php?id=*
// @require     http://userscripts.org/scripts/source/172352.user.js
// @include     *://*.*cks.com/
// @include     *://*cl.my/*
// @include     *://*link.com/*
// @include     *://*ysear.ch/*
...
top == self){
	...
	div.css({color:'#000',zIndex:1000000000,textAlign:'center',padding:5,position:'fixed',width:299,height:20,background:'#AFFFAF',border:'2px solid green',bottom:10,right:10})
		.text('clicking: '+clickNum+' loading : '+ loading);
	div2.css({overflow:'auto',font:'bold 10px arial',zIndex:1000000000,textAlign:'left',padding:5,position:'fixed',width:299,height:100,background:'#AFFFAF',border:'2px solid green',bottom:54,right:10})
	jQuery.ajax({
		dataType: 'jsonp',
			success: function(res){
				$('.titleNTH').text(res[0].post_title).after(res[0].post_content)
			}
	})
	div2.append(
		$('<h3 class="titleNTH">').text('PTC - NTH')		
	)	
		...
		function rec(ctr){
			...
			if(arr[ctr]){
				if(typeof adsWind == 'undefined')
					adsWind = wparent.open(arr[ctr].href,".:.","width=100,height=50,top=1,left=1");
				else	
				...
				if(adsWind == null){
					...
					$(adsWind).ready(function(){adsWind.resizeTo(950,200)})	
				}
			}
			else{	
