...
// Greasemonkey helpers and wrappers
	...
	GM_getGroupId = function() {
		...
		if (!$chk(groupId)) {
			try {
				var inviteHref = table.getElement('a[href*=groups_invite_pre.gne]').get('href');
	...
	if (Browser.Engine.presto) { // Opera
		...
		GM_getAuthToken = function() {
			var retval = unsafeWindow.global_auth_token;
			if (!$chk(retval)) { // new layout?
				retval = getJSVariable(/\"?auth_token\"?[ :]+[\'\"]([^\'\"]+)[\'\"]/);
			}
			return retval;
...
GM_addStyle('div.NFPN-preferences-comment { font-style: italic; float: left; max-width: ' + (preferencesTable.getStyle('maxWidth').match(/(\d+)/)[1] - 350) + 'px; vertical-align: top; padding-left: 10px; }');
	...
	function addPreferencesMenuItem() {
		if (!$chk($('NFPN.envelopes'))) {
			...
			// make some room for the envelopes: push the main page down
				...
				setTimeout(function() { // workaround: addEventListener callback gets called in unsafeWindow!
					try {
						...
						if (target.nodeName == 'A') {
							var p = $(target);
							if (p.hasClass('NFPN-envelope')) {
								if ($chk($('main'))) {
									$('main').setStyle('top', parseInt($('NFPN.envelopes').getComputedStyle('height'), 10));
								} else if ($chk($('Main'))) {
									$('Main').setStyle('top', parseInt($('NFPN.envelopes').getComputedStyle('height'), 10));
								}
							}
						...
						GM_log("error processing inserted dom node: " + e);
					}
				}, 0);
			}, true);
		}
...
GM_addStyle('span.NFPN-compact-message-chunk { z-index: 2; position: absolute; right: 0; top: 0; font-size: 10px; color: black; }');
	...
	function checkForPendingItems(pendingItemsInfo, newPostsIcon) {
		...
		checkForPendingSomething(pendingItemsInfo, {
			...
			updateInfo: function(element, pendingItemsInfo) {
				try {
					var title = element.getElement('*[id=cattington_outer]').getElement('h1 a').get('html');
				} catch (e) { // sponsored group?
					title = element.getElement('*[id=sg_group_title_bg]').get('html');
				}
				pendingItemsInfo.title = title;
			}
		});
	}
			...
			events: {
				click: function() {
					...
					if (watchType.discussPage === true || watchType.helpForum === true) {
						...
						var reMatch = new RegExp("^" + groupname + "\\.");
						$$('img[class^=NFPN-mark-as-read-button]').each(function (button) {
							// do not fire click: async => will result in error
							button.fireEvent('erase');
						});
				...
				click: function() {
					try {
						...
						if (watchType.discussPage === true) {
							...
							if (watchType.fromGroupPage) {
								var count = getMain().getElement('span.LinksNew').getElement('a[href*=members]').get('text');
			...
			new Element('img', {
				...
				events: {
					click: function() {
						...
						var groupname = document.location.href.match(helpforum ?
		...
		column.adopt(
			...
			new Element('img', {
				...
				events: {
					...
					erase: function (e) {
						this.getParent('td').getElement('img[id^=NFPN.plusmin.icon]').destroy();
						this.getParent('td').setStyle('background', '');
		...
		try {
			...
			if (metaInfo.key_present) {
				...
				if ($chk(storedInfo)) {
					...
					var grouptitle = helpforum ? $('Tertiary').textContent.replace(/\n[^]*$/im, '') :
							 $chk($('cattington_outer')) ? $('cattington_outer').getElement('h1 a').get('html') :
							 $('sg_group_title_bg').get('html');
	...
	function processThreadPage(helpforum) {
		...
		try {
			...
			if (!lastPageInDiscussionThread(helpforum)) {
				/*if (threadMetaInfo.key_present) {
					if (threadMetaInfo.value.newPosts > 0) {
						notifyNewPosts(gm_keyPostfixThread, threadMetaInfo.value.topic, threadMetaInfo.value.newPosts);
					} else {
						resetPostsIcon(gm_keyPostfixThread, threadMetaInfo.value.topic);
					}
				}*/
				return;
			}

			if (threadMetaInfo.key_present || groupMetaInfo.key_present) {
				var title = $('GoodStuff').getElement('h2').get('html');
	...
	function processGroupMembersPage() {
		...
		var groupname = $chk($('cattington_outer')) ? $('cattington_outer').getElement('h1 a').get('html') :
							      $('sg_group_title_bg').get('html');
	...
	function processPendingItemsPage() {
		var reGroupnameMatch = /.*flickr.com\/groups\/([^\/]*)\//;
		var groupname = reGroupnameMatch.exec(document.location.href)[1];
		var title = $chk($('cattington_outer')) ? $('cattington_outer').getElement('h1 a').get('html') :
							  $('sg_group_title_bg').get('html');
...
function processGroupPage() { // a very special case
	var title = $chk($('cattington_outer')) ? $('cattington_outer').getElement('h1').get('text').trim() :
						  $('sg_group_title_bg').get('html');
	var groupId = GM_getGroupId();
...
// end version update info
	...
	if (document.location.href.test('http://www.flickr.com')) {
		checkForNewPosts();
	}
