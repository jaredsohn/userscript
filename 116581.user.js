...
// @namespace      noobs
// @version        20111030x
// @description    It is not your business ;)
...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  mainPop = new CPopup ('pt', Options.ptWinPos.x, Options.ptWinPos.y, 750,750, Options.ptWinDrag,
      function (){
        tabManager.hideTab();
  ...
  AddMainTabLink('MENU', eventHideShow, mouseMainTab);
    ...
    var e=unsafeWindow.Object.values(Seed.outgoing_marches["c"+city]).detect(function(a){return a.marchId&&parseInt(a.marchStatus,10)===unsafeWindow.Constant.MarchStatus.INACTIVE})    
	  ...
	  case 'unt19': params.u19 = wagons_needed;break;
         ...
         new AjaxRequest(unsafeWindow.g_ajaxpath + "march.php" + unsafeWindow.g_ajaxsuffix, {
                  ...
                  if (rslt.ok) {
                  logit('Trade   From: ' + cityname + "   To: " + xcoord + ',' + ycoord + "    ->   Wagons: " + wagons_needed);
                  ...
                  logit('TRANSPORT FAIL: ' + cityname + ' -> ' + rslt.msg);
                  }
                  },
...
/****************************  Tower Tab  ******************************/
	var uu1 = 'Supply trucks';
	var uu4 = 'AT';
	var uu5 = 'Infantry';
	var uu6 = 'Snipers';
	var uu7 = 'SAAMS';
	var uu8 = 'Tanks';
	var uu9 = 'Supp Choppers';
	var uu10 = 'Fighters';
	var uu11 = 'GunShips';
	var uu12 = 'Bombers';
	var uu13 = 'Stealths';
...
Tabs.tower = {
  ...
  init: function(div){
    ...
    var m = '<DIV class=pbStat>TOWER ALERTS</div><TABLE class=pbTab><TR align=center>';
      ...
      m += '<TD width=95><SPAN id=pbtacity_'+ i +'>' + Cities.cities[i].name + '<BR>'+ Cities.cities[i].x +','+ Cities.cities[i].y +"<BR>"+'</span></td>';
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
  ...
  postToChat : function (m){
    ...
    addCityData2Pop: function(cityId){
        var t = Tabs.tower;
        var rownum = 0;
        var names = ['Sup', 'AT', 'INF', 'Snip', 'SAM', 'Tank', 'Chop', 'Fight', 'GunSh', 'Bomb', 'Stea', 'Nuke', 'Hell', 'Drone', 'SFor', 'Carg'];
        enc = {};
        numSlots = 0;
		...
		s2 += '<TR><TD colspan=16><BR></td></tr><TR align=right><TD class="attack" align=left><B>Attackers:</b></td>';
        for (a = 1; a < 17; a++)
...
//DebugTimer.start(); 
    try {
      ...
      for(i=0; i<Cities.numCities; i++) {
        ...
        if (officers) {
          RESOfficer[i] = parseInt(Seed.leaders["city" + Cities.cities[i].id].resourcefulnessKnightId);
          TRNOfficer[i] = parseInt(Seed.leaders["city" + Cities.cities[i].id].combatKnightId);
          BLDOfficer[i] = parseInt(Seed.leaders["city" + Cities.cities[i].id].politicsKnightId);
          RDOfficer[i] = parseInt(Seed.leaders["city" + Cities.cities[i].id].intelligenceKnightId);
          if (officers["knt" + RESOfficer[i]])
            var pRESOfficer = parseNaN(officers["knt" + RESOfficer[i]].knightLevel);
          else
            var pRESOfficer = 0; 
          if (officers["knt" + TRNOfficer[i]])
            var pTRNOfficer = parseNaN(officers["knt" + TRNOfficer[i]].knightLevel);
          else
            var pTRNOfficer = 0; 
          if (officers["knt" + RDOfficer[i]])
            var pRDOfficer = parseNaN(officers["knt" + RDOfficer[i]].knightLevel);
          else
            var pRDOfficer = 0; 
          if (officers["knt" + BLDOfficer[i]])
            var pBLDOfficer = parseNaN(officers["knt" + BLDOfficer[i]].knightLevel);
          else
            var pBLDOfficer = 0; 
          var Wage = (pRESOfficer+pTRNOfficer+pRDOfficer+pBLDOfficer)*20;
          if (Revenue[i]< Wage)
            timeLeft = timestrShort(Gold[i] / (Wage - Revenue[i]) * 3600  );
        }
        str += "<TD width=81>" + addCommas(Gold[i]) + "<BR>"+ Tax[i] + "%<BR>"+ addCommas(Revenue[i]) + "<BR>"+ addCommas(Wage) + "<BR>"+ timeLeft + "<BR><BR>"+ pRESOfficer + "%("+RESOfficer[i]+")<BR>" + pTRNOfficer/2 +'%<BR>'+ pRDOfficer/2 +'%<BR>'+ pBLDOfficer/2 +"%</td>";
      }
      str += "</TABLE>";
        str += '<TR><TD colspan=12><BR></td></tr>';  
//    Generals 
// itte
    var citystr=[];      
		for(i=0; i<Cities.numCities; i++){
    		  cityID = 'city'+Cities.cities[i].id;
          citystr[i] = Cities.cities[i].name.substring(0,11) +'</b>('+ Cities.cities[i].x +','+ Cities.cities[i].y +')'; 
    }
      str += '<TABLE><TR>';
    str += '<TD class=xtab colspan=4 width=120><BR><b>Active City:</b> '+ htmlSelector ({0:citystr[0], 1:citystr[1], 2:citystr[2], 3:citystr[3]}, Options.aCity, 'id=ptaCity') +'</TD>';
    str += '</td><TD class=xtab colspan=4 width=81><BR><INPUT type=CHECKBOX id=ptEnergy'+ (Options.Show0Energy?' CHECKED':'') +'>Show 0 Energy</TD></TR>';
      str += "</table></div>";
       str += "<TABLE class=ptTabOverview cellpadding=0 cellspacing=0 text-align:left></td><TR><TD width=160><b>Name:</b><TD width=81, text-align:right><b>Experience:</b></TD><TD width=21><b>Energy:</b></TD><TD width=90><b>Status:</b></TD><TD width=21><b>General:</b></TD><TD width=21><b>Refill:</b></TD></TR>";
    		  var x=0;
          if (Options.aCity)
    		    x=Options.aCity;
          cityID = 'city'+Cities.cities[x].id;
	        for( key in Seed.knights[cityID]) {
              var kName = Seed.knights[cityID][key].knightName;
              var kID = parseInt(Seed.knights[cityID][key].knightId);	        
              var kFBUID = Seed.knights[cityID][key].knightFbuid;	        
              var kLvl = parseInt(Seed.knights[cityID][key].knightLevel);
              var kExp = parseInt(Seed.knights[cityID][key].experience);
              var kEnergy = parseInt(Seed.knights[cityID][key].knightEnergy);
              var kStatus = parseInt(Seed.knights[cityID][key].knightStatus);
              var kLord = parseInt(Seed.knights[cityID][key].knightLordUserid);
//              var domain =
//              var server = new String(domain); // 
              var serverID = unsafeWindow.domainName.replace(/[a-z]/ig,"");
              if(Seed.appFriends)
                if (Seed.appFriends[kFBUID]) 
                  var kGName = Seed.appFriends[kFBUID].displayName;
                else
                  var kGName = "[not friend]";
              else
                var kGName = "[loading]";
              var strkEnergy="____";
              if(kEnergy>0)
                for(i=0;i<kEnergy; i++) {
                  strkEnergy = "X" + strkEnergy;
                }                    
              if (kStatus==1 && kEnergy>0) 
                var strkStatus = "Ready";
              if (kEnergy==0) 
                var strkStatus = "Sleeping";
              if (kStatus==10) 
                var strkStatus = "Marching";
              if (kID==RESOfficer[x]) 
                var strkStatus = "Quartermaster";
              if (kID==TRNOfficer[x]) 
                var strkStatus = "Army Chief of Staff";
              if (kID==RDOfficer[x]) 
                var strkStatus = "Chief Scientist";
              if (kID==BLDOfficer[x]) 
                var strkStatus = "Chief Engineer";
                            
//                p+pTRNOfficer+pRDOfficer+pBLDOfficer
              if (Options.Show0Energy || kEnergy>0)
//                str += "<TR><TD>" +kName + "</TD><TD>" + kExp + "(" + kLvl +")</TD><TD>"+strkEnergy.substring(0,4)+"</TD><TD>"+strkStatus + "</TD><TD>" + kGName + "</TD></TR>";
//                str += '<TR><TD align=center><A style="color:#1D0AF2" target="_tab" href="http://www.facebook.com/profile.php?id='+ kFBUID +'">' +kName + "</a></TD><TD>" + kExp + "(" + kLvl +")</TD><TD>"+strkEnergy.substring(0,4)+"</TD><TD>"+strkStatus + "</TD><TD>" + kGName + '</TD></TR>';
                str += '<TR><TD align=center><A style="color:#1D0AF2" target="_tab" href="http://www.facebook.com/profile.php?id='+ kFBUID +'">' +kName + "</a></TD><TD>" + kExp + "(" + kLvl +")</TD><TD>"+strkEnergy.substring(0,4)+"</TD><TD>"+strkStatus + "</TD><TD>" + kGName + '</TD><TD align=center><A style="color:#1D0AF2" target="_tab" href="http://apps.facebook.com/globalwarfaregame/?page=claimStuff&in='+kLord+'&lp=14&kid='+kID+'&cid='+cityID+'&ins=' + serverID +'">link</a></TD></TR>';
//http://apps.facebook.com/globalwarfaregame/?page=claimStuff&in='+kLord+'&lp=14&kid='+kID+'&cid='+cityID+'&ins='+unsafeWindow.domain
...
//	  http://apps.facebook.com/globalwarfaregame/?page=claimStuff&in=3036528&lp=14&kid=27485&cid=67155&ins=30
      ...
      document.getElementById('ptaCity').addEventListener('change', e_aCity, false);
      document.getElementById('ptEnergy').addEventListener('click', e_clickShow0Energy, false);
...
//DebugTimer.display ('Draw Generals');    
    ...
    function e_aCity(evt){
      Options.aCity = evt.target.value;
      t.show ();
    }
    function e_clickShow0Energy(){
      Options.Show0Energy = document.getElementById('ptEnergy').checked;
      t.show ();
    }
...
Tabs.Queue = {
  ...
  show : function (){
    ...
    function _row (name, row,rowTotal){
      if (rownum++ % 2)
        style = '';
      else
        style = ' style = "background: #e8e8e8"';
...
//      rows[10] = [];
      ...
      str = '<DIV class=ptstat style="margin-top:5px; margin-bottom:5px;">';             
      str += "<DIV id=overMainDiv style= 'font-size:"+ Options.overviewFontSize +"px'><TABLE class=ptTabOverview cellpadding=0 cellspacing=0></td>";
      str += "<TR style='background: #87CEEB'><TD width=81><B>City names:"
      for(i=0; i<Cities.numCities; i++) {
      ...
      for(i=0; i<Cities.numCities; i++) {
        rows[0][i] = timestr(0);  
	      for( key in Seed.training_queue['c'+ Cities.cities[i].id]) {            
              if (parseInt(Seed.training_queue['c'+ Cities.cities[i].id][key].status)>0)
                ...
                if (parseInt(Seed.training_queue['c'+ Cities.cities[i].id][key].status)==1) {
                  var ttime = parseInt(Seed.training_queue['c'+ Cities.cities[i].id][key].eta);
                  if(ttime>now) 
                    rows[0][i] = timestr(ttime - now);
      ...
      str += '<TR><TD colspan=12><BR></td></tr>';
      str += _row ('Recent Finishes:', rows[0]);
      str += _row ('Total Training Time:', row, true);      
      str += '<TR><TD colspan=12><BR></td></tr>';
// -------------------------------------------------Fortification queue
      for (r=0; r<21; r++){
		    for(i=0; i<Cities.numCities; i++) {
      ...
      for(i=0; i<Cities.numCities; i++) {
        cityID = 'city'+ Cities.cities[i].id;
        var perimeter = parseInt(Seed.buildings[cityID].pos1[1]);
        var ttime2=0;
        rows[0][i] = timestr(0);
// Wall 7-12
        for (r=7; r<11; r++){         
            ...
            if (r==8) {
              ...
              if (temp < disposable[perimeter])
                ...
                rows[11][i] = '<SPAN class=boldWhite><B>'+ temp +'</b></span>';
            }
            if (r==10) {
              var temp=parseInt(Seed.fortifications[cityID]['fort'+x]) + rows[3][i];
               if (temp < stable[perimeter])
                  ...
                  rows[12][i] = '<SPAN class=boldWhite><B>'+ temp + '</b></span>';    
            }
          }
          rows[19][i]=disposable[perimeter];
          rows[20][i]=stable[perimeter];
// Build Queue 1-6
          var ttime = 0;
	        for( key in Seed.fortify_queue['c'+ Cities.cities[i].id]) {
              var x = parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].type)- 51;
              if (parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].status)>0 && x>0){
                if(ttime2==0)
                  ttime2=parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].eta);
                rows[x][i] = rows[x][i] + parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].quant);
                if (parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].status)==1) {
                  if (Seed.fortify_queue['c'+ Cities.cities[i].id][key])
                  ttime = parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].eta);
                }
              }  
          }
          if (ttime=0)
          ...
          temp=rows[1][i]+rows[2][i];
          if (temp < stable[perimeter])
            rows[5][i] = temp;
          else
            rows[5][i] = '<SPAN class=boldWhite><B>'+ temp + '</b></span>';    
          temp=rows[3][i]+rows[4][i];
          if (temp < stable[perimeter])
            rows[6][i] = temp;
          else
            rows[6][i] = '<SPAN class=boldWhite><B>'+ temp + '</b></span>';    
// Totals 13-18
          for (r=13; r<17; r++){         
            x=r+51-10;
            y=r-12;
            z=r-6;
            rows[r][i]=rows[y][i]+rows[z][i];
          }
          rows[17][i]=rows[1][i]+rows[2][i]+rows[7][i]+rows[8][i];
          rows[18][i]=rows[3][i]+rows[4][i]+rows[9][i]+rows[10][i];
      }
// Mines
	    str += _row ('Build Mines:', rows[2]);
	    str += _row ('Wall Mines:', rows[8]);
	    str += _row ('Total Mines:', rows[14],true);
// Stingers
      str += _row ('Build Stingers:', rows[1]);
	    str += _row ('Wall Stingers:', rows[7]);
	    str += _row ('Total Stingers:', rows[13],true);
// Disposables
	    str += _row ('Build Disposable:', rows[5]);
	    str += _row ('Wall Disposable:', rows[11]);
	    str += _row ('Total Disposable:' , rows[17],true);
	    str += _row ('Max Disposable'+'('+ perimeter +')' +':', rows[19],true);
// Artillery
      str += _row ('Build Artillery:', rows[3]);
	    str += _row ('Wall Artillery:', rows[9]);
	    str += _row ('Total Artillery:', rows[15],true);
// Airguns
      str += _row ('Build Airguns:', rows[4]);
	    str += _row ('Wall Airguns:', rows[10]);
	    str += _row ('Total Airguns:', rows[16],true);
// Stable
      str += _row ('Build Stable:', rows[6]);
	    str += _row ('Wall Stable:', rows[12]);
	    str += _row ('Total Stable:' , rows[18],true);
	    str += _row ('Max Stable'+'('+ perimeter +')' +':', rows[20],true);
      ...
      for(i=0; i<Cities.numCities; i++) {
        ...
        var totTime = 0;
          ...
          ttime = parseInt(Seed.fortify_queue['c'+ Cities.cities[i].id][key].eta)
		      ...
		      if(parseInt(ttime2)>0) { finTime=parseInt(finTime + Math.abs(ttime2)) }
        }
        row[i] = timestr(finTime);
      }    
      str += '<TR><TD colspan=12><BR></td></tr>';
      str += _row ('Recent Finishes:', rows[0]);
      str += _row ('Total Wall Queue:', row, true);
      str += '<TR><TD colspan=12><BR></td></tr>';
/********************************* Incoming attacks *************************************/
// itte
      for (r=1; r<maxunit; r++){
	    ...
	    if (Seed.queue_atkinc){	    
	      for( marches in Seed.queue_atkinc) {
	       if (marches){
	         if (Seed.queue_atkinc[marches]['unts']!=null){
            for (r=1; r<maxunit; r++){
              for(i=0; i<Cities.numCities; i++) {
                if(Cities.cities[i].id==parseInt(Seed.queue_atkinc[marches].toCityId))
                  x=i;
              }
              a=parseInt(Seed.queue_atkinc[marches]['unts']['u'+r]);
              if (a>0)
                rows[r][x] = rows[r][x] + a;
            }
            acnt++;
      ...
      str += '<TR><TD colspan=12><b>Incoming attacks:</b> ('+acnt+')</td></TR>';
	    str += _row (unsafeWindow.arStrings.unitName['u1'], rows[1]);
      str += _row (unsafeWindow.arStrings.unitName['u5'], rows[5]);
      str += _row (unsafeWindow.arStrings.unitName['u6'], rows[6]);
      str += _row (unsafeWindow.arStrings.unitName['u4'], rows[4]);
