// ==UserScript==
// @name           MythosTheftManager
// @namespace      http://userscripts.org/users/64075
// @description    Keeps track of the last five people you successfully stole from, you failed to steal from, and displays them on the spy missions page.
// @include        http://www.mythicwars.com/mythos/g_spyMissions*
// ==/UserScript==
...
function addTheftData(newTable, theftList, type)
{
	...
	for each(var theft in theftList) {
		...
		anchorEl.setAttribute('href', 'http://www.mythicwars.com/mythos/g_spyMissions.asp');
...
function createSubHeader(newTable, headerText, type)
{
	...
	colMainSubHeader.setAttribute('class', 'mainsubheader');
...
if(document.location.toString().indexOf("http://www.mythicwars.com/mythos/g_spyMissionst.asp") != -1)
{
	...
	// If the list is past the number of thefts we want displayed (NUMBER_OF_THEFTS
	// assigned at the top) then remove the first element from the list.
	...
	colMainHeader.setAttribute('class', 'mainheader');
	colMainHeader.setAttribute('height', '55');
