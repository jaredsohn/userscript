...
// @namespace      http://userscripts.org/scripts/show/114065
// @date           02.01.2012
// @version        2.4.2.04
...
texte["inBehandlung"] = "in einem Raum";
...
var scriptUrl = "http://userscripts.org/scripts/show/114065";
...
function do_main(){
	...
	GM_addStyle(".allcured{border:0px solid green!important;}");
	GM_addStyle(".needminitreatment{border:0px solid yellow;border-left 0px solid yellow!important;}");
	GM_addStyle(".unhealable{border:0px solid red;border-top:0px solid red!important;border-right:0px solid red!important;}");
	...
	if (GM_getValue("valUpdate",true)) {
		...
		if(valLastUpdate=="") {
			GM_xmlhttpRequest({
				method: "GET",
				url: "http://userscripts.org/scripts/show/114065.meta.js",
	...
	newdiv1 = createElement("div",{class:"miniicon cursorclickable",style:"border:0;"},newdiv,"P");
	...
	*/
	...
	function buildInfoPanel(mode){
		...
		if (mode=="patients"){
			function buildPatientsTable(mode,showCured,minipic){
				...
				newtablebody=createElement("tbody",{style:"overflow-y:auto;overflow-x:hidden;height:560px;"},newtable);
					...
					for(var r in sumDiseases){
						...
						for(var v=0;v<4;v++){
							...
							if (v == 3)	{
								...
								newdiv = createElement("table",{style:"margin-right:2px;font-weight:bold;padding-top:20px;"},newtd,time2str(totalTime,1));
							}
						}
					}
				}
			}
			buildPatientsTable(2,true,false);
		}
		}
	}
	...
	function plotPatient(target,currPatientId,showCured,minipic){ //(target) , (target,showCured)
		...
		for(var disease in patientDiseases[currPatientId]) if(!isNaN(disease)){
			if(showCured || patientDiseases[currPatientId][disease]!="cured"){
				...
				if(minipic){
					...
					if(patientDiseases[currPatientId][disease]!="cured"){
											//var help = Global.refRooms.get("r"+roomId)["currentjobends"]; //rather hard to get the remaining time in here for the currently treated  disease
						newdiv2 = createElement("div","",newdiv,time2str(Global.availableDiseases[disease]["basetime"],1));
											
											//var help = Global.refRooms.get("r"+roomId);
											//time2str(roomTimes[Global.selectedFloor][currRoom["topleft"]][1]-now)
		...
		if(patientDiseases[currPatientId]["m"]==4){
			if(minipic){
				...
				newdiv = createElement("div",{style:"position:relative;float:left;width:30px;height:45px;margin-left:30px;"},target);
		...
		for(var v=0;v<canddiv.length;v++){
			...
			if(patientDiseases[currPatientId]){
				if(patientDiseases[currPatientId]["room"+roomid]){
					canddiv[v].style.border="0px solid ";
				}
		...
		for(var v=0;v<canddiv.length;v++){
			...
			if(currPatientId){
				if(!$("p"+currPatientId)){
					if(patientDiseases[currPatientId]){
						if(patientDiseases[currPatientId]["room"+roomid]){
							canddiv[v].style.backgroundColor="0";
						}
					} else { initPatient(currPatientId); }
				}
			}
	...
	function do_Patientenblatt(){
		...
		createElement("div",{"style":"position:absolute;top:370px;left:30px;right:100px;color:red;"},
		$("ref_divdetailsbig"),"10%: "+hT_formatgr(0.10*maxprice)+", 20%: "+hT_formatgr(0.20*maxprice)+", 30%: "+hT_formatgr(0.30*maxprice)+", 40%: "+hT_formatgr(0.40*maxprice)+", 50%: "+hT_formatgr(0.50*maxprice)+", <br>60%: "+hT_formatgr(0.60*maxprice)+", 70%: "+hT_formatgr(0.70*maxprice)+", 80%: "+hT_formatgr(0.8*maxprice)+", 90%: "+hT_formatgr(0.90*maxprice));
		
		var newimg = createElement("img",$("ref_divdetailsbig"));
...
Log("do_Patientenboerse");
	...
	candtd[5].innerHTML = "Kup";
	createElement("td",{style:"text-align:right;"},candtr[0],"Zysk");
	createElement("td",{style:"text-align:right;"},candtr[0],"Czas");
	...
	for(var tr=1;tr<candtr.length;tr++){
		...
		createElement("td",{style:"text-align:right;"},candtr[tr],"&nbsp;"+(price>priceMax?"-":"")+number_format(priceMax-price,2));
