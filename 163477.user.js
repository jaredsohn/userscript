// ==UserScript==
// @name           Facebook - Fishwrangler Smart Autofish Plus
// @namespace      iispyderii
// @description    A smart autofisher for the Fishwrangler facebook app
// @version       2.5.0.0 Client-side timer functionality. Dynamic title bar.
// @version       2.4.2.0 FIXED all saftey alerts.
// @version       2.4.1.0 Added Devil's Cove. (Not 100% on Deadwater pass) Added some GUI updates. Thanks, exentric!
...
nightFishing = (document.body.innerHTML.indexOf('You are currently on a Night Fishing Trip') != -1); //check if a Night Fishing is detected
...
/*
  var d = document.body.innerHTML.indexOf('a8138090269_j');
  var d2 = document.body.innerHTML.indexOf('(',d);
  var d3 = document.body.innerHTML.indexOf(')',d2);
  ddit = document.body.innerHTML.substring(d2+2,d3-1)
  if(document.getElementById('app8138090269_'+ddit) == null) {duig=-1}
  else if (document.getElementById('app8138090269_'+ddit) != null)
  {duig = document.getElementById('app8138090269_'+ddit).value}
  timeoutvalue = (parseInt(duig)+Math.round(Math.random()*30)+3)*1000;
*/
...
function gatherFishingInfo()
{
    ...
    if (!needChum)
  {
    ...
    break;
    case 14:activeChum = 'Gold Goop';
    break;
    case 15:activeChum = 'Diamond Dip';
    break;
...
{ 
  ...
  /* 1. RIGHT ISLAND, WRONG CHUM
    ...
    if (island == "Magma Reef" && !(activeChum == "Fire Sludge" 
    || activeChum == "Steak" 
    || activeChum == "Red Love" 
    ...
    && activePole != "Steam Powered Hydro-pole" 
    && activePole !=  "Sonar Pulverizer" 
    && activePole !=  "not_set")
    {
    ...
    if (island == "Sans Culpra" 
    && activePole != "Spear Gun" 
    && activePole != "Pneumatic Spear" 
    && activePole != "not_set")
    {
    ...
    if (island != "Sans Culpra" && island != "Sig" 
    && (activePole == "Spear Gun" 
    || activePole == "Pneumatic Spear") 
    && (activePole != "not_set"))
    {
    ...
    // SPECIFIC POLES DON'T WORK OUTSIDE OF SIG'S
    if (island != "Sig" 
    && (activePole == "Planktonite" 
    || activePole == "Algaenite") 
    && (activePole != "not_set"))
    {
    ...
    if ((island != "Glacier Bay" 
    && island != "San Digloo" 
    && island != "Lake Freezberg" 
    && island != "Snowpeak River" 
    && island != "Snowpeak Summit")
    && (activePole == "Evil Obliterator" 
    || activePole == "Holy Liberator") 
    && activePole != "not_set")
    {
    ...
    if ((island == "Devil" 
    || island == "Deadwater")
    && (activePole != "Pyratic Plunder" 
    && activePole != "Royal Rescuer") 
    && activePole != "not_set")
    {
...
{ 
  ...
  if (needChum && (window.location.href == "http://apps.facebook.com/fishwrangler/customize/chum" || window.location.href == "http://apps.facebook.com/fishwrangler/earl-lavashack"))
  {
    ...
    if(inputs)
    {
      for(i = 0; i < inputs.length; i++)
      { 
        if (island == ("WaterPort" || "Fishertonville"))
        {
        ...
        else if (island == ("Glacier Bay" || "San Digloo" || "Lake Freezberg" || "Snowpeak River" || "Snowpeak Summit"))
        {
        ...
        if(island == ("Devil" || "Deadwater"))
        {
          if (GM_getValue('Are you Evil?') == true)
          {
            ...
            {
              inputs[i].value = 50
              var idDiamond = (forms[2].id)
              setTimeout(function() {document.getElementById(idDiamond).submit();}, 5000);
            }
          }
          else if (GM_getValue('Are you Evil?') == false)
          {
            ...
            {
              inputs[i].value = 50
              var idGold = (forms[1].id)
              setTimeout(function() {document.getElementById(idGold).submit();}, 5000);
            }
          }
        }
      }
...
{	
	...
	if(window.location.href == "http://apps.facebook.com/fishwrangler/start" && document.body.innerHTML.indexOf("hsuccess") != -1) 
	{GM_setValue("lastfish", String(timeNow))}
	
	var timeAgo = new Date(GM_getValue("lastfish", new Date(timeNow.getTime() - 10*60*100)));
	
	if((timeNow - timeAgo) > 900000 && GM_getValue("Auto Fish", true)) 
	{	document.title = "Fish NOW!"
		window.setTimeout(function () { window.location.href = "http://apps.facebook.com/fishwrangler/start" }, 5000);
	} else {
		timervalue = 900000 - (timeNow - timeAgo)
...
function FormatCountDown(time)
{
	...
	if (min > 0) {appearance = min + " m " + secs + " s";}
	else {appearance = secs + " s";}
	
	remaining_time=null;min=null;secs=null;
	return appearance;
	
}

function fishWhenNeeded()
{
	if (timeoutvalue > 0 )
	{   //document.title ="Active - Next trip at " + nextAvailableTime(timeoutvalue);
		document.title = "Active - Next trip in: " + FormatCountDown((timeoutvalue));
        timeoutvalue = timeoutvalue - 1000;
		window.setTimeout(function () { (fishWhenNeeded)() }, 1000);
	}
	else if (timeoutvalue <= 0)
	{
		document.title = "Fishing Now!";
...
{ 
  ...
  { document.title = 'Oxygen is out, but you can buy more in this location!'
  ...
  else if (window.location.href == "http://www.facebook.com/common/error.html")
  {
    ...
    window.setTimeout(function () { window.location.href = "http://apps.facebook.com/fishwrangler/my" }, 5000);
  }
  ...
  {
    setTimeout(function() {window.location.href = 'http://apps.facebook.com/fishwrangler/my';}, ((Math.round(Math.random() * 5) + 1) * 1000));
  }
  else if (window.location.href == "http://apps.facebook.com/fishwrangler/raffle?show-me-the-rlc#rafflina-bumbalina-gimme-da-goods")
  {
    ...
    window.setTimeout(function () { window.location.href = "http://apps.facebook.com/fishwrangler/my" }, 15000);
  }
  ...
  { 
	  ...
	  { 
		  window.setTimeout(function() { if(confirm("It's time fish!\nDo you want to fish now?\n\nNOTE: By clicking OK, you will be redirected, so please click cancel if you need to finish something on the page.")) window.location.href = "http://apps.facebook.com/fishwrangler/start";}, (timervalue*1000))
	  } else {
	  fishWhenNeeded();
	  }
}
}/*
  else if (timeoutvalue == -1 && ((document.body.innerHTML.indexOf('Go Fishing Now!') != -1) || document.body.innerHTML.indexOf('Last <a class="ul"') != -1 || (document.body.innerHTML.indexOf('win-500-rlc.gif') != -1)) && (safeToFish))
  {
    document.title ="Fish NOW!";
    if (GM_getValue('Auto Fish', true))
    {
