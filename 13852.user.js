...
Binsearch.info Easy Click - (c) 2006, 2007 J.Q. la Poutre
...
function bs_rowsClickable() {
	...
	var theArray = document.getElementById('r2')
	if (theArray) {
		var rows = document.getElementById('r2').rows
...
//modifiy for adding labels on radio buttons
var inputs = document.getElementsByTagName('input');
var servInp = [];
for (var i=0; i<inputs.length; i++) {
	var inp = inputs[i];
	if (inp.name == 'server') break;
}
...
for (var i=0; i<p.childNodes.length; i++) {
	...
	if (node.nodeName == 'INPUT') {
		var nodeText = p.childNodes[i+1];
		var label = document.createElement('label');
		label.innerHTML = nodeText.nodeValue;
		label.htmlFor = 'BSServer'+node.value;
		label.style.cursor = 'pointer';
		node.id = label.htmlFor;
		node.style.verticalAlign = 'middle';
		p.insertBefore(label, nodeText);
		p.removeChild(nodeText);
	}
}
// end user script