...
/* farm */				"50": {"w": [ 50, 200, 400, 1400, 3500, 6000, 10000, 16000, 25000, 38000 ], "s": [ 0, 0, 200, 600, 1500, 3000, 5000, 8000, 13000, 20000 ], "th": 3},
		 ...
		 //try {
					...
					webfrontend.data.City.getInstance().addListener("changeVersion", function() { window.setTimeout(function(){LT.main.getCity()}, 250);}, this.layoutWindow);
					this.app.visMain.addListener("changeMapLoaded", function(){this.tabView.setSelection([this.tabView.getChildren()[0]]); this.win.close();}, this.layoutWindow);
					LT.main.getCity();
		 ...
		 // }
				...
				getValidBuildings: function(_ids) {
					...
					_arr = new Array();
					_wallIn = false;
					for (k=0; k<LT.city.length; k++) {
						if (_ids.indexOf("|" + LT.city[k][2] + "|") != -1 && LT.city[k][1] < 10 && LT.city[k][1] > -1) {
							if ((_wallIn && LT.city[k][2] == 23) || this.self(arguments).bd[LT.city[k][2]].th > th)
								continue;
							_arr.push(LT.city[k]);
							if (LT.city[k][2] == 23)
								_wallIn = true;
				...
				gettingCity: false,
				getCity : function() {
					if (LT.main.gettingCity)
					{
						return;
					}
					LT.main.gettingCity = true;
					var app = qx.core.Init.getApplication();
					if (app.visMain.mapmode != "c") 
					{
						LT.main.gettingCity = false;
						return;
					}
					_cgi = webfrontend.data.City.getInstance();
					if (LT.cityId != _cgi.getId()) LT.main.layoutWindow.removeObjects();
					_cells = app.visMain.cells;
					if (!_cells[0]) {
						LT.main.gettingCity = false;
					...
					LT.city = new Array(441);
					_empty = [0, 1, 19, 20, 21, 41, 399, 419, 420, 421, 439, 440];
					_water = [352, 353, 373, 374, 375, 395, 396, 397, 398, 417, 418, 438];

					for (i=0; i<LT.city.length; i++) LT.city[i] = null;

					for (i=0; i<_empty.length; i++)	LT.city[_empty[i]] = [-1, -1, -1]; // [buildingID/placeID, buildingLvl, buildingType]

					if (waterCity) {
						for (i=0; i<_water.length; i++) LT.city[_water[i]] = [-1, -1, -2];
					}

					try {
						for (i=0, c=0; i<_se.length; i++) {
							while(LT.city[c] != null) c++;
							if (_se[i][0].getResType != undefined)
								LT.city[c] = [_se[i][0].getId(), 0, _se[i][0].getResType()+900]; // resource node
							else if (_se[i][0].getType != undefined) {
								if (_se[i][0].getLevel != undefined) // building
									LT.city[c] = [_se[i][0].getId(), _se[i][0].getLevel()+LT.main.checkBuilding(_se[i][0].getId()), _se[i][0].getType()];
								else
									LT.city[c] = [_se[i][0].getId(), _cgi.getWallLevel()+LT.main.checkBuilding("wall"), _se[i][0].getType()]; // wall
								...
								if (_se[i][0].drawNode != null) {
									if (_se[i][0].drawNode.image != undefined) {
										if (_se[i][0].drawNode.image.indexOf("tower") != -1) {
											LT.city[c] = [_se[i][0].getPlaceId(), 0, 99]; // tower place
										} else {
											LT.city[c] = [_se[i][0].getPlaceId(), 0, 98]; // empty, can be corn field
										}
									} else if (_se[i][0].drawNode.basename == "EffectNode") {
										LT.city[c] = [_se[i][0].getPlaceId(), 0, 99]; // ??? bottom left tower in water city
									}
								} else {
									if (waterCity && /\b(331|332|351|354|372|376|394|416)\b/.test(c)) {
										LT.city[c] = [_se[i][0].getPlaceId(), 0, 97]; // water building place
									} else {
										LT.city[c] = [_se[i][0].getPlaceId(), 0, 98]; // empty
									}
								}
							}
						}

						for (i=0; i<LT.city.length; i++) {
							if (LT.city[i] == null) {
								LT.city = new Array(441);
								LT.main.gettingCity = false;
								window.setTimeout(function(){LT.main.getCity()}, 1000);
						...
						if (qx.lang.Object.isEmpty(app.visMain.selectableEntities) || LT.main.cityId != app.visMain.mapid) {
							LT.main.gettingCity = false;
							window.setTimeout(function(){LT.main.getCity()}, 1000);
							return;
						}
						var c = LT.city;
						var cId = LT.main.cityId;
						ss = null;
						LT.main.layoutWindow.generateSharestring();
						if (LT.main.layoutWindow.cityLayouts[LT.main.layoutWindow.srvName].hasOwnProperty(cId))
							ss = LT.main.layoutWindow.cityLayouts[LT.main.layoutWindow.srvName][cId];
						if (ss == null || ss == undefined) {
							...
							if (ss != null) {
								...
								if (ss.indexOf("map=") != -1) {
									...
									ss = LT.main.layoutWindow.convertToSharestring(ss);
								}
							}
						}
						if (ss == undefined || ss == null || app.visMain.mapmode != "c") 
						{
							LT.main.gettingCity = false;
							return;
						}
						
						LT.main.layoutWindow.removeObjects();
						LT.main.layoutWindow.oObjs = new Array();
						try{
						for (i=0; i<ss.length; i++) {
							if (/T|#|W|Q|F|I/.test(ss.charAt(i))) continue;
							id = 0;
							if (!/\;|\:|\,|\.|\-|\_/.test(ss.charAt(i))) {
								id = window.louTweak.layoutWindow.ssToId[ss.charAt(i)];
							...
							LT.main.layoutWindow.oObjs.push(new window.louTweak.overlayObject(app.visMain, 163+128*(i%21), 67+80*Math.floor(i/21), id));
						}
						...
						_palaceLvl = 0;
						for (i=0; i<LT.city.length; i++) {
							if (LT.city[i][1] > -1 && LT.city[i][1] < 10 && !/\b(-1|-2|23|27|28|29|30|60|61|62|63|900|901|902|903|904|905|906|907|97|98|99)\b/.test(LT.city[i][2]) && !(LT.city[i][2] > 52 && LT.city[i][2] < 60)) _upCount++;
							else if (LT.city[i][2] == 23) _wallLvl = LT.city[i][1];
							else if (LT.city[i][2] > 51 && LT.city[i][2] < 60) _palaceLvl = LT.city[i][1];
						}
						if (_wallLvl < 10) _upCount++;
						if (_palaceLvl > 0 && _palaceLvl < 10) _upCount++;
						_cba = _cgi.getBuildingLimit() - _cgi.getBuildingCount();
						var bQc = app.getCityInfoView().buildingQueue;
						if (bQc.buildingSlotsTooltip.getLabel().indexOf("CM_cUp") == -1) {
							bQc.buildingSlotsValue.setValue(_cba + " (" + _upCount + ")");
							_ctxt = '</tr><tr><td id="CM_cUp">' + "Buildings available for upgrade:" + '</td><td>' + _upCount + '</td></tr></table>';
							_ttxt = bQc.buildingSlotsTooltip.getLabel().replace("</tr></table>", _ctxt);
							bQc.buildingSlotsTooltip.setLabel(_ttxt);
						}
					} catch (e) {console.error(e.name + ":" + e.message); }
					LT.main.gettingCity = false;
				},
				...
				*/
				...
				getIndex: function(_buildingId) {
					this.getCity();
					for (i=0; i<LT.city.length; i++) {
						if (LT.city[i][0] == _buildingId) return i;
					}
					return -1;
				},
		...
		try {
				...
				*/
				...
				applyLayout: function() {
					...
					this.cityLayouts[this.srvName][cId] = txt;
					this.cityLayouts[this.srvName][cId + "o"] = o;
					this.saveCityLayouts();
					setTimeout(function() {
						try {
							LT.main.getCity();
				...
				saveToCityNotes: function(c, s) {
					...
					var ta = this.findObject(qx.core.Init.getApplication().getCityNotesWidget(), qx.ui.form.TextArea, false);
					if (ta && typeof ta.setValue == "function")
					{
						ta.setValue(cnt);
					}
