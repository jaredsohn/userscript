// ==UserScript==
// @name           Create Vampire wars invites.
// @namespace      Vampire wars friends.
// @description    Automatically invites people to your clan that you find on the fight page. Frisno altered the  the original code  from Andy Calderbanks  "Create dragon wars invites" http://userscripts.org/scripts/show/31912
...
if (document.location.href.indexOf('fight') == -1) {
	...
	var vars = false;
	var members = document.getElementsByXPath("//a[contains(@href,'/vampiresgame/stats.php?user=')]");
	...
	if (vars) {
		...
		GM_xmlhttpRequest({
			method: "POST",
			url: 'http://apps.'+((newlayout)?'new.':'')+'facebook.com/vampiresgame/recruit.php?action=create',
