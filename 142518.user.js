...
// @version        1.1.0
// @author         eCitizen Scyld & hesar - mod by S, edited by J
// @namespace      eCitizenScyld
// @description    updated with Q6 support by hesar - mod by S, added used Q7 weapon in battle window calculated with max current damage (only finished kills without NE) - edited by J, BH & CH top 5 fixed; energy fight max fixed - J
...
// Upewniamy siê czy wymagana eRepowa jQuery jest za³adowana
function GM_wait() {
...
function letsJQuery() {
	...
	if (subURL.match(/^citizen\/profile\/(\d+)$/)) {
		...
		$j('div.citizen_military:last').after(
            ...
            '<br />q6: ' + dmgCalc(rank[mRank], str, 120, 1, 1) + ' q7: ' + dmgCalc(rank[mRank], str, 200, 1, 1) +
			'</small></div></div>' +
			...
			'<option value="80">Q4</option><option value="100">Q5</option><option value="120" selected="selected">Q6</option>' +
            '<option value="200" selected="selected">Q7</option>' +
			'</select>&nbsp;&nbsp;&nbsp;&nbsp;# of fights: <input id="dmgFights" name="dmgFights" value="144" size="4" maxlength="4" />' +
			'&nbsp;<button id="dmgCalc">go!</button>' +
			'</span></h3><div class="citizen_military" style="margin-bottom:2px">' +
			'<div id="dmgResults"><strong>144 fights: </strong>' +
			'<h4 style="margin-left:37px">' + dmgCalc(rank[mRank], str, 200, 144, 1) + '</h4>' +
			'<div class="stat"><small>With NE bonus: <strong><span style="font-size:12px;margin-right:15px">' +
			dmgCalc(rank[mRank], str, 200, 144, 1.1) + '</span></strong></small></div></div></div>'
		);
	...
	var styles =
		'<style type="text/css"> ' +
		'div.BHTable { width: 200px; height: 85px; border: 1px solid #777; border-radius: 5px; padding: 6px; background-color: #333; } ' +
	...
	var content = $j('div#pvp_battle_area');
        ...
        var strength = $j('#fighter_skill').text().trim();
            strength = parseFloat(str_replace(strength, ',', ''));
        var rankString = $j('#rank_icon').attr('title').split(':')[1].trim();
        var militaryRank = rank[rankString];
        
        var max_hit = dmgCalc(militaryRank, strength, 200, 1, 1);
        $j('#total_damage').prepend('<tr><td><div>'+
        '<div><b >&nbsp;</b><small>Hit / Q7:</small>'+
        '<p id="pCurrentHit">'+ max_hit +' / ' + (current_damage/(max_hit*10)).toFixed(2) +'</p></div></td></tr>');
        $j('#total_damage').css('margin-top','-100px'); 
	...
	content.prepend(
		...
		'<div id="myOverBox" style="position: absolute; top: 200px; left: 273px; width: 200px; opacity: 0.8;"></div>'                
	);
	content.append(
	...
	$j('div#enemy_defeated').before(
		...
		'<big>MultiHIT:&nbsp;</big>' +
		'<input id="multihit_count" type="text" size="3" maxlength="3" value="74" /><button id="multihit_start">HIT!</button>' +
		...
		'</div><br />' +
		'<fieldset id="BLOG"><legend>v1.0.8"moded" BATTLE LOG:&nbsp;&nbsp;<button id="battlelog_clear">clear</button></legend><div id="battlelog"></div></fieldset>'
	);
	...
	$j("body").ajaxSuccess(function(e, res, opt) {
                ...
                var top5CHRightSide = 0;
                var top5CHLeftSideKills = 0;
                var top5CHRightSideKills = 0;
		if (opt.url.indexOf('/battle-stats/') > -1 && unsafeWindow.SERVER_DATA.onlySpectator != 1) {
			...
			for ( var i = 0; i < history.length; i++ ) {
				...
				top5HIST = top5HIST+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[history[i].citizen_id].id+'">'+bh.fightersData[history[i].citizen_id].name+'</a></td><td>'+history[i].kills+'</td><td><strong>'+history[i].damage+'</strong></td></tr>';
                        ...
                        top5HIST = top5HIST+'<tr style="color:yellow;"><td>Suma influ TOP5:</td><td>'+top5CHLeftSideKills+'</td><td>' + top5CHLeftSide + '</td></tr>';
		  histStats(top5HIST);
			...
			if (typeof bh['stats']['current'][zone][division][att] != 'undefined') {
				...
				for ( var i = 0; i < bh['stats']['current'][zone][division][att].length; i++ ) {
					...
					top5ABH = top5ABH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['stats']['current'][zone][division][att][i].citizen_id].id+'">'+bh.fightersData[bh['stats']['current'][zone][division][att][i].citizen_id].name+'</a></td><td>'+bh['stats']['current'][zone][division][att][i].kills+'</td><td><strong>'+bh['stats']['current'][zone][division][att][i].damage+'</strong></td></tr>';
                        ...
                        top5ABH = top5ABH+'<tr style="color:yellow;"><td>Suma influ TOP5:</td><td>'+top5LeftSideKills+'</td><td>' + top5LeftSide + '</td></tr>';
		  if (typeof bh['stats']['current'][zone][division][def] != 'undefined') {
				...
				for ( var i = 0; i < bh['stats']['current'][zone][division][def].length; i++ ) {
					...
					top5DBH = top5DBH+'<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/'+bh.fightersData[bh['stats']['current'][zone][division][def][i].citizen_id].id+'">'+bh.fightersData[bh['stats']['current'][zone][division][def][i].citizen_id].name+'</a></td><td>'+bh['stats']['current'][zone][division][def][i].kills+'</td><td><strong>'+bh['stats']['current'][zone][division][def][i].damage+'</strong></td></tr>';
                        ...
                        top5DBH = top5DBH+'<tr style="color:yellow;"><td>Suma influ TOP5:</td><td>'+top5RightSideKills+'</td><td>' + top5RightSide + '</td></tr>';
			setLink2BH(aBH, dBH);
