...
// -----------------------------------------------------------------------------
//              Copyright (c) by Xxxx Yyyy, http://xxxx.yy/
...
(function(){
	...
	var lang = {
		'en': {
			'settings': 'Rekrutacja dla grupy',
			'troupsize': 'Ilość jednostek',
			'troupsizedesc': 'Model wojska w tej grupie wedłóg którego będzie przebiegała rekrutacja.',
			'openend': 'Do oporu:',
			'openenddesc': 'po wyprodykowaniu zleconych będzie produkcja do wyczerpania miejsca w zagrodzie.',
			'farmfree': 'Pozostawić miejsca w zagrodzie:',
			'farmfreedesc': 'Ile ma zostać wolnego miejsca w zagrodzie (np. 100 na Ulunga ;).',
			'resfree': 'Pozostawić surowców:',
			'resfreedesc': 'Ile ma zostać surowców.',
			'save': 'Zapisz',
			'fill': 'Wprowadź',
			'recruit': 'Rekrutuj',
			'running': 'czekaj...',
			'troupneeds': '(Rekrutowane wojsko potrzebuje %1 miejsc w zagrodzie)',
			'filled': 'Wprowadzono: %1 w %2 wioskach'
		},
		'de': {
	...
	var set_cookie = function(arr) {
		document.cookie = 'massrecruiting=' + escape(_JSON.stringify(arr)) + '; expires=' + (new Date(2036, 1, 1)).toGMTString() + ';';
	};
	var get_cookie = function() {
		var massrecruiting = /massrecruiting=(.*?)(?:;|$)/.exec(document.cookie);
		if(massrecruiting) {
			return _JSON.parse(unescape(massrecruiting[1]));
		}
		return [];
	};
	...
	var groups = [];
	var selectedgroup = '';
	var grps = $x('//td/a[contains(@href,"&screen=train&mode=mass&group=") and not(contains(@href,"&page=")) and starts-with(.," [")]');
	if(grps.length > 0) {
	...
	var recform = $x('//form[contains(@action,"screen=train&mode=mass&action=train_mass&h=")]');
	if(recform.length == 0)
		return;
	
	// get available units
	var units = [];
	var u = $x('table[1]/tbody/tr[1]/th/img[contains(@src,"unit_")]', recform[0]);
	...
	btn.addEventListener('click', function(evt){
		...
		var currentsettings = getCurrentSettings(selectedgroup);
		var rows = $x('//form[contains(@action,"screen=train&mode=mass&action=train_mass&h=")]/table[1]/tbody/tr/td/parent::tr');
		var builtsum = [];
		for(var i = 0; i < units.length; i++) {
			builtsum[i] = 0;
		}
