// ==UserScript==
// @name           MAY KL Script for Blog V.1.0
// @namespace      http://indonesianhottrix.blogspot.com
// @description    Script emot for Blog by May (armamei)
// @include        http://indonesianhottrix.blogspot.com/*
// @include		   http://http://indonesianhottrix.co.nr/*
// ==/UserScript==
//
//      Clubbing Kapanlagi Emoticons Relased
//      
//      Copyright 2011 MAY <armamei@yahoo.com>

String.prototype.isPrefixOf = function (str, from) {
    if (arguments.length < 2) from = 0;
    else from = parseInt(from);

    if (from < 0 || from >= str.length) return false;

    if (from + this.length > str.length) return false;

    for (var i = 0; i < this.length; i++)
    if (this.charCodeAt(i) != str.charCodeAt(from + i)) return false;
...
function f(o1, o2) {
    if (o1.emoticon.isPrefixOf(o2.emoticon)) return 1;

    if (o1.emoticon > o2.emoticon) return 1;
...
function replaceTextNode(textNode, sortedEmoticonSet) {
    ...
    while (i < content.length) {
        ...
        if (sortedEmoticonSet[firstChar]) for (j = 0; j < sortedEmoticonSet[firstChar].length; j++)
        if (sortedEmoticonSet[firstChar][j].emoticon.length && sortedEmoticonSet[firstChar][j].emoticon.isPrefixOf(content, i)) {
            if (currentStopPosition < i) htmls.push(document.createTextNode(content.substr(currentStopPosition, i - currentStopPosition)))
    ...
    if (currentStopPosition > 0 && currentStopPosition < content.length - 1) htmls.push(document.createTextNode(content.substr(currentStopPosition)));
...
function replaceElement(element, emos) {
    ...
    for (i = 0; i < pathResult.snapshotLength; i++) {
        ...
        if (tNode.parentNode) {
            ...
            if (span) tNode.parentNode.replaceChild(span, tNode);
        }
    }
}