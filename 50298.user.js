...
var autoPlayInterval = getValue("config_autoPlayInterval__", 3000);
...
var autoPlayQuestLink = $("#blinker.smarttip");
var autoPlayQuestLinkActived = getValue("AutoPlayQuestLinkActived__", 0);
...
function autoPlayOn()
{
    if( autoPlay )
    {
        ...
        // verifica se tem direção para quest
        else if( autoPlayQuestLink.length > 0 )
        {
            oLog.innerHTML = "AutoPlay ON (Quest)<br><span class='quest'></span> Direcionando para quest.<br>";
            autoPlayActiveUrl = autoPlayQuestLink[0].href;
            oLog.innerHTML += autoPlayActiveUrl;
            
            setValue("AutoPlayQuestLinkActived__",1);
        }
            
        // verifica se está nessa pagina por causa de uma ativação de quest
        else if ( autoPlayQuestLinkActived == 1 )
        {
            setValue("AutoPlayQuestLinkActived__",2);
            alert("Mapa com quest.\n\nLocalize e execute a quest.");
            autoPlay = false;
        }
...
function init()
{
    ...
    if( window.location.href.search("bar") > 0 )
    {
      ...
      {
        printLogMessage("Configurando Drink");
      ...
      // Se não tivr, etão inclue os link de 'use'
      else
      {
        var divItens = $("div.inventory_slotsw div.json[m=n]");
        printLogMessage("Colocando links nos Drink ... analisando " + divItens.length + " itens");
          
        divItens.each(function() {
          var data = $.parseJSON($(this).attr("rel"));
          if( data.classs == "drink" )
          {            
              codigoDrink = data.id;
              $(this).append("<span style='position: absolute; bottom: 0; left: 0;'>[<a href='" + window.location.href + "&configDrink=" + codigoDrink + "'>use</a>]</span>");
          }
        });
      }
    ...
    if ( autoPlayQuestLinkActived == 2 )
    {
        // se o QuestLink esta com valor 2 significa que pode reativar o autoPlay
        autoPlay = true;
    }
    else if( statusAutoPlay && statusAutoPlay.length > 0 )
    {
