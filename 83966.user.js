...
// @name           Оповещатель
// @namespace      none
// @description    igorecheck edit.
// @author         Peety (thx Heinzel) Aenderung DS6.0 by Hypix
// @version        1.90.00 201003061500
...
function check_angriff() {
	...
	try {	// auf Angriffssymbol pruefen
        awert = Body.match(/graphic\/unit\/att\.png[\?*][0-9*]"[^>]*> \((\d+)\)/)[1];  // THX Hypix!

    // bis Version 5.5 awert = Body.match(/graphic\/unit\/att\.png[?*][0-9*]" alt=""> \((\d+)\)/)[1];
	} catch(e) {awert = '';}
