// ==UserScript==
// @name Travian Gestione Risorse T3.5
// @namespace http://userscripts.org/users/60057 / http://userscripts.org/users/59418
// @author http://traviantrucchi.altervista.org (Dark Simon - Matt93) / http://www.travianutility.com
...
" Добив наресурси.","Neutral balance." , "� азширете склада" , "� азширете хамбара"];
break;
default:
langfile[lang]=['Needed for upgrading:','Overflow', 'Depleted','Resource Meters','Resource Bars',
' Clock' ,'Enough resources today at ',"Enough resources tomorrow at ", "Enough resources in ",
...
function addElement(theElement) { // adds an element to the page either by the center object lcontentlc or appends to html
var htmldoc=returnObjById("content");

if(!htmldoc) {
htmldoc=document.getElementsByTagName('body')[0];
if(!htmldoc) returnObjById("content");
...
var need = document.getElementsByClassName('required');
var end = need.length;
if(end==0) return;

for(var i=0; i<end; i++){
//var current=resourceBlock[i].textContent.split(" | ");
var current=getResourcePerBuild(i);
var stuff=[];
var maxtime=0;
for(var j=0; j<4; j++){
...
document.getElementsByClassName('required')[i].appendChild( holdtable );
}
}