// I just translated this script into Hebrew, all credit goes to the author below.

// ==UserScript==
// @name           Ikariam Building List
// @namespace      http://www.dokislogeek.net
// @description    Fast Buildings info and access in city view, Ressources Level and Fast Inactive info on Island view (Hebrew Translation)
...
if( document.getElementById('island') ){
	...
	if (leftbox) {
		...
		rdiv.innerHTML = '<div class="dynamic"> \
				<h3 class="header">משאבים</h3> \
				<div class="content"> \
					<ul id="reslvl"> \
						<li> \
							<span>'+ woodlevel + ' </span>' + woodname + ':</li> \
						<li> \
							<span>' + ressourcelevel + ' </span>' + ressourcename +':	</li> \
					</ul> \
				</div> \
	...
	if (leftbox) {
		...
		for ( i = 0; i < inactives.length; i++ ){
			idivcontent += '<li><a href="#" onclick="' + inactives[i]['onclick'] + '" onmouseover="showCity(\'' + inactives[i]['citypos'] + '\');" onmouseout="hideCity(\'' + inactives[i]['citypos'] + '\');"><span>' + inactives[i]['citylvl'] + '&nbsp;(' + inactives[i]['owner'] + ')</span> ' + inactives[i]['city'] +'</a></li>';
		}
		...
		if (bl >= lmax[tclass[torder[i]]]) {
			...
			titles += '<li class="buildmax"> \
					<a href="' + burl + '" onmouseover="showBuilding(this);" onmouseout="hideBuilding(this);"> \
						<span class="buildname">' + b[1] + '</span> \
						<span style="color:grey;">' +b[0]+':</span> \
					</a> \
				</li>';
